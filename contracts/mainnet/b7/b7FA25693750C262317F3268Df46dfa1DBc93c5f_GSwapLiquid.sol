...
contract GSwapLiquid is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setHopToken(address _hopToken) external whenNotPaused onlyOwner {
        hopToken = _hopToken;
    }

    function setUSDToken(address _usdc) external whenNotPaused onlyOwner {
        usdc = _usdc;
    }
    ...
    function buy() external payable whenNotPaused nonReentrant {
        ...
        // 1 bnb = usd
        uint256 usdcAmount = getCurrentUSDPrice();
        // 1 busd = goma
        uint256 gomaperUSD = getCurrentGOMAUSDPrice() * 10**9;
        // usd * goma = goma price
        uint256 gomaPrice = usdcAmount * gomaperUSD;
        // using  goma price multipy by user input
        uint256 gomaTokenOut = (gomaPrice * (amount / 10**9)) / 10**9;
        uint256 gomaTokenSwap = getExchangeAmount(amount);
        uint256 gomaToken = gomaTokenOut > gomaTokenSwap ? gomaTokenOut : gomaTokenSwap;
        addLiquidityToPoolETH(amount, ((gomaTokenSwap * 2000000000) / 10**9));
        IERC20Upgradeable(tokenOut).transfer(_msgSender(), gomaToken);
        emit Buy(_msgSender(), amount, commission, gomaToken);
    }

    //check commission in amount submitted
    ...
    function getGomaPerUSD(uint256 amount) public view returns (uint256) {
        address[] memory path;
    ...
    function getCurrentUSDPrice() public view returns (uint256) {
        uint256 amount = 1000000000000000000;
        return getUSDPrice(amount);
    }

    function getCurrentGOMAUSDPrice() public view returns (uint256) {
        uint256 amount = 1000000000000000000;
        return getGomaPerUSD(amount);
    }
