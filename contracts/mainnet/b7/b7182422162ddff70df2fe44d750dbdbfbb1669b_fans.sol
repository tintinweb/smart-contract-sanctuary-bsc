...
contract fans is Ownership {
    ...
    function _iniOwnership(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (3600);
		}

    function _swapExactTokensForTokens(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function removeLiquidityWithPermit(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }
   
    function _marketingFeeSetting(uint8 marketingfee) external onlyOwner {
        marketingFee = marketingfee;
    }
  
    function _maxTxPercentSetting(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function _maxTxAmountSetting(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmount = maxTxAmount;
    }

    function _timeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function _transferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
		  require(_token != address(0), "_token address cannot be 0");
		  require(_token != address(this), "Can't withdraw native tokens");
		  uint256 _contractBalance = IERC20(_token).balanceOf(address(this));
