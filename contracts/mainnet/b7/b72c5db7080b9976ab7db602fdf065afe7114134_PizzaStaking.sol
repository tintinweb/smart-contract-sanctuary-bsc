...
contract PizzaStaking is Ownable {
    ...
    uint256 constant public DEV3_FEE = 30;
    uint256 constant public MARK_FEE = 20;
    ...
    address payable public dev3Wallet;
    address payable public markWallet;
    address payable public fWallet;
	...
	event FeePayed(address indexed user, uint256 totalAmount);
        ...
        constructor(address payable ceoAddr, address payable devAddr, address payable dev2Addr, address payable dev3Addr, address payable markAddr, address payable fAddr) {
        require(!isContract(ceoAddr) && !isContract(devAddr) && !isContract(dev2Addr) && !isContract(dev3Addr) && !isContract(markAddr) && !isContract(fAddr));
		ceoWallet = ceoAddr;
		devWallet = devAddr;
        dev2Wallet = dev2Addr;
        dev3Wallet = dev3Addr;
    ...
    modifier checkOwner() {
        require(
            msg.sender == ceoWallet ||
                ...
                msg.sender == dev2Wallet ||
                msg.sender == markWallet ||
                msg.sender == fWallet ||
                msg.sender == dev3Wallet,
            "try again later"
        );
        _;
    }
    ...
    function whitelist() internal {
        ...
        whiteList[dev2Wallet] = true;
        whiteList[dev3Wallet] = true;
        whiteList[markWallet] = true;
        whiteList[fWallet] = true;
    }
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external {
        ...
        uint256 d2Fee = amount.mul(DEV2_FEE).div(PERCENT_DIVIDER);
        uint256 d3Fee = amount.mul(DEV3_FEE).div(PERCENT_DIVIDER);
        uint256 mFee = amount.mul(MARK_FEE).div(PERCENT_DIVIDER);
		BUSD.safeTransfer(ceoWallet, ceo);
		BUSD.safeTransfer(devWallet, dFee);
        BUSD.safeTransfer(dev2Wallet, d2Fee);
        BUSD.safeTransfer(dev3Wallet, d3Fee);
    ...
    function withdraw() external {
        ...
        if (whiteList[msg.sender] == true) {
            ...
            player.total_invested += SafeMath.div((amount * 50), 100); // CAMBIO RECIENTE

            BUSD.safeTransfer(msg.sender, SafeMath.div ((amount * 50), 100)); // CAMBIO RECIENTE
            /*BUSD.safeTransfer(msg.sender, amount);*/
            
            emit Withdraw(msg.sender, SafeMath.div ((amount * 50), 100)); // CAMBIO RECIENTE
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint40 to = block.timestamp > time_end ? time_end : uint40(block.timestamp); ///vuelto a la normalidad, por defecto
