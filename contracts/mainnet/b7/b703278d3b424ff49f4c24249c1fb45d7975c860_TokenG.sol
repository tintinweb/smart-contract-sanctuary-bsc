...
contract TokenG is Ownable, ERC20 {
	...
	uint256 public maxTxAmount = initialSupply * 125 / 10000;

	bool private _swapping;
	uint256 public minimumTokensBeforeSwap = initialSupply * 25 / 100000;
	
    address public FUTC = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //0x049d68029688eAbF473097a2fC38ef61633A3C7A;
	...
	constructor() ERC20(_name, _symbol) {
		...
		IRouter _uniswapV2Router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0xF491e7B69E4244ad4002BC14e878a34207E38c29
	...
	function deactivateTrading() external onlyOwner {
        require(isTradingEnabled, "GroveToken: Trading is already deactivated");
    ...
    function blockAccount(address account) external onlyOwner {
        require(!_isBlocked[account], "GroveToken: Account is already blocked");
        if (_isLaunched) {
            require((block.timestamp - _launchStartTimestamp) < 172800, "GroveToken: Time to block accounts has expired");
        }
        _isBlocked[account] = true;
        emit BlockedAccountChange(account, true);
    }
    function unblockAccount(address account) external onlyOwner {
        require(_isBlocked[account], "GroveToken: Account is not blcoked");
	...
	function claim() external {
		bool success = dividendTracker.processAccount(payable(msg.sender), false);
        require(success, "GroveToken: Claim failed");
    }
	...
	function _transfer(
		...
		uint256 amount
			...
			if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
                require(!_isBlocked[to], "GroveToken: Account is blocked");
                require(!_isBlocked[from], "GroveToken: Account is blocked");
			...
			_adjustTaxes(automatedMarketMakerPairs[from]);
	...
	function _adjustTaxes(bool isBuyFromLp) private {
		...
		if (isBuyFromLp) {	
			if (block.timestamp - _launchBlockNumber <= 5) {
                _liquidityFee = 100;
                _marketingFee = 0;
                _buyBackFee = 0;
                _burnFee = 0;
	...
	function _swapAndLiquify() private {
		...
		uint256 totalFTMFee = _totalFee - ((_liquidityFee / 2) + _burnFee + _holdersFee);
		...
		uint256 amountFTMBuyBack = FTMBalanceAfterSwap - (amountFTMLiquidity + amountFTMMarketing);

		Address.sendValue(payable(buyBackWallet),amountFTMBuyBack);

        _swapFTMForCustomToken(amountFTMMarketing, FUTC, marketingWallet);
    ...
    function _swapFTMForCustomToken(uint256 ftmAmount, address token, address wallet) private {
        address[] memory path = new address[](2);
...
contract GroveTokenDividendTracker is DividendPayingToken {
	...
	event ExcludeFromDividends(address indexed account, bool indexed excluded);
	...
	function setRewardToken(address token) external onlyOwner {
        require(token != address(0), "GroveToken_Dividend_Tracker: Token is zero address");
	...
	function excludeFromDividends(address account, bool excluded) external onlyOwner {
        if (excluded) {
            ...
            uint256 tokenBalance = IERC20(owner()).balanceOf(account);
            if (tokenBalance > 0){
