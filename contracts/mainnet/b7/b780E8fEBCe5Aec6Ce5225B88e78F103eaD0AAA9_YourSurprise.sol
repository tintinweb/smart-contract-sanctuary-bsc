...
library Address{
 ...
 contract YourSurprise is Context, IBEP20, Ownable {
    ...
    function _takeMarketing(uint256 rPromoprizes, uint256 tPromoprizes) private {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rPromoprizes > 0 || s.tPromoprizes > 0) _takeMarketing(s.rPromoprizes, s.tPromoprizes);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        uint256 marketingAmt = unitBalance * 2 * temp.promoprizes;
        if(marketingAmt > 0){
            payable(promoprizesReceiver).sendValue(marketingAmt);
        }
        uint256 donationAmt = unitBalance * 2 * temp.dev;
        if(donationAmt > 0){
            payable(devReceiver).sendValue(donationAmt);
        }
    }
