...
contract RaidBasic is Initializable, Raid {
    ...
    function completeRaid(uint256 seed) public override restricted {
        ...
        /*for(uint i = 0; i < raiders.length; i++) {
            ...
            emit DckWinner(address(r.owner), payout);
        }*/
...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    IERC20 public dckToken;
    IPriceOracle public priceOracleDckPerUsd;
    IRandoms public randoms;

    function initialize(IERC20 _dckToken, Characters _characters, Weapons _weapons, IPriceOracle _priceOracleDckPerUsd, IRandoms _randoms) public initializer {
        ...
        weapons = _weapons;
        priceOracleDckPerUsd = _priceOracleDckPerUsd;
        randoms = _randoms;
    ...
    event FightOutcome(address indexed owner, uint256 indexed character, uint256 weapon, uint32 target, uint24 playerRoll, uint24 enemyRoll, uint16 xpGain, uint256 dckGain);

    function recoverDck(uint256 amount) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not admin");

        dckToken.transfer(msg.sender, amount);
    }
    ...
    function performFight(address user, uint256 char, uint256 wep, uint32 target) private {
        ...
        emit FightOutcome(characters.ownerOf(char), char, wep, target, playerRoll, monsterRoll, getXpGainForFight(char, wep, target), usdToDck(getTokenGainForFight(target)));
    }
    ...
    modifier requestPayFromPlayer(int128 usdAmount) {
        uint256 dckAmount = usdToDck(usdAmount);
        require(dckToken.balanceOf(msg.sender) >= dckAmount,
            string(abi.encodePacked("Not enough DCK! Need ",RandomUtil.uint2str(dckAmount))));
        _;
    }
    ...
    function _payContract(address playerAddress, int128 usdAmount) internal {
        _payContractConverted(playerAddress, usdToDck(usdAmount));
    }
    ...
    function _payPlayer(address playerAddress, int128 baseAmount) internal {
        _payPlayerConverted(playerAddress, usdToDck(baseAmount));
    }

    function _payPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        dckToken.approve(address(this), convertedAmount);
        dckToken.transferFrom(address(this), playerAddress, convertedAmount);
    }
    ...
    function usdToDck(int128 usdAmount) public view returns (uint256) {
        return usdAmount.mulu(priceOracleDckPerUsd.currentPrice());
    }
