...
contract Uprise is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    UpriseDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Uprise", "UP") {
        marketingWallet = payable(0x76B4CC0D439a7B76e17feA2Ffb468e486B49E7F7);
        devWallet = payable(0x76B4CC0D439a7B76e17feA2Ffb468e486B49E7F7);
        ...
        dividendTracker = new UpriseDividendTracker(
            payable(this),
            router,
...
contract UpriseDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        UpriseContract = Uprise(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Uprise_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Uprise_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Uprise contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(UpriseContract);

            uint256 prevBalance = UpriseContract.balanceOf(address(this));
            ...
            {
                uint256 received = UpriseContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    UpriseContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
