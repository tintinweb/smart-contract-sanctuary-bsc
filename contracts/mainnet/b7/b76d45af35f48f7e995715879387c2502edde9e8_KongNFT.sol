...
contract KongNFT is ERC721A
{
    using Strings for uint256;

    address public dev = 0xBb61125269A86b90b5E1e23547811A3fF04bD11D;
    address public kongStakeAddress;
    string public baseURI;
    string public unrevealedURI;
    uint256 public maxSupply;
    uint256 public maxMintAmount = 1;
    uint256 public maxMintPerTransaction = 1;
    uint256 public KongStakeInjection = 25; // 25% BUSD sent to Kong stake TVL
    ...
    constructor() ERC721A("Kong NFT", "KONG", maxMintAmount) {
        owner = msg.sender;
        maxSupply = 1000;
    }
    ...
    // Minting functions
    function mint(uint256 _mintCount, uint256 _amount) external {
        require(saleOpen || whitelistPresaleOpen, "mint not live");
        require(_amount >= getPrice(),"Less BUSD amount passed");
        require(busd.allowance(msg.sender, address(this)) >= _amount,"Not enough BUSD approved for transfer");
        uint256 kongStakeShare = (_amount * KongStakeInjection) / 100;
        uint256 devShare = (_amount * 75) / 100;
        bool success;
        success = busd.transferFrom(msg.sender, kongStakeAddress, kongStakeShare);
        require(success, "BUSD Transfer failed.");
        success = busd.transferFrom(msg.sender, dev, devShare);
        require(success, "BUSD Transfer failed.");
        address _to = msg.sender;
        uint256 minted = mintCount[_to];
        require(minted + _mintCount <= maxMintAmount, "mint over max");
        require(_mintCount <= maxMintPerTransaction, "amount must < max");
        require(totalSupply() + _mintCount <= maxSupply, "mint over supply");

        mintCount[_to] = minted + _mintCount;
        _safeMint(msg.sender, _mintCount);
    }

    // Only Owner executable functions
    function airdropByAccount(address _to, uint256 _mintCount) external onlyOwner {
        require(totalSupply() + _mintCount <= maxSupply, "mint over supply");
        if (_mintCount <= maxBatchSize) {
            _safeMint(_to, _mintCount);
            return;
        }
        
        uint256 leftToMint = _mintCount;
        while (leftToMint > 0) {
    ...
    function getPrice() public view returns(uint256) {
        uint256 price;
        if(totalSupply() <= 50){
            price = 25 ether; //25 BUSD
        }else if(totalSupply() <= 100){
            price = 50 ether; //50 BUSD
        }else if(totalSupply() <= 1000){
            price = 100 ether; //100 BUSD
        }
    ...
    function setStakeContractAddress(address _stakeContractAddress) external onlyOwner {
        kongStakeAddress = _stakeContractAddress;
    }
