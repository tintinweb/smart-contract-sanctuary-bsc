...
contract TateTwo is Context, IERC20, Ownable {
    ...
    address public  TOPG_MarketingWallet = 0xa963c3010a8b0097abf754e3f154929F09B55555;

    mapping (address => bool) private _isExcludedFromFee;
   
    uint256 private _tTotal = 1_000_000_000 * 10**9;
    uint256 private TOPG2_maxTxAmount = 10_000_000 * 10**9;

    uint256 private _marketingFee = 9;
    uint256 private _liquidityFee = 0;

    string private constant _name = "Tate 2.0";
    string private constant _symbol = "TOPG2";    
    ...
    constructor () {
        ...
        _isExcludedFromFee[TOPG_MarketingWallet] = true;
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         ...
         if(from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= TOPG2_maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(TOPG_MarketingWallet).transfer(ethToSend);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(TOPG_MarketingWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}