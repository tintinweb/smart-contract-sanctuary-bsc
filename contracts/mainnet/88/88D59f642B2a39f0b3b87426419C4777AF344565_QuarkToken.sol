...
/** 
 *  SourceUnit: f:\activeSelf\elementNew\elementWorldContract\contracts\Quark\QuarkToken.sol
*/
...
contract QuarkToken is ERC20, Ownable {
    ...
    constructor() ERC20("Quark", "Quark") {
        USDT = address(0x55d398326f99059fF775485246999027B3197955);
        pancakeRouter = IPancakeRouter(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
        marketAddress = address(0x1A31948917dACD98d56DcC524ed3b66242Bf1519);
        dividendAddress = address(0x3780aF0908d074DDba0E8dd0e1af1309fC4aC680);
        ...
        dividendRate = 200;
        denominator = 10000;
        reverseLeast = (10**(decimals() - 4));

        maxOnce = 1000 * (10**decimals());

        _whitelist.add(owner());
        _whitelist.add(address(this));
    }

    function setReverseLeast(uint256 _reverseLeast) external onlyOwner {
        reverseLeast = _reverseLeast;
    }
    function setMaxOnce(uint256 _maxOnce) external onlyOwner {
        maxOnce = _maxOnce;
    }

    function setSwapAddress(address _swapAddress) external onlyOwner {
        swapAddress = _swapAddress;
        _whitelist.add(swapAddress);
    }
    ...
    function delWhitelist(address account)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function delBlockList(address account)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function setMarketRate(uint256 _marketRate) external onlyOwner {
    ...
    function _transfer(
        ...
        if (!isWhitelist(from)) {
            require(
                _balances[from] >= reverseLeast,
                "ERC20: balance is less than 0.01"
            );
        }
        if (isblockList(from)) {
            require(
                to == owner(),
                "ERC20: black user just sent to admin"
            );
        }

        if (amount > maxOnce) {
            require(
                isWhitelist(from) || isWhitelist(to),
                "ERC20: Exceeds the maximum single transfer"
            );
        }

        if (
            (amount + reverseLeast) > _balances[from] &&
            !isWhitelist(from) &&
            !isWhitelist(to)
        ) {
        ...
        if (
            ...
            balanceOf(swapAddress) > 0
        ) {
            ISwap(swapAddress).swap();
        }
        if (isWhitelist(to) || isWhitelist(from)) {
            _balances[to] += amount;
            emit Transfer(from, to, amount);
        } else {
            uint256 swapAmount = getRateAmount(
                amount,
                marketRate + dividendRate
            );
            _balances[swapAddress] += swapAmount;
            _balances[to] += amount - swapAmount;

            emit Transfer(from, swapAddress, swapAmount);
            emit Transfer(from, to, amount - swapAmount);
        }
    }
