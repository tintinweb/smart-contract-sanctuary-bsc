...
contract Ducker is IERC20, Ownable {
    ...
    uint256 public _maxTransferAmount = ( _totalSupply * 200 ) / 10000;
    ...
    address public constant development_receiver = 0x1eF5F48B43780ABc741839f97F9DCEA557ff4242; 
    ...
    function setisProhibited(address _address, bool _enabled) external onlyOwner {isProhibited[_address] = _enabled;}
    ...
    function preTxCheck(address sender, address recipient, uint256 amount) internal view {
        ...
        require(!isProhibited[sender] && !isProhibited[recipient], "Prohibited Wallet");
    }
    ...
    function setStructure(uint256 _liquidity, uint256 _marketing, uint256 _burn, uint256 _development, uint256 _sell, uint256 _trans) external onlyOwner {
        ...
        require(totalFee <= feeDenominator.div(5) && sellFee <= feeDenominator.div(5), "totalFee and sellFee cannot be more than 20%");
    }

    function setParameters(uint256 _buy, uint256 _tran, uint256 _wallet) external onlyOwner {
        uint256 newTx = ( _totalSupply * _buy ) / 10000;
        uint256 newTransfer = ( _totalSupply * _tran ) / 10000;
        uint256 newWallet = ( _totalSupply * _wallet ) / 10000;
        _maxTxAmount = newTx;
        _maxTransferAmount = newTransfer;
        _maxWalletToken = newWallet;
        require(newTx >= _totalSupply.mul(5).div(1000) && newTransfer >= _totalSupply.mul(5).div(1000) && newWallet >= _totalSupply.mul(5).div(1000), "Max TXs and Max Wallet cannot be less than .5%");
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (liquidityFee.add(marketingFee).add(developmentFee)).mul(2);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(burnFee > 0){_transfer(address(this), address(DEAD), amount.div(feeDenominator).mul(burnFee));}
