...
contract DFS is Context, IERC20, Ownable {
    ...
    address public destroyAddress = 0xC83Fa9Edb26A5A24A63313C8712674C21dD48795;
    IBond public bond;
    IERC20 public usdt;
        ...
        if (to == address(pair) || from == address(pair) ) { 
            if (to == address(pair)) {
                ...
                if (_reserve0 > 0 && _reserve1 > 0) {
                    (uint256 numerator, uint256 denominator) = address(usdt) < address(this) ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
                    uint256 price = numerator.div(denominator);
                    require(price >= bond.terms().minimumPrice, "reached minimumPrice");
                }
            }
