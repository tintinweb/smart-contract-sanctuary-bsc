...
contract TAxesStaking is IStaking, AccessControl, IERC721Receiver, ReentrancyGuard {
    ...
    */
    function stake(uint256[] calldata _tokenIds) external nonReentrant checkTokensOwner(msg.sender, _tokenIds) {
    ...
    */
    function setMiningPower(uint256 _tokenId, uint256 _miningPower, bool _isActive) public nonReentrant onlyRole(OPERATOR_ROLE) {
    ...
    * @param _tokenIds Array of nft token identifiers
    */
    function unstake(uint256[] calldata _tokenIds) external nonReentrant checkStakesOwner(msg.sender, _tokenIds) checkMinStakesTime(_tokenIds) {
        recalculateReward(msg.sender);
        for (uint i = 0; i < _tokenIds.length; i++) {
