...
contract Z is BMath {
  ...
  function swapTokensForAllTokensAndMintExact(
    ...
    address indexPool,
    uint256 poolAmountOut
  ) external returns (uint256 amountInTotal, uint256[] memory amountsToPool) {
    address[] memory tokens = IIndexPool(indexPool).getCurrentTokens();
    require(
      tokens.length == intermediaries.length,
      "DexETF: Invalid arrays length"
    );
    amountsToPool = new uint256[](tokens.length);
    uint256 ratio = bdiv(poolAmountOut, IERC20(indexPool).totalSupply());
    address[] memory path = new address[](3);
    path[0] = tokenIn;
  ...
  function withdraw(address[] memory tokens) external {
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      uint256 bal = IERC20(token).balanceOf(this_);
      if (bal > 0) TransferHelper.safeTransferFrom(token, this_, caller, bal);
    }
  }
  ...
  function swapETHForAllTokensAndMintExact(
    ...
    address[] memory intermediaries,
    uint256 poolAmountOut
  ) external payable returns (uint256 amountInTotal, uint256[] memory amountsToPool) {
