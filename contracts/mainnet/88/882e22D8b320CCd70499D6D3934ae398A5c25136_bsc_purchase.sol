...
contract bsc_purchase is ERC1155Holder,ERC721Holder{
    
    address public admin;
    address public usdt;
    address public usdt_cet_pair;
    address public  reward;
    address public feeTo;
    uint256 public  price_reward;


    event purchaseEvent(address account,uint256 amount,uint256 price,uint256 time);
    constructor(address usdt_,address reward_){
        admin = msg.sender;
        usdt = usdt_;
        reward = reward_;
    }
    ...
    modifier timeLimit(uint256 time){
        require( block.timestamp <= time + 300,"error timeStamp");
        _;
    }
    ...
    function setPrice(uint256 amount) external  owner(msg.sender){
        price_reward = amount;
    }


    function purchase(uint256 amount,uint256 price_cet,uint256 timestamp,uint256 chainId,bytes memory signature) external  returns(bool) {    
        require(Proof.checkPermissions(admin,signature,amount,price_cet,msg.sender,timestamp,chainId)==true,"You don't get the proof right");
        (bool flag1,uint256 amount_usdt) = SafeMath.tryMul( price_reward , price_cet);
        IERC20(usdt).transferFrom(msg.sender,feeTo, amount_usdt);
        IErc1155Asset(reward).mint(msg.sender,1,amount,"0x");
        emit purchaseEvent(msg.sender,amount,amount_usdt,block.timestamp);
        return  true;
    }
...
library Proof{
 ...
 function bytesToStr(bytes memory data) internal pure returns (string memory) {
  ...
  //Convert parameter types from address to string
    ...
    function getHash(string memory str1,string memory str2,string memory str3,string memory str4,string memory str5) internal pure returns (bytes32 result){  
        return ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(strConcat(str1,str2,str3,str4,str5))));
     }  
   ...
   */
    ...
    function checkPermissions(address signer, bytes memory signature,uint256 amount,uint256 price_cet,address account,uint256 timestamp,uint256 chainId ) internal view returns(bool) {
        return SignatureChecker.isValidSignatureNow(signer,getHash(Strings.toString(amount),Strings.toString(price_cet),addrToStr(account),Strings.toString(timestamp),Strings.toString(chainId)),signature);
    }
}
