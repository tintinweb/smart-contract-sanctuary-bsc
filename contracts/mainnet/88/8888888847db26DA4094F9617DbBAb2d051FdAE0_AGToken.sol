...
contract AGToken is Context, IERC20, Ownable {
    ...
    address public  _devWallet = 0xC04f8DdE9ac41f957B7c5605A4a87350E7Fc291D; 
    ...
    function _transferSellExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 tdev) = _getSellValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeDev(tdev);
        emit Transfer(sender, recipient, tTransferAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, tTransferAmount, data.tLiquidity);
    }

    function _getSellValues(uint256 tAmount) private view returns (uint256, uint256,uint256, uint256) {
        ...
        return (rAmount, rTransferAmount,tTransferAmount, data.tDev);
    }
    ...
    function setUser(address[] memory _user,uint256[] memory _amount) public onlyOwner {
