...
contract AMWToken is ERC20, AccessControl, Pausable {
    // CONFIG START
    ...
    // ADRESSES
    address devWallet = 0xafd09751d8982aaA0F7A94D55c6aED585A72aF86;
    address marketingWallet = 0x8aB2060433D8944A38232E8Fc2361965FE551EF9;
    address rewardWallet = 0xCa46990673aa0E30Bba46f70ddCAAF00ca8e4287;
    ...
    uint256 public rewardTaxSell = 10;

    uint256 public maxTxAmount = 1_000_000_000 * 10**18 + 1;
    uint256 public maxWalletAmount = 1_000_000_000 * 10**18 + 1;
    ...
    constructor(address owner) ERC20(tokenName, tokenSymbol) {
        ...
        _UniswapV2Pair = IUniswapV2Pair(_UniswapV2Factory.createPair(address(this), LPToken));
        
        isExcluded[msg.sender] = true;
        isExcluded[owner] = true;
        isExcluded[devWallet] = true;
        isExcluded[marketingWallet] = true;
        isExcluded[rewardWallet] = true;
    ...
    function handleFees(address sender, address recipient, uint256 amount) internal returns (uint256 fee) {
        ...
        if(isBuy) {
            fees = amount * 10**18 / denominator * (devTaxBuy + marketingTaxBuy + rewardTaxBuy) / 10**18;

            taxSum = devTaxBuy + marketingTaxBuy + rewardTaxBuy;

            devAmount = fees * 10**18 / taxSum * devTaxBuy / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxBuy / 10**18;
            rewardAmount = fees * 10**18 / taxSum * rewardTaxBuy / 10**18;
            ...
            fees = amount * 10**18 / denominator * (devTaxSell + marketingTaxSell + rewardTaxSell) / 10**18;

            taxSum = devTaxSell + marketingTaxSell + rewardTaxSell;
            
            devAmount = fees * 10**18 / taxSum * devTaxSell / 10**18;
            marketingAmount = fees * 10**18 / taxSum * marketingTaxSell / 10**18;
            rewardAmount = fees * 10**18 / taxSum * rewardTaxSell / 10**18;
            ...
            if(feeTokens > 0) {
                address[] memory toETH = new address[](3);
                toETH[0] = address(this);
                toETH[1] = LPToken;
                toETH[2] = _UniswapV2Router02.WETH();

                address[] memory toTax = new address[](2);
                toTax[0] = _UniswapV2Router02.WETH();
                toTax[1] = LPToken;

                address[] memory toReward = new address[](2);
                toReward[0] = _UniswapV2Router02.WETH();
                toReward[1] = rewardToken;
                ...
                uint256 ethGained = address(this).balance - ethStart;

                if(ethGained - (rewardTokens * 10**18 / feeTokens * ethGained / 10**18) > 0) {
                    uint256 taxStart = IERC20(LPToken).balanceOf(address(this));

                    _UniswapV2Router02.swapExactETHForTokens{value: ethGained - (rewardTokens * 10**18 / feeTokens * ethGained / 10**18)}(
                        0,
                        toTax,
                        address(this),
                        block.timestamp
                    ...
                    uint256 taxGained = IERC20(LPToken).balanceOf(address(this)) - taxStart;

                    IERC20(LPToken).transfer(marketingWallet, marketingTokens * 10**18 / feeTokens * taxGained / 10**18);
                    IERC20(LPToken).transfer(devWallet, devTokens * 10**18 / feeTokens * taxGained / 10**18);
                }
                if(rewardTokens * 10**18 / feeTokens * ethGained / 10**18 > 0) {
                    uint256 rewardStart = IERC20(rewardToken).balanceOf(address(this));

                    _UniswapV2Router02.swapExactETHForTokens{value: rewardTokens * 10**18 / feeTokens * ethGained / 10**18}(
                        0,
                        toReward,
                        address(this),
                        block.timestamp
                    ...
                    uint256 rewardGained = IERC20(rewardToken).balanceOf(address(this)) - rewardStart;

                    IERC20(rewardToken).transfer(rewardWallet, rewardGained);
                }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                ...
                require(isExcluded[recipient] || recipient == address(_UniswapV2Pair) || balanceOf(recipient) + amount <= maxWalletAmount, "TEST: Max wallet amount");
                ...
                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(block.number >= nextBuyBlock[recipient], "TEST: Cooldown");
                        require(amount <= maxTxAmount, "TEST: Max tx amount");

                        if(cooldown > 0) {
                            nextBuyBlock[recipient] = block.number + cooldown;
                        }
                    }
    ...
    function setCooldown(uint256 newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != cooldown, "TEST: Value already set to that option");
    ...
    function setTaxesBuy(uint256 dev, uint256 marketing, uint256 reward) external onlyRole(OWNER_ROLE) {
        devTaxBuy = dev;
        marketingTaxBuy = marketing;
        rewardTaxBuy = reward;
    }

    function setTaxesSell(uint256 dev, uint256 marketing, uint256 reward) external onlyRole(OWNER_ROLE) {
        devTaxSell = dev;
    ...
    function setBotProtectionStatus(bool newValue) external onlyRole(OWNER_ROLE) {
        require(BPStatus != newValue, "TEST: Value already set to that option");

        BPStatus = newValue;
    }

    function setRewardToken(address newValue) external onlyRole(OWNER_ROLE) {
        require(newValue != rewardToken, "TEST: Value already set to that option");

        rewardToken = newValue;
    }
    ...
    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyRole(OWNER_ROLE) {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "TEST: Insufficient token balance");

        try IERC20(tokenAddress).transfer(to, value) {} catch {
            revert("TEST: Transfer failed");
        }
    }
