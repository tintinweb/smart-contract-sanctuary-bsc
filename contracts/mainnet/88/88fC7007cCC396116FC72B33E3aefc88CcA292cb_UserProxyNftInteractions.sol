 ...
 * @author Unknown
 * @notice Core logic for all user CONE/veNFT interactions
...
contract UserProxyNftInteractions is UserProxyStorageLayout {
    ...
    /*******************************************************
     *                 CONE and veNFT interactions
     *******************************************************/

    // Modifiers
    modifier syncPools() {
        unkwnLens.unkwnPoolFactory().syncPools(1);
        _;
    }
    ...
    /**
     * @notice CONE -> veNFT -> UnCONE
     * @param amount The amount of CONE to convert to UnCONE
     */
    function convertConeToUnCone(uint256 amount)
        external
        ...
        syncPools
    {
        ...
        // Lock CONE
        uint256 tokenId = _lockCone(amount);

        // Convert NFT to UnCONE
        _convertNftToUnCone(tokenId);

        // Transfer UnCONE to owner
        unkwnLens.unCone().transfer(ownerAddress, amount);
    }

    /**
     * @notice Convert veNFT to UnCONE
     * @param tokenId The tokenID to convert
     */
    function _convertNftToUnCone(uint256 tokenId) internal {
        // Fetch UnCONE balanceOf(owner) before conversions
        uint256 unConeBalanceOfBefore = unkwnLens.unCone().balanceOf(
            address(this)
        );

        // Determine amount of UnCONE to mint
        IVe ve = unkwnLens.ve();
        uint256 amount = ve.locked(tokenId);
        assert(amount > 0);

        // Deposit NFT to UnCONE and receive UnCONE
        ve.approve(unConeAddress, tokenId);
        unkwnLens.unCone().convertNftToUnCone(tokenId);

        // Make sure correct amount of UnCONE was received
        uint256 unConeBalanceOfAfter = unkwnLens.unCone().balanceOf(
            address(this)
        );
        assert(unConeBalanceOfAfter == unConeBalanceOfBefore + amount);
    }
     ...
     */
    function _lockCone(uint256 amount) internal returns (uint256 tokenId) {
        // No empty locking
        require(amount > 0, "Amount to lock must be greater than zero");

        // Receive CONE from msg.sender
        ICone cone = unkwnLens.cone();
        cone.transferFrom(msg.sender, address(this), amount);

        // Allow ve to spend CONE
        address veAddress = unkwnLens.veAddress();
        cone.approve(veAddress, amount);

        // Lock CONE (CONE -> veNFT)
        uint256 lockTime = 4 * 365 * 86400; // 4 years
        tokenId = unkwnLens.ve().createLock(amount, lockTime);
    }

    /**
     * @notice CONE -> veNFT -> UnCONE -> Staked UnCONE
     * @param amount The amount of CONE to convert and stake
     */
    function convertConeToUnConeAndStake(uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
     ...
     * @param tokenId The tokenId to convert
     */
    function convertNftToUnCone(uint256 tokenId)
        public
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
        // Amount of underlying CONE locked
        IVe ve = unkwnLens.ve();
        uint256 amount = ve.locked(tokenId);
    ...
    /**
     * @notice veNFT -> UnCONE -> Staked UnCONE
     * @param tokenId The tokenId to convert and stake
     */
    function convertNftToUnConeAndStake(uint256 tokenId)
        public
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
    ...
    /**
     * @notice Stake UnCONE in rewards pool
     */
    function _stakeUnCone(uint256 amount) internal {
        address stakingAddress;

        // Determine partner status
        bool isPartner = unkwnLens.isPartner(address(this));

        // Set staking address
        if (isPartner) {
            stakingAddress = unkwnLens.partnersRewardsPoolAddress();
        } else {
            stakingAddress = unConeRewardsPoolAddress;
        }

        // Allow UnCONE mutlirewards to spend UnCONE
        unkwnLens.unCone().approve(stakingAddress, amount);

        // Stake UnCONE in multirewards
        IMultiRewards(stakingAddress).stake(amount);
    }

    /**
     * @notice UnCONE -> staked UnCONE
     */
    function stakeUnCone(uint256 amount)
        public
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
        // Transfer UnCONE from msg.sender to UserProxy
        unkwnLens.unCone().transferFrom(msg.sender, address(this), amount);

        // Stake UnCONE on behalf of user
        _stakeUnCone(amount);
    }

    /**
     * UnCONE -> staked UnCONE (UNKWN v1 staking contract)
     */
    function stakeUnConeInUnkwnV1(uint256 amount)
        public
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
        ...
        // Stake UnCONE on behalf of user
        _stakeUnConeInUnkwnV1(amount);
    }

    /**
     * @notice Perform UnCONE stake into v1 staking pool
     */
    function _stakeUnConeInUnkwnV1(uint256 amount) internal {
        address stakingAddress = unkwnLens.unkwnV1RewardsAddress();
    ...
    /**
     * @notice Staked UnCONE -> UnCONE
     * @param amount The amount of UnCONE to unstake
     */
    function unstakeUnCone(uint256 amount)
        public
        onlyUserProxyInterfaceOrOwner
    {
        address stakingAddress;
        ...
        unstakeUnCone(stakingAddress, amount);
    }

    /**
     * @notice Staked UnCONE (UNKWN v1 rewards) -> UnCONE
     * @param amount The amount to unstake
     */
    function unstakeUnConeInUnkwnV1(uint256 amount)
        public
        onlyUserProxyInterfaceOrOwner
    {
        unstakeUnCone(unkwnLens.unkwnV1RewardsAddress(), amount);
    }
     ...
     * @param amount The amount to unstake
     */
    function unstakeUnCone(address stakingAddress, uint256 amount)
        public
        onlyUserProxyInterfaceOrOwner
        syncPools
    {
    ...
    /**
     * @notice Redeem UNKWN v1 for UnCONE
     */
    function redeemUnkwnV1(uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
    {
        IERC20 unkwnV1 = IERC20(unkwnLens.unkwnV1Address());
        unkwnV1.transferFrom(msg.sender, address(this), amount);

        address unkwnV1RedeemAddress = unkwnLens.unkwnV1RedeemAddress();
        IUnkwnV1Redeem unkwnV1Redeem = IUnkwnV1Redeem(unkwnV1RedeemAddress);

        unkwnV1.approve(unkwnV1RedeemAddress, amount);
        unkwnV1Redeem.redeem(amount);

        IMultiRewards unkwnV1Rewards = IMultiRewards(
            unkwnLens.unkwnV1RewardsAddress()
        );
        IUnCone unCone = unkwnLens.unCone();
        unCone.transfer(ownerAddress, unCone.balanceOf(address(this)));

        uint256 rewardTokensLength = unkwnV1Rewards.rewardTokensLength();
        for (uint256 i; i < rewardTokensLength; i++) {
            IERC20 rewardToken = IERC20(unkwnV1Rewards.rewardTokens(i));
    ...
    /**
     * @notice Redeem UNKWN v1 for UnCONE and stake into unkwnV1Rewards
     */
    function redeemAndStakeUnkwnV1(uint256 amount)
        external
        onlyUserProxyInterfaceOrOwner
    {
        //establish vars
        IERC20 unkwnV1 = IERC20(unkwnLens.unkwnV1Address());
        IUnCone unCone = unkwnLens.unCone();
        address unkwnV1RewardsAddress = unkwnLens.unkwnV1RewardsAddress();
        IUnkwnV1Rewards unkwnV1Rewards = IUnkwnV1Rewards(unkwnV1RewardsAddress);
        address unkwnV1RedeemAddress = unkwnLens.unkwnV1RedeemAddress();
        IUnkwnV1Redeem unkwnV1Redeem = IUnkwnV1Redeem(unkwnV1RedeemAddress);

        //transfer UNKWNv1 to userProxy
        unkwnV1.transferFrom(msg.sender, address(this), amount);

        //approve and redeem UNKWNv1 from unkwnV1Redeem
        unkwnV1.approve(unkwnV1RedeemAddress, amount);
        unkwnV1Redeem.redeem(amount);

        //check redeemed UnCONE balance against stakingCap, transfer excess to owner
        uint256 unConeBalance = unCone.balanceOf(address(this));
        uint256 stakingCap = unkwnV1Rewards.stakingCap(address(this));
        uint256 stakedBalance = unkwnV1Rewards.balanceOf(address(this));
        ...
        if (stakingCap > stakedBalance) {
            if (stakingCap - stakedBalance < unConeBalance) {
                uint256 unConeToTransfer = unConeBalance - stakingCap;
                unCone.transfer(ownerAddress, unConeToTransfer);
                unConeBalance = stakingCap;
            }
        } else {
            // if stakingCap < stakedBalance, transfer all UnCONE to owner
            unCone.transfer(ownerAddress, unConeBalance);
            unConeBalance = 0;
        }

        //approve and stake UnCONE in unkwnV1Rewards
        if (unConeBalance > 0) {
            unCone.approve(unkwnV1RewardsAddress, unConeBalance);
            unkwnV1Rewards.stake(unConeBalance);
        }
     ...
     */
    function migrateUnConeToPartner() external onlyUserProxyInterfaceOrOwner {
        IMultiRewards unConeRewardsPool = IMultiRewards(
            unkwnLens.unConeRewardsPoolAddress()
        );
        IMultiRewards partnersRewardsPool = IMultiRewards(
            unkwnLens.partnersRewardsPoolAddress()
        );

        uint256 nonPartnerStakedBalance = unConeRewardsPool.balanceOf(
            address(this)
        );
        unConeRewardsPool.exit();
        unkwnLens.unCone().approve(
     ...
     * @dev The generalized function doesn't work because vlUNKWN is a bit different from other multirewards
     */
    function claimVlUnkwnRewards() public onlyUserProxyInterfaceOrOwner {
        address stakingPoolAddress = unkwnLens.vlUnkwnAddress();
        IVlUnkwn multiRewards = IVlUnkwn(stakingPoolAddress);
