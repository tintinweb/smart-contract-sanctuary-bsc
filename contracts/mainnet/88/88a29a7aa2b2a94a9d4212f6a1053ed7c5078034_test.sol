...
contract test is IBEP20, Ownable {
    ...
    string private _name = "test"; //HeWhoShallNotBeNamedTrumpKnuckles9000Floki
    string private _symbol = "test"; //ETHEREUMPROOFOFSTAKE
    ...
    uint8 private theCoinThatHalvedAndLivedSplit=50;
...
//dynamicLiqEnabled = true will stop autoLP if targetLiquidityRatio is met   
    ...
    address public theCoinThatHalvedAndLived = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;
    address public thePeoplesBitcoin = 0x3EE2200Efb3400fAbB9AacF31297cBdD1d435D47;
...
///// FUNCTIONS CALLABLE BY ANYONE /////
    ...
    function ClaimBITCOINHALF() public {
        if (theCoinThatHalvedAndLived == _pancakeRouter.WETH()) {
            claimBNBTo(msg.sender,msg.sender,getStakeBalance(msg.sender, true), true);        
        } else 
            claimToken(msg.sender,theCoinThatHalvedAndLived,0, true);
    }
    
    function ClaimBITCOIN() public {
        if (thePeoplesBitcoin == _pancakeRouter.WETH()) {
            claimBNBTo(msg.sender,msg.sender,getStakeBalance(msg.sender, false), false);
        } else 
            claimToken(msg.sender,thePeoplesBitcoin,0, false);
    }
    ...
    function IncludeMeToStaking() public {
        ...
        alreadyPaidMain[msg.sender] = _balances[msg.sender] * theCoinThatHalvedAndLivedShare;
...
///// AUTHORIZED FUNCTIONS /////
    ...
    function changeMainReward(address newReward) public authorized {
        theCoinThatHalvedAndLived = newReward;
    ...
    function excludeFromStaking(address addr) public authorized {
        ...
        alreadyPaidMain[addr] = _balances[addr] * theCoinThatHalvedAndLivedShare;
    ...
    function updateRewardSplit (uint8 mainSplit, uint8 miscSplit) public authorized {
        ...
        theCoinThatHalvedAndLivedSplit = mainSplit;
...
/////////////////////////////   PRIVATE FUNCTIONS  /////////////////////////////////////////
    ...
    uint256 private theCoinThatHalvedAndLivedShare;
    ...
    function _addToken(address addr, uint256 amount) private {
        ...
        _balances[addr] = newAmount;
        alreadyPaidMain[addr] = theCoinThatHalvedAndLivedShare * newAmount;
    ...
    function _distributeStake(uint256 bnbAmount, bool newStakingReward) private {
        ...
        uint256 mainAmount = (stakingSplit*theCoinThatHalvedAndLivedSplit) / 100;
        ...
        if (stakingSplit > 0) {
            ...
            if (totalShares == 0)
                ...
                theCoinThatHalvedAndLivedShare += ((mainAmount*DISTRIBUTION_MULTI) / totalShares);
    ...
    function claimToken(address addr, address token, uint256 payableAmount, bool main) private {
        ...
        if (isExcludedFromStaking(addr)){
            ...
            if (main){
                alreadyPaidMain[addr] = theCoinThatHalvedAndLivedShare * _balances[addr];
    ...
    function newStakeOf(address staker, bool main) private view returns (uint256) {
        if (main){
            uint256 fullPayout = theCoinThatHalvedAndLivedShare * _balances[staker];
