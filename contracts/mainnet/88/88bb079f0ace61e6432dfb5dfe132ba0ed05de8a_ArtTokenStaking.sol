...
import "./ArtTokenStakingStorage.sol";
import "./ArtBaseUpgradable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/SafeERC20Upgradeable.sol";


contract ArtTokenStaking is ArtBaseUpgradable, ArtTokenStakingStorage {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMathUpgradeable for uint256;

    event StakingArtt (address indexed staker, uint stakingAmount, uint period, uint indexed stakingIndex);
    ...
    function stakeArtt(uint amount, uint periodInDays) external allowedStaking returns (bool){
        ...
        stakedPool += amount;
        emit StakingArtt(msg.sender, amount, periodInDays, stakedBalances.length - 1);
        return IERC20Upgradeable(artToken).transferFrom(msg.sender, address(this), amount);
    }

    function applyToWithdrawStakedArtt(uint index) external {
    ...
    function withdrawStakedArtt(uint index) external returns (bool){
        uint256 i = stakedIndexesPerAddress[msg.sender][index];
        require(index < stakedBalances.length, "ERR_NOT_FOUND");
    ...
    function _calculateStakingReward(uint amountStaked, uint startDate, uint endDate, StakingMargin margin) internal view returns (uint _reward, uint _apy){
        ...
        if (margin == StakingMargin.LOW) {
            if (endDate.sub(startDate) == lockupPeriod1) {
                ...
                return (amountStaked.mul(apy).div(10000).div(uint(360).div(lockupPeriod1.div(86400))), apy);
                ...
                return (amountStaked.mul(apy).div(10000).div(uint(360).div(lockupPeriod2.div(86400))), apy);
            } else {
                apy = _calculateApy(LOW_STAKE_PERIOD3_CEILING_APY, LOW_STAKE_PERIOD3_FLOOR_APY, startDate);
                return (amountStaked.mul(apy).div(10000).div(uint(360).div(lockupPeriod3.div(86400))), apy);
            }
...
contract ArtTokenStakingStorage {
