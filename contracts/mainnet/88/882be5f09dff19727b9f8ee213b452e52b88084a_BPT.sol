...
contract BPT is ERC20, Ownable {
    ...
    uint256 public poolThresholdValue;

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event MarketingWalletChanged(address marketingWallet);

    constructor () ERC20("BePulser", "BPT") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0xAd90b3461da57812427E83Cb88f653BD683107B4;
        ...
        poolThresholdValue = 1000 * (10 ** 18); // 1000 BNB;
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        bool poolThreshold = false;

        if (IERC20(uniswapV2Router.WETH()).balanceOf(address(uniswapV2Pair)) > poolThresholdValue) {
            poolThreshold = true;
        }

        uint256 _totalFees;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || poolThreshold) {
            _totalFees = 0;
