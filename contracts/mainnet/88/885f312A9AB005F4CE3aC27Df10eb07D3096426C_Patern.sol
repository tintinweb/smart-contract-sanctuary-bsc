...
contract Patern{
    ...
    //代币合约地址
    address public Dmt4Contract = address(0xbeC159dc0abdaE5365eF33bdd4A097a9DCfa246D);
    ...
    //事件
    //购买&&升级事件
    event buyEvent(address indexed owner,address indexed superior,uint256 gear,uint256 usdt,uint256 glod,uint256 time,uint256 buytype);
    ...
    */
    ...
    function buy(uint256 gear_,address superior_)
        ...
        returns(bool)
        {
            ...
            require(msg.sender != superior_, "Patern:The current gear is greater than the one purchased this time");
            // if(Teams.team(msg.sender) == address(0x00)){
            //     Teams.bindingWhite(msg.sender,superior_);//-----需要白名单
            // }
            uint256 thisusdt = Gear[gear_] - userGearing[msg.sender].usdt;
            // usdttoken.transferFrom(msg.sender,address(this),thisusdt);
            // usdttoken.transfer(lpAddress,thisusdt*25/100);
            // usdttoken.transfer(fundAddress,thisusdt*5/100);
            // usdttoken.approve(swapRouterAddress,thisusdt*70/100);
            // address[] memory paths = new address[](2);
            // paths[0] = UsdtContract;
            // paths[1] = Dmt4Contract; 
            // uint256[] memory amounts = swap.swapExactTokensForTokens(thisusdt*70/100,0,paths,address(this),block.timestamp+1800);
            ...
            userGearing[msg.sender].glod = Patern.UsdtToTokens(Gear[gear_],Dmt4Contract);//更新算力
            if(userGearing[msg.sender].gear == 0){
                emit buyEvent(msg.sender,superior_,gear_,thisusdt,userGearing[msg.sender].glod,block.timestamp,1);
            }else{
                emit buyEvent(msg.sender,superior_,gear_,thisusdt,userGearing[msg.sender].glod,block.timestamp,2);
            }
    ...
    * usdt兑换代币的数量测试
    */
    function UsdtToTokens(uint256 amount_,address token_) 
        public 
        view 
        returns(uint256)
        {
