...
contract OwnableMOTG is ContextMOTG {
    address private _owner;

    event OwnerMOTGpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerMOTGpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableMOTG: caller is not the owner");
        _;
    }

 
    function renounceOwnerMOTGp() public virtual onlyOwner {
        emit OwnerMOTGpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerMOTGp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerMOTGp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableMOTG: new owner is the zero address");
        emit OwnerMOTGpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract MetaOctagon is ContextMOTG, BinanceBNB, OwnableMOTG {
    ...
    string private _name = 'MetaOctagon';
    string private _symbol = 'MOTG';
