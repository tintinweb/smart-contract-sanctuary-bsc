...
// File: contracts/Products/PodsNFT/PodsNFTv2.sol
...
contract PodsNFT is ERC721Enumerable, Ownable {
    ...
    modifier onlyOwnerOrLaunchpad() {
        require(msg.sender == owner() || msg.sender == LAUNCHPAD , "must call by owner or launchpad");
        _;
    }
    ...
    function mintTo(address to, uint256 size) public onlyOwnerOrLaunchpad {
        require(to != address(0), "can't mint to empty address");
        require(size > 0, "size must greater than zero");
        require(mintCount + size <= maxSupply, "max supply reached");

        for (uint256 i = 1; i <= size; i++) {
            _safeMint(to, mintCount); // ERC721Enumerable.totalSupply() + 1
            mintCount++;
        }
    }
