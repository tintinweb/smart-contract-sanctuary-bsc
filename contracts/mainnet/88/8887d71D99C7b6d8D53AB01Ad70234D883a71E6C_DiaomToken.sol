...
contract DiaomToken is ERC20PresetFixedSupply, Ownable {
    ...
    function _afterTokenTransfer(address _from, address _to, uint256 _amount) internal virtual override {
        ...
        // Initiate fee deduction and transfer accounts are not whitelisted
        if(isVerifyPair && !whitelist[_to] && _from != address(0) && _to != address(0)){
    ...
    function setBuyFee(uint256 _buyFeeNumer, uint256 _buyFeeDenom) public onlyOwner {
        buyFeeNumer = _buyFeeNumer;
        buyFeeDenom = _buyFeeDenom;
        emit LogSetBuyFee(buyFeeNumer, buyFeeDenom);
    }

    function setSellFee(uint256 _sellFeeNumer, uint256 _sellFeeDenom) public onlyOwner {
        sellFeeNumer = _sellFeeNumer;
        sellFeeDenom = _sellFeeDenom;
        emit LogSetSellFee(sellFeeNumer, sellFeeDenom);
    }
    ...
    event LogSetBuyFee(uint256, uint256);
    event LogSetSellFee(uint256, uint256);
