...
/**
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Welcome to MayeMoon -  The official token of Maye Musk and Mother's Day; we will moon as Elon tweets to Maye!

On May 9th, 2021, Elon Musk, in a mother's day gesture, gifted his mother, Maye Musk, Dogecoin.

Buy Tax
12% [Before Mother's Day]
12% [After Mother's Day]

Sell Tax 
16% [Before Mother's Day]
12% [On & After Mother's Day]
...
library Address{
 ...
 contract MAYEMOON is Context, IERC20, Ownable {
    ...
    address public marketing = 0x074e7c887fa6cdcE29442CAA4eAf3b9603B372b1;

    string private constant _name = "MayeMoon";
    string private constant _symbol = "MEM";
    ...
    Taxes public sellTaxes = Taxes(3, 6, 4, 3);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[DOGE] = true;
        _isExcludedFromFee[marketing] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[DOGE] = true;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferDogeReward) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferDogeReward) {
    ...
    function setTaxes(uint256 _DogeReward, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       taxes = Taxes(_DogeReward,_marketing,_liquidity,_development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _DogeReward, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       sellTaxes = Taxes(_DogeReward,_marketing,_liquidity,_development);
        emit FeesChanged();
    }

    function _reflectDogeReward(uint256 rDogeReward, uint256 tDogeReward) private {
        _rTotal -=rDogeReward;
        totFeesPaid.DogeReward +=tDogeReward;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rDogeReward, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tDogeReward = tAmount*temp.DogeReward/100;
        s.tMarketing = tAmount*temp.marketing/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tdevelopment = tAmount*temp.development/100;
        s.tTransferAmount = tAmount-s.tDogeReward-s.tMarketing-s.tLiquidity-s.tdevelopment;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rDogeReward,uint256 rMarketing, uint256 rLiquidity){
        ...
        rDogeReward = s.tDogeReward*currentRate;
        rMarketing = s.tMarketing*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rdevelopment = s.tdevelopment*currentRate;
        rTransferAmount =  rAmount-rDogeReward-rMarketing-rLiquidity-rdevelopment;
        return (rAmount, rTransferAmount, rDogeReward,rMarketing,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rDogeReward > 0 || s.tDogeReward > 0) _reflectDogeReward(s.rDogeReward, s.tDogeReward);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(DOGE).sendValue(marketingAmt);
        }
        uint256 developmentAmt = unitBalance * 2 * temp.development;
        if(developmentAmt > 0){
            payable(marketing).sendValue(developmentAmt);
        }
    }
    ...
    function updateDOGE(address newWallet) external onlyOwner{
        DOGE = newWallet;
    }
