...
contract Vator_Distributor is Ownable {
    ...
    function AddUsersWithDecimal(address[] memory users, uint256[] memory amount) public onlyOwner {
        require(users.length == amount.length, "Vator_Distributor: users and amount length is not equal..");
        for (uint256 i = 0; i < users.length; i++) {
            
            if (totalVTRAmount[users[i]] == 0) {
                totalVTRAmount[users[i]] = totalVTRAmount[users[i]].add(amount[i]);
            }
        }
    }
    ...
    function youCanClaimMax(address user) public view returns (uint256) {
        return totalVTRAmount[user];
    }

    function youHaveClaimed(address user) public view returns (uint256) {
        return totalVTRClaimed[user];
    }

    function _totalClaimed() public view returns (uint256) {
        return totalClaimed;
    }
