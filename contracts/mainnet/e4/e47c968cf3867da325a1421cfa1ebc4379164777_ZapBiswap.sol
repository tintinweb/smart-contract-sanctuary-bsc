...
contract ZapBiswap is OwnableUpgradeable {
    ...
    IBiswapRouter02 private constant ROUTER = IBiswapRouter02(0x3a6d8cA21D1CF76F653A67577FA0D27453350dD8);
    IBiswapFarm private constant FARM = IBiswapFarm(0xDbc1A13490deeF9c3C12b44FE77b503c1B061739);
    IBiswapFactory private constant FACTORY = IBiswapFactory(0x858E3312ed3A876947EA49d572A7C42DE08af7EE);
    ...
    */
    ...
    function priceFeed(address token) external view returns (uint256){
     ...
     function lpPrice(address token) public view returns (
        uint256 _reserve0,
        uint256 _reserve1,
        uint256 _price0,
        uint256 _price1,
        ...
        IBiswapPair pair = IBiswapPair(token);
        ...
        )
    {
        //PoolInfo storage pool = FARM.poolInfo[_pid];	
            ...
            IBiswapPair pair = IBiswapPair(lpToken);	
    ...
    function zapInToken(address _from, uint amount, address _to) external {
        ...
        if (isFlip(_to)) {
            IBiswapPair pair = IBiswapPair(_to);
    ...
    function zapOut(address _from, uint amount) external {
        ...
        if (!isFlip(_from)) {
            ...
            IBiswapPair pair = IBiswapPair(_from);
            address token0 = pair.token0();
            address token1 = pair.token1();
   ...
   function _lpPrice(address token) private view returns (uint256) {
    ...
    function  _priceFeed(address token) private view returns (uint256){
        ...
        if (!isFlip(token)){
            ...
            IBiswapPair pair = IBiswapPair(lp);
            (uint112 r0, uint112 r1,) = pair.getReserves();
            address pricefeed = chainLink[WBNB];
            (, int wbnbPrice,,,) = AggregatorEthereumV3(pricefeed).latestRoundData();
    ...
    function _swapBNBToFlip(address flip, uint amount, address receiver) private {
        if (!isFlip(flip)) {
            ...
            // flip
            IBiswapPair pair = IBiswapPair(flip);
            address token0 = pair.token0();
            address token1 = pair.token1();
            if (token0 == WBNB || token1 == WBNB) {
