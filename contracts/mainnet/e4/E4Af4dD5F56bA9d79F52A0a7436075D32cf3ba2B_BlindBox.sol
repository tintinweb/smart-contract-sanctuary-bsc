 ...
 using Counters for Counters.Counter;
    ...
    */
    ...
    modifier validateCurrencyType(uint256 seriesId, uint256 currencyType, bool isPayable) {
        ...
        require(isValid, "123");
        require((isPayable && currencyType == 2) || currencyType < 2, "126");
        _;
    }
...
/** 
    ...
    */
    function buyNonGenBox(uint256 seriesId, uint256 currencyType, address collection, string memory ownerId, bytes32 user) validateCurrencyType(seriesId,currencyType, false) internal {
        require(!_isWhiteListed[seriesId] || crypoWhiteList[msg.sender][seriesId] || (msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e && _whitelisted[user][seriesId]), "not authorize");
       ...
       uint256 price = calculatePrice(nonGenSeries[seriesId].price + _registrationFee[seriesId] , baseCurrency[seriesId], currencyType);
         if(currencyType == 0){
   ...
   if(msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e){
       return _boxesNoncryptoUser[ownerId][seriesId];
   }
    return   _boxesCrytpoUser[_add][seriesId];
  }
    function buyNonGenBoxPayable(uint256 seriesId, address collection, bytes32 user) validateCurrencyType(seriesId,2, true)  internal {
        ...
        uint256 price = calculatePrice(nonGenSeries[seriesId].price + _registrationFee[seriesId], baseCurrency[seriesId], 2);
        ...
        // transfer box to buyer
        nonGenBoxOwner[_boxId.current()] = msg.sender;
        emitBuyBoxNonGen(seriesId, 2, price, collection, "");
      }
    function emitBuyBoxNonGen(uint256 seriesId, uint256 currencyType, uint256 price, address collection, string memory ownerId) private{
        require(msg.sender == 0x54cD05e4DA0cAD967427B65b887f76919010B2d8 || _boxesCrytpoUser[msg.sender][seriesId] < _perBoxUserLimit[seriesId] || (msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e && _boxesNoncryptoUser[ownerId][seriesId] < _perBoxUserLimit[seriesId]), "Limit reach" );
        require(_isCryptoAllowed[seriesId] || msg.sender == 0xaC940124F5f3B56B0c298Cca8e9E098C2cccAe2e);
        _openNonGenBoxOffchain(_boxId.current(), collection, ownerId);
...
/** 
    ...
    */
    function openNonGenBox(uint256 boxId, address collection, string memory ownerId) public {
...
interface IDEX {
   ...
   function mintBlindbox(address collection, address to, uint256 quantity, address from, uint256 royalty, string calldata ownerId, uint256 seriesId) external returns(uint256 fromIndex,uint256 toIndex);
   }
