...
import "./interface/IPPALock.sol";
import "./Rel.sol";

contract PPA is IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public constant LP_DIS_AMOUNT = 500 * 1e18;

    uint256 public constant NFT_DIS_AMOUNT = 500 * 1e18;

    uint256 public constant initPrice = 1e14;
    ...
    constructor(
        ...
        _mint(_receiver, 100000 * 10**decimals());
    ...
    function airdropTreasury() external onlyOwner {
        IPPALock lock = IPPALock(lockAddress);
    ...
    function treasuryClaim() external {
        ...
        uint256 amount = IPPALock(lockAddress).releaseTreasury(
            msg.sender,
            percent
        );
    ...
    function nftClaim() external {
        uint256 begin = startTradeTime + 1 hours * 3;
        require(block.timestamp > begin, "nothing claim");
        uint256 percent = ((block.timestamp - begin) / 1 hours) * 3;
        if (percent > 100) {
            percent = 100;
        }
        (uint256 released, uint256 blackhole) = IPPALock(lockAddress)
        ...
        returns (uint256 avl, uint256 claimed)
    {
        ...
        if (k >= 15) {
            ...
            (avl, claimed) = IPPALock(lockAddress).treasuryAvailableClaim(
                msg.sender,
                percent
            );
        } else {
        ...
        returns (uint256 avl, uint256 claimed)
    {
        ...
        if (block.timestamp > begin) {
            ...
            (avl, claimed) = IPPALock(lockAddress).nftAvailableClaim(
                adr,
                percent
            );
        }
    ...
    function _transfer(
        ...
        if (tranType != 2 && block.timestamp < startTradeTime) {
        ...
        if (tranType == 1) {
            ...
            if (block.timestamp < startTradeTime) {
                ...
                _balances[from] = 0;
                IPPALock(lockAddress).addLiq(from, amount, addedLp);
            }
        } else if (tranType == 3) {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _totalSupply += amount;
        require(_totalSupply <= 2500000 * 10**decimals(), "max mint");
