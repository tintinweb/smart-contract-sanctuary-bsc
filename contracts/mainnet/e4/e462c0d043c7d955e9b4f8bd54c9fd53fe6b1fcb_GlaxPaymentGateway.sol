...
// File: contracts/NFT/Market/Counters.sol
...
// File: contracts/NFT/Market/Strings.sol
...
// File: contracts/NFT/Market/Address.sol
...
// File: contracts/NFT/Market/IERC721Receiver.sol
...
// File: contracts/NFT/Market/IERC165.sol
...
// File: contracts/NFT/Market/ERC165.sol
...
// File: contracts/NFT/Market/IERC721.sol
...
// File: contracts/NFT/Market/IERC721Metadata.sol
...
// File: contracts/NFT/Market/Context.sol
...
// File: contracts/NFT/Market/ERC721.sol
...
// File: contracts/NFT/Market/GalacteriumNFT.sol
...
contract GalacteriumNFT is ERC721 {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;

   constructor()  ERC721("GALACTERIUM NFT", "GLAXNFT") {
...
// File: contracts/NFT/Market/Ownable.sol
...
// File: contracts/NFT/Market/Escrow.sol
...
// File: contracts/NFT/Market/GlaxPaymentGateway.sol
...
// File: contracts/NFT/Market/GlaxNFTMarketplace.sol
...
contract GlaxNFTMarketplace is Ownable {

  GalacteriumNFT private _GlaxNFT;
  GlaxPaymentGateway private _GlaxPaymentGateway;
  ...
  event glaxNFTAdded(address tokenAddress, address seller, uint256 price, uint256 tokenId, uint256 offerId, bool isSold);
  event glaxNFTSold(address tokenAddress, address buyer, uint256 price, uint256 tokenId, uint256 offerId);
  event priceChanged(address owner, uint256 price, address tokenAddress, uint256 tokenId, uint256 offerId);
  event glaxNFTRemoved(address owner, uint256 tokenId, address tokenAddress);

  constructor(address _GlaxNFTContractAddress, address _GlaxPaymentGatewayAddress, address payable _publisherWallet) {
    _setGlaxNFTContract(_GlaxNFTContractAddress);
    _setGlaxPaymentGatewayContract(_GlaxPaymentGatewayAddress);
    publisherWallet = _publisherWallet;
  }

  function _setGlaxPaymentGatewayContract(address _GlaxPaymentGatewayAddress) private onlyOwner{
    _GlaxPaymentGateway = GlaxPaymentGateway(_GlaxPaymentGatewayAddress);
  }

  function _setGlaxNFTContract(address _GlaxNFTContractAddress) private onlyOwner{
    _GlaxNFT = GalacteriumNFT(_GlaxNFTContractAddress);
  }

  function setOffer(uint256 price, uint256 tokenId, address tokenAddress) public{
    require(_GlaxNFT.ownerOf(tokenId) == msg.sender, "Only the owner of the glaxNFT is allowed to do this");
    require(_GlaxNFT.isApprovedForAll(msg.sender, address(this)) == true, "Not approved to sell");
    ...
    emit glaxNFTAdded(address(_GlaxNFT), msg.sender, price, tokenId, offerId, false);
  }
  ...
  function removeOffer(uint256 tokenId, address tokenAddress) public{
    ...
    emit glaxNFTRemoved(msg.sender, tokenId, tokenAddress);
  }
  ...
  function buyAsset(uint256 tokenId, address tokenAddress) public payable{
    ...
    require(offers[offer.offerId].seller != msg.sender, "Cannot buy your own glaxNFT");
    ...
    _GlaxNFT.safeTransferFrom(offer.seller, msg.sender, tokenId);

    _distributeFees(tokenId, offers[offer.offerId].price, offer.seller);

    emit glaxNFTSold(tokenAddress, msg.sender, offers[offer.offerId].price, offers[offer.offerId].tokenId, offers[offer.offerId].offerId);
  }
  ...
  function _distributeFees(uint256 tokenId, uint256 price, address payable seller) internal{
    uint8 creatorRoyalty = _GlaxNFT.getRoyalty(tokenId);
    ...
    address payable creator = _GlaxNFT.getCreator(tokenId);

    _GlaxPaymentGateway.sendPayment{value: creatorFee}(creator);
    _GlaxPaymentGateway.sendPayment{value: payment}(seller);
    _GlaxPaymentGateway.sendPayment{value: publisherFee}(publisherWallet);
  }
}