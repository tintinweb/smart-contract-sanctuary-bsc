...
abstract contract Project {
    ...
    string constant _name = "FaithAI";
    string constant _symbol = "FaithAI";
    ...
    uint256 public swapLpFee          = 10;
    uint256 public swapRewardFee      = 2;
    uint256 public swapRewardBoostFee = 2;
    uint256 public swapMarketing      = 10;
    uint256 public swapTreasuryFee    = 0;
    uint256 public swapBurnFee        = 0;
    uint256 public swapTotalFee       = swapBurnFee + swapMarketing + swapRewardFee + swapLpFee + swapTreasuryFee + swapRewardBoostFee;
...
contract DividendDistributor is IDividendDistributor  {
    ...
    mapping (address => uint256) public holderLockedInfo;
    ...
    uint256 public totalBoostFunds;
    uint256 public totalBoostPaid;

    uint256 public dividendsPerBoostShare;
    ...
    function setShare (address shareholder, uint256 amount, uint256 boostAmount) external override onlyToken {

        if(boostAmount == 0 && amount == 0) {
            ...
            shares[shareholder].boostAmount = boostAmount;
            shares[shareholder].totalShareAmount = amount + boostAmount;

            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].totalShareAmount, dividendsPerShare);

            if(boostAmount > 0) {
                shares[shareholder].totalBoostExcluded = getCumulativeDividends(shares[shareholder].totalShareAmount, dividendsPerBoostShare);
            }
        }
    }
    ...
    function depositBoost() external payable onlyToken {
        ...
        totalBoostFunds = totalBoostFunds.add(amount);
        dividendsPerBoostShare = dividendsPerBoostShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalBoostFunds));
    }
    ...
    function distributeDividend(address shareholder) internal  {
        if(shares[shareholder].amount == 0 && shares[shareholder].boostAmount == 0){ return; }

        uint256 amount = getUnpaidEarnings(shareholder);
        uint256 boostAmount = getUnpaidEarnings(shareholder);
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount, dividendsPerShare);
            rewardsInfo[shareholderRwrd].totalDistributed = rewardsInfo[shareholderRwrd].totalDistributed.add(amount);

            if(boostAmount > 0) {
                shares[shareholder].totalBoostPaid = shares[shareholder].totalBoostPaid.add(boostAmount);
                shares[shareholder].totalBoostExcluded = getCumulativeDividends(shares[shareholder].boostAmount, dividendsPerBoostShare);
                rewardsInfo[shareholderRwrd].totalBoostPaid = rewardsInfo[shareholderRwrd].totalBoostPaid.add(boostAmount);
            }
        }
    }
    ...
    function getUnpaidBoostEarnings(address shareholder) public view returns (uint256) {
        if(shares[shareholder].boostAmount == 0){ return 0; }

        uint256 shareholderTotalDividends = getCumulativeDividends(shares[shareholder].boostAmount, dividendsPerBoostShare);
    ...
    function getTotalBoosted() external onlyToken view returns (uint256) {
    ...
    function getRewardInfo(address _reward) public view returns (
        ...
        if(rewardsInfo[_reward].totalBoostPaid > 0) {
            totalBoostSent = rewardsInfo[_reward].totalBoostPaid;
        } else {
            totalBoostSent = 0;
        }
    }
...
// Stats on the Reward itself
    function getShareholderInfo(address _account) public view returns(
        ...
        uint256 totalRealised,
        uint256 totalBoostRealised,
        uint256 lastClaimTime,
        uint256 nextClaimTime,
        uint256 secondsUntilAutoClaimAvailable,
        uint256 _totalDistributed,
        uint256 _totalBoostDistributed,
        address currentRWRD){
        account = _account;
        ...
        totalBoostRealised = shares[_account].totalBoostPaid;
        ...
        _totalBoostDistributed = totalBoostPaid;
    ...
    // We allow a holder to lock their tokens in the parent contract
    // here we just ensure this state change has been reflected
    function holderLockedAmount(address holder, uint256 amount) external onlyToken {
        holderLockedInfo[holder] = amount;
    }

    function setBoostPercent(uint256 percent) external onlyToken {
 ...
 * MainContract
 */
contract MainContract is Project, IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public distributorGas = 2000000;
    bool public manualDividends = true;
    ...
    constructor () {
        ...
        distributor.setBoostPercent(swapRewardBoostFee);
...
// ***
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBBoost = amountBNB.mul(swapRewardBoostFee).div(totalBNBFee);
...
// *** 
    ...
    function setSwapFees(
        ...
        swapRewardBoostFee = _newSwapRewardBoostFee;
        swapMarketing = _newSwapMarketingFee;
        swapTreasuryFee = _newSwapTreasuryFee;
        swapTotalFee = _newSwapLpFee.add(_newSwapRewardFee).add(_newSwapMarketingFee).add(_newSwapTreasuryFee).add(_newSwapRewardBoostFee);
        feeDenominator = _feeDenominator;

        distributor.setBoostPercent(swapRewardBoostFee);

        require(swapTotalFee < feeDenominator/6, "Fees cannot be more than 16%");
    }
    ...
    function setAllowManualDividends(bool _enabled) external onlyOwner() {
    	manualDividends = _enabled;
    }
    ...
    function getTotalDistributedBoost() public view returns (uint256) {
        return distributor.getTotalBoosted();
    }
