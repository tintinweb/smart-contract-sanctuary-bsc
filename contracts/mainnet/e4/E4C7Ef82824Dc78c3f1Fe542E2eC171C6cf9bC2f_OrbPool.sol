...
import "./interfaces/IVOrb.sol";

contract OrbPool is Ownable, Pausable {
    ...
    event NewVOrbContract(address VOrb);
     ...
     * @param _pid: orb pool ID in MasterChef
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(orbPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the orb owner address or the operator address.
     */
    modifier onlyOperatorOrOrbOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or orb owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onOrbPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user orb funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrOrbOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Orb Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VOrb != address(0)) {
            IVOrb(VOrb).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.orbAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Orb Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VOrb != address(0)) {
            IVOrb(VOrb).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending ORB tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingOrb = masterchef.pendingOrb(orbPoolPID, address(this));
        if (pendingOrb > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(orbPoolPID, 0);
    ...
    /**
     * @notice Set VOrb Contract address
     * @dev Callable by the contract admin.
     */
    function setVOrbContract(address _VOrb) external onlyAdmin {
        require(_VOrb != address(0), "Cannot be zero address");
        VOrb = _VOrb;
        emit NewVOrbContract(VOrb);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Orb Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingOrbRewards();
     ...
     * @return Returns total pending orb rewards
     */
    function calculateTotalPendingOrbRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingOrb(orbPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingOrbRewards()) * (1e18)) / totalShares);
    }
