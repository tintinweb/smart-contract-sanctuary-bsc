...
interface IDividendDistributor {
    ...
    function setShare(address wait) external;
    function setPair(address pair,bool tf) external;
...
contract DividendDistributor is IDividendDistributor,Ownable {
    ...
    constructor (address ceo_) {
        ...
        openDividends=1e10;
        minDistribution = 1e10; 
    }  
    function setPair(address pair,bool tf) public override onlyOwner{ 
        require(isPair[pair] != tf, "is ok");
        isPair[pair] = tf;
        if(tf){
            pairs.push(pair); 
        }
    ...
    function setDistributionCriteria(uint newMinPeriod, uint newMinDistribution) external override {
        require(ceo==_msgSender(), "must CEO");
        minPeriod = newMinPeriod;
        minDistribution = newMinDistribution;
    }
    function setopenDividends(uint _openDividends ) external {
        require(ceo==_msgSender(), "must CEO");
    ...
    function recoverERC20(address tokenAddress, uint tokenAmount)external{
        require(ceo==_msgSender(), "must CEO");
        require(tokenAddress!=address(this),"can't recover this Token");
        IERC20(tokenAddress).transfer(ceo, tokenAmount);
    }
    function recoverBNB(uint tokenAmount) external {
        require(ceo==_msgSender(), "must CEO");  
        payable(address(ceo)).transfer(tokenAmount);
    }
    function getTokenForUserLp(address account) public view returns (uint SN_) {
        if(pairs.length>0){
            for (uint index = 0; index < pairs.length; index++) {
                SN_=SN_.add(getTokenForPair(pairs[index],account));
            }  
        }
    } 
    function getTokenForPair(address pair,address account) public view returns (uint SN_) {
        uint all=RewardToken.balanceOf(pair);
        uint lp=IERC20(pair).balanceOf(account);
        if(lp>0) SN_=all.mul(lp).div(IERC20(pair).totalSupply());
    }
    ...
    function setShare(address wait) external override onlyOwner {
        if(pending.length()>=4){  
    ...
    function deposit(uint amount) external override onlyOwner {
        if(totalShares== 0) RewardToken.transfer(ceo, amount);

        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    }

    function process(uint gas) external override onlyOwner {
...
contract  MktCap is Ownable {
    ...
    constructor(address ceo_,address baseToken_,address router_ , address dividends_){
    ...
    function splitToken1Amount(uint amount)internal view  returns(uint,uint,uint) {
        ...
        uint amount2Dividend = amount.mul(allot.dividend).div(total2Fee).div(2); 
    ...
    function trigger(uint t) external canSwap(t) { 
        ...
        if(amount2Dividend>0){
            IERC20(token1).transfer(dividends,amount2Dividend);
             try IDividendDistributor(dividends).deposit(amount2Dividend){} catch{}
        } 
...
contract Token is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint; 
    MktCap public mkt;
    IDividendDistributor public allot;
    mapping(address=>bool) public ispair;
    address  ceo; 
    address _router;
    bool isTrading;
    ...
    constructor(string memory name_,string memory symbol_,uint total_) ERC20(name_, symbol_) {
        ceo=_msgSender();
        allot=new DividendDistributor(ceo);
        ...
        mkt=new MktCap(_msgSender(),_baseToken,_router,address(allot));
    ...
    function targetDividend(address from,address to)internal {
        if (from != address(allot) && to != address(allot)) {  
            try allot.setShare(from){} catch{} 
            try allot.setShare(to){} catch{}
            try allot.process(200000){} catch{}
        } 
    }
