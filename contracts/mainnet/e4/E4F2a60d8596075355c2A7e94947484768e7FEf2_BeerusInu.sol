...
▄█████████▀    ██████████   ██████████   ███    ███ ████████▀   ▄████████▀       █▀    ▀█   █▀  ████████▀  
                                         ███    ███                                                        
Beerus Inu - $BEERUS
Ready to gain X on your capital with us?
Join Berus Inu meme community.

Token Basic Info:
Name: Beerus Inu
Symbol: BEERUS
Supply: 1,000,000,000,000
Telegram: https://t.me/beerus_inu
Website:  https://beerusinu.net
...
contract BeerusInu is Context, IERC20, Ownable {
    ...
    address public marketingWallet = address(0xe2fa437dadcc0768f413670C8164F6Ac2d865c59);
    address public devWallet = address(0x721a15ae6793c1ba7Ea02ABC7Ea6F85e5f8cDC69);
    ...
    function changeMaxTx(uint256 amount) public onlyOwner {
        require(amount >= 5000000000, "Can't set maxTx Below 0.5%");
        _maxTxAmount = amount * 10**_decimals;
    }

    function changeMaxWallet(uint256 amount) public onlyOwner {
        require(amount >= 5000000000, "Can't set maxWallet Below 0.5%");
        _maxWalletAmount = amount * 10**_decimals;
    }

    function changeBuyFee(uint256 LiqFee, uint256 MktFee, uint256 DevFee, uint256 BurnFee) public onlyOwner {
        require(LiqFee + MktFee + DevFee + BurnFee <= 20, "Can't change BuyFee above 20%");

        buyFee.liquidity = LiqFee;
        buyFee.marketing = MktFee;
        buyFee.dev = DevFee;
        buyFee.burn = BurnFee;
    }

    function changeSellFee(uint256 LiqFee, uint256 MktFee, uint256 DevFee, uint256 BurnFee) public onlyOwner {
        require(LiqFee + MktFee + DevFee + BurnFee <= 20, "Can't change SellFee above 20%");
        sellFee.liquidity = LiqFee;
        sellFee.marketing = MktFee;
        sellFee.dev = DevFee;
        sellFee.burn = BurnFee;
    }
    ...
    function takeBuyFees(uint256 amount, address from) private returns (uint256) {
        ...
        balances[address(this)] += liquidityFeeToken + marketingFeeTokens + devFeeToken;
        balances[address(0x0000)] += burnFeeTokens;
        _tTotal -= burnFeeTokens;

        emit Transfer (from, address(this), marketingFeeTokens + liquidityFeeToken + devFeeToken);
        emit Transfer (from, address(0x0000), burnFeeTokens);
        return (amount -liquidityFeeToken -marketingFeeTokens -devFeeToken -burnFeeTokens);
    }
