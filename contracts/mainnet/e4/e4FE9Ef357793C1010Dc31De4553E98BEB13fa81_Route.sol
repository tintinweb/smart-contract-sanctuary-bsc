...
contract Route {
    ...
    address private _tslgdAddress = 0xE0D2A4d03b168130c93B456a8dc958e9F6b48661;
    ...
    function setBuyTokenContracts(address[] memory _address, bool _state)
        public
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _address.length; i++) {
            buyTokenContracts[_address[i]] = _state;
        }
        return true;
    }
    ...
    function buy(address _address, address _superior) public returns (bool) {
        ...
        uint usdtBalanceBefore = usdtContract.balanceOf(collectionAddress);
        uint tslgdBalanceBefore = tslgdContract.balanceOf(collectionAddress);

        uint256 useUsdt = 10 * 99;
        uint256 useTslgd = 10 * 99;
        if (totalUsdt == 0) {
            totalUsdt = 1 * 10**18;
            useUsdt = totalUsdt;
            useTslgd = 0;
            ...
            useUsdt = totalUsdt.div(2);
            useTslgd = usdtToToken(totalUsdt.sub(useUsdt), _tslgdAddress);
            usdtContract.transferFrom(
                msg.sender,
                address(collectionAddress),
                useUsdt
        ...
        require(
            usdtContract.balanceOf(collectionAddress).sub(usdtBalanceBefore) >=
                useUsdt,
            "Router: USDT Unreceived payment"
        );
        require(
            tslgdContract.balanceOf(collectionAddress).sub(
                tslgdBalanceBefore
            ) >= useTslgd,
            "Router: TSLGD Unreceived payment"
        );
