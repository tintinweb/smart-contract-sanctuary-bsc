...
contract AutomationOrderDecrease is AutomationCompatible, Ownable {
    //---------- Libraries ----------//
    ...
    //---------- Modifiers ----------//
    modifier onlyKeeper() {
        require(allowed.contains(_msgSender()), "AOD: forbidden");
        _;
    }
    ...
    function _validateDecreaseOrder(
        ...
        ) = orderBook.getDecreaseOrder(orderId);
    ...
    function totalOrders() public view returns (uint256) {
        return orderBook.totalDecreaseOrders();
    }

    function orderAt(uint256 _index) public view returns (bytes32) {
        return orderBook.decreaseOrderAt(_index);
    }
    ...
    function checkUpkeep(
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            if (_validateDecreaseOrder(orderId)) {
    ...
    function performUpkeep(
        ...
        require(_validateDecreaseOrder(orderId), "Not executable");
        try
            positionManager.executeDecreaseOrder(orderId, keeperFunds)
        {} catch {
            orderBook.cancelDecreaseOrder(orderId);
        }
    }
