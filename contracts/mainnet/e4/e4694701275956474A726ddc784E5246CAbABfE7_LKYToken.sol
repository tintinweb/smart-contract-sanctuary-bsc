...
contract LKYToken is Context, IERC20, Ownable {
    ...
    string private _symbol = "LKY";
    uint8 private _decimals = 18;
    
    uint256 public _taxFee = 5;
    uint256 private _previousTaxFee;

    address private projectAddress = 0x68b4862b4240ed9aC6Bb4e3Ccb90107BfF927f2e;
    address private liquidityAddress = 0x000000000000000000000000000000000000dEaD;
    ...
    uint256 private _tradingEnabledTime = 1661356800;
    mapping (address => bool) private _isuniswapV2Pair;
    uint256 private _BeginTime = 1661356800;
    ...
    function set_beginTime(uint256 beginTime) public onlyOwner {
    ...
    function _transfer(
        ...
        if(_isuniswapV2Pair[from] ){
            if(block.timestamp <= _tradingEnabledTime) require(_isExcludedFromFee[to]);
        }else if(_isuniswapV2Pair[to]){
            if(block.timestamp <= _tradingEnabledTime) require(_isExcludedFromFee[from]);
        }else{
            if(block.timestamp <= _BeginTime) require(_isExcludedFromFee[from]); 
            takeFee = false ;
        }       
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount ,bool takeFee) private {
        ...
        if(!takeFee){
            ...
            _tOwned[liquidityAddress] = _tOwned[liquidityAddress].add(tFee);
            emit Transfer(sender,recipient, tTransferAmount);
            emit Transfer(sender,liquidityAddress, tFee);
        }
    }   
}