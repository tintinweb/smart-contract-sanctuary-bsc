...
contract WittyP2P is ReentrancyGuard, Ownable {
    ...
    constructor (address _revenueAccount, address _witty, address _pancakeswapV2Router, address _WBNB)  {
        revenueAccount = _revenueAccount;
        token = _witty;
        WBNB = _WBNB;
            
        pancakeswapV2Router = IPancakeRouter02(_pancakeswapV2Router);
    ...
    function getTokenPrice(address[] memory path, uint256 amountA) public view returns(uint256) {
        require(amountA > 0, "Invalid amount");

        address[] memory sellToBuyPath = path;

        uint256[] memory amounts = pancakeswapV2Router.getAmountsOut(amountA, sellToBuyPath);
        return amounts[1];
    }
    ...
    function exchange(uint _tradeID, address _sell, uint _sellAmount, address[] memory _ref, uint8 buychoice) public payable isLock Trade(_tradeID) nonReentrant {
        ...
        address[] memory sellToBuy = new address[](2);
        
        if(_sell == address(this)) {
            sellToBuy[0] = WBNB;
        } else {
            sellToBuy[0] = _sell;
        }
        
        if(_Trade.given == address(this)) {
            sellToBuy[1] = WBNB;
        } else {
            sellToBuy[1] = _Trade.given;
        }
        ...
        uint256 _buyAmount = getTokenPrice(sellToBuy, sellAmount);
    ...
    function updateWBNB(address _WBNB) public onlyOwner {
        WBNB = _WBNB;
    }

    function updatePancakeRouter(address _pancakeswapV2Router) public onlyOwner {
        pancakeswapV2Router = IPancakeRouter02(_pancakeswapV2Router);
    }
