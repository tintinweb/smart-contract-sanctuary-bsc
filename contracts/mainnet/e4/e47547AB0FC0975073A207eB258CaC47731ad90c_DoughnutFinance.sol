...
contract DoughnutFinance {
    ...
    uint256 stoppingTime = block.timestamp + 1 days;
    ...
    mapping (address => uint) private startTime; ///your address
    mapping (address => uint) private stopTime; ///your address
    ...
    function buyDough(address adr) public payable {
       ...
       if(block.timestamp > 0){
        if(addressed[msg.sender]==adr){

            if(block.timestamp > stopTime[msg.sender]){
               uint256 latest = SafeMath.add(SafeMath.mul(SafeMath.div(5,100),getMyDough(adr)),getMyDough(adr));
               boughtDough[adr] = SafeMath.add(latest,eggsBought);
               startTime[adr] = block.timestamp;
               stopTime[adr] = stoppingTime;
            }

            else{
                boughtDough[adr] = SafeMath.add(presentWorth(adr),eggsBought);
                startTime[adr] = block.timestamp;
            }
    ...
    function hatchDough() public{
        ...
        require(block.timestamp > stopTime[msg.sender],"Investment plan still running");
               boughtDough[msg.sender] = SafeMath.add(SafeMath.mul(SafeMath.div(5,100),getMyDough(msg.sender)),getMyDough(msg.sender));
    ...
    function sellDough() public {
        ...
        require(msg.sender==addressed[msg.sender],"You need to have an account");
        require(block.timestamp > stopTime[msg.sender],"Investment plan still running");
        uint256 doughTosell = calculateSell(msg.sender);
        uint256 devwith = SafeMath.mul(refEarning[msg.sender],SafeMath.mul(doughTosell,SafeMath.div(5,100)));
        recAdd.transfer(devwith); //send to developer
        payable (msg.sender).transfer(SafeMath.sub(doughTosell,devwith));
    ...
    function calculateSell(address adr) public view returns(uint256){
        uint256 latest = SafeMath.div(SafeMath.mul(getMyDough(adr),5),100);
        return SafeMath.div(latest,210000);
    }
    ...
    function presentWorth(address adr) public view returns(uint256) {
        uint256 latest = SafeMath.mul((block.timestamp - startTime[adr]),5);
        uint256 latest2 = SafeMath.mul(latest,getMyDough(adr));
        return SafeMath.div(latest2,8640000);
