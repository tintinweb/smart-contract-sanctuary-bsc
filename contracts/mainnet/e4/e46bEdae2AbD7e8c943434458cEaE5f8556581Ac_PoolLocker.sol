...
// File contracts/interfaces/IPoolLocker.sol

pragma solidity =0.8.4;

interface IPoolLocker {
    // uint256 unlockIntervalAmount, // amount each time to unlock.
    // uint256 unlockIntervalTime, // delay time for next unlock.
    function lock(
        address owner,
        address token,
        bool isLpToken,
        uint256 amount,
        uint256 unlockIntervalAmount,
        uint256 unlockIntervalTime,
        uint256 unlockDate
    ...
    //     uint256 lockId,
    //     uint256 newAmount,
    //     uint256 newUnlockIntervalAmount, // amount each time to unlock.
    //     uint256 newUnlockIntervalTime, // delay time for next unlock.
...
// File contracts/PoolLocker.sol

pragma solidity =0.8.4;

contract PoolLocker is IPoolLocker,Ownable {
    ...
    bool public _ownerCanUnlock; // if set false , unlock function is disabled.

    // lock info
    struct Lock {
        uint256 id; // lock index
        address token; // lock the token
        address owner; // address that can unlock
        uint256 amount; // lock total amount

        uint256 unlockIntervalAmount; // amount each time to unlock.
        uint256 unlockIntervalTime; // delay time for next unlock.

        uint256 lockDate; // lock time
        uint256 unlockDate; // next unlock time
    }
    ...
    Lock[] private _locks; // the lock info list
    ...
    constructor() {
        _ownerCanUnlock = true; // if set false , unlock function is disabled.
    }
    ...
    function lock(
        ...
        require(unlockIntervalAmount > 0, "unlockIntervalAmount should be greater than 0");
        require(unlockIntervalTime > 0, "unlockIntervalTime should be greater than 0");
        ...
        if (isLpToken) {
            ...
            require(possibleFactoryAddress != address(0) &&_isValidLpToken(token, possibleFactoryAddress),"This token is not a LP token.");
            id = _lockLpToken(
                owner,
                token,
                possibleFactoryAddress,
                amount,
                unlockIntervalAmount, // amount each time to unlock.
                unlockIntervalTime, // delay time for next unlock.
                unlockDate
            );
        } else {
            id = _lockNormalToken(owner, token, amount, unlockIntervalAmount, unlockIntervalTime,  unlockDate);
        }
        safeTransferFromEnsureExactAmount(token, msg.sender, address(this), amount);
        emit LockAdded(id, token, owner, amount,unlockIntervalAmount, unlockIntervalTime, unlockDate);
        return id;
    }
    ...
    function _lockLpToken(
        ...
        id = _addLock(owner, token, amount,unlockIntervalAmount, unlockIntervalTime, unlockDate);
    ...
    function _addLock(
        ...
        Lock memory newLock = Lock({
            ...
            unlockIntervalAmount:unlockIntervalAmount, // amount each time to unlock.
            unlockIntervalTime:unlockIntervalTime, // delay time for next unlock.
    ...
    // complex next lock date by unlockIntervalTime
    function _getNextLockDate( uint256 unlockDate , uint256 unlockIntervalTime ) public view returns (uint256) {
        // ((block.timestamp - unlockDate)/unlockIntervalTime+1) * unlockIntervalTime + unlockDate;
        return block.timestamp.sub(unlockDate).div(unlockIntervalTime).add(1).mul(unlockIntervalTime).add(unlockDate);
    }

    // release: unlock by unlockIntervalAmount
    function release(uint256 lockId) external override validLock(lockId) {
        Lock storage userLock = _locks[lockId];
        require(userLock.owner == msg.sender, "You are not the owner of this lock");
        require(block.timestamp >= userLock.unlockDate, "It is not time to release");
        require(userLock.amount > 0, "Nothing to release");
        require(userLock.unlockIntervalAmount > 0, "unlockIntervalAmount should be greater than 0");
        require(userLock.unlockIntervalTime > 0, "unlockIntervalTime should be greater than 0");

        CumulativeLockInfo storage tokenInfo = cumulativeLockInfo[userLock.token];
        bool isLpToken = tokenInfo.factory != address(0);

        uint256 unlockAmount = userLock.unlockIntervalAmount;

        if (userLock.amount <= unlockAmount) {
            unlockAmount = userLock.amount; // release the last amount
            userLock.amount = 0;
        } else {
            userLock.amount = userLock.amount - unlockAmount;
        }

        // set next unlock time to ;
        userLock.unlockDate =  _getNextLockDate( userLock.unlockDate , userLock.unlockIntervalTime );
    ...
    // set the unlock function enable or disable.
    function setUnlockFunctionStatus(bool status) public onlyOwner {
         _ownerCanUnlock = status ; //
    }

    // unlock: release all userLock amount
    function unlock(uint256 lockId) external override validLock(lockId) {
        require( _ownerCanUnlock , "Unlock function is closed! Use release function");

        Lock storage userLock = _locks[lockId];
        require(userLock.owner == msg.sender, "You are not the owner of this lock");
        // require(block.timestamp >= userLock.unlockDate, "It is not time to unlock"); // the unlock is not limit time. When online this function is closed!
