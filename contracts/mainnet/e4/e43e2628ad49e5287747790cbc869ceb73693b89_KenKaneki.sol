...
contract KenKaneki is IERC20, Context, Ownable {
    ...
    address private Ken__Kaneki__Marketing_Wallet = 0x271db71bCa58ea0354476f981722934E488Fa005;
    address private Ken__Kaneki__Team_Wallet = 0x271db71bCa58ea0354476f981722934E488Fa005;
    
    string private constant _symbol = "KANEKI";
    string private constant _name = "Ken Kaneki";

    uint256 public Ken__Kaneki__max_BuyLimit = 20000000 * 10**9;
    uint256 public Ken__Kaneki__max_SellLimit = 5000000 * 10**9;

    uint256 public Ken__Kaneki__max_WalletLimit = 20000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Ken__Kaneki__Marketing_Wallet] = true;
        _isExcludedFromFee[Ken__Kaneki__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Ken__Kaneki__Marketing_Wallet] = true;
        allowedTransfer[Ken__Kaneki__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Ken__Kaneki__max_BuyLimit, "You are exceeding Ken__Kaneki__max_BuyLimit");
            require(balanceOf(to) + amount <= Ken__Kaneki__max_WalletLimit, "You are exceeding Ken__Kaneki__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Ken__Kaneki__max_SellLimit, "You are exceeding Ken__Kaneki__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Ken__Kaneki__max_WalletLimit, "You are exceeding Ken__Kaneki__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Ken__Kaneki__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Ken__Kaneki__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Ken__Kaneki__Update_MarketingWallet(address newWallet) external onlyOwner{
        Ken__Kaneki__Marketing_Wallet = newWallet;
    }
    
    function Ken__kaneki__Update_TeamWallet(address newWallet) external onlyOwner{
        Ken__Kaneki__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Ken__Kaneki__max_BuyLimit = maxBuy * 10**decimals();
        Ken__Kaneki__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Ken__Kaneki__Max_Walletlimit(uint256 amount) external onlyOwner{
        Ken__Kaneki__max_WalletLimit = amount * 10**decimals();
    }
