...
contract TDelegatorInterface is TDelegationStorage {
    /**
...
contract TDelegateInterface is TDelegationStorage {
    /**
...
contract TENTrollerErrorReporter {
    ...
    /**
      * @dev use this when reporting a known error  market orfrom the money a non-upgradeable collaborator
      */
 ...
 * @title Exponential module for storing fixed-precision decimals
 * @author TENLend
 * @dev Legacy contract for comptatibility reasons with existing contracts that still use MathError
 * @notice Exp is a struct which stores decimals with a fixed precision of 18 decimal places.
 *         Thus, if we wanted to store the 5.1, mantissa would store 5.1e18. That is:
 *         `Exp({mantissa: 5100000000000000000})`.
 */
...
/**
 * @title TENLend's TErc20 Contract
 * @notice TTokens which wrap an EIP-20 underlying
 * @author TENLend
 */
contract TErc20 is TToken, TErc20Interface {
    /**
    ...
    function initialize(address underlying_,
                        TENTrollerInterface tentroller_,
                        InterestRateModel interestRateModel_,
                        uint initialExchangeRateMantissa_,
                        string memory name_,
                        string memory symbol_,
        ...
        super.initialize(tentroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_);
     ...
     */
    function liquidateBorrow(address borrower, uint repayAmount, TTokenInterface tTokenCollateral) external returns (uint) {
        (uint err,) = liquidateBorrowInternal(borrower, repayAmount, tTokenCollateral);
        return err;
    }
...
// File: contracts\TErc20Delegate.sol

pragma solidity ^0.5.16;


/**
 * @title TENLend's TErc20Delegate Contract
 * @notice TTokens which wrap an EIP-20 underlying and are delegated to
 * @author TENLend
 */
contract TErc20Delegate is TErc20, TDelegateInterface {
    /**
