...
contract BikeNRunStaking is ReentrancyGuard, Pausable, Ownable {
    using SafeMath for uint256;
    ...
    updateReward(msg.sender)
    {
		...
		require(block.timestamp >= lockedReward, "Cannot withdraw before period finish");
    ...
    updateReward(address(0))
    {
        ...
        lockedReward = block.timestamp.add(rewardsDuration);
    ...
    function setLockDuration(uint256 _lockDuration) external onlyOwner {
        require(
            _lockDuration > 86400,
            "Locked time must be more than 1 days"
        );
        lockedReward = block.timestamp.add(_lockDuration);
    }
