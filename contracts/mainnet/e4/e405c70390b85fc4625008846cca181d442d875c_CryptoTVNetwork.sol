...
contract CryptoTVNetwork is Context, IERC20, Ownable {
    ...
    string private _name     = "Crypto TV Network";
    ...
    uint256 public totalFees = taxFee + liquidityFee + marketingFee + burnFee;

    address public marketingWallet = 0xfaD26f2Da33A9367ae93Ba080b00ff30Bd2aE29b;
   ...
   function setAntiBotStatus (bool status) external onlyOwner {
    ...
    function setWhiteListedContract (address account, bool value) external onlyOwner {
        require(account != address(this), "Owner cannot set native token contract");
    ...
    function removeAllFee() private {
        if(taxFee == 0 && liquidityFee == 0 && marketingFee == 0 && burnFee == 0) return;
    ...
    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        taxFee = _taxFee;
        totalFees = taxFee + marketingFee + liquidityFee + burnFee;
    ...
    uint256 private maxTransactionRateBuy     = 50;
    uint256 private maxTransactionRateSell    = 50;
