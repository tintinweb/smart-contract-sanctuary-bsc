...
contract DOTC is Configurable {
    ...
    function make(SMake memory make_,bool isPrivate) virtual external nonReentrant returns(uint makeID) { 
        ...
        if (make_.adPex>0)
            IERC20(address(config[_rewardToken_])).safeTransferFrom(msg.sender, address(config[_feeTo_]), make_.adPex);
        makeID = makesN;
    ...
    function _payFee(uint takeID, address asset, uint vol) internal returns(uint fee) {
        ...
        IERC20(tokenA).safeTransfer(address(config[_feeTo_]), amt1);
        uint feeBuf = _feeBuf();
    ...
    function _addLiquidity(uint value, uint amount) internal {
        ...
        //(, uint amt,) = router.addLiquidity(tokenA, rewardToken, value, amount, 0, 0, address(this), now);
        //if(amount > amt)
        //    IERC20(rewardToken).safeTransferFrom(address(this), address(config[_mine_]), amount - amt);
    }
