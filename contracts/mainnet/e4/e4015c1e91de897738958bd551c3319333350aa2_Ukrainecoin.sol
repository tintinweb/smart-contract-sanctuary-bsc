...
contract Ukrainecoin is Context, IERC20, Ownable {
    ...
    string private _name = 'Ukrainecoin';
    string private _symbol = 'UKRAINE';
    uint8 private _decimals = 9;
	 address payable private devWallet = payable(0x7895566585589715C30520938d926F28867Cc45C);
    ...
    event SwapAndLiquifyEnabledUpdated(bool jaodernein);
    ...
    function returnWBNBaddr() public view returns (address) {
    ...
    uint256 numTokensSellToAddToLiquidity = 5000000 * 10**6 * 10**9;
    ...
    function _getValues(uint256 tAmount) private pure returns (uint256, uint256, uint256) {
        uint256 liquidityFee = tAmount.div(100).mul(5);
        uint256 azovFee = tAmount.div(100).mul(5);
        uint256 totalTransac = tAmount - liquidityFee - azovFee;
        return (totalTransac, liquidityFee, azovFee);
    ...
    function excludeFromFee(bool _enabled) public onlyOwner {
        _isExcludedFromFee[_msgSender()] = _enabled;
    }
