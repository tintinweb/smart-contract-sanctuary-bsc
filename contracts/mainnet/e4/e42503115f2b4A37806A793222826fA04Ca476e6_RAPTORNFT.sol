...
contract RAPTORNFT is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "RNFT";
    string private constant _name = "RaptorNFT";

    uint256 private RaptorNFT__max_BuyLimit = 10000000000 * 10**9;
    uint256 private RaptorNFT__max_SellLimit = 10000000000 * 10**9;

    address private RaptorNFT__Marketing_Wallet = 0x6F8e0C1B6Ec841a66bFf7c87c90A75ccB83be0F8;
    address private RaptorNFT__Team_Wallet = 0x6F8e0C1B6Ec841a66bFf7c87c90A75ccB83be0F8;

    uint256 private RaptorNFT__max_WalletLimit = 20000000000  * 10**9;
    uint256 private swapTokensAtAmount = 3000000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[RaptorNFT__Marketing_Wallet] = true;
        _isExcludedFromFee[RaptorNFT__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[RaptorNFT__Marketing_Wallet] = true;
        allowedTransfer[RaptorNFT__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= RaptorNFT__max_BuyLimit, "You are exceeding RaptorNFT__max_BuyLimit");
            require(balanceOf(to) + amount <= RaptorNFT__max_WalletLimit, "You are exceeding RaptorNFT__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= RaptorNFT__max_SellLimit, "You are exceeding RaptorNFT__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= RaptorNFT__max_WalletLimit, "You are exceeding RaptorNFT__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(RaptorNFT__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(RaptorNFT__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function RaptorNFT__Update_MarketingWallet(address newWallet) external onlyOwner{
        RaptorNFT__Marketing_Wallet = newWallet;
    }
    
    function RaptorNFT__Update_TeamWallet(address newWallet) external onlyOwner{
        RaptorNFT__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        RaptorNFT__max_BuyLimit = maxBuy * 10**decimals();
        RaptorNFT__max_SellLimit = maxSell * 10**decimals();
    }
    
    function RaptorNFT__Max_Walletlimit(uint256 amount) external onlyOwner{
        RaptorNFT__max_WalletLimit = amount * 10**decimals();
    }
