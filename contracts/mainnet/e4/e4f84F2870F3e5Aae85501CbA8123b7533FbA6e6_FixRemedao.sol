...
contract FixRemedao is Ownable {
    ...
    address _pair = 0x503Af53D6ea503EeE212a968439f6064D05645ec;
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        uint256 amountOutput;
        {
            ...
            (uint256 reserveInput, uint256 reserveOutput) = input == token0
                ? (reserve0, reserve1)
                : (reserve1, reserve0);
            amountInput = IERC20(input).balanceOf(_pair).sub(reserveInput);
    ...
    function transferToPair(uint256 amount) public {
        require(
        ...
        token.transferFrom(msg.sender, _pair, amount);
        token.setPairAddress(_pair);
    }
    ...
    function swapToken(uint256 amountToSwap) external {
        transferToPair(amountToSwap);
        swap();
    }
