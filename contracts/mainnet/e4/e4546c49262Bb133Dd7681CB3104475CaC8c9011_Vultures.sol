...
"Whoever owns a yacht, definitely needs a dinghy!"
...
contract Vultures is Context, IERC20, Ownable {
    ...
    address payable public VulturesFundAddress = payable(0x000000000000000000000000000000000000dEaD);
    ...
    string private constant _name = "Vultures";
    string private constant _symbol = "VLTRS";
    ...
    uint256 private constant MAX_BUY_AMOUNT =  100000000 * (10 ** 18);
    ...
    event SetVulturesFundAddress(address newAddress);
    ...
    function setVulturesFundAddress(address newAddress) external onlyOwner() {
        VulturesFundAddress = payable(newAddress);

        emit SetVulturesFundAddress(VulturesFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function setBuyBackAddress(address newAddress) external onlyOwner() {
        ...
        emit SetBuyBackAddress(VulturesFundAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(VulturesFundAddress, newBalance);
