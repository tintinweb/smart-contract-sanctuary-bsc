...
contract USDFIPair is IUSDFIPair, UniswapERC20 {
  ...
  */
  ...
  function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
    require(balance0 <= uint112(- 1) && balance1 <= uint112(- 1), "USDFIPair: OVERFLOW");
  ...
  function burn(address to) external lock returns (uint amount0, uint amount1) {
    ...
    // using balances ensures pro-rata distribution
    require(amount0 > 0 && amount1 > 0, "USDFIPair: INSUFFICIENT_LIQUIDITY_BURNED");
  ...
  function _swap(uint amount0Out, uint amount1Out, address to, bytes memory data, address protocol) internal lock {
    ...
    require(amount0In > 0 || amount1In > 0, "USDFIPair: INSUFFICIENT_INPUT_AMOUNT");
    ...
    {// scope for protocol fee management
      ...
      if (protocolInputFeeAmount > 0) {
        if (amount0In > 0) {
          address _token0 = token0;
          _safeTransfer(_token0, protocol, amount0In.mul(protocolInputFeeAmount) / (feeDenominator ** 2));
          balance0 = IERC20(_token0).balanceOf(address(this));
        }
        if (amount1In > 0) {
          address _token1 = token1;
  ...
  function sync() external lock {
    ...
    require(token0Balance != 0 && token1Balance != 0, "USDFIPair: liquidity ratio not initialized");
