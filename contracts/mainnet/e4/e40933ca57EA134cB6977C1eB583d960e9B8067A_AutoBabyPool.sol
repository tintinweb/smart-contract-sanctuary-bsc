...
contract AutoBabyPool is Ownable, Pausable {
    ...
    uint256 public constant MAX_WITHDRAW_FEE = 500; // 1%
    uint256 public constant MAX_STAKE_FEE = 500; // 1%

    uint256 public performanceFee = 0; // 2%
    uint256 public callFee = 0; // 0.25%
    uint256 public stakeFee = 500; // 1%
    ...
    constructor(
        ...
        require(address(_masterchef) != address(0), "_masterchef should not be address(0)");
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_masterchef), 2**256 - 1);
    }
     ...
     */
    function deposit(uint256 _amount) external whenNotPaused notContract nonReentrant("deposit") {
        ...
        if (user.amount != 0) {
            ...
            if (pending != 0) {
                token.safeTransferFrom(rewardHolder, msg.sender, pending);
                user.claimed += pending;
            }
        }

        // Auto Compounding Share calculation
        ...
        if (stakeFee != 0) {
            ...
            token.safeTransfer(treasury, stakeFeeAmount / 5);
            token.safeTransfer(jackPot, stakeFeeAmount - (stakeFeeAmount / 5) - (stakeFeeAmount * 60) / 100);
        ...
        if (stakeFeeAmount != 0) {
            token.safeTransfer(rewardHolder, (stakeFeeAmount * 60) / 100);
        ...
        user.babyAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        _earn();

        // Xpot share calculation
     ...
     * @dev Only possible when contract not paused.
     */
    function harvest() external notContract whenNotPaused nonReentrant("harvest") {
    ...
    /**
     * @notice Sets stake fee
     * @dev Only callable by the contract admin.
     */
    function setStakeFee(uint256 _stakeFee) external onlyAdmin {
        require(_stakeFee <= MAX_STAKE_FEE, "stakeFee cannot be more than MAX_STAKE_FEE");
        stakeFee = _stakeFee;
    }
     ...
     */
    function calculateTotalPendingBabyRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingCake(0, address(this));
        amount = amount + available();
     ...
     */
    function withdraw(uint256 _shares) public notContract nonReentrant("withdraw") {
        UserInfo storage user = userInfo[msg.sender];
