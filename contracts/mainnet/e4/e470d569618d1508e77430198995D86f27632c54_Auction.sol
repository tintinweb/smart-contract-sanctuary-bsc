...
contract Auction is Configurable {
    ...
    function isLimit(string memory name) public view returns(bool) {
        if (reserves[name])
            return true;
        return false;
    }
	...
	function bid(string memory name,uint wlNum,bytes32[] calldata _merkleProof) payable public nonReentrant {
        ...
        require(!isLimit(name),"name limit");
    ...
    function setReserves(string [] calldata names,bool[] calldata isReserves) public governance {
