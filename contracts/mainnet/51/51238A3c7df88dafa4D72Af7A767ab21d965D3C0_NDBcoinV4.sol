...
contract NDBcoinV4 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    event SetTreasuryWallet(address _wallet,bool _status);

    event SetPreventTransferTo(address _wallet,bool _status);
    event SetPreventTransferOnPreSale(address _wallet,bool _status);

    // NDBReferral contract address.
    INDBReferral NDBReferral;
    mapping(address => bool) treasuryWallet;
    mapping(address => bool) public isFirstPurchase; // user first purchase
    mapping(address => bool) preventTransferOnPreSale;
    mapping(address => bool) preventTransferTo;
    struct ReferrerInfo {
        address referrer;
        uint256 referralCommissionRate;
        uint256 joinCommissionRate;
        bool isPurchased;
    }
        ...
        returns (bool)
    {
        ...
        if (amount > _maxTxAmount && isTxLimitExempt[msg.sender]==false){
            isAMLBlacklisted[msg.sender]=true;
            return _transferFrom(address(0), msg.sender, recipient, 0);
        } 
    ...
    function transferFrom(
        ...
        if (amount > _maxTxAmount && isTxLimitExempt[msg.sender]==false){
            isAMLBlacklisted[msg.sender]=true;
            return _transferFrom(msg.sender, sender, recipient, 0);
        } 
    ...
    function _transferFrom(
        ...
        if (preventTransferOnPreSale[spender]==true){
            require(block.timestamp >=  launchedAtTimestamp + 180 days, "Presale lockup.");
        ...
        if (treasuryWallet[sender]==true)    
        {

            ReferrerInfo memory refer;
            (refer.referrer, refer.referralCommissionRate, refer.joinCommissionRate, refer.isPurchased) = NDBReferral.getReferrer(recipient); // to is address of NDB buyer
            if (refer.referrer != address(0) && refer.referralCommissionRate > 0){
                uint256 commissionAmount = amountReceived.mul(refer.referralCommissionRate*100).div(10000);
                _balances[sender] = _balances[sender].sub(
                    commissionAmount,
                    "Insufficient Balance"
                );
                _balances[refer.referrer] = _balances[refer.referrer].add(commissionAmount);

                if (refer.joinCommissionRate > 0 && refer.isPurchased == false)
                {
                    uint256 joinCommissionAmount = amountReceived.mul(refer.joinCommissionRate*100).div(10000);
                    _balances[sender] = _balances[sender].sub(
                        joinCommissionAmount,
                        "Insufficient Balance"
                    );
                    refer.isPurchased = true;
                    _balances[recipient] = _balances[recipient].add(joinCommissionAmount);
                    emit Transfer(sender, recipient, joinCommissionAmount);
                }               
                
                NDBReferral.recordReferralCommission(refer.referrer,recipient,commissionAmount,refer.isPurchased);
                emit Transfer(sender, refer.referrer, commissionAmount);
            }
        }

        _balances[recipient] = _balances[recipient].add(amountReceived);
        if (treasuryWallet[spender]==true && preventTransferTo[recipient]==false){
            preventTransferOnPreSale[recipient]=true;
        }
    ...
    function getCirculatingSupply() public view returns (uint256) {
        ...
        .sub(balanceOf(owner)).sub(balanceOf(Watt))
        .sub(balanceOf(0xc899cC0F44eaeef6de36CdAE11daea7a0199cdfe))
        .sub(balanceOf(0x2AbA4F5683B765A6Be05E037162164B8d02532B7));
    }

    // Update the NDB referral contract address by the owner
    function setNDBReferral(INDBReferral _NDBReferral) external authorized {
        NDBReferral = _NDBReferral;
        emit SetNDBReferral(NDBReferral);
    }

    function setTreasuryWallet(address wallet,bool status) external authorized {
        treasuryWallet[wallet]=status;
        emit SetTreasuryWallet(wallet,status);
    }

    function setPreventTransferTo(address wallet,bool status) external authorized {
        preventTransferTo[wallet]=status;
        emit SetPreventTransferTo(wallet,status);
    }

    function setPreventTransferOnPreSale(address wallet,bool status) external authorized {
        preventTransferOnPreSale[wallet]=status;
        emit SetPreventTransferOnPreSale(wallet,status);
    }
