...
contract ExercisecAFA {
    ...
    address public circulatingAFAContract;
    ...
    constructor(
        ...
        // address _usdc,
        address _treasury,
        address _circulatingAFAContract
    ) {
        owner = msg.sender;
    ...
    function setTokens(
        ...
        tokenUsdt = _token;
        AFA = _afa;
        circulatingAFAContract = _circul;
        return true;
    }
    ...
    function redeemable(Term memory _info) internal view returns (uint256) {
        return (ICirculatingAFA(circulatingAFAContract).AFACirculatingSupply().mul(_info.percent).mul(percentDenominator)).sub(_info.claimed);
    }
...
interface ICirculatingAFA {
    function AFACirculatingSupply() external view returns (uint256);
}
