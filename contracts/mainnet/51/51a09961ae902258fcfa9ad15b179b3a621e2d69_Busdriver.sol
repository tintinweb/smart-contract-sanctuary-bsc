...
A simple and stress-free way to grow your BUSD without dangerous function in the contract.
Enjoy Rewards in Busd from 5% Up to 25% Thanks to the NIGHT SHIFT function!
...
contract Busdriver {
    ...
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL = 9; // compound days, for no tax withdrawal.
    ...
    constructor(address _dev1, address _dev2, address _mkt) {
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256){
        return calculateTrade(NIGHT_SHIFT * eggs, marketEggs, getBalance());
    }
    ...
    function calculateEggSellForYield(uint256 eggs,uint256 amount) public view returns(uint256){
        return calculateTrade(NIGHT_SHIFT * eggs,marketEggs, getBalance() .add(amount));
    }
    ...
    function BONUS_DAILY_COMPOUND_BONUS_MAX_TIMES(uint256 value) external {
        ...
        function SET_NIGHT_SHIFT(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 5); /** MAX NIGHT_SHIFT X5 **/
        NIGHT_SHIFT = value;
    }
...
/**
BUSDriver
 Jump on the bus, get fuel, take stops to collect BUSD. 
  5% Up to 25% daily daily Rate Thanks to the Night Shift Function.
  10% Referral Bonus, will go directly to referrer wallet.
  1.0% stacking compound bonus every 12 hrs, max of 5 days. (10%)
  48 hours cut off time.
  10 BUSD minimum investment.
  15,000 BUSD max deposits per wallet.
  90% feedback for withdrawals that will be done not after 9 consecutive compounds.(12.5%)
