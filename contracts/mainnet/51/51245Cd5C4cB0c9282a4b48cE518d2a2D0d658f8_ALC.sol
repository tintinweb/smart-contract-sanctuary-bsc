...
contract ALC is Ownable{
    ...
    function testin(uint _amount) external view returns(uint[] memory _result){
        address[] memory path = new address[](2);
        path[0] = SSID;
        path[1] = BUSD;
        _result = Uni(uniRouter).getAmountsIn(_amount, path);
    }

    function testout(uint _amount) external view returns(uint[] memory _result){
        address[] memory path = new address[](2);
        path[0] = SSID;
        path[1] = BUSD;
        _result = Uni(uniRouter).getAmountsOut(_amount, path);
    }
