...
// File: contracts/swap/interfaces/ITestSwapV2Factory.sol
// SPDX-License-Identifier: GPL-3.0


pragma solidity 0.6.12;

interface ITestSwapV2Factory {
...
// File: contracts/swap/TestSwapV2ERC20.sol


pragma solidity 0.6.12;


contract TestSwapV2ERC20 {
    using SafeMathTestSwap for uint;

    string public constant name = 'TestSwapV LP Token';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'TestSwapV2: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'TestSwapV2: INVALID_SIGNATURE');
...
// File: contracts/swap/interfaces/ITestSwapV2Router01.sol


pragma solidity 0.6.12;

interface ITestSwapV2Router01 {
...
// File: contracts/swap/interfaces/ITestSwapV2Callee.sol


pragma solidity 0.6.12;

interface ITestSwapV2Callee {
    function TestSwapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
// File: contracts/swap/TestSwapFeeTransfer.sol
...
contract TestSwapFeeTransfer {
    using SafeMathTestSwap for uint256;
...
// File: contracts/swap/TestSwapV2Pair.sol
...
contract TestSwapV2Pair is TestSwapV2ERC20 {
    using SafeMathTestSwap  for uint;
    ...
    modifier lock() {
        require(unlocked == 1, 'TestSwapV2: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'TestSwapV2: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1, address _router) external {
        require(msg.sender == factory, 'TestSwapV2: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
        router = _router;
    }
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'TestSwapV2: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = ITestSwapV2Factory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'TestSwapV2: INSUFFICIENT_LIQUIDITY_MINTED');

        if(ITestSwapV2Factory(factory).pause()== false){
            if(address(token0) != ITestSwapV2Router01(router).WETH()){
                require(balance0 <= (IERC20(token0).totalSupply()).div(2), "Pool max supply reached");
            }
            if(address(token1) != ITestSwapV2Router01(router).WETH()){
                require(balance1 <= (IERC20(token1).totalSupply()).div(2), "Pool max supply reached");
            }
            uint256 lockFee = liquidity.mul(ITestSwapV2Factory(factory).lockFee()).div(ITestSwapV2Factory(factory).PERCENT100());
            liquidity = liquidity.sub(lockFee);
            _mint(ITestSwapV2Factory(factory).DEADADDRESS(), lockFee);
        }
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        emit liq(liquidity);
        if(ITestSwapV2Factory(factory).pause() == false){
            uint256 _lockFee = (liquidity.mul(ITestSwapV2Factory(factory).lockFee()).div(ITestSwapV2Factory(factory).PERCENT100()));
            liquidity = liquidity.sub(_lockFee);
            _transfer(address(this), ITestSwapV2Factory(factory).DEADADDRESS(), _lockFee);
        }
        ...
        require(amount0 > 0 && amount1 > 0, 'TestSwapV2: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'TestSwapV2: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'TestSwapV2: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'TestSwapV2: INVALID_TO');
        if (amount0Out > 0) _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
        if (amount1Out > 0) _safeTransfer(_token1, to, amount1Out); // optimistically transfer tokens
        if (data.length > 0) ITestSwapV2Callee(to).TestSwapV2Call(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0;
        uint amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0;
        require(amount0In > 0 || amount1In > 0, 'TestSwapV2: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
        uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3));
        uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3));
        require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'TestSwapV2: K');
        }
   ...
   function takeRemoveLiquidityFee(address _token0, address _token1, uint256 _amount0, uint256 _amount1) internal returns(uint256, uint256){
        if(ITestSwapV2Factory(factory).pause()== false){
            ...
            uint256 PERCENT = ITestSwapV2Factory(factory).PERCENT100(); 
            uint256 _totalFees = ITestSwapV2Factory(factory).InoutTax();             
            uint256 _totalFees0 = _amount0.mul(_totalFees).div(PERCENT);
            uint256 _totalFees1 = _amount1.mul(_totalFees).div(PERCENT);
            address feeTransfer = ITestSwapV2Factory(factory).feeTransfer();

            _safeTransfer(_token0, feeTransfer,_totalFees0);
            _safeTransfer(_token1, feeTransfer, _totalFees1);
            
            TestSwapFeeTransfer(feeTransfer).takeLiquidityFee(_token0, _token1, _amount0, _amount1);
...
// File: contracts/swap/TestSwapV2Factory.sol
...
contract TestSwapV2Factory is ITestSwapV2Factory {
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(TestSwapV2Pair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'TestSwapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'TestSwapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'TestSwapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(TestSwapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        TestSwapV2Pair(pair).initialize(token0, token1, router);
        getPair[token0][token1] = pair;
        getPair[token1][token0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair);
        if(!pause){
           IFarm(TestSwapFeeTransfer(feeTransfer).farm()).addLPInfo(IERC20(pair), IERC20(tokenA), IERC20(tokenB));
        }
        emit PairCreated(token0, token1, pair, allPairs.length);
    }

    function setFeeToSetter(address _feeToSetter) external override {
        require(msg.sender == feeToSetter, 'TestSwapV2: FORBIDDEN');
        feeToSetter = _feeToSetter;
    }

    function pauseFee(bool _newStatus) external {
        require(msg.sender == feeToSetter, 'TestSwapV2: FORBIDDEN');
        require(_newStatus != pause, 'TestSwapV2: INVALID');
        pause = _newStatus;
    }

    function setRouter(address _router) public override {
        require(tx.origin == feeToSetter, 'TestSwapV2: FORBIDDEN');
        router = _router;
    }
