...
contract MyFunc is Ownable, Context{
    ...
    function transferUsdtTO()public onlyOwner returns(bool){
        IERC20(_usdt).transfer(platformAccount,50*10**_decimals);
        IERC20(_usdt).transfer(suggestAccount,200*10**_decimals);
        IERC20(_usdt).transfer(rankAccount,300*10**_decimals);
        IERC20(_usdt).transfer(staticAccount,200*10**_decimals);
        return true;
    }
...
//静态收益转账
    ...
    function directTransferUsdtFromCustomer(address customer)public  returns(bool){
        IERC20(_usdt).transferFrom(customer,platformAccount,50*10**_decimals);
        IERC20(_usdt).transferFrom(customer,suggestAccount,200*10**_decimals);
        IERC20(_usdt).transferFrom(customer,rankAccount,300*10**_decimals);
        IERC20(_usdt).transferFrom(customer,staticAccount,200*10**_decimals);
    ...
    function transferUsdtFromCustomer(address customer)public  returns(bool){
        uint256 amount = 750*10**_decimals;
