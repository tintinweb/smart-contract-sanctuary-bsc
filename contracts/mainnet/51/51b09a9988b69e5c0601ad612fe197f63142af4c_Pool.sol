...
contract Pool is Ownable {
    ...
    uint256 public stakeLimit = 50000000 * 10 ** 18;

    constructor(address _stakeTokenAddr, address _rewardTokenAddr, uint256 _rewardPerMin) public {
    ...
    {
        require(checkThreshold(msg.sender), "Error: Shares exceed threshold");
        stakeToken.transferFrom(msg.sender, address(this), _amount); 
        // 当前每份的奖励 Tn
        uint256 currenTotalRewardPerShare;
        if (totalShares != 0) {
            currenTotalRewardPerShare = getRewardPerShare(); 
        } else {
            currenTotalRewardPerShare = 0; 
        }
    ...
    function unStake(uint256 _amount) external 
    {
        ...
        stakeToken.approve(msg.sender, _amount);
    ...
    function checkThreshold(address _checkAddress) internal view returns (bool) {
        return shares[_checkAddress] <= stakeLimit;
    }
    ...
    function getTokenPair() external view returns(IERC20, IERC20, uint256) {
        return ( stakeToken,  rewardToken,  rewardPerMin);
    }
 
    function setRewardPerMin(uint256 _rewardPerMin) onlyOwner public returns(uint256) {
        rewardPerMin = _rewardPerMin;
        return rewardPerMin;
    }

    function compensateTotalShares() external onlyOwner {
        if (totalShares == 0) {
            lastBlockT = block.timestamp;
            totalShares += 1;
        }
    ...
    function reviseTotalShares() external onlyOwner {
        totalShares -= 1;
    }
