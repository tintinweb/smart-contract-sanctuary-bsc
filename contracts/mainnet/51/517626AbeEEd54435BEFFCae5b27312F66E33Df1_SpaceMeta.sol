...
// File: contracts/BabyTokenDividendTracker.sol
...
contract SpaceMeta is ERC20, Ownable, BaseToken {
  using SafeMath for uint256;
  ...
  mapping(address => bool) private _isExcludedFromMaxWalletBalance;

  uint256 public maxTransferAmount = 0;
  mapping(address => bool) private _isExcludedFromMaxTransferAmount;
  ...
  event ExcludeFromMaxWalletBalance(address indexed account, bool isExcluded);
  ...
  constructor(
    ...
    address[5] memory addrs, // reward, router, marketing wallet, team, dividendTracker
    uint256[4] memory feeSettings, // rewards, liquidity, marketing, team
    uint256 minimumTokenBalanceForDividends_
  ) payable ERC20(name_, symbol_) {
    rewardToken = addrs[0];
    _marketingWalletAddress = addrs[2];
    _teamWalletAddress = addrs[3];
    require(
      msg.sender != _marketingWalletAddress,
      'Owner and marketing wallet cannot be the same'
    );
    require(
      msg.sender != _teamWalletAddress,
      'Owner and team wallet cannot be the same'
    );

    tokenRewardsFee = feeSettings[0];
    liquidityFee = feeSettings[1];
    marketingFee = feeSettings[2];
    teamFee = feeSettings[3];
    totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(
      teamFee
    );
    ...
    dividendTracker = BABYTOKENDividendTracker(payable(Clones.clone(addrs[4])));
  ...
  function excludeFromMaxWalletBalance(address account, bool excluded)
    external
    onlyOwner
  {
    require(
      _isExcludedFromMaxWalletBalance[account] != excluded,
      "FRESH BABYTOKEN: Account is already the value of 'excluded'"
    );
    _isExcludedFromMaxWalletBalance[account] = excluded;

    emit ExcludeFromMaxWalletBalance(account, excluded);
  }
  ...
  function excludeFromMaxTransferAmount(address account, bool excluded)
    external
    onlyOwner
  {
    require(
      _isExcludedFromMaxTransferAmount[account] != excluded,
      "FRESH BABYTOKEN: Account is already the value of 'excluded'"
    );
    _isExcludedFromMaxTransferAmount[account] = excluded;

    emit ExcludeFromMaxTransferAmount(account, excluded);
  }

  function updateDividendTracker(address newAddress) public onlyOwner {
    require(
      newAddress != address(dividendTracker),
      'FRESH BABYTOKEN: The dividend tracker already has that address'
    );
    ...
    require(
      ...
      'FRESH BABYTOKEN: The new dividend tracker must be owned by the BABYTOKEN token contract'
    );
  ...
  function updateUniswapV2Router(address newAddress) public onlyOwner {
    require(
      ...
      'FRESH BABYTOKEN: The router already has that address'
    );
    ...
    onlyOwner
  {
    require(
      pair != uniswapV2Pair,
      'FRESH BABYTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs'
    );
  ...
  function _setAutomatedMarketMakerPair(address pair, bool value) private {
    require(
      ...
      'FRESH BABYTOKEN: Automated market maker pair is already set to that value'
    );
    automatedMarketMakerPairs[pair] = value;
  ...
  function updateGasForProcessing(uint256 newValue) public onlyOwner {
    require(
      ...
      'FRESH BABYTOKEN: gasForProcessing must be between 200,000 and 500,000'
    );
    require(
      newValue != gasForProcessing,
      'FRESH BABYTOKEN: Cannot update gasForProcessing to same value'
    );
  ...
  function _transfer(
    ...
    if (
      ...
      !_isExcludedFromMaxTransferAmount[from] &&
      maxTransferAmount > 0
    ) {
      require(
        amount <= maxTransferAmount,
        'FRESH BABYTOKEN: Transfer amount exceeds max transfer amount'
      );
    }
    ...
    if (
      ...
      from != address(uniswapV2Router) &&
      to != address(uniswapV2Router) &&
      !_isExcludedFromMaxWalletBalance[to] &&
      maxWalletBalance > 0
    ) {
      require(
        balanceOf(to).add(amount) <= maxWalletBalance,
        'FRESH BABYTOKEN: Recipient wallet balance would exceed max wallet balance'
      );
    }
  ...
  function swapAndSendToTeam(uint256 tokens) private {
    ...
    IERC20(rewardToken).transfer(_teamWalletAddress, newBalance);
  }
