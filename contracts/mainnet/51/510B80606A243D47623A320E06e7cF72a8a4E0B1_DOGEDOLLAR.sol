...
Doge Dollar.
An open source peer to peer smart crypto currency in Doge World! 
TG: https://t.me/doge_dollar_bsc
...
contract DOGEDOLLAR is IERC20, Context, Ownable {
    ...
    string private constant _name = "DOGE DOLLAR";

    uint256 public Max_tx_Buy = 20000 * 10**9;
    uint256 public Max_tx_Sell = 20000 * 10**9;

    address private __Marketing_Wallet =
        0x93B2f9595D21a17a00FAA89e0B0c8b0dE7166666;
    address public Stake_And_Utility = 0x7FF0B30bAbCC0B8B6F828900a41d55e6cC48221c;

    uint256 public Max_Wallet_Holding = 20000 * 10**9;
    ...
    Taxes private sellTaxes = Taxes(0, 4, 2, 0);
    Taxes private taxes = Taxes(0, 4, 2, 0);
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[Stake_And_Utility] = true;

        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[__Marketing_Wallet] = true;
        allowedTransfer[Stake_And_Utility] = true;
    ...
    function _transfer(
        ...
        if (from == pair && !_isExcludedFromFee[to] && !swapping) {
            require(amount <= Max_tx_Buy, "You are exceeding Max_tx_Buy");
            require(
                balanceOf(to) + amount <= Max_Wallet_Holding,
                "You are exceeding Max_Wallet_Holding"
            );
        }
        ...
        if (
            ...
            require(amount <= Max_tx_Sell, "You are exceeding Max_tx_Sell");
            if (to != pair) {
                require(
                    balanceOf(to) + amount <= Max_Wallet_Holding,
                    "You are exceeding Max_Wallet_Holding"
        ...
        lockTheSwap
    {
        ...
        if (teamAmt > 0) {
            payable(Stake_And_Utility).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell)
        ...
        onlyOwner
    {
        Max_tx_Buy = maxBuy * 10**decimals();
        Max_tx_Sell = maxSell * 10**decimals();
    }
