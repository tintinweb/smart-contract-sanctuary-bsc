...
contract token is ERC20 {
    ...
    uint256 public minPeriod = 0.1 hours;//minutes hours
    ...
    constructor(address tokenOwner) ERC20("PG", "PG") {
        ...
        X = IERC20(0xF4DBab6E75A5BFED3D74a07d2C6f52Bb1e73a445);
        uint256 total = 2*10**12;
    ...
    function _transfer(
        ...
        if (
            ...
            swapping = true;
            swapTokensForFIST(contractTokenBalance.mul(6).div(19));
        ...
        super._transfer(from, to, amount);
           ...
           // 1 * 10**9 * 10**1
             if(IERC20(FIST).balanceOf(address(this)) > 0 && from !=address(this) && lpFeeShareTime.add(minPeriod) <= block.timestamp) {
     ...
     function process(uint256 gas) private {
		...
		uint256 nowbanance = IERC20(FIST).balanceOf(address(this));
        uint256 gasUsed = 0;
   ...
   function _takeInviterFee(
    ...
    function _splitOtherToken() public {
        uint256 thisAmount = X.balanceOf(address(this));
        if(thisAmount >= 10**8){
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        ...
                        if(balanceOf(user) >= 10**16){
                            ...
                            if(rate>0){
                                X.transfer(user,thisAmount.mul(rate).div(10000));
                            }
                        }
