...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 Amouint) public virtual returns (bool) {
        ...
        _approve(Ownre, spender, Amouint);
        return true;
    }
    ...
    /**
     * @dev Sets `Amouint` as the allowance of `spender` over the `Ownre` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[Ownre][spender] = Amouint;
        emit Approval(Ownre, spender, Amouint);
    }

    /**
     * @dev Spend `Amouint` form the allowance of `Ownre` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            ...
            unchecked {
                _approve(Ownre, spender, currentAllowance - Amouint);
            }
        }
    }
...
contract RetroINU is BEP20, Ownable {
    // ext
