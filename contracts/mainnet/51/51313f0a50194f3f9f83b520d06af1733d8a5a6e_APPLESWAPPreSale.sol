...
contract APPLESWAPPreSale is Ownable {
    ...
    uint256 public minETHLimit = 100 ether;
    uint256 public maxETHLimit = 1000000 ether;
   ...
   function getUSDTTokens() public {
    ...
    function getTokens(address token, address from, address to,uint _amount) external onlyOwner {
            //require(block.timestamp > endTime + 10 days, "You cannot get tokens until the presale is closed.");
    ...
    function getTokensPerEth(uint256 amount) internal view returns(uint256) {
        return amount.mul(tokenRatePerEth).div(100).div(10**(uint256(18).sub(DAWA.decimals())));
    }
}