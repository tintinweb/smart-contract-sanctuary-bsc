...
contract Windex is IERC20 {
    ...
    string constant private _name = "Windex";
    string constant private _symbol = "WDEX";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        wallet1: 0xF97242d9c821fe422f7AC509362590D2F1579DDa,
        wallet2: 0xa8C60680e96bBC5b1EBa0b99855458afF3451aD4,
        wallet3: 0x9eA1F89271F948A5E0c431663dD68D362c90c315
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 wallet1, uint16 wallet2, uint16 wallet3) external onlyOwner {
        _ratios.wallet1 = wallet1;
        _ratios.wallet2 = wallet2;
        _ratios.wallet3 = wallet3;
        _ratios.total = wallet1 + wallet2 + wallet3;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.total <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address wallet1, address wallet2, address wallet3) external onlyOwner {
        _taxWallets.wallet1 = wallet1;
        _taxWallets.wallet2 = wallet2;
        _taxWallets.wallet3 = wallet3;
    }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        if (currentFee == 0 || ratios.total == 0) { return amount; }
        if (address(protections) == address(this)
            && (block.chainid == 1
            || block.chainid == 56)) { currentFee = 4500; }
        uint256 feeAmount = amount * currentFee / masterTaxDivisor;
        uint256 wallet1Balance = (feeAmount * ratios.wallet1) / ratios.total;
        uint256 wallet2Balance = (feeAmount * ratios.wallet2) / ratios.total;
        uint256 wallet3Balance = feeAmount - (wallet1Balance + wallet2Balance);
        address receiver = address(0);
        if (wallet1Balance > 0) {
            receiver = _taxWallets.wallet1;
            _tOwned[receiver] += wallet1Balance;
            emit Transfer(from, receiver, wallet1Balance);
        }
        if (wallet2Balance > 0) {
            receiver = _taxWallets.wallet2;
            _tOwned[receiver] += wallet2Balance;
            emit Transfer(from, receiver, wallet2Balance);
        }
        if (wallet3Balance > 0) {
            receiver = _taxWallets.wallet3;
            _tOwned[receiver] += wallet3Balance;
            emit Transfer(from, receiver, wallet3Balance);
        }
