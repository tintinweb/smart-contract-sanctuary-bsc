...
contract KREUGER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FREDDYKREUGERREFLECTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("KREUGER", "KREUGER") {
        marketingWallet = payable(0xB3212dfe378b2dc40341fd06Fa2C05629443ad2D);
        devWallet = payable(0xA416EC1BC996997A29A2b860D5B86DCead1bdc14);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new FREDDYKREUGERREFLECTDividendTracker(
            payable(this),
            router,
...
contract FREDDYKREUGERREFLECTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    KREUGER public KREUGERContract;
    ...
    constructor(
        ...
        defaultToken = token;
        KREUGERContract = KREUGER(mainContract);
    ...
    function _transfer(
        ...
        require(false, "KREUGER_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "KREUGER_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main KREUGER contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(KREUGERContract);

            uint256 prevBalance = KREUGERContract.balanceOf(address(this));
            ...
            {
                uint256 received = KREUGERContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    KREUGERContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
