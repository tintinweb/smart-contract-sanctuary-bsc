...
contract Ultrab0xBscNFT is Ownable, ERC721 {
    using Strings for string;
    using Strings for uint256;
    using SafeMath for uint256;

    constructor () ERC721("Ultrab0x Bsc NFT" ,"b0xBSC"){}

    uint256 public priceMint  = 0.015 ether;

    uint256 public MAX_CYBER = 11111;
    string public baseExtension = ".json";
    bool public pauseMint = false;

    address payable public marketing = payable(0xC1514C08bDCa99627C389a59F6AD38DAd85C0BB9);

    IERC20 public tokenNFT = IERC20(0x9619B5d48017D5E1a954a396aF47460a89D7E235);
    ...
    function flipPauseMint() public onlyOwner {
        pauseMint = !pauseMint;
    }
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(totalSupply().add(_amount) <= MAX_CYBER, "Limit NFT exceeded.");
        ...
        uint256 tokenToMint = _amount.mul(1 ether);

        tokenNFT.mint(msg.sender, tokenToMint);
    }

    function mintBoxers(uint256 _amount) public onlyOwner {
        require(totalSupply().add(_amount) <= MAX_CYBER, "Limit NFT exceeded.");
        for(uint256 i = 0; i < _amount; i++){   
            _mint(msg.sender, totalSupply().add(1));
        }
