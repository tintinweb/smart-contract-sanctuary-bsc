...
contract MOAI is Context, IERC20, Ownable {
    ...
    address private Moai_Marketing_Wallet = 0xE3c5Dee9bdfA3994055b6657Bec1B79b9af3925F;
    address private Moai_Team_Wallet = 0x921BFa081d9d814682E11B9650C292A66B7E34Af;

    uint256 private _max_WalletLimit = 2000000000000 * 10**9;
    uint256 private swapTokensAtAmount = 300000000000 * 10**9;
    uint256 private _max_BuyLimit = 1000000000000 * 10**9;
    uint256 private _max_SellLimit = 1000000000000 * 10**9;
    ...
    Taxes public taxes = Taxes(0, 70, 0, 20);
    Taxes public sellTaxes = Taxes(0, 70, 0, 20);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Moai_Marketing_Wallet] = true;
        _isExcludedFromFee[Moai_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Moai_Marketing_Wallet] = true;
        allowedTransfer[Moai_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= _max_BuyLimit, "You are exceeding _max_BuyLimit");
            require(balanceOf(to) + amount <= _max_WalletLimit, "You are exceeding _max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= _max_SellLimit, "You are exceeding _max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= _max_WalletLimit, "You are exceeding _max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Moai_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Moai_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Moai_Update_MarketingWallet(address newWallet) external onlyOwner{
        Moai_Marketing_Wallet = newWallet;
    }
    
    function Moai_Update_TeamWallet(address newWallet) external onlyOwner{
        Moai_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        _max_BuyLimit = maxBuy * 10**decimals();
        _max_SellLimit = maxSell * 10**decimals();
    }
    
    function _Max_Walletlimit(uint256 amount) external onlyOwner{
        _max_WalletLimit = amount * 10**decimals();
    }
