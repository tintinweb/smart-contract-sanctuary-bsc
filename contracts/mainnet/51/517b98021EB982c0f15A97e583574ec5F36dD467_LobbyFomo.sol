...
contract LobbyFomo is Ownable, ReentrancyGuard {
    ...
    uint256 public fomoDeposit = 1e18; // 1 HMINE
    uint256 public timeIncrement = 5 minutes; //
        ...
        nonReentrant
    {
        ...
        auctionRounds[_round].hmineInRound += _amount;
    ...
    function joinLobby(uint256 _amount) external nonReentrant {
        uint256 _round = _getCurrentRound();
        ...
        // Checks to make sure there is HMINE in the lobby before users can join.
        require(auctionRounds[_round].hmineInRound > 0, "No auctions yet");

        auctionRounds[_round].sharesInRound += _amount;
        userSharesInLobby[msg.sender][_round] += _amount;
    ...
    function userLobbyEarning(address _u, uint256 _round)
        external
        view
        returns (uint256 _lobbyReward)
    {
        uint256 _shareInRound = auctionRounds[_round].sharesInRound;
        uint256 _hmineInRound = auctionRounds[_round].hmineInRound;
        uint256 _userShare = userSharesInLobby[_u][_round];
