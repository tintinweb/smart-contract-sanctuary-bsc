...
contract SpacePresale is Ownable {
    ...
    function _approveContributor(address newContributor) internal {
        approvedContributors[newContributor] = true;
    }

    function approveContributor(address newContributor) external onlyOwner {
        _approveContributor(newContributor);
    }
    ...
    function contribute(uint256 contributionAmount) external {
        ...
        if (whiteListEnabled) {
            require(
                approvedContributors[msg.sender],
    ...
    function finalize() external onlyOwner {
        ...
        require(
            tokensNeeded == SPACE.balanceOf(address(this)),
            "Not enough SPACE in contract"
        );
        finalized = true;
    }
