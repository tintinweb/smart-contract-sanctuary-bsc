...
contract FruitIco is TransferOwnable, TimeLib {   
    ...
    function Redeem() external payable {
        ...
        uint ico_redeemable_time = _timeFromDate(ico_redeemable_year,ico_redeemable_month,ico_redeemable_day); 
        require(ico_available==1, "Error: ICO available require");
    ...
    function Purchase() external payable{
        ...
        uint ico_start_time = _timeFromDate(ico_start_year,ico_start_month,ico_start_day); 
        uint ico_end_time = _timeFromDate(ico_end_year,ico_end_month,ico_end_day);
        require(ico_available==1, "Error: ICO available require"); 
        require(block.timestamp>=ico_start_time, "Error: Start time not reached");
        require(block.timestamp<=ico_end_time, "Error: Past end time");
        require(account != address(0), "Error: Purchase to the zero address");
    ...
    function set_ico_available(uint v) external antiHacking onlyPartner {
        ico_available = v;
        emit log_ico_available(v);
    }
    
    event log_ico_available(uint256 _value);
