...
contract Exchange is Ownable {
    ...
    uint256 bonusTransaction = 10000; //100% 
    ...
    function ExchangeTokens(uint256 amount) public {
        ...
        if( useDirectWay) {
            currentPrice = getPriceDirect(_OriginalToken, _NewToken, 1 * 10**18)[1];
        }
        else {
            currentPrice = getPrice(_OriginalToken, uniswapV2Router.WETH(), _NewToken, 1 * 10**18)[2];
        }
        newAmount = newAmount.mul(currentPrice).div(1*10**18);
