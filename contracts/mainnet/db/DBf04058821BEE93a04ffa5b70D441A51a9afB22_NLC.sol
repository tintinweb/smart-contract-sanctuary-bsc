...
abstract contract Multisig {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _limit; i++) {
            trans[i] = transactions[_offset + i];
        }
    }
...
contract NLC is IBEP20, Multisig, Context {
  ...
  */
  ...
  function _burn(address source, uint256 amount) internal override {
      ...
      _balances[source] = _balances[source] - amount;
   ...
   */
  function updateBlacklist(address account, bool isListed) external isOwnerOrAuthorizer {
   ...
   */
  function suspend(bool suspend_) external isOwnerOrAuthorizer {
