...
// File: contracts/NFT/CertificateNFT.sol
...
contract CertificateNFT is ERC721URIStorage, Ownable, IMintable, Pausable {
  ...
  // Events
    ...
    modifier onlyIMX() {
        ...
        require(msg.sender == imx, "CertificateNFT: Function can only be called by IMX");
  ...
  uint256 public NFT_MINT_UNIT_PRICE = 0.00 ether;
  ...
  constructor(address _imx) ERC721("CertificateNFT", "EKOC") {

        require(_imx != address(0x0), 'CertificateNFT : Treasury address cannot be the 0x0 address');
  ...
  function setImx(address newAdd_) external onlyOwner {
  ...
  function mint(address[] memory tos) external whenNotPaused  onlyOwner {
    ...
    for (uint i = 0; i < tos.length; i++)

        {
          ...
          _addressMinted[tos[i]] = _addressMinted[tos[i]] + 1;
  ...
  function emergencyTransfer(address token) public onlyOwner  {
        ...
        tokenToTransfer.transfer(withdrawTo, tokenToTransfer.balanceOf(address(this)));
