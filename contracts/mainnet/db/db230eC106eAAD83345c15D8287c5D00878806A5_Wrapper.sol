...
import "./SpsTokent6.sol";



contract Wrapper is Ownable {
  SpsTokent6 private outputToken;
 


  constructor(address outputTokenAddress) {
    outputToken = SpsTokent6(outputTokenAddress);
    }
   ...
   */
    ...
    // Transfer SPS token from sender to wrapper
     ...
     */
        function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256){
         ...
         // we then multiply each token by the hours staked , then divide by the rewardPerHour rate 
          ...
          if (((block.timestamp - _current_stake.since) / 24 hours)>= 1 || ((block.timestamp - _current_stake.since) / 24 hours) < 180){
              return (((block.timestamp - _current_stake.since) / 24 hours) * _current_stake.amount) / rewardPerHour;
          }

          if (((block.timestamp - _current_stake.since) / 24 hours) >= 180 || ((block.timestamp - _current_stake.since) / 24 hours) < 365) {
               return _current_stake.amount * 25 / 100;
          }

          if (((block.timestamp - _current_stake.since) / 24 hours) >= 365) {
               return _current_stake.amount * 150 / 100;
          }
          return 0;
        }
...
contract SpsTokent6 is ERC20("Wrapped SPS", "WSPS"), Ownable, Stakeable {
  ...
  modifier onlyWrapper() {
    require(wrapper != address(0), "Wrapped SPS: Invalid wrapper address.");
    require(msg.sender == wrapper, "Wrapped SPS: Only wrapper is allowed.");
    _;
  }
  constructor() {
  ...
  function setWrapper(address wrapperAddress) external onlyOwner {
    require(wrapperAddress != address(0), "Wrapped SPS: Invalid wrapper address.");
