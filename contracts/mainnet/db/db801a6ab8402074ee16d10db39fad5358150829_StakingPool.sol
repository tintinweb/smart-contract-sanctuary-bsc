...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    constructor() {
        ...
        yieldRate = 5; // 1.5% daily APR @ 28000 blocks/day
    ...
    function claimableRewards(address _wallet) internal view returns(uint256) {
        // Claimable rewards = 1000000000000000000 * (28000 * 5 / 100000 / 100)
        uint256 currentBlock = block.number;
        uint256 prevBlock = lastStaked[_wallet];
        uint256 blocksPassed = currentBlock.sub(prevBlock);

        uint256 rewards = stakedBalances[_wallet].mul(blocksPassed).mul(yieldRate).div(100000).div(100);
    ...
    function getClaimableRewards(address _wallet) external view returns(uint256) {
        return claimableRewards(_wallet);
    }
    ...
    function getLastStakedForUser(address _user) external view returns(uint256) {
        return lastStaked[_user];
    }

    function getBlocksPassed(address _user) external view returns(uint256) {
        uint256 currentBlock = block.number;

        return currentBlock.sub(lastStaked[_user]);
    }
    ...
    function withdrawReserves(uint256 _amount) external onlyOwner {
        require(totalReserves.sub(_amount) >= 0, "Cannot withdraw more!");

        // Send remaining tokens to msg.sender.
        TransferHelper.safeTransfer(tokenToStake, msg.sender, totalReserves);
