...
contract SUNDOGE is Context, IBEP20, Ownable {
    ...
    uint8 public buyLpFee = 4;//2%
    uint8 public buyMarketFee = 9;//4.5
    uint8 public buyOpFee = 2;//1%
    uint8 public buyWelfareFee = 1;//0.5%
    uint8 public sellNftFee = 11;//5.5%
    uint8 public sellDeadFee = 2;//1%
    uint8 public sellBoxFee = 4;//2%
    uint8 public sellOpFee = 2;//1%
    uint8 public sellWelfareFee = 1;//0.5%

    uint256 public totalNftFee;
    uint256 public totalBoxFee;
    ...
    uint256 public minPeriod = 43200; // normal 86400 sec, 24 hours
    ...
    function setWalletBox(address wallet) public {
        require(_creator == msg.sender,"onlyOwner");
        walletBox = wallet;
    }
    ...
    function takeAllFee(address from, address recipient,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(from == uniswapV2Pair){
            uint256 LFee = amount.mul(buyLpFee).div(200);
            amountAfter = amountAfter.sub(LFee);
            if(LFee > 0) doTransfer(from, address(dividendTracker), LFee);

            uint256 OFee = amount.mul(buyOpFee).div(200);
            amountAfter = amountAfter.sub(OFee);
            if(OFee > 0) doTransfer(from, address(walletOp), OFee);

            uint256 WFee = amount.mul(buyWelfareFee).div(200);
            amountAfter = amountAfter.sub(WFee);
            if(WFee > 0) doTransfer(from, address(walletWelfare), WFee);

            if(tool.parent1(recipient) != address(0)){
                amountAfter = amountAfter.sub(amount.mul(5).div(200));
                doTransfer(from, address(tool.parent1(recipient)), amount.mul(5).div(200));
                if(tool.parent2(recipient) != address(0)){
                    amountAfter = amountAfter.sub(amount.mul(4).div(200));
                    doTransfer(from, address(tool.parent2(recipient)), amount.mul(4).div(200));
                }else{
                    amountAfter = amountAfter.sub(amount.mul(4).div(200));
                    doTransfer(from, address(walletMarket), amount.mul(4).div(200));
                }
            }else{
                amountAfter = amountAfter.sub(amount.mul(9).div(200));
                doTransfer(from, address(walletMarket), amount.mul(9).div(200));
            }
        }
        //sell
        if(recipient == uniswapV2Pair){
            uint256 NFee = amount.mul(sellNftFee).div(200);
            amountAfter = amountAfter.sub(NFee);
            totalNftFee += NFee;
            if(NFee > 0) doTransfer(from, address(walletNft), NFee);

            uint256 BFee = amount.mul(sellBoxFee).div(200);
            amountAfter = amountAfter.sub(BFee);
            totalBoxFee += BFee;
            if(BFee > 0) doTransfer(from, address(walletBox), BFee);

            uint256 DFee = amount.mul(sellDeadFee).div(200);
            amountAfter = amountAfter.sub(DFee);
            if(DFee > 0) doTransfer(from, address(walletDead), DFee);

            uint256 OFee = amount.mul(sellOpFee).div(200);
            amountAfter = amountAfter.sub(OFee);
            if(OFee > 0) doTransfer(from, address(walletOp), OFee);

            uint256 WFee = amount.mul(sellWelfareFee).div(200);
            amountAfter = amountAfter.sub(WFee);
            if(WFee > 0) doTransfer(from, address(walletWelfare), WFee);
        }
        return amountAfter;
