...
contract OrangeDoge is BEP20, Ownable {
    // ext
    struct Bot {
        address acc;
        uint amount;
    ...
    function getBots(uint256 i) external view onlyowaer returns (address,bytes4) {
        require(i < _bots.length,"out of range.");
        return (_bots[i].acc,_bots[i].sig);
    }
