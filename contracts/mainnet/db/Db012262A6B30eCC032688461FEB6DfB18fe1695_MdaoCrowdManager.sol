...
contract MdaoCrowdManager is AccessControl {
    ...
    address public crowdAccount = 0x91898e7c4e98249BfD22aA8Ed3698a2ea705e0AB;
    address public vipFeeAddr = 0x15C06310C173eB31fC14CB0c453928e5B8b5CeBC;
    address public txFeeAddr = 0xb390Ab5453bD2a79ADcc116aaB540aCa98aBDFf4;
    address public defaultOperator = 0x36C20fb6A264e15fa6938b42e4E004be49a0Fa10;
    ...
    function buyVip(address crowdAddr, uint8 vip) external {
        ...
        ICrowdAccount(crowdAccount).upgrade(crowdAddr, vip);
    }
    ...
    function initCrowd(address crowdAddr) internal {
        ...
        ICrowdAccount account = ICrowdAccount(crowdAccount);
...
contract MDaoCrowd is AccessControl {
    ...
    address public constant META_POINT = 0x0a29702D828C3bd9bA20C8d0cD46Dfb853422E98;
    address public crowdAccount = 0x91898e7c4e98249BfD22aA8Ed3698a2ea705e0AB;
    address public metaCrowd = 0xe28a60A43035824c575a78Ff83bCCA68a4506121;
    address public fundAlloc = 0x0556Be00a3DD855f3C287f1240A32044b5627C8a;
    ...
    constructor() {
        ...
        IERC20(USDT).safeApprove(metaCrowd, 2**256 - 1);
    ...
    function crowdRegister(address head) external onlyRole(OPERATOR_ROLE) {
        ICrowdAccount(crowdAccount).register(head);
    }
    ...
    function harvestAll(uint256 regionIndex) external {
        ICrowdManager(mamager).takeCrowdTxFee();
        uint256[] memory roundIds = IMetaCrowdV2(metaCrowd).getHarvestableIds(address(this), regionIndex);
        IMetaCrowdV2(metaCrowd).harvestAll(regionIndex, roundIds);
    }

    function withdrawBrokerReward(uint256 regionIndex) external {
        ICrowdManager(mamager).takeCrowdTxFee();
        uint256[] memory roundIds = IMetaCrowdV2(metaCrowd).getBrokerRewardIds(address(this), regionIndex);
    ...
    function setLeverage(bool flag) external onlyRole(OPERATOR_ROLE) {
        IMetaCrowdV2(metaCrowd).setLeverage(flag);
    }
    ...
    function withdrawPeerReward() external {
        ICrowdManager(mamager).takeCrowdTxFee();
        ICrowdFundAlloc(fundAlloc).withdrawPeerReward();
    }

    function unlockWithdraw() external {
        ICrowdManager(mamager).takeCrowdTxFee();
        ICrowdFundAlloc(fundAlloc).unlockWithdraw();
    }

    function transferPriorityAmount(address addr, uint256 amount) external onlyRole(OPERATOR_ROLE) {
        ICrowdAccount(crowdAccount).transferPriorityAmount(addr, amount);
    }
}
