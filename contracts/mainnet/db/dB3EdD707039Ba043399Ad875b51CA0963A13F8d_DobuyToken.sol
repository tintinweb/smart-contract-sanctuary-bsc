...
contract DobuyToken is BEP20Detailed, BEP20 {
  constructor() BEP20Detailed("Dobuy", "DBY", 8) {
...
contract BEP20 is BEPContext, IBEP20, BEPPausable {
  ...
  /**
   * @dev See {IBEP20-transferWhenPaused}.
   *
   * Requirements:
   *
   * - `recipient` cannot be the zero address.
   * - called by the owner to transfer when paused.
   * - the caller must have a balance of at least `amount`.
   */
  function transferWhenPaused(address recipient, uint256 amount) public virtual onlyOwner whenPaused returns (bool) {
    _transfer(_msgSender(), recipient, amount);
    return true;
  }
