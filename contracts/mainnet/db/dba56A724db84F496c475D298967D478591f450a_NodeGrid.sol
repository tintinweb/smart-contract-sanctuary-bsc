...
contract NodeGrid is ERC20Upgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            uint256 taxAmount = 0;
            if(_isSelling) {
                if(checkNodeBeforeSell && nodeManagerAddress != address(0)) {
                ...
                if(sellBackFee>0) taxAmount = amount.mul(sellBackFee).div(10000);
            } else if(_isBuying) {
                if(buyBackFee>0) taxAmount = amount.mul(buyBackFee).div(10000);
    ...
    function updateRouter(address _router) public onlyOwner {
        ...
        emit uniswapV2RouterUpdated(msg.sender, address(uniswapV2Router), address(uniswapV2Pair));
    }
