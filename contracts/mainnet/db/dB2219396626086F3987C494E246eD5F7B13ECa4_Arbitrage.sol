...
import {IERC20, SafeERC20} from "@balancer-labs/v2-solidity-utils/contracts/openzeppelin/SafeERC20.sol";
...
contract Arbitrage is Ownable {
    ...
    function startArbitrage(
        ...
        address pairAddress = IPancakeFactory(factory).getPair(_inputToken, _outputToken); 

        require(pairAddress != address(0), 'Could not find pool on uniswap');

        IPancakePair(pairAddress).swap(_inputTokenAmount, 0, address(this), bytes('flashloan'));
        acsiVault = _acsiVault;
        acsiPool = _acsiPool;
    }

    function pancakeCall(address sender, uint _amount0, uint _amount1, bytes calldata data) external {
        require(sender == address(this) && data.length > 0, "pancakeCall: Invalid Call");
        address[] memory path = new address[](2); 
        // call it amountToken and will use later in the function 
        ...
        require(msg.sender == PancakeLibrary.pairFor(factory, token0, token1), 'Invalid Request');
        ...
        // approve the apeSwapRouter to spend our tokens so the trade can occur
        //token.approve(acsiVault, amountToken);

        // calculate the amount of tokens we need to reimburse uniswap for the flashloan
        uint amountRequired = PancakeLibrary.getAmountsOut(factory, amountToken, path)[1];
        
        // finally sell the token we borrowed from uniswap on sushiswap
        // amountToken is the amount to sell
        ...
        // uint amountReceived = apeSwapRouter.swapExactTokensForTokens(amountToken, amountRequired, path, msg.sender, deadline)[1];
        ...
        outputToken.transfer(myOwner, amountReceived - amountRequired);
    }
}
