...
// SPDX-License-Identifier: Nocopypasterino
// IP of @Yu-Gi-Oh Doge - Official telegram: https://t.me/YuGiOhDogeBSC
// Website: https://yugiohdoge.monster/
...
contract YuGiOhDogeNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public cost = 0.01 ether; //around $2.7 in bnb
  ...
  // Lists 
  uint256[25000] public remainingIds;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Yu-Gi-Oh Doge: minting has not started yet");
    require(_mintAmount > 0, "Yu-Gi-Oh Doge: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Yu-Gi-Oh Doge: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Yu-Gi-Oh Doge: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Yu-Gi-Oh Doge: total cost doesn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Yu-Gi-Oh Doge: zero address");
    require(_amount > 0, "Yu-Gi-Oh Doge: you have to mint at least one");
    require(lastSupply != 0,  "Yu-Gi-Oh Doge: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Yu-Gi-Oh Doge: URI query for nonexistent token");
