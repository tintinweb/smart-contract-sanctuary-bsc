...
contract FCToken is Context, IERC20, Ownable {
    ...
    string private _name = "Football Champion";
    string private _symbol = "FC";
    
    uint public _buyLqFee = 10;
    uint public _buyLpFee = 10;
    uint public _buyMkFee = 20;
    uint public _buyBurnFee = 9;

    uint public _sellLqFee = 10;
    uint public _sellLpFee = 10;
    uint public _sellMkFee = 50;
    uint public _sellBurnFee = 9;

    uint public _adFee = 1;
    uint public totalBuyFee = 50;
    uint public totalSellFee = 80;

    uint public _transFee = 9;
    ...
    function setSellFee(uint lqFee,uint lpFee,uint mkFee,uint burnFee)external onlyOwner{
        _sellLqFee = lqFee;
        _sellLpFee = lpFee;
        _sellMkFee = mkFee;
        _sellBurnFee = burnFee;
        totalSellFee = _sellLqFee + _sellLpFee + _sellMkFee + _sellBurnFee + _adFee;
        require(totalSellFee < 1000,"param error");
    }
    ...
    function setaddLiquidityValue(uint _addLiquidityValue)external onlyOwner{
    ...
    function _transfer(
        ...
        if( block.timestamp < swapStartTime && (ammPairs[from] || ammPairs[to]) ){
            require(isAddLiquidity || _isExcludedFromFee[from] || _isExcludedFromFee[to] ,"swap no start");
        }
    ...
    function processEst() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            if (
                balanceOf(address(this)) < amount || surplusAmount < amount ) break;
