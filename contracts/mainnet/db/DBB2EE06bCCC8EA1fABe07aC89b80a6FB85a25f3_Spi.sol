...
contract Spi is ERC20, Ownable {
    ...
    function setBlackFee(uint256 val) public onlyOwner {
        blackFee = val;
    }
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(uniswapV2Pair[from] || uniswapV2Pair[to]){
            uint256 totalFeeAmount = amount.mul(blackFee).div(100);
            if(totalFeeAmount > 0) super._transfer(from, deadWallet, totalFeeAmount);
            amountAfter = amountAfter.sub(totalFeeAmount);        
        }
    }
