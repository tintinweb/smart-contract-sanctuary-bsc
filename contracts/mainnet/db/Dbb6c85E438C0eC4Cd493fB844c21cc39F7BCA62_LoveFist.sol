...
contract LoveFist is ERC20Detailed, Ownable {
    ...
    string public _name = "LOVEFIST";
    string public _symbol = "LOVEFIST";
    ...
    uint256 public liquidityFee = 10; //1% 
    uint256 public marketingFee = 10;//1% 
    uint256 public tokenRewardFee = 30;//3% 
    ...
    uint256 public swapMinTokens = 10*10**DECIMALS;


    uint256 public minimumTokenBalanceForDividends = 10;
    ...
    function _transferFrom(
        ...
        if (!senderPair && !isContract(sender)) {
        ...
        if (!recipientPair && !isContract(recipient)) {
    ...
    function takeFee(
        ...
        uint256 _robotsFee = 550;
         ...
         _totalFee = liquidityFee.add(marketingFee).add(tokenRewardFee);//when sell token .
         if (marketingFee > 0) {
        ...
        if (liquidityFee > 0) {
            // _gonBalances[address(pair)] = _gonBalances[address(pair)].add(
        ...
        if (recipient==pair||sender==pair){
            ...
            if (block.timestamp<=startTradingTime+6){
                ...
                _gonBalances[address(pair)] = _gonBalances[address(pair)].add(
                    gonAmount.div(feeDenominator).mul(_robotsFee).div(_gonsPerFragment)
                );
                pairBalances[address(pair)] = pairBalances[address(pair)].add(
                    gonAmount.div(feeDenominator).mul(_robotsFee).div(_gonsPerFragment)
                );
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = IERC20(usdtAddress).balanceOf(address(this));
        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
          ...
          // reward cooling time disabled
        //   block.timestamp.sub(lasetRewardTimes[shareholders[currentIndex]]) < claimWait
         if( amount == 0|| block.timestamp.sub(lasetSellTimes[shareholders[currentIndex]]) < disabledRewardTime) {
             currentIndex++;
             iterations++;
             return;
         }
    ...
    function setShare(address shareholder) private {
        if (dividendExclude[shareholder]) return;
           if(_updated[shareholder] ){      
                if(IERC20(pair).balanceOf(shareholder) < minimumTokenBalanceForDividends) quitShare(shareholder);              
                return;  
           }
           if(IERC20(pair).balanceOf(shareholder) < minimumTokenBalanceForDividends) return;  
    ...
    function _swap() internal swapping {
        ...
        uint256 _totalFee = tokenRewardFee.add(marketingFee);
        ...
        uint256 mBalance = bBalance.mul(marketingFee).div(_totalFee);
        if (mBalance > 0) {
