...
// File: contracts/interfaces/uniswapV2/ICSTPair.sol
...
// File: contracts/handlers/CryptoSwapHandler.sol

pragma solidity ^0.6.8;

/// @notice UniswapV2 Handler used to execute an order
contract CryptoSwapHandler is IHandler {
    using SafeMath for uint256;
    ...
    receive() external override payable {
        require(msg.sender != tx.origin, "CryptoSwapHandler#receive: NO_SEND_ETH_PLEASE");
    }
    ...
    function handle(
        ...
        require(successRelayer, "CryptoSwapHandler#handle: TRANSFER_ETH_TO_RELAYER_FAILED");
    }
    ...
    function _estimate(address _inputToken, address _outputToken, uint256 _inputAmount) internal view returns (uint256 bought) {
        ...
        ICSTPair pair = ICSTPair(UniswapUtils.pairForSorted(FACTORY, token0, token1, FACTORY_CODE_HASH));
    ...
    function _swap(address _inputToken, address _outputToken, uint256 _inputAmount, address _recipient) internal returns (uint256 bought) {
        ...
        require(SafeERC20.transfer(IERC20(_inputToken), address(pair), inputAmount), "CryptoSwapHandler#_swap: ERROR_SENDING_TOKENS");
