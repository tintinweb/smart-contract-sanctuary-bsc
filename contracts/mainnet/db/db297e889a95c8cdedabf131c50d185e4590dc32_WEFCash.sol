...
contract WEFCash is ERC20, Ownable{
    ...
    uint256 public maxTxAmount = 4_200_000 * 10**18;
    uint256 public maxWalletAmount = 210_000_000 * 10**18;
    uint256 discountFactor = 1;
 
    address public marketingWallet = 0xBaCFC57DfEf29d4Ee01d888B249240FC6fC7cad3;
    address public controllerWallet = 0xBaCFC57DfEf29d4Ee01d888B249240FC6fC7cad3;
    ...
    constructor() ERC20("WEFCASH","WEF Cash") {
        _Initiate(msg.sender, 210_000_000 * 10 ** decimals());
        ...
        excludedFromFees[controllerWallet] = true;
    }
...
function _transfer(address sender, address recipient, uint256 amount) internal override {
    ...
    function swapForFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            uint256 inflationAmt = unitBalance * 2 * sellTaxes.inflation;
            if(inflationAmt > 0){
                payable(controllerWallet).sendValue(inflationAmt);
            }
        }
    }
    ...
    function updatecontrollerWallet(address newWallet) external onlyOwner{
        controllerWallet = newWallet;
    }
    ...
    function updateBuyTaxes(uint256 liquidity, uint256 inflation, uint256 marketing) external onlyOwner {
        taxes.liquidity = liquidity;
        taxes.inflation = inflation;
        taxes.marketing = marketing;
        totTax = liquidity+inflation+marketing;
    }

   function updateSellTaxes(uint256 liquidity, uint256 inflation, uint256 marketing) external onlyOwner {
        sellTaxes.liquidity = liquidity;
        sellTaxes.inflation = inflation;
        sellTaxes.marketing = marketing;
        totSellTax = liquidity+inflation+marketing;
    }
    ...
    function manualSwap(uint256 amount, uint256 devPercentage, uint256 marketingPercentage) external onlyOwner{
        ...
        if(devPercentage > 0) payable(controllerWallet).sendValue(newBalance * devPercentage / (devPercentage + marketingPercentage));
    }
