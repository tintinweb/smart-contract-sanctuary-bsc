...
contract Web3Capital is IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xb9492e103aaA57f4548b72244C07BFDf0f79eF22)
        });
...
//===============================================================================================================
    ...
    function setPriceImpactSwapAmount(uint256 priceImpactSwapPercent) external onlyOwner {
        require(priceImpactSwapPercent <= 200, "Cannot set above 5%.");
    ...
    string public _error;

    function processRewards(address from, address to) internal {
        if (!_isExcludedFromDividends[from]) {
            try reflector.tally(from, _tOwned[from]) {} catch { _error = "From errored."; }
        }
        if (!_isExcludedFromDividends[to]) {
            try reflector.tally(to, _tOwned[to]) {} catch { _error = "To errored."; }
        }
        if (processReflect) {
            try reflector.cashout(reflectorGas) {} catch { _error = "Auto errored."; }
        }
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal swapping {
        ...
        if (ratios.rewards > 0) {
            try reflector.load{value: rewardsBalance}() {} catch { _error = "Load errored."; }
        }
