...
contract MerkleProofMint is Ownable {
    ...
    function safeMint(bytes32[] memory proof) external view returns (bool){
        require(isValid(proof, keccak256(abi.encodePacked(msg.sender))), "invalid proof");
        return true;
    }
    ...
    mapping(address => uint256) public _exchanger;
    ...
    function safeMint3(bytes32[] memory proof, address addr, string calldata data) external returns (bool){
        ...
        _exchanger[addr] = block.timestamp;
        emit Deposit(addr, data);
        return true;
    }

    function test(address addr, string calldata data) external pure returns (bytes32){
