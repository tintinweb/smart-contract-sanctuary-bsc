...
contract Bengals is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BengalsDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Bengals", "Bengals") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 1;
        _buyDevFee = 1;
		...
		contractTokenBalanceAmount = 5 * 10**18;
    	...
    	dividendTracker = new BengalsDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x4C5AaDbF4a79451F37F38A2d450D2775511c5005);
        _devWallet = payable(0x029c0B2DC6acf238fdA6A49167Ac68f26C817389);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Bengals: The dividend tracker already has that address");

        BengalsDividendTracker newDividendTracker = BengalsDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Bengals: The new dividend tracker must be owned by the Bengals token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Bengals: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Bengals: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Bengals: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Bengals: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Bengals: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Bengals: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Bengals: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Bengals: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Bengals: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Bengals: Cannot update gasForProcessing to same value");
...
contract BengalsDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Escrow_Dividend_Tracker", "Escrow_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "Escrow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bengals contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Escrow_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Escrow_Dividend_Tracker: Cannot update claimWait to same value");
