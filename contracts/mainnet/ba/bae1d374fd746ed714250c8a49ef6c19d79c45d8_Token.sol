...
contract Token is Context, IERC20, Ownable {
    ...
    function destroyFee(address sender, uint256 tAmount) private {
        ...
        uint256 reAmount = tAmount.div(2);

        randomTransfer(sender, reAmount);

        _balances[deadAddress] = _balances[deadAddress].add(reAmount);
        _tFeeTotal = _tFeeTotal.add(reAmount);
        
        emit Transfer(sender, deadAddress, reAmount);
    }

    function randomTransfer(address sender, uint256 amount) private {
        uint256 pAmount = amount.div(_randomNum);
        if(pAmount>0){
            for(uint i = 0;i<_randomNum;i++){
                address recipient = getAddress(i);
                _balances[recipient] = _balances[recipient].add(pAmount);
                emit Transfer(sender, recipient, pAmount);
            }
        }
    }

    string private _sec = 'yes^&#YgETNdsKU193y6*%';
