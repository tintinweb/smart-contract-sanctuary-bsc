...
contract Zapper is ReentrancyGuard, ZapperFee {
        ...
        returns (uint256 amountOut)
    {
        if (p.path[0] == p.path[1]) {
          return _direct20Bet(p, amountIn);
        } else {
            amountIn = _before20Swap(p, amountIn);
    ...
    function _direct20Bet(iPar memory p, uint256 amountIn) internal returns (uint256 amountOut) {
        address oToken = p.path[0];
        uint256 fee = _calculateFee(amountIn, oToken);
        IERC20(oToken).transferFrom(msg.sender, address(this), fee);
        uint256 adjustedAmount = amountIn - fee;
        IERC20(oToken).transferFrom(msg.sender, p.predictionAddress, adjustedAmount);
        amountOut = _zap(p, adjustedAmount, address(0), 0);
        return amountOut;
    }

    function _before20Swap(iPar memory p, uint256 amount) internal returns (uint256) {
        IERC20(p.path[0]).safeTransferFrom(msg.sender, address(this), amount);
        amount = _adjustFee(amount, p.path[0]);
...
contract ZapperFee {
    ...
    // internals
    function _calculateFee(uint256 amountIn, address token) internal returns (uint256 fee)  {
        fee = (amountIn * treasuryFee) / FEE_DIVIDER;
    ...
    {
        uint256 fee = _calculateFee(amountIn, token);
        return amountIn - fee;
    }
