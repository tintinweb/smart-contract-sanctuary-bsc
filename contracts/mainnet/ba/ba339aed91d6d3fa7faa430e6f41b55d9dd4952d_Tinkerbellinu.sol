...
contract Tinkerbellinu is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    TinkerbellinuDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x5be375075cA2f211e07E800F6f060866A93F134a);
    address payable public _buybackWalletAddress = payable(0x111dCD91436957d9B4f989f39bB67FaC2776Eb8c);
    ...
    constructor() BEP20("Tinkerbell Inu", "TINU") 
    {
        dividendTracker = new TinkerbellinuDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Tinkerbellinu: The dividend tracker already has that address");
        TinkerbellinuDividendTracker newDividendTracker = TinkerbellinuDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Tinkerbellinu: The new dividend tracker must be owned by the Tinkerbellinu token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Tinkerbellinu: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Tinkerbellinu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Tinkerbellinu: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Tinkerbellinu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Tinkerbellinu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Tinkerbellinu: Cannot update gasForProcessing to same value");
...
contract TinkerbellinuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Tinkerbellinu_Dividend_Tracker", "Tinkerbellinu_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Tinkerbellinu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Tinkerbellinu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Tinkerbellinu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Tinkerbellinu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Tinkerbellinu_Dividend_Tracker: Cannot update claimWait to same value");
