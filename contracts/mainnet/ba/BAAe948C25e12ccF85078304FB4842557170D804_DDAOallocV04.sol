...
contract DDAOallocV04 is AccessControl, Ownable
{
	...
	struct info
	{
		...
		//AddrVaultChange(0x208b02f98d36983982eA9c0cdC6B3208e0f198A3);
...
//	function SaleModify(uint256 id,string memory name,address vault,bool enabled,uint256 min1,uint256 min2,uint256 min3,address[] memory tokens)public onlyAdmin
	function SaleModify(uint256 id,string memory name,bool enabled,uint256 min1,uint256 min2,uint256 min3,address[] memory tokens)public onlyAdmin
	{
		uint256 i = id;
		Sale[i].id = id;
		Sale[i].name = name;
		//Sale[i].vault = AddrVault;
		...
		emit DDAOSale(id,enabled,name,AddrVault,min1,min2,min3,tokens);
	}
