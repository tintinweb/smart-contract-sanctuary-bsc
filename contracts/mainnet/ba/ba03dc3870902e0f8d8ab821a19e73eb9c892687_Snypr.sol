...
contract Snypr is INEW, Context, Administration {
    using SafeMath for uint256;
    ...
    string constant _name = "Snypr Token";
    string constant _symbol = "SNYPR";
    uint8 constant _decimals = 9;
    uint256 private _tTotal = 100000000 * 10**_decimals; // 100,000,000 (one hundred million)
    ...
    address public WBNB;
    uint256 public _maxTxAmount = 5000000 * 10**_decimals; // 5,000,000 (5 million, e.g., 5% of the total supply)
    ...
    constructor(address _router, address _rewardAdmin) {
        ...
        _defaultFees = Fees(
            ...
            400, // marketing
            200, // liquidity
            0x537d1A7cf0D0DEa3B18c9C7322a7753775778a1e, // marketing 
            0xe254187997ad6794D8Ba94Bf23266C71c6B5D4bA, // development
            0x469769E3375405c496484c2b0D001D21442ddE26 // liquidity
        );

        // Set base token in the pair as WBNB, which acts as the tax token
        INEWSwapPair(newSwapPair).setBaseToken(WBNB);
        INEWSwapPair(newSwapPair).updateTotalFee(900); // this number should equal the sum of the defaultFees
        
        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function blacklistAddress(address addressToBlacklist) public onlyAdmin {
        require(!isBlacklisted[addressToBlacklist] , "Address is already blacklisted!");
        isBlacklisted[addressToBlacklist] = true;
    }

    function removeFromBlacklist(address addressToRemove) public onlyAdmin {
    ...
    function totalSupply() public view override returns (uint256) {
        return _tTotal - _balances[_burnAddress];
    }
    ...
    function setReflectionPercent(uint256 _reflectionFee) external onlyAdmin {
        _defaultFees.reflectionFee = _reflectionFee;
        _updatePairsFee();
    }

    function setLiquifyPercent(uint256 _liquidityFee) external onlyAdmin {
        _defaultFees.liquidityFee = _liquidityFee;
        _updatePairsFee();
    }

    function setMarketingPercent(uint256 _marketingFee) external onlyAdmin {
        _defaultFees.marketingFee = _marketingFee;
        _updatePairsFee();
    }

    function setMarketingAddress(address _marketing) external onlyAdmin {
        require(_marketing != address(0), "NEW: Address Zero is not allowed");
        _defaultFees.marketingAddress = _marketing;
    }

    function setDevelopmentAddress(address _development) external onlyAdmin {
        require(_development != address(0), "NEW: Address Zero is not allowed");
        _defaultFees.developmentAddress = _development;
    }

    function setLiquifyAddress(address _liquidity) external onlyAdmin {
    ...
    function updateRouterAndPair(address _router, address _pair) public onlyAdmin {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyAdmin { // '1' = 0.01% of the total supply, '100' = 1.0% of the total supply
    ...
    function setIsRewardExempt(address holder, bool exempt) external onlyAdmin { 
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _tTotal.sub(balanceOf(address(0)).sub(balanceOf(_burnAddress)));
    }
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyAdmin {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    function setDistributorSettings(uint256 _gas) external onlyAdmin {
