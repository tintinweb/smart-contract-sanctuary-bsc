...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable USTC = address(0x23396cF899Ca06c4472205fC903bDB4de249D6fC); //USTC
  ...
  function distributeUSTCDividends(uint256 amount) public onlyOwner{
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IBEP20(USTC).transfer(user, _withdrawableDividend);
...
contract ustcdoge is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    ustcdogeDividendTracker public dividendTracker;
    ...
    uint256 public USTCRewardsFee = 1;
    uint256 public liquidityFee = 1;
    uint256 public marketingFee = 1;
    uint256 public devFee = 1;

    uint256 public totalFees = USTCRewardsFee.add(liquidityFee).add(marketingFee).add(devFee);

    address payable public _marketingWalletAddress = payable(0xB336acCA495Fc7a57221F50a68a7990b9037973e);
    address payable public _devWalletAddress = payable(0x4A6b3E0965FaBb527d7F12a69c4bcf8AEf27c43C);
    ...
    constructor() BEP20("ustcdoge", "ustc") 
    {
        dividendTracker = new ustcdogeDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "ustcdoge: The dividend tracker already has that address");
        ustcdogeDividendTracker newDividendTracker = ustcdogeDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "ustcdoge: The new dividend tracker must be owned by the ustcdoge token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ustcdoge: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ustcdoge: Account is already the value of 'excluded'");
    ...
    function setUSTCRewardsFee(uint256 value) external onlyOwner{
        USTCRewardsFee = value;
        totalFees = USTCRewardsFee.add(liquidityFee).add(marketingFee).add(devFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ustcdoge: The PanUSTCSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ustcdoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ustcdoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ustcdoge: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(USTCRewardsFee); // marketingFee+devFee+liquidityFee.
    ...
    {
       uint256 swapableFee = totalFees.sub(USTCRewardsFee);
        ...
        uint256 bnbForbb = newBalance.sub(bnbForLiquidity).sub(bnbForMarketing);
        _marketingWalletAddress.transfer(bnbForMarketing);
        _devWalletAddress.transfer(bnbForbb);
    }
    ...
    function swapTokensForUSTC(uint256 tokenAmount) private 
    {
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForUSTC(tokens);
        uint256 dividends = IBEP20(USTC).balanceOf(address(this));
        bool success = IBEP20(USTC).transfer(address(dividendTracker), dividends);

        if (success) {
            dividendTracker.distributeUSTCDividends(dividends);
...
contract ustcdogeDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ustcdoge_Dividend_Tracker", "ustcdoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 400_000 * (10**18); 
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ustcdoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ustcdoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ustcdoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ustcdoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ustcdoge_Dividend_Tracker: Cannot update claimWait to same value");
