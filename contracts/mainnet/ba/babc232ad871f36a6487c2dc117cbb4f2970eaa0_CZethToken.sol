...
// File contracts/Tokens/CZethToken.sol
...
contract CZethToken is ERC20Burnable, Operator {
    ...
    ERC20Burnable private constant _czemp = ERC20Burnable(0xa700Ec63f9D3D9A03981CF7F8Eb805c444EDBC21);
     ...
     */
    constructor(string memory name_, string memory symbol_, uint8 decimals_, address _taxFund) public ERC20(name_, symbol_) {
    ...
    function migrate(uint256 amount_) external returns (uint256 czEthMintAmount) {
        require(amount_ >= 0.01 ether, "Amount must be larger than 0.01 unit");
        require(_czemp.transferFrom(msg.sender, address(this), amount_), "Tokens could not be transferred");
        _czemp.burn(amount_);
        czEthMintAmount = amount_.div(4000);
        _mint(msg.sender, czEthMintAmount);
    }
