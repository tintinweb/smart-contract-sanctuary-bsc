...
contract ApexproCommunity is Ownable {
	...
	uint256          public INVEST_MIN_AMOUNT = 0.03 ether; // Min 0.005 bnb 
    ...
    function _canYield(address userAddress) internal view returns(bool) {
		return (block.timestamp-users[userAddress].clickcheckpoint >= YIELD_COOLDOWN);
	}
  ...
  function reinvest() public {
    ...
    function yieldClick() public {
        ...
        user.clickcheckpoint = block.timestamp;
    ...
    function yieldTimer(address userAddress) public view returns(uint256) {
		return (YIELD_COOLDOWN.add(users[userAddress].clickcheckpoint));
	}
