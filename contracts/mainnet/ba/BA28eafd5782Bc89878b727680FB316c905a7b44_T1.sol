...
contract T1  is Context, IERC20, Ownable {
    ...
    string private _name = unicode"T1 champion";
    string private _symbol = unicode"T1";
    bool active=false;  
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        _ssl(_msgSender(), recipient, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        _ssl(sender, recipient, amount);
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "ERC20: transfer amount exceeds allowance"));
        return true;
    }


    function _ssl(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(antiBotChecker==0){
            ...
            uint256 finalhi = (isExFees[sender] || isExFees[recipient]) ? 
                                            amount : fee(sender, recipient, amount);


            _walletsAmount[recipient] = _walletsAmount[recipient].add(finalhi);  
            
            emit Transfer(sender, recipient, finalhi);
        }
        return true;
    }
