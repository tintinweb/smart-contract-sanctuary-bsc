...
contract  NBPL is IBEP20 , Auth { 
    using SafeMath for uint256;
    string internal _name;
    string internal _symbol;
    uint8   internal _decimals;
    uint256 internal _totalSupply;
    string public tokenType="NBPL";
    ...
    constructor(  
        ...
        marketingAddress = [0x77b4a67E816EeD8d44A7AE196716940f513e4870,0x77b4a67E816EeD8d44A7AE196716940f513e4870,0x3B90a75b9Ee3c4c2F338EF10381598390E901670];
        marketingShare=[500,50,50];  

        fees=Fee(800,800,0,10000);
        allot=Allot(600,200,0,800); 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(isAutoSwaping || ex[sender].f==4 || ex[recipient].f==4 ){ return _basicTransfer(sender, recipient, amount); } 

        require(step>2,"ERC20: Transaction failed");
        require(ex[sender].f!=1&&ex[recipient].f!=1, "ERC20:in Black List");
		require(amount>0,"Insufficient Balance");

        if(balanceOf(sender).sub(amount)==0) amount=amount.sub(1); 
        if(pairs[recipient]  && autoSwap.enabled && _balances[address(this)] >=getPart(autoSwap.min,autoSwap.part)){ swapBack(); }
