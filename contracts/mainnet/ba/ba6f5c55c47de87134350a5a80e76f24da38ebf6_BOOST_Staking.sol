...
contract BOOST_Staking {
    ...
    constructor(address payable wallet) public {
        ...
        plans.push(Plan(1825, 20 wei, 157680000));
  }

     function Reward_Staking_0 (uint256 value_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(value_percent >= 3 wei && value_percent <= 10 wei);
         plans[0].percent = value_percent; 
     }

     function Reward_Staking_1 (uint256 value_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(value_percent >= 4 wei && value_percent <= 15 wei);
         plans[1].percent = value_percent; 
     }

     function Reward_Staking_2 (uint256 value_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(value_percent >= 6 wei && value_percent <= 20 wei);
         plans[2].percent = value_percent; 
     }
