...
contract XENTHIUMCOIN is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    // Multisig Protocol Wallets

    address payable public marketingAddress = payable(0x1E9a95cdDAcC7aeB1EBf988ea1AA4c60cB9bA1c2);
    address payable public teamAddress = payable(0x88bEE3d73a9Fc75297535E6b0E913FaE3866FA55);
    address payable public environmentAddress = payable(0x72dBDf670f6A9D3Dc57B00efeb7d013f46790876);
    ...
    string private constant _name = "XENTHIUMCOIN";
    ...
    uint256 public _buyEnvironmentFee = 1;
    uint256 public _buyAllSwapableFees = _buyTeamFee+_buyLiquidityFee+_buyMarketingFee+_buyEnvironmentFee;
    ...
    uint256 public _sellEnvironmentFee = 2;
    uint256 public _sellAllSwapableFees = _sellTeamFee+_sellLiquidityFee+_sellMarketingFee+_sellEnvironmentFee;
    
    uint256 public _bMaxTxAmount = _tTotal.mul(2).div(100);
    uint256 public _sMaxTxAmount = _tTotal.mul(2).div(100);
    uint256 public _walletMax = _tTotal.mul(2).div(100);
    ...
    constructor () 
    {
        ...
        _isExcludedFromFee[environmentAddress] = true;
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) 
        {
            if (to == uniswapV2Pair) 
            {
                if (overMinimumTokenBalance) 
                {
                    ...
                    uint256 marketingAndDevelopmentToken = contractTokenBalance.mul(_sellMarketingFee+_sellEnvironmentFee+_sellTeamFee).div(_sellAllSwapableFees);
                    uint256 liquidityToken = contractTokenBalance.sub(marketingAndDevelopmentToken);
    ...
    function _takeVault(uint256 tVault) private 
    {
        ...
        _rOwned[environmentAddress] = _rOwned[environmentAddress].add(rVault);
        if(_isExcluded[environmentAddress])
            _tOwned[environmentAddress] = _tOwned[environmentAddress].add(tVault);
    }
    ...
    function setWalletLimit(uint256 newLimit) external onlyOwner {
        _walletMax = newLimit;
        require(_walletMax>_tTotal.div(1000), "Too less limit");
    }
    ...
    function setEnvironmentAddress(address _environmentAddress) external onlyOwner {
        environmentAddress = payable(_environmentAddress);
        _isExcludedFromFee[environmentAddress] = true;
    }
    ...
    function setAllBuyAndTxFees(uint256 taxFee, uint256 vaultFee, uint256 teamFee,  uint256 liquidityFee,  uint256 marketingFee, uint256 environmentFee) 
    external onlyOwner 
    {
            _buyTaxFee = taxFee;
            ...
            _buyEnvironmentFee = environmentFee;
            _buyAllSwapableFees = _buyTeamFee+_buyLiquidityFee+_buyMarketingFee+_buyEnvironmentFee;
    ...
    function setAllSellFees(uint256 taxFee, uint256 vaultFee, uint256 teamFee,  uint256 liquidityFee,  uint256 marketingFee, uint256 environmentFee) 
    external onlyOwner 
    {
            _sellTaxFee = taxFee;
            ...
            _sellEnvironmentFee = environmentFee;
            _sellAllSwapableFees = _sellTeamFee+_sellLiquidityFee+_sellMarketingFee+_sellEnvironmentFee;
    ...
    function manualDistribute(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
        Distribute(bnbLiquifyAmount);
    }
    ...
    function Distribute(uint256 bnbLiquifyAmount) private lockTheSwap {
        uint256 environmentFunds = bnbLiquifyAmount.mul(_sellEnvironmentFee).div(_sellAllSwapableFees);
        uint256 teamFunds = bnbLiquifyAmount.mul(_sellTeamFee).div(_sellAllSwapableFees);
        uint256 marketingFunds = bnbLiquifyAmount.sub(environmentFunds).sub(teamFunds);

        transferToAddressBNB(marketingAddress,marketingFunds);
        transferToAddressBNB(teamAddress,teamFunds);
        transferToAddressBNB(environmentAddress,environmentFunds);
