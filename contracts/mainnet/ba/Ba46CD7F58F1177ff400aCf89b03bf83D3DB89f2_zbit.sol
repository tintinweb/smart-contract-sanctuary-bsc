...
contract zbit is IERC20,Context,Ownable,Pausable{
    ...
    mapping (address => bool) private _isBlocking;

    mapping (address => bool) private _isExcluded;
    address[] private _excluded;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 5**10 * 10**18;
    ...
    //Bridge Setting
    event SendToOtherNetwork(string nameNetwork,uint256 total);
    event ReceiveFromOtherNetwork(string nameNetwork,uint256 total);

    function SendSupplyToOtherNetwork(uint256 total, string memory toNetwork) public onlyOwner() {
        _tTotal = _tTotal.sub(total);
        emit SendToOtherNetwork(toNetwork,total);
    }
     function ReciveSupplyToOtherNetwork(uint256 total, string memory fromNetwork) public onlyOwner() {
        _tTotal = _tTotal.add(total);
        emit ReceiveFromOtherNetwork(fromNetwork,total);
    }
    /////////////

    function IsBlockUser(address account) public view  returns(bool){
        return _isBlocking[account];
    }
    

    function BlockUserToggle(address account) public onlyOwner() {
        if(_isBlocking[account]==false) _isBlocking[account]=true;
        else _isBlocking[account]=false;
    }
    ...
    function transfer(address recipient, uint256 amount) public override whenNotPaused() returns (bool) {
        require(_isBlocking[_msgSender()]==false,"account Is Blocked");
