    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
     ...
     function _transfer(address from, address to, uint256 value) internal {
        ...
        if(_totalSupply > MAX_STOP_FEE_TOTAL){
            if (from == pancakePair) {
                _balances[from] = _balances[from].sub(value);
                _balances[to] = _balances[to].add(value.sub(mcfee + mrfeeXH));
                _balances[feetoken] = _balances[feetoken].add(mcfee);
                _balances[XHtoken] = _balances[XHtoken].add(mrfeeXH);
                emit Transfer(from, feetoken, mcfee);
                emit Transfer(from, XHtoken, mrfeeXH);
                emit Transfer(from, to, value.sub(mcfee + mrfeeXH));
                ...
                _balances[from] = _balances[from].sub(value);
                _balances[to] = _balances[to].add(value.sub(zzfeeZZ + zzfeeXH));
                _balances[feetoken] = _balances[feetoken].add(zzfeeZZ);
                _balances[XHtoken] = _balances[XHtoken].add(zzfeeXH);
                emit Transfer(from, feetoken, zzfeeZZ);
                emit Transfer(from, XHtoken, zzfeeXH);
                emit Transfer(from, to, value.sub(zzfeeZZ + zzfeeXH));
            }
        }else{
            _balances[from] = _balances[from].sub(value);
            _balances[to] = _balances[to].add(value);
