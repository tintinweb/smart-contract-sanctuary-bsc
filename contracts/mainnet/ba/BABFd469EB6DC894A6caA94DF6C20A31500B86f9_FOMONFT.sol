...
contract FOMONFT is ERC721Enumerable, Ownable {
    ...
    uint256 private _mintUsdtAmount;
    ...
    function setMintUsdtAmount(uint256 amount) external onlyOwner {
        _mintUsdtAmount = amount * 10 ** IERC20(_usdtAddress).decimals();
    }
    ...
    function getTokenInfo() external view returns (
        ...
        uint256 mintUsdtAmount, uint256 mintTokenAmount
    ){
        ...
        mintTokenAmount = tokenAmountOut(mintUsdtAmount, tokenAddress);
    }

    function getUserInfo(address account) external view returns (
        uint256 nftBalance, uint256 firstNftId, uint256 tokenBalance, uint256 tokenAllowance
    ){
        nftBalance = balanceOf(account);
        if (nftBalance > 0) {
            firstNftId = tokenOfOwnerByIndex(account, 0);
        }
        tokenBalance = IERC20(_tokenAddress).balanceOf(account);
        tokenAllowance = IERC20(_tokenAddress).allowance(account, address(this));
    }

    function mint(uint256 maxTokenAmount) external {
        address account = msg.sender;
        require(account == tx.origin, "not origin");
        uint256 tokenAmount = tokenAmountOut(_mintUsdtAmount, _tokenAddress);
        require(tokenAmount <= maxTokenAmount, "gt maxTokenAmount");
        _takeToken(_tokenAddress, account, _tokenReceiveAddress, tokenAmount);
        _batchMint(account, 1);
    }
