...
contract Rule is Context, Ownable {
    ...
    uint256[] public _rate=[10,6,4];
    uint256 public _pool1=0;
    uint256 public _pool2=0;
    address public _dpool=address(0x96d6D9902e37DFfaF9885150E70487a41f2dd2D6);
    mapping (address => bool) public _isBlack;

    address public _admin;
    address public _tokenAddr=address(0xA4d8410CaEFfC108F6D14e77f00976b19f98c917);
    ...
    function setPool1(uint256 x) public  {
        require(msg.sender==owner() || msg.sender==_admin);
        _pool1 = x;
    }
    function setPool2(uint256 x) public  {
        require(msg.sender==owner() || msg.sender==_admin);
        _pool2 = x;
    }
    ...
    function _tokenTransfer( address sender, address recipient, uint256 tAmount, bool takeFee ) private returns(uint256 sy) {
        ...
        if(takeFee){
            if(_rate[0]>0){
                uint256 bamount = tAmount*_rate[0]/1000;
                if(bamount>0){
                    k=k.add(bamount);
                    uint256 newBalance =  Token(_tokenAddr).balanceOf(_dpool).add(k);
                    Token(_tokenAddr).setBalance(_dpool,newBalance);
                    addTxLog(sender,_dpool,k);
                }
            }
            if(_rate[1]>0){
                uint256 bamount = tAmount*_rate[1]/1000;
                if(bamount>0){
                    k=k.add(bamount);
                   _pool1=_pool1.add(bamount);
                   uint256 newBalance =  Token(_tokenAddr).balanceOf(address(this)).add(k);
                    Token(_tokenAddr).setBalance(address(this),newBalance);
                    addTxLog(sender,address(this),k);
                }
            }
            if(_rate[2]>0){
                uint256 bamount = tAmount*_rate[2]/1000;
                if(bamount>0){
                     k=k.add(bamount);
                   _pool2=_pool2.add(bamount);
                    uint256 newBalance =  Token(_tokenAddr).balanceOf(address(this)).add(k);
                    Token(_tokenAddr).setBalance(address(this),newBalance);
                    addTxLog(sender,address(this),k);
                }
