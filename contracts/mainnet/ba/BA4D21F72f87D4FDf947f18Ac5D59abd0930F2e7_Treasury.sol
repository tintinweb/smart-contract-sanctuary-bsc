...
contract Treasury is ContractGuard {
    ...
    uint256 public susdSupplyTarget;

    uint256 public maxSupplyExpansionPercent;
    uint256 public subondDepletionFloorPercent;
    uint256 public seigniorageExpansionFloorPercent;
    uint256 public maxSupplyContractionPercent;
    uint256 public maxDebtRatioPercent;

    // 21 first epochs (1 week) with 3.5% expansion regardless of SUSD price
    ...
    uint256 public maxDiscountRate; // when purchasing subond
    uint256 public maxPremiumRate; // when redeeming subond
    ...
    address public sushareRewardPool;
    uint256 public sushareRewardPoolExpansionRate;
    uint256 public sushareRewardPoolContractionRate;


    /* =================== Events =================== */

    event Initialized(address indexed executor, uint256 at);
    event RedeemedSUBonds(address indexed from, uint256 susdAmount, uint256 subondAmount);
    event BoughtSUBonds(address indexed from, uint256 susdAmount, uint256 subondAmount);
    ...
    function getBurnableSusdLeft() public view returns (uint256 _burnableSusdLeft) {
        ...
        if (_susdPrice <= susdPriceOne) {
            ...
            uint256 _subondMaxSupply = _susdSupply.mul(maxDebtRatioPercent).div(10000);
            uint256 _subondSupply = IERC20(subond).totalSupply();
            if (_subondMaxSupply > _subondSupply) {
                uint256 _maxMintableSUBond = _subondMaxSupply.sub(_subondSupply);
                uint256 _maxBurnableSusd = _maxMintableSUBond.mul(getSUBondDiscountRate()).div(1e18);
                _burnableSusdLeft = Math.min(epochSupplyContractionLeft, _maxBurnableSusd);
            }
        }
    }

    function getRedeemableSUBonds() public view returns (uint256 _redeemableSUBonds) {
        uint256 _susdPrice = getSusdPrice();
        if (_susdPrice > susdPriceCeiling) {
            uint256 _totalSusd = IERC20(susd).balanceOf(address(this));
            uint256 _rate = getSUBondPremiumRate();
            if (_rate > 0) {
                _redeemableSUBonds = _totalSusd.mul(1e18).div(_rate);
            }
        }
    }

    function getSUBondDiscountRate() public view returns (uint256 _rate) {
        uint256 _susdPrice = getSusdPrice();
        if (_susdPrice <= susdPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _subondAmount = susdPriceOne.mul(1e18).div(_susdPrice); // to burn 1 SUSD
                uint256 _discountAmount = _subondAmount.sub(susdPriceOne).mul(discountPercent).div(10000);
    ...
    function getSUBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _susdPrice = getSusdPrice();
        if (_susdPrice > susdPriceCeiling) {
    ...
    function initialize(
        ...
        susdPriceOne = 10**18;
        susdPriceCeiling = susdPriceOne.mul(1001).div(1000);

        susdSupplyTarget = 5000000 ether;

        maxSupplyExpansionPercent = 350; // Upto 3.5% supply for expansion

        boardroomWithdrawFee = 5; // 5% withdraw fee when under peg
        subondDepletionFloorPercent = 10000; // 100% of SUBond supply for depletion floor
        ...
        initialized = true;
        operator = msg.sender;
        sushareRewardPool = _sushareRewardPool;
        sushareRewardPoolExpansionRate = 0.0007916603 ether; // 50000 sushare / (731 days * 24h * 60min * 60s)
        sushareRewardPoolContractionRate = 0.0015833206 ether; // 2x
    ...
    function setOperator(address _operator) external onlyOperator {
     ...
     function setSUShareRewardPool(address _sushareRewardPool) external onlyOperator {
        sushareRewardPool = _sushareRewardPool;
    }
    ...
    function setsushareRewardPoolExpansionRate(uint256 _sushareRewardPoolExpansionRate) external onlyOperator {
       sushareRewardPoolExpansionRate = _sushareRewardPoolExpansionRate;
    }
    function setsushareRewardPoolContractionRate(uint256 _sushareRewardPoolContractionRate) external onlyOperator {
        sushareRewardPoolContractionRate = _sushareRewardPoolContractionRate;
    }
    ...
    function setSUBondDepletionFloorPercent(uint256 _subondDepletionFloorPercent) external onlyOperator {
        require(_subondDepletionFloorPercent >= 500 && _subondDepletionFloorPercent <= 10000, "out of range"); // [5%, 100%]
        subondDepletionFloorPercent = _subondDepletionFloorPercent;
    }
    ...
    function setSusdSupplyTarget(uint256 _susdSupplyTarget) external onlyOperator {
        require(_susdSupplyTarget > getSusdCirculatingSupply(), "too small"); // >= current circulating supply
        susdSupplyTarget = _susdSupplyTarget;
    }
    ...
    function buySUBonds(uint256 _susdAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_susdAmount > 0, "Treasury: cannot purchase subonds with zero amount");

        uint256 susdPrice = getSusdPrice();
        require(susdPrice == targetPrice, "Treasury: SUSD price moved");
        require(
            susdPrice < susdPriceOne, // price < $1
            "Treasury: susdPrice not eligible for subond purchase"
        );

        require(_susdAmount <= epochSupplyContractionLeft, "Treasury: not enough subond left to purchase");

        uint256 _rate = getSUBondDiscountRate();
        require(_rate > 0, "Treasury: invalid subond rate");

        uint256 _subondAmount = _susdAmount.mul(_rate).div(1e18);
        uint256 susdSupply = getSusdCirculatingSupply();
        uint256 newSUBondSupply = IERC20(subond).totalSupply().add(_subondAmount);
        require(newSUBondSupply <= susdSupply.mul(maxDebtRatioPercent).div(10000), "over max debt ratio");

        IBasisAsset(susd).burnFrom(msg.sender, _susdAmount);
        IBasisAsset(subond).mint(msg.sender, _subondAmount);

        epochSupplyContractionLeft = epochSupplyContractionLeft.sub(_susdAmount);
        _updateSusdPrice();

        emit BoughtSUBonds(msg.sender, _susdAmount, _subondAmount);
    }

    function redeemSUBonds(uint256 _subondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        require(_subondAmount > 0, "Treasury: cannot redeem subonds with zero amount");
        ...
        uint256 _susdAmount = _subondAmount.mul(_rate).div(1e18);
        ...
        IBasisAsset(subond).burnFrom(msg.sender, _subondAmount);
        IERC20(susd).safeTransfer(msg.sender, _susdAmount);

        _updateSusdPrice();

        emit RedeemedSUBonds(msg.sender, _susdAmount, _subondAmount);
    }
    ...
    function _sendToBoardroom(uint256 _amount) internal {
        ...
        if (insuranceFundSharedPercent > 0) {
            ...
            IERC20(susd).transfer(insuranceFund, _insuranceFundSharedAmount);
        ...
        IERC20(susd).safeApprove(boardroom, 0);
        IERC20(susd).safeApprove(boardroom, _amount);
        IBoardroom(boardroom).allocateSeigniorage(_amount);
        emit BoardroomFunded(now, _amount);
    }

    function _calculateMaxSupplyExpansionPercent(uint256 _susdSupply) internal returns (uint256) {
        if (_susdSupply >= susdSupplyTarget) {
            susdSupplyTarget = susdSupplyTarget.mul(12500).div(10000); // +25%
    ...
    function getSusdExpansionRate() public view returns (uint256 _rate) {
        if (epoch < bootstrapEpochs) { // 21 first epochs with 3.5% expansion
            _rate = bootstrapSupplyExpansionPercent;
        } else {
            uint256 _twap = getSusdPrice();
            if (_twap >= susdPriceCeiling) {
                uint256 _percentage = _twap.sub(susdPriceOne); // 1% = 1e16
    ...
    function getSusdExpansionAmount() external view returns (uint256) {
        uint256 susdSupply = getSusdCirculatingSupply().sub(seigniorageSaved);
        uint256 subondSupply = IERC20(subond).totalSupply();
        uint256 _rate = getSusdExpansionRate();
        if (seigniorageSaved >= subondSupply.mul(subondDepletionFloorPercent).div(10000)) {
            // saved enough to pay debt, mint as usual rate
            return susdSupply.mul(_rate).div(10000);
        } else {
            // have not saved enough to pay debt, mint more
            uint256 _seigniorage = susdSupply.mul(_rate).div(10000);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(susdSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
            emit Seigniorage(epoch, previousEpochSusdPrice, susdSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochSusdPrice >= susdPriceCeiling) {
                IBoardroom(boardroom).setWithdrawFee(0);
                // Expansion ($SUSD Price > 1 $PEG): there is some seigniorage to be allocated
                uint256 subondSupply = IERC20(subond).totalSupply();
                uint256 _percentage = previousEpochSusdPrice.sub(susdPriceOne);
                uint256 _savedForSUBond;
                ...
                if (seigniorageSaved >= subondSupply.mul(subondDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForBoardroom = susdSupply.mul(_percentage).div(1e18);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = susdSupply.mul(_percentage).div(1e18);
                    _savedForBoardroom = _seigniorage.mul(seigniorageExpansionFloorPercent).div(10000);
                    _savedForSUBond = _seigniorage.sub(_savedForBoardroom);
                    if (mintingFactorForPayingDebt > 0) {
                        _savedForSUBond = _savedForSUBond.mul(mintingFactorForPayingDebt).div(10000);
                    }
                }
                if (_savedForBoardroom > 0) {
                    _sendToBoardroom(_savedForBoardroom);
                }
                if (_savedForSUBond > 0) {
                    seigniorageSaved = seigniorageSaved.add(_savedForSUBond);
                    IBasisAsset(susd).mint(address(this), _savedForSUBond);
                    emit TreasuryFunded(now, _savedForSUBond);
                }

                if (
                    sushareRewardPool != address(0) &&
                    IRewardPool(sushareRewardPool).getRewardPerSecond() !=
                    sushareRewardPoolExpansionRate
                ) {
                    IRewardPool(sushareRewardPool).updateRewardRate(
                        sushareRewardPoolExpansionRate
                    );
                }
                
                emit Seigniorage(epoch, previousEpochSusdPrice, _savedForBoardroom);
            } else {
                IBoardroom(boardroom).setWithdrawFee(boardroomWithdrawFee);
                if (sushareRewardPool != address(0) && 
                IRewardPool(sushareRewardPool).getRewardPerSecond() != sushareRewardPoolContractionRate
                ) {
                    IRewardPool(sushareRewardPool).updateRewardRate(
                        sushareRewardPoolContractionRate
                    );
                }


                emit Seigniorage(epoch, previousEpochSusdPrice, 0);
            }
        }
        if (allocateSeigniorageSalary > 0) {
            IBasisAsset(susd).mint(address(msg.sender), allocateSeigniorageSalary);
        }
    }
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(subond), "subond");
        require(address(_token) != address(sushare), "sushare");
