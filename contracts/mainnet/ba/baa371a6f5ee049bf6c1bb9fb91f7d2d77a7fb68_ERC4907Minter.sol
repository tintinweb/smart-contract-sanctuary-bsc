...
// File: contracts\4907\IERC4907.sol
...
// File: contracts\4907\ERC4907.sol
...
// File: contracts\ERC4907Minter.sol
...
contract ERC4907Minter is ERC4907, Ownable {
    event Minted(uint256 tokenId, address to, uint256 price);
    event Burned(uint256 tokenId, address from, uint256 price);
    ...
    constructor(
        ...
        setCashier(address(this));
    }

    function mint(uint256 tokenId, address to, uint256 price) public onlyOwner  {
        if (msg.sender != cashier) {
    ...
    function burn(uint256 tokenId) public {
        ...
        _burn(tokenId);
        emit Burned(tokenId, msg.sender, priceMap[tokenId]);
        delete priceMap[tokenId];
    }
    function setCashier(address cashier_) public onlyOwner {
        require(cashier_ != address(0), "cashier_ can not be Zero Address");
        cashier = cashier_;
    }
}