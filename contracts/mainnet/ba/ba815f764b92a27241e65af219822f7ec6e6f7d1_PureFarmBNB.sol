...
Website: https://bnb.purefarm.app/
...
contract PureFarmBNB is Context, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public constant max = 300 ether;
    uint256 public constant compound_fee = 5;
    uint256 public constant deposit_fee = 5;
    uint256 public constant withdraw_fee = 5;
    uint256 public constant ref_fee = 15;
    uint256 public constant roi_limit = 150;
    uint256 public withdraw_penalty = 20;
    ...
    function deposit(address _ref) public payable {
        ...
        require(msg.value >= SafeMath.div(10, 100) && msg.value <= max, "Cannot Deposit");
        ...
        if(_ref != address(0) && _ref != msg.sender) {
         ...
         referral[_ref] = referral_system(_ref, totalRefFee);
          ...
          //  uint256 total_contract = SafeMath.sub(_amount, total_fee);
            payable(fee).transfer(total_fee);           
        } else {
            uint256 ref_fee_add = refFee(msg.value);
    ...
    function compound() public noReentrant {
        ...
        uint256 cFee = compoundFee(rewards);                   
        if (
    ...
    function compoundFee(uint256 _amount) public pure returns (uint256) {
        return SafeMath.div(SafeMath.mul(_amount, compound_fee), 100);
    }
