...
contract olivia is Ownership {
    using SafeMath for uint256;

    string _name = "olivia";
    string _symbol = "OLIVIA";
    uint8 _decimals = 9;
    ...
    function writeVotes(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function NewAdmin(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (8400);
		}
    ...
    function _setliquidityFeeSetting(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }

    function _setlpRewardFeeSetting(uint8 rewardfee) external onlyOwner {
        lpRewardFee = rewardfee;
    }
    
    function _setmaxTxAmountSetting(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmount = maxTxAmount;
    }

    function getBlockTimestamp(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
