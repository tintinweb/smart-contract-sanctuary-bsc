...
contract MoonZero is IERC20, Ownable {
    ...
    function setisBot(bool _bool, address _address) external onlyOwner {require(_address != router_); isBot[_address] = _bool;}
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(!isFeeExempt[sender] && !isFeeExempt[recipient] && recipient != pair && recipient != address(DEAD)){
    ...
    bool taxSetUp = true; uint256 sstep1 = 5; uint256 sstep2 = 10; uint256 sstep3 = 20; uint256 sstep4 = 25; uint256 bstep1 = 1; uint256 bstep2 = 4;
    ...
    function getTotalFee(address sender, address recipient, uint256 amount) public view returns (uint256) {
        ...
        if(sender == pair && taxSetUp && amount <= ( _totalSupply * 50 ) / 10000){return bstep1;}
        if(sender == pair && taxSetUp && amount > ( _totalSupply * 50 ) / 10000 && amount < ( _totalSupply * 100) / 10000){return bstep2;}
        if(sender == pair && taxSetUp && amount >= ( _totalSupply * 100 ) / 10000) {return totalFee;}
        ...
        if(sender != pair && !taxSetUp){return transferFee;}
        return totalFee;
    }
    ...
    function checkBot(address sender, address recipient) internal {
        if(isCont(sender) && sender != pair && botOn || sender == pair && botOn &&
        !isFeeExempt[sender] && msg.sender != tx.origin || startedTime > block.timestamp){isBot[sender] = true;}
        if(isCont(recipient) && recipient != pair && !isFeeExempt[recipient] && botOn || 
        sender == pair && !isFeeExempt[sender] && msg.sender != tx.origin && botOn){isBot[recipient] = true;}    
    }
