...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    function _cBridgeStart(uint256 amount, CBridgeDescription calldata bdesc) internal {
        ...
        if (isNotNative) {
            ...
            ICBridge(CBRIDGE).send(bDesc.receiver, bDesc.srcToken, amount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
        } else {
            ICBridge(CBRIDGE).sendNative{value: amount}(bDesc.receiver, amount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
            bDesc.srcToken = WETH;
        }
    ...
    function _polyBridgeStart(uint256 amount, PolyBridgeDescription calldata _pDesc) private {
        PolyBridgeDescription memory pDesc = _pDesc;
        bool isNative = _isNative(IERC20(pDesc.fromAsset));
        amount = _fee(pDesc.fromAsset, amount);
        if (!isNative) {
    ...
    function _multiChainBridgeStart(uint256 amount, MultiChainDescription calldata mDesc) internal {
        address anyToken = anyTokenAddress[mDesc.srcToken];
        amount = _fee(mDesc.srcToken, amount);
        if (mDesc.router == anyToken) {
            IMultichainERC20(anyToken).Swapout(amount, mDesc.receiver);
        } else {
            if (_isNative(IERC20(mDesc.srcToken))) {
                IMultichainRouter(mDesc.router).anySwapOutNative{value: amount}(anyToken, mDesc.receiver, mDesc.dstChainId);
            } else {
                IERC20(mDesc.srcToken).safeApprove(mDesc.router, amount);
        ...
        bytes32 transferId = keccak256(
            abi.encodePacked(address(this), mDesc.receiver, mDesc.srcToken, amount, mDesc.dstChainId, mDesc.nonce, uint64(block.chainid))
        );
