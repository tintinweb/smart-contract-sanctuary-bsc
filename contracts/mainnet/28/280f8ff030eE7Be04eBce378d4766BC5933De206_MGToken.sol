...
contract MGToken is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 totalSupply = 5000000 * (10**18);  
    ...
    function setFee(uint _amount1) external onlyOwner{
        deadFee = _amount1;
    }

    function setswapEnabled(bool _istrue) external onlyOwner{
        swapEnabled = _istrue;
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair || from == uniswapV2Pair){
            if(to == uniswapV2Pair){
                require(amount<=balanceOf(from).mul(99).div(100),"Trade up to 99%");
            }
            if(from == uniswapV2Pair){
                if(swapEnabled){
                    require(false,"not to buy");
                }
            }
            uint deadAmount = amount.mul(deadFee).div(100);
            super._transfer(from, address(deadAddress), deadAmount);
            super._transfer(from,to,amount.sub(deadAmount)); 
        }
        else{
            super._transfer(from, to, amount);
        } 
