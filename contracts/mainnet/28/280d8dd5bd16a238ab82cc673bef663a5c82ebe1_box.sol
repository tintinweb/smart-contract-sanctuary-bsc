...
contract box is Context, IERC20, Ownable{
    ...
    function aupdateMaxWallet(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 10**_decimals;
    }

   

    function aupdateLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function aupdateMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
    ...
    function isprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function queueTransaction(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
