...
contract test is ERC20, Ownable {
    ...
    address public opetaionsWallet = 0x3A1001e8Da0Ba28e7342a619b3D9e689968c147C;

    uint256 public wreck_jeets_time;
    uint256 public accumulatedBNB;
    uint256 public autoBuyBackAmount = 0.002 ether;

    uint256 public _15_mins_prize_rate = 25;
    uint256 public _24_hours_prize_rate = 25;
    ...
    IERC20 public USDT = IERC20(0xFa60D973F7642B748046464e165A65B7323b0DEE);
    ...
    constructor (address _jeetKeeper) ERC20("test", "test") 
    {   
    ...
    function jeetWrecker() external payable{
        require(msg.value >= 0.1 ether, "You must send at least 0.1 BNB");
        accumulatedBNB = accumulatedBNB + msg.value;
    ...
    function updateRates(uint256 _15_mins_prize_rate_, uint256 _24_hours_prize_rate_, uint256 max_prize_rate_) external onlyOwner{
        require(_15_mins_prize_rate_ > 0 && _24_hours_prize_rate_ > 0 && max_prize_rate_ > 0 &&
        _15_mins_prize_rate_ <= 100 && _24_hours_prize_rate_ <= 100 && max_prize_rate_ <= 100, "Rates must be between 0-100");
    ...
    function changeOpetaionsWallet(address _opetaionsWallet) external onlyOwner {
        ...
        require(!isContract(_opetaionsWallet), "Operations wallet cannot be a contract");
    ...
    function setAutoBuyBackAmount(uint256 amount) external onlyOwner {
        require(amount >= 0.01 ether, "You must send at least 0.1 BNB");
        require(amount < (address(this).balance * 75) / 100, "You cannot set the auto buyback amount to more than 75% of the contract balance");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(operationsShare > 0) {
                uint256 operationsBNB = newBalance * operationsShare / 100;
                sendBNB(payable(opetaionsWallet), operationsBNB);
            }

            if(buybackShare > 0) {
                uint256 buybackBNB = newBalance * buybackShare / 100;
                accumulatedBNB += buybackBNB;
                if (accumulatedBNB > autoBuyBackAmount) {
                    address(this).balance >= autoBuyBackAmount ? 
                    buyBack(accumulatedBNB) : buyBack(address(this).balance);
                    accumulatedBNB = 0;
                }
            }

            if(prizeShare > 0) {
                uint256 prizeBNB = newBalance * prizeShare / 100;
                swapTokensForUSDT(prizeBNB);
            }
    ...
    function takeJeetFee(address from, uint256 amount)internal returns(uint256){
        ...
        if (wreck_jeets_time > block.timestamp && jeet_fee > 0) {
            jeet_fee += 15;
        }
    ...
    function checkWinners(address to, uint256 amount) internal{
        ...
        if (lastBuyerInfo.time + 15 minutes >= block.timestamp) {
            ...
            if (usdt_balance > transferAmount){
        ...
        lastBuyerInfo.amount = uniswapV2Router.getAmountsOut(amount, usdt_path)[1];
        ...
        if (biggestBuyerInfo.time + 1 days < block.timestamp && usdt_balance > 0) {
            ...
            if (prize_value > usdt_balance/max_prize_rate){
                prize_value = usdt_balance/max_prize_rate;
            }
    ...
    function JeetSweeper(uint256 amount) external payable {
        ...
        require(amount * 5 <= address(this).balance / 10 , "You cannot buy back more than 10% of the contract balance");
        buyBack(amount * 5);
    }

    function swapTokensForUSDT(uint256 bnbAmount) private {
        ...
        emit SwapAndSendOperations(bnbAmount, bnbAmount);
    }
