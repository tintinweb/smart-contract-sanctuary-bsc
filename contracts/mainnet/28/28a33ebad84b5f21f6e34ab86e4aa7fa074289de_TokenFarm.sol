...
contract TokenFarm is Ownable {
    ...
    function getUserLockTime (uint256 _pid, address _user, uint256 _stakeNumber) public view returns (uint256) {
        return (userInfo[_pid][_user].stakingTime[_stakeNumber]).add((poolInfo[_pid].lockDays).mul(1 days));
    }

    function getUserStakingAmount (uint256 _pid, address _user) public view returns (uint256 [] memory) {
        return userInfo[_pid][_user].amount;
    }
    function getUserStakingTime (uint256 _pid, address _user) public view returns (uint256 [] memory) {
        return userInfo[_pid][_user].stakingTime;
    }
    ...
    function stakeTokens (uint256 _pid, uint256 _amount) public {
        ...
        for (uint256 i = 0; i < userInfo[_pid][msg.sender].numberOfStakes; i++){
            totalStakedTokens = totalStakedTokens.add(userInfo[_pid][msg.sender].amount[i]);
        }
        require (totalStakedTokens.add(_amount) <= poolInfo[_pid].maxContribution , "Max Contribution exceeds");
        ...
        uint256 numberOfStakes = userInfo[_pid][msg.sender].numberOfStakes;
        (userInfo[_pid][msg.sender].amount).push(_amount);
        (userInfo[_pid][msg.sender].stakingTime).push(block.timestamp);
        (userInfo[_pid][msg.sender].numberOfStakes) = numberOfStakes.add(1);
        
    }

    function claimableRewards (uint256 _pid, address _user, uint256 _stakeNumber) public view returns (uint256) {
        require (_pid < poolLength(), "Invalid pool ID");
        ...
        uint256 _refundValue = ((userInfo[_pid][_user].amount[_stakeNumber] * poolInfo[_pid].rewardNum).mul(10 ** rewardToken.decimals())).div (poolInfo[_pid].rewardDen * 10 ** token.decimals());
        return _refundValue;
    }

    function unstakeTokens (uint256 _pid, uint256 _stakeNumber) public {
        require (_pid < poolLength(), "Invalid pool ID");
        require (userInfo[_pid][msg.sender].amount[_stakeNumber] > 0 , "You don't have any staked tokens");
        require (userInfo[_pid][msg.sender].stakingTime[_stakeNumber] > 0 , "You don't have any staked tokens");
        require (getUserLockTime(_pid, msg.sender, _stakeNumber) < block.timestamp , "Your maturity time is not reached. If you want you can do EmergencyWithdraw");
        ...
        uint256 _refundValue = claimableRewards(_pid, msg.sender, _stakeNumber);
        poolInfo[_pid].currentPoolSize = (poolInfo[_pid].currentPoolSize).sub(userInfo[_pid][msg.sender].amount[_stakeNumber]);

        bool success1 = token.transfer(msg.sender, userInfo[_pid][msg.sender].amount[_stakeNumber]);
        bool success2 = rewardToken.transfer(msg.sender, _refundValue);
        require(success1 && success2, "Transfer failed");
        userInfo[_pid][msg.sender].amount[_stakeNumber] = 0;

    }

    function emergencyWithdraw (uint256 _pid, uint256 _stakeNumber) public {
        require (_pid < poolLength(), "Invalid pool ID");
        require (userInfo[_pid][msg.sender].amount[_stakeNumber] > 0 , "You don't have any staked tokens");
        require (getUserLockTime(_pid, msg.sender, _stakeNumber) > block.timestamp , "Your maturity time is reached. You can unstake tokens and enjoy rewards");

        uint256 _emergencyFees = poolInfo[_pid].emergencyFees;

        uint256 _refundValue = (userInfo[_pid][msg.sender].amount[_stakeNumber])
            .sub((_emergencyFees)
            .mul(userInfo[_pid][msg.sender].amount[_stakeNumber])
            .div(1000));
        poolInfo[_pid].currentPoolSize = (poolInfo[_pid].currentPoolSize).sub(userInfo[_pid][msg.sender].amount[_stakeNumber]);
        userInfo[_pid][msg.sender].amount[_stakeNumber] = 0;
