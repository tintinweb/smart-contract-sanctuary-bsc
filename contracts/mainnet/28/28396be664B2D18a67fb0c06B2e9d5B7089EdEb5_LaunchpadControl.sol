...
contract LaunchpadControl is 
  ...
  function initialize(
    ...
    __Membership_init();
    __PancakePairPrice_init(_aluVaultAddress, _aluBnbVaultAddress, _pancakeSwapRouter, _tokenAddress, _lpTokenAddress);
  ...
  function buyNFT(uint256 _id, uint256 _amount) external payable {
    ...
    if (item.currency == address(0x0)) {
      aluAmount = getPairPrice(msg.value, wethAddress);
    } else {
  ...
  function addAdmin(address newAdmin) external onlyOwner{
    require(!hasRole(CREATOR_ROLE, newAdmin), "Already has admin role");
    require(newAdmin != address(0x0), "Invalid new admin address");
  ...
  function addCreator(address newCreator) external {
    require(hasRole(ADMIN_ROLE, msg.sender), "Only admin can add creator");
  ...
  function lastAdmin() external view returns (address admin) {
    uint256 totalAdmins = getRoleMemberCount(ADMIN_ROLE);
    admin = getRoleMember(ADMIN_ROLE, totalAdmins - 1);
  }
...
contract PancakePairPrice {
  ...
  // Get pair price
  function getPairPrice(uint _amount, address _pairAddress) internal view returns (uint256 pairPrice) {
    if (_pairAddress == tokenAddress) {
      pairPrice = _amount;
    } else {
  ...
  function getTotalStaked(address account, address _wethAddress) public view returns (uint256 totalStakedAlu) {
    // ALU staked in ALU valut
    uint256 aluStaked = (IAluVault(aluVaultAddress).getPricePerFullShare().mul(IAluVault(aluVaultAddress).balanceOf(account))).div(10**18);
    // ALU-WBNB LP staked in ALU-BNB valut
    uint256 lpStaked = (IAluVault(aluBnbVaultAddress).getPricePerFullShare()).mul(IAluVault(aluBnbVaultAddress).balanceOf(account)).div(10**18);

    // WBNB balance of LP token address
    uint256 wethBalance = IERC20(_wethAddress).balanceOf(lpTokenAddress);
    // ALU balance of LP token address
    uint256 aluBalance = IERC20(tokenAddress).balanceOf(lpTokenAddress);

    // Convert WBNB to ALU to get total balance in ALU
    uint256 totalAluBalance = aluBalance.add(getPairPrice(wethBalance.div(10**10), _wethAddress).mul(10**10));
    // Total supply of LP token in LP token address
    uint256 lpTotalSupply = IPancakeERC20(lpTokenAddress).totalSupply();

    // Staked balance of account in ALU
    uint256 lpStakedInAlu = lpStaked.mul(totalAluBalance).div(lpTotalSupply);

    totalStakedAlu = aluStaked.add(lpStakedInAlu);
  }
}
...
contract Membership {
  ...
  function __Membership_init() internal {
    ...
    memberships[0].to = 20000*(10**18);
    memberships[0].discountRate = 3;
    memberships[0].maxAllocation = 20000*(10**18);

    // Bronze
    memberships[1].from = 20000*(10**18);
    memberships[1].to = 40000*(10**18);
    memberships[1].discountRate = 10;
    memberships[1].maxAllocation = 40000*(10**18);

    // Silver
    memberships[2].from = 40000*(10**18);
    memberships[2].to = 100000*(10**18);
    memberships[2].discountRate = 20;
    memberships[2].maxAllocation = 100000*(10**18);

    // Gold
    memberships[3].from = 100000*(10**18);
    memberships[3].to = 250000*(10**18);
    memberships[3].discountRate = 35;
    memberships[3].maxAllocation = 250000*(10**18);

    // Platinum
    memberships[4].from = 250000*(10**18);
