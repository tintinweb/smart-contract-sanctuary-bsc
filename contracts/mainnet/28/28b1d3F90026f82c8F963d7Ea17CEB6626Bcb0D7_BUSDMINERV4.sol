...
contract BUSDMINERV4 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    IERC20Upgradeable public Busd; //BUSD token
    ...
    uint256 public restrictedBuyAmountBusd;
    ...
    function store() public initializer {
        ...
        SpellSyrup = 750000;
        restrictedBuyAmountBusd = 1000_000000000000000000; // 1000 BUSD
    ...
    function sellSeas() public {
        ...
        // uint developerFee = devFee(seasValue);

        // uint deductedAmount = seasValue.sub(developerFee);

        // claimedSeas[msg.sender] = 0;
        // lastHatch[msg.sender] = block.timestamp;
        // marketSeas = SafeMathUpgradeable.add(marketSeas, hasSeas);

        // IERC20Upgradeable(Busd).transfer(developerWallet,developerFee);
        // IERC20Upgradeable(Busd).transfer(address(msg.sender), deductedAmount);
    ...
    function buySeas(address ref, uint256 amount) public {
        ...
        require(amount <= restrictedBuyAmountBusd,"Error: Cann't Purchase More than 10K!!");
        require(!blacklisted[msg.sender],"Opps: Your Blacklisted by Security!!");
        ...
        uint256 balance = IERC20Upgradeable(Busd).balanceOf(address(this));
        ...
        require(trident[msg.sender]._invested.add(amount) <= restrictedBuyAmountBusd,"Error: Cann't Purchase More than 10K!!");
    ...
    function magicSurf(address _target) internal {
        ...
        IERC20Upgradeable(Busd).transfer(developerWallet,developerFee);
        IERC20Upgradeable(Busd).transfer(address(_target), deductedAmount);
    ...
    function getEstimatedSeas(uint _amount) public view returns (uint256) {
        uint _balance = IERC20Upgradeable(Busd).balanceOf(address(this));
    ...
    function setBusdInCan(uint256 _Busd) public onlyOwner {
        Seas_TO_HATCH_1MINERS = _Busd;
    }

    function setBusd(address _adr) public onlyOwner {
        require(!initialized); //ones start cannot change token
        Busd = IERC20Upgradeable(_adr);
    }
