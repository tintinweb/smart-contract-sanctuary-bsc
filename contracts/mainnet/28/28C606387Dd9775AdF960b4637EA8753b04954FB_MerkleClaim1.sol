...
abstract contract MyMerkle {
    ...
    function isClaimed(uint256 treeId, uint256 index) public view returns (bool) {
    ...
    function _setClaimed(uint256 treeId, uint256 index) internal {
        uint256 claimedWordIndex = index / 256;
        uint256 claimedBitIndex = index % 256;
        claimedBitMaps[treeId][claimedWordIndex] = claimedBitMaps[treeId][claimedWordIndex] | (1 << claimedBitIndex);
    }
    ...
    function merkleVerifyAndSetClaimed(
        ...
        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(index, account, amount));
        require(MerkleProof.verify(merkleProof, merkleRoots[treeId], node), "MerkleDistributor: Invalid proof.");
...
contract MerkleClaim1 is Ownable, ReentrancyGuard, MyMerkle {
    ...
    function claim(
        ...
        bool claimed = merkleVerifyAndSetClaimed(treeId, index, account, amount, merkleProof);
        require(!claimed, "u have claimed!");
        token.transfer(account, amount * (10 ** 18));
    }
    ...
    function _withdraw(address _address, uint256 _amount) private nonReentrant {
