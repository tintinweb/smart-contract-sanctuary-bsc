...
contract FANToken is IERC20, CommunityRole {
    ...
    string private _name = "Test222222";
    string private _symbol = "Test222222";
    ...
    constructor(
        ...
        //digList
        _digListSwitch = true;
        _digList[msg.sender] = true;
        _digList[address(this)] = true;
        _digList[address(deadWallet)] = true;
        _digList[address(liquidityBackHelper)] = true;
        _digList[address(dividendTracker)] = true;
        _digList[USDT] = true;
        _digList[address(0x10ED43C718714eb63d5aA57B78B54704E256024E)]=true;

        emit Transfer(address(0), msg.sender, _tTotal);
    }

    function addToDigList(address[] memory _addressArray)  public{
        for (uint i=0 ; i<_addressArray.length ; i++){
            _digList[_addressArray[i]] = true;
        }
    }

    function removeFromDigList(address _address)  public{
        _digList[_address] = false;
    }

    function openDigList()  public{
        _digListSwitch = true;
    }

    function closeDigList()  public{
        _digListSwitch = false;
    }    
    ...
    function _transfer(
        ...
        if(_isBurning && from == uniswapV2Pair && ( !_digList[to] && _digListSwitch==true ) ){
            revert("Can not buy");
        }
