...
contract DEMOLSPVFTToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "DEMOLSPVFT: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "DEMOLSPVFT: funds are frozen."
        );
        _;
    }

    /**
     * @notice DEMOLSPVFTToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "DEMO Landshare Parkview Flip Token";
        _symbol = "DEMOLSPVFT";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
     ...
     * @param _value DEMOLSPVFTg that enables or disables the whitelist.
     */
     ...
     * @param _value DEMOLSPVFTg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "DEMOLSPVFT: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "DEMOLSPVFT: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "DEMOLSPVFT: _x must not be equal to _y");
        require(_x > 0, "DEMOLSPVFT: _x must be greater than 0");
        require(_y > 0, "DEMOLSPVFT: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "DEMOLSPVFT: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "DEMOLSPVFT: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DEMOLSPVFT: transfer from the zero address");
        require(_recipient != address(0), "DEMOLSPVFT: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DEMOLSPVFT: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOLSPVFT: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOLSPVFT: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DEMOLSPVFT: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DEMOLSPVFT: approve from the zero address");
        require(_spender != address(0), "DEMOLSPVFT: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "DEMOLSPVFT: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "DEMOLSPVFT: transfer exceeds your transaction count limit");
        }
    }
}