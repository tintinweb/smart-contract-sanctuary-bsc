...
interface IMMMInterest {
    function setBuyTimes(address _user) external;

    function checkHasInterest(address _user) external view returns (bool);
}
...
contract GMH is ERC20, Ownable {
    ...
    address public _burn = 0x705B5343B81D1813B2A2064beE8CA50525ab179c;
    address public _market = 0xbdBeBc4F9A7dB4adA68D07F78bcD5200a2EC050A;
    address public _interest;
    ...
    uint256 public dividendTokensAtAmount;
    ...
    uint256 private buyDividendFee = 4;
    uint256 private sellDividendFee = 4;
    ...
    uint256 private AmountDeadFee = 0;
    ...
    constructor() ERC20("GMH", "GMH") {
        ...
        excludeFromFees(_burn, true);
        excludeFromFees(address(this), true);
        _creator = msg.sender;

        swapTokensAtAmount = 20000000000 * (10**18);
        dividendTokensAtAmount = 500 * (10**18);
    ...
    function setInterestAddr(address _addr) external onlyOwner {
        _interest = _addr;
        dividendTracker.setInterest(_addr);
    }
    ...
    function getCurPrice() public view returns (uint256 _price) {
    ...
    function setSwapAndXEnabled(
        bool _swapAndLiquifyEnabled,
        bool _swapAndDividendEnabled
    ) public onlyOwner {
        swapAndLiquifyEnabled = _swapAndLiquifyEnabled;
        swapAndDividendEnabled = _swapAndDividendEnabled;
    }
    ...
    function setTokensAtAmount(
        uint256 _swapTokensAtAmount,
        uint256 _dividendTokensAtAmount
    ) public onlyOwner {
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount &&
            AmountLiquidityFee + AmountMarketFee + AmountDeadFee >=
            swapTokensAtAmount;
        bool canDividend = contractTokenBalance >= AmountDividendFee &&
            (AmountDividendFee * getCurPrice()) / 10**18 >=
            dividendTokensAtAmount;
        ...
        if (
            ...
            swapping = true;
            swapAndSendDividends(AmountDividendFee);
            swapping = false;
        } else if (
            canSwap &&
            !swapping &&
            ...
            if (AmountDeadFee > 0) {
                swapTokensForDToken(AmountDeadFee, _burn);
                AmountDeadFee = 0;
            }
            swapping = false;
        }
        ...
        if (automatedMarketMakerPairs[from]) {
            IMMMInterest(_interest).setBuyTimes(to);
        }
        if (
            !automatedMarketMakerPairs[from] &&
...
contract dDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "d_Dividend_Tracker: claimWait must be updated to between 0.5 and 24 hours"
        );
        require(
    ...
    function setInterest(address _addr) external onlyOwner {
        interest = _addr;
    }
    ...
    function processAccount(address payable account, bool automatic)
        ...
        returns (bool)
    {
        if (!IMMMInterest(interest).checkHasInterest(account)) {
            if (!excludedFromDividends[account]) {
