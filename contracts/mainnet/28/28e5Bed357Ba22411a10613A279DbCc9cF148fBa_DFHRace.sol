...
import "./lib/IDHorseNft.sol";

contract DFHRace is Ownable {
    ...
    event ReFunded(uint256 raceId, uint256 horseId, uint256 amount, address receiver);
    ...
    function setPcent(uint32[3] memory _pcent) external isOwnerOrOperator{
        pcent = _pcent;
    }

    function setFee(uint256 _fee) external isOwnerOrOperator{
        fee = _fee;
    }
    ...
    function getReFund(uint256 _raceId, uint256 _participationIndex, uint256 _tokenId) external {
        ...
        require(nftToken.ownerOf(_tokenId) == msg.sender, "Invalid horse owner");
