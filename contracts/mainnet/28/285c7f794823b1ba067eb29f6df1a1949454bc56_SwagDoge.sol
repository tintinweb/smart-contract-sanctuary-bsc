...
contract SwagDoge is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SwagDogeDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
    
    uint256 public swapTokensAtAmount = 1000 * (10**18); //0.00001%
    ...
    address public _marketingWalletAddress = 0x12E6D014c0F05631e9EE86eD9E8f9d4BD2d9cca1;
    ...
    constructor() public ERC20("Swag Doge", "SwagDoge") {

    	dividendTracker = new SwagDogeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SwagDoge: The dividend tracker already has that address");

        SwagDogeDividendTracker newDividendTracker = SwagDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SwagDoge: The new dividend tracker must be owned by the SwagDoge token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SwagDoge: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SwagDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SwagDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SwagDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "SwagDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SwagDoge: Cannot update gasForProcessing to same value");
...
contract SwagDogeDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SwagDoge_Dividen_Tracker", "SwagDoge_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "SwagDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SwagDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SwagDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SwagDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SwagDoge_Dividend_Tracker: Cannot update claimWait to same value");
