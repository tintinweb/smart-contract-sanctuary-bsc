...
/* Interface of Litedex Router 01 */
...
/* Router Smart Contract of Litedex */
contract LitedexRouter is ILitedexRouter02 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "Msg: Expired");
        _;
    }

    constructor(address _factory, address _WETH) public {
        require(_factory != address(0) && _WETH != address(0), "Msg: zero address");
        factory = _factory;
        WETH = _WETH;
    }
    ...
    receive() external payable {
        ...
        require(msg.sender == WETH, "Msg: msg sender is not main currency");
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "Msg: Insufficient B Amount");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = LitedexLibrary.quote(amountBDesired, reserveB, reserveA);
                require(amountAOptimal <= amountADesired, "Msg: amountAOptimal higher than amountADesired");
                require(amountAOptimal >= amountAMin, "Msg: Insufficient A Amount");
    ...
    function addLiquidity(
        ...
        require(tokenA != address(0) && tokenB != address(0), "Msg: zero address");
    ...
    function addLiquidityETH(
        ...
        require(IWETH(WETH).transfer(pair, amountETH), "Msg: transfer ETH Failed");
        liquidity = ILitedexPair(pair).mint(to);
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, "Msg: Insufficient A Amount");
        require(amountB >= amountBMin, "Msg: Insufficient B Amount");
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, "Msg: Insufficient Output Amount");
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, "Msg: Excessive Input Amount");
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, "Msg: Invalid Path");
        amounts = LitedexLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, "Msg: Insufficient Output Amount");
        IWETH(WETH).deposit{value: amounts[0]}();
        require(IWETH(WETH).transfer(LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]), "Msg: transfer ETH Failed");
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, "Msg: Invalid Path");
        amounts = LitedexLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "Msg: Excessive Input Amount");
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, "Msg: Invalid Path");
        amounts = LitedexLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, "Msg: Excessive Input Amount");
        IWETH(WETH).deposit{value: amounts[0]}();
        require(IWETH(WETH).transfer(LitedexLibrary.pairFor(factory, path[0], path[1]), amounts[0]), "Msg: transfer ETH failed");
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        require(_to != address(0), "Msg: zero address");
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = LitedexLibrary.sortTokens(input, output);
        ...
        ensure(deadline)
    {
        ...
        require(IWETH(WETH).transfer(LitedexLibrary.pairFor(factory, path[0], path[1]), amountIn), "Msg: transfer ETH Failed");
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, "Msg: Insufficient Output Amount");
...
library LitedexLibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, "Msg: Identical Address");
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f64cc087db7d69bd3b8fc640af3b763073350d7877c099bd086e2725087a25d5' // init code hash
            ))));
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, "Msg: Insufficient Amount");
        require(reserveA > 0 && reserveB > 0, "Msg: Insufficient Liquidity");
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, "Msg: Insufficient Input Amount");
        require(reserveIn > 0 && reserveOut > 0, "Msg: Insufficient Liquidity");
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, "Msg: Insufficient Output Amount");
        require(reserveIn > 0 && reserveOut > 0, "Msg: Insufficient Liquidity");
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, "Msg: Invalid Path");
