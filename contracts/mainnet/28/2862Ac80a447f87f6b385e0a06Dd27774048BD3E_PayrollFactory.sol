...
contract PayrollFactory is Ownable, Pausable {
    ...
    constructor(address _masterInstance) {
        setMasterInstance(_masterInstance);
        _transferOwnership(tx.origin);
    }

    function create() external whenNotPaused {
        require(address(instances[msg.sender]) == address(0), 'PayrollFactory: sender already have owned instance');
    ...
    function updateOwner(address currentOwner, address newOwner) external {
        ...
        require(newOwner != address(this), 'PayrollFactory: factory can not be an employer');

        require(instances[currentOwner] == msg.sender, 'PayrollFactory: unknown instance');
        require(address(instances[newOwner]) == address(0), 'PayrollFactory: newOwner already have owned instance');
    ...
    //     require(address(instances[employer]) != address(0), 'PayrollFactory: not created');
    ...
    //     require(address(instances[employer]) != address(0), 'PayrollFactory: not created');
    //     return instances[employer].lastUpdateTime();
    // }
    ...
    function setMasterInstance(address newInstance) public onlyOwner {
        require(newInstance != address(0), 'PayrollFactory: master instance address can not be a zero');
        require(newInstance != address(this), 'PayrollFactory: factory can not be a master instance');
        require(newInstance != masterInstance, 'PayrollFactory: already set');
