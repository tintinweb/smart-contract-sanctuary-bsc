...
contract CompoundSpyVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function getSwappingPair() internal view returns (IUniswapV2Pair) {
        return IUniswapV2Pair(
            factory.getPair(address(spy), address(token1))
        );
    }
    ...
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 token1 and an amount in SPY corresponding to 500 token1
    */
    ...
    * exp. amount = 1000 SPY
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 SPY and an amount in token1 corresponding to 500 SPY
    */
    ...
    function deposit(uint256 amount, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        uint256 spyOut = swapToken1ToSpy(amountToSwap);
        ...
        if(amount > token1Added+amountToSwap){
            token1.transfer(msg.sender, amount - (token1Added + amountToSwap));
        }
    }
    ...
    function depositTokenPair(
        ...
        uint256 balanceOfSpyAfterTransfer = spy.balanceOf(address(this));
        uint256 amountSpyReceived = balanceOfSpyAfterTransfer - balanceOfSpyBeforeTrasnfer;
        // note spyAdded is might reduced by ~1%
        (uint256 spyAdded, uint256 token1Added, uint256 liquidityAmount) = router.addLiquidity(
            address(spy),
            address(token1),
            amountSpyReceived,
            amountToken1,
        ...
        if(amountToken1 > token1Added){
            token1.transfer(msg.sender, amountToken1 - token1Added);
        }
        _depositLP(msg.sender, liquidityAmount, referrer);
    }
    ...
    function withdraw(uint256 amount, bool isReceiveToken1) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if(isReceiveToken1){
            ...
            token1.transfer(msg.sender, swapSpyToToken1(amountA).add(amountB));
        }else{
    ...
    function harvest(bool isReceiveToken1) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (reward > 0) {
            ...
            (uint256 amountSpy,uint256 amountToken1) = removeLiquidity(lpNeeded);
            if(isReceiveToken1) {
                ...
                spy.transfer(msg.sender, amountSpy);
            }
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 token1Out = swapSpyToToken1(spyAmountToSwap);
            (,, uint256 liquidityAmount) = router.addLiquidity(
                address(spy),
                address(token1),
                rewardForPool.sub(spyAmountToSwap),
                token1Out,
                0,
                0,
                address(this),
    ...
    function swapToken1ToSpy(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function swapSpyToToken1(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function removeLiquidity(uint256 lpAmount) internal returns (uint256 amountSpy, uint256 amountToken1){
        uint256 spyBalanceBefore = spy.balanceOf(address(this));
    ...
    function getPairReserves() internal view returns (uint reserveA, uint reserveB) {
        address token0 = address(spy) < address(token1) ? address(spy) : address(token1);
    ...
    function getToken1SpyRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(token1);
        paths[1] = address(spy);
    }

    function getSpyToken1Route() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(spy);
