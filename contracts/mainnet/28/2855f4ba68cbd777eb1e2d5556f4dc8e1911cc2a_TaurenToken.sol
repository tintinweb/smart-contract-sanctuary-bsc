...
contract TaurenToken is Context, IBEP20, Ownable {
    ...
    uint256 private numTokensToSwap =  2000000 * 10**18;
    // uint256 private swapCoolDownTime = 20;
    // uint256 private lastSwapTime;


    address public BUSD = 0x55d398326f99059fF775485246999027B3197955;
    // address public BUSD = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;

    address public castingPoolAddress = 0xAEAcecC74Ab607CE5431A903f0f2cD28dDFF5DA4;//铸造池

    address public operationAddress = 0xa0669bCB3f37dEbeB8490f2CcaB4c1816CB61a43;//运营，开发2%

    address public genesisPoolAddress = 0x43433e85eB95E81026E6067f2Ff678EEeB0f0F65;//创世分红,创世销毁14%

    address public ecologicalPool = 0xB99f7eEaa97fa52971741785ce52779174E9c2d4;//生态、质押，42%

    address public genesisReceiveAddress = 0xaBeB2a70d69E5a00b0C2a77360ad4665E1dD8e72;//创世100BNB接收地址


    address public capitalPool = 0xBe194A5F3EF947D0F675b29FE38d605778Ef5489;//发射21%

    address public IDOAddress = 0x75158D8FEF335c46AC9Bc0ef6140595c58898D8F;//IDO 21%

    address public relayAddress = 0x54D5E16123E53c204401daF1097AA2Ba4721da56;//手续费中转地址
    ...
    mapping (uint256 => uint256) public bullPeriodTXAmount;

    uint256 public bullPeriodIndex = 1;

    uint256 public periodTXNum = 50;
    ...
    constructor () {
        ...
        _balances[capitalPool] = 2100000000 * 10**18;
        _balances[IDOAddress] = 2100000000 * 10**18;
        _balances[genesisPoolAddress] = 1400000000 * 10**18;
        _balances[ecologicalPool] = 4200000000 * 10**18;
        _balances[operationAddress] = 200000000 * 10**18;

        emit Transfer(address(0), capitalPool, 2100000000 * 10**18);
        emit Transfer(address(0), IDOAddress, 2100000000 * 10**18);
        emit Transfer(address(0), genesisPoolAddress, 1400000000 * 10**18);
        emit Transfer(address(0), ecologicalPool, 4200000000 * 10**18);
        emit Transfer(address(0), operationAddress, 200000000 * 10**18);

        TRN = TaurenNFT(0xD382903BEf60fB9E9B272bC7dFBbE67fe6DC6F0A);

        pledgeTypeToRate[10] = 100;
        pledgeTypeToRate[20] = 100;
        pledgeTypeToRate[30] = 200;
        pledgeTypeToRate[40] = 200;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            uint256 tokenAmount = BUSDEqualToToken(100 * 10 ** 18);

            takeLiquidFee = true;

            if(amount >= tokenAmount){

                bullPeriodTXAmount[bullPeriodIndex]++;

                if(from == pancakeswapV2Pair) bullUsers[bullPeriodIndex].push(to);

                if(bullPeriodTXAmount[bullPeriodIndex] >= periodTXNum && bullUsers[bullPeriodIndex].length > 0){
                    _activateBullNFT();
                }
            }       
        }
    ...
    function swapAndCharge(uint256 tokenBalance) public lockTheSwap {
        uint256 initialBalance = ERC20(BUSD).balanceOf(relayAddress);

        swapTokensForEth(tokenBalance);

        uint256 newBalance = ERC20(BUSD).balanceOf(relayAddress).sub(initialBalance);
        ...
        ERC20(BUSD).transferFrom(relayAddress, castingPoolAddress, castingPoolFee);
        ERC20(BUSD).transferFrom(relayAddress, operationAddress, operationFee);
        ERC20(BUSD).transferFrom(relayAddress, genesisPoolAddress, genesisPoolFee);

        emit SwapAndCharged(tokenBalance, ERC20(BUSD).balanceOf(relayAddress), newBalance);
    }
    ...
    function activateBullNFT() public onlyOwner {
        _activateBullNFT();
    }

    function _activateBullNFT() private {
    ...
    function getHammerOwner(uint256 _bullPeriodIndex) private returns(address user) {
        ...
        require(bullUsers[_bullPeriodIndex].length > 0, "There are no purchasing users in this period");
    ...
    function setCastingPoolAddress(address _castingPoolAddress) external onlyOwner {
        castingPoolAddress = _castingPoolAddress;
    }
    ...
    function setGenesisPoolAddress(address _genesisPoolAddress) external onlyOwner {
        genesisPoolAddress = _genesisPoolAddress;
    }

    function setEcologicalPool(address _ecologicalPool) external onlyOwner {
        ecologicalPool = _ecologicalPool;
    }

    function setGenesisReceiveAddress(address _genesisReceiveAddress) external onlyOwner {
        genesisReceiveAddress = _genesisReceiveAddress;
    }

    function setCapitalPool(address _capitalPool) external onlyOwner {
    ...
    function setPeriodTXNum(uint256 _periodTXNum) public onlyOwner {
        periodTXNum = _periodTXNum;
    }

    function setTotalSealTimes(uint256 _totalSealTimes) public onlyOwner {
        totalSealTimes = _totalSealTimes;
    }

    function getPrivatePlacementProgress() public view returns(uint256) {
        return genesisPrivatePlacementAmount.mul(100).div(genesisPrivatePlacementTop);
    }

    function getPrivatePlacementDetail() public view returns(uint256,uint256,uint256) {
        return (genesisPrivatePlacementAmount.div(10 ** 15),genesisPrivatePlacementTop.div(10 ** 15),genesisPrivatePlacementAmount.mul(100).div(genesisPrivatePlacementTop));
    }

    function genesisPrivatePlacement() public payable returns(uint256[] memory tokenIds) {

        uint256 BNBAmount = msg.value.div(10 ** 18);
        ...
        payable(genesisReceiveAddress).transfer(msg.value);
        ...
        genesisPrivatePlacementAmount = genesisPrivatePlacementAmount.add(msg.value);
        ...
        _transfer(genesisPoolAddress, deadWallet, 14000000 * 10 **18);
    ...
    function rescueBullNFT(uint256 _bullPeriodIndex) public {
        ...
        require(hammerFixTime[_bullPeriodIndex] + 30 minutes < block.timestamp, "Hammer not yet repaired");
    ...
    function sellTaurenNFT(uint256 tokenId) public {
        ...
        ERC20(BUSD).transferFrom(ecologicalPool, msg.sender, TRN.typeToNFTPrice(1));
    ...
    function pledgeNFT(uint256 types, uint256 tokenId) public {
        ...
        if(types == 2){
            ...
            _transfer(msg.sender, ecologicalPool, tokenAmount);
        }
    ...
    function quitPledgeNFT(uint256 tokenId) public {
        ...
        if(tokenAmount + profit > 0){
            _transfer(ecologicalPool, msg.sender, tokenAmount + profit);
        }
    ...
    function withdrawPledgeProfit(uint256 tokenId) public {
        ...
        if(profit > 0){
            _transfer(ecologicalPool, msg.sender, profit);
        }
    ...
    function getMyPledgeNFT(address user) public view returns(Pledge[] memory myPledgeNFTs, uint256[] memory rates, uint256[] memory profits) {

        uint256[] memory ids = TRN.getUserPledgeNFTIDs(user);

        myPledgeNFTs = new Pledge[](uint256(ids.length));
        rates = new uint256[](uint256(ids.length));
        profits = new uint256[](uint256(ids.length));

        for(uint256 i = 0; i < ids.length; i++){
            myPledgeNFTs[i] = tokenIdToPledges[ids[i]];
            rates[i] = pledgeTypeToRate[tokenIdToPledges[ids[i]].types];
            profits[i] = getPledgeProfit(ids[i]).div(10 ** 18);
        }

        return (myPledgeNFTs,rates,profits);
    }
    ...
    function tokenEqualToBUSD(uint256 tokenAmount) public view returns(uint256) {
        
        uint256 tokenOfPair = balanceOf(pancakeswapV2Pair);

        uint256 BUSDOfPair = ERC20(BUSD).balanceOf(pancakeswapV2Pair);

        uint256 amount = tokenAmount.mul(BUSDOfPair).div(tokenOfPair);
    ...
    function getPledgeProfit(uint256 tokenId) public view returns(uint256){
        ...
        if(tokenIdToPledges[tokenId].lastWithdrawTime > tokenIdToPledges[tokenId].joinTime){
            pledgeDays = block.timestamp.div(24 hours) - tokenIdToPledges[tokenId].lastWithdrawTime.div(24 hours);
        }else{
            pledgeDays = block.timestamp.div(24 hours) - tokenIdToPledges[tokenId].joinTime.div(24 hours);
        }   

        uint256 profit = tokenAmount.mul(pledgeTypeToRate[tokenIdToPledges[tokenId].types]).mul(pledgeDays).div(36500);
    ...
    function getGenesisUser() public view returns(address[] memory) {
        return genesisUser;
    }

    function checkGenesisUser(address user) public view returns(bool) {
        for(uint256 i = 0; i < genesisUser.length; i++){
            if(user == genesisUser[i]) return true;
        }
