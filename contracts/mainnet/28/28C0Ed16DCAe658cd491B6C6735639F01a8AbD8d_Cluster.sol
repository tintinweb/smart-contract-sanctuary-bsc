...
interface ICosmicDelegator {
    function getMultiplier(uint256 _from, uint256 _to) external view returns (uint256);
    function pendingReward(address _user, uint256 _depositBlock, uint256 _totalShares, uint256 _share) external view returns(uint256); // total earn * _ share / totalShare
    function harvestUserReward(address _user, uint256 _depositBlock, uint256 _totalShares, uint256 _userShares) external;
    function getRewardBlocks() external view returns(uint256,uint256);
    function getRewardToken() external view returns(IERC20);
    function getTotalEarning() external view returns(uint256);
    function updateDelegator() external;
}
pragma solidity >=0.8.9;
...
interface IHeadAstronaut {
    ...
    function buzzPerBlock() external view returns (uint256);
    function poolLength() external view returns (uint256);
    function getMultiplier(uint256 _from, uint256 _to) external view returns (uint256);
    function pendingBuzz(uint256 _pid, address _user) external view returns (uint256);
...
// Clusters are locked stake system of Cosmosium Finance



contract Cluster is Ownable, Pausable {
    using SafeERC20 for IERC20;
    using SafeBEP20 for BEP20; // changed
    ...
    BEP20 public mockToken;
    IHeadAstronaut public immutable Astronaut; 
    ...
    struct UserInfo {
        ...
        uint256 buzzAtLastUserAction; // keeps track of cake deposited at the last user action
    ...
    uint256 public poolId; // pool id of $BUZZ farm.
    // this address should be excluded from $BUZZ fees to working properly.
    uint256 public timeLock = 7 days;
    bool public disableDelegators = false;

    constructor(BEP20 _mockToken, IERC20 _stakingToken, IHeadAstronaut _astronaut, address _treasury, uint256 _depositFee, uint256 _minDeposit, uint256 _poolId) {
        ...
        BEP20(_mockToken).approve(address(_astronaut), type(uint256).max);
    }
    ...
    function deposit(uint256 _amount) public whenNotPaused notContract{
        ...
        user.buzzAtLastUserAction = (user.shares * balanceOf()) / totalShares;
     ...
     * @return Returns total pending buzz rewards
     */
    function calculateTotalPendingBuzzRewards() external view returns (uint256) {
        uint256 amount = Astronaut.pendingBuzz(poolId, address(this));
    ...
    function userBalance(address _user) external view returns(uint256) {
        ...
        return (balance * user.shares) / totalShares;
    }

    function withdrawTime(address _user) external view returns(uint256) {
        UserInfo storage user = userInfo[_user];
        if(user.lastUserActionTime + timeLock <= block.timestamp) {
            return 0;
        } else {
            return (user.lastUserActionTime + timeLock) - block.timestamp;
        }
    }
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        require(user.lastUserActionTime + timeLock <= block.timestamp, "Still have time to withdraw");
        ...
        if (bal < currentAmount) {
            ...
            Astronaut.withdraw(poolId, balWithdraw);
    ...
    function _earn() internal {
        ...
        // harvest or initial deposit
        uint256 balance = mockToken.balanceOf(address(this));
        IHeadAstronaut(Astronaut).deposit(poolId, balance);
    ...
    function addDelegator(ICosmicDelegator _delegator, bool _start) public onlyOwner { // Delegator will be token distributor. Staking tokens should be approved if it BUZZ farm
        //approve unutma
    ...
    function emergencyWithdraw() external onlyAdmin {
        Astronaut.emergencyWithdraw(poolId);
    }

    function setPoolId(uint256 _poolId) external onlyAdmin {
        poolId = _poolId;
    }

    function setTimeLock(uint256 _time) external onlyAdmin {
        timeLock = _time;
    }
    /**
