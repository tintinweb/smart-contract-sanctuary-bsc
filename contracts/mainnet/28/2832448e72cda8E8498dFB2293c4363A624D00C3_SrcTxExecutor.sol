...
contract SrcTxExecutor is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(usdcAddresses.length == chainIds.length, "Construstor params length is not the same");
    ...
    function initCrossSwap(
        ...
        emit SrcCrossSwap(srcToken, srcAmount, destChainId, destToken, destAmount, destUser, usdcIncome);
    }
    ...
    function executeSrcSwap(
        ...
        if (usdcByChainId[block.chainid] == srcToken) {
            IERC20Upgradeable(srcToken).safeTransferFrom(msg.sender, ccSmartWallet, srcAmount);
        } else {
