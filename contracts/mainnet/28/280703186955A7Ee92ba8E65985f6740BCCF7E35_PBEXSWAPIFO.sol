...
contract PBEXSWAPIFO is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 pbexAtLastUserAction; // keeps track of pbex deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked pbex numbers (not include compoud pbex) at last action
        uint256 lastValidActionBalance; // staked pbex numbers in ifo valid period
    ...
    IERC20 public immutable token; // Pbex token
    ...
    /**
     * @notice Constructor
     * @param _token: Pbex token contract
    ...
    /**
     * @notice Deposits funds into the Pbex Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in PBEX)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.pbexAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    function blockUpdate(uint256 _startBlock,uint256 _endBlock) public onlyOwner {
        startBlock = _startBlock;
        endBlock = _endBlock;
    }

    function increaseAllowance() public onlyOwner {
        IERC20(token).safeApprove(address(masterchef), type(uint256).max);
    }
     ...
     * @param _lastValidActionBalance: staked pbex number at last action.
     ...
     * @param _amount:the pbex amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as Pbex Vault withdraw
    ...
    /**
     * @notice Reinvests PBEX tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Pbex Vault
     */
     ...
     * @return Expected reward to collect in PBEX
     */
    function calculateHarvestPbexRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingPbex(0, address(this));
     ...
     * @return Returns total pending pbex rewards
     */
    function calculateTotalPendingPbexRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingPbex(0, address(this));
