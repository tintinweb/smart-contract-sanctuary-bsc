...
contract MUCHO is IERC20, Ownable {
    ...
    function ClaimRewardOfTheDay() external {claimRewards(msg.sender, MEME);}
    ...
    function setCustomReward(address _reward) external onlyOwner {MEME = _reward;}
    ...
    function claimRewards(address shareholder, address _reward) internal {
        ...
        totalDistributed = totalDistributed.add(amount);
        eUSDC.approve(address(router), amount);
            ...
            address(this),
            block.timestamp);
        uint256 balance = (address(this).balance).sub(balanceBefore);
        swapEthtoToken(balance, shareholder, _reward);
        currentDividends = currentDividends.sub(amount);
        shareholderClaims[shareholder] = block.timestamp;
        shares[shareholder].totalRealised = shares[shareholder].totalRealised.add(amount);
        shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);}
    ...
    function swapEthtoToken(uint256 amount, address shareholder, address _reward) internal {
        address[] memory path = new address[](2);
        path[0] = _router.WETH();
        path[1] = address(_reward);
