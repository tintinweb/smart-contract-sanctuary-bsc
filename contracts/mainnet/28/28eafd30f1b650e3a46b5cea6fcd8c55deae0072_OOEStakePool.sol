...
contract OOEStakePool is ReentrancyGuard,Ownable{
    ...
    mapping (uint256 => Lock) public _lock;
    mapping (address => uint256[]) private _locksId;
    mapping (address => bool) public signers;
    mapping (address => uint256) public gasRefundNonce;
    mapping (uint256 => uint256) public rates; //rates, not APY. ex: need 12% : insert 1200000000
    ...
    function setRates(uint256[] memory months, uint256[] memory rates_) external onlyOwner {
        require(months.length == rates_.length, "OPENOCEAN_STAKING_POOL_V1:WRONG_INPUT");
        uint256 locksAmount = months.length;
        for(uint256 i = 0; i<locksAmount; i++){
            rates[months[i]] = rates_[i];
        }
    }
        ...
        for(uint256 i = 0; i < locksAmount; i++){
            ...
            if(lo.state == State.LOCKING){
                if(lo.timestampLockEnd > block.timestamp){
                    ...
                    power = power + lo.XOOEBaseAmount + (block.timestamp - lo.timestampLockEnd) * rate * lo.XOOEBaseAmount / 3110400000000;
                }
            }
        }
        return power;
...
// harvest all the locks OOE
    function harvestall() external notPause{
        ...
        for(uint256 i = 0; i<locksAmount; i++){
            ...
            uint256 rate = _minimumIncrease();
            availableOOE[msg.sender] = availableOOE[msg.sender] + lo.XOOEBaseAmount + (block.timestamp - lo.timestampLockEnd) * rate * lo.XOOEBaseAmount / 3110400000000;
            totalOOE[msg.sender] = totalOOE[msg.sender] + lo.XOOEBaseAmount - lo.OOELockedAmount + (block.timestamp - lo.timestampLockEnd) * rate * lo.XOOEBaseAmount / 3110400000000;
...
//harvest aimed locks
    ...
    function lock(Lock memory lock_) external notPause{
        //require(block.timestamp > lock_.timestampLockStart && block.timestamp - lock_.timestampLockStart < 120,"OPENOCEAN_STAKING_POOL_V1:WRONG_START_TIME");
        require(_lock[lock_.lockId].OOELockedAmount == 0, "OPENOCEAN_STAKING_POOL_V1:LOCK_ALREADY_EXIST");
        //require((lock_.timestampLockEnd - lock_.timestampLockStart) / 2592000 == lock_.lockMonths, "OPENOCEAN_STAKING_POOL_V1:WRONG_ENDTIME");
    ...
    function GasRefund(gasRefund memory input) external nonReentrant notPause{
        ...
        gasRefundNonce[msg.sender] = gasRefundNonce[msg.sender] + 1;
    ...
    function _verifyInputSignature(gasRefund memory input) internal virtual {
        bytes32 hash = keccak256(abi.encode(input.user, input.amount, input.nonce, input.chainID));
        require(hash == input.Hash,"WRONG_ENCODE");
        require(block.chainid == input.chainID, "OPENOCEAN_STAKING_POOL_V1:WRONG_CHAIN");
    ...
    function _punishment(Lock memory lock_) internal returns(uint256){
        uint256 punishmentAmount = 4 * (lock_.XOOEBaseAmount - lock_.OOELockedAmount) * (block.timestamp - lock_.timestampLockStart) / (lock_.timestampLockEnd - lock_.timestampLockStart) / 5;
        return punishmentAmount;
    }

    function _singleLock(Lock memory lock_) internal{
        //uint256 rate = (((1045 ** lock_.lockMonths) / 9) - 50 + 1000) ** (12/);
        //uint256 rate = lock_.lockMonths * 200 + (lock_.lockMonths - 1) * 265 + 100000;
        require(rates[lock_.lockMonths] != 0, "OPENOCEAN_STAKING_POOL_V1:ILLEGAL_MONTH_OR_MONTH_RATES_NOT_SET");
        require(lock_.XOOEBaseAmount == lock_.OOELockedAmount * (rates[lock_.lockMonths] + 10000000000) / 10000000000,"OPENOCEAN_STAKING_POOL_V1:NOT_CORRECT_AMOUNT");
