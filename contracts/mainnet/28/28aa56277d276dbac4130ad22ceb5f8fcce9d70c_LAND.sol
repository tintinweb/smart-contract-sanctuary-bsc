...
// assert(a == b * c + a % b); // There is no case in which this doesn't hold
    ...
    function transferOwnership(address newOwner) public onlyOwner {
      ...
      function transferOperatorShip(address new_operator) public onlyOwner {
...
// 玩家 赛事  投注类型  
 ...
 contract LAND is DataPlayer {
   ...
   public {
    ...
    function startUpMatch(uint256 ID,uint256 startTime ,uint256 endTime,uint256 AwinningOdds,uint256 BwinningOdds,uint256 CwinningOdds) public onlyOperator()  {
    ...
    function Draw_A_Prize(uint256 ID,uint256 result) public onlyOperator()  {
        require(matchState[ID] <= block.timestamp, "4");
        matchSetails[ID].result = result;
    }


    function extraTime(uint256 ID,uint256 time,uint256 AwinningOdds,uint256 BwinningOdds,uint256 CwinningOdds) public onlyOperator()  {
