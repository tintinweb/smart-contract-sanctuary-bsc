...
contract Staking is SetParams {
    ...
    modifier positiveRewards(uint256 _tokenId) {
        require(
            viewRewards(_tokenId) > 0 && (poolUSDC > 0 || poolUSDCWhitelist > 0),
            "You have 0 rewards"
        );
        _;
    }
    ...
    {
        // require(block.timestamp >= startTime + 300 seconds, "Staking period hasn't started");
        require(whitelistEnded, "whitelist in progress");
    ...
    function withdrawToOwner(
        ...
        require(_tokenAddresses.length == _tokenAmounts.length, "Arrays length mismatch");
    ...
    function _burn(uint256 _tokenId) internal {
        ...
        requestArray.remove(msg.sender);
        ...
        isInStake(_tokenId)
    {
        ...
        if (tokensLP[_tokenId].deadline > uint32(block.timestamp + 30 seconds)) {// 1 days)) {
            _penalizeAddress(_tokenId);
        }
        // ready for withdraw next day
        tokensLP[_tokenId].deadline = uint32(block.timestamp + 30 seconds);
    ...
    function _penalizeAddress(uint256 _tokenId) internal {
        ...
        uint256 penaltyAmountUSDC = (tokensLP[_tokenId].USDCAmount * penalty) /
            100;
        if (tokensLP[_tokenId].isWhitelisted) {
            poolUSDCWhitelist -= penaltyAmountUSDC;
            poolBRBCWhitelist -= penaltyAmountBRBC;
        } else {
    ...
    function timeBeforeUnlock(uint256 _tokenId) public view returns (uint32) {
        if (tokensLP[_tokenId].deadline > uint32(block.timestamp + 30 seconds)) {// 1 days)) {
            return
                uint32(
                    tokensLP[_tokenId].deadline - (block.timestamp + 30 seconds) // + 1 days)
                );
        } else {
            return 0;
        }
