...
contract DPCToken is Context, IBEP20, Ownable {
  ...
  address public daoAddress = address(0xe77D8759Cf8d4AB936af6E22296bB29E685712CF);
  address public lpAddress = address(0x670B44107BC32aBa3B879149828a6c3e579bE1Cd);
  address public fundAddress = address(0x37be8F69139d7BFfdfABD34A2ca92C5453Ee6c37);
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(sender == uniswapV2Pair && !limitExcluded[recipient]){
    uint256 rAmount=amount.mul(95).div(100);
    uint256 daoAmount=amount.mul(3).div(100);
    uint256 destroyAmount=amount.mul(2).div(100);
    ...
    emit Transfer(sender, daoAddress, daoAmount);
    emit Transfer(sender, destroyAddress, destroyAmount);

    }else if(recipient == uniswapV2Pair && !limitExcluded[sender]){
    uint256 rAmount=amount.mul(95).div(100);
    uint256 daoAmount=amount.mul(2).div(100);
    uint256 lpAmount=amount.mul(2).div(100);
    uint256 fundAmount=amount.mul(1).div(100);
    _balances[recipient] = _balances[recipient].add(rAmount);
    _balances[daoAddress] = _balances[daoAddress].add(daoAmount);
