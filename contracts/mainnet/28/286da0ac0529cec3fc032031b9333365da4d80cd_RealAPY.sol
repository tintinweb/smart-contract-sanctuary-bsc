...
contract RealAPY is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "RealAPY";
    string public constant _symbol = "RA";
    ...
    uint256 public constant inubRiskFreeValueFee = 20;
    uint256 public constant firePitFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(inubRiskFreeValueFee).add(
            firePitFee
        );
    ...
    address public constant firePit =
        0x21f9c4260EB8E69812144E29CED5F66929449318;

    address public autoLiquidityReceiver;
    address public treasuryReceiver;
    address public inubRiskFreeValueReceiver;
    ...
    constructor()
        ERC20Detailed("RealAPY", "RA", uint8(DECIMALS))
        Ownable()
    {
        ...
        autoLiquidityReceiver = 0x52fB9801c93FfB8179aA039258A1c849Cf5fa90C;
        treasuryReceiver = 0x21f9c4260EB8E69812144E29CED5F66929449318;
        inubRiskFreeValueReceiver = 0xa20C5CfdeDa7B6d4bD628A1537A5F390f1d7B8eE;
    ...
    function takeFee(
        ...
        uint256 _inubRiskFreeValueFee = inubRiskFreeValueFee;

        if (recipient == pair) {
            _totalFee = 190;
            _liquidityFee = 50;
            _treasuryFee = 50;
            _inubRiskFreeValueFee = 70;
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(_inubRiskFreeValueFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndRFV.mul(treasuryFee).div(
                treasuryFee.add(inubRiskFreeValueFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(inubRiskFreeValueReceiver).call{
            value: amountETHToTreasuryAndRFV
                .mul(inubRiskFreeValueFee)
                .div(treasuryFee.add(inubRiskFreeValueFee)),
            gas: 30000
        }("");
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no INUB token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _inubRiskFreeValueReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        inubRiskFreeValueReceiver = _inubRiskFreeValueReceiver;
    }
