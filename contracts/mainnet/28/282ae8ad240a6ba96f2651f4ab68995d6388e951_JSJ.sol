...
contract JSJ is ERC20 {
	...
	address payable _fundAddress = payable(address(0x449b8D00f6927F101062967c4291E9789F3F4F61));
    ...
    function _transfer(
        ...
        if(startTime == 0 && balanceOf(uniswapV2Pair) == 0 && to == uniswapV2Pair){
            startTime = block.timestamp;//加池子时间
        ...
        if (takeFee) {
            ...
            if(theFee>0 && to == uniswapV2Pair) {
                ...
                super._transfer(from, _destroyAddress, amount.div(100).mul(theFee+1));
            }else{
    ...
    function swapAndLiquify(uint256 tAmount) internal lockTheSwap {
        ...
        uint256 usdtForLiquify=amountReceived.div(3);
        uint256 usdtForFund=amountReceived.sub(usdtForLiquify);
        if(amountReceived > 0){
