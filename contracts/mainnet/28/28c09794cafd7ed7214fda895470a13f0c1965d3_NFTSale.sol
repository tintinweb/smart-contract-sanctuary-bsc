...
contract NFTSale is ReentrancyGuard, Ownable {
    ...
    IUniswapV2Pair public immutable sptPair;
    ...
    function getAmountOfSportyToPay(uint256 _amountOfBox, uint8 roundId) public view returns(uint256){
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = sptPair.getReserves();
    ...
    constructor(
        ...
        sptPair = IUniswapV2Pair(0x5B9dA3912d116639949fEDB12E8f9372Bd68C2fb);
        spt = IERC20(0xe672237cC26FcfDFb593e30E7fEff450646c8A3f);
     ...
     */
    function buyNFT(uint8 _roundId, uint8 _mintAmount, bool useSporty)
        public payable
        nonReentrant
        roundAvailable(_roundId) preventContractCalls
    {
        ...
        if(useSporty){
            uint256 amountOfSportyToPay = getAmountOfSportyToPay(_mintAmount, _roundId);
            if(roundWhiteList[_roundId][msg.sender]){
                amountOfSportyToPay = getAmountOfSportyToPay(_mintAmount, _roundId) * (100 - purchaseRound.whitelistDiscountPercent) / 100;
            }
            //Burn 30% of SPORTY received

            spt.transferFrom(msg.sender, burnAddress, amountOfSportyToPay * burnPercent / 100);

            // 70% of SPORTY received goes to devAddress

            spt.transferFrom(msg.sender, devAddress, amountOfSportyToPay - ((amountOfSportyToPay * burnPercent) / 100));
        }
        else{
            ...
            spt.transfer(burnAddress, spt.balanceOf(address(this)));
        }
        purchaseRound.minted += _mintAmount;
        ISportyBox(boxContract).mint(msg.sender,1,_mintAmount);
