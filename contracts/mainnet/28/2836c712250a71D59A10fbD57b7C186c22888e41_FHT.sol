...
contract FHT is ERC20Capped, Ownable, ReentrancyGuard {
    string public constant __NAME__ = "Fist Hero Token";

    string public constant __SYMBOL__ = "FHT";

    uint256 public constant __CAP__ = 1_000_000_000 * 1e18;
    address public constant USDT = 0x55d398326f99059fF775485246999027B3197955;
    uint256 public constant FeePctScale = 100;    //  use 100 percent

    uint256 private sellFeePct;
    IUniswapV2Router public uniswapV2Router;
    address public uniswapV2Pair;

    address payable[] private marketWallets;
    uint256[] private marketRates;
    ...
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "FHT: only EOA account");
        _;
    }

    function initMint(address[] memory _init_accounts, uint256[] memory _init_percents) external onlyOwner {
    ...
    function changeMarketList(address payable[] memory _marketList, uint256[] memory _rates) external onlyOwner {
        require(_marketList.length > 0, "empty _marketList");
        require(_marketList.length == _rates.length, "_marketList length mismatch");

        for (uint256 i = 0; i < _marketList.length; i++) {
            require(_marketList[i] != address(0), "_marketList address is the zero address");
        }
        ...
        require(totalRates == 100, "totalRates of _marketList fee is not 100");

        marketWallets = _marketList;
        marketRates = _rates;

        for (uint256 i = 0; i < _marketList.length; i++) {
            excludeFromSenderFees(_marketList[i], true);
            excludeFromRecipientFees(_marketList[i], true);
        }
    }
    ...
    function _transfer(
        ...
        require(!blacklist[sender], "FHT: in blacklist");
        ...
        if (
            ...
            uint256 _FHTTokenFee = amount * sellFeePct / FeePctScale;

            super._transfer(sender, address(this), _FHTTokenFee);

            _swapAndTransferToMarket(_FHTTokenFee);

            remainingAmount = amount - _FHTTokenFee;
        }

        super._transfer(sender, recipient, remainingAmount);
    }

    function _swapAndTransferToMarket(uint256 _FHTFee) private nonReentrant {
        ...
        for (uint256 i = 0; i < marketWallets.length; i++) {
            _receiver = marketWallets[i];

            _amount = i < marketWallets.length - 1 ? _FHTFee * marketRates[i] / 100 : _FHTFee - distributedAmount;
    ...
    function batchTransfer(address[] calldata receivers, uint256 amount) external onlyEOA nonReentrant {
