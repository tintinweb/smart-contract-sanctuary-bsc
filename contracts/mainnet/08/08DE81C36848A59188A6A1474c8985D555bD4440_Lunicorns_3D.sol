...
contract Lunicorns_3D is ERC721Metadata, ReentrancyGuard, VRFConsumerBaseV2{
    ...
    uint32 chainlinkCallbackGasLimit = 600000;
    ...
    address public v1Luni2D;
    address public v2Luni3D;

    // Start time for main drop
    uint256 public startTime = 0;

    uint256 public devTax = 90;
    uint256 public reflectionTax = 10;
    uint256 public royaltiesTax = 200;
    ...
    uint256 public priceStep1 = 0.2 ether;
    uint256 public priceStep2 = 0.3 ether;
    uint256 public priceStep3 = 0.4 ether;
    ...
    constructor(
        ...
        bytes32 _keyHash
    ) ERC721Metadata(_name, _symbol, _baseURI) VRFConsumerBaseV2(_chainlinkCoordinatorAddress) {
    ...
    function requestRandomValues(uint32 numValues, address _to) internal  {
        ...
        uint256 requestId = COORDINATOR.requestRandomWords(chainlinkKeyHash, chainlinkSubscriptionId, chainlinkRequestConfirmations, chainlinkCallbackGasLimit,numValues);
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(!isContract(msg.sender), "You can't purchase from a contract");
    ...
    function setTaxes(uint256 _devTax, uint256 _reflectionTax) external onlyOwner{
        require(_devTax + _reflectionTax == 100, "Total tax must be 100%");
        devTax = _devTax;
    ...
    function bulkMint(uint256[] memory _ids, address[] memory recipients) external onlyOwner{
        ...
        for(uint256 i; i < size; i++){
            ...
            _totalSupply += 1;
            address originalMinter = ILunicorns(v2Luni3D).minters(tokenId);
            if(originalMinter == address(0) && tokenId <= 5000){
                minters[tokenId] = ILunicorns(v1Luni2D).ownerOf(tokenId);
            } else {
                minters[tokenId] = originalMinter;
            }
            lastDividendAt[tokenId] = totalDividend;
    ...
    */
    ...
    function setV2Luni3D(address _v2Luni3D) external onlyOwner {
        v2Luni3D = _v2Luni3D;
    }

    function setV1Luni2D(address _v1Luni2D) external onlyOwner {
        v1Luni2D = _v1Luni2D;
    }
