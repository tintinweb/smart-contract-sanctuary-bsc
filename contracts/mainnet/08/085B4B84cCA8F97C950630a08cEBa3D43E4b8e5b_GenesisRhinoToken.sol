...
import "./GenesisRhinoTokenStorage.sol";
import {List} from "./GenesisRhinoTokenStructs.sol";



// 创世会卡牌
contract GenesisRhinoToken is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, GenesisRhinoTokenStorage {
    ...
    constructor(string memory baseURI_, address payee_) ERC721("GenesisRhinoToken","GRT") {
        baseURI = baseURI_;
        payee = payee_;
    }
    ...
    function withdraw() external nonReentrant returns(uint _mintTokenId){
        address _recipient = msg.sender;
        List memory list = buyList[_recipient];
        require(list.amount > 0, "This address has no subscription quota");
        require(list.reward + 1 <= list.amount, "Account subscription share is insufficient");
        uint256 newTokenId;
        _mint(_recipient, (newTokenId = _tokenId++));
        firstHolder[newTokenId] = _recipient;

        List storage buy = buyList[_recipient];
        buy.reward += 1;
        return newTokenId;
    }
    ...
    function allWithdraw() external nonReentrant returns(uint256[] memory){
        ...
        List memory list = buyList[_recipient];
        require(list.amount > 0, "This address has no subscription quota");
        uint256 available = list.amount - list.reward;
        require(available > 0, "Account subscription share is insufficient");

        uint256[] memory id = new uint256[](available);
        ...
        List storage buy = buyList[_recipient];
        buy.reward += available;
        return id;
    }

    function withdrawList(uint256 mintAmount) external nonReentrant returns(uint256[] memory){
        address _recipient = msg.sender;
        List memory list = buyList[_recipient];
        require(list.amount > 0, "This address has no subscription quota");
        uint256 available = list.amount - list.reward;
        require(mintAmount <= available, "Account subscription share is insufficient");

        uint256[] memory id = new uint256[](mintAmount);
    ...
    function subscribeList(address[] calldata account, uint256[] calldata amount) external onlyOwner {
        require(account.length > 0, "Account array length is less than 0");
        for (uint j = 0; j < amount.length; j++) {
            subscribeAmount += amount[j];
        }
        ...
        for (uint i = 0; i < account.length; i++) {
            ...
            List memory lt = buyList[account[i]];
            require(lt.account != account[i], "This address has already been subscribed");

            List memory vars;
            vars.account = account[i];
            vars.amount = amount[i];
            buyList[account[i]] = vars;
        }
    }

    function oneSubscribeList(address account, uint256 amount) external onlyOwner {
        require(account != address(0), "ERC721: record to zero address");

        List memory lt = buyList[account];
        require(lt.account != account, "This address has already been subscribed");
