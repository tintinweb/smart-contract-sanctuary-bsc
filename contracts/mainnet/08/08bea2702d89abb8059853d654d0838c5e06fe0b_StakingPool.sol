...
// File contracts/interfaces/IVToken.sol
pragma solidity ^0.8.0;
...
contract StakingPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 tokenAtLastUserAction; // keep track of token deposited at the last user action.
    ...
    address public VToken; // set to 0x0 at PancakeSwap
    ...
    event NewVTokenContract(address VToken);
    ...
    event UpdateFundingToken(address fundingToken, uint256 amount, bool isStaking);

    /**
     * @notice Constructor
     * @param _token: Staking token contract (CUB)
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _fundingPID: pool ID in MasterChefV2 that is paying out rewards
     */
    constructor(
        IERC20 _token,
        ...
        treasury = _treasury;
        operator = _operator;
        fundingPID = _fundingPID;
    }

    /**
     * @notice Deposits a dummy token to `MASTER_CHEF` MCV2.
     * It will transfer _amount of the `dummyToken` in the tx sender address.
     * @param fundingToken The address of the token to be deposited into MCV2.
     * @param _amount Amount to stake
     */
    function init(IERC20 fundingToken, uint256 _amount) external onlyOwner {
        fundingToken.approve(address(masterchefV2), _amount);
        masterchefV2.deposit(fundingPID, _amount);
        emit UpdateFundingToken(address(fundingToken), _amount, true);
    }

    function unstakeFundingToken(IERC20 fundingToken, uint256 _amount) public onlyOwner {
        masterchefV2.withdraw(fundingPID, _amount);
        fundingToken.transfer(msg.sender, _amount);
        emit UpdateFundingToken(address(fundingToken), _amount, false);
    }

    function updateFundingPID(uint256 _new) public onlyOwner {
        fundingPID = _new;
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the token owner address or the operator address.
     */
    modifier onlyOperatorOrTokenOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or token owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onTokenPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.tokenAtLastUserAction;
    ...
    /**
     * @notice Unlock user funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrTokenOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
     ...
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in staking token (CUB))
    ...
    function depositOperation(
        ...
        if (VToken != address(0)) {
            IVToken(VToken).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.tokenAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VToken != address(0)) {
            IVToken(VToken).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending reward tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingTokens = masterchefV2.pendingCub(fundingPID, address(this));
        if (pendingTokens > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(fundingPID, 0);
    ...
    /**
     * @notice Set Boost Contract address (FarmBooster)
    ...
    /**
     * @notice Set VToken Contract address
     * @dev Callable by the contract admin.
     */
    function setVTokenContract(address _VToken) external onlyAdmin {
        VToken = _VToken;
        emit NewVTokenContract(VToken);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Staking Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingTokenRewards();
    ...
    function calculateTotalPendingTokenRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingCub(fundingPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingTokenRewards()) * (1e18)) / totalShares);
    }
