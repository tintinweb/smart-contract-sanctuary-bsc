...
interface INFTGamePatchOne {
    ...
    function replace(uint256  _orderId, uint256 _planetId,uint256 _oldStationId,uint256 _newStationId ,string memory _remark,uint256 _nonce,bytes memory _sign)  external returns(bool);
    function getReplaceOrder(uint256 _orderId) view external returns (address owner,uint256 planetId, uint256 oldStationId,uint256 newStationId,uint256 time,string memory remark);

    event Append(uint256 indexed orderId,address owner ,uint256 planetId,uint256[]   stationIds,string remark);
    event Replace(uint256 indexed orderId,address owner ,uint256 planetId,uint256  oldStationId, uint256 newStationId,string remark);
}
...
contract Domains{
    ...
    struct ReplaceOrder{
        address owner;
        uint256 planetId;
        uint256 oldStationId;
        uint256 newStationId;
        uint256 time;
        string remark;
        bool succeed;
    }
...
contract  NFTGamePatchOne is INFTGamePatchOne,Domains,Ownable,Sign{
    ...
    mapping (uint256 => ReplaceOrder) private replaceOrders;
    ...
    function append(uint256  _orderId, uint256 _planetId,uint256 []  memory _stationIds ,string memory _remark,uint256 _nonce,bytes memory _sign) external override returns(bool){
      ...
      function replace(uint256  _orderId, uint256 _planetId,uint256 _oldStationId ,uint256 _newStationId ,string memory _remark,uint256 _nonce,bytes memory _sign) external override returns(bool){
        uint256 [] memory _list = new uint256[](5);
        _list[0]=_orderId;
        _list[1]=_planetId;
        _list[2]=_oldStationId;
        _list[3]=_newStationId;
        _list[4] = _nonce;
        _check(_list,_msgSender(),_nonce,_sign);
        require(!replaceOrders[_orderId].succeed,"NFTGame: Order already exists");
        require(IERC721TokenInterface(_planetNFTAddress).ownerOf(_planetId)==_msgSender(),"NFTGame: invalid planet owner");
        require(IERC721TokenInterface(_stationNFTAddress).ownerOf(_newStationId)==_msgSender(),"NFTGame: invalid station owner");
        IERC721TokenFactoryInterface(_stationNFTAddress).burn(_newStationId);
        replaceOrders[_orderId]=ReplaceOrder({
            owner:_msgSender(),
            planetId:_planetId,
            oldStationId:_oldStationId,
            newStationId:_newStationId,
            time:block.timestamp,
            remark:_remark,
            succeed:true
            });
        emit Replace(_orderId,_msgSender(),_planetId,_oldStationId,_newStationId,_remark); 
        return true;
    }
    ...
    function getReplaceOrder(uint256 _orderId)view  override external returns (address owner,uint256 planetId, uint256 oldStationId,uint256 newStationId,uint256 time,string memory remark){
        ReplaceOrder memory order = replaceOrders[_orderId];
        if(order.succeed){
            return (order.owner,order.planetId,order.oldStationId,order.newStationId,order.time,order.remark);
        }
    }
