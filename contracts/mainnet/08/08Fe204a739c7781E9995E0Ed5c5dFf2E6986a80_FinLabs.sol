...
contract FinLabs is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "FLabs";
    string private constant _name = "FinLabs";

    uint256 public FinLabs__Max_Buy = 10000000 * 10**9;
    uint256 public FinLabs__Max_Sell = 10000000 * 10**9;

    address private FinLabs__Marketing_Wallet = 0x9d2d651fc81f566f6366675A34F3a1199335b96C;
    address private FinLabs__Team_Wallet = 0x9d2d651fc81f566f6366675A34F3a1199335b96C;

    uint256 public FinLabs__Max_WalletLimit = 20000000  * 10**9;
    uint256 public FinLabs__Threshold_Amount = 3000000 * 10**9;
    ...
    Taxes public taxes = Taxes(0, 25, 0, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[FinLabs__Marketing_Wallet] = true;
        _isExcludedFromFee[FinLabs__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[FinLabs__Marketing_Wallet] = true;
        allowedTransfer[FinLabs__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= FinLabs__Max_Buy, "You are exceeding FinLabs__Max_Buy");
            require(balanceOf(to) + amount <= FinLabs__Max_WalletLimit, "You are exceeding FinLabs__Max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= FinLabs__Max_Sell, "You are exceeding FinLabs__Max_Sell");
            if(to != pair){
                require(balanceOf(to) + amount <= FinLabs__Max_WalletLimit, "You are exceeding FinLabs__Max_WalletLimit");
            }
            if(coolDownEnabled){
        ...
        bool canSwap = balanceOf(address(this)) >= FinLabs__Threshold_Amount;
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(FinLabs__Threshold_Amount, sellTaxes);
            else  swapAndLiquify(FinLabs__Threshold_Amount, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(FinLabs__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(FinLabs__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function FinLabs__Update_Marketing_Wallet(address newWallet) external onlyOwner{
        FinLabs__Marketing_Wallet = newWallet;
    }
    
    function FinLabs__Update_Team_Wallet(address newWallet) external onlyOwner{
        FinLabs__Team_Wallet = newWallet;
    }
    ...
    function updateFinThreshold_Amount(uint256 amount) external onlyOwner{
        FinLabs__Threshold_Amount = amount * 10**_decimals;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        FinLabs__Max_Buy = maxBuy * 10**decimals();
        FinLabs__Max_Sell = maxSell * 10**decimals();
    }
    
    function FinLabs__Max_Walletlimit(uint256 amount) external onlyOwner{
        FinLabs__Max_WalletLimit = amount * 10**decimals();
    }
