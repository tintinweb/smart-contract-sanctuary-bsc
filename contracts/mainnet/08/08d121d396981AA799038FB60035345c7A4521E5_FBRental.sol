...
contract FBRental {
    ...
    mapping(address => mapping(uint256 => address))                         public renter;  // renter => Number renting => host
    mapping(address => mapping(uint    => mapping(uint    => uint256)))     public duration;// host   => number NFT => time => amount money 
    ...
    event RentingNft(address renter_, uint256 rentId_, address hoster_);
    event ClaimNft(address renter_, uint256 rentId_, address hoster_);
    ...
    function rentNFT(address host_, uint duration_, uint256[] memory nfts_, uint256 amount_) external payable returns(uint256) {
        ...
        emit RentingNft(msg.sender, renting.length-1, host_);
    ...
    function claimNFT(uint256 rentId_) external {
        ...
        emit ClaimNft(renting[rentId_].renter, rentId_, renting[rentId_].host);
    }
    ...
    function lengthRenting() public view returns (uint256) {
        return renting.length;
    }
