...
10% rewards Fees in BTCB - directly auto claimed into your wallet just by holding McToshi Tokens !
3% to marketing and 2% to liquidity pool.
Diamond hands will be rewarded !!

Official Telegram group : https://t.me/mctoshiofficial
...
import "./McToshiDividendTracker.sol";
...
contract McToshi is ERC20, Ownable {
    ...
    McToshiDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("McToshi", "$McT") {

    	dividendTracker = new McToshiDividendTracker();

        _marketingWalletAddress = 0x686BdC5605445BE0736C9fA45E6BAf17E0477170;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "McToshi: The dividend tracker already has that address");

        McToshiDividendTracker newDividendTracker = McToshiDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "McToshi: The new dividend tracker must be owned by the McToshi token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "McToshi: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "McToshi: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "McToshi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "McToshi: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "McToshi: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "McToshi: Cannot update gasForProcessing to same value");
