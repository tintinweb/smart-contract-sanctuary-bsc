...
contract Retopup_Staking is Ownable {
    ...
    function retopupWithRTC(uint256 amount, uint256 tokenKey) public {
        require(
            tokenKey < 2,
        ...
        if (tokenKey == 0) {
            ...
            investors[msg.sender].investedAmount = return_token_in_RTC;
            totalRTC_PlusInvested += return_token_in_RTC;
            investors[msg.sender].investedAt = block.timestamp;
            investors[msg.sender]._retopup.push(
                retopup(return_token_in_RTC, block.timestamp, 0, 0, 0)
            );
        }
    ...
    function generateReward(address user) public view returns (uint256 amount) {
        ...
        for (uint256 i = 0; i < investor._retopup.length; i++) {
            ...
            uint256 matureTime = retop.timeAt + secondIN1000Days;
            uint256 startingTime = investor.investedAt > retop.timeAt
                ? investor.investedAt
                : retop.timeAt;
