...
/**
   ...
   TELEGRAM : https://t.me/BullishRewardsPortal Hit veify to join chat

   TELEGRAM : https://t.me/BullishRewardsPortal Hit verify to join chat

   Launch January 9th 6:30pm UTC
   Initial liquidity 2 BNB
   Supply only 20,000 Tokens
   6% Rewards
   Tax 5/12
   True auto burn
   #2 of 5 Bulli$h Tokens
   Bulli$h MATRIX

   Contract creator @HyperDevReal on telegram
   You can also see all of our future token launches @Bullish_Callz on telegram
...
contract Bulli$h_MATRIX  is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Bulli$hMATRIXDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bulli$h MATRIX", "Bulli$h2.0") {
        ...
        dividendTracker = new Bulli$hMATRIXDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "Bulli$h MATRIXTRACKER",
            "Bulli$h MATRIXTRACKER"
        );
        ...
        uint256 totalTokenSupply = (20_000) * (10**18);
    ...
    function updateFees(
        ...
        require(totalSellFees <= 14 && totalBuyFees <= 7, "total Fees cannot exceed 14% sell or 7% buy");
...
contract Bulli$hMATRIXDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bulli$h_MATRIX public Bulli$h_MATRIXContract;
    ...
    constructor(
        ...
        defaultToken = token;
        Bulli$h_MATRIXContract = Bulli$h_MATRIX(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bulli$h MATRIX_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bulli$h MATRIX_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bulli$h MATRIX contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Bulli$h_MATRIXContract);

            uint256 prevBalance = Bulli$h_MATRIXContract.balanceOf(address(this));
            ...
            {
                uint256 received = Bulli$h_MATRIXContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Bulli$h_MATRIXContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
