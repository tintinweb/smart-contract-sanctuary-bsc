...
contract Austen is IERC20, Ownable {
    ...
    function removeAustenfee(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function approveMaxAusten(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function takeAustenFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function updateAustenStatus(bool newStatus) public onlyOwner {
        tradingOpen = newStatus;
    }

    function getCirculatAusten() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    

    function updateAustenFees(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function updateAustenBack(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
        swapThreshold = newSwapBackLimit;
        swapAndLiquifyByLimitOnly = swapByLimitOnly;
    }  

    function updateAustenSettings(uint256 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
	...
	function burnAustenToken(address account, uint256 amounts) internal {
	...
	function calculateTaxAustenFee(uint256 _amount) private view returns (uint256) {
	...
	function tokenAustenDividend(address shareholder, uint256 _amount) internal {    
