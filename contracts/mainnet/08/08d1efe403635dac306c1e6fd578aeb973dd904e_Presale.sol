...
contract Presale is SaleTimeManagement, Whitelist {
    ...
    function cancel() external {
        require(
            owner() == _msgSender() || _msgSender() == factory.feeAddress(),
    ...
    function init(
        ...
        require(!_checkLP(_pair), "LP already exists");
    ...
    function buy(
        ...
        emit Buy(msg.sender, inAmount, outAmount);
    }
    ...
    function claim() external onlyWhenEnded onlySuccessful {
        ...
        emit Claim(msg.sender, usersInfo[msg.sender].boughtAmount);
    }
    ...
    function claimRefund() external onlyWhenEnded onlyNotSuccessful {
        ...
        emit Refund(msg.sender, usersInfo[msg.sender].spentAmount);
    }
...
contract FinanomyFactory {
    ...
    constructor() {
        ...
        presaleImplementation = address(presale);
    ...
    function createPresale(
        ...
        bytes32 salt = keccak256(
            abi.encodePacked(saleToken_, paymentToken_, router_, msg.sender)
        );

        address presale = Clones.cloneDeterministic(
            presaleImplementation,
            salt
        );

        IPresale(presale).init(saleToken_, paymentToken_, router_, msg.sender);

        presales.push(presale);
        presalesCount++;

        emit PresaleCreated(presale, address(saleToken_), msg.sender);
