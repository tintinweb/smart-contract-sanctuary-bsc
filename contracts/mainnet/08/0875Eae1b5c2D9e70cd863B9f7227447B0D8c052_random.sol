...
contract random is Governance {
    ...
    address public usdb;
    address public gainer;

     constructor (address usdbToken,address _gainer) public {
         usdb = usdbToken;
         gainer = _gainer;
    }

    function swapUSDB(uint amount) external {
        IERC20(usdt).safeTransferFrom(msg.sender,address(this),amount);

        IERC20(usdb).safeTransfer(msg.sender,amount);
    }

    function swapUSDT(uint amount) external {
    ...
    function govwithdraw(address ercaddr, uint amount) external onlyGovernance {
        IERC20(ercaddr).safeTransfer(gainer,amount);
    }

	function setgainer(address _gainer) external onlyGovernance {
         gainer = _gainer;
    }
}