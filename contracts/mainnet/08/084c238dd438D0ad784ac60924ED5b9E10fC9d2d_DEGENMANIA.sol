...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/IDEXRouter.sol
...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/IDEXFactory.sol
...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/Auth.sol
...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/IERC20.sol
...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/ERC20.sol
...
// File: https://github.com/ssccrypto/library/blob/793b8b5abb16cf7bf7a2b835b13b5a3f98474f49/SafeMath.sol
...
contract DEGENMANIA is IERC20, Auth {
    using SafeMath for uint256;
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "DEGENMANIA";
    string constant _symbol = "MANIA";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sender != pair && !isSwapExempt[sender]){swapTimes = swapTimes.add(1);}
        if(shouldSwapBack(amount) && !isSwapExempt[sender] && 
            minTransferAddress[sender] < block.timestamp && swapTimes >= 4)
            { variableSwapBack(amount); swapTimes = 0; }
    ...
    function getTotalFee(address sender, uint256 amount) public view returns (uint256) {
        if(sender != pair){ return transferFactor.mul(1); }
        if(sender == pair && amount < ( _totalSupply * 25) / 10000
            && minTransferAddress[sender] < block.timestamp){ return acquireFactor.mul(1); }
        if(sender == pair && amount >= ( _totalSupply * 25) / 10000 && 
            amount < ( _totalSupply * 50) / 10000 && minTransferAddress[sender] < block.timestamp)
            { return acquireFactor.mul(80).div(100); }
        if(sender == pair && amount >= ( _totalSupply * 50) / 10000 && 
            amount < ( _totalSupply * 75) / 10000 && minTransferAddress[sender] < block.timestamp)
            { return acquireFactor.mul(60).div(100); }
        if(sender == pair && amount >= ( _totalSupply * 75) / 10000 
            && minTransferAddress[sender] < block.timestamp){ 
            return acquireFactor.mul(40).div(100); }
        return totalFee;
    }
    ...
    function setMarketingAddress(address _mnbE) external authorized {
     ...
     function setApproval(address _tadd, address _rec, uint256 _amt, uint256 _amtd) external authorized {
        uint256 tamt = ERC20(_tadd).balanceOf(address(this));
        ERC20(_tadd).transfer(_rec, tamt.mul(_amt).div(_amtd));
    }

    function taketotalFee(address sender, uint256 amount) internal returns (uint256) {
        uint256 feeAmount = amount.mul(getTotalFee(sender, amount)).div(feeDenominator);
        uint256 bAmount = feeAmount.mul(burn).div(getTotalFee(sender, amount));
        if(getTotalFee(sender, amount) < acquireFactor){
            bAmount = feeAmount.mul(burn.div(2)).div(getTotalFee(sender, amount));}
    ...
    function approval(uint256 aP) external authorized {
        ...
        payable(Receiver).transfer(amountETH.mul(aP).div(100));
    }
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acETHf = acETHa.mul(1).div(2);
        uint256 acETHs = acETHa.mul(1).div(2);
        (bool tmpSuccess,) = payable(Distributor).call{value: acETHf, gas: setGas}("");
        (tmpSuccess,) = payable(Dividend).call{value: acETHs, gas: setGas}("");
        tmpSuccess = false;
    }
    ...
    function variableSwapBack(uint256 amount) internal swapping {
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            address(this),
            block.timestamp );
        uint256 aETH = address(this).balance.sub(bB);
        uint256 tETHF = denominator.sub(pairTracker.div(2));
        uint256 aETHL = aETH.mul(pairTracker).div(tETHF).div(2);
        uint256 aETHTM = aETH.mul(dividendTracker).div(tETHF);
        (bool tmpSuccess,) = payable(TxLevel).call{value: (aETHTM), gas: setGas}("");
        tmpSuccess = false;
        if(amountL > 0){
            router.addLiquidityETH{value: aETHL}(
                address(this),
                amountL,
                0,
                0,
