...
contract LotteryNFT is ERC721,AccessControl {
    ...
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE"); //byte for operator role
    ...
    function newLotteryItem(address player, uint8[4] memory _lotteryNumbers, uint256 _amount, uint256 _issueIndex)
        ...
        returns (uint256)
    {
        require(hasRole(OPERATOR_ROLE, msg.sender), "invalid role");
        _tokenIds.increment();
    ...
    function multiClaimReward(uint256[] memory _tokenId) external {
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        virtual override(AccessControl,ERC721)
        returns (bool)
    {
...
// 4 numbers
contract Lottery is LotteryOwnable,AccessControl,Initializable {
    ...
    function initialize(
        ...
        allocation = [50, 30, 20];
        lotteryNFT.initialize(address(this));
        initOwner(_owner);
    }
