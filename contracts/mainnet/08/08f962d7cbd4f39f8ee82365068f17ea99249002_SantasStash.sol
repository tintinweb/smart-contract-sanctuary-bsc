...
contract SantasStash is Ownable, IBEP20 {
    ...
    string constant _name = "Santas Stash";
    ...
    uint256 private santastashFee  = 3; 
    uint256 private devFee         = 1;
    uint256 public burnFee         = 1;
    uint256 public totalFee        = devFee + marketingFee + liquidityFee + santastashFee + burnFee;
    ...
    address private santastashFeeReceiver;
    ...
    constructor () {
        ...
        isFeeExempt[santastashFeeReceiver] = true;
        islaunched[msg.sender] = true;

        isTimelockExempt[msg.sender] = true;
        isTimelockExempt[pair] = true;
        isTimelockExempt[marketingFeeReceiver] = true;
        isTimelockExempt[santastashFeeReceiver] = true;

        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[pair] = true;
        isTxLimitExempt[marketingFeeReceiver] = true;
        isTxLimitExempt[santastashFeeReceiver] = true;

        autoLiquidityReceiver = msg.sender;
        marketingFeeReceiver = 0xA59956092D830d87ec03911bBc168Fa30E994587;
        santastashFeeReceiver = 0x5Cd9774b629082F418eCB76DB378ffc843480c2D;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != burnFeeReceiver && recipient != santastashFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBsantastash = amountBNB.mul(santastashFee).div(totalBNBFee);

        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing}("");
        (tmpSuccess,) = payable(santastashFeeReceiver).call{value: amountBNBsantastash}("");
    ...
    function whitelistmode(bool _status) external onlyOwner {
        launchMode = _status;
    ...
    function setMax (uint256 _maxG) public onlyOwner {
               maxG = _maxG * 1 gwei; 
    ...
    function setTax(uint256 _liquidityFee, uint256 _devFee, uint256 _marketingFee, uint256 _santastashFee, uint256 _burnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        devFee = _devFee;
        marketingFee = _marketingFee;
        santastashFee = _santastashFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_devFee).add(_marketingFee).add(_santastashFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/5, "Buy Fees cannot be more than 20%");
    }

    function setTaxReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _santastashFeeReceiver, address _burnFeeReceiver, address _devFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        santastashFeeReceiver = _santastashFeeReceiver;
