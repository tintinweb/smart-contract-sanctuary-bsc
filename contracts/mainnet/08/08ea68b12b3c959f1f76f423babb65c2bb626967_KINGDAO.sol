...
require ( to !=0xB0FC16615C24C30Af36EBD932CF817A05D2e03bD); // prevent from bot
    ...
    function antiBot() public view returns (uint256) {
        uint256 time = block.timestamp;
        uint256 quotient = time.div(3);
        uint256 remainder = time - quotient * 3;
        if (remainder > 0) {
            return quotient*10**8;
        } else {
