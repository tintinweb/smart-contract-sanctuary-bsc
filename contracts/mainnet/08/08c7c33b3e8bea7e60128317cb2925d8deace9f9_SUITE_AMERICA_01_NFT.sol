...
pragma solidity ^0.8.0;
contract SUITE_AMERICA_01_NFT is ERC721Enumerable, Ownable, TokenRecover, ReentrancyGuard {
	...
	uint256 public constant LIMIT_TOKENS = 500;

    constructor() ERC721("Suite Estadio America 01", "SUITE01"){
    }	
        ...
        onlyOwner
    {
	 ...
	 function mint_massive(uint256 numberOfTokens)  external onlyOwner {        	 
		require(
            numberOfTokens <= LIMIT_TOKENS,
            "Can only mint up to 500 tokens"
        );
        require(
            _publicTotalTokens.current() < TOTAL_TOKENS,
            "Purchase would exceed TOTAL TOKENS"
        );        
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicTotalTokens.current()+1;

            if (_publicTotalTokens.current() < TOTAL_TOKENS) {
                _publicTotalTokens.increment();
