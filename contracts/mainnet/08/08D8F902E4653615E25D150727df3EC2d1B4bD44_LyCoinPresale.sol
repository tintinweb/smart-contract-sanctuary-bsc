...
// File: contracts/LyCoinPresale.sol
...
contract LyCoinPresale is ReentrancyGuard {
	using SafeMath for uint256;
	...
	// Price of presale LyCoin, 5 BUSD
	uint256 BUSDPerToken = 5 * 10 ** 18;
	// Amount of BUSD received in presale
	uint256 busdReceived = 0;

    uint256 maxHardCap =5 * 10 ** 8 * 10 ** 18;

    uint256 public minimumDepositEthAmount = 100 * 10 ** 18;
    uint256 public maximumDepositEthAmount = 5 * 10 ** 8 * 10 ** 18;

    address public DAO_address = 0xa1b677444C4910f71d6FB260b0710A0030f0F042;
	...
	function getLyCoinTokensLeft() external view returns (uint256) {
		return LyCoin.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some LyCoin tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed LyCoin tokens");
		require(LyCoin.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough LyCoin tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		LyCoin.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldLyCoin() external onlyOwner {
		uint256 amount = LyCoin.balanceOf(address(this)) - totalTokensSold;
		LyCoin.safeTransfer(msg.sender, amount);
	}

	function withdrawAllLyCoin() external onlyOwner {
		LyCoin.safeTransfer(msg.sender, LyCoin.balanceOf(address(this)));
	}
