...
contract swallow is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    function clearowner(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function receivedaddress(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
   ...
   function replygasfee(uint256 gas) external pure returns(uint256) {
      require(gas >= 75);        
      return gas * 1 gwei;
  }
