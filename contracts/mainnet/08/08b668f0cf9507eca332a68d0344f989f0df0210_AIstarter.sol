...
/**
Telegram: https://t.me/aistarter_bsc
*/
...
contract AIstarter is IERC20, Context, Ownable {
    ...
    address private Marketing_Wallet = 0x83DC0a534A1092EA6e29298344486d254207eEF2;
    address private Team_Wallet = 0x1eB2CEC1E7AD59e11e715D0E884f91fF49778886;
    
    string private constant _symbol = "AIstarter";
    string private constant _name = "AIstarter";

    uint256 public max_BuyLimit = 20000 * 10**9;
    uint256 public max_SellLimit = 20000 * 10**9;

    uint256 public max_WalletLimit = 20000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 12, 3, 10);
    Taxes public taxes = Taxes(0, 12, 3, 10);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 5) {
            require(to != pair, "Sells not allowed for first 5 blocks");
        }
        
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= max_BuyLimit, "You are exceeding max_BuyLimit");
            require(balanceOf(to) + amount <= max_WalletLimit, "You are exceeding max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= max_SellLimit, "You are exceeding max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= max_WalletLimit, "You are exceeding max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        max_BuyLimit = maxBuy * 10**decimals();
        max_SellLimit = maxSell * 10**decimals();
    }
    
    function Max_Walletlimit(uint256 amount) external onlyOwner{
        max_WalletLimit = amount * 10**decimals();
    }
