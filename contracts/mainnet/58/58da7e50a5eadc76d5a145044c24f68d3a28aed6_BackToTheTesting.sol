...
/**
BackToTheTesting SmartContract 
*/
...
contract BackToTheTesting is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BackToTheTestingDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BackToTheTesting", "BTTT") {
        ...
        dividendTracker = new BackToTheTestingDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BackToTheTestingTRACKER", "BTTTTRACKER");
...
contract BackToTheTestingDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BackToTheTesting public BackToTheTestingContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BackToTheTestingContract = BackToTheTesting(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BackToTheTesting contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BackToTheTestingContract);

            uint256 prevBalance = BackToTheTestingContract.balanceOf(address(this));
            ...
            {
                uint256 received = BackToTheTestingContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BackToTheTestingContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
