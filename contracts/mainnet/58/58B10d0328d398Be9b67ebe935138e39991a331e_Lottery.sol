...
Max Transaction: 1000 (1%)




The use of this smart contract is for the L-ottery team only.

It should not be taken as constituting professional advice from the contract creator - AAptitudeToken (AAPT).

AAptitude is not a financial adviser. You should consider seeking independent legal, financial, taxation or other advice.

AAptitude is not liable for any loss caused, whether due to negligence or otherwise arising from the use of, or reliance on, the information provided directly or indirectly, by use of this contract.

Contract Developed by Kodi - AAptitude.com
*/
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress =
        payable(0xBabe9292D332a0F49EDDF9896f46C331A724186a); // Marketing Address
    address payable public devAddress =
        payable(0xBabe9292D332a0F49EDDF9896f46C331A724186a); // Dev Address
    ...
    uint256 public _taxFee = 2; // rewards to holder
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 12; // Marketing + dev + buyback
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public devAndMarketingDivisor = 8;

    uint256 public _maxTxAmount = 1 * 10**3 * 10**9; // Max Transaction: 1 Thousand
    uint256 private minimumTokensBeforeSwap = 5 * 10**2 * 10**9;
    uint256 private buyBackUpperLimit = 1 * 10**2 * 10**9;
    uint256 private minimumBeforeBuyBack = 1 * 10**2 * 10**9;

    uint256 public percentToBuyBack = 4; // percent of fund to be used to buy back each time
    ...
    function _transfer(
        ...
        if (
            ...
            _isOriginExcludedFromFee[from] ||
            _isDestinationExcludedFromFee[to]
        ) {
            takeFee = false;
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 devPlusMarketing = transferredBalance.div(_liquidityFee).mul(
            devAndMarketingDivisor
        );
    ...
    function isOriginExcludedFromFee(address account)
        public
        view
        returns (bool)
    {
    ...
    function isDestinationExcludedFromFee(address account)
        public
        view
        returns (bool)
    {
    ...
    function setMinimumBeforeBuyBack(uint256 _minimumBeforeBuyBack)
        external
        onlyOwner
    {
...
contract Lottery is ERC1363, TokenRecover {
    constructor() ERC1363("L-ottery", "LTRY") {}
}