...
//------------------------------------------------------------------------------
contract YMBToken is StandardToken,Ownable {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;

    string public name = "YMB";
    string public symbol = "YMB";
    ...
    address public tokenAddressA = 0xDF288f624c7E153fEaF2CC021919a5fCBaB09255;
    ...
    address public poolAccount = 0xFEc6EE75327E749BfF3B046c03b5786F13c301Be;
    address public nftAccount = 0x85D5f46365a57697cb436c7098efe682df1cD7d9;
    address public daoAccount = 0x463d96a50b7A7A3b53b95D6572A2545eF93E9aad;
    ...
    function transfer(address dst, uint256 wad) public returns (bool){
      ...
      if(state==0){
        ...
        if(sort==BUY){
          if((balances[dst]+wad)>10000000000000000000000){
            return false;
          }

          uint256 wadPool = wad.div(100).mul(1);
          uint256 wadBlackhole = wad.div(100).mul(1);
          uint256 wadNft = wad.div(100).mul(1);
          ...
          super.transfer(nftAccount,wadNft);
        ...
        if(sort==BUY){
          ...
          super.transfer(poolAccount,wadPool);
          if(totalSupply_<=10000000000000000000000000){
            super.transfer(daoAccount,wadBlackhole);
          }else{
            super.transfer(blackholeAccount,wadBlackhole);
            totalSupply_ = totalSupply_.sub(wadBlackhole);
          }
          ...
          if(wad.mul(10000).div(balances[msg.sender])>9999){
            return false;
          }
          super.transfer(dst, wad);
          return true;
    ...
    { 
      ...
      if(src == poolAccount || dst == poolAccount){
        return super.transferFrom(src,dst,wad);
      }
      ...
      if(state==0){
        ...
        if(sort==SALE){
          if(wad.mul(10000).div(balances[src])>9999){
            return false;
          }
          ...
          uint256 wadBlackhole = wad.div(100).mul(1);
          uint256 wadNft = wad.div(100).mul(1);
          uint256 wadDao = wad.div(100).mul(2);
          ...
          if(totalSupply_<=10000000000000000000000000){
            super.transferFrom(src,daoAccount,wadBlackhole);
          }else{
            super.transferFrom(src,blackholeAccount,wadBlackhole);
            totalSupply_ = totalSupply_.sub(wadBlackhole);
          }

          super.transferFrom(src,nftAccount,wadNft);
    ...
    function updatePoolAccount(address _poolAccount) public onlyControler onlySelf{
