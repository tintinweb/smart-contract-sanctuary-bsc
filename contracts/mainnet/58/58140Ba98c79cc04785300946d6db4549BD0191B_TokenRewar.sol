...
contract TokenRewar is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    address ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    ...
    string name_ = "TigerDad";
    string symbol_ = "TigerDad";
    uint256 totalSupply_ = 1000000000000000 * 10**18;
    uint256 minimumTokenBalanceForDividends_ = 50000000000 *10**18;
    ...
    uint256 public maxsell = 50000000000 * 10**18;
    uint256 public maxhold = 150000000000 * 10**18;
    uint256 public maxburn = 700000000000000 * 10**18;
    ...
    constructor() payable ERC20(name_, symbol_) {
        ...
        _marketingWalletAddress = 0x890259125D230E030d467342Fe228E391Fc5026b;
        _fundAddress = 0x1d2950afd98e9a240720b1Cf3A96e87c732Fd30d;
        _dvpAddress= 0x749c376D4Ef29294252319C34A243d04FB016ccE;
        require(
        ...
        fundFee =  1;
        dvpFee =  1;
        sbfees =  marketingFee.add(fundFee).add(dvpFee);
        totalFees = tokenRewardsFee.add(liquidityFee).add(sbfees).add(burnFee);
       
        if(referralFee1 != 0  && referralFee2 !=0 ){
            referralFee1 = 2;
            referralFee2 = 1;
            isOpenRef = true;
        ...
        swapTokensAtAmount = totalSupply_ .div(100000);
        // 0.002%
    ...
    function inBlack(address account) public onlyOwner {
    ...
    function _transfer(
        ...
        if(start == 0 && _isExcludedFromFees[from] && automatedMarketMakerPairs[to]) {
        ...
        if(start != 0 && !_isExcludedFromFees[to] && !automatedMarketMakerPairs[to]){
            if((launchedAt + deadBlocks) > block.number){
                require(_isBlacklisted[to] = true);
            }
        }    

        if(start != 0 && !_isExcludedFromFees[to] && automatedMarketMakerPairs[from]){
        ...
        if(start != 0 && !_isExcludedFromFees[to] && automatedMarketMakerPairs[from] && automatedMarketMakerPairs[to]){
            if(block.timestamp <= (start + duration)){
        ...
        if (takeFee) {
            ...
            if(isOpenRef) {
                ...
                if(ref1 != address(0)){
                    uint256 refFee1 = tmp.mul(referralFee1).div(100);
                    super._transfer(from, ref1, refFee1);
                    fees = fees.sub(refFee1);
                    address ref2 = referrerMap[ref1];
                    if(ref2 != address(0)){
                        uint256 refFee2 = tmp.mul(referralFee2).div(100);
    ...
    function swapAndSendToFee(uint256 tokens) private {
       ...
       payable(address(_marketingWalletAddress)).transfer(WalletAddressBalance.mul(marketingFee).div(sbfees));
       payable(address(_fundAddress)).transfer(WalletAddressBalance.mul(fundFee).div(sbfees));
       payable(address(_dvpAddress)).transfer(WalletAddressBalance.mul(dvpFee).div(sbfees));
    }
