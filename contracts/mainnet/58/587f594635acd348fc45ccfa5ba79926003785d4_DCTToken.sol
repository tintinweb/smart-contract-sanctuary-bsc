...
contract DCTToken is IERC20, Ownable {
    ...
    string private _name = "DCT";
    string private _symbol = "DCT";
    uint8 private _decimals = 18;

    uint256 public idoAmount = 10800000000 * 10 ** _decimals;
    uint256 public supply = 210000000000 * 10 ** _decimals;
    uint256 private _tTotal = supply - idoAmount;
    ...
    uint256 public minReferAmount = 1;
    uint256[] public REFERRAL_PERCENTS = [200, 150, 100, 50, 50, 50];
    ...
    function setMinReferAmount(uint256 amount) public onlyOwner {
        minReferAmount = amount;
    }

    function setAddress(address addr1, address addr2, address addr3) public onlyOwner {
        agentAddress = addr1;
    ...
    function setIdoPaused(bool _idoPaused) public onlyOwner {
    ...
    function ido(uint256 uAmount) public {
        require(!idoPaused, "ido have paused");
        uint256 uTotal = 500 * 10 ** 18;
        require(uAmount % 50 * 10 ** 18 == 0 && uAmount >= 50 * 10 ** 18, "uAmount error");
        require(userIdo[msg.sender] <= uTotal.sub(uAmount), "ido too big");
        userIdo[msg.sender] = userIdo[msg.sender].add(uAmount);
        uint256 price = 1080000 * 10 ** _decimals;
        uint256 getAmount = price.mul(uAmount).div(50 * 10 ** 18);
    ...
    function _mint(address account, uint256 amount) private {
        ...
        require(_tTotal <= supply, "mint too big");
    ...
    function _transfer(
        ...
        if (isContract(to) && !_isExcludedFromFee[from]) {
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) {
            ...
            uint256 recipientAmount = amount.sub(burnFee + lpFee + inviterFee);
    ...
    function _takeInviterFee(
        ...
        uint256 tFee = amount.mul(600).div(baseRate);
        address cur;
