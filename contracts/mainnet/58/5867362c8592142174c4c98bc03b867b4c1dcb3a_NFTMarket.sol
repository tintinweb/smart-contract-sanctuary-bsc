...
// royality and platform fee will be distributed
  ...
  function withdrawYourBid(uint256 itemId , address nftContractAddress) public nonReentrant{
    ...
    if(idToMarketItem[itemId].isOnAuction == true){
    require(idToMarketItem[itemId].isactive,"Item is not on auction");
    require(idToMarketItem[itemId].seller == msg.sender,"you are not owner");
    require(idToMarketItem[itemId].bidEndTime<=block.timestamp,"Bidding is still in progress");
    uint tokenId = idToMarketItem[itemId].tokenId;
    require(highestBidderMapping[itemId].bidderAddr == address(0),"Only highest bidder can claim the NFT");
...
//   owner can withdraw the contract's eth
  ...
  function withdraw(address _nftContractAddress  ,uint256 tokenId) public onlyOwner{
