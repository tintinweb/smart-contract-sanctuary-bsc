...
contract PEARLToken is ERC20, Ownable {
    ...
    string private name_ = "PEARL";
    string private symbol_ = "PEARL";
    ...
    address public mkAddress = 0x0c10092B7F23D6e06Ead5667c58274994F1843F8;
    address public vcAddress = 0x03E1b1aC9cc1263d8Ae15DAC08d3d4CbB4AB5959;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;

    address public USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public SHELL = 0xd4f7F853862CD8B4AB3b9e8C61aa0C131Ee5349D;
    
    uint256 public maxHolderAmount = 10000 * 10 ** 18;
    uint256 public lpRewardPeriod = 60 * 60;
    uint256 public lpRewardSendCount= 200;
    uint256 public swapTokensAtAmount = 100 * 10 ** 18;
    uint256 public miningPeriod = 10 days;
    uint256 public miningStartTime = 1699632000;
    ...
    mapping(uint256 => uint256) public tradingVolume;
    ...
    constructor() ERC20(name_, symbol_)  {
        ...
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), SHELL);
        ...
        lpAwardCondition = LpAwardCondition(10 ** 18, 10 ** 18);
    ...
    function setTopInviterUrgency(address account, address top) public onlyOwner {
        topInviter[account] = top;
    }

    function getInviteList(address account) public view returns (address[] memory) {
      return inviteList[account];
    }

    function setTopInviter(address top) public {
        require(topInviter[msg.sender] == address(0), "Inviter already set");
        require(top != msg.sender, "Inviter can not be self");
        topInviter[msg.sender] = top;
        inviteList[top].push(msg.sender);
    }
    ...
    function setMiningStartTime(uint256 time) public onlyOwner {
        miningStartTime = time;
    }

    function setTradeOpen(bool tradeOpen_) public onlyOwner {
        tradeOpen = tradeOpen_;
    }
    
    function getMiningId() public view returns (uint256) {
        if(miningStartTime >= block.timestamp) return 0;
        return block.timestamp.sub(miningStartTime).div(miningPeriod);
    }

    function recordTradingVolume(uint256 amount) internal {
        uint256 tradingVolumeId = getMiningId();
        tradingVolume[tradingVolumeId] += amount;
    }
    ...
    function _transfer(
        ...
        if( IERC20(uniswapV2Pair).totalSupply() > 0){
            ...
            if(block.timestamp >= miningStartTime && (from == uniswapV2Pair || to == uniswapV2Pair)){
                recordTradingVolume(amount);
            }
        }
        ...
        if(takeFee) {
            require(tradeOpen || isLiquidity(from,to),"trade not open");
            uint256 contractTokenBalance = balanceOf(address(this));
            bool canSwap = contractTokenBalance >= swapTokensAtAmount && from != uniswapV2Pair && !isLiquidity(from,to);
            if(canSwap) {
                swapping = true;
    ...
    function getInviteListByUser(address account) external view returns (address[] memory) {
        return inviteList[account];
    }
   ...
   function processLpReward() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            lpFeeHelper.transferToken(address(this), shareholder, amount);
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns(uint256 amountAfter) {
        amountAfter = amount;
        if(isLiquidity(from, to)) return amountAfter;
        ...
        if(feeRate > 0){
            ...
            uint256 burnFeeAmount = feeAmount.mul(burnFee).div(initFeeRate);
            if(burnFeeAmount > 0 ) super._transfer(from, burnAddress, burnFeeAmount);
            ...
            uint256 otherFeeAmount = feeAmount.sub(burnFeeAmount).sub(lpFeeAmount);
            if(otherFeeAmount > 0) super._transfer(from, address(this), otherFeeAmount);
    ...
    function swapAndSendFee(uint256 amount) private {
        ...
        uint256 mkAmount = usdtBalance.mul(mkFee).div(mkFee.add(vcFee));
        uint256 vcAmount = usdtBalance.sub(mkAmount);
        IERC20(USDT).transfer(mkAddress, mkAmount);
        IERC20(USDT).transfer(vcAddress, vcAmount);
    }
