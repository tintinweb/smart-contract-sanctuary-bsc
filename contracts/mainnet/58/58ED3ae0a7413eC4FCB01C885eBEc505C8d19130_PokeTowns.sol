...
contract PokeTowns {
    ...
    function addTools(address ref) external payable initialized {
    ...
    function withdrawEmerald(uint256 emerald) external initialized {
        address user = msg.sender;
        require(emerald <= buildings[user].emerald && emerald > 0);
        buildings[user].emerald -= emerald;
        ...
        payable(user).transfer(amount - fee2);
    }

    function collectEmerald() public initialized {
        address user = msg.sender;
        syncBuildings(user);
    ...
    function upgradeMarket() external initialized {
        address user = msg.sender;
        syncBuildings(user);
