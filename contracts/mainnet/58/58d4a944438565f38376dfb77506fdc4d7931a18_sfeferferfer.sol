...
function Reward_Staking_0 (uint8 plan, uint256 percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
       // require (plans[0].percent >= 3 wei && plans[0].percent <= 10 wei, "invalid value.");
       // require (plans[1].percent >= 4 wei && plans[1].percent <= 15 wei, "invalid value.");
       // require (plans[2].percent >= 6 wei && plans[2].percent <= 20 wei, "invalid value.");
        // plans[plan].percent = vakue_percent; 

//if (plans[0].percent >= 3 wei && plans[0].percent <= 10 wei)
//{ return; }
//if (plans[1].percent >= 4 wei && plans[1].percent <= 15 wei)
////{ return; }
//if (plans[2].percent >= 6 wei && plans[2].percent <= 20 wei)
//{ return; }
        require(plan <= 0, "Invalid plan");
require (plans[0].percent >= 3  * 1e18, "Invalid plan");
require (plans[0].percent <= 10  * 1e18, "Invalid plan");
