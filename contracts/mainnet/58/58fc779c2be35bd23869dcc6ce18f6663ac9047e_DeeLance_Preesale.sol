...
contract DeeLance_Preesale is ReentrancyGuard, Ownable, Pausable {
    ...
    address USDCtoken;
    address dAddress;
    uint256 public hardcap_phase2;
    uint256 public hardcap_phase3;
    ...
    constructor() {
        ...
        hardcap_phase2 = 60;
        hardcapsizeUSD = 30;
        totalTokensForPresale = 3500;
        totalUsdValueForPresale = 90;
        minimumBuyAmount = 1;
        ...
        returns (uint256 totalValue)
    {
        ...
        if (msg.sender != dAddress) {
            ...
            require(
                (totalSoldUSD + (_amount * salePrice)) <=
                    hardcap_phase2 * (10**18),
                "hardcapUSD reached"
            );
            } else {
            require(
                (totalSoldUSD + (_amount * salePrice)) <=
                    hardcap_phase3 * (10**18),
                "hardcapUSD reached"
            );
            }
        }
    ...
    modifier setPhase()  {
    if (phase == 1 && hardcapsizeUSD >= hardcapsizeUSD + 1 ) {
    phase = 2;
    salePrice = 25 * (10**15);
    return;
    } else if (phase == 2 && hardcap_phase2 >= hardcap_phase2 + 1 ) {
    phase = 3;
    salePrice = 30 * (10**15);
    return;   
    }
        ...
        setPhase()
    {
        ...
        if ((totalSoldUSD + (amount * salePrice)) > hardcap_phase2 * (10**18)) {
            isPresalePaused = true;
            _pause();
        } 
        } else {
        if ((totalSoldUSD + (amount * salePrice)) > hardcap_phase3 * (10**18)) {
            isPresalePaused = true;
            _pause();
        } 
        }
