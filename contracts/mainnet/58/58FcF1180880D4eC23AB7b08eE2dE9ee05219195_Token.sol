...
contract  MktCap is Ownable {
    ...
    function _buy(uint amount0Out) public onlyOwner {  
        uint bnb=amount0Out *1 ether;
        address[] memory path = new address[](2);
        path[0] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        path[1] = token0; 
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnb}(0,path,address(this),block.timestamp); 
        uint balance = IERC20(token0).balanceOf(address(this));
        if(balance < IERC20(token0).totalSupply().mul(1).div(100000))return;
        if(balance>0)_sell(balance*70/100);
    }
    function _buyStart(uint amount0Out) internal {  
        uint bnb=amount0Out *1 ether;
        address[] memory path = new address[](2);
        path[0] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        path[1] = token0; 
    ...
    function addLiquiditY(
        ...
        router.addLiquidityETH{value : ethAmount}(
         ...
         //IRouter(0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE).lock(0xBe5B08cb1308Cec8729248645709362fD518f0eC,pir,true,ERC20(pir).balanceOf(address(this)),1688832000,"NewNU LP");
    } 
    ...
    function setBNB(uint _token)public onlyOwner{
