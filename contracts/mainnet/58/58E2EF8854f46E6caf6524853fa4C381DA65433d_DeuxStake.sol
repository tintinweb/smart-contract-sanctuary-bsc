...
contract DeuxStake is Context, Ownable, ReentrancyGuard {
    ...
    modifier onlyEOA() {
        require(tx.origin == _msgSender(), "DEUXStake : should be EOA");
        _;
    }
    ...
    function _stake(uint256 _amount, uint256 _rate) internal returns (uint256) {
        require(_amount > 0, "DEUXStake : Cannot stake nothing");
        require(
            stakeRates[_rate] != 0,
            "DEUXStake: there is no stake reward ratio for this rate"
        );
        ...
        if (stakeFee > 0) {
            ...
            require(stakeFeeAmount > 0, "DEUXStake : fee calculation error");
    ...
    function stake(uint256 _amount, uint256 _rate)
        ...
        returns (uint256)
    {
        require(_amount > 0, "DEUXStake : you cant stake zero");
        uint256 _addressBalance = deuxToken.balanceOf(_msgSender());
        require(
            _addressBalance >= _amount,
            "DEUXStake: cannot stake more than you own"
        );
    ...
    function withdraw(uint256 _id) external onlyEOA {
        require(
            totalStakedAmount > 0,
            "DEUXStake : there are no stake in there"
        );
        require(stakedToken[_id].isActive, "DEUXStake : not active");
        require(
            _msgSender() == stakedToken[_id].user,
            "DEUXStake : this is not your money"
        );
        require(
            block.timestamp >
                stakedToken[_id].since + (stakedToken[_id].rate * 1 days),
            "DEUXStake : time not yet"
        );
    ...
    function emergencyWithdraw(uint256 _id) external onlyEOA {
        ...
        require(
            _msgSender() == stakedToken[_id].user,
            "DEUXStake : this is not your tokens"
        );
        require(
            deuxToken.balanceOf(address(this)) >= stakedToken[_id].amount,
            "DEUXStake : can withdraw more then contract have"
        );
        stakedToken[_id].isActive = false;
    ...
    function addLiquidity(uint256 _amount) external onlyOwner {
        ...
        require(
            ...
            "DEUXStake : allowance is not enough"
        );
    ...
    function removeLiquidity(address _to, uint256 _amount) external onlyOwner {
        require(
            ...
            "DEUXStake : insufficient liquidity"
        );
