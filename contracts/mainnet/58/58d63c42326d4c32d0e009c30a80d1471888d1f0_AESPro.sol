...
contract AESPro is Util, WhitelistAdminRole,CoinTokenWrapper {
    ...
    struct User{
        ...
        uint hisTokenAward;
        //AES静态奖励
        uint hisAesStaticAmount;
        uint allDynamicAmount;
        uint hisDynamicAmount;
        
        uint hisFiveDynamicAmount;
        uint allFiveDynamicAmount;
    ...
    mapping(address => uint) tokenServiceChargeMapping; //token手续费
    ...
    //测试five
    address fiveAddr = address(0xEc0029a08BcD725Ff3327288bEFB99ecEE29Bc36);
    ...
    constructor () public {
        ...
        supportTokenMapping[fiveAddr] = true;
        tokenServiceChargeMapping[fiveAddr] = 130;
    }
    ...
    //多Token投资
    function investIn(string memory inviteCode,string memory referrer,uint256 value,address token)
        public
        ...
        isHuman()
    {
        ...
        //扣除token的数量
        //uint256 tokenValue = getTokenPrice(usdtAddr,miningTokenAddr,value.div(usdtWei));
        uint256 tokenValue = value;
        IERC20(token).transferFrom(msg.sender,address(this),tokenValue);
        ...
        //用户动态奖励自动分红
        tjUserDynamicTree(userGlobal.referrer,tokenValue,value,isNewNode,token);
        
        //三倍算力挖矿
        super.stake(newLimitAmount);
        
        //抽水token
        uint drawAmount = tokenValue.mul(10).div(100);
        //IERC20(token).transfer(marketAddr,drawAmount);
    ...
    function autoBuy(uint amount) internal {
        ...
        IERC20 aes = IERC20(miningTokenAddr);
    ...
    //统计用户节点
    function tjUserDynamicTree(string memory referrer, uint tokenValue,uint usdtValue,uint8 isNewNode,address token) private {
        string memory tmpReferrer = referrer;

        //两倍本金算力
        uint doubleValueHash = usdtValue.mul(2);
        //计算代数收益
        uint recommendSc = getRecommendScaleByAmountAndTim();
        //收益TOKEN
        uint tmpDynamicTokenAmount = tokenValue.mul(90).div(100).mul(recommendSc).div(100);
        //收益USDT
        uint tmpDynamicUsdtValue = usdtValue.mul(90).div(100).mul(recommendSc).div(100);
        //TOKEN手续费
        uint serviceCharge = tokenServiceChargeMapping[token];
        //提现手续费
        uint withdrawalCharge =  tmpDynamicTokenAmount.mul(serviceCharge).div(1000);
        ...
        for (uint i = 1; i <= 10; i++) {
            ...
            //实时到账
            uint tmpDynamicAmount = tmpDynamicTokenAmount.sub(withdrawalCharge);
            //IERC20(token).transfer(tmpUserAddr,tmpDynamicAmount);
            
            //累计用户USDT收益
            calUser.hisDynamicAmount = calUser.hisDynamicAmount.add(tmpDynamicUsdtValue);
            
            //累计用户Five收益
            calUser.hisFiveDynamicAmount = calUser.hisFiveDynamicAmount.add(tmpDynamicAmount);
            //calUser.allFiveDynamicAmount = calUser.allFiveDynamicAmount.add(tmpDynamicAmount);
    ...
    //提现Five
    function withdrawFive() updateReward(msg.sender) public
    {
        // User storage user = userRoundMapping[rid][msg.sender];
        ...
        // uint resultMoney = user.allFiveDynamicAmount;
        // if (resultMoney > 0) {
        //     //提Five
        //     //fiveToken.transfer(msg.sender,resultMoney);
        //     user.allFiveDynamicAmount = 0;
    ...
    function getMiningInfo(address _user) public view returns(uint[47] memory ct,string memory inviteCode, string memory referrer) {
        ...
        ct[37] = userInfo.allFiveDynamicAmount;
        ct[38] = userInfo.hisFiveDynamicAmount;
        ...
        ct[44] = userInfo.nodeCount;
        ct[45] = userInfo.hisAesStaticAmount;
        ct[46] = balanceOf(_user);
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            //累计静态收益
            user.hisAesStaticAmount = reward;
            //流通量
            turnover = turnover.add(reward);
    ...
    function closeReward(uint staticReward) internal {
        ...
        //查询Aes的USDT价格
        uint aesUsdtValue = getTokenPrice(miningTokenAddr,usdtAddr,staticReward);
        //累计USDT收益
        calUser.earnAmount = calUser.earnAmount.add(aesUsdtValue);
        ...
        if (calUser.earnAmount >= calUser.limitAmount) {
            ...
            if(correction > 0){
                ...
                aesUsdtValue = aesUsdtValue.sub(correction);
        ...
        //动态减少算力
        super.reduce(msg.sender,aesUsdtValue);
    }
    ...
    function setUsdtToken(address _add) external onlyWhitelistAdmin
    {
        usdtAddr = address(_add);
        usdtToken = IERC20(usdtAddr);
    }

    function setOpenFiveRechard(bool _flag) external onlyWhitelistAdmin
    {
        isOpenFiveRechard = _flag;
    }
