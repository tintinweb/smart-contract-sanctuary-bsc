...
contract MFETStake is Context, Ownable, ReentrancyGuard {
    ...
    function setMfetToken(address _contract) public onlyOwner returns (bool) {
        require(
            _contract != address(0),
    ...
    function setStakeFee(uint256 _fee) public onlyOwner returns (bool) {
        stakeFee = _fee;
        return true;
    }
    ...
    function _monthlyRewards(uint256 _id) internal returns (uint256) {
        ...
        require(totalDaysPast > 0, "MFET Stake : day past must bigger then 0");

        uint256 dayPassRatio = (totalDaysPast *
            stakedToken[_id].rewardRatio *
            1e9) / (stakedToken[_id].rate * 30);

        require(dayPassRatio > 0, "MFET Stake : dayPass must bigger then 0");
