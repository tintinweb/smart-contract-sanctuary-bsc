...
interface IBPARTAgent {
    function addBPARTPoolToken(address _token) external;
    function delBPARTPoolToken(address _token) external;
...
// BPART AGENT THAT WILL PERFORM ALL THE BUYING AND SELLING
contract BPARTAgent is IBPARTAgent {
    ...
    address[] public bpartPool;
    mapping (address => uint256) public bpartBuy;
    mapping (address => uint256) public bpartSell;
    ...
    // Add tokens to the BPART Buy Pool
    function addBPARTPoolToken(address _token) external override onlyToken {
        bpartPool.push(_token);
    }

    // Remove token from the BPART pool
    function delBPARTPoolToken(address _token) external override onlyToken {
        for (uint256 i = 0; i < bpartPool.length; i++) {
            if (bpartPool[i] == _token) {

                bpartPool[i] = bpartPool[bpartPool.length - 1];
                bpartPool.pop();
    ...
    // Show BPARTpool token
    function showBPARTPoolToken(uint256 _position) external view onlyToken returns (address) {
        return bpartPool[_position];
    }
    function showBPARTBuy(address _token) external view onlyToken returns (uint256) {
        return bpartBuy[_token];
    }
    function showBPARTSell(address _token) external view onlyToken returns (uint256) {
        return bpartSell[_token];
    }

    // TOKEN OPERATIONS
    // Accept buy request from BPART
    function processBuyRequest(address _token, uint256 _buyamount) external override onlyToken {
        bpartBuy[_token] = bpartBuy[_token].add(_buyamount);
        emit agentError(384, _token, _buyamount);
    }

    // Accept sell requets from BPART
    function processSellRequest(address _token, uint256 _sellpercentage) external override onlyToken {
        ...
        bpartSell[_token] = bpartSell[_token].add(tokenSellAmount);
    ...
    function processAgentActions(uint256 gas) external override onlyToken {
        uint256 bpartPoolCount = bpartPool.length;

        if(bpartPoolCount == 0) { return; }
        ...
        while(gasUsed < gas && iterations < bpartPoolCount) {
            if(currentIndex >= bpartPoolCount){
                currentIndex = 0;
            }

            emit agentError(410, bpartPool[currentIndex], currentIndex);
            if(bpartSell[bpartPool[currentIndex]] > 0) {
                doSellToken(bpartPool[currentIndex]);
            } else {
                doBuyToken(bpartPool[currentIndex]);
            }
    ...
    function doBuyToken(address _token) public onlyToken {
        uint256 tokenBuyAmount = bpartBuy[_token];
    ...
    function doSellToken(address _token) public onlyToken {
        ...
        bpartSell[_token] = bpartSell[_token].sub(tokensToSell);
    }
    ...
    function clearStuckBPARTBNB(address _wallet, uint256 _amount) external onlyToken {
...
// MAIN BBFT CONTRACT
contract BBFTPartners is BPARTAuth {
    using SafeMath for uint256;
    ...
    BPARTAgent bpartagent;
    uint256 public bpartagentGas = 600000;
    ...
    function addPoolToken(address _token, uint256 _tokenWeight) external authorizedFor(Permission.AdjustContractVariables) {
        ...
        bpartagent.addBPARTPoolToken(_token);
    }
    ...
    function delPoolToken(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if (tokenPool[i] == _token) {
                ...
                bpartagent.delBPARTPoolToken(_token);
    ...
    // Set the minimum number of BNB required to perform BPART buys
   ...
   // Set the agent gas
    function setBPARTAgentGas(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bpartagentGas = _amount;
    }
    ...
    function setBBFTAddress(address _token) external authorizedFor(Permission.AdjustContractVariables) {
        BBFT = _token;
        bpartagent.setAgentBBFTAddress(_token);
    }
    ...
    function requestTokenBuys() internal {
        if(checkBuyState()) {
            ...
            bpartagent.processAgentActions(bpartagentGas);
        }
    }
    ...
    function performTokenBuyRequests() internal swapping {
        ...
        emit bpartError(693, address(this), curBNBBuyAmount);
        for (uint256 i = 0; i < tokenPool.length; i++) {
            if(isBuyToken[tokenPool[i]]) {
                curTokenBuyAmount = curBNBBuyAmount.mul(tokenWeight[tokenPool[i]]).div(totalWeight);
                emit bpartError(697, tokenPool[i], curTokenBuyAmount);
                bpartagent.processBuyRequest(tokenPool[i], curTokenBuyAmount);
            }
        }

        (bool sentBNB, ) = payable(address(bpartagent)).call{value: curBNBBuyAmount}("");
        ...
        if(!inSwap) {
            ...
            for (uint256 i = 0; i < tokenPool.length; i++) {
                if(isSellToken[tokenPool[i]]) {
                    emit bpartError(724, tokenPool[i], tokenSellP[tokenPool[i]]);
                    ...
                    emit bpartError(731, tokenPool[i], curSellP);
                    bpartagent.processSellRequest(tokenPool[i], curSellP);
                }
            }
        }
    }
    ...
    function manualConvertBNBtoBUSD(uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(address(this).balance > _amount) {
            bpartagent.doManualBUSD(_amount);
            emit bpartError(743, address(this), _amount);
            (bool sentBNB, ) = payable(address(bpartagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
        }
    }
    ...
    function doMigrateToken(address _token, uint256 _percentage, address _destination) external authorizedFor(Permission.AdjustContractVariables) {
        bpartagent.doMigrateToken(_token, _percentage, _destination);
    }
    ...
    function clearStuckBPARTBNB(address _wallet, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        bpartagent.clearStuckBPARTBNB(_wallet, _amount);
    }
    function doBurnBBFT() external authorizedFor(Permission.AdjustContractVariables) {
        bpartagent.doBurnBBFT();
    }
    ...
    function emergencySellToken(address _token, uint256 _percentage) external authorizedFor(Permission.AdjustContractVariables) {
        bpartagent.emergencySellToken(_token, _percentage);
    }
    ...
    function emergencyBuyToken(address _token, uint256 _amount) external authorizedFor(Permission.AdjustContractVariables) {
        if(!inSwap) {
            (bool sentBNB, ) = payable(address(bpartagent)).call{value: _amount}("");
            require(sentBNB, "BNB not sent");
            bpartagent.processBuyRequest(_token, _amount);
        }
    }

    // DEBUGGING FUNCTIONS
    function manualRequestTokenBuys() external authorizedFor(Permission.AdjustContractVariables) {
        emit bpartError(787, address(this), 0);
        requestTokenBuys();
    }
    function showBPARTPoolToken(uint256 _position) external view authorizedFor(Permission.AdjustContractVariables) returns (address) {
        return bpartagent.showBPARTPoolToken(_position);
    }
    function showBPARTBuy(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bpartagent.showBPARTBuy(_token);
    }
    function showBPARTSell(address _token) external view authorizedFor(Permission.AdjustContractVariables) returns (uint256) {
        return bpartagent.showBPARTSell(_token);
    }
    event bpartError(uint256 indexed _eventid, address indexed _eventaddr, uint256 indexed _eventvalue);
}