...
// File @openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.[email protected]
...
// File contracts/Interfaces/IFreezer.sol
...
// File contracts/Freezer/Freezer.sol
    ...
    IFreezer
{
    ...
    function initialize(
        ...
        HoneyToken = IHoney(honeyTokenAddress);
        StakingPool = IStakingPool(stakingPoolAddress);
        timePeriods.push(TimePeriod(2592000, 100));
        timePeriods.push(TimePeriod(7776000, 400));
        timePeriods.push(TimePeriod(15552000, 700));

        IERC20Upgradeable(honeyTokenAddress).safeApprove(
            address(StakingPool),
            type(uint256).max
        );
    ...
    function freeze(uint256 honeyAmount, uint256 periodIndex)
        external
        override
        whenNotPaused
        nonReentrant
    {
        require(honeyAmount > 0, "freezing amount needs to be non zero");
        ...
        StakingPool.stake(honeyAmount);

        uint256 currentRound = rounds[msg.sender] + 1;

        participants[msg.sender][currentRound].honeyRewardMask = honeyRoundMask;
        participants[msg.sender][currentRound].bnbRewardMask = bnbRoundMask;
        participants[msg.sender][currentRound].amount = honeyAmount;
        participants[msg.sender][currentRound].periodIndex = periodIndex;
        participants[msg.sender][currentRound].freezingStartTime = block
            .timestamp;

        rounds[msg.sender] = currentRound;

        // update freezer points with investment amount in ghny * freezing period months
        freezerPoints[msg.sender] += (honeyAmount *
            (timePeriods[periodIndex].period / 2592000));

        totalFreezedAmount += honeyAmount;
        freezedAmountDistribution[periodIndex] += honeyAmount;

        emit Freezed(msg.sender, currentRound, honeyAmount);
    }
    ...
    {
        _claimAllStakingRewards();
        Participant memory participant = participants[freezerAddress][
            freezingRound
        ];
        if (
            participant.honeyRewardMask == 0 || participant.bnbRewardMask == 0
        ) {
            return (0, 0, 0);
        }
        honeyRewards =
        ...
        multipliedRewards = (honeyRewards * (participantMultiplier)) / 1000;
    }
    ...
    function _claimPendingRewards(uint256 freezingRound) internal {
        ...
        participants[msg.sender][freezingRound]
            .honeyRewardMask = honeyRoundMask;
        ...
        participants[msg.sender][freezingRound]
            .claimedHoneyRewards += (honeyRewards + multipliedRewards);

        HoneyToken.claimTokens(multipliedRewards);

        IERC20Upgradeable(address(HoneyToken)).safeTransfer(
            msg.sender,
            honeyRewards + multipliedRewards
        );
