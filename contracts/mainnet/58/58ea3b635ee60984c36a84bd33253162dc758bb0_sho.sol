...
contract sho is Ownership {
	...
	function isTaiYiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
	...
	function queueTransaction(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function isMaxHold(uint256 _maxHold) public view returns(uint256) {
    ...
    function newTaiYiCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
