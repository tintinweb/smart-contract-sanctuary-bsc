...
contract FeeReceiver {
    ...
    function setAllocation(address recipient, uint allocation) external onlyOwner {
        totalAllocation = totalAllocation + allocation - allocations[recipient];

        if (allocations[recipient] == 0 && allocation > 0) {
    ...
    function _trigger(address token) internal {
        ...
        for (uint i = 0; i < len;) {
            uint allocation = allocations[recipients[i]];
            if (allocation > 0) {
                IERC20(token).transfer(
                    recipients[i],
                    (( bal * allocation ) / totalAllocation ) - 1
                );
            }
            unchecked { ++i; }
        }
