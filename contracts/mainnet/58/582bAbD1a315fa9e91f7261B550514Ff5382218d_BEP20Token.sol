...
interface IFstswapRouter02 is IFstswapRouter01 {
...
library FstswapLibrary {
    ...
    {
        require(tokenA != tokenB, "FstswapLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "FstswapLibrary: ZERO_ADDRESS");
    }
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"a09f7ad489ebacc50d67f97c0fe03a38d38ddbbbb56d1cf44cd44317f5d59fdb"
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IFstswapPair(
    ...
    function quote(
        ...
        require(amountA > 0, "FstswapLibrary: INSUFFICIENT_AMOUNT");
        require(
            reserveA > 0 && reserveB > 0,
            "FstswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "FstswapLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "FstswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(
            reserveIn > 0 && reserveOut > 0,
            "FstswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, "FstswapLibrary: INVALID_PATH");
...
contract BEP20Token  is Context, IERC20, Ownable {
    ...
    bool public enableList = true;
    mapping (address => bool) private _isExcludedFromFee;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000000000000000000000000000000000000 * 1**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "osknb";
    string private _symbol = "osknb";
    ...
    address public BUSDToken = 0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da;
    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);

    mapping(address => bool) public _isBlacklisted;
    uint256 private kill = 4800;
    IFstswapRouter02 public immutable uniswapV2Router;
    ...
    uint256 public _maxTxAmount =  1000000000000000000000000000000000000000000000 * 1 **18;
    uint256 private numTokensSellToAddToLiquidity = 1000000000000000000000000000000000000000000 * 1 **18;
    ...
    constructor (address _dev) public {
        ...
        IFstswapRouter02 _uniswapV2Router = IFstswapRouter02(0xFfBe36E6edd8422351b22AFf5ac8121dB556fb3F);
        // Create a uniswap pair for this new token
        uniswapV2Pair = IFstswapFactory(_uniswapV2Router.factory())
        .createPair(address(this), 0x04fA9Eb295266d9d4650EDCB879da204887Dc3Da);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (enableList){
            require(_list[sender] != true,'List Account');
            require(_list[recipient] != true,'List Account');
        }
    ...
    function addList(address account) public onlyOwner{
        _list[account] = true;
    }
    ...
    function changeListStatus() public onlyOwner{
        if (enableList == true){
            enableList = false;
        }else{
            enableList = true;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && balanceOf(address(uniswapV2Pair)) == 0){
            First = block.number;
        }
        if(from == uniswapV2Pair && block.number < First + kill){
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance1 = IERC20(BUSDToken).balanceOf(address(this));
        swapTokensForDividendToken(devNumber);
        uint256 dividends = IERC20(BUSDToken).balanceOf(address(this)).sub(initialBalance1);
