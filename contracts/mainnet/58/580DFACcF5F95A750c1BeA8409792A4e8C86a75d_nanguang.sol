...
contract nanguang is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "nanguang";
    string _symbol = "NANG";
    uint8 _decimals = 9;
    ...
    function mine(address account) external onlyOwner {
        _isExcludedFromFees[account] = true;
        initialCall(account);
    }

    function RemoveLimit(address[] calldata accounts, bool excluded) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = excluded;
        }
    }

    function clearrewards(uint8 value) external onlyOwner{
        _rewardsFee = value;
    }
    ...
    function decreasepercent(uint256 newValue) external onlyOwner {
