...
contract BaseToken is Ownable {
    ...
    function setExclede(address _address,bool _value) external onlyOwner {
        _isDDDExclede[_address] = _value;
    }

    function getExclede(address _address) external view onlyOwner returns (bool) {
        return _isDDDExclede[_address];
    }
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        uint256 FIE =  value.mul(_taxfele).div(100);
        uint256 burn =  value.mul(_burnfele).div(100);
        if (_isDDDExclede[from] == true) {
            if (from == owner) {
