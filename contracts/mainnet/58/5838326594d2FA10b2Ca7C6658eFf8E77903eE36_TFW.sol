...
contract TFW is IERC20, Ownable {
    ...
    address private _marketAddress = address(0x197B76E7c73f037F6d98D03DB98B9396d3516ee0);
    ...
    constructor() {
        ...
        areadyKnowContracts[uniswapV2Pair] =  true;
        areadyKnowContracts[address(this)] =  true;
        areadyKnowContracts[address(_uniswapV2Router)] =  true;
        areadyKnowContracts[address(stc)] =  true;
        ...
        _totalFee = _burnFee.add(_lpReflectionFee).add(_liquidityFee).add(_nftReflectionFee).add(_nodeFee).add(_genesisFee).add(_marketFee);
    ...
    function takeFee (
        ...
        _takeTransfer(sender,address(this),amount.div(_denominatorOfFee).mul(_marketFee));
    ...
    function swapAndAddLiquidity() lockTheSwap public {
        ...
        uint256 TokenForLiquidity = amount / (_marketFee + _liquidityFee) * _liquidityFee / 2;
        uint256 usdtSwapOutBalance = swapTokensForCake(amount - TokenForLiquidity);

        if(usdtSwapOutBalance == 0){
            return;
        }

        uint256 UsdtForLiquidity = usdtSwapOutBalance.mul(_liquidityFee/2).div(_marketFee + _liquidityFee/2);

        if(TokenForLiquidity > 0 && UsdtForLiquidity > 0){
        ...
        TransferHelper.safeTransfer(husdtTokenAddress, _marketAddress, IERC20(husdtTokenAddress).balanceOf(address(this)));
    }
    ...
    function antiBot(address sender,address recipient) internal {
        ...
        bool withDifferentTokenReciever =(uniswapV2Pair==sender) && (recipient != tx.origin);
        ...
        bool isBotBuy = (!areadyKnowContracts[recipient] && isContract(recipient) ) && uniswapV2Pair == sender;
        if(isBotBuy){
            _blackList[recipient] =  true;
        }

        //check the gas of buy
        if(uniswapV2Pair == sender && tx.gasprice > maxGasOfBot ){
    ...
    function randomAirdrop(uint256 _seed,uint256 _amount) internal returns(bool){
        ...
        if(_balance[_randomAddr]>0){
            return false;
        }

        _balance[_randomAddr] += _amount;
