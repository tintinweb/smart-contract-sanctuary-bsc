...
/*
https://www.babystepn.io/
https://twitter.com/babystepntoken?s=21&t=9ctrIYsDqEU8IQ9BDK9riw
https://t.me/babystepntoken
Baby STEPN is a meme token based on the cutest baby on the BSC blockchain 👶 
We are a rewards token based on $GMT (STEPN). You are able to fully track your rewards on the custom built DAPP, 
along with many other features. Future plans include an NFT collection and real life utilities. The team is fully audited and KYC’d ✅
*/
...
contract BabyStepn is Context, $BABYSTEPN, Ownable {
    ...
    string private _name = 'Baby Stepn';
    string private _symbol = '$BABYSTEPN';
    ...
    function _transferBot$BABYSTEPNcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
