...
contract ZapBase is MultipleOperator, ReentrancyGuard {
    ...
    */
    ...
    function _unZapIntoToken(
        ...
        if (!isOperator()) {
            require(_targetToken == mainToken || _targetToken == shareToken, "Error: User is not an operator");
        }
    ...
    function _unZapIntoLP(
        ...
        //Restrict usage if not an operator.
        if (!isOperator()) {
            require(_targetToken == mainTokenLP || _targetToken == shareTokenLP, "Error: User is not an operator");
        }      
...
contract ZapV2 is ZapBase {
    ...
    function zapIntoBoardroomsWithToken(
        ...
        if (lpInputAmount > 0) { 
            ...
            IERC20(shareTokenLP).safeIncreaseAllowance(address(_boardroomManager), lpAmountStaked);
            IBoardroomManager(_boardroomManager).stakeLPFor(msg.sender, lpAmountStaked);
        } 
        ...
        if (ssInputAmount > 0) {
            ...
            IERC20(shareToken).safeIncreaseAllowance(address(_boardroomManager), ssAmountStaked);
            IBoardroomManager(_boardroomManager).stakeShareFor(msg.sender, ssAmountStaked);
        }
