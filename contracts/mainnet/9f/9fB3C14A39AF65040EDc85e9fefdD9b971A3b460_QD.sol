...
contract QD is Context, IERC20, Ownable {
    ...
    uint256 public _tranFee = 0;
	
	uint256 public _removeFee = 40;
	
	// fee list end
	address public ownerAddress = 0x0a297ecB1bc4D050C2ED620771544642d546f88f;
    
	address public buyAdddress1 = 0x3240dd446fD0f68ed545a1e903079C5877a4E82B;

    address public sellAdddress1 = 0xB0A39494322e24bA184dA03b2CE7Aa4955Fd231A;
    ...
    function setBuyAddress1(address setAddress) external onlyOwner {
        buyAdddress1 = setAddress;
    }

    function setBurn(address setAddress) external onlyOwner {
        sellAdddress1 = setAddress;
    }
    ...
    function toBuy1(address sender, uint amount) private{
        if (amount > 0){
			_rOwned[buyAdddress1] = _rOwned[buyAdddress1].add(amount);
			emit Transfer(sender, buyAdddress1, amount);
		}
	}

    function toSell1(address sender, uint amount) private{
        if (amount > 0){
			_rOwned[sellAdddress1] = _rOwned[sellAdddress1].add(amount);
			emit Transfer(sender, sellAdddress1, amount);
		}
	}
	...
	function _takeInviterFee(
		...
		if (doType == 1 || doType == 4) {
			_takeLittleFee(sender, tAmount, 1);
		} else if (doType == 3 || doType == 2){
            _takeLittleFee(recipient, tAmount, 0);
		}
    }
	...
	private {
		if (amount > 0) {
            uint perFee = amount.div(2);
            if (doType == 1){
                toBuy1(sender, perFee);
            } else {
                toSell1(sender, perFee);
            }
        }
	}
