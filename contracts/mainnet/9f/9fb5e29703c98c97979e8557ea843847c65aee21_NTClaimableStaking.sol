...
contract NTClaimableStaking {
    ...
    function stake(uint256 amount, uint256 planIndex) public {
        ...
        stakersRecord[msg.sender][index].rewardpersecond = stakersRecord[msg.sender][index].reward.div(Duration[planIndex]);
    ...
    function claimRewards(uint256 index) public {
        ...
        require(block.timestamp.sub(stakersRecord[msg.sender][index].lastclaimtime) >= 86400, "Please wait 24hours to claim again");
        
        uint256 _claimamount;
        uint256 _persecondreward = stakersRecord[msg.sender][index].rewardpersecond;
        uint256 _sincelastclaim = block.timestamp.sub(stakersRecord[msg.sender][index].lastclaimtime);
        uint256 _claimable = _persecondreward.mul(_sincelastclaim);

        if(_claimable > (stakersRecord[msg.sender][index].reward).sub(stakersRecord[msg.sender][index].rewardclaimed)){
            _claimamount = (stakersRecord[msg.sender][index].reward).sub(stakersRecord[msg.sender][index].rewardclaimed);
        }else{
            _claimamount = _claimable;
        }
         
        token.transfer(msg.sender, _claimamount*1e9);

        stakersRecord[msg.sender][index].lastclaimtime = block.timestamp;
        totalClaimedRewardToken = totalClaimedRewardToken.add(_claimamount);
        Stakers[msg.sender].totalClaimedRewardTokenUser = Stakers[msg.sender].totalClaimedRewardTokenUser.add(_claimamount);
        stakersRecord[msg.sender][index].rewardclaimed = stakersRecord[msg.sender][index].rewardclaimed.add(_claimamount);
