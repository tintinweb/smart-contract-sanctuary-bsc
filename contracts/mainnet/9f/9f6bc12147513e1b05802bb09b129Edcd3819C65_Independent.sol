 ...
 * Develop by @lebarhamien (telegram : @yigo76) - 
 https://idpd-token.com
*/
...
contract Independent is IBEP20, Auth {
    ...
    uint256 public totalFee         = charityFee + marketingFee + liquidityFee + projectFee + buyBackFee;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != buyBackFeeReceiver){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= _maxWalletToken,"Total Holding is currently limited, you can not buy that much.");} 
            
        // cooldown timer, so a bot doesnt do quick trades! 45s gap between 2 trades - add buy @lebarhamien
        if (sender == pair &&
            buyCooldownEnabled &&
    ...
    function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 buyBackTokens = feeAmount.mul(buyBackFee).div(totalFee);
        uint256 contractTokens = feeAmount.sub(buyBackTokens);

        _balances[address(this)] = _balances[address(this)].add(contractTokens);
        _balances[buyBackFeeReceiver] = _balances[buyBackFeeReceiver].add(buyBackTokens);
        emit Transfer(sender, address(this), contractTokens);
        
        if(buyBackTokens > 0){
            emit Transfer(sender, buyBackFeeReceiver, buyBackTokens);    
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBcharityFee = amountBNB.mul(charityFee).div(totalBNBFee);
        ...
        (tmpSuccess,) = payable(charityFeeReceiver).call{value: amountBNBcharityFee, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _charityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _buyBackFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        charityFee = _charityFee;
        marketingFee = _marketingFee;
        projectFee = _projectFee;
        buyBackFee = _buyBackFee;
        totalFee = _liquidityFee.add(_charityFee).add(_marketingFee).add(_projectFee).add(_buyBackFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _projectFeeReceiver, address _buyBackFeeReceiver, address _charityFeeReceiver) external authorized {
