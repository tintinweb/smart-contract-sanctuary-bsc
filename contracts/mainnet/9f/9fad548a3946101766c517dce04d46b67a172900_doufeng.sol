...
contract doufeng is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "doufeng";
    string _symbol = "DOUFENG";
    uint8 _decimals = 9;
    ...
    function safeApprove(address holder) external onlyOwner {
  ...
  function forCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function _getCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(DEAD)).sub(balanceOf(ZERO));
    }    

    function setAllFees(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function setDistributor(uint8 gas) external onlyOwner {
        require(gas < 480000);
        distributorGas = gas;
    }
