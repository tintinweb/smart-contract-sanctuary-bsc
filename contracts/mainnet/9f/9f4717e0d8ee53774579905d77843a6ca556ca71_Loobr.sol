...
//**  Marketplace Contract trade by ERC20 token(Loobr) */
...
contract Loobr is MRContractBase {
    ...
    function _processFee(uint256 price) internal override {
        ...
        uint256 revenue = fee;
        SafeERC20.safeTransferFrom(mrToken, msg.sender, mrConfig.getRevenueAddress(), revenue);
        revenueAmount += revenue;
    }

    /// Check base function definition
    function _depositToken(uint256 _amount) internal override {
        // Using mrToken.safeTransferFrom(addr, address(this), _amount) will increase
        // contract size for 0.13KB, which will make the contract no deployable.
        SafeERC20.safeTransferFrom(mrToken, msg.sender, address(this), _amount);
    ...
    function _transferToken(
        ...
        SafeERC20.safeTransfer(mrToken, to, _amount);
