...
contract HASHFLOWFARM {
	...
	constructor(address payable wallet, uint256 startDate)  {
		...
		plans.push(Plan(23, 50)); // 5% per day for 23 days
        plans.push(Plan(21, 65)); // 6.5% per day for 21 days
        plans.push(Plan(21, 70)); // 7% per day for 28 days
		plans.push(Plan(23, 85)); // 8.5% per day for 14 days (at the end)
        plans.push(Plan(24, 90)); // 9% per day for 21 days (at the end)
        plans.push(Plan(25, 100)); // 10% per day for 28 days (at the end)
	}
    ...
    modifier UNN() {
      require(msg.sender == commissionWallet, "F");
      _;
    }
