...
contract MFLATERELOADED is Context, IERC20, Ownable, LockToken {
    ...
    string private _name = "MFLATE RELOADED";
    string private _symbol = "MFL";
    uint8 private _decimals = 18;
 
    address payable public marketingWallet = payable(0x43642e03C4fc87C73ee2CF648196Fb4909C415ac);
...
for (uint256 i = 0; i < _excluded.length; i++) {
 ...
 if (_excluded[i] == account){ //updating _rowned to make sure the balances stay the same
...
break;
    ...
    function _transfer(address from, address to, uint256 amount) open(from, to) private 
    {
        ...
        if (
            ...
            //add liquidity and send bnb  marketing wallet
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        // swap token share
        uint256 tokensForSwap = contractTokenBalance.mul((_saleLiquidityFee + _liquidityFee)/2).add(_saleMarketingFee).add(_marketingFee)
                                                    .div(_saleLiquidityFee.add(_saleMarketingFee).add(_liquidityFee).add(_marketingFee));
        // Liquidity tokens share
        uint256 tokensForLiq = contractTokenBalance - tokensForSwap;
        ...
        // swap tokens for ETH
        swapTokensForBnb(tokensForSwap); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
 
        // how much ETH did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 liqETH = newBalance.mul((_saleLiquidityFee + _liquidityFee)/2).add(_saleMarketingFee).add(_marketingFee)
                                    .div(_saleLiquidityFee.add(_saleMarketingFee).add(_liquidityFee).add(_marketingFee));
 
        // add liquidity to uniswap
        addLiquidity(tokensForLiq, liqETH);
 
        marketingWallet.transfer(address(this).balance);
 
        emit SwapAndLiquify(tokensForLiq, liqETH);
    }
    ...
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) private {
        require(taxFee.add(liquidityFee).add(marketingFee) <= 15, "tax too high");
        _taxFee = taxFee;
