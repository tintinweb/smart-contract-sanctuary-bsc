...
contract GongSwap is Context, IERC20, Ownable {
    ...
    constructor (uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma,address _tokenOwner,uint256 _DevFee,address _routerAddress,address _devAdd) {
        ...
        _devAddress = payable(_devAdd);//0xEe89c2E6462141356c580f97BE3D5a35Abc3b27e
       ...
       // devFee = 5;
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_routerAddress); //0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(_devAddress, transferredBalance.div(_liquidityFee).mul(_devFee));
        transferToAddressETH(_marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(_devFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= _devFee, 'Marketing fee can not be less than 5 percent');
    ...
    function setMarketingAddress(address _marketingAddr) external onlyOwner() {
        _marketingAddress = payable(_marketingAddr);
    }

      function setDevAddress(address _devAddr) external onlyOwner() {
        _devAddress = payable(_devAddr);
    }
