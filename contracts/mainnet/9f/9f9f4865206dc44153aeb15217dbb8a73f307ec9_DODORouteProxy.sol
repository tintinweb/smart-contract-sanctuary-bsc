...
// File: contracts/SmartRoute/intf/IFeeManager.sol
        ...
        returns (uint256 returnAmount)
    {
        ...
        uint256 _fromTokenAmount = fromTokenAmount;
        {
            ...
            if(feeInfo.fee > 0) {
                ...
                if(_fromToken == _ETH_ADDRESS_) {
                    IWETH(_WETH_).deposit{value: feeInfo.feeAmount}();
                    SafeERC20.safeTransfer(IERC20(_WETH_), _FEE_MANAGER_, feeInfo.feeAmount);
                } else {
                    SafeERC20.safeTransfer(IERC20(_fromToken), _FEE_MANAGER_, feeInfo.feeAmount);
                }
    ...
    function dodoMutliSwap(
        ...
        }
        {
            ...
            if(feeInfo.fee > 0) {
                feeInfo.feeAmount = _fromTokenAmount.mul(feeInfo.fee).div(10000);
                _fromTokenAmount -= feeInfo.feeAmount;
                if(midToken[0] == _ETH_ADDRESS_) {
                    SafeERC20.safeTransfer(IERC20(_WETH_), _FEE_MANAGER_, feeInfo.feeAmount);
                } else {
                    SafeERC20.safeTransfer(IERC20(midToken[0]), _FEE_MANAGER_, feeInfo.feeAmount);
                }
