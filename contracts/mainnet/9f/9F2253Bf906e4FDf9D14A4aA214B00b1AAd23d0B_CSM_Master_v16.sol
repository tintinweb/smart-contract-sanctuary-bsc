...
// Modified by aknashu Date 01-12-2022
    ...
    function makeAdmin(address _admin_1,address _admin_2)public {
    require(supportWallet == msg.sender,"permision Denied");
        flushCycleDays = [90 days, 90 days, 90 days ];
        admin1 =_admin_1;
        admin2 =_admin_2;
    }
    ...
    function flushBusinessByAdmin(address _user) public {
    require(supportWallet == msg.sender || admin1 == msg.sender || admin2 == msg.sender, "permision Denied");
    if(businessLogs[_user].length >0){
        for(uint i =0 ; i<businessLogs[_user].length ;i++){
        if(businessLogs[_user][i].ispaid==false){
            if(businessLogs[_user][i].received ==0){
                settlementAmount[_user] =settlementAmount[_user].add(businessLogs[_user][i].matching);
            }
           businessLogs[_user][i].ispaid = true; 
        }
        }
    }
          uint nextcycledate = flushCycleDays[0];
        flushBusiness[_user].startDate = block.timestamp;
        flushBusiness[_user].endDate = block.timestamp.add(nextcycledate);
        flushBusiness[_user].flushdate = flushBusiness[_user].endDate.add(1 days);

        resetDowlineBusiness(_user);
