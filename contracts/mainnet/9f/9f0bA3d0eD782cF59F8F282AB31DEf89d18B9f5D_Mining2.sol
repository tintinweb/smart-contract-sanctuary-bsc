...
// Mining合约
contract Mining2 is Ownable, ReentrancyGuard {
    ...
    // 计算USDT价值多少个ETE。(1e10)个USDT等于多少个ete
    function getEtePrice() public view returns(uint256) {
        ...
        return reserveEte.mul((1e10)).div(reserveUsdt);
    }
    ...
    function userDeposit(uint256 lpAmount) public nonReentrant {
        ...
        if(_nowIssueMsg.userCount == 0 && _nowIssueMsg.nft1Count == 0 &&  _nowIssueMsg.nft2Count == 0) {
            ...
            if(_nowIssue > 1) {
                ...
                // uint256 _v2 = _IssueMsg[turn][_lastIssue].totalUsdt.mul(8).div(100);
                // uint256 _v1 = _IssueMsg[turn][_lastIssue].totalUsdt.mul(133).div(10000);
                uint256 _price = getEtePrice();
                uint256 _v2 = _IssueMsg[turn][_lastIssue].totalUsdt.mul(_price).div((1e10)).mul(8).div(100);
                uint256 _v1 = _IssueMsg[turn][_lastIssue].totalUsdt.mul(_price).div((1e10)).mul(133).div(10000);
    ...
    function userTake(bytes32 order) public nonReentrant {
        ...
        // 时间条件也要满足
        require(block.timestamp > _orderIssueMsg.endTime, "time error");
        // 必须是没有领取
        ...
        // 计算本期LP价值的U的价格
        // uint256 _valueUsdt = _orderIssueMsg.totalUsdt.mul(_orderMsg.count).div(_orderIssueMsg.total);
        // uint256 _earnMy = _valueUsdt.mul(_ratioMy).div(100);             // 自己的收益
        // uint256 _earnMySuper = _valueUsdt.div(100);                      // 1%    // 上一级的收益
        // uint256 _earnMyOtherSuper = _valueUsdt.mul(5).div(1000);         // 0.5%  // 上2-5级的收益
        uint256 _price = getEtePrice();
        uint256 _valueEte = _orderIssueMsg.totalUsdt.mul(_orderMsg.count).div(_orderIssueMsg.total).mul(_price).div((1e10));
    ...
    function _burnCrystalsNumber(OrderMsg memory _orderMsg, IssueMsg memory _orderIssueMsg) private {
        uint256 _burnNumber = _orderIssueMsg.totalUsdt.mul(_orderMsg.count).div(_orderIssueMsg.total).div((1e18)).mul(burnCrystalsRatio).div(100);
