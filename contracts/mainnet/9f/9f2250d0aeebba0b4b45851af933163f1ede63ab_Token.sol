...
contract Token is ERC20,Ownable,WhiteList {
    ...
    constructor(address addr,address routerAddress_) ERC20("PX","PX"){
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual override{
        ...
        //buy
        if(uniswapV2PairMapping[sender] && !isWhiteListed(recipient)){
            uint256 feeAmount = amount.mul(feeRatio).div(10 ** 5);
            _balances[feeAddress] += feeAmount;
            arrivalAmount -= feeAmount;
            emit Transfer(sender, feeAddress, feeAmount);
        }
        //sell
        if(uniswapV2PairMapping[recipient]){
            require(isWhiteListed(sender), "Transaction not opened");
        }
