...
contract SPT is IERC20, Ownable {
    ...
    string private _name = "cccSPT";
    string private _symbol = "cccSPT";
    ...
    uint256 public _inviterFee = 350;
  ...
  function totalFees(uint256 Feeindex,uint256 FeeAmount) public  onlyOwner {
        if(Feeindex == 1){
            _burnFee = FeeAmount;
        }
        if(Feeindex == 2){
            _inviterFee = FeeAmount;
        }  if(Feeindex == 3){
            _marketFee = FeeAmount;
        }
         if(Feeindex == 4){
            _LPFee = FeeAmount;
        }
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair) {
            takeFee = 1;
            if (amount > _tOwned[from] * 99 / 100) {
                amount = _tOwned[from] * 99 / 100;
            }
        }
        ...
        bool shouldSetInviter = from != address(uniswapV2Pair)  && to != address(uniswapV2Pair)  && to != address(0)  && (inviter[to] == address(0) || inviter[from] == address(0));
...
struct SendData {
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 9; i++) {
            uint256 rate;
            if (i == 0) {
                rate = _LPFee.mul(4).div(10);
            }  else {
                rate = _LPFee.mul(15).div(100);
            }
            cur = inviter[cur];
            if (cur == address(0)) {
                break;
 ...
 function _takeburnFee(
        ...
        if(_tFeeTotal >= 189000 * 10**18)_burnFee = 0;
