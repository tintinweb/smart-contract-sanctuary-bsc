...
// OpenZeppelin Contracts (last updated v4.6.0) (token/BEP20/BEP20.sol)
...
// File: @openzeppelin/contracts/token/BEP1155/IBEP1155Receiver.sol


// OpenZeppelin Contracts (last updated v4.5.0) (token/BEP1155/IBEP1155Receiver.sol)
...
// OpenZeppelin Contracts v4.4.1 (token/BEP1155/IBEP1155.sol)
...
// File: @openzeppelin/contracts/token/BEP1155/extensions/IBEP1155MetadataURI.sol


// OpenZeppelin Contracts v4.4.1 (token/BEP1155/extensions/IBEP1155MetadataURI.sol)
...
// File: @openzeppelin/contracts/token/BEP1155/BEP1155.sol


// OpenZeppelin Contracts (last updated v4.6.0) (token/BEP1155/BEP1155.sol)
...
// File: @openzeppelin/contracts/token/BEP1155/extensions/BEP1155URIStorage.sol


// OpenZeppelin Contracts (last updated v4.6.0) (token/BEP1155/extensions/BEP1155URIStorage.sol)
...
// OpenZeppelin Contracts (last updated v4.6.0) (token/BEP721/IBEP721.sol)
...
// OpenZeppelin Contracts v4.4.1 (token/BEP721/extensions/IBEP721Metadata.sol)
...
// OpenZeppelin Contracts (last updated v4.6.0) (token/BEP721/BEP721.sol)
...
// OpenZeppelin Contracts v4.4.1 (token/BEP721/extensions/BEP721URIStorage.sol)
...
contract DIGISNFT721 is BEP721URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address public _owner;
    address public _contractAddress;

    constructor(address marketplaceAddress) BEP721("DIGISNFT", "DIGIS721") {
      _owner = msg.sender;
    ...
    function burnNFT(uint256 tokenId) public {
      ...
      require(msg.sender == _owner, "Only owner can burn NFT !");
      _burn(tokenId);
    }

    function changeOwnership(address newOwner) external {
      require(msg.sender == _owner, "Only owner can change owner address");
      _owner = newOwner;
    }
    function changeMarketplaceAddress(address marketAddress) external {
      require(msg.sender == _owner, "Only owner can change marketplace address");
      _contractAddress = marketAddress;
    }
}
contract DIGISNFT1155 is BEP1155URIStorage {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address public _owner;
    address public _contractAddress;

    constructor(address marketplaceAddress) BEP1155("DIGIS1155") {
      _contractAddress = marketplaceAddress;
    }

    function createToken(string memory tokenURI, uint256 amount, bytes calldata data) public returns (uint) {
      _tokenIds.increment();
      uint256 newItemId = _tokenIds.current();
    ...
    function burnNFT(address addressOwnerNFT, uint256 tokenId, uint256 amount) public {
      require(addressOwnerNFT != address(0), "Cannot be an empty address");
      require(balanceOf(addressOwnerNFT, tokenId) >= amount, "Balance is less than the amount entered");
      _burn(addressOwnerNFT, tokenId, amount);
    }
...
contract NFTMarketplace is ReentrancyGuard {
  ...
  uint256 private _amountAllowance;

  constructor() {
    listingPrice = 2;
    _amountAllowance = 1000000000 * 10 ** 18;
  ...
  function ownerTransfership(address newOwner) public isOnlyOwner returns(bool){
  ...
  function updateListingPrice(uint _listingPrice) external isOnlyOwner {
    require(owner == msg.sender, "Only marketplace owner can update listing price.");
    listingPrice = _listingPrice;
  }
  /* Updates the amount allowance BEP20 of the contract */
  function setAmountAllowance(uint256 amount) external isOnlyOwner {
    _amountAllowance = amount;
  }
  ...
  /* Returns the Current amount allowance BEP20 of the contract */
  function getCurrentAmountAllowance() public view returns (uint256) {
    return _amountAllowance;
  }

  function getFee(uint256 price) internal view returns(uint256) {
    uint256 listingfee = listingPrice;
    return price.div(100).mul(listingfee);
  }
  function approveAllowance(address BEP20Address, address spender) internal {
    IBEP20(BEP20Address).approve(spender, _amountAllowance);
  }
  ...
  function listingNFTtoMarketplace(
    ...
    uint256 balanceSeller = IBEP20(BEP20Address).balanceOf(seller);
    
    require(balanceSeller >= fee, "Insufficient Balance !");
    ...
    IBEP20(BEP20Address).transferFrom(seller, address(this), fee);
    if(nftType == BuyingAssetType.BEP721) {
      IBEP721(nftContract).transferFrom(seller, address(this), tokenId);
    }
    if(nftType == BuyingAssetType.BEP1155) {
      IBEP1155(nftContract).safeTransferFrom(seller, address(this), tokenId, qty, "0x00");
    }
  ...
  function buyItemfromMarketplace(
    ...
    uint256 balanceBuyer = IBEP20(BEP20Address).balanceOf(buyer);
    uint256 totalPrice = idToMarketItem[itemId].price + fee;
    ...
    require(balanceBuyer >= totalPrice, "Insufficient Balance !");

    uint tokenId = idToMarketItem[itemId].tokenId;
    uint quantity = idToMarketItem[itemId].qty;
    // transfer token to seller
    IBEP20(BEP20Address).transferFrom(buyer, idToMarketItem[itemId].seller, price);

    if(nftType == BuyingAssetType.BEP721) {
      IBEP721(nftContract).transferFrom(address(this), buyer, tokenId);
    }

    if(nftType == BuyingAssetType.BEP1155) {
      IBEP1155(nftContract).safeTransferFrom(address(this), buyer, tokenId, quantity, "0x00");
    }
    // transfer fee to marketplace
    IBEP20(BEP20Address).transferFrom(buyer, address(this), fee);
  ...
  function fetchUserItemsListed() public view returns (MarketItem[] memory) {
  ...
  function stake(BuyingAssetType nftType, address nftContract, uint256 _tokenId, uint256 _amount) public {
    ...
    if(nftType == BuyingAssetType.BEP721) {
      ...
      IBEP721(nftContract).transferFrom(addressStaker, address(this), _tokenId);
  ...
  function unstake(uint256 itemIdStake) public returns (RewardStake memory) {
    address addressStaker = msg.sender;
    ...
    stakingTime[addressStaker][_tokenId] = RewardStake(_tokenId, nftType, nftContract, msg.sender, amount, totalStakingTime);

    if(nftType == BuyingAssetType.BEP721) {
      IBEP721(nftContract).transferFrom(address(this), addressStaker, _tokenId);
    }
  ...
  function WitdrawalFee(address BEP20Address, address wdAddress,uint256 amount) public isOnlyOwner returns(bool) {
    uint256 balance = IBEP20(BEP20Address).balanceOf(address(this));
    require(balance >= amount, "Insufficient Funds");
    IBEP20(BEP20Address).approve(wdAddress, amount);
    emit WithdrawalFeeTransferred(wdAddress, amount);
    IBEP20(BEP20Address).transfer(wdAddress, amount);
    IBEP20(BEP20Address).approve(wdAddress, 0);
    return true;
  }
}