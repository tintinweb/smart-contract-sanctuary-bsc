...
contract CrypterToken is ERC20, Ownable {
    ...
    address public constant UNISWAP_V2_ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public pair;
    ...
    uint256 public gonSwapThreshold = 10 * (10**18);

    address public stakingContract = 0xb26ED91B55F30152F8DA097336EE08a65df395A5;
    address public teamWallet = 0x42567B693B651Bf1d94d92C2F9EB3F2d8102335B;
    ...
    constructor() ERC20("CrypterToken", "CRYPT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (shouldTakeFee(sender, recipient)) {
            uint256 totalFeeAmount = amount.mul(totalFee).div(1000);
    ...
    function swapBack() internal swapping {
        ...
        uint256 stakingFeeAmount = contractBalance.mul(stakingFee).div(totalFee);
        uint256 marketingFeeAmount = contractBalance.mul(marketingFee).div(totalFee);
        uint256 teamRewardAmount = contractBalance.mul(teamFee).div(totalFee);

        super._transfer(address(this), teamWallet, teamRewardAmount);
    ...
    function shouldSwapBack(address sender, address recipient) internal view returns (bool) {
        ...
        !inSwap &&
        swapEnabled &&
        balanceOf(address(this)) >= gonSwapThreshold;
    }

    function shouldTakeFee(address from, address to) internal view returns (bool) {
        return (pair == from || pair == to) && !inSwap && !(_isExcludedFromFee[from] || _isExcludedFromFee[to]);
    }
    ...
    function setTaxAddresses(address _staking, address _teamWallet, address _marketingWallet) public onlyOwner {
    ...
    function setTaxFee(uint8 _stakingFee, uint8 _teamFee, uint8 _marketingFee) public onlyOwner {
        require(_stakingFee + _teamFee + _marketingFee < MAXFEE, "Max Fee is reached");
