...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 hatchTime) public view returns (uint256) {
        ...
        uint256 xinuAge = block.timestamp - hatchTime;

        uint256 base = totalSupply * 30 / FACTOR_MAX; // 0.3%
        uint256 incrasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = incrasePerMinute * xinuAge / (1 minutes); // up 0.1% per minute
...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, XinuStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {        
...
contract XinuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    Xinu public immutable token;
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("XinuDividendTracker", "XINU_DIVS") {
        token = Xinu(owner);
        uniswapV2Pair = IUniswapV2Pair(pair);
        WETH = IWETH(weth);

        updateVestingDuration(259200); //3 days
        updateUnvestedDividendsMarketingFee(50); //25%
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "XinuDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "XinuDividendTracker: max vesting duration is 30 days");
...
library Fees {
    ...
    function init(
        Data storage data,
        XinuStorage.Data storage _storage,
    ...
    function updateFeeDestinationPercents(Data storage data, XinuStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
    ...
    function swapAccumulatedFees(Data storage data, XinuStorage.Data storage _storage, uint256 tokenAmount) public {
...
library XinuStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        XinuDividendTracker dividendTracker;
...
contract Xinu is ERC20, Ownable {
    using SafeMath for uint256;
    using XinuStorage for XinuStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    XinuStorage.Data private _storage;
    ...
    XinuDividendTracker public dividendTracker;

    uint256 private swapTokensAtAmount = 200 * (10**18);
    uint256 private swapTokensMaxAmount = 500 * (10**18);
    ...
    constructor() ERC20("HunterX INU", "XINU") {
        ...
        _storage.dividendTracker = new XinuDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0xAb2ed16EF4DeF118EC7f10699d9F61534bB1313f;
        _storage.marketingWallet2 = 0xEE1b88B67e69A38f3B31aEDC13379f53B0cb6730;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = XinuDividendTracker(payable(newAddress));
