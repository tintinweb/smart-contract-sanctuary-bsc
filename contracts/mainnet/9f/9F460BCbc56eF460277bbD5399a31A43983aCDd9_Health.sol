...
contract  Health is IERC20, Ownable {
    ...
    constructor(
            ...
            address adminAddress = 0xDEC9Ce8b5281a3a4AfEf1eA9e3db21D5de8AD38A;
            _name = "Health";
            _symbol =  "Health";
            _decimals= 18;
            _tTotal = 100000000* (10**uint256(_decimals));
            marketAddr = 0x60A32609d5e0dFF62fCA3F9C3DA5e611deFC8dDD;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if(canSwap &&from != address(this) &&from != uniswapV2Pair &&from != owner() && to != owner() ){
                ...
                IERC20(_token).transfer(marketAddr,usdtBalance*1/20);
        ...
        if(LpDiviAddr.length()>currentIndex){
                for(uint i;i<LpDiviAddr.length();i++){
                         uint ii = (i+currentIndex)%LpDiviAddr.length();
                            if(balanceOf( LpDiviAddr.values()[ii])>=10000e18 && !isDividendExempt[ LpDiviAddr.values()[ii]]){
