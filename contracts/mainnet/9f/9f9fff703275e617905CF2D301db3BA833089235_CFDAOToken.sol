...
contract CFDAOToken is StandardToken,Ownable {
    ...
    address public shareholderAccount = 0x50475C9D725E55133845efe59f90c3D411cD2Dca;//股东
    address public transferfeeAccount = 0x7298Bbee68E82784169A757D391c2faD2C89cD9d;//转账手续费
    address public minningAccount = 0x1fF8B995Dd8664804206E296989B072E0e6Fa98D;//nft分红官方LP的分红地址
    ...
    { 
      if(src == controler || dst == controler){
        addLpPlayer(src);
      ...
      if(state == 0){
        ...
        if(sort==SALE){
          ...
          uint256 wadShareholder = wad.div(100);
          uint256 wadLpPrise = wad.div(100).mul(2);

          wad = wad.sub(wadShareholder);
          wad = wad.sub(wadLpPrise);
          
          super.transferFrom(src,shareholderAccount,wadShareholder);
    ...
    function checkLpBonus() public {
        ...
        for(uint256 i=0;i<lpUserListIndex;i++){
          ...
          //官方领取无主底池分红
          if(i==0){
              balances[minningAccount] = balances[minningAccount].add(prise);
              balances[address(this)] = balances[address(this)].sub(prise);
              emit Transfer(address(this), minningAccount, prise);
          }else{
    ...
    //-----------------------------------------------------------------------------
    function updateMinningAccount(address _minningAccount) public onlyControler onlySelf{
      minningAccount = _minningAccount;
    }
