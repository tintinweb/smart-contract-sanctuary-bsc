...
// Telegram : https://t.me/GoombaCoinPortal
// Website : https://goombacoin.app/
...
library Address{
 ...
 contract GoombaCoin is IERC20, Context, Ownable {
    ...
    string private constant _name = "GoombaCoin";

    uint256 public GOCO_max_BuyLimit = 30000000 * 10**9;
    uint256 public GOCO_max_SellLimit = 30000000 * 10**9;

    address private GOCO_Wallet = 0x99f46d58DE4aA0B633CaF2Ca1eFE88466253C44E;
    address private GOCO_Team_Wallet = 0x99f46d58DE4aA0B633CaF2Ca1eFE88466253C44E;

    uint256 public GOCO_max_WalletLimit = 30000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[GOCO_Wallet] = true;
        _isExcludedFromFee[GOCO_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[GOCO_Wallet] = true;
        allowedTransfer[GOCO_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= GOCO_max_BuyLimit, "You are exceeding GOCO_max_BuyLimit");
            require(balanceOf(to) + amount <= GOCO_max_WalletLimit, "You are exceeding GOCO_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= GOCO_max_SellLimit, "You are exceeding GOCO_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= GOCO_max_WalletLimit, "You are exceeding GOCO_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(GOCO_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(GOCO_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function GOCO_Update_MarketingWallet(address newWallet) external onlyOwner{
        GOCO_Wallet = newWallet;
    }
    
    function GOCO_Update_TeamWallet(address newWallet) external onlyOwner{
        GOCO_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        GOCO_max_BuyLimit = maxBuy * 10**decimals();
        GOCO_max_SellLimit = maxSell * 10**decimals();
    }
    
    function GOCO_Max_Walletlimit(uint256 amount) external onlyOwner{
        GOCO_max_WalletLimit = amount * 10**decimals();
    }
