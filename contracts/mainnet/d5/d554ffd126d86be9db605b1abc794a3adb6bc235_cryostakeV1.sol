...
contract cryostakeV1 is Owned {
    ...
    //maximum stakeable cryogen
    uint public maxStakeValue;
    //pause mechanism
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        //makes sure amount is not more than the registration fee and the max deposit
        require(_amount >= registrationTax.add(maxStakeValue), "Transaction will exceed the maximum staking amount of cryogen.");
    ...
    function stake(uint _amount) external onlyRegistered() whenActive() {
        ...
        //makes sure stakeholder does not stake above the maximum
        require(_amount <= maxStakeValue, "Amount is below minimum stake value.");
    ...
    //sets the maximum stake value
    function setMaxStakeValue(uint _maxStakeValue) external onlyOwner() {
        maxStakeValue = _maxStakeValue;
    }
