...
contract Logic is Ownable {
    ...
    function addXTokens(
        ...
        if ((token) != address(0)) {
            IERC20(token).approve(xToken, type(uint256).max);
    ...
    function setBLID(address blid_) external onlyOwner {
        ...
        blid = blid_;
        IERC20(blid).approve(apeswap, type(uint256).max);
        IERC20(blid).approve(pancake, type(uint256).max);
        IERC20(blid).approve(pancakeMaster, type(uint256).max);
        IERC20(blid).approve(apeswapMaster, type(uint256).max);
        IERC20(blid).approve(_storage, type(uint256).max);
        emit SetBLID(blid_);
    }
