...
contract RockBuyer is ERC20, SafeOwnable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address=>bool) managerList;
    IRouter public router;

    constructor() ERC20("RockBuyer", "RockBuyer") {
    ...
    function swap(uint256 amount, address token0,address token1,address to) external onlyManager  returns(uint256 swapTokenAmount) {
        uint256 beforeBalance=IERC20(token1).balanceOf(address(this));
        swapTokensForToken(amount,token0,token1,to);
