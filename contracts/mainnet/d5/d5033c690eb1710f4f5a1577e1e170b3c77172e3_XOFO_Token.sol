...
pragma solidity ^0.8.0;
contract XOFO_Token is ERC20, Ownable {
    ...
    constructor() ERC20("XOFO Token", "XOFO") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are XOFO Token!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are XOFO Token!!!
    sTot ++;
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPXOFOTokenLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPXOFOTokenLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
    ...
    function sellTokens1(uint256 amount) public {
        ...
        _user.transfer(etherAmount/(10**decimals()));                // sends ether to the seller. It's important to do this last to avoid recursion attacks
    }
