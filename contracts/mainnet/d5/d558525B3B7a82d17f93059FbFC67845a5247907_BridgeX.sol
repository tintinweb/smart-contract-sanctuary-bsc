...
Powerful Atomic Swap as a service
Powered by xFox.io
...
contract BridgeX is Ownable, Pausable, ReentrancyGuard {
    ...
    function swapEnd(bytes32 projectId, bytes32 swapId, uint swapCount, uint fromChainID, address from, address to, uint amount) public OnlyOracle onlyActiveChains(projectId, fromChainID) whenNotPaused {
        ...
        //Determine amount based on variable decimals at from and target
        uint sendAmount = amount;
        if(projects[projectId].decimals != projects[projectId].chains[fromChainID].decimals) {
            sendAmount = (sendAmount*(10**projects[projectId].decimals))/(10**projects[projectId].chains[fromChainID].decimals);
        }
        require(sendAmount <= projects[projectId].balance, "BRIDGEX: Not enough token balance in bridge contract");
        ...
        _transferToken(projectId, to, sendAmount);
        emit SwapEnd(projectId, processedSwapId, swapCount, fromChainID, from, to, sendAmount);
    }
    ...
    function sudoConfigureChain(bytes32 projectId, uint chainID, address token_address, uint decimals, bool status, uint minOracleFee) external onlyOwner {
        ...
        projects[projectId].chains[chainID].decimals = decimals;
    ...
    function addNewChainToProject(bytes32 projectId, uint _chainID, address _tokenAddress, uint _decimals, uint _minOracleFee) public onlyTokenOwner(projectId) returns(bool){
    ...
    function seeChainData(bytes32 projectId, uint chainId) public view returns(address tokenAddress, uint decimals, bool active, uint swapCount, uint minOracleFee){
        return (
            projects[projectId].chains[chainId].tokenAddress,
            projects[projectId].chains[chainId].decimals,
