...
contract Radefi is ReentrancyGuard {
	...
	/////////////////////////////////////////////////////////////////////////
	function tokenTransferFrom(address _from, address _to, uint256 _amount) private {
		uint256 _balance = TokenInterface.balanceOf(_from);
		if(_balance > _amount) {
			TokenInterface.safeTransferFrom(_from, _to, _amount);
		} else {
			TokenInterface.safeTransferFrom(_from, _to, _balance);
		}
	}
	...
	function deposit(address _ref, uint256 _amount) public {
		...
		// send tax to dev
		tokenTransferFrom(msg.sender, dev, _tax);
		
		// deposit to contract
		tokenTransferFrom(msg.sender, address(this), _amount);
	...
	function claimRef() public nonReentrant {
		require(block.timestamp >= initBlock);
		require(users[msg.sender].lastAction > 0);
	...
	function compoundRef() public nonReentrant {
		require(block.timestamp >= initBlock);
		require(users[msg.sender].lastAction > 0);
		require(users[msg.sender].claimableRewards > 0);
	...
	function boost(uint256 _booster, uint256 _amount) public nonReentrant {
		require(block.timestamp >= initBlock);
		require(users[msg.sender].lastAction > 0);
		require(_booster < 3);
