...
import {BasePrediction} from "./BasePrediction.sol";
import {IErc20Prediction} from "../interfaces/IErc20Prediction.sol";

/**
 * @title DexFinanceErc20Prediction
 */
contract Erc20Prediction is BasePrediction, IErc20Prediction {
		using SafeERC20 for IERC20; // TODO: ? already exists in Base?
        ...
        notContract
    {
        ...
        _betBear(epoch, msg.sender, amount, address(0), 0);
    }
    ...
    {
        _safeTransferFrom(msg.sender, address(this), amount);
        _betBull(epoch, msg.sender, amount, address(0), 0);
    }
    ...
    function _safeTransfer(address to, uint256 value) internal override {
		...
		function recoverToken(address _token, uint256 _amount) external override onlyOwner {
...
import { BetParams } from "../interfaces/IPrediction.sol";

/**
 * @title DexFinancePredictionV2
 */
abstract contract BasePrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    address public devAddress; // address of the dev
    ...
    uint256 public constant DEV_TREASURY_FEE = 1000; // 10%
    ...
    event BetBear(address indexed sender, uint256 indexed epoch, uint256 amount, address oToken, uint256 oAmount);
    event BetBull(address indexed sender, uint256 indexed epoch, uint256 amount, address oToken, uint256 oAmount);
    ...
    event NewDevAddress(address dev);
    ...
    event NewZapper(address zapper);
    ...
    event TreasuryClaim(address indexed owner, address indexed dev, uint256 ownerAmount, uint256 devAmount);
    ...
    modifier onlyDevOrOperator() {
        require(msg.sender == devAddress || msg.sender == operatorAddress, "Not operator/dev");
        _;
    }

    modifier onlyDevOrOwner() {
        require(msg.sender == devAddress || msg.sender == owner(), "Not operator/owner");
        _;
    }

    modifier onlyZapper() {
        require(msg.sender == zapperAddress, "Not zapper");
        _;
    }
    ...
    function zapBear(BetParams calldata p) external onlyZapper {
      _betBear(
          p.epoch,
          p.account,
          p.amount,
    ...
    function zapBull(BetParams calldata p) external onlyZapper {
      _betBull(
          p.epoch,
          p.account,
          p.amount,
     ...
     */
    function executeRound() external whenNotPaused onlyDevOrOperator {
        require(
     ...
     * @dev Callable by operator
     */
    function genesisLockRound() external whenNotPaused onlyDevOrOperator {
     ...
     * @dev Callable by dev or operator
     */
    function genesisStartRound() external whenNotPaused onlyDevOrOperator {
    ...
    /**
     * @notice called by the dev to pause, triggers stopped state
     * @dev Callable by dev or operator
     */
    function pause() external whenNotPaused onlyDevOrOperator {
        _pause();
     ...
     * @dev Callable by dev
     */
    function claimTreasury() external nonReentrant onlyDevOrOwner {
        uint256 currentTreasuryAmount = treasuryAmount;
        treasuryAmount = 0;
        uint256 devFeeAmount = currentTreasuryAmount * DEV_TREASURY_FEE / FEE_DIVIDER;
        uint256 ownerFeeAmount = currentTreasuryAmount - devFeeAmount;

        address _owner = owner();
        _safeTransfer(devAddress, devFeeAmount);
        _safeTransfer(_owner, ownerFeeAmount);

        emit TreasuryClaim(_owner, devAddress, ownerFeeAmount, devFeeAmount);
    }

    /**
     * @notice called by the dev to unpause, returns to normal state
     * Reset genesis state. Once paused, the rounds would need to be kickstarted by genesis
     */
    function unpause() external whenPaused onlyDevOrOperator {
     ...
     * @dev Callable by dev
     */
    function setMinBetAmount(uint256 _minBetAmount) external whenPaused onlyDevOrOwner {
     ...
     * @dev Callable by dev
     */
    function setOperator(address _operatorAddress) external onlyDevOrOwner {
     ...
     */
    // on
    function setZapper(address _zapper) external whenPaused onlyDevOrOwner {
        require(_zapper != address(0), "Cannot be zero address");
        zapperAddress = _zapper;
     ...
     */
    function setOracle(address _oracle) external whenPaused onlyDevOrOwner {
     ...
     * @dev Callable by dev
     */
    function setOracleUpdateAllowance(uint256 _oracleUpdateAllowance) external whenPaused onlyDevOrOwner {
        oracleUpdateAllowance = _oracleUpdateAllowance;
    ...
    function _betBear(
      ...
      uint256 originalAmount
        ...
        require(ledger[epoch][account].amount == 0, "Can only bet once per round");
        ...
        // Update user data
        BetInfo storage betInfo = ledger[epoch][account];
        betInfo.position = Position.Bear;
        betInfo.amount = amount;
        userRounds[account].push(epoch);

        emit BetBear(account, epoch, amount, originalToken, originalAmount);
    }
    ...
    function _betBull(
      ...
      uint256 originalAmount
        ...
        emit BetBull(account, epoch, amount, originalToken, originalAmount);
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (round.closePrice > round.lockPrice) {
            ...
            treasuryAmt = (round.totalAmount * treasuryFee) / FEE_DIVIDER;
