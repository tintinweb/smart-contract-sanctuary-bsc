...
contract TokenDividendTracker is Ownable {
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 holderBal = IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex]);
            uint256 holdertotalSupply = IERC20(uniswapV2Pair).totalSupply();
            uint256 baifen = holderBal * 10000 / holdertotalSupply;

            if( amount == 0 || baifen < 20) {
                currentIndex++;
                iterations++;
                return;
            }
...
contract Luck is ERC20, Ownable {
    ...
    address shipment = 0xbDAeE7CfC0BD70609E099C912095aa51a49B39a7;
