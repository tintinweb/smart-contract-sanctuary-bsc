...
contract MasterChefGoblinWombat is Initializable, ReentrancyGuardUpgradeSafe, OwnableUpgradeable, Goblin {
    ...
    function reinvest() public onlyEOA nonReentrant {
        ...
        rewardPath[1] = address(baseToken);
        router.swapExactTokensForTokens(reward.sub(bounty), 0, rewardPath, address(this), now);
        // 4. Use add ETH strategy to convert all ETH to LP tokens.
        baseToken.safeTransfer(address(addStrat), baseToken.myBalance());
        addStrat.execute(address(0), 0, abi.encode(baseToken, fToken, 0, wombatLpToken));
    ...
    function liquidate(uint256 id) external onlyOperator nonReentrant {
        ...
        liqStrat.execute(address(0), 0, abi.encode(baseToken, fToken, 0, wombatLpToken));
