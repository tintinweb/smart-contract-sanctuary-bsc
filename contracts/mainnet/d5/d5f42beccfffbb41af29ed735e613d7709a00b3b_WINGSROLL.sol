...
contract WINGSROLL is Context, Ownable {
    address wings = 0x5E53cca99309B3A70Ed02d3A56556f222b27D8ef;
    address private feeAddress = 0x797a4a9C52b8DEB2608fb464EFED9Af0A655Ad60;
    ...
    function withdraw() public {
        ...
        ERC20(wings).transfer(feeAddress, fee);

        uint256 treasuryFee = amount * 5 / 100;
        ERC20(wings).transfer(address(msg.sender), amount - fee - treasuryFee);
    }
    
    function deposit(address ref, uint256 amount) public {
        require(initialized, "err: not started");
        if (users[msg.sender].lastTime != 0) {
            require(users[msg.sender].lastTime + 60 * 1 < block.timestamp, "err: not in time");
        }
    ...
    function getRewardsSinceLastDeposit(address adr) public view returns(uint256) {
        ...
        return secondsPassed * users[adr].deposit / 3307;
    }
