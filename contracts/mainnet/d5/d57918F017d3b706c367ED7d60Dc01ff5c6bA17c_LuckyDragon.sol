...
contract LuckyDragon is ERC20, Ownable {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
       ...
       swapWethForRewardToken(ethAmount);
        uint256 tokenBamount = (IERC20(BUSD).balanceOf(address(this)));
        
        _approve(address(BUSD), address(uniswapV2Router), tokenAmount);
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        // add the liquidity
    ...
    function swapBack() private {
        ...
        if(dividendReward){
            success = IERC20(token).transfer(stakeContract, tokenBalance);
            IStake(stakeContract).dividendReward(tokenBalance);
        }else{
    ...
    function updatePairToken(address _newPairToken) external onlyOwner {
        BUSD = _newPairToken;
    }
}