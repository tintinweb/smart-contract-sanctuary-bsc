...
contract Liquidifier is AccessControl, ReentrancyGuard, Operations {
    ...
    uint256 public feedapp = 500; //Equal 5%
    IUniswap public dex;
        ...
        returns (uint256 lp)
    {
        ...
        if (!userinvestment[msg.sender][LP].farming) {
    ...
    function makeaSwapETH(
        ...
        (uint256 reserve1, uint256 reserve2) = Operations.reserves(_lp);
        //(uint256 _minamount, uint256 diference) = Operations.minamount(amountA, amountB);
...
contract Operations is AccessControl, ReentrancyGuard {
    ...
    function minamount(uint256 amountA, uint256 amountB)
        internal
        pure
        returns (uint256 _minamount, uint256 diference)
    {
        diference = amountA - amountB;
        //el 60 % de la diferencia como amount minimo
       /* _minamount = (diference * 6000) / 10000;
        return (_minamount, diference);*/
    }
