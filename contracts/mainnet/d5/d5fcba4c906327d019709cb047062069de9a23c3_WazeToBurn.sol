...
/**
Introducing a new meta called “WazeToBurn” based off the well known and loved application “Waze To Earn.” 

$WAZE will launch the new meta “Waze To Burn” that will launch with a brand new innovative utility contract and will be tradable through the Binance Smart Chain Network (BEP20).
 All you have to do is jump in and help us spread the word. We look forward to this exciting launch and hope you and the rest of the cryptocurrency space will come and enjoy this journey with us. 

We welcome the entire defi space to come and join our “Waze To Burn”.. Stay tuned! Cheers! 

Telegram:
https://t.me/WazeToBurn
 
Twitter:
https://twitter.com/wazetoburn?s=21&t=h0S7wR0tRt2uA0-D6bxIEQ

基于广为人知和喜爱的应用程序“Waze To Earn”引入一个名为“Waze To Burn”的新元。可通过币安智能链网络（BEP20）进行交易。您所要做的就是加入并帮助我们传播信息。我们期待这次激动人心的发布，并希望您和加密货币领域的其他人能来和我们一起享受这段旅程。我们欢迎整个 defi 空间加入我们的“Waze To Burn”。敬请期待！干杯!
*/
...
contract WazeToBurn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    WazeToBurnDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WazeToBurn", "WAZE") {
        marketingWallet = payable(0x7042C6aF4Aa1eDF5e45dfF021c133787c1197667);
        devWallet = payable(0x0D8eCC8bc901eD879167e6dfd96695D605af4Fe5);
        ...
        dividendTracker = new WazeToBurnDividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
                                                   "WazeToBurnTRACKER", "WAZETRACKER");
...
contract WazeToBurnDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    WazeToBurn public WazeToBurnContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        WazeToBurnContract = WazeToBurn(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main WazeToBurn contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WazeToBurnContract);

            uint256 prevBalance = WazeToBurnContract.balanceOf(address(this));
            ...
            {
                uint256 received = WazeToBurnContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WazeToBurnContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
