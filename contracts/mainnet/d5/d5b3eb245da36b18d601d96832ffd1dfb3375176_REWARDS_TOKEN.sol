...
contract REWARDS_TOKEN is Context, IERC20 { 
    ...
    */
    ...
    function Contract_SetUp_03__Wallet_Limits(
        ...
        // Buyer protection Limits: Min: 0.5% of Supply, Max: 3% of Supply
        require(Max_Tokens_Each_Transaction >= _tTotal / 200 / 10**_decimals, "E05");
        require(Max_Total_Tokens_Per_Wallet >= _tTotal / 200 / 10**_decimals, "E06");

        require(Max_Tokens_Each_Transaction <= _tTotal * 3 / 100 / 10**_decimals, "E07");
        require(Max_Total_Tokens_Per_Wallet <= _tTotal * 3 / 100 / 10**_decimals, "E08");
    ...
    */
    ...
    function Rewards__Distribution_Triggers(uint256 Minutes_Between_Payments, uint256 Required_Reward_Balance) external onlyOwner {
        ...
        // Max Wait is 2 Hours
        require(Minutes_Between_Payments <= 120,"E10");

        // Max Required Balance for Payout is $2.00
        require(Required_Reward_Balance <= 2_000_000_000_000_000_000,"E11");
    ...
    */
    ...
    function Project__Update_Wallets(
        ...
        require(Token_Fee_Wallet != address(0), "E12");
        Wallet_Tokens = Token_Fee_Wallet;

        // Update LP Collection Wallet
        require(BNB_Fee_Wallet != address(0), "E13");
    ...
    */
    ...
    function Maintenance__Transfer_Ownership(address payable newOwner) public onlyOwner {
        require(newOwner != address(0), "E16");
    ...
    */
    ...
    function Processing__Remove_Random_Tokens(
        ...
        uint256 number_of_Tokens
            ...
            require (random_Token_Address != address(this), "E18");
    ...
    */
    ...
    function _transfer(
        ...
        if (!Trade_Open){
        require(_isWhitelisted[from] || _isWhitelisted[to], "E19");
        }
        ...
        if (LaunchMode) {
            ...
            if (block.timestamp > LaunchTime + (1 * 1 hours)){
                ...
                // Stop Snipers
                require(!_isSnipe[to], "E20");


                // Tag and Restrict Snipers
                if (_isPair[from] && block.timestamp <= LaunchTime + 3) {

                    require(amount <= max_Hold / 10, "E21");
                    _isSnipe[to] = true;

                } 

                // Limit Max Holding During Launch
                if (_isPair[from] && !_isLimitExempt[to]) {
                    ...
                    if (block.timestamp < LaunchTime + (15 * 1 minutes)) {
                        ...
                        require((heldTokens + amount) <= max_Hold / 4, "E22");

                        } else if (block.timestamp < LaunchTime + (30 * 1 minutes)) {

                        // Limit to 50% of max hold for first 30 mins
                        require((heldTokens + amount) <= max_Hold / 2, "E23");
...
---------------------
    ...
    E02 - Buy fees are limited of 15% max to protect buyers (includes 1% dev fee if applicable)
    E03 - Sell fees are limited of 15% max to protect buyers (includes 1% dev fee if applicable)
    E05 - The max transaction limit must be set to more than 0.5% of total supply
    E06 - The max wallet holding limit must be set to more than 0.5% of total supply
    E07 - The max wallet holding limit must be set to less than 3% of total supply
    E08 - The max wallet holding limit must be set to less than 3% of total supply
    E09 - Blacklisting of wallets only permitted during launch phase (Max 1 hour)
    E10 - Max time to wait between reward distribution is 2 hours (120 mins)
    E11 - Max required reward before payout is $2
    E12 - A valid BSC wallet must be entered when updating the token fee wallet 
    E13 - A valid BSC wallet must be entered when updating the Cake LP Collection wallet 
    E14 - A valid BSC wallet must be entered when updating the BNB fee wallet 
    E15 - A fee of 2 BNB is required, enter the number 2 into the field and try again
    E16 - A valid BSC wallet must be entered when transferring ownership
    E17 - Contract is currently processing fees, try later
    E18 - Can not remove the native token
    E19 - Trade is not open, only whitelisted wallets can trade
    E20 - Wallet was flagged as a sniper bot and can not purchase again during launch phase
    E21 - Trying to buy over the launch block max transaction limit
    E22 - Max hold limited to 25% during first 15 mins
    E23 - Max hold limited to 50% during first 30 mins
