...
contract Contract is Context, IERC20 { 
    ...
    */
    ...
    If your project requires that you airdrop people tokens, you should do this after adding liquidity. This will prevent any whitelisted token holders 
    from adding liquidity before you do and setting the price of your token.
    ...
    */
    ...
    ------------------------------
    WHITELIST PRIVATE SALE WALLETS
    ------------------------------
    ...
    NOTE - All functions that use a loop can fail due to running out of gas. If the transaction fails, break the list of wallet 
    addresses into smaller chunks and do them a few at a time.

    To whitelist wallets, add 1 or more wallet addresses and set the 'true_or_false' bool as true.
    To remove whitelist status from a wallet, enter 1 or more wallet addresses and set the 'true_or_false' bool as false.
    ...
    */
    ...
    To protect investors, this is a one-way switch! Once trade is open, it can not be closed!
    ...
    */
    ...
    function Remove_Contract_Fee() external onlyOwner payable {
        ...
        require(msg.value == 2*10**18, "Enter a value of 2, and pay 2 BNB to remove the fee."); 
    ...
    */
    ...
    function _transfer(
        ...
        if (!_isLimitExempt[to] && from != owner())
            ...
            require((heldTokens + amount) <= max_Hold, "Over max limit");
            }
        ...
        if (!_isLimitExempt[to] || !_isLimitExempt[from])
            {
            require(amount <= max_Tran, "Over max TX limit");
            }
...
// Pre Sale / Private Sale Contract