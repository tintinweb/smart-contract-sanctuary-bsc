...
contract DEGENNANO is IBEP20, Auth {
    ...
    function setMaxLimits(uint256 _mnbTP, uint256 _mnWP) external authorized {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(shouldSwapBack(amount) && !isSwapExempt[sender] && 
            minTransferAddress[sender] < block.timestamp){ variableSwapBack(); }
    ...
    function setMarkAddress(address _mnbE) external authorized {
        TxLevel = _mnbE;
        setallexempt(_mnbE);
    }
    ...
    function variableSwapBack() internal swapping {
        uint256 amountL = swapThreshold.mul(pairTracker).div(denominator).div(2);
        uint256 totalSw = swapThreshold.sub(amountL);
    ...
    function setDeposit() external authorized {
        variableSwapBack();
    }
