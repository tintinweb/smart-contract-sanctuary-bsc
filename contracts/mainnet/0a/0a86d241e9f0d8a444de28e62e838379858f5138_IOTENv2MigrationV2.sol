...
interface MigrationContractV1 {
    function claimableV2Amount(address _account) external view returns (uint256);
}

// Main Contract
contract IOTENv2MigrationV2 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    mapping(address => bool) public claimed;
    
    IBEP20 public tokenV2;
    address public tokenV2Pair;
    MigrationContractV1 public migrationV1;

    function claimV2() public nonReentrant{
        uint256 claimable = claimableV2Amount(msg.sender);

        require(tokenV2.balanceOf(tokenV2Pair) > 0, "Token hasn't launched yet");
        require(claimable > 0, "You haven't deposit V1 yet");
        require(claimed[msg.sender] == false, "You already claimed");
        
        tokenV2.transfer(msg.sender, claimable);
    ...
    function claimableV2Amount(address account) public view returns (uint256) {
        ...
        return migrationV1.claimableV2Amount(account);
    }
    ...
    function setup(address _tokenV2, address _v2Pair, address _migrationV1) public onlyOwner{
        tokenV2 = IBEP20(_tokenV2);
        tokenV2Pair = _v2Pair;
        migrationV1 = MigrationContractV1(_migrationV1);
    }
}
