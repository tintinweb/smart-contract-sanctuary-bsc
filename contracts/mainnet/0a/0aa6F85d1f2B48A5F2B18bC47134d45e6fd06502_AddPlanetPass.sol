...
contract AddPlanetPass is AccessControl, Pausable {

    bytes32 public constant ADMIN = keccak256("ADMIN");

    IPlanetPass public planetPass;

    constructor (IPlanetPass _planetPass) {
        planetPass = _planetPass;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function setPlanetPass(address[] calldata _to, uint[] calldata _planets, uint _timeFromBuy, uint _digCount) external onlyRole(ADMIN) whenNotPaused {
        uint passType = getPassType(_timeFromBuy, _digCount);
        uint expirationTime = block.timestamp + _timeFromBuy; 
        for (uint256 i = 0; i < _to.length; i++) {
            planetPass.addPlanetPassBatch(_to[i], _planets, passType, expirationTime, _digCount);
        }
    }

    function getPassType(uint _timeFromBuy, uint _digCount) internal pure returns (uint) {
        if(_timeFromBuy > 0 && _digCount > 0) {
            return 1;
        } else if (_digCount > 0) {
            return 2;
        } else if(_timeFromBuy > 0) {
            return 3;
        } else {
            return 4;
        }
    ...
    function pauseHandler() external onlyRole(DEFAULT_ADMIN_ROLE) {
        _pause();
    }

    function unpauseHandler() external onlyRole(DEFAULT_ADMIN_ROLE) {
        _unpause();
    }
}
