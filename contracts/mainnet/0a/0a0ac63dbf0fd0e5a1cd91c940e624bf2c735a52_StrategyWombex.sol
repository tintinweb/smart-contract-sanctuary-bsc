...
contract StrategyWombex is StratManager, FeeManager {
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 toNative1 = IERC20(output1).balanceOf(address(this)).mul(100).div(1000);
        uint256 toNative2 = IERC20(output2).balanceOf(address(this)).mul(100).div(1000);
        if(toNative1 > 0) {
        ...
        uint256 callFeeAmount = nativeBal.mul(100).div(1000);
        IERC20(native).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 beefyFeeAmount = nativeBal.mul(300).div(1000);
        IERC20(native).safeTransfer(beefyFeeRecipient, beefyFeeAmount);
        uint256 strategistFeeAmount = IERC20(native).balanceOf(address(this));
    ...
    function rewardsAvailable() public view returns (uint256, uint256) {
        ...
        return (unharvestedBal1, getMintedWmxAmount(unharvestedBal2));
    }
    ...
    function callReward() public view returns (uint256) {
        ...
        return nativeOut.mul(100).div(1000).mul(100).div(1000);
    }
