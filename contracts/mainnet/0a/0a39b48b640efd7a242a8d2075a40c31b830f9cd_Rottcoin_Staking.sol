...
contract Rottcoin_Staking is Ownable {
    ...
    // Fees address...
    address public devAddress = 0x639f72d9ce1f2bcC2024eAb4Ba99fa3b98C430AF;
    
    // Rottcoin token contract...
    address public RottToken = 0x1CC41b493EEcc0CE084776a7a4BDdfF50D77EC91;
    ...
    function withdraw(uint256 amountToWithdraw) public {
        ...
        require(_lastClaimedTime > cliffTime, "You recently staked, please wait before withdrawing.");
        
        if (timeperiod[msg.sender] == 7){
        if (_lastClaimedTime >= MinimumWithdrawTime) {
            require(Token(RottToken).transfer(msg.sender, amountToWithdraw), "Could not transfer tokens.");
        }
        
        if (_lastClaimedTime < MinimumWithdrawTime) {
            uint256 WithdrawFee = amountToWithdraw.div(1e2).mul(15);
            uint256 amountAfterFee = amountToWithdraw.sub(WithdrawFee);
            require(Token(RottToken).transfer(msg.sender, amountAfterFee), "Could not transfer tokens.");
            require(Token(RottToken).transfer(devAddress, WithdrawFee), "Could not transfer tokens.");
        }}
    ...
    function transferAnyERC20Tokens(address _tokenAddress, address _to, uint256 _amount) public onlyOwner {
        require(_tokenAddress != RottToken, "You can't transfer ROTTCOIN token.");
