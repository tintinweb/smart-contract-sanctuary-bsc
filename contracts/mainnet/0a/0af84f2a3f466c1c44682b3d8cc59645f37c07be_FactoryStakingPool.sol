...
contract StakePoolTokenForBNB is States, Ownable {
    ...
    function setTokens(uint256 _apy, uint256 _earlyUstakeFee, IBEP20 _stakingToken, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner {
        apyTier = _apy;
    ...
    function setData(uint256 _apy, uint256 _days, address _rewardingTokenWallet, uint256 _maxReward, uint256 _earlyUstakeFee, uint256 _earlyRewardUstakeFee) public payable onlyOwner {
        apyTier = _apy;
        lockPeriod = _days;
        rewardingTokenWallet = _rewardingTokenWallet;
        earlyUstakeFee = _earlyUstakeFee;
    ...
    function _claimBNB(address _address) internal {
        ...
        if(claimcurrentReward <= address(this).balance){
          payable(_address).transfer(claimcurrentReward);
        }
        else{
        ...
        emit Claim(_msgSender() , claimcurrentReward);
    }
...
contract FactoryStakingPool is Ownable{
    ...
    function createTokenForBNB(address ownerAddress, uint256 _apy, uint256 _earlyUstakeFee, IBEP20 _stakingToken, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolTokenForBNB toDeploy = new StakePoolTokenForBNB();
        toDeploy.setTokens(_apy, _earlyUstakeFee, _stakingToken, _rewardingTokenWallet, _lockPeriod, _maxReward, _earlyRewardUstakeFee, _autoCompund, _earlyUstake);
