...
/**
 * @title pea price Oracle
 *      This Oracle calculates the average USD price of pea based on the BNB-pea and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private peaBnbPrice0CumulativeLast;
    uint private peaBnbPrice1CumulativeLast;
    uint32 private peaBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _pea_bnb;
    ...
    constructor(
        address __pea_bnb,   // Address of the pea-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _pea_bnb = IUniswapV2Pair(__pea_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        peaBnbPrice0CumulativeLast = _pea_bnb.price0CumulativeLast();
        peaBnbPrice1CumulativeLast = _pea_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, peaBnbBlockTimestampLast) = _pea_bnb.getReserves();
    ...
    // Get the average price of 1 pea in the smallest BNB unit (18 decimals)
    function getpeaBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_pea_bnb));

        require(_blockTimestamp != peaBnbBlockTimestampLast, "pea Last and current are equal");

        FixedPoint.uq112x112 memory peaBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - peaBnbPrice0CumulativeLast) / (_blockTimestamp - peaBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, peaBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (peaBnbPrice0CumulativeLast, peaBnbPrice1CumulativeLast, peaBnbBlockTimestampLast, peaBnbAverage) = getpeaBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, peaBnbAverage) = getpeaBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = peaBnbAverage*(1e6) / busdBnbAverage;
