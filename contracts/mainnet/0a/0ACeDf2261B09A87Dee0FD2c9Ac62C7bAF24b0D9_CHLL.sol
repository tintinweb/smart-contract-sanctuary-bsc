...
contract CHLL {
  ...
  uint256 constant private REFERRAL_COMMISSION = 30; 
	...
	uint256 constant private INCREMENT = 1e8;
    ...
    uint256 _nonReferralRewards = _rewards - info.users[msg.sender].referralEarnings;
    info.users[msg.sender].referralEarnings = 0;
    info.users[msg.sender].scaledPayout += int256(_nonReferralRewards * BIG_NUMBER);
		info.busd.transfer(msg.sender, _rewards);
		emit Withdraw(msg.sender, _rewards);
		return _rewards;
	}

	function restake(address _referrer) external returns (uint256) {
		uint256 _rewards = rewardsOf(msg.sender);
		require(_rewards > 0);
    info.users[msg.sender].referralEarned += info.users[msg.sender].referralEarnings;
    info.users[msg.sender].referralEarnings = 0;
		info.users[msg.sender].scaledPayout += int256(_rewards * BIG_NUMBER);
		return _restake(_rewards, _referrer);
	}
  ...
  function totalRewards() public view returns (uint256) {
    ...
    if (uint256(info.users[_user].referralEarnings) > 0){
      ...
      return uint256(int256(info.scaledBUSDPerToken * balanceOf(_user)) - info.users[_user].scaledPayout) / BIG_NUMBER;
    }
	}
  ...
  function allRewardsInfoFor(address _user) public view returns (uint256 totalRewardsEarned, uint256 userRewards,uint256 userRewardsEarned, uint256 userReferralEarnings, uint256 userReferralEarned) {
	...
	function _restake(uint256 _amount, address _referrer) internal returns (uint256 tokens) {
    require(msg.sender != _referrer);
