...
contract GankPresale is Auth {
    IERC20Upgradeable public gankToken;
    ...
    function initialize(address _owner, IERC20Upgradeable _gankToken)
        public
        initializer
    {
        __Auth_init(msg.sender);

        gankToken = _gankToken;

        isClaimable = false;

        softCapEthAmount = 300 ether;
        hardCapEthAmount = 560 ether;

        minimumDepositEthAmount = 0.5 ether;
        maximumDepositEthAmount = 2 ether;
    ...
    function setToken(address _address) external onlyOwner {
        ...
        gankToken = IERC20Upgradeable(_address);
    }
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < userClaimableTokens.length; i++) {
            if (userClaimableTokens[i].lastClaimedDate == 0) {
                tokenAmount += userClaimableTokens[i].tokenAmount / 2;
            ...
            if (
                ...
                tokenAmount +=
                    numDurationsPassed *
                    ((userClaimableTokens[i].tokenAmount / 2) / vestingPeriods);
    ...
    function claim() external {
        ...
        gankToken.transfer(msg.sender, tokenAmount);
    ...
    function releaseTokens() external onlyOwner {
        gankToken.transfer(msg.sender, gankToken.balanceOf(address(this)));
    }
