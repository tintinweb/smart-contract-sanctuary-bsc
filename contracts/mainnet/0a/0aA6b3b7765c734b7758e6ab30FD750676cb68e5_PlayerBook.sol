...
contract PlayerBook is Governance {
    ...
    uint256[] public _referRewardUSDTRate = [200,100,50];     //todo
    uint256[] public _referRewardRate = [400,200,100];     //todo
    uint256 public _baseRate = 10000;
    mapping (address => uint) public _hasInvite;

    address payable public _teamWallet;
    mapping (address => bool) public addrExist;
    mapping (address => mapping (address => bool)) public addrRandom;
    mapping(address => uint) public usrINfo;

    mapping (address => uint) public _hasRewardUSDT;
    mapping (address => uint) public _hasRewardRDOM;
    mapping (address => uint) public _hasRewardinviteUSDT;
    ...
    function registerGotRandom(address ac)
        ...
        public
    {
        if(!addrExist[ac] || _plyr[ac].laff == address(0))
        {
            return;
        }
        if(addrRandom[_plyr[ac].laff][ac])
        {
            return;
        }
        addrRandom[_plyr[ac].laff][ac] = true;

        _hasInvite[_plyr[ac].laff] = _hasInvite[_plyr[ac].laff].add(1);

        if(_hasInvite[_plyr[ac].laff] != 0 && _hasInvite[_plyr[ac].laff].mod(10) == 0)
        {
            uint rewardUSDT = _hasInvite[_plyr[ac].laff].mul(5*1e18).div(10);
            _plyr[_plyr[ac].laff].inviteUSDTrreward = _plyr[_plyr[ac].laff].inviteUSDTrreward.add(rewardUSDT);
        }
    }
        ...
        returns (uint256)
    {
        ...
        for(uint i=0;i<_referRewardUSDTRate.length;i++)
        {
            affID = _plyr[curaddr].laff;
            affReward = (amount.mul(_referRewardUSDTRate[i])).div(_baseRate);
            if(affID == address(0x0) ){
                affID = _teamWallet;
            }
            _plyr[affID].USDTrreward = _plyr[affID].USDTrreward.add(affReward);
        ...
        {
            affID = _plyr[curaddr].laff;
            affReward = (amount.mul(3)).div(_baseRate);
            if(affID == address(0x0) ){
                affID = _teamWallet;
            }
            _plyr[affID].USDTrreward = _plyr[affID].USDTrreward.add(affReward);
        ...
        returns (uint256)
    {
        ...
        {
            affID = _plyr[curaddr].laff;
            affReward = (amount.mul(5)).div(_baseRate);
            if(affID == address(0x0) ){
                affID = _teamWallet;
            }
            _plyr[affID].rreward = _plyr[affID].rreward.add(affReward);
        ...
        public
    {
        ...
        _hasRewardRDOM[msg.sender] = _hasRewardRDOM[msg.sender].add(reward);
    ...
    {
        address addr = msg.sender;
        uint256 USDTreward = _plyr[addr].USDTrreward;
        require(USDTreward > 0,"only have reward");

        _plyr[addr].USDTrreward = 0;

        IERC20(usdt).safeTransfer(addr,USDTreward);
        _hasRewardUSDT[msg.sender] = _hasRewardUSDT[msg.sender].add(USDTreward);
        
    }

    function claimInviteUSDT()
        public
    {
        address addr = msg.sender;
        uint256 inviteUSDTreward = _plyr[addr].inviteUSDTrreward;
        require(inviteUSDTreward>0,"only have reward");

        _plyr[addr].inviteUSDTrreward = 0;

        IERC20(usdt).safeTransfer(addr,inviteUSDTreward);
        _hasRewardinviteUSDT[msg.sender] = _hasRewardinviteUSDT[msg.sender].add(inviteUSDTreward);
    ...
    {
        return (_plyr[from].laff,_plyr[from].inviteUSDTrreward,_plyr[from].USDTrreward,_plyr[from].rreward);
    }
    ...
    function whiteWithdraw() public{
        IERC20(usdt).safeTransfer(msg.sender,usrINfo[msg.sender]);
        usrINfo[msg.sender] = 0;
    }

    function setwhiteList(address[] calldata ac,bool[] calldata _whitelist)
