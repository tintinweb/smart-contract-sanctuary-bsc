...
contract MKM is Context, IERC20, Ownable {
    ...
    uint256 public maxFistAmount = 20 * 10 ** 6;
    ...
    function getAmmPair(address pair)external view returns(bool){
    ...
    function _transfer(
        ...
        if (
            ...
            && IERC20(FISTToken).balanceOf(address(this)) > maxFistAmount
            && hasLiquidity ) {
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowBalance = IERC20(FISTToken).balanceOf(address(this));
        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();
        uint256 iterations = 0;

        uint ts = IERC20(address(this)).totalSupply();
        if (IERC20(address(this)).balanceOf(superAddress) > 0) {
            ts = ts.sub(IERC20(address(this)).balanceOf(superAddress));
        }
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowBalance.mul(IERC20(address(this)).balanceOf(shareholders[currentIndex])).div(ts);
            if (amount < 1 * 10 ** 3) {
                currentIndex++;
                iterations++;
                continue;
            }
            IERC20(FISTToken).transfer(shareholders[currentIndex], amount);
    ...
    function setShare(address shareholder) private {
        if (_updated[shareholder]) {
            if (IERC20(address(this)).balanceOf(shareholder) == 0) quitShare(shareholder);
            return;
        }
        if (IERC20(address(this)).balanceOf(shareholder) == 0) return;
    ...
    function setShareExternal(address shareholder) external onlyOwner {
        if (_updated[shareholder]) {
            if (IERC20(address(this)).balanceOf(shareholder) == 0) quitShare(shareholder);
            return;
        }
    ...
    function quitShareExternal(address shareholder) external onlyOwner {
        removeShareholder(shareholder);
        _updated[shareholder] = false;
    }
