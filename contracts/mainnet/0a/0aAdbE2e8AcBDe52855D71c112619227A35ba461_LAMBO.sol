...
contract LAMBO is IERC20Metadata, Ownable, _LAMBO {
    ...
    uint256 public constant lamboInsuranceFundFee = 50;
    ...
    bool public _autoBuyBNBAndSHIB = false;
    ...
    constructor() IERC20Metadata("Lambo", "LAMBO", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x214d183FE270A4d151182cE4900147B8DD09Abed;
        treasuryReceiver = 0xD7058c207cF40Eb301715AB23D7dD8C03F5eDbed;
        lamboInsuranceFundReceiver = 0xAeE701525471A14e63615bD32b14bB4A55598bed;
        firePit = DEAD;
    ...
    function setClaimContract(address _contract) external onlyOwner {
        ...
        _allowedFragments[address(claimContract)][
            address(router)
        ] = MAX_UINT256;
    ...
    function swapReward() internal {
        uint256 lastRewardEpoch = claimContract.getLastRewardEpoch();
        if (currentRewardsEpoch() > lastRewardEpoch + 1) {
            claimContract.swapReward();
        }
    }

    function setAutoReward(bool _flag) external onlyOwner {
        _autoBuyBNBAndSHIB = _flag;
    }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(lamboInsuranceFundFee)).div(
                feeDenominator
            )
        );
        ...
        ].add(gonAmount.mul(autoBNBFee.add(autoSHIBFee)).div(feeDenominator));

        claimContract.addFee(
            currentRewardsEpoch(),
            gonAmount.mul(autoBNBFee.add(autoSHIBFee)).div(feeDenominator)
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(lamboInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(lamboInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(lamboInsuranceFundFee).div(
                treasuryFee.add(lamboInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no Lambo token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
