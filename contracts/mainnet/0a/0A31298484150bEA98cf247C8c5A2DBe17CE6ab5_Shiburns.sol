...
hyper deflationary burn mechanism with SHIB rewards
...
contract Shiburns is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ShiburnsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Shiburns", "SHBRNS") {
        marketingWallet = payable(0x83d803796E9F01BD3042CbB3CdF4cF111f369184); 
        ...
        dividendTracker = new ShiburnsDividendTracker(
            payable(this),
            router,
            0x2859e4544C4bB03966803b044A93563Bd2D0DD4D, //bSHIB
            "ShiburnsTRACKER",
            "SHBRNSTRACKER"
        );
        ...
        if (amount == 0) {
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                if (swapAndLiquifyEnabled && liquidityFee > 0 && totalBuyFees > 0) {
                    ...
                    if (buyAmount > 0){
                    swapAmountBought = contractTokenBalance
                        .mul(buyAmount)
                        .div(totalBuySell);
                    ...
                    if (sellAmount > 0) {
                    swapAmountSold = contractTokenBalance
                        .mul(sellAmount)
                        .div(totalBuySell);
...
contract ShiburnsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Shiburns public ShiburnsContract;
    ...
    constructor(
        ...
        defaultToken = token;
        ShiburnsContract = Shiburns(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Shiburns_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Shiburns_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Shiburns contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(ShiburnsContract);

            uint256 prevBalance = ShiburnsContract.balanceOf(address(this));
            ...
            {
                uint256 received = ShiburnsContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    ShiburnsContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
