...
contract DaoToken is ERC20Burnable, Ownable, Pausable {
    using SafeMath for uint256;
    mapping(address => bool) public minter;
    IFifaSportDao public dao;

    modifier onlyMinter() {
        require(_msgSender() == owner() || minter[_msgSender()], 'only accept by minter');
        _;
    }

    constructor(address _dao) ERC20('FiFaSportDAO', 'FFSD') {
        dao = IFifaSportDao(_dao);
    }
    ...
    function setDao(address _address) public onlyOwner {
        dao = IFifaSportDao(_address);
    }
