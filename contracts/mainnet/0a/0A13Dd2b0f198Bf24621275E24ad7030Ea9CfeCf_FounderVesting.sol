...
contract FounderVesting is ReentrancyGuard, Ownable {
    ...
    function setFounderAndAllocation(address _founder, uint _amount) external onlyOwner {
        ...
        vestToBeneficiary[index] = _founder;
        beneficiaryVestedCount[_founder]++;
    }
    ...
    function release(uint _index, uint _period) external nonReentrant {
        ...
        uint amount = _amountByPeriod(_index);
    ...
    function _amountByPeriod(uint _index) internal view returns (uint tokenAmount) {
        Vest storage vest = vests[_index];
        tokenAmount = vest.amount / settings.numberOfEmmissions;
    }
}
