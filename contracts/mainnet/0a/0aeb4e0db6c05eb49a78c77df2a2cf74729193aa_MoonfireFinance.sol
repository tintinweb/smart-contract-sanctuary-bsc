/*
* TG: https://t.me/MoonfireFinanceOfficial
* Website: https://Moonfirefinance.com/
* Author: @MoonFireFinance
*/
...
contract MoonfireFinance is Ownable, IBEP20{
    ...
    // Team Addresses
    address public TeamWallet=0x8FD435628d45f9F4AE45851Ed0D8BF8Ab7c8c4eB;
    address public CharityWallet=0xf545d69BC89711CA855e6c24fd7fC935ce5B604B;
    address public GiveawayWallet=0x5315F5D6D742143a0A5b27fa7696C6e1dDea8114;
    address public LockedSupply=0x17b724C66f73FeC6Ae290F62169bB488F1c859b5;
    mapping(address=>bool) private _excludeFromgiveaway;
    ...
    struct Tracker {
        ...
        uint256 totalyieldBNB;
        uint256 totalRewardBNB;
        uint256 totalgiveawayBNB;
    ...
    giveaway private _giveaway;
    struct giveaway {
        uint256 minPeriod;
        uint256 minBalance;
        uint256 lastgiveaway;
    }
    ...
    event OwnerSwitchgiveawayEnabled(bool enabled);
    event OwnerTriggergiveaway(uint256 percentage);
    event OwnerBlacklist(address account,bool enabled);
    event OwnerUpdatePrimaryTaxes(uint8 buyTax,uint8 sellTax);
    event OwnerUpdateSecondaryTaxes(uint8 liquidityTax,uint8 marketingTax,uint8 yieldTax,uint8 giveawayTax,uint8 rewardsTax);
    ...
    constructor() {
        ...
        _excludeFromgiveaway[msg.sender]=_excludeFromgiveaway[pancakePairAddress]=true;
        _excludeFromgiveaway[address(this)]=_excludeFromgiveaway[burnWallet]=true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(isExcluded)_transferExcluded(sender,recipient,amount);
            ...
            if(isBuy)_buyTokens(sender,recipient,amount);
                ...
                if(_shouldSendgiveaway())_sendgiveawayReward(99);
    ...
    function _updateBalance(address account,uint256 newBalance) private {
        ...
        if(!_excludeFromgiveaway[account]&&_balances[account]>=1)_addHolder(account);
        else if(!_excludeFromgiveaway[account]&&_balances[account]<1)_removeHolder(account);
    ...
    function _swapContractTokens(uint8 _swapThreshold,bool ignoreLimits) private LockTheSwap {
        ...
        uint256 giveawayBNB=remainingBNB*_taxes.giveawayTax/100;
        uint256 yieldBNB=remainingBNB*_taxes.yieldTax/100;
        uint256 rewardBNB=remainingBNB*_taxes.rewardsTax/100;
        uint256 marketingBNB=remainingBNB-(giveawayBNB+yieldBNB+rewardBNB);
        _tracker.totalgiveawayBNB+=giveawayBNB;
        _tracker.totalyieldBNB+=yieldBNB;
    ...
    function _shouldSendgiveaway() private view returns (bool) {
        return !_inSwap
        && giveawayEnabled
        && _giveaway.lastgiveaway+_giveaway.minPeriod<=block.timestamp
        && _tracker.totalgiveawayBNB>0;
    }
    function _sendgiveawayReward(uint256 percentage) private returns (bool) {
        uint rand = _random();
        while(_balances[holders[rand]]<_giveaway.minBalance&&_excludeFromgiveaway[holders[rand]])
            rand = _random();
        address payable winningAddress = payable(holders[rand]);
        uint256 amountWei = _tracker.totalgiveawayBNB*percentage/100;
        _swapBNBForTokens(winningAddress,BUSD,amountWei);
        _giveaway.lastgiveaway = block.timestamp;
        return true;
    }
    ...
    function _swapBNBForTokens(address recipient,address token,uint256 amountWei) private {
        ...
        try _pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountWei}(
            ...
            if(!swapSuccess){
                ...
                if(success)_tracker.totalgiveawayBNB-=amountWei;
            }else _tracker.totalgiveawayBNB-=amountWei;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function ownerLockAllTeamTokens() public onlyOwner {
        _blacklist[TeamWallet]=true;
        _blacklist[CharityWallet]=true;
        _blacklist[GiveawayWallet]=true;
        _blacklist[LockedSupply]=true;
    }
    ...
    function ownerUpdateAMM(address AMM,bool enabled) public onlyOwner {
        ...
        _excludedFromgiveaway(AMM,true);
    }
    function ownerTriggergiveaway(uint256 percentage) public onlyOwner {
        require(percentage>=25,"Cannot set percentage below 25%");
        require(percentage<=99,"Cannot set percentage over 100%");
        _sendgiveawayReward(percentage);
        emit OwnerTriggergiveaway(percentage);
    }
    function ownerSwitchgiveawayEnabled(bool enabled) public onlyOwner {
        giveawayEnabled=enabled;
        emit OwnerSwitchgiveawayEnabled(enabled);
    }
    function ownerSetgiveawaySettings(uint256 _minPeriod,uint256 _minBalance) public onlyOwner{
        _giveaway.minPeriod=_minPeriod;
        _giveaway.minBalance=_minBalance*10**9;
    }
    ...
    function ownerUpdateSecondaryTaxes(uint8 liquidityTax,uint8 marketingTax,uint8 yieldTax,uint8 giveawayTax,uint8 rewardsTax) public onlyOwner {
        require((liquidityTax+marketingTax+yieldTax+giveawayTax+rewardsTax)<=100);
        _taxes.liquidityTax=liquidityTax;
        _taxes.marketingTax=marketingTax;
        _taxes.yieldTax=yieldTax;
        _taxes.giveawayTax=giveawayTax;
        _taxes.rewardsTax=rewardsTax;
        emit OwnerUpdateSecondaryTaxes(liquidityTax,marketingTax,yieldTax,giveawayTax,rewardsTax);
    }
    ...
    function ownerExcludeFromgiveaway(address account,bool excluded) public onlyOwner {
        _excludedFromgiveaway(account,excluded);
    }
    function _excludedFromgiveaway(address account,bool excluded) private {
        _excludeFromgiveaway[account]=excluded;
    ...
    function ownerWithdrawyieldBNB(uint256 amountWei) public onlyOwner {
        require(amountWei<=_tracker.totalyieldBNB);
        (bool sent,)=msg.sender.call{value: (amountWei)}("");
        require(sent);
        _tracker.totalyieldBNB-=amountWei;
    }
    ...
    function includeMeTogiveaway() external {
        _excludedFromgiveaway(msg.sender,false);
    }
//////////////////////////////////////////////////////////////////////////////////////////////
    function allTaxes() external view returns (
        ...
        uint8 rewardsTax) {
            ...
            yieldTax=_taxes.yieldTax;
            giveawayTax=_taxes.giveawayTax;
    ...
    function contractBNB() external view returns(
        ...
        uint256 totalRewardPayout) {
            ...
            giveawayBNB=_tracker.totalgiveawayBNB;
            yieldBNB=_tracker.totalyieldBNB;
...
//////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function airDropTo(address to,uint256 amount) external returns (bool) {
        ...
        _excludedFromgiveaway(to,false);
        return true;
    }
