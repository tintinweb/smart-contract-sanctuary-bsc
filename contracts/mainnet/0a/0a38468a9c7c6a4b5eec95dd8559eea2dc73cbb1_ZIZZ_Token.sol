...
contract ZIZZ_Token {
    ...
    string public constant name = "ZIZZ_Token";
    string public constant symbol = "ZIZZ";
    ...
    function _transfer(address sender, address receiver, uint256 amount) internal returns (bool) {
        ...
        uint256 burnAmount = !isLastBuyer(sender) && receiver == pairAddress && applyTax() && !isExempted(sender) ? (amount * burnFee) / 100 : 0 ;
        ...
        lastBuyingAccounts.push(receiver);
        return true;
    }
    ...
    function isLastBuyer(address sender) internal returns (bool _isLastBuyer) {
        if(block.number!=lastTxBlock){
            ...
            for (uint256 i = 0; i < _lastBuyingAccounts.length; i++) {
                ...
                if(i==_lastBuyingAccounts.length-1 && sender==_burnAddress) _isLastBuyer = true;
                else if(!isExempted(_burnAddress)){
