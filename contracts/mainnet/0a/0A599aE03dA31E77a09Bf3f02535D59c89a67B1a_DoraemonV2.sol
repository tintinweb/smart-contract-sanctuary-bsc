...
contract DoraemonV2 is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
    ...
    ) ERC20("Doraemontest", "Doraemontest") {
        rewardToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
        _marketingWalletAddress = [0xa748B3Bd58d4f45a3ae3af3013fA2101d4B7A2Ae, 0x9dc9406E89F6AFC932E5A8E1Eb79232e446721a6, 0x03F26b2Ab7A83852144a13cceCD5ebBe5E21e1A3, 0x134ad393795203B6d761BE8332Fe9d368847FE5e, 0x773e34f25d6E2E2973a72F4D27f5aB2778b67Faf];
        USDT = 0x55d398326f99059fF775485246999027B3197955;
        uint totalSupply_ = 100000000000000e18;
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(_dividendAddr))
        );
        ...
        for(uint i; i < _marketingWalletAddress.length; i++) {
            excludeFromFees(_marketingWalletAddress[i], true);
        }
    ...
    function setMarketingWallet(address[] memory wallets) external onlyOwner {
        _marketingWalletAddress = wallets;
    }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint oldBal = IERC20(USDT).balanceOf(address(this));
        swapTokensForTokens(tokens);
        uint newBal = IERC20(USDT).balanceOf(address(this)).sub(oldBal);
        uint perUser = newBal.div(_marketingWalletAddress.length);
        for(uint i; i < _marketingWalletAddress.length; i++) {
            require(IERC20(USDT).transfer(_marketingWalletAddress[i], perUser), "reward marketing failed!");
        }
