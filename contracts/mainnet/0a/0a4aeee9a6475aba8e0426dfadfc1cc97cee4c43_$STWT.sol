...
// File: contracts/$STWTDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/$STWT.sol
pragma solidity ^0.7.6;
contract $STWT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    $STWTDividendTracker public dividendTracker;
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "$STWT: The dividend tracker already has that address");

        $STWTDividendTracker newDividendTracker = $STWTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$STWT: The new dividend tracker must be owned by the $STWT token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$STWT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "$STWT: Account is already the value of 'excluded'");
   ...
   function blacklistAddress(address account, bool excluded) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "$STWT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$STWT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$STWT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$STWT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "$STWT: This account cannot send tokens until trading is enabled");
        }
...
contract $STWTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("$STWT_Dividend_Tracker", "$STWT_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "$STWT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "$STWT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $STWT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "$STWT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "$STWT_Dividend_Tracker: Cannot update claimWait to same value");
