...
abstract contract ParentToken is ERC20, uniSwapTool {
    ...
    uint256 internal rewardSwapThreshold = 0.3 ether;
    ...
    function aliasReward() internal {
        ...
        if (amount > 200) handRewards(amount);
    }
...
contract ZCCY is Ownable, ERC20, ParentToken, ReentrancyGuard, System {
...
//    address returnAddress;
    ...
    constructor(address _router, address _reward, address[] memory addrs) ERC20("ZCCY Token", "ZCCY") {
        ...
        initIRouter(_router);
        initAddrs(addrs);
//        super._mint(liquidityAddress, 6666 ether);
...
//        dAddress = addrs[4];
    ...
    function handSwap() private returns (bool) {
        ...
        if (WETHAmount * amountDesire / TOKENAmount >= swapThreshold) {
            ...
            payable(backAddress).transfer(address(this).balance);
            return true;
        }
        return false;
    }
