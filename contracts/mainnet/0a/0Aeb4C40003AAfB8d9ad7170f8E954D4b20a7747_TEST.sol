...
contract TEST is Context, IERC20, Ownable {
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, uint256 _txFee,uint256 _lpFee,uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT,address routerAddress,address tokenOwner) public {
        ...
        _maxSwapAmount = _MAXAMOUNT * 10 ** _decimals;
    ...
    function setMaxSwapAmount(uint256 value) public onlyOwner {
        _maxSwapAmount = value * 10 ** _decimals;
    }

    function getMaxSwapAmount() public onlyOwner view returns(uint256) {
        return _maxSwapAmount;
    }

    function getIsJackpot() public onlyOwner view returns(bool) {
        return isJackpot;
    }
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && !isExcludedFromFee(from)) {
            if (isJackpot) {
                require(amount <= _maxSwapAmount, "Transfer amount exceeds the maxTxAmount.");
            }
        }
