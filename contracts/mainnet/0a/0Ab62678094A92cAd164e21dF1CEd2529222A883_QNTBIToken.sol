...
contract QNTBIToken is ERC20 {
    ...
    uint ratio;
    address public marketAddress = 0xb8E34190548e1065A851DFc0Aa8c6B4d50ed25F8;
	address payable _fundAddress = payable(address(0x05E833D1790e7bB9B34314D816Eb40752a950A2f));
    ...
    constructor() ERC20("QNTBI", "QNTBI") {
        ...
        uint256 total = 210000000*10**decimals();
	...
	function setSwapRatio(uint _ratio) public onlyOwner {
        ratio = _ratio;
    }
    ...
    function setFunderAddress(address payable _addr) external onlyOwner {
        _fundAddress = _addr;
    ...
    function swapAndLiquify(uint256 tAmount) internal lockTheSwap {
        ...
        uint256 tokensForSwap = tAmount.div(4).mul(3);
        ...
        if(amountReceived >0){
            uint256 fundAmount = amountReceived.div(2);
            uint ratioFee = fundAmount.div(10).mul(ratio);
            
            IERC20(_usdtAddress).transferFrom(address(_tokenDistributor),marketAddress, fundAmount);
            IERC20(_usdtAddress).transferFrom(address(_tokenDistributor),_fundAddress, ratioFee);
            IERC20(_usdtAddress).transferFrom(address(_tokenDistributor), address(uniswapV2Pair), amountReceived.sub(fundAmount).sub(ratioFee));
        }
