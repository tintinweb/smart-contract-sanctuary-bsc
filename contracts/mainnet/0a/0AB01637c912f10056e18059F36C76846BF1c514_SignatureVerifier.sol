...
contract SignatureVerifier is ISignatureVerifier, Ownable, ReentrancyGuard, Pausable {
    ...
    returns (bool) {
        address signer = keccak256(
            abi.encode(nonce, user_)
        ).ethSignedMessage().recover(signature);
        require(signer == TRUSTED_PARTY, "Invalid request");
        nonceUsed[nonce] = true;
        emit UsedNonce(tx.origin, nonce, "claim");
        return true;
    }
