...
contract BEP20HHB is Ownable, ERC20 {
    ...
    uint256[2] public sellFees = [7, 5]; //[7%LP,5%dead]
        ...
        ERC20(name_, symbol_)
    {
        ...
        _cast(owner(), 49_0000 * 10**decimals());
    }

    function _initSwap() private {
        // address usdt =  0xEdA5dA0050e21e9E34fadb1075986Af1370c7BDb;
        // address dex =  0xB6BA90af76D139AB3170c7df0139636dB6120F7e;
        address usdt =  0x55d398326f99059fF775485246999027B3197955;
        address dex =  0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function _transfer(
        ...
        if (!takeFee) {
            ...
            if (
                ...
                if (from != uniswapV2Pair && to != uniswapV2Pair) {} else {
                    if (to == uniswapV2Pair) {  //sell
                        aFees = _calcFees(amount, sellFees[0]); //7
                        bFees = _calcFees(amount, sellFees[1]); //5
                        rewardTotal += aFees;
                        allFees = aFees.add(bFees);
                        amount = amount.sub(allFees);
                        super._transfer(from, address(this), aFees);
                        super._transfer(from, deadAddr, bFees);
                    }
        ...
        fromAddress = from;
        toAddress = to;
        if(_balances[address(this)] >= (1 * 10**18) && from != address(this) && lpFeeShareTime.add(minPeriod) <= block.timestamp) {
    ...
    function _calcFees(uint256 amount, uint256 fee)
        private
        pure
        returns (uint256)
    {
