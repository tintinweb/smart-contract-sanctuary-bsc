...
contract TartarusInu  is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Tartarus Inu";
    string private constant _symbol = unicode"Tartarus";
    ...
    address payable public DevWallet = payable(0x722726ED83Fe9d474c754880D28446fE65a4Acdb);
    address payable public MarketingWallet = payable(0x81B5660A65e7c7482CB5c30B24eEb90D4e391399);
    address payable private feereceiver01 = payable(0x7F0b33FCD5F3D31C8bA9CaBFB0EDb77A2Fd0df29);
    address payable private feereceiver02 = payable(0x3F833a7030a910C5097885ebeA7f04454B165Bd2);
    address payable private feereceiver03 = payable(0x68EaAca0A3fcce80FB9886051D818ECF0a99221f);
    ...
    constructor () {
        ...
        _isExcludedFromFee[feereceiver01] = true;
        _isExcludedFromFee[feereceiver02] = true;
        _isExcludedFromFee[feereceiver03] = true;
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (tradingActiveBlock + 8 <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 10 <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + 9 <= block.timestamp) {idclaim();}
