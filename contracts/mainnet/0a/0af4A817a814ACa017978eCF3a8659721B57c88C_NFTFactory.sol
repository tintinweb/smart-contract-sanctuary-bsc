...
contract NFTFactory is Governance, INFTFactory {
    ...
    INFTSignature public _spyNftToken ;
    ...
    constructor(address spyNftToken) {
        _spyNftToken = INFTSignature(spyNftToken);
    }
    ...
    function setGegoContract(address gego) public onlyGovernance{
        _spyNftToken = INFTSignature(gego);
    }
    ...
    function mint(MintData memory mintData) public lock
    {
        ...
        _randomNonce++;
        INFTSignature.Gego memory gego = _ruleProxys[mintData.nftType].generate(msg.sender, mintData.ruleId, _randomNonce);
    ...
    function inject(uint256 tokenId, uint256 amount) external override returns (bool) {
        ...
        require(_spyNftToken.ownerOf(tokenId) == msg.sender, "not owner");
        ...
        (uint256 mintAmount,) = _ruleProxys[gego.nftType].inject(params, gego.amount);
