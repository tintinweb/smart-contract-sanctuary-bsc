...
contract ArbitrageBot is Ownable {
    ...
    mapping(address => bool) public OOtokenList;
    mapping(address => bool) public whitelist;
    mapping(address => bool) public admins;

		modifier onlyWhitelisted {
			require(
				whitelist[msg.sender] || admins[msg.sender] || owner() == msg.sender,
				"User is not whitelisted to use this contract"
			);
			_;
		}
		...
		constructor() {
			_OOApprove_ = 0x6352a56caadC4F1E25CD6c75970Fa768A3304e64;
		}
    ...
    function checkOOapprovedToken(address _token) external view returns (bool) {
			return OOtokenList[_token];
    }

    // Approve Arbitrage Bot
    function approveArbitrageBot(address _token) external onlyAdmin {
			require(tokenList[_token] == false, "Token Already Approved");
			IERC20(_token).safeApprove(address(this), 2**256 - 1);
			tokenList[_token] = true;
    }

    // Approve OpenOcean
    function approveMaxOpenOcean(address _token, uint256 _amount) external onlyAdmin {
			require(OOtokenList[_token] == false, "Token Already Approved");
			address OOAddress = _OOApprove_;
			uint256 allowance = IERC20(_token).allowance(address(this), OOAddress);
			if (allowance < _amount) {
				if (allowance > 0) {
					IERC20(_token).safeApprove(OOAddress, 0);
				}
				IERC20(_token).safeApprove(OOAddress, 2**256 - 1);
			}
			OOtokenList[_token] = true;
    }
    ...
    function multicallArbitrage(
			...
			uint256 _amount // inAmount with dicimals
    ) external payable onlyWhitelisted {
			require(_targets.length == _data.length, "Target length should be equal to Data length");
			uint256 fromAmount = _amount * 1e18;
			if (_fromToken != _BNB_ADDRESS_) {
				...
				require(fromAmount == msg.value, "Amount must be equal to msg.value");
			}
			for (uint256 i; i < _targets.length; i++) {
				(bool success, ) = _targets[i].call{
					value: _fromToken == _BNB_ADDRESS_ ? fromAmount : 0
				}(_data[i]);
				require(success, "Arbitrage failed");
			}

			uint256 returnAmount = balanceOf(_toToken, address(this));

			if (returnAmount < _amount) revert("Arbitrage failed because there was no profit");
			...
			returns (uint256)
    {
		...
		function setWhitelist(address _address, bool _access) public onlyOwner {
			whitelist[_address] = _access;
		}

		// Add new admin
		function setAdmin(address _address, bool _access) public onlyOwner {
			admins[_address] = _access;
		}
