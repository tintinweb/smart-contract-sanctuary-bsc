...
contract BlueRendering is Ownable {
    ...
    // function updateAddresses(address _pancakeAddr, address _BNB, address _BUSD, address _admin) public onlyOwner {
    //     pancakeRouterAddress = _pancakeAddr;
    //     addressBNB = _BNB;
    //     addressBUSD = _BUSD;
    //     addressAdmin = _admin;
    ...
    function depositBNB() public payable {
        ...
        Pool storage userPool = userPools[msg.sender];
    ...
    function withdrawlAvailablefundsforwithdrawal(uint256 _total) public {
        ...
        if (_to == meAddr) {
            ...
            Pool storage userPool = userPools[_to];
    ...
    function panicWithdrawBUSD() public {
        ...
        if (userAddrs.length > 1) {
            ...
            for (uint i = 0; i < userAddrs.length; i++) {
                ...
                Pool storage otherPool = userPools[userAddr];
    ...
    function convertAllBNBToBUSD() public onlyOwner { // Sell
        ...
        Pool storage adminPool = userPools[addressAdmin];
        ...
        for (uint i = 0; i < trades.length; i++) {
            Trade storage tradeInfo = trades[i];
        ...
        uint256 avgPrice = 0;
        if (buyBNB > 0) {
            avgPrice = buyBUSD * 1000000000000000000 / buyBNB;
        }
        ...
        for (uint i = 0; i < userAddrs.length; i++) {
            address userAddr = userAddrs[i];
            Pool storage userPool = userPools[userAddr];
        ...
        if (estimateAdmin == false) {
            Pool storage userPool = userPools[addressAdmin];
