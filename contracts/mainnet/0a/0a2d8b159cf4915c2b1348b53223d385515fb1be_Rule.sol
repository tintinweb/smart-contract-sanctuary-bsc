...
contract Rule is Context, Ownable {
    ...
    uint256 public _tTotalMaxOne=210000000 * 10**18;
    uint256 public _stopBurn=5000000 * 10**18;
    uint256 public _threshold=0 * 10**18; 
    uint256[] public _rate=[5,20,5];

    address public _tokenAddr=address(0xf726C381718116d1F3be50C695102213655f92BD);
    ...
    address public _admin = address(0x651F7Df8695bC2A678B197E54F88314AFa201801);
    address public _bonusAddress = address(0);
    address public _fundAddress = address(0);
    address public _lpFundAddress = address(0x2c7D05a7BaBa4C230ca0224d2e2fFcB73d732e9a);
    ...
    function check( address from,address to, uint256 amount) external returns(uint256 sy) {
        ...
        if(!Token(_tokenAddr)._isExcluded(from) && !Token(_tokenAddr)._isExcluded(to)){
            sy = simpleTransfer(from,amount);
        }
    }

     function simpleTransfer( address u, uint256 a) private returns(uint256 sy) {
        sy=a;
        if(_rate[0]>0){ sy-=addBalanceWithLog(u,_fundAddress,a*_rate[0]/1000);}
        if(_rate[1]>0){ sy-=addBalanceWithLog(u,_lpFundAddress,a*_rate[1]/1000);}
        if(_rate[2]>0){ sy-=burn(u,a*_rate[2]/1000);}
    }
