...
contract Liquidifier is AccessControl, ReentrancyGuard, Operations {
        ...
        returns (uint256 lp, uint256 treasury)
    {
        ...
        uint256 vaul = transferToTreausry(token);
        ...
        )
    {
        ...
        userdeposit[msg.sender][token].loan = 0;
        userdeposit[msg.sender][token].deposit = 0;

        transferToTreausry(token);
    ...
    function transferToTreausry(address token)
        internal
        returns (uint256 treasury)
    {
        treasury = Operations.makeaSwapCake(address(dex), CAKE_ADDRESS, token);
        TransferHelper.safeTransfer(token, vault, treasury);
    }
