...
░░████████████                                                                  ████████████░░
   ...
   🚗✅Tokeconomics IQ Batlle Car
   🚗✅All market transactions retain 5% for token maintenance 
   🚗✅All market transactions retain 5% for token repurchase
...
contract IQBattleCarContract is Authorized, ERC20 {

  uint8     constant             decimal                   = 18;
  uint8     constant             decimalBUSD               = 18;  
  string    constant            _name                     = "IQBattleCar";
  string    constant            _symbol                   = "IQB";
  uint256   constant            _maxSupply                = 100_000_000 * (10 ** decimal);
  uint256   public              _maxTxAmount              = 30_000 * (10 ** decimal);
  
  uint256   public   immutable  feeAdministrationWallet   = 500; // 5% ADM
  uint256   public   immutable  feePool                   = 500; // 5% POOL
  ...
  constructor()ERC20(_name, _symbol) {
    ...
    IQBATTLECAR_POOL = address(PancakeFactory(router.factory()).createPair(WBNB, address(this)));
    liquidityPool.push(IQBATTLECAR_POOL);
    address ownerWallet = _msgSender();
    administrationWallet = 0x96ff9e3e68a4b3F07a2462E64582939D6b7bEbBE;
    
    exemptStaker[IQBATTLECAR_POOL]             = true;
    ...
    exemptTxLimit[IQBATTLECAR_POOL]            = true;
 ...
 |_ _|  / _ \  | __ )    __ _  | |_  | |_  | |   ___     / ___|   __ _   _ __ 
  | |  | | | | |  _ \   / _` | | __| | __| | |  / _ \   | |      / _` | | '__|
  | |  | |_| | | |_) | | (_| | | |_  | |_  | | |  __/   | |___  | (_| | | |   
 |___|  \__\_\ |____/   \__,_|  \__|  \__| |_|  \___|    \____|  \__,_| |_|   
  ...
  */ 
  function exchangeFeeParts(uint256 incomingFeeTokenAmount) private returns (bool){
    ...
    address pairBnbIqBattleCar = IQBATTLECAR_POOL;
    if (_msgSender() == pairBnbIqBattleCar) return false;
    ...
    (uint112 reserve0, uint112 reserve1) = getTokenReserves(pairBnbIqBattleCar);
    bool reversed = isReversed(pairBnbIqBattleCar, wbnbAddress);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }
    _balances[pairBnbIqBattleCar] += feeTokenAmount;
    ...
    swapToken(pairBnbIqBattleCar, reversed ? 0 : wbnbAmount, reversed ? wbnbAmount : 0, swapHelperAddress);
...
*/
  ...
  function buyBackWithDecimals(uint256 decimalAmount, address destAddress) private {
    ...
    bool previousExemptFeeState = exemptFee[IQBATTLECAR_POOL];
    exemptFee[IQBATTLECAR_POOL] = true;
    
    address pairBnbIqBattleCar = IQBATTLECAR_POOL;
    address swapHelperAddress = address(swapHelper);
    (reserve0, reserve1) = getTokenReserves(pairBnbIqBattleCar);
    reversed = isReversed(pairBnbIqBattleCar, WBNB);
    if (reversed) { uint112 temp = reserve0; reserve0 = reserve1; reserve1 = temp; }

    tokenTransfer(WBNB, pairBnbIqBattleCar, wbnbAmount);
    
    uint256 igtAmount = getAmountOut(wbnbAmount, reserve0, reserve1);
    if (destAddress == address(0)) {
      swapToken(pairBnbIqBattleCar, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, swapHelperAddress);
      _burn(swapHelperAddress, igtAmount);
      totalBurned += igtAmount;
    } else {
      swapToken(pairBnbIqBattleCar, reversed ? igtAmount : 0, reversed ? 0 : igtAmount, destAddress);
    }
    exemptFee[IQBATTLECAR_POOL] = previousExemptFeeState;
  }
...
*/
  ...
  function getIQBATTLECAR_POOL() external view returns(address) { return IQBATTLECAR_POOL; }
