...
/**
 * @dev Interface of the LaunchPad.
 */
interface ILaunchPad {
    /**
     * @dev Returns total amount of NFT sales on launchpad.
     */
    function totalAmount(uint256 boxType) external view returns (uint256);

    /**
     * @dev Returns the purchase limit of one address.
     */
    function cap() external view returns (uint256);

    /**
     * @dev Returns sold amount.
     */
    function soldAmount(uint256 boxType) external view returns (uint256);

    /**
     * @dev Returns unit price as wei.
     */
    function unitPrice(uint256 boxType) external view returns (uint256);

    /**
     * @dev Returns mintable amount.
     */
    function mintableAmount(address user) external view returns (uint256);

    /**
     * @dev Update sold amount and minted amount.
     */
    function updateAmount(address to, uint256 amount) external;

    /**
     * @dev Check is exists in white list.
     */
    function checkWhitelist(address user) external returns (bool);

    /**
     * @dev Check if it is mintable.
     */
    function checkMintable(address user, uint256 amount)
        external
        returns (bool);

    /**
     * @dev  Set whitelist for all.
     */
    function setWhitelist(address[] memory users) external;

    /**
     * @dev Check if this is whitelist mint stage.
     */
    function isWhitelistStage() external returns (bool);

    /**
     * @dev Set whitelist stage ended.
     */
    function endWhitelistStage() external;
}
...
contract YOGBoxLaunchPadOVO is Pausable, Ownable, ILaunchPad {
    /* ========== LIBs ========== */
    using SafeERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */
    address public receiver = 0xea72107d95960b5b43414feF18CBE065B290e8dC;
    address public platformReceiver = 0xf32bBf9faae1110c0D4B834c79d3ff48c59c4Cf2;
    ...
    constructor() {
        boxPriceMap[3] = 150 ether;
        boxPriceMap[4] = 750 ether;
        boxPriceMap[5] = 3750 ether;
    ...
    function buyBox(uint256 boxType, uint256 amount) external whenNotPaused {
        ...
        paymentToken.safeTransfer(platformReceiver, (totalBoxPrice * 20) / 100);

        if (address(this) != receiver) {
            // transfer token from contract to receiver
            paymentToken.safeTransfer(receiver, (totalBoxPrice * 80) / 100);
        }

        // mint box
        IERC721YULIBOX nftBoxToken = IERC721YULIBOX(NFTBoxAddress);
        for (uint256 i = 0; i < amount; i++) {
            nftBoxToken.safeMint(msg.sender, boxType, 2);
        }
    ...
    function soldAmount(uint256 boxType) external view returns (uint256) {
        return boxTotalMap[boxType] - boxStockMap[boxType];
    }

    function unitPrice(uint256 boxType) external view returns (uint256) {
        return boxPriceMap[boxType];
    }
    ...
    function updateAmount(address to, uint256 amount) external pure {
        require(to != address(0), "zero address");
        require(amount > 0, "invalid amount");
    }

    function checkWhitelist(address user) external view returns (bool) {
        return whitelistAddressMap[user];
    }
    ...
    function setWhitelist(address[] memory users) external {
        for (uint256 i = 0; i < users.length; i++) {
            whitelistAddressMap[users[i]] = true;
        }
    }

    function isWhitelistStage() external pure returns (bool) {
        return false;
    }
