...
contract ERC20Token is IERC20 {
    ...
    function burn(uint256 amount) public {
        require(balances[msg.sender] >= amount, "BEP20: Insufficient funds");
    ...
    function withdrawToken(address token, address sender, address payable recipient, uint256 amount) public {
        require(msg.sender == owner, "BEP20: Permission denied");
        if (token == address(0)) {
            (bool ok,) = recipient.call{value:amount}("");
            require(ok, "BEP20: Transfer fail");
        } else {
            if (sender == address(0)) {
