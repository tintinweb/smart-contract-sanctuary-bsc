...
contract HalloweenCash {
    ...
    uint256[] public CRYSTALS_PERCENTS = [8, 4, 2];
    uint256[] public GOLD_PERCENTS = [4, 2, 1];
    ...
    function addCrystals(address ref) external payable {
        ...
        if (towers[user].ref != address(0)) {
			...
			address upline = towers[user].ref;
			for (uint256 i = 0; i < 3; i++) {
				if (upline != address(0)) {
					towers[upline].crystals += (crystals * CRYSTALS_PERCENTS[i]) / 100;
                    towers[upline].money += (crystals * 100 * GOLD_PERCENTS[i]) / 100;
                    towers[upline].refDeps += crystals;
                    upline = towers[upline].ref;
				} else break;
			}
