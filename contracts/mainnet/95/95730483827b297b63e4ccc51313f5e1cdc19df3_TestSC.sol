...
contract TestSC is Context, IERC20, Ownable {
    ...
    address public operationsAddress = 0xed3dea276886647d1BDC0b72A4E5ce9Ec7864cAb;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "Test Sc";
    string private constant _symbol = "Test Sc";
    ...
    function setFeeRates(uint256 _rfi, uint256 _operations, uint256 _burn, uint256 _liquidity) external onlyOwner {
        feeRates.rfi = _rfi;
    ...
    function setSellFeeRates(uint256 _rfi, uint256 _operations, uint256 _burn, uint256 _liquidity) external onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi,to_return.rOperations, to_return.rBurn, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if(isSale){
            ...
            s.tTransferAmount = tAmount-s.tRfi-s.tOperations-s.tBurn-s.tLiquidity;
        }
        else{
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi, uint256 rOperations,uint256 rBurn, uint256 rLiquidity) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && swapEnabled && canSwap && from != pair){
            ...
            if ( balance > uint256(1 * 10**18) && to == pair) {
