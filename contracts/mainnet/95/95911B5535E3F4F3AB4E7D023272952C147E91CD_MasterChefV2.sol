...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare;   // Accumulated Odenas per share, times 1e12. See below.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = address(pool.lpToken) == address(odenaAddress) ? totalStakedAmount : pool.lpToken.balanceOf(address(this));
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = IBEP20(odenaAddress).balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > tokenBalance) {
            transferSuccess = IBEP20(odenaAddress).transfer(_to, tokenBalance);
        } else {
            transferSuccess = IBEP20(odenaAddress).transfer(_to, _amount);
        }
