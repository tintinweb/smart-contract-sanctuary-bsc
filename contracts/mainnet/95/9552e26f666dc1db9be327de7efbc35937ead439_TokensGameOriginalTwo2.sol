...
struct User {
    ...
    mapping (address => bool) private BUY_MINERS;
    ...
    function buyMinersRef(address ref, uint256 amount) public payable { // first purchase with referral link only
        require(amount >= 50000000000000000000);
        require(initialized);
        require(BUY_MINERS[msg.sender] == false);
        countUsers += 1;
        deals +=1;
        volume += amount;
    ...
    function buyMiners2(uint256 amount) public payable { //other purchases
        require(amount >= 50000000000000000000);
        require(initialized);
        require(BUY_MINERS[msg.sender] == true);
        deals +=1;
        volume += amount;
        BUY_MINERS[msg.sender] = true;
    ...
    function checkUser(address userAddr) external view returns (address ref) {
