...
//    function getAllToken(address owner) public view  returns (uint256[] memory) {
...
abstract contract Ownable is Context {
    ...
    modifier isManager{
     ...
     uint private managerNum;
       
    function addManager(address manager) public onlyOwner{
        if (managers[manager] == 0) {
            managerNum++;
        }
        managers[manager] = 1;
    }
...
abstract contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  contract MMGNFT is ERC721, Ownable,VRFv2Consumer{
    ...
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
       uint256 public  pricePerMNFT;
    uint256 public  maxMNFT;
     address[] public mapping_address;
    address public jobaddress;
    address public Mmg_contract = 0x1C376275791069349fef8bd9F431382D384657a3;
    address public Mmc_contract = 0x1C376275791069349fef8bd9F431382D384657a3;
    ...
    mapping(uint256 => bool) public is_open;
        struct nft{
        uint256 gold;//金币
        uint256 aggressivity;//攻击力
        uint256 Magnification;//倍率
        uint256 probability;//概率
        uint256 synthesis;//合成几率
        uint256 cooling_time;//合成冷却时间
        uint256 consume_number;//消耗的代币数量
        uint256 life_cycle;//生命周期
        uint256 total;//能开出来的总量
        string image_url;
    }
    ...
    mapping(uint256 => uint256) token_value;
    mapping(uint256=>nft) public nfts;
   mapping (uint256 => nft_struct) public token_nft;
    constructor() VRFConsumerBaseV2(vrfCoordinator)
       public ERC721("MMGNFT", "MGN")
    {
      
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        pricePerMNFT = 1 * 10 ** 6;
        maxMNFT = 1000000;
        jobaddress = msg.sender;
    ...
    function set_address(address _mmg_address,address _mmc_address) public isManager {
        require(_mmg_address != address(0x0));
        require(_mmc_address != address(0x0));
        Mmg_contract = _mmg_address;
        Mmc_contract = _mmc_address;
    }
    function open_nft(uint256 start_tokenId,uint256 end_tokenId,uint256 size)public isManager{
        require(start_tokenId != 0 && end_tokenId != 0 && size != 0);
    ...
    function owner_open_nft(uint256 start_tokenId,uint256 end_tokenId,uint256 size)public isManager{
        require(start_tokenId != 0 && end_tokenId != 0 && size != 0);
   ...
   function setPrice(uint256 _price) public isManager{
        pricePerMNFT = _price;
    }
    function setjob(address _job)public isManager{
            jobaddress = _job;
    }
    function setBaseURI(string memory baseURI) public isManager {
     ...
     //设置nft属性  加添加
    function AddNft(uint256 _gold,uint256 _aggressivity,uint256 _Magnification,uint256 probability,uint256 level,uint256 _synthesis,uint256 _cooling_time,uint256 _consume_number,uint256 _life_cycle,uint256 _total,string memory _image_url) public isManager{
           if(nfts[level].gold == 0){
          all_nft = all_nft+1;
          }
          nfts[level].gold = _gold;
          nfts[level].aggressivity = _aggressivity;
          nfts[level].Magnification = _Magnification;
          nfts[level].probability = probability;
          nfts[level].synthesis = _synthesis;     
          nfts[level].cooling_time = _cooling_time;
          nfts[level].consume_number = _consume_number;
          nfts[level].life_cycle = _life_cycle;
          nfts[level].total = _total;
          nfts[level].image_url = _image_url;
    }

    //合成方法
     function synthesis(uint256 _tokenId1,uint256 _tokenId2) public returns(uint256){
         require(tx.origin == msg.sender);
        require(ERC721.ownerOf(_tokenId1) == msg.sender && ERC721.ownerOf(_tokenId2) == msg.sender,"is not owner");
        require(token_nft[_tokenId1].level == token_nft[_tokenId2].level,"level not");
          require(is_open[_tokenId1] && is_open[_tokenId2],"is not owner");
        require(nfts[token_nft[_tokenId1].level+1].gold > 0,"Insufficient gold coins");
        require(token_nft[_tokenId1].create_time+nfts[token_nft[_tokenId1].level+1].cooling_time <= block.timestamp && token_nft[_tokenId2].create_time+nfts[token_nft[_tokenId2].level+1].cooling_time <= block.timestamp,"time not");
        ERC20(Mmc_contract).transferFrom(msg.sender,jobaddress,nfts[token_nft[_tokenId1].level+1].consume_number);
         uint256 number = rand(msg.sender,_tokenId1);

         if(number < nfts[token_nft[_tokenId1].level+1].synthesis){
             ERC721.transferFrom(msg.sender,deadAddress,_tokenId1);
             ERC721.transferFrom(msg.sender,deadAddress,_tokenId2);
            uint256 _tokenId = totalSupply();  
             _nftTokens[msg.sender].remove(_tokenId1);
             _nftTokens[msg.sender].remove(_tokenId2);
            token_nft[_tokenId].level = token_nft[_tokenId1].level+1;
            token_nft[_tokenId].create_time = block.timestamp;
    ...
    //创建nft方法
     function CreateNft(uint256 _tokenId) public returns(uint256) {
         require(tx.origin == msg.sender);
        //   boxnft.transferFrom(msg.sender,deadAddress,_tokenId);
           require(ERC721.ownerOf(_tokenId) == msg.sender || msg.sender == owner() ,"is not owner");
          uint256 number;
          if(token_value[_tokenId] == 0){
            number = rand(msg.sender,_tokenId);
          }else{
            number = token_value[_tokenId];
          }
            require(token_nft[_tokenId].level == 0);
            require(!is_open[_tokenId]);
        uint256 level = 1;
        for(uint i=1;i<=all_nft;i++){
            if(number < nfts[i].probability && nfts[i].total > 0){
                 level = i;
            } 
    ...
    function openAll(uint256[] memory tokenIds)public returns(uint256[] memory){
        require(tokenIds.length > 0);
        require(tx.origin == msg.sender);
         uint256[] memory mnfts = new uint256[](tokenIds.length);
         uint256 number;
        for(uint m;m<tokenIds.length;m++){   
          if(token_value[tokenIds[m]] == 0){
            number = rand(msg.sender,tokenIds[m]);
          }else{
            number = token_value[tokenIds[m]];
          }
        uint256 level = 1;
        for(uint i=1;i<=all_nft;i++){
            if(number < nfts[i].probability ){
                 level = i;   
            } 
        }
        if(token_nft[tokenIds[m]].level == 0 && !is_open[tokenIds[m]]){
    ...
    function getBoxPageToken(address owner,uint256 _limit, uint256 _pageNumber) public view  returns (uint256[] memory) {
     ...
     function getNftPageToken(address owner,uint256 _limit, uint256 _pageNumber) public view  returns (uint256[] memory) {
       ...
       function BuyBox(uint256 numberOfTokens,address agent_address) public {
        ...
        mmgtranFrom(msg.sender,deadAddress,price*90/100);
        uint256 radio = 10;
        ...
        if(agent_address != address(0x0)){
                radio = 5;
           mmgtranFrom(msg.sender,agent_address,price*radio/100); 
           sharerecord[agent_address].push(ShareRecord(block.timestamp,msg.sender,price*radio/100));
       
        }
        if(check_member(msg.sender)){
             mapping_address.push(msg.sender);
        }
        mmgtranFrom(msg.sender,jobaddress,price*radio/100);
    ...
    function mmgtranFrom(address _addr1,address _addr2,uint256 _amount)  internal virtual{
        ERC20(Mmg_contract).transferFrom(_addr1,_addr2,_amount);
    }
   function shuffle(
        uint256 size, 
        uint256 entropy
