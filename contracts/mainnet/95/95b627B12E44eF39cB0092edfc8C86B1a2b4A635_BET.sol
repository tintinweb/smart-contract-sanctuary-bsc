...
contract BET is Context, IBEP20, LockToken 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress = payable(0x1c2507ceba9B2938C4E00AAB3cD77071AD60895D);
    address payable public charityAddress = payable(0x1c2507ceba9B2938C4E00AAB3cD77071AD60895D);
    address payable public stakingAddress = payable(0x1c2507ceba9B2938C4E00AAB3cD77071AD60895D);
    address payable public lotteryAddress = payable(0x1c2507ceba9B2938C4E00AAB3cD77071AD60895D);
    ...
    uint256 private _tTotal = 333_333_333 * 10**18;
    ...
    uint256 public _walletHoldingMaxLimit =  _tTotal.div(100).mul(6); // 6%
    ...
    {
        uint256 _totalFees = _marketingFee.add(_stakingFee).add(_charityFee).add(_lotteryFee);
        return _amount.mul(_totalFees).div(100);
    }
    ...
    event updateFee(uint256 totalFee, uint256 timestamp);
    function setAllFeePercentages(uint256 marketingFee, uint256 charityFee, uint256 stakingFee, uint256 lotteryFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _marketingFee.add(_charityFee).add(_stakingFee).add(_lotteryFee);

        require(totalFee<=10, "Too High Fee");
        emit updateFee(totalFee, block.timestamp);
    }
    ...
    {
        charityAddress = payable(_Address);
    }

    function setLotteryAddress(address _Address) external onlyOwner() 
    {
        lotteryAddress = payable(_Address);
    }

    function setStakingAddress(address _Address) external onlyOwner() 
    {
        stakingAddress = payable(_Address);
    }
