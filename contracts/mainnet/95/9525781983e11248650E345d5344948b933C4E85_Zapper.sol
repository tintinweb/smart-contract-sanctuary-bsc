...
contract Zapper is ReentrancyGuard, ZapperFee {
    ...
    {
        address oToken = p.path[0];
        if (oToken == p.path[1]) {
            IERC20(oToken).transferFrom(
                msg.sender,
                p.predictionAddress,
                amountIn
            );
            amountIn = _adjustFee(amountIn, oToken);
    ...
    function zapNativeInto20(iPar memory p)
        ...
        returns (uint256 amountOut)
    {
        uint256 amountIn = _adjustFee(msg.value, address(0));
    ...
    {
        _before(p, amountIn);
        amountIn = _adjustFee(amountIn, p.path[0]);
...
contract ZapperFee {
    ...
    EnumerableMap.AddressToUintMap private treasury;
    ...
    function getTreasuryTokens(uint256 from, uint256 to)
        external
        view
        returns (
            address[] memory,
    ...
    {
        to = to == 0 ? treasury.length() : to;
        require(from < to, "From musted be < to");
        ...
        for (uint256 i = from; i < to; i += 1) {
            (address token, uint256 balance) = treasury.at(i);
            tokens[i - from] = token;
            balances[i - from] = balance;
        }
        return (tokens, balances);
    }

    function getTreasuryIn(address token) external view returns (uint256) {
        (bool exists, uint256 balance) = treasury.tryGet(token);
    ...
    // internals
    function _adjustFee(uint256 amountIn, address token)
        internal
        returns (uint256 adjustedAmount)
    {
        uint256 fee = (amountIn * treasuryFee) / FEE_DIVIDER;
        (, uint256 balance) = treasury.tryGet(token);
        treasury.set(token, balance + fee);
        return amountIn - fee;
    }
