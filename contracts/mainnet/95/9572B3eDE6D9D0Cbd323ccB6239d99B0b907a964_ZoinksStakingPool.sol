...
contract ZoinksStakingPool is Ownable, ReentrancyGuard {
    ...
    event RewardTransfer(address indexed user, uint256 zoinksReward, uint256 snacksReward);
    ...
    function deposit(uint256 _amount) external {
        ...
        totalStakedZoinks = totalStakedZoinks + _amount;
    ...
    function rewardTransfer() internal {
        ...
        if (rewardDebt > 0) {
            if (block.timestamp < user.lastDepositDate + 1 days) {
                stakedToken.safeTransfer(seniorageAddress, rewardDebt.div(2));
                stakedToken.safeTransfer(msg.sender, rewardDebt.div(2));
            } else {
                stakedToken.safeTransfer(msg.sender, rewardDebt);
            }
            user.rewardDebt = 0;
            totalRewardDebtZoinks = totalRewardDebtZoinks.sub(rewardDebt);
        }
        uint256 rewardDebtSnacks = user.rewardDebtSnacks;
        if (rewardDebtSnacks > 0) {
            if (block.timestamp < user.lastDepositDate + 1 days) {
                stakedToken.safeTransfer(seniorageAddress, rewardDebtSnacks.div(2));
                stakedToken.safeTransfer(msg.sender, rewardDebtSnacks.div(2));
            } else {
                stakedToken.safeTransfer(msg.sender, rewardDebtSnacks);
            }
            user.rewardDebtSnacks = 0;
            totalRewardDebtSnacks = totalRewardDebtSnacks.sub(rewardDebtSnacks);
        }
        emit RewardTransfer(msg.sender, rewardDebt, rewardDebtSnacks);
    }
