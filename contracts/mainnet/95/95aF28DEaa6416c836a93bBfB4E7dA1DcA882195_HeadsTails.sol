...
contract HeadsTails is Ownable {
    ...
    **/
        ...
        external
    {
        ...
        require(question.status.answered == false, 
            "HeadsTails: Question has already been answered.");
        require(question.status.cancelled == false, 
        ...
        IERC20(question.token).transferFrom(msg.sender, address(this), stakeAmount);
    ...
    **/
    function unstake(uint256 questionId) external {
        ...
        if(question.status.cancelled == false){
    ...
    **/
    function harvest(uint256 questionId) external {
        ...
        _totalAmount = _totalAmount - _commissionAmount - _submitterAmount;
        ...
        onlyOwner
    {
        ...
        if(_submitterAmount > 0){
            if(question.token == address(this)){
                ...
                require(success, "HeadsTails: Selecting a Winning Answer failed");
            }else{
    ...
    **/
    function withdraw(address token, address payable to) external onlyOwner {
        ...
        require( to != address(0), 
            "HeadsTails: Invalid address. Please enter a valid token deposit address.");
        ...
        if(token == address(this)) {
            ...
            require(success, "HeadsTails: Withdrawal failed");
        }else{
