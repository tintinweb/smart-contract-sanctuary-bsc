...
No team tokens
No private sale/pre sale
50BNB STARTING LIQUIDITY 

TG: https://t.me/SantaCoinHoHoHo
*/
...
contract SantaCoin is IERC20, Ownable
{
    ...
    uint public constant InitialSupply= 6053134000 * 10**_decimals;
    uint public buyTax = 5;
    uint public sellTax = 5;
    uint public transferTax = 5;
    ...
    address private marketingWallet=0x92860Cf0D2cB4f431AB9F46113553e60c05D2f26;
    ...
    function SetTaxes(uint buy, uint sell, uint transfer_, uint marketing,uint liquidity) external onlyOwner{
        uint maxTax=150;
        require(buy<=maxTax&&sell<=maxTax&&transfer_<=maxTax,"Tax exceeds maxTax");
        require(marketing+liquidity==TAX_DENOMINATOR,"Taxes don't add up to denominator");
        buyTax=buy;
        sellTax=sell;
        transferTax=transfer_;
        marketingTax=marketing;
        liquidityTax=liquidity;
        emit OnSetTaxes(buy, sell, transfer_, marketing,liquidity);
    }
    ...
    function SetMarketingWallet(address _address) external onlyOwner{
        ...
        require(_address != marketingWallet,"Address is same");
        marketingWallet = _address;
        emit NewMarketingWalletSet(_address);
    }
    ...
    function emergencyTokenrecovery(address tokenAddress, uint256 amountPercentage) external onlyOwner {
        require(tokenAddress!=address(0)&&tokenAddress!=address(this));
    ...
    event OnSetTaxes(uint buy, uint sell, uint transfer_, uint marketing,uint liquidity);
    ...
    event NewMarketingWalletSet(address _address);
