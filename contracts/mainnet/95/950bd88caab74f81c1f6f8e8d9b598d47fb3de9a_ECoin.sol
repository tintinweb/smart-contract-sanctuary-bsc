...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(owner() == _msgSender(), "E-COIN: caller is not the owner");
        _;
    }

    modifier onlyPendingOwner() {
        require(pendingOwner() == _msgSender(), "E-COIN: caller is not the owner");
        _;
    }

    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "E-COIN: new owner is the zero address");
...
abstract contract ERC20 is Context, IERC20 {
    ...
    function decreaseAllowance(address spender_, uint256 subtractedValue_) public returns (bool) {
        ...
        require(currentAllowance >= subtractedValue_, "E-COIN: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(address from_, address to_, uint256 amount_) internal {
        require(from_ != address(0), "E-COIN: transfer from the zero address");
        require(to_ != address(0), "E-COIN: transfer to the zero address");

        uint256 fromBalance = _balances[from_];
        require(fromBalance >= amount_, "E-COIN: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account_, uint256 amount_) virtual internal {
        require(account_ != address(0), "E-COIN: mint to the zero address");
    ...
    function _burn(address account_, uint256 amount_) virtual internal {
        require(account_ != address(0), "E-COIN: burn from the zero address");

        uint256 accountBalance = _balances[account_];
        require(accountBalance >= amount_, "E-COIN: burn amount exceeds balance");
        unchecked {
    ...
    function _approve(address owner_, address spender_, uint256 amount_) internal {
        require(owner_ != address(0), "E-COIN: approve from the zero address");
        require(spender_ != address(0), "E-COIN: approve to the zero address");
    ...
    function _spendAllowance(address owner_, address spender_, uint256 amount_) internal {
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount_, "E-COIN: insufficient allowance");
            unchecked {
...
contract ECoin is Context, Ownable, ERC20 {
