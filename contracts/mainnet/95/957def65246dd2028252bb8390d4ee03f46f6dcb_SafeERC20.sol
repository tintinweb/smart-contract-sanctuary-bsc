...
contract MTDMining is Context, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant PROJECT_FEE = 50; // project fee 4% of deposit
    uint256 public constant MARKETING_FEE = 50; // marketing fee 4% of deposit
    uint256 public constant FUND_FEE = 400; // fund fee 40% of deposit 
    uint256 public constant PERCENT_REFFERAL = 80; // referral fee 8% of deposit
    uint256 public constant PERCENTS_DIVIDER = 1000;

    uint256 private SHARES_TO_HATCH_1MINERS = 2592000; //for final version should be seconds in a day
    ...
    function hatchShares(address ref) public {
        require(initialized, "MTDMining is not initialized");
    ...
    function seedMarket() public {
        if (block.timestamp > startDate && !initialized) {
            ...
            initialized = true;
            marketShares = 25920000000;
        }
    }

    function updateStartUnix(uint256 newStartUnix) public onlyOwner {
        require(block.timestamp < startDate, "MTDMining is already started");
