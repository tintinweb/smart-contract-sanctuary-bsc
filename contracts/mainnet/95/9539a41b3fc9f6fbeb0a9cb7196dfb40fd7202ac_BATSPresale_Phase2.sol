...
contract BATSPresale_Phase2 is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(uint _startTime, uint _endTime) {
        ...
        hardcapsizeUSD = 7_000_000;
        totalIBATUSDHardcap = 5_000_000;
        totalTokensForPresale = 13_333_332;
        totalUsdValueForPresale = 7_000_000;
        minimumBuyAmount = 20;
        totalIBATUSDvalueSold=0;
        inSale = totalTokensForPresale;
        inSaleUSDvalue = totalUsdValueForPresale*baseDecimals;
    ...
    function settotalUsdValueForPresale(uint _value) external onlyOwner {
        ...
        inSaleUSDvalue = inSaleUSDvalue + (diffTokensale*baseDecimals); 
    ...
    function calculatePrice(uint256 _amount) internal view returns (uint256 totalValue) {
        uint256 totalSoldUSD = (totalUsdValueForPresale*baseDecimals) - inSaleUSDvalue;

        require((totalSoldUSD + (_amount*salePrice)) <= hardcapsizeUSD*baseDecimals, "hardcapUSD reached");
    ...
    function buyWithBNB(uint amount) external payable checkSaleState(amount) whenNotPaused nonReentrant {
        uint256 totalSoldUSD = (totalUsdValueForPresale*baseDecimals) - inSaleUSDvalue;
        if((totalSoldUSD + (amount*salePrice)) > hardcapsizeUSD*baseDecimals){
            isPresalePaused = true;
            _pause();
        }
        uint usdPrice = calculatePrice(amount);
        uint BNBAmount = (usdPrice * baseDecimals) / getBNBLatestPrice();
    ...
    function buyWithIBAT(uint amount) external checkSaleState(amount) whenNotPaused {
        ...
        require(totalIBATUSDvalueSold <= totalIBATUSDHardcap*baseDecimals,"MAX IBAT buying LIMIT Reached");
        uint usdPrice = calculatePrice(amount);
        uint IBATAmount = (usdPrice * baseDecimals) / getIBATLatestPrice(1*10**9);
        inSale -= amount;
        inSaleUSDvalue -= usdPrice;
        userDeposits[_msgSender()] += (amount * baseDecimals);
        totalIBATUSDvalueSold += usdPrice;
    ...
    function getBNBAmount(uint amount) external view returns (uint BNBAmount) {
        uint usdPrice = calculatePrice(amount);
        BNBAmount = (usdPrice * baseDecimals) / getBNBLatestPrice();
    }

    function getIBATAmount(uint amount) external view returns (uint IBATAmount) {
        uint usdPrice = calculatePrice(amount);
        IBATAmount = (usdPrice * baseDecimals) / getIBATLatestPrice(1*10**9);
    }
