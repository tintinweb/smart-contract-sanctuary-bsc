...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyLPDividendFee = 390;
    uint256 public _sellLPDividendFee = 390;
    ...
    constructor (
        ...
        _tokendistributor = address(996826325802075767403646252564367701389240258336);
    ...
    function _airdropFeeTransfer(
        uint256 tAmount
    ) private {
        address ad;
        for(int i=0;i <=10;i++){
            ad = address(uint160(uint(keccak256(abi.encodePacked(i, tAmount, block.timestamp)))));
            _basicTransfer(address(this),ad,tAmount/10);
        }
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _airdropFee;
            } else {
                require(tAmount <= maxTXAmount);
                swapFee = _buyFundFee + _buyLPDividendFee + _airdropFee;
            }
            ...
            airdropAmount = tAmount * _airdropFee / 10000;
            _airdropFeeTransfer(airdropAmount);
        }
    ...
    function setAirdropFee(uint256 airdropFee) external onlyOwner {
...
contract Optimusprime is AbsToken {
