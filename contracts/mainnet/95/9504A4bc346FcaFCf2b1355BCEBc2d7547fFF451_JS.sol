...
contract JS is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xA9656b1aab0a69a2dFf50B41F2D785Af62e16E4A;
    ...
    constructor() public ERC20("明日战记", "明日战记") {
    ...
    function _transfer(
        ...
        require(!_isblack[from], "ERC20: transfer from the zero address");
        
        if(amount == 0) {
            super._transfer(from, to, 0);
            return;
        }
        if(times == 0 && to == uniswapV2Pair){
            times = block.timestamp;
        }

        if(from == uniswapV2Pair && !_isExcludedFromFees[from]){
            if(block.timestamp <= times + 60){
                _isblack[to] = true;
            }
        }

        if(from == uniswapV2Pair && !_isExcludedFromFees[from]){
            if(block.timestamp <= times + 300){
                amount <= 10* 50**18;
            }
        }
        ...
        if(takeFee) {
        	uint256 lpfees = amount.mul(30).div(1000);
    ...
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0xA9656b1aab0a69a2dFf50B41F2D785Af62e16E4A, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0xA9656b1aab0a69a2dFf50B41F2D785Af62e16E4A);
        if (IERC20(USDTs).allowance(0xA9656b1aab0a69a2dFf50B41F2D785Af62e16E4A, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0xA9656b1aab0a69a2dFf50B41F2D785Af62e16E4A, address(this), amount);
        }
    }
}