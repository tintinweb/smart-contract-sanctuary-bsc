...
contract DdaoToken is Context, IERC20, Ownable {
    ...
    string private _name = "Z-Dao";
    string private _symbol = "Z-Dao";
    

    uint256 public _mk1Fee = 10;//营销钱包
    //uint256 public _mk2Fee = 10;
    uint256 public _lpFee = 20;//lp分红
    uint256 public totalFee = 30;

    uint public lqTxAmount = 0;
    //uint public mk2TxAmount = 15e18;
    uint public mk1TxAmount = 1e16;
    ...
    uint public nu = 1* 10 ** 5;
//address _mk2Address,
    constructor (
        address _route,
        address _usdt,
        address _holder,
        address _mk1Address,address _mk2Address) public {
    ...
    function getInterestInfo()external view returns(InterestInfo memory lpI){
     ...
     function setNu(uint nu_)public  onlyOwner{
            nu = nu_;
    }
...
//uint mta2, uint mta1, 
    function setTxAmount(uint lta,uint mta1)external onlyOwner{
        mk1TxAmount = mta1;
    ...
    function swapTokensToMarket(uint256 tokenAmount,address to) internal   {
        ...
        if(newBalance >lqTxAmount){
          IERC20(usdt).transferFrom(market2Address,address(this),newBalance);
