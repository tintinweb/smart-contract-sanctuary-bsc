...
contract LockToken is Ownable {
    ...
    function tradeStatus(bool _status) external onlyOwner {
        isOpen = _status;
    }
...
contract FapCoin is Context, IBEP20, LockToken {
    ...
    uint256 private _tTotal = 1 * 1e11  ether; // 100 billion total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
 
    string private _name = "FapCoin"; // token name
    string private _symbol = "$FAP"; // token ticker
    uint8 private _decimals = 18; // token decimals
 
    IPancakeRouter02 public pancakeRouter;
    address public pancakePair;
    address payable public marketWallet = payable(0x9313A4CE43fc82192E22EfC033D723bDDB042bcB);      //change wallet
    address payable public roadmapWallet = payable(0x1E6C6CC8B44E44a17782ad7d4b5b652Db8d14680);     //change wallet
    uint256 minTokenNumberToSell = 1000000 ether; // 1000000 max tx amount will trigger swap and add liquidity
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 25; // 2.5% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnBuying = 15; // 1.5% will be added to the liquidity pool
    uint256 public marketingwalletFeeOnBuying = 20; // 2% will go to the marketingwallet address
    uint256 public roadmapFeeOnBuying = 10; // 1% will go to the roadmap wallet address
 
    // sell tax fee
    uint256 public redistributionFeeOnSelling = 25; // 2.5% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnSelling = 25; // 2.5% will be added to the liquidity pool
    uint256 public marketingwalletFeeOnSelling = 20; // 2% will go to the market address
    uint256 public roadmapFeeOnSelling = 20; // 2% will go to the roadmap wallet address
    ...
    uint256 public roadmapFee = 0; // 0% will go to the roadmap wallet address
    ...
    uint256 private _currentroadmapFee;
    ...
    function setmWallets(address payable _marketWallet, address payable _roadmapWallet) external onlyOwner {
        require(_marketWallet!=address(0),"Market wallet cannot be address zero");
        require(_roadmapWallet!=address(0),"Market wallet cannot be address zero");
        marketWallet = _marketWallet;
        roadmapWallet = _roadmapWallet;
    }
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount*(_currentRedistributionFee+(_currentLiquidityFee)+(_currentmarketingwalletFee)+(_currentroadmapFee))/(1e3);
        return percentage;
    }
    ...
    function _takeMarketFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tMarketingFee = tAmount*(_currentmarketingwalletFee)/(1e3);
        uint256 rMarketingFee = tMarketingFee*(currentRate);
        _rOwned[marketWallet] = _rOwned[marketWallet]+(rMarketingFee);
        if (_isExcluded[marketWallet])
            _tOwned[marketWallet] = _tOwned[marketWallet]+(tMarketingFee);
        emit Transfer(_msgSender(), marketWallet, tMarketingFee);
    }
 
    function _takeRoadmapFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 roadmapFees = tAmount*(_currentroadmapFee)/(1e3);
        uint256 rRoadmapFee = roadmapFees*(currentRate);
        _rOwned[roadmapWallet] = _rOwned[roadmapWallet]+(rRoadmapFee);
        if (_isExcluded[roadmapWallet])
            _tOwned[roadmapWallet] = _tOwned[roadmapWallet]+(roadmapFees);
        emit Transfer(_msgSender(), roadmapWallet, roadmapFees);
    }
    ...
    function setBuyFee() private {
        ...
        _currentroadmapFee = roadmapFeeOnBuying;
    }
    ...
    function setSellFee() private {
        ...
        _currentroadmapFee = roadmapFeeOnSelling;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeRoadmapFee(tAmount, currentRate);
        _reflectFee(tAmount);
