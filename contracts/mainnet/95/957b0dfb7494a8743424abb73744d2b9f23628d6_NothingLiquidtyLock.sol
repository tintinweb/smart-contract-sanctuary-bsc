...
// File: contracts/NothingLiquidityLock.sol
...
contract NothingLiquidtyLock is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    IERC20 public immutable nothingLPToken;
    ...
    constructor(uint256 _unlockTimestamp, address _nothingLPToken) {
        UNLOCK_TIMESTAMP = _unlockTimestamp;
        nothingLPToken = IERC20(_nothingLPToken);
    }

    function UnlockLPTokens() external nonReentrant onlyOwner {
        require(block.timestamp > UNLOCK_TIMESTAMP, "Unlock: Too early");

        uint256 balanceToWithdraw = nothingLPToken.balanceOf(address(this));

        // Transfer tokens to owner
        nothingLPToken.safeTransfer(msg.sender, balanceToWithdraw);

        emit TokensUnlocked(balanceToWithdraw);
    }

    function withdrawOtherCurrency(address _currency) external nonReentrant onlyOwner {
        require(_currency != address(nothingLPToken), "Owner: Cannot withdraw LP Token");
