...
// File: contracts/interfaces/IAgeraLottery.sol

pragma solidity ^0.8.4;

interface IAgeraLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in AGERA per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in AGERA token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInAgera: price of a ticket in AGERA
...
// File: contracts/AgeraLottery.sol
 ...
 */
contract AgeraLottery is ReentrancyGuard, IAgeraLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAgera = 50 * 10**DECIMALS;
    uint256 public minPriceTicketInAgera = 50 * 10**(DECIMALS-4);
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAgera;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ageraPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAgera;
        uint32 finalNumber;
    }
     ...
     * @param _ageraTokenAddress: address of the AGERA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _ageraTokenAddress, address _randomGeneratorAddress) {
        ageraToken = IERC20(_ageraTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of AGERA to this contract
        uint256 amountAgeraToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAgera,
            _ticketNumbers.length
        );

        // Transfer agera tokens to this contract
        ageraToken.safeTransferFrom(address(msg.sender), address(this), amountAgeraToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAgera += amountAgeraToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAgeraToTransfer
        uint256 rewardInAgeraToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAgeraToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        ageraToken.safeTransfer(msg.sender, rewardInAgeraToTransfer);

        emit TicketsClaim(msg.sender, rewardInAgeraToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAgera) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in AGERA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].ageraPerBracket[j] =
                ...
                // A. No AGERA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].ageraPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAgera - amountToShareToWinners);

        // Transfer AGERA to treasury address
        ageraToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ageraToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAgera += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAgera >= minPriceTicketInAgera) && (_priceTicketInAgera <= maxPriceTicketInAgera),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAgera: _priceTicketInAgera,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            ageraPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAgera: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ageraToken), "Cannot be AGERA token");
    ...
    /**
     * @notice Set AGERA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAgera: minimum price of a ticket in AGERA
     * @param _maxPriceTicketInAgera: maximum price of a ticket in AGERA
     */
    function setMinAndMaxTicketPriceInAgera(uint256 _minPriceTicketInAgera, uint256 _maxPriceTicketInAgera)
        external
        onlyOwner
    {
        require(_minPriceTicketInAgera <= _maxPriceTicketInAgera, "minPrice must be < maxPrice");

        minPriceTicketInAgera = _minPriceTicketInAgera;
        maxPriceTicketInAgera = _maxPriceTicketInAgera;
    }
     ...
     * @param _priceTicket price of a ticket (in AGERA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ageraPerBracket[_bracket];
        } else {
            return 0;
        }
    }
