...
contract BlackRabbit is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('BlackRabbit', 'BRC') {
        require(
            feeAccount != address(0),
            'BlackRabbit: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'BlackRabbit: incorrect fee percent');
        require(owner != address(0), 'BlackRabbit: incorrect owner address');
        require(mintToAddr != address(0), 'BlackRabbit: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'BlackRabbit: transfer from the zero address');
        require(recipient != address(0), 'BlackRabbit: transfer to the zero address');
