...
contract HipHopGrow is IBEP20, Auth {
    ...
    string constant _name = "HipHop Grow";
    string constant _symbol = "HHGROW";
    ...
    function takeFee(address sender, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnTokens = feeAmount.mul(buybackFee).div(totalFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee, uint256 _devFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        marketingFee = _marketingFee;
        devFee = _devFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_marketingFee).add(_devFee);
