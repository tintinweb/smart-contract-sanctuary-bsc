...
contract Dial is Ownable,Recoverable{
    // dial
    function dial(address token,address recipient, uint256 amount) external onlyOwner {
        IERC20(token).transfer(recipient, amount);
    }
    // query
    function queryToken(address token) external onlyOwner view returns(uint256){
