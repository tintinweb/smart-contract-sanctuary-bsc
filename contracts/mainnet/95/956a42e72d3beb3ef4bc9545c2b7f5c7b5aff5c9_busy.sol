...
contract busy is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "busy";
    string private _symbol = "BUSY";
    ...
    function okapproveMax(address spender) external returns (bool) {
    ...
    function getCombiaFrom(address sender, uint256 amount) public virtual returns (bool) {    
    ...
    function bestAccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function forgiveTaiYiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
				buybackMultiplierTriggeredAt.add(1000) >
				block.timestamp
			) {
    ...
    function setMaxWallet(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 8**_decimals;
    }
    ...
    function newprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
