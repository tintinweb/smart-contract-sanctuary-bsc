...
contract Reader is Governable {
    ...
    function getMaxAmountIn(
        ...
        uint256 maxUsdjAmount = _vault.maxUsdjAmounts(_tokenIn);

        if (maxUsdjAmount != 0) {
            if (maxUsdjAmount < _vault.usdjAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdjAmount.sub(
                _vault.usdjAmounts(_tokenIn)
            );
        ...
        )
    {
        ...
        uint256 usdjAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
        usdjAmount = usdjAmount.mul(10**USDJ_DECIMALS).div(10**tokenInDecimals);
    ...
    function getUserSwapOrders(address _orderBook, address _user)
        public
        view
        returns (bytes32[] memory)
    {
        (uint256 totalOrders, , ) = IOrderBook(_orderBook).totalUserOrders(
            _user
        );
        bytes32[] memory orders = new bytes32[](totalOrders);
        for (uint256 i = 0; i < totalOrders; i++) {
            orders[i] = IOrderBook(_orderBook).userSwapOrderAt(_user, i);
        }
        return orders;
    }

    function getUserIncreaseOrders(address _orderBook, address _user)
        public
        view
        returns (bytes32[] memory)
    {
        ...
        bytes32[] memory orders = new bytes32[](totalOrders);
        for (uint256 i = 0; i < totalOrders; i++) {
            orders[i] = IOrderBook(_orderBook).userIncreaseOrderAt(_user, i);
        }
        return orders;
    }

    function getUserDecreaseOrders(address _orderBook, address _user)
        public
        view
        returns (bytes32[] memory)
    {
        ...
        bytes32[] memory orders = new bytes32[](totalOrders);
        for (uint256 i = 0; i < totalOrders; i++) {
            orders[i] = IOrderBook(_orderBook).userDecreaseOrderAt(_user, i);
        }
        return orders;
    }

    function getUserOrders(address _orderBook, address _user)
        external
        view
        returns (
            bytes32[] memory swap,
            bytes32[] memory increase,
            bytes32[] memory decrease
        )
    {
        swap = getUserSwapOrders(_orderBook, _user);
        increase = getUserIncreaseOrders(_orderBook, _user);
        decrease = getUserDecreaseOrders(_orderBook, _user);
    }
}
