...
contract EternalCakes is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint amount) public payable nonReentrant {
        ...
        if (ezBalance > 0) {
            require((whitelistClaimed[msg.sender] + amount) <= (ezBalance * 10), "EC: Whitelist mint amount exceeded");
        } else {
            require((whitelistClaimed[msg.sender] + amount) <= MAX_WHITELIST, "EC: Whitelist mint amount exceeded");
        }
