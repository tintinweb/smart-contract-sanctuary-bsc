...
interface IAlfaProfitSharing {
    ...
    function addEarnEx(
        ...
        uint256[] memory _epochStarts,
        uint256[] memory _epochEnds
    ) external returns (bool);
...
contract AlfaProfitSharing is IAlfaProfitSharing, Ownable {
    ...
    mapping(uint256 => uint256) public epochIndex; // epoch--->Index

    function setEpochIndex(uint256 _epoch, uint256 _index) public _onlyOwner {
        epochIndex[_epoch] = _index;
    }
        ...
        )
    {
        ...
        if (data.length > _index) {
            ...
            matured_ = data[_index].matured;
             epochStart = data[_index].epochStart;
              epochEnd = data[_index].epochEnd;
    ...
    function pendingFor(address _user, uint256 _index) public view returns (uint256 payout_, bool matured_) {
        ...
        //vg Gon
        uint256 epochStart = _earn.epochStart;
        uint256 epochEnd = _earn.epochEnd;

        if (epochEnd > 0 && epochStart > 0 && _earn.typeId == 1) {
            // to gAFA
            // payout_ = IvgAFA(_earn.vgAfa).balanceForGons(payoutGon_);
            uint256 indexStart = epochIndex[epochStart];
            uint256 indexEnd = epochIndex[epochEnd];
            uint256 indexDiff;
            if (indexEnd > 0 && indexStart > 0) {
                indexDiff = indexEnd - indexEnd;
                payout_ = _earn.payout.mul(indexDiff).div(1e18);
            } else {
                payout_ = 0;
            }
            ...
            if (!hasSubmit) {
                earns[_users[i]].push(
                    Earn({
                        ...
                        epochStart: _epochStarts[i],
                        epochEnd: _epochEnds[i],
                        redeemed: 0,
                        takeNumber: _takeNumber,
                        typeId: _typeId,
                        sub: _subs[i],
