...
// Website: https://apelonball.com
// Telegram: https://t.me/ApElonBall
// Twitter: https://twitter.com/ApElonBall
// Whitepaper: https://docs.apelonball.com/
...
/* Token contract */
contract APELONBALL is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x0439C459288321E9045C2209230866199B78bd9E;
    ...
    // Name and symbol
    string constant _name = "ApElon Ball";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 100 / 10000; // 10m tokens
    uint256 public _maxSellTxAmount = _totalSupply * 100 / 10000; // 10m tokens
    uint256 public _maxWalletToken = ( _totalSupply * 100 ) / 10000; // 10m tokens
    ...
    // FRIEZA
    uint256 FRIEZATriggeredAt;
    uint256 FRIEZADuration = 3600;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x536D487Aa4d35123eA74201d24b6ff43fcCc2fC0;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in FRIEZATime
        bool FRIEZAMode = inFRIEZATime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, FRIEZAMode) : amount;
    ...
    // Check if we are in FRIEZA time
    function inFRIEZATime() public view returns (bool){
        if(FRIEZATriggeredAt.add(FRIEZADuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the FRIEZA Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool FRIEZAMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are FRIEZA
        if (FRIEZAMode){
            if(isSell){
    ...
    // Enable FRIEZA
    function enableFRIEZA(uint256 _seconds) public authorized {
        FRIEZATriggeredAt = block.timestamp;
        FRIEZADuration = _seconds;
    }

    // Disable the FRIEZA mode
    function disableFRIEZA() external authorized {
        FRIEZATriggeredAt = 0;
    }
