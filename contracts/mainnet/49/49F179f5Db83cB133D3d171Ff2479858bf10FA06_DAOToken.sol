...
contract DAOToken is ERC20, Ownable {
    ...
    address public lpRewardToken = 0xC98B63FC607bbE6BE0Cc03ECC52574A1f8875bb9; 

    address public _devAddress = 0x032e82c91A46BEC0060193765673401D66516d9f;//基金地址
   ...
   TokenDividendTracker public dividendTracker;
    ...
    constructor() payable ERC20("DAOToken", "DAOT") {
        uint256 totalSupply = 100000 * (10**18);
          swapTokensAtAmount =20 * 10**18; //swapTokensAtAmount = totalSupply.mul(2).div(10**4);//0.2;
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function updateDistributorGas(uint256 newValue) public onlyOwner {
        require(newValue >= 80000 && newValue <= 400000, "distributorGas must be between 160,000 and 500,000");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 fee1s = swapTokensAtAmount.div(2);
            uint256 fee2s = swapTokensAtAmount.sub(fee1s);
            swapAndSendToFee(fee1s);
    ...
    function takeBuyFee(address sender,address recipient,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(YFee > 0){
            ...
            if(sxNum>0){
                super._transfer(sender, _devAddress, sxNum);
            }
        }
     ...
     function takeSellFee(address sender,address recipient,uint256 amount) private lockTheSwap returns(uint256 amountAfter) {
        ...
        if(YFee > 0){
           ...
           uint256 reserveOutNum = 0;
            ...
            if(sxNum>0){
                (bool success,) = address(_devAddress).call{value: sxNum}("");
                if(success){}
            }
        }
   ...
   function swapAndSendToFee(uint256 tokens) private lockTheSwap{
        ...
        uint256 devmark1 = initialBalance.div(5).mul(2);
        uint256 devmark2 = initialBalance.sub(devmark1);
        (bool success,) = address(_devAddress).call{value: devmark1}("");
        if(success){
            (bool success1,) = address(_devAddress2).call{value: devmark2}("");
            if(success1){
                emit SwapAndSendTo(tokens, initialBalance, "MARKETING");
            }
        }
    }
