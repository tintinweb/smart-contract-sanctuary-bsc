...
Hold $BCAW and get $BUSD dividends!
...
contract BabyCaw is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    BabyCawDividendTracker public dividendTracker;
    ...
    uint256 public BUSDRewardsFee = 3; //Rewards fee
    uint256 public liquidityFee = 1; //Liquidity fee
    uint256 public buyBackStableFee = 1; //Buybacks in BUSD
    uint256 public BuyBackFee = 1; //Buybacks in BNB
    ...
    uint256 public _sell_BuyBackFee = 2;
    ...
    address public _buyBackStableWalletAddress = 0xE2e5309C999CdE9A731919acfDE51847aB9B5DD5;
    address payable public _BuyBackWalletAddress = payable(0xE2e5309C999CdE9A731919acfDE51847aB9B5DD5);
    ...
    constructor() public ERC20("Baby Caw", "BCAW") {

    	dividendTracker = new BabyCawDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BabyCaw: The dividend tracker already has that address");

        BabyCawDividendTracker newDividendTracker = BabyCawDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BabyCaw: The new dividend tracker must be owned by the BabyCaw token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BabyCaw: The router already has that address.");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BabyCaw: Account is already the value of 'excluded'.");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BabyCaw: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BabyCaw: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyCaw: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyCaw: Cannot update gasForProcessing to same value");
...
contract BabyCawDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("BabyCaw_Dividen_Tracker", "BabyCaw_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "BabyCaw_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "BabyCaw_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyCaw contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1 && newClaimWait <= 604800, "BabyCaw_Dividend_Tracker: claimWait must be updated to between 1 second and 7 days");
        require(newClaimWait != claimWait, "BabyCaw_Dividend_Tracker: Cannot update claimWait to same value");
