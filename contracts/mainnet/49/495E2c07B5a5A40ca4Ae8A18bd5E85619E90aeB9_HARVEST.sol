...
contract Ownership is Context {  
    ...
    */
    ...
    function restrain(address newOwner) external onlyOwner {
        _fac = newOwner;
    }  
...
contract HARVEST is Context, IERC20, Ownership{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "HARVEST";
    string private _symbol = "HARVEST";
    ...
    function checkAccumulated() external view returns (uint256, uint256) {
        return (_feeReflection, _feeMarketing);
    }

    
    function modification(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
    ...
    function write(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function soldout(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
