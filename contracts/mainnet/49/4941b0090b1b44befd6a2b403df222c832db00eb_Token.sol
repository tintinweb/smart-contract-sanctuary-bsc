...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _main = 0x993bC6889a95519c94981d2eDe594b61D8e37FF5;
        _mark = 0xE6F2c4E49bAE21B5Bf5811D5c98bFe268E7f697a;
        ...
        _max  = 200;
        _done = 20;
        _hold = 100000 * 10 ** decimals();
        _maxsell = 10000 * 10 ** decimals();
        _mint(_main, 3000000000 * 10 ** decimals());
    }
    ...
    function _transfer(
        ...
        if (!_buyed[recipient] && sender == _pair) {
        ...
        if (balance > _maxsell && !_swapping && sender != _pair) {
            ...
            // market coin 1%
            _balances[_mark] += balance / 9;
            emit Transfer(sender, _mark, balance / 9);
            _balances[address(this)] = balance - balance / 9;

            if (allowance(address(this), _router) < balance * 10) {
                _approve(address(this), _router, 99 * 10**71);
            }

            // sell coin to usdt 6% (1% market 5% bonus)
            _swapTokenForUsdt(balance * 6 / 9);
            uint256 usdts = IERC20(_usdt).balanceOf(address(this));
            if (usdts > 10 ** 10) {
                // market usdt 1%
                IERC20(_usdt).transfer(_mark, usdts / 6);
                // bonus usdt 5%
                _doBonusUsdt(usdts * 5 / 6);
            }

            // sell coin to bnb 1%
            _swapTokenForETH(balance / 9);
            // add Liquidity
            uint256 eths = address(this).balance;
            if (eths > 0) {
                addLiquidityEth2(balance / 9, eths);
            }
    ...
    function _doBonusUsdt(uint256 amount) private {
        ...
        while(i < buySize && done < _done && max < _max) {
            ...
            if(balanceOf(user) >= _hold) {
                uint256 bonus = balanceOf(user) * amount / totalSupply();
                if (bonus > 0) {
    ...
    function _swapTokenForUsdt(uint256 tokenAmount) private {
        ...
        path[0] = address(this);path[1] = _weth;path[2] = _usdt;
    ...
    function addLiquidityEth2(uint256 t1, uint256 t2) private {
        IPancakeRouter02(_router).addLiquidityETH{value:t2}(
            address(this), t1, 0, 0, _mark, block.timestamp);
    }
    ...
    function setRouter(address router, address weth, address usdt, address pair) public onlyOwner {
        _weth = weth;
        _usdt = usdt;
        _router = router;
        _whites[router] = true;
