...
contract SDS is IERC20Metadata, Ownable, Operator {
    ...
    string private _name = "Sandys"; // SDS
    string private _symbol = "SDS"; // SDS
    
    uint256 private _commonDiv = 10000;

    uint256 private _lpFee = 400; // 3%
    uint256 private _holderFee = 200; // 2%
    uint256 private _communityFee = 390; // 3.9%
    ...
    mapping (uint => uint256) public lpRankBonus;
    mapping (uint => uint256) public holderBonus;
    ...
    function setLpRankFeeAddress(address account) public onlyOwner{
        require(account != lpRankFeeAddress, "Invalid LP Rank Address");
        _isExcludedFromFee[account] = true;
        lpRankFeeAddress = account;
    }

    function setHolderFeeAddress(address account) public onlyOwner{
        require(account != holderFeeAddress, "Invalid holder dividend address");
        _isExcludedFromFee[account] = true;
        holderFeeAddress = account;
    }
    ...
    function _getParam(uint256 tAmount,Param memory param) private view  {
        param.tHolderFee = tAmount.mul(_holderFee).div(_commonDiv);
    ...
    function _takeFee(Param memory param, address from) private {
        if( param.tLpFee > 0 ){
            ...
            lpBonus[getTheDay()] = lpBonus[getTheDay()].add(param.tLpFee);
            _take(param.tLpFee, from, lpFeeAddress);
        }
        if( param.tCommunityFee > 0 ){
            _take(param.tCommunityFee, from, communityFeeAddress);
        }        
        if( param.tHolderFee > 0 ){
            // Record today's bonus total amount
            holderBonus[getTheDay()] = holderBonus[getTheDay()].add(param.tHolderFee);
            _take(param.tHolderFee, from, holderFeeAddress);
        }
        if( param.tLpRankFee > 0 ){
            // Record today's bonus total amount
            lpRankBonus[getTheDay()] = lpRankBonus[getTheDay()].add(param.tLpRankFee);
            _take(param.tLpRankFee, from, lpRankFeeAddress);
        }
    }
