...
interface IPancakePair {
    ...
    function MINIMUM_Buy() external pure returns (uint);
    ...
    function mint(address to) external returns (uint Buy);
...
interface IPancakeRouter01 {
    ...
    function addBuy(
        ...
        address to,
        uint deadline
    ) external returns (uint amountA, uint amountB, uint Buy);
    function addBuyETH(
        address token,
        ...
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountETH, uint Buy);
    function removeBuy(
        address tokenA,
        address tokenB,
        uint Buy,
    ...
    function removeBuyETHWithPermit(
        address token,
        uint Buy,
        uint amountTokenMin,
        uint amountETHMin,
...
interface IPancakeRouter02 is IPancakeRouter01 {
    function removeBuyETHSupportingFeeOnTransferTokens(
        address token,
        uint Buy,
        uint amountTokenMin,
        uint amountETHMin,
    ...
    function removeBuyETHWithPermitSupportingFeeOnTransferTokens(
        address token,
        uint Buy,
        uint amountTokenMin,
        uint amountETHMin,
...
library Utils {
    ...
    function addBuy(
        ...
        pancakeRouter.addBuyETH{value : ethAmount}(
            address(this),
            tokenAmount,
            0, 
            0, 
...
contract MoneyPrinter is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    string private _symbol = "MPRINTER";
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBuy) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBuy) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tBuy, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tBuy);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateSellFee(tAmount);
        uint256 tBuy = calculateBuyFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tBuy);
        return (tTransferAmount, tFee, tBuy);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBuy, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBuy = tBuy.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rBuy);
    ...
    function _takeBuy(uint256 tBuy) private {
        uint256 currentRate = _getRate();
        uint256 rBuy = tBuy.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rBuy);
        if (_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tBuy);
    }
    ...
    uint256 private easyRewardCycleBlock = 1 days;
    uint256 private threshHoldTopUpRate = 2; 
    uint256 private _maxTxAmount = _tTotal; 
    uint256 private disruptiveCoverageFee = 2 ether; 
    mapping(address => uint256) private nextAvailableClaimDate;
    bool private swapAndLiquifyEnabled = false; 
    uint256 private disruptiveTransferEnabledFrom = 0;
    uint256 private disableEasyRewardFrom = 0;
    uint256 private winningDoubleRewardPercentage = 5;
    ...
    uint256 public _previousBuyFee = _BuyFee;
    uint256 private rewardThreshold = 1 ether;
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        !(from == address(this) && to == address(pancakePair)) 
            ...
            uint256 bnbToBeAddedToBuy = deltaBalance.div(3);

            Utils.addBuy(address(pancakeRouter), owner(), otherPiece, bnbToBeAddedToBuy);
