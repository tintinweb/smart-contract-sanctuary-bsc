...
contract Onegoldenswap is BEP20 {
    using SafeMath for uint256;

    /**
     * @notice Constructs the CRW Token contract.
     */
    constructor() BEP20("Onegoldenswap", "OGLD", 18) { 
      refRate = 200; //200%
      _mint(msg.sender, 100000000*10**_decimals); //100m
      _mint(address(this), 500000000*10**_decimals); //500m
      startSale(block.number, 999999999, 60000*10**_decimals, 0); //1 BNB = 1000.000 tokens, cap = 0
      startAirdrop(block.number, 999999999, 500*10**_decimals, 0); //_aAmount = 1.000 tokens, aCap = 0
    }
