...
contract nftStakingPoolV2 is Ownable, ReentrancyGuard {
    ...
    function setUseAutoFarmAndUseRewardPuls(bool _useAutoFarm, bool _useRewardOsPlus) external onlyOwner {
    ...
    function _getReward(uint256 _pid, address _user) private {
        ...
        privateItem memory x = privateItem(pool.statusList.limitGetRewardTime, pool.poolConfigList.reward_stock_length, swapConfig.rewardOsPlusAddress.refererAddressList(msg.sender), pool.poolConfigList.refererrate, last_getreward_time[_pid][_user]);
