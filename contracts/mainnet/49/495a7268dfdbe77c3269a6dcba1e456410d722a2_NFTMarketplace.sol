...
contract NFTMarketplace{
    ...
    function buyNFT(address _contractAddress, uint256 _tokenId) public payable returns (bool){
        ...
        payable(listed_nft.currentOwner).transfer(listed_nft.salePrice);
    ...
    function getListedNFTDetails(uint256 _listId) external view returns(address, uint256, uint256){
        nft memory listed_nft = nfts[_listId];
        require(listed_nft.listedTime > 0,'Invalid List Id');
        return (listed_nft.currentOwner, listed_nft.salePrice, listed_nft.listedTime);
    }
