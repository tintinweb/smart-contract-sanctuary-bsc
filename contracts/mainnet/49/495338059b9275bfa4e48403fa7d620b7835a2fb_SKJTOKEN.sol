...
contract SKJTOKEN is ERC20, Ownable  {
    mapping (address => bool) private _isExcludedFromFee;

    uint256 private constant total =  10 * 1e8 * 1e18;

    address public constant marketAddress = 0xd8246958C07bD0b448F7B08b7CC85B9eDee8b2a4;
    uint256 public constant _liquidityFee = 3;
    uint256 public constant _buymarketFee = 6;
    uint256 public constant _sellmarketFee = 12;
    ...
    uint256 public minDistribution = total / 1000;
    ...
    function _getFeeValues(uint256 _amount, bool sell) private pure returns (uint256,uint256,uint256) {
        uint256 liquidityFee = _amount * _liquidityFee / 100;

        uint marketFeePercent =  sell ? _sellmarketFee: _buymarketFee;
        uint256 marketFee = _amount * marketFeePercent / 100;
        
        uint256 transferAmount = _amount - liquidityFee - marketFee;
        return (transferAmount, liquidityFee, marketFee);
    }
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(amount >= 1e16 && _balances[address(this)] >= amount) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(takeFee){
            (uint256 tTransferAmount, uint256 liquidityFee, uint256 marketFee) = _getFeeValues(amount, recipient == pancakeswapV2Pair);
