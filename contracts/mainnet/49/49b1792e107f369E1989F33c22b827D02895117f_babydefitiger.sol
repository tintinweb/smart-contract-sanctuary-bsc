...
contract babydefitiger is Context, IERC20, Ownable {
    ...
    // exclude an address from max wallet can hold
    ...
    uint256 internal _tokenAmount = 1 * 10**12 * 10**9;
    uint256 public _maxTxAmount = _tokenAmount;
    uint256 public _maxWalletAmount = _tokenAmount;
    uint256 private numTokensSellToAddToLiquidity = _tokenAmount;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = _tokenAmount;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "babydefitiger";
    string private _symbol = "$BDTG";
    ...
    address public liquidityFeeAddress =
        0x0e8c3Dc8Cd409D46381308f5B9569b5A74A76BFe;
    ...
    constructor() public {
       ...
       //  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xc5f442007e08e3b13C9f95fA22F2a2B9369d7C8C); // Kuswap Testnet
        // Create a uniswap pair for this new token
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _uniswapV2Router.WETH());

        // // set the rest of the contract variables
        // uniswapV2Router = uniswapV2Pair;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[liquidityFeeAddress] = _rOwned[liquidityFeeAddress].add(
            rLiquidity
        );
        if (_isExcluded[liquidityFeeAddress])
            _tOwned[liquidityFeeAddress] = _tOwned[liquidityFeeAddress].add(
                tLiquidity
            );
    }
    ...
    function _transfer(
        ...
        // check if both addresses are excluded from max wallet
        ...
        //if account sending belongs from _isExcludedFromFee account then remove the fee
