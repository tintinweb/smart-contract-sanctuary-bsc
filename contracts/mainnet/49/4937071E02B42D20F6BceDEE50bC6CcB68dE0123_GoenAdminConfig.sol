...
contract GoenAdminConfig is IGoenAdminConfig, Ownable {
    ...
    function schedule() external override view returns (address) {
        return _schedule; 
    }

    function setSchedule(address schedule_) external onlyOwner {
        _schedule = schedule_;
    }

    function setGoenReleased() external override {
        require(msg.sender == _schedule, "caller is not schedule");
