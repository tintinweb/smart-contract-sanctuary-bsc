...
contract staking is Ownable {
    ...
    function withdrawRefferalReward(address token)public {
        ...
        if(token==tokens[0]){
            ...
            require(totalDeposite<=getContractBUSDBalacne(),"Not Enough Token for withdrawal from contract please try after some time");
            totalWithdrawBUSD+=totalDeposite;
            investor[msg.sender].refferalRewardsBUSD=0;
        }else{
            totalDeposite=getUserTotalRefferalRewardsWBNB(msg.sender);
            require(totalDeposite>0,"No WBNB Deposit Found");
            require(totalDeposite<=getContractWBNBBalacne(),"Not Enough WBNB for withdrawal from contract please try after some time");
    ...
    function withdrawTokensBUSD(uint256 id)public {
        ...
        require (investor[msg.sender].deposites[id].isToken,"Not A BUSD Deposit");
    ...
    function withdrawTokensWBNB(uint256 id)public {
        require (id <= investor[msg.sender].deposites.length,"Invalid Id");
        require (!investor[msg.sender].deposites[id].isToken,"Not A WBNB Deposit");
        uint256 totalDeposite=investor[msg.sender].deposites[id].amount;
        require(totalDeposite>0,"No Deposit Found");
        require(totalDeposite<=getContractWBNBBalacne(),"Not Enough Token for withdrawal from contract please try after some time");
        uint256 tax=totalDeposite.mul(withdrawTax).div(divider);
    ...
    function withdrawRewardBUSD()public {
        ...
        require(totalRewards<=getContractBUSDBalacne(),"Not Enough Token for withdrawal from contract please try after some time");
    ...
    function withdrawRewardWBNB()public {
        (,uint256 totalRewards)=calclulateReward(msg.sender);
        require(totalRewards>0,"No Rewards Found");
        require(totalRewards<=getContractWBNBBalacne(),"Not Enough Token for withdrawal from contract please try after some time");
        uint256 tax=totalRewards.mul(rewardTax).div(divider);
        uint256 taxR=totalRewards.mul(withdrawRTax).div(divider);
        totalDevRewardsWBNB+=tax;
