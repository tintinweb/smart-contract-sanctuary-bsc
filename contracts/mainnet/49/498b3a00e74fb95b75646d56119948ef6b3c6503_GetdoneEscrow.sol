...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    event ApplistUpdated(uint256 merchantId, uint256[] appIds, address wallet);
    event TransactionFeePercentUpdated(uint256 merchantId, uint256 appId, uint256 percent);
    event MerchantUpdated(uint256 merchantId, address wallet);
    ...
    // merchantId => adminAddress
    mapping(uint256 => address) public merchants;
    // merchantId => appId => address
    mapping(uint256 => mapping(uint256 => address)) public apps;
        ...
        initializer
    {
        ...
        merchants[MERCHANT_ID_GETDONE] = defaultAdminWallet;
        minDisputationTime = 300 seconds;
    }
    ...
    function validMerchant(uint256 merchantId) internal view returns (bool) {
        return merchants[merchantId] != address(0);
    }

    function validApp(uint256 merchantId, uint256 appId) internal view returns (bool) {
        return validMerchant(merchantId) && apps[merchantId][appId] != address(0);
    }

    function adminWallet(uint256 merchantId, uint256 appId) internal view returns(address) {
        return appId != 0 && apps[merchantId][appId] != address(0) ? apps[merchantId][appId] : merchants[merchantId];
    }
    ...
    function updateMerchant(uint256 merchantId, address wallet)
        public
        onlyOwner
    {
        ...
        emit MerchantUpdated(merchantId, wallet);
    }

    function updateApp(uint256 merchantId, uint256[] memory appIds, address wallet)
        public
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            apps[merchantId][appIds[i]] = wallet;
        }
        emit ApplistUpdated(merchantId, appIds, wallet);
    }
        ...
        nonReentrant
    {
        ...
        if (amountRefund > 0 && amountRefund <= contractBalance) {
            contractInfo.refund = contractInfo.refund + amountRefund;
            if (contractInfo.erc20 == address(0)) {
                payable(adminWallet(contractInfo.merchantId, contractInfo.appId)).transfer(amountRefund);
            } else {
                address erc20 = contractInfo.erc20;
                IERC20Upgradeable(erc20).safeTransfer(adminWallet(contractInfo.merchantId, contractInfo.appId), amountRefund);
            }
        }
