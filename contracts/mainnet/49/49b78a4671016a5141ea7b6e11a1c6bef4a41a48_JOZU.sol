...
contract JOZU is ERC20 {
	...
	address public _fundAddress = address(0x35336FFAC1E92800a1B1A5975052612F9Fc4A6d6);
    address public _JozuAddress = address(0x1db02d271f269CDC51dD74e9b690297ba5441AFd);
	address public _robotAddress = address(0xa64950Cec9f74A848d832525371eFbA31673a783);
    ...
    constructor(address tokenOwner) ERC20("JOZU", "JOZU") {
	...
	uint256 public robotTime = 303;
	function changeRobotTime(uint256 _robotTime) public onlyOwner {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				if(startTime.add(robotTime) > block.timestamp){
					super._transfer(from, _robotAddress, amount.div(100).mul(99));
					amount = amount.div(100);
				}
				require(balanceOf(to).add(amount) <= 10**23,"max amount");
        ...
        if (takeFee) {
			...
			super._transfer(from, address(this), amount.div(100).mul(tFee));
			_takeInviterFeeKt(amount.div(10000));
			amount = amount.div(100).mul(100 - tFee);
        }
        super._transfer(from, to, amount);
    }
	...
	function swapAndLiquify() public {
		...
		if(allAmount > 0){
			USDT.transfer(_fundAddress,allAmount.div(6).mul(2));
			USDT.transfer(_JozuAddress,allAmount.div(6).mul(4));
		}
    }
