...
contract MrGreenJackpot is IBEP20 {
    ...
    uint256 public minStakeForTicket = 50_000 * (10**_decimals);
    
    uint256 public priceOfTicketBurnLottery = 25_000 * (10**_decimals);
    uint256 public maxSecondaryJackpot = 500_000 * (10**_decimals);


    uint256 private launchTime;
    uint256 private totalJackPotsPaidOut;
    ...
    function _basicTransfer(
        ...
        if (!addressExcludedFromJackpot[recipient]) handleJackpotFunctions(recipient);
    ...
    function setTokens(address holder) internal {
        if (tokens[holder] < minTokensForRewards && _balances[holder] + stakingAmounts[holder] >= minTokensForRewards) addHolder(holder);
        if (tokens[holder] >= minTokensForRewards && _balances[holder] + stakingAmounts[holder] < minTokensForRewards) removeHolder(holder);
        if (_balances[holder] + stakingAmounts[holder] >= minTokensForRewards) tokens[holder] = _balances[holder] + stakingAmounts[holder];
    }

    function checkAllHolders() internal {
        uint256 holderCount = eligibleForJackpot.length;
        if (holderCount <= holdersToCheckPerTx) return;
        ...
        for (
            ...
            setTokens(eligibleForJackpot[currentIndex]);
            currentIndex++;
        }
    }
...
/////////////////// ADMIN FUNCTIONS ///////////////////////////////////////////////////////////////////////
 ...
 /////////////////////////Testing////////////////////////////////////////   
    ...
    function setJackpotToken(address newJackpotToken) external onlyOwner {
        ...
        address jackpotPair = IDEXFactory(IDEXRouter(router).factory()).getPair(WETH, address(this));
    ...
    function setHolderCheckingParameters(uint256 _holdersToCheckPerTx) external onlyOwner {
    ...
    function setAddressWithoutLimits(address unTaxedAddress, bool status) external onlyOwner {
    ...
    function participateInBurnLottery(uint256 tokensToSend) external {
        ...
        require(tokensToSend >= 25_000, "Minimum tokens not reached");
        
        // make sure the amount is a multiple of 25k
        if(tokensToSend % 25_000 != 0) tokensToSend -= tokensToSend % 25_000;
        
        // add decimals
        tokensToSend *= (10**_decimals);

        // make sure the jackpotBalance never surpasses 500k
        if(secondaryJackpotBalance + tokensToSend > maxSecondaryJackpot) {
            tokensToSend = maxSecondaryJackpot - secondaryJackpotBalance;
        }
        ...
        uint256 tickets = tokensToSend / priceOfTicketBurnLottery;
        for(uint256 i= 1; i<=tickets; i++) secondaryJackpotPlayers.push(msg.sender);

        // if the jackpot is full, draw a winner
        if(secondaryJackpotPlayers.length >= 20) drawWinnerOfSecondaryJackpot();
    }
    ...
    function stake(uint256 tokensToStake) external {
        ...
        uint256 tickets = tokensToStake / 50_000;
    ...
    function returnStakingAmountsToStakers() internal {
        uint256 stakerCount = stakingPlayers.length;

        for (uint256 stakersChecked = 0; stakersChecked < stakersToCheckPerTx; stakersChecked++) {
            
            if (currentStakerIndex >= stakerCount) {
                allStakersChecked = true;
                currentStakerIndex = 0;
                return;
            }

            _basicTransfer(address(this),stakingPlayers[currentStakerIndex], minStakeForTicket);
            stakingPoolBalance -= minStakeForTicket;
            currentStakerIndex++;
        }
    }
