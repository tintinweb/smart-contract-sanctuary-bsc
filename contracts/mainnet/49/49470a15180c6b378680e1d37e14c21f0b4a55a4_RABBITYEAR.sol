...
contract RABBITYEAR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RABBITYEARDividendTracker public dividendTracker;
    ...
    constructor() ERC20("HAPPY CHINESE NEW YEAR", "RABBIT YEAR") {
        ...
        dividendTracker = new RABBITYEARDividendTracker(
            payable(this),
            router,
...
contract RABBITYEARDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RABBITYEAR public RABBITYEARContract;
    ...
    constructor(
        ...
        defaultToken = token;
        RABBITYEARContract = RABBITYEAR(mainContract);
    ...
    function _transfer(
        ...
        require(false, "RABBITYEAR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RABBITYEAR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RABBITYEAR contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(RABBITYEARContract);

            uint256 prevBalance = RABBITYEARContract.balanceOf(address(this));
            ...
            {
                uint256 received = RABBITYEARContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RABBITYEARContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
