...
contract Staking is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    struct UserInfo {
        ...
        uint256 stakedMDEAmount;

        uint256 rewardPerNFTPaidMDE;
        uint256 rewardPerNFTPaidAnto;

        uint256 rewardMDE;
        uint256 rewardAnto;

        uint256 claimedRewardMDE;
        uint256 claimedRewardAnto;
    }

    modifier updateReward(address _account) {
        (rewardPerNFTStoredMDE, rewardPerNFTStoredAnto) = _rewardPerNFT();
        
        lastUpdateTimeAnto = block.timestamp;
        reserveMDE = IERC20Upgradeable(MDE_ADDRESS).balanceOf(address(this));

        if (_account != address(0)) {
            (uint256 _rewardMDE, uint256 _rewardAnto) = _earned(_account);
            UserInfo storage userInfo = userInfoOf[_account];
            
            userInfo.rewardMDE = _rewardMDE;
            userInfo.rewardAnto = _rewardAnto;

            userInfo.rewardPerNFTPaidMDE = rewardPerNFTStoredMDE;
    ...
    function _rewardPerNFT() private view returns (uint256, uint256) {
        uint256 _rewardPerNFTStoredMDE = rewardPerNFTStoredMDE;
        uint256 _rewardPerNFTStoredAnto = rewardPerNFTStoredAnto;
        if (totalSupplyNFT > 0) {
            // calc MDE reward
            uint256 deltaMDE = IERC20Upgradeable(MDE_ADDRESS).balanceOf(address(this)) - reserveMDE;
            if (deltaMDE > 0) {
                _rewardPerNFTStoredMDE += deltaMDE * 1 ether / totalSupplyNFT;
            }
        ...
        return (_rewardPerNFTStoredMDE, _rewardPerNFTStoredAnto);
    }

    function _earned(address _account) private view returns (uint256 _rewardMDE, uint256 _rewardAnto) {
        (uint256 _rewardPerNFTStoredMDE, uint256 _rewardPerNFTStoredAnto) = _rewardPerNFT();

        uint256 _miningPower = userInfoOf[_account].stakedNFTIds.length;
        _rewardMDE = userInfoOf[_account].rewardMDE + (_rewardPerNFTStoredMDE - userInfoOf[_account].rewardPerNFTPaidMDE) * _miningPower / 1 ether;
    ...
    function stake(uint256 _num) external updateReward(msg.sender) {
        uint256 _mdeAmount = _num * stakedMDEPerNFT;
        uint256 _heysheepBalance = IERC721Upgradeable(HEYSHEEPII_ADDRESS).balanceOf(msg.sender);
        uint256 _mdeBalance = IERC20Upgradeable(MDE_ADDRESS).balanceOf(msg.sender);
        require(_heysheepBalance >= _num && _mdeBalance >= _mdeAmount, 'Staking: insufficient balance.');

        totalSupplyNFT += _num;
        userInfoOf[msg.sender].stakedMDEAmount += _mdeAmount;
    ...
    function unstake(uint256 _num) external updateReward(msg.sender) {
        ...
        uint256 _mdeAmount = 0;
        uint256 _stakedMDEAmount = userInfoOf[msg.sender].stakedMDEAmount;
        uint256 _keepMDEAmount = (_stakedNFTNum - _num) * stakedMDEPerNFT;
        if (_stakedMDEAmount > _keepMDEAmount) {
            _mdeAmount = _stakedMDEAmount - _keepMDEAmount;
        }
    ...
    function claimReward() external updateReward(msg.sender) {
        (uint256 _rewardMDE,) = _earned(msg.sender);
        if (_rewardMDE > 0) {
            reserveMDE -= _rewardMDE;
            userInfoOf[msg.sender].rewardMDE = 0;
            userInfoOf[msg.sender].claimedRewardMDE += _rewardMDE;
    ...
    (
        ...
        uint256 _stakedMDEAmount,
        uint256 _claimableRewardMDE,
        uint256 _claimableRewardAnto,
        uint256 _totalRewardMDE,
        ...
        _stakedMDEAmount = userInfoOf[_account].stakedMDEAmount;

        (_claimableRewardMDE, _claimableRewardAnto) = _earned(_account);
        _totalRewardMDE = userInfoOf[_account].claimedRewardMDE + _claimableRewardMDE;
