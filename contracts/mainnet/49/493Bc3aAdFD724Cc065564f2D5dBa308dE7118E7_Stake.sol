...
contract Stake is Ownable{
    ...
    address public sns = address(0);
    address public szns = address(0);

    address public usdtReceiver = address(0);

    bool public init = false;

    uint256[] public referRewardRate = [20,10,10,10,10,3,3,3,3,3,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2];
    ...
    constructor(address _usdt, address _sns, address _szns) public {

        usdt = _usdt;
        sns = _sns;
        szns = _szns;

        usdtReceiver = address(0xc77090a015963E3b1597846FC70914eF014e8A0E);
        ...
        poolInfo[0].token1 = sns;
        poolInfo[0].awardToken = sns;
        poolInfo[0].awardPerDay = 575342 * 1e16;
    ...
    function addToken2(uint8 pid, uint amount) public returns (bool) {        
        ...
        require(BEP20(szns).balanceOf(msg.sender) >= amount, 'szns balance insufficient');
        require(BEP20(szns).allowance(msg.sender, address(this)) >= amount, 'szns allow insufficient');
        //收币
        TransferHelper.safeTransferFrom(szns, msg.sender, destroyAddress, amount);
