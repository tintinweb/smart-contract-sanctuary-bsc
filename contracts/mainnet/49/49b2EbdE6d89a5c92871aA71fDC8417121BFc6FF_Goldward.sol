...
contract Goldward is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        if (rewardAddress_ == address(0)) {
            require(
                rewardFeeBps_ == 0,
                "Cant set both reward address to address 0 and reward percent more than 0"
            );
        }
        require(
            taxFeeBps_ + liquidityFeeBps_ + rewardFeeBps_ <= MAX_FEE,
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _rewardFee <= MAX_FEE,
            "Total fee is over 25%"
        );
    }
    ...
    function setrewardFeePercent(uint256 rewardFeeBps) external onlyOwner {
        _rewardFee = rewardFeeBps;
        require(
            _taxFee + _liquidityFee + _rewardFee <= MAX_FEE,
            "Total fee is over 25%"
        );
        ...
        )
    {
        ...
        return (tTransferAmount, tFee, tLiquidity, trewardFee);
    }
    ...
    function _takerewardFee(uint256 treward) private {
        if (treward > 0) {
            uint256 currentRate = _getRate();
            uint256 rreward = treward.mul(currentRate);
            _rOwned[_rewardAddress] = _rOwned[_rewardAddress].add(rreward);
            if (_isExcluded[_rewardAddress])
                _tOwned[_rewardAddress] = _tOwned[_rewardAddress].add(
                    treward
                );
            emit Transfer(_msgSender(), _rewardAddress, treward);
        }
    }
    ...
    function calculaterewardFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_rewardAddress == address(0)) return 0;
