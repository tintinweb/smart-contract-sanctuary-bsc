...
contract DarumaToken is IERC20 {
   ...
   mapping (address => bool) private BotAddress;
    ...
    constructor() {  
       ...
       _maxTxAmont = 1 *10**7 * 10**_decimals; 
    ...
    function _transfer(address from, address to , uint256 amount) internal returns(bool) {
        ...
        if(!ExcloudAddress[from] || !ExcloudAddress[to]) {
             require(amount <= _maxTxAmont);
        }
        ...
        if(_ismobility) {
            if(startblock.add(3) > block.number && !ExcloudAddress[from] && !ExcloudAddress[to]) {
