...
contract Coin4Weed is Context, IERC20, Ownable {
    ...
    address payable public Coin4WeedFundAddress = payable(0xd122FD86aB5f490e6D72121C8FAcCb1E04d6F3E7);
    ...
    uint256 private _tTotal = 990000000000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private constant _name = "Coin4Weed";
    string private constant _symbol = "Coin4Weed";
    ...
    uint256 public _buyBackSize = 1000; // 1 %
    ...
    event SetCoin4WeedFundAddress(address newAddress);
    ...
    function setCoin4WeedFundAddress(address newAddress) external onlyOwner() {
        Coin4WeedFundAddress = payable(newAddress);

        emit SetCoin4WeedFundAddress(Coin4WeedFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(Coin4WeedFundAddress, newBalance);
