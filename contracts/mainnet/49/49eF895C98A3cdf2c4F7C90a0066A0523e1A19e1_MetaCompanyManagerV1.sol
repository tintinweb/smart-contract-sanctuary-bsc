...
contract MetaCompanyManagerV1 is
    ...
    PausableUpgradeable
{
    ...
    function createFranchise(address _account, uint256 _number) external {
        if (!ceos[_account]) {
            this.createCompany(_account);
        }
        metaFranchiseManager.createMetaFranchiseUsingTokens(
            address(_account),
    ...
    function createMetaFranchiseUsingUnclaimed(uint256 _number)
        external
        payable
    {
        require(ceos[msg.sender], "not is ceo");
        metaFranchiseManager.createMetaFranchiseUsingUnclaimed(
    ...
    function getCreateFranchisePrice() external view returns (uint256) {
        return metaFranchiseManager.getCreateFranchisePrice();
    }
    ...
    function getNumberOfMetaFranchisesTokens(address account)
        external
        view
        returns (uint256)
    {
        return
            metaFranchiseManager.getNumberOfMetaFranchisesTokens(
                this.getCompanyId(account)
            );
    }
...
interface IMetaFranchiseManager {
    ...
    function createMetaFranchiseUsingUnclaimed(
        address _to,
        uint256 _companyId,
