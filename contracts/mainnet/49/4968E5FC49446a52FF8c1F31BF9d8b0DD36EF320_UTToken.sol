...
contract UTToken is ERC20, Ownable,ReentrancyGuard {
    ...
    event SetChargeFee(uint256 _inFee, uint256 _outFee, address _feeTo);
    ...
    event EmergencyWithdraw( address token, address to );
    event EmergencyWithdrawNative( address to , uint256 amount);

    constructor(string memory name,string memory symbol,uint256 _chargeInFee,uint256 _chargeOutFee,address _feeTo)  ERC20(name, symbol){
    ...
    function setChargeFee(uint256 _chargeInFee,uint256 _chargeOutFee,address _feeTo) external onlyOwner{
        ...
        emit SetChargeFee( _chargeInFee, _chargeOutFee , _feeTo);
    }
    ...
    function deposit(address _token,uint amountToken,address _to) external nonReentrant returns(uint256 amountU){
    ...
    function withdraw(address _token,uint256 amountU,address _to) external nonReentrant returns(uint256 amountToken) {
        require(amountU>MINIMAL_U,'amount to low');
    ...
    function withdrawEmergency(address tokenAddress,address to) public onlyOwner{	
        ...
        emit EmergencyWithdraw( tokenAddress, to);
    }
