...
// Dice Roll
    ...
    function resetHighCard() internal {
        HighCard storage user = highCard[msg.sender];
            bool[] memory cards = new bool[](52);
            user.cards = cards;
            user.currentCardNumber = 0;
            user.currentCardSuit = 0;
            user.multiplier = 5;
            user.currentBet = 0;
    ...
    event HighCardGuess(address player, uint256 LastCardSuit, uint256 LastCardNumber, uint256 newCardSuit, uint256 newCardNumber, bool isWinner, uint256 multiplier);
    ...
    function highCardGuess(bool guessHigh) external returns (bool resultHigh, uint256 LastCardSuit, uint256 LastCardNumber, uint256 newCardSuit, uint256 newCardNumber, bool isWinner ) {
        HighCard storage user = highCard[msg.sender];
        require(user.gameStarted, "StartNewGame to play again");
        // require(user.currentBet * user.multiplier / 10 <= maxBetAllowed(), "above max bet, claim and start new game");
        ...
        emit HighCardGuess( msg.sender, LastCardSuit,  LastCardNumber,  newCardSuit,  newCardNumber,  isWinner, user.multiplier);
        return ( resultHigh, LastCardSuit,  LastCardNumber,  newCardSuit, newCardNumber, isWinner);
    ...
    function highCardTakeMoney() public returns( uint256 amount) {
        ...
        // amount = sendIsWinner(user.winnings, user.multiplier);
