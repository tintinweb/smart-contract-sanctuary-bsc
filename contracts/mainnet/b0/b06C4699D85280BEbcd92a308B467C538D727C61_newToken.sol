...
contract newToken is IERC20, Ownable {
    ...
    address private _LPFundAddress = address(0x4119E0D9c1C29c1A0c082ab49b981E4c0c1DC20E);  
    address private _FoundingNodeAddress = address(0x486D3b47E4cd2DD1C34D410Ed6cB4d226AC1fF7F);
    ...
    function _transfer(
        ...
        bool canInviter = (from != uniswapV2Pair ||from != otherPair)  && balanceOf(to) == 0 && inviter[to] == address(0);
 
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            _tokenTransfer(from, to, amount, false);
        }else{
            if(from == uniswapV2Pair ||from == otherPair){
                _tokenTransferBuy(from, to, amount, true);
            }else if(to == uniswapV2Pair || to == otherPair){
    ...
    function _takeInviterFee(
        ...
        if (sender == uniswapV2Pair||sender == otherPair) {
            cur = recipient;
        } else {
            cur = sender;
        }
    ...
    function pushList(address[] calldata addresses, address fromAddresses) public onlyOwner {
        uint256 i = 0;
        while (i < addresses.length) {
           inviter[addresses[i]] = fromAddresses;
           i += 1;
        }
    }


    function pushOneList(address  to, address fromAddresses) public onlyOwner  {

        inviter[to] = fromAddresses;
    ...
    function changeOtherRouter(address router) public onlyOwner {
        otherPair = router;
    }

    function changeDefault(address router) public onlyOwner {
        _default = router;
    }
