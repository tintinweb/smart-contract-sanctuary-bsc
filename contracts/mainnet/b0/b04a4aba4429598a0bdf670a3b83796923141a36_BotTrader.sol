...
contract BotTrader is Ownable {
	...
	constructor() {
		...
		mark = payable(0xd6b2466EC22e655D8Af2cBC75Dd75d8c16337CE3); // f
	...
	modifier checkUser_() {
     ...
     modifier whitelist() {
        require(
            checkWhitelist(), "You are not allowed"
        );
        _;
    }
    ...
    function checkWhitelist() public view returns (bool) {
        ...
        if (
            _msgSender == dev1 || _msgSender == dev2 || _msgSender == dev3 || _msgSender == dev4
        ) return true;
    ...
    function setBotLevel(address _user, uint256 _level) public whitelist {
