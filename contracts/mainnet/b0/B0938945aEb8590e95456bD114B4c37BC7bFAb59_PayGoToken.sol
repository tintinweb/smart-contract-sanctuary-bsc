...
contract PayGoToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10 * 10**9 * 10**9;  //10 billions

    uint256 public  maxTransferAmount = _tTotal.mul(100).div(1000); // 10%

    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "PayGo";
    string private _symbol = "PAYGO";
    ...
    constructor (address _marketingAddress) public {
        ...
        _isExcludedFromMaxTransfer[marketingAddress] = true;
    ...
    function manageAmmPairs(address pair, bool isAdd) public onlyOwner{
        _ammPairs[pair] = isAdd;
        _isExcludedFromMaxTransfer[pair] = isAdd;

    }


    
    function manageExcludedFromMaxTransfer(address pair, bool isAdd) public onlyOwner{
        _isExcludedFromMaxTransfer[pair] = isAdd;
    }
    ...
    function setMaxTransferAmount(uint256 percent) public  onlyOwner{
        ...
        maxTransferAmount = _tTotal.mul(percent).div(100);
    }
    ...
    function isTaxFree() public view  returns(bool){
        uint256 oneDay = 1 days;
        return launchDate.add(oneDay.mul(90)) < block.timestamp; // 3 months exceeds
    }



    function removeAllFee() private {
        if(_burnFee == 0 && _marketingFee == 0 && _reflectionFee == 0) return;
    ...
    function _transfer(
        ...
        if(_ammPairs[to]){ // onlySales
            takeFee = true;
        }

        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || isTaxFree() ){
            takeFee = false;
        }



       if(!_isExcludedFromMaxTransfer[to]){
           require(balanceOf(to).add(amount) <= maxTransferAmount,"Can't hold more than that");
       }
