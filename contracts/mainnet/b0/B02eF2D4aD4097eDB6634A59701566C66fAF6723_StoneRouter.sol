...
contract Stone is ERC721Enumerable, ReentrancyGuard {
    ...
    constructor(address _token, string memory _name, string memory _symbol) ERC721(_name, _symbol) {
        token = _token;
    }

    function mint(address to, uint256 unlockTime) public nonReentrant {
        uint256 balance = balanceOf();
        require(balance >= reserve, "balance-lt-reserve");
        uint256 amountIn = balance - reserve;
        require(amountIn > 0, "amountin-zero");

        uint256 tokenId = ++lastMinted;
    ...
    function burn(uint256 tokenId) public nonReentrant {
        ...
        require(ownerOf(tokenId) == address(msg.sender), "not-owner-call");
        require(block.timestamp >= stone.unlockTime, "not-unlocked-stone");

        _update(reserve - stone.value);
        _burn(tokenId);
    }

    function withdraw(address to) public nonReentrant returns (uint256) {
        uint256 balance = balanceOf();
        require(balance >= reserve, "balance-lt-reserve");
        uint256 amountIn = balance - reserve;
        require(amountIn > 0, "amountin-zero");
...
contract StoneRouter {
    ...
    function createStone(address token) public returns (address) {
        require(map[token] == address(0), "already-exists-stone");
        string memory tokenSymbol = IERC20(token).symbol();
        string memory name = string(abi.encodePacked("STONE-", tokenSymbol));
        string memory symbol = string(abi.encodePacked("S", tokenSymbol));
        Stone stone = new Stone(token, name, symbol);

        list.push(address(stone));
        map[token] = address(stone);
        emit NewStone(address(stone), token);
        return address(stone);
    }

    function createOrGetStone(address token) public returns (address stone) {
        if(map[token] == address(0)) {
    ...
    function deposit(address token, address to, uint256 value, uint256 unlockTime) external {
        address stone = createOrGetStone(token);
        token.safeTransferFrom(msg.sender, stone, value);
        IStone(stone).mint(to, unlockTime);
    }
    
    function withdraw(address token, uint256 tokenId, address to) external {
        address stone = createOrGetStone(token);
        IERC721(stone).safeTransferFrom(msg.sender, address(this), tokenId, "");
    ...
    function onERC721Received(address operator, address, uint256 tokenId, bytes calldata) external returns (bytes4) {
        address target = IStone(msg.sender).token();
        require(msg.sender == map[target], "non-stone-caller");

        IStone(msg.sender).burn(tokenId);
        IStone(msg.sender).withdraw(operator);
