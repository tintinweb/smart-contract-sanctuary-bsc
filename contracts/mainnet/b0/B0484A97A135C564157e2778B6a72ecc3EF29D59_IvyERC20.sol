...
/**
 * @title Ivy (IVY) ERC20 token
 *
 * @notice Ivy is a core ERC20 token powering the game.
 *      It serves as an in-game currency, is tradable on exchanges,
 *      it powers up the governance protocol (Ivy DAO) and participates in Yield Farming.
 *
 * @dev Token Summary:
 *      - Symbol: IVY
 *      - Name: Ivy
 *      - Decimals: 18
 *      - Initial token supply: 4,000 IVY
 *      - Maximum final token supply: 10,000 IVY
 *          - Up to 6,000 IVY may get minted in 120 days period via yield farming
 *
 ...
 * @author Basil Gorin
 */
contract IvyERC20 is AccessControl {
  /**
   ...
   */
  string public constant name = "Ivy";

  /**
   * @notice Symbol of the token: IVY
   *
   ...
   */
  string public constant symbol = "IVY";
   ...
   */
  uint256 public totalSupply; // is set to 4000 * 10^18 in the constructor
  ...
  constructor(address _initialHolder) {
    ...
    // mint initial supply 4000
    mint(_initialHolder, 4_000e18);
  }
  ...
  function mint(address _to, uint256 _value) public {
    ...
    // max supply 10000
    require(totalSupply + _value <= 10_000e18, "total supply cap 10000");
