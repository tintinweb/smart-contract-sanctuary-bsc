...
interface stakeCon
{
  ...
  function balanceOfPower(address account) external view returns (uint256);
  function inviteWithdraw(address ac,uint amounts) external;
...
contract PlayerBook is Governance {
    ...
    address public _stakeCon;
    mapping (address => bool) public addrExist;

    bool public isinvite = false;

    constructor(address _usdbToken,address _rewardToken) public {
        usdt = _usdbToken;
    ...
    function addStakeAdddr(address _newstakeCon)
        onlyGovernance
        public
    {
        _stakeCon = _newstakeCon;
    }
    ...
    function stakeregister(address ac)
        public
    {
        require(msg.sender == _stakeCon,"not stakeCon");
        require(addrExist[ac],"not exist");
        require(_plyr[ac].laff != address(0), "sorry not register");

        _hasInvite[_plyr[ac].laff] = _hasInvite[_plyr[ac].laff].add(1);
    }
        ...
        returns (uint256)
    {
        ...
        for(uint i=0;i<_referRewardRate.length;i++)
        {
            ...
            curaddr = affID;
            affReward = (amount.mul(_referRewardRate[i])).div(_baseRate);
            if(affID == address(0x0) ){
                affID = _teamWallet;
            }
            if(_hasInvite[affID] < i)
            {
                if(affID == _teamWallet)
                {
                    break;
                }
                continue;
            ...
            callbackpath[0] = rewardToken;
            callbackpath[1] = usdt;
            uint[] memory invpower = IUniswapV2Router02(_uniswapV2Router).getAmountsOut(affReward, callbackpath);
            if(stakeCon(_stakeCon).balanceOfPower(affID) >= invpower[invpower.length - 1])
            {
                stakeCon(_stakeCon).inviteWithdraw(affID,invpower[invpower.length - 1]);
            }else{
                if(affID == _teamWallet)
                {
                    break;
    ...
    {
        address addr = msg.sender;
        uint256 reward = _plyr[addr].rreward;

        require(reward > 0,"only have reward");
        // require(stakeCon(_stakeCon).balanceOfPower(msg.sender) > 0,"only have stake");

        _plyr[addr].rreward = 0;
    ...
    {
        return (_plyr[from].laff,_plyr[from].rreward);
    }
