...
contract AutomatedBUSD is Context, Ownable, IERC20 {
    ...
    function drawTopDepositWinner() private {
  	...
  	function reinvest() public isInitialized {
        ...
        for (uint i = 0; i < user.depoList.length; i++){
          if (!user.depoList[i].initialWithdrawn && !user.depoList[i].initialWithdrawRequest) { //don't allow requested and initial withdrawn stake dividends to be reinvested
    ...
    function withdraw() public isInitialized returns (uint256 withdrawAmount) {
        ...
        withdrawAmount = getUserDividends(msg.sender);
      	...
      	for (uint i = 0; i < user.depoList.length; i++) {
          if (!user.depoList[i].initialWithdrawn && !user.depoList[i].initialWithdrawRequest) { //don't allow requested and initial withdrawn stake to accumulate dividends
            user.depoList[i].depoTime = block.timestamp;
          }
        }
        ...
        if(this.getBalance() < withdrawAmount) {
            withdrawAmount = this.getBalance();
        }
    ...
    function requestWithdrawInitial(uint256 key) public isInitialized {
        User storage user = users[msg.sender];
        require(!user.depoList[key].initialWithdrawRequest, "This user stake already has an existing request.");
        require(!user.depoList[key].initialWithdrawn, "This user stake is already forfeited.");
    ...
    function withdrawInitial(uint256 key) public isInitialized {
        ...
        require(user.depoList[key].initialWithdrawRequest, "Can only unstake when there is an active withdraw request for the deposit plan.");
        require(!user.depoList[key].initialWithdrawn, "This user stake is already forfeited.");
        ...
        uint256 refundAmount = amount + (dailyReturn.mul(elapsedTime).div(hardDays)) - (amount.mul(100).div(percentDivider));
        
        if(this.getBalance() < refundAmount) {
            refundAmount = this.getBalance();
        }

        uint256 devStakeFee  = refundAmount.mul(development).div(percentDivider); 
        uint256 projStakeFee = refundAmount.mul(project).div(percentDivider);

        BUSD.safeTransfer(dev, devStakeFee);
        BUSD.safeTransfer(proj, projStakeFee);
        BUSD.transfer(msg.sender, refundAmount.sub(devStakeFee.add(projStakeFee)));
        user.activeStakesCount--;
        user.totalInvested -= amount;
        user.depoList[key].amt = 0;
        user.depoList[key].initialWithdrawn = true;
        user.depoList[key].depoTime = 0;
        user.depoList[key].requestTime = 0;
    ...
    function sellTokens(uint256 amount) external {       
        ...
        if(this.getBalance() < cvtAmt) {
            cvtAmt = this.getBalance();
        }
    ...
    function changeWithdrawInitialStepTime(uint256 timeStep) external onlyOwner {
        require(timeStep >= 1 hours || timeStep <= 14 days, "Time step can only changed to 1 day up to 30 days."); 
