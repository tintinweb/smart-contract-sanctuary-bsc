...
abstract contract StoVesting is BEPOwnable {
  using SafeMath for uint256;

  // Address of STO Token.
  IBEP20 public stoToken;
  ...
  // @param _token Address of STO token
  ...
  function addBeneficiary(address _beneficiary, uint256 _amount) public onlyOwner {
    ...
    require(stoToken.transferFrom(_msgSender(), address(this), _amount), "cannot-transfer-token-to-this-contract");
  ...
  function claimVestedToken(address _beneficiary) public {
    ...
    require(stoToken.transfer(_beneficiary, tokenVestable), "fail-to-transfer-token");
...
/**
 * @dev Travel2EarnStoVesting will be blocked for 1 month,
 * then releaseed linearly each month during the next 4 year.
 * Hence, the _vestingStartAt should delay 1 month
 * and the vestingDuration should be 48 months.
 *
 */
contract Travel2EarnStoVesting is StoVesting {
  //uint256 private SECONDS_PER_MONTH = 2628000;
  constructor(
    address _token,
    address _owner,
    uint256 _vestingStartAt
  ) StoVesting(_token, _owner, (_vestingStartAt + 2628000), 48) {}
}

/**
 * @dev TeamStoVesting will be blocked for 3 month,
 * then releaseed linearly each month during the next 4 year.
 * Hence, the _vestingStartAt should delay 3 month
 * and the vestingDuration should be 48 months.
 *
 */
contract TeamStoVesting is StoVesting {
  //uint256 private SECONDS_PER_MONTH = 2628000;
  constructor(
    address _token,
    address _owner,
    uint256 _vestingStartAt
  ) StoVesting(_token, _owner, (_vestingStartAt + 7884000), 48) {}
}

/**
 * @dev AdvisorStoVesting will be blocked for 6 month,
 * then releaseed linearly each month during the next year.
 * Hence, the _vestingStartAt should delay 6 month
 * and the vestingDuration should be 12 months.
 *
 */
contract AdvisorStoVesting is StoVesting {
  //uint256 private SECONDS_PER_MONTH = 2628000;
  constructor(
    address _token,
    address _owner,
    uint256 _vestingStartAt
  ) StoVesting(_token, _owner, (_vestingStartAt + 15768000), 12) {}
}

/**
 * @dev DexLiquidityStoVesting will be blocked for 1 month,
 * then releaseed 5% each month during the next year.
 * Hence, the _vestingStartAt should delay 1 month
 * and the vestingDuration should be 20 months.
 *
 */
contract DexLiquidityStoVesting is StoVesting {
  //uint256 private SECONDS_PER_MONTH = 2628000;
  constructor(
    address _token,
    address _owner,
    uint256 _vestingStartAt
  ) StoVesting(_token, _owner, (_vestingStartAt + 2628000), 20) {}
}

/**
 * @dev EcosystemStoVesting will be blocked for 1 month,
 * then releaseed linearly each month during the next 4 year.
 * Hence, the _vestingStartAt should delay 1 month
 * and the vestingDuration should be 48 months.
 *
 */
contract EcosystemStoVesting is StoVesting {
  //uint256 private SECONDS_PER_MONTH = 2628000;
  constructor(
    address _token,
    address _owner,
...
/**
 * @dev StoVestingFactory is the main and is the only contract should be deployed.
 * Notice: remember to config the Token address and approriate startAtTimeStamp
 */
contract StoVestingFactory {
  // put the token address here
  // This should be included in the contract for transparency
  address public STO_TOKEN_ADDRESS = 0xdaB4CD0DDa22f765Eb7F8054317C3fB42d6956A0;

  // put the startAtTimeStamp here
  // To test all contracts, change this timestamp to time in the past.
  uint256 public startAtTimeStamp = 1657461600;

  // address to track other information
  address public owner;
  address public travel2EarnStoVesting;
  address public teamStoVesting;
  address public advisorStoVesting;
  address public dexLiquidityStoVesting;
  address public ecosystemStoVesting;

  constructor() {
    owner = msg.sender;

    Travel2EarnStoVesting _travel2EarnStoVesting = new Travel2EarnStoVesting(
      STO_TOKEN_ADDRESS,
      owner,
      startAtTimeStamp
    );
    travel2EarnStoVesting = address(_travel2EarnStoVesting);

    TeamStoVesting _teamStoVesting = new TeamStoVesting(STO_TOKEN_ADDRESS, owner, startAtTimeStamp);
    teamStoVesting = address(_teamStoVesting);

    AdvisorStoVesting _advisorStoVesting = new AdvisorStoVesting(STO_TOKEN_ADDRESS, owner, startAtTimeStamp);
    advisorStoVesting = address(_advisorStoVesting);

    DexLiquidityStoVesting _dexLiquidityStoVesting = new DexLiquidityStoVesting(
      STO_TOKEN_ADDRESS,
      owner,
      startAtTimeStamp
    );
    dexLiquidityStoVesting = address(_dexLiquidityStoVesting);

    EcosystemStoVesting _ecosystemStoVesting = new EcosystemStoVesting(STO_TOKEN_ADDRESS, owner, startAtTimeStamp);
    ecosystemStoVesting = address(_ecosystemStoVesting);
  }
}