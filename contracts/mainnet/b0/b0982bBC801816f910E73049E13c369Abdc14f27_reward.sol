...
contract reward is Ownable {
    ...
    //     return balance +  communityClaimed;
    // }
    ...
    //     return ERC20(creatorAddress).balanceOf(account);
    // }
    ...
    function claimCommunity(uint256 tokenID, uint256 balance) public {
        if (balance > 0) {
    ...
    function claimRewardsToHolders() external {
        require(msg.sender == creatorAddress, "only creator can claim");
        uint256 totalCount = ERC721(collectionAddress).totalSupply();
        uint256 balance = ERC20(tokenFeesAddress).balanceOf(address(this)) / totalCount;
        for (uint256 i=1; i<=totalCount; i++) {
            claimCommunity(i, balance);
        }
    }
