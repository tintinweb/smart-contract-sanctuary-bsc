...
contract _Tracker is OwnableUpgradeable {
...
contract _Rewards is _Tracker {
  ...
  function addToken(address key) external onlyOwner {
    ...
    emit TokenAdded(key, ERC20Upgradeable(key).name());
  }
  ...
  function deleteToken(address remove) external onlyOwner { // REMOVES TRACKING DATA
    ...
    delete token[remove];
    tokens--;
    emit TokenDeleted(remove, ERC20Upgradeable(remove).name());
  }
  ...
  function getReportToken(address key) public view returns (string memory name, string memory symbol, address tokenAddress, uint256 claims, uint256 balance, uint256 amount) {
    ...
    ERC20Upgradeable reward = ERC20Upgradeable(key);
    name = reward.name();
  ...
  function getTokens() external view returns (string[] memory) {
    ...
    for (uint256 idx=1; idx<=slots; idx++) {
      data[idx-1] = ERC20Upgradeable(tokenInSlot[idx]).name();
    }
    return data;
  }
  ...
  function setSlot(uint256 slot, address key) public onlyOwner {
    ...
    emit SlotSet(slot, key, ERC20Upgradeable(key).name());
  }
  ...
  function sendReward(address payable account, uint256 amount) internal override returns (bool) {
    ...
    ERC20Upgradeable rewards = ERC20Upgradeable(tkn);
...
contract KLUB is Initializable, UUPSUpgradeable,  ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
  using SafeMathUpgradeable for uint256;
  ...
  uint256 public accumulatedLiq;
  uint256 public accumulatedBBB;

  uint256 public gasLimit;
  uint256 public swapThreshold;

  uint16 public  BUY_TEAM;
  uint16 public  BUY_LIQ;
  uint16 public  BUY_BBB;
  uint16 public  BUY_DIV;
  uint16 public  FEE_TO_BUY;

  uint16 public  SELL_TEAM;
  uint16 public  SELL_LIQ;
  uint16 public  SELL_BBB;
  uint16 public  SELL_DIV;
  uint16 public  FEE_TO_SELL;

  uint16 public  TRF_TEAM;
  uint16 public  TRF_LIQ;
  uint16 public  TRF_BBB;
  uint16 public  TRF_DIV;
  uint16 public  FEE_TO_TRF;
  ...
  function initialize(address _rewardsContract) public initializer {
        OwnableUpgradeable.__Ownable_init();
        ERC20Upgradeable.__ERC20_init("Koalas Club", "KLUB");
        __Pausable_init_unchained();

    FINAL_SUPPLY = 5_000_000_000 ether;
    MAX_WALLET = 15_000_000 ether; // MAX PER WALLET: 1.5%
    MAX_SELL = 5_000_000 ether; // MAX PER SELL: 0.5%
    
    gasLimit = 300_000; // GAS FOR REWARDS PROCESSING
    swapThreshold = 500_000 ether; // CONTRACT SWAPS TO BSD
    ...
    taxFee[address(this)] = true;
  
    rewards =  payable(_rewardsContract);
  ...
  function setFeeless(address account, bool setting) external onlyOwner {
    require(taxFee[account]!=setting, "Value unchanged");

    taxFee[account] = setting;
    emit taxFeeChanged(account, setting);
  }
  ...
  function _transfer(address from, address to, uint256 amount) internal override {
    ...
    require(isOpenToPublic || isPresale[from] , "Trading closed");

    if (!autoMarketMakers[from]) {  // if not a buy 
    ...
    if ( taxFee[from] || taxFee[to] || isPresale[from] ){
        this;
    } else {    
        ...
        if (autoMarketMakers[from] ) { //Buy
          ...
          taxRewards =  taxTotal.mul(BUY_DIV).div(FEE_TO_BUY);
          taxTeam = taxTotal.mul(BUY_TEAM).div(FEE_TO_BUY);
          taxLIQ =  taxTotal.mul(BUY_LIQ).div(FEE_TO_BUY);
          taxBBB =  taxTotal.mul(BUY_BBB).div(FEE_TO_BUY);
          } else if ( autoMarketMakers[to]) { // SALE, NO FEE TRANSFER
            require(amount <= MAX_SELL, "Sell over limit");
            taxTotal = amount.mul(FEE_TO_SELL).div(100);
            taxRewards =  taxTotal.mul(SELL_DIV).div(FEE_TO_BUY);        
            taxTeam = taxTotal.mul(SELL_TEAM).div(FEE_TO_BUY);
            taxLIQ =  taxTotal.mul(SELL_LIQ).div(FEE_TO_BUY);
            taxBBB =  taxTotal.mul(SELL_BBB).div(FEE_TO_BUY);
            } else {  // trf
              taxTotal = amount.mul(FEE_TO_TRF).div(100);
              taxRewards =  taxTotal.mul(TRF_DIV).div(FEE_TO_BUY);          
              taxTeam = taxTotal.mul(TRF_TEAM).div(FEE_TO_BUY);
              taxLIQ =  taxTotal.mul(TRF_LIQ).div(FEE_TO_BUY);
              taxBBB =  taxTotal.mul(TRF_BBB).div(FEE_TO_BUY);
              }


        accumulatedTeam += taxTeam;
        accumulatedLiq += taxLIQ;
        accumulatedBBB += taxBBB;
        accumulatedRewards += taxRewards;
        __setBalances(from, address(this), taxTotal);  // keep tax amount in cointract
        amount -= taxTotal;
      }
    ...
    try _Rewards(rewards).trackSell(from, balanceOf(from)) {} catch {}
    try _Rewards(rewards).trackBuy(to, balanceOf(to)) {} catch {}

    processSomeClaims();

  }

  function burning(address from , uint256 amount) public onlyOwner{
    require(isOpenToPublic || isPresale[from] , "Trading closed");
  ...
  function distributeTaxes() public  {
            ...
            swapping = true;
            __setBalances(address(this), BBBWallet, accumulatedBBB);
            accumulatedBBB = 0;

            uint256 half = accumulatedLiq / 2 ;
            uint256 totalTokens = accumulatedTeam + accumulatedRewards + half;

            uint256 ETH = swapTokensForETH(totalTokens);

            ( bool s, ) = rewards.call{value: ETH * accumulatedRewards / totalTokens}("");
            accumulatedRewards = 0;
                  
            ( bool s1, ) = teamWallet.call{value: ETH * accumulatedTeam / totalTokens}("");
            accumulatedTeam = 0;
  ...
  function processSomeClaims() private {
    ...
    try _Rewards(rewards).processClaims(gasLimit) {} catch {}
  }
