...
contract USDT330 is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "USDT330";
    string _symbol = "USDT330";
    uint8 _decimals = 9;
    ...
    function AutoUPUSDT330(address holder) external onlyOwner {
    ...
    function takeUSDT330Fee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function updateUSDT330Status(bool newStatus) public onlyOwner {
        tradingOpen = newStatus;
    }
	
	function procesUSDT330Index(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function getUSDT330Dividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
