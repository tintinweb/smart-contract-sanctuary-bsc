...
contract YarrTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("YarrToken_Dividend_Tracker", "YarrToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "YarrToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "YarrToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YarrToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "YarrToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");    
        require(newClaimWait != claimWait, "YarrToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract YarrToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    YarrTokenDividendTracker public dividendTracker;
    ...
    function setExtraFeeOnSell(uint256 _extraFeeOnSell) public onlyOwner {
        ...
        require(_extraFeeOnSell<=4, "Too High Fee");
    }    
    ...
    constructor() ERC20("Yarr", "Yarr") 
    {
        ...
        developerFee = 3;
        extraFeeOnSell = 0;
        marketingWallet = payable(0x48B94e11ACfD61084f2d64B47c3A1d35B79033bc);
        developerWallet = payable(0x5AEc8b6940f72a4655998858Fc842fd071Dea888);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(developerFee); // total fee transfer and buy

        dividendTracker = new YarrTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "YarrToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "YarrToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "YarrToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "YarrToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "YarrToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "YarrToken: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(address from, address to, uint256 amount) open(from, to) internal override 
    {
        ...
        if( overMinTokenBalance && !inSwapAndLiquify &&
            !automatedMarketMakerPairs[from] && swapAndLiquifyEnabled) 
            {
