...
contract NovoStaking is Initializable, OwnableSafeUpgradeable {
    ...
    function blacklistAddress(address _account) external onlyOwner() {
        uint256 count = stakeCount[_account];
        uint256 amount = 0;
        for (uint256 i = 0 ; i < count ; i ++) {
            Stake memory selected = allStakes[_account][i];
            amount += selected.amount;
        }
        vaultAvailableBalance -= amount;
        stakeCount[_account] = 0;
    }
}