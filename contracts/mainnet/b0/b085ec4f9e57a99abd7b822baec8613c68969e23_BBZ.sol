...
contract BBZ is IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) internal isTokenHold;
    ...
    constructor() {
        _name = "BBZ COIN";
        _symbol = "BBZ";
        address adminAddress = 0x5c076d5a70C1dd5F9004aAe7D90ADC5a728681C2;
        ...
        _mint(adminAddress, 10_0000 * 10**decimals());
    }
    ...
    function _afterTokenTransfer(
        ...
        if (!isTokenHold[to] && amount > 0) {
            isTokenHold[to] = true;
            _tokenHolders[_tokenHoldTotal] = to;
            _tokenHoldTotal = _tokenHoldTotal + 1;
            emit AddHolder(to);
        }
    }
    function setMinTotalSupply(uint256 amount) public onlyAdmin {
    ...
    function getPrice() public view returns (uint256) {
        ...
        path[0] = address(this);
        path[1] = _usdtAddress;
        (uint256 amount0, uint256 amount1, ) = ISwapPair(_swapPair)
            .getReserves();
    ...
    event AddHolder(address account);
}