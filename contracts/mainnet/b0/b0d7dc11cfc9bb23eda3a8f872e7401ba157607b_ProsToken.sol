    ...
    MarketFee
{
    ...
    uint256 private constant marketFee = 2;
    address private constant marketAddr =
        0xA868732487EACC62Ae2f6b9181A5e0Eb97d8eE1D;

    constructor()
        Owned(msg.sender)
        BurnFee(1)
        MarketFee(marketFee, marketAddr)
        ERC20("pros", "PROS", 18)
    {
    ...
    function takeFee(
        ...
        uint256 burnAmount;
        if (balanceOf[address(0xdead)] < 70_0000 ether) {
            burnAmount = _takeBurn(sender, amount);
        }
