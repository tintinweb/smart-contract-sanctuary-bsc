...
contract StakeMO is Pausable, Ownable, ReentrancyGuard {
    ...
    function transferToken(address to,uint256 amount) external onlyOwner{
        require(Moj.transfer(to, amount), "Token transfer failed!");  
    }
    ...
    function claimReward() external returns (bool){
        ...
        Moj.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(Moj.balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");
        
           Moj.transferFrom(_msgSender(), address(this), stakeAmount);
            totalStakers++;
