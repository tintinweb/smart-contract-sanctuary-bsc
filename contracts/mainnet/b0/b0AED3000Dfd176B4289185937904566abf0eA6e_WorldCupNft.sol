...
import "./ERC721WCF.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract WorldCupNft is Ownable, ERC721WCF, ReentrancyGuard {
    uint256 public immutable maxPerAddressDuringMint;
    uint256 public immutable amountForDevs;

    uint256[] public itemPrices = [0.1 ether, 0.2 ether, 0.3 ether, 0.5 ether, 0.7 ether, 1 ether];
    ...
    ) ERC721WCF("WorldcupChain Nft", "WCFN", maxBatchSize_, collectionSize_) {
    ...
    function setBaseURI(string calldata baseURI, string calldata extension) external onlyOwner {
        _baseTokenURI = baseURI;
        _baseTokenExt = extension;
    }

    function getPrice() public view returns (uint256) {
        uint256 idx = totalSupply() / 100;
        if (idx >= itemPrices.length) {
 ...
 */
contract ERC721WCF is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721WCF.ownerOf(tokenId);
