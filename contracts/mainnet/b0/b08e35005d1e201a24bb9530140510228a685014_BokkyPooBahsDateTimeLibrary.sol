...
contract NstTokenStakingPool is Ownable, ReentrancyGuardUpgradeable {
    ...
    event PoolAdded(uint256 _pid, IERC20 _lpToken, bool _enableBonus, address _bonusToken, address _swapPairAddress, uint256 _lockSeconds, uint256 _depositMin, uint256 _depositMax);
    ...
    constructor(
        ...
        userToReferrer[rooter] = address(0);
        currentId = 2;
    }
    ...
    function addDirectReferral(address _referrer) public notPause nonReentrant{
        ...
        require(addressToId[_referrer] > 0, "DirectReferral::referrer is invalid");
    ...
    function getTowParent(address _user) public view returns (address, address){
        address _referrer0 = userToReferrer[_user];
        require(_user != address(0), "getTowParent::user is zero address");
    ...
    function setBonus(uint256 _pid, bool _enableBonus, address _bonusToken, address _swapPairAddress, uint256 _lockSeconds) public onlyAdmin {
        ...
        if (_enableBonus) {
            ...
            require(_swapPairAddress != address(0x0), "swapPairAddress error");
        } else {
            require(_swapPairAddress == address(0x0), "swapPairAddress should be 0");
        }
    ...
    function add(IERC20 _lpToken, uint256 _apr, bool _enableBonus, address _bonusToken, address _swapPairAddress, uint256 _lockSeconds, uint256 _depositMin, uint256 _depositMax) public onlyAdmin {
        ...
        emit PoolAdded(_pid, _lpToken, _enableBonus, _bonusToken, _swapPairAddress, _lockSeconds, _depositMin, _depositMax);
    }
    ...
    function setUserInfolastRewardTime(uint256 _pid, address _user, uint256 _lastRewardTime) public notPause onlyAdmin{
        UserInfo storage user = userInfo[_pid][_user];
        if (user.amount > 0) {
    ...
    function claimUserReward(uint256 _pid, address _user) private {
        ...
        if (user.amount > 0) {
            ...
            if (diffDays > 0) {
                ...
                if (pendingAmount > 0) {
                    //for parents
                    (address referrer0, address referrer1) = getTowParent(_user);
                    //first level parent
                    addPendingInviteReward(_pid,referrer0, pendingAmount, 0);
                    //second level parent
                    addPendingInviteReward(_pid,referrer1, pendingAmount, 1);
    ...
    function addPendingInviteReward(uint256 _pid,address _referrer, uint256 _pendingAmount, uint256 _bonusesIndex) private {
    ...
    function safeITokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(_amount <= iTokenBal, "pool to not have enough iTokens");
