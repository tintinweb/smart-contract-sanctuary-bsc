...
contract TRQ is IERC20, Ownable {
    ...
    bool public _isLimitDay=true;
    mapping(address => uint256) public countDayBuy;
    mapping(address => uint256) public countDaySell;
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(_isLimitDay   && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
                require(countDayBuy[to].add(1 days)<=block.timestamp,"countDayBuy limit 1");
                countDayBuy[to]=block.timestamp;
            }
            ...
            if(_isLimitDay   && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
                require(countDaySell[from].add(1 days)<=block.timestamp,"countDaySell limit 1");
                countDaySell[from]=block.timestamp;
            }
    ...
    function setisLimitDay() public onlyOwner {
        _isLimitDay=!_isLimitDay;
    }
