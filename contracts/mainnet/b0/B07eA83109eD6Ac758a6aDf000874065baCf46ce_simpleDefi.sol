...
contract simpleDefi is Storage, Ownable, AccessControl {
    ...
    function swap(uint amountIn, address _token0, address _token1) internal returns (uint){
        ...
        path[0] = _token0;
        path[1] = _token1;
        return poolUtil.swap(exchangeInfo, iData, amountIn,path);
    }
    ...
    ///@notice Allows admin to override intermediate token for an specific pool
    ///@param _iToken0 intermediate token for token0
    ///@param _iToken1 intermediate token for token1
    function setInterToken(address _iToken0, address _iToken1) public allowAdmin {
        if (_iToken0 != address(0))
            exchangeInfo.intermediateToken = _iToken0;
        if (_iToken1 != address(0))
            exchangeInfo.baseToken = _iToken1;
    }
