...
contract LOWTAX is IBEP20, Renounce {
    ...
    uint256 swapTimes; uint256 minSells = 3;
    bool private inSwap; bool botOn = true;
    uint256 public swapThreshold = ( _totalSupply * 300 ) / 100000;
    uint256 public _minTokenAmount = ( _totalSupply * 20 ) / 100000;
    modifier swapping() { inSwap = true; _; inSwap = false; }
    address autoLiquidityReceiver; address developmentFeeReceiver; address marketingFeeReceiver;
    uint256 targetLiquidity = 20; uint256 targetLiquidityDenominator = 100;

    constructor(address _liquidity, address _development, address _marketing) Renounce(msg.sender) {
    ...
    function swapBack(address sender, uint256 amount) internal {
        ...
        if(!inSwap && swapEnabled && sender != pair && aboveM && !isInternal[sender] && swapTimes >= minSells)
        {swapAndLiquify(swapAmount); swapTimes = 0;}
    }

    function swapAndLiquify(uint256 _swapThreshold) internal swapping {
        ...
        if(amountToLiquify > 0){router.addLiquidityETH{value: amountBNBLiquidity}( address(this), amountToLiquify, 0, 0, autoLiquidityReceiver, block.timestamp);}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(developmentFeeReceiver).call{value: balanceOf(address(this)), gas: 30000}("");
        tmpSuccess = false;}
