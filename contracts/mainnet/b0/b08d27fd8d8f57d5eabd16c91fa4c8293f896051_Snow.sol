...
contract Snow is ERC20, Ownable {
    ...
    SnowDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public XmasGiftWallet;
    ...
    uint256 public buyXmasGiftFee;
    uint256 public sellXmasGiftFee;
    ...
    event UpdateFees(
        ...
        uint256 buyXmasGiftFee,
        uint256 sellXmasGiftFee
    );
    ...
    constructor() ERC20("Snow", "SNOW") {
        marketingWallet = payable(0x351e15ef40f8D44121E6F3630D2A243Fa8110867);
        XmasGiftWallet = payable(0x47b079073327366071f3C223c2Bb6CaC8f32b1a3);
        ...
        dividendTracker = new SnowDividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[address(XmasGiftWallet)] = true;
    ...
    function updateFees(
        ...
        buyXmasGiftFee = XmasGiftBuy;
        sellXmasGiftFee = XmasGiftSell;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + XmasGiftFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyXmasGiftFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellXmasGiftFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellXmasGiftFee) +
            buyMarketingFees.add(buyXmasGiftFee);
        ...
        uint256 XmasGiftPayout = buyXmasGiftFee.add(sellXmasGiftFee) * feePortions;

        if (marketingPayout > 0) {
            (successOp1, ) = address(marketingWallet).call{value: marketingPayout}("");
        }
        if (XmasGiftPayout > 0) {
            (successOp2, ) = address(XmasGiftWallet).call{value: XmasGiftPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + XmasGiftPayout,
...
contract SnowDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        SnowContract = Snow(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Snow_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Snow_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Snow contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SnowContract);

            uint256 prevBalance = SnowContract.balanceOf(address(this));
            ...
            {
                uint256 received = SnowContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SnowContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
