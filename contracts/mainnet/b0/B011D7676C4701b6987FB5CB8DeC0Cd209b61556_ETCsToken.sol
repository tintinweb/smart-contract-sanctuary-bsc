...
contract ETCsToken is ERC20, Ownable {
    ...
    uint256 public startTime = 1659855600;
    ...
    uint256 public exrtraRate2=200;
    uint256 public exrtraRate3=300;
    ...
    address public liquidityDividendAddress;

    address public receiveAddress = 0x6951fe9a4D0F847837259BAd80A84EC252Ac3E0c;
    ...
    constructor(
        ...
        _totalSupply = 5200000 * (10 ** 18);

        _balances[receiveAddress] = 5200000 * (10 ** 18);
        emit Transfer(address(0), receiveAddress, 5200000 * (10 ** 18));

        marketingWalletAddress = 0xCbB43d4Ba2769DFfDDf632021187F5B8b31d9780;
        liquidityDividendAddress = 0xbB190a4DC8196F8DEE3800b945997d987E2AB3EF;

        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        excludeFromFees(marketingWalletAddress, true);
        excludeFromFees(receiveAddress, true);
        excludeFromFees(liquidityDividendAddress, true);
    ...
    function setLiquidityDividendAddress(address addr) public onlyOwner {
        liquidityDividendAddress = addr;
    }

    function setExtraRate(uint256 rate1,uint256 rate2,uint256 rate3) public onlyOwner {
        exrtraRate1 = rate1;
        exrtraRate2 = rate2;
        exrtraRate3 = rate3;
    }
    ...
    function takeAllFee(address from, address to, uint256 amount) internal returns (uint256 amountAfter) {
        ...
        if (from == uniswapV2Pair) {
            ...
            if (lpAmount > 0) {
                if (autoLiqudityEnable) {
                    super._transfer(from, address(this), lpAmount);
                    lpDividendProcess(lpAmount);
                } else {
                    super._transfer(from, liquidityDividendAddress, lpAmount);
                }
            }

            uint256 marketingAmount = amount.mul(buyMarketingRate).div(maxRate);
            super._transfer(from, marketingWalletAddress, marketingAmount);
            ...
            if (currentDownRate >= 3) {
                ...
                if (currentDownRate >= 3 && currentDownRate < 6) {
                    exrtraRate = exrtraRate1;
                } else if (currentDownRate >= 6 && currentDownRate < 9) { 
                    exrtraRate = exrtraRate2;
                }else{ 
                    exrtraRate = exrtraRate3;
                }
    ...
    // 改时间   删测试
    function test(uint256 ind, uint256 val) public onlyOwner {
        priceMap[ind] = val;
    }
