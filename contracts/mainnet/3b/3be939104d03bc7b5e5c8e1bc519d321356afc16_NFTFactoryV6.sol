...
contract NFTFactoryV6 is Ownable {
    ...
    IBEP20 public cakeToken;
    uint256 public tierCommisionrate = 4500;
    uint256 referrerCommision = 45*1e18;
    uint256 public referDepth = 5;
    IWedexReferral public wedexReferral;
    ...
    // Map if address has already claimed a NFT
    mapping(address => bool) public hasClaimed;
    // Map if address has get bonus Reward for their referer;
    ...
    function mintNFT(uint8 _bunnyId, address referrer,bool _vipBranch, uint256 _leaderCommission) external {
        ...
        if(address(wedexReferral) != address(0)){
            if(wedexReferral.getReferrer(_msgSender())!=address(0)&&referrerCommision>0){
                payRefCommision(_msgSender(),0,referrerCommision);
                payBonusCommission(wedexReferral.getReferrer(_msgSender()));
            }
    ...
    function payBonusCommission(address _referrer) internal{
        if(bonusAddress[_referrer]&&_referrer!=address(0)) {
            safeWedexTransfer(_referrer,bonusAmount);
            payBonusCommission(wedexReferral.getReferrer(_referrer));
        }
    }
    // Safe Wedex transfer function, just in case if rounding error causes pool to not have enough Wedex to pay.
    function safeWedexTransfer(address _to, uint256 _amount) internal {
    ...
    function setCaketoken(IBEP20 _cakeToken) public onlyOwner {
        cakeToken = _cakeToken;
    }
