...
contract BreakMePlxAgain is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BreakMePlxAgainDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BreakMePlxAgain", "BMPA") {
        ...
        dividendTracker = new BreakMePlxAgainDividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BreakMePlxAgainTRACKER", "BMPATRACKER");
...
contract BreakMePlxAgainDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BreakMePlxAgain public BreakMePlxAgainContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BreakMePlxAgainContract = BreakMePlxAgain(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BreakMePlxAgain contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BreakMePlxAgainContract);

            uint256 prevBalance = BreakMePlxAgainContract.balanceOf(address(this));
            ...
            {
                uint256 received = BreakMePlxAgainContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BreakMePlxAgainContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
