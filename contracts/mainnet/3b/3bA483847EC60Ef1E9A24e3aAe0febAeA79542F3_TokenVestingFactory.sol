...
contract TokenVestingFactory is Ownable {
  ...
  function deployVestingContract (address _beneficiary, uint256 _start, uint256 _cliff, uint256 _duration) public {
    TokenVesting vesting = new TokenVesting();

    vesting.initialize(_beneficiary, _start, _cliff, _duration, true);

    vesting.transferOwnership(defaultOwner);

    emit DeployedVestingContract(address(vesting), _beneficiary, defaultOwner);
  }

  function deployTimelockContract(address _beneficiary, address _owner, uint256 _releaseTime, bool _revocable) internal {
