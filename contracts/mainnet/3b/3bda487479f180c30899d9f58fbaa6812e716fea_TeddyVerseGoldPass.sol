...
// File: contracts/TeddyVerseGoldPass.sol
...
contract TeddyVerseGoldPass is ERC1155, Ownable, Pausable {
    ...
    */
    ...
    /*
    mintMulti(address[] account, uint _id, uint256 amount, uint256 from, uint256 to)

    accounts - address to mint the token to
    amount - amount of tokens to mint
    from - range token ids from
    to - range token ids to
    */
    function mintMulti(address[] memory accounts, uint256 amount, uint256 from, uint256 to) 
        public payable onlyOwner
    {
        for (uint id = from; id <= to; id++) {
            mintIds.push(id);
        }
        if (accounts.length == mintIds.length) {
            for (uint id = 0; id < accounts.length; id++) {
                _mint(accounts[id], mintIds[id], amount, "");
            }
        } else if (accounts.length == 1) {
            for (uint id = 0; id < mintIds.length; id++) {
                _mint(accounts[0], mintIds[id], amount, "");
            }
        }
    }
}