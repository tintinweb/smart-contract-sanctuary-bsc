...
contract Presale is IPreIDOBase, Ownable {
    ...
    uint256 private constant MIN_LOCK = 30 days; // 1 month;
    ...
    constructor(
        ...
        discountsLock[1] = MIN_LOCK;
        discountsLock[2] = 2 * MIN_LOCK;
        discountsLock[3] = 3 * MIN_LOCK;
    }

    receive() external payable inPresalePeriod {
        int256 price = getPrice();
        _order(msg.value, 18, price, priceFeed.decimals(), 1); // default to 10% discount rate
    }
    ...
    function _order(
        ...
        uint256 tokenPriceX4 = (100 * (100 - discountsRate)) / 100; // 300 = 0.03(default price) * 10^4
       ...
       // uint8 upperPow = token.decimals() + 4; // 4(token price decimals) => 10^4 = 22
        uint8 upperPow = token.decimals() + 2;
    ...
    function claimRemainingToken(address receiver) external onlyOwner afterPresalePeriod{
         uint256 remainingToken = token.balanceOf(address(this)) - totalDistributed;
          safeTransferToken(receiver,remainingToken);
    ...
    function collectFunds(address fundsAddress) external onlyOwner afterPresalePeriod {
       ...
       function collectTo(address receiver) external onlyOwner afterPresalePeriod {
