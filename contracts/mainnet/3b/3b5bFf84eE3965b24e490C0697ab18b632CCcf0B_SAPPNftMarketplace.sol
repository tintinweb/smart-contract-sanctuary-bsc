...
import "./interfaces/ISAPPNftMarketplace.sol";
import "./interfaces/ISAPPNftPool.sol";

contract SAPPNftMarketplace is ISAPPNftMarketplace, ERC721, IERC721Pausable{
    ...
    address public SAPPPool;

    constructor() ERC721("SAPP Marketplace", "SAPPMarket") {
        tokenId = 1;
    ...
    function setPool(address pool) public onlyOwner(){
        ...
        if(SAPPPool != address(0)){
            ISAPPNftPool(SAPPPool).migratePool(pool, address(0));
        }
        SAPPPool = pool;
    ...
    function setDefaultAmountEarn(address _user, address _tokenAddress) external override{
        require(msg.sender == SAPPPool, "Isn`t Pool");
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isNotRootToken(_tokenId) onlyEOA { 
        ...
        if(_subOwnerFee > 0 && flag){
            ISAPPNftPool(SAPPPool).subOwnerFeeBalance{value: _subOwnerFee}();
        }
        if(_subOwnerFee > 0 && !flag){
