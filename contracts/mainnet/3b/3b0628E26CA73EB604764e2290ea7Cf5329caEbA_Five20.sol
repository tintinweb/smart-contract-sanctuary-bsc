...
contract  Five20 is IERC20, Ownable {
    ...
    uint256 private _intervalSecondsForSwap = 60 ;

    uint256 public minLPDividendToken =  45e17;
    ...
    constructor(
            ...
            _symbol =  "520";
            _decimals= 18;
            _tTotal = 520* (10**uint256(_decimals));
            marketAddr = 0xcF24d2A17D660B82aDba36c55A7E2eB15D9B7D71;
            marketAddr2 = 0x9412b2EDD432aa119a43B44eB6E46F00cea4753F;
            ...
            if( block.chainid == 56){
                ...
                adminAddress =0x763430e4A24bda8CD9EC88fA9c69A226481CD506;
                minPeriod = 180;
            }
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    uint256 public maxHave = 5e18 ;
    function setLimit(uint256 maxHave_) public onlyOwner{
        maxHave = maxHave_ ; 
    }
    ...
    function _transfer(
        ...
        if( !_isExcludedFromFee[from] &&!_isExcludedFromFee[to]){
            if(  to!=uniswapV2Pair ){
                require((balanceOf(to).add(amount)) <= maxHave, "Transfer amount exceeds the max have.");
            }
        ...
        if(canSwap &&from != address(this) &&from != uniswapV2Pair &&from != owner() && to != owner() ){
                ...
                IERC20(_token).transfer(marketAddr,usdtBalance*1/5);
                IERC20(_token).transfer(marketAddr2,usdtBalance*2/5);
    ...
    function process() private {
        ...
        for(uint i;i<divi.length;i++){
            if(divi[i]!=address(0)){
                ...
                IERC20(_token).transferFrom(_lpRouter, divi[i],amount);
            }else{
                break;
            }
        }
    ...
    function getLpDiviAddr() public view returns(address[] memory,uint index  ){
        address[] memory adds = new address[](everyDivi);
        if(LpDiviAddr.length()>currentIndex){
                for(uint i;i<LpDiviAddr.length();i++){
                         uint ii = (i+currentIndex)%LpDiviAddr.length();
                            if(balanceOf( LpDiviAddr.values()[ii])>=minLPDividendToken && !isDividendExempt[ LpDiviAddr.values()[ii]]){
