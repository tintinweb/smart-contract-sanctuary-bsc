...
______         _                    ______      _   
|  ___|       | |                   | ___ \    | |  
| |_ ___  _ __| |_ _   _ _ __   __ _| |_/ / ___| |_ 
|  _/ _ \| '__| __| | | | '_ \ / _` | ___ \/ _ \ __|
| || (_) | |  | |_| |_| | | | | (_| | |_/ /  __/ |_ 
\_| \___/|_|   \__|\__,_|_| |_|\__,_\____/ \___|\__|

Predict market and earn money!
Referral system, bring your friend and earn with them !

https://fortunabet.app
*/                                                 
...
// OPERATOR
abstract contract FortunaBetOperator
{
   function owner() public view virtual returns (address);
}

// REFERRALS
abstract contract FortunaBetReferrals
{
...
//CONTRACT
contract FortunaBetPrediction is Ownable, Pausable, ReentrancyGuard 
{
    ...
    {
        _safeTransferBNB(payable(FortunaBetOperator(operator).owner()),  value);
    }
    ...
    {
        Referrals = FortunaBetReferrals(_referralsContract);
    }
    ...
    {                                                                                               
     ...
     function ForceFirstRoundExecute(int256 price, uint256 houseBetBullAmount, uint256 houseBetBearAmount) external onlyOwnerOrOperator whenNotPaused 
    {                                                                                               
        require(startOnce && lockOnce,"Can only execute round after startRound and lockRound is triggered");
        require(block.timestamp >= Rounds[currentEpoch - 2].closeTimestamp, "Can only execute round round after previous round ended");
        require(block.timestamp >= Rounds[currentEpoch].lockTimestamp, "Can only execute round after lock timestamp reached");
    ...
    function getcurrentTimeStamp() external view returns (uint32) {
