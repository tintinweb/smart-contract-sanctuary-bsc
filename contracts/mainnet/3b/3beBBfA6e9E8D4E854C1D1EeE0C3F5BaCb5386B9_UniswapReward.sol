...
interface IPlayerBook {
    function settleRewardBMC(address from, uint256 amount) external;
}

interface nodeCtr {
    function isNode( address from) external view returns(bool);
    function setActivation(address _node) external;
}
...
contract UniswapReward is LPTokenWrapper{
    ...
    constructor(address _rewardToken ,address _inviteADDR,address[] memory _route,address _nodeCtrAddr,address _gainer) public {
        rewardToken = _rewardToken;
        inviteADDR = _inviteADDR;
        route = _route;
        nodeCtrAddr = _nodeCtrAddr;
        gainer = _gainer;
    }
        ...
        checkStart
    {
        ...
        require(nodeCtr(nodeCtrAddr).isNode(msg.sender), "isNode err");
        nodeCtr(nodeCtrAddr).setActivation(msg.sender);

        amount= amount.mul(desNum);         
        uint[] memory tokennum = uniswap(swapAddr).getAmountsOut(amount,route);
        super.stake(tokennum[tokennum.length-1]);
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            uint256 inviteBMCAmounts = leftReward.div(10);
            IPlayerBook(inviteADDR).settleRewardBMC(msg.sender,inviteBMCAmounts);
            IERC20(rewardToken).transfer(inviteADDR, inviteBMCAmounts );
            leftReward = leftReward.sub(inviteBMCAmounts);

            if(leftReward>0){
                IERC20(rewardToken).transfer(msg.sender, leftReward );
    ...
    function setinvADDR( address _invADDR) external onlyGovernance {
        inviteADDR = _invADDR;
    }
