...
contract VRFRandomSeed is VRFConsumerBaseV2, ConfirmedOwner ,IVRFRandomSeed{
    ...
    function setKeyHas(bytes32 _keyHash) external onlyOwner {
        keyHash=_keyHash;
    }
   ...
   function lastRequestID() external override view returns(uint256){
       return lastRequestId;
   }

   function setCallbackGasLimit(uint32  _callbackGasLimit) external onlyOwner {
       require(_callbackGasLimit>=20000,"atleast 20000 gas for requesting 1 VRF words");
       callbackGasLimit=_callbackGasLimit;
   }
   
   function getsetCallbackGasLimit() external view returns(uint32){
...
// mainnet 200 gwei Key Hash
    ...
    function requestRandomNumber() external override onlyOwner returns (uint256 requestId) {
    ...
    function getRequestStatus(uint256 _requestId) external override view returns (bool fulfilled, uint256[] memory randomWords) {
        require(s_requests[_requestId].exists, 'request not found');
