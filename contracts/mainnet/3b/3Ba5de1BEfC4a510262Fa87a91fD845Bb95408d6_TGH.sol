...
contract TGH  is Context, IERC20, Ownable {
    ...
    uint256 public maxPrice = 400 * 10 ** 9;
    ...
    function _transfer(address from, address to, uint amount) private {
        ...
        if(isPair[from]){
            uint256 toBalance = _balances[to].add(amount);
            require(maxPrice >= toBalance, "ERC20: user amount Max");
        }else{
    ...
    function setByMaxTr(uint256 maxnum) external onlyOwner{
        maxPrice = maxnum;
    }
