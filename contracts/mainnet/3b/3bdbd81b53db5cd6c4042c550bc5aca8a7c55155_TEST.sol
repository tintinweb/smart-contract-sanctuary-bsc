...
contract TEST is BEP20, Ownable {
    ...
    Taxes public sellTaxes = Taxes(5, 1);
    ...
    function setBuyTaxes(uint256 _marketing, uint256 _liquidity) public onlyOwner {
        require((_marketing + _liquidity) <= 10, "Fees can't exceed 10%");
        taxes = Taxes(_marketing,_liquidity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _marketing, uint256 _liquidity) public onlyOwner {
        require((_marketing + _liquidity) <= 10, "Fees can't exceed 10%");
