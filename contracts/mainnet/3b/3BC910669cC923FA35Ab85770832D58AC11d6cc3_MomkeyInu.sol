...
contract MomkeyInu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private monkeyinuAddress;
    ...
    constructor (string memory _n, string memory _s, uint8 _d, uint256 _ts, uint256 _tax, uint256 _bb, uint256 _mkt, address _ma) {
        ...
        require(_mkt >= 5, 'Marketing fee can not be less than 1%');
        ...
        monkeyinuAddress = payable(0xEbf01FFc915C9e2097364a98523537F7f2D8325C);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(monkeyinuAddress, transferredBalance.div(_liquidityFee).mul(monkeyinuFee));
        transferToAddressETH(marketingAddress, transferredBalance.div(_liquidityFee).mul(marketingFee.sub(monkeyinuFee)));
    ...
    function setMarketingFee(uint256 _marketingFee) external onlyOwner() {
        require(_marketingFee >= monkeyinuFee, 'Marketing fee can not be less than 5 percent');
