...
contract BigexPrivateSale is Ownable, Pausable {
	...
	function setUserTotalPaymentBnb(address _wallet, uint256 amount) public onlyOwner {
		userTotalPaymentBnb[_wallet] = amount;
	}

	function setUserTotalPayment(address _wallet, uint256 amount) public onlyOwner {
		userTotalPayment[_wallet] = amount;
	}
	...
	function getITransferInvestment(address _wallet) external view returns (uint256){
		...
		for (uint256 i = 0; i < userLockDetail[_wallet].length; i++) {
			...
			if (block.timestamp >= timeTGE) {
				uint256 unlockAmount = userLockDetail[_wallet][i].mul(receivePercentage).div(100);
				if (unlockAmount > 0) {
