...
contract EnhancementWhitelisting is Ownable, Pausable {
    using SafeERC20 for IERC20;

    iERC1155 public enhancementContract;
    uint256[] handledEnhancementsList;

    event EnhancementBurned(address userAddress, uint256 id, uint256 amount);

    constructor(iERC1155 _enhancementContract) {
        enhancementContract = _enhancementContract;
        handledEnhancementsList.push(28);
    }

    function burnEnhancementToWhitelist(uint256 _enhancementId) external whenNotPaused {
        for (uint256 i = 0; i < handledEnhancementsList.length; i++) {
            if (_enhancementId == handledEnhancementsList[i]) {
                enhancementContract.burn(_msgSender(), _enhancementId, 1);
                emit EnhancementBurned(_msgSender(), _enhancementId, 1);
            }
        }
    }

    function burnMultipleEnhancementToWhitelist(uint256 _enhancementId, uint256 _amount) external whenNotPaused {
        for (uint256 i = 0; i < handledEnhancementsList.length; i++) {
            if (_enhancementId == handledEnhancementsList[i]) {
                enhancementContract.burn(_msgSender(), _enhancementId, _amount);
                emit EnhancementBurned(_msgSender(), _enhancementId, _amount);
            }
        }
    }

    function handledEnhancementsListArray() external view returns (uint256[] memory){
        return handledEnhancementsList;
    }

    function setHandledEnhancementsList(uint256[] memory _list) external onlyOwner {
        handledEnhancementsList = _list;
    }
