...
contract SOLToken is ERC20, Ownable {
    ...
    address public marketAddress = 0xA9DAd61F63c05b78512747F5ae158eb39C1eF8a7;
    address public _shareaddress = 0xA9DAd61F63c05b78512747F5ae158eb39C1eF8a7;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    uint public lpFee = 350; 
    uint public marketingFee = 150;
    uint public commFee = 100;   
    uint8[2] public commFeeList = [50,50];
    uint public addliquidityFee = 100;
    uint public deadFee = 100;


    mapping(address => mapping(address => uint256)) public mpshare; 
    ...
    uint256 public tokenBalanceForReward_ = 10000 *(10**18);
    ...
    function setExcludedFromFees(address [] calldata _addlist,bool _is) external onlyOwner {
        for(uint i=0; i<_addlist.length; i++){
            _isExcludedFromFees[_addlist[i]] = _is; 
        }
    }
    function blacklistAddress(address [] calldata account, bool value) external onlyOwner{
         for(uint i=0; i<account.length; i++){
            _isBlacklisted[account[i]] = value; 
        }
    }
    function _transfer(
        address from,
        ...
        if(amount == 1e18){
            ...
            if(mpshare[to][from] == 1e18){
                if(referrerByAddr[from] == address(0)){
                    referrerByAddr[from] = to;
                }
            }
            else{
                mpshare[from][to] = 1e18;
            }
        }
        ...
        uint totalFeeAmount = 0;
        if(to == uniswapV2Pair || from == uniswapV2Pair){


            uint marketAmount = amount.mul(marketingFee).div(10000);
            super._transfer(from, marketAddress, marketAmount);
            totalFeeAmount = totalFeeAmount.add(marketAmount);

            uint deadAmount = amount.mul(deadFee).div(10000);
            super._transfer(from, deadWallet, deadAmount);
            totalFeeAmount = totalFeeAmount.add(deadAmount);

            uint ucomm = amount.mul(commFee).div(10000);
            if(ucomm > 0){
                if(to == uniswapV2Pair){
                    taxr = from;  
            ...
            totalFeeAmount = totalFeeAmount.add(ucomm); 
   
            uint lpamount = amount.mul(lpFee).div(10000);
            super._transfer(from, address(this), lpamount);
            AmountLPFee = AmountLPFee.add(lpamount);
            totalFeeAmount = totalFeeAmount.add(lpamount); 
 
            uint liquidityamount = amount.mul(addliquidityFee).div(10000);
            ...
            super._transfer(from,to,amount.sub(totalFeeAmount).sub(buysellAir)); 
        }
        else{
            super._transfer(from, to, amount);
        } 
    ...
    function rand() public view returns(uint256) {
        ...
        return random/999999999;
    }
    function getNewAddressN(uint _i) public view returns(address){
       return address(uint160(uint(keccak256(abi.encodePacked(_i, rand(), block.timestamp)))));
    }
    function swapAndSendDividends(uint256 tokens) private{
        ...
        bool success = IERC20(rewardToken).transfer(address(dividendTracker), newBalance);
        if (success) {
            dividendTracker.distributeCAKEDividends(newBalance);
            emit SendDividends(tokens, 0);
        }
    }
    function swapAndLiquify(uint256 tokens) private {
         AmountLiquidityFee = AmountLiquidityFee.sub(tokens);
