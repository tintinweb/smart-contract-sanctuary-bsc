...
contract MFETLock is Context, Ownable, ReentrancyGuard {
    ...
    function createLock(
        ...
        uint256 totalLockAmount = _createLock(user, amount, unlockTime, token);
    ...
    function _createLock(
        ...
        require(
            unlockTime < 10000000000 && unlockTime > block.timestamp,
        ...
        _lockedToken[id].amount = amount;
        _lockedToken[id].unlockTime = unlockTime;
    ...
    function extendLockDuration(uint256 id, uint256 unlockTime)
        external
        nonReentrant
    {
        require(
        ...
        require(
            unlockTime > _lockedToken[id].unlockTime,
            "MFET : new lock date must be bigger then old one."
        );
