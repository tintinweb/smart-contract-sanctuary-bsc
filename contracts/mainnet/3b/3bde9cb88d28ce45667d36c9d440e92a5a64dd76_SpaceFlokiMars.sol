...
contract SpaceFlokiMarsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SpaceFlokiMars_Dividend_Tracker", "SpaceFlokiMars_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SpaceFlokiMars_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SpaceFlokiMars_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SpaceFlokiMars contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SpaceFlokiMars_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SpaceFlokiMars_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SpaceFlokiMars is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SpaceFlokiMarsDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SpaceFlokiMars", "$SpaceFlokiMars") {
        
        BNBRewardsFee = 1;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra sell fee
        marketingFee = 1;
        marketingWallet = payable(0x588B3bC9F1b6684476043d54Ac6b0c719885B8A1);

    
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SpaceFlokiMarsDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SpaceFlokiMars: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SpaceFlokiMars: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SpaceFlokiMars: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SpaceFlokiMars: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SpaceFlokiMars: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SpaceFlokiMars: Cannot update gasForProcessing to same value");
