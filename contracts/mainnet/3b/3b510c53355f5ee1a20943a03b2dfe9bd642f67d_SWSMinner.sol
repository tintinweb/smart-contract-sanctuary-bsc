...
contract SWSMinner is Ownable {
    ...
    uint256 public rate3 = 3;
    uint256 public rate4 = 2;
    ...
    function setParentAddress(address _addr) public {
        ...
        uint256 userTokenDeposit = userDepositTotal(_addr).amount;
        require(userTokenDeposit>=minstake || _addr == _creator, "ParentAddress is not stake");
    ...
    mapping (address => uint256) activeStake;
    ...
    function addShareholder(address shareholder) private {
        if( !_updated[shareholder] ){
            ...
            if (parentAddress[shareholder] != address(0) ){
                activeStake[parentAddress[shareholder]]=activeStake[parentAddress[shareholder]].add(1);
            }
        }
    }
    ...
    function removeShareholder(address shareholder) private {
        ...
        if (parentAddress[shareholder] != address(0)){
            activeStake[parentAddress[shareholder]]=activeStake[parentAddress[shareholder]].div(1);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        uint256 rewardPrice = pool.allocRate.mul(_micWorth).div(100).div(currentprice).mul(10**18);
        ...
        while (pid != address(0)){
            ...
            if (i == 1){
                IERC20(minerToken).transfer(pid, rewardPrice.mul(rate1).div(100));
            }else if (i == 2){
                if (activeStake[pid]>=2){
                    IERC20(minerToken).transfer(pid, rewardPrice.mul(rate2).div(100));
                }
            }else if (i == 3){
                if (activeStake[pid]>=3){
                    IERC20(minerToken).transfer(pid, rewardPrice.mul(rate3).div(100));
                }
            }else if (i == 4){
                if (activeStake[pid]>=4){
                    IERC20(minerToken).transfer(pid, rewardPrice.mul(rate4).div(100));
                }
            }
