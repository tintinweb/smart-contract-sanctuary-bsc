...
/*
 __       __   ______   _______   __        _______          ______   __    __  _______  
|  \  _  |  \ /      \ |       \ |  \      |       \        /      \ |  \  |  \|       \ 
| $$ / \ | $$|  $$$$$$\| $$$$$$$\| $$      | $$$$$$$\      |  $$$$$$\| $$  | $$| $$$$$$$\
| $$/  $\| $$| $$  | $$| $$__| $$| $$      | $$  | $$      | $$   \$$| $$  | $$| $$__/ $$
| $$  $$$\ $$| $$  | $$| $$    $$| $$      | $$  | $$      | $$      | $$  | $$| $$    $$
...
contract FIFAWORLDCUP is IERC20, Context, Ownable {
    ...
    address private FifaWorldCup_Marketing_Wallet = 0xC767a84B2D474F56eF8B96e47E68cB96Be5D3002;
    address private FifaWorldCup_Team_Wallet = 0xC767a84B2D474F56eF8B96e47E68cB96Be5D3002;

    uint256 private FifaWorldCup_max_BuyLimit = 10000000000 * 10**9;
    uint256 private FifaWorldCup_max_SellLimit = 10000000000 * 10**9;

    uint256 private FifaWorldCup_max_Wallet = 20000000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[FifaWorldCup_Marketing_Wallet] = true;
        _isExcludedFromFee[FifaWorldCup_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[FifaWorldCup_Marketing_Wallet] = true;
        allowedTransfer[FifaWorldCup_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= FifaWorldCup_max_BuyLimit, "You are exceeding FifaWorldCup_max_BuyLimit");
            require(balanceOf(to) + amount <= FifaWorldCup_max_Wallet, "You are exceeding FifaWorldCup_max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= FifaWorldCup_max_SellLimit, "You are exceeding FifaWorldCup_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= FifaWorldCup_max_Wallet, "You are exceeding FifaWorldCup_max_Wallet");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(FifaWorldCup_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(FifaWorldCup_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function FifaWorldCup_Marketing(address newWallet) external onlyOwner{
        FifaWorldCup_Marketing_Wallet = newWallet;
    }
    
    function FifaWorldCup_Team(address newWallet) external onlyOwner{
        FifaWorldCup_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        FifaWorldCup_max_BuyLimit = maxBuy * 10**decimals();
        FifaWorldCup_max_SellLimit = maxSell * 10**decimals();
    }
    
    function FifaWorldCup_Max_Wallet(uint256 amount) external onlyOwner{
        FifaWorldCup_max_Wallet = amount * 10**decimals();
    }
