...
contract SellToken {
    ...
    event RefReward(
        address indexed user,
        uint256 indexed reward_amount,
        uint8 indexed level
    );

    modifier onlyIDO_ADMIN() {
        require(msg.sender == IDO_ADMIN, "INVALID IDO ADMIN");
        _;
    }
   ...
   */
     ...
     */
    function updateEnable(bool _is_enable) public onlyIDO_ADMIN {
     ...
     * @dev Update is ref
     */
    function updateRef(bool _is_ref) public onlyIDO_ADMIN {
        is_ref = _is_ref;
    }
     ...
     * @dev Update rate
     */
    function updateRate(uint256 rate) public onlyIDO_ADMIN {
        tokenRate = rate;
    }
     ...
     * @param recipient recipient of the transfer
     */
    function withdrawBNB(address recipient) public onlyIDO_ADMIN {
     ...
     * @param recipient recipient of the transfer
     */
    function updateLock(address recipient, uint256 _lockAmount)
        public
        onlyIDO_ADMIN
    {
     ...
     **/
    function buyIDO(uint256 buy_amount, address _referrer)
        public
        returns (uint256)
    {
        if (
            ...
            msg.sender != referrers[referrers[_referrer]]
        ) {
            ...
            if (referrers[referrers[_referrer]] != address(0)) {
                emit NewReferral(
                    referrers[referrers[referrers[_referrer]]],
                    msg.sender,
                    3
                );
            }
        ...
        if (is_ref) {
            ...
            if (referrers[referrers[msg.sender]] != address(0)) {
                ...
                IERC20(IDO_TOKEN).transfer(
                    referrers[referrers[msg.sender]],
                    f2_reward
                );
            ...
            if (referrers[referrers[referrers[msg.sender]]] != address(0)) {
                ...
                IERC20(IDO_TOKEN).transfer(
                    referrers[referrers[referrers[msg.sender]]],
                    f3_reward
                );
                refAmount[
                    referrers[referrers[referrers[msg.sender]]]
