...
contract CommonPassage is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, Ownable {
    using Address for address;
    using SafeMath for uint256;

    // Token name
    string private constant _name = "Common Passage";

    // Token symbol
    string private constant _symbol = "COMMONPASSAGE";
    ...
    receive() external payable {
        uint before = rewardToken.balanceOf(address(this));
        (bool s,) = payable(address(rewardToken)).call{value: address(this).balance}("");
