...
contract Faction is IERC20, Ownable {
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (liquidityFee, _sellFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 __sellFee, uint256 _giveawayFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        _sellFee = __sellFee;
        giveawayFee = _giveawayFee;
        totalFee = liquidityFee.add(reflectionFee).add(_sellFee).add(giveawayFee);
