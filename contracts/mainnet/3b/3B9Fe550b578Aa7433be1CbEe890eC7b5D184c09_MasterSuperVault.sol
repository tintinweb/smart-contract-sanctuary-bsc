...
abstract contract AVault is ERC20 {
...
contract MasterSuperVault is ERC20, Ownable {
    ...
    function updateVaults(address[] memory _vaults) public {
        ...
        require (AVault(_vaults[0]).strategist() == strategist);
        require (AVault(_vaults[1]).strategist() == strategist);
        require (AVault(_vaults[2]).strategist() == strategist);
        require (AVault(_vaults[3]).strategist() == strategist);
        require (AVault(_vaults[4]).strategist() == strategist);

        // Check the deployer address
        require (AVault(_vaults[0]).addrFactory() == addrFactory);
        require (AVault(_vaults[1]).addrFactory() == addrFactory);
        require (AVault(_vaults[2]).addrFactory() == addrFactory);
        require (AVault(_vaults[3]).addrFactory() == addrFactory);
        require (AVault(_vaults[4]).addrFactory() == addrFactory);        
