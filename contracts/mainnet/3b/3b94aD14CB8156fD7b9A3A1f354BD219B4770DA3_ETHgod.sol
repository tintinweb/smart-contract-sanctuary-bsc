...
contract ETHgod is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    uint256 public immutable  tokenRewardsFee=8;
    uint256 public immutable  liquidityFee=3;
    uint256 public immutable  marketingFee=1;
    uint256 public immutable  burnfee=1;
    uint256 public immutable  inviterFee=1;
    uint256 public immutable totalFees=14;
    address private Owner;
    address public immutable old_ETHgod =0x86BaAa2EBb6eA924FA9C54A7CA279906bD1D8882;//老合约地址
    address public _marketingWalletAddress;
    bool public is_update = true;
    ...
    mapping(address => address) public inviters1;
    ...
    constructor(
        ...
        address tokenowner=0x00a3A59F0EdA323E6112f28229B4a6078639a408;
        rewardToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
        _marketingWalletAddress = 0xDc19A05155D51CF653bDbE034AF7167b278dE077;
        swapTokensAtAmount =1*1e12*1e18; // 0.002%
        Owner=msg.sender;
        ...
        dividendTracker.excludeFromDividends(address(0xdead));
         dividendTracker.excludeFromDividends(address(0x00));
    ...
    function upgrade() public{
        require(is_update);
        require(IERC20(old_ETHgod).balanceOf(msg.sender) > 0);
        uint256 amount1=IERC20(old_ETHgod).balanceOf(msg.sender);
        address inviters  = iETH(old_ETHgod).inviter(msg.sender);
         if(inviters!= address(0))
        {
            inviters1[msg.sender] = inviters;//记录邀请人
        }
        TransferHelper.safeTransferFrom(old_ETHgod, msg.sender, address(this),IERC20(old_ETHgod).balanceOf(msg.sender));
         _mint(msg.sender, amount1);
       emit TokenCreated(msg.sender, address(this), TokenType.baby, VERSION);
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BABYTOKEN: gasForProcessing must be between 200,000 and 5000,000"
        );
        require(
    ...
    function _transfer(
        ...
        require(blacklist[from]==false, "must be in whitelist");
       ...
       require(to != address(0), "ERC20: transfer to the zero address");
        ...
        if (
            ...
            uint256 swapTokens = contractTokenBalance.mul(3).div(
                12
            );
        ...
        bool shouldSetInviter = balanceOf(to) == 0 && inviters1[to] == address(0) 
            && !isContract(from) && !isContract(to);
        

        if (takeFee) {
            uint256 fees = amount.mul(14).div(100);
            uint256 fff= amount.mul(12).div(100);
            uint256 burnff=amount.mul(1).div(100);
            uint256 inviterfee=amount.mul(1).div(100);
            amount = amount.sub(fees);
            super._transfer(from,burnaddress,burnff);
            super._transfer(from, address(this), fff);
             if(inviters1[to] == address(0)){
              super._transfer(from, burnaddress, inviterfee);
            }else{
             super._transfer(from, inviters1[to], inviterfee);}
        } 
  ...
  function swapAndSendToFee(uint256 tokens) private {
    ...
    function setinviters(address _account,address _inviter) public onlyOwner{
      inviters1[_account] =_inviter;
    }
