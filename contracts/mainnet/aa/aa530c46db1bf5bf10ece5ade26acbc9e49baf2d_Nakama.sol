...
contract Nakama is ERC20, Ownable {
    ...
    constructor() ERC20("Nakama", "NKMA") {
        ...
        dividendTracker = new NakamaDividendTracker(payable(this), router, 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
                                                   "NakamaDivTRACKER", "NKMADivTRACKER");
    ...
    function UpdateDividendTracker(address NewAddress) public onlyOwner {
        dividendTracker = NakamaDividendTracker(payable(NewAddress));
    }
...
contract NakamaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        NakamaContract = Nakama(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main Nakama contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NakamaContract);

            uint256 prevBalance = NakamaContract.balanceOf(address(this));
            ...
            {
                uint256 received = NakamaContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NakamaContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
