...
contract TokenStaking is ReentrancyGuard, Context, Ownable {
    ...
    constructor() {
        _token = IERC20(0x73FDC3C5F4adC2D9728f8577aFfcA143cc275b0A);
        BUSD = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        _wallet = payable(0x3C7989A630d2c8B79fd7BCF3C56823411E17BAc1);
        _rate = 250;
        _amountOfBUSD = 0;
        saleState=true;
    }
    ...
    function buyTokensWithBUSDforLp(uint256 amtBUSD) public nonReentrant {
        require(saleState==true,"Sale is not activated");
    ...
    function unstakeTokens(uint256 amount) public nonReentrant{
        ...
        uint256 canClaim = getReleaseAmt(msg.sender);
    ...
    function getReleaseAmt(address user) public view returns (uint256) {
        uint256 stakingPeriod = block.timestamp.sub(stakingTime[user]);
        uint256 canClaim = 0;
        uint256 state = 0;
        if (2 * 365 days > stakingPeriod && stakingPeriod > 365 days) {
            canClaim = stakingBalance[user].div(100).mul(20);
            state = 1;
        } else if (
            3 * 365 days > stakingPeriod && stakingPeriod > 2 * 365 days
        ) {
            canClaim = stakingBalance[user].div(100).mul(40);
        } else if (
            4 * 365 days > stakingPeriod && stakingPeriod > 3 * 365 days
        ) {
            canClaim = stakingBalance[user].div(100).mul(60);
            state = 3;
        } else if (
            5 * 365 days > stakingPeriod && stakingPeriod > 4 * 365 days
        ) {
            canClaim = stakingBalance[user].div(100).mul(80);
            state = 4;
        } else if (stakingPeriod > 5 * 365 days) {
            canClaim = stakingBalance[user];
            state = 5;
        }
        return canClaim;
    }
    ...
    function getNextReleaseTime(address user) public view returns (uint256) {
        if (!hasStaked[user]) {
            ...
            uint256 remainTime = time.mod(31536000);
            uint256 passedDays = remainTime.div(86400);
            if (passedDays > 365) {
                return 0;
            } else {
                uint256 remainDays = 365 - passedDays;
                return remainDays;
            }
        }
    }
    ...
    function getNextClaimTime(address user) public view returns (uint256) {
        if (!hasStaked[user]) {
            ...
            uint256 time = block.timestamp.sub(claimTime[user]);

            uint256 remainTime = time.mod(2592000);
            uint256 passedDays = remainTime.div(86400);
            if (passedDays > 30) {
                return 0;
            } else {
                uint256 remainDays = 30 - passedDays;
                return remainDays;
            }
        }
    }
    ...
    function withdraw() external onlyOwner {
        require(saleState==false,"Sale is activated");
