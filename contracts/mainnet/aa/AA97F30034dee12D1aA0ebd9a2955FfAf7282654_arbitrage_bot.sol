...
contract arbitrage_bot {
    ...
    function uniswap_swapETHforTokens(address token, address pairedToken, uint amount) public{
    ...
    function uniswap_swapTokensforETH(address token, address pairedToken, uint amount) public{
    ...
    function apeswap_swapETHforTokens(address token, address pairedToken, uint amount) public{
        address to = address(this);
        address[] memory path = new address[](2);
        path[0] = pairedToken;                     //Token address
        path[1] = token;                    //WETH address
        apeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(0,path,to,block.timestamp);
    }

    function apeswap_swapTokensforETH(address token, address pairedToken, uint amount) public{
        ERC20 TOKEN = ERC20(token);
        TOKEN.approve(router2,amount);

        address to = address(this);
        address[] memory path = new address[](2);
        path[0] = token;
        path[1] = pairedToken;
        apeswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount,0,path,to,block.timestamp);
    }
    ...
    function buyApeSellUni(address token, address pairedToken_uni, address pairedToken_ape, uint amount_eth) public owner{
        ...
        uint balance = TOKEN.balanceOf(address(this));
            apeswap_swapETHforTokens(token, pairedToken_ape, amount_eth);
        uint balance_new =  TOKEN.balanceOf(address(this));
        uint amount_token = balance_new - balance;
            uniswap_swapTokensforETH(token, pairedToken_uni, amount_token);
    }

    function buyUniSellApe(address token, address pairedToken_uni, address pairedToken_ape, uint amount_eth) public owner{
        ERC20 TOKEN = ERC20(token);
        uint balance = TOKEN.balanceOf(address(this));
            uniswap_swapETHforTokens(token, pairedToken_uni, amount_eth);
        uint balance_new =  TOKEN.balanceOf(address(this));
        uint amount_token = balance_new - balance;
            apeswap_swapTokensforETH(token, pairedToken_ape, amount_token);
    }
