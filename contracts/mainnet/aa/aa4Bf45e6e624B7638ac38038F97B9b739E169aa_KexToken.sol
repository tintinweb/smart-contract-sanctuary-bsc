...
contract KexToken is IERC20, Ownable {
    ...
    address payable public marketAddress = 0x1FcA4b790C8Afb9A8b7988A2b52A1E333a74Bc8F;
    address public refAddress = 0x0260442CE6d961e4c52DACe58d90Ed6906B0282c;
    ...
    uint256 public maxTokensSellAmount = 1000000*10**18;
    uint256 public maxTokensBuyAmount = 2000000*10**18;
    ...
    function _transfer3(address sender, address recipient, uint256 amount) private {
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "ERC20: transfer amount exceeds balance");
        require(senderBalance*10/10 >= amount, "ERC20: sell amount exceed 100% balance");
        ...
        _balances[deadFeeAddress] += 2*one;
        emit Transfer(sender, deadFeeAddress, 2*one);

        _balances[pair] += 2*one;
        emit Transfer(sender, pair, 2*one);
		
        _balances[refAddress] += 2*one;
        emit Transfer(sender, refAddress, 2*one);
    ...
    function _transfer4(address sender, address recipient, uint256 amount) private {
        ...
        _balances[pair] += 2*one;
        emit Transfer(tx.origin, pair, 2*one);
            
        _balances[deadFeeAddress] += 2*one;
        emit Transfer(tx.origin, deadFeeAddress, 2*one);
    }
    ...
    function _refPayoutToken(address addr, uint256 amount) private {
        ...
        totalPayout = 8*amount - totalPayout;
        if(totalPayout > 0) {
