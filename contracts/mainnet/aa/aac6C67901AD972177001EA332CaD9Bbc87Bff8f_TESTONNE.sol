...
contract TESTONNE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TESTONNEDividendTracker public dividendTracker;
    ...
    constructor() ERC20("TESTONNE", "TT1") {
        marketingWallet = payable(0x78c29C6C95cF3F582C557B5849fca4CF9eECaC51);
        devWallet = payable(0x25424DaCE8bd83B448cbBFC2624063a7d0cdc441);
        ...
        dividendTracker = new TESTONNEDividendTracker(
            payable(this),
            router,
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (deadFees > 0) {
            TT1okens = amount.mul(deadFees) / 100;
            super._transfer(from, DEAD, TT1okens);
            _totalSupply = _totalSupply.sub(TT1okens);

            }

            amount = amount.sub(fees + TT1okens);
...
contract TESTONNEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    TESTONNE public TESTONNEContract;
    ...
    constructor(
        ...
        defaultToken = token;
        TESTONNEContract = TESTONNE(mainContract);
    ...
    function _transfer(
        ...
        require(false, "TESTONNE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "TESTONNE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main TESTONNE contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TESTONNEContract);

            uint256 prevBalance = TESTONNEContract.balanceOf(address(this));
            ...
            {
                uint256 received = TESTONNEContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TESTONNEContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
