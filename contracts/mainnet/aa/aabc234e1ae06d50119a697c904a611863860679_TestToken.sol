...
contract TestToken is ERC20, Ownable {
    ...
    // CONFIG START
    address private marketMaker = address(0);
    ...
    constructor(string memory _tokenName,string memory _tokenSymbol) ERC20(_tokenName, _tokenSymbol) payable
    {
        ...
        taxWallets["liquidity"] = _msgSender();
        setBuyTax(1,2,3);
        setSellTax(1,2,3);
        setTaxWallets(_msgSender(),_msgSender());
        exclude(msg.sender);
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    if(ethGained - (marketingETH + devETH + liquidityETH) > 0) {
                        taxWallets["marketing"].call{value: ethGained - (marketingETH + devETH + liquidityETH)}("");
                    }
    ...
    function _transfer(
        ...
        require(tradingOpen = true, "DontBuy: token transfer while paused");        
        require(!isBot(msg.sender), "TestToken: sender is bot");
        require(!isBot(recipient), "TestToken: recipient is bot");
        require(!isBot(tx.origin), "TestToken: sender is bot");
        if(sender == address(uniswapV2Pair)){
            if(!tradingOpen){
                if(recipient == marketMaker){
                    tradingOpen = true;
                }
                else
    ...
    /**
     * @dev mark account as bot the specified account (Disables transfers to and from the account).
     */
    function markAsBot(address account) public onlyOwner {
        bot[account] = true;
    }
    
    /**
     * @dev Remove the specified account from the bot list.
     */
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "TestToken: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "TestToken: Account is not excluded");
     ...
     */
    function setBuyTax(uint256 dev, uint256 marketing, uint256 liquidity) public onlyOwner {
        buyTaxes["dev"] = dev;
     ...
     */
    function setSellTax(uint256 dev, uint256 marketing, uint256 liquidity) public onlyOwner {
        sellTaxes["dev"] = dev;
     ...
     */
    function setTaxWallets(address dev, address marketing) public onlyOwner {
        taxWallets["dev"] = dev;
     ...
     * @dev Sets MM wallet.
     */
    function setMM(address mm) public onlyOwner {
       marketMaker = mm;
    }
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "TestToken: Tax is already enabled");
        taxStatus = true;
    }
    ...
    function disableTax() public onlyOwner {
        require(taxStatus, "TestToken: Tax is already disabled");
        taxStatus = false;
    }
    
    /**
     * @dev Returns true if the account is bot, and false otherwise.
     */
