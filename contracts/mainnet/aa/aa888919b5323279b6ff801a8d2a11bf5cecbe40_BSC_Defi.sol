...
BSC Defi - MTD Miner
  ...
  ~ 1% stacking compound bonus every 24 hours, max of 7 days.
  ~ 30% of fees will go to MTD Miner.
  ~ 40% Tax of on 2 consecutive withdrawals.
    *40% Tax will stay in the contract.
    *This will be implemented from day 1.

  Community Lottery:
  ~ Lottery will run every 4hours or 100 participants.
  ~ 50 tickets max per user. 10000000 MTD = 50 tickets.
  ~ 3 MTD deposit = 1 per ticket. based on user deposit/compound.
  ~ 80% of the pot will be converted to miners and be sent to the winning address.
  ~ 20% fee will go to MTD Miner

  Whale Control Feature:
  ~ 36 hours cut off time.
  ~ 24 hours withdraw cooldown.
  ~ 2,000,000,000 MTD max withdrawal.
  ~ 10,000,000,000 MTD max deposits per wallet.
...
contract BSC_Defi {
    ...
    uint256 public WITHDRAWAL_TAX_DAYS = 400;
    ...
    uint256 public LOTTERY_TICKET_PRICE = 1000000 ether; /** 1000000  mtd **/
    ...
    uint256 public MIN_INVEST = 10000000 ether; /** 10000000 MTD  **/
	uint256 public WITHDRAW_COOLDOWN = 24 * 60 * 60; /** 24 hours  **/
    uint256 public WITHDRAW_LIMIT = 500000000 ether; /** 500000000 MTD  **/
    ...
    function buyEggs(address ref, uint256 amount) public payable{
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** referral rewards will be in MTD **/
                ...
                /** referral rewards will be in MTD value **/
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
    ...
    /** How many miners and eggs per day user will recieve based on MTD deposit **/
    ...
    function PROMO_EVENT_SPECIAL(bool value, uint256 addCompoundBonus, uint256 addReferralEvent, uint256 addMaxTicket) external returns (bool){
        require(msg.sender == owner, "Admin use only.");
        require(addCompoundBonus <= 800, "Additional compound bonus max value is 900(90%)."); /** 80% max **/
        require(addReferralEvent <= 200, "Additional referral bonus max value is 100(10%).");  /** 20% max **/
