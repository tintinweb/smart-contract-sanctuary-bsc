...
contract NftRandomMint is ERC721, ERC721Enumerable {
    ...
    uint256 public mintPriceBnb = 0.002 ether;
    ...
    function mintOne() external payable{
        ...
        require(_mintsStarted < MAX_NFT_SUPPLY,"All NFTs already minted");
    ...
    function getBestTierOfHolder(address holder) public view returns (uint256) {
        ...
        if(nftsHeld == 0) return 0;
        uint256 bestTier = rarity[tokenOfOwnerByIndex(holder, 0)];
        if(nftsHeld == 1) return bestTier;   
        
        uint256 check;

        for (uint i = 1; i <= nftsHeld; i++){
            check = tokenOfOwnerByIndex(holder, i);
            if(check < bestTier) bestTier = check;
        }
        return bestTier;
    }

    function setRarities(uint256[] calldata ids, uint256 tier) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) rarity[i] = tier;
    }
    ...
    function _popPendingAtIndex(uint256 _index) internal returns (uint256) {
        ...
        if (_index != pendingCount+alreadyMinted) {
            _pendingIds[_index] = 
                _pendingIds[pendingCount+alreadyMinted] 
                + pendingCount 
                + alreadyMinted 
                - _index;
        }
