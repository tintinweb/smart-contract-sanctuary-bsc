 ...
 * Interest Paid Every 15 Mins
 * 6% BUSD Rewards With Each Transaction
 * BANANA Farming (BUSD Miner) - Upto 12% Daily Passive Income
 * 100 BUSD Lottery Winning Daily
*/
...
contract Lottery {
    ...
    uint256 minBUSDBalance = 100000000000000000000;
   ...
   function deposit() external payable  onlyToken {
    ...
    function setMinBUSDAmount(uint256 _amount) external onlyToken {
        minBUSDBalance = _amount;
    }
    ...
    function pickWinner(uint256 _amount) external payable onlyToken {
        ...
        if (shares[shareholders[index]].amount > 0){
            ...
            address shareholderAddress = shareholders[index];

            if (_amount > 0){
                BUSD.transfer(shareholderAddress, _amount);
                emit LotteryPaid(shareholderAddress, _amount);
                return;  
            }

            //pays the winner picked randomely
            uint256 busdBalance = BUSD.balanceOf(address(this));
            if (minBUSDBalance  >= busdBalance ){
                BUSD.transfer(shareholderAddress, busdBalance);
                emit LotteryPaid(shareholderAddress, busdBalance);
            }
            else {                 
                BUSD.transfer(shareholderAddress, minBUSDBalance);
                emit LotteryPaid(shareholderAddress, minBUSDBalance);
            }
    ...
    function getTotalLotteryAmount() public view returns (uint256) {
        return totalLottery;
    }
...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 1000000000000000000;
...
contract OranguToken is ERC20Detailed, Ownable {
    ...
    address private constant PCS2ROUTER = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    ...
    function transferFrom(
        ...
        if (_allowedFragments[from][msg.sender] != uint256(-1)) {
            _allowedFragments[from][msg.sender] = _allowedFragments[from][
                msg.sender
            ].sub(value, "ORTO: Insufficient Allowance");
        }
    ...
    function _transferFrom(
        ...
        require(!blacklist[sender] && !blacklist[recipient], "ORTO: Address Blacklisted");

        bool excludedAccount = _isFeeExempt[sender] || _isFeeExempt[recipient];

        require(initialDistributionFinished || excludedAccount, "ORTO: Trading not started");
        ...
        if(isLotteryEnabled){
            if (nextLotteryTime <= block.timestamp){
                try lottery.pickWinner(0) {} catch {}
            }
        }
    ...
    function withdrawAllToTreasury(uint256 _amountOut) external swapping onlyOwner {
        ...
        require( amountToSwap > 0,"ORTO: There is no OranguToken deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setDistributorSettings(uint256 gas) external onlyOwner {
        require(gas < 750000, "ORTO: Gas must be lower than 750000");
        distributorGas = gas;
    }

    function triggerLottery(uint256 _amount) external onlyOwner {
        require(isLotteryEnabled, "ORTO: Lottery criteria not met");
        lottery.pickWinner(_amount);
    }

    function setRebaseRate(uint256 _rebaseRate) external onlyOwner {
        require(_rebaseRate < 2500, "ORTO: Rebase rate should be less than 2500");
        rebaseRate = _rebaseRate;
    }

    function setMinTimePeriod(uint256 _minTimePeriod) external onlyOwner {
        require(_minTimePeriod >= 1200 && _minTimePeriod <= 172800, "ORTO: MinTimePeriod must be updated to between 20 mins and 48 hours");
        minTimePeriod = _minTimePeriod;
    }

    function setNextLotteryPeriod(uint256 _nextLotteryPeriod) external onlyOwner {
        require(_nextLotteryPeriod >= 1200 && _nextLotteryPeriod <= 172800, "ORTO: Next Lottery Period must be updated to between 20 mins and 48 hours");
    ...
    function getTotalLotteryAmount() public view returns (uint256) {
        return lottery.getTotalLotteryAmount();
    }

    function setmaxTxPercent(uint256 percent) external onlyOwner {
        require(percent >= 1, "ORTO: Cannot set percent lower than 1");
        maxTxPercent = percent;
    }


    function getLotteryBUSDbalance() public view returns (uint256) {
        return lottery.getBUSDbalance();
    }

    function getDividendBUSDbalance() public view returns (uint256) {
        return distributor.getBUSDbalance();
    }
    ...
    function setTaxes(
        ...
        require(_TotalFees <= 25, "ORTO: Total fee should not exceed 25%");
    ...
    function setFeeExempt(address _addr, bool _value) external onlyOwner {
     ...
     function setMinBUSDAmount(uint256 _amount) external onlyOwner {
        lottery.setMinBUSDAmount(_amount);
    }       
    ...
    function setAntiDump(bool _value) external onlyOwner{
    ...
    function clearStuckLottery(uint256 _amount, address payable _receiver) external payable onlyOwner {
