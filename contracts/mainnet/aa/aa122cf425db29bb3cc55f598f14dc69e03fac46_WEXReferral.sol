...
interface IOldWedexReferral {
    ...
    function referrers(address _address) external view returns (ReferrersInfo memory);
}
...
contract WEXReferral is IWedexReferral, Ownable {
    ...
    IOldWedexReferral public oldReferralContract;
    ...
    constructor(
        IOldWedexReferral _oldWedexreferral,
    ...
    function migration(address[] memory _referrers ) public onlyOperator {
                ...
                if(!migrated[_referrers[j]]){
                    ...
                    if(oldteam.length>0){
                        for(uint256 i = 0; i<oldteam.length;++i){
                            IOldWedexReferral.ReferrersInfo memory referrersInfo = oldReferralContract.referrers(oldteam[i]);
                            referrers[oldteam[i]] = ReferrersInfo({
                              referrer: _referrers[j],
                              vipBranch: referrersInfo.vipBranch,
                              leaderCommission: referrersInfo.leaderCommission
                            });   
                        }
                        migration(oldteam);
