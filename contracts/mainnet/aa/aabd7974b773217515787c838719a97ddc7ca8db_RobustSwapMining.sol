...
contract RobustSwapMining is Ownable {
	...
	// Owner calls this function to remove 1 LP token from MasterChef
	...
	function getQuantity(address outputToken, uint256 outputAmount, address inputBNB) internal view returns (uint256) {

		uint256 quantity = 0;

		if (IRobustSwapFactory(factory).getPair(outputToken, inputBNB) != address(0)) {

			quantity = IOracle(oracle).consult(outputToken, outputAmount, inputBNB);
		}
