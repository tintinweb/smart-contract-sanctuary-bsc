...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minDistribution = 1000 * (10 ** 8);
...
contract DoggistaBsc is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Doggista";
    string constant _symbol = "DOGI";
    uint8 constant _decimals = 8;

    uint256 _totalSupply = 100 * 10**9 * (10 ** _decimals);
    uint256 public _maxTxAmountBuy = _totalSupply * 1 / 100; 
    uint256 public _maxTxAmountSell = _totalSupply * 1 / 100; 
    ...
    uint256 public totalFee  = liquidityFee + reflectionFee + MarketDevFee;

    uint256 feeDenominator  = 100;
    uint256 sellMultiplier  = 100;
    address autoLiquidityReceiver;
    address MarketDevFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        MarketDevFeeReceiver = msg.sender;
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        ...
        payable(MarketDevFeeReceiver).transfer(amountBNB * amountPercentage / 100);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBMarketing = amountBNB.mul(MarketDevFee).div(totalBNBFee);
                
        if (amountBNBReflection > 0){
            try distributor.deposit{value: amountBNBReflection}() {} catch {}
        }

        (bool tmpSuccess,) = payable(MarketDevFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _MarketDevFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        MarketDevFee= _MarketDevFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_MarketDevFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _MarketDevFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        MarketDevFeeReceiver = _MarketDevFeeReceiver;
       
        isFeeExempt[_autoLiquidityReceiver];
        isFeeExempt[_MarketDevFeeReceiver];
    }
