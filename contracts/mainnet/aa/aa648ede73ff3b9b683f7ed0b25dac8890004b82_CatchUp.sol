...
contract CatchUp is ERC20, Ownable {
    ...
    uint256 public  teamFeeOnBuy      = 0;
    uint256 public  teamFeeOnSell     = 0;

    uint256 private _totalFeesOnBuy     = 0;
    uint256 private _totalFeesOnSell    = 0;

    address public teamVestingWallet=0xFBA2d8d727cDBb5C956D09A5549A4ed7670e18Bf; //Team Vesting
    ...
    constructor (address newOwner) ERC20("Catch Up", "CU"){
        ...
        swapTokensAtAmount = 500_000_000_000 * (10 ** 18) / 5000;
    }
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _teamFeeOnBuy) external onlyOwner {
        require(
            _liquidityFeeOnBuy  + _teamFeeOnBuy <= 10,
    ...
    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _teamFeeOnSell) external onlyOwner {
        require(
            _liquidityFeeOnSell + _teamFeeOnSell <= 10,
            "Fees must be less than 10%"
        );
    ...
    function _transfer(
        ...
        if(takeFee && !zeroTax) {
            ...
            if(from == uniswapV2Pair) {
                _totalFees = 5+_totalFeesOnBuy;
            } else {
                _totalFees = 20+_totalFeesOnSell;
            }
