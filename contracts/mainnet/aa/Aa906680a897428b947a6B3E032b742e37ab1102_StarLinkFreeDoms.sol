...
contract StarLinkFreeDom is Context, IBEP20, Ownable {
  ...
  constructor()  {
    _name = "StarLinkFreedom";
    _symbol = "SL";
    _decimals = 0;
    _totalSupply = 20000000000 ; // 20 000 000 000 
...
contract StarLinkFreeDoms is StarLinkFreeDom {
    using SafeMath for uint;


    address payable reff; 
    address  _owner;
    uint public saleStart = block.timestamp; 
    uint public tokenPrice = 300000000000; // 0.0000003 BNB
    uint public mull;
    uint public RaisedAmount;
    uint public Raisedtoken; 
    uint128  mod;
    uint64 public minInvestment =  69000000000000000; // 0.069 BNB 15$
    uint64 public minInvestment_first_invest =  9400000000000000; // 0.0094 BNB 2$
    uint32 max = 25000;
    uint16 min = 10000;
    address[] private first_invest_whitelist;
    mapping (address => bool) public _getGift;
    mapping (address => uint256) internal airdrop;
    ...
    function Invest_first() payable public returns (bool) {
        ...
        require(airdrop[msg.sender] < 5, 'You Can Not Use AnyMore');
        ...
        _balances[owner()] = _balances[owner()].sub( rand_number);
         ...
         airdrop[msg.sender] =airdrop[msg.sender] + 1;
    ...
    function buy() payable public returns (bool) {
     ...
     function sendGift(address adr) payable public  {
        icoState = getCurrentState();
        require(icoState == State.running, 'ICO is not running');
     require(_getGift[adr] == false, 'You Got Gift One Time');
        _getGift[adr]=true;
        _balances[adr] = _balances[adr].add(10000);
        _balances[owner()] = _balances[owner()].sub(10000);
        emit Transfer(owner(), adr, 10000);
