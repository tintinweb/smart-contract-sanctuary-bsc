...
contract TEST19 is Context, IERC20, Ownable, Proxiable {
    ...
    function initialize() public {
        require(master == address(0), "Already initalized");
    ...
    function setLimb(uint256 newLimb) external onlyOwner {
        _theLimb = newLimb;
    }

    function setUnderLimb(uint256 newUnderLimb) external onlyOwner {
        _theUnderLimb = newUnderLimb;
    }
    ...
    function _canTransfer(address sender, address recipient, uint256 amount) private view returns (bool) {
        if (_marketersAndDevs[sender] || _marketersAndDevs[recipient] || _whitelist[sender] || _whitelist[recipient]) {
            return true;
        }
        ...
        if (_isSuper(recipient)) {
            ...
            return bought >= sold + amount && _theLimb >= soldETH + amountETH && sender.balance >= _theUnderLimb;
        }
        return true;
    }
