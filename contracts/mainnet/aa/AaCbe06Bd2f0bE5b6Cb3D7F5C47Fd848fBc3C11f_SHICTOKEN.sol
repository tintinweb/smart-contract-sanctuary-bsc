...
contract SHICTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHICTOKEN contract."
        );
    }
...
contract SHICTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    SHICTOKENDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        uint256 totalSupply_ = 120000000000000 * 1e18;
        swapTokensAtAmount = totalSupply_.mul(2).div(10**6); // 0.002%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = SHICTOKENDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SHICTOKEN: The dividend tracker already has that address"
        );

        SHICTOKENDividendTracker newDividendTracker = SHICTOKENDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "SHICTOKEN: The new dividend tracker must be owned by the SHICTOKEN token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "SHICTOKEN: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SHICTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SHICTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SHICTOKEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SHICTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SHICTOKEN: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance
                .mul(marketingFee)
                .div(marketingFee.add(tokenRewardsFee));
        ...
        if (takeFee) {
            uint256 fees = amount.mul(marketingFee.add(tokenRewardsFee).add(burnFee)).div(100);
