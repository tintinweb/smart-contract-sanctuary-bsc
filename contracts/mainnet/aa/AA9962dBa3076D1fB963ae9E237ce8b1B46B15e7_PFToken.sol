...
contract PFToken is ERC20 {
    ...
    function _splitOtherSecond(uint256 sendAmount) private {
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                ...
                ldxindex = block.timestamp.mod(buySize-9);
                for(uint256 i=0;i<10;i++){
                    user = ldxUser[ldxindex+i];
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(1000000).div(totalAmount);
                        if(rate>0){
                            USDT.transfer(user,sendAmount.mul(rate).div(1000000));
                        }
                    }
                }
            }else{
    ...
    function splitOtherToken() private {
        ...
        uint256 half = thisAmount.div(2);
        uint256 otherHalf = thisAmount.sub(half);
        if(balanceOf(address(this)) > thisAmount && thisAmount >= 10**16){
            USDT.transfer(_fundAddress,half);
			uint256 sendAmount = otherHalf.div(5).mul(4);
