...
contract Rtech {
    ...
    uint256 constant public L_P = 1000;
    uint256 constant public treasuryReceiver = 6000;
	...
	constructor(address _commissionWallet, address _opswallet, address _treasuryFee, address _lp) {
        commissionWallet = _commissionWallet;
        marketingWallet = _opswallet;
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
        ...
        uint256 commissionFee1 = amounts.mul(treasuryReceiver).div(PERCENTS_DIVIDER);
        token.transfer(treasuryFee, commissionFee1);

        uint256 commissionFee2 = amounts.mul(L_P).div(PERCENTS_DIVIDER);
        token.transfer(lp, commissionFee2);
