...
contract Along is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "Along";
    string private _symbol = "ALONG";
    uint8 private _decimals = 9;    
    bool public _swapEnabled = true;

    uint8 public _coolAlongBlock;
    uint8 public _coolAlongMarket;
    uint8 public _swapAlongTarget;
    uint8 public _swapAlongLimit;
    ...
    function excludeAlongReward(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded");        
        _isExcluded[account] = true;
    }   

    function updateAlongMaxWallet(uint256 amount) external onlyOwner{
    ...
    function updateAlongDevFee(uint256 rDev) external onlyOwner {       
	...
	function timeAlongClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
