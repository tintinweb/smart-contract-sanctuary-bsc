...
library PrivateStorage {
    ...
    struct OwnerVestingSettings {
        ...
        uint256 tgeLockDuration; //Private s
...
contract PrivatePool is IPrivatePool, ReentrancyGuard, Ownable {
  ...
  event ClaimedType(uint256 timestamp);
  ...
  event Finalized(uint256 projectAmount, uint256 timestamp);
  ...
  function finalize() external override onlyOwner {
    ...
    emit Finalized(projectAmount, block.timestamp);
  }
   ...
   function refundToken() internal {
    ...
    if(poolStates.totalRaised < poolSettings.hardCap) {
      ...
      if(refundType == PrivateStorage.RefundType.Burn) {
        IERC20(poolSettings.token).transfer(address(0), _refundAmount);
      } else {
  ...
  function claimType() external override onlyOwner _onlyGovernance {
    require(poolStates.state  != PrivateStorage.State.Ended, "pool ended");
    ClaimType = true;
    poolStates.finishTime = block.timestamp;

    emit ClaimedType(block.timestamp);
  }
  ...
  function withdrawPrivate() external _isFinalize onlyOwner {
    ...
    uint256 amount = actualCurrencyAmount.sub(firstCurrencyAmount);
    ...
    emit PrivateWithdrawn(amount, block.timestamp);
  }
