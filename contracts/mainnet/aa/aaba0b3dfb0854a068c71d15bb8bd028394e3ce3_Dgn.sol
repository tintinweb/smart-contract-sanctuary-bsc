...
contract Dgn is ERC20Burnable, Operator {
    ...
    uint256 public constant INITIAL_DGN_REWARD_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the DGN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Dragon Finance", "DGN") {
        // Mints 1 DGN to contract creator for initial pool setup
    ...
    function _getDgnPrice() internal view returns (uint256 _dgnPrice) {
        try IOracle(dgnOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Dgn: failed to fetch DGN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _dgnPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_dgnPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDgnOracle(address _dgnOracle) public onlyOperatorOrTaxOffice {
        require(_dgnOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints DGN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DGN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDgnPrice = _getDgnPrice();
            currentTaxRate = _updateTaxRate(currentDgnPrice);
            if (currentDgnPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        address _dgnRewardPool,
        address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        require(_dgnRewardPool != address(0), "!_dgnRewardPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_dgnRewardPool, INITIAL_DGN_REWARD_POOL_DISTRIBUTION);
