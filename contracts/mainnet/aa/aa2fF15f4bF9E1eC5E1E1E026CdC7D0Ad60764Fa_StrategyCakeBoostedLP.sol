...
contract StratManager is Ownable, Pausable {
    ...
    address public leavenFeeRecipient;
    
    event NewKeeper(address account);
    event NewStrategist(address account);
    event NewUniRouter(address addr);
    event NewVault(address addr);
    event NewLeavenFeeRecipient(address account);
    /**
     ...
     * @param _leavenFeeRecipient address where to send Leaven's fees.
     */
    constructor(
        address _keeper,
        address _strategist,
        address _unirouter,
        address _vault,
        address _leavenFeeRecipient
    ) public {
        require(_keeper != address(0) && _strategist!= address(0) && _unirouter!= address(0) && _leavenFeeRecipient!= address(0),
         "Address can not be the zero address");
        keeper = _keeper;
        strategist = _strategist;
        unirouter = _unirouter;
        vault = _vault;
        leavenFeeRecipient = _leavenFeeRecipient;
    }
    ...
    function setKeeper(address _keeper) external onlyManager {
        require(_keeper != address(0), "Address can not be the zero address");
        keeper = _keeper;
    ...
    function setStrategist(address _strategist) external {
        ...
        require(_strategist != address(0), "Address can not be the zero address");
        strategist = _strategist;
    ...
    function setUnirouter(address _unirouter) external onlyOwner {
        require(_unirouter != address(0), "Address can not be the zero address");
        unirouter = _unirouter;
    ...
    function setVault(address _vault) external onlyOwner {
        require(_vault != address(0), "Address can not be the zero address");
        vault = _vault;
        emit NewVault(_vault);
    }

    /**
     * @dev Updates leaven fee recipient.
     * @param _leavenFeeRecipient new leaven fee recipient address.
     */
    function setLeavenFeeRecipient(address _leavenFeeRecipient) external onlyOwner {
        require(_leavenFeeRecipient != address(0), "Address can not be the zero address");
        leavenFeeRecipient = _leavenFeeRecipient;
        emit NewLeavenFeeRecipient(_leavenFeeRecipient);
    }
...
abstract contract FeeManager is StratManager {
    ...
    uint public leavenFee = MAX_FEE - STRATEGIST_FEE - callFee;
    
    event UpdateCallFee(uint256 fee);
    ...
    function setCallFee(uint256 _fee) external onlyManager {
        ...
        callFee = _fee;
        leavenFee = MAX_FEE - STRATEGIST_FEE - callFee;
        emit UpdateCallFee(_fee);
    }
...
contract GasThrottler {
    ...
    address public constant GASPRICE = address(0xA43509661141F254F54D9A326E8Ec851A0b95307);
...
contract StrategyCakeBoostedLP is StratManager, FeeManager, GasThrottler {
    ...
    event ChargedFees(uint256 callFees, uint256 leavenFees, uint256 strategistFees);        
    event BeforeDeposit();
    event Harvest(address account);
    event HarvestOnDeposit(bool harvestOnDeposit);
    event ShouldGasThrottle(bool shouldGasThrottle);
    event RetireStrat();
    event PanicFunction();
    event Pause();
    event UnPause();
    event NewBootStaker(address account);
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _leavenFeeRecipient) public {
        require(_want != address(0) && _chef != address(0) && _boostStaker != address(0), "Address can not be the zero address");
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 leavenFeeAmount = nativeBal.mul(leavenFee).div(MAX_FEE);
        IERC20(native).safeTransfer(leavenFeeRecipient, leavenFeeAmount);

        uint256 strategistFee = nativeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(native).safeTransfer(strategist, strategistFee);

        emit ChargedFees(callFeeAmount, leavenFeeAmount, strategistFee);
    }
    ...
    function setHarvestOnDeposit(bool _harvestOnDeposit) external onlyManager {
        ...
        emit HarvestOnDeposit(_harvestOnDeposit);
    }

    function setShouldGasThrottle(bool _shouldGasThrottle) external onlyManager {
        shouldGasThrottle = _shouldGasThrottle;
        emit ShouldGasThrottle(_shouldGasThrottle);
    }
    ...
    function setBoostStaker(address _boostStaker) external whenPaused onlyOwner {
        require(_boostStaker != address(0), "Address can not be the zero address");
        boostStaker = _boostStaker;

        emit NewBootStaker(_boostStaker);
    }
