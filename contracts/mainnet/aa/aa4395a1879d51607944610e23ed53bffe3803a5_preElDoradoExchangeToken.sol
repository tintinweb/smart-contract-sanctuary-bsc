...
contract preElDoradoExchangeToken is Divine {
    ...
    constructor(address _vesting) Divine("preElDoradoExchange", "pEDE", 18) {
        require(_vesting != address(0), "vesting address is the zero address");
        uint256 initialSupply_ = 6363000 * 1e18;
    ...
    function setVestEnabled(bool _vestingEnabled) public onlyOwner {
    ...
    function _beforeTokenTransfer(address from_, address to_, uint256 amount_) internal override {
        if (to_ != vesting) {
            require((isApprovedSeller[from_] == true), "Account not approved to transfer pEDE.");
        } else {
            require(vestingEnabled, "Vesting no open yet.");
        }
    }
