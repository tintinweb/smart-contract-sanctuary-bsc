...
// Staking Pools for $ 
// 40% tokenA boost + 20% BNB boost FIXED
contract StakingStandard is RewardsDistributionRecipient, ReentrancyGuard, Pausable  {
    ...
    function boostByToken1(uint256 amount) external nonReentrant whenNotPaused {
        ...
        stakingTokenMultiplier1.safeTransferFrom(msg.sender, devFundAdd, amount);
    ...
    function boostByNFTsBulk(address[] calldata _erc721NFTContracts, uint256[] calldata _tokenIds) external nonReentrant whenNotPaused {
        for(uint i=0; i<_erc721NFTContracts.length; i++){
