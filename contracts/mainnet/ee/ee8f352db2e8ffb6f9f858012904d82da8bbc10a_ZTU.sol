...
interface ZtuWarp {
    ...
    function rewardNft(address user,uint256 amount) external ;
}
...
contract ZTU is ERC20 {
    ...
    address private tokenOwner = 0xa4804478461834Da7A0470423D988015a2b2243D;
    address private sellFeeAddr = 0x376CcfB92F1F5c33Df7E5EA50b61C1CfA8300b6f;
    address private buyFeeAddr = 0x376CcfB92F1F5c33Df7E5EA50b61C1CfA8300b6f;
    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);  
    
    uint256 private startTime = 1662037200;

    uint private buyFee = 50;
    uint private sellFee = 50;
	...
	function changeSwapWarp(ZtuWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if(startTime > block.timestamp){
            if(from == uniswapV2Pair || to == uniswapV2Pair){
                require(isAddLdx,"Only add liquidity before the start time");
            }
        }

		if(_isExcludedFromVipFees[from] || _isExcludedFromVipFees[to] || isAddLdx){
        ...
        if(from == uniswapV2Pair || to == uniswapV2Pair){
            ...
            if(from == uniswapV2Pair){
                super._transfer(from, _destroyAddress, a.mul(10));
                super._transfer(from, buyFeeAddr, a.mul(buyFee-10));                
                amount = a.mul(1000-buyFee);

            }else{                
                if(balanceOf(from) == amount)
                {
                    super._transfer(from, _destroyAddress, a.mul(10).sub(1));
                }
                else
                {
                    super._transfer(from, _destroyAddress, a.mul(10));
                }
                super._transfer(from, sellFeeAddr, a.mul(sellFee-10));
                amount = a.mul(1000-sellFee);
            }
        }
    ...
    function swapAndLiquify(uint256 tokenAmount) private {
        uint256 halfusdt = tokenAmount.div(10).mul(9);
    ...
    function swapAndLiquifyStepv1() public {
        ...
        if(usdtBalance.mul(tokenBalance) > 10**30){
