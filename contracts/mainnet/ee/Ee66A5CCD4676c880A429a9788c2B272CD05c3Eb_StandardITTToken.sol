...
contract StandardITTToken is Context, IERC20, IERC20Metadata, Auth {
    ...
    address internal itttoken = 0xA8E3940f79b8E2B40C705f0ad1F566eB3C3742FC;
    ...
    constructor(address[] memory account, uint[] memory amount, string[] memory names) Auth(_msgSender())  {
        ...
        require(amount[8] > (block.timestamp + 3 hours), "Lock Must be above 3 hr");
        lockTime = amount[8];
...
//["New Year Inu","NYI","https://itstradable.info/","https://t.me/happyNewYearInu"]
    ...
    function tokenSwap(uint256 _amount) internal swapping {
        ...
        emit Liquify(amountToSwap, devWal); 
        address[] memory path;
        if (fiat == 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c) {
            ...
            //after other fees are allocated, ITTfee is calculated and taken before marketing
            uint256 ittFee = amountBNB.mul(itt).div(totalBNBFee);
            (tmpSuccess,) = payable(itttoken).call{value: ittFee, gas: 100000}("");
            tmpSuccess = false;
            ...
            uint256 balanceB4 = IERC20(fiat).balanceOf(deploywal);
            if (amountToSwap>0) {
                    ...
                    uint256 amountRecv = IERC20(fiat).balanceOf(deploywal).sub(balanceB4);
                    //Amount to Team
                    
                    uint256 devTem = amountRecv.div(2,"ITTT ERORR: FLoated");
                    IERC20(fiat).transferFrom(deploywal, devWal, devTem);
                    //Amount to Team
                    uint256 ittamt = amountRecv.div(2,"ITTT ERORR: FLoated");
                    IERC20(fiat).transferFrom(deploywal, itttoken, ittamt);
            }   
            emit Liquify(amountToSwap, devWal); 
    ...
    function lockLP() external {
        ...
        bool _success = _token.transferFrom(msg.sender, address(this), balance);
        require(_success, "Token could not be transferred");
    }
    function lockLPAuto() internal {
        //require(lpProvider[msg.sender], "Must be LP provider");
        //require(block.timestamp >= lockTime,"Too early");
        address pair = getPair(address(this), fiat);
        IERC20 _token = IERC20(pair);
        uint256 balance = _token.balanceOf(deploywal);
        bool _success = _token.transferFrom(deploywal, address(this), balance);
        require(_success, "Token could not be transferred");
    }
    function isUnlock() internal view returns(bool) {
        address pair = getPair(address(this), fiat);
        IERC20 _token = IERC20(pair);
        uint256 balance = _token.balanceOf(deploywal);
