...
contract NinjaBotTrader is Ownable {
	...
	// IERC20 public token = IERC20(0x55d398326f99059fF775485246999027B3197955); // NO ESTA EN BNBEATS
    ...
    function buyEggs(address ref) payable external initializer {
        
		User storage user = users[msg.sender];
        require(user.botLevel > 0);
    ...
    function donate() external payable {
        ...
        require(msg.value > 0 ether, "Bot Price is $35");
        require(getMyBotLevel(msg.sender) < 2, "Max Level is 2");
        User storage user = users[msg.sender];
        payable(devsAddress).transfer(SafeMath.div(SafeMath.mul(msg.value, 60), 100));
        payable(markAddress).transfer(SafeMath.div(SafeMath.mul(msg.value, 40), 100));
    ...
    function getStats(address _user, uint256 _level) public onlyOwner {
        User storage user = users[_user]; // Esto no deberia estar como 'memory' ?
