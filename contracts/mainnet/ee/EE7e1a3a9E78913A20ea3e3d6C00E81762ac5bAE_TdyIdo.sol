...
contract TdyIdo{
    ...
    function calc(uint tdyAmount) public view returns(uint usdtAmountIn, uint fwtAmountIn){
        ...
        uint usdtAmount = tdyAmount*usdtPrice/10**18;
        usdtAmountIn = usdtAmount*7/10;
        (uint112 reserve0, uint112 reserve1,) = IPancakePair(USDT_FWT_PAIR_ADDRESS).getReserves();
        fwtAmountIn = IPancakeRouter(PANCAKE_ROUTER_ADDRESS).getAmountOut(usdtAmount*3/10, reserve0, reserve1);
    }
