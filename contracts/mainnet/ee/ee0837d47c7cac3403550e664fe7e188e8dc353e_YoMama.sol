...
contract YoMama is Context, IERC20, Ownable {
    ...
    string private constant _name = "YoMama Inu";
   ...
   function LETSGO(
    ...
    function setBuyTaxes(uint256 _Reflections, uint256 _Marketing, uint256 _Liquidity, uint256 _Operations) public onlyOwner {
       require(_Reflections+(_Marketing)+(_Liquidity)+(_Operations) <=12, "Buy Taxes cannot be higher than 12%");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuyLimit <= _tTotal / 1000, "MaxBuy can't be set lower than 0.1% of total supply");
        maxBuyLimit = maxBuy * 10**decimals();
        require(maxSellLimit <= _tTotal / 1000, "MaxSell can't be set lower than 0.1% of total supply");
        maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateMaxWalletlimit(uint256 amount) external onlyOwner{
        require(maxWalletLimit <= _tTotal / 1000, "MaxWallet can't be set lower than 0.1% of total supply");
