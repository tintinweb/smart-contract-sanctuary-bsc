...
contract RESToken is IBEP20
{
    ...
    address _miner;
    address _minerpool;
    uint256 _maxSupply= 1300000000 * 1e8;

     string constant  _name = 'Resource aggregator';
     string constant _symbol = 'RES';
        ...
        if(recipient == _pancakeAddress)
        {
            if(!isExcluded(sender))
            {
                ...
                if(onepercent > 0)
                {
                    ...
                    IRESMiner(_miner).AddOneCut(onepercent.mul(3));
