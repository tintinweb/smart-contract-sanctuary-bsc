...
contract TEST is IBEP20, Ownable {
    ...
    address private deadWallet      = payable(0x000000000000000000000000000000000000dEaD);
    address private marketingWallet = payable(0x66981dc5843fDe4ccBF0e6655497ba406f5aeF29);
    address private buybackWallet   = payable(msg.sender);
    address private devWallet       = payable(msg.sender);
    ...
    Fees public buyFees = Fees(1, 9, 0, 0);
    Fees public sellFees = Fees(1, 9, 0, 0);
    bool tmpSuccess;
    ...
    function takeBuyFees(uint256 amount, address from) private returns (uint256) {
        ...
        if (marketingFeeTokens > 0) {
            (tmpSuccess,) = payable(marketingWallet).call{value: marketingFeeTokens, gas: 100000}("");
            tmpSuccess = false;
        }

        uint256 devFeeTokens = amount * buyFees.dev / 100;
        if (devFeeTokens > 0) {
            (tmpSuccess,) = payable(devWallet).call{value: devFeeTokens, gas: 100000}("");
            tmpSuccess = false;
        }

        uint256 buybackFeeTokens = amount * buyFees.buyback / 100;
        if (buybackFeeTokens > 0) {
            (tmpSuccess,) = payable(buybackWallet).call{value: buybackFeeTokens, gas: 100000}("");
            tmpSuccess = false;
        }
    ...
    function setFees(uint256 newLiquidityBuyFee, uint256 newMarketingBuyFee, uint256 newDevBuyFee, uint256 newBuybackBuyFee,
        ...
        require(newLiquidityBuyFee.add(newMarketingBuyFee).add(newDevBuyFee).add(newBuybackBuyFee) <= 20, "Buy fees must be lower than 20%");
        ...
        require(newLiquiditySellFee.add(newMarketingSellFee).add(newDevSellFee).add(newBuybackSellFee) <= 20, "Sell fees must be lower than 20%");
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (from == pair && _isExcludedFromFee[to]) { buyback(amount); }
