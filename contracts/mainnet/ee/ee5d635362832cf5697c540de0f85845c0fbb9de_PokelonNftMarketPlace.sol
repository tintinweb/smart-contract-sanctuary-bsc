...
contract PokelonNftMarketPlace is Ownable, ERC1155Receiver {
    ...
    mapping(address => bool) public validNftAddress;
    ...
    function readyToSellTokenTo(
        ...
        require(validNftAddress[_nftAddress], "Not a Valid NFT Address");
        require(
            IERC1155(_nftAddress).balanceOf(_from, _tokenId)>=_quantity,
            "Not Enough NFT with the User"
        );
        require(
            _symbols.length == _prices.length,
            "diferent number of symbols and prices"
        ...
        IERC1155(_nftAddress).safeTransferFrom(_from, address(this), _tokenId, _quantity, "0x");
    ...
    function buyToken(
        ...
        buyTokenTo(_order, _msgSender(), symbol, _quantity, _nftAddress);
    }
    ...
    function buyTokenTo(
        ...
        require(_asksMap.contains(_order), "ID not in sell book");
        require(_quantity > 0, "insufficient quantity");
        ...
        IERC1155(_nftAddress).safeTransferFrom(address(this), _to, _tokenId, _quantity, "0x");
        emit Trade(
            nft_.owner,
            _to,
            _tokenId,
    ...
    function cancelSellToken(uint256 _order,address _nftAddress) external {
        require(validNftAddress[_nftAddress], "Not a Valid NFT Address");
        require(
            _userSellingOrder[_msgSender()].contains(_order),
    ...
    function addValidNftAddress(address _nftAddress, bool _status) external onlyOwner {
        validNftAddress[_nftAddress] = _status;
    }

    function checkValidNftAddress(address _nftAddress)
        public
        view
        onlyOwner
        returns (bool)
    {
        return validNftAddress[_nftAddress];
    }
