...
contract NFT is ERC721URIStorage {
    ...
    mapping(uint256 => string) private _downloadLink; // file link
    ...
    string private furl = "https://sourcesea-user-code.s3.ap-northeast-2.amazonaws.com/";
    ...
    constructor(address _marketplaceAddress) ERC721("SourceSeaNFT", "SSN") {
        marketplaceAddress = _marketplaceAddress;
        owner = payable(msg.sender);
    }
    
    function changeMarketplace(address _marketplaceAddress) public {
        require(msg.sender == owner, "you can't access this method");
        marketplaceAddress = _marketplaceAddress;    
    }

    function changeOwner(address _walletAddress) public {
        require(msg.sender == owner, "you can't access this method");
        owner = payable(_walletAddress);
    }
    
    // validate : test 완료
    function setfurl(string memory url) public {
        require(msg.sender == owner, "you can't access this method");
        furl = url;
    }

    // validate file : test 완료
    function getDownloadLinks() public view returns (string[] memory) {
        require(msg.sender == owner, "you can't access this method");

        uint256 _length = _tokenIds.current();
        string[] memory downloadlink_ = new string[](_length);

        for(uint256 i = 0; i<_length; i++) downloadlink_[i] = _downloadLink[i+1];

        return downloadlink_;
    }

    // Mint : IPFS URI - NFTInfo, DownloadURI, category
    function mintToken(string memory tokenURI, string memory downloadURI, uint256 category) public contains (furl, downloadURI) returns (uint256) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(msg.sender, newItemId);
        
        _creators[newItemId] = msg.sender;
        _category[newItemId] = category;
        _downloadLink[newItemId] = downloadURI;
    ...
    function getDownloadLink(uint256 tokenId) public view returns (string memory) {
        ...
        return _downloadLink[tokenId];
    }

    function setDownloadLink(uint256 tokenId, string memory downloadURI) public {
        require(ownerOf(tokenId) == msg.sender, "you are not owner");
        _downloadLink[tokenId] = downloadURI;
    }
    ...
    // 자기자신이 만든 NFT 리스트를 가져옴 (royalty를 받을 수 있는 NFT 목록을 가져옴)
...
contract Marketplace is ReentrancyGuard {
    ...
    // 이 부분 조정이 필요
    uint256 private ratioOfRoyaltyOwner = 3 ;
    uint256 private ratioOfRoyaltyProvider = 7;
    uint256 private ratioOfHundred = 100;
    address private owner;

    mapping(uint256 => MarketItem) private marketItemIdToMarketItem;
    mapping(uint256 => string) private blackList; // 불건전하거나 적합하지 않은 NFT를 Market에 올리지 못하도록 blacklist 추가
    ...
    /** 악성매물에 대한 blacklist */
    function setBlackList(uint256 tokenId, string memory reason) public {
        require(msg.sender == owner, "you are not owner");
        require(tokenId > 0, "this is invalid tokenId");
        require(compareStrings(reason,"") == false, "this is invalid reason");
        blackList[tokenId] = reason;
    }
    
    function cancelBlackList(uint256 tokenId) public {
        require(msg.sender == owner, "you are not owner");
        require(tokenId > 0, "this is invalid tokenId");
        blackList[tokenId] = "";
    }

    function isBlackList(uint256 tokenId) public view returns (string memory) {
        require(compareStrings(blackList[tokenId], "") == false, "not exist blacklist data");
        return blackList[tokenId];
    }
    
    /** RatioOfRoyalty */
    function setRatioOfRoyalty(uint256 ratio_owner, uint256 ratio_provider) public {
        require(msg.sender == owner, "you are not owner");
        require(ratio_owner > 0, "this is invalid ratio_owner");
        require(ratio_provider > 0, "this is invalid ratio_provider");
        ratioOfRoyaltyOwner = ratio_owner;
        ratioOfRoyaltyProvider = ratio_provider;
    }

    function getRatioOfRoyaltyOwner() public view returns (uint256) {
        return ratioOfRoyaltyOwner;
    }

    function getRatioOfRoyaltyProvider() public view returns (uint256) {
        return ratioOfRoyaltyProvider;
    }
    ...
    function createMarketItem(
        ...
        uint256 category = NFT(nftContractAddress).getCategory(tokenId);
     ...
     * marketplace to the msg.sender. It also sends the Royalty to the marketplace owner.
     */
    function createMarketSale(address nftContractAddress, uint256 marketItemId) public payable nonReentrant {
        ...
        uint256 priceOfRoyaltyOwner = price * ratioOfRoyaltyOwner / ratioOfHundred;
        uint256 priceOfRoyaltyProvider = price * ratioOfRoyaltyProvider / ratioOfHundred;
        uint256 totalPrice = price - priceOfRoyaltyOwner - priceOfRoyaltyProvider;

        marketItemIdToMarketItem[marketItemId].creator.transfer(priceOfRoyaltyOwner);
        payable(owner).transfer(priceOfRoyaltyProvider);
        marketItemIdToMarketItem[marketItemId].seller.transfer(totalPrice); // 판매자한테 거래대금 전송
        ...
        emit MarketItemSelled(
            ...
            nftContractAddress,
            tokenId,
            marketItemIdToMarketItem[marketItemId].creator, // 민팅한 사용자
            marketItemIdToMarketItem[marketItemId].seller, // 판매자
            marketItemIdToMarketItem[marketItemId].owner, // 구매자
            price
        );
    }

    function fetchMarketItemsByCategory(uint256 _category) public view returns (MarketItem[] memory) {
        uint256 itemsCount = _marketItemIds.current();
        ...
        MarketItem[] memory items = fetchAvailableMarketItems();
        
        for (uint256 i = 0; i < availableItemsCount; i++) {
            MarketItem storage item = marketItemIdToMarketItem[i + 1];
            if (item.category != _category) continue;
