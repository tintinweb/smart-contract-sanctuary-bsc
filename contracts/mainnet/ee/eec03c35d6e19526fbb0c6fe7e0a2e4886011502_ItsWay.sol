...
contract ItsWay is Context, Ownable {
    using SafeMath for uint256;

    string _name = "ItsWay";
    string _symbol = "ItsWay";
    uint8 _decimals = 9;
	...
	function calDevFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
		return _amount.mul(_devFee).div(
			10**2
		);
	}
	
	function procesIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
