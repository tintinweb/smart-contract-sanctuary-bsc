...
library NCommon {
    ...
    function getPairAddress(
        ...
        return
            IPancakeFactory(router.factory()).getPair(tokenAddrA, tokenAddrB);
    }
}
...
contract NoboToken is Context, IBEP20, Ownable {
    ...
    constructor(address payable routerAddress, address _usdt) public {
        usdt = _usdt;
    ...
    function _transfer(
        ...
        address pancakePair = NCommon.getPairAddress(
            pancakeRouter,
            address(this),
            usdt
        );
        ...
        if (from == pancakePair) {
            if (to == address(pancakeRouter)) {
                ...
                if (swapUnlock) {
                    ...
                    if (whiteListActive[to]) {
                        ...
                        uint256 totalFees = amount.mul(X2.add(Y2)).div(100);
            ...
            if (swapUnlock || from == owner()) {
                ...
                if (whiteListActive[from]) {
                    ...
                    uint256 totalFees = amount.mul(X3.add(Y3)).div(100);
    ...
    function getTokenBack(address tokenAddr) external {
        ...
        require(tokenAddr != address(this), "can not withdraw native tokens");
    ...
    function setBuyNFee(
        ...
        require(tx.origin == owner());
        require(X + Y + Z + X1 + Y1 + Z1 <= 25);
        X0 = X;
        Y0 = Y;
        Z0 = Z;
    }

    function setSellNFee(
        uint256 X,
        uint256 Y,
        uint256 Z
    ) public {
        require(tx.origin == owner());
        require(X + Y + Z + X0 + Y0 + Z0 <= 25);
        X1 = X;
        Y1 = Y;
        Z1 = Z;
    }

    function setBuyNotNFee(uint256 X, uint256 Y) public {
        require(tx.origin == owner());
        require(X + Y + X3 + Y3 <= 25);
        X2 = X;
        Y2 = Y;
    }

    function setSellNotNFee(uint256 X, uint256 Y) public {
        require(tx.origin == owner());
        require(X + Y + X2 + Y2 <= 25);
        X3 = X;
        Y3 = Y;
    }
        ...
        payable
    {
        ...
        if (tokenAddress == usdt) {
            ...
            path[1] = usdt;
            uint256[] memory usdtCanSwap = pancakeRouter.getAmountsOut(
                amount,
                path
            );
            ...
            usdtSwap = payNSwapTxFee(
                msg.sender,
                usdtCanSwap[1],
                usdtCanSwap[1].mul(X0) / 100,
                usdtCanSwap[1].mul(Y0) / 100,
                usdtCanSwap[1].mul(Z0) / 100
            );
        } else {
            amount = _amount;
            IBEP20(tokenAddress).transferFrom(
    ...
    function swapNForToken(uint256 amount, address tokenAddress) public {
        ...
        usdtSwap = payNSwapTxFee(
            msg.sender,
            usdtCanSwap[1],
            usdtCanSwap[1].mul(X1) / 100,
            usdtCanSwap[1].mul(Y1) / 100,
            usdtCanSwap[1].mul(Z1) / 100
        );
    ...
    function getBuyNotNFee() public view returns (uint256, uint256) {
        return (X2, Y2);
    }

    function getSellNotNFee() public view returns (uint256, uint256) {
        return (X3, Y3);
    }
...
contract NInternal {
    ...
    constructor(address _root, address _usdt) public {
        root = _root;
        usdt = _usdt;
    }
