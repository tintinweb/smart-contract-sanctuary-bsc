...
contract AUA is ERC20, Ownable {
    ...
    uint256 public buyDeadFee = 4;
    uint256 public sellDeadFee = 4;
    ...
    constructor(
        ...
        address[4] memory addrs, // reward, router, dividendTracker, _onwer
        uint256 tokenBalanceForReward_
    ) payable ERC20(name_, symbol_)  {
        rewardToken = addrs[0];

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(_inviterFee).add(buyDeadFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(_inviterFee).add(sellDeadFee) <= 25, "Total sell fee is over 25%");
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 deadFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(deadFee) <= 25, "Total buy fee is over 25%");
    ...
    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 deadFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(deadFee) <= 25, "Total sel fee is over 25%");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                IFee = amount.mul(_inviterFee).div(100);

                DFee = amount.mul(buyDeadFee).div(100);
                fees = LFee.add(RFee).add(IFee).add(DFee);
            }
            ...
            // inviter
            if (IFee > 0) _takeInviterFee(from, to, amount);
    ...
    function _takeInviterFee(
        ...
        if (sender == uniswapPair) {
            ...
            super._transfer(sender, deadWallet, tAmount.mul(_inviterFee).div(100));
            return;
        }
        ...
        super._transfer(sender, deadWallet, tAmount.mul(_inviterFee.sub(accurRate)).div(100));
    }
