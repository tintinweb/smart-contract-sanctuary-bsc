...
contract AMBTOKEN is ERC20, Ownable {
    ...
    address public feeWallet = payable(0x1f40b3c714A28734523da39f6a5653f88EDD1D64);
    ...
    constructor() ERC20("Ambrionic Token ", "AMBToken ") {
        ...
        uint256 totalSupply = 840* 1e6 * 1e8;   
        
        maxTransactionAmount = totalSupply.mul(5).div(1e5);   // 0.1% maxTransactionAmountTxn
        maxWallet = totalSupply.mul(3).div(100);              // 3% maxWallet
        swapTokensAtAmount = 100 * 1e8; 
    ...
    function updateBuyFees(uint256 liquidityFee, uint256 devFee) external onlyOwner {
        _buyLiquidityFee = liquidityFee;
        _buyDevFee = devFee;
        buyTotalFees =_buyLiquidityFee + _buyDevFee;
    }
    
    function updateSellFees(uint256 liquidityFee, uint256 devFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                ...
                DFEE = fees * _sellDevFee / sellTotalFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && buyTotalFees > 0) {
        	    fees = amount.mul(buyTotalFees).div(100);
        	    _tokensForLiquidity += fees * _buyLiquidityFee / buyTotalFees;
                DFEE = fees * _buyDevFee / buyTotalFees;
            }
            
            if (fees > 0) {
                super._transfer(from, address(this), fees);
                super._transfer(address(this), feeWallet , DFEE);
            }
    ...
    function swapBack() private {
        ...
        uint256 HalfliquidityTokens = _tokensForLiquidity.div(2);
        uint256 OtherHalfliquidityTokens = _tokensForLiquidity.sub(HalfliquidityTokens);

        uint256 initialETHBalance = address(this).balance;
        _swapTokensForEth(OtherHalfliquidityTokens); 
        uint256 ethForLiquidity = address(this).balance.sub(initialETHBalance);
        
                
        if (HalfliquidityTokens > 0 && ethForLiquidity > 0) {
            _addLiquidity(HalfliquidityTokens, ethForLiquidity);
            emit SwapAndLiquify(OtherHalfliquidityTokens, ethForLiquidity, _tokensForLiquidity);
        }
