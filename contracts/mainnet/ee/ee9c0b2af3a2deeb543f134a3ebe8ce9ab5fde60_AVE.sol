...
contract BEP20 is Ownable, IBEP20, IBEP20Metadata {
    ...
    function _transfer(
        ...
        require(!isFreezed[from], "BEP20: Account is freezed");
        require(!isFreezed[to], "BEP20: can't sent to freezed account");
    ...
    /**@dev freeze bots/ illicit accounts
     */

    function freezeAddress (address _user) external onlyOwner {
        isFreezed[_user] = true;
    }

    /**@dev Unfreeze accounts if accidently freezed by owner
     */

    function unFreezeAddress (address _user) external onlyOwner {
        isFreezed[_user] = false;
    }
...
// OpenZeppelin Contracts (last updated v4.5.0) (token/BEP20/extensions/BEP20Burnable.sol)
...
contract AVE is BEP20, BEP20Burnable, Pausable {
    constructor() BEP20("AVEH20", "AVE") {

        _mint(msg.sender, 1_000_000_000_000 * 10 ** decimals());
         // 1,000,000,000,000 initial supply
    }
    ...
    function transfer(address to ,uint256 amount) public whenNotPaused
     ...
     function claimStuckedTokensAndBNB(address _token) external onlyOwner {
        ...
        if (_token == address(0x0)) {
            ...
            return;
             ...
             require(balanceOf(address(this))> 0, "AVE: Contract balance should be greator than zero");
           _transfer(address(this), owner(),balanceOf(address(this)));

        } else {

        uint256 balance = IBEP20(_token).balanceOf(address(this));
        IBEP20(_token).transfer(owner(), balance);
        }
    }
