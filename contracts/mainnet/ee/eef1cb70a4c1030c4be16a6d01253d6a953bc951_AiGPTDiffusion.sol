...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
...
contract AiGPTDiffusion is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   AiGPTDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("AI ML LLM LSTM GPT Diffusion", "AiGPT") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 4;
		_buyMarketingFee = 3;
        _buyDevFee = 1;
    	...
    	dividendTracker = new AiGPTDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x97B3aD409316954306670936250A571eFDCC6248);
        _devWallet = payable(0x16e89F5e80620F1215F18C8bf1145A452FBC9BBd);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AiGPT: The dividend tracker already has that address");

        AiGPTDividendTracker newDividendTracker = AiGPTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AiGPT: The new dividend tracker must be owned by the AiGPT token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "AiGPT: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AiGPT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AiGPT: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "AiGPT: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AiGPT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AiGPT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "AiGPT: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "AiGPT: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AiGPT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AiGPT: Cannot update gasForProcessing to same value");
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(2,2,2,2);
        updateSellFees(4,4,4,4);

        updateMaxWallet(500000000);
...
contract AiGPTDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AiGPT_Dividend_Tracker", "AiGPT_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "AiGPT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AiGPT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "AiGPT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AiGPT_Dividend_Tracker: Cannot update claimWait to same value");
