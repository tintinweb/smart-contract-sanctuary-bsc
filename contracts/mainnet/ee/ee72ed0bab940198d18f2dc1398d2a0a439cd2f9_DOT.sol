...
contract DOT is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletLimit = _tTotal * 1 / 66;
    
    uint256 public genesis_block;
    uint256 private deadline;
    
    address public marketingWallet = 0xA4A93C8004870fA4AA6978C64dc2F9207a9f7419;
    address private operationsWallet = 0xA4A93C8004870fA4AA6978C64dc2F9207a9f7419;
    address private extraWallet = 0xA4A93C8004870fA4AA6978C64dc2F9207a9f7419;

    string private constant _name = "DOT";
    string private constant _symbol = "DOTDOTDOT";
    ...
    Taxes public sellTaxes = Taxes(0, 3, 1, 1, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[extraWallet] = true;
        ...
        allowedTransfer[extraWallet] = true;
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _extra) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _operations + _extra) <= 20, "Fees can't exceed 20%");
        buyTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_extra);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _extra) public onlyOwner {
        require((_rfi + _marketing + _liquidity + _operations + _extra) <= 20, "Fees can't exceed 20%");
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_extra);
        emit FeesChanged();
    }
    ...
    function _takeextra(uint256 rextra, uint256 textra) private {
        totFeesPaid.extra +=textra;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=textra;
        }
        _rOwned[address(this)] +=rextra;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.roperations, to_return.rextra) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.textra = tAmount*temp.extra/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.toperations-s.textra;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rextra = s.textra*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-roperations-rextra;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 roperations,uint256 rextra) {
        ...
        rextra = s.textra*currentRate;
        return (roperations,rextra);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.toperations+ s.textra);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rextra > 0 || s.textra > 0) _takeextra(s.rextra, s.textra);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.operations + temp.extra) * 2;
        ...
        uint256 extraAmt = unitBalance * 2 * temp.extra;
        if(extraAmt > 0){
            payable(extraWallet).sendValue(extraAmt);
        }
    ...
    function updateextraWallet(address newWallet) external onlyOwner{
        require(newWallet != address(0),"Fee Address cannot be zero address");
