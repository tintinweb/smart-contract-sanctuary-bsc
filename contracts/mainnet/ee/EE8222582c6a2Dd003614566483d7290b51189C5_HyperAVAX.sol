...
contract HyperAVAX is Context, IERC20, Ownable {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    uint256 avaxLiquidityToken = contractTokenBalance.mul(_sellRewardFee).div(_sellLiquidityFee);
                    uint256 busdLiquidityToken = contractTokenBalance.mul(_sellMarketingFee).div(_sellLiquidityFee);
                    uint256 liquidityToken = contractTokenBalance.sub(avaxLiquidityToken+busdLiquidityToken);

                    // Remove Hate Swap and Liquidity by breaking Token in proportion
                    addLiquidityToToken(liquidityToken);
                    
                    // Token Swapping in BNB for avax
                    swapTokens(avaxLiquidityToken);    
                    // Fetch balance of the token swapped for BNB for avax
                    uint256 bnbBalavax = address(this).balance;
                    // Swap BNB for AVAX and send to Staking Address
                    SwapAndLiquifyBNBtoAvax(bnbBalavax);
    ...
    function manualSwapAndLiquifyBNBtoAvax(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
    ...
    function SwapAndLiquifyBNBtoAvax(uint256 bnbLiquifyAmount) private lockTheSwap {
        ...
        uint256 stakingFunds = newTokenBalance;
