...
contract RigToken is Context, IERC20, Ownable {
    ...
    uint[] internal shareConfig = [30,5,5,5,5,5,5];

    uint256 public totalFee = 80;

    uint256 public swapAmountLimit = 20 * 10 ** 9;
    ...
    constructor (address _route,address _holder,address _marketAddress,address _wbnb) public {
         wbnb = _wbnb;
         holder = _holder;
    ...
    function setBuyLimit(uint limit)external onlyOwner{
        swapAmountLimit = limit;
    }
    ...
    function _takeShare(uint tShare,address from,address user) private {
        ...
        if( tShare > sended ){
            _take(tShare - sended,from,holder);
        }
    }
    ...
    function _transfer(
        ...
        if( 
            ...
            uint marketAmount = contractTokenBalance * _marketFee / 20;
            swapAndMarket(marketAmount);
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(amount <= swapAmountLimit,"tx exceed limit");
        }
