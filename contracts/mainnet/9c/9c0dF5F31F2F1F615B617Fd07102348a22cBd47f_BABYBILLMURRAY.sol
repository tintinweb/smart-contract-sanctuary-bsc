...
contract BABYBILLMURRAY is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 _totalSupply = 100 * 1e12 * 1e9; //100 tillion
    ...
    function swapBack() private {
        ...
        uint256 balanceBeforeSwap = RealCoinCasino.balanceOf(address(this));

        swapTokensForRcc(rccLiquidityTokens);

        uint256 balanceAfterSwap = RealCoinCasino.balanceOf(address(this)).sub(
            balanceBeforeSwap
        );
        ...
        uint256 ethAvailableForLiq = ethBalance -
            ethForMarketing -
            ethForRewards -
            ethForTeam;
        ...
        if (balanceAfterSwap > 0 && ethForRccLiquidity > 0) {
            addLiquidityRCC(balanceAfterSwap, ethForRccLiquidity);
