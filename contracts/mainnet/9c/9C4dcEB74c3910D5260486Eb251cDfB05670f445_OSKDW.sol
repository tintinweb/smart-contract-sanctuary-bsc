...
contract OSKDW is ERC20 {
    ...
    address _tokenOwner;
    address public marketAddress = 0x4C10C39caF7c25352a7500f221d2D226982353Cb ;
    ...
    uint256 private tokensForSellToUSDT;
    ...
    function _transfer(
        ...
        tokensForSellToUSDT = AmountMarketingCurrent+AmountDivtokenBonusCurrent;
        tokensForLiquidity = AmountLiquidityCurrent;
        ...
        if( IERC20(uniswapV2Pair).totalSupply() > 0 && balanceOf(address(this)) > balanceOf(address(uniswapV2Pair)).div(10000) && to == address(uniswapV2Pair)){
            if (
                ...
                if(tokensForSellToUSDT >= swapTokensAtAmount) swapProc;
                if(tokensForLiquidity >= swapTokensAtAmount) swapAndLiquify;
                swapping = false;
            }
        ...
        if(takeFee) {
            ...
            if(from == uniswapV2Pair){
               ...
               finalAmount = finalAmount.sub(_LiquidityFee).sub(_MarketingFee).sub(_DeadFee).sub(_lpdivdendFee);
                ...
                _MarketingFee = amount.mul(marketingFee).div(1000);
                _DeadFee = amount.mul(deadFee).div(1000);
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_lpdivdendFee).sub(_MarketingFee).sub(_DeadFee);
            }
   ...
   function swapProc() public {
           ...
           tokensForSellToUSDT = AmountMarketingCurrent+AmountDivtokenBonusCurrent;
            ...
            swapTokensForCake(tokensForSellToUSDT);
