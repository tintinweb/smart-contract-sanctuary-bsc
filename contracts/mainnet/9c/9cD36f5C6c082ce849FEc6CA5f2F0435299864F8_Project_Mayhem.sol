...
contract Project_Mayhem is IERC20, Ownable {
    ...
    string private _name = "Project Mayhem";
    string private _symbol = "FCLUB";
    ...
    uint256 public _maxTx = (_tTotal * 10) / 500; //2%
    uint256 public _maxWallet = (_tTotal * 10) / 500; //2%

    uint256 private numTokensSellToAddToLiquidityAndBuyback =
        (_tTotal * 10) / 4000; //0.01%

    address public buybackAddress = 0x65CA6d2835A5651c094d683EdB8bdda2Fe64f459;
    address private buyback2 = 0x65CA6d2835A5651c094d683EdB8bdda2Fe64f459;
...
// Testnet router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
    ...
    function setMaxTxExempt(address holder, bool exempt) external onlyOwner {
        _isMaxTxExempt[holder] = exempt;
    }    

    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner {
        _maxWallet = _tTotal.mul(maxWalletPercent).div(10**2);
    }
    ...
    function setSwapPercent(uint256 maxSwapPercent) external onlyOwner {
        numTokensSellToAddToLiquidityAndBuyback = _tTotal.mul(maxSwapPercent).div(
            10**5
        );
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >=
            numTokensSellToAddToLiquidityAndBuyback;
        if (
            overMinTokenBalance &&
            !inSwap &&
            from != uniswapV2Pair &&
            ...
            contractTokenBalance = numTokensSellToAddToLiquidityAndBuyback;
            swap(contractTokenBalance);
        }

        uint256 contractBalanceRecipient = balanceOf(to);
        require(contractBalanceRecipient + amount <= _maxWallet,
        "Exceeds maxWallet." );
    ...
    function swap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (forBuyback > 0) {
            ...
            uint256 ethforBuybacks = address(this).balance.sub(initialBalance);
            uint256 ethForBuyback = ethforBuybacks.div(2);
            uint256 ethforBuyback2 = ethforBuybacks.sub(ethForBuyback);
            (bool sent1, ) = payable(buybackAddress).call{value: ethForBuyback}("");
            (bool sent2, ) = payable(buyback2).call{value: ethforBuyback2}("");
            require(sent1);
            require(sent2);
        }
