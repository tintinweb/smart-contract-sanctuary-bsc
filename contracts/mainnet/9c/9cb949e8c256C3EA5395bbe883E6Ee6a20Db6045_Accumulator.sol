 ...
 */
contract Accumulator is IERC20, Ownable {

    using SafeMath for uint256;

    // total supply
    uint256 private _totalSupply = 2_100_000 * 10**18;

    // token data
    string private constant _name = 'Accumulator';
    string private constant _symbol = 'ACCU';
    ...
    uint256 public emission_rate = 1458333333300000000;
    ...
    function setDontTriggerEmissions(address account, bool dontTrigger) external onlyOwner {
        require(account != address(0), 'Zero Address');
        permissions[account].dontTriggerEmissions = dontTrigger;
    }
    ...
    function setEmissionRate(uint256 newRate) external onlyOwner {
        require(
            newRate <= 10**38,
            'Cannot Raise Emission Rate Above 10^20 Accumulator Per Block'
        );
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (
            permissions[msg.sender].dontTriggerEmissions == false
        ) {
    ...
    function _stake(uint256 amount, address recipient) internal returns (bool) {
        require(
            amount > 0,
            'Zero Amount'
        );
        require(
            recipient != address(0) && recipient != address(this) && recipient != staking,
            'Invalid Recipient'
        );
        require(
        ...
        _balances[staking] = _balances[staking].add(amount);
        emit Transfer(msg.sender, staking, amount);

        // sell token for user
        IStaking(staking).stake(amount, recipient);
        return true;
    }
