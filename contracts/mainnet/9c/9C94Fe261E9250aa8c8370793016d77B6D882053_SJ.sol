...
// 提取产出（级别）
    function updateTX(uint256 id, uint256 OutGold,uint256 SEOSQuantity) external canCall {
        require(id > 0, "isplayer");  
        _SEOSPlayerMap[id].SEOSQuantity  = SEOSQuantity;
...
// 提取产出（级别）
    function updatecommunity(uint256 id, uint256 OutGold,uint256 communitySEOSQuantity, bool isjf) external canCall   
    {
        if(!isjf){
            _SEOSPlayerMap[id].communitySEOSQuantity  = communitySEOSQuantity;
        }else{
            JFFH[id].communitySEOSQuantity = communitySEOSQuantity;
        }
        _SEOSPlayerMap[id].EOSmining.OutGold  = OutGold;
    }
...
// 更新当日代币产出
        ...
        if(paytype == 3){
            ...
            levelgod(msg.sender,USDT_Num,15,3,true);
        }else
        {
            grantProfit(_SEOSPlayerMap[id].superior,USDT_Num.div(20),2,false);
            levelgod(msg.sender,USDT_Num,15,3,false);
    ...
    function updatepIDO(address Destination,address SEOSPlayerAddress,uint256 USDT_T_Quantity) external     canCall   {
        ...
        if(id == 0)
        {
            this.SEOSPlayeRegistry(msg.sender,Destination);
        }
