...
contract DaYu is ERC20, Ownable {
    ...
    uint256 public startTime = 1643545800;
    mapping(address => bool) public automatedMarketMakerPairs;
    receive() external payable {}

    constructor(address tokenOwner) ERC20("DaYu", "DaYu") {
    ...
    function setTax(uint256 taxBuy, uint256 taxSell) public onlyOwner {
        buyTax = taxBuy;
        sellTax = taxSell;
    }
    ...
    function _transfer(
        ...
        if((!automatedMarketMakerPairs[to] && !automatedMarketMakerPairs[from]) || _isExcludedFromFees[from] || _isExcludedFromFees[to]){
        ...
        if (to == uniswapV2Pair) {
            ...
            uint256 amountTax = amount.mul(sellTax).div(100);
            super._transfer(from, address(this), amountTax);
            amount = amount.sub(amountTax);
        } else if (from == uniswapV2Pair) {
            //buy
            uint256 amountTax = amount.mul(buyTax).div(100);
            super._transfer(from, _destroyAddress, amountTax);
            amount = amount.sub(amountTax);
        }

        super._transfer(from, to, amount);
        if (to == uniswapV2Pair && !isAddLdx) {
            uint256 canswap = balanceOf(address(this));
            swapTokensForEth(canswap);
        }
        if (from != address(this)) {
                if (
                    address(this).balance >= 10**12 &&
                    curPerFenhongVal == 0
                ) {
