...
// DegenGovernance.sol
// This contract is 'vToken', representative of on-chain vote weight.
contract VoteToken is ERC20, ERC20Burnable, Pausable, Whitelist {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal whenNotPaused override {
        ...
        _addressData[from].votesTransferred += amount;
        _addressData[to].votesReceived += amount;
...
contract VoteContract is Ownable, Pausable, ReentrancyGuard {
    using Address for address;
    using SafeMath for uint256;
    ...
    modifier ifLockedEquals(bool _set) {
        require(proposalSet == _set, "VoteContract: NOT_SET");
        _;
    }
    ...
    event onVoteOptionAdded(uint8 _optionId, uint256 _timestamp);
    event onVoteStarted(uint256 _timestamp);
    event onVoteFinished(uint256 _timestamp);
    ...
    function vote(uint8 _optionId, uint256 _votes) public whenNotPaused() nonReentrant() requiresVotes() returns (bool _success) {
        require(votesEnabled == true, "VoteContract: VOTING_DISABLED");
        ...
        require(voteToken.transferFrom(msg.sender, address(this), _votes));
    ...
    // Add vote option
    function addVoteOption() public onlyOwner() whenPaused() {
        require(votesEnabled == false, "VoteToken: VOTING_ENABLED");

        // Add another option to the count
        totalOptions++;

        // Tell the network
        emit onVoteOptionAdded(totalOptions, block.timestamp);
    }

    // Start the vote
    function startVote() public onlyOwner() whenPaused() ifLockedEquals(false) {

        // Enable voting for everyone and lock the proposal
        votesEnabled = true;
        proposalSet = true;

        // Tell the network
        emit onVoteStarted(block.timestamp);
    }

    // Conclude the vote
    function finishVote() public onlyOwner() whenPaused() ifLockedEquals(true) {
        ...
        // Tell the network
        emit onVoteFinished(block.timestamp);
    }

    // Recover used tokens for re-use
    function recoverTokens() public onlyOwner() whenPaused() {

        // Count how many tokens are in this contract
        uint256 tokenAmount = voteToken.balanceOf(address(this));

        // Transfer them to the caller
        voteToken.transfer(msg.sender, tokenAmount);

        // Tell the network
        emit onRecoverTokens(tokenAmount, block.timestamp);
    }

    // Reset options
    function resetOptions() public onlyOwner() whenPaused() {
        ...
        // Reset the total options count
        totalOptions = 0;

        // Tell the network
        emit onResetOptions(block.timestamp);
    }
}