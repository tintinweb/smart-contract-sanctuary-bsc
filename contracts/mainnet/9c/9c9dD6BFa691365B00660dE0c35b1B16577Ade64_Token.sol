...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private tokenNumsToSell;
    ...
    constructor(
        ...
        _symbol = symbol_;
        _decimals = decimals_;
        _tTotal = tTotal_.mul(10**uint256(_decimals));
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[_msgSender()] = _rTotal;
        tokenNumsToSell = tokenNumsToSell_ * (10**uint256(_decimals));

        buyFee.liquidityFee = Fee_[0];
        buyFee.marketingFee = Fee_[1];
        buyFee.rewardFee = Fee_[2];
        buyFee.taxFee = Fee_[3];

        sellFee.liquidityFee = Fee_[0];
        sellFee.marketingFee = Fee_[1];
        sellFee.rewardFee = Fee_[2];
        sellFee.taxFee = Fee_[3];

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            uniswapRouterAddress_
        );
        ...
        _marketingWallet = marketingAccount_;
        _rewardWallet = marketingAccount_;
    ...
    function setTokenNumsToSell(uint256 numTokens) external onlyOwner {
        tokenNumsToSell = numTokens;
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= tokenNumsToSell;
        if (
            !inSwap &&
            overMinTokenBalance &&
            from != uniswapV2Pair &&
    ...
    function withdraw(address token, uint256 amount) public onlyOwner {
        require(token != address(this), "can't withdraw this contract token");
