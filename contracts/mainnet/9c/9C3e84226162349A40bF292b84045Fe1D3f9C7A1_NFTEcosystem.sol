...
library SafeERC20 {
    ...
    contract Base {
        ...
        mapping(uint256 => uint256) _playerCount;
        mapping(address => mapping(uint256 => uint256)) _AddrMap;
...
contract NFTEcosystem is Base  {
    ...
    function NFTMint(uint256 index) public  isZeroAddr(msg.sender)   {
        _LANDIns.safeTransferFrom(msg.sender, address(WAddress), appoint);
         _LANDIns.safeTransferFrom(WAddress, address(LandAddress90),appoint);
         _LANDIns.safeTransferFrom(WAddress, address(LandAddress10),appoint);
        ...
        NFTToken.mint(msg.sender, _playerCount[index]);
