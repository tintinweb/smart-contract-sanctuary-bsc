...
contract RunEarner is ERC20Detailed, Ownable {
    ...
    uint256 public gmtWbnbFee = 80;
    uint256 public insuranceFundFee = 40;
    uint256 public autoBurnFee = 10;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(gmtWbnbFee).add(autoBurnFee).add(
            insuranceFundFee
        );
    ...
    constructor() ERC20Detailed("RunEarner", "RunEarn", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xd4F9e6874650EEBD9989B0C42dFCb35df2C26Fb4;
        treasuryReceiver = 0x7cF0E9C4EbA47059A942E4f07Efb1B94B8d8286d;   
        autoBurn = 0x000000000000000000000000000000000000dEaD;
        insuranceFundReceiver =  0x363db7D7c5Df33658Bf44EC786157737535DE175;
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(gmtWbnbFee).add(insuranceFundFee)
            ),
            gas: 30000
        }("");
        ...
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(gmtWbnbFee).div(treasuryFee.add(gmtWbnbFee).add(insuranceFundFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Ethereum token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
