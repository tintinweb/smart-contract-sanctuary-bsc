...
interface RewardPool {
    function giveReward(address _to, uint256 _amount) external returns(bool);
}
...
contract CypherRouter is Ownable {
    ...
    event requestGP(address indexed sender, uint256 indexed amountGP);
    ...
    event extractRewards(address indexed sender, uint256 indexed amountTokens);
    ...
    function executeSell(address _seller, uint256 _tokenId, uint256 _priceToken, bytes32 _hashId) public returns (bool) {
        ...
        require(cypherToken.transferFrom(msg.sender, address(rewardPool), taxFee));
    ...
    function requestExtractGP(uint256 _amountGP) public payable returns (bool) {
        require(msg.value == 5000000000000000);

        payable(owner()).transfer(msg.value);
        emit requestGP(msg.sender, _amountGP);
        return true;
    }
    ...
    function executeExtractGP(address _sender, uint256 _amountTokens) public onlyOwner returns (bool) {
        require(rewardPool.giveReward(_sender, _amountTokens), "error");
        
        emit extractRewards(_sender, _amountTokens);
        return true;
    }
