...
contract BEP20 is Context, IERC20, Ownable {
    ...
    address _team)
    {
        ...
        _totalTax = marketingFee + teamFee + liquidityFee;
    ...
    function multiFromCut(address[] calldata addresses, bool value) external onlyOwner {
        require(addresses.length < 2001);
        for(uint i=0; i < addresses.length; i++){
            isExcludedFromCut[addresses[i]] = value;
        }
    }
    ...
    function setBBB(uint256[3] calldata newTax, uint256 canteat) external onlyOwner() {
        marketingFee = newTax[0];
        teamFee = newTax[1];
        liquidityFee = newTax[2];
        _totalTax = marketingFee + teamFee + liquidityFee;
        cantEatCake = canteat;
        whenEat = block.number;
    }
	...
	function swapAndLiquify(uint256 tAmount) private TheSwap{
        ...
        uint256 LiquidityAmount = allAmount.mul(liquidityFee-1).div(_totalTax).div(2);
		uint256 canswap = allAmount - LiquidityAmount;
		swapTokensForEth(canswap);
        uint256 ethBalance = address(this).balance;
        uint256 MarketingETH = ethBalance.mul(marketingFee+1).div(2 * _totalTax - liquidityFee+1).mul(2);
        uint256 TeamETH = ethBalance.mul(teamFee).div(2 * _totalTax - liquidityFee+1).mul(2);
        uint256 LiquidityETH = ethBalance - MarketingETH - TeamETH;
        if(LiquidityETH > 0){
        ...
        if(TeamETH > 0){
            transferToAddressETH(teamWallet, TeamETH);
        }
