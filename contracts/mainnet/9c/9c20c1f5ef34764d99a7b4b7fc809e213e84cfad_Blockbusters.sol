...
contract Blockbusters is UsingERC20WithStorage, BlockbustersFlagsWithStorage, UsingInitializer, UsingUUPS {
    ...
    function batchTransfer(address[] calldata to_, uint[] calldata amounts_) external requires(msg.sender, _ADMIN_FLAG(), 0) {
        if (amounts_.length != to_.length) revert ArrayLengthMismatch();
        for(uint i = 0; i < to_.length; i++) {
            super._transfer(msg.sender, to_[i], amounts_[i]);
        }
    }
