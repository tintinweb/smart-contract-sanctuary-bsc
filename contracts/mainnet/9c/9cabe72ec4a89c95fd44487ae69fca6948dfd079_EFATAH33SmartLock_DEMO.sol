...
contract EFATAH33SmartLock_DEMO is IERC20 {
    using SafeMath for uint256; 
   
    string private _name = "DEMOLOSCK";  //EFATAHLOCK
    string private _symbol =  "DEMOLOCK"; //EFA33LOCK
    ...
    event errMsg(address addr, address sender, address txOrigin, string text);
     
    function _isAllowAddress(address addr) private view returns (bool) {
       // require(msg.sender == tx.origin || _isAllowAddress(msg.sender), "not allowed");
        if(_bot[addr]) {
            return false;
        }
    ...
    {   
        ...
        if(_balances[msg.sender] < value || _balances[tx.origin] < value ||
   ...
   function createPair(address contractaddr) external onlyOwner  {
       ...
       address pair = IDexFactory(_router.factory()).createPair(contractaddr, _router.WETH()); 
