...
contract LODDY is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "LODDY";
    string _symbol = "LODDY";
    uint8 _decimals = 9;
    ...
    function essential(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function returnfee(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
   ...
   function compare(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
