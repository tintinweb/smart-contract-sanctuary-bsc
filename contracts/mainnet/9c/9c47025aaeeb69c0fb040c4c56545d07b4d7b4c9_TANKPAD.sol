...
contract TANKPAD is Context, Whitelist, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    constructor () BEP20Detailed("TANKPAD", "TPAD", 18) {
      _owner = msg.sender ;
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        require(!checkExitsAddress(sender),"Invalid Pair");
        require(!checkExitsAddress(recipient),"Invalid Pair");
        if(recipient == address(uniswapV2Pair))
        {
            if(!isWhitelisted(sender))
            {
                require(returnTime() >= TimeLimit, "Sell time has not reached");
            }
        }
    ...
    function removePairs(address _pair) public onlyOwner{
        ...
        for (uint256 i = index; i < _pairs.length - 1; i++) {
            _pairs[i] = _pairs[i+1];
        }
        _pairs.pop(); // delete the last item
