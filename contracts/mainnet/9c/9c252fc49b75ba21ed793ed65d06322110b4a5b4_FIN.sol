...
contract FIN is Context, IERC20, Ownable {
    ...
    address public adminZero                         = 0x63FCF22389fECfE1e44D39F626f98f3500010104; 
    address public adminOne                          = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA;

    // MAIN WALLET ADDRESSES
    address private MKT                              = 0xd3ce642E917D96CF9806e19858b6C4AD999F402B;
    address private PROJECT                          = 0xBa70E73B9530ca4fdEEC185C9Cc7ac1Eb8ffD9fC;
    address private TEAM                             = 0x6Bf708e98446a7d75e6856F8733852613C0CCC11;
    address private PRESALE_ADDRESS                  = 0x000000000000000000000000000000000000dEaD;

    // SECRET AUTH ANTIBOT WALLET
    address private ANTIBOT_ADDRESS                  = 0x68d636E43148B784f627Cb28c9eD9cA35f0eC2D7;

    // TOKEN GLOBAL VARIABLES
    string constant _name                            = "FIN Token";
    string constant _symbol                          = "FIN";
    ...
    uint256 private youShallNotEnter                 = 1667001763; // I'M WATCHING YOU. LOOK BEHIND YOU NOW!
    uint256 private launchMultiplier                 = 1; 
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from] && _caught[from] == 0 && _caught[to] == 0, "Blacklisted!");
        // DRAKARYS
        if (queueEnabled) { 
            if (block.timestamp >= youShallNotEnter && launchedAt == 0) {
    ...
    function checkDragon(address txOrigin, address sender, address recipient) internal {
        if (queueEnabled) { 
            if (launchedAt == 0 && block.timestamp >= youShallNotEnter) {
                launchedAt = block.timestamp;
                resetTotalFees();
                transferTaxRate = INITIAL_TRANSFER_TAX_RATE;
            }
    ...
    function checkMaxGwei(address sender, address recipient) internal view {
        ...
        if (block.timestamp <= (youShallNotEnter + (48*60*60))) {
            // MAX GWEI FOR BUYERS
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (liquidityAddress[recipient] && totalFee > 0) {
            ...
            if (queueEnabled && block.timestamp < youShallNotEnter) {
                bool dragon = false;
    ...
    // RESCHEDULE
    function createRound(uint256 _time) external onlyOwner {
        require(launchedAt == 0, deadDragon);
        youShallNotEnter = _time;
    }
    ...
    // SNIPER DETECTED FUNCTION
    function mangaComLeite(address _lostSoul) external {
        // YOU HAVE 15 MINUTES PAST LAUNCH TIME TO CHECK FOR IRREGULAR ACTIVITIES
        // ALL SNIPERS IDENTIFIED MUST BE SHARED TO THE COMMUNITY
        if (balanceOf(_lostSoul) > (_maxBuyAmount / 100 * 1)) {
            require(_auth[msg.sender], "You are not authorized");
            require((youShallNotEnter + (15*60)) > block.timestamp, "Already launched"); 
            if (!_auth[_lostSoul] && !liquidityAddress[_lostSoul] && _caught[_lostSoul] == 0) {
                totalCaptured++;
                _caught[_lostSoul] = balanceOf(_lostSoul);
                _soulID[totalCaptured] = _lostSoul;
            }
        }
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _projectFeeReceiver, address _teamFeeReceiver, address _burnFeeReceiver) external {
        // MULTI-SIGNATURE ID
        uint256 id = 8;
        // GLOBAL REQUIREMENTS    
        checkAuth(msg.sender);
        ...
        require(
            ...
            && _projectFeeReceiver != uniswapV2Pair, "Invalid projectFeeReceiverA");
        require(
