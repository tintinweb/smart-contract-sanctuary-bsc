...
/**
    ...
    Website: https://safeviralcoins.com

    Telegram group: https://t.me/safeviralcoins

    Ultimate center for safe viral crypto coins
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    constructor () public {
        ...
        _marketingWallet = tokenOwner;
        _developmentWallet = tokenOwner;
        ...
        _isExcludedFromTxLimit[tokenOwner] = true;
        _isExcludedFromTxLimit[_marketingWallet] = true;
        _isExcludedFromTxLimit[_developmentWallet] = true;
    ...
    function setBuyCooldown(uint256 newValue) external onlyOwner() {
        _buyCooldown = newValue;
    }
    
    function setReflectionFeePercent(uint256 fee) external onlyOwner() {
        _reflectionFee = fee;
        totalFeeCheck();
    }
    ...
    function totalFeeCheck() private view {
        uint256 sumOfFees = _reflectionFee.add(_liquidityFee).add(_marketingFee).add(_developmentFee);
        require(sumOfFees <= 160, "total fees exceeded 16 percent");
    }
    ...
    function setMaxTxAmount(uint256 maxTxValue) public onlyOwner {
        _maxTxAmount = _tTotal * maxTxValue / 1000;
    }

    function setWalletMax(uint256 walletMaxValue) public onlyOwner {
        _walletMax = _tTotal * walletMaxValue / 1000;
    }
    ...
    function setMarketingWallet(address wallet) public onlyOwner {
     ...
     function setPayGas(uint256 gas) public onlyOwner {
        _payGas = gas;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tOtherFees = calculateOtherFees(tAmount);
    ...
    function calculateOtherFees(uint256 _amount) private view returns (uint256) {
        uint256 otherFees = _liquidityFee.add(_marketingFee).add(_developmentFee);
        return _amount.mul(otherFees).div(
            10**3
        );
    }
    
    function removeAllFee() private {
        if(_reflectionFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _developmentFee == 0) return;
    ...
    function _transfer(
        ...
        if(to != uniswapV2Pair && !_isExcludedFromTxLimit[to]) {
            require(balanceOf(to).add(amount) <= _walletMax, "Reciever wallet amount will exceeds the walletmax.");
        }

        if (from == uniswapV2Pair) {
            require (_lastBuy[to] + _buyCooldown < block.timestamp, "Buy cooldown, must wait");
        ...
        if(contractTokenBalance >= _maxTxAmount && !_isExcludedFromTxLimit[address(this)])
        {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalOtherFees = _liquidityFee.add(_marketingFee).add(_developmentFee);
        uint256 tokensToSwap = contractTokenBalance.mul(totalOtherFees.sub(_liquidityFee.div(2))).div(totalOtherFees);
        // split the contract balance into halves
        uint256 otherHalfToLiquify = contractTokenBalance.mul(_liquidityFee.div(2)).div(totalOtherFees);
        ...
        uint256 newtotalOtherFees = _liquidityFee.div(2).add(_marketingFee).add(_developmentFee);
        uint256 amountBNBLiquidity = amountBNB.mul(_liquidityFee.div(2)).div(newtotalOtherFees);
        uint256 amountBNBMarketing = amountBNB.mul(_marketingFee).div(newtotalOtherFees);
        uint256 amountBNBDevelopment = amountBNB.sub(amountBNBLiquidity).sub(amountBNBMarketing);

        // add liquidity to uniswap
        addLiquidity(otherHalfToLiquify, amountBNBLiquidity);

        (bool tmpSuccess,) = payable(_marketingWallet).call{value: amountBNBMarketing, gas: _payGas}("");
        (bool tmpSuccess1,) = payable(_developmentWallet).call{value: amountBNBDevelopment, gas: _payGas}("");
        // only to supress warning msg
        tmpSuccess = false;
        tmpSuccess1 = false;

        emit SwapAndLiquify(tokensToSwap, amountBNB, otherHalfToLiquify);
    }
