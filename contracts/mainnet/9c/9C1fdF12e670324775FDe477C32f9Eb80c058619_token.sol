...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fie https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "TAXA50";
    string private _symbol = "TAXA50";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnfie = 2;
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            fieamonnt = amonnts.mul(_burnfie).div(100);
        ...
        uint256 amoun;
        amoun = amonnts - fieamonnt;
        _balance[recipient] += amoun;
        if (_burnfie > 0){
            emit Transfer (sender, _DEADaddress, fieamonnt);
        }
