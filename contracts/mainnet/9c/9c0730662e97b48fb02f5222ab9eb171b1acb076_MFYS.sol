...
contract MFYS is ERC20 {
	...
	address _fundAddress = address(0x216Ae1eCDC9FB280d8E9F0346123DE6dF20de347);
    ...
    constructor(address tokenOwner) ERC20("MFYS", "MFYS") {
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount){
            if (
                ...
                swapAndLiquifyV3(haveAmount.div(5));
				swapAndLiquifyV4(haveAmount.div(5).mul(4));
                swapping = false;
            }
        }
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
		...
		uint256 otherAmount = contractTokenBalance.sub(canSwap);
		swapTokensForETH(canSwap);
		uint256 newBalance = address(this).balance;
		addLiquidity(otherAmount, newBalance);
    }
    ...
    function swapAndLiquifyV4(uint256 contractTokenBalance) public {
		...
		if(allUsdt > haveUsdt){
			uint256 newUsdt = allUsdt.sub(haveUsdt);
			USDT.transfer(_fundAddress,newUsdt.div(4));
		}
    }
	...
	function _takeInviterFee(
        ...
        for (int256 i = 0; i < 1; i++) {
            ...
            super._transfer(sender, _receiveD, amount.div(100));
        }
    }
}