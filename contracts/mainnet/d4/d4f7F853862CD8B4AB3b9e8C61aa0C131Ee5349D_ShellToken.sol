...
contract ShellToken is Context, IERC20, Ownable {
    ...
    uint public _vcFee = 10;
    uint public _lpFee = 50;
    uint public _mkFee = 20;
    uint public _nftFee = 20;
    uint public initFee = 50;
    ...
    uint public addPriceTokenAmount = 1e8;

    uint public sellLimitPerDay = 10e9;
    mapping(address => mapping(uint => uint)) public sellAmountPerDays;

    uint public balLimit = 10e9;
    mapping(address => bool) public balLimitWhiteList;
    ...
    constructor (
        ...
        balLimitWhiteList[uniswapV2Pair] = true;
        balLimitWhiteList[holder] = true;
        balLimitWhiteList[address(0)] = true;
        balLimitWhiteList[address(this)] = true;
    ...
    function setsellLimitPerDay(uint limit)external onlyOwner{
        sellLimitPerDay = limit;
    }

    function setbalLimit(uint limit)external onlyOwner{
        balLimit = limit;
    }

    function setbalLimitWhiteList(address owner,bool isWhite)external onlyOwner{
        balLimitWhiteList[owner] = isWhite;
    }
    ...
    function getBuyFeeRate(uint zero,uint price)internal view returns(uint){
        uint base = todayBasePrices[zero];

        if( price >= base ) return initFee;
    ...
    function setAddress(
        ...
        balLimitWhiteList[vcAddress] = true;
        balLimitWhiteList[mkAddress] = true;
        balLimitWhiteList[nftAddress] = true;
    }
    ...
    function _initParam(uint256 tAmount,Param memory param) private view  {
        uint tFee = 0;
        if( param.takeFee ){
            tFee = tAmount * param.fee / 1000;
            param.tLp = tFee * _lpFee / 100;
            param.tMk = tFee * _mkFee / 100;
            param.tVc = tFee * _vcFee / 100;
            param.tNft = tFee * _nftFee / 100;
        }
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if( param.tVc > 0 ){
            _take(param.tVc, from, vcAddress);
        }
        if( param.tNft > 0 ){
    ...
    function _sellableAmount(address owner,uint zero)internal view returns(uint v){
        ...
        if( selled < sellLimitPerDay) {
            v = sellLimitPerDay - selled;
        } 
    }
    ...
    function _transfer(
        ...
        if( !ammPairs[from] && from != address(this) ){
            uint allowAmount = balanceOf(from) * 99 / 100;
        ...
        if( takeFee ){
            ...
            if( !isTrans){
                ...
                if( ammPairs[from]){
                    fee = getBuyFeeRate(zero - 1 days, price);
                }

                if( ammPairs[to]){

                    fee = getSellFeeRate(zero - 1 days, price);
        ...
        if( !isDelLiquidity && !balLimitWhiteList[to] ){
            require(balanceOf(to) <= balLimit,"bal exceed limit");
        }
