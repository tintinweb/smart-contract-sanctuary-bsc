...
contract Squidmas is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SquidmasDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Squidmas", "SMAS") {
        marketingWallet = payable(0x09C6e352D836B9b85B101C8e762a0a88C04D32d5);
        devWallet = payable(0xDE0D2c2FF13Fa3dF05F5d7caA3A936B575081A30);
        ...
        dividendTracker = new SquidmasDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 2000; // 2%
        swapTokensAtAmount = totalTokenSupply / 2000; // 0.25%;
    ...
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 2000, "max wallet cannot be set to less than 1%");
        maxWallet = value;
    }
...
contract SquidmasDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Squidmas public SquidmasContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SquidmasContract = Squidmas(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Squidmas_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Squidmas_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Squidmas contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SquidmasContract);

            uint256 prevBalance = SquidmasContract.balanceOf(address(this));
            ...
            {
                uint256 received = SquidmasContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SquidmasContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
