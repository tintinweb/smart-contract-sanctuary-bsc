...
interface IStakingMoneyTreeNFT {
    function originalOwner(uint256 _tokenURI) external returns (address);
}

interface IMoneyTreeLINK {
    function requestRandomWords() external returns (uint256);
    function requestIdToRandomNumber(uint256) external returns (uint256);
}

contract MoneyTreeCoin is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable,  OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable, VRFConsumerBase {
    ...
    bool internal numberdomeActive;
    
    //VS Mode mappings
    mapping(uint => mapping(uint => mapping(address => uint256))) internal _vsModeSideEntries;
    mapping(uint => mapping(uint => uint256)) internal _vsModeSideTotals;
    mapping(uint => uint) internal _vsModeResult;
    
    //VS Mode variables
    uint internal vsModeCurrentID;
    bool internal isVSModeActive;
    bool internal vsModeNoMoreBets;
    string internal vsModeTeam1;
    string internal vsModeTeam2;
    ...
    mapping (uint256 => uint256) internal _poolTokenAmount;
    ...
    function initialize() public virtual initializer {
        ...
        devWallet = payable(0x6f02797A1176F6DB9b7965DD42ea61D398cCBAf9);
        ...
        _isExcludedPool[devWallet] = true;
    ...
    //turn tax on/off
    function toggleTaxActive(bool toggler) internal onlyOwner {
        taxActive = toggler;
    }
    
    //adjust tax rates
    function TaxRateAdjuster() internal onlyOwner {
    }
    
    //adjust minimum number of tokens needed for a dex swap to occur
    function minTokensForSwapAdjuster() internal onlyOwner {
    }
    
    //adjust anti whale measure max amount transferred
    function maxTokensForTransferAdjuster() internal onlyOwner {
    }
    ...
    function depositSplitAndLiquify() internal {
        ...
        uint256 newBNBLottery = newBNBGained.div(3);
        lotteryAmount = lotteryAmount + newBNBLottery;
        
        uint256 newBNBMarketing = newBNBLottery;
        mWallet.transfer(newBNBMarketing);

        uint256 newBNBDividends = newBNBGained - newBNBLottery - newBNBLottery;
        _bnbWalletSnapshotTotal[_currentSnapshotId.current()+1] += newBNBDividends;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(sender != owner()){ require(amount <= antiWhaleLimit); }
        ...
        //if it's a direct purchase from pancakeswap or withdrawal from whitebit, charge 5% in dividends split for token and nft holders
        if(taxActive && (sender == uniswapV2Pair || sender == 0x33Eac50b7fAf4B8842A621d0475335693F5D21fe)){
            uint256 divAmount = amount.div(40);
            receiveAmount = amount - divAmount - divAmount;

            _balances[nWallet] = _balances[nWallet] + divAmount;
            _nWalletSnapshotTotal[_currentSnapshotId.current()+1] += divAmount;

            _balances[dWallet] = _balances[dWallet] + divAmount;
            _dWalletSnapshotTotal[_currentSnapshotId.current()+1] += divAmount;
        
        //if it's a direct sell to pancakeswap charge 10% in tax in BNB (in depositSplitAndLiquify)
        } else if(taxActive && recipient == uniswapV2Pair){
            uint256 taxAmount = amount.div(10);
            receiveAmount = amount - taxAmount;

            _balances[tWallet] = _balances[tWallet] + taxAmount;
    
        //if it's a peer to peer transfer charge 10% in dividends split for token and nft holders
        } else if(taxActive && !_isExcludedTax[sender] && !_isExcludedTax[recipient]){
            uint256 divAmount = amount.div(20);
            receiveAmount = amount - divAmount - divAmount;
        ...
        //else standard bep-20 transfer
        } else {
            receiveAmount = amount;
        }

        //if sells require liquify to bnb then perform on sell
        if(recipient == uniswapV2Pair && _balances[tWallet]>=minTokensForSwap){
    ...
    //swap BNB for USDC
    function swapBNBForTokens(address _winner, uint256 _swapAmount) internal {
        ...
        // make the swap
        uniswapV2Router.swapExactETHForTokens{value: _swapAmount}(
            0, 
            path,
            _winner,
            block.timestamp
    ...
    function setNFTAddress(address _NFTAddress) internal onlyOwner {
    ...
    //buy NFT using BNB
    function buyNFTBNB() payable public {
    }
    
    //buy NFT using MONEY
    function buyNFTTokens() internal {
    ...
    function buyLootBoxBNB(uint numLootBoxes) payable public {
        ...
        if(numLootBoxes == 1){
            require(msg.value == 0.155 ether);
        } else if(numLootBoxes == 5){
            require(msg.value == 0.645 ether);
        } else if(numLootBoxes == 10){
            require(msg.value == 1.125 ether);
        } else if(numLootBoxes == 50){
            require(msg.value == 5.605 ether);
        } else {
            require(msg.value == 1000000 ether);
        }

        uint256 newBNBLottery = msg.value / 4;
        lotteryAmount = lotteryAmount + newBNBLottery;

        payable(owner()).transfer(msg.value - newBNBLottery - newBNBLottery); //add half bnb sale to the dev wallet

        _bnbWalletSnapshotTotal[_currentSnapshotId.current()+1] += newBNBLottery;

        uint256 requestID = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestRandomWords();
        addressToRequestID2[_msgSender()] = requestID;
    ...
    function buyLootBoxTokens(uint numLootBoxes) payable public {
        ...
        chainlinkWithdraw = chainlinkWithdraw + 1;
        require(_owedLootBoxes[_msgSender()] == 0);
        ...
        _transfer(_msgSender(), nWallet, dividendTokens/2);
        _nWalletSnapshotTotal[_currentSnapshotId.current()+1] += dividendTokens/2;

        _transfer(_msgSender(), dWallet, dividendTokens/2);
        _dWalletSnapshotTotal[_currentSnapshotId.current()+1] += dividendTokens/2;
    ...
    function mintOwedLootBoxes() public {
        uint256 result = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestIdToRandomNumber(addressToRequestID2[_msgSender()]);
    ...
    function mintOwedUpgrade(uint256[] calldata tokenIdList1, uint256[] calldata tokenIdList2) public {
        uint256 result = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestIdToRandomNumber(addressToRequestID2[_msgSender()]);

        require(result != 0);
        require(_owedUpgrade[_msgSender()] != 0);

        require(tokenIdList1.length == tokenIdList2.length);

        for (uint256 i = 0; i < tokenIdList1.length; i++) {
    ...
    function changeNFTContractOwner(address newOwner) internal onlyOwner{
    ...
    function numberdomeActiveToggle(bool toggler) internal onlyOwner {
    ...
    function playNumberdome(uint256 luckyPicks) public {
        ...
        _burn(_msgSender(), 10000 * 10 ** decimals());
    ...
    function drawNumberdome1() public onlyOwner {
        ...
        uint256 requestID = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestRandomWords();
        addressToRequestID2[address(2)] = requestID;        
    ...
    uint256 result = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestIdToRandomNumber(addressToRequestID2[address(2)]);

        require(!numberdomeActive && result != 0);
        uint256 numberdomeResult = result;
        ...
        if(numberdomeNumberOfWinners != 0){
            uint256 numberdomePrizePerPerson = 1250000 * 10 ** decimals();
            for (uint i=0; i<numberdomeNumberOfWinners; i++) {
                ...
                _mint(currentWinner, numberdomePrizePerPerson);
                _owedLootPrizeRandomNumber[currentWinner] = result;
    ...
    function setVSModeActive (string memory team1Selection, string memory team2Selection) internal onlyOwner {

    }

    //make a VS mode selection for active gameID
    function playVSMode(uint chosenTeam, uint256 tokensStaked) internal {
        
    }
    
    //set VS mode to no more bets an hour before the event starts
    function setVSModeNoMoreBets() internal onlyOwner {

    }
    
    //set vs mode as unactive and enter the events outcome
    function setVSModeUnactive(uint winningTeam) internal onlyOwner {

    }
    
    //claim prize owed from vs mode result
    function claimVSMode(uint vsModeID) internal {
    ...
    function gridlockActiveToggle(bool toggler) internal onlyOwner {
    ...
    function drawGridlock1() public onlyOwner {
        ...
        uint256 requestID = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestRandomWords();
        addressToRequestID2[address(3)] = requestID;
    ...
    function drawGridlock2() public onlyOwner {
        uint256 result = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestIdToRandomNumber(addressToRequestID2[address(3)]);
    
        require(!gridlockActive && result != 0);

        uint256 gridlockResult = result % 12 + 1;
    ...
    function getHoldersInPool(uint poolID) internal view returns (address[] memory){
    ...
    function pickLotteryWinningNumber() public onlyOwner {
        ...
        uint256 requestID = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestRandomWords();
        addressToRequestID2[address(4)] = requestID;
	  
        pause();
    }
    
    function payLotteryWinner(uint _position) public onlyOwner {

        uint256 result = IMoneyTreeLINK(0xbdc17A033e2765ab1340c4304beA3bc7d097E471).requestIdToRandomNumber(addressToRequestID2[address(4)]);
        ...
        winningLotteryNumber = uint256(keccak256(abi.encode(result, _position)))  % totalPooledTokens;
        ...
        for (uint i=0; i<winningPoolHolders.length; i++) {
            if(currentTotal + _balances[winningPoolHolders[i]] < winningLotteryNumber){
                ...
                _lotteryWinnerPosition[_position] = winningPoolHolders[i];
        ...
        uint256 lotteryPayout;
        
        if(_position == 1){
            lotteryPayout = (lotteryAmount*66)/100;
        } else if(_position == 2){
            lotteryPayout = (lotteryAmount*60)/100;
        } else if(_position == 3){
            lotteryPayout = lotteryAmount;
        }

        _lotteryIDWinnerAddress[lotteryID][_position] = _lotteryWinnerPosition[_position];
        _lotteryIDWinnerAmount[lotteryID][_position] = lotteryPayout;
        
        swapBNBForTokens(_lotteryWinnerPosition[_position], lotteryPayout);
        lotteryAmount = lotteryAmount - lotteryPayout;
        
        _owedLootPrizeRandomNumber[_lotteryWinnerPosition[_position]] = result;
        _owedLootPrizes[_lotteryWinnerPosition[_position]] = _owedLootPrizes[_lotteryWinnerPosition[_position]] + 1;
    ...
    function checkDividendsOwed(address addressCheck) public view returns(uint256, uint256){
        ...
        for (uint i= lastClaimedID; i<=currentID; i++) {
            ...
            if (balanceAt != 0 && _dWalletSnapshotTotal[i] != 0) {
                ...
                uint256 eligbleSupplyAt = totalSupplyAt(i) - balanceOfAt(uniswapV2Pair, i) - 
                balanceOfAt(0x6f02797A1176F6DB9b7965DD42ea61D398cCBAf9, i) - 
                balanceOfAt(0x81E0eF68e103Ee65002d3Cf766240eD1c070334d, i) - 
                balanceOfAt(0x33Eac50b7fAf4B8842A621d0475335693F5D21fe, i) - 
                balanceOfAt(0x8195b7bd26c672d02aF0b28413c7E7D198D597b0, i) -
                balanceOfAt(tWallet, i) - 
                balanceOfAt(nWallet, i) - 
                balanceOfAt(vsWallet, i) -
                balanceOfAt(dWallet, i); 
                
                dividendOwed = dividendOwed + _dWalletSnapshotTotal[i].mul(balanceAt).div(eligbleSupplyAt);

                if(_bnbWalletSnapshotTotal[i] != 0){
                    bnbDividendOwed = bnbDividendOwed + _bnbWalletSnapshotTotal[i].mul(balanceAt).div(eligbleSupplyAt);
                }
            }
        }
        return(dividendOwed, bnbDividendOwed);
    }
    ...
    function claimAllDividends() public {
        ...
        (uint256 dividendOwed, uint256 bnbOwed) = checkDividendsOwed(_msgSender());
        ...
        if(bnbOwed > 0){
            payable(_msgSender()).transfer(bnbOwed);
        }
    ...
    function withdrawLink() internal onlyOwner {
     ...
     */
    function fulfillRandomness() internal override {
    ...
    mapping(uint => address) public _lotteryWinnerPosition;
    
    
    mapping(uint => address) internal _simXP;


    function sim1() internal onlyOwner {
        chainlinkWithdraw = 0;
    }
    
    function sim2() internal onlyOwner {
        
    }
    
    address public NFTLotteryWinner;
    
    function nftLottery() public onlyOwner{
        
        uint256 winningNFT = requestIdToRandomNumber[lotteryLastRequestID] % IMoneyTreeNFT(nftAddress).totalSupply() + 1;
        uint256 winningTokenID = IMoneyTreeNFT(nftAddress).tokenByIndex(winningNFT);
        uint256 winningTokenRarity = IMoneyTreeNFT(nftAddress)._rarity(winningTokenID);
        NFTLotteryWinner = IMoneyTreeNFT(nftAddress).ownerOf(winningTokenID);

        if (NFTLotteryWinner == 0x8fa734E43b2a25904bB3B3FBe9f93534796fA632){
            NFTLotteryWinner = IStakingMoneyTreeNFT(0x8fa734E43b2a25904bB3B3FBe9f93534796fA632).originalOwner(winningTokenID);
        }

        uint256 NFTPrize;
        
        if(winningTokenRarity == 0){
            NFTPrize = 1;
        } else if (winningTokenRarity == 1){
            NFTPrize = 2;
        } else if (winningTokenRarity == 2){
            NFTPrize = 4;
        } else if (winningTokenRarity == 3){
            NFTPrize = 8;
        } else if (winningTokenRarity == 4){
            NFTPrize = 16;
        }

        _lotteryIDWinnerAddress[lotteryID][4] = NFTLotteryWinner;
        _lotteryIDWinnerAmount[lotteryID][4] = NFTPrize;

        lotteryID = lotteryID + 1;
        
        IMoneyTreeNFT(nftAddress).Lootbox(NFTLotteryWinner, 
                                          requestIdToRandomNumber[lotteryLastRequestID],
                                           NFTPrize);
    }
    
    function excludeAddress(address _excluded) internal onlyOwner{
        _isExcludedTax[_excluded] = true;
        _isExcludedPool[_excluded] = true;
    }

    mapping(uint => uint256) public _nWalletSnapshotTotal;
    mapping(uint => uint256) public _bnbWalletSnapshotTotal;

    function claimFromStakingContract(address _holder, uint256 amount) public {
        require(msg.sender == 0x8fa734E43b2a25904bB3B3FBe9f93534796fA632);
        _transfer(nWallet, _holder, amount);
    }

     function buyLLCredits(address player, uint256 creditsPurchased) public {
        require(msg.sender == 0x940e58Ae2bb63e524405C9a98575868cf7FC1Ecc);
        _burn(player, creditsPurchased * 3000 * 10 ** decimals());
    }

    function claimLLPrizes(address winner, uint256 tokenAmount, uint256 nftAmount, uint256 randomness) public {
        require(msg.sender == 0x940e58Ae2bb63e524405C9a98575868cf7FC1Ecc);
        
        _mint(winner, tokenAmount * 10 ** decimals());

        if(nftAmount > 0){

        uint256 dividendTokens = nftAmount * 20000 * 10 ** decimals();

        _mint(nWallet, dividendTokens/2);
        _nWalletSnapshotTotal[_currentSnapshotId.current()+1] += dividendTokens/2;

        _mint(dWallet, dividendTokens/2);
        _dWalletSnapshotTotal[_currentSnapshotId.current()+1] += dividendTokens/2;

        IMoneyTreeNFT(nftAddress).Lootbox(winner, randomness, nftAmount);
        
        }
    }  

    mapping(uint => mapping(uint => address)) public _lotteryIDWinnerAddress;
    mapping(uint => mapping(uint => uint256)) public _lotteryIDWinnerAmount;
    uint public lotteryID;

    mapping(address => uint256) public addressToRequestID2;

    uint256 public chainlinkWithdraw;
