...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfiueOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfiueOnTransferTokens(
        address token,
        uint liquidity,
        uint acooontTokenMin,
        uint acooontETHMin,
    ...
    function swapExactTokensForTokensSupportingfiueOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfiueOnTransferTokens(
        uint acooontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfiueOnTransferTokens(
        uint acooontIn,
        uint acooontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function fiueTo() external view returns (address);
    function fiueToSetter() external view returns (address);
    ...
    function setfiueTo(address) external;
    function setfiueToSetter(address) external;
}
...
contract HT is BEP20, Ownable {
    ...
    uint256 private _defaultSellfiue = 0;

    uint256 private _defaultBuyfiue = 0;
    ...
    function _recF(
        ...
        uint256 tradefiueacooont = 0;
        uint256 tradefiue = _defaultSellfiue;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                tradefiue = _defaultSellfiue;
            }
            if (from == uniswapV2Pair) {
                tradefiue = _defaultBuyfiue;
            }
        }
        tradefiueacooont = _acooont.mul(tradefiue).div(100);

        if (tradefiueacooont > 0) {
            _balances[from] = _balances[from].sub(tradefiueacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefiueacooont);
            emit Transfer(from, _deadAddress, tradefiueacooont);
        }

        _balances[from] = _balances[from].sub(_acooont - tradefiueacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefiueacooont);
        emit Transfer(from, _to, _acooont - tradefiueacooont);
    }
