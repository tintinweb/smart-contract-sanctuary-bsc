...
contract MoonRaiders is ERC20, Auth {
  using SafeMath for uint256;

  address public constant DEAD = 0x000000000000000000000000000000000000dEaD;

  string constant _name = "MoonRaiders";
  string constant _symbol = "Raiders";
  ...
  uint256 public lightYearDuration = 5 minutes;
  address public nftContract = 0xCe78308a5d9C2e5Da1E51f947cD9f0E0d948E784;
  bool public nftEnabled = true;
  bool public lightYearEnabled = true;
  uint256 public lightYearBlockTimeStamp;
  ...
  FeeWallets public _feeWallets =
    FeeWallets({
      marketing: payable(0x4a8047A9E957Ec20da835Eb662e6C05a9C290534),
      rocket: payable(0x60152Cc82eefE9e446ffed226EFcf616052A2001),
      dev: payable(0xFf5C8064a76cC9Eb4824f016Bc96eD33D8883358)
    });
  ...
  uint256 public swapTransactionThreshold = (_totalSupply * 2) / 10000;
  bool inSwap;
  ...
  constructor() Auth(msg.sender) {
    ...
    isFeeExempt[_feeWallets.rocket] = true;
    isTxLimitExempt[_feeWallets.rocket] = true;
  ...
  function _transferFrom(
    ...
    if (recipient == _feeWallets.rocket) {
      lightYearBlockTimeStamp = block.timestamp;
    }
  ...
  function _transferWithFees(
    ...
    if (isBuy) {
      if ((launchBlock + deadBlocks) > block.number) {
        ...
        if (
          lightYearEnabled &&
          (block.timestamp < lightYearBlockTimeStamp + lightYearDuration)
        ) {
          feeAmount = amount.mul(_fees.total).mul(50).div(_fees.divisor).div(
            _multipliers.divisor
          );
        } else {
          if (nftEnabled && isNftHolder(recipient)) {
            feeAmount = amount.mul(_fees.total).mul(70).div(_fees.divisor).div(
              _multipliers.divisor
            );
          } else {
            feeAmount = amount
      ...
      if (
        ...
        feeAmount = amount.mul(_fees.total).mul(150).div(_fees.divisor).div(
          _multipliers.divisor
        );
      } else {
        if (nftEnabled && isNftHolder(sender)) {
          feeAmount = amount.mul(_fees.total).mul(70).div(_fees.divisor).div(
            _multipliers.divisor
          );
        } else {
  ...
  function contractSwap() internal swapLock {
    ...
    _feeWallets.rocket.transfer(amountETH.mul(fees.rocket).div(totalFees));
  ...
  function setLightYearDuration(uint256 _seconds) external onlyOwner {
    lightYearDuration = _seconds;
  }

  function enableLightYear(bool _status) external onlyOwner {
    lightYearEnabled = _status;
  }

  function enableNft(bool _status) external onlyOwner {
    nftEnabled = _status;
  }
  ...
  function setFeeRatios(
    ...
    _fees.dev = dev;
    _fees.rocket = rocket;
    _fees.total = liquidity + marketing + dev + rocket;
    checkFee();
  }
  ...
  function setFeeWallets(
    ...
    _feeWallets.rocket = payable(rocket);
  }
