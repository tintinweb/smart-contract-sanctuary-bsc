...
contract TokensByGEN is Context {
    ...
    uint256 private PayNowFEE = 1*10**18; // Set Pay Now Fee to 1 BNB
...
contract Contract is Context, IERC20 { 
    ...
    event updated_Buy_fees(uint256 Marketing, uint256 Liquidity, uint256 Tokens, uint256 Contract_Development_Fee);
    event updated_Sell_fees(uint256 Marketing, uint256 Liquidity, uint256 Tokens, uint256 Contract_Development_Fee);
    ...
    modifier LockFunction {
        require(!Functions_Locked, "Function is Locked");
        _;
    }
    ...
    */
    ...
    They will not arrive in the burn wallet, instead they will be removed
    ...
    */
    ...
    function Deploy_01__Deflationary_Burn(bool true_or_false) external LockFunction onlyOwner {
    ...
    -------------------------------------------
    SET UP PRE-SALE CONTRACT ADDRESS - OPTIONAL
    -------------------------------------------
    ...
    */ 
    ...
    function Deploy_02__PreSale_Wallet (address PreSale_Wallet_Address) external LockFunction onlyOwner {
    ...
    This counts the number of transactions with a fee. When it reaches the trigger number the
    contract will process the accumulated fees on the next sell. 

    (Remember, it is not possible to process the fees during a buy, only a sell. So fee
    processing needs to wait until the next sell.)

    Why would you need to change the Processing Trigger Count

    The default is 10, so the contract will process the accumulate fees on the next sell
    after 10 transactions. If your fee is 10%, the contract sell will be the average of the
    last 10 transaction, so it'll always be in keeping with the transaction amounts on your chart. 

    If your fees are more than 10% then the accumulated fees after 10 transaction will be
    greater than the average transaction. In this case, reducing the Trigger Count will cause
    the contract to process fees more frequently, reducing the size of the contract sell. 

    Similarly, if your fees are less than 10% then you can process the accumulated fees less 
    frequently (increase the Trigger Count) This will cause larger contract sells, but they'll
    be less frequent.

    If you have a flurry of buys and nobody is selling, then the contract will not process the fees. 
    Fee processing can only be triggered by a sell. If you want to trigger it yourself, you can
    sell a token or use the 'Processing_Process_Now' function.

    */


    // Set Buy Fees
    function Deploy_03__Fees_on_Buy(uint256 BNB_on_BUY, 
        ...
        require (BNB_on_BUY          + 
                 ...
                 Burn_on_BUY         + 
                 Tokens_on_BUY       + 
                 _Fee__Buy_Contract <= 20, "Buy Fee Limit is 20%"); 
        ...
        emit updated_Buy_fees(_Fee__Buy_BNB, _Fee__Buy_Liquidity, _Fee__Buy_Tokens, _Fee__Buy_Contract);
    }

    // Set Sell Fees
    function Deploy_04__Fees_on_Sell(uint256 BNB_on_SELL,
        ...
        require (BNB_on_SELL        + 
                 ...
                 Burn_on_SELL       + 
                 Tokens_on_SELL     + 
                 _Fee__Sell_Contract <= 20, "Sell Fee Limit is 20%"); 
        ...
        emit updated_Sell_fees(_Fee__Sell_BNB, _Fee__Sell_Liquidity, _Fee__Sell_Tokens, _Fee__Sell_Contract);
    }
    ...
    */
    ...
    function Deploy_05__Wallet_Limits(
        ...
        require(Max_Tokens_Per_Transaction >= _tTotal / 1000 / 10**_decimals, "Transaciton limit must be greater than 0.1% of supply");
        require(Max_Total_Tokens_Per_Wallet >= _tTotal / 1000 / 10**_decimals, "Wallet limit must be greater than 0.1% of supply");
    ...
    */
    ...
    function Deploy_06__Set_Wallets(address Token_Fee_Wallet, address payable BNB_Fee_Wallet, address Liquidity_Collection_Wallet) external LockFunction onlyOwner {

        // Update Token Fee Wallet
        require(Token_Fee_Wallet != address(0), "Token wallet Can not be 0 address");
        ...
        require(BNB_Fee_Wallet != address(0), "BNB wallet can not be 0 address");
    ...
    The information that you add here will appear on BSCScan, helping potential investors
    to find out more about your project. Be sure to enter the complete URL as many websites
    will automatically detect this, and add links to your token listing.

    If you are updating one link, you will also need to re-enter the other two links.

    */

    function Deploy_07__Update_Socials(string memory Website_URL, string memory Telegram_URL, string memory Liquidity_Locker_URL) external onlyOwner{
    ...
    You can also check the launch of other recent contracts and block any wallets
    that purchased several times in the first second of trade.
    
    This function does not have LockFunction modifier as the 'add blacklist' can only be used before launch
    ...
    */
    ...
    function Deploy_08__Blacklist_Bots(address Wallet, bool true_or_false) external onlyOwner {
        
        // Buyer Protection - Blacklisting can only be done before launch
        if (true_or_false){require(LaunchBlock == 0, "Blacklisting only permitted before launch");}
    ...
    and enter your contract address into the 'Select a currency' field.
    ...
    do and setting the price of your token.
    ...
    */
    ...
    -----------------------------------------
    WHITELIST PRIVATE SALE WALLETS - OPTIONAL
    -----------------------------------------

    Before whitelisting private sale wallets, set your transaction and holding limits, your
    buy and sell fees, and add liquidity.

    You can whitelist several wallets at a time, comma separate wallets (with no spaces)

    IMPORTANT - All functions that use a loop can fail due to running out of gas. If the transaction
    fails, break the list of wallet addresses into smaller chunks and do them a few at a time.

    To whitelist wallets, add 1 or more wallet address and set 'true_or_false' as true.
    To remove whitelist status from a wallet, enter 1 or more wallet address and set 'true_or_false' as false.

    */

    function Deploy_09__Private_Sale_WhiteList(address[] calldata Whitelist_Wallets, bool true_or_false) external LockFunction onlyOwner {
    ...
    */
    ...
    function Deploy_10__OpenTrade() external LockFunction onlyOwner {
    ...
    */
    ...
    function Remove_Contract_Fee() external onlyOwner payable {
        ...
        require(msg.value == 2*10**18, "Requires 2 BNB payment"); 
    ...
    In order to have wallet to wallet transfers without a fee, we need to tell the contract when
    to apply a fee. We do this for any address that represents a liquidity pairs, so that the contract
    knows to apply a fee for transactions to or from these addresses. 
    
    Anybody can create a new liquidity pair on any token.

    So if the owner renounces the contract, and then somebody creates a new liquidity pair, buys and
    sells via that pair would not charge a fee, and without access to the functions, the owner would
    not be able to tell the contract that the address is a pair.

    For this reason, the owner can never fully renounce this contract. But they can permanently
    lock other functions. 

    The contract includes a modifier called "LockFunction" any function with this modifier requires
    that the bool "Functions_Locked" is false. This bool can be set to true using a one-way switch.
    Once set to true it can not be set back to false. 

    For all functions that include the LockFunction modifier, this works in exactly the same way
    as renouncing the contract.

    --------------------------------
    FUNCTIONS THAT CAN NOT BE LOCKED
    --------------------------------

    Some functions can not be locked. 
        ...
        5. The function that allows the owner to remove blacklist status from a wallet.
    ...
    */
    ...
    function Processing_Process_Now (uint256 Percent_of_Tokens_to_Process) external LockFunction onlyOwner {
        require(!inSwapAndLiquify, "Currently in swap!"); 
    ...
    */
    ...
    function _transfer(
        ...
        if (!TradeOpen){
        require(_isWhiteListed[from] || _isWhiteListed[to], "Trade is closed");
        }
        ...
        The launch phase lasts for 40 blocks. This is approximately 2 minutes on BSC. 
        During this time the max transaction is halved, and wallet are restricted from buying 
        again within 10 seconds. 

        This helps to keep the launch fair, and prevents one or two people getting max wallets for almost nothing.

        If you have very low liquidity, these restrictions can cause the contract to flag as a honeypot,
        and possibly disable the trade button on PooCoin, trade will always be possible on other exchanges, such as PancakeSwap. 

        The honeypot tool does a 0.1 BNB buy/sell simulation. Most contracts with low initial liquidity fail this test.
        However, as people buy, and the price increases, the token soon passes the test and no longer flags as a honey pot. 

        For PooCoin, the test is 0.01 BNB, so most tokens should pass this without issue. However, if you have very low liquidity
        at launch, you may find that your trade button is disabled.

        This is only temporary, as people buy and the price goes up, it will soon pass their test and the trade button will work like normal. 

        Thankfully, if trade is possible at all, then PancakeSwap will always permit trade. They don't do simulations or have limits.

        The contract also detects and tags all buys in the first block as snipers. Without inflating gas or using bots, 
        it's not possible for a person to genuinely buy that fast. This will stop most bots, but it will not protect against more sophisticated or custom bots.

        You also have an option to blacklist wallets, so they can not buy at all. But this ability is only available before launch.
        The moment that you open trade, you lose the ability to blacklist wallets (this is to protect buyers and improve your audit score)

        After approximately 2 minutes, the next buy will automatically end the launch phase and from this point on the code will be skipped.
        ...
        */
        ...
        if (LaunchPhase && to != address(this) && _isPair[from])
            ...
            require(amount <= max_Tran / 2, "Launch phase limit exceeded");

            // Stop repeat buys within 10 seconds
            require (block.timestamp >= _Last_Buy[to] + 10, "Please wait - Launch phase 10 second buy delay");

            // Anybody that bought during the launch block (first 3 seconds) is blocked until block 40 (first 2 minutes)
            require(!_isSnipe[to]);

            // Detect and restrict snipers - We must allow them to buy a small amount in order to tag them so we can block them during the launch phase
            if (block.number == LaunchBlock) {
                require(amount <= _tTotal / 10000, "1st block Limit exceeded");
                _isSnipe[to] = true;
                }
            ...
            // End Launch Phase after 40 blocks (2 minutes)
            if (block.number > LaunchBlock + 40){LaunchPhase = false;}
        ...
        require(!_isBlacklisted[to],"Blacklisted to");
        require(!_isBlacklisted[from],"Blacklisted from");
    ...
    */
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        // Add tokens to recipient supply
        ...
        // Take tokens
        if(tTokens != 0){_tOwned[Wallet_Tokens] += tTokens;}

        // Take fees that require processing during swap and liquify
        if(tSwapFeeTotal != 0){_tOwned[address(this)] += tSwapFeeTotal;}

        // Remove Deflationary Burn from Total Supply
        if(tBurn != 0){_tTotal = _tTotal - tBurn;}
