...
contract YentityFeeApprover is OwnableUpgradeSafe {
    using SafeMath for uint256;

    function initialize(
        address _YENTITYAddress,
        address _WETHAddress,
        address _yentityVaultAddress
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        yentityTokenAddress = _YENTITYAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IUniswapV2Factory(address(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73)).getPair(WETHAddress,yentityTokenAddress);
        ...
        )
    {
        ...
        // Dont have a fee when yentityVault is sending, or infinite loop
        // And when pair is sending ( buys are happening, do not tax on it)
        if(sender == yentityVaultAddress || sender == tokenUniswapPair ) {
