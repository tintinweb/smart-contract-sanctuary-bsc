...
contract AyaTEST is Context, IERC20, Ownable {
    ...
    string private _name = "AyaTEST"; 
    string private _symbol = "AyaT";
    ...
    uint16 private _opsFee;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    address public _marketingWallet = payable(address(0xb5582409e2ac06c56aBF4C1530Ed1f85Eb7014f1)); 
    address public _opsWallet = payable(address(0xb5582409e2ac06c56aBF4C1530Ed1f85Eb7014f1)); 
    ...
    uint8 public constant BLOCKCOUNT = 10;
    ...
    constructor() {
        ...
        buyFee.liquidityFee = 2; //liquidity
        buyFee.marketingFee = 1; //marketing
        buyFee.opsFee = 1; //OPS
        buyFee.taxFee = 1; //reflections

        sellFee.liquidityFee = 2; //liquidity
        sellFee.marketingFee = 1; //marketing
        sellFee.opsFee = 1; //OPS
        sellFee.taxFee = 1; //reflections 
        ...
        )
    {
        ...
        uint256 tOps = calculateOpsFee(tAmount);
        uint256 tMarketing = calculateMarketingFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity);
        tTransferAmount = tTransferAmount.sub(tOps).sub(tMarketing);
        return (tTransferAmount, tFee, tLiquidity, tOps, tMarketing);
    }
    ...
    function _takeRewardAndMarketing(uint256 tOps, uint256 tMarketing)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rOps = tOps.mul(currentRate);
    ...
    function calculateOpsFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_opsFee).div(10**2);
    }
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, OPS, liquidity fee
    ...
    function swapAndConvert(
        ...
        swapTokensForBNB(forOps + forMarketing + half);
        ...
        uint256 opsAmount = newBalance.mul(forOps).div(
            forOps + forMarketing + half
        );
        IERC20(BUSD).transfer(_opsWallet, opsAmount);
        ...
        addLiquidity(half, newBalance - opsAmount - marketingAmount);
    }
