...
contract KooraInu is Context, IERC20, Ownable {
    ...
    string private constant _name = "KOORA INU"; // 
    string private constant _symbol = "KINU"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _bigestbuyFee = 4;
    uint256 public _marketingFee = 4;
    uint256 public _liquidityFee = 2;
    address public  _bigestbuyWallet = 0xce355c3316CE806020dBff7b8DbCCdeBa1fFa4bB;
    address public  _marketingWallet = 0x2737c3914f0316E17e2B577E9F5e75014Aeb32fB;
...
IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
   ...
   _isExcludedFromFee[_bigestbuyWallet] = true;
...
function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
       uint256 bigestbuyFee = amount * _bigestbuyFee / 100;
        uint256 marketingFee = amount * _marketingFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += bigestbuyFee + marketingFee + liquidityFee;
        emit Transfer (from, address(this), bigestbuyFee + marketingFee + liquidityFee);
        return (amount - bigestbuyFee - marketingFee - liquidityFee);
    }
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
 payable(_bigestbuyWallet).transfer(ethToSend);         
payable(_marketingWallet).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_bigestbuyFee + _liquidityFee) / (_marketingFee + _liquidityFee);
...
payable(_bigestbuyWallet).sendValue (ethAmount - ethFromLiquidity);            
