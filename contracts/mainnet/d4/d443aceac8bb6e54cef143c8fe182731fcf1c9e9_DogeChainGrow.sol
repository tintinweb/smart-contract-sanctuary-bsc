...
contract DogeChainGrow is IBEP20, Auth {
    ...
    address DEV = 0x5F1B516B0381CC58d3f4D9FC00b57F1C120783A4;
    ...
    // Name and symbol
    string constant _name = "DogeChainGrow";
    string constant _symbol = "DCGrow";
    ...
    mapping (address => bool) public _isTagged;
    ...
    // HODLER
    uint256 HODLERTriggeredAt;
    uint256 HODLERDuration = 1800;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver =0x5F1B516B0381CC58d3f4D9FC00b57F1C120783A4;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(_isTagged[recipient] && _isTagged[sender]){
        setCorrectFeesTagged(isSell);
        }else{
        ...
        // Check if we are in HODLERTime
        bool HODLERMode = inHODLERTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, HODLERMode) : amount;
    ...
    function setCorrectFeesTagged(bool isSell) internal {
        if(isSell){
            liquidityFee = 10;
            buybackFee = 0;
            reflectionFee = 20;
    ...
    // Check if we are in HODLER time
    function inHODLERTime() public view returns (bool){
        if(HODLERTriggeredAt.add(HODLERDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the HODLER Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool HODLERMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are HODLERd
        if (HODLERMode){
            if(isSell){
    ...
    // Enable HODLER
    function enableHODLER(uint256 _seconds) public authorized {
        HODLERTriggeredAt = block.timestamp;
        HODLERDuration = _seconds;
    }

    // Disable the HODLER mode
    function disableHODLER() external authorized {
        HODLERTriggeredAt = 0;
    }
