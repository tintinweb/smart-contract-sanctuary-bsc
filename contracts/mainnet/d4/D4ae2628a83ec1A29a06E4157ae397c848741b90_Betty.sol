...
// File: contracts/ERC721PsiUpgradeable.sol
...
contract Betty is ERC721PsiUpgradeable,UUPSUpgradeable,OwnableUpgradeable,vrfContract {
    ...
    function calAttribute(uint256 tokenId) public view returns (uint256) {
        require(tokenId<32000 && tokenId >= 0, "TokenId out of bounds.");
    ...
    function tokensOfOwner(address owner) public view returns (uint256[] memory) {
        ...
        uint256[] memory tokens = new uint[](count);
        uint index;
        for(uint i; i < _minted; i++){
            if(_exists(i) && owner == ownerOf(i)){
                tokens[index]=i;
    ...
    function _revealNFT(uint256 requestId,uint256 index,address requestAddress) internal{
        uint256[] memory Sales=tokensOfOwner(saleWallet);
        require(Sales.length>0, "NFTs have been revealed out. ");
        require(index < Sales.length, "ERC721: index out of bounds");
        _safeTransfer(saleWallet,requestAddress,Sales[index],"");
        orders[requestAddress]-=1;
        revealCount++;
        emit OpenMysteryBox(requestId,Sales[index],requestAddress);
    }
    ...
    function NFTsOfOwner(address owner) public view returns (NFT[] memory) {
        ...
        for(uint i; i < tokens.length; i++){
            uint256 team=calTeam(tokens[i]);
            uint256 attribute=calAttribute(tokens[i]);
            NFT memory newNFT = NFT(tokens[i],team,attribute);
            NFTs[i]=newNFT;
        }
        return NFTs;
    }
    ...
    function getNFTById(uint256 tokenId) public view returns (NFT memory) {
        ...
        NFT memory newNFT = NFT(tokenId,team,attribute);
        return newNFT;
    }
