...
contract AggregatorExecutor is Ownable {
    ...
    IMultiOracle public multiOracle;
    IWBNB public WBNB;
    address public BNB = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    //=========== external functions ============//
    function swap(SwapStep[] calldata _swaps, address _receiver, bool _isFromETH) external payable {
        if(_isFromETH) {
            require( _swaps[0].amountIn == msg.value,"AggregatorExecutor: !msgValue");
            WBNB.deposit{value: _swaps[0].amountIn}();
        } else {
            IERC20(_swaps[0].fromToken).safeTransferFrom(msg.sender, address(this), _swaps[0].amountIn);
        }
        uint _length = _swaps.length;
        for(uint i = 0; i < _length; i++) {
            _swap(_swaps[i], i == _length - 1, _receiver);
        }
    }
    ...
    function setMultiOracle(address _new) external onlyOwner {
        require(_new != address(0), "AggregatorExecutor: !zero");
        address _old = address(multiOracle);
        multiOracle = IMultiOracle(_new);
        emit UpdateMultiOracle(_old, _new);
    }

    function setWBNB(address _new) external onlyOwner {
        require(_new != address(0), "AggregatorExecutor: !zero");
        address _old = address(WBNB);
        WBNB = IWBNB(_new);
        emit UpdateWBNB(_old, _new);
    }
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "AggregatorExecutor: Cannot convert");
        ...
        //check price impact
        require(amountOut > _step.amountOutMin, "AggregatorExecutor: price impact so high");

        // check receiver
        receiver = finish ? receiver : address(this);
        ...
        //event trade mining
        uint _price = 0;
        if(address(multiOracle) != address(0)) {
            if(multiOracle.tradingPair(address(pair))) {
                _price = multiOracle.consult(_step.toToken);
            }
        }
        emit TradeMining(tx.origin, address(pair), _step.fromToken, _step.toToken, amountOut, _price);
    ...
    event TradeMining(address _user, address _pool, address _tokenIn, address _tokenOut, uint _amount, uint _price);
}
