...
// SPDX-License-Identifier: Unlicensed
    ...
    contract CYFpublicsales is ReentrancyGuard, PriceConsumerV3{
        ...
        BSCYieldFarm internal BSCYFv1 = BSCYieldFarm(0xFd6240Ba2174b56E4d7d66231Df1B7CA07417434);
        BSCYieldFarm internal BSCYFv2 = BSCYieldFarm(0x1b74FAcC863672294A2A031e20995c9E7d6082cD);
        BSCYieldFarm internal BSCYFv3 = BSCYieldFarm(0x23f647304B48ec44477cF8f2C60e3Eab46D6321A);
        BSCYieldFarm internal BSCYFv4 = BSCYieldFarm(0xdA95522baC148AFACD69D694FC5132eE97AA3e62);

        uint256 public constant PUBLIC_SALE_ENDS = 1667692799; // Saturday, November 5, 2022 11:59:59 PM UTC
        ...
        uint256 public _contestBNB; // will convert to USD at the moment of the draw.

        uint256 internal _devFeeBNB; 
        uint256 internal _devFeeUSD; 
        uint256 internal _yieldfarmSupport_;
        ...
        function drawWinner() public {
            ...
            // Convert the BNB to USD 
            swapEthForTokens(_contestBNB);
            _contestBNB = 0;
            ...
            if(block.timestamp >= PUBLIC_SALE_ENDS){
                // transfer remaining to liquiditywallet
                if(_usdToken.balanceOf(address(this)) > 0){
                    _usdToken.transfer(liquidity_, _usdToken.balanceOf(address(this)));
                }
            }
        }
        ...
        function bonusePerPurchase(uint256 _amount) private pure returns(uint256 _multiplier){
        ...
        function bonusPerVolume(address _player) private view returns(uint256 _multiplier, uint8 i){
        ...
        function dispatach(uint256 _amount, bool _bnb) private{
            ...
            if(_bnb){
                // 1.8% for Constest
                _contestBNB += _amount * CONTESTRESERVE / PERCENT_DIVIDER;
                // pay devfee 2.2%
                _devFeeBNB += _amount * DEV_FEE / PERCENT_DIVIDER;
                ...
                // pay devfee 2.2%
                _devFeeUSD += _amount * DEV_FEE / PERCENT_DIVIDER;
                _yieldfarmSupport_ += _yieldfarmsupport;
        ...
        function buyCYFBNB(address _refBy) public payable{
            require(block.timestamp <= PUBLIC_SALE_ENDS, 'PublicSaleEnded');
        ...
        function transferUSDtoFarm() public{
            require(_yieldfarmSupport_ > 0, 'NotWallowed');
            swapAndLiquify(_yieldfarmSupport_);
            _yieldfarmSupport_ = 0;
        }

        function transferDevFees() public{
            require(_devFeeUSD > 0 || _devFeeBNB > 0, 'notAllowed');
            if(_devFeeBNB > 0){
                payable(dev_).transfer(_devFeeBNB);
            }
            if(_devFeeUSD > 0){
                _usdToken.transfer(dev_, _devFeeUSD);
            }
            _devFeeUSD = 0;
            _devFeeBNB = 0;
        }
