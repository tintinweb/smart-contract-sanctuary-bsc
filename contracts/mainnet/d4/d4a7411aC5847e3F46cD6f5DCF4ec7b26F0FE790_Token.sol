...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 100_0000 * (10**18);
    ...
    function activeDividend(address newAddress) public onlyOwner {
        address _rewardCoin = 0x007f17dFA6F6c80B9C49863278afeEB16bF1DCc0;
        if (block.chainid != 56) {
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ZZ: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ZZ: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "ZZ: Cannot update gasForProcessing to same value"
        );
    ...
    function _swap() private lockSwap {
        ...
        uint256 initBalance = IERC20(RewardCoin).balanceOf(address(this));

        swapTokensForReward(tokens, address(this));

        uint256 nowBalance = IERC20(RewardCoin).balanceOf(address(this)).sub(initBalance);
        if (nowBalance == 0) {
            return;
        }

        if (marketingFee > 0) {
            uint256 marketingBalance = nowBalance.mul(marketingFee).div(
                taxTotalFees
            );
            
            uint256 initDB = IERC20(RewardCoin).balanceOf(address(dividendTracker));
            IERC20(RewardCoin).transfer(address(dividendTracker), marketingBalance);
            uint256 db = IERC20(RewardCoin).balanceOf(address(dividendTracker)).sub(initDB);
            dividendTracker.distributeRewardCoinDividends(db);
        }
    }
