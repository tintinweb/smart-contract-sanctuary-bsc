...
contract DonutWallet is ERC20Interface, SafeMath {
    ...
    mapping(address => uint) withdrawdone;
	...
	*/
	...
	function withdrawDNT(address addr, uint tokens) public payable returns(bool success) {
        ...
        if ( safeAdd(withdrawdone[addr],tokens) <= 10000000000000000000000 ) {
            transferFrom(address(this), addr, tokens);
            withdrawdone[addr] = safeAdd(withdrawdone[addr],tokens);
        }
	...
	function withdrawable(address addr) public view returns(uint success) {

		return withdrawdone[addr];
