...
contract Benfica is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "Benfica";
    string _symbol = "BEFCA";
    uint8 _decimals = 9;
    ...
    function Flamengo(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function provideBonus(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function inputValue(uint8 newLiqFee, uint8 newRewardFee, uint8 newMarketingFee, uint8 newExtraSellFee) external onlyOwner {
    ...
    function newAddress(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
