...
This marks the beginning of our ecosystem, join us on the wild ride to build it brick by brick with us.

Telegram - https://t.me/theassemblyhall

DO NOT BUY untill we tell you to and you have read our Whitepaper and Lightpaper .

Smart contract developed by  Crypto Titan - telegram @thecryptotitan1
*/
...
contract FreeTheSloths is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "Free The Sloths";
    string private constant _symbol = "FLOTHS";
    uint8 private constant _decimals = 9;
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    uint256 private constant _tTotal = 100000000000 * 10**9; // 100 Billion
    uint256 private _firstBlock;
    uint256 private _botBlocks;
    uint256 public _maxWalletAmount;
    uint256 private _maxSell;
    uint256 private _maxBuy;
    uint256 public numTokensToSwap = 200000000 * 10**9; // 0.2%

    // buy fees
    uint256 public _buyLiquidityFee = 0; //50; // divided by 1000
    uint256 private _previousBuyLiquidityFee = _buyLiquidityFee;
    uint256 public _buyEcosystemFee = 0; //15; // divided by 1000
    uint256 private _previousBuyEcosystemFee = _buyEcosystemFee;
    uint256 public _buyMarketingFee = 0; //25; // divided by 1000
    uint256 private _previousBuyMarketingFee = _buyMarketingFee;
    uint256 public _buyTeamFee = 10; // divided by 1000
    uint256 private _previousBuyTeamFee = _buyTeamFee;

    // sell fees
    uint256 public _sellLiquidityFee = 0; //50; // divided by 1000
    ...
    uint256 public _sellMarketingFee = 0; //25; // divided by 1000
    uint256 private _previousSellMarketingFee = _sellMarketingFee;
    uint256 public _sellTeamFee = 10; // divided by 1000
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        if(block.timestamp < _firstBlock + (7 days)) {
            ...
            if (teamPortion > 0) {
                address payable addr = payable(0x6Fa3A5763eE85BC28D43B0906da773Fc3E5F5a66);
                addr.transfer(teamPortion);
            }
            if (ecoPortion > 0) {
    ...
    function setBuyTaxes(uint256 marketingFee, uint256 liquidityFee, uint256 teamFee, uint256 ecosystemFee) external onlyOwner() {
        ...
        require(totalFee <= 500, "Sum of fees must be less than 50");
        if (block.timestamp < _firstBlock + (7 days)) {
            require(teamFee >= 10, "Cannot set teamFee lower than 10");
        }
    ...
    function updateMaxSell(uint256 maxSell) external onlyOwner() {
        require(maxSell >= _tTotal.mul(5).div(10000) , "cant make the limit lower than 0.05% of the supply");
        _maxSell = maxSell;
    }
    
    function updateMaxBuy(uint256 maxBuy) external onlyOwner() {
        require(maxBuy >= _tTotal.mul(5).div(10000) , "cant make the limit lower than 0.05% of the supply");
        _maxBuy = maxBuy;
    }
