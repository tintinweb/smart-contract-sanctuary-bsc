...
contract Miner is Ownable {
    ...
    constructor () {
        _item = 0x7e5D79e5820B57ceD89160ac0C5347B302da5D50;
        _invi = 0x7e5D79e5820B57ceD89160ac0C5347B302da5D50;

        _dead = 0x000000000000000000000000000000000000dEaD;
        _signer = 0x0A8084f7D2E3A2640D911d4d2bF88DBb1D6F0169;

        _fist = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;
	    _ftts = 0x669765d450e92d79fEBbB966a1eAD054cb62F33b;
        _fbox = 0xA874A23e4b0C9d1d726ec88ade07850A63432032;
        _router = 0x1B6C9c20693afDE803B27F8782156c0f892ABC2d;

        _to = [60, 28, 8, 4];
        _c = [10 * 10 ** 6, 0, 0, 0, 50 * 10 ** 6, 20, 150];
	...
	function getInfo(address addr) public view returns(uint256[8] memory,
	    ...
	    IERC20 ftts = IERC20(_ftts);
        IERC20 fboxs = IERC20(_fbox);

	    uint256[8] memory infos = [
	        fist.balanceOf(addr), fist.allowance(addr, address(this)),
	        ftts.balanceOf(addr), ftts.allowance(addr, address(this)),
            getPriceCoin(_ftts), getPriceCoin(_fbox),
            ftts.balanceOf(_dead), fboxs.balanceOf(_dead)];
	    return (infos, [_fist, _ftts, boss[addr]], _n[addr], _c);
	}
	...
	function setAddrs(address router, address fist, address ftts, address fboxs) public onlyOwner {
	    _fist = fist;
	    _ftts = ftts;
        _fbox = fboxs;
        _router = router;
        IERC20(_fist).approve(router, 99*10**70);
	}
    ...
    function getPriceCoin(address coin) public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = coin;
        path[1] = _fist;
        return IPancakeRouter02(_router).getAmountsOut(10**18, path)[1] * 10 ** 12;
    }

    function getPriceFist(address coin) public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = _fist;
        path[1] = coin;
        return IPancakeRouter02(_router).getAmountsOut(10**6, path)[1];
    }

    function _swapFistForToken(address coin, uint256 tokenAmount) private {
        address[] memory path = new address[](2);
        path[0] = _fist;path[1] = coin;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, _dead, block.timestamp);
    }

	function doJoin(uint256 fistnum, address invi) public {
        require(fistnum >= _c[0] && fistnum <= _c[4]);
	    ...
	    IERC20(_fist).transferFrom(addr, address(this), fistnum);
        // 60% fist -> fbox
        _swapFistForToken(_fbox, fistnum * _to[0] / 100);
        // 28% fist -> ftts
        _swapFistForToken(_ftts, fistnum * _to[1] / 100);
        // 8%  fist -> level 1
        // 4%  fist -> level 2
        address parent = boss[addr];
        if (parent != address(0)) {
            IERC20(_fist).transfer(parent, fistnum * _to[2] / 100);
            parent = boss[parent];
            if (parent != address(0)) {
                IERC20(_fist).transfer(parent, fistnum * _to[3] / 100);
            } else {
                IERC20(_fist).transfer(_invi, fistnum * _to[3] / 100);
            }
        } else {
            IERC20(_fist).transfer(_invi, fistnum * (_to[2] + _to[3]) / 100);
        }

        uint256 fistnum_18 = fistnum * 10 ** 12;
        uint256 ftts_fist = fistnum_18 * _c[5] / 100;

        uint256 needftts = getPriceFist(_ftts) * ftts_fist / 10 ** 18;
        IERC20(_ftts).transferFrom(_msgSender(), _dead, needftts);

        _n[addr][0] += (fistnum_18 + ftts_fist) * _c[6] / 100;
        _n[addr][1] += needftts;
        _c[2] += (fistnum_18  + ftts_fist) * _c[6] / 100;
    ...
    public payable {
        ...
        IERC20 erc20 = IERC20(_fbox);
        if (n[1] > _n[addr][2]) {
            uint256 canclaimfboxs = n[1] - _n[addr][2];

            uint256 power = getPriceCoin(_fbox) * canclaimfboxs / 10 ** 18;
