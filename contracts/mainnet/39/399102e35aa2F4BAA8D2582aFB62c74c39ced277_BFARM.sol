...
contract BFARM is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 public minReserveFraction = 10;
    ...
    function _transfer(
        ...
        // My implementation
        require(blacklist[from] < block.timestamp && blacklist[to] < block.timestamp, "sender or recipient is blacklisted!");
    ...
    function swap() private lockTheSwap {
        ...
        if(totalCollected > maxSwapTokens)
        {
            totalCollected = maxSwapTokens;
        }

        (uint res0, uint res1,) = IUniswapV2Pair(pair).getReserves();
        if(address(this) == IUniswapV2Pair(pair).token0()
            && totalCollected > res0 / minReserveFraction){
            totalCollected = res0 / minReserveFraction;
        }else if(address(this) == IUniswapV2Pair(pair).token1()
            && totalCollected > res1 / minReserveFraction){
            totalCollected = res1 / minReserveFraction;
        }
    ...
    function setMinReserveFraction(uint256 amount) external onlyOwner {
        require(amount>=3, "Reserve fraction is too high!");
        minReserveFraction = amount;
    }
    ...
    function setBlacklist(address account, uint timeLimit) external onlyOwner {
        require(account != pair, "Can't blacklist pair");
        require(timeLimit < 10 days, "Can't blacklist for more than 10 days");
        blacklist[account] = block.timestamp + timeLimit;
    }

    function multiBlacklist(address[] memory addresses, uint timeLimit) external onlyOwner {
        require(timeLimit < 10 days, "Can't blacklist for more than 10 days");
        for (uint256 i = 0;i < addresses.length; i++){
            require(addresses[i] != pair, "Can't blacklist pair");
            blacklist[addresses[i]] = block.timestamp + timeLimit;
        }
    }
