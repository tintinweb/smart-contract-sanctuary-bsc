...
contract Waterdrop is IBEP20, Ownable {
    ...
    string private constant _name               = 'Waterdrop';  
    string private constant _symbol             = 'WDROP';    
    ...
    uint16 private constant SellLimitDivider    = 1000;
    uint8 public constant MaxSellTax            = 20; // Max buytax = 12%

    //tax distribution
    //total normal tax = 12%
    uint8 private _devTax                       = 0;
    uint8 private _burnTax                      = 0;
    uint8 private _marketingTax                 = 40; // Buy: 3% - Sell: 4.5%
    uint8 private _liquidityTax                 = 18; // Buy: 2% - Sell: 3%
    uint8 private _stakingTax                   = 42; // Buy: 3% - Sell: 4.5%
    uint8 private _buyBackTax                   = 0; 

    //buy, sell and transfer tax
    uint8 public _buyTax                        = 8;
    uint8 public _sellTax                       = 12;
    uint8 public _transferTax                   = 0;
    ...
    uint256 private tokensBurnt; 

    bool public blacklistEnabled                = true;
    bool public liquidityAdded                  = false;
    uint256 public liquidityBlock               = 2;
    uint256 public constant blacklistBlocks     = 1;
    uint256 public snipersRekt;
 
    address public MainReward = 0x31987dB40794125D4C91143479383B312997EFc9;   // Water BSC ~$WTR
    ...
    function getvCOOLRewardBalance(address addr) public view returns (uint256){
        uint256 amount=getDividents(addr, true);
        return amount;
    }

    //gets balance of claimable MiscReward
    function getMiscBalance(address addr) private view returns (uint256){
    ...
    function ChangeMainReward(address newReward) private authorized{
    ...
    function ChangeMiscReward(address newReward) private authorized{
...
//OnlyOwner has control of LP functions
    ...
    //TotalTaxes is 100%. Max BuyTax is 99% due to bot/snipers by listing; Selltax can't be higher than 20% to prevent a honeypot; TransferTax is also max. 12%
    ...
    function UpdateTaxes(uint8 burnTaxes, uint8 buybackTaxes, uint8 devTaxes, uint8 marketingTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public authorized{
        ...
        require(sellTax<=MaxSellTax,"Selltax is higher than 20%.");
        require(transferTax<=4,"TransferTax is higher than 4%.");
