...
contract AlfaProfitSharing is IAlfaProfitSharing, Ownable {
    ...
    function pendingFor(address _user, uint256 _index) public view returns (uint256 payout_, bool matured_) {
        ...
        //vg Gon
        uint256 payoutGon_ = _earn.vgGonAmount;

        if (payoutGon_ > 0 && _earn.typeId == 1) {
            // to gAFA
            payout_ = IvgAFA(_earn.vgAfa).balanceForGons(payoutGon_);
            payout_ = payout_ - _earn.payout;
    ...
    function addEarnEx(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            bool hasSubmit = false;
            Earn[] memory list = earns[_users[i]];
            for (uint256 j = 0; j < list.length; j++) {
                Earn memory _temp = list[j];
                ...
                } else if (_temp.takeNumber == _takeNumber && _temp.typeId == _typeId) {
                    hasSubmit = true;
                    break;
                }
            }
            uint256 gonAmount = IvgAFA(_vgAfas[i]).gonsForBalance(_payouts[i]);
            if (!hasSubmit) {
                earns[_users[i]].push(
                    Earn({
                        payout: _payouts[i],
    ...
    // function checkEarnForNote(address _user) external view returns (bool) {
    //     require(_user != address(0), 'agent error');
    //     Earn[] memory list = earns[_user];
    //     uint256 _totalPayout;
    //     for (uint256 i = 0; i < list.length; i++) {
    //         Earn memory _temp = list[i];
    //         if (_temp.sub != address(0)) {
    //             _totalPayout = _totalPayout.add(_temp.payout);
    //         }
    //     }
    //     uint256 totalPayoutNote = INoteStake(gStaking).getNotesTotalIn(_user, 0);
    //     uint256 shouldEarn = totalPayoutNote.mul(bonusPercent).div(rateDenominator);
    //     if (_totalPayout > shouldEarn.add(1e17)) {
    //         return false;
    //     }
    //     return true;
    // }

    // function setBonusPercent(uint256 _bonusPercent) external _onlyOwner {
