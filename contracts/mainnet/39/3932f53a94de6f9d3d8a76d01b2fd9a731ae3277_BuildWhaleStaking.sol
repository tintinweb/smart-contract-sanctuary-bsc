...
contract BuildWhaleStaking is Ownable, ReentrancyGuard {
    using SafeERC20 for IBEP20;

    IBEP20 public immutable stakingToken;
    IBEP20 public immutable rewardToken;

    uint public duration = 180 days;
    uint public minimumStake = 70000 * 1e18;
    ...
    function changeMinimumStake(uint _newMinimum) external onlyOwner {
        require(_newMinimum <= (100000 * 1e18), "Minimum cannot be greater than 1%");
    ...
    function clearStuckTokens(address contractAddress) external onlyOwner {
        if (IBEP20(contractAddress) == IBEP20(rewardToken)) { require(block.timestamp > finishAt, "Staking Period Not Complete"); }
        if (IBEP20(contractAddress) == IBEP20(stakingToken)) { require(block.timestamp > finishAt, "Staking Period Not Complete"); }
        IBEP20 stuckTokens = IBEP20(contractAddress);
