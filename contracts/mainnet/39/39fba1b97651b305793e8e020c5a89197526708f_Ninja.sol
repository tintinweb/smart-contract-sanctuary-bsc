...
contract Ninja is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
        ...
        // Old Collection Contract Address
        IERC721 private migrationContract = IERC721(0xea66B7D94dea53C4f20E88AB317C08c6BB2D64DD);
        ...
        constructor(string memory name, string memory symbol, string memory base, uint256 max) ERC721(name, symbol)  {
            _name = name;
            _symbol = symbol;
            _max_supply = max;
        ...
        * Caping.
        */
        function mintAirdrop(uint256[] memory indexs,  address owner) external onlyOwner() {
            require(totalSupply() <= _max_supply, "NFT: Supply Exeded Cap");
             for(uint256 i = 0; i < indexs.length; i++) {
                _safeMint(owner, indexs[i]);
                _setTokenURI(indexs[i], string(abi.encodePacked(_baseURI(), Strings.toString(indexs[i]), ".json")));             
             }
        }

        /**
        * Migrate from old contract 1:1 ratio airdrops.
        * Caping.
        */

        function migrateAirdrop(uint256[] memory indexs) external onlyOwner() {
             require(totalSupply() <= _max_supply, "NFT: Supply Exeded Cap");
             for(uint256 i = 0; i < indexs.length; i++) {
                _safeMint(migrationContract.ownerOf(indexs[i]), indexs[i]);
                _setTokenURI(indexs[i], string(abi.encodePacked(_baseURI(), Strings.toString(indexs[i]), ".json")));             
             }
        }
        ...
        */
        ...
        receive () external payable {
            for(uint256 i = 0; i < wallets.length; i++) {
                (bool sent,) =  payable(address(wallets[i].walletAddress)).call{value: msg.value * wallets[i].percentage / 100}("");
