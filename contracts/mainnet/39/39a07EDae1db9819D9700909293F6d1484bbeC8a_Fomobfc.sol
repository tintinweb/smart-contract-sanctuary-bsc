...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _holdDividendFee = 6;
    ...
    constructor (
        ...
        holderCondition = 5000 * tokenDecimals;
        limitAmount = 30000 * tokenDecimals;
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                uint256 swapFee = _lpFee + _holdDividendFee + _fundFee;
    ...
    function _checkLimit(address to, uint256 amount) private view {
        if (0 == limitAmount) {
            return;
        }
        if (!_swapPairList[to] && !_feeWhiteList[to]) {
            if (block.number < startTradeBlock + 1200) {
                require(limitAmount >= amount, "exceed LimitAmount");
            }
        }
    }
    ...
    function _tokenTransfer(
        ...
        if (fee > 0) {
            ...
            uint256 destroyAmount = tAmount * _inviteFee / 100;
            if (destroyAmount > 0) {
                feeAmount += destroyAmount;
                uint256 perInviteAmount = destroyAmount / 3;
                address current;
            ...
            uint256 swapAmount = fee * tAmount / 100;
            if (swapAmount > 0) {
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 fundUsdt = usdtBalance * 2 * _fundFee / swapFee;
        if (fundUsdt > 0) {
            USDT.transferFrom(tokenDistributor, fundAddress, fundUsdt);
        }

        uint256 thisUsdt = usdtBalance - fundUsdt;
        if(thisUsdt>0){
            USDT.transferFrom(tokenDistributor, address(this), thisUsdt);
        }

        uint256 lpUsdt = usdtBalance * lpFee / swapFee;
        if (lpAmount > 0 && lpUsdt > 0) {
            _swapRouter.addLiquidity(
                address(this), usdtAddress, lpAmount, lpUsdt, 0, 0, _lpReceiveAddress, block.timestamp
            );
        }

        uint256 buyUsdt = USDT.balanceOf(address(this));
        if (buyUsdt > 0) {
            path[0] = usdtAddress;
            path[1] = _baseToken;
            _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    ...
    function processLP(uint256 gas) private {
        ...
        uint256 rewardBalance = BaseToken.balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (holdBalance > holdCondition && !excludeLpProvider[shareHolder]) {
                amount = rewardBalance * holdBalance / totalBalance;
                if (amount > 0) {
