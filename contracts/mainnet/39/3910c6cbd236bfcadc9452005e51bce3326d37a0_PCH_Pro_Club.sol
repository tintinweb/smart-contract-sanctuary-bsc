...
contract PCH_Pro_Club is Ownable {
    ...
    function transferTokens(uint256 _amount) public onlyOwner{
        require(IBEP20(token).balanceOf(address(this)) > _amount , "Not Enough Tokens");
        IBEP20(token).transfer(owner(), _amount);
    } 
    ...
    function claim(bytes32 _hash, uint8 _v, bytes32 _r, bytes32 _s, string memory message) public returns (bool){      
        ...
        uint256 amt_id = st2num(message);  
        uint256 id = amt_id % 100000000;
        uint256 amount = amt_id / 100000000;
        if(ecrecover(prefixedHashMessage, _v, _r, _s) == signer && hashedMessage == _hash){        
            require(userDetails[msg.sender].id == id ,"You are not a staker");  
