...
contract StakingV2 is Context, Ownable, ReentrancyGuard {
    ...
    bool public disableStake = false;
    bool public emergencyOverride = false;
    ...
    function stake(uint256 amount) external nonReentrant poolStarted {
        ...
        require(disableStake == false, "StakingRewards::stake: Staking period is disabled");
  ...
  function claimReward() public nonReentrant {
    ...
    function addStake(uint256 _addAmount) external payable onlyOwner{
        if(_rewardsTokenAddress == weth) {
            IWETH(weth).deposit{value: _addAmount}();
        }
        else{
            TransferHelper.safeTransferFrom(_rewardsTokenAddress, msg.sender, address(this), _addAmount);
        }
        poolAmount += _addAmount;
        emit PoolRefilled(poolAmount);
    }

    function setDisableStake(bool _value) external onlyOwner() {
        disableStake = _value;
    }

    function setEmergencyOverride(bool _flag) external onlyOwner() {
        emergencyOverride = _flag;
    }


    /* ========== MODIFIERS ========== */
    modifier poolStarted() {
        require(emergencyOverride == false && startTime != 0 && block.timestamp > startTime && block.timestamp < startTime + poolPeriod, "pool not started");
        _;
    }
    modifier poolEnded() {
        //place overide here
        require(emergencyOverride == false && startTime == 0 || block.timestamp > startTime + poolPeriod, "pool not ended");
        _;
    }

    /* ========== EVENTS ========== */

    event PoolRefilled(uint256 reward);
