    ...
    OwnableUpgradeable
{
    ...
    mapping(uint256 => string) private _tokenURIs; //Unused variable, remove on mainnet deployment

    event BaseURIChanged(string newBaseURI);

    function initialize() public initializer {
        __ERC721_init("CryptonaireTicket", "CTicket");
        __Ownable_init();
    }

    /**
     * @notice Set the address for the CrypotonaireLottery
     * @param _Lottery: address of the Crypotonaire lottery
     */
    function setLotteryAddress(address _Lottery) external onlyOwner {
        ICryptonaireLottery lottery = ICryptonaireLottery(_Lottery);
        uint256 currentTicketId = lottery.currentTicketId();
        require(
    ...
    function mint(
        ...
        require(msg.sender == Lottery, "Only CryptonaireLottery");
        require(_tokenURI.length == quantity, "Invalid quantity");

        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(to, currentTokenId);
            _setTokenURI(currentTokenId, _tokenURI[i]);
            currentTokenId++;
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0)) {
            ICryptonaireLottery lottery = ICryptonaireLottery(Lottery);
            lottery.updateTicketOwner(from, to, tokenId);
        }
    }
...
interface ICryptonaireLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in BUSD per group, and make lottery claimable
     * @param _lotteryId: lottery id
