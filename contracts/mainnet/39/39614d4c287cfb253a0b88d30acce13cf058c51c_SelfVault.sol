...
// File: contracts/SELFVault.sol

pragma solidity 0.6.12;

contract SelfVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 SELFAtLastUserAction; // keeps track of SELF deposited at the last user action
    ...
    /**
     * @notice Deposits funds into the SELF Vault
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.SELFAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Reinvests SELF tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the SELF Vault
     */
     ...
     * @return Expected reward to collect in SELF
     */
    function calculateHarvestSELFRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingSELF(0, address(this));
     ...
     */
    function calculateTotalPendingSELFRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingSELF(0, address(this));
        amount = amount.add(available());
    ...
    /**
     * @notice Withdraws from funds from the SELF Vault
