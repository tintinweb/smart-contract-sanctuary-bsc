...
interface RouterLike {
    function volume(address, address) external view returns (uint256,uint256);
}
contract InviterLevel {
    ...
    RouterLike public router = RouterLike(0x9a5d0fB31Fb21198F3c9043f65DF761510831Fc9);
    address public pair = 0x6b4C62743cD28E8e611E2182707c1cdd8043D14B;
    ...
    function setSelfLevel(address referrer) public {
        require(referrer != address(0) && referrer != msg.sender, "InviterLevel/It cannot be a zero address");
    ...
    function directReward(address ust) public view returns (uint256, uint256, uint256, address[] memory, uint256[2][] memory) {
        uint256 n = count[ust];
        address[] memory uline = new address[](n);
        uint256[2][] memory amounts = new uint256[2][](n);
        if (n == 0) return (0, 0, 0,uline,amounts);
        uint totaleat;
        uint totalusdt;
        for (uint i = 1; i <=n ; i++) {
            address underline = referrals[ust][i];
            (uint256 eatwad,uint256 usdtwad) = router.volume(underline,pair);
            uint256 eatamount = eatwad/100;
            uint256 usdtamount = usdtwad/100;
            totaleat += eatamount;
            totalusdt += usdtamount;
            uline[i-1] = underline;
            amounts[i-1] = [eatamount,usdtamount];
        }
        return (n, totaleat, totalusdt, uline, amounts);
    }

    function getTotalReward(address ust) public view returns (uint256, uint256){
          (,uint256 eatamount, uint256 usdtamount, ,) = directReward(ust);
        return (eatamount, usdtamount);
    }
}