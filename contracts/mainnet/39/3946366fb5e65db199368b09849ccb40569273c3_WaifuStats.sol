...
contract WaifuStats {
    ...
    function getSpeed(uint256 waifuID) public view returns (uint256) {
        IShoujoStats.Shoujo memory waifuToCheck = waifuStats.tokenStatsByIndex(waifuID);
        return ((waifuToCheck.aggressiveness + 1) * (waifuToCheck.genki + 1) + waifuToCheck.boyish);
    }
	...
	function getWinChance(uint256 myWaifu) external view returns(uint256) {
        (uint256 wins, uint256 losses) = getWinsAndLosses(myWaifu);

        return (wins * 100 / (wins + losses));
    }

    function rewardsNeededToFight(uint256 myWaifu) external view returns(uint256) {
        (uint256 wins, uint256 losses) = getWinsAndLosses(myWaifu);
    ...
    function getWinsAndLosses(uint256 myWaifu) public view returns(uint256, uint256) {
        uint256 wins;
        uint256 losses;
        (uint256 speedMe, uint256 rarityMe)  = getSpeedAndRarity(myWaifu);
        uint256 healthMe = 100 + (rarityMe * 10);

        for (uint256 i = 0; i < 9001; i++){
            
            (uint256 speedHer, uint256 rarityHer)  = getSpeedAndRarity(i);
            uint256 healthHer = 100 + (rarityHer * 10);
