...
contract Gammatroller is GammatrollerV1Storage, GammatrollerInterface, GammatrollerErrorReporter, ExponentialNoError {
    ...
    /// @notice Emitted when a market is added to the list of Boosted Markets
    event MarketBoostAdded(GTokenInterface indexed gToken);
    ...
    function _initializeMarket(address gToken) internal {
        ...
        if (supplyState.index == 0) {
            ...
            supplyState.index = gammaInitialIndex;
            supplyState.boostIndex = gammaInitialIndex;
        }

        if (borrowState.index == 0) {
            // Initialize borrow state index with default value
            borrowState.index = gammaInitialIndex;
            borrowState.boostIndex = gammaInitialIndex;
    ...
    function updateGammaSupplyIndex(address gToken) internal {
        ...
        if (deltaBlocks > 0 ) {
            ...
            Double memory index = Double({mantissa: supplyState.index});
            Double memory boostIndex = Double({mantissa: supplyState.boostIndex});
            ...
            if(supplyTokens > 0 && supplySpeed > 0 ){
                ...
                Double memory ratio = fraction(gammaAccruedRegular, supplyTokens);
                index = add_(index, ratio);
                ...
                if (gammaBoostPercentage != 0 && totalFactor != 0) {
                    ...
                    Double memory boostRatio = fraction(gammaAccruedBoosted, totalFactor);
                    boostIndex = add_(boostIndex, boostRatio);
                }
            }

            gammaSupplyState[gToken] = GammaMarketState({
                index: safe224(index.mantissa, "new index > 224bits"),
                boostIndex: safe224(boostIndex.mantissa, "new index > 224bits"),
                block: safe32(blockNumber, "block number > 32bits")
            });
        }
    }
    ...
    function distributeSupplierGamma(address gToken, address supplier) internal {
        ...
        Double memory supplyIndex = Double({mantissa: gammaSupplyState[gToken].index});
        ...
        if (supplierIndex.mantissa == 0 && supplyIndex.mantissa >= gammaInitialIndex) {
            supplierIndex.mantissa = gammaInitialIndex;
        }

        (uint256 supplierTokens, uint256 supplierFactor, bool lastRewardReceived) = GTokenInterface(gToken).getUserData(supplier);

        Double memory deltaIndex = sub_(supplyIndex, supplierIndex);
        uint supplierDelta = mul_(supplierTokens, deltaIndex);
        uint supplierAccrued = add_(gammaAccrued[supplier], supplierDelta);

        Double memory supplyBoostIndex = Double({mantissa: gammaSupplyState[gToken].boostIndex});

        // If supplyBoostIndex is 0, then we don't need to update it
        if (supplyBoostIndex.mantissa != 0 && lastRewardReceived == false) {

            Double memory supplierBoostIndex = Double({mantissa: gammaSupplierBoostIndex[gToken][supplier]});
            gammaSupplierBoostIndex[gToken][supplier] = supplyBoostIndex.mantissa;

            if (supplierBoostIndex.mantissa == 0) {
                supplierBoostIndex.mantissa = gammaInitialIndex;
            }

            if (supplierFactor != 0){
                Double memory deltaBoostIndex = sub_(supplyBoostIndex, supplierBoostIndex);
                uint supplierBoostDelta = mul_(supplierFactor, deltaBoostIndex);
                supplierAccrued = add_(supplierAccrued, supplierBoostDelta);
            }

            if (markets[address(gToken)].isBoostDeprecated == true){
                GTokenInterface(gToken).updateLastRewardReceived(supplier);
            }
    ...
    function updateFactorsAllowed(address gToken) override external view returns(uint){
        Market storage market = markets[gToken];
        if (market.isBoosted == true &&  !market.isBoostDeprecated){
            return 0;
        }
        return 1;
    }
    ...
    function _addBoostedMarket(GTokenInterface[] memory gTokens) external onlyOwner{
       uint256 len = gTokens.length;
       for (uint256 i = 0; i < len; i++){
            GTokenInterface gToken = gTokens[i];
            Market storage market = markets[address(gToken)];
            require(market.isListed && !market.isBoosted && !market.isBoostDeprecated);          
            market.isBoosted = true;
            allBoostedMarkets.push(GTokenInterface(gToken));

            emit MarketBoostAdded(gToken);
       }
    ...
    function _setGammaBoostInternal(GTokenInterface gToken, uint gammaBoost) internal {
        Market storage market = markets[address(gToken)];
        require(market.isBoosted == true, "gamma not boosted");
        require(market.isBoostDeprecated == false, "gamma boost permanently turned off");
    ...
    function _deprecateBoostMarket(address gToken) external onlyOwner{
        Market storage market = markets[gToken];
        require(market.isListed && market.isBoosted, "not listed/boosted");
        //require(market.isBoosted == true, "market not boosted");
        require(gammaBoostPercentage[address(gToken)] == 0, "gammaBoost% not zero");

        market.isBoosted = false;
        market.isBoostDeprecated = true;
