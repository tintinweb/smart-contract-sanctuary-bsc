...
contract EMMToken is IERC20, Ownable {
    ...
    address private _marketAddress = address(0xA08C93E18A4be46cb21eBa6182Fd7EB515245C72);
    ...
    uint256 private maxBuyAmount = 100000 * 1e18;
    ...
    constructor() {
        ...
        _tTotal = 600000000 * 10 ** _decimals;
		...
		if(sender == uniswapV2Pair) {
            ...
            if(fee && startSwap) {
                if(block.timestamp.sub(startTime) <= 30*60) {
                    require(tAmount <= maxBuyAmount, "limit max buy 100000");
                }

                if(block.timestamp.sub(startTime) <= 0) {
                    _blackList[recipient] = true;
                }
                
            }

            _tokenTransferFee(sender, recipient, tAmount, fee);
        } // sell
            ...
            if(fee) {
                require(tAmount <= IERC20(this).balanceOf(sender).div(100).mul(SELLMAX), "sell must limit lower 100 percent");
            }
            _tokenTransferFee(sender, recipient, tAmount, fee);
        } else{
    ...
    function _tokenTransferFee(
		...
		if(fee){
            ...
            allfee = allfee.add(_lpRewardFee);
            ...
            _rOwned[recipient] = _rOwned[recipient].add(rAmount.div(percent).mul(percent.sub(allfee)));
            emit Transfer(sender, recipient, tAmount.div(percent).mul(percent.sub(allfee)));
