...
contract CompanionWeeklyBasicNeedsContract {
	...
	uint public timeQuantStart = 1639155600;
	...
	address public constant REGISTRY_CONTRACT = 0xc8Be846F770F7E2b2E039d432d56dE228F35916E;
	CompanionContractsRegistryContract private rt = CompanionContractsRegistryContract(REGISTRY_CONTRACT);
	

    modifier isNotZeroAddr(address addr) {
    ...
    modifier isRegistryValid() {
        require(rt.isRegistryValid(), "Invalid registry. Access denied.");
        _;
    }
	function updateExternalFee() isAdmin isRegistryValid external payable {
	    updateExternalFeePrivate();
	}
	function updateExternalFeeAndWithdraw() isAdmin isRegistryValid external payable {
		startGas = gasleft();
		updateMaxBalance();
		updateExternalFeePrivate();
		uint fee = getFeeGasCorrectionWithd();
        uint withdLimit = historyMaxBalance / 10 * 4;
        if(address(this).balance < withdLimit)
        {
		    uint withdResult = withdLimit / tx.gasprice;
		    if(fee < withdResult)
			    fee = withdResult;
        }
        uint gasUsed = (startGas - gasleft() + fee) * tx.gasprice;    
    ...
    function registerBinarySlot(address addr) isNotZeroAddr(addr) private {
    ...
    function makeDeposit(address addr) isRegistryValid private {
    ...
    function makeDeposit() isRegistryValid private {
        Companion storage cmp = companions[msg.sender];
        if(cmp.deposit == 0)
    ...
    function charityToContract() isRegistryValid external payable {
        require(        
