...
contract Vaultpancakeswap is Ownable {
   ...
   (amountA,amountB,liquidity) = pancakeRouter.addLiquidity(tokenA,tokenB,amountADesired,amountBDesired,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    ...
    function addLiquidityETH(
        ...
        uint amountETHMin,
        uint duration
    ) external onlyManager payable returns (uint amountToken, uint amountETH, uint liquidity) {
    BEP20(token).approve(pancake, amountTokenDesired);
    (amountToken,amountETH,liquidity)= pancakeRouter.addLiquidityETH(token,amountTokenDesired,amountTokenMin, amountETHMin, safeHouse, block.timestamp + duration);
    }
