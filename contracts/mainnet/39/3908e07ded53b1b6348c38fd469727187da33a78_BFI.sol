...
contract BFI {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 20000 ether;
	...
	uint[8] public ref_bonuses = [10,5,8,3,3,3,3,3];
    
    
    uint256[9] public defaultPackages = [20000 ether,500000 ether,100000 ether,200000 ether,1000000 ether,4000000 ether,10000000 ether,20000000 ether,50000000 ether];
    
    
    mapping(uint256 => address payable) public singleLeg;
    uint256 public singleLegLength;
    uint[8] public requiredDirect = [1,1,3,4,5,6,7,8];
    ...
    function invest(address referrer, uint256 buy_amount ) public {
		...
		//  8 Level Referral
		   _refPayout(msg.sender,msgValue);
            ...
            uint256 _dev_fees = buy_amount.mul(2).div(PERCENTS_DIVIDER);
  ...
  function getEligibleLevelCountForUpline(address _user) public view returns(uint8 uplineCount, uint8 downlineCount){
      ...
      else if(TotalDeposit >= defaultPackages[5] && TotalDeposit < defaultPackages[6] && TotalDeposit < defaultPackages[7] && TotalDeposit < defaultPackages[8]){
          uplineCount = 20;
          downlineCount = 30;
      }
