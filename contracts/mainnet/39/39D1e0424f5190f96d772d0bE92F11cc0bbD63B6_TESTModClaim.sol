...
// File: contracts/interfaces/IApple.sol
...
interface IPair {
    function getReserves() external view  returns (uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast, address _baseToken);
}
...
// File: contracts/TestModClaim.sol
...
// *** Deploy Contract on a FRI, set each mod's start date the same as the launch timestamp ***
...
contract TESTModClaim is Ownable, ReentrancyGuard {
    ...
    uint chiefMultiplier = 2;
    uint leadMultiplier = 1;
    bool public applePaymentsEnabled;
    bool public BUSDPaymentsEnabled;
    ...
    constructor() {
        ...
        applePaymentsEnabled = true;
    }

    modifier modCheck() {
        require(isChief[msg.sender] || isLead[msg.sender] , "Not a mod");
        require(isChief[msg.sender] != isLead[msg.sender] , "Cannot be chief and lead simultaneously");
        _;
    }

    modifier paymentCheck {
        require(applePaymentsEnabled || BUSDPaymentsEnabled, "Payments are disabled");
        require(applePaymentsEnabled != BUSDPaymentsEnabled , "Both payments cannot be enabled at once");
        _;
    } 

    modifier timeCheck() {
        require((block.timestamp.sub(startTime[msg.sender])).div(200) >= 1 , "Must wait one week");
        require(block.timestamp - lastClaim[msg.sender] >= 200 , "You have not waited a week since your last payment");
        _;
    }
    ...
    function getPaid() external nonReentrant modCheck() paymentCheck timeCheck() {
        ...
        uint timeMultiple;
        uint timeSinceLastClaim = (block.timestamp.sub(lastClaim[msg.sender]));   
        if (lastClaim[msg.sender] == 0) { // if mod has never claimed yet
            timeMultiple = block.timestamp.sub(startTime[msg.sender]).div(200);
        } else {
            timeMultiple = (block.timestamp.sub(lastClaim[msg.sender])).div(200);
        }       

        if (applePaymentsEnabled && !BUSDPaymentsEnabled) {

            if (isChief[msg.sender]) {
                uint chiefPayment;
                (chiefPayment,) = calculateTokens();
                AppleInterface.mint(msg.sender, chiefPayment * timeMultiple);
                lastClaim[msg.sender] = timeSinceLastClaim.sub(200);
        ...
        } else if (BUSDPaymentsEnabled && !applePaymentsEnabled) {
    ...
    function setStartTime(address _member, uint256 _startTime) external onlyOwner modCheck() {
        startTime[_member] = _startTime;
    }

    function enableBUSDPayments() external onlyOwner {
        applePaymentsEnabled = false;
        BUSDPaymentsEnabled = true;
        assert(applePaymentsEnabled != BUSDPaymentsEnabled);
    }

    function enableApplePayments() external onlyOwner {
        BUSDPaymentsEnabled = false;
        applePaymentsEnabled = true;
        assert(applePaymentsEnabled != BUSDPaymentsEnabled);
    }

    function disablePayments() external onlyOwner {
        BUSDPaymentsEnabled = false;
        applePaymentsEnabled = false;
        assert(!applePaymentsEnabled && !BUSDPaymentsEnabled);
    }
