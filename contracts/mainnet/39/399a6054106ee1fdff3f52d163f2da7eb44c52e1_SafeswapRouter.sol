                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"c5c0f41407c4baa59af66c427d16b88ec7c38d2a749bd6347c223ef8b0abb7cf" // init code hash
                        )
                    )
                )
            )
...
contract SafeswapRouter is ISafeswapRouter02, Initializable {
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint256 deadline
    ) external virtual override ensure(deadline) onlyRouterTrade returns (uint256[] memory amounts) {
    ...
    function swapExactETHForTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override ensure(deadline) onlyRouterTrade returns (uint256[] memory amounts) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external virtual override ensure(deadline) onlyRouterTrade {
        if (nameToInfo[path[0]].enabled == true && killSwitch == false && (nameToInfo[path[0]].sellFeePercent > 0)) {
            uint256 deduction = (amountIn * nameToInfo[path[0]].sellFeePercent) / ONE;
            amountIn = amountIn - deduction;
            TransferHelper.safeTransferFrom(path[0], from, nameToInfo[path[0]].feesAddress, deduction);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint256 deadline
    ) external payable virtual override ensure(deadline) onlyRouterTrade {
        require(path[0] == WETH, "SafeswapRouter: INVALID_PATH");
