...
// File: contracts/interfaces/IBSFactory.sol


pragma solidity 0.6.12;

interface IBSFactory {
    function createPool(
        IERC20 tokenA,
        IERC20 tokenB,
        uint32 ampBps,
...
// File: contracts/interfaces/IBSCallee.sol
...
// File: contracts/interfaces/IBSPool.sol
...
interface IBSPool {
    ...
    function factory() external view returns (IBSFactory);
...
contract BSPool is IBSPool, ERC20Permit, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    IBSFactory public override factory;
    ...
    constructor() public ERC20Permit("BitindiSwap LP", "BS-LP", "1") {
        factory = IBSFactory(msg.sender);
    }
    ...
    function initialize(
        ...
        require(msg.sender == address(factory), "BS: FORBIDDEN");
        token0 = _token0;
        token1 = _token1;
        ampBps = _ampBps;
    ...
    function mint(address to) external override nonReentrant returns (uint256 liquidity) {
        ...
        require(liquidity > 0, "BS: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(balance0 >= data.reserve0 && balance1 >= data.reserve1, "BS: UNSYNC_RESERVES");
        ...
        require(amount0 > 0 || amount1 > 0, "BS: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(
        ...
        require(amount0Out > 0 || amount1Out > 0, "BS: INSUFFICIENT_OUTPUT_AMOUNT");
        (bool isAmpPool, ReserveData memory data) = getReservesData(); // gas savings
        require(
            amount0Out < data.reserve0 && amount1Out < data.reserve1,
            "BS: INSUFFICIENT_LIQUIDITY"
        );
        ...
        ReserveData memory newData;
        {
            ...
            require(to != address(_token0) && to != address(_token1), "BS: INVALID_TO");
            ...
            if (callbackData.length > 0)
                IBSCallee(to).bsSwapCall(msg.sender, amount0Out, amount1Out, callbackData);
        ...
        uint256 amount1In = newData.reserve1 > data.reserve1 - amount1Out
            ...
            : 0;
        require(amount0In > 0 || amount1In > 0, "BS: INSUFFICIENT_INPUT_AMOUNT");
        verifyBalance(
            amount0In,
            amount1In,
    ...
    function name() public override view returns (string memory) {
        ...
        return string(abi.encodePacked("BitindiSwap LP ", _token0.symbol(), "-", _token1.symbol()));
    }

    function symbol() public override view returns (string memory) {
        IERC20Metadata _token0 = IERC20Metadata(address(token0));
        IERC20Metadata _token1 = IERC20Metadata(address(token1));
        return string(abi.encodePacked("BS-LP ", _token0.symbol(), "-", _token1.symbol()));
    }
    ...
    function safeUint112(uint256 x) internal pure returns (uint112) {
        require(x <= MAX_UINT112, "BS: OVERFLOW");
        return uint112(x);
    }
}

// File: contracts/BSFactory.sol


pragma solidity 0.6.12;


contract BSFactory is IBSFactory {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function createPool(
        ...
        require(tokenA != tokenB, "BS: IDENTICAL_ADDRESSES");
        (IERC20 token0, IERC20 token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(address(token0) != address(0), "BS: ZERO_ADDRESS");
        require(ampBps >= BPS, "BS: INVALID_BPS");
        // only exist 1 unamplified pool of a pool.
        require(
            ampBps != BPS || getUnamplifiedPool[token0][token1] == address(0),
            "BS: UNAMPLIFIED_POOL_EXISTS"
        );
        require(feeUnits > 0 && feeOptions[feeUnits], "BS: FEE_OPTION_NOT_EXISTS");
        pool = address(new BSPool());
        BSPool(pool).initialize(token0, token1, ampBps, feeUnits);
    ...
    function setFeeConfiguration(address _feeTo, uint24 _governmentFeeUnits)
        ...
        onlyFeeSetter
    {
        require(_governmentFeeUnits > 0 && _governmentFeeUnits < 20000, "BS: INVALID FEE");
        feeTo = _feeTo;
    ...
    function enableFeeOption(uint24 _feeUnits) external override onlyFeeSetter {
        require(_feeUnits > 0, "BS: INVALID FEE");
