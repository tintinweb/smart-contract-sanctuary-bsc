    ...
    Ownable
{
    ...
    constructor() {
        ...
        if (block.chainid == 56) {
            tokenAddress = 0x5416f06830C7826A2ee774c53a3589e707269AB3; //RCC
            busdAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        } else if (block.chainid == 97) {
            tokenAddress = 0xE02dF9e3e622DeBdD69fb838bB799E3F168902c5; //TESTNET TOKEN
            busdAddress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
        } else revert();

        rewardTokens.push(address(tokenAddress)); // RCC - 0
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
        claimWait = 1 hours;
        minimumTokenBalanceForDividends = 1000 * 1e9; //must hold 1 tokens to receive dividends
    }
...
contract Pokermai is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public stopBotsStatus = true;
    ...
    uint256 public rccLiqBuyFee;
    uint256 public liquidityBuyFee;
    uint256 public devBuyFee;

    uint256 public tokensForRewards;
    uint256 public tokensForMarketing;
    uint256 public tokensForRccLiquidity;
    ...
    event EnableStopBot(bool enabled);

    constructor(
        address _owner,
        address _marketingWallet,
        address _devWallet
    ) ERC20("PokerMai", "PKR POKEROI", 9) {
        address newOwner = _owner; //make sure to transfer tokens to owner
        uint256 _totalSupply = 10 * 1e6 * 1e9; //100 million

        maxBuyTransactionAmount = (_totalSupply * 2) / 100; // 2% maxBuyTransactionAmount
        maxSellTransactionAmount = (_totalSupply * 2) / 100; // 2% maxSellTransactionAmount
        ...
        */
        ...
        transferOwnership(msg.sender); //give ownership to dev
    }
    ...
    //Toggle stopBot on and off
    function enableBlacklist(bool enabled) external onlyOwner {
        stopBotsStatus = enabled;
        emit EnableStopBot(enabled);
    }

    function updateMaxBuyAmount(uint256 newNum) external onlyOwner {
        require(
            newNum > ((totalSupply() * 5) / 1000) / 1e9,
            "Cannot set maxBuyTransactionAmount lower than 0.5%"
        );
    ...
    function updateMaxSellAmount(uint256 newNum) external onlyOwner {
        require(
            ...
            "Cannot set maxSellTransactionAmount lower than 0.2%"
        );
    ...
    function addWalletToBotList(address wallet, bool status)
        external
        onlyOwner
    {
        require(wallet != uniswapV2Pair, "can not add trading pair");
        isBot[wallet] = status;
    }

    //blaclist mutiple wallets
    function setBotStatusMutiple(address[] calldata addresses, bool status)
        external
        onlyOwner
    {
    ...
    function viewRewardsToken(uint256 indexToView)
        public
        view
        returns (address)
    {
    ...
    function _transfer(
        ...
        if (stopBotsStatus) {
            require(isBot[from] == false, "wallet is a sniper bot");
        }
        ...
        if (takeFee) {
            ...
            if (
                ...
                uint256 sniperFees = amount.mul(99).div(100);
                super._transfer(from, owner(), sniperFees); //transfer 99% of fee for marketing
            }
            // on sell
                ...
                tokensForRccLiquidity += (fees * rccLiqSellFee) / totalSellFees;
            ...
            // on buy
                ...
                tokensForRccLiquidity += (fees * rccLiqBuyFee) / totalBuyFees;
    ...
    function swapTokensForRMAI(uint256 tokenAmount) private {
    ...
    function swapAndBurnRcc(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(RMAI);
    ...
    function addLiquidityRCC(uint256 tokenAmount, uint256 busdAmount) private {
        // approve token transfer to cover all possible scenarios
        RMAI.approve(address(uniswapV2Router), tokenAmount);
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForRccLiquidity +
            tokensForMarketing +
            tokensForRewards +
            tokensForDev +
            tokensForLiquidity;
        ...
        // Halve the amount to add to Rcc Liquidity
        uint256 rccLiquidityTokens = (contractBalance * tokensForRccLiquidity) /
            totalTokensToSwap;
        //halve the amount to add to token liquidity
        uint256 liquidityTokens = (contractBalance * tokensForLiquidity) /
            totalTokensToSwap /
            2;
        // uint256 totalLiquidityTokens = rccLiquidityTokens.add(liquidityTokens);

        uint256 amountToSwapForETH = contractBalance.sub(
            rccLiquidityTokens.add(liquidityTokens)
        );
        ...
        uint256 balanceBeforeSwap = RMAI.balanceOf(address(this));

        swapTokensForRMAI(rccLiquidityTokens / 2);

        uint256 balanceAfterSwap = RMAI.balanceOf(address(this)).sub(
            balanceBeforeSwap
        );

        uint256 busdBalanceBeforeSwap = BUSD.balanceOf(address(this));

        swapTokensForBUSD(rccLiquidityTokens / 2);

        uint256 busdBalanceAfterSwap = BUSD.balanceOf(address(this)).sub(
            busdBalanceBeforeSwap
        );

        // swapAndBurnRcc(liquidityTokens);
        ...
        uint256 ethForTeam = ethBalance.mul(tokensForDev).div(
            totalTokensToSwap
        );
        ...
        // uint256 ethForRccLiquidity = ethAvailableForLiq
        //     .mul(liquidityTokens)
        //     .div(totalLiquidityTokens);
        // uint256 ethForLiquidity = ethAvailableForLiq - ethForRccLiquidity;

        tokensForRccLiquidity = 0;
        ...
        if (balanceAfterSwap > 0 && busdBalanceAfterSwap > 0) {
            addLiquidityRCC(balanceAfterSwap, busdBalanceAfterSwap);
            emit SwapAndLiquifyRCC(
                balanceAfterSwap,
                busdBalanceAfterSwap,
                tokensForRccLiquidity
            );
        }
    ...
    uint256 public sellCooldownperiod = 1 minutes; //1 hour cooldown
