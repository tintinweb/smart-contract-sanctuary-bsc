...
contract RussianRoulette is Ownable, VRFConsumerBaseV2 {
    ...
    /// @notice time until a re-request of the random number is allowed
    uint256 public RE_REQUEST_TIME = 3_600;
    ...
    constructor(
        ...
        feeRecipient = 0xD69341f9127907d9c1Dc0DFB39e9b1601B06eFc9;
    }
    ...
    function startGame(uint256 tableId) external payable isValidTable(tableId) {
        _startGame(tableId);
    }
    ...
    function joinGame(uint256 tableId, uint256 ref) external payable isValidTable(tableId) {
        ...
        if (games[gameId].players.length == 2) {
            games[gameNonce].startTime = block.timestamp;
        }
    ...
    function startAndJoin(uint256 tableId, uint256 ref) external payable isValidTable(tableId) {
        _startGame(tableId);
        _joinGame(tableId, ref);
    }
    ...
    function destroyGame(uint256 tableId) external isValidTable(tableId) {
    ...
    function donateToPot(uint256 tableId) external payable isValidTable(tableId) {
    ...
    function _startGame(uint256 tableId) internal {
        ...
        games[gameNonce].initiator = msg.sender;
    ...
    function _requestRandom(uint256 gameId) internal {
        ...
        games[gameId].requestTime = block.timestamp;
    ...
    function fulfillRandomWords(
        ...
        if (nPlayers > 0) {
            ...
            if (nPlayersToSplit == 0) {
                ...
                uint potValueForEachWinner = ( games[gameId].pot / nPlayersToSplit ); // - 1 to avoid round off error
    ...
    function timeToRerequest(uint256 gameId) public view returns (uint256) {
        ...
        return endTime <= block.timestamp ? 0 : endTime - block.timestamp;
    }
    ...
    function listTableAndGamesInfo() external view returns (
        ...
        adjustedPots = new uint256[](tableNonce - 1);
        ...
        for (uint i = 1; i < tableNonce;) {
            ...
            adjustedPots[i - 1] = games[gameId].pot - ( games[gameId].pot * ( platformFee + tokens[gameTokens[i-1]].fee ) / FEE_DENOM );
    ...
    function getGameInfo(uint256 gameId) external view returns(
        ...
        adjustedPot = pot - ( pot * ( platformFee + tokens[tables[games[gameId].tableId].token].fee)) / FEE_DENOM;
    }

    function getAdjustedPot(uint256 tableId) external view returns(uint256) {
        return games[tables[tableId].gameID].pot - 
            ( games[tables[tableId].gameID].pot * ( platformFee + tokens[tables[tableId].token].fee ) / FEE_DENOM );
    }
