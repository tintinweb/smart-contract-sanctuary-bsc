...
contract HMC is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("HMC", "HMC") {
        data = IData(dataAddr);

        _mint(
            data.string2addressMapping("perwallet"),
            2400 * 10**8 * 10**decimals()
        );
    ...
    function _transfer(
        ...
        if (pairInclude(from) || pairInclude(to)) {
            ...
            if (
                balanceOf(to) == 0 &&
                data.address2addressMapping(to) == address(0) &&
                amount >= 10 * 10**decimals()
            ) {
                data.setAddress2AddresssData(to, from);
            }
    ...
    function takeFee(
        ...
        if (fee[2] > 0) {
            _transfer(
                address(this),
                data.string2addressMapping("marketwallet"),
                fee[2]
            );
        }
        ...
        if (fee[3] > 0) {
            _transfer(
                address(this),
                data.address2addressMapping(user),
                fee[0].sub(fee[1]).sub(fee[2])
            );
        }
    ...
    function calFee(
        ...
        uint256 marketFeeRate = data.string2uintMapping(
            sell ? "sellFeeRateForMarket" : "buyFeeRateForMarket"
        );
        uint256 inviteFeeRate = data.string2uintMapping(
            sell ? "sellFeeRateForInvite" : "buyFeeRateForInvite"
        );
        if (data.address2addressMapping(user) == address(0)) inviteFeeRate = 0;

        result[1] = amount.mul(destroyFeeRate).div(1000000).div(100);
        result[2] = amount.mul(marketFeeRate).div(1000000).div(100);
        result[3] = amount.mul(inviteFeeRate).div(1000000).div(100);
        result[0] = result[1].add(result[2]).add(result[3]);
        return result;
    }
