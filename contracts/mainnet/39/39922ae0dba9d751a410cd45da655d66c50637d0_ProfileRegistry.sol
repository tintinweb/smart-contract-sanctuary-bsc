...
interface IERC721ABurnable is IERC721AUpgradeable {
    function burn(uint256 tokenId) external;
}

interface IProfileRegistry {
    function customTokenUri(uint256 tokenId)
        external
        view
...
contract ProfileRegistry is
    ...
    IProfileRegistry
{
    uint256[] public _seed; // here we use an empty seed
    ...
    function mint() public payable nonReentrant {
        require(msg.value >= price, "eth not attached");
        _mint(msg.sender, 1);
    }
    ...
    // revoke
    function revokeId(uint256 tokenId) external onlyOwner {
        _burn(tokenId);
    }
    ...
    /// @dev add customTokenUri overlay, for json spec see https://docs.opensea.io/docs/metadata-standards
    function setTokenOverride(uint256 tokenId, string calldata uri_) external {
        require(msg.sender == ownerOf(tokenId), "you don't own this profile");
        customTokenUri[tokenId] = uri_;
    }
