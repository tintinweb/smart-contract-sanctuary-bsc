...
contract AutoSwap is Ownable {
    ...
    function swapTwo(uint256 amountOut, uint256 amountInMax, address[] memory path, address[] calldata receiver) public {
        for (uint256 i = 0; i < receiver.length; i++) {
            try swapRouter.swapTokensForExactTokens(amountOut, amountInMax, path, receiver[i], block.timestamp + 15) {} catch {}
        }
    }
