...
contract DrivenLockup is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        lockups.push(Lockup(0, 30, 0, 0, 951293759, 0, 0, 0, false, 150));
        lockups.push(Lockup(1, 90, 0, 0, 1426940638, 0, 0, 0, false, 150));
        lockups.push(Lockup(2, 180, 0, 0, 1902587518, 0, 0, 0, true, 100));
        lockups.push(Lockup(3, 360, 0, 0, 2378234397, 0, 0, 0, true, 100));
    ...
    function claimReward(uint8 _stakeType) external nonReentrant {
        ...
        if (pending > 0) {
            ...
            IERC20(earnedToken).mint(address(msg.sender), pending);
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        ...
        uint256 withdrawFee = amountToTransfer.mul(lockup.emergencyWithdrawFee).div(1000);
    ...
    function getActiveStake(address user, uint8 stakeType) external view returns(Stake memory stake_){
