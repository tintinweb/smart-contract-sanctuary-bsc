...
contract VNine is ERC20 {
    ...
    IERC20 public USDT;IERC20 public pair;IERC20 public SCORE;
    VNineWarp warp;
	VNineWarp nftWarp;
	...
	function changeSwapWarp(VNineWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function maxPriceBuyRewardCheck(address _buyUser, uint256 buyAmount) private {
    ...
    function _transfer(
		...
		bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**16;
        ...
        if (takeFee) {
			...
			if(from == uniswapV2Pair){
				...
				super._transfer(from, address(warp), amount.div(100).mul(2));
				oneNftRewardAmount = oneNftRewardAmount.add(amount.div(10000));
		...
		if(from == uniswapV2Pair){
			...
			if(inviter[to] != address(0)){
                if(SCORE.balanceOf(address(this)) > amount){
                    SCORE.transfer(inviter[to],amount);
                }
			}
		}
	...
	function getNftId(address user) public view returns(uint256){
		uint256[] memory list = nftWarp._tokensOfOwner(user);
	...
	function withdrawNftReward(address user) private {
		...
		if(nodeNftIdFirst < 100){
			uint256 rewardAmountCanWithdraw = oneNftRewardAmount.sub(nftRewardSendAmount[nodeNftIdFirst]);
			if(rewardAmountCanWithdraw > 10**17){
				oneNftRewardOverAmount = oneNftRewardOverAmount.add(rewardAmountCanWithdraw);
				nftRewardSendAmount[nodeNftIdFirst] = nftRewardSendAmount[nodeNftIdFirst].add(rewardAmountCanWithdraw);
	...
	function swapAndLiquify() private {
		...
		if(oneNftRewardAmount.mul(100) > oneNftRewardOverAmount){
			uint256 nftRewardRemain = oneNftRewardAmount.mul(100).sub(oneNftRewardOverAmount);
			allAmount = allAmount.sub(nftRewardRemain);
		}
		if(allAmount > 5 * 10**18){
			uint256 canswap = allAmount.div(4).mul(3);
			uint256 oldBalance = USDT.balanceOf(address(this));
			swapTokensForOther(canswap);
			uint256 usdtBalance = USDT.balanceOf(address(this));
			if(usdtBalance > 10**15){
				uint256 newAmount = usdtBalance.sub(oldBalance);
				addLiquidityUsdt(newAmount.div(3), canswap.div(4));
			}
		}
    }
