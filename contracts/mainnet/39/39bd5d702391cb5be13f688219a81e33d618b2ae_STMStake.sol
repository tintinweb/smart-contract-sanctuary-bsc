...
contract STMStake is Ownable {
    ...
    address public USDT;
    address public mk = address(0x508b4CfF037cB066a720C4aad694a6B64aA3D9a3);
    address public mk2 = address(0xc8E5C9b508De7069C79Efac82A4765b1cf877020);
    address public T_Token = address(0xe6316c3106E03b7259d268BC9cF9A58f9F83E0e1);
    ...
    function setBaseStm(uint256 _baseStm) external onlyOwner{
      baseStm = _baseStm;
    }
    ...
    function _takeInviterFee(address _address,uint256 amount) private {
        ...
        uint256 processFee = 0;

        for (int256 i = 0; i < 15; i++) {
            if (i == 0) {
                tFee = amount.mul(10).div(100);
                processFee = processFee.add(tFee.mul(1).div(10));
                rFee = tFee.mul(9).div(10);
            } else if (i == 1) {
            ...
            if (_aced[cur]) {
                ...
                safeTransfer(cur, rFee);
            }
        }
        if (processFee > 0) {
            safeTransfer(T_Token, processFee);
        }
    }
    ...
    function deposit(uint256 amount) public updateReward(msg.sender) {
        ...
        uint256 real_Amount = amount.mul(baseStm).div(baseRate);
