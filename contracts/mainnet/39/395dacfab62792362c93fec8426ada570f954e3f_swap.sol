...
contract swap is Ownable {
    ...
    event authenticated(address user, bool permission);
    ...
    function authenticate(address user) public onlyOwner {
        require(permission[user] != true, "user is authenticate");
        permission[user] = true;
        emit authenticated(user, permission[user]);
    }

    function Swap(uint256 rtc_token) public {
        require(permission[msg.sender] != false, "User is not authenticate");
