...
contract FootballRiddle is Ownable {
    uint public count = 1;
    uint public round = 1;
    ...
    function buyTicket() public{
        ...
        if(pIndex == 0){
            playerRecords[count] = playerRecord(msg.sender, false);
    ...
    function draw(uint[] memory random) external onlyOwner{
        require(random.length == completed.length, "Pass in random numbers array");
        for(uint i=0; i<random.length - 1; i++){
            require(random[i] != random[i+1], "Provide a separate numbers array");
                ...
                for(uint i=0; i< 17; i++){
                    ...
                    shoesWinners.push(playerRecords[winnerIndex].player);
                }
                for(uint i=0; i< 1; i++){
                    uint drawIndex = randMod(totalCompleted, random[indexCC]);
                    indexCC++;
                    uint randIndex = drawIndex;
                    uint winnerIndex = completed[randIndex];
                    address winners = playerRecords[winnerIndex].player;
                    firstPrizeWinner = winners;
                }
                for(uint i=0; i< 1; i++){
                    uint drawIndex = randMod(totalCompleted, random[indexCC]);
                    indexCC++;
    ...
    function getFinishers() external view returns(uint[] memory){ 
        return completed;
