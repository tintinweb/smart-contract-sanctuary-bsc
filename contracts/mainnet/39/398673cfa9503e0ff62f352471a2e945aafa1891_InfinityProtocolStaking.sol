...
contract InfinityProtocolStaking is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    IUniswapV2Router02 public routerAddress;
    address[] public swapPath = [0x2ae6852A408E7A004830888ea79d4cD0BBF03CE0,0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c,0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56];
    uint public baseTime = 1 days; 
    ...
    IERC20 public token;
    IERC20 public rewardToken = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
    ...
    mapping(address => uint256) public totalRewardEarnBUSD;
    ...
    constructor(
       ...
       address _routerAddress
        ...
        pooldata[30].returnPer = 1500; // 15%

        //90 days
        pooldata[90].lockupDuration = 90;
        pooldata[90].returnPer = 4500; // 45%
        ...
        pooldata[365].returnPer = 18500; // 185%
    ...
    function setSwapPath(address[] memory _swapPath) public onlyOwner{
        require(_swapPath.length > 2 , "Address length is invalid!!");
        swapPath = _swapPath;
    }
    ...
    function setBaseTime(uint _baseTime) external onlyOwner{
        require(_baseTime > 0 , "basetime is zero!!");
        baseTime = _baseTime;
    }

   function pendingRewards(uint256 _orderId ) external view returns (uint) {
        ...
        if(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed){
            ...
            if(block.timestamp >= orderInfo.endtime){
                ...
                uint256 claimAvailable =  reward.sub(orderInfo.claimedReward).add(orderInfo.amount);
                (uint[] memory amounts)  = routerAddress.getAmountsOut(claimAvailable, swapPath);
            ...
            uint256 claimAvailableNow =  totalReward.sub(orderInfo.claimedReward).add(orderInfo.amount);
            (uint[] memory amount)  = routerAddress.getAmountsOut(claimAvailableNow, swapPath);
    ...
    function withdraw(uint256 orderId) external nonReentrant{
        ...
        uint getBusd = _swap(total);
        orderInfo.claimedBUSD += getBusd;
        totalRewardEarnBUSD[msg.sender] = totalRewardEarnBUSD[msg.sender].add(getBusd);
        rewardToken.transfer(address(msg.sender), getBusd );
    ...
    function withdrawBNB(address payable _reciever, uint256 _amount) external onlyOwner {
    ...
    function getAmountOut(uint256 _tokenAmount , uint256 _lockupDuration) external view returns(uint){
        PoolInfo storage pool = pooldata[_lockupDuration];
        uint256 userReward = (_tokenAmount.mul(pool.returnPer).mul(_lockupDuration)).div(10000*365);
        uint256 totalToken = userReward.add(_tokenAmount);
       (uint[] memory amounts)  = routerAddress.getAmountsOut(totalToken, swapPath);
       return amounts[amounts.length-1];
    }

    function _swap(uint256 tokenAmount) internal returns(uint){
        uint deadline = block.timestamp + 500;
        uint256 balanceBefore = rewardToken.balanceOf(address(this));
        IERC20(token).approve(address(routerAddress) , tokenAmount);
        routerAddress.swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAmount, 0, swapPath , address(this) , deadline);
        uint256 recAmount = rewardToken.balanceOf(address(this)) - (balanceBefore);
        return recAmount;
    }
