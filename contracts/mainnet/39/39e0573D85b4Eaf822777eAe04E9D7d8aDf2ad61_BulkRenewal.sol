...
import "./BNBRegistrarControllerV2.sol";
...
contract BulkRenewal is IBulkRenewal {
    ...
    function getController() internal view returns (BNBRegistrarControllerV2) {
        Resolver r = Resolver(sid.resolver(BNB_NAMEHASH));
        return
            BNBRegistrarControllerV2(
    ...
    {
        BNBRegistrarControllerV2 controller = getController();
...
contract BNBRegistrarControllerV2 is Ownable, EIP712 {
    ...
    function valid(string memory name) public pure returns(bool) {
        // zero width for /u200b /u200c /u200d
        bytes memory nb = bytes(name);
        for(uint i; i < nb.length-2; i++) {
          if(bytes1(nb[i]) == 0xe2 && bytes1(nb[i+1]) == 0x80 && bytes1(nb[i+2]) == 0x8b) {
            return false;
          } else if(bytes1(nb[i]) == 0xe2 && bytes1(nb[i+1]) == 0x80 && bytes1(nb[i+2]) == 0x8c) {
            return false;
          } else if(bytes1(nb[i]) == 0xe2 && bytes1(nb[i+1]) == 0x80 && bytes1(nb[i+2]) == 0x8d) {
            return false;
          }
        }
