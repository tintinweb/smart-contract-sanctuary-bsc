...
contract FarmContract1207 is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getUserDeservedReferralReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 deservedReferralReward){
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            uint256 mintedAmount = pendingMiningReward(_pid, InviteeAddress[i]).add(mintAmount[_pid][InviteeAddress[i]]);
            deservedReferralReward = deservedReferralReward.add(mintedAmount);
        }
    }

    function getUserDeservedNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 deservedNodeReward){
        address[] memory InviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
    ...
    function getAllPendingReferralReward(address _user) public view returns(uint256 totalReferralBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if(getUserDeservedReferralReward(i,_user) > userCalculatedReferralRewards[i][_user]) {
                uint256 pendingReward = getUserDeservedReferralReward(i,_user).sub(userCalculatedReferralRewards[i][_user]);
    ...
    function getAllPendingNodeReward(address _user) public view returns(uint256 totalNodeBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            if(getUserDeservedNodeReward(i,_user) > userCalculatedNodeRewards[i][_user]) {
                uint256 pendingReward = getUserDeservedNodeReward(i,_user).sub(userCalculatedNodeRewards[i][_user]);
                uint256 nodeBonus = pendingReward.mul(getNodeRewardPercent(i, _user)).div(rateDenominator);
    ...
    function claimAllReferralReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 refferalReward = getUserDeservedReferralReward(i, _msgSender()).sub(userCalculatedReferralRewards[i][_msgSender()]);
    ...
    function claimAllNodeReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 nodeReward = getUserDeservedNodeReward(i, _msgSender()).sub(userCalculatedNodeRewards[i][_msgSender()]);
    ...
    function getOneStarRequiredLP(uint256 pid) public view returns(uint256) {
        ...
        uint256 amountOut = IPancakeRouter02(Router).getAmountOut(oneStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(oneStarUserpledgeThreshold.mul(amountOut));
    }

    function getTwoStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 amountOut = IPancakeRouter02(Router).getAmountOut(twoStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(twoStarUserpledgeThreshold.mul(amountOut));
    }

    function getThreeStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 amountOut = IPancakeRouter02(Router).getAmountOut(threeStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(threeStarUserpledgeThreshold.mul(amountOut));
    }
