...
contract SpyritToken is BEP20, MinterRole {
    ...
    //total 6% : 3% marketing wallet, 2% dev, 1% burn
    uint256 public markFee = 30;
    uint256 public devFee = 20;   
    uint256 public burnFee = 10;
    uint256 public taxFee = markFee + devFee + burnFee;
    // Supply capped at 5B
    uint256 public cap = 5000000000 * 10 ** 18;
    // marketing and dev Fee Wallet address
    ...
    event UpdateFeeWallets(address indexed devAddress, address indexed markAddress);
    event UpdateMarkFee(uint256 markFee);
    event UpdateDevFee(uint256 devFee);
    event UpdateBurnFee(uint256 burnFee);


    /**z
     * @notice Constructs the SPY token contract.
     */
    constructor(address devAddress_, address markAddress_) public BEP20("Spyrit", "SPYRIT") {
        ...
        require(devAddress_ != address(0), "wallet address is zero");
        devAddress = devAddress_;

        require(markAddress_ != address(0), "wallet address is zero");
        markAddress = markAddress_;

        setExcludeFromFee(msg.sender, true);
        setExcludeFromFee(address(this), true);
        setExcludeFromFee(devAddress, true);
        setExcludeFromFee(markAddress, true);
        setExcludeFromFee(BURN_ADDRESS, true);
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyMinter {
        require(spySupply().add(_amount) <= cap, "SPY: cap exceeded");
        _mint(_to, _amount);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            require(tradingOpen == true, "SPY: Trading is not yet open.");

            uint256 sendAmount = amount;
            if(automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient] ) {
                uint256 taxAmount = amount.mul(taxFee).div(1000);
                uint256 markFeeAmount = amount.mul(markFee).div(1000);
                uint256 devFeeAmount = amount.mul(devFee).div(1000);
                uint256 burnAmount = taxAmount.sub(markFeeAmount).sub(devFeeAmount);
                require(taxAmount == burnAmount.add(devFeeAmount).add(markFeeAmount), "SPY:transfer: DevFeeAmount or MarkFeeAmount value invalid");

                // default 94% of transfer sent to recipient
                sendAmount = amount.sub(taxAmount);
                require(amount == sendAmount.add(taxAmount), "SPY::transfer: Tax value invalid");

                // Distributing SPYRIT
                super._transfer(sender, markAddress, markFeeAmount);
    ...
    /** 
     * @dev Update the Fee wallet Addresses.
     * Can only be called by owner.
     */
    function updateFeeWallets(address _devAddress, address _markAddress) public onlyOwner {
        require( _devAddress != address(0), "SPY:: Dev wallet address is zero" );
        require( _markAddress != address(0), "SPY:: Marketing wallet address is zero" );
        devAddress = _devAddress;
        markAddress = _markAddress;
        emit UpdateFeeWallets(_devAddress, _markAddress);
    }

    function updateMarkFee(uint256 _markFee) public onlyOwner {
        require( _markFee <= 200, "SPY:: Fee can't exceed 20%" );
        markFee = _markFee;
        taxFee = markFee + devFee + burnFee;
        emit UpdateMarkFee(_markFee);
    }

    function updateDevFee(uint256 _devFee) public onlyOwner {
        require( _devFee <= 200, "SPY:: Fee can't exceed 20%" );
        devFee = _devFee;
        taxFee = markFee + devFee + burnFee;
        emit UpdateDevFee(_devFee);
    }

    function updateBurnFee(uint256 _burnFee) public onlyOwner {
        require( _burnFee <= 200, "SPY:: Fee can't exceed 20%" );
        burnFee = _burnFee;
        taxFee = markFee + devFee + burnFee;
        emit UpdateBurnFee(_burnFee);
    }

    function spySupply() public view returns (uint256) {
