...
contract StakePool is
    Ownable,
    ReentrancyGuard,
    ERC20("VISA Pools", "BUSD-VISA")
{
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public VISA_CASTLE_FACTORY;
    ...
    // The block number when VisaMetafi mining ends.
    uint256 public bonusEndBlock;

    // The block number when VisaMetafi mining starts.
    ...
    // VisaMetafi tokens created per block.
    ...
    constructor() public {
        VISA_CASTLE_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == VISA_CASTLE_FACTORY, "Not factory");
        require(
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function withdrawNew(bool isHarvest) external nonReentrant {
        uint256 _amount = 0;
    ...
    // function emergencyWithdrawStakedToken(uint256 _amount)
    //     external
    //     onlyOwner
    // {
    //     if (_amount > 0) {
        ...
        returns (uint256[] memory, ERC20[] memory)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                tokenReward = multiplier.mul(rewardPerBlock[rewardTokens[i]]);
                adjustedTokenPerShare = accTokenPerShare[rewardTokens[i]].add(
                    tokenReward.mul(PRECISION_FACTOR[rewardTokens[i]]).div(
                        stakedTokenSupply
                    )
                );
        ...
        returns (uint256)
    {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 tokenReward = multiplier.mul(rewardPerBlock[_token]);
            uint256 adjustedTokenPerShare = accTokenPerShare[_token].add(
                tokenReward.mul(PRECISION_FACTOR[_token]).div(stakedTokenSupply)
            );
