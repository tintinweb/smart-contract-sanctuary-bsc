...
contract TreasuryV2 is ContractGuard {
    ...
    function initialize(
        ...
        supplyTiers = [0 ether, 250000 ether, 500000 ether, 1000000 ether, 2000000 ether, 4000000 ether, 5000000 ether, 10000000 ether, 25000000 ether];
        maxExpansionTiers = [250, 200, 150, 100, 75, 50, 25, 10, 5];
    ...
    function setPaper(address _paper) external onlyOperator {
        paper = _paper;
    }

    function setInk(address _ink) external onlyOperator {
        ink = _ink;
    }
    ...
    function setMaxExpansionTiersEntry(uint8 _index, uint256 _value) external onlyOperator returns (bool) {
        require(_index >= 0, "Index has to be higher than 0");
        require(_index < 9, "Index has to be lower than count of tiers");
        require(_value >= 1 && _value <= 1000, "_value: out of range"); // [0.01%, 10%]
    ...
    function setBridgeOperator(address _address, bool _enabled) external onlyBridgeAdmins {
        bridgeOperators[_address] = _enabled;
    }

    function setBridgeAdmin(address _address, bool _enabled) external onlyBridgeAdmins {
        bridgeAdmins[_address] = _enabled;
    }
