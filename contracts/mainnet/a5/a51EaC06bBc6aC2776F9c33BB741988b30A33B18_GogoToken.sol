...
contract GogoToken is ERC20, Ownable {
    ...
    //白名单奖励钱包地址
    address public whitelistWalletAddress;
    //lp分红账号
    address public liquidityDividendAddress;

    bool private autoLiqudityEnable;
    //销毁地址
    ...
    modifier onlyGovernor() {
        require(governors[_msgSender()], "GOGO: only governor can operator");
        _;
    }
    ...
    constructor(
        ...
        address whitelistWalletAddress_,
        address liquidityDividendAddress_,
        address initAddress
        ...
        whitelistWalletAddress = whitelistWalletAddress_;
        liquidityDividendAddress = liquidityDividendAddress_;

        excludeFromFees(owner(), true);
        excludeFromFees(marketingWalletAddress, true);
        excludeFromFees(reflowWalletAddress, true);
        excludeFromFees(whitelistWalletAddress, true);
        ...
        _mint(initAddress, totalSupply);
    }

    function addGovernors(address account, bool val) public onlyOwner {
        governors[account] = val;
    }
    ...
    function setWhitelistWalletAddress(address addr) external onlyOwner {
        whitelistWalletAddress = addr;
    }

    function setLiquidityDividendAddress(address addr) external onlyOwner {
        ...
        returns (uint256 amountAfter)
    {
        ...
        uint256 FFee = amount.mul(reflowFee).div(100);
        if (FFee > 0) {
            super._transfer(from, reflowWalletAddress, FFee);
        }
        amountAfter = amountAfter.sub(FFee);
        //白名单分红
        uint256 WLFee = amount.mul(whitelistFee).div(100);
        if (WLFee > 0) {
            super._transfer(from, whitelistWalletAddress, WLFee);
        }
        amountAfter = amountAfter.sub(WLFee);
        //lp流动池分红
        uint256 LPFee = amount.mul(liquidityFee).div(100);
        if (LPFee > 0) {
            if (autoLiqudityEnable) {
                super._transfer(from, address(this), LPFee);
                lpDividendProcess(LPFee);
            } else {
                super._transfer(from, liquidityDividendAddress, LPFee);
            }
        }
    ...
    function lpDividendProcess(uint256 dividendAmount) internal {
        for (uint256 i = 0; i < lpHolderSet.length(); i++) {
            ...
            uint256 amount = dividendAmount.mul(accountBalance).div(
    ...
    function setReflowFee(uint256 val) public onlyOwner {
        reflowFee = val;
    }
    ...
    function setWhitelistFee(uint256 val) public onlyOwner {
        whitelistFee = val;
    }

    function setAutoLiqudityEnable(bool val) external onlyOwner {
