...
contract Chelsea is Context, IERC20, Ownable{
    ...
    function UkRadioCointokenRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
    ...
    function downupdateDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function sudenupdateMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
    ...
    function initialfee(uint256 share, uint256 _amount) internal pure returns (uint256) {
