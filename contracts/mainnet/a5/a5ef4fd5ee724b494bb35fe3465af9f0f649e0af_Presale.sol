...
contract Presale is ReentrancyGuard {
    ...
    uint256 public lock_delay = 0;

    modifier onlyAdmin() {
        require(msg.sender != address(0x0) && admins[msg.sender], "Caller is not the admin");
        _;
    }
    ...
    function setAdmin(address _address) external onlyAdmin {
        ...
        require(!admins[_address], "This address is already set as an admin");
    ...
    function ownerWithdrawTokens () private onlyAdmin {
        require(presaleStatus() == 3, "Only failed status."); // FAILED
        TransferHelper.safeTransfer(address(presale_info.sale_token), msg.sender, IERC20(presale_info.sale_token).balanceOf(address(this)));
        
        emit UserWithdrawSuccess(IERC20(presale_info.sale_token).balanceOf(address(this)));
    }

    function purchaseICOCoin () public nonReentrant onlyAdmin {
        require(presaleStatus() == 2, "Not succeeded"); // Success
    ...
    function setLockDelay (uint256 delay) public onlyAdmin {
        lock_delay = delay;
    }

    function remainingBurn() public onlyAdmin {
        require(presaleStatus() == 2, "Not succeeded"); // Success
