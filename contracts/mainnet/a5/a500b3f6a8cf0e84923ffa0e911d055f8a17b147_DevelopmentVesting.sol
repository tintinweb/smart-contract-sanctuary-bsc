...
contract DevelopmentVesting is Ownable {
	...
	/* the balances (in INERY) of all development members */
	mapping(address => uint256) public balanceOfINERY;

	/* the whitelist of the Development Vesting */
	...
	/**
	 * @dev
	 * Transfers vested tokens to address.
	 */

    function submit(address addr) external afterClosed {

		require(whitelist[addr], "Caller is not whitelisted");

		require(balanceOfINERY[addr] > 0, "Non-contribution");

		uint256 unreleasedAmount = _releasableAmount(addr);
		require(unreleasedAmount > 0, "All-claimed");
		
		uint256 balance = tokenReward.balanceOf(address(this));
		require(balance >= unreleasedAmount, "Lack-of-funds");

		releasedAmount[addr] = releasedAmount[addr].add(unreleasedAmount);

        tokenReward.transfer(addr, unreleasedAmount);
    }
