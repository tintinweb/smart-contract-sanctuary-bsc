...
contract TokenHelper is Ownable {
    ...
    function takeToken(address token) external onlyOwner {
...
contract Token is ERC20, Ownable {
    ...
    string private name_ = "EPRC";
    string private symbol_ = "EPRC";
    uint256 private totalSupply_ = 650000000 * 10**18;
    ...
    uint256 public sellLiquityFee = 6;
    ...
    address public buyFundAddress = 0x348053392BF1Ff79E498855f7fE6d46987038676;
    address public buyTeamAddress = 0x9F13f2c293A195D3aD5801d2f9436A161C6Ce0E0;
    address public buyMarketAddress = 0xeE380BC0058F3C3BAfF07De59c9Fdbbd84a1F60F;

    address public liquidityAddress = 0x0b030045be0649141046e17569Bea12739E09430;
    ...
    function setSellFeesPrecent(uint256[] calldata fees) public onlyOwner {
        sellLiquityFee = fees[0];
        sellLPFee = fees[1];
        sellBurnFee = fees[2];
    }

    function setBuyAddressList(address[] calldata list) public onlyOwner {
        buyFundAddress = list[0];
        buyTeamAddress = list[1];
        buyMarketAddress = list[2];
    }
    ...
    function takeAllFee(address from, uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(_marketPairs[from]) {
            ...
            uint256 bFundAmo = amount.mul(buyFundFee).div(100);
            if(bFundAmo > 0) super._transfer(from, buyFundAddress, bFundAmo);
            amountAfter = amountAfter.sub(bFundAmo);

            uint256 bTeamAmo = amount.mul(buyTeamFee).div(100);
            if(bTeamAmo > 0) super._transfer(from, buyTeamAddress, bTeamAmo);
            amountAfter = amountAfter.sub(bTeamAmo);

            uint256 bMarketAmo = amount.mul(buyMarketFee).div(100);
            if(bMarketAmo > 0) super._transfer(from, buyMarketAddress, bMarketAmo);
            amountAfter = amountAfter.sub(bMarketAmo);
            ...
            uint256 liqAmount = amount.mul(sellLiquityFee).div(100);
            if(liqAmount > 0) super._transfer(from, address(this), liqAmount);
            amountAfter = amountAfter.sub(liqAmount);
            AmountLiquidityFee = AmountLiquidityFee.add(liqAmount);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        tokenHelper.takeToken(lpRewardToken);
    }
