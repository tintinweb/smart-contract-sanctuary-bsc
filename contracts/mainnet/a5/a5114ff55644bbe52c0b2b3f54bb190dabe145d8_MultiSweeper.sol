...
import "./IMasterSweeper.sol";
import "./utils/Recoverable.sol";

contract MultiSweeper is Recoverable {

    event SweepFailed(address indexed market, address indexed collection, uint256 tokenId, uint256 price, bytes reason);
    event MultiSweep(uint256 success, uint256 total);
    ...
    function sweep(
        address _masterSweeper,
        SweepData[] calldata _sweepdata
    ) external {
        require(
        ...
        require(_sweepdata.length > 0, "No data provided");

        IMasterSweeper master = IMasterSweeper(_masterSweeper);
        uint256 maxAge = master.signatureMaxAge();


        require(
            _sweepdata[0].timestamp + maxAge > block.timestamp,
            "Signature expired"
        );

        uint256 success;
        for (uint256 i = 0; i < _sweepdata.length; ++i) {
            SweepData memory data = _sweepdata[i];
            try master.sweep(data.market, data.collection, data.tokenId, data.price, data.timestamp, data.signature) {
                ++success;
            } catch (bytes memory reason) {
                emit SweepFailed(data.market, data.collection, data.tokenId, data.price, reason);
            }
        }

        require(success > 0, "All sweeps failed");

        emit MultiSweep(success, _sweepdata.length);
    }
