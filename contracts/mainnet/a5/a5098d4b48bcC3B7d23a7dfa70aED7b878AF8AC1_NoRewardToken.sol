...
contract NoRewardToken is ERC20, BigBase, BigBox4fee, BigLimiter, BigSystem, UniSwapModule, ReentrancyGuard {
    ...
    uint256 public swapTokensAtEther = 30 ether;
    ...
    constructor(
        ...
        address[] memory _conf, // routerContractAddress, usd(pairB)
        address[] memory _addrs // marketingAddress
    ) ERC20(_name, _symbol) {
        ...
        _approve(_addrs[0], address(uniswapV2Router), type(uint256).max);
    }

    function _totalSupply_init(uint256 _totalSupply) internal virtual {
        _cast(_msgSender(), _totalSupply);
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _cast(address account, uint256 amount) internal virtual {_mint(account, amount);}
