...
contract DarkHorse is IBEP20, Ownable {
    ...
    // Basic Contract Info
    string private constant _name = "Dark Horse"; // TO DO -- ADD NAME
    string private constant _symbol = "DHorse"; // TO DO -- ADD SYMBOL
    ...
    // Transaction Limits
    uint256 private _maxTxAmount = _tTotal; // Max transaction: 10_000_000 -- Owner can set it between 0.2 and 2%
    uint256 private _maxWalletSize = _tTotal; // Max Wallet: 10_000_000 -- Owner can set it between 0.2 and 2%
    ...
    // Fee Receiver
    address public marketingFeeReceiver = 0xdfb8102610D7263796dd5D5EbD0B7387460b5FBC; // Marketing Address -- TO DO -- ADD MARKETING WALLET
    ...
    uint256 public swapThreshold = _tTotal / 10000 * 5; // 0.05% - Minimum number of tokens needed to make the swap
...
/* To set MaxWallet or MaxTransaction only need to add the number of tokens without decimals -- 0.2% = 2_000_000 -- 2% = 20_000_000 -- Underscore used for easier read */
    ...
    function setMaxWallet(uint256 _maxWallet) external onlyOwner {
        require(_maxWallet * (10 **_decimals) >= _tTotal / 500 &&
                _maxWallet * (10 **_decimals) <= _tTotal, "Can't set MaxWallet lower than 0.2% or higher than 2%");
    ...
    function setMaxTX(uint256 _maxTX) external onlyOwner {        
        require(_maxTX * (10 **_decimals) >= _tTotal / 500 &&
                _maxTX * (10 **_decimals) <= _tTotal, "Can't set MaxWallet lower than 0.2% or higher than 2%");
