...
contract LTToken is Context, IERC20, Ownable {
    ...
    uint[] shareConfig = [35,15];
    uint public shareFee = 50;
    ...
    function setSwapSwitch(bool _swapSwitch)external onlyOwner{
        swapSwitch = _swapSwitch;
    }

    function setBlackList(address owner,bool isBlack)external onlyOwner{
        _isBlackList[owner] = isBlack;
    }

    function setBuyFee(uint ___buyBurnFee,uint ___buyLpFee)external onlyOwner{
        _buyBurnFee = ___buyBurnFee;
        _buyLpFee = ___buyLpFee;
        totalBuyFee = ___buyBurnFee + ___buyLpFee;
        require(totalBuyFee < 1000,"error");
    }

    function setSellFee(uint __sellBurnFee,uint __sellLpFee)external onlyOwner{
        _sellBurnFee = __sellBurnFee;
        _sellLpFee = __sellLpFee;
        totalSellFee = __sellBurnFee + __sellLpFee;
        require(totalSellFee < 1000,"error");
    }

    function setShareFee(uint _shareFee,uint[] memory _shareConfig)external onlyOwner{
        shareFee = _shareFee;
        ...
        require( sum < 100 && sum == shareFee,"error");
    }
    ...
    function _transfer(
        ...
        require( !_isBlackList[from] && !_isBlackList[to],"is black list");
        ...
        if( !swapSwitch){
            require( isAddLiquidity || (!ammPairs[to] && !ammPairs[from]),"swap not start");
        }
    ...
    function processEst() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            if (amount >= lpInterest.minAward) {
                ...
                uint shareAmount = amount * shareFee / 100;
                uint sended = _takeShare(shareAmount,address(this),shareholder);

                if( amount > sended ){
                    uint selfAmount = amount - sended;
                    _doTransfer(address(this),shareholder,selfAmount);
                }
            }
            sendedCount ++;
            iterations++;
    ...
    function _takeShare(uint tShare,address from,address user) private returns(uint sended){
        ...
        for( uint i = 0; i < len; i++ ){
            ...
            uint tv = tShare * shareConfig[i] / shareFee;
            _doTransfer(from,parent,tv);
            sended += tv;
        }  
    }
