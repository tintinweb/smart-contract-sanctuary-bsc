    ...
    ReentrancyGuard
{
    ...
    function setAnitoRarities(
        ...
        for (uint256 i = 0; i <= _anitoRarities.length; i++) {
            anitoRarities[i + 1] = _anitoRarities[i + 1];
        }
    }
    ...
    function airdropAnito(
        ...
        require(
            anitoNew.ownerOf(fromTokenId) == address(0) &&
                anitoNew.ownerOf(toTokenId) == address(0),
            "Already airdroped"
        );
    ...
    function airdropDuende(
        ...
        require(
            duendeNew.ownerOf(fromTokenId) == address(0) &&
                duendeNew.ownerOf(toTokenId) == address(0),
            "Already airdroped"
        );

        for (uint256 i = 0; i <= toTokenId; i++) {
            duendeNew.safeMintSingle(duende.ownerOf(fromTokenId + i));
        }
    }
}
