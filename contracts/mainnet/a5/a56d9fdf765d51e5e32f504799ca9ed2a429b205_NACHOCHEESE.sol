...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal {
        if(sender == pair && cooldownEnabled && !_isFeeExempt[recipient] && _balances[recipient].div(_PerFragment) <= 1 && !isCont(recipient)){
            cooldownTimer[recipient] = block.timestamp + cooldownTimerInterval;
            lastTxTime[recipient] = block.timestamp;}
        if(sender != pair && cooldownEnabled && !_isFeeExempt[sender] && swapTime[sender] < block.timestamp) {
    ...
    function viewCooldownLeft(address _address) public view returns (uint256) {
        ...
        uint256 time; if(amt > 0){time = amt;} if(amt <= 0){time = 0;}
        return time;
    }
