...
/** @title SantaRabbit Universe Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract SantaRabbitLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
        ...
        nonReentrant
    {
        ...
        // Calculate number of SantaX to this contract
        ...
        // Transfer SantaX tokens to this contract
        santaX.safeTransferFrom(address(msg.sender), address(this), amountCakeToTransfer);
    ...
    function claimTickets(
        ...
        santaX.safeTransfer(msg.sender, rewardInCakeToTransfer);
    ...
    /**
     * @notice Draw the final number, calculate reward in SantaX per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @param _autoInjection: reinjects funds into next lottery (vs. withdrawing all)
     * @dev Callable by operator
     */
        ...
        nonReentrant
    {
        ...
        // Calculate prizes in SantaX for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // A. No SantaX to distribute, they are added to the amount to withdraw to treasury address
            } else {
        ...
        // Transfer SantaX to treasury address
        santaX.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in SantaX token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        santaX.safeTransferFrom(address(msg.sender), address(this), _amount);
     ...
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInCake: price of a ticket in SantaX
     * @param _discountDivisor: the divisor to calculate the discount magnitude for bulks
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     * @param _treasuryFee: treasury fee (10,000 = 100%, 100 = 1%)
     */
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(santaX), "Cannot be SantaX token");
    ...
    /**
     * @notice Set SantaX price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInCake: minimum price of a ticket in SantaX
     * @param _maxPriceTicketInCake: maximum price of a ticket in SantaX
     */
     ...
     * @param _priceTicket price of a ticket (in SantaX)
