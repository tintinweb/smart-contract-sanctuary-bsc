   ...
   */
    ...
    */
        ...
        returns (uint256)
    {
        ...
        PoolInfo memory pool = poolInfo[poolId];
        uint256 amount;
        if (compareStringsbyBytes(poolId, "622206381a068d7f60f5b777") || compareStringsbyBytes(poolId, "6222042c1a068d7f60f5b75f")) {
            amount = item.balance * (rewardPerToken(poolId) - item.rewardPerTokenPaid) / 1e18 + item.reward;
        } else {
            amount = item.balance * (rewardPerToken(poolId) - item.rewardPerTokenPaid) / 1e8 + item.reward;
        }
    ...
    */
    function rewardPerToken(string memory poolId) public view returns (uint256) {
        ...
        uint256 rewardPool;
        if (compareStringsbyBytes(poolId, "622206381a068d7f60f5b777") || compareStringsbyBytes(poolId, "6222042c1a068d7f60f5b75f")) {
            rewardPool = pool.initialFund * (currentTimestamp - pool.lastUpdateTime) * 1e18;
        } else {
            rewardPool = pool.initialFund * (currentTimestamp - pool.lastUpdateTime) * 1e8;
        }
