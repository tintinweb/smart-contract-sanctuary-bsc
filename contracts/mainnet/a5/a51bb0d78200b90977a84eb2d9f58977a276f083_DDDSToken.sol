...
abstract contract MarketFee is Owned, ERC20 {
    ...
    function _takeMarketing(
        ...
        if (sender == uniswapV2Pair) {
            ...
            uint256 donateAmount = (amount * donateFee) / 100;
            super._transfer(sender, donateAddr, donateAmount);

            uint256 m1Amount = (amount * m1Fee) / 100;
            super._transfer(sender, buyMarketAddr904, m1Amount);

            uint256 m2Amount = (amount * m2Fee) / 100;
            super._transfer(sender, buyMarketAddr354, m2Amount);

            return burnAmount + donateAmount + m2Amount + m1Amount;
        } else {
            uint256 burnAmount = (amount * burnFee) / 100;
            super._transfer(sender, address(0xdead), burnAmount);
            ...
            uint256 sm2Amount = (amount * sellMarketFee) / 100;
            super._transfer(sender, sellMarketAddr5aF, sm2Amount);

            return burnAmount + donateAmount + sm2Amount;
        }
    }
}

contract DDDSToken is ExcludedFromFeeList, MarketFee, LiquidityFeeUSDT {
    ...
    constructor()
        ...
        LiquidityFeeUSDT(2, _totalSupply / 50_0000, true)
    {
        ...
        returns (uint256)
    {
        ...
        return amount - liquidityAmount - marketAmount;
    }
