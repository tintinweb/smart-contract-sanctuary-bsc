...
contract RarePassage is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard, Ownable {
    using Address for address;
    using SafeMath for uint256;

    // Token name
    string private constant _name = "Rare Passage";

    // Token symbol
    string private constant _symbol = "RAREPASSAGE";
    ...
    // mint NFT
    IERC721 public UnCommonPassage = IERC721(0x64b247384a91b8049598f3743C93928886B7F53F);
    ...
    function setUncommonPassage(address UnCommonPassage_) external onlyOwner {
        UnCommonPassage = IERC721(UnCommonPassage_);
    }
     ...
     */
    function mint(uint256[] calldata uncommonTokenIds) external {
        require(
            address(UnCommonPassage) != address(0),
            'Uncommon Not Paired'
        );
        require(
            tradingEnabled,
        ...
        uint len = uncommonTokenIds.length;
        ...
        for (uint i = 0; i < len;) {
            ...
            require(
                UnCommonPassage.ownerOf(uncommonTokenIds[i]) == msg.sender,
                'Caller Not Owner'
            );
            UnCommonPassage.transferFrom(msg.sender, address(this), uncommonTokenIds[i]);
            require(
                UnCommonPassage.ownerOf(uncommonTokenIds[i]) == address(this),
                'Contract Not Owner'
            );
