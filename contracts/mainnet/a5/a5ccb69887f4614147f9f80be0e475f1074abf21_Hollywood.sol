...
contract Hollywood is IERC20, Auth {
    
    using SafeMath for uint256;

    string _name = "Hollywood";
    string _symbol = "Hollywood";
    uint8 _decimals = 9;
    ...
    function DividendExemptHollywood(address holder) external onlyOwner {
	...
	function ifgetLastRebase(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
	...
	function ownergetSafeRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}	

	function launchedgetMaxHold(uint256 _maxHold) public view returns(uint256) {
