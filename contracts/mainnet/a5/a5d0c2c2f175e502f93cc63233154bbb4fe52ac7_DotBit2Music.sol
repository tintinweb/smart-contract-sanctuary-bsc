...
// File: contracts/DotBit2Music.sol
...
contract DotBit2Music is ERC721Enumerable, Ownable, AccessControl {

    using strings for *;

    using Counters for Counters.Counter;
    Counters.Counter public tokenId;
    uint256 private verificationId;
    ...
    enum NFTPurchaseStatus{ AVAILABLE, PENDING, VERIFIED, PURCHASED, SONGIFIED, REDEEMED }
    ...
    event LogMusicPassCreated(uint256 indexed musicPassId, address indexed ownerWalletAddress, string bitDomain, uint32 genre);
    event LogMusicNftCreated(uint256 indexed musicNftId, address indexed ownerWalletAddress, string bitDomain, string metadataUri);
    event LogSongifyCompleted(uint256 indexed musicPassId);
    event LogVerificationRequestSubmitted(uint256 indexed verificationId, address ownerWalletAddress, string bitDomain);
    event LogVerificationRequestCompleted(uint256 indexed verificationId, address ownerWalletAddress, string bitDomain);
    ...
    // Mapping from domain name to verification id
    mapping(string => uint256) private domainNameToVerificationId;

    // Mapping domain name to owner
    mapping(string => address) private domainNameToOwnerAddress;
    ...
    function verifyDidOwnership(string memory domainName) external {
        // Revert the transaction if the music pass has been purchased
        require(getPurchaseStatus(domainName) != NFTPurchaseStatus.VERIFIED, "This domain has been purchased.");
        ...
        setPurchaseStatus(domainName, NFTPurchaseStatus.PENDING);
        verificationId += 1;
        domainNameToVerificationId[domainName] = verificationId;
        emit LogVerificationRequestSubmitted(verificationId, msg.sender, domainName);
    }
    ...
    function purchaseMusicPass(string calldata domainName, uint32 genreId) external payable {
        ...
        require(getPurchaseStatus(domainName) == NFTPurchaseStatus.VERIFIED, "You are not the owner of this .bit domain.");
        require(msg.sender == domainNameToOwnerAddress[domainName],  "You are not the owner of this .bit domain.");
    ...
    function setDomainVerifyStatus(address[] memory ownerWalletAddressMap, string[] memory domainNameMap) public onlyMember {
        require(ownerWalletAddressMap.length == domainNameMap.length, "The number of domain and wallet address doesn't match.");
        for (uint i=0; i<domainNameMap.length; i++) {
            string memory domainName = domainNameMap[i];
            address walletAddress = ownerWalletAddressMap[i];
            if(getPurchaseStatus(domainName) != NFTPurchaseStatus.PENDING) {
                continue;
            }
            domainNameToOwnerAddress[domainName] = walletAddress;
            setPurchaseStatus(domainName, NFTPurchaseStatus.VERIFIED);
            emit LogVerificationRequestCompleted(domainNameToVerificationId[domainName], walletAddress, domainName);
        }
    }
    ...
    function checkStatus(string memory domainName) external view returns (string memory) {
        if(getPurchaseStatus(domainName) == NFTPurchaseStatus.PENDING) {
            return "PENDING";
        } else if(getPurchaseStatus(domainName) == NFTPurchaseStatus.VERIFIED) {
            return "VERIFIED";
        } else if(getPurchaseStatus(domainName) == NFTPurchaseStatus.PURCHASED) {
            return "PURCHASED";
