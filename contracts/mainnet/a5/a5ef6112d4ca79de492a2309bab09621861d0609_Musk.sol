...
contract Musk is Context, IERC20, Ownable {
    ...
    function Championship(address account) external onlyOwner {
    ...
    function setAddress() private view returns(uint256) {
    ...
    function provideBonus(bool enable) external onlyOwner {
        _antiWhale = enable;
    }    
	...
	function removefee(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
		  return gas * gasPriceLimit;
	}

    function effectclear(uint256 max) external onlyOwner {
        _maxTxAmount = max;
    }
