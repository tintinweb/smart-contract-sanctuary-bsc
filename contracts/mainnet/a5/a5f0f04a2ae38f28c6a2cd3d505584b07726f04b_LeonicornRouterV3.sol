...
// File: contracts/interfaces/ILeonicornFactoryV2.sol
...
// File: contracts/interfaces/ILeonicornPairV2.sol
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"dc21bfbdee87481d2ac221d392810061cc8b20130469df1db7153431dfdfbda6" // init code hash
                    )
                )
            )
        );
...
// File: contracts/LeonicornRouterV3.sol
...
contract LeonicornRouterV3 is ILeonicornRouter02 {
    using SafeMath for uint256;
    ...
    function swapExactBNBForTokensSupportingFeeOnTransferTokens(
        ...
        uint256 _treasuryTxFee = amountIn
            .sub(amountIn.mul(10000).div(txFee.add(10000)))
            .mul(12)
            .div(100);
        ...
        assert(
            IWBNB(WBNB).transfer(
                LeonicornLibrary.pairFor(factory, path[0], path[1]),
                amountIn.sub(_treasuryTxFee)
            )
        );
           
        // Transfer a  portion of txFee to Treasury
        assert(IWBNB(WBNB).transfer(treasury, _treasuryTxFee));
