...
contract INRD_StakePool_4 is Ownable,ReentrancyGuard {
    ...
    uint256 public poolRewardPercent = 16;
    ...
    function PoolStake(uint256  _amount) external nonReentrant returns (bool) {
        ...
        uint256 stakeRewards = (((_amount*poolRewardPercent)/100));
    ...
    function rewardsAccumulated(address _userAdd) external view returns(uint256){
        uint256 daysCompleted = ((block.timestamp).sub(users[_userAdd].pool_deposit_time)).div(1 days);
        if(block.timestamp > (users[_userAdd].pool_deposit_time + fullMaturityTime)){
            daysCompleted = totaldays;
        }
        uint256 amount = users[_userAdd].poolBal;
        return (((amount*poolRewardPercent)/100)/totaldays)*daysCompleted;
    }
    function rewardsAccumulatedAfterPenalty(address _userAdd) external view returns(uint256){
        uint256 daysCompleted = ((block.timestamp).sub(users[_userAdd].pool_deposit_time)).div(1 days);
        if(block.timestamp > (users[_userAdd].pool_deposit_time + fullMaturityTime)){
            daysCompleted = totaldays;
        }
        uint256 amount = users[_userAdd].poolBal;
        uint256 accumulatedReward = (((amount*poolRewardPercent)/100)/totaldays)*daysCompleted;
        return accumulatedReward.sub((accumulatedReward).mul(earlyPenalty).div(100));
    }
    ...
    function claimPool() external nonReentrant returns(bool){
        ...
        if(block.timestamp < (users[msg.sender].pool_deposit_time + fullMaturityTime)){
            accumulatedReward = (((amount*poolRewardPercent)/100)/totaldays)*daysCompleted;
