...
interface IMasterChefPancake {
    ...
    function lpToken(uint256 _index) external view returns (IBEP20 _token);

    function CAKE() external view returns (IBEP20 _CAKE);
}
...
// DUAL SIDED PANCAKE FARMER //

contract StrategyDualSidedPancake is BaseStrategyDualSided {
    using SafeERC20 for IERC20;
    ...
    {
        require(
            address(IMasterChefPancake(_chef).lpToken(_poolId)) == want,
            "!POOLID"
        );
    ...
    function _panic() internal override {
        IMasterChefPancake(chef).emergencyWithdraw(poolId);
    }
    ...
    function balanceOfPool() public view override returns (uint256) {
        return IMasterChefPancake(chef).userInfo(poolId, address(this)).amount;
    }

    // returns rewards unharvested
    function pendingRewards() public view override returns (uint256) {
        return IMasterChefPancake(chef).pendingCake(poolId, address(this));
    }
    ...
    function _setChefId(uint256 _poolId) internal override {
        require(
            address(IMasterChefPancake(chef).lpToken(_poolId)) == address(want),
            "wrong pool id"
        );
        poolId = _poolId;
    }
    ...
    function _deposit() internal override returns (uint256 wantBal) {
        ...
        if (wantBal > 0) {
            IMasterChefPancake(chef).deposit(poolId, wantBal);
        }
    }
    ...
    function _withdraw(uint256 _amount) internal override returns (uint256) {
        ...
        if (wantBal < _amount) {
            ...
            IMasterChefPancake(chef).withdraw(poolId, toWithdraw);
            return Math.min(_amount, IERC20(want).balanceOf(address(this)));
        }
    }

    function _claimRewards() internal override {
        IMasterChefPancake(chef).deposit(poolId, 0);
    }
    ...
    function _prepareRetirement() internal override {
        ...
        if (balanceOfPool() != 0) {
            try
                IMasterChefPancake(chef).withdraw(poolId, balanceOfPool())
            {} catch {
                IMasterChefPancake(chef).emergencyWithdraw(poolId);
            }
        }
