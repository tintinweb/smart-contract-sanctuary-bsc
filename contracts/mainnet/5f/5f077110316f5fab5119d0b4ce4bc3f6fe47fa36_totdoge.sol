...
contract totdoge is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _TrickOrTreatPoolFee, uint256 _GrandBonanzaFeeFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _TrickOrTreatPoolFee, uint256 _GrandBonanzaFeeFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _MarketingReceiver,address _TrickOrTreatPoolFeeReceiver, address _GrandBonanzaFeeFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "Trick or Treat Doge";
    string constant private _symbol = "TOTDOGE";
    ...
    uint256 private TrickOrTreatPoolFee = 2;
    uint256 private GrandBonanzaFeeFee = 1;
    uint256 private BurnFee = 0;

    //sellFees
    uint256 private sellFeeLiquidity = 2;
    uint256 private sellFeeMarketing = 6;
    uint256 private sellFeeTrickOrTreatPool = 2;
    uint256 private sellFeeGrandBonanzaFee = 1;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(MarketingFee).add(TrickOrTreatPoolFee).add(GrandBonanzaFeeFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeTrickOrTreatPool).add(sellFeeGrandBonanzaFee).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver = msg.sender ;
    address private MarketingFeeReceiver =msg.sender;
    address private TrickOrTreatPoolFeeReceiver =msg.sender;
    address private GrandBonanzaFeeFeeReceiver =msg.sender;
    ...
    function clearStuckBalance() external {
        payable(GrandBonanzaFeeFeeReceiver).transfer(address(this).balance);
    }

    function rescueERC20(address tokenAddress, uint256 amount) external returns (bool) {
        return ERC20(tokenAddress).transfer(GrandBonanzaFeeFeeReceiver, amount);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHTrickOrTreatPool = amountETH.mul(TrickOrTreatPoolFee.add(sellFeeTrickOrTreatPool)).div(totalETHFee);
        uint256 amountETHGrandBonanzaFee = amountETH.mul(GrandBonanzaFeeFee.add(sellFeeGrandBonanzaFee)).div(totalETHFee);

        (bool tmpSuccess,) = payable(MarketingFeeReceiver).call{value: amountETHMarketing}("");
        (tmpSuccess,) = payable(TrickOrTreatPoolFeeReceiver).call{value: amountETHTrickOrTreatPool}("");
        (tmpSuccess,) = payable(GrandBonanzaFeeFeeReceiver).call{value: amountETHGrandBonanzaFee}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _MarketingFee, uint256 _TrickOrTreatPoolFee, uint256 _GrandBonanzaFeeFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        MarketingFee = _MarketingFee;
        TrickOrTreatPoolFee = _TrickOrTreatPoolFee;
        GrandBonanzaFeeFee = _GrandBonanzaFeeFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_MarketingFee).add(_TrickOrTreatPoolFee).add(_GrandBonanzaFeeFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _MarketingFee,_TrickOrTreatPoolFee, _GrandBonanzaFeeFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _MarketingFee,uint256 _TrickOrTreatPoolFee, uint256 _GrandBonanzaFeeFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _MarketingFee;
        sellFeeTrickOrTreatPool = _TrickOrTreatPoolFee;
        sellFeeGrandBonanzaFee = _GrandBonanzaFeeFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_MarketingFee).add(_TrickOrTreatPoolFee).add(_GrandBonanzaFeeFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _MarketingFee,_TrickOrTreatPoolFee, _GrandBonanzaFeeFee, _BurnFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _MarketingFeeReceiver,address _TrickOrTreatPoolFeeReceiver, address _GrandBonanzaFeeFeeReceiver, address _BurnFeeReceiver) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _MarketingFeeReceiver != address(0) && _TrickOrTreatPoolFeeReceiver != address(0) && _GrandBonanzaFeeFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        MarketingFeeReceiver = _MarketingFeeReceiver;
        TrickOrTreatPoolFeeReceiver = _TrickOrTreatPoolFeeReceiver;
        GrandBonanzaFeeFeeReceiver = _GrandBonanzaFeeFeeReceiver;
        BurnFeeReceiver = _BurnFeeReceiver; 

        emit SetFeeReceivers(_autoLiquidityReceiver, _MarketingFeeReceiver, _TrickOrTreatPoolFeeReceiver, _GrandBonanzaFeeFeeReceiver, _BurnFeeReceiver);
    }
