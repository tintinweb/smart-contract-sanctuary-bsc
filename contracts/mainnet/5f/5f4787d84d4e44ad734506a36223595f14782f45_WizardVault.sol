 ...
 */
contract WizardVault is ERC20, Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _shares) public {
        uint256 r = (balance().mul(_shares)).div(totalSupply());  // 100
        _burn(msg.sender, _shares);

        uint b = want().balanceOf(address(this));               // 90
        if (b < r) {
            uint _withdraw = r.sub(b);                
            strategy.withdraw(_withdraw);             // 10
            uint _after = want().balanceOf(address(this));     //     105
            uint _diff = _after.sub(b);           // 105 - 90. = 15
            if (_diff < _withdraw) {            //     
                r = b.add(_diff);                //  
            }
        }
