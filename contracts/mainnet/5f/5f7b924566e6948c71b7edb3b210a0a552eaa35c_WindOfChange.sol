...
contract WindOfChange is Context, IBEP20, Ownable {
    ...
    string  private constant _NAME = 'WindOfChange';
    ...
    uint256 private _WoCSwapRotations = 0;
    uint256 private _qTradeRotations = 0;
    uint256 private _qBurnRotations = 0;
    uint256 private transferredWoCSwap = 0;
    uint256 private countOfWoCSwap = 0;
    ...
    function _rebase(uint256 supplyDelta) internal {
        _WoCSwapRotations = _WoCSwapRotations.add(1);
        _qTotal = _qTotal.add(supplyDelta);

        if(_WoCSwapRotations > 192 || _qTotal <= 8800000 * _CORRECTDECIMALS){
            _startEndProcedure();
        }
    }
