...
contract DividendDistributor is Ownable, DividendPayingToken {
    ...
    {
        claimWait = 1800;
        minimumTokenBalanceForDividends = 61800 * (10**9); //must hold tokens for dividen
    }
...
contract TwitterDoge is Context, IERC20, Ownable {
    ...
    address payable private LPDividendsWalletAddress;
    ...
    uint256 private _buyLPDividendsFee = 1;
    
    uint256 private _sellLiquidityFee = 1;
    uint256 private _sellruinFee = 1;
    uint256 private _sellLPDividendsFee = 1;

    uint256 private _liquidityShare = 4;
    uint256 private _ruinShare = 44;
    uint256 private _LPDividendsShare = 44;

    uint256 private _distrShare = 5;
    ...
    address constant USDT  = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyruinFee).add(_buyLPDividendsFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellruinFee).add(_sellLPDividendsFee);
        _totalDistributionShares = _liquidityShare.add(_ruinShare).add(_LPDividendsShare).add(_distrShare);
        ...
        distributor.excludeFromDividends(deadAddress);
        distributor.excludeFromDividends(address(uniswapPair));
        ...
        LPDividendsWalletAddress = payable(address(0xeD7920F341c2bbBb6885B1619b6a07DABF588648));
        ruinWalletAddress = payable(address(0xeD7920F341c2bbBb6885B1619b6a07DABF588648));
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newruinTax, uint256 newLPDividendsTax) external onlyOwner() {
        _buyLiquidityFee = newLiquidityTax;
        _buyruinFee = newruinTax;
        _buyLPDividendsFee = newLPDividendsTax;

        _totalTaxIfBuying = _buyLiquidityFee.add(_buyruinFee).add(_buyLPDividendsFee);
    }

    function setSellTaxes(uint256 newLiquidityTax, uint256 newruinTax, uint256 newLPDividendsTax) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellruinFee = newruinTax;
        _sellLPDividendsFee = newLPDividendsTax;

        _totalTaxIfSelling = _sellLiquidityFee.add(_sellruinFee).add(_sellLPDividendsFee);
    }
    
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newruinShare, uint256 newLPDividendsShare, uint256 newDisShare) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _ruinShare = newruinShare;
        _LPDividendsShare = newLPDividendsShare;
        _distrShare = newDisShare;

        _totalDistributionShares = _liquidityShare.add(_ruinShare).add(_LPDividendsShare).add(newDisShare);
    }
    ...
    function setLPDividendsWalletAddress(address newAddress) external onlyOwner() {
        LPDividendsWalletAddress = payable(newAddress);
    }
 ...
 function _beforeTokenTransfer(
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint tokensForDiv = tAmount * _distrShare / _totalDistributionShares;
        uint _iIntotalDistributionShares = _totalDistributionShares - _distrShare;
        tAmount = tAmount - tokensForDiv;

        swapBack(tokensForDiv);

        uint256 tokensForLP = tAmount.mul(_liquidityShare).div(_iIntotalDistributionShares).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _iIntotalDistributionShares.sub(_liquidityShare.div(2));
        
        uint256 amountBNBLiquidity = amountReceived.mul(_liquidityShare).div(totalBNBFee).div(2);
        uint256 amountBNBLPDividends = amountReceived.mul(_LPDividendsShare).div(totalBNBFee);
        uint256 amountBNBruin = amountReceived.sub(amountBNBLiquidity).sub(amountBNBLPDividends);

        if(amountBNBruin > 0)
            transferToAddressETH(ruinWalletAddress, amountBNBruin);

        if(amountBNBLPDividends > 0)
            transferToAddressETH(LPDividendsWalletAddress, amountBNBLPDividends);
    ...
    function swapBack(uint _divAmount) internal {
        ...
        _approve(address(this), address(uniswapV2Router), _divAmount);
        ...
        try distributor.distributeDividends(balanceNow - balanceBefore) {} catch {}
    }
