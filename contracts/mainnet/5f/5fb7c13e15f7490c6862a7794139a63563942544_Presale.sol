...
contract Presale is Ownable {
    ...
    uint256 public immutable tokensPerUSD; // * 100
    ...
    uint32 public referralPercentage;
    ...
    function changeReferralPercentage(uint32 _referralPercentage) public onlyOwner() {
    ...
    function buyTokensWithReferral(address _referral) external payable {
        require(contributions[_referral] > 0, "Invalid referral id.");
        uint256 weiToHardcap = hardCap.sub(weiRaised);
        uint256 weiAmount = weiToHardcap < msg.value ? weiToHardcap : msg.value;
    ...
    function _buyTokensWithReferral(address beneficiary, uint256 weiAmount, address referral) internal {
        ...
        uint256 referralBonus = weiAmount * referralPercentage / 100;
        weiRaised = weiRaised.add(weiAmount);
        contributions[beneficiary] = contributions[beneficiary].add(weiAmount + referralBonus);
        contributions[referral] = contributions[referral].add(referralBonus);
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        ...
        return weiAmount.mul(bnbPrice).mul(tokensPerUSD).div(10 ** uint256(priceFeed.decimals())).div(100);
    }
