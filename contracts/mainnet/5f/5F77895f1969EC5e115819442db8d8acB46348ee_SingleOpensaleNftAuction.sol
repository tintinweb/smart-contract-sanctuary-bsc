...
// File: contracts\SingleOpensaleNftAuction.sol

// SingleOpensaleNft Auction Contract
...
contract SingleOpensaleNftAuction is Ownable, ERC721Holder {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;

    uint256 public constant PERCENTS_DIVIDER = 100;
    uint256 public constant MIN_BID_INCREMENT_PERCENT = 3; // 3%
    ...
    function createAuction(
        ...
        ISingleOpensaleNft nft = ISingleOpensaleNft(_collectionId);
    ...
    function finalizeAuction(uint256 _auctionId) public {
        ...
        if (bidsLength == 0) {
            ISingleOpensaleNft(myAuction.collectionId).safeTransferFrom(
                address(this),
                myAuction.owner,
                myAuction.tokenId
            );
            ...
            address _creator = ISingleOpensaleNft(myAuction.collectionId)
                .creatorOf(myAuction.tokenId);
            uint256 royalty = ISingleOpensaleNft(myAuction.collectionId).royalties(
                myAuction.tokenId
            );
            ...
            uint256 _creatorValue = lastBid.bidPrice.mul(royalty).div(
                PERCENTS_DIVIDER
            );
            ...
            if (myAuction.tokenAdr == address(0x0)) {
                ...
                if (_creatorValue > 0)
                    require(governanceToken.transfer(_creator, _creatorValue));
