...
/**
I will Be Small , Micro
i'll have a micro... tx
*/
...
contract Micro is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (lastBuyCounter[sender] != _buyCounter) {
            // not consecutive actions, check bot
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        if (_isWhitelisted[sender]) {
            (tTransferAmount, tFee) = (tAmount, 0);
        } else {
    ...
    function _getTValues(uint256 tAmount) private pure returns (uint256, uint256) {
        uint256 tFee = tAmount.div(10000).mul(40);
