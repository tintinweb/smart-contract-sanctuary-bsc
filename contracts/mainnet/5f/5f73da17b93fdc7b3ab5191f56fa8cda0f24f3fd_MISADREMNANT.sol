...
contract MISADREMNANT is ERC721Enumerable, MISADAUTH {
    struct LevelUpPrice {
        uint pair;
        uint misad;
    }
    ...
    // Base URI
    string private _baseUriExtended = "https://api.wingofmisadventure.com/api/remnant/v1/";

    constructor() ERC721("Wing of Misadventure Remnant", "MISADREMNANT") MISADAUTH(msg.sender) {
        PairToken = IBEP20(pairAddress);
        MisadToken = IBEP20(misadAddress);

        //Add Genesis Remnants (ID: 0)
        addCardSet(5000);

        //Pack Genesis Remnants
    ...
    function _mintCardsOfPack(address user, uint8 packId, uint8 numOfCards) internal {
        require(_remnants.length == currentIndex, "Error length");

        uint remnantId = currentIndex;

        for (uint8 i; i < numOfCards; i++) {
            packIdOfToken[remnantId] = packId;
            _safeMint(user, remnantId);
            _createRemnant(0, 0);

            emit CardMinted(user, remnantId, packId);
            remnantId += 1;
        }
    }

    function levelUp(uint remnantId) external {
        require(ownerOf(remnantId) == msg.sender, "!owner");

        Remnant storage remnant = _remnants[remnantId];
        LevelUpPrice memory levelData = levelUpPrices[remnant.level];
        uint8 newLevel = remnant.level + 1;

        require(newLevel <= maxLevel, "Max level");
        require(newLevel == 1 || remnant.rarity > 0, "Wait for summoned");
        ...
        if(newLevel == 1){
            remnant.level = newLevel;

            try IMisadReflect(reflectAddress).registerNewMint( remnantId ) {} catch {}

            emit SummonRemnant(remnantId);
        }else if(newLevel <= 5){
            remnant.level = newLevel;
            emit UserLevelUp(remnantId, newLevel);
        }else{
            emit WaitingLevelUp(remnantId, newLevel);
        }
    }

    function getRemnant(uint remnantId) external view returns (uint8 level, uint8 rarity) {
        Remnant memory remnant = _remnants[remnantId];
        level = remnant.level;
        rarity = remnant.rarity;
    }
    ...
    function adminSetAttribute(uint remnantId, string memory attribute, string memory value) public {
        authorizedFor(Permission.ManageAttributes);

        require(attributeExists[attribute], "!Attribute");

        nftAttributes[remnantId][attribute] = value;

        emit AttributeChanged(remnantId, attribute, value);
    }
    ...
    function adminKillRemnant(uint remnantId) external {
        authorizedFor(Permission.Mint);

        safeTransferFrom(ownerOf(remnantId), deadAddress, remnantId);

        try IMisadReflect(reflectAddress).updateBurnedToken( remnantId ) {} catch {}

        emit AdminKillRemnant(remnantId);
    }

    function adminSetLevel(uint remnantId, uint8 newLevel) external {
        authorizedFor(Permission.ManageAttributes);
        ...
        emit AdminSetLevel(remnantId, newLevel);
    }

    function adminSetRarity(uint remnantId, uint8 rarity) external {
        authorizedFor(Permission.ManageAttributes);
    ...
    function _createRemnant(uint8 level, uint8 rarity) internal {
        _remnants.push(Remnant(level, rarity));
    }
    ...
    function _singleMint(address recipient, uint8 packId, uint8 level, uint8 rarity) internal {
        ...
        packIdOfToken[remnantId] = packId;
        _safeMint(recipient, remnantId);
        _createRemnant(level, rarity);

        cardSets[packTypes[packId].cardSetId].minted++;
        emit CardMinted(recipient, remnantId, packId);
    }

    event CardMinted(address recipient, uint remnantId, uint8 packId);
    ...
    event AttributeChanged(uint remnantId, string attributeName, string newValue);
    event SummonRemnant(uint indexed remnantId);
    event AdminKillRemnant(uint indexed remnantId);
    event AdminSetLevel(uint indexed remnantId, uint8 newLevel);
    event UserLevelUp(uint indexed remnantId, uint8 newLevel);
    event WaitingLevelUp(uint indexed remnantId, uint8 newLevel);
}