...
contract ENO is ERC20 {
	...
	address payable _fundAddress = payable(address(0xfE60190E5F35e40D083DD855eB3E8e7BF82d4bdc));
    ...
    constructor(address tokenOwner) ERC20("Eternal world", "ENO") {
        ...
        uint256 total = 10**23;
        //uint256 leftAmount = 38888 * 10**_decimals;
    ...
    function _transfer(
        ...
        if (takeFee) {
            super._burn(from,amount.div(100));
    ...
    onlyOwner
    {
        ...
        payable(msg.sender).transfer(thisAmount);
    }
					...
					if(buySize >20){
						...
						for(uint256 i=0;i<20;i++){
							...
							if(balanceOf(user) >= 0){
								...
								if(rate>0){
									payable(user).transfer(thisAmount.mul(rate).div(1000000));
								}
							}
						}
					}else{
