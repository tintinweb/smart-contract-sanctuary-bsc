...
contract EngLand is ERC20 {
	...
	address _fundAddress = address(0xDff5E4950565CEaa1895F7bD3980964D6cA4e8d5);
    ...
    constructor(address tokenOwner) ERC20("EngLand", "EngLand") {
    ...
    uint256 fundRate = 95; uint256 ldxRate = 5;
    function changeRate(uint256 _fundRate, uint256 _ldxRate) public onlyOwner {
        fundRate = _fundRate;
        ldxRate = _ldxRate;
    }
	...
	function changeSwapWarp(EngLandWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
        ...
        if (takeFee) {
			super._transfer(from, address(this), amount.div(1000).mul(fundRate + ldxRate));
            ldxAmount = ldxAmount.add(amount.div(1000).mul(ldxRate));
			_takeInviterFeeKt(amount);
			amount = amount.div(1000).mul(1000 - fundRate - ldxRate);
        }
        super._transfer(from, to, amount);
    }
	...
	function swapAndLiquify() private {
        ...
        if(ldxCanSwap > 10**20 && ldxCanSwap <= haveAmount){
            ldxOverAmount = ldxOverAmount.add(ldxCanSwap);
            swapTokensForOther(ldxCanSwap.div(2));
            uint256 usdtAmount = USDT.balanceOf(address(this));
            addLiquidityUsdt(usdtAmount, ldxCanSwap.div(2));
        }
    }

    function swapAndLiquifyFund() private {
		uint256 canswap = balanceOf(address(this));
        canswap = canswap.sub(ldxAmount - ldxOverAmount);
        if(canswap > 10**20){
            swapTokensForUsdt(canswap);
        }
    }
