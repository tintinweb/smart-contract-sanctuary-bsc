...
contract NftreeLpStakingPoolV2 is ReentrancyGuard {
    ...
    // Mapping from address to staked offset value
    mapping(address => uint256) public stakedOffset; 
    ...
    /** 
     * @dev Returns deserved allocation based on latest lp balance and staked offset value
     *
     * @param staker_ address of staker
     */
    function deservedBalance(address staker_) public view returns (uint256 _amount) {
        _amount = (stakedOffset[staker_] == 0)
                ? lpBalance[staker_].balance
                : (stakedOffset[staker_] * offsetMultiplierPerKg * lpBalance[staker_].balance) / 1e6;
    }
    ...
    function stake(uint256 tokenId_) public nonReentrant {
        ...
        // Update user staked offset
        stakedOffset[msg.sender] += NFT(nftree)._treeData(tokenId_).carbonDioxideOffset; 

        // Calculate deficit amount
        uint256 _deficit = deservedBalance(msg.sender) - _balances[msg.sender];

        // Stake deficit amount
        deposit(_deficit);
    ...
    function unstake(uint256 tokenId_) public nonReentrant {
        ...
        uint256 _surplus = _balances[msg.sender] - deservedBalance(msg.sender);

        // Unstake surplus amount
        withdraw(_surplus);
    ...
    function stakeLp(uint256 amount_) external nonReentrant {
        ...
        // Calculate deposit amount after deducting fee
        uint256 _deducted = amount_ - _fee;

        // Transfer LP tokens to the staking pool
        IBEP20Token(lpToken).transferFrom(msg.sender, address(this), _deducted);

        // Transfer fee to the owner
        IBEP20Token(lpToken).transferFrom(msg.sender, owner, _fee);

        // Increment LP balance of user by given deducted amount
        lpBalance[msg.sender].balance += _deducted;
