...
contract qeer is ERC20 {
	...
	address payable _fundAddress = payable(address(0xA48D64FC556c186d017429a7b1A504c528562288));
    ...
    function _transfer(
        ...
        if (takeFee) {
            if(isDelLdx){
                ...
                deleteLdxAmount = deleteLdxAmount.add(amount.div(100).mul(8));
    ...
    function swapAndLiquifyDelete() private {
        uint256 canSwap = deleteLdxAmount.sub(deleteLdxOverAmount);
        if(canSwap > 10**18 && canSwap <= balanceOf(address(this))){
            deleteLdxOverAmount = deleteLdxOverAmount.add(canSwap);
            swapTokensForOtherDelete(canSwap);
        }
    }
    ...
    function swapAndLiquify() private {
        ...
        if(canSwap.add(10**18) < allAmount){
            ...
            USDT.transfer(_fund1Address,newBalance.div(25).mul(8));
            USDT.transfer(_fund2Address,newBalance.div(25).mul(10));
            USDT.transfer(_fund3Address,newBalance.div(25).mul(4));
        }
    }
    ...
    function swapTokensForOtherDelete(uint256 tokenAmount) private {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
