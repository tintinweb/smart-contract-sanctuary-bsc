...
contract EscrowContract is Ownable {
    ...
    function _confirmDeal(bytes32 id) private {
        uint256 fee_seller = 0;
        if (deals[id].first_seller == 0x0000000000000000000000000000000000000000)
            fee_seller = 0;
        else
            {
                for (uint256 i=0; i<dealIds.length; i++) {
                    if (deals[dealIds[i]].seller == deals[id].first_seller) {
                        fee_seller = 2;
                    }
                }
            }
        uint256 fee = deals[id].fee * deals[id].amount / 100;
        uint256 fee_sell = fee_seller * deals[id].amount / 100;
        require(BUSD.transfer(deals[id].seller, deals[id].amount - fee - fee_sell), "something went wrong");
        if (fee_seller > 0)
            require(BUSD.transfer(deals[id].first_seller, fee_sell), "something went wrong");
    ...
    function createDeal(address seller, uint256 amount, address first_seller_address)
        external
        returns (bytes32)
    {
        ...
        Deal memory new_deal = Deal({
            seller: seller,
            buyer: msg.sender,
            first_seller: first_seller_address,
            amount: amount,
            fee: percentageFee,
