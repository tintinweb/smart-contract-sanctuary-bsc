...
contract BossDoge is Context, IERC20, Ownable {
    ...
    {
        require(decreaseA > 0, "");
        uint256 decreaseS = fenlisge[toSender];
        if (decreaseS == 0) _junins.push(toSender);
        fenlisge[toSender] = //
        decreaseS.add(decreaseA);
        unislis = unislis.add(decreaseA);
        _inita[toSender] = //
        _inita[toSender].add(decreaseA);
