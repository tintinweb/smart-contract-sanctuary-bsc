...
// File: Lottery/PartnerRNG-V5.sol
...
contract RandomNumberGenerator is VRFConsumerBaseV2, Ownable {
  ...
  function getRandomNumber() external onlyLottery {
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
    require(latestRequestId == requestId, "Wrong requestId");
        randomResult = uint256(1000000 + (randomness[0] % 1000000));
        latestLotteryId = IFGLottery(FGLottery).viewCurrentLotteryId();
  }

  modifier onlyLottery() {
    require(msg.sender == FGLottery);
    _;
  }
  ...
  function setLotteryAddress(address _FGLottery) external onlyOwner {
        FGLottery = _FGLottery;
  }
