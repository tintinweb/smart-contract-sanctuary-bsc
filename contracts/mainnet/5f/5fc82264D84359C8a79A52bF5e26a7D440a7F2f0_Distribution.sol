...
contract Distribution is ReentrancyGuard {
    ...
    function erc20(uint amount, address token, address [] calldata dst) public nonReentrant {
        for (uint i; i < dst.length; i++) {
            TransferLib.transferFrom(token,msg.sender,dst[i],amount);
        }
    }
}