...
contract BtcmSaylor is IERC20 {
    ...
    uint256 constant private startingSupply = 21_000_000_000;
    string constant private _name = "BtcmSaylor";
    string constant private _symbol = "BTCMS";
    ...
    uint256 constant public maxRoundtripTaxes = 2500;
    ...
    // XRP MAINNET TOKEN CONTRACT ADDRESS
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xc8BA76F94Aa75d2eB6e3fe788735B52c7295a1b8),
        charity: payable(0xD46Aa9a38403C6C203b0BF21e52759cc16F079A7)
    });
...
//===============================================================================================================
    ...
    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 transferFee) external onlyOwner {
        ...
        require(buyFee + sellFee <= maxRoundtripTaxes
                && transferFee <= maxTransferTaxes, "Cannot exceed maximums.");
        _taxRates.buyFee = buyFee;
        _taxRates.sellFee = sellFee;
        _taxRates.transferFee = transferFee;
    }

    function setRatios(uint16 rewards, uint16 liquidity, uint16 marketing, uint16 charity) external onlyOwner {
        _ratios.rewards = rewards;
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.charity = charity;
        _ratios.total = rewards + liquidity + marketing + charity;
    ...
    function burn(uint256 amountTokens) external {
        ...
        try cashier.tally(sender, _tOwned[sender]) {} catch {}
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 marketingBalance = amtBalance - (rewardsBalance + charityBalance);

        if (ratios.rewards > 0) {
            try cashier.load(rewardsBalance) {} catch {}
        }
        if (ratios.marketing > 0){
            try IERC20_PairedToken.transfer(_taxWallets.marketing, marketingBalance) {} catch {}
        }
        if (ratios.charity > 0){
            try IERC20_PairedToken.transfer(_taxWallets.charity, charityBalance) {} catch {}
        }
    }
    ...
    function manualDeposit() external onlyOwner {
        cashier.load(IERC20_PairedToken.balanceOf(address(this)));
    }

    function externalLoad(uint256 amount) external {
        require(amount > 0, "Cannot load 0.");
        IERC20_PairedToken.transferFrom(msg.sender, address(this), amount);
        cashier.load(amount);
    }
