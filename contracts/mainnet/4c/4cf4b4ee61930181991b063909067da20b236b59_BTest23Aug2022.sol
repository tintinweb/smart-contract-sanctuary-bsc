...
contract BTest23Aug2022 is Context, IBEP20, Ownable {
    ...
    string private constant _name = "BTest23Aug2022V2";
    string private constant _symbol = "BTest23Aug2022V2";
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = _BEACHEthBurn(amount); //(100)
        // (tAmount, tBurn) -> (100,2)
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getValues(tAmount, tBurn); //(100,2)
        // (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTeam) -> (100,92,0,90,0,8)
        _rOwned[sender] = _rOwned[sender].sub(rAmount); // sender balance - 100
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount); // recevier balance + 92
        _takeTeam(tTeam); // (8)
        _reflectFee(rFee, tFee); // (0,0)
        emit Transfer(sender, recipient, tTransferAmount); // tTransferAmount -> 90
    }

    function _takeTeam(uint256 tTeam) private {
        uint256 currentRate = _getRate(); 
        uint256 rTeam = tTeam.mul(currentRate); // 8 * 1  = 8  
        _rOwned[address(this)] = _rOwned[address(this)].add(rTeam); // contract balance + 8 
    }

    function _BEACHEthBurn(uint amount) private returns (uint, uint) {
        uint orgAmount = amount; // 100
        uint256 currentRate = _getRate(); // 1 
        uint256 tBurn = amount.mul(_burn).div(100); // 2 token ( burn 2%)
        uint256 rBurn = tBurn.mul(currentRate); // 2 * 1 = 2 
        _tTotal = _tTotal.sub(tBurn);  //total_supply - 2 
        _rTotal = _rTotal.sub(rBurn); // -2 
        _BEACHBurned = _BEACHBurned.add(tBurn); // totalBurn + 2
        return (orgAmount, tBurn); //(100,2)
    }

    function _reflectFee(uint256 rFee, uint256 tFee) private {
        _rTotal = _rTotal.sub(rFee);
        _tFeeTotal = _tFeeTotal.add(tFee); // total reflection + reflection
    }

    function _getValues(uint256 tAmount, uint256 tBurn) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) = _getTValues(tAmount, _reflection, _contractFee, tBurn); //(100,0,8%,2)
        //(tTransferAmount, tFee,tTeam) - (90,0,8)
        uint256 currentRate = _getRate(); // rate = 1 
        uint256 taxBurn = tBurn;
        uint256 taxAmount =  tAmount;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(taxAmount, tFee, tTeam, taxBurn, currentRate); // (100,0,8,2,1)
        // (rAmount, rTransferAmount,rFee) -> (100,92,0)
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTeam); // (100,92,0,90,0,8)
    }
    ...
    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 teamFee, uint256 tBurn) private pure returns (uint256, uint256, uint256) {
        //(100,0,8%,2)
        uint256 tFee = tAmount.mul(taxFee).div(100); //reflection share - 100 * 0/100
        uint256 tTeam = tAmount.mul(teamFee).div(100); // contractFee share 100 * 8/100
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tTeam).sub(tBurn); // 100 - 0 - 8 - 2 = 90
        return (tTransferAmount, tFee, tTeam); //(90,0,8)
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tTeam,uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rFee = tFee.mul(currentRate); // reflection 
        uint256 rTeam = tTeam.mul(currentRate); // contractFee
        uint256 rBurn =  tBurn.mul(currentRate); // burn 
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rTeam).sub(rBurn); // 100 - 0 - 8 -2 = 90
        return (rAmount, rTransferAmount, rFee); // (100,90,0)
    }
    ...
    function distributeETH(uint256 amount) private {
        if (amount > 0){
            if(_boostFee > 0){
            _development.transfer(amount.div(_boostFee));   
            }
            if(_futureFee > 0 ){
            _boost.transfer(amount.div(_futureFee));
            }
        }
    }
