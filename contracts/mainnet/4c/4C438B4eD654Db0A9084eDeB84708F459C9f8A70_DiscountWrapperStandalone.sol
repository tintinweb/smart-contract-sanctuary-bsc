...
contract DiscountWrapperStandalone {
    ...
    // applied_percentages = [quantity_percentage, level_percentage, total_percentage, slippage_percentage]  --  Use applied_percentages[2] for the discounted percentage applied
    ...
    function getDiscountsInfos(uint userID, address user_address, uint16 packageID) public view returns (int8 index, uint8 level, uint16[] memory applied_percentages, uint[] memory prices, uint8[] memory info_codes) {
        ...
        applied_percentages = new uint16[](4);
        prices = new uint[](4);
        ...
        applied_percentages[3] = packageConversionSettings[packageID].override_default ? packageConversionSettings[packageID].slippage_tax_percentage : def_slippage_tax_percentage;
