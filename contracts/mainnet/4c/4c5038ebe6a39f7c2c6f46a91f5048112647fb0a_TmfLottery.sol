...
// File: contracts/interfaces/ITmfLottery.sol
//LBC-01
pragma solidity 0.8.4;

interface ITmfLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in TMF per group, and make lottery claimable
     * @param _lotteryId: lottery id
     * @param _autoInjection: reinjects funds into next lottery (vs. withdrawing all)
     * @param _tmfPerBracket: distribution of winnings by bracket
     * @param _countTicketsPerBracket: total number of tickets in each bracket
     * @dev Callable by operator
     */
    function drawFinalNumberAndMakeLotteryClaimable(
        uint256 _lotteryId,
        uint[8] calldata _tmfPerBracket,
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TMF token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInUSDT: price of a ticket in TMF
    ...
    function getCurrentTicketPriceInTMF(uint _lotteryId) external view returns(uint);
}

// File: contracts/TmfLottery.sol
//LBC-01
pragma solidity 0.8.4;
pragma abicoder v2;
 ...
 */
contract TmfLottery is ReentrancyGuard, ITmfLottery, Ownable {
    ...
    address public tmfTokenAddress;
    ...
    uint256 public maxPriceTicketInTMF = 50 ether;
    uint256 public minPriceTicketInTMF = 0.00000005 ether; //TODO change in prod 0.005 ether
    ...
    struct Lottery {
        ...
        uint256[8] tmfPerBracket;
        uint256[8] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTMF;
        uint256 amountInjectedInTMF;
        uint32 finalNumber;
    }
     ...
     * @param _tmfTokenAddress: address of the TMF token
     * @param _usdtTokenAddress: address of the USDT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     * @param _priceOracleAddress: address of oracle
     */
    constructor(
        address _tmfTokenAddress,
        address _usdtTokenAddress,
        address _randomGeneratorAddress,
        address _priceOracleAddress
    ) {
        //LBC-03
        require(_tmfTokenAddress != address(0), "_tmfTokenAddress address cannot be 0");
        require(_usdtTokenAddress != address(0), "_usdtTokenAddress address cannot be 0");
        tmfToken = IERC20(_tmfTokenAddress);
        tmfTokenAddress = _tmfTokenAddress;
        ...
        nonReentrant
    {
        ...
        // Update TMF price for _lotteryId
        _updateTMFPrice(_lotteryId);

        // Calculate number of TMF to this contract
        uint256 amountTMFToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTMF,
            _ticketNumbers.length
        );

        // Transfer TMF tokens to this contract
        tmfToken.safeTransferFrom(address(msg.sender), address(this), amountTMFToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTMF += amountTMFToTransfer;
        ...
        emit TicketsPurchase(msg.sender, _lotteryId, _ticketNumbers.length, amountTMFToTransfer);
    }
        ...
        nonReentrant
    {
        ...
        // Initializes the rewardInTMFToTransfer
        uint256 rewardInTMFToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTMFToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        tmfToken.safeTransfer(msg.sender, rewardInTMFToTransfer);

        emit TicketsClaim(msg.sender, rewardInTMFToTransfer, _lotteryId, _ticketIds.length);
    }

    function getCurrentTicketPriceInTMF(uint _lotteryId) override external view returns(uint){
        ...
        nonReentrant
    {
        ...
        require(_tmfPerBracket.length == 8, 'Wrong tmfPerBracket array size!');
        ...
        uint tmfSumPerBrackets = 0;
        for (uint i = 0; i < 8; i++){
            uint winningPoolPerBracket = _tmfPerBracket[i] * _countTicketsPerBracket[i];
            ...
            tmfSumPerBrackets += winningPoolPerBracket;
        }
        require(tmfSumPerBrackets <= amountToDistribute, 'Wrong brackets Total amount');

        _lotteries[_lotteryId].tmfPerBracket = _tmfPerBracket;
        ...
        // Transfer not winning TMF to treasury address if _autoInjection is false
        if (_autoInjection) {
            pendingInjectionNextLottery = amountToDistribute - tmfSumPerBrackets;
        } else {
            tmfToken.safeTransfer(treasuryAddress, amountToDistribute - tmfSumPerBrackets);
        }
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");
        //LBC-02
        _lotteries[_lotteryId].amountCollectedInTMF += _amount;
        tmfToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function startLottery(
        ...
        //Calculation price in TMF
        uint256 _priceTicketInTMF = _getPriceInTMF(_priceTicketInUSDT);

        require(
            (_priceTicketInTMF >= minPriceTicketInTMF) && (_priceTicketInTMF <= maxPriceTicketInTMF),
            "Price ticket in TMF Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTMF: _priceTicketInTMF,
            priceTicketInUSDT: _priceTicketInUSDT,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            tmfPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTMF: 0,
            amountInjectedInTMF: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(tmfTokenAddress), "Cannot be TMF token");
    ...
    /**
     * @notice Set TMF price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInTMF: minimum price of a ticket in TMF
     * @param _maxPriceTicketInTMF: maximum price of a ticket in TMF
     */
    function setMinAndMaxTicketPriceInTMF(uint256 _minPriceTicketInTMF, uint256 _maxPriceTicketInTMF)
        external
        onlyOwner
    {
        require(_minPriceTicketInTMF <= _maxPriceTicketInTMF, "minPrice must be < maxPrice");

        minPriceTicketInTMF = _minPriceTicketInTMF;
        maxPriceTicketInTMF = _maxPriceTicketInTMF;
    }
     ...
     * @param _priceTicket price of a ticket (in TMF)
    ...
    function _withdrawBurnAndCompetition(uint _lotteryId) internal returns(uint){
        ...
        uint collectedAmount = _lotteries[_lotteryId].amountCollectedInTMF;
        uint burnSum = (collectedAmount * burningShare) / 10000 ;
        uint competitionAndRefSum = (collectedAmount * competitionAndRefShare) / 10000 ;
        tmfToken.safeTransfer(burningAddress, burnSum);
        tmfToken.safeTransfer(competitionAndRefAddress, competitionAndRefSum);
        return (collectedAmount - burnSum - competitionAndRefSum);
    }

    /**
     * @notice Update TMF price for lotteryID
     */
    function _updateTMFPrice(uint256 _lotteryId) private {
        uint oldPriceInTMF = _lotteries[_lotteryId].priceTicketInTMF;
        uint newPriceInTMF = priceOracle.consult(usdtTokenAddress, _lotteries[_lotteryId].priceTicketInUSDT, tmfTokenAddress);

        require(_chekPriceDifference(newPriceInTMF, oldPriceInTMF, maxDiffPriceUpdate), 'Oracle give invalid price');
        _lotteries[_lotteryId].priceTicketInTMF = newPriceInTMF;
    }
    ...
    /**
     * @notice Get current exchange rate TMF/USDT from oracle
     */
    function _getPriceInTMF(uint256 _priceInUSDT) internal view returns(uint256) {
        return priceOracle.consult(usdtTokenAddress, _priceInUSDT, tmfTokenAddress);
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].tmfPerBracket[_bracket];
        } else {
            return 0;
        }
    }
