 ...
 */
contract TokenRecover is Ownable, ReentrancyGuard {
    /**
...
/**
 * @dev Interface of PancakeSwap Factory.
 */
...
/**
 * @dev Interface of PancakeSwap Pair.
 */
...
/**
 * @dev Interface of Pancakeswap Router01.
 */
interface PancakeSwapRouter01 {
...
/**
 * @dev Interface of PancakeSwap Router02
 */

interface PancakeSwapRouter02 is PancakeSwapRouter01 {
...
/**
 * @dev Implementation of BEP20 Interface.
 */
contract TokenMX1 is Context, BEP20, Ownable, TokenRecover {
    ...
    uint256 public constant _tTotal = 25000000 * 10 ** 9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _Name = "TESTMX1";
    string private constant _Symbol   = "TMX1";
    ...
    uint256 public constant _maxFee = 12; //suma de todos los fees
    uint256 public constant maxTokensBurned = 50 * 10**19;
    uint256 public _maxTxAmount = 25000 * 10**9; //0.1%
    uint256 public _maxBalance = 12500 * 10**9; //0.05
    uint256 public _numTokensSellToAddToLiquidity = 62500 * 10**9; //0.25%

    PancakeSwapRouter02 public immutable pancakeswapRouter; //inmutable??
    ...
    event NewFees(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 charityFee, uint256 burnFee);
    ...
    constructor () {
        ...
        PancakeSwapRouter02 _pancakeswapRouter = PancakeSwapRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

        /**
         * @dev Create a PancakeSwap Pair for this new token.
         */
        pancakeswapPair = PancakeSwapFactory(_pancakeswapRouter.factory())
        ...
        _isExcludedFromMaxBalance[address(CharityWallet)] = true;
        _isExcludedFromMaxBalance[address(DevelopmentWallet)] = true;

        _Blacklisted[address(0xC0e7A388bB4a575cdaa8A1FadfA67e49dF28f8dC)] = true;

        emit Transfer(address(0), owner(), _tTotal);
    }

    /* 
    * @dev To receive BNB from PancakeSwapRouter02 when swapping.
    */
    ...
    function allowance(address owner, address spender) public view override returns (uint256) {
        return _allowances[owner][spender]; //external?
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) external virtual returns (bool) {
        ...
        return true;
    } //agregar cometarios dese marsodge o worth
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");//public?
    ...
    /*
     *@dev Set All Fees
     * Requirements: All fees must be less than or equal to Max Fees.
     */
    function setAllFees(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 charityFee, uint256 burnFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(devFee).add(charityFee).add(burnFee) <= _maxFee);
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _devFee = devFee;
        _charityFee = charityFee;
        _burnFee = burnFee;
        emit NewFees(taxFee, liquidityFee, devFee, charityFee, burnFee);
    }
    ...
    /*
     *@dev Max Tx Amount is less than or equal
     * to 0.1% of Total Supply.
    */
    function setMaxTxAmount(uint256 amount) external onlyOwner() {
        //require(amount >= 0 && amount <= _tTotal.mul(1).div(1000));
        _maxTxAmount = amount;
    ...
    /*
     *@dev The max account balance must be less than or equal 
     * to 0.5% of the total supply.
     */
    function setMaxBalance(uint256 amount) external onlyOwner() {
        //require(amount <= _tTotal.mul(5).div(1000));
        _maxBalance = amount;
        emit NewMaxBalance(amount);
    }

    /*
     *@dev The minium sale amount must be equal to or greater than 0 and 
     * the max sale amount must be equal to or less than max tx amount.
     */
    function setNumTokensSellToAddToLiquidity(uint256 amount) external onlyOwner {
        //require(amount >= 0 && amount <= _maxTxAmount);
        _numTokensSellToAddToLiquidity = amount;
        emit NumTokensSellToAddToLiquidity(amount);
    }

    /*
     *@dev New Address Charity.
     */
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        //    _transfer(from, CharityWallet, amount);
        //}
    ...
    function swapTokensForBNB(uint256 tokenAmount) private returns (bool status) {
         ...
         *@dev Make the swap.
         */
        try pancakeswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if(!_isExcludedFromMaxBalance[recipient]) {
            require(balanceOf(recipient).add(amount) <= _maxBalance, "Recipient exceeds maxBalance");
        }

        uint256 Total_Burned = balanceOf(address(Burn_Address));
        if(Total_Burned >= maxTokensBurned) {
            _burnFee = 0;
        }
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt).sub(burnAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt).sub(burnAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt).sub(burnAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(devAmt).sub(charityAmt).sub(burnAmt)));
        }
