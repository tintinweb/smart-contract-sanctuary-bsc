...
contract BiswapStrategyAddBaseTokenOnly is
  OwnableUpgradeSafe,
  ...
  IStrategy
{
  ...
  function _calculateAIn(
    ...
    uint256 nominator = AlpacaMath
      .sqrt(rIn.mul(balance.mul(feeConstantB).add(rIn.mul(feeConstantC))))
      .sub(rIn.mul(feeConstantA));
