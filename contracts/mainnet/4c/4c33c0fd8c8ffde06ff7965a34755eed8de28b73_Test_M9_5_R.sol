...
if (user.REG_activatesRefer >= 1)
{
    ...
    user.DEP_activatesRefer_Timer = block.timestamp;
 	...
 	function GET_DEP_activatesRefer_timers_Bool(address userAddress) public view returns (bool) {
		User storage user = users[userAddress];


if(users[user.LevelReferrals1].DEP_activatesRefer >= block.timestamp.add(5 minutes))
{
    user.RERE == true;
}
if(users[user.LevelReferrals1].DEP_activatesRefer <= block.timestamp.add(5 minutes))
...
function withdraw () public {
 ...
 if (users[user.LevelReferrals1].DEP_activatesRefer >= 2  && users[user.LevelReferrals1].DEP_activatesRefer >= block.timestamp.add(1 minutes)/*&& user.checkpointLVR1RefBonus >= 5 minutes*/)
