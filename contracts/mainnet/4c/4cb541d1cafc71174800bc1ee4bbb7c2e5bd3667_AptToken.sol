...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract AptToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "APT Token";
    ...
    constructor() public {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x0C8094a69e8e44404371676f07B2C32923B5699c);
        
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
            
        uniswapV2Router = _uniswapV2Router;
        marketingWallet = 0xcFEA8Ad95f8efe5cDfebEa0Cc3Bf53B2F858c83F;
        lqWallet = 0xcFEA8Ad95f8efe5cDfebEa0Cc3Bf53B2F858c83F;
      
        isTaxless[_msgSender()] = true;
        isTaxless[address(this)] = true;
        isTaxless[lqWallet] = true;
    ...
    function collectFee(address account, uint256 amount, uint256 rate) private returns (uint256) {
        ...
        if(_liquidityFee != 0){
            ...
            _reflectionBalance[lqWallet] = _reflectionBalance[lqWallet].add(liquidityFee.mul(rate));
            /*if(_isExcluded[lqWallet]){
                _tokenBalance[lqWallet] = _tokenBalance[lqWallet].add(liquidityFee);
            }*/
            _liquidityFeeTotal = _liquidityFeeTotal.add(liquidityFee);
            emit Transfer(account,lqWallet,liquidityFee);
        }
    ...
    function _getReflectionRate() private view returns (uint256) {
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = lqWallet.balance;

        swapTokensForEth(half); 

        uint256 newBalance = lqWallet.balance.sub(initialBalance);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(lqWallet, address(uniswapV2Router), tokenAmount);
    ...
    function swapliquifi(uint256 contractTokenBalance) external onlyOwner {
        swapAndLiquify(contractTokenBalance);
    }
    ...
    function setLqWallet(address account) external onlyOwner {
        lqWallet = account;
    }
