...
import "../../nft/SmartDisPatchInitializable.sol";
import "../utils/Base64.sol";

contract DNCCard is ERC721("Dnc card", "DNC"), SafeOwnable {
    using Strings for uint256;
    ...
    function mint(address _recipient, uint256 _type) public onlyMinner returns (uint256) {
        require(
            _recipient != address(0),
            "BridgeCoinClub: recipient is zero address"
        );
        uint256 _tokenId = totalSupply() + 1;
        cardType[_tokenId] = _type;
        _mint(_recipient, _tokenId);
        emit Mint(_recipient, _tokenId);
        return _tokenId;
    }

    function batchMint(address[] memory _recipients, uint256 _type)
        external
        onlyMinner
    {
        for (uint256 i = 0; i != _recipients.length; i++) {
            mint(_recipients[i], _type);
        }
    }

    function batchMint(address _recipients, uint256 _type, uint256 _amount)
        external
        onlyMinner
    {
        for (uint256 i = 0; i != _amount; i++) {
            mint(_recipients, _type);
        }
    }
            ...
            bytes(uri).length > 0
                ? string(
                    abi.encodePacked(
                        uri,
                        Base64.encode(
                            bytes(uint256(cardType[tokenId]).toString())
                        )
                    )
                )
                : "";
