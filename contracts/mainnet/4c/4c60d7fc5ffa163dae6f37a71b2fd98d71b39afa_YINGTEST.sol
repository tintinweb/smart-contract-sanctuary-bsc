...
contract YINGTEST is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    YINGTESTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("YINGTEST", "YING") {
       
        
        marketingWallet = payable(0xbf635290dc054c6B2c3196aEe2D74e74E1Bac51a);
        devWallet = payable(0x01e016DDa2c8FAdb73c2a51912A810c2A7201633);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new YINGTESTDividendTracker(
            payable(this),
            router,
    ...
    function claimStakedRewards(address _tokenContract, uint256 _percentage) external {
        require(_percentage <= 100, "Revert: Percentage cannot be grater then 100");
        require(stakingUntilDate[msg.sender] >= block.timestamp, "Revert: Tokens are not staked!");
        dividendTracker.processAccountBep20(_tokenContract, payable(msg.sender), _percentage);
    }
...
contract YINGTESTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    YINGTEST public YINGTESTContract;
    ...
    constructor(
        ...
        defaultToken = token;
        YINGTESTContract = YINGTEST(mainContract);
        ...
        tokens.push(Token({contractAddress: IERC20(0xFACE67C5CE2bb48c29779b0Dede5360cC9ef5fd5), percentage: 20, active: true}));
        tokens.push(Token({contractAddress: IERC20(0xdD8B490001D081eD065239644dae8D1a77b8A91F), percentage: 80, active: true}));
        
        permitedTokens[0xdD8B490001D081eD065239644dae8D1a77b8A91F] = true; // BITV
        permitedTokens[0x8B6bF63e2b1C221dC0Fb01AE85868b83dDee1B54] = true; // BBR
        permitedTokens[0xeA795BB91Fd2e1A885Bc13C549955ff6c124cA15] = true; // BBRR
        permitedTokens[0xFACE67C5CE2bb48c29779b0Dede5360cC9ef5fd5] = true; // Affinity
        permitedTokens[0x6e61579c22F9a6dA63a33e819f29B6697d2a126E] = true; // RocketFi
        permitedTokens[0x69B0af16FDd2E80968eb505cd41DC26efb2B80BF] = true; // BBL
    ...
    function _transfer(
        ...
        require(false, "YINGTEST_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YINGTEST_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YINGTEST contract."
        );
    }
    ...
    function processAccountBep20(address tokenAddress, address payable account, uint percentage)
        public
        onlyOwner
        returns (bool)
    {
        ...
        uint256 amount = _withdrawDividendOfStakedUser(account, percentage, tokenAddress);
        if (amount > 0) {
            lastClaimTimes[account] = block.timestamp;
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YINGTESTContract);

            uint256 prevBalance = YINGTESTContract.balanceOf(address(this));
            ...
            {
                uint256 received = YINGTESTContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YINGTESTContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
    ...
    function _withdrawDividendOfStakedUser(address payable user, uint percetage, address tokenAddress)
        internal
        returns (uint256)
    {
        uint256 _withdrawableDividend = withdrawableDividendOf(user) * percetage / 100;
        ...
        if (_withdrawableDividend > 0) {
            ...
            if (tokenAddress == address(0)) {
               ...
               success = claimdDistribute(_withdrawableDividend, user, tokenAddress);
            }
    ...
    function claimdDistribute(uint256 amount, address user, address _tokenAddress) internal returns(bool) {
             address[] memory path = new address[](2);
             uint deadline = block.timestamp;
             path[0] = uniswapV2Router.WETH();
             path[1] = address(_tokenAddress);
             uint256 _amount = uniswapV2Router.getAmountsOut(amount, path)[1];
             try uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(0, path, user, deadline) { 
                 permitedTokensTotalClaims[_tokenAddress] = permitedTokensTotalClaims[_tokenAddress] +  _amount;
                 permitedTokensUserClaims[_tokenAddress][user] = permitedTokensUserClaims[_tokenAddress][user] + _amount;
                 return true; 
                 } catch { 
                 return false;    
            }
