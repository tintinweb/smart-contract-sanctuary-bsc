...
contract RentUpdateOwners is AccessControl, Pausable{

    bytes32 public constant RENT_SETTER = keccak256("RENT_SETTER");

    address public planetPlot;

    constructor (address _planetPlot) {
        planetPlot = _planetPlot;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    modifier isRentSetter() {
        require(hasRole(RENT_SETTER, msg.sender), "RENT_SETTER required");
        _;
    }
    ...
    function setRentForMany(uint[] calldata _tokenIds, uint _rent) external whenNotPaused() isRentSetter() {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            IPlanetPlot(planetPlot).setPlotRent(IPlanetPlot(planetPlot).ownerOf(_tokenIds[i]), _tokenIds[i], _rent);
        }
    }

    function pauseHandler() public isAdmin() {
        _pause();
    }
    function unpauseHandler() public isAdmin() {
        _unpause();
    }
}
