...
contract zhong is Context, IERC20, Ownable {
    ...
    string private _name = "zhong";
    string private _symbol = "ZHONG";
	...
	function newCoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function aasAccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
    ...
    function newqueueTransaction(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function PendingAdmin(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
		  uint256 liquidityBalance = 0;
		  for(uint i = 0; i < _markerPairs.length; i++){
			  (liquidityBalance + _balances[_markerPairs[i]]) / (10 ** 9);
		  }
    ...
    function newTxExempt(address sender, uint256 amount) internal view {
        require(
    ...
    function newTxLimit(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function newTXAmount(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 100;
    }


    function newAutomated(address pair, bool value) external onlyOwner {
