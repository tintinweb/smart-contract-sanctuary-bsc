...
contract bolaoCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10_000_000 * 10 **_decimals;    // Total supply = 100m
    uint256 private ONE_TENTH_SUPPLY = 1_000_000* 10 **_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public _maxInAmount = 10_000_000 * 10**_decimals;    // 100% supply = 10m - Initial max buy
    uint256 public _maxOutAmount = 10_000_000 * 10**_decimals;    // 100% supply = 10m - Initial max sell
    uint256 public _maxWallet = 10_000_000 * 10**_decimals;      //  100% Initial = 10m  max Wallet
    uint256 public numTokensToSwapMarketing = 5_000 * 10**_decimals;  // 5k - tokens to swap (marketing balance wallet)
    uint256 public numTokensToSwapReward = 5_000 * 10**_decimals;  // 5k - tokens to swap (marketing balance wallet)
    ...
    balances public contractBalance = balances (
        {
            marketing_balance:0,
            reward_balance:0,
            lp_balance:0
    ...
    address private antiBot = 0xBD57b06349DDe6Bc296bE8f1639690E25A8D1FC5;
    address public bbRewardWallet = payable(0x3B982040d9d31f09B3E16fbB8C7E62C738FeDcd5); // reward wallet 


    bool public Trading = false;
    bool inSwapAndLiquify;
    bool public _EnableTransferFrom = false;
    ...
    event RewardAdded(uint256 e_tokenAmount, address e_bbRewardWallet);
    ...
    constructor () {
        // initial distribution 97% liquidity - 3% project
        _rOwned[owner()] = ((_rTotal / 100 )* 97);
        _rOwned[address(bbCoinProject)] = ((_rTotal / 100 )* 2);
        _rOwned[address(bbCoinWalletGame)] = ((_rTotal / 100 )* 1);
    ...
    function setTokenToSwapReward(uint256 top) public onlyOwner {
        numTokensToSwapReward = top * 10**_decimals;
    }
    ...
    function _takeReward(uint256 rReward, uint256 tReward) private {
        contractBalance.reward_balance+=tReward;
        totFeesPaid.reward+=tReward;
        _rOwned[address(this)] = _rOwned[address(this)]+rReward;
        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)] = _tOwned[address(this)]+tReward;
        }
    }
 ...
 function _transfer(address from, address to, uint256 amount) private {
		...
		if(_EnableTransferFrom == true){
		    ...
		    if (contractBalance.reward_balance>= numTokensToSwapReward && !inSwapAndLiquify && from != uniSwapV2Pair && swapAndLiquifyEnabled) {
			   	swapAndSendToReward(numTokensToSwapReward);
			}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        if(takeFee)
        {
            ...
            _takeReward(s.rReward, s.tReward);
            emit Transfer(sender, address(this), s.tMarketing+s.tLiquidity+s.tReward);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        function swapAndSendToReward(uint256 tokenAmount) private lockTheSwap {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = UniSwapV2Router.WETH();
        ...
        contractBalance.reward_balance-=tokenAmount;
        UniSwapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
        ...
        emit RewardAdded(tokenAmount, bbRewardWallet);
    }
