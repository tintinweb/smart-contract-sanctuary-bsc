...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function setTokenAddress(address _dividend) external onlyOwner {
...
contract huangdi is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;        
    
    string private _name = "huangdi";
    string private _symbol = "HUANGDI";
    ...
    function setTXPercent(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent ) / 1000;
    }

    function setMaxPercent(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage ) / 1000;
    }
  
    function removeFee(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
      if(shareholder == address(0)){ return 0; }     
      if(totalDividends <= totalExcluded){ return 0; }
      return totalDividends - (totalExcluded);
  }

    function clearBuyFees() external onlyOwner {     
        taxLiquidity = 0;
        taxMarketing = 0;
        taxSellBack = 0;
        taxBuyBack = 0;
