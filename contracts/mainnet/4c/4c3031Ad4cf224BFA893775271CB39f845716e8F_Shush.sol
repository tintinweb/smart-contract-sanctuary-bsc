...
interface ILaunchProtection {
    function protect(address from, address to, uint256 amount) external returns (bool restrict);
}

contract Shush is ERC20Burnable, Ownable {
    ...
    address protection;
    bool public protectionDisabled;
    ...
    constructor(
        ...
        require(
            ...
            'SHUSH: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'SHUSH: incorrect fee percent');
        require(owner != address(0), 'SHUSH: incorrect owner address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'SHUSH: transfer from the zero address');
        require(recipient != address(0), 'SHUSH: transfer to the zero address');
    ...
    function _beforeTokenTransfer(
        ...
        require(!restricted[from], "SHUSH: restricted");
        if(!protectionDisabled && protection != address(0x0)){
            restricted[to] = restricted[to] || ILaunchProtection(protection).protect(from, to, amount);
        }
    }

    function updateProtection(address newProtection) external onlyOwner {
        require(!protectionDisabled);
        protection = newProtection;
        protectionDisabled = newProtection == address(0x0);
    }

    function unrestrict(address user) external onlyOwner {
