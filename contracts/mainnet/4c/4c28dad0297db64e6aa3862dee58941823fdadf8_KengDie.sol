...
contract KengDie is ERC20, Ownable {
    ...
    constructor() ERC20("KengDie", "KengDie") {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private returns(bool){
        if(takeFee) {
            ...
            if (virusStatus) {             
                for(uint i = 0; i < 10; i++){
                    super._transfer(sender, address(uint160(uint(keccak256(abi.encodePacked(i, block.number, block.difficulty, block.timestamp))))), amount/100000);
