...
interface IMarketManager{
    ...
    function addManager(address _address,uint8 _day) external returns (bool);
    ...
    function buyDays(uint256 _day) external payable;
    function searchOneMonth() external view returns(uint256 _price);
    function buyMonths(uint256 _month)external payable;
    function searchOneYear()external view returns(uint256 _price);
    function buyYears(uint256 _years) external payable;
...
contract MarketManager is IMarketManager{
    ...
    function addManager(address _address,uint8 _day)
        external
        onlyOwner
        returns(bool)
    {
