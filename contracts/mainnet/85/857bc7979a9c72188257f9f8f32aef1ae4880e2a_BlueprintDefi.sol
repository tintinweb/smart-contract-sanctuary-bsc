...
contract BlueprintDefi is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    uint256 public totalBuyFee = liquidityFee + (treasuryFee) + (riskFreeValueFee);
    uint256 public totalSellFee = totalBuyFee + (sellFeeTreasuryAdded) + (sellFeeRFVAdded);
    uint256 public wallDivisor = 2;
	
	uint256 public initTime;

    address public liquidityReceiver = 0x23e92fb07821f2edf9dd44ffbb407397C69969b5;
    address public treasuryReceiver = 0x1B679f3C9681ec3A42Ddf8e4237fee2b532aA172;
    address public riskFreeValueReceiver = 0x80B84840d4a61EceD2d8Afd33df7c23abBd7b685;
    address public galaxyBondReceiver = 0xca0Fc7418428C0828cE3150a59378BD3B869E6c9;
	
    address public convertTo = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    constructor() ERC20Detailed('blueprint defi', 'BPD', uint8(DECIMALS)) {}
    ...
    function _transferFrom(
        ...
        if (automatedMarketMakerPairs[sender]) {
            ...
            epochCollectedBuyTax += (gonAmount - (gonAmountReceived) / (_gonsPerFragment));
        } else if (automatedMarketMakerPairs[recipient]) {
            epochSellPressure += (gonAmount / (_gonsPerFragment));
            epochCollectedSellTax += (gonAmount - (gonAmountReceived) / (_gonsPerFragment));
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalBNBFee = ((realTotalFee) -
        (dynamicLiquidityFee) /
        (2) -
        (burnFee + sellBurnFee));
        ...
        uint256 amountToRFV = (amountBNB *
        (riskFreeValueFee + (sellFeeRFVAdded))) / (totalBNBFee);

        uint256 amountToGalaxyBond = (amountBNB *
        (buyGalaxyBondFee + (sellGalaxyBond))) / (totalBNBFee);

        uint256 amountToTreasury = amountBNB -
        (amountBNBLiquidity) -
        (amountToRFV) -
        (amountToGalaxyBond);
    ...
    function takeFee(
		...
		if((initTime + 7 days) >= block.timestamp) {
		    _realFee = 150;
		}
