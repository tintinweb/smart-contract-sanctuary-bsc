...
contract SupportAssistV6 is Ownable {
    ...
    uint256 public tokenAPrice = 0;//3 * 1e18;
    ...
    function setTokenAPrice(uint256 _amount) external onlyAdmin {
        tokenAPrice = _amount;
    }

    function setSwapAt(uint256 _amount) external onlyAdmin {
    ...
    function getTokenB(uint256 amountU) public view returns (uint256) {
        uint256 a = tokenB.balanceOf(address(tokenBPair));
        uint256 u = USDT.balanceOf(address(tokenBPair));
        return (a * amountU) / u;
    }
    ...
    function deposit(uint256 _ftype, uint256 amount0) external {
        ...
        if (tokenAPrice > 0) {
            amountU = (tokenAPrice * amount0) / 1e18; // getUSDT(amount0);
            amount1 = getTokenB(amountU);
        }
        ...
        emit Deposit(_account, _ftype, amount0, amount1, amountU, lp, power);
    }
