...
contract Radar is ERC20 {
    ...
    address _bornToken = address(0x308eE56641C55FDb2ED509809671D3076aAB1A1f);
    IERC20 public USDT;
    GogezillaWarp warp;
	...
	address public _fundAddress = address(0xC7a99fC693Fb2E47Af3bB1DE1F73C172c1abD783);
    address public _marketAddress = address(0x308eE56641C55FDb2ED509809671D3076aAB1A1f);
	...
	address luckAddress = address(0x4229f3A2c308DB590D9410EA2A766FFA0510B783);//500w;
	address toLuckAddress = address(0x42571EA99B68e5e5C079CE638Bdf343E1051769f);//100w;
	address receiveAddress = address(0xe970d11b16a769840dC0c910D51b46E87dDF1280);//3500w;
    constructor(address tokenOwner) ERC20("Radar", "Radar") {
        ...
        excludeFromFees(toLuckAddress, true);
        excludeFromFees(address(this), true);
        _isExcludedFromVipFees[address(this)] = true;
        swapTokensAtAmount = total.div(100000);
        _mint(tokenOwner, total.div(50).mul(9));
        luckAmount = total.div(50).mul(5);
		_mint(luckAddress, total.div(50).mul(5));
		_mint(receiveAddress, total.div(50).mul(35));
		_mint(toLuckAddress, total.div(50));
    ...
    function getAdmin500LuckAmount() public view returns(uint256){
        uint256 overMouth = (block.timestamp.sub(startTime)).div(30 * 86400);
    ...
    function _transfer(
		...
		if(from == toLuckAddress && to != uniswapV2Pair){
		...
		if(from == luckAddress){
			...
			require(balanceOf(luckAddress).sub(amount) >= remainAmount,"luckAddress amount is not release");
		}
        ...
        if (takeFee) {
			super._transfer(from, _destroyAddress, amount.div(25));
	...
	function swapAndLiquify() public {
		...
		USDT.transfer(_fundAddress,usdtAmount.div(2));
		USDT.transfer(_marketAddress,usdtAmount.div(2));
    }
