...
contract AimedisStakingPoolOld180 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount == 0, "Cannot deposit tokens unless owner");
    ...
    function forceWithdrawForHolders(address[] calldata accounts) external onlyOwner {
        updatePool(0);
        for(uint256 i = 0; i < accounts.length; i++){
            require(holderUnlockTime[accounts[i]] <= block.timestamp, "May not do normal withdraw early");
            ...
            if(_amount > 0) {
                user.amount = 0;
                totalStaked -= _amount;
                pool.lpToken.safeTransfer(address(accounts[i]), _amount);
            }

            user.rewardDebt = user.amount.mul(pool.accTokensPerShare).div(1e12);
            
            holderUnlockTime[accounts[i]] = 0;

            emit Withdraw(accounts[i], _amount);
        }
    }
