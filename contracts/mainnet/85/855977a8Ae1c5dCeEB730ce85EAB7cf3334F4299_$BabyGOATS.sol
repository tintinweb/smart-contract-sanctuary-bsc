...
/*
*
* Telegram: https://t.me/babygoatscoin
* Twitter: https://Twitter.com/babygoatscoin
* Website: https://www.Babygoatscoin.com
*/
...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0xCC42724C6683B7E57334c4E856f4c9965ED682bD);
...
contract $BabyGOATS is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   $BabyGOATSDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Baby Goatscoin", "$BabyGOATS") {
        _buyRewardsFee = 1;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 1;
        _buyDevFee = 1;
    	...
    	dividendTracker = new $BabyGOATSDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xe8CF0702D8C5A62156d5660E2f826F1bb842C84F);
        _devWallet = payable(0x0125FaeCC8805656D9C131b86d81Ff93A380053C);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "$BabyGOATS: The dividend tracker already has that address");

        $BabyGOATSDividendTracker newDividendTracker = $BabyGOATSDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$BabyGOATS: The new dividend tracker must be owned by the $BabyGOATS token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "$BabyGOATS: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "$BabyGOATS: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "$BabyGOATS: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "$BabyGOATS: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "$BabyGOATS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$BabyGOATS: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "$BabyGOATS: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "$BabyGOATS: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$BabyGOATS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$BabyGOATS: Cannot update gasForProcessing to same value");
...
contract $BabyGOATSDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("$BabyGOATS_Dividend_Tracker", "$BabyGOATS_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "$BabyGOATS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $BabyGOATS contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "$BabyGOATS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "$BabyGOATS_Dividend_Tracker: Cannot update claimWait to same value");
