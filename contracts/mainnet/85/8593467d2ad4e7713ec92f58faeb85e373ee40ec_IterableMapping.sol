...
contract USDCDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Web3Meta_Usdc_Dividend_Tracker", "Web3Meta_Usdc_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Web3Meta_Usdc_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Web3Meta_Usdc_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Web3Meta contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Web3Meta_Usdc_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Web3Meta_Usdc_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Web3Meta is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _usdcDividendRewardsFeeBuy = 5;
    uint256 public _marketingFeeBuy = 2;
    uint256 public _LpFeeBuy = 1;

    uint256 public _usdcDividendRewardsFeeSell=5;
    ...
    constructor() ERC20("Web3Meta", "WEB3M") {
        	
    	marketingWallet = 0x92BfB5D6579e648093d01d04b5fEb05B47289b8E; 
    	usdcDividendToken = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d; 
    ...
    address[] private _blackListedBots;
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "Web3Meta: The marketing wallet is already this address");
    ...
    function updateusdcDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(usdcDividendTracker), "Web3Meta: The dividend tracker already has that address");
 
        USDCDividendTracker newusdcDividendTracker = USDCDividendTracker(payable(newAddress));
 
        require(newusdcDividendTracker.owner() == address(this), "Web3Meta: The new dividend tracker must be owned by the Web3Meta token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "Web3Meta: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "Web3Meta: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Web3Meta: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "Web3Meta: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "Web3Meta: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
      ...
      require(!_isBlackListedBot[tx.origin], "You have no power here!");
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "Web3Meta: Trading has not started yet");
