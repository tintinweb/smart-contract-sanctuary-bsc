...
contract Twizzler is DataShare, Ownable{
    ...
    mapping(string => bool) existRefCode;
    mapping(string => address) refCodeOwner;
    RefCode[] refCodes;

    constructor(address collectible, address _token, uint _refPrice) {
        videoCollection = Collectible(collectible);
        paymentToken = IERC20(_token);
        refPrice = _refPrice;
    }

    function playVideo(uint256 _id, uint _price, string memory _refCode) public payable {
        _price *= 10**12;
        (, , uint256 vidoPrice, , , , ) = videoCollection.videos(_id);
        (, , , , uint256 owner_fees, , ) = videoCollection.videos(_id);
        uint _refBenefit = calcRefBenefit(vidoPrice);
        uint256 creator_fees = vidoPrice.sub(owner_fees);
        (, , , address creator, , , ) = videoCollection.videos(_id);
        require(!subscribers[_id][msg.sender], "Error 1");
        require(msg.sender != creator, "Error 2");
        require(msg.sender != owner(), "Error 3");

        if (existRefCode[_refCode]) {
            address refOwner = refCodeOwner[_refCode];
            user_funds[refOwner] += _refBenefit;
            vidoPrice -= _refBenefit;
            creator_fees = creator_fees - ( 2 * _refBenefit) ;
        }
        require(_price == vidoPrice, "Error 4");
        ...
        videoCollection.setActivity(msg.sender, vidoPrice, "Play Video");
    ...
    function transferOwnership(address newOwner) public override onlyOwner{
        videoCollection.transfer_ownership(newOwner);
        super.transferOwnership(newOwner);
    }

    function addRefCode(string memory code) public{
        require(!hasRefCode[_msgSender()], "error");
        refCodes.push(RefCode(_msgSender(), code));
        hasRefCode[_msgSender()] = true;
        existRefCode[code] = true;
        refCodeOwner[code] = _msgSender();
    }

    function getRefCode(address _address) public view returns(string memory _code){
        _code = "null";
        for (uint i = 0; i < refCodes.length; i++) {
            if (refCodes[i]._address == _address) {
                _code = refCodes[i]._code;
            }
        }
    }

    function calcRefBenefit(uint _price) internal view returns(uint _benefit){
        _benefit = refPrice.mul(_price.div(2000));
    }

    function actualPrice(uint id) public view returns(uint _actualPrice){
        (, , uint price, , , , ) = videoCollection.videos(id);
            _actualPrice = price - calcRefBenefit(price);
    }
}
