...
contract Token is ERC20 {
    ...
    IERC20 public BTCB;
    GetWarp public warp;
    GetWarp public warp_fork;
    bool private swapping;
    uint256 public swapTokensAtAmount;
    uint256 _destroyMaxAmount;
	address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address private _MarketAddress = address(0x0eCC6Ce4F1BF9aF685EaFDa22D37F7326497b2b2);
    address private _MarketAddress_fork = address(0x9cc9151Ab7288ad26eFcf58A906E8CbA31aA5025);
    ...
    uint256 private intervalTime = 0 minutes;
    ...
    constructor(address tokenOwner) ERC20("QJEB", "QJEB") {
        
        //mainnet    
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        //testnet
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xe8239921214fF1272AdF9e17bC081dBf03b1412D);
        ...
        excludeFromFees(_MarketAddress_fork,true);
        USDT = IERC20(0x55d398326f99059fF775485246999027B3197955);//mainnet
        // USDT = IERC20(0x114061e2a2f677D5902AB5B35706c3394C7195B9);//testnet
        ETH = IERC20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
        // ETH = IERC20(0xCF06Fb332A2bfdfA466bAc4a25caE39248D09B71);//testnet
        BTCB = IERC20(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);
        // BTCB = IERC20(0xE1F3311C05dbC0595c9A219Fc4422912A157e007); //testnet
        pair = IERC20(_uniswapV2Pair);
        uint256 total = 21 * 10**20* 10**3;
        swapTokensAtAmount = 5 * 10**19* 10**3;
    ...
    function changeSwapWarpfork(GetWarp _warp_fork) public onlyOwner {
        warp_fork = _warp_fork;
    }
    function warpWithdraw() public onlyOwner {
        warp.withdraw();
    }
    function warp_forkWithdraw() public onlyOwner {
        warp_fork.withdraw();
    }
    ...
    function setBuyEnable(bool _enabled) public onlyOwner {
        buyEnable = _enabled;
    }
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount && block.timestamp >= (currentTime + intervalTime)){
            if (
                ...
                // BTCswapAndLiquifyV3(balanceOf(address(this)));
                swapping = false;
            }
        }
        ...
        if (takeFee) {
            ...
            if(to == uniswapV2Pair){//卖出
                ...
                super._transfer(from, _MarketAddress, amount.div(100).mul(1));//营销1%
                super._transfer(from, _MarketAddress_fork, amount.div(100).mul(1));//营销1%
                super._transfer(from, address(this), amount.div(100).mul(4));//分红4%
                super._transfer(from, _destroyAddress, amount.div(100).mul(1));//1%销毁

                amount = amount.div(100).mul(93);//93%

            }else if(from == uniswapV2Pair){// 买入
                if(amount > 200000 *10**18 && startTime.add(23280) > block.timestamp){
                    revert();
                }
            if(!buyEnable){revert();}
                super._transfer(from, _MarketAddress, amount.div(100).mul(1));//营销1%
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        swapTokensForOther(contractTokenBalance.mul(50).div(100));
        uint i = contractTokenBalance.sub(contractTokenBalance.mul(50).div(100));
        swapTokensForBTC(i);
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
        ...
        // path[1] = address(0x114061e2a2f677D5902AB5B35706c3394C7195B9);//testnet
        path[2] = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);// ETH
        // path[2] = address(0xCF06Fb332A2bfdfA466bAc4a25caE39248D09B71);// testETH
    ...
    function swapTokensForBTC(uint256 tokenAmount) private {
        ...
        path[1] = address(0x55d398326f99059fF775485246999027B3197955);//mainnet
        // path[1] = address(0x114061e2a2f677D5902AB5B35706c3394C7195B9);//testnet
        path[2] = address(0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c);//mainnet BTC
        // path[2] = address(0xE1F3311C05dbC0595c9A219Fc4422912A157e007);//test BTC
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
        ...
        if(buySize>0){
            ...
            if(buySize >20000){
                for(uint256 i=0;i<20000;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(100000).div(totalAmount);
                        uint256 amountSafe = thisAmount.mul(rate).div(100000);
                        if(amountSafe>10**17){
                            ETH.transfer(user,amountSafe);
                        }
                    }
                }
            }else{
      ...
      function _splitOtherTokenSecondForBTC(uint256 thisAmount) private {
        uint256 buySize = buyUser.length;
        if(buySize>0){
            address user;
            uint256 totalAmount = pair.totalSupply();
            ...
            if(buySize >20000){
                for(uint256 i=0;i<20000;i++){
                    ...
                    if(balanceOf(user) >= 0){
                        rate = pair.balanceOf(user).mul(100000).div(totalAmount);
                        uint256 amountBTC = thisAmount.mul(rate).div(100000);
                        if(amountBTC > 10**14){
                            BTCB.transfer(user,amountBTC);
                        }
                    }
                }
            }else{
    ...
    function _splitOtherToken() public {
        uint256 thisAmount = ETH.balanceOf(address(this));
        uint256 thisAmountOfBTCB = BTCB.balanceOf(address(this));
        
        if(thisAmount >= 10**15 || thisAmountOfBTCB >= 10**15){
            _splitOtherTokenSecond(thisAmount);
            _splitOtherTokenSecondForBTC(thisAmountOfBTCB);
