...
// Thirdweb Contracts v3.1.10 (contracts/marketplace/Marketplace.sol)
...
import "./interfaces/token/IFNDNFT721.sol";

contract BLSMarketplace is
    Initializable,
    IMarketplace,
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("BLSMarketplace");
    ...
    /// @dev Default action duration in seconds
    uint256 private defaultAuctionDuration;
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        } else if (startTime < block.timestamp && tokenTypeOfListing == TokenType.ERC1155) {
        ...
        uint256 endTime = _params.secondsUntilEndTime > 0 && startTime >= block.timestamp ? startTime + _params.secondsUntilEndTime : 0;
    ...
    function updateListing(
        ...
        if (isAuction) {
            require(targetListing.startTime == 0 || block.timestamp < targetListing.startTime, "STARTED");
            require(_buyoutPricePerToken >= _reservePricePerToken, "RESERVE");
        }

        if (_startTime < block.timestamp && targetListing.tokenType == TokenType.ERC1155) {
            // do not allow listing to start in the past (1 hour buffer)
            // require(block.timestamp - _startTime < 1 hours, "ST");
            _startTime = block.timestamp;
        }

        uint256 newStartTime = _startTime > 0 ? _startTime : targetListing.startTime;
        uint256 newEndTime = _secondsUntilEndTime == 0 && newStartTime >= block.timestamp ? targetListing.endTime : newStartTime + _secondsUntilEndTime;
    ...
    /// @dev Lets a direct listing creator cancel their listing.
    function cancelDirectListingByAdmin(uint256 _listingId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Listing memory targetListing = listings[_listingId];
        ...
        emit ListingRemovedByAdmin(_listingId, targetListing.tokenOwner);
    }
    ...
    /// @dev Lets an account buy a given quantity of tokens from a listing. Only for listing type Direct
    function buy(
        uint256 _listingId,
        address _buyFor,
    ...
    function offer(
        ...
        require(
            (targetListing.endTime > block.timestamp || targetListing.endTime == 0)
        ...
        if (targetListing.listingType == ListingType.Auction) {
            ...
            if (targetListing.endTime == 0) {
                targetListing.startTime = block.timestamp;
                targetListing.endTime = block.timestamp + defaultAuctionDuration;
    ...
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid) internal {
        ...
        if (
            ...
            if (_targetListing.endTime - block.timestamp <= timeBuffer) {
                ...
                listings[_targetListing.listingId].endTime = block.timestamp + timeBuffer;
            }
        }
    ...
    function payout(
        ...
        if (royaltyRecipient == address(0)) {
            (royaltyRecipient, royaltyCut) = _getCreator(_listing.assetContract, _listing.tokenId, _totalPayoutAmount);
        }
   ...
   */
    function _getCreator(address nftContract, uint256 tokenId, uint256 _totalPayoutAmount)
    internal view returns (address royaltyRecipient, uint256 royaltyCut)
    {
        try IFNDNFT721(nftContract).tokenCreator(tokenId) returns (address payable creator) {
            ...
            royaltyCut = (_totalPayoutAmount * platformFeeBps) / MAX_BPS;
        } catch { }
    }
    ...
    function validateDirectListingSale(
        ...
        // Check if sale is made within the listing window or perpetual sale
        require((block.timestamp < _listing.endTime || _listing.endTime == 0) && block.timestamp > _listing.startTime, "not within sale window.");
    ...
    /// @dev Returns default action duration.
    function getDefaultAuctionDuration() external view returns (uint256) {
    ...
    function setDefaultAuctionDuration(uint256 _duration) external onlyRole(DEFAULT_ADMIN_ROLE) {
        defaultAuctionDuration = _duration;
    }

    function setETHHandler(address _ethHandler) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ethHandler = _ethHandler;
    }
    ...
    function transferCurrencyWithWrapper(
        ...
        if (_currency == NATIVE_TOKEN) {
            if (_from == address(this)) {
                ...
                // IWETH(_nativeTokenWrapper).transfer(_ethHandler, _amount);
                // IEthHandler(_ethHandler).withdraw(_nativeTokenWrapper, _amount);
...
interface IMarketplace is IThirdwebContract, IPlatformFee {
    ...
    **/
    ...
    /// @dev Emitted when a listing is cancelled.
    event ListingRemovedByAdmin(uint256 indexed listingId, address indexed listingCreator);
