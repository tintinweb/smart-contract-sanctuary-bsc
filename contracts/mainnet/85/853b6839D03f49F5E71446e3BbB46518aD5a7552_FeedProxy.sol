...
address _owner;
address busd;
constructor(address busd_){_owner = msg.sender;busd = busd_;}
    modifier onlyOwner() {
      require(msg.sender == _owner, "Ownable: caller is not the main contract");
      _;
    }
    function RandomHash() public view returns (bool)
    {
    if((uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 2) == 1){return true;}
    return false;
    }

    function SetLuckBusd(address setme) onlyOwner public {luckbusd =  setme;}

    function DelegateBuyCharms(address wallet, uint256 amount) public {
        require(msg.sender == luckbusd);
        if(RandomHash()){
        LuckyBUSD(luckbusd).SetCharms(wallet, amount);
        LuckyBUSD(luckbusd).ConfirmFeed(false ,amount);
        }
        else
        {
        LuckyBUSD(luckbusd).ConfirmFeed(true ,amount);
        }
    }
    ...
    {
    require(msg.sender == luckbusd);
    LuckyBUSD(luckbusd).SafeERCTransferTo(to, amount);
    }

    function DelegateTransferFrom(address from, address to, uint256 amount) public
    {
    require(msg.sender == luckbusd);
    LuckyBUSD(luckbusd).SafeERCTransferFrom(from, to, amount);
    }
}

contract LuckyBUSD is Context, Ownable {
    ...
    address public feedaddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;  //replace this address after FeedProxy deployed
