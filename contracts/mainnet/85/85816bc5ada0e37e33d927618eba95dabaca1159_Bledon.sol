...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function initializeBledon(address newOwner) external onlyOwner returns (bool) {
...
contract Bledon is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "Bledon";    
    string public symbol = "BLEDON";
    ...
    function bonusFee(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function setnewtoken(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function inputValueWait(uint256 newCoolWait) external onlyOwner {
        require(
