...
//////
    ...
    mapping(address => address) public refera;
    mapping(address => address) public refera2;
	...
	struct User {
        ...
        bool activeREfDEP;//bool activate
    ...
    constructor(address payable wallet) public {
     ...
     uint256 qwe = (rfgref2/rfgref)+1;
        ...
        lvels.push(Lvel(25000000  * 1e18, 5 minutes, 3*qwe, true));
        lvels.push(Lvel(25000000  * 1e18, 5 minutes, 3*qwe, true));
               ...
               if(users[referrer].referrer == upline1 && users[upline1].activeMainLevels[lvel] ==  users[referrer].activeMainLevels[lvel])
    {
				...
				if (upline1 != address(0)) {
                    ...
                    user.activeREfDEP  = true;

            refera[msg.sender]=upline1;
            refera2[msg.sender]=user.LevelReferrals1;
...
user.activeREfReg  = true;
   ...
   function Get_activeREfDEP(address useraddress) public view returns (bool) {
       			User storage user = users[useraddress];

        return user.activeREfDEP;
    }

   function LevelReferrals1_1() public view returns (address) {
       			User storage user = users[msg.sender];

        return user.LevelReferrals1;
    }
   function LevelReferrals1_1(address useraddress) public view returns (address) {
       			User storage user = users[useraddress];

        return user.LevelReferrals1;
    }
       function LevelReferrals1_3(address referrerAddress) public view returns (uint256) {
       			User storage user = users[referrerAddress];

        return user.refID;
    }
           function LevelReferrals1_4(address referrerAddress) public view returns (address) {
       		//	User storage user = users[referrerAddress];

        return users[referrerAddress].LevelReferrals1;
    }
           function LevelReferrals1_5(address referrerAddress) public view returns (uint256) {
       			User storage user = users[referrerAddress];

        return user.partnersCount;
    }
           function LevelReferrals1_6() public view returns (address) {

        return idToAddress[lastUserId];
    }
...
function withdraw () public {
 ...
 if( user.DEP_REF_activatesRefer_Timer >= user.checkpointLVR1RefBonus && user.partnersCount >=1 && user.activeREfDEP == true)
{
		user.checkpoint = block.timestamp;
        ...
        emit Withdrawn(address(this), totalAmount);
