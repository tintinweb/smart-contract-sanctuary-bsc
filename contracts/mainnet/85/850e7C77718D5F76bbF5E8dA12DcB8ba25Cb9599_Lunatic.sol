...
contract Lunatic is IERC20, Ownable {
    ...
    uint256 taxLiquidity = 10;
    uint256 giveawayFee = 1;
    uint256 varMarketingFee = 15;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (taxLiquidity, varMarketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeFees(uint256 _taxLiquidity, uint256 _reflectionFee, uint256 _varMarketingFee, uint256 _giveawayFee) external onlyOwner {
        taxLiquidity = _taxLiquidity;
        reflectionFee = _reflectionFee;
        varMarketingFee = _varMarketingFee;
        giveawayFee = _giveawayFee;
        totalFee = taxLiquidity.add(reflectionFee).add(varMarketingFee).add(giveawayFee);
