...
//   https://t.me/recessionchain

//   https://recessionchain.org/#
...
contract Recession is IERC20, Context, Ownable {
    ...
    uint256 private _tTotal = 23000000000 * 10 **_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    
    uint256 public genesis_block;
    
    string private constant _symbol = "RESS";
    string private constant _name = "Recession Chain";

    uint256 public Resi_max_BuyLimit = 4600000000 * 10**9;
    uint256 public Resi_max_SellLimit = 4600000000 * 10**9;

    address private Resi_Only_Wallet = 0xd5Fd58E27e2ba1F84EF7EF727E60e8c7f0370765;
    address private Resi_Team_Wallet = 0x8EA9e682409b4A0C1743B29a9E35788aAB8cb2DC;

    uint256 public Resi_max_WalletLimit = 4600000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Resi_Only_Wallet] = true;
        _isExcludedFromFee[Resi_Only_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Resi_Only_Wallet] = true;
        allowedTransfer[Resi_Only_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Resi_max_BuyLimit, "You are exceeding Resi_max_BuyLimit");
            require(balanceOf(to) + amount <= Resi_max_WalletLimit, "You are exceeding Resi_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Resi_max_SellLimit, "You are exceeding Resi_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Resi_max_WalletLimit, "You are exceeding Resi_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Resi_Only_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Resi_Only_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Resi_Update_MarketingWallet(address newWallet) external onlyOwner{
        Resi_Only_Wallet = newWallet;
    }
    
    function Resi_Update_TeamWallet(address newWallet) external onlyOwner{
        Resi_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Resi_max_BuyLimit = maxBuy * 10**decimals();
        Resi_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Resi_Max_Walletlimit(uint256 amount) external onlyOwner{
        Resi_max_WalletLimit = amount * 10**decimals();
    }
