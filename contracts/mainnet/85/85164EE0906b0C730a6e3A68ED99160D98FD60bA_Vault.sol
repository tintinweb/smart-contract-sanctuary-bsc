...
contract Vault is ERC20, ReentrancyGuard {
    ...
    function poolSize() public view returns (uint256) {
        if (position == 0) {
            return (IERC20(quoteToken).balanceOf(address(this)) + _calculateQuoteFromBase());
        } else {
            return (IERC20(baseToken).balanceOf(address(this)) + _calculateBaseFromQuote());
        }
    }
    ...
    function _calculateBaseFromQuote() internal view returns(uint256) {
        uint256 amountQuote = IERC20(quoteToken).balanceOf(address(this));

        if (amountQuote < SWAP_MIN) {
            return 0;
        }
        uint256[] memory amounts = UniswapRouterV2(pancakeRouter).getAmountsOut(amountQuote, pathForward);
        return amounts[amounts.length - 1];
    }
