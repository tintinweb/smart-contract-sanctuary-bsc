...
// File contracts/MetasnailPrivatePresale.sol


pragma solidity ^0.8.4;
contract PrivatePresale is ReentrancyGuard, Pausable, Ownable {
    ...
    modifier open() {
        require( _whiteList[msg.sender], "Not whitelisted user");
        _;
    }

    uint256 public sellStartAt;
    uint256 public sellEndAt;
    uint256 public claimStartAt;

    uint256 public tokenPerBNB = 150 * 1000 * 10**9; //10000 token per 1 BNB

    uint256 public minPerTransaction = 0; // min amount per transaction

    uint256 public totalSold;

    uint256 public maxMarketCap = 5 ether;
    ...
    modifier checkSaleRequirements(uint256 buyAmount) {
        require(
            block.timestamp >= sellStartAt && block.timestamp < sellEndAt,
            "Presale time mismatch"
        );
    ...
    constructor(address _TOKEN) {
        TOKEN = IERC20(_TOKEN);
        sellStartAt = block.timestamp;
        sellEndAt = block.timestamp + 14 days;
        claimStartAt = block.timestamp + 15 days;
    }
    ...
    function buyToken(uint256 buyAmount)
        external
        payable
        checkSaleRequirements(buyAmount)
        open
        nonReentrant
    {
        ...
        require(
            buyAmount >= minPerTransaction,
            "Lower than the minimal transaction amount"
        );
        ...
        uint256 totalCap = calculateBNBAmount(sumSoFar);

        require(totalCap <= maxMarketCap, "Greater than the maximum cap limit");
    ...
    function claimToken() external nonReentrant {
        require(block.timestamp >= claimStartAt, "Claim time mismatch");
        ...
        emit TokensClaimed(msg.sender, boughtAmount, block.timestamp);
    }
    ...
    // only owner can call this function
    function setClaimStartDate(uint256 _claimStartAt) external onlyOwner {
        claimStartAt = _claimStartAt;
    }

    // function to set the total tokens to sell
    // only owner can call this function
    function setMaxMarketCap(uint256 _maxMarketCap) external onlyOwner {
    ...
    function withdrawCollectedBNB(address recipient, uint256 amount)
        external
        onlyOwner
    {
    ...
    function withdrawUnsoldTokens() external onlyOwner {
        uint256 remainedTokens = TOKEN.balanceOf(address(this));
        require(remainedTokens > 0, "No remained tokens");
        TOKEN.safeTransfer(address(msg.sender), remainedTokens);
    }


    //function to return the amount of unsold tokens
    function unsoldTokens() public view returns (uint256) {
        uint256 remained = TOKEN.balanceOf(address(this));
        if(remained > 0 && remained > totalSold){
            return remained.sub(totalSold);
        }
        return 0;
    }
    ...
    function calculateBNBAmount(uint256 tokenAmount)
        public
        view
        returns (uint256)
    {
        require(
            tokenPerBNB > 0,
            "TOKEN price per BNB should be greater than 0"
        );
        uint256 bnbAmount = tokenAmount.div(tokenPerBNB).mul(10**18);
        return bnbAmount;
    }
