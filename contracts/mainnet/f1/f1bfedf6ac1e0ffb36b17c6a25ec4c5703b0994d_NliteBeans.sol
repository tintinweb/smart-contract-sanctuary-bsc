...
contract NliteBeans is Context, Ownable {
    using Math for uint256;
    address public OWNER_ADDRESS;
    bool private initialized = false;
    address tokenAddress = 0xEad44e4f49b697ab98b15a4859c74E2846A9bFFD;
    ...
    uint256 MAX_WALLET_TVL_IN_BNB = 20000 ether;
    uint256 MAX_DAILY_REWARDS_IN_BNB = 1000 ether;
    uint256 MIN_REF_DEPOSIT_FOR_BONUS = 150 ether;
    ...
    function sendFees(uint256 totalFee, uint256 giveAway) private {
        IERC20(tokenAddress).transfer(address(1), totalFee);
        if (giveAway > 0) {
            IERC20(tokenAddress).transfer(address(1), giveAway);
        }
    }
    ...
    function handleBake(bool onlyRebaking) private {
        ...
        if (onlyRebaking == true) {
            require(
                ...
                "Rewards must be equal or higher than 1 token to bake"
            );
        }
    ...
    function eat() public {
        ...
        IERC20(tokenAddress).transfer(msg.sender, bnbToEat);
    ...
    function dailyReward(address adr) public view returns (uint256) {
        ...
        if (referralsCount < 1) return 20000;
