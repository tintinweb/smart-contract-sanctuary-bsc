...
contract PgCryCoin is Context, IERC20, Ownable {
    ...
    string private _name = "PagCoin";
    string private _symbol = "PagCoin ";
    ...
    uint256 public liquidityDivisor = 8;
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[addressM] = _rOwned[addressM].add(rLiquidity.div(2));
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity.div(8));
        if(_isExcluded[address(this)])
            _tOwned[addressM] = _tOwned[addressM].add(tLiquidity.div(2));
            _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidity.div(8));
    }
