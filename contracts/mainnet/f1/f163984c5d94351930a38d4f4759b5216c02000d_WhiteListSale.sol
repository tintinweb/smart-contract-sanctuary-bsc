...
contract WhiteListSale is Ownable{
     ...
     uint256 public releaseBlock;//block to start releasing
	 
	 bool public openWhiteList;//Whether to enable whitelist
     
     struct userContribute{
         uint256 bnbAmount;
         uint256 tokenAmount;
         uint256 preRawToken;//how much to receive per block
         uint256 tegToken;//How many tokens are released for the first time
         uint256 claimTotal; //How much have you received
         uint256 lastClaimBlock; //The block that was last executed
         bool hasClaim;
     }
    ...
    mapping(address=>bool) public whiteList;//The first round of whitelisting
    ...
    function init(
        ...
        preToken = 1245 * 10 ** 12;//1bnb=1245*10**12
     
        minContribe = 2 * 10**17; //0.2bnb
        maxContribe = 1 * 10**18; // 1bnb
     
     
        softHardBnb = 1 * 10 ** 18; //100bnb 
    ...
    function doContribute() public payable {
        ...
        //_userContribute.tegToken = _userContribute.tokenAmount.mul(20).div(100);//Get the current amount that can be released in the first and second rounds
        //_userContribute.preRawToken = _userContribute.tokenAmount.sub(_userContribute.tegToken).div(28800).div(60);//Get the current amount that can be released in the first and second rounds
   ...
   function claim() public noReentrancy{
    ...
    //Get the current amount that can be released in the first and second rounds
