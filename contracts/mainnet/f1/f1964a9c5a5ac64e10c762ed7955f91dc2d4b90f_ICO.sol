...
contract ICO is Initializable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        address _priceFeedAddress) public initializer {
    ...
    function setDiscount(uint256 _discount) external onlyOwner {
        require(_discount > 0, "Discount Invalid");
        discount = _discount;
    }

    function recall1() external onlyOwner {
        ...
        require(sent, "Fail To Sent BNB");
    }

    function recall2() external onlyOwner {
        token.transfer(owner(), token.balanceOf(address(this)));
    }
    ...
    function buyByBnb(address _referrer) external payable {
        ...
        uint256 stablePerBnb = uint256(getLatestPrice());
        ...
        if (_referrer != address (0)) {
            (bool sent, ) = _referrer.call{value: msg.value * discount / 100}("");
            require(sent, "Transfer Fail");
        }

        emit BuyICO(0, msg.value, buyAmount, _referrer);
    }
    ...
    function buyByBusd(uint256 _amount, address _referrer) external {
        ...
        if (_referrer != address (0)) {
            busdToken.transfer(_referrer, _amount * discount / 100);
        }

        emit BuyICO(1, _amount, buyAmount, _referrer);
    }
    ...
    function buyByUsdt(uint256 _amount, address _referrer) external {
        ...
        if (_referrer != address (0)) {
            usdtToken.transfer(_referrer, _amount * discount / 100);
        }

        emit BuyICO(2, _amount, buyAmount, _referrer);
    }

    function validate(uint256 _amount) internal view {
