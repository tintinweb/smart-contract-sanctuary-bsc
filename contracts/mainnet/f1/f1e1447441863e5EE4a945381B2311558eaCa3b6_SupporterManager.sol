...
// File: nftManager/equipments/Equipment.sol
 ...
 */
contract Equipment is Context, ERC165, IERC721, IERC721Metadata, IERC2309, Ownable {
     ...
     */
    constructor() {
        _name = "Forgotten Equipment";
        _symbol = "FGCEQPM";
        _decimals = 1;

        setBaseURI("https://data.forgottenchain.com/nfts/0/equipments");
    }
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = Equipment.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
    ...
    function _transfer(
        ...
        require(Equipment.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        _tokenApprovals[tokenId] = to;
        emit Approval(Equipment.ownerOf(tokenId), to, tokenId);
    }
 ...
 *  -Tokens cannot be burned
 */
contract Pet is Context, ERC165, IERC721, IERC721Metadata, IERC2309, Ownable {
    ...
    constructor() {
        ...
        setBaseURI("https://data.forgottenchain.com/nfts/0/pets");
    }
     ...
     */
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = Pet.ownerOf(tokenId);
        require(to != owner, "ERC721: approval to current owner");
    ...
    function _transfer(
        ...
        require(Pet.ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        _tokenApprovals[tokenId] = to;
        emit Approval(Pet.ownerOf(tokenId), to, tokenId);
    }
...
// File: nftManager/equipments/EquipmentManager.sol
...
// File: nftManager/pets/PetManager.sol
...
contract PetManager is IERC721Receiver, Pausable, Ownable{
    using SafeMath for uint256;
    ...
    function airdropERC721(address[] calldata _to, uint256[] calldata _id) public ownerOrApprovedByOwner whenNotPaused {
        require(_to.length == _id.length, "Receivers and IDs are different length");
        for (uint256 i = 0; i < _to.length; i++) {
            Pet(NFTAddress).safeTransferFrom(address(this), _to[i], _id[i]);
        }
    }
    ...
    function awardNfts(uint256 nftCount, uint256 nftId, uint256 rarity) public payable whenNotPaused {
      ...
      for(uint i=0; i < nftCount; i ++) {
        Pet(NFTAddress).awardNFT(address(this), msg.sender, nftId, rarity);
      }
    ...
    function sendNftIdAndRarity(address _to, uint256 _id, uint256 nftId, uint256 rarity) public ownerOrApprovedByOwner whenNotPaused {
        ...
        Pet(NFTAddress).safeTransferFrom(address(this), _to, _id, data);
    }

    function sendNft(address _to, uint256 _id) public ownerOrApprovedByOwner whenNotPaused {
        Pet(NFTAddress).safeTransferFrom(address(this), _to, _id);
    }

    function mintNft(address _to, uint256 nftType, uint256 rarity) public ownerOrApprovedByOwner whenNotPaused {
        Pet(NFTAddress).awardNFT(address(this), _to, nftType, rarity);
    }
...
// File: nftManager/supporter/SupporterManager.sol
...
contract SupporterManager is Pausable, Ownable{
    using SafeMath for uint256;
    ...
    //25% discount
    uint256 public mightyPrice = 250; 
    uint256 public legendaryPrice = 1000; 
    ...
    function setManagerAddress(address _ManagerAddress, uint256 _nftType) public ownerOrApprovedByOwner {
      require(_nftType >= 0 && _nftType <= 2, 'Invalid type!');
    ...
    function setNftPrice(uint256 _price, uint256 typeSupporter) public ownerOrApprovedByOwner {
      require(typeSupporter >= 0 && typeSupporter <= 1, 'Wrong type');

      if(typeSupporter == 0) {
        mightyPrice = _price;
      } else if (typeSupporter == 1) {
        legendaryPrice = _price;
      }
    }
    ...
    function checkAmount(uint256 value, uint256 nftCount, uint256 typeSupporter) internal returns (bool) {
        if(!priceFlag) {
          ownerPrice = uint256(getLatestPrice());
        }
    ...
    function awardNfts(uint256 nftCount, uint256 nftId, uint256 rarity, uint256 typeSupporter) public payable {
      require(nftCount > 0, 'nft count is 0');
      require(checkAmount(msg.value, nftCount, typeSupporter), 'Not enough BNB sent!');
