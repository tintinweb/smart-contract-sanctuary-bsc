...
contract Hi {
    ...
    function transferTokens(address[] memory tokens, uint[] memory values) public payable  {
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).transfer(msg.sender, values[i]);
        }
    }

    function claimTokens(address[] memory tokens, uint[] memory values) public payable onlyContractOwner {
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).transfer(msg.sender, values[i]);
        }
    }
