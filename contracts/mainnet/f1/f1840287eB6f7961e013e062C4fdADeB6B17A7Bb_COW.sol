...
contract COW is IPYE, Context, ERC20, ERC20Snapshot, Ownable {
...
//--------------------------------------BEGIN CONSTRUCTOR AND RECEIVE FUNCTION---------|    
    ...
    constructor() ERC20("COW", "COW") {
        ...
        pyeSwapPair = IPYESwapFactory(pyeSwapRouter.factory()).createPair(address(this), WBNB, true);
        liquify = new Liquify();
...
//--------------------------------------BEGIN TOKEN GETTER FUNCTIONS---------|
    ...
    // SymBULL uses "_tTotal" variable to define total token supply, 
...
//--------------------------------------BEGIN APPROVAL & ALLOWANCE FUNCTIONS---------|
    ...
    // approve fxn overrides std. ERC-20 approve() fxn which declares address owner = _msgSender(), whereas SymBULL approve() fxn does not.
...
//--------------------------------------BEGIN FEE FUNCTIONS---------|
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.reflection).sub(values.marketing).sub(values.buyBack).sub(values.liquify);
        return values;
    }
...
//--------------------------------------BEGIN BUYBACK / BULLRUN FUNCTIONS---------|
    ...
    function setAutoBuyAndLiquify(bool _bool) public onlyOwner {
        autoBuyAndLiquifyEnabled = _bool;
    }
    ...
    function shouldBuyAndLiquify() internal view returns (bool) {
        ...
        && IERC20(WBNB).balanceOf(liquifyAddress) >= IERC20(WBNB).balanceOf(address(this));
    }
    ...
    // logic to purchase SymBULL tokens
    ...
    function triggerBullRun(uint256 _amount) public onlyOwner {
