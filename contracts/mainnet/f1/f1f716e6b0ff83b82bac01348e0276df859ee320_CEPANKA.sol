...
contract CEPANKA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public reflectionToken = address(0x000000000000000000000000000000000000dEaD); // dividend tracker smart contract
    
    address public devWallet =address(0x1DC6B493D7c24c5e18B2907849E3E0F7D5D7BFeC); // Dev wallet 
    address public gameWallet = address (0xDFcbD1F369c04AA89ce6915b1Fae24cbB3D081C0); // gameWallet
    address public constant burnWallet = address(0x000000000000000000000000000000000000dEaD); // Burn wallet

    uint256 public swapTokensAtAmount = 5 * 1e18; // 1 * 1e4 * 1e18; 10000 tokens min for swap
    ...
    constructor() ERC20("CEPKA Token", "CEPKA") {
        dividendTracker = new TOKENDividendTracker(reflectionToken);
        ...
        //excludeFromFees(gameWallet, true);
        ...
        */
        _mint(owner(), 100 * 1e18); // Total Supply: 100 million tokens.

        maxTxAmount = totalSupply().mul(100).div(1000); // MaxTxAmount set to 1% of the total supply.
        maxWallet = totalSupply().mul(100).div(1000);//  MaxWalletAmount set to 1% of the total supply.
    }
   ...
   function setWallets(address _dev, address _game ) external onlyOwner {
        devWallet = _dev;
        gameWallet = _game;
    }
    ...
    function setBuyFees(
        ...
        buyFee.dev = _dev;        
        buyFee.game = _game;
        buyFee.reflections = _reflections;

        totalBuyFee = buyFee.autoLP + buyFee.dev + buyFee.game + buyFee.reflections;
       // Max buy Fees limit is 20 percent, 10000 is used a divisor
       // used only at launch, after that goes to 0
    ...
    function setSellFees(
        ...
        sellFee.dev = _dev;        
        sellFee.game = _game;
        sellFee.reflections = _reflections;

        totalSellFee = sellFee.autoLP + sellFee.dev + sellFee.game + sellFee.reflections;
       //Max sell Fees limit is 12 percent, 10000 is used a divisor
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 swapTokens = tokens.mul((buyFee.autoLP + sellFee.autoLP)/2
                                        + buyFee.dev + sellFee.dev + buyFee.game + sellFee.game + buyFee.reflections + sellFee.reflections).div(totalFee);
        ...
        uint256 gamePart = newBalance.mul(buyFee.game + sellFee.game).div(totalFee - feePart);    
        uint256 devPart = newBalance.mul(buyFee.dev + sellFee.dev).div(totalFee - feePart);
        uint256 reflectionPart = newBalance.mul(buyFee.reflections + sellFee.reflections).div(totalFee - feePart);  
                         
        if (reflectionPart > 0){
            sendDividends(reflectionPart); 
        }       
        if (devPart > 0){
            sendToWallet(payable(devWallet), devPart);
        }
        if (gamePart > 0) {
            sendToWallet(payable(gameWallet), gamePart);
        }
    
        if (address(this).balance > 0){
        
        addLiquidity(liqTokens, newBalance - gamePart - devPart - reflectionPart);
        }
    }
    ...
    function swapTokensForReflectionToken(uint256 amount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = reflectionToken;
    ...
    function sendDividends(uint256 tokens) private {
        uint256 initialReflectionTokenBalance = IERC20(reflectionToken).balanceOf(address(this));

        swapTokensForReflectionToken(tokens);

        uint256 newBalance = (IERC20(reflectionToken).balanceOf(address(this))).sub(
            initialReflectionTokenBalance
        );
...
contract TOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address rewardToken)
        DividendPayingToken("CEPANKA_Dividend_Tracker", "CEPANKA_Dividend_Tracker", rewardToken)
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 20000+ tokens to get rewards
    }
