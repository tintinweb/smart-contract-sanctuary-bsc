...
contract PartnerEnglishAuction is
    Initializable,
    OwnableUpgradeable,
    ...
    IERC721ReceiverUpgradeable
{
    ...
    function _setupAuction(
        ...
        nftContractAuction.userFeeRecipient = userFeeRecipient;
        nftContractAuction.userFeePercentage = userFeePercentage;
        ...
        internal
    {
        ...
        nftContractAuction.userFeePercentage = 0;
        nftContractAuction.userFeeRecipient = address(0);
    ...
    function _transferNftAndPaySeller(
        ...
        if (nftContractAuction.nftHighestBid != 0) {
            _payFeesAndSeller(
                ...
                nftContractAuction.ERC20Token,
                nftContractAuction.userFeeRecipient,
                nftContractAuction.userFeePercentage
            );
            IERC721Upgradeable(nftContractAddress).safeTransferFrom(
                address(this),
    ...
    function _payFeesAndSeller(
        ...
        uint256 outerfee = _getPortionOfBid(highestBid, defaultFeePercentages);
        uint256 userFee;
        if (userFeeRecipient != address(0) && userFeePercentatge != 0) {
            userFee = _getPortionOfBid(
                highestBid,
                userFeePercentatge
            );
            _payout(
        ...
        _payout(defaultFeeRecipient, outerfee, erc20Token);
        if ((highestBid - outerfee - userFee) > 0){
            _payout(
                nftSeller,
                (highestBid - outerfee - userFee),
                erc20Token
            );
        }
    }
