...
contract SSPF is ERC20 {
	...
	mapping(address => uint256) private sellTime;
	...
	uint256 total;
    constructor(address tokenOwner) ERC20("SSPF", "SSPF") {
	...
	function changeSwapWarp(CDAOWarp _warp) public onlyOwner {
        warp = _warp;
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**15){
			uint256 canswap = allAmount.div(7).mul(6);
			...
			if(allusdtAmount > usdtAmount){
				...
				addLiquidityUsdt(newUsdtAmount.div(6),otherAmount);
			}
		}
    }
        ...
        if(buySize>0 && sendAmount > 0 && totalAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
					...
					if(sellTime[user].add(600) < block.timestamp && balanceOf(user) >= 10**24){
						rate = balanceOf(user).mul(10000000000).div(totalAmount);
						uint256 amountUsdt = sendAmount.mul(rate).div(10000000000);
						if(amountUsdt>10**24){
