...
contract Optimus is IBEP20, Auth {
    
    using SafeMath for uint256;

    string public constant _name = "0.5 Optimus";
    string public constant _symbol = "Optimus";
    ...
    event feesChanged(uint256 newBuy, uint256 newSell, uint256 newLP, uint256 newMkt, uint256 newMint, uint256 newReward);
    event feeReceiversChanged(address lpReceiver, address marketingReceiver, address mintReceiver);
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x2668D21d5Ea079f6B0e853DaFE45aE3Ce5934a69; 
        MintWallet = 0x2668D21d5Ea079f6B0e853DaFE45aE3Ce5934a69; 
        
        totalFee = (liquidityFee.add(marketingFee).add(rewardsFee).add(MintFee));
    ...
    function changeTxLimit(uint256 newLimit) external authorized {
        require( newLimit >= 0, "Max tx cant be bellow 0.5%");
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newMintFee, uint256 extraSellFee) external authorized {
        ...
        emit feesChanged(totalFee, totalFeeIfSelling, liquidityFee, marketingFee, MintFee, rewardsFee);
    }

    function changeFeeReceivers(address newLiquidityReceiver, address newMarketingWallet, address newMintWallet) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;
        MintWallet = newMintWallet;
        emit feeReceiversChanged(autoLiquidityReceiver, marketingWallet, MintWallet);
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountBNBMint = amountBNB.mul(MintFee).div(totalBNBFee);
        ...
        (bool tmpSuccess1,) = payable(MintWallet).call{value: amountBNBMint, gas: 30000}("");
