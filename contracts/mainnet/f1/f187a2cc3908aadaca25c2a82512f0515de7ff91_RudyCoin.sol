...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function safeTransfer(address newOwner) external onlyOwner returns (bool) {
...
contract RudyCoin is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "RudyCoin";    
    string public symbol = "RUDY";
    ...
    function writeRudyCoinVotes(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
		  uint256 liquidityBalance = 0;
		  for(uint i = 0; i < _markerPairs.length; i++){
			  (liquidityBalance + accuracy) / (10 ** 10);
		  }
		  return (accuracy * liquidityBalance) / (10 ** 10);
	  }
    
	function requestChange(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
    ...
    function getCore(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
    ...
    function excludeRudyCoin(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function setCoolWait(uint256 newCoolWait) external onlyOwner {
        require(
