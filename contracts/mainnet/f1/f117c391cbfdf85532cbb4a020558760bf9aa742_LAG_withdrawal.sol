...
contract LAG_withdrawal is Ownable{
  ...
  address public sub_owner;
       ...
       function changeTax(uint256 amount) external onlyOwner 
   {
    ...
    function claimAirdrop(uint256 _dolAmt) external {
        ...
        require(orignalValue>=0,"You are not claimer");
        require(Token.balanceOf(address(this))>=orignalValue,"Contract is ran out of funds!");
        require(block.timestamp>=lastClaim[msg.sender]+time,"Can't claim twice in given one epoch!");
        Token.transfer(msg.sender, orignalValue);
