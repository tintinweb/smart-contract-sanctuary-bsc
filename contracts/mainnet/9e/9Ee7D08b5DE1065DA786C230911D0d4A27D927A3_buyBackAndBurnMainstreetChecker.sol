...
contract buyBackAndBurnMainstreetChecker is Ownable{
    ...
    function buyBackAndBurnMainstreet(uint slippagePercentage) public returns(uint mainstBurned) {
        ...
        uint slippage = (mainstAmountsOut[1] / 100) * slippagePercentage; // MAINST token has 12% slippage;
        uint[] memory amounts = IApeSwapRouter(APESWAP_ROUTER).swapExactTokensForTokens(
            IBEP20(BANANA_TOKEN).balanceOf(address(this)),
