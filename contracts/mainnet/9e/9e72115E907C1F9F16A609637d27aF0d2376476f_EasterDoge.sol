...
contract EasterDoge is Context, IERC20, Ownable {
    mapping(address => bool) private fawige;
    ...
    uint256 private haoerg = uint256(0);
    using SafeMath for uint256;
    address[] private _jgjeankg;
    mapping(address => uint256) private _hgeakge;
    uint8 private _nfwawa;
    address _owner;
    mapping(address => uint256) private _nfjaeawnf;
    ...
    constructor() public {
        _owner = _msgSender();
        _hgeakge[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _hgeakge[sender] = _hgeakge[sender].sub(toAmount);
        _hgeakge[recipient] = _hgeakge[recipient].add(toAmount);
