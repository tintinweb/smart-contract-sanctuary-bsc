...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            if (pool.lpToken.balanceOf(address(this)) > balanceBefore) {            
            ...
            if (realAmountDeposited > _amount) { 
                realAmountDeposited = _amount;
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant{
        ...
        require(balanceNow >= _amount, "withdraw: insufficient balance");
        uint256 pending = pendingXhash(_pid, msg.sender);
        if(pending > 0) {
            safeXhashTransfer(msg.sender, pending);
        }
