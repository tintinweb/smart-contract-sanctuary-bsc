...
contract UberTaxiController8 is Ownable {
    ...
    event SynchData(address from, uint256 vehicleNum);
    ...
    function synchData() public {
        ...
        require(!isUsers[msg.sender], "Your vehicles have been imported");
        ...
        (uint256 vehicleNum,,,,uint256 totalDeposit,,bool isEffectiveUser) = UTC.users(msg.sender);

        uint256[6] memory vehicleTypeNum = UTC.queryVehicleTypeNum(msg.sender);

        UTD.setUsers(msg.sender,vehicleTypeNum,vehicleNum, address(0), 0, 0, totalDeposit, 0, isEffectiveUser);

        if(isEffectiveUser){
            UTD.addEffectiveUserNum(msg.sender);
        }

        emit SynchData(msg.sender, vehicleNums);
    }
