...
contract THESTAR is IERC20, Ownable {
    ...
    address public vault = 0x208EcB4Daf451719c11C55a471A4989AcFB8227a;
    address public dividend = 0x9509DdC8fe61b075bb10B5456C232b2f943978c4;
    address public USDT = 0x55d398326f99059fF775485246999027B3197955;

    uint256 public constant BASE_RATIO = 10**18;
    uint256 public constant SPY = (108 * BASE_RATIO) / 10000 / 1 days;
    uint256 private _tTotal = 9600 * 10**8 * BASE_RATIO;
    uint256 swapTokensAtAmount = 1 * (10**18);
    ...
    string private _name = "THESTAR";
    string private _symbol = "THESTAR";
    uint8 private _decimals = 18;

    uint256 public _LPFee = 200;
    uint256 private _previousLPFee;
    uint256 public _rewardPollFee = 600;
    uint256 private _previousRewardPollFee;
    uint256 public _topHoldersFee = 100;
    uint256 private _previousTopHoldersFee;
    ...
    constructor(address tokenOwner) {
        ...
        setRewardBlacklist(uniswapV2Pair, true);
        ...
        _isExcludedFromFee[dividend] = true;
        _isExcludedFromFee[tokenOwner] = true;

        isDividendExempt[address(this)] = true;
        isDividendExempt[address(0)] = true;
        isDividendExempt[vault] = true;
        isDividendExempt[dividend] = true;
        isDividendExempt[tokenOwner] = true;
    ...
    function totalSupply() public view override returns (uint256) {
        return _tTotal.add(extraSupply);
    }

    function balanceOf(address account) public view override returns (uint256) {
        return _tOwned[account].add(getReward(account));
    }
    ...
    function removeAllFee() private {
        ...
        _previousRewardPollFee = _rewardPollFee;
        _previousTopHoldersFee = _topHoldersFee;
    ...
    function restoreAllFee() private {
        ...
        _rewardPollFee = _previousRewardPollFee;
        _topHoldersFee = _previousTopHoldersFee;
    }
    ...
    function _takeRewardPollFee(address sender, uint256 tAmount) private {
        if (_rewardPollFee == 0) return;
        _tOwned[vault] = _tOwned[vault].add(tAmount);
        emit Transfer(sender, vault, tAmount);
    }

    function _takeTopHoldersFee(address sender, uint256 tAmount) private {
        if (_topHoldersFee == 0) return;
        _tOwned[dividend] = _tOwned[dividend].add(tAmount);
        emit Transfer(sender, dividend, tAmount);
    }
    ...
    function _transferStandard(
        ...
        uint256 _extraFee = 0;
        if (
            block.timestamp.sub(startTime) <= 10 minutes && _rewardPollFee != 0
        ) {
            _extraFee = 3000;
        }
        ...
        _takeRewardPollFee(
            sender,
            tAmount.div(10000).mul(_rewardPollFee + _extraFee)
        );

        if (sender == uniswapV2Pair) _topHoldersFee = 0;
        _takeTopHoldersFee(sender, tAmount.div(10000).mul(_topHoldersFee));
    ...
    function getReward(address account) public view returns (uint256) {
        ...
        return
        _tOwned[account].mul(SPY).div(BASE_RATIO).mul(
    ...
    modifier calculateReward(address account) {
        if (account != address(0)) {
            ...
            if (reward > 0) {
                _tOwned[account] = _tOwned[account].add(reward);
