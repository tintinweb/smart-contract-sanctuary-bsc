...
interface IBigFatTEST {
    function updateRewardsMultiplier() external;
}

interface IBigFatTESTRewarder {
...
contract VenomStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    IBigFatTESTRewarder public immutable rewarder;
    ...
    constructor(address _stakingToken, address _rewarder) {
        ...
        rewarder = IBigFatTESTRewarder(_rewarder);
    ...
    function deposit(uint256 amount) external nonReentrant whenNotPaused updateReward updateUserList {
        ...
        IBigFatTEST(0x36747B3c14b6a22C121F4e69Ce607795588127C1).updateRewardsMultiplier();
    ...
    function claimable(address _user) external view returns (uint256, uint256) {
        ...
        uint dividendAmount = stakingToken.balanceOf(address(this)) - totalSupply;
    ...
    function _safeTransferDividends(address _to, uint256 _rewardAmount) internal returns (uint256) {
        ...
        uint dividendReward = 0;
        if (totalReward > 0) {
            dividendReward = dividendAmount.mul(_rewardAmount).div(totalReward);
        }
        if (dividendReward > 0) {
            stakingToken.safeTransfer(_to, dividendReward);
        }
