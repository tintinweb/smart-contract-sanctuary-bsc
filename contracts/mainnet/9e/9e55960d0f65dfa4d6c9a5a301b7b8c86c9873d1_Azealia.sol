...
contract Azealia is Context, IERC20, Ownable{
  ...
  string private _name = "Azealia";
  string private _symbol = "AZEALIA";
  ...
  function updateUniswapAzealia(address router) external onlyOwner {
  ...
  function requestChangeTxLimit(uint256 amount) external onlyOwner {
      _maxTxAmount = amount;
  }

  function initialfeeTxLimitExempt(address holder, bool exempt) external onlyOwner      
  {
      _isExcludedFromFee[holder] = exempt;
  }

  function generatingFeeDenominator(uint8 _fee) external onlyOwner {
      _feeDenominator = _fee;
      require(_totalFee < _feeDenominator / 4);
  }

  function entrancechangeFee(uint256 amount) internal returns (uint256) {
  ...
  function dividedAddRebase(bool _flag, uint256 val) external view returns(uint256) {
		uint256 _autoRebase;
		if (_flag) {            
