...
abstract contract BAIZEToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        USDT.transferFrom(
            address(_tokenDistributor),
            fundAddress,
            (usdtBalance * remainingFee) / _txFee
        );
        USDT.transferFrom(
            address(_tokenDistributor),
            address(this),
            (usdtBalance * _LPDividendsFee) / _txFee
        );
    }
    ...
    function processLP(uint256 gas) private {
        ...
        //交易对没有余额
        uint256 totalPair = _usdtPair.totalSupply();
        if (0 == totalPair) {
            return;
        }
    ...
    //批量空头 格式 ["   ","   "]
    function airdrop(address[] memory airdropAddress, uint256 airdropAmount)
        public
    {
