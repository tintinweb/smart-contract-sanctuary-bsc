...
contract Dcglobo {
	...
	uint[12] public ref_bonuses = [500,200,100,50,50,25,25,25,25,10,10,10]; 
	uint[12] public requiredDirect = [1,2,2,3,3,5,5,5,5,5,5,5];

	uint[12] public roi_level_bonuses = [5000,1000,1000,1000,300,300,300,300,300,100,100,100];
	...
	constructor(address payable _takeone, address payable _taketwo, address payable _takethree, address payable _admin, Dcglobov1 _address, Dcglobov1 _address2) public {
		takeone = _takeone;
		taketwo = _taketwo;
	...
	function migrateUserDeposite(address _userAddress) internal{
		...
		(uint256 _amount, uint256 _Withdrawal,uint256 _start,bool _end, uint256 _roi) = Dcglobov.getUserDepositInfo(_userAddress,i);
	...
	function migrateUserlevel(address _userAddress) internal{
		User storage user = users[_userAddress];
		for (uint i = 0; i < 12; i++) {
		(uint256 refs, uint256 refsBusiness) = Dcglobov_s.getUseruplineInfo(_userAddress,i);
		user.refs[i] = refs;
	...
	function migrateUser(address _userAddress, uint256 _checkpoint,address _referrer,uint256 _bonus,uint256 _match_bonus,uint256 _totalWithdrawn,uint256 _WithdrawnInADay,uint256 _remainingWithdrawn,bool _activeForRoi,bool _activeForWithdraw) external{
		require(msg.sender == admin, 'permission denied!');
...
function migrateDeposite(address _userAddress, uint256 _amount, uint256 _Withdrawal,uint256 _start,bool _end, uint256 _roi, bool _newone) external{
		require(msg.sender == admin, 'permission denied!');
		User storage user = users[_userAddress];
		user.deposits.push(Deposit(_amount, _Withdrawal, _start,_end, _roi));
		if(_newone){
	...
	function migrateUserlevel(address _userAddress, uint _i, uint256 refs, uint256 refsBusiness) external{
		require(msg.sender == admin, 'permission denied!');
		User storage user = users[_userAddress];
		user.refs[_i] = refs;
		user.refsBusiness[_i] = refsBusiness;
	}
