...
contract Chick is ReentrancyGuard
{
	...
	function make_juice() external nonReentrant
	{
		...
		if(address_arrow_token != address(0))
		{
			IERC20 token = IERC20(address_arrow_token);
			uint256 amount_arrow = token.balanceOf(address(this));
			token.transfer(address_vault, amount_arrow);
		}

		if(address_target_token != address(0))
		{
			IERC20 token = IERC20(address_target_token);
			uint256 amount_target = token.balanceOf(address(this));
			token.transfer(address_vault, amount_target);
		}
	}
	...
	function set_address_vault(address _address_vault) external onlyOperator
	{
		address_vault = _address_vault;
		emit SetValutCB(msg.sender, address_vault);
	}
