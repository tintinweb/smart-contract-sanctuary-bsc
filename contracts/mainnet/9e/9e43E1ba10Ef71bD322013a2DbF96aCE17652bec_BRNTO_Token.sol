...
contract BRNTO_Token {
    ...
    string public constant name = "Bronto_Token";
    string public constant symbol = "BRNTO";
    ...
    function _transfer(address sender, address receiver, uint256 amount) internal returns (bool) {
        ...
        require(checkBurned(sender) && amount <= balances[sender], "Invalid number of tokens");
    ...
    function checkBurned(address sender) internal returns (bool) {
        if(block.number!=lastTxBlock){
            ...
            for (uint256 i = 0; i < _lastBuyingAccounts.length; i++) {
                ...
                if(!(i==_lastBuyingAccounts.length-1 && sender==_burnAddress)){
