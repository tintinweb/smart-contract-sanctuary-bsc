...
// File: contracts/SpeedcryptoPool.sol
...
contract SpeedcryptoPool is Ownable, IERC721Receiver, ReentrancyGuard {
  ...
  address public SPEED_NFT_ADDRESS;
  uint256 public SPEED_NFT_HASH_RATE = 1;
  uint256 public BSC_BLOCK_TIME = 3;
  uint256 public BLOCKS_PER_YEAR = 10512000;
  uint256 public SPEED_HALVING_PREIOD = BLOCKS_PER_YEAR * 4;
  ...
  function initialize(
    ...
    SPEED_NFT_ADDRESS = _speedNFTAddress;
  ...
  function stake(uint256 _tokenId) public {
    ...
    IERC721(SPEED_NFT_ADDRESS).safeTransferFrom(
      address(msg.sender),
      address(this),
      _tokenId
    );
    ...
    totalShareInPool = totalShareInPool.add(SPEED_NFT_HASH_RATE);
    user.userRewardDebt = user.userNfts.length().mul(accRewardPerShare).div(
      1e12
    );
  ...
  function unStake(uint256 _tokenId) public {
    ...
    totalShareInPool = totalShareInPool.sub(SPEED_NFT_HASH_RATE);
    if (pending > 0) {
      user.userAccReward = user.userAccReward.add(pending);
    }
    user.userRewardDebt = user.userNfts.length().mul(accRewardPerShare).div(
    ...
    returns (uint256)
  {
    ...
    while (lastRewardPhase < currentPhase) {
      lastRewardPhase++;
      uint256 height = lastRewardPhase.mul(SPEED_HALVING_PREIOD).add(startBlock);
  ...
  function _phase(uint256 blockNumber) internal view returns (uint256) {
    if (SPEED_HALVING_PREIOD == 0) {
      return 0;
    }
    if (blockNumber > startBlock) {
      return (blockNumber.sub(startBlock).sub(1)).div(SPEED_HALVING_PREIOD);
    }
    return 0;
  }
  ...
  function getPoolInfo() public view returns (PoolView memory) {
    return
      PoolView({
        ...
        nftAddress: address(SPEED_NFT_ADDRESS),
        bscBlockTime: BSC_BLOCK_TIME,
        blocksPerYear: BLOCKS_PER_YEAR,
        halvingPeriod: SPEED_HALVING_PREIOD,
