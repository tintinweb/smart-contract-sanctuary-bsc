...
contract BengalInu is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BengalInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Bengal Inu", "Bengal") {
        _buyRewardsFee = 2;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 10;
        _buyDevFee = 2;
    	...
    	dividendTracker = new BengalInuDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x4eB8Ee12B7Eb739D14A7D73d12144F8B0C39320E);
        _devWallet = payable(0xaB9BC0993E1F39230DaCF3B253576025fB123376);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Bengal: The dividend tracker already has that address");

        BengalInuDividendTracker newDividendTracker = BengalInuDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Bengal: The new dividend tracker must be owned by the Bengal token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Bengal: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Bengal: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Bengal: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Bengal: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Bengal: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Bengal: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Bengal: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Bengal: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Bengal: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Bengal: Cannot update gasForProcessing to same value");
...
contract BengalInuDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Bengal_Dividend_Tracker", "Bengal_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "Bengal_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bengal contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Bengal_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Bengal_Dividend_Tracker: Cannot update claimWait to same value");
