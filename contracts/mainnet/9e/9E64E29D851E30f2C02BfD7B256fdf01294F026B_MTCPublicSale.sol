...
contract MTCPublicSale is math{
	...
	uint256 public ActiveTime = 1648699200;       //Thu Mar 31 2022 12:00:00 UTC+0800
    ...
    function withdrawToLR(uint xToLR) external onlyManager{
        LRAddr().toPayable().transfer(xToLR);
    }
    ...
    function isClosed() public view returns (bool) {
        return block.timestamp >= _PublicEndTime;
    }
	
    function isRedeemTime() public view returns (bool) {
        return block.timestamp <= _RedeemTime;
    }
	
	function CheckActiveTime() public view returns (uint256) {
        return ActiveTime;
    }
	...
	function setActiveTime(uint xActiveTime) public onlyManager{
	...
	function setPublicSaleAmount(uint xPublicSaleAmount) public onlyManager{
        PublicSaleAmount = xPublicSaleAmount;
    }
    ...
    function PublicSale() external payable{
		require(IsTimeActive() && !isClosed(), "MTC Public Sale : Public sale deadline has arrived.");
    ...
    function setTraitDataGaia(uint _tokenID, uint8 stype, uint publicMTC, uint redeemMTC, address ArinaChain, uint CitizensQuota) external onlyManager {
        IERC20(FPCitizenShip()).setTraitDataGaia(_tokenID, stype, publicMTC, redeemMTC, ArinaChain, CitizensQuota);
    }
}