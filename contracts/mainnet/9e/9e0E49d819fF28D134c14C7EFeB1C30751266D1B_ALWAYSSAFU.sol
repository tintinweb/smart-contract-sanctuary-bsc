...
contract ALWAYSSAFU is ERC20Detailed, Auth {
    ...
    string public _symbol = 'AlwaysSafu';

    IPancakeSwapPair pairContract;
    IERC20 token = IERC20(0x8bea2A2426E010cb340E4Bb1f7F8ceffe395f6aE);
    ...
    uint256 public swapThreshold = 4000 * 10**DECIMALS;
    uint256 public minAmounttoSwap = 40 * 10**DECIMALS;
    ...
    constructor(address _CentralBank, address _marketingReceiver) ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Auth(msg.sender) {
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        if(sender == pair && token.balanceOf(recipient) > minDiscount){
    ...
    function setNFTDiscount(address _address, uint256 _minHoldings) external authorized {
