...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	{
		// this is a costly operation not designed to be used, should be avoided
	...
	function _swapBNBToReward(address _rewardToken, uint256 _bnbAmount) internal
	{
		...
		RewardInfo storage _rewardInfo = rewardInfo[_rewardToken];
		uint256 _rewardAmount = IUniswapV2Router(router).swapExactETHForTokens{value: _bnbAmount}(0, _rewardInfo.path, address(this), block.timestamp)[_rewardInfo.path.length - 1];
