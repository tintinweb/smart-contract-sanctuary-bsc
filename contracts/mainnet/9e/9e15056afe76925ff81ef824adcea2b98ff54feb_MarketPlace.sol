...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function syncRentingOrders(uint[] calldata _ids) external onlyMainAdmin {
    for (uint i = 0; i < _ids.length; i++) {
      require(rentingOrders[IMarketPlace.OrderKind.hero][_ids[i]].rentingDuration > 0, "Order not found");
      rentingOrders[IMarketPlace.OrderKind.hero][_ids[i]].rentingDuration += 7 days;
    }
  }

  function syncCurrentRentedHeroCounter(address[] calldata _addresses, uint[] calldata _count) external onlyMainAdmin {
    for (uint i = 0; i < _addresses.length; i++) {
      currentRentedHeroCounter[_addresses[i]] = _count[i];
    }
  }
}
