...
abstract contract Auth {
    ...
    address public adminOne      = 0xf6783310393EC128730EAC16eD15508a0a7360C0;
    address public adminTokenA   = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA; 
    address public adminTokenB   = 0x864fc813BF7C646628f8eA6283255e5b226a9016;
...
contract BULLS is IBEP20, Auth {
    ...
    address MKT     = 0x408603D8D8Ab36CF3360025b12481052af60bF8A;
    address PROJECT = 0x348c01c061D7B5b4a8d5Aa028C028C8b8b8fBb94;
    address DEV     = 0xB23983E0775cCD2f6df5a59e69eb23a7b5bD44a6;
    address TOKEN_B = 0x39E899dE32e8fe7dBDa1f4147D4297ff66f9dc0C;
    ...
    // INITIAL MAX TRANSACTION AMOUNT SET TO 0,3%
    uint256 public  _maxBuyAmount = (_totalSupply * 3) / 1000;
    ...
    // SETS UP TOTAL FEE
    uint256 public totalFee = liquidityFee.add(marketingFee).add(projectFee).add(devFee).add(burnFee).add(tokenBusinessFee);
    ...
    uint256 public swapThreshold = 1 ether; // 1 token minimum
    bool inSwap;
    ...
    function swapBack() internal swapping {
        ...
        if (totalFeeWithoutBurn > 0 && balanceOf(address(this)) > 0) {
            ...
            if (amountBNB > 0) {
                ...
                uint256 totalBNBFee = liquidityFee.add(marketingFee).add(projectFee).add(devFee).add(tokenBusinessFee);
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _devFee, uint256 _burnFee) external {
        ...
        checkCircuitBreak(msg.sender);        
        checkAuth(msg.sender);
         _tmpTotalFee = _liquidityFee.add(_marketingFee).add(_projectFee).add(devFee).add(_burnFee).add(tokenBusinessFee); 
    ...
    function setTokenBusinessFee(uint256 _tokenBusinessFee, address _tokenBusinessFeeReceiver) external {
        ...
        checkAuthTokenBusiness(msg.sender);
         _tmpTotalFee = liquidityFee.add(marketingFee).add(projectFee).add(devFee).add(burnFee).add(_tokenBusinessFee); 
        require(_tmpTotalFee <= MAX_TOTAL_FEE, "totalFee cant be higher than MAX_TOTAL_FEE");
        if (ZERO == multiSignatureAddress) {
            // SETTING UP MULTI-SIGNATURE
