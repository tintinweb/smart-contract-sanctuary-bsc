...
import "./Storage.sol";
contract transferLP is Storage, Ownable, AccessControl {
  event sdChangeLP(address _user, uint _units, uint _initAmount, uint _amountLP);
  event sdTotalLP(uint _startLP, uint _totalLP, address _newAddress, uint _totalUnits);
  ...
  function migrate(address _logic_contract, address _new_contract) public allowAdmin returns (uint _totalLP) {
    (bool success, ) = address(_logic_contract).delegatecall(abi.encodeWithSignature("harvest()"));
    if (!success) {
        revert("Error calling harvest()");
    }

    migration[] memory migrations = new migration[](mHolders.iQueue.length);
    uint _totalUnits;

    (uint _bal,) =  iMasterChef(exchangeInfo.chefContract).userInfo(iData.poolId,address(this));

    for(uint i = 0; i < mHolders.iQueue.length;i++) {
      address _user = mHolders.iQueue[i];

      bytes memory data;
      (success, data) = address(_logic_contract).delegatecall(abi.encodeWithSignature("userInfo(address)", _user));
      if (success) {
        (,uint _units,,,) = abi.decode(data,(uint256,uint256,uint256,uint256,uint256));
        _totalUnits += _units;

        uint _initAmount = mHolders.iHolders[_user].amount;
    
        uint _lpbal = (_units * _bal)/1e18;
        _totalLP += _lpbal;
        ...
        emit sdChangeLP(_user, _units, _initAmount, _lpbal);
      }
    }

    for(uint i = 0; i < mHolders.iQueue.length;i++) {
      mHolders.iHolders[migrations[i].user].amount = migrations[i].amount;
    }

    iData.poolTotal = _bal;
    logic_contract = _new_contract;
    emit sdTotalLP(_bal, _totalLP,_new_contract,_totalUnits);
    return _totalLP;
  }

  function updatePoolTotal() public allowAdmin {
    (uint _bal,) =  iMasterChef(exchangeInfo.chefContract).userInfo(iData.poolId,address(this));
