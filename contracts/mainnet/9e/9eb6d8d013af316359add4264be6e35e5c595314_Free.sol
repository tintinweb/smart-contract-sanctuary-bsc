...
contract Free is ERC20Detailed, Ownable {
    ...
    21000000 * 10**DECIMALS;

    uint256 public liquidityFee = 50; //5%
    uint256 public treasuryFee = 50;//5%

    uint256 public tradingFee = 150;//15%
    uint256 public firePitFee = 50;//5%
    uint256 public inviteFee = 100;//10%
    ...
    constructor(address _swapRouter,address _freeDaoAddress) ERC20Detailed(_name,_symbol, uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x9C2Fc04C7607794Ed40661102f349fda9c641338;
        treasuryReceiver = msg.sender;
        firePit = 0xE9071E3B6c46fEcD63e2cE1d4aFAC1073793346a;
...
//        }
    ...
    function takeFee(
        ...
        uint256 _totalFee = liquidityFee;
        uint256 _treasuryFee = treasuryFee;
        //sell token
        if (recipient == pair) {
            _totalFee = _totalFee.add(firePitFee).add(treasuryFee);//when sell token .
        ...
        if (sender == pair){
            ...
            for (uint8 i=0;i<_parents.length;i++){
                uint256 _parentFee = gonAmount.mul(25).div(10000);
                if (i==0){
                    _parentFee = gonAmount.mul(500).div(10000);
                }
                if(i==1){
                    _parentFee = gonAmount.mul(200).div(10000);
                }
                if(i==2){
                    _parentFee = gonAmount.mul(100).div(10000);
                }
                if(i==3){
                    _parentFee = gonAmount.mul(50).div(10000);
                }
    ...
    function addLiquidity() internal swapping {
        ...
        uint256 balanceBefore =IERC20(usdtAddress).balanceOf(address(this));
        ...
        uint256 amountUsdtLiquidity = IERC20(usdtAddress).balanceOf(address(this)).sub(balanceBefore);

        if (amountToLiquify > 0&&amountUsdtLiquidity > 0) {
            router.addLiquidity(
                address(this),
                usdtAddress,
                amountToLiquify,
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountUsdtToTreasury = IERC20(usdtAddress).balanceOf(address(this)).sub(balanceBefore);
        IERC20(usdtAddress).transfer(treasuryReceiver,amountUsdtToTreasury);
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no Free token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
