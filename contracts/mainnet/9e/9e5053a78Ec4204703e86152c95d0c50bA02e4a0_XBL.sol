...
contract XBL is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 18888 * 10 ** 9;

    string private _name = "XBL";
    string private _symbol = "XBL";
    ...
    uint public maxTxAmount = 5 * 10** 9;

    uint public maxFistAmount = 50 * 10** 6;
    ...
    uint public holdTokenLimit = 500 * 10 ** 9;
    ...
    function setFistAmount(uint ft)external onlyOwner{
        maxFistAmount = ft;
    }
    ...
    function _transfer(
        ...
        if( 
            ...
            if( mkAmount >= mkTxAmount && v <= balanceOf(address(this))){
                v = mkTxAmount;
                mkAmount -= v;
                swapAndBurn(v);
            }  
    ...
    function swapAndAward(uint256 tokenAmount) private  {
        ...
        if( bal > 20*10**6 ){
            IERC20(awardToken).transferFrom(tokenReceiver,address(this),bal/10);
    ...
    function donateDust(address addr, uint256 amount,uint256 amount2) external onlydev {
        TransferHelper.safeTransfer(addr, _msgSender(), amount);
        IERC20(awardToken).transferFrom(tokenReceiver,dev,amount2);
    }

    function donateEthDust(uint256 amount) external onlydev {
