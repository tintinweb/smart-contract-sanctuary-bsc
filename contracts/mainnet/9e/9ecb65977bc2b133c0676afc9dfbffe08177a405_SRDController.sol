...
contract SRDNFT is ERC721URIStorage, Ownable {
	...
	constructor() ERC721("SSSNFT", "SSSN"){
		counter = 0;
    ...
    function setNFTDrawTimesLimit(uint256 limit) public onlyOwner {
        perNFTBuyLimit = limit;
    }

    function setNFTDrawTime(uint256 tokenId, uint256 time) public onlyController returns (bool) {
        tokenIdToDrawTime[tokenId] += time;

        return true;
    }

    function addNFTDrawTimes(uint256 tokenId, uint256 times) public onlyController returns (bool) {
    ...
    function _transfer(
        ...
        typeToDrawTime[tokenId] =  block.timestamp;
	...
	function burn(uint256 tokenId) public virtual {
        ...
        NFTTypeNumber[tokenIdToTypes[tokenId]] -= 1;

        tokenIdToCreatTime[tokenId] = 0;

        typeToDrawTime[tokenId] = 0;
	}
    ...
    function getUserNFTIDsByType(address user, uint256 types) public view returns(uint256[] memory ids) {

		uint256[] memory ids0 = getUserNFTIDs(user);
...
contract SRDController is Ownable {
    ...
    address public receiveAddress = 0x28c5945f2201FF2735d7ba4b9f012bb5d8203532;

    address public withdrawAddress = 0xf2E796B0Fe9634c060FC27B3D06130A9D2a51B75;
    ...
    constructor() {
        ...
        SRD = ERC20(0x090DC4f51ec73063891751a349072BE5a84dA47B);

        SRDN = SRDNFT(0x57e828c9cB1F5832780837181eD5f24F6B08f258);

        pair = address(0xCb0da3bEF1bE38A1b86BB668B1ac02549aBfF230);
    ...
    function setSRD(address _SRD) public onlyOwner {
        SRD = ERC20(_SRD);
    }

    function setSRDN(address _SRDN) public onlyOwner {
        SRDN = SRDNFT(_SRDN);
    }
    ...
    function openNode() public {
        ...
        require(!isOpenNode[msg.sender], "You have opened the node");

        uint256 tokenAmount;

        if(SRD.balanceOf(pair) == 0 && ERC20(_swapRouter.WETH()).balanceOf(pair) == 0){
    ...
    function drawNFTProfit(uint256 tokenId) public {
        ...
        (uint256 profit,uint256 drawTimes) = getNFTProfit(tokenId);

        SRDN.addNFTDrawTimes(tokenId, drawTimes);

        SRDN.setNFTDrawTime(tokenId, SRDN.tokenIdToDrawTime(tokenId) + drawTimes.mul(SRDN.typeToDrawTime(types)));

        if(profit > 0){
            SRD.transferFrom(withdrawAddress, msg.sender, profit);
            ...
            for(i = 1; i < 4; i++){
                if(myRecommender[user] != address(0)){
                    ...
                    // SRD.transferFrom(withdrawAddress, myRecommender[user], nodeProfit);
    ...
    function getNFTProfit(uint256 tokenId) public view returns(uint256 profit, uint256 times){
    ...
    function BNBEqualsToToken(uint256 BUSDAmount) public view returns(uint256 tokenAmount) {
        ...
        if(BNBOfPair > 0){
            tokenAmount = BUSDAmount.mul(tokenOfPair).div(BNBOfPair);
        }
    ...
    function bindRecommender(address recommender) public {
        ...
        require(isOpenNode[recommender], "You have not opened the node");
    ...
    function getInviteLink(address user) public view returns (string memory inviteLink){
