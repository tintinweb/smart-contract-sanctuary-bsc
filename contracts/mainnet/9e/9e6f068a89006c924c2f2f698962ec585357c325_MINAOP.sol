...
contract MINAOP is Context, IERC20, Ownable {
    ...
    string private _name = "Mina Of OneP";
    string private _symbol = "MNOP";
    ...
    address payable public _marketingWallet = payable(0xfC8B261FAAd4085268Ccc3B110b94346636D0583);
    ...
    function setBuyTaxes( uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) external onlyOwner() {
    ...
    function setSellTaxes( uint256 liquidityFee, uint256 marketingFee, uint256 burnFee) external onlyOwner() {
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(marketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
        require(burnFee <= maxBurnFee); // Prevents owner from abusing fees.
