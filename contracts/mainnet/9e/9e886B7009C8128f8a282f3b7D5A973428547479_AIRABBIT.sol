...
contract AIRABBIT is ERC20, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            'AIRABBIT: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'AIRABBIT: incorrect fee percent');
        require(owner != address(0), 'AIRABBIT: incorrect owner address');
        require(mintToAddr != address(0), 'AIRABBIT: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'AIRABBIT: transfer from the zero address');
        require(recipient != address(0), 'AIRABBIT: transfer to the zero address');
