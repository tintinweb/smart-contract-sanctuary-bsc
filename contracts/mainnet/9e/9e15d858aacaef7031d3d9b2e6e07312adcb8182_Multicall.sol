...
contract Multicall is Ownable,Pausable{
    ...
    function call(address token,bytes[] calldata data,bytes calldata signData) external whenNotPaused {
        require(signVerify(signDecodeParams(signData)), "sign error");
    ...
    function multiCall(callStruct[] memory vars,bytes calldata signData) external whenNotPaused {
        require(callStatus, "unable to access");
        require(signVerify(signDecodeParams(signData)), "sign error");        
