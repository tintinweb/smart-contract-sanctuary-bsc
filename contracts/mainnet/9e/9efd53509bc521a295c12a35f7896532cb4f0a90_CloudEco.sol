...
contract CloudEco is Ownable {
    ...
    mapping(address => bool) public spAdd;
    ...
    modifier onlySpAdd() {
        require(spAdd[msg.sender] == true, " ");
        _;
    }

    constructor(IBEP20 _salsa,IBEP20 _rewardToken,IReferral _referral, address _pancakeRouter, address _ref,address _sc,IBEP20 _usdtToken) public {
    ...
    function pendingReward(uint256 _pid,address _user) public view returns (uint256) {
        ...
        return rew*10000 + user.pendingReward;
    }
    ...
    function harvest(uint256 _pid,uint256 val) public {
        harv(_pid,msg.sender,val);
    }

    function harv(uint256 _pid,address buyer,uint256 val) internal {
        UserInfo storage user = userInfo[buyer];

        if (user.amount > 0) {
            if(val<=pendingReward(_pid,buyer)){
                uint256 pending = val;
                user.pendingReward = pendingReward(_pid,buyer)-val;
                if(pending > 0) {
                    //transfer to buyback
                    ...
                    intSaveQueue(address(buyer),pending.mul(8).div(10));
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if(commissionAmount>0){
            ...
            if(tpLeader != address(0)){
                ...
                intSaveQueue(tpLeader,commissionAmount);
            ...
            if(lowlead != address(0)){
                ...
                intSaveQueue(lowlead, commissionAmountLead);
            ...
            if(address(referral) != address(0)){
                ...
                if(_referrallevel1 != address(0)){
                    ...
                    intSaveQueue(_referrallevel1, commissionAmount1);
                    ...
                    if(_referrallevel2 != address(0)){
                        ...
                        intSaveQueue(_referrallevel2, commissionAmount2);
                        ...
                        if(_referrallevel3 != address(0)){
                            ...
                            intSaveQueue(_referrallevel3, commissionAmount3);
    ...
    function intSaveQueue(address sellerAdd,uint256 totalXOSParam)  internal{
        ...
        p2p.amountUSDT = getAmountOuts(totalXOSParam,path[0])*markdown/100;
    ...
    function updateBuyback(uint256 totalXOSParam) public onlySpAdd{
    ...
    function updateSellback(uint256 totalXOSParam) public onlySpAdd{
