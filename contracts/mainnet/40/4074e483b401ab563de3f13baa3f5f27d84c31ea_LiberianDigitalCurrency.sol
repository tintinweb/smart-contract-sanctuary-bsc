...
contract  LiberianDigitalCurrency is ERC20, ReentrancyGuard {
    string public constant symbol = "LDC"; //LDC
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as default
         ...
         _RewardTokens[recipient].Slice2 = 20; //20%  LDC
         _RewardTokens[recipient].Slice3 = 20; //20% ETHFIN
         _RewardTokens[recipient].Slice4 = 20; //20% BTC
         _RewardTokens[recipient].Slice5 = 20; //20% Ethereum
         ...
         _RewardTokens[recipient].Token4 = 4;
    ...
    */
       ...
       require(amount<=_maxTxAmount);
         if((recipient==_pancakePairAddress))                   //sell
           ...
           {
               TaxTokens = amount / 4; //6% buy tax applied to all non-sell transfers
...
{   //payouts
    ...
    if(SellTaxBNBPot>10000)  //skip if pot is empty
    {
      ...
      //2% total tax * 100/79.5     = 2.52 % bnb tax portion
         ...
         //40% holder BNB @@@
            HolderPotBNB += SellTaxBNBPot * 5000 / 10000;  //adds to holder reward pot
     
       //0% promo BNB   @@@
         MarketingBNB += SellTaxBNBPot * 0 / 10000;
             
         //4% admin in bnb      
            AdminPotBNB += SellTaxBNBPot * 2800 / 10000;
        
        //0% mod in bnb     
            ModPotBNB += SellTaxBNBPot * 0 / 10000;
...
function _swapTaxTokenForBNB(uint256 selltaxamount,uint256 buytaxamount) private returns (bool){
  ...
  if(total > 0)
  {
   ...
   {uint256 tokenportionofselltax = Sell * 205/1000;
      ...
      //17.5% total tax * 100/20.5     = 85.36 % token tax portion
                    ...
                    //1% airdrop token 
                     ...
                     //liquidity tokens
                     LQPotToken += tokenportionofselltax * 8536 / 10000;// 17.5% to liquidity tokens => 
   ...
   uint256 extraLQbnb = bnbs * 1/200; //0.5%
   bnbs -= extraLQbnb;
   LQPotBNB += extraLQbnb;
...
function doBuyback() public returns (bool) //set to private
{
   if((_isLaunched == true)&&(N_holders >= NextBuybackMemberCount)) 
   {
    ...
    if(__balanceOf[MainWallet]>tokens)  //netto balance of contract increased
    {
     ...
     Buyback_increment = Buyback_increment * 105/100; //wait 5% longer each time
     return true;
    }
