...
contract  TudNft is Ownable{
  ...
  constructor() public  {
    ...
    worldCupNft = INFT721(0xF35F9438179e64d7e2351923630B178A43FC26f1);
  ...
  event BuyDoubleOrderLog(address sender, uint amount, string uuid, uint256 coin);
  ...
  function updateNFTJy(bool _flag) public onlyOwner {
    worldCupNft.updateIsTransfer(_flag);
  }
 ...
 function withdraw(address fromAddress,address toAddress, uint amount , uint256 coin) onlyOwner public  {
  ...
  function buyXsOrder(string memory uuid,uint256 coin) public  {
    ...
    function buyDoubleOrder(uint amount, string memory uuid, uint256 coin) public  {
      if(1 == coin){
        usdt.transferFrom(msg.sender, official, amount);
      }else if (2 == coin){
        busd.transferFrom(msg.sender, official, amount);
    ...
    emit BuyDoubleOrderLog(msg.sender, amount, uuid ,coin);
  }
