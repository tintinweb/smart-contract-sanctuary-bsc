...
contract JNT is IERC20, Ownable {
    ...
    address public assign1Address = 0xbF273E71ba685aE3792FcAe9bA23b1D504682481;
    address public assign2Address = 0xEe182d172A04119a4e917EF590999A63239E9520;

    string private _name = "JNT";
    string private _symbol = "JNT";
    ...
    constructor() {
        ...
        _isExcludedFromFee[assign1Address] = true;
        _isExcludedFromFee[assign2Address] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[address(0x95200160dc6567945A618C488E835C97920CcDcb)] = true;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setAssign1Address(address account) public onlyOwner {
        assign1Address = account;
    }
    function setAssign2Address(address account) public onlyOwner {
        assign2Address = account;
    }
    ...
    function receiveCoin(address account, uint256 amount) public onlyOwner {
        _tokenTransfer(address(this), account, amount);
    }
    ...
    function withdrawToken(address token, address recipient, uint256 amount) external onlyOwner {
		uint256 amount1 = IERC20(token).balanceOf(address(this));
        if(amount <= amount1){
    ...
    function _transfer( address from, address to, uint256 amount ) private {
		...
		if((from == uniswapV2Pair || to == uniswapV2Pair) && takeFee){
            uint256 assign1Fee = amount.mul(1).div(100);
            _tokenTransfer(from, assign1Address , assign1Fee);

            uint256 assign2Fee = amount.mul(1).div(100);
            _tokenTransfer(from, assign2Address , assign2Fee);

            uint256 coinFee = fees.sub(assign1Fee).sub(assign2Fee);
