...
// Part: IReferralManagerV3

interface IReferralManagerV3 {
    ...
    function referreeCashbackAllowed(address _referrer, address _referree, address _directReferrer, uint256 _pid) external view returns (bool);
}
...
// File: StrategyManagerV3.sol

contract StrategyManagerV3 is StrategyManager {
    using SafeERC20 for IERC20;
    ...
    struct UserPoolInfo {
        ...
        uint256 referralShares; //Referral shares added.
    ...
    IReferralManagerV3 public referralManager;
    ...
    function _setUpline(address _user, address _upline) internal {
        if( userInfo[_user].upline == address(0) && sumOfDeposits(_upline) > 0 ) {
    ...
    function _refPayout(
        ...
        //Set upline address.
        address _upline = userInfo[_user].upline;
        ...
        IERC20 depositToken = poolInfo[_pid].stakeToken;
        (uint256 referrerAmount, uint256 referreeAmount) = referralManager.calculateReferralAmounts(
            _upline,
            userInfo[_upline].firstReferralTime,
            _user,
            userInfo[_user].setReferralTime,
            _pid,
            _amount
        );
    ...
    function setReferralManager(address _referralManager) external onlyOwner {
        referralManager = IReferralManagerV3(_referralManager);
    }

    function setReferralDepth(uint8 _referralDepth) external onlyOwner {
    ...
    function setUserUpline(address _user, address _upline) external onlyOwner {
        //Remove upline.
    ...
    function setUserPoolInfo(
        ...
        userPoolInfo[_user][_pid] = UserPoolInfo({
            ...
            referralShares: _referralShares,
            lastWithdrawalTime: _lastWithdrawalTime
        });
    }

    function setUserDepositTrackingInfo(
        uint256 _pid,
