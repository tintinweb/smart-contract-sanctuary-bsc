...
contract DNCPackage is ERC721("Dnc package", "DNC"), SafeOwnable {
    IBlockhashMgr private blockhashMgr;

    IDNCCard private dayOfRightsClub;
    ...
    mapping(uint256 => uint8) public shopCardType;
    ...
    constructor(
        address _blockhashMgr,
        address _dayOfRightsClub,
        uint256[] memory cardWeight,
        uint8[] memory cardType
    ) {
        blockhashMgr = IBlockhashMgr(_blockhashMgr);
        dayOfRightsClub = IDNCCard(_dayOfRightsClub);
        odds[1] = 3;
        cardWeights = cardWeight;

        for (uint256 index = 0; index < cardType.length; index++) {
            shopCardType[index + 1] = cardType[index];
        }
    }
    ...
    function openPackage(uint256 tokenId) external returns (uint256) {
        ...
        if (result <= odds[package.types]) {
            ...
            cardId = dayOfRightsClub.mint(msg.sender, cardType);
        }
        _burn(tokenId);
        return cardId;
    }
