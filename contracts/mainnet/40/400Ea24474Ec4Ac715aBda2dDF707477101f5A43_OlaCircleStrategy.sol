...
contract OlaCircleStrategy is LogicUpgradeable {
  ...
  modifier isUsedToken(address token) {
    require(oTokens[token] != address(0), "E2");
    _;
  }
  ...
  function init(address token, address oToken) public onlyOwner {
    require(oTokens[token] == address(0), "E5");
    oTokens[token] = oToken;

    // Add token/oToken to Logic
    IOlaLogic(logic).addXTokens(token, oToken, 1);

    // Entermarkets with token/otoken
    address[] memory tokens = new address[](1);
    tokens[0] = oToken;
    IOlaLogic(logic).enterMarkets(tokens, 1);

    // Approve rewards token
    if (!bananaInit) {
      IOlaLogic(logic).approveTokenForSwap(rewardsToken);
      bananaInit = true;
    }
  }
  ...
  function destroy(address token) external onlyOwnerAndAdmin isUsedToken(token) {
    ...
    uint256 balanceToken = IERC20Upgradeable(token).balanceOf(logic);
    ...
    if (amountStorage < balanceToken)
      IOlaLogic(logic).returnTokenToStorage(amountStorage, token);
    else
      IOlaLogic(logic).returnTokenToStorage(balanceToken, token);
  }
    ...
    isUsedToken(token)
  {
    ...
    address[] memory tokens = new address[](1);
    tokens[0] = token;
    IOlaLogic(logic).claim(tokens, 1);

    // Covert BANANA to token
    ...
    IOlaLogic(logic).swapExactTokensForTokens(
      apeswapRouter,
      amountIn,
      amountOutMin,
      path,
    ...
    // Convert token to BLID
    amountIn = IERC20Upgradeable(token).balanceOf(logic);
    amountOutMin = 0;
    path[0] = token;
    path[1] = BNB;
    path[2] = blid;
    ...
    uint256 amountBLID = IERC20Upgradeable(blid).balanceOf(logic);
    IOlaLogic(logic).addEarnToStorage(amountBLID);
  }
    ...
    onlyStorage
  {
    ...
    address thisAddress = logic; // to safe gas

    for (uint256 i = circlesCount; i >= 0; i--) {
      uint256 redeemAmount = (amount * (collateralFactor ^ i)) / (100 ^ i);
      IOlaLogic(logic).redeemUnderlying(cToken, redeemAmount);
      IOlaLogic(logic).borrow(cToken, (amount * collateralFactor) / 100, 1);
    }

    if (IERC20Upgradeable(token).balanceOf(thisAddress) < amount)
      revert("no money");
  }
   ...
   */
  function createCircles(address token, uint256 iterateCount) private  {
    uint256 collateralFactor; // the maximum proportion of borrow/lend
    address cToken = oTokens[token];
    (, collateralFactor, , , , ) = IComptrollerOla(olaComptroller).markets(
      cToken
    ...
    uint256 amount = IERC20Upgradeable(token).balanceOf(thisAddress);
    for (uint256 i = 0; i < iterateCount; i++) {
      IOlaLogic(logic).mint(cToken, amount);

      uint256 borrowAmount = (amount * collateralFactor) / 100;
      IOlaLogic(logic).borrow(cToken, borrowAmount, 1);
   ...
   */
  function destructCircles(address token, uint256 iterateCount) private {
    // luquidationFactor - is the same as collateralFactor
    uint256 luquidationFactor;
    address cToken = oTokens[token];
    (, luquidationFactor, , , , ) = IComptrollerOla(olaComptroller).markets(
      cToken
    );
    luquidationFactor = ((luquidationFactor * 100) / (10**18)) - 2; // 70% - 2%(to avoid liquidation) = 68%

    iterateCount += 3; // additional iteration to repay all borrowed

    address thisAddress = address(logic);
    for (uint256 i = 0; i < iterateCount; i++) {
      ...
      (, cTokenBalance, borrowBalance, exchangeRateMantissa) = IXToken(cToken)
        .getAccountSnapshot(thisAddress);
      // calculates of supplied balance, divided by 10^18 to safe digits correctly
      uint256 supplyBalance = (cTokenBalance * exchangeRateMantissa) / 10**18; // 2.456
      ...
      uint256 withdrawBalance = ((luquidationFactor -
        ((100 * borrowBalance) / supplyBalance)) * supplyBalance) / 100; // (0.72-(1.4568/2.456))*2.456

      // if nothing to repay
      if (borrowBalance == 0) {
        if (cTokenBalance > 0) {
          // redeem and exit
          IOlaLogic(logic).redeemUnderlying(cToken, supplyBalance);
          return;
        }
      }
      // if already redeemed
      ...
      // if redeem tokens
      IOlaLogic(logic).redeemUnderlying(cToken, withdrawBalance);
      uint256 repayAmount = IERC20Upgradeable(token).balanceOf(thisAddress);
      // if there is something to repay
      if (repayAmount > 0) {
        // if borrow balance more then we have on account
        if (borrowBalance > repayAmount) {
          IOlaLogic(logic).repayBorrow(cToken, repayAmount);
        } else {
          IOlaLogic(logic).repayBorrow(cToken, borrowBalance);
        }
      }
    }
  }
