...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20  SOL = IBEP20(0x570A5D26f7765Ecb712C0924E4De545B89fD43dF);
...
/* Token contract */
contract SOLFUU is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x237BC4fCaeDcA19c33d71c9A84829d72bB9BD490;
    ...
    // Name and symbol
    string constant _name = "SOLFUU TOKEN";
    string constant _symbol = "SLFU";
    uint8 constant _decimals = 18;

    // Total supply
    uint256 _totalSupply = 50000000 * 10**0 * (10 ** _decimals);

      // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 20 / 1000; // 2% on launch or 1M tokens
    uint256 public _maxSellTxAmount = _totalSupply * 20 / 1000; //2%
    uint256 public _maxWalletToken = ( _totalSupply * 20 ) / 1000; // 2% or 2m tokens
    ...
    // REKTEM
    uint256 REKTEMTriggeredAt;
    uint256 REKTEMDuration = 3600;
    
    // Dead blocks
    uint256 deadBlocks = 3;

    // Sell amount of tokens when a sell takes place
    uint256 public swapThreshold = _totalSupply * 30 / 10000; // .3% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x83156C075DF9F4fB89Af27Ab72Be222878639EFd;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in REKTEM
        bool REKTEMMode = inREKTEMTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, REKTEMMode) : amount;
    ...
    // Check if we are in REKTEM time
    function inREKTEMTime() public view returns (bool){
        if(REKTEMTriggeredAt.add(REKTEMDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the REKTEM Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool REKTEMMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are REKTEM
        if (REKTEMMode){
            if(isSell){
    ...
    // Enable REKTEM
    function enableREKTEM(uint256 _seconds) public authorized {
        REKTEMTriggeredAt = block.timestamp;
        REKTEMDuration = _seconds;
    }

    // Disable the REKTEM mode
    function disableREKTEM() external authorized {
        REKTEMTriggeredAt = 0;
    }
