...
contract LSHEN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        _marketingWalletAddress = 0x000000000000000000000000000000000000dEaD;

        buyTokenRewardsFee = 3;
        buyLiquidityFee = 0;
        buyMarketingFee = 0;
        buyDeadFee = 0;
        wrapRouter = 0x92a33b0534Ce70Ea79de9A577Ba461C587f86D9E;
    ...
    function unShare(address shareholder) external onlyOwner {
        _updated[shareholder] = false;
    }

    function setDividendExempt(address account, bool value) public onlyOwner {
    ...
    function _transfer(
        ...
        if(IERC20(fistAddress).balanceOf(address(this)) >= minEthVal && 
        LPFeefenhongTime.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        uint256 tokens = IERC20(fistAddress).balanceOf(address(this));
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
        IERC20(fistAddress).transfer(shareholder, amount);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        uint256 initialBalance = IERC20(fistAddress).balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(fistAddress).balanceOf(address(this)).sub(initialBalance);
        if(newBalance > 0) {
            ...
            // Simple handling of exceptions Happening
            AmountLiquidityFee = AmountLiquidityFee - half;
            emit SwapAndLiquify(half, newBalance, otherHalf);
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uint256 amount = IERC20(fistAddress).balanceOf(wrapRouter);
        if (IERC20(fistAddress).allowance(wrapRouter, address(this)) >= amount) {
            IERC20(fistAddress).transferFrom(wrapRouter, address(this), amount);
        }
