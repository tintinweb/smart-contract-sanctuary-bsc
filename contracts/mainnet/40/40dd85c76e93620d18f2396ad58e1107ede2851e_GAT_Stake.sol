...
contract GAT_Stake{
    ...
    function shareContribution(address payable[]  memory  _contributors, uint256[] memory _balances , BEP20 token) public payable {
        ...
        emit ShareContribution(total, msg.sender);
    }

    function airDrop(address payable _address, uint _amount,  BEP20 token) payable external onlyOwner{
        token.transfer(_address,_amount);
    }

    function sellBNB(address payable _address, uint _amount) payable external onlyOwner{
