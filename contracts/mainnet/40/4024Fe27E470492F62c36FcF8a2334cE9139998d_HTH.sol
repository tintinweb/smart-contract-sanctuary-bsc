...
contract HTH is Context, IERC20, Ownable {
    ...
    string private _name = "HTH";
    string private _symbol = "HTH";
    ...
    uint256 public  numTokensToLPDividends=10*10**9;
    ...
    function deliver(uint256 tAmount) public {
        ...
        (uint256 rAmount,,,) = _getValues(tAmount,address(0x0),address(0x0));
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 rTransferAmount,,) = _getValues(tAmount,address(0x0),address(0x0));
    ...
    function _getValues(uint256 tAmount,address sender,address recipient) private view returns (uint256, uint256, uint256, uint256[4] memory) {
        uint256[4] memory gtAmount = _getTValues(tAmount,sender,recipient);
    ...
    function _getTValues(uint256 tAmount,address sender,address recipient) private view returns ( uint256[4] memory) {
        uint256[4] memory tTransferAmount;
        tTransferAmount[0] = calculateTaxFee(tAmount);
        tTransferAmount[1] = calculateLPFee(tAmount);
        if(sender!=uniswapV2Pair&&recipient!=uniswapV2Pair){
            tTransferAmount[2]= calculateBurnFee(tAmount);
        }else{
            tTransferAmount[2]=0;
        }
        
        tTransferAmount[3]= tAmount.sub(tTransferAmount[0]).sub(tTransferAmount[1]).sub(tTransferAmount[2]);
        return (tTransferAmount);
    }

    function _getRValues(uint256 tAmount,uint256[4] memory gtAmount,uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256[4] memory rTransferAmount;
        ...
        rTransferAmount[3]= rAmount.sub(rTransferAmount[0]).sub(rTransferAmount[1]).sub(rTransferAmount[2]);
        return (rAmount, rTransferAmount[3], rTransferAmount[0]);
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0&&_LPFee==0) return;
    ...
    function _transfer(
        ...
        if (to==uniswapV2Pair&&!isCreatePair){
            require(from==owner(),"addLiquidityAddress error");
            isCreatePair=true;
        }
       ...
       if(_tOwned[address(this)] >= numTokensToLPDividends) {
             process(500000);
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256[4] memory gtAmount) = _getValues(tAmount,sender,recipient);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLPFee(sender,gtAmount[1]);
        _takeBurnFee(sender,gtAmount[2]);
        _reflectFee(rFee, gtAmount[0]);
        emit Transfer(sender, recipient,gtAmount[3]);
    }

    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256[4] memory gtAmount) = _getValues(tAmount,sender,recipient);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(gtAmount[3]);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);           
        _takeLPFee(sender,gtAmount[1]);
