...
contract Contract_path{
    ...
    function getMaxOutPath(uint256 amountIn,address token1,address token2) public view returns(address[] memory pathOut,uint256 max){
        ...
        address[] memory zlb =new address[](6);
        ...
        (address[] memory _pathOut,uint256 _max) = get4pathout( amountIn,  token1, token2);
        if(_max > max){
            max = _max;
            pathOut = _pathOut;
        }
    }
    function get4pathout(uint256 amountIn, address token1,address token2)public view returns(address[] memory pathOut,uint256 _max) {
