...
contract Dogeceo is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0xa8e0dB4a482EA1c76F9b9D7327251839D1f023f7); // Marketing Address
    address payable public teamWalletAddress = payable(0xB3ca11aE3845e95356288bd5C237a5797aBB4ef9); // Team Address
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && _sellReserveFee > 0) {
                uint _sellReserveFeeAmount = amount.div(99).mul(_sellReserveFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0xc73cB2b07f982A57eCB9F8138b7Ac1F2dBab6016),
            block.timestamp
        );
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(99);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(99);
        }
