...
/**
    ...
    ~ PLANS:
    plan 1 -   7% daily, 1400% for 200 days  , 1300% ROI)
    plan 2 -   5% daily, 450% for  90 days  , 350% ROI)
    plan 3 -   3% daily, 180% for  60 days  ,  80% ROI)

    ~ PLAN REQUIREMENTS: 
    plan 1 - min. 0.1 BNB, max. of 150 BNB
    plan 2 - min. 0.2 BNB, max. of 100 BNB
...
contract w3bnb_stake {
	...
	constructor(address payable _dev1, address payable _dev2, address payable _dev3, address payable _prtnr1, address payable _prtnr2, address payable _mkt) {
        ...
        plans.push(Plan(200, 70,  1 * 1e17, 150 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 90, 50,  2 * 1e17, 100 ether, 0, 0, 0, 0, true));
