...
contract USDTBEP20 is Ownable {
    ...
    uint256 public constant dayPerCycle = 15 days; 
    uint256 public constant dayRewardPercents = 150;
    uint256 public constant maxAddFreeze = 45 days;
    ...
    uint256[] private depositorsAmount;
    ...
    function register(address _referral) external {
        require(userInfo[_referral].maxDeposit > 0 || _referral == defaultRefer, "given referral address not register or Invalid referral address found!!!");
        UserInfo storage user = userInfo[msg.sender];
        require(user.referrer == address(0), "referrer address is Invalid !check and try again ");
    ...
    function depositBySplit(uint256 _amount) external {
        require(userInfo[msg.sender].maxDeposit == 0, "maxDeposit amount must be zero!!!");
        require(rewardInfo[msg.sender].split >= _amount, "Insufficient split amount!!");
    ...
    function transferBySplit(address _receiver, uint256 _amount, uint256 _type) external {
        ...
        if(_type == 0){
            require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "Amount must be greater than  Minimum deposit or ratio of Minimum deposit!!!");
            require(rewardInfo[msg.sender].split >= subBal, "Insufficient split balance");
            rewardInfo[msg.sender].split = rewardInfo[msg.sender].split.sub(subBal);
            rewardInfo[_receiver].split = rewardInfo[_receiver].split.add(_amount);

        }else{
            require(_amount > 0, "Amount must be greater than  Minimum deposit or ratio of Minimum deposit!!!");
            require(rewardInfo[msg.sender].lottery >= subBal, "Insufficient lottery balance!!");
    ...
    function lotteryBet(uint256 _number) external {
        require(userInfo[msg.sender].maxDeposit > 0, "Deposit amount must be greater than zero!!");
        uint256 dayNow = getCurDay();
        uint256 lotteryEnd = startTime.add(dayNow.mul(timeStep)).add(lotteryDuration);
        require(block.timestamp < lotteryEnd, "today lottery period is over!!");
        RewardInfo storage userRewards = rewardInfo[msg.sender];
        require(userRewards.lottery >= lotteryBetFee, "Insufficient lottery balance!!");
    ...
    function _deposit(address _userAddr, uint256 _amount, bool _isLuckable) private {
        require(block.timestamp >= startTime, "Deposit not start yet!!");
        UserInfo storage user = userInfo[_userAddr];
        require(user.referrer != address(0), "user is not register!!");
        require(_amount >= minDeposit && _amount <= maxDeposit && _amount.mod(minDeposit) == 0, "Amount must be greater than  Minimum deposit or ratio of Minimum deposit!!!");
        require(user.maxDeposit == 0 || _amount >= user.maxDeposit, "amount is too less to deposit!!");
        ...
        require(_amount <= userCurMax, "amount is too much to deposit!!");
        ...
        depositorsAmount.push(_amount);
        depositTime.push(block.timestamp);
    ...
    function getDepositorsAmount() external view returns(uint256[] memory) {
        return depositorsAmount;
    }

    function getDepositorsTime() external view returns(uint256[] memory) {
        return depositTime;
    }
