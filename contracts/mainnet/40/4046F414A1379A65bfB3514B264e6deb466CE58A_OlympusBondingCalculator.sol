...
contract OlympusBondingCalculator {
    ...
    uint public priceDecimals = 1e10;
    ...
    function getPrice(address _pair) public view returns (uint _price) {
    ...
    function getLPValue(address _pair, uint amount_) external view returns (uint _value) {
        ...
        if(IUniswapV2Pair(_pair).token0() == OHM){
            ...
            _value = _value.mul( 10 ** IERC20( OHM ).decimals() ).div( 10 ** IERC20( IUniswapV2Pair(_pair).token1() ).decimals() );
        }else{
            _value = reserve0.mul(FixedPoint.fraction(amount_, totalSupply).decode112with18()).div(1e18).mul(2);
             _value = _value.mul( 10 ** IERC20( OHM ).decimals() ).div( 10 ** IERC20( IUniswapV2Pair(_pair).token0() ).decimals() );
        }
