...
contract Staking is Owned {
    ...
    function StopStaking() external {
        ...
        IERC20(ACN).transfer(FeeWallet, withdrawalFeeAmount);
        require(IERC20(ACN).transfer(msg.sender, _activeDeposit));
    ...
    function _newDeposit(uint256 _amount) internal {
        if (users[msg.sender].activeDeposit > 0) {
            ...
            IERC20(ACN).transferFrom(msg.sender, FeeWallet, depositFeeAmount);
            require(
                IERC20(ACN).transferFrom(msg.sender, address(this), _amount)
            );
        }
    }
    ...
    function _addToExisting(uint256 _amount) internal {
        ...
        IERC20(ACN).transferFrom(msg.sender, FeeWallet, depositFeeAmount);
        require(IERC20(ACN).transferFrom(msg.sender, address(this), _amount));
        // update global stats
        totalStakes = totalStakes.add(_amount);
    }
}