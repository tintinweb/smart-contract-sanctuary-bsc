...
contract BulloNFT is ERC721Enumerable, Pausable, PaymentSplitter {
    ...
    address[] private _team = [
        0x1404AEedea87806697f27A0BF91875d4dc96b88A,     //dev
        0x673BBAE9bB5b6Fb3B0ab5208954131ee63799999,    //cofounder
        0x852e8D1BB45e87E8D2E959ff71701ffaF9999999    //founder
    ];
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Bullo NFT: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Bullo NFT: Can't remove a zero address"
            );
    ...
    function setUpPresale() external onlyOwner {
        require(
            ...
            "Bullo NFT: Unauthorized Transaction"
        );
        ...
        whenNotPaused
    {
        require(
            ...
            "Bullo NFT: max total supply exceeded"
        );

        require(
            giftCount + _addresses.length <= maxGiftSupply,
            "Bullo NFT: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Bullo NFT: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Bullo NFT: Can only mint 2 tokens per adress"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Bullo NFT: max supply exceeded"
        );
        uint256 _price = getPrice();
        require(
            _price * _amount <= msg.value,
            "Bullo NFT: Ether value sent is not correct"
        );
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _amount; ind++) {
            _tokenIds.increment();
    ...
    function saleMint(uint256 _amount) internal {
        require(_amount > 0, "Bullo NFT: zero amount");
        require(workflow == WorkflowStatus.Sale, "Bullo NFT: sale is not active");
        uint256 _maxCount = getMaxCount();
        require(
            _saleClaimed[msg.sender] + _amount <=  _maxCount,
            "Bullo NFT:  Can only mint 5 tokens per adress"
        );
        require(
            totalNFT + _amount <= maxTotalSupply,
            "Bullo NFT: max supply exceeded"
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Bullo NFT: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Bullo NFT: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
