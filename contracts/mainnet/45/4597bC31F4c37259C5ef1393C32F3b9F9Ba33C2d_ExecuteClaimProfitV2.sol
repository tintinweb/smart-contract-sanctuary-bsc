...
contract ExecuteClaimProfitV2 is UnstakeLogic, StakeLogic, IExecutable {
  ...
  */
  function claimProfitInToken(
    ...
    ) = factory.getTokenConversion();

    (uint256 estimatedLiquidity, uint256 estimatedRewards) = PersonalContractLibrary.estimateInvestmentTrade(
...
library PersonalContractLibrary {
  ...
  */
    function estimateInvestment(
        ...
        external
        view
        returns (uint256 estimatedLiquidity, uint256 estimatedRewards)
    {
        for (uint256 i = 0; i < _rewards.length; i++) {
            uint256 balance = IERC20(_rewards[i]).balanceOf(address(this));
    ...
    function claimRewards(
        ...
        (bool status, ) = address(_strategy).delegatecall(
            abi.encodeWithSelector(
                _strategy.claimRewards.selector,
  ...
  */
    function convertTokenToToken(
        ...
        (bool status, bytes memory result) = address(tokenConversion)
            .delegatecall(
                abi.encodeWithSelector(
    ...
    function estimateInvestmentTrade(
        address lpPool,
        uint256 lpAmount,
        IExchange.LiquidityTrade memory trade,
        address[] memory rewards,
        address toToken,
        ITokenConversionStorage tokenConversionStorage,
        ITokenConversionLibrary tokenConversionLibrary
    )
        external
        view
