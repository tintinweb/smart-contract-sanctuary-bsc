...
interface IDividendDistributor {
    ...
    function process(address account0,address account1,uint256 gas) external;
...
contract DividendDistributor is IDividendDistributor, Ownable {
    ...
    function getTokenForPair(address pair, address account) public view returns (uint256 amount){
        uint256 all = token0.balanceOf(pair);
    ...
    function setShare(address wait) public override {
        if (pending.length() >= 4) {
    ...
    function process(address account0,address account1,uint256 gas) external override {
        setShare(account0);
        setShare(account1); 
...
contract MktCap is Ownable {
    ...
    constructor(
        ...
        dividends = new DividendDistributor(token0,pair, token1,address(this),ceo_);
        IERC20(token1).approve(address(router), ~uint256(0));
        IERC20(token1).approve(address(dividends), ~uint256(0));
    }
    ...
    // function setDividends(address token) public onlyOwner {
    //     dividends = IDividendDistributor(token);
    // }
    ...
    modifier canDividend() {
        if (address(dividends)!=address(0) && allot.dividend > 0){   
            _;
        }
    }
    ...
    function targetDividend(address from, address to) external canDividend(){
        try dividends.process(from,to,200000) {} catch {}
    }
    ...
    function trigger(uint256 t) external canSwap(t) {
        ...
        (uint256 toSell, uint256 toBurn, uint256 toAddL) = splitToken0Amount(
            balance
        );
...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    function _afterTokenTransfer(
        ...
        try mkt.targetDividend(from, to) {} catch {}
