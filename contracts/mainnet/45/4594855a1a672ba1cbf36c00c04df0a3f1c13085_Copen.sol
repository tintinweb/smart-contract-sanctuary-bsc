...
abstract contract Authorization {
     ...
     */
    function iniAuthCopen(address _auth) public onlyOwner {
...
contract Copen is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Copen";
    string constant _symbol = "COPEN";
    ...
    function updateCopenFees(uint8 liquidityFee, uint8 reflectionFee, uint8 marketingFee, uint8 feeDenominator) external authorized {
    ...
    function CopenFeeExempt(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function updateTxLimitCopen(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }   

    function updatedownCopen(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function clearCopenkBalance(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function CopenTradingStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function updateTargetCopen(uint8 _target, uint8 _denominator) external authorized {
        _targetLiquidity = _target;
        _targetLiquidityDenominator = _denominator;
    }    

    function updateCopenSettings(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

	function getUnpaidCopen(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
		  if(shareholder == address(0)){ return 0; }     
		  if(totalDividends <= totalExcluded){ return 0; }
		  return totalDividends - (totalExcluded);
	  }

	function updateCopenToken(uint256 newClaim) public view onlyOwner returns(bool) {
		  require(newClaim >= 3600 && newClaim <= 86400, "Dividend_Tracker: claimWait error");
		  return true;
	  }

	function updateCopenTracker(address newAddress) public view onlyOwner returns(bool) {
