...
contract SocverseGoldPassportNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    ...
    function mintAndTransfer(address _receiver, uint _quantity) public onlyOwner {     
        for (uint i = 0; i < _quantity; i++) {
    ...
    function bulkMintAndTransfer(address[] memory _receivers, uint[] memory _quantities) public onlyOwner { 
        for (uint iReceiver = 0; iReceiver < _receivers.length; iReceiver++) {
            for (uint iQuantity = 0; iQuantity < _quantities[iReceiver]; iQuantity++) {
                _safeMint(_receivers[iReceiver], totalMinted);
                totalMinted += 1;
                emit nftMinted(
                    totalMinted - 1,
                    _receivers[iReceiver]
                );
            }
        }
    }
    ...
    constructor() ERC721("Socverse Gold Passport", "SGP") Ownable() {}
}