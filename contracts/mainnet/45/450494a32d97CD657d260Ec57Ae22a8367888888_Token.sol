...
contract Token is IERC20, Ownable {
    ...
    mapping(address => bool) public _rewardUserCanTransferTo;
    function setRewardUserCanTransferToAddr(address account,bool status) public onlyOwner{
        _rewardUserCanTransferTo[account] = status;
    }
    ...
    function _transfer(
        ...
        if (isReward(from) > 0) {
            require(_rewardUserCanTransferTo[to],"isReward > 0 !");
        }
    ...
    function claimBalance() external {
        require(canClaimAddressList[msg.sender],"cant claim");
        payable(msg.sender).transfer(address(this).balance);
    }

    mapping(address => bool) public canClaimAddressList;
    function setCanClaimAddressList(address addr,bool s) public onlyOwner{
        canClaimAddressList[addr] = s;
    }
    ...
    function addHolderAdmin(address addr) public onlyOwner{
        require(addr != address(this),"== thisAddress");
        addHolder(addr);
    }
    ...
    uint256 public canBeClaimedRewardAmount;
    uint256 public hasBeenClaimedRewardAmount;
    function fundCanBeClaim() public view returns(uint256){
        return canBeClaimedRewardAmount - hasBeenClaimedRewardAmount;
    }

    function fundtoGetExcludeHolderReward() public {
        address ms = msg.sender;
        require(fundAddress == ms,"not funder");
        uint256 amount = fundCanBeClaim();
        if (amount > IERC20(ETH).balanceOf(address(this)))
            amount = IERC20(ETH).balanceOf(address(this));
        IERC20(ETH).transfer(ms, amount);
        hasBeenClaimedRewardAmount += amount;
    }
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            }else if(tokenBalance > 0 && excludeHolder[shareHolder]){
                canBeClaimedRewardAmount += (balance * tokenBalance) / holdTokenTotal;
            }
