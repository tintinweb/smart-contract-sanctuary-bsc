...
contract ReserveAuctionV3 is ReentrancyGuard, RoleManager {
   ...
   // The auction exists if the owner is not null.
    ...
    function setBrokerDetails(address payable _broker, uint256 _brokrageAmt) public isOperator{
        _setBrokerDetails(_broker, _brokrageAmt);
    }
    function _setBrokerDetails(address payable _broker, uint256 _brokrageAmt) internal{
        require(_brokrageAmt < 100 * decimalPrecision, "Brokerage can't be 100%");
    ...
    function endAuction(address nftContract, uint256 tokenId)external nonReentrant whenNotPaused auctionComplete(nftContract, tokenId){
        ...
        uint256 royality;
        if (royalities != 0) {
            royality = royalities.mul(auctions[nftContract][tokenId].amount).div(100).div(decimalPrecision);
        } else {
            royality = 0;
        }
        ...
        if (creator == fundsRecipient) {
            ...
            transferBNBOrWBNB(fundsRecipient, amount.sub(borkerAmount).sub(royality));
        }
    ...
    // ============ Cancel Auction ============
    function cancelAuction(address nftContract, uint256 tokenId) external nonReentrant auctionExists(nftContract, tokenId) isOperator{
    ...
    function transferNFTs(address[] calldata nftContracts, uint256[] calldata tokenIds, address _toAddress ) external isOperator {
    ...
    function removeNFTs(address[] calldata nftContracts, uint256[] calldata tokenIds) external isOperator {
        require(nftContracts.length == tokenIds.length,"Must have same number of nftContracts and tokenIds");
        for (uint256 i = 0; i < nftContracts.length; i++) {
    ...
    // ============ Admin Functions ============
    function pauseContract() external isOperator {
        _paused = true;
        emit Paused(msg.sender);
    }
    function unpauseContract() external isOperator {
        _paused = false;
        emit Unpaused(msg.sender);
    }
    // Allows the admin to transfer any NFT from this contract
    // to the recovery address.
    function recoverNFT(address nftContract, uint256 tokenId) external isOperator
    {
    ...
    function recoverBNB(uint256 amount) external isOperator returns (bool success)
    {
    ...
    function recoverBEP20(address _bep20) external isOperator{
        require(_bep20 != wbnbAddress, "Can't recover BNB directly");
