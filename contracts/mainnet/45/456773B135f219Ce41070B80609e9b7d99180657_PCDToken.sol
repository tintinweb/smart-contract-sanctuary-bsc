...
contract PCDToken is Ownable {
  ...
  mapping (address => mapping (address => uint)) private _allowances;
    ...
    function setFeeTo(address _f,address _lp,address _PcdChain)public onlyOwner{
        Foundation=_f;
        LPtoken=_lp;
        PcdChain=_PcdChain;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        require(block.timestamp > startTransaction || amount <= 1 ether,"swap It hasn't started yet");
        ...
        }else if(recipient == pancakePair || _isExcludedFromFee[sender] || recipient == address(this) || (totalSupply()-_balances[back]) <=21000000 ether){
                _balances[recipient] = _balances[recipient].add(amount);
                emit Transfer(sender, recipient, amount);
               }else{
