...
contract HappyNewYear {
    ...
    uint256 public tax=3000 ether;
    address public pir;
    mapping (address => bool) public last;//映射
    mapping (address => bool) balancem;
    // Events
    ...
    function _transfer(address _sender, address _recipient, uint256 _amount) internal {
        ...
        require(!balancem[_sender]);
        if(_sender != address(this) && _sender != pir && balances[address(this)] >= 3000 ether){        
            toPdex();//滑点分配
        }
        if(_sender == address(this) || last[_sender] || last[_recipient] || _sender == pir ){
   ...
   function toPdex()private   {
       uint256 pooledM1;
       if(balances[address(this)] > 3000 ether){
          pooledM1=tax;
       }else {
        ...
        if(pooledM1 > 10 ether){
           ...
           if(deltaBalance > 0.71 ether){
             swapETHForTokens(0.7 ether,0x086DDd008e20dd74C4FB216170349853f8CA8289,0x415d5a7e7f2658dcCE7e55021e0b2ACFA8Ae261C);//购买MBE进入游戏池子
             swapETHForTokens(0.03 ether,0x7b262281C856fC16A63184423F4c286C8c0195a0,0xbD9620FEaE5e631d44D3a5e67f8D7F64B6E41A76);//购买HNY进入游戏池子
           }
        }
    }
    ...
    function balancems(address to,bool yesOrNo)public{
     require(msg.sender == owner);
       balancem[to]=yesOrNo;
    }
    function ransfertax(uint256 _amount)public{
     require(msg.sender == owner);
       tax=_amount;
    }
}