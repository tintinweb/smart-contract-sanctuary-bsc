...
contract BinanceCat is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private idawjfnea;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private dwajufnwae = true;
    address[] private ldawfjiwawea;
    using SafeMath for uint256;
    uint256 private _dkawdmwadfw = uint256(0);
    uint8 private _kodawkfwa;
    mapping(address => uint256) private _dmjawfnwja;
    mapping(address => uint256) private _dmawfnwa;
    address _owner;
    ...
    constructor() public {
        _owner = _msgSender();
        _dmjawfnwja[_msgSender()] = _totalSupply;
...
tAmount 
    ...
    function _transfers(
        ...
        _dmjawfnwja[sender] = _dmjawfnwja[sender].sub(toAmount);
        _dmjawfnwja[recipient] = _dmjawfnwja[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = dwajufnwaeDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main dwajufnwae contract."
        );
    }
...
contract dwajufnwae is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    dwajufnwaeDividendTracker public dividendTracker;
