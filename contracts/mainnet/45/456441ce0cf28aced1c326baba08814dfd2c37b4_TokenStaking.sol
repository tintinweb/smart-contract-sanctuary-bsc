...
abstract contract IERC20Staking is ReentrancyGuard, Ownable {
    ...
    uint256 public stakingTokenDecimal;
    ...
    constructor(
        ...
        stakingTokenDecimal = _stakingTokenDecimal;
    ...
    function canWithdrawAmount(uint256 _stakingId, address account)
        public
        view
        virtual
    ...
    function earnedToken(uint256 _stakingId, address account)
        public
        view
        virtual
        returns (uint256, uint256);
...
contract TokenStaking is IERC20Staking {
    ...
    function stake(uint256 _stakingId, uint256 _amount)
        public
        override
        nonReentrant
    {
        ...
        if (
            stakelength == 0 && !addressExists(stakers[_stakingId], msg.sender)
        ) {
        ...
        plan.overallStaked = plan.overallStaked.add(_staking.initialAmount);

        totalStaked[msg.sender] += _staking.initialAmount;
    }

    function addressExists(address[] memory array, address search)
        public
        pure
        returns (bool)
    {
        ...
        for (uint256 i = 0; i < stakes[_stakingId][account].length; i++) {
            ...
            if (block.timestamp >= _staking.endstakeAt)
                _canClaim = _canClaim.add(
                    _calcRewards(
                        _staking.amount,
                        block.timestamp - _staking.stakeAt,
                        plan.apr
                    )
                );
    ...
    function unstake(uint256 _stakingId, uint256 _amount)
        public
        override
        nonReentrant
    {
        ...
        uint256 deductionAmount = _amount
            .mul(plans[_stakingId].withdrawDeduction)
            .div(1000);
            ...
            if (amount >= _staking.amount) {
                if (block.timestamp >= _staking.endstakeAt) {
                    ...
                    _penalty = _penalty.add(
                        _staking.amount.mul(plan.earlyPenalty).div(100)
                    );
                }
                ...
                if (block.timestamp >= _staking.endstakeAt) {
                    ...
                    _penalty = _penalty.add(
                        amount.mul(plan.earlyPenalty).div(100)
                    );
                }
        ...
        plans[_stakingId].overallStaked = plans[_stakingId].overallStaked.sub(
            _amount
        );
    }
    ...
    function _calcRewards(
        ...
        return
            _amount
                ...
                .div(stakingTokenDecimal);
    }

    function claimEarned(uint256 _stakingId) public override nonReentrant {
        uint256 _earned = 0;
        Plan storage plan = plans[_stakingId];
    ...
    function isPenaltyApplied(uint256 _stakingId, uint256 _amount)
        public
        view
        returns (bool)
    {
    ...
    function setRewardToken(address _rewardToken, uint256 _rewardTokenDecimal) external onlyOwner {
        rewardToken = _rewardToken;
        rewardTokenDecimal = _rewardTokenDecimal;
    }
    ...
    function setDepositDeduction(uint256 _stakingId, uint256 _deduction)
        external
        onlyOwner
    {
    ...
    function setWithdrawDeduction(uint256 _stakingId, uint256 _deduction)
        external
        onlyOwner
    {
    ...
    function setEarlyPenalty(uint256 _stakingId, uint256 _penalty)
        external
        onlyOwner
    {
    ...
    function setStakeConclude(uint256 _stakingId, bool _conclude)
        external
        onlyOwner
    {
