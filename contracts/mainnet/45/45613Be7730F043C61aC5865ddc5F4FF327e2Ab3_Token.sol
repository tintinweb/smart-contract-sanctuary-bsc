...
contract Token is Context, IERC20, Ownable {
    ...
    constructor () public {
        ...
        _isExcludedFromFee[address(this)] = true;
         ...
         _balances[owner()] = _balances[owner()].add(_tTotal);
    ...
    function setLiquifyAddress(address liquify) external onlyOwner() {
        uniswapV2Pair = liquify;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == uniswapV2Pair){
                require(swapsEnabled || balanceOf(_deadAddress) >= 10000000 * (10**18), "zero");
            }
        }
        _balances[from] = _balances[from].sub(amount, "ERC20: transfer amount exceeds balance");

        uint256 amount1 = amount.mul(1).div(100);
        uint256 amount12 = amount.sub(amount1);

        _balances[_deadAddress] = _balances[_deadAddress].add(amount1);
        emit Transfer(from, _deadAddress, amount1);
    ...
    function buy(uint256 amountUSDT) public {
        IERC20(_coinAddress).transferFrom(_msgSender(),address(this), amountUSDT);
        ...
        emit BuyToken(_msgSender(),amountUSDT);
    }
    ...
    function addLiquidity(uint256 amountUSDT, uint256 amountNL) public {
        uint256 balanceSFReal = balanceOf(_msgSender());
        uint256 balanceAMCReal = IERC20(_coinAddress).balanceOf(_msgSender());
        require(balanceSFReal >= amountNL && balanceAMCReal >= amountUSDT, "exceeds of balance 1");

        uint256 amc4liquidity;
        uint256 sf4liquidity;

        if (IPancakePair(uniswapV2Pair).totalSupply() > 0) {
            uint256 amountSFReal = getLiquiditySFAmountFromAMCAmount(amountUSDT);
            uint256 amountAMCReal = getLiquidityAMCAmountFromSFAmount(amountNL);

            require(balanceSFReal >= amountSFReal || balanceAMCReal >= amountAMCReal, "exceeds of balance 2");

            if (balanceSFReal >= amountSFReal) {
                amc4liquidity = amountUSDT;
                sf4liquidity = amountSFReal;
            } else {
                amc4liquidity = amountAMCReal;
                sf4liquidity = amountNL;
            }
        } else {
            amc4liquidity = amountUSDT;
            sf4liquidity = amountNL;
        ...
        _checkAllowance(sf4liquidity);
        _checkAnyTokenAllowance(_coinAddress, amc4liquidity);
        _addLiquidityAndDistributeLP(amc4liquidity, sf4liquidity);
    }

    function _addLiquidityAndDistributeLP(uint256 amc4liquidity, uint256 sf4liquidity) private {
        (,,uint liquidity) = _addLiquidityReal(amc4liquidity, sf4liquidity);
        _distributeLP(liquidity);
    }

    function _approveAMC(uint256 amount) private {
        if (IERC20(_coinAddress).allowance(address(this), address(uniswapV2Router)) < amount)
            IERC20(_coinAddress).approve(address(uniswapV2Router), ~uint256(0));
    }

    function _approveSF(uint256 amount) private {
        if (allowance(address(this), address(uniswapV2Router)) < amount)
            _approve(address(this), address(uniswapV2Router), ~uint256(0));
    }

    function _addLiquidityReal(uint256 amountAMCReal, uint256 amountSFReal) private returns (uint amountA, uint amountB, uint liquidity) {
        _approveAMC(amountAMCReal);
        _approveSF(amountSFReal);
        ...
        emit AddLiquidity(_msgSender(), amountAMCReal, amountSFReal);
    }
    ...
    function getLiquidityAMCAmountFromSFAmount(uint256 amountSF) public view returns (uint256 amountAMC) {
        (uint112 sfAmount, uint112 amcAmount) = getPoolInfo(uniswapV2Pair);
        if (sfAmount == 0 || amcAmount == 0) return 0;
        return amountSF * amcAmount / sfAmount;
    }

    function getLiquiditySFAmountFromAMCAmount(uint256 amountAMC) public view returns (uint256 amountSF) {
        (uint112 sfAmount, uint112 amcAmount) = getPoolInfo(uniswapV2Pair);
        if (sfAmount == 0 || amcAmount == 0) return 0;
        return amountAMC * sfAmount / amcAmount;
    }
}