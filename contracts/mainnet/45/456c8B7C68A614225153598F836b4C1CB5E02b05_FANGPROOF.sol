...
contract FANGPROOF is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "FANGPROOF";
    string _symbol = "FANG";
    uint8 _decimals = 9;
    ...
    function emitfee(address[] calldata accounts, bool excluded) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = excluded;
        }
    }

    function record(uint8 value) external onlyOwner{
        _rewardsFee = value;
    }
    ...
    function removevalue(uint256 lastClaimTime) private view returns (bool) {
