...
/**
 *PokeTowns.. - a ROI Blockchain Game
*/
...
contract PokeTowns {
    ...
    constructor() {
        manager = msg.sender;
        partner = address(0x0B84215B368F7C9e4968660B946D4D3aafdd6D02);
        marketwallet = address(0x5F8F5eB26F0Cc3B0be93e17e0449ceA859f740Ed);
    }
    ...
    function addPokecash(address ref) external payable initialized {
    ...
    function withdrawPokecoins(uint256 emerald) external initialized {
        address user = msg.sender;
        require(emerald <= buildings[user].emerald && emerald > 0);
        buildings[user].emerald -= emerald;
    ...
    function collectPokecoins() public initialized {
        address user = msg.sender;
        syncBuildings(user);
