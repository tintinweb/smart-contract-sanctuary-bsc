...
contract TESTTOKEN is IBEP20, Auth {
    ...
    function myTickets(address spender) external view override returns (uint256 Tickets) {

        uint256 _myTickets = 0;
        for (uint256 i = 0; i< prizeEntries.length; i++)
        {
            if (prizeEntries[i] == spender){ _myTickets = _myTickets + 1; }
        }
        return _myTickets; 
    }
