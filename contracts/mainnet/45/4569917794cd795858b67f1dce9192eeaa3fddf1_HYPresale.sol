...
contract HYPresale is Ownable {
    ...
    constructor(
        address USDTAddress, address HYAddress, address NFTAddress,
        address CashAddress
    ){
        _usdtAddress = USDTAddress;
        _tokenAddress = HYAddress;
        ...
        minUSDT = 50*usdtUnit;
        HYPrice = 5;
        tokenUnit =  10 ** IERC20(HYAddress).decimals();
    }
    ...
    function buy(uint256 amount) external {
        ...
        if(!isFirst[account]){
            _userInfo[invitor].saleInviteAccount += 1;
            isFirst[account] = true;
        }

        if (invitor!=_cash) {
                UserInfo storage invitorInfo = _userInfo[invitor];
                uint256 intitorNum = invitorInfo.saleInviteAccount;
                if(intitorNum>=2 && !invitorInfo.ZZNFT){
                    _nft.batchMint(invitor, 2, 1);
                    _userInfo[invitor].YDToken = 1;
                    invitorInfo.ZSNFT = true;
                }
                if(intitorNum>=3 && !invitorInfo.ZZNFT){
                    _nft.batchMint(invitor, 3, 1);
                    _userInfo[invitor].YDToken += 2;
        ...
        if(targetAmount>0){
            ...
            cur = invitors[cur];
            if (cur != address(0)) {
                _userInfo[cur].usdtReward += targetAmount.mul(rate).div(100);
            }
                }
            }
        //增加地址购买数量
    ...
    function bindInvitor(address account) public {
         ...
         uint256 buytoken = _userInfo[account].buyToken;
         require(buytoken != 0,"error invitor");
        
    }

    //领取YD
    function getYdToken() public {
        address account = msg.sender;
        uint256 count = _userInfo[account].YDToken;
        require(count > 0 ,"amount zero");
        uint256 amount = count * 10 ** IERC20(YdAddress).decimals();
        _userInfo[account].YDToken -=count;
        _giveToken(YdAddress, account, amount);   
    }
    //领取USDT
    function getUsdtReward() public {
        address account = msg.sender;
        uint256 amount = _userInfo[account].usdtReward;
        require(amount > 0 ,"amount zero");
        _userInfo[account].usdtReward -=amount;
        _giveToken(_usdtAddress, account, amount);   
    }
    //待领取的已释放的代币
    ...
    //设置领取间隔时间
    function setYdAddress(address yd) external onlyOwner {
        YdAddress = yd;
    }
    //设置NFT价格
    ...
    function setFirst(address first) external onlyOwner {
        UserInfo storage userInfo = _userInfo[first];
        userInfo.buyToken = 1;
    }
    ...
    //结束预售
    function endSale(bool flag) external onlyOwner {
        _endSale = flag;
    }
