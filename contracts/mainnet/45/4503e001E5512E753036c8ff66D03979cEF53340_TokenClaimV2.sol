...
contract TokenClaimV2 is Pausable, Ownable, ReentrancyGuard {
    ...
    modifier _hasAllowance(address allower, uint256 amount) {
        ...
        ERC20Interface = IERC20(fundToken);
    ...
    constructor(
        ...
        require(address(_fundToken) != address(0), "Invalid address");
    ...
    function setFundToken(address _fundToken) external isOperatorOrOwner {
        require(address(_fundToken) != address(0), "Invalid address");
        fundToken = _fundToken;
    }
    ...
    /**
     *  Requirements:
     *  `addFunds` funds to be added to this contract
     *  once the allowance is given to this contract for 'amount' by the user
     */
    function addFunds(uint256 amount)
        ...
        returns (bool)
    {
        require(amount > 0, "Funds must be positive");
        fundAdded = fundAdded.add(amount);
        
        IERC20(fundToken).safeTransferFrom(msg.sender, address(this), amount);
        // emit FundsAdded(amount, block.timestamp);
        return true;
    }

    function withdrawFundedToken() external isOperatorOrOwner {
        require(fundToken != address(0), "Empty token address");

        uint256 balance = IERC20(fundToken).balanceOf(address(this));
        require(balance > 0, "Empty balance");

        IERC20(fundToken).safeTransfer(msg.sender, balance);
    }

    /**
     *  Requirements:
     *  `setWhitelist` keep funders info
     *  whiteListedFunds: Their claimable amounts in total
     *  whitelistFunders: keep each funder their claimable amount
     */
    function setWhitelist(address[] calldata addrs, uint256[] calldata amounts) external isOperatorOrOwner {
        ...
        for (uint256 index = 0; index < addrs.length; index++) {
            address buyer = addrs[index];
            uint256 amount = amounts[index];
            //whitelistAmount[addrs[index]] = amounts[index];

            // Simulate user had bought the token already
            whiteListedFunds = whiteListedFunds.add(amount);
    ...
    function getFunderInfo(address funder) external view returns (User memory) {
        User memory info;
        info.totalFunded = whitelistFunders[funder].totalFunded;
        info.released = whitelistFunders[funder].released ;
        return info;
    }
    ...
    function getUnlockedTokenAmount(address addr, uint256 totalFundedAmount) private view returns (uint256) {
        require(addr != address(0), "Invalid address!");

        // calculate in this time, he can get how many token
        uint256 distributeAmountAtClaim = (totalFundedAmount * distributePercentAtClaim) / 1000;

        if (block.timestamp < cliffTime) {
            return distributeAmountAtClaim;
        }
        if (distributeAmountAtClaim == totalFundedAmount) {
            return distributeAmountAtClaim;
        }
        ...
        uint256 lockedAmount = totalFundedAmount - distributeAmountAtClaim;
    ...
    function getClaimableAmount(address addr) public view returns (uint256) {
        return getWhitelistClaimableAmount(addr);
    }

    function _claimTo(address to, uint256 claimableAmount) private {
        require(to != address(0), "Invalid address");
        
        IERC20(fundToken).safeTransfer(to, claimableAmount);
        whitelistFunders[to].released = whitelistFunders[to].released + claimableAmount;
        totalReleased = totalReleased + claimableAmount;
        emit TokenClaimed(to, claimableAmount);
    }

    function claim() external whenNotPaused nonReentrant onlyEOA {
        require(cliffTime <= block.timestamp, "Not claimable");
        uint256 claimableAmount = getClaimableAmount(msg.sender);
        require(claimableAmount > 0, "Nothing to claim");
        _claimTo(msg.sender, claimableAmount);
    }

    function getCurrentVestId() public view returns (uint256) {
        if (block.timestamp < cliffTime) return 0;
        uint256 finalTime = cliffTime + vestingDuration - vestingPeriodicity;
        if (finalTime <= block.timestamp) {
            return vestingDuration / vestingPeriodicity;
        }
        return (block.timestamp - cliffTime) / vestingPeriodicity + 1;
    }

    function pause() public isOperatorOrOwner {
        _pause();
    }

    function unpause() public isOperatorOrOwner {
        _unpause();
    }

    receive() external payable {
        // DO NOT SEND TOKEN TO THIS CONTRACT DIRECTLY
