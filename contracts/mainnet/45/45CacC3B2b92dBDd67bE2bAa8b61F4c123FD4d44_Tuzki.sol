...
contract Tuzki is ERC20, Ownable {
    ...
    address public marketWallet = 0xB4c94620EDcEf1f944950C4f47Bd8288F451796C;
    ...
    constructor() ERC20("Tuzki", "Tuzki") {
        ...
        */
        _mint(marketWallet, 100000000000 * 1e18);
    }
    ...
    function setBuyMarketingAFee(uint256 _buyMarketingFee) external onlyOwner {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if(takeFee) {
            ...
            if(sender == uniswapV2Pair) { //buy
                totalFeeToThis = buyLpFee + buyMarketingFee;
            } else if (recipient == uniswapV2Pair) {
                totalFeeToThis = sellMarketingFee;
                totalFeeToBurn = sellBurnFee;
            } 

            if(totalFeeToThis > 0) {
                uint256 feeAmount = initialAmount * totalFeeToThis / 100;
                super._transfer(sender, address(this), feeAmount);
                amount -= feeAmount;
            }

            if(totalFeeToBurn > 0) {
                uint256 feeAmount = initialAmount * totalFeeToBurn / 100;
    ...
    function swapAndDividend(uint256 tokenAmount) private lockTheSwap {
        ...
        uint totalSellShare = sellMarketingFee;

        uint amountToLp = tokenAmount * buyLpFee / (totalBuyShare + totalSellShare);
