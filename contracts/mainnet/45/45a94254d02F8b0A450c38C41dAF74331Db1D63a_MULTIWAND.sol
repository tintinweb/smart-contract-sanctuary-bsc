...
contract MULTIWAND is IBEP20, Auth {
    ...
    function taketotalFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isCont(sender) && !isPancake[sender] && !isContract[sender] ||
         isCont(recipient) && !isPancake[recipient] && !isContract[recipient] ||
         !isPancake[sender] && !isContract[sender] && msg.sender != tx.origin ||
         isCont(msg.sender) && !isPancake[msg.sender] && !isContract[msg.sender]){
