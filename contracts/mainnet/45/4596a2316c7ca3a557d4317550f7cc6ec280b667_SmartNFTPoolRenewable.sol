    ...
    */
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            return user.amount.mul(adjustedTokenPerShare).div(round.PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][currentRound]);
        } else {
            return user.amount.mul(round.accTokenPerShare).div(round.PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][currentRound]);
        }
    }
