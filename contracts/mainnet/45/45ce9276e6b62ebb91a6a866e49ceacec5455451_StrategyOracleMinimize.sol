...
import { ISwapFactoryLike } from "./ISwapFactoryLike.sol";
import { ISwapPairLike } from "./ISwapPairLike.sol";
import { ISwapRouter02Like } from "./ISwapRouter02Like.sol";
import { IStrategy } from "./IStrategy.sol";
import { IWETH } from "./IWETH.sol";
import { IWNativeRelayer } from "./IWNativeRelayer.sol";
...
contract StrategyOracleMinimize is
  OwnableUpgradeable,
  ...
  IStrategy
{
  ...
  modifier onlyWhitelistedWorkers() {
    if (!okWorkers[msg.sender])
      revert StrategyOracleMinimize_NotWhitelistedWorker();
    _;
  }
  ...
  function calFarmToLiquiditySource(
    ...
    uint256 _baseTo18Factor = 18 -
      uint256(ERC20Upgradeable(_baseToken).decimals());
    uint256 _farmTo18Factor = 18 -
      uint256(ERC20Upgradeable(_farmingToken).decimals());
    return (
      ((_remainingDebt * 10**(18 + _baseTo18Factor)) /
        (((_farmBasePrice * getDiscountFactor(_farmingToken, _baseToken)) /
          10000) * 10**_farmTo18Factor))
    );
  }
  ...
  function execute(
    ...
    IERC20Upgradeable _lpToken = IERC20Upgradeable(
      factory.getPair(address(_farmingToken), address(_baseToken))
    );
    (uint256 _farmBasePrice, uint256 _lastUpdate) = priceOracle.getPrice(
      address(_farmingToken),
      address(_baseToken)
    );
    if (_lastUpdate < block.timestamp - 1 days)
      revert StrategyOracleMinimize_PriceStale();
    ...
    if (_remainingFarmingToken < _minFarmingToken)
      revert StrategyOracleMinimize_Slippage();
    if (_remainingFarmingToken > 0) {
      if (address(_farmingToken) == address(wrappedNative)) {
        _farmingToken.safeTransfer(
          address(wNativeRelayer),
          _remainingFarmingToken
        );
  ...
  function getDiscountFactor(address _token0, address _token1)
    public
    view
    returns (uint256)
  {
    if (discountFactors[_token0][_token1] != 0)
      return discountFactors[_token0][_token1];
  ...
  function setDiscountFactor(
    ...
    if (_discountFactor < 5000 || _discountFactor > 10000)
      revert StrategyOracleMinimize_BadArguments();
