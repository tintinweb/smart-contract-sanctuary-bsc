...
contract ProofOfSwapToken is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "ProofOfSwap";
    ...
    uint256 private _totalSupply = 2100000* 10**8 * 10**_decimals;
    ...
    uint256 public _buyTotalFee = _buyDestoryFee.add(_buyReflectionFee).add(_buyLpFenhongFee).add(_buyMarketingFee);

    uint256 public _sellDestoryFee = 2;
    uint256 public _sellReflectionFee = 2;
    uint256 public _sellLpFenhongFee = 3;
    uint256 public _sellMarketingFee = 1;
    uint256 public _sellTotalFee = _sellDestoryFee.add(_sellReflectionFee).add(_sellLpFenhongFee).add(_sellMarketingFee);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(!_isExcluFromFrr[sender] && !_isExcluFromFrr[recipient] && deployTime != 0 && block.timestamp.sub(deployTime) <= 600) {
        ...
        if(!_isExcluFromFrr[sender] && !_isExcluFromFrr[recipient] && sender == uniswapPair && (block.number > ( genesisBlock + coolBlock) && block.number <= ( genesisBlock + _limitBuyBlock)))
        {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {//buy
            ...
            //uint256 buyBackLpNum = amount.mul(_buyReflectionFee).div(100);
        ...
        else if(isMarketPair[recipient] && !_isExcluFromFrr[sender]) {//sell
            uint256 burnNum = amount.mul(_sellDestoryFee).div(100);
            _takeFee(sender,deadAddress, burnNum);
            uint256 lpNum = amount.mul(_buyLpFenhongFee).div(100);
            _takeFee(sender,address(this), lpNum);
         
            //uint256 sellBackLpNum = amount.mul(_sellReflectionFee).div(100);
