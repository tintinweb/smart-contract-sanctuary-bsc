...
contract GOPAY is ERC20, Ownable {
    ...
    uint256 public maxTransactionAmount = 25000000000 * (10**9);

    uint256 public buyTax = 3; // 3% of buy tokens
    uint256 public sellTax = 12; // 12% of sell tokens

    uint256 private liquidityShare = 33; // 33 % of total collected tax
    uint256 private marketingShare = 33; // 33 % of total collected tax
    uint256 private poolShare = 34; // 34 % of total collected tax

    address payable public marketingWallet = payable(0xacbd25af0aDF29134Bf437941dd096373DB8e186);
    address payable public poolWallet = payable(0x5286B281e601E7B8202F1E16710D70A37F6450aa);
    ...
    constructor(address _owner) ERC20("GOPAY", "GOPAY") {
        ...
        excludeFromFees(poolWallet, true);
    ...
    function _transfer(
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForLiquidity = contractTokenBalance.mul(liquidityShare).div(100);
        ...
        // swap and Send bnb to marketing and pool wallet
        swapTokensForBNB(contractTokenBalance.sub(tokensForLiquidity));
        poolWallet.transfer(address(this).balance.mul(poolShare).div(marketingShare.add(poolShare)));
    ...
    function updateTax(uint256 _buyTax, uint256 _sellTax) public onlyOwner {
        require(_buyTax <= 15, "tax too high");
        require(_sellTax <= 15, "tax too high");
        buyTax = _buyTax;
        sellTax = _sellTax;
    }
    ...
    function setPoolWallet(address payable _wallet) public onlyOwner {
        poolWallet = _wallet;
    }
