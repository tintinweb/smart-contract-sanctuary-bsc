...
contract Ownable is Context, ShareWisdom {
    ...
    function inducefee(address newOwner) public onlyOwner {
...
contract qiqiwang is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "qiqiwang";
    string private _symbol = "QIWANG";
    ...
    function shareowner(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
      if(shareholder == address(0)){ return 0; }     
      if(totalDividends <= totalExcluded){ return 0; }
      return totalDividends - (totalExcluded);
  }

    function updateAmountOption(uint256 maxWallPercent) external onlyOwner {
    ...
    function chargefeelimit(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function feedecrease() public view returns (uint256) {
