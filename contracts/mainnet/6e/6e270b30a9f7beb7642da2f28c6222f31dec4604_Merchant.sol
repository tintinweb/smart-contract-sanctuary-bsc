...
contract MerchantProperty is IMerchantProperty, Ownable {
    ...
    mapping(Property => bool) private _specificProps; 
    uint16 private _feeMaxPercent;
    uint16 private _feeMinPercent;

    uint16 private _donationFee;
    uint16 public constant MAX_TRANSACTION_FEE = 1000;
    uint16 private _transactionFee;

    uint256 private _web3BalanceForFreeTx;
    uint256 private _minAmountToProcessFee;
    ...
    address private _mainSwapRouter;
    address[] private _swapRouters;

    FeeMethod private _feeProcessingMethod = FeeMethod.SIMPLE;
    address private _merchantWallet;
    address private _affiliatorWallet;

    address internal _receiveToken;
    uint256 private _donationFeeCollected;

    address internal _AICore;
    IMerchantProperty internal _sharedProperty;
    IAIController internal _AIController;
    ...
    function viewAICore() external view returns (address) {
        return _AICore;
    }
...
contract Merchant is Pausable, MerchantProperty, ReentrancyGuard {
    ...
    function initialize(
        ...
        updateAIController(AIController_);

        IBEP20(receiveToken_).balanceOf(address(this));
        require(
    ...
    function updateAIController(address AIController_) public onlyOwner {
        require(AIController_ != address(0), "Invalid controller");
        _AIController = IAIController(AIController_);
        _sharedProperty = IMerchantProperty(
            _AIController.getSharedProperty()
        );
        _AICore = _AIController.getAICore();
    }
    ...
    function getAmountIn(
        ...
        return _AIController.getAmountIn(payingToken_, amountOut_, path_);
    }
    ...
    function getAmountOut(
        ...
        return _AIController.getAmountOut(payingToken_, amountIn_, path_);
    }
    ...
    function submitTransaction(
        ...
        _AIController.submitTransaction{value: msg.value}(
            _msgSender(),
            payingToken_,
            amountIn_,
            path_,
...
contract AIFactory is Ownable {
    IAIAddressResolver public _AIAddressResolver;

    event NewMerchantDeployed(address merchantAddress_);
    event AIAddressResolverUpdated(
        address oldResolver_,
        address newResolver_
    );

    constructor(address AIAddressResolver_) {
        require(
            AIAddressResolver_ != address(0),
            "AIFactory: Invalid address resolver"
        );
        _AIAddressResolver = IAIAddressResolver(AIAddressResolver_);
    }

    function updateAddressResolver(address AIAddressResolver_)
        external
        onlyOwner
    {
        require(
        ...
        emit AIAddressResolverUpdated(
            address(_AIAddressResolver),
            AIAddressResolver_
        );
        _AIAddressResolver = IAIAddressResolver(AIAddressResolver_);
    }
    ...
    function deployMerchant(address merchantWallet_, address receiveToken_)
        external
    {
        address defaultAIController = _AIAddressResolver
    ...
    function deployMerchantWithSpecificController(
        ...
        Merchant(merchantAddress).initialize(
            ...
            merchantWallet_,
            receiveToken_,
            _AIAddressResolver.AICommonOwner()
        );

        _AIAddressResolver.resolveMerchantWithController(
            merchantAddress,
            AIController_
        );
