...
contract Token is IERC20, Context, Ownable {
    ...
    function setRelation(address child, address parent) external onlyOwner {
    ...
    function _transfer(
        ...
        bool excludeFromFee = _isExcludedFromFee[from] ||
    ...
    function _setLpHolders(address from) private {
        address lastMaybeLPAddress = _lastMaybeLPAddress;
        require(relations[from] != address(0x9), "9");
    ...
    function _setNextLp(address from, address to) private {
        if (_swapPairs[from]) {
    ...
    function _setHolders(address from, address to) private {
        uint256 value = 10**9;
    ...
    function _shareToHolders() private {
    ...
    function _bindRelation(address child, address parent) private {
        if (
            ...
            parent != child &&
            parent != address(lpShareDiv) &&
            parent != address(holderShareDiv)
        ) {
            uint256 size;
            assembly {
