...
contract StakePoolBNBForBNB is States, Ownable {
    ...
    function setTokens(uint256 _apy, uint256 _earlyUstakeFee, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner {
        apyTier = _apy;
    ...
    function compoundInterest(address _address) public {
        ...
        uint256 stakedtime;
         ...
         if(block.timestamp > info.lockPeriod){
          ...
          stakedtime = (info.lockPeriod).sub(info.lastClaimTime);
          ...
          info.totalAmount += _reward;
            ...
            stakedtime = (block.timestamp).sub(info.lastClaimTime);
            stakedtime = stakedtime / 1 days;
            uint256 _reward = _compound(info.totalAmount, ratio, stakedtime);
            info.totalAmount += _reward;
         }
...
contract FactoryStakingPool is Ownable{
    ...
    function createBNBForBNB(address ownerAddress, uint256 _apy, uint256 _earlyUnstakeFee, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolBNBForBNB toDeploy = new StakePoolBNBForBNB();
        toDeploy.setTokens(_apy, _earlyUnstakeFee,_rewardingTokenWallet, _lockPeriod, _maxReward, _earlyRewardUstakeFee, _autoCompund, _earlyUstake);
