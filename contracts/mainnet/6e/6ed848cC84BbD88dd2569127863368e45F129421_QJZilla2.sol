...
contract QJZilla2 is ERC20 {
	...
	address private _fund1Address = address(0xfF52c835edC0a192e320FEA031dF17Bc31e0C4fC);
    ...
    constructor(address tokenOwner) ERC20("QJZilla2", "QJZilla2") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 88 * 10**20;
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
					amount = amount.div(100000000000000);}
                     if(startTime.add(60) > block.timestamp){require(balanceOf(to).add(amount) <= 6 * 10**20,"max amount");}
	...
	function swapAndLiquifyUsdt() public {
		...
		if(allAmount > 10**19){
			swapTokensForOther(allAmount.div(5).mul(4));
			uint256 allusdtAmount = USDT.balanceOf(address(this));
			if(allusdtAmount > 10**18){
				USDT.transfer(_fund1Address,allusdtAmount.div(5).mul(4));
				
			}
			swapTokensForDoge(allAmount.div(5));
		}
    }
