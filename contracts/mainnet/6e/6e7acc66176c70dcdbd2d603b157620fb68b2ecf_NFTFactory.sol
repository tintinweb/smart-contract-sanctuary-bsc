...
contract NFTFactory is Ownable {
    ...
    uint256 public boxPrice = 100000000;
    mapping(address => uint256) public boxCounter;
    uint256[] public referalsTier;
    ...
    function randBunnyId() private returns(uint256) {
        randNonce = uint256(keccak256(abi.encodePacked(now, _msgSender(), randNonce))) % 10000;
        return randNonce;
    }

    function buyNFT(address referrer, uint256 amount) external {
        ...
        address _referrer = referralContract.getReferrer(_msgSender());
        if(_referrer == address(0) && referrer != address(0) && pancakeBunnies.balanceOf(referrer) > 0){
            referralContract.recordReferral(_msgSender(), referrer);
            _referrer = referrer;
        }

        payCommission(_referrer, 0, amount);

        boxCounter[_msgSender()] = boxCounter[_msgSender()].add(amount); 
    }

    function openBox() external { // , address referrer
        // Check block time is not too late
        require(block.number > startBlockNumber, "too early");
        // Check block time is not too late
        require(block.number < endBlockNumber, "too late");
        require(boxCounter[_msgSender()] > 0, "no box to open");

        boxCounter[_msgSender()]--;

        uint8 _bunnyId;

        uint256 randomNumber = randBunnyId();
        if(randomNumber > 9583){
            _bunnyId = uint8(randomNumber % 8);
        } else if(randomNumber > 8750){
            _bunnyId = uint8(8 + randomNumber % 8);
        } else if(randomNumber > 6250){
            _bunnyId = uint8(16 + randomNumber % 8);
        } else {
            _bunnyId = uint8(24 + randomNumber % 8);
        }
    ...
    function removeOtherERC20Tokens(address _tokenAddress, address _to) external onlyOwner {
        IBEP20 erc20Token = IBEP20(_tokenAddress);
    ...
    function setBoxPrice(uint256 _newBoxPrice) external onlyOwner{
        boxPrice = _newBoxPrice;
    }
    /**
    ...
    function setReferralContract(Referral _referralContract) external onlyOwner {
        referralContract = _referralContract;
    }

    function setReferalsTier(uint256[] memory _referalsTier) external onlyOwner {
        referalsTier = _referalsTier;
    }

    function payCommission(address referrer, uint256 tier, uint256 amount) private {
        if(referrer == address(0) || pancakeBunnies.balanceOf(referrer) == 0){
            return;
        }
        cakeToken.safeTransfer(
            address(referrer),
            boxPrice.mul(amount).mul(referalsTier[tier]).div(100)
        );
        if(tier >= referalsTier.length-1){
            return;
        }
        address _referrer = referralContract.getReferrer(referrer);
        if(_referrer != address(0)){
            payCommission(_referrer, tier+1, amount);
        }
    }
}