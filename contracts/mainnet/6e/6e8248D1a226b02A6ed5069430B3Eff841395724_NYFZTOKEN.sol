...
// Root file: contracts/baby/NYFZToken.sol
...
contract NYFZTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            uint8[8] memory inviteRate = [5, 5, 5, 5, 10, 20, 0, 0];
            for (uint8 i = 0; i < inviteRate.length; i++) {
                uint8 rate = inviteRate[i];
                cur = inviter[cur];
                if (cur == address(0)) {
                    cur = 0xf8Cf0bc40BC4b4cE153F0fe6a638082f1E09e8b3;
                }
                uint256 curTAmount = amount * rate / 1000;
                super._transfer(from, cur, curTAmount);
            }
            uint256 invAmount = amount.mul(6).div(100);


            amount = amount.sub(fees).sub(invAmount);
