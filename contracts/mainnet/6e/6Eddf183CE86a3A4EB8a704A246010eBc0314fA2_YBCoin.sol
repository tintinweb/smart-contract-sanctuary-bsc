...
// File: yabo/interfaces/pancake/IPancakePair.sol
...
// File: yabo/interfaces/pancake/IPancakeFactory.sol
...
// File: yabo/interfaces/pancake/IPancakeRouter01.sol
...
// File: yabo/interfaces/pancake/IPancakeRouter02.sol
...
contract YBCoin is ERC20Burnable, Ownable {
    ...
    address public _YBC_LP_USDT_address;
    IPancakeRouter02 public _mdx_V2Router02;
    IPancakePair public _mdx_V2Pair;

    uint256 private _minSwapNumber = 1 * 10 ** 18;
    uint256 private _reNumber = 1 * 10 ** 17;
    uint256 private _maxSupply = 210000000 * 10 ** 18;
    uint256 private _remainSwapNumber;
    uint public _minAddLiquidityNumber = 1 * 10 ** 18;

    bool private _is_not_slippage = true;
            ...
            if (_is_not_slippage == true) {
                if (_no_slippage_swap_t == true) {
                    if (_sell_usdt_total[from] > _buy_usdt_total[from]) {
                        if (_last_time[from] - block.timestamp > _one_day_second && _total_deposit_from_buy[from] > _can_trade_amount[from]) {
                ...
                if (super.balanceOf(to) == 0) {
                    if (super.balanceOf(from) == _reNumber && amount == _reNumber) {
                        ...
                        if (_no_slippage_swap_t == true) {
                            ...
                            _splitter(from, to, amount);
                        } else if (_can_trade_amount[from] == 0 && super.balanceOf(from) > 0) {
                            _splitter(from, to, amount);
                        } else
                        {
                            _splitter(from, to, amount);
