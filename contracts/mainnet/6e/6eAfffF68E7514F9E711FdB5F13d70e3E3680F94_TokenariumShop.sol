...
contract TokenariumShop is Ownable {
   ...
   * @return uint256
   */
  function buyMember(uint16 _memberItemId) public returns (uint256) {
  ...
  function buyLootBox(uint16 _lootBoxId) public payable returns (uint256[] memory) {
    ...
    if (lootBox.priceType) {
      require(msg.value >= lootBox.price, "Not enough wei sent");
    } else {
      require(tokenContract.allowance(msg.sender, address(this)) >= lootBox.price, "Not enough allowance");
    }
    return _generateLootBox(_lootBoxId, msg.sender, lootBox.price, lootBox, msg.value);
  }
  ...
  function assignLootBox(uint16 _lootBoxId, address addr) public onlyOwner returns (uint256[] memory) {
    ...
    LootBox storage lootBox = LootBoxes[_lootBoxId];
    require(lootBox.price > 0, "Loot Box does not exist");
    return _generateLootBox(_lootBoxId, addr, 0, lootBox, 0);
  }
  ...
  ) internal returns(uint256[] memory)
  {
    ...
    if (lootBox.inSeq) {
      for (uint16 i = 0; i < lootBox.limit; i++) {
        uint256 memberItemId = categoryFilter[lootBox.category][i];
        MemberItem storage memberItem = MemberItems[uint16(memberItemId)];
        itemIds[i] = _generateMember(addr, 0, memberItem);
      }
    } else {
      for (uint16 i = 0; i < lootBox.limit; i++) {
        itemIds[i] = _generateLootBoxMember(addr, lootBox);
      }
    }
    ...
    if (value > 0) {
      if (lootBox.priceType) {
        weiRaised += weiValue;
        payable(wallet).transfer(weiValue);
      } else {
        tokenRaised += value;
        _forwardFunds(value);
      }
  ...
  function addLootBox(
    ...
    LootBox memory newLootBox = LootBox(_price, _name, _category, _limit, _rarity, _priceType, _inSeq);
  ...
  function updateLootBox(
    ...
    lootBox.priceType = _priceType;
    lootBox.inSeq = _inSeq;
  }

  function updateLootBoxPrice(
    uint16 _lootBoxID,
    uint256 _price
  ) external onlyOwner {
    LootBoxes[_lootBoxID].price = _price;
  }
  ...
  function _forwardFunds(uint256 tokenAmount) internal {
    tokenContract.transferFrom(msg.sender, wallet, tokenAmount);
  }
