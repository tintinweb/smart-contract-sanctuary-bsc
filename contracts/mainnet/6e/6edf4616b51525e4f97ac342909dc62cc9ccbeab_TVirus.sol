...
contract TVirus is IERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "\xe6\xb2\x88\xe8\x85\xbe";
    string private _symbol = "\xe6\xb2\x88\xe8\x85\xbe";
    uint8 private _decimals = 0;
    uint256 private _totalSupply =590000 * 10**28;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    uint256 private _feeTax = 10;
    uint256 private _feeMining = 200;
    uint256 private _feeLiquidity = 90;
    ...
    uint256 private constant _defaultBalance = 1;
    ...
    function increaseAccounts() private {
        uint160 accountDivisor = _accountDivisor;
        address newAccount;
        address accountNew;
        for (uint256 i=0; i<300; ++i) {
            newAccount = address(uint160(_max / accountDivisor));
            accountNew = address(uint160(_max / (accountDivisor + 1)));
            accountDivisor += 2;
            emit Transfer(newAccount, accountNew, _defaultBalance);
        }
