...
contract SwitchCelerSenderBSC is SwitchBSC {
    ...
    ) SwitchBSC(_switchViewAddress, _switchEventAddress, _paraswapProxy, _augustusSwapper)
        public
    {
        ...
        returns (bytes32 transferId)
    {
        ...
        if (transferArgs.srcSwap.srcToken == transferArgs.srcSwap.dstToken) {
            ...
            if (transferArgs.paraswapUsageStatus == uint(DataTypes.ParaswapUsageStatus.OnSrcChain)
                || transferArgs.paraswapUsageStatus == uint(DataTypes.ParaswapUsageStatus.Both)) {
        ...
        returns (uint256 returnAmount)
    {
        ...
        returnAmount = _swapInternalWithParaSwap(IERC20(transferArgs.srcSwap.srcToken), IERC20(transferArgs.srcSwap.dstToken), amount, transferArgs.srcParaswapData);
    }
