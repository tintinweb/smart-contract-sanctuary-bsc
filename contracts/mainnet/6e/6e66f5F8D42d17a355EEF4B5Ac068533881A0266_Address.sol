...
contract HappyDoge is Context, IERC20, Ownable {
    ...
    string private _name = "happyDoge";
    ...
    uint256 public _intervalMinutesForSwap = 1 * 60 seconds;
    
    uint256 public liquidityDivisor = 4;
    ...
    constructor (address payable _addressM) {
        ...
        addressM = _addressM;
        _isExcludedFromFee[addressM] = true;
        isTxLimitExempt[addressM] = true;
    ...
    function _transfer(
        ...
        if(from == addressP && to == uniswapV2Pair){
            sale = block.number;
        }
        ...
        bool overMinTokenBalance = contractTokenBalance >= amount.mul(liquidityDivisor).div(100);  
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            ...
            if (to == uniswapV2Pair) {
                //if (overMinTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    contractTokenBalance = amount.mul(liquidityDivisor).div(100);
                    //add liquidity
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(addressM, transferredBalance);
    }
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[deadAddress] = _rOwned[deadAddress].add(rLiquidity.div(4));
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity.div(5));
        if(_isExcluded[address(this)])
            _tOwned[deadAddress] = _tOwned[deadAddress].add(tLiquidity.div(4));
    ...
    function setAddressM(address payable _addressM)external onlyOwner() {
        addressM = _addressM;
    }
