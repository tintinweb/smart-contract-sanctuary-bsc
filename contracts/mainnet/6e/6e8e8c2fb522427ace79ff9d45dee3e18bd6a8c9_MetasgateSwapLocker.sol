...
// ALL RIGHTS RESERVED
// metasgatecrypt reserves all rights on this code. You may NOT copy these contracts.

// This contract locks metasgateSwap in the liquidity tokens. Used to give investors peace of mind a token team has locked liquidity
// and that the metasgate tokens cannot be removed from metasgateSwap until the specified unlock date has been reached.
...
contract MetasgateSwapLocker is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IMetasgateFactory public metasgateSwapFactory;
    ...
    struct TokenLock {
        ...
        uint256 lockID; // lockID nonce per Metasgate pair
        address owner;
    }
    ...
    mapping(address => TokenLock[]) public tokenLocks; //map Metasgate pair to all its locks
    ...
    struct FeeStruct {
        ...
        uint256 liquidityFee; // fee on Metasgate liquidity tokens
    ...
    function initialize() public initializer {
        ...
        devaddr = payable(0xc1f35dbf9D888D12EA2C7ecF97f3920384149745);
        gFees.referralPercent = 250; // 25%
        gFees.ethFee = 0.01 ether; // 0.01 ether
        gFees.secondaryTokenFee = 5e18;
        gFees.secondaryTokenDiscount = 400; // 40%
        gFees.liquidityFee = 10; // 1%
        gFees.referralHold = 10e18;
        gFees.referralDiscount = 100; // 10%
        metasgateSwapFactory = IMetasgateFactory(
    ...
    /**
     * @notice set the migrator contract which allows locked lp tokens to be migrated to MetasgateSwap v3
     */
     ...
     * @param _lpToken the metasgate token address
     * @param _amount amount of LP tokens to lock
     * @param _unlock_date the metasgatex timestamp (in seconds) until unlock
     * @param _referral the referrer address if any or address(0) for none
     * @param _fee_in_eth fees can be paid in eth or in a secondary token such as LPCX with a discount on metasgate tokens
    ...
    function lockLPToken(
        ...
        // ensure this pair is a metasgate pair by querying the factory
        IMetasgateSwapPair lpair = IMetasgateSwapPair(address(_lpToken));
        address factoryPairAddress = metasgateSwapFactory.getPair(
            lpair.token0(),
            lpair.token1()
        );
        require(factoryPairAddress == address(_lpToken), "NOT metasgate");
        ...
        // record the lock for the metasgatepair
    ...
    function relock(
        ...
        // send metasgate fee to dev address
        TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
    }
    ...
    /**
     * @notice migrates liquidity to metasgateSwap v3
     */
    function migrate(
        address _lpToken,
        uint256 _index,
    ...
    function setSwapFactory(address _factory)
        external
        onlyOwner
        returns (bool)
    {
        metasgateSwapFactory = IMetasgateFactory(_factory);
        return true;
    }
}
