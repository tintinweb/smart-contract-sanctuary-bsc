...
contract DaQiang is ERC20 {
    using SafeMath for uint256;
	mapping(address => address) inviter;
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address _tokenOwner;
	IERC20 public BORNTOKEN;
    IERC20 public pair;
    bool private swapping;
    uint256 public swapTokensAtAmount;
	address payable _fundAddress = payable(address(0x593E253A48B7A0619681dF5FC4BA71Ead629298d)); //团队钱包地址
    ...
    constructor(address tokenOwner) ERC20("DaQiang", "DaQiang") { //定义token 名称和代号
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//pancakeswap流动性池子[不用改]
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _uniswapV2Router.WETH());
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**36);//pancakeswap流动性池子[不用改]
		...
		BORNTOKEN = IERC20(0x55d398326f99059fF775485246999027B3197955); //LP分红哪一种币
    ...
    function _transfer(
        ...
        if(balanceOf(address(this)) > swapTokensAtAmount){
            if (
                ...
                swapAndLiquifyBorn(haveAmount.div(4));
                swapAndLiquifyV3(haveAmount.div(4).mul(3));
                swapping = false;
            }
        }
    ...
    function swapAndLiquifyBorn(uint256 contractTokenBalance) private {
        swapTokensForBorn(contractTokenBalance);
    }

    function swapTokensForBorn(uint256 tokenAmount) private {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(0x55d398326f99059fF775485246999027B3197955); //LP分红哪一种币
    ...
    function swapAndLiquifyV3(uint256 contractTokenBalance) public {
        ...
        uint256 allHaveBnb = address(this).balance;
        if(allHaveBnb > haveBnb){
            uint256 newBnb = allHaveBnb.sub(haveBnb);
            _fundAddress.transfer(newBnb.div(3).mul(2));
        }
    }
