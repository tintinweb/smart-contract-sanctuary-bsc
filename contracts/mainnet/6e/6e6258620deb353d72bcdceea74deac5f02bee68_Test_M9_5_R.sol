...
user.DEP_REF_activatesReferADD =  user.partnersCount.add(1);
...
adrSE = user.SE_total_LevelReferrals1US_ADR;
	...
	function DEP_REF(address userAddress) public view returns(uint256 checkpointTimer, address adres1, uint256 Count) {
	    User storage user = users[userAddress];

checkpointTimer = user.DEP_REF_activatesRefer_Timer;
Count = user.DEP_REF_activatesReferADD ;
adres1 = user.DEP_REF_activatesReferADR ;
	}
