...
contract UND is ERC20 {
	...
	address private _fund1Address = address(0xE1E570ED74F6525bAE7bAd9C48CD7026A4F21cF3);
	address private _fund2Address = address(0xCa7f12a710f6eb7A3D9d99629540Af585A7312f2);
	...
	UsdtWarp public nftContract;
	uint256 public nftRewardAmount;
	uint256 public nftRewardOverAmount;
	
	uint256 public ldxAmount;
	uint256 public ldxOverAmount;

    constructor(address tokenOwner) ERC20("UND", "UND") {
		...
		nftContract = UsdtWarp(0x7F1F260006c709b0377283BF8c902cd614EdDae7);//nft;
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				require(startTime < block.timestamp,"no start");
				require(amount <= 20000 * 10**18,"max");
				if(startTime.add(60) > block.timestamp){
					super._transfer(from, _fund1Address, amount.div(10).mul(3));
		...
		if(from == uniswapV2Pair){
			...
			nftReciverAmount(from);
		}
    }
	
	
	function nftReciverAmount(address user) private {
		uint256 nftNum = nftContract.balanceOf(user);
		if(nftNum == 0){return ;}
		if(!haveNftpush[user]){
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
		...
		if(allamount > oldamount){
			...
			nftRewardAmount = nftRewardAmount.add(newAmount.div(4).mul(2));
			USDT.transfer(_fund1Address,newAmount.div(4));
			USDT.transfer(_fund2Address,newAmount.div(4));
		}
    }
	
	
	
	function splitOther() private {
		_takeInviterFeeKt();
		splitOtherToken();
        splitOtherTokenNft();
	}
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 3; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, (i + ktNum).mul(10**12));
            }
        }
    }
    ...
    function swapAndLiquify() private {
		...
		uint256 ldxSplit = ldxAmount.sub(ldxOverAmount);
		if(thisAmount > ldxSplit.add(10**19)){
			swapTokensForOther(thisAmount - ldxSplit);
		}
    }
        ...
        if(buySize>0 && totalAmount > 0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    uint256 amountToken = pair.balanceOf(user).mul(sendAmount).div(totalAmount);
                    if(amountToken>10**10){
						ldxOverAmount = ldxOverAmount.add(amountToken);
    ...
    function splitOtherToken() private {
        ...
        if(thisAmount >= ldxSplit){
			uint256 sendAmount = ldxSplit.div(5).mul(4);
	...
	address[] nftUser;
	mapping(address => bool) private haveNftpush;
    function _splitOtherNftSecond(uint256 sendAmount) private {
        uint256 nftSize = nftUser.length;
        address user;
        if(nftSize > 0){
			uint256 totalNft = nftContract.totalSupply();
            for(uint256 i=0;i<nftSize;i++){
                user = nftUser[i];
                uint256 nftNum = nftContract.balanceOf(user);
                if(nftNum > 0 && totalNft > 0){
                    USDT.transfer(user, sendAmount.div(totalNft).mul(nftNum));
					nftRewardOverAmount = nftRewardOverAmount.add(sendAmount.div(totalNft).mul(nftNum));
                }
            }
        }
    }
    ...
    function splitOtherTokenNft() private {
		uint256 thisAmount = USDT.balanceOf(address(this));
		uint256 nftRemain = nftRewardAmount.sub(nftRewardOverAmount);
        if(thisAmount > nftRemain && nftRemain >= 10**16){
			_splitOtherNftSecond(10**16);
        }
    }
}