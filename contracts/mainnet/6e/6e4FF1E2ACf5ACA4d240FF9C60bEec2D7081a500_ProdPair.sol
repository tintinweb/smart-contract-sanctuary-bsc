...
pragma solidity ^0.5.16;
    ...
    interface IProdCallee {
        function ProdCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
    }
    
    contract ProdERC20 is IProdERC20 {
        using SafeMath for uint;
    
        string public constant name = 'Prod LP';
        string public constant symbol = 'Prod-LP';
    ...
    contract ProdPair is IProdPair, ProdERC20 {
        using SafeMath  for uint;
        ...
        function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
            address feeTo = IProdFactory(factory).feeTo();
        ...
        function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
            ...
            if (data.length > 0) IProdCallee(to).ProdCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
            }
    ...
    contract ProdFactory is IProdFactory {
        bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(ProdPair).creationCode));
        ...
        function createPair(address tokenA, address tokenB) external returns (address pair) {
            ...
            bytes memory bytecode = type(ProdPair).creationCode;
            bytes32 salt = keccak256(abi.encodePacked(token0, token1));
            assembly {
                pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
            }
            IProdPair(pair).initialize(token0, token1);
        ...
        function setWeightFee(address _pair,uint8 _devFee, uint8 _liqFee) external {
            ...
            ProdPair(_pair).setWeightFee(_devFee, _liqFee);
        }
        
        function setSwapFee(address _pair, uint32 _swapFee) external {
            require(msg.sender == feeToSetter, 'Msg: FORBIDDEN');
            ProdPair(_pair).setSwapFee(_swapFee);
        }
    }
