...
contract Peanut is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 PEANUT-WBNB LP -> PEANUT pool
    // uint256 public constant INITIAL_PEANUT_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the PEANUT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("Peanut Token", "PEANUT") {
        // Mints 1 PEANUT to contract creator for initial pool setup
    ...
    function _getPeanutPrice() internal view returns (uint256 _peanutPrice) {
        try IOracle(peanutOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Peanut: failed to fetch PEANUT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _peanutPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_peanutPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPeanutOracle(address _peanutOracle) public onlyOperatorOrTaxOffice {
        require(_peanutOracle != address(0), "oracle address cannot be 0 address");
        peanutOracle = _peanutOracle;
    }
    ...
    /**
     * @notice Operator mints PEANUT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PEANUT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPeanutPrice = _getPeanutPrice();
            currentTaxRate = _updateTaxRate(currentPeanutPrice);
            if (currentPeanutPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // require(_peanutPool != address(0), "!_peanutPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_peanutPool, INITIAL_PEANUT_POOL_DISTRIBUTION);
    ...
    function setPairAddress(address _pairAddress) public onlyOperator {
        pairAddress = _pairAddress;
    }
    function setTradingOpen() public{
        require(block.timestamp > sellStartTime, 'Wait until sellStartTime');
        tradingOpen = true;
    }
    function getRemainingTimeForPeanutSellingEnable() public view returns(uint256) {
        uint256 remaingTime = 0;
        if(block.timestamp < sellStartTime){
            remaingTime = sellStartTime - block.timestamp;
        }
        return remaingTime;
    }
        /// @dev overrides transfer function to meet tokenomics of Peanut
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // Pre-flight checks
        require(amount > 0, "Transfer amount must be greater than zero");
        // Sell Token
        if ( recipient == pairAddress ){
            require(tradingOpen, 'Selling is unable. Wait until sellStartTime'); 
        }
