...
contract ZSwapToken is ERC20("ZSwap Zap", "ZAP") {
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of ZAPs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accZapPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accZapPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     ZAP rewards each pool gets.
    /// `accZapPerShare` Accumulated ZAPs per share, times 1e16.
    ...
    ///     the ZAP rewards to all the ZSwap products.
    ...
    // The ZAP TOKEN!
    ZSwapToken public zap;

    // Dev address.
    address public devaddr;

    /// @notice The only address can withdraw and burn ZAP.
    ...
    ///  @notice 0.5 ZAP per block in MC
    uint256 public constant MASTERCHEF_ZAP_PER_BLOCK = 5000 * 1e14;
    uint256 public constant ACC_ZAP_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total zap rate = toBurn + toRegular + toSpecial
    uint256 public constant ZAP_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of ZAP burn action being executed.
    /// @notice ZAP distribute % for burn
    uint256 public zapRateToBurn = 0;
    /// @notice ZAP distribute % for regular farm pool
    uint256 public zapRateToRegularFarm = 250000000000;
    /// @notice ZAP distribute % for special pools
    uint256 public zapRateToSpecialFarm = 750000000000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accZapPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateZapRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _zap The ZAP token contract address.
    ...
    /// only for ZAP distributions within ZSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
        ...
        // stake ZAP token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != zap, "ZAP token can't be added to farm pools");
        ...
        lastRewardBlock: _startBlockNumber,
        accZapPerShare: 0,
    ...
    /// @notice Update the given pool's ZAP allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending ZAP rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingZap(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accZapPerShare = pool.accZapPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 zapReward = multiplier.mul(zapPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accZapPerShare = accZapPerShare.add(zapReward.mul(ACC_ZAP_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accZapPerShare).div(ACC_ZAP_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update zap reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of ZAP per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function zapPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_ZAP_PER_BLOCK.mul(zapRateToRegularFarm).div(ZAP_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_ZAP_PER_BLOCK.mul(zapRateToSpecialFarm).div(ZAP_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of ZAP per block to burn.
    function zapPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_ZAP_PER_BLOCK.mul(zapRateToBurn).div(ZAP_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accZapPerShare = pool.accZapPerShare.add((zapReward.mul(ACC_ZAP_PRECISION).div(lpSupply)));
                zap.mint(devaddr, zapReward.div(10));
                zap.mint(address(this), zapReward);
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accZapPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingZap(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accZapPerShare).div(
            ACC_ZAP_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Send ZAP pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnZap(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingZapToBurn = multiplier.mul(zapPerBlockToBurn());

        // SafeTransfer ZAP
        _safeTransfer(burnAdmin, pendingZapToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of ZAP distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of ZAP to burn each block.
    /// @param _regularFarmRate The % of ZAP to regular pools each block.
    /// @param _specialFarmRate The % of ZAP to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateZapRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChef: Zap rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == ZAP_RATE_TOTAL_PRECISION,
            "MasterChef: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn zap base on old burn zap rate
        burnZap(false);

        zapRateToBurn = _burnRate;
        zapRateToRegularFarm = _regularFarmRate;
        zapRateToSpecialFarm = _specialFarmRate;

        emit UpdateZapRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingZap(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accZapPerShare).div(
            ACC_ZAP_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending ZAP rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingZap(
        address _user,
        uint256 _pid,
        ...
        uint256 accZap = boostedAmount.mul(poolInfo[_pid].accZapPerShare).div(ACC_ZAP_PRECISION);
        uint256 pending = accZap.sub(user.rewardDebt);
        // SafeTransfer ZAP
    ...
    /// @notice Safe Transfer ZAP.
    /// @param _to The ZAP receiver address.
    /// @param _amount transfer ZAP amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            uint256 balance = zap.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            zap.transfer(_to, _amount);
        }
    }
}