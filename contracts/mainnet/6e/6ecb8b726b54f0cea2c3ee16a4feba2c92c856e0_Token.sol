...
contract Token is Context, IERC20, Owner {
    using SafeMath for uint256;
    using Address for address;
    ...
    function excludeMultipleAccountsFromFees(address[] calldata account, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < account.length; i++) 
        _isExcludedFromFee[account[i]] = excluded;
    }
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && to != address(0) && to != dead && to != pcsV2Pair && to != feeWallet)
