...
contract DaYingJia {
  ...
  function buyStage(uint _stage) public payable userRegistered() validStage(_stage) validStageAmount(_stage) {
    ...
    for (uint s = _stage - 1; s > 0; s--) {
      ...
      require(token.balanceOf(msg.sender) >= 1000000000000000000, 'Not enough UAT token');
    }
    ...
    transferStagePayment(_stage*9/10, msg.sender);
    transferStagePayment(_stage/10, 0xA2Bd2CD2F3877DDa333E24323cA7173F3f57ef30);
