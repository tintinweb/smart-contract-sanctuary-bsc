...
contract ExchangeAgent is AbstractExchangeAgent {
    mapping(address => uint256) public storedTokensOf;
    ...
    function _transferTokensForDeposit(address from, uint256 amount) internal override {
        storedTokensOf[msg.sender] += amount;
    ...
    function _transferTokensForClaim(address to, uint256 amount) internal override {
        require(storedTokensOf[to] >= amount, "ExchangeAgent: not enough stored tokens");

        storedTokensOf[to] -= amount;
