...
/**
 *Submitted for verification at BscScan.com on 2022-08-13 DEV @ROBDBEME TG / Official TG @mineswaptoken
*/
...
contract MineSwapToken is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    MineSwapTokenDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 1_000_000 * (10**18); //1 million. 
    ...
    constructor() BEP20("Mine Swap Token", "MineSwap") 
    {
        dividendTracker = new MineSwapTokenDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "MineSwapToken: The dividend tracker already has that address");
        MineSwapTokenDividendTracker newDividendTracker = MineSwapTokenDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "MineSwapToken: The new dividend tracker must be owned by the MineSwapToken token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MineSwapToken: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MineSwapToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MineSwapToken: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MineSwapToken: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MineSwapToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MineSwapToken: Cannot update gasForProcessing to same value");
...
contract MineSwapTokenDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MineSwapToken_Dividend_Tracker", "MineSwapToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MineSwapToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MineSwapToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MineSwapToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MineSwapToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MineSwapToken_Dividend_Tracker: Cannot update claimWait to same value");
