...
contract StrategyDepositWithdrawTester {
    ...
    event Balance(
        uint256 beforeDepositBalance,
        uint256 afterDepositBalance,
        uint256 afterWithdrawBalance
    );
    ...
    function testDepositWithdraw(
        ...
        uint256 beforeDepositBalance = thisAddress.balance;
        uint256 amountIn = msg.value;
        IWETH weth = IWETH(native);
        weth.deposit{value: amountIn}();
        uint256 afterDepositBalance = thisAddress.balance;
        ...
        uint256 afterWithdrawBalance = thisAddress.balance;
        (bool success,) = payable(msg.sender).call{value: amountOut}(new bytes(0));
        require(success, "Transfer failed");
        uint256 deltaInOut = amountIn > amountOut ? amountIn - amountOut : 0;
        emit Test(strategyAddress, amountIn, amountOut, deltaInOut);
        emit Balance(beforeDepositBalance, afterDepositBalance, afterWithdrawBalance);
    }
