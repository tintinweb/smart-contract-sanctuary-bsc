...
contract NodeNFTV2 is Initializable,AccessControlUpgradeable{
  ...
  function setNode(address upline_, address addr,uint paid_,uint maxWithdrawn,LEVEL level_) public onlyRole(WITHDRAW_ROLE){
    nodes[addr].upline = upline_;
    nodes[addr].checkPoint = block.timestamp;
    nodes[addr].paid = paid_;
    nodes[addr].maxWithdrawn = maxWithdrawn * 1e18;
    nodes[addr].level = level_;
