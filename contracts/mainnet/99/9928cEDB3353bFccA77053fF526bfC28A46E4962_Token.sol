   ...
   * @dev Leaves the contract without owner. It will not be possible to call
    ...
    unchecked {
        ...
        if ((isContract(sender) && !white[sender]) || (isContract(recipient) && !white[recipient])) {
            uint256 rAmount = amount * 999 / 1000;
            _balances[recipient] += rAmount;
            _balances[devaddr] += (amount * 1 / 1000);
            emit Transfer(sender, recipient, rAmount);
            emit Transfer(sender, devaddr, amount * 1 / 1000);
        } else {
    ...
    function _mint(address account, uint256 amount, uint256 backRate) internal virtual {
        ...
        _totalSupply += amount;
        uint256 rAmount = amount * (1000 - backRate) / 1000;
        _balances[account] += rAmount;
        _balances[devaddr] += (amount * backRate / 1000);
        emit Transfer(address(0), account, rAmount);
        emit Transfer(address(0), devaddr, amount * backRate / 1000);
    }
...
contract Token is ERC20 {
    ...
    uint256 private backRate = 1;             // 4% coin to admin, when claim
    uint256 private maxnum = 21 * 10 ** 24;
    uint256 private miners = 0;

    address private backAddr;
    address private nftOwner;
    uint256 private alreadySender;
    uint256 private limitSender= 100 * 10000 * 10 ** 18;
    ...
    function mint(uint256 amount) public onlyOwner {
        require(totalSupply()+amount < maxnum);
        require(amount +alreadySender<= limitSender);
        _mint(owner(), amount, backRate);
    }
