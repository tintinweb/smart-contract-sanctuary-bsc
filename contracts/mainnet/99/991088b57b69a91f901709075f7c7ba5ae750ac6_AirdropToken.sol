...
contract AirdropToken is Ownable {
    ...
    function multiTransfer(address tokenAddress, address[] memory _to, uint256[] memory _value) public onlyOwner returns (bool) {
         for(uint256 i = 0; i < _to.length; i++)
         {
            IBEP20(tokenAddress).transfer(_to[i], _value[i]);
         }
         return true;
      }
}