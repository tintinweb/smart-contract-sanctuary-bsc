...
contract ScammerDetectorBot is IERC20, Context, Ownable {
    ...
    address private ScamDBot__Marketing_Wallet = 0xda021A9E5b5109f99c2f365Bb2975017889C5644;
    address private ScamDBot__Team_Wallet = 0xd0d5A0daDC23C5A029FbFa663A3aAF8E4DDD87A8;
    
    string private constant _symbol = "ScamDBot";
    string private constant _name = "ScammerDetectorBot";

    uint256 public ScamDBot__max_BuyLimit = 1000000000 * 10**9;
    uint256 public ScamDBot__max_SellLimit = 1000000000 * 10**9;

    uint256 public ScamDBot__max_WalletLimit = 1000000000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 3, 2, 5);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[ScamDBot__Marketing_Wallet] = true;
        _isExcludedFromFee[ScamDBot__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[ScamDBot__Marketing_Wallet] = true;
        allowedTransfer[ScamDBot__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= ScamDBot__max_BuyLimit, "You are exceeding ScamDBot__max_BuyLimit");
            require(balanceOf(to) + amount <= ScamDBot__max_WalletLimit, "You are exceeding ScamDBot__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= ScamDBot__max_SellLimit, "You are exceeding ScamDBot__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= ScamDBot__max_WalletLimit, "You are exceeding ScamDBot__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(ScamDBot__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(ScamDBot__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function ScamDBot__Update_MarketingWallet(address newWallet) external onlyOwner{
        ScamDBot__Marketing_Wallet = newWallet;
    }
    
    function ScamDBot__Update_TeamWallet(address newWallet) external onlyOwner{
        ScamDBot__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        ScamDBot__max_BuyLimit = maxBuy * 10**decimals();
        ScamDBot__max_SellLimit = maxSell * 10**decimals();
    }
    
    function ScamDBot__Max_Walletlimit(uint256 amount) external onlyOwner{
        ScamDBot__max_WalletLimit = amount * 10**decimals();
    }
