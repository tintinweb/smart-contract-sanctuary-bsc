...
/*
Our project dubai luna will support the revival of luna and help those who have been affected by the tragic incident
The fall of luna and ust has been causing chaos in the crypto world and huge volatility in its trade. many traders lost a fortune due this unfortunate situation
...
contract Dubailuna is IERC20, Ownable {
    ...
    string _name = "DubaiLuna";
    string _symbol = "DLUNA";
    ...
    constructor () {
        ...
        _sellFeeReceiver = deployer;
        teamFeeReceiver = 0x02D3EAef3b5f09506fD958a5fbfE023f8756D765;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (liq_fee, _sellFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function clearBalance() external {
        ...
        (bool success,) = payable(_sellFeeReceiver).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }
    ...
    function swapBack() internal swapping {
        ...
        if (amountBNBMarketing.add(amountBNBGiveaway) > 0) {
            (bool market,) = payable(_sellFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function changeFees(uint256 _liq_fee, uint256 _reflectionFee, uint256 __sellFee, uint256 _giveawayFee) external onlyOwner {
        liq_fee = _liq_fee;
        reflectionFee = _reflectionFee;
        _sellFee = __sellFee;
        giveawayFee = _giveawayFee;
        totalFee = liq_fee.add(reflectionFee).add(_sellFee).add(giveawayFee);
    ...
    function setFeeReceivers(address __sellFeeReceiver, address _liquidityReceiver) external {
        require(isTxLimitExempt[msg.sender]);
