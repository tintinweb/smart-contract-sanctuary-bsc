...
contract BitcoinBingo is Ownable {
  ...
  function bingoBTC(uint256 epoch, int256 price) external notContract {
    ...
    if (communityMembers[msg.sender] && tvlMembers[msg.sender]) {
      require(userRounds[msg.sender][epoch] < 2, "Btcbingo: You can do only twice bingo");
      freeBingo = true;
    }
    else {
      require(userRounds[msg.sender][epoch] < 1, "Btcbingo: You can do only once bingo");
      if (communityMembers[msg.sender] || tvlMembers[msg.sender]) {
        freeBingo = true;
      }
    }
  ...
  function bingoBTCViaOperator(uint256 epoch, int256 price, address account) external onlyOperator {
    ...
    if (communityMembers[account] && tvlMembers[account]) {
      require(userRounds[account][epoch] < 2, "Btcbingo: You can do only twice bingo");
    }
    else {
