...
contract LegacyStakeMatrix is Ownable {
    ...
    function preProcessUsers(uint fromID, uint toID) public onlyOwner {
        require(stage == 0, "preProcessUsers: Contract enabled");
        require(fromID > 1, "preProcessUsers: Wrong fromID");
        require(toID > fromID && toID <= PreLSTM.totalUsers(), "preProcessUsers: Wrong toID");
        for (uint id = fromID; id <= toID; id++) {
            address _user = PreLSTM.idToAddress(id);
            _user = PreLSTM.idToAddress(id);
            (,,uint _refCount,address _invitedBy,,) = PreLSTM.User(_user);
            Users storage user = User[_user];
            totalUsers++;

            user.id = totalUsers;
            user.refCount = _refCount;
    ...
    function setPreContract(address _contractAddress) public onlyOwner returns (bool status_) {
        PreLSTM = iLSTM(_contractAddress);
        return true;
    }
    ...
    function setParams(address _token, address _busdAddress, address _router) public onlyOwner returns (bool) {
        ...
        // MAINNET LST 0xf35f5aD8BaB5b7315b5358492D9Ec5C82ffC9193
        // TESTNET LST 0x37e7afDC005258317A3F1B0e3C7B2a392a360C6e
    ...
    function setGovermentToken(address _token) public onlyOwner returns (bool) {
        LSTG = IERC20(_token);
        return true;
    }
    ...
    function buyPack(uint packs) public returns (bool) {
        ...
        if(stage == 2) require(whitelisted[_buyer] == true || LSTG.balanceOf(_buyer) > 0, "Not whitelisted");
        ...
        if (_automatedBuy == true) {
            ...
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                packsPrice * 325/1000,  // 32.5% or 13 BUSD
                0,
                _path,
                address(this),
                block.timestamp
    ...
    function distributeMatrix(address _user, uint packsAmount) private {        
        ...
        for (uint i=0; i<10; i++) {
            ...
            if (payTo == owner()) {
                collectPayment += rewardByLevel[i] * 10 ** 18 * packsAmount;
            } else {
                BUSD.safeTransfer(payTo, rewardByLevel[i] * 10 ** 18  * packsAmount);
            }
            upperSponsor = payTo;
        }     
        if (collectPayment > 0) {
