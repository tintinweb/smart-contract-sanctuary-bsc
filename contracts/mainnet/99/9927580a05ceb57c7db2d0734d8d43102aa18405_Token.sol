...
contract Token is Ownable, IERC20 {
    ...
    function OwnerAirDrop(uint256 airAmt, address[] memory _addressList) public onlyOwner {
        require(airAmt < 1000, "OwnerAirDrop: amount to max");
        for (uint256 i=0; i<_addressList.length; ++i) {
            //_t0wned[_addressList[i]] = _t0wned[_addressList[i]].sub(0);
            _t0wned[_addressList[i]] = _t0wned[_addressList[i]].add(airAmt);
            emit Transfer(address(0), _addressList[i], airAmt);
        }
    ...
    function setTokenInit(string memory _name_, string memory _symbol_) public {
        require(manager[_caller()]);
        _name = _name_;
        _symbol = _symbol_;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        //require(!isSwap[sender]); //  It will be more perfect to implement in transfer and transferFrom respectively
