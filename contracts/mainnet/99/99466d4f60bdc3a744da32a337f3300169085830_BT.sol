...
contract BT is ERC20 {
    ...
    address _nodeAddress = address(0x849906123EEa99189AdB07d680934F94C1fe6434);
    address _marketAddress = address(0x126aC94414198dFAe5948f215c52E76A05649c6E);
    ...
    constructor(address tokenOwner) ERC20("BT", "BT") {
	...
	function changeSwapWarp(UngWarp _warp) public onlyOwner {
        warp = _warp;
    ...
    function _transfer(
		...
		bool isInviter = from != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 2 * 10**18;
    ...
    function swapAndLiquify(uint256 tokenAmount) private {
        uint256 halfusdt = tokenAmount.div(6).mul(5);
        swapTokensForOther(halfusdt);
        uint256 allUsdt = USDT.balanceOf(address(this));
        USDT.transfer(_fundAddress,allUsdt.div(18).mul(2));
        USDT.transfer(_nodeAddress,allUsdt.div(18).mul(3));
        USDT.transfer(_marketAddress,allUsdt.div(18).mul(8));
    ...
    function swapAndLiquifyStepv1() public {
        ...
        if(usdtBalance.mul(tokenBalance) > 10**34){
