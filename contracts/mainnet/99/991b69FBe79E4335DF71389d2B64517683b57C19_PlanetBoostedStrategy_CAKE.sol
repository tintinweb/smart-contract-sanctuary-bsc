...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    bool public infinityStaking = true;
    ...
    event SetInfinityStaking(bool _staking);
    ...
    function _update() internal virtual {
        ...
        diff = cakeRewardsAfter.sub(cakeRewardsBefore);
        else
        diff = 0;
        ...
        uint256 amount = gamma_bal_after.sub(gamma_bal_before);

        stratRewardPerShare = stratRewardPerShare.add(amount.mul(1e12).div(sharesTotal));
    }
    ...
    function safeRewardTransfer(address caller,uint pendingProfit) internal {
        ...
        GammaInfinityVault(gammaInfinityVault).depositAuthorized(caller, pendingProfit);

    }

    function safeGAMMATransfer(address _to, uint256 _GAMMAAmt) internal {
        uint256 GAMMABal = IERC20(GAMMAAddress).balanceOf(GAMMAAddress);
        if (_GAMMAAmt > GAMMABal) {
            IERC20(GAMMAAddress).safeTransfer(_to, GAMMABal);
        } else {
            IERC20(GAMMAAddress).safeTransfer(_to, _GAMMAAmt);
        }
    }

    function earnGammaProfits() external {
        
        _update(); // claim pending CAKE rewards from Pancake and convert it to GAMMA
	    ...
	    uint256 rewardsToBeTransferred = user.pendingRewards; 
	    user.pendingRewards = 0;

	    if(infinityStaking)	
            safeRewardTransfer(_msgSender(),rewardsToBeTransferred);
	    else
		    safeGAMMATransfer(_msgSender(), rewardsToBeTransferred);
        }
    ...
    function _farm(uint256 wantAmt) internal virtual {
        ...
        IPancakeswapFarm(farmContractAddress).enterStaking(wantAmt); // Just for CAKE staking, we dont use deposit() for cake staking

    }

    function _unfarm(uint256 _wantAmt) internal virtual {
        
        IPancakeswapFarm(farmContractAddress).leaveStaking(_wantAmt); // Just for CAKE staking, we dont use withdraw() for cake staking
        ...
        returns (uint256)
    {
        ...
        require(user.shares >= _wantAmt, "_wantAmt greater than user shares");
...
function userPendingGammaProfit(address userAddress) external view virtual returns (uint256 pendingProfitInGamma){
            
        UserInfo memory user = userInfo[userAddress];
        
        if(stratRewardPerShare > 0 && user.shares > 0){
            uint256 pendingGamma;
            ...
            if(cakeBal > 0){
            	uint256[] memory amounts = IPancakeRouter02(planetRouterAddress).getAmountsOut(cakeBal,earnedToGAMMAPath);
            	pendingGamma = amounts[amounts.length - 1];
            }

            uint256 tempRewardPerShare = 0;
            if(pendingGamma > 0 && sharesTotal > 0)
            	tempRewardPerShare = stratRewardPerShare.add(pendingGamma.mul(1e12).div(sharesTotal));
            else
            	tempRewardPerShare = stratRewardPerShare;
            
            if((user.shares).mul(tempRewardPerShare) > (user.earnRewardDebt))
            	pendingProfitInGamma = ((user.shares).mul(tempRewardPerShare).div(1e12).sub(user.earnRewardDebt));
            else
            	pendingProfitInGamma = 0;
            
        }
	pendingProfitInGamma += user.pendingRewards;
    }
    ...
    function setInfinityStaking(bool _staking) public virtual onlyAllowGov {
        infinityStaking = _staking;
        emit SetInfinityStaking(_staking);
    }
...
contract PlanetBoostedStrategy_CAKE is StratX2 {
    ...
    constructor(
        ...
        planetRouterAddress = _addresses[8];
        entranceFeeFactor = _entranceFeeFactor;
        withdrawFeeFactor = _withdrawFeeFactor;
    ...
    function changeGammaInfinityVault(address _newGammaInfinityVault) public onlyAllowGov{
