...
contract OmniMetros is Ownable, ERC721, ILayerZeroReceiver {
    ...
    constructor(address _endpoint, string memory _baseURL)
        ERC721("OmniMetros", "OMTRO")
    {
    ...
    function mint(uint256 amount) external payable {
        ...
        require(nextId + amount <= MAX, "Purchase exceed max supply of tokens");
        require(
            amount > 0 && amount <= 5,
            "Purchase exceed max limit per transaction"
        );

        uint256 amountToPay = PRICE * amount;

        require(amountToPay <= msg.value, "Insufficient fund");

        counter += 1;

        for (uint256 i = 0; amount > i; i++) {
    ...
    function withdrawBalance() public payable onlyOwner {
        ...
        _withdraw(0xe432e8ab4033A8645e8EF05155D3A0E72e95FEf3, balance);
    }
