...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        holderCondition = 200000 * 10 ** Decimals;
        NFTRewardCondition = 20 ** IERC20(USDTAddress).decimals();
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if(sender == PinkAddress && !_swapPairList[recipient])
            {
                uint256 oldbalance = _balances[recipient];
                uint256 totalvalue = _userHoldPrice[recipient].mul(oldbalance); 
                totalvalue += tAmount.mul(Startprice);
                _userHoldPrice[recipient]= totalvalue.div(oldbalance.add(tAmount));
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        if(tokenAmount > balanceOf(address(this))) return;
        ...
        uint256 fundAmount_A = fundAmount.mul(25).div(100);
    ...
    function setStartPrice(uint256 price) external onlyOwner{
        Startprice = price;
    }

    function setPinkAddress(address pink) external onlyOwner{
        PinkAddress = pink;
        _feeWhiteList[pink] = true;
        excludeHolder[pink] = true;
        excludeNFTHolder[pink] = true;
    }
    ...
    function getCurrentPrice() public view returns (uint256)
    {
        ...
        if(IUniswapV2Pair(_mainPair).token0() == address(_swapRouter.WETH()))
        {
...
contract Nian is AbsToken {
    constructor(address _f) AbsToken(
        ...
        18,
        1_0000_0000,
        address(0x5770a608F97424552Cdf8c3097156c08312a594b),
        address(0x5487601B71d08cAD493b4a911141a04A989993c8),
        address(0x9B7726f677c956a5fE3BA147ceB7fDeD27e6349D)
    ){}
}