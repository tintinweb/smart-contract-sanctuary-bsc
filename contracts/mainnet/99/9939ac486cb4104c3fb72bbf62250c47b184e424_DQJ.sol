...
contract DQJ is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("DQJ", "DQJ") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(20) > block.timestamp){
                  ...
                  if(startTime.add(60) > block.timestamp){require(balanceOf(to).add(amount) <= 2 * 10**20,"max amount");} 
        ...
        if (takeFee) {
			...
			changeUsdtAmount = changeUsdtAmount.add(amount.div(100).mul(29));
			amount = amount.div(100).mul(71);
				}else{
            super._transfer(from, address(this), amount.div(100).mul(3));
			changeUsdtAmount = changeUsdtAmount.add(amount.div(100).mul(3));
			amount = amount.div(100).mul(97);
