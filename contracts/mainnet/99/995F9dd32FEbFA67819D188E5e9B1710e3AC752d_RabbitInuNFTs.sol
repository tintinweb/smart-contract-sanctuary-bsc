...
contract RabbitInuNFTs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0x740eBb632DA8856129cb50649aCe8120Ba623AfF;
    address public FeeReceiver = 0xD8eB865fDF1421Ce9EEEF4434FCb18620e5CeFA4;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "RabbitInuNFTs: minting has not started yet");
        require(_mintAmount > 0, "RabbitInuNFTs: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "RabbitInuNFTs: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "RabbitInuNFTs: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "RabbitInuNFTs: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "RabbitInuNFTs: zero address");
        require(_amount > 0, "RabbitInuNFTs: you have to mint at least one");
        require(lastSupply != 0, "RabbitInuNFTs: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "RabbitInuNFTs: URI query for nonexistent token");
