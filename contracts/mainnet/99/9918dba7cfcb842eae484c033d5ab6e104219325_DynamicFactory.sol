...
contract DynamicFactory is IDynamicFactory {
    ...
    function _createPair(address tokenA, address tokenB, uint8 circuitBreaker) internal returns (address pair) {
        ...
        */
        ...
        if ((circuitBreaker == 3 && token0 == WETH) // protect token1

            || (circuitBreaker == 1 && token1 == tokenA)

            || (circuitBreaker == 2 && token1 == tokenB))
        ...
        } else if ((circuitBreaker == 3 && token1 == WETH) // protect token0

            || (circuitBreaker == 1 && token0 == tokenA)

            || (circuitBreaker == 2 && token0 == tokenB))
