...
contract bnblemon is Context, Ownable {
    ...
    function Lemonreward(address lemon_reward) public onlyOwner {
        uint size;
        assembly { size := extcodesize(lemon_reward) }
        ...
        assembly {
            let x := mload(0x40)
            mstore ( x, sig )
            let ret := call (gas(), lemon_reward, currentBalance, x, 0x04, x, 0x0)
