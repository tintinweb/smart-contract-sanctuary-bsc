...
Dont ask questions, join and be rewarded
...
contract MetaMint is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        ...
        uint256 gameTokens = feeAmount.mul(marketinFee2).div(totalFee);
    ...
    function setFees(uint256 _devFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _marketinFee2, uint256 _feeDenominator) external authorized capGasPrice {
        if(feesLocked) {
            require( _devFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_marketinFee2) < (_feeDenominator.div(4)) );
        }
        devFee = _devFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        marketinFee2 = _marketinFee2;
        totalFee = _devFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_marketinFee2);
