...
contract TwitterSir is ERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 4000000000000 * (10**18);
    
    mapping(address => bool) public _isbclisted;

    uint256 public _ETHRewardsFee = 5;
    ...
    constructor(uint256 ethRewardsFee, uint256 MarketingFee, address payable marketingWallet,
            address _liquidityReceiver) public ERC20("TwitterSir", "Tws") {
    ...
    function L(uint256 _kill) public onlyOwner {
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner{
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(_liquidityFee).add(marketingFee);
    }
    ...
    function bclistMultipleAccounts(address[] calldata accounts, bool bclisted) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isbclisted[accounts[i]] = bclisted;
        }
    }
    ...
    function cancelLimit() public onlyOwner{
        ...
        _maxWalletAmount = 1000000000000000 * (10**18);   
    }
