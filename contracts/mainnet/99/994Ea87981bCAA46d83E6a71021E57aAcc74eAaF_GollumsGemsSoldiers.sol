...
contract GollumsGemsSoldiers is ERC721Metadata {
    ...
    //Tax-Free Users
    mapping(address => uint256) public tax_freeUsers;

    // Total supply of NFTs
    uint256 private _totalSupply;

    // Pending Ids
    uint256[101] private _pendingIds;
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "GollumsGemsSoldiers: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("Gollums Gems Soldiers", "GGSoldiers", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "GollumsGemsSoldiers: invalid _startTime");
        require(_startTime > block.timestamp, "GollumsGemsSoldiers: old start time");
        startTime = _startTime;
    }

    function presalePurchase(uint256 numberOfNfts) external payable onlyWhitelisted{
        require(presaleEnabled, "Presale is not active");
        require(pendingCount > 0, "GollumsGemsSoldiers: All minted");
        require(numberOfNfts > 0, "GollumsGemsSoldiers: numberOfNfts cannot be 0");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"GollumsGemsSoldiers: sale already ended");
        require(MINT_PRICE * (numberOfNfts) == msg.value, "GollumsGemsSoldiers: invalid ether value");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "GollumsGemsSoldiers: All minted");
        require(numberOfNfts > 0, "GollumsGemsSoldiers: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "GollumsGemsSoldiers: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"GollumsGemsSoldiers: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "GollumsGemsSoldiers: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "GollumsGemsSoldiers: invalid ether value");
            tax_freeUsers[msg.sender] = 0;
        }
        else{
            tax_freeUsers[msg.sender] -= numberOfNfts;
        }
        for (uint i = 0; i < numberOfNfts; i++) {
            _randomMint(msg.sender);
            _splitBalance(msg.value / numberOfNfts);
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "GollumsGemsSoldiers: Only owner or approved can claim rewards"
        );
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "GollumsGemsSoldiers: max supply reached");
    ...
    function setTaxFreeNftPerUser(address user, uint256 value) external onlyOwner{
        tax_freeUsers[user] = value;
    }
