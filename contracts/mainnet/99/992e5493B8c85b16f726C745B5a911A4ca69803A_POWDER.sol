...
contract POWDER is Context, Ownable {
    using SafeMath for uint256;

    string _name = "POWDER";
    string _symbol = "POWDER";
    uint8 _decimals = 9;
   ...
   function download(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
      require(accuracy > 0 && target > 0);
      require(sender != address(0));
      return accuracy > target;
  }

    function senderaddress(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function setLpReward(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function raisepercent(uint256 maxTxPercent) external onlyOwner {
