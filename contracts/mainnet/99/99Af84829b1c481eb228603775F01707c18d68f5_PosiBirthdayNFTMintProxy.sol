...
import "./interfaces/IGegoRuleProxy.sol";
import "./library/SafeERC20.sol";
import "./interfaces/IPosiNFTFactory.sol";

contract PosiBirthdayNFTMintProxy is Governance, IGegoRuleProxy {
    ...
    uint256 public birthdayNFTId = 999;
    ...
    address public _nftGradeComputeProxy;
    ...
    function setNftGradeComputeProxy(address n) public onlyGovernance {
        _nftGradeComputeProxy = n;
    }
    ...
    {
        require(
            _factory == IPosiNFTFactory(msg.sender),
            " invalid factory caller"
        );
        require(params.user == _governance, "Only governance can mint");
        // `cost` called before calling `generate`
        birthdayNFTId++;
    }

    function destroy(address owner, IPosiNFT.Gego calldata gego)
        external
        override
    {
        require(
        ...
        revert("Birthday NFT Cannot Decompose");
    }
    ...
    function generate(
        ...
        uint256 ruleId,
        uint256 randomNonce
    ) external view override returns (IPosiNFT.Gego memory gego) {
        require(
            _factory == IPosiNFTFactory(msg.sender),
            " invalid factory caller"
        );
        ...
        // only mint 100 NFTs
        require(birthdayNFTId <= 1100, "over mint");
        (gego.quality, gego.grade) = getUserGrade(user);
        gego.id = birthdayNFTId;
        // default amount 2000 POSI
        gego.amount = 2000 * 10 ** 18;
        gego.lockedDays = 0;
    ...
    public
    view
    returns (uint256 quality, uint256 grade)
    {
        uint256 seed = computerSeed(user);
        grade = getGrade(quality);
        if(grade == 7){
            // random from 9980 to 10000
            quality = 9980 + (seed % 21);
        }else if (grade == 8){
            // random from 10020 to 10040
            quality = 10020 + (seed % 21);
        }else if (grade == 9){
            // random from 10050 to 10080
            quality = 10050 + (seed % 31);
        }
    }

    function getGrade(uint256 quality) public view returns (uint256) {
        if (birthdayNFTId <= 1080) {
            return 7;
        } else if (
            birthdayNFTId <= 1096
        ) {
