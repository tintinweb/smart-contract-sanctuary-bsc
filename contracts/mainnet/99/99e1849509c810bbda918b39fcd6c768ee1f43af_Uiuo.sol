...
contract Uiuo is Context, Ownable {
    // 静态收益
    uint public STATIC_RATE_100_1999 = 125; // 1.5%一天，12次每天，每次分125/100000的利息
    uint public STATIC_RATE_2000_4999 = 150; // 每次分150/100000的利息
    uint public STATIC_RATE_5000 = 166; // 每次分166/100000的利息
    // 推荐直接佣金U
    ...
    function checkCanActive(address account) public view returns(bool){
        ...
        if(totalBuyAmount < 300000000000000000000){ // 300U以上才能激活
            return false;
        }else{
            return true; // 可以激活
        }
    ...
    function doActive() public returns(bool){
        ...
        _userInfo[msg.sender].totalDeposit += 200000000000000000000; // 送200 U
    ...
    function getStaticRewardRate(address account) public view returns (uint){
        ...
        uint rewardRate = 0;
        if(totalDeposit < 100000000000000000000){ // 0-99U没有收益
            return 0;

        }else if(totalDeposit >= 100000000000000000000 && totalDeposit <= 1999000000000000000000){ // 1000-1999U
            rewardRate = STATIC_RATE_100_1999;

        }else if(totalDeposit >= 2000000000000000000000 && totalDeposit <= 4999000000000000000000){ //2000-4999U
            rewardRate = STATIC_RATE_2000_4999;

        }else{
            // >= 5000U
            rewardRate = STATIC_RATE_5000;
        }
        return rewardRate;
    }
