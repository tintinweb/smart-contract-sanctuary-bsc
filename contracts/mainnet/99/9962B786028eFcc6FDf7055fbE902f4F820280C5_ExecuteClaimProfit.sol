...
contract ExecuteClaimProfit is UnstakeLogic, StakeLogic, IExecutable {
  ...
  */
  function claimProfitAndExit(
    ...
    if (stakedRewards[_un.stakeContractAddress][_un.pid].length > 0) {
      return claimProfitAndExitYieldPool(_un, _wl.fromliquidityPoolAddress, _rewards[0]);
    } else {
      return claimProfitAndExitPool(_un, _vl, _wl, _rewards, _claimProfitType);
    }
  }
  ...
  @param _claimProfitType type of claim profit
  */
  function claimProfitAndExitPool(
    UnstakeSet memory _un,
    VaultToLiquiditySet memory _vl,
    WithdrawLiquiditySet memory _wl,
    address[] memory _rewards,
    ...
    uint256 profit = investment.initialAmount > withdrawn ? investment.initialAmount - withdrawn : 0; 
  ...
  @param _un used in unstakeAndWithdrawLiquidity
  */
  function claimProfitAndExitYieldPool(
    UnstakeSet memory _un,
    address _yieldLpPool,
    address _yieldReward
  ) internal {
    exitYieldPool(_un);
    IERC20(_yieldLpPool).transfer(investor, _un.amount);
    IERC20(_yieldReward).transfer(investor, IERC20(_yieldReward).balanceOf(address(this)));
  }
  ...
  */
  function calculateAndSendYieldProfit(
    ...
    emit ProfitYieldClaimed(toBurn, toDevelopment, profit);
  }
...
abstract contract UnstakeLogic is ExecutableParams, IUnstakeLogic {
  ...
  */
  ...
  function exitYieldPool(UnstakeSet memory _un) internal {
    require(stakedRewards[_un.stakeContractAddress][_un.pid].length > 0, "Wrong pool");

    refreshStakedRewards(_un.stakeContractAddress, _un.pid, _un.amount);
    _unstake(_un.poolTemplate, _un.stakeContractAddress, _un.amount, _un.pid, _un.extraBytes);
  }
  ...
  @param _stakeContract the stake contract address. In case default stake contract was changed,
   client can still claim rewards from previous pool..
  ...
  */
  function refreshStakedRewards(address _stakeContract, uint256 _pid, uint256 _amountToRemove) internal {
    ...
    delete stakedRewards[address(0)][_pid];
