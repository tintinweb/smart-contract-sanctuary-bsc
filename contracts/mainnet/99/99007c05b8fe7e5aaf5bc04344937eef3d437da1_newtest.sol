...
contract newtest is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    address payable public _marketingWalletAddress = payable(0xc36a02721D9f60cE9F28c7099834fd91aa26175B);
    ...
    constructor() BEP20("New Test", "NewTest") 
    {
    ...
    {
        require(newAddress != address(dividendTracker), "newtest: The dividend tracker already has that address");
        newtestDividendTracker newDividendTracker = newtestDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "newtest: The new dividend tracker must be owned by the newtest token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "newtest: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "newtest: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "newtest: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "newtest: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "newtest: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "newtest: Cannot update gasForProcessing to same value");
...
contract newtestDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("newtest_Dividend_Tracker", "newtest_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(false, "newtest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main newtest contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "newtest_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "newtest_Dividend_Tracker: Cannot update claimWait to same value");
