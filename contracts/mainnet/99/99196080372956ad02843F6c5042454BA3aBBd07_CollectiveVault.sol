...
contract CollectiveVault is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        XIV = IERC20(xiv);
        fees = 2500;
        //resolve duration to 24 hours
        resolveDuration = 86400;
    }
    ...
    function predict(
        ...
        require(!_isContract(_msgSender()), "CV: Contract not allowed");
    ...
    function resolving(
        ...
        // 24 hours on mainnet
        require(
            (predictionDetails.endTime + (resolveDuration)) < block.timestamp,
            "CV: Invalid resolved time"
        );
        ...
        require(
            _predictionType != 2,
            "CV: Prediction should be solo and user-user"
        );
        //solo or user-user
        Prediction storage userDetails = UserPrediction[_predictionType][
            _token
        ][_counter][predictionDetails.user[0]];
        ...
        for (uint8 i = 0; i < predictionDetails.user.length; i++) {
            ...
            userDetails = UserPrediction[_predictionType][_token][_counter][
                user
            ];

            uint256 a = abs(int256(userDetails.price) - int256(finalPrice));
        ...
        for (uint8 i = 0; i < _winnerCounter; i++) {
            ...
            if (winners[i] == winners[_winnerCounter - 1]) {
                require(
                    ...
                    "CV: Transfer failed in resolving"
                );
    ...
    function resolvingShared(
        ...
        if (oddUser != address(0)) {
            ...
            amt =
                (predictionDetails.totalAmount - userDetails.amount) /
                winners.length;
        }
        address t = _token;
        ...
        for (uint8 i = 0; i < predictionDetails.user.length; i++) {
            ...
            if (oddUser != address(0) && user == oddUser) {
                ...
                require(
                    ...
                    "CV: Transfer failed in resolve shared"
                );
                emit ResolvedPredictions(
                    _counter,
                    userDetails.amount,
    ...
    function setOperator(address _operator) public onlyOwner {
        require(_operator != address(0), "CV: Invalid operator address");
        operator = _operator;
    }

    //set the resolve duration in seconds
    function setResolveDuration(uint256 _seconds) public onlyOwner {
        require(_seconds > 0, "CV: Invalid duration");
        resolveDuration = _seconds;
    }
