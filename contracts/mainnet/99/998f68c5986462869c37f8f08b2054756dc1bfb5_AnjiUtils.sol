...
contract AnjiUtils {
    ...
    function getPairAddress(address tokenAddr, address[] memory possiblePairs, address dexFactory, bytes32 dexHash) public view virtual returns (address, address) {
        ...
        for (uint256 index = 0; index < possiblePairs.length; index++) {
            ...
            if(IDEXFactory(dexFactory).getPair(tokenAddr, possiblePairs[index]) == address(0)) { continue; }           
            
            (uint256 reserve0, uint256 reserve1) = DEXLibrary.getReserves(dexHash, dexFactory, tokenAddr, possiblePairs[index]);
            (address token0, ) = DEXLibrary.sortTokens(tokenAddr, possiblePairs[index]);

            if (token0 == tokenAddr) {
                if (reserve0 > largestReserve) {
                    pair = IDEXFactory(dexFactory).getPair(tokenAddr, possiblePairs[index]);
                    paired_coin = possiblePairs[index];
                }
            } else {
                if (reserve1 > largestReserve) {
                    pair = IDEXFactory(dexFactory).getPair(tokenAddr, possiblePairs[index]);
                    paired_coin = possiblePairs[index];
                }
            }
        ...
        return (pair, paired_coin);
    }

    function getDexBestPairAddresses(address tokenAddr, address[] memory possiblePairs, address[] memory dexFactories, bytes32[] memory dexHashes) public view virtual returns (address[] memory, address[] memory) {
        require(dexFactories.length == dexHashes.length, 'Factories and Hexs much match in amounts');
        address[] memory pairs = new address[](dexFactories.length);
        address[] memory paired_coins = new address[](dexHashes.length);

        for (uint256 index = 0; index < dexFactories.length; index++) {
            (address pair, address paired_coin) = getPairAddress(tokenAddr, possiblePairs, dexFactories[index], dexHashes[index]);
            pairs[index] = pair;
            paired_coins[index] = paired_coin;
        }

        return (pairs, paired_coins);
    }
    ...
    function getTokenPairReservesInfo(address[] memory token_addresses, address[] memory token_pair_addresses) public view virtual returns (PairReserveInfo[] memory) {
        ...
        for (uint256 index = 0; index < token_pair_addresses.length; index++) {
            reserveInfo[index] = getTokenPairReserveInfo(token_addresses[index], token_pair_addresses[index]);
        }

        return reserveInfo;
    }

    function getTokenPairReserveInfo(address tokenAddress, address tokenPairAddress) public view virtual returns (PairReserveInfo memory) {
