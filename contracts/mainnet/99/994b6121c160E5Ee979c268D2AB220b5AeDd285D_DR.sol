...
contract DR is ERC20 {
    ...
    function _checkLimit(address account) private view {
        if(account == pair || excludedFee[account]) {
            return;
        }
        if(startBlock != 0 && block.number > startBlock + 28800) { // 28800
            return;
        }
        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(pair).getReserves();
        address token0 = IUniswapV2Pair(pair).token0();
        uint256 deltaAmount;
        uint256 lpAmount = IERC20(pair).balanceOf(account);
        uint256 lpSupply = IERC20(pair).totalSupply();
        if(lpAmount > 0) {
            if(token0 == address(this)) {
                deltaAmount = lpAmount * reserve0 / lpSupply;
            } else {
                deltaAmount = lpAmount * reserve1 / lpSupply;
            }
        }
        deltaAmount += 1e18;
        require(deltaAmount >= balanceOf(account), "exceed LimitAmount"); 
    }
    ...
    function _transfer(
        ...
        if(from == pair || to == pair) {
            ...
            if(!excludedFee[addr]) {
                if(from == addr) {
                    require(balanceOf(addr) * 99 / 100 >= amount, "1% of the token is reserved");
                }
                ...
                uint fee = amount * taxFee / DENOMINATOR;
                amount -= fee;
    ...
    function swap() private {
        ...
        if(bal > 0) {
            ...
            if(balance > 0) {
                uint256 marketFee = balance / 2;
                IERC20(token).transfer(marketAddr, marketFee);
                IERC20(token).transfer(pool, balance - marketFee);
            }
        }
    }
