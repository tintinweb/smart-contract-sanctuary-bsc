...
contract FuoIdo is Ownable {
    ...
    address private excFeeAddr = 0x80e98fb28BE591EeEE503e963B36A51CB4877DAc;
    ...
    function exchange(uint256 exType,uint256 amount) public {
        ...
        if(exType == 1){
            ...
            if(excInFee > 0){
                uint256 feeAmount = excInFee * amount / 1000;
                usdtToken.transferFrom(address(msg.sender),excFeeAddr,feeAmount);
                usdtAmount = amount - feeAmount;
            }
            usdtToken.transferFrom(address(msg.sender),masterAddr,usdtAmount);
            ...
            if(excOutFee > 0){
               uint256 feeAmount = excOutFee * amount / 1000;
               futToken.transferFrom(address(msg.sender),excFeeAddr,feeAmount);
               tokenAmount = amount - feeAmount;
            }
            futToken.transferFrom(address(msg.sender),masterAddr,tokenAmount);
    ...
    function init2(address target,uint256 myPower,uint256 rewardPower,uint256 takeTime,uint256 takeAmount,uint256 takeValue,uint256 lockAmount,uint256 lockValue,uint256 active,uint256 totalInvest,uint256 waitProfit) public onlyOwner {
        pledgeMap[target].myPower = myPower;
        pledgeMap[target].rewardPower = rewardPower;
