...
contract TTT is IERC20, Ownable {
    ...
    event UpdatedFees(uint256 burnFee, uint256 taxFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getValues(tAmount, isTrading(sender, recipient)==2);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _getValues(uint256 tAmount, bool isSell) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tBurn) = _getTValues(tAmount, isSell);
    ...
    function updateFees(uint256 BurnFee, uint256 TaxFee) public onlyOwner {
        require(TaxFee <= 3, "Total taxes must be under 3%");

        _burnFee = BurnFee;

        _taxFee = TaxFee;

        emit UpdatedFees(_burnFee, _taxFee);
    }
