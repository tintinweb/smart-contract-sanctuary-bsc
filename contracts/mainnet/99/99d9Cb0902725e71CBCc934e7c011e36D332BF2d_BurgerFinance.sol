...
/// ALWAYS CHECK HERE
    ...
    constructor(address _owner, address _mkt, address _treasury1, address _treasury2, address _treasury3) {
		require(!isContract(_owner) && !isContract(_mkt) && !isContract(_treasury1) && !isContract(_treasury2) && !isContract(_treasury3));
        owner = _owner;       
        mkt = _mkt;
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_BUSD.transfer(treasury3, tax);
        return tax.mul(5);
    }
    ...
    function CHANGE_TREASURY1_WALLET(address value) external {
        require(msg.sender == treasury1, "Admin use only.");
        treasury1 = value;
    }

    function CHANGE_TREASURY2_WALLET(address value) external {
    require(msg.sender == treasury2, "Admin use only.");
    treasury2 = value;
    }
    function CHANGE_TREASURY3_WALLET(address value) external {
    require(msg.sender == treasury3, "Admin use only.");
    treasury3 = value;
    }
...
BURGERHUT SALES
Contract Renounced
  Buy Burgers, Upsize and Sell Burgers for BUSD.
  8% daily Rate
  5% Referral Bonus, will go directly to referrer wallet.
  2% stacking compound bonus every 12 hrs, max of 5 days. (20%)
  48 hours cut off time.
  20 BUSD minimum investment.
  10,000 BUSD max deposits per wallet.
  80% penalty for withdrawals less than 5 compounds
