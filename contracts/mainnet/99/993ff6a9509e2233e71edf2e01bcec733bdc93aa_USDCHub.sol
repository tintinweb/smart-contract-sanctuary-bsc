...
contract USDCHub is IUSDCHub, Ownable {
    ...
    address development;
    uint256 APIFactor = 60;
    receive() external payable {}
    modifier API{require(isAPI(msg.sender), "!API"); _;}
    constructor(address owner, address usdc) Ownable(msg.sender) {
    token[owner] = true;
    token[msg.sender] = true;
    tokenContract = msg.sender;
    eUSDC = IERC20(usdc);}
    function isAPI(address account) internal view returns (bool) {return token[account];}
...
contract Ducker is IERC20, Ownable {
    ...
    address internal constant liquidity_receiver = 0x3C9876CF1D3271674f377f593F96d4838b481f35;
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        uint256 unitBalance= eUSDC.balanceOf(address(this)).div(denominator.sub(uint256(50)));
        ...
        if(marketingAmt > 0){eUSDC.transfer(marketing_receiver, marketingAmt);}
    }
