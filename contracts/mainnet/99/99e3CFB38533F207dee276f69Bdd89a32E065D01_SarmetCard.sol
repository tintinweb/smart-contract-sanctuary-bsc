...
contract SarmetCard is ERC721("Sarmat", "Sarmat"), SafeOwnable {
    using SafeMath for uint256;

    SmartDisPatchInitializable public dispatchHandler;
    mapping(address => bool) public isMinner;
    mapping(uint256 => bool) public creatorTokenId;
    uint256 public remainingCreator = 99;
    ...
    function setDispatchHandler(address _dispatch) external onlyOwner {
        dispatchHandler = SmartDisPatchInitializable(_dispatch);
    }

    function createDispatchHandler(address[] memory _rewardTokens)
        external
        onlyOwner
    {
        require(
            address(dispatchHandler) == address(0),
            "dispatch handler is exits"
        );
        ...
        dispatchHandler = SmartDisPatchInitializable(poolAddress);
    }

    function addMinner(address _minner) external onlyOwner {
        require(_minner != address(0), "SMTC: minner is zero address");
    ...
    function mintSarmetCard(address _recipients, uint256 _numberOfTokens)
        public
        onlyMinner
    {
        require(
            totalSupply().add(_numberOfTokens) <= MAX_SUPPLY,
            "Reserve would exceed max supply of SMTC"
        );

        for (uint256 i = 0; i < _numberOfTokens; i++) {
            uint256 mintIndex = drawIndex();
            _safeMint(_recipients, mintIndex);
        }
    }

    function setCreatorTokenId(uint256 tokenId) external onlyOwner {
        require(_exists(tokenId), "does not exist");
        require(!creatorTokenId[tokenId], "token is creator");
        if (remainingCreator > 0) {
            remainingCreator--;
            address account = ownerOf(tokenId);
            _beforeTokenTransfer(account, address(0), tokenId);
            creatorTokenId[tokenId] = true;
            _beforeTokenTransfer(address(0), account, tokenId);
        }
    }
    ...
    modifier onlyMinner() {
        require(isMinner[msg.sender], "SMTC: caller is not the minner");
        _;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (address(dispatchHandler) != address(0)) {
            uint256 power = 1;
            if (creatorTokenId[tokenId]) {
                power = 10;
            }
            if (from != address(0)) {
                dispatchHandler.withdraw(from, power);
            }
            if (to != address(0)) {
                dispatchHandler.stake(to, power);
            }
        }
    }
}
...
contract SmartDisPatchInitializable is SafeOwnable, Initializable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    mapping(address => bool) private _isExcludedHolderBlack;
    ...
    mapping(address => uint256) public excludeBalances;
        ...
        initializer
    {
        ...
        setPendingOwner(admin_);
    }

    function isExcludedHolderBlack(address account) public view returns (bool) {
        return _isExcludedHolderBlack[account];
    }
    ...
    function excludedHolderBlack(address account)
        public
        updateDispatch(account)
        onlyOwner
    {
        require(!_isExcludedHolderBlack[account], "is holder black");
        _isExcludedHolderBlack[account] = true;

        _totalSupply = _totalSupply.sub(_balances[account]);
        excludeBalances[account] = _balances[account];
        _balances[account] = 0;
    }

    function includeHolderBlack(address account)
        public
        updateDispatch(account)
        onlyOwner
    {
        require(_isExcludedHolderBlack[account], "not is holder black");
        _isExcludedHolderBlack[account] = false;

        _totalSupply = _totalSupply.add(excludeBalances[account]);
        _balances[account] = excludeBalances[account];
        excludeBalances[account] = 0;
    }
    ...
    {
        require(msg.sender == SMART_DISPATCH_FACTORY, "Not factory");
        if (_isExcludedHolderBlack[account]) {
            excludeBalances[account] = excludeBalances[account].add(amount);
            return;
        }
        ...
        updateDispatch(account)
    {
        require(msg.sender == SMART_DISPATCH_FACTORY, "Not factory");
        if (_isExcludedHolderBlack[account]) {
            if (amount > excludeBalances[account]) {
                amount = excludeBalances[account];
            }
            excludeBalances[account] = excludeBalances[account].sub(amount);
            return;
        }
    ...
    function claim(address token) external updateDispatch(msg.sender) {
        ...
        claimedTotal[token] = claimedTotal[token].add(reward);
