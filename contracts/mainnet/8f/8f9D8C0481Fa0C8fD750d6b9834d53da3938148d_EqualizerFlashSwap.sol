...
contract EqualizerFlashSwap is IERC3156FlashBorrower {
    ...
    address[] public TokensWithBalance;
    uint256 MAX_INT =
        0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
    address EQZ_FL_Provider = 0xEe7e961f77066c5E995615ae7e7E8e4366d9eC5A;
    event EventMessage(string msg, uint256 value, address Address);

    function testFunction() public returns (address) {
        emit EventMessage("testFunction", 1234, WBNB);
        console.log("EQZ.sol testFunction");
        return WBNB;
    }
    ...
    function startArbitrage(
        ...
        uint256 maxFlashLoan = IERC3156FlashLender(EQZ_FL_Provider)
            .maxFlashLoan(A);
        require(maxFlashLoan > 0, "maxFlashLoan is zero");
        console.log("maxFlashLoan ", maxFlashLoan);

        uint256 flashFee = IERC3156FlashLender(EQZ_FL_Provider).flashFee(
            A,
            loanAmount
        );
        console.log("EQZ.sol flashFee=", flashFee);
        bool flashLoan_success = IERC3156FlashLender(EQZ_FL_Provider).flashLoan(
    ...
    function onFlashLoan(
        ...
        // _sender = EQZ.sol address
        //msg.sender = Pair.address
        // Decode data for calculating the repayment

        console.log("EQZ.sol msg.sender", msg.sender);
        (uint256 loanAmount, address[7] memory adressesArray) = abi.decode(
            data,
            (uint256, address[7])
        );
        ...
        console.log("EQZ.sol onFlashLoan  flashFee=", flashFee);

        uint256 amountToRepay = loanAmount + flashFee;
        console.log("EQZ.sol flashFee=", flashFee); //
        console.log("EQZ.sol _LoanAmount= ", loanAmount);
        console.log("EQZ.sol fee= ", flashFee);
        console.log(
            "EQZ.sol amountToRepay= _LoanAmount + fee = ",
            amountToRepay
        );

        // DO ARBITRAGE
        // Place Trades
        console.log("EQZ.sol DO ARBITRAGE ");
        ...
        console.log("EQZ.sol A to B  = ", B_amount);
        ...
        console.log("EQZ.sol A amountToRepay  = ", amountToRepay);
        A_amount = getBalanceOfToken(adressesArray[0]);
        console.log("EQZ.sol A balance  = ", A_amount);
        console.log("BREAKPOINT DEX_CALL 1");
        console.log("EQZ.sol PAY FLASH LOAN BACK");

        // PAY FLASH LOAN
        // IERC20(adressesArray[0]).safeApprove(msg.sender, MAX_INT);

        //PAY IN WBNB

        console.log("EQZ.sol onFlashLoan END");
    ...
    function Calculate_Arbitrage(
        ...
        if (ratio > best_ratio && (K == BUSD || K == WBNB)) {
            best_ratio = ratio;
            best_order = [K, J, M_FACTORY, M_ROUTER, N_FACTORY, N_ROUTER];
        }

        //  J K - N M
        ratio = try_arbitrage(J, K, amountInBNB, N_ROUTER, M_ROUTER);
        if (ratio > best_ratio && (J == BUSD || J == WBNB)) {
            best_ratio = ratio;
            best_order = [J, K, N_FACTORY, N_ROUTER, M_FACTORY, M_ROUTER];
        }
