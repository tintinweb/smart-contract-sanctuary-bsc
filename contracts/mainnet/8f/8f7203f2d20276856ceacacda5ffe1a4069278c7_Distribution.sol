...
contract Distribution is Ownable {
    ...
    DistributionInfo[] public distributionTokens;  // list of distribution tokens
    ...
    // Add a new lp to the pool. Can only be called by the owner.
    function addDistributionManual(
        IERC20 _token,
        uint256 _distributionAmount,
        address[] memory _accounts, 
        uint256[] memory _accountHoldings
    ) external onlyOwner {
        ...
        // Confirm there are holders
        require(_accounts.length > 0 , "Accounts arrays must be > 0");
        require(_accountHoldings.length > 0 , "Account Holdings arrays must be > 0");
        require(_accounts.length == _accountHoldings.length , "Arrays must be the same length");
        ...
        for (uint i = 0; i < _accounts.length; i++) {
            ...
            // if not blacklisted and balance greater than 0 -- add to total / total accounts
            if(!isBlacklisted(holder)){
                uint256 addressBalance = _accountHoldings[i];
    ...
    // Withdraw current distribution tokens
    ...
    // method for user to claim all pending distributions
    function claimAll() public {

        // check account can claim
        require(usersDistributionIds[msg.sender].length > 0, "claimAll: you are not part any distribution lists");

        // Deter claimable amount 
        for (uint i = 0; i < usersDistributionIds[msg.sender].length; i++) {

            // load accounts distribution information
            uint256 distributionId = usersDistributionIds[msg.sender][i];
            DistributionInfo storage distribution = distributionTokens[distributionId];
            uint256 usersInfoIndex = getUserDistributionStatusId(distributionId, msg.sender);      
            UserDistributionInfo storage userDistribution = userDistributionStatus[msg.sender][usersInfoIndex];
            ...
            if(!userDistribution.claimed){
                ...
                // emit event       
                emit Withdraw(msg.sender, distributionId, _amount);
            }
        }
    }

    // method for owner to send remaining unclaimed tokens
    function sendRemainingDistribution(uint256 _distributionId) external onlyOwner {
    
        // check there are distributions
        require(totalDistributionTokens > 0, "sendRemainingDistribution: there currently are no distributions");

        // load accounts distribution information
        DistributionInfo storage distribution = distributionTokens[_distributionId];

        // Deter claimable amount 
        for (uint i = 0; i < distribution.distributionAddresses.length; i++) {

            // load accounts distribution information
            address account = distribution.distributionAddresses[i];
            uint256 usersInfoIndex = getUserDistributionStatusId(_distributionId, account);      
            UserDistributionInfo storage userDistribution = userDistributionStatus[account][usersInfoIndex];
            ...
            if(!userDistribution.claimed){
                ...
                // formaula e.g (2000 * 5) / 100 = 100 tokens
                uint256 _amount = (distribution.totalDistributionTokens*userDistribution.rewardPercentage)/100; 
                safeTokenTransfer(distribution.token, account, _amount);
            
                // emit event      
                emit Withdraw(account, _distributionId, _amount);
            }
        }    
    }
