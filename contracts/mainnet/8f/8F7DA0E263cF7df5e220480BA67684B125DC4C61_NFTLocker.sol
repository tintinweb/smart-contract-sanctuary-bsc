...
import "./lib/Token/IERC721Receiver.sol";
import "./lib/AccessControlEnumerable.sol";
import "./DragonaireNFTManager.sol";

// For NFT locking purpose, i.e. lock a NFT when player wants to use it in game.
contract NFTLocker is ERC165, AccessControlEnumerable, IERC721Receiver {

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant GAME_MASTER_ROLE = keccak256("GAME_MASTER_ROLE");

    IERC721 public immutable nftContract;
    DragonaireNFTManager public manager;

    uint256 public unlockingDelay = 1 days;
    mapping(uint256 => Locking) public lockingOf;

    constructor(
        IERC721 _nftContract,
        DragonaireNFTManager _manager
    ) {
        ...
        nftContract.setApprovalForAll(address(manager), true);
    }

    event Locked(address indexed owner, uint256 indexed tokenId);
    event RequestedUnlock(address indexed owner, uint256 indexed tokenId);
    ...
    function onERC721Received(
        ...
        if (operator != address(this)) {
            require(msg.sender == address(nftContract), "NFTLocker: unacceptable NFT");
            _lock(from, tokenId);
        }
    ...
    function setManager(DragonaireNFTManager value) external onlyRole(MANAGER_ROLE) {
        manager = value;
    }

    function _getValidLocking(uint256 tokenId) internal view returns (Locking storage locking) {
        locking = lockingOf[tokenId];
        require(locking.state != LockingState.INVALID, "NFTLocker: non-managed NFT");
    }

    // Equivalent to transfer a NFT to this contract directly
    ...
    function _lock(address from, uint256 tokenId) internal {
        Locking storage locking = lockingOf[tokenId];
        locking.state = LockingState.LOCKED;
        locking.originalOwner = from;

        emit Locked(from, tokenId);
    }

    function requestUnlock(uint256 tokenId) public {
        Locking storage locking = _getValidLocking(tokenId);
        require(locking.originalOwner == msg.sender, "NFTLocker: not original owner");
        require(locking.state == LockingState.LOCKED, "NFTLocker: not locked");

        locking.state = LockingState.WAIT_FOR_UNLOCK;
        locking.unlockTime = block.timestamp + unlockingDelay;

        emit RequestedUnlock(locking.originalOwner, tokenId);
    }

    function requestUnlockBatch(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            requestUnlock(tokenIds[i]);
        }
    }

    function unlock(uint256 tokenId) public {
        Locking memory locking = _getValidLocking(tokenId);
        require(locking.originalOwner == msg.sender, "NFTLocker: not original owner");
        require(locking.state == LockingState.WAIT_FOR_UNLOCK, "NFTLocker: no unlocking request");
        require(locking.unlockTime <= block.timestamp, "NFTLocker: not yet unlock time");

        _unlock(tokenId);

        nftContract.transferFrom(address(this), locking.originalOwner, tokenId);
    }
    ...
    function _unlock(uint256 tokenId) internal {
        ...
        emit Unlocked(locking.originalOwner, tokenId);

        delete lockingOf[tokenId];
    }

    function mintAndLock(address to, uint256 artifacts, bytes32 dna) external onlyRole(GAME_MASTER_ROLE) {
        uint256 tokenId = manager.mint(address(this), artifacts, dna);

        _lock(to, tokenId);
    }

    function unlockAndBurn(uint256 tokenId) external onlyRole(GAME_MASTER_ROLE) {
        Locking storage locking = _getValidLocking(tokenId);
        require(locking.state == LockingState.LOCKED, "NFTLocker: not locked");
