...
contract Fullerene is BEP20, BEP20Burnable, Pausable, Ownable {
    mapping(address => bool) public lockedOf;
    mapping(address => uint256) public lockedOfamount;

    uint256 public _totalSupply;

    constructor() BEP20('Fullerene', 'C60') {
    ...
    function isUnLockedOf(address account, uint256 amount)
        public
        view
        returns (bool)
    {
    ...
    function lockAddress(address account, uint256 amount)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function multiApprove(address[] memory spenders, uint256[] memory amounts)
        public
        returns (bool)
    {
        require(
            spenders.length == amounts.length,
            'Invalid spenders and amounts length'
        );
        for (uint256 i = 0; i < spenders.length; i++) {
            super._approve(_msgSender(), spenders[i], amounts[i]);
        }
        return true;
    }
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            'C60::delegateBySig: invalid signature'
        );
        require(
            nonce == nonces[signatory]++,
            'C60::delegateBySig: invalid nonce'
        );
        require(block.timestamp <= expiry, 'C60::delegateBySig: signature expired');
        ...
        returns (uint256)
    {
        require(
            ...
            'C60::getPriorVotes: not yet determined'
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            'C60::_writeCheckpoint: block number exceeds 32 bits'
        );
