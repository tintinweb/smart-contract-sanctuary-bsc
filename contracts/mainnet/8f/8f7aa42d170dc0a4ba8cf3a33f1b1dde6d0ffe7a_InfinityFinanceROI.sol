...
contract InfinityFinanceROI {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [60, 40, 20]; 
    ...
    constructor() {
        ...
        uint256 tarifPercent = 12000;
        for (uint256 tarifDuration = 80; tarifDuration <= 200; tarifDuration++) {
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external {
        ...
        require(amount >= 50 ether, "Minimum deposit amount is 50 BUSD");
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint to = block.timestamp > time_end ? time_end : uint40(block.timestamp);
