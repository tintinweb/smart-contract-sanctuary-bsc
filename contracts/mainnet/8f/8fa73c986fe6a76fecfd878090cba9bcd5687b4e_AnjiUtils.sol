...
interface AnjiRouter {
    function getAmountsOut(uint, address[] memory, uint256) external view returns (uint[] memory);
    function getAmountsIn(uint, address[] memory, uint256) external view returns (uint[] memory);
}
...
contract AnjiUtils {
    ...
    function getBestMultiDexAmounts (address routerAddr, uint amountIn, address[][] memory possiblePaths, uint256[] memory dexIds, bool out) public view virtual returns (DexAmounts[] memory) {        
        DexAmounts[] memory dexAmounts = new DexAmounts[](dexIds.length);

        for (uint256 index = 0; index < dexIds.length; index++) {
            dexAmounts[index] = getDexBestAmounts(routerAddr, amountIn, possiblePaths, dexIds[index], out);
        }

        return dexAmounts;
    }

    function getDexBestAmounts (address routerAddr, uint amount, address[][] memory paths, uint256 dexId, bool out) public view virtual returns (DexAmounts memory) {
        AnjiRouter router = AnjiRouter(routerAddr);

        uint[] memory largestAmounts = new uint[](3);
        address[] memory largestDexPath = new address[](3);

        for (uint256 index = 0; index < paths.length; index++) {
            uint[] memory amounts = new uint[](paths[index].length);

            if (out) {
                try router.getAmountsOut(amount, paths[index], dexId) returns (uint[] memory _amounts) {
                    amounts = _amounts;
                } catch (bytes memory /*lowLevelData*/) {
                }
            } else {
                try router.getAmountsIn(amount, paths[index], dexId) returns (uint[] memory _amounts) {
                    amounts = _amounts;
                } catch (bytes memory /*lowLevelData*/) {
                }
            }

            if (amounts[amounts.length - 1] > largestAmounts[largestAmounts.length - 1]) {
                largestDexPath = paths[index];
                largestAmounts = amounts;
            }
        }

        return DexAmounts({
            id: dexId,
            amounts: largestAmounts,
            paths: largestDexPath
        });
    }
}