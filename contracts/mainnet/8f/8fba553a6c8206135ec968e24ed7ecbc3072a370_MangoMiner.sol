...
contract MangoMiner is Context, Ownable , ReentrancyGuard {
    ...
    function withdrawal() public noReentrant {
        require(init, "Not Started Yet");    
        require(weekly[msg.sender].deadline <= block.timestamp, "You can not withdraw");
        require(totalRewards[msg.sender].amount <= SafeMath.mul(investments[msg.sender].invested,5), "You can not withdraw anymore, you have already collected five times your deposit"); // hh new
    ...
    function claimDailyRewards() public noReentrant{
        require(init, "Not Started Yet");
        require(claimTime[msg.sender].deadline <= block.timestamp, "You can not claim");
