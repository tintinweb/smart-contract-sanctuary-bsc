...
interface ITreasury {
    function treasuryFund() external view returns (address);
...
contract RebateTreasury is Ownable {
    ...
    modifier onlyRebateOperator() {
        require(rebateOperator == msg.sender, "RebateTreasury: caller is not the operator");
        _;
    }
    ...
    constructor(address tomb, address tombOracle, address treasury) {
        ...
        rebateOperator = msg.sender;
    }

    function treasuryFund() external view returns (address) {
        return Treasury.treasuryFund();
    }

    function setRebateOperator(address operator) external onlyOwner {
        rebateOperator = operator;
    }
    ...
    function redeemAssetsForBuyback(address[] calldata tokens) external onlyRebateOperator {
        ...
        for (uint256 t = 0; t < tokens.length; t ++) {
            ...
            Token.transfer(Treasury.treasuryFund(), Token.balanceOf(address(this)));
        }
    }
