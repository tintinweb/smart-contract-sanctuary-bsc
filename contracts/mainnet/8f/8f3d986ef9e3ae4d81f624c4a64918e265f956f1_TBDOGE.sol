...
contract TBDOGE is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "TBDOGE";
    string private _symbol = "TBDOGE";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x0E7A63F88c162719862b53F2a2D5EFe502c56bA7);
    address payable public teamWalletAddress = payable(0x0E7A63F88c162719862b53F2a2D5EFe502c56bA7);
    ...
    bool public _isCheckTxLimit = true;
    uint256 public _maxBUYTxAmount =   1000000000000000 * 10**_decimals;  
    uint256 public _maxSELLTxAmount =  1000000000000000 * 10**_decimals;  
    ...
    function tradingStatus(bool status, uint256 dead) public onlyOwner {
        tradingOpen = status;
    ...
    function changeCheckLimitEnable(bool status) external onlyOwner(){
        _isCheckTxLimit = status;
    }

    function setMaxTxAmount_BUY(uint256 maxTxAmount) external onlyOwner() {
        _maxBUYTxAmount = maxTxAmount;
    }

    function setMaxTxAmount_SELL(uint256 maxTxAmount) external onlyOwner() {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if(!isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && _isCheckTxLimit) {
                if(isMarketPair[sender]){ require(amount <= _maxBUYTxAmount); }
                else if(isMarketPair[recipient]){ require(amount <= _maxSELLTxAmount); }
