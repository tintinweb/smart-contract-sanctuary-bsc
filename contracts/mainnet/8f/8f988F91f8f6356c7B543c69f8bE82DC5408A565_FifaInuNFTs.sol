...
contract FifaInuNFTs is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[1022] public remainingIds;
  ...
  // Constructor
  constructor() ERC721("Fifa Inu NFTs", "FIFANFT") {
      // Save URI
      _baseTokenURI = "ipfs://bafybeibxy5mj7j5wvz2vyfzoggzm5uk4rlgph4ffjfdzohftv35utf2cha/";
      // Save dev wallet
      _wallet = 0x72Fdcf560d29929F21D268001f4bDf8882794B62;
      _regularMint();
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "FifaInuNFTs: minting has not started yet");
    require(_mintAmount > 0, "FifaInuNFTs: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "FifaInuNFTs: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "FifaInuNFTs: the collection is sold out");
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "FifaInuNFTs: total cost isn't match"
            );
        }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "FifaInuNFTs: zero address");
    require(_amount > 0, "FifaInuNFTs: you have to mint at least one");
    require(lastSupply != 0,  "FifaInuNFTs: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"FifaInuNFTs: URI query for nonexistent token");
