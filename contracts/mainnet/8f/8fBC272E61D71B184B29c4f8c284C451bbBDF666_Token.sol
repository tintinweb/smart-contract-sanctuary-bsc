...
contract Token is IERC20, Ownable, Pausable {
  ...
  uint256 private _gonsCollectedFeeThreshold = DEFAULT_GONSWAP_THRESHOLD;
  ...
  uint256 public athPriceDeltaPermille = 10;
...
contract $Token is Token {
    constructor(address _dexRouter, address _busd, address _autoLiquidityReceiver, address _treasury) Token(_dexRouter, _busd, _autoLiquidityReceiver, _treasury) {}

    function $_saleHistories(address arg0) external view returns (Token.SaleHistory memory) {
        return _saleHistories[arg0];
    }

    function $_getDailySellLimitAmountInternalInGons(address _address) external view returns (uint256) {
        return super._getDailySellLimitAmountInternalInGons(_address);
    }

    function $_swapBack() external {
        return super._swapBack();
    }
