...
// File: contracts/LegacyMatrix.sol


// Legacy Stake Pool
// https://matrix.legacystake.com/
...
contract LegacyStakeMatrix is Ownable {
    ...
    /* SETTINGS */
    uint256 public packPrice = 40;
    uint256 public _buyFee = 3;
    uint256 public daySeconds = 86400;
    uint256[10] public rewardByLevel = [1, 1, 1, 1, 1, 1, 2, 2, 4, 7];
    ...
    event BuyPack(address user, uint packs, uint inAmount);
    event ClaimFromPacks(address user, uint amount);
    ...
    function buyPack(uint packs) public returns (bool) {
        ...
        if ((_packs[_buyer].length == 0) && (userSponsor.paidRefCount < 2)) {            
            userSponsor.paidRefCount++;
        }
        BUSD.safeTransfer(sponsor, packsPrice*125/1000); // 12.5% or 5 BUSD

        if ((userSponsor.positioned == false) && (userSponsor.paidRefCount == 2)) {
        ...
        if (_automatedBuy == true) {
            BUSD.safeApprove(address(uniswapV2Router), 0); // 32.5% or 13 BUSD
            BUSD.safeApprove(address(uniswapV2Router), packsPrice * 325/1000); // 32.5% or 13 BUSD
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                packsPrice * 325/1000,  // 13 * BUSD
                0,
                _path,
                address(this),
                block.timestamp
        ...
        BUSD.safeTransfer(leaderBoard, packsPrice*250/10000); // 2.5% or 1 BUSD
        ...
        if (_buyFee > 0) {
            newPack.amount = (amounts[_path.length - 1] * packs * 2) * (100 - _buyFee) / 100;
            lockedLST += (amounts[_path.length - 1] * packs * 2) * (100 - _buyFee) / 100;
        } else {
            newPack.amount = amounts[_path.length - 1] * packs * 2;
            lockedLST += amounts[_path.length - 1] * packs * 2;
        }

        newPack.multiplier = packs;
        emit BuyPack(_buyer, packs, amounts[_path.length - 1] * packs * 2);

        if (user.positioned == true) {
            distributeMatrix(_buyer);
        } else {
            distributeMatrix(owner());
        }
    ...
    function distributeMatrix(address _user) private {        
        ...
        for (uint i=0; i<10; i++) {
            ...
            if (payTo == owner()) {
                collectPayment += rewardByLevel[i] * 10 ** 18;
            } else {
                BUSD.safeTransfer(payTo, rewardByLevel[i] * 10 ** 18);
            }
        }     
        if (collectPayment > 0) {
    ...
    function claimPacks() public returns (bool) {
        ...
        for(uint i=0; i<_packs[_buyer].length; i++) {
            ...
            uint availablePercent = (currentTime - packItem.createdAt) / daySeconds;
            if (availablePercent >= 100) availablePercent = 100;
            uint availableToClaim = (packItem.amount * availablePercent / 100) - packItem.claimed;
