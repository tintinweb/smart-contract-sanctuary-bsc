...
contract SiriusTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSRS }
    ...
    address public sSRS;
    uint public sSRSQueue; // Delays change to sSRS address
    ...
    constructor (
        ...
        isReserveToken[ _sSRS ] = true;
        reserveTokens.push( _sSRS );

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SRS
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SRS needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SRS ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SRS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISRSERC20( SRS ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSRS ).balanceOf( msg.sender ); // Can only borrow against sSRS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SRS
        @param _amount uint
     */
    function repayDebtWithSRS( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISRSERC20( SRS ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SRS, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SRS ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SRS ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SRS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SRS decimals
            value_ = _amount.mul( 10 ** IERC20( SRS ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSRS ) { // 9
            sSRSQueue = block.number.add( blocksNeededForQueue );
        } else return false;
