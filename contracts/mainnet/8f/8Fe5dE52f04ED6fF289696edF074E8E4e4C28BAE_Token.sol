...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 1_0000_0000 ether;
    ...
    bool public poolState = false;
    bool public idoStart = false;

    mapping (address => bool) public _joined;
    ...
    uint256 public txMarketFee = 200;
    uint256 public txLPFee = 300;
    ...
    address public rewardAddr = address(0x88888888);
    ...
    constructor() ERC20("TAISHAN", "TAISHAN") {
        ...
        _marketWallet = 0x2F9DEAD2D1b5Cdc607dfe19D9d4F3e072FaC2077;
        _marketWallet2 = 0xEF97AFfE60b1dC0Ae7Aa080d1eE27E8A1A3001c0;
        _idoHolder = msg.sender;
        ...
        excludeFromFees(address(_idoHolder), true);

        _mint(_idoHolder, _total);
    ...
    function setIdoHolder(address wallet) external onlyOwner {
        _idoHolder = wallet;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if(to != uniswapV2Pair)
            {
                ...
                _userHoldPrice[to]= totalvalue.div(oldbalance.add(tokens));
            }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) { // buy
                ...
                uint256 cutcount = getCutCount(from,tokens,currentprice);
                if(cutcount > 0)
                {
                    // _balances[address(_tokenDistributor)] =  _balances[address(_tokenDistributor)].add(cutcount);
                    super._transfer(from, rewardAddr, cutcount);
                    tokens = tokens.sub(cutcount);
                }
            }

            if(tokens > 0 && to != uniswapV2Pair) {
                uint256 oldbalance = balanceOf(to);
                uint256 totalvalue = _userHoldPrice[to].mul(oldbalance); 
                totalvalue += tokens.mul(currentprice);
                _userHoldPrice[to]= totalvalue.div(oldbalance.add(tokens));
            }
    ...
    function _swap() private lockSwap {
        ...
        uint256 rewardTokens = balanceOf(rewardAddr);
        if (rewardTokens > 0) {
            tokens += rewardTokens;
            super._transfer(rewardAddr, address(this), rewardTokens);
        }
        ...
        IERC20(tokenB).transfer(_marketWallet2, bBalance.mul(lpTokens).div(swapTokens));
    ...
    function setWhiteUserPrice(address[] memory accountArray, uint256 newValue) public onlyManager {
    ...
    function changeIdoState(bool enabled) public onlyOwner {
        idoStart = enabled;
    }

    function deposit(uint256 value) internal {
        require(idoStart, "not start");
        require(value == 0.1 ether, "invalid bnb");
        require(!_joined[msg.sender], "have joined");
        _joined[msg.sender] = true;
        super._transfer(_idoHolder, msg.sender, 24000 ether);

        _userHoldPrice[msg.sender] = uint256(30 ether).mul(1e18).div(24000 ether);
    }
    ...
    function getCurrentPrice() public view returns (uint256)
    {
        if(uniswapV2Pair==address(0)) {
            return uint256(30 ether).mul(1e18).div(24000 ether);
        }

        (uint112 a,uint112 b,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        if (a == 0 || b == 0) {
            return uint256(30 ether).mul(1e18).div(24000 ether);
        }
