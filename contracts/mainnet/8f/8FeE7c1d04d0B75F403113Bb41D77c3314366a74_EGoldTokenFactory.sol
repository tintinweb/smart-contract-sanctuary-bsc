...
contract EGoldTokenFactory is AccessControl {
    using SafeCast for *;
    ...
    event createTokenEvent( address indexed Token);
    ...
    function createToken(
        ...
        ctr = ctr.add(1);
        EGoldToken MWH = new EGoldToken( name_ , symbol_ , msg.sender );
        TokenList[ctr] = address(MWH);
        emit createTokenEvent(address(MWH));
    }
    //Factory Fx

    function getToken(uint256 _ctr) external view returns (address) {
        return TokenList[_ctr];
    }
...
contract EGoldToken is AccessControl, ERC20Snapshot, Pausable, ReentrancyGuard {
    ...
    constructor(string memory name_, string memory symbol_ , address _DFA) AccessControl() ERC20Snapshot() ERC20(name_, symbol_) {
        ...
        _mint(_DFA, 21000000000000000000000000);
    }
    ...
    function setCoolDown(uint256 _limit) external virtual onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        limit = _limit;
        return true;
    }
    ...
    function setVolumeLimit(uint256 _amt) external virtual onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        VolumeLimit = _amt;
        return true;
    }
    ...
    function fetchCooldown(address _to) external view virtual returns (uint256) {
        return cooldown[_to];
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(amount <= VolumeLimit, "ERC20Blockable: Volume Limit Reached");
        if ( to.isContract() == true && isWhitelist == true ) {
            require( whitelist[to] == true , "ERC20Blockable: Not whitelisted contract for use");
        }else{
            require( block.number > cooldown[from] + limit , "ERC20Blockable: Cool down period active");
        }
        cooldown[from] = block.number;
    }
}
