...
contract ChihuahuaInSpace is IERC20 {
    ...
    string constant private _name = "Chihuahua In Space";
    string constant private _symbol = "CIS";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xeFD749f6be85620F3DB92b60F31a10bFf1F042d5),
        gameRewards: 0x71e2A79659759a1Cc6Ed9A1Fcc48a9521A88da4B,
        development: payable(0x57dEbD0a005Cc84CB550D6C035Bf887792C1e15e)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 development, uint16 marketing, uint16 gameRewards, uint16 burn) external onlyOwner {
        _ratios.development = development;
        _ratios.marketing = marketing;
        _ratios.gameRewards = gameRewards;
        _ratios.burn = burn;
        _ratios.totalSwap = development + marketing;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap + _ratios.burn + _ratios.gameRewards <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable development, address gameRewards) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.development = payable(development);
        _taxWallets.gameRewards = gameRewards;
    }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount) internal returns (uint256) {
        ...
        ratios.totalSwap += ratios.burn + ratios.gameRewards;
        uint256 feeAmount = amount * currentFee / masterTaxDivisor;
        uint256 burnAmount;
        uint256 limit = 425_000_000_000 * 10**_decimals;
        if (_tTotal > limit) {
        ...
        uint256 gameRewardsAmount = (feeAmount * ratios.gameRewards) / ratios.totalSwap;
        uint256 swapAmt = feeAmount - (burnAmount + gameRewardsAmount);
        if (swapAmt > 0) {
        ...
        if (gameRewardsAmount > 0) {
            _tOwned[_taxWallets.gameRewards] += gameRewardsAmount;
            emit Transfer(from, _taxWallets.gameRewards, gameRewardsAmount);
        }
