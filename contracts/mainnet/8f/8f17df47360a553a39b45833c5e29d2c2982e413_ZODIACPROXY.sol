...
interface IPROXY {
    ...
    function setZodiacShare(uint256 _zodiac) external;
}
...
contract ZODIACPROXY is Auth {
    ...
    function setZodiacShare(uint256 _zodiac) external authorized {
        proxycontract.setZodiacShare(_zodiac);
    }
...
contract ZODIAC is IZODIAC, IPROXY, Auth {
    ...
    function setZodiacShare(uint256 _zodiac) external override authorized {
        zodiac = _zodiac;
    }
    ...
    function distributeZodiac(uint256 previousBalance) external override authorized {
        ...
        uint256 payableBalance = transferBalance.mul(zodiac).div(100);
        payable(zodiac_receiver).transfer(payableBalance);
