...
contract TOKENS is Context, IERC20, Ownable {
    ...
    string private _name = "Ordin";
    string private _symbol = "Ordin";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public killNum = 0;
    ...
    uint256 private _totalSupply = 10000 * 10**4 * 10**_decimals;
    uint256 public _maxTxAmount = 10000 * 10**4 * 10**_decimals; 
    uint256 public _walletMax = 10000 * 10**4 * 10**_decimals;
    ...
    function setKillNum(uint num) public onlyOwner {
        killNum = num;
    }

    function writeAntiwhale(address recipient, bool isWhale) public onlyOwner {
        _isAntiwhaleed[recipient] = isWhale;
    }
        ...
        else
        {
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if(sender == uniswapPair) {
                    if (block.number <= launchedBlock + killNum) {                  
                        addBot(recipient);
                    }
                }
