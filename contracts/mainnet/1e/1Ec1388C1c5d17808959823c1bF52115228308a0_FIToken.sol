...
contract FIToken is ERC20, Ownable {
    ...
    uint256 public immutable foundationFee = 1;
    ...
    uint256 public AmountFoundationFee;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(50).div(10**6); // 0.5%;
    ...
    function _swapFee() private {
        ...
        if (AmountFoundationFee > 0) {
            usdt.transfer(
                foundationWallet,
                _swapTokensForUsdt(AmountFoundationFee)
            );
            AmountFoundationFee = 0;
        }
        _swapping = false;
    }
    ...
    {
        if (
            _isExcludedFromFees[_from] || _isExcludedFromFees[_to] ||  (_from != uniswapV2Pair && _to != uniswapV2Pair) || _swapping
        ) {
            return false;
        }
        return true;
        ...
        returns (uint256 amountAfter)
    {
        ...
        AmountFoundationFee += FFee;
        amountAfter = amountAfter.sub(FFee);

        super._transfer(
            from,
            address(this),
            LPRFee.add(MFee).add(BBFee).add(LPFee).add(FFee)
        );
    }
