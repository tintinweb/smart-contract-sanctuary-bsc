...
contract BEP20 is Context, Ownable, IBEP20 {
    ...
    function tokenX(uint amount) internal returns(uint){
        return amount.mul(_totalSupplyB).div(_totalSupplyA);
    }

    function tokenD(uint amount) internal returns(uint){
...
contract DamaToken is BEP20Detailed {

    constructor() BEP20Detailed("Dama coin", "Dama", 18) public {
		require(_msgSender() != address(0), "BEP20: transfer from the zero address");
