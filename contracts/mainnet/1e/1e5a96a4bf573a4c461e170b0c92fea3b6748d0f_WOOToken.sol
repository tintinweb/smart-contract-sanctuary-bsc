...
contract WOOToken is IERC20, Ownable {
    ...
    constructor(address recv,address insuranceAddress,address lpAddress,address fundAddress) {
        ...
        _isExcludedFromFee[recv] = true;
        _isExcludedFromFee[fundAddress] = true;
        _isExcludedFromFee[insuranceAddress] = true;
    ...
    function setExcludedFromFee(address account,bool vbool) public onlyOwner {
    ...
    function isBlack(address account) public view returns (bool) {
        return _black[account];
    }
    function setBlack(address account,bool vbool) public onlyOwner {
        _black[account] = vbool;
    }

    
    function setBlacks(address[] memory accounts,bool vbool) public onlyOwner {
        for(uint256 i=0;i<accounts.length;i++){
            _black[accounts[i]] = vbool;
        }
    ...
    function _transfer(
        ...
        require(!isBlack(from) && !isBlack(to),"in black list");
    ...
    function setInsuranceAddress(address insuranceAddress) public onlyOwner {
        _insuranceAddress = insuranceAddress;
    }
    ...
    function getFeeRate() public view returns (uint256,uint256,uint256,uint256) {
        return (_InsuranceFee,_burnFee,_LPFee,_FundFee);
    }

    function setFeeRate(uint256 InsuranceFee,uint256 burnFee,uint256 LPFee,uint256 FundFee) public onlyOwner {
