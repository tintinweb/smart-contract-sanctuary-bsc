# @version 0.3.4
# (c) Curve.Fi, 2021 - used with permission
...
interface CurveToken:
    ...
    def mintRelative(_to: address, frac: uint256) -> uint256: nonpayable
...
interface FeeConverter:
    def convertFees(_swap: address) -> bool: nonpayable
...
WETH20: constant(address) = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
    ...
    if xcp_profit > xcp_profit_a:
        ...
        if fees > 0:
            ...
            if receiver != ZERO_ADDRESS:
                ...
                claimed: uint256 = CurveToken(self.token).mintRelative(receiver, frac)
                xcp_profit -= fees*2
...
@internal
def _exchange(sender: address, mvalue: uint256, _i: int128, _j: int128, dx: uint256, min_dy: uint256,
              use_eth: bool, receiver: address, callbacker: address, callback_sig: bytes32) -> uint256:
    i: uint256 = convert(_i, uint256)
    j: uint256 = convert(_j, uint256)
...
def exchange(i: int128, j: int128, dx: uint256, min_dy: uint256,
    ...
    """
    return self._exchange(msg.sender, msg.value, i, j, dx, min_dy, use_eth, receiver, ZERO_ADDRESS, empty(bytes32))
...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256,
    ...
    Exchange using ETH
    """
    return self._exchange(msg.sender, msg.value, i, j, dx, min_dy, True, receiver, ZERO_ADDRESS, empty(bytes32))


@payable
@external
@nonreentrant('lock')
def exchange_extended(i: int128, j: int128, dx: uint256, min_dy: uint256,
...
@external
@nonreentrant('lock')
def remove_liquidity_one_coin(token_amount: uint256, _i: int128, min_amount: uint256,
                              use_eth: bool = False, receiver: address = msg.sender) -> uint256:
    i: uint256 = convert(_i, uint256)
    A_gamma: uint256[2] = self._A_gamma()
...
def withdraw_admin_fees():
    self._claim_admin_fees()
    receiver: address = Factory(self.factory).fee_receiver()
    FeeConverter(receiver).convertFees(self)
...
@external
@view
def get_dy(_i: int128, _j: int128, dx: uint256) -> uint256:
    i: uint256 = convert(_i, uint256)
    j: uint256 = convert(_j, uint256)
...
@view
@external
def calc_withdraw_one_coin(token_amount: uint256, i: int128) -> uint256:
    return self._calc_withdraw_one_coin(self._A_gamma(), token_amount, convert(i, uint256), True, False)[0]
