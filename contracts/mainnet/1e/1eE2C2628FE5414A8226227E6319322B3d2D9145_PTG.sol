...
contract PTG is Context, IERC20, Ownable {
	...
	uint256 private _minSwapCoin = 1;
	...
	address public inviteBurnAddress = address(0xa141F272f5807d2aDEE17076A33d00AF284351b6); // burn address for invite address is address(0)
    
	address public marketAddress = address(0xa141F272f5807d2aDEE17076A33d00AF284351b6); // for market buy 20 per
	
	address public lpAddress = address(0xa141F272f5807d2aDEE17076A33d00AF284351b6); // for market buy 20 per
	...
	private lockTheSwap {
		if (amount > 0) {
            uint256 shareUNum = amount.mul(price).div(1000000);
			(uint256 shareFee, uint256 LpshareFee, uint256 poolFee, uint256 daoFee) = _getbuyFeeList (shareUNum);
			
			_inviteFeeU(sender, shareFee);
			_toLpPool(LpshareFee);
			_toPool(poolFee);
			_toDao(daoFee);
        }
	}
		...
		if (amount != 0){
			for (int256 i = 0; i < 13; i++) {
				...
				if (IERC20(swapToken).balanceOf(cur1) >= _minSwapCoin && cur1 != address(0)){
					if (curTAmount > 0){
						payable(cur1).transfer(curTAmount);
					}
				} else {
					if (curTAmount > 0){
						payable(inviteBurnAddress).transfer(curTAmount);
					}
				}
			}
			...
			for (int256 i = 0; i < 5; i++) {
				...
				if (IERC20(swapToken).balanceOf(cur2) >= _minSwapCoin && cur2 != address(0)){
					if (curTAmount > 0){
						payable(cur2).transfer(curTAmount);
					}
				} else {
					if (curTAmount > 0){
						payable(inviteBurnAddress).transfer(curTAmount);
	...
	function _toLpPool(uint amount) private{
        if (amount > 0){
			payable(lpAddress).transfer(amount);
		}
	}
	
	function _toPool(uint amount) private{
		if (amount > 0){
			payable(lpAddress).transfer(amount);
		}
	}
	
	function _toDao(uint amount) private{
        if (amount > 0){
			payable(lpAddress).transfer(amount);
		}
	}
    ...
    function _takeInviterFee(
		...
		if (recipient == uniswapV2Pair || recipient == uniswapV2Pair2) {
            ...
            uint price = uniswapV2Router.getAmountOut(1000000, _reserve0, _reserve1);
			// buy
			_getBuyFee(recipient, tAmount, price);
    ...
    function _transfer(
        ...
        // set invite
        bool shouldSetInviter = balanceOf(to) == 0 && inviter[to] == address(0) && !isContract(from) && !isContract(to);
