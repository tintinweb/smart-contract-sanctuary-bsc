...
contract HAMBABToken is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    function claim() external nonReentrant whenNotPaused returns (uint256) {
        ...
        if (_claimed[sender] == true) revert AlreadyClaimed();
        if (bab.balanceOf(sender) == 0) revert BabNotOwned();
        ...
        emit Claimed(sender, tokenId);

        return tokenId;
    }

    function canClaim(address _user) external view returns (bool) {
        return _claimed[_user] == false && bab.balanceOf(_user) > 0;
    }
    ...
    function tokenIdOf(address _user) external view returns (uint256) {
        return this.balanceOf(_user) > 0 ? this.tokenOfOwnerByIndex(_user, 0) : 0;
    }

    //---- admin functions

    function safeMint(address to) public onlyOwner {
        if (_claimed[to] == true) revert AlreadyClaimed();
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _claimed[to] = true;
        _safeMint(to, tokenId);
