...
contract MWCrafflev2 is ERC721, ERC721Enumerable, Ownable, ERC721Pausable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;

    uint256 public MAX_NFT = 199;
    uint256 public MAX_BY_MINT = 50;
	uint256 public PRICE = 4 * 10**17;
	
    string public baseTokenURI;
	
    event CreateLottery(uint256 indexed id);
	
    constructor(string memory baseURI) ERC721("MWC RAFFLE V2", "MWCRAFFLEV2") {
        setBaseURI(baseURI);
        pause(true);
    }
