...
// File: contracts/common/MintingFee.sol
...
contract MintingFee is AccessController {
    constructor() {
    ...
    function seeMintingFee(address token, uint256 amount) public onlyAdmin {
        feeToken = IERC20Burnable(token);
        feeAmount = amount;
        emit SetMintingFee(token, amount);
    }
}
// File: contracts/token/FusionEnergyNFT.sol
...
contract FusionEnergyNFT is AccessController, MintingFee {
    constructor() {
      _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }
    ...
    modifier validLength(uint256[] memory tokenIds) {
        require(tokenIds.length > 0 && tokenIds.length % threshold == 0, "FusionEnergyNFT: Invalid number of NFTs");
        _;
    }
    ...
    modifier sameSpecies(uint256[] memory tokenIds) {
        ...
        for (uint256 i = 1; i < tokenIds.length; i++) {
            ...
            require(rawRanking == ranking, "FusionEnergyNFT: NFTs do not have the same ranking");    
            require(rawLevel == level, "FusionEnergyNFT: NFTs do not have the same level");
        }
        _;
    }
    ...
    function setRawContract(address _rawContract) public onlyAdmin {
        rawContract = _rawContract;
    }

    function setThreshHold(uint8 _threshold) public onlyAdmin {
