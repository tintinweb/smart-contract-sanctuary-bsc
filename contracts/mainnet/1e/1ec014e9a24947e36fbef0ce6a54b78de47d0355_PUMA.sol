...
contract PUMA is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT =  5 * 10**9 * 10**9; //
    string private constant _name = "PUMA"; // 
    string private constant _symbol = "PUMA"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _marketingFee = 7;
    uint256 public _liquidityFee = 1;
    address private  _marketing_Wallet = 0x908039D9eD8a13621CC6124F84e856f4a9749aA3;
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_marketing_Wallet).transfer(ethToSend);
    }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_marketing_Wallet).sendValue (ethAmount - ethFromLiquidity);
    }
}