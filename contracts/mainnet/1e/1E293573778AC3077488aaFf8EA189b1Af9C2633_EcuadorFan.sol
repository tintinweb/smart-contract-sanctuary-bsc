...
Ecuador WorldCup Winner !
https://t.me/Ecuadorinu
...
/* Token contract */
contract EcuadorFan is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "Ecuador Inu - Fan Token";
    string constant _symbol = "$ECU";
  ...
  // Sell fees
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && !isTxLimitExempt[sender] && !isTxLimitExempt[recipient] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != gamble){
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBgamble = amountBNB.mul(gambleFee).div(totalBNBFee); 


        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool successMarketing, /* bytes memory data */) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool successgamble, /* bytes memory data */) = payable(gamble).call{value: amountBNBgamble, gas: 30000}(""); 
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successgamble, "gamble receiver rejected ETH transfer");
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _gambleFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        buybackFeeBuy = _buybackFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        gambleFeeBuy = _gambleFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy).add(gambleFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _gambleFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        buybackFeeSell = _buybackFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        gambleFeeSell = _gambleFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell).add(gambleFeeSell);
        feeDenominator = _feeDenominator;
    }
