...
contract Storage is events, Ownable {
    ...
    mapping(uint256 => mapping(address => bool)) private checkVoteInit;
    ...
    function _updateAndEmit(uint256 productID) internal {
        ...
        uint256 __allCurrentMembers = loopp(productID);

        if ((__allCurrentMembers + 1) > maxNumbersofCouncils) { 

            if (block.timestamp < raisedDispute[productID].votesEnded) revert VoteCommence();

            if (tips[productID].qualified == address(0)) {
                uint256 lent = tips[productID].allTippers.length;
                for (uint256 i; i < lent; ) {
                    address joinnedUser =  tips[productID].allTippers[i];
                    if (!_checkIfAccountHasVote(productID, joinnedUser) && checkVoteInit[productID][joinnedUser]) {
                        checkVoteInit[productID][joinnedUser] = false;
        ...
        uint256 _allCurrentMembers = loopp(productID);

        if (_allCurrentMembers < maxNumbersofCouncils) {
            tips[productID].allTippers.push(msg.sender);
            checkVoteInit[productID][msg.sender] = true;
        }

        if (loopp(productID) == maxNumbersofCouncils ) {
            raisedDispute[productID].votesEnded = block.timestamp + votingEllapseTime;
        }
        emit JoinDispute(msg.sender, productID, 0);
    }

    function loopp(uint256 productID) private view returns(uint256) {
        uint256 all;
        uint256 lent = tips[productID].allTippers.length;

        for(uint256 i; i < lent; ) {
            if(checkVoteInit[productID][tips[productID].allTippers[i]]) {
                all++;
            }
            unchecked {
                i++;
    ...
    function _checkIfAccountHasJoined(uint256 productID) internal view {
        if(checkVoteInit[productID][msg.sender]) revert not_Permitted();
    }

    function __checkIfAccountHasJoined(uint256 productID) internal view {
        if(!checkVoteInit[productID][msg.sender]) revert not_Permitted();
    }
    ...
    function _l(uint256 productID, uint256 amountShared, uint256 fee,address token, address who) internal {       
        ...
        for (uint256 j; j < mem; ) {
            ...
            emit rewards(token, consM , amountShared, block.timestamp);
            unchecked {
                j++;
            }                
        }
    ...
    function _updateTipForBuyerAndSeller(uint256 productID, address who) private {
        ...
        uint256 div2 = loopp(productID) / 2;
    ...
    function _buyerAndSellerConsensus(uint256 productID,address who) internal {
        ...
        uint256 len = loopp(productID);
        uint256 s_amount = allProducts[productID].amount;
        address s_token = allProducts[productID].token;

        (uint256 buyersDisputeFee, uint256 sellerDisputeFee, ) = _getDisputeFees(s_amount, s_token);

        uint256 _lent = tips[productID].allTippers.length;

        if (_who) {
            allProducts[productID].amount = (s_amount - buyersDisputeFee);
            uint256 forEach = buyersDisputeFee / len;
            if (len > 0) {
                for (uint256 i; i < _lent; ){
                    address voters = returnAllCurrentMembers(productID, i);
                    if(voters != address(0)) {
                        _update(voters);
                        _contractTransfer(s_token, voters, forEach); 
                        emit rewards(s_token, voters, buyersDisputeFee, block.timestamp);  
                    }  
                    unchecked {
                        i++;
                    }
            ...
            if (len > 0) {
                for (uint256 i; i < _lent; ) {
                    ...
                    if(voters != address(0)) {
                        ...
                        emit rewards(RGP, voters, sellerDisputeFee, block.timestamp);
                    }  
                    unchecked {
                        i++;
                    }     
    ...
    function returnAllCurrentMembers(uint256 productID, uint256 _indexed) private view returns(address _ad) {
        address addr = tips[productID].allTippers[_indexed];
        if(checkVoteInit[productID][addr]) {
            return addr;
        }
    }
    ...
    function getWhoIsleading(uint256 productID) external view returns(MembersTips memory, address[] memory) {
        return (tips[productID], returnAllMembers(productID));
    }

    function returnAllMembers(uint256 productID) private view returns(address[] memory all) {
        uint256 lent = tips[productID].allTippers.length;
        all = new address[](lent);
        for(uint256 i; i < lent; ) {
            address addr = tips[productID].allTippers[i];
            if(checkVoteInit[productID][addr]) {
                all[i] = addr;
            }
            unchecked {
...
contract RigelDecentralizedP2PSystem is Rigelp2pPriceSet {
    uint256 private constant standard = 100000;
    ...
    */
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external returns(uint256){   
        ...
        uint256 getInStandard = amountInDecimals / standard;
        purchaseToken.transferFrom(from, address(this), getInStandard);
        _sellerFeeDebit(from);
        _sortRewards(from, referral);
        uint256 id = _storeProducts(address(purchaseToken), from, to, getInStandard);
        emit Buy(from, address(this), address(purchaseToken), getInStandard, id, block.timestamp);
        return id;
    ...
    function castVote(uint256 productID, address who) external {   
        __checkIfAccountHasJoined(productID);
