...
// File: contracts/GCUSDTExchange.sol
...
contract GCExchangeSwap is ReentrancyGuard, Ownable {
    ...
    constructor(address _USDT, address _GCS) {
        _transferOwnership(0x8A25ea29363894eAcf4f0ef1Bfe0cAfc75a224FF);
        USDT = IERC20(_USDT);
        GCS = IERC20(_GCS);
    ...
    function updateGCS(address _GCS) external onlyOwner {
        GCS = IERC20(_GCS);
    }

    function setFees(uint8 _val) external onlyOwner {
        feesUSDTGCS = _val;
    }

    // accepts amount of USDT and exchanges it for GCS
    function swapUSDTTOGCS(uint256 amountUSDT, uint256 _updatedBlockVal) external nonReentrant returns (uint256) {
        uint256 ratioUSDTTOGCS = _updatedBlockVal;
        //check if amount given is not 0
        // check if current contract has the necessary amout of Tokens to exchange
        require(amountUSDT > 0, "amountUSDT must be greater then zero");
        require(
            USDT.balanceOf(msg.sender) >= amountUSDT,
            "sender doesn't have enough Tokens"
        );

        uint256 exchangeA = (amountUSDT * ratioUSDTTOGCS) / 10**18;
    ...
    // accepts amount of GCS and exchanges it for USDT
    function swapGCSTOUSDT(uint256 amountGCS, uint256 _updatedBlockVal) external nonReentrant returns (uint256) {
        uint256 ratioGCSTOUSDT = _updatedBlockVal;
        //check if amount given is not 0
        // check if current contract has the necessary amout of Tokens to exchange and the sender
        //require(amountGCS >= ratioGCSUSDT, "amountGCS must be greater then ratio");
        require(
            (msg.sender).balance >= amountGCS,
            "sender doesn't have enough Tokens"
        );

        uint256 exchangeA = (amountGCS * ratioGCSTOUSDT) / 10**18;
        uint256 feeDeduction = exchangeA * feesUSDTGCS / 1000;
        uint256 exchangeAmount = exchangeA - feeDeduction;
