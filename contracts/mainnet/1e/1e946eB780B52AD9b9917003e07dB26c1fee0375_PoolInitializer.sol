...
contract PoolInitializer is IPoolInitializer {
  ...
  IDexETFUniswapV2Oracle public _poolOracle;
  ...
  function getCreditForTokens(
    ...
    require(desiredAmount > 0, "DexETF: Desired amount is zero");
  ...
  constructor(address controller_) {
    require(controller_ != address(0), "DexETF: Controller is zero address");
  ...
  function initialize(
    ...
    IDexETFUniswapV2Oracle poolOracle
  ) external override _control_ returns (bool) {
    address zero = address(0);
    require(_poolAddress == zero, "DexETF: Already initialized");
    require(poolAddress != zero, "DexETF: Pool address is zero address");
    require(address(poolOracle) != zero, "DexETF: Pool oracle is zero address");
    _poolAddress = poolAddress;
    _poolOracle = poolOracle; 
    uint256 len = tokens.length;
    require(amounts.length == len, "DexETF: Invalid arrays length");
    _tokens = tokens;
  ...
  function finish() external override _lock_ _not_finished_ returns (bool) {
    ...
    for (uint256 i = 0; i < len; i++) {
      ...
      require(_remainingDesiredAmounts[token] == 0, "DexETF: Pending tokens is needed");
    }
  ...
  function contributeTokens(
    ...
    uint256 desiredAmount = _remainingDesiredAmounts[token];
    require(desiredAmount > 0, "DexETF: Already contributed");
    if (amountIn > desiredAmount) {
      amountIn = desiredAmount;
    }
    ...
    require(credit > 0 && amountIn > 0, "DexETF: Amounts too low");
    require(credit >= minimumCredit, "DexETF: Minimum credit");
  ...
  function contributeTokens(
    ...
    address caller = msg.sender;
    uint256 len = tokens.length;
    require(amountsIn.length == len, "DexETF: Invalid arrays length");
    credit = 0;
    for (uint256 i = 0; i < len; i++) {
      ...
      require(creditOut > 0 && amountIn > 0, "DexETF: Amounts too low");
      IERC20(token).safeTransferFrom(caller, address(this), amountIn);
      _remainingDesiredAmounts[token] = desiredAmount.sub(amountIn);
  ...
  function _claimTokens(address account) internal returns (bool) {
    ...
    require(credit > 0, "DexETF: Credit is zero");
  ...
  modifier _control_ {
    require(msg.sender == controller, "DexETF: Caller is not controller");
    _;
  }

  modifier _finished_ {
    require(_finished, "DexETF: Not finished");
    _;
  }

  modifier _not_finished_ {
    require(!_finished, "DexETF: Already finished");
    _;
  }
}
...
interface IMarketCapSqrtController {
  ...
  function computeAverageMarketCap(address token, IDexETFUniswapV2Oracle oracle) external view returns (uint144);
