...
contract BshareRewardPool {
    ...
    uint256 public runningTime = 5 * 365 days; // 5 years
    uint256 public constant TOTAL_REWARDS = 115750 ether;
    ...
    function updateBsharePerSecond() public onlyOperator {
        if (block.timestamp > (poolStartTime + 31536000 * 4)) {
            BsharePerSecond = 0.00382587448 ether; // 59500 bshare / (180 days * 24h * 60min * 60s)
        }else if (block.timestamp > (poolStartTime + 31536000 * 3)) {
            BsharePerSecond = 0.00382587448 ether; // 59500 bshare / (180 days * 24h * 60min * 60s)
        } else if (block.timestamp > (poolStartTime + 31536000 * 2)) {
            BsharePerSecond = 0.00382587448 ether; // 59500 bshare / (180 days * 24h * 60min * 60s)
        } else if (block.timestamp > (poolStartTime + 31536000 * 1)) {
            BsharePerSecond = 0.00382587448 ether; // 59500 bshare / (180 days * 24h * 60min * 60s)
        } else if (block.timestamp > poolStartTime) {
            BsharePerSecond = 0.00382587448 ether; // 59500 bshare / (180 days * 24h * 60min * 60s)
        }
    }
}