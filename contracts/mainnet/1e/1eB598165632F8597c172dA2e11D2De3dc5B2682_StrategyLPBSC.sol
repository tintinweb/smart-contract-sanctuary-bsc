...
contract StrategyLPBSC is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        token0id = IXswapFarm(nativeFarmAddress).getTokenId(token0Address);
        token1id = IXswapFarm(nativeFarmAddress).getTokenId(token1Address);
    ...
    function BUSDToWant(uint256 _wantAmt) internal returns (uint256) {
        ...
        // Increase allowance on routerAddress0
        address uniRouterAddress0 = IXswapFarm(nativeFarmAddress).uniRouterAddress(token0id);
        IERC20(BUSD).safeIncreaseAllowance(uniRouterAddress0, _wantAmt);
        // If BUSD is different from token0
        if (BUSD != token0Address) {
            // Swap half earned to token0
            if (address(uniRouterAddress0) == address(IXswapFarm(nativeFarmAddress).ELLIPSIS_3EP_POOL())) {
                uint256 dy = IEllipsis(uniRouterAddress0).get_dy(0, int128(int(token0id)), _wantAmt.div(2));
                IEllipsis(uniRouterAddress0).exchange(0, int128(int(token0id)), _wantAmt.div(2), dy);
            } else {
                IXRouter02(uniRouterAddress0)
                    .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    _wantAmt.div(2),
                    0,
                    IXswapFarm(nativeFarmAddress).getBUSDToTokenPath(token0id),
                    address(this),
                    block.timestamp + 300
                );
            }
        ...
        // Increase allowance on routerAddress1
        address uniRouterAddress1 = IXswapFarm(nativeFarmAddress).uniRouterAddress(token1id);
        IERC20(BUSD).safeIncreaseAllowance(uniRouterAddress1, _wantAmt);
        // If BUSD is different from token1
        if (BUSD != token1Address) {
            // Swap half earned to token1
            if (address(uniRouterAddress1) == address(IXswapFarm(nativeFarmAddress).ELLIPSIS_3EP_POOL())) {
                uint256 dy = IEllipsis(uniRouterAddress1).get_dy(0, int128(int(token1id)), _wantAmt.div(2));
                IEllipsis(uniRouterAddress1).exchange(0, int128(int(token1id)), _wantAmt.div(2), dy);
            } else {
                IXRouter02(uniRouterAddress1)
                    .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    _wantAmt.div(2),
                    0,
                    IXswapFarm(nativeFarmAddress).getBUSDToTokenPath(token1id),
                    address(this),
                    block.timestamp + 300
                );
            }
    ...
    function wantToBUSD(uint256 _wantAmt) internal returns (uint256) {
        ...
        if (BUSD != token0Address) {
            // Increase allowance on routerAddress0
            address uniRouterAddress0 = IXswapFarm(nativeFarmAddress).uniRouterAddress(token0id);
            IERC20(token0Address).safeIncreaseAllowance(uniRouterAddress0, token0Amt);
            // Swap half token0 to BUSD
            if (address(uniRouterAddress0) == address(IXswapFarm(nativeFarmAddress).ELLIPSIS_3EP_POOL())) {
                uint256 dy = IEllipsis(uniRouterAddress0).get_dy(int128(int(token0id)), 0, token0Amt);
                IEllipsis(uniRouterAddress0).exchange(int128(int(token0id)), 0, token0Amt, dy);
            } else {
                IXRouter02(uniRouterAddress0)
                    .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    token0Amt,
                    0,
                    IXswapFarm(nativeFarmAddress).getTokenToBUSDPath(token0id),
                    address(this),
                    block.timestamp + 300
                );
            }
        ...
        if (BUSD != token1Address) {
            // Increase allowance on routerAddress0
            address uniRouterAddress1 = IXswapFarm(nativeFarmAddress).uniRouterAddress(token1id);
            IERC20(token1Address).safeIncreaseAllowance(uniRouterAddress1, token1Amt);
            // Swap half token1 to BUSD
            if (address(uniRouterAddress1) == address(IXswapFarm(nativeFarmAddress).ELLIPSIS_3EP_POOL())) {
                uint256 dy = IEllipsis(uniRouterAddress1).get_dy(int128(int(token1id)), 0, token1Amt);
                IEllipsis(uniRouterAddress1).exchange(int128(int(token1id)), 0, token1Amt, dy);
            } else {
                IXRouter02(uniRouterAddress1)
                    .swapExactTokensForTokensSupportingFeeOnTransferTokens(
                    token1Amt,
                    0,
                    IXswapFarm(nativeFarmAddress).getTokenToBUSDPath(token1id),
                    address(this),
                    block.timestamp + 300
                );
            }
