...
contract ZTPNFT is ERC721PresetMinterPauserAutoId {
    event ModifyPropsEvent(uint256 tokenId);
    struct NFTProps {
        uint256 picId;
        uint256 quality;
        uint256 atype;
    ...
    mapping(uint256 => NFTProps) public Props;

    function mintZTPAndSet(address d, uint256[] memory _val) public returns(uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ERC721PresetMinterPauserAutoId: must have minter role to modify");
        uint256 tokenId = mint(d);
        Props[tokenId].picId = _val[0];
        Props[tokenId].quality = _val[1];
        Props[tokenId].atype = _val[2];
        Props[tokenId].basegain = _val[3];
        Props[tokenId].gainfactor = _val[4];
        emit ModifyPropsEvent(tokenId);
        return tokenId;
    }
    ...
    function transferList(address _sender, address _to, uint256[] calldata _tokenIdList) public {
        for (uint256 i = 0; i < _tokenIdList.length; i++) {
            transferFrom(_sender, _to, _tokenIdList[i]);
        }
    }

    function getUserZTPListArray(address _user) public view returns(uint256[] memory,uint256[]  memory) {
        uint256 count = balanceOf(_user);
        uint256[] memory tokenIds = new uint256[](count);
        uint256[] memory picIds = new uint256[](count);
        for(uint256 i=0;i<count;i++){
            uint256 tid = tokenOfOwnerByIndex(_user,i);
            tokenIds[i] = tid;
            picIds[i] = Props[tid].picId;
        }
        return (tokenIds,picIds);
    }
}