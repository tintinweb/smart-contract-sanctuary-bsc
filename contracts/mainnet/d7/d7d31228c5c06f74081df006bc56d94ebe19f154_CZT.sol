...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyteamFee = 1000;
    ...
    uint256 public _sellteamFee = 1000;
    ...
    constructor (
        ...
        address FundAddress,  address TeamAddress,address ReceiveAddress
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyteamFee + _buyLPDividendFee + _sellFundFee + _sellteamFee + _sellLPDividendFee + _sellLPFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellteamFee;
                require(tAmount <= maxSellAmount,"over max sell amount");
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _buyteamFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        uint256 teamAmount = usdtBalance * (_buyteamFee + _sellteamFee) * 2 / swapFee;
        ...
        USDT.transferFrom(address(_tokenDistributor), address(this), usdtBalance - fundAmount- teamAmount );
    ...
    function setBuyteamfee(uint256 teamfee) external onlyOwner {
        _buyteamFee = teamfee;
    }

    function setsellteamfee(uint256 teamfee) external onlyOwner {
...
contract CZT is AbsToken {
    constructor() AbsToken(
        ...
        9,//Decimals
        100000000,//Supply
        address(0x7282542a9FD36eDeFd5C704Cb295A0510CC40d51),//FundAddress
         address(0x122D2B0eE2E26A5E5c3219d03c8cA3c6A2d9aF87),//TeamAddress
        address(0xCC66e0c4Adc911514cF2149350C13c0D382526cD)//ReceiveAddress
    ){
    }
}