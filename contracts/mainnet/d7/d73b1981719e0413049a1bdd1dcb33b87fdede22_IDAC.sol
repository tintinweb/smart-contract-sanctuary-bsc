...
contract IDAC is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public deadFee = 3; // 销毁
    uint256 public liquidityFee = 3; // 流动性
    uint256 public marketingFee = 1; // 营销
    uint256 public foundationFee = 1; // 基金
    uint256 public lpRewardFee = 5; // LP手续费
    ...
    bool public swapFStats;
    bool public swapTStats;
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function changeSwapFStats() public onlyOwner {
        swapFStats = !swapFStats;
    }

    function changeSwapTStats() public onlyOwner {
        swapTStats = !swapTStats;
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair) {
            require(swapFStats, "can not trade");
        }

        if(to == uniswapV2Pair) {
            require(swapTStats, "can not trade");
        }
