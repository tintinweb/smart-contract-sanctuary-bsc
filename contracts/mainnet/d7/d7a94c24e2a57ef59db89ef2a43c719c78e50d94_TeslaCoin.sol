...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function ReactionDividend(address _dividend) external onlyOwner {
...
contract TeslaCoin is Context, IERC20, Ownable{
    ...
    function setTCPercent(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent ) / 1000;
    }

    function setTCAmount(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    } 

    function setTCFeeMarketing(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner {
        taxLiquidity = newLiquidityTax;
        taxMarketing = newMarketingTax;
        taxSellBack = newBuyBackTax;
    }

    function clearAllTCFees() external onlyOwner {     
        taxLiquidity = 0;
        taxMarketing = 0;
        taxSellBack = 0;
        taxBuyBack = 0;
	...
	function transferTCToken(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
	...
	function preTCLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
