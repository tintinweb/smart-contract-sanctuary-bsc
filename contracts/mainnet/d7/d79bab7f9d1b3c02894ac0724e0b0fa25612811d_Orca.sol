...
contract Orca is Ownable, ERC721, ERC721Enumerable {
    ...
    // mint shares
    uint private _mintSharesTeam = 800;
    ...
    // -- utils -- //
    function _actualMintPrice() public view returns (uint) {

        uint currentSupply = totalSupply();

        if (currentSupply >= 1751) {
            return 330000000000000000; // 1751 - 2500 0.33 BNB
        } else if (currentSupply >= 1001) {
            return 220000000000000000; // 1001 - 1750 0.22 BNB
        } else {
            return 110000000000000000; // 1 - 1000 0.11 BNB
        }
    }

    // -- reflections update -- //
    function _updateReflections() internal {
        uint currentSupply = totalSupply();

        if (currentSupply >= 1751) {
            _reflection = (330000000000000000 * _mintSharesReflect) / (10 ** _decimalBase); // 1751 - 2500 0.33 BNB
        } else if (currentSupply >= 1001) {
            _reflection = (220000000000000000 * _mintSharesReflect) / (10 ** _decimalBase); // 1001 - 1750 0.22 BNB
        } else {
            _reflection = (110000000000000000 * _mintSharesReflect) / (10 ** _decimalBase); // 1 - 1000 0.11 BNB
        }
    }
   ...
   function teamPayout() public onlyOwner {
        uint p = _balanceTeam;
        payable(0xD7671c2Dc4b3C8D15FBda1375072a033B502612C).transfer((p*2500)/10000); //20% Charity
        payable(0xC694c44a735935564DeFC2303f13b007C015c2B5).transfer((p*2000)/10000); // 16% Treasury
        payable(0xD90297E33Cfad95F0E829222F1fe22f60eF1AC71).transfer((p*1250)/10000); // 10% Orca Fund
        payable(0x433E3D4F53928682B98fA3c44Be34557B4F9C051).transfer((p*2000)/10000); // 16% Dev Fund
        payable(0xEA54305d63Dbd32339637CB5B8A23905e59BE16e).transfer((p*750)/10000); // 6% Team
        payable(0xaFd164d218a00C04A3d3b6b5ce2F22Db7F1d26A8).transfer((p*750)/10000); // 6% Team
        payable(0xD991edBF5e93993312520F4570958d6f93c3aaEe).transfer((p*750)/10000); // 6% Team
        _balanceTeam = 0;
    }
    ...
    function cData() public view returns (bool, uint, uint, uint, uint, uint, uint, string memory) {
        return (_mintingStarted, _decimalBase, totalSupply(), _maxSupply, _actualMintPrice(), _reflection, _maxMintPerTx, _assetsBaseURI);
    }
