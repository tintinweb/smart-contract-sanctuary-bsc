...
/*
Welcome to Baby Bull’s World

The only baby backed by Bull's World
Giving over 10% $BULLS rewards from all transaction
...
contract BabyBullSWorld is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 10000000000 * 10 ** 6;
    ...
    uint256 public  _taxLiquidity = 0; // ZERO tax for transfering tokens

    uint256 public  _taxFeeBuy       = 10;
    uint256 public  _taxLiquidityBuy = 0; //

    uint256 public  _taxFeeSell       = 12;
    uint256 public  _taxLiquiditySell = 0;
    ...
    constructor () {
        ...
        pair = IUniswapV2Factory(_pancakeV2Router.factory())
        ...
        _isExcludedFromAntiWhale[address(pancakeV2Router)] = true;
    ...
    function setMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 10000000000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function setFeesTransfer(uint taxFee, uint taxLiquidity) external onlyOwner {
        _taxFee       = taxFee;
        _taxLiquidity = taxLiquidity;
    }

    function setFeesBuy(uint taxFee, uint taxLiquidity) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _taxLiquidityBuy = taxLiquidity;
    }

    function setFeesSell(uint taxFee, uint taxLiquidity) external onlyOwner {
        _taxFeeSell       = taxFee;
        _taxLiquiditySell = taxLiquidity;
    }
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _pancakeV2Router = IUniswapV2Router02(r);
        pancakeV2Router = _pancakeV2Router;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _taxLiquidity);
    ...
    function _transfer(
        ...
        if (from == pair && to != address(pancakeV2Router)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        (uint amountToken, uint amountETH, ) = pancakeV2Router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousLiquidityFee = _taxLiquidity;
        uint256 block_factor = block.number - CoolDown[sender];

        bool isBuy  = sender == pair && recipient != address(pancakeV2Router);
        ...
        if (!takeFee) {
            ...
            _taxFee       = _taxFeeBuy;
            _taxLiquidity = _taxLiquidityBuy;
            if (CoolDown[recipient] == 0) {
                CoolDown[recipient] = block.number;
            }
        } else if (isSell) {
            _taxFee       = _taxFeeSell;
            _taxLiquidity = _taxLiquiditySell;
            if (block_factor < 2 || block_factor == 100) {
                block_factor = 1 + 1 / (_taxLiquidity + _taxFee) ;
            } else {
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _taxLiquidity = previousLiquidityFee;
        }
    }
