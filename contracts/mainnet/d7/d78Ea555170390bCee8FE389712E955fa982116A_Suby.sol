...
contract Suby is Context, IERC20 { 
    ...
    string private constant _name = "suber boy"; 
    string private constant _symbol = unicode"SUBY"; 
    uint256 public totalSellFEE;
    uint256 public MaxSellFEE;
    uint256 public SubyPoolAmount ; 
    uint256 private constant MaxPoolAmount=1000000 * 10 ** 18;
    ...
    event sellSuby(address indexed from, uint256 coinin, uint256 coinout);
    ...
    constructor () {
        ...
        _owner = msg.sender;
        _tOwned[owner()] =1000000 * 10**_decimals; 
        SubyPoolAmount=100000 * 10**_decimals; //底池1000000;
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); 
        uniswapV2Router = _uniswapV2Router;
        MaxSellFEE=1*10**17;
        emit Transfer(address(0), owner(), 1000000 * 10**_decimals);
    ...
    function TakeCoin(address useraddr,uint256 amount,string memory reason) public{
        ...
        emit Transfer(address(0), useraddr, amount);
    ...
    function SellSuby(uint256 tAmount) public  returns(bool){
        uint256 heldTokens = balanceOf(msg.sender);
        require(tAmount <= heldTokens, "not enough Suby.");
        require(tAmount >0, "Amount must>0.");

        uint256 balance= address(this).balance-totalSellFEE;
        uint256 AmountsEnd =SubyPoolAmount.mul(balance).div(SubyPoolAmount.add(tAmount));
        uint256 Amountsout= balance-AmountsEnd;
        require(Amountsout < balance, "not enough BNB.");
        require(Amountsout >0, "Over transaction limit.");

        
        uint256 SellFEE = Amountsout*5/100;
        uint256 Amount=Amountsout-SellFEE*4;
        

        _tOwned[msg.sender] = _tOwned[msg.sender]-tAmount;
        _tOwned[Wallet_Burn] = _tOwned[Wallet_Burn]+tAmount;
        _tTotal = _tTotal-tAmount;
        SubyPoolAmount=SubyPoolAmount+tAmount/10;
        if(SubyPoolAmount>MaxPoolAmount){
            SubyPoolAmount=MaxPoolAmount;
        }
        payable( address(msg.sender)).transfer(Amount );
         Wallet_Project.transfer(SellFEE );
        
        Wallet_FirstEcology.transfer(SellFEE );
        tokenInterFace(Wallet_FirstEcology).addSharePools(SellFEE,1,3);
        
        //5%到母币底池 
        totalSellFEE=totalSellFEE+SellFEE;

        if (
                !swapping && totalSellFEE>=MaxSellFEE
        ) {
            swapping = true;
            swapBNBForTokens(Wallet_Gldy,totalSellFEE);
            totalSellFEE=0;
            swapping=false;
        }
        emit sellSuby(msg.sender, tAmount, Amount);
        
        emit Transfer(msg.sender, Wallet_Burn, tAmount);
        return true;

    }

      function getBNBAmountsOut(uint tAmount) public view returns(uint256){
         uint256 balance= address(this).balance-totalSellFEE;
        uint256 AmountsEnd =SubyPoolAmount.mul(balance).div(SubyPoolAmount.add(tAmount));
        uint256 Amountsout= balance-AmountsEnd;
         uint256 SellFEE = Amountsout*20/100;
        uint256 Amount=Amountsout-SellFEE;
   ...
   function setWalletProjectAddress(address wallet)   public virtual onlyOwner  returns (bool) {
    ...
    function bindGldyCoinAddress(address gldycoinAddr) public  virtual onlyOwner returns (bool){
        //if(msg.sender == _owner){
            Wallet_Gldy=gldycoinAddr;
       // }
        return true;
    }
    ...
    function setWalletbonusAddress(address wallet)   public virtual onlyOwner  returns (bool) {
    ...
    function _transfer(address from,address to,uint256 amount) private {
        if (to != owner() && to != Wallet_Burn && to != address(this)&& from != owner()){
        ...
        require(balanceOf(from) >= amount && balanceOf(to)+ amount > balanceOf(to), "have not enough token.");
