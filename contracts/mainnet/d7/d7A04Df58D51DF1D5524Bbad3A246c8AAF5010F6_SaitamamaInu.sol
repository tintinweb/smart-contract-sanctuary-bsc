...
contract SaitamamaInu is Context, IERC20, Ownable {
    ...
    string private _name = "Saitamama Inu";
    string private _symbol = "SAITAMAMA";
    ...
    address public _marketingWallet = 0xa3Cbc0F89A582E73f6482404e2d882822F20989D;
    address public _devWallet = 0x926c16D59e91165620711D1efe696124619B8770;
    ...
    uint256 public _maxWalletLimit = 10000000 * 10**9;
    uint256 public _maxSoldin24Hrs = 10000000 * 10**9;
   ...
   function balanceOf(address account) public view override returns (uint256) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        tTransferAmount = tTransferAmount.sub(calculateBurnFee(tAmount));
        return (tTransferAmount, calculateTaxFee(tAmount), calculateLiquidityFee(tAmount), calculateDevFee(tAmount), tMarketing);
    }
    ...
    function _takeFees(uint256 tAmount) private {
        (,,uint256 tLiquidity, uint256 tDev, uint256 tMarketing) = _getTValues(tAmount);
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rDev = tDev.mul(currentRate);
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 tVal = tDev.add(tLiquidity).add(tMarketing);
        uint256 rVal = rDev.add(rLiquidity).add(rMarketing);
        uint256 rBurn = calculateBurnFee(tAmount).mul(currentRate);
        ...
        if(_isExcluded[address(0xdead)])
            _tOwned[address(0xdead)] = _tOwned[address(0xdead)].add(calculateBurnFee(tAmount));    
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _devFee == 0 && _marketingFee == 0 && _burnFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 devAmount = contractTokenBalance.mul(_devFee).div(_devFee+_liquidityFee+_marketingFee);
            swapTokensForEth(devAmount,_devWallet);

            //Sending to marketing wallet
            uint256 marketingAmount = contractTokenBalance.mul(_marketingFee).div(_devFee+_liquidityFee+_marketingFee);
            swapTokensForEth(marketingAmount,_marketingWallet);

            //add liquidity
            uint256 liquidityAmount = contractTokenBalance.mul(_liquidityFee).div(_devFee+_liquidityFee+_marketingFee);
