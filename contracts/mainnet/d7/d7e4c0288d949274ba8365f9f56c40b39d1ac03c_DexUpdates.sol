...
contract DexUpdates is Ownable, DexStorage, SecondDexStorage {
  event WithdrawNFT(uint256 tokenId, address collection, address to);
  ...
  event AirDrop(address indexed collection, uint256 indexed tokenId, address from, address to);
  event AirDropNonCrypto(address indexed collection, uint256 indexed tokenId, address from, address to, string ownerId);
  ...
  function buyNFT(address nft_a,uint256 tokenId, string memory ownerId, uint256 currencyType) isValid(nft_a) public{
        ...
        require(currencyType == 0 || (_allowedCurrencies[tokenId][nft_a][currencyType] && currencyType != 2), "123");
        require(msg.sender != nonCryptoNFTVault || currencyType == 0, "124" );
        require(msg.sender != nonCryptoNFTVault, "non crypto off" );
