...
contract DividendDistributor {
    ...
    IERC20 ftm = IERC20(0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23); // CRO wrapped 
...
contract TopKEK is IERC20, Ownable {
    ...
    string constant _name = "TOPKEK";
    string constant _symbol = "TOPKEK";
    ...
    uint256 public _totalFee = _liquidityFee + _reflectionFee + _marketingFee + _devFee ;
    uint256 public _liquidityFeeSell = 150;
    uint256 public _reflectionFeeSell = 0;
    uint256 public _marketingFeeSell = 650;
    uint256 public _devFeesell = 200;
    uint256 public _totalFeeSell = _liquidityFeeSell + _reflectionFeeSell + _marketingFeeSell + _devFeesell ;
    ...
    address payable private _marketingWallet = payable(0x79Eb402b870a131Cb10072fC4c9C53112f2844DD);
    ...
    uint256 private immutable snipeBlockAmt = 4;
    ...
    constructor() payable {
        ...
        } else if(block.chainid == 25 || block.chainid == 338){
            _routerAddress= 0x145677FC4d9b8F19B5D56d1820c48e0443049a30;
        } else {
            revert();
        }
    ...
    function setTaxes(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee,uint256 devfee, uint256 divisor) external onlyOwner {
        _liquidityFee = liquidityFee;
        _reflectionFee = reflectionFee;
        _marketingFee = marketingFee;
        _devFee = devfee;
        _totalFee = liquidityFee + reflectionFee + marketingFee + _devFee;
        masterTaxDivisor = divisor;
    }

    function setTaxesSales(uint256 liquidityFee, uint256 reflectionFee, uint256 marketingFee,uint256 devfeesell, uint256 divisor) external onlyOwner {
        _liquidityFeeSell = liquidityFee;
        _reflectionFeeSell = reflectionFee;
        _marketingFeeSell = marketingFee;
        _devFeesell = devfeesell;
        _totalFeeSell = liquidityFee + reflectionFee + marketingFee + _devFeesell;
        masterTaxDivisor = divisor;
    }
    ...
    function swapBack(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBdev = (amountBNB - (amountBNBReflection + amountBNBLiquidity + amountBNBMarketing));
