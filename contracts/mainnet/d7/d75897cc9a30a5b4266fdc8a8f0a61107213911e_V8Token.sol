...
contract V8Token is ERC20 {
	...
	uint256 total;
	uint256 maxTotal;
    constructor(address tokenOwner) ERC20("V8", "V8") {
	...
	function changeMintTime(uint256 _mintTime) public onlyOwner {
        mintTime = _mintTime;
	...
	uint256 public inviterAmount = 10**17;
    ...
    function getNowPrice() public view returns(uint256){
        uint256 poolwbnb = WBNB.balanceOf(uniswapV2Pair);
        uint256 pooltoken = balanceOf(uniswapV2Pair);
        if(pooltoken > 0){
            return poolwbnb.mul(10000000000).div(pooltoken);
        }
        return 0;
    }
	...
	function swapAndLiquifyLDX() public {
        ...
        if(allAmount >= ldxCanSwapAmount && ldxCanSwapAmount > 10**19 ){
			...
			addLiquidity(otherHalf, thisBalance);
		}
    }
