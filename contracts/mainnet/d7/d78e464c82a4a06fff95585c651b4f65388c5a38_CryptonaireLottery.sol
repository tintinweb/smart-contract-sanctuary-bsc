    ...
    ICryptonaireLottery
{
    ...
    function initialize(
        ...
        BUSD = IERC20Upgradeable(_BUSDTokenAddress);
     ...
     * @param _priceTicketInBUSD: price of a ticket in BUSD token
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = ((
            _lotteries[_lotteryId].amountCollectedInBUSD
        ) * 2) / 3;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                // C. No BUSD to distribute, they are added to the amount to withdraw to burn address
            } else {
        ...
        amountToWithdraw += (_lotteries[_lotteryId].amountCollectedInBUSD -
            amountToShareToWinners);
    ...
    function claimTickets(
        ...
        BUSD.safeTransfer(msg.sender, rewardInBUSDToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInBUSDToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(BUSD), "Cannot be BUSD token");
        ...
        onlyOwner
    {
        ...
        BUSD.safeTransfer(address(msg.sender), amountToShareToWinners);
    ...
    function _transferFee(uint256 _amount) private {
        for (uint32 i = 0; i < 4; i++) {
            uint256 amountToTransfer = (_amount * feeBreakdown[i]) /
                BASE_DENOMINATOR;
            BUSD.safeTransfer(feeTakerAddress[i], amountToTransfer);
        }
    }
