...
contract MythCityNFTShop {
    ...
    function buyModMythral(uint256 _modId) external {
        ...
        mythralContract.transferFrom(msg.sender, owner, tempItem.mythCost);
        require(tempItem.limit > 0, "Limit Reached");
        modShopItems[_modId].limit -= 1;
        MythCityMods tempMod = MythCityMods(modsAddress);
        require(
    ...
    function buyEngram(uint256 _engramId) external payable {
        ...
        require(tempItem.limit > 0, "Limit Reached");
        engramShopItems[_engramId].limit -= 1;
        ...
        emit engramShopBought(_engramId, engramShopItems[_engramId].limit);
    }

    function buyEngramMythral(uint256 _engramId) external {
        engramShopItem memory tempItem = engramShopItems[_engramId];
        ERC20 mythralContract = ERC20(mythralAddress);
        mythralContract.transferFrom(msg.sender, owner, tempItem.mythCost);
        require(tempItem.limit > 0, "Limit Reached");
