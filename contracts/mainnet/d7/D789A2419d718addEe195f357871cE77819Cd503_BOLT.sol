...
Techbolt Smart Cryptocurrency! Meet BOLT;
BOLT focuses on the development of NFT minting and the development of the marketplace itself based on BOLT payments. we also provide staking with great benefits.
...
contract BOLT is IERC20, Context, Ownable {
    ...
    string private constant _name = "Techbolt";
    ...
    address private Techbolt_Marketing_Wallet = 0x75bA9131E03B36153EF627784A8Fb45Eedb33ce0;

    uint256 private Techbolt_Max_Buy_Limit = 10000000 * 10**9;
    uint256 private Techbolt_Max_Sell_Limit = 10000000 * 10**9;

    address private Techbolt_Team_Wallet = 0x75bA9131E03B36153EF627784A8Fb45Eedb33ce0;

    uint256 private Techbolt_Max_Wallet_Limit = 20000000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 30, 10, 0);
    Taxes public taxes = Taxes(0, 30, 10, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Techbolt_Marketing_Wallet] = true;
        _isExcludedFromFee[Techbolt_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Techbolt_Marketing_Wallet] = true;
        allowedTransfer[Techbolt_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Techbolt_Max_Buy_Limit, "You are exceeding Techbolt_Max_Buy_Limit");
            require(balanceOf(to) + amount <= Techbolt_Max_Wallet_Limit, "You are exceeding Techbolt_Max_Wallet_Limit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Techbolt_Max_Sell_Limit, "You are exceeding Techbolt_Max_Sell_Limit");
            if(to != pair){
                require(balanceOf(to) + amount <= Techbolt_Max_Wallet_Limit, "You are exceeding Techbolt_Max_Wallet_Limit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Techbolt_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Techbolt_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Update_MarketingWallet(address newWallet) external onlyOwner{
        Techbolt_Marketing_Wallet = newWallet;
    }
    
    function Update_TeamWallet(address newWallet) external onlyOwner{
        Techbolt_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Techbolt_Max_Buy_Limit = maxBuy * 10**decimals();
        Techbolt_Max_Sell_Limit = maxSell * 10**decimals();
    }
    
    function updateTechbolt_Max_Wallet_Limit(uint256 amount) external onlyOwner{
        Techbolt_Max_Wallet_Limit = amount * 10**decimals();
    }
