...
contract TrekBusd is Context, Ownable , ReentrancyGuard {
    ...
    uint256 public constant royalityFee = 5; 
    uint256 public constant ref_fee = 10;
    address public dev = 0xe1Cc58974a5FeC77CA030BF6F2D7d9A8126b5205;
    address public dinoBusd = 0xb9150107F2820930D997a91f03Ba81A8d625F337;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
       ...
       // fees 
        ...
        uint256 total_withdrawFee = dinoFee(_amount);
        uint256 totalToSend = SafeMath.add(total_fee,total_withdrawFee); 
        uint256 total_contract = SafeMath.sub(_amount,totalToSend);
        BusdInterface.transferFrom(msg.sender,dev,total_fee);
        BusdInterface.transferFrom(msg.sender,dinoBusd,total_withdrawFee);
    ...
    if(totalRewards[msg.sender].amount >= SafeMath.mul(investments[msg.sender].invested,4)){
    ...
    function dinoFee(uint256 _amount) public pure returns(uint256){
     return SafeMath.div(SafeMath.mul(_amount,royalityFee),100);
    }
