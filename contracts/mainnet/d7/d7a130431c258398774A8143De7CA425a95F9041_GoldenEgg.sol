...
// File: contracts/goldenEggCro.sol
...
contract GoldenEgg is Context, IERC20, Ownable {
    ...
    address public _poorContract = 0x682cA13182eCA9e5230d8654215037815288B556;

    address payable public _wMarketing = 0x3E1b6a509800849306D9f440C22679b09f9e95a5;

    //buy tax
    uint256 public _maxBuyTax = 25;   // max 15% total buy tax
    ...
    //sell tax
    uint256 public _maxSellTax = 25;   // max 15% total buy tax
    uint256 public _currentSellTax = 15;   // 15% total sell tax
    uint256 public _totalSellTax = 15;   // 15% total sell tax
    uint256 private _previousRefSellTax = _totalSellTax;
    
    //reflections
    uint256 public _refPer = 0; // 5% for reflection

    //use percentage function so value is different to refPer, these values are after reflection has been done. 
    uint256 public _buyBuyBackPer = 5000; 
    uint256 public _sellAutoLiqPer = 3000;  
    uint256 public _sellBurnPer = 2000; 
    uint256 public _sellBuyBackPer = 5000; 

    uint256 public _maxHoldAmount =  100000000000 * 10**6 * 10**9;
    uint256 public _maxTransAmount = 100000000000 * 10**6 * 10**9;                             
    uint256 public _minTokensForLiquidity = 0; 
    ...
    function _setPoorContract(address poorContract) public onlyOwner() {

        _poorContract = poorContract;
    }
    ...
    function _setSellAutoLiqPer(uint256 sellAutoLiqPer) external onlyOwner() {

        _sellAutoLiqPer = sellAutoLiqPer;
    }

    function _setSellBurnPer(uint256 sellBurnPer) external onlyOwner() {

        _sellBurnPer = sellBurnPer;
    }

    function _setSellBuyBackPer(uint256 sellBackPer) external onlyOwner() {

        _sellBuyBackPer = sellBackPer;
    }

    function _setBuyBuyBackPer(uint256 buyBackPer) external onlyOwner() {

        _buyBuyBackPer = buyBackPer;
    }

    function setMaxHold(uint256 maxHold) external onlyOwner() {

        uint256 maxHoldLimit = 2000000000 * 10**6 * 10**9;

        require(maxHold >= maxHoldLimit, "Max hold is not high enough");
    ...
    function setMaxTrans(uint256 maxTransAmount) external onlyOwner() {
        ...
        uint256 maxTransLimit = 2000000000 * 10**6 * 10**9;

        require(maxTransAmount >= maxTransLimit, "Max TX is not high enough");
    ...
    function _doBuyTokenomics() public lockLiquidities {
        
        uint256 amount_of_tokens = balanceOf(address(this));
        
        if(amount_of_tokens >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true && amount_of_tokens > 0) {
            uint256 buybackAmount = _findPercent(amount_of_tokens, _buyBuyBackPer);
            uint256 marketingAmount = amount_of_tokens.sub(buybackAmount);
            
            _doBuyBack(buybackAmount);
            _doMarketing(marketingAmount);
        }
    }

    function _doSellTokenomics() public lockLiquidities {
        
        uint256 amount_of_tokens = balanceOf(address(this));
        
        if(amount_of_tokens >= _minTokensForLiquidity && _lockLiquiditiesEnabled == true && amount_of_tokens > 0) {

            uint256 liqAmount = _findPercent(amount_of_tokens, _sellAutoLiqPer);
            uint256 burnAmount = _findPercent(amount_of_tokens, _sellBurnPer);
            uint256 buybackAmount = _findPercent(amount_of_tokens, _sellBuyBackPer);
            uint256 marketingAmount = amount_of_tokens.sub(liqAmount).sub(burnAmount).sub(buybackAmount);
    ...
    function _doLiquidity(uint256 liqTokensAmount, uint256 marketingTokensAmount) private {
        ...
        _swapTokensForEth(bnbHalf + marketingTokensAmount, address(this)); 

        uint256 bnbBalance = address(this).balance; 

        uint256 liquidityBNB = _findPercent(bnbBalance, _sellAutoLiqPer);
        uint256 marketingAmountBNB = bnbBalance.sub(liquidityBNB);

        _addLiquidity(tokenHalf, liquidityBNB); 
        _sendMarketing(marketingAmountBNB);
    }

    function _sendMarketing(uint256 marketingAmountBNB) private {

        Address.sendValue(_wMarketing, marketingAmountBNB); 
    }

    function _doMarketing(uint256 amount_of_tokens) private {

        _swapTokensForEth(amount_of_tokens, address(this));

        uint256 bnbBalance = address(this).balance; 

        uint256 buybackAmountBNB = _findPercent(bnbBalance, _buyBuyBackPer);
        uint256 marketingAmountBNB = bnbBalance.sub(buybackAmountBNB);
    ...
    function _doBurn(uint256 burnTokensAmount) private {

        _burn(address(this), burnTokensAmount);
    }
