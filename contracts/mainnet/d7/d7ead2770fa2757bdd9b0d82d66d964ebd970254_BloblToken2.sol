...
contract BloblToken2 is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 88_888_888_888 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'BOBL2';
    string private _symbol = 'BOL2';
    uint8 private _decimals = 18;

    // Base Fees

    uint256 public _marketingFee = 5;//fee buy 5% for Market Address
    uint256 public _sellMarketingFee = 5;//fee sell 5% for Market Address
    ...
    uint256 public _AddressTokenLimit = 888_888_888 * 10**18;
    ...
    //Address Limit
    bool public AddressTokenLimitEnabled;
    ...
    function _transfer(
        ...
        if(AddressTokenLimitEnabled){
            uint256 maxAllowedl = _AddressTokenLimit;
    ...
    function setMarketingFeePercent(uint256 marketingFee) public onlyOwner {
        require(marketingFee <= 15, 'Marketing fee cannot be more than 15%');
    ...
    function setAddressTokenLimit(uint256 AddressTokenLimit) external onlyOwner {
        _AddressTokenLimit = AddressTokenLimit;
    }

    function setSellMarketingFeePercent(uint256 sellMarketingFee) public onlyOwner {
        require(sellMarketingFee <= 15, 'Sell marketing fee cannot be more than 10%');
    ...
    function setAddressTokenLimitEnabled(bool enabled) external {
        require(
            msg.sender == owner() || msg.sender == antiManager,
            'Only admin or anti manager allowed'
        );
        AddressTokenLimitEnabled = enabled;
    }
