...
contract AirDropLottery0318 is Manager, math{
	...
	address[] public DiamondBadgeList;
	...
	function getDiamondBadgeListLength() public view returns (uint256) {
		return DiamondBadgeList.length;
	}
    ...
    function CheckAllDiamondBadge() public view returns (address[] memory) {
        uint _balanceOf = getDiamondBadgeListLength();
		address[] memory AddrReturn = new address[](_balanceOf);
        for (uint i = 0; i < _balanceOf; i++){
		    AddrReturn[i] = DiamondBadgeList[i];
        }
        return AddrReturn;
    }
    ...
    function drawWinner() public onlyManager{
		...
		require(winnerLength < 20, "AirDropNFTLottery : Length error.");
		...
		}else if(winnerLength < 3 && winnerLength >= 1){
			RestCabinList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 20, 2);
		}else if(winnerLength < 6 && winnerLength >= 3){
			DiamondBadgeList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 13, 4);
		}else if(winnerLength < 10 && winnerLength >= 6){
			OxygenMaskList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 11, 1);
		}else if(winnerLength < 20 && winnerLength >= 10){
			GravityShoesList.push(_winner);
			GaiaMeta(FPANFT).GaiaMint(_winner, 5, 1);
		}
