...
contract SVEHeroCore is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        CoreFee _coreFee,
        uint256 _breedMax
    ) ERC721("spacemarvel.io", "SVE721-Hero") {
    ...
    {
        require(address(_SVEAccessManager) != address(0), "Error: address(0)");
        emit UpdateSVEManager(
            address(SVEAccessManager),
            address(_SVEAccessManager),
            block.timestamp
        );
        SVEAccessManager = _SVEAccessManager;
    }

    event UpdateSVEFee(address oldAddress, address newAddress, uint256 time);

    function setCoreFee(CoreFee _coreFee) external onlyOwner {
        require(address(_coreFee) != address(0), "Error: address(0)");
        emit UpdateSVEFee(address(coreFee), address(_coreFee), block.timestamp);
        coreFee = _coreFee; // check
    }

    event UpdateBreedMax(uint256 oldValue, uint256 newValue, uint256 time);

    function setBreedMax(uint256 _breedMax) external onlyOwner {
        require(_breedMax != 0, "Error: invalid input");
        emit UpdateBreedMax(breedMax, _breedMax, block.timestamp);
        breedMax = _breedMax;
    }
        ...
        returns (uint256)
    {
        ...
        require(_nftIds.length > 0, "Error: NFT ids invalid");
        require(_newGene > 0, "Error: gene invalid");
        ...
        heros[_newNftId] = Hero(_nftIds, _newGene, 0, block.timestamp);
    ...
    function breed(
        ...
        if (breedMax > 0) {
            require(heros[_nftId1].breedCount < breedMax, "NFT 1 breed max");
            require(heros[_nftId2].breedCount < breedMax, "NFT 2 breed max");
        }
...
contract CoreFee is Ownable {
    ...
    address public nftsve;
    uint256 public bornFee;
    uint256 public evolveFee;
    uint256 public breedFee;
    uint256 public destroyFee;

    constructor(IERC20 _feeContract) {
        require(address(_feeContract) != address(0), "Error: address(0)");
        feeContract = _feeContract;
    }

    modifier onlySVE() {
        require(_msgSender() == nftsve, "Error: Not SVECore");
        _;
    }

    function setSVECore(address _nftsve) external onlyOwner {
        require(_nftsve != address(0), "Error: address(0)");
        nftsve = _nftsve;
    }

    function setFeeContract(IERC20 _feeContract) external onlyOwner {
        require(address(_feeContract) != address(0), "Error: address(0)");
        feeContract = _feeContract;
    }
