...
contract casino is Ownable {
    ...
    function configsAdd(address joinToken) public onlyOwner {
        _joinToken = IERC20Mintable(joinToken);
    }

    function addHistory(uint256 _amount, address add, string memory id) private {
        ...
        _historyStakes[totalPackage].id = id;
        totalPackage ++;
    }
    ...
    function deposit(uint256 amountss, string memory _myString) public returns(bool ) {
               
        _joinToken.safeTransferFrom(msg.sender, address(this), amountss);

        addHistory(amountss, msg.sender, _myString);
        return true;
    }
