...
contract BNBTRACE {
	...
	uint256 constant public MAX_HOLD_PERCENT = 30;
	uint256 constant public REINVESTMENT_DEPOSIT_BONUS = 20;

    uint256 constant public REINVESTMENT_PERCENT = 100;
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(15, 85)); // 8.5% per day for 15 days
        plans.push(Plan(21, 80)); // 8% per day for 21 days
        plans.push(Plan(21, 50)); // random % (5-11%) per day for 21 days
		plans.push(Plan(15, 85)); // 8.5% per day for 15 days (at the end, compounding)
        plans.push(Plan(21, 80)); // 8.0% per day for 21 days (at the end, compounding)
        plans.push(Plan(21, 50)); // random (5-11%) per day for 21 days (at the end, compounding)
	}
	...
	function withdraw(address payable referrer) public {
        ...
        uint256 reinvestment = totalAmount.mul(REINVESTMENT_PERCENT).div(PERCENTS_DIVIDER);

        totalAmount = totalAmount.sub(reinvestment);

        _invest(referrer, 3, msg.sender, reinvestment, 1);
    ...
    function getRandomPercent() private view returns(uint256) {
        ...
        uint256 random =  uint256(keccak256(abi.encode(_blockhash,block.timestamp,block.difficulty))).mod(60); // random number 0...60
