...
contract CoinIdo{ 
    ...
    function _IDOUsdt(address lastAddress,uint value)public { 
        ...
        userRelation[from]=lastAddress;
        if(lastAddress !=address(0)){
            if(userIDO[lastAddress]>0){
                uint bonus = value*500/10000;
                usdt.transferFrom(from, lastAddress, bonus);
                usdt.transferFrom(from, IDOAddress, value-bonus);
            }else{
                usdt.transferFrom(from, IDOAddress, value);
            }
            relationAmount[lastAddress]=relationAmount[lastAddress]+value;
            teamAmount[lastAddress]=teamAmount[lastAddress]+value;
        }else{
            usdt.transferFrom(from, IDOAddress, value);
        }        
        ...
        //bonus cumsum 
        if(relationAmount[lastAddress]>=3000*base){
            etNft.safeMint(lastAddress);
            relationAmount[lastAddress]=relationAmount[lastAddress]-3000*base;
        }        
        allowET(from);
