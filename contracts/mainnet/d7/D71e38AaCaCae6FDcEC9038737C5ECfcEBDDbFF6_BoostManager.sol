...
contract BoostManager is CheckerManaged {
	IPillManager public pillManager;
	IDispatchManager public dispatchManager;

	uint public constant I_LAST_BOOST = 4;
	uint public constant I_BOOST_DURATION = 5;
	uint public constant I_BOOST_RATE = 6;
		...
		external
	{
		...
		require(_prices.length == _durations.length, "length");
		require(_prices.length == _rates.length, "length");

		for (uint i; i < _prices.length; i++) {
			setAll(i, _names[i], _prices[i], _durations[i], _rates[i]);
		}
	}
		...
		onlyOwner 
	{
		...
		require(prices.length == durations.length, "length");
		require(prices.length == rates.length, "length");

		Boost storage boost = boosts[i];

		boost.name = name;
		for (uint j; j < prices.length; j++) {
			_setData(boost.prices, j, prices[j]);
			_setData(boost.durations, j, durations[j]);
			_setData(boost.rates, j, rates[j]);
		}
	}
	...
	function setPrice(uint i, uint j, uint price) public onlyOwner {
		_ensureIndex0(i);
		_setData(boosts[i].prices, j, price);
	}
	
	function setDuration(uint i, uint j, uint duration) public onlyOwner {
		_ensureIndex0(i);
		_setData(boosts[i].durations, j, duration);
	}
	
	function setRate(uint i, uint j, uint rate) public onlyOwner {
		_ensureIndex0(i);
		_setData(boosts[i].rates, j, rate);
	}
	...
	// GETTERS
	function viewCountBoosts() external view returns (uint) {
		return boosts.length;
	}

	function viewCountPrices(uint i) external view returns (uint) {
		_ensureIndex0(i);
		return boosts[i].prices.length;
	}
	
	function viewCountDurations(uint i) external view returns (uint) {
		_ensureIndex0(i);
		return boosts[i].durations.length;
	}
	
	function viewCountRates(uint i) external view returns (uint) {
		_ensureIndex0(i);
		return boosts[i].rates.length;
	}
		...
		)
	{
		...
		uint[] storage prices = boosts[i].prices;
		...
		)
	{
		...
		uint[] storage durations = boosts[i].durations;

		uint maxLength = durations.length;
		if (length > maxLength - cursor)
			length = maxLength - cursor;
		...
		)
	{
		...
		uint[] storage rates = boosts[i].rates;

		uint maxLength = rates.length;
		if (length > maxLength - cursor)
			length = maxLength - cursor;
		...
		for (uint j; j < length; j++)
			arr[j] = rates[cursor + j];

		return (arr, cursor + length);
	}

	function boostWithTokens(
		uint i,
		uint j,
		...
		external
	{
		...
		uint price = boost.prices[j] * tokenIds.length;
		setManagedData(boost, msg.sender, j, tokenIds);
		dispatchManager.create(msg.sender, price);
	}
	
	function boostWithPending(
		uint i,
		uint j,
		uint[] calldata tokenIds,
		...
		canManage(msg.sender)
	{
		...
		setManagedData(boost, from, j, tokenIds);
	}
	...
	function _ensureIndex0(uint i) internal view {
		require(i < boosts.length, "doesnt exist");
	}
	
	function _ensureIndex1(Boost storage boost, uint i) internal view {
		require(i < boost.prices.length &&
				i < boost.durations.length &&
				i < boost.rates.length, 
				"doesnt exist"
			);
	}
}
