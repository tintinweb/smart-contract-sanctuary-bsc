...
contract SGToken is IERC20, Ownable {
    ...
    uint256 public _poolFee = 300;
    uint256 private _previousPoolFee;

    uint256 public _inviterFee = 500;
    uint256 private _previousInviterFee;

    uint256 public swapTokensAtAmount = 786 * 10 ** _decimals;

    uint256 private _tTotal = 786800 * 10 ** _decimals;
    uint256 private minTotal = 785800 * 10 ** _decimals;

    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);

    address private LPAddress = address(0x4d9D2fd64B7cDB7171d284Bc2352d63e633bb56B);
  ...
  function _tokenOlnyTransfer(
      ...
      if (_isExcludedFromFee[recipient] || _isExcludedFromFee[sender]) {
        ...
        uint256 reciRate = 10000 - _burnFee;
        _tOwned[recipient] = _tOwned[recipient].add(tAmount.div(10000).mul(reciRate));
        emit Transfer(sender, recipient, tAmount.div(10000).mul(reciRate));
      }
    }
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        require(isswaps || _isExcludedFromFee[from] || _isExcludedFromFee[to], "zero");
          ...
          if (canSwap && !inSwapAndLiquify && from != owner() && to != owner() && from != uniswapV2Pair) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= minTotal)_burnFee = 0;
        _tOwned[_destroyAddress] = _tOwned[_destroyAddress].add(tAmount);
    ...
    function _takePoolFee(address sender,uint256 tAmount) private {
    ...
    function _transferStandard(
        ...
        _takePoolFee(sender, tAmount.div(10000).mul(_poolFee));
