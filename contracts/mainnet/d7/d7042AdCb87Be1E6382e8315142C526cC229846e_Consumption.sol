...
contract Consumption is Ownable {
    ...
    function takeBack() external {
        ...
        for (uint i = 0; i < userOrders[msg.sender].length; i++) {
            ...
            uint tReward = _earned(o.amount, o.stime, block.timestamp);
            if (tReward > 0) {
                totalRewards += tReward - o.reward;
                orderInfo[userOrders[msg.sender][i]].reward += tReward - o.reward; 
            }         
        }
    ...
    function _earned(uint amount, uint fromTimestamp, uint toTimestamp) private pure returns (uint) {
