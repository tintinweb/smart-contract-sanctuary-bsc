...
contract MagicUnicorn4 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn4DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn4", "MU4") {
        ...
        dividendTracker = new MagicUnicorn4DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn4TRACKER", "MU4TRACKER");
...
contract MagicUnicorn4DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn4 public MagicUnicorn4Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn4Contract = MagicUnicorn4(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn4 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn4Contract);

            uint256 prevBalance = MagicUnicorn4Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn4Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn4Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
