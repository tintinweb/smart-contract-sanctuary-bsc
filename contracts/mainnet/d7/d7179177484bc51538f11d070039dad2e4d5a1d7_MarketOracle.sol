...
/**
 * @title GOH price Oracle
 *      This Oracle calculates the average USD price of GOH based on the BNB-GOH and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private gohBnbPrice0CumulativeLast;
    uint private gohBnbPrice1CumulativeLast;
    uint32 private gohBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _goh_bnb;
    ...
    constructor(
        address __goh_bnb,   // Address of the GOH-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx 
        ) public {

        controller = msg.sender;

        _goh_bnb = IUniswapV2Pair(__goh_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        gohBnbPrice0CumulativeLast = _goh_bnb.price0CumulativeLast();
        gohBnbPrice1CumulativeLast = _goh_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, gohBnbBlockTimestampLast) = _goh_bnb.getReserves();
    ...
    // Get the average price of 1 GOH in the smallest BNB unit (18 decimals)
    function getGohBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_goh_bnb));

        FixedPoint.uq112x112 memory gohBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - gohBnbPrice0CumulativeLast) / (_blockTimestamp - gohBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, gohBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        (gohBnbPrice0CumulativeLast, gohBnbPrice1CumulativeLast, gohBnbBlockTimestampLast, gohBnbAverage) = getGohBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, gohBnbAverage) = getGohBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = (gohBnbAverage)*1e6 / busdBnbAverage;
