...
contract TaxCollector is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyWhitelisted() {
        require(
            ...
            "GymTaxCollector: not whitelisted or owner"
        );
        _;
    }
    ...
    function isManagementAddress(address _address) external onlyOwner {
         managementAddress = _address;
    }
    function isBusdAddress(address _address) external onlyOwner {
    ...
    function swapTokensForBNB() external onlyWhitelisted returns (bool) {
        uint256 totalBalance = token.balanceOf(address(this));
        if (totalBalance > 0) {
            _swapWholeBalanace(collectedManagementTax);
    ...
    function updateTaxationAmount(bool isBuy, uint96 amount) external onlyWhitelisted {
        if (isBuy) {
    ...
    function updateManagementTaxationAmount(uint96 amount) external onlyWhitelisted {
    ...
    function _sendAmount() private onlyWhitelisted {
        require(sellTaxAddress != address(0), "Sell Tax Address is Zero");
        require(buyTaxAddress != address(0), "Sell Tax Address is Zero");
        token.safeTransfer(sellTaxAddress, collectedSellTax);
        token.safeTransfer(buyTaxAddress, collectedBuyTax);
        collectedBuyTax = 0;
        collectedSellTax = 0;
    }
    function _sendToManagement() private  {
        require(managementAddress != address(0), "Sell Tax Address is Zero");
        IERC20Upgradeable token = IERC20Upgradeable(wbnbAddress);
