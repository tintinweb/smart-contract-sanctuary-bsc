...
contract FreMiner is ReentrancyGuard {
    ...
    function InitalContract(
        ...
        require(msg.sender == _owner);
        _freaddr = address(0xA9ef15CF0D72Bf6ae5e408967fa9A03A8e21129f);
        _fretrade = address(0xCeDd0A1EbC204d764d2172a7aDa0192DEEE4ACe2);
        _bnbtradeaddress = address(0x16b9a82891338f9bA80E2D6970FddA79D1eb0daE);
        _usdtaddress = address(0x55d398326f99059fF775485246999027B3197955);
        _wrappedbnbaddress = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
        _minepool = new FreMinePool(address(0xA9ef15CF0D72Bf6ae5e408967fa9A03A8e21129f), _owner);
        ...
        _currentMulitiper1 = uint256(188e18).div(86400);
        _currentMulitiper2 = uint256(1888e18).div(86400);
        _currentMulitiper3 = uint256(18888e18).div(86400);
        _currentMulitiper4 = uint256(188888e18).div(86400);
        ...
        returns (bool)
    {
        ...
        if (tokenAddress == _freaddr) {
            require(balancea >= minTakeBack, "less fre token.");
        } else {
            require(balanceb >= minTakeBack, "less fre token.");
        }
    ...
    function getAPY(address user, address tokenAddress) public view returns (uint256) {
        uint256 cm;
        if (_nowtotalhash <= 1e23) {
            cm = _currentMulitiper1.mul(86400);
        } else if (_nowtotalhash > 1e23 && _nowtotalhash <= 1e24) {
            cm = _currentMulitiper2.mul(86400);
        } else if (_nowtotalhash > 1e24 && _nowtotalhash <= 1e25) {
            cm = _currentMulitiper3.mul(86400);
        } else {
            cm = _currentMulitiper4.mul(86400);
        }

        uint256 sta = _userLphash[user][tokenAddress];
        uint256 tvlsta = _lphash[tokenAddress];

        if (sta == 0 || tvlsta == 0 || _nowtotalhash == 0) return 0;

        uint256 output = tvlsta.mul(cm).div(_nowtotalhash);

        uint256 moutput = sta.mul(output).div(tvlsta);

        uint256 tprc = getExchangeCountOfOneUsdt(tokenAddress);

        uint256 fprc = getExchangeCountOfOneUsdt(_freaddr);

        uint256 sa = tprc.mul(sta);

        return moutput.mul(fprc).mul(365).div(sa);
    }
}