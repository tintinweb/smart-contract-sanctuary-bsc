    ...
    TokenWithdrawable
{
    ...
    modifier onlyWastedOwner(uint256 warriorId, address caller) {
        require(ownerOf[warriorId] == caller);
        _;
    }

    event OwnerWarriorUpdated(uint256 tokenId, address userAddress);
    ...
    function setOwner(address user, uint256 warriorId) external onlyOwner {
        ownerOf[warriorId] = user;
    }
}
