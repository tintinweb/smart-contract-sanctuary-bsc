...
// https://www.busd200.com/

pragma solidity ^0.8.11;

import "./ContractData.sol";

contract BUSD200 is ContractData {
  constructor() {
    ownerNode = owner();
    accounts[ownerNode].unlockedLevel = 15;
    accounts[ownerNode].registered = true;
    accounts[mainNode].up = ownerNode;
    accounts[mainNode].unlockedLevel = 15;
  ...
  function leaderTransaction(
    ...
    if (amount > 0) {
      ...
      _payCumulativeNetworkPool();
    }
  }

  function transactionD(address ref, uint amount) external {
  ...
  function transactionRW(uint amount) public {
    _registerDeposit(msg.sender, amount);
    _payCumulativeNetworkPool();
  }

  function transactionWR(uint amount) public {
    _withdraw(0);
    _registerDeposit(msg.sender, amount);
    _payCumulativeNetworkPool();
  }

  function directBonusDeposit(address receiver, uint amount) public isAuthorized(1) {
    require(amount > 0, "Invalid amount");
    ...
    _payNetworkPool(amount, true);
    _reflectPool(amount);
    _payCumulativeNetworkPool();
  }

  function doLuckyPool(uint amount) public {
    address sender = msg.sender;
    require(amount > 0, "Invalid amount");
    require(token.allowance(msg.sender, address(this)) >= amount, "Missing token allowance");
    token.transferFrom(msg.sender, address(this), amount);

    // emit NewDoLuckyPool(sender, amount);
  ...
  function _withdraw(uint amount) private {
    ...
    uint passive = calculateTransactionPool(depositTime, allowedWithdraw, receivedTotalAmount, receivedPassiveAmount);
    ...
    if (levelBonusAmount > remainingWithdraw - (passive + toWithdrawReflect + directBonusAmount))
      levelBonusAmount = remainingWithdraw - (passive + toWithdrawReflect + directBonusAmount);
    ...
    uint poolAmount = _payNetworkPool(totalToWithdraw, false);
    networkWithdraw += totalToWithdraw + poolAmount;
    operationCount += 1;

    _reflectPool(totalToWithdraw);
  ...
  function _payWithdrawAmount(uint totalToWithdraw) private {
    ...
    token.transfer(sender, partialPayment);

    for (uint i = 0; i < shareCount; i++) {
      token.transfer(accountsShared[sender][i], partialPayment);
    }
  }
  ...
  function _distributeLevelBonus(address sender, uint amount) private {
    ...
    for (uint8 i = 0; i < _passiveBonusLevel.length; i++) {
      ...
      uint lockLevel = accounts[up].depositMin >= minToGetBonus ? 15 : 0;
  ...
  function _processROU(address sender, uint depositMin) private view {
    ...
    uint passive = calculateTransactionPool(depositTime, depositMin, receivedTotalAmount, receivedPassiveAmount);
  ...
  function _reflectPool(uint amount) private returns (uint) {
    uint reflectPool = (amount * reflectPoolPercent) / 1000;
    reflectAmount += (reflectPool / reflectAccounts);
    return reflectPool;
  }

  uint cumulativeNetworkPool = 0;

  function _payNetworkPool(uint amount, bool registerWithdrawOperation) private returns (uint) {
    uint networkPool = (amount * networkPoolPercent) / 1000;
    cumulativeNetworkPool += networkPool;
    if (registerWithdrawOperation) {
      networkWithdraw += networkPool;
      operationCount += 1;
    }
    return networkPool;
  }

  function _payCumulativeNetworkPool() private {
    uint networkPool = cumulativeNetworkPool;
    if (networkPool <= 0) return;
    token.transfer(networkReceiverA, (networkPool * 500) / 1000);
    token.transfer(networkReceiverB, (networkPool * 500) / 1000);
    cumulativeNetworkPool = 0;

    if (luckyAutoEnabled) _runLuckyDay(false);
  }

  function collectMainPool() external {
  ...
  function _collectMainPool(address sender) internal {
    ...
    token.transfer(networkReceiverB, (totalToWithdraw * 500) / 1000);
  }

  // ------------- LUCKY AUTHORIZED METHODS -----------------------
  function startLuckyForced() external isAuthorized(1) {
    _restartLucky();
  }

  function runLuckyDayForced(bool forced) external isAuthorized(1) {
    _runLuckyDay(forced);
  }
  ...
  function _runLuckyDay(bool forced) internal {
    ...
    token.transfer(winnerWallet, winnerAmount);
...
contract ContractData is Authorized {
  ...
  string public constant name = "BUSD200";
  string public constant url = "www.busd200.com";
  IERC20 public constant token = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // PROD
  ...
  uint internal constant ratio = 1;  //PROD

  uint public minAllowedDeposit = 5e18 / ratio;

  uint public minAmountToLvlUp = 30e18 / ratio;
  uint public minAmountToGetBonus = 30e18 / ratio;
  uint public minAmountToGetReflect = 2200e18 / ratio;
  uint public maxAmount = 20000e18 / ratio;

  uint public dailyRentability = 8;  //0.8% daily

  uint public directBonus = 100;
  uint public networkPoolPercent = 70;
  uint public reflectPoolPercent = 30;
  
  uint public maxPercentToWithdraw = 200;
  uint public maxPercentToReceive = 200;
  ...
  address mainNode = 0x9d4228141d44149bb1B4C784157931628BBdD200;
  address ownerNode;

  constructor() {
    _passiveBonusLevel[0]  = 100;
    _passiveBonusLevel[1]  = 50;
    _passiveBonusLevel[2]  = 25;
    _passiveBonusLevel[3]  = 25;
    _passiveBonusLevel[4]  = 25;
    _passiveBonusLevel[5]  = 25;
    _passiveBonusLevel[6]  = 25;
    _passiveBonusLevel[7]  = 25;
    _passiveBonusLevel[8]  = 25;
    _passiveBonusLevel[9]  = 25;
    _passiveBonusLevel[10] = 25;
    _passiveBonusLevel[11] = 25;
    _passiveBonusLevel[12] = 25;
    _passiveBonusLevel[13] = 25;
    _passiveBonusLevel[14] = 25;
  }
  ...
  uint public luckyMinAmount = 2e18 / ratio;
  ...
  // event NewDoLuckyPool(address indexed addr, uint256 amount, string message);
  ...
  function updateReflectFP(uint reflect) external isAuthorized(1) { reflectPoolPercent = reflect; }
  ...
  function updateMaxAmount(uint _maxAmount) external isAuthorized(1) { maxAmount = _maxAmount; }
  ...
  function getMaxLevel(address sender) view public returns(uint) {
    ...
    uint lockLevel = accounts[sender].depositMin >= minAmountToGetBonus ? 15 : 0;
    if (lockLevel < currentUnlockedLevel) return lockLevel;
    return currentUnlockedLevel;
  }

  function calculateTransactionPool(uint depositTime, uint depositMin, uint receivedTotalAmount, uint receivedPassiveAmount) view public returns(uint){
  ...
  function getAccountEarningsFlow(address sender) view public returns(MoneyFlow memory total, MoneyFlow memory toWithdraw, MoneyFlow memory toMaxEarning, MoneyFlow memory toReceiveOverMax) {
    ...
    uint passive = calculateTransactionPool(accounts[localSender].depositTime, depositMin, receivedTotalAmount, accounts[localSender].receivedPassiveAmount);
    {
