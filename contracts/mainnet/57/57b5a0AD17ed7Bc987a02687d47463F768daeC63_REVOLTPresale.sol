...
contract REVOLTPresale is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Hard Cap 100 BNB
    uint256 public hardCapAmount = 300 ether;  
    ...
    uint256 public maxBuyETHAmount = 5 ether;
	...
	// REVOLT token
	IBEP20 public REVOLT;

    constructor(
        address _REVOLT
    ) public {
        REVOLT = IBEP20(_REVOLT);
    }
    ...
    function buy() public payable nonReentrant{
        require(isSaleActive, "REVOLT: sale is not active");
        require(totalDepositedBalance.add(msg.value) <= hardCapAmount, "REVOLT: deposit limits reached");
    ...
    function claimTokens() external nonReentrant {
		require(isClaimActive, "REVOLT: Claim is not active");
		require(tokensUnclaimed[msg.sender] > 0, "REVOLT: User should have unclaimed REVOLT tokens");
		require(REVOLT.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "REVOLT: There are not enough REVOLT tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		REVOLT.safeTransfer(msg.sender, callerTokensUnclaimed);
    ...
    function withdrawUnsoldREVOLT() external onlyOwner {
		uint256 amount = REVOLT.balanceOf(address(this));
		REVOLT.safeTransfer(msg.sender, amount);
	}
