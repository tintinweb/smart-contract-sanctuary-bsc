...
contract AS is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("AS", "AS") {
	...
	function swapAndLiquify() public {
		...
		uint256 canswap = allAmount.div(3).mul(2);
		...
		if(allBalance > usdtBalance){
			uint256 newUsdt = allBalance.sub(usdtBalance);
			addLiquidityUsdt(otherAmount, newUsdt.div(2));
		}
    }
