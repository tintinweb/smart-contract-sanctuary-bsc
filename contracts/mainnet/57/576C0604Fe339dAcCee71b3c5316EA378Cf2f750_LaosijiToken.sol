...
contract LaosijiToken is Context, IERC20, Ownable {
    ...
    address public blackHoleWallet;

    string private constant _name = "Laosiji";
    string private constant _symbol = "LSJ";
    uint8 private constant _decimals = 18;

    uint256 private _totalSupply = 21000000 * 10**uint256(_decimals);
    uint256 private _maxRewardLimit = 1500;
    uint256 private _poolReward = 300;
    uint256 private _marketingReward = 200;
    uint256 private _blackHoleReward = 200;
    ...
    constructor(
        ...
        blackHoleWallet = blackHoleWallet_;
    }
    ...
    function _transfer(
        ...
        if (isExcludedFromFee[sender] != true && (isPairAddress[sender] || isPairAddress[recipient])) {
            uint256 blackHoleFee = findPercent(amount, _blackHoleReward);
            _balances[blackHoleWallet] = _balances[blackHoleWallet] + blackHoleFee;
            tokensToTransfer = tokensToTransfer - blackHoleFee;

            emit Transfer(sender, blackHoleWallet, blackHoleFee);
        }
    ...
    function setPoolReward(uint256 reward) external onlyOwner {
        require(reward + _marketingReward + _blackHoleReward <= _maxRewardLimit, "Pancake swap limitation");
        _poolReward = reward;
    }

    function setMarketingReward(uint256 reward) external onlyOwner {
        require(reward + _poolReward + _blackHoleReward <= _maxRewardLimit, "Pancake swap limitation");
        _marketingReward = reward;
    }

    function setBlackHoleReward(uint256 reward) external onlyOwner {
        require(reward + _poolReward + _marketingReward <= _maxRewardLimit, "Pancake swap limitation");
        _blackHoleReward = reward;
    }
    ...
    function setPairAddress(address pairAddress_, bool enable_) external onlyOwner {
        isPairAddress[pairAddress_] = enable_;
    }
