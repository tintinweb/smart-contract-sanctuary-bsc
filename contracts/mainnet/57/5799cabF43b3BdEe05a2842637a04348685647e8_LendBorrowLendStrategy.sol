...
contract LendBorrowLendStrategy is LogicUpgradeable {
    ...
    uint8 private avoidLiquidityFactor;
    ...
    event DestroyCircle(address token);
    ...
    modifier onlyStorage() {
        require(msg.sender == _storage, "L1");
        _;
    }
     ...
     * @param storage_ Address of Storage Contract
     */
    function setStorage(address storage_) external onlyOwner {
        require(_storage == address(0), "L4");
        _storage = storage_;
    ...
    /**
     * @notice Set avoidLiquidityFactor
     * @param _factor factor value
     */
    function setAvoidLiquidityFactor(uint8 _factor) external onlyOwner {
        avoidLiquidityFactor = _factor;
    }
        ...
        isUsedToken(token)
    {
        ...
        uint256 amount = IERC20Upgradeable(token).balanceOf(_logic);
        ...
        isUsedToken(token)
    {
        ...
        uint256 amountStorage = IStorage(_storage).getTokenDeposited(token) -
            IStorage(_storage).getTokenBalance(token);

        if (amountStorage < balanceToken)
            ILogicContract(_logic).returnTokenToStorage(amountStorage, token);
        else ILogicContract(_logic).returnTokenToStorage(balanceToken, token);

        emit DestroyCircle(token);
    }
        ...
        onlyStorage
    {
        ...
        uint256 balance = IERC20Upgradeable(token).balanceOf(logic);

        if (balance < amount) revert("no money");
