...
contract HoneyKiller {
    ...
    function checkFeesOnSell(address router, address tokenAddress, uint256 bnbIn) external payable returns(uint256, uint256){
        ...
        //Sell token
        approve(tokenAddress, router);
        (uint256 idealBnbOut, uint bnbOut) = sellSomeTokens(router, tokenAddress, tokenAmountOut);
        
        return(idealBnbOut, idealBnbOut - bnbOut);
    }

    function checkHoneypot(address router, address tokenAddress, uint256 bnbIn, uint256 tolerance) external payable{
        address[] memory path = new address[](2);
        uint[] memory amounts;
        path[0] = IRouter(router).WETH();
        path[1] = tokenAddress;
        ...
        //Sell token
        (,uint bnbOut) = sellSomeTokens(router, tokenAddress, tokenAmountOut);

        //Check tolerance
        require(bnbIn - bnbOut <= tolerance, "Honeypot");
    }
