...
contract CrazyApez3DTest is ERC721, ERC721Enumerable {
    using Strings for uint256;
    ...
    uint256 public mintPriceBnb = 0.003 ether; // for testing
    ...
    uint256 public dailyPrizeAmount = 1 ether; // for testing
    ...
    constructor(address admin_, string memory _name, string memory _symbol) ERC721(_name, _symbol) {
        ...
        bananaJackpots.push(0);
*/ // testing
        bananaJackpots.push(7);
        bananaJackpots.push(7);
        bananaJackpots.push(7);
        bananaJackpots.push(7);
        bananaJackpots.push(7);
        bananaJackpots.push(0);
        bananaJackpots.push(3);
        bananaJackpots.push(0);
    }
    ...
    function mint(uint256 numberOfNfts, address referrer) external payable{
        ...
        if(referrer != address(0)) {
            payable(referrer).transfer(price * 7 / 100);  // pay referrer 7%
        ...
        if(dailyPrizePool < 12 ether && !dailyPrizePoolFull) dailyPrizePool += price / 20;  // update daily price pool
    ...
    function reactivateDailyPricePoolAutoFill() external onlyOwner {
    ...
    address private teamMember5 = 0xe6497e1F2C5418978D5fC2cD32AA23315E7a41Fb;
    ...
    uint256 private fixedAmount = 5 ether;
    ...
    function sendSalaryToTeam() external {
        ...
        if(bnbValueEqualToFixedAmount > 0) {
            if(address(this).balance > bnbValueEqualToFixedAmount + 0.01 ether) {
                payable(teamMember5).transfer(bnbValueEqualToFixedAmount);
                fixedAmount = 0;
            } else if(address(this).balance > 1 ether) {
                uint256 paidAmount = address(this).balance - 0.01 ether;
    ...
    function sendSalaryToTeamAndSomeToProject(uint256 amountToBeSentToProject) external onlyOwner{
        ...
        if(bnbValueEqualToFixedAmount > 0) {
            if(address(this).balance > bnbValueEqualToFixedAmount + 0.011 ether) {
                payable(teamMember5).transfer(bnbValueEqualToFixedAmount);
                fixedAmount = 0;
            } else if(address(this).balance > 1 ether) {
                uint256 paidAmount = address(this).balance - 0.011 ether;
                payable(teamMember5).transfer(paidAmount);     
                fixedAmount -= bnbToBusd(paidAmount);
            }
        }
        ...
        if(address(this).balance > (amountToBeSentToProject * 1 ether) + 0.01 ether) {
                payable(_admin).transfer(amountToBeSentToProject * 1 ether);
        } else if(address(this).balance > 1 ether) {
                uint256 paidAmount = address(this).balance - 0.01 ether;
