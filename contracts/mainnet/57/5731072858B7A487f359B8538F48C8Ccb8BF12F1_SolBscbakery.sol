...
contract SolBscbakery is Ownable {
	using SafeMath for uint256;

	IERC20 public token=IERC20(0x570A5D26f7765Ecb712C0924E4De545B89fD43dF);
	...
	uint256 private REINVEST_TO_WITHDRAW = 6;
	...
	// uint256 private waitTime = 1 minutes;
	...
	function checkUser(address _user) public view returns (bool){
		uint256 check = block.timestamp.sub(users[_user].checkpoint);
		if(check > TIME_STEP) {
			return true;
		}
		return false;
	...
	function canWithDraw(address _user) public view returns(bool) {
		if(users[_user].reinvestCount >= REINVEST_TO_WITHDRAW) {
			return true;
		}
		return false;
	}

	modifier userCanWithdraw() {
		require(canWithDraw(msg.sender), "you cant withdraw yet");
		_;
	}


	function knead(address ref) internal {
		if(ref == msg.sender) {
			ref = address(0);
		}
	...
	function hatchEggs(address ref) external initializer checkUser_ {		
		users[msg.sender].checkpointToHatch = block.timestamp;
		users[msg.sender].reinvestCount += 1;
		knead(ref);		
	}

	function sellEggs() external initializer checkUser_ userCanWithdraw {
		User storage user =users[msg.sender];
	...
	function buyEggs(uint _amount, address ref) external initializer {
		...
		if(user.invest == 0) {
			user.checkpoint = block.timestamp;
			user.checkpointToHatch = block.timestamp;
		}
		user.invest += _amount;
	...
	function payFees(uint _amount) internal {
		...
		transferHandler(devAddress, _amount.sub(toOwners));
	}
