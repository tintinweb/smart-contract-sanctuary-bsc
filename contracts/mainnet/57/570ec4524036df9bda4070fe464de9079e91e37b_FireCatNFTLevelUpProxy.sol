...
import {IFireCatNFTLevelUpStorage} from "./interfaces/IFireCatNFTLevelUpStorage.sol";
import {IFireCatInspire} from "../src/interfaces/IFireCatInspire.sol";

/**
 * @title FireCat's FireCatNFTLevelUpProxy contract
 * @notice main: isQualified
 * @author FireCat Finance
 */
contract FireCatNFTLevelUpProxy {
    ...
    */
    function isQualified(uint256 tokenId) external returns (bool) {
        if (_tokenLevel[tokenId] == _highestLevel) {
            return false;
        }

        if (IFireCatNFTLevelUpStorage(upgradeStorage).isStakeQualified(tokenId)) {
            address levelUpPayToken = IFireCatNFTLevelUpStorage(upgradeStorage).levelUpPayToken();
            uint256 payNum = IFireCatNFTLevelUpStorage(upgradeStorage).levelUpRequirePay(_tokenLevel[tokenId]);
            uint balanceBefore = IERC20(levelUpPayToken).balanceOf(address(msg.sender));

            // call fireCatInspire to share the token
            address fireCatInspire = IFireCatNFTLevelUpStorage(upgradeStorage).fireCatInspire();
            IERC20(levelUpPayToken).transferFrom(msg.sender, address(this), payNum);
            IERC20(levelUpPayToken).approve(fireCatInspire, payNum);
            uint256 restAmount = IFireCatInspire(fireCatInspire).nftLevelUpShare(msg.sender, tokenId, payNum);

            // burn the rest of the token
            IERC20(levelUpPayToken).transfer(address(1), restAmount);
...
/**
* @notice IFireCatNFTLevelUpStorage
*/
interface IFireCatNFTLevelUpStorage {
    ...
    function fireCatInspire() external view returns (address);
    ...
    function isStakeQualified(uint256 tokenId_) external returns (bool);
