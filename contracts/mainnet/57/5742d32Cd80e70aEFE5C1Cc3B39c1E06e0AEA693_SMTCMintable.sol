...
contract SMTCMintable is SafeOwnable {
    ...
    mapping(address => uint256) public collectNumber;
    mapping(address => uint256) public subscribeNumber;
    ...
    constructor(
        ...
        Referral _referral,
        address _vault,
        address _nftReferralNFTReserve
    ) {
        USDT = _USDT;
        vault = _vault;
        SMTCard = _SMTCard;
        referral = _referral;
        nftReferralNFTReserve = _nftReferralNFTReserve;
    }
    ...
    function setNftReferralNFTReserve(address _nftReferralNFTReserve)
        external
        onlyOwner
    {
        nftReferralNFTReserve = _nftReferralNFTReserve;
    ...
    function subscribe() external {
        ...
        subscribeNumber[msg.sender] += 1;
        USDT.safeTransferFrom(msg.sender, vault, amount);
    }

    function collect() external {
        uint256 pendingCollect = getTaskCompleted(msg.sender);
        require(pendingCollect > 0, "task not completed");

        SMTCard.mintSarmetCard(msg.sender, pendingCollect);
        address referrer = referral.referrers(msg.sender);
        if (referrer != address(0) && SMTCard.balanceOf(referrer) > 0) {
            uint256 amount = referralFee * pendingCollect;
            if (USDT.balanceOf(nftReferralNFTReserve) > amount) {
                USDT.safeTransferFrom(nftReferralNFTReserve, referrer, amount);
                referralReward[referrer] = referralReward[referrer].add(amount);
            }
        }
    }

    function getTaskCompleted(address account) public view returns (uint256) {
        if (address(boxIDO) == address(0) || address(SMTCard) == address(0)) {
            return 0;
        }
        uint256 pending = subscribeNumber[account].sub(collectNumber[account]);
        if (pending > 0) {
            uint256 number = boxIDO.numberOfsuccessfulReferrals(account);
            return number.sub(collectNumber[account] * 10).div(10);
        }
        return 0;
    }
}
...
contract SarmetCard is ERC721("Sarmet Card", "SMTC"), SafeOwnable {
    using SafeMath for uint256;
    ...
    function setCreatorTokenId(uint256 tokenId) external onlyOwner {
        ...
        if (remainingCreator > 0) {
            ...
            _beforeTokenTransfer(account, account, tokenId);
        }
    }
