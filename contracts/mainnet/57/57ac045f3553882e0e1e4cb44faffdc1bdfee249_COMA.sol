...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    // staking
    mapping (address => StakingInformation) public _stakers; 
    ...
    function addStaker(address _staker, uint256 _amount) external onlyOwner {
        _stakers[_staker]._active = true;
        _stakers[_staker]._amount = _amount;
    }

    function removeStaker(address _staker) external onlyOwner {
        _stakers[_staker]._active = false;
        _stakers[_staker]._amount = 0;
    }

    function checkActiveStaker(address _staker) public view returns(bool) {
        return _stakers[_staker]._active;
    }

    function checkStakerAmount(address _staker) public view returns(uint256) {
        return _stakers[_staker]._amount;
    }

    function setStakingStatus(bool _enabled) external onlyOwner {
        stakingActive = _enabled;
    }
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
        ...
        if(newBalance >= minimumTokenBalanceForDividends) {
            ...
            // if staker has tokens in staking contract && staking active
            if(checkActiveStaker(account) && stakingActive){
                newBalance += checkStakerAmount(account);
            }

            _setBalance(account, newBalance);
            tokenHoldersMap.set(account, newBalance);

        } else if (checkActiveStaker(account) && stakingActive) { // not enough token balance, but is a staker with balance && stakingActive

            if((checkStakerAmount(account) + newBalance) >= minimumTokenBalanceForDividends) {               
                _setBalance(account, checkStakerAmount(account) + newBalance);
                tokenHoldersMap.set(account, checkStakerAmount(account) + newBalance);
            }
        }
        else {
            _setBalance(account, 0);
...
contract NegativeTax is Ownable {
    address public ownerNT;

    constructor() {
        ownerNT = msg.sender;
    }

    modifier onlyOwnerNT() { 
        require(ownerNT == msg.sender, "Not allowed"); 
        _; 
    }
...
contract COMA is ERC20, Ownable {
    ...
    constructor() payable ERC20("Compound Meta", "COMA") {
        ...
        treasuryWallet = 0x68e36c4Ea7d3A79D821bfA7F95Faff0655E1B534;
        updateStakeContract = 0x93ed4F38f6Cf958dC3eADE08fa4dA48ae39EaA4F;
        stakeContract = DEAD;

        dividendTracker = new DividendTracker(1_000, rewardToken);

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); // PCS test
    ...
    modifier onlyStaking() {
        require(stakeContract == _msgSender(), "Staking: caller is not the staking contract"); // update this
        _;
    }

    modifier onlyCanUpdateStaking() {
        require(updateStakeContract == _msgSender(), "Staking: caller is not the staking contract"); // update this
        _;
    }

    function setUpdateStakeContractAddress(address _updateStakeContract) external onlyCanUpdateStaking {
        require(_updateStakeContract != address(0), "_updateStakeContract is invalid");
        updateStakeContract = _updateStakeContract;
    }

    function updateStakeAddress(address _stakeAddress) external onlyCanUpdateStaking {
        require(_stakeAddress != address(0), "_stakeAddress is invalid");
        stakeContract = _stakeAddress;
        dividendTracker.excludeFromDividends(stakeContract);
        _isExcludedFromFees[stakeContract] = true;
    }

    function addStaker(address _staker, uint256 _amount) external onlyStaking {
        dividendTracker.addStaker(_staker, _amount);
        try dividendTracker.setBalance(payable(_staker), balanceOf(_staker)) {} catch {}
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(liquidityFeeOnBuy + liquidityFeeOnSell > 0) {
                liquidityTokens = contractTokenBalance - feeTokensBuyRewardBNB - feeTokensSellRewardBNB - feeTokensBuyMarketingBNB - feeTokensSellMarketingBNB;
                swapAndLiquify(liquidityTokens);
            }

            contractTokenBalance -= liquidityTokens;

            uint256 bnbShare = feeTokensBuyRewardBNB + feeTokensSellRewardBNB + feeTokensBuyMarketingBNB + feeTokensSellMarketingBNB;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if((feeTokensBuyMarketingBNB + feeTokensSellMarketingBNB) > 0) {
                    uint256 treasuryBNB = newBalance * (feeTokensBuyMarketingBNB + feeTokensSellMarketingBNB) / bnbShare;
                ...
                if((feeTokensBuyRewardBNB + feeTokensSellRewardBNB) > 0) {
                    uint256 rewardBNB = address(this).balance - initialBalance;
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]) {
                ...
                feeTokensBuyRewardBNB += amount * (rewardsFeeOnBuy) / 100;
                feeTokensBuyMarketingBNB += amount * (treasuryFeeOnBuy) / 100;
            } else {
                _totalFees = totalSellFee;
                feeTokensSellRewardBNB += amount * (rewardsFeeOnSell) / 100;
                feeTokensSellMarketingBNB += amount * (treasuryFeeOnSell) / 100;
            }
        ...
        if(from == uniswapV2Pair && negativeTaxEnabled && negativeTaxFeeOnBuy > 0 && !_isExcludedFromNegativeTax[to]){
            uint256 _negativeTaxAmount = initialAmount * negativeTaxFeeOnBuy / 1000;
            if(balanceOf(address(negativeTax)) > _negativeTaxAmount){
                super._transfer(address(negativeTax), to, _negativeTaxAmount);
            } else{
                super._transfer(address(negativeTax), to, balanceOf(address(negativeTax)));
            }
        }
