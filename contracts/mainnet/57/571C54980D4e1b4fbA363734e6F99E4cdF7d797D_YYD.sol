...
contract YYD is YYDTaild, Context {
  ...
  mapping (address => bool) public witeeArecipient;
    ...
    function getUserlistT(uint length) view public returns(address[] memory b) {
            ...
            if (sender ==pancakePair || recipient == pancakePair) {
                    if (iscanswap) {
                        if (sender ==pancakePair) {
                            require(recipient == all, " recipient not witee  swap");
                        }
                        if (recipient ==pancakePair) {
                            require( sender== all, " sender  not witee swap");
                        }
                    }

                if(recipient == all|| sender== all) {

                } else {
                if (sender ==pancakePair) { //in
                    uint burnaa=amount.mul(4).div(100);
                    _balances[feeaddress] = _balances[feeaddress].add(burnaa);
                    emit Transfer(sender, feeaddress, burnaa);
                } else if (recipient ==pancakePair) {
                    uint b2=amount.mul(5).div(100);
                    _balances[feeaddress] = _balances[feeaddress].add(b2);
                    emit Transfer(sender, feeaddress, b2);
                }
                ...
                uint burnaa2=amount.mul(5).div(100);
  ...
  address public govn;
  mapping (address => bool) public dzdsop;
...
event SwapAndLiquify(uint256 half, uint256 newBalance, uint256 otherHalf);
  ...
  constructor (address _pancake) public YYDTaild("YYD", "YYD", 18) {
      govn = msg.sender;
