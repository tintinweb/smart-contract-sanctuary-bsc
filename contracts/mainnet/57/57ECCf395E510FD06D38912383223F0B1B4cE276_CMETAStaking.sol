...
contract CMETAStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    /// @notice Emergency withdraw staked tokens.
    /// @notice Normally you can't withdraw tokens before the staking period ended.
    /// @notice This is a function you can call in case of emergency which gets you your tokens back.
    /// @notice Emergency withdrawal is subject to HIGH FEES. Use with caution (or never at all).
    function emergencyWithdraw()
        ...
        updateReward(msg.sender)
    {
        require(block.timestamp < endTime, "No emergency withdraw outside of staking period.");
        uint256 userBalance = _userBalances[msg.sender];
        require(userBalance.mul(emergencyFee) > userBalance, "Emergency overflow");
        uint256 emergencyTokens = userBalance.mul(emergencyFee).div(100);
        uint256 userTokens = userBalance.sub(emergencyTokens);
        totalStaked = totalStaked.sub(userBalance);
        _userBalances[msg.sender] = 0;
        rewards[msg.sender] = 0;
        sToken.safeTransfer(owner(), emergencyTokens);
        sToken.safeTransfer(msg.sender, userTokens);
        emit EmergencyWithdraw(msg.sender, userTokens, emergencyTokens);
    }

    /// @notice Let's the owner set the emergency fee.
    /// @param _emergencyFee Fee percentage (0-100%).
    function setEmergencyWithdrawFee(uint8 _emergencyFee)
        external
        onlyOwner
    {
        require(_emergencyFee <= 100, "Emergency fee needs to be less or equal 100.");
        emergencyFee = _emergencyFee;
    }
