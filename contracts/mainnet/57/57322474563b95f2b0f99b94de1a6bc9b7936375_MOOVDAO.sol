...
contract OwnableMOOV is ContextMOOV {
    address private _owner;

    event OwnerMOOVpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerMOOVpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableMOOV: caller is not the owner");
        _;
    }

 
    function renounceOwnerMOOVp() public virtual onlyOwner {
        emit OwnerMOOVpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerMOOVp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerMOOVp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableMOOV: new owner is the zero address");
        emit OwnerMOOVpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract MOOVDAO is ContextMOOV, BinanceBNB, OwnableMOOV {
    ...
    string private _name = 'MOOV DAO';
    string private _symbol = 'MOOV DAO';
