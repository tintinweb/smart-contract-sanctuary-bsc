...
contract CakeDao {
    ...
    uint256 public EGGS_TO_HATCH_1MINERS=259200;//for final version should be seconds in a day
    uint256 PSN=10000;
    uint256 PSNH=5000;
    uint256 public minBuyValue=20e18;
    uint256 public maxBuyValue=50e18;
    ...
    uint[] public rate = [200,40,40,40,40];

    IERC20 public Cake;
    address markBuy = 0x3317D54cB3BE22FB63C555d7C411780c42c3e0B6;
    address markSell = 0xFa36bF9d5fFf54384A65FFE23a74FeFdb6e60Dcc;



    constructor( address market,address cake_)
    {
        ceoAddress = market;
        Cake = IERC20(cake_);
    }
    ...
    function buyEggs(address ref, uint256 value) public {
        require(initialized);
        require(value >= minBuyValue, "Not Enough Cake");
        require(value <= maxBuyValue, "Not Enough Cake");
        uint256 eggsBought=calculateEggBuy(value,Cake.balanceOf(address(this)));
        eggsBought=SafeMath.sub(eggsBought,devBuyFee(eggsBought));
        uint256 fee=devBuyFee(value);
        Cake.transferFrom(msg.sender,markBuy,fee);
        Cake.transferFrom(msg.sender,address(this),SafeMath.sub(value,fee));
    ...
    function getDetail(address user) public view returns(
        ...
        return (calculateEggSell(_users[user].claim),_users[user].addrs.length() ,teamAmount_);
    }
