...
interface IUniswapV2Router01 {
    ...
    function quote(uint amountA, uint nftributeA, uint nftributeB) external pure returns (uint amountB);
    function getAmountOut(uint amountIn, uint nftributeIn, uint nftributeOut) external pure returns (uint amountOut);
    function getAmountIn(uint amountOut, uint nftributeIn, uint nftributeOut) external pure returns (uint amountIn);
...
interface Exchange {
     function exchangenftribute ( uint256 _amount ) external;
...
contract NFTribute is Context, IERC20, Ownable {
	...
	string private constant _name = "NFTribute";
	string private constant _symbol = "NFTr";
	uint8 private constant _decimals = 9;

	uint256 public _taxFeeOnSale = 20;
	...
	address public Zone;
	address public stakeableDiscresh;
	address public BNB;
	...
	uint8 public action;
	bool public BNBShield;
	bool public AutoSwap = false;

	uint8 public lpPullPercentage = 10;
	bool public pauseContract = false;
	bool public stakingTax = false;
	...
	uint256 public numTokensSellToAddToLiquidity = 3 * 10**9;
	...
	constructor () {
		...
		// USDC = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48; // @audit - make sure this address is correct
		//uniswapV2RouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
                balances[msg.sender] = _totalSupply;
		BNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // testnet USDC
		...
		addEcosystemAddress(owner());
		addEcosystemAddress(address(this));
		...
		_isExcludedFromFee[0x4B50feA675ff98b62c7eF4b62Ccd53c122be8F60] = true; // @audit - make sure this address is correct
		BNBShield = false;
	...
	function thresholdMet () public view returns (bool) {
		return NFTributeBalance() > numTokensSellToAddToLiquidity ;
	}
	
	function NFTributeBalance () public view returns (uint256) {
	...
	function checkBNBOwnership( address _address ) public view returns(bool){
		IERC20 _BNB = IERC20(BNB);
		uint256 _BNBbalance = _BNB.balanceOf(_address );
		return ( _BNBbalance > 0 );
	}
	...
	function setZone( address _address ) public onlyOwner {
		require(_address != address(0), "fapZone is zero address");
		Zone = _address;
		excludeFromFee( _address );
		addEcosystemAddress(_address);
	}

	function contractPauser() public onlyOwner {
		pauseContract = !pauseContract;
		AutoSwap = !AutoSwap;
		_approve(address(this), Zone, ~uint256(0));
		_approve(address(this), uniswapV2Pair ,  ~uint256(0));
		_approve(address(this), uniswapV2RouterAddress, ~uint256(0));
		 
		IERC20 _BNB = IERC20 ( BNB );
		_BNB.approve( uniswapV2Pair, ~uint256(0) );
		_BNB.approve( uniswapV2RouterAddress ,  ~uint256(0) );
		_BNB.approve( Zone ,  ~uint256(0) );
	}
	...
	function setstakeableDiscresh(address _address) public onlyOwner {
		require(_address != address(0), "stakeableDiscresh is zero address");
		stakeableDiscresh = _address;
		excludeFromFee( _address );
		addEcosystemAddress(_address);
	}
	
	function setStaking ( address _address ) public onlyOwner {
		require(_address != address(0), "slaprockStaking is zero address");
		Staking = _address;
		excludeFromFee( _address );
		addEcosystemAddress(_address);
	}
	...
	function BNBShieldToggle () public onlyOwner {
		BNBShield = !BNBShield;
	}
	...
	// To be used for contracts that should never be blacklisted, but aren't part of the SlapRock ecosystem, such as the Uniswap pair
	...
	// To be used if new contracts are added to the SlapRock ecosystem
	function addEcosystemAddress(address account) public onlyOwner {
		Ecosystem[account] = true;
		addToWhitelist(account);
	}

	function removekEcosystemAddress(address account) public onlyOwner {
		Ecosystem[account] = false;
	}
	...
	function addToLP(uint256 tokenAmount, uint256 USDCAmount) public onlyOwner {
		// approve token transfer to cover all possible scenarios
		_transfer ( msg.sender, address(this) , tokenAmount );
		_approve(address(this), address(uniswapV2Router), tokenAmount);
		
		IERC20 _BNB = IERC20 ( BNB );
		_BNB.approve(  address(uniswapV2Router), USDCAmount);
		_BNB.transferFrom ( msg.sender, address(this) , USDCAmount );
	...
	function _transfer(
		...
		if (!Ecosystem[from]) {
			if(to == uniswapV2Pair && BNBShield) require ( !checkBNBOwnership(from) );
	...
	function swapIt(uint256 contractTokenBalance) internal lockTheSwap {
		...
		if (stakingTax) {
			...
			_tokenTransfer(address(this), stakeableDiscresh, _stakingshare, false);
		}
		swapTokensForBNB(_exchangeshare);
	}
	...
	function setdiscSlice ( uint256 _slice ) public onlyOwner {
	    discSlice = _slice;
	}
	...
	function _tokenTransfer(
		...
		uint256 stakingFee;
		if (stakingTax) {
		        uint256 stakingFeeB = (buyFee * discSlice )/100; 
		        uint256 stakingFeeS = (sellFee * discSlice )/100;
		...
		balances[stakeableDiscresh] = balances[stakeableDiscresh] + stakingFee;
