...
contract TESTERNFTS is Auth, ERC721, ERC721Enumerable {
    ...
    // basic
    bool private _mintingStarted = true;
    ...
    uint256 tLevelOne = 3000000 * (10 ** _decimals);
    uint256 tLevelTwo = 5000000 * (10 ** _decimals);
    uint256 tLevelThree = 8000000 * (10 ** _decimals);
    uint256 tLevelFour = 10000000 * (10 ** _decimals);
    uint256 tLevelFive = 12000000 * (10 ** _decimals);
    uint256 _decimals = 9;
    ...
    function setWhitelistMint(bool _enabled) external authorized {
        whitelistMint = _enabled;
    }

    function setisWhitelisted(bool _enabled, address _address) external authorized {
        _whitelisted[_address] = _enabled;
    }

    function setisWhitelist(bool _enabled, address[] calldata addresses) external authorized {
        for(uint i=0; i < addresses.length; i++){
        _whitelisted[addresses[i]] = _enabled;}
    }
    ...
    function getreflection() public view returns (uint256) {
        uint256 reflection;
        reflection = (calculatetokenPrice() * _mintSharesAllotment) / (_mintDenominator);
        return reflection;
    }
    ...
    function mintBNB(uint amount) public payable whenMintingAllowed {
        ...
        if(whitelistMint){require(_whitelisted[msg.sender]);}
        mintTokenBNB(_msgSender(), amount, msg.value);
    }

    function mintToken(uint amount, uint256 tokenAmt) public whenMintingAllowed {
        uint256 tAmount = tokenAmt * (10**_decimals);
        require(((_tokenIds.current() + amount) <= _maxSupply) && (amount <= _maxMintPerTx) && (tAmount >= calculatetokenPrice() * amount) && _mintedWallet[_msgSender()] + amount <= _maxMintPerWallet, "Mint failed");
        if(whitelistMint){require(_whitelisted[msg.sender]);}
        mintToken(_msgSender(), amount, tAmount);
    }
