...
import "../interfaces/IHonorDaoChef.sol";
import "../libs/Babylonian.sol";

/*
    HonorDaoZap is a ZapperFi's simplified version of zapper contract which will:
    1. use ETH to swap to target token
    2. make LP between ETH and target token
    3. add into HonorDaoChef farm
*/
contract HonorDaoZapKnightSwap is Ownable, ReentrancyGuard {
    ...
    ZapInfo[] public zaps;
    IHonorDaoChef public honorDaoChef;
    IUniswapV2Router02 public uniRouter;

    address public constant knightAddress = 0xD23811058Eb6e7967D9a00dc3886E75610c4AbBa; // KNIGHT

    constructor(address _honorDaoChef, address _uniRouter) {
        require(_honorDaoChef != address(0), "Zap::constructor: Invalid address");
        require(_uniRouter != address(0), "Zap::constructor: Invalid address");
        honorDaoChef = IHonorDaoChef(_honorDaoChef);
    ...
    function zap(
        ...
        IERC20(knightAddress).safeTransferFrom(msg.sender, address(this), _ethIn);

        address _lp = address(honorDaoChef.lpToken(_info.pid));
        ...
        approveToken(knightAddress, address(uniRouter), _ethAmtToAddLP);
        ...
        approveToken(_lp, address(honorDaoChef), _liquidity);
        honorDaoChef.deposit(_info.pid, _liquidity, msg.sender);
        ...
        if (_transferResidual) {
            ...
            if (_ethAmtToAddLP > _ethAmtUsedInLp) {
                IERC20(knightAddress).safeTransfer(msg.sender, _ethAmtToAddLP - _ethAmtUsedInLp);
            }
        }
    ...
    function doSwapETH(address _toToken, uint256 _ethAmt) internal returns (uint256 _tokenReceived) {
        ...
        _path[0] = knightAddress;
        _path[1] = _toToken;

        approveToken(knightAddress, address(uniRouter), _ethAmt);
