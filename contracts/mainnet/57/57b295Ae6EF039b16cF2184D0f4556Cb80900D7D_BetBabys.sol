...
contract BetBabys is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xF94E5AB536FA50969a9eBB235c374f98127D7C34;
      _regularMint();
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "BetBabys: minting has not started yet");
    require(_mintAmount > 0, "BetBabys: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "BetBabys: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "BetBabys: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "BetBabys: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "BetBabys: zero address");
    require(_amount > 0, "BetBabys: you have to mint at least one");
    require(lastSupply != 0,  "BetBabys: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"BetBabys: URI query for nonexistent token");
