...
contract XSM is ERC20 {
	...
	address payable _fundAddress = payable(address(0x8F425bA2f3A776C718734BC64ec18AA7722183BA));
    address payable _marketAddress = payable(address(0xf6a342259f51857AE2a1525D25a479187a7F13Bc));
    address payable _backBuyAddress = payable(address(0x13e7eFD1029F64928F79D9E72157F4FB187bA602));
    address _hideAddress;
    ...
    constructor(address tokenOwner,address hideAddress) ERC20("XSM", "XSM") {
    ...
    function swapAndLiquify(uint256 canSwapAmount) public {
        if(balanceOf(address(this)) >= canSwapAmount){
			uint256 half54 = canSwapAmount.div(30).mul(26);
			uint256 otherHalf = canSwapAmount.sub(half54);
			uint256 initialBalance = address(this).balance;
			swapTokensForEth(half54);
			uint256 newBalance = address(this).balance.sub(initialBalance);
			addLiquidity(otherHalf, newBalance.div(33));
		}
        uint256 haveBnb = address(this).balance;
        if(haveBnb > 10**12){
            _fundAddress.transfer(haveBnb.div(22).mul(8));
            _marketAddress.transfer(haveBnb.div(22).mul(7));
            _backBuyAddress.transfer(haveBnb.div(22).mul(7));
        }
    }
