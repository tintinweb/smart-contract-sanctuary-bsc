...
interface DividendHandler {
    function distributeDividend(address pair, uint256 amount) external;
}
...
contract Vegas is ERC20, Ownable {
    using SafeMath for uint256;

    address public uniswapV2Pair;
    DividendHandler public dividendHandler;
    ...
    address public mainAddress = 0xF10a37Fe176C892D3fE4A2F6A9CA5AC46682574E;

    mapping(address => bool) private _isExcludedFromFees;

    event ExcludeFromFees(address indexed account, bool isExcluded);

    constructor() public ERC20("Vegas", "Vegas") {

        require(USDT < address(this), "contract address must be token1");
        setDividendHandler(address(0x9BFb5fA297a956CaE12475657a99c9Ff5E901bA2));
        IUniswapV2Router02 pancake = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        // Create a uniswap pair for this new token
        address _pancakeV2Pair = IUniswapV2Factory(pancake.factory())
    ...
    function setDividendHandler(address newAddress) public onlyOwner {
        dividendHandler = DividendHandler(newAddress);
        _approve(address(this),newAddress,10 ** 30);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Vegas: Account is already the value of 'excluded'");
    ...
    function _transfer(
        ...
        uint256 fees = 0;
        if (takeFee) {
            fees = amount.mul(swapFee).div(base);
        ...
        if (fees > 0) {
            ...
            dividendHandler.distributeDividend(pair, fees);
