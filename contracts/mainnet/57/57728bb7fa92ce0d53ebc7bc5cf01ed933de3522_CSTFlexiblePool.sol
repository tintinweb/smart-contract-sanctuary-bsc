...
interface ICSTPool {
    struct UserInfo {
        ...
        uint256 CSTAtLastUserAction; // keep track of CST deposited at the last user action.
...
contract CSTFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 CSTAtLastUserAction; // keeps track of CST deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // CST token
    ICSTPool public immutable CSTPool; //CST pool
    ...
    //When call CSTpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositCST(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    /**
     * @notice Constructor
     * @param _token: CST token contract
     * @param _CSTPool: CSTPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_CSTPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the CST Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in CST)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.CSTAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    CSTPool.withdrawByAmount(withdrawAmount);
        ...
        user.CSTAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositCST(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the CST Flexible Pool
    ...
    /**
     * @notice Withdraws from CST Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking CST");
        staking = false;
        CSTPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the CST Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in CSTPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = CSTPool.userInfo(address(this));
        uint256 pricePerFullShare = CSTPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into CSTPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            CSTPool.deposit(bal, 0);
        }
    }
}