    ...
    ERC721Royalty
{
    ...
    constructor() ERC721("MYTHICAL ANIMALS", "MA") {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(MINTER_ROLE, _msgSender());
    }

    function characters(uint256 tokenId)
        external
        view
        returns (
            uint256 quality,
    ...
    function ownedTokens(
        ...
        require(
            startIndex < endIndex && endIndex <= balanceOf(account),
            "Invalid index"
        );
    ...
    function mint(address to, uint256 quality)
        external
        ...
        returns (uint256 tokenId)
    {
        ...
        emit Minted(tokenId, to, 0, quality, character.score);
    }
...
interface ICards is IERC721 {
    event Minted(uint256 indexed tokenId, address indexed to, uint256 level,uint256 quality,uint256 score);
    
    function mint(address to,uint256 quality) external returns (uint256 tokenId);
}