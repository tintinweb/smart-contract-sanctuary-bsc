...
interface IBEP20 {
    ...
    function setBurnTax(uint256 tax) external;
...
interface IRandomNumberGenerator {
    function requestRandomNumber(uint256 userProvidedSeed_, uint256 mod_) external returns (uint256);
...
contract Roulette is Ownable, ReentrancyGuard {
    ...
    struct PlayerInfo {
        uint256 lastWin;                            // The block number of the user's last win. [OFP]
        mapping(address => int256) totalNetWins;    // The user's total net win amount based on a token address.
                                                    // (NOT the USD value) [OFP]
    }

    mapping(address => TokenProviderInfo) private providerInfo;    // Info of each user that provided tokens to the House.
    mapping(address => PlayerInfo) private playerInfo;             // Info of each player. [OFP]
    ...
    uint256 private transferTax = 0;    // Transfer-tax tokens support variable. Resets at every gamble(). The actual amount of tokens
                                        // that a gambler sends to the House in a lost bet.
    ...
    function provideTokensToHouse(address token_, uint256 amount_, uint256 riskPerc_) external nonReentrant noUnallowedTokens(token_) {
        ...
        uint256 beforeTransfer = IBEP20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
        IBEP20(token_).transferFrom(msg.sender, houseAddress, amount_);
        uint256 afterTransfer = IBEP20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.

        amount_ = (afterTransfer - beforeTransfer); // Support for transfer-tax tokens.
        ...
        noUnbetableTokens(token_)
    {
        ...
        if (amount_[randomNumber] > 0) {
            ...
            // If the win is higher than how much the player betted, providers lose a share in the House based on their riskable amount.
            if ((winAmount - winFeeAmount) > (totalBet + txFeeAmount)) {
                IHouse(houseAddress).payout(msg.sender, (winAmount - winFeeAmount) - (totalBet + txFeeAmount), token_);
            // If the win does not get a net reward to the player cause his total bet was higher, providers get a bump to their share, based on their riskable amount.
            } else if ((winAmount - winFeeAmount) < (totalBet + txFeeAmount)) {
                transferTax = IBEP20(token_).balanceOf(houseAddress); // Support for transfer-tax tokens.
            ...
            // Fees refund in GAMBLE.
            if (gambleToken != address(0) && priceCalc != address(0)) {
                IBEP20(gambleToken).mint(msg.sender, feeRefundInGamble(token_, winFeeAmount + txFeeAmount));
            }
            ...
            if (winAmount > totalBet) {
                ...
                if (transferTax != 0) {
                    updateProviders(token_, IBEP20(token_).balanceOf(houseAddress) - transferTax, 0); // Support for transfer-tax tokens.
                } else {
            ...
            /* [OFP] */
            tokenWinAmount[token_] += winAmount;
            player.lastWin = block.number;
            player.totalNetWins[token_] += int256(winAmount - winFeeAmount) - int256(totalBet + txFeeAmount);
            /* [OFP] */
            ...
            transferTax = IBEP20(token_).balanceOf(houseAddress) - transferTax; // Support for transfer-tax tokens.

            IBEP20(token_).transferFrom(msg.sender, ustFundAddress, txFeeAmount);

            // Fees refund in GAMBLE.
            if (gambleToken != address(0) && priceCalc != address(0)) {
                IBEP20(gambleToken).mint(msg.sender, feeRefundInGamble(token_, txFeeAmount));
            }

            updateProviders(token_, transferTax, 0); // Support for transfer-tax tokens.

            /* [OFP] */
            player.totalNetWins[token_] -= int256(totalBet + txFeeAmount);
            /* [OFP] */
    ...
    // Only callable by the owner. Removes a specific token from the list of tokens that can be provided (and then gambled).
    function unallowToken(address token_) external onlyOwner returns (bool) {
        ...
        for (uint i = indexToRemove; i < tokens.length - 1; i++) {
            tokens[i] = tokens[i+1];
        }
        tokens.pop();
    ...
    // Calculates how many GAMBLEs you get when playing (based on the gambled token price and GAMBLE price).
    function feeRefundInGamble(address tokenIn_, uint256 amountIn_) public view returns (uint256 amountOut) {
        uint256 tokenInPrice = IPriceCalculator(priceCalc).getUSDValue(tokenIn_, amountIn_);
        uint256 gamblePrice = IPriceCalculator(priceCalc).getUSDValue(gambleToken, 1e18);
        return ((tokenInPrice * 1e18) / gamblePrice);
    }
    ...
    // Only callable by the owner. Changes the GAMBLE token burn tax.
    function setGambleBurnTax(uint256 newBurnTax_) external onlyOwner {
        IBEP20(gambleToken).setBurnTax(newBurnTax_);
    }
    ...
    // Retrieves information about gamblers.
    function roulettePlayerInfo(address user_, address token_) external view returns (uint256 lastWin, int256 totalNetWins) {
        return (playerInfo[user_].lastWin, playerInfo[user_].totalNetWins[token_]);
    }
    /* [OFP] */
}