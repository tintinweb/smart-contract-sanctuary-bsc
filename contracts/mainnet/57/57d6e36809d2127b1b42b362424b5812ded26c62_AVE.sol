...
contract AVE is Context, IERC20, Ownable {
    ...
    constructor(address routerAddress) {
        ...
        _pair = IPancakePair(IPancakeFactory(_router.factory()).createPair(address(this), Usdt));
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        if (_canTra(_msgSender(), recipient, amount)) {
    ...
    function transferFrom(
        ...
        if (_canTra(sender, recipient, amount)) {
    ...
    function _canTra(address sender, address recipient, uint256 amount) private view returns (bool) {
