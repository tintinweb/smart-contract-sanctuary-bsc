...
contract SHTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHTOKEN contract."
        );
    }
...
contract SHTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    SHTOKENDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20(name_, symbol_,totalSupply_) {
        rewardToken = addrs[0];
        ...
        dividendTracker = SHTOKENDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "SHTOKEN: The dividend tracker already has that address"
        );
        SHTOKENDividendTracker newDividendTracker = SHTOKENDividendTracker(
            payable(newAddress)
        );
        require(
            newDividendTracker.owner() == address(this),
            "SHTOKEN: The new dividend tracker must be owned by the SHTOKEN token contract"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "SHTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "SHTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "SHTOKEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "SHTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "SHTOKEN: Cannot update gasForProcessing to same value"
        );
