...
contract SaleWL {
    ...
    StakingMP constant private staking = StakingMP(0xe33E05B2522355B103CDD281aD22e2f4517462fF); // Staking With List
    ...
    function invest(uint256 _amount) public {
        ...
        require(getLeftToken() >= _minBUSD.mul(_divider).div(_RATE),"Sale ended");
        ...
        if(block.timestamp >= _START_TIME.add(_SALE_DAYS) && getLeftToken() >= _minBUSD.mul(_divider).div(_RATE)){
        ...
        uint256 tokens = _amount.mul(_divider).div(_RATE);
    ...
    function withdrawingUserTokens() public {
        require(getLeftToken() < _minBUSD.mul(_divider).div(_RATE) || _status == false,"Expect the end of the sell");
    ...
    function getUserLimitNow(address userAddress) public view returns(uint256){
        ...
        if(getLeftToken() < _minBUSD.mul(_divider).div(_RATE)){
            return 0;
        }
    ...
    function getUserWithdrawNow(address userAddress) public view returns(uint256) {
        if(getLeftToken() < _minBUSD.mul(_divider).div(_RATE)){
            ...
            return users[userAddress].balance.mul(percent).div(100).div(100);
        }else{
            return 0;
        }
    }
    ...
    function setLimits(uint256 _timestamp,uint256 _percent) public returns(Limit[] memory) {
        require(msg.sender == _owner);
        limits.push(Limit({timestamp: _timestamp,percent: _percent}));
        return limits;
    }

    function editLimits(uint256 id, uint256 _timestamp,uint256 _percent) public returns(Limit memory) {
        require(msg.sender == _owner);
    ...
    function delLimit() public returns(Limit[] memory) {
        require(msg.sender == _owner);
        limits.pop();
        return limits;
    }
