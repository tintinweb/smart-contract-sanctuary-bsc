...
contract BoxShop is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function initialize() public initializer {
        ...
        boxSaleConfigs[1].price = 20*10**18;
        boxSaleConfigs[1].priceWhitelist = 18*10**18;
        boxSaleConfigs[1].isActive =  true;
        boxSaleConfigs[1].limit= 700;

        boxSaleConfigs[2].id=2;
        boxSaleConfigs[2].rank = 2;              
        boxSaleConfigs[2].price = 50*10**18;
        boxSaleConfigs[2].priceWhitelist = 45*10**18;
        boxSaleConfigs[2].isActive =  false;
        boxSaleConfigs[2].limit= 120;

        boxSaleConfigs[3].id=3;
        boxSaleConfigs[3].rank = 3;              
        boxSaleConfigs[3].price = 100*10**18;
        boxSaleConfigs[3].priceWhitelist = 90*10**18;
        boxSaleConfigs[3].isActive =  false;
        boxSaleConfigs[3].limit= 50;
        __Ownable_init();
    }
...
function buyBox(uint256 _id) external  whenNotPaused returns (uint256) {
        require(boxSaleConfigs[_id].isActive==true,'Shop not active');
        require(boxSaleConfigs[_id].limit >= totalBuys[_id].current(),'Can not buy more');
        totalBuys[_id].increment();
        uint256 _bep20 = boxSaleConfigs[_id].price;
        if(indexOfWhitelist[msg.sender]!=0){
            _bep20 = boxSaleConfigs[_id].priceWhitelist;
        }
        bep20.approve(address(this), _bep20);
        bep20.transferFrom(msg.sender, address(this), _bep20);
        uint256 id= boxNFT.buyBox(msg.sender,boxSaleConfigs[_id].rank);
        emit buyBoxEvent(id);
        return id;
    }

  function getConfig(uint256 _id )public view returns(uint256 _rank,uint256 _price, uint256 _priceWhitelist,uint256 _limit, uint256 _totalBuys, bool _isActive){
      _rank = boxSaleConfigs[_id].rank;
      _price = boxSaleConfigs[_id].price;
      _isActive = boxSaleConfigs[_id].isActive;
      _priceWhitelist =  boxSaleConfigs[_id].priceWhitelist;
      _limit= boxSaleConfigs[_id].limit;
      _totalBuys= totalBuys[_id].current();
  }

  function setConfig(uint256 _id,uint256 rank,uint256 _price,uint256 _priceWhitelist,uint256 _limit, bool _isActive) public onlyOwner{
      boxSaleConfigs[_id].id = _id;
      boxSaleConfigs[_id].rank = rank;
      boxSaleConfigs[_id].price = _price;
      boxSaleConfigs[_id].isActive = _isActive;
      boxSaleConfigs[_id].priceWhitelist = _priceWhitelist;
