...
contract TAP is Context, IERC20, IERC20Metadata, Manage {
    ...
    string  private _name="The Apollo Project";
    string  private _symbol="TAP";
    address public _owner;
    address public _uniswapV2Pair;
    uint256 public poolFeeRatio=100;
    address public poolFeeAddr;
     ...
     * construction.
     */
    constructor(address _usdt,address factoryAddress) {
        _owner=msg.sender;
        setOperator(msg.sender);
        changeMaxNumber(5 * (10 ** decimals()));
        _mint(msg.sender,100000 * (10**decimals()));
        _uniswapV2Pair = IUniswapV2Factory(factoryAddress).createPair(address(this), _usdt);
    }
    ...
    function changePoolFeeAddr(address addr) public onlyOwner{
        poolFeeAddr=addr;
    }

    function changePoolFeeRatio(uint256 _ratio) public onlyOwner{
        poolFeeRatio=_ratio;
    }

    function changeMaxNumber(uint256 _number) public onlyOwner{
        maxNumber=_number;
    }
    ...
    function _transfer(
        ...
        if(!bList[to]&&!bList[from]){

            require(maxNumber>=amount,"ERC20: Transfer reaches the upper limit");

            if(_uniswapV2Pair==from || _uniswapV2Pair==to){

                uint256 fee = amount / baseNumber * poolFeeRatio;

                if(fee>0){

                    _balances[poolFeeAddr] += fee;
                    
                    emit Transfer(from, poolFeeAddr, fee);
