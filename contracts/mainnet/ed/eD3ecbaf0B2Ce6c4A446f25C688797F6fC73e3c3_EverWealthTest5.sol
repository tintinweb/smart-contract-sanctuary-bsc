...
contract EverWealthTest5 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EverWealthTest5DividendTracker public dividendTracker;
    ...
    constructor() ERC20("EverWealthTest5", "WealthTest5") {
        ...
        dividendTracker = new EverWealthTest5DividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "EverWealthTest5TRACKER",
            "WealthTest5TRACKER"
        );
    ...
    function PermaLockForDoubleRewards(address wallet, uint256 amount) public onlyOwner {
    ...
    function getLockBalance(address account) private view returns (uint256) {
        uint256 lockBalance = balanceOf(account).add(tokensLocked[account].mul(lockBonus[account].add(100)).div(100));
        return
                lockBalance;
    }
...
contract EverWealthTest5DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EverWealthTest5 public EverWealthTest5Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        EverWealthTest5Contract = EverWealthTest5(mainContract);
    ...
    function _transfer(
        ...
        require(false, "EverWealthTest5_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EverWealthTest5_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EverWealthTest5 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EverWealthTest5Contract);

            uint256 prevBalance = EverWealthTest5Contract.balanceOf(address(this));
            ...
            {
                uint256 received = EverWealthTest5Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EverWealthTest5Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
