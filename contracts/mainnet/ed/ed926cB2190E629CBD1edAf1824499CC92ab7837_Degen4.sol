...
contract Degen4 is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    function updateFees(uint256 burnBuy, uint256 burnSell) public onlyOwner {
        ...
        require(burnBuy <= 6 && burnSell <= 6, "Burn fees can't exceed 6%");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            uint256 dead = _balances[DEAD];
            amount = amount.sub(fees);
            _balances[from] = _balances[from] - fees;
            _totalSupply = _totalSupply.sub(fees).sub(dead);
