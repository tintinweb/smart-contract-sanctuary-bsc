...
contract SwapV2 is BaseContract
{
    ...
    function sellOutput(uint256 amount_) external view returns (uint256) {
        uint256 _output_ = _getOutput(address(fur), address(usdc), amount_);
        _output_ -= calculateTax(msg.sender, _output_);
        _output_ -= calculateFurpool(msg.sender, _output_);
        return _output_;
    }

    /**
     * Get token furpool output.
     * @param amount_ Amount sold.
     * @return uint256 Amount of tokens going to furpool.
     */
    function sellFurpoolOutput(uint256 amount_) external view returns (uint256) {
        uint256 _output_ = _getOutput(address(fur), address(usdc), amount_);
        _output_ -= calculateTax(msg.sender, _output_);
        return calculateFurpool(msg.sender, _output_);
    }

    /**
     * Get token tax output.
     * @param amount_ Amount sold.
     * @return uint256 Amount of tokens going to tax.
     */
    function sellTaxOutput(uint256 amount_) external view returns (uint256) {
        uint256 _output_ = _getOutput(address(fur), address(usdc), amount_);
        return calculateTax(msg.sender, _output_);
    }
