...
/**
 * @title WetradeMigration.
 */
contract WetradeMigration is Ownable, Whitelist {
    ...
    function _migrate(IERC20 _token, address _recipient, uint256 _amountToMigrate, uint256 _maxAmount, bytes memory _signature) private {
        ...
        require(_amountToMigrate <= tokenV3.balanceOf(address(this)), "out of tokens v3, refill contract");

        claimed[msg.sender] += _amountToMigrate;
        require(claimed[msg.sender] <= _maxAmount, "exceeds account migration limit");

        tokenV3.transfer(_recipient, _amountToMigrate);
    }
}
