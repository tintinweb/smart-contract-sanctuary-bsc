...
contract PriceNFT is ERC721Enumerable{
    ...
    function createMulti(uint256 collectionid,address to,string memory _tokenURI,string memory _mintname,string memory _nftowner,uint256 _copies,string memory description, uint256 _times, uint256 _price) external 
    {
        for(uint256 i=1; i<=_times; i++){
           ...
           nftcollectionid[tokenidmint]=collectionid;
           originalowner[tokenidmint]=msg.sender;
           mint(to,tokenidmint,_tokenURI,string(abi.encodePacked(_mintname, " - ", uint2str(tokenidmint))),timeperiod,_nftowner,_copies,description);
           //stopduplicate[_tokenURI]=true;
    ...
    function setOwnerWallet(address _owner) external 
    {
