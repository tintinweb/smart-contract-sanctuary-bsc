...
abstract contract JetsetFlags is UsingFlagsWithStorage {
    ...
    uint constant LIQUIDITY_PAIR = ADMIN << 1; // 4
    uint constant FEE_EXEMPT = LIQUIDITY_PAIR << 1; // 5
    uint constant BLOCKED = FEE_EXEMPT << 1; // 6
    uint constant DEVELOPER = BLOCKED << 1; // 7
    uint constant LIQUIDITY_INJECTION_DISABLED = DEVELOPER << 1; // 8
    ...
    uint constant THIS_LIQUIDITY_INJECTION_DISABLED = LIQUIDITY_INJECTION_DISABLED << THIS_OFFSET;
...
contract Jetset is JetsetFlags, UsingBatchTransfer, UsingERC20WithStorage, UsingPermitWithStorage, UsingUUPS {
    ...
    /// @notice used for tracking contract state
    ...
    /// @notice disable liquidity injection
    function disableLiquidityInjection() external requiresAdmin {
        _setFlags(address(this), LIQUIDITY_INJECTION_DISABLED, 0);
    }

    /// @notice enable liquidity injection
    function enableLiquidityInjection() external requiresAdmin {
        _setFlags(address(this), 0, LIQUIDITY_INJECTION_DISABLED);
    }
    ...
    function withdrawTokens(address token_, address to_,  uint256 amount_) external requiresAdmin {
        ...
        if (token_ == address(this)) {
            _marketingBalance = _balanceOf(address(this));
        }
    }
    ...
    function _processTransfer(address source_, uint amount_, uint flags_) internal returns (uint) {
        if (_checkFlags(flags_, SOURCE_IS_LIQUIDITY_PAIR, 0)) {
            ...
            uint liquidityBalance = _liquidityBalance();
            if (_checkFlags(flags_, 0, THIS_LIQUIDITY_INJECTION_DISABLED) && liquidityBalance > _marketingBalance) {
                _executeLiquidityInjection(liquidityBalance);
            } else {
                _executeMarketingSwap();
            }
            if (amount_ > 0) {
    ...
    function _liquidityBalance() internal view returns (uint) {
        uint thisBalance = _balanceOf(address(this));
        return thisBalance > _marketingBalance ? thisBalance - _marketingBalance : 0;
    }
    ...
    function _executeMarketingSwap() internal  {
        if (_marketingBalance > 0) {
            try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(_marketingBalance, 0, _path, _marketingAddress, block.timestamp) {
                _marketingBalance = 0;
            } catch {}
        }
    }

    function _executeLiquidityInjection(uint liquidityTokenBalance_) internal {
        if (liquidityTokenBalance_ > 0) {
            uint balance = address(this).balance;
            if (balance > 0) {
                try _router.addLiquidityETH{value: balance}(address(this), liquidityTokenBalance_, 0, 0, address(this), block.timestamp + 1) {} catch {}
            } else {
                try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(liquidityTokenBalance_, 0, _path, address(this), block.timestamp + 1) {} catch {}
            }
        }
    }
