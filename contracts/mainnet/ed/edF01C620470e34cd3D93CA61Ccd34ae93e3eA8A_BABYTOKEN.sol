...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public userLotteryAmount = 1000000000000 * 10 ** 18;

    uint256 public userLPAmount = 0;

    address public _marketingWalletAddress;
    address public marketingToken = 0x55d398326f99059fF775485246999027B3197955;
    ...
    constructor(
        ...
        uint256[6] memory sellFeeSetting_, // rewards,lp,market,dead
        ...
        buyLotteryFee = buyFeeSetting_[3];

        sellTokenRewardsFee = sellFeeSetting_[0];
        sellLiquidityFee = sellFeeSetting_[1];
        sellMarketingFee = sellFeeSetting_[2];
        sellLotteryFee = sellFeeSetting_[3];
        sellDeadFee = sellFeeSetting_[4];
        sellLpFee = sellFeeSetting_[5];

        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyLotteryFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellLotteryFee).add(sellDeadFee).add(sellLpFee) <= 25, "Total sell fee is over 25%");
    ...
    function setUserLotteryAmount(uint256 _userLotteryAmount) external onlyOwner {
        userLotteryAmount = _userLotteryAmount;
    }
    ...
    function swapManual() public onlyOwner {
        ...
        if (AmountLotteryFee > 0) splitOtherToken();
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 lotteryFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(lotteryFee) <= 25, "Total buy fee is over 25%");
    ...
    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 lotteryFee, uint256 deadFee, uint256 lpFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(lotteryFee).add(deadFee).add(lpFee) <= 25, "Total sel fee is over 25%");
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                LtFee = amount.mul(sellLotteryFee).div(100);
                AmountLotteryFee += LtFee;
                LPFee = amount.mul(sellLpFee).div(100);
                AmountLPFee += LPFee;
                DFee = amount.mul(sellDeadFee).div(100);
                fees = LFee.add(RFee).add(MFee).add(DFee).add(LtFee).add(LPFee);
            } else {
                ...
                AmountMarketingFee += MFee;
                LtFee = amount.mul(buyLotteryFee).div(100);
                AmountLotteryFee += LtFee;
                fees = LFee.add(RFee).add(MFee).add(LtFee);
            }
    ...
    function swapAndSendToFee() private {
        ...
        uint256 initialCAKEBalance = IERC20(marketingToken).balanceOf(address(this));
        swapTokensForToken(tokens, marketingToken);
        uint256 newBalance = (IERC20(marketingToken).balanceOf(address(this))).sub(initialCAKEBalance);
    ...
    function splitOtherToken() private {
        uint256 tokens = AmountLotteryFee;
        if (tokens >= 10 ** 18) {
            ...
            if (buySize > 0) {
                ...
                if (buySize > 10) {
                    for (uint256 i = 0; i < 10; i++) {
                        user = buyUser[startIndex + i];
                        if (balanceOf(user) >= userLotteryAmount) {
                            rate = balanceOf(user).mul(10000).div(totalAmount);
                            if (rate > 0) {
                                AmountLotteryFee = AmountLotteryFee - tokens.mul(rate).div(10000);
                                super._transfer(address(this), user, tokens.mul(rate).div(10000));
                            }
                        }
