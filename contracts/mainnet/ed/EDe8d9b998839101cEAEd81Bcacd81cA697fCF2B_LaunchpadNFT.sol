...
/**
 * @title ShoeBox NFT Launchpad
 * @notice Pre-market sell ShoeBox NFT tokens.
 */
contract LaunchpadNFT is ReentrancyGuard, Ownable, Pausable {
     ...
     * @param _NFT: ShoeBox NFT interface
     */
    constructor(
        INFT _NFT
    ) {
        NFT = _NFT;
        treasuryAddress = payable(0x065F3A514B4260017190a97BAA30E56690267FaE);
        launches.push(
            Launchpad({
                totalCount: 292,
                soldCount: 0,
                priceInBNB: 0.5 ether,
                maxToUser: 10
            })
        );
    }
     ...
     * @param _priceInBNB: price in USDT for 1 token
    ...
    /**
     * @notice Buy ShoeBox NFT token from launch
        ...
        whenNotPaused
    {
        ...
        if (msg.value >= _launch.priceInBNB) {
