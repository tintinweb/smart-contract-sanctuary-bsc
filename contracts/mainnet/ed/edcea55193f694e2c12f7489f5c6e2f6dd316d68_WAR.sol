...
contract WAR is IERC20, Ownable {
    ...
    address private _projectAddress = address(0xf1da914e931CbDC6152EB4BaA82e0A43872C96d5);
    ...
    function _tokenTransferBuy(
        ...
        uint256 rate;
        if (takeFee) {
            uint256 inviterFee = tAmount.div(100).mul(5);
            uint256 projectFee = tAmount.div(100).mul(1);
            uint256 whiteFee = tAmount.div(100).mul(1);
    ...
    function _tokenTransferSell(
        ...
        uint256 rate;
        if (takeFee) {            
            uint256 inviterFee = tAmount.div(100).mul(10);
            uint256 projectFee = tAmount.div(100).mul(2);
            uint256 whiteFee = tAmount.div(100).mul(2);
            _takeInviterFee(sender, recipient,inviterFee, currentRate);
            _takeTransfer(sender, _projectAddress, projectFee, currentRate);
            _takeTransfer(sender, address(this), whiteFee, currentRate);
            rate = 10;
    ...
    function _tokenTransfer(
        ...
        uint256 rate;
        if (takeFee) {
            _takeTransfer(sender, _destroyAddress, tAmount.div(100).mul(5), currentRate);
            rate = 5;
        }
        uint256 recipientRate = 100 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(
