...
import "./ITravelstaked.sol";
import "./TravelstakedOptimized.sol";

abstract contract RewardAdder is ITravelstaked {
    address  public rewardTokenAddress;
    TravelstakedLib.TravelstakeRewardState public rewardState;
    ...
    external returns (bool) {
        return TravelstakedLib.addReward(rewardAmount, withdrawableAmount,
            rewardTokenAddress, rewardState);
    }
}

contract TravelstakedWithReward is TravelstakedOptimized, RewardAdder {
    constructor (string memory name_,
        ...
        uint withdrawEnds_,
        uint256 stakingCap_) TravelstakedOptimized (
            name_,
            tokenAddress_,
            stakingStarts_,
            stakingEnds_,
        ...
        require(rewardTokenAddress_ != address(0), "Travelstaking: 0 reward address");
    ...
    external {
        require(msg.sender == rewardSetter, "Travelstaking: Not allowed");
        rewardState.earlyWithdrawReward = withdrawableAmount;
        TravelstakedLib.addMarginalReward(rewardTokenAddress, tokenAddress,
    ...
    public
    returns (bool) {
        return TravelstakedLib.withdraw(
            msg.sender,
            tokenAddress,
            rewardTokenAddress,
            amount,
...
import "./ITravelstaked.sol";
import "./Travelstaked.Library.sol";

/**
 * A staking contract distributes rewards.
 * One can create several TraditionalTravelstaking over one
 * staking and give different rewards for a single
 * staking contract.
 */
contract TravelstakedOptimized is ITravelstaked {
    ...
    TravelstakedLib.TravelstakeState public stakeState;
    ...
    constructor (
        ...
        uint withdrawEnds_,
        uint256 stakingCap_) {
        require(TravelstakedLib.VERSION() == LIB_VERSION, "Bad linked library version");
        _name = name_;
        require(tokenAddress_ != address(0), "Travelstaking: 0 address");
        tokenAddress = tokenAddress_;

        require(stakingStarts_ > 0, "Travelstaking: zero staking start time");
        ...
        require(stakingEnds_ >= stakingStarts, "Travelstaking: staking end must be after staking starts");
        stakingEnds = stakingEnds_;

        require(withdrawStarts_ >= stakingEnds, "Travelstaking: withdrawStarts must be after staking ends");
        withdrawStarts = withdrawStarts_;

        require(withdrawEnds_ >= withdrawStarts, "Travelstaking: withdrawEnds must be after withdraw starts");
        withdrawEnds = withdrawEnds_;

        require(stakingCap_ >= 0, "Travelstaking: stakingCap cannot be negative");
    ...
    */
    ...
    function _stake(address payer, address staker, uint256 amount) internal virtual returns (bool) {
        return TravelstakedLib.stake(payer, staker, amount,
...
/**
 * @dev Travel Care Staking interface
 */
interface ITravelstaked {
