...
contract DefiPay is ERC20, Ownable{
    ...
    constructor() ERC20("DefiPay", "Dpay") {
        ...
        excludedFromFees[buybackFunds] = true;
    }
    ...
    function swapForFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            if(buybackAmt > 0){
                payable(buybackFunds).sendValue(buybackAmt);
            }
    ...
    function setBuyTaxes(uint256 _marketing, uint256 _buyback, uint256 _liquidity) external onlyOwner{
        buyTaxes = Taxes(_marketing, _buyback, _liquidity);
        totalBuyTax = _marketing + _buyback + _liquidity;
    }
 
    function setSellTaxes(uint256 _marketing, uint256 _buyback, uint256 _liquidity) external onlyOwner{
        sellTaxes = Taxes(_marketing, _buyback, _liquidity);
        totalSellTax = _marketing + _buyback + _liquidity;
    }
    ...
    function updateBuybackFunds(address newWallet) external onlyOwner{
        buybackFunds = newWallet;
    }
