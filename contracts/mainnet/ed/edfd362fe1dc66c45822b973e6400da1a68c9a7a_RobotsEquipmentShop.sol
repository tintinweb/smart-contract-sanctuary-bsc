...
import "../TestContracts/LiquidiftyCollection.sol";

contract RobotsEquipmentShop{
    LiquidiftyCollection public equipmentContainers;
    RobotsEquipment public equip;

    uint32 private commonRareCount;
    uint32 private rareEpicCount = 1500;

    uint32 public scavengingId = 7053994;
    uint32 public commonRareId = 5145017;
    uint32 public rareEpicId = 8602020;


    constructor(LiquidiftyCollection _equipmentContainers, RobotsEquipment _equipment){
        equipmentContainers = _equipmentContainers;
        equip = _equipment;
    }

    function openContainer(uint id, uint amount) public {
        require(id == commonRareId || id == rareEpicId || id == scavengingId, "invalid token id");
        require(!(id == rareEpicId) || rareEpicCount + amount <= 2000, "no rare-epic items left");
        require(equipmentContainers.balanceOf(msg.sender, id) >= amount, "insufficient balance");

        equipmentContainers.burn(msg.sender, id, amount);
        for(uint i = 0; i < amount; i++){
            equip.mint(msg.sender, id == rareEpicId? rareEpicCount++: commonRareCount++);
        }
    }
}
