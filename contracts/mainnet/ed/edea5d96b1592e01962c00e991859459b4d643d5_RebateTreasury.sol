...
contract RebateTreasury is Ownable {
    ...
    constructor(address bull, address bullOracle, address treasury) {
        Bull = IERC20(bull);
        BullOracle = IOracle(bullOracle);
        Treasury = ITreasury(treasury);
    }
    
    // Bond asset for discounted Bull at bond rate

    function bond(address token, uint256 amount) external onlyAsset(token) {
        require(amount > 0, "RebateTreasury: invalid bond amount");
        uint256 bullAmount = getBullReturn(token, amount);
        require(bullAmount <= Bull.balanceOf(address(this)) - totalVested, "RebateTreasury: insufficient bull balance");

        IERC20(token).transferFrom(msg.sender, address(this), amount);
        _claimVested(msg.sender);

        VestingSchedule storage schedule = vesting[msg.sender];
        schedule.amount = schedule.amount - schedule.claimed + bullAmount;
        schedule.period = bondVesting;
        schedule.end = block.timestamp + bondVesting;
        schedule.claimed = 0;
        schedule.lastClaimed = block.timestamp;
        totalVested += bullAmount;
    }

    // Claim available Bull rewards from bonding
    ...
    function setBull(address bull) external onlyOwner {
        Bull = IERC20(bull);
    }

    // Set Bull oracle

    function setBullOracle(address oracle) external onlyOwner {
        BullOracle = IOracle(oracle);
    }
    ...
    function _claimVested(address account) internal {
        ...
        Bull.transfer(account, claimable);
    }
    ...
    // Calculate Bull return of bonding amount of token

    function getBullReturn(address token, uint256 amount) public view onlyAsset(token) returns (uint256) {
        uint256 bullPrice = getBullPrice();
        uint256 tokenPrice = getTokenPrice(token);
        uint256 bondPremium = getBondPremium();
        return amount * tokenPrice * (bondPremium + DENOMINATOR) * assets[token].multiplier / (DENOMINATOR * DENOMINATOR) / bullPrice;
    }
    ...
    function getBondPremium() public view returns (uint256) {
        uint256 bullPrice = getBullPrice();
        if (bullPrice < 1e18) return 0;

        uint256 bullPremium = bullPrice * DENOMINATOR / 1e18 - DENOMINATOR;
        if (bullPremium < bondThreshold) return 0;
        if (bullPremium <= secondaryThreshold) {
            return (bullPremium - bondThreshold) * bondFactor / DENOMINATOR;
        } else {
            uint256 primaryPremium = (secondaryThreshold - bondThreshold) * bondFactor / DENOMINATOR;
            return primaryPremium + (bullPremium - secondaryThreshold) * secondaryFactor / DENOMINATOR;
        }
    }

    // Get BULL price from Oracle

    function getBullPrice() public view returns (uint256) {
        return BullOracle.consult(address(Bull), 1e18);
    }
    ...
    // Get claimable vested Bull for account

    function claimableBull(address account) external view returns (uint256) {
