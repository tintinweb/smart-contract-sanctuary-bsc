...
contract BNBClla is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xAC9eb20D60A09456a63fC08a95DC78F64cb11Cec); // Marketing Address
    address payable public charityAddress = payable(0xAC9eb20D60A09456a63fC08a95DC78F64cb11Cec); // Charity/Dev Address
    ...
    uint256 public _maxTxAmount;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 10**18;  //allow maximum sell amount(anti-dump setup)
    ...
    function setCharityFeePercent(uint256 divisor) external onlyOwner() {
        ...
        require(charityDivisor <= 10, "Must be less than 12% total");
    }
  ...
  function setSaleFeePercent(uint256 burnTaxFee,uint256 sellLiquidityFee,uint256 sellTaxFee) external onlyOwner() {
        ...
        require(totalSwapableSaleFee <= 12, "Must be less than 12% total");
    ...
    function setBuyFeePercent(uint256 burnTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
        ...
        require(totalSwapableBuyFee <= 12, "Must be less than 12% total");
