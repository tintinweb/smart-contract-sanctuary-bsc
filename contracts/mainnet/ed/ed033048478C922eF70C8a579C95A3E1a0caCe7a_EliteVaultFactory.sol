...
contract EliteVaultFactory {
    ...
    mapping(address => bool) private vaultExists;
    mapping(uint256 => address) private vaultIndex;
    mapping(uint256 => VaultMetadata) private vaultMetadata;
    ...
    function getVault(uint256 _vaultId) public view returns (address) {
        return vaultIndex[_vaultId];
    }

    function getVaults() public view returns (address[] memory) {
        address[] memory vaults = new address[](totalVaults);

        for (uint256 i = 0; i < totalVaults; i++) {
            vaults[i] = vaultIndex[i];
        }

        return vaults;
    }

    function getVaultMetadata(uint256 _vaultId) public view returns (VaultMetadata memory) {
        return vaultMetadata[_vaultId];
    }
    ...
    function createVault(string memory _name, string memory _symbol, string memory _description) public {
        vaultIndex[totalVaults] = address(new EliteVault(stakingToken, _name, _symbol));

        vaultMetadata[totalVaults] = VaultMetadata({
            name: _name,
    ...
    function distributeToVault(uint256 _vaultId, uint256 _amount) public {
        IERC20(stakingToken).safeTransferFrom(msg.sender, vaultIndex[_vaultId], _amount);
    }

    function distributeToVaults(uint256[] memory _vaultIds, uint256[] memory _amounts) public {
        require(_vaultIds.length == _amounts.length, "INVALID_INPUT");

        for (uint256 i = 0; i < _vaultIds.length; i++) {
            distributeToVault(_vaultIds[i], _amounts[i]);
        }
    }
}