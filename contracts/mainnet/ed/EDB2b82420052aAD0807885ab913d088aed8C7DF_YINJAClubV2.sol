    ...
    ReentrancyGuardUpgradeable
{
    ...
    function isPublicSaleOn() public view returns (bool) {
        return
            publicSaleConfig.saleStartTime != 0 &&
            block.timestamp >= publicSaleConfig.saleStartTime;
    }

    function isWhitelistSaleOn(bool isFreemint, bool isDiscount)
        public
        view
        returns (bool)
    {
        WhitelistSaleConfig memory config = isFreemint
            ? freemintSaleConfig
            : isDiscount
            ? discountSaleConfig
            : fullpriceSaleConfig;
        return
            config.saleStartTime != 0 &&
            block.timestamp >= config.saleStartTime &&
            config.merkleRoot != "";
    }
    ...
    function addressMintType(
        address account,
        bytes32[] calldata wlFreeMintProof,
        bytes32[] calldata wlDiscountMintProof,
        bytes32[] calldata wlFullPriceMintProof
    ) external view returns (AddressMintType) {
        bool isFreeMint = isWhitelistAddress(
            true,
            false,
            account,
            wlFreeMintProof
        ...
        if (isFreeMint) {
            return AddressMintType.WL_FREE_MINT;
        }

        bool isDiscountMint = isWhitelistAddress(
            false,
            true,
            account,
            wlDiscountMintProof
        ...
        if (isDiscountMint) {
            return AddressMintType.WL_DISCOUNT_MINT;
        }

        bool isFullPriceMint = isWhitelistAddress(
            false,
            false,
            account,
            wlFullPriceMintProof
        );
        if (isFullPriceMint) {
            return AddressMintType.WL_FULL_PRICE_MINT;
        }

        return AddressMintType.PUBLIC_SALE_MINT;
    }
    ...
    function whitelistMint(
        ...
        uint256 quantity,
        bytes32[] calldata proof
    ) external payable regularCheck(quantity) nonReentrant {
        WhitelistSaleConfig memory config = isFreemint
            ? freemintSaleConfig
            : isDiscount
            ? discountSaleConfig
            : fullpriceSaleConfig;
        require(
            isWhitelistSaleOn(isFreemint, isDiscount),
            "YINJA Club: sale not started yet"
        );
        require(
            isWhitelistAddress(isFreemint, isDiscount, msg.sender, proof),
            "YINJA Club: caller is not in whitelist"
        );
        require(
            numberMinted[msg.sender] + quantity <=
                config.maxPerAddressDuringMint,
            "YINJA Club: whitelist can not mint this many"
        );
        emit WhitelistMinted(isFreemint, isDiscount, msg.sender, quantity);
    ...
    function publicSaleMint(uint256 quantity)
        external
        payable
        regularCheck(quantity)
        nonReentrant
    {
        require(isPublicSaleOn(), "YINJA Club: public sale not started yet");
        require(
            numberMinted[msg.sender] + quantity <=
                publicSaleConfig.maxPerAddressDuringMint,
            "YINJA Club: publicSale can not mint this many"
        );
        emit PublicMinted(msg.sender, quantity);
        uint256 totalCost = publicSaleConfig.price * quantity;
        numberMinted[msg.sender] += quantity;
        while (quantity > 0) {
            _safeMint(msg.sender, _nextId++);
            quantity -= 1;
    ...
    function teamMint(uint256 quantity) external nonReentrant onlyOwner {
        require(
            numberMinted[team] + quantity <= amountForTeam,
            "YINJA Club: team mint maximum"
        );

        emit TeamMinted(team, quantity);

        numberMinted[team] += quantity;
        while (quantity > 0) {
            _safeMint(team, _nextId++);
            quantity -= 1;
        }
    }

    function airdropMint(address[] memory accounts) external onlyOwner
    {
        for (uint256 idx = 0; idx < accounts.length; idx++) {
            _safeMint(accounts[idx], _nextId++);
            emit AirdropMinted(accounts[idx]);
        }
    }
    ...
    modifier regularCheck(uint256 quantity) {
        require(quantity > 0, "YINJA Club: quantity less than 0");
        require(
            !msg.sender.isContract(),
            "YINJA Club: caller cannot be another contract"
        );
        require(
            totalSupply() + quantity <= saleSupplyAmount + numberMinted[team],
            "YinJa Club: maximum sale supply"
        );
        require(
            totalSupply() + quantity <=
                maxSupplyAmount - amountForTeam + numberMinted[team],
            "YinJa Club: not enough reserve for team"
        );
        _;
    }
