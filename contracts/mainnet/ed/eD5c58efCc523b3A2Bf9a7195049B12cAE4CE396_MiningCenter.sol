...
contract MiningCenter is ERC1155Holder {
    ...
    modifier onlyPermit() {
        require(
            owner() == msg.sender || msg.sender == address(this),
    ...
    function manageStakingType(
        ...
        manageStakingTypeIntern(_stakeDays, _index, _mode);
    }
     ...
     * @param _mode 0: update 1: add 2: remove
     */
    function manageStakingTypeIntern(
        uint256 _stakeDays,
        uint256 _index,
        uint16 _mode
    ) private {
        if (_mode == 0) {
            ...
            stakingType[_index] = _stakeDays;
        } else if (_mode == 1) {
            stakingType[stakingTypeLength++] = _stakeDays;
        } else if (_mode == 2) {
            require(stakingType[_index] != 0, "_index is not valid");
    ...
    function stakingProperty(
        ...
        manageStakingTypeIntern(stakingType[_stakingType], _nftNum, 1);
    }
        ...
        if (different >= stakingHistory[stakingIndex].stakedType) {
            incomeToken.transfer(
                ...
                utility.getIncomeTokenFromBNB(
                    utility.getBnbTokenFromDollar(_earnedUSDT)
                )
            );
        } else {
            incomeToken.transfer(
                ...
                utility.getIncomeTokenFromBNB(
                    utility.getBnbTokenFromDollar(
                        (_earnedUSDT * (100 - marketingPros)) / 100
                    )
                )
            );
