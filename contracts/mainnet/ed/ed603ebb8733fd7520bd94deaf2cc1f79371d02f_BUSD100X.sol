...
contract BUSD100X {
	...
	uint[] public					REFERRAL_PERCENTS				= [5, 3, 2, 1, 1];	// 0.50% 0.30% 0.2% 0.1% 0.1%
	uint constant public			PROJECT_FEE						= 40;					// project fee 4% of deposit
	uint constant public			DEV_FEE							= 10;					// dev fee 1% of deposit
	uint constant public			MAX_WITHDRAW_AMOUNT				= 1000 ether;			// claim 1000 BUSD max
	...
	constructor(address payable _walletProject, address payable _walletDev) {
		...
		PLANS.push( TPlan(1,1100) );
		PLANS.push( TPlan(2,560) );
		PLANS.push( TPlan(3,376) );
		PLANS.push( TPlan(4,285) );
		PLANS.push( TPlan(5,230) );
		PLANS.push( TPlan(6,193) );
