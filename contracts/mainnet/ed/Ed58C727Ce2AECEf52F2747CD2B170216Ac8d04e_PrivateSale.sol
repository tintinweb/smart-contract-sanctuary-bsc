...
contract PrivateSale {
    ...
    uint256 public minInvestInWei = 1e17; // Minimum wei amount that can be invested per wallet address
    uint256 public maxInvestInWei = 2 ether; // Maximum wei amount that can be invested per wallet address
    uint256 public hardCapInWei = 100 ether; // Maximum wei amount that can be invested in presale
    ...
    function deposit() external payable whitelistedAddressOnly returns (uint256) {
        ...
        require(recipients[msg.sender].amountDepositedWei + weiAmount >= 1 ether, "Deposit: Sender can not invest less than 1 BNB");
        require(recipients[msg.sender].amountDepositedWei + weiAmount <= 2 ether, "Deposit: Sender already invested maximum amount");
