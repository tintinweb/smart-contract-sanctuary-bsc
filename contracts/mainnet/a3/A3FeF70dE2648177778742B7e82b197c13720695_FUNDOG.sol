...
Fun Dog is a token on the Binance Smart Chain that never stops growing through TRUE deflationary techniques and further income generation for its ecosystem. 

Fun Dog a Hyper Deflationary token that generates passive income to holders just by holding and receiving a percentage of auto reflections on every transaction with a primary use case developing an easy to use dapp, dashboard and nft character collection.
...
contract FUNDOG is Context, IBEP20, Ownable {
    ...
    string private _name = "Fun Dog"; // token name
    string private _symbol = "FUNDOG"; // token ticker
