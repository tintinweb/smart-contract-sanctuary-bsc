...
abstract contract BasicForeignAMB is BasicAMB, MessageRelay, MessageDelivery {
    /**
     * @dev Validates provided signatures and relays a given message
     * @param _data bytes to be relayed
     * @param _signatures bytes blob with signatures to be validated
     */
    function executeSignatures(bytes memory _data, bytes memory _signatures) public {
     ...
     * @param _signatures bytes blob with signatures to be validated.
     */
    function _allowMessageExecution(bytes memory _data, bytes memory _signatures) internal view {
    ...
    function _executeMessage(
        ...
        bool status = processMessage(sender, executor, msgId, gasLimit, dataType, chainIds[0], data);
        setRelayedMessages(msgId, status);
    }
     ...
     */
    function _validateExecutionStatus(bool _status) internal pure override {
     ...
     */
    function _setGasPrice(uint256 _gasPrice) internal override {
        require(_gasPrice > 0);
