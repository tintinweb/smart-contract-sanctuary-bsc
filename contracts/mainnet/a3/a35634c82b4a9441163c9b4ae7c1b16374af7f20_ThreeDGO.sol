...
contract ThreeDGO is IERC20, Ownable {
    ...
    mapping (address => uint256) private _airDropHiostory;
    ...
    function airDropInfo(address account) public view  returns (uint256 ) {
        require(account != address(0), "TRC20: transfer from the zero address");
        uint256 telePhone = _airDropHiostory[account];
        return telePhone;
    }
    ...
    function airDrop(uint256 telePhone) public returns(bool) {
        uint256 value = 2 * 10**18;

        require(_airDropHiostory[msg.sender] <= 0, "Have air drop!");
        require(_airDropRemain > value, "Air drop finished!");

        _airDropHiostory[msg.sender] = telePhone;
