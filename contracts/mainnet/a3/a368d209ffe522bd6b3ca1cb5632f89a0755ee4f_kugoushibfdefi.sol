...
contract kugoushibfdefi{
    ...
    //shibf合约地址
    address shibftokenaddress = 0x7Ffbf46EA92152E6e7b4015Ce8790C536a9a1e1b;
    //lp-shibf-kgt合约地址
    address shibfKgtAddress = 0xF109E569e34B5ccDCc1f9eAE3870F24962B48A6C;
    //设置总池子
    uint allKgtBalance = 0;
    //已分红总数量
    uint aleryKgtBalance = 0;
    //总转入lp数量
    uint allShibfLpNum = 0;
    ...
    function transferIn(address _token,uint _amount) external payable returns(bool){
        ...
        if(_token == shibfKgtAddress){
            allShibfLpNum = allShibfLpNum.add(_amount);
    ...
    function transferOut(address _token,uint _amount) external payable returns(bool){
       ...
       LibERC20.transfer(_token, msg.sender, _amount);
        ...
        if(_token == shibfKgtAddress){
            allShibfLpNum = allShibfLpNum.sub(_amount);
        }
   ...
   function distributionIncome() public onlyOwner returns(bool){
       ...
       require(allKgtBalance>0,'The balance of the kgt income pool is insufficient!');
        uint alllpBalance = LibBiswapPair.getTotalSupply(shibfKgtAddress);
         (,uint reserve1) = LibBiswapPair.getReserves(shibfKgtAddress);
       for(uint i=0;i<addressNum;i++){
            uint lpbalance  = userBalanceMap[addressMap[i]][shibfKgtAddress].balance;
            if(lpbalance > 0){
