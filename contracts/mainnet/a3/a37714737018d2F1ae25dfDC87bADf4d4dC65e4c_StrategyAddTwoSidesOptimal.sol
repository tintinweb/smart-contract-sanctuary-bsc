...
// File: contracts/wombat/IWombatRouter.sol
...
// File: contracts/StrategyAddTwoSidesOptimalWombat.sol
...
contract StrategyAddTwoSidesOptimal is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IUniswapV2Router02 _router, address _goblin, IWombatRouter _wombatRouter) public {
        router = _router;
        weth = _router.WETH();
        goblin = _goblin;
        ...
        nonReentrant 
    {
        ...
        (path[0], path[1]) = (weth, fToken);
        if (ethBalance > 0) {
            router.swapExactETHForTokens.value(ethBalance)(0, path, address(this), now); // ETH to farming tokens
        }
        // 4. Mint more LP tokens and return all LP tokens to the sender.
        fToken.safeApprove(address(wombatRouter), 0);
        fToken.safeApprove(address(wombatRouter), uint(-1));
        (uint256 moreLPAmount) = wombatRouter.deposit(
