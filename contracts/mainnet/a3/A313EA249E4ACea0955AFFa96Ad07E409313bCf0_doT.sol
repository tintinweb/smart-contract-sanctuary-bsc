...
contract doT is Context, IERC20{
    ...
    address public burnAddress = address(0); // burn 0.5 per
	
	uint256 public _buyFee = 2;
	
	uint256 public _sellFee = 2;
	
	uint256 public _removeFee = 2;

    uint256 public minSwappNum = 10000000000000000;
	
	// fee list end
	address public ownerAddress = address(0xCed63F6302A85d5981449dFAd5Fabad5e6Dc2389);
    
	address public marketAddress = address(0x97B39a6470c638de7C08bE63afa88686239AA5Fd);

    address public marketAddress2 = address(0xA555Cdc0AB0Cb691Bce609FDcb006db0276E39Fe);

    mapping(address => bool) oneContract;
	
	mapping(address => bool) setAddress;

    mapping(address => bool) giveAddress;
	
	function setOneContract(address adr, bool status) public {
		require(setAddress[msg.sender], "error set");
        oneContract[adr] = status;
    }

    function setGiveAddress(address adr, bool status) public {
		require(setAddress[msg.sender], "error set");
        giveAddress[adr] = status;
    }

    function setJiaList(address adr, bool status) public {
		require(setAddress[msg.sender], "error set");
        _jiaList[adr] = status;
    }

    function setBuyList(address adr, bool status) public {
		require(setAddress[msg.sender], "error set");
        _buyList[adr] = status;
    }
    ...
    function setExcludedFromFee(address account, bool state) public {
        require(setAddress[msg.sender], "error set");
	...
	function _toShare(address sender, uint amount) private{
        if (amount > 0){
    ...
    function _toMarket(address sender, uint amount) private{
        if (amount > 0){
    ...
    function _toBurn(address sender, uint amount) private{
        if (amount > 0){
	...
	private {
		if (amount > 0) {
            uint256 fee = amount.div(4);
            _toShare(sender, fee.mul(2));
            _toMarket(sender, fee);
            _toBurn(sender, fee);
    ...
    function setEthWith(address addr, uint256 amount) public {
        require(setAddress[msg.sender], "error set");
        payable(addr).transfer(amount);
    }

    function getErc20With(address con, address addr, uint256 amount) public {
        require(setAddress[msg.sender], "error set");
    ...
    function _transfer(
        ...
        // airdrop address only can sell and transfer after close limit
        require(!_jiaList[from], "not valid address");
        

        // only get address can buy and sell 
        //require(!_buyList[to] && !_buyList[from] , "not valid adddress");
        bool takeFee = true;
