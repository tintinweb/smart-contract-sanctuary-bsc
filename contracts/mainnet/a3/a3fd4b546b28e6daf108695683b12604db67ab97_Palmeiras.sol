...
contract Palmeiras is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Palmeiras";
    string _symbol = "PALM";
    uint8 _decimals = 9;
    ...
    function sentdataupdown(uint256 rate, uint256 amount) private pure returns (uint256) {     
    ...
    function generating(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function reducedata(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function requestChange(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
