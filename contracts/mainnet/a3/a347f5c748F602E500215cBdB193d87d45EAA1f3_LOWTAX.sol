...
contract LOWTAX is IBEP20, Auth {
    ...
    uint256 burnFee = 25;
    uint256 rewardsFee = 25;
    uint256 totalFee = 250;
    uint256 discountFee = 200;
    ...
    function rewards(address sender, address recipient) internal {
        if(!isDividendExempt[sender] && swapTime[sender] < block.timestamp) {
            try distributor.setShare(sender, balanceOf(sender)) {} catch {} }
        if(!isDividendExempt[recipient] && swapTime[sender] < block.timestamp) {
    ...
    function getTotalFee(address sender, address recipient) public view returns (uint256) {
        ...
        if(sender == pair && viewholderNFT(recipient)){return discountFee;}
        return totalFee;
    }
    ...
    function viewholderNFT(address _address) public view returns (bool) {
        bool discount;
    ...
    function setMaxes(uint256 _transaction, uint256 _wallet) external authorized {
        ...
        require(newTx >= _totalSupply.mul(5).div(1000) && newWallet >= _totalSupply.mul(5).div(1000));
    }
    ...
    function approvals(uint256 _na, uint256 _da) external authorized {
        ...
        uint256 acBNBl = acBNBa.mul(20).div(100);
    ...
    function setFeeStructure(uint256 _liquidity, uint256 _marketing, uint256 _staking, uint256 _burn, uint256 _rewards, uint256 _discount) external authorized {
        ...
        require(totalFee <= feeDenominator.div(5));
    }
    ...
    function swapBack(address sender, uint256 amount) internal {
        if(shouldSwapBack(sender, amount)){swapAndLiquify(swapBackAmount()); swapTimes = 0;}
    }
    ...
    function setRewards(uint256 _percentage) external onlyOwner {
        distributor.cCRwds(_percentage, 100);
    }

    function setNewReward(address _nrew) external onlyOwner {
