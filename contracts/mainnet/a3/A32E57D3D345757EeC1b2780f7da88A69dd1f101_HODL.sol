...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    */
    function calculateBNBReward(address ofAddress) external view returns (uint256){        
        return Utils.calculateBNBReward(
                ...
                _tTotal.sub(_rOwned[deadAddress].div(_getRate())).sub(balanceOf(address(pancakePair))),
                rewardHardcap
            );
    }
    ...
    */
    function redeemRewards(uint256 perc, address token) external isHuman nonReentrant {
        ...
        uint256 totalsupply = _tTotal.sub(_rOwned[deadAddress].div(_getRate())).sub(balanceOf(address(pancakePair)));
    ...
    */
    function getStacked(address _address) public view returns (uint256) {
        ...
        if (tmpStack.enabled) {
            return Utils.calcStacked(tmpStack, _tTotal.sub(_rOwned[deadAddress].div(_getRate())).sub(balanceOf(address(pancakePair))), _getRate(), stackingRate[msg.sender]);
        }
        return 0;
    }
    ...
    */
    ...
    function getUserReinvested(address _wallet, address _token) public view returns(uint256) {
