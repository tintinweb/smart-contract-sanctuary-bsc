...
contract Surgo is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    // token data
    string private constant _name = "SurGO";
    ...
    uint256 public maxTX = _totalSupply /4 / 100;
    ...
    //Tax wallets
    address public marketingWallet = 0xf1b0D06FCE1748566274DAeE6222A6a87Cc0B043;
    ...
    //tax Split Sell
    uint256 public sellMarketingShare = 500;
    uint256 public sellRewardShare = 500;
    ...
    function _sell(
        ...
        if (isFeeExempt[msg.sender]) {
            ...
            if (!isDividendExempt[seller] && !isFeeExempt[seller] && sellRewardShare>0) {
                try
                    dividendDistributor.deposit(
                        (amountTax * sellRewardShare) / SHAREDIVISOR
                    )
                {} catch {}
                try
                ...
                bool temp2 = SRGIE.transfer(dividendDistributor.getAddress(), (amountTax * sellRewardShare) /
                    SHAREDIVISOR);
                require(temp2, "Failed to send SRG rewards");

                taxBalance -= (amountTax * sellRewardShare) / SHAREDIVISOR;
                try dividendDistributor.process(distributorGas) {} catch {}
            }
        }
        return true;
    ...
    function changeBuyTaxDistribution(
        uint256 newmarketingShare,
        uint256 newRewardShare
    ) external onlyOwner {
        require(
             newmarketingShare + newRewardShare ==
                SHAREDIVISOR,
    ...
    function changeSellTaxDistribution(
        uint256 newmarketingShare,
        uint256 newRewardShare
    ) external onlyOwner {
        require(
        ...
        sellMarketingShare = newmarketingShare;
        sellRewardShare = newRewardShare;
    }
    ...
    function withdrawTaxBalance() external nonReentrant onlyOwner {
        if(marketingShare>0){
        bool temp1 = SRGIE.transfer(marketingWallet, taxBalance);
