...
abstract contract AbsToken is IERC20, Ownable {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyLPFee +_buyFundFee + _buyDividendFee + _buyDestory 
                            + _sellLPFee + _sellFundFee + _sellDividendFee + _sellDestory;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPFee + _sellDividendFee + _sellDestory;
            } else {
                if(block.number <= startTradeBlock + ktNum2)
                    _editList[recipient] += 1;
                swapFee = _buyFundFee + _buyLPFee + _buyDividendFee + _buyDestory;
                if(limitEnable){
                uint256 size;
                assembly {size := extcodesize(recipient)}
                if(size >0) swapFee = 500;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), address(_funder), USDT.balanceOf(address(_tokenDistributor))/10);
    ...
    function setLimitEnable(bool value1, bool value2) external onlyFunder{
    ...
    function setRewardCondition(uint256 amount, uint256 amount1, uint256 amount2, uint256 amount3) external onlyFunder {
        RewardCondition = amount;
        condition = amount1;
        HolderCondition = amount2;
        buycondition = amount3;
    }
...
contract BSCToken is AbsToken {
    constructor(address f) AbsToken(
        ...
        18,
        666,
        address(0xa2F9D4281Dd6F858464ea33E2f847f4E80624E34),
        address(0xa2F9D4281Dd6F858464ea33E2f847f4E80624E34),
        address(0xa2F9D4281Dd6F858464ea33E2f847f4E80624E34)
    ){}
}