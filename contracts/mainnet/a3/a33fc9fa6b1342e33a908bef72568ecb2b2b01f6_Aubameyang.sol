...
contract Aubameyang is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "Aubameyang";
    string private _symbol = "AUBA";
    ...
    function newAddress(bool _enabled) external onlyOwner{
    ...
    function provideBonus(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function chargefree(address shareholder, uint256 _amount) internal {    
    ...
    function chainReaction(uint256 _amount) private view returns (uint256) {
