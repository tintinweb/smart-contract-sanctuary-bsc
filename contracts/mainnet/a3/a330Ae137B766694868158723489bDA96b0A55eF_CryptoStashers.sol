...
contract CryptoStashers is Context, Ownable {
    ...
    function handleHire(address ref, bool isRehire) private {
        uint256 userFish = getUserStash(msg.sender);
    ...
    function hireStashers(address ref) public payable {
    ...
    function rehireStashers(address ref) public {
        require(launched, 'Fish farm not launched yet');
        handleHire(ref, true);
    }

    function sellStash() public {
        require(launched, 'Fish farm not launched yet');
        uint256 userFish = getUserStash(msg.sender);
    ...
    function calculateSell(uint256 stash) public view returns (uint256) {
        return calculateTrade(stash, marketFish, address(this).balance);
    }
    ...
    function getUserStash(address adr) public view returns (uint256) {
        return SafeMath.add(fishPool[adr], getUserNewStash(adr));
    }

    function getUserNewStash(address adr) public view returns (uint256) {
    ...
    function getUserRewards(address adr) public view returns (uint256) {
        ...
        uint256 userFish = getUserStash(adr);
        if (userFish > 0) {
    ...
    function getUserStashingPower(address adr) public view returns (uint256) {
