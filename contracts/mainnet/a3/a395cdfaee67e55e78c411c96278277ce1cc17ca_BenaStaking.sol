...
contract BenaStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(60).div(BLOCK_PER_DAY).div(uint256(365));
            if (user.block < block.number) {
                if (pending > 0) {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            return user.amount.mul(60).div(BLOCK_PER_DAY).div(uint256(365)).add(user.reward);
        } else {
            return 0;
        }
    }
