...
contract ChristmasMoon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    string  _name = "Christmas Moon";
    string  _symbol = "ChristmasMoon";
    address _marketingFeeReceiver = 0xbADBF1662C65c444D0B05aeefffFd8dD763C80A5;
    address _teamFeeReceiver = 0x839f7Bee7E801A3D7902e5C4fFFFE656d8a140be;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool isSell) private {
        uint256 feeAmount;
        if (isSell) {
            uint256 sellFundFee = tAmount * _sellFee / 100;
            if (sellFundFee > 0) {
                feeAmount += sellFundFee;
                _takeTransfer(sender, address(this), sellFundFee);
            }
        } else {
            uint256 buyFundFee = tAmount * _buyFee / 100;
            if (buyFundFee > 0) {
