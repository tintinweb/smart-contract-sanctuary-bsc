...
contract XmasROCKET is IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**9; // 1,000,000,000.000000000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public MAX_PER_WALLET = 40000000 * 10**9; // 20,000,000   
    uint256 private _tFeeTotal;

    string private _name = "XmasROCKET";
    string private _symbol = "XMROCKET";
    ...
    uint256 public _liquidityFee = 3; // Liquidity Fee + Marketing Fee + Dev Fee 
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require (maxTxAmount >= 500000, "invalid max tx amount");
        _maxTxAmount = maxTxAmount * (10**9);
    }

    function setMaxPerWallet(uint256 maxAmount) external onlyOwner {
        require (maxAmount >= 4000000, "invalid max amount per wallet");
        MAX_PER_WALLET = maxAmount * (10**9);
    }
    ...
    function updateFees(uint256 LiquidityFee, uint256 BurnFee, uint256 TaxFee, uint256 MarketingFee, uint256 DevFee) public onlyOwner {
        require(LiquidityFee + TaxFee <= 100, "Total taxes must be under 100%");
    ...
    function updateSellFee(uint256 _fee) external onlyOwner {
        require (_fee <= 100, "Sell fee must be under 100%");
        _sellFee = _fee;
