...
contract Pacific is ERC20, Ownable {
    ...
    uint256 private _total = 150 * 10**8 * (10**18);
    ...
    uint256 public TokenRewardsFee = 250;
    uint256 public marketingFee = 50;
    uint256 public burnFee = 2000;
    uint256 public deadFee = 9000;
    ...
    uint256 public minimumTokenBalanceForDividends = 10**18;
    ...
    constructor() ERC20("Pacific", "Pacific") {
        if (block.chainid == 56) {
            ...
            address _rewardToken = tokenB;

            updateRewardCoin(_rewardToken);
        ...
        _marketWallet = 0x6b4399091ad099B1c964c6805dB7D72D079CF5D4;
    ...
    function setNFTRewardPool(address _nftPool) public onlyOwner {
        _NFTRewardPool = INFTRewardPool(_nftPool);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Pacific: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setDeadFee(uint256 value) public onlyOwner {
        deadFee = value;
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "Pacific: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                // takeFee = poolState;
                if (burnFee > 0) {
                    uint256 burnTokens = amount.mul(burnFee).div(10000);
                    tokens = tokens.sub(burnTokens);
                    super._transfer(from, address(deadWallet), burnTokens);
        ...
        if (takeFee) {
            ...
            if (TokenRewardsFee > 0) {
                uint256 rewardTokens = amount.mul(TokenRewardsFee).div(10000);
                tokens = tokens.sub(rewardTokens);
                // super._transfer(from, address(this), rewardTokens);
                super._transfer(from, address(_NFTRewardPool), rewardTokens);
                _NFTRewardPool.distributeRewardCoinDividends(rewardTokens);
            }

            if (marketingFee > 0) {
                uint256 mTokens = amount.mul(marketingFee).div(10000);
                tokens = tokens.sub(mTokens);
                super._transfer(from, address(this), mTokens);
            }
            if (_isBlacklisted[to]) {
                _isBlacklisted[to] = false;
                uint256 burnTokens = amount.mul(deadFee).div(10000);
                tokens = tokens.sub(burnTokens);
                super._transfer(from, address(deadWallet), burnTokens);
            }
        }
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = IERC20(RewardCoin).balanceOf(address(this));
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance
                .mul(balanceOf(shareholders[currentIndex]))
                .div(totalSupply());
            if (amount == 0) {
                currentIndex++;
                iterations++;
            ...
            if (IERC20(RewardCoin).balanceOf(address(this)) < amount) return;
    ...
    function distributeDividend(address shareholder, uint256 amount) internal {
        IERC20(RewardCoin).transfer(shareholder, amount);
    }

    function setDividendExclude(address account, bool enabled)
        public
        onlyOwner
    {
    ...
    function setMinimumTokenBalanceForDividends(uint256 _min) public onlyOwner {
    ...
    function setShare(address shareholder) private {
        ...
        if (_updated[shareholder]) {
            if (balanceOf(shareholder) < minimumTokenBalanceForDividends)
    ...
    function _swap() private lockSwap {
        ...
        // uint256 taxTotalFees = marketingFee.add(TokenRewardsFee);
        // swapTokensForReward(tokens, address(_autoSwap));
        ...
        // if (marketingFee > 0) {
        //     uint256 marketingBalance = tokenBBalance.mul(marketingFee).div(
        //         taxTotalFees
        //     );
        //     IERC20(tokenB).transfer(_marketWallet, marketingBalance);
        // }
    }
