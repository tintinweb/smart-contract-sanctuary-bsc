...
contract JulCardV2 is OwnerConstants, SignerRole {
  ...
  function getSpendAmountToday(address userAddr) public view returns (uint256) {
    uint256 currentDate = (block.timestamp - 8 hours + 12 hours) / 1 days; // UTC -> PST time zone 12 PM
  ...
  function getBatchUserBalanceInUsd(address userAddr) public view returns (address[] memory, uint256[] memory){
    uint256[] memory assets = new uint256[](allMarkets.length);

    for (uint256 i = 0; i < allMarkets.length; i++) {
      assets[i] = getUsdAmount(allMarkets[i], usersBalances[userAddr][allMarkets[i]]);
    }
