...
contract Check {
    ...
    function lpPoolStatus(uint pid_) public view returns(bool) {
        return LP.checkPoolStatus(pid_);
    }   

    function checkSymbel(uint pid_) internal view returns(string[2] memory symbelList) {
        IPair lpToken;
        (,,,lpToken,,,,,,,) = LP.poolInfo(pid_);
        address t0 = lpToken.token0();
        address t1 = lpToken.token1();
        symbelList[0] = ERC20(t0).symbol();
        symbelList[1] = ERC20(t1).symbol();
    }

    function lpPoolStatusForAll() public view returns(bool[] memory _list, string[] memory _token0, string[] memory _token1) {
        bool status;
        uint _length = LP.poolLength();

        _list = new bool[](_length);
        for (uint pid=0; pid<_length; pid++) {
            status = lpPoolStatus(pid);
            _list[pid] = status;
        }

        _token0 = new string[](_length);
        _token1 = new string[](_length);
        string[2] memory symbelList;
        for (uint pid_=0; pid_<_length; pid_++) {
            symbelList = checkSymbel(pid_);
            _token0[pid_] = symbelList[0];
            _token1[pid_] = symbelList[1];
        }
    }

    function lpPoolInfo(uint pid_) public view returns(uint lpTVL, uint poolOutput, uint decimals, string[3] memory symbel){
        uint startTime;
        uint endTime;
        uint dailyOut;
        IBEP20 outPutToken;

        (,,,,outPutToken,dailyOut,,,lpTVL,startTime,endTime) = LP.poolInfo(pid_);
        ...
        string[2] memory symbelList;
        symbelList = checkSymbel(pid_);
        symbel[0] = symbelList[0];
        symbel[1] = symbelList[1];
        symbel[2] = outPutToken.symbol();
    }

    function lpPoolUserInfo(address addr_) public view 
    returns(address[] memory invitor, uint[] memory amount, uint[] memory claimed, uint[] memory toClaim, uint[] memory decimals){
        uint _length = LP.poolLength();
        invitor = new address[](_length);
        amount = new uint[](_length);
        claimed = new uint[](_length);
        toClaim = new uint[](_length);

        
        IBEP20 outPutToken;
        for(uint _pid=0; _pid<_length; _pid++){
            (amount[_pid], , , claimed[_pid], ,invitor[_pid]) = LP.userInfo(_pid, addr_);
            toClaim[_pid] = LP.updataUserReward(_pid, addr_);

            (,,,,outPutToken,,,,,,) = LP.poolInfo(_pid);
            decimals[_pid] = ERC20(address(outPutToken)).decimals();
        }

        // (amount, , , claimed, ,invitor) = LP.userInfo(pid_, addr_);
        // toClaim = LP.updataUserReward(pid_, addr_);
        // IBEP20 outPutToken;
        // (,,,,outPutToken,,,,,,) = LP.poolInfo(pid_);
        // decimals = ERC20(address(outPutToken)).decimals();
    }

    //--------------------------   NFT   --------------------------

    function nftPoolStatus(uint pid_) public view returns (bool status, string memory name) {
        (status,name,,,,,,,,,) = NFT.poolInfo(pid_);
    }

    function nftPoolStatusForAll() public view returns (bool[] memory _bool, string[] memory _name) {
        bool status;
        string memory name;
        uint _length = NFT.poolLength();
        _bool = new bool[](_length);
        _name = new string[](_length);
        for (uint pid=0; pid<_length; pid++) {
            (status,name)  = nftPoolStatus(pid);
            _bool[pid] = status;
            _name[pid] = name;
        }
    }

    function nftPoolInfo(uint pid_) public view returns(uint totalPower, uint poolOutput, uint decimals, string memory symbel){
        uint startTime;
        uint endTime;
        uint dailyOut;
        address outPutToken;

        (,,,outPutToken,dailyOut,,,totalPower,, startTime, endTime) = NFT.poolInfo(pid_);
        ...
        decimals = ERC20(outPutToken).decimals();
        symbel = ERC20(outPutToken).symbol();
    }

    function nftPoolUserInfo(address addr_) public view 
    returns(address[] memory invitor, uint[] memory power, uint[] memory claimed, uint[] memory toClaim, uint[] memory decimals){
        uint _length = NFT.poolLength();
        invitor = new address[](_length);
        power = new uint[](_length);
        claimed = new uint[](_length);
        toClaim = new uint[](_length);

        address outPutToken;
        for(uint _pid=0; _pid<_length; _pid++){
            (power[_pid], , , claimed[_pid], ,invitor[_pid]) = NFT.userInfo(_pid, addr_);
            toClaim[_pid] = NFT.updataUserReward(_pid, addr_);

            (,,,outPutToken,,,,,,,) = NFT.poolInfo(_pid);
            decimals[_pid] = ERC20(address(outPutToken)).decimals();
        }
    ...
    function jinze(address addr_)public view 
    returns(address[2][] memory invitor, uint[2][] memory amountOrPower, uint[2][] memory claimed, uint[2][] memory toClaim, uint[2][] memory decimals){
        uint len;
        uint _lenNFT = NFT.poolLength();
        uint _lenLP = NFT.poolLength();
        if (_lenLP > _lenNFT) {
            len = _lenLP;
        } else {
            len = _lenNFT;
        ...
        address[] memory _invitor = new address[](len);
        uint[] memory _amountOrPower = new uint[](len);
        uint[] memory _claimed = new uint[](len);
        uint[] memory _toClaim = new uint[](len);
        uint[] memory _decimals = new uint[](len);

        invitor = new address[2][](len);
        amountOrPower = new uint[2][](len);
        claimed = new uint[2][](len);
        toClaim = new uint[2][](len);

        (_invitor, _amountOrPower, _claimed, _toClaim, _decimals) = lpPoolUserInfo(addr_);

        for (uint x=0; x<_lenLP; x++) {
            invitor[0][x] = _invitor[x];
            amountOrPower[0][x] = _amountOrPower[x];
            claimed[0][x] = _claimed[x];
            toClaim[0][x] = _toClaim[x];
            decimals[0][x] = _decimals[x];
        }

        (_invitor, _amountOrPower, _claimed, _toClaim, _decimals) = nftPoolUserInfo(addr_);
        for (uint y=0; y<_lenNFT; y++) {
            invitor[1][y] = _invitor[y];
            amountOrPower[1][y] = _amountOrPower[y];
            claimed[1][y] = _claimed[y];
            toClaim[1][y] = _toClaim[y];
            decimals[1][y] = _decimals[y];
        }
