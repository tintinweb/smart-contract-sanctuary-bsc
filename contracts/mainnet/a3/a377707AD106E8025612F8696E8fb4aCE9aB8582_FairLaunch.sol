...
// FairLaunch is a smart contract for distributingLVRGD by asking user to stake the ERC20-based token.
contract FairLaunch is IFairLaunch, Ownable, ReentrancyGuard {
  ...
  struct UserInfo {
    ...
    //
    // We do some fancy math here. Basically, any point in time, the amount ofLVRGDs
  ...
  struct PoolInfo {
    ...
    uint256 lastRewardBlock; // Last block number thatLVRGDs distribution occurs.
    uint256 accLvrgdPerShare; // AccumulatedLVRGDs per share, times 1e12. See below.
    uint256 accLvrgdPerShareTilBonusEnd; // AccumatedLVRGDs per share until Bonus End.
  }

  // TheLvrgd TOKEN!
  LeveragedToken public lvrgd;
  // Dev address.
  ...
  // Block number when bonusLVRGD period ends.
  ...
  // The block number whenLVRGD mining starts.
  ...
  */
  ...
  // Update the given pool'sLVRGD allocation point. Can only be called by the owner.
  function setPool(
    uint256 _pid,
    uint256 _allocPoint,
    bool /* _withUpdate */
  ...
  */
  ...
  // View function to see pendingLVRGDs on frontend.
  function pendingLvrgd(uint256 _pid, address _user) external override view returns (uint256) {
  ...
  function updatePool(uint256 _pid) public override {
    ...
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      uint256 lvrgdBonusPortion = bonusEndBlock.sub(pool.lastRewardBlock).mul(bonusMultiplier).mul(lvrgdPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
      lvrgd.lock(devaddr, lvrgdBonusPortion.mul(bonusLockUpBps).div(100000));
      pool.accLvrgdPerShareTilBonusEnd = pool.accLvrgdPerShareTilBonusEnd.add(lvrgdBonusPortion.mul(1e12).div(lpSupply));
    }
    pool.lastRewardBlock = block.number;
  }

  // Deposit Staking tokens to FairLaunchToken forLVRGD allocation.
  ...
  // HarvestLVRGDs earn from the pool.
  ...
  function emergencyWithdraw(uint256 _pid) external nonReentrant {
    ...
    // Safe lvrgd transfer function, just in case if rounding error causes pool to not have enoughLVRGDs.
  function safeLvrgdTransfer(address _to, uint256 _amount) internal {
    uint256 lvrgdBal = lvrgd.balanceOf(address(this));
    if (_amount > lvrgdBal) {
      require(lvrgd.transfer(_to, lvrgdBal), "failed to transferLVRGD");
    } else {
      require(lvrgd.transfer(_to, _amount), "failed to transferLVRGD");
    }
  }
...
contract LeveragedToken is ERC20("LeveragedToken", "LVRGD"), Ownable {
  uint256 private constant CAP = 1000000000e18;
  uint256 private _totalLock;

  uint256 public startReleaseBlock;
  uint256 public endReleaseBlock;
  uint256 public constant MANUAL_MINT_LIMIT = 70000000e18;
  ...
  constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock) public {
    ...
    manualMint(msg.sender, 70000000e18);
  }
