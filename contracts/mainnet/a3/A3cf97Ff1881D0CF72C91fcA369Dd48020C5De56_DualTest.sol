...
contract DualTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DualTestDividendTracker public dividendTracker;
    ...
    uint256 public gasForProcessing = 1200000;
    ...
    event UpdateFirstToken(address firstToken);
    event UpdateSecondToken(address secondToken);

    constructor() ERC20("DualTest", "DTEST") {
        ...
        dividendTracker = new DualTestDividendTracker(
            payable(this),
            router,
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 1500000);
    ...
    function _transfer(
        ...
        if (to == DEAD) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + nftFee + buybackFee);
    ...
    function updateFirstToken(address firstToken) public onlyOwner {
        dividendTracker.updateFirstToken(firstToken);
        emit UpdateFirstToken(firstToken);
    }

    function updateSecondToken(address secondToken) public onlyOwner {
        require(secondToken == address(0), "cant be the zero address");
        dividendTracker.updateSecondToken(secondToken);
        emit UpdateSecondToken(secondToken);
    }    

    function getFirstPayoutToken() public view returns (address) {
        return dividendTracker.getFirstPayoutToken();
    }

    function getSecondPayoutToken() public view returns (address) {
        return dividendTracker.getSecondPayoutToken();
    }    
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyNftFee + buyBuyBackFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellNftFee + sellBuyBackFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellNftFee).add(sellBuyBackFee) +
            buyMarketingFees.add(buyNftFee).add(buyBuyBackFee);
        ...
        uint256 nftPayout = buyNftFee.add(sellNftFee) * feePortions;
        ...
        if (nftPayout > 0) {
            (successOp2, ) = address(nftWallet).call{value: nftPayout}("");
        }
        if (buybackPayout > 0) {
        ...
        emit SendDividends(
            dividends,
            marketingPayout + nftPayout + buybackPayout,
...
contract DualTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DualTest public DualTestContract;
    ...
    constructor(
        ...
        firstReward = firstToken;
        secondReward = secondToken;
        DualTestContract = DualTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DualTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DualTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DualTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DualTestContract);

            uint256 prevBalance = DualTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = DualTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DualTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address firstTokenAddress = firstReward;
            address secondTokenAddress = secondReward;
            uint256 firstHalf = _withdrawableDividend / 2;
            uint256 secondHalf = _withdrawableDividend / 2;
            bool success;
            bool successOp1;

            if(firstTokenAddress == address(0)) {
                (success, ) = user.call{
                    value: firstHalf,
                    gas: 30000
                }("");    
                ...
                path[0] = uniswapV2Router.WETH();
                path[1] = firstTokenAddress;
                secondpath[0] = uniswapV2Router.WETH();
                secondpath[1] = secondTokenAddress;
                try
                    uniswapV2Router
                        .swapExactETHForTokensSupportingFeeOnTransferTokens{
                        value: firstHalf
