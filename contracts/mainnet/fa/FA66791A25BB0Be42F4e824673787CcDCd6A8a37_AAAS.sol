...
interface IMDBP {
    ...
    function mintWithBacking(uint256 amount, address recipient) external returns (uint256);
}
...
contract AAAS is IFlashBorrower, IXFlashBorrower{
    ...
    /** MDB+ */
    address constant MDBPlus = 0x9f8BB16f49393eeA4331A39B69071759e54e16ea;
    ...
    constructor(
        ...
        approvedStables[BUSD] = true;
        approvedStables[0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d] = true;
        approvedStables[0x55d398326f99059fF775485246999027B3197955] = true;
    }
    ...
    function handleSwaps(
        ...
        if (_token0 == BUSD && _token1 == MDBPlus) {
            // Mint MDB+
            _mintMDBPlus(amount);
        } else if(_token0 == MDBPlus && _token1 == BUSD) {
            // Sell MDB+
            _sellMDBPlus(amount);
        } else if (_token0 == XUSD && approvedStables[_token1]) {
    ...
    function _sellMDBPlus(uint256 amount) internal {
        IMDBP(MDBPlus).sell(amount);
    }

    function _mintMDBPlus(uint256 amount) internal {
        IERC20(BUSD).approve(MDBPlus, amount);
        IMDBP(MDBPlus).mintWithBacking(amount, address(this));
    }
