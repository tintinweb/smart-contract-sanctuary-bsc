...
contract Bashthebear is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BATBCDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Bash The Bear", "BATBC") {
        ...
        dividendTracker = new BATBCDividendTracker(
            payable(this),
            router,
...
contract BATBCDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Bashthebear public BashthebearContract;
    ...
    constructor(
        ...
        defaultToken = token;
        BashthebearContract = Bashthebear(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Bashthebear_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Bashthebear_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Bashthebear contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BashthebearContract);

            uint256 prevBalance = BashthebearContract.balanceOf(address(this));
            ...
            {
                uint256 received = BashthebearContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BashthebearContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
