...
contract BTC90k is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        marketingAddress = 0x5427EbA7b6f01950170455d8c6e30C16D4310E50;
        developmentAddress = 0x851F32D5f260a8B892fbC110561B0AA17C522f06;
        coreteamAddress = 0x92DBb46f406b1370Ac07223b808ce6A8E238Fd41;
        ...
        /*
           ...
           *** Must buy these amounts to qualify
           Tier 1: 6%/10% (0+    $90k balance)
           Tier 2: 3%/8%  (100K+ $90k balance)
           Tier 3: 0%/6%  (400K+ $90k balance) 

           Sell Transaction Tax - 3 tiers:
           *** Must hold these amounts to qualify
           Tier 1: 6%/10% (0+    $90k balance)
           Tier 2: 3%/8%  (150K+ $90k balance)
           Tier 3: 0%/6%  (300K+ $90k balance) 
        ...
        */
        ...
        // Create a PancakeSwap (DEX) Pair For $90k 
        // This will be used to track the price of $90k & charge taxes to all pool buys/sells
        ...
        // Send 100,000,000 $90k tokens to the dev (one time only)
    ...
    function setTxSettings(uint _buyMaxTx, uint _sellMaxTx, uint _txcooldown) external onlyOwner {
        ...
        require(_txcooldown <= 0 minutes, "Cooldown should be 4 minutes or less!");
    ...
    function _transfer(
        ...
        // Buys from PancakeSwap's $90k pool
        if(from == pair) {
        ...
        // Sells from PancakeSwap's $90k pool
        else if(to == pair) {
        ...
        // Calculate taxes if this wallet is not excluded and buys/sells from $90k's PCS pair pool
        if(hasTaxes && (to == pair || from == pair)) {
            BTC90kTax memory BTC90kTaxes;
            BTC90kTaxes.forDAOAndFarming = amount.mul(daoandfarmingFee).mul(100 - taxDiscount).div(10000);
            BTC90kTaxes.forDevelopment = amount.mul(developmentFee).mul(100 - taxDiscount).div(10000);
            BTC90kTaxes.forCoreTeam = amount.mul(coreteamFee).mul(100 - taxDiscount).div(10000);
            BTC90kTaxes.forMarketing = amount.mul(marketingFee).mul(100 - taxDiscount).div(10000);
             

            // Calculate total taxes and deduct from the transfered amount
            uint totalTaxes =
                BTC90kTaxes.forDAOAndFarming
                .add(BTC90kTaxes.forDevelopment)
                .add(BTC90kTaxes.forCoreTeam)
                .add(BTC90kTaxes.forMarketing);
            amount = amount.sub(totalTaxes);

            // Pay DAO And Farming Taxes
            _balances[daoandfarmingAddress] += BTC90kTaxes.forDAOAndFarming;
            emit Transfer(from, daoandfarmingAddress, BTC90kTaxes.forDAOAndFarming);

            // Pay Development Fund Taxes
            _balances[developmentAddress] += BTC90kTaxes.forDevelopment;
            emit Transfer(from, developmentAddress, BTC90kTaxes.forDevelopment);

            // Pay Core-Team Taxes
            _balances[coreteamAddress] += BTC90kTaxes.forCoreTeam;
            emit Transfer(from, coreteamAddress, BTC90kTaxes.forCoreTeam);

            // Pay Marketing Taxes
            _balances[marketingAddress] += BTC90kTaxes.forMarketing;
            emit Transfer(from, marketingAddress, BTC90kTaxes.forMarketing);
