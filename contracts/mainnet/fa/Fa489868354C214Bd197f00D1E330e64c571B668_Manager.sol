    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint256 => bool) public hasMigrated;
    ...
    function availableRewards(uint256 id) public view returns (uint256) {
        ...
        } else if (totalClaimed + rewards > (value * maxROI) / 100) {
            rewards = (value * maxROI) / 100 - totalClaimed;
        } else if (hasMigrated[id] && value - rewards < realValue) {
            rewards = value - realValue;
        }
        return rewards;
    }
    ...
    function migrationCompound(address user) public onlyManager {
        uint256 balance = NFT_CONTRACT.balanceOf(user);
        require(balance > 0, "MIGRATIONCOMPOUND: You don't own a NFT");
        for (uint256 i = 0; i < balance; i++) {
            uint256 id = NFT_CONTRACT.tokenOfOwnerByIndex(_msgSender(), i);
            uint256 rewards_ = availableRewards(id);
            if (rewards_ > 0) {
