...
contract DAO is IERC20, Ownable {
    ...
    uint256 private _tTotal = 5162 * 10**18;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 1  * 10**17 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee == 1) BuyFee();
        if (takeFee == 2) sellFee();
 
        _transferStandard(sender, recipient, amount);

        if (takeFee == 1&&takeFee == 2&&takeFee == 0) restoreAllFee();
    }
    ...
    function _takeInviterFee(
        ...
        emit Transfer(sender, address(address(0)), tAmount.div(10000).mul(_inviterFee.sub(accurRate)));
    }
