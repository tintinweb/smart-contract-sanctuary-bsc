...
contract BambooAgent is Context, Ownable {
    ...
    //设置余额
    // function balance(address account,uint _type,uint amount) external {
    ...
    function withdraw(uint _type,uint amount) external{
        ...
        if(token!=outToken){
            ...
            path[0]=token;
            path[1]=outToken;
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(outAmount,0,path,address(this),block.timestamp);
        }else{
