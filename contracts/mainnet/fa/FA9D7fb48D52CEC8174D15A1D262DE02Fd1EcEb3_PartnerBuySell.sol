    ...
    IERC721ReceiverUpgradeable
{
    ...
    struct Sales {
        ...
        address userFeeRecipient;
        uint32 feePercentage;
        uint32 userFeePercentage;
    }
    ...
    uint32 public outerFeePercentages;
    address public outerFeeRecipient;
    ...
    function initialize(
        uint32 outerFeePercentages_,
        address outerFeeRecipient_
    )
        external
        initializer
        isPercentageInBounds(outerFeePercentages_)
    {
        require(
            outerFeeRecipient_ != address(0),
            "fee recipient must be != address(0)"
        );
        outerFeePercentages = outerFeePercentages_;
        outerFeeRecipient = outerFeeRecipient_;
        __Ownable_init();
    ...
    event BuySaleParametersUpdated(
        address nftContractAddress,
        uint256 tokenId,
        uint256 newBuyPrice,
        address newUserFeeRecipient,
        uint32 newUserFeePercentage
    );
    ...
    {
        require(
            userFeePercentage >= 0 && userFeePercentage <= 9600,
            "Error: incorrect percentage"
        );
        _setupSale(nftContractAddress, tokenId, erc20Token, buyPrice,userFeeRecipient, userFeePercentage);
    ...
    /// Updates Parameter of a sale
    /// @dev Price must be grater than 0
    /// @param nftContractAddress Contract address NFT on sale
    /// @param tokenId Identifier of the NFT
    /// @param newBuyPrice NFT The new price
    /// @param newUserFeeRecipient User new Recipient
    /// @param newUserFeePercentage User new Fee Percentatge
    ...
    {
        nftContractSales[nftContractAddress][tokenId].userFeeRecipient = newUserFeeRecipient;
        nftContractSales[nftContractAddress][tokenId].userFeePercentage = newUserFeePercentage;
        nftContractSales[nftContractAddress][tokenId].buyPrice = newBuyPrice;

        emit BuySaleParametersUpdated(nftContractAddress, tokenId, newBuyPrice, newUserFeeRecipient, newUserFeePercentage);
    }
        ...
        onlyOwner
    {
        ...
        outerFeeRecipient = newDefaultFeeRecipient;
        emit ChangedDefaultFeeRecipient(outerFeeRecipient);
    }
    ...
    function _payAndTransferNFT(
        ...
        _payFeesAndSeller(
            nftContractSale.userFeeRecipient,
            nftContractSale.userFeePercentage,
    ...
    function _payFeesAndSeller(
        address userFeeRecipient,
        uint32 userFeePercentatge,
        address nftSeller,
        uint256 buyPrice,
        address erc20Token
    ) internal {
        uint256 outerfee = _getPortionOfPush(buyPrice, outerFeePercentages);
        uint256 userFee;
        if (userFeeRecipient != address(0) && userFeePercentatge != 0){
            userFee = _getPortionOfPush(buyPrice, userFeePercentatge);
            _payout(
                userFeeRecipient,
                userFee,
                erc20Token
            ...
            _payout(outerFeeRecipient, outerfee, erc20Token);
        if ((buyPrice - outerfee - userFee) > 0){
            _payout(
                nftSeller,
                buyPrice - outerfee - userFee,
                erc20Token
            );
        }
    }
    ...
    function _resetSale(address nftContractAddress, uint256 tokenId) internal {
        ...
        nftContractSales[nftContractAddress][tokenId].userFeeRecipient = address(0);
        nftContractSales[nftContractAddress][tokenId].userFeePercentage = 0;
    }
    ...
    function _setupSale(
        ...
        nftContractSales[nftContractAddress][tokenId]
            .feePercentage = outerFeePercentages;
        nftContractSales[nftContractAddress][tokenId]
            .feeRecipient = outerFeeRecipient;
        nftContractSales[nftContractAddress][tokenId]
            .userFeePercentage = userFeePercentage;
        nftContractSales[nftContractAddress][tokenId]
            .userFeeRecipient = userFeeRecipient;
