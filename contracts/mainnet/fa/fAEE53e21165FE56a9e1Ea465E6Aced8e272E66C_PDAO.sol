...
contract PDAO is IERC20, Ownable {
    ...
    string private _name = "PDAO";
    string private _symbol = "PDAO";
    uint8 private _decimals = 18;

    mapping(address => bool) public clearList;
    ...
    uint256 public unionPct = 6;
    uint256 public burnPct = 1;
    uint256 public receivePct = 1;

    bool inSwapAndLiquify;
    uint256 public total = 287500 * 1e18;
    ...
    uint256 public minAmount = 0.01 * 1e18;
    uint256 public minBalance = 100 * 1e18;
    address private fromAddress;
    address private toAddress;
    bool public swapAndLiquifyEnabled = true;
    uint256 public numTokensSellToAddToLiquidity = 500 * 1e18;

    uint public LPlimit = 1e18;

    constructor (address _usdt, address _router, address _receive) {
        usdt = IERC20(_usdt);
        uniswapV2Router = IUniswapV2Router02(_router);
        union = address(new Node(_usdt));
        ...
        clearList[owner()] = true;
        clearList[address(this)] = true;
        clearList[union] = true;
        clearList[receiveAddress] = true;
    ...
    function setClearList(address[] memory addrs, bool flag) public onlyOwner() {
        for(uint i=0;i<addrs.length;i++){
            clearList[addrs[i]] = flag;
        }
    }
    ...
    function process1(uint256 gas) private {
        ...
        uint256 nowbanance = usdt.balanceOf(pool);
        uint256 gasUsed = 0;
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            if(usdt.balanceOf(pool) < amount ) return;
            Node(pool).transferOut(shareholders1[currentIndex1],amount);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!inSwapAndLiquify){
            if(!clearList[sender] && !clearList[recipient]){
                require(canSwap || (uniswapV2Pair != sender && uniswapV2Pair != recipient),"Can not Swap");
                uint256 toUnion = amount.mul(unionPct).div(100);
                uint256 toReceive;
                uint256 toBurn;

                _balances[address(this)] = _balances[address(this)].add(toUnion);
                emit Transfer(sender, address(this), toUnion);

                toBurn = amount.mul(burnPct).div(100);
                _balances[address(dead)] = _balances[address(dead)].add(toBurn);
                emit Transfer(sender, address(dead), toBurn);
                
                toReceive = amount.mul(receivePct).div(100);
                _balances[receiveAddress] = _balances[receiveAddress].add(toReceive);
                emit Transfer(sender, receiveAddress, toReceive);

                amount = amount.sub(toUnion).sub(toReceive).sub(toBurn);
            }
        }
    ...
    //change
    function changePct(uint _union, uint _receive, uint _burnpct) public onlyOwner{
        unionPct = _union;
        receivePct = _receive;
        burnPct = _burnpct;
    }
    ...
    function changeCanSwap(bool _state) public onlyOwner{
        canSwap = _state;
    }
