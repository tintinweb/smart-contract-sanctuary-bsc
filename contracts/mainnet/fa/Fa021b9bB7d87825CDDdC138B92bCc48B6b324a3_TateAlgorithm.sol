...
/**
    ...
    © TheTateAlgorithm. All rights reserved.

    ~ PLANS:
    plan 1 - 2.0% daily, 400% for 200 days
    plan 2 - 3.5% daily, 315% for  90 days
    plan 3 - 4.0% daily, 240% for  60 days

    Withdrawals are subject to 8% Sustainability tax.
    *sustainability tax stays in the contract.
    
    ~ PLAN REQUIREMENTS: 
    plan 1 - min. 0.01 BNB, max. of 40 BNB
...
contract TateAlgorithm {
	using SafeMath for uint256;

	/** default percentages **/
	uint256 public PROJECT_FEE = 40;
	uint256 public MKT_BUYBACK_FEE = 10;
	uint256 public REFERRAL_PERCENT = 150;
	uint256 public SUSTAINABILITY_TAX = 80;
    ...
    uint256 public REINVEST_BONUS = 45;
	...
	constructor(address payable _dev1, address payable _dev2, address payable _mkt) {
        ...
        plans.push(Plan(200, 20,  1 * 1e16, 40 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 90, 35,  2 * 1e17, 45 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 60, 40,  3 * 1e17, 50 ether, 0, 0, 0, 0, true));
	}
