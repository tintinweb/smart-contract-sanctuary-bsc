...
// File: contracts/Upgredeable-PiqsolMarketplace.sol
...
contract PiqsolMarketplace is  Initializable , ERC721HolderUpgradeable ,OwnableUpgradeable,UUPSUpgradeable {
   ...
   function initialize(
    ...
    modifier AuctionItemExists(uint256 id) {
        require(
            id < AuctionItemsForSale.length && FixedItemsForSale[id].id == id,
            "Piqsol:Could not find Item"
        );
        _;
    }
    ...
    function auctionEnd(uint256 id) public {
        ...
        if (AuctionItemsForSale[id].highestBidder == address(0)) {
            IERC721Upgradeable(AuctionItemsForSale[id].tokenAddress).safeTransferFrom(address(this),AuctionItemsForSale[id].beneficiary, AuctionItemsForSale[id].tokenId);
        } else {
