...
contract BNBTRADERBANK {
	...
	uint256[] public REFERRAL_PERCENTS 	= [700, 500, 400, 300, 100, 100, 80, 75, 50, 50];
	...
	uint256 constant public DIRECTROI = 5000;
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(150, 200));
		plans.push(Plan(120, 300));
	...
	function withdraw() public {
		...
		uint256 seedDirectAmount = getdirectseedincome(msg.sender);
		...
		totalAmount = totalAmount.add(seedDirectAmount);
	...
	function getdirectSeedIncome(address userAddress) public view returns (uint256){
	
		uint256 totaldirectSeedAmount;
		uint256 seedshare;
		
		uint256 count = getUserTotalDirect(userAddress);
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			for (uint256 i = 0; i < downline.deposits.length; i++) {
				...
				if (downline.deposits[i].start < finish) {
					...
					uint256 to = finish < block.timestamp ? finish : block.timestamp;
					//seed income
                    seedshare = share.mul(DIRECTROI).div(PERCENTS_DIVIDER);
					
					if (from < to) {
					
							totaldirectSeedAmount = totaldirectSeedAmount.add(seedshare.mul(to.sub(from)).div(TIME_STEP));	
	...
	function getdirectseedincome(address userAddress) public view returns (uint256){
	    User storage user = users[userAddress];
	    return (getdirectSeedIncome(userAddress).sub(user.withdrawnseed));
