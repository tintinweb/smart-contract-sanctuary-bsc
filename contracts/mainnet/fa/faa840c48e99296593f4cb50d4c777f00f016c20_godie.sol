...
contract godie is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "godie";
    string private _symbol = "GODIE";
	...
	function dispreLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function psetMaxWallet(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 10**_decimals;
    }


    function psetLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function psetDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
