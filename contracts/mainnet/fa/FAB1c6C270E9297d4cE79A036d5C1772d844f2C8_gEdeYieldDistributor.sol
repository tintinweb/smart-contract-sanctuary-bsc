...
contract gEdeYieldDistributor is Owned, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // Yield tracking
    uint256 public yieldPergEDEStored = 0;
    mapping(address => uint256) public userYieldPerTokenPaid;
    mapping(address => uint256) public yields;

    // gEDE tracking
    uint256 public totalgEDEParticipating = 0;
    uint256 public totalgEDESupplyStored = 0;
    mapping(address => bool) public userIsInitialized;
    mapping(address => uint256) public usergEDECheckpointed;
    mapping(address => uint256) public usergEDEEndpointCheckpointed;
    ...
    function setElp(address _elp) external onlyByOwnGov {
        elp = IElp(_elp);
    }

    function withdrawToEDEPool() external {
        elp.withdrawToEDEPool();
    }


    function fractionParticipating() external view returns (uint256) {
        return totalgEDEParticipating.mul(PRICE_PRECISION).div(totalgEDESupplyStored);
    }


    function eligibleCurrentgEDE(address account) public view returns (uint256 eligible_gEDE_bal, uint256 stored_ending_timestamp) {
        uint256 curr_gEDE_bal = gEde.balanceOf(account);

        // Stored is used to prevent abuse
        stored_ending_timestamp = usergEDEEndpointCheckpointed[account];
        ...
        if (stored_ending_timestamp != 0 && (block.timestamp >= stored_ending_timestamp)) {
            ...
            eligible_gEDE_bal = curr_gEDE_bal;
        }
    }
    ...
    function yieldPergEDE() public view returns (uint256) {
        if (totalgEDESupplyStored == 0) {
            return yieldPergEDEStored;
        } else {
            return (
            yieldPergEDEStored.add(
                lastTimeYieldApplicable()
                .sub(lastUpdateTime)
                .mul(yieldRate)
                .mul(1e9)
                .div(totalgEDESupplyStored)
            )
            );
        }
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        (uint256 eligible_current_gEDE, uint256 ending_timestamp) = eligibleCurrentgEDE(account);

        // If your gEDE is unlocked
        uint256 eligible_time_fraction = PRICE_PRECISION;
        if (eligible_current_gEDE == 0) {
        ...
        // If the amount of gEDE increased, only pay off based on the old balance
        // Otherwise, take the midpoint
        uint256 gEDE_balance_to_use;
        {
            uint256 old_gEDE_balance = usergEDECheckpointed[account];
            if (eligible_current_gEDE > old_gEDE_balance) {
                gEDE_balance_to_use = old_gEDE_balance;
            }
            else {
                gEDE_balance_to_use = ((eligible_current_gEDE).add(old_gEDE_balance)).div(2);
            }
        }

        return (
        gEDE_balance_to_use
        .mul(yieldPergEDE().sub(userYieldPerTokenPaid[account]))
    ...
    function _checkpointUser(address account) internal {
        ...
        // Get the old and the new gEDE balances
        uint256 old_gEDE_balance = usergEDECheckpointed[account];
        uint256 new_gEDE_balance = gEde.balanceOf(account);

        // Update the user's stored gEDE balance
        usergEDECheckpointed[account] = new_gEDE_balance;

        // Update the user's stored ending timestamp
        IgEDE.LockedBalance memory curr_locked_bal_pack = gEde.locked(account);
        usergEDEEndpointCheckpointed[account] = curr_locked_bal_pack.end;

        // Update the total amount participating
        if (new_gEDE_balance >= old_gEDE_balance) {
            uint256 weight_diff = new_gEDE_balance.sub(old_gEDE_balance);
            totalgEDEParticipating = totalgEDEParticipating.add(weight_diff);
        } else {
            uint256 weight_diff = old_gEDE_balance.sub(new_gEDE_balance);
            totalgEDEParticipating = totalgEDEParticipating.sub(weight_diff);
        }
    ...
    function _syncEarned(address account) internal {
        if (account != address(0)) {
            ...
            userYieldPerTokenPaid[account] = yieldPergEDEStored;
        }
    }
    ...
    function sync() public {
        // Update the total gEDE supply
        yieldPergEDEStored = yieldPergEDE();
        totalgEDESupplyStored = gEde.totalSupplyAtNow();
    ...
    function notifyRewardAmount() external {
        ...
        uint256 amount = tokenAmount.sub(this.rewardAmount()).mul(scaleBalance).div(denominator);
