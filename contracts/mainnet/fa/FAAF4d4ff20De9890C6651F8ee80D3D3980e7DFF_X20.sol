...
// SPDX-License-Identifier: MIT 
 ...
 /*   X20 - investment platform based on Binance Smart Chain blockchain smart-contract technology. Safe and legit!
 ...
 *
 *   - Basic interest rate: +0.2% every 24 hours - only for new deposits
 *   - Minimal deposit: 10 MATIC, no maximal limit
 *   - Total income: based on your tarrif plan (from 5% to 20% daily!!!) + Basic interest rate !!!
...
contract X20 {
	...
	uint256 constant public WITHDRAW_FEE = 2000; //In base point
	uint256 constant public PERCENTS_DIVIDER = 1000;
	uint256 constant public TIME_STEP = 1 days;
    uint256 public PACKAGE_PERCENT_LIMIT = 13;
	...
	constructor(address payable wallet, address payable dev_wallet, uint256 startDate) public {
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		emit FeePayed(msg.sender, projectFee.add(devFee));
	...
	function getPercent(uint8 plan) public returns (uint256) {
		if (block.timestamp > startUNIX && PACKAGE_PERCENT_LIMIT < 20) {
            PACKAGE_PERCENT_LIMIT = PACKAGE_PERCENT_LIMIT.add(PERCENT_STEP.mul(block.timestamp.sub(startUNIX)).div(TIME_STEP));
	...
	function getResult(uint8 plan, uint256 deposit) public returns (uint256 percent, uint256 profit, uint256 finish) {
    ...
    function updateDevWallet(address payable wallet) onlyAdminstrator public  {
		    devWallet = wallet;
	}

    function updateCommissionWallet(address payable wallet) onlyAdminstrator public  {
		    commissionWallet = wallet;
	}
