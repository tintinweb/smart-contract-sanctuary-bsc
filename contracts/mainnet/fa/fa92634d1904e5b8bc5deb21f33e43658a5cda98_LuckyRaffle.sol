...
contract LuckyRaffle {
    ...
    address payable private lucky;
    address payable private lp;
    ...
    function setLuckyWallet(address payable _lucky) private onlyOwner {
        require(msg.sender == Owner, "Only owner can set Wallet address");
        lucky = _lucky;
    }

    function setLPWallet(address payable _LP) private onlyOwner {
        require(msg.sender == Owner, "Only owner can set Wallet address");
        lp = _LP;
    }

    function setDevWallet(address payable _devWallet) private onlyOwner {
        require(msg.sender == Owner, "Only owner can set Wallet address");
    ...
    function resetPlayers() private onlyOwner {
...
function payWinner() public onlyOwner {
    ...
    lucky.transfer(address(this).balance * 10 / 100);
