...
contract AggregatorExecutor is Ownable {
    ...
    //=========== external functions ============//
    function swap(SwapStep[] memory _swaps, uint256 _amountIn, uint256 _amountOutMin, address payable _receiver, uint256 _deadline)
    external payable
    ensure(_deadline)
    {
        //check input eth
        ...
        uint256 _amountOut = IERC20(_swaps[_length - 1].toToken).balanceOf(address(this));
        ...
        if (_needConvert) {
            ...
            IERC20(_swaps[_length - 1].toToken).safeTransfer(_receiver, _amountOut);
        }
    }
    ...
    event TradeMining(address _user, address _pool, address _tokenIn, address _tokenOut, uint256 _amount);
}
