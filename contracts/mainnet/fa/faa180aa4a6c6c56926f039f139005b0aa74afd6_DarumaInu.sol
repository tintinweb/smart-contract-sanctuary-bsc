...
contract DarumaInu is Context, IERC20, Ownable {
    ...
    uint256 public _sellMarketingFee = _marketingFee;
    ...
    // Divisor for BNB split after selloff
    ...
    constructor () {
        ...
        marketingAddress = payable(0x731E1D8913f33A365Cd7f8550adC90f14373D165);
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if(_addressFees[from].enable) {
                ...
                // If receive account has a special fee
                if(_addressFees[to].enable) {
                    removeAllFee();
                    _reflectionFee = _addressFees[to]._reflectionFee;
                    _marketingFee = _addressFees[to]._marketingFee;
    ...
    /** @dev get values without reflection  */
    ...
    /** @dev get values with reflection  */
    ...
    /** @dev
    * rate is the reflected supply / non-reflected token supply
    * */
    ...
    /** @dev
    * returns total tokens and reflected tokens minus tokens in excluded accounts
    * */
    ...
    // add tokens to token SC for use as liquidity
    ...
    function setSellFee(uint256 sellReflectionFee, uint256 sellMarketingFee, uint256 sellLiquidityFee) external onlyOwner {
    ...
    function setMarketingDivisor(uint256 marketing) external onlyOwner {
    ...
    function setAddressFee(address _address, bool _enable, uint256 _addressReflectionFee, uint256 _addressLiquidityFee) external onlyOwner {
    ...
    function setSellAddressFee(address _address, bool _enable, uint256 _addressReflectionFee, uint256 _addressLiquidityFee) external onlyOwner {
        _addressFees[_address].enable = _enable;
