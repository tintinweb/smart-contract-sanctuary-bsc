...
contract Bullish is ReentrancyGuard, Pausable, ERC1155Holder
{
	...
	function harvest(uint256 _pool_id) public whenNotPaused nonReentrant
	{
		...
		if(user.locked_reward_amount >= 0 && block.number >= user.last_harvest_block_id + pool.harvest_interval_block_count)
		{
			...
			emit HarvestCB(address_user, _pool_id, user.locked_reward_amount);
		}
		else
	...
	// Internal Method
	//---------------------------------------------------------------
	function _collect_reward(uint256 _pool_id, address _address_user) private
	{
		PoolInfo storage _pool = pool_info[_pool_id];
		UserInfo storage _user = user_info[_pool_id][_address_user];
		RewardInfo storage _reward = reward_info[_pool.address_token_reward];

		_safe_reward_transfer(_pool, _address_user, _user.locked_reward_amount);

		_user.paid_reward_amount += _user.locked_reward_amount;
	...
	function _distribute_rewards(uint256 _pool_id, address _address_user) public
	{
		...
		uint256 final_user_reward_amount = user_boosted_staked_amount * accu_rps_e12 / 1e12;
