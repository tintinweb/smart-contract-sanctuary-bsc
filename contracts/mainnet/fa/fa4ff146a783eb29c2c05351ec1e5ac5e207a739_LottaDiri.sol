...
contract LottaDiri is ERC20, Ownable {
    ...
    IERC20 public immutable wbnb = IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);

    IERC20 public immutable buyTaxToken;
    IERC20 public immutable sellTaxToken;
   ...
   // event UpdateTransferFee(uint256 transferFee);
    ...
    constructor() ERC20("LottaDiri", "Lodiri") {

        buyTaxToken = IERC20(0x651a89fed302227d41425235F8E934502FB94C48); //ADACash
        sellTaxToken = IERC20(0x7B86f5ca09Dc00502E342b0FeF5117E1c32222Ce); //SOLCash
        ...
        maxWallet = totalTokenSupply / 200000; // 2%
  ...
  /*  function updateTransferFee(uint256 newTransferFee) public onlyOwner {
    ...
    function _transfer(
        ...
        else if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) 
        {
           ...
           isBuying = automatedMarketMakerPairs[from];
            ...
            if (BurnFee > 0) {
                burntokens = amount.mul(BurnFee) / 100;
            }
        ...
        if (isSelling) {
            _distributeSell(sellTaxToken);
        } else {
            _distributeBuy(buyTaxToken);
        }
      
    }

    function _distributeBuy(IERC20 taxtoken) internal {
        ...
        if(contractTokenBalance >= 1){
            swap_tokens_for_bnb(contractTokenBalance);
            nativeBalance = address(this).balance;
            //Keep 1% of BNB for gasfees
            distributeAmount = (nativeBalance*99)/100;
            swap_bnb_for_tokens(distributeAmount,taxtoken);


            tokenBalance = taxtoken.balanceOf(address(this));

            taxtoken.transfer(buyLottery, (tokenBalance*buyLotteryFee)/100);
            taxtoken.transfer(DEAD, (tokenBalance*buyBurnFee)/100);
        }
       
    }


     function _distributeSell(IERC20 taxtoken) internal{
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 tokenBalance;
        uint256 nativeBalance;
        uint256 distributeAmount;
        ...
        if(contractTokenBalance > 1){
            ...
            taxtoken.transfer(sellLottery, (tokenBalance*sellLotteryFee)/100);
            taxtoken.transfer(DEAD, (tokenBalance*sellBurnFee)/100);
        }
    }
    ...
    function swap_tokens_for_bnb(uint256 _amount) private {
      // generate the uniswap pair path of token -> weth
      address[] memory path = new address[](2);
      path[0] = address(this);
      path[1] = uniswapV2Router.WETH();
    ...
    function withdrawBNB(address _address) public onlyOwner {
        wbnb.transfer(_address, wbnb.balanceOf(address(this)));
    }
}