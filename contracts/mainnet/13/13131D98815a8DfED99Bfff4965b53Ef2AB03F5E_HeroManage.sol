...
contract HeroManage is IHeroManage, ReentrancyGuard, Configable {
    ...
    function calcRemainSummonCnt(HeroMetaData memory _meta) internal pure returns (uint32)
    {
        if (_meta.gen == 0) {
            return 100;
        }
    ...
    function calcSummonBurgerInner(HeroMetaData memory _meta) internal view returns (uint)
    {
    ...
    function makeNew(address _to, uint32 _token_id1, HeroMetaData memory _meta1, uint32 _token_id2, HeroMetaData memory _meta2) internal
    {
        ...
        hero721.createDescendantsTo(_to, newmeta);
