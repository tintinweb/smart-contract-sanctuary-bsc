    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint256 => address) public mapFirstOwner;
    ...
    function initialize(
        ...
        __ERC721_init("Outer Ring Clan Badge", "ORCB");
    ...
    function setFirstOwners(
        uint256[] calldata tokenIds,
        address[] calldata addresses
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            mapFirstOwner[tokenIds[i]] = addresses[i];
        }
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        mapFirstOwner[tokenId] = to;
        emit Minted(to, tokenId);
    }

    function lootBoxMint(address to, uint8 _tier)
        public
        onlyRole(MINTER_ROLE)
    {}
