...
contract FreeManager is Ownable {
    ...
    address  public rewardAddr=0x238585f63C03E2e5FbFa7C9B793313De268ef945;
    address  public teamAddr=0x70997970C51812dc3A010C7d01b50e0d17dc79C8; //address(0);
...
contract FireToken is ERC20, ERC20Detailed  {
    ...
    constructor ()  ERC20Detailed("Firk", "Firk", 18) {
    ...
    function _transfer(
        ...
        if(!launched() && to == teamAddr && amount>0) {
            launch();
        }
        ...
        if(takeFee) {
            ...
            // if(nftWhites[from]==false && nftWhites[to]==false){
                if(from == teamAddr){    
                    ...
                    if(from != owner() && to != teamAddr && block.number < launchedAt + KillNum ){
                        isXXKING[to] = true;
                    }
                ...
                if(to == teamAddr){
                    ...
                    assert(balance.sub(amount) < totalBuyAmount); 
                    ...
                    uint256 burnamount = amount.mul(330).div(1000);
                    ...
                    uint256 afterSAmount = totalBuyAmount.sub(amount);
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = address(0x55d398326f99059fF775485246999027B3197955); // rewardToken;//
