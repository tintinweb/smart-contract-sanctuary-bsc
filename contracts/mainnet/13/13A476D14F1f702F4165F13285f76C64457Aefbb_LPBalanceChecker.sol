...
contract LPBalanceChecker is Ownable {
    ...
    constructor(
        ...
        IApeFactory[] memory _factories
    ) Ownable() {
        require(_stakingContracts.length == _factories.length);
        for (uint256 i = 0; i < _stakingContracts.length; i++) {
            addStakingContract(_stakingContracts[i], _factories[i]);
        }
    }
    ...
    function getBalance(
        ...
        address token0,
        address token1
    ) public view returns (Balance[] memory balances) {
        balances = new Balance[](stakingContracts.length);
        for (uint256 i = 0; i < stakingContracts.length; i++) {
            IMasterApe stakingContract = IMasterApe(stakingContracts[i]);

            IApeFactory factory = stakingContractToFactory[
            ...
            balance.wallet = IApePair(lp).balanceOf(user);
            balance.total += balance.wallet;

            uint256 poolId = type(uint256).max;
            for (uint256 n = 0; n < stakingContract.poolLength(); n++) {
                (address lpToken, , , ) = stakingContract.poolInfo(n);
                if (lpToken == lp) {
                    poolId = n;
                    continue;
                }
    ...
    function addStakingContract(address stakingContract, IApeFactory factory)
        public
        onlyOwner
    {
        stakingContracts.push(stakingContract);
        stakingContractToFactory[stakingContract] = factory;
    }
}
