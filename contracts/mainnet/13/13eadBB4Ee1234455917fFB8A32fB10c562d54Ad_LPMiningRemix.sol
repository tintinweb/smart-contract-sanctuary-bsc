...
// File: contracts/LPMiningRemix.sol
...
contract LPMiningRemix is Ownable {
    ...
    function getLpPowerinPool(uint pid_, address lp_) public view returns(uint power) {
        power = lpPower[pid_][lp_];
    }
        ...
        if (pool.lockToTime && pool.lockTime != 0) {
            ...
            if (amount != 0){
                ...
                // out of deposit cycle
                    ...
                    if(user_slot.lastClaimTime < endTime) {
                        uint _rate = pool.dailyOut / 1 days;
                    ...
                    } else if (user_slot.lastClaimTime >= endTime) {
                        reward =0;
                    }
                }
            } else {
    ...
    function setLPRemove(uint pid_, address lpp_) public onlyAdmin {
        lpInPool[pid_][lpp_] = true;
        lpPower[pid_][lpp_] = 0;
        uint len = poolInfo[pid_].lpToken.length;
        uint index;
        for (uint i=0; i<len; i++ ) {
            if (poolInfo[pid_].lpToken[i] == lpp_) {
                index = i;
            }
        }
        if (poolInfo[pid_].lpToken[index] == lpp_){
            poolInfo[pid_].lpToken[index] = poolInfo[pid_].lpToken[len-1];
            poolInfo[pid_].lpToken.pop();
        }
