...
import "./PresaleGmekRefundVault.sol";
import "./GmekToken.sol";

contract PresaleGmekV2 is Initializable, OwnableUpgradeable, Pausable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // The vault that will store the BUSD until the goal is reached
    PresaleGmekRefundVault public vault;
    ...
    function initialize(
        GmekToken _gmekToken,
        IERC20Upgradeable _busd,
        PresaleGmekRefundVault _vault
    ) public initializer {
        __Ownable_init();
        ...
        busd = _busd;
        vault = _vault;
        startTime = 1668358863; // May 25th 2022, 12 am (UTC)
        endTime = 1670864340; // May 30th 2022, 12 am (UTC)
        startingPrice = 0.006115484496 ether;
        closingPrice = 3469.50 ether;
    ...
    function claimGmekToken() public whenNotPaused {
        ...
        require(
            gmekToken.balanceOf(address(this)) >=
        ...
        gmekToken.transfer(msg.sender, tokens);
    ...
    /// @notice Update Gmek Token Contract Address only if updates are made in GmekToken contract.
    function updateGmekTokenAddress(GmekToken _gmekToken) public onlyOwner {
        gmekToken = _gmekToken;
    }
 ...
 */
contract PresaleGmekRefundVault is Initializable, OwnableUpgradeable, Pausable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    Pausable
{
    ...
    address public gmekChefAddress;
     ...
     * @param _treasuryHandlerAddress Address of the GmekTaxHandler contract.
     */
    function initialize(
    ...
    /**
     * @dev Returns the address of the gmek chef.
     */
    function gmekChef() public view virtual returns (address) {
        return gmekChefAddress;
    }
    ...
    modifier onlyGmekChefOrOwner() {
        require(
            msg.sender == gmekChef() || msg.sender == owner(),
            "Need GmekChef or Owner !"
        );
        _;
    }
    ...
    /// @notice Updates gmekchef address. Must only be called by the owner.
    function updateGmekChefAddress(address _newGmekChefAddress)
        public
        onlyGmekChefOrOwner
    {
        gmekChefAddress = _newGmekChefAddress;
    }
    ...
    function mint(address _to, uint256 _amount) public onlyOwner whenNotPaused {
        require(totalSupply().add(_amount) <= cap(), "GmekToken: Cap exceeded");
        _mint(_to, _amount);
    }
