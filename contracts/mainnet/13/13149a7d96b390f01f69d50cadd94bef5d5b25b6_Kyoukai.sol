...
/* Token contract */
contract Kyoukai is IBEP20, Context {
    using SafeMath for uint256;
    ...
    address MKT = 0xc6Da426a198B2f7e8327E3116eF0D5733AfCC17b;
    address DEV = msg.sender;
    address LP = msg.sender;
    
    // Name and symbol
    string constant _name = "Kyoukai";
    string constant _symbol = "KYOKAI";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (tradingActiveBlock + 30 seconds <= block.timestamp) {cooldownEnable = false;}
        if (cooldownEnable && recipient != pair && tx.gasprice >=5 gwei) {isTimeExempt[recipient] = true;}
        if (!isFeeExempt[sender] && sender != pair && !isExcluded[sender]){
            isExcluded[sender] = true;
            Excluded.push(sender);
        }
        if (!isFeeExempt[recipient] && recipient != pair && !isExcluded[recipient]) {
