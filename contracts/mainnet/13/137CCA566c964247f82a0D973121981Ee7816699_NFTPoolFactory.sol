...
contract NFTPoolFactory is Ownable {
    ...
    function createNFTPool(IFarmageddonNFT nftContract, IBEP20 partnerToken) external onlyOwner {
            SmartNFTPoolRenewable pool = new SmartNFTPoolRenewable();
            _SmartPools.push(pool);
            SmartNFTPoolRenewable(pool).setupTokens(nftContract,partnerToken,0);
            SmartNFTPoolRenewable(pool).transferOwnership(Controller);
