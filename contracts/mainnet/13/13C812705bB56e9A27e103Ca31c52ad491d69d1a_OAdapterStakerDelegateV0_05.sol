...
/**
 * @title Ola's OAdapterStaker Contract V0.05
 * @notice OTokens which wrap an EIP-20 underlying and delegate to an implementation of a 'AdapterStaker'
 * @author Ola
 * versions:
 * 5.6 - Adds 'doMigrate'
 */
contract OAdadpterStakerV0_05 is CompoundingStakeableOToken, OMasterStakerStorageV0_05 {
    ...
    function sweepToken(EIP20NonStandardInterface token) external {
        require(msg.sender == admin, "OAdapterStaker::sweepToken: Only admin");
    	require(address(token) != underlying, "OAdapterStaker::sweepToken: can not sweep underlying token");
    ...
    function doMigrate(address newStakingTarget, bytes memory params) internal returns (bool) {
        uint strategyBalance = IStakerAdapterForV0_5(stakingTarget).balanceOf();
        IStakerAdapterForV0_5(stakingTarget).withdraw(strategyBalance);

        uint selfBalance = EIP20NonStandardInterface(underlying).balanceOf(address(this));

        EIP20Interface(underlying).approve(newStakingTarget, 0);
        EIP20Interface(underlying).approve(newStakingTarget, selfBalance);
        IStakerAdapterForV0_5(newStakingTarget).earn(selfBalance);
