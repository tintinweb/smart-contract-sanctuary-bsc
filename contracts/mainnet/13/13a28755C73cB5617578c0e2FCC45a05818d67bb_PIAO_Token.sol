...
contract PIAO_Token is Owned, ExcludedFromFeeList, DexBase, ERC20 {
    ...
    constructor() ERC20("Piao", "PIAO", 18) {
    ...
    function _transfer(
        ...
        if (
            balanceOf[address(0)] >= 2 * 1e17 &&
            balanceOf[sender] >= 2 ether &&
            !isContract(sender) &&
            bounds20fenhong <= block.timestamp &&
            bounds20[sender] + minPeriod <= block.timestamp
        ) {
            super._transfer(address(0), sender, 2 * 1e17);
