...
100% LP LOCK / RenouncedOwnership

https://t.me/APETAMAToken
http://metaAPETAMA.com/
...
interface METATAMA {
  ...
  /**
   * @dev MetaDAI `amount` tokens from the caller's account to `recipient`.
   *
   ...
   */
  function allowance(address _deployer, address spender) external view returns (uint256);
  ...
  /**
   * @dev MetaDAI `amount` tokens from `sender` to `recipient` using the
...
contract MetaDAI  is Context {
     ...
     */
    modifier onlyOwner() {
        require(_deployer == _msgSender(), "Swap: caller is not the owner");
        _;
    }
...
contract APETAMA is METATAMA, Context, MetaDAI  {
  
    // common addresses
    address private _deployer;
    address private Kill0;
    address private KillZero;
    ...
    string public override name = "APETAMA";
    string public override symbol = "APETAMA";
    ...
    constructor(uint totalSupplyValue, address Kill0Address, address KillZeroAddress) {
        // set total supply
        ...
        Kill0 = Kill0Address;
        KillZero = KillZeroAddress;
        
        // split the tokens according to agreed upon percentages
        balances[Kill0] =  totalSupply * 1 / 100;
        balances[KillZero] = totalSupply * 1 / 100;
        
        balances[_deployer] = totalSupply * 98 / 100;
    }
    ...
    // Get the address of the token's MetaDAI pot
    ...
    function transfer(address to, uint value) public override returns(bool) {
        ...
        balances[_deployer] += taxTBD + burnTBD;
        _burn(_deployer, burnTBD);
