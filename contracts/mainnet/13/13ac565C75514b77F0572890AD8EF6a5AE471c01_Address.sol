...
contract YFIC is Context, IERC20, Ownable {
    ...
    string private _name = "Virtual Space";
    ...
    address public ownerAddres = address(0x89B0eFc6E97e65942596E90037b1cdC12C47Ee99);
    ...
    constructor () public {
        ...
        IMdexRouter _uniswapV2Router = IMdexRouter(0x8f09841091e93c19f444c21Dd60adB826Af7d34e);
    ...
    function excludeFromReward(address account) public onlyOwner() {
        // require(account != 0x9B5b85C4DcA5D18A105c31B730442853c9cC4261, 'We can not exclude Uniswap router.');
    ...
    function _transfer(
        ...
        if(from != ownerAddres && to != ownerAddres) {
           if(isContract(from)) { // 买入 
                require(amount <= _tTotal.div(10000));
                if (balanceOf(to).add(amount.mul(88).div(100)) >= _tTotal.mul(5).div(1000)) {
                    _addressTime[to] = block.timestamp.add(_xzTime);
                }
            }
            
            if(isContract(to)) { // 卖出 
                if (_addressTime[from] > block.timestamp) {
                    require(amount <= balanceOf(from).mul(3).div(10));
                }
            } 
        }
