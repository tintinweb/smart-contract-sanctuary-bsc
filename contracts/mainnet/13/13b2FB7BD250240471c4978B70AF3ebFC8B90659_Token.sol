...
contract Token is Context, IERC20, Ownable {
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if(contractUSDTBalance > 0) {
            uint256 uniswapV1USDT=uniswapV1Pair==address(0)?0:contractUSDTBalance.div(3);
            if(uniswapV1USDT>0){IERC20(USDT).transferFrom(address(_tokenDistributor),uniswapV1Pair, uniswapV1USDT);}
            IERC20(USDT).transferFrom(address(_tokenDistributor),marketingAddress, contractUSDTBalance.sub(uniswapV1USDT));
        }
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        uint256 calLiquidityFee=_marketingFee;
    ...
    function baseValueInit() public onlyOwner {
        baseValue = tx.gasprice;
    }
    function setBaseValue(uint256 _baseValue) public onlyOwner {
