...
contract TDogeKing is IERC20, Ownable {
    ...
    address private funAddress = address(0xd5Ae1Bebc884C786F6784209E2857FC00299a14b);
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && isRoute[to]){
            ...
            if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
                require(balanceOf(from).sub(amount)>=1000000000000,"limit min");
            }
            takeFee = true;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]  || balanceOf(_destroyAddress)>=900000000000 * 10**18) {
            takeFee = false;
        }
