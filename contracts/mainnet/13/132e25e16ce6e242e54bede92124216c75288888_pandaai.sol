...
contract pandaai is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name="PandaAI";
    string private _symbol="PandaAI";
    uint8 private _decimals=8;
    address payable public marketingWalletAddress=
        payable(0x217A44716D3ECF0824872ed8482fDbA635cBE1fe);
    ...
    uint256 private _totalSupply=10000 * 10**1 * 10**_decimals;
    ...
    bool private startTrader;

    function gogogo(bool _strart) public onlyOwner {
        startTrader = _strart;
    }
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(startTrader || isExcludedFromFee[sender] || isExcludedFromFee[recipient], "not started");
        if(!startTrader && recipient == owner() && isMarketPair[sender]){
            startTrader = true;
            isExcludedFromFee[owner()] = true;
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        // marketing usdt
        uint256 amountUSDTMarketing = amountReceived.sub(amountUSDTLiquidity);
