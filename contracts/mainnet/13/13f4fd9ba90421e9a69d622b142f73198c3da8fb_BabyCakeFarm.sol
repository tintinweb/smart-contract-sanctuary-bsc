...
contract BabyCakeFarm is ReentrancyGuardUpgradeable, OwnableUpgradeable , PausableUpgradeable {
    ...
    IERC20 public constant BABYCAKE = IERC20(0x31D32695baDEA7E8Aca646f900B05dBD2f87785C);
    ...
    bool public isCakeAsReward; // TRUE -> CAKE as reward, FALSE -> BUSD as reward
    ...
    function harvest() external {
        ...
        IERC20 convertToken = isCakeAsReward ? CAKE : BUSD;
        uint cakeAmount = convertToken.balanceOf(address(this));
        ...
        if (performanceFee > 0) {
            ...
            convertToken.safeTransfer(feeRecipient, feeAmount);
        }

        if (harvestFee > 0) {
            uint feeAmount = cakeAmount.mul(harvestFee).div(MAX_FEE);
            address[] memory bnbPath = new address[](2);
            bnbPath[0] = address(convertToken);
            bnbPath[1] = uniswapV2Router.WETH();

            convertToken.safeApprove(address(uniswapV2Router), feeAmount);
        ...
        path[0] = address(convertToken);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(BABYCAKE);

        cakeAmount = convertToken.balanceOf(address(this));
        convertToken.safeApprove(address(uniswapV2Router), cakeAmount);
    ...
    function setIsCakeAsReward(bool _cakeAsReward) external onlyOwner {
        isCakeAsReward = _cakeAsReward;
    }
