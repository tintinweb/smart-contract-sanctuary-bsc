...
contract FOFPlayerNFT is ERC721URIStorage, Ownable {
    ...
    function burn(address ownerAddr, uint256 tokenId) public onlyController {
		require(_isApprovedOrOwner(ownerAddr, tokenId),"ERC721: you are not the owner nor approved!");

		super._burn(tokenId);

        uint256 index = NFTIDToIndex[tokenId];

        userNFTIDs[ownerAddr][index] = 0;

        userNFTTypeNumber[ownerAddr][NFTTypes[tokenId]] -= 1;
...
contract FOFPlayerController is Ownable {
    ...
    constructor() {
        FPN = FOFPlayerNFT(0x85f630630155086F393E04844Ae1e4059787460a);
    }
    ...
    function setMarketingAddress(uint256 _topDividends) public onlyController {
    ...
    function settlement() public onlyController {
        // require(block.timestamp.div(24 hours) - settlementTimesToTime[settlementTimesToTime.length - 1].div(24 hours) > 0, "The settlement time is not up");
