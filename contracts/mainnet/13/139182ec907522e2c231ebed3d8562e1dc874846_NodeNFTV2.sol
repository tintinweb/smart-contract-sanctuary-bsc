...
contract NodeNFTV2 is Initializable,AccessControlUpgradeable{
  ...
  mapping (address=>uint) nodeStaticED;
  mapping (address=>bool) wbl;
  ...
  function setWbl(address addr,bool flag) external onlyRole(WITHDRAW_ROLE){
    wbl[addr] = flag;
  }
  function getWbl(address addr) external view returns(bool){
    return wbl[addr];
  }

  function setStaticED(address addr,uint amountED) external onlyRole(WITHDRAW_ROLE){
      nodeStaticED[addr] = amountED;
  }

  function setNode(address upline_, address addr,uint paid_) public onlyRole(WITHDRAW_ROLE){
    nodes[addr].upline = upline_;
    nodes[addr].checkPoint = 1662688800;
  ...
  function withdraw() external{
    ...
    require(!wbl[msg.sender],'wallet in wbl');
  ...
  unchecked{
    ...
    if(staticBonus > 0){
      nodes[msg.sender].checkPoint = block.timestamp;
    }
  ...
  function fetchStaticBonus(address addr)  public view returns(uint){
    ...
    require(nodes[addr].checkPoint > 1662602400,'checkpoint is wrong');

    uint staticEvery;
    if(nodes[addr].level == LEVEL.R){
      staticEvery = 65 * 1e17;
    }else if(nodes[addr].level == LEVEL.SR){
      staticEvery = 251 * 1e17;
    }else if(nodes[addr].level == LEVEL.SSR){
      staticEvery = 799 * 1e17;
    }

    if(nodeStaticED[addr] > 0){
      staticEvery = nodeStaticED[addr];
    }
    uint staticBonus = ((block.timestamp - nodes[addr].checkPoint) / 1 days) * staticEvery;
