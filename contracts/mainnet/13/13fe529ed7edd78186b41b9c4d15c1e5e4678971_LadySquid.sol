...
contract LadySquid is Context, IERC20, Ownable {
    ...
    string private constant _name = "LadySquid";
    string private constant _symbol = "LADYSQUID";
    ...
    uint256 public _sellLiquidityFeeExtra = 1000;
    ...
    event UpdatedMaxTransactionAmount(uint256 newAmount);
    ...
    constructor() payable {
        ...
        _rOwned[address(0xdead)] = _rTotal / 100 * 50; // automatically send token to dead wallet
        
        maxTransactionAmount = _tTotal * 1 / 100;
        maxWallet = _tTotal * 2 / 100;
        minimumTokensBeforeSwap = _tTotal * 50 / 100000;
        
        operationsAddress = payable(0xa7Ab8bdA5E429aeD758ba6DE5D24A601563896BC);
        devAddress = payable(0x55a320E6B78F3b2c5A6fc319eb1b601cd0942764);
        liquidityAddress = payable(0x13fe529Ed7EDd78186b41b9c4d15C1E5e4678971); //LockerContract
        ...
        emit Transfer(address(0), address(this), _tTotal / 100 * 50);       // sends Token directly to the contract
  //    emit Transfer(address(0), address(msg.sender), _tTotal/100*20); // sends Token to deployer wallet
  //      emit Transfer(address(0), address(), _tTotal / 100 * 20); // sends Token to token locker
        emit Transfer(address(0), address(0xdead), _tTotal / 100 * 50); // sends Token to dead wallet
    }
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 1 / 100), "Cannot set maxTransactionAmount lower than 0.5%");
        maxTransactionAmount = newNum * (1e18);
    }

    function updateMaxWallet(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 2 / 100), "Cannot set maxTransactionAmount lower than 1%");
    ...
    function updateMaxTransactionAmount (uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 1 / 100)/1e18, "Cannot set max tx amount lower than 1%");
        maxTransactionAmount = newNum * (1e18);
        emit UpdatedMaxTransactionAmount(maxTransactionAmount);
    }

    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (_tTotal * 2 / 100)/1e18, "Cannot set max wallet amount lower than 2%");
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellOperationsFee + _sellDevFee <= 3000, "Must keep sell taxes below 25%");
        ...
        onlyOwner
    {
        ...
        require(_sellTaxFeeExtra + _sellLiquidityFeeExtra + _sellOperationsFeeExtra + _sellDevFeeExtra <= 3200, "Must keep sell taxes below 25%");
