...
contract Multistake_OPB {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    address private tokenAddr = 0x427cC5A120B97AC348922C5449CE386C6d9E87C0; // OPB
    IERC20 public token;

    uint256 public constant INVEST_MIN_AMOUNT = 1000 ether; // 1000 OPB
    uint256[] public REFERRAL_PERCENTS = [50, 20, 10];
    uint256 public constant TOTAL_REF = 80;
    uint256 public constant FUND_FEE = 180;
    ...
    function FeePayout(uint256 msgValue) internal {
        ...
        emit FeePayed(msg.sender, fFee.add(dFee));
    }
    ...
    function invest(
        ...
        if (user.referrer != address(0)) {
            ...
            if (refsamount > 0) {
                ...
                token.safeTransfer(devWallet, refsamount.div(2));
                token.safeTransfer(fundWallet, refsamount.div(2));
               
            }
        } else {
            uint256 refAmount = amount.mul(TOTAL_REF).div(PERCENTS_DIVIDER);
            token.safeTransfer(fundWallet, refAmount.div(2));
            token.safeTransfer(devWallet, refAmount.div(2));
            totalReferral = totalReferral.add(refAmount);
        }
