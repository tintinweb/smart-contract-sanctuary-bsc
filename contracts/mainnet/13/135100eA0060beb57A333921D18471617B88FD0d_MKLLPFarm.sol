...
contract MKLLPFarm is Ownable {
    ...
    IBEP20 public mklLpToken = IBEP20(0x37Dff02489cE07bf68CD9f6Ad2Aaf39d704413d9);
    uint256 public FEE = 0.002 ether;
    uint256 public totalAmount;

    event Stake(address indexed user, uint256 amount);
    event CancelStake(address indexed user, uint256 amount);
    ...
    function stake(uint256 _amount) public {
        ...
        mklLpToken.transferFrom(address(msg.sender), address(this), _amount);
    ...
    function cancelStake() public {
        ...
        require( amount > 0, 'no stake amount');

        users[msg.sender].amount = 0;
        totalAmount = totalAmount.sub(amount);
        mklLpToken.transfer(msg.sender, amount);

        emit CancelStake(msg.sender, amount);
    }

    function reward(address _addr, uint256 _amount) public onlyOwner {
        require(_amount > 0, 'no amount');
        require(mklToken.balanceOf(address(this)) >= _amount, 'no balance');
        mklToken.transfer(_addr, _amount);
