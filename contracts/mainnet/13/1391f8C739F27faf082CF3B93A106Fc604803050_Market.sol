...
contract Market is ReentrancyGuard, Ownable, Pausable {
    ...
    address private constant SAFEMARS_TOKEN = address(0x3aD9594151886Ce8538C1ff615EFa2385a8C3A88);
    ...
    event EvUpdatedCollection(uint256 _id, address _addrs, string _name, string _image);
    event EvCollectionAdded(address _addrs, string _name, string _image);
    ...
    mapping(address => bool) public blacklistCollections;
    ...
    function addCollection(address _addrs, string memory _name, string memory _image) external {
        require(addedCollections[_addrs] == false, 'MKT: already added collection');
        require(blacklistCollections[_addrs] == false, 'MKT: blacklist collection');
        collections.push(
            Collection({
                addrs: _addrs,
                name: _name,
        ...
        addedCollections[_addrs] = true;
        emit EvCollectionAdded(_addrs, _name, _image);
    }
    

    function updateCollection(uint256 _id, address _addrs, string memory _name, string memory _image) external {
        require(_id < collections.length, 'id does not exist');
        require(addedCollections[_addrs] == true, 'MKT: not existing collection');
        require(blacklistCollections[_addrs] == false, 'MKT: blacklist collection');
        Collection storage _collection = collections[_id];
        _collection.addrs = _addrs;
        _collection.name = _name;
        _collection.image = _image;
        emit EvUpdatedCollection(_id, _addrs, _name, _image);
    }

    function removeCollection(address _addrs) external onlyOwner {
        require(addedCollections[_addrs] == true, 'MKT: not existing collection');
        uint256 index;
        for (uint256 i = 0; i < collections.length; i++) {
            if (collections[i].addrs == _addrs) {
                index = i;
                break;
            }
        }
        ...
        collections.pop();
        blacklistCollections[_addrs] = true;
    }

    function allowCollection(address _addrs) external onlyOwner {
        require(blacklistCollections[_addrs] == true, 'MKT: no blacklist collection');
        blacklistCollections[_addrs] = false;
    }
    ...
    function _distributePayment(uint256 totalAmount, address seller) internal {
        uint256 fee = (totalAmount * feeRate) / 100;
