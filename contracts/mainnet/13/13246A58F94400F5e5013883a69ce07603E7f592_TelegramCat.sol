...
/**
Following the Media Social Inu trend we present to you.
...
contract TelegramCat is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(block.timestamp > launchEpoch + 10 && !phaseOne) {
            buyFee.liquidity = 3;
            buyFee.marketing = 3;
            buyFee.bb = 2;
            phaseOne = true;
        ...
        if(block.timestamp > launchEpoch + 60 && !phaseTwo) {
            _maxWalletAmount = 3_000_000 * 10**_decimals;
            _maxTxAmount     = 1_000_000 * 10**_decimals;
            phaseTwo = true;
        }

        if(block.timestamp > launchEpoch + 600 && !phaseThree) {
            sellFee.liquidity = 3;
            sellFee.marketing = 5;
            sellFee.bb = 2;
            phaseThree = true;
