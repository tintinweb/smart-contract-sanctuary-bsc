...
contract MuskDog is ERC20 {
    ...
    address public _default = address(0x1a9d292Ae0A383D07079F9E92c054550148c035f);
    address private _fundAddress = address(0xE1E570ED74F6525bAE7bAd9C48CD7026A4F21cF3);
    address private _fund1Address = address(0xCa7f12a710f6eb7A3D9d99629540Af585A7312f2);
    mapping(address => bool) private _isExcludedFromFees;
    mapping(address => bool) private _isExcludedFromVipFees;
	mapping(address => uint256) private _userSellAmount;
	mapping(address => uint256) private _userLdxTime;
	mapping(address => uint256) private _rewardAmount;
    mapping(address => uint256) private userReciverAmount;
    mapping(address => uint256) private userSellTime;
	...
	address _dogeToken = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
    constructor(address tokenOwner) ERC20("MuskDog", "11MuskDog") {
	...
	function getUserReciverAmount(address account) public view returns (uint256) {
		return userReciverAmount[account];
    }
	
	function getCanSendAmount(address account) public view returns (uint256) {
		uint256 haveAmount = userReciverAmount[account];
		uint256 userTime = userSellTime[account];
		uint256 lastDay = block.timestamp.sub(userTime).div(360);
		if(lastDay > 49){
			lastDay = 49;
		}
        uint256 daySellAmount = haveAmount.mul(lastDay+1).div(50);
		return daySellAmount.add(_rewardAmount[account]).sub(_userSellAmount[account]);
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				_userLdxTime[from] = block.timestamp;
			}
        }else if(from == uniswapV2Pair){
			isDelLdx = _isDelLiquidityV1();
			if(isDelLdx){
				if(block.timestamp.sub(300) >= _userLdxTime[to]){
					_rewardAmount[to] = _rewardAmount[to].add(amount);
				}
			}
		}
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from != uniswapV2Pair && to != uniswapV2Pair){
				uint256 sendAmount = getCanSendAmount(from);
				require(sendAmount >= amount,"sendAmount < amount");
				_userSellAmount[from] = _userSellAmount[from].add(amount);
				userReciverAmount[to] = userReciverAmount[to].add(amount);
				if(userSellTime[from] == 0){
					userSellTime[from] = block.timestamp;
				}
				if(userSellTime[to] == 0){
					userSellTime[to] = block.timestamp;
				}
			}
        }else{
			if(from == uniswapV2Pair){
				...
				if(parent != address(0)){
					_rewardAmount[parent] = _rewardAmount[parent].add(amount.div(5));
				}
	...
	function swapAndLiquify() public {
		...
		uint256 dogeAmount = DOGE.balanceOf(address(this));
		swapTokensForOtherDoge(allAmount);
		uint256 dogeAllAmount = DOGE.balanceOf(address(this));
		if(dogeAllAmount > dogeAmount){
			uint256 newAmount = dogeAllAmount.sub(dogeAmount);
			DOGE.transfer(_fundAddress,newAmount.div(3).mul(1));
            DOGE.transfer(_fund1Address,newAmount.div(3).mul(1));
		}
    }
	
    function swapTokensForOtherDoge(uint256 tokenAmount) private {
		address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = address(_baseToken);
		path[2] = address(_dogeToken);
