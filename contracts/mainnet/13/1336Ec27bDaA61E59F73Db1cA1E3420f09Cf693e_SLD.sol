...
abstract contract AbsToken is IERC20, Ownable {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyBurnFee+ _buyLPDividendFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellBurnFee;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 swapFee;
            if (isSell) {
                swapFee = _sellFundFee + _sellLPDividendFee + _sellLPFee + _sellBurnFee;
            } else {
                swapFee = _buyFundFee + _buyLPDividendFee + _buyLPFee + _buyBurnFee;
            }
    ...
    event CustomLog(uint256 _fistBalance,uint256 _lpFist,uint256 _fundAmount,uint256 _lpFee,uint256 _swapFee,uint256 _tokenAmount ,uint256 _lpAmount ,uint256 _burnAmount);

    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        swapFee += swapFee; // 24
        uint256 lpFee = _sellLPFee + _buyLPFee; // 4
        // 整体都x2 , 这里是lp的一半,所以不用x2
        uint256 lpAmount = tokenAmount * lpFee / swapFee; // 留下4/24

        uint256 burnFee = _sellBurnFee + _buyBurnFee; // 4
        uint256 burnAmount = tokenAmount * burnFee * 2 / swapFee; // 4/12
        ...
        uint256 fistBalance = FIST.balanceOf(address(_tokenDistributor)); // 卖出的到的usdt
        // 1/6 要用来加池子的数量
        uint256 lpFist = fistBalance * lpFee / swapFee;
        // 其余都是营销钱包
        // 要发到营销钱包的数量
        uint256 fundAmount = fistBalance - lpFist;
...
contract SLD is AbsToken {
    constructor() AbsToken(
          address(0x10ED43C718714eb63d5aA57B78B54704E256024E), //Router
            ...
            "SLD", //SYMBOL
            18, //DECIMALS
            138880000, //totalSupply
            address(0x671A04b64cbe07da44cfc3Bee9bc341f95818fff), //fund address
            msg.sender //receiver address
    ){
