...
contract MrGreenNFT is ERC165, IERC721Metadata, IERC721Enumerable {
    ...
    function mintNFT() external payable{
        require(msg.value >= mintPrice(), "Please pay mint price");
...
////////////////////// making sure MrGreenCrypto.com is taken care of in the future /////////////////////////////
    ...
    function z_withdrawEverythingBecauseNobodyUsesTheUtilities() external onlyOwner{
        (uint256 busdValue, uint256 bnbValue) = valueOfPortfolio();
        require(busdValue < 500 ether || bnbValue < 2 ether, "Only give up if the funds are running too low to continue");
    ...
    function valueOfPortfolio() public view returns (uint256, uint256) {
        ...
        for(uint256 i = 0; i < niceProjects.length; i++){
            if(balancesOfNiceProjects[i]>0){
                ...
                bnbValue += router.getAmountsOut(balancesOfNiceProjects[i], path)[1];
                busdValue += router.getAmountsOut(balancesOfNiceProjects[i], path)[2];
            }
        }
        return (busdValue / 1 ether, bnbValue * 100 / 1 ether);
    }
