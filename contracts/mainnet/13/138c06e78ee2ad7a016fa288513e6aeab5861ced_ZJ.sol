...
contract ZJ is ERC20 {
	...
	address private _marktAddress = address(0xa363715042193D589FEf1b0F4c0C72ebd7da3dbc);
    ...
    constructor(address tokenOwner,address root,address fundAddress) ERC20("ZJ", "ZJ") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || _destroyAmount >= 10**23) {
            takeFee = false;
            if(to != uniswapV2Pair){
                require(balanceOf(to).add(amount) <= 500 * 10**18,"max 500");
            }
        }else{
        ...
        if (takeFee) {
			...
			super._transfer(from, _marktAddress, amount.div(25));
    ...
    function _splitOtherToken() public {
        ...
        if(thisAmount >= 10**12){
            USDT.transfer(_fundAddress,thisAmount);
        }
    }
}