...
contract Gamezland is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    GamezlandDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x73517B582186f03D069cF35460C17cFEA403Ec22);
    address payable public _buybackWalletAddress = payable(0x53c007a1e182974094fa7826a129D3d68BF875E4);
    ...
    constructor() BEP20("Gamezland", "GAME") 
    {
        dividendTracker = new GamezlandDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Gamezland: The dividend tracker already has that address");
        GamezlandDividendTracker newDividendTracker = GamezlandDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Gamezland: The new dividend tracker must be owned by the Gamezland token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Gamezland: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Gamezland: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Gamezland: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Gamezland: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Gamezland: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Gamezland: Cannot update gasForProcessing to same value");
...
contract GamezlandDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Gamezland_Dividend_Tracker", "Gamezland_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Gamezland_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Gamezland_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Gamezland contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Gamezland_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Gamezland_Dividend_Tracker: Cannot update claimWait to same value");
