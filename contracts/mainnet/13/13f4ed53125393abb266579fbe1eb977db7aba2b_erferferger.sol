...
contract erferferger {
    IBEP20 token;
    using SafeMath for uint256;
    address bst = 0x5F3DF2FED24e8eC0885C6655cD9fDc759D9e2142;
    ...
    constructor(address payable wallet) public {
        ...
        OwnerAddress = wallet;
        plans.push(Plan(730, 10 wei, 63072000));
        plans.push(Plan(1095, 15 wei, 94608000));
        plans.push(Plan(2, 20 wei, 157680000));
  }

     function Reward_Staking_0 (uint256 vakue_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(vakue_percent >= 3 wei && vakue_percent <= 10 wei);
         plans[0].percent = vakue_percent; 
     }

     function Reward_Staking_1 (uint256 vakue_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(vakue_percent >= 4 wei && vakue_percent <= 15 wei);
         plans[1].percent = vakue_percent; 
     }

     function Reward_Staking_2 (uint256 vakue_percent ) external {
        require(msg.sender == OwnerAddress, "Admin use only.");
         require(vakue_percent >= 6 wei && vakue_percent <= 20 wei);
         plans[2].percent = vakue_percent; 
     }
