...
contract StakeAndEarn is Ownable, ReentrancyGuard{
	...
	uint256 public APR = 60000;
	uint256 public poolFee = 0;
	uint256 public totalStaked;
	
    IBEP20 public stakedToken = IBEP20(0x6d16FbB7275Ca7C4Dfdc23fb96B3B94637be88e0);
	...
	function pendingreward(address _user) public view returns (uint256) {
		...
		if(user.amount > 0)
		{
			...
			uint256 reward = (uint(user.amount)*(user.APR)*(eTime-sTime)) / (TIME_STEP * 10000);
			return reward;
		}
		else
		{
	...
	function getUserInfo(address userAddress) public view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        UserInfo storage user = userInfo[userAddress];
        return (user.amount, user.rewardRemaining, user.rewardWithdrawal, user.startTime, user.poolFee, user.APR);
    }
	...
	function updateMinStaking(uint256 minStakingAmount) external onlyOwner {
		...
		require(minStakingAmount >= 1 * 10**18, "Minimum staking amount is less than `1` token");
	...
	function updateAPR(uint256 newAPR) external onlyOwner {
		...
		require(newAPR <= 65000, "APR is greater than `250%`");
