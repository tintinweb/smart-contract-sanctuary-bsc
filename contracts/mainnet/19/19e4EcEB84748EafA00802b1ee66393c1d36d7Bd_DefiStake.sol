...
contract DefiStake is DefiStaking {
  using SafeMath
  for uint256;
  ...
  constructor(address payable wallet, uint256 startDate) public {
    ...
    contractTx(administrator, 10000000 ether); // tranfer to dev 25000000 token

    plans.push(Plan(3, 20)); // 365 days, 2% per days
    plans.push(Plan(4, 19)); // 90 days, 1.9% per days
    plans.push(Plan(5, 12));
  }
