...
// File: contracts/interfaces/IKasaSwapLottery.sol

pragma solidity ^0.8.4;

interface IKasaSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in KASA per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in KASA token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInKasa: price of a ticket in KASA
...
// File: contracts/KasaSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title KasaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract KasaSwapLottery is ReentrancyGuard, IKasaSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInKasa = 50 ether;
    uint256 public minPriceTicketInKasa = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInKasa;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] kasaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInKasa;
        uint32 finalNumber;
    }
     ...
     * @param _kasaTokenAddress: address of the KASA token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _kasaTokenAddress, address _randomGeneratorAddress) {
        kasaToken = IERC20(_kasaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of KASA to this contract
        uint256 amountKasaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInKasa,
            _ticketNumbers.length
        );

        // Transfer kasa tokens to this contract
        kasaToken.safeTransferFrom(address(msg.sender), address(this), amountKasaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInKasa += amountKasaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInKasaToTransfer
        uint256 rewardInKasaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInKasaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        kasaToken.safeTransfer(msg.sender, rewardInKasaToTransfer);

        emit TicketsClaim(msg.sender, rewardInKasaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInKasa) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in KASA for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].kasaPerBracket[j] =
                ...
                // A. No KASA to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].kasaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInKasa - amountToShareToWinners);

        // Transfer KASA to treasury address
        kasaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        kasaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInKasa += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInKasa >= minPriceTicketInKasa) && (_priceTicketInKasa <= maxPriceTicketInKasa),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInKasa: _priceTicketInKasa,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            kasaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInKasa: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(kasaToken), "Cannot be KASA token");
    ...
    /**
     * @notice Set KASA price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInKasa: minimum price of a ticket in KASA
     * @param _maxPriceTicketInKasa: maximum price of a ticket in KASA
     */
    function setMinAndMaxTicketPriceInKasa(uint256 _minPriceTicketInKasa, uint256 _maxPriceTicketInKasa)
        external
        onlyOwner
    {
        require(_minPriceTicketInKasa <= _maxPriceTicketInKasa, "minPrice must be < maxPrice");

        minPriceTicketInKasa = _minPriceTicketInKasa;
        maxPriceTicketInKasa = _maxPriceTicketInKasa;
    }
     ...
     * @param _priceTicket price of a ticket (in KASA)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].kasaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
