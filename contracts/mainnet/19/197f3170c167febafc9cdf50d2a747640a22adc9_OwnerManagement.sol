...
contract OwnerManagement is Ownable {
    ...
    mapping(string => mapping(address => bool)) private addressWhitelisted;
    ...
    mapping(address => mapping(string => address)) private ownerOfContract;
    ...
    function whitelist(address _address, string memory _name) public onlyOwner {
        require(
            addressWhitelisted[_name][_address] == false,
            "Error: Address has been whitelisted"
        );
        addressWhitelisted[_name][_address] = true;
    ...
    function deploy(
        ...
        _addInstitution();
        addressWhitelisted[_name][msg.sender] = false;
        ownerOfContract[msg.sender][_name] = address(instance);
    ...
    function setOwnerOfContract(
        ...
        require(
            ownerOfContract[_oldOwner][_name] == msg.sender,
            "Error: Contract must exist and Caller must be the owner"
        );
        ownerOfContract[_oldOwner][_name] = address(0);
        ownerOfContract[_newOwner][_name] = msg.sender;
    }
    ...
    function ownerChecked(address _contractOwner, string memory _name)
        ...
        returns (bool _check)
    {
        _check = (ownerOfContract[_contractOwner][_name] != address(0));
    }
    ...
    {
        require(
            ownerOfContract[_contractOwner][_name] != address(0),
            "Error: Contract does not exist"
        );
        return ownerOfContract[_contractOwner][_name];
    }
    ...
    modifier onlyWhitelisted(string memory _name) {
        require(
            addressWhitelisted[_name][msg.sender],
