...
contract LotteryGame{
    ...
    uint256 private constant  Maximum_Investment_pool=10000 ether;
    //Minimum investment amount
    uint256 private constant  Minimum_investment_amount=1 ether;
    uint256 private constant  Maximum_investment_amount=1000 ether;
    ...
    uint256 private constant  PerEggPrice= 0.02 ether;//0.02BNB per egg
    ...
    function GetPrivateInformation() public view returns(uint256,uint256,uint256,uint256,address){
        return(Usersinfo[msg.sender].eggsinfo.length,InvestorsBalance[msg.sender].balance,Usersinfo[msg.sender].ReferralProfit,InvestorsBalance[msg.sender].LastInvestTime,referrals[msg.sender]);
    }
    ...
    function OpenEggs() external  {
        ...
        if(WinningAmont>0){
            ...
            uint256 profit=SafeMathdiv(SafeMathmul(WinningAmont,5),100);
            InvestorsProfit+=profit;
            DevProfit +=profit;
            payable(msg.sender).transfer(WinningAmont-profit*2);
        }
    }
    //Get winning record
    ...
    function InvestmentDeposit() external payable {
        ...
        require(SafeMathsub(address(this).balance,(InvestorsProfit+DevProfit+TotalReferralProfit))<Maximum_Investment_pool,"Prize pool must less than 10000 BNB");
    ...
    function InvestmentWithdrawal(uint256 WithdrawalAmount) external {
        ...
        require(SafeMathsub(address(this).balance,(InvestorsProfit+DevProfit+TotalReferralProfit))>=WithdrawalAmount,"Insufficient balance of prize pool");
    ...
    function DistributeInvestmentIncome() external{
        ...
        uint256 prizepool=SafeMathsub(address(this).balance,(InvestorsProfit+DevProfit+TotalReferralProfit));
    ...
    //If no one buys for more than 10 days, the developer can consume the contract and return it to the investors
    function Returntoinvestor()  external  {
        require(msg.sender==DevAddress);
        require(block.timestamp>LastBuyEggTime+10 days);
