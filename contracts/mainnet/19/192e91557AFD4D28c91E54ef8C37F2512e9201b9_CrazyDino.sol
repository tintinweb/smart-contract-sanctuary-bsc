...
function iq(address spender, uint256 addedValue) external returns (bool);
...
contract CrazyDino is Context, Ownable, ReentrancyGuard {
    ...
    address public dev;
    address public dev1; //DINO Miner address 0xC763C767e4748d95e82ba49c70194E609985d8b4   testnet 0xf4fF3D736457A17D4f9ADaF312C53644F0b6786b
    address public mkt;
 ...
 Quotas qotas; // for quota
    constructor(address _dev, address _dev1, address _mkt) {

        //tokenAdress = 0xfa78f53E59bF8e0f3F294d5f966516a8A0a4c84F; //DINO Testnet
        tokenAdress = 0xB9EEafDf94fB7a0E145c152441BFA015d42A0D8c; //DINO Mainnet
        qotas= new Quotas(tokenAdress);// for quota
    ...
    mapping(address => can_withdraw) public canwithdraw;
...
// investment details
        ...
        uint256 deadline_weekly = block.timestamp + (random() * 0.06 days); //0.06 days

        weekly[msg.sender] = weeklyWithdraw(msg.sender,weeklyStart,deadline_weekly);
        
        // Claim Setting
       uint256 claimTimeStart = block.timestamp;
       uint256 claimTimeEnd = block.timestamp + (random() * 0.03 days); //0.02 days
       ...
       // fees 
        ...
        canwithdraw[msg.sender]._can = true;

        }
        else { 
      
        //prevents abuses when new season begins.
        require (_amount > SafeMath.add(userReward(msg.sender),approvedWithdrawal[msg.sender].amount),"Reinvest needed");   
       ...
       uint256 claimTimeEnd = block.timestamp + (random() * 0.03 days); //0.03 days
...
uint256 nowTime = block.timestamp;
    ...
    if (checktime >= (9 days)){  //9 days of inactivity needs to redeposit (prevents abuses next season)                
        
        canwithdraw[msg.sender]._can = false;
    }

    if (BusdInterface.balanceOf(address(this)) > approvedWithdrawal[msg.sender].amount){

        require (canwithdraw[msg.sender]._can == true, "Reinvest needed");
        if (totalRewards[msg.sender].amount > limit){     
        uint256 amount2 = totalWithdraw[msg.sender].amount;
        uint256 totaltotake = SafeMath.sub(limit,amount2);
        uint256 aval_withdraw = approvedWithdrawal[msg.sender].amount;
        uint256 diff = SafeMath.sub(aval_withdraw,totaltotake);
        uint256 totalAmount = SafeMath.add(amount2,totaltotake); 
        ...
        qotas.setQuotas(msg.sender,totalAmountToWithdraw);// for quota
    ...
    else if (totalRewards[msg.sender].amount <= limit)
        {         
        require(random() != 0, "Random equals 0 retry");
        require((totalRewards[msg.sender].amount <= limit), "You cant withdraw you have doubled your investment already"); 
        ...
        BusdInterface.transfer(dev1,wFee1);
        BusdInterface.transfer(mkt,wFee1);
        approvedWithdrawal[msg.sender] = userWithdrawal(msg.sender,aval_withdraw3); 
