...
contract Revolutium is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    RevolutiumDividendTracker public dividendTracker;
    ...
    uint256 public swapTokensAtAmount = 50_000_000 * (10**18); //50 millions. 
    ...
    address payable public _marketingWalletAddress = payable(0xBC5929B6Cd4bF573a6753845911B9838a5af530D);
    address payable public _buybackWalletAddress = payable(0xBD453E50bfaae0ee5fd33be3656008306B60A556);
    ...
    constructor() BEP20("Revolutium", "RELM") 
    {
        dividendTracker = new RevolutiumDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "Revolutium: The dividend tracker already has that address");
        RevolutiumDividendTracker newDividendTracker = RevolutiumDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "Revolutium: The new dividend tracker must be owned by the Revolutium token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Revolutium: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Revolutium: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Revolutium: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Revolutium: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Revolutium: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Revolutium: Cannot update gasForProcessing to same value");
...
contract RevolutiumDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Revolutium_Dividend_Tracker", "Revolutium_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Revolutium_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Revolutium_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Revolutium contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Revolutium_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Revolutium_Dividend_Tracker: Cannot update claimWait to same value");
