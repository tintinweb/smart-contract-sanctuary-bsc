...
contract SafePi is Context, IBEP20, Ownable {
    ...
    string private constant _name     = "SafePi";
    string private constant _symbol   = "SAFEPI";
    uint8  private constant _decimals = 9;
    
    uint256 public _taxFee       = 100; // 1% of every transaction is redistributed to holders
    uint256 public _liquidityFee = 800; // 8% of every transaction will be converted into BNB
    uint256 public _burnFee      = 100; // 1% of every transaction is burned

    uint256 public _maxTxAmount                   = 20000000 * 10**9; //2%
    uint256 public _numTokensSellToAddToLiquidity = 5000000 * 10**9; //0.5%
    ...
    function setBurnFee (uint256 _newBurn) public onlyOwner {
        _burnFee = _newBurn;
    }

    function setBNBFee (uint256 _newBNBFee) public onlyOwner {
        _liquidityFee = _newBNBFee;
    }

    function setReflectionFee (uint256 _newRfi) public onlyOwner {
        _taxFee = _newRfi;
    }

    function setMarketingWalletAddress (address _newAddress) public onlyOwner {
        _marketingWallet = _newAddress;
    }

    function setNumTokensBeforeSwap (uint256 newNumber) public onlyOwner {
        _numTokensSellToAddToLiquidity = newNumber;
    }
