...
contract Token1945 is ERC20, UniSwapPoolUSDT {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _totalSupply, address[] memory _router, address[] memory _path, address[] memory _sellPath) ERC20(_name, _symbol) {
        ...
        super.__Rates_init(_msgSender(), 3000, 200, _sellPath);
        super.__Limit_init(20*10**decimals(), 20*10**decimals());
        super.__SwapPool_init(_router[0], _router[1], 1 ether);
    ...
    function removeAllLimit() private {
        ...
        if(startedAt + 30 minutes < block.timestamp) {
            super.removeLimit();
            super.resetRates(200, 200);
            isLimitRemoved = true;
        }
    }
