...
//--- Contract v1 ---//
contract PRICE is Context, Ownable, IERC20 {
    ...
    string constant private _name = "PriceAI";
    ...
    constructor () {
        ...
        // IERC20(busdToken).approve(address(swapRouter), type(uint256).max);
    ...
    function internalSwap(uint256 contractTokenBalance) internal inSwapFlag {    
        ...
        uint256 amtBUSD = IERC20(busdToken).balanceOf(address(this)) - balanceBUSDBefore;

        uint256 rate = sellTaxes.marketing + sellTaxes.rewards;
        uint256 marketingBUSD = amtBUSD * sellTaxes.marketing / rate;
        uint256 rewardsBUSD = amtBUSD - marketingBUSD;

        IERC20(busdToken).transferFrom(address(this), marketingAddress, marketingBUSD);
        IERC20(busdToken).transferFrom(address(this), rewardsAddress, rewardsBUSD);
