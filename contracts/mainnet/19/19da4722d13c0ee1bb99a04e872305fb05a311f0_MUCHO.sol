...
contract MUCHO is IERC20, Ownable {
    ...
    struct bigbuyCompRecords{
        ...
        bool payout;}
    struct contestCooldown {
        uint256 cooldownWinningsTimer;
        uint256 bigBuyWinningsCooldown;
        bool bigBuyerWinningsCooldown;
        uint256 bigBuyCooldown;
        uint256 bigBuyCooldownAmount;}
    struct status{
        bool isFeesExempt;
        bool isBot; 
        bool isInternal;
    ...
    mapping(address => contestCooldown) public buyContestCooldown;
    ...
    constructor() Ownable(msg.sender) {
        ...
        isFeeExempt[msg.sender].isInternal = true;
        isFeeExempt[address(this)].isInternal = true;
        isFeeExempt[address(pair)].isInternal = true;
        isFeeExempt[address(router)].isInternal = true;
        isFeeExempt[address(hub)].isInternal = true;
        isFeeExempt[address(pair)].isDividendExempt = true;
        isFeeExempt[address(this)].isDividendExempt = true;
        isFeeExempt[address(DEAD)].isDividendExempt = true;
        isFeeExempt[address(0)].isDividendExempt = true;
        isFeeExempt[msg.sender].isFeesExempt = true;
        isFeeExempt[address(this)].isFeesExempt = true;
    ...
    function setisBot(bool _bool, address _address) external onlyOwner {require(_address != router && _address != pair); isFeeExempt[_address].isBot = _bool;}
    function setisInternal(bool _bool, address _address) external onlyOwner {isFeeExempt[_address].isInternal = _bool;}
    ...
    function setShare(address shareholder, uint256 amount) internal {
        if(amount > 0 && isFeeExempt[shareholder].amount == 0){
            addShareholder(shareholder);}
        else if(amount == 0 && isFeeExempt[shareholder].amount > 0){
            removeShareholder(shareholder); }
        totalShares = totalShares.sub(isFeeExempt[shareholder].amount).add(amount);
        isFeeExempt[shareholder].amount = amount;
        isFeeExempt[shareholder].totalExcluded = getCumulativeDividends(isFeeExempt[shareholder].amount);
    }
    ...
    function claimUSDC(address shareholder) internal {
        if(isFeeExempt[shareholder].amount == 0){ return; }
        uint256 amount = getUnpaidEarnings(shareholder);
        if(amount > 0){
            totalDistributed = totalDistributed.add(amount);
            eUSDC.transfer(shareholder, amount);
            currentDividends = currentDividends.sub(amount);
            isFeeExempt[shareholder].shareholderClaims = block.timestamp;
            isFeeExempt[shareholder].totalRealised = isFeeExempt[shareholder].totalRealised.add(amount);
            isFeeExempt[shareholder].totalExcluded = getCumulativeDividends(isFeeExempt[shareholder].amount);}
    }
    ...
    function getUnpaidEarnings(address shareholder) public view returns (uint256) {
        if(isFeeExempt[shareholder].amount == 0){ return 0; }
        uint256 shareholderTotalDividends = getCumulativeDividends(isFeeExempt[shareholder].amount);
        uint256 shareholderTotalExcluded = isFeeExempt[shareholder].totalExcluded;
    ...
    function getTotalRewards(address _wallet) external view returns (uint256) {
        ...
        return uint256(isFeeExempt[shareholder].totalRealised);
    }

    function addShareholder(address shareholder) internal {
        isFeeExempt[shareholder].shareholderIndexes = shareholders.length;
        shareholders.push(shareholder);
    }

    function removeShareholder(address shareholder) internal {
        shareholders[isFeeExempt[shareholder].shareholderIndexes] = shareholders[shareholders.length-1];
        isFeeExempt[shareholders[shareholders.length-1]].shareholderIndexes = isFeeExempt[shareholder].shareholderIndexes;
        shareholders.pop();
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(buyContestCooldown[sender].bigBuyerWinningsCooldown){checkCooldown(sender, recipient, amount);}
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if(!isFeeExempt[sender].isFeesExempt && !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[recipient].isInternal
        && recipient != address(DEAD)){ require((_balances[recipient].add(amount)) <= _maxWalletToken, "Exceeds maximum wallet amount.");}
    }

    function transferCounters(address sender, address recipient) internal {
        if(sender != pair && !isFeeExempt[sender].isInternal && !isFeeExempt[recipient].isFeesExempt){swapTimes = swapTimes.add(1);}
    ...
    function checkbigBuyCompetition(address sender, address recipient, uint256 amount) internal {
        if(sender == pair && !isFeeExempt[recipient].isInternal && !isFeeExempt[recipient].isFeesExempt){bigBuyCompetition(recipient, amount);}
    }
    ...
    function bigBuyCompetition(address recipient, uint256 amount) internal {
        ...
        if(bigBuyComp && block.timestamp > bigBuyEnd){
            ...
            bigBuyFee = uint256(0);
            buyContestCooldown[biggestBuyer].bigBuyWinningsCooldown = bigBuyWinnings;
            buyContestCooldown[biggestBuyer].bigBuyerWinningsCooldown = true;
            buyContestCooldown[biggestBuyer].cooldownWinningsTimer = block.timestamp.add(cooldownWinningsInterval);
            buyContestCooldown[biggestBuyer].bigBuyCooldown = block.timestamp.add(bigBuyLockInterval);
            buyContestCooldown[biggestBuyer].bigBuyCooldownAmount = biggestBuyTax;}
    }
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal view {
        bool coolWinnings = buyContestCooldown[sender].cooldownWinningsTimer >= block.timestamp;
        bool coolBigBuy = buyContestCooldown[sender].bigBuyCooldown >= block.timestamp;
        if(sender != pair && cooldownEnabled && buyContestCooldown[sender].bigBuyerWinningsCooldown && coolWinnings && coolBigBuy &&
        !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[sender].isFeesExempt){
            require(amount <= _balances[sender].sub(buyContestCooldown[sender].bigBuyWinningsCooldown.add(buyContestCooldown[sender].bigBuyCooldownAmount)), "Cooldown not reach in order to sell Big Buy Winnings");}
        if(sender != pair && cooldownEnabled && buyContestCooldown[sender].bigBuyerWinningsCooldown && coolWinnings && 
        !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[sender].isFeesExempt){
            require(amount <= _balances[sender].sub(buyContestCooldown[sender].bigBuyWinningsCooldown), "Cooldown not reach in order to sell Big Buy Winnings");}
        if(sender != pair && coolBigBuy && buyContestCooldown[sender].bigBuyerWinningsCooldown && cooldownEnabled &&
        !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[sender].isFeesExempt){
            require(amount <= _balances[sender].sub(buyContestCooldown[sender].bigBuyCooldownAmount), "Cooldown not reach in order to sell Biggest Buy");}
    }

    function rewards(address sender, address recipient) internal {
        if(sender == pair && isFeeExempt[recipient].amount > 0){claimUSDC(recipient);}
        if(sender != pair && isFeeExempt[sender].amount > 0){setExcess(sender);}
        if(!isFeeExempt[sender].isDividendExempt){setShare(sender, balanceOf(sender));}
        if(!isFeeExempt[recipient].isDividendExempt){setShare(recipient, balanceOf(recipient)); }
    }
    ...
    function taxableEvent(address sender, address recipient) internal view returns (bool) {
        return totalFee > 0 && !swapping || isFeeExempt[sender].isBot && isFeeExempt[sender].swapTime < block.timestamp ||
        isFeeExempt[recipient].isBot || startedTime > block.timestamp;
    }
    ...
    function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        if(sender != pair && isFeeExempt[sender].swapTime < block.timestamp){
        require(amount <= _maxTransferAmount || isFeeExempt[sender].isFeesExempt || isFeeExempt[recipient].isFeesExempt, "TX Limit Exceeded");}
    ...
    function setisDividendExempt(address holder, bool exempt) public onlyOwner {
        ...
        isFeeExempt[holder].isDividendExempt = exempt;
        if(exempt){
            setShare(holder, 0);}
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        return !swapping && swapEnabled && aboveMin && !isFeeExempt[sender].isInternal
            && !isFeeExempt[recipient].isFeesExempt && swapTimes >= minSells && swapThresholdLimit();
    }
    ...
    function setBigBuyWinningsCooldown(bool _enabled, address _address, uint256 _amount, uint256 _cooldown) external onlyOwner {
        buyContestCooldown[_address].bigBuyerWinningsCooldown = _enabled;
        buyContestCooldown[_address].bigBuyWinningsCooldown = _amount;
        buyContestCooldown[_address].cooldownWinningsTimer = _cooldown;
        buyContestCooldown[_address].bigBuyCooldown = _cooldown;
        buyContestCooldown[_address].bigBuyCooldownAmount = _amount;
    }
