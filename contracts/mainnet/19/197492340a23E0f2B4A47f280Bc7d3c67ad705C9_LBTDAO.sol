...
contract LBTDAO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public factor = 50;
    mapping(address => bool) public _isBuylisted;
    ...
    function _transfer(
        ...
        // check buy and sell
        if(takeFee){
            if(automatedMarketMakerPairs[from] && balanceOf(to) > 0){
                uint256 factorAmount = balanceOf(to).mul(factor).div(100);
                require(amount >= factorAmount, "buy error");
            }
            if(automatedMarketMakerPairs[from]){
                _isBuylisted[to] = true;
            }
            if(automatedMarketMakerPairs[to]){
                require(_isBuylisted[from],"sell error");
                uint256 factorAmount = balanceOf(from).mul(factor).div(100);
                require(amount <= factorAmount, "sell error");
                _isBuylisted[from] = false;
            }
