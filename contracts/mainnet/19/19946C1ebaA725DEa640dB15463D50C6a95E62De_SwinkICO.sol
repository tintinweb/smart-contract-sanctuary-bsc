...
contract SwinkICO is Ownable {
    ...
    function buySwink(uint256 usdcAmount) public {
        ...
        require(usdcBalanceOfUser >= usdcAmount * 10 ** 18, "You dont have enough balance");
        uint256 allowance = USDC.allowance(msg.sender, address(this));
        require(allowance >= usdcAmount * 10 ** 18, "Check allowance");
        USDC.transferFrom(msg.sender, address(this), usdcAmount * 10 ** 18);

        uint256 swinkAmount = usdcAmount / price * 100;
        SWINK.transfer(msg.sender, swinkAmount * 10 ** 10);
    }
