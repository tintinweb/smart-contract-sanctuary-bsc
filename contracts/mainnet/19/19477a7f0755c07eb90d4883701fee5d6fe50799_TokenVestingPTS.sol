 ...
 * @title TokenVesting
 */
contract TokenVestingPTS is Ownable, ReentrancyGuard {
    ...
    modifier onlyIfBeneficiaryExists(address beneficiary) {
        require(
            ...
            "TokenVestingPTS: INVALID Beneficiary Address! no vesting schedule exists for that beneficiary"
        );
        _;
    }
    ...
    modifier onlyIfVestingScheduleExists(bytes32 vestingScheduleId) {
        require(
            ...
            "TokenVestingPTS: INVALID Vesting Schedule ID! no vesting schedule exists for that ID"
        );
        _;
    }
    ...
    function getVestingScheduleByBeneficiaryAndIndex(
        ...
        require(
            ...
            "TokenVestingPTS: INVALID Vesting Schedule Index! no vesting schedule exists at this index for that beneficiary"
        );
        return
            _getVestingSchedule(
    ...
    function createVestingSchedule(
        ...
        require(_amount > 0, "TokenVestingPTS: amount must be > 0");
        tokenAddress.transferFrom(_msgSender(), address(this), _amount);
        require(_duration > 0, "TokenVestingPTS: duration must be > 0");
        require(
            _slicePeriodSeconds >= 1,
            "TokenVestingPTS: slicePeriodSeconds must be >= 1"
        );
