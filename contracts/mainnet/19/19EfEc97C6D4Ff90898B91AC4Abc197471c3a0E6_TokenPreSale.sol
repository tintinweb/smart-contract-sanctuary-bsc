...
contract TokenPreSale is Ownable {
    ...
    function purchaseaToken(uint256 _val) external returns (bool) {
        ...
        IERC20(UAddress).safeTransferFrom(msg.sender ,DAOAddress, _val);
        uint256 _purchaseAmount = _calculateSaleQuote(_val);
        IERC20(tokenAddress).safeTransfer(msg.sender, _purchaseAmount);
        return true;
    }

    function _calculateSaleQuote(uint256 paymentAmount_) internal view returns (uint256) {
        return (10 ** decimals).mul(paymentAmount_).div(salePrice);
    }
