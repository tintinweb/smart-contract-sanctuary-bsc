...
contract MetaFacebook is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MetaFacebookDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(MarketingandDevFee);

    address public _MarketingandDevWalletAddress = 0x72426b14376e102fDE4aAd26e065Af597C2f8284;


    // use by default 300,000 gas to process auto-claiming dividends
    uint256 public gasForProcessing = 300000;
    uint256 public maxFee = 14;
    uint256 public buyTimer = 60 seconds;
    uint256 public sellTimer = 300 seconds;
    ...
    uint256 private percentEquivalent = 2000000 * (10**9);
    ...
    constructor() public ERC20("GrimacePrinter", "GrimacePrinter") {

    	dividendTracker = new MetaFacebookDividendTracker();
        ...
        excludeFromFees(_MarketingandDevWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MetaFacebook: The dividend tracker already has that address");

        MetaFacebookDividendTracker newDividendTracker = MetaFacebookDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MetaFacebook: The new dividend tracker must be owned by the MetaFacebook token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaFacebook: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaFacebook: Account is already the value of 'excluded'");
    ...
    function setMarketingandDevWallet(address payable wallet) external onlyOwner{
        _MarketingandDevWalletAddress = wallet;
    }

    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        BUSDRewardsFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(MarketingandDevFee);
    }
    ...
    function setMarketingandDevFee(uint256 value) external onlyOwner{
        MarketingandDevFee = value;
        totalFees = BUSDRewardsFee.add(liquidityFee).add(MarketingandDevFee);

    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaFacebook: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaFacebook: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaFacebook: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaFacebook: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 MarketingandDevTokens = contractTokenBalance.mul(MarketingandDevFee).div(totalFees);
   ...
   function getCurrentPrice() public view returns (uint256 currentPrice) {//This value serves as a reference to calculate profit only.
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(BUSD).transfer(_MarketingandDevWalletAddress, newBalance);
    }
...
contract MetaFacebookDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("MetaFacebook_Dividend_Tracker", "MetaFacebook_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 2000 * (10**9); //must hold 2000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "MetaFacebook_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MetaFacebook_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaFacebook contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaFacebook_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaFacebook_Dividend_Tracker: Cannot update claimWait to same value");
