...
contract Staking  {
    ...
    modifier checkAllowance( address token , uint amount)  {
        require(IERC20(token).allowance(msg.sender, address(this)) >= amount, "Transfering more than allowed");
        _;
    }
    ...
    function stakeTokens() public checkAllowance(currentToken, currentTokenDepositPrice()) {        
        require(!userProfile[msg.sender].hasDeposited, "Cannot deposit more than once.");
        uint256 currentPriceAll = currentTokenDepositPrice();
