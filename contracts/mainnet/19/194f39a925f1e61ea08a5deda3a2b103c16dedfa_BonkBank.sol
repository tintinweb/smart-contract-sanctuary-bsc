...
interface IUniswapV2Router02 is IUniswapV2Router01 {
 ...
 contract BonkBank is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xe653D650954a1773Ee8f2A213e94552aAe144363); // Marketing Address

    address payable public bonkBankAddress =
        payable(0x27eb09aa6c1e18e83CDbE48Dd2893CC574B769E7); // bonkBank Address

    address payable public liquidityAddress =
        payable(0x33b75672Ca9ce882bCDd7CF4aE128ae2FF30D74b); // Liquidity Address
    ...
    string private constant _name = "Bonk Bank";
    ...
    uint256 private _bonkBankTokensToSwap;
    ...
    event UpdatedbonkBankAddress(address buyback);
    ...
    constructor() {
        ...
        maxWalletAmount = _tTotal * 15 / 1000; // 1.5% maxWalletToken
        ...
        _isExcludedFromFee[bonkBankAddress] = true;
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _bonkBankTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buybonkBankFee;
                buyOrSellSwitch = BUY;
            }
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellbonkBankFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_bonkBankTokensToSwap).add(_marketingTokensToSwap);
        ...
        uint256 bnbForbonkBank = bnbBalance.mul(_bonkBankTokensToSwap).div(totalTokensToSwap);

        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForbonkBank);

        _liquidityTokensToSwap = 0;
        _marketingTokensToSwap = 0;
        _bonkBankTokensToSwap = 0;

        (bool success,) = address(bonkBankAddress).call{value: bnbForbonkBank}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _bonkBankTokensToSwap += tLiquidity * _buybonkBankFee / _liquidityFee;
            ...
            _bonkBankTokensToSwap += tLiquidity * _sellbonkBankFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buybonkBankFee)
        external
        onlyOwner
    {
        ...
        _buybonkBankFee = buybonkBankFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buybonkBankFee <= 20, "Must keep taxes below 20%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buybonkBankFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellbonkBankFee)
        external
        onlyOwner
    {
        ...
        _sellbonkBankFee = sellbonkBankFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellbonkBankFee <= 30, "Must keep taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellbonkBankFee);
    }
    ...
    function setbonkBankAddress(address _bonkBankAddress) external onlyOwner {
        require(_bonkBankAddress != address(0), "_bonkBankAddress address cannot be 0");
        bonkBankAddress = payable(_bonkBankAddress);
        _isExcludedFromFee[bonkBankAddress] = true;
        emit UpdatedbonkBankAddress(bonkBankAddress);
    }
