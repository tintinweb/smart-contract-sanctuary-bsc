...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 public requestNum = 188 * 10**18;
    uint256 public responseNum = 88 * 10**18;

    mapping(address => address) public inviter;
    mapping(address => bool) public marketPairs;
    mapping(address => mapping(address => uint256)) public requestInvite;  
    mapping(address => uint256) public inviteCount;     
    mapping(address => uint256) public swapLimitMap;     

    address public burnAddress = address(0x000000000000000000000000000000000000dEaD);
    address public devAddress = address(0xe881CD150Cb08F5a2C15d15A5260ca48B9A0DE42);
    address public teamAddress = address(0xA2497ddb8ec70FB4065CcB9a6ecA73d9D2295b64);
    address public husdtToken = address(0x55d398326f99059fF775485246999027B3197955);
    ...
    uint256 public _maxSwapLimit = 20000000 * 10**18;   
    uint256 public _maxBurnAmount = 99000000000 * 10**18; 
    ...
    event Invite(address account, address father);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        _takeDev(sender, data.tDev);
        _takeTeam(sender, data.tTeam);
    ...
    function _takeTeam(address sender, uint256 tDev) private {
        if(tDev == 0 || _teamFee == 0) return;
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[teamAddress] = _rOwned[teamAddress].add(rDev);
        emit Transfer(sender, teamAddress, tDev);
    }
    ...
    function _takeInviterFee(
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            address addr = cur;
            uint256 count = inviteCount[addr];
            if(count < i + 1) {
                addr = burnAddress;
            }

            uint256 curTAmount = tAmount.mul(rate).div(_inviterFee).div(10);
            uint256 curRAmount = curTAmount.mul(currentRate);
            _rOwned[addr] = _rOwned[addr].add(curRAmount);
            emit Transfer(sender, addr, curTAmount);
        }
    }
    ...
    function setRequestAmount(uint256 amount) public onlyOwner {
        requestNum = amount;
    }

    function setResponseAmount(uint256 amount) public onlyOwner {
        responseNum = amount;
    }
    ...
    function setMarketPairs(address account, bool state) external onlyOwner {
        marketPairs[account] = state;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        return (tTransferAmount, TData(0,tFee, tLiquidity, tBurn, tDev, tInv, tTeam));
    }
    ...
    function _getRValues(TData memory _data, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTeam = _data.tTeam.mul(currentRate);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(_liquidityFee == 0 || tLiquidity == 0) return;
        uint256 currentRate =  _getRate();
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _devFee == 0 && _inviterFee == 0 && _teamFee == 0) return;
    ...
    function _transfer(
        ...
        if(balanceOf(burnAddress) >= _maxBurnAmount) {
            _burnFee = 0;
    ...
    function _invite(address from, address to, uint256 amount) private {
        ...
        if(inviter[from] == to || inviter[to] == from) {
            return;
        }

        if(requestInvite[from][to] == 0 && requestInvite[to][from] == 0 && amount == requestNum && inviter[to] == address(0)) {
            requestInvite[from][to] = block.timestamp;
        } 

        if (requestInvite[from][to] == 0 && requestInvite[to][from] > 0 && amount == responseNum && inviter[from] == address(0)) {
            requestInvite[from][to] = block.timestamp;
        } 

        uint256 time0 = requestInvite[to][from];
        uint256 time1 = requestInvite[from][to];
        
        if (time0 > 0 && time1 > time0 && inviter[from] == address(0)) {
            inviter[from] = to;
            inviteCount[to] = inviteCount[to].add(1);
            emit Invite(from, to);
        }
    }
