...
contract zBNB is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 zBNB-WETH LP -> zBNB pool
    // uint256 public constant INITIAL_zBNB_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the zBNB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("zBNB", "zBNB") {
        // Mints 1 zBNB to contract creator for initial pool setup
    ...
    function _getzBNBPrice() internal view returns (uint256 _zBNBPrice) {
        try IOracle(zBNBOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("zBNB: failed to fetch zBNB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _zBNBPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_zBNBPrice >= taxTiersTwaps[tierId]) {
    ...
    function setzBNBOracle(address _zBNBOracle) public onlyOperatorOrTaxOffice {
        require(_zBNBOracle != address(0), "oracle address cannot be 0 address");
        zBNBOracle = _zBNBOracle;
    }
    ...
    /**
     * @notice Operator mints zBNB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of zBNB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentzBNBPrice = _getzBNBPrice();
            currentTaxRate = _updateTaxRate(currentzBNBPrice);
            if (currentzBNBPrice < burnThreshold) {
                burnTax = true;
            }
        }
