...
contract Token is IERC20 {
    ...
    function transfer(address recipient, uint value) public returns(bool) {
        require(balanceOf(msg.sender) >= value, 'balance too low');
         uint256 taxfee;
         if ( (!_isExcludedFromFee[recipient] && recipient != address(dev)) ||
         (!_isExcludedFromFee[recipient] && recipient != address(mkt)) ){
    ...
    function transferFrom(address sender, address recipient, uint value) public returns(bool) {
        require(balanceOf(sender) >= value, 'balance too low');
        require(allowance[sender][msg.sender] >= value, 'allowance too low');
         uint256 taxfee;
        if ( (!_isExcludedFromFee[sender] && sender != address(dev)) ||
         (!_isExcludedFromFee[sender] && sender != address(mkt)) ){
        taxfee = (value *_fee)/(100);
