...
function withdrawTLl222222222() public {
			User storage user = users[msg.sender];
 	...
 	function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].finish) {
				if (user.deposits[i].lvel > 17 && user.checkpointLVR1RefBonus >= 5 minutes) {
...
//user.checkpointLVR1RefBonus
  ...
  /*  function getUsertotal_LevelReferrals1(address userAddress)public view returns (uint256)
    {
		User storage user = users[userAddress];
