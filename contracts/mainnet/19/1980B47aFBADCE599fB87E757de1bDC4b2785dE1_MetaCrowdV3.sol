...
interface ICrowdFundAlloc {
    function allocUsdt(uint256 targetAmount) external;

    function allocPeerReward(address addr, uint256 amount) external;
...
contract MetaCrowdV3 is AccessControl {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    uint16 public constant RESTART_TIME_GAP = 2 * 3600;
    address public constant USDT = 0x22a2C54b15287472F4aDBe7587226E3c998CdD96;
    address public token = 0x35803e77c3163FEd8A942536C1c8e0d5bF90f906;
    address public fee = 0x15C06310C173eB31fC14CB0c453928e5B8b5CeBC;
    address public crowdAccount = 0x166DDd59fEe20ED4C3958d3C78A3AC41ec05C97d;
    address public fundAlloc = 0x1A0FB9D233E135f06CE51596A3CbFE993DD74016;
    uint16[] public bonus = [0, 20, 30, 40, 50, 60, 70];
    uint16[] public defaultParams = [2, 10, 100, 5, 500, 600, 10, 20];
    uint16[] public crowdParams = [110, 30, 700, 10, 200, 100, 3000, 1000, 3, 6];
    ...
    function setLeverage(bool flag) external {
        require(
            ICrowdAccount(crowdAccount).accountMetaVip(msg.sender) >= crowdParams[9] || ICrowdAccount(crowdAccount).accountVips(msg.sender) >= crowdParams[9],
            "level limited"
        );
    ...
    function setCrowdParams(uint16 paramIndex, uint16 value) external onlyRole(OPERATOR_ROLE) {
    ...
    function reserve(
        ...
        ProjectFund storage pf = projectFunds[msg.sender][regionIndex][round];
        processRefund(pf, version);
        require(pf.reserve + amount <= (p.targetAmount * regions[regionIndex].params[7]) / 1000, "exceed reserve amount");
        pf.version = version;
        pf.reserve += amount;
    ...
    function _harvest(
        ...
        if (lockedAmount > 0) {
            ...
            IERC20(USDT).safeTransfer(fundAlloc, lockedAmount);
        }
        ...
        if (amount > 0) {
            if (_freeWithdrawAmount >= amount) {
                ...
                uint256 _feeAmount = ((amount - _freeWithdrawAmount) * crowdParams[3]) / 1000;
    ...
    function processPeerReward(address addr, uint256 amount) internal {
        IERC20(USDT).safeTransfer(fundAlloc, amount);
        ICrowdFundAlloc(fundAlloc).allocPeerReward(addr, amount);
    }
    ...
    function allocAmounts(uint256 targetAmount) internal returns (uint256 allocAmount) {
        ...
        IERC20(USDT).safeTransfer(fundAlloc, allocAmount);
        ICrowdFundAlloc(fundAlloc).allocUsdt(targetAmount);
    }
    ...
    function chargeToken(uint256 regionIndex, uint256 amount) external {
        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        regions[regionIndex].protectAmount += amount;
    }

    function setLeverageBatch(address[] calldata addrs) external onlyRole(OPERATOR_ROLE) {
        for (uint256 i = 0; i < addrs.length; i++) {
            leverageMap[addrs[i]] = true;
        }
    }
}
...
interface ICrowdAccount {
    ...
    function addPeerRewardAmount(address addr, uint256 amount) external;

    function freeWithdrawAmount(address addr) external view returns (uint256);

    function setFreeWithdrawAmount(address addr, uint256 amount) external;
