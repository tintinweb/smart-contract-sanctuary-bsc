...
interface _LAMBO_CLAIM {
    ...
    function addFee(uint256 epoch, uint256 SHIBReward) external payable;
...
contract LAMBO is IERC20Metadata, Ownable, _LAMBO {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    _LAMBO_CLAIM public claimContract;
    ...
    string _name = "Lambo";
    string _symbol = "LAMBO";
    uint8 _decimals = uint8(DECIMALS);
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10**6 * 10**DECIMALS;
    uint256 public constant sellLiquidityFee = 30;
    uint256 public constant sellTreasuryFee = 50;
    uint256 public constant sellInsuranceFundFee = 50;
    uint256 public constant sellFirePitFee = 20;
    uint256 public constant sellBNBSHIBFee = 50;

    uint256 public constant buyLiquidityFee = 15;
    uint256 public constant buyTreasuryFee = 25;
    uint256 public constant buyInsuranceFundFee = 25;
    uint256 public constant buyFirePitFee = 10;
    uint256 public constant buyBNBSHIBFee = 25;

    uint256 public totalFeeSell = sellLiquidityFee.add(sellTreasuryFee).add(sellInsuranceFundFee).add(sellFirePitFee).add(sellBNBSHIBFee);
    uint256 public totalFeeBuy = buyLiquidityFee.add(buyTreasuryFee).add(buyInsuranceFundFee).add(buyFirePitFee).add(buyBNBSHIBFee);
    uint256 public constant feeDenominator = 1000;
    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO = 0x0000000000000000000000000000000000000000;

    address public autoLiquidityReceiver =
        0x214d183FE270A4d151182cE4900147B8DD09Abed;
    address public treasuryReceiver =
        0xD7058c207cF40Eb301715AB23D7dD8C03F5eDbed;
    address public lamboInsuranceFundReceiver =
        0xAeE701525471A14e63615bD32b14bB4A55598bed;
    address public firePit = DEAD;
    ...
    uint256 private constant MAX_SUPPLY = 2 * 10**10 * 10**DECIMALS;
    ...
    uint256 _initBlock;
    uint256 public numberOfBlocksPerRewardsCycle = 9600; //8 hours

    address SHIB = 0x2859e4544C4bB03966803b044A93563Bd2D0DD4D;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    IERC20 SHIBContract = IERC20(SHIB);
    bool hasClaimContract = false;

    uint256 holders = 1;
    bool _startRebase = false;

    constructor() IERC20Metadata("Lambo", "LAMBO", uint8(DECIMALS)) Ownable() {
    ...
    function setClaimContract(address _contract) external onlyOwner {
        require(_contract != address(0x0) && !hasClaimContract);
        hasClaimContract = true;
        claimContract = _LAMBO_CLAIM(_contract);
        claimContract.setLamboData(_initBlock);
    ...
    function currentRebaseEpoch() public view returns (uint256) {
        return (block.timestamp - _initRebaseStartTime).div(8 minutes);
    }

    function timeToNextEpoch() public view returns (uint256) {
        return
            _initRebaseStartTime +
            (currentRebaseEpoch() + 1).mul(8 minutes) -
            block.timestamp;
    }
    ...
    function takeFee(
        ...
        uint256 _firePitFee = 0;
        uint256 _insuranceFundFee = 0;
        uint256 _BNBSHIBFee = 0;
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount
                .mul(_treasuryFee.add(_insuranceFundFee).add(_BNBSHIBFee)).div(feeDenominator)
        );
        _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].add(gonAmount.mul(_liquidityFee).div(feeDenominator));
    ...
    function _swapBack() internal swapping {
        ...
        uint256 totalSwapBackFee = buyTreasuryFee
            .add(buyInsuranceFundFee)
            .add(buyBNBSHIBFee);
        uint256 amountETH = _sellToken(amountToSwap);
        uint256 BNBReward = amountETH
            .mul(buyBNBSHIBFee)
            .div(totalSwapBackFee)
            .div(2);
        uint256 BNBToSwapSHIB = amountETH
            .mul(buyBNBSHIBFee)
            .div(totalSwapBackFee)
            .sub(BNBReward);
        uint256 SHIBReward = buySHIB(BNBToSwapSHIB);
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETH.mul(buyTreasuryFee).div(totalSwapBackFee),
            gas: 30000
        }("");
        (success, ) = payable(lamboInsuranceFundReceiver).call{
            value: amountETH.mul(buyInsuranceFundFee).div(totalSwapBackFee),
            gas: 30000
        }("");
        SHIBContract.transfer(address(claimContract), SHIBReward);
        claimContract.addFee{value: BNBReward}(
            currentRewardsEpoch(),
            SHIBReward
        );
    }

    function buySHIB(uint256 amount) public returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WBNB;
        path[1] = SHIB;
        if (amount == 0) {
        ...
        uint256 balanceBefore = SHIBContract.balanceOf(address(this));
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: amount
        }(0, path, address(this), block.timestamp);
        return SHIBContract.balanceOf(address(this)).sub(balanceBefore);
    }
    ...
    function setFeeReceivers(
        ...
        address _lamboInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        lamboInsuranceFundReceiver = _lamboInsuranceFundReceiver;
        firePit = _firePit;
    }
