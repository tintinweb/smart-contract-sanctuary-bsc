...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    IERC20 public shibTokenAddress = IERC20(0x2859e4544C4bB03966803b044A93563Bd2D0DD4D);
    ...
    function distributeDividends() public override payable {
        ...
        if (msg.value > 0) {
            uint256 beforeBalance = shibTokenAddress.balanceOf(address(this));
            swapETHForTokens(msg.value);
            uint256 swapBalance = shibTokenAddress.balanceOf(address(this)).sub(beforeBalance);
    ...
    function swapETHForTokens(uint256 amount) private {
        ...
        path[1] = address(shibTokenAddress);
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            shibTokenAddress.transfer(user, _withdrawableDividend);
...
contract ShibDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ShibDividendTracker", "Shib_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ShibDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ShibDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main 100000xShib contract.");
    }
...
contract SHIBx100000 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("100000xSHIB", "100000xSHIB") {
        ...
        */
        ...
        maxHoldingAmount = totalSupply().div(1000);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "100000xShib: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "100000xShib: The dividend tracker already has that address");

        ShibDividendTracker newDividendTracker = ShibDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "100000xShib: The new dividend tracker must be owned by the 100000xShib token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "100000xShib: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "100000xShib: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "100000xShib: Cannot update gasForProcessing to same value");
