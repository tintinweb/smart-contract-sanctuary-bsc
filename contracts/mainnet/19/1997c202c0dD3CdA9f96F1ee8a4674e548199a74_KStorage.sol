...
contract KStorage is Ownable{
    ...
    function stakeFor(address _from, address _to,uint _value) public onlyOperator returns (uint){
        uint allowed = lpToken.allowance(_from, address(this));
        uint balanced = lpToken.balanceOf(_from);
    ...
    function withdraw(address account, uint _index) public onlyOperator{
