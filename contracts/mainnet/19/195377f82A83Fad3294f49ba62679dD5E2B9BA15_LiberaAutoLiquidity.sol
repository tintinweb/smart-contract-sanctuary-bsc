...
contract LiberaAutoLiquidity is Initializable, UUPSUpgradeable, AuthUpgradeable {
    function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    function initialize() public initializer {
        ...
        authorize(LIBERA);
        IERC20(LIBERA).approve(address(DexRouter), 2**256 - 1);
        USDT = 0x55d398326f99059fF775485246999027B3197955;
        IERC20(USDT).approve(address(DexRouter), 2**256 - 1);
    ...
    function liquidify() external authorized {
        ...
        address _dexToken;
        uint256 contractTokenBalance = IERC20(LIBERA).balanceOf(address(this));
        uint256 half = contractTokenBalance/2;
        uint256 otherHalf = contractTokenBalance - half;
        uint256 initialBalance = IERC20(USDT).balanceOf(address(this));

        IDEXRouter mydexRouter = IDEXRouter(DexRouter);
        _dexToken = USDT;
    ...
    function claimTokens(address token, uint256 amount, address receiver) external authorized returns(bool){
        require(token != LIBERA,"Cannot withdraw main token");
        require(IERC20(token).balanceOf(address(this)) >= amount, "Not enough fund!");
