...
contract UnseaNft is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress =
        payable(0x3A133A723Ee26BF73D539275029357934E12DF2c);

    uint256 public NFT_MAX_COUNT = 25;
    uint256 public PUBLIC_PRICE = 0.5 ether;
    uint256 public PURCHASE_LIMIT = 5;
    uint256 public PUBLIC_MINT_LIMIT = NFT_MAX_COUNT;

    mapping(address => uint256) private _mappingPublicMintCount;

    uint256 public _activeDateTime = 1648800000; //(GMT): Friday, April 1, 2022 8:00:00 AM
    ...
    constructor(
        ...
        uint256 nftCount_,
        uint256 nftPrice_
    ) ERC721(nftName_, nftSymbol_) {
        NFT_MAX_COUNT = nftCount_;
        PUBLIC_MINT_LIMIT = nftCount_;
        PUBLIC_PRICE = nftPrice_;
    }
    ...
    function setMaxLimit(uint256 maxLimit) external onlyOwner {
        NFT_MAX_COUNT = maxLimit;
    }

    function setPurchaseLimit(uint256 purchaseLimit, uint256 publicMintLimit)
        external
        onlyOwner
    {
        ...
        onlyOwner
    {
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            for (uint256 i = 0; i < numberOfTokens; i++) {
                uint256 tokenId = _publicCount.current();

                if (_publicCount.current() < NFT_MAX_COUNT) {
    ...
    function purchase(uint256 numberOfTokens) external payable {
        require(
            _publicCount.current() < NFT_MAX_COUNT,
            "Purchase would exceed NFT_MAX_COUNT"
        );
        ...
        if (msg.sender != owner()) {
            ...
            _mappingPublicMintCount[msg.sender] =
                ...
                numberOfTokens;
            require(
                _mappingPublicMintCount[msg.sender] <= PUBLIC_MINT_LIMIT,
