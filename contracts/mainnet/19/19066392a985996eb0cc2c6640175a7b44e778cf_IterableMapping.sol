...
contract WWODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WWO_Dividend_Tracker", "WWO_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WWO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WWO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WWO contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WWO_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WWO_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WWO is ERC20, Ownable {
    ...
    WWODividendTracker public dividendTracker;
    ...
    constructor() ERC20("Warriors World Online", "WWO") {
        BNBRewardsFee = 8;
        liquidityFee = 2;
        marketingFee = 2;

        onSellBNBRewardsFee = 14;
        onSellliquidityFee = 2;
        onSellmarketingFee = 2;

        marketingWallet = payable(0xF0BC3A6D2532bCC2Ed7AcF4e0a039649505b146C);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee);

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee);

    	dividendTracker = new WWODividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WWO: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WWO: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WWO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WWO: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WWO: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WWO: Cannot update gasForProcessing to same value");
