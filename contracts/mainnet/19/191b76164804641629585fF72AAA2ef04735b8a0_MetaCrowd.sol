...
contract MetaCrowd is AccessControlEnumerable {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.UintSet;
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");
    bytes32 public constant UPGRADE_ROLE = keccak256("UPGRADE_ROLE");
    uint16 public constant RESTART_TIME_GAP = 2 * 3600;
    address public constant LP = 0xAbF64b7EA6220eFE9C42e4283b16583A83A66Edc;
    ...
    address public constant STAKE = 0x9c88692C6740359D849230139171176a77a96d63;
    address public fee = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa;
    address public dao = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa;
    uint256 public donateAmount;
    uint16[5] public bonus = [0, 0, 10, 20, 25];
    uint16[11] public rates = [100, 25, 800, 10, 50, 600, 200, 10, 200, 10, 50];
    uint16 public maxReserveRound = 5;
    uint16 public fundInterval = 6;
    uint256 public maxFundAmount = 500e18;
    ...
    function upgrade(address addr, uint8 vips) external onlyRole(UPGRADE_ROLE) {
        accounts[addr].vips = vips;
    }

    function setAddresses(address _fee, address _dao) external onlyRole(OPERATOR_ROLE) {
        fee = _fee;
        dao = _dao;
    }
    ...
    function setFundParams(
        uint16 maxRound,
        uint256 maxAmount,
        uint16 interval
    ) external onlyRole(OPERATOR_ROLE) {
        maxReserveRound = maxRound;
        maxFundAmount = maxAmount;
    ...
    function setRegionParams(
        ...
        regions[regionIndex].restartType = restartType;
    }
    ...
    function _addProject(uint256 regionIndex) internal {
        ...
        if (pl == 1) {
            ...
            if (projects[regionIndex][pl - 2].state == 4) {
                p.targetAmount = (regions[regionIndex].restartType == 1 && regions[regionIndex].poolAmount > 1e18)
                    ? roundup(regions[regionIndex].poolAmount / 2, 1e18)
                    : regions[regionIndex].initAmount;
            } else {
                p.targetAmount = roundup((projects[regionIndex][pl - 2].targetAmount * region.multiplier) / 1000, 1e18);
            }
            p.startTime = region.poolType == 1 ? block.timestamp : projects[regionIndex][pl - 2].startTime + region.period;
        }
    }
    ...
    function fund(uint256 regionIndex, uint256 amount) external {
        ...
        require(amount <= getMaxAmountPerTime(p.targetAmount, regions[regionIndex].poolType), "max amount limited");
        if (block.timestamp < p.startTime || block.timestamp >= p.startTime + regions[regionIndex].period) revert ProjectTimeError();
    ...
    function reserve(
        ...
        require(recordMetaPoint(msg.sender), "no meta point account");
        require(amount > 0, "zero amount error");
        require(regions[regionIndex].poolType == 0, "wrong region");
        uint32 currentRound = regions[regionIndex].currentRound;
        uint256 maxRound = min(projects[regionIndex].length, currentRound + maxReserveRound);
        Project storage p = projects[regionIndex][round];
        if (p.state > 1 || block.timestamp > p.startTime || round >= maxRound) revert ProjectStateError();
        if (block.timestamp > p.startTime) revert ProjectTimeError();
        require((amount + p.reserveFundAmount) * 2 <= p.targetAmount, "no reserve amount left");
        require(amount <= accounts[msg.sender].priorityFundAmount, "priority amount not enough");
        uint16 version = p.version;
        ProjectFund storage pf = projectFunds[msg.sender][regionIndex][round];
        processRefund(pf, version);
        pf.version = version;
        pf.reserveAmount += amount;
    ...
    function processRefund(ProjectFund storage pf, uint16 version) internal {
        if (pf.version < version) {
            ...
            if (reserveAmount > 0) {
                pf.priorityRefundAmount += reserveAmount;
                pf.reserveAmount = 0;
            }
            if (fundAmount > 0) {
                pf.refundAmount += fundAmount;
                pf.fundAmount = 0;
    ...
    function finishReserve(uint256 regionIndex, uint256 round) external {
        ...
        pf.reserveFundAmount += amount;
        pf.fundAmount += amount;
        p.reserveFundAmount += amount;
        accounts[msg.sender].lastFundTime = block.timestamp;
    ...
    function finishProject(uint256 regionIndex) public {
        uint32 round = regions[regionIndex].currentRound;
        Project storage p = projects[regionIndex][round];
        require(block.timestamp > p.startTime + regions[regionIndex].period || p.targetAmount == p.currentAmount, "not met finish condition");
        if (p.state > 2) revert ProjectStateError();
        uint256 poolAmount = regions[regionIndex].poolAmount + p.reserveFundAmount;
        ...
        if (currentAmount == p.targetAmount) {
            ...
            if (round >= 3 && projects[regionIndex][round - 3].state == 3) {
                ...
                uint256 stakeAmount = (targetAmount * rates[4]) / 1000;
                uint256 _donateAmount = (targetAmount * rates[3]) / 1000;
                uint256 refundAmount = (targetAmount * (1000 + rates[0] + rates[1])) / 1000 - p3.brokerRewardLeft;
                processStake(stakeAmount);
                donateAmount += _donateAmount;
                p3.refundAmount = refundAmount;
                poolAmount -= (refundAmount + stakeAmount + _donateAmount);
            }
        } else {
            p.state = 4;
            if (round > 0) {
                ...
                for (uint256 i = start; i < round; i++) {
                    ...
                    if (pi.state == 3) {
                        ...
                        poolAmount -= pi.refundAmount;
                    }
                }
            }
            if (currentAmount > 0) {
                poolAmount -= currentAmount;
                uint256 bonusAmount = (poolAmount * rates[5]) / 1000;
    ...
    function resetProjects(uint256 regionIndex, uint256 round) internal {
        ...
        for (uint256 i = round + 1; i < length; i++) {
            ...
            if (i == round + 1) {
                _targetAmount = (regions[regionIndex].restartType == 1 && regions[regionIndex].poolAmount > 0)
                    ? roundup(regions[regionIndex].poolAmount / 2, 1e18)
                    : regions[regionIndex].initAmount;
                ...
                p.targetAmount = roundup((_targetAmount * multiplier) / 1000, 1e18);
                if (resetTime) p.startTime = _startTime + period;
            }
        ...
        )
    {
        ...
        if (pf.version == projects[regionIndex][round].version) {
            if (state == 4) {
                amounts[1] = fundAmount;
                uint256 currentAmount = p.currentAmount;
                amounts[2] = currentAmount == 0 ? 0 : ((p.refundAmount - currentAmount) * fundAmount) / currentAmount;
            } else if (state == 5) {
                amounts[1] = fundAmount;
                amounts[2] = (fundAmount * rates[0]) / 1000;
            } else if (state == 6) {
                amounts[1] = (fundAmount * rates[2]) / 1000;
                amounts[3] = (fundAmount * rates[6]) / rates[7];
    ...
    function _harvest(
        ...
        uint256 lockedAmount = (rewardAmount * rates[8]) / 1000;
        if (lockedAmount > 0) {
            accounts[msg.sender].lockedWithdrawAmount += lockedAmount;
        }
        if (priorityFundAmount > 0) {
            accounts[msg.sender].priorityFundAmount += priorityFundAmount;
        }
        uint256 _freeWithdrawAmount = accounts[msg.sender].freeWithdrawAmount;
        ...
        if (amount > 0) {
            if (_freeWithdrawAmount >= amount) {
                ...
                uint256 _feeAmount = ((amount - _freeWithdrawAmount) * rates[7]) / 1000;
                _freeWithdrawAmount = 0;
                IERC20(LP).safeTransfer(msg.sender, amount - _feeAmount);
                IERC20(LP).safeTransfer(fee, _feeAmount);
            }
        }
        accounts[msg.sender].freeWithdrawAmount = _freeWithdrawAmount;
    }

    function recordMetaPoint(address account) internal returns (bool) {
        if (!accounts[account].pointFlag) {
            IMetaPoint.Account memory _account = IMetaPoint(META_POINT).accounts(account);
            address head = _account.head;
            if (head == address(0)) return false;
            if (accounts[account].vips == 0) {
                accounts[account].vips = _account.vip < 2 ? _account.vip : 1;
            }
            accounts[account].point = _account.point;
            accounts[account].head = _account.head;
            accounts[account].pointFlag = true;
        }
        return true;
    }
    ...
    function processBrokerReward(
        ...
        for (uint256 i = 0; i < 45; i++) {
            ...
            if (bonus[parentLevel] > bonus[level]) {
                ...
                used += diff;
                BrokerReward storage br = brokerRewards[parent][regionIndex][round];
                brokerRewardIds[parent][regionIndex].add(round);
                ...
                if (parentLevel == 4) break;
                level = parentLevel;
            }
            parent = accounts[addr].head;
            if (parent == address(0)) {
                parent = IMetaPoint(META_POINT).accounts(addr).head;
            }
            if (parent == address(0)) {
                break;
            }
        ...
        if (rates[1] > used) {
            projects[regionIndex][round].brokerRewardLeft += (fundAmount * (rates[1] - used)) / 1000;
        }
    }
    ...
    function pendingForUnlock(address account) public view returns (uint256 lockedAmount, uint256 unlockAmount) {
        uint256 unlockedAmount = accounts[account].unlockedAmount;
        lockedAmount = accounts[account].lockedWithdrawAmount - unlockedAmount;
        if (accounts[account].pointFlag) {
            uint256 point = IMetaPoint(META_POINT).accounts(account).point;
            uint256 _point = accounts[account].point;
            if (point > _point) {
    ...
    function unlockWithdraw() external {
        ...
        accounts[msg.sender].unlockedAmount += amount;
        IERC20(LP).safeTransfer(msg.sender, amount);
    }
    ...
    function getMaxAmountPerTime(uint256 targetAmount, uint16 poolType) public view returns (uint256) {
        uint256 _max = (targetAmount * (poolType == 0 ? rates[9] : rates[10])) / 1000;
        return _max > maxFundAmount ? _max : maxFundAmount;
    }

    function parent_mapping(address[] calldata addresses, address[] calldata parents) external onlyRole(OPERATOR_ROLE) {
        require(addresses.length == parents.length, "data error.");
        for (uint16 i = 0; i < addresses.length; i++) {
            accounts[addresses[i]].head = parents[i];
        }
    }
