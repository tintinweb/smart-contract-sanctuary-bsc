...
contract SafuuBNBminer is Context, Ownable {
    using SafeMath for uint256;

    uint256 private COINS_TO_NEW_SAFUU_MINER = 1080000;
    uint256 private OFFSET_TOP = 10000; //Offsets used in trade calculation
    uint256 private OFFSET_BOTTOM = 5000;
    bool private initialized = false;
    mapping (address => uint256) private safuuMiners;
...
*/
    function startThesafuuMachine() public payable onlyOwner {
...
//COMPOUND REWARDS -> MORE SAFUU MINERS
    function buySafuuMinersWithCoin(address ref) public {
        require(initialized);
        ...
        uint256 newMiners = SafeMath.div(coinsUsed, COINS_TO_NEW_SAFUU_MINER);
        safuuMiners[msg.sender] = SafeMath.add(safuuMiners[msg.sender],newMiners);
...
//SELLING
    function sellCoins() public {
		...
		uint256 taxedSellValue = SafeMath.div( SafeMath.mul(coinValue, 9), 10); //10% tax on selling, no taxes on reinvesting with 'buySafuuMinersWithCoin' function.
    ...
    function getCoinsSinceLastHarvest(address adr) public view returns(uint256) {
        uint256 secondsPassed = min(COINS_TO_NEW_SAFUU_MINER, SafeMath.sub(block.timestamp, lastHarvest[adr]));
        return SafeMath.mul(secondsPassed, safuuMiners[adr]);
    }
