...
interface ITreasury {
    function processTax(address sender, uint tax) external;
}

// File: contracts/STEEL.sol
...
contract STEEL is Initializable,UUPSUpgradeable,ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable,ERC20VotesCompUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;
    address public treasury;

    mapping(address => uint) private taxSetting;
    uint256 private defaultTax;
    ...
    function setWhitelist(address[] memory addresses, uint[] memory settings) public onlyOwner {
    ...
    function _transfer(
        ...
        if (AddressUpgradeable.isContract(treasury)) {
            ITreasury(treasury).processTax(sender, taxAmount);   
        }
    }
}