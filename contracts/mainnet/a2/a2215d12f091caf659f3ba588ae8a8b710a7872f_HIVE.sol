...
contract HIVE is Context, IERC20, Ownable {
    ...
    event LockTheSwap(bool indexed lockTheSwap);

    bool private _lockTheSwap;
   ...
   function transfer(address recipient, uint256 amount) external override returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(checkedAccount(sender),"HIVE: 85% of the funds have not been unlocked");
    ...
    function extractFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        require(!_lockTheSwap,"HIVE: The unlocking time has not yet arrived");
    ...
    function settlement()public {
        if (_balances[_lpAdmin] > liquifyThreshold * 10 ** decimals()){
    ...
    function lockTheSwap()external onlyOwner{
        _lockTheSwap = !_lockTheSwap;

        emit LockTheSwap(_lockTheSwap);
    }
