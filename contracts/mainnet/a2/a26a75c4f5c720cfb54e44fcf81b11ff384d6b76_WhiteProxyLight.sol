...
interface IinstanceRegistry {
    ...
    function instances(IWhiteInstance _white) external view returns (
        bool isERC20,
        IERC20 token,
        InstanceState state,
...
contract WhiteProxyLight {
    ...
    function deposit(
        ...
        (bool isERC20, IERC20 token, IinstanceRegistry.InstanceState state, , ) = instanceRegistry.instances(_white);
        require(state != IinstanceRegistry.InstanceState.DISABLED, "The instance is not supported");

        if (isERC20) {
            token.transferFrom(msg.sender, address(this), _white.denomination());
        }
        _white.deposit {
            value: msg.value
        }(_commitment);
    ...
    function withdraw(
        ...
        (, , IinstanceRegistry.InstanceState state, , ) = instanceRegistry.instances(_white);
        require(state != IinstanceRegistry.InstanceState.DISABLED, "The instance is not supported");
        // relayerRegistry.burn(msg.sender, _relayer, _white);
