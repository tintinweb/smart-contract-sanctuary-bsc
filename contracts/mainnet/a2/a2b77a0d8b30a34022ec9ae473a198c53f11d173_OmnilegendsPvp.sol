...
contract OmnilegendsPvp is Initializable, PausableUpgradeable, OwnableUpgradeable,   ReentrancyGuardUpgradeable {
    ...
    function payEntryFee(uint256 nonce, bytes memory sig, bytes memory walletSig) external nonReentrant whenNotPaused {
        require(!usedNonces[nonce],"Used nonce");
        usedNonces[nonce] = true;
        require(feeAmount>0,"Fee amount not set");

        require(isValidData(feeAmount, nonce, sig, walletSig)==true,"Invalid data");
        ...
        emit PayPvpFee(currentPvp, buyer, feeAmount, nonce, sig, walletSig);
    }
    ...
    // encryption
    function isValidData(uint256 amount, uint256 nonce, bytes memory sig, bytes memory walletSig) private view returns(bool){
        address _walletAddress = _msgSender();
        
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(_walletAddress, amount, currentPvp, nonce, sig))
        );

        // verify that the wallet signed the message
        require(recoverSigner(message, walletSig)==_msgSender(),"Not signed by the user"); 

        bytes32 signedMessage = keccak256(abi.encodePacked(_walletAddress, amount, currentPvp, nonce));
