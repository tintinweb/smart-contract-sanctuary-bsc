...
contract Token is Ownable, IERC20Metadata {
    ...
    uint256 public  _buyrate;
    uint256 public  _selrate;
    ...
    constructor() {
        ...
        _invi = _msgSender();
        _main = 0x38175132237D4aF0081a3F84385df54a4B4D62e2;
        _mark = 0x38175132237D4aF0081a3F84385df54a4B4D62e2;
    ...
    function _transfer(
        ...
        if (recipient == _pair) {
            ...
            // 15%
            _balances[address(this)] += (amount * _selrate / 100);
            emit Transfer(sender, address(this), (amount * _selrate / 100));
            amount = amount * (100-_selrate) / 100;
        } else if (sender == _pair) {
            address cur = recipient;
            for (uint256 i=0; i<_rate.length; i++) {
                cur = inviter[cur];
                if (cur == address(0) || isContract(cur)) { cur = _invi; }
                uint256 temp = (amount * _rate[i] / 100);
                if (temp > 0) {
                    _balances[cur] += temp;
                    emit Transfer(sender, cur, temp);
                }
            }
            amount = amount * (100 - _buyrate) / 100;
        }
    ...
    function setAddrs(address mark, address invi) public onlyOwner {
        _mark = mark;
        _invi = invi;
    }
    ...
    function setBuyRate(uint256 val) public onlyOwner {
        _buyrate = val;
    }

    function setSelRate(uint256 val) public onlyOwner {
        _selrate = val;
    }

    function setRate(uint256[] memory val) public onlyOwner {
        _rate = val;
    }
