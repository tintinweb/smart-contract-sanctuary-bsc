...
abstract contract CrossChainERC20 is ERC20, AccessControl, RouterCrossTalk {
...
contract BattleSquad is CrossChainERC20,Ownable{
    ...
    uint256 public burnP = 50;
    uint256 public maxtranscation = totaltokensupply.mul(5).div(10000);
    ...
    bool public selfDestructSellFeeEnable = false;
    bool public selfDestructBuyFeeEnable = false;

    bool public completeFeeDestruct = false;
    bool public selfDestructActive = false;
    ...
    constructor(address _genericHandler)RouterCrossTalk(_genericHandler)
        AccessControl() public ERC20("BattleSquad", "BATS") {
    ...
    function setbuyFee(uint256 value) external onlyOwner{
        require(selfDestructBuyFeeEnable = false,"now it cannot be changed premanantly");
        require(value<=12,"value cannot be more than 12");
        buyFee = value;
    }

    function setsellFee(uint256 value) external onlyOwner{
        require(selfDestructSellFeeEnable = false,"now it cannot be changed premanantly");
        require(value<=12,"value cannot be more than 12");
        sellFee = value;
    }
    ...
    function selfDestructSellFee() external onlyOwner{
        require(selfDestructActive = true,"self destruct not enabled");
        selfDestructSellFeeEnable = true;
    }

    function selfDestructBuyFee() external onlyOwner{
        require(selfDestructActive = true,"self destruct not enabled");
        selfDestructBuyFeeEnable = true;
    }

    function _completeFeeDestruct() external onlyOwner{
        require(selfDestructActive = true,"self destruct not enabled");
        completeFeeDestruct = true;
        feeActive = false;
    }

    function _selfDestructActive(bool value) external onlyOwner{
        selfDestructActive = value;
    }
    ...
    function setfeeActive(bool value) external onlyOwner {
        require(completeFeeDestruct = false,"its self destructed");
        feeActive = value;
    }
    ...
    function _transfer(
      ...
      pinkAntiBot.onPreTransferCheck(from, to, amount);
        ...
        if(takeFee) {
            ...
            uint256 burnAmount = fees.mul(burnP).div(100);
            fees = fees.sub(burnP);
