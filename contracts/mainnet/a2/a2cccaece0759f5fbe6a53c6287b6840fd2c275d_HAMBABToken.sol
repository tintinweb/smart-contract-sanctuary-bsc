...
contract HAMBABToken is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
    ...
    constructor(ISBT721 _bab, string memory _uri) ERC721('HAMBAB', 'HAMBAB') {
        bab = _bab;
        _unrevealedUri = _uri;
    }
    ...
    function claim() external nonReentrant whenNotPaused returns (uint256) {
        if (_claimed[msg.sender] == true) revert AlreadyClaimed();
        if (bab.balanceOf(msg.sender) == 0) revert BabNotOwned();
