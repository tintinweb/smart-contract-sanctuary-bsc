...
contract RedCoin is BEP20 {
    ...
    function transfer(address to, uint amount) public returns (bool success) {
        ...
        return Accounting(accounting).doTransfer(msg.sender, msg.sender, to, amount); // can cause burn when with pancake
    }

    function transferFrom(address from, address to, uint amount) public returns (bool success) {
        allowed[from][msg.sender] = allowed[from][msg.sender].sub(amount);
        emit Transfer(from, to, amount);
        return Accounting(accounting).doTransfer(msg.sender, from, to, amount); // can cause burn when with pancake
    }
