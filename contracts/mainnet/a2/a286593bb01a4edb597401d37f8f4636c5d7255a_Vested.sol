...
library Address{
 ...
 contract Vested is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 20_000_000 * 10**9; // swap at 0.2%
    uint256 public maxBuyLimit = 100_000_000 * 10**9; // 1%
    uint256 public maxSellLimit = 100_000_000 * 10**9; // 1%
    uint256 public maxWalletLimit = 100_000_000 * 10**9; // 1%
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xACDb365C55dC3279aB0592cabCC30B2CfFd13e49;
    address public developerWallet = 0xACDb365C55dC3279aB0592cabCC30B2CfFd13e49;

    string private constant _name = "Vested Inu";
    string private constant _symbol = "Vested";
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[developerWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _developer) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_developer);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _developer) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_developer);
        emit FeesChanged();
    }
    ...
    function _takedeveloper(uint256 rdeveloper, uint256 tdeveloper) private {
        totFeesPaid.developer +=tdeveloper;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tdeveloper;
        }
        _rOwned[address(this)] +=rdeveloper;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rdeveloper) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tdeveloper = tAmount*temp.developer/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tdeveloper;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rdeveloper = s.tdeveloper*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rdeveloper;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rdeveloper) {

        if(!takeFee) {
          return(0);
        }

        rdeveloper = s.tdeveloper*currentRate;
        return (rdeveloper);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tdeveloper);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rdeveloper > 0 || s.tdeveloper > 0) _takedeveloper(s.rdeveloper, s.tdeveloper);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.developer) * 2;
        ...
        uint256 developerAmt = unitBalance * 2 * temp.developer;
        if(developerAmt > 0){
