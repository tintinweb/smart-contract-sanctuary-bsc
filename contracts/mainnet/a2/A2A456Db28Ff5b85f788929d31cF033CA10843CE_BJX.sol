...
contract BJX is ERC20 {
	...
	address private _fundAddress = address(0xC913998b6bD78d58609Cb06449a423Cd4306f47C);
	address private _mrketAddress = address(0x741709Ad65a18C1Ff1F999C42929C7A7f0e2E838);
	address private _applicationAddress = address(0x3D53688ab8C3DbB99efEEA736D32646F5472b7a9);
    ...
    constructor(address tokenOwner) ERC20("BJX", "BJX") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx || balanceOf(_destroyAddress) >= _destroyMaxAmount) {
			...
			if(from == uniswapV2Pair){
				...
				if(startTime.add(300) > block.timestamp){require(amount <= 5 * 10**18);}
	...
	function swapAndLiquify() public {
		...
		if(allUsdtBalance > usdtBalance){
			...
			USDT.transfer(_applicationAddress,newAmount.div(225).mul(50));
			addLiquidityUsdt(newAmount.div(225).mul(75), otherAmount);
		}
    }
