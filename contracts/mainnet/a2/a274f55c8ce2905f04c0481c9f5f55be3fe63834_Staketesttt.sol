...
contract Staketesttt is Ownable, ReentrancyGuard {
    ...
    function setTokenRewardAmount(uint _amount) external onlyOwner {
        massUpdatePools();
        _setTokenRewardAmount(_amount);
    }

    function _setTokenRewardAmount(uint _amount) internal {
    ...
    function ownerUpdate(address[] calldata _users) external onlyOwner {
    ...
    function _updatePool(uint amount, uint _timestamp) internal returns (uint){
        ...
        uint n1 = pool.lastRewardChange.sub(startTimestamp).div(24 hours).add(1);
        if (n1 > 9) {
            uint num10 = n1.div(10);
            uint rem = n1.sub(10 * num10);
            for (uint i = 0; i < num10; i++) {
                _setTokenRewardAmount(tokenRewardAmount.mul(rewardPercent).mul(pow(10000 - (rewardPercent), 9)).div(pow(10000, 10)));
            }
            _setTokenRewardAmount(tokenRewardAmount.mul(rewardPercent).mul(pow(10000 - (rewardPercent), rem.sub(1))).div(pow(10000, rem)));
        }
    ...
    function _deposit(address _user, uint _amount) internal {
        ...
        if(userExistence[_user] == false) {
            userExistence[_user] = true;
            users.push(_user);
            numUsers++;
        }
    ...
    function getUserDetails(uint _startIndex, uint _endIndex) external view returns (UserInfo[] memory) {
        if (_endIndex == 0) {
            _endIndex = numUsers;
        }
        UserInfo[] memory userInfos = new UserInfo[](_endIndex - _startIndex);
        for (uint i = _startIndex; i < _endIndex; i++) {
            userInfos[i] = userInfo[users[i]];
        }
        return userInfos;
    }
