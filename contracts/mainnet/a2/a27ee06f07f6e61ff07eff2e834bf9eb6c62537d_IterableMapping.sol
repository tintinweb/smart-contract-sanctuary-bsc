...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "QFM: The router already has that address");
  ...
  constructor() {
    ...
    InvestorAddress = payable(0x5CF5fd0c93C393233A191818C9a1190EB0B51fe8);
  }
...
contract QUEENFLOKIMON is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public defaultToken = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    constructor() ERC20("QueenFlokimon", "QFM", 18) {
    	...
    	operationsWallet = address(0x66aFE9f841C4d2ad94743FdB9d1d5682328172bC);
    	investorWallet = address(0x5CF5fd0c93C393233A191818C9a1190EB0B51fe8);
    	marketingWallet = address(0x97e495a4fBF5ED9c0da007B0F5d878cF04BE0e8e);
        buybackWallet = address(0xB1b0201D56C8704dA6988988Cdf5eEad540DD767);
        ...
        maxWallet = totalSupply  * 1 / 190; // 3% Max wallet
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "QFM: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "QFM: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "QFM: The new dividend tracker must be owned by the QFM token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "QFM: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "QFM: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "QFM: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "QFM: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "QFM: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "QFM: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "QFM: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "QFM: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "QFM: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "QFM: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "QFM: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "QFM: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "QFM: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "QFM: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "QFM: Automated market maker pair is already set to that value");
...
contract DividendTracker is DividendPayingToken {
   ...
   function setOverRideTokenAmm (address overridetoken) external onlyOwner {
    overridingTokenInPlay = overridetoken;
    }
 
   
  	function rewardOverride (bool overridden) external onlyOwner {
     overrideActive  = overridden;
   }

    function withdrawDividend() pure external override {
        require(false, "QFM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QFM contract.");
    }
    ...
    function updateDividendMinimum(uint256 minimumToEarnDivs) public onlyOwner {
