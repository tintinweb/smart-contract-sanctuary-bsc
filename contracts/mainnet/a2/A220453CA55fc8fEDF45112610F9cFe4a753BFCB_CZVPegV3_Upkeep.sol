...
contract CZVPegV3 is ReentrancyGuard, Ownable, Pausable {
    ...
    function repegEllipsisOnly(uint256 _usdEllipsis, bool isOverPeg)
        external
        whenNotPaused
        whenNotRestricted
    {
        if (isOverPeg) {
            czusd.mint(address(this), _usdEllipsis);
            _correctOverPegEllipsis(_usdEllipsis);
        } else {
            uint256 busdBalance = busd.balanceOf(address(this));
            if (busdBalance < _usdEllipsis) {
                _withdrawBusd(_usdEllipsis - busdBalance);
            }
