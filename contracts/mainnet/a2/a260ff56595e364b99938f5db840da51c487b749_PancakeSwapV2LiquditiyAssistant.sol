...
contract PancakeSwapV2LiquditiyAssistant is Ownable,Pausable,ReentrancyGuard {
    ...
    function forceRemoveLiquidity(address token,uint256 amount) public whenNotPaused nonReentrant {
        ...
        function forceRemoveLiquiditySupportingFeeOnTransferTokens(address token,uint256 amount) public whenNotPaused nonReentrant {
        IBEP20(token).safeTransferFrom(_msgSender(),address(this),amount);
        ...
        // Need a smart contract dev? telegram @hialias
        function forceRemoveLiquidityETHwithPermit(address token,uint256 amount) public whenNotPaused nonReentrant {
        IBEP20(token).safeTransferFrom(_msgSender(),address(this),amount);
