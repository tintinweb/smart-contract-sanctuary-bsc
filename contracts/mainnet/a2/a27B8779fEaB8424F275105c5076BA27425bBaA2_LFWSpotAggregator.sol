...
interface ILFWDEXUtils {
    ...
    function findInfoOut(uint amountIn, address[] memory path) external view returns (uint256[] memory reservesIn, uint256[] memory userGet);
    function findInfoIn(uint amountIn, address[] memory path) external view returns (uint256[] memory reservesIn, uint256[] memory userIn);
...
contract LFWSpotAggregator is LFWAuth, ILFWDEXUtils {
    ...
    function findInfoOut(uint amountIn, address[] memory path) public view override returns (uint256[] memory reservesIn, uint256[] memory userGet) {
    ...
    function findInfoIn(uint amountOut, address[] memory path) public view override returns (uint256[] memory reservesIn, uint256[] memory userIn) {
        userIn = new uint256[](dexCount);
        reservesIn = new uint256[](dexCount);
        uint256[] memory reserveOut = new uint256[](dexCount);
        for (uint i; i < dexCount; i++) {
            (uint balanceA, uint balanceB) = getPairBalances(i, path[0], path[1]);
            if (balanceA == 0 && balanceB == 0) {
                ...
                userIn[i] = IDEXRouter(dexList[i].router).getAmountsIn(amountOut, path)[0];
                (reservesIn[i], reserveOut[i]) = getReserves(i, path[0], path[1]);
            } 
