...
contract RebateTreasury is Ownable, Pausable {
    ...
    constructor(
        ...
        krakenOracle = IOracle(_krakenOracle);
    ...
    function _bond(address _token, uint256 _amount, bool _intoLP, address _recipient) internal onlyAsset(_token) whenNotPaused {
        ...
        if (_intoLP == false) {
            ...
            uint256 krakenPrice = getKrakenPrice(); //Kraken price in DAI... therefore KRAKEN_USD/DAI_USD

            //Manage token balances.
            uint256 daiBalance = IERC20(DAI).balanceOf(address(this));
            uint256 daiAmount = rebateTokenAmount.div(2).mul(krakenPrice).div(1e18);
            ...
            if (daiAmount > daiBalance) {
                ...
                uint256 rebateTokenAmountToSwap = daiNeeded.mul(1e18).div(krakenPrice).mul(110).div(100);
    ...
    function getAssetPremium(
        ...
        assetPremium = tokenPrice.mul(bondPremium.add(DENOMINATOR)).div(DENOMINATOR).mul(assets[_token].multiplier).div(krakenPrice);
        if (_intoLP == true) {
    ...
    function getBondPremium() public view returns (uint256) {
        ...
        uint256 rebateTokenPremium = krakenPrice.mul(DENOMINATOR).div(treasury.krakenPriceOne()).sub(DENOMINATOR); 
    ...
    //Get KRAKEN price from Oracle
    function getKrakenPrice() public view returns (uint256) {
        return krakenOracle.consult(address(rebateToken), 1e18);
    }
