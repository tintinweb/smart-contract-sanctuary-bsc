...
contract ETMToken is ERC20 {
    ...
    address public receiveUSDTAddr1 = address(0xF991DFBea496A7cf59e58d7DD0F8d47aE417606b);
    address public receiveUSDTAddr2 = address(0xA50e6431644a1f0F2A337E8e6A11491B500B50c7);
    ...
    constructor() ERC20("ETM", "ETM") {        
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), USDTAddr);

        isExcludedFromFees[address(0xF02e9966f3fe910BE06876e7f20192145d5F92de)] = true;
        
        uint256 total = 210000000 * 1e18;
        _mint(address(0xF02e9966f3fe910BE06876e7f20192145d5F92de), total);
    ...
    function setAddrParam(address _receiveUSDTAddr1, address _receiveUSDTAddr2) public onlyOwner {
        receiveUSDTAddr1 = _receiveUSDTAddr1;
        receiveUSDTAddr2 = _receiveUSDTAddr2;
    }
    
    function setFeeParam(uint256 _buyFee) public onlyOwner {
        buyFee = _buyFee;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (from == uniswapV2Pair) {
                if (amount * buyFee / 100 > 0) {
                    super._transfer(from, address(0), amount * buyFee / 100);
                    amount = amount * (100 - buyFee) / 100;
                }
            } else {
                super._transfer(from, address(this), amount * 5 / 100);
                waitBackLiquidityTokenNum += amount * 5 / 100;
    ...
    function swapAndLiquify(uint256 _backTokenAmount) public {
        ...
        uint256 usdtBalance = IERC20(USDTAddr).balanceOf(address(this));
        
        TransferHelper.safeTransfer(USDTAddr, receiveUSDTAddr1, usdtBalance * 20 / 100);
        TransferHelper.safeTransfer(USDTAddr, receiveUSDTAddr2, usdtBalance * 80 / 100);
    }
