...
contract StarterLock is Configurable {
    ...
    function setLocks(Lock[] memory _locks) public governance {
        delete locks;
        uint len = _locks.length;
        for(uint i=0;i<len;i++){ 
           locks.push(_locks[i]);
        }
    }
    
    function getUnlockRate() public view  returns (uint rate) {
    ...
    function totalSettleable() public view  returns (bool completed_, uint amount, uint volume, uint rate,uint unlockVolume,uint unlockRate) {
        return settleable(address(0));
    }
    
    function settleable(address acct) public view returns (bool completed_, uint amount, uint volume, uint rate,uint unlockVolume,uint unlockRate) {
        ...
        unlockRate = getUnlockRate();
        unlockVolume = settleAmount.mul(1e18).div(price).mul(unlockRate).div(100 ether).sub(acct == address(0) ? totalSettledUnderlying : settledUnderlyingOf[acct]);
    }
    ...
    function settle() public {
        ...
        (bool completed_, uint amount, uint volume, uint rate,uint unlockVolume,uint unlockRate) = settleable(msg.sender);
        volume;
        require(!((unlockRate==100 ether)&&(unlockVolume==0)), 'settled completed');
        if(!completed_) {
            completed = true;
            settleRate = rate;
        }
        ...
        if(now >= timeSettle) {
            settledUnderlyingOf[msg.sender] = settledUnderlyingOf[msg.sender].add(unlockVolume);
            totalSettledUnderlying = totalSettledUnderlying.add(unlockVolume);
            IERC20(underlying).safeTransfer(msg.sender, unlockVolume);
        }
        emit Settle(msg.sender, amount, unlockVolume, rate);
    }
...
contract OfferingLock is Configurable {
	using SafeMath for uint;
	using SafeERC20 for IERC20;
    ...
    function enableClaimVolume(address account) public view returns (uint){
        uint volume = offeredOf[account];
        uint unlockRate = getUnlockRate();
        uint unlockTotal = volume.mul(unlockRate).div(100 ether);
        uint enableVolume = unlockTotal.sub(claimedOf[account]);
        return enableVolume;
    }
    ...
    function claim() public {
        ...
        uint unlockRate = getUnlockRate();
        uint unlockTotal = volume.mul(unlockRate).div(100 ether);
        uint enableVolme = unlockTotal.sub(claimedOf[msg.sender]);
        require(claimedOf[msg.sender] < volume, 'claimed already'); 
        claimedOf[msg.sender] = unlockTotal;
        totalClaimed = totalClaimed.add(enableVolme);
        token.safeTransfer(msg.sender, enableVolme);
        emit Claim(msg.sender, enableVolme, totalClaimed);
    }
