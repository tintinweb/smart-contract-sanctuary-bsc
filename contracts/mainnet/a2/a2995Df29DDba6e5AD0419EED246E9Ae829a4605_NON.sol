...
contract NON is ERC721Enumerable, Ownable {
    using Address for address;
    using Strings for uint256;
    struct Card {
        bool isValid;
    ...
    string private _uri = "http://nft.natbox.xyz/";
    event MintNFT(address to, uint256 tokenId, uint256 level, uint256 price);
    constructor() ERC721("NFT OF NAT", "NON") {}
    ...
    function mint(
        ...
        require(!_exists(tokenId), "Has Exist");
        return _mintNFT(to, tokenId, tokenId / 10000, price);
    }
    function _mintNFT(
        address to,
        uint256 tokenId,
        ...
        emit MintNFT(to, tokenId, level, price);
        return cardTotal;
    }
        ...
        returns (string memory)
    {
        ...
        string memory uri = _strConcat(
            _baseURI(),
            _cards[tokenId].level.toString()
        );
        uri = _strConcat(uri, "/");
        uri = _strConcat(
            uri,
            (tokenId - _cards[tokenId].level * 10000).toString()
        );
        uri = _strConcat(uri, ".png");
        return uri;
    }
