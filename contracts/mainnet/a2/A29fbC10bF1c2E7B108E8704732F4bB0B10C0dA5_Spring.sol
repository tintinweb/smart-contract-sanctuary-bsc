...
contract Spring is ERC20, Owners {
    ...
    function setBlacklist(address[] calldata _addresses, bool _new) external onlyOwners {
        for(uint256 i = 0; i < _addresses.length; i++) {
            blacklist[_addresses[i]] = _new;
        }
    }

    function setWhitelist(address[] calldata _addresses, bool _new) external onlyOwners {
        for(uint256 i = 0; i < _addresses.length; i++) {
