    ...
    ERC20VotesUpgradeable
{
    ...
    uint256 public timeLockTransfer;
    address public LOCK_CONTRACT;
    address public SELL_CONTRACT;
    mapping(address => bool) private whiteList;
    mapping(address => uint256) private lockTo;
    mapping(address => uint256) private lockTransfer;
    mapping(address => uint256) private unlockAmount;

    constructor() initializer {}

    function initialize() public initializer {
        __ERC20_init("DOG", "GOD");
        __Ownable_init();
    ...
    function _afterTokenTransfer(
        ...
        if (!whiteList[from]) {
            if (lockTo[from] <= block.timestamp){
                lockTo[from] = block.timestamp + lockTime;
            }
            // Buy Token
            if (_isPair(from) && !_isPair(to)) {
                lockTransfer[to] = block.timestamp + timeLockTransfer;
            }
        }
    }

    function getLock(address add)public view returns(uint256, uint256){
        return (block.timestamp, lockTransfer[add]);
    }
    ...
    function setSellContract(address sell_contract) public onlyOwner {
    ...
    function setTimeLockTransfer(uint256 _timeLockTransfer) public onlyOwner {
        timeLockTransfer = _timeLockTransfer;
    }
    ...
    function _transfer(
        ...
        uint256 buyAmount = 0;
        uint256 unlock = unlockAmount[sender];
        if (lockTransfer[sender] > block.timestamp){
            revert();
        }
        if (!whiteList[sender]) {
            if (LOCK_CONTRACT != address(0)) {
                airdropAmount = LockContract(LOCK_CONTRACT).airdropAmount(sender);
            }
            if (SELL_CONTRACT != address(0)) {
                buyAmount = SellContract(SELL_CONTRACT).receivedAmount(sender);
            }
            if (buyAmount != 0 && lockTo[sender] <= block.timestamp){
                unlock = unlock + (buyAmount - unlockAmount[sender]) / 100;
                unlockAmount[sender] = unlock;
            }

            if ((airdropAmount + buyAmount + amount - unlock) > balanceOf(sender)) {
                revert();
            }
        ...
        if (bpEnabled && !bpDisabledForever) {
            bpContract.protect(sender, recipient);
        }
     ...
     * @param enable status of address
     */
    function addWhileList(address _address, bool enable) public onlyOwner {
    ...
    function getMaxSwap(address _address) public view returns(uint256) {
        uint256 airdropAmount = 0;
        uint256 buyAmount = 0;
        uint256 unlock = unlockAmount[_address]; 
        if (!whiteList[_address]) {
            if (LOCK_CONTRACT != address(0)) {
                airdropAmount = LockContract(LOCK_CONTRACT).airdropAmount(_address);
            }
            if (SELL_CONTRACT != address(0)) {
                buyAmount = SellContract(SELL_CONTRACT).receivedAmount(_address);
            }
            if (buyAmount != 0 && lockTo[_address] <= block.timestamp){
                unlock = unlock + (buyAmount - unlock) / 100;
            } 
        }
        return (balanceOf(_address) + unlock - airdropAmount - buyAmount);
    } 
}
...
contract LockContract is Initializable , OwnableUpgradeable{
  ...
  event RefReward(address indexed user, address indexed ref, uint256 reward_amount, uint256 rate);
  ...
  /**
   * @dev Set LockAll admin
   */
  function setLockAll(bool _status) public onlyOwner {
    isLockAll = _status;
  }
  ...
  function airdropAmount(address _address) external view returns (uint256){
    ...
    return airdropUser[_address];
    // uint256 sellLockAmount = SellContract(SELL_CONTRACT).receivedAmount(_address); 
...
contract SellContract is Initializable , OwnableUpgradeable{
  ...
  uint256 public ref_rate;
  bool public lockAll = false;

  event NewReferral(address indexed user, address indexed ref);
  event SellIDO(address indexed user, uint256 indexed sell_amount, uint256 indexed buy_amount);
  event RefReward(address indexed user, address ref, uint256 indexed reward_amount);
   ...
   * @dev Update is enable
   */
  function setLockAll(bool _lockAll) public onlyOwner {
    lockAll = _lockAll;
  }
   ...
   */
  function updateRateRef(uint256 _ref_rate) public onlyOwner {
    ref_rate = _ref_rate;
  }
  ...
  function buyTokenBNB( address _referrer) public payable onlyNonContract returns (bool) {
    if (referrers[msg.sender] == address(0)
        ...
        emit NewReferral(_referrer, msg.sender);
    }
    ...
    // send ref reward
    if (referrers[msg.sender] != address(0)){
        uint256 ref_reward = sold_amount * ref_rate  / 100 ;
        if (ref_reward > 0) {
          refReward[referrers[msg.sender]] += ref_reward;
          IERC20Upgradeable(IDO_TOKEN).transfer(referrers[msg.sender], ref_reward);
          emit RefReward(msg.sender,referrers[msg.sender], ref_reward);
        }
    }
