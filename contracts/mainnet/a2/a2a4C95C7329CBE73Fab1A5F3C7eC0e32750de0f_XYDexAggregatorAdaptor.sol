...
// FIXME: A simplified version of XY dex aggregator (previously Aggregator).
...
contract XYDexAggregatorAdaptor is IDexAggregator, Ownable {
    using SafeERC20 for IERC20;
    using UniERC20 for IERC20;

    /* ========== STATE VARIABLES ========== */
    /// A contract that performs multi-swap according to given input data
    /// See XYDexAggregatorAdaptor.sol for more info
    ...
    constructor(address payable _caller) {
        require(Address.isContract(_caller), "ERR_XY_AGGREGATOR_NOT_CONTRACT");
        aggregator = _caller;
    }
        ...
        returns (uint256)
    {
        ...
        if (!fromToken.isETH()) {
            amount = fromToken.balanceOf(aggregator);
            fromToken.safeTransferFrom(msg.sender, aggregator, desc.amount);
            amount = fromToken.balanceOf(aggregator) - amount;
        }
        ...
        Address.functionCallWithValue(aggregator, data, msg.value, "Caller multiswap failed");
