...
contract SafeFarm is ReentrancyGuard, AccessControl {
    ...
    */
    ...
    function depositLP(uint256 _amount,
        ...
        _depositLP(msg.sender, _amount, route0, route1);
    }
    ...
    function depositAutoLP(
        ...
        if (_fee > 0) {
            address feeRecipient = strategy.safeFarmFeeRecipient();
            IERC20(route0[0]).safeTransferFrom(from, feeRecipient, _fee);
            _amount = _amount.sub(_fee);
        }

        _depositLP(from, _amount, route0, route1);
    }
    ...
    function _depositLP(
        ...
        uint256 amountHalf = calcLpAmount(
            _route0[_route0.length-1],
            _route1[_route1.length-1],
            _amount
        );
        LPToken memory lpt0 = _swapLPToken(unirouter, _depositor, amountHalf, _route0);
        LPToken memory lpt1 = _swapLPToken(unirouter, _depositor, _amount.sub(amountHalf), _route1);
        ...
        earn();
        uint256 _after = balance();
        uint256 amount = _after.sub(_pool); // Additional check for deflationary tokens
    ...
    public view returns (uint256 amount0) {
        ...
        uint256 bal0 = IERC20(token0).balanceOf(_want);
        uint256 bal1 = IERC20(token1).balanceOf(_want);

        return amount.mul(bal0).div(bal0.add(bal1));
    }
