...
contract SaleAndDrop2Level {
    ...
    function buyByErc20(uint256 id, uint256 amount, address invite) public {
        ...
        uint256 leftnum = amount;
        if (boss[_msgSender()] != address(0)) {
            invite = boss[_msgSender()];
        }
		if (invite != address(0) && invite != _msgSender()) {
            uint256 levelnum = confs[id].level1 * amount / 100;
            leftnum -= levelnum;
            IERC20(confs[id].payContract).transferFrom(_msgSender(), invite, levelnum);
        }
        IERC20(confs[id].payContract).transferFrom(_msgSender(), _owner, leftnum);
