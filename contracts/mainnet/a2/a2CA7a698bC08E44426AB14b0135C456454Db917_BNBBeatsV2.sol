...
contract BNBBeats {
    ...
    function mapUser(address adr)
        external
        view
        returns (
            uint256 investOrigin,
            uint256 withdrawOrigin,
            address referralsOrigin,
            uint256 hatcheryMinersOrigin,
            uint256 claimedBeatsOrigin,
            uint256 lastHatchOrigin,
            uint256 checkpointOrigin
        )
    {
        User memory user = users[adr];
        investOrigin = user.invest;
        withdrawOrigin = user.withdraw;
        referralsOrigin = user.referrals;
        hatcheryMinersOrigin = user.hatcheryMiners;
        claimedBeatsOrigin = user.claimedBeats;
        lastHatchOrigin = user.lastHatch;
        checkpointOrigin = user.checkpoint;
    }
}
...
contract BNBBeatsV2 is Ownable {
    ...
    address private contractOriginAddress =
        0xDe875E515296423F3872E709d7520Df78Fd3035b;
    ...
    constructor(address _dev, address _owner) {
        ...
        contractOrigin = BNBBeats(contractOriginAddress);
    }
    ...
    function connectUserOrigin() public {
        updateUserOrigin();
    }

    function updateUserOrigin() private {
        ...
        if (!user.originDone) {
            (
                ...
                uint256 lastHatchOrigin,
                uint256 checkpointOrigin
            ) = contractOrigin.mapUser(msg.sender);
            if (user.invest > 0) {
                uint256 dif = SafeMath.sub(investOrigin, withdrawOrigin);
                if (dif > 0) {
                    user.withdrawOrigin = withdrawOrigin;
                    user.investOrigin = investOrigin;
                    user.invest = dif;
                    user.hatcheryMiners = hatcheryMinersOrigin;
                    user.claimedBeats = claimedBeatsOrigin;
                    user.lastHatch = lastHatchOrigin;
                    user.checkpoint = checkpointOrigin;
                    user.referrals = referralsOrigin;
                } else {
                    user.originDone = true;
                }
            } else {
                if (user.withdrawOrigin != withdrawOrigin) {
    ...
    function sellBeats() external initializer {
        ...
        if (whiteList[msg.sender]) {
            ...
            hasBeats -= (hasBeats / SafeMath.div(333, 100));
