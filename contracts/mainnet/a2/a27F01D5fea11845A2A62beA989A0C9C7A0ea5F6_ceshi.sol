...
contract ceshi is Ownable, IERC20 {
    ...
    address private marketingWalletAddress =0xEBeAF5d918b6C41D8FE1781396E6602d3446CbE1;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (!_isExcludedFrom[sender] && !_isExcludedFrom[recipient] && recipient != address(this)) {
            ...
            if(isMarketPair[sender] || isMarketPair[recipient]){
                ...
                if(MFee > 0){
                    _balance[marketingWalletAddress] = _balance[marketingWalletAddress].add(MFee);
                    emit Transfer(sender, marketingWalletAddress, MFee);
                }
                uint256 LFee = amount.mul(_liquidityFee).div(100);
                if(LFee > 0){
                    _balance[address(this)] = _balance[address(this)].add(LFee);
                    emit Transfer(sender, address(this), LFee);
                }
                feeAmount = MFee.add(LFee);
            }
        }
