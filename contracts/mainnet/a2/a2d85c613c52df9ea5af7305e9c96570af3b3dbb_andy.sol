...
contract IDividendsClaims {
    ...
    uint8 public buybackAccumulator;
...
contract andy is IERC20, Ownable, IDividendsClaims{
    ...
    function NewPendingAdmin(address holder) external onlyOwner {
	...
	function catimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function imrecoverBEP20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
			// do not allow recovering self token
			require(tokenAddress != address(this), "Self withdraw");
			IERC20(tokenAddress).transfer(owner(), tokenAmount);
		}

    function settakeFee(address recipient, uint256 amount) internal returns (uint256) {        
