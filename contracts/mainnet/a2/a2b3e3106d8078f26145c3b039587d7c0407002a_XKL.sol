...
contract XKL is Ownable {
    ...
    event isAddLiquidity(bool _isAdd, string _type);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (pairAddressList.contains(recipient)) {
            ...
            emit isAddLiquidity(ldxAdd, "AddLiquidity");
        }
        if (pairAddressList.contains(sender)) {
            ldxAdd = checkIsAddLiquidity(sender);
            emit isAddLiquidity(ldxAdd, "RemoveLiquidity");
        }
        if (
            canAddLiquify &&
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            !pairAddressList.contains(sender) && pairAddressList.contains(recipient) && !sellerSet[sender] && !ldxAdd
        ) {
