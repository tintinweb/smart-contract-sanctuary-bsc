...
contract UberTaxiController8 is Ownable {
    ...
    function synchData(address user) public onlyOwner {

        uint256 vehicleNums;
        uint256 userNum;

        uint256[] memory ids = UTC.querymyVehiclesIds(user);

        for(uint256 i = 0; i < ids.length; i++){
            if(ids[i] < 17630){
                (uint256 types,uint256 buyDays,uint256 vehicleState,uint256 expireTime,uint256 durability,uint256 tokenId,address hold,uint256 buyPrice,uint256 profit,uint256 totalProfit,uint256 sellPrice) = UTC.allVehicles(ids[i]);
        ...
        if(!isUsers[user]){
            ...
            (uint256 vehicleNum,,uint256 bindNum,,uint256 totalDeposit,uint256 UTAXIReward,bool isEffectiveUser) = UTC.users(user);

            uint256[6] memory vehicleTypeNum = UTC.queryVehicleTypeNum(user);

            UTD.setUsers(user,vehicleTypeNum,vehicleNum, address(0), bindNum, 0, totalDeposit, UTAXIReward, isEffectiveUser);

            if(isEffectiveUser){
                UTD.addEffectiveUserNum(user);
            }
    ...
    function synchData() public {

        uint256 vehicleNums;
        uint256 userNum;

        uint256[] memory ids = UTC.querymyVehiclesIds(msg.sender);
        ...
        if(!isUsers[msg.sender]){

            userNum++;

            (uint256 vehicleNum,,uint256 bindNum,,uint256 totalDeposit,uint256 UTAXIReward,bool isEffectiveUser) = UTC.users(msg.sender);

            uint256[6] memory vehicleTypeNum = UTC.queryVehicleTypeNum(msg.sender);

            UTD.setUsers(msg.sender,vehicleTypeNum,vehicleNum, address(0), bindNum, 0, totalDeposit, UTAXIReward, isEffectiveUser);
