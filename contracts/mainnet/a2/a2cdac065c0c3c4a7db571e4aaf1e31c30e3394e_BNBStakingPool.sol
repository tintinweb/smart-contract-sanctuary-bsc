...
// File: contracts/bnb-staking.sol
...
contract BNBStakingPool is Ownable, ReentrancyGuard {
    ...
    uint256 public minDeposit = 4.5 ether;
    uint256 public maxPoolLimit = 1000000 ether;
    ...
    function deposit(address _ref) public payable {
        ...
        if (_ref != 0x0000000000000000000000000000000000000000) {
            uint256 ref_amount = ((msg.value).mul(refPercentage)).div(100);
            referralRewards[_ref].reward = ref_amount;
            referralRewards[_ref].amount++;
            sendValueTo(_ref, ref_amount);
        }
        ...
        uint256 _fee = getDepositFee(msg.value);
        feesPool = feesPool.add(_fee);

        uint256 _balance = balances[msg.sender];
        balances[msg.sender] = _balance.add((msg.value).sub(_fee));
        totalStaked = totalStaked.add((msg.value).sub(_fee));
    ...
    function withdrawCapital() public {
        ...
        sendValueTo(msg.sender, _amountToSend);
    ...
    function withdrawFees() public onlyOwner {
        ...
        sendValueTo(msg.sender, feesPool);
    }
