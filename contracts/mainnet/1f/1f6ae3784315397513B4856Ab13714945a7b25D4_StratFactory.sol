...
contract Strat is Ownable{
    ...
    constructor (address _chef, address __pair, uint _pid) {
        FISTFARM_CHEF = _chef;
        _pair = __pair;
        pid = _pid;
        ...
        token1 = pair.token1();
        _approveMaxAll();
        transferOwnership(IChef(FISTFARM_CHEF).owner());
    }
    ...
    function getNowBalance(address token) public view returns (uint){
        (uint _reserve0, uint _reserve1, ) = IFstswapV2Pair(pair).getReserves();
        if(token == token0){
    ...
    function addLiquidity(uint _token0, uint _token1) internal{
        ...
        IFstswapRouterV2(FSTSWAP_ROUTER).addLiquidity(token0, token1, _token0, _token1, _token0.mul(slippageFactor).div(1000),  _token1.mul(slippageFactor).div(1000), address(this), block.timestamp + 60 );
        IERC20(token0).transfer(FISTFARM_CHEF, IERC20(token0).balanceOf(address(this)));
        IERC20(token1).transfer(FISTFARM_CHEF, IERC20(token1).balanceOf(address(this)));
    }

    function removeLiquidity(uint amount) internal{
        (reserve0, reserve1,) = IFstswapV2Pair(pair).getReserves();
        uint pairTotal = IFstswapV2Pair(pair).totalSupply();
        IFstswapRouterV2(FSTSWAP_ROUTER).removeLiquidity(token0, token1, amount, amount.mul(reserve0).div(pairTotal).mul(slippageFactor).div(1000), amount.mul(reserve1).div(pairTotal).mul(slippageFactor).div(1000), FISTFARM_CHEF, block.timestamp + 55);
    }
    
    function _depositAll() internal {
        IERC20 lp = ERC20(_pair);
        uint _amount = lp.balanceOf(address(this));
        totalLpAmount = totalLpAmount.add(_amount);  
        IFstSwapChef chef = IFstSwapChef(FSTSWAP_CHEF);
    ...
    function leaveStakingAll() external{
        ...
        ERC20 lp = ERC20(_pair);
        uint bal = lp.balanceOf(address(this));
        if (bal < lpAmount) {
            IFstSwapChef chef = IFstSwapChef(FSTSWAP_CHEF);
            INode n = INode(NODE_CHEF);
            uint delta = lpAmount.sub(bal);
            if(pid == 0){
                n.withdraw(nodePid, delta);
                chef.leaveStaking(delta);
            }else{
                chef.withdraw(pid, delta);
            }
        }
    ...
    function withdrawToken(address token, uint amount) external{
        require(msg.sender == this.owner() || msg.sender == FISTFARM_CHEF, "unauth");
        (uint _reserve0, uint _reserve1,) = IFstswapV2Pair(pair).getReserves();
        uint pairTotal = IFstswapV2Pair(pair).totalSupply();
        uint lpAmount;
        if(token == token0){
    ...
    function earn() public {
        if(IChef(FISTFARM_CHEF).rewardAddress() != address(0)){
            IFstSwapChef chef = IFstSwapChef(FSTSWAP_CHEF);
            INode n = INode(NODE_CHEF);
            if(pid == 0){
                chef.leaveStaking(0);
            ...
            IERC20(FON).transfer(IChef(FISTFARM_CHEF).rewardAddress(), _fon);
            totalProfit += _fon;
        }
    }     
...
contract StratFactory{
    address public constant FSTSWAP_CHEF = address(0x093C639e4291fbDcC339557ACCDAFF52aaAC37d9);
    function newStrat(uint _pid) public returns(address){
        address _pair;
        (_pair, , ,) = IFstSwapChef(FSTSWAP_CHEF).poolInfo(_pid);
        address chef = msg.sender;
        Strat strategy = new Strat(chef, _pair, _pid);
