...
/**
    The Monetrip Protocol is a decentralized finance (DeFi) token.
    ...
    Then a tax(which defaults to 5% of every transfer and can be set by operator anytime) is charged during
    ...
    transferTaxRate is 0(which defaults to 500 set by the owner when the contract deploys and can be
    changed by operator after)
*/

contract OneToken is IERC20, Ownable {
    ...
    string private constant _name = 'MONE TRIP';
    string private constant _symbol = 'MONETRIP';
    ...
    modifier onlyGnosisSafeWallet() {
        require(gnosisSafeWallet == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
    ...
    modifier antiWhale(address sender, address recipient, uint amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "OneToken::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }

    constructor(address _TokenStrategy, address _gnosisSafeWallet) {
        _decimals = 18;
        gnosisSafeWallet = _gnosisSafeWallet;
        BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
        require(_TokenStrategy != address(0), "OneToken::constructor: invalid TokenStrategy");
        TokenStrategy = _TokenStrategy;

        transferTaxRate = 500;
        MAXIMUM_TRANSFER_TAX_RATE = 1000;
        maxTransferAmountRate = 5;
     ...
     */function mint(uint256 amount) external onlyGnosisSafeWallet returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint amount) internal virtual antiWhale(sender, recipient, amount) {
        if (_excludedFromTax[sender] || recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            if(taxAmount > 0) __transfer(sender, TokenStrategy, taxAmount);

            __transfer(sender, recipient, sendAmount);
        }
    }
     ...
     */function updateTransferTaxRate(uint16 _transferTaxRate) external onlyGnosisSafeWallet {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "OneToken::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
     ...
     */function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) external onlyGnosisSafeWallet {
        require(_maxTransferAmountRate <= 10000, "OneToken::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     */function setExcludedFromAntiWhale(address _account, bool _excluded) external onlyGnosisSafeWallet {
        require(_account != address(0), "OneToken::setExcludedFromAntiWhale: invalid account");
    ...
    function setExcludedFromTax(address _account, bool _excluded) external onlyGnosisSafeWallet {
        require(_account != address(0), "OneToken::setExcludedFromTax: invalid account");
