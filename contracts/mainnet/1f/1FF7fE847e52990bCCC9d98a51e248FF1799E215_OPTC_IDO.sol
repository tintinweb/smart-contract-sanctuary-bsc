...
contract OPTC_IDO is OwnableUpgradeable {
    ...
    function getStmPrice() public view returns (uint){
        uint stmAmount = stm.balanceOf(stmPair);
        uint usdtAmount = usdt.balanceOf(stmPair);
        return usdtAmount * 1e18 / stmAmount;
    }
    ...
    function buyIDO(bool withStm, address invitor) public onlyEOA {
        ...
        if (!withStm) {
            usdt.transferFrom(msg.sender, address(this), IDOPrice);
        } else {
            usdt.transferFrom(msg.sender, address(this), IDOPrice / 2);
            stm.transferFrom(msg.sender, address(this), IDOPrice * 1e18 / 2 / getStmPrice());
        }
...
interface IRefer {
    ...
    function getUserReferLevel(address addr) external view returns (uint);
