...
contract LPLocker is Ownable, ReentrancyGuard {
    ...
    struct FeeStruct {
        uint256 ethFeeLock; // Small eth fee to prevent spam on the platform
        uint256 ethFeeRelock; // Small eth fee to prevent spam on the platform
        uint256 ethFeeIncrementLock; // Small eth fee to prevent spam on the platform
        uint256 ethFeeSplitLock; // Small eth fee to prevent spam on the platform
    ...
    constructor() {
        feeAddress = payable(0x13Ccd2fa8af0439492401DE2f0a29180ba03d421);
        gFees.referralPercent = 2500; // 25%
        gFees.ethFeeLock = 0.05 ether;
        gFees.secondaryTokenFee = 100e18;
        gFees.secondaryTokenDiscount = 2000; // 20%
        gFees.referralHold = 10e18;
        gFees.referralDiscount = 1000; // 10%
    }
   ...
   */
    ...
    function setFees(
        ...
        uint256 _ethFeeLock, 
        uint256 _ethFeeRelock, 
        uint256 _ethFeeIncrementLock, 
        uint256 _ethFeeSplitLock,  
        ...
        public onlyOwner {
            ...
            gFees.ethFeeLock = _ethFeeLock;
            gFees.ethFeeRelock = _ethFeeRelock;
            gFees.ethFeeIncrementLock = _ethFeeIncrementLock;
            gFees.ethFeeSplitLock = _ethFeeSplitLock;
   ...
   */
    function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
        ...
        address factoryPairAddress = IFactory(lpair.factory()).getPair(lpair.token0(), lpair.token1());
        ...
        if (!feeWhitelist.contains(msg.sender)) {
            if (_fee_in_eth) { // charge fee in eth
                uint256 ethFee = gFees.ethFeeLock;
                if (_referral != address(0)) {
                    ethFee = ethFee.mul(10000 - gFees.referralDiscount).div(10000);
                }
                require(msg.value == ethFee, 'FEE NOT MET');
                uint256 devFee = ethFee;
                if (ethFee != 0 && _referral != address(0)) { // referral fee
                    uint256 referralFee = devFee.mul(gFees.referralPercent).div(10000);
                    _referral.transfer(referralFee);
                    devFee = devFee.sub(referralFee);
                }
                feeAddress.transfer(devFee);
            } else { // charge fee in token
                uint256 burnFee = gFees.secondaryTokenFee;
                if (_referral != address(0)) {
                    burnFee = burnFee.mul(10000 - gFees.referralDiscount).div(10000);
                }
                ...
                if (gFees.referralPercent != 0 && _referral != address(0)) { // referral fee
                    uint256 referralFee = burnFee.mul(gFees.referralPercent).div(10000);
        ...
        // percent fee
        uint256 liquidityFee = _amount.mul(gFees.liquidityFee).div(10000);
        if (!_fee_in_eth && !feeWhitelist.contains(msg.sender)) { // fee discount for large lockers using secondary token
            liquidityFee = liquidityFee.mul(10000 - gFees.secondaryTokenDiscount).div(10000);
        }
        if(liquidityFee > 0) {
            TransferHelper.safeTransfer(_lpToken, feeAddress, liquidityFee);
        }
   ...
   */
    function relock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _unlock_date) external payable nonReentrant {
        require(_unlock_date < 10000000000, 'TIMESTAMP INVALID'); // prevents errors when timestamp entered in milliseconds
        uint256 ethFee = gFees.ethFeeRelock;
        require(msg.value == ethFee, 'FEE NOT MET');
        ...
        if(gFees.liquidityFee > 0) {
            uint256 liquidityFee = userLock.amount.mul(gFees.liquidityFee).div(10000);
            TransferHelper.safeTransfer(_lpToken, feeAddress, liquidityFee);
            amountLocked = userLock.amount.sub(liquidityFee);
        } else {
            amountLocked = userLock.amount;
        }

        userLock.amount = amountLocked;
        userLock.unlockDate = _unlock_date;

        emit onDeposit(_lpToken, msg.sender, amountLocked, userLock.lockDate, _unlock_date);
    }
   ...
   */
    function incrementLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
        require(_amount > 0, 'ZERO AMOUNT');
        uint256 ethFee = gFees.ethFeeIncrementLock;
        require(msg.value == ethFee, 'FEE NOT MET');
        uint256 lockID = users[msg.sender].locksForToken[_lpToken][_index];
        TokenLock storage userLock = tokenLocks[_lpToken][lockID];
        require(lockID == _lockID && userLock.owner == msg.sender, 'LOCK MISMATCH'); // ensures correct lock is affected
   ...
   */
    function splitLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
        ...
        require(msg.value == gFees.ethFeeSplitLock, 'FEE NOT MET');
        feeAddress.transfer(gFees.ethFeeSplitLock);
