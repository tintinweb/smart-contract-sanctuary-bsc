...
contract ElonsBadDay is IBEP20, Auth {
    ...
    uint256 SellFee1= 10000;


    //Charity/Team/Marketing percentages
    uint256 Team1Mult = 20;
    uint256 Team2Mult = 20;
    uint256 Team3Mult = 20;
    uint256 Team4Mult = 20;
    uint256 MarketingMult = 10;
    uint256 CharityMult = 10;
    ...
    address public team1FeeReceiver;
    address public team2FeeReceiver;
    address public team3FeeReceiver;
    address public team4FeeReceiver;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require( isTokenLive, "HMF is not live yet");
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            return totalFee.mul(SellFee1).div(feeDenominator);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBProject = amountBNB.mul(marketingFee).div(totalBNBFee);
        ...
        try distributor3.deposit{value: amountBNBReflection3}() {} catch {}
          ...
          if(CharityEnabled){
        uint256 amountBNBforT1 = amountBNBProject.mul(Team1Mult).div(100);
        uint256 amountBNBforT2 = amountBNBProject.mul(Team2Mult).div(100);
        uint256 amountBNBforT3 = amountBNBProject.mul(Team3Mult).div(100);
        uint256 amountBNBforT4 = amountBNBProject.mul(Team4Mult).div(100);
        uint256 amountBNBforCH = amountBNBProject.mul(CharityMult).div(100);
        uint256 amountBNBforMKTG = amountBNBProject.mul(MarketingMult).div(100);
   
            payable(team1FeeReceiver).transfer(amountBNBforT1);
            payable(team2FeeReceiver).transfer(amountBNBforT2);
            payable(team3FeeReceiver).transfer(amountBNBforT3);
            payable(team4FeeReceiver).transfer(amountBNBforT4);
            payable(charityFeeReceiver).transfer(amountBNBforCH);
            payable(marketingFeeReceiver).transfer(amountBNBforMKTG);


           }else{
                 payable(marketingFeeReceiver).transfer(amountBNBProject);
                 }
    ...
    function setBuybackMultiplierSettings(uint256 numerator, uint256 denominator, uint256 length,uint256 _sellFee1) external authorized {
    ...
    function setTeamMult(uint256 _Team1Mult, uint256 _Team2Mult, uint256 _Team3Mult,uint256 _Team4Mult,uint256 _charityMult,uint256 _marketingMult,uint256 total) external authorized {
        Team1Mult = _Team1Mult;
        Team2Mult = _Team2Mult;
        Team3Mult = _Team3Mult;
        Team4Mult = _Team4Mult;
        CharityMult =   _charityMult;
        MarketingMult = _marketingMult;
        total = Team4Mult + Team3Mult + Team2Mult + Team1Mult + CharityMult + MarketingMult;
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _team1FeeReceiver, address _team2FeeReceiver,address _team3FeeReceiver,address _team4FeeReceiver,address _charityFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        team1FeeReceiver = _team1FeeReceiver;
        team2FeeReceiver = _team2FeeReceiver;
        team3FeeReceiver = _team3FeeReceiver;
        team4FeeReceiver = _team4FeeReceiver;
