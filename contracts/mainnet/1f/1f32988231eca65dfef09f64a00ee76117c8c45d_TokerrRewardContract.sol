...
interface DivFactory{
    function divSetup(address _router, address _reward, address _token) external returns (address);
}
...
contract TokerrRewardContract is IERC20, Auth {
    ...
    constructor (uint[] memory numbers, address[] memory addresses, string[] memory names, 
                address antiBot, address builder, address _divs, address _launch) Auth(msg.sender) {
        router = IDEXRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        pair = IDEXFactory(router.factory()).createPair(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c, address(this));

        divs = DivFactory(_divs);
        
        distributor = IDividendDistributor(divs.divSetup(address(router), addresses[4], address(this)));
        ...
        totalBuyFee = marketingBuyFee.add(liquidityBuyFee).add(devBuyFee).add(rewardBuyFee).add(5);
        ...
        totalSellFee = marketingSellFee.add(liquiditySellFee).add(devSellFee).add(rewardSellFee).add(5);
        ...
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee).add(10);
    ...
    function exemptFromDividends(address holder, bool _enabled) external authorized{
        isDividendExempt[holder] = _enabled;
    }
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, 
        ...
        require((_marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee)) <= 100);
        ...
        totalBuyFee = _marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee).add(5);
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee).add(10);
    }
    ...
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, 
        ...
        totalSellFee = _marketingFee.add(_liquidityFee).add(_devFee).add(_rewardFee).add(5);
        totalFee = liquidityFee.add(marketingFee).add(devFee).add(rewardFee).add(10);
    }
