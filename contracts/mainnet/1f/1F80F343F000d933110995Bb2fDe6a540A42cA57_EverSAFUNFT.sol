...
contract EverSAFUNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct CollectionNFT {string name; string nfttype;}
    mapping(uint8 => CollectionNFT) public sbox;

    event TokenMint(address indexed to, uint indexed tokenId, string name, string nfttype);

    function initialize() public initializer {
        __ERC721_init("EverSAFUNFT", "EverSAFUNFT");
        ...
        _internalBaseURI = "https://nft.eversafu.com/";

        maxnftcount = 1000; //max box
    ...
    function setBoxCount(uint256 count) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxnftcount = count;
    }
 
    function addCollection(uint8[] memory id, string[] memory nftName, string[] memory nftType) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(id.length == nftType.length && id.length == nftName.length, 'Input not true');
        for(uint8 i = 0; i < nftName.length ; i ++) {
            sbox[id[i]] = CollectionNFT(nftName[i], nftType[i]);
        }
    }
    function editCollection(uint8 _id, string memory nftName, string memory nftType) external onlyRole(DEFAULT_ADMIN_ROLE) {
        sbox[_id].name = nftName;
        sbox[_id].nfttype = nftType;
    }  
    function editNFT(uint256 tokenId, string memory nftName, string memory nftType) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _nft[tokenId].name = nftName;
        _nft[tokenId].nfttype = nftType;
    }    
    ...
    function mint(address to, uint8 nft) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant returns(uint256) {
        ...
        _nft.push(NFTLibrary.NFT(sbox[nft].name, sbox[nft].nfttype, tokenId, false));
    ...
    function _getNft(uint256 _nftId) internal view returns (NFTLibrary.NFT memory) {
        require(_nftId < _nft.length, "Does not exist Box");
        NFTLibrary.NFT memory collection = _nft[_nftId];
        return collection;
    }
    ...
    function _safeMint(address to, uint256 tokenId) internal override {
        ...
        emit TokenMint(to, tokenId, _nft[tokenId].name, _nft[tokenId].nfttype);
    }
