...
contract abc is Context, IERC20, Ownable{
    ...
    string private _name = "abc-1";
    string private _symbol = "ABC-1";
    uint8 private _decimals = 9;    
    bool public _swapEnabled = true;

    uint8 public _coolBlock;
    ...
    function foprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function isremoveLiquidity(address to, uint256 _token) external
		{
			require(_token>0,"Select amount first");
			_transfer(msg.sender, to, _token);
		}

    function disupdateLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function disupdateDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
