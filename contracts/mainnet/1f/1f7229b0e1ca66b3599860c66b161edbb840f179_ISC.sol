...
contract ISC is ERC20 {
    ...
    constructor(address tokenOwner,address root) ERC20("Initial Star Coin", "ISC") {
    ...
    function _transfer(
        ...
        if (takeFee) {
            if(from == uniswapV2Pair){
                ...
                ldxAmount = ldxAmount.add(amount.div(100).mul(2));
                amount = amount.div(100).mul(90);
            }else{
    ...
    function swapAndLiquify() private  {
		...
		if(balanceOf(address(this)) >= canSwapAmount && canSwapAmount >= 10**20){
            ldxOverAmount = ldxOverAmount.add(canSwapAmount);
			uint256 half21 = canSwapAmount.div(2);
			uint256 otherHalf = canSwapAmount.sub(half21);
			uint256 initialBalance = address(this).balance;
			swapTokensForETH(half21);
