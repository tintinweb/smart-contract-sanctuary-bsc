...
contract EmpireToken is IERC20, Ownable {
    ...
    function _takeMarketingAndBurn(uint256 tMarketing, uint256 tBurn) private {
        ...
        uint256 rMarketing = tMarketing.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rBurn).add(
            rMarketing
        );
        if (_isExcluded[address(this)])
        ...
        lockTheSwap
    {
        ...
        uint256 forMarketing = contractTokenBalance.mul(marketingFee).div(
            total
        );
