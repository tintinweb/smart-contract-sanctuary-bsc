...
contract MLGoToken is Context, IERC20, Ownable {
    ...
    address public MLAddr = 0x779ed4FFfDFcdD6bCdBe6F6826E528B0AC999999;
    ...
    constructor (
        ...
        _isExcludedFromFees[teamWalletAddress] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        //check if tradingEnabled 
        if (!_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient]) {
            require(tradingEnabled, "Trading not enabled");
        }

        if(!_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient] && block.number < genesis_block + deadblock){
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !isMarketPair[sender] && swapAndLiquifyEnabled) 
            {
                ...
                swapTokensForML(address(this).balance);  
    ...
    function swapTokensForML(uint256 tokenAmount) private lockTheSwap{
    ...
    function setTotalTaxBuyingFee(uint256 value) external onlyOwner {
        _totalTaxIfBuying = value;
    }

    function setTotalTaxSellingFee(uint256 value) external onlyOwner {
