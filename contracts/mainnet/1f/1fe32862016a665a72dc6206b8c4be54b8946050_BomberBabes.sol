...
contract BomberBabes is ERC20Detailed, Ownable {
    ...
    uint256 public totalFee = liquidityFee.add(marketingFee).add(developmentFee).add(vaultFee);
    uint256 public feeDenominator = 1000;
    uint256 public rebaseInterval = 60 minutes;
    uint256 public rebaseRate = 9998;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10_000_000e5;
    uint256 public maxSupply = 100_000_000e5;
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (365 days)) {
            ...
            _rebaseRate = rebaseRate / 10;
        } else if (deltaTimeFromInit >= ((15 * 365 days) / 10)) {
            _rebaseRate = rebaseRate / 100;
        } else if (deltaTimeFromInit >= (7 * 365 days)) {
            _rebaseRate = rebaseRate / 1000;
        }

        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply.mul((10**RATE_DECIMALS).add(_rebaseRate)).div(10**RATE_DECIMALS);
        }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_marketingFee.add(developmentFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(marketingReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(marketingFee).div(marketingFee.add(developmentFee)),
            gas: 30000
        }('');
        (success, ) = payable(developmentReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(developmentFee).div(marketingFee.add(developmentFee)),
            gas: 30000
        }('');
    }
    ...
    function setFeeStructure(
        ...
        sellFee = _sellFee;
        vaultFee = _vaultFee;
        totalFee = liquidityFee.add(marketingFee).add(developmentFee).add(vaultFee);
    }
