...
contract Dantest3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Dantest3DividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public devWallet;
    address payable public moonshotWallet;
    ...
    uint256 public buyMoonshotFee;
    uint256 public sellMoonshotFee;
    ...
    event UpdateFees(
        ...
        uint256 buyDevFee,
        uint256 sellDevFee,
        uint256 buyMoonshotFee,
        uint256 sellMoonshotFee
    );
    ...
    constructor() ERC20("Dantest3", "DT3") {
        marketingWallet = payable(0xd5Ec3679ef8f050B3A47E3fd8c6F8f7Dde93fA18);
        devWallet = payable(0x7D3671fD93E4a83B0E2264ff4b75447C6942f6E3);
        moonshotWallet = payable(0x28AeF20C6CaBE81C6D17DD5e1e8C02BF1BbD0cfa);
        ...
        dividendTracker = new Dantest3DividendTracker(
            payable(this),
            router,
        ...
        _isExcludedFromFees[address(moonshotWallet)] = true;
    ...
    function updateFees(
        ...
        require(devBuy <= 5 && devSell <= 5, "Dev fees can't exceed 5%");
        require(moonshotBuy <= 5 && moonshotSell <= 5, "Moonshot fees can't exceed 5%");
        buyDeadFees = deadBuy;
        ...
        buyDevFee = devBuy;
        sellDevFee = devSell;
        buyMoonshotFee = moonshotBuy;
        sellMoonshotFee = moonshotSell;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee
                .add(liquidityFee + marketingFees + devFees + moonshotFees);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFees + buyDevFee + buyMoonshotFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFees + sellDevFee + sellMoonshotFee);
        }
        ...
        uint256 _completeFees = sellMarketingFees.add(sellDevFee + sellMoonshotFee) +
            buyMarketingFees.add(buyDevFee + buyMoonshotFee);
        ...
        uint256 moonshotPayout = buyMoonshotFee.add(sellMoonshotFee) * feePortions;
        ...
        if (moonshotPayout > 0) {
            (successOp3, ) = address(moonshotWallet).call{value: moonshotPayout}("");
        }

        emit SendDividends(
            dividends,
            marketingPayout + devPayout + moonshotPayout,
...
contract Dantest3DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Dantest3 public Dantest3Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        Dantest3Contract = Dantest3(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Dantest3_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Dantest3_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Dantest3 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Dantest3Contract);

            uint256 prevBalance = Dantest3Contract.balanceOf(address(this));
            ...
            {
                uint256 received = Dantest3Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Dantest3Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
