...
contract CardMerge is SafeOwnable {
    using SafeERC20 for IERC20;

    ERC721 public oldCard;
    ...
    event Merge(address account, uint256[] _type);

    constructor(address _nftCard, address _oldCard) {
        nftCard = IDNCCard(_nftCard);
        oldCard = ERC721(_oldCard);
    }
    ...
    function merge(uint256[] memory tokenId) external {
        ...
        require(tokenId.length == 9, "tokenId min 9");
        for (uint256 index = 0; index != tokenId.length; index++) {
            oldCard.safeTransferFrom(msg.sender, hole, tokenId[index]);
        }

        nftCard.mint(msg.sender, 1);
