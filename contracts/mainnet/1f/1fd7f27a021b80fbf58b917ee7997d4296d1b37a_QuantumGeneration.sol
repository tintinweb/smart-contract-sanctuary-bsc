...
contract QuantumGeneration is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet = payable(0xFA199fb54921f5A52692988a69f8432dc2FB9EEF);
    ...
    string private constant _name = "Quantum Generation";
    string private constant _symbol = "Q369";
    ...
    uint256 public _maxWalletHoldingLimit = 20 * 10 ** 9 * 10 ** 9;
    ...
    function setMarketingWallet(address payable newMarkeitngWallet) external onlyOwner() {
        require(newMarkeitngWallet != address(0), "You can't set Zero Address");
        MarketingWallet = newMarkeitngWallet;
    }

    function setBuyTaxFeePercentage(uint256 taxFee, uint256 liquidityFee, uint256 burnFee, uint256 MarketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(burnFee).add(MarketingFee) <= 25);
        _taxFee = taxFee;
        _previousTaxFee = _taxFee;
        ...
        emit BuyTaxFeeUpdated(taxFee.add(liquidityFee).add(burnFee).add(MarketingFee));
    }

    function setSellTaxFeePercentage(uint256 taxFee, uint256 liquidityFee, uint256 burnFee, uint256 MarketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(burnFee).add(MarketingFee) <= 25);
        _sellTaxFee = taxFee;
       ...
       emit SellTaxFeeUpdated(taxFee.add(liquidityFee).add(burnFee).add(MarketingFee));
    }
    ...
    function setMaxWalletHoldingPercentage(uint256 maxWalletPercentage) external onlyOwner() {
        require(maxWalletPercentage >= 1);
        _maxWalletHoldingLimit = _tTotal.mul(maxWalletPercentage).div(
            10 ** 2
        );
    ...
    function _transfer(
        ...
        if (!_isExcludedFromWalletHoldingLimit[to] && from != owner()) {
            require(balanceOf(to).add(amount) <= _maxWalletHoldingLimit, "Wallet Holding limit exceeding");
        }
        if(balanceOf(DeadWallet) >= _tTotal.mul(25).div(100)){
            _burnFee = 0;
        }
   ...
   function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private
    {
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || _isAtomaticMarketMaker[sender] || _isAtomaticMarketMaker[recipient]) {
            restoreAllFee();
        }
    }
