...
// Mad Dog DAO @ telegram: https://t.me/maddogDao
// NFT ON THE GOING airdrop to the 1000 big holder 
// Rewards doge coin every swaping： 3%
// Every week superJackpot Reward to the strongest community leader：2%
// rewards, liquidity, marketing, dev, superJackpot
// [3,3,4,3,3]
// total fee : 3 + 3 + 4 + 3 + 3 = 16
// buy & sell Slippage set to : 16 or more.
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    uint256 public superJackpotFee;
    uint256 public totalFees;

    address public _marketingWalletAddress;
    address public _devWalletAddress;
    address public _superJackpotWalletAddress;
    ...
    constructor(
        ...
        address[6] memory addrs, // reward, router, marketing wallet, dividendTracker, dev, superJackpot
        uint256[5] memory feeSettings, // rewards, liquidity, marketing, dev, superJackpot
        uint256 minimumTokenBalanceForDividends_
        ...
        _devWalletAddress = addrs[4];
        require(
            msg.sender != _devWalletAddress,
            "Owner and dev wallet cannot be the same"
        );

        _superJackpotWalletAddress = addrs[5];
        require(
            msg.sender != _superJackpotWalletAddress,
            "Owner and super_jackpot wallet cannot be the same"
        );

        tokenRewardsFee = feeSettings[0];
        liquidityFee = feeSettings[1];
        marketingFee = feeSettings[2];
        devFee = feeSettings[3];
        superJackpotFee = feeSettings[4];
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(devFee).add(superJackpotFee);
    ...
    function updateRewardToken(address newAddress) public onlyOwner {
        require(
            newAddress != address(rewardToken),
            "BABYTOKEN: The rewardToken already has that address"
        );
    ...
    function setSuperJackpotWallet(address payable wallet) external onlyOwner {
        _superJackpotWalletAddress = wallet;
    }
    ...
    function setSuperJackpotFee(uint256 value) external onlyOwner {
        superJackpotFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(devFee).add(superJackpotFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
