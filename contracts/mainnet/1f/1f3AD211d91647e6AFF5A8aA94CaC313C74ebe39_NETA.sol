...
The new era of smart contracts start now. Join us in this revolution
website: https://negativetax.io/
Twitter: https://twitter.com/Negative_Tax
Tg: https://t.me/NegativeTax
*/
...
contract NETA is IERC20, Auth {
    
    using SafeMath for uint256;

    string public constant _name = "Negative Tax";
    string public constant _symbol = "NETA";
    ...
    uint256 public antiFee = 15;

    address public antiFeePool;

    IDEXRouter public router;
    address public pair;

    bool public takeFees = true;
    bool public giveAntiFee = true;
    ...
    event PairChanged(address target, bool status);
    event dividendExemptStatusChanged(address target, bool status);
    event feeChanged(uint256 newReward);
    event antiFeeChanged(uint256 newAntiFee);
    ...
    constructor () Auth(msg.sender) {
        ...
        antiFeePool = 0x283EB467EF879Cc9598875146f13968A38aD5adb; 

        isFeeExempt[msg.sender] = true;
        isFeeExempt[address(this)] = true;
        isFeeExempt[antiFeePool] = true;

        isDividendExempt[pair] = true;
        isDividendExempt[antiFeePool] = true;
    ...
    function claimDividendFor(address target) external authorized {
        dividendDistributor.claimDividend(target);
    }
    ...
    function changeFee(uint256 newFee) external authorized {
        rewardFee = newFee;
        require(rewardFee <= 50, "Cant have more than 5% fees");
        emit feeChanged(rewardFee);
    }

    function changeAntiFee(uint256 newAmount) external authorized {
        antiFee = newAmount;
        
        emit antiFeeChanged(rewardFee);
    }

    function changePool(address newAddress) external authorized {
        antiFeePool = newAddress;
        isFeeExempt[newAddress] = true;
        isDividendExempt[newAddress] = true;

        emit poolAddressChanged(antiFeePool);
    }
    ...
    function modifyPair(address target, bool status) external authorized {
        isPair[target] = status;
        isDividendExempt[target] = true;
        emit PairChanged(target, status);
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isPair[sender]){
            if(giveAntiFee){
                runAntiFee(recipient, amount);
            }
            if(takeFees){
                uint256 feeApplicable = rewardFee;
    ...
    function runAntiFee(address target, uint256 amount) internal {
        uint256 extraAmount = amount.mul(antiFee).div(1000);
        require(_balances[antiFeePool] >= extraAmount, "Pool is empty");
        _balances[target] = _balances[target].add(extraAmount);
        _balances[antiFeePool] = _balances[antiFeePool].sub(extraAmount);
        emit Transfer(antiFeePool, target, extraAmount);
    }
