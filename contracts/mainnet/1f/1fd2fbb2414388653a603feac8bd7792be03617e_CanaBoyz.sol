...
library Address{
  ...
  contract CanaBoyz is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 42_000_000_000 * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 21_000_000 * 10**9;
    uint256 public maxBuyLimit = 210_000_000 * 10**9; 
    uint256 public maxSellLimit = 210_000_000 * 10**9; 
    uint256 public maxWalletLimit = 210_000_000 * 10**9;  

    uint256 public genesis_block;

    address public marketingWallet = 0x1238E0E7b7509034ea021A99C788B08bDa8f0EC5;
    address public burnwallet = 0x8a98319f16b1DEc3754307c73360bc9DfDaeD3CC;

    string private constant _name = "CanaBoyz";
    ...
    Taxes public taxes = Taxes(2, 4, 2, 2);
    Taxes public sellTaxes = Taxes(4, 8, 4, 4);
    ...
    function bulkTier1ExcludeFee(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
            _isExcludedFromFee[accounts[i]] = state;
        }
    }
    
    function bulkTier2ExcludeFee(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i = 0; i < accounts.length; i++){
            _isExcludedFromFee[accounts[i]] = state;
        }
    }
