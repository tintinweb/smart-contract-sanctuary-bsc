...
// DRIVENfarm is the master of DVX. He can make DVX and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once DVX is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract DrivenFarm is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of DVXs
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DVXs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DVXs distribution occurs.
        uint256 accTokenPerShare;   // Accumulated DVXs per share, times 1e12. See below.
    ...
    // DVX tokens created per block.
    uint256 public rewardPerBlock;
    // Bonus muliplier for early DVX makers.
    ...
    // The block number when DVX mining starts.
    ...
    constructor(IERC20 _DVX, uint256 _rewardPerBlock) {
        DVX = _DVX;
    ...
    // Update the given pool's DVX allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending DVX on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 DVXReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTokenPerShare = accTokenPerShare.add(DVXReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DVXReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(DVXReward.mul(1e12).div(lpSupply));

        
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to DRIVENfarm for DVX allocation.
    ...
    // Withdraw LP tokens from DRIVENfarm.
    ...
    // Safe DVX transfer function, just in case if rounding error causes pool to not have enough DVXs.
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 DVXBal = DVX.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > DVXBal) {
            transferSuccess = DVX.transfer(_to, DVXBal);
        } else {
            transferSuccess = DVX.transfer(_to, _amount);
        }
    ...
    function emergencyWithdrawRewards(uint256 _amount) external onlyOwner {
        if(_amount == 0) {
            uint256 amount = DVX.balanceOf(address(this));
    ...
    function recoverWrongToken(address _token) external onlyOwner {
        require(_token != address(DVX), "cannot recover reward token or reflection token");
