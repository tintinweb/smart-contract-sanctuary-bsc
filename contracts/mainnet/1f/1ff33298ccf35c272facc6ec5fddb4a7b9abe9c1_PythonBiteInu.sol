...
contract PythonBiteInu is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xFe1d93b05d1E93B07AD6D0dc178cD19934bB9EcD;
    address public venturefundWallet = 0x44E1919fcBA97a6ee446f965bD3F54F9a3308e0C;
    address private operationsWallet = 0xd9848cf468C04b33E7Ed6C74b5EF1CD65a6f751B;

    string private constant _name = "Python Bite Inu";
    ...
    Taxes public taxes = Taxes(0, 4, 2, 1, 0);
    Taxes public sellTaxes = Taxes(0, 4, 2, 1, 0);
    ...
    struct valuesFromGetValues{
      ...
      uint256 rMarketing;
      uint256 rLiquidity;
      uint256 rventurefund;
      uint256 roperations;
      uint256 tTransferAmount;
      uint256 tRfi;
      uint256 tMarketing;
      uint256 tLiquidity;
      uint256 tventurefund;
      uint256 toperations;
    }
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[venturefundWallet] = true;
        ...
        allowedTransfer[venturefundWallet] = true;
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, venturefund, operations);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _venturefund, uint256 _operations) public onlyOwner {
       require(_rfi+(_marketing)+(_liquidity)+(_venturefund)+(_operations) <=22, "Buy Taxes cannot be higher than 22%");
       
       taxes = Taxes(_rfi,_marketing,_liquidity,_venturefund,_operations);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _venturefund, uint256 _operations) public onlyOwner {
        require(_rfi+(_marketing)+(_liquidity)+(_venturefund)+(_operations) <=22, "Sell Taxes cannot be higher than 22%");

       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_venturefund,_operations);
        emit FeesChanged();
    }
    ...
    function _takeventurefund(uint256 rventurefund, uint256 tventurefund) private {
        totFeesPaid.venturefund +=tventurefund;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tventurefund;
        }
        _rOwned[address(this)] +=rventurefund;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rventurefund, to_return.roperations) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tventurefund = tAmount*temp.venturefund/100;
        s.toperations = tAmount*temp.operations/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tventurefund-s.toperations;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rventurefund = s.tventurefund*currentRate;
        uint256 roperations = s.toperations*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rventurefund-roperations;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rventurefund,uint256 roperations) {

        if(!takeFee) {
          return(0,0);
        }

        rventurefund = s.tventurefund*currentRate;
        roperations = s.toperations*currentRate;
        return (rventurefund,roperations);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 1) {
            require(to != pair, "Sells not allowed for first 1 blocks");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tventurefund+ s.toperations);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.roperations > 0 || s.toperations > 0) _takeoperations(s.roperations, s.toperations);
        if(s.rventurefund > 0 || s.tventurefund > 0) _takeventurefund(s.rventurefund, s.tventurefund);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.venturefund + temp.operations) * 2;
        ...
        uint256 venturefundAmt = unitBalance * 2 * temp.venturefund;
        if(venturefundAmt > 0){
            payable(venturefundWallet).sendValue(venturefundAmt);
        }
        
        uint256 buybackAmt = unitBalance * 2 * temp.operations;
        if(buybackAmt > 0){
            payable(operationsWallet).sendValue(buybackAmt);
        }
    ...
    function updateventurefundWallet(address newWallet) external onlyOwner{
        venturefundWallet = newWallet;
    }
