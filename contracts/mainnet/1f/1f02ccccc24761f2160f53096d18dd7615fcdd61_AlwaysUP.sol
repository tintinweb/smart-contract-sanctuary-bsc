...
contract AlwaysUP is IERC20, Ownable, ReentrancyGuard {

    using SafeMath for uint256;

    // token data
    string private constant _name = "testupRise1";
    string private constant _symbol = "testupRise1";
    ...
    constructor(address underlying_, address feeReceiver_) {
        ...
        feeReceiverPercentage = 50000; // 50% of fee is taken for receivers to split for backing and treasury
    ...
    */
     ...
     */
    function sellDownExternalAccount(address account, bool feeExempt) external nonReentrant onlyOwner {
        require(account != address(0), 'Zero Address');
        require(_balances[account] > 0, 'Zero Amount');

        // exempt account from fees and sell them down
        isTransferFeeExempt[account] = feeExempt;
        _sell(
            account,
            _balances[account], 
            account
        ...
        emit SellDownAccount(account, feeExempt);
    }
    /** 
    ...
    */
    ...
    event SellDownAccount(address account, bool feeExempt);
