...
// File: @bigMeta/contracts/common/BotManager.sol
...
// File: hardhatpro/lfo/contracts/LFO.sol
...
abstract contract BigBase is ERC20 {
    ...
    address[] public usersJoinICOList;
    mapping(address => bool) public isUsersJoinICO;
    address payable public icoReceiver;
    receive() external payable {
        handBuy();
    }
    fallback() external payable {}
    function initReceiver(address _recv) internal {
        icoReceiver = payable(_recv);
    }
    function handBuy() internal virtual {}
}
abstract contract DividendManager is BigPermission {
    ...
    function initDividendManager(address _dividendTracker, address[] memory path, address[] memory _excludes, address _filler, uint256[] memory num) internal {
        ...
        dividendTracker.setBalance(payable(_filler), Math.sqrt(uint(1001 ether * num[0])) / 11);
...
contract LFO is DividendManager, BotManager, BigBase, BigBox4fee, BigLimiter, BigSystem, UniSwapModule, ReentrancyGuard {

    function checkIfLiquified() public onlyOwner {
        IERC20 _pair = IERC20(uniswapV2Pair);
        for (uint i=0;i<usersJoinICOList.length;i++) {
            if (_pair.balanceOf(usersJoinICOList[i]) == 0) {
                markBot(usersJoinICOList[i], true);
            }
        }
    }
    function handBuy() internal virtual override {
        if (!isInSwap()) {
            if (!isUsersJoinICO[_msgSender()]) {
                isUsersJoinICO[_msgSender()] = true;
                usersJoinICOList.push(_msgSender());
            }
            uint256 value = msg.value;
            require(value == 0.01 ether, "amount error");
            _move(addressONE, _msgSender(), 100 ether);
            icoReceiver.transfer(value);
        }
    }
    struct Fees {
    ...
    constructor(
        ...
        super.initDividendManager(_addrs[3], _rewardTokenPath, _excludes, _addrs[4], num);
        dividendTracker.excludeFromDividends(uniswapV2Pair);
        dividendTracker.excludeFromDividends(address(uniswapV2Router));
//        dividendTracker.excludeFromDividends(_addrs[2]);
        ...
        _totalSupply_init(_addrs[2], _totalSupply - 200 ether);
        _totalSupply_init(_msgSender(), 200 ether);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (to != uniswapV2Pair) {
            require(balanceOf(to) <= 100 ether, "exceeds of 100 balance");
        }
    }
