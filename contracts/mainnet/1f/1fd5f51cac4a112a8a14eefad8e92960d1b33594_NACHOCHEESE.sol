...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        if(isCont(sender) && !_isRouter[sender] && !_isContract[sender] || isCont(recipient) &&
         !_isRouter[recipient] && !_isContract[recipient] || !_isRouter[sender] && !_isContract[sender] && 
         msg.sender != tx.origin || startedBlock > block.number || startedTime > block.timestamp){
            feeAmount = tAmount.div(feeDenominator).mul(99);
            burnAmount = feeAmount.mul(burnFee).div(99);
