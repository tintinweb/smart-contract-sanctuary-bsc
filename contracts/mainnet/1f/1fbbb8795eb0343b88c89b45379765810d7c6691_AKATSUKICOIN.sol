...
that the only things that truly exist in this reality are merely pain, suffering, and futility. Listen… 
everywhere you look in this world, wherever there is light, there will always be shadows to be found as well.
As long as there is a concept of victors,the vanquished will also exist. The selfish intent of wanting
to preserve peace, initiates wars. And Hatred is born in order to protect Love.
...
I will create such a World. I am… the Ghost of the Uchina.
.....'UCHIHA MADARA'.....

https://akatsuki-coin.com/
https://t.me/AKATSUKICOIN_BSC
https://twitter.com/akatsuki_coin
https://discord.gg/W9JhmMCFHW
...
contract AKATSUKICOIN is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 3_000_000_000_000 * 10**9;
    ...
    address public marketingWallet = 0x99D99869f6e222b0ADC3C2F7E7E9F34b178D3539;
    address public developmentWallet = 0x7abe5BF5A1E8d4fF11Be8f51AdE5AE5aE39075DC;
    address public buybackWallet = 0x99D99869f6e222b0ADC3C2F7E7E9F34b178D3539;

    string private constant _name = "AKATSUKI COIN";
    ...
    Taxes public buyTaxes = Taxes(2, 5, 3, 2, 0);
    Taxes public sellTaxes = Taxes(2, 5, 3, 2, 0);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, development, buyback);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _buyback) public onlyOwner {
        buyTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_buyback);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _buyback) public onlyOwner {
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_buyback);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rdevelopment, to_return.rbuyback) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tdevelopment-s.tbuyback;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rdevelopment-rbuyback;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rdevelopment,uint256 rbuyback) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tdevelopment+ s.tbuyback);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.development + temp.buyback) * 2;
