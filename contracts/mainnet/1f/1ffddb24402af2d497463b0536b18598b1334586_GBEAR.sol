...
contract GBEAR is Context, IERC20, Ownable {
    ...
    address payable public _marketingWallet = 0xE45ca377CE235966342d21feF343d8C3B04bA926;
    ...
    function setMarketingWallet(address payable _newAddr) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 _halfFee = _liquidityFee.div(2);
        uint256 liquidityAmount = contractTokenBalance.mul(_halfFee).div(_liquidityFee + _marketingFee);
        uint256 swapAmount = contractTokenBalance.sub(liquidityAmount);
        ...
        uint256 liquidtyBalance = newBalance.mul(_halfFee).div(_halfFee + _marketingFee);
        // add liquidity to PancakeSwap
        addLiquidity(liquidityAmount, liquidtyBalance);

        uint256 marketingBalance = newBalance.sub(liquidtyBalance);

        _marketingWallet.transfer(marketingBalance);
        
        emit SwapAndLiquify(liquidityAmount, liquidtyBalance, swapAmount);
    }
