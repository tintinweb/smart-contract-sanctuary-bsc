...
contract BankRandom is AccessControlEnumerable {
    ...
    uint8[] public final_ = [0, 3, 4, 5, 6, 7, 10, 15, 20, 30, 40, 50, 60, 70, 80, 100, 140, 160, 200];
    ...
    function setFinal(uint8 _index, uint8 random) external onlyRole(OPERATOR_ROLE) {
        final_[_index] = random;
    }
    ...
    function withdrawDays(uint256 k, uint8 level) external view onlyRole(BANK_ROLE) returns (uint256 days_) {
        if (k < 150) {
