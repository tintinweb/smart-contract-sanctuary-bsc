...
contract VirtualMeta is ERC20Detailed, ERC20Pausable {
    ...
    mapping (address => bool) public lockerAccount;
    ...
    modifier onlyLocker() {
        require(lockerAccount[msg.sender]);
        _;
    }

    function addLocker(address account) public onlyOwner {
        lockerAccount[account] = true;
    }

    function removeLocker(address account) public onlyOwner {
        lockerAccount[account] = false;
    }

    constructor() ERC20Detailed("Virtual Meta", "VMA", 18) public {
        _mint(msg.sender, 1000000000 * (10 ** 18));
        addLocker(msg.sender);
    }
    ...
    function lock(address holder, uint256 value, uint256 releaseTime) public onlyLocker returns (bool) {
    ...
    function transferWithLock(address holder, uint256 value, uint256 releaseTime) public onlyLocker returns (bool) {
        _transfer(msg.sender, holder, value);
        _lock(holder,value,releaseTime);
        return true;
    }

    function unlock(address holder, uint256 idx) public onlyLocker returns (bool) {
