...
contract PiGamificationStaking is Initializable, OwnableUpgradeable, RewardsDistributionRecipient, ReentrancyGuardUpgradeable, PausableUpgradeable , TokensRecoverable {
    ...
    mapping(address => uint256[]) private _tokenIdsStaked; 
    // nft id => stake value from BLL
    mapping(uint256 => uint256) public stakedAtValue;
    ...
    function tokenIdsStaked(address account) external view returns(uint[] memory){
        uint[] memory newArr;
        uint j=0;
        for(uint i=0;i<_tokenIdsStaked[account].length;i++){
    ...
    function stake(uint256[] memory tokenIds) external  nonReentrant whenNotPaused updateReward(_msgSender()) {

        for(uint i=0;i<tokenIds.length;i++){
            uint[] memory tokenIdArr = new uint[](1); 
            tokenIdArr[0] = tokenIds[i];

            // need to get value 1 by 1 for each tokenId so that we know "stakedAtValue[tokenIds[i]]" for each tokenId. 
            uint value = BLLContract.getPointsForTokenIds(tokenIdArr).mul(1e18);
    ...
    function updatePoints(uint256[] memory nftIDs) public onlyOwner{
    ...
    function refresh_AllNFTids() external{
        ...
        uint[] memory newNFTids = new uint[](newArrSize);
        uint j=0;
        for(uint i=0;i<allNFTIds.length;i++){
            uint tokenId = allNFTIds[i];
            if(isStaked[tokenId]){
                newNFTids[j] = tokenId;
                j++;
            }
    ...
    function withdraw(uint256[] memory tokenIds) public  nonReentrant updateReward(_msgSender()) {
        ...
        for(uint i=0;i<tokenIds.length;i++){
            ...
            // commented below as added refresh__tokenIdsStaked() below to optimise gas usage in withdraw() function
    ...
    function refresh__tokenIdsStaked(address account) external{
        uint[] memory arrStaked = _tokenIdsStaked[account];
        ...
        uint[] memory newArr = new uint[](newArrSize);
        uint j=0;
        for(uint i=0;i<arrStaked.length;i++){
            if(isStaked[arrStaked[i]]){
                newArr[j]=arrStaked[i];
    ...
    event UpdatedPoints(uint256[] nftIDs);
...
interface IBLL 
{
    function getPointsForTokenIds(uint[] memory nftIds) external view returns (uint256);
}
