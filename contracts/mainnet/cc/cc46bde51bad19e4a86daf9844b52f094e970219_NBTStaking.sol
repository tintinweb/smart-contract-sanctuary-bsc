...
contract NBTStaking is UUPSUpgradeable, AccessControlUpgradeable , ReentrancyGuardUpgradeable {
    ...
    event SetMaxStakingTime(uint maxStakingTime, address sender);   
    ...
    @param maxStakingTime_ the max locked staking period in timestamp
    */

  
    function initialize(address tokenAddress_, address rewardToken_, uint minClaimPeriod_, uint rewardRate_, uint maxStakingWeeks_, uint maxStakingTime_ ) initializer external {
        ...
        require(maxStakingTime_ > block.timestamp, "INVALID_MAX_STAKING_TIME");
    ...
    */
    ...
    @param account_ account to get the earned amount for
    */
    ...
    */
    function stake(uint amount_, uint lockWeek_) external nonReentrant() updateReward(msg.sender) {
        ...
        uint timeToUnlock = block.timestamp + lockWeek_ * 604800;
        require(timeToUnlock <= maxStakingTime, "LOCK_TIME_ERROR");
        ...
        uint tokensWithBonus = amount_;
        if(lockWeek_>0){
            uint bonusTokenWeightage = 6 * lockWeek_* (lockWeek_+110) +119330;
            tokensWithBonus = amount_ * bonusTokenWeightage / 1e5;
        }
        ...
        data.timeToUnlock = timeToUnlock;
    ...
    */
    function unstake(uint stakeId_) external nonReentrant() updateReward(msg.sender) {
        ...
        require(block.timestamp > info.timeToUnlock,"STAKE_ID_IS_LOCKED");
        ...
        getRewardInternal();
        stakingToken.safeTransfer(msg.sender, info.tokensStaked);
    ...
    */
    function getReward() external nonReentrant() updateReward(msg.sender) {
        require(stakingNonce[msg.sender] != 0, "INVALID_SENDER");
    ...
    */
    ...
    @param stakingEnabled_ whether or not staking should be enabled
    */
    ...
    */
    function setMaxStakingWeeks(uint maxStakingWeeks_) external onlyRole(OPERATOR_ROLE) {
        require(maxStakingWeeks_ > 0,"INVALID_MAX_WEEK");
        maxStakingWeeks = maxStakingWeeks_;
    ...
    /** 
    @dev Sets the maximum locked staking period in time
    @param maxStakingTime_ the max locked staking period in time
    */
    function setMaxStakingTime(uint maxStakingTime_) external onlyRole(OPERATOR_ROLE) {
        require(maxStakingTime_ > 0,"INVALID_MAX_TIME");
        maxStakingTime = maxStakingTime_;

        emit SetMaxStakingTime(maxStakingTime,msg.sender);
    }
