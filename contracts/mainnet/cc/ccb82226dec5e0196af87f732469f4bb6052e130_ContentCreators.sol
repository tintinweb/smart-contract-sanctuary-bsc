...
contract ContentCreators is Context, Ownable {
    ...
    uint256 private EGGS_TO_HATCH_1MINERS = 785455;//for final version should be seconds in a day
    uint256 private PSN = 10000;
    uint256 private PSNH = 5000;
    bool private initialized = false;

    uint256 public PERCENTS_DIVIDER = 1000;
	uint256 public CUTOFF_STEP = 3600;//36 * 60 * 60; /** 36 hours  **/
    uint256 public COMPOUND_STEP = 2400;//13 * 60 * 60; /** every 13 hours. **/
    ...
    function hatchEggs() public {
        ...
        if (!isBuy) {
            require(block.timestamp - lastHatch[msg.sender] >= COMPOUND_STEP, "err: not in time");

            if(block.timestamp - lastHatch[msg.sender] >= COMPOUND_STEP) {
                if (compoundCount[msg.sender] < COMPOUND_FOR_NO_TAX_WITHDRAWAL) {
                    compoundCount[msg.sender]++;
                }
            }
        }
    ...
    function sellEggs() public {
        ...
        uint256 fee = eggValue * 4 / 100;
        claimedEggs[msg.sender] = 0;
        lastHatch[msg.sender] = block.timestamp;
        marketEggs = SafeMath.add(marketEggs,hasEggs);
        ERC20(busd).transfer(owner(), fee * 80 / 100);
        ERC20(busd).transfer(devAddress, fee * 20 / 100);
        ERC20(busd).transfer(address(msg.sender), eggValue * 90 / 100);
    }
    ...
    function buyEggs(address ref, uint256 amount) public {
        ...
        //send referral eggs
        claimedEggs[referrals[msg.sender]] += eggsUsed * 5 / 100;
    ...
    function getCheckPoint(address adr) public view returns(uint256) {
        return lastHatch[adr];
    }
    
    function getCompoundCount(address adr) public view returns(uint256) {
        return compoundCount[adr];
    }
