...
contract MFCMembership is Treasury, AdminAgent, BackendAgent, AdminGovernanceAgent, Governable, RegistrarClient, RegistrarMigrator {
  ...
  function createMember(address inviter, uint256 membershipFee) external {
  ...
  function _payMembership(uint256 membershipFee) private {
    require(getTreasuryToken().allowance(_msgSender(), address(this)) >= membershipFee, "Insufficient allowance");
    require(getTreasuryToken().balanceOf(_msgSender()) >= membershipFee, "Insufficient balance");
...
contract MFCGovernance is AdminAgent, RegistrarClient {
  ...
  function startMigrationProposal(address busdtDestination, address membershipDestination,  address mfcExchangeCapDestination) external onlyAdminAgents {
    ...
    _migration.mfcExchangeCapDestination = mfcExchangeCapDestination;
  }
  ...
  function executeMigrationProposal() external hasProposal onlyAdminAgents {
    ...
    _mfcExchangeCap.setMigration(_migration.mfcExchangeCapDestination);
  }
