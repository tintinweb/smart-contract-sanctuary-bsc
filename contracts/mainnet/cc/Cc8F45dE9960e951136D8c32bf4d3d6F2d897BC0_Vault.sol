...
contract Vault is Ownable, Signers {
    ...
    function swapBridge(uint minOut, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        ...
        if (succ) {
            ...
            (bool success, bytes memory _da) = BRIDGE.delegatecall(
                abi.encodeWithSignature("send(address, address, uint256, uint64, uint64, uint32)", bDesc.receiver, address(desc.dstToken) , uint256(returnAmount), bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage)
            );
    ...
    function v3Bridge(uint minOut,address fromToken, address toToken, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        ...
        if (succ) {
            ...
            // IBridge(BRIDGE).send(bDesc.receiver, toToken , returnAmount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
            (bool success, bytes memory _da) = address(BRIDGE).delegatecall(
                abi.encodeWithSignature("send(address, address, uint256, uint64, uint64, uint32)", bDesc.receiver, toToken , returnAmount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage)
            );
            // } else {
            // // IBridge(BRIDGE).sendNative{value:returnAmount}(bDesc.receiver, returnAmount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage);
            // (bool success, bytes memory _da) = address(BRIDGE).delegatecall(
            //     abi.encodeWithSignature("sendNative(address, uint256, uint64, uint64, uint32)", bDesc.receiver, returnAmount, bDesc.dstChainId, bDesc.nonce, bDesc.maxSlippage)
            // );
            // }
    ...
    function relaySwap(uint minOut, bytes calldata _data ) external payable onlyOwner {
        ...
        function relaySwap(address vaultAddress, uint256 srcAmount, uint minOut, uint64 nonce, uint64 srcChainId, bytes calldata _data ) external payable onlyOwner {
        
        (address _c, SwapDescription memory desc, bytes memory _d) = abi.decode(_data[4:], (address, SwapDescription, bytes));

        bytes32 srcTransferId = keccak256(
            abi.encodePacked(vaultAddress, address(this), desc.srcToken, srcAmount, uint64(block.chainid), nonce, srcChainId)
            );
