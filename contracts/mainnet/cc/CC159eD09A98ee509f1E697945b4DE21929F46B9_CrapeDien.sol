...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 REWARDS = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); //the reward token you want distributed - ETH
...
contract CrapeDien is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address private boosterWallet = 0x98e3868cb5C6aC4a842Ce51fB2cf17cD466d912A;
    ...
    string constant _name = "CrapeDian";
    string constant _symbol = "CKD";
    ...
    uint256 public buyingBoosterFee = 3;
    ...
    uint256 public buyersTotalFees = buyingBoosterFee.add(buyingRewardFee).add(buyingLpFee).add(buyingDevFee);
    uint256 public sellersTotalFees = sellingBoosterFee.add(sellingRewardFee).add(sellingLpFee).add(sellingDevFee);
    ...
    uint256 public rewardPortionOfSwap = 10; // 10%
    ...
    function calcBuyersTotalFees() private {
        buyersTotalFees = buyingBoosterFee.add(buyingRewardFee).add(buyingLpFee).add(buyingDevFee);
    }
    
    function calcSellersTotalFees() private {
        sellersTotalFees = sellingBoosterFee.add(sellingRewardFee).add(sellingLpFee).add(sellingDevFee);
    }
    ...
    uint256 DevPortion, uint256 lpPortion) external onlyOwner {
        ...
        uint256 totalPortion = boosterPortion.add(rewardPortion).add(DevPortion).add(lpPortion);
