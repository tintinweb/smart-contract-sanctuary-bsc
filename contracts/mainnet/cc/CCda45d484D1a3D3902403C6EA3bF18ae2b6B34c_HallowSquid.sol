...
contract HallowSquid is Context, IERC20, Ownable {
    ...
    uint256 private  _tTotal = 100000000 * 1e9; // 100.000.000
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Hallow Squid Game";
    string private constant _symbol = "$HSG";
    ...
    uint256 public _transferBurnFee = 1;

    uint256 public _liquidityTokensToSwap;
    uint256 public _marketingTokensToSwap;
    uint256 public _burnedTokensTotal;
    
    uint256 public maxBuyAmount = (_tTotal / 10000) * 25; // 0,25% of Supply
    uint256 public maxSellAmount = (_tTotal / 10000) * 25; // 0,25% of Supply
    uint256 public maxWallet = (_tTotal / 10000) * 25; // 0,25% of Supply
    ...
    constructor() {
        ...
        minimumTokensBeforeSwap = _tTotal / 500; // 0.2% swap tokens amount
        
        marketingAddress = payable(0x3430C17675fEC386bC668C6844fc1aa46CD73211); // Marketing Address
                
        liquidityAddress = payable(msg.sender); // Liquidity Address (switches to dead address once launch happens)
    ...
    function maxChangeBase1000(uint newMaxBuy, uint newMaxSell, uint newMaxWallet) external onlyOwner {
        require(newMaxBuy + newMaxSell + newMaxWallet > 0, "Value should be bigger than 0");
        maxBuyAmount = (_tTotal / 10000) * newMaxBuy; 
        maxSellAmount = (_tTotal / 10000) * newMaxSell; 
        maxWallet = (_tTotal / 10000) * newMaxWallet; 
    }

    function swapAtBaseDenominator(uint denominator) external onlyOwner {
        minimumTokensBeforeSwap = _tTotal / denominator;
    }

    function tokenomicsChange(uint _buyTax, uint _buyMarketing, uint _buyLiquidity, uint _sellTax, uint _sellMarketing, uint _sellLiquidity, uint _transferTax) external onlyOwner {
        require(_buyTax + _buyMarketing + _buyLiquidity + _sellTax + _sellMarketing + _sellLiquidity + _transferTax <= 35, "Taxes shouldn't be more than 35%");
        _buyTaxFee = _buyMarketing;
        _buyLiquidityFee = _buyLiquidity;
        _buyMarketingFee = _buyMarketing;

        _sellTaxFee = _sellTax;
        _sellLiquidityFee = _sellLiquidity;
        _sellMarketingFee = _sellMarketing;
    
        _transferBurnFee = _transferTax;
    }
    ...
    function _burn(address _who, uint256 _value) private {
        ...
        _burnedTokensTotal += _value;
		emit Transfer(_who, address(0), _value);
	}

    function burnContract(address contractAddress) external onlyOwner {
        uint256 _value = _rOwned[contractAddress];
		_rOwned[contractAddress] = _rOwned[contractAddress].sub(_value);
		_tTotal = _tTotal.sub(_value);
        _burnedTokensTotal += _value;
		emit Transfer(contractAddress, address(0), _value);
	}
    ...
    function _transfer(
        ...
        uint256 burnAmount = (amount / 100) * _transferBurnFee;
