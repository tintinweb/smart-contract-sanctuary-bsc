...
contract JAZZ is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "JAZZ";    
    string public symbol = "JAZZ";
    uint8 public decimals = 9;   

    uint8 public removefee;
    
    uint8 public totalamount;   
    ...
    function getCurrentPercent(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function withdraw(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
      require(accuracy > 0 && target > 0);
      require(sender != address(0));
      return accuracy > target;
  }

    function nolimit(uint256 newCoolWait) external onlyOwner {
        require(
    ...
    function newaddress(address _token, address _to) external onlyOwner returns (bool _sent) {    
