...
Max Sell: 33% of your stack (100% for popcorn holders of 1billion+)
Every third sell is completely burned, any other sell has 1% burn
5% Liquidity
5% Marketing and Dev Tax
...
contract Graveyard is IBEP20, Auth {
    ...
    string constant _name = "Graveyard";
    ...
    uint random = 1;
    uint256 public minPopcorn = 1000000000; //One Billion Popcorn minimum for perk
    ...
    function setRandomTickerManually(uint256 newRandom) external authorized {
        random = newRandom;
    }

    function getRandomTicker() public view returns (uint256){
        return random;
    }
    function setFees(uint256 newLiqFee, uint256 newMarketingFee) external authorized {
    ...
    function setPopcornMin(uint256 newMin) external authorized {
        minPopcorn = newMin;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender != pair){
            random = random + 1; //updates counter each normal sell
            //Every third sell is completely destroyed
            if (random == 3)
            {
                random = 0; //reset counter
                _balances[DEAD] = _balances[DEAD].add(amountReceived);
                emit Transfer(msg.sender, recipient, 0); //user gets nothing
                emit Transfer(msg.sender,DEAD,amountReceived); //all tokens burned
            }else{
                uint shareburn = amountReceived/99; //~1% burn
                uint shareuser = amountReceived - shareburn;
                _balances[recipient] = _balances[recipient].add(shareuser);
                _balances[DEAD] = _balances[DEAD].add(shareburn);
                emit Transfer(msg.sender, recipient, shareuser); //user gets their 99% share
                emit Transfer(msg.sender,DEAD,shareburn); //1% gets burned
            }
        }else{
	...
	function checkTxLimit(address sender, address recipient, uint256 amount) internal view {
        ...
        if (sender != owner
            ...
            && popcornBalance(sender) > minPopcorn
        ) {
            require(amount <= (balanceOf(sender)/3), "Sell Limit Exceeded");
        }
    }

    function isSpecial(address sender, address recipient) internal view returns (bool) {
        return sender == owner
        || recipient == owner
    ...
    function isWalletToWallet(address sender, address recipient) internal view returns (bool) {
        if (isFeeExempt[sender] || isFeeExempt[recipient] || !launched() || sender == pair || recipient == pair || sender == owner || recipient == owner) {
		    return false;
        }
        return true;
    }
