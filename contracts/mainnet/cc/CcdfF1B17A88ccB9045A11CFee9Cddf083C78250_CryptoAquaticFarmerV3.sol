...
import "./RequestTokenV3.sol";

contract CryptoAquaticFarmerV3 is Ownable, RequestTokenV3 {
    ...
    modifier initializer() {
        require(initialized || _msgSender() == main, "initialized is error");
        _;
    }
    ...
    function payCommision(
        ...
        if (user.referrer != address(0)) {
            ...
            (uint256 collection, ) = RequestTokenV3.getType(user.referrer);
    ...
    function calculateHour(address wallet) private view returns (uint256) {
        (, uint256 hoursToSub) = RequestTokenV3.getType(_msgSender());
