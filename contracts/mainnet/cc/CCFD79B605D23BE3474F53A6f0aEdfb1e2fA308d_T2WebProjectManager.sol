  ...
  OwnableUpgradeable
{
  ...
  mapping(uint256 => bool) private _transferUnsoldDisabled;
  ...
  function createERC721Project(
    ...
    uint256 maxSupply = saleData[3] + saleData[8];
    require(
      maxSupply > 0,
    ...
    _transferUnsoldDisabled[projectId] = transferUnsoldDisabled;
  ...
  function buyPresale(
    ...
    uint256 amount,
    bytes calldata signature
  ) external payable notZeroAddress(_msgSender()) {
    Project storage project = _projects[projectId];
    ...
    notZeroAddress(_msgSender())
  {
    ...
    if (project.publicsaleMaxPurchase > 0) {
      require(
    ...
    uint256 maxSupply = project.publicsaleAmount;
    if (!_transferUnsoldDisabled[project.id]) {
      uint256 remainPresale = project.presaleAmount - project.presaleSold;
      maxSupply = maxSupply + remainPresale;
    }
    require(project.publicsaleSold + amount <= maxSupply, "AMOUNT_INVALID");
  ...
  function claimItems(uint256 projectId, uint256 amount) external {
    ...
    require(
      block.timestamp > project.publicsaleEndDate ||
        project.state == ProjectState.FINISHED,
      "ProjectManager: the project is not finished"
    );
    require(amount > 0, "ProjectManager: amount must be greater than 0");
    require(amount <= 50, "ProjectManager: amount must be less than 50");

    uint256 remain = IT2WebERC721(project.contractAddress).maxSupply() -
      IT2WebERC721(project.contractAddress).totalSupply();
    require(remain >= amount, "ProjectManager: out of supply");

    for (uint256 i = 0; i < amount; i++) {
      IT2WebERC721(project.contractAddress).mint(msg.sender);
    }
  }
  ...
  function updateProjectState(uint256 projectId, uint256 state) external {
    Project storage project = _projects[projectId];
    ...
    ProjectState _state = ProjectState(state);
    require(
      _state == ProjectState.CREATED || _state == ProjectState.FINISHED,
      "ProjectManager: invalid state"
    );
    require(project.state != _state, "ProjectManager: invalid state");
