...
contract PreSaleFactory is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // Mapping of admins who can create new pools
    ...
    function initialize(address payable _feeWallet) external initializer {
        __Ownable_init();
        __Pausable_init();
    ...
    function changeFeeWallet(address payable _feeWallet) external onlyOwner {
        feeWallet = _feeWallet;
    }
    ...
    function changeAdminPermission(address _admin, bool _isAdmin) external onlyOwner {
    ...
    function registerPool(
        ...
        require(admins[msg.sender], "ICOFactory::NO_PERMISSION");
...
contract PreSalePool is Ownable, ReentrancyGuard, Pausable, RedKiteWhitelist {
    ...
    //     require(_verifyWhitelist(_candidate, _maxAmount, _minAmount, _fee, _signature), "POOL:INVALID_SIGNATURE");
    ...
    function buyTokenByTokenWithPermission(
        ...
        require(_validPurchase(), "POOL::ENDED");
        require(_verifyWhitelist(_candidate, _maxAmount, _minAmount, _fee, _amount, _signature), "POOL:INVALID_SIGNATURE");
     ...
     * @dev Determines how ETH is stored/forwarded on purchases.
     */
    // function _forwardFee(uint256 _value) internal {
    //     address payable wallet = companyWallet;
    //     (bool success, ) = wallet.call{value: _value}("");
    //     require(success, "POOL::WALLET_TRANSFER_FAILED");
    // }
     ...
     * @dev Determines how Token is stored/forwarded on purchases.
     */
    function _forwardTokenFee(address _token, uint256 _amount) internal {
        address companyWallet = IPoolFactory(factory).getFeeWallet();
        TransferHelper.safeTransferFrom(_token, msg.sender, companyWallet, _amount);
    }
    ...
    function _verifyWhitelist(
        ...
        if (useWhitelist) {
            return (verify(signer, _candidate, _maxAmount, _minAmount, _fee, _amount, _signature));
        }
        return true;
    }
...
contract RedKiteWhitelist {
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_candidate, _maxAmount, _minAmount, _feeAmount, _amount));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_candidate, _maxAmount, _minAmount, _feeAmount, _amount);
