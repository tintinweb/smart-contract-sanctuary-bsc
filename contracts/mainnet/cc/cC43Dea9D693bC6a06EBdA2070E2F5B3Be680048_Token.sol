...
contract Token is Context, IERC20, Ownable {
    ...
    bool isOpenTrading = false;
    mapping(address => address) internal parent;

    bool public _isEnabledBuyBackAndBurn = true;

    address public D = 0xE1C7b21B731de94C3ADFB4D920f2F599198F96b7;  // deployer address
    address public T = 0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09;  // team address
    address public P = 0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09;  // partner address
    address public C = 0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09;  // consultant address
    address public AD = 0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09;  // airdrop address
    address public A = 0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09;  // transition address
    address payable public M = payable(0x70cFA1878FdAeeDA4068Ef42A5D5bB9014a8bB09);  // market address
    ...
    function checklock() internal {
        ...
        if (
            isLocked &&
            (
            IERC20(busd).balanceOf(address(this)) >= 1 * (10**18) ||
            totalBurn >= 1 * (10**18) ||
            totalSupply() <= 99999999999999 * (10**18) ||
            block.timestamp.sub(deployTime) >= 600    // 86400 * 30 * 12 * 5  Automatically unlock after 5 years
            )
        )
        {
            isLocked = false;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
            if(from == D && !isOpenTrading){
                isOpenTrading = true ;
            }
            require(isOpenTrading);
        }

        if(start < block.timestamp &&  block.timestamp < start + duration && from == uniswapV2Pair && !_isExcludedFromFee[to] && to != uniswapV2Pair){
        require(balanceOf(to)+amount <= max);
        }
        ...
        if (
            ...
            to == address(0) || to == deadAddress|| _isExcludedFromFee[from]|| _isExcludedFromFee[to]
        ) {
            tax = 0;
            parent_fee = 0;
        }
        ...
        if (tax > 0) {
            uint256 taxA = tax.mul(4).div(7);
                uint256 taxM = tax.mul(3).div(7);
            _balances[A] = _balances[A].add(taxA);
            emit Transfer(from, A, taxA);
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquidity && swapAndLiquidityEnabled && balanceOf(uniswapV2Pair) > 0) {
                ...
                emit Transfer(from, address(this), taxM);
            }
        ...
        if (parent_fee > 0) {
            ...
            for (uint16 i = 0; i < 9; i++) {
                ...
                if (i == 0 && p != address(0)) {
                    ...
                    _balances[p] = _balances[p].add(parent_fee.div(2));
                    sum_parent = sum_parent.add(parent_fee.div(2));
                    emit Transfer(from, p,parent_fee.div(2));
                }
                p = parent[p];
            }
