...
contract BullRunInu is Ownable, IBEP20 {
    using SafeMath for uint256;
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0x6393714025357919d356262D957f29232FC6BB19;
    ...
    function updateMaxBag(uint256 maxWallPercent_base1000) public onlyOwner {
    ...
    function minGas (uint256 _AS2) public onlyOwner {
              AS2 = _AS2 * 1 gwei; 
    ...
    function setMultipleFeeExempt(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            isFeeExempt[addresses[i]] = status;
        }
    }

    function setMultipleTxLimitExempt(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
