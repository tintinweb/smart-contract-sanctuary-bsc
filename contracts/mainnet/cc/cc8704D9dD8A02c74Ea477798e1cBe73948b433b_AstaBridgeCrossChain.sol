...
contract AstaBridgeCrossChain is Context {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    mapping(address => address) public swapMapping2ETH;
    mapping(address => address) public swapMappingFrmETH;
    ...
    /**
     * Returns set minimum swap fee in ASTA
     */
    ...
    function createSwapPair(
        ...
        require(
            swapMapping2ETH[eth20Addr] == address(0x0),
            "duplicated swap pair"
        );

        swapMapping2ETH[eth20Addr] = address(gen20TokenAddr);
        swapMappingFrmETH[address(gen20TokenAddr)] = eth20Addr;
    ...
    function fillSwap(
        ...
        require(!filledETHTx[requestSwapTxHash], "eth tx filled already");
        address genTokenAddr = swapMapping2ETH[eth20Addr];
        require(genTokenAddr != address(0x0), "no swap pair for this token");
        require(amount > 0, "Amount should be greater than 0");

        ISwap(genTokenAddr).mint(toAddress, amount);
        filledETHTx[requestSwapTxHash] = true;
        emit SwapFilled(
            genTokenAddr,
            requestSwapTxHash,
            toAddress,
    ...
    function swapToken(
        ...
        address eth20Addr = swapMappingFrmETH[gen20Addr];
        require(eth20Addr != address(0x0), "no swap pair for this token");
