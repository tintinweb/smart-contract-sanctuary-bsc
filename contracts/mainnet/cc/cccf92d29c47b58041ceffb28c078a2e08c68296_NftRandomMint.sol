...
contract NftRandomMint is ERC721, ERC721Enumerable {
    ...
    function mintWithBNB(uint256 numberOfNfts) external payable{
        ...
        randomnessSupplier.requestRandomness{value: vrfCost*numberOfNfts}(_nonce, numberOfNfts);
        _nonce++;
    ...
    function sellBnbForBusd() external {
    ...
    function supplyRandomness(uint256 nonce, uint256[] memory randomNumbers) external onlyVRF {
        ...
        for (uint i = 0; i < randomNumbers.length; i++) {
            uint256 index = (randomNumbers[i] % pendingCount) + 1;
    ...
    function claimPrize(uint256 nftId) external{
    ...
    function rescueBEP20(address tokenAddress, uint256 amount) external onlyOwner {
        IBEP20(tokenAddress).transfer(_admin, amount);
    }
}