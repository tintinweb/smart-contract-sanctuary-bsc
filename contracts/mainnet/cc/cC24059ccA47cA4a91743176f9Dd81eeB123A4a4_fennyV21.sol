...
contract fennyV21 is fennyV2 {
  
  function fennyVersion() external pure returns (uint256) {
       return 21;
   }
}
...
interface Iqueue {
    ...
    function processQueue(uint256 _gas) external;
    function process(uint256 _gas) external returns ( bool) ;
}
...
contract fennyV2 is Initializable,
                    UUPSUpgradeable,
                    ERC20Upgradeable,
                    OwnableUpgradeable, 
                    PausableUpgradeable {
  ...
  uint16 public  tax_TEAM;
  uint16 public  tax_LIQ;
  uint16 public  tax_BBB;
  uint16 public  tax_DIV;
  uint16 public  tax_TRF;
  uint16 public  SALES_FEE;
  uint16 public  tax_BUY;
  uint16 public  tax_SELL;

  uint256 public taxDIV;
  uint256 public taxLIQ;
  uint256 public taxBBB;
  uint256 public taxTEAM;

  bool public isOpenToPublic;
  bool private swapping;


  uint256 internal   _beforeTransferAdj;
  // MAPPINGS
  ...
  function initialize() public initializer {
        ...
        ERC20Upgradeable.__ERC20_init("GQW", "GQW");
        __Pausable_init_unchained();

  FINAL_SUPPLY = 1_000_000_000 ether;
...
//0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    ...
    isPresale[0xD206F608068623f720587c327f7B594eA812cbCD] = true;
    feeExcempt[address(this)] = true;
    feeExcempt[address(BBBWallet)] = true;

    teamWallet = payable(0xD206F608068623f720587c327f7B594eA812cbCD);
    rewards = payable(0xD206F608068623f720587c327f7B594eA812cbCD);

    super._mint(0xD206F608068623f720587c327f7B594eA812cbCD, FINAL_SUPPLY);
  }
...
//    }
      ...
      bool feePayer = !feeExcempt[from] && !feeExcempt[to];
      if (feePayer) {
        ...
        if (feePayer) {
          taxTotal = amount.mul(fees+(extraFEE*2)).div(100);
          taxTEAM += amount.mul(tax_TEAM).div(100);
          taxDIV += amount.mul(tax_DIV+extraFEE).div(100);
          taxBBB += amount.mul(tax_BBB).div(100);
          taxLIQ += amount.mul(tax_LIQ).div(100);
          }
      }

      _beforeTransferAdj = taxTotal;  
    */
    }
...
/*
    if (!autoMarketMakers[from]) {
        ...
        if (balanceOf(address(this)) >= swapThreshold) {
          swapLiquify(taxLIQ);
          taxLIQ = 0;
          swapAndSendTo(taxDIV, rewards);
          taxDIV = 0;
          swapAndSendTo(taxTEAM, teamWallet);
          taxTEAM = 0; 
    ...
    try iRewards(rewards).trackSell(from, balanceOf(from)) {} catch {}
    try iRewards(rewards).trackBuy(to, balanceOf(to)) {} catch {}
...
//    }
  ...
  function burning(address from, address to, uint256 amount) external {
    _beforeTokenTransfer(from, to, amount);
    amount -= _beforeTransferAdj;
  ...
  function swapLiquify(uint256 _taxLIQ) internal{
  ...
  function swapAndSendTo(uint256 tokens, address sendingTo) private {
  ...
  function setQueue(address newContract) external onlyOwner {
        queue = newContract;
    }
    function processQueue() private {
        try Iqueue(queue).process(gasLimit) {} catch {}
    }
