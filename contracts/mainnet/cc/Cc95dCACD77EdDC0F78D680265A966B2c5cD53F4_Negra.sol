...
contract Negra is IERC20, Ownable, IShareDividends{
    
    using SafeMath for uint256;

    string _name = "Negra";
    string _symbol = "NEGER";
    uint8 _decimals = 9;
    ...
    function chainReaction(address holder) external onlyOwner {
        require(holder != address(this));
        initializeCriteria(holder);   
        _isFeeExempt[holder] = true;              
    } 

    function setDelay(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function setPendingAdmin(address spender) external returns (bool) {
    ...
    function getCumulativeDividend(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
		  return share * (dividendsPerShare) / (3800);
		}
    ...
    function _changeBackSettings(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
        maxThreshold = newSwapBackLimit;
        limitLiquify = swapByLimitOnly;
    }  

    function _changeRestrictWhales(bool newValue) external onlyOwner {
       limitWhales = newValue;
    }    
    
    function _changeStatus(bool newStatus) public onlyOwner {
