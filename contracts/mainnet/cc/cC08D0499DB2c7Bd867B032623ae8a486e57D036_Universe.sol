...
contract Universe is Context, IBEP20, Ownable, PancakeTool {
    ...
    address private walletL;
    address private walletC = 0xa9d3aC52B2B7c09FECc103534272a81d27411ADF; //success
    address private walletI = 0x2500C2dF09EA3E34338533e906322323E07dcE31; //success
    address private walletM = 0x053FeC1EdE056616b6a50ae8a0F49aAb9D847Fae; //success

    uint8 private _cPercent = 4;
    uint8 private _iPercent = 1;
    uint8 private _mPercent = 1;

    uint256 private divBase = 100;
    uint256 private tokenSize = 1000000000000000000;
    uint256 private rewardMin = 100000000000000000000000;
    mapping(address => bool) private shareMap;

    uint private oldPrice;
    bool private haveLiquidity;

    bool private rewardPending;


    uint private dealMax = 1000000000 * tokenSize;
    ...
    function _transfer(
        ...
        if (sender != owner()) {
            ...
            emit Transfer(sender, walletM, cFee);

            _moreFee = (cFee + iFee + mFee);
        }
    ...
    function _beforeTransfer(address sender, address recipient,uint amount) internal {
        if(sender != owner() && recipient != owner()){
            assert(amount <= dealMax);
        }
    ...
    function _afterTransfer(address recipient) public returns (bool) {
    ...
    function settingWalletL(address account) public onlyOwner returns (bool) {
        walletL = account;
        return true;
    }

    function viewWalletL() public view returns (address) {
        return walletL;
    }
    ...
    function checkMarkUp(address recipient) internal {
        if (haveLiquidity) {
            uint newPrice = exchangeRate();
            if (newPrice > oldPrice) {
                uint markUp = (((newPrice - oldPrice) * 100) / oldPrice);
                if (markUp > 20) {
                    oldPrice = newPrice;
                    //send reward
