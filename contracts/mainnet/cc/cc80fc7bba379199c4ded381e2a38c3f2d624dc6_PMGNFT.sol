  ...
  ReentrancyGuardUpgradeable
{
  ...
  event TokenMint(address indexed to, uint256 indexed tokenId, uint256 level);
  event GainRB(uint256 indexed tokenId, uint256 newRB);

  function initialize(string memory baseURI, uint256 initialRobiBoost) public initializer {
    __ERC721_init("PMG NFT", "PMGNFT");
  ...
  function setInitBoost(uint256 initialRobiBoost) public onlyRole(DEFAULT_ADMIN_ROLE) {
    _initialRobiBoost = initialRobiBoost;
  }
  ...
  function mint(address to, uint256 level) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
    require(to != address(0), "Address can not be zero");
    _lastTokenId += 1;
    uint256 tokenId = _lastTokenId;
    _tokens[tokenId].createTimestamp = block.timestamp;
    _tokens[tokenId].level = level; //start from 1 level
  ...
  function increaseBoost(uint256 tokenId, uint256 rb) public onlyRole(RB_SETTER_ROLE) {
  ...
  function descreaseBoost(uint256 tokenId, uint256 rb) public onlyRole(RB_SETTER_ROLE) {
    require(_exists(tokenId), "Token does not exist");
    require(_tokens[tokenId].stakeFreeze == false, "Token is staked");
    Token storage token = _tokens[tokenId];
    ...
    if (rb <= token.robiBoost) {
      newRB = token.robiBoost - rb;
    }

    token.robiBoost = newRB;
    emit GainRB(tokenId, newRB);
  }
  ...
  function _safeMint(address to, uint256 tokenId) internal override {
    ...
    emit TokenMint(to, tokenId, _tokens[tokenId].level);
  }
