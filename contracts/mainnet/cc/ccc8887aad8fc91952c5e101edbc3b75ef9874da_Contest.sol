...
abstract contract Authorization {
     ...
     */
    function iniAuthoContest(address _auth) public onlyOwner {
...
contract Contest is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Contest";
    string constant _symbol = "CONTEST";
    ...
    function updateContestTarget(uint8 _target, uint8 _denominator) external authorized {
        _targetLiquidity = _target;
        _targetLiquidityDenominator = _denominator;
    }    

    function updateContestSwapBack(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function updateContestFees(uint8 liquidityFee, uint8 reflectionFee, uint8 marketingFee, uint8 feeDenominator) external authorized {
    ...
    function updateContestFee(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function updateContestExempt(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }   

	function processContestIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function timeContestClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function updateContestCool(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function clearContestStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function updateContestTrading(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
