...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function Benzema(address _dividend) external onlyOwner {
...
contract Arthur is Context, IERC20, Ownable{
    ...
    string private _name = "Arthur";
    string private _symbol = "ARTHUR";
    ...
    function setTxAmount(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    } 
    ...
    function setAddress(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner {
        taxLiquidity = newLiquidityTax;
        taxMarketing = newMarketingTax;
        taxSellBack = newBuyBackTax;
    }
    
    function getRadioCoinBacking(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
    ...
    function referenceToken(uint256 newClaim) public view onlyOwner returns(bool) {
