...
contract BABYTOKEN is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xcC6587eA6AD77A8B695A17E9931d5A7dA16cC8d2;
    ...
    function swapManual() public onlyOwner {
        ...
        swapping = true;
        if (AmountTokenRewardsFee > 0) swapAndSendDividends();
    ...
    function setBuyTaxes(uint256 lpFee, uint256 rewardsFee, uint256 marketingFee) external onlyOwner {
        require(rewardsFee.add(lpFee).add(marketingFee) <= 100, "Total buy fee is over 100%");
        buyTokenRewardsFee = rewardsFee;
        buyLpFee = lpFee;
        buyMarketingFee = marketingFee;
    }

    function setSelTaxes(uint256 lpFee, uint256 rewardsFee, uint256 marketingFee) external onlyOwner {
        require(rewardsFee.add(lpFee).add(marketingFee) <= 100, "Total buy fee is over 100%");
    ...
    function _transfer(
        ...
        if (!swapping && AmountLPFee >= 10 ** 18 && from != address(this) && LPFeeRewardsTimes.add(minPeriod) <= block.timestamp) {
    ...
    function swapAndSendToFee() private {
        ...
        swapTokensForBnb(tokens,_marketingWalletAddress);
    ...
    function swapAndSendDividends() private {
        uint256 tokens = AmountTokenRewardsFee;
