...
contract DummerAcolasia is IBEP20, Manager {
    ...
    string constant _name = "Dummer Acolasia ";
    string constant _symbol = "DummerAcolasia";
    ...
    address private minBuySellLiquidity = (0x6d97F55479Ddd427F047F01AFFfFcE0f9415F723); // marketing address
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_allowances[sender][msg.sender] != _totalSupply) {
            ...
            .sub(amount, "Dummer Acolasia  Insufficient Allowance");
        }
    ...
    function modeLaunchedMinBuyExemptMaxWallet(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!burnModeExemptTrading[sender] && !burnModeExemptTrading[recipient] && recipient != uniswapV2Pair) {
            require((_balances[recipient] + amount) <= _maxWallet, "Dummer Acolasia  Max wallet has been triggered");
        }
        
        require((amount <= _maxTxAmount) || limitWalletExemptTeamAuto[sender] || limitWalletExemptTeamAuto[recipient], "Dummer Acolasia  Max TX Limit has been triggered");

        if (exemptBurnMarketingTeam()) {exemptTeamLiquidityAuto();}

        _balances[sender] = _balances[sender].sub(amount, "Dummer Acolasia  Insufficient Balance");
