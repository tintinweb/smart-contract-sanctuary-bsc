...
contract stakereward {
    ...
    uint[] public stakeLevel = [500*1e14,1000*1e14,3000*1e14,5000*1e14,10000*1e14,20000*1e14];

    mapping(address => uint) public usrLastLevel;
    mapping(address => mapping(uint => uint)) public usrLevelStakeCounts;
    ...
    constructor(address new_lpToken,address new_ticketToken,address new_inviteAddr,uint256 new_DURATION,uint256 new_rewardRate) {
        ...
        public
    {
        ...
        require(_type < stakeLevel.length,"_type err");
        require(_type >= usrLastLevel[msg.sender] && _type - usrLastLevel[msg.sender] <= 1,"Level err");
        if(_type - usrLastLevel[msg.sender] == 1)
        {
            require(usrLevelStakeCounts[msg.sender][usrLastLevel[msg.sender]] >= 3,"LevelStakeCounts err");
        }
        require(usrHaveStake[msg.sender] < 2,"usrHaveStake err");

        usrLevelStakeCounts[msg.sender][_type] = usrLevelStakeCounts[msg.sender][_type].add(1);
        usrLastLevel[msg.sender] = _type;

        uint lpAmounts = getLPAmount(_type).mul(3).div(10);
        uint ticketAmounts = getticketAmount(_type).mul(3).div(100);
        ...
        external 
    {
        ...
        uint invAmount = rewardAmt.mul(inviteFee).div(BASE);
        IERC20(lpToken).safeTransfer(inviteAddr,invAmount);
        IERC20(lpToken).safeTransfer(msg.sender,rewardAmt.sub(invAmount));

        invItem memory itemRecord;
        itemRecord.invTime = block.timestamp;
        itemRecord.rewards = invAmount;
        ...
        external onlyOwner
    {
        ...
        {
            if(stakeRecord[_id[i]]._stakeTotal)
            {
                continue;
            }
        ...
        external onlyOwner
    {
        for(uint i=0;i<_id.length;i++)
        {
            if(stakeInfo[_id[i]].trueEndUpdateTime <= block.timestamp)
            {
                continue;
            }
