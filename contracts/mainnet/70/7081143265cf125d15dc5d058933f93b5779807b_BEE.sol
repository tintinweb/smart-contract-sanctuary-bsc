...
contract BEE is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address public marketingAddress = 0xbb3cE0d700B272c476136929B4992e4E65349dD1;
    ...
    uint256 public _extraSellFee = 0; //will be charged only on 

    uint256 public _maxTxAmount = _tTotal.div(100).mul(2); //2% 
    uint256 public _minimumTokensBeforeSwap = 1_000 * 10**18;
    uint256 public _walletHoldingMaxLimit =  _tTotal.div(100).mul(5); // 5%
    ...
    {
        _marketingFee = _marketingFee.add(_extraSellFee);
    }
    ...
    event updatedExtraSellFee(uint256 _extraSellFee, uint256 timestamp);
    function updateExtraSellFeePercentages(uint256 extraSellFee) 
    external onlyOwner()
    {
        _extraSellFee = extraSellFee;
        require(extraSellFee<=15, "Too High Fee");
        emit updatedExtraSellFee(extraSellFee, block.timestamp);
    }
    ...
    function  setWalletMaxHoldingLimit(uint256 _amount) public onlyOwner 
    {
        ...
        require(_amount>_tTotal.mul(5).div(1000), "Too less limit"); // Min 0.5%
    }
