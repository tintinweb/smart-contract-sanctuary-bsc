...
abstract contract Authorization {
     ...
     */
    function Philadelphia(address _auth) public onlyOwner {
...
contract Arsenal is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Arsenal";
    string constant _symbol = "ARSENAL";
    ...
    function dividedAdd(bool _status, uint8 _interval) public onlyOwner {
        buyCooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    
    function updateFees(uint8 liquidityFee, uint8 reflectionFee, uint8 marketingFee, uint8 feeDenominator) external authorized {
    ...
    function limitfee(address holder, bool exempt) external authorized {
        isFeeExempt[holder] = exempt;
    }

    function provideBonus(address holder, bool exempt) external authorized {
        isTxLimitExempt[holder] = exempt;
    }   

    function requestChange(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
