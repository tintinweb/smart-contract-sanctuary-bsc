...
contract X8MultiSwap {
    ...
    function swap(
        ...
        require(
            IERC20(tokenIn).transferFrom(address(this), pairs[0], amountToSwap),
            "failed to init swap"
        );
        ...
        for (uint256 i = 0; i < pairs.length - 1; i++) {
            ...
            address swapTo = i < pairs.length - 2 ? pairs[i + 1] : msg.sender; // swap to next pool or sender if final swap
