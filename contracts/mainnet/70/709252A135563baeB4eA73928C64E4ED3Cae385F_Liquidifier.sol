...
contract Liquidifier is AccessControl, ReentrancyGuard, Operations {
    ...
    address internal constant TENESSE_TOKEN =
        0x5430A9E06E2dcc4Fc2760522057fb46128AE463f; //mainnet
    ...
    IStabilityVault internal stabilityVault;
    ...
    constructor(address _vault, address _liquidation, address _oracle, address _stabilityVault) {
        Vault = _vault;
        vault = IVault(Vault);
        ...
        stabilityVault = IStabilityVault(_stabilityVault);
    ...
    function cakeDistribution(
        ...
        uint256 busdvalue = (_rewards * _price) / 1e8;

        uint256 rewardUser = (busdvalue * (percentageUser)) / 100000;

        uint256 rewardLiquidifier = (busdvalue * (percentageLoan)) / 100000;

        uint8 _flag = liquidation.getFlag();

        ///@dev Flag = 0 || Flag = 1 Take Panorams from the rewards vault
        ///@dev Flag = 2 Buy Swap BUSD to Panorams in the pool.
        if(_flag == 0 || _flag == 1 ){
            vault.addReward(rewardLiquidifier);
            uint price = liquidation.getTennessePrice();
           

            reward = (rewardUser * price) / 1e18;
            stabilityVault.withdraw(reward,TENESSE_TOKEN,2);
    ...
    function verifyLiqStatus(uint256 _id, address _lp) public {
        ...
        if (userinvestment[_user][_lp].farming) {
            ...
            emit liquidateStatus(_id, _user, liqprice, status);
        }
    }
    ...
    function liquidationDistribution(
        ...
        uint _price = oracle.getLatestPrice(0);
        vars.busdvalue = (_rewards * _price) / 1e8;
...
abstract contract Operations is AccessControl, ReentrancyGuard {
    ...
    function bnbtoBusd(
        ...
        uint256 _amount = (_ethAmount * _price) / 1e8;
...
interface ILiquidation {
    ...
    function getFlag() external view returns(uint8);

    function getTennessePrice() external view returns (uint);
...
interface IStabilityVault {
    ...
    function withdraw(uint256,address, uint8) external;
