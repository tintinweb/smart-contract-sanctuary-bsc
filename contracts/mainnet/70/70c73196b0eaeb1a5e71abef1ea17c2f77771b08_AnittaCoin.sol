...
contract AnittaCoin is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "Anitta";
    string private _symbol = "ANITTA";
    ...
    function AnittaFromReward(address account) external onlyOwner() {
    ...
    function setAnittaFeeS(uint256 rLiquidity) external onlyOwner {       
    ...
    function setMarketingFeesAnitta(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
	...
	function AnittaDevFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
		return _amount.mul(_devFee).div(
			10**2
		);
	}
	
	function AuotoAnittaIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function timeAnittaClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
