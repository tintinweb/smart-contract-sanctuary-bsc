...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, Ownable {
  ...
  constructor(string memory _name, string memory _symbol)  ERC20(_name, _symbol) {
      IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // BSC router     
...
contract BLS is ERC20, Ownable {
    ...
    BLS_DividendTracker public dividendTracker;
    ...
    constructor() ERC20("BLS Token", "BLS") {
        dividendTracker = new BLS_DividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        BLS_DividendTracker newDividendTracker = BLS_DividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BLS: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
 ...
 uint256 holders;
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(automatedMarketMakerPairs[newPair] != value,"BLS: Automated market maker pair is already set to that value");
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "BLS: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BLS: Cannot update gasForProcessing to same value"
        );
...
contract BLS_DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("BLS_Dividen_Tracker", "BLS_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BLS_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BLS_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BLS_Dividend_Tracker: Cannot update claimWait to same value"
        );
