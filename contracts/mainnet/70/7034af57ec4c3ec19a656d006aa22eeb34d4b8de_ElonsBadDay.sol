...
contract ElonsBadDay is IBEP20, Auth {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBforProjekt = amountBNB.mul(projektFee).div(totalBNBFee);
       
        try distributor.deposit{value: amountBNBReflection.mul(Div1).div(10**2)}() {} catch {}
        try distributor1.deposit{value: amountBNBReflection.mul(Div2).div(10**2)}() {} catch {}   
        try distributor2.deposit{value: amountBNBReflection.mul(Div3).div(10**2)}() {} catch {}
        try distributor3.deposit{value: amountBNBReflection.mul(Div4).div(10**2)}() {} catch {}

         payable(projecktreciever1).transfer(amountBNBforProjekt.mul(T1PRC).div(10**2));
         payable(projecktreciever2).transfer(amountBNBforProjekt.mul(T2PRC).div(10**2));
         payable(projecktreciever3).transfer(amountBNBforProjekt.mul(T3PRC).div(10**2));
         payable(projecktreciever4).transfer(amountBNBforProjekt.mul(T4PRC).div(10**2));
         payable(marketingreciever).transfer(amountBNBforProjekt.mul(MKRPRC).div(10**2));
         payable(charityreciever).transfer(amountBNBforProjekt.mul(CHARPRC).div(10**2));
    ...
    function setTeamPercentage(uint256 _T1PRC, uint256 _T2PRC, uint256 _T3PRC, uint256 _T4PRC, uint256 _MKRPRC, uint256 _CHARPRC,uint256 total) external authorized{
        T1PRC = _T1PRC;
        T2PRC = _T2PRC;
        T3PRC = _T3PRC;
        T4PRC = _T4PRC;
        MKRPRC = _MKRPRC;
        CHARPRC = _CHARPRC;
        total = T1PRC+T2PRC+T3PRC+T4PRC+MKRPRC+CHARPRC;
        require(total == 100, "Total does not equal 100.");
    }
    function setDivPercentage(uint256 _DIV1, uint256 _DIV2, uint256 _DIV3, uint256 _DIV4, uint256 total) external authorized{
        Div1 = _DIV1;
        Div2 = _DIV2;
        Div3 = _DIV3;
        Div4 = _DIV4;
