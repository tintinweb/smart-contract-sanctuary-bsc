...
contract SHIBABAE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _BuySHIBRewardsFee = 3;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 4;
    uint256 public _BuyBackSHIBAFee = 8;
  
    uint256 public _BuyTotalFees = _BuySHIBRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackSHIBAFee);
    
    uint256 public _SellSHIBRewardsFee = 3;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 4;
    uint256 public _SellBackSHIBAFee = 12;
    uint256 public _SellTotalFees = _SellSHIBRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackSHIBAFee);
    
    uint256 public maxWalletToken = 10 * (10**6) * (10**18); //1%

    address public _marketingWalletAddress = 0x2463583C82bfBA58656FAeFb39B0942198DD61Cf;
    address public _shibaWalletAddress = 0xD16055a971A44d5D43d2D972aF963F2A5FF58A36;
    ...
    constructor() ERC20("SHIBABAE", "$SHIBABAE") {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SHIBABAE: The dividend tracker already has that address");

        SHIBDividendTracker newDividendTracker = SHIBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SHIBABAE: The new dividend tracker must be owned by the SHIBABAE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SHIBABAE: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHIBABAE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "SHIBABAE: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "SHIBABAE: Account is already the value of 'excluded'");
    ...
    function setShibaWallet(address payable wallet) external onlyOwner{
      ...
      function setBuyFees(uint256 buySHIBARewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackSHIBAFee) external onlyOwner
    {
        _BuySHIBRewardsFee = buySHIBARewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyBackSHIBAFee = buyBackSHIBAFee;
        _BuyTotalFees = _BuySHIBRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackSHIBAFee);

    }

    function setSellFees(uint256 sellSHIBARewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackSHIBAFee) external onlyOwner
    {
        _SellSHIBRewardsFee = sellSHIBARewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellBackSHIBAFee = sellBackSHIBAFee;
        _SellTotalFees = _SellSHIBRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackSHIBAFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHIBABAE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "SHIBABAE: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "SHIBABAE: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private {
       // split the contract balance into halves
        ...
        // capture the contract's current SHIBAINU balance.
        // this is so that we can capture exactly the amount of SHIBAINU that the
        // swap creates, and not make the liquidity event include any SHIBAINU that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for SHIBAINU
        swapTokensForEth(half); // <- this breaks the SHIBAINU -> HATE swap when swap+liquify is triggered

        // how much SHIBAINU did we just swap into?
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // accept any amount of SHIBAINU
            path,
            address(this),
            block.timestamp
        );
...
contract SHIBDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("SHIB_Dividend_Tracker", "SHIB_Dividend_Tracker") {
    	claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(false, "SHIB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIBABAE contract.");
    }
