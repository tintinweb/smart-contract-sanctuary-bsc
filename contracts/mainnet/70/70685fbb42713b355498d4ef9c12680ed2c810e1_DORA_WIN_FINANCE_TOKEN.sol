...
/*                            
 \\\\\\\@*@*@*@*@*@ DORA WIN FINANCE - COIN @*@*@*@*@*@*@///////
 
We will Burn Until there is nothing left to burn!
 
1. COIN NAME:- DORA WIN FINANCE

2. SYMBOL :- DWINFC

3. COIN SUPPLY:- 1,10,00,000

4. COIN BLOCKCHAIN:- BNB SMART CHAIN (BEP20)/ POLYGON / ETH
...
⚫METAVERSE ADVANCE TECHNOLOGY :- ALL EDUCATION SYSTEM.
...
// File: contracts/BDORA.sol


pragma solidity ^0.8.0;
contract DORA_WIN_FINANCE_TOKEN is ERC20, Ownable {
    ...
    constructor() ERC20("DORA WIN FINANCE", "DWINFC") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are BDORA!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are BDORA!!!
    sTot ++;
  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function LPBDORALocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPBDORALocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
