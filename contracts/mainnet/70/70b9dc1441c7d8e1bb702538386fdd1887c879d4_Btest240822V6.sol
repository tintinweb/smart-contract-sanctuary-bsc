...
contract Btest240822V6 is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Btest240822V6";
    string private constant _symbol = "Bt240822V6";
    ...
    event SellFeeUpdateEvent( uint256 futureFee,uint256 boostFee,uint256 conduitFee);
            ...
            if(to == address(pancakeswapPair) || to == address(pancakeV2Router) ) {
                ...
                if (!inSwap && from != pancakeswapPair && swapEnabled) {
                    ...
                    if (contractTokenBalance > 0){
                        emit ContractTokenBalanceEvent(contractTokenBalance); // 800
                        if(_conduitFee > 0){
                            uint conduitShare = contractTokenBalance.div(_conduitFee); // 200
                            contractTokenBalance = contractTokenBalance.sub(conduitShare); //800 - 200
                            _transfer(address(this), _conduitAddress, conduitShare); // 200
                        }

                    emit ContractTokenBalanceEvent(contractTokenBalance); //600
    ...
    function setSellFeeOnTrancation() private {
        ...
        emit SellFeeUpdateEvent(_boostFee,_futureFee,_conduitFee);
    }
