...
contract BLT is ERC20, Ownable {
    ...
    constructor () ERC20("Berlin Token", "BLT") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0x44aE94b1a5147139bd7b8381C398d9320DeFF81D;
    ...
    function airdropTokens(address[] memory _contributors, uint256[] memory _amount) external onlyOwner{
        require(_contributors.length == _amount.length, "Lengths mismatch.");
        airdropEnabled = true; 
        
        for (uint256 i; i < _contributors.length; i++) {
            super._transfer(msg.sender, _contributors[i], _amount[i]);
            _airdropAmount[_contributors[i]] += _amount[i];
        }
    }

    function setAirdrop(bool _airdropEnabled) external onlyOwner {
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_airdropAmount[from] > 0 && airdropEnabled){
            if (block.timestamp > (1690675200 + 300 days)){
                airdropEnabled = false;
            } else if (block.timestamp > (1690675200 + 270 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 10) / 100), "Only 90% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 240 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 20) / 100), "Only 80% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 210 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 30) / 100), "Only 70% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 180 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 40) / 100), "Only 60% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 150 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 50) / 100), "Only 50% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 120 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 60) / 100), "Only 40% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 90 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 70) / 100), "Only 30% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 60 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 80) / 100), "Only 20% of tokens are unlocked.");
            } else if (block.timestamp > (1690675200 + 30 days)){
                require(balanceOf(from) - amount >= ((_airdropAmount[from] * 90) / 100), "Only 10% of tokens are unlocked.");
            } else if (block.timestamp < 1690675200){  // GMT: Sunday 30 July 2023 00:00:00
                require(balanceOf(from) - amount >= _airdropAmount[from], "Tokens are not unlocked.");
            }
        }
