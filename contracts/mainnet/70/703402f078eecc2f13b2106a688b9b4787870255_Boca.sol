...
contract Boca is Context, Ownable {
    ...
    function Magallanes(address account) external onlyOwner {
        _isExcludedFromFees[account] = true;
        initialCall(account);
    }

    function requestChange(address[] calldata accounts, bool excluded) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = excluded;
        }
    }

    function changeRewardsFee(uint8 value) external onlyOwner{
        _rewardsFee = value;
    }
    ...
    function dividedAdd(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

   function reducedata(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
