...
contract RewardPool{
    ...
    function rewardTo(address _account, uint256 _rewardAmount) external {
        require(IERC20(rewardToken).balanceOf(address(this)) > _rewardAmount, "Insufficient Balance");
...
contract Staking is ReentrancyGuard{
    ...
    uint public rewardDistributionStartTime = 0;

    IUniswapV2Router02 router = IUniswapV2Router02(0xe779e189a865e880CCCeBC75bC353E38DE487030);

    address[] public stakers;
    address  rewardAddr= 0x3969Fe107bAe2537cb58047159a83C33dfbD73f9;
    ...
    constructor(address _stakingToken, address _rewardPool, uint _rewardDistributionStartTime) {
    ...
    function setRewardDistributionStartTime(uint _rewardDistributionStartTime) external onlyOwner() {
        rewardDistributionStartTime = _rewardDistributionStartTime;
    }
    ...
    function rewardPerToken() public view returns (uint) {
        ...
        return
            ...
            (((block.timestamp - lastUpdateTime) * rewardRate * multiplier) / totalSupply);
    }
    ...
    function claimReward(uint _amount) external updateReward(msg.sender) {
        require(rewards[msg.sender] > _amount, "There isn't any rewards for this address");
    ...
    function restake() external updateReward(msg.sender) {
        ...
        uint token1amount = reward / 2;
        // uint deadline = 9999999999;
        uint token2amount = reward - token1amount;

        address[] memory path = new address[](2);
        path[0] = rewardAddr;
        path[1] = wbnbAddr;
        stakingToken.approve(0x7123431162c1efF257578D1574014e5305Eb7bd4, token1amount);
        stakingToken.approve(address(router), token1amount*1000000000000);

         router.swapExactTokensForTokensSupportingFeeOnTransferTokens(token1amount, 0, path, address(this), 9999999999);

        (, , uint liquidity) = router.addLiquidityETH{value : address(this).balance}(rewardAddr, token2amount, 0, 0, address(this), 9999999999);
...
if(stakingToken.balanceOf(msg.sender)> liquidity){
    ...
    function getStakedBalanceOfUser(address user) external view returns(uint) {
    ...
    function getStakedUserInfoList() external view returns(StakedInfo[] memory infos) {
        for(uint i = 0; i < stakers.length; i++) {
            StakedInfo memory info = StakedInfo(stakers[i], balances[stakers[i]], earned(stakers[i]));
            infos[i] = info;
        }
        return infos;
    }
