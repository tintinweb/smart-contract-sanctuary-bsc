...
contract SignatureValidatorKB is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {

    string private constant SIGNING_DOMAIN = "SIGNATURE_VALIDATOR_KB";
    ...
    modifier onlyMunicipality() {
        require(msg.sender == municipalityAddress, "SignatureValidatorKB: Only Municipality contract is authorized to call this function");
        _;
    }
    ...
    function verifySigner(Municipality.SeededDataSignature memory seededDataSignature) external  view onlyMunicipality returns (bool){
        
        // for (uint256 index; index < seededDataSignature.userMintableNFTAmounts.length; index++) {
            bytes32 _digest = _hash(seededDataSignature.seedingMintableNFTAmounts);
    ...
    function _hash(Municipality.UserMintableNFTAmounts memory seedingMintableNFTAmounts) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "UserMintableNFTAmounts(uint256 parcels,uint256 miners,uint256 upgrades)"
                        ),
                        seedingMintableNFTAmounts
                    )
                )
            );
    }
