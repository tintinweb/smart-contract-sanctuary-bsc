...
contract Fidometa is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!frozenAccount[_msgSender()], "funds are frozen in sender account");
    ...
    function unlock(address target_) external {
        ...
        uint256 percentage = locks[target_].percentage;
       ...
       uint256 parts=  100/percentage;

        uint256 monthNumber = (uint256(timePassed) + (uint256(30 days) - 1)) /
            uint256(30 days);
        if(monthNumber>parts) monthNumber=parts;


        uint256 remainedMonth = monthNumber - monthCount;

        if (remainedMonth > parts) remainedMonth = parts;
        require(remainedMonth > 0, "Releasable token till now is released");

        uint256 receivableToken = (lockedToken * (remainedMonth * percentage)) / 100;
