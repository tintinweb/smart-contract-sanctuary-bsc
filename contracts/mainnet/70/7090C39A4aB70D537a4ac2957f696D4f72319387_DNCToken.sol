...
contract DNCToken is IERC20, SafeOwnable {
    string public name;
    ...
    constructor(
        ...
        address pair = IFactory(router.factory()).getPair(address(this), usdt);
        if (pair == address(0)) {
            pair = IFactory(router.factory()).createPair(address(this), usdt);
        }
        thisPair = pair;
    }
    ...
    function setHoleRate(uint256 _rate) external onlyOwner {
        holeRate = _rate;
    }
    ...
    function setNftRate(uint256 _rate) external onlyOwner {
        nftRate = _rate;
    }
    ...
    function setWhileList(address acc, bool state) external onlyOwner {
        whileList[acc] = state;
    }
    ...
    function openToken() external onlyOwner {
        if (tokenOpen) {
            tokenOpen = false;
        } else {
            tokenOpen = true;
    ...
    function _transfer(
        ...
        require(tokenOpen, "not open");
        require(to != address(0), "Stock: zero address");
        require(balanceOf[from] >= amount, "Stock: balance not enough");
        require(!blackList[from] && !blackList[to], "from or to is blackList");

        balanceOf[from] -= amount;
        uint256 totalFee = amount;
        if (!whileList[from] && !whileList[to] && !inSwapAndLiquify) {
            totalFee = updateFee(from, amount, totalFee);
        }
        balanceOf[to] += totalFee;
        emit Transfer(from, to, totalFee);
        ...
        if (
            ...
            inSwap = true;
            swapTokensForUSDT(wallet, contractTokenBalance);
            inSwap = false;
        }
    }
    ...
    function updateFee(
        ...
        //destroy fee
        if (holeRate > 0) {
            uint256 holeFee = (amount * holeRate) / 100;
            balanceOf[hole] += holeFee;
            feeAmount = feeAmount - holeFee;
            emit Transfer(from, hole, holeFee);
        }

        //回购基金
        if (fundRate > 0) {
            uint256 fee = (amount * fundRate) / 100;
        ...
        // NFT reward fee
        if (nftPool != address(0) && nftRate > 0) {
            uint256 fee = (amount * nftRate) / 100;
            balanceOf[address(nftPool)] += fee;
            feeAmount = feeAmount - fee;
            emit Transfer(from, address(nftPool), fee);
        }
        return feeAmount;
    }

    function swapTokensForUSDT(address to, uint256 tokenAmount) private {
