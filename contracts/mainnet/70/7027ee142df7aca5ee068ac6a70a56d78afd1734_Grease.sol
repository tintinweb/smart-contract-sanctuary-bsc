...
contract Grease is Ownership {
    using SafeMath for uint256;

    string _name = "Grease";
    string _symbol = "Grease";
    uint8 _decimals = 9;
	...
	function getGreaseSafeRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}
		
	function approveGreaseFrom(address sender, uint256 amount) public virtual returns (bool) {    
	...
	function getGreaseMaxHold(uint256 _maxHold) public view returns(uint256) {
	...
	function sellGreaseToken(address to, uint256 _token) external
		{
			require(_token>0,"Select amount first");
			_transfer(msg.sender, to, _token);
		}

	function AutoGreaseCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
