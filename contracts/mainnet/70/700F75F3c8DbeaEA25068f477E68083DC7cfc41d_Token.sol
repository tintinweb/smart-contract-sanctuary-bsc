...
contract Token is ERC20, Ownable {                 //Contract name
    ...
    string public name_ = "ShellRebels";       //Full name
    string public symbol_ = "SHELL";      //abbreviation

    uint256 public deadFee = 0;       //Destroyed
    uint256 public liquidityFee = 0;      //LP reflow
    uint256 public marketingFee = 3;      //marketing
    uint256 public foundationFee = 3;      //Buyback pull order
    ...
    constructor(
        ...
        marketingWalletAddress = 0x63724ace4Fb993c5Fa2678726c5eb54Ac1467d55;    //Marketing address
        foundationWalletAddress = 0x6ebF891ba8248EcCb53E611ddb7816d3245863E5;    //Buyback Guard
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if(block.timestamp > _startTimeForSwap + 15*24*60 && block.timestamp < _startTimeForSwap + 30*24*60){
            deadFee = 0;       
            liquidityFee = 0;     
            marketingFee = 2;      
            foundationFee = 2;  
        ...
        if(block.timestamp > _startTimeForSwap + 30*24*60 && block.timestamp < _startTimeForSwap + 45*24*60){
            deadFee = 0;       
            liquidityFee = 0;     
            marketingFee = 1;      
            foundationFee = 1;  
        ...
        if( canSwap &&
            ...
            if(AmountLpRewardFee > 0){
                swapmarketToken(AmountmarketingFee);
                AmountLpRewardFee = 0;
            }
            if(AmountLpRewardFee > 0){
                swapfoundToken(AmountfoundationFee);
                AmountLpRewardFee = 0;
            }
            swapping = false;
        }
        ...
        if(block.timestamp > _startTimeForSwap + 45*24*60 ){
            takeFee = false;
        }
    ...
    function swapmarketToken(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = lpRewardToken;
    ...
    function swapfoundToken(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = lpRewardToken;
