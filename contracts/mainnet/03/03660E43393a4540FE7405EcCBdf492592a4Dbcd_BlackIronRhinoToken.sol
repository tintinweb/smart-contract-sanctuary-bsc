...
import "./BlackIronRhinoTokenStorage.sol";


contract BlackIronRhinoToken is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, BlackIronRhinoTokenStorage {
    ...
    constructor(string memory baseURI_, address payee_) ERC721("BlackIronRhinoToken","BIRT") {
        baseURI = baseURI_;
        payee = payee_;
    }
    ...
    function burn(address owner, uint256 tokenId) external onlyMerge returns(address, uint256){
        require(owner == ownerOf(tokenId), "ERC721: ID does not exist");
        _burn(tokenId);
...
contract BlackIronRhinoTokenStorage {
