...
contract ChainBook is ERC20, Ownable {
    ...
    uint256 public MktFee = 3;
    uint256 public DevFee = 2;
    uint256 public LpFee = 2;
    uint256 public BurnFee = 1;

    uint256 public AllShare;
    uint256 public UsefulShare = 14;
    uint256 public OtherShare = 2;
    ...
    constructor() public ERC20("ChainBook", "CNB") {
        ...
        burnEndAmount = totalSupply_.mul(90).div(100); // 90% stop burn
        ...
        excludeFromFees(_AppWalletAddress, true);
    ...
    function _transfer(
        ...
        if (from != address(_PresaleAddress)) {
            if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
                uint256 fees = amount.mul(AllFee).div(100);
        ...
        if( canSwap &&
            ...
            uint256 tokenAmountForUseful = tokenAmount.mul(UsefulShare).div(AllShare);
            uint256 tokenAmountOther = tokenAmount.sub(tokenAmountForUseful);
            uint256 tokenAmountLP = tokenAmountOther.div(2);
            uint256 tokenAmountBurn = tokenAmountOther.sub(tokenAmountLP);

            if(tokenAmountBurn>0){
                uint256 burnAmount = balanceOf(address(deadWallet));
                if (burnAmount < burnEndAmount) {
                    super._transfer(address(this),deadWallet,tokenAmountBurn);
                    tokenAmountBurn = 0;
                }
            }
            
            swapAndLiquify(tokenAmountLP+tokenAmountBurn);
            swapTokensForEth(tokenAmountForUseful);
            ...
            payable(_FoudWalletAddress).transfer(BNBAmountForMkt);
