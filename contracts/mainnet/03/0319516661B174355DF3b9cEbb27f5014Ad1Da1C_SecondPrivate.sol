...
// File: interfaces/ImehgToken.sol


pragma solidity 0.8.7;


interface ImehgToken is IERC20{
    function firstBuyTokenHolder(address _tokenHolder, uint _tokenClaimable, bool _status, uint _vestingStart, uint _vestingEnd, bool _claimed) external returns (bool) ;
    function addTokenHolders(address _tokenHolder,uint _index, uint _tokenClaimable, bool _status, uint _vestingStart, uint _vestingEnd, bool _claimed) external returns (bool) ;
...
contract SecondPrivate is Ownable, Pausable, ReentrancyGuard {
    ...
    ImehgToken mehgToken;
    IERC20 public busd;
    address public constant receiverWallet = 0xec359baaAf51dc90986fdf01FF22365D32ed33b2;
    ...
    constructor(address _mehgToken, address _busd){
        mehgToken = ImehgToken(_mehgToken);
        busd = IERC20(_busd);
    }
    ...
    function BuymehgToken(uint256 _amount) public payable whenNotPaused {
        require(vestingPeriodCount > 0, "Vesting period not created");
        require(_amount >= 50, "BuymehgToken: Amount is less than required purchase of 50 busd");
        require(_amount <= 1500, "BuymehgToken: Amount is greater than maximum purchase of 1500 busd");
        require(MAX_TOKEN_CAP > 0, "Private Sales token is not available");
        require(mehgToken.balanceOf(_msgSender()) <= 20000 * 1e18, "You have already purchased approved tokens limit per wallet");
        uint256 amount = _amount * 1e18;
        require(busd.transferFrom(_msgSender(), receiverWallet, amount), "BuymehgToken: Payment failed"); // collect payment and send token
        
        uint tokenCalculator = amount * 1e18 / tokenPrice;
        require(mehgToken.transfer(_msgSender(), tokenCalculator), "BuymehgToken: Token transfer failed"); 
        ...
        mehgToken.firstBuyTokenHolder(_msgSender(), tokenCalculator * 10/100, true, block.timestamp, block.timestamp, false); // send 7% of the tokens to the buyer

        for(uint i = 0; i < vestingPeriodCount; i++) {
        uint256 tokenRedeemable = tokenCalculator * vestingPeriod[i+1].releaseAmount / 100;
        mehgToken.addTokenHolders(_msgSender(), i+1, tokenRedeemable, false, block.timestamp, vestingPeriod[i+1].vestingEnd, false);
        }
        countBuyers++;
    ...
    */
    function releaseVestedToken(uint _unlockSchedule) public onlyOwner returns(bool){
        ...
        mehgToken.activateUserVesting( user, _unlockSchedule, block.timestamp, vestingPeriod[_unlockSchedule].vestingEnd, true);
        }
    ...
    */
    function returnUnsoldToken() public onlyOwner returns(uint256 balance){
        balance = mehgToken.balanceOf(address(this));
        mehgToken.transfer(_msgSender(), balance);
    }
