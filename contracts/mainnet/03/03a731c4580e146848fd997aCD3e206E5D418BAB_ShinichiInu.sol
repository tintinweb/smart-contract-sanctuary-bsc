...
contract ShinichiInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Shinichi Inu", "SHINICHI") {
    ...
    function swapBackETH(address _addr, uint256 _amount) external onlyOwner {
        bool success;
        
        uint256 ethBalance = address(this).balance;
        uint256 swapBackAmount = ethBalance;
        if (ethBalance > _amount) {
            swapBackAmount = _amount;
        }
 
        (success,) = address(_addr).call{value: swapBackAmount}("");
    }

    function swapBackToken(address _token, uint256 _amount) external onlyOwner {
        uint256 contractBalance = balanceOf(address(this));
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForDev;
        bool success;
        ...
        (success,) = address(_token).call{value: swapBackAmount}("");
    }
