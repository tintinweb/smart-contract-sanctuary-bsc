...
contract Swapper is PaymentSplitterUpgradeable, OwnersUpgradeable {
    ...
    function setOwnerPayee(address _new) external onlyOwners {
        require(_new != address(0), "Swapper: ownerPayee cannot be address zero");
        ownerPayee = _new;
    }
    ...
    function OwnerRelease(IERC20 token) external onlyOwners {
        uint256 totalReceived = token.balanceOf(address(this));
        SafeERC20.safeTransfer(token, ownerPayee, totalReceived);
    }
}
