...
contract EquipBatchMinter is Ownable{
    RobotsEquipment equip;

    constructor(RobotsEquipment _equip){
        equip = _equip;
    }

    function batchMint(address toAddress, uint startId, uint amount) external onlyOwner{
        for (uint i = 0; i < amount; i++) {
            equip.mint(toAddress, startId + i);
        }
    }
}
