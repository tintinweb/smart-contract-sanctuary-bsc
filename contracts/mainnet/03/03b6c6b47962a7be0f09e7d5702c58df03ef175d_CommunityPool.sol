...
contract CommunityPool is ReentrancyGuard, Ownable, Pausable {
    address public NFT_SERVICE_CONTRACT;
    uint256 public TOTAL_SALE_1;
    uint256 public TOTAL_SALE_2;
    uint256 public TOTAL_SALE_3;
    uint256 public STARTED_AT;
    uint256 public FINISHED_AT;
    address public PAYMENT_TOKEN;
    address public TOP_UP_ADDRESS;
    ...
    function createMarketSale(
        ...
        require(quantity > 0 && quantity <= 10, "Invalid quantity to sale");
        require(quantity <= REMAIN_1 + REMAIN_2 + REMAIN_3, "Not enough seed to sale");

        uint256 price;
        if (TOTAL_NFT_SOLD < 1000) {
            price = 1 * 10 ** 18;
        } else if (TOTAL_NFT_SOLD < 2000) {
            price = 2 * 10 ** 18;
        } else {
            price = 3 * 10 ** 18;
        }
