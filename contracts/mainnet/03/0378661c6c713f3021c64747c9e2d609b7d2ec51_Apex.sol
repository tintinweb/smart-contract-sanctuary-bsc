...
contract Apex is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 APEX-WETH LP -> APEX pool
    uint256 public constant INITIAL_APEX_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the APEX ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("APEX", "APEX") {
        // Mints 1 APEX to contract creator for initial pool setup
    ...
    function _getApexPrice() internal view returns (uint256 _apexPrice) {
        try IOracle(apexOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Apex: failed to fetch APEX price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _apexPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_apexPrice >= taxTiersTwaps[tierId]) {
    ...
    function setApexOracle(address _apexOracle) public onlyOperatorOrTaxOffice {
        require(_apexOracle != address(0), "oracle address cannot be 0 address");
        apexOracle = _apexOracle;
    }
    ...
    /**
     * @notice Operator mints APEX to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of APEX to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentApexPrice = _getApexPrice();
            currentTaxRate = _updateTaxRate(currentApexPrice);
            if (currentApexPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_apexPool != address(0), "!_apexPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_apexPool, INITIAL_APEX_POOL_DISTRIBUTION);
