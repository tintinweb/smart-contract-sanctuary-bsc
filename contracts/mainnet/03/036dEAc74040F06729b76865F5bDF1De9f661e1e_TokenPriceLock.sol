...
contract TokenPriceLock is ITokenToolLock,Ownable {
    ...
    function lock(
        ...
        revert("This token is not a LP token");
            ...
            id = _lockNormalToken(owner, token, amount, unlockPrice);
        }
    ...
    function _lockLpToken(
        ...
        id = _addLock(owner, token, amount, unlockPrice);
    ...
    function unlock(uint256 lockId) external override validLock(lockId) {
        ...
        require(getPrice(userLock.token) >= userLock.unlockPrice, "It is not time to unlock");
    ...
    function editLock(
        ...
        if (newUnlockPrice > 0) {
            require(
                newUnlockPrice >= userLock.unlockPrice && newUnlockPrice > getPrice(userLock.token),
    ...
    function getPrice(address token)public  view  returns (uint256){
        ...
        address factoryPair1 = IUniswapV2Factory(factory).getPair(
         ...
         uint256  factoryPair1_balance = IERC20(token).balanceOf(factoryPair1);

         address factoryPair2 = IUniswapV2Factory(factory).getPair(
            token,
            USDT
        );
        uint256  factoryPair2_balance = IERC20(token).balanceOf(factoryPair2);

        if(factoryPair1_balance>=factoryPair2_balance){
           path[0]=token;
           path[1]=WBNB;
           path[2]=USDT;
                 uint256[] memory AmountsOuts= IUniswapV2Router02(Router).getAmountsOut(1*10**IERC20MetadataUpgradeable(token).decimals(),path);
