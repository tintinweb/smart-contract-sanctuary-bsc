...
contract NFTSales is Ownable, NFTType, ReentrancyGuard, Proxyable {
    ...
    function uncrate(uint32 id) external payable senderOwnsToken(id) {
        ...
        string memory unknownIDs = uint256(id).toString();
        for (uint x; x < nftsToMint; x++) {
            unknownIDs = string.concat(unknownIDs,",",(nextTokenID + x).toString());
        }
        ...
        senderOwnsTokens(ids)
    {
        ...
        for (uint256 x; x < ids.length; x++) {
            ...
            unknownIDs = string.concat(unknownIDs, uint256(ids[x]).toString(), ",");
        }
