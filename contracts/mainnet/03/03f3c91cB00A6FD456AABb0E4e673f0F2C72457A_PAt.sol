...
contract PAt is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee,uint256 tBurn,uint256 tReferrerFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tMarketingFee,tBurn,tReferrerFee);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tMarketingFee, uint256 tBurn,uint256 tReferrerFee) private view returns (uint256, uint256, uint256) {
        uint256 currentRate=_getRate();
    ...
    function _takeReferrerFee(address userAddress,uint256 tReferrerFee) private {
    ...
    function dividendsToReferrer(address from,uint256 tAmount,uint256 rAmount)private{
        uint8 i=1;
        ...
        while (true) {
            ...
            uint tAmountDividend=getAmountDividend(tAmount,i);
            uint rAmountDividend=getAmountDividend(rAmount,i);
            if (referalAddress==address(0)){ 
                _rOwned[defaultReferrerAddress] = _rOwned[defaultReferrerAddress].add(rAmountDividend);
                
                if(_isExcluded[defaultReferrerAddress]){
                 _tOwned[defaultReferrerAddress] = _tOwned[defaultReferrerAddress].add(tAmountDividend);
                }
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair){
            if (block.timestamp <tradingEnabledTimestamp + 3 hours){
                ...
                require(buyAmount[to]<=100000*10**9,"Transaction exceeds limit");    
            }
        }
        ...
        if (!address(from).isContract()&&!address(to).isContract()&&to!=owner()){
        ...
        if (to==uniswapV2Pair||to==uniswapV2Pair2){
            if(from!=address(this)){
             require(balanceOf(from).sub(amount)>=100000000,"You must keep 0.1 tokens in your wallet");
            }
