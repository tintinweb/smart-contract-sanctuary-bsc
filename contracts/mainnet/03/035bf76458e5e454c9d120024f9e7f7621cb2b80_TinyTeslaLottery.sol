...
// File: contracts/interfaces/IPantintSwapLottery.sol


interface IPantintSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in TInt per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TINT token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInTint: price of a ticket in TINT
...
// File: contracts/TinyTeslaLotteryV2.sol

pragma abicoder v2;

/** @title tintyTesla Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract TinyTeslaLottery is ReentrancyGuard, IPantintSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInTint = 50 ether;
    uint256 public minPriceTicketInTint = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTint;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] tintPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTint;
        uint32 finalNumber;
    }
     ...
     * @param _tintTokenAddress: address of the TINT token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _tintTokenAddress, address _randomGeneratorAddress) {
        tintToken = IERC20(_tintTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of TINT to this contract
        uint256 amountTintToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTint,
            _ticketNumbers.length
        );

        // Transfer tint tokens to this contract
        tintToken.safeTransferFrom(address(msg.sender), address(this), amountTintToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTint += amountTintToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the Tint
        uint256 rewardInTintToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInTintToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        tintToken.safeTransfer(msg.sender, rewardInTintToTransfer);

        emit TicketsClaim(msg.sender, rewardInTintToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTint) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in TINT for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].tintPerBracket[j] =
                ...
                // A. No TINT to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].tintPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTint - amountToShareToWinners);

        // Transfer TINT to treasury address
        tintToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        tintToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTint += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInTint >= minPriceTicketInTint) && (_priceTicketInTint <= maxPriceTicketInTint),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTint: _priceTicketInTint,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            tintPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTint: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(tintToken), "Cannot be TINT token");
    ...
    function setMinAndMaxTicketPriceInTint(uint256 _minPriceTicketInTint, uint256 _maxPriceTicketInTint)
        external
        onlyOwner
    {
        require(_minPriceTicketInTint <= _maxPriceTicketInTint, "minPrice must be < maxPrice");

        minPriceTicketInTint = _minPriceTicketInTint;
        maxPriceTicketInTint = _maxPriceTicketInTint;
    }
     ...
     * @param _priceTicket price of a ticket (in TINT)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].tintPerBracket[_bracket];
        } else {
            return 0;
        }
    }
