...
contract Zapper is ReentrancyGuard {
    ...
    modifier withValidPath(iPar memory p) {
        ...
        require(
            tokenOutAddress ==
                IErc20Prediction(p.predictionAddress).tokenAddress(),
    ...
    function zap20Into20(
        iPar memory p,
        uint256 amountIn
    ) public nonReentrant notContract withValidPath(p) returns (uint256 amountOut) {
        _before(p, amountIn);
        ...
        amountOut = _zap(p, amounts, p.path[0], amounts[0]);
    }

    function zapNativeInto20(
        iPar memory p
    )
        public
        payable
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = _zap(p, amounts, address(0), amounts[0]);
    }

    function zap20IntoNative(
        iPar memory p,
        uint256 amountIn
    ) public nonReentrant notContract returns (uint256 amountOut) {
        _before(p, amountIn);
