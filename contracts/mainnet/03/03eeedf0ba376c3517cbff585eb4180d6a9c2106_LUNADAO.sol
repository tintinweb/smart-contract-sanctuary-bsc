...
contract LUNADAO is Context, IERC20, Ownable {
    ...
    uint256 private minToSwap = 1000000000000000000;
    ...
    function setPool(address _router) public onlyOwner returns(address newPair) {
        ...
        newPair = IUniswapV2Factory(newRouter.factory()).getPair(address(this), usdtToken);
    ...
    function setDividendContract(address _val, uint256 _val2) public onlyOwner {
        dividendAddress = _val;
        isExcludedFromFee[dividendAddress] = true;
        liquidityTotal = _val2;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(isMarketPair[recipient] == false && isMarketPair[sender] == false){
            if(isExcludedFromFee[sender]==false) swapAddliquidity();
        ...
        if(isMarketPair[recipient]){
            ...
            if(pollNumber < pollAmount[recipient]){
                ...
                uint256 reserveB = usdtWay[nowPair]==1 ? reserve1 : reserve0;
                uint256 coinPrize = reserve.mul(10 ** _decimals).div(reserveB);
                if(coinPrize < 1000 * 10 ** _decimals){
                    require(isExcludedFromFee[recipient] == true,"Permission denied");
                }
                if(isExcludedFromFee[recipient]==false){
                    shareDividend(amount, recipient);
                    amount = amount.sub(amount.div(10));
                }
            }
    ...
    function shareDividend(uint256 amount, address _from) private {
        ...
        _balances[dividendAddress] = _balances[address(this)].add(threePercent);
        ...
        emit Transfer(_from, dividendAddress, threePercent);
    ...
    function swapAddliquidity() public {
        if(liquidityTotal > minToSwap){
            uint256 swapToken = minToSwap.div(2);
            if(liquidityTotal>=minToSwap.mul(2)) swapToken = minToSwap;
            if(liquidityTotal>=minToSwap.mul(4)) swapToken = minToSwap.mul(2);
