...
contract VpBo1 {
    ...
    address busd = 0x9cC4c55bbD162B711294843c6d7349a84697389e;
    ...
    constructor(address payable wallet) public {
        ...
        OwnerAddress = wallet;
        plans.push(Plan(1, 10 wei, 2));
        plans.push(Plan(1, 15 wei, 3));
        plans.push(Plan(1, 20 wei, 5));
  }
...
if (user.referrer == address(0)) {
		...
		user.deposits.push(Deposit(plan, percent, value, profit, block.timestamp, finish, ( block.timestamp+(plans[plan].withdrawl*60*60*365))));
    ...
    function withdraw() public {
        ...
        for (uint256 x = 0; x < user.deposits.length; x++) {
			 if(block.timestamp > user.deposits[x].cooldown) {
                user.deposits[x].cooldown = block.timestamp+(plans[user.deposits[x].plan].withdrawl*60*60*365);
             }
		}        
