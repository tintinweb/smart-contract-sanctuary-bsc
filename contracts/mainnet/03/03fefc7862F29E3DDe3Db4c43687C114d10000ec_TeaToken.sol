...
contract TeaToken is ERC20,Ownable{
    using SafeMath for *;
    ...
    constructor (address addr_)  ERC20("TEA", "TEA") {
        _mint(addr_, 100000000*decimalVal);
        isFeeAddress[addr_] = true;
        isFeeAddress[address(this)] = true;
    }
    function setAddr(address lpAddr_) public onlyOwner {
    ...
    function common(address addr,address to,uint value) internal returns (uint){
        uint transferAmount = value;
           if(!isFeeAddress[addr] && !isFeeAddress[to]){
            ...
            transferAmount = value.sub(lpAmount);
           }    
