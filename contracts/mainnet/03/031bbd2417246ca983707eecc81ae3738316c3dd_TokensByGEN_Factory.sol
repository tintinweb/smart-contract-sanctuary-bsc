...
Reflections Contract Factory by TokensByGEN
...
contract TokensByGenDotCom is Context, IERC20 { 
    ...
    // Init Fees
    uint256 public _fee_Liquidity;
    uint256 public _fee_Marketing;
    uint256 public _fee_Reflection;
    ...
    _fee_Liquidity      = 0; // Set fees after deploying
    _fee_Marketing      = 0; // Set fees after deploying
    _fee_Reflection     = 0; // Set fees after deploying
    _fee_Contract       = 0; // Set fees after deploying

    // Calculate total fee
    _SwapFeeTotal       = _fee_Liquidity + _fee_Marketing + _fee_Contract;
    ...
    event updated_fees(uint256 Marketing, uint256 Liquidity, uint256 Reflection, uint256 Contract_Development_Fee);
    ...
    event set_Contract_Fee(uint256 Contract_Development_Fee_Set);
    ...
    */
    ...
    function Set_Fees(uint256 Marketing, uint256 Liquidity, uint256 Reflection) external onlyOwner {

        _fee_Contract = ContractFee;

        // Buyer protection - max fee can not be set over 20% (including possible 1% contract fee if chosen)
        require(Marketing + Liquidity + Reflection + _fee_Contract <= 20, "Fees too high"); 

        // Update fees
        _fee_Marketing  = Marketing;
        _fee_Liquidity  = Liquidity;
        _fee_Reflection = Reflection;

        // Fees that will need to be processed during swap and liquify
        _SwapFeeTotal   = _fee_Marketing + _fee_Liquidity + _fee_Contract;

        emit updated_fees(_fee_Marketing, _fee_Liquidity, _fee_Reflection, _fee_Contract);
    }
    ...
    */
    ...
    function OpenTrade() external onlyOwner {
        ...
        _fee_Contract = ContractFee;
        emit set_Contract_Fee(_fee_Contract);
    }
    ...
    function swapAndLiquify(uint256 Tokens) private {
        ...
        uint256 LP_Tokens   = Tokens * _fee_Liquidity / _SwapFeeTotal / 2;

        // Calculate tokens that need to be swapped to BNB
        ...
        uint256 LP_BNB      = Swapped * _fee_Liquidity / (_SwapFeeTotal * 2);
        ...
        uint256 BNB_Contract = Swapped * (_fee_Contract * 2) / (_SwapFeeTotal * 2 - _fee_Liquidity);
        if (BNB_Contract > 0){
