...
contract PeaceGigaFactory is Ownable,ERC1155Receiver {
    ...
    constructor(address _nftAddress) {
        ...
        addressInfo.treasuryAddress = payable(0xEb899B87260618c777f3E51d3c5c531f21b71965);
        royaltyInfo.teamPercent = 100;
        addressInfo.teamAddress = payable(0x5BA9C34a9C1468192aD6061C4aAA551Fb291FFB3);
        royaltyInfo.tradingPercent = 100;
        royaltyInfo.PeaceGigaROI = 125;
        royaltyInfo.OneYearROI = 50;
        royaltyInfo.TwoYearROI = 16;
    }
    ...
    function getCurrentROIofNFT(uint256 _nftID) view private returns(uint256) {
        ...
        if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 80) {
    ...
    function getRewardInfoByNFT(uint256 _nftID) view private returns(uint256){
        ...
        if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 80) {
            ...
            if (nftCreatedTime + ONE_DAY_TIME * 80 >= nftClaimedTime) {
                rewardAmount += (nftCreatedTime + ONE_DAY_TIME * 80 - nftClaimedTime) * nftPrice * royaltyInfo.PeaceGigaROI / 10000 / ONE_DAY_TIME;
            }
            rewardAmount += (block.timestamp - (nftCreatedTime + ONE_DAY_TIME * 80)) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
        }
        else if (block.timestamp - nftCreatedTime <= ONE_DAY_TIME * 365 * 2) {
            if (nftCreatedTime + ONE_DAY_TIME * 80 >= nftClaimedTime) {
                rewardAmount += (nftCreatedTime + ONE_DAY_TIME * 80 - nftClaimedTime) * nftPrice * royaltyInfo.PeaceGigaROI / 10000 / ONE_DAY_TIME;
                rewardAmount += (ONE_DAY_TIME * 285) * nftPrice * royaltyInfo.OneYearROI / 10000 / ONE_DAY_TIME;
            }
    ...
    function buyNow(uint256 _tokenID) payable external nonReentrant{
        ...
        destroySale(_tokenID);
        _nftIDsOfUser[msg.sender].push(_tokenID);
    }
    ...
    function customizedTransfer(address payable _to, uint256 _amount, uint8 _kind) internal {
        ...
        if(_amount > 0) {
            if (_kind == 0) {
                ...
                if(_stablecoinAddress == address(0)) return;
                IERC20 token = IERC20(_stablecoinAddress);
