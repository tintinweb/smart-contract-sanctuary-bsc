...
interface IFireRobotV2 {

    function addAmounts(uint256 _amountForUsdt, uint256 _amountToBPool) external;
...
contract FireDaoToken is ERC20, AccessControl {
    ...
    mapping(address => uint256) private bpoolFreeAmounts;
    ...
    function initialize(address _bpool, address _sbp, address _wallet, address _robot, address _exchanger) public onlyRole(ROLE_ADMIN) {
        bpool = _bpool;
        sbp = _sbp;
        wallet = _wallet;
        robot = _robot;
    ...
    function _transfer(address _from, address _to, uint256 _amount) override internal {
        ...
        _doBPoolLogic(_from, _to, _amount);
        ...
        if (_from == exchanger || _to == exchanger) {
            _internalTransfer(_from, _to, _amount);
            return;
        }
        ...
        _doHoldersCheck(_from, _to, _amount);
    ...
    function _doHoldersCheck(address _from, address, uint256 _amount) private view {
        require(_amount < balanceOf(_from), "Transfer: can not transfer all assets");
    }
    ...
    function _doBPoolBuySlippage(address _from, address _to, uint256 _amount) private returns (uint256) {
        ...
        _internalTransfer(address(this), robot, _bpoolTokenAmount);

        IFireRobotV2(robot).addAmounts(_bpoolUsdtAmount, _bpoolTokenAmount);
    ...
    function _doBPoolSellSlippage(address _from, address, uint256 _amount) private returns (uint256) {
        ...
        IFireRobotV2(robot).addAmounts(_bpoolUsdtAmount, 0);

        return _bpoolAmount;
    }

    function _doBPoolLogic(address _from, address _to, uint256 _amount) private {
        if (_from != address(pair)) {
            bpoolFreeAmounts[_to] = bpoolFreeAmounts[_to].add(_amount);
        }

        _doBPoolWithdraw(_from, _amount);
    }

    function _doBPoolWithdraw(address _account, uint256 _transferAmount) private {
        uint256 _currentFreeAmount = bpoolFreeAmounts[_account];
        uint256 _bpoolWithdrawAmount;

        if (_currentFreeAmount >= _transferAmount) {
            bpoolFreeAmounts[_account] = _currentFreeAmount.sub(_transferAmount);
            _bpoolWithdrawAmount = 0;
        } else {
            bpoolFreeAmounts[_account] = 0;
            uint256 _realAmount = _transferAmount.sub(_currentFreeAmount);
            _bpoolWithdrawAmount = _realAmount.mul(30).div(PRECISION);
        }

        if (_bpoolWithdrawAmount != 0) {
            uint256 _bpoolBalance = IFireBPool(bpool).balanceOf(_account);
            if (_bpoolWithdrawAmount > _bpoolBalance) {
                _bpoolWithdrawAmount = _bpoolBalance;
            }

            IFireBPool(bpool).withdraw(_account, _bpoolWithdrawAmount);
        }
    }
