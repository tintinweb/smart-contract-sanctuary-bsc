...
contract Lunavey is IBEP20, Auth {
    ...
    string constant _name = "Lunavey";
    string constant _symbol = "LUNAVEY";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1 * 10000000000 * 10**_decimals;
    ...
    uint256 public rfiFee          = 0;
    uint256 public totalFee        = developmentFee + marketingFee + liquidityFee + buybackFee + rfiFee;
    ...
    constructor () Auth(msg.sender) {
        ...
        address routerV2 = 0xcfCb26A907c69ea12c48D9E5C59E55B12386d3f8; 
        ...
        marketingFeeReceiver = 0xcfCb26A907c69ea12c48D9E5C59E55B12386d3f8;
        buybackFeeReceiver = msg.sender;
        developmentFeeReceiver = msg.sender;
        rfiFeeReceiver = msg.sender; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != rfiFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function shouldTakeFee(address sender) internal view returns (bool) {
     ...
     function takeFee(address sender, uint256 amount, address recipient) internal returns (uint256) {
        ...
        uint256 rfiTokens = feeAmount.mul(rfiFee).div(totalFee);
        uint256 contractTokens = feeAmount.sub(rfiTokens);

        _balances[address(this)] = _balances[address(this)].add(contractTokens);
        _balances[rfiFeeReceiver] = _balances[rfiFeeReceiver].add(rfiTokens);
        emit Transfer(sender, address(this), contractTokens);
        
        if(rfiTokens > 0){
            emit Transfer(sender, rfiFeeReceiver, rfiTokens);    
        }
    ...
    function setFees(uint256 _liquidityFee, uint256 _developmentFee, uint256 _marketingFee, uint256 _buybackFee, uint256 _rfiFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        developmentFee = _developmentFee;
        marketingFee = _marketingFee;
        buybackFee = _buybackFee;
        rfiFee = _rfiFee;
        totalFee = _liquidityFee.add(_developmentFee).add(_marketingFee).add(_buybackFee).add(_rfiFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Buy Fees cannot be more than 50%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buybackFeeReceiver, address _rfiFeeReceiver, address _developmentFeeReceiver) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        buybackFeeReceiver = _buybackFeeReceiver;
        rfiFeeReceiver = _rfiFeeReceiver;
