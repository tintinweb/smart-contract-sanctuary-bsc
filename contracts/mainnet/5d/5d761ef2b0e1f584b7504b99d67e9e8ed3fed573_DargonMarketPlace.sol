 ...
 * @dev Required interface of an ERC721 compliant contract.
 */
interface IERC721checkIsDestroy {
    function checkIsDestroyNFT(uint256 id) external returns (bool database);
}

contract DargonMarketPlace is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address _tokenBase, address _addressReceiceFee) {
     ...
     */
    function setFeeSeller(uint256 _newFee)
        public
        onlyOwner
    {
        feeSeller = _newFee;
    ...
    function createMarketItem(
        ...
        require(!IERC721checkIsDestroy(nftContract).checkIsDestroyNFT(tokenId), "NFT is destroyed");
     ...
     */
    function buyItemDirectly(uint256 itemId) external nonReentrant whenNotPaused {
        ...
        require(
            !idToMarketItem[itemId].isCanceled,
        ...
        uint256 feeSellerAmount = price * feeSeller / 10000;
        uint256 receiveSellerAmount = price - feeSellerAmount;

        IERC20(tokenBase).transferFrom(msg.sender, seller, receiveSellerAmount);
        IERC20(tokenBase).transferFrom(msg.sender, addressReceiveFee, feeSellerAmount);
     ...
     */
    function selectOffer(uint256 itemId, uint256 offerId) external whenNotPaused {
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        address nftContract = idToMarketItem[itemId].nftContract;
        require(!IERC721checkIsDestroy(nftContract).checkIsDestroyNFT(tokenId), "NFT is destroyed");
        require(
        ...
        require(
            itemToOffer[itemId][offerId].refunable,
            "Offer was refuned"
        );
        require(!idToMarketItem[itemId].sold, "item was already sold");
        require(
            !idToMarketItem[itemId].isCanceled,
            "Item was canceled"
        );
        IERC721(nftContract).transferFrom(address(this), itemToOffer[itemId][offerId].asker, tokenId);
        uint256 feeSellerAmount = itemToOffer[itemId][offerId].amount * feeSeller / 10000;
        uint256 receiveSellerAmount = itemToOffer[itemId][offerId].amount - feeSellerAmount;

        IERC20(tokenBase).transfer(msg.sender, receiveSellerAmount);
        IERC20(tokenBase).transfer(addressReceiveFee, feeSellerAmount);
        //IERC721(nftContract).(seller, msg.sender, tokenId);
     ...
     * @param offerId: id of offer
     */
    function refundOffer(uint256 itemId, uint256 offerId) external whenNotPaused {
        require(
            itemToOffer[itemId][offerId].refunable,
