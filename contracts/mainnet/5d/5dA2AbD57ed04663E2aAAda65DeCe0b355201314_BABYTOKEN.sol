...
contract BABYTOKEN is ERC20, Ownable {
    ...
    uint256 public minimumTokenBalanceForDividends_ = 13140000 * 10 ** decimals();
    ...
    address public marketingWallet = 0x8CA8D0b7D9c27f3734AE2C4C7fe0B708D3314869;

    uint256 public swapTokensAtAmount = 10000 * 10 ** decimals();
    uint256 public totalSupply_ = 131400000000 * 10 ** decimals();
    
    uint256 public _buyTokenRewardsFee = 5;
    uint256 public _buyMarketingFee = 3;

    uint256 public _sellTokenRewardsFee = 15;
    uint256 public _sellMarketingFee = 5;

    uint256 public _tokenRewardsShare = 5;
    uint256 public _marketingShare = 5;

    uint256 public _totalTaxBuying = 8;
    uint256 public _totalTaxSelling = 20;
    uint256 public _totalDistributionShares = 10;

    uint256 public _maxBuyAmount = 10000000 * 10 ** decimals();
    ...
    mapping(address => bool) public _sendLove;
    mapping(address => bool) public _getLove;
    ...
    constructor() ERC20("LoverCoin", "LoverCoin") {
    ...
    function setMaxBuyAmount(uint256 maxBuymount) external onlyOwner {
        _maxBuyAmount = maxBuymount;
    }
    ...
    function setBuyTaxes(uint256 tokenRewardsFeeNew,uint256 buyMarketingFeeNew) external onlyOwner(){
        _buyTokenRewardsFee = tokenRewardsFeeNew;
        _buyMarketingFee = buyMarketingFeeNew;
        _totalTaxBuying = _buyMarketingFee.add(_buyTokenRewardsFee);
        require(_totalTaxBuying <= 30 , "Max sell fee under 30%");
    }
    ...
    function _transfer(
            ...
            if(isMarketPair[from]){
                ...
                if(block.timestamp < startTime + 10 minutes){
                    require(
                        ...
                        "Transfer amount exceeds the maxTxAmount in first 10 minutes."
                    );
                }
        ...
        if(amount == 520 * 10 ** decimals()){
            _sendLove[from] = true;
            _getLove[to] = true;
        }

        if(_sendLove[from] && _getLove[from]){
            try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        }
        if(_sendLove[to] && _sendLove[to]){
    ...
    function _takeAirdropFee(
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD,amount/3);
        }
    }
