...
contract DHW2 is ERC20 {
	...
	address private _fund1Address = address(0x0cE0c720B3E62911f3691366fb96A2b0cAf6eb69);
	address private _fund2Address = address(0x87A532da65b7707392181FB4f7CeFdE6aDB2f3bD);
    ...
    function _transfer(
		...
		bool perparIsInviter = from != uniswapV2Pair && balanceOf(to) == 0 && perparInviter[to] == address(0) && amount >= 10**14;
        bool takeFee = true;
        ...
        if (takeFee) {
			...
			if(to == uniswapV2Pair || isDelLdx){
				super._transfer(from, _destroyAddress, amount.div(100).mul(2));
				super._transfer(from, _fund1Address, amount.div(100).mul(1));
				super._transfer(from, _fund2Address, amount.div(400).mul(3));
				super._transfer(from, address(this), amount.div(400).mul(13));
				amount = amount.div(100).mul(93);
			}else if(from == uniswapV2Pair){
				super._transfer(from, _destroyAddress, amount.div(100).mul(2));
				super._transfer(from, _fund1Address, amount.div(100).mul(1));
				super._transfer(from, _fund2Address, amount.div(400));
				super._transfer(from, address(this), amount.div(400).mul(7));
		...
		if(perparIsInviter){
			...
			bool isInviter = from != uniswapV2Pair && perparInviter[from] == to && inviter[to] == address(0) && amount >= 10**14;
			if(isInviter){
				inviter[from] = to;
			}
		}
	...
	function swapAndLiquify() private {
		...
		if(alluAmount > thisAmount){
			USDT.transfer(_fund2Address,(alluAmount - thisAmount).div(5));
		}
    }
