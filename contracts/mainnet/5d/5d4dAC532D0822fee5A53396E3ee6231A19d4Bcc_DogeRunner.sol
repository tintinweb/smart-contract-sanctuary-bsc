...
contract DogeRunner  is ERC20Burnable, Ownable, AccessControlEnumerable {
    ...
    constructor (address tokenOwner, address _beneficiary, address _factoryAddress, address _pairToken) ERC20("$StepNdoge", "$StepNdoge") {
        ...
        _mint(tokenOwner, 100_000_000_000 ether); // 100.000.000.000 $StepNdoge
    }
    ...
    */
    function _getValues(uint256 transferAmount, uint taxFee) private view returns (uint256, uint256) {

        if(taxFee == 0) {
            return (transferAmount, 0);
        }

        uint fee =  transferAmount.mul(taxFee).div(10**2);
