...
abstract contract LPDividendFee is Owned, DexBase, ERC20 {
	...
	function swapAndToDividend() internal lockTheSwap {
        ...
        uint256 balanceBeforeThis = IERC20(DOGE).balanceOf(address(this));
		...
		uint256 balanceNowThis = IERC20(DOGE).balanceOf(address(this));
        IERC20(DOGE).transfer(address(distributor), (balanceNowThis - balanceBeforeThis) * 2 / 3);
	...
	function _takeDividendFee(address sender, uint256 amount) internal returns (uint256) {
		uint256 dividendAmount = amount * (distributefee + 1) / 100;
...
// File contracts/library/MarketFee.sol
...
abstract contract MarketFee is Owned, ERC20 {
	...
	function _takeMarketing(address sender, uint256 amount) internal returns (uint256) {
...
contract LPDividendMarketToken is ExcludedFromFeeList, MarketFee, LPDividendFee { 
  ...
  MarketFee(2, 0xd1929772C64E7B3a83Ee7c15dB62Ce8873C79745)
  LPDividendFee(500 * 1e18, true, 0x55d398326f99059fF775485246999027B3197955, 2)
  {
