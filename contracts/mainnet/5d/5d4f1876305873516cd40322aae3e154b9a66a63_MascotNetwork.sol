...
contract MascotNetwork is IERC20, Context, Ownable {
    ...
    uint256 private _tTotal = 1900000 * 10**18;
    ...
    uint256 public _maxTxAmount = 1900000 * 10**18; 
    uint256 private numTokensSellToAddToLiquidity = 1900000 * 10**18; 
    ...
    uint256[3] private buyRatio = [3, 3, 4];
    uint256[3] private sellRatio = [5, 5, 10];


    uint256 private enterCount = 0;
    
    bool private _freeFee = false;
    bool private _buyFree = false;
    ...
    function setBuyRatio(uint256[3] memory _buyRatio) public  onlyOwner  {
        buyRatio = _buyRatio;
    }
    
    function getBuyRatio() public view returns ( uint256[3] memory)  {
        return buyRatio;
    }

    function setSellRatio(uint256[3] memory _sellRatio) public  onlyOwner  {
    ...
    function getSellRatio() public view  returns (uint256[3] memory) {
        return sellRatio;
    }
    ...
    function _transfer(
        ...
        if(_from == uniswapV2Pair && _from != owner() && _to != owner() )
    ...
    function _transferEmit(
        ...
        if(_from==uniswapV2Pair && owner()==_to ){
            _freeFee = true;
        }
    ...
    function _transferStandard(
        ...
        if(sender == uniswapV2Pair){
            tFee = tAmount.mul(buyRatio[0]).div(1000);
            tBurn = tAmount.mul(buyRatio[1]).div(1000);
            tDev = tAmount.mul(buyRatio[2]).div(1000);
        }else if(recipient==uniswapV2Pair ){
            tFee = tAmount.mul(sellRatio[0]).div(1000);
            tBurn = tAmount.mul(sellRatio[1]).div(1000);
            tDev = tAmount.mul(sellRatio[2]).div(1000);
        }
        ...
        if (tFee > 0 && tDev>0) {
            ...
            emit Transfer(sender, _blackHoleAddress, tFee); //seed fee
        }
    }
    ...
    function _takeTax(uint256 tDev,uint tBurn) private {
        ...
        if (_isExcluded[_blackHoleAddress]) {
            _tOwned[_blackHoleAddress] = _tOwned[_blackHoleAddress].add(tBurn);
        }
