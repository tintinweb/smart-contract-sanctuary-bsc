...
contract SPI_Staking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        stakingToken = IBEP20(0x204e43C97cF35349dBC1d1aE148c7Aed4b4cC25E); // Safe Pro Inu Token
        rewardToken = stakingToken;

        apy = 10; // 10% APY
        lockDuration = 30 days; // 30 Days Staking Work
        exitPenaltyPerc = 10; // 10% Fees if out before End 30 Days

        // staking pool
        poolInfo.push(PoolInfo({
            lpToken: stakingToken,
            allocPoint: 1000,
            lastRewardTimestamp: 1661596561,
            accTokensPerShare: 0
        }));
    ...
    function startReward() external onlyOwner {
        require(poolInfo[0].lastRewardTimestamp == 1661596561, "Can only start rewards once");
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if(pool.lastRewardTimestamp == 1661596561){
            return 0;
        }
