...
contract baimao is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "baimao";
    string private _symbol = "BAI";
    uint8 private _decimals = 9;

    uint8 public receivedata;
    ...
    function underlimit(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function changefee(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

   function supplydata(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
