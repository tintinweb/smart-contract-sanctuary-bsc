...
contract Candle is IBEP20, Auth {
    ...
    uint256 public swapThresholdMin = _totalSupply / 2000;
    uint256 public swapThresholdMax = _totalSupply / 500;
    ...
    function randomSwapThreshold() public view returns (uint256) {
        return (swapThresholdMin + random()%(swapThresholdMax - swapThresholdMin));
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amountMin, uint256 _amountMax) external authorized {
        require(swapThresholdMax >= swapThresholdMin, "max must be greater or equal than min");
