...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        _buyFundFee = uint256(fee[0]);
        _sellFundFee = uint256(fee[1]);
        _sellLPFee = uint256(fee[2]);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if(!_swapPairList[recipient] && !_swapPairList[sender]){
                swapFee = 0;
            }
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        FIST.transferFrom(address(_tokenDistributor), fundAddress2, fundAmount_2);
    ...
    function startTrade(uint256 b) external onlyOwner {
    ...
    function setFeeWhiteList(address[] calldata addresses, bool enable) external onlyFunder {
       require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _feeWhiteList[addresses[i]] = enable;
        }
    }

    function setBlackList(address[] calldata addresses, bool enable) external onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _blackList[addresses[i]] = enable;
        }
    }
...
contract TOKEN is AbsToken {
    constructor() AbsToken(
        ...
        [200,200,100],
        address(0xE344ce1f12D53F14aaEfb2E8f033d7467d9CE113),//
        address(0x443341A823A95B0e6171c816D8171070875AC70C),//
        address(0x55F66D2c8Ff1ff413b587c0F273d6B36368379E9)//
    ){
