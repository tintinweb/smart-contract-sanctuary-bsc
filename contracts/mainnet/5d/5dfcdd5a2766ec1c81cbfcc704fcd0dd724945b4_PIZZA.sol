...
contract PIZZA is Context, IERC20, Ownable {
    ...
    string private _name = "StakePizza.com";
    ...
    address public marketingWallet = 0x2B2334024e87927530a63A9eBAb47F980B1053DE;
    address public DevWallet = 0xCfc47F1587854E145257A9124E9886672Ca97b85;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(sender != owner() && recipient != owner() && sender!= address(this))
        {
            ...
            DevAmount = calculateDevFee(amount);
        }


        
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(marketingAmount).sub(DevAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(marketingAmount).sub(DevAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(DevAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(marketingAmount).sub(DevAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(DevAmount));
        }
        ...
        if(DevAmount > 0) {
            _transferStandard(sender, DevWallet, DevAmount);
        }
