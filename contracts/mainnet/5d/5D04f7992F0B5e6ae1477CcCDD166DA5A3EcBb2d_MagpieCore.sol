    ...
    IMagpieCore
{
    ...
    ) internal returns (uint256[] memory amountOuts) {
        uint256 amountIn = swapArgs.getAmountIn();
    ...
    function swapIn(SwapInArgs calldata args, BridgeType bridgeType)
        external
        payable
        override
        ...
        )
    {
        ...
        address[] memory intermediaries = bridgeType == BridgeType.Wormhole
            ? config.wormholeIntermediaries
            : config.hyphenIntermediaries;

        require(
            intermediaries.includes(args.swapArgs.getToAssetId()) == true,
            "MagpieCore: invalid intermediary"
        );

        if (intermediaries.includes(args.swapArgs.getFromAssetId())) {
        ...
        if (bridgeType == BridgeType.Wormhole) {
            ...
            tokenSequence = IWormhole(config.tokenBridgeAddress)
        ...
        returns (uint256[] memory amountOuts)
    {
        ...
        if (args.encodedVmBridge.length > 0) {
            bytes memory bridgePayload = IWormhole(config.tokenBridgeAddress)
                .completeTransferWithPayload(args.encodedVmBridge, msg.sender);
            payload = bridgePayload.parse();
        } else {
        ...
        address[] memory intermediaries = payload.bridgeType == BridgeType.Wormhole
            ? config.wormholeIntermediaries
            : config.hyphenIntermediaries;

        if (payload.bridgeType == BridgeType.Wormhole) {
            IWormhole(config.tokenBridgeAddress).completeTransferWithPayload(
        ...
        require(
            intermediaries.includes(fromAssetId) == true,
            "MagpieCore: invalid intermediary"
        );
        require(
        ...
        if (intermediaries.includes(toAssetId)) {
...
interface IMagpieCore {
    ...
    function swapIn(SwapInArgs calldata swapArgs, BridgeType bridgeType)
        external
        payable
        returns (
            uint256[] memory amountOuts,
...
library LibBytes {
    ...
    function toBridgeType(bytes memory self, uint256 start)
        internal
        pure
        returns (IMagpieCore.BridgeType)
    {
        require(self.length >= start + 1, "LibBytes: toBridgeType outOfBounds");
        IMagpieCore.BridgeType tempBridgeType;
...
contract MagpieRouter is ReentrancyGuard, Ownable, IMagpieRouter {
    ...
    function _swapRoute(
        ...
        for (uint256 i = 0; i < route.hops.length; i++) {
            ...
            address toAssetId = assets[hop.path[hop.path.length - 1]];
            uint256 beforeSwapBalance = toAssetId.getBalance();
            _swapHop(amountIn, hop, assets, deadline);
            uint256 afterSwapBalance = toAssetId.getBalance();
