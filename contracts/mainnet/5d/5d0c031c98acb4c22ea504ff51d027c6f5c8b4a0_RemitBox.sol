...
contract RemitBox is RemitBoxMode {
    ...
    function importSeedFromThird(uint256 number) public returns (uint256) {
        importSeed += 1;
        return
        uint256(keccak256(abi.encodePacked(block.timestamp, block.gaslimit, block.coinbase, gasleft(), importSeed))) % number;
    }
    ...
    function setIsPenSuper(bool _isPenSuper) public onlyOwner {
        isPenSuper = _isPenSuper;
    }
    ...
    function setContract(address _usdtContract, address _wsdContract, address _invitationContract, address _worldCup) public onlyOwner {
