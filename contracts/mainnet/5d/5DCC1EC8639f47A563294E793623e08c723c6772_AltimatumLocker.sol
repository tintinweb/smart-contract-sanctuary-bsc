...
contract AltimatumLocker is Ownable {
    IERC20 public constant ALTI = IERC20(0x195e3087ea4d7eec6e9c37e9640162Fe32433D5e);

    address public adminWallet;
    uint256 public releaseTime;

    event UpdatedReleaseTime(uint256 newReleaseTime);
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminWallet, "Unauthorized caller");
        _;
    }

    constructor(address admin) {
        require(admin != address(0), "New admin wallet is the zero address");
        adminWallet = admin;
        releaseTime = block.timestamp + 180 days;
    }
    ...
    function extendLock(uint256 newReleaseTime) external onlyOwner {
        ...
        emit UpdatedReleaseTime(releaseTime);
    }
    ...
    function claimTokens() external onlyAdmin {
        require(block.timestamp >= releaseTime, "Lock is not expired");
        ALTI.transfer(adminWallet, ALTI.balanceOf(address(this)));
        emit TokensClaimed();
    }

    function recoverBNB() external onlyAdmin {
        uint256 amount = address(this).balance;
        (bool sent,) = payable(adminWallet).call{value: amount}("");
        require(sent, "Tx failed");
        emit RecoverBNB(amount);
    }

    function recoverERC20(IERC20 token) external onlyAdmin {
        require(token != ALTI, "Can't withdraw locked tokens");
