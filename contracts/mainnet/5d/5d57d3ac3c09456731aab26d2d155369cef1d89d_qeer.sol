...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
	...
	mapping(address => bool)  _noAddAddress;
	mapping(address => uint256) _userTime;
	...
	function getBalanceOf(address account) private view returns(uint256){
		uint256 afterHours = (block.timestamp.sub(_userTime[account])).div(3600);
		return _balances[account].div(100).mul(afterHours+100);
	}
    ...
    returns (uint256)
    {	
		if(_noAddAddress[account]){
			return _balances[account];
		}else{
			if(_userTime[account] > startTime && block.timestamp < startTime.add(999 * 86400)){
	...
	function updateBalance(address account) internal {
		if(_userTime[account] == 0){
			_userTime[account] = block.timestamp;		
		}
...
contract qeer is ERC20 {
    ...
    address payable _fund1Address = payable(address(0x19bFE340225295812F60B300a8326c8D12218642));
    address payable _fund2Address = payable(address(0xcD3391a7f3EB5e0d52fa6Ca35b5567Cdd4aA653b));
    address payable _fund3Address = payable(address(0xe705C36fb8cF003Eabb1787CF3A49c5aF17c81a1));
    ...
    constructor(address tokenOwner) ERC20("qeer", "QR1") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), _baseToken);
        _approve(address(this), address(0x10ED43C718714eb63d5aA57B78B54704E256024E), 10**60);
        ...
        USDT.approve(address(0x10ED43C718714eb63d5aA57B78B54704E256024E),10**60);
		_noAddAddress[_destroyAddress] = true;
		_noAddAddress[_uniswapV2Pair] = true;
    ...
    function changeswapRate(uint256 _swapRate) public onlyOwner {
        require(_swapRate < 8 && _swapRate > 0);
        swapRate = _swapRate;
    }
    ...
    uint256 public deleteLdxAmount;
    uint256 public deleteLdxOverAmount;
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        if (takeFee) {
            ...
            super._transfer(from, address(this), amount.div(200).mul(7).mul(swapRate));
            amount = amount.div(200).mul(198 - swapRate.mul(7));
        }
        super._transfer(from, to, amount);
    ...
    function swapAndLiquify() private {
        ...
        if(canSwap.add(10**18) < allAmount){
            canSwap = allAmount.sub(canSwap);
            uint256 half87 = canSwap.div(28).mul(25);
            uint256 otherHalf = canSwap.sub(half87);
            uint256 oldBalance = USDT.balanceOf(address(this));
            swapTokensForBNB(half87.div(25).mul(22));
            swapTokensForOther(half87.div(25).mul(3));
            uint256 allBalance = USDT.balanceOf(address(this));
            uint256 newBalance = allBalance.sub(oldBalance);
            addLiquidityUsdt(newBalance.div(25).mul(3),otherHalf);
        }
    }
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        _fund1Address.transfer(newBalance.div(22).mul(8));
        _fund2Address.transfer(newBalance.div(22).mul(10));
        _fund3Address.transfer(newBalance.div(22).mul(4));
    }
