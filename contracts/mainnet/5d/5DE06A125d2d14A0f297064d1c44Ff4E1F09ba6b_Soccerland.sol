...
contract Soccerland is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Soccerland";
    string private constant _symbol = "SOLAND";
    ...
    function changeSwapAtAmount(uint256 _swapAtAmount) public onlyOwner {
        swapTokenAtAmount = _swapAtAmount;
    }

    function withdrawERC20Token(address tokenAddress, uint256 amount) public onlyOwner {
        require(tokenAddress != address(this), "Withdrawing your own token isn't allowed");
        IBEP20(tokenAddress).transfer(address(msg.sender), amount);
    }
    ...
    function _transfer(
        ...
        if(launched && block.timestamp > launchEpoch + 120 && !first) {
            buyFee.liquidity = 20;
            buyFee.marketing = 20;
            first = true;
        }
