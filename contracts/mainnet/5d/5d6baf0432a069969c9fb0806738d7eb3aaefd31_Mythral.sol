...
Made by our Tokerr factory bot
...
contract Mythral is IERC20, Auth {
    ...
    constructor (uint[] memory numbers, address[] memory addresses, string[] memory names, 
        ...
        require(totalBuyFee <= 150, "Buy tax too high!"); //25% buy tax
        ...
        require(totalSellFee <= 150, "Sell tax too high!"); //25% sell tax
    ...
    //Nothing in this contract manually changes lock time, it is set at creation, and re-upped at launch
    function withdrawLPAfterLockTime() external onlyOwner {
    ...
    function setWallets(address _marketingWallet, address _liquidityWallet, 
                        address _devWallet, address _rewardWallet) external authorized {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        _totalFee = (recipient == pair) ? totalSellFee.sub(rewardSellFee) : totalBuyFee.sub(rewardBuyFee);
        _rewardFee = (recipient == pair) ? rewardSellFee : rewardBuyFee;

        uint256 feeAmount = amount.mul(_totalFee).div(1000);
        uint256 rewardFeeAmount = amount.mul(_rewardFee).div(1000);

        _balances[address(this)] = _balances[address(this)].add(feeAmount);
        _balances[rewardWallet] = _balances[rewardWallet].add(rewardFeeAmount);

        emit Transfer(sender, address(this), feeAmount);
        emit Transfer(sender,rewardWallet,rewardFeeAmount);

        return amount.sub(feeAmount).sub(rewardFeeAmount);
    }
    ...
    function tokenSwap(uint256 _amount) internal swapping {
        ...
        uint256 amountToLiquify = (liquidityFee > 0) ? amount.mul(liquidityFee).div(
                (totalFee.sub(rewardFee))).div(2) : 0;
        ...
        uint256 totalBNBFee = ((liquidityFee > 0) ? totalFee.sub(rewardFee).sub(liquidityFee.div(2))
                         : totalFee.sub(rewardFee));
