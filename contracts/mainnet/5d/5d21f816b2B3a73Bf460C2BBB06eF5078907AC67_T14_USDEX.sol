...
// USDEX
contract T14_USDEX is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // FIRST INITIAL MINT = 600,005 TOKENS
    ...
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 600003 ether; // Presale, pegRegulation & DAO
    ...
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("T14_USDEX", "T14_USDEX") {
    ...
    function setTaxTiersRate(uint8 _index, uint256 _value) public onlyTaxOffice returns (bool) {
        require(_index >= 0, "Index has to be higher than zero");
        ...
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_dexPrice >= taxTiersTwaps[tierId]) {
                    require(taxTiersRates[tierId] < 10000, "Tax cannot be equal or larger than 100%");
    ...
    function setTokenOracle(address _dexOracle) public onlyOperatorOrTaxOffice {
        require(_dexOracle != address(0), "Oracle address cannot be zero address");
        dexOracle = _dexOracle;
    }

    function setTaxOffice(address _taxOffice) public onlyOperatorOrTaxOffice {
        require(_taxOffice != address(0), "Tax office address cannot be zero address");
    ...
    function transferFrom(
        ...
        _approve(sender, _msgSender(), allowance(sender, _msgSender()).sub(amount, "ERC20: Transfer exceeds allowance"));
        return true;
    }
