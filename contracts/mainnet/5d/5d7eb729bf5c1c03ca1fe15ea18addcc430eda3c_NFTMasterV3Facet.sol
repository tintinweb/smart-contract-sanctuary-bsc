...
import "../Vendor.sol";
 ...
 abstract contract  NFTMasterBaseFacet {
...
import "./NFTMasterBaseFacet.sol";

contract NFTMasterV3Facet is NFTMasterBaseFacet{

    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    uint256 constant IMMEDIATELY_OPEN = 1;
        ...
        returns (uint256)
    {
        ...
        require(hasFlags(collection.flags, IMMEDIATELY_OPEN) || collection.soldCount == collection.size, "Not finished");
    ...
    function randomNft(uint256 collectionId_) private view returns (NFT memory) {
    ...
    function drawBoxeAndClaimNFT(uint256 collectionId_) public payable {
        drawBoxes(collectionId_, 1);
        claimNFT(collectionId_, address(_msgSender()));
        OrderInfo[] memory orders = orderListByUserPending(collectionId_, _msgSender());
        orderInfoMap[collectionId_][orders[0].id].claimAt = block.timestamp;
    }
    ...
    function drawBoxes(uint256 collectionId_, uint256 times_) public payable {
        ...
        require(collection.soldCount.add(times_) <= nftsByCollectionId[collectionId_].length, "Not enough left");
        ...
        uint256 canUseWhiteListTimes = whiteListLimit.sub(whiteList.usedCount);
        uint256 whiteListTimes = times_ > canUseWhiteListTimes ? canUseWhiteListTimes : times_;
        uint256 cost = collection.averagePrice.mul(times_.sub(whiteListTimes)).add(whiteListTimes.mul(whiteList.price));
        address(this).delegatecall(abi.encodeWithSignature("addWhiteListUsedCountV2(uint256,address,uint256)", collectionId_, _msgSender(), whiteListTimes));
...
interface ICollectionWhiteList
{
    ...
    function setWhiteListV2(uint256 collectionId, address[] memory addressArray, uint256[] memory priceArray, uint256[] memory limitArray) external;
    ...
    function addWhiteListUsedCountV2(uint256 collectionId, address addr, uint256 count) external;
