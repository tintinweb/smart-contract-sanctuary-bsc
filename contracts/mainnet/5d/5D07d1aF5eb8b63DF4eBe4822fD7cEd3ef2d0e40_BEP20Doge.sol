...
contract BEP20Doge is IBEP20, Auth {
  ...
  function swapUSDTmarketing() internal {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = BUSD;
    router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    _swapthreshold.mul(marketingfee).div(feeDenominator),
    0,
    path,
    marketingwallet,
    block.timestamp
  ...
  function swapUSDTliquidity() internal {
    address[] memory path = new address[](2);
    path[0] = address(this);
    path[1] = BUSD;
    router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    _swapthreshold.mul(liquidityfee).div(feeDenominator),
    0,
    path,
    liquiditywallet,
    block.timestamp
