...
contract ruby is Context, IERC20, Ownable{
    ...
    function gettimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function forCoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function _getDevFee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function emprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
