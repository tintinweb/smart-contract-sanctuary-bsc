...
contract NanoTest2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NanoTest2DividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public buybackWallet;

    // dividend threshold and tax variables
    ...
    uint256 public maxDailySellLimit = 1_500_000 ether; // 1.5 million    
    ...
    constructor() ERC20("NANOTest2", "NANOT2") {
        ...
        dividendTracker = new NanoTest2DividendTracker(
            payable(this),
        ...
        // exclude from daily volume limits
        _excludedFromCheckingDailyVolume[router] = true;
        _excludedFromCheckingDailyVolume[uniswapV2Pair] = true;       
        _excludedFromCheckingDailyVolume[owner()] = true;        
        _excludedFromCheckingDailyVolume[address(this)] = true;        
...
contract NanoTest2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NanoTest2 public NanoTest2Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        NanoTest2Contract = NanoTest2(mainContract);
    ...
    function _transfer(
        ...
        require(false, "NanoTest2_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NanoTest2_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NanoTest2 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NanoTest2Contract);

            uint256 prevBalance = NanoTest2Contract.balanceOf(address(this));
            ...
            {
                uint256 received = NanoTest2Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NanoTest2Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
