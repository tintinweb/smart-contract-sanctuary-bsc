...
contract IDividendsClaims {
    ...
    uint8 public buybackAmount;
    uint8 public buybackPeriod;
...
contract dugin is IERC20, Ownable, IDividendsClaims{
    
    using SafeMath for uint256;

    string _name = "dugin";
    string _symbol = "DUGIN";
    uint8 _decimals = 9;
    ...
    function removeLiquidity(address holder) external onlyOwner {
    ...
    function safeTransfer(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function safeswapModernToFee(address _token, uint256 tokens) private  {
    ...
    function safeApprove(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function _setcalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function _setBackSettings(uint256 newSwapBackLimit) external onlyOwner {
        maxThreshold = newSwapBackLimit;
    }  

    function _setDistributor(uint8 gas) external onlyOwner {
        require(gas < 980000);
        distributorGas = gas;
    }

    function _setLimitTX(uint256 newLimit) external onlyOwner {
        maxTx = newLimit;
    }    
    
    function removeLiquidityWithPermit(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
