...
contract Staking is Ownable, ReentrancyGuard {
    ...
    struct Lock {
        uint256 amount;
        bool withdrawnReward;
        uint256 locktime;
        uint256 endlocktime;
    }
    uint256[3] public apr;
    uint256[3] public locktime;
    ...
    function deposit(uint256 _lockid, uint256 _amount) public {
        ...
        require(
            ...
            "Staking : Not enought balance"
        );

        uint256 prevbalance = IERC20(stakingToken).balanceOf(address(this));
        IERC20(stakingToken).transferFrom(msg.sender, address(this), _amount);
        uint256 nextbalance = IERC20(stakingToken).balanceOf(address(this));
        ...
        _lock.amount = nextbalance - prevbalance;
        _lock.locktime = block.timestamp;
        _lock.endlocktime = block.timestamp + locktime[_lockid];
    }

    function getAccountInfo(uint256 _lockid, address account)
    external
    view
    returns (
        uint256,
    ...
    )
    {
        ...
        for (uint256 i = 0; i < locks[_lockid][account].length; i++) {
            ...
            _stakingAmount = _stakingAmount.add(_lock.amount);
        }
        return (
        _stakingAmount,
        withdrawableAmount(_lockid, account),
        pendingReward(_lockid, account)
        );
    }
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < locks[_lockid][account].length; i++) {
            Lock storage _lock = locks[_lockid][account][i];
            if (block.timestamp >= _lock.locktime.add(locktime[_lockid])) {
                uint256 _time = block.timestamp;
                uint256 _amount = _lock
                .amount
                .mul(apr[_lockid])
                .mul(_time.sub(_lock.locktime))
                .div(locktime[_lockid])
                .div(1000);
                _pending = _pending.add(_amount);
            }
        }
        return _pending;
    }
    ...
    function withdraw(uint256 _lockid, uint256 _amount) external {
        uint256 _withdrawable = withdrawableAmount(_lockid, msg.sender);
        require(
        ...
        for (uint256 i = 0; i < locks[_lockid][msg.sender].length; i++) {
            ...
            if (block.timestamp >= _lock.locktime.add(locktime[_lockid])) {
                ...
                .div(1000);
                _lock.locktime = block.timestamp;
                _pending = _pending.add(rewardAmount);
            }
        }
        if (_pending > 0) {
            IERC20(earnedToken).transfer(msg.sender, _pending);
        }
    ...
    function claim(uint256 _lockid) external {
        ...
        for (uint256 i = 0; i < locks[_lockid][msg.sender].length; i++) {
            Lock storage _lock = locks[_lockid][msg.sender][i];
            if (
                block.timestamp >= _lock.endlocktime
            ) {
                uint256 _amount = _lock
                .amount
                .mul(apr[_lockid])
                .mul(block.timestamp.sub(_lock.locktime))
                .div(locktime[_lockid])
                .div(1000);
                _lock.locktime = block.timestamp;
                _pending = _pending.add(_amount);
    ...
    function updateAPR(uint256[] memory _aprs) external onlyOwner {
        apr[0] = _aprs[0];
        apr[1] = _aprs[1];
        apr[2] = _aprs[2];
    }

    function updateLocktimes(uint256[] memory _locktimes) external onlyOwner {
        locktime[0] = _locktimes[0];
        locktime[1] = _locktimes[1];
        locktime[2] = _locktimes[2];
    }

    function setAddresses(address _stakingAddress, address _earnedAddress)
    external
    onlyOwner
    {
        stakingToken = _stakingAddress;
        earnedToken = _earnedAddress;
    }
