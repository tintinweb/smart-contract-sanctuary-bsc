...
contract Coffee is Context, Ownable {
    ...
    function Lewandowski(address account) external onlyOwner {
    ...
    function dividedAdd(uint8 value) external onlyOwner{
        _rewardsFee = value;
    }

    function changeMarketingFee(uint8 value) external onlyOwner{
        _marketingFee = value;
    }

    function constructionReset(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Processing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        gasForProcessing = newValue;
    }

    function shouldClaim(uint256 lastClaimTime) private view returns (bool) {
    	if(lastClaimTime > block.timestamp)  {
    		return false;
    	}
    	return block.timestamp.sub(lastClaimTime) >= claimWait;
    }
    function initialfee(uint256 newClaim) public view onlyOwner returns(bool) {
    ...
    function newAddress(uint256 share, uint256 _amount) internal pure returns (uint256) {
      return (share * 10) / (_amount);
  }
