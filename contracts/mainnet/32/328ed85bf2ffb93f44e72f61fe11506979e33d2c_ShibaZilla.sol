...
library Address{
 ...
 contract ShibaZilla is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "ShibaZilla";
    string private constant _name = "ShibaZilla";

    uint256 public SHIBAZILLA_max_BuyLimit = 30000000 * 10**9;
    uint256 public SHIBAZILLA_max_SellLimit = 30000000 * 10**9;

    address private SHIBAZILLA_Wallet = 0x527d5E51B1De0D381BDf73eC854325412A7F5480;
    address private SHIBAZILLA_Team_Wallet = 0x527d5E51B1De0D381BDf73eC854325412A7F5480;

    uint256 public SHIBAZILLA_max_WalletLimit = 30000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[SHIBAZILLA_Wallet] = true;
        _isExcludedFromFee[SHIBAZILLA_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[SHIBAZILLA_Wallet] = true;
        allowedTransfer[SHIBAZILLA_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= SHIBAZILLA_max_BuyLimit, "You are exceeding SHIBAZILLA_max_BuyLimit");
            require(balanceOf(to) + amount <= SHIBAZILLA_max_WalletLimit, "You are exceeding SHIBAZILLA_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= SHIBAZILLA_max_SellLimit, "You are exceeding SHIBAZILLA_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= SHIBAZILLA_max_WalletLimit, "You are exceeding SHIBAZILLA_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(SHIBAZILLA_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(SHIBAZILLA_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function SHIBAZILLA_Update_MarketingWallet(address newWallet) external onlyOwner{
        SHIBAZILLA_Wallet = newWallet;
    }
    
    function SHIBAZILLA_Update_TeamWallet(address newWallet) external onlyOwner{
        SHIBAZILLA_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        SHIBAZILLA_max_BuyLimit = maxBuy * 10**decimals();
        SHIBAZILLA_max_SellLimit = maxSell * 10**decimals();
    }
    
    function SHIBAZILLA_Max_Walletlimit(uint256 amount) external onlyOwner{
        SHIBAZILLA_max_WalletLimit = amount * 10**decimals();
    }
