...
contract AvionCollection is ERC721EnumerableUpgradeable, OwnableUpgradeable, VRFConsumerBaseUpgradable {
    ...
    function setChainLinkFees(uint256 _amount) external onlyOwner {
        chainlink_fees = _amount;
    }
    ...
    function getRandomNumber() internal returns (bytes32 requestId) {
        return requestRandomness(KEY_HASH, chainlink_fees);
    }

    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        uint256 random = randomness % TOTAL_SUPPLY;

        if (random <= 750 && totalNftMinted[3] < 750 ) return mintInternal(3, VRFRequests[requestId]);
        if (random <= 3000 && totalNftMinted[2] < 2250 ) return mintInternal(2, VRFRequests[requestId]);

        mintInternal(1, VRFRequests[requestId]);
    }
    ...
    function _mint(uint256 _amount, uint256 _price) internal {
        ...
        require(LINK.balanceOf(address(this)) >= chainlink_fees * _amount, "AC: not enough LINK");
        distributeTokens(msg.sender, _price * _amount);
 ...
 */
abstract contract VRFConsumerBaseUpgradable is Initializable {
