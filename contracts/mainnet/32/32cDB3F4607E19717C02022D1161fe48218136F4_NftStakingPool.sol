...
contract NftStakingPool is Ownable, ReentrancyGuard {
    ...
    uint256 public toAddAllocPoint;
    ...
    function setUpdatePrice(uint256 _updatePrice,uint256 _toAddAllocPoint) external onlyOwner {
        updatePrice = _updatePrice;
        toAddAllocPoint = _toAddAllocPoint;
    }
    ...
    function applyUpdatePool(uint256 _pid) external payable {
        ...
        totalAllocPoint = totalAllocPoint.add(toAddAllocPoint);
        poolInfo[_pid].poolConfigList.allocPoint = poolInfo[_pid].poolConfigList.allocPoint.add(toAddAllocPoint);
    }
