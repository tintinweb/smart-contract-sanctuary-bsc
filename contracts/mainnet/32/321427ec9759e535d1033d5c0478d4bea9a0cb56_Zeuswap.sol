...
contract Zeuswap is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 liquidityFeeOnBuy, uint256 marketingFeeOnBuy , uint256 insuranceFeeOnBuy, uint256 devFeeOnBuy);
    event UpdateSellFees(uint256 liquidityFeeOnSell, uint256 marketingFeeOnSell , uint256 insuranceFeeOnSell, uint256 devFeeOnSell);
    ...
    constructor () ERC20("Zeuswap", "ZEUS") 
    {   
        ...
        insuranceFeeOnSell = 0;
        devFeeOnSell       = 7;


        _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy + insuranceFeeOnBuy  + devFeeOnBuy;
        _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell + insuranceFeeOnSell + devFeeOnSell;

        walletToWalletTransferFee = 0;

        marketingWallet = 0x2069628b4Fd09A77E3F0226B69d1df3e6493cb82;
        insuranceWallet = 0x2f1503FD7928e2F3639F14758c24f181b0EE8Ab0;
        devWallet       = 0x2a7EA7a0FDE3f5c2e4Bcc6103C26552E87B3B12F;

        _isExcludedFromFees[owner()] = true;
        _isExcludedFromFees[DEAD] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[marketingWallet] = true;
        _isExcludedFromFees[insuranceWallet] = true;
...
//------------------FeeManagement------------------//
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _insuranceFeeOnBuy, uint256 _devFeeOnBuy ) external onlyOwner {
        ...
        require(_totalFeesOnBuy + _totalFeesOnSell <= 25, "Total Fees cannot be more than 25%");

        emit UpdateBuyFees(liquidityFeeOnBuy, marketingFeeOnBuy, insuranceFeeOnBuy, devFeeOnBuy);
    }

    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _marketingFeeOnSell , uint256 _insuranceFeeOnSell, uint256 _devFeeOnSell) external onlyOwner {
        ...
        emit UpdateSellFees(liquidityFeeOnSell, marketingFeeOnSell, insuranceFeeOnSell, devFeeOnSell);
    }

    function updateWalletToWalletTransferFee(uint256 _walletToWalletTransferFee) external onlyOwner{
        require(_walletToWalletTransferFee <= 25, "Wallet to Wallet Transfer Fee cannot be more than 25%");
    ...
    function changeInsuranceWallet(address _insuranceWallet) external onlyOwner{
        require(_insuranceWallet != insuranceWallet,"Insurance wallet is already that address");
        require( _insuranceWallet != address(0),"Insurance wallet cannot be the zero address");
...
//------------------Swap------------------//
    ...
    function swapAndSendFee(uint256 tokenAmount) private {
        ...
        uint256 totalShare = marketingShare + insuranceShare + devShare;
        ...
        if (insuranceShare > 0) {
                uint256 insuranceTokens = (newBalance * insuranceShare) / totalShare;
                payable(insuranceWallet).sendValue(insuranceTokens);
        }

        if (devShare > 0) {
                uint256 devTokens = (newBalance * devShare) / totalShare;
                payable(devWallet).sendValue(devTokens);
        }
