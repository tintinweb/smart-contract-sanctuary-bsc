 ...
 * Implemented and modified by WojakSwap teams.
 */
contract WojakZap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // WojakRouter interface
    IWojakRouter02 public wojakRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from WojakSwap)
    ...
    // Address WojakRouter
    address private wojakRouterAddress;
     ...
     * @param _wojakRouter: address of the WojakRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _wojakRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        wojakRouterAddress = _wojakRouter;
        wojakRouter = IWojakRouter02(_wojakRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. WOJK/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. WOJK) to receive in the intermediary swap (e.g. BNB --> WOJK)
     */
     ...
     * @param _lpToken: LP token address (e.g. WOJK/USDC)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. WOJK) in the intermediary swap (e.g. USDC --> WOJK)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. WOJK/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. WOJK --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. WOJK/USDC)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. WOJK or USDC)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. WOJK) in the intermediary swap (e.g. USDC --> WOJK)
     */
    ...
    {
        address token0 = IWojakPair(_lpToken).token0();
        address token1 = IWojakPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IWojakPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = wojakRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = wojakRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IWojakPair(_lpToken).token0() || _token0ToZap == IWojakPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IWojakPair(_lpToken).token0() || _token1ToZap == IWojakPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IWojakPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IWojakPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IWojakPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = wojakRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = wojakRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IWojakPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "WojakRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "WojakRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        wojakRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), wojakRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(wojakRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(wojakRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = wojakRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = wojakRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = wojakRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = wojakRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = wojakRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
