...
contract TESTShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TESTSHAREs to distribute per block.
        uint256 lastRewardTime; // Last time that TESTSHAREs distribution occurs.
        uint256 accTSharePerShare; // Accumulated TESTSHAREs per share, times 1e18. See below.
    ...
    // The time when TESTSHARE mining starts.
    uint256 public poolStartTime;

    // The time when TESTSHARE mining ends.
    uint256 public poolEndTime;

    uint256 public tSharePerSecond = 0.00382587 ether; // 59500 TESTSHARE / (180 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "TESTShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "TESTShareRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's TESTSHARE allocation point. Can only be called by the owner.
    ...
    // View function to see pending TESTSHAREs on frontend.
    ...
    // Safe TESTSHARE transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (TESTSHARE or lps) if less than 90 days after pool ends
            require(_token != tshare, "TESTshare");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
