...
contract PnftStartPools is
    Ownable,
    ReentrancyGuard,
    ERC721Holder,
    ERC20("POLKAS NFT Pool", "POLKAS-NFT")
{
    using SafeMath for uint256;
    ...
    function deposit(uint256 _nftId, address _nftAddress, uint256 _price, bytes calldata sig) external nonReentrant {
        ...
        emit EventDeposit(msg.sender, _nftId, _nftAddress, block.timestamp);
    }
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        IERC20(rewardToken).safeTransfer(address(msg.sender), _amount);
    }
