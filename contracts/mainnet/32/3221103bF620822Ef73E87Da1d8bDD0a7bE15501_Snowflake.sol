...
contract Snowflake is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 3_000 * 10**9;
    ...
    address public marketingWallet = 0x87D897A3Df73B598fa7D8BDD5a196b5c9BBCF48d;
    address public charityWallet = 0x330ABCCd0d692896Ec5B143Bb62281e76640e325;
    address public teamWallet = 0x5448F855Be7Fa54C663943072C0bC1F77B9e8890;
    ...
    Taxes public taxes = Taxes(0, 4, 1, 2, 1);
    Taxes public sellTaxes = Taxes(0, 16, 4, 3, 2);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _charity, uint256 _team) public onlyOwner {
        taxes = Taxes(_rfi,_marketing,_liquidity,_charity,_team);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _charity, uint256 _team) public onlyOwner {
        sellTaxes = Taxes(_rfi,_marketing,_liquidity,_charity,_team);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rTeam) = _getRValues3(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tCharity-s.tTeam;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rCharity-rTeam;
    ...
    function _getRValues3(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rTeam) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tCharity+ s.tTeam);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.charity + temp.team) * 2;
