...
contract withdrawNFT is Governance {
    ...
    address public _nfttokenAddr;

    mapping(address => bool[]) public _male;
    mapping(address => uint[]) public _ids;
    ...
    constructor (address nfttokenAddr) public {
        _nfttokenAddr = nfttokenAddr;
    }

    function withdraw() public{
        uint lenn = _male[msg.sender].length;
        // uint ccurtime = block.timestamp;
        for(uint i=0;i<lenn;i++)
        {
            nfttoken(_nfttokenAddr).mint(
            msg.sender,
            _male[msg.sender][i],
            0,
            param1[_ids[msg.sender][i]],
            param2[_ids[msg.sender][i]],
            param3[_ids[msg.sender][i]],
            urls[_ids[msg.sender][i]]
            );
        }
        for(uint i=0;i<lenn;i++)
        {
            _male[msg.sender].pop();
            _ids[msg.sender].pop();
        }
    ...
    function setNFTwithdraw(
      ...
      external
      onlyGovernance{
        require(nmale.length == num.length,"err");

        for(uint i=0;i<nmale.length;i++)
        {
            _male[ac].push(nmale[i]);
            _ids[ac].push(num[i]);
        }
        
    }

    function setNFTAddr(address nftaddr)
     external onlyGovernance
    {
        _nfttokenAddr = nftaddr;
    }
