...
contract Stones is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "Stones";
    string _symbol = "STONE";
    uint8 _decimals = 9;
    ...
    function Clemence(address account) external onlyOwner {
    ...
    function chargefree(uint256 lastClaimTime) private view returns (bool) {
    ...
    function outLimit(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
      return share * (dividendsPerShare) / (3600);
    }

    function increasetoken(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
