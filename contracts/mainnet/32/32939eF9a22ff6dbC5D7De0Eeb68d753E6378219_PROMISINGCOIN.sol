...
// Promising Coin is an innovative cryptocurrency that will correlate the physical world with the digital world (crypto) 
// bringing shares in crypto assets, thus, we will facilitate the dream of many people of having shares in physical companies. 
// Noting that the profits will be distributed after the first month of opening of the 1st store, which will be financed by the group 
// of VIP holders.

//  🌐 http://Promisingpsc.online
...
contract PROMISINGCOIN is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "PromisingCoin";
    ...
    constructor () Auth(msg.sender) {
        ...
        address _inicial = 0xF7A91B66d57c376F5525cCF411FaA1aaff7562Ef; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        //max wallet code
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != projectFeeReceiver && recipient != developmentReceiver){
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingdevelopmentFee, uint256 _projectFee, uint256 _burnFee, uint256 _extraFeeOnSell, uint256 _feeDenominator) external authorized {
        ...
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingdevelopmentFee).add(_projectFee);
        extraFeeOnSell = _extraFeeOnSell;
        feeDenominator = _feeDenominator;
    }
    
    function setFeeReceivers(address _developmentReceiver, address _marketingFeeReceiver, address _projectFeeReceiver) external authorized {
    ...
    function claimDividendFromOthers(address shareholder) external {
