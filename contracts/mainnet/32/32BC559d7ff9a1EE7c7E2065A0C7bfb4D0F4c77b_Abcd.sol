...
contract Abcd is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x9Ec3c06733833fE84Be05995795becFE3978Fb66);
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address private _fist = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A; 
    ...
    function transferToAddressToken(address payable recipient, uint256 amount) private {
        recipient.transfer(amount);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        swapTokenForToken(tAmount);
         ...
         uint256 tokenval = IERC20(_fist).balanceOf(address(this));
        if (tokenval > 0) {
            IERC20(_fist).transfer(marketingWalletAddress, tokenval);
            return;
        }
        
        // uint256 amountReceived = address(this).balance;
        // uint256 amountReceived = IERC20(_fist).balanceOf(address(this));
        // if(amountReceived > 0)
    ...
    function swapTokenForToken(uint256 tokenAmount) private {
        ...
        // uint256 amount = IERC20(_fist).balanceOf(address(this));
        // if (IERC20(_fist)._allowances(marketingWalletAddress, address(this)) >= amount) {
        //     IERC20(_fist).transferFrom(marketingWalletAddress, address(this), amount);
        // }
