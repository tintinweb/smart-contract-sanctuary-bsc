...
contract OrionGovernance is IOrionGovernance, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function rewardPerToken() public view override returns (uint256) {
        ...
        return
            rewardPerTokenStored.add(
                lastTimeRewardApplicable().sub(lastUpdateTime).mul(rewardRate).mul(1e18).div(uint256(total_balance_))
            );
    }

    function earned(address account) public view override returns (uint256) {
        return
            uint256(balances_[account].balance)
    ...
    //  Stake
    function stake(uint56 adding_amount) public override nonReentrant updateReward(msg.sender) {
        require(adding_amount > 0, 'CNS0');
        staking_token_.safeTransferFrom(msg.sender, address(this), adding_amount);
        _stake(adding_amount);
    }

    function _stake(uint56 adding_amount) internal {
    ...
    // Unstake
    function withdraw(uint56 removing_amount) public override nonReentrant updateReward(msg.sender) {
        ...
        vaults_[msg.sender].push(UserVault({amount: removing_amount, created_time: uint64(block.timestamp)}));

        emit Withdrawn(msg.sender, uint256(removing_amount));
    }

    function vaultStake(uint256 index) public updateReward(msg.sender) {
        UserVault memory vault = vaults_[msg.sender][index];

        uint256 len = vaults_[msg.sender].length;
        if (index < len - 1) vaults_[msg.sender][index] = vaults_[msg.sender][len - 1];
    ...
    function vaultWithdraw(uint256 index) public nonReentrant {
        //  ? Do we need extra check there
        UserVault memory vault = vaults_[msg.sender][index];
        uint256 fee_percent = basic_fee_percent;
        ...
        uint256 vault_amount = uint256(vault.amount);
        uint256 fee_orn = vault_amount.mul(fee_percent).div(1000);
    ...
    function setVaultParameters(
        uint16 extra_fee_percent_,
        uint64 extra_fee_seconds_,
        uint16 basic_fee_percent_
    ) external onlyOwner {
    ...
    function acceptNewLockAmount(address user, uint56 new_lock_amount) external override onlyVotingContract {
    ...
    function acceptLock(address user, uint56 lock_increase_amount) external override onlyVotingContract {
    ...
    function acceptUnlock(address user, uint56 lock_decrease_amount) external override onlyVotingContract {
    ...
    function getTotalLockedBalance(address user) public view returns (uint56) {
        ...
        if (voteBurnAvailable()) {
            ...
            if (burn_vote_balance > locked_balance) locked_balance = burn_vote_balance;
        }
    ...
    function notifyRewardAmount(uint256 reward, uint256 _rewardsDuration) external onlyOwner updateReward(address(0)) {
        ...
        uint256 balance = staking_token_.balanceOf(address(this)); //  TODO: review
    ...
    function voteBurn(uint56 voting_amount, bool vote_for_burn) public {
        ...
        if (vote_for_burn) total_votes_burn_ += voting_amount;
        else total_votes_dont_burn_ += voting_amount;
    }
