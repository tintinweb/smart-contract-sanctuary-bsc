...
contract KinectVault {
    ...
    //Check what rewards haven't been added yet
    function pendingRewards(address _user) public view returns(uint) {
        uint deposited = Stake[_user];
        uint contractStake = contractAddress.balanceOf(address(tx.origin));
        uint exchangeRate = contractAddress.exchangeRateStored();
        uint contractPendingRewards = (contractStake.mul(exchangeRate)).div(10**18);
        uint userRewards = deposited.mul(10**18).mul(Total.mul(10**18)).mul(contractPendingRewards).div(10*18);
        return userRewards;
    }
