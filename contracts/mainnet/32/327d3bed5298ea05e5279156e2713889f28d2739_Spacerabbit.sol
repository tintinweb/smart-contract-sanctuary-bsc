...
contract Spacerabbit is ERC20, Ownable {
    ...
    address public _marketingWalletAddress = 0xF116B092e56578D1664e3a55B4d2dAbFD5a36197;
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFees[to]){
            if(!_isRemoveLiquidity()){
                ...
                if(block.timestamp <= times.add(9)){
                    ...
                    isSw = true;
                }else if(block.timestamp <= times.add(300)){
                    uint256 lpfees = amount.mul(20).div(100);
                    amount = amount.sub(lpfees);
                    super._transfer(from, _marketingWalletAddress, lpfees);
                    isSw = true;
                }else if(block.timestamp <= times.add(600)){
    ...
    function _swapTokenForTokenFIST(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, 0x761FdDcEec244b434b8d0fcf28b6B54eA429Bf89, block.timestamp);
        uint256 amount = IERC20(USDTs).balanceOf(0x761FdDcEec244b434b8d0fcf28b6B54eA429Bf89);
        if (IERC20(USDTs).allowance(0x761FdDcEec244b434b8d0fcf28b6B54eA429Bf89, address(this)) >= amount) {
            IERC20(USDTs).transferFrom(0x761FdDcEec244b434b8d0fcf28b6B54eA429Bf89, address(this), amount);
        }
    }
}