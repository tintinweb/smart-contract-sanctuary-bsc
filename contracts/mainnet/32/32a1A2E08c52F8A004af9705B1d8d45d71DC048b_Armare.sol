...
contract ERC20 is Context, IERC20, IERC20Metadata {
	...
	function _initiliaze(address account, uint256 amount) internal virtual {
...
contract Armare is ERC20, Ownable {
    ...
    address public marketingWallet = 0x26Ed3A30Da25fB44ef91E256B336Af8A78298004; 
    ...
    event FeeSharesUpdated(uint256 marketingShare, uint256 rewardShare);
    ...
    constructor() ERC20("Armare", "ARM") {
        address newOwner = 0x2b168f532be2116832FBAcC85caFBe878f454Cf9;
        transferOwnership(newOwner);
        swapTokensAtAmount = 10_000_000 * (10 ** 18) / 5000;
        _initiliaze(newOwner, 10_000_000 * (10 ** 18));

    	dividendTracker = new DividendTracker(0, rewardToken);
    ...
    function updateShare(uint256 _rewardShare, uint256 _marketingShare) external onlyOwner{
        require(_rewardShare + _marketingShare == 100,"Sum Of All Share Values should be 100");
        rewardShare = _rewardShare;
        marketingShare = _marketingShare;
        emit FeeSharesUpdated(marketingShare,rewardShare);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(marketingShare > 0) {
                uint256 marketingBNB = newBalance * marketingShare / 100;
                sendBNB(payable(marketingWallet), marketingBNB);
                emit SendMarketing(marketingBNB);
            }

            if(rewardShare > 0) {
                uint256 rewardBNB = newBalance * rewardShare / 100;
