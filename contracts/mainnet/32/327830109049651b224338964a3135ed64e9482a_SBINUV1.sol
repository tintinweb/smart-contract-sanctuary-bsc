...
contract SBINUV1 is Context, IERC20, Ownable, LockToken 
{
    ...
    mapping(address => bool)  _blacklist; //blacklist function
    ...
    string private _name = "GOAT WOLF";
    string private _symbol = "GOATWOLF";
	...
	uint256 public totalBuyFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_CharityFee); //12% Total Buy Fee
    ...
    uint256 public _saleCharityFee = 4;

    uint256 public totalSaleFee = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee); //12% Total Sale Fee
    

    address payable public marketingWallet =  payable(0x24140e7F41cff0442e40a35cd30C605F5CfA8D93); //Define Marketing Wallet
	address payable public CharityWallet =  payable(0x419c796a29d7C5D4D333dd605a5228b4F56e3040); //Define Charity Wallet
    ...
    uint256 public _maxBuyAmount = 5_000_000_0 * 10**9; //5%
    ...
    function setAllBuyFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 CharityFee) public onlyOwner() 
    {
        ...
        uint256 totalBuyFees = _taxFee.add(_liquidityFee).add(_marketingFee).add(_CharityFee);
    ...
    function setAllSaleFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 CharityFee) public onlyOwner() 
    {
        ...
        uint256 totalSaleFees = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleCharityFee);
    ...
    {  
        return _amount.mul(_liquidityFee.add(_marketingFee).add(_CharityFee)).div(100);
    }
    ...
    open(from, to) 
    {
        ...
        require (!isBlackListed(to), "Token transfer refused. Receiver is on blacklist"); //Blacklist
    ...
    {
        uint256 allFee = _liquidityFee.add(_marketingFee).add(_CharityFee);
        ...
        CharityWallet.transfer(newBalance.div(allFee).mul(_CharityFee)); //Charity Fee Distribution
    }
