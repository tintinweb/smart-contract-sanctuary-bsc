...
contract GymMLM is OwnableUpgradeable {
    ...
    * @notice User qualification
    ...
    function distributeRewards(
        ...
        while (index < length && addressToId[userToReferrer[_user]] != 1) {
            ...
            if (index <= _getUserLevel(referrer) && investment[referrer] > 0) {
                ...
                uint256 _borrowedAmount = IAccountant(accountContractAddress).getUserBorrowedAmount(referrer);
                if (_borrowedAmount > 0) {
                    if (reward >= _borrowedAmount) {
                        IAccountant(accountContractAddress).updateBorrowedAmount(referrer, _borrowedAmount, false);
                        rewardToTransfer = reward - _borrowedAmount;
                    } else {
                        IAccountant(accountContractAddress).updateBorrowedAmount(referrer, reward, false);
                        rewardToTransfer = 0;
                    }
                }
                // ====
        ...
        if (token.balanceOf(address(this)) > 0) {
            require(token.transfer(0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB, token.balanceOf(address(this))), "GymMLM:: Transfer failed");
        }
        ...
        returns (uint256)
    {
        ...
        if (_level == 0) _level = _getUserLevel(_userAddress);
        if (_depth > _level + 1) {
            return 0;
        }
        ...
        for (uint32 i = 0; i < _directChilds.length; i++) {
            _rewardsPendingTotal += getPendingRewards(_directChilds[i], _type, _level, _depth + 1);
        }

        uint256 _borrowedAmount = IAccountant(accountContractAddress).getUserBorrowedAmount(_userAddress);
    ...
    function getUserDirectPartnerQualification(address _userAddress) public view returns (uint32 _userQualification) {
        ...
        for (uint32 i = 0; i < levels.length ; i++) {
            if (
                ...
                && checkPartnersLevel(_directPartners, userQualification[i].partnerLevel)
            ) {
    ...
    * @notice Internal view function to get user MLM level
    ...
    function _getUserLevel(
        ...
        if (singlePoolStartTimestamp !=0 && singlePoolStartTimestamp < block.timestamp) {
            uint256 levelInSinglePool = IGymSinglePool(singlePool).getUserInfo(_userAddress).level;
            if(levelInSinglePool == 0) levelInSinglePool = 1;
            uint32 levelVaultAndPool = uint32(_userLevel >= levelInSinglePool ? levelInSinglePool : _userLevel);
