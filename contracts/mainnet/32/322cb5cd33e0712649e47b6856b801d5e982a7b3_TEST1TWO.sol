...
contract TEST1TWO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    TEST1TWODividendTracker public dividendTracker;
    ...
    constructor() ERC20("TEST1TWO", "TESTONE2") {
        ...
        dividendTracker = new TEST1TWODividendTracker(payable(this), router, 0x0000000000000000000000000000000000000000,
                                                   "TEST1TWOTRACKER", "TESTONE2TRACKER");
        ...
        uint256 totalTokenSupply = (877_000) * (10**18);
        _mint(owner(), totalTokenSupply); // ONLY time internal mint function is ever called is to create INITIAL supply
        swapTokensAtAmount = totalTokenSupply / 500; // 0.05%;
    ...
    function setmaxTX(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 26000, "max tx cannot be set to less than 0.1%");
        maxTX = value;
    }
    ...
    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 1, "transfer fee cannot exceed 5%");
    ...
    function updateFees(uint256 deadBuy, uint256 deadSell, uint256 marketingBuy, uint256 marketingSell,
        ...
        require(totalSellFees <= 10 && totalBuyFees <= 12, "total fees cannot exceed 16% sell or buy");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            uint256 TESTONE2okens;

            if (deadFees > 0) {
            TESTONE2okens = amount.mul(deadFees) / 100;
            super._transfer(from, DEAD, TESTONE2okens);
            _totalSupply = _totalSupply.sub(TESTONE2okens);

            }

            amount = amount.sub(fees + TESTONE2okens);
...
contract TEST1TWODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address public defaultToken; // BNB
    ...
    TEST1TWO public TEST1TWOContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        TEST1TWOContract = TEST1TWO(mainContract);
        minimumTokenBalanceForAutoDividends = 500_000000000000000000; // 500 tokens
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main TEST1TWO contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TEST1TWOContract);

            uint256 prevBalance = TEST1TWOContract.balanceOf(address(this));
            ...
            {
                uint256 received = TEST1TWOContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TEST1TWOContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
