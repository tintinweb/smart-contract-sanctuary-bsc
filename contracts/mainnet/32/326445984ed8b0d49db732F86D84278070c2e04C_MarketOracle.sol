...
/**
 * @title Peace price Oracle
 *      This Oracle calculates the average USD price of Peace based on the BNB-Peace and USDC-BNB pools.
...
contract MarketOracle is Ownable {
    ...
    uint private PeaceBnbPrice0CumulativeLast;
    uint private PeaceBnbPrice1CumulativeLast;
    uint32 private PeaceBnbBlockTimestampLast;
    ...
    IUniswapV2Pair private _Peace_bnb;
    ...
    constructor(
        address __Peace_bnb,   // Address of the Peace-BNB pair on Pancakeswap
        address __wbnb_busd   // Address of the WBNB-BUSD on Pancakeswapx
        ) public {

        controller = msg.sender;

        _Peace_bnb = IUniswapV2Pair(__Peace_bnb);
        _wbnb_busd = IUniswapV2Pair(__wbnb_busd);

        uint112 _dummy1;
        uint112 _dummy2;

        PeaceBnbPrice0CumulativeLast = _Peace_bnb.price0CumulativeLast();
        PeaceBnbPrice1CumulativeLast = _Peace_bnb.price1CumulativeLast();

        (_dummy1, _dummy2, PeaceBnbBlockTimestampLast) = _Peace_bnb.getReserves();
    ...
    // Get the average price of 1 Peace in the smallest BNB unit (18 decimals)
    function getPeaceBnbRate() public view returns (uint256, uint256, uint32, uint256) {
        (uint price0Cumulative, uint price1Cumulative, uint32 _blockTimestamp) =
            UniswapV2OracleLibrary.currentCumulativePrices(address(_Peace_bnb));

        require(_blockTimestamp != PeaceBnbBlockTimestampLast, "Peace Last and current are equal");

        FixedPoint.uq112x112 memory PeaceBnbAverage = FixedPoint.uq112x112(uint224(1e9 * (price0Cumulative - PeaceBnbPrice0CumulativeLast) / (_blockTimestamp - PeaceBnbBlockTimestampLast)));

        return (price0Cumulative, price1Cumulative, _blockTimestamp, PeaceBnbAverage.mul(1).decode144());
    }
   ...
   function update() external onlyControllerOrOwner {
        ...
        uint PeaceBnbAverage;
        uint busdBnbAverage;

        (PeaceBnbPrice0CumulativeLast, PeaceBnbPrice1CumulativeLast, PeaceBnbBlockTimestampLast, PeaceBnbAverage) = getPeaceBnbRate();
    ...
    function getData() external view returns (uint256) {
        ...
        (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, PeaceBnbAverage) = getPeaceBnbRate();

        uint busdBnbAverage;

         (_price0CumulativeLast, _price1CumulativeLast, _blockTimestampLast, busdBnbAverage) = getBusdBnbRate();

        uint answer = PeaceBnbAverage*(1e6) / busdBnbAverage;
