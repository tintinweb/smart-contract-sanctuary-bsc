...
abstract contract ContextPlatform {
...
contract OwnablePlatform is ContextPlatform {
    address private _owner;

    event OwnerPlatformpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerPlatformpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnablePlatform: caller is not the owner");
        _;
    }

 
    function renounceOwnerPlatformp() public virtual onlyOwner {
        emit OwnerPlatformpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerPlatformp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerPlatformp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnablePlatform: new owner is the zero address");
        emit OwnerPlatformpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract OdinPlatform is ContextPlatform, BinanceBNB, OwnablePlatform {
    ...
    string private _name = 'Odin Platform';
    string private _symbol = 'Odin Platform';
