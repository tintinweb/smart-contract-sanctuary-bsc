...
contract XXX  is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'XXX Token';
    string private constant _symbol = 'XXXXX';
    uint8 private constant _decimals = 18;
    uint256 private _totalSupply = 11 * 10**6 * (10 ** _decimals);
    ...
    constructor() Auth(msg.sender) {
        IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //0x10ED43C718714eb63d5aA57B78B54704E256024E - MAINNET  «»  0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 TESTNET
    ...
    function setMaxes(uint256 _transaction, uint256 _wallet) external authorized {               
        _maxTxAmount = _transaction;
        _maxWalletToken = _wallet;        
        require(_transaction >= _totalSupply.mul(5).div(1000) && _wallet >= _totalSupply.mul(5).div(1000), "Max TX and Max Wallet cannot be less than 0.5%");
    }
    ...
    function setStructure(uint256 _liq, uint256 _mark, uint256 _stak, uint256 _burn, uint256 _tran) external authorized {
        ...
        burnFee = _burn;
        transferFee = _tran;
        totalFee = liquidityFee.add(marketingFee).add(stakingFee).add(burnFee).add(transferFee);
        require(totalFee <= feeDenominator, "Tax cannot be more than 25%");
    }

    function setMaxTAX (uint256 MaxTAX) external authorized{
        feeDenominator = MaxTAX;

        totalFee = liquidityFee.add(marketingFee).add(stakingFee).add(burnFee).add(transferFee);

        require(totalFee <= MaxTAX, "Check setStructure Tax !");
        require (MaxTAX <= 2500 , "Tax cannot be more than 25%");
    }
