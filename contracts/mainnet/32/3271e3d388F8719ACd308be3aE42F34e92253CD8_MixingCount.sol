...
contract MixingCount is VRFConsumerBase, Ownable {
    ...
    function getLogsLimit(uint256 limit, uint256 offset)
        public
        view
        returns (Log[] memory)
    {
        Log[] memory logs_ = new Log[](limit);

        for (uint256 i = 0; i < limit; i++) {
            if (i + offset >= logs.length) break;
            logs_[i] = logs[i + offset];
        }
