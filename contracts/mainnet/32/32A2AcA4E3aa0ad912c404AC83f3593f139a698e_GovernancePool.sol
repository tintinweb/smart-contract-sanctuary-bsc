...
contract GovernancePool is Ownable {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare; // Accumulated native tokens per share, times 1e12. See below.
        uint256 lockupDuration;  // Lockup Duration
        uint256 withdrawalFeeBP;  // Withdrawal fee in base points (/ 10.000)
    }

    // The native TOKEN!
    IBEP20 public native;
    ...
    // The block number when native mining ends.
    ...
    // Withdraw native tokens from STAKING.
    function withdraw(uint256 _amount) public {
        ...
        // Update pool variables up to date
        updatePool(0);

        // Sum pending amount to user amount
        uint256 pending = user.amount.mul(pool.accTokenPerShare).div(1e12).sub(user.rewardDebt);
        user.amount = user.amount.add(pending);
        lpSupply = lpSupply.add(pending);
    ...
    function emergencyRewardWithdraw(uint256 _amount) public onlyOwner {
        require(_amount < native.balanceOf(address(this))-lpSupply, 'not enough token');
        native.safeTransfer(address(msg.sender), _amount);
    }
    ...
    function setWithdrawalFeeBP(uint256 _withdrawalFeeBP) public onlyOwner {
        require(_withdrawalFeeBP <= 3000, "updateWithdrawalFeeBP: invalid withdrawal fee basis points");
        PoolInfo storage pool = poolInfo[0];
