...
contract SousChef {
    ...
    // Info of each user that stakes Envoys tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = evb.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 evbSupply = evb.balanceOf(address(this));
        if (evbSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(evbSupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit Envoys tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        evb.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw Envoys tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        evb.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        evb.safeTransfer(address(msg.sender), user.amount);
