...
contract LUNC30 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "LUNC30";
    string private _symbol = "LUNC30";
    ...
    function setMarketing(uint256 newMarketingFee) external onlyOwner() {
	...
	function setInsights(uint256 newValue) public onlyOwner {
        Insights = newValue;
    }
	function setRouterver(uint256 router) public onlyOwner {
        routerver = router;
    }
    ...
    function takeFee(address sender,address recipient,uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender] || isMarketPair[recipient]) {
            feeAmount = amount.mul(_total).div(1000);
        }
