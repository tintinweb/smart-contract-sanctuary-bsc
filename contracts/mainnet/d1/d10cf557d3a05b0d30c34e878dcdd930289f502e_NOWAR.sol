...
pragma solidity ^0.8.5;
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also NOWARed.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all NOWAR just
	...
	contract NOWAR is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address public marketingFeeReceiver = 0x39F31d71d19577D23E348659Eb0Fd41C835CF3a3;
		address public charityFeeReceiver = 0x39F31d71d19577D23E348659Eb0Fd41C835CF3a3;
		...
		event excludeMultipleAccountFromRewards(address[] NOWAR, bool isExcluded);
		...
		constructor() ERC20("WORLD WITHOUT WAR", "NOWAR"){
		...
		function setIsFeeExempt(address account, bool exempt) public onlyOwner {
			require(_isFeeExempt[account] != exempt, "NOWAR: Account is already the value of 'exempt'");
			_isFeeExempt[account] = exempt;
		}
        
        function setIsTxLimitExempt(address account, bool exempt) public onlyOwner {
			require(_isTxLimitExempt[account] != exempt, "NOWAR: Account is already the value of 'exempt'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "NOWAR: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "NOWAR: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != marketingFeeReceiver, "NOWAR: The liquidity wallet is already this address");
			emit SetIsFeeExempt(newLiquidityWallet, true);
			emit LiquidityWalletUpdated(newLiquidityWallet, marketingFeeReceiver);
			marketingFeeReceiver = newLiquidityWallet;
		}
   
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "NOWAR: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "NOWAR: Cannot update distributorGas to same value");
		...
		function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee) public onlyOwner {
		...
		function setFeeReceiver(address _marketingFeeReceiver, address _charityFeeReceiver)  public onlyOwner {
		...
		function _transfer(
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(marketingFee);
