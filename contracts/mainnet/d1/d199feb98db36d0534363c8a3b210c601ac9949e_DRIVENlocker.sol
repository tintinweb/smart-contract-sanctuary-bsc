...
// File: contracts/DRIVENlock.sol

// DRIVENlock (Version 1.0.0) - Built by DRIVENecosystem Team on BSC
...
contract DRIVENlocker is Ownable, ReentrancyGuard {
    ...
    // Variables
    uint256 public bnbFee = 500000000000000000;
    uint256 public bnbSubFee = 100000000000000000;
    uint256 public renewFee = 150000000000000000;
    ...
    // Lock tokens
    function lockTokens(address _tokenAddress, uint256 _amount, uint256 _unlockTime) external payable callerIsUser nonReentrant{

        require(msg.value >= bnbFee, "BRISE fee not provided");

        _lockTokens(_tokenAddress, _amount, _unlockTime);
    }

    // Create multiple locks - vesting
    function lockTokenMultiple(address _tokenAddress, uint256[] memory _amount,uint256[] memory _unlockTime) external payable callerIsUser nonReentrant {

        uint256 requiredFee = (_amount.length - 1) * (bnbSubFee) + bnbFee;
        require( msg.value >= requiredFee, 'BRISE fee not provided');
    ...
    // Renew lock
    function renewToken(uint256 _id, uint256 _unlockTime) external payable callerIsUser {
        ...
        require(msg.value > renewFee, "Pay the fee!");
    ...
    // Withdraw tokens
    function withdrawTokens(uint256 _id) external callerIsUser nonReentrant {
    ...
    function _lockTokens(address _tokenAddress, uint256 _amount, uint256 _unlockTime) internal returns (uint256 _id) {
    ...
    function withdrawWrongTokens(address _whatToken, address _to, uint256 _amount) external onlyOwner {
        
        IERC20 _tokenToWitdhraw = IERC20(_whatToken);

        uint256 _balanceOfTheSmartContract = _tokenToWitdhraw.balanceOf(address(this));
        require(_balanceOfTheSmartContract > 0 && _amount < _balanceOfTheSmartContract, "Not enough tokens!");

       require(_tokenToWitdhraw.transfer(_to, _balanceOfTheSmartContract), "Erc20 Transfer Failed");
    
       emit WithdrawSpecificToken(_whatToken, _to, _amount);
    }

    // Withdraw BRISE
    function withdrawBRISE() external onlyOwner {
    ...
    function setbnbFee(uint256 fee) external callerIsUser onlyOwner {
        require(fee > 0, "Fee is too small");
        bnbFee = fee;
    }

    function setbnbSubFee(uint256 fee) external callerIsUser onlyOwner {
        require(fee > 0, "Fee is too small");
        bnbSubFee = fee;
    }

    function setRenewbnbFee(uint256 fee) external callerIsUser onlyOwner {
        require(fee > 0, "Fee is too small");
