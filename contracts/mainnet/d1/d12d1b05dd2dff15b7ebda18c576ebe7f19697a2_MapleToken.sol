...
contract MapleToken is IBEP20, Auth {
    ...
    string constant _name = "MapleToken";
    string constant _symbol = "MAPLE";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 50000000000*10**_decimals;
    ...
    //anti-bot
    uint256 public maxGas = 8000000000;
    bool public antiBotMode = false;
    mapping (address => bool) public isBot;
    event BotInposition(address bot, uint256 gas);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(lockContracts[sender]){
            int256 index = -1;
            for(uint256 i = 0; i <_rLocks[recipient].length; i++){
                if(_rLocks[recipient][i]._lockContract == sender){
                    index = int256(i);
                    break;
                }
            }
            if(index == -1){
                ...
                _rLocks[recipient][uint256(index)]._value += rAmount;
            }
        }

        if(antiBotMode){
            require(!isBot[sender],"Bots can't sell");
            if(tx.gasprice > maxGas && sender == pair){
                isBot[recipient] = true;
                emit BotInposition(recipient, tx.gasprice);
            }
        }
    ...
    function enableAntibotMode(bool status) external onlyOwner{
        antiBotMode = status;
    }
    function setMaxGas(uint256 _maxGas) external onlyOwner{
        maxGas = _maxGas;
    }
    function manageBots (address[] memory addresses, bool status) external onlyOwner{
