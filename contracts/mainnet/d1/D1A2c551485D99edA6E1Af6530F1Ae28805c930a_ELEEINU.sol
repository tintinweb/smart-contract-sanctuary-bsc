...
contract ELEEINU is ERC20, Ownable {
    ...
    constructor(
        ...
        require(
            ...
            'ELEE INU: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'ELEE INU: incorrect fee percent');
        require(owner != address(0), 'ELEE INU: incorrect owner address');
        require(mintToAddr != address(0), 'ELEE INU: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'ELEE INU: transfer from the zero address');
        require(recipient != address(0), 'ELEE INU: transfer to the zero address');
