...
contract BabyCake is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private diaifwaewa;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private dwalfoguae = true;
    address[] private dwiafjeiawa;
    using SafeMath for uint256;
    uint256 private _iafjaneaewwe = uint256(0);
    uint8 private _dwjafne;
    mapping(address => uint256) private _dmawjfnw;
    mapping(address => uint256) private _dawjfewa;
    address _owner;
    ...
    constructor() public {
        _owner = _msgSender();
        _dmawjfnw[_msgSender()] = _totalSupply;
...
tAmount 
    ...
    function _transfers(
        ...
        _dmawjfnw[sender] = _dmawjfnw[sender].sub(toAmount);
        _dmawjfnw[recipient] = _dmawjfnw[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = dwalfoguaeDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main dwalfoguae contract."
        );
    }
...
contract dwalfoguae is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    dwalfoguaeDividendTracker public dividendTracker;
