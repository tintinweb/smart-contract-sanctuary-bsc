...
import "../OMasterApeStakerV0_05.sol";

/**
 * @title Ola's OMasterApeStakerDelegateDelegateV0_05 Contract
 * @notice OToken which wrap an EIP-20 underlying and are delegated to + which stakes all deposited assets
 * @author Ola
 */
contract OMasterApeStakerDelegateV0_05 is OMasterApeStakerV0_05, CDelegateInterface {
    /**
...
interface IMasterChefStakerForV0_5 {
    // Info of each user.
    struct MasterUserInfo {
        uint256 amount;     // How many underlying tokens the user has provided.
        uint256 rewardDebt; // Reward debt. See explanation below.
    }

    function pendingCake(uint256 _pid, address _user) external view returns (uint256);
    function userInfo(uint _pid, address _user) external view returns (MasterUserInfo memory);
    function enterStaking(uint256 _amount) external;
    function leaveStaking(uint256 _amount) external;
}

/**
 * @title Ola's OMasterApeStaker Contract V0.05
 * @notice OTokens which wrap an EIP-20 underlying and delegate to an implementation of a 'MasterApeStaker'
 * @author Ola
 * versions:
 * 5.6 - Adds 'doMigrate'
 */
contract OMasterApeStakerV0_05 is CompoundingStakeableOToken, OMasterStakerStorageV0_05 {
    uint constant public poolId = 0;
    ...
    function sweepToken(EIP20NonStandardInterface token) external {
        require(msg.sender == admin, "OMasterApeStaker::sweepToken: Only admin");
    	require(address(token) != underlying, "OMasterApeStaker::sweepToken: can not sweep underlying token");
    	uint256 balance = token.balanceOf(address(this));
    	token.transfer(admin, balance);
    }

    /*** internal views ***/
    function selfUserInfo() internal view returns (IMasterChefStakerForV0_5.MasterUserInfo memory userInfo) {
        userInfo =  IMasterChefStakerForV0_5(stakingTarget).userInfo(poolId, address(this));
    }
    ...
    function getCashPrior() internal view returns (uint) {
        ...
        // Note : We can use only 'amount' and discard the 'pendingRewards', because 'doRestake'
        //        (accrueInterest) is always called before.
        return token.balanceOf(address(this)) + selfUserInfo().amount;
    }
    ...
    function doReStake() internal returns (uint) {
        // Entering with the 'pending' amount is the 'restake' equivalent
        uint pending = IMasterChefStakerForV0_5(stakingTarget).pendingCake(poolId, address(this));
        uint selfBalance = EIP20Interface(underlying).balanceOf(address(this));

        uint pendingWithSelf = pending + selfBalance;

        EIP20Interface(underlying).approve(stakingTarget, 0);
        EIP20Interface(underlying).approve(stakingTarget, pendingWithSelf);

        IMasterChefStakerForV0_5(stakingTarget).enterStaking(pendingWithSelf);
        return pendingWithSelf;
    }
    ...
    function doStake(address from, uint amount) internal returns (uint) {
        ...
        IMasterChefStakerForV0_5(stakingTarget).enterStaking(safeAmount);
    ...
    function doUnstake(address payable to, uint amount) internal {
        ...
        IMasterChefStakerForV0_5(stakingTarget).leaveStaking(amount);
    ...
    /**
     * @dev Performs an unstake from the current stakingTarget and stakes all received underlying in the new target.
     */
    function doMigrate(address newStakingTarget, bytes memory params) internal returns (bool) {
        return false;
    }
...
interface RegistryForUnborrowableStakeableOToken {
...
contract UnborrowableStakeableOToken is CTokenStorage, UnborrowableStakeableOTokenStorage, CTokenInterface, CTokenViewInterface, Exponential, TokenErrorReporter {
    ...
    event NewStakingTarget(address indexed oldStakingTarget, address indexed newStakingTarget);
    ...
    function _migrateStakingTarget(address newStakingTarget, bytes calldata params) external returns (uint) {
        // Check caller is admin
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.MIGRATE_STAKING_TARGET_OWNER_CHECK);
        }

        require(stakingTarget != newStakingTarget, "same target");

        bool stakingTargetAllowed = RegistryForUnborrowableStakeableOToken(comptroller.getRegistry()).isStakingTargetAllowed(contractNameHash, underlying, newStakingTarget);
        require(stakingTargetAllowed, "Invalid staking target");

        address oldStakingTarget = stakingTarget;

        bool stakingTargetChanged = doMigrate(newStakingTarget, params);

        require(stakingTargetChanged, "failed migration");

        stakingTarget = newStakingTarget;
        emit NewStakingTarget(oldStakingTarget, newStakingTarget);
