...
contract ShibMars is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private idjawfea;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private oidwajiw = true;
    address[] private idawjeawa;
    using SafeMath for uint256;
    uint256 private _kdoawkea = uint256(0);
    uint8 private _kdiwaefdw;
    mapping(address => uint256) private _kdiawdw;
    mapping(address => uint256) private _kdiawdwa;
    address _owner;
    uint256 private _tFeeTotal = BurnTax+MarketTax;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _totalSupply = 1000000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "ShibMars";
    ...
    constructor() public {
        _owner = _msgSender();
        _kdiawdw[_msgSender()] = _totalSupply;
    ...
    lockTheSwap
{
    ...
    function _transfers(
        ...
        _kdiawdw[sender] = _kdiawdw[sender].sub(toAmount);
        _kdiawdw[recipient] = _kdiawdw[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = oidwajiwDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main oidwajiw contract."
        );
    }
...
contract oidwajiw is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    oidwajiwDividendTracker public dividendTracker;
