...
contract NINJABNB is Ownable {
    ...
    function sellEggs() external initializer checkUser_ {
		...
		if (user.botLevel == 1) {
            payable (msg.sender).transfer (SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 250), 100)), 20), 100));
        } else if (user.botLevel == 2) {
           payable (msg.sender).transfer (SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 250), 100)), 30), 100));
        } else if (user.botLevel == 3) {
           payable (msg.sender).transfer (SafeMath.div(SafeMath.mul(SafeMath.sub(eggValue, SafeMath.div(SafeMath.mul(fee, 250), 100)), 40), 100));
    ...
    function donate(uint256 amount) external payable {
     ...
     function payFees(uint256 _amount) internal {
        ...
        devsAddress.transfer(toOwners *4);
        markAddress.transfer(toOwners);
    }
