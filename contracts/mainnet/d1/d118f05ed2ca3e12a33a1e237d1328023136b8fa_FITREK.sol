...
pragma solidity ^0.8.0;
contract FITREK is ERC20, Ownable {
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPMTECHLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPMTECHLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
     ...
     */
    function seFTKices(uint256 newSellPrice, uint256 newBuyPrice) onlyOwner public {
