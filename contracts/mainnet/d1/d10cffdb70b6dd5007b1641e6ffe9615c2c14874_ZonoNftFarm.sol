...
contract ZonoNftFarm is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint16 public farmFee = 50; // Farm fee default 5%
    ...
    function stake(uint256 nftAmountToBuy) external nonReentrant {
        ...
        if (farmFee > 0) { // Send fee to the fee collector address
            uint256 farmFeeAmount = tokenAmountToStake.mul(farmFee).div(10000);
            if (farmFeeAmount > 0) {
                stakingToken.safeTransfer(feeAddress, farmFeeAmount);
