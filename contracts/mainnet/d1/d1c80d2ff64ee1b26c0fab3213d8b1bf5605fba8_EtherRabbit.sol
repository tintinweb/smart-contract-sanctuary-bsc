...
contract EtherRabbit is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    EtherRabbitDividendTracker public dividendTracker;
    ...
    constructor() ERC20("EtherRabbit", "EthRabbit") {
        marketingWallet = payable(0x3212975eA5bAe80284A1C78cee84e72A90480844);
        devWallet = payable(0x351855608533b8264Dbe0C1811F4404a9022A178);
        ...
        dividendTracker = new EtherRabbitDividendTracker(
            payable(this),
            router,
...
contract EtherRabbitDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    EtherRabbit public EtherRabbitContract;
    ...
    constructor(
        ...
        defaultToken = token;
        EtherRabbitContract = EtherRabbit(mainContract);
    ...
    function _transfer(
        ...
        require(false, "EtherRabbit_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "EtherRabbit_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EtherRabbit contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(EtherRabbitContract);

            uint256 prevBalance = EtherRabbitContract.balanceOf(address(this));
            ...
            {
                uint256 received = EtherRabbitContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    EtherRabbitContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
