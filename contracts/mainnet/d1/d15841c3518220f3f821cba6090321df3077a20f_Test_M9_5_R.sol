 ...
 //bool public activTORSREF_REWARD;//bool activate
               ...
               if(users[referrer].referrer == upline1 && users[upline1].activeMainLevels[lvel] ==  users[referrer].activeMainLevels[lvel])
    {
				...
				if (upline1 != address(0)) {
                    ...
                    users[upline1].DEP_checkpoint_levels_Referals = block.timestamp;
                    users[upline1].DEP_Dep_levels_Referals = upline1;
                    users[upline1].DEP_Count_levels_Referals=  user.partnersCount.add(1);
                    users[upline1].DEP_Raddress_levels_Referals =  users[upline1].LevelReferrals1;
...
address upline = user.referrer;
			for (uint256 i = 0; i < 1; i++) {
				if (upline != address(0)) {
                    ...
                    users[upline].REG_checkpoint_levels_Referals = block.timestamp;
                    users[upline].REG_Dep_levels_Referals = upline;
                    users[upline].REG_Count_levels_Referals=  user.partnersCount.add(1);
                    users[upline].REG_Raddress_levels_Referals =  referrerAddress;
...
adrSE = user.SE_total_LevelReferrals1US_ADR;
	...
	function REG__levels_Referals(address userAddress) public view returns(uint256 checkpointTimer, address adres1, uint256 Count, address adres2) {
	    User storage user = users[userAddress];
