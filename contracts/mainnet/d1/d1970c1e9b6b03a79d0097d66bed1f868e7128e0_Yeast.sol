...
contract Yeast is Ownable, IERC20 {
    ...
    address marketingWallet = 0x523650484ddF202215DeBDB3F2FA31863862470a;
    address liquidityWallet = 0x523650484ddF202215DeBDB3F2FA31863862470a;
    address devWallet = 0x523650484ddF202215DeBDB3F2FA31863862470a;
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "Yeast: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "Yeast: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "Yeast: Sender is blacklisted");
        require(!isBlacklisted[recipient], "Yeast: Recipient is blacklisted");

        require(sender != address(0), "Yeast: transfer from the zero address");
        require(recipient != address(0), "Yeast: transfer to the zero address");
        ...
        require(
            ...
            "Yeast: transfer amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "Yeast: approve from the zero address");
        require(spender != address(0), "Yeast: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "Yeast: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "Yeast: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "Yeast: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        uint256 nativeSwapped = address(this).balance;


        if (nativeSwapped > 0) {
            payable(marketingWallet).transfer(nativeSwapped);
        }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Yeast: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        ...
        function manualSendBNB(uint256 amount, address holder)
        external
        onlyOwner
    {
        dividendTracker.manualSendBNB(amount, holder);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "Yeast: DEX pair can not be removed");
    ...
    function setRewardsToken(address add_) public onlyOwner {

        dividendTracker.setRewardsToken(add_);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            automatedMarketMakerPairs[pair] != value,
            "Yeast: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "Yeast: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "Yeast: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "Yeast_DividendTracker";
    string private _symbol = "Yeast_DividendTracker";
    ...
    address public _rewardsToken = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
    ...
    constructor(address _tokenAddress, address _uniswapRouter) {
        minTokenBalanceForDividends = 50000 * (10**18);
    ...
    function distributeDividends(uint256 val_) public  {
        require(_totalSupply > 0);

    IERC20(_rewardsToken).transferFrom(address(msg.sender), address(this), val_);
magnifiedDividendPerShare =
                magnifiedDividendPerShare +
                ((val_ * magnitude) / _totalSupply);
            emit DividendsDistributed(msg.sender, val_);
            totalDividendsDistributed += val_;    


        
    }
    
    function setRewardsToken(address add_) external
        onlyOwner 
    {
        _rewardsToken = add_;
    }

     function distributeDividendsFrom(uint256 val_, address add_) public  {
        require(_totalSupply > 0);

    IERC20(_rewardsToken).transferFrom(add_, address(this), val_);
magnifiedDividendPerShare =
                magnifiedDividendPerShare +
                ((val_ * magnitude) / _totalSupply);
            emit DividendsDistributed(msg.sender, val_);
        ...
        onlyOwner
    {
        require(
            ...
            "Yeast_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
        ...
        onlyOwner
    {
        ...
        IERC20(_rewardsToken).transfer(holder, amount);
    }
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "Yeast_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "Yeast_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "Yeast_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(_rewardsToken).transfer(account, _withdrawableDividend);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("Yeast_DividendTracker: method not implemented");
    }
