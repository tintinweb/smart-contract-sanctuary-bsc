...
contract FireCatNFT is ERC721, Ownable, FireCatNFTStorage, ReentrancyGuard {
    using Strings for uint256;

    event Mint(address minter, address recipient, uint256 newTokenId);
    event UpgradeToken(address minter, uint256 tokenId);
    ...
    */
    function _mint(address recipient) internal returns (uint256) {
        ...
        _ownerTokenId[recipient] = newTokenId;
    ...
    * @param tokenId uint256.
    */
    function upgradeToken(uint256 tokenId) external nonReentrant {
        ...
        emit UpgradeToken(msg.sender, tokenId);
    }
    ...
    */
    function setHighestLevel(uint256 level_) external onlyOwner {
        require(_highestLevel + level_ > _highestLevel, "NFT:E09");
