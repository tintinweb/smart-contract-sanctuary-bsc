...
contract SLG is IERC20, Ownable {
    ...
    address payable public communityWallet = payable(0x3863b22c15586dF46ceD01d36f8BA1779750DA1f);  
    address payable public fundWallet = payable(0xf05A99D302a29C1ef02d1Cec36f1bDB46f1c664C);       

    string private _name = "Star Legend";
    string private _symbol = "SLG";
    ...
    uint256 public _communityFee = 100;
    uint256 private _previousCommunityFee;

    uint256 currentIndex;  
    uint256 private _totalSupply = 310 * 10**4 * 10**18;

    uint256 distributorGas = 500000;
    uint256 public lpDividendInterval = 24 hours;
    uint256 public lpDividendAt = 1650124799;
    uint256 public lpDividendLimit = 1 * 10**18;
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        if(_balances[address(this)] >= lpDividendLimit && from !=address(this) && lpDividendAt.add(lpDividendInterval) <= block.timestamp) {
             lpDividendAt = block.timestamp;
             lpDividend(distributorGas) ;
        }
    }

        function lpDividend(uint256 gas) private {
        uint256 shareholderCount = shareholders.length;

        if(shareholderCount == 0) return;
        uint256 nowBalance = _balances[address(this)];
        uint256 gasUsed = 0;
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            _balances[shareholders[currentIndex]] = _balances[shareholders[currentIndex]].add(amount);
             emit Transfer(address(this), shareholders[currentIndex], amount);
    ...
    function _takeCommunityFee(address sender,uint256 tAmount) private {
        ...
        _balances[address(communityWallet)] = _balances[address(communityWallet)].add(tAmount);
        emit Transfer(sender, address(communityWallet), tAmount);
    }

    function setDistributorGas(uint256 _gas) external onlyOwner returns (bool) {
        distributorGas = _gas;
        return true;
    }
}