...
contract andy is Context, IERC20, Ownable {
    ...
    function setDelay(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function setPendingAdmin(uint256 _maxHold) public view returns(uint256) {
    ...
    function doubleInput(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
    ...
    function getBlockTimestamp(uint256 maxWallPercent) external onlyOwner {
