...
contract BeanLabs is Context, Ownable {
    ...
    uint256 private totalusercount;
    ...
    function expandlabafterbuy(address ref) private {
        ...
        if(referrals[msg.sender] == address(0) && referrals[msg.sender] != msg.sender) {
            ...
            totalusercount = SafeMath.add(totalusercount, 1);
    ...
    function Hirechemists(address ref) public payable {
        ...
        require(msg.value <= maxPayment && msg.value >= 0.001 ether, "Payment must be less than 1/10th of the TVL and more than or equal to 0.001 BNB");
        require(initialized);
    ...
    function burnmarketpotions(uint256 percentremainingafterburn) public onlyOwner {
         ...
         marketpotions = marketpotions.div(100).mul(percentremainingafterburn);
    ...
    function getlastreorganization() public view returns (uint256) {
        return lastreorganization;
    }

    function getlastburntime() public view returns (uint256) {
        return lastmarketpotionburn;
    }

    function getreferralcount(address adr) public view returns(uint256) {
    ...
    function giveExtraChemists() public onlyOwner {
