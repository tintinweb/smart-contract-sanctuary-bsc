...
contract Wtf is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 WTF-WETH LP -> WTF pool
    uint256 public constant INITIAL_WTF_POOL_DISTRIBUTION = 0 ether;
    ...
    /**
     * @notice Constructs the WTF2 ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Waterfall2", "WTF2") {
        // Mints 210 WTF to contract creator for initial pool setup
        require(_taxRate < 10000, "tax equal or bigger to 100%");
        require(_taxCollectorAddress != address(0), "tax collector address must be non-zero address");

        excludeAddress(address(this));

        _mint(msg.sender, 210 ether);
        taxRate = _taxRate;
    ...
    function _getBombPrice() internal view returns (uint256 _bombPrice) {
        try IOracle(wtfOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function setwtfOracle(address _wtfOracle) public onlyOperatorOrTaxOffice {
        require(_wtfOracle != address(0), "oracle address cannot be 0 address");
        wtfOracle = _wtfOracle;
    }
    ...
    /**
     * @notice Operator mints WTF to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of WTF to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentWtfPrice = _getBombPrice();
            currentTaxRate = _updateTaxRate(currentWtfPrice);
            if (currentWtfPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_wtfPool != address(0), "!_wtfPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_wtfPool, INITIAL_WTF_POOL_DISTRIBUTION);
