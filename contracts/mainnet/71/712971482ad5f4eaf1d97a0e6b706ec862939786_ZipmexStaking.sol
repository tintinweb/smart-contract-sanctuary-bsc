...
contract ZipmexStaking is ReentrancyGuardUpgradeable {
                ...
                uint256[4] memory claimableReward)
    {
        for (uint256 i=0; i < 4; i++) {
            if(!_stakeExist[account][i])
                continue;
            (stakeData[i],totalReward[i],claimableReward[i])=getStake(account, i);
        }
    }
     ...
     */
    function getLastClaimTime(address _account, uint256 _vault) external view returns (uint256) {
     ...
     */
    function isVaultModified(address _account, uint256 _vault) external view returns (bool) {
    ...
    /**
     * @notice This function is used to withdraw the contract balance
     * @dev Only the owner can call this function
    ...
    function withdrawBalance() external onlyOwner nonReentrant {
        uint256 balance = _tokenAddress.balanceOf(address(this)) -
            _totalStake -
            _totalExpectedBonus;
        require(balance > 0, "ZipmexStaking: Zero balance");
    ...
    function claimReward(uint256 vault) external nonReentrant {
        ...
        require(interval > 0, "ZipmexStaking: No rewards");
     ...
     */
    function _getPenaltyAndRewards(StakeData memory stakeData,uint256 claimTime, bool isUnstake)
        private
        returns (
            uint256,
            uint256,
        ...
        )
    {
        ...
        if (block.timestamp < stakeData.stakeUnlockTime) {
            ...
            if(stakeData.stakeUnlockTime > claimTime) interval = stakeData.stakeUnlockTime - claimTime;
            unlockOver = true;
        }
    ...
    {
        if (unlockTime > block.timestamp)  interval = block.timestamp - lastClaimTime;
        else if(unlockTime > lastClaimTime) interval = unlockTime - lastClaimTime;
    }
