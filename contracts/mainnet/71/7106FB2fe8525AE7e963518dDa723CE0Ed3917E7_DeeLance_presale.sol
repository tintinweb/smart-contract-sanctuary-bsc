...
contract DeeLance_presale is ReentrancyGuard, Ownable, Pausable {
    ...
    uint public phase;
    uint public inSaleUSDvalue_1;
    uint public inSaleUSDvalue_2;
    uint public inSaleUSDvalue_3;
    ...
    constructor() {
        ...
        totalTokensForPresale = 5000;
        totalUsdValueForPresale = 120;
   ...
   function setSalePricePhase() internal  {
        if(phase == 2) {
            salePrice = 25 * (10 ** 15);
        }  
        if(phase == 3) {
    ...
    function buyWithBNB(uint amount) external payable checkSaleState(amount) whenNotPaused nonReentrant {
        ...
        inSaleUSDvalue_1 -= usdPrice;

        } else if (phase == 2 ) {
        inSaleUSDvalue -= usdPrice;
        inSaleUSDvalue_2 -= usdPrice;

        } else {

        inSaleUSDvalue -= usdPrice;
        inSaleUSDvalue_3 -= usdPrice;

        }

        if (phase == 1 && inSaleUSDvalue_1 <= 0) {
            phase = 2;
            setSalePricePhase();
        } else if (phase == 2 && inSaleUSDvalue_2 <= 0) {
            phase = 3;
            setSalePricePhase();
        } 
