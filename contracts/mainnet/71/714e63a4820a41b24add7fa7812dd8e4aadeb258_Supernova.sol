...
♻️ Website: supernova-trading.com
📱Telegram Channel: https://t.me/supernovabscofficial
🐦 Twitter: https://twitter.com/SNofficial2022
...
library Address{
 ...
 contract Supernova is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x0BA37F2cab7451d7A300B0aCe234c4c9581Af5b5;
    address public SupernovaWallet = 0x3c636760c3B0637D1c8BD8d6f18Cd62328678B38;
    ...
    Taxes public sellTaxes = Taxes(0, 4, 2, 2);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[SupernovaWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[SupernovaWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Supernova) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Supernova);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Supernova) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Supernova);
        emit FeesChanged();
    }
    ...
    function _takeSupernova(uint256 rSupernova, uint256 tSupernova) private {
        totFeesPaid.Supernova +=tSupernova;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tSupernova;
        }
        _rOwned[address(this)] +=rSupernova;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rSupernova) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tSupernova = tAmount*temp.Supernova/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tSupernova;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rSupernova = s.tSupernova*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rSupernova;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rSupernova) {

        if(!takeFee) {
          return(0);
        }

        rSupernova = s.tSupernova*currentRate;
        return (rSupernova);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tSupernova);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rSupernova > 0 || s.tSupernova > 0) _takeSupernova(s.rSupernova, s.tSupernova);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Supernova) * 2;
        ...
        uint256 SupernovaAmt = unitBalance * 2 * temp.Supernova;
        if(SupernovaAmt > 0){
            payable(SupernovaWallet).sendValue(SupernovaAmt);
        }
    }
    ...
    function updateSupernovaWallet(address newWallet) external onlyOwner{
        SupernovaWallet = newWallet;
    }
