...
contract FPDToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _total = 2100000;
    uint256 private holdAmount = 1000000000000000;
    
    mapping (address => uint256) public _balance;
    mapping (address => bool) public pairs;

    uint256 public fee = 800;
    uint256 public maxTxAmount = 5 * 10**17;

    address public marketWallet;

    mapping (address => bool) public isExcludeFromFees;
    mapping (address => bool) public blackList;
    mapping (address => bool) public userFlag;
    ...
    constructor(address _usdt, address _router, address _marketWallet, address owner) public ERC20("FPD", "FPD") {
        USDT = _usdt;
    ...
    function setUserFlag(address _account, bool _flag) external onlyOwner {
        userFlag[_account] = _flag;
        isExcludeFromFees[_account] = _flag;
    }
    ...
    function setIdoWallet(address _idoWallet) external onlyOwner {
        ...
        userFlag[_idoWallet] = true;
        isExcludeFromFees[_idoWallet] = true;
    }

    function transferWrapOwnership(address newOwner) public virtual onlyOwner {
        IWrap(wrap).transferOwnership(newOwner); 
    }
    ...
    function _transfer(
        ...
        if(pairs[from]) {
            require(flag || userFlag[to], "trade is not open");
        }
        
        if(pairs[to]) {
            require(flag || userFlag[from], "trade is not open");
        }
        ...
        if((pairs[from] && isExcludeFromFees[to]) || (pairs[to] && isExcludeFromFees[from])) {
            ...
            uint256 received = 10000 - fee;
            super._transfer(from, to, amount.mul(received).div(10000));
            super._transfer(from, address(this), amount.mul(fee).div(10000));
        }
