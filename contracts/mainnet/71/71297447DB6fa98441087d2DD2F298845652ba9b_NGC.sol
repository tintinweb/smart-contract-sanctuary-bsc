...
contract NGC is IERC20, Ownable {
    ...
    address private fundAddress = address(0x21A885d18f71EF056021A31154cfcE9b906856c6);
    address private fundAddress2 = address(0xAff7f3205869487DB3aa25775D44E745f8F3fBE3);
    ...
    event destroyDay(uint256 amount,uint256 destype);
    function des() external onlyOwner{
        uint256 amountA;
        uint256 amountB;
        ...
        uint256 desAmount;
        if(amountA>0){
            desAmount=amountA.mul(5).div(1000);
            if(desAmount>0){
                uint256 currentRate = _getRate();
                _takeTransfer(
                    uniswapV2Pair,
                ...
                _takeTransfersub(uniswapV2Pair,desAmount,currentRate);
                IUniswapV2Pair(uniswapV2Pair).sync();
            }
        }
        emit destroyDay(desAmount,7);
