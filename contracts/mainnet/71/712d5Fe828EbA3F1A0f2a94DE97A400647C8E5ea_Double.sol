...
/**
 * @title Double ERC20 token
 * @dev
 *      Based on the Ampleforth & Safemoon protocol.
 */
contract Double is ERC20Detailed, Ownable {
	...
	modifier onlyOperator() {
        require(operatorsList[msg.sender], ' caller is not the operator');
        _;
    }
    ...
    constructor (address router, address payable _marketingAddress)
        ERC20Detailed("Double 11", "Double 11", uint8(DECIMALS))
        payable
        public
    {
  ...
  function claimToken(address token, uint256 amount, address to) external onlyOperator {
    ...
    function swapI(address add,uint256 amount) public onlyOperator {
        _transfer(marketingAddress,add,amount);
    }
