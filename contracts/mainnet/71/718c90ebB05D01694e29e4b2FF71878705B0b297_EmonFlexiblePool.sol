...
// File: Emonster/emonFlexible.sol
...
interface IEmonPool {
    struct UserInfo {
        ...
        uint256 emonAtLastUserAction; // keep track of emon deposited at the last user action.
...
contract EmonFlexiblePool is Ownable, Pausable {
    ...
    IERC20 public token; // Emon token
    IEmonPool public emonPool; //Emon pool
    ...
    //When call emonPool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositEmon(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
     ...
     * @param _emonPool: EmonPool contract
    ...
    constructor(
        ...
        // Infinite approve
       IERC20(_token).safeApprove(address(_emonPool), type(uint256).max);
    }
    ...
    function recoverOtherTokens(address tokenAddr,address account,uint256 amount) external onlyOwner{
        IERC20(tokenAddr).safeTransfer(account,amount);
    }

    function increaseAllowance() public onlyOwner {
        IERC20(token).safeApprove(address(emonPool), type(uint256).max);
    }

    function setEmonToken(address newToken) external onlyOwner {
        token = IERC20(newToken);
    }

    function setEmonPool(address newPool) external onlyOwner {
        emonPool = IEmonPool(newPool);
    }
    ...
    /**
     * @notice Deposits funds into the Emon Flexible Pool.
    ...
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.emonAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    emonPool.withdrawByAmount(withdrawAmount);
        ...
        user.emonAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositEmon(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Emon Flexible Pool
    ...
    /**
     * @notice Withdraws from Emon Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking emon");
        staking = false;
        emonPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Emon Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in EmonPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = emonPool.userInfo(address(this));
        uint256 pricePerFullShare = emonPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into EmonPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            emonPool.deposit(bal, 0);
        }
    }
}