...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 hatchTime) public view returns (uint256) {
        ...
        uint256 ddogeAge = block.timestamp - hatchTime;

        uint256 base = totalSupply * 200 / FACTOR_MAX; // 2%
        uint256 incrasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = incrasePerMinute * ddogeAge / (1 minutes); // up 0.1% per minute
...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, DDOGEStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {        
...
contract DDOGEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    Dunkindoge public immutable token;
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("DunkinDogeDividendTracker", "DDOGE_DIVS") {
        token = Dunkindoge(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "DDOGEDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "DDOGEDividendTracker: max vesting duration is 30 days");
    ...
    function updateUnvestedDividendsMarketingFee(uint256 newUnvestedDividendsMarketingFee) public onlyOwner {
        require(newUnvestedDividendsMarketingFee <= 30, "DDOGEDividendTracker: max marketing fee is 30%");
...
library Fees {
    ...
    function init(
        Data storage data,
        DDOGEStorage.Data storage _storage,
    ...
    function updateFeeDestinationPercents(Data storage data, DDOGEStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
    ...
    function swapAccumulatedFees(Data storage data, DDOGEStorage.Data storage _storage, uint256 tokenAmount) public {
...
library DDOGEStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        DDOGEDividendTracker dividendTracker;
...
contract Dunkindoge is ERC20, Ownable {
    using SafeMath for uint256;
    using DDOGEStorage for DDOGEStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    DDOGEStorage.Data private _storage;
    ...
    DDOGEDividendTracker public dividendTracker;

    uint256 private swapTokensAtAmount = 200 * (10**18);
    uint256 private swapTokensMaxAmount = 3000 * (10**18);
    ...
    constructor() ERC20("DUNKINDOGE", "DDOGE") {
        ...
        _storage.dividendTracker = new DDOGEDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0x797737Cb25F52FBDDcbd9BdE50Fe79Ff6211Fc53;
        _storage.marketingWallet2 = 0x797737Cb25F52FBDDcbd9BdE50Fe79Ff6211Fc53;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = DDOGEDividendTracker(payable(newAddress));
