...
abstract contract Project {
    ...
    string constant PROJECT_NAME = "tst";
...
contract DividendDistributor is IDividendDistributor  {
    ...
    function setNewRewardForShareholder(address shareholder, address customRwrd) external onlyToken {
        ...
        if(shareholderShare.currentRwrd != customRwrd) {
            ...
            if(rewardsInfo[shareholderShare.currentRwrd].totalHolders > 0) {
                rewardsInfo[shareholderShare.currentRwrd].totalHolders = rewardsInfo[shareholderShare.currentRwrd].totalHolders.sub(1);
            }
 ...
 * MainContract
 */
contract TST is Project, IBEP20, Ownable {
    using SafeMath for uint256;
...
// ***
     ...
     */
        ...
        if(payableFeesInfo.marketing > 0) {
            (bool marketingSendSuccess,) = payable(wallets.marketing).call{value: payableFeesInfo.marketing}("");
            require(marketingSendSuccess, "Failed to send to MW");
        }

        if(payableFeesInfo.treasury > 0) {
            (bool treasurySendSuccess,) = payable(wallets.treasury).call{value: payableFeesInfo.treasury}("");
            require(treasurySendSuccess, "Failed to send to Treasury");
        }

        if(payableFeesInfo.coinwars > 0) {
            (bool coinwarsSendSuccess,) = payable(wallets.coinwars).call{value: payableFeesInfo.coinwars}("");
            require(coinwarsSendSuccess, "Failed to send to Coinwars");
        }
...
// ***
    ...
    function setSwapFees(
        ...
        uint256 newSwapTreasuryFee,
        uint256 newCoinwarsFee
    ) external onlyOwner() {
        taxes.lp = newSwapLpFee;
        taxes.reward = newSwapRewardFee;
        taxes.boost = newSwapRewardBoostFee;
        taxes.marketing = newSwapMarketingFee;
        taxes.treasury = newSwapTreasuryFee;
        taxes.coinwars = newCoinwarsFee;

        require(getTotalSwapFee() < FEE_DENOMINATOR/6, "Fees cannot be more than 16%");
    }

    function setTreasuryFeeReceiver(address newWallet) external onlyOwner() {
        isFeeExempt[wallets.treasury] = false;
        isFeeExempt[newWallet] = true;
        wallets.treasury = newWallet;
    }

    function setCoinwarsWallet(address newWallet) external onlyOwner() {
        wallets.coinwars = newWallet;
    }
