...
contract DYOR is ERC20, Ownable {
    ...
    uint256 public constant supply = 1000000e18;

    bool public protectionEnabled_ = true;
    uint256 public protectionModifier_ = 10;
    ...
    constructor(
        ...
        require(
            treasury != address(0),
            "DYOR: treasury address can not be a null address"
        );
        _transferOwnership(msg.sender);

        treasury_ = treasury;
        if (initialMint > 0) super._mint(msg.sender, initialMint);
    ...
    function setBuyFee(uint256 buyFee) public onlyOwner {
        require(
            buyFee >= 0 && buyFee <= 300,
            "DYOR: buy fee must be between 0 and 300"
        );
        buyFee_ = buyFee;
        ...
        onlyOwner
    {
        require(
            ...
            "DYOR: protection modifier must be greater than 0 and less than 7"
        );
    ...
    function disableProtection() public onlyOwner {
        require(
            protectionEnabled_,
            "DYOR: sell protection is already disabled"
        );
    ...
    function excludeFromFees(address holder) public onlyOwner {
        require(
            holder != address(0),
            "DYOR: holder can not be a null address"
        );
    ...
    function setPancakePair(address pair) public onlyOwner {
        require(pair != address(0), "DYOR: pair can not be a null address");
        pancakePair_ = pair;
    }
