...
contract OmnichainRouterV2 is IOmnichainRouter, NonblockingLzApp, IAxelarExecutable, ReentrancyGuard {
    ...
    */
    ...
    function _lzProcess(uint16 chainId, RouteCall memory params, bool isRedirect, bool isRedirected) internal {
        ...
        bytes memory adapter = _getAdapter(isRedirect ? 0 : params.gas, bytes32(0xa5ee791689000b578f8f2a84373503715da00e599e88e4c58bb433b4629e83d7));
    ...
    function _getAdapter(uint gas, bytes32 nativeRecipient) private pure returns (bytes memory) {
        if (gas == 0) {
            return bytes("");
        }
        uint16 v = 2;
        uint native = 0;
        return abi.encodePacked(v, gas, native, nativeRecipient);
    }
    ...
    function estimateFees(uint16 chainId, bytes memory payload, uint gas) external view returns (uint) {
        (uint fee,) = lzEndpoint.estimateFees(chainId, address(this), payload, lzConfig.payInZRO, _getAdapter(gas, bytes32(0xa5ee791689000b578f8f2a84373503715da00e599e88e4c58bb433b4629e83d7)));
        return fee;
    }
