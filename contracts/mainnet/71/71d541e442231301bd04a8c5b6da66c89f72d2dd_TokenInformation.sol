...
contract TokenInformation {
  ...
  function balances(address user, address[]memory tokens) public view returns (address[] memory){
    address[] memory valid_addresses = new address[](tokens.length);
    uint addrIdx = 0;
    for(uint i = 0; i < tokens.length;i++){
      uint balance = Token(tokens[i]).balanceOf(user);
      if (balance > 0 ){
        valid_addresses[addrIdx] = tokens[i];
        addrIdx++;
      }
    }
