...
interface IMarketUtility {
    function getBasicMarketData() external view returns (uint256, uint256, uint256, address payable, address payable, address payable, address payable); 
...
contract MarketOraclized is Ownable, ReentrancyGuard {
    ...
    address public marketUtility = 0x940CAE047Aa6Fc9ca0A43c762a0B03216d476428; 
    address payable public WOLFYBridge;
    ...
    modifier noContract(address account) {
        require(Address.isContract(account) == false, "No contracts allowed here!");
        _;
    }
    ...
    function calcAndDistributePredictionFee(uint256 _fee) private {
        uint256 base = _fee / 16;
        uint256 marketLiquidityReserve = base * 3; // (0.15%)
        uint256 WPACKStakers = base * 8; // (0.4%)
        uint256 WOLFYStakers = base * 4; // (0.2%)
        uint256 ecosystem = base; // 0.05%

        IMarketLiquidity(marketLiquidity).notifyReward(marketLiquidityReserve);
        IWOLFPACKStakingManager(WPACKStakingManager).notifyReward(WPACKStakers);

        marketLiquidity.transfer(marketLiquidityReserve);
        WPACKStakingManager.transfer(WPACKStakers);
        WOLFYBridge.transfer(WOLFYStakers);
        address payable o = payable(owner());
        o.transfer(ecosystem);        
    } 
    ...
    function resetTimebox(uint256 marketUuid) external noContract(msg.sender) {
    ...
    function settleMarket(uint256 marketUuid) external noContract(msg.sender) {
        require(!markets[marketUuid].settled == true, "market already settled");
        uint256 timebox = markets[marketUuid].timebox;
