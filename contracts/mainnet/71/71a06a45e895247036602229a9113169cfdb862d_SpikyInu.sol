...
contract SpikyInu is Context, IBEP20, Ownable {
    ...
    uint256 public maxBuyLimit = 1e10 * 10**9;
    uint256 public maxSellLimit = 1e10 * 10**9;
    uint256 public maxWalletLimit = 1e10 * 10**9;

    uint256 public genesis_block;
    uint256 private deadline = 3;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x80FAfee166F4a79ad27428677c8B6301Dc2Ae3D9;
    address public devWallet = 0xaa3D9E4513266D5056Da19d5B9562d5b72f1a8cB;
    address public opsWallet = 0x76432c076268F2c017C8Ff05656c75aA5533bC35;
    address public charityWallet = 0x8B99F3660622e21f2910ECCA7fBe51d654a1517D;
    address public stakingWallet = 0x80FAfee166F4a79ad27428677c8B6301Dc2Ae3D9;

    string private constant _name = "Spiky Inu";
    string private constant _symbol = "SPK";
    ...
    Taxes public taxes = Taxes(2, 4, 0, 1, 1, 1, 1);
    Taxes public sellTaxes = Taxes(2, 4, 0, 1, 1, 1, 1);
    Taxes private launchtax = Taxes(0, 99, 0, 0, 0, 0, 0);
    ...
    function setTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _dev + _ops + _charity + _staking) <= 20, "Must keep fees at 20% or less");
        taxes = Taxes(_rfi, _marketing, _liquidity, _dev , _ops,  _charity, _staking);
        emit FeesChanged();
    }
    ...
    function setSellTaxes(
        ...
        require((_rfi + _marketing + _liquidity + _dev + _ops + _charity + _staking) <= 20, "Must keep fees at 20% or less");
        sellTaxes = Taxes(_rfi, _marketing, _liquidity, _dev, _ops, _charity, _staking);
        emit FeesChanged();
    }
    ...
    function _getValues(
        ...
        (to_return.rDev, to_return.rOps,  to_return.rCharity, to_return.rStaking) = _getRValues2(
            to_return,
            takeFee,
            _getRate()
        );
    ...
    function _getRValues2(
        ...
        pure
        returns (
            uint256 rDev, uint256 rOps, uint256 rCharity, uint256 rStaking
        )
    {
        if (!takeFee) {
            return (0,0,0,0);
        ...
        return (rDev, rOps, rCharity, rStaking);
    }
    ...
    function _tokenTransfer(
        ...
        if (s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(
                sender,
                address(this),
                s.tLiquidity + s.tMarketing + s.tDev + s.tOps + s.tCharity + s.tStaking
            );
        }
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount <= 1e10, "Cannot set swap threshold amount higher than 1% of tokens");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell, uint256 maxWallet) external onlyOwner {
        require(maxBuy >= 1e9, "Cannot set max buy amount lower than 0.1% of tokens");
        require(maxSell >= 1e9, "Cannot set max sell amount lower than 0.1% of tokens%");
        require(maxWallet >= 1e10, "Cannot set max wallet amount lower than 1% of tokens");
