...
contract ZEUBUSD is Context, Ownable , ReentrancyGuard {
    ...
    uint256 public unstakePercentage = 500;
    address public dev = 0x5a42E0942678A720F17EF34ef6193639b8121e3f;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
       ...
       // fees 
        uint256 total_fee = estimatedFee(_amount);
    ...
    require(totalRewards[msg.sender].amount <= SafeMath.mul(investments[msg.sender].invested, 2), "You cant withdraw you have collected two times Already"); // hh new
    ...
    uint256 wFee = estimatedFee(aval_withdraw2); // changed from aval_withdraw
    ...
    function claimDailyRewards() public noReentrant{
        ...
        uint256 cFee = estimatedFee(totalRewardAmount);
        BusdInterface.transfer(dev, cFee);
        uint256 claimTimeStart = block.timestamp;
        uint256 claimTimeEnd = block.timestamp + 1 days;
    ...
    function unStake() external noReentrant {
        ...
        uint256 lastFee = estimatedFee(I_investment);
        uint256 currentBalance = SafeMath.sub(I_investment, lastFee);

        uint256 UnstakeValue = SafeMath.sub(currentBalance, t_withdraw);
        uint256 uFee = estimatedFee(UnstakeValue);
        BusdInterface.transfer(dev, uFee);

        uint256 UnstakeValueCore = SafeMath.div(UnstakeValue, SafeMath.div(unstakePercentage, 10));
    ...
    function Ref_Withdraw() external noReentrant {
        ...
        uint256 Rfee = estimatedFee(value);

        BusdInterface.transfer(dev, Rfee);
    ...
    // change unstake percentage

    function changePercentage(uint256 newPercentage) public onlyOwner {
        require(newPercentage <= 500, "This amount cannot be greater than 500");
        unstakePercentage = newPercentage;
    }
    ...
    function estimatedFee(uint256 _amount) public pure returns(uint256){
     return SafeMath.div(SafeMath.mul(_amount, marketing_fee + dev_fee), 100);
    }
