...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    event IncludeFromDividends(address indexed account);
    ...
    function includeFromDividends(address account,uint256 amount) external onlyOwner {
    	require(excludedFromDividends[account]);
    	excludedFromDividends[account] = false;
    	_setBalance(account, amount);
    	tokenHoldersMap.set(account, amount);
    	emit IncludeFromDividends(account);
    }
...
contract Hyperflair_Token is ERC20, Ownable {
    uint256 public rewardFeeOnBuy       = 40;
    uint256 public marketingFeeOnBuy    = 20;
    uint256 public liquidityFeeOnBuy    = 40;
    uint256 public burnTaxFeeOnBuy      = 20;
    uint256 public rewardFeeOnSell      = 40;
    uint256 public marketingFeeOnSell   = 20;
    uint256 public liquidityFeeOnSell   = 80;
    uint256 public burnTaxFeeOnSell     = 20;
    uint256 public totalBuyFee          = 120;
    uint256 public totalSellFee         = 160;
    uint256 public walletToWalletFee    = 0;
    address public marketingWallet = 0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7;
    ...
    constructor() ERC20("Hyperflair", "FLAIR") {
        address newOwner = 0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7;
        transferOwnership(newOwner);
    	dividendTracker = new DividendTracker(1e5, rewardToken);
        ...
        maxWalletAmount       = (1e9 * (10 ** decimals()) * 4) / 100;
        maxTransactionLimitEnabled = true;
        maxTransactionAmountBuy    = 1e9 * (10 ** decimals());
        maxTransactionAmountSell   = (1e9 * (10 ** decimals())) / 100;
        ...
        swapTokensAtAmount = (1e9 * (10 ** decimals())) / 2000;
        _mint(newOwner, 1e9 * (10 ** decimals()));
        operator = _msgSender();
    }
    ...
    function excludePresale(address account) external onlyOwner {
        ...
        emit ExcludedFromMaxWalletLimit(account, true);
        emit ExcludedFromMaxTransactionLimit(account, true);
    }
    ...
    function updateBuyFees(uint256 _rewardFeeOnBuy,uint256 _marketingFeeOnBuy,uint256 _liquidityFeeOnBuy,uint256 _burnTaxFeeOnBuy) external onlyOwner {
        require(_rewardFeeOnBuy + _marketingFeeOnBuy + _liquidityFeeOnBuy + _burnTaxFeeOnBuy <= 150, "Fees must be equal or less than 15%");
        rewardFeeOnBuy = _rewardFeeOnBuy;
        marketingFeeOnBuy = _marketingFeeOnBuy;
        liquidityFeeOnBuy = _liquidityFeeOnBuy;
        burnTaxFeeOnBuy = _burnTaxFeeOnBuy;
        totalBuyFee = _rewardFeeOnBuy + _marketingFeeOnBuy + _liquidityFeeOnBuy + _burnTaxFeeOnBuy;
        emit FeesBuyUpdated( rewardFeeOnBuy, marketingFeeOnBuy, liquidityFeeOnBuy, burnTaxFeeOnBuy);
    }
    function updateSellFees(uint256 _rewardFeeOnSell, uint256 _marketingFeeOnSell, uint256 _liquidityFeeOnSell, uint256 _burnTaxFeeOnSell) external onlyOwner {
        require(_rewardFeeOnSell + _marketingFeeOnSell + _liquidityFeeOnSell + _burnTaxFeeOnSell <= 250, "Fees must be equal or less than 25%");
        rewardFeeOnSell = _rewardFeeOnSell;
        marketingFeeOnSell = _marketingFeeOnSell;
        liquidityFeeOnSell = _liquidityFeeOnSell;
        burnTaxFeeOnSell = _burnTaxFeeOnSell;
        totalSellFee = _rewardFeeOnSell + _marketingFeeOnSell + _liquidityFeeOnSell + _burnTaxFeeOnSell;
        emit FeesSellUpdated( rewardFeeOnSell, marketingFeeOnSell, liquidityFeeOnSell, burnTaxFeeOnSell);
    }
    function updateWalletToWalletFee(uint256 _walletToWalletFee) external onlyOwner{
        require(_walletToWalletFee <= 150, "Wallet to wallet fee must be 15% at max");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 burnTaxShare = burnTaxFeeOnBuy + burnTaxFeeOnSell;
            uint256 shares = totalBuyFee + totalSellFee - burnTaxShare;
            uint256 liquidityTokens;
            if(liquidityShare > 0)
            {
                liquidityTokens = (liquidityShare * contractTokenBalance) / (shares * 2);
            ...
            if(liquidityShare > 0)
            {   
                liquidityBNB = (newBalance * liquidityShare) / (shares * 2);
                uniswapV2Router.addLiquidityETH{value: liquidityBNB}(
                address(this),
                liquidityTokens,
                ...
                emit SwapAndLiquify(liquidityTokens, liquidityBNB, liquidityTokens);
            }
            contractTokenBalance = balanceOf(address(this));
            if(contractTokenBalance> 1e18)
            {
            path[0] = address(this);
            path[1] = uniswapV2Router.WETH();
            uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            shares = marketingShare + rewardShare;
            uint256 marketingBNB;
            if(marketingShare > 0) {
                marketingBNB = (newBalance * marketingShare) / shares;
                sendBNB(payable(marketingWallet), marketingBNB);
                emit SendMarketing(marketingBNB);
            }
            uint256 rewardBNB;
            if(rewardShare > 0) {
                rewardBNB = newBalance - marketingBNB;
        ...
        if(takeFee) {
            uint256 _totalFees;
            uint256 burnTaxShare;
            if(from == uniswapV2Pair) {
                _totalFees = totalBuyFee;
                burnTaxShare = burnTaxFeeOnBuy;
            } else if(to == uniswapV2Pair) {
                _totalFees = totalSellFee;
                burnTaxShare = burnTaxFeeOnSell;
            }
            else{
    ...
    function setMaxWalletAmount(uint256 _maxWalletAmount) external onlyOwner {
        require(
            _maxWalletAmount >= ((totalSupply()*2) / (10 ** decimals())) / 100, 
    ...
    event MaxTransactionLimitAmountChanged(uint256 maxTransactionAmountSell);
    ...
    function setMaxTransactionAmountSell(uint256 _maxTransactionAmountSell) external onlyOwner {
        require(
            _maxTransactionAmountSell  * (10 ** decimals())  >= maxTransactionAmountSell, "Max transaction amount sell cannot be lower than current amount"
        ); 
        maxTransactionAmountSell = _maxTransactionAmountSell * (10 ** decimals());
        emit MaxTransactionLimitAmountChanged(maxTransactionAmountSell);
    }
    ...
    function includeFromRewards(address account) external onlyOwner{
        dividendTracker.includeFromDividends(account,balanceOf(account));
    }
    function isExcludedFromRewards(address account) public view returns(bool) {
        return dividendTracker.isExcludedFromRewards(account);
    }
