...
contract staking is Ownable {
    ...
    mapping (address=>userbonus) public userbonusInfo;
    ...
    constructor() Ownable(0x778E05eDCFb8F0F8904517F3c2c8AA09389a0229){  // paste owner address in ownable
   ...
   function setBonusWithdrawUsdLimit(uint256 _usd) public onlyOwner{
    ...
    function invest(address reffer) public payable {
        ...
        if(!checkExitsAddress(msg.sender)){
            userbonus storage bonusUsers = userbonusInfo[msg.sender]; 
            bonusUsers.amount = usdToBnbAmount(bonusUsd);
            bonusUsers.checkpoint = block.timestamp;
            bonusUsers.withdraw = false;
    ...
    function withdrawBonusTokens() public {        
        userbonus storage users = userbonusInfo[msg.sender];  
        require(users.withdraw == false,"You already withdraw your bonus");
        require(users.amount > 0,"You dont have invested yet");
        uint256 totalRewards=calclulateBonusReward(msg.sender);
        require(totalRewards>0,"You don't have sufficient Rewards to withdraw");
        require(totalRewards<=getContractBNBBalacne(),"Not Enough Balance for withdrwal from contract please try after some time");                        
        ...
        if(bnbToUsdAmount(totalRewards) >= bonusWithdrawUsdLimit){
            ...
            revert("Your rewards is less then to bonus limit");
        }     
       
        emit Withdrawn(msg.sender, totalRewards);        
    }    
    ...
    function calclulateBonusReward(address _user) public view returns(uint256){
        uint256 totalRewardToken;
        userbonus storage users = userbonusInfo[_user];                     
        uint256 depositeAmount = users.amount;
        uint256 time = block.timestamp.sub(users.checkpoint);
        totalRewardToken += depositeAmount.mul(rewardPercentage).div(divider).mul(time).div(1 days).div(365);                    
        return(totalRewardToken);
    }    
