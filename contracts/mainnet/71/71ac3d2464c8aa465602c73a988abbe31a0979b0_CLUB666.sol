...
contract CLUB666 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    CLUB666DividendTracker public dividendTracker;
    ...
    constructor() ERC20("CLUB666", "666") {
        marketingWallet = payable(0x019A96532618873297359A76cc3949D967596B37);
        devWallet = payable(0xBe9A5D8DB2bb592A2A86eA0c7fA430DCF4Bd333a);
        ...
        dividendTracker = new CLUB666DividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (666) * (10**18);
    ...
    function updateFees(
        ...
        require(totalSellFees <= 15 && totalBuyFees <= 6, "total fees cannot exceed 15% sell or buy");
...
contract CLUB666DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    CLUB666 public CLUB666Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        CLUB666Contract = CLUB666(mainContract);
    ...
    function _transfer(
        ...
        require(false, "CLUB666_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "CLUB666_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CLUB666 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(CLUB666Contract);

            uint256 prevBalance = CLUB666Contract.balanceOf(address(this));
            ...
            {
                uint256 received = CLUB666Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    CLUB666Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
