...
interface CommunityEconomyNFT {
    function balanceOf(address account) external view returns (uint256);
    function balanceInUse(address account) external view returns (uint256);
}

contract CommunityEconomy is IBEP20, Auth {
    ...
    string constant _name = "CommunityEconomy";
    string constant _symbol = "CE";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 95_888 * (10 ** _decimals);
    ...
    uint256 buyTeamFee = 400;

    uint256 sellReflectionFee = 300;
    uint256 sellMarketFee = 300;
    uint256 sellTeamFee = 400;

    uint256 feeDenominator = 10000;

    address public marketFeeReceiver = 0xEa369EA3e88da5Ea89444354A081a7026ba01D49;
    address public teamFeeReceiver = 0xf09d6A22eED963E524A903B0b49D7205fb533571;//team address
    address public communityGrowthFeeReceiver = 0x4629735EdfEE8ad817434d83Ec276c0Bbc33B7F6;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (_launch != true && ((!isFeeExempt[sender] && recipient == pair) || (!isFeeExempt[recipient] && sender == pair))) {
            require(_launch == true, "CommunityEconomy hasn't launched yet!");
            return false;
        }
    ...
    function getFees(bool selling, address user) public view returns (uint256, uint256, uint256) {
        ...
        if(nftSwitch == true && CommunityEconomyNFT(NFTAddress).balanceInUse(user) > 0){
            marketFee = 0;
            teamFee = 0;
            if(selling){
        ...
        return (reflectionFee, marketFee, teamFee);
    }

    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        address user;
        if(sender == pair){user=recipient;}else{user=sender;}
        (uint256 reflectionFee, uint256 marketFee, uint256 teamFee) = getFees(recipient == pair, user);
        _balances[marketFeeReceiver] = _balances[marketFeeReceiver].add(amount.mul(marketFee).div(feeDenominator));
        _balances[teamFeeReceiver] = _balances[teamFeeReceiver].add(amount.mul(teamFee).div(feeDenominator));
        ...
        uint256 totalAmount = amount.mul(reflectionFee.add(marketFee).add(teamFee)).div(feeDenominator);
        
        emit Transfer(sender, marketFeeReceiver, amount.mul(marketFee).div(feeDenominator));
        emit Transfer(sender, teamFeeReceiver, amount.mul(teamFee).div(feeDenominator));
    ...
    function launch() external authorized {
        require(_launch != true, "CommunityEconomy has launched!");
        _launch = true;
    }
