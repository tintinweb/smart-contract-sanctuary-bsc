...
contract Therabbit is ERC20 {
	...
	address private _fundAddress = address(0xf1645ff96E81A5167d582E02D8Ed3365e6933F30);
    ...
    constructor(address tokenOwner) ERC20("The rabbit", "The rabbit") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(this), address(_baseToken));
		uint256 total = 100000000 * 10**25;
    ...
    function changeFeeRate(uint256 _feeRate) public onlyOwner {
        require(_feeRate > 3 && _feeRate < 100,"max or min");
        feeRate = _feeRate;
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				require(amount <= 10**22,"max amount");
        ...
        if (takeFee) {
			...
			if(feeRate > 3){
				super._transfer(from, address(this), amount.div(100).mul(feeRate - 3));
			}
	...
	function swapAndLiquify() public {
		...
		if(allAmount > canswap.add(10**19) && canswap > 10**19){
			ldxOverAmount = ldxOverAmount.add(canswap);
			swapTokensForOther(canswap.div(2));
			uint256 ethBalance = ETH.balanceOf(address(this));
			if(ethBalance > 10**12){
				addLiquidityUsdt(ethBalance, canswap.div(2));
			}
		}
		
		allAmount = balanceOf(address(this));
		if(allAmount > 10**19){
			swapTokensForFund(allAmount);
		}
    }
	...
	function swapTokensForFund(uint256 tokenAmount) public {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
