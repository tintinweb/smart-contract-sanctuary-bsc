...
import './XENCrypto/XENCrypto.sol';
import './XENFTHolder.sol';
...
contract Traits {
  ...
  uint256 public constant MAX_REWARD_COLOR = 1e8 ether;

  XENCrypto public immutable xen;

  constructor(XENCrypto xen_) {
    xen = xen_;
  }

  function getHSL(uint256 amount) internal pure returns (uint256) {
    if (amount > MAX_REWARD_COLOR) return 359;
    return max((359 * amount) / MAX_REWARD_COLOR, 25);
  }

  function getDay(uint256 ts, uint256 timeNow) internal pure returns (uint256) {
    if (timeNow > ts) return 360;
    uint256 res = (ts - timeNow) / SECONDS_IN_DAY;
    if (res > 359) res = 1;
    return 360 - res;
  }

  function drawSVG(
    XENCrypto.MintInfo memory mintInfo,
    XENCrypto.StakeInfo memory stakeInfo,
    uint256 stakeReward,
    uint256 mintAward
  ) public view returns (string memory) {
    uint256 timeNow = block.timestamp;
    uint256 mintClose = getDay(mintInfo.maturityTs, timeNow);
    uint256 stakeClose = getDay(stakeInfo.maturityTs, timeNow);
    string memory mintStr = mintAward > 0
      ? string(
        abi.encodePacked(
          '<circle r="300" cx="320" cy="320" fill="transparent" stroke="hsl(',
          getHSL(mintAward).toString(),
          'deg 68% 46%)" stroke-width="600" stroke-dasharray="calc(600 * 3.1415926)" stroke-dashoffset="calc((600 * 3.1415926) / 360 * (360 - ',
          mintClose.toString(),
          '))"/>'
        )
      )
      : '';
    string memory stakeStr = stakeReward > 0
      ? string(
        abi.encodePacked(
          '<circle r="90" cx="320" cy="320" fill="transparent" stroke="hsl(',
          getHSL(stakeReward).toString(),
          'deg 68% 46%)" stroke-width="180" stroke-dasharray="calc(180 * 3.1415926)" stroke-dashoffset="calc((180 * 3.1415926) / 360 * (360 - ',
          stakeClose.toString(),
          '))"/>'
        )
      )
    ...
    return
      string(
        abi.encodePacked(
          '<svg id="XENFT" width="100%" height="100%" version="1.1" viewBox="0 0 640 640" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">',
          mintStr,
          '<circle r="180" cx="320" cy="320" fill="#fff" />',
          stakeStr,
          '<circle r="30" cx="320" cy="320" fill="#fff" />',
          '</svg>'
        )
      );
  }
  ...
  function _calculateMintReward(
    ...
    uint256 rankDelta = max(xen.globalRank() - cRank, 2);
    uint256 EAA = (1_000 + eeaRate);
    uint256 reward = xen.getGrossReward(rankDelta, amplifier, term, EAA);
  ...
  function compileAttributes(
    ...
    string memory traits = attributeForTypeAndValue('contract', address(owner).toHexString());
    if (mintInfo.rank > 0) {
      traits = string(
        abi.encodePacked(
          traits,
          ',',
          attributeForTypeAndValue('mint:reward', mintAward.toString()),
          ',',
          attributeForTypeAndValue('mint:term', mintInfo.term.toString()),
          ',',
          attributeForTypeAndValue('mint:maturityTs', mintInfo.maturityTs.toString()),
          ',',
          attributeForTypeAndValue('mint:rank', mintInfo.rank.toString()),
          ',',
          attributeForTypeAndValue('mint:amplifier', mintInfo.amplifier.toString()),
          ',',
          attributeForTypeAndValue('mint:eaaRate', mintInfo.eaaRate.toString())
        )
      );
    }
    ...
    if (stakeInfo.amount > 0) {
      traits = string(
        abi.encodePacked(
          traits,
          mintInfo.rank > 0 ? ',' : '',
          attributeForTypeAndValue('stake:reward', stakeReward.toString()),
          ',',
          attributeForTypeAndValue('stake:term', stakeInfo.term.toString()),
          ',',
          attributeForTypeAndValue('stake:maturityTs', stakeInfo.maturityTs.toString()),
          ',',
          attributeForTypeAndValue('stake:amount', stakeInfo.amount.toString()),
          ',',
          attributeForTypeAndValue('stake:apy', stakeInfo.apy.toString())
        )
      );
    }
  ...
  function tokenURI(uint256 tokenId, XENFTHolder owner) public view returns (string memory) {
    XENCrypto.MintInfo memory mintInfo = owner.getUserMint();
    XENCrypto.StakeInfo memory stakeInfo = owner.getUserStake();
    uint256 stakeReward;
    uint256 mintAward;
    if (mintInfo.rank > 0) {
      mintAward = _calculateMintReward(mintInfo.rank, mintInfo.term, mintInfo.maturityTs, mintInfo.amplifier, mintInfo.eaaRate);
    }
    if (stakeInfo.amount > 0) {
      stakeReward = (stakeInfo.amount + _calculateStakeReward(stakeInfo.amount, stakeInfo.term, stakeInfo.apy)) / (1 ether);
    }
    string memory metadata = string(
      abi.encodePacked(
        '{"name": "XEN #',
        tokenId.toString(),
        '", "description": "This NFT holds XEN for a certain period of time. Do not buy NFT that is about to expire or has expired, because people will withdraw its value before you get it. More information: https://XENFT.github.io", "image": "data:image/svg+xml;base64,',
        base64(bytes(drawSVG(mintInfo, stakeInfo, stakeReward, mintAward))),
        '", "attributes":',
        compileAttributes(owner, mintInfo, stakeInfo, stakeReward, mintAward),
        '}'
      )
    );
...
contract XENFTHolder is Initializable {
  XENFT public XENFTContract;
  ...
  function owner() public view returns (address) {
    return XENFTContract.ownerOf(tokenId);
  }
  ...
  function claimRank(uint256 term) external onlyOwner {
    xen.claimRank(term);
  }

  function claimMintReward() external onlyOwner {
    xen.claimMintReward();
  }

  function claimMintRewardAndShare(address other, uint256 pct) external onlyOwner {
    xen.claimMintRewardAndShare(other, pct);
  }

  function claimMintRewardAndStake(uint256 pct, uint256 term) external onlyOwner {
    xen.claimMintRewardAndStake(pct, term);
  }

  function stake(uint256 amount, uint256 term) external onlyOwner {
  ...
  function withdrawAnyToken(IERC20 token, uint256 amount) external onlyOwner {
...
contract XENFT is ERC721Enumerable {
  Traits public immutable traits;
  XENCrypto public immutable xen;
  address public immutable proxy;

  uint256 public id = 1;
  mapping(uint256 => XENFTHolder) public holders;

  constructor(
    XENCrypto xen_,
    Traits traits_,
    address proxy_
  ) ERC721('the NFT of XENCrypto', 'XENFT') {
    xen = xen_;
    traits = traits_;
    proxy = proxy_;
  }
  ...
  function mint(uint256 term) public {
    _mint(msg.sender, id);
    XENFTHolder holder = holders[id] = XENFTHolder(Clones.clone(proxy));
    holder.initialize(this, id, term, xen);
    id++;
  }

  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    _requireMinted(tokenId);
    return traits.tokenURI(tokenId, XENFTHolder(holders[tokenId]));
  }
}
