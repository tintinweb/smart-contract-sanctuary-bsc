...
contract BUSDOGE is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    BUSDOGEDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0xB5F9CA92A0542815095F72D7c9E60586546E1FE7);
    address payable public _buybackWalletAddress = payable(0xf2b4f285E862Dc74D8c499bF343566eE2872437F);
    ...
    constructor() BEP20("BUS DOGE", "BDG") 
    {
        dividendTracker = new BUSDOGEDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "BUSDOGE: The dividend tracker already has that address");
        BUSDOGEDividendTracker newDividendTracker = BUSDOGEDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "BUSDOGE: The new dividend tracker must be owned by the BUSDOGE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BUSDOGE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BUSDOGE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BUSDOGE: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BUSDOGE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BUSDOGE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BUSDOGE: Cannot update gasForProcessing to same value");
...
contract BUSDOGEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BUSDOGE_Dividend_Tracker", "BUSDOGE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BUSDOGE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BUSDOGE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BUSDOGE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BUSDOGE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BUSDOGE_Dividend_Tracker: Cannot update claimWait to same value");
