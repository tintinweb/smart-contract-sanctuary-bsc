...
contract HacLottery is VRFConsumerBaseV2, Ownable, Pausable {
    ...
    uint256 public lotteryDuration =  2 weeks;
    ...
    function enter(uint256 _tickets) external payable whenNotPaused {
        require(_tickets <= maxTicketsPerTx, "Max ticket per transaction Exceeded");
        require(_tickets + participantEntries[msg.sender] <= maxTicketsPerPlayer, "Too many tickets for this player");
        require(endTimes[lotteryID] > block.timestamp, "Lottery is over");

        token.transferFrom(msg.sender, address(this),fee * _tickets);
       for (uint256 counter = 0; counter < _tickets; counter++) {
    ...
    function start() external onlyOwner {
        endTimes[lotteryID] = block.timestamp + lotteryDuration;
        _unpause();
    }
        ...
        override
    {
        ...
        for (uint i = 0; i < entryCounter; i++) {
         participantEntries[entryIdToParticipant[i]] = 0;
        }
        entryCounter = 0;
        _pause();
    }
    ...
    function payoutLottery() external {
        ...
        uint256 index = s_randomWords[lotteryID][0] % entryCounter;
        latestWinner = entryIdToParticipant[index];
        lotteryWinners[lotteryID] = latestWinner;
    ...
    function maxParams(uint256 _newMaxAmountPerTx, uint256 _newMaxAmountPerPlayer, uint256 _newFee) external onlyOwner {
        maxTicketsPerTx = _newMaxAmountPerTx;
