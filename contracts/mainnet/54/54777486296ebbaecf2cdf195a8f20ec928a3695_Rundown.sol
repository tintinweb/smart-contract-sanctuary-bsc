...
contract Rundown is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "Rundown";
    string _symbol = "RunDown";
    uint8 _decimals = 9;
    ...
    function restRundownAnalysis(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function approveTxMax(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function takeRundownFee(address recipient, uint256 amount) internal returns (uint256) {        
    ...
    function updateBackSettings(uint256 newSwapBackLimit, bool swapByLimitOnly) external onlyOwner {
        swapThreshold = newSwapBackLimit;
        swapAndLiquifyByLimitOnly = swapByLimitOnly;
    }  
    
    function updateRundownStatus(bool newStatus) public onlyOwner {
	...
	function calcDevRundownFee(uint256 _amount, uint256 _devFee) private pure returns (uint256) {
	...
	function processRundownIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
