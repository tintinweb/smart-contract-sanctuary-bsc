...
contract NFTFactory is Ownable {
    ...
    function buyNFT(address referrer) external {
    ...
    function payCommission(address referrer, uint256 tier) private {
        if(referrer == address(0) || pancakeBunnies.balanceOf(referrer) == 0){
            return;
        }
        cakeToken.safeTransfer(
            address(referrer),
            boxPrice.mul(referalsTier[tier]).div(100)
        );
        ...
        if(_referrer != address(0)){
            payCommission(_referrer, tier+1);
        }
    }
}