...
import "./interfaces/IaxBNB_Direct.sol";
import "./openzeppelin/TokensRecoverable.sol";
import "./openzeppelin/Owned.sol";
import "./libraries/UniswapV2LibraryPCS.sol";
import "./interfaces/IUniswapV2Factory.sol";
import "./interfaces/IMagicTransferGate.sol";
import "./interfaces/IUniswapV2Router02.sol";
import "./interfaces/IERC31337.sol";
import "./interfaces/IWizardZapper.sol";
import "./openzeppelin/ReentrancyGuard.sol";
import "./interfaces/IERC1155.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/IUniswapV2Pair.sol";

// zapper contract for Vein-Magic LP => Wyrmsoul
contract WyrmsoulZapper is Owned, TokensRecoverable, ReentrancyGuard
{
    ...
    constructor(IAxBNB _axBNB, IERC20 _magic, address _LPAddress, IERC20 _Vein, IaxBNB_Direct _axBNBDirect, IWizardZapper _wizardDirect, IERC31337 _LPNFTWrapper, IERC1155 _ERC1155Token, uint _erc1155TokenID)
    {
        axBNB = _axBNB;
        Vein = _Vein;
        axBNBDirect = _axBNBDirect;
        wizardDirect = _wizardDirect;
        ...
        _axBNB.approve(address(_wizardDirect), uint256(-1));
        _magic.approve(address(_wizardDirect), uint256(-1));
        _Vein.approve(address(_wizardDirect), uint256(-1));
        
        _axBNB.approve(address(_axBNBDirect), uint256(-1));
        _magic.approve(address(_axBNBDirect), uint256(-1));

        _magic.approve(address(uniswapV2Router), uint256(-1));
        _Vein.approve(address(uniswapV2Router), uint256(-1));
    ...
    function easyBuyNFT() external payable nonReentrant returns (uint256)
    {
        ...
        uint256 prevVein = Vein.balanceOf(address(this));

        //swap half BNB to Vein    
        uint256 tBNB=SafeMath.div(msg.value,2);
        wizardDirect.easyBuy{ value: tBNB }();

        uint256 veinAmt = Vein.balanceOf(address(this)).sub(prevVein);
        axBNBDirect.easyBuy{ value: tBNB }();
        uint256 magicAmt = magic.balanceOf(address(this)).sub(prevmagicAmount);

        uint256 prevLPBalance = IERC20(LPAddress).balanceOf(address(this));

        uniswapV2Router.addLiquidity(address(Vein), address(magic), veinAmt, magicAmt, 1, 1, address(this), block.timestamp+60);
        ...
        if(Vein.balanceOf(address(this))>prevVein)
            Vein.transfer(msg.sender, Vein.balanceOf(address(this)).sub(prevVein));
    ...
    //  axBNB => NFT
    function easyBuyNFTFromAXBNB(uint256 axBNBAmt) public nonReentrant returns (uint256)
    {
        ...
        //swap half axBNB to Vein    
        uint256 axBNBForBuy = axBNBAmt.div(2);
        axBNBDirect.easyBuyFromAXBNB(axBNBForBuy);
        uint256 magicAmt = magic.balanceOf(address(this)).sub(prevmagicAmount);

        wizardDirect.easyBuyFromAXBNB(axBNBForBuy);
        uint256 veinAmt = Vein.balanceOf(address(this)).sub(prevVein);
        ...
        ERC1155Token.safeTransferFrom(address(this), msg.sender, erc1155TokenID, mints, "0x");
    ...
    //  magic => NFT
    function easyBuyNFTFromMagic(uint256 magicAmt) external nonReentrant returns (uint256)
    {
        uint256 prevAxBNBAmount = axBNB.balanceOf(address(this));
        uint256 prevmagicAmount = magic.balanceOf(address(this));
        uint256 prevVein = Vein.balanceOf(address(this));
        ...
        //swap half magic to Vein    
        uint256 magicForBuy = magicAmt.div(2);

        wizardDirect.easyBuyFromMagic(magicForBuy);
        uint256 veinAmt = Vein.balanceOf(address(this)).sub(prevVein);
        uint256 prevLPBalance = IERC20(LPAddress).balanceOf(address(this));
    ...
    //  axBNB => NFT
    function easyBuyNFTFromVein(uint256 veinAmt) public nonReentrant returns (uint256)
    {
        uint256 prevAxBNBAmount = axBNB.balanceOf(address(this));
        uint256 prevmagicAmount = magic.balanceOf(address(this));
        uint256 prevVein = Vein.balanceOf(address(this));

        Vein.transferFrom(msg.sender,address(this),veinAmt);

        //swap half Vein to magic    
        uint256 VeinForBuy = veinAmt.div(2);
        wizardDirect.easySellToMagic(VeinForBuy);
        uint256 magicAmt = magic.balanceOf(address(this)).sub(prevmagicAmount);
...
interface IWizardZapper
{
    ...
    function easyBuyFromAXBNB(uint256 axBNBIn) external;
    function easyBuyFromMagic(uint magicIn) external ;

    function easySell(uint256 wiz) external;
    function easySellToAXBNB(uint256 wiz) external;
    function easySellToMagic(uint256 wiz) external;
