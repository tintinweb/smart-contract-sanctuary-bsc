...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = address(0xCaE8fB729FFBa7F498cBAE0920BBa77E95340477);
...
contract GORKHA is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet =
        payable(0x9555F504B53ef078Ac32BcBCdD4c8004F250222f);
    ...
    string private constant _name = "GORKHA";
    ...
    uint256 private _MarketingFee = 150;
    uint256 private _previousMarketingFee = _MarketingFee; 

    uint256 private _burnFee = 150; 
    uint256 private _previousBurnFee = _burnFee;

    uint256 internal feeDenominator = 10_000;
    ...
    uint256 public MaxTaxLimit = 25 * 10**3 * 10**9;
    uint256 public MaxWalletHoldingLimit = 25 * 10**3 * 10**9;

    event BuyTaxFeeUpdated(uint256 feeUpdated);
    event SellTaxFeeUpdated(uint256 feeUpdated);
    event MaxWalletHoldingAmountUpdated(uint256 _maxWalletHoldingLimit);
    ...
    function UpdateBuyAndSellTaxFeePercentage(
        uint256 taxFee,
        ...
        require(
            taxFee.add(liquidityFee).add(MarketingFee).add(BurnFee) <=
                _feeDenominator.div(5),
    ...
    {
        require(
            maxWalletPercentage * (10**_decimals) <= 25 * 10**3 * (10**_decimals),
            "You can't set less than 25k"
        );
    ...
    function UpdateMaxTaxAmount(uint256 maxTxAmount) public onlyOwner {
        require(
            maxTxAmount * (10**_decimals) <= 25 * 10**3 * (10**_decimals),
            "You can't set less than 25k"
        );
        MaxTaxLimit = maxTxAmount * (10**_decimals);

        emit MaxTxAmountUpdated(MaxTaxLimit);
    }
    ...
    function _transfer(
        ...
        if (balanceOf(DeadWallet) >= _tTotal.mul(20).div(10**2)) {
            _burnFee = 0;
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_liquidityFee.add(_MarketingFee) == 0) {
            return;
        }
    ...
    function _tokenTransfer(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            require(amount <= MaxTaxLimit, "Max Tx Exceeded");
        }
