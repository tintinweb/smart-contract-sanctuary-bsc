...
contract TestERC20Farm is Ownable, ReentrancyGuard, IERC20Farm{
    ...
    struct UserInfo {
        uint256 shares; // How many shares the user has for staking
        uint256 lastAccTokenPerShare; // Reward debt
        uint256 depositBlock; // Reward debt
    }
...
//        defaultStakePPS = 10 ** (decimalsStakeToken - decimalsStakeToken/2);
    ...
    function _deposit(uint256 amount, address account) internal nonReentrant {
        ...
        uint256 pending = 0;
        if (user.shares > 0) {
            pending = user.shares * (accTokenPerShare - user.lastAccTokenPerShare) / PRECISION_FACTOR;
            if (pending > 0) {
                rewardToken.transfer(account, pending);
            }
            totalPendingReward -= pending;
        ...
        user.lastAccTokenPerShare = accTokenPerShare;
        user.depositBlock = block.number;
        lastRewardTokenBalance = rewardToken.balanceOf(address(this));
    ...
    function withdraw(uint256 _shares) external nonReentrant {
        ...
        uint256 pending = user.shares * (accTokenPerShare - user.lastAccTokenPerShare) / PRECISION_FACTOR;
        uint256 transferAmount = _shares * pricePerShare();
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakeTotalShares != 0) {
            ...
            uint256 adjustedTokenPerShare = accTokenPerShare +
                cakeReward * PRECISION_FACTOR / stakeTotalShares;
            return user.shares * (adjustedTokenPerShare - user.lastAccTokenPerShare) / PRECISION_FACTOR;
        } else {
            return user.shares * (accTokenPerShare - user.lastAccTokenPerShare) / PRECISION_FACTOR;
        }
    }
...
//SPDX-License-Identifier: MIT
pragma solidity 0.8.11;
import "./TestERC20Farm.sol";
import "../IFarmDeployer.sol";

contract TestFarmDeployer20 is IFarmDeployer20 {
    ...
    function deploy(
        ...
        farmAddress = address(new TestERC20Farm());
        IERC20Farm(farmAddress).initialize(
            _stakeToken,
            _rewardToken,
            _startBlock,
