...
contract Stakeable {
    ...
    */
    function _stake(uint256 _amount, uint256 _duration) internal returns(uint256){
        ...
        stakeholders[index].address_stakes.push(Stake(msg.sender, _amount,block.timestamp, _duration, stakeOptions[_duration].rewardRate, stakeOptions[_duration].penaltyRate));
    ...
    function calcMyReward(uint256 stakeID) public view returns(uint256) {
        require(stakes[msg.sender] > 0, "TOKO: Stakeholder not found");
        require(stakeholders[stakes[msg.sender]].address_stakes[stakeID].rewardRate > 0, "TOKO: Stake not found");
...
contract OnLearnyToken is ERC20, ERC20Burnable, Pausable, AccessControl, Claimable, Stakeable{
    ...
    uint256 public constant RELEASE_TIME = 1670847132;
    ...
    constructor() ERC20("OnLearny Token", "TOKO"){
        ...
        _mint(0xeD76bA5D4bC71842F70B0d45b37747c78271fCa5, 30000000 * 10 ** 18);
        _mint(0x80b1d8184CcbA38c0F4e3dcfe8330BC81eb143C8, 30000000 * 10 ** 18);
        
        _pause();
        
    }

    function viewSupplyLimit() public view returns(uint256){
        return totalSupply() + UNCLAIMED_AMOUNT;
    }
    ...
    function addClaim(address forAddr, uint256 amount) public onlyRole(PLATFORM_ROLE){
    ...
    function removeClaim(address forAddr) public onlyRole(PLATFORM_ROLE){
    ...
    /*Raise UNCLAIMED AMOUNT in emergency situation*/
    ...
    function mints(address[] memory tos, uint256[] memory amounts) public onlyRole(MINTER_ROLE) {
        require(tos.length == amounts.length, "TOKO: array length not match");
        for(uint256 i=0; i < tos.length; i++){
            if(UNCLAIMED_AMOUNT < amounts[i]){
                break;        
            }
            UNCLAIMED_AMOUNT -= amounts[i];
    ...
    function _withdrawStake(address stakerAddr, uint256 stakeID, bool safeMode) internal{
        require(stakes[stakerAddr] > 0, "TOKO: Stakeholder not found");
        require(stakeholders[stakes[stakerAddr]].address_stakes[stakeID].rewardRate > 0, "TOKO: Stake not found");
        if(safeMode){
        ...
        uint256 reward =  _calcReward(stakerAddr, stakeID);
        uint256 wdAmount = stakeholders[stakes[stakerAddr]].address_stakes[stakeID].amount + reward;
        if(reward == 0){
            wdAmount -= _calcPenalty(stakerAddr, stakeID);
        }
        _mint(stakerAddr, wdAmount);
        delete stakeholders[stakes[stakerAddr]].address_stakes[stakeID];
    }

    function withdrawMyStake(uint256 stakeID, bool safeMode) public{
        _withdrawStake(msg.sender, stakeID, safeMode);
    }
