...
interface BinanceBNB {
     ...
     *
     * Returns a boolean value indiLXTOing whBNBer the operation succeeded.
     *
     ...
     * that someLXTO may use both the old and the new allowance by unLXTOunate
     * transaction ordering. LXTO possible solution to mitigate this race
    ...
    /**
     * @dev Emitted when `value` tokens are moved from LXTO account (`from`) to
     * another (`to`).
     *
...
library SafeMath {
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMath: multipliLXTOion overflow");
...
contract OwnableLXTO is ContextLXTO {
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableLXTO: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableLXTO: new owner is the zero address");
        emit OwnershipTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract LuxTTO is ContextLXTO, BinanceBNB, OwnableLXTO {
    ...
    string private _name = 'LuxTTO';
    string private _symbol = 'LXTO';
