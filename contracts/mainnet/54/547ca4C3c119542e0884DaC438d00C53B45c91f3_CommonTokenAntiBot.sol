...
contract CommonTokenAntiBot is IERC20, Ownable {
    ...
    constructor(
        address owner_,
        string[2] memory name_symbol_,
        uint8 decimals_,
        uint256[2] memory initSet, // initialSupply_, totalSupply_
        address whaleAntiBotAddress, // owner_, whaleAntiBotAddress
        uint256 feeEnableAntiBot_
    ) {
        transferOwnership(owner_);
        _name = name_symbol_[0];
        _symbol = name_symbol_[1];
        _decimals = decimals_;
        _mint(owner_, initSet[0]);
