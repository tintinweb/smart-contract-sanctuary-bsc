...
contract SELFPool is Ownable, Pausable {
    ...
    IERC20 public immutable token; // SELF token.
    ...
    event NewVSELFContract(address VSELF);
    ...
    /**
     * @notice Constructor
     * @param _token: SELF token contract
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(SELFPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the SELF owner address or the operator address.
     */
    modifier onlyOperatorOrSELFOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or SELF owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onSELFPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.SELFAtLastUserAction;
    ...
    /**
     * @notice Unlock user SELF funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrSELFOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the SELF Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VSELF != address(0)) {
            IVSELF(VSELF).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.SELFAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the SELF Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VSELF != address(0)) {
            IVSELF(VSELF).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.SELFAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.SELFAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending SELF tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingSELF = masterchefV2.pendingSELF(SELFPoolPID, address(this));
        if (pendingSELF > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(SELFPoolPID, 0);
    ...
    /**
     * @notice Set VSELF Contract address
     * @dev Callable by the contract admin.
     */
    function setVSELFContract(address _VSELF) external onlyAdmin {
        require(_VSELF != address(0), "Cannot be zero address");
        VSELF = _VSELF;
        emit NewVSELFContract(VSELF);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the SELF Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingSELFRewards();
     ...
     * @return Returns total pending SELF rewards
     */
    function calculateTotalPendingSELFRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingSELF(SELFPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingSELFRewards()) * (1e18)) / totalShares);
    }
