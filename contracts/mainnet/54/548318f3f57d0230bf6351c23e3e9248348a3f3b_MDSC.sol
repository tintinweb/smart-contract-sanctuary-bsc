...
contract MDSC is IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
        _name = "Mediterranean Sea";
        _symbol = "MDSC";
        _decimals = 18;
        _tTotal = 10**7 * 10**_decimals;
        uint256 leaveAmount = 2100 * 10**_decimals;
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || destroyAmount >= _destroyMaxAmount){
    ...
    function _tokenTransferSell(
        ...
        uint256 rate;
        if (takeFee) {
            _takeInviterFeeSell(sender, recipient, tAmount, currentRate);
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(100),
    ...
    function _tokenTransferBuy(
        ...
        uint256 rate;
        if (takeFee) {
			_takeInviterFeeBuy(sender, recipient, tAmount, currentRate);
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(100),
