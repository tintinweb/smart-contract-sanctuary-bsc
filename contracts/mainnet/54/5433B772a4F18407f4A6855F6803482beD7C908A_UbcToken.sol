...
contract UbcToken is Ownable, IERC20Metadata {
    ...
    function _doDividendAndLP(uint256 forUsdt, uint256 forLP) private {
        ...
        IERC20(_usdt).transfer(_dividend1, dividendUsdtAmount);
        IERC20(_usdt).transfer(_dividend2, dividendUsdtAmount);
        IERC20(_usdt).transfer(_dividend3, dividendUsdtAmount);
    ...
    function _swapTokenForMbank(uint256 tokenAmount) private {
        ...
        path[0] = address(this);path[1] = _usdt;path[2] = _mbank;
        IPancakeRouter02(_router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount, 0, path, address(this), block.timestamp);
    }

    function _addLP(uint256 ubcAmount, uint256 usdtAmount) private {
        (uint amountA, uint amountB) = _addLiquidity(ubcAmount, usdtAmount, 0, 0);

        TransferHelper.safeTransfer(tokenA, _pair, amountA);
        TransferHelper.safeTransfer(tokenB, _pair, amountB);

        IPancakePair(_pair).mint(_receiverLP);
    }
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(_pair, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = PancakeLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'UbcToken: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = PancakeLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'UbcToken: INSUFFICIENT_A_AMOUNT');
    ...
    function setTokenAddress(address tokenA_, address tokenB_) public onlyOwner {
        tokenA = tokenA_;
        tokenB = tokenB_;
    }
