...
abstract contract TradableErc20 is ERC20 {
    ...
    function _transfer(
        ...
        if (!_inSwap && uniswapV2Pair != address(0) && to == uniswapV2Pair) {
            ...
            if(coolDownEnabled) {
                require((block.timestamp - _lastSell[from]) >= coolDownTime);
    ...
    function getSellTaxImpact(uint256 amount) internal view returns (uint256) {
        ...
        // additive tax, interval between 0-35%
        ...
        returns (uint256)
    {
        ...
        uint256 priceImpactTax = getSellTaxImpact(amount); // burn count

        amount -= devFee + priceImpactTax;
        _balances[account] -= devFee + priceImpactTax;        
        _balances[address(this)] += devFee + priceImpactTax;  
        emit Transfer(account, address(this), devFee + priceImpactTax);
        return amount;
    }
