...
contract DanTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DanTestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DanTest", "DT") {
        marketingWallet = payable(0x4119d18659C72884f5bB6FD3939Db00bDc84E216);
        devWallet = payable(0xD9F1d4e46061C9C2558e58E5E4e3CC54A1286c0E);
        moonshotWallet = payable(0x2dB5f9c05775AA6A0a2722438CB039f787d807A1);
        ...
        dividendTracker = new DanTestDividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(moonshotBuy <= 3 && moonshotSell <= 3, "Moonshot fees can't exceed 3%");
        buyDeadFees = deadBuy;
...
contract DanTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DanTest public DanTestContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DanTestContract = DanTest(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DanTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DanTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DanTest contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DanTestContract);

            uint256 prevBalance = DanTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = DanTestContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DanTestContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
