...
contract SweepingFunds is Recoverable {
    ...
    constructor(address _wbnb, address _master, address _collection) {
        WBNB = _wbnb;
        MASTER = _master;
    ...
    function fundSweep(uint256 price) external {
        ...
        IWETH(WBNB).deposit{value: price}();
        IWETH(WBNB).transfer(msg.sender, price);
    }

    function closeFunds(address payable receiver) external {
