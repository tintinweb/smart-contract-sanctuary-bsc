...
contract MITO is IBEP20, Auth {
    ...
    address MKT  = 0x910B7FF27083aAa87184703541DA0C38aEe462Ca;
    address CAMPAIGN = 0x910B7FF27083aAa87184703541DA0C38aEe462Ca;

    string constant _name = "Mito22";
    ...
    uint256 campaignFee       = 2;
    uint256 public totalFee = 13;
    uint256 feeDenominator  = 100;

    address public campaignFeeReceiver = CAMPAIGN;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBCampaign = amountBNB.mul(campaignFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool tmpCampaignSuccess,) = payable(campaignFeeReceiver).call{value: amountBNBCampaign, gas: 30000}("");

        // only to supress warning msg
        tmpSuccess = false;
        tmpCampaignSuccess = false;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _campaignFee, uint256 _marketingFee,  uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        campaignFee = _campaignFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_campaignFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _campaignFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        campaignFeeReceiver = _campaignFeeReceiver;
    }
