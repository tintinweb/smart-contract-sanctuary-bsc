...
////// 🌎 Website: https://ismarten.com/ismarten/
////// AUTOMATIC DIVIDEND PAID IN BUSD, With the auto-claim feature.
///// hold iSmarten and you receive BUSD automatically in your wallet.
...
abstract contract Ownable is Context {
    ...
    constructor () {
        ...
        _owner = 0x93B15F02B93B77aE956bFcad66439697603060C6;
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("iSmarten_Busd_Dividend_Tracker", "iSmarten_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1000000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "iSmarten_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "iSmarten_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main iSmarten contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "iSmarten_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "iSmarten_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract iSmarten is ERC20, Ownable, ERC20Burnable {
    using SafeMath for uint256;
    ...
    mapping (address => bool) public  ContractOwner;
    ...
    uint256 public maxWalletBalance = 1000000000 * (10**18);
       uint256 public maxTransaction = 1000000000 * (10**18);
   
    uint256 public swapTokensAtAmount = 10 * 10**6 * 10**18;
    ...
    uint256 public _busdDividendRewardsFeeBuy = 1;
    uint256 public _marketingFeeBuy = 5;
    uint256 public _LiquidityFeeBuy = 4;

    uint256 public _busdDividendRewardsFeeSell=1;
    ...
    constructor() ERC20("iSmarten", "iSmarten") {
    	...
    	marketingWallet = 0x93B15F02B93B77aE956bFcad66439697603060C6; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "iSmarten: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "iSmarten: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "iSmarten: The new dividend tracker must be owned by the iSmarten token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "iSmarten: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "iSmarten: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "iSmarten: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "iSmarten: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "iSmarten: Cannot update gasForProcessing to same value");
...
function UpdateSellFee(uint256 marketingFeeSell,uint256 LiquidityFeeSell,uint256 busdDividendRewardsFeeSell) external onlyOwner() {

   _marketingFeeSell=marketingFeeSell;  
   _LiquidityFeeSell=LiquidityFeeSell;
  _busdDividendRewardsFeeSell=busdDividendRewardsFeeSell;
    }
  
    
function UpdateBuyFee(uint256 marketingBuy,uint256 LiquidityFeeBuy,uint256 busdDividendRewardsFeeBuy) external onlyOwner() {
        _marketingFeeBuy = marketingBuy;
        _LiquidityFeeBuy=LiquidityFeeBuy;
        _busdDividendRewardsFeeBuy=busdDividendRewardsFeeBuy;
...
function Airdrop(address[] calldata addresses, uint256[] calldata amounts) external onlyOwner {
        ...
        for (uint i = 0; i < addresses.length; i++) {
            if(!automatedMarketMakerPairs[addresses[i]] && !automatedMarketMakerPairs[addresses[i]]) {
...
function mint(address to, uint256 amount) public onlyOwner {
 ...
 if(!tradingIsEnabled){ require(ContractOwner[from] || ContractOwner[to], "Trading not open yet."); }
    ...
    if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && tradingIsEnabled && !excludedAccount ){
            require(amount <= maxTransaction, 'transfer amount is exceeding maxTransaction');
        }
