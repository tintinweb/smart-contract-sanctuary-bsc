...
import "./interfaces/IJlpManager.sol";
...
contract JlpManager is ReentrancyGuard, Governable, IJlpManager {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant PRICE_PRECISION = 10**30;
    uint256 public constant USDJ_DECIMALS = 18;
    uint256 public constant JLP_PRECISION = 10**18;
    uint256 public constant MAX_COOLDOWN_DURATION = 48 hours;
    uint256 public constant BASIS_POINTS_DIVISOR = 10000;

    IVault public immutable vault;
    IShortsTracker public shortsTracker;
    address public immutable override usdj;
    address public immutable jlp;
    ...
    event SetInPrivateMode(bool active);
    event SetShortsTracker(address newShortsTracker);
    event SetShortsTrackerAveragePriceWeight(
        uint256 shortsTrackerAveragePriceWeight
    );
    ...
    function setShortsTracker(IShortsTracker _shortsTracker) external onlyGov {
        ...
        emit SetShortsTracker(address(_shortsTracker));
    }
    ...
    function setShortsTrackerAveragePriceWeight(
        ...
        require(
            ...
            "JlpManager: invalid weight"
        );
        shortsTrackerAveragePriceWeight = _shortsTrackerAveragePriceWeight;
        emit SetShortsTrackerAveragePriceWeight(
            _shortsTrackerAveragePriceWeight
        );
    }

    function setHandler(address _handler, bool _isActive) external onlyGov {
        require(_handler != address(0), "JlpManager: invalid _handler");
        ...
        onlyGov
    {
        require(
            ...
            "JlpManager: invalid _cooldownDuration"
        );
        cooldownDuration = _cooldownDuration;
    ...
    function addLiquidity(
        ...
        if (inPrivateMode) {
            revert("JlpManager: action not enabled");
        }
        return
            _addLiquidity(
                msg.sender,
    ...
    function getPrice(bool _maximise) external view returns (uint256) {
        ...
        uint256 supply = IERC20(jlp).totalSupply();
        return aum.mul(JLP_PRECISION).div(supply);
    }
    ...
    function getAumInUsdj(bool maximise)
        public
        view
        override
        returns (uint256)
    {
        ...
        return aum.mul(10**USDJ_DECIMALS).div(PRICE_PRECISION);
    }
    ...
    function _addLiquidity(
        ...
        require(_amount > 0, "JlpManager: invalid _amount");

        // calculate aum before buyUSDJ
        uint256 aumInUsdj = getAumInUsdj(true);
        uint256 jlpSupply = IERC20(jlp).totalSupply();
        ...
        uint256 usdjAmount = vault.buyUSDJ(_token, address(this));
        require(usdjAmount >= _minUsdj, "JlpManager: insufficient USDJ output");

        uint256 mintAmount = aumInUsdj == 0
            ? usdjAmount
            : usdjAmount.mul(jlpSupply).div(aumInUsdj);
        require(mintAmount >= _minJlp, "JlpManager: insufficient JLP output");

        IMintable(jlp).mint(_account, mintAmount);
    ...
    function _removeLiquidity(
        ...
        require(_jlpAmount > 0, "JlpManager: invalid _jlpAmount");
        if (!isHandler[_account]) {
            require(
                lastAddedAt[_account].add(cooldownDuration) <= block.timestamp,
                "JlpManager: cooldown duration not yet passed"
            );
        }

        // calculate aum before sellUSDJ
        uint256 aumInUsdj = getAumInUsdj(false);
        uint256 jlpSupply = IERC20(jlp).totalSupply();

        uint256 usdjAmount = _jlpAmount.mul(aumInUsdj).div(jlpSupply);
        uint256 usdjBalance = IERC20(usdj).balanceOf(address(this));
        if (usdjAmount > usdjBalance) {
            IUSDJ(usdj).mint(address(this), usdjAmount.sub(usdjBalance));
        }

        IMintable(jlp).burn(_account, _jlpAmount);

        IERC20(usdj).safeTransfer(address(vault), usdjAmount);
        uint256 amountOut = vault.sellUSDJ(_tokenOut, _receiver);
        require(amountOut >= _minOut, "JlpManager: insufficient output");
    ...
    function _validateHandler() private view {
        require(isHandler[msg.sender], "JlpManager: forbidden");
    }
}
