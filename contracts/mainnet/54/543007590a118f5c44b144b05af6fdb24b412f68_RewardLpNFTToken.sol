...
abstract contract PairManager is Context {
    address private _pairManager;

    event PairManagerTransferred(address indexed previousOwner, address indexed newOwner);

    /**
     * @dev Initializes the contract setting the deployer as the initial owner.
     */
    constructor() {
        _setPairManager(_msgSender());
    }

    /**
     * @dev Returns the address of the current owner.
     */
    function pairManager() public view virtual returns (address) {
        return _pairManager;
    }
    ...
    modifier onlyPairManager() {
        require(pairManager() == _msgSender(), "pairManager: caller is not the owner");
        _;
    }
     ...
     * thereby removing any functionality that is only available to the owner.
     */
    function renouncePairManager() public virtual onlyPairManager {
        _setPairManager(address(0));
    }
     ...
     * Can only be called by the current owner.
     */
    function transferPairManager(address newOwner) public virtual onlyPairManager {
        require(newOwner != address(0), "PairManager: new owner is the zero address");
        _setPairManager(newOwner);
    }

    function _setPairManager(address newOwner) private {
        address oldOwner = _pairManager;
        _pairManager = newOwner;
        emit PairManagerTransferred(oldOwner, newOwner);
    }
}
...
contract RewardLpNFTToken is ERC20, Ownable, PairManager {
    ...
    function _transfer(
        ...
        if (
            ...
            swapTokensForCurrency(contractTokenBalance);
