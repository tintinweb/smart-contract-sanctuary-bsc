...
contract JNFT is AccessControlUpgradeable, ERC1155SupplyUpgradeable
{
    ...
    function uri(uint256 _tokenId) public view override returns (string memory) {
        return string(abi.encodePacked(super.uri(_tokenId), Strings.toHexString(_tokenId, 31), ".json"));
    }

    function name() external pure returns (string memory) {
        return "JNFT Shared Storefront";
    }
...
contract Redeem is AccessControlUpgradeable
{
    using EnumerableSet for EnumerableSet.UintSet;

    mapping(uint256 => bool) public claimed;

    uint256[] public claimedTokenIds;
    ...
    JNFT public jnft;
    EnumerableSet.UintSet private acceptedTokenIds;
    ...
    function initialize(
        ...
        __AccessControl_init_unchained();
        __Redeem_init_unchained(_admin, _jnft);
    }

    function __Redeem_init_unchained(
         address _admin,
         JNFT _jnft
    ) internal initializer {
        _setupRole(DEFAULT_ADMIN_ROLE, _admin);
    ...
    function redeem(
        ...
        require(acceptedTokenIds.contains(_tokenId), "not accepted");
        _amount = jnft.balanceOf(msg.sender, _tokenId);
        require(_amount != 0, "no token");

        claimed[_tokenId] = true;
        claimedTokenIds.push(_tokenId);

        emit RedeemLog(msg.sender, _tokenId, _amount);
    }

    function addAcceptedTokenIds(
        uint256[] memory _tokenIds
    ) external onlyAdmin {
        for (uint256 i; i < _tokenIds.length; i++) {
            acceptedTokenIds.add(_tokenIds[i]);
        }
    }

    function removeAcceptedTokenIds(
        uint256[] memory _tokenIds
    ) external onlyAdmin {
        for (uint256 i; i < _tokenIds.length; i++) {
            acceptedTokenIds.remove(_tokenIds[i]);
        }
    }

    function isAcceptedTokenId(
        uint256 _tokenId
    ) external view returns (bool) {
        return acceptedTokenIds.contains(_tokenId);
    }

    function acceptedTokenIdsCount() external view returns (uint256) {
        return acceptedTokenIds.length();
    }

    function getAcceptedTokenId(
        uint256 _index
    ) external view returns (uint256) {
        return acceptedTokenIds.at(_index);
    }
