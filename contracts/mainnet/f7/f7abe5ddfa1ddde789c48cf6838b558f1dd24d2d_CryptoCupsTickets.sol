...
contract CryptoCupsTickets is ERC721A, Ownable {
    ...
    // Router
    Router router = Router(0x10ED43C718714eb63d5aA57B78B54704E256024E); // Mainnet
    // Router router = Router(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // Testnet
    ...
    mapping (uint => address) private referralAddresses;

    // Discounts
    uint public basicReferedDiscount = 4;
    uint public boostReferedDiscount = 7;

    // Fees
    uint public basicReferralFee = 2;
    uint public boostReferralFee = 3;
    ...
    // Owners Fees
    uint basicOwnersFees = 5;
    uint boostOwnersFees = 10;

    // Metadata
    string private _name = "Crypto Cup Tickets";
    ...
    constructor() ERC721A (_name, _symbol) {
        ...
        // Mint 150 Tickets for airdrop.
        _mint(msg.sender, 100, 0);
    ...
    event newPoolAddress(address);
    event newFeesCollector(address);

    event currencyRemoved(uint);
    event batchChanged(uint, uint);
    event newReferralValues(uint, uint, uint, uint);
    event newFees(uint, uint);
    event newBatchDeadline(uint);

    event newPauseStatus(bool);
    


    // Functions
    function mint(uint _amount, uint _currency, uint _type, uint _referralCode) public onlyOngoingBatch checkNotSelfReferral(_referralCode) unpaused {
        ...
        uint _unitPrice = (_type == 0? basicPrice : boostPrice) - (_discount? (_type == 0? basicReferedDiscount : boostReferedDiscount) : 0);
        ...
        require(_selectedCurrency.balanceOf(msg.sender) >= _totalPriceAmount, "Not Enough Balance to Pay.");
        require(_selectedCurrency.allowance(msg.sender, address(this)) >= _totalPriceAmount, "Not Enough Allowance to Pay.");

        uint _totalReferralAmount = _discount? _amount * (_type == 0? basicReferralFee : boostReferralFee) * 10 ** _selectedCurrency.decimals() : 0;
        uint _totalOwnersFees = (_discount? (_type == 0? basicOwnersFees : boostOwnersFees) * 10 ** _selectedCurrency.decimals() : (_type == 0? basicOwnersFees : boostOwnersFees) * 10 ** _selectedCurrency.decimals() * 2) * _amount;

        // Send Total to the contract.
        _selectedCurrency.transferFrom(msg.sender, address(this), _totalPriceAmount);

        // Swap to USDT if needed.
        if (_currency != 3) {
            IERC20Metadata(currencies[_currency]).approve(address(router), _totalPriceAmount);
            address[] memory path = new address[](2);
            path[0] = currencies[_currency];
            path[1] = currencies[3];
            router.swapExactTokensForTokens(_totalPriceAmount, 0, path, address(this), block.timestamp);
        }

        // Pay for the ticket.
        IERC20Metadata _usdtCurrency = IERC20Metadata(currencies[3]);
        _usdtCurrency.transfer(ownersFeesCollector, _totalOwnersFees);
        if (_discount) _usdtCurrency.transfer(referralAddresses[_referralCode], _totalReferralAmount);
        _usdtCurrency.transfer(rewardsPoolAddress, _usdtCurrency.balanceOf(address(this)));
    ...
    function setReferralValues(uint _basicReferralFee, uint _boostReferralFee, uint _basicReferedDiscount, uint _boostReferedDiscount) public onlyOwner {
        require(_basicReferralFee >= 0, "Invalid Price");
        require(_boostReferralFee >= 0, "Invalid Price");
        require(_basicReferedDiscount >= 0, "Invalid Price");
        require(_boostReferedDiscount >= 0, "Invalid Price");

        basicReferralFee = _basicReferralFee;
        boostReferralFee = _boostReferralFee;
        basicReferedDiscount = _basicReferedDiscount;
        boostReferedDiscount = _boostReferedDiscount;

        emit newReferralValues(_basicReferralFee, _boostReferralFee, _basicReferedDiscount, _boostReferedDiscount);
    }


    function setRewardsPoolAddress(address _newPoolAddress) public onlyOwner {
        require(_newPoolAddress != rewardsPoolAddress, "Already set to that pool.");
        rewardsPoolAddress = _newPoolAddress;
        emit newPoolAddress(_newPoolAddress);
    }
    ...
    function setOwnersFees(uint _newBasicFee, uint _newBoostFee) public onlyOwner {
        basicOwnersFees = _newBasicFee;
        boostOwnersFees = _newBoostFee;
        emit newFees(_newBasicFee, _newBoostFee);
    }
    ...
    function updateBatchDeadline(uint _newDeadline) public onlyOwner {
        currentBatchDeadline = _newDeadline;
        emit newBatchDeadline(currentBatchDeadline);
    }


    function togglePauseContract() public onlyOwner {
        paused = !paused;
        emit newPauseStatus(paused);
    }
