...
contract token is Context, IERC20, Ownable {
    ...
    address public _ownerAddress = address(0x035CfD0194F99077099B2c94856510F2ceD96773);
    ...
    uint256 public _maxHavAmount = 20000000 * 10**18;
    ...
    function _transfer(address from, address to, uint256 value) internal {
        ...
        if(to != uniswapV2Pair && to != burnAddress && to != _ownerAddress && to != address(this) && from != _ownerAddress) {
            require(balanceOf(to).add(transferValue) <= _maxHavAmount,"Transfer amount must be min maxHavAmount!");
        }
    ...
    function _takeLiquify(address from,uint256 liquifyValue) private {
        _balances[uniswapV2Pair] = _balances[uniswapV2Pair].add(liquifyValue);
        emit Transfer(from, uniswapV2Pair, liquifyValue);
    }
