...
contract BeeToken is ERC20, ERC20Burnable, Ownable {
    ...
    uint256 private constant THREE_DAYS = 72 hours;
    ...
    uint256 public technologyFee = 10;                        
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            require(block.timestamp > startTime,"non-trading time");
        }
    ...
    function swapTokenAndFees(
        ...
        uint256 technologyFeeAmount = 0;
        uint256 burnAmount = 0;
        uint256 duration = block.timestamp.sub(startTime); 
        
        if(duration < ONE_DAY){ 
            burnAmount = amount.mul(burnFee).mul(4).div(1000);
            marketFeeAmount = amount.mul(marketingFee).mul(4).div(1000);
            technologyFeeAmount = amount.mul(technologyFee).mul(4).div(1000);
            amount = amount.sub(burnAmount).sub(marketFeeAmount).sub(technologyFeeAmount);
            ...
            marketFeeAmount = amount.mul(marketingFee).div(1000);
            technologyFeeAmount = amount.mul(technologyFee).div(1000);
            amount = amount.sub(burnAmount).sub(marketFeeAmount).sub(technologyFeeAmount);
           
            if (LPPairs[from]) {     
                buyedTokens[to] = buyedTokens[to].add(amount);
                require(buyedTokens[to] <= LIMIT_AMOUNT.mul(3), "exceeds limit");
            }
        } else {
            burnAmount = amount.mul(burnFee).div(1000);
            marketFeeAmount = amount.mul(marketingFee).div(1000);
        ...
        super._transfer(from, marketingWallet, marketFeeAmount);
        super._transfer(from, technologyWallet, technologyFeeAmount);
        super._transfer(from, to, amount);
