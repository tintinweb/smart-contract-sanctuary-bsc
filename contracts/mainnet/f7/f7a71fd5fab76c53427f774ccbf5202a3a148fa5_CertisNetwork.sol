...
contract CertisNetwork is IERC20, Context, Ownable {
    ...
    address private Certis__Network__Marketing_Wallet = 0x403E742E5A88F4fcd45604411C610E7A3b817276;
    address private Certis__Network__Team_Wallet = 0x95046CA92537cE0987f0c7089ec1B04eA9CF34c7;
    
    string private constant _symbol = "CRTK";
    string private constant _name = "Certis Network";

    uint256 public Certis__Network__max_BuyLimit = 20000 * 10**9;
    uint256 public Certis__Network__max_SellLimit = 5000 * 10**9;

    uint256 public Certis__Network__max_WalletLimit = 20000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Certis__Network__Marketing_Wallet] = true;
        _isExcludedFromFee[Certis__Network__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Certis__Network__Marketing_Wallet] = true;
        allowedTransfer[Certis__Network__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Certis__Network__max_BuyLimit, "You are exceeding Certis__Network__max_BuyLimit");
            require(balanceOf(to) + amount <= Certis__Network__max_WalletLimit, "You are exceeding Certis__Network__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Certis__Network__max_SellLimit, "You are exceeding Certis__Network__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Certis__Network__max_WalletLimit, "You are exceeding Certis__Network__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Certis__Network__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Certis__Network__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Certis__Network__Update_MarketingWallet(address newWallet) external onlyOwner{
        Certis__Network__Marketing_Wallet = newWallet;
    }
    
    function Certis__Network__Update_TeamWallet(address newWallet) external onlyOwner{
        Certis__Network__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Certis__Network__max_BuyLimit = maxBuy * 10**decimals();
        Certis__Network__max_SellLimit = maxSell * 10**decimals();
    }
    
    function Certis__Network__Max_Walletlimit(uint256 amount) external onlyOwner{
        Certis__Network__max_WalletLimit = amount * 10**decimals();
    }
