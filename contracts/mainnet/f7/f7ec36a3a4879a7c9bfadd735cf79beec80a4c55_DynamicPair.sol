...
// File: Github/DynamicSwap-v2-core/contracts/interfaces/IDynamicCallee.sol
...
// File: Github/DynamicSwap-v2-core/contracts/libraries/UQ112x112.sol
...
// File: Github/DynamicSwap-v2-core/contracts/libraries/Math.sol
...
// File: Github/DynamicSwap-v2-core/contracts/libraries/SafeMath.sol
...
// File: Github/DynamicSwap-v2-core/contracts/interfaces/IDynamicERC20.sol
...
// File: Github/DynamicSwap-v2-core/contracts/DynamicERC20.sol
...
// File: Github/DynamicSwap-v2-core/contracts/DynamicVoting.sol
...
contract DynamicVoting is DynamicERC20 {
    ...
    function initialize() internal {
        ...
        rules[4] = Rule(75,"setBaseToken(address)");    // set base token for floor price (which token price should not drop below floor)

        rules[5] = Rule(75,"setFloorPrice(uint256)");   // allow decrease token floor price if another token is WETH and activate it
...
// File: Github/DynamicSwap-v2-core/contracts/DynamicPair.sol
...
*/
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        if (_totalSupply == 0) {
            ...
            floorPrice0 = FloorPrice(uint224(priceBefore0), uint32(2)); // set floor price (balance1/balance0)
    ...
    // set base token for floor price (which token price should not drop below floor)

    function setBaseToken(address baseToken) external onlyVoting {

        if (baseToken == token0) floorPrice0.baseToken = 0;

        else if (baseToken == token1) floorPrice0.baseToken = 1;

        else floorPrice0.baseToken = 2;

    }



    // allow decrease token floor price if another token is WETH and activate it

    // floorPrice - price of token in WETH (native coin) with 18 decimals

    function setFloorPrice(uint floorPrice) external onlyVoting {

        address WETH = IDynamicFactory(factory).WETH();

        floorPrice = floorPrice * 2**112 / 1e18;



        if (token0 == WETH) {

            floorPrice = 2**224 / floorPrice;

            floorPrice0 = FloorPrice(uint224(floorPrice), uint32(1));

        } else if (token1 == WETH) {

            floorPrice0 = FloorPrice(uint224(floorPrice), uint32(0));
