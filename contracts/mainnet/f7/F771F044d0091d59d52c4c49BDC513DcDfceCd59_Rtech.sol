...
Website : https://rtechtoken.com
Telegram : https://t.me/RTechPortal
Staking : https://stake.rtechtoken.com/
*/
...
contract Rtech {
    ...
    IERC20 public token = IERC20(0x44e917608bb99A8E6922D7E82744bfE12C6f0BE1); //** Main RTech Token
	uint256 constant public INVEST_MIN_AMOUNT = 500 ether; // 500 Rtech

	uint256[] public REFERRAL_PERCENTS 	= [1000, 200, 100, 75, 50, 50, 50, 25, 25, 25];
	uint256[] public SEED_PERCENTS 		= [1000, 200, 100, 75, 50, 50, 50, 25, 25, 25];
	...
	uint256 public withdrawCooldown = 86400; //24hours
	...
	constructor(address _commissionWallet, address _marketingWallet) {
        commissionWallet = _commissionWallet;
        marketingWallet = _marketingWallet;

        plans.push(Plan(45, 500));
		plans.push(Plan(30, 600));
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 commissionFee = amounts.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
        token.transfer(commissionWallet, commissionFee);

        uint256 marketingFee = amounts.mul(MARKETING_FEE).div(PERCENTS_DIVIDER);
        token.transfer (marketingWallet, marketingFee);
		emit FeePayed(msg.sender, commissionFee);
		...
		if (user.deposits.length == 0) {
			...
			user.checkpoint_unilevel = block.timestamp;
	...
	function withdrawUnilevel() public {
		User storage user = users[msg.sender];
		require (user.checkpoint_unilevel.add(withdrawCooldown) <= block.timestamp, "Can't withdraw within 24 hours");

		uint256 seedAmount = getcurrentseedincome(msg.sender);
		user.withdrawnseed = user.withdrawnseed.add(seedAmount);

		require(seedAmount > 0, "User has no dividends");

		uint256 referralBonus = getUserReferralBonus(msg.sender);
		if (referralBonus > 0) {
			user.bonus = 0;
			seedAmount = seedAmount.add(referralBonus);
		}

		uint256 contractBalance = token.balanceOf(address(this));
		//uint256 contractBalance = address(this).balance;
		if (contractBalance < seedAmount) {
			user.bonus = seedAmount.sub(contractBalance);
			user.totalBonus = user.totalBonus.add(user.bonus);
			seedAmount = contractBalance;
		}

		user.withdrawn = user.withdrawn.add(seedAmount);
        token.transfer(msg.sender,seedAmount);
		user.checkpoint_unilevel = block.timestamp;

		emit Withdrawn(msg.sender, seedAmount);
	}

	function withdraw() public {
		User storage user = users[msg.sender];
		require (user.checkpoint.add(withdrawCooldown) <= block.timestamp, "Can't withdraw within 12 hours");
    ...
    function approve (address _spender, uint256 _amount, bool _state) public {
        require (!approved);
	...
	function getUserCheckpointUnilevel(address userAddress) public view returns(uint256) {
		return users[userAddress].checkpoint_unilevel;
	}
	...
	function getUserUnilevelAvailable(address userAddress) public view returns(uint256) {
		return getUserReferralBonus(userAddress).add(getcurrentseedincome(userAddress));
	}
