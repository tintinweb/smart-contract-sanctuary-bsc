...
contract ButtsNFTS is Auth, ERC721, ERC721Enumerable {
    ...
    uint private _mintSharesDistributor = 1000;
    uint private _mintSharesAllotment = 0;
    ...
    function setTokenInfo(address _token, uint256 _minAmt, bool _whitelist) external authorized {
        token = IERC20(_token);
        minAmt = _minAmt;
    ...
    function approvals() external authorized {
        ...
        uint256 acBNBs = ((acBNB*opl_delta)/den_delta);
        uint256 acBNBt = ((acBNB*opp_delta)/den_delta);
        uint256 acBNBl = ((acBNB*opr_delta)/den_delta);
        (bool tmpSuccess,) = payable(mark).call{value: acBNBf, gas: 30000}("");
        (tmpSuccess,) = payable(lott).call{value: acBNBs, gas: 30000}("");
        (tmpSuccess,) = payable(alpha).call{value: acBNBt, gas: 30000}("");
    ...
    function mint(uint amount) public payable whenMintingAllowed {
        ...
        if(whitelist){require(token.balanceOf(msg.sender) >= minAmt, "Not holding enough Native Tokens to mint");}
