...
// File: contracts/FswapVault.sol

pragma solidity 0.6.12;

contract FswapVault is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 fswapAtLastUserAction; // keeps track of fswap deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Fswap token
    ...
    /**
     * @notice Constructor
     * @param _token: Fswap token contract
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.fswapAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     */
    function calculateHarvestFswapRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingFswap(0, address(this));
     ...
     * @return Returns total pending fswap rewards
     */
    function calculateTotalPendingFswapRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingFswap(0, address(this));
        amount = amount.add(available());
