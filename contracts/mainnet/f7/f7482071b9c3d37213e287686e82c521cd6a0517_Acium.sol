...
contract Acium{
    ...
    function lock(uint _period, uint _value) public {
        ...
        timeLocked[msg.sender][_period] = block.timestamp + _period * (30 * 24 * 60 * 60);//_period * 24 * 60 * 60;
    ...
    function claim(uint _period, uint _value) public {
        ...
        uint256 timeStaked = ((block.timestamp - lockedIn[msg.sender][_period])/(30 * 24 * 60 * 60));//30 * 24 * 60 * 60;
        require(balanceOf[owner] >= ((_value) + (_value * (feePerMonth[_period] * timeStaked))/10**decimals), "Sorry, the owner balance is insufficient.");
        balanceOf[owner] -= ((_value) + (_value * (feePerMonth[_period] * timeStaked))/10**decimals);
        balanceOf[msg.sender] += ((_value) + (_value * (feePerMonth[_period] * timeStaked))/10**decimals);
        lockedIn[msg.sender][_period] = 0;
        rewards[msg.sender][_period] -= _value;
