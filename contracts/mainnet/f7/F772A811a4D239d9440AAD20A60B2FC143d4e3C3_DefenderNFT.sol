...
// DfdMinter
contract DfdMinter {
    address public dfdMinter;

    modifier onlyDFDMinter() {
        require(msg.sender == dfdMinter, 'Defender: minter error');
        _;
    }

    function transferDfdMinter(address newDfdMinter) public onlyDFDMinter {
        if (newDfdMinter != address(0)) {
            dfdMinter = newDfdMinter;
        }
    }

    function renounceDfdMinter() public onlyDFDMinter {
        dfdMinter = address(0);
    }
}

contract DefenderNFT is ERC721A, Ownable, DfdMinter {
    ...
    function mintByContract(uint16 _num, address recipient) public onlyDFDMinter{
        require(totalSupply() + _num <= maxTotal,"Exceeds maximum supply");
        _safeMint(recipient, _num);
    }
    ...
    function setDfdMinter(address _minter) public onlyOwner {
        dfdMinter = _minter;
    }
