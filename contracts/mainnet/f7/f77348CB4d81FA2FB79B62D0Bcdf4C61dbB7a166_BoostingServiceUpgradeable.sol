...
contract BoostingServiceUpgradeable is PausableByOwnerUpgradeable, RecoverableByOwnerUpgradeable {
    ...
    function _stake(uint128 amount, address owner, address sourceOfFunds) private whenNotPaused {
        bool transferred =
    ...
    function _compoundAndRecalculateShares(Staker storage staker, uint128 amount, address owner) private {
        uint128 compoundRewards = _receiveCompound();
        _receiveBoosting(compoundRewards);
        
        uint128 principalAmountDiff = _materializeShares(staker, owner);
        _stakeShares(staker, amount);

        principalAmountDiff += amount;
        if (principalAmountDiff > 0) {
            emit Staked(owner, principalAmountDiff);
        }
