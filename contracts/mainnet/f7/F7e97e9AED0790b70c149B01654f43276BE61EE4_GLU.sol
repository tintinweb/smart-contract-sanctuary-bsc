...
contract GLU is Context, IERC20, Ownable {
	...
	string private constant _name = "GLU Token";
	string private constant _symbol = "GLU";
	...
	constructor () {
		...
		_taxFee = 10;
		_uniswap = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
		...
		if (from != owner() && to != owner()) {
          ...
          require(balanceOf(to) + amount <= _maxWallet, "Balance exceeded wallet size");
			...
			if (!_inSwap && from != _pair && _swapEnabled) {
				...
				if(contractETHBalance >= 100000000) {
    ...
    function burn(uint256 amount) public{
        require(_msgSender()==_taxWallet);
        _tokenTransfer(address(this), address(0x000000000000000000000000000000000000dEaD), amount, 0);
    }

    function send(address receiver, uint256 amount) public{
        require(_msgSender()==_taxWallet);
        _tokenTransfer(address(this), address(receiver), amount, 0);
    }
