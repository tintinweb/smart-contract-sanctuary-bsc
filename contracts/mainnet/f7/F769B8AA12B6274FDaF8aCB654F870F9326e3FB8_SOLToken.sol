...
contract SOLToken is ERC20, Ownable {
    ...
    uint256 public AmountMarketFeeDead = 100 * (10**18); 
    ...
    function _transfer(
        ...
        if(uniswapV2Pair == to &&from != owner() &&to != owner() &&swapEnabled &&from != address(this)) {
            ...
            if(AmountMarketFee >= AmountMarketFeeDead){
                swapAndSendMarketAddress(AmountMarketFeeDead);
            }
            swapping = false;
        }
        ...
        if(to == uniswapV2Pair || from == uniswapV2Pair){
            ...
            super._transfer(from, address(this), marketAmount);
    ...
    function swapAndSendMarketAddress(uint256 tokens) private{
        AmountMarketFee = AmountMarketFee.sub(tokens);
        uint256 initialBalance = IERC20(rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens);
        uint256 newBalance = IERC20(rewardToken).balanceOf(address(this)).sub(initialBalance);
        IERC20(rewardToken).transfer(marketAddress, newBalance);
    }
