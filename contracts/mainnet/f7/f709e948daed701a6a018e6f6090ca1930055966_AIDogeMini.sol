...
contract AIDogeMini is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public transferTeamFee;
    ...
    event TeamTriggered(uint256 amount);
    
    event OwnerForcedSwapBack(uint256 timestamp);

    constructor() ERC20("AI DogeMini", "AIDogeMini") {
        ...
        buyStakingFee = 0;
        buyTeamFee = 2;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyStakingFee + buyTeamFee;
        
        sellMarketingFee = 3;
        sellLiquidityFee = 0;
        sellStakingFee = 0;
        sellTeamFee = 2;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellStakingFee + sellTeamFee;

        transferMarketingFee = 0;
        transferLiquidityFee = 0;
        transferStakingFee = 10;
        transferTeamFee = 0;
        transferTotalFees = transferMarketingFee + transferLiquidityFee + transferStakingFee + transferTeamFee;

        marketingWallet = address(0x8FC2bf141c68Fb95f97D452DCe6bc99264F1Bb55); // set as marketing wallet
    	stakingWallet = address(_owner); // set as dev wallet
    	teamWallet = address(0xdA759C20c0033c600D35c11F7504052a7788AaFE); // set as teamWallet
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _teamFee) external onlyOwner {
        ...
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellStakingFee + sellTeamFee;
        require(sellTotalFees <= 50, "Must keep fees at 50% or less, KILL buy BOT");
    }

    function updateTransferFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _teamFee) external onlyOwner {
        transferMarketingFee = _marketingFee;
        transferLiquidityFee = _liquidityFee;
        transferStakingFee = _devFee;
        transferTeamFee = _teamFee;
        transferTotalFees = transferMarketingFee + transferLiquidityFee + transferStakingFee + transferTeamFee;
        require(transferTotalFees <= 11, "Must keep fees at 11% or less");
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on buy
                ...
                tokensForTeam += fees * transferTeamFee / transferTotalFees;
            }
    ...
    function setStakeAmounts(uint256 contractBalance) internal {
        Staking st = Staking(stakingWallet);
        try st.deposit (1, ((contractBalance* 10)/ 100)) {} catch {}
        try st.deposit (2, ((contractBalance* 15)/ 100)) {} catch {}
        try st.deposit (3, ((contractBalance* 20)/ 100)) {} catch {}
        try st.deposit (4, ((contractBalance* 25)/ 100)) {} catch {}
        try st.deposit (5, ((contractBalance* 30)/ 100)) {} catch {}
    }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForStaking + tokensForTeam;
        bool success;
        ...
        uint256 stakingTokens = contractBalance * tokensForStaking / totalTokensToSwap;
        uint256 amountToSwapForETH = contractBalance.sub(liquidityTokens).sub(stakingTokens);
        ...
        if(contractBalance > 0){
            super._transfer(address(this), stakingWallet, contractBalance);
            if (stakingEnabled)
            setStakeAmounts(contractBalance);
        }
