...
BUSDMafia a miner fork edit 2022, MaidzNZ
...
contract BUSDMafia is Context, Ownable , ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public constant min = 10 ether;
    uint256 public constant max = 2500 ether;
    ...
    uint256 public constant mandatory_fee = 50;
    ...
    constructor() {
        dev = payable(0xc23e321F96436A44d74b7b7f7355A70160e4c25E);
    ...
    function withdrawal() public noReentrant { 
        ...
        if(investments[msg.sender].cycleCount < max_cycle){
            ...
            claimTime[msg.sender] = claimDaily(msg.sender,0,0);
        }
    ...
    function claimDailyRewards() public noReentrant{
        require(INIT == true, "contract does not launch yet");
        require(investments[msg.sender].invested > 0, "user should has an active deposit");
    ...
    function Ref_Withdraw() external noReentrant {
        require(INIT == true, "contract has not launched yet");
