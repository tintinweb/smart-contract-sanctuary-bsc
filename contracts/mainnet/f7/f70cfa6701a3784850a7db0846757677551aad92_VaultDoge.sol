...
Three ways of burning the $VOGE supply effectively raising the floor. 
We aim for longterm! Join our telegram channel.

Telegram: https://t.me/vaultdoge
Website: https://www.vdcsk.site
Twitter: https://twitter.com/vaultdoge_
...
contract VaultDoge is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _MrktFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _MrktFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _MrktReceiver,address _communityFeeReceiver, address _teamFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "Vault Doge";
    string constant private _symbol = "VOGE";
    ...
    uint256 public sellFeeMrkt = 0;
    uint256 public sellFeecom = 0;
    uint256 public sellFeeteam = 0;
    uint256 public sellFeeBurn = 15;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(MrktFee).add(comFee).add(teamFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMrkt).add(sellFeecom).add(sellFeeteam).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address public autoLiquidityReceiver = 0x1BD13C07b8974027F47Fa56BE71be66326F3DD70;
    address public MrktFeeReceiver = 0xb478c4e2E28beFEA93541C7f77c5F200Dde3a351;
    address public communityFeeReceiver = 0x1BD13C07b8974027F47Fa56BE71be66326F3DD70;
    address public teamFeeReceiver = 0x1BD13C07b8974027F47Fa56BE71be66326F3DD70;
    ...
    constructor () Auth(msg.sender) {
        ...
        isFeeExempt[address(0xf2FC4CD869B2F168Ad8A6405974C9dec39030faB)] = true; 
        isTxLimitExempt[address(0xf2FC4CD869B2F168Ad8A6405974C9dec39030faB)] = true;
        isMaxWalletExempt[address(0xf2FC4CD869B2F168Ad8A6405974C9dec39030faB)] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHcom = amountETH.mul(comFee.add(sellFeecom)).div(totalETHFee);
        ...
        (tmpSuccess,) = payable(communityFeeReceiver).call{value: amountETHcom}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _MrktFee, uint256 _comFee, uint256 _teamFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        MrktFee = _MrktFee;
        comFee = _comFee;
        teamFee = _teamFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_MrktFee).add(_comFee).add(_teamFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _MrktFee,_comFee, _teamFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _MrktFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMrkt = _MrktFee;
        sellFeecom = _comFee;
        sellFeeteam = _teamFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_MrktFee).add(_comFee).add(_teamFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _MrktFee,_comFee, _teamFee, _BurnFee);
    }


    function setFeeReceivers(address _autoLiquidityReceiver, address _MrktFeeReceiver,address _communityFeeReceiver, address _teamFeeReceiver, address _BurnFeeReceiver) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _MrktFeeReceiver != address(0) && _communityFeeReceiver != address(0) && _teamFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _MrktFeeReceiver, _communityFeeReceiver, _teamFeeReceiver, _BurnFeeReceiver);
    }
