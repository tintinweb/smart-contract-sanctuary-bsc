...
contract BioDeposit_Core is AccessControlUpgradeable, PausableUpgradeable
{
    struct Level{
...
*/
        ...
        // test version $0.118-$0.048-$0.010
        tempProduct = Product({
        ...
        tempProduct = Product({
           ...
           cost1:  50000000000000000,
           cost2:  20000000000000000,
           costAddress_1: address(0xfA66559CbBB70de7459d2fC3036C22358f835D30),
           costAddress_2: address(0xB40295157c6E72D8f2bA3cbeb8F4b997747C0d6F),
           itemsAmount: 100,
           lastItemID: 0,
        ...
        products[1] = tempProduct;

/*
       // version for remix tests
       admin_mint (0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 1, 9, true);
       admin_mint (0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2, 1, 0, true);
       admin_mint (0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db, 2, 7, true);
       admin_mint (0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB, 3, 5, true);
       admin_mint (0x617F2E2fD72FD9D5503197092aC168c91465E7f2, 3, 0, false);
       admin_mint (0x17F6AD8Ef982297579C203069C1DbfFE4348c372, 3, 0, false);
       admin_mint (0x5c6B0f7Bf3E7ce046039Bd8FABdfD3f9F5021678, 4, 2, true);
       admin_mint (0x03C6FcED478cBbC9a4FAB34eF9f40767739D1Ff7, 4, 0, false);
*/
       admin_mint (0xD4E0EA6108315D62A7c08c7236bec70d0e47c6E9, 1, 9, true);
       admin_mint (0x9aEcf27d32598e74C20F887FaF69e812A29a2444, 1, 0, false);
       admin_mint (0x5C13cff82c73A73F1d3f7624804D4089D4E1CE34, 2, 7, true);
       admin_mint (0x56dcF8A73994abc49d99A3E7f0478904Bd706b48, 3, 5, true);
       admin_mint (0x2293EFf3ea6A1Aade782A42F4750CcD7F514cA76, 3, 0, false);
       admin_mint (0xBA3e9DbF0df642562dD9Af0f45400110ad9b381C, 3, 0, false);


       // ID=3 initialize
       totalBuyAmounts[0] += 10148000000000000000000;
       users[3].personalDeposit += 10148000000000000000000;
       addItems(3,0,86);
       users[2].firstLineVolume += 10148000000000000000000;
       users[2].structureVolume += 10148000000000000000000;
       users[2].mainBranchVolume += 10148000000000000000000;

       users[1].structureVolume += 10148000000000000000000;
       users[1].mainBranchVolume += 10148000000000000000000;
       users[1].USDBalance += 1928120000000000000000;
       users[1].USDBalanceWithdrawn += 1928120000000000000000;

       counters[0] += 1928120000000000000000;
       counters[2] += 1928120000000000000000;


       // test version for mainnet      
/*       admin_mint (0xD4E0EA6108315D62A7c08c7236bec70d0e47c6E9, 1, 9, true);
...
//*************** USER FUNCTIONS ******************************* */
   ...
   function mint(uint256 _parentTokenID) public whenNotPaused(){
    ...
    function buyProduct(uint8 _productID, uint32 _itemsAmount) public whenNotPaused(){
        ...
        require(!isContract(msg.sender), "Contract calls are not available");
//        require(IBioDepositNFT(NFTContract).ownerOfToken(_userTokenID) == msg.sender, "Not a token owner");
...
//        require(IBioDepositNFT(NFTContract).ownerOfToken(_tokenID) == msg.sender, "Not a token owner");
...
//*************** ADMIN FUNCTIONS ******************************* */
   ...
   function admin_mint(address _receiver, uint256 _parentTokenID, uint8 _startLevel, bool _levelProtected) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused(){
    ...
    //admin sets address of BioDeposit Init contract
    function setInitContractAddress(address _initContract) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getTotals() external view onlyRole(DEFAULT_ADMIN_ROLE) returns(uint128[10] memory) {
        return totalBuyAmounts;
    }

    // admin gets admin counters (18 decimals)
    function getCounters() external view onlyRole(DEFAULT_ADMIN_ROLE) returns(uint128[3] memory) {
        return counters;
    }


    function initLevels() public onlyRole(DEFAULT_ADMIN_ROLE) {

        uint8 i;
        //Levels Initialize 
        for (i=0; i<10; i++){

            (levels[i].personalDeposit,
             levels[i].firstLineVolume,
             levels[i].structureVolume,
             levels[i].investorVolume) = IBioDepositInit(initContract).getLevelData_1(i);

            (levels[i].USDBonus,
             levels[i].revenueBonus,
             levels[i].utilityBonus) = IBioDepositInit(initContract).getLevelData_2(i);
        }

        //LBonus Initialize
        for (i=0; i<22; i++){
            (LBonus[i].personalDeposit,
             LBonus[i].firstLineVolume,
             LBonus[i].mainBranchVolume,
             LBonus[i].sideBranchVolume,
             LBonus[i].USDBonus) = IBioDepositInit(initContract).getLBonusData(i);
       }
    }
...
/*
    function getUserProductIDs(uint256 _userTokenID) external view returns(uint32[][10] memory){
        require(IBioDepositNFT(NFTContract).tokenExists(_userTokenID), "user doesn't exist");
       
        return users[_userTokenID].productIDs;
...
*/
    ...
    function addItems(uint256 _userTokenID, uint8 _productID, uint32 _itemsAmount) private{
        ...
        while (i < _itemsAmount){
            users[_userTokenID].productIDs[_productID].push(products[_productID].lastItemID);
            i++;
        }
