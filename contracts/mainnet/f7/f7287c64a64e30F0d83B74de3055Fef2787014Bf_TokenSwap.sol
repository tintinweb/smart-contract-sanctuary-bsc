...
contract TokenSwap is Ownable {
    ...
    function swapTokens(uint amountIn, uint amountOutMin, address[] memory path, address[] memory routerPath) public {
        ...
        address factory = IUniswapV2Router02(routerPath[0]).factory();
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address[] memory routerPath) internal virtual {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            if (routerPath.length > 1 && i < routerPath.length) {
                factory = IUniswapV2Router02(routerPath[i]).factory();
            }
            address to = i < path.length - 2 ? UniswapLibrary.pairFor(factory, output, path[i + 2]) : msg.sender;
