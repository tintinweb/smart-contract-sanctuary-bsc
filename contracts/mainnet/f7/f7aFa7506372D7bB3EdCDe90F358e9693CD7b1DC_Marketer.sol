...
contract Marketer is Ownable {
    ...
    function buyBotU(address recipient, uint8 _id) external payable {
        ...
        if (token.balanceOf(msg.sender) >= price) {
            ...
            totalBuyerAmount[recipient] = totalBuyerAmount[recipient].add(price);
        } else {
    ...
    function setPricing(
        ...
        if (pricingU[_id] == 0) pricingCount++;
        pricingU[_id] = _price;        
    }

    function setBuyerList(address recipient, uint256 t) public onlyOwner {
        require(t != 0, "t must > 0");
        buyerAmount[recipient] = block.timestamp.add(t);
    }
