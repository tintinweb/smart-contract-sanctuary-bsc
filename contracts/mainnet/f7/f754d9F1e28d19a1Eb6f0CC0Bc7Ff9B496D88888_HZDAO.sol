...
contract HZDAO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    // dead Amount
    uint256 public deadAmount;
    // total dead Amount
    uint256 public totalDeadAmount;
    ...
    constructor(
        ...
        address[2] memory addrs, // router,onwer
        uint256[3] memory buyFeeSetting_, // Liquidity,lp,dead
        uint256[3] memory sellFeeSetting_ // Liquidity,lp,dead
        ...
        swapTokensAtAmount = 1000 * (10 ** 8); // 1000 swap
        minLPFeeRewards = 10 * (10 ** 18); // min Lp Rewards Dividend
        totalDeadAmount = 13000000 * (10 ** 8);
    ...
    function setTotalDeadAmount(uint amount) public onlyOwner {
        totalDeadAmount = amount;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                LPFee = amount.mul(sellLpFee).div(1000);
                AmountLPFee += LPFee;
                DFee = amount.mul(sellDeadFee).div(1000);
                fees = LFee.add(LPFee).add(DFee);
            }
            if(automatedMarketMakerPairs[from]) {
                LFee = amount.mul(buyLiquidityFee).div(1000);
                AmountLiquidityFee += LFee;
                LPFee = amount.mul(buyLpFee).div(1000);
                AmountLPFee += LPFee;
