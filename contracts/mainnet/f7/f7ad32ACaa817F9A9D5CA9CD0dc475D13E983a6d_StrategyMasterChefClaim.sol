...
contract StrategyMasterChefClaim is BaseStrategyLP {
    ...
    // Harvests vault and distributes controller fees to _to
    // This is overloading the earn function with one function parameter _to
    function earn(address _to) public nonReentrant whenNotPaused {
        // Harvest farm tokens
        _vaultHarvest();
        ...
        if (earnedAmt > 0) {
            earnedAmt = distributeFeesTo(earnedAmt, _to);
            earnedAmt = distributeRewards(earnedAmt);
            earnedAmt = buyBack(earnedAmt);
    ...
    // harvests first and then sends fees to _to
    function claim(address _to, uint256 _amount) external {
        require(msg.sender == vaultChefAddress, "only vaultChef");
        // Call overloaded earn(_to)
        earn(_to);
