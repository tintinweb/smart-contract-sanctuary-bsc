...
/*
       Telegram: https://t.me/NFTidentity
       Website:  https://nftidentity.eu/
       Twitter:  https://twitter.com/nft_identity

         Total taxes: 8% buys/sells
         1% reflections
         2% project Wallet
         1% buyback Wallet
         4% marketing
...
contract NFT_Identity is Context, IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = unicode"NFT_Identity";
    string private constant _symbol = "NFTID";
    ...
    uint256 public numTokensSellToAddToLiquidity = 25 * 10**14;
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 amount = address(this).balance.sub(initialBalance); // 7%
        uint256 marketing = amount.mul(1000).div(1750);            // 4%
        uint256 project = amount.mul(1000).div(3500); // 2%
        uint256 buyback = amount.sub(marketing).sub(project); // 1%
    ...
    function _transferStandard(
        ...
        // _getTValues
        uint256 tFee = tAmount.mul(currentFee).mul(1).div(800); // 8 * 1/8 = 1%
        uint256 tLiquidityFee = tAmount.mul(currentFee).mul(7).div(800); // 8 * 7/8 = 7%
