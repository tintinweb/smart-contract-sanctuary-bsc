...
interface IMasterChefV2 {
    ...
    function pendingIrs(uint256 _pid, address _user) external view returns (uint256);
...
contract IrsPool is Ownable, Pausable {
    ...
    function harvest() internal {
        uint256 pendingIrs = masterchefV2.pendingIrs(cakePoolPID, address(this));
        if (pendingIrs > 0) {
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingIrsRewards();
     ...
     */
    function calculateTotalPendingIrsRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingIrs(cakePoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingIrsRewards()) * (1e18)) / totalShares);
    }
