...
contract MultiSaleSOSAmazon is BEP20, Ownable{
    ...
    function getBnbfactory() public view returns (address) {
        return bnb_factory;
    }
    ...
    function transferFromUser(address recipient, uint256 amount) public isPausable() {
        ...
        require(sender != address(this), "Transaction not allowed");
        require(balanceOf(sender) > 0 && amount > 0 && amount <= balanceOf(sender), "You do not have enough balance for this withdrawal");
        if(!inTransfer){
            inTransfer = true;
                uint256 fee = amount.mul(_TRANSACTION_FEE).div(100);
                pendingBalance[sender] -= amount;
                pendingBalance[address(this)] += fee;
    ...
    */
    function buyLandWithPartial(string memory tokenName, address tokenAddress, uint256 tokenPrice,  uint256 SOSPrice) public isPausable() {
        require(tokenPrice > 0, "Insufficient amount for this transaction");
        require(tokenAddress != address(0), "We do not accept this token");
        IBEP20 ContractAdd = IBEP20(tokenAddress);
        uint256 dexBalance1 = ContractAdd.balanceOf(msg.sender);
        require(tokenPrice > 0 && tokenPrice <= dexBalance1, "Insufficient amount for this transaction");
        require(ContractAdd.transferFrom(msg.sender, address(this), tokenPrice.mul(_DECIMALFACTOR)), "A transaction error has occurred. Check for approval.");

        IBEP20 ContractToken = IBEP20(token_Address);
        uint256 dexBalance2 = ContractToken.balanceOf(msg.sender);
        require(SOSPrice > 0 && SOSPrice <= dexBalance2, "Insufficient amount for this transaction");
        require(ContractToken.transferFrom(msg.sender, address(this), SOSPrice), "A transaction error has occurred. Check for approval.");

        emit ReceivedLand(msg.sender, tokenPrice, SOSPrice, tokenName);
    }

    function buyLandWithBNB(uint256 tokenPrice,  uint256 SOSPrice) public payable {
        require(tokenPrice > 0, "Insufficient amount for this transaction");
        ...
        require(SOSPrice > 0 && SOSPrice <= dexBalance, "Insufficient amount for this transaction");
        require(ContractToken.transferFrom(msg.sender, address(this), SOSPrice), "A transaction error has occurred. Check for approval.");

        emit ReceivedLand(msg.sender, tokenPrice, SOSPrice, "BNB");
    }

    function buyLandWithToken(uint256 tokenPrice,  uint256 SOSPrice) public isPausable() {
        require(tokenPrice > 0, "Insufficient amount for this transaction");
        ...
        emit ReceivedLand(msg.sender, tokenPrice, SOSPrice, "SOSAMZ");
    }

    function buyCardsWithToken(uint256 CardsPrice, uint256 CardsQuant, string memory deck) public isPausable() {
        require(CardsQuant > 0, "Insufficient amount for this transaction");
        require(CardsPrice > 0, "Insufficient amount for this transaction");
        uint256 Tokenprice = CardsPrice.mul(CardsQuant);

        IBEP20 ContractToken = IBEP20(token_Address);
        uint256 dexBalance = ContractToken.balanceOf(msg.sender);
        require(Tokenprice > 0 && Tokenprice <= dexBalance, "Insufficient amount for this transaction");
        ...
        emit ReceivedCards(msg.sender, Tokenprice, CardsQuant, "SOSAMZ", deck);
    }
    ...
    */
    function getpriceBNB(uint256 amount) public view virtual returns (uint256) {
        ...
        IUniswapV2Pair _tokenPair = IUniswapV2Pair(bnb_factory);
    ...
    * @param addr of the token address
    */
    function setBnbFactory(address BnbFactory) external virtual onlyOwner {
        require(BnbFactory.isContract(), "The address entered is not valid");
        bnb_factory = BnbFactory;
    }
    ...
    event ReceivedLand(address indexed from, uint256 amount, uint256 sosPrice, string token);
    event ReceivedCards(address indexed from, uint256 amount, uint256 cardQuant, string token, string deck);
