...
contract ERC20 is Context, IERC20 {
     ...
     *
     * To learn more about hooks, head to xref:PSHARE:extending-contracts.adoc#using-hooks[Using Hooks].
     */
...
contract SCT is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 SCT-WBNB LP -> SCT pool
    // uint256 public constant INITIAL_SCT_POOL_DISTRIBUTION = 0 ether;
    ...
    uint256[] public taxTiersRates = [   5000, 4000, 3000, 2000, 1600, 1500,   0, 0, 0, 0, 0, 0, 0, 0];
    ...
    /**
     * @notice Constructs the SCT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SCT Token", "SCT") {
        // Mints 1 SCT to contract creator for initial pool setup
    ...
    function _getSctPrice() internal view returns (uint256 _sctPrice) {
        try IOracle(sctOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Sct: failed to fetch SCT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _sctPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_sctPrice >= taxTiersTwaps[tierId]) {
    ...
    function setSctOracle(address _sctOracle) public onlyOperatorOrTaxOffice {
        require(_sctOracle != address(0), "oracle address cannot be 0 address");
        sctOracle = _sctOracle;
    }
    ...
    /**
     * @notice Operator mints SCT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SCT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSctPrice = _getSctPrice();
            currentTaxRate = _updateTaxRate(currentSctPrice);
            if (currentSctPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        //require(_sctPool != address(0), "!_sctPool");
        //require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        //_mint(_sctPool, INITIAL_SCT_POOL_DISTRIBUTION);
