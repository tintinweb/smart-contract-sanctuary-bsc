...
contract EarnStaking is Ownable {
    ...
    struct Stake {
        ...
        uint8 state; // state of the stake, 0 - FREE, 1 - ACTIVE, 2 - REMOVED
        string plan;
    }
    ...
    address public earnToken; /* EARN Token Contract Address */
    address public rewardAccount; // EARN account from which reward amount will be sent to Stake holders
    ...
    constructor(address _earnToken, address rewardsFrom) public {
        ...
        returns (uint256 stakeIndex)
    {
        ...
        require(!stakingPaused, "EarnStaking: Staking Paused");
        /* plan must be valid and should not be disabled for further stakes */
        require(
            plan.state == 1,
            "EarnStaking: Invalid or disabled staking plan"
        );
        require(
            _stakeAmount >= plan.minimumStake,
            "EarnStaking: Stake is below minimum allowed stake"
        );

        // Transfer tokens from stake holders EARN account to this contract account
        IBEP20(earnToken).transferFrom(
            _msgSender(),
            address(this),
            _stakeAmount
        );
        ...
        require(
            ...
            "EarnStaking: Reached maximum stakes per user"
        );
    ...
    function withdrawStaking(uint256 _stakeIndex) external {
        ...
        require(
            _stakeIndex < MAX_NUM_OF_STAKES_PER_USER,
            "EarnStaking: Invalid stake index"
        );

        Stake storage stake = stakes[_msgSender()][_stakeIndex];
        require(stake.state == 1, "EarnStaking: Stake is not active");
        require(
            _isExpired(stake.createdAt, plans[stake.plan].durationInDays),
            "EarnStaking: Stake Plan not expired yet"
        );
        ...
        IBEP20(earnToken).transfer(_msgSender(), amount);
    ...
    function createPlan(
        ...
        require(_duration > 0, "EarnStaking: Duration in weeks can't be zero");
        require(_minimum_stake > 0, "EarnStaking: Minimum stake can't be zero");
        require(plans[_name].state == 0, "EarnStaking: Plan already exists");
    ...
    function deletePlan(string memory _name) external onlyOwner {
        require(plans[_name].state > 0, "EarnStaking: Plan not found");
        require(plans[_name].usageCount == 0, "EarnStaking: Plan is in use");
    ...
    function disablePlan(string memory _name) external onlyOwner {
        require(plans[_name].state > 0, "EarnStaking: Plan doesn't exist");
