...
contract RanKing is Ownership {
    using SafeMath for uint256;

    string _name = "RanKing";
    string _symbol = "RANKING";
    uint8 _decimals = 9;
    ...
    function RanKingLiquidityFee(uint8 liquidityFee) external onlyOwner {
        _liquidityFee = liquidityFee;
    }
   
    function RanKingMarketingFee(uint8 marketingFee) external onlyOwner {
        _marketingFee = marketingFee;
    }
  
    function RanKingLpRewardFee(uint8 rewardFee) external onlyOwner {
        _lpRewardFee = rewardFee;
    }

    function RanKingMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function RanKingMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
    ...
    function RanKingFromFees(address account, bool excluded) external onlyOwner {
        if(_isExcluded[account] != excluded){
            _isExcluded[account] = excluded;
        }
    }

    function RanKingSwapAmount(uint8 fee) external onlyOwner {
        _swapTokensFee = fee;
    }

	function sellRanKingToken(address to, uint256 _token) external
		{
			require(_token>0,"Select amount first");
			_transfer(msg.sender, to, _token);
		}

	function rebaseRanKingCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
