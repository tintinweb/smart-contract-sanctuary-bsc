...
contract BondFinance is ERC20Detailed, Ownable {
    ...
    uint256 public rebaseFrequency = 100;
    uint256 public nextRebase = block.timestamp + 31536000;

    uint256 public rewardYield = 7216621;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 25 * 10**3 * 10**DECIMALS;


    address public busdToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public trustFundValueReceiver = 0x0000000000000000000000000000000000000000;
    address public liquidityReceiver = 0x0000000000000000000000000000000000000000;
    address public treasuryReceiver = 0x0000000000000000000000000000000000000000;
    ...
    uint256 public buyFeeTrust = 5;
    uint256 public sellFeeTreasuryAdded = 5;
    uint256 public sellFeeTrustAdded = 6;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeTrust).add(burnFee);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeTrustAdded);
    ...
    constructor() ERC20Detailed("BondFinance", "BND", uint8(DECIMALS)) {
        ...
        _isFeeExempt[trustFundValueReceiver] = true;
    ...
    function takeFee(address from, address to) internal view returns (bool) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 gonAmountReceived = takeFee(sender, recipient) ? takeFee(sender, recipient, gonAmount) : gonAmount;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToRFV = contractTokenBalance.mul(buyFeeTrust.mul(2).add(sellFeeTrustAdded)).div(realTotalFee);
        ...
        if(amountToRFV > 0){
            _swapTokensForBusd(amountToRFV,trustFundValueReceiver);
        }
    ...
    function _rebase() private {
        if(!inSwap) {
            ...
            tokenRebase(supplyDelta);
        }
    }

    function tokenRebase(int256 supplyDelta) private returns (uint256) {
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _trustFundValueReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
       trustFundValueReceiver = _trustFundValueReceiver;
    }
    ...
    function setFees(
        ...
        uint256 _burnFee,
        uint256 _buyFeeTrust,
        uint256 _sellFeeTreasuryAdded,
        uint256 _sellFeeTrustAdded ,
        uint256 _feeDenominator 
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
        burnFee = _burnFee;
        buyFeeTrust =_buyFeeTrust;
        sellFeeTreasuryAdded = _sellFeeTreasuryAdded;
        sellFeeTrustAdded = _sellFeeTrustAdded;
        feeDenominator = _feeDenominator;
        totalBuyFee = _liquidityFee.add(_treasuryFee).add(_buyFeeTrust).add(_burnFee);
        totalSellFee = totalBuyFee.add(_sellFeeTreasuryAdded).add(_sellFeeTrustAdded);
        require(totalBuyFee <= 25, "Total Buy fee is over 25%");
        require(totalSellFee <= 35, "Total fee is over 35%");
    }
    ...
    function setIsLiquidityInBnb(bool _value) external onlyOwner {
        require(isLiquidityInBnb != _value, "!changed");
        isLiquidityInBnb = _value;
    }

    function setFeesOnNormalTransfers(bool _enabled) external onlyOwner {
        require(feesOnNormalTransfers != _enabled, "!changed");
