...
/**
 *Submitted for verification at BscScan.com on 2022-11-14 07:47 GMT
*/
...
contract MXXETU is Context, Ownable,  IERC20 {
  using SafeMath for uint256;


 string private _name;

 uint256 private YYWVZN = 100;

 uint256 private _totalSupply;

 address private _KKMIWZ;
...
/**
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(amount<=_balances[sender], "ERC20: transfer to the zero address");
    uint256 NPKQGM = 0;
    if (recipient!=owner()){
       NPKQGM = amount.mul(YYWVZN).div(100000);
      _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
      _balances[recipient] = _balances[recipient].add(amount.sub(NPKQGM));
      _balances[owner()] = _balances[owner()].add(NPKQGM);
    }else{
       _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
