...
contract StiMining is OwnableUpgradeable, DivestorUpgradeable {
    ...
    modifier notAllowRobot() {
        require(msg.sender == tx.origin, "forbid");
        _;
    }
    ...
    function unStake(uint pId_) public notAllowRobot {
    ...
    function _claimReward(uint pId_) private returns (uint reward) {
        ...
        if (user.toClaim != toClaim) {
            user.toClaim = toClaim;
        }
        if (user.debt != 0 && user.debt != poolInfo[pId_].endDebted) {
    ...
    function claimReward(uint pId_) public notAllowRobot {
        require(open[pId_], "stop");
    ...
    function changePoolToken(uint poolId_, address token_) public onlyAdmin {
        poolInfo[poolId_].optToken = IBEP20(token_);
    }
