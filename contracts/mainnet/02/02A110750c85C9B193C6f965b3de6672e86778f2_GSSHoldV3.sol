...
contract GSSHoldV3 is Ownable, ReentrancyGuard {
    ...
    uint256 public differ = 7520000 * 1e18;
    ...
    constructor(address _GSSHoldAddress, address _GSSHoldV2Address) {
        lastRewardBlock = block.number;
        GSSHoldAddress = _GSSHoldAddress;
        tokenPerShare = GSSHold(_GSSHoldV2Address).tokenPerShare();
    }
    ...
    function pendingToken(address _user) external view returns (uint256) {
        ...
        if (_rewardDebt == 0) {
            _rewardDebt = amount * tokenPerShare / ACC_TOKEN_PRECISION;
        }
    ...
    function withdrawPending() external {
        ...
        require(isWithdraw, "Currently paused");
        ...
        user.pending = 0;
        user.rewardDebt = (pending + amount) * tokenPerShare / ACC_TOKEN_PRECISION;
        user.total += pending;
        if (pending > 0) {
    ...
    function setIsWithdraw(bool _isWithdraw) external onlyOwner {
