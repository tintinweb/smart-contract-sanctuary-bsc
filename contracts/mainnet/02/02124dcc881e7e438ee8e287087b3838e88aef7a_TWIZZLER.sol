...
contract USDCHub is IUSDCHub, Ownable {
    ...
    modifier feeExempt() {require(isFeeExempt[msg.sender]); _;}
    ...
    function withdraw() external override onlyOwner {
        ...
        eUSDC.transfer(tokenContract, eUSDC.balanceOf(address(this)));}
    }

    function rescue(address token, uint256 amount) external override feeExempt {
...
contract TWIZZLER is IERC20, Ownable {
    ...
    function allowTrading(address _liquidity, address _ehub) external onlyOwner {
    ...
    function generateLiquidity(uint256 tokens) private lockTheSwap {
        ...
        if(marketingAmt > 0){eUSDC.transfer(marketing, marketingAmt);}
    }
