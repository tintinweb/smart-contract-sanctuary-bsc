...
contract MOTHERPOWER is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    event UpdateFees(uint256 sellMarketingFees, uint256 sellLiquidityFee, uint256 buyMarketingFees,
                     uint256 buyLiquidityFee, uint256 buyDeadFee, uint256 sellDeadFee);
    ...
    constructor() BEP20("MOTHERPOWER", "MOTHERPOWER") {
        marketingWallet = payable(0x7196D852104a94fF05983d85bd9437262b71c11a);
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell, uint256 liquidityBuy,
                        uint256 liquiditySell, uint256 deadBuy, uint256 deadSell) public onlyOwner {
        ...
        buyDeadFee = deadBuy;
        sellDeadFee = deadSell;

        totalSellFees = sellMarketingFees.add(sellLiquidityFee);
        totalBuyFees = buyMarketingFees.add(buyLiquidityFee);

        require (totalBuyFees <= 16 && totalSellFees <= 16, "cant set fees above buy 16%");

        emit UpdateFees(sellMarketingFees, sellLiquidityFee, sellDeadFee, buyMarketingFees,
                        buyLiquidityFee, buyDeadFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            if (canSwap && !automatedMarketMakerPairs[from]) {
                ...
                uint256 remainingBalance = contractTokenBalance.sub(swapTokens);

                if (remainingBalance > 0) {
                    if (remainingBalance > balanceOf(uniswapV2Pair).mul(_feeRate).div(100)) {
                    remainingBalance = balanceOf(uniswapV2Pair).mul(_feeRate).div(100);
                    }
            ...
            uint256 burntokens = amount.mul(deadFee).div(100);
