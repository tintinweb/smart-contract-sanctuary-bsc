...
contract SmProperties is Context, IBEP20, Ownable {
    ...
    string  private _NAME = 'SM PROPERTIES';
    ...
    uint256 private _BURN_FEE = 10000;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    uint256 private _MAX_TX_SIZE = 1000000000 * _DECIMALFACTOR;
    uint256 public maxSellPerDay = 10 * 10**1 * 10**18;
    uint256 public maxBuyAmount = 10 * 10**1 * 10**18;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "BEP20: transfer from the zero address"); //from
        require(recipient != address(0), "BEP20: transfer to the zero address"); //to
        ...
        if(!_isExcludedFromFee[sender] && recipient == uniswapV2Pair){
    ...
    function _getTValues(uint256 tAmount, uint256 taxFee, uint256 burnFee) private pure returns (uint256, uint256, uint256) {
        uint256 tFee = ((tAmount.mul(taxFee)).div(_GRANULARITY)).div(10000);
        uint256 tBurn = ((tAmount.mul(burnFee)).div(_GRANULARITY)).div(10000);
    ...
    function EnableMaxSell(address account) public onlyOwner {
        _isExcludedFromFee[account] = false;
    }

    function DisableMaxSell(address account) public onlyOwner {
