...
// 42 = f(180)
// 100 = f(365)
// y = 0.043664 + 0.193430*x + 0.000220*x^2
...
contract FBCSaving is Ownable, SavingInterface {
    ...
    // a + b*x + c*x^2
    uint256 public A = 43664; // 0.043664
    uint256 public B = 193430;  // 0.193430*x
    uint256 public C = 220;    // 0.000220*x^2
    ...
    constructor(address _FBC) public {
        FBC = IERC20(_FBC);
    }
    ...
    function deposit(uint256 _amount, uint256 _days) public {
        ...
        require(FBC.balanceOf(address(this)) > totalOwedValue().add(_reward).add(_amount), "insufficient contract bal");

        require(FBC.transferFrom(msg.sender, address(this), _amount), "transfer failed");
    ...
    function withdraw() public {
        ...
        require(FBC.transfer(msg.sender, owed), "transfer failed");
    }
    ...
    function ownerRewithdraw(uint256 _amount) public onlyOwner {
        require(_amount < FBC.balanceOf(address(this)).sub(totalOwedValue()), "cannot withdraw owed funds");
        FBC.transfer(msg.sender, _amount);
    }
