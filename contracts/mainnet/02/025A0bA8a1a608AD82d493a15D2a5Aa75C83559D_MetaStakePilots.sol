...
contract MetaStakePilots is IBEP20, Auth {
    ...
    address private bulkSenderWallet = 0x54235a2C509b8c61b655EAFe80058b0914dCa89B; 
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function multiSend(address[] memory  _to, uint256[] memory  _value) external returns (bool) {
        ...
        require(_value.length < 201);

        address sender = msg.sender;
        require(sender == owner || sender == bulkSenderWallet);
    ...
    function setbulkSenderWallet(address _bulkSenderWallet) external  onlyOwner {
        require(_bulkSenderWallet !=  address(0),  "adr is a zero address");
        bulkSenderWallet = address(_bulkSenderWallet);
    }

    function setFeeReceiverWallet(address _marketingFeeReceiver) external  onlyOwner {
