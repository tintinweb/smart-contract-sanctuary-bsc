...
contract OMG is ERC20, AccessControlEnumerable {
    ...
    uint256 public charityShare = 100;
    uint256 public liquidityShare = 200;
    uint256 private totalShares = 1000;

    uint256 public maxBalance = 3 * 10**7 * 10**18;
    ...
    event CharityDeposited(address charityWallet, uint256 charityBNB);
    ...
    event SharesUpdated(uint256 liquidityShare, uint256 marketingShare, uint256 charityShare);
    ...
    constructor(address marketing, address charity) ERC20("Zelon", "ZELON") {
    ...
    function _swapBack(uint256 amount) private swapping {
        ...
        uint256 charityBNB = receivedBNB * charityShare / totalBNBShares;
        ...
        if (charityBNB > 0) {
            (bool success,) = payable(charityWallet).call{value: charityBNB, gas: transferGas}("");
            if (success) {
                emit CharityDeposited(charityWallet, charityBNB);
            }
        }
    ...
    function setIsLimitExempt(address account, bool exempt) external onlyRole(ADMIN_ROLE) {
        require(account != pair, "Can't modify pair");
        isLimitExempt[account] = exempt;
        emit LimitExemptUpdated(account, exempt);
    }

    function setMaxBalance(uint256 amount) external onlyRole(ADMIN_ROLE) {
        require(amount >= 3 * 10**7 * decimals(), "Max balance too low");
        maxBalance = amount;
        emit MaxBalanceUpdated(amount);
    }

    function setTaxes(uint256 newBuyTax, uint256 newSellTax, uint256 newTransferTax) external onlyRole(ADMIN_ROLE) {
        require(newBuyTax <= 1000 && newSellTax <= 2000 && newTransferTax <= 1000, "Too high taxes");
        buyTax = newBuyTax;
        sellTax = newSellTax;
    ...
    function setShares(
        ...
        totalShares = liquidityShare + marketingShare + charityShare;
        require(totalShares > 0, "totalShares must be a positive number");
        emit SharesUpdated(liquidityShare, marketingShare, charityShare);
    }
    ...
    function setCharityWallet(address newCharityWallet) external onlyRole(ADMIN_ROLE) {
