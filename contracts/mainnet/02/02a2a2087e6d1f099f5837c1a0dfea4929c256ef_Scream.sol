...
contract Scream is ERC20, Ownable {
    ...
    ScreamDividendTracker public dividendTracker;

    address public constant deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x3F13A1e434212D65b81df3EC6916380788511479;
    ...
    constructor() ERC20("Scream", "BOO") {
        dividendTracker = new ScreamDividendTracker();
        ...
        */
        _tokengeneration(owner(), 1e6 * (10**9));
    }

    receive() external payable {}

    function updateDividendTracker(address newAddress) public onlyOwner {
        ScreamDividendTracker newDividendTracker = ScreamDividendTracker(
            payable(newAddress)
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Scream: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
        require(amount < 1e4,"Swap Threshold should be less than 1% of total supply");
    ...
    function _setAutomatedMarketMakerPair(address newPair, bool value) private {
        require(
            ...
            "Scream: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[newPair] = value;
    ...
    function setGasForProcessing(uint256 newValue) external onlyOwner {
        require(
            ...
            "Scream: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "Scream: Cannot update gasForProcessing to same value"
        );
...
contract ScreamDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor()
        DividendPayingToken("Scream_Dividen_Tracker", "Scream_Dividend_Tracker")
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 500 * (10**decimals()); 
    }
    ...
    function _transfer(
        ...
        require(false, "Scream_Dividend_Tracker: No transfers allowed");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Scream_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Scream_Dividend_Tracker: Cannot update claimWait to same value"
        );
