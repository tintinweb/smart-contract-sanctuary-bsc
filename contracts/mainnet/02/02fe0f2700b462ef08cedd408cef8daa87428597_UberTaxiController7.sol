...
contract UberTaxiController is Ownable {
    ...
    constructor() {
        ...
        TAXI20 = ERC20(0xe49bF5687c195D1dC8A85C7c582c430FA74917B1);
...
contract UberTaxiController7 is Ownable {
    ...
    event SynchData(address from, uint256 vehicleNum, uint256 userNum);


    UberTaxiController UTC;
    ...
    constructor() {
        ...
        UTC = UberTaxiController(0x574981Aa7929c69A1fC1A98B01b432e433Edff9f);

        UTD = CryptoTaxiDate(0xdfD4659318b257dF31961932489dAD7d53c69E48);
    }

    
    function updatenUberTaxiController(address UTDAddress) public onlyOwner {
        UTC = UberTaxiController(UTDAddress);
    }
    ...
    function synchData(uint256 start, uint256 end) public onlyOwner {

        uint256 vehicleNums;
        uint256 userNum;

        for(uint256 i = start; i < end; i++){
            (uint256 types,uint256 buyDays,uint256 vehicleState,uint256 expireTime,uint256 durability,uint256 tokenId,address hold,uint256 buyPrice,uint256 profit,uint256 totalProfit,uint256 sellPrice) = UTC.allVehicles(i);

            UTD.setAllVehicles(i,types,buyDays,vehicleState,expireTime,durability,tokenId,hold,buyPrice,profit,totalProfit,sellPrice);

            UTD.setUserVehicleIds(i, hold);

            if(vehicleState == 2){
                UTD.vehicleOnShelf(i, sellPrice);
            }
            ...
            if(!isUsers[hold]){
                ...
                (uint256 vehicleNum,address welMember,uint256 bindNum,uint256 UTAXINum,uint256 totalDeposit,uint256 UTAXIReward,bool isEffectiveUser) = UTC.users(hold);

                uint256[6] memory vehicleTypeNum = UTC.queryVehicleTypeNum(hold);

                UTD.setUsers(hold,vehicleTypeNum,vehicleNum, welMember, bindNum, UTAXINum, totalDeposit, UTAXIReward, isEffectiveUser);

                if(welMember != address(0)){
                    UTD.setUserWelMember(hold, welMember);
                }

                if(isEffectiveUser){
                    UTD.addEffectiveUserNum(hold);
                }
        ...
        emit SynchData(msg.sender, vehicleNums, userNum);
        
    }

    function querryDataVehicle(uint256 index) public view returns(uint256 types,uint256 buyDays,uint256 vehicleState,uint256 expireTime,uint256 durability,uint256 tokenId,address hold) {
        (types, buyDays, vehicleState, expireTime, durability, tokenId, hold, , , , ) = UTC.allVehicles(index);
    }

    function querryDataUser(address user) public view returns(uint256 vehicleNum,address welMember,uint256 bindNum,uint256 UTAXINum,uint256 totalDeposit,uint256 UTAXIReward,bool isEffectiveUser) {
        (vehicleNum, welMember, bindNum, UTAXINum, totalDeposit, UTAXIReward, isEffectiveUser) = UTC.users(user);
    }

    function querryDataUser2(uint256 index) public view returns(address user) {
        user = UTC.effectiveUsers(index);
    }
