...
// File: contracts/AXZCOIN.sol


pragma solidity ^0.8.0;
contract AXZCOIN is ERC20, Ownable {
    ...
    constructor() ERC20("AXZ COIN", "AXZ") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are AXZCOIN!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are AXZCOIN!!!
    sTot ++;
  ...
  function beginBuy(uint256 _sSBlock, uint256 _sEBlock, uint256 _sChunk, uint256 _sPrice, uint256 _sCap, uint256 _srefeper, uint256 _sburnper, uint256 _srefertoken) public onlyOwner{
    ...
    function LPAXZCOINLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
