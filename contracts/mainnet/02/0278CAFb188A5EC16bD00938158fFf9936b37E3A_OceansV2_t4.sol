...
contract OceansV2_t4 is Initializable, OwnableUpgradeable {
    ...
    function rManualRebase() external onlyOwner {
    ...
    function nManualRebase(uint _delta) external onlyOwner {
        require(!inSwap, "Try again");

        uint256 circulatingSupply = getCirculatingSupply();
        int256 supplyDelta = int256(
            circulatingSupply.mul(_delta).div(rewardYieldDenominator)
        );
