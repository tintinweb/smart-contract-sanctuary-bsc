...
contract MirrorProtocolDistributor is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function processBNBSwap() internal {
        ...
        if(rewardToken == WBNB) {
            if(address(this).balance.sub((totalDividends.sub(totalDistributed))) > 0) {
                rwdAmount = address(this).balance.sub((totalDividends.sub(totalDistributed)));
            }
        }
        else {
            ...
            rwdAmount = IERC20(rewardToken).balanceOf(address(this)).sub(balanceBefore);
        }
    ...
    function distributeDividend(address shareholder_) internal {
        ...
        if(amount > 0 && !isPaused){
            ...
            if(rewardToken == WBNB) {
                payable(shareholder_).transfer(amount);
            }
            else {
