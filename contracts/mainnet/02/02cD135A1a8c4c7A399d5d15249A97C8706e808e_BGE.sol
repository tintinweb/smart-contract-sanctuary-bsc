...
contract BGE is ERC20, Ownable {
    ...
    constructor(address dataAddr, address tokenAddr) ERC20("BGE", "BGE") {
        data = IData(dataAddr);

        _mint(
            data.string2addressMapping("perwallet"),
            1300 * 10**4 * 10**decimals()
        );
    ...
    function takeFee(
        ...
        if (fee[0] > 0) {
            ...
            _transfer(
                address(this),
                data.string2addressMapping("destroyAddress"),
                fee[1]
            );
            if (swap) {
                swapToken();
    ...
    function calFee(
        ...
        if (data.address2uintMapping(user) == 2) {
            return result;
        }
