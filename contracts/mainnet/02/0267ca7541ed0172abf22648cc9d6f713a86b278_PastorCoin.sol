...
contract PastorCoin is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Pastor";
    string constant _symbol = "PASTOR";
    ...
    function accuracyEnabled(bool _status, uint8 _interval) public onlyOwner {
        cooldownEnabled = _status;
        cooldownTimerInterval = _interval;
    }

    function betweenclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function callerStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function divisionTarget(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }    

    function externalsetSwapSettings(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  

    function falsesetFees(uint8 liquidityFee, uint8 marketingFee, uint8 denominator) external authorized {
        liqFee = liquidityFee;
        marketFee = marketingFee;
        feeDenominator = denominator;
    }

    function getOwnersetFeeExempt(address holder, bool exempt) external authorized {
