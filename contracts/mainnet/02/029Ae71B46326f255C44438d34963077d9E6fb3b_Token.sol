...
// 1.转账需要扣除1%的手续费
// 2.提交到博饼上，交易滑点买卖滑点1%，买入手续费1%到指定的合伙人钱包，卖出1%卖1%销毁黑洞
// 3.用户在操作卖出代币后，当日就不能操作转账。
// 4.新发一个叫HRD，发行的总量改为2100万个。
// 5.需要在现有的dapp上开发一个兑换的功能，旧合约的HR可以兑换成新合约的HRD，按照新合约价格来时时兑换。
...
contract Token is ERC20, Ownable {
    ...
    uint256 startTime;
    mapping(address=>mapping(uint256=>bool)) public isLock;
    bool public setLock = true;



    constructor() ERC20("HRD", "HRD") {

        address mainCol = 0x946EEf9Ab4852fF0A31dDE193176B1024445E93A;

        _mint(mainCol, 21000000 * 1e18);
        _router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
        address uAddr = 0x55d398326f99059fF775485246999027B3197955;
        

        feeWallet = burnWallet;
        partnerWallet = 0xeB2cFfe429AdDBe565758CBd038237D48FFCd086;


        resLimit =  10;
        startTime = 1662307200;

        startTimeForSwap = 1693843200;
    ...
    function setFeeRate(uint256 amount) public onlyOwner{
        feeRate = amount;
    }
    ...
    function setPartnerRate(uint256 amount) public onlyOwner{
        partnerRate = amount;
    }
    ...
    function setSetLock(bool flag) public onlyOwner{
        setLock = flag;
    }
    ...
    function setBlackList(address[] memory accounts, bool flag) public onlyOwner {
         for(uint i = 0;i<accounts.length;i++){
            address addr = address(accounts[i]);
            blackMap[addr] = flag; 
        }
    ...
    function getDiff() public view returns(uint256){
        return DateTime.diffDays(startTime,block.timestamp);
    }
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (from == address(uniswapV2Pair) && to == _router){
                ...
                if (to == address(uniswapV2Pair)){ 
                    require(!setLock || !isLock[from][diff],"locked");
                    isLock[from][diff] = true;
                    ...
                    totalFee = totalFee.add(partnerFee);
                ...
                uint256 taxFee = amount.mul(feeRate).div(1000);
                super._transfer(from, feeWallet, taxFee);
