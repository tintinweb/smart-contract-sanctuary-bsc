...
contract paySystem is Ownable {
    ...
    address public KELE_ADDRESS = 0x77E8a47994ED7E300b4bc9011E105b777C165080;
    ...
    function setKeleAddress(address account) public onlyOwner{
        require(account != address(0), "ERC20: fundAddress to the zero address");
        KELE_ADDRESS = account;
    }
    ...
    function payTokenSystem(uint _orderId,uint _payAmount,uint _commodityRegion) external payable {
        require(!payOrderIdOf[_orderId], "payOrderIdOf: Duplicate payment _orderId");
        uint FUNDAmount = 0;
        uint merchantAmount = 0;
        uint _payType = 2;

        uint _amountOutMin = _payAmount;
        
        IERC20(KELE_ADDRESS).safeTransferFrom(msg.sender, address(this), _amountOutMin);

        FUNDAmount = _amountOutMin * commodityRegionRate[_commodityRegion] / _decimals;
        merchantAmount = _amountOutMin - FUNDAmount;
        IERC20(KELE_ADDRESS).safeTransfer(FUND_ADDRESS, FUNDAmount);
