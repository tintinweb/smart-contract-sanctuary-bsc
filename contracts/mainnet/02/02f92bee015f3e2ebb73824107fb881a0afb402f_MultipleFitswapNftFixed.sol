...
// File: contracts\MultipleFitswapNft.sol

// Multiple FitSwap token

pragma solidity ^0.8.0;



contract MultipleFitswapNft is ERC1155, AccessControl {
    ...
    constructor() ERC1155("MultipleFitswapNft") {
        factory = msg.sender;
    }
...
// File: contracts\MultipleFitswapNftFixed.sol

// Multiple FitSwap Fixed Price Marketplace contract
...
contract MultipleFitswapNftFixed is Ownable, ERC1155Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;

	uint256 constant public PERCENTS_DIVIDER = 100;
	uint256 constant public swapFee = 2;	
	address public feeAddress; 

	uint256 public totalEarnedBNB = 0;
	...
	function initialize(address _feeAddress) external onlyOwner {
		...
		createCollection("Global", "https://cloudflare-ipfs.com/ipfs/QmdHRkLVKnDY5dxRUW5iymtb8stgTNqrY2Q72ucgdJawns", true);
	}
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(MultipleFitswapNft).creationCode;
        ...
        IMultipleFitswapNft(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function multipleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _amount, uint256 _price) public {
		...
		IMultipleFitswapNft nft = IMultipleFitswapNft(_collection);
	...
	function multipleDelist(uint256 _id) external {
		...
		IMultipleFitswapNft(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId, pairs[_id].balance, "delist Marketplace");
		pairs[_id].balance = 0;
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external payable {
		...
		if (pairs[_id].tokenAdr == address(0x0)) {
			...
			totalEarnedBNB = totalEarnedBNB + feeAmount;
        } else {
            IBEP20 governanceToken = IBEP20(pairs[_id].tokenAdr);	
		...
		IMultipleFitswapNft(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, item.tokenId, _amount, "buy from Marketplace");
