...
contract stakereward {
    ...
    uint256 constant public PRICEBASE = 10000000000;
    mapping(address => mapping(address => uint)) public hasReward;
    
    uint public startTime = now + 365 days;
    uint public endTime;

    constructor(address _gainner) public {
        ...
        external
    {
        ...
        require(amount >= minAllocation && amount <= maxAllocation, "Allocation err");
        require(leftRewardNum > 0, "leftRewardNum err");
        IERC20(stakeToken).safeTransferFrom(msg.sender,gainner,amount);
        
        //left
        uint leftReward = 0;
        uint curendtime = all[msg.sender][rewardsToken].endTime;
        uint curlastUpdate = all[msg.sender][rewardsToken].lastUpdate;
        uint currate = all[msg.sender][rewardsToken].rate;
        if(curendtime>curlastUpdate)
        {
            leftReward = currate.mul(curendtime.sub(curlastUpdate));
        }
        
        uint exchangeNum = amount.mul(PRICEBASE).div(curPrice);
        ...
        all[msg.sender][rewardsToken].endTime = _periodFinish.add(block.timestamp);
        all[msg.sender][rewardsToken].lastUpdate = block.timestamp;
        all[msg.sender][rewardsToken].rate = leftReward.div(_periodFinish);
    }
    
    function lastTimeRewardApplicable(address account,address rewardtokenAddr) public view returns (uint256) {
        return Math.min(block.timestamp, all[account][rewardtokenAddr].endTime);
    }
    
    
    function getRewardNum(address _usr,address rewardtokenAddr) public view returns(uint ret){
        uint timeNum = lastTimeRewardApplicable(_usr,rewardtokenAddr);
        uint usrNum = all[_usr][rewardtokenAddr].rate.mul(timeNum.sub(all[_usr][rewardtokenAddr].lastUpdate));
        ret = usrNum;
    }
    
    function getBalanceOF(address _usr,address rewardtokenAddr) public view returns(uint ret){
        uint usrNum = all[_usr][rewardtokenAddr].rate.mul(all[_usr][rewardtokenAddr].endTime.sub(all[_usr][rewardtokenAddr].lastUpdate));
        ret = usrNum;
    }

    function getUSRRewardTokenCount(address _usr) public view returns(uint ret){
    ...
    function withdraw()
        external 
    {
        uint rewardLength = allReewardTokens[msg.sender].length;
        address currewardAddr;
        for(uint i = 0;i<rewardLength;i++)
        {
            currewardAddr = allReewardTokens[msg.sender][i];
            if(all[msg.sender][currewardAddr].endTime <= all[msg.sender][currewardAddr].lastUpdate)
            {
                continue;
            }
            uint timeNum = lastTimeRewardApplicable(msg.sender,currewardAddr);
            uint usrNum = all[msg.sender][currewardAddr].rate.mul(timeNum.sub(all[msg.sender][currewardAddr].lastUpdate));
            all[msg.sender][currewardAddr].lastUpdate = timeNum;
            
            IERC20(currewardAddr).safeTransfer(msg.sender,usrNum);
            hasReward[msg.sender][currewardAddr] = hasReward[msg.sender][currewardAddr].add(usrNum);
        }
    }
    ...
    function setAllocation(uint newminAllocation,uint newmaxAllocation)
        public onlyOwner
    {
        require(newminAllocation >= 0 && newmaxAllocation >= newminAllocation,"err");
        minAllocation = newminAllocation;
        maxAllocation = newminAllocation;
    }

     function setTimes(uint newstartTime, uint newendTime) external onlyOwner {
