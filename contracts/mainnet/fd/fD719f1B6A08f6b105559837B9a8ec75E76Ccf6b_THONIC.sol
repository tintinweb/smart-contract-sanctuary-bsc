...
contract THONIC is IERC20, Ownable {
    ...
    string constant _name = "THONIC";
    string constant _symbol = "THONIC";
    ...
    constructor() {
        ...
        companyFeeReceiver = 0x97Fb30609742F91620FCBfA9F082Cfdf952E1307;
    ...
    function takeFee(
        ...
        if (to == pair) feeAmount = amount.mul(sellTotalFees).div(100);
        else feeAmount = amount.mul(buyTotalFees).div(100);
    ...
    function updateBuyFees(
        ...
        buyTotalFees = reward.add(company).add(liquidity);
    ...
    function updateSellFees(
        ...
        sellTotalFees = reward.add(company).add(liquidity);
    ...
    function updateSwapPercentages(
        ...
        totalSwap = reward.add(company).add(liquidity);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 companyAndRewardFee = companySwap.add(rewardSwap);
        uint256 tokensForCompanyAndReward = contractTokenBalance.sub(
            tokensToLiquidity
        );

        if (tokensForCompanyAndReward > 0) {
            swapTokensForTokens(tokensForCompanyAndReward, REWARD);
            ...
            uint256 tokensToCompany = swappedBusdBalance.sub(tokensToReward);
            if (tokensToReward > 0) {
            ...
            if (tokensToCompany > 0)
                IERC20(REWARD).transfer(companyFeeReceiver, tokensToCompany);
        }
    ...
    function setcompanyWallet(address _companyFeeReceiver) external onlyOwner {
        companyFeeReceiver = _companyFeeReceiver;
    }
