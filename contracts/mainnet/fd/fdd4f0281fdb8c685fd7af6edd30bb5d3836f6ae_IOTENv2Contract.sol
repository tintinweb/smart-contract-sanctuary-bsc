...
// Main token Contract
contract IOTENv2Contract is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    string private _name = "IOTEN"; // token name
    string private _symbol = "IOTN"; // token ticker
    ...
    uint256 public minTokenToSwap = _tTotal / 10000;
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 1; // 1% will be distributed among holder as token divideneds
    uint256 public liquidityFeeOnBuying = 1; // 1% will be added to the liquidity pool
    ...
    uint256 public liquidityFeeOnSelling = 1; // 1% will be added to the liquidity pool
    uint256 public burnFeeOnSelling = 1; // 1% will go to burn address

    // normal tax fee
    uint256 public redistributionFee = 0; // 0% will be distributed among holder as token divideneds
    uint256 public liquidityFee = 1; // 1% will be added to the liquidity pool
    uint256 public burnFee = 1; // 1% will go to burn address
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tLiquidFee = tAmount
            .mul(_currentLiquidityFee)
            .div(1e2);

        if(tLiquidFee > 0){
            sendFee(tLiquidFee, currentRate, address(this));
        }

        uint256 tBurnFee = tAmount
            .mul(_currentBurnFee)
            .div(1e2);

        if(tBurnFee > 0){
            sendFee(tBurnFee, currentRate, burnAddress);
        }
    }

    function sendFee(uint256 tAmount, uint256 currentRate, address receiver) private {
            uint256 rAmount = tAmount.mul(currentRate);
            if (_isExcludedFromReward[receiver])
                _tOwned[receiver] = _tOwned[receiver].add(tAmount);
            else _rOwned[receiver] = _rOwned[receiver].add(rAmount);

            emit Transfer(_msgSender(), receiver, tAmount);
    }

    // for automatic redistribution among all holders on each tx
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_currentRedistributionFee).div(1e2);
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 otherHalfLiquid = contractTokenBalance.sub(halfLiquid);

            // now is to lock into liquidty pool
            Utils.swapTokensForEth(address(dexRouter), halfLiquid);
