...
// File: contracts\PHCFinanceNFTLoot.sol

// Multiple PHCFinance Fixed PHCFinanceNFTLoot contract
...
contract PHCFinanceNFTLoot is ERC1155Holder, ERC721Holder {
    ...
    event PHCFinanceNFTLootStatus(bool boxStatus);
    event OwnerShipChanged(address newAccount);
    event PHCFinanceNFTLootNameChanged(string newName);
    event PHCFinanceNFTLootUriChanged(string newUri);
    ...
    function enableThisPHCFinanceNFTLoot() public onlyOwner {
        status = true;
        emit PHCFinanceNFTLootStatus(status);
    }

    function disableThisPHCFinanceNFTLoot() public onlyOwner {
        status = false;
        emit PHCFinanceNFTLootStatus(status);
    }
    ...
    function changePHCFinanceNFTLootName(string memory name) public onlyOwner {
        boxName = name;
        emit PHCFinanceNFTLootNameChanged(name);
    }

    function changePHCFinanceNFTLootUri(string memory _uri) public onlyOwner {
        boxUri = _uri;
        emit PHCFinanceNFTLootUriChanged(_uri);
    }
    ...
    function spin(uint256 times) public onlyHuman {
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            if (_cards[cardKey].cardType == 0) {  
                ...
                // ERC1155             
                IERC1155(_cards[cardKey].collectionId).safeTransferFrom(address(this), msg.sender, _cards[cardKey].tokenId, 1, "Your prize from PHCFinance PHCFinanceNFTLoot");
            }
    ...
    function emergencyWithdrawCard(address collectionId, uint256 tokenId, uint256 amount) public onlyOwner {
        ...
        if (card.cardType == 0) {
            ...
            IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, amount, "Reset PHCFinanceNFTLoot");
        }  
        ...
        for(uint256 i = 0 ; i < cardKeyCount(); i++) {
            ...
            if(_cards[key].amount > 0) {
                ...
                if (card.cardType == 0) {
                    ...
                    // withdraw multiple card
                    IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, card.amount, "Reset PHCFinanceNFTLoot");
                }   
...
// File: contracts\PHCFinanceNFTLootFactory.sol

// Multiple PHCFinance PHCFinanceNFTLoot Factory contract
 
pragma solidity ^0.8.0;



contract PHCFinanceNFTLootFactory is Ownable {
    using SafeMath for uint256;
    
    /** Create PHCFinanceNFTLoot fee (BNB) */
    ...
    /** Events */
    event PHCFinanceNFTLootCreated(address box_address, address owner, string name, string uri, address paymentToken, uint256 paymentTokenId, uint256 price);
    ...
    function createPHCFinanceNFTLoot(string memory _name, 
		string memory _uri,
		address _paymentCollection,
		uint256 _paymentTokenId, 
		...
		bytes memory bytecode = type(PHCFinanceNFTLoot).creationCode;
        ...
        PHCFinanceNFTLoot(lootbox).initialize(_name, _uri, _paymentCollection, _paymentTokenId, _price, msg.sender);
		boxes.push(lootbox);
		boxCreators[lootbox] = msg.sender;

		emit PHCFinanceNFTLootCreated(lootbox, msg.sender, _name, _uri, _paymentCollection, _paymentTokenId, _price);
	}
