...
contract CatVillsStaking is Ownable, ReentrancyGuard {
	using SafeMath for uint256;
	...
	function stake(uint256 _pid, uint256 _amount) external nonReentrant {    
		...
		pool.totalStakingAmount += stakingAmount;				
		...
		user.stakingAmount += stakingAmount;
		user.blockStart = block.number;
		
		emit Staked(msg.sender, address(pool.token) ,stakingAmount);
	}
	...
	function pendingRewardsFromPool(uint256 _pid, address _user) public view returns (uint256) {
		...
		if(user.stakingAmount > 0){
			...
			TotalShare = percent(user.stakingAmount, pool.totalStakingAmount, 4);
