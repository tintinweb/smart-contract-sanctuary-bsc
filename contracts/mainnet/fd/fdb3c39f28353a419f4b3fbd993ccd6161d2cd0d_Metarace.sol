...
contract Metarace is Context, IERC20, Ownable {
    ...
    string private _name = "Metarace Def1";
    string private _symbol = "$DRace";
    uint8 private _decimals = 8;

    address public marketingWallet = 0x10154cffB6b55523c92100E0acd9c8d281bEa469;
    address public developmentWallet = 0x1F42ee0e81F634AA968F47818b49C0B11876269E;
    address public rewardWallet = 0x9c4c2439E7836E00C92bCeE0C712dC050d981473;
    ...
    constructor () {
        ...
        totalBuy = _buyLiquidityFee.add(_buyMarketingFee).add(_buyDevelopmentFee).add(_buyRewardFee);
        totalSell = _sellLiquidityFee.add(_sellMarketingFee).add(_sellDevelopmentFee).add(_sellRewardFee);
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint _MarketingShare = _buyMarketingFee.add(_sellMarketingFee);
        uint _DevelopmentShare = _buyDevelopmentFee.add(_sellDevelopmentFee);
        ...
        uint256 totalBNBFee = totalShare.sub(_liquidityShare.div(2));

        uint amountBNBLiquidity = rBal.mul(_liquidityShare).div(totalBNBFee).div(2);
        uint amountBNBMarketing = rBal.mul(_MarketingShare).div(totalBNBFee);
        uint amountBNBDevelopment = rBal.mul(_DevelopmentShare).div(totalBNBFee);
        uint amountBNBReward = rBal.sub(amountBNBLiquidity).sub(amountBNBMarketing).sub(amountBNBDevelopment);
        ...
        if(amountBNBReward > 0)
            transferToAddressETH(rewardWallet, amountBNBReward);
    ...
    function setBuyFee(uint _newLP , uint _newMarket , uint _newDevelopment, uint _newReward) public onlyOwner {     
    ...
    function setSellFee(uint _newLP , uint _newMarket, uint _newDevelopment, uint _newReward) public onlyOwner {        
    ...
    function setDevelopmentWallet(address _newWallet) public onlyOwner {
