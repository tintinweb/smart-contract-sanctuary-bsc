...
contract PlayerBook is Governance {
    ...
    mapping (address => bool) public addrWork;
    mapping (address => mapping (uint => uint)) public _hasInvite;
    mapping (address => bool) public blackList;

    constructor(address new_lpToken) public {
    ...
    function setNodeList(address[] memory accountAddr,uint[] memory acnodetype)
        onlyGovernance
        public
    {
        require( acnodetype.length >= accountAddr.length, "err");

        for(uint i=0;i<accountAddr.length;i++)
        {
            if(!addrExist[accountAddr[i]])
            {
                continue;
            }
            _plyr[accountAddr[i]]._nodetype = acnodetype[i];
        }
    }

    function setReward(address[] memory accountAddr,uint[] memory _amounts)
        onlyGovernance
        public
    {
        require( _amounts.length >= accountAddr.length, "err");
        ...
        for(uint i=0;i<accountAddr.length;i++)
        {
            ...
            _plyr[accountAddr[i]].reward = _plyr[accountAddr[i]].reward.add(_amounts[i]);
        }
    }

    function setBlackList(address[] memory accountAddr,bool[] memory _amounts)
        onlyGovernance
        public
    {
        require( _amounts.length >= accountAddr.length, "err");
        ...
        for(uint i=0;i<accountAddr.length;i++)
        {
            ...
            blackList[accountAddr[i]] = _amounts[i];
        }
    }
    ...
    function mainBuy(address Code)
        ...
        public
    {
        if(!addrExist[Code] || addrWork[Code])
        {
            return;
        }
        ...
        if(_plyr[Code].laff == address(0))
        {
            return;
        }else{
            address _laff = _plyr[Code].laff;
            address _curAddr = Code;
            while(_laff != _teamWallet)
            {
                if(_plyr[_laff]._nodetype <= _plyr[_curAddr]._nodetype)
                {
                    _hasInvite[_laff][_plyr[_curAddr]._nodetype] = _hasInvite[_laff][_plyr[_curAddr]._nodetype].add(1);
                    if(_hasInvite[_laff][_plyr[_laff]._nodetype] >= 3 && _plyr[_laff]._nodetype < 6)
                    {
                        _plyr[_laff]._nodetype = _plyr[_laff]._nodetype+1;

                        _curAddr = _laff;
                        _laff = _plyr[_laff].laff;
                        if(_laff == address(0))
                        {
                            _laff = _teamWallet;
                        }
        ...
        external
    {
        ...
        require(reward > 0,"err");
        _plyr[addr].reward = 0;
        _plyr[addr].hasReward = _plyr[addr].hasReward.add(reward);

        IERC20(usdt).safeTransfer(msg.sender,reward);
    ...
    {
        return (_plyr[from].laff,_plyr[from].reward,_plyr[from]._nodetype,_plyr[from].hasReward);
    }
