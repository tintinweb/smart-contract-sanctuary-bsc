...
contract Konstake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function calculateNewRewards() public view returns (uint256) {
        ...
        return (((block.timestamp - pool.lastRewardTimestamp) * totalStaked) * apy / 100 / 1 days);
    }
    ...
    function updateApy(uint256 newApy) external onlyOwner {
        require(newApy <= 1000000000, "APY must be below 1000000000%");
        updatePool(0);
        apy = newApy;
    }
