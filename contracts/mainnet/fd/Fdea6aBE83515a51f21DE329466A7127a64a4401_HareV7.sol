...
contract HareV7 is ContextUpgradeable, IBEP20, OwnableUpgradeable {
    ...
    event MaxWalletUpdated(uint256 maxTxAmount);
    ...
    function setMaxTxAmount(uint256 newMaxAmount)
        external
        onlyOwner
    {
         uint256 newMaxTxAmount = newMaxAmount * 10**uint8(_decimals);
        _maxTxAmount = newMaxTxAmount;

        emit MaxTxAmountUpdated(_maxTxAmount);
    }

    function setMaxWallet(uint256 newMaxAmount) external onlyOwner {
        uint256 newMaxWalletAmount = newMaxAmount * 10**uint8(_decimals);
    ...
    function restoreAllFee() private {
		...
		_companyTotalFees = _previousTotalCompanyFees;
