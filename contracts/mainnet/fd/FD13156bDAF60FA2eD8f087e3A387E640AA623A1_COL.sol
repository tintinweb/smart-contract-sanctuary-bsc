...
contract COL is ERC20, Ownable {
    ...
    uint256 public foundationFee = 4; 
    ...
    address public nftReceiveAddress;
    address public daoReceiveAddress;
   ...
   TokenDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        address nftReceiveAddress_,
        address daoReceiveAddress_

    ) payable ERC20(name_, symbol_)  {
        uint256 totalSupply = totalSupply_ * (10**18);

        deadFinal = deadFinal_ * (10**18);

        //swapTokensAtAmount = totalSupply.mul(2).div(10**6); // 0.002%;
        swapTokensAtAmount = totalSupply_ * 10**18; // 
        ...
        nftReceiveAddress = nftReceiveAddress_;
        daoReceiveAddress = daoReceiveAddress_;
        ...
        excludeFromFees(nftReceiveAddress, true);
        excludeFromFees(daoReceiveAddress, true);
    ...
    function setDaoFee(uint256 val) public onlyOwner {
        daoFee = val;
    }
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 NFTFee = amount.mul(nftFee).div(100);
        if(NFTFee > 0) super._transfer(from, nftReceiveAddress, NFTFee);
        amountAfter = amountAfter.sub(NFTFee);


        uint256 DAOFee = amount.mul(daoFee).div(100);
        if(DAOFee > 0) super._transfer(from, daoReceiveAddress, DAOFee);
        amountAfter = amountAfter.sub(DAOFee);
