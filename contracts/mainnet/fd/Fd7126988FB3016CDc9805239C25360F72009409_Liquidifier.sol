...
contract Liquidifier is AccessControl, ReentrancyGuard, Operations {
    ...
    mapping(address => mapping(uint256 => RegisterDeposit)) public userdeposit;
    ...
    constructor(
        ...
        TransferHelper.safeApprove(
            pool,
            address(masterchef),
            type(uint256).max //this is the maximum in UINT256 or replace by 10000000000000000000000000000
        ); //mainnet
        ...
        returns (uint256 lp, uint256 treasury)
    {
        ...
        userdeposit[msg.sender][positionsId].loan = tokenamount;
        userdeposit[msg.sender][positionsId].deposit = ETHamount;
        ...
        (uint256 vaul, uint256 _cake) = transferToTreasury(token);

        lp = userinvestment[msg.sender][LP].lp;

        emit addETH(msg.sender, positionsId, liquidity, _cake);
    ...
    function increaseLiqETH(
        ...
        uint256 loanamount,
        uint256 _position
    ) external payable returns (uint256 lp, uint256 treasury) {
        if (openPositions[_position] != msg.sender) {
            revert nottheownerposition();
        }
        uint256 balanceBefore = address(this).balance;
        ...
        emit addETH(msg.sender, _position, liquidity, _cake);
        ...
        )
    {
        ...
        uint256 lptokens = userinvestment[msg.sender][LP].lp;
        uint256 _debt = userinvestment[msg.sender][LP].reward;
        ...
        (uint256 _userfunds, uint256 _treasury) = loanCalculation(
            LP,
            token,
            amountToken,
            amountETH,
        ...
        emit removeETH(msg.sender, _position, amountlp, _cake);
    }
    ...
    function partiallyRemoveLiquidityETH(
        ...
        uint256 amountlp,
        uint256 _loanamount,
        uint256 _position //BUSD to be withdrawn by the customer
    )
        external
        returns (
            uint256 amountToken,
        ...
        )
    {
        ...
        (uint256 _userfunds, uint256 _treasury) = partialloanCalculation(
            LP,
            token,
            amountToken,
            amountETH,
    ...
    function cakeDistribution(
    ...
    ) internal returns (uint256 rewardUser) {
        (uint256 liquidity1, uint256 liquidity2) = Operations.reserves(
            0x804678fa97d91B974ec2af3c843270886528a9E6 //cake-BUSD lp pool
        );

        uint256 busdvalue = dex.getAmountOut(_rewards, liquidity1, liquidity2);

        rewardUser = (busdvalue * (percentageUser)) / 100000;
        ...
        //Transfer to the user
        TransferHelper.safeTransfer(_token, _to, rewardUser);
    ...
    function makeaSwapToken(
        ...
        TransferHelper.safeTransfer(
            0x5430A9E06E2dcc4Fc2760522057fb46128AE463f, //Tennesse Token Address
            msg.sender,
            _panom
        );
...
interface IVault {
    ...
    function setLiquidifier(address _liquidifier) external;
}
...
contract Operations is AccessControl, ReentrancyGuard {
    ...
    function makeaSwapPanom(
        ...
        if (busdAmount != 0) {
            address[] memory path = Operations.makePath(
                busd,
                0x224c764610AB246aE7a6262649057358A9f05b05 //Tenesse token
            );
            uint256[] memory swap = IUniswap(dex).swapExactTokensForTokens(
                busdAmount,
                0,
    ...
    function bnbtoBusd(
        ...
        uint256 _ethAmount,
        uint256 amountToken
    ) internal view returns (uint256 percentageUser, uint256 percentageLoan) {
        (uint256 reserve1, uint256 reserve2) = Operations.reserves(_lp);
        uint256 _amount = IUniswap(dex).getAmountOut(
            _ethAmount,
            reserve1,
            reserve2
        );
