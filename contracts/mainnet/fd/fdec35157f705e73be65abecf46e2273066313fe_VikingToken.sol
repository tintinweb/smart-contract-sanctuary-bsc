...
contract VikingToken is Context, IERC20, Ownable {
    ...
    string private _name = "Viking token";
    string private _symbol = "FRGG";
    ...
    address payable public marketingWallet = payable(0x1113C187f21e6BD3198052F48DFC05040006Fe33);

    uint256 public _devFee = 2;
    uint256 private _previousDevFee = _devFee;
    address payable public devWallet = payable(0xB7887144Dd06aE69f7Adf4a98701cA6b6f0EA186);
    ...
    function _transfer(
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && to == uniswapV2Pair && swapAndLiquifyEnabled) {
            ...
            //add liquidity and swap for marketing, dev
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityShare = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_devFee).add(_marketingFee));
        ...
        devWallet.transfer(balanceAfterSwap.mul(_devFee).div(_marketingFee.add(_devFee)));
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(devFee).add(marketingFee) <= 20, "tax too high");
        _taxFee = taxFee;
