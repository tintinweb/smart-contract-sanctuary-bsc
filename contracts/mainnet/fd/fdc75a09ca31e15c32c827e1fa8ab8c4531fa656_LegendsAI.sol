...
Telegram : https://t.me/Legends_AI
...
contract LegendsAI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LegendsAIDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LegendsAI", "LegendsAI") {
        marketingWallet = payable(0xbb35bA8fb072391F86266d0Bc8aCc06Cb320bd6c);
        buybackfeeWallet = payable(0x5B325C1541678BD4a76B176E96EB33DC8779844F);
        ...
        dividendTracker = new LegendsAIDividendTracker(
            payable(this),
            router,
...
contract LegendsAIDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    LegendsAI public LegendsAIContract;
    ...
    constructor(
        ...
        defaultToken = token;
        LegendsAIContract = LegendsAI(mainContract);
    ...
    function _transfer(
        ...
        require(false, "LegendsAI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "LegendsAI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LegendsAI contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LegendsAIContract);

            uint256 prevBalance = LegendsAIContract.balanceOf(address(this));
            ...
            {
                uint256 received = LegendsAIContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LegendsAIContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
