...
// 普通场竞赛
contract NormalRace is Caller, Fallback {
    ...
    function injectAsset(address token, uint256 amount) external payable {
        ...
        if (address(0) != token) {
            ...
            if (!exist) {
                (tokens, amounts) = config.getNormalTicketFeesDiscount();
                for (uint256 i = 0; i < tokens.length; i++) {
                    if (token == tokens[i] && 0 < amounts[i]) {
                        exist = true;
                        break;
    ...
    function signUp(uint256[] memory tokenIds) external {
        ...
        uint8 registed;
        uint256 tokenId;
        INormalTicket.MintInfo memory mintInfo;
        address burn = config.getNormalBurnAddress();
        INormalTicket ticket = config.getNormalTicket();
    ...
    function exit(uint256 tokenId) external {
        ...
        emit ExitEvent(address(config.getNormalTicket()), msg.sender, tokenId);
    }
    ...
    function clear(
        ...
        address owner;
        uint256 tokenId;
        address ticket = address(config.getNormalTicket());
    ...
    function _setChampion() private {
        ...
        //送高级场门票
        ISeniorTicket seniorTicket = config.getSeniorTicket();
        seniorTicket.mintBatch(raceInfo.championAddress, 1);
    }
