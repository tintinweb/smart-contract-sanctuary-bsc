...
/**
    Website : https://foxplaytoken.com/
    Telegram : https://t.me/foxplaytoken
...
contract FOX is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedFromFeesFOX;
    mapping (address => bool) private _isExcludedFromMaxBalanceFOX;

    uint256 private constant _maxFeesFOX = 12;
    uint256 private _totalFeesFOX;
    uint256 private _totalFeesFOXContract;
    uint256 private _burnFeesFOX;
    uint256 private _liquidityFeesFOX;
    uint256 private _marketingFeesFOX;
    
    uint256 private _maxBalanceWalletFOX;
    ...
    constructor () {
        ...
        _isExcludedFromFeesFOX[owner()] = true;
        _isExcludedFromFeesFOX[address(this)] = true;
        
        _isExcludedFromMaxBalanceFOX[owner()] = true;
        _isExcludedFromMaxBalanceFOX[address(this)] = true;
        _isExcludedFromMaxBalanceFOX[uniswapV2Pair] = true;

        _liquidityFeesFOX = 1;
        _marketingFeesFOX = 11;
        _burnFeesFOX = 0;
        _totalFeesFOX = _liquidityFeesFOX.add(_marketingFeesFOX).add(_burnFeesFOX);
        _totalFeesFOXContract = _liquidityFeesFOX.add(_marketingFeesFOX);

        _liquidityThreshhold = 3000 * 10**6 * 10**_decimals;
        _maxBalanceWalletFOX = 1000000 * 10**6 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeesFOX).add(_marketingFeesFOX) <= _maxFeesFOX, "Fees are too high.");
        _liquidityFeesFOX = newLiquidityFee;
        _totalFeesFOX = _liquidityFeesFOX.add(_marketingFeesFOX).add(_burnFeesFOX);
        _totalFeesFOXContract = _liquidityFeesFOX.add(_marketingFeesFOX);
        emit SetLiquidityFee(_liquidityFeesFOX);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeesFOX.add(_burnFeesFOX).add(newMarketingFee) <= _maxFeesFOX, "Fees are too high.");
        _marketingFeesFOX = newMarketingFee;
        _totalFeesFOX = _liquidityFeesFOX.add(_marketingFeesFOX).add(_burnFeesFOX);
        _totalFeesFOXContract = _liquidityFeesFOX.add(_marketingFeesFOX);
        emit SetMarketingFee(_marketingFeesFOX);
    }

    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_liquidityFeesFOX.add(newBurnFee).add(_marketingFeesFOX) <= _maxFeesFOX, "Fees are too high.");
        _burnFeesFOX = newBurnFee;
        _totalFeesFOX = _liquidityFeesFOX.add(_marketingFeesFOX).add(_burnFeesFOX);
        emit SetBurnFee(_burnFeesFOX);
    }
    ...
    function _transfer(
        ...
        if(
            ...
            !_isExcludedFromMaxBalanceFOX[to]  // is excludedFromMaxBalance
        ){
            require(
                balanceOf(to).add(amount) <= _maxBalanceWalletFOX,
        ...
        if(
            ...
            _totalFeesFOXContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeesFOX[from] || _isExcludedFromFeesFOX[to])
            && _totalFeesFOX > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeesFOXContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeesFOX).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeesFOX).div(_totalFeesFOXContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeesFOX).div(_totalFeesFOXContract);
