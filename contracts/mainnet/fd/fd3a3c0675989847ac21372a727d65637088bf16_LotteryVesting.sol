...
// File contracts/LotteryVesting.sol

pragma solidity ^ 0.8.14;
/*
This contract is reserved for Linear Vesting to the Honey Lottery.
*/
contract LotteryVesting is
Initializable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 private assignedHoneyToken;
    ...
    function initialize(address _honeyAddress, address _admin)
    public
    initializer
    {
        honeyToken = IERC20Upgradeable(_honeyAddress);
    ...
    function addEntityVestingBatch(
        ...
        assignedHoneyToken += _sumTotalSupplies;

        honeyToken.safeTransferFrom(
            msg.sender,
            address(this),
            _sumTotalSupplies
        );
    ...
    onlyRole(UPDATER_ROLE)
    {
        ...
        assignedHoneyToken += _totalSupply;
        ...
        honeyToken.safeTransferFrom(msg.sender, address(this), _totalSupply);
    }
    ...
    {
        sendHoneyTokenToEntity(_entity);
    ...
    public
    entityRuleExists(msg.sender)
    whenNotPaused
    {
        sendHoneyTokenToEntity(msg.sender);
    }

    function sendHoneyTokenToEntity(address _entity) private {
        uint256 unclaimedAmount = getClaimableHoney(_entity);
        if (unclaimedAmount == 0) return;

        Rule storage entityRule = entitiesVesting[_entity];
        entityRule.claimed += unclaimedAmount;

        assignedHoneyToken = assignedHoneyToken - unclaimedAmount;
        honeyToken.safeTransfer(_entity, unclaimedAmount);

        emit ClaimedTokens(_entity, unclaimedAmount);
    }

    function transferUnassignedHoney() external onlyRole(RECOVERY_ROLE) {
        uint256 unassignedTokens = getUnassignHoneyTokensAmount();

        if (unassignedTokens == 0) return;

        honeyToken.safeTransfer(msg.sender, unassignedTokens);
    }

    function getClaimableHoney(address _entity)
    public
    view
    ...
    function getUnassignHoneyTokensAmount() public view returns(uint256) {
        return honeyToken.balanceOf(address(this)) - assignedHoneyToken;
    }
