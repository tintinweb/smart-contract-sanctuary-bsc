...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    mapping (address => bool) public _isRouter;
    ...
    uint256 public cooldownTimerInterval = 5 minutes;
    ...
    function viewCooldownLeft(address _address) public view returns (uint256) {uint256 amt; amt = cooldownTimer[_address].sub(lastTxTime[_address]); return amt;}
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldSwapBack(sender, recipient, amount) && !_isInternal[sender]){swapBack(swapThreshold); swapTimes = 0; }
        performTransfer(sender, recipient, amount);
        return true;
    }

    function performTransfer(address sender, address recipient, uint256 amount) internal {
        uint256 tAmount = amount.mul(_PerFragment);
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal {
        if(sender == pair && cooldownEnabled && !_isFeeExempt[recipient] && cooldownTimer[recipient] < block.timestamp){
            cooldownTimer[recipient] = block.timestamp + cooldownTimerInterval;
            lastTxTime[recipient] = block.timestamp;}
        if(sender != pair && cooldownEnabled && !_isFeeExempt[sender]) {
        ...
        if(sender != pair && cooldownEnabled && !_isFeeExempt[sender] && !_isFeeExempt[recipient])
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal returns (uint256) {
        ...
        if(isCont(sender) && !_isRouter[sender] && !_isInternal[sender] && !_isRouter[recipient] || isCont(recipient) &&
         !_isRouter[recipient] && !_isInternal[recipient] || startedTime > block.timestamp){
    ...
    function shouldSwapBack(address sender, address recipient, uint256 amount) internal view returns (bool) {
        ...
        && tAmount >= minAmounttoSwap && _balances[address(this)].div(_PerFragment) >= swapThreshold && swapTimes >= minSells;
    }
