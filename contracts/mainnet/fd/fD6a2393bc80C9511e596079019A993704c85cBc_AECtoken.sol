...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        foundation = address(0xd34Ffd85BbFBb70a83e5F1CeA05AB674c84f8604);
        defaultNode = address(0x7f7D163Ffa843467c68E583F4ea08A4Cb1F48824);
        lpFee = 50;
    }
    ...
    function _transfer(
        ...
        if (sendExcludeFee[sender] || receiveExcludeFee[recipient]) {
            bool isSell;
            if(receiveExcludeFee[recipient]){
                isSell = true;
            }

            _transferIncludeFee(sender, recipient, amount,isSell);
    ...
    function _transferIncludeFee(address sender, address recipient, uint256 amount,bool isSell) internal {

        _balances[sender] = _balances[sender].sub(amount); 
        //95% recipient
        uint256 recipientFee = 1000 - lpFee;
        uint256 addRate = amount.mul(recipientFee).div(1000);
        _balances[recipient] = _balances[recipient].add(addRate);
        emit Transfer(sender, recipient, addRate);

        //5% Dao
        uint256 temp = amount.mul(lpFee).div(1000);
        if(isSell)
        {
...
contract AECtoken is ERC20 {
    ...
    constructor() ERC20("An Energy Coin", "AEC") {      
