...
contract FlashBorrowerMock is IERC3156FlashBorrower {
    ...
    function onFlashLoan(
        ...
        address[] memory addresses = decodeAddressArray(data);
        for (uint256 i = 0; i < addresses.length - 1; i++) {
            address input = addresses[i];
            address output = addresses[i + 1];
            swap(input, output);
        }
    ...
    function encodeAddresses(address[] calldata path) public pure returns (bytes memory data) {
        for (uint256 i = 0; i < path.length; i++) {
            data = abi.encodePacked(data, path[i]);
        }
    }

    function decodeAddressArray(bytes calldata data)
        public
        pure
