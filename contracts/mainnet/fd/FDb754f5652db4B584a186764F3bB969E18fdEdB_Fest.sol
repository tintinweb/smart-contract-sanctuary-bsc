...
contract Fest is Context, IBEP20, Ownable {
    ...
    mapping (address => uint256) public _lastHotTx;
    ...
    string private constant _name     = "Fest";
    string private constant _symbol   = "Fst";
    uint8  private constant _decimals = 9;
    

    // buy tax
    uint256 public  _taxFee       = 1; // 1% will be redistributed to holders
    uint256 public  _burnFee      = 1; // 1% will be burned
    uint256 public  _liquidityFee = 3; // 3% marketing fee

    // sell tax
    uint256 public  _taxFeeSell       = 3;  // 3% will be redistributed to holders
    uint256 public  _burnFeeSell      = 2;  // 2% will be burned
    uint256 public  _liquidityFeeSell = 15; // 15% marketing fee

    // at start, 100% of liq fee is allocated for marketing
    uint256 public  _percentageOfLiquidityForMarketing = 1000;

    // sell cooldown
    uint256 public _cooldownPeriod   = 15 minutes;
    uint256 public _hotlistTxAmount  = _tTotal / 1000; // 0.1% of total supply
    ...
    constructor () {
        ...
        _marketingWallet = 0x71926da1492A5551d56a89C5fB86b11B08e9e4Dc; // set marketing wallet
    ...
    function setBuyTaxes(uint256 taxFee, uint256 burnFee, uint256 liquidityFee) external onlyOwner {
        _taxFee       = taxFee;
        _burnFee      = burnFee;
        _liquidityFee = liquidityFee;
    }

    function setSellTaxes(uint256 taxFee, uint256 burnFee, uint256 liquidityFee) external onlyOwner {
        _taxFeeSell       = taxFee;
    ...
    function setHotlistTxAmount(uint256 amount) external onlyOwner {
        _hotlistTxAmount = amount;
    }
    ...
    function _transfer(
        ...
        // hot sell cooldown
        if (to == uniswapV2Pair && amount > _hotlistTxAmount) {
            require(block.timestamp - _lastHotTx[from] >= _cooldownPeriod, "Still in cooldown period");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 bnbForMarketing = newBalance.div(1000).mul(_percentageOfLiquidityForMarketing);

        emit MarketingSent(_marketingWallet, bnbForMarketing);
        payable(_marketingWallet).transfer(bnbForMarketing);
        
        uint256 bnbForLiquidity = newBalance.sub(bnbForMarketing);
