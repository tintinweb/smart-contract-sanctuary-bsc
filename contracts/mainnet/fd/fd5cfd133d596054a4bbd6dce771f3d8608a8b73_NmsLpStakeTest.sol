...
contract NmsLpStakeTest is Pausable, Ownable, ReentrancyGuard {
    ...
    function _updatePool(uint amount, uint _timestamp) internal returns (uint){
        ...
        // console.log("HERE2", n1);
        if (n1 > 2) {
            uint num10 = n1.div(3);
            uint rem = n1.sub(3 * num10);
            // console.log(n1, num10, rem);
            // console.log(tokenRewardAmount, rewardPercent, pow(10000 - (rewardPercent), 2), pow(10000, 3));
            // console.log(tokenRewardAmount.mul(pow(10000 - (rewardPercent), 2)).div(pow(10000, 3)).mul(rewardPercent));
            for (uint i = 0; i < num10; i++) {
                _setTokenRewardAmount(tokenRewardAmount.mul(pow(10000 - (rewardPercent), 2)).div(pow(10000, 3)).mul(rewardPercent));
            }
            // console.log("HERE");
            if (rem!=0) {
    ...
    function _calcReward(uint amount, uint _timestamp) internal view returns(uint, uint, uint, uint){
        ...
        // console.log("HERE5", block.timestamp.sub(pool.lastRewardChange), block.timestamp.sub(pool.lastRewardChange) < 24 hours);
    ...
    function _deposit(address _user, uint _amount) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.rewardDebt > user.amount.mul(pool.accTokenPerShare).div(1e18) ? 0 : user.amount.mul(pool.accTokenPerShare).div(1e18).sub(user.rewardDebt);
            // uint256 pending =
    ...
    function withdrawRewards() external nonReentrant whenNotPaused {
        ...
        // console.log("balBefWithdraw", user.amount, user.rewardDebt, pool.accTokenPerShare);
        // console.log(user.amount.mul(pool.accTokenPerShare).div(1e18), user.rewardDebt > user.amount.mul(pool.accTokenPerShare).div(1e18));
