...
contract InsuranceFi is Context, Ownable, IBEP20, BEP20Detailed {
    ...
    uint256 public insurancePercent = 20;
    uint256 public marketingPercent = 40;
    uint256 public stableUSDPercent = 40;

    uint256 public constant ONE_HUNDRED_PERCENT = 100;


    address payable public marketingAddress = payable(0x8473311b3f5456d7398A30e47a87E1b3BD217cEF);
    address payable public insuranceAddress = payable(0x0Ac93363ED5d73dA3fD4931D9c3EBd7Ad47B4e9A);
    address payable public stableUSDAddress = payable(0xA7Ae47616642ac8D23Ef5f49d611811E8D9FDF9E);
    ...
    constructor () BEP20Detailed("InsuranceFi", "IF", 18) {
        _owner = msg.sender ;
    ...
    function setBuyFeePercent(uint256 newFee) external onlyOwner {
        require(newFee <= 10, "Buy fee should be less than 4%");
        buyFee = newFee;

    }

    function setSellFeePercent(uint256 newFee) external onlyOwner {
        require(newFee <= 10, "Sell fee should be less than 4%");
        sellFee = newFee;
    }
    ...
    function setFees(
        ...
        require(stableUSD + marketing + insurance == ONE_HUNDRED_PERCENT);
        stableUSDPercent = stableUSD;
    ...
    function setInsuranceAddress(address payable wallet) external onlyOwner
    {
        require(wallet != insuranceAddress,"insuranceAddress wallet is already that address");
        require(!isContract(wallet),"insuranceAddress wallet cannot be a contract");
        require(wallet != address(0), "Can't set to dead address!");
        insuranceAddress = wallet;
    }

    function setStableUSDAddress(address payable wallet) external onlyOwner
    {
        require(wallet != stableUSDAddress,"stableUSDAddress wallet is already that address");
        require(!isContract(wallet),"stableUSDAddress wallet cannot be a contract");
        require(wallet != address(0), "Can't set to dead address!");
        stableUSDAddress = wallet;
    }
    ...
    {
        require(_numTokensSellToFee >= 1 * 10**18 && _numTokensSellToFee <= 100000 * 10**18, "Threshold must be set within 1 to 100,000 tokens");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 insurancePart = contractTokenBalance.mul(insurancePercent).div(ONE_HUNDRED_PERCENT);
        uint256 swapAmount = contractTokenBalance.sub(insurancePart);
        _transfer(address(this),insuranceAddress, insurancePart);
        swapTokensForEth(swapAmount); // <- this breaks the ETH -> IF swap when swap+liquify is triggered
        uint256 remainPercent = ONE_HUNDRED_PERCENT.sub(insurancePercent);
        uint256 marketingPart = address(this).balance.mul(marketingPercent).div(remainPercent);
        payable(marketingAddress).transfer(marketingPart);
        payable(stableUSDAddress).transfer(address(this).balance);
