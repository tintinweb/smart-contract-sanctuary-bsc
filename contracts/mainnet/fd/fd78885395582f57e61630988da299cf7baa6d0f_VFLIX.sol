...
contract VFLIX is Context, IERC20, Ownable {
    using Address for address;
    address payable public marketingWalletAddress = payable(0x120e5375901eBD0149D3b54C1c3C0a850716E428); // Marketing Wallet Address
    ...
    string private constant _name = "VFLIX";
    string private constant _symbol = "VFLIX";
    ...
    uint256 public totalSwapableSaleFee = _saleTaxFee + _saleLiquidityFee + _saleMarketingFee ;

    uint256 public _transferTaxFee = 0;
    uint256 public _transferLiquidityFee = 0;
    uint256 _transferMarketingFee = 0;

    uint256 public totalSwapableTransferFee = _transferTaxFee + _transferLiquidityFee + _transferMarketingFee ;
    ...
    function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) 
    external isAuth() 
    {
        _taxFee = taxFee;
        ...
        totalSwapableFee = _taxFee + _liquidityFee + _marketingFee ; 
        require(totalSwapableFee <= 25, "Must be less than 25% total");
        emit Log("We have updated total buy tax", totalSwapableFee);
    }

    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) 
    external isAuth() 
    {
        _saleTaxFee = taxFee;
        ...
        totalSwapableSaleFee = _saleTaxFee + _saleLiquidityFee + _saleMarketingFee;
        require(totalSwapableSaleFee <= 10, "Must be less than 10% total");
        emit Log("We have updated total sale tax", totalSwapableSaleFee);
    }

    function setTransferFeePercent (uint256 taxFee, uint256 liquidityFee, uint256 marketingFee)
    external isAuth()
    {
        ...
        totalSwapableTransferFee = _transferTaxFee + _transferLiquidityFee + _transferMarketingFee;
        require(totalSwapableTransferFee <= 10, "Must be less than 10% total");
