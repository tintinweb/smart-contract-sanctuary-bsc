...
contract MetaswapstarToken is Context, IERC20, Ownable {
    ...
    uint256 private _mintMasterCHef = 0 * 10 ** 18;
    uint256 private _mintNodeReward = 0 * 10 ** 18;
    ...
    function _mintMasterChef(address account, uint256 amount) internal {
        require(account != address(0), "BEP20: mint to the zero address");
        require(_tTotal<_maxSupply, "exceed max supply");// reflected total
		require(_mintMasterCHef<_maxMasterChefMint, "exceed max masterchef supply");// reflected total
        uint256 currentRate =  _getRate();
        uint256 rAmount = amount.mul(currentRate);
		_tTotal = _tTotal.add(amount);
        _mintMasterCHef = _mintMasterCHef.add(amount);
		_rOwned[account] = _rOwned[account].add(rAmount);
		emit Transfer(address(0), account, amount);
    }

    function _mintAdmin(address account, uint256 amount) internal {
        require(account != address(0), "BEP20: mint to the zero address");
		require(_tTotal<_maxSupply, "exceed max supply");// reflected total
        require(_mintNodeReward<_maxNodeReward, "exceed max node reward");
        uint256 currentRate =  _getRate();
        uint256 rAmount = amount.mul(currentRate);
		_tTotal = _tTotal.add(amount);
        _mintNodeReward = _mintNodeReward.add(amount);
		_rOwned[account] = _rOwned[account].add(rAmount);
		emit Transfer(address(0), account, amount);
    }
