...
pragma solidity ^ 0.6.2;
 ...
 contract NBGFARM is Ownable {
 	...
 	//Create   Farms
 	function create(address token_staking, address token_reward, uint256 _pembagi,uint256 _daylock) public onlyOwner {
	...
	function updatePembagi(uint256 _pid,uint256 _pembagi) public onlyOwner {
 	...
 	function updatePool(uint256 _pid) public {
 		...
 		uint256 lpSupply = pool.totalLP;
 		uint256 rewardAvailable = IERC20(pool.tokenReward).balanceOf(address(this));
 	...
 	function deposit(uint256 _pid, uint256 _amount) public {
 		...
 		if (user.amount > 0) {
 			...
 			if (pending > 0 && remaining == 0) {
 				unsend = 0;
 				IERC20(pool.tokenReward).transfer(address(msg.sender), pending);
    ...
    function balanceAvailable(uint256 _pid, address _user) external view returns(uint256) {
 		UserInfo storage user = userInfo[_pid][_user];
        PoolInfo storage pool = poolInfo[_pid];
