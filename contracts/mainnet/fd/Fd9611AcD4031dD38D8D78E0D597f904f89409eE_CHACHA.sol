...
contract CHACHA is IBEP20, Ownable {
    ...
    function createLiquidity(uint256 tokenAmount) payable public lockTheSwap {
        ...
        uint256 tokensLeft = balanceOf(address(this)).sub(beforeToken);
        uint256 bnbLeft = (address(this).balance).sub(beforeBNB);
        _transfer(address(this), msg.sender, tokensLeft);
        payable(msg.sender).transfer(bnbLeft);
    }
