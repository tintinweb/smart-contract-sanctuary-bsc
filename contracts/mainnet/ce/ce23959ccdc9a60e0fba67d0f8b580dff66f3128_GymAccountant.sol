 ...
 */
contract GymAccountant is IAccountant, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Mapping for store borrowed amount by type (1 - VaultBank, 2 - Farming, 3 - SinglePool)
    mapping(uint256 => mapping(address => uint256)) public borrowedAmountByType;
     ...
     */
    function claimPendingRewards(uint32 _type) external nonReentrant {
        require(
            mlmAddress != address(0),
            "GymAccountant:: GymMLM contract address is zero address"
        );

        uint256 _pendingRewards = IGymMLM(mlmAddress).getPendingRewards(msg.sender, _type);
    ...
    function updateBorrowedAmount(
        ...
        _updateBorrowedAmount(_userAddress, _amount, _type, _increase);
    }
    ...
    function _claimPendingRewards(
        ...
        IERC20Upgradeable(tokenAddress).safeTransfer(_userAddress, _pendingRewards);

        _updateBorrowedAmount(_userAddress, _pendingRewards, _type, true);
    }
    ...
    function _updateBorrowedAmount(
        ...
        uint256 returnAmount = borrowedAmountByType[_type][_userAddress];
