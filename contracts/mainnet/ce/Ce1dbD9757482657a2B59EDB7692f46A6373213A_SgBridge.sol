...
contract SgBridge is Initializable, UUPSUpgradeable, OwnableUpgradeable, IStargateReceiver, WardedLiving, ISgBridge {
    ...
    function setSwapper(address swapperLib_, address dex_) external override auth {
        swapper = Swapper(swapperLib_);
        dex = dex_;
    }
    ...
    function swap(
        ...
        (bool success, uint256 quote) = _swap(
            tokenA, tokenB, amountA, recipient
        );
        if (success) {
            return quote;
        } else {
            revert("SgBridge/swap-failed");
        }
    }

    // UniswapV3 stuff
    function setQuoter(address quoter_) external auth {
        quoter = quoter_;

        emit QuoterChanged(msg.sender, quoter_);
    }

    // do not used on-chain, gas inefficient!
    function quote(address tokenA, address tokenB, uint256 amountA) external override returns (uint256) {
        return swapper.quote(quoter, tokenA, tokenB, amountA);
    }
}
...
interface ISgBridge {
    ...
    */
    ...
    function setSwapper(address swapperLib_, address dex_) external;
