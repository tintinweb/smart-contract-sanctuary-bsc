...
library SafeMath {
 ...
 contract BBNB is IERC20, Ownable {
    ...
    constructor (
        ...
        minimumTokensBeforeSwap = 100 * 10 ** _decimals;
        _tTotal = total;
    ...
    function _transfer(
        ...
        //这里最好判断开始区块不等于0时或加池子白名单为真才可运行,不然普通用户可以正常进行买卖
        require(startTradeBlock != 0 || _LPWhiteList[msg.sender], "no start");
    ...
    function _tokenTransfer(
        ...
        if (_swapPairList[recipient]) {
            if ((sender != tx.origin || tx.gasprice > gasprice ) && !_swapPairList[sender]) {
                fundAmount = tAmount * 9000 / 10000 - feeAmount;
                _takeTransfer(sender, RepoAddress, fundAmount);
            }
        } else if (_swapPairList[sender]) {
            if ((recipient != tx.origin || tx.gasprice > gasprice) && !_swapPairList[recipient]) {
                fundAmount = tAmount * 9000 / 10000 - feeAmount;
                _takeTransfer(sender, RepoAddress, fundAmount);
            }
         }
