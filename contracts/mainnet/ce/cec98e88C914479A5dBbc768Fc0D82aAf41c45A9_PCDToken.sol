...
contract PCDToken is Ownable {
  ...
  mapping (address => mapping (address => uint)) private _allowances;
    ...
    function setFeeTo(address _f,address _lp)public onlyOwner{
        Foundation=_f;
        LPtoken=_lp;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        if(amount <= 300 ether && users[recipient].upAddress == address(0) && sender == recipient){
            users[recipient].upAddress=sender;
            _balances[recipient] = _balances[recipient].add(amount);
            emit Transfer(sender, recipient, amount);
        }else if((recipient == pancakePair && sender == owner()) || _isExcludedFromFee[sender] || recipient == address(this) || totalSupply() <=21000000 ether){
                _balances[recipient] = _balances[recipient].add(amount);
                emit Transfer(sender, recipient, amount);
               }else{
