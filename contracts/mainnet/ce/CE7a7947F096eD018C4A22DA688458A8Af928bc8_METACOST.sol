...
/*
 ___ __ __   ______   _________  ________   ______   ______   ______   _________  
/__//_//_/\ /_____/\ /________/\/_______/\ /_____/\ /_____/\ /_____/\ /________/\ 
\::\| \| \ \\::::_\/_\__.::.__\/\::: _  \ \\:::__\/ \:::_ \ \\::::_\/_\__.::.__\/ 
 \:.      \ \\:\/___/\  \::\ \   \::(_)  \ \\:\ \  __\:\ \ \ \\:\/___/\  \::\ \   
  \:.\-/\  \ \\::___\/_  \::\ \   \:: __  \ \\:\ \/_/\\:\ \ \ \\_::._\:\  \::\ \  
   \. \  \  \ \\:\____/\  \::\ \   \:.\ \  \ \\:\_\ \ \\:\_\ \ \ /____\:\  \::\ \ 
    \__\/ \__\/ \_____\/   \__\/    \__\/\__\/ \_____\/ \_____\/ \_____\/   \__\/ 

Deployed by : Metacost IT Team
*/
...
contract METACOST is Context, IERC20, Ownable {
    ...
    // Metacost Max Transaction & Max Wallet Alg
    uint256 public Metacost__maxWallet = 20000000 * 10**9; //2% Default
    uint256 public Metacost__maxBuyTx = 10000000 * 10**9; //1% Default
    uint256 public Metacost__maxSellTx = 10000000 * 10**9; //1% Default

    uint256 public Metacost__swapTokensAtAmount = 5000000 * 10**9; //Threshold swap distribute wallet

    address public Metacost__Marketing = 0x5e8ba633d44c883A415911F9007C4BEf997b7EE2;
    address private Metacost__Team = 0x5e8ba633d44c883A415911F9007C4BEf997b7EE2;

    string private constant _name = "METACOST";
    string private constant _symbol = "METACOST";
    ...
    Taxes public Metacost__sellTaxes = Taxes(0, 9, 0, 6);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Metacost__Marketing] = true;
        _isExcludedFromFee[Metacost__Team] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Metacost__Marketing] = true;
        allowedTransfer[Metacost__Team] = true;
    ...
    function setMetacost__SellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _team) public onlyOwner {
       Metacost__sellTaxes = Taxes(_rfi,_marketing,_liquidity,_team);
        emit FeesChanged();
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        Taxes memory temp;
        if(isSell) temp = Metacost__sellTaxes;
        else temp = taxes;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Metacost__maxBuyTx, "You are exceeding Metacost__maxBuyTx");
            require(balanceOf(to) + amount <= Metacost__maxWallet, "You are exceeding Metacost__maxWallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Metacost__maxSellTx, "You are exceeding Metacost__maxSellTx");
            if(to != pair){
                require(balanceOf(to) + amount <= Metacost__maxWallet, "You are exceeding Metacost__maxWallet");
            }
            if(coolDownEnabled){
        ...
        bool canSwap = balanceOf(address(this)) >= Metacost__swapTokensAtAmount;
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(Metacost__swapTokensAtAmount, Metacost__sellTaxes);
            else  swapAndLiquify(Metacost__swapTokensAtAmount, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Metacost__Marketing).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Metacost__Team).sendValue(teamAmt);
        }
    }
    ...
    function Metacost__updateSwapTokensAtAmount(uint256 amount) external onlyOwner{
        Metacost__swapTokensAtAmount = amount * 10**_decimals;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Metacost__maxBuyTx = maxBuy * 10**decimals();
        Metacost__maxSellTx = maxSell * 10**decimals();
    }
    
    function updateMetacost__MaxWallet(uint256 amount) external onlyOwner{
        Metacost__maxWallet = amount * 10**decimals();
    }
