...
contract BEP20PayToken is Ownable{
    ...
    event PledgeMining(address indexed token, address indexed sender, uint  amount);
    event BuyEquity(address indexed token, address indexed sender, uint  amount);

    event WithDrawalToken(address indexed token, address indexed sender, uint indexed amount);

    function pledgeMining(address token, uint amount) external returns(bool){
        ...
        emit PledgeMining(token, sender, amount);

        return true;

    }

    function buyEquity(address token, uint amount) external returns(bool){
