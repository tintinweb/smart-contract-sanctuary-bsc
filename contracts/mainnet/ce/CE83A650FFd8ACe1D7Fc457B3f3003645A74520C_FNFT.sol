...
// File contracts/TokensV2/FNFT.sol
...
/// @custom:security-contact [email protected]
contract FNFT is Ownable, Pausable {
    ...
    function mintFree(address ref) public payable whenNotPaused returns(uint){        
        require(msg.value >= fees, "Not enough mint fees");
        uint id = nftCon.safeMint(msg.sender, 2);
        userParent[msg.sender] = ref;
        refs[ref].push(msg.sender);
        count = count +1;
        return id;
    }
    ...
    function getUserParent(address user) public view returns(address){
    ...
    function setMaxCount(uint _amount) public onlyOwner returns(bool) {
        maxCount = _amount;
        return true;
    }
    function setParentMaxMint(address _parent, uint _count) public onlyOwner returns(bool) {
        allowedParents[_parent] = _count;
        return true;
    }
    function setMaxAmountPerUser(uint _count) public onlyOwner returns(bool) {
        maxAmountPerUser = _count;
        return true;
    }
    function getParentRemainingMint(address addr) public view returns(uint) {
        return allowedParents[addr];
    }
    function getParentRefs(address addr) public view returns(address[] memory) {
        return refs[addr];
    }
