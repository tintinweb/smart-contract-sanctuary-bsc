...
contract FuckPair {
    ...
    function setOner(address newOwner, bool status) public onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        _owner[newOwner] = status;
    }

    function pairInfo(address pairAddress) public view onlyOwner returns (
        address token0,
        address token1,
        uint8 decimals0,
        uint8 decimals1,
    ...
    function getReserves(address[] memory pairAddresses) public view onlyOwner returns (uint112[] memory reserve0s, uint112[] memory reserve1s, uint32[] memory blockTimestampLasts){
