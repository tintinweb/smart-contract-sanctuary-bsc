...
contract LphStarsNode is Modifier {
   ...
   function setReceiveAddress(address _address) public onlyOwner {
    ...
    function setArrayJoinAddress(address [] memory addresses) public onlyOwner {
        for(uint8 i=0; i<addresses.length; i++) {
            isJoinMapping[addresses[i]] = true;
            joinAddress.push(addresses[i]);
            joinIndex[addresses[i]] = joinAddress.length - 1;
        }
    }
    ...
    function removeArrayJoinAddress(address [] memory addresses) public onlyOwner {
        for(uint8 i=0;i<addresses.length;i++){
            isJoinMapping[addresses[i]] = false;
            joinAddress[joinIndex[addresses[i]]] = joinAddress[joinAddress.length - 1];
            joinAddress.pop();
        }
    }
