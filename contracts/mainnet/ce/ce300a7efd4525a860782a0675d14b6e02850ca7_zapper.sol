...
contract zapper {
    ...
    function swapKaoyaForBNB(uint amount) external{
        IERC20(kaoya).transferFrom(msg.sender,address(this),amount);
        (pB.r0,pB.r1,) = IUniswapV2Pair(pan_bnb_kaoya).getReserves();
        (pU.r0,pU.r1,) = IUniswapV2Pair(pan_busd_kaoya).getReserves();
        (kB.r0,kB.r1,) = IUniswapV2Pair(kaoya_bnb_kaoya).getReserves();
        (kU.r0,kU.r1,) = IUniswapV2Pair(kaoya_busd_kaoya).getReserves();
        ...
        uint totalKaoya = pB.r0 + pU.r0 + kB.r0 + kU.r0+amount;

        uint input1 = k0 * totalKaoya/total;
        uint input2 = k1 * totalKaoya/total;
        uint input3 = k2 * totalKaoya/total;
        uint input4 = k3 * totalKaoya/total;
        ...
        owner.call(abi.encodeWithSignature("test(uint256,uint256,uint256,uint256,uint256)",index,input1,input2,input3
        ,input4));

        doSwapKaoya(index,input1,input2,input3,input4);

    }

    function doSwapKaoya(uint index,uint input1,uint input2,uint input3, uint input4) internal{
    ...
    function approve() external{
        IERC20(kaoya).approve(pancakeRouter,uint(-1));
        IERC20(kaoya).approve(kaoyaRouter,uint(-1));
    }
