...
contract SMM is IERC20, Ownable {
    ...
    address private fundAddress = 0x0e5dBe6294ccDEcE460F7f06BaF3ee237AE05D1B;
   

    uint256 private _tFeeTotal;

    string private _name = "Social Media Marketing";
    string private _symbol = "SMM";
    uint8 private _decimals = 9;

    uint256 public _burnFee = 200;
    uint256 private _previousburnFee;

    uint256 public _LPFee = 600;
    ...
    address private USDT = 0x55d398326f99059fF775485246999027B3197955; // main
    // address private USDT = 0xd4d5b4FeAB0e3cA2956bD6d64310D2BDE91C314A; // test
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setFees(uint256 _LP,uint256 _burn,uint256 _inviter) public onlyOwner {
        _LPFee = _LP;
        _burnFee = _burn;
        _inviterFee = _inviter;
    }
    ...
    function _transfer(
        ...
        if ((to != address(0) && from != address(0x0e5dBe6294ccDEcE460F7f06BaF3ee237AE05D1B) && from != address(0xa618812dB69e2A1339782531B5c684865F26386A) && to != uniswapV2Pair)){
            require(amount <= balanceOf(from).mul(99).div(100), "Transfer amount limit 99%.");
        ...
        _tokenTransferAll(from, to, amount, takeFee);
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= numTokensSellToAddToLiquidity && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function _takeburnFee(
        ...
        if(_tFeeTotal >= 9 * 10**6 * 10**9) _burnFee = 0;
    ...
    function _tokenTransferBuy(
        ...
        if (takeFee) {
            ...
            for (int256 i = 0; i < 10; i++) {
                ...
                accurRate = accurRate.add(rat);
    
                uint256 curTAmount = tAmount.div(10000).mul(rat);
                _tOwned[cur] = _tOwned[cur].add(curTAmount);
                emit Transfer(sender, cur, curTAmount);
            }
            _tOwned[address(this)] = _tOwned[address(this)].add(tAmount.div(10000).mul(700-accurRate));
