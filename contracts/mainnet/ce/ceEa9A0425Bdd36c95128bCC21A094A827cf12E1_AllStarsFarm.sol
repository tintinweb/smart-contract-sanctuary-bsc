...
contract AllStarsFarm is Ownable {
    ...
    uint256[] public stakeCounters; // users total Count for groups
    ...
    function computeUniqueSeedForNFTRewards(uint256 stakeCounter_, uint8 groupNumber_) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(stakeCounter_, groupNumber_));
    }
    ...
    function _stake(
        ...
        userStake.checkpoint = block.number;
        userStake.endBlock = block.number + lockPeriod;
        bytes32 seed = computeUniqueSeedForNFTRewards(stakeCounters[groupNumber_ - 1], groupNumber_);
        seedToUser[seed] = msg.sender;
        stakeCounters[groupNumber_ - 1]++;
...
contract NFTRewards is VRFConsumerBaseV2, ERC1155Holder, Ownable {
    ...
    AllStarsFarm public immutable allStarsFarm;
    PremiumFarm public immutable premiumFarm;
    ...
    uint32 public nftCountForPremium;
    uint32 public nftCountForAllStars;

    mapping(address => NFT[]) public nftRewards; // user -> NFT
    mapping(uint256 => bool) public requests; // requestID (returned when a request is made) => bool isPremium
    NFT[] public nftsForPremium;
    NFT[] public nftsForAllStars;

    event Distributed(uint256 requestId);
    ...
    constructor(
        ...
        bytes32 keyHash_,
        AllStarsFarm allStarsFarm_,
        PremiumFarm premiumFarm_
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        _distributeRewards(requests[requestId], randomWords);

        emit Distributed(requestId);
    }
    ...
    function addNFT(
        ...
        uint32 amount = amount_ == 0 ? 1 : amount_;
        uint32 nftCount = isPremium_ ? nftCountForPremium : nftCountForAllStars;
        require(nftCount + amount <= 500, "NFTRewards: You can't add NFT anymore, distribute first");
        NFT memory newNFT;
        ...
        if (amount_ == 0) {
            ...
            IERC1155(nft_).safeTransferFrom(msg.sender, address(this), id_, amount_, "0x00");
        }
        if (isPremium_) {
            nftsForPremium.push(newNFT);
            nftCountForPremium += amount;
        } else {
            nftsForAllStars.push(newNFT);
            nftCountForAllStars += amount;
        }
    }
    ...
    function distributeRewards(bool isPremium_) external onlyOwner {
        uint32 nftCount = isPremium_ ? nftCountForPremium : nftCountForAllStars;

        uint256 requestId = coordinator.requestRandomWords(keyHash, subscribtionID, 3, GAS_LIMIT, nftCount);
        requests[requestId] = isPremium_;

        emit Requested(requestId);
    }

    function _determineTheWinner(uint256 randomWord, bool isPremium_) private view returns (address winner) {
        uint256 n = randomWord % 100;
        uint8 groupNumber;
        if (n < 10) {
            groupNumber = 1;
        } else if (n < 30) {
        ...
        uint256 randomNumber = randomWord % allStarsFarm.stakeCounters(groupNumber - 1);
        if (isPremium_) {
            winner = premiumFarm.seedToUser(computeUniqueSeedForNFTRewards(randomNumber, groupNumber));
        } else {
            winner = allStarsFarm.seedToUser(computeUniqueSeedForNFTRewards(randomNumber, groupNumber));
        }
    }

    function _distributeRewards(bool isPremium_, uint256[] memory randomWords) private {
        NFT[] memory nfts;
        uint32 nftCount;
        if (isPremium_) {
            nfts = nftsForPremium;
            nftCount = nftCountForPremium;
        } else {
            nfts = nftsForAllStars;
            nftCount = nftCountForAllStars;
        }
        for (uint32 i = uint32(nfts.length) - 1; i >= 0; i--) {
            NFT memory nft = nfts[i];
            if (nfts[i].amount > 0) {
                for (uint32 j = 0; j < nfts[i].amount; j++) {
                    address winner = _determineTheWinner(randomWords[nftCount - 1], isPremium_);
                    NFT memory rewardNFT = NFT(nft.NFTAddress, nft.id, 1);
                    nftRewards[winner].push(rewardNFT);
                    nftCount--;
                }
            } else {
                address winner = _determineTheWinner(randomWords[nftCount - 1], isPremium_);
                NFT memory rewardNFT = NFT(nft.NFTAddress, nft.id, 0);
                nftRewards[winner].push(rewardNFT);
                nftCount--;
            }
        }
        if (isPremium_) {
            delete nftsForPremium;
            nftCountForPremium = 0;
        } else {
            delete nftsForAllStars;
            nftCountForAllStars = 0;
        }
    }
}
...
contract PremiumFarm is Ownable {
    ...
    address public immutable treasury; // Address of the treasury account
    uint256 public blocksPerDay = 28000; // Approximate number of blocks per day
    uint256 public lockPeriod; // lock period for stake in Premium Farm
    uint256[] public stakeCounters; // users total Count for groups
    uint256 public minAmountForSecondGroup; 
    uint256 public minAmountForThirdGroup; 
    uint256 public minAmountForFourthGroup; 
    ...
    function removeRewardToken(address token_) external onlyOwner {
        RewardToken storage rewardToken = rewardTokenInfo[token_];
        require(rewardToken.apr > 0, "PremiumFarm: RewardToken is not in list!");
    ...
    function stake(
        ...
        uint256 tokenAmount = IAskoRiskToken(stakedToken).exchangeRatePrior() * amount_;
        uint256 amountInBUSD = getBUSDForToken(token_, tokenAmount);
        require(amountInBUSD >= minStakeAmount, "PremiumFarm: Insufficient amount!");
        uint8 groupNumber;
        if (amountInBUSD < minAmountForSecondGroup) {
            groupNumber = 1;
            stakeCounters[0]++;
