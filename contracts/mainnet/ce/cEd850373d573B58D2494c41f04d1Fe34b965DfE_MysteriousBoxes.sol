    ...
    ERC721EnumerableUpgradeable
{
    ...
    event MintedBatch(address[] indexed reveivers);
     ...
     */
    function mintBatch(address[] memory _receivers) external onlyOwner nonReentrant {
        require(_receivers.length > 0, "Empty list");

        uint256 _times = _receivers.length;
        require(tokenCounter + _times <= TOTAL_SUPPLY, "Sold out");
        ...
        // mint
        for (uint256 i = 0; i < _times; i++) {
            _mint(_receivers[i], tokenCounter);
            isOpened[tokenCounter] = false;
            tokenCounter++;
        }

        emit MintedBatch(_receivers);
    }
