...
contract JeetsAreFucked is IBEP20 {
    string constant _name = "JeetsAreFucked6";
    string constant _symbol = "JAF6";
    ...
    uint256 private minPartySellInBusd = 10;
    uint256 private minPartyBuyInBusd = 10;
    uint256 private sellDelay = 1 minutes;
            ...
            if(buysUntilEvent - buys == 0){
                response = string(
                    abi.encodePacked(
                        ...
                        " jeets are out. Buy tax is 0%, go crash the party now! ",
                        uintToString(jeetTax),
    ...
    function setLetTheJeetsOutEventParameters(
        ...
        minPartySellInBusd = _minPartySellInBusd;
        minPartyBuyInBusd = _minPartyBuyInBusd;
    ...
    function takeJeetTax(address sender, address recipient, uint256 amount) internal returns (uint256) {
        uint256 taxAmount = (amount * tax * sellMultiplier) / taxDivisor;
        if (recipient == pair && lastSell[sender] + sellDelay <= block.timestamp && valueInBusd(amount) >= minPartySellInBusd * 1 ether) sells++;
        if (recipient == pair) lastSell[sender] = block.timestamp;
        if (!letTheJeetsOutEvent && sender == pair) buys++;
        if (letTheJeetsOutEvent && sender == pair && valueInBusd(amount) >= minPartyBuyInBusd * 1 ether) buys++;
        if (sender == pair) require(_balances[recipient] + amount <= maxWallet, "Exceeds max wallet"); 
