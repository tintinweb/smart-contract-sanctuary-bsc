...
contract IDO is ReentrancyGuard, Context, Ownable {
    ...
    //Pre-Sale 
    function buyTokens(uint _amount) public nonReentrant idoActive {
        IERC20(baseToken).transferFrom(msg.sender , _wallet , _amount);
        IERC20 tokenBEP = _stakeContract;
        require (tokenBEP.stakes(msg.sender) >= minimumStake, "You need to stake more than minimum MonkeyInu amount");
