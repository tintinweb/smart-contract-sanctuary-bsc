...
contract CoinFlip is Ownable {
    ...
    function maxBetAllowed() public view returns (uint256){
        return ( IERC20(gameToken).balanceOf(address(this)) / 10 )- feesCollected ;
    }
    ...
    function flipTheCoin(uint256 _betAmount, bool chooseHeads) internal returns (bool resultIsHeads, bool isWinner) {
        ...
        if(isWinner) {
            _betAmount = _betAmount * payOutDivisor / 100;
        ...
        emit results(msg.sender, isWinner, _betAmount, resultIsHeads);
    }
