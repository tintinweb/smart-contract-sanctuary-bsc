...
interface IPEARL is IERC20 {
    ...
    function burnTotalAmount(uint256) external view returns (uint256);
...
contract PEARLFarm is Ownable {
    ...
    IUserStack[] public userStacks;
    mapping (address => uint256) public userStackIndexes;
    ...
    constructor(address _pearl) {
        pearl = IPEARL(_pearl);
        lpToken = IERC20(pearl.uniswapV2Pair());
    }
    ...
    function lpStack(uint256 _amount) public {
        ...
        addUserStack(msg.sender, _amount);
    }

    function getAllUserStacks() public view returns (IUserStack[] memory) {
        return userStacks;
    }

    function userStacksLength() public view returns (uint256) {
        return userStacks.length;
    }

    function addUserStack(address account, uint256 amount) internal {
        if(isStacked[account]){
            userStacks[userStackIndexes[account]].amount += amount;
        }else{
            userStackIndexes[account] = userStacks.length;
            userStacks.push(IUserStack(account,amount));
            isStacked[account] = true;
        }
    }

    function subUserStack(address account, uint256 amount) internal {
        if(isStacked[account]){
            userStacks[userStackIndexes[account]].amount -= amount;
            if(userStacks[userStackIndexes[account]].amount == 0){
                isStacked[account] = false;
                userStacks[userStackIndexes[account]] = userStacks[userStacks.length-1];
                userStackIndexes[userStacks[userStacks.length-1].account] = userStackIndexes[account];
                userStacks.pop();
            }
        }
    }
    ...
    function withdrawAmount(address _account, uint256 _index) public view returns (uint256) {
        ...
        uint256 months = block.timestamp.sub(stack.startTime).div(30 days);
    ...
    function lpWithdraw(uint256 _index) public {
        ...
        subUserStack(msg.sender, amount);
    ...
    function claimeAmount(address _account, uint256 _index) public view returns (uint256) {
        ...
        return pearl.burnTotalAmount(miningId-1).mul(userAmount).div(lpAmount);
    }
    ...
    function claime(uint256 _index) public {
        ...
        address inviterAddress1 = topInviter[msg.sender];
        if(inviterAddress1 != address(0)){
            ...
            address inviterAddress2 = topInviter[inviterAddress1];
