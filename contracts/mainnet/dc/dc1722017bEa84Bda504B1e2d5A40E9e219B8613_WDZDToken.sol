...
contract WDZDToken is Context, IERC20, Ownable {
    
	using SafeMath for uint256;
	using Address for address;

	string private _name = "WDZD";
	string private _symbol = "WDZD";
	uint8 private _decimals = 18;
	address public feeRadd;	
	...
	uint256 public _swapFee = 1;
	uint256 public _minswap = 1 * 10**16;    
	...
	function setFeeRadd(address _Radd) public onlyOwner {
		feeRadd = _Radd;
	}
	...
	function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
		...
		if(inSwapAndLiquify || _whitelist[sender]>0 || _whitelist[recipient]>0){ 
	...
	function swapAndLiquify(uint256 tAmount) private lockTheSwap {
		swapThisTokenForToken(tAmount, feeRadd);
	}
