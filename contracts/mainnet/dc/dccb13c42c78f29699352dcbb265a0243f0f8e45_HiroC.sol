...
//NFT Dashboard:- https://nft.hirodashboard.com/

//Hiroki Official Website:- https://www.hirokitoken.com/
...
contract HiroC is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Supply
  uint256 public maxSupply = 260;
  uint256 public lastSupply = maxSupply;
  uint256 public maxMintAmount = 10;

  // Lists 
  uint256[260] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x7dF87f29a01F12659Cadf2Cea9C262dCb14a0892;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "HiroC: minting has not started yet");
    require(_mintAmount > 0, "HiroC: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "HiroC: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "HiroC: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "HiroC: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "HiroC: zero address");
    require(_amount > 0, "HiroC: you have to mint at least one");
    require(lastSupply != 0,  "HiroC: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"HiroC: URI query for nonexistent token");
