...
contract MyToken is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("MyToken", "MTK") {
        ...
        _burn(msg.sender, 10000 * 10 ** decimals());
    }
    ...
    function buyTokens(uint256 amount) public {
        ...
        uint256 remainingSupply = totalSupply() - balanceOf(owner());
        if (burnAmount > remainingSupply) {
            burnAmount = remainingSupply;
        }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(_isPurchaseAllowed(msg.sender), "Transfer not allowed before 24 hours");
        return super.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(_isPurchaseAllowed(sender), "Transfer not allowed before 24 hours");
        return super.transferFrom(sender, recipient, amount);
    }

    function _isPurchaseAllowed(address user) private view returns (bool) {
        uint256 lastPurchaseTime = _lastPurchaseTime[user];
        if (lastPurchaseTime == 0) {
            return true;
        }
        return block.timestamp - lastPurchaseTime >= 24 hours;
    }
