...
// File: contracts/UnseaLottery.sol
 ...
 */
contract UnseaLottery is ReentrancyGuard, IUnseaLottery, Ownable {
    ...
    uint256 public maxPriceTicketInUNC = 5000 ether;
    uint256 public minPriceTicketInUNC = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInUNC;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] uncPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInUNC;
        uint32 finalNumber;
    }
     ...
     * @param _uncTokenAddress: address of the UNC token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _uncTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of UNC to this contract
        uint256 amountUNCToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInUNC,
            _ticketNumbers.length
        );

        // Transfer UNC tokens to this contract
        uint256 balanceBefore = uncToken.balanceOf(address(this));
        uncToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountUNCToTransfer
        );
        amountUNCToTransfer = uncToken.balanceOf(address(this)).sub(
            balanceBefore
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInUNC = _lotteries[_lotteryId]
            .amountCollectedInUNC
            .add(amountUNCToTransfer);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInUNCToTransfer
        uint256 rewardInUNCToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInUNCToTransfer = rewardInUNCToTransfer.add(
                rewardForTicketId
            );
        }

        // Transfer money to msg.sender
        uncToken.safeTransfer(msg.sender, rewardInUNCToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInUNCToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        // Calculate prizes in UNC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].uncPerBracket[j] =
                ...
                // A. No UNC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].uncPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury = amountToWithdrawToTreasury
            .add(_lotteries[_lotteryId].amountCollectedInUNC)
            .sub(amountToShareToWinners);

        // Transfer UNC to treasury address
        if (amountToWithdrawToTreasury > 0) {
            uncToken.safeTransfer(
                treasuryAddress,
        ...
        onlyOwnerOrInjector
    {
        ...
        _amount = uncToken.balanceOf(address(this)).sub(balanceBefore);
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInUNC >= minPriceTicketInUNC) &&
                (_priceTicketInUNC <= maxPriceTicketInUNC),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInUNC: _priceTicketInUNC,
            ...
            amountCollectedInUNC: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(uncToken), "Cannot be UNC token");
    ...
    /**
     * @notice Set UNC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInUNC: minimum price of a ticket in UNC
     * @param _maxPriceTicketInUNC: maximum price of a ticket in UNC
     */
    function setMinAndMaxTicketPriceInUNC(
        uint256 _minPriceTicketInUNC,
        uint256 _maxPriceTicketInUNC
    ) external onlyOwner {
        require(
            _minPriceTicketInUNC <= _maxPriceTicketInUNC,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInUNC = _minPriceTicketInUNC;
        maxPriceTicketInUNC = _maxPriceTicketInUNC;
    }
     ...
     * @param _priceTicket price of a ticket (in UNC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].uncPerBracket[_bracket];
        } else {
            return 0;
        }
    }
