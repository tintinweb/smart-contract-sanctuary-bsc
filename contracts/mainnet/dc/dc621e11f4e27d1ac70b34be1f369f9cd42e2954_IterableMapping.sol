...
// REWARDTOKEN + ANTIBOT v1.2.3 generated with LaMachine.io 
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    address public lockedInterface;
...
contract BABYTOKEN_SEC  is ERC20Upgradeable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        dividendTracker.initialize(addrs[0],minimumTokenBalanceForDividends_);
        ...
        require(antiBotSettings[2] > 99, "Max 0.1%");
        maxTxAmount = totalSup.div(antiBotSettings[2]);  //0.1%
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        ...
        require(!wallet.isContract(), "");
        _marketingWalletAddress = wallet;
    }

    function setFees(uint256 rewardFee, uint256 liquidityF, uint256 marketingF ) external onlyOwner {
        tokenRewardsFee = rewardFee;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "Mst be between 200,000 and 500,000"
        );
        require(
    ...
    function _transfer(
        ...
        require(to != address(0), "");
         if(AntibotEnabled == true){
            ...
            if(coolDownEnabled==true && startCoolDownTime.add(disableCoolDownAfter) > block.timestamp){
                require(amount <= maxTxAmount || isExcludedFromTransactionlock[from] || maxTxAmount == 0, "Amount limit exceeded ");
        ...
        if(automatedMarketMakerPairs[from] &&from != owner() &&to != owner()){
            if(startCoolDownTime == 0){
                startCoolDownTime = block.timestamp;
            }
        }
