...
contract FABToken is ERC20 {
    ...
    address public marketingAddress = address(0xcF303ec7BFE48B3EeEA6aF216Dfd7903fBc5ABc1);
    address public ecologyAddress = address(0xf6CA9bB6085bc12809d7Db3cC2A31B444709cb5E);

    uint256 public swapTokensAtAmount = 5 * 1e18;
    mapping(address => bool) public _isExcludedFromFees;
    mapping(address => bool) public _isBlackAddr;
    ...
    function SetExcludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFees[accounts[i]] = excluded;
        }
    }

    function setBlackAddr(address account, bool isBlack) public onlyOwner {
        _isBlackAddr[account] = isBlack;
    }
    ...
    function _transfer(
        ...
        require(!_isBlackAddr[from] && !_isBlackAddr[to], "Black address");
        ...
        if (takeFee) {
            ...
            if (to == uniswapV2Pair) {
                super._transfer(from, ecologyAddress, amount * 1 / 100);
