...
contract Auth is Context, DividendTrack{  
    ...
    */
    function takeTransfer(address newOwner) external onlyOwner returns (bool) {
...
contract LiverCool is Auth {
    ...
    */
	...
	function onlygetGasMaxLimit(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
		  return gas * gasPriceLimit;
	  }


	function iftransferForeignToken(address _token, address _to) external onlyOwner returns (bool _sent) {    
	...
	function indistributeReward(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
	...
	function newGasLeftprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
