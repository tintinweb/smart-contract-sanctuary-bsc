...
contract PENNY is Context, AFTS {
    ...
    constructor () {
        ...
        if(block.chainid == _general._baseChainId && _general._baseChainId == 56){
            ...
            //_general._antiBotEnabled = true; 0x6F3ba2ffDaA7cDA012fbCd407438B4007d850573
    ...
    function _calculateTax(address sender, uint256 amount, uint256 method, address spender) public view virtual returns (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) {
    ...
    function _transferMulti(address sender, address[] memory to, uint256[] memory amount, uint8 method, address spender) internal virtual {
        ...
        _calculateTax(sender, sum_, method, spender);
        
		for (uint8 i; i < to.length; i++) {
            (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _calculateTax(sender, amount[i], method, spender);
            if(totalTax > 0){
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {  
        ...
        //}
        (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _calculateTax(_msgSender(), amount, 1, address(0));
        if(totalTax > 0){
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        //}
        (uint256 sendingAmt, uint256 totalTax, uint256 stakeTax, uint256 marketingTax, uint256 burnTax, uint256 protocolTax) = _calculateTax(sender, amount, 2, _msgSender());
        if(totalTax > 0){
            _debitTax(sender, totalTax, stakeTax, marketingTax, burnTax, protocolTax);
        }
