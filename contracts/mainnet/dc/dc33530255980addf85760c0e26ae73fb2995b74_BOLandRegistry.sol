...
contract BOLandRegistry is ILandRegistry, Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function mint(address user, int16 x, int16 y) external onlyRole(MINTER_ROLE) {
        _safeMint(user, _encodeTokenId(x, y));
    }

    function setName(int16 x, int16 y, string memory newName) external onlyRole(METADATA_SETTER_ROLE) {
    ...
    function _decodeTokenId(uint32 value) internal pure returns (int16 x, int16 y){
