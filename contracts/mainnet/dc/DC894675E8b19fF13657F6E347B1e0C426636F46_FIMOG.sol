...
// ♻️TG  : https://t.me/fifamoongoal
// ♻️WB :  http://fifamoongoal.com

/*
FIFAMOONGOAL

The Qatar 2022 World Cup will be held from November to December 2022.

FIFA and Qatar as the parties involved in organizing the 2022 World Cup
 continue to prepare to welcome the countries that will come. In total, 
 there will be 32 countries participating in the Qatar 2022 World Cup

We won't miss launching this project, 
as a tribute to the OG mascot in preparation for the 2022 World Cup.
**/
...
contract FIMOG is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "FIMOG";
    string private constant _name = "Fifa Moon Goal";

    uint256 public Fifa_max_BuyLimit = 25000000 * 10**9;
    uint256 public Fifa_max_SellLimit = 25000000 * 10**9;

    address private Fifa_Goal_Wallet = 0x3E6dDdE49b120AF7063Dd84fE883b6b1EF670D69;
    address private Fifa_Team_Wallet = 0x5b286b812d6963D6Ca75b8d07a7519cAb04b26Ad;

    uint256 public Fifa_max_WalletLimit = 500000000 * 10**9;
    uint256 public swapTokensAtAmount = 75000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Fifa_Goal_Wallet] = true;
        _isExcludedFromFee[Fifa_Goal_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Fifa_Goal_Wallet] = true;
        allowedTransfer[Fifa_Goal_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Fifa_max_BuyLimit, "You are exceeding Hwi_max_BuyLimit");
            require(balanceOf(to) + amount <= Fifa_max_WalletLimit, "You are exceeding Fifa_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Fifa_max_SellLimit, "You are exceeding Fifa_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Fifa_max_WalletLimit, "You are exceeding Fifa_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Fifa_Goal_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Fifa_Goal_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Fifa_Update_MarketingWallet(address newWallet) external onlyOwner{
        Fifa_Goal_Wallet = newWallet;
    }
    
    function Fifa_Update_TeamWallet(address newWallet) external onlyOwner{
        Fifa_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Fifa_max_BuyLimit = maxBuy * 10**decimals();
        Fifa_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Fifa_Max_Walletlimit(uint256 amount) external onlyOwner{
        Fifa_max_WalletLimit = amount * 10**decimals();
    }
