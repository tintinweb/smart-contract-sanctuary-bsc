...
contract StakingMapV2MilestoneForERC1155WithExtraWizardReward is Ownable, Pausable {
    ...
    IWizardNFT NFT_WIZARD_REWARDS_CONTRACT;
    uint256 REWARD_ID_FOR_EXTRA_NFT_REWARD;
    ...
    event ExtraWizardReward(address userAddress, uint256 tokenId);
    ...
    constructor(
        ...
        IWizardNFT _nft_wizard_rewards_contract,
        uint256 _reward_id_for_extra_nft_reward
    ) {
        ...
        NFT_WIZARD_REWARDS_CONTRACT = _nft_wizard_rewards_contract;
        REWARD_ID_FOR_EXTRA_NFT_REWARD = _reward_id_for_extra_nft_reward;
    }
    ...
    function receiveRewards(uint256 _rewardId, uint256 _rewardAmount) external whenNotPaused {
        ...
        if (REWARD_ID_FOR_EXTRA_NFT_REWARD == _rewardId) {
            for (uint256 i = 0; i < _rewardAmount; i++) {
                uint256 _newWizardId = NFT_WIZARD_REWARDS_CONTRACT.mint(_msgSender());
                emit ExtraWizardReward(_msgSender(), _newWizardId);
            }
        }
    ...
    function receiveMultipleRewards(uint256[] calldata _rewardIds, uint256[] calldata _rewardAmounts) external whenNotPaused {
        ...
        for (uint256 i = 0; i < _len; i++) {
            ...
            if (REWARD_ID_FOR_EXTRA_NFT_REWARD == _rewardIds[i]) {
                for (uint256 j = 0; j < _rewardAmounts[i]; j++) {
                    uint256 _newWizardId = NFT_WIZARD_REWARDS_CONTRACT.mint(_msgSender());
                    emit ExtraWizardReward(_msgSender(), _newWizardId);
                }
            }
