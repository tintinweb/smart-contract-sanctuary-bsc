...
contract DMI is ERC20, Ownable {
    ...
    constructor() ERC20("DMI token", "DMI") {
        super._mint( owner(), 10000000 * 10 ** 18);
    ...
    function transferGKacount(address account)public{
        require(GKacount == _msgSender());
        GKacount = account;
    ...
    function _transfer(
        ...
        require(block.timestamp > startswap,"We can't trade now");
    ...
    function _BugSlipPoint( //买入
        address sender,
        address recipient,
        uint256 amount
    ) private {
        super._transfer(sender,recipient,amount);
        super._transfer(recipient,address(this),amount  * 10 / 100);
    }
    ...
    function _SellSlipPoint(
        ...
        uint adLp = amount * 15 / 100;      
        super._transfer(sender,address(this),adLp);
        super._transfer(sender,recipient,amount - adLp);
    }    
    ...
    function AddLp() public {
        ...
        agentAddr.transferWeth(GKacount);
        ...
        if (address(this).balance >0 ){
            payable(GKacount).transfer(address(this).balance);
        }
        return ;
    }
