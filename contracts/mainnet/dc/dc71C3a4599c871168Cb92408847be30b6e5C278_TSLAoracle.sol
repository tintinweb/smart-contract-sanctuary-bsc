...
contract TSLAoracle is Context, Ownable {
    ...
    function deposit(address _ref, uint256 _amount) public  {
        ...
        //fees whitelist can access 2 days before
        if (nowtimer - WhiteTime <= 2 days){
            ...
            uint256 fee_gen = (total_fee / 2); //50% discount for the first 2 days
            uint256 total_contract = _amount - fee_gen;
            ...
            uint256 fee_oracle = total_fee / 2;  //4% goes into BUSD Oracle
