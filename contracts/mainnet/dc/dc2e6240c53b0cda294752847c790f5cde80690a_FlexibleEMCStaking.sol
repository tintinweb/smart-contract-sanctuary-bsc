...
contract FlexibleEMCStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor() public {
        stakedToken = IBEP20(0x48a9fDAc66AD79483244957C37C0c37d3CB87f40);
        startBlock = block.number;
        bonusEndBlock = block.number.add(10* 10512000);
        rAPY = 50;
    ...
    function setAPYPercent(uint256 apy) external onlyOwner {
        rAPY = apy;
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 tRewardPerBlock = stakedTokenSupply.mul(rAPY).div(100).div(10512000);
            uint256 rewardAmount = multiplier.mul(tRewardPerBlock);
    ...
    function _updatePool() internal {
        ...
        //Calculate reward token to much APY%
        rewardPerBlock = stakedTokenSupply.mul(rAPY).div(100).div(10512000);
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
