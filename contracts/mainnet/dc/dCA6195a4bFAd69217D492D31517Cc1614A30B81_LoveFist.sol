...
contract LoveFist is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed(_name,_symbol, uint8(DECIMALS)) Ownable() {
        ...
        if (chainId == 56) {
            ...
            usdtAddress = 0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A;
        } else {
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
         ...
         if( amount == 0 || lasetSellTimes[shareholders[currentIndex]].add(disabledRewardTime) <= block.timestamp && lasetRewardTimes[shareholders[currentIndex]].add(claimWait) <= block.timestamp) {
             currentIndex++;
             iterations++;
             return;
         }
