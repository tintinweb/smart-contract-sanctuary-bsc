...
contract swapper {
    ...
    address dogecoin = 0x641EC142E67ab213539815f67e4276975c2f8D50; //0x641EC142E67ab213539815f67e4276975c2f8D50
    ...
    function _rebalance() public returns(bool) {
        ...
        _Path[1] = wbnb;
        _Path[2] = busd;
        uint _wbnb = IERC20(wbnb).balanceOf(pool);
        uint _dogeking = IERC20(dogeking).balanceOf(pool);
        uint _dollarBNB = router.getAmountsOut(_wbnb, Path)[1];
        uint _dollarDK = router.getAmountsOut(_dogeking, _Path)[2];
        int _diff = int(_dollarBNB - _dollarDK);
        if(_diff > 1e13) {
            ...
            uint _in = ((_wbnb * (100e18 * uint(_diff) / _dollarBNB) / 100e18) - 1) / 2;
            require(IERC20(wbnb).approve(address(router), _in*5), "approval failed!");
            require(IERC20(wbnb).transferFrom(pool, address(this), _in-1));
            ...
            uint _in = ((_dogeking * (100e18 * uint(_diff / (-1)) / _dollarDK) / 100e18) - 1) / 2;
            require(IERC20(dogeking).approve(address(router), _in*5), "approval failed!");
            require(IERC20(dogeking).transferFrom(pool, address(this), _in-1));
            router.swapExactTokensForTokens(
                _in-1,
                0,
                path,
