...
contract NftMining is ERC721Holder, Ownable{
    ...
    function getkNftTokenUrl(uint pid_, address nft721Addr_) public view returns(uint[] memory tokenIds, string[] memory urls) {
        ...
        urls = new string[](len);
        for (uint x; x < len; x++) {
            _url = nftCons(nft721Addr_).tokenURI(nftTokenIdList[pid_][msg.sender][nft721Addr_][x]);
            urls[x] = _url;
        }
    }

    function getUserNftTokenUrl(address nft721Addr_) public view returns(uint[] memory tokenIds, string[] memory urls) {
        uint balanceLen = IERC721(nft721Addr_).balanceOf(msg.sender);
        uint id;
        for(uint x=0; x<balanceLen; x++){
            id = nftCons(nft721Addr_).tokenOfOwnerByIndex(msg.sender, x);
            tokenIds[x] = id;
        }
        string memory _url;
        for (uint y=0; y<balanceLen; y++) {
