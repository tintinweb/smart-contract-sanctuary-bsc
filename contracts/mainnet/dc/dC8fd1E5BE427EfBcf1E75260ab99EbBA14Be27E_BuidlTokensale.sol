...
contract BuidlTokensale is Ownable {
    using SafeMath for uint256;
    ...
    // Start time 03/11/2022 @ 1:00 pm (UTC) //
    uint256 public constant CROWDSALE_START_TIME = 1647003600;
    
    //Minimum contribution is 0.05 BNB
    uint256 public constant MIN_CONTRIBUTION_BNB = 50000000000000000;
    
    //Maximum contribution is 50 BNB
    uint256 public constant MAX_CONTRIBUTION_BNB = 50000000000000000000;

    //Minimum contribution is 20 BUSD
    uint256 public constant MIN_CONTRIBUTION_BUSD = 20000000000000000000;
    
    //Maximum contribution is 20000 BUSD
    uint256 public constant MAX_CONTRIBUTION_BUSD = 20000000000000000000000;
    ...
    mapping(address => uint256) public totalReferralCounter;
    mapping(address => uint256) public firstLevelReferralcounter;
    ...
    // Pointer to the BuidlToken
    IERC20 public buidlToken;

    // How many buidls do we send per BNB contributed.
    uint256 public BuidlsPerBnb;
    // How many buidls do we send per BUSD contributed.
    uint256 public BuidlsPerBusd;
    ...
    //                 Constructor                   //
    //===============================================//
    constructor(uint256 _buidlsPerBnb, uint256 _buidlPerBusd) {
        BuidlsPerBnb = _buidlsPerBnb;
        BuidlsPerBusd = _buidlPerBusd;
    }
    ...
    function purchaseBUIDLTokensInBNB(address _referrer) external payable {
          // Validations.
            require(
                msg.sender != address(0),
                "BuidlsCrowdsale: beneficiary is the zero address"
            );
            ...
            require(msg.value >= MIN_CONTRIBUTION_BNB, "BUIDLCrowdsale: minimum contribution is 0.05 BNB");
            require(contributionsInBNB[msg.sender] + msg.value <= MAX_CONTRIBUTION_BNB, "BUIDLCrowdsale: maximum contribution is 50 BNB");
            
            // If we've passed validations, let's get them $BUIDLs
            _buyTokensInBNB(msg.sender, msg.value, _referrer);
      
    }

    // Main entry point for buying into the Pre-Sale. Contract Receives $BUSD
    function purchaseBUIDLTokensInBUSD(uint256 _busdAmount, address _referrer) public  {
          // Validations.
            require(
                msg.sender != address(0),
                "BuidlsCrowdsale: beneficiary is the zero address"
            ...
            require(_busdAmount >= MIN_CONTRIBUTION_BUSD, "BUIDLCrowdsale: minimum contribution is 20 BUSD");
            require(contributionsInBUSD[msg.sender] + _busdAmount <= MAX_CONTRIBUTION_BUSD, "BUIDLCrowdsale: maximum contribution is 20000 BUSD");
    ...
    /**
     * Function that perform the actual transfer of $BUIDLs
     */
    function _buyTokensInBNB(address beneficiary, uint256 _amount, address _referrer) internal {
        ...
        if(referrer[beneficiary] == address(0) && _referrer != beneficiary && _referrer != address(0))
            ...
            firstLevelReferralcounter[_referrer]++;
        }
       address _tempReferrer = referrer[beneficiary];
        for(uint i=0; i<2; i++){
            if(_tempReferrer != address(0)){
                if(i == 0)
                    referralRewardInBUSD[_tempReferrer] += _amount.mul(15).div(100);
                else if(i == 1)
                    referralRewardInBUSD[_tempReferrer] += _amount.mul(10).div(100);

                totalReferralCounter[_tempReferrer]++;
            }
            else
                break;

            _tempReferrer = referrer[_tempReferrer];
        }
    }
    ...
    function _buyTokensInBUSD(address beneficiary, uint256 _busdAmount, address _referrer) internal {
        ...
        for(uint i=0; i<2; i++){
            ...
            if(_tempReferrer != address(0)){
                if(i == 0)
                    referralRewardInBUSD[_tempReferrer] += _busdAmount.mul(15).div(100);
                else if(i == 1)
                    referralRewardInBUSD[_tempReferrer] += _busdAmount.mul(10).div(100);
    ...
    function claimTokens() public {
        ...
        // Calculate how many $BUIDLs can be bought with contributed amount
        ...
        // Transfer the $BUIDLs to the beneficiary
        buidlToken.safeTransfer(msg.sender, finalTokenAmount);
    ...
    function claimReferralTokens() public {
        ...
        //give 69% ineterest of 1 month
        finalTokenAmount += finalTokenAmount.mul(30).div(100); 
    ...
    // Calculate how many $BUIDLs do they get given the amount of wei
    function _getTokenAmountBNB(uint256 weiAmount) internal view returns (uint256)
    {
        return weiAmount.mul(BuidlsPerBnb);
    }

    // Calculate how many $BUIDLs do they get given the amount of BUSD
    function _getTokenAmountBUSD(uint256 _amount) internal view returns (uint256)
    {
        return _amount.mul(BuidlsPerBusd);
    }
    ...
    function getDirectReferralCountForUser(address _userAddress) public view returns (uint256){
        return firstLevelReferralcounter[_userAddress];
    }

    function getTotalReferralCountForUser(address _userAddress) public view returns (uint256){
        return totalReferralCounter[_userAddress];
    }
    ...
    function changeBuidlBnbRate(uint256 _newRate) public onlyOwner returns(bool) {
        require(_newRate != 0, "New Rate can't be 0");
        BuidlsPerBnb = _newRate;
        return true;
    }
    ...
    function addTokenAddress(IERC20 _buidlToken)public onlyOwner{
        buidlToken = _buidlToken;
    }
    
    function takeOutRemainingTokens() public onlyOwner {
        buidlToken.safeTransfer(msg.sender, buidlToken.balanceOf(address(this)));
    }
