...
library ANPANOracleLibrary {
    ...
    function currentCumulativePrices(
        ...
        price0Cumulative = IAnpansPair(pair).price0CumulativeLast();
        price1Cumulative = IAnpansPair(pair).price1CumulativeLast();

        // if time has elapsed since the last update on the pair, mock the accumulated price values
        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IAnpansPair(pair).getReserves();
...
contract Oracle {
    ...
    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'ANPANSwapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'ANPANSwapFactory: ZERO_ADDRESS');
    }
    ...
    function update(address tokenA, address tokenB) external {
        require(msg.sender == priceUpdater, 'ANPANOracle: Price can update only price updater address');
        address pair = pairFor(tokenA, tokenB);

        Observation storage observation = pairObservations[pair];
        uint timeElapsed = block.timestamp - observation.timestamp;
        require(timeElapsed >= CYCLE, 'ANPANOracle: PERIOD_NOT_ELAPSED');
        (uint price0Cumulative, uint price1Cumulative,) = ANPANOracleLibrary.currentCumulativePrices(pair);
