...
/** 
The only token which only goes UP

Anti-dump measures with BUSD rewards contract; the holder's dream... 

https://t.me/+FQu6-KnyFe85YjNl
...
/* Token contract */
contract UPBUSD is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address Buyback = 0x363974418F84AD7403071fB62B6fAD296F7367E2;
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 300 / 10000; // 0.5%
    uint256 public _maxSellTxAmount = _totalSupply * 10 / 10000; // 0.5%
    uint256 public _maxWalletToken = ( _totalSupply * 300 ) / 10000; // 0.5%
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 10000; // 0.25% of supply
    ...
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _feeDenominator) external authorized {
        ...
        totalFeeBuy = _liquidityFeeBuy.add(_buybackFeeBuy).add(_reflectionFeeBuy).add(_marketingFeeBuy).add(BuybackFeeBuy);
    ...
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _feeDenominator) external authorized {
        ...
        totalFeeSell = _liquidityFeeSell.add(_buybackFeeSell).add(_reflectionFeeSell).add(_marketingFeeSell).add(BuybackFeeSell);
        feeDenominator = _feeDenominator;
    }
