...
contract SmartPoolRenewable is Ownable, ReentrancyGuard {
    ...
    function RenewOrExtendSubscriptionSixWeeks() external payable onlySub {
        address treasury = SmartPoolFactory(SMART_CHEF_FACTORY).treasury();
...
contract SmartPoolFactory is Ownable {
    ...
    event createdFGandFRTPool(address FGPool, address FRTPool);
    event createdPartnerPool(address Pool);

    SmartPoolRenewable[] private _SmartPools;
    address public Controller;
    IBEP20 FG = IBEP20(0x0253781A5D78CCfA40072118b48fE106BC7F6E1C);
    IBEP20 FRT = IBEP20(0xd51237A6F3219d186f0C8d8Dd957b1Bcb3Ce5d48);
    ...
    require(howMany < 4, "4 Max at a time");
        for (uint i = 0; i < howMany; i++){
            SmartPoolRenewable pool = new SmartPoolRenewable();
            _SmartPools.push(pool);
            SmartPoolRenewable(pool).transferOwnership(Controller);
        }
    }
    
    function createFG_FRTforToken(IBEP20 rewardToken) external onlyOwner {
            SmartPoolRenewable pool = new SmartPoolRenewable();
            _SmartPools.push(pool);
            SmartPoolRenewable(pool).setupTokens(FG,rewardToken,0);
            SmartPoolRenewable(pool).transferOwnership(Controller);
            SmartPoolRenewable pool2 = new SmartPoolRenewable();
            _SmartPools.push(pool2);
            SmartPoolRenewable(pool2).setupTokens(FRT,rewardToken,0);
            SmartPoolRenewable(pool2).transferOwnership(Controller);
            
            emit createdFGandFRTPool(address(pool), address(pool2));
    }
    
    function createPartnerPool(IBEP20 partnerToken) external onlyOwner {
            SmartPoolRenewable pool = new SmartPoolRenewable();
            _SmartPools.push(pool);
            SmartPoolRenewable(pool).setupTokens(partnerToken,partnerToken,0);
            SmartPoolRenewable(pool).transferOwnership(Controller);
            
            emit createdPartnerPool(address(pool));
    }
