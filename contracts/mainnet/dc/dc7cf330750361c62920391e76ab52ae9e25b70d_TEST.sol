...
contract TEST is Context, IERC20, Ownable {
    ...
    mapping(address => bool) public _isExcludedFromDailyWin;
    ...
    constructor(address pinkAntiBot_ ) {
        ...
        _isExcludedFromDailyWin[owner()] = true;
        _isExcludedFromDailyWin[address(this)] = true;
        _isExcludedFromDailyWin[uniswapV2Pair] = true;
   ...
   antiBotEnabled = _enable;
    ...
    function setWinnerForDaily(address _user, bool _status) external onlyOwner {
        _isExcludedFromDailyWin[_user] = _status;
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromDailyWin[from]) tokenHoldersMap.set(from,balanceOf(from));
        if(!_isExcludedFromDailyWin[to]) tokenHoldersMap.set(to,balanceOf(to));
    }
   ...
   function processDailyWinner(uint256 amount) private {

        uint256 random = uint256(
            keccak256(
                abi.encodePacked(block.difficulty, block.timestamp, lastBuyers)
            )
        ) % tokenHoldersMap.keys.length;

        address payable winner = payable(tokenHoldersMap.keys[random]);
