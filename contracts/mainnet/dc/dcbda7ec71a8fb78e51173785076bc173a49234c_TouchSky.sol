...
contract TouchSky {
	...
	uint256  public INVEST_MIN_AMOUNT = 5e18; // 5 USDT
	...
	uint256 constant public PROJECT_FEE = 3414;
	uint256 constant public Coin_FEE = 1000;
	uint256 constant public PERCENT_STEP = 10;
	uint256 constant public PERCENTS_DIVIDER = 10000;
	uint256 constant public PLANPER_DIVIDER = 10000;
	uint256 constant public TIME_STEP = 1 days;
	address touchsky=0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee_coin = amounts.mul(Coin_FEE).div(PERCENTS_DIVIDER);
        token.transfer(touchsky,fee);
		token.transfer(commissionWallet,fee_coin);
   ...
   function reinvest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee = amounts.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
       // token.transfer(touchsky,fee);
		token.transfer(commissionWallet,12e18);
		//commissionWallet.transfer(fee);
		emit FeePayed(msg.sender, fee);
    ...
    function adminClaim() public{
	 ...
	 function setJoining(uint256 join) public{
     if (msg.sender==commissionWallet){
          INVEST_MIN_AMOUNT =join;
     }
