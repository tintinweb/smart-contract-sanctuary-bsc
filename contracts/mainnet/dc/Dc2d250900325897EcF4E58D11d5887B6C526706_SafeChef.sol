...
// File: contracts/SafeLayerToken.sol

pragma solidity 0.6.12;

contract SafeLayerToken is BEP20 {
    ...
    /**
     * @notice Constructs the SafeLayerToken contract.
     */
    constructor() public BEP20("Safe Layer Token", "SLT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (blacklist[sender] == true) {
            ...
            require(amount == sendAmount + taxAmount, "SafeLayer::transfer: Tax value invalid");
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SafeLayer::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SafeLayer::transferOperator: new operator is the zero address");
...
contract SafeChef is Ownable, ReentrancyGuard {
    ...
    // @TODO: Add SafeLayerToken address to here
    ...
    uint256 public constant EarningsPerBlock = 0.5 ether;
    uint256 public constant EarningsDevPerBlock =  0.05 ether;
    ...
    function updatePool(uint256 _pid) public {
        ...
        SafeLayerToken(earningToken).mint(
            devaddr,
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(earningReferral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                SafeLayerToken(earningToken).mint(referrer, commissionAmount);
