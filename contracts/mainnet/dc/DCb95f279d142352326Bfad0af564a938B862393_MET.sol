...
// File: contracts/METDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/MET.sol
pragma solidity ^0.7.6;
contract MET is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public totalFees = BNBRewardsFee.add(liquidityFee).add(marketFee).add(charityFee).add(buyBackFee).add(burnFee);
    // buy fees
    ...
    uint256 public buyTotalFees = buyBNBRewardsFee.add(buyLiquidityFee).add(buyMarketFee).add(buyCharityFee).add(buyBuyBackFee).add(buyBurnFee);
    // sell fees
    ...
    uint256 public sellTotalFees = sellBNBRewardsFee.add(sellLiquidityFee).add(sellMarketFee).add(sellCharityFee).add(sellBuyBackFee).add(sellBurnFee);
    // fee tracker
    ...
    uint256 private previousBNBRewardsFee = BNBRewardsFee;
    ...
    constructor() public ERC20("Moon Exchange Token", "M.E.T") {
        // LP address
        Liquidityaddress = address(0xE169240b9965E691BBa73231c81C5890dd6E5655);
    ...
    function restoreFees() internal {
            ...
            BNBRewardsFee = previousBNBRewardsFee;
    ...
    function updateLPAddress(address payable newAddress) external onlyOwner {
    ...
    function burnTokens(uint256 amountNoDecimals) external onlyOwner {
        ...
        super._transfer(address(this), Liquidityaddress, sendAmount);
    }
    function updateFees(uint256 _BNBRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        ...
        burnFee = _burnFee;
        totalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateBuyFees(uint256 _BNBRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        buyBNBRewardsFee = _BNBRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketFee = _marketFee;
        buyCharityFee = _charityFee;
        buyBuyBackFee = _buyBackFee;
        buyBurnFee = _burnFee;
        buyTotalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }    
    
    function updateSellFees(uint256 _BNBRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _charityFee, uint256 _buyBackFee, uint256 _burnFee) external onlyOwner {
        sellBNBRewardsFee = _BNBRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellMarketFee = _marketFee;
        sellCharityFee = _charityFee;
        sellBuyBackFee = _buyBackFee;
        sellBurnFee = _burnFee;
        sellTotalFees = _BNBRewardsFee.add(_liquidityFee).add(_marketFee).add(_charityFee).add(_buyBackFee).add(_burnFee);
    }
    
    
    //security patch
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "MET: The dividend tracker already has that address");

        METDividendTracker newDividendTracker = METDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MET: The new dividend tracker must be owned by the ME.T token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MET: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "MET: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "MET: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MET: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MET: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MET: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (
            ...
            previousBNBRewardsFee = BNBRewardsFee;
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "MET: This account cannot send tokens until trading is enabled");
        }
        ...
        if(takeFee || buyOrSell ) {
            ...
            super._transfer(from, Liquidityaddress, burnAmount);
            }
        }else{
            if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
...
contract METDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("MET_Dividend_Tracker", "MET_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "MET_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "MET_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MET contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MET_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MET_Dividend_Tracker: Cannot update claimWait to same value");
