...
//    function initialize() public virtual {
    ...
    function redeemWithPenalty(IERC3475.Transaction calldata _transaction) external {
        ...
        if(nonce._values[5].uintValue > block.timestamp) {  // maturityDate is not reached - take penalty from principal
            // penalty = tokenAmount * penalty% * timePassed / maxTime
    ...
    function getLimit(uint256 classId, uint256 nonceId, address user) public view virtual override returns(int256) {
        Nonce storage nonce = _classes[classId]._nonces[nonceId];
    ...
    returns(uint256 principalAmount, uint256 availableAmount) {
        ...
        (uint112 reserve0, uint112 reserve1,) = IPair(poolAddress).getReserves();
        if (token > pairToken) (reserve0, reserve1) = (reserve1, reserve0); // reserve0 of project token, reserve1 of pair token
        availableAmount = uint256(reserve1) * balance / totalLP;    // amount of pair tokens belong to user's bond
    }
    ...
    function _issue(
        ...
        uint256 principalAmount = uint256(reserve1) * _transaction.amount / totalLP;
        nonce._principalAmount[_to] += principalAmount;
