 ...
 * 
*/
contract Tools is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function multiTransfer(address[] calldata addresses, uint256[] calldata tokens, IERC20Upgradeable _tokenAddress) external onlyOwner {
        address from = msg.sender;
        uint256 addrLength = addresses.length;
        uint256 tokenLength = tokens.length;
        if (addrLength != tokenLength) revert MismatchLength(addrLength, tokenLength);

        IERC20Upgradeable tokenAddress = _tokenAddress;
        uint256 senderBalance = tokenAddress.balanceOf(from);
        ...
        for (uint i = 0; i < addrLength; i++) {
            ...
            tokenAddress.transfer(addresses[i], totalToken);
        }
    }
}
