...
//there once was a project i wont name ran by a man a bit insane. tho clever he was he made mistakes
//driven by greed and self gain. blaming others on his rise to fame. 
//gave the keys away to run and hide. only to blame others that stood by his side.
//defi is dirty and and yet so young. let's moon this shit for those he shunned
...
contract REKTrickyINU is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   REKTrickyINUDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("REKTrickyINU", "REKTrickyINU") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 2;
        _buyDevFee = 2;
    	...
    	dividendTracker = new REKTrickyINUDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xA82D0A34ff87d11c11F7E38F9b380b7BcF558382);
        _devWallet = payable(0xFa6bdA86a6F07ea5A6ee345CD1CAEeCb23258b4D);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "REKTrickyINU: The dividend tracker already has that address");

        REKTrickyINUDividendTracker newDividendTracker = REKTrickyINUDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "REKTrickyINU: The new dividend tracker must be owned by the REKTrickyINU token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "REKTrickyINU: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "REKTrickyINU: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "REKTrickyINU: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "REKTrickyINU: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "REKTrickyINU: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "REKTrickyINU: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "REKTrickyINU: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "REKTrickyINU: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "REKTrickyINU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "REKTrickyINU: Cannot update gasForProcessing to same value");
...
contract REKTrickyINUDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("REKTrickyINU_Dividend_Tracker", "REKTrickyINU_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "REKTrickyINU_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main REKTrickyINU contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "REKTrickyINU_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "REKTrickyINU_Dividend_Tracker: Cannot update claimWait to same value");
