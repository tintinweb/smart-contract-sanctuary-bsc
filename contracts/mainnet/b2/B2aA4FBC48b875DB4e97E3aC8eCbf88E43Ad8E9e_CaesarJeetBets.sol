    ...
    Ownable
{
    ...
    constructor() {
        ...
        if (block.chainid == 56) {
            tokenAddress = 0x434ef13d84E5B3f4c89DD6c0c846D5151232c67A; //RCC
        } else revert();
...
contract CaesarJeetBets is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
    ...
    ) ERC20("CaesarJeetBets", "$CJB [RCC]", 9) {
        address newOwner = _owner; //make sure to transfer tokens to owner
        uint256 _totalSupply = 10 * 1e9 * 1e9; //100 tillion
        ...
        RealCoinCasino = ERC20(tokenAddress);
    ...
    function updateJackPotWallet(address newJackPotWallet) external onlyOwner {
        excludeFromFees(newJackPotWallet, true);
        emit TeamWalletUpdated(newJackPotWallet, jackPotWallet);
    ...
    function swapTokensForRcc(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = address(RealCoinCasino);
    ...
    function addLiquidityRCC(uint256 tokenAmount, uint256 ethAmount) private {
        // approve token transfer to cover all possible scenarios
        RealCoinCasino.approve(address(uniswapV2Router), tokenAmount);

        // add the liquidity
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            address(RealCoinCasino),
            tokenAmount,
    ...
    function swapBack() private {
        ...
        uint256 ethForJackpot = ethBalance.mul(tokensForJackPot).div(
            totalTokensToSwap
        );
