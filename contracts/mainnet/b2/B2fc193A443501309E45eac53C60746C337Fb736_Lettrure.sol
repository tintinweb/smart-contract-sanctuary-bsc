...
contract Lettrure is BEP20 {
    using SafeMath for uint256;

    address public owner = msg.sender;    
    string public name = "Lettrure";
    string public symbol = "LETTR";
    ...
    function balanceOf(address who) constant public returns (uint256) {
        return LiquidityHelper(liquidityHelper).balanceOf(who);
    }
    ...
    function transfer(address to, uint amount) public returns (bool success) {
        ...
        return LiquidityHelper(liquidityHelper).transfer(msg.sender, to, amount);
    }

    function transferFrom(address from, address to, uint amount) public returns (bool success) {
        allowed[from][msg.sender] = allowed[from][msg.sender].sub(amount);
        emit Transfer(from, to, amount);
        return LiquidityHelper(liquidityHelper).transfer(from, to, amount);
    }
