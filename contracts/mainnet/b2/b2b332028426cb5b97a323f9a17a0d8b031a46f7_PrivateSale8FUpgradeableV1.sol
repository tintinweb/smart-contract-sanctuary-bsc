...
contract PrivateSale8FUpgradeableV1 is Initializable, OwnableUpgradeable {
    ...
    function calculateWithdrawAvailable(uint256 tokenId) public view vestingStarted onlyTokenOwner(tokenId) returns (uint256, uint256, uint256)  {
        ...
        //return (fullTokensAmount,tgePercent,feeMuplipier);
        ...
        if (block.timestamp < timeToCheck) {
            return (tokensForTge, block.timestamp, timeToCheck);
        }
        if (block.timestamp - timeToCheck >= vestingTime) {
            amount = tokensAfterTge - balances[tokenId].payment.withdrawed;
        } else {
            amount = (
                (
