...
contract Token is ERC20, Ownable {
    ...
    // uint256 public lpRewardFee = 6; 
    address public lpRewardToken = 0x55d398326f99059fF775485246999027B3197955; //USDT地址
    ...
    address public _devAddress = 0x071b2A25D0c317D4295860304de117a7cE494F99;//基金地址

    address public _devAddress2 = 0x071b2A25D0c317D4295860304de117a7cE494F99;//基金地址2
    address public _markeAddress = 0x071b2A25D0c317D4295860304de117a7cE494F99;//营销地址
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            swapping = true;
            uint256 dev1Fees = swapTokensAtAmount.mul(2).div(10);
            uint256 dev2Fees = swapTokensAtAmount.mul(3).div(10);
            uint256 lpFees = swapTokensAtAmount.sub(dev1Fees).sub(dev2Fees);
            swapLPRewardTokenDev1(dev1Fees);
            swapLPRewardTokenDev2(dev2Fees);
            swapLPRewardToken(lpFees);
            swapping = false;
        }
    ...
    function takeBuyFee(address sender,address recipient,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(YFee > 0){
            ...
            for (int256 i = 0; i < 3; i++) {
                ...
                uint256 curTAmount = amount.div(100);
     ...
     function takeSellFee(address sender,address recipient,uint256 amount) private returns(uint256 amountAfter) {
        amountAfter = amount;
        ...
        if(YFee > 0){
            ...
            for (int256 i = 0; i < 3; i++) {
                ...
                uint256 curTAmount = contractUSDTBalance.div(3);
                c_usdt.transfer(cur, curTAmount);
            }
        }
    ...
    function swapLPRewardTokenDev1(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = lpRewardToken;
    ...
    function swapLPRewardTokenDev2(uint256 tokenAmount) private {
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
        path[2] = lpRewardToken;
