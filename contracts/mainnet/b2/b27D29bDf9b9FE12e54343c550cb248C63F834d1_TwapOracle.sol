...
contract TwapOracle {
    ...
    uint256 public constant PERIOD = 0;
    ...
    address private constant Tennesse =
    ...
    function update() public {
        ...
        */
        ...
        uint256 priceAverage = price0Average.mul(1e18).decode144();
    ...
    function priceTWAP(address token) public view returns (uint256 Twapprice) {
        if (token == token0) {
            Twapprice = price0Average.mul(1e18).decode144();
        } else {
            if (token != token1) {
                revert("INVALID_TOKEN");
            }
            Twapprice = price1Average.mul(1e18).decode144();
        }
    }

    function priceVariationPercentage() public {

        int256 currentprice = int256(priceTWAP(Tennesse));
        update();
        int256 updatePrice = int256(priceTWAP(Tennesse));
        int256 variation;

        console.logString("Currentprice");
        console.logInt(currentprice);
        console.logString("updatePrice");
        console.logInt(updatePrice);

        PRBMath.SD59x18 memory _current = currentprice.fromInt();
        PRBMath.SD59x18 memory _update = updatePrice.fromInt();
        PRBMath.SD59x18 memory diference = _update._sub(_current);
        PRBMath.SD59x18 memory _percentage = diference._div(_current);
        variation = adjustamount(_percentage.value);

        console.logString("_percentage");
        console.logInt(_percentage.value);
        console.logString("Diference");
        console.logInt(diference.value);
        console.logString("variation");
        console.logInt(variation);
        percentage = variation;
    }

    function adjustamount(int256 _amount)
        internal
        pure
    ...
    function testPriceVariationPercentage(
        int256 currentprice,
        int256 updatePrice
    ) public {
        int256 variation;
...
library PRBMathSD59x18Typed {
    ...
    /// @dev _log2(e) as a signed 59.18-decimal fixed-point number.
    int256 internal constant _log2_E = 1_442695040888963407;
    ...
    function abs(PRBMath.SD59x18 memory x)
        internal
        pure
    ...
    function _add(PRBMath.SD59x18 memory x, PRBMath.SD59x18 memory y)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
    ...
    function ceil(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue > MAX_WHOLE_SD59x18) {
            ...
            revert("PRBMathSD59x18__CeilOverflow");
        }
    ...
    function _div(PRBMath.SD59x18 memory x, PRBMath.SD59x18 memory y)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (rAbs > uint256(MAX_SD59x18)) {
            ...
            revert("PRBMathSD59x18__DivOverflow");
        }
        ...
        result = PRBMath.SD59x18({
            value: sx ^ sy == 1 ? -int256(rAbs) : int256(rAbs)
        });
    }
    ...
    ///
    /// @dev Based on the insight that e^x = 2^(x * _log2(e)).
    ///
    /// Requirements:
    /// - All from "_log2".
    ...
    function _exp(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue >= 133_084258667509499441) {
            ...
            revert("PRBMathSD59x18__ExpInputTooBig");
        }

        // Do the fixed-point multiplication inline to save gas.

        int256 doubleScaleProduct = xValue * _log2_E;
        PRBMath.SD59x18 memory exponent = PRBMath.SD59x18({
    ...
    /// @param x The exponent as a signed 59.18-decimal fixed-point number.
    /// @return result The result as a signed 59.18-decimal fixed-point number.
    function exp2(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue < 0) {
            ...
            if (xValue >= 192e18) {
                ...
                revert("PRBMathSD59x18__Exp2InputTooBig");
            }
    ...
    function floor(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue < MIN_WHOLE_SD59x18) {
            ...
            revert("PRBMathSD59x18__FloorUnderflow");
        }
    ...
    function frac(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        returns (PRBMath.SD59x18 memory result)
    {
        if (x < MIN_SD59x18 / SCALE) {
            ...
            revert("PRBMathSD59x18__FromIntUnderflow");
        }
        if (x > MAX_SD59x18 / SCALE) {
            //revert PRBMathSD59x18__FromIntOverflow(x);
            revert("PRBMathSD59x18__FromIntOverflow");
        }
        ...
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xy / x.value != y.value) {
            ...
            revert("PRBMathSD59x18__GmOverflow");
        }

        // The product cannot be negative.
        if (xy < 0) {
            //revert PRBMathSD59x18__GmNegativeProduct(x.value, y.value);
            revert("PRBMathSD59x18__GmNegativeProduct");
        }
    ...
    function inv(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
    ...
    ///
    /// @dev Based on the insight that ln(x) = _log2(x) / _log2(e).
    ///
    /// Requirements:
    /// - All from "_log2".
    ///
    ...
    function ln(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        // Do the fixed-point multiplication inline to save gas. This is overflow-safe because the maximum value that _log2(x)
        // can return is 195205294292027477728.

        int256 rValue = (_log2(x).value * SCALE) / _log2_E;
        result = PRBMath.SD59x18({value: rValue});
    }
    ...
    /// logarithm based on the insight that log10(x) = _log2(x) / _log2(10).
    ///
    /// Requirements:
    /// - All from "_log2".
    ///
    ...
    function log10(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue <= 0) {
            ...
            revert("PRBMathSD59x18__LogInputTooSmall");
        }
        ...
        if (rValue != MAX_SD59x18) {
            ...
            // Do the fixed-point division inline to save gas. The denominator is _log2(10).

            rValue = (_log2(x).value * SCALE) / 3_321928094887362347;
            result = PRBMath.SD59x18({value: rValue});
        }
    }
    ...
    function _log2(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        // This works because _log2(x) = -_log2(1/x).
        int256 sign;
        if (xValue >= SCALE) {
            sign = 1;
        } else {
    ...
    function _mul(PRBMath.SD59x18 memory x, PRBMath.SD59x18 memory y)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (rAbs > uint256(MAX_SD59x18)) {
            ...
            revert("PRBMathSD59x18__MulOverflow");
        }
    ...
    ///
    /// @dev Based on the insight that x^y = 2^(_log2(x) * y).
    ///
    /// Requirements:
    /// - All from "exp2", "_log2" and "mul".
    /// - z cannot be zero.
    ///
    /// Caveats:
    /// - All from "exp2", "_log2" and "mul".
    ...
    /// @return result The result as a signed 59.18-decimal fixed-point number.
    function powu(PRBMath.SD59x18 memory x, uint256 y)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (rAbs > uint256(MAX_SD59x18)) {
            ...
            revert("PRBMathSD59x18__PowuOverflow");
        }

        // Is the base negative and the exponent an odd number?
        bool isNegative = x.value < 0 && y & 1 == 1;
        result = PRBMath.SD59x18({
            value: isNegative ? -int256(rAbs) : int256(rAbs)
        });
    }
    ...
    function sqrt(PRBMath.SD59x18 memory x)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
        ...
        if (xValue < 0) {
            ...
            revert("PRBMathSD59x18__SqrtNegativeInput");
        }
        if (xValue > MAX_SD59x18 / SCALE) {
            //revert PRBMathSD59x18__SqrtOverflow(xValue);
            revert("PRBMathSD59x18__SqrtOverflow");
        }
    ...
    function _sub(PRBMath.SD59x18 memory x, PRBMath.SD59x18 memory y)
        internal
        pure
        returns (PRBMath.SD59x18 memory result)
    {
    ...
    function toInt(PRBMath.SD59x18 memory x)
        internal
        pure
...
library PRBMath {
    ...
    function mulDiv(
        ...
        if (prod1 >= denominator) {
            ...
            revert("PRBMath__MulDivOverflow");
        }
        ...
        returns (uint256 result)
    {
        ...
        if (prod1 >= SCALE) {
            ...
            revert("PRBMath__MulDivFixedPointOverflow");
        }
    ...
    function mulDivSigned(
        ...
        if (
            ...
            x == maxnegative || y == maxnegative || denominator == maxnegative
        ) {
        ...
        if (rAbs > uint256(maxpositive)) {
            //revert PRBMath__MulDivSignedOverflow(rAbs);
            revert("PRBMath__MulDivSignedOverflow");
        }
