...
contract CookieTokenController is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	function initialize(address _Piston) external virtual initializer {
	...
	/* getters */
	function pistonBalance() external view  returns (uint256, uint256, uint256) {
	...
	function getPairBalance() external view  returns (uint256, uint256) {
		return (pistonToken.balanceOf(address(uniswapV2Pair)), IERC20Upgradeable(BUSD).balanceOf(address(uniswapV2Pair)));
	}
	
	function getTokenAndControllerAddress() external view  returns (address, address) {
