    ...
    uint256 public nautlimitTier4 = 4000000000;
     ...
     //hardcap 
      ...
      uint public receivedFund;
        ...
        constructor () public  {
            owner = msg.sender;
            IBEP20 _naut = IBEP20(0x05B339B0A346bF01f851ddE47a5d485c34FE220c);
            naut = _naut;
         }
                 

        function checkTier(address _user) public view returns (uint256 _tier ){
           uint checkUserbalance = naut.balanceOf(_user);
            uint tier = 0 ; 
              ...
              else if ( checkUserbalance >= nautlimitTier4  ) {
                  tier = 4;
              }
              return tier ; 
            }

       function checkMaxInvestment(address _user) public view returns (uint256 _maxInv ){
           
           uint tier = checkTier(_user);
        
            uint maxInv = 0 ;            
           if(round == 1 || round == 0 ){
              if ( (round ==  1 && tier == 1 ) || (round ==  0 && checkWhitelistTierWise(_user,1)) ) {
                 
                  maxInv = Tier1maxInvestment;
              }
              else if ( (round ==  1 && tier == 2) ||  (round ==  0 && checkWhitelistTierWise(_user,2)) ) {
    
                      maxInv = Tier2maxInvestment;

              }
              else if (  (round ==  1 && tier == 3) ||  (round ==  0 && checkWhitelistTierWise(_user,3)) ) {
                   maxInv = Tier3maxInvestment;

              }
              else if (  (round ==  1 && tier == 4 ) ||  (round ==  0 && checkWhitelistTierWise(_user,4)) ) { 
                      maxInv = Tier4maxInvestment;
                  
              }
           }
           else{
               uint256 _balance = IBEP20(inputtoken).balanceOf(_user) ;
               maxInv = Tier3maxInvestment ; 
               if(_balance < Tier3maxInvestment){
                    maxInv = _balance ;            
               }
         ...
         function checkWhitelistTierWise(address _user,uint256 _tier) public view returns (bool ){
         ...
         function Investing(uint256 _amount) public {
            ...
            // check claim Status
             ...
             require(_amount  <= remainingContribution(msg.sender) , "max Investment reached");
         ...
         function remainingContribution(address _account) public view returns (uint256) {
