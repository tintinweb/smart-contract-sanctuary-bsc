...
contract Sale is Ownable {
    ...
    address private gctcAddress;
    address private tokenAddress;
    uint256 private _totalSold;
    uint256 private price;

    constructor(address _admin,address _gctcAddress,address _tokenAddress,uint256 _price) {
        admin = _admin;
    ...
    IERC20(gctcAddress).transfer(msg.sender,tokenAmount * 10**uint(18));
    }

    function withdrawToken (uint256 tokenAmount) external onlyOwner{   
    IERC20(gctcAddress).transfer(msg.sender,tokenAmount);  
    }
}