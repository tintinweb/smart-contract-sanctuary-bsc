...
contract ContractName{
    ...
    address  ServiceCharge ;
     mapping(uint256=>mapping(uint256=>uint256)) public NFTcommodityPrice;
    ...
    function setProduct(uint256 commodityID,uint256 TypeID,uint256 price,uint256 NFTID ) public   onlyOwner {
        if(NFTID > 0){
            NFTcommodityPrice[commodityID][TypeID] = NFTID;
        }
        if(price > 0){
    ...
    function PurchaseProduct(uint256 commodityID,uint256 TypeID , uint256 addressID,
       ...
       ( uint256 AMAPrice, uint256 LandPrice) =  L_A_price();
        ...
        }else if (Erc20TokenType == 1)
        {
            ...
            NFT.safeTransferFrom(msg.sender, address(ServiceCharge), NFTcommodityPrice[commodityID][TypeID], 1, "0x00");
    ...
    function ApplicationBankCardInformation(uint256 ID) public    {
    }
