...
contract PriceLock is Ownable {
    ...
    uint256 unlockPrice = 300 * (10**18);
    address private immutable _to;
    address public immutable _pairAddress;

    constructor(address to, address pairAddress){
        _to = to;
        _pairAddress = pairAddress;
    }

    function release(address contractAddress) external onlyOwner {
        uint256 currentprice = getTokenPrice(_pairAddress);
        if (currentprice >= unlockPrice){
