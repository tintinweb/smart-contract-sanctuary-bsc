...
contract NBCC is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || _destroyAmount >= _destroyMaxAmount){
            ...
            require(amount <= 5 * 10**18,"max 20");
            if(to == uniswapV2Pair){

            }else{
                require(balanceOf(to).add(amount) <= 20 * 10**18,"max 20");
            }
        }
	...
	function _tokenTransfer(
        ...
        uint256 rate;
        if (takeFee) {
			_takeInviterFee(sender, recipient, tAmount, currentRate);//4
