...
interface Staking{
    function referralDeposit(address userAddress, uint amount) external;
}

contract GenieReferral is Initializable, ContextUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    mapping(uint256 => ReferrralPlans) public referralPlans;
    address public depositToken;
    IERC20Upgradeable public saftiTokenAddress;
    IPancakeRouter02 public router;
    address[] public path;
    address[] public stakingContract;
    uint256[] public referralLevels;

    modifier onlyStaking(address _staking){
        bool validCaller = false;
        for(uint i = 0; i < stakingContract.length; i++){
            if(stakingContract[i] == _staking){
                validCaller = true;
            }   
        }
    ...
    function initialize() public initializer {
        ...
        saftiTokenAddress = IERC20Upgradeable(0x84952dd90cdC57170b13D2291A8afCEdC8F97c71);
    }

    function setPath(address path0, address path1, address path2) public onlyOwner{
        path.push(path0);
        path.push(path1);
        path.push(path2);
        // path[0] = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
        // path[1] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        // path[2] = address(saftiTokenAddress);
    }
    ...
    function setSaftiTokenAddress(IERC20Upgradeable _token) public onlyOwner{
        saftiTokenAddress = _token;
    }
    ...
    function addNewLevel(uint _levelRate, uint _tokenHoldings) public onlyOwner{
        referralPlans[referralLevels.length].refRate = _levelRate;
        referralPlans[referralLevels.length].tokenHoldings = _tokenHoldings;
        referralLevels.push(referralLevels.length);
    }
    ...
    function setUserReferral(address beneficiary, address referral) public onlyStaking(msg.sender){
        userReferral[beneficiary] = referral;
    }

    function setReferralAddressesOfUsers(address beneficiary, address referral) public onlyStaking(msg.sender){
    ...
    function getUsersTokenHoldings(address _userAddress) public view returns(uint) {
        uint256[] memory saftiPrice = router.getAmountsOut(1 ether, path);
        return saftiTokenAddress.balanceOf(userReferral[_userAddress]).mul(saftiPrice[2]).div(1e5);
    }

    function payReferral(address _userAddress, address _secondaryAddress, uint _index, uint256 _mainAmount) public onlyStaking(msg.sender) returns(bool) {
        ReferrralPlans memory _refPlan = referralPlans[_index];
        if( _index >= referralLevels.length ){
            return true;
        }else {
            if(userReferral[_userAddress] != address(0)){

                if(getUsersTokenHoldings(userReferral[_userAddress]) >= _refPlan.tokenHoldings){
                    uint256 transferAmount =  _mainAmount * _refPlan.refRate /10000;
                    ...
                    Staking(msg.sender).referralDeposit(userReferral[_userAddress], transferAmount);
                    require(
                    IERC20Upgradeable(depositToken).transfer(msg.sender, transferAmount),
