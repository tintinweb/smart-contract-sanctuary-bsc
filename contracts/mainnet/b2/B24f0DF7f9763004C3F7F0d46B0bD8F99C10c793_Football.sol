...
contract Football is Ownable, Pausable, ReentrancyGuard {
        ...
        if (reward > 0) {
            if(ledger[epochs][msg.sender].betType == BetType.BNB) {
                ...
                if(referral[msg.sender] != address(0)) {
                    ...
                    referralAmount = referralAmount.sub(referralReward);
                }                    
            }
    ...
    function startRound(uint256 epoch) external onlyOwner whenNotPaused nonReentrant notContract {
        ...
        if(epoch > 3) require(rounds[epoch-3].oracleCalled, "Last Lottery not Over");
        currentEpoch = epoch;
        _startRound(epoch);
    }

    function executeRound(uint256 epoch, uint256 random) external whenNotPaused nonReentrant onlyOwner {
        ...
        else if(random > 55) round.result = Position.Bull;
        else round.result = Position.Draw;
    ...
    function claimTreasuryInBNB() external nonReentrant whenPaused onlyOwner {
    ...
    function claimTreasuryInBUSD() external nonReentrant whenPaused onlyOwner {
        require(treasuryAmount > 0, "Treasury is empty");
    ...
    function claimTreasuryInUSDT() external nonReentrant whenPaused onlyOwner {
        require(treasuryAmount > 0, "Treasury is empty");
        uint256 currentTreasuryAmount = treasuryAmount.mul(10 ** (DECIMAL - 2));
    ...
    function _bettable(uint256 epoch) internal view returns (bool) {
        return
            ...
            block.timestamp < rounds[epoch].closeTimestamp - 5 seconds;
    }
