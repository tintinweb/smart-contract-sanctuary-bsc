...
contract FOMOge is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isFomosterized;

    address[] private _excluded;

    bool public tradingEnabled;
    bool public getFomoged;
    ...
    address public marketingWallet = 0x7843206e6C878CA2cF010B5a518fd3AE409D60E6;
    address public developmentWallet = 0xf9dc8BB8Aa72bF1Ff3b466c7a732642B6320d7a4;
    address public stakingWallet = 0xf9dc8BB8Aa72bF1Ff3b466c7a732642B6320d7a4;

    string private constant _name = "FOMOge";
    string private constant _symbol = "$FMGE";
    ...
    Taxes public taxes = Taxes(0, 7, 2, 0, 0);
    Taxes public sellTaxes = Taxes(0, 7, 2, 0, 0);
    ...
    function setFomogedStatus(bool state) external onlyOwner{
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _staking) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_development,_staking);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _staking) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_staking);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rdevelopment, to_return.rstaking) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tdevelopment-s.tstaking;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rdevelopment-rstaking;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rdevelopment,uint256 rstaking) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isFomosterized[from] && !_isFomosterized[to], "Get rekt");
        ...
        if(!swapping && getFomoged && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tdevelopment+ s.tstaking);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.development + temp.staking) * 2;
    ...
    function updategetFomoged(bool _enabled) external onlyOwner{
        getFomoged = _enabled;
    }
    
    function updateIsFomosterized(address account, bool state) external onlyOwner{
        _isFomosterized[account] = state;
    }
    
    function bulkIsFomosterized(address[] memory accounts, bool state) external onlyOwner{
        for(uint256 i =0; i < accounts.length; i++){
            _isFomosterized[accounts[i]] = state;
