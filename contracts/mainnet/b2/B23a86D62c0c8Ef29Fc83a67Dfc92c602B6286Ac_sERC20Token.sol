...
contract sERC20Token is ERC20Permit, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 100000000000000 * 10**9;
    ...
    constructor() ERC20("Staked Token", "sToken", 9) ERC20Permit() {
    ...
    function transfer( address to, uint256 value ) public override returns (bool) {
        ...
        updateSupportAmount(msg.sender ,_gonBalances[msg.sender]);
        updateSupportAmount(to,_gonBalances[to]);
        return true;
    }
    ...
    function transferFrom( address from, address to, uint256 value ) public override returns ( bool ) {
       ...
       emit Approval( from, msg.sender,  _allowedValue[ from ][ msg.sender ] );
        ...
        updateSupportAmount(from,_gonBalances[from]);
        updateSupportAmount(to,_gonBalances[to]);
        return true;
    }

    function updateSupportAmount(address from,uint256 gons) internal {
        if(supportAddress != address(0)){
            ISupport(supportAddress).updateStakingTeam(from,uint(gons));
        }
    }
    ...
    function setSupportAddress(address _supportAddress) external onlyManager(){
