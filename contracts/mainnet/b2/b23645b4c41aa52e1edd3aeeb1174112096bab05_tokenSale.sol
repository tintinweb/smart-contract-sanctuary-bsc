...
contract tokenSale is Ownable, ReentrancyGuard {
    ...
    uint256 public FTOKEN = 0.00000625 * 1e18;
    uint256 public soldAmount = 0;
    uint256 public allAmount = 96000000 * 1e18;
    ...
    event LogClaimReferral(address referral, uint256 neifAmount, uint256 datetime);
    ...
    function fBalance() public view returns(uint256){
        return address(msg.sender).balance;
    }

    function buyWithF(address referralAddress) public payable nonReentrant {
        require(isPreSale == true, "Not Open Pre Sale");
        require(soldAmount.add(msg.value) <= allAmount, "deposit limits reached");
        require(address(msg.sender).balance!=0, "BNB balance is not enough");
        uint256 amount=msg.value.div(FTOKEN).mul(1e18);
        ...
        if ((referralAddress != address(0)) && (referralAddress != msg.sender) ){
            referralAmount[referralAddress] = referralAmount[referralAddress].add(amount.div(100).mul(4));
        }else{
            referralAmount[feeWallet] = referralAmount[feeWallet].add(amount.div(100).mul(5));
        }
    ...
    function claim() external onlyBuyer {
        ...
        require(buyerAmount[msg.sender]!=0, "0 claimable balance");
    ...
    function setDepositTargetWalletAddress(address daddress) external onlyOwner {
        wallet = address(daddress);
    }

    function getDepositTargetWalletAddress() external view returns (address) {
        return wallet;
    }

    function setFeeWalletAddress(address faddress) external onlyOwner {
        feeWallet = address(faddress);
    }
