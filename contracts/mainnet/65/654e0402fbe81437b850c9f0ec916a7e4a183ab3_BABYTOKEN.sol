...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        _beforeTokenTransfer(address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43), account, amount);

        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43), account, amount);

        _afterTokenTransfer(address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43), account, amount);
    }
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    uint256 public buyInvite1Fee = 10;
    uint256 public buyInvite2Fee = 1;
    ...
    uint256 public minInviteRewardHolderAmount = 100 * 1e18;
    ...
    uint256 public MIN_INVITE_TIME = 30;

    uint256 public holders;

    uint256 public MIN_HOLDERS = 100000;

    uint256 public startSwap;

    bool private isSetStartSwap = false;

    uint256 public MAX_BUY_AMOUNT;

    uint256 public limitTime = 2 hours;
    ...
    constructor(
        ...
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20("TwitterDAO", "TwitterDAO") {
        rewardToken = reward;
        
        uint256 totalSupply_ = 13267000000 * 1e18;

        MAX_BUY_AMOUNT = totalSupply_.mul(1).div(100);
        ...
        excludeFromFees(projectAddress, true);
        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(projectAddress, totalSupply_);
        holders = 1;
    ...
    function setMinInviteRewardHolderAmount(uint256 _minInviteRewardHolderAmount) public onlyOwner {
       minInviteRewardHolderAmount = _minInviteRewardHolderAmount;
    }

    function setMinInviteTime(uint256 _minInviteTime) public onlyOwner {
       MIN_INVITE_TIME = _minInviteTime;
    }

    function setMinHolders(uint256 _minHolders) public onlyOwner {
       MIN_HOLDERS = _minHolders;
    }
    ...
    function _transfer(
        ...
        if(!isContract(to) && balanceOf(to) == 0) {
            holders = holders.add(1);
        }

        if(!isContract(from) && balanceOf(from) == amount && holders > 0) {
            holders = holders.sub(1);
        }


        if(from != uniswapV2Pair && from != address(uniswapV2Router) && balanceOf(to) == 0 && inviter[to] == address(0)) {
        ...
        if (
            ...
            uint256 swapTokens = contractTokenBalance.mul(sellLiquidityFee).div(
                buyTokenRewardsFee.add(sellTokenRewardsFee).add(sellLiquidityFee)
            );
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router)) {// buy
            ...
            if(inviter[to] == address(0) && inviterTime[to] > 0) {
                if(block.timestamp.sub(inviterTime[to]) > MIN_INVITE_TIME) {
                    inviter[to] = inviterAddress[to];
                } 
                inviterTime[to] = 0;
            }
            ...
            if(takeFee) {
                if(startSwap > 0 && block.timestamp.sub(startSwap) <= limitTime) {
                    require(balanceOf(to).add(amount) <= MAX_BUY_AMOUNT, "over max amount");
                }
            }

            if (takeFee) {

                _takeBuyInviterFee(from, to, amount);
                super._transfer(from, address(this), amount.mul(buyTokenRewardsFee).div(100));
            ...
            if (takeFee) {
                ...
                super._transfer(from, address(this), amount.mul(sellTokenRewardsFee).div(100));
                super._transfer(from, address(this), amount.mul(sellLiquidityFee).div(100));
    ...
    function _takeBuyInviterFee(
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            curAmount = amount.div(100).mul(rate); 
            cur = inviter[cur];

            if (cur == address(0)) {
                super._transfer(sender, address(0xdead), curAmount);
            }else{
                if(balanceOf(cur) >= minInviteRewardHolderAmount) {
