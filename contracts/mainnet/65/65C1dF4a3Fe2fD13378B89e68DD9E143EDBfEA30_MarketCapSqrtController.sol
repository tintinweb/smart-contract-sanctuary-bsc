...
import "../interfaces/IDexETFUniswapV2Oracle.sol";
...
contract MarketCapSqrtController is MarketCapSortedTokenCategories {
  ...
  function getInitialTokensAndBalances(
    ...
    for (uint256 i = 0; i < indexSize; i++) {
      ...
      require(targetBalance >= MIN_BALANCE, "DexETF: Min balance overflow");
  ...
  constructor(
    ...
    require(address(factory) != zero, "DexETF: Factory is zero address");
    require(address(proxyManager) != zero, "DexETF: Proxy manager is zero address");
    require(
  ...
  function prepareIndexPool(
    ...
    require(indexSize >= MIN_INDEX_SIZE, "DexETF: Min index size overflow");
    require(indexSize <= MAX_INDEX_SIZE, "DexETF: Max index size overflow");
    require(initialWethValue < uint144(-1), "DexETF: Initial weth value uint144 overflow");
    address router = _categoryRouters[categoryID];
    IDexETFUniswapV2Oracle oracle = _categoryOracles[categoryID];
  ...
  function finishPreparedIndexPool(
    ...
    uint256 valueSum = 0;
    require(caller == computeInitializerAddress(poolAddress), "DexETF: Prepare index pool first");
    ...
    require(!meta.initialized, "DexETF: Already initialized");
    uint96[] memory denormalizedWeights = new uint96[](len);
    uint144[] memory ethValues = IDexETFUniswapV2Oracle(oracleAddress).computeAverageEthForTokens(
      tokens,
      balances,
      SHORT_TWAP_MIN_TIME_ELAPSED,
      SHORT_TWAP_MAX_TIME_ELAPSED
  ...
  function updateMinimumBalance(
    ...
    require(!record.ready, "DexETF: Token not ready");
    uint256 poolValue = _estimatePoolValue(pool);
    address oracleAddress = IIndexPool(pool).oracle();
    PriceLibrary.TwoWayAveragePrice memory price = IDexETFUniswapV2Oracle(oracleAddress).computeTwoWayAveragePrice(
      tokenAddress,
      SHORT_TWAP_MIN_TIME_ELAPSED,
      SHORT_TWAP_MAX_TIME_ELAPSED
    );
  ...
  function reindexPool(address poolAddress) external returns (bool) {
    ...
    IndexPoolMeta memory meta = _poolMeta[poolAddress];
    require(meta.initialized, "DexETF: Pool not found");
    require(time - meta.lastReweigh >= POOL_REWEIGH_DELAY, "DexETF: Pool reweigh delay overflow");
    require((++meta.reweighIndex % (REWEIGHS_BEFORE_REINDEX + 1)) == 0, "DexETF: Pool reweigh index overflow");
    uint256 size = meta.indexSize;
    uint256 categoryID = meta.categoryID;
    address[] memory tokens = getTopCategoryTokens(categoryID, size);
    address oracleAddress = IIndexPool(poolAddress).oracle();
    PriceLibrary.TwoWayAveragePrice[] memory prices = IDexETFUniswapV2Oracle(
  ...
  function _estimatePoolValue(IIndexPool pool) internal view returns (uint144) {
    ...
    return IDexETFUniswapV2Oracle(oracleAddress).computeAverageEthForTokens(
      token,
      value,
      SHORT_TWAP_MIN_TIME_ELAPSED,
      SHORT_TWAP_MAX_TIME_ELAPSED
  ...
  modifier _havePool(address pool) {
    require(_poolMeta[pool].initialized, "DexETF: Pool not found");
    _;
  }
}
...
// SPDX-License-Identifier: MIT
pragma solidity 0.7.6;
import "./IDexETFUniswapV2Oracle.sol";
...
interface IDexETFUniswapV2Oracle {
...
contract OwnableProxy is Context {
  ...
  function transferOwnership(address newOwner) public virtual onlyOwner {
    require(newOwner != address(0), "DexETF: New owner is zero address");
    _transferOwnership(newOwner);
  }

  // For proxy owner
  function _initializeOwnership() internal {
    require(_owner == address(0), "DexETF: Owner has already been initialized");
    _transferOwnership(_msgSender());
  }
  ...
  modifier onlyOwner() {
    require(_owner == _msgSender(), "DexETF: Caller is not the owner");
    _;
  }
}
...
contract MarketCapSortedTokenCategories is OwnableProxy {
  ...
  mapping(uint256 => IDexETFUniswapV2Oracle) internal _categoryOracles;
  ...
  ) public view validCategory(categoryID) returns (address[] memory tokens)
  {
    ...
    require(num <= categoryTokens.length, "DexETF: Category size overflow");
    require(block.timestamp - _lastCategoryUpdate[categoryID] <= MAX_SORT_DELAY, "DexETF: Category not ready");
  ...
  function createCategory(
    ...
    address zero = address(0);
    require(address(oracle) != zero, "DexETF: Oracle is zero address");
    require(router != zero, "DexETF: Router is zero address");
  ...
  function addToken(
    ...
    require(_categoryTokens[categoryID].length < MAX_CATEGORY_TOKENS, "DexETF: Max category tokens overflow");
  ...
  function addTokens(
    ...
    require(
      ...
      "DexETF: Max category tokens overflow"
    );
    require(weights.length == len, "DexETF: Invalid arrays length");
    for (uint256 i = 0; i < len; i++) {
  ...
  function removeToken(
    ...
    require(len > 0, "DexETF: Category is empty");
    require(_isCategoryToken[categoryID][token], "DexETF: Token not bound");
  ...
  function _addToken(uint256 categoryID, address token, FixedPoint.uq112x112 memory weight) internal {
    require(!_isCategoryToken[categoryID][token], "DexETF: Token not bound");
    require(weight._x != 0, "DexETF: Weight is zero");
  ...
  modifier validCategory(uint256 categoryID) {
    require(categoryID <= categoryIndex && categoryID > 0, "DexETF: Invalid category id");
    _;
  }
}
