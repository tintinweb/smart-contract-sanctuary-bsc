...
contract Bnbninja is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BnbNinjaDividendTracker public dividendTracker;
    address public marketingWallet = 0xEaC29c20b5bd562FFE483Db0A3EBBA1d402b9738;
    address public buybackWallet = 0xF0946cB3Ddf998C54F652C45D78023d918EA229b;
    ...
    constructor() ERC20("BNBNINJA", "BNBN") {
        dividendTracker = new BnbNinjaDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "BbnNinja: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BnbNinja: The dividend tracker already has that address");

        BnbNinjaDividendTracker newDividendTracker = BnbNinjaDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BnbNinja: The new dividend tracker must be owned by the HoneyPad token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BnbNinja: Account is already the value of 'excluded'");
    ...
    function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromPeriodLimit[account] != excluded, "BnbNinja: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "BnbNinja: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BnbNinja: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BnbNinja: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "BnbNinja: This account cannot send tokens until trading is enabled");
        }
...
contract BnbNinjaDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BnbNinjaDividendTracker", "Bnbn_DividendTracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BnbNinjaDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BnbNinjaDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main Bnbn contract.");
    }
