...
contract Game is Addon {
    ...
    */
    function repair(uint tokenId, uint percent) external {
        ...
        subResource(msg.sender, repairCost);
    ...
    */
    function harvest(uint tokenId, bool[5] memory harvestingReward) external {
        ...
        if (deadTime == 0)
            subResource(msg.sender, [powerCost, 0, 0, 0, 0]);
    ...
    */
    function buyPowerWithLandtoken(uint amount) external payable {
        bool isValid;
        uint powerAmount;
        (isValid, powerAmount) = validator.canBuyPowerWithLandtoken(amount, calculateTotalPowerAmount(msg.sender), msg.sender);
        if (!isValid) return;

        /// auto harvest power before buy power using landtoken
        landToken.transferFrom(msg.sender, address(this), amount);
        autoPowerHarvest(msg.sender);
    ...
    */
    function gatherLumberWithPower(uint amount) external {
        if (!validator.canGatherLumberWithPower(amount, getLastGatherLumberTime(), msg.sender)) return;
    ...
    */
    function buyResourceOverdrive(uint tokenId, uint facilityType) external {
        ...
        subResource(msg.sender, [15 * PRECISION, 0, 0, 0, 0]);
...
contract Addon is Resource {
    ...
    */
    function salvageAddon(uint tokenId, uint addonId) external {
        ...
        subResource(msg.sender, sellCost);
    ...
    */
    function fertilizeGarden(uint tokenId) external {
        ...
        subResource(msg.sender, setting.getFertilizeGardenCost());
    ...
    */
    function burnLumberToMakePower(uint tokenId, uint lumber) external {
        bool isValid;
        uint generatedPower;
        (isValid, generatedPower) = validator.canBurnLumber(tokenId, lumber, userResources[msg.sender][1], calculateTotalPowerAmount(msg.sender), msg.sender);
        if (!isValid) return;

        /// Subtract lumber from user and add generated power to uer
        subResource(msg.sender, [0, lumber, 0, 0, 0]);
...
contract Resource is Ownable, IGameEvents {
    ...
    */
    function subResource(address user, uint[5] memory resource) internal {
        require (calculateTotalPowerAmount(user) >= resource[0], "Insufficient power");
    ...
    */
    function getResource(address user) public view returns (uint[5] memory) {
        ...
        resource[0] = calculateTotalPowerAmount(user);
        return resource;
    }
    ...
    */
    ...
    /**
        ...
        @param user: User
    */
    function autoPowerHarvest(address user) internal {
        userResources[user][0] = calculateTotalPowerAmount(user);
        house.setPowerRewardTime(user);
    }

    /**
        @notice Calculate all power amount: user power amount + reward power amount
        @return Return all power amount
    */
    function calculateTotalPowerAmount(address user) public view returns(uint) {
        uint sumPowerReward = house.calculateTotalPowerReward(user);
        uint maxLimit = house.calculateMaxPowerLimitByUser(user);

        if (sumPowerReward + userResources[user][0] >= maxLimit) {
            return maxLimit;
        } else {
            return userResources[user][0] + sumPowerReward;
        }
    }
...
abstract contract IValidator {
    ...
    function canBuyPowerWithLandtoken(uint amount, uint totalPowerAmount, address user) external view virtual returns (bool, uint);
    function canGatherLumberWithPower(uint amount, uint[3] memory lastGatherLumberTime, address sender) external view virtual returns (bool);
