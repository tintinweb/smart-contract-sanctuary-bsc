...
/*
If you can, make it for BSC first, i hate testing on ethereum its so slow xD

Make the following update please:

- Make the tax work as we said that it sends BNB to a team wallet on each buy/sell tx

- Remove all max wallet and max transaction functions (all anti-whale functions)

- Remove the need for a max tax, so that the tax can be any set value

- Remove whitelist functions if not needed for tax purpose

- Change transferOwnership into RenounceOwnership so there is no transfer function, only a renounce.

*/

contract GretaThunberg is ERC20, ERC20Burnable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    address public lpPair; // Liquidity token address
    address public feeReceiverAddress; // address where fee will be sent
    ...
    // NUMBERS ----------------------------------------------------------------------------------------------
    uint256 public divisor; // divisor | 0.0001 max presition fee
    uint256 public swapThreshold;

    // MAPPINGS
    mapping(address => bool) public _isExcludedFromFee; // list of users excluded from fee
    mapping(address => bool) public automatedMarketMakerPairs; // list of pairs
    ...
    IUniswapV2Router02 public dexRouter; // uniswap router
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("GretaThunberg", "GT") {
        // set owner
        owner = msg.sender;
        ...
        // 3% on SELL
        // 0% on Transfer
        _feesRates = Fees({buyFee: 9900, sellFee: 9900, transferFee: 0});
        ...
        // exclude from fees and max limit
        // owner, token
        _isExcludedFromFee[owner] = true;
        _isExcludedFromFee[address(this)] = true;

        swapThreshold = 1000 ether; // amount of tokens to swap for BNB
    }

    // function for renounce ownership
    ...
    // function for set fee receiver address
    function setFeeReceiverAddress(address _feeReceiverAddress)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (contractMustSwap(contractTokenBalance, from, to)) {
            swapTokensForEth(contractTokenBalance, feeReceiverAddress);
        }
    ...
    function swapTokensForEth(uint256 _tokenAmount, address _to) private {
