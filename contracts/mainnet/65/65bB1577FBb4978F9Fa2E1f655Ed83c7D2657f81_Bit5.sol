...
import {Bit5Treasury} from "./Bit5Treasury.sol";
    ...
    ReentrancyGuard
{
    ...
    event TreasuryPercentageChanged(address collection, uint256 newTP);
    ...
    // Percentages calculated from fees that are going to Collection treasury
    mapping(address => uint256) public treasuryPercentages;
    ...
    Bit5Treasury public immutable Bit5_TREASURY;
    ...
    constructor(address[] memory _paymentTokens, address treasury) {
        FEE = 200; // 2%
        maxRoyalty = 4000; // 40%

        Bit5_TREASURY = Bit5Treasury(treasury);

        for (uint256 i; i < _paymentTokens.length; ++i) {
            paymentTokens[_paymentTokens[i]] = true;

            IERC20(_paymentTokens[i]).approve(
                address(Bit5_TREASURY),
                type(uint256).max
            );
        }
    }
    ...
    function _doTransfers(
        ...
        uint256 treasuryFeePercentage = (treasuryPercentages[order.nftAddress] *
            FEE) / BP;
        ...
        if (supports2981(order.nftAddress)) {
            ...
            IERC20(order.paymentToken).transfer(receiver, royaltyAmount);
        } else {
        ...
        if (order.privileges.privilegedCollection != address(0)) {
            ...
            if (isPrivileged) {
                ...
                // if there is privilege, we need to update treasuryFeePercentage
                treasuryFeePercentage =
                    (treasuryPercentages[order.nftAddress] * FEE) /
                    BP;
                FEE = oldFee;
            }
        }

        uint256 toCollectionTreasury = (treasuryFeePercentage * order.price) /
            BP;

        if (toCollectionTreasury > 0) {
            Bit5_TREASURY.deposit(
                order.nftAddress,
                order.paymentToken,
                toCollectionTreasury
            );
        }
    ...
    function changeTreasuryPercentage(address collection, uint256 pp)
        external
        onlyOwner
    {
        treasuryPercentages[collection] = pp;
        emit TreasuryPercentageChanged(collection, pp);
    }
...
import {IBit5Treasury} from "./IBit5Treasury.sol";
import {Bit5Errors} from "./Errors.sol";

contract Bit5Treasury is IBit5Treasury, Bit5Errors, Ownable {
    event Withdraw(
        address indexed fromCollection,
        address indexed tokenAddress,
        uint256 amount
    );
    event WithdrawRaw(address indexed tokenAddress, uint256 amount);
    event Deposit(
    ...
    mapping(address => uint256) public collectionBalances;
    ...
    function deposit(
        ...
        if (msg.sender != bit5) revert Unauthorized();
        bool res = IERC20(tokenAddress).transferFrom(
            bit5,
            address(this),
            amount
        );

        if (!res) revert TokenTransferFailed();

        collectionBalances[collectionAddress] += amount;

        emit Deposit(collectionAddress, tokenAddress, amount);
    }

    function setBit5(address _bit5) external onlyOwner {
        bit5 = _bit5;
    }
    ...
    function withdraw(
        ...
        if (amount > collectionBalances[fromCollection]) {
            revert UnsufficientTreasuryFunds();
        }

        collectionBalances[fromCollection] -= amount;

        IERC20(tokenAddress).transfer(msg.sender, amount);

        emit Withdraw(fromCollection, tokenAddress, amount);
    }
}
