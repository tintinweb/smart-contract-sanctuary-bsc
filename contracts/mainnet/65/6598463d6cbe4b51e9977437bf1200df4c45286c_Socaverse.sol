...
contract Socaverse is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    IERC20 public tokenV1 = IERC20(0x57Af121A8dDb1F9A8FafcF3229c92bF2856A8a29); //address of the old version
    uint256 public amountV1Migrated; // amount of tokens V1 migrated
    address public tokenV1Recipient; // address that receive V1 tokens
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && (taxes.liquidity + taxes.marketing) > 0) {
...
/// @notice A contract to migrate from old to new token version
    ...
    function setMigrationStatus(bool state) external onlyOwner{
    ...
    function migrate() external nonReentrant{
        ...
        require(balanceOf(address(this)) >= amount, "No enough V2 liquidity");
        tokenV1.transferFrom(msg.sender, tokenV1Recipient, amount);
        _tokenTransfer(address(this), msg.sender, amount, false);
        amountV1Migrated += amount;
        emit Migrated(msg.sender, amount);
    }

    function setTokenV1Recipient(address addr) external onlyOwner{
        tokenV1Recipient = addr;
    }
