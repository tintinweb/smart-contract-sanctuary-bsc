...
contract WolfLaunchShield is Ownable {
    ...
    modifier vcMatch(VcParams memory params) {
        bytes32 _hash = keccak256(abi.encodePacked(secret, params.sender, params.path[0], params.path[params.path.length-1], params.amount, params.deadline));
    ...
    function _swapExactETHForTokens(
        ...
        if (
            ...
            require(
                swapInfo.amountToSend <= launchInfos[launchTokenAddr].maxTxAmount,
        ...
        afterLaunchTime(launchTokenAddr)
    {
        ...
        if (
            ...
            require(amountOut <= launchInfos[launchTokenAddr].maxTxAmount, "Max Tx Amount Exceed");
        }
        ...
        afterLaunchTime(launchTokenAddr)
    {
        ...
        if (
            ...
            require(amountIn <= launchInfos[launchTokenAddr].maxTxAmount, "Max Tx Amount Exceed");
        }
        // Check zero address
        require(to != address(0), "WolfLaunchShield: To address can't be 0");
        ...
        afterLaunchTime(launchTokenAddr)
    {
        ...
        if (
            ...
            require(amountInMax <= launchInfos[launchTokenAddr].maxTxAmount, "Max Tx Amount Exceed");
        }
        ...
        returns (uint256[] memory)
    {
        ...
        uint256 amountInMax = amounts[0] * 1e4 / (1e4 - tokenFeeInfos[sellTokenAddress].sellTotalFee);
        amountInMax = amountInMax * 1e4 / (1e4 - tokenFeeInfos[buyTokenAddress].buyTotalFee);
    ...
    function setLaunchTime(address _launchTokenAddr, uint256 _launchTime) external onlyOwner {
        launchInfos[_launchTokenAddr].launchTime = _launchTime;
    }

    function setLaunchInfo(address _launchTokenAddr, uint256 _launchTime, uint256 _maxTx, uint256 _maxWallet)
        external
        onlyOwner
    {
        launchInfos[_launchTokenAddr].launchTime = _launchTime;
    ...
    function setMaxTxWalletAmount(address _launchTokenAddr, uint256 _maxTx, uint256 _maxWallet)
        external
        onlyOwner
    {
        launchInfos[_launchTokenAddr].maxTxAmount = _maxTx;
