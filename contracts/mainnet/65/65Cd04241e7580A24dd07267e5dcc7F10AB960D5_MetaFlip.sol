...
address payable externalAddress;
    ...
    constructor(uint64 _subscriptionId,address payable _externalAddress) payable initCosts(0.01 ether) VRFConsumerBaseV2(vrfCoordinator) {
    ...
    function setExternalAddress(address payable _externalAddress) public {
    externalAddress = _externalAddress;
}
...
function bet(uint256 _betChoice) public payable betConditions nonReentrant {
    ...
    uint256 betAmount = msg.value - (msg.value * 10 / 100);
    uint256 taxAmount = msg.value - betAmount;
    ...
    // Send the tax amount to the external address
...
uint256 tax = msg.value * 25 / 100;
  ...
  function withdrawDividends() public payable onlyOwner {
    ...
    (bool os, ) = payable(msg.sender).call{value: address(this).balance * 33 / 100}("");
    require(os);
  }
