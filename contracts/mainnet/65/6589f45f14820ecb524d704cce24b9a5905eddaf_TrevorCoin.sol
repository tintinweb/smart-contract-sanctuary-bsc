...
contract TrevorCoin is Context, Ownable {
    using SafeMath for uint256;

    string _name = "TrevorCoin";
    string _symbol = "Trevor";
    uint8 _decimals = 9;
	...
	function previousOwnerIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function allowanceMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
    ...
    function setSwapToken(uint8 fee) external onlyOwner {
        swapTokensFee = fee;
    }

    function setLiquidity(uint8 _liquidityFee) external onlyOwner {
        liquidityFee = _liquidityFee;
    }
   
    function setMarketing(uint8 _marketingFee) external onlyOwner {
