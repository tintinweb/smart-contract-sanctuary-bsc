...
/**
 * Contract: VAST Token
 * VAST style Token Forked by Papa Escobear.
 *
 * Liquidity-less Token, DEX built into Contract
 * Send BNB to contract and it mints VAST Token to your receive Address
 ...
 *
 */
contract VASTToken is ReentrancyGuard, INativeVAST {
    ...
    uint256 public fundingBuySellDenominator = 4;
    uint256 public fundingTransferDenominator = 2;
    address public VASTFund = 0x06B7eBf8F88a5C8374A16a0ca18Bd113aDd2C851;
    ...
    // Activates VAST Token Trading
    bool VAST_Token_Activated;
    ...
    constructor ( address peggedToken, string memory tokenName, string memory tokenSymbol, uint8 tokenDecimals, uint256 _buyFee, uint256 _sellFee, uint256 _transferFee
        ...
        require(_buyFee <= 100 && _sellFee <= 100 && _transferFee <= 100 && _buyFee >= 30 && _sellFee >= 30 && _transferFee >= 30, 'Invalid Fees, Must Range From 50 - 100');
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if (_owner==(msg.sender)) {
            ...
            require(sender == msg.sender, 'Only VASTToken Owner Can Transfer Funds');
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (allowFunding && sender != VASTFund && recipient != VASTFund) {
            // allocate percentage of the tax for VAST Fund
            uint256 allocation = tax.div(fundingTransferDenominator);
            // how much are we removing from total supply
            tax = tax.sub(allocation);
            // allocate funding to VAST Fund
            _balances[VASTFund] = _balances[VASTFund].add(allocation);
            // Emit Donation To VAST Fund
            emit Transfer(sender, VASTFund, allocation);
        }
        // burn the tax
    ...
    /** Purchases VAST Tokens and Deposits Them in Sender's Address */
    function purchase() private nonReentrant returns (bool) {
        // make sure emergency mode is disabled
        require((!emergencyModeEnabled && VAST_Token_Activated) || _owner == msg.sender, 'EMERGENCY MODE ENABLED');
        ...
        // revert if under 1
        require(tokensToSend > 0, 'Must Purchase At Least One VAST');

        if (allowFunding && msg.sender != VASTFund) {
            // allocate tokens to go to the VAST Fund
            ...
            // mint to Fund
            mint(VASTFund, allocation);
            // Tell Blockchain
            emit Transfer(address(this), VASTFund, allocation);
        }
    ...
    /** Sells VAST Tokens And Deposits the Underlying Asset into Seller's Address */
    function sell(uint256 tokenAmount) external nonReentrant override {
        ...
        if (allowFunding && msg.sender != VASTFund) {
            // allocate percentage to VAST Fund
            ...
            // Allocate Tokens To VAST Fund
            _balances[VASTFund] = _balances[VASTFund].add(allocation);
            // Tell Blockchain
            emit Transfer(msg.sender, VASTFund, allocation);
        }
    ...
    /** Enables Trading For This VAST Token, This Action Cannot be Undone */
    function ActivateVASTToken() external onlyOwner {
        require(!VAST_Token_Activated, 'Already Activated Token');
        VAST_Token_Activated = true;
        allowFunding = true;
        emit VASTTokenActivated();
    }
    
   /*
    * Fail Safe Incase Withdrawal is Absolutely Necessary
    * Allows Users To Withdraw 100% Of The Underlying Asset
    * This will disable the ability to purchase VAST Tokens
    ...
    */
    ...
    function changeFundingValues(uint256 newBuySellDenominator, uint256 newTransferDenominator) external onlyOwner {
        require(newBuySellDenominator >= 50, 'BuySell Tax Too High!!');
        require(newTransferDenominator >= 2, 'Transfer Tax Too High!!');
    ...
    /** Change The Address For The Charity or Fund That VAST Allocates Funding Tax To */
    function swapFundAddress(address newFundReceiver) external onlyOwner {
        VASTFund = newFundReceiver;
