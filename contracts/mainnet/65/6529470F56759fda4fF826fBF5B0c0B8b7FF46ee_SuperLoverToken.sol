...
contract SuperLoverToken is ERC20, Ownable {
    ...
    uint256 private totalSupply_ = 10E7 * (10**18);
    ...
    uint256 public minimumTokenBalanceForDividends = 10E4* (10**18);
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(2).div(4);
            swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(1).div(4);
        ...
        if (takeFee) {
            if((ammPairs[from])||(ammPairs[to])){
                ...
                super._transfer(from, deadWallet, amount.div(100).mul(1));//1%销毁 
