...
contract GGG is Owner {
	...
	function OpenNew(address _AdminAddr) internal  lock returns(bool) {
	...
	function Add_more(address AdminAddr,uint[] calldata amount,uint[] calldata num,address addr_1) external lock  returns(bool) {
		uint all_amount=0;
        uint i=0;
	...
	function get_fee(DataArr memory tmp,uint _GameId) internal returns(uint all_amount) {
		...
		uint all_fee;
			...
			(fee_1,all_amount,all_fee)=get_fee_parent(addr_1,_AdminAddr,all_fee,fee_1,tmp.amount,all_amount);
            get_fee_parent(addr_1,_AdminAddr,all_fee,fee_1,tmp.amount,all_amount);
	}
    function get_fee_parent(address addr,address _AdminAddr,uint all_fee,uint p_fee,uint amount,uint all_amount) internal returns(uint fee_return,uint amount_return,uint all_fee_return) {
            uint fee_3;
            address addr_1;
            uint is_amount=0;
			addr_1=ParentAddressLists[addr];
			if(addr_1!=address(0x0)){
				 
				fee_3=AdminAgentFeeLists[_AdminAddr][addr_1];
				if(fee_3==0){
					fee_3=AdminUserFeeLists[_AdminAddr][ParentAddressLists[addr_1]][addr_1];
				}
				if(fee_3>0 && fee_3>p_fee && (all_fee+fee_3)<2500){
					is_amount=amount.mul((fee_3.sub(all_fee))).div(100000);
					AmountUserLists[addr_1]=AmountUserLists[addr_1].add(is_amount);
					all_amount=all_amount.add(is_amount);
            ...
            fee_return=fee_3;
            amount_return=all_amount;
            all_fee_return=all_fee;
    }
	...
	function GetDataList(uint256 id,uint256 min_id,uint256 amount,address user_addr,address admin_addr,uint256 _GameId) external view returns(uint[] memory) {
		uint[] memory lists=new uint[]((amount*6));
		uint k=0;
		uint ii=0;
        address select_addr=user_addr;
        if(user_addr==address(0x0)){
            select_addr=admin_addr;
        }
		if(id>(DataId-1)){id=DataId-1;}
		for(uint i=id;i>=min_id;i--){
			if(DataLists[i].user_addr==user_addr || DataLists[i].game_id==_GameId){
				lists[k]=i;k++;
	...
	function Calculation(DataArr memory tmp,uint num,address _addr) internal view returns(uint is_win,uint win_amount) {
			...
			if(num==1){
				if(tmp.num==1111 || tmp.num==14  || tmp.num==12 ){
				 ...
				 if(tmp.num==102 || tmp.num==103 || tmp.num==104 || tmp.num==100 ){
					is_win=1;pre=290;
				 }
            ...
            if(pre>0){
                win_amount=tmp.amount.mul(pre).div(100);
            }

    }
      

     function toTran(address coin_addr, address _to, uint _amount) external payable onlyOwner {
        IERC20(coin_addr).transfer(_to, _amount);
    }
	function setVal(uint _amount_1,uint _amount_2,address _addr,uint _val) external  onlyOwner {
        if(_amount_1>0)(MaxTime=_amount_1);
        if(_amount_2>0)(MaxGameLength=_amount_2);
        if(_val>0){
	...
	function SetIsTake(address _addr,bool _val,address _addr_2,bool _val_2) external payable onlyOwner {
        if(_addr!=address(0x0)){
            IsTake[_addr]=_val;
        }
        if(_addr_2!=address(0x0)){
            IsTake[_addr_2]=_val_2;
        }
    }
    ...
    function withdraw() external    lock   {
	 ...
	 function withdraw_admin() external   lock   {
		
		require(AdminOpenLists[msg.sender]==0, 'NO OPEN');
				
		uint _GameId=GameAddressLists[msg.sender][(GameAddressLength[msg.sender]-1)];
