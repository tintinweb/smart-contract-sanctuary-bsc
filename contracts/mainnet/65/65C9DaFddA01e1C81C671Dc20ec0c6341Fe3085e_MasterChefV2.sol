...
contract OMG is Ownable, ERC20 {
    ...
    address public constant BMCC_ADDRESS = 0xb6D8EE99D5d6cfe7D80b666e6fF5e74e3f72756b;
    ...
    modifier onlyManager {
        require(msg.sender == owner() || msg.sender == ownerAddress);
        _;
    }

    constructor(address _router, address _owner) ERC20("Omnichain Governance", "OMG") {
    ...
    // startTime = block.timestamp;
      ...
      pancakeV2Pair = IPancakeFactory(router.factory())
        .createPair(address(this), BMCC_ADDRESS);
    ...
    function updateOwner(address _newOwner) external onlyManager {
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // uint256 initialBalance = IERC20(BMCC_ADDRESS).balanceOf(address(this));
        uint256 initialBalance = address(this).balance;

        swapTokensForBNB(half);

        // uint256 newBalance = IERC20(BMCC_ADDRESS).balanceOf(address(this)).sub(initialBalance);
        uint256 newBalance = address(this).balance.sub(initialBalance);
...
contract SyrupBar is Ownable, ERC20 {
    ...
    constructor(
        address _omg
    ) ERC20("SyrupBar Token", "SYRUP") {
        omg = IERC20(_omg);
    }

    // Safe omg transfer function, just in case if rounding error causes pool to not have enough OMGs.
    function safeOmgTransfer(address _to, uint256 _amount) public onlyOwner {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "DMSUK::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "DMSUK::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "DMSUK::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "DMUSK::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying OMGs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "DMUSK::_writeCheckpoint: block number exceeds 32 bits");
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of OMGs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accOmgPerShare) - user.rewardDebt
    ///
    ...
    ///     OMG rewards each pool gets.
    /// `accOmgPerShare` Accumulated OMGs per share, times 1e12.
    ...
    ///     the OMG rewards to all the PanomgSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accOmgPerShare;
    ...
    /// @notice Address of OMG contract.
    OMG public immutable omg;

    SyrupBar public immutable SYRUP;

    /// @notice The only address can withdraw all the burn OMG.
    ...
    ///  @notice 40 omgs per block in MCV1
    uint256 public MASTERCHEF_OMG_PER_BLOCK = 200000 * 10 ** 18;
    uint256 public constant ACC_OMG_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total omg rate = toBurn + toRegular + toSpecial
    uint256 public constant OMG_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of OMG burn action being executed.
    /// @notice OMG distribute % for burn
    uint256 public omgRateToBurn = 0;
    /// @notice OMG distribute % for regular farm pool
    uint256 public omgRateToRegularFarm = OMG_RATE_TOTAL_PRECISION;
    /// @notice OMG distribute % for special pools
    uint256 public omgRateToSpecialFarm = 0;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accOmgPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateOmgRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _OMG The OMG token contract address.
    ...
    function setOMGPerBlock (uint256 _number) public onlyOwner {
        MASTERCHEF_OMG_PER_BLOCK = _number * 10 ** 18;
    }
    ...
    /// only for OMG distributions within PanomgSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        uint256 _depositFee,
        ...
        // stake OMG token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != omg, "OMG token can't be added to farm pools");
    ...
    /// @notice Update the given pool's OMG allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending OMG rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingOmg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 omgReward = multiplier.mul(omgPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accOmgPerShare = accOmgPerShare.add(omgReward.mul(ACC_OMG_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accOmgPerShare).div(ACC_OMG_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update omg reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of OMG per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function omgPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_OMG_PER_BLOCK.mul(omgRateToRegularFarm).div(OMG_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_OMG_PER_BLOCK.mul(omgRateToSpecialFarm).div(OMG_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of OMG per block to burn.
    function omgPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_OMG_PER_BLOCK.mul(omgRateToBurn).div(OMG_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accOmgPerShare = pool.accOmgPerShare.add((omgReward.mul(ACC_OMG_PRECISION).div(lpSupply)));

                omg.mint(address(SYRUP), omgReward);
            }

            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accOmgPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingOmg(msg.sender, _pid, multiplier);
        }else if(_amount > 0) {
        ...
        if (_amount > 0) {
            ...
            uint256 _feeAmount = _amount.mul(pool.depositFee).div(FEE_PRECISION);

            _amount = _amount.sub(_feeAmount);

            uint256 before = lpToken[_pid].balanceOf(address(this));
            lpToken[_pid].safeTransferFrom(msg.sender, address(this), _amount);
            lpToken[_pid].safeTransferFrom(msg.sender, feeReceiver, _feeAmount);
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accOmgPerShare).div(
            ACC_OMG_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Send OMG pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnOmg(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingOmgToBurn = multiplier.mul(omgPerBlockToBurn());

        // SafeTransfer OMG
        _safeTransfer(burnAdmin, pendingOmgToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of OMG distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of OMG to burn each block.
    /// @param _regularFarmRate The % of OMG to regular pools each block.
    /// @param _specialFarmRate The % of OMG to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateOmgRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: OMG rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == OMG_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn omg base on old burn omg rate
        burnOmg(false);

        omgRateToBurn = _burnRate;
        omgRateToRegularFarm = _regularFarmRate;
        omgRateToSpecialFarm = _specialFarmRate;

        emit UpdateOmgRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingOmg(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accOmgPerShare).div(
            ACC_OMG_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending OMG rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingOmg(
        address _user,
        uint256 _pid,
        ...
        uint256 accOmg = boostedAmount.mul(pool.accOmgPerShare).div(ACC_OMG_PRECISION);
        uint256 pending = accOmg.sub(user.rewardDebt);


        // uint256 feePercent = pool.withdrawFee.sub(block.timestamp.sub(user.depositTime).div(1 days).mul(20));
        // if(feePercent < 20) feePercent = 20;
        uint256 _fee = pending.mul(pool.withdrawFee).div(FEE_PRECISION);

        pending = pending.sub(_fee);

        // SafeTransfer OMG
        _safeTransfer(_user, pending);
        _safeTransfer(feeReceiver, _fee);
    }

    /// @notice Safe Transfer OMG.
    /// @param _to The OMG receiver address.
    /// @param _amount transfer OMG amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        SYRUP.safeOmgTransfer(_to, _amount);
    }

    function setFeeReceiver(address _new) public onlyOwner {
      require(_new != address(0), "Null address");
      require(_new != feeReceiver, "Already set");
