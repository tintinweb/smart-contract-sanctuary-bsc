...
contract ShrimDeposit is Ownable {
    using SafeMath for uint256;
    using Address for address;
   // IBEP20 public baseToken;

   event Registration(address indexed  investor,uint investment);
    ...
    constructor(address ownerAddress,address _presaleToken,IBEP20 _BUSD) {
    ...
    function Buypackage(uint investment) public payable
	{
    ...
    function Stake(uint investment) public payable
	{
	   // BUSD.transferFrom(msg.sender ,owner, investment);
		emit Registration(msg.sender,investment);
	}
}