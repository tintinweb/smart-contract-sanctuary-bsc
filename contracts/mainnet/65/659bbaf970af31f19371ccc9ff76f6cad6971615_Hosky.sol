...
contract Hosky is ERC20, Auth {
    ...
    string constant private _name = "Hosky";
    string constant private _symbol = "$Hosky";
    ...
    uint256 private sellFeeCharity = 1;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(marketingFee).add(charityFee).add(devFee).add(stakingFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeMarketing).add(sellFeeCharity).add(sellFeeDev).add(sellFeeStaking);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver;
    address private marketingFeeReceiver = 0xB96BEc299F0Fe9638Bbbd899Ac72a9d322F76689;
    address private charityFeeReceiver = 0x9442016041B3a6d8FEe5e0Fe6BA31d56180FF4F4;
    address private stakingFeeReceiver = 0xB96BEc299F0Fe9638Bbbd899Ac72a9d322F76689;
    address private devFeeReceiver = 0x39Ac98cAbc4DA89f7f17de8bEC61689D7E8dE165;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(automatedMarketMakerPairs[sender]) { //buy
            ...
            if (!isFeeExempt[sender]) {	
                require(_balances[recipient].add(amount) <= _maxWalletAmount || isMaxWalletExempt[recipient], "Max Wallet Limit Exceeded");
                require(amount <= _maxTxAmount || isTxLimitExempt[sender], "TX Limit Exceeded");
    ...
    function takeBuyFee(address sender, address recipient, uint256 amount) internal returns (uint256){  
        ...
        uint256 feeAmount = amount.mul(realFee.sub(stakingFee)).div(feeDenominator);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBcharity = amountBNB.sub(amountBNBLiquidity).sub(amountBNBMarketing).sub(amountBNBDev);
        ...
        (tmpSuccess,) = payable(charityFeeReceiver).call{value: amountBNBcharity}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        charityFee = _charityFee;
        devFee = _devFee;
        stakingFee = _stakingFee;
        totalBuyFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be more than 30%");

        emit BuyFeesChanged(_liquidityFee, _marketingFee, _charityFee, _devFee, _stakingFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _charityFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeMarketing = _marketingFee;
        sellFeeCharity = _charityFee;
        sellFeeDev = _devFee;
        sellFeeStaking = _stakingFee;
        totalSellFee = _liquidityFee.add(_marketingFee).add(_charityFee).add(_devFee).add(_stakingFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be more than 30%");

        emit SellFeesChanged(_liquidityFee, _marketingFee, _charityFee, _devFee, _stakingFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _charityFeeReceiver, address _devFeeReceiver, address _stakingFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _marketingFeeReceiver != address(0) && _charityFeeReceiver != address(0)  && _devFeeReceiver != address(0) && _stakingFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _marketingFeeReceiver, _charityFeeReceiver,_devFeeReceiver, _stakingFeeReceiver);
    }
