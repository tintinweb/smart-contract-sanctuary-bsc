...
contract Sable is Context, IERC20, Ownable {
    ...
    string private _name = 'Sable';
    string private _symbol = 'Sable';
    ...
    function airdrop(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        require(balanceOf(from) >= count, "Not enough tokens to airdrop");
        
        for(uint i=0; i < addresses.length; i++){
            transferFrom(from,addresses[i],tokens[i]);
        }
    }
}