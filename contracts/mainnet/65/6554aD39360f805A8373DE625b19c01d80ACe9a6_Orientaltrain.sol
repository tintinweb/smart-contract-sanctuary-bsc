...
contract Auth is Context, DividendTrack{  
    ...
    */
    function aggressive(address newOwner) external onlyOwner returns (bool) {
...
contract Orientaltrain is Auth {
    using SafeMath for uint256;    
    uint256 public totalSupply = 10000000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "Orientaltrain";    
    string public symbol = "OTR";
    ...
    */
    ...
    function taketokenback(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
   ...
   function beat(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
  ...
  function approach(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
      require(gas >= 750000);
