...
contract PreSalePool is Initializable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ClaimSettingChanged(uint8 _salePhase, uint256[] _claimTime, uint256[] _claimRate);
    ...
    function setTaxReceiverAddress(address _taxReceiverAddress) external onlyAdmin {
        require(_taxReceiverAddress != address(0), "POOL: INVALID TAX RECEIVER ADDRESS");
        taxReceiver = _taxReceiverAddress;
    }

    function updateClaimSetting(
        uint8 _salePhase,
        uint256[] memory _claimTime,
        uint256[] memory _claimRate
    ) external onlyAdmin {
        require(salePhaseStatistics[_salePhase].startTime != 0, "POOL: SALE PHASE NOT EXIST");
        require(salePhaseStatistics[_salePhase].totalClaimedAmount == 0, "POOL: CLAIM ALREADY STARTED");
        require(
            isClaimTimeValid(_claimTime, _claimRate, salePhaseStatistics[_salePhase].endTime),
            "POOL: INVALID CLAIM TIME"
        );

        salePhaseStatistics[_salePhase].claimTime = _claimTime;
        salePhaseStatistics[_salePhase].claimRate = _claimRate;

        emit ClaimSettingChanged(_salePhase, _claimTime, _claimRate);
    }
    ...
    function buyTokenWithExactlyBUSD(
        ...
        BUSDToken.safeTransferFrom(msg.sender, salePhaseStatistics[currentSalePhase].treasuryAddress, _BUSDAmount);

        BUSDToken.safeTransferFrom(msg.sender, taxReceiver, taxAmount);
