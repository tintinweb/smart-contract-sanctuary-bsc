...
/**
 * @dev TENFIFarm functions that do not require less than the min timelock
 */
interface ITENFIFarm {
    function add(
        uint256 _allocPoint,
        address _want,
        bool _withUpdate,
 ...
 * _Available since v3.3._
 */
contract TENLock is AccessControl {
   using SafeERC20 for IERC20;
    ...
    uint256 public minDelay = 60; // To be changed in production
    ...
    constructor()
    {
        ...
        //deployer + devWalletAddress
        _setupRole(PROPOSER_ROLE, devWalletAddress);
        _setupRole(PROPOSER_ROLE, _msgSender()); // To be revoked after deployment after adding pools.

        //deployer + devWalletAddress
        _setupRole(EXECUTOR_ROLE, devWalletAddress);
        _setupRole(EXECUTOR_ROLE, _msgSender());  // To be revoked after deployment after adding pools.
    ...
    function executeSet(
        ...
        _beforeCall(predecessor);
        ITENFIFarm(_TENFIAddress).set(_pid, _allocPoint, _withUpdate);
        _afterCall(id);
    }
    ...
    {
        ITENFIStrategy(_stratAddress).noTimeLockFunc1();
    }
    ...
    function noTimeLockFunc2(address _stratAddress)
        ...
        onlyRole(EXECUTOR_ROLE)
    {
        ITENFIStrategy(_stratAddress).noTimeLockFunc2();
    }
    ...
    function noTimeLockFunc3(address _stratAddress)
        ...
        onlyRole(EXECUTOR_ROLE)
    {
        ITENFIStrategy(_stratAddress).noTimeLockFunc3();
    }
}