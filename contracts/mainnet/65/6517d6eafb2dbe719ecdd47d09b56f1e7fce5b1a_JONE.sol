...
// JONE Token with Governance.

contract JONE is ERC20Burnable, Ownable, NativeMetaTransaction {
    using SafeMath for uint256;
    uint256 public totalAmount = 500000000e8;

    constructor() ERC20("JONE", "JOT", totalAmount) {
        _initializeEIP712("JONE");
        _moveDelegates(address(0), _delegates[msg.sender], totalAmount);
    }
    ...
    function permit(
        ...
        require(holder == ecrecover(digest, v, r, s), "JONE: INVALID-PERMIT");
        require(
            expiry == 0 || block.timestamp <= expiry,
            "JONE: PERMIT-EXPIRED"
        );
        require(nonce == nonces[holder]++, "JONE: INVALID-NONCE");
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "JONE::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "JONE::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "JONE::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "JONE::getPriorVotes: not yet determined"
        );
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying JONEs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "JONE::_writeCheckpoint: block number exceeds 32 bits"
        );
