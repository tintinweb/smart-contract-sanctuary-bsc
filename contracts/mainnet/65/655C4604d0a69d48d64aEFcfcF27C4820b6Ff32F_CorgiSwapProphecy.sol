...
interface ICorgiSwapProphecy {
 ...
 /**
     ...
     * @param _endTime: endTime of the performance
...
contract CorgiSwapProphecy is ReentrancyGuard, ICorgiSwapProphecy, Ownable {
    ...
    // Bracket calculator is used for verifying claims for prophecy prizes
    ...
    */
    ...
    function startPerformance(
        ...
        require(
            _endTime > _startTime,
            "End time prformance is over"
        );

        require(
            _startTime > block.timestamp,
            "Start time prformance is over"
        );
    ...
    function claimProphecies(
        ...
        for (uint256 i = 0; i < _propheciesIds.length; i++) {
            ...
            rewardInCorisToTransfer += rewardForProphecyId;
        }

        // Transfer money to msg.sender
        corisToken.safeTransfer(msg.sender, rewardInCorisToTransfer);

        emit ProphecyClaim(msg.sender, rewardInCorisToTransfer, _performanceId, _propheciesIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_performances[_performanceId].amountCollected) * (10000 - _performances[_performanceId].treasuryFee))
        ) / 10000;
        ...
        amountToWithdrawToTreasury += (_performances[_performanceId].amountCollected - amountToShareToWinners);
