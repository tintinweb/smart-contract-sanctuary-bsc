...
contract AggregatorExecutor is Ownable {
    ...
    modifier checkSigner(bytes memory _signature, uint256 _nonce) {
        require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encodePacked(address(this), _nonce)).toEthSignedMessageHash();
    ...
    //=========== external functions ============//
    function swap(SwapStep[] memory _steps, uint256 _amountIn, uint256 _amountOutMin, address payable _receiver, uint256 _deadline, Signature calldata _signature)
    external payable
    ensure(_deadline)
    checkSigner(_signature.data, _signature.nonce)
    {
        //check input eth
    ...
    function verifySignature(bytes memory _signature, uint256 _nonce) external view returns(bool) {
        require(!usedNonce.get(_nonce), "AggregatorExecutor: _nonce has been used");
        bytes32 _hash = keccak256(abi.encodePacked(address(this), _nonce)).toEthSignedMessageHash();
        address _signer = _hash.recover(_signature);
