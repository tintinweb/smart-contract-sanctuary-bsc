...
contract BIMToken is ERC20 {
    ...
    address public oldBimTokenAddr = address(0xBF3073Cdb6192589c12851e898cabEC5B3a3aC6D);
    
    bool public swapping;
    uint256 public swapTokensAtAmount;
    uint256 public startTime;
    bool public isOpenSwap = true;
    ...
    address private ensureFunAddress  = address(0xd7DC635f40737fEFea90A0B50af5DeE0bb2D8Bf8);  
    address private bimToUsdtReceiveAddress = address(0xcfa7AabbC69e3286695aB313e68052466390242E);  
    ...
    constructor() ERC20("BitMessage", "BIM", 18) {
    ...
    function swapOldBimToNew() public returns (bool) {
        uint256 oldAmt = IERC20(oldBimTokenAddr).balanceOf(_msgSender());
        require(oldAmt > 0, "swapOldBimToNew: balance not enough");

        TransferHelper.safeTransferFrom(oldBimTokenAddr, _msgSender(), destroyAddress, oldAmt);

        super._transfer(address(this), _msgSender(), oldAmt);
    ...
    function chgAddresses(address ensureFunAddress_, address bimToUsdtReceiveAddress_) public onlyOwner {
        ensureFunAddress  = ensureFunAddress_;
        bimToUsdtReceiveAddress = bimToUsdtReceiveAddress_;
    }
    ...
    function _transfer(
        ...
        if(waitSellTokenNum > 0) {
            if (
                ...
                swapping = true;
                swapTokensForOther(waitSellTokenNum);
                waitSellTokenNum = 0;
                swapping = false;
            }
        }
        ...
        if (takeFee) {
            ...
            super._transfer(from, ensureFunAddress, amount * 2 / 100);

            if(uniswapPairAddrs[from]){  
                super._transfer(from, address(this), amount * 2 / 100);
                waitSellTokenNum += amount * 2 / 100;           
            } else {  
                
                super._transfer(from, destroyAddress, amount * 2 / 100);
            }
