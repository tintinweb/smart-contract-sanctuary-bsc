...
abstract contract ERC20 is IERC20, Config, ERC20Detailed {
    ...
    function swapAndLiquify(address sender, uint256 amount) public lockTheSwap returns(bool) {
        uint256 half = amount.div(2);
        uint256 otherHalf = amount.sub(half);

        uint256 initialBalance = usdtToken.balanceOf(sender);   //记录对换前余额

        swapTokensForToken(half, address(this), address(usdtToken), sender);    //用一半token兑换usdt

        uint256 newBalance = usdtToken.balanceOf(sender).sub(initialBalance);   //获取兑换到的usdt数量
    ...
    function swapTokensForToken(
        ...
        _approve(to, address(uniswapV2Router), tokenAmount);
...
abstract contract Appreciation is ERC20 {
    ...
    function reviseTotalSupply() public returns(bool) {
        ...
        for(uint256 i=0;i<(block.timestamp-_reviseTimestamp).div(addTime);i++) {
            ...
            _totalSupply = oldTotalSupply.add(oldTotalSupply.sub(_balances[address(uniswapV2Pair)]).mul(getAddedFee(_reviseCount[address(this)])).div(1000));

            emit AppreciationLog(address(this), oldTotalSupply, _totalSupply, _reviseCount[address(this)]);
    ...
    function reviseBalance(address addr) public returns(bool) {
        if (addr != address(uniswapV2Pair)) {
            if (_reviseCount[addr] == 0) { _reviseCount[addr] = _reviseCount[address(this)]; }
