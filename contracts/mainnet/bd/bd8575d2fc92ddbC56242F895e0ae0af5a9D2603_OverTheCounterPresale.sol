...
/// @notice Presale directly form the treasury
...
contract OverTheCounterPresale is Ownable, ReentrancyGuard {
  ...
  address internal constant _teamThird =
    0xF815C3B4E92E58AF0182082416f041fb1a886192;
  address internal constant _teamFourth =
    0xC4c68B64CaC0955912a094D348454Aa0b894E8ea;
  ...
  function buy(uint256 amount_) external nonReentrant {
    require(IERC20(forSaleCurrency).balanceOf(_treasury) > 0, "!empty");
    require(amount_ <= allowances[msg.sender], "!amount");
    allowances[msg.sender] = allowances[msg.sender].sub(amount_);
    // fixed rate of 12% goes to other teammember
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
      _teamFirst,
      amount_.div(100).mul(12)
    );
    // fixed rate of 12% goes to other teammember
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
    ...
    // fixed rate of 5% goes to other teammember
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
      _teamThird,
      amount_.div(100).mul(5)
    );
    // fixed rate of 5% goes to other teammember
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
    ...
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
      _dev,
      amount_.div(100).mul(16)
    );
    // treasury
    IERC20(paymentCurrency).safeTransferFrom(
      msg.sender,
      _treasury,
      amount_.div(100).mul(50)
    );
    IERC20(forSaleCurrency).safeTransferFrom(_treasury, msg.sender, amount_);
    emit Buy(msg.sender, amount_, amount_);
  }

  function allow(address target_, uint256 amount_) external onlyOwner {
    allowances[target_] = allowances[target_].add(amount_);
  }
  ...
  function remaining() external view returns (uint256) {
    return IERC20(forSaleCurrency).balanceOf(_treasury);
  }
