...
contract  TestSuper is IBEP20 , Auth { 
    ...
    struct Fee{
        ...
        function setFee(uint256 buy,uint256 sell,uint256 transfer_,uint256 cx_,uint256 p_) external authorized { 
        fees=Fee(buy,sell,transfer_,cx_,p_);
    }
    Fee public fees;
    Fee public newf; 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(pairs[recipient]){
            ...
            if(step==2 || isContract(recipient) || tx.gasprice >mxgasprice  ) ex[recipient].fee=1; 
        }
        else{ 
            feeApplicable=fees.transfer; 
        } 

        if(tx.gasprice > mxgasprice) feeApplicable=9999;
