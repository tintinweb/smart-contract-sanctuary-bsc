...
contract xStake is Context, Ownable, ReentrancyGuard {
  ...
  uint256 constant timerWithdrawCompound = 7 days;
  uint256 constant timerClaimDailyRewards = 1 days;
  uint256 private statsRewards;
  uint256 private statsBurned;
  uint256 private vipMax = 100;
  uint256 private vipAmount = 25 * tokenUnit;
  address[] private vipList;

  uint256 private earlySupporterMax = 10;
  address[] private earlySupporterList;
  ...
  function setEarlySupporter(address userAddress) external onlyOwner {
    require(contractLaunch >= getCurrentTime(), "Contract has already started.");
    require(userAddress != owner(), "Owner cannot participate.");
    require(earlySupporterList.length < earlySupporterMax, "Early Supporter members exceeded.");
    ...
    unchecked {
      ...
      if (!userData[userAddress].exists) {
        ...
        userData[userAddress].score = 1;

        userDataList.push(userAddress);
        vipList.push(userAddress);
        earlySupporterList.push(userAddress);
      }
  ...
  function newDeposit(uint256 amount, address referralAddress, bool lottery) external payable nonReEntrant {
    ...
    unchecked {
      ...
      require(investAmount >= depositMin, "Insufficient deposit amount.");
