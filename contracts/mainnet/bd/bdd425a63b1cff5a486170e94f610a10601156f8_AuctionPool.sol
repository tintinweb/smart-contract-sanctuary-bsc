...
contract AuctionPool {
    ...
    uint256 public poolDuration = 24 hours;
    ...
    function finalizePool() public onlyOwner {
        ...
        userBids[winner][currentRound][
            userBids[winner][currentRound].length - 1
        ].isExist = false;
        winners.push(winner);
        rounds[currentRound].winner = winner;
        rounds[currentRound].active = false;
        users[winner].totalWinning = rewardAmount;
        users[winner].winCount++;
        users[winner].lastWinAt = block.timestamp;
        if (winner != address(0)) {
