...
contract CHX is Context, IERC20, Ownable {
    ...
    uint public maxBuy = 20000 * 10 ** _decimals;
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && ammPairs[from]) {
          uint256 limitAmount = balanceOf(to).add(amount);
          require(limitAmount <= maxBuy, 'Exceed buying and selling limit');
        }
