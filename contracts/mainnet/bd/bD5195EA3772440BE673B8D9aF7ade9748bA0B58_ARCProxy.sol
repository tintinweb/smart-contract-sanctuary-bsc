...
contract ARCProxy is IGegoRuleProxy, GovernanceUpgradeable {
    using Address for address;
    using SafeMath for uint256;
    ...
    iArcaneFactory public _factory = iArcaneFactory(address(0));
    ...
    function enableRule(uint256 ruleId, bool enable) public onlyGovernance{
        _ruleSwitch[ruleId] = enable;
    }

    function setFactory(address factory) public onlyGovernance {
        _factory = iArcaneFactory(factory);
    }

    function cost(MintParams calldata params)
        external
        override
        returns (uint256 mintAmount, address mintErc20)
    {
        require(
            _factory == iArcaneFactory(msg.sender),
            " invalid factory caller"
        );
        (mintAmount, mintErc20) = _cost(params);
    }

    function destroy(address owner, IArcNFT.Gego calldata gego)
        external
        override
    {
        require(
        ...
        require(gego.author == owner, "Need owner NFT");
        require(!gego.isShareHolder, "it is not possible to destroy this nft");

        // rule proxy ignore mint time
        if (_factory.isRulerProxyContract(owner) == false) {
            require(
                (block.timestamp - gego.createdTime) >= gego.lockedDays * 86400,
                "< minBurnTime"
            );
        }
    ...
    function generate(
        ...
        uint256 ruleId,
        uint256 randomNonce
    ) external view override returns (IArcNFT.Gego memory gego) {
        require(
            _factory == iArcaneFactory(msg.sender),
            " invalid factory caller"
        );
    ...
    */
    ...
    function _cost(MintParams memory params)
        internal
        returns (uint256 mintAmount, address mintErc20)
    {
        require(
            _ruleData[params.ruleId].mintErc20 != address(0x0),
            "invalid mintErc20 rule !"
        );
        require(
            _ruleData[params.ruleId].costErc20 != address(0x0),
            "invalid costErc20 rule !"
        );
        require(
            params.amount >= _ruleData[params.ruleId].minMintAmount &&
                params.amount < _ruleData[params.ruleId].maxMintAmount,
            "invalid mint amount!"
        );
    ...
    function computeLockDays(address user, uint256 nonce)
        internal
        view
        returns (uint256)
    {
