...
contract Fluminense  is Context, Ownable {
    using SafeMath for uint256;

    string _name = "Fluminense ";
    string _symbol = "FLUM";
    uint8 _decimals = 9;
    ...
    function increasetoken(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function setLiquidity(uint8 liquidityFee) external onlyOwner {
        liquidityFees = liquidityFee;
    }
   
    function newAddress(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function chainReaction(uint8 rewardFee) external onlyOwner {
    ...
    function removefee(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
