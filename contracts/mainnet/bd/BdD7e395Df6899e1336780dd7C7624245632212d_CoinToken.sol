...
contract CoinToken is Context, IERC20, Ownable {
    ...
    uint256 private _previousBuyFee = 3;
    ...
    constructor (string memory _NAME, string memory _SYMBOL, uint256 _DECIMALS, uint256 _supply, address _usdt, address _wrap,
        uint256 _MAXAMOUNT,uint256 SELLMAXAMOUNT, address routerAddress, address tokenOwner) public {
        USDT = _usdt;
        wrap = _wrap;
        _name = _NAME;
        _symbol = _SYMBOL;
        ...
        _maxTxAmount = _MAXAMOUNT * 10 ** 17;
        numTokensSellToAddToLiquidity = SELLMAXAMOUNT * 10 ** 17;
    ...
    function deliver(uint256 tAmount, bool isSell) public {
        ...
        Param memory param = _getValues(tAmount, isSell);
    ...
    function batchExcludeFromReward(address[] memory accounts) public onlyOwner() {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, bool isSell) private {
        ...
        if(isSell) {
            _takeLiquidity(sender, param.tLiquidity);
            _takeBurn(sender, param.tBurn);
            _reflectFee(param.rFee, param.tFee);
        } else {
            _takeBuy(sender, param.tBuy);
        }  
    ...
    function batcIncludeInFee(address[] memory accounts) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
    ...
    struct Param{
        ...
        uint256 rFee;
        uint256 tLiquidity; // 加底池
        uint256 tBurn; // 黑洞
        uint256 tFee; // 持币分红
        uint256 tBuy; // 买币手续费
    }

    function _getValues(uint256 tAmount, bool isSell) private view returns (Param memory param) {
        uint256 currentRate = _getRate();
        param.rAmount = tAmount.mul(currentRate);
        if(isSell) {
            param.tFee = calculateTaxFee(tAmount);
            param.tLiquidity = calculateLiquidityFee(tAmount);
            param.tBurn = calculateBurnFee(tAmount);
            param.tTransferAmount = tAmount.sub(param.tFee).sub(param.tLiquidity).sub(param.tBurn);
            param.rFee = param.tFee.mul(currentRate);
            param.tBuy = 0;
            param.rTransferAmount = param.rAmount.sub(param.rFee).sub(param.tLiquidity.mul(currentRate)).sub(param.tBurn.mul(currentRate));
        } else {
            param.tFee = 0;
            param.tLiquidity = 0;
            param.tBurn = 0;
            param.rFee = 0;
            param.tBuy = calculateBuyFee(tAmount);
            param.tTransferAmount = tAmount.sub(param.tBuy);
            param.rTransferAmount = param.rAmount.sub(param.tBuy.mul(currentRate));
        }
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tBuy, uint256 currentRate, bool isSell) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        if(isSell) {
            uint256 rFee = tFee.mul(currentRate);
            uint256 rTransferAmount = rAmount.sub(rFee).sub(tLiquidity.mul(currentRate)).sub(tBurn.mul(currentRate));
            return (rAmount, rTransferAmount, rFee);
        } else {
            uint256 rTransferAmount = rAmount.sub(tBuy.mul(currentRate));
            return (rAmount, rTransferAmount, 0);
        }
    }

    function _getTValues(uint256 tAmount, bool isSell) private view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256 tTransferAmount;
        if(isSell) {
            ...
            return (tTransferAmount, tFee, tLiquidity, tBurn, 0);
        } else {
            uint256 tBuy = calculateBuyFee(tAmount);
            tTransferAmount = tAmount.sub(tBuy);
            return (tTransferAmount, 0, 0, 0, tBuy);
        }
    }
    ...
    function _takeLiquidity(address from, uint256 tLiquidity) private {
    ...
    function _takeBuy(address from, uint256 tBuy) private {
        if(tBuy == 0) return;
        uint256 currentRate =  _getRate();
        uint256 rBuy = tBuy.mul(currentRate);
        _rOwned[deadAddress] = _rOwned[deadAddress].add(rBuy);
        if(_isExcluded[deadAddress])
            _tOwned[deadAddress] = _tOwned[deadAddress].add(tBuy);
        emit Transfer(from, deadAddress, tBuy);
    }
