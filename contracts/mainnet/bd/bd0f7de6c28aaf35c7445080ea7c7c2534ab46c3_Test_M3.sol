...
//////
    ...
    function invest(address referrer, uint8 plan) public payable{
        ...
        emit FeePayed(msg.sender, fee);
			...
			address upline1 = user.firstLevelReferrals;
				if (upline1 != address(0)) {
                    ...
                    //block.timestamp.add(plans[plan].time.mul(TIME_STEP))
			}
		...
		if (user.deposits.length == 0) {
            ...
            user.checkpointRefBonusW = block.timestamp + (plans[plan].time);
  ...
  function withdraw() public {
		...
		require(block.timestamp >= user.checkpointRefBonusW);
...
//}
    ...
    function getUsercheckpointRefBonusW(address userAddress)
        public
        view
        returns (uint256)
    {

        return users[userAddress].checkpointRefBonusW;
    }
