...
contract TrialBrandNew is Context, IBEP20, Ownable {
    ...
    string private constant _name = "TrialNew";
    string private constant _symbol = "TrialNew";
    ...
    // TAX
    // 2% reward holder (On buy)
    // 1% marketing  (On buy)
    // 1% burn referred as dev (On buy)
    //
    // 4% reward holder  (On sell)
    // 1% marketing   (On sell)
    // 1% burn referred as dev  (On sell)

    // The taxes are not changable by anyone for safu purposes
    Taxes public taxes = Taxes(2, 1, 0, 1);
    Taxes public sellTaxes = Taxes(4, 1, 0, 1);
