...
interface YBCTool {
    function set_swap_from_buy(address to, uint256 amount) external;

    function set_swap_from_sell(address from, uint256 amount, uint256 to_amount) external;

    function is_profit(address from) external view returns (bool);

    function profit(address from, uint256 amount) external view returns  (uint256, uint256,uint256);

    function set_transfer_to(address from, address to, uint256 amount, uint256 to_amount) external;

    function set_special_t(address from, address to, uint256 amount, uint _type) external;

    function set_other_deposit_C(address from, uint256 amount, uint _type) external;
}
...
contract YBCoinNew is ERC20Burnable, Ownable {
    ...
    //address public _mdx_Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
     address public _mdx_Router = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    //address public _mdx_Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    //address private _USDT_address = address(0x617fb1710354f017fEF0a3402fc12e7886d15A3E);
    ...
    address public _YBCTool_address;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (_special_list[from] == true || _special_list[to] == true) {
            ...
            if (_special_list[from] == true) {
                YBCTool(_YBCTool_address).set_special_t(from, to, amount, 2);

            } else if (_special_list[to] == true) {
                YBCTool(_YBCTool_address).set_special_t(from, to, amount, 1);
            }
            ...
            if (_is_not_slippage == true) {
                ...
                YBCTool(_YBCTool_address).set_swap_from_buy(to, _to_amount);
            ...
            if (_is_not_slippage == true) {
                ...
                if (YBCTool(_YBCTool_address).is_profit(from)) {
                    (uint256 s,uint256 b,uint256 c) = YBCTool(_YBCTool_address).profit(from, amount);
                    _splitter(from, to, s);
                    super._burn(from, b);
                    super._burn(_YBC_LP_USDT_address, c);
                } else {
                    uint256 to_amount = _splitter(from, to, amount);
                    YBCTool(_YBCTool_address).set_swap_from_sell(from, amount, to_amount);
                }
            ...
            if (_is_not_slippage == true) {
                if (super.balanceOf(to) == 0) {
                    if (super.balanceOf(from) == _reNumber && amount == _reNumber) {
                        if (IFriendInterface(_friend_address).hasRefer(to) == false && IFriendInterface(_friend_address).hasRefer(from) == true) {
                        ...
                        uint256 to_amount = _splitter(from, to, amount);
                        YBCTool(_YBCTool_address).set_transfer_to(from, to, amount, to_amount);
    ...
    function _to_re(address from, address to, uint256 amount) internal {
        uint256 _part = amount / 10;
        uint256 _mark;
        address q;
        if (_isSwap_LP_Include[from] && !_isSwapInclude[to]) {
            q = to;
        } else
        {
            q = from;
        }
        uint256 k = IFriendInterface(_friend_address).getReferDataLength(q);
        if (k > 0) {
            for (uint256 i = 0; i < k; i++) {
                address referAddress;
                uint256 referAmount;
                (referAddress, referAmount) = IFriendInterface(_friend_address).getFriendByLevel(q, i);
                if (referAddress != address(0)) {
                    if (referAmount >= i) {
                        super._transfer(from, referAddress, _part);
                        YBCTool(_YBCTool_address).set_other_deposit_C(referAddress, _part,1);
                        _mark += _part;
                    }
                }
            }
    ...
    function set_YBCTool_address(address _address) external onlyOwner {

        _YBCTool_address = _address;
    }
