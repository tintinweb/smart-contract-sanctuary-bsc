...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all FamilyVerse just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also FamilyVerseed.
	...
	// File: contracts/FamilyVerseBEP20PayingToken.sol
	...
	// File: contracts/FamilyVerse.sol
	...
	contract FamilyVerse is ERC20, Ownable {
		...
		event excludeMultipleAccountFromRewards(address[] FamilyVerse, bool isExcluded);
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "FamilyVerse: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "FamilyVerse: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "FamilyVerse: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "FamilyVerse: Automated market maker pair is already set to that value");
		...
		function updateLiquidityWallet(address newLiquidityWallet) private onlyOwner {
			require(newLiquidityWallet != liquidityWallet, "FamilyVerse: The liquidity wallet is already this address");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "FamilyVerse: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "FamilyVerse: Cannot update distributorGas to same value");
		...
		function _transfer(
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "FamilyVerse: This account cannot send tokens until trading is enabled");
			}
