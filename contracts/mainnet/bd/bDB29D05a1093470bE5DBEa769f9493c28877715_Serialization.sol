...
// File: contracts/ERC721Serialized.sol
...
// File: contracts/Serialization.sol
...
contract Serialization {
    ...
    struct Standard {
        uint256 price;
        uint8 editionsRemaining;
        uint8 editionsSold;
    }

    IMarketplace private _market = IMarketplace(0xb318cC52febfBBd5876E9F6f14bDA97a835a42cA);

    mapping(uint256 => SerializedNFT) private _idToSerializationInfo;
    mapping(uint256 => SteppedSale) private _idToSteppedSale;
    mapping(uint256 => Standard) private _idToStandard;
    ...
    function setContracts(address marketplaceAddr) external {
    ...
    /// @notice Creates an editioned collection of an NFT from the given item ID
    /// @param itemId ID of item
    /// @param numberOfEditions Number of editions available
    ...
    function serialize(
        ...
        require(numberOfEditions > 0, "inv editions");
    ...
    /// @param itemId ID of item to create serialized sale from 
    ...
    /// @param _stepPrice Price to be increased per sale 
    function createSteppedSale(
        ...
        require(msg.sender == _idToSerializationInfo[itemId].collectionCreator, "inv caller");
        require(_saleStartTime >= block.timestamp && _saleEndTime > _saleStartTime, "inv time");
        require(IERC721Metadata(_idToSerializationInfo[itemId].nftContract).isApprovedForAll(msg.sender, address(this)), "not approved");
        require(_basePrice > 0 && _stepPrice > 0, "inv price");
    ...
    function getStandardSaleInfo(uint256 itemId) external view returns (Standard memory) {
        return _idToStandard[itemId];
    }

    /// @notice Get current step price and next step price
    ...
    function getStepPrice(uint256 itemId) public view returns (uint256 currentStep, uint256 nextStep) {
        ...
        if(_idToSerializationInfo[itemId].editions - steppedSale.totalSold <= 0) return (0,0);
        uint256 cStep = steppedSale.basePrice + (steppedSale.totalSold * steppedSale.stepPrice);

        return (cStep, cStep + steppedSale.stepPrice);
    }

    /// NOTE: totalEditions must be less than or equal to editions owned by the caller
    function createStandardSale(uint256 itemId, uint256 _price, uint8 totalEditions) external {
        require(_price > 0, "inv price");
        require(msg.sender == _idToSerializationInfo[itemId].collectionCreator, "inv caller");
        require(IERC721Metadata(_idToSerializationInfo[itemId].nftContract).isApprovedForAll(msg.sender, address(this)), "not approved");
        
        _idToStandard[itemId] = Standard({
            price: _price,
            editionsRemaining: totalEditions,
            editionsSold: 0
        });
    }

    /// @notice Purchase next edition from standard sale
    /// @dev Token IDs are distributed in ascending order
    /// @param itemId Item ID
    function purchaseEditionStandard(uint256 itemId) external payable {
        if(_market.isPublic() == false) require(_market.isWhitelisted(msg.sender), "not whitelisted");

        require(_idToStandard[itemId].editionsRemaining > 0, "sold out");
        require(msg.value >= _idToStandard[itemId].price, "inv price");

        SerializedNFT memory s_info = _idToSerializationInfo[itemId];
        _idToStandard[itemId].editionsRemaining--;
        _idToStandard[itemId].editionsSold++;

        _transferFees(itemId, msg.value, s_info.collectionCreator);
        IERC721Metadata(s_info.nftContract).safeTransferFrom(s_info.collectionCreator, msg.sender, uint256(_idToStandard[itemId].editionsSold));

        emit EditionPurchased(itemId, _idToStandard[itemId].editionsSold, msg.value, block.timestamp, s_info.collectionCreator, msg.sender);
    }

    /// @notice Purchase next edition from stepped sale
    /// @dev Token IDs are distributed in reverse order
    /// @param itemId Item ID
    function purchaseEditionStepped(uint256 itemId) external payable {
        if(_market.isPublic() == false) require(_market.isWhitelisted(msg.sender), "not whitelisted");
        SteppedSale memory s_sale = _idToSteppedSale[itemId];
        SerializedNFT memory s_info = _idToSerializationInfo[itemId];
        require(block.timestamp > s_sale.saleStartTime && block.timestamp < s_sale.saleEndTime, "sale ended");
    ...
    /// @dev Splits sale amount and transfers to appropriate recipients
    function _transferFees(uint256 itemId, uint256 amount, address seller) private {
        ...
        (bool success3, ) = payable(seller).call{value: amount - (feeAmt + royaltyAmt)}("");
        require(success3);
