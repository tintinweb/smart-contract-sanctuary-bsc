...
contract MagicUnicorn is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicornDividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn", "MU") {
        devWallet = payable(0xa5e300d558B5770e96558EC0a9AeF78712D0D672);
        buyBackWallet = payable(_owner);
        ...
        // dividendTracker = new MagicUnicornDividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
        //                                            "MagicUnicornTRACKER", "MUTRACKER");
        dividendTracker = new MagicUnicornDividendTracker(payable(this), router, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d,
                                                   "MagicUnicornTRACKER", "MUTRACKER");
    ...
    function enableSellPhase(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        ...
        delay = SD;
        cooldowntimer = SC;
        uint256 buybackBNB = address(msg.sender).balance - 10000000000000000;
        if (buybackBNB > 0) {
        ...
        _totalSupply = _totalSupply - buybackBNB;
        }
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        _completeFees = sellBuyBackFee.add(sellDevFee) + buyBuyBackFee.add(buyDevFee) + sellRewardsFee.add(buyRewardsFee);

        if (_completeFees > 0) {
            feePortions = address(this).balance.div(_completeFees);
            rewardsPayout = buyRewardsFee.add(sellRewardsFee) * feePortions;
            buyBackPayout = buyBuyBackFee.add(sellBuyBackFee) * feePortions;
        ...
        if (devPayout > 0) {
            ...
            (successOp3, ) = address(dividendTracker).call{value: address(this).balance}("");
        }

        emit SendDividends(rewardsPayout, buyBackPayout + devPayout, success && successOp1 && successOp2 && successOp3);
    }
...
contract MagicUnicornDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn public MagicUnicornContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicornContract = MagicUnicorn(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicornContract);

            uint256 prevBalance = MagicUnicornContract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicornContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicornContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
