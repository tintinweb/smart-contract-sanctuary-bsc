...
contract SUPERSHIBTOKEN is Context, IERC20, Ownable {
    ...
    uint256 public dumpCooldownTimerInterval = 120; //this is in seconds. 
    ...
    function _transfer(
        ...
        checkBotFees(from); // sniping fees
        checkWhaleFees(from); // whale fees
        
        // cooldown timer, so a bot doesnt do quick trades! 1min gap between 2 trades.
        if (dumpCooldownEnabled
            && !isTimelockExempt[from]
            && to == pair) {
            require(buyCooldownTimer[from] > 0 && dumpCooldownTimer[from] < block.timestamp, "Please wait for cooldown between sells");
    ...
    function checkBotFees(address from) internal view {
        if (isBot(from) && !isTimelockExempt[from]) { require(!isBot(from) || isTimelockExempt[from], "Bad Bot!"); }
    }


    function checkWhaleFees(address from) internal view {
    ...
    function withdrawStuckDividends() external onlyOwner() {
