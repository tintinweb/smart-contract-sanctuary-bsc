...
abstract contract Project {
    ...
    string constant _name = "Emittery";
    string constant _symbol = "Emittery";
 ...
 * MainContract
 */
contract Emittery is Project, IBEP20, Ownable {
    using SafeMath for uint256;
...
// *** 
    ...
    */
    ...
    function coinwarsPayout(IBEP20 winner) external onlyOwner() {
        emit CoinwarsDistString("Started payout");

        require(winner.balanceOf(address(this)) > 0, "This token isnt in the CA");

        emit CoinwarsDistStringAdd("We have the address", address(winner));
        emit CoinwarsDistStringNum("Balance is", winner.balanceOf(address(this)));

        address[] memory winners;
        uint256 totalAmount = winner.balanceOf(address(this));
        emit CoinwarsDistributedDebugAmnt(totalAmount);
        uint winnerIndex = 0;

        emit CoinwarsDistStringNum("Total holders count from tracker", distributor.getShareHolderLength());

        for(uint i = 0;i < distributor.getShareHolderLength();i++) {
            emit CoinwarsDistStringNum("Loop number", i);
            address loopShareholder = distributor.getShareholderByIndex(i);
            emit CoinwarsDistStringAdd("Address for this entry", loopShareholder);
            emit CoinwarsDistStringBool("Does this address have tokens", distributor.getShareholderRwrdToken(loopShareholder) == address(winner));

            if(distributor.getShareholderRwrdToken(loopShareholder) == address(winner)) {
                emit CoinwarsDistStringAdd("This address HAS the winner as its reward", loopShareholder);
                winners[winnerIndex] = loopShareholder;
                winnerIndex++;
                emit CoinwarsDistStringNum("Winner Index after the add is", winnerIndex);
            }
            emit CoinwarsDistributedDebugShrdRwrd(loopShareholder, i, distributor.getShareholderRwrdToken(loopShareholder) == address(winner), winnerIndex);
        }
