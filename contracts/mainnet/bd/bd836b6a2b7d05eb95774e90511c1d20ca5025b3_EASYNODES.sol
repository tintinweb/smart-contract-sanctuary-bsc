...
contract EASYNODES {
    ...
    function withdrawToManager(address _token, uint _amount) external {
        require(msg.sender == manager || msg.sender == feeCollector, "Not Authorized!");
    ...
    function buyShares(address _token, uint _shareCount) external {
        ...
        uint _tokenAmount = sharePrice.mul( _tenToThePowerDecimals).mul( _shareCount ).div(_price);

        IERC20(_token ).safeTransferFrom( msg.sender, address(this), _tokenAmount);

        totalInvestmentsInUSD = totalInvestmentsInUSD.add( _shareCount.mul( _price).mul(sharePrice));
    ...
    //     require(_enableAutoCompound(msg.sender) > 0, "Not enable to autocompound");

    //     uint tenToThePowerDecimals = 10 ** IERC20(rewardToken ).decimals();
        
    //     uint _amount = sharePrice.mul(tenToThePowerDecimals).div(rewardTokenPrice);
    ...
    //     enableRewardsInUSD = enableRewardsInUSD.sub(sharePrice); 
    ...
    function autoCompound(uint _shareAmount) external returns (bool) {
        uint _enableShareCount = _enableAutoCompound(msg.sender);
        require(_shareAmount <= _enableShareCount, "Not enough claimable amount");
        uint tenToThePowerDecimals = 10 ** IERC20(rewardToken ).decimals();

        uint _amount = _shareAmount.mul(sharePrice).mul(tenToThePowerDecimals).div(rewardTokenPrice);
        claimableReward[msg.sender] -= _amount;

        shareCount[msg.sender] = shareCount[msg.sender].add(_shareAmount);
        totalShareCount = totalShareCount.add( _shareAmount);
    ...
    function enableAutoCompoundCount() external view returns (uint) {
        return _enableAutoCompound(msg.sender);
    }

    function _enableAutoCompound(address _address) internal view returns (uint) {
        uint claimableAmount = claimableReward[_address];
        uint tenToThePowerDecimals = 10 ** IERC20(rewardToken ).decimals();

        uint claimableAmountInUSD = claimableAmount.mul(rewardTokenPrice).div(tenToThePowerDecimals);
        uint _shareCount = claimableAmountInUSD.div(sharePrice);

        return _shareCount;
    }

    function changesharePrice(uint _newPrice) external returns(bool) {
        require(msg.sender == manager, "Not Authorized!");
