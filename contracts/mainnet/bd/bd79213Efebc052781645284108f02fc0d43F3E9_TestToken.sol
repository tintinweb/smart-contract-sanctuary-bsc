...
contract TestToken is Context, IERC20, Ownable {
    ...
    string constant private _name = "DontBuyTestOnly";
    string constant private _symbol = "DontBuyTestOnly";
    uint8 constant private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xeEBE80A2d80d4e54500c5AE081BD052E0ff9E6D7); // Marketing Address
    address payable public maintenanceWalletAddress = payable(0x5b7f00DF069Cb67189B57ff44693AeE455738E0d); // Maintenance Address
    address payable public rewardsWalletAddress = payable(0x6876414b57cEAc9Cff0e23d03FF113F19f479BE3); // Rewards Address
    address payable public buybackWalletAddress = payable(0x1D0b8d759bC1dc3472AB4124373263b9dEBBD1C0); // Buyback Address
    ...
    event BotStatusUpdated(address account, bool isBot);
    ...
    event BuyTaxesChanged(uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newMaintenanceFee, uint256 newRewardsFee, uint256 newBuyBackFee);

    event SellTaxesChanged(uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newMaintenanceFee, uint256 newRewardsFee, uint256 newBuyBackFee);

    event MarketingWalletChanged(address newAddress);

    event RewardsWalletChanged(address newAddress);

    event BuybackWalletChanged(address newAddress);

    event MaintenanceWalletChanged(address newAddress);
    ...
    function setBotStatus(address account, bool value) public onlyOwner {
        isBot[account] = value;
        emit BotStatusUpdated(account, value);
    }
    
    function setBuyTaxes(uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newMaintenanceFee, uint256 newRewardsFee, uint256 newBuyBackFee) external onlyOwner() {
        ...
        emit BuyTaxesChanged(newLiquidityFee, newMarketingFee, newMaintenanceFee, newRewardsFee, newBuyBackFee);
    }
    ...
    function setSellTaxes(uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newMaintenanceFee, uint256 newRewardsFee, uint256 newBuyBackFee) external onlyOwner() {
        ...
        emit SellTaxesChanged(newLiquidityFee, newMarketingFee, newMaintenanceFee, newRewardsFee, newBuyBackFee);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(bnbForMaintenance > 0)
            transferBNBToAddress(maintenanceWalletAddress, bnbForMaintenance);

        if(bnbForRewards > 0)
            transferBNBToAddress(rewardsWalletAddress, bnbForRewards);

        if(bnbForBuyback > 0)
            transferBNBToAddress(buybackWalletAddress, bnbForBuyback);
