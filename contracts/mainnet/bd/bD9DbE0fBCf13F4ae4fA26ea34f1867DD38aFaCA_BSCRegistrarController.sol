...
import "../staking/StakeForDistributeProfits.sol";

/**
 * @dev A registrar controller for registering and renewing names at fixed cost.
 */
contract BSCRegistrarController is Ownable, StakeForDistributeProfits {
    ...
    constructor(BaseRegistrarImplementation _base, PriceOracle _prices, uint _minCommitmentAge, uint _maxCommitmentAge, IERC20 _token) public {
        ...
        ownerController = msg.sender;
    }

    modifier isOwnerController() {
        require(ownerController == msg.sender, '401');
        _;
    }

    function setOwnerController(address _owner) external isOwnerController {
    ...
    function registerFreeForDAO(string calldata name, address owner, uint duration, bytes32 secret) external isOwnerController {
        registerFreeForDAOWithConfig(name, owner, duration, secret, address(0), address(0));
    }

    function registerFreeForDAOWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) public isOwnerController {
        bytes32 commitment = makeCommitmentWithConfig(name, owner, secret, resolver, addr);
...
abstract contract BaseRegistrar is Ownable {
    ...
    // A map of addresses that are authorised to registry and renew names.
    mapping(address=>bool) public controllers;

    // Authorises a controller, who can registry and renew domains.
...
contract BaseRegistrarImplementation is ERC721, BaseRegistrar  {
    ...
    constructor(ENS _ens, bytes32 _baseNode) ERC721("CANASTA DOMAIN","CNSD") {
        ens = _ens;
        baseNode = _baseNode;
    }
...
interface PriceOracle {
    /**
     * @dev Returns the price to registry or renew a name.
