...
contract FRONTIERPROJECT is IERC20, Context, Ownable {
    ...
    string private constant _name = "Frontier Project";

    uint256 public __Max_BuySze = 10000000 * 10**9;
    uint256 public __Max_SellSze = 10000000 * 10**9;

    address private __Marketing_Wallet = 0x8496F6370d9D4a10d9ce90026fE21d6e4255c059;
    address private __Team_Wallet = 0x8496F6370d9D4a10d9ce90026fE21d6e4255c059;

    uint256 public __max_WalletSze = 20000000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 5, 1, 1);
    Taxes public taxes = Taxes(0, 5, 1, 1);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= __Max_BuySze, "You are exceeding __Max_BuySze");
            require(balanceOf(to) + amount <= __max_WalletSze, "You are exceeding __max_WalletSze");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= __Max_SellSze, "You are exceeding __Max_SellSze");
            if(to != pair){
                require(balanceOf(to) + amount <= __max_WalletSze, "You are exceeding __max_WalletSze");
            }
            if(coolDownEnabled){
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        __Max_BuySze = maxBuy * 10**decimals();
        __Max_SellSze = maxSell * 10**decimals();
    }
    
    function __Max_WalletSze(uint256 amount) external onlyOwner{
        __max_WalletSze = amount * 10**decimals();
    }
