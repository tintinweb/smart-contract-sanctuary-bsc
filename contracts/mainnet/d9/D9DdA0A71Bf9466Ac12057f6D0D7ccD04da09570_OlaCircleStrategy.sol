...
import "./Interfaces/IOlaLogic.sol";
import "./Interfaces/IXToken.sol";
import "./Interfaces/ICompoundOla.sol";
import "./Interfaces/IStorage.sol";

contract OlaCircleStrategy is LogicUpgradeable {
  ...
  function __OlaCircleStrategy_init(
  ...
  modifier onlyStorage() {
    require(msg.sender == _storage, "OC1");
    _;
  }

  modifier isUsedToken(address token) {
    require(oTokens[token] != address(0), "OC2");
    _;
  }
  ...
  function setStorage(address storage_) external onlyOwner {
    require(_storage == address(0), "OC4");
    _storage = storage_;
   ...
   */
  function setPathToSwapRewardsToBLID(address[] calldata path) external onlyOwner {
    uint256 length = path.length;
    require(length >= 2, "OC5");
    require(path[0] == rewardsToken, "OC6" );
    require(path[length - 1] == blid, "OC7" );
  ...
  function init(address token, address oToken) public onlyOwner {
    require(oTokens[token] == address(0), "OC9");

    address _logic = logic;
    oTokens[token] = oToken;

    // Add token/oToken to Logic
    IOlaLogic(_logic).addXTokens(token, oToken, 1);

    // Entermarkets with token/otoken
    address[] memory tokens = new address[](1);
    tokens[0] = oToken;
    IOlaLogic(_logic).enterMarkets(tokens, 1);

    // Approve rewards token
    if (!rewardsInit) {
      (IERC20Upgradeable(rewardsToken).approve(rewardsSwapRouter, type(uint256).max));
    ...
    isUsedToken(token)
  {
    ...
    IOlaLogic(logic).takeTokenFromStorage(amount, token);

    // create circle with total balance of token
    createCircles(token, circlesCount);
   ...
   * @param token token address
   */
  function destroy(address token) external onlyOwnerAndAdmin isUsedToken(token) {
    address _logic = logic;
    ...
    uint256 amountStorage = IStorage(_storage).getTokenDeposited(token) - IStorage(_storage).getTokenBalance(token);

    if (amountStorage < balanceToken)
      IOlaLogic(_logic).returnTokenToStorage(amountStorage, token);
    else
      IOlaLogic(_logic).returnTokenToStorage(balanceToken, token);
   ...
   * @param _circlesCount Count number
   */
  function rebuild(address token, uint8 _circlesCount) external onlyOwnerAndAdmin isUsedToken(token) {
    ...
    // create circle with total balance of token
    createCircles(token, _circlesCount);
    ...
    isUsedToken(token)
  {
    ...
    IOlaLogic(_logic).addEarnToStorage(amountBLID);
  ...
  //   for (uint256 i = circlesCount; i > 0;) {
  //     IOlaLogic(logic).redeemUnderlying(cToken, redeemAmount);
  //     IOlaLogic(logic).repayBorrow(cToken, redeemAmount);
  ...
  //   if (IERC20Upgradeable(token).balanceOf(_logic) < amount)
  //     revert("no money");
  // }
   ...
   */
  function createCircles(address token, uint8 _iterateCount) private  {
    uint256 collateralFactor; // the maximum proportion of borrow/lend
    address cToken = oTokens[token];
    address _logic = logic;
    ...
    require(amount > 0, "OC11");

    for (uint256 i = 0; i < iterateCount;) {
      IOlaLogic(_logic).mint(cToken, amount);

      uint256 borrowAmount = (amount * collateralFactor) / 100;
      IOlaLogic(_logic).borrow(cToken, borrowAmount, 1);
   ...
   */
  function destructCircles(address token, uint8 _iterateCount) public {
    ...
    for (uint256 i = 0; i < iterateCount;) {
      ...
      if (borrowBalance == 0) {
        if (cTokenBalance > 0) {
          // redeem and exit
          IOlaLogic(_logic).redeemUnderlying(cToken, supplyBalance);
          return;
        }
      }
      // if already redeemed
      ...
      // if redeem tokens
      IOlaLogic(_logic).redeemUnderlying(cToken, withdrawBalance);
      ...
      if (repayAmount > 0) {
        ...
        if (borrowBalance > repayAmount) {
          IOlaLogic(_logic).repayBorrow(cToken, repayAmount);
        } else {
          IOlaLogic(_logic).repayBorrow(cToken, borrowBalance);
        }
      }
