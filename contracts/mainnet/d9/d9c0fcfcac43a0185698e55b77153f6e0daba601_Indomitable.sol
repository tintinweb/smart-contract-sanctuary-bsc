...
contract Indomitable is ERC20, Ownable {
    using SafeMath for uint256;

    uint256 private _totalSupply = uint256(25100000).mul(1e18);
    uint256 public creationTime;
    bool public disolveApproved = false;
    uint256 internal lockInDuration = 38880000;
    ...
    constructor(
        ...
        address _LIQUIDITY,
        address _DEV_TEAM
    ) ERC20("Indomitable","INDO"){

        creationTime = block.timestamp;
        uint256 _reserveTokens = uint256(1500000).mul(1e18);
        // // Set Phase Details.
        phaseDetails[1] = Phase(_reserveTokens, _reserveTokens, 1000000000000000000); //1
        phaseDetails[2] = Phase(_reserveTokens, _reserveTokens, 1200000000000000000); //1.2
        phaseDetails[3] = Phase(_reserveTokens, _reserveTokens, 1400000000000000000); //1.4
        phaseDetails[4] = Phase(_reserveTokens, _reserveTokens, 1600000000000000000); //1.6
        phaseDetails[5] = Phase(_reserveTokens, _reserveTokens, 1800000000000000000); //1.8
        ...
        uint256 _additionalReserve = uint256(1500000).mul(1e18);

        _transfer(address(this),ECO_SYSTEM,_additionalReserve);
        _transfer(address(this),ADVISORY,_additionalReserve);
        _transfer(address(this),MARKETPLACE,_additionalReserve);
        _transfer(address(this),LIQUIDITY,_additionalReserve);
        _transfer(address(this),DEV_TEAM,_additionalReserve);
    }
    ...
    function addCallerToWhitelist(address _account) public onlyOwner returns(bool){
        require(!whiteListedAccount[_account], "Error: Account already whitelisted!");
    ...
    function removeCallerFromWhitelist(address _account) public onlyOwner returns(bool){
        require(whiteListedAccount[_account], "Error: Account already removed from whitelist!");
    ...
    // Set the approval prior to disolve, to prevent unintended consequences
    function setDisolveApproval(uint256 _status) external onlyOwner returns (bool) {
        require(_status == 1 || _status == 0,"ERC20: Pass either 1 to approve 0 to disapprove!");
        bool newStatus = false;
        if(_status == 1){
            newStatus = true;
        }
        disolveApproved = newStatus;
        return true;
    }

    // Disolve contract to be used only in case of emergency
    function disolve() external onlyOwner returns (bool) {
        require(disolveApproved,"ERC20: Disolve request approval needed!");
