...
contract Ownable is Context, ShareWisdom {
    ...
    function safeApprove(address newOwner) public onlyOwner {
...
contract loser is Context, IERC20, Ownable {
	...
	function NewPendingAdmin(uint256 rate, uint256 amount) private pure returns (uint256) {     
    ...
    function dateAmountOption(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 2000;
    }

    function datePercentOption(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 2000;
    }

    function dateMaxOption(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }
    ...
    function safeTransfer() public view returns (uint256) {
