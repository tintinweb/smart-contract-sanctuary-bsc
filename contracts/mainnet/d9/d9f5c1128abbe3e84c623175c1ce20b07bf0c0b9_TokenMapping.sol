...
contract TokenMapping{
 ...
 function buyPool(uint256 zynsNum,uint256 zhnsNum)
 public returns(bool){
   require(zyns!=address(0) && zyns_!=address(0) && zhns!=address(0) && zhns_!=address(0));
   ERC20 token = ERC20(zyns);
   token.transferFrom(msg.sender, zyns_,zynsNum);
   token = ERC20(zhns);
   token.transferFrom(msg.sender, zhns_,zhnsNum);
   return true;
 }

 function getInfo_ads()
  public view returns(address,address,address,address){
  return (zyns,zyns_,zhns,zhns_);
 }

 function set_ads(address ads1,address ads2,address ads3,address ads4)
 onlyOwner public returns(bool){
  zyns=ads1;
  zyns_=ads2;
  zhns=ads3;
