...
contract PNM is Context, IERC20, Ownable {
    ...
    // Total BNB/LAS collected by various mechanisms (jackpot)
    ...
    event JackpotAwarded(uint256 cashedOut, uint256 tokensOut, uint256 buyerShare, uint256 tokensToBuyer, uint256 toBuyback, uint256 tokensToBuyback);
    event BigBang(uint256 cashedOut, uint256 tokensOut);

    event BuyFeesChanged(uint256 liquidityFee, uint256 jackpotFee);
    event SellFeesChanged(uint256 liquidityFee, uint256 jackpotFee);

    event JackpotFeaturesChanged(uint256 jackpotCashout, uint256 jackpotBuyerShare, uint256 jackpotMinBuy);
    ...
    event BigBangFeaturesChanged(uint256 jackpotHardBuyback, uint256 jackpotHardLimit);
    ...
    constructor(
        ...
        _name = name_;
        _symbol = symbol_;
        _tOwned[msg.sender] = 2000 ether;
        _tOwned[_daoFund] = 3_000_000 ether;
        _tOwned[_devFund] = 3_000_000 ether;
        _tOwned[_treasuryFund] = 3_000_000 ether;
        _tOwned[_reserve] = _tTotal - 9_002_000 ether;
    ...
    function setBuyFees(uint256 liquidityFee, uint256 jackpotFee) external onlyAuthorized {
        bLiquidityFee = liquidityFee;
        bJackpotFee = jackpotFee;

        emit BuyFeesChanged(bLiquidityFee, bJackpotFee);
    }

    function getBuyTax() public view returns (uint256) {
        return bLiquidityFee.add(bJackpotFee);
    }

    function setSellFees(uint256 liquidityFee, uint256 jackpotFee) external onlyAuthorized {
        sLiquidityFee = liquidityFee;
        sJackpotFee = jackpotFee;

        emit SellFeesChanged(sLiquidityFee, sJackpotFee);
    }

    function getSellTax() public view returns (uint256) {
        return sLiquidityFee.add(sJackpotFee);
    }

    function setJackpotFeatures(uint256 _jackpotCashout, uint256 _jackpotBuyerShare, uint256 _jackpotMinBuy) external onlyAuthorized {
        require(
            _jackpotCashout >= 4000 && _jackpotCashout <= 7000,
            "Jackpot cashout percentage needs to be between 40% and 70%"
        );
        require(
            _jackpotBuyerShare >= 5000 && _jackpotBuyerShare <= 10000,
            "Jackpot buyer share percentage needs to be between 50% and 100%"
        );
        require(
            _jackpotMinBuy >= 5 * 10**(BNB_DECIMALS - 2) && _jackpotMinBuy <= 5 * 10**(BNB_DECIMALS - 1),
        ...
        emit JackpotFeaturesChanged(jackpotCashout,jackpotBuyerShare,jackpotMinBuy);
    }

    function setJackpotHardFeatures(uint256 _jackpotHardBuyback,uint256 _jackpotHardLimit) external onlyAuthorized {
        require(
            _jackpotHardBuyback >= 3000 && _jackpotHardBuyback <= 7000,
        ...
        require(
            hardLimitUsd >= 30000 * 10**USDT_DECIMALS && hardLimitUsd <= 250000 * 10**USDT_DECIMALS,
    ...
    function setJackpotTimespanInSeconds(uint256 _jackpotTimespan) external onlyAuthorized {
        require(
            _jackpotTimespan >= 30 && _jackpotTimespan <= 1200,
    ...
    function setNumTokensSellToAddToLiquidity(uint256 numTokens) external onlyAuthorized {
    ...
    function isJackpotEligible(uint256 tokenAmount) public view returns (bool) {
        ...
        uint256 tokensOut = uniswapV2Router.getAmountsOut(jackpotMinBuy, path)[1].mul(MAX_PCT.sub(ROUTER_FEE)).div(
    ...
    function getTokenShares(uint256 tokenAmount) private returns (uint256,uint256) {
        uint256 accSum = 0;
        ...
        if (_liquidityTokens >= LIQ_SWAP_THRESH) {
            (liquidityTokens, accSum) = getUsedTokens(accSum,tokenAmount,_liquidityTokens);
            _liquidityTokens = _liquidityTokens.sub(liquidityTokens);
        }

        (jackpotTokens, accSum) = getUsedTokens(accSum,tokenAmount,_jackpotTokens);
        _jackpotTokens = _jackpotTokens.sub(jackpotTokens);

        return (liquidityTokens, jackpotTokens);
    }
    ...
    function transfer(address from, address to,uint256 amount) private {
        ...
        if (isOverMinTokenBalance && !_inSwapAndLiquify && !_isExcludedFromSwapAndLiquify.contains(from) && swapAndLiquifyEnabled) {
            swapAndLiquify(numTokensSellToAddToLiquidity);
        }

        bool takeFee = true;
        if (_isExcludedFromFee.contains(from) || _isExcludedFromFee.contains(to) || (uniswapV2Pair != from && uniswapV2Pair != to)) {
            takeFee = false;
        }
    ...
    function jackpotBuyerShareAmount() public view returns (uint256, uint256) {
        uint256 bnb = _pendingJackpotBalance.mul(jackpotCashout).div(MAX_PCT).mul(jackpotBuyerShare).div(MAX_PCT);
        uint256 tokens = _jackpotTokens.mul(jackpotCashout).div(MAX_PCT).mul(jackpotBuyerShare).div(MAX_PCT);
        return (bnb, tokens);
    }

    function jackpotBuybackAmount() public view returns (uint256, uint256) {
        uint256 bnb = _pendingJackpotBalance.mul(jackpotCashout).div(MAX_PCT).mul(MAX_PCT.sub(jackpotBuyerShare)).div(MAX_PCT);
        uint256 tokens = _jackpotTokens.mul(jackpotCashout).div(MAX_PCT).mul(MAX_PCT.sub(jackpotBuyerShare)).div(MAX_PCT);
        return (bnb, tokens);
    }
    ...
    function getLastAwarded() public view returns (address, uint256, uint256, uint256) {
        return (
            _lastAwarded,
            _lastAwardedCash,
            _lastAwardedTokens,
    ...
    function getLastBigBang() public view returns (uint256, uint256, uint256) {
        return (_lastBigBangCash, _lastBigBangTokens, _lastBigBangTimestamp);
    }

    function getPendingBalances() public view onlyAuthorized returns (uint256) {
        return (_pendingJackpotBalance);
    }

    function getPendingTokens() public view onlyAuthorized returns (uint256, uint256) {
        return (_liquidityTokens, _jackpotTokens);
    }

    function processBigBang() private lockTheSwap {
        uint256 cashedOut = _pendingJackpotBalance.mul(jackpotHardBuyback).div(MAX_PCT);
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        (uint256 liqTokens, uint256 jackpotTokens) = getTokenShares(tokenAmount);
        uint256 toBeSwapped = liqTokens.add(jackpotTokens);
        ...
        // Same for Jackpot
        uint256 balanceToJackpot = gainedBnb.mul(jackpotTokens).div(tokensForBnbExchange);
        _pendingJackpotBalance += balanceToJackpot;

        uint256 remainingBnb = gainedBnb.sub(balanceToJackpot);
    ...
    function transferBasic(address sender,address recipient,uint256 amount) private {
    ...
    function transferStandard(address sender,address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tJackpot) = processAmount(tAmount);
        uint256 tFees = tLiquidity.add(tJackpot);
        if (recipient != uniswapV2Pair && recipient != DEAD) {
            require(
                isExcludedFromFee(recipient) || balanceOf(recipient).add(tTransferAmount) <= maxWalletSize,
    ...
    function processAmount(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tLiquidity = tAmount.mul(_liquidityFee).div(MAX_PCT);
        uint256 tJackpot = tAmount.mul(_jackpotFee).div(MAX_PCT);
        uint256 tTransferAmount = tAmount.sub(tLiquidity.add(tJackpot));
        return (tTransferAmount, tLiquidity, tJackpot);
    }
