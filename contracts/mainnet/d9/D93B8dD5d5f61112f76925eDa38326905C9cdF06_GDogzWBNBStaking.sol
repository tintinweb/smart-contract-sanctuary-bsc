...
contract GDogzWBNBStaking is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        uint256 length = stakedNFT[msg.sender].length;

        for (uint256 j = 0; j < length; j++) {
            if (stakedNFT[msg.sender][j] == tokenId) {
                stakedNFT[msg.sender][j] = stakedNFT[msg.sender][length - 1];
    ...
    //Get token Ids of all tokens staked by _owner
    function getStakedNFTs(address _owner)
        external
        view
        returns (uint256[] memory)
    {
        return stakedNFT[_owner];
    }
}