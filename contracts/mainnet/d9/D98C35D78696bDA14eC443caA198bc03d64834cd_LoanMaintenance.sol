...
contract LoanMaintenance is State, LoanMaintenanceEvents, VaultController, InterestUser, SwapsUser, LiquidationHelper {
    ...
    function depositCollateral(
        bytes32 loanId,
        uint256 depositAmount) // must match msg.value if BNB is sent
        external
        payable
        nonReentrant
    {
        ...
        require(msg.value == 0 || collateralToken == address(wbnbToken), "wrong asset sent");
        ...
        if (msg.value == 0) {
            ...
            require(msg.value == depositAmount, "BNB deposit mismatch");
            vaultBnbDeposit(
                msg.sender,
                msg.value
            );
        ...
        returns (uint256 actualWithdrawAmount)
    {
        ...
        if (collateralToken == address(wbnbToken)) {
            vaultBnbWithdraw(
                receiver,
        ...
        returns (uint256 secondsExtended)
    {
        ...
        require(msg.value == 0 || (!useCollateral && loanParamsLocal.loanToken == address(wbnbToken)), "wrong asset sent");
        ...
        returns (uint256 secondsReduced)
    {
        ...
        // withdraw interest
        if (loanParamsLocal.loanToken == address(wbnbToken)) {
            vaultBnbWithdraw(
                receiver,
                withdrawAmount
            );
        ...
        view
    {
        ...
        if (_maxSwapSize != 0) {
            ...
            if (tokenAddress == address(wbnbToken)) {
                ...
                amountInBnb = IPriceFeeds(priceFeeds).amountInBnb(tokenAddress, amount);
            }
            require(amountInBnb <= _maxSwapSize, "swap too large");
        }
    }
}
        ...
        internal
    {
        ...
        (bool success, bytes memory data) = _priceFeeds.staticcall(
            abi.encodeWithSelector(
                ...
                feeToken,
                nbuTokenAddress, // price rewards using NBU price 
...
contract State is Constants, Objects, ReentrancyGuard, Ownable {
    ...
    uint256 public maxSwapSize = 1500 ether;                                                // maximum supported swap size in BNB
