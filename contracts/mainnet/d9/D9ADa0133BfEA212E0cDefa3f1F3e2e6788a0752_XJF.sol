...
contract XJF is ERC20, Ownable, SafeToken, LockToken {
    ...
    BNBcoinDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 99999999999999999999;
    ...
    constructor() ERC20("XJF", "XJF") {
        BNBRewardsFee = 1;
        liquidityFee = 1;
        burnFee = 0;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(0x53E1796756320a85bAc339128Bf12bcf4840b761);
 
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee); // total fee transfer and buy

    	dividendTracker = new BNBcoinDividendTracker();
        ...
        */
        _mint(owner(), 99999999999999999999);
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BNB_COIN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BNB_COIN: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BNB_COIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BNB_COIN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BNB_COIN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BNB_COIN: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            uint256 burnFees =  (fees*burnFee)/totalFees;
            require(fees >= burnFees ,"inf burn amount");  
            super._transfer(from, 0x000000000000000000000000000000000000dEaD, burnFees); // get burn fee 
            uint256 feesWithoutBurn = fees - burnFees;
            super._transfer(from, address(this), feesWithoutBurn); // get other fee 
            if(extraFee>0)
            {
...
contract BNBcoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BNB_COIN_Dividend_Tracker", "BNB_COIN_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000000000; //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BNB_COIN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BNB_COIN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BNB_COIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BNB_COIN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "BNB_COIN: Cannot update claimWait to same value");
