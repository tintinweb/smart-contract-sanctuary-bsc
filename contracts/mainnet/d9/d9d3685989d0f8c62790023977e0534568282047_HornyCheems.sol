...
abstract contract Auth is Context{
    ...
    constructor() {
        owner = _msgSender();
        emit OwnershipTransferred(_msgSender());
    }
...
contract HornyCheems is IBEP20, Auth {
    ...
    mapping (address => uint256) _tBalance;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 10**12 * 10**_decimals; //1 Trillion
    uint256 public swapThreshold = rSupply / 1000; //0.1%
    ...
    uint256 private _maxBuyAmount = rSupply.div(100).mul(100);
    uint256 private _maxSellAmount = rSupply.div(100).mul(2);
    uint256 private _maxWalletToken = rSupply.div(100).mul(100);

    constructor () {  // PancakeSwap: 0x10ED43C718714eb63d5aA57B78B54704E256024E // Uniswap V2: 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
        ...
        marketingFeeReceiver = 0x7D4389BEE86570372D383d9792a480Cb13Fda301;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (gonDone) { return _transferGon(sender, recipient, amount); }
        ...
        //Max Wallet
        if (!isFeeExempt[sender] && recipient != address(this) && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver) { 
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + rAmount) <= _maxWalletToken, "Total Holding is currently limited, you can not buy that much.");
        }

        //Buy Cooldown        
        if (sender == pair && buyCooldownEnabled && !isFeeExempt[recipient] && recipient != address(this)) {
        ...
        require(rAmount <= (sender == pair ? _maxBuyAmount : _maxSellAmount) || isFeeExempt[sender] || isFeeExempt[recipient], "TX Limit Exceeded");
        
        //swap and liquify
        if(msg.sender != pair && !inSwap && swapEnabled && _rBalance[address(this)] >= swapThreshold) {
            swapBack();
        }
    ...
    function _transferGon(address sender, address recipient, uint256 amount) internal returns (bool) {
          _tBalance[sender] = _tBalance[sender].sub(amount);
    ...
    function setMaxBuyPercent(uint256 maxWallPercent_base1000) external onlyOwner() {
        _maxBuyAmount = rSupply.div(1000).mul(maxWallPercent_base1000);
    }
    
    function setMaxSellPercent(uint256 maxWallPercent_base1000) external onlyOwner() {
        _maxSellAmount = rSupply.div(1000).mul(maxWallPercent_base1000);
    }
