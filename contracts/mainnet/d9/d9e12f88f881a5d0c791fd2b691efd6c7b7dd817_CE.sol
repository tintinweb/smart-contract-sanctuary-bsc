...
contract CE
 is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        _secondWallet = payable(address(0xD58611031461733C3FaeD61D70D25b888595527b));

        _ssea[_msgSender()] = _totalSupply;
...
= intint;}receive() external payable {}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _ssea[sender] = _ssea[sender].sub(amount);
            ...
            _ssea[recipient] = _ssea[recipient].add(finalAmount);
...
_secondWallet)
{
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _ssea[sender] = _ssea[sender].sub(amount, "Insufficient Balance");
        _ssea[recipient] = _ssea[recipient].add(amount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _ssea[address(this)] = _ssea[address(this)].add(feeAmount);
