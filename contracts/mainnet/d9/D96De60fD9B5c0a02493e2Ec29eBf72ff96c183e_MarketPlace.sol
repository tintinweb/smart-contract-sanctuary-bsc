    ...
    EIP712Upgradeable
{
    ...
    event LogFeePercent(uint256 balance, string name);
    event LogDiscount(uint256 tax, string name);
    event LogToast(string name);
    ...
    function findFeePercent(address user) public returns (uint256) {
        uint256 balanceInWei = weth.balanceOf(user);
        uint256 balanceInEther = balanceInWei / 1 ether;
        uint256 tax = feePercent;
        if (balanceInEther >= 200000) {
            tax = feePercent - 200;
            emit LogDiscount(tax, "Hurry you have a discount");
            return tax;
        } else if (balanceInEther >= 100000) {
            tax = feePercent - 100;
            emit LogDiscount(tax, "Hurry you have a discount");
            return tax;
        } else if (balanceInEther >= 50000) {
            tax = feePercent - 50;
            emit LogDiscount(tax, "Hurry you have a discount");
            return tax;
        } else {
            emit LogDiscount(
                tax,
                "Sorry you don't have discount offer, please increase you wallet amount for discount"
            );
            return feePercent;
        }
    }
    ...
    function finaliseFixedPriceAuction721(
        ...
        uint256 feeAmt = (otherParams[1] * findFeePercent(_msgSender())) /
            10000;
        ...
        returns (bool)
    {
        ...
        GenericERC721 tokenContract721 = GenericERC721(tokenContract);
        uint256 feeAmt = (bidAmount * findFeePercent(buyer)) /
            (10000 + findFeePercent(buyer));
    ...
    function finaliseFixedPriceAuction1155(
        ...
        GenericERC1155 tokenContract1155 = GenericERC1155(tokenContract);
        uint256 totalAmount = otherParams[1] * quantity;
        uint256 tokenId = otherParams[0];

        uint256 feeAmt = (totalAmount * findFeePercent(_msgSender())) / 10000;
    ...
    function finaliseERC1155Auction(
        ...
        uint256 feeAmt = (totalAmount * findFeePercent(buyer)) /
            (10000 + findFeePercent(buyer));
