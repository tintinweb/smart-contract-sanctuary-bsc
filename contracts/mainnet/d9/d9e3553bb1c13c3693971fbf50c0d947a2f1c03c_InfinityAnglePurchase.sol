...
// File: InfinityAnglePurchase.sol
...
contract InfinityAnglePurchase is
    Initializable,
    PausableUpgradeable,
    ReentrancyGuard,
    OwnableUpgradeable
{
     ...
     */
    function initialize(address payable _foundation, address _verifier)
        public
        initializer
    {
        __Pausable_init();
     ...
     */
    function purchase(bytes calldata orderData)
        external
        payable
        whenNotPaused
        nonReentrant
    {
        ...
        (ItemStruct memory item, bytes memory verifierSignature) = abi.decode(
            orderData,
            (ItemStruct, bytes)
        );
        ...
        require(
            validateVerifierSignature(item, verifierSignature),
            "Verifier side signature is invalid"
        );

        // Check order type use native coin
        if (item.paymentContract == address(0)) {
            require(msg.value == item.price, "Value is not equal price");
            sendValue(foundationAddress, item.price);
     ...
     */
    function setFoundationAddress(address payable newFoundationAddress)
        external
        onlyOwner
    {
    ...
    function hashEIP191(ItemStruct memory item)
        internal
        view
        returns (bytes32)
    {
    ...
    function validateVerifierSignature(
        ItemStruct memory item,
        bytes memory verifierSignature
    ) internal view returns (bool) {
        bytes32 ECDSAHash = ECDSA.toEthSignedMessageHash(hashEIP191(item));
