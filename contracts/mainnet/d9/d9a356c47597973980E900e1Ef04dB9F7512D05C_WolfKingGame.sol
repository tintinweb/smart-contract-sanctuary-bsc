...
import "./IKing.sol";
import "./IPlanetary.sol";
import "./ITraits.sol";
import "./Minerals.sol";


contract WolfKingGame is IKing, ERC721Enumerable, Ownable, Pausable {

  // mint price
  uint256 public constant MINT_PRICE = .30 ether;
  // max number of tokens that can be minted - 100000 in production
  uint256 public immutable MAX_TOKENS;
  // number of tokens that can be claimed for free - 30% of MAX_TOKENS
  uint256 public PAID_TOKENS;
  // number of tokens have been minted so far
  uint16 public minted;

  // mapping from tokenId to a struct containing the token's traits
  mapping(uint256 => WolfKing) public tokenTraits;
  // mapping from hashed(tokenTrait) to the tokenId it's associated with
  // used to ensure there are no duplicates
  mapping(uint256 => uint256) public existingCombinations;

  // list of probabilities for each trait type
  // 0 - 9 are associated with Wolf, 10 - 18 are associated with Wolf King
  uint8[][18] public rarities;
  // list of aliases for Walker's Alias algorithm
  // 0 - 9 are associated with Wolf, 10 - 18 are associated with Wolf King
  uint8[][18] public aliases;

  // reference to the Planetary for choosing random Wolf King thieves
  IPlanetary public planetary;
  // reference to $MAS for burning on mint
  Minerals public minerals;
  // reference to Traits
   ...
   */
  constructor(address _minerals, address _traits, uint256 _maxTokens) ERC721("Wolf King Game", 'WKG') {
    minerals = Minerals(_minerals);
    traits = ITraits(_traits);
    MAX_TOKENS = _maxTokens;
    PAID_TOKENS = _maxTokens * 3 / 10;
  ...
  /** 
   * mint a token - 90% Wolf, 10% King
   * The first 20% are free to claim, the remaining cost $MAS
   */
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!stake || recipient != _msgSender()) {
        ...
        _safeMint(address(planetary), minted);
        tokenIds[i] = minted;
      }
      totalMineralsCost += mintCost(minted);
    }
    
    if (totalMineralsCost > 0) minerals.burn(_msgSender(), totalMineralsCost);
    if (stake) planetary.addManyToPlanetaryAndPack(_msgSender(), tokenIds);
  }

  /** 
   * the first 30% are paid in BNB
   * the next 20% are 20000 $MAS
   * the next 40% are 40000 $MAS
   * the final 10% are 80000 $MAS
  ...
  function transferFrom(
    ...
    // Hardcode the Planetary's approval so that users don't have to waste gas approving
    if (_msgSender() != address(planetary))
   ...
   */
  function generate(uint256 tokenId, uint256 seed) internal returns (WolfKing memory t) {
    t = selectTraits(seed);
  ...
  /**
   * the first 30% (BNB purchases) go to the minter
   * the remaining 80% have a 10% chance to be given to a random staked king
   * @param seed a random value to select a recipient from
   * @return the address of the recipient (either the minter or the King thief's owner)
   */
  function selectRecipient(uint256 seed) internal view returns (address) {
    if (minted <= PAID_TOKENS || ((seed >> 245) % 10) != 0) return _msgSender(); // top 10 bits haven't been used
    address thief = planetary.randomKingOwner(seed >> 144); // 144 bits reserved for trait selection
   ...
   */
  function selectTraits(uint256 seed) internal view returns (WolfKing memory t) {
    t.isWolf = (seed & 0xFFFF) % 10 != 0;
    uint8 shift = t.isWolf ? 0 : 9;
    seed >>= 16;
    ...
    seed >>= 16;
    t.clothes = selectTrait(uint16(seed & 0xFFFF), 7 + shift);
    seed >>= 16;
   ...
   */
  function structToHash(WolfKing memory s) internal pure returns (uint256) {
  ...
  function getTokenTraits(uint256 tokenId) external view override returns (WolfKing memory) {
  ...
  /**
   * called after deployment so that the contract can get random king thieves
   * @param _planetary the address of the Planetary
   */
  function setPlanetary(address _planetary) external onlyOwner {
    planetary = IPlanetary(_planetary);
  }
