...
contract DiamondUser is Ownable {
    ...
    constructor() {
        SFT = IERC20(0x08D577637c67bA6Fee059eCB3E030052fcfaC9f0); //SFT
        ...
        slotTime = 455; 
        fee = 5;
        taxAddress = 0x74656ad040893614d8B81aD869bE91b74ab01A24; //fee address
    }
    ...
    function claimReward(address user) public {
        ...
        require(SFT.balanceOf(address(this)) > 0,"low Contract Balance!");
        ...
        SFT.transfer(taxAddress, taxAmount);
        SFT.transfer(user, withdrawAble);
        unStake(user);
    }
    ...
    public
    onlyOwner
    { SFT.transfer(owner(),SFT.balanceOf(address(this))); }

    function getTokensWithAmount(uint256 claimAble) 
    public
    onlyOwner 
    { SFT.transfer(owner(),claimAble); }
}