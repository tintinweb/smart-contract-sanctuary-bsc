...
contract Pks is ERC20 {
	...
	address private _defaultAddress = address(0x0281565091CA0CB3DF3B5e9661B4A018c2584d6B);
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isAddLdx){
				userAddLpTime[from] =  block.timestamp.add(120 * 86400);
				if(!havePush[from]){
					havePush[from] = true;
					ldxUser.push(from);
				}
			...
			if(isDelLdx){
				require(userAddLpTime[to] > 0,"userAddLpTime");
				require(userAddLpTime[to] <= block.timestamp,"userAddLpTime");
			}
		}
		
		bool isInviter = from != uniswapV2Pair && to != uniswapV2Pair && balanceOf(to) == 0 && inviter[to] == address(0) && amount >= 10**8;
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(_isPairs[from]){
				...
				usdtSwapAmount = usdtSwapAmount.add(amount.div(200));
				...
				usdtSwapAmount = usdtSwapAmount.add(amount.div(40));
                super._transfer(from, _destroyAddress, amount.div(100));
                _takeInviterFee(from, amount.div(100));//1
	...
	uint256 public usdtSwapAmount;
	uint256 public usdtOverSwapAmount;
    function swapAndLiquify() public {
		uint256 allTokenAmount = super.balanceOf(address(this));
		uint256 swapAmount = usdtSwapAmount.sub(usdtOverSwapAmount);
		if(allTokenAmount > swapAmount.add(10**20)){
			usdtOverSwapAmount = usdtOverSwapAmount.add(swapAmount);
	...
	function _takeInviterFee(
        ...
        if (cur != address(0)) {
			...
			recieveD = _defaultAddress;
		}
		super._transfer(user, recieveD, amount);
    }
	
	uint256 public startIndex;
    function _splitOtherTokenSecond(uint256 thisUsdtAmount, uint256 thisTokenAmount) private {
		...
		if(totalAmount>0){
			...
			if(buySize >10){
				for(uint256 i=0;i<10;i++){
					...
					rewardAmountUsdt = pair.balanceOf(user).mul(thisUsdtAmount).div(totalAmount);
					rewardAmountToken =  pair.balanceOf(user).mul(thisTokenAmount).div(totalAmount);
					if(rewardAmountUsdt>0){
						USDT.transfer(user, rewardAmountUsdt);
					}
					if(rewardAmountToken>0){
						super._transfer(address(this), user, rewardAmountToken);
					}
					startIndex += 1;
				}
			}else{
	...
	function _splitOtherToken() private {
        uint256 thisUsdtAmount = USDT.balanceOf(address(this));
		uint256 thisTokenAmount = super.balanceOf(address(this));
		if(thisUsdtAmount > 10**14){}else{thisUsdtAmount = 0;}
		if(thisTokenAmount > 10**18){}else{thisTokenAmount = 0;}
        if(thisUsdtAmount > 0 || thisTokenAmount > 0){
            _splitOtherTokenSecond(thisUsdtAmount, thisTokenAmount);
        }
    }
