...
library LibProtocolX {
    ...
    uint256 internal constant LIQUIDITY_FEE = 30;
    uint256 internal constant LIQUIDITY_FEE_SELL = 50;
    uint256 internal constant TREASURY_FEE = 50;
    uint256 internal constant TREASURY_FEE_SELL = 80;
    uint256 internal constant XSHARE_FUND_FEE = 20;
    uint256 internal constant AFTERBURNER_FEE = 30;
        ...
        returns (bool)
    {
        ...
        return (s.pair == from || s.pair == to) && !isExemptFromFees(from);
    }
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        return s.swapEnabled && !s.inSwap && LibContext.msgSender() != s.pair;
    }
    ...
    function allowance(address owner_, address spender)
        ...
        returns (uint256)
    {
        return _storage().allowedFragments[owner_][spender];
    }
    ...
    function setSwapEnabled(bool flag) internal {
        ...
        require(s.swapEnabled != flag, "Not changed");
    ...
    function setAutoAddLiquidity(bool flag) internal {
        ...
        require(s.autoAddLiquidity != flag, "Not changed");
        if (flag) {
            // solhint-disable-next-line not-rely-on-time
            s.lastAddLiquidityTime = block.timestamp;
        }
    ...
    function setRebaseRate(uint256 rebaseRate) internal {
        ...
        require(s.rebaseRate != rebaseRate, "not changed");
        require(
        ...
        internal
    {
        for (uint256 i = 0; i < accounts.length; ++i) {
            setExemptFromFees(accounts[i], flag);
    ...
    function enforceNotPaused() internal view {
        Storage storage s = _storage();
        address msgSender = LibContext.msgSender();

        if (
            LibPausable.paused() &&
            msgSender != s.autoLiquidityReceiver &&
            msgSender != s.treasuryReceiver &&
            msgSender != s.xshareFundReceiver &&
            msgSender != s.afterburner
        ) {
...
library LibPausable {
     ...
     */
    function enforceNotPaused() internal view {
        require(!paused() || LibUtils.isOwner(), "Pausable: paused");
    }
    ...
    function enforcePaused() internal view {
        require(paused() || LibUtils.isOwner(), "Pausable: not paused");
    }
