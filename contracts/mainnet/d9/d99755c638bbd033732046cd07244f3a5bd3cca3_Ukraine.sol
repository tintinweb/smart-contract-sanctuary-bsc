...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant BTCB = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD
...
contract Ukraine is ERC20, Ownable {
    ...
    uint256 public totalFees = charityFee.add(buybackFee).add(BUSDRewardsFee).add(liquidityFee).add(teamFee);

    address public charityAddress = 0x5Bc9a8e2D4Da09Ee4058Ea3d55fd86E372edE241;
    address public teamAddress = 0xe7b763452185813bEf65c8748Daf8F443Ca6C4Ff;
    ...
    event WalletsUpdated(address indexed operator, address indexed newMarketingWallet, address indexed newTeamWallet);
    ...
    event CharityFeeUpdated(address indexed operator, uint256 oldcharityfee, uint256 newBUSDRewardsfee);
    event UkraineTokensWithdrawn(address indexed operator, address indexed recipient, uint256 amount);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "UKRAINE:: antiWhale: Transfer amount exceeds the maxTransferAmount");
                //sale
                if ( automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient]) {
                    require(amount <= maxSaleAmount(), "UKRAINE:: antiWhale: Sale amount exceeds the maxSaleAmount");
                }
            }
        }
        _;
    ...
    constructor() public ERC20("Rescue Ukraine", "UKRAINE") {
        ...
        _excludedFromAntiWhale[charityAddress] = true;
        _excludedFromAntiWhale[teamAddress] = true;
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "UKRAINE:: excludeFromFees: Account is already the value of 'excluded'");
    ...
    function updateMaxTransferAmountRate(uint256 _maxTransferAmountRate) external onlyOwner {
        require(_maxTransferAmountRate <= 1000000, "UKRAINE:: updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
        require(_maxTransferAmountRate >= 10000, "UKRAINE:: updateMaxSaleAmountRate: Max transfer amount rate must not exceed the minimum rate.");
        emit MaxTransferAmountRateUpdated(msg.sender, maxTransferAmountRate, _maxTransferAmountRate);
        maxTransferAmountRate = _maxTransferAmountRate;
    }

    function updateMaxSaleAmountRate(uint16 _maxSaleAmountRate) external onlyOwner {
        require(_maxSaleAmountRate <= 1000000, "UKRAINE:: updateMaxSaleAmountRate: Max sale amount rate must not exceed the maximum rate.");
        require(_maxSaleAmountRate >= 1000, "UKRAINE:: updateMaxSaleAmountRate: Max sale amount rate must not exceed the minimum rate.");
    ...
    function updateSwapStartBlock(uint256 _swapStartblock) external onlyOwner {
        require(block.number < swapStartblock, "UKRAINE:: updateSwapStartBlock: Trading is started already.");
        emit SwapStartBlockUpdated(msg.sender, swapStartblock, _swapStartblock);
        swapStartblock = _swapStartblock;
    }

    function seWallets(address payable wallet1, address payable wallet2) external onlyOwner{
        require(wallet1 != address(0), "UKRAINE:: setMarketingWallet: Please use vaild payable address.");
        require(wallet2 != address(0), "UKRAINE:: setMarketingWallet: Please use vaild payable address.");
        excludeFromFees(charityAddress, false);
        excludeFromFees(teamAddress, false);
        _excludedFromAntiWhale[charityAddress] = false;
        _excludedFromAntiWhale[teamAddress] = false;
        excludeFromFees(wallet1, true);
        excludeFromFees(wallet2, true);
        _excludedFromAntiWhale[wallet1] = true;
        _excludedFromAntiWhale[wallet2] = true;
        emit WalletsUpdated(msg.sender, wallet1, wallet2);
        charityAddress = wallet1;
        teamAddress = wallet2;
    }

    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        totalFees = value.add(liquidityFee).add(charityFee);
        require(totalFees <= 20, "UKRAINE:: setBUSDRewardsFee: Too high Fees");
        emit BUSDRewardsFeeUpdated(msg.sender, BUSDRewardsFee, value);
        BUSDRewardsFee = value;
    }

    function setLiquidityFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(charityFee);
        require(totalFees <= 20, "UKRAINE:: setBUSDRewardsFee: Too high Fees");
    ...
    function setCharityFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(liquidityFee);
        require(totalFees <= 20, "UKRAINE:: setBUSDRewardsFee: Too high Fees");
        emit CharityFeeUpdated(msg.sender, charityFee, value);
        charityFee = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "UKRAINE:: setAutomatedMarketMakerPair: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "UKRAINE:: _setAutomatedMarketMakerPair: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "UKRAINE:: updateGasForProcessing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "UKRAINE:: updateGasForProcessing: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( swapAndLiquifyDividendEnabled &&
            ...
            swapAndSendToFee(charityTokens, charityAddress);

            uint256 teamTokens = contractTokenBalance.mul(teamFee).div(totalFees);
            swapAndSendToFee(teamTokens, teamAddress);
            ...
            swapTokensForBuyback(buybackTokens);
    ...
    function swapAndSendToFee(uint256 tokens, address to) private  {
        ...
        uint256 newBalance = (address(this).balance).sub(initialBUSDBalance);
        IERC20(BUSD).transfer(address(to), newBalance);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        try IERC20(BUSD).transfer(address(dividendTracker), dividends){
            ...
            revert("UKRAINE:: BUSD transfer reverted because of insufficient BUSD balance");
        }
    }

    function withdrawUkraine(address toAddress, uint256 amount) external onlyOwner {
        uint256 maxblance = balanceOf(address(this));
        if(maxblance <= amount) {
            amount = maxblance;
        }
        super._transfer(address(this), toAddress, amount);
        emit UkraineTokensWithdrawn(msg.sender, toAddress, amount);
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "UKRAINE:: delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "UKRAINE:: delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "UKRAINE:: delegateBySig: signature expired");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying UKRAINES (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "UKRAINE:: _writeCheckpoint: block number exceeds 32 bits");
...
contract UKRAINEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("UKRAINE_Dividend_Tracker", "UKRAINE_DTker") {
    	claimWait = 21600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        revert("UKRAINE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        revert("UKRAINE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIVA contract.");
    }
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        require(newMinimumTokenBalanceForDividends >= 100 * (10**18) && newMinimumTokenBalanceForDividends <= 10000000 * (10**18), "UKRAINE_Dividend_Tracker: MinimumTokenBalanceForDividends must be updated to between 100 and 10000000");
        require(newMinimumTokenBalanceForDividends != minimumTokenBalanceForDividends, "UKRAINE_Dividend_Tracker: Cannot update MinimumTokenBalanceForDividends to same value");
