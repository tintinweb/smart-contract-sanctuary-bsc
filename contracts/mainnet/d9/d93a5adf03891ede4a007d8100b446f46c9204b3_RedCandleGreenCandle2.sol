...
/**
RedCandleGreenCandle2 SmartContract 
*/
...
contract RedCandleGreenCandle2 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RedCandleGreenCandle2DividendTracker public dividendTracker;
    ...
    bool public sellPhase;
    uint256 private gasPriceLimit; 
    mapping(address => uint256) private _holderLastTransferBlock; 
    mapping(address => uint256) private _holderLastTransferTimestamp; 
    uint256 public buyPhaseBlock;
    uint256 public sellPhaseBlock;
    uint256 public buyPhaseTimestamp;
    uint256 public sellPhaseTimestamp;
    uint256 public autoChangePhaseTimestamp;
    ...
    constructor() ERC20("RedCandleGreenCandle2", "RCGC2") {
        ...
        dividendTracker = new RedCandleGreenCandle2DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "CandleTRACKER", "RCGC2TRACKER");
    ...
    function enableBuyPhase(uint256 MG, uint256 MW, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        ...
        MTX = MTX * (10**18);
        require(!buyPhase);
        require(block.timestamp >= sellPhaseTimestamp + 1 hours);
        require(block.timestamp >= autoChangePhaseTimestamp + 1 days);
    ...
    function enableSellPhase(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        MTX = MTX * (10**18);
        require(!sellPhase);
        require(block.timestamp >= buyPhaseTimestamp + 1 hours);
        require(block.timestamp >= autoChangePhaseTimestamp + 1 days);
        require(MTX >= _totalSupply / 1000,"cannot set below 0.1%");
        require(SC <= 300,"cannot set above 300");
        require(MG >= 5,"cannot set below 5");
    ...
    // exclude or include a wallet from fees 
    ...
    // recover Native balance of address 
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to == DEAD) {
            ...
            if (!isBuying && !isSelling) {
                ...
                require(sellPhase,"sell phase is not active");
                
                if (block.timestamp >= sellPhaseTimestamp + 1 weeks) {
                    buyPhase = true;
                    sellPhase = false;
                    buyPhaseTimestamp = block.timestamp;
                    buyPhaseBlock = block.number;
                ...
                require(buyPhase,"buy phase is not active");

                if (block.timestamp >= buyPhaseTimestamp + 1 weeks) {
                    buyPhase = false;
                    sellPhase = true;
                    sellPhaseTimestamp = block.timestamp;
                    sellPhaseBlock = block.number;
...
contract RedCandleGreenCandle2DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    RedCandleGreenCandle2 public RedCandleGreenCandle2Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        RedCandleGreenCandle2Contract = RedCandleGreenCandle2(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main RedCandleGreenCandle2 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(RedCandleGreenCandle2Contract);

            uint256 prevBalance = RedCandleGreenCandle2Contract.balanceOf(address(this));
            ...
            {
                uint256 received = RedCandleGreenCandle2Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    RedCandleGreenCandle2Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
