...
/*
ApyMoon is Decentralized Finance with Auto-Compounding & Auto-Staking Protocol Providing Investors with Sustainable 
Fixed Annual Yield of 365%. ApyMoon Protocol is powered by Positive Rebase Algorithm, which allowed token distribution 
...
contract ApyMoon is ERC20Detailed, Ownable, WhitelistedRole {
	...
	address public devFeeReceiver = 0x77fCfB635a43d1FbcEd5961754d4102C7e714b02;
	...
	uint256 public insuranceFundFee = 3;
	uint256 public devFee = 2;
	uint256 public burnFee = 0;
	uint256 public sellFeeLiquidityAdded = 1;
	uint256 public sellFeeInsuranceAdded = 0;
	uint256 public sellFeeTreasuryAdded = 0;
	uint256 public sellBurnFeeAdded = 0;
	uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(insuranceFundFee).add(burnFee).add(devFee);
	uint256 public totalSellFee = totalBuyFee.add(sellFeeLiquidityAdded).add(sellFeeInsuranceAdded).add(sellFeeTreasuryAdded).add(sellBurnFeeAdded);
	...
	constructor(
		address[3] memory addrs // liquidityReceiver, treasuryReceiver, insuranceFundReceiver
	) ERC20Detailed("APYMOON", "APYMOON", uint8(DECIMALS)) {
		...
		liquidityReceiver = addrs[0];
		treasuryReceiver = addrs[1];
		insuranceFundReceiver = addrs[2];
	...
	function swapBack() internal swapping {
		...
		uint256 amountToLiquify = contractTokenBalance.mul(dynamicLiquidityFee.mul(2).add(sellFeeLiquidityAdded)).div(realTotalFee);
		uint256 amountToRFV = contractTokenBalance.mul(insuranceFundFee.mul(2).add(sellFeeInsuranceAdded)).div(realTotalFee);
		uint256 amountToDev = contractTokenBalance.mul(devFee.mul(2)).div(realTotalFee);
		...
		if (amountToDev > 0) {
			_swapTokensForBusd(amountToDev, devFeeReceiver);
		}
		emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToTreasury, amountToDev);
	}
	...
	function takeFee(
		...
		uint256 _realFee = totalBuyFee.sub(burnFee);
		uint256 burnFeeAmount;
		if (automatedMarketMakerPairs[recipient]) {
			_realFee = totalSellFee.sub(burnFee.add(sellBurnFeeAdded));
			burnFeeAmount = gonAmount.mul(burnFee.add(sellBurnFeeAdded)).div(feeDenominator);
		} else {
			burnFeeAmount = gonAmount.mul(burnFee).div(feeDenominator);
		}
		...
		_gonBalances[burnAddress] = _gonBalances[burnAddress].add(burnFeeAmount);
		emit Transfer(sender, burnAddress, burnFeeAmount.div(_gonsPerFragment));
	...
	function setSellLimitPercent(uint256 _sellLimit) external onlyOwner {
		require(_sellLimit >= 5 && _sellLimit <= 20, "Sell limit must be between 5 and 20%");
		sellLimit = _sellLimit;
	}

	function setTransferTax(uint256 _transferTAX) external onlyOwner {
		require(_transferTAX == 1 || _transferTAX == 100, "Transfer tax cannot be different than 100 or 1");
	...
	function setFees(
		...
		uint256 newTotalBuyFees = _fees[0].add(_fees[1]).add(_fees[2]).add(_fees[3]).add(_fees[4]);
		require(newTotalBuyFees <= 12 && newTotalBuyFees >= 0, "Total buy fees cannot be greater than 12%");
		liquidityFee = _fees[0];
		treasuryFee = _fees[1];
		insuranceFundFee = _fees[2];
		burnFee = _fees[3];
		devFee = _fees[4];
		totalBuyFee = liquidityFee.add(treasuryFee).add(insuranceFundFee).add(burnFee).add(devFee);
		totalSellFee = totalBuyFee.add(sellFeeLiquidityAdded).add(sellFeeInsuranceAdded).add(sellFeeTreasuryAdded).add(sellBurnFeeAdded);
	}

	function setSellAddFees(
		uint256[4] memory _sellFees // liquidity, treausry, insurance, burn
	) external onlyOwner {
		uint256 newTotalSellFees = totalBuyFee.add(_sellFees[0]).add(_sellFees[1]).add(_sellFees[2]).add(_sellFees[3]);
		require(newTotalSellFees <= 13 && newTotalSellFees >= 0, "Total sell fees cannot be greater than 13%");
	...
	event SwapBack(uint256 contractTokenBalance, uint256 amountToLiquify, uint256 amountToRFV, uint256 amountToTreasury, uint256 amountToDev);
