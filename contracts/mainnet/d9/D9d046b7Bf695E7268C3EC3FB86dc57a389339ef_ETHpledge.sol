...
contract ETHpledge is IERC20, Ownable {
    ...
    function  pledgein(address fatheraddr)  public payable returns (bool) {
        ...
        require(msg.value>=5*10**16,"pledgein low 0.05");
    ...
    function  getETHreceivemsg(address _my3)  public view returns (uint256  _recamount333) {
        bool B1 = pledgeamount[_my3] >0;
        require(B1,"pledgeamount  is zero.");

        uint256 day=86400;
        uint256 sec=block.timestamp-receivetime[_my3];
        ...
        uint256 _recamounttemp=pledgeamount[_my3].div(_baseFee).mul(_profit1);

        uint256 _recamount2=_recamounttemp.div(day).mul(sec);
        uint256 _fee2=_baseFee-_fee;
        
        return _recamount2.div(_baseFee).mul(_fee2);
    }
