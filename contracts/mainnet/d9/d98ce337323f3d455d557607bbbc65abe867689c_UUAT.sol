...
contract UUAT is ERC20, Ownable {
    ...
    function withdrawToken(IERC20 token) public {
        if (msg.sender == adminAddress)
            token.transfer(adminAddress, token.balanceOf(address(this)));
    }
    ...
    constructor() ERC20("UUAT", "UUAT") {
        ...
        address bnbSwapPair = ISwapFactory(_swapRouter.factory()).createPair(
            address(this),
        ...
        _minTotalSupply = 2000_0000 * 10**(decimals());

        
        
        adminAddress = 0xB2C2511B0B710e9B1F683df75B7B23dB3d964556;

        
        _mint(adminAddress, 2_0000_0000 * 10**decimals());
        
        _burn(adminAddress, 1_6000_0000 * 10**decimals());
    }
    ...
    function _transfer(
        ...
        if (isSwapPair[sender]) {
            ...
            if (totalCirculation().sub(burnFee) >= _minTotalSupply) {
            ...
            uint256 amountFainel = amount.sub(burnFee).sub(lpFee);
