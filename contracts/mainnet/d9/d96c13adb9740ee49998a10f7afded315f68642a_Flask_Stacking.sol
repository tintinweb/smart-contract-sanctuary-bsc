...
contract Flask_Stacking is Base, Math, Adressable, AccessControl, With_DAORole, Terminatable,
                              With_UpdaterRole, With_PWRToken, With_FundContract, I_Flask_Stacking{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    constructor(){
        ...
        stackingBonusFactor.numerator = 5000;
    ...
    function setStackingRewardRatio(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Flask_Stacking: Denominator cannot be null");
    ...
    function stackPWR(uint256 nbPWR, uint256 nbCycle) external onlyNotTerminated{
        require(PWRToken.balanceOf(msg.sender) >= nbPWR, "Flask_Stacking: Not enough PWR.");
        require(nbCycle >= 1, "Flask_Stacking: Stacking duration is a minimum of 1 complete cycle.");
        require(nbCycle <= maxCycleLock, "Flask_Stacking: Stacking duration greater than the max.");

        StackingGroup memory group;
        group.nbPWR = nbPWR;
        group.nbSPWR = nbPWR + (nbPWR*nbCycle*stackingBonusFactor.numerator) / stackingBonusFactor.denominator;
        group.startCycle = currentCycle + 1;
    ...
    function getUnstackFees(uint256 groupId) public view returns (uint256){
        ...
        require(groupId < stackingGroup.length, "Flask_Stacking: Unknown stacking group.");

        StackingGroup storage group = stackingGroup[groupId];

        require(group.stacked, "Flask_Stacking: Stacking group already unstacked.");
    ...
    function unstackPWR(uint256 groupId) external{
        ...
        require(feesPWR < group.nbPWR, "Flask_Stacking: Early unstacking fees are greater than 100%");
        require(group.userAddr == msg.sender, "Flask_Stacking: Only owner of stacking group can unstack it.");
    ...
    function getFirstStackingGroup() external view returns(uint256){
        ...
        require(false, "Flask_Stacking: No stacking group to update");
        return 0;
    }
    ...
    function updateOneClaimRewardNeeded(uint256 groupId) external view returns (bool){
        ...
        return group.currentCycle <= _min(group.endCycle, currentCycle);
    }
    ...
    function updateOneClaimReward(uint256 groupId) external onlyRole(UPDATER_ROLE){
        require(groupId < stackingGroup.length, "Flask_Stacking: Unknown stacking group.");

        StackingGroup storage group = stackingGroup[groupId];
        
        uint256 nbPWR = _getOneCycleStackingReward(group);
        group.currentCycle = _min(group.currentCycle + 1, currentCycle + 1);
    ...
    function claimUserStackingReward() external{
        ...
        require(nbPWR>0, "Flask_Stacking: There is no PWR to claim.");
    ...
    function claimUserOneCycleStackingReward() external{
        uint256[] storage ids = userGroupId[msg.sender];
        uint256 l = ids.length;
        ...
        for(uint256 i=0; i<l; i++){
            StackingGroup storage group = stackingGroup[ids[i]];
            nbPWR += _getOneCycleStackingReward(group);
    ...
    function claimStackingReward(uint256 groupId) external{
        ...
        require(group.userAddr == msg.sender, "Flask_Stacking: Only owner of stacking group can claim reward.");
    ...
    function _getOneCycleStackingReward(StackingGroup storage group) internal view returns(uint256){
        if (!group.stacked){
            return 0;
        }
        ...
        if (group.currentCycle > lastCycle){
            return 0;
        }
        
        CycleData storage data = cycleData[group.currentCycle];
        uint256 nbPWR = (data.totalPWRMinted * group.nbSPWR) / (data.sPWRSupply);
