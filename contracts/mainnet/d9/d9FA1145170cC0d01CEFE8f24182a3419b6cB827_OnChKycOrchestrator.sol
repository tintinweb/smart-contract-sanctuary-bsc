...
contract OnChKycOrchestrator is OnChOwnableWithWhitelist {
    ...
    function checkKyc(address account) public view returns (bool) {
        return _kycs[account];
    }

    function completeKyc(address account) public whitelistedOnly returns (bool) {
        if (!_kycs[account]) {
            _kycs[account] = true;
    ...
    function revokeKyc(address account) public whitelistedOnly returns (bool) {
        if (_kycs[account]) {
            _kycs[account] = false;
    ...
    function completeKycForBatch(address[] calldata addresses) public whitelistedOnly returns (bool) {
        for (uint256 i = 0; i < addresses.length; i++) {
            completeKyc(addresses[i]);
        }
        return true;
    }

    function revokeKycForBatch(address[] calldata addresses) public whitelistedOnly returns (bool) {
        for (uint256 i = 0; i < addresses.length; i++) {
            revokeKyc(addresses[i]);
        }
        return true;
    }
