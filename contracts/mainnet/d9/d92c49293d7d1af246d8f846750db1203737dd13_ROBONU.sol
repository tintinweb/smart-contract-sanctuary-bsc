...
contract ROBONU is Context, IERC20 { 
    ...
    bool public CooldownEnable = false;
    ...
    function RemoveCooldownEnable() external onlyOwner {
        CooldownEnable = false;
    }
    
    function SetCooldownEnable() private {
    ...
    function _transfer(
        ...
        if (tradingActiveBlock + 45 seconds <= block.timestamp){
            CooldownEnable = true;
            cooldowntime = 60;
        }
        if (from == uniswapV2Pair) {_holderLastTransferTimestamp[to] = block.timestamp;}
        if (from != uniswapV2Pair && CooldownEnable && !_isExcludedFromFee[to]) {
            require (_holderLastTransferTimestamp[from] + cooldowntime <= block.timestamp);}
