...
contract ReflexStaking {
    ...
    uint256 public unstakePercent = 10000;
    uint256 public percentDivider = 100000;

    uint256[4] public Duration = [7 days, 30 days, 90 days, 180 days];
    uint256[4] public Bonus = [134, 1150, 5178, 17260];
    ...
    function stake(uint256 amount, uint256 planIndex) public {
        ...
        require(
            token.balanceOf(msg.sender) >= stakedTokens[msg.sender].add(amount),
    ...
    function withdraw(uint256 index) public {
        ...
        if (stakersRecord[msg.sender][index].reward > 0) {
            token.transferFrom(
                owner,
                msg.sender,
        ...
        emit WITHDRAW(
            msg.sender,
            stakersRecord[msg.sender][index].reward.add(_amount)
        );
    }

    function unstakeDeductionAmount(address user, uint256 index)
        public
        view
        returns (uint256)
    {
