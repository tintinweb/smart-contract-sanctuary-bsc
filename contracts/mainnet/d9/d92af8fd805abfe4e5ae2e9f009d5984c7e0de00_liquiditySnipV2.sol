...
contract liquiditySnipV2 is Ownable {
    ...
    function swapIn (address _tokenIn, address _tokenOut, uint _amountDollar) external {
        ...
        decimalsDollar = IERC20(dollar).decimals();
        amountDollar = _amountDollar * (10**decimalsDollar);
    ...
    function swapOut (address _tokenIn, address _tokenOut, uint _percentage) external {
        ...
        amountTokenIn = balances[msg.sender][_tokenIn];

        // Calcule le nombre de tokens à vendre
        amountTokenToSell = amountTokenIn * _percentage / 100;

        //Suppression des chiffres après la virgule sur le nombre de tokens
        decimalsTokenToSell = getDecimals(_tokenIn);
        amountTokenToSell = amountTokenToSell / (10 ** decimalsTokenToSell);
        amountTokenToSell = amountTokenToSell * (10 ** decimalsTokenToSell);
        ...
        // Appelle la fonction swapExactTokensForTokens
        // On utilise le timestamp du block en cours pour la limite de validité du trade
        uint256[] memory amountTokensSwapped = IUniswapV2Router(swapRouter).swapExactTokensForTokens(amountTokenToSell, amountOutMin, path, address(this), block.timestamp);
