...
contract NFTStore is Ownable, SignerRole, StoreBase {
    ...
    function batchMint(
        ...
        string[] memory _uri) public {
         require(from.length == to.length && from.length == _id.length &&
         from.length == _supply.length && from.length == _uri.length,"Invalid params");

        bytes32 msgHash = prepareHash(_id.length, blockExpiry);
        require(
            isSigner(verifySignature(msgHash,_signature)),
