...
contract Sidus_Partner_Marsdao is Pausable, AccessControl {
    ...
    struct CardsUserLimit {
        uint min;
        uint max;
    }

    mapping(uint => CardsUserLimit) public CARDS_PER_USER;
    ...
    function registerForPreSale(
        address _erc20,
        LimitItem[] calldata _wanted
    ) external whenNotPaused {
        uint256 currLimit;
        uint256 userDebt;
        require(block.timestamp >= activeAfter, "Cant buy before start");
        require(block.timestamp <= closedAfter, "Cant buy after closed");
        for (uint256 i; i < _wanted.length; ) {
            require(
                registered[msg.sender] <= maxRegisterCount,
                "Max registrations"
            );
            ...
            require(
                priceForCard[_erc20][_wanted[i].tokenId].value > 0,
                "Cant buy with this token"
            );
            uint currentLimit = (currLimit +
                _addLimitForCard(
                    msg.sender,
                    _wanted[i].tokenId,
                    _wanted[i].limit
            ...
            require(
                currentLimit >= CARDS_PER_USER[_wanted[i].tokenId].min &&
                    currentLimit <= CARDS_PER_USER[_wanted[i].tokenId].max,
                "Purchase limit exceeded"
            );
            uint amount = _getERC20AmountPerItem(_erc20, _wanted[i]);
            userDebt += amount;
            require(
                (_wanted[i].limit +
                    maxTotalSupply[_wanted[i].tokenId].currentSupply) <=
                    maxTotalSupply[_wanted[i].tokenId].maxTotalSupply,
                "Max Total Supply limit exceeded"
            );
            maxTotalSupply[_wanted[i].tokenId].currentSupply += _wanted[i]
                .limit;
            emit Purchase(
                msg.sender,
                _erc20,
    ...
    function setMaxRegisterCount(
        uint value
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        maxRegisterCount = value;
    }
    ...
    function setCardsPerUser(
        uint tokenId,
        uint256 min,
        uint max
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        CARDS_PER_USER[tokenId].min = min;
        CARDS_PER_USER[tokenId].max = max;
    }
    ...
    function getUserLimitsInfo(
        address _user
    ) external view returns (LimitItem[] memory) {
    ...
    function pauseSetOn() external whenNotPaused onlyRole(PAUSEMODE_ROLE) {
        _pause();
    }

    function pauseSetOff() external whenPaused onlyRole(PAUSEMODE_ROLE) {
        _unpause();
    }
    ...
    function _addLimitForCard(
        address _user,
        uint256 _tokenId,
        uint256 _increment
    ) internal returns (uint256 increment) {
        ...
        if (increment == 0) {
            whiteList[_user].push(
                LimitItem({tokenId: _tokenId, limit: _increment})
            );
            increment = _increment;
        }
    }

    function _getUserLimitForCard(
        address _user,
        uint256 _tokenId
    ...
    function _getERC20AmountPerItem(
        address _erc20,
        LimitItem calldata _wanted
    ) internal view returns (uint256) {
        return
            (_wanted.limit * priceForCard[_erc20][_wanted.tokenId].value) /
