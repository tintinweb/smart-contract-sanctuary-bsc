...
contract ShellToken is ERC20, Ownable {
    ...
    function setStTime(uint256 ts_) public onlyOwner {
        require(stTime == 0 || stTime > block.timestamp, "ShellToken: invalid start time");
        stTime = ts_;
    }
   ...
   function processLpReward() private {
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns(uint256 amountAfter) {
        ...
        require(block.timestamp >= stTime, "ShellToken: try again later");
