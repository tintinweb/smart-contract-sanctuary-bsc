...
contract SmartMintableInitializable is ReentrancyGuard, Ownable {
    ...
    function batchMint(uint number) external payable nonReentrant onlyWhitelist {
        ...
        AAANFTToken.batchMint(recipients);
    }

    modifier onlyWhitelist() {
        require(
            !hasWhitelistLimit ||
                AAANFTMakeFactory(SMART_MINNER_FACTORY).whitelist(
                    msg.sender
                ),
...
contract AAANFTMakeFactory is Ownable {
    ...
    modifier onlyAdmin() {
        require(isAdmin[msg.sender], "AAANFTMakeFactory: caller is not the admin");
        _;
    }
