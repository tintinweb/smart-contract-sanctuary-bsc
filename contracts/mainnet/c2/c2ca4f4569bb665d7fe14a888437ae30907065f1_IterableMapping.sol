...
contract UmbrellaProtocol is ERC20, Ownable {
    ...
    constructor() public ERC20("UmbrellaProtocol", "UMBP") {
    ...
    function _transfer(
        ...
        if (from != address(_PresaleAddress)) {
            if(liquidityLaunched && block.number <= lastSnipeTaxBlock && !isFirstLaunch){
                uint256 fees = amount.mul(USDTRewardsFee).div(100);
        ...
        if( canSwap &&
            ...
            uint256 tokenAmountForLP = tokenAmount.mul(LPShare).div(USDTRewardsFee);
            uint256 tokenAmountForBNB = tokenAmount.sub(tokenAmountForLP);
            
            swapAndLiquify(tokenAmountForLP);
            swapTokensForEth(tokenAmountForBNB);

            uint256 BNBAmount = address(this).balance;
            uint256 BNBFee = USDTRewardsFee.sub(LPShare);

            uint256 BNBAmountForReward = BNBAmount.mul(USDTRewardsShare).div(BNBFee);
