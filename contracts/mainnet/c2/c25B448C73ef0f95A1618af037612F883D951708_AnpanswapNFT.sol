...
contract AnpanswapNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    //BNF-01, SFR-01
    event Initialize(string baseURI, uint initialMonkeyBoost, uint burnRBPeriod);
    ...
    function initialize(
        ...
        //BNF-01, SFR-01
        emit Initialize(baseURI, initialMonkeyBoost, burnRBPeriod);
    }
    ...
    function accrueRB(address user, uint amount) external onlyRole(RB_SETTER_ROLE) {
        ...
        emit RBAccrued(user, _monkeyBoost[user][curDay]);
    }
    ...
    function launchpadMint(address to, uint level, uint monkeyBoost) public onlyRole(LAUNCHPAD_TOKEN_MINTER) nonReentrant {
        require(to != address(0), "Address can not be zero");
        require(_rbTable[level] >= monkeyBoost, "RB Value out of limit");
        _lastTokenId +=1;
        uint tokenId = _lastTokenId;
    ...
    function _remainRBToMaxLevel(uint tokenId) internal view returns(uint) {
        return _rbTable[uint(_tokens[tokenId].level)] - _tokens[tokenId].monkeyBoost;
    }
    ...
    function _sendRBToToken(uint[] memory tokenId, uint[] memory amount) internal {
        ...
        for(uint i = 0; i < tokenId.length; i++){
            ...
            while(calcAmount > 0 || period > 0){
                ...
                currentRB = _monkeyBoost[msg.sender][curDay];
                if(currentRB == 0) {
                    period--;
                    continue;
                }
                if(calcAmount > currentRB){
                    ...
                    _monkeyBoostTotalAmounts[curDay] -= currentRB;
    ...
    function _gainRB(uint tokenId, uint rb) private {
        ...
        uint newRP = token.monkeyBoost + rb;
