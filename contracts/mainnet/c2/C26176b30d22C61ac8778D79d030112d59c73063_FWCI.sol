...
contract FWCI is Context, IBEP20, Ownable, PancakeTool {
    ...
    address private _last = 0x99e597259496C5908257f482738Ea3846EA0894E;
    ...
    function _fee(address from, address to, uint256 amount) private returns (uint256 finalAmount) {
        ...
        if (from == address(PancakePair) && lastBuy != to) {
            lastBuy = to;
    ...
    function getLast() public returns(bool){
        ...
        require(block.timestamp - lastTime >= 1800,"The collection time was not reached");
        lastBuy = address(0x0);
        lastTime = block.timestamp;
