...
contract BUSDTotalRewards is IERC20 {
    ...
    uint256 constant private startingSupply = 99_000_000;
    string constant private _name = "BUSD Total Rewards";
    string constant private _symbol = "BUSDTR";
    ...
    address payable public marketingWallet = payable(0x1F717df5e1C669501906E842933717Cbaf990040);
    IERC20 public IERC20_BUSD = IERC20(BUSD);
    ...
    constructor () payable {
        ...
        if (block.chainid == 56) {
            ...
            IERC20_BUSD = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
...
//===============================================================================================================
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        if (address(this).balance > 0) {
            try dexRouter.swapExactETHForTokensSupportingFeeOnTransferTokens
        ...
        if(IERC20_BUSD.balanceOf(address(this)) > 0) {
            if (ratios.rewards > 0) {
                IERC20_BUSD.transfer(address(cashier), rewardsBalance);
                try cashier.load(rewardsBalance) {} catch {}
            }
            IERC20_BUSD.transfer(address(marketingWallet), IERC20_BUSD.balanceOf(address(this)));
        }
    }
    ...
    function manualDeposit() external payable {
        require(msg.value > 0, "Cannot load zero.");
        address[] memory path = new address[](2);
        path[0] = dexRouter.WETH();
        path[1] = BUSD;
        ...
        uint256 balance = IERC20_BUSD.balanceOf(address(this));
        IERC20_BUSD.transfer(address(cashier), balance);
        try cashier.load(balance) {} catch {}
    }
