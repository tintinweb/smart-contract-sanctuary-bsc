...
interface IOldFund {
    ...
    function balanceOf(address victim) external view returns (uint256);
}
...
contract SurgeFundStorage {
    ...
    function updateVictims(uint nVictims) external {
        for (uint i = 0; i < nVictims;) {
            ...
            uint claim = oldFund.balanceOf(victim_) / 420;
    ...
    function nVictimsRepaid() external view returns (uint256) {
        return allVictims.length - remainingVictims.length;
    }
