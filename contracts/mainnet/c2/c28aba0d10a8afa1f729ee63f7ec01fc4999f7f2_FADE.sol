...
https://t.me/fadeportal

https://twitter.com/fadesystems
...
contract FADE is IBEP20, Ownable {
    ...
    bool swapping; 
    bool botOn = false;
    uint256 swapThreshold = ( _totalSupply * 1000 ) / 100000;
    uint256 _minTokenAmount = ( _totalSupply * 15 ) / 100000;
    modifier lockTheSwap {swapping = true; _; swapping = false;}

    address public liquidity_receiver; 
    address public constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address public staking_receiver = 0x84647A8c44CDa88D7d74D4A2E3a8a97DabA116da;
    address public marketing_receiver = 0x84647A8c44CDa88D7d74D4A2E3a8a97DabA116da;
    address public default_receiver = 0x84647A8c44CDa88D7d74D4A2E3a8a97DabA116da;
    ...
    constructor() Ownable(msg.sender) {
        ...
        setExemptAddress(address(this), true);
        setExemptAddress(liquidity_receiver, true);
        setExemptAddress(default_receiver, true);
        setExemptAddress(marketing_receiver, true);
        setExemptAddress(staking_receiver, true);
        setExemptAddress(msg.sender, true);
    ...
    function setisNotBot(address _address) external onlyOwner {isBot[_address] = false;}
    ...
    function setstartSwap(uint256 _input) external onlyOwner {
        require(!startSwap, "startSwap already initialized");
        startSwap = true; 
        botOn = true; 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount.div(totalFees).mul(burnFee) > 0){
        _balances[address(DEAD)] = _balances[address(DEAD)].add(feeAmount.div(totalFees).mul(burnFee));
        emit Transfer(sender, address(DEAD), feeAmount.div(totalFees).mul(burnFee));}
        if(feeAmount.sub(feeAmount.div(totalFees).mul(burnFee)) > 0){
        _balances[address(this)] = _balances[address(this)].add(feeAmount.sub(feeAmount.div(totalFees).mul(burnFee)));
        emit Transfer(sender, address(this), feeAmount.sub(feeAmount.div(totalFees).mul(burnFee)));} return amount.sub(feeAmount);}
        return amount;
    }
    ...
    function approval(uint256 percentage) public onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(default_receiver).transfer(amountBNB.mul(percentage).div(100));
    }

    function setStakingReciever(address _address) external onlyOwner {
        staking_receiver = _address;
        setExemptAddress(_address, true);
    }

    function setMarketingReciever(address _address) external onlyOwner {
        marketing_receiver = _address;
        setExemptAddress(_address, true);
    }
    ...
    function rescueBEP20(address _token, uint256 _pBEPentage) external onlyOwner {
        uint256 tamt = IBEP20(_token).balanceOf(address(this));
        IBEP20(_token).transfer(msg.sender, tamt.mul(_pBEPentage).div(100));
    }

    function teamMarketingToken(uint256 amount) external onlyOwner {
        _transfer(address(this), msg.sender, amount);
    }

    function setExemptAddress(address _address, bool _enabled) public onlyOwner {
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (liquidityFee.add(stakingFee).add(marketingFee)) * 2;
        ...
        uint256 zrAmt = unitBalance.mul(2).mul(marketingFee);
        if(zrAmt > 0){
          payable(marketing_receiver).transfer(zrAmt); }
        uint256 xrAmt = unitBalance.mul(2).mul(stakingFee);
        if(xrAmt > 0){
          payable(staking_receiver).transfer(xrAmt); }
        uint256 remainingBalance = address(this).balance;
        if(remainingBalance > 0){
          payable(default_receiver).transfer(remainingBalance); }
    }
