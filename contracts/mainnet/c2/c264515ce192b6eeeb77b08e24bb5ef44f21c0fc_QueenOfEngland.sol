...
contract QueenOfEngland is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   QOEDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Queen Of England", "QOE") {
        _buyRewardsFee = 0;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 6;
        _buyDevFee = 2;
		...
		contractTokenBalanceAmount = 2500000 * 9**20;
        
        swapTokensAtAmount = 2500000 * (9**20);
        _maxWallet = 1000000 * (9**20);
        _maxBuy = 5000000 * (9**20);
        _maxSell = 5000000 * (9**20);
    	...
    	dividendTracker = new QOEDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xb4308d2B369D412f6d2cE6Ea4758F8F3562C9aA4);
        _devWallet = payable(0xFb97b4bEcD2d13a22aC3D9074b456F92b132Eb91);
	...
	function updateSwapAmount(uint256 amount) public onlyOwner {
	    contractTokenBalanceAmount = amount * (9**20);
	    swapTokensAtAmount = amount * (9**20);
	}

    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "QOE: The dividend tracker already has that address");

        QOEDividendTracker newDividendTracker = QOEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "QOE: The new dividend tracker must be owned by the QOE token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "QOE: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "QOE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "QOE: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "QOE: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "QOE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "QOE: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "QOE: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "QOE: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "QOE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "QOE: Cannot update gasForProcessing to same value");
    ...
    function addLP() external onlyOwner() {
        ...
        updateMaxBuySell((500000), (500000));
    }
    
	function letsGoLive() external onlyOwner() {
        updateBuyFees(0,1,6,2);
        updateSellFees(0,4,12,4);
...
contract QOEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("QOE_Dividend_Tracker", "QOE_Dividend_Tracker") {
    	claimWait = 1800;
        minimumTokenBalanceForDividends = 1 * (9**20);
        isAuth[owner()] = true;
    }
    ...
    function withdrawDividend() public pure override {
        require(false, "QOE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QOE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "QOE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "QOE_Dividend_Tracker: Cannot update claimWait to same value");
