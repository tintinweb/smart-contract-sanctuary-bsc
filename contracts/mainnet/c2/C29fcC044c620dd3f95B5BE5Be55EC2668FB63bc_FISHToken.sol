...
contract FISHToken is Context, IERC20, Ownable {
    ...
    uint256 public _mkFee1 = 20;
    uint256 public _mkFee2 = 10;
    uint256 public _mkFee3 = 20;
    ...
    function setAmount(uint amount)external onlyOwner{
    ...
    struct Param{
     ...
     function _initParam(uint256 tAmount,Param memory param) private view  {
        ...
        if( param.takeFee ){
            ...
            param.tMk3 = tAmount * _mkFee3 / 1000;
    ...
    function _takeFee(Param memory param,address from)private {
        if( param.tMk1 > 0 ){
            _take(param.tMk1, from, marketAddress1);
        }
        if( param.tMk2 > 0 ){
            _take(param.tMk2, from, marketAddress2);
        }
        if( param.tMk3 > 0 ){
            _take(param.tMk3, from, marketAddress3);
        }
        if( param.tBurn > 0 ){
