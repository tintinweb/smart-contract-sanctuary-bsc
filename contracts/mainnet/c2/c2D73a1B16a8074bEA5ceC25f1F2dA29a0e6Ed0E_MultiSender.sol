...
contract MultiSender is Ownable, ReentrancyGuard {
     ...
     */
    function multisender(address token, address[] memory _contributors, uint256 _balances) external nonReentrant {
        uint256 total = 0;
        for (uint256 i; i < _contributors.length; i++) {
            IERC20Metadata(token).transferFrom(msg.sender, _contributors[i], _balances);
            total += _balances;
        }
     ...
     */
    function collectCoin() external onlyOwner {
     ...
     */
    function collectToken(address _token) external onlyOwner {
