...
/**
 * @title TokenStakingPTS
 */
contract TokenStakingPTS is Ownable, ReentrancyGuard {
    ...
    event StakedPTS(address from, uint256 amount, uint256 tierNumber);
    event RewardsClaimed(address beneficiary, uint256 amount);
    event UnstakedPTS(address from, uint256 amount);

    modifier onlyIfBeneficiaryExists(address beneficiary) {
        require(
            _holdersStakingCount[beneficiary] > 0,
            "TokenStakingPTS: INVALID Beneficiary Address! no staking schedule exists for that beneficiary"
        );
        _;
    }
    ...
    function getStakingIdAtIndex(uint256 index) private view returns (bytes32) {
        require(
            ...
            "TokenStakingPTS: index out of bounds"
        );
        ...
        returns (StakingSchedule memory)
    {
        require(
            ...
            "TokenStakingPTS: INVALID Staking Schedule Index! no staking schedule exists at this index for that beneficiary"
        );
    ...
    function stakeTokens(uint256 amount, uint256 tierNumber) external {
        ...
        emit StakedPTS(from, amount, tierNumber);
    }
    ...
    function _stakeTokens(
        ...
        require(
            (tier >= 1) && (tier <= 4),
            "TokenStakingPTS: invalid tier, choose 1-4"
        );

        if (tier == 1) {
            require(
                (amount >= _tier1MinAmount && amount < _tier1MaxAmount),
                "TokenStakingPTS: Tier 1 only allows staking >=_tier1MinAmount and <_tier1MaxAmount PTS tokens"
            );
        }
        if (tier == 2) {
            require(
                (amount >= _tier2MinAmount && amount < _tier2MaxAmount),
                "TokenStakingPTS: Tier 2 only allows staking >=_tier2MinAmount and <_tier2MaxAmount PTS tokens"
            );
        }
        if (tier == 3) {
            require(
                (amount >= _tier3MinAmount && amount < _tier3MaxAmount),
                "TokenStakingPTS: Tier 3 only allows staking >=_tier3MinAmount and <_tier3MaxAmount PTS tokens"
            );
        }
        if (tier == 4) {
            require(
                (amount >= _tier4MinAmount && amount < _tier4MaxAmount),
                "TokenStakingPTS: Tier 3 only allows staking >=_tier4MinAmount and <_tier4MaxAmount PTS tokens"
            );
        }
    ...
    function _unstakeTokens(address beneficiary) private returns (uint256) {
        ...
        require(
            ...
            "TokenStakingPTS: tokens are locked, wait till the staking period end"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "TokenStakingPTS: not enough withdrawable funds in reward reserves"
        );
        ...
        onlyIfBeneficiaryExists(msg.sender)
    {
        ...
        require(
            totalReleaseableAmount > 0,
            "TokenStakingPTS: no new claimable rewards, check back tomorrow"
        );
        require(
            totalReleaseableAmount <= _getRewardReservesBalance(),
            "TokenStakingPTS: not enough reward funds available"
        );
        tokenAddress.transfer(beneficiary, totalReleaseableAmount);
