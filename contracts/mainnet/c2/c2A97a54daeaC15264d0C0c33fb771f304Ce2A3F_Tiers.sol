...
contract Tiers is ERC1155Holder, Ownable {
    ...
    struct NFTInfo {
        uint256 currentStartTokenID;
        uint256 lastUpdateTime;
        bool started;
    }

    NFTInfo public nftInfo;
    uint256 public requiredPercent;
    uint256[5] public tiers;
    ...
    function startNFTDistribution() external onlyOwner {
        require(!nftInfo.started, "NFT distribution has already started");
        nftInfo.currentStartTokenID = 1;
        nftInfo.lastUpdateTime = block.timestamp;
        nftInfo.started = true;
    }
    ...
    function claimNFT(uint256 _tier) external {
        require(_tier >= 1 && _tier <= 5, "Invalid tier for claiming NFT");
        uint256 userTier = checkUserTier(msg.sender);
        require(userTier != 0, "Not enough points to claim NFT");
        require(userTier >= _tier, "Your tier doesn't allow to claim this NFT");
        if (block.timestamp > nftInfo.lastUpdateTime + 30 days) {
            nftInfo.currentStartTokenID += 5;
            nftInfo.lastUpdateTime = block.timestamp;
        }
        uint256 tokenId = nftInfo.currentStartTokenID + _tier - 1;
        require(
            !claimed[msg.sender][tokenId],
            "NFT for this tier is already claimed in this month"
        );
        nft.safeTransferFrom(address(this), msg.sender, tokenId, 1, "");
        claimed[msg.sender][tokenId] = true;
        emit ClaimedNFT(msg.sender, _tier, tokenId);
    }
    ...
    function checkUserTier(address _account) public view returns (uint256) {
        uint256 totalFixedStaked = fixedStaking.totalStaked(_account);
...
contract FixedStaking is Ownable {
    ...
    function returnAccidentallySent(IERC20 _token) public onlyOwner {
        require(
            address(_token) != address(fitToken),
...
contract FlexibleStaking is Ownable {
    ...
    // ~3835.6164383561643744 tokens/day (2% of total supply for 5 years)
    uint256 public rewardRate = 44393708777270421;
