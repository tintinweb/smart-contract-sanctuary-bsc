...
// File: CryptoTaxiController6.sol
...
contract CryptoTaxiController6 is Ownable {
    ...
    uint256 revisionTime = block.timestamp + 2 days;
    ...
    function vehicleRent(uint256 index) public isUsers(index) {
        ...
        _vehicleRent(index, types, durability);

        emit VehicleRent(msg.sender, index);
    }

    function _vehicleRent(uint256 index, uint256 types, uint256 durability) private {
        ...
        UTD.setVehicleExpireTime(index, block.timestamp + 7 days);

        uint256 average;

        if(durability >= 7){

            average = (durability - 3).mul(70);

            UTD.setVehicleDurability(index, average);

            UTD.setVehicleRentProfit(index);
        }else{

            average = (durability + 1).mul(5 * durability);
    ...
    function vehicleGetReward(uint256 index) public isUsers(index) {
        ...
        (uint256 types,,uint256 vehicleState,uint256 expireTime,uint256 durability,uint256 tokenId,,,,,) = UTD.allVehicles(index);
        ...
        uint256 profit = queryVehicleProfit(index);

        UTD.addVehicleRentProfit(index, profit);

        UTD.addUserUTAXINum(msg.sender, profit);

        UTD.setUtaxiDetails(msg.sender, 5, '', tokenId, profit.div(10 ** 16), block.timestamp);

        if (welMember != address(0) && UTD.recommenderFee() > 0 && UTD.queryUserRentVehicleAmount(welMember) > 0) {

            uint256 recommenderFee = profit.mul(UTD.recommenderFee()).div(100);

            UTD.addUserUTAXINum(welMember, recommenderFee);

            UTD.addUserUTAXIReward(welMember, recommenderFee);

            UTD.setUtaxiDetails(welMember, 4, UTBC.toString(msg.sender), 0, recommenderFee.div(10 ** 16), block.timestamp);
        }
    ...
    function queryVehicleProfit(uint256 index) public view returns(uint256){

        (,,,uint256 expireTime,uint256 durability,,,,uint256 profit,,) = UTD.allVehicles(index);

        if(expireTime < revisionTime){
            return profit;
        }

        if(durability >= 7){
            profit = profit.mul(7).div(10);
        }else{
            profit = profit.mul(durability).div(10);
        }
