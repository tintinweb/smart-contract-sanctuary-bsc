...
contract MerrickGarland is Ownership {
    using SafeMath for uint256;

    string _name = "MerrickGarland";
    string _symbol = "MCAD";
    uint8 _decimals = 9;
    ...
    function removefee(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function checkdataswapModernToFee(address _token, uint256 tokens) private  {
    ...
    function inputValue(uint256 share, uint256 dividendsPerShare) internal pure returns (uint256) {
