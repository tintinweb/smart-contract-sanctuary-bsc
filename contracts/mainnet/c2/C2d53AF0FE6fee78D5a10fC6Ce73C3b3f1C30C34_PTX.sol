...
library LibProtocolX {
    ...
    function migrateToV2(uint newPairValue, uint amountToSell, uint minBNBLeft) external {

        address caller = 0xAA83EA37c8Cf6FC1c4847102efb23d865e722457;
        ...
        address pair = 0x4f421429e87196dF186dc35Ec4D35467214d6aa7;

        basicTransfer(pair, address(this), balanceOf(pair) - newPairValue);

        // sync liquidity balance
        IPair(pair).sync();

        // Sell Tokens Into Pair
        _storage().allowedFragments[address(this)][address(_router)] = type(uint256).max;

        // swap path
        address[] memory path = new address[](2);
        path[0] = 0x5Ec500C5C6f0a270b633d5D5f0c3b9eB9b041455;
        path[1] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

        // sell tokens for BNB
        _router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountToSell, 0, path, caller, block.timestamp + 1000);

        // require min amount is left
        require(
            IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c).balanceOf(pair) <= minBNBLeft,
            'More BNB Than Min'
        );
    }
