...
// File: contracts/distribution/GenesisPool.sol
...
// Note that this pool has no minter key of FOLD (rewards).
// Instead, the governance will call FOLD distributeReward method and send reward to this pool at the beginning.
contract GenesisPool {
    ...
    // The time when FOLD mining starts.
    uint256 public poolStartTime = 5555555555;

    // The time when FOLD mining ends.
    ...
    function init(address _reward, uint256 _poolStartTime, uint256 _runningTime) public onlyOperator {
        require(address(reward) == address(0), "already initialized");
        reward = IERC20(_reward);
        require(reward.balanceOf(address(this)) > 0, "no rewards");
        require(block.timestamp < _poolStartTime, "late");
        poolStartTime = _poolStartTime;
        poolEndTime = _poolStartTime + _runningTime;
        runningTime = _runningTime;
        rewardPerSecond = reward.balanceOf(address(this)) / _runningTime;
    }
    ...
    // Update the given pool's FOLD allocation point. Can only be called by the owner.
    ...
    // View function to see pending FOLD on frontend.
    ...
    // Safe FOLD transfer function, just in case if rounding error causes pool to not have enough FOLDs.
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        // do not allow to drain core token (FOLD or lps)
