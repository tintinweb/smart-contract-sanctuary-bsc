...
contract AuctionManager is ReentrancyGuard {
    ...
    function getAuctionInfo(address[] calldata _auctionsList)
        ...
        returns (
            ...
            bool[] memory buyerWithdrawn,
            bool[] memory bidderWithdrawn
        )
    {
        auctionAddress = new address[](_auctionsList.length); 
        ...
        highestBidder = new address[](_auctionsList.length); 
        highestBid = new uint256[](_auctionsList.length);
        tokenIds = new uint256[](_auctionsList.length);
        startTime = new uint256[](_auctionsList.length);
        ...
        buyerWithdrawn = new bool[](_auctionsList.length);
        bidderWithdrawn = new bool[](_auctionsList.length);


        for (uint256 i = 0; i < _auctionsList.length; i++) { // for each auction
            auctionAddress[i] = Auction(auctions[i]).auctionAddress(); // get the owner of the auction
            ...
            highestBidder[i] = Auction(auctions[i]).maxBidder(); // get the high
            highestBid[i] = Auction(auctions[i]).maxBid(); // get the highest bidder
            tokenIds[i] = Auction(auctions[i]).tokenId(); // get the token id
            startTime[i] = Auction(auctions[i]).startTime();
            ...
            buyerWithdrawn[i] = bool(Auction(auctions[i]).buyerWithdrawn()); //buyer withdrawn
            bidderWithdrawn[i] = bool(Auction(auctions[i]).bidderWithdrawn()); //bidder withdrawn
        }
