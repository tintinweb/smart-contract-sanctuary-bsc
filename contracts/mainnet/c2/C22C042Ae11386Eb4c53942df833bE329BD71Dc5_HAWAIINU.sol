...
// https://t.me/hawaiiinu
// https://hawaiiinu.com
// https://twitter.com/Hawaiiinu_ofc

/*

Although we have a strong meme culture,
 our utility roadmaps gives us a very strong usercase. 
 Our utilities are designed for our community.
  Hawaii Inu token is the driver of the Hawaii Inu utilities economy.
...
contract HAWAIINU is IERC20, Context, Ownable {
    ...
    string private constant _name = "Hawai Inu";

    uint256 public Hwi_max_BuyLimit = 1000000000 * 10**9;
    uint256 public Hwi_max_SellLimit = 1000000000 * 10**9;

    address private Hwi_Inu_Wallet = 0xa1e59d2Ea99aB69558c8DF978AE235bE726973eB;
    address private Hwi_Team_Wallet = 0x827E3397da80b19470f8A39Af50d87C8b0c3aC8B;

    uint256 public Hwi_max_WalletLimit = 2000000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 27, 3, 0);
    Taxes public taxes = Taxes(0, 27, 3, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Hwi_Inu_Wallet] = true;
        _isExcludedFromFee[Hwi_Inu_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Hwi_Inu_Wallet] = true;
        allowedTransfer[Hwi_Inu_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Hwi_max_BuyLimit, "You are exceeding Hwi_max_BuyLimit");
            require(balanceOf(to) + amount <= Hwi_max_WalletLimit, "You are exceeding Hwi_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Hwi_max_SellLimit, "You are exceeding Hwi_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Hwi_max_WalletLimit, "You are exceeding Hwi_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Hwi_Inu_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Hwi_Inu_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Hwi_Update_MarketingWallet(address newWallet) external onlyOwner{
        Hwi_Inu_Wallet = newWallet;
    }
    
    function Hwi_Update_TeamWallet(address newWallet) external onlyOwner{
        Hwi_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Hwi_max_BuyLimit = maxBuy * 10**decimals();
        Hwi_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Hwi_Max_Walletlimit(uint256 amount) external onlyOwner{
        Hwi_max_WalletLimit = amount * 10**decimals();
    }
