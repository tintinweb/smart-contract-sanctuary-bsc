...
contract LayerZeroGateway is Pausable, ReentrancyGuard, BalanceManagement, IGateway {
    ...
    uint16 private constant ADAPTER_PARAM_VERSION = 1;
    ...
    event LZReceivePausedFailure();
    event LZReceiveFromAddressFailure(uint256 indexed sourceStandardChainId, address indexed fromAddress);
    event LZReceiveClientFailure(uint256 indexed sourceStandardChainId, address indexed fromAddress);
        ...
        onlyLayerZeroProxy
    {
        ...
        if (!condition) {
            emit LZReceiveFromAddressFailure(sourceStandardChainId, fromAddress);
            return;
        }

        try client.handleExecutionPayload(sourceStandardChainId, _payload) {
        } catch {
            emit LZReceiveClientFailure(sourceStandardChainId, fromAddress);
        }
    }
    ...
    function _getAdapterParam() private view returns (bytes memory) {
        return abi.encodePacked(ADAPTER_PARAM_VERSION, targetGas);
    }
}