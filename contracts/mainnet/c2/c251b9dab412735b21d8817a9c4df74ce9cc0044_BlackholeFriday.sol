...
contract BlackholeFriday {
    ...
    function getBnbTicketPrice() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = address(busdAddress);
        path[1] = pancakeswapV2Router.WETH();

        uint256[] memory prices = pancakeswapV2Router.getAmountsOut(ticketPrice, path);
        return prices[1];
    }
    ...
    function enter(uint amount) public payable {
        ...
        uint256 bnbTicketPrice = getBnbTicketPrice();
        require(msg.value == bnbTicketPrice * amount, 'Lottery: WRONG_TICKET_PRICE');
        address[] memory path = new address[](2);
