...
interface ExchangeCnt {
    function stake(uint256 _amount)
        external;
}


contract Ztoken is Context, IBEP20, Ownable {
  ...
  address public _destory = 0xD67c38C9902b4C3B24746Ee5cA1cac5726Bf7E47;

  constructor(string memory new_name,string memory new_symbol) public {
    _name = new_name;
    _symbol = new_symbol;
    _decimals = 18;
    _totalSupply = 1*1e23;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient] && !isAddLiquidity && !isDelLiquidity)
    {
        ...
        {
            _totalfee = amount.mul(_Fee).div(BASE);

            _balances[_destory] = _balances[_destory].add(_totalfee);
            
            emit Transfer(sender, _destory, _totalfee);
        }
    }
    
    _balances[recipient] = _balances[recipient].add(amount.sub(_totalfee));
    
    emit Transfer(sender, recipient, amount.sub(_totalfee));
  }
  ...
  function excludeFromFee(address account) public onlyOwner {
    ...
    function set_Fee(uint new_Fee) onlyOwner external  {
        _Fee =  new_Fee;
    }

    function set_destory(address new_destory) onlyOwner external {
