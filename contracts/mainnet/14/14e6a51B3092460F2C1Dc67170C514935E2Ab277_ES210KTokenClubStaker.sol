...
/******************************************************************************\
* EverSafuv2 10K Token CLub NFT Staking Pool
*
...
contract ES210KTokenClubStaker is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function stake(uint256[] memory tokenIds) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        require(keyToken.balanceOf(address(msg.sender)) > 999999999, "Must have at least 10,000 ES2 in wallet to stake NFT"); // must have at least 10,000 ES2 in wallet to stake NFT
        require(stakingToken.balanceOf(address(msg.sender)) < 2, "Maximum staked amount is 1 NFT"); //can only have 1 nft in wallet to use pool
        uint256 amount;
    ...
    function withdraw(uint256[] memory tokenIds) public nonReentrant updateReward(msg.sender) {
        require(tokenIds.length != 0, "Staking: No tokenIds provided");
        require(keyToken.balanceOf(address(msg.sender)) > 999999999, "Must have at least 10,000 ES2 in wallet to withdraw NFT"); // must have at least 10,000 ES2 in wallet to withdraw NFT
        require(stakingToken.balanceOf(address(msg.sender)) < 2, "Maximum staked amount is 1 NFT"); //can only have 1 nft in wallet to use pool
        uint256 amount;
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
    ...
    function getReward() public nonReentrant updateReward(msg.sender) {
        ...
        require(keyToken.balanceOf(address(msg.sender)) > 999999999, "Must have at least 10,000 ES2 in wallet to clain rewards"); // must have at least 10,000 ES2 in wallet to claim rewards
        require(stakingToken.balanceOf(address(msg.sender)) < 2, "Maximum staked amount is 1 NFT"); //can only have 1 nft in wallet to use pool
        if (reward > 0) {
