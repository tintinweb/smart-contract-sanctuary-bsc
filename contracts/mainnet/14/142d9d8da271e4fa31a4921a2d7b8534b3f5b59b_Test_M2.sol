...
//////
    ...
    constructor(address payable wallet) public {
        ...
        plans.push(Plan(0.01 ether, 5 minutes));
        plans.push(Plan(0.02 ether, 10 minutes));
        plans.push(Plan(0.03 ether, 15 minutes));
        plans.push(Plan(0.04 ether, 20 minutes));
        plans.push(Plan(0.05 ether, 30 minutes));
        plans.push(Plan(0.06 ether, 40 minutes));
        plans.push(Plan(0.07 ether, 60 minutes));
        plans.push(Plan(0.08 ether, 1 minutes));
        plans.push(Plan(0.09 ether, 1 minutes));
        plans.push(Plan(0.10 ether, 1));
        plans.push(Plan(0.11 ether, 1));
        plans.push(Plan(0.12 ether, 1));
        plans.push(Plan(0.13 ether, 1));
        plans.push(Plan(0.14 ether, 1));
        plans.push(Plan(0.15 ether, 1));
        plans.push(Plan(0.16 ether, 1));
...
///////
 ...
 //for (uint256 i = 0; i < user.deposits.length; i++) {
    //  sfr = user.checkpointRefBonus+(user.deposits[i].finish);
     //require(block.timestamp >= (user.checkpointRefBonus+(user.deposits[i].finish)));
 

 //require(block.timestamp >= sfr);
...
/////////
 ...
 uint256 sfr;
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
      sfr = users[userAddress].checkpointRefBonus+(user.deposits[i].finish);
        }
     return sfr;
    }
    ...
    function getPlanInfo(uint8 plan)
        public
        view
        returns (uint256 time, uint256 price)
    {
	...
	function getResult(uint8 plan) public view returns ( uint256 profit, uint256 finish) {
		//percent = getPercent(plan);
		profit = (PERCENTS_DIVIDER).mul(plans[plan].time);
...
*/
	...
	function getUserLastDepositInfo(address userAddress, uint8 plan) public view returns(  uint256 amount, uint256 start, uint256 finish, uint256 profit) {
	    User storage user = users[userAddress];
		if(user.deposits.length > 0){
