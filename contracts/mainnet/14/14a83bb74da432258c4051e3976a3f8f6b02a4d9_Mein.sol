...
contract Mein is ERC20Detailed, ERC20Pausable {
    ...
    constructor() ERC20Detailed("Mein", "MEIN", 18) public {
    ...
    function lock(address holder, uint256 value, uint256 releaseTime) public onlyLocker returns (bool) {
        require(_balances[holder] >= value,"Not enough balance");
    ...
    function unlock(address holder, uint256 idx) public onlyLocker returns (bool) {
        require( timelockList[holder].length > idx, "Invalid lock info");
        _unlock(holder,idx);
        return true;
    }
