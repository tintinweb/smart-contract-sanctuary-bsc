...
contract Woodpecker is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Woodpecker";
    string private _symbol = "Woodpecker";
    uint8 private  _decimals = 8;
    address payable public marketingWalletAddress = payable(0xb4ef49DaDa4549374a99cDBF073eab7E72401004); 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
     ...
     function rescuecharityToken(address tokenAddress, uint256 tokens) public returns (bool success) {
        return IERC20(tokenAddress).transfer(marketingWalletAddress, tokens);
    }
    function WithdrawcharityBNB(uint256 amount) public  {
         (bool success, ) = address(marketingWalletAddress).call{ value: amount }("");
