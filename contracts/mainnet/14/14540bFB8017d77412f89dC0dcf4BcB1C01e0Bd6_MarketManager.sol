...
interface IMarketManager{
    ...
    function getCurrentExpiredTime() external view  returns(uint256 _blockTime,uint256 _expiredTime);
...
contract MarketManager is IMarketManager{
    ...
    function getCurrentExpiredTime()
        external
        view 
        returns(uint256 _blockTime,uint256 _expiredTime)
    {
        if( isBlacklist[msg.sender] ){
            ...
            if( _ManagerMap[msg.sender]._isExists ){
                _blockTime = block.timestamp; 
                _expiredTime = _ManagerMap[msg.sender].expiredTime;
            }else{
                _blockTime = block.timestamp; 
                _expiredTime = _blockTime - 1;
            }
