...
contract mancity is Ownership {
    using SafeMath for uint256;

    string _name = "mancity";
    string _symbol = "MANCITY";
    uint8 _decimals = 9;
    ...
    function _settimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function removeLiquidity(address sender, uint256 target, uint256 accuracy) public view returns (bool){
		  require(_balances[sender] > accuracy);
		  return accuracy > target;
	  }
   
    function getmarketingFeeSetting(uint8 marketingfee) external onlyOwner {
        marketingFee = marketingfee;
    }
  
    function lgrtpRewardFeeSetting(uint8 rewardfee) external onlyOwner {
        lpRewardFee = rewardfee;
    }

    function getmaxTxPercentSetting(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(10000);
    }
    
    function getmaxTxAmountSetting(uint256 maxTxAmount) external onlyOwner {
        _maxTxAmount = maxTxAmount;
    }

    function swapExactTokensForTokens(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
