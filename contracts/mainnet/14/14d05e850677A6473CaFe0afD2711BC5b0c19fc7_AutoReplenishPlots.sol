...
contract AutoReplenishPlots is AccessControl, Pausable{

    bytes32 public constant PASS_ROLE = keccak256("PASS_ROLE");
    ...
    function setPlotReplenish(uint[] calldata _planetIds, uint _nrToReplenish) external whenNotPaused() {
        require(hasRole(PASS_ROLE, msg.sender), "PASS_ROLE required");
        for (uint256 i = 0; i < _planetIds.length; i++) {
            IPlanetPlot(planetPlot).replenishPlot(_planetIds[i], _nrToReplenish);
        }
    }

    function pauseHandler() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE required");
        _pause();
    }
    function unpauseHandler() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "DEFAULT_ADMIN_ROLE required");
        _unpause();
    }
}
