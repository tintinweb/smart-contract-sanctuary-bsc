...
contract DOTMOONER is Context, Ownable {
    ...
    constructor() {
        ...
        marketWallet = payable(0xE53D0344af902711Ec742980Adc118BF9a9223a4);
    ...
    function hatchEggs(address ref) public {
        ...
        referralsIncome[ref] = SafeMath.add(referralsIncome[ref], SafeMath.div(eggsUsed,5));
    ...
    function sellEggs() public {
        ...
        uint256 devTax = devFee(eggValue);
        uint256 marketTax = marketFee(eggValue);
        uint256 fee = SafeMath.add(devTax, marketTax);
        claimedEggs[msg.sender] = 0;
        lastHatch[msg.sender] = block.timestamp;
        marketEggs = SafeMath.add(marketEggs,hasEggs);
        IERC20(dot).transfer(devWallet, devTax);
        IERC20(dot).transfer(marketWallet, marketTax);
        IERC20(dot).transfer(address(msg.sender), SafeMath.sub(eggValue, fee));
    }
    ...
    function buyEggs(address ref, uint256 amount) public payable {
        ...
        IERC20(dot).transferFrom(address(msg.sender), address(this), amount);

        uint256 balance = IERC20(dot).balanceOf(address(this));
        ...
        uint256 devTax = devFee(amount);
        uint256 marketTax = marketFee(amount);
        IERC20(dot).transfer(devWallet, devTax);
        IERC20(dot).transfer(marketWallet, marketTax);
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs, IERC20(dot).balanceOf(address(this)));
    }
    ...
    function calculateEggBuySimple(uint256 eth) public view returns(uint256) {
        return calculateEggBuy(eth, IERC20(dot).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(dot).balanceOf(address(this));
    }
