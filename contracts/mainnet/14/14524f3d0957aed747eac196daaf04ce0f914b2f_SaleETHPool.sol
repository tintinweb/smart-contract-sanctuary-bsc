...
contract SaleETHPool is OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function getCurrentMatch() external view returns (uint256) {
  ...
  function updateMatch(
    ...
    require(_match.timestamp == 0, "updateMatch: the match is over");
    ...
    emit UpdateMatch(_id, _team1Id, _team2Id, _startTime, _match.amount);
  }

  function removeMatch(uint256 _id) external {
    require(hasRole(UPDATER_ROLE, msg.sender) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "removeMatch: invalid role");
    MatchInfo storage _match = matches[_id];
    require(!_match.forceDone, "removeMatch: the match is over");
    require(
      _match.timestamp == 0 || _match.timestamp.add(GRACE_PERIOD) > block.timestamp,
      "removeMatch: the match is over"
    );
    distributedETH = distributedETH.sub(_match.amount);
    _matchCounter.decrement();
    _match.status = MatchStatus.NA;
    _match.amount = 0;
    _match.team1Goals = 0;
    _match.team2Goals = 0;
    _match.root = bytes32(0);
    _match.timestamp = 0;
  }
