...
contract sweepOutSwapV3 is Context, Ownable {
  ...
  function processVolume(
    ...
    uint256 amount;
    uint256 receiver;
    uint256 interavel = (addrs.length*2)+1;
    do{
        if(randomInt(2,i,addrs[i])>0 && inputamount > 0){
            amount = min + randomInt(range,i,addrs[i]);
  ...
  function randomInt(uint256 _mod,uint256 i,address adr) public view returns (uint256) {
    uint256 randomNum = uint256(
      keccak256(abi.encodePacked(block.timestamp,i,adr))
    );
