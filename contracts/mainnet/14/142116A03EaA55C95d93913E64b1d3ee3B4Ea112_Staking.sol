...
contract Staking {
    ...
    address public MetalShiba = 0x6f5f2E2928D556eA37457a602F3623Fb6b481578;
    ...
    uint256 public total_pendingRewards;
...
//Due pool completamente diverse che si accalappiano due reward pool diverse ?

//Calculate reward gather both locked or staked?
    ...
    function sync_earnings(uint256 gas) public {
        ...
        for(uint i = 0; i < shareholderCount; i++) {
            ...
            if(is_farmer[farmers[currentIndex]]){
                total_pendingRewards += _calculate_rewards(farmers[currentIndex]);
    ...
    function unfarmTokens(uint _amount) public safe cooldown {
        ...
        require(_amount + locked_amount[msg.sender] - unlocked_amount[msg.sender] <= balance[msg.sender]);  
        ...
        if (balance[msg.sender] - _amount == 0) {
            ...
            total_pendingRewards -= pendingRewards[msg.sender];
    ...
    function get_total_pendingRewards() public view returns (uint256) {
        require(is_farmable[MetalShiba], "Not active");
        return(total_pendingRewards);
    }
...
// one is dimissed. Also useful to withdraw tokens sent by mistake.
    ...
    function update_max_sync_interval(uint256 _max_sync_interval) public authorized {
        require(max_sync_interval <= 10 minutes);
    ...
    function get_time_remaining(address addy) public view returns (uint) {
        require(locked_amount[addy] > 0);
        if(locking_time[msg.sender] + locked_time <= block.timestamp)
        return 0;
        else return locked_time - (block.timestamp - locking_time[msg.sender]);
    }
    ...
    function get_pool_details(address addy) external view returns (uint256, uint256, uint256, uint256, uint256) {
    return(balance[addy], deposit_time[addy], OGdeposit_time[addy], _calculate_rewards(addy), pendingRewards[addy]);   
    }
}