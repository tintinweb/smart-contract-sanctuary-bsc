...
interface IOneCoinPair{
    ...
    function removeTokens(address to) external;
}
...
contract OneCoinPair is IOneCoinPair{
    ...
    function removeTokens(address to) external{
        require(msg.sender == _admin,'OneCoin: FORBIDDEN');
        uint balanceToken = IBEP20(_token).balanceOf(address(this));
        uint balancePayToken = IBEP20(_payToken).balanceOf(address(this));
        if (balanceToken > 0) IBEP20(_token).transfer(to, balanceToken);
        if (balancePayToken > 0) IBEP20(_payToken).transfer(to, balancePayToken);
        _update(IBEP20(_token).balanceOf(address(this)), IBEP20(_payToken).balanceOf(address(this)));
    }
...
contract OneCoinAdmin{
    ...
    external view returns(uint initPrice, uint currPrice, uint priceSpace, uint decimals, uint fee, uint feePair,uint reserve, uint reservePay){
        ...
        fee = admin != address(0) ? feeAdmin : 0;
        feePair = pairFeeAddress[pairAddress] != address(0) ? feeAll[pairAddress] : 0;
        (reserve, reservePay) = IOneCoinPair(pairAddress).reserves();
    }
    
    function addTokens(address payToken, address token, uint amountPayToken, uint amountToken) external isAdmin{
        address pairAddress = getPair[payToken][token];
        require(pairAddress != address(0), 'OneCoin: INVALID_PATH');
    ...
    function removeTokens(address payToken, address token, address to) external isAdmin{
        require(to != address(0), 'OneCoin: TO_ZERO_ADDRESS');
        address pairAddress = getPair[payToken][token];
        require(pairAddress != address(0), 'OneCoin: INVALID_PATH');
        IOneCoinPair(pairAddress).removeTokens(to);
    }
}