...
contract VAULT is BEP20 {
    ...
    mapping(address => bool) _multiSig;
    ...
    constructor () {
        ...
        _stakingAddress = msg.sender;
        _pause = false;
    }

    function transferBUSD(address addr, uint amount) public payable stakingCaller Pausable noEntry{
        require(STAKE[address(busd)] > amount, "Something went wrong");
        require(busd.balanceOf(address(this)) >= amount, "Not enough tokens");
        STAKE[address(busd)] = STAKE[address(busd)].sub(amount);
        busd.transfer(payable(addr), amount);
    }

    function transferDUC(address addr, uint amount) public payable stakingCaller Pausable noEntry{
        require(STAKE[address(duc)] > amount, "Something went wrong");
        require(duc.balanceOf(address(this)) >= amount, "Not enough tokens");
        STAKE[address(duc)] = STAKE[address(duc)].sub(amount);
        duc.transfer(payable(addr), amount);
    }

    function SaleBUSD(address _to, uint _value, bytes32 _id) external onlyTeam Pausable noEntry{
        require(SALES[address(busd)] > _value, "Something went wrong");
        require(busd.balanceOf(address(this)) >= _value, "Not enough tokens");
        require(!_isPaid[_to][_id], "Payment already done");
        SALES[address(busd)] = SALES[address(busd)].sub(_value);
    ...
    function SaleDUC(address _to, uint _value, bytes32 _id) external onlyTeam Pausable noEntry{
        require(SALES[address(duc)] > _value, "Something went wrong");
        require(duc.balanceOf(address(this)) >= _value, "Not enough tokens");
        require(!_isPaid[_to][_id], "Payment already done");
        SALES[address(duc)] = SALES[address(duc)].sub(_value);
        _isPaid[_to][_id] = true;
         uint output = _value;
        if (!_isExcludedFromFee[_to]) {
            output = output.sub(_value.mul(FEE).div(1000));
    ...
    function SaleBNB(address _to, uint _value, bytes32 _id) external onlyTeam Pausable noEntry{
        require(SALES[address(wbnb)] > _value, "Something went wrong");
        require(address(this).balance > _value, "Not enough liquidity");
        require(!_isPaid[_to][_id], "Payment already done");
        SALES[address(wbnb)] = SALES[address(wbnb)].sub(_value);
        _isPaid[_to][_id] = true;
         uint output = _value;
        if (!_isExcludedFromFee[_to]) {
            output = output.sub(_value.mul(FEE).div(1000));
    ...
    function SignMultiSig() external onlyOwner {
        require (msg.sender == _salesWallet || msg.sender == _stakeWallet);
        require (!_multiSig[msg.sender]);
        _multiSig[msg.sender] = true;
    }

    function WithdrawMultiSig(address token, uint value) external payable onlyOwner noEntry {
        require (_multiSig[_salesWallet] && _multiSig[_stakeWallet]);
        _multiSig[_salesWallet] = false;
        _multiSig[_stakeWallet] = false;
        IBEP20(token).transfer(payable(_liquidityWallet), value);
    }
    ...
    function getMultiSig(address _addr) external view onlyTeam returns(bool){
        return _multiSig[_addr];
    }

    function getPayment(address _addr, bytes32 _id) external view onlyTeam returns(bool){
        return _isPaid[_addr][_id];
    }

    function getSalesMax(address addr) external view onlyTeam returns(uint){
        return SALES[addr];
    }
    function getStakeMax(address addr) external view onlyTeam returns(uint){
        return STAKE[addr];
    }

    function getStakeWallet() external view onlyTeam returns(address){
        return _stakeWallet;
    }

    function getSaleWallet() external view onlyTeam returns(address){
        return _salesWallet;
    }

    function getLiquidityWallet() external view onlyTeam returns(address){
