...
Shinrai Inu is a Hyper Deflationary token that generates passive income to holders just 
by holding and receiving a percentage of auto reflections on every transaction with 
a primary use case developing an easy to use dapp, dashboard and nft character 
collection with Staking features for huge apy and future Play to Earn gaming access.
...
contract SHINRAINUV2 is Context, IBEP20, Ownable {
    ...
    string private _name = "SHINRAI INU V2"; // token name
    string private _symbol = "SHINRAI V2"; // token ticker
