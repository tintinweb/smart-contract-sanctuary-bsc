...
contract bscnetwork {
    ...
    IBEP20 public token;
    uint256[] public reward = [250e18,1000e18,2500e18,3500e18,5000e18,7500e18,12500e18,25000e18,50000e18,100000e18];
    ...
    uint256[] public reward_self_business_condition = [250e18,1000e18,2500e18,3500e18,5000e18, 7500e18, 10000e18, 20000e18,25000e18,50000e18];
    uint256[] public GI_PERCENT = [10,10,10,10,10,7,7,7,7,7,5,5,5,5,5,3];
    uint256 public  BASE_PERCENT = 50; // 0.5% per day
    uint256 public constant booster = 50; // 0.5% per day
    uint256 INVEST_MIN_AMOUNT = 100e18;
    ...
    constructor(address payable marketingAddr,IBEP20 _tokenUSDT ) {
    ...
    function invest(address payable referrer,uint256 token_quantity) public  beforeStarted() {
        ...
        token.transfer(owner,tokenWei.mul(1200).div(PERCENTS_DIVIDER));
       
         _setUpline(msg.sender, referrer,tokenWei);
          address upline  = user.referrer;
         if(users[upline].level_business[0] >= 25000e18 && users[upline].is_networker == false)
        { 
    ...
    function withdraw() public beforeStarted() {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (user.available > 0) {
                ...
                {   
                    dividends += (
                        user.deposits[i].amount.mul(booster).div(PERCENTS_DIVIDER)
                    )
                    .mul(block.timestamp.sub(user.is_networker_timestamp)).div(TIME_STEP);
        ...
        uint256 min_check_value = totalAmount.add(user.direct_amount).add(user.gi_bonus);
        ...
        totalAmount -= fees;
        user.checkpoint = block.timestamp;
        user.is_networker_timestamp = block.timestamp;
