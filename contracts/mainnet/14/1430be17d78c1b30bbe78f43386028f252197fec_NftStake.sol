...
contract NftStake is IERC721Receiver, ReentrancyGuard {
    ...
    function remover(uint256 index ) internal {
       uint256 tokenCount = nftToken.balanceOf(address(this));

        for(uint256 i = index; i < tokenCount - 1; i++ ){
            jefe[msg.sender].idnft[i] = jefe[msg.sender].idnft[i + 1];
        }
        jefe[msg.sender].idnft.pop;
    }
    ...
    function misnft()  public view returns(uint256[] memory){
        
        uint256 tokenCount = nftToken.balanceOf(address(this));
        uint256[] memory tokensId = new uint256[](tokenCount);

        for (uint256 i = 0; i <  tokenCount; i++) {
            tokensId[i] = jefe[msg.sender].idnft[i];
        }
        return tokensId;
    }
