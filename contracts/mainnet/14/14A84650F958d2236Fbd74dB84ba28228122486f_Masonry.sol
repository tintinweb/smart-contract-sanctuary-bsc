...
interface ITreasury {
    ...
    function getSctPrice() external view returns (uint256);
...
contract PshareWrapper {
    ...
    function stake(uint256 amount) public virtual {
        ...
        pshare.safeTransferFrom(msg.sender, address(this), amount);
    }

    function withdraw(uint256 amount) public virtual {
        uint256 masonPshare = _balances[msg.sender];
        require(masonPshare >= amount, "Masonry: withdraw request greater than staked amount");
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = masonPshare.sub(amount);
        pshare.safeTransfer(msg.sender, amount);
    }
}

contract Masonry is PshareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        MasonrySnapshot memory genesisSnapshot = MasonrySnapshot({time : block.number, rewardReceived : 0, rewardPerPshare : 0});
    ...
    function getSctPrice() external view returns (uint256) {
        return treasury.getSctPrice();
    }

    // =========== Mason getters

    function rewardPerPshare() public view returns (uint256) {
        return getLatestSnapshot().rewardPerPshare;
    }

    function earned(address mason) public view returns (uint256) {
        uint256 latestRPS = getLatestSnapshot().rewardPerPshare;
        uint256 storedRPS = getLastSnapshotOf(mason).rewardPerPshare;
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            sct.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 prevRPS = getLatestSnapshot().rewardPerPshare;
        uint256 nextRPS = prevRPS.add(amount.mul(1e18).div(totalSupply()));

        MasonrySnapshot memory newSnapshot = MasonrySnapshot({
            time: block.number,
            rewardReceived: amount,
            rewardPerPshare: nextRPS
        });
        masonryHistory.push(newSnapshot);

        sct.safeTransferFrom(msg.sender, address(this), amount);
        emit RewardAdded(msg.sender, amount);
    }

    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        // do not allow to drain core tokens
        require(address(_token) != address(sct), "sct");
        require(address(_token) != address(pshare), "pshare");
