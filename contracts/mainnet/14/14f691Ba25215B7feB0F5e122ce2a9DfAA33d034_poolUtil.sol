...
library poolUtil {
    ...
    function swap(iBeacon.sExchangeInfo memory exchangeInfo,stData memory iData,uint amountIn, address[] memory path) external returns (uint){
        ...
        if ((exchangeInfo.intermediateToken != address(0) && (path[0] == iData.token0 || path[1] == iData.token0)) ||
             (exchangeInfo.intermediateToken != address(0) && exchangeInfo.baseToken == address(0))) {
            pathLength = 3;
            intermediateToken = exchangeInfo.intermediateToken;
        }

        if (exchangeInfo.baseToken != address(0) && (path[0] == iData.token1 || path[1] == iData.token1)) {
            pathLength = 3;
            intermediateToken = exchangeInfo.baseToken;
        }
