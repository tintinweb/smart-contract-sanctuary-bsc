...
contract Prosper365 is DataStorage, Access, Events, UUPS {
  ...
  function handlePayout(address _addr, address _sponsor, address _activeSponsor, uint _package, uint _matrixNum, uint _amount) internal {
    ...
    payout = locateBonusMatchPayout(_addr, payout[0].receiver, _package, _amount, payout);
  ...
  function locateBonusMatchPayout(address _addr, address _matrixReceiver, uint _package, uint _amount, Payout[] memory _payout) internal returns (Payout[] memory) {
    address receiver = members[_matrixReceiver].sponsor;

    while (members[receiver].accountType == TYPE_AFFILIATE) {
