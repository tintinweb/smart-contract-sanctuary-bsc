...
contract QTest2 {
    ...
    function compoundAutoCubic(address adr, bool isCompound) internal {
        ...
        uint256 cubicForCompound = cubicUsed.sub(payAuto(cubicUsed));
...
*/
    ...
    function ADD_AUTOMATE() external {
    ...
    function runAutomate() external {
        ...
        while(iterations < automateCount) {
            ...
            uint256 hasCubic = users[adr].claimedCubic.add(getCubicSinceLastCompound(adr));
            if(hasCubic > 0){
                if ((block.timestamp - automations[adr].lastrun) >= 3600) {  //86400=24hrs, 3600=1hr
                    if(automations[adr].day == 7) {
                        automations[adr].day = 1;
                        automations[adr].lastrun = block.timestamp;
                        sellAutoCubic(adr);
                    }
                    else {
                        automations[adr].day++;
