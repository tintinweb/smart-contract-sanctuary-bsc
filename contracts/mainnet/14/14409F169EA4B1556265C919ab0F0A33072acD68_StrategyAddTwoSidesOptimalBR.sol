...
// File: contracts/StrategyAddTwoSidesOptimalBR.sol
...
contract StrategyAddTwoSidesOptimalBR is Ownable, ReentrancyGuard, Strategy {
    ...
    constructor(IUniswapV2Router02 _router, address _goblin, ISwapV2 _swap) public {
        ...
        nonReentrant 
    {
        ...
        // use bestrate
        {
            ...
            (uint256 swapAmt, bool isReversed) = optimalDeposit(address(this).balance, fToken.myBalance(), ethReserve, fReserve);
            (address src, address dest) = isReversed ? (fToken, swapWETH) : (swapWETH, fToken);

            if (src == swapWETH) {
                swap.swapQuery.value(swapAmt)(swapAmt, src, dest);
            } else {
        ...
        bool isReversed;
        {
            (uint256 r0, uint256 r1, ) = lpToken.getReserves();
            (uint256 ethReserve, uint256 fReserve) = lpToken.token0() == weth ? (r0, r1) : (r1, r0);
            (swapAmt, isReversed) = optimalDeposit(address(this).balance, fToken.myBalance(), ethReserve, fReserve);
        }
