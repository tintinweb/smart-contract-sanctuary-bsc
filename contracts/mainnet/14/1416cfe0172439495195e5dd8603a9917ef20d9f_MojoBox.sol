...
contract MojoBox is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Mojo Box", "Mojo Box") {
        LAUNCHPAD = 0x190449C9586a73dA40A839e875Ff55c853dBc2f8;
        LAUNCH_MAX_SUPPLY = 2000;

        contract_owner = payable(msg.sender);

        saleConfig.max_num = 2;
        saleConfig.MojoBox_total = 2000;
        saleConfig.startTime = 1663214400;
        saleConfig.endTime = 1663646400;
        saleConfig._price = 1*10**16;
        saleConfig.vip_price = 1*10**16;
    }
    ...
    function mintTo(address to, uint size) external onlyLaunchpad {
        ...
        require(size <= saleConfig.max_num, "Exceeded max token purchase");
        require(totalSupply() + size <= saleConfig.MojoBox_total, "Purchase would exceed max supply of tokens");
        require(mint_num[to] + size <= saleConfig.max_num, "Exceeded max token purchase.");

        for (uint256 i=1; i <= size; i++) 
        {
            uint mintIndex = ERC721Enumerable.totalSupply();
            if (ERC721Enumerable.totalSupply() < saleConfig.MojoBox_total) 
            {
                _mint(to, mintIndex);
                LAUNCH_SUPPLY++;

                mint_num[to] = mint_num[to]+1;
            }
        }
    }
    ...
    function mintBatch(address account, uint256 num) external onlyOwner() {
        ...
        require(totalSupply() + num <= saleConfig.MojoBox_total, "Purchase would exceed max supply of tokens");

        for(uint i = 0; i < num; i++) 
        {
            uint mintIndex = totalSupply();
            if (totalSupply() < saleConfig.MojoBox_total) 
            {
    ...
    function mint(uint num, uint8 _type) public payable {
        require(IsActive, "Sale must be active to mint Tokens");
        require(saleConfig.startTime <= block.timestamp && block.timestamp <= saleConfig.endTime, "Sale has not started yet.");
        ...
        if(_type==1)
        {
            require(saleConfig.vip_price * num <= msg.value, "BNB value sent is not correct.(whitelist)");
        }
        else
        {
            require(saleConfig._price * num <= msg.value, "BNB value sent is not correct.");
        }
    ...
    function set_vip_price(uint256 vip_p) public onlyOwner {
        saleConfig.vip_price = vip_p;
    }
