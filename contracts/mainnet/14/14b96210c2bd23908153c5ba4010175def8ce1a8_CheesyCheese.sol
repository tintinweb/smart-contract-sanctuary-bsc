...
contract CheesyCheese is ERC20, Ownable {
    ...
    function setPermaStakedNFTAndSwiss() public onlyOwner {
        require(SwissArmyContract != address(0) && NFTRewardsContract != address(0),"addresses not set");
        _isStaked6Months[SwissArmyContract] = true;
        _isStaked6Months[NFTRewardsContract] = true;
    }
    ...
    function getStakingBalance(address account) private view returns (uint256) {
        ...
        unstakedUpdate = unstakedBalance.mul(55).div(100);
...
contract Test is ISwissArmyInterface, Ownable {

    IUniswapV2Router02 public uniswapV2Router;
    string private testName;
    string private testTicker;

    mapping(address => bool) public longTermStaked;
    ...
    function setLongTerm(address account, uint256 amount) external {
        longTermStaked[account] = true;
        stakedAmount[account] = amount;
    }
    function removeLongTerm(address account) external {
        longTermStaked[account] = false;
