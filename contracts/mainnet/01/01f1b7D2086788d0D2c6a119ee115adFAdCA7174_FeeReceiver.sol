...
contract FeeReceiver {
    ...
    /**
        Minimum Amount Of PLUSPLUSPLUS In Contract To Trigger `trigger` Unless `approved`
    ...
    modifier onlyOwner(){
        require(
            msg.sender == IPLUSPLUSPLUS(token).getOwner(),
            'Only PLUSPLUSPLUS Owner'
        );
        _;
    }
    ...
    constructor(address _token, address _fund) {
        ...
        minimumTokensRequiredToTrigger = 0;
    ...
    function trigger() external {
        ...
        // PLUSPLUSPLUS Balance In Contract
        ...
        // sell PLUSPLUSPLUS in contract for BNB
        IERC20(token).approve(address(router), balance);
        router.swapExactTokensForTokensSupportingFeeOnTransferTokens(balance, 0, path, trustFund, block.timestamp + 300);
    }
