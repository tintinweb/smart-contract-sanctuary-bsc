...
contract Futuria is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    uint256 public txFee = 500000000000000; // 0.0005 BNB
    ...
    // Volume
    mapping(address => uint256) addressToDirectCustomersVolume;
    mapping(address => uint256) public addressToWeeklyDirectCustomersVolume;
    mapping(address => uint256) addressToPurchases;
    ...
    mapping(address => uint256) addressToMinRank;
    ...
    mapping(uint256 => uint256) tokenIdToSubsLength;
    ...
    constructor() ERC721("Futuria", "FUTURIA") {
        ...
        subscriberToTimestamp[defaultReferralAccount] = 5000 days;
        addFounderIfPossible(defaultReferralAccount);

        if (isTestnet) {
            safeMint(1, msg.sender, "1", 1 ether, 30 days);
        } else {
            safeMint(1, admin, "1", 400 ether, 365 days);
            safeMint(1, admin, "1", 250 ether, 182 days);
            safeMint(1, admin, "1", 60 ether, 30 days);
            safeMint(1, admin, "1", 150 ether, 91 days);
        }
    }
    ...
    //     addToTreeIfNeeded(sponsor, referralAddress);
    //     saveAddressIfNeeded(msg.sender);
    ...
    {
        if (!isAdmin()) {
            require(msg.value >= txFee, "Insufficient BNB");
        }
        ...
        sponsor = addToTreeIfNeeded(sponsor, referralAddress);
    ...
    function toggleRefundGasEnabled(bool _isRefundGasEnabled)
        external
        onlyAdmin
    {
        isRefundGasEnabled = !_isRefundGasEnabled;
    }
    ...
    function addToTreeIfNeeded(address sponsor, address referralAddress)
        internal
        returns (address)
    {
        ...
        if (!isDefaultReferralAccount) {
            ...
            if (sponsor == address(0)) {
                bool isRightLegSelected = addressToIsRightLegSelected[sponsor];

                sponsor = referralAddress;
                addressToSponsor[msg.sender] = sponsor;
                addToTree(sponsor, msg.sender, isRightLegSelected, 0);
            }
        }
    ...
    function setLegPositionIfNeeded(address newUser, uint256 leg) internal {
    ...
    function saveLegData(
        ...
        setLegPositionIfNeeded(newUser, leg);
    }
    ...
    function addToTree(
        ...
        if (!isRightLegSelected) {
            if (leftLegAddr == address(0)) {
                ...
                // 2 legs occupied
                setLegPositionIfNeeded(newUser, leftLeg);
            ...
            if (rightLegAddr == address(0)) {
                saveLegData(rootAddress, newUser, rightLeg);
                addressToLeg2Address[rootAddress] = newUser;
            } else if (leftLegAddr == address(0) && rightLegAddr != newUser) {
                saveLegData(rootAddress, newUser, leftLeg);
                addressToLeg1Address[rootAddress] = newUser;
            } else {
                // 2 legs occupied
                setLegPositionIfNeeded(newUser, rightLeg);
