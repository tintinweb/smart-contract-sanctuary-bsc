...
contract SaudiGrowPump is Context, Ownable, IERC20, IERC20Metadata {
    using SafeMath for uint256;
    string private _name = "Saudi Grow Pump";
    string private _symbol = "SGP";
    uint8 private _decimals = 9;
    uint256 private _totalSupply;
    uint256 public _taxSell = 4;
    uint256 public _taxBuy = 4;
    uint256 public _taxDivider = 100;
    address public _recipientFee;
    uint256 public _taxRFI = 3;
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "SaudiGrowPump: decreased allowance below zero"
        );
        unchecked {
    ...
    function _transfer(
        ...
        require(
            sender != address(0),
            "SaudiGrowPump: transfer from the zero address"
        );
        require(
            recipient != address(0),
            "SaudiGrowPump: transfer to the zero address"
        );
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(
            account != address(0),
            "SaudiGrowPump: mint to the zero address"
        );
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(
            account != address(0),
            "SaudiGrowPump: burn from the zero address"
        );

        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "SaudiGrowPump: burn amount exceeds balance"
        );
        unchecked {
    ...
    function _approve(
        ...
        require(
            owner != address(0),
            "SaudiGrowPump: approve from the zero address"
        );
        require(
            spender != address(0),
            "SaudiGrowPump: approve to the zero address"
        );
