...
contract NftHub is ControllableInit, BaseProxyStorage, IUpgradeSource, INftHub {
        ...
        returns (uint256)
    {
        ...
        uint256 daysCount = blockTime.sub(lastUpdate).div(24 hours);
        return daysCount.mul(totalLanttiPerDay);
    }
     ...
     */
    function harvest(uint256 _amount) internal {
        userLastUpdate[msg.sender] = block.timestamp;
        if (_amount > 0) {
            lantti.mint(msg.sender, _amount);
        }
...
contract VaultyNft is ERC1155Mintable {
    ...
    constructor(address _proxyRegistryAddress) public ERC1155Mintable("Vaulty NFT", "VaultyNFT", _proxyRegistryAddress) {
        _setBaseMetadataURI("https://nft-gallery.s3.eu-central-1.amazonaws.com/nfts/");
    }
