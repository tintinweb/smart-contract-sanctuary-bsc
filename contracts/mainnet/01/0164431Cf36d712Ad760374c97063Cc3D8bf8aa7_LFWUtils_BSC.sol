...
interface IApeStakingV1Interface {
...
contract LFWUtils_BSC {
    ...
    function getPancakeStakingInfo(
        ...
        if (scInterface.bonusEndBlock() > block.number) {
    ...
    function getApeStakingV1Info(
        ...
        // Initialize interface
        IApeStakingV1Interface scInterface = IApeStakingV1Interface(_scAddress);
        ...
        // [2] Calculate an optional term to calculate APR for backend
        uint256 rewardPerYear = scInterface.cakePerBlock()*dailyBlock*yearDay;
        address stakedTokenAddress = banana;
        uint256 stakedTokenBalance = IERC20(stakedTokenAddress).balanceOf(_scAddress);
        stakingInfo[2] = rewardPerYear;
        stakingInfo[3] = stakedTokenBalance;
        ...
        stakingData[0] = banana;
        stakingData[1] = banana;
    }

    function getApeStakingV2Info(
        address _scAddress, 
        address _userAddress
    ) public view returns(uint256[] memory stakingInfo, address[] memory stakingData) {
        // Define array to return
        ...
        // [2] Calculate an optional term to calculate APR for backend
        uint256 rewardPerYear = scInterface.rewardPerBlock()*dailyBlock*yearDay;
        address stakedTokenAddress = scInterface.stakeToken();
        uint256 stakedTokenBalance = IERC20(stakedTokenAddress).balanceOf(_scAddress);
        stakingInfo[2] = rewardPerYear;
        stakingInfo[3] = stakedTokenBalance;
    ...
    function getApeStakingV3V4Info(
        address _scAddress, 
        address _userAddress
    ) public view returns(uint256[] memory stakingInfo, address[] memory stakingData) {
        // Define array to return
        ...
        stakingData[0] = stakedTokenAddress;
        stakingData[1] = scInterface.REWARD_TOKEN();
    }
    ...
    function getBiswapStakingInfo(
        ...
        if (endDay > scInterface.getCurrentDay()){
