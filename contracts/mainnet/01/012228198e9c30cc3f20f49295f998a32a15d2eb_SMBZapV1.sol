 ...
 * Implemented and modified by SMBSwap teams.
 */
contract SMBZapV1 is Ownable, ReentrancyGuard {
    ...
    // SMBRouter interface
    ISMBRouter02 public SMBRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from SMBSwap)
    ...
    // Address SMBRouter
    address private SMBRouterAddress;
     ...
     * @param _SMBRouter: address of the SMBRouter
    ...
    constructor(
        ...
        SMBRouterAddress = _SMBRouter;
        SMBRouter = ISMBRouter02(_SMBRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. SELF/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. SELF) to receive in the intermediary swap (e.g. BNB --> SELF)
     */
     ...
     * @param _lpToken: LP token address (e.g. SELF/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. SELF) in the intermediary swap (e.g. BUSD --> SELF)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. SELF/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. SELF --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. SELF/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. SELF or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. SELF) in the intermediary swap (e.g. BUSD --> SELF)
     */
    ...
    {
        address token0 = ISMBPair(_lpToken).token0();
        address token1 = ISMBPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ISMBPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = SMBRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = SMBRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ISMBPair(_lpToken).token0() || _token0ToZap == ISMBPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ISMBPair(_lpToken).token0() || _token1ToZap == ISMBPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ISMBPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ISMBPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ISMBPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = SMBRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = SMBRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ISMBPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "SMBRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "SMBRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        SMBRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), SMBRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(SMBRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(SMBRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = SMBRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = SMBRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = SMBRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = SMBRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = SMBRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
