...
contract BeefyUniV2Zap {
    ...
    function beefInETH (address beefyVault, uint256 tokenAmountOutMin, address refferal) external payable {
        require(msg.value >= minimumAmount, 'Beefy: Insignificant input amount');

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(beefyVault, tokenAmountOutMin, WETH, refferal);
    }

    function beefIn (address beefyVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount, address refferal) external {
        ...
        _swapAndStake(beefyVault, tokenAmountOutMin, tokenIn, refferal);
    }
    ...
    function _swapAndStake(address beefyVault, uint256 tokenAmountOutMin, address tokenIn, address referral) private {
        (IBeefyVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(beefyVault);
