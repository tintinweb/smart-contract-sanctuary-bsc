...
contract CongRess is AuthCIRCLE {
    using SafeMath for uint256;    
    uint256 public totalSupply = 10000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "CongRess";    
    string public symbol = "CongRess";
    uint8 public decimals = 9;   

    uint8 public _bliquidityFee;
    uint8 public _bbuybackFee;
    ...
    function modifyCoolDownSet(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
    ...
    function modifyAmountSet(uint256 maxWallPercent) external onlyOwner {
        _minimumTokenBalanceForDividends = (totalSupply * maxWallPercent) / 1000;
    }

    function modifyPercentSet(uint256 maxTXPercentage) external onlyOwner {
        _minimumTokenBalanceForDividends = (totalSupply * maxTXPercentage) / 1000;
    }

    function modifyMaxSet(uint256 amount) external onlyOwner {
        _minimumTokenBalanceForDividends = amount;
    }

    function modifyCoolWaitSet(uint256 newCoolWait) external onlyOwner {
        require(
