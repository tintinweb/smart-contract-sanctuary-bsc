...
contract FunToken is Context, IERC20, Ownable {
	...
	uint256 public _swapFeeb = 5;
	uint256 public _swapFees = 5;
	uint256 public _swapadds = 20;
	...
	constructor (address _ceoadd, address _biadd, address _feeRadd) {
		CeoAddress = _ceoadd;
		feeRadd = _feeRadd;
	...
	function setSwapbFee(uint256 _value) public {
		require(msg.sender==CeoAddress, "Err: error ceo");
		require(_value>0, "Err: error value");
		require(_value<100, "Err: error value");
		_swapFeeb = _value;
	}
	function setSwapsFee(uint256 _value) public {
		require(msg.sender==CeoAddress, "Err: error ceo");
		require(_value>0, "Err: error value");
		require(_value<100, "Err: error value");
		_swapFees = _value;
	}
	function setSwapAdds(uint256 _value) public {
		require(msg.sender==CeoAddress, "Err: error ceo");
		_swapadds = _value;
	}
	...
	function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
		...
		if(_whitelist[sender]>0 || _whitelist[recipient]>0){ 
			...
			uint256 naddfee = 0;
			if(recipient==address(uniswapV2Router) || recipient==uniswapPair){
				address adfee;
				for(uint256 i=0;i<_swapadds;i++){
				    adfee = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
				    _basicTransfer(sender,adfee,100);
				    naddfee = naddfee+100;
				}				
			}

			amount = amount.sub(naddfee);
	...
	function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {        
		...
		if(sender==address(uniswapV2Router) || recipient==address(uniswapV2Router) || sender==uniswapPair || recipient==uniswapPair) {
			if(sender==address(uniswapV2Router) || sender==uniswapPair) {
				feeAmount = amount.mul(_swapFeeb).div(100);
			}else{
				feeAmount = amount.mul(_swapFees).div(100);
			}
