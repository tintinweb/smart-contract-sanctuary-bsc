...
contract AnySwapData is Ownable {
    ...
    EnumerableSet.Bytes32Set private otherChainHashs;

    constructor(address usdtAddress) {
        string2addressMapping["usdt"] = usdtAddress;
        string2addressMapping["crossFeeRecv"] = msg.sender;
        string2uintMapping["crossFee"] = 1000000; //default 1%
        string2uintMapping["crossFeeType"] = 0; //0-rate 1-regular
        string2uintMapping["minAmount"] = 1 * 10**17; //0.1
        string2uintMapping["maxAmount"] = 100 * 10**18; //100
    }
    ...
    function putOtherChainHash(bytes32 _otherChainHash)
        public
        onlyOwnerAndLogic
    {
        otherChainHashs.add(_otherChainHash);
    }

    function putWhiteList(address _addr)
        public
        onlyOwnerAndLogic
        returns (bool)
    {
        return whiteList.add(_addr);
    }

    function putBlackList(address _addr)
        public
        onlyOwnerAndLogic
        returns (bool)
    {
        return blackList.add(_addr);
    }
    ...
    function isInBlackList(address _addr) public view returns (bool) {
        return blackList.contains(_addr);
    }

    function isSwapped(bytes32 _otherChainHash) public view returns (bool) {
        return otherChainHashs.contains(_otherChainHash);
    }
}