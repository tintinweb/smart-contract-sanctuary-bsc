...
contract BackToTesting3 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BackToTesting3DividendTracker public dividendTracker;
    ...
    event TradingEnabled(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
                           uint256 setDelay);
    ...
    constructor() ERC20("BackToTesting3", "BTTT3") {
        ...
        dividendTracker = new BackToTesting3DividendTracker(payable(this), router, 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
                                                   "BackToTesting3TRACKER", "BTTT3TRACKER");
    ...
    function enableTrading(uint256 initialMaxGwei, uint256 initialMaxWallet, uint256 initialMaxTX,
        ...
        delay = setDelay;
        emit TradingEnabled(gasPriceLimit,maxWallet,maxTX,delay);
    }
...
contract BackToTesting3DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    BackToTesting3 public BackToTesting3Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        BackToTesting3Contract = BackToTesting3(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main BackToTesting3 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(BackToTesting3Contract);

            uint256 prevBalance = BackToTesting3Contract.balanceOf(address(this));
            ...
            {
                uint256 received = BackToTesting3Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    BackToTesting3Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
