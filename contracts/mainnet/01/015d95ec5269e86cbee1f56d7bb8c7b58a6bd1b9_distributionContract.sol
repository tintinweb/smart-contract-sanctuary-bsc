...
contract distributionContract is Ownable{
    ...
    function addTokens(address userAddress, uint256 amount1, uint256 amount2)  public onlyOwner{
        if(exists[userAddress]){
          delete lockedBalances[userAddress];
          delete releaseTimestamps[userAddress];
        }
        uint256 tokenTotal = amount1 + amount2;
        totalLockedTokens = totalLockedTokens + tokenTotal;
        if(amount1!=0){
          ...
          lockedBalances[userAddress].push(0);
          lockedBalances[userAddress].push(0);
          lockedBalances[userAddress].push(0);
          lockedBalances[userAddress].push(0);
          lockedBalances[userAddress].push(0);
