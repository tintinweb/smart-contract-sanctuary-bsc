...
contract NFTHelper  is Owner {
    ...
    function multiMint(address _recipient,uint256 _nftAmount,NFTType _type) external onlyOwner returns(uint256[] memory) {
          uint256[] memory nftIds = new uint256[](_nftAmount); 
    ...
    function setRecipient(address _tokenRecipientAddress) external onlyOwner {
        tokenRecipientAddress=_tokenRecipientAddress;
    }
