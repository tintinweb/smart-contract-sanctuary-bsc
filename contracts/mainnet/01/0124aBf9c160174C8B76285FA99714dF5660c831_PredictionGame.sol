...
contract PredictionGame is OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct BettingFeeInfo {
        uint feePercInPGT;
        uint feePercInBNB;
    }
    BettingFeeInfo public bettingFee;
    ...
    uint32  private constant INDEX_PGT = 1;
    ...
    function initialize(
        ...
        // The index must be equal to INDEX_PGT
        tokens.push(PGT);
        enabled.push(true);
        feeRates.push(100); // 1%

        // Of 10000, 300 = 3%
        bettingFee.feePercInPGT = 300;
        bettingFee.feePercInBNB = 500;
    }
    ...
    function betBearOnBehalf(uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin, address account) external payable {
        if (tokenIndex == INDEX_BNB) {
            tokenAmount = msg.value;
        } else {
            address tokenAddress = tokens[tokenIndex];
        ...
        _bet(account, tokenIndex, tokenAmount, amountOutMin, false);
    }
    ...
    function betBullOnBehalf(uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin, address account) external payable {
        if (tokenIndex == INDEX_BNB) {
            tokenAmount = msg.value;
        } else {
            address tokenAddress = tokens[tokenIndex];
        ...
        _bet(account, tokenIndex, tokenAmount, amountOutMin, true);
    }
    ...
    function _bet(address account, uint32 tokenIndex, uint256 tokenAmount, uint256 amountOutMin, bool isBull) internal {
        ...
        if (tokenIndex != INDEX_PGT) {
            uint fee = tokenAmount * bettingFee.feePercInBNB / 10000;
            tokenAmount -= fee;
            (bool success,) = admin.call{value: fee}("");
            require(success);
            tokenAmount = _swap(tokens[tokenIndex], PGT, tokenAmount, amountOutMin);
        } else {
            uint fee = tokenAmount * bettingFee.feePercInPGT / 10000;
            tokenAmount -= fee;
            IERC20Upgradeable(PGT).safeTransfer(jackpot, fee);
        }
    ...
    function donateTreasury(uint256 amount) external {
        IERC20Upgradeable(PGT).safeTransferFrom(msg.sender, address(this), amount);
    ...
    function claimTreasury(address payable _to, bool keepSeed) external onlyOwner {
        ...
        IERC20Upgradeable(PGT).safeTransfer(_to, amount);
    }
    ...
    function _claim(uint256 epoch, address user, uint256 reward, uint256 fee) internal {
        ...
        IERC20Upgradeable(PGT).safeTransfer(user, reward);
    ...
    function getPrincipalOut(uint32 tokenIndex, uint256 amountIn) public view returns (uint256) {
        ...
        uint[] memory amounts = router.getAmountsOut(amountIn, _getPath(tokenIn, PGT));
