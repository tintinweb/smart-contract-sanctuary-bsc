...
// File: contracts\OpensaleNFTLoot.sol

// Multiple Opensale Fixed OpensaleNFTLoot contract
...
contract OpensaleNFTLoot is ERC1155Holder, ERC721Holder {
    ...
    event OpensaleNFTLootStatus(bool boxStatus);
    event OwnerShipChanged(address newAccount);
    event OpensaleNFTLootNameChanged(string newName);
    event OpensaleNFTLootUriChanged(string newUri);
    ...
    function changePaymentAddress(address _receipt) external onlyOwner {
        require(
            _receipt != address(0x0),
            "Payment address cannot Zero address"
        );
        paymentAddress = _receipt;
    }

    function enableThisOpensaleNFTLoot() public onlyOwner {
        status = true;
        emit OpensaleNFTLootStatus(status);
    }

    function disableThisOpensaleNFTLoot() public onlyOwner {
        status = false;
        emit OpensaleNFTLootStatus(status);
    }
    ...
    function changeOpensaleNFTLootName(string memory name) public onlyOwner {
        boxName = name;
        emit OpensaleNFTLootNameChanged(name);
    }

    function changeOpensaleNFTLootUri(string memory _uri) public onlyOwner {
        boxUri = _uri;
        emit OpensaleNFTLootUriChanged(_uri);
    }
    ...
    function addTokenBatch(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (cardType == 0) {
                ...
                // ERC1155
                require(
                    IERC1155(collection).balanceOf(msg.sender, tokenId) >=
                        amount,
                    "You don't have enough Tokens"
                );
                IERC1155(collection).safeTransferFrom(
    ...
    function spin(uint256 times) public onlyHuman {
        ...
        require(
            paymentCollection.balanceOf(msg.sender, paymentTokenId) >= playFee,
            "You don't have enough Tokens"
        );
        paymentCollection.safeTransferFrom(
            msg.sender,
            paymentAddress,
            paymentTokenId,
            playFee,
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            if (_cards[cardKey].cardType == 0) {
                ...
                IERC1155(_cards[cardKey].collectionId).safeTransferFrom(
                    ...
                    _cards[cardKey].tokenId,
                    1,
                    "Your prize from OpensaleNFTLoot"
                );
            }
    ...
    function allCards() public view returns (Card[] memory cards) {
    ...
    modifier onlyHuman() {
        require(
            !isContract(address(msg.sender)) && tx.origin == msg.sender,
            "Only for human."
        );
        _;
    }
...
// File: contracts\OpensaleNFTLootFactory.sol

// Multiple OpensaleNFTLoot Factory contract

pragma solidity ^0.8.0;



contract OpensaleNFTLootFactory is Ownable {
    using SafeMath for uint256;

    /** Create OpensaleNFTLoot fee (BNB) */
    ...
    function createOpensaleNFTLoot(
        string memory _name,
        string memory _uri,
        address _paymentCollection,
        uint256 _paymentTokenId,
        ...
        bytes memory bytecode = type(OpensaleNFTLoot).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_name, _paymentCollection, block.timestamp)
        );
        address lootbox;
        assembly {
            lootbox := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        OpensaleNFTLoot(lootbox).initialize(
            _name,
            _uri,
            _paymentCollection,
            _paymentTokenId,
