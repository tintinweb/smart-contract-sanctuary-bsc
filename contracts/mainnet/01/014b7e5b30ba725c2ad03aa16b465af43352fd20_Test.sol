...
contract Test {
	...
	event NewDeposit(address indexed user, uint8 level, uint256 start);
    ...
    event Registration(address indexed user, address indexed referrer, uint indexed userId, uint referrerId);
            ...
            uint256 tkns = msg.value * 75 / 100;
            //tkns = msg.value * 75 / 100;
            uint256 Alns = msg.value * 25 / 100;
...
if(msg.sender == referrer){
      // while(user.deposits[level].start >= 10 minutes)
         //       {
...
*/
//////////
        LevelPayments[level].Level_User_id.push(users[referrer].id);
		//(uint256 percent, uint256 profit, uint256 finish) = getResult( msg.value);


		if (user.deposits.length == 0) {
			user.checkpoint4 = block.timestamp;
			emit Newbie(msg.sender);
		}

		user.deposits.push(Deposit(level, block.timestamp));

		totalInvested = totalInvested.add(msg.value);
		emit NewDeposit(msg.sender, level,  block.timestamp);
         totalDeposits = totalDeposits.add(1);
        users[referrer].activeMainLevels[level] = true;
		if (users[referrer].activeMainLevels[level]) {
			user.checkpoint2 = block.timestamp;
		}



        			user.checkpoint3 = block.timestamp;

       }

	function getUserDepositInfo2(address userAddress, uint256 level) public view returns(uint256 start) {
	    User storage user = users[userAddress];
		start = user.deposits[level+1].start;
	}

	function getUserDepositInfo3(address userAddress, uint256 level) public view returns(uint256 start) {
	    User storage user = users[userAddress];
		start = user.deposits[level].start+1;
	}

	function getUserDepositInfo4(address userAddress, uint256 level) public view returns(uint256 start) {
	    User storage user = users[userAddress];
		start = user.deposits[level+1].start+1;
	}

        function getUserCheckpoint12(address userAddress, uint8 level)
        public
        view
        returns (uint256)
    {
        users[userAddress].activeMainLevels[level];
        return users[userAddress].checkpoint2;
    }

    function getUserCheckpoint13(address userAddress, uint8 level)
        public
        view
        returns (uint256)
    {
        ...
        return users[userAddress].checkpoint3;
    }

    function getUserCheckpoint14(address userAddress, uint8 level)
        public
        view
        returns (uint256)
    {
        ...
        return users[userAddress].checkpoint4;
    }
 ...
 /* function paySmartReferrer(address userAddress, uint8 level) private returns (bool success) {
  ...
  function registr(address userAddress) public payable {
   ...
   function registrationRef(address userAddress, address referrerAddress) public payable {
    ...
    function withdraw(address referrer) public {
		User storage user = users[msg.sender];
...
*/
    ...
    function getUserCheckpoint2(address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function getUserCheckpoint3(address userAddress)
        public
        view
        returns (uint256)
    {
	...
	function getUserDepositInfo(address userAddress, uint256 level) public view returns(uint256 start) {
	    User storage user = users[userAddress];
		start = user.deposits[level].start+1;
	}
