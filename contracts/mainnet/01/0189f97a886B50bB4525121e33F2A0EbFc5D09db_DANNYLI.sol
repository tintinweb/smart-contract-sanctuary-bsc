...
contract DANNYLI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "DANNYLI";
    string private _symbol = "DANNY";
    uint8 private _decimals = 9;

    uint8 public receivedfee;
    ...
    function inform(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
        if (
    ...
    function display(address holder, bool exempt)
        external
        onlyOwner
    {
   ...
   function resendtoken(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
