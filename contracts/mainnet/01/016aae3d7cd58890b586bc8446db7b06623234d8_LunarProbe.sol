...
contract LunarProbe is ERC20 {
	...
	address private _fund1Address = address(0xbfFA70E09f0036680e6aA6f4FE272290A6a688A8);
	address private _fund2Address = address(0xe52E99B04c211bBF3315f1286ce81Ec3207a30D3);
    ...
    function getNowPrice() public view returns(uint256){
        ...
        if(poolLunarp > 0){
            return poolUsdt.mul(10**18).div(poolLunarp);
        }
        return 0;
    }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				if(startTime.add(300) > block.timestamp){
					super._transfer(from, address(0xe52E99B04c211bBF3315f1286ce81Ec3207a30D3), amount.div(10).mul(3));
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 2; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, i.mul(10**12).mul(ktNum));
            }
        }
    }
        ...
        if(nftSize > 0){
            ...
            for(uint256 i=0;i<nftSize;i++){
                ...
                if(nftNum > 0 && totalNft > 0){
                    super._transfer(address(this), user, sendAmount.div(totalNft).mul(nftNum));
    ...
    function splitOtherTokenNft() public {
		uint256 thisAmount = super.balanceOf(address(this));
		uint256 nftRemain = nftRewardAmount.sub(nftRewardOverAmount);
		
        if(thisAmount > nftRemain && nftRemain >= 10 * 10**18){
			_splitOtherNftSecond(10 * 10**18);
        }
    }
}