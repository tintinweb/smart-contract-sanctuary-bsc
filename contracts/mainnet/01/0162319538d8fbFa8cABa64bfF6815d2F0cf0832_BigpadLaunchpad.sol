...
contract BigpadLaunchpad {
    using SafeMath for uint256;

    IUniswapV2Router02 private constant uniswapRouter =IUniswapV2Router02(address(0x10ED43C718714eb63d5aA57B78B54704E256024E));
    ...
    bool public uniLiquidityAdded = false; // if true, liquidity is added only in Uniswap
    bool public onlyWhitelistedAddressesAllowed = true; // if true, only whitelisted addresses can invest
    bool public bigpadDevFeesExempted = false; // if true, presale will be exempted from fees
    ...
    constructor(address _bigpadFactoryAddress, address _bigpadDevAddress) public {
        require(_bigpadFactoryAddress != address(0));
        require(_bigpadDevAddress != address(0));

        bigpadFactoryAddress = payable(_bigpadFactoryAddress);
        bigpadDevAddress = payable(_bigpadDevAddress);
    }

    modifier onlybigpadDev() {
        require(bigpadFactoryAddress == msg.sender || bigpadDevAddress == msg.sender);
        _;
    }

    modifier onlybigpadFactory() {
        require(bigpadFactoryAddress == msg.sender);
        _;
    }

    modifier onlyPresaleCreatorOrbigpadFactory() {
        require(
            presaleCreatorAddress == msg.sender || bigpadFactoryAddress == msg.sender,
    ...
    ) external onlyPresaleCreatorOrbigpadFactory {
    ...
    function setbigpadInfo(
        address _bigpadLiqLockAddress,
        uint256 _bigpadDevFeePercentage,
        uint256 _bigpadId
    ) external onlybigpadDev {
        bigpadLiqLockAddress = _bigpadLiqLockAddress;
        bigpadDevFeePercentage = _bigpadDevFeePercentage;
        bigpadId = _bigpadId;
    }

    function setbigpadDevFeesExempted(bool _bigpadDevFeesExempted)
    external
    onlybigpadDev
    {
        bigpadDevFeesExempted = _bigpadDevFeesExempted;
    }

    function setOnlyWhitelistedAddressesAllowed(bool _onlyWhitelistedAddressesAllowed)
    external
    onlyPresaleCreatorOrbigpadFactory
    {
    ...
    function addLiquidityAndLockLPTokens() external presaleIsNotCancelled {
        ...
        uint256 bigpadDevFeeInWei;
        if (!bigpadDevFeesExempted) {
            uint256 pctDevFee = finalTotalCollectedWei.mul(bigpadDevFeePercentage).div(100);
            bigpadDevFeeInWei = pctDevFee;
        }
        if (bigpadDevFeeInWei > 0) {
            finalTotalCollectedWei = finalTotalCollectedWei.sub(bigpadDevFeeInWei);
            bigpadDevAddress.transfer(bigpadDevFeeInWei);
        }
        ...
        presaleCreatorClaimTime = block.timestamp + 3 days;
    }
    ...
    function cancelAndTransferTokensToPresaleCreator() external {
        if (!uniLiquidityAdded && presaleCreatorAddress != msg.sender && bigpadDevAddress != msg.sender) {
            revert();
        }
        if (uniLiquidityAdded && bigpadDevAddress != msg.sender) {
            revert();
        }
