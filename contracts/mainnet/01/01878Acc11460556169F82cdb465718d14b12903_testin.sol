...
contract testin {
	...
	uint256[] public REFERRAL_PERCENTS 	= [500, 200, 100, 100, 100];
	uint256[] public SEED_PERCENTS 		= [500, 200, 100, 100, 100];
	uint256 constant public PROJECT_FEE = 333;
    uint256 constant public SEC_FEE = 100;
	...
	constructor(address payable wallet, address payable wallet2, address payable wallet3, address payable wallet4) {
		require(!isContract(wallet));
        require(!isContract(wallet2));
        require(!isContract(wallet3));
        require(!isContract(wallet4));
		a = wallet;
        b = wallet2;
        c = wallet3;
        d = wallet4;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		User storage feeUser = users[a];
        feeUser.bonus = feeUser.bonus.add(fee);
		feeUser.totalBonus = feeUser.totalBonus.add(fee);
        User storage bfeeUser = users[b];
        bfeeUser.bonus = bfeeUser.bonus.add(fee);
		bfeeUser.totalBonus = bfeeUser.totalBonus.add(fee);
        User storage cfeeUser = users[c];
        cfeeUser.bonus = cfeeUser.bonus.add(fee);
		cfeeUser.totalBonus = cfeeUser.totalBonus.add(fee);
    ...
    function reinvest(address referrer, uint8 plan) public {
        User storage user = users[msg.sender];
		...
		emit NewDeposit(msg.sender, plan, totalAmount);
    }
