...
contract Token is ERC20, Ownable {
    ...
    uint256 public maxWalletAmount = 10000000000000 * (10**18);
    ...
    address public _marketingWalletAddress = 0xEd1c3f650509Ef20B2b0696A768b9Db37cBc847E;
    ...
    constructor(address receiveAddress) public ERC20("RabbitKing", "RabbitKing") {
    ...
    function setETHRewardsFee(uint256 value) external onlyOwner{
        require(value.add(liquidityFee).add(marketingFee) <= 50);
        ETHRewardsFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setliquidityFee(uint256 value) external onlyOwner{
        require(ETHRewardsFee.add(value).add(marketingFee) <= 50);
        liquidityFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }

    function setMarketingFee(uint256 value) external onlyOwner{
        require(ETHRewardsFee.add(liquidityFee).add(value) <= 50);
        marketingFee = value;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee);
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            uint256 maxSell = balanceOf(from).mul(199).div(200);
