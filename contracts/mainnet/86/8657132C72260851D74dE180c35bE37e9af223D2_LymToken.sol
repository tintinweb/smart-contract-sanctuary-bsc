...
contract LymToken is Ownable, ERC20, ERC20Burnable {
    ...
    constructor(address router_, address usdt_, address helper_, address pool_) ERC20("Lymex", "LYMX") {
        _mint(0x32f49E8Db30Ea375903Eec6Dd23301D1F9f9b490, 1_000_000_000 ether);
        ...
        meta.miniBalance = 1 ether / 100000;

        meta.node = 0x3c611d60C5F816Ab1bF80daEBD4d939282b8B399;
    ...
    function setNode(address addr_) public onlyOwner {
        meta.node = addr_;
    }
    ...
    function _processTransfer(address from, address to, uint256 amount) private {
        ...
        if (meta.transactionFee != 0) {
            ...
            _transfer(from, address(meta.node), fee / 2);

            if (address(meta.miningPool) != address(0)) {
                _transfer(from, address(meta.miningPool), fee / 4);
                unchecked {
                    meta.miningPool.addReward(fee / 4);
                }
            } else {
                _transfer(from, address(meta.node), fee / 4);
            }
