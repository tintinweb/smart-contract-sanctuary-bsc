...
contract UnlimitedTokenAntiBot is IERC20, Ownable {
    using SafeMath for uint256;

    mapping(address => string) public _rolebase;
    ...
    constructor(
        ...
        whaleAntiBot = IWhaleAntiBot(whaleAntiBot_);
        // Enable using whaleAntiBot in this contract
        antiBotEnabled = false;
        _whaleAntiBot = whaleAntiBot_;
        _feeEnableAntiBot = feeEnableAntiBot_;
    }

    modifier onlyAdmin() {
        require(
            keccak256(abi.encodePacked(_rolebase[_msgSender()])) ==
    ...
    function setUsingAntiBot(bool enabled_) external payable onlyOwner {
        uint8 checkAddedLiquiAndCheckSetBlockNum = whaleAntiBot
            .checkAddedLiquidityAndCheckSetBlockNum(owner());
        ...
        if (checkAddedLiquiAndCheckSetBlockNum == 2) {
            bool addedLiquiAndSetBlockNum = whaleAntiBot
                .setAddedLiquidityAndSetBlockNum(owner());

            if (addedLiquiAndSetBlockNum == true) {
                // Check payfee at first enable Antibot
                bool hasPayFee = whaleAntiBot.checkHasPayFee(owner());
                if (hasPayFee == false) {
                    require(
                        msg.value == _feeEnableAntiBot,
                        "Fee Service incorrect"
                    );
                    whaleAntiBot.settingPayFee(owner());
    ...
    // decimal value usually used value of 18
    ...
    /**
        ...
        sender must have a balace of at least amount
        caller must have allowance for sender' token of at least amount
     */
    ...
    function _transfer(
        ...
        if (antiBotEnabled) {
            ...
            whaleAntiBot.onPreTransferCheck(sender, recipient, amount, owner());
        }
    ...
    // Just Owner can set Admin
    function setRoleAdmin(address account) public onlyOwner returns (bool) {
        // require account != OWNER
        require(owner() != account, "Cant set OWNER to ADMIN");
        _rolebase[account] = "ADMIN";
        return true;
    }

    // Just Owner or Admin can set Minter
    function setRoleMinter(address account) public returns (bool) {
        require(
            owner() == _msgSender() ||
                keccak256(abi.encodePacked(_rolebase[_msgSender()])) ==
                keccak256(abi.encodePacked("ADMIN")),
        ...
        // require account != OWNER
        require(owner() != account, "Cant set OWNER to MINTER");

        // require account != ADMIN
        require(
            keccak256(abi.encodePacked(_rolebase[account])) !=
                keccak256(abi.encodePacked("ADMIN")),
            "Cant set ADMIN to MINTER"
        );

        _rolebase[account] = "MINTER";
        return true;
    }

    function deleteRoleMinter(address account) public returns (bool) {
        // require account != OWNER
        require(owner() != account, "Cant delete OWNER");
    ...
    function deleteRoleAdmin(address account) public onlyOwner returns (bool) {
        // require account != OWNER
        require(owner() != account, "Cant delete OWNER");
    ...
    function getRoleBase(address account) public view returns (string memory) {
        return _rolebase[account];
    }
}
