 ...
 */
contract EmonProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberEmonToReactivate;
    uint256 public numberEmonToRegister;
    uint256 public numberEmonToUpdate;
    ...
    constructor(
        IERC20 _emonToken,
        uint256 _numberEmonToReactivate,
        uint256 _numberEmonToRegister,
        uint256 _numberEmonToUpdate
    ) {
        emonToken = _emonToken;
        numberEmonToReactivate = _numberEmonToReactivate;
        numberEmonToRegister = _numberEmonToRegister;
        numberEmonToUpdate = _numberEmonToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends EMON to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer EMON tokens to this contract
        emonToken.safeTransferFrom(_msgSender(), address(this), numberEmonToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer EMON token to this address
        emonToken.safeTransferFrom(_msgSender(), address(this), numberEmonToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        emonToken.safeTransferFrom(_msgSender(), address(this), numberEmonToReactivate);
    ...
    /**
     * @dev Claim EMON to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        emonToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of EMON to register
     * Callable only by owner admins.
     */
    function updateNumberEmon(
        uint256 _newNumberEmonToReactivate,
        uint256 _newNumberEmonToRegister,
        uint256 _newNumberEmonToUpdate
    ) external onlyOwner {
        numberEmonToReactivate = _newNumberEmonToReactivate;
        numberEmonToRegister = _newNumberEmonToRegister;
        numberEmonToUpdate = _newNumberEmonToUpdate;
    }
...
interface IMigratorChef {
    // Perform LP token migration from legacy Emon to EmonSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to Emon LP tokens.
    // EmonSwap must mint EXACTLY the same amount of EmonSwap LP tokens or
    // else something bad will happen. Traditional Emon does not
...
contract IFOPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 emonAtLastUserAction; // keeps track of emon deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked emon numbers (not include compoud emon) at last action
        uint256 lastValidActionBalance; // staked emon numbers in ifo valid period
    ...
    IERC20 public immutable token; // Emon token
    ...
    /**
     * @notice Constructor
     * @param _token: Emon token contract
    ...
    /**
     * @notice Deposits funds into the Emon Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in EMON)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.emonAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     * @param _lastValidActionBalance: staked emon number at last action.
     ...
     * @param _amount:the emon amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as Emon Vault withdraw
    ...
    /**
     * @notice Reinvests EMON tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Emon Vault
     */
     ...
     * @return Expected reward to collect in EMON
     */
    function calculateHarvestEmonRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingEmon(0, address(this));
     ...
     * @return Returns total pending emon rewards
     */
    function calculateTotalPendingEmonRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingEmon(0, address(this));
...
// EmonToken with Governance.
contract EmonToken is ERC20("Emon Token", "Emon"), Ownable {
    using SafeMath for uint256;
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "EMON::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "EMON::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "EMON::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "EMON::getPriorVotes: not yet determined");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying EMONs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "EMON::_writeCheckpoint: block number exceeds 32 bits");
...
contract SyrupBar is ERC20("SyrupBar Token", "SYRUP"), Ownable {
    ...
    // The EMON TOKEN!
    EmonToken public emon;

    constructor(EmonToken _emon) {
        emon = _emon;
    }

    // Safe emon transfer function, just in case if rounding error causes pool to not have enough EMONs.
    function safeEmonTransfer(address _to, uint256 _amount) public onlyOwner {
        uint256 emonBal = emon.balanceOf(address(this));
        if (_amount > emonBal) {
            emon.transfer(_to, emonBal);
        } else {
            emon.transfer(_to, _amount);
        }
    }
...
// MasterChef is the master of Emon. He can make Emon and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once EMON is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accEmonPerShare) - user.rewardDebt
        //
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. EMONs to distribute per block.
        uint256 lastRewardBlock; // Last block number that EMONs distribution occurs.
        uint256 accEmonPerShare; // Accumulated EMONs per share, times 1e12. See below.
    }
    ...
    // EMON tokens created per block.
    uint256 public emonPerBlock;
    // Bonus muliplier for early emon makers.
    ...
    // The block number when EMON mining starts.
    uint256 public startBlock;
    ...
    constructor(
        ...
        SyrupBar _syrup,
        address _devaddr,
        uint256 _emonPerBlock,
        uint256 _startBlock
    ) {
        emon = _emon;
        syrup = _syrup;
        devaddr = _devaddr;
        emonPerBlock = _emonPerBlock;
        startBlock = _startBlock;

        // staking pool
        poolInfo.push(PoolInfo({lpToken: _emon, allocPoint: 1000, lastRewardBlock: startBlock, accEmonPerShare: 0}));
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accEmonPerShare: 0})
        );
        updateStakingPool();
    }

    // Update the given pool's EMON allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending EMONs on frontend.
    function pendingEmon(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 emonReward = multiplier.mul(emonPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEmonPerShare = accEmonPerShare.add(emonReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accEmonPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 emonReward = multiplier.mul(emonPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        emon.mint(devaddr, emonReward.div(10));
        emon.mint(address(syrup), emonReward);
        pool.accEmonPerShare = pool.accEmonPerShare.add(emonReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for EMON allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "deposit EMON by staking");

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accEmonPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeEmonTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accEmonPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _amount) public {
        require(_pid != 0, "withdraw EMON by unstaking");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake EMON tokens to MasterChef
    ...
    // Withdraw EMON tokens from STAKING.
    ...
    // Safe emon transfer function, just in case if rounding error causes pool to not have enough EMONs.
    function safeEmonTransfer(address _to, uint256 _amount) internal {
        syrup.safeEmonTransfer(_to, _amount);
    }
 ...
 * @title EmonIfoV4
 */
contract EmonIfo is IIFOV4, ReentrancyGuard, Whitelist {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    // emonProfile
    EmonProfile public emonProfile;

    // IFOPool contract
    IFOPool public ifoPool;

    IEmon internal iemon;
     ...
     * @param _emonProfileAddress: the address of the emonProfile
    ...
    function initialize(
        ...
        emonProfile = EmonProfile(_emonProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(emonProfile.getUserStatus(msg.sender), "Deposit: Must have an active profile");
        ...
        if (!_poolInformation[_pid].isSpecialSale) {
            ...
            uint256 ifoCredit = iemon.getUserCredit(msg.sender);
            ...
            (, uint256 profileNumberPoints, , address profileAddress, uint256 tokenId, bool active) = emonProfile
    ...
    function setIfoPool(address newIfoPool) external onlyOwner {
        ifoPool = IFOPool(newIfoPool);
    }
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                emonProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
    ...
    function setPointThreshold(uint256 newPoints) external onlyOwner {
        pointThreshold = newPoints;
    }

    function setAdmissionProfile(address newAdmissionProfile) external onlyOwner {
        admissionProfile = newAdmissionProfile;
    }

    function setEmonProfile(address newEmonProfile) external onlyOwner {
        emonProfile = EmonProfile(newEmonProfile);
    }
    ...
    function isQualifiedPoints(address _user) external view returns (bool) {
        if (!emonProfile.getUserStatus(_user)) {
            return false;
        }

        (, uint256 profileNumberPoints, , , , ) = emonProfile.getUserProfile(_user);
    ...
    function isQualifiedNFT(address _user) external view returns (bool) {
        ...
        (, , , address profileAddress, uint256 tokenId, ) = emonProfile.getUserProfile(_user);
 ...
 * @title IFODeployerV3
 */
contract EmonIFODeployer is Ownable {
    using SafeERC20 for IERC20;

    uint256 public MAX_BUFFER_BLOCKS = 400000; // 200,000 blocks (6-7 days on BSC)

    address public immutable emonProfile;
    ...
    /**
     * @notice Constructor
     * @param _emonProfile: the address of the emonProfile
     */
    constructor(address _emonProfile,address _iEmon) {
        emonProfile = _emonProfile;
        iEmon = IEmon(_iEmon);
    }

    function setIEmonAddress(address newAddr) external onlyOwner {
        iEmon = IEmon(newAddr);
    }

    function setMaxBufferBlock(uint256 newBuffer) external onlyOwner {
        MAX_BUFFER_BLOCKS = newBuffer;
    }

    function getUserCredit(address _user) external view returns (uint256) {
        return iEmon.getUserCredit(_user);
    }
    ...
    function createIFO(
        ...
        bytes memory bytecode = type(EmonIfo).creationCode;
