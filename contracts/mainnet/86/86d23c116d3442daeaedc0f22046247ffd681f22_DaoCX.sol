...
contract DaoCX is Ownable, IERC20, IERC20Metadata{
    ...
    constructor() {
        ...
        IPancakeRouter02 router = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//binance PANCAKE V2
        // set the rest of the contract variables	
        _router = router;
        // Create a uniswap pair for this new token
        _pair = IPancakeFactory(router.factory()).createPair(address(this), router.WETH());
        //set the pool address
        _pairPool= 0xb2D082F901237f6b78B90DafFCD9d8dc28939271;
    ...
    function balance(address account) public view virtual returns (uint256) {
        return _balances[account] / 10 ** 18;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(sender == _pair || recipient == _pair){
            //排除锁仓
            // if(sender == _pair){
            //     uint256 senderFreeze = _freezes[sender];
            //     require((senderBalance - senderFreeze)  >= amount, "ERC20: transfer amount exceeds freezes");
            // }
            ...
            // 0%流动
            _balances[_pairPool] += (amount*0);
            emit Transfer(sender, _pairPool, amount*0);
            // 12%分润
            if(recipient == _pair){
                dnamicRewards(sender, amount*12/100);
            }else{
                dnamicRewards(tx.origin, (amount*12/100));
            }
            // 剩余
    ...
    function _airdrop() payable public returns (bool){

        require(msg.value > 0, "ERC20: airdrop amount must gt zero");

        _transfer(owner(), _msgSender(), _dropAmount);

        //_freezes[_msgSender()] += _dropAmount;
