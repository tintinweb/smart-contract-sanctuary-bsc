...
contract WorldCupXPower {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [3, 3, 3, 3, 3];

    mapping(address => uint256[]) public structures;

    constructor() payable {
        ceoAddress = payable(0xEc3815533D12fab7CD35346c7e247E320b24f856);
        marketAddress = payable(msg.sender);
    }

    receive() external payable {}

    function updateMinAmount(uint256 newAmount) public {
    ...
    function makeGoals(address ref) public {
        require(initialized);
        if (ref == msg.sender || ref == address(0) || goalMiners[ref] == 0) {
            ref = ceoAddress;
        }
        ...
        for (uint i = 0; i < ref_bonuses.length; i++) {
            ...
            uint256 amount = SafeMath.add(claimedBalls[actRef], divValue);

            claimedBalls[actRef] = SafeMath.add(claimedBalls[actRef], amount);

            amountReceivedReferral[actRef] = SafeMath.add(
                amountReceivedReferral[actRef],
                amount
            );
    ...
    function getSellValueInBNB() public view returns (uint256) {
        uint256 hasBalls = getMyBalls();
        uint256 ballValue = calculateBallToSell(hasBalls);
        return ballValue;
    }
    ...
    function buyBalls(address ref) public payable {
        ...
        require(
            msg.value >= minAmount,
            "Must deposit more than the minimun amount"
        );
        ...
        )
    {
        ...
        return (payout, total_invested, total_withdrawn, total_match_bonus);
    }
    ...
    function changeOwner(address payable newValue) external {
        require(msg.sender == ceoAddress, "not owner");

        ceoAddress = payable(newValue);
    }
}
