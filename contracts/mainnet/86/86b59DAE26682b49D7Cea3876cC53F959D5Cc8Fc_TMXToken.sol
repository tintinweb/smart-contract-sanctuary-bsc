...
interface ITMX {
    ...
    function setFather(address father) external returns (bool);
}

contract TMXToken is ERC20, ITMX {
    ...
    address public marketingAddr = address(0x30a7017BE1536B2b11dBfC50F8a08BEcE7758F2C);  // Market
    address public firstInviteAddr = address(0xD7f625721102D850b3796528c7a1C0D79d3Ee1eb);  // First Invite

    mapping (address => address) public inviteMap; 
    address[] public inviteKeys;  

    uint256[] public inviteRewardConfig = [3,2];  // div 13
    ...
    address public tmxsToken;

    constructor() ERC20("TMX China", "TMX") {
        ...
        _mint(address(0x489D839423DD19094c236a7F410C29F63Dd5CcA0), 36666 * 1e18);
    }

    function setTmxsToken(address _tmxsToken) public onlyOwner {
        tmxsToken = _tmxsToken;
    }
    ...
    function setFather(address self, address father) public override returns (bool) {
        require(_msgSender() == tmxsToken, "No Permition");

        return setFatherPrivate(self, father);
    }

    function setFather(address father) public override returns (bool) {
    ...
    function reward(address _from, address _user, bool _isBuy, uint256 _amount) private {
        ...
        if (_isBuy) {  
            ...
            super._transfer(_from, marketingAddr, _amount - rewardInviteAmt);
        } else {   
            ...
            TransferHelper.safeTransfer(USDTAddr, marketingAddr, usdtAmt - rewardInviteAmt);
        }
    }
