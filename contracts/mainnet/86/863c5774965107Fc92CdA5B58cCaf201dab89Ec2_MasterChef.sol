...
// will be transferred to a governance smart contract once DOGBALL is sufficiently
...
contract MasterChef is Ownable, Multicall {
    ...
    // The DOGBALL TOKEN!
    IFungibleToken public immutable farmToken;
    // Dev address.
    address public devaddr;
    // DOGBALL tokens created per block.
    uint256 public miningSpeed;
    // Bonus muliplier for early farmToken makers.
    ...
    // The block number when DOGBALL mining starts.
    ...
    constructor(
        IFungibleToken _farmToken,
        address _devaddr,
        uint256 _miningSpeed,
        uint256 _startBlock
    ) {
        require(
            address(_farmToken) != address(0) &&
            _devaddr != address(0),
    ...
    // Update the given pool's DOGBALL allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
        ...
        returns (uint256 reward)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 umReward = multiplier * miningSpeed * pool.allocPoint / totalAllocPoint;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 umReward = multiplier * miningSpeed * pool.allocPoint / totalAllocPoint;
        farmToken.mint(devaddr, umReward * 12 / 100);
        farmToken.mint(address(this), umReward);
        pool.accTokensPerShare = pool.accTokensPerShare + (umReward * 1e12 / lpSupply);
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DOGBALL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            _rewardTokenTransfer(msg.sender, pending);
        }
        if (_amount != 0) {
    ...
    // Safe farmToken transfer function, just in case if rounding error causes pool to not have enough TOKENs.
    function _rewardTokenTransfer(address _to, uint256 _amount) internal {
        uint256 tokenBalance = farmToken.balanceOf(address(this));
        if (_amount > tokenBalance) {
            farmToken.safeTransfer(_to, tokenBalance);
        } else {
            farmToken.safeTransfer(_to, _amount);
        }
    }
