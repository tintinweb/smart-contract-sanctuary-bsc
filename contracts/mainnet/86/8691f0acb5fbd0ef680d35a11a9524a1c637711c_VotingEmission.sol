...
import "../interfaces/IVotingEmission.sol";
import "../interfaces/IVotingEscrow.sol";
import "../library/CoreUtility.sol";
import "../library/SafeDecimalMath.sol";
import "../library/System.sol";

contract VotingEmission is IVotingEmission, OwnableUpgradeable, CoreUtility {
    /// @dev Reserved storage slots for future base contract upgrades
    uint256[192] private _reservedSlots;
    using SafeMath for uint256;
    using SafeDecimalMath for uint256;

    event PoolReplaced(address oldPool, address newPool);
    event PoolAdded(address pool);
    event WeightUpdated(address indexed pool, uint256 indexed timestamp, uint256 weight);
    event Voted(
    ...
    mapping(uint256 => mapping(address => uint256)) public override weightsPerWeek; // timestamp (end of week) => address of pool => weight

    function initialize(address votingEscrow_, address operator_) public initializer {
        __Ownable_init();
        votingEscrow = IVotingEscrow(votingEscrow_);
        operator = operator_;
        _maxTime = IVotingEscrow(votingEscrow_).getMaxtime();
    }

    function initializeEmission(uint256 guardedLaunchStart_, uint256 lastTimestamp_) public onlyOwner {
        require(
            _endOfWeek(guardedLaunchStart_) == guardedLaunchStart_ + System.SYSTEM_INTERVAL,
            "must be end of a week"
        );
        guardedLaunchStart = guardedLaunchStart_;
        require(lastTimestamp == 0, "Already initialized");
        require(
            _endOfWeek(lastTimestamp_) == lastTimestamp_ + System.SYSTEM_INTERVAL &&
                lastTimestamp_ >= guardedLaunchStart - System.SYSTEM_INTERVAL
        );
        uint256 size = poolSize;
        uint256 weights0;
        uint256 weights1;
        for (uint256 i = 0; i < size; i++) {
            weights0 = weights0.add(weightsPerWeek[lastTimestamp_ + System.SYSTEM_INTERVAL][_pools[i]]);
            weights1 = weights1.add(weightsPerWeek[lastTimestamp_][_pools[i]]);
        }
        require(weights0 == 0, "Next week already updated");
        if (lastTimestamp_ >= guardedLaunchStart) {
            require(weights1 > 0, "Last week not updated");
        }
        lastTimestamp = lastTimestamp_;
    }

    function setWeight(uint256[] memory weights_) public {
        require(msg.sender == operator, "Only operator");
        require(weights_.length == poolSize, "Invalid number of weights");
        uint256 totalWeight = 0;
        for (uint256 i = 0; i < poolSize; i++) {
            totalWeight = totalWeight.add(weights_[i]);
        }
        require(totalWeight == 1e18, "Invalid weights");
        for (uint256 i = 0; i < poolSize; i++) {
            _fixedWeight[i] = weights_[i];
        }
    }
    ...
    function addPool(address newPool) external {
        require(msg.sender == operator, "Only operator");
        require(!poolExisted(newPool), "pool has existed");
        uint256 size = poolSize;
        _pools[size] = newPool;
        poolSize = size + 1;
    ...
    function poolExisted(address pool) internal view returns (bool) {
        uint256 psize = poolSize;
        for (uint256 i = 0; i < psize; i++) {
            if (_pools[i] == pool) {
                return true;
            }
        }
    ...
    function sumAtTimestamp(address pool, uint256 timestamp) public view returns (uint256) {
        ...
        for (
            ...
            sum = sum.add(poolScheduledUnlock[pool][weekCursor].mul(weekCursor - timestamp) / _maxTime);
        }
        return sum;
    }

    function getUserWeights(address account) public override view returns (uint256[] memory) {
        uint256 size = poolSize;
    ...
    function getWeights(uint256 timestamp)
        public
        view
        override
    ...
    function setFirstWeights() external override {
        //require(block.timestamp < guardedLaunchStart, "Too late");
        require(msg.sender == operator, "Only operator");
        uint256 lastTimestamp_ = guardedLaunchStart - System.SYSTEM_INTERVAL;
        uint256 weights0;
        for (uint256 i = 0; i < poolSize; i++) {
            weights0 = weights0.add(weightsPerWeek[lastTimestamp_][_pools[i]]);
        }
        require(weights0 == 0, "This week already updated");
        for(uint256 i = 0; i < poolSize; i++) {
            weightsPerWeek[lastTimestamp_][_pools[i]] = _fixedWeight[i];
        }
    }

    function getPoolRelativeWeight(address pool, uint256 timestamp) external override returns (uint256) {
        require(timestamp <= block.timestamp, "Too soon");
        if (timestamp < guardedLaunchStart) {
            return weightsPerWeek[guardedLaunchStart - System.SYSTEM_INTERVAL][pool];
        }
        uint256 weekTimestamp = _endOfWeek(timestamp).sub(System.SYSTEM_INTERVAL);
        uint256 lastTimestamp_ = lastTimestamp; // gas saver
        require(weekTimestamp <= lastTimestamp_ + System.SYSTEM_INTERVAL, "Previous week is empty");
        if (weekTimestamp <= lastTimestamp_) {
            return weightsPerWeek[weekTimestamp][pool];
        }
        lastTimestamp = lastTimestamp_ + System.SYSTEM_INTERVAL;
        return _updatePoolWeight(weekTimestamp, pool);
    }

    function _updatePoolWeight(uint256 weekTimestamp, address pool) private returns (uint256 weight) {
        (uint256[] memory poolWeights, address[] memory pools) = getWeights(weekTimestamp);
        uint256 totalWeight;
        for (uint256 i = 0; i < poolWeights.length; i++) {
            weightsPerWeek[weekTimestamp][pools[i]] = poolWeights[i];
            emit WeightUpdated(pools[i], weekTimestamp, poolWeights[i]);
            if (pools[i] == pool) {
                weight = poolWeights[i];
            }
            totalWeight = totalWeight.add(poolWeights[i]);
        }
    ...
    function cast(uint256[] memory weights) external {
        ...
        IVotingEscrow.LockedBalance memory lockedBalance = votingEscrow.getLockedBalance(msg.sender);
        require(lockedBalance.amount > 0 && lockedBalance.unlockTime > block.timestamp, "No veGoen");
...
interface IVotingEmission {
    function poolSize() external view returns(uint256);
    function getPools() external view returns (address[] memory);
    function guardedLaunchStart() external view returns (uint256);
    function lastTimestamp() external view returns (uint256);
    function weightsPerWeek(uint256 week, address pool) external view returns (uint256);
    function getUserWeights(address account) external view returns (uint256[] memory weights);
    function setFirstWeights() external;
    ...
    function getPoolRelativeWeight(address pool, uint256 timestamp) 
        external
        returns (uint256);
...
library System {
    ...
    address constant GOEN = 0xc52109393ca4b9eAFEa9Bc21A428c93Aad6cFBb1;
