...
contract NFTSales is Ownable, NFTType, ReentrancyGuard, Proxyable {
    ...
    uint256 public uncratePrice = 250000000000000000;
...
abstract contract NFTType is Ownable, ERC721Base {
    ...
    {
        if (tokenIDs.length > maxNFTsToParse)
            revert tooManyIDs({sent: tokenIDs.length, max: maxNFTsToParse});
    ...
    function getNFTTypesForUser(address user) external view returns(uint32[] memory) {
        ...
        if (bal > maxNFTsToParse) revert tooManyIDs({sent: bal, max: maxNFTsToParse});
