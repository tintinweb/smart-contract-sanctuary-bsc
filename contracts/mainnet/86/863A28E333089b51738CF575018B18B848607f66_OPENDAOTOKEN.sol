...
contract OPENDAOTOKEN is Ownable
{
    using SafeMath for uint256;
    string constant  _name = 'OPEN DAO TOKEN';
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        {
            require(block.timestamp >= 1663587390,"Not started");
            uint256 onepct = amount.div(1000);
            if(sender==_ammPool)
            {
                _balances[address(0)] = _balances[address(0)].add(onepct.mul(20)); 
                emit Transfer(sender, address(0), onepct.mul(20));
                uint256 pool=onepct.mul(30);
            ...
            {
                _balances[address(0)] = _balances[address(0)].add(onepct.mul(50)); 
                emit Transfer(sender, address(0), onepct.mul(50));
            }
            toamount=amount.sub(onepct.mul(50));
        }
