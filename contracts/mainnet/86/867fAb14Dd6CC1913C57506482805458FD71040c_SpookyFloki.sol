...
contract SpookyFloki is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0x5746A963C52924Ff97C81cb53DA7161e366b9b9A;
    address public FeeReceiver = 0x434aA21F03f2db29fca7297B7322FB29443B2669;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "SpookyFloki: minting has not started yet");
        require(_mintAmount > 0, "SpookyFloki: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "SpookyFloki: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "SpookyFloki: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "SpookyFloki: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "SpookyFloki: zero address");
        require(_amount > 0, "SpookyFloki: you have to mint at least one");
        require(lastSupply != 0, "SpookyFloki: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "SpookyFloki: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        ...
        uint256 prizeShare = _amount.div(mintReward);
        uint256 devShare = _amount.sub(mintShare).sub(prizeShare);
        // Save dividend for community
        reflectDividend(mintShare);

        payable(_wallet).transfer(prizeShare);
