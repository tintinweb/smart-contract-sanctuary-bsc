...
contract StakingPoolImplementation is StakingPoolStorageStructure {
    ...
    */
    ...
    function setRank( uint256[8] memory _ranks, uint256[8] memory _percentages) external onlyPoolCreator {
        for (uint256 i = 0; i < _ranks.length; i++) {

            if (_percentages[i] == 0) break;

            if (i < prizeRewardRates.length ) {
                prizeRewardRates[i] = PrizeRewardRate({
                    rank: _ranks[i], 
