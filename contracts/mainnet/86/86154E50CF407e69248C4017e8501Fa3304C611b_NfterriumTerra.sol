...
contract NfterriumTerra is Initializable, ERC1155Upgradeable, IERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        __ERC1155_init("https://nfterrium.com/terra/{id}.json");
    ...
    function mintTerra(address _wallet) public onlyRole(MINTER_ROLE) {
    ...
    {
        for (uint256 i = 0; i < ids.length; i++) {
            for (uint256 j = 0; j < tokenOwners[from].length; j++) {
                if (tokenOwners[from][i] == ids[i]) {
