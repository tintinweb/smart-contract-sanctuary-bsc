...
stableUP is a community crypto club and the birth of an ecosystem! 
Bringing a new kind of utility to the ever-thriving defi space.

Telegram: https://t.me/StableUP
Twitter: https://twitter.com/1stableup?s=21
Website: https://www.stableup.net/
...
contract Testerosa1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    Testerosa1DividendTracker public dividendTracker;
    ...
    // set taxable transfer
    mapping(address => bool) private _taxableTransfer;
    ...
    constructor() ERC20("Testerosa1", "Testy1") {
        marketingWallet = payable(0x6824409A577dC46330E3eC3df73Cde09e7501544); 
        ...
        dividendTracker = new Testerosa1DividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (42_000_000) * (10**18);
        _mint(owner(), totalTokenSupply); // create supply only called here and never again
        maxWallet = totalTokenSupply / 67; // 1.5%
        swapTokensAtAmount = totalTokenSupply / 400; // 0.25%;
    ...
    //set an address to be a taxable transfer (true) or remove from taxable (false)
    function setTaxableTransfer(address account, bool taxable) public onlyOwner {
        _taxableTransfer[account] = taxable;
        emit TaxableTransfer(account, taxable);
    }
    ...
    function updateFees(
        ...
        require(liquidityBuy <= 7 && liquiditySell <= 7, "Liquidity fees can't exceed 7%");
        require(deadBuy <= 5 && deadSell <= 5, "Burn fees can't exceed 5%");
        require(RewardsBuy <= 12 && RewardsSell <= 12, "Reward fees cannot exceed 10%");
        require(marketingBuy <= 12 && marketingSell <= 12, "Marketing fees can't exceed 12%");
        buyDeadFees = deadBuy;
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 burntokens = amount.mul(deadFees).div(100);
...
contract Testerosa1DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    Testerosa1 public Testerosa1Contract;
    ...
    constructor(
        ...
        defaultToken = token;
        Testerosa1Contract = Testerosa1(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Testerosa1_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Testerosa1_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Testerosa1 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Testerosa1Contract);

            uint256 prevBalance = Testerosa1Contract.balanceOf(address(this));
            ...
            {
                uint256 received = Testerosa1Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    Testerosa1Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
