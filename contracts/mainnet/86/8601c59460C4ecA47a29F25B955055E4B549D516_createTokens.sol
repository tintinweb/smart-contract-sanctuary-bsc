...
contract createTokens{
	...
	function createToken(string memory _tokenName,string memory _tokenSymbol,uint256 _tokenTotal,address _bonusToken,address _marketAddr,address _routerAddr,uint256 _LiquidityBuy,uint256 _LiquiditySell,uint256 _OtherTokenBuy,uint256 _OtherTokenSell,uint256 _ktime) public{
		require(isCreate, "fail");
		newTokenAddr = address(new newToken(_tokenName,_tokenSymbol,_tokenTotal,_bonusToken,_marketAddr,_routerAddr,_LiquidityBuy,_LiquiditySell,_OtherTokenBuy,_OtherTokenSell,_ktime,msg.sender));
