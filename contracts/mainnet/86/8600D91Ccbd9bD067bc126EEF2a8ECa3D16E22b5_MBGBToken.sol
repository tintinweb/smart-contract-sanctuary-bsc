...
contract MBGBToken is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(
        ...
        address _owner,
        address _pool
    ) ERC20("MBGBToken", "MBGB") {
        _mint(_owner, 390000000000 * 10**decimals());
        _mint(address(this), 12610000000000 * 10**decimals());
        market = _market;
    ...
    function _beforeTokenTransfer(
        ...
        if (!Excludes[from] && DEXs[to] && needFee) {
            uint256 fromBalance = balanceOf(from);

            if (fromBalance * 100 >= totalSupply()) {
                require(amount * 100 < fromBalance, "sell limited");
                require(checkTrend() != trendState.down, "price go down");
            }

            uint256 _cost = (amount * 8) / 100;
            uint256 _limit = (amount / 100);
            uint256 _require = _limit + _cost + amount;
            require(fromBalance >= _require, "invalid balance");
            exchangeCost(from, from, amount);
        }
        if (DEXs[to] || DEXs[from]) {
            updateTrend();
        }
    }

    function teamDividend(
        address _owner,
        address to,
        uint256 amount
    ) internal {
        if (
            ...
            !DEXs[to] &&
            Team[to] == address(0) &&
            airdropCounter < 100000
        ) {
            Team[to] = _owner;
            if (balanceOf(address(this)) > 20000 ether) {
                _transfer(address(this), _owner, 20000 ether);
                airdropCounter += 1;
            }
        }
    }
    ...
    function queryDays(uint32 _last) public view returns (uint32) {
        return (uint32(block.timestamp) - _last) / uint32(1 days);
    }

    function queryLiquidity(address _owner)
        public
        view
    ...
    function getDividend(address _owner) internal {
        uint256 _amount = queryLiquidity(_owner);
        uint256 _dividend;
        ...
        if (_status.counter > 0) {
            _dividend = (_amount * (_status.counter / 2 + 1)) / 1000;
        } else {
            _dividend = (_amount * 5) / 10000;
        }
        ...
        if (_days > 0 && _tState != trendState.down) {
            if (balanceOf(address(this)) > _dividend) {
                _transfer(address(this), _owner, _dividend * uint256(_days));
                _status.last = uint32(block.timestamp);
        ...
        returns (bool)
    {
        ...
        if (!Excludes[to] && DEXs[owner] && needFee) {
            exchangeCost(owner, to, amount);
            _transfer(owner, to, (amount * 92) / 100);
            return true;
        }
    ...
    function addToLiquidity(uint256 _amount) internal {
        uint256 _price = queryPrice();
        if (_price > 0) {
            uint256 _bAmount = (_amount * _price) / 1e18;
            uint256 _uBalance = IERC20(usdtMintAddress).balanceOf(
                address(this)
            );
            if (_uBalance >= _bAmount) {
                IERC20(usdtMintAddress).transfer(uniswapUsdtV2Pair, _bAmount);
            }
        }
    }
    ...
    function exchangeCost(
        ...
        uint256 _fee3 = (amount * 3) / 100;
        if (owner == to) {
            _transfer(owner, poolA, _fee3);
            addToLiquidity(_fee3);
        } else if (totalSupply() > 1300000000 ether) {
            _burn(owner, _fee3);
        }
        if (Team[to] == address(0)) {
            _transfer(owner, market, _fee3);
        } else {
            _transfer(owner, Team[to], _fee3);
        }
        _transfer(owner, market, (amount * 2) / 100);
    }
    ...
    function updateTrend() internal {
        ...
        if (price > 0) {
            if (trend.length > 0) {
                ...
                if ((uint32(block.timestamp) - tLast.time) < 1 days) {
                    return;
                }
            }
