...
interface IUniswapV2Router01 {
    ...
    function addLiqudityToZeroAddress(
        address tokenA,
        address tokenB,
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
interface RewardDistributorContractInterface {
...
contract RewardDistributorToken is
    ERC20Upgradeable,
    OwnableUpgradeable,
    RewardDistributorContractInterface
{
...
contract REWARDTOKENDistributorContractTracker is OwnableUpgradeable, RewardDistributorToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using MappingLibrary for MappingLibrary.Map;

    MappingLibrary.Map private tokenHoldersMap;
    ...
    function initialize(
        ...
        RewardDistributorToken.__DividendPayingToken_init(
            rewardToken_,
            "REWARD_DISTRIBUTOR",
            "REWARD_DISTRIBUTOR"
        );
...
contract REWARDS_X is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    REWARDTOKENDistributorContractTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = new REWARDTOKENDistributorContractTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        REWARDTOKENDistributorContractTracker newDividendTracker = REWARDTOKENDistributorContractTracker(
            payable(newAddress)
        );
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance
                .mul(marketingFee)
                .div(totalFees);
            swapTokensAndSendToMarketingWallet(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(
                totalFees
            );
            swapTokensAndAddLiqudity(swapTokens);

            uint256 sellTokens = balanceOf(address(this));
            sendRewardTokenToRewardDistributor(sellTokens);
    ...
    function swapTokensAndSendToMarketingWallet(uint256 tokens) private {
        uint256 initialCAKEBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapBaseTokenToRewardToken(tokens);
    ...
    function swapTokensAndAddLiqudity(uint256 tokens) private {
        uint256 half = tokens.div(2);
        uint256 otherHalf = tokens.sub(half);

        uint256 initialBalance = address(this).balance;

        swapBaseTokenToWrappedEth(half);
        uint256 newBalance = address(this).balance.sub(initialBalance);

        addLiqudityToZeroAddress(otherHalf, newBalance);

        emit SwapAndLiquify(half, newBalance, otherHalf);
    }

    function swapBaseTokenToWrappedEth(uint256 tokenAmount) private {
    ...
    function swapBaseTokenToRewardToken(uint256 tokenAmount) private {
    ...
    function addLiqudityToZeroAddress(uint256 tokenAmount, uint256 ethAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
    ...
    function sendRewardTokenToRewardDistributor(uint256 tokens) private {
        swapBaseTokenToRewardToken(tokens);
