...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = address(0x0063a33378C3529532F632992877Ab0cb1219B16);
...
contract LUXURYCATTOKEN is Ownable, IBEP20 {
    ...
    string private _name = "Luxury Cat Token";
    string private _symbol = "LCT";

    address public immutable DeadWalletAddress =
        0x000000000000000000000000000000000000dEaD;
    address payable public MarketingWalletAddress =
        payable(0x2cfD16893E4Da3384b759541E0dF5A0395217CE4);
    address payable public BuyBackWalletAddress =
        payable(0x9B54E26583784BA9c765c75754b38394401A3e69);
    address payable public MarketWalletAddress =
        payable(0x2cfD16893E4Da3384b759541E0dF5A0395217CE4);

    uint256 internal _liquidityFee = 100;
    uint256 internal _BurnFee = 200;
    uint256 internal _TeamFee = 200;
    uint256 internal _MarketingFee = 450;
    uint256 internal _MarketingFeeb = 50;

    uint256 public TotalTaxFee =
        _liquidityFee + _BurnFee + _TeamFee + _MarketingFee + _MarketingFeeb;
    ...
    uint256 public _maxSellLimit = 1 * 10**6 * 10**_decimals;
...
//0x10ED43C718714eb63d5aA57B78B54704E256024E main
    constructor() {
        ...
        _isExcludedFromWalletHoldingLimit[MarketWalletAddress] = true;
        _isExcludedFromWalletHoldingLimit[BuyBackWalletAddress] = true;
    ...
    function UpdateMaxSellLimit(uint256 maxSellAmount)
        external
        onlyOwner
    {
        _maxSellLimit = maxSellAmount * 10**_decimals;
    ...
    function UpdateWallets(
        ...
        address payable newBuyBackWallet,
        address payable newMarketWallet
    ) external onlyOwner {
        require(
        ...
        BuyBackWalletAddress = newBuyBackWallet;
        MarketWalletAddress = newMarketWallet;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
                ...
                totalTax =
                    (amount *
                        (_liquidityFee + _TeamFee + _MarketingFeeb + _MarketingFee)) /
                    (feeDenominator);
        ...
        if (
            ...
            if (overMinTokenBalance) {
                ...
                if (_MarketingFee + _MarketingFeeb + _TeamFee + _liquidityFee > 0) {
                    remainingLiquidityToken =
                        (contractTokenBalance *
                            (_MarketingFee + _MarketingFeeb + _TeamFee)) /
                        (_MarketingFee + _MarketingFeeb + _TeamFee + _liquidityFee);
                }
                ...
                if (_liquidityFee > 0) {
                    ...
                    if (_MarketingFee + _MarketingFeeb + _TeamFee > 0) {
    ...
    function swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        uint256 combineFee = _MarketingFee + _TeamFee + _MarketingFeeb;
        ...
        uint256 seedBalance = (transferredBalance * (_MarketingFeeb)) / (combineFee);
        if (marketingBalance > 0) {
            transferToAddressETH(MarketingWalletAddress, marketingBalance);
        }
        if (TeamBalance > 0) {
            transferToAddressETH(BuyBackWalletAddress, TeamBalance);
        }
        if (seedBalance > 0) {
            transferToAddressETH(MarketWalletAddress, seedBalance);
        }
    }
