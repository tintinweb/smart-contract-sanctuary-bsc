...
contract NDBStaking is Ownable {
    using SafeMath for uint256;

    uint256 public totalStake;
    uint256 public totalRewards;

    enum StakingPeriod{ ONE_YEAR,TWO_YEARS,THREE_YEARS,FOUR_YEARS,FIVE_YEARS }
    ...
    constructor(address _ndbToken)
    { 
        ndbToken = IERC20(_ndbToken);
        apr[StakingPeriod.ONE_YEAR] = 600; //6%
        apr[StakingPeriod.TWO_YEARS] = 1500; //15%
        apr[StakingPeriod.THREE_YEARS] = 3600; //36%
        apr[StakingPeriod.FOUR_YEARS] = 6100; //61%
        apr[StakingPeriod.FIVE_YEARS] = 10000; //100%
    }
    ...
    function createStake(uint256 _stake, StakingPeriod _stakePeriod) public {
        ...
        require(ndbToken.transferFrom(msg.sender, address(this), _stake), "Token Transfer Failed");
    ...
    function unStake(uint256 _stake, StakingPeriod _stakePeriod) public {
        ...
        ndbToken.transfer(msg.sender, _stake);
        ndbToken.transferFrom(owner(), msg.sender, _investorReward);
    ...
    function getStakingPeriodInNumbers(stake memory _investor) internal pure returns (uint256){
        return _investor.stakePeriod == StakingPeriod.ONE_YEAR ? 365 days : _investor.stakePeriod == StakingPeriod.TWO_YEARS ? 370 days : _investor.stakePeriod == StakingPeriod.THREE_YEARS ? 1095 days : _investor.stakePeriod == StakingPeriod.FOUR_YEARS ? 1460 days : _investor.stakePeriod == StakingPeriod.FIVE_YEARS ? 1825 days : 0; 
    }
    ...
    function remainingTokens() public view returns (uint256) {
        return Math.min(ndbToken.balanceOf(owner()), ndbToken.allowance(owner(), address(this)));
    }
