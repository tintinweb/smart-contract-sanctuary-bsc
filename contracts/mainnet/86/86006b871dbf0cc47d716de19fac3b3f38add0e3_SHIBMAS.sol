...
contract SHIBMAS is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SHIBMASDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SHIBMAS", "SHIBM") {
        marketingWallet = payable(0xbe22120811d5C4bD6A15c0fa757fa5B766Cd9a3a);
        devWallet = payable(0x4Bd976B53661014d4D4dA5f749C5E825408619c5);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new SHIBMASDividendTracker(
            payable(this),
            router,
...
contract SHIBMASDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SHIBMAS public SHIBMASContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SHIBMASContract = SHIBMAS(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SHIBMAS_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SHIBMAS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHIBMAS contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SHIBMASContract);

            uint256 prevBalance = SHIBMASContract.balanceOf(address(this));
            ...
            {
                uint256 received = SHIBMASContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SHIBMASContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
