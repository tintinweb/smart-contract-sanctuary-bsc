...
/**
 * @title Gooodfellas Binions Contract
 */
contract GooodfellasBinions is ERC721Base, MintpassMintable, ERC721Reservable, ERC721Whitelist, GallerLaunchpad, ReentrancyGuard {
    ...
    uint256 public priceWithMintpass = 0.32 ether;
    ...
    function _mintTo(address _to, uint256 _amount) internal override(ERC721Reservable, GallerLaunchpad, MintpassMintable) nonReentrant {
        require(mintActive || msg.sender == owner(), "Mint not started yet");
        require(_amount > 0 && _amount <= maxMintPerTx, "Invalid amount");
        require((totalSupply() + _amount + reservedOpen + _mintpassMintsOpen() + _launchpadSupplyOpen()) <= maxSupply, "Exceeds maxSupply");
...
abstract contract MintpassMintable is Ownable {
     ...
     */
    function useMintpasses(uint256[] memory _tokenIds) external {
