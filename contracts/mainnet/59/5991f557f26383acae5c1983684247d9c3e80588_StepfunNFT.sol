...
interface IBox is IERC1155Upgradeable {
    ...
    function getMinPriceAddListing(uint256 boxType)
        external
        view
        returns (uint256);
}

interface IStepfunNFT is IERC721Upgradeable {
...
interface IRandomTable {
    function getRandomTable(uint256 index)
        external
        view
        returns (uint256[] memory);
}
...
contract StepfunNFT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    function mintAdmin(
        ...
        emit MintAdmin(minter, tokenId, nftType, nftRarity);
    }

    function mintBatchAdmin(
        address minter,
        uint256[] memory nftsType,
        uint256[] memory nftsRarity
    ) external onlyOwner {
        require(nftsType.length == nftsRarity.length, "not equal length");

        for (uint256 i = 0; i < nftsType.length; i++) {
            uint256 tokenId = nextTokenId;
            _safeMint(minter, nextTokenId);
            nextTokenId++;

            emit MintAdmin(minter, tokenId, nftsType[i], nftsRarity[i]);
        }
    }
