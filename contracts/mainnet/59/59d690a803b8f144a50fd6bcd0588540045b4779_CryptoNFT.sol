...
contract CryptoNFT is ERC20Detailed, Ownable {
    ...
    string public _name = "Crypto NFT Galaxy";
    string public _symbol = "CNG";
    ...
    uint256 public CNGInsuranceFundFee = 10;
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(CNGInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public CNGInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("Crypto NFT Galaxy", "CNG", uint8(DECIMALS)) Ownable() {
...
//Variables have been defined now we move on the rest of the configuration	
        ...
        CNGInsuranceFundReceiver = 0xf01ECc072F02aCd869B789A2f0F862999B6E6D21;
    ...
    function takeFee(
        ...
        uint256 _CNGInsuranceFundFee = CNGInsuranceFundFee;
        ...
        if (recipient == pair) {
            ...
            _CNGInsuranceFundFee = CNGInsuranceFundFee.add(sellFees);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(CNGInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(CNGInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(CNGInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(CNGInsuranceFundFee).div(
                treasuryFee.add(CNGInsuranceFundFee)
            ),
            gas: 30000
        }("");
   ...
   antiBotEnabled = _enable;
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no CNG token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function setFeeReceivers(
        ...
        address _CNGInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        CNGInsuranceFundReceiver = _CNGInsuranceFundReceiver;
        firePit = _firePit;
    }
