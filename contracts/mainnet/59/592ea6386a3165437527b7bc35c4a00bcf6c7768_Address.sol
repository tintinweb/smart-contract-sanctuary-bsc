...
library Address{
 ...
 contract Taechu is Context, IERC20, Ownable {
    ...
    address public TaechuWallet = 0x3c636760c3B0637D1c8BD8d6f18Cd62328678B38;

    string private constant _name = "Taechu";
    string private constant _symbol = "$Taechu";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[TaechuWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[TaechuWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Taechu) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Taechu);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Taechu) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Taechu);
        emit FeesChanged();
    }
    ...
    function _takeTaechu(uint256 rTaechu, uint256 tTaechu) private {
        totFeesPaid.Taechu +=tTaechu;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tTaechu;
        }
        _rOwned[address(this)] +=rTaechu;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rTaechu) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTaechu = tAmount*temp.Taechu/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tTaechu;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rTaechu = s.tTaechu*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rTaechu;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rTaechu) {

        if(!takeFee) {
          return(0);
        }

        rTaechu = s.tTaechu*currentRate;
        return (rTaechu);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tTaechu);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rTaechu > 0 || s.tTaechu > 0) _takeTaechu(s.rTaechu, s.tTaechu);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Taechu) * 2;
        ...
        uint256 TaechuAmt = unitBalance * 2 * temp.Taechu;
        if(TaechuAmt > 0){
            payable(TaechuWallet).sendValue(TaechuAmt);
        }
    }
    ...
    function updateTaechuWallet(address newWallet) external onlyOwner{
        TaechuWallet = newWallet;
    }
