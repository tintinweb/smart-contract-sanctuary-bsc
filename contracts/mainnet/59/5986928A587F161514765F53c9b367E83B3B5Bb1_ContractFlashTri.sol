...
contract ContractFlashTri {
    ...
    // Trade Variables
    address private constant WBNB = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7;
    ...
    function placeTrade(
        ...
        address[] memory path = new address[](2); // does 2 mean 3 elements?
        path[0] = _fromToken;
        path[1] = _toToken;
    ...
    function triangularArbitrage(
        ...
        address dummyToken;
        if (_tokens[0] != WBNB && _tokens[1] != WBNB) {
            dummyToken = WBNB;
        } else if (_tokens[0] != BUSD && _tokens[1] != BUSD) {
            dummyToken = BUSD;
        } else if (_tokens[0] != CAKE && _tokens[1] != CAKE) {
            dummyToken = CAKE;
        } else {
            dummyToken = USDC;
        }
    ...
    function joeCall(
        ...
        // Profit check
        bool profCheck = checkProfitability(loanAmount, trade2AcquiredCoin);
        require(profCheck, "Arbitrage not profitable");

        // Pay yourself back first
        IERC20 otherToken = IERC20(tradeDetails[sender].tokenA);
        otherToken.transfer(sender, trade2AcquiredCoin - amountToRepay);
