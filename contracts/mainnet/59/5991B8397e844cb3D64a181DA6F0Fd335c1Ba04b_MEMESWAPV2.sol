...
contract MEMESWAPV2 is Ownable {
    ...
    constructor(){
        ...
        transferHelper=SwapTransferHelper(0x0d36d74EDdC17839CeE33882c6DcEbc23d5C9b11);
        //transferHelper=SwapTransferHelper(transferHelper_);
    }
    ...
    function BuyTokens(address token, uint minOut) external payable lock{
        ...
        if(data.TaxHandler!=address(0)){
            ITaxHandler taxHandler=ITaxHandler(data.TaxHandler);
            (baseTax, tokenTax)=taxHandler.getBuyTax(token,data.BuyTax,address(this).balance,address(0));
        }else{
            baseTax=data.BuyTax;
            tokenTax=data.TokenShare;     
        }
