...
contract Batcher {
   ...
   */
  function batch(address[] calldata recipients, uint256 values)
    external
    payable
    lockCall
  {
    require(msg.value == recipients.length * values, 'value wrong');
    ...
    for (uint8 i = 0; i < recipients.length; i++) {
      ...
      emit BatchTransfer(msg.sender, recipients[i], values);
    }
  }
