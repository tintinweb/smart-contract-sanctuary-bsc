...
contract KeyResolver is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public _nftPool=10;
    uint256 public _nftSigle = 10;
    ...
    event Console(address nftAddess,uint fundTo);
    
    constructor (address _usdt, address _router, address _team, address _wrap,address fundAddress) {
        TEAM = _team;
        USDT = _usdt;
        wrap = _wrap;
    ...
    function setNftPool(uint256 nftPoolFee) external onlyOwner {
        _nftPool = nftPoolFee;
    }

    function setNftPSigle(uint256 nftSigleFee) external onlyOwner {
        _nftSigle = nftSigleFee;
    }
    ...
    function setDivAddress(address divAddress) public onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (Param memory param) {
        ...
        if(takeFee && isSell){
            ...
            param.toFund =  tAmount*_nftSigle / 100;
            param.toPool = tAmount*_nftPool / 100;
        
            //nft指定分红
            uint _totalFee = _liquidityFee + _burnFee + _taxFee+_nftPool+_nftSigle;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 initialBalance = IUSDT(USDT).balanceOf(address(this));

        swapTokensForUsdt(half); 

        uint256 newBalance = IUSDT(USDT).balanceOf(address(this)).sub(initialBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount) private {
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IUSDT(USDT).approve(address(uniswapV2Router), usdtAmount);
    ...
    function _takeFee(Param memory param, address from) private {
        emit Console(param.nftAddess,param.fundTo);
        uint256 currentRate = _getRate();
        ...
        if(param.toFund >0){
            ...
            _rOwned[_fundAddress] = _rOwned[_fundAddress].add(param.toFund.mul(currentRate));
            emit Transfer(msg.sender, _fundAddress, param.toFund);
        }

        if(param.fundTo>0){
           
            _rOwned[_fundAddress] = _rOwned[_fundAddress].sub(param.fundTo.mul(currentRate));
            _rOwned[param.nftAddess] = _rOwned[param.nftAddess].add(param.fundTo.mul(currentRate));
            emit Transfer(_fundAddress,param.nftAddess,param.fundTo); 
        }
        if(param.toPool>0){ 
         
            _rOwned[_divAddress] = _rOwned[_divAddress].add(param.toPool.mul(currentRate));

             if (IDIVC(_divAddress).totalSupply() > 0) {
                 IDIVC(_divAddress).distributeUSDTDividends(param.toPool);
           
             }
            emit Transfer(msg.sender,_divAddress,param.toPool);
    ...
    function claim() external {
	    IDIVC(_divAddress).processAccount(msg.sender, false);
    }
