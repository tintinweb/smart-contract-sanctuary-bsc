...
contract Ceshiyixia is Context, IERC20, Ownable {
    ...
    uint private TaxBlock = 10;
    
    address payable public marketingWalletAddress = payable(0xB7EBF07cad896568f1169844f06a88b5D212D486);
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax) external onlyOwner() {
        require(newLiquidityTax.add(newMarketingTax) < 7);
    ...
    function setLimitAll(uint256 _newLimit) external onlyOwner{
        require(_newLimit * 10**_decimals >= _totalSupply.div(100));
    ...
    uint256 public deservedPercent = 94;
    function updateDeserve(uint value) external onlyOwner{
        deservedPercent = value;
    }

    function updateTaxBlock(uint _block) external onlyOwner{
        TaxBlock = _block;
    }
