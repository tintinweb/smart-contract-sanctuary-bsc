...
contract faye is IERC20, AuthLibrary {
    ...
    function NewAdmin(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
    ...
    function txHash(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function _setStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function _setSwapFee(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        _swapThreshold = _amount;
    }  

    function _setFeeLimit(address holder, bool exempt) external authorized {
        isFeeHolderExempt[holder] = exempt;
    }

    function _setTxLimit(address holder, bool exempt) external authorized {
