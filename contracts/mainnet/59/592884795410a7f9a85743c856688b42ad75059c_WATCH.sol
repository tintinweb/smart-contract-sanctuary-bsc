...
contract WATCH is ERC20, Ownable {
    ...
    WATCHDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WATCH", "EYE") {
        marketingWallet = payable(0x1aeADbA20E49273524CBA75EA1f7C5ab776cC7C6);
        devWallet = payable(0xFCB58316787e836dF3cdE3da9fa4d3AF867A84dA);
        ...
        dividendTracker = new WATCHDividendTracker(
            payable(this),
            router,
...
contract WATCHDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        WATCHContract = WATCH(mainContract);
    ...
    function _transfer(
        ...
        require(false, "WATCH_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "WATCH_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WATCH contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(WATCHContract);

            uint256 prevBalance = WATCHContract.balanceOf(address(this));
            ...
            {
                uint256 received = WATCHContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    WATCHContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
