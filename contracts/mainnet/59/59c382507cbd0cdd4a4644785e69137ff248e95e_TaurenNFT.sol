...
contract TaurenNFT is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => uint256) public tokenIdToStatus;//0正常, 1解救中, 2质押中
    ...
    function getUserPledgeNFTIDs(address user) public view returns(uint256[] memory myPledgeIDs) {

        uint256[] memory ids = getUserNFTIDs(user);
        ...
        for(uint256 i = 0; i < ids.length; i++){
            if(tokenIdToStatus[ids[i]] == 2){
                tokenIds0[count] = ids[i];

                count++;
            }
        }

        myPledgeIDs = new uint256[](uint256(count));

        for(uint256 i = 0; i < count; i++){
            myPledgeIDs[i] = tokenIds0[i];
        }
