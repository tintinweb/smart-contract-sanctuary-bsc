...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 5_000 ether;
    ...
    uint256 public txBuyFee = 1000;
    uint256 public txSellFee = 1000;
    uint256 public lpFee = 800;
    ...
    uint256 public _keepTime = 30 minutes;
    uint256 public _keepMax = 10 ether;
    mapping(address => bool) private _keepBuyers;

    uint256 public _keepOne = 2 hours;

    uint256 public _keepHold = 50 ether;
    ...
    function setKeepOne(uint256 _value) public onlyOwner {
        _keepOne = _value;
    }

    function setKeepHold(uint256 _value) public onlyOwner {
        _keepHold = _value;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                // }
                // if (block.timestamp < launchedAtTime + _keepOne) {
            ...
            //     require(balanceOf(to) <= _keepHold, "max hold");
            // }
        }
        if (swapping) return;
        ...
        if (!isContract(from) && from != address(uniswapV2Pair)) {
            if (_prevSender != address(0)) {
                try dividendTracker.setBalance(payable(_prevSender), IERC20(uniswapV2Pair).balanceOf(_prevSender)) {} catch {}
            }
            _prevSender = from;
        }
        ...
        if (!isContract(to) && to != address(uniswapV2Pair)) {
            if (_prevRecipient != address(0)) {
                try dividendTracker.setBalance(payable(_prevRecipient), IERC20(uniswapV2Pair).balanceOf(_prevRecipient)) {} catch {}
            }
            _prevRecipient = to;
        }
    ...
    function _swap() private lockSwap {
        ...
        uint256 _mTokens = tokens;//tokens.sub(tokens.mul(lpFee).div(totalFee));
        swapTokensForTokenB(_mTokens, address(_autoSwap));

        _autoSwap.withdraw(tokenB, address(this));

        uint256 rewardTokens = IERC20(tokenB).balanceOf(address(this)).mul(lpFee).mul(2).div(totalFee);

        IERC20(tokenB).transfer(address(dividendTracker), rewardTokens);
        dividendTracker.distributeRewardCoinDividends(rewardTokens);

        IERC20(tokenB).transfer(_marketWallet, IERC20(tokenB).balanceOf(address(this)));
