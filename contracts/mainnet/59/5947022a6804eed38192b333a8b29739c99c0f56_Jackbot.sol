...
contract Jackbot is Context, IERC20 {
  ...
  address payable public constant PS_Lock =
    payable(0x407993575c91ce7643a4d4cCACc9A98c36eE1BBE);
  address payable public constant MD_Lock =
    payable(0xD55d70E9F7243A11e650E7d1569649B2E9BB6b68);
  address payable public Development =
    payable(0xeaEd594B5926A7D5FBBC61985390BaAf936a6b8d);
...
*/
  ...
  /*
Calculate 1 token price via router getAmountsOut router method
*/
  ...
  function buyLotteryTickets(uint256 numberOfTickets, address player) private {
    ...
    if (players[player] == false) {
      if (listOfPlayers.length > _numActivePlayers) {
        ...
        indx[player] = listOfPlayers.length - 1;
      }
      players[player] = true;
    ...
    if (players[player] == true) {
      if (tickets[player] == 0) {
        if (listOfPlayers.length > 1) {
          listOfPlayers[indx[player]] = listOfPlayers[listOfPlayers.length - 1];
        }
        listOfPlayers.pop(); // Implicitly recovers gas from last element storage
  ...
  mapping(address => uint256) public indxDaily;
    ...
    private
  {
    ...
    if (playersDaily[player] == false) {
      if (listOfPlayersDaily.length > _numActivePlayers) {
        ...
        indxDaily[player] = _numActivePlayers;
      } else {
        listOfPlayersDaily.push(player); // otherwise append to array
        indxDaily[player] = listOfPlayersDaily.length - 1;
      }
    ...
    private
  {
    ...
    ticketsDaily[player] = ticketsDaily[player] - _numTickets; // account for if user has already tix previously for this current lottery
    numTotalTicketsDaily = numTotalTicketsDaily - _numTickets; // update the total # of tickets
    if (playersDaily[player] == true) {
      if (ticketsDaily[player] == 0) {
        if (listOfPlayersDaily.length > 1) {
          listOfPlayersDaily[indxDaily[player]] = listOfPlayersDaily[
            listOfPlayersDaily.length - 1
          ];
        }
        listOfPlayersDaily.pop(); // Implicitly recovers gas from last element storage

        playersDaily[player] = false;
        numActivePlayersDaily = _numActivePlayers - 1;
      }
    }
  }
