...
contract BscStake is Owned {
    
    using SafeMath for uint;

    address public zfm_token;
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(zfm_token).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough  to pay registration fee.");
        require(IERC20(zfm_token).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
    ...
    function calculateEarnings(address _stakeholder) public view returns(uint) {
        uint activeDays = (now.sub(lastClock[_stakeholder])).div(8);
        return ((stakes[_stakeholder]).mul(dailyROI).mul(activeDays)).div(4629600000);
    }
    ...
    function stake(uint _amount) external onlyRegistered() whenActive() {
        ...
        uint remainder = (now.sub(lastClock[msg.sender])).mod(8);
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(zfm_token).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(zfm_token).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient  balance in pool');
        stakeRewards[msg.sender] = 0;
        referralRewards[msg.sender] = 0;
        referralCount[msg.sender] = 0;
        uint remainder = (now.sub(lastClock[msg.sender])).mod(8);
        lastClock[msg.sender] = now.sub(remainder);
        IERC20(zfm_token).transfer(msg.sender, totalReward);
        emit OnWithdrawal(msg.sender, totalReward);
        return true;
    }

    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(zfm_token).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        require((IERC20(zfm_token).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient  balance in pool');
        IERC20(zfm_token).transfer(msg.sender, _amount);
