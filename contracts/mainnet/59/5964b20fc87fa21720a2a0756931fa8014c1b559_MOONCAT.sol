...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    function _updateMinimumTokensForDividends(uint256 _newLimit) external onlyOwner {
        minimumTokenBalanceForDividends = _newLimit * 10**18;
    }
...
contract MOONCAT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public rewardSellFee = 6;
    ...
    address payable public  marketingWallet = payable(0xc85e4E4432De658C65CdF28b472352e781964792);
    ...
    constructor() ERC20("Moon Cat", "MOONCAT") {
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function updateBuyFees(uint256 _rewardFee, uint256 _liquidityFee, uint256 _marketingFee) public onlyOwner {
        require(_rewardFee.add(_liquidityFee).add(_marketingFee) <= 25, "total tax must not be greater than 25%");
    ...
    function setMaxBuytxAmount(uint256 _maxBuyTxAmount) public onlyOwner {
        require(_maxBuyTxAmount*10**18 >= totalSupply().div(1000), "Transaction amount must be greater than or equal to 0.1% of total supply");
        maxBuyTransactionAmount = _maxBuyTxAmount * (10**18);
    }

    function setMaxSelltxAmount(uint256 _maxSellTxAmount) public onlyOwner {
        require(_maxSellTxAmount*10**18 >= totalSupply().div(1000), "Transaction amount must be greater than or equal to 0.1% of total supply");
        maxSellTransactionAmount = _maxSellTxAmount * (10**18);
    }

    function UpdateMinimumTokensForDividends(uint256 _newLimit) public onlyOwner {
