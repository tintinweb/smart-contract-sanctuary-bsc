...
contract GymLevelPool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    modifier userQualified(address _user) {
        require(users[_user].level != 0, "GymLevelPool: user not qualified for any pool");
        _;
    }
    ...
    function claim(uint256 _poolId) external nonReentrant validLevel(_poolId) {
        ...
        uint256 rewardAmount = _claim(_poolId);
    ...
    function claimAll() external nonReentrant {
        ...
        for (uint256 i = 0; i < poolLevels.length; ++i) {
            rewardAmount += _claim(poolLevels[i]);
        }
    ...
    function _claim(uint256 _poolId) internal returns (uint256) {
        ...
        if (rewardAmount > 0) {
            IERC20Upgradeable(rewardToken).safeTransfer(msg.sender, rewardAmount);
