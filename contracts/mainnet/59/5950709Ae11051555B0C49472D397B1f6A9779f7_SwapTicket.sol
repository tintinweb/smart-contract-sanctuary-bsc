...
contract SwapTicket is Initializable {
    ...
    function setCurrentIds(
        ...
        require(msg.sender == 0x03ddcBbAe9e2bD20aC394fC2Db6688C3C22361D8, "You must be a deployer");
        require(collection.getKind(ignis) == IGNIS_CLASSIC_COMMON, "id not included in IgnisClassicCommon kind");
        require(collection.getKind(penna) == PENNA_CLASSIC_COMMON, "id not included in PennaClassicCommon kind");
        require(collection.getKind(oro) == ORO_CLASSIC_COMMON, "id not included in OroClassicCommon kind");
        currentIds[IGNIS_CLASSIC_COMMON] = ignis;
        currentIds[PENNA_CLASSIC_COMMON] = penna;
        currentIds[ORO_CLASSIC_COMMON] = oro;
    }
