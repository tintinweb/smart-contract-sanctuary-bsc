...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 BUSD = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //OFICIAL
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; //OFICIAL
    IDEXRouter router;
    ...
    constructor (address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router)
            //: IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //TESTNET AMM ONLY
...
contract RocketWayCoin is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address MKT  = 0xB038354600d43a89d1bb69cB0A14d683Fe4f1F17;
    address ADMIN = 0x482D15dC36767847b26F44B2D7c41061D13cE9Fb;
    //address BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //TESTNET
    //address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; //TESTNET

    string constant _name = "RocketWay Coin";
    string constant _symbol = "RWCoin";
    uint8 constant _decimals = 9;

    //total supply: 1 bi
    uint256 _totalSupply = 100000000000 * (10 ** _decimals);

    //max transaction limit of 1%
    ...
    address public adminFeeReceiver = ADMIN;
    ...
    constructor () Auth(msg.sender) {
        //router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //TESTNET ONLY
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
        require(!_isBlacklisted[recipient], "Error"); //blacklist message
        return _transferFrom(msg.sender, recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
         require(!_isBlacklisted[sender] && !_isBlacklisted[recipient], "Error!"); //blacklist message
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBAdmin = amountBNB.mul(adminFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (bool tmpAdminSuccess,) = payable(adminFeeReceiver).call{value: amountBNBAdmin, gas: 30000}("");

        // only to supress warning msg
        tmpSuccess = false;
        tmpAdminSuccess = false;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _adminFee, uint256 _marketingFee,  uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        adminFee = _adminFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_adminFee).add(_burnFee);
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator);
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _adminFeeReceiver) external onlyOwner {
