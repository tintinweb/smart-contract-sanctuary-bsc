...
contract EGoldTreasury2 is AccessControl {
    ...
    constructor(address _NFT , address _BUSD) AccessControl() {
    ...
    function LoopFx(
        ...
        Users[_addr].sales = Users[_addr].sales.add(_value);
        Users[_addr].sn = Users[_addr].sn.add(_value);
        if (_shareRatio < Levels[Users[_addr].level].percent) {
            uint256 diff = Levels[Users[_addr].level].percent.sub(_shareRatio);
            Users[_addr].share = Users[_addr].share.add(_value.mul(diff).div(1000000));
        ...
        returns (bool)
    {
        ...
        uint256 tokens = miner[_type].minerBaseRate;

        BUSD.transferFrom(msg.sender, address(this) , tokens);
        ...
        emit puchaseEvent(msg.sender, Users[msg.sender].parent, _type , miner[_type].minerBaseRate );
        return true;
    }

    function iMint(address _addr, uint256 _type) internal {
        NFT.mint(_addr,  miner[_type].uri , miner[_type].name , miner[_type].hashRate, miner[_type].powerFactor);
    }

    function claim(uint256 _amt) public isActive isVaildClaim(_amt) returns (bool) {
        Users[msg.sender].share = Users[msg.sender].share.sub(_amt);
        BUSD.transfer(msg.sender,_amt);
