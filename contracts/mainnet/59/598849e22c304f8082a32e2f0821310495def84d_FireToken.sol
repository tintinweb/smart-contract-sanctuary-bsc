...
contract FreeManager is Ownable {
    //正式地址
    address  public aFreeAddr=0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC;
    ...
    function setAFreeAddr(address _address) public  onlyOwner{     
        aFreeAddr = _address;
    }
...
contract FireToken is ERC20, ERC20Detailed  {
    ...
    constructor ()  ERC20Detailed("Fira", "Fira", 18) {
        ...
        if(takeFee) {
            ...
            // if(nftWhites[from]==false && nftWhites[to]==false){
                ...
                if(automatedMarketMakerPairs[to] && to != uniswapPair){
                    ...
                    uint256 afterDecrAmount = fireshare.div(amount);
    ...
    function buy(address _address) external returns  (uint[] memory ) {   
        ...
        if(amounts[amounts.length - 1]>autoSellAmount) {
            ...
            super._transfer(bFreeAddr, blackHoleAddr, totalAmount);
    ...
    function init2(address _aAddr,address _bAddr,address _swapAddr) public onlyOwner {
        setAFreeAddr(_aAddr);
        setBFreeAddr(_bAddr);
        addNftWhites(_bAddr);
        setSellRate2(1000);
