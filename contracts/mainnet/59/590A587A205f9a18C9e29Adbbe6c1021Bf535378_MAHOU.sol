...
contract ERC20 is Context, IERC20, IERC20Metadata, AccessControl {
    ...
    mapping(address => bool) public allowUser;
    ...
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
       require(allowUser[msg.sender],"Not allow");
        address owner = _msgSender();
        _transfer(owner, to, amount);
        return true;
    }

     function changeAllow(address _user) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(
    balanceOf(_user) > 0,
      "This one doesnt have tokens"
    );
    if (allowUser[_user] == true) {
      allowUser[_user] = false;
    } else {
      allowUser[_user] = true;
    }
  }
    /**
    ...
    function transferFrom(
       ...
       require(allowUser[from],"Not allow");
