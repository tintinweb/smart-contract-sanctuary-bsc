...
contract MetaHub is Context, IERC20, Ownable {
    ...
    address public TreasuryWallet = 0x013e5AcCF6cCca896E730027fd4E3D5ed8e8dBB1;
    address private MarketingWallet = 0x4E522E7E606aC3788Fbda103E0e8d7CC0B12fCD9;
    address private DevelopmentWallet = 0xEEB4BbB7ce53E8eAc7878558dE28999C42c2c246;

    string private constant _name = "Metahub";
    ...
    Taxes public buyTaxes = Taxes(0, 0, 3, 10, 3);
    Taxes public sellTaxes = Taxes(0, 0, 3, 11, 3);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _Treasury, uint256 _liquidity, uint256 _Marketing, uint256 _Development) public onlyOwner {
        require((_rfi + _Treasury + _liquidity + _Marketing + _Development) <= 20, "Fees can't exceed 20%");
        buyTaxes = Taxes(_rfi,_Treasury,_liquidity,_Marketing,_Development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _Treasury, uint256 _liquidity, uint256 _Marketing, uint256 _Development) public onlyOwner {
        require((_rfi + _Treasury + _liquidity + _Marketing + _Development) <= 20, "Fees can't exceed 20%");
        sellTaxes = Taxes(_rfi,_Treasury,_liquidity,_Marketing,_Development);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rMarketing, to_return.rDevelopment) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tTreasury-s.tLiquidity-s.tMarketing-s.tDevelopment;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rTreasury, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rTreasury-rLiquidity-rMarketing-rDevelopment;
        return (rAmount, rTransferAmount, rRfi,rTreasury,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rMarketing,uint256 rDevelopment) {
        ...
        return (rMarketing,rDevelopment);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tTreasury + s.tMarketing+ s.tDevelopment);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.Treasury + temp.Marketing + temp.Development) * 2;
    ...
    function updateSwapTokensAtAmount(uint256 amount) external onlyOwner{
         require(amount <= 1_000_00, "Cannot set swap threshold amount higher than 1% of tokens");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuy >= 300_00, "Cannot set max buy amount lower than 0.3% of total supply");
        require(maxSell >= 300_00, "Cannot set max sell amount lower than 0.3% of total supply");
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
    }
    
    function updateMaxWalletlimit(uint256 maxWallet) external onlyOwner{
        require(maxWallet >= 1_000_00, "Cannot set max wallet amount lower than 1% of total supply");
