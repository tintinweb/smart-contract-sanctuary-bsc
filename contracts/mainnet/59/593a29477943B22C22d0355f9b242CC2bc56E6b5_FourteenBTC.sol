...
contract FourteenBTC is ERC20, Ownable {
    ...
    constructor() ERC20("14BTC", "14BTC") {

        starting = true;
        end = 25;
        maxWallet = 50 * 10 ** 9 * 10 ** decimals();
        maxTransaction = 50 * 10 ** 9 * 10 ** decimals();
        maxWalletTimer = 1667451600;

        _mint(msg.sender, 210 * 10 ** 9 * 10 ** decimals());
    }
    ...
    function _transfer(
       ...
       if(current <= start.add(end) && from != owner() && to != owner()) {
           ...
           _burn(address(this), balanceOf(address(this)));
                ...
                require(amount <= maxTransaction, "Transfer amount exceeds maximum transaction");
                require(!(tx.origin.isContract()), "Illegal contract transaction");
                require(!(to.isContract()), "Illegal transaction");
