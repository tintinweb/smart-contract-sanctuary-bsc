...
// File: contracts/ApplePresale.sol
...
contract ApplePresale is ReentrancyGuard {
	using SafeMath for uint256;
    ...
    uint256 public maximumDepositEthAmount = 600 * 10 ** 18;
	...
	function getAppleTokensLeft() external view returns (uint256) {
		return Apple.balanceOf(address(this));
	}

	function claimTokens() external nonReentrant {
		require(isClaimActive, "Claim is not allowed yet");
		require(tokensOwned[msg.sender] > 0, "User should own some Apple tokens");
		require(tokensUnclaimed[msg.sender] > 0, "User should have unclaimed Apple tokens");
		require(Apple.balanceOf(address(this)) >= tokensUnclaimed[msg.sender], "There are not enough Apple tokens to transfer.");

		uint256 callerTokensUnclaimed = tokensUnclaimed[msg.sender];
		tokensUnclaimed[msg.sender] = 0;
		Apple.safeTransfer(msg.sender, callerTokensUnclaimed);
	...
	function withdrawUnsoldApple() external onlyOwner {
		uint256 amount = Apple.balanceOf(address(this)) - totalTokensSold;
		Apple.safeTransfer(msg.sender, amount);
	}

	function withdrawAllApple() external onlyOwner {
		Apple.safeTransfer(msg.sender, Apple.balanceOf(address(this)));
	}
}