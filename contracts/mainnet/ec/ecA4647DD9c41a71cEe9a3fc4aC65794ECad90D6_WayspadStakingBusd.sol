...
contract WayspadStakingBusd {
    ...
    uint256[4] public Duration = [60 days, 60 days, 60 days, 60 days];
    uint256[4] public Bonus = [40 , 40, 40, 40];
    struct Stake {
        uint256 unstaketime;
        uint256 staketime;
        uint256 amount;
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        totalFee = 0;
        maxStakeableToken = 2000000000000000000000;
        percentDivider = 1000;
        minimumStakeToken = 50000000000000000000;
        maxpoolbalance = 50000000000000000000000;
        maxaccountbalance = 2000000000000000000000;
    }
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 WAYSVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(WAYSVAL)).div(1e18);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address WAYS_WBNB = 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16; // WAYS_WBNB pancake pool address
        IERC20 WAYSTOKEN = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); // WAYS Token address 
        uint256 WBNBSUPPLYINWAYS_WBNB =(WBNBTOKEN.balanceOf(WAYS_WBNB));
        uint256 WAYSSUPPLYINWAYS_WBNB = (WAYSTOKEN.balanceOf(WAYS_WBNB));
        uint256 WAYSUSDVAL = (((WBNBSUPPLYINWAYS_WBNB.mul(1e18)).div((WAYSSUPPLYINWAYS_WBNB))).mul(BNBPRICE)).div(1e18);
        return WAYSUSDVAL;
    }
