...
contract REC is IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        // 持币分红 3%
        if (takeFee && recipient == pair && tokenDividendEnabled && allAddr.length > 0) {

            //待分红值 3%
            uint256 _tDivide = tAmount.div(100).mul(3);
            uint256 _tdivAmount;
            uint256 _tActualDivTotal=_tTotal.sub(_tDivide);
            for (uint i = 0; i < allAddr.length; i++) {

                if (balanceOf(allAddr[i]) >= minDivToken) {
                    _tdivAmount = balanceOf(allAddr[i]).mul(_tDivide).div(_tActualDivTotal);

                    if (allAddr[i] == pair || allAddr[i] == address(this) || allAddr[i] == _destroyAddress)
                    {
                        _rOwned[_destroyAddress] = _rOwned[_destroyAddress].add(
                            _tdivAmount.mul(currentRate)
                        );
                        emit Transfer(sender, _destroyAddress, _tdivAmount);
                    } else {
                        _rOwned[allAddr[i]] = _rOwned[allAddr[i]].add(
                            _tdivAmount.mul(currentRate)
                        );
                        emit Transfer(sender, allAddr[i], _tdivAmount);
                    }
