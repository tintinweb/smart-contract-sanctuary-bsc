...
contract Metazen{
  ...
  event Airdropped(address indexed _userAddress, uint256 _amount);
    ...
    constructor(address ownerAddress,IBEP20 _BUSD,IBEP20 _MTZ) public
    {
        owner = ownerAddress;  
        BUSD = _BUSD;
        MTZ = _MTZ;
  ...
  function Staking(string memory investorId,uint investment,string memory pool) public payable
	{
	 require(MTZ.balanceOf(msg.sender)>=investment*1e18);
		MTZ.transferFrom(msg.sender ,owner,investment*1e18);
		emit Reinvestment( investorId,investment*1e18,msg.sender,pool);
	}
