...
contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
            ...
            if (_swapPairList[from] && limit) {
                require(amount <= maxBuyAmt, "maxBuyAmt");
                require(balanceOf(to).add(amount) <= maxBuyAmt, "maxBuyAmt");
            }
        }
    ...
    function enableLimit(bool status) public {
        require(msg.sender == fundAddress, "not dev");
        limit = status;
    }

    function setMaxBuyAmt(uint256 _maxbuy) public {
        require(msg.sender == fundAddress, "not dev");
        maxBuyAmt = _maxbuy;
    }
...
contract AAToken is AbsToken {
    constructor()
        AbsToken(
            ...
            6,
            30000,
            address(0x8FF69c82a3173F3128B0cb000e772723b3Be4Be8)
        )
    {}
}