...
contract NFTC is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (isSell && destroyAmount <= maxDestroyLimit ) {
            emit LogBool("isSell", isSell);
            emit LogUint256("destroyAmount", destroyAmount);
            emit LogUint256("maxDestroyLimit", maxDestroyLimit);
            emit LogUint256("balance pair", balanceOf(pair));
        //    uint256 burnAmount = _tOwned[pair] * 30 / 100;
        //    _tOwned[pair] = _tOwned[pair] - burnAmount;
        //    emit Transfer(pair, destroyAddress, burnAmount);
        }
