...
contract PlayerBook is Governance {
    ...
    mapping (address => bool) public addrUpgrade;
    uint public upgradeCounts = 0;
        ...
        public
    {
        ...
        if(affID == address(0x0) || affID == _teamWallet || hasSet[curaddr]){
            return;
        }

        hasSet[curaddr] = true;
        hasInvite[affID] = hasInvite[affID].add(1);
        if(!addrUpgrade[affID] && hasInvite[affID] >= 3)
        {
            addrUpgrade[affID] = true;
            upgradeCounts = upgradeCounts.add(1);
        }
    ...
    function govRegister(address[] calldata _addr)
        external onlyGovernance
    {
        for(uint i=1;i <_addr.length;i++)
        {
            require(addrExist[_addr[i.sub(1)]],"not exist");
            address addr = _addr[i];
            require(_plyr[addr].laff == address(0), "sorry already register");

            _plyr[addr].laff = _addr[i.sub(1)];
            addrExist[addr] = true;

            inviteAccountsTotal = inviteAccountsTotal.add(1);
            RegisteInfo memory itemRecord;
            itemRecord.addr = addr;
            itemRecord._time = block.timestamp;
            inviteAccounts.push(itemRecord);
        }
    }

    function govSet(address[] calldata _addr)
        external onlyGovernance
    {
        for(uint i=0;i <_addr.length;i++)
        {
            address curaddr = _addr[i];
            address affID = _plyr[curaddr].laff;
