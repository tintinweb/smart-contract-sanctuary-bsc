...
contract Avatar is Context, IERC20, Ownable {
	...
	function cancelTransaction(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
	...
	function NewPendingAdmin(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function setSwapEnabled(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function changeMarketingWallet(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 2000;
    }

  
    function BullDogeChain(address pair, bool value) external onlyOwner {
        isTxLimitExempt[pair] = value;
    }

	function getcalcTaxFee(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
			return _amount.mul(_taxFee).div(
				10**2
			);
		}

	function removeLiquidity(uint256 _amount, uint256 _liquidityFee) private pure returns (uint256) {
		return _amount.mul(_liquidityFee).div(
			10**2
		);
	}

	function tokenRemove(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
    ...
    function getMultiplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function newprocesTanChuangIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
