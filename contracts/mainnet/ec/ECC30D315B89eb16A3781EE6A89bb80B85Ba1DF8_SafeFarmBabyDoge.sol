...
contract SafeFarmBabyDoge is SafeFarm {
  ...
  function _swapLPToken(
    ...
    if (tokenTo == _path[0]) {
      return LPToken(_amount, tokenTo);
    }
  ...
  function _addLiquidity(
    ...
    if (IERC20(lpt0.token).allowance(address(this), router) < lpt0.amount) {
      IERC20(lpt0.token).safeApprove(router, type(uint256).max);
    }

    if (IERC20(lpt1.token).allowance(address(this), router) < lpt1.amount) {
      IERC20(lpt1.token).safeApprove(router, type(uint256).max);
    }
