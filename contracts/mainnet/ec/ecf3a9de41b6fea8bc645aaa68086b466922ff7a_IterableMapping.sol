...
contract DividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Dividend_Tracker", "Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
...
contract villagefed is ERC20, Ownable {
//library
    using SafeMath for uint256;
 //custom
    IUniswapV2Router02 public uniswapV2Router;
    DividendTracker public rewardsDividendTracker;
//address
    address public uniswapV2Pair;
    address public marketingWallet = 0x9829f636f0B8d0Cd2F26922B50E82C611092Cd45;
    address public teamWallet = 0xA3F18Cac7118f97BFc6fAb88f1c77D2839D46F98;
    address public liqWallet = 0xA3F18Cac7118f97BFc6fAb88f1c77D2839D46F98; 
    address public rewardsDividendToken;
 ...
 //bool
    ...
    bool public rewardsDividendEnabled = true;
 ...
 //uint
    uint256 public buySecondsLimit = 10;
    ...
    uint256 public minimumTokensBeforeSwap = 75000 *10**9;
    uint256 public intervalSecondsForSwap = 60;
    uint256 public tokensToSwap = minimumTokensBeforeSwap;
    uint256 public RewardsBuyFee = 3;
    uint256 public RewardsSellFee = 3;
    ...
    uint256 public totalBuyFees = RewardsBuyFee.add(teamBuyFee).add(marketingBuyFee).add(liqBuyFee);
    uint256 public totalSellFees = RewardsSellFee.add(teamSellFee).add(marketingSellFee).add(liqSellFee);
    uint256 public gasForProcessing = 200000;
    uint256 public maxWallet = 2000000 * (10**9); // 2%
...
//struct
 ...
 //event
    ...
    event UpdaterewardsDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event ProcessedrewardsDividendTracker(
    	uint256 iterations,
    	uint256 claims,
    ...
    constructor() ERC20("Village Fed", "NOTREKT") {
        uint256 _total_supply = 100000000 * (10**9);
    	rewardsDividendToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // BUSD REWARDS
        rewardsDividendTracker = new DividendTracker(rewardsDividendToken);
        ...
        excludeFromDividend(address(rewardsDividendTracker));
        ...
        */
        ...
        maxSellTxAmount =  1000000 * (10**9); // 1%
        maxBuyTxAmount =  1500000 * (10**9); // 1.5%
    }
    ...
    function setSwapSend(bool _marketing, bool _team, bool _liq) external onlyOwner {
    ...
    function setTokensToSwap(uint256 amount) external onlyOwner {
        tokensToSwap = amount;
    }
    ...
    function setFees(uint256 _reward_buy, uint256 _liq_buy, uint256 _marketing_buy,
        ...
        RewardsBuyFee = _reward_buy;
        RewardsSellFee = _reward_sell;
        teamBuyFee = _teamBuy;
        ...
        totalBuyFees = RewardsBuyFee.add(teamBuyFee).add(marketingBuyFee).add(liqBuyFee);
        totalSellFees = RewardsSellFee.add(teamSellFee).add(marketingSellFee).add(liqSellFee);
    }
    function Migration(address[] memory _address, uint256[] memory _amount) external onlyOwner {
        for(uint i=0; i< _amount.length; i++){
            address adr = _address[i];
            uint amnt = _amount[i] *10**decimals();
            super._transfer(owner(), adr, amnt);
            try rewardsDividendTracker.setBalance(payable(adr), balanceOf(adr)) {} catch {}
        } 
    }
    ...
    function swapTokens(uint256 minTknBfSwap) private {
        ...
        swapTokensForBNB(minTknBfSwap);
    if(ProcessDividendStatus) {
        uint256 balanceAfter = address(this).balance;
        uint256 Balance = (balanceAfter - balanceBefore) * RewardsSellFee / 100;
        swapETHForTokens(Balance, rewardsDividendToken);
        uint256 FrverDividends = IERC20(rewardsDividendToken).balanceOf(address(this));
        transferDividends(rewardsDividendToken, address(rewardsDividendTracker), rewardsDividendTracker, FrverDividends);
        }
        isInternalTransaction = false;
    } 

  	function prepareForPartherOrExchangeListing(address _partnerOrExchangeAddress) external onlyOwner {
  	    rewardsDividendTracker.excludeFromDividends(_partnerOrExchangeAddress);
    ...
    function setAuthOnDividends(address account) public onlyOwner {
        rewardsDividendTracker.setAuth(account);
    }
 
    function setDividendEnabled(bool _enabled) external onlyOwner {
        rewardsDividendEnabled = _enabled;
    } 
 
    function updaterewardsDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(rewardsDividendTracker), "The dividend tracker already has that address");
 
        DividendTracker newrewardsDividendTracker = DividendTracker(payable(newAddress));
 
        require(newrewardsDividendTracker.owner() == address(this), "The new dividend tracker must be owned by the token contract");
 
        newrewardsDividendTracker.excludeFromDividends(address(newrewardsDividendTracker));
        newrewardsDividendTracker.excludeFromDividends(address(this));
        newrewardsDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newrewardsDividendTracker.excludeFromDividends(address(deadWallet));
 
        emit UpdaterewardsDividendTracker(newAddress, address(rewardsDividendTracker));
 
        rewardsDividendTracker = newrewardsDividendTracker;
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        rewardsDividendTracker.excludeFromDividends(address(account));
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            rewardsDividendTracker.excludeFromDividends(pair);
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        rewardsDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }
 
    function updateClaimWait(uint256 claimWait) external onlyOwner {
        rewardsDividendTracker.updateClaimWait(claimWait);

    }
 
    function getClaimWait() external view returns(uint256) {
        return rewardsDividendTracker.claimWait();
    }
 
  
 
    function getTotalDividendsDistributed() external view returns (uint256) {
        return rewardsDividendTracker.totalDividendsDistributed();
    }
 
    function withdrawableDividendOf(address account) external view returns(uint256) {
    	return rewardsDividendTracker.withdrawableDividendOf(account);
  	}
 
  	
 
	function rewardsDividendTokenBalanceOf(address account) external view returns (uint256) {
		return rewardsDividendTracker.balanceOf(account);
	}
    ...
    function getAccountDividendsInfo(address account)
        external view returns (
            ...
            uint256,
            uint256) {
        return rewardsDividendTracker.getAccount(account);
    }
	...
	function getAccountDividendsInfoAtIndex(uint256 index)
        external view returns (
            ...
            uint256,
            uint256) {
    	return rewardsDividendTracker.getAccountAtIndex(index);
    }
 
    
	function processDividendTracker(uint256 gas) public onlyOwner {
		(uint256 busdIterations, uint256 busdClaims, uint256 busdLastProcessedIndex) = rewardsDividendTracker.process(gas);
		emit ProcessedrewardsDividendTracker(busdIterations, busdClaims, busdLastProcessedIndex, false, gas, tx.origin);
	
    }
  	function updateDividendToken(address _newContract, uint gas) external onlyOwner {
        rewardsDividendTracker.process(gas);
  	    rewardsDividendToken = _newContract;
  	    rewardsDividendTracker.setDividendTokenAddress(_newContract);
  	}
 
    function claim() external {
		rewardsDividendTracker.processAccount(payable(msg.sender), false);
		
    }
    function getLastDividendProcessedIndex() external view returns(uint256) {
    	return rewardsDividendTracker.getLastProcessedIndex();
    }
 
    
 
    function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return rewardsDividendTracker.getNumberOfTokenHolders();
    }
    ...
    // normal transaction
        ...
        // tx limits & tokenomics
            ...
            if(address(this).balance > minimumWeiForTokenomics) {
                ...
                if(LiqSwapSendActive) {
                    ...
                    if(success) {
                    emit LiquidityCollected(LiquidityWTokens);
                    }
                }
            }
        // fees management
        ...
        //set dividends
        try rewardsDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try rewardsDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        // auto-claims one time per transaction
        if(!isInternalTransaction && ProcessDividendStatus) {
	    	uint256 gas = gasForProcessing;
	    	try rewardsDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
	    		emit ProcessedrewardsDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
	    	} catch {}
        }
    }
    ...
    function swapETHForTokens(uint256 amount, address tokenToBuy) private returns(bool) {
        // Generate the uniswap pair path of token -> WETH
        address[] memory path = new address[](2);
    ...
    function approveContractOnDex(uint amount) external onlyOwner() {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        isInternalTransaction = true;
        swapTokensForDividendToken(tokens, address(this), rewardsDividendToken);
        uint256 rewardsDividends = IERC20(rewardsDividendToken).balanceOf(address(this));
        transferDividends(rewardsDividendToken, address(rewardsDividendTracker), rewardsDividendTracker, rewardsDividends);
        isInternalTransaction = false;
    }
