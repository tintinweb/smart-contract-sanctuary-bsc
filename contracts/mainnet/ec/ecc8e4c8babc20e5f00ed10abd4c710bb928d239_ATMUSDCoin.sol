...
//Functions
    ...
    function _deposit(uint256 _amount) public payable {
        ...
        (, bytes memory data) = _busdContract.call(abi.encodeWithSignature("balanceOf(address)", msg.sender));
        uint balance = abi.decode(data, (uint));
        require(balance>=_USD, "Insufficient BUSD.");
        
        (bool success, ) = _busdContract.call(abi.encodeWithSignature("transfer(address, uint256)", _atmbusdcoin, _USD));
    ...
    function withdrawBUSD(uint256 _id) public isDev {
    ...
    function cancelSellOrder(uint256 _orderId) public payable notPaused returns (bool){
        ...
        if(msg.sender!=_dev){
            require(_orders[_orderId]._owner==msg.sender, "Not sell order owner.");
        }
        uint256 _amount = _orders[_orderId]._amount;
        _sellOrders[_orderId]=_orders[_sellOrders.length-1];
        _sellOrders.pop();
    ...
    function getAllowedSpenders() public payable isDev returns (address[] memory){
    ...
    function devWithdrawBUSD(uint _amount) public payable isDev returns (bool){
        (bool success,) = _busdContract.call(abi.encodeWithSignature("transfer(address, uint256)", _dev, _amount));
            return success;
    }
    
    function devWithdraw(uint _amount) public payable isDev returns (bool) {
