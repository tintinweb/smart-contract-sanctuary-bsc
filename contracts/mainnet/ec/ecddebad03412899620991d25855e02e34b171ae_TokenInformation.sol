...
contract TokenInformation {
  ...
  function getInfoTokens(uint amount, address[] memory tokens, address user, address tokenSwapOne, address tokenSwapTwo, address router, address factory) public view returns (TokenInformationStruct[] memory){
      TokenInformationStruct[] memory secondtokeninfostruct = new TokenInformationStruct[](tokens.length);

      for(uint i = 0; i < tokens.length; i++){
          secondtokeninfostruct[i] = getInfoToken(amount, tokens[i], user, tokenSwapOne, tokenSwapTwo, router, factory);
      }
      return secondtokeninfostruct;
  }
  

  function getInfoToken(uint amount, address token, address user, address tokenSwapOne, address tokenSwapTwo, address router, address factory) public view returns (TokenInformationStruct memory){
     // check if token is actually a contract
    uint256 tokenCode;
    ...
    uint amountIn = amount*(10**tokeninfostruct.decimal);
