...
contract luca is Context, IERC20, Rmath {
    ...
    function getModPay() public view returns(uint256) {
        ...
        if(mcap >= 500000000e18 && mcap < 750000000e18){
            mult = 600;
        }
        if(mcap >= 750000000e18 && mcap < 1000000000e18){
            mult = 700;
        }
        if(mcap >= 1000000000e18 && mcap < 2000000000e18){
            mult = 800;
        }
        if(mcap >= 2000000000e18 && mcap < 5000000000e18){
            mult = 900;
        }
        if(mcap >= 5000000000e18){
            mult = 1000;
        }
        return (amount * mult);
    }
    ...
    function addDAO(address addy) external {
        require(dao == address(0), "Can only add once");
        dao = addy;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {   
        ...
        require(getMarketCap() >= 15000000e18, "mcap under 150m"); //fix 15m
