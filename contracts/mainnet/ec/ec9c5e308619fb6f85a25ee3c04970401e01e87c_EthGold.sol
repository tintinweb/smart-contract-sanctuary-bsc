...
contract EthGold is ERC20 {
	...
	address private _fundAddress = address(0xF77Cd14326C5C5dEdf421A5ce11B0a8b21e2EDa3);
    ...
    constructor(address tokenOwner) ERC20("EthGold", "EthGold") {
	...
	function lpInit() public onlyOwner {
        _lpBalance[_tokenOwner] = pair.balanceOf(_tokenOwner);
    }
	
	function lpBalanceOf(address account) public view returns (uint256) {
        return pair.balanceOf(account).add(_lpBalance[account]);
    }
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
                if(startTime.add(600) >= block.timestamp){
                    ...
                    require(super.balanceOf(to).add(amount) <= 2 * 10**23, "maxAmount hodler");
                }
                _splitOtherToken();
    ...
    function swapTokensForOther(uint256 tokenAmount) private {
		...
		uint256 preAmount = USDT.balanceOf(address(warp));
