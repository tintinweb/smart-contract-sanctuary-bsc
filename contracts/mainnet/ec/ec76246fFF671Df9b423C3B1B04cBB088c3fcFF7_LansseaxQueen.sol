...
contract LansseaxQueen is Context, IERC20, Ownable {
    ...
    string private constant _name = "Lansseax Queen";
    string private constant _symbol = "LANSQUEEN";
    ...
    address public marketingWallet = address(0xCefD925BA9584f9aEF98d4549e329a1A1CF847Fb);
    ...
    function _transfer(
        ...
        if(launched && block.timestamp > launchEpoch + 60 && !preLaunch) {
            _maxWalletAmount = 1_000* 10**_decimals;
            _maxTxAmount = 1_000 * 10**_decimals;
            preLaunch = true;
        }

        if(launched && block.timestamp > launchEpoch + 3600 && !phaseOne) {
            _maxWalletAmount = 2_000* 10**_decimals;
            _maxTxAmount = 2_000 * 10**_decimals;
            phaseOne = true;
        }
