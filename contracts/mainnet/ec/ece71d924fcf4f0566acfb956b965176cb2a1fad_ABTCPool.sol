...
contract ABTCPool is Ownable, Pausable {
    ...
    event NewVAbtcContract(address VAbtc);
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(abtcPoolPID, balance, address(0));
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the abtc owner address or the operator address.
     */
    modifier onlyOperatorOrAbtcOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or abtc owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onAbtcPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user abtc funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrAbtcOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the ABTC Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ABTC)
     * @param _lockDuration: Token lock duration
     */
    function deposit(uint256 _amount, uint256 _lockDuration, address _referrer) external whenNotPaused {
    ...
    function depositOperation(
        ...
        if (VAbtc != address(0)) {
            address referrer = IReferral(referral).getReferrer(_user);
            IVAbtc(VAbtc).deposit(_user, _amount, _lockDuration, referrer);
        }
        ...
        user.abtcAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the ABTC Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VAbtc != address(0)) {
            IVAbtc(VAbtc).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending ABTC tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingAbtc = masterchefV2.pendingAbtc(abtcPoolPID, address(this));
        if (pendingAbtc > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(abtcPoolPID, 0);
    ...
    /**
     * @notice Set VAbtc Contract address
     * @dev Callable by the contract admin.
     */
    function setVAbtcContract(address _VAbtc) external onlyAdmin {
        require(_VAbtc != address(0), "Cannot be zero address");
        VAbtc = _VAbtc;
        emit NewVAbtcContract(VAbtc);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the ABTC Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingAbtcRewards();
     ...
     * @return Returns total pending abtc rewards
     */
    function calculateTotalPendingAbtcRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingAbtc(abtcPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingAbtcRewards()) * (1e18)) / totalShares);
    }
