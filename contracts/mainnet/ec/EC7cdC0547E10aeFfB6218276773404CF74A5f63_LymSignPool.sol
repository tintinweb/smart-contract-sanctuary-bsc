...
contract LymSignPool is Ownable, Divestor {
    struct Meta {
        bool isOpen;
        uint8 staticRate;
        uint256 power;
    ...
    function _takeInviterReward(address account_, uint256 reward) internal {
        ...
        for (uint8 i = 0; i < refLevel; i++) {
            if (inviter == address(this) || inviter == address(0) || userInfo[inviter].dynamicHP == 0) {
                break;
            }
    ...
    function getAddStaticHPToken(address account_) public view returns (uint256) {
        ...
        uint256 needToken = ((5 days - hp) * userInfo[account_].power) / 5 days;
    ...
    function registerSignMining(uint8 nodeLevel_, address inviter_) public onlyOrigin {
        require(meta.isOpen, "not open");
        ...
        // addr.usdt.transfer(addr.foundation, (needUsdt * 5) / 100);
    }

    function addStaticHP() public onlyOrigin {
    ...
    function claimStaticReward() public onlyOrigin {
        User storage user = userInfo[msg.sender];
        require(block.timestamp <= user.lastReSetTm + 30 days || user.lastReSetTm == 0, "can not claim");
        uint256 hp = getStaticHP(msg.sender);
        require(hp > 0, "no hp");
    ...
    function refreshStaticPower() public onlyOrigin {
        User storage user = userInfo[msg.sender];
    ...
    function addDynamicHP() public onlyOrigin {
    ...
    function claimDynamicReward() public onlyOrigin {
    ...
    function info0(address account_)
        external
        view
        returns (
