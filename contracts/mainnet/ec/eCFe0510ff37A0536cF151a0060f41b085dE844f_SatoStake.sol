...
//  -----------------------------------------
//  ----------------------------------------
//  --JOIN SatoStake COMMUNITY --------------
//  --      https://t.me/Satostake      ----
//  --      https://satostake.finance  ----
//  -----------------------------------------
//  -----------------------------------------
...
contract SatoStake is Context, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    uint256 public constant min = 0.002 ether;
    uint256 public constant max = 5 ether;
    uint256 public roi = 4;
    ...
    constructor(address _dev, address _dev1) {
        //tokenAdress = 0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8; //BTCB Tesntet
        tokenAdress = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c; //BTCB Mainnet
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        uint256 deadline_weekly = block.timestamp + 6 days;
       ...
       // fees 
        ...
        uint256 fee_dev = SafeMath.div(depositFee(_amount),2);
        uint256 fee_dev1 = SafeMath.sub(depositFee(_amount),fee_dev);
        uint256 total_contract = SafeMath.sub(_amount,SafeMath.add(fee_dev,fee_dev1));
        BusdInterface.transferFrom(msg.sender,dev,fee_dev);
        BusdInterface.transferFrom(msg.sender,dev1,fee_dev1);
    ...
    uint256 wFee1 = SafeMath.div(wFee,2); // fees go into two different wallets
    uint256 wFee2 = SafeMath.sub(wFee,wFee1); 

    uint256 totalAmountToWithdraw = SafeMath.sub(aval_withdraw2,SafeMath.add(wFee1,wFee2)); // total withdrawal for the user
    BusdInterface.transfer(msg.sender,totalAmountToWithdraw);
    BusdInterface.transfer(dev,wFee1);
    BusdInterface.transfer(dev1,wFee2);
    ...
    uint256 totalAmount = SafeMath.add(amount,aval_withdraw3); // it will add 25% to total withdraw
    ...
    function unStake() external noReentrant {
        ...
        require(I_investment > t_withdraw, "You already withdrew than your investment");
