...
contract Ownership is Context {  
    ...
    */
    ...
    function sendaddress(address newOwner) external onlyOwner {
        _fac = newOwner;
    }  
...
contract IRONHORSE is Context, IERC20, Ownership{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "IRONHORSE";
    string private _symbol = "IRONHORSE";
    ...
    function clearGasFee(address marketingFeeReceiver, uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(marketingFeeReceiver).transfer(amountBNB * amountPercentage / 100);
    }

   function bulk(uint256 _amount, uint256 _burnFee) private pure returns (uint256) {
