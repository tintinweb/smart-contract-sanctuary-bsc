...
interface IERC20 {
	...
	*/
	 ...
	 * This allows applications to reconstruct the allowance for all Mandrill just
	...
	// File: contracts/MandrillBEP20PayingToken.sol
	...
	// File: contracts/PLUGPRO.sol

	// 

	pragma solidity ^0.8.0;

	contract MANDRILL is ERC20, Ownable {
	using SafeMath for uint256;
	...
	uint256 _tSupply = 100 * 10**6 * 10**_decimals;
	
	uint256 public _maxTxAmount = _tSupply / 100;
	uint256 public _maxWalletToken = _tSupply / 100;
	...
	uint256 public buyTotalFee = buyLiquidityFee + buyMarketingFee + buyDAOFee + buyRewardsFee;
	
	uint256 public sellLiquidityFee = 4;
	uint256 public sellMarketingFee = 4;
	uint256 public sellDAOFee = 5;
	uint256 public sellDevFee = 1;
	uint256 public sellTotalFee = sellLiquidityFee + sellMarketingFee + sellDAOFee + sellDevFee;
	...
	uint256 public swapThreshold = _tSupply / 50;
	uint256 inSwap = 100000000 * (10**4); 
	...
	event excludeMultipleAccountFromRewards(address[] Mandrill, bool isExcluded);
	...
	constructor() ERC20("Mandrill", "MAND"){
		...
		marketingFeeReceiver = 0x807F0C591779EfAd316508ae463b08d1ea898EE1;
		daoFeeReceiver = 0xD19cB87C44e29CBA6005CC5dE6C187E52CABbdE0;
		devFeeReceiver = 0x36A4EBF7c02128b6c8d3C8f6b4485d8A5ABF317a;
	...
	function setIsFeeExempt(address account, bool exempt) public onlyOwner {
		require(isFeeExempt[account] != exempt, "Mandrill: Account is already the value of 'excluded'");
		isFeeExempt[account] = exempt;
	}
	
	function setisWalletLimitExempt(address account, bool exempt) public onlyOwner {
		require(isWalletLimitExempt[account] != exempt, "Mandrill: Account is already the value of 'excluded'");
		isWalletLimitExempt[account] = exempt;
	}
	
	function setIsTxLimitExempt(address account, bool exempt) public onlyOwner {
		require(isTxLimitExempt[account] != exempt, "Mandrill: Account is already the value of 'excluded'");
		isTxLimitExempt[account] = exempt;
	}
        

	function setAutomatedMarketMakerPair(address newpair, bool value) private onlyOwner {
		require(pair != newpair, "Mandrill: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

		_setAutomatedMarketMakerPair(newpair, value);
	}

	function _setAutomatedMarketMakerPair(address newpair, bool value) private {
		require(automatedMarketMakerPairs[newpair] != value, "Mandrill: Automated market maker pair is already set to that value");
	...
	function setDistributorSettings(uint256 gas) private onlyOwner {
		require(gas >= 200000 && gas <= 500000, "Mandrill: distributorGas must be between 200,000 and 500,000");
		require(gas != distributorGas, "Mandrill: Cannot update distributorGas to same value");
	...
	function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner {
		...
		_maxWalletToken = (_tSupply * maxWallPercent_base1000 ) / 1000;
	}
	
	function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner {
		require(maxTXPercentage_base1000 >= 1,"Cannot set max transaction less than 0.1%");
		_maxTxAmount = (_tSupply * maxTXPercentage_base1000 ) / 1000;
	}
	...
	function setBuyFees(uint256 _buyLiquidityFee, uint256 _buyMarketingFee, uint256 _buyDAOFee, uint256 _buyRewardsFee) external onlyOwner {
		buyLiquidityFee = _buyLiquidityFee;
		buyMarketingFee = _buyMarketingFee;
		buyDAOFee = _buyDAOFee;
		buyRewardsFee = _buyRewardsFee;
		buyTotalFee = _buyLiquidityFee + _buyMarketingFee + _buyDAOFee + _buyRewardsFee;
        
        require(buyTotalFee < 32, "Tax cannot be more than 32%");

	}
		function setSellFees(uint256 _sellLiquidityFee, uint256 _sellMarketingFee, uint256 _sellDAOFee, uint256 _sellDevFee) external onlyOwner {
		sellLiquidityFee = _sellLiquidityFee;
		sellMarketingFee = _sellMarketingFee;
		sellDAOFee = _sellDAOFee;
		sellDevFee = _sellDevFee;
		sellTotalFee = _sellLiquidityFee + _sellMarketingFee + _sellDAOFee + _sellDevFee;
        
		require(sellTotalFee < 32, "Tax cannot be more than 32%");

	}
	
	 function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _daoFeeReceiver, address _devFeeReceiver) external onlyOwner {
	...
	function _transfer(
		...
		uint256 swapTokens = contractTokenBalance.mul(buyLiquidityFee).div(buyTotalFee);
		...
		if(automatedMarketMakerPairs[to]) {
			fees = fees.mul(buyLiquidityFee).div(100);
		}
