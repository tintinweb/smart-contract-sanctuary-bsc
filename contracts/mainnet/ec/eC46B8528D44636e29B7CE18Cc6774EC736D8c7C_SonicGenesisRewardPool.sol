...
// Note that this pool has no minter key of Sonic (rewards).
// Instead, the governance will call Sonic distributeReward method and send reward to this pool at the beginning.
contract SonicGenesisRewardPool {
    using SafeMath for uint256;
    ...
    // TESTNET
    // uint256 public bombPerSecond = 3.0555555 ether; // 25000 sonic / (1h * 60min * 60s)
    // uint256 public runningTime = 48 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 25000 ether;
    // END TESTNET

    // MAINNET
    uint256 public bombPerSecond = 0.1446499 ether; // 25000 sonic / (24h * 60min * 60s)
    ...
    function add(
        ...
        require(_depositFeeBP <= 100, "add: invalid deposit fee basis points"); // 1%
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _depositFeeBP) public onlyOperator {
        require(_depositFeeBP <= 100, "set: invalid deposit fee basis points"); //// 1%
    ...
    function safeBombTransfer(address _to, uint256 _amount) internal {
        uint256 _bombReward = bomb.balanceOf(address(this));
        if (_bombReward > 0) {
            if (_amount > _bombReward) {
                bomb.safeTransfer(_to, _bombReward);
            } else {
