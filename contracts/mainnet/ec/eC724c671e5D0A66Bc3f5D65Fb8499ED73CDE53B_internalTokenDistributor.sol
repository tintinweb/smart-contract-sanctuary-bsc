...
contract internalTokenDistributor is Ownable {
    ...
    function batch(address token, address[] memory _receivers, uint256 amount) external onlyOwner {
        uint256 total = 0;
        require(_receivers.length <= getArrayLimit(),"Array length exceeds limit");
