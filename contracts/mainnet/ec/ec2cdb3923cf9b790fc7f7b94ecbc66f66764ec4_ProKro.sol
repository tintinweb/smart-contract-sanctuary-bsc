...
library Address{
  ...
  contract ProKro is Context, IERC20, Ownable {
    ...
    address public promarketingWallet = 0xfF33DF742269A75122827Dad2cAFC7806A253445;
    address public prodevwallet = 0xfF33DF742269A75122827Dad2cAFC7806A253445;

    string private constant _name = "ProDev and Kronos Halloween";
    string private constant _symbol = "ProKro";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[promarketingWallet] = true;
        _isExcludedFromFee[prodevwallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[promarketingWallet] = true;
        allowedTransfer[prodevwallet] = true;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(promarketingWallet).sendValue(marketingAmt);
        }
        uint256 devAmt = unitBalance * 2 * temp.dev;
        if(devAmt > 0){
            payable(prodevwallet).sendValue(devAmt);
        }
    }
    ...
    function updateproMarketingWallet(address newWallet) external onlyOwner{
        promarketingWallet = newWallet;
    }
    
    function updateprodevwallet(address newWallet) external onlyOwner{
        prodevwallet = newWallet;
    }
