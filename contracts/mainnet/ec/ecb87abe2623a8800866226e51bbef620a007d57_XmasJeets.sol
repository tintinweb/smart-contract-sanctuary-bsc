...
contract XmasJeets is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _CallsFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _CallsFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _CallsReceiver,address _communityFeeReceiver, address _teamFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "XmasJeets";
    string constant private _symbol = "xJ";
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(CallsFee).add(comFee).add(teamFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeCalls).add(sellFeecom).add(sellFeeteam).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address public autoLiquidityReceiver = msg.sender;
    address public CallsFeeReceiver = 0x09Cd5F56AB3D744FE405d60E126AA437194E6Fba;
    address public communityFeeReceiver = 0xf2FC4CD869B2F168Ad8A6405974C9dec39030faB;
    address public teamFeeReceiver = 0xf2FC4CD869B2F168Ad8A6405974C9dec39030faB;
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _CallsFee, uint256 _comFee, uint256 _teamFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        CallsFee = _CallsFee;
        comFee = _comFee;
        teamFee = _teamFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_CallsFee).add(_comFee).add(_teamFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than Maxfee");

        emit BuyFeesChanged(_liquidityFee, _CallsFee,_comFee, _teamFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _CallsFee,uint256 _comFee, uint256 _teamFee, uint256 _BurnFee, uint256 _feeDenominator) external onlyOwner {
        sellFeeLiquidity = _liquidityFee;
        sellFeeCalls = _CallsFee;
        sellFeecom = _comFee;
        sellFeeteam = _teamFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_CallsFee).add(_comFee).add(_teamFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than Maxfee%");

        emit SellFeesChanged(_liquidityFee, _CallsFee,_comFee, _teamFee, _BurnFee);
    }


    function setFeeReceivers(address _autoLiquidityReceiver, address _CallsFeeReceiver,address _communityFeeReceiver, address _teamFeeReceiver, address _BurnFeeReceiver) external onlyOwner {
        require(_autoLiquidityReceiver != address(0) && _CallsFeeReceiver != address(0) && _communityFeeReceiver != address(0) && _teamFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        ...
        emit SetFeeReceivers(_autoLiquidityReceiver, _CallsFeeReceiver, _communityFeeReceiver, _teamFeeReceiver, _BurnFeeReceiver);
    }
