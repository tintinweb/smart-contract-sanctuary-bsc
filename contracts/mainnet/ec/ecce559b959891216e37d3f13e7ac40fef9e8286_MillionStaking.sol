...
contract MillionStaking {
	...
	address payable public			WALLET_PROJECT_1 				= payable(0x62d6A63792fC5A078833418aFb7Effff840Af8e0);
	address payable public			WALLET_PROJECT_2 				= payable(0x0a61D672DB25cAc6bb653442A8360F6774DaD057);
	uint constant public			PERCENTS_DIVIDER				= 1000;
	uint constant public			TIME_STEP						= 1 days;
	uint constant public			INVEST_MIN_AMOUNT				= 0.1 ether;			// 0.1 BNB
	uint[] public					REFERRAL_PERCENTS				= [10];					// 10%
	uint constant public			PROJECT_FEE						= 80;					// project fee 8% of deposit
	uint constant public			PROJECT_FEE_PORC_1				= 50;					// % of fee 5%
	uint constant public			PROJECT_FEE_PORC_2				= 30;					// % of fee 3%
	...
	uint constant public			REINVEST_PERCENT				= 700;					// auto reinvest 70% of claim
	...
	constructor() {
		...
		PLANS.push( TPlan(14,80) );
		PLANS.push( TPlan(21,100) );
		PLANS.push( TPlan(30,120) );
	...
	function invest(address _referrer, uint8 _planIdx) public payable {
		...
		//transfer project fee - Total 8%
		uint pfeeporce1 = msg.value * PROJECT_FEE_PORC_1 / PERCENTS_DIVIDER;
		uint pfeeporce2 = msg.value * PROJECT_FEE_PORC_2 / PERCENTS_DIVIDER;

		// Wallets Devs%
		WALLET_PROJECT_1.transfer(pfeeporce1);
		emit ProjectFeePaid(pfeeporce1);
		WALLET_PROJECT_2.transfer(pfeeporce2);
		emit ProjectFeePaid(pfeeporce2);
	...
	function reinvest(uint8 planId) public {
		...
		uint claimAmount = _getUserDividends(msg.sender) + user.debtBuffer;
