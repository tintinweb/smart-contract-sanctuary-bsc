...
contract BtcSnacks is SnacksBase {
    ...
    function _beforeDistributeFee(
        uint256 feeBtcSnacks_,
        uint256 totalBtcHoldersBalance_
    ) internal override {
        if (feeBtcSnacks_ > 0) {
            // % BtcSnacks holders
            uint256 feeToHolders = feeBtcSnacks_
            ...
            if (feeSnacksHolders > 0) {
                _selfTokenSnacks.transfer(
                    snacks,
                    feeBtcSnacks_.mul(feeSnacksHolders).div(100)
                );
            }
        }
    }
