...
// File contracts/drachma-core/asset/Asset.sol
pragma solidity 0.8.5;
/**
 * @title Asset
...
contract Asset is Ownable, ERC20, ERC20Permit, IAsset {
    ...
    modifier onlyPool() {
        if (msg.sender != pool) revert DRACHMA_FORBIDDEN();
        _;
    }
    ...
    function setPool(address pool_) external override onlyOwner {
        require(pool_ != address(0), 'Drachma: Pool address cannot be zero');
     ...
     * @dev Not to be confused with transferring Drachma Assets.
    ...
    function mint(address to, uint256 amount) external override onlyPool {
        if (maxSupply != 0) {
            ...
            require(amount + this.totalSupply() <= maxSupply, 'Drachma: MAX_SUPPLY_REACHED');
        }
    ...
    function removeCash(uint256 amount) external override onlyPool {
        require(cash >= amount, 'Drachma: INSUFFICIENT_CASH');
        cash -= uint120(amount);
    }
    ...
    function removeLiability(uint256 amount) external override onlyPool {
        require(liability >= amount, 'Drachma: INSUFFICIENT_LIABILITY');
        liability -= uint120(amount);
    }
}