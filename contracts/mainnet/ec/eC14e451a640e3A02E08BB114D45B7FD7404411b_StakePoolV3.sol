    ...
    Ownable
{
    ...
    mapping(address => uint256) public validInvite;
    ...
    uint8[] public inviterLevelRewardRatio;
    ...
    constructor(
        ...
        inviterLevelRewardRatio = [70, 50, 40, 30, 20, 10, 5, 5, 5, 5];
        levelRewardRatio[TYPELEVEL.LEVEL1] = 6; //8%
        levelRewardRatio[TYPELEVEL.LEVEL2] = 8; //10%
        levelRewardRatio[TYPELEVEL.LEVEL3] = 10; //12%
        levelRewardRatio[TYPELEVEL.LEVEL4] = 12; //15%
    ...
    function setInviteLevelRatio(uint _index, uint8 _inviteLevelRewardRatio) external onlyOwner {
        uint8[] storage inviterLevelRewardRatios = inviterLevelRewardRatio;
        inviterLevelRewardRatios[_index] = _inviteLevelRewardRatio;
    }
        ...
        updateReward(msg.sender)
    {
        ...
        if(currnValue >= inviteRequiredLessAmount) {
            address inviter = IInviter(inviteAddress).getInviter(msg.sender);
            if(inviter != address(0)) {
                validInvite[inviter] = validInvite[inviter].add(1);
            }
        }
    ...
    function sendRecommend(address toAddress, uint256 amount) private {
        ...
        for(uint256 i = 0; i < 10; i++) {
             reward = values.mul(inviterLevelRewardRatio[i]).div(1e21);
            if (index == address(0)) {
                break;
            }
            ...
            if(i >= 5 && validInvite[index] < 3) {
                index = IInviter(inviteAddress).getInviter(index);
                continue;
            } else if(i >= 2 && validInvite[index] < 2) {
                index = IInviter(inviteAddress).getInviter(index);
                continue;
            } else if(i >= 0 && validInvite[index] < 1) {
                index = IInviter(inviteAddress).getInviter(index);
                continue;
            }
            if(reward > 0) {
                rewards[index] = earned(index);
        ...
        updateReward(msg.sender)
    {
        ...
        super.withdraw();
        if(userActValue[msg.sender] >= inviteRequiredLessAmount) {
            address inviter = IInviter(inviteAddress).getInviter(msg.sender);
            if(inviter != address(0)) {
                validInvite[inviter] = validInvite[inviter] > 1 ? validInvite[inviter].sub(1) : 0;
            }
        }
