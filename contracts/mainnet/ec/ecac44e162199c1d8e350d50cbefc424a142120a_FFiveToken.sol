...
contract FFiveToken is IERC20, Ownable {
    ...
    address private cakeAddr = 0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82;
   
    uint256 private _tFeeTotal;
    string private _name = "F5";
    string private _symbol = "F5";
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _takeCakeFee (
        address sender,
        address recipient,
        uint256 tAmount
    ) private lockTheSwap {
        if (_cakeFee == 0) return; 
        if(_tOwned[address(this)]  < tAmount)return; 
        
        address getCake = recipient;
        if(sender != uniswapV2Pair) getCake = sender;

        swapThisTokenForToken(tAmount,getCake);
    }

    function swapThisTokenForToken(uint256 thisTokenAmount,address receiver) private{
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        IERC20 cakeToken = IERC20(cakeAddr);//Cake代币地址
        path[0] = address(this);//本币地址
        path[1] = address(cakeToken);//代币地址
    ...
    function _transferStandard(
       ...
       //1%奖励蛋糕
       _takeCakeFee(sender, recipient,tAmount.div(10000).mul(_cakeFee));
