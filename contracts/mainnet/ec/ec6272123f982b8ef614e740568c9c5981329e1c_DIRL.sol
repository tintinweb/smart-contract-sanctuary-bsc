...
contract DIRL is Context, IERC20, Ownable  {
	...
	address payable public devWalletAddress = payable(0x046f5BEaF3Cb12c61C6762f1F3EBe27e32aedead); // Dev Wallet 
    ...
    uint256 private _tTotal = 100_000_000_000_000_000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Diferencial";
    string private _symbol = "DRL";
    ...
    uint256 totalSwapableFee = _liquidityFee.add(_marketingFee).add(_taxFee).add(_devFee);
    ...
    uint256 totalSwapableSaleFee = _saleTaxFee.add(_saleLiquidityFee).add(_saleMarketingFee).add(_saleDevFee);
...
//    address public pinkAntiBot_ = 0xbb06F5C7689eA93d9DeACCf4aF8546C4Fe0Bf1E5;
	...
	address private PinkSaleAntiBotRouter;
...
// Finishing Variables for constructor
    ...
    {   
        ...
        uint256 walletsTotal = marketingTokensCollected.add(devTokensCollected);
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee.add(_taxFee).add(_marketingFee).add(_devFee);
    }
    ...
    function setSaleFee() private {
        ...
        totalSwapableFee = _liquidityFee.add(_taxFee).add(_marketingFee);
    ...
    function setAllFeePercent(uint256 taxFee, uint256 liquidityFee,  uint256 marketingFee, uint256 devFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        totalSwapableFee = _liquidityFee.add(_taxFee).add(_marketingFee).add(_devFee);
        require(totalSwapableFee <= 10, "Total fee is over 10%");
    }

    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee,uint256 devFee) 
    external onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        totalSwapableSaleFee = _saleLiquidityFee.add(_saleMarketingFee).add(_saleTaxFee).add(_saleDevFee);
		require(totalSwapableSaleFee <= 10, "Total fee is over 10%");
    ...
    function setMarketingWalletAddress(address _marketingWallet) external onlyOwner() {
        require(_marketingWallet != address(0), "setMarketingWalletAddresss: ZERO");
    ...
    function setpinkAntiBotAddress(address _AntiBotAddress) external onlyOwner() {
        ...
        PinkSaleAntiBotRouter = _AntiBotAddress;
    }
    ...
    function recoverETHfromContract(address _Address) external onlyOwner {
