...
contract NekoStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setUserV2(address _user) external onlyOwner {
        userV2[_user] = true;
    }
    ...
    function pendingReward(uint256 _pid, address _user) public view whenNotPaused returns (uint256) {
        ...
        if (_pid == 2 ) {
            if(!enableLimitApy || userV2[_user] || user.amount == 100000* 10**18) {
    ...
    function unStakeV2(uint256 _pid) public whenNotPaused {
        ...
        bool check =(!enableLimitApy || userV2[msg.sender] || user.amount == 100000* 10**18 );
        if( _pid == 2 && check) {
        ...
        if (_pid == 2) {
            ...
            totalNinoV2Claimed += totalReward;
        }
