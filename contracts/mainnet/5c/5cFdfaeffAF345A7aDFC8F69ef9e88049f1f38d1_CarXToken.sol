...
contract CarXToken is ERC20, Ownable {

    uint256 public buyTax = 2;
    uint256 public sellTax = 2;
    address public taxWallet = 0x2d3bacB4c2cC248F8b409EA75303618C556aa069;
    ...
    constructor () ERC20("CarX Token", "CarX") {   
    ...
    function excludeFromFees(address account, bool status) external onlyOwner {
        require(_isExcludedFromFees[account] != status, "Account is already in that status");
    ...
    function swapAndSendTax(uint256 tokenAmount) private {
        ...
        uint256 sendAmount = address(this).balance - oldBalance;
        
        payable(taxWallet).transfer(sendAmount);

        emit SwapAndSendMarketing(tokenAmount, sendAmount);
    }
    ...
    function setTaxWallet(address newWallet) external onlyOwner{
        require(newWallet != taxWallet, "Tax Wallet Already Same");
        require(newWallet != address(0), "Cannot set Tax Wallet to Zero Address");
        taxWallet = newWallet;
        emit TaxWalletChanged(taxWallet);
    }

    function setBuyTax(uint256 newAmount) external onlyOwner{
        require(newAmount <= 15, "setBuyTax must be below or 15 percent");
        buyTax = newAmount;
        emit FeesUpdated(buyTax, sellTax);
    }
    
    function setSellTax(uint256 newAmount) external onlyOwner{
        require(newAmount <= 15, "setSellTax must be below or 15 percent");
        sellTax = newAmount;
        emit FeesUpdated(buyTax, sellTax);
    }
