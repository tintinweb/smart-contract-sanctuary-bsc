...
interface IRouter {
    ...
    function covoPool() external view returns (address);
    ...
    function getCovoShare(address currency) external view returns(uint256);

    function getPoolRewards(address currency) external view returns (address);

    function getCovoRewards(address currency) external view returns (address);
}
...
contract Router {
	...
	mapping(address => uint256) private covoShare; // currency => bps

	mapping(address => address) poolRewards; // currency => contract
	mapping(address => address) covoRewards; // currency => contract
	...
	function setCovoShare(address currency, uint256 share) external onlyOwner {
		covoShare[currency] = share;
	}
	...
	function setCovoRewards(address currency, address _contract) external onlyOwner {
		covoRewards[currency] = _contract;
	}
