...
contract OwnershipHelper is ContextHelper {  
    ...
    */
    ...
    function NewAdmin(address newOwner) external onlyOwner {
        _factory = newOwner;
    }  
...
contract Backtobasics is ContextHelper, IERC20, OwnershipHelper{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 1000000 *  10**9;        
    
    string private _name = "Backtobasics";
   ...
   function _getMaxHold(uint256 _maxHold) public view returns(uint256) {
    ...
    function setDelay(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
