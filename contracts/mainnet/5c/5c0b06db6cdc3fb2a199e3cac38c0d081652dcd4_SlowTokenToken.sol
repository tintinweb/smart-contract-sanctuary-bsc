...
contract SlowTokenToken is IERC20Extended, Auth, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        treasureFeeReceiver = msg.sender;
        require(
            !treasureFeeReceiver.isContract(),
            "Treasure wallet cannot be a contract"
        );
    ...
    function swapBack() internal swapping {
        ...
        if (totalBNBFee > 0) {
            ...
            uint256 amountBNBTreasure = amountBNB.mul(treasureFee).div(
                totalBNBFee
            );

            try distributor.deposit{value: amountBNBReflection}() {} catch {}
            payable(treasureFeeReceiver).transfer(amountBNBTreasure);
        }
    ...
    function setFeeReceivers(address _treasureFeeReceiver)
        external
        authorized
    {
        require(
            _treasureFeeReceiver != treasureFeeReceiver,
            "Treasure wallet is already that address"
        );
        require(
            !_treasureFeeReceiver.isContract(),
            "Treasure wallet cannot be a contract"
        );
        treasureFeeReceiver = _treasureFeeReceiver;
    }
