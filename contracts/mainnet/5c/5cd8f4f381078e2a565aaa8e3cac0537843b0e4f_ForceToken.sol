...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public minDividend = 20 * 10**18;
    ...
    // uint256 private swapCoolDownTime = 1 days;
    uint256 private swapCoolDownTime = 10 minutes;
    ...
    function _transfer(
        ...
        if (_swapPairList[from] || _swapPairList[to]) {
            if (!_feeList[from] && !_feeList[to]) {
                ...
                if (_swapPairList[to]) {
                    require(lastTxTimes[from] + swapCoolDownTime < block.timestamp, "swapCoolDownTime");
    ...
    function _tokenTransfer(
        ...
        _takeTransfer(sender, recipient, tAmount - sellFeeAmount);
    }
    ...
    function setSellDividendFee(uint256 sellDividendFee) external onlyOwner {
