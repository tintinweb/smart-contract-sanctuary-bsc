...
contract Vault4 {
    ...
    event RewardWithdrawn(address indexed account, uint256 indexed nonce, uint256 interest, uint256 withdrawType);
    ...
    function currentWithdraw(
        ...
        bytes memory message = abi.encode(nonce, account, deadline, principal, interest, rate);
        ...
        if (rate == 0) {
            ...
            dot.safeTransfer(account, (amount * rate) / 10000);
            bdot.safeTransfer(account, (amount * (10000 - rate)) / 10000);
        }
    ...
    function rewardWithdraw(
        ...
        bytes memory message = abi.encode(nonce, account, deadline, interest, rate, 1);
        bytes32 hash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(bytes(message).length), message)
        );
        if (SignatureChecker.isValidSignatureNow(signer, hash, signature) == false) revert();
        ...
        if (rate == 0) {
            bdot.safeTransfer(account, interest);
        } else {
            dot.safeTransfer(account, (interest * rate) / 10000);
            bdot.safeTransfer(account, (interest * (10000 - rate)) / 10000);
        }

        emit RewardWithdrawn(account, nonce, interest, 1);
    }

    function achievementRewardWithdraw(
        address account,
        uint256 deadline,
        uint256 interest,
        uint256 rate,
        ...
        if (block.timestamp > deadline) revert();
        uint256 nonce = achievementNonces[account];
        bytes memory message = abi.encode(nonce, account, deadline, interest, rate, 2);
        bytes32 hash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(bytes(message).length), message)
        );
        if (SignatureChecker.isValidSignatureNow(signer, hash, signature) == false) revert();
        achievementNonces[account]++;
        ...
        emit RewardWithdrawn(account, nonce, interest, 2);
    }
