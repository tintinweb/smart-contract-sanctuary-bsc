...
contract ERC20 is IERC20, Owner{
    ...
    uint256 private _totalFreeze = 0;
    ...
    function getUserFreeze(address account) public view returns(uint){
        if(account == freezeAddress){
            ...
            if(startTime > 0){
                ...
                total = uint(400000000000).mul(difdays);            
            }
            if(900000000000000 > total){
                return uint(900000000000000).sub(total);
            }
        } 
        return 0;
    }
    ...
    function startReleaseFreeze() public onlyOwner returns (bool) {
    ...
    function burn(uint256 value) public {
        ...
        _totalSupply = _totalSupply.sub(value.mul(9).div(10));
        _totalBurn = _totalBurn.add(value.mul(9).div(10));
        _transfer(msg.sender, burnProfitAddress, value.mul(1).div(10));
...
contract DECToken is ERC20 {
    constructor () 
        ERC20("Domino Effect Coin", "DEC", 6) {
        _mint(totalAddress, 100000000 * (10 ** uint256(decimals())));
        _mint(freezeAddress, 900000000 * (10 ** uint256(decimals())));
    }
}