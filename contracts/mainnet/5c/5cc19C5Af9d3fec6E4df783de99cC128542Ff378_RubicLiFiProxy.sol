...
contract RubicLiFiProxy is ReentrancyGuard, AccessControl, Pausable {
    ...
    modifier onlyManager() {
        require(isManager(msg.sender) || isAdmin(msg.sender), 'Caller is not a manager');
        _;
    }
    ...
    function lifiCall(
        ...
        inputToken.safeTransferFrom(msg.sender, address(this), totalInputAmount);

        uint256 inputAmount = _calculateFee(integrator, totalInputAmount, address(inputToken));

        uint256 _allowance = inputToken.allowance(address(this), LifiDiamond);
        if (_allowance < totalInputAmount) {
            if (_allowance == 0) {
                inputToken.safeApprove(LifiDiamond, type(uint256).max);
            } else {
                try inputToken.approve(LifiDiamond, type(uint256).max) returns (bool res) {
                    require(res == true, 'approve failed');
                } catch {
                    inputToken.safeApprove(LifiDiamond, 0);
                    inputToken.safeApprove(LifiDiamond, type(uint256).max);
                }
            }
        }
        ...
        Address.functionCall(LifiDiamond, data);

        require((balanceBefore - inputToken.balanceOf(address(this))) == inputAmount, 'different amount spent');
    }

    function lifiCallWithNative(
        address integrator,
        bytes calldata data
    ) external payable onlyEOA whenNotPaused {
        uint256 inputAmount = _calculateFee(integrator, msg.value, address(0));

        Address.functionCallWithValue(LifiDiamond, data, inputAmount);
    }
    ...
    function setLifiDiamond(address _lifiDiamond) external onlyManager {
        LifiDiamond = _lifiDiamond;
    }
    ...
    function collectIntegratorFeeManual(address _token, address _integrator) external onlyManager {
        uint256 amount = amountOfIntegrator[_token][_integrator];
        require(amount > 0, 'amount is zero');

        amountOfIntegrator[_token][_integrator] = 0;

        if (_token == address(0)) {
            Address.sendValue(payable(_integrator), amount);
        } else {
            IERC20(_token).safeTransfer(_integrator, amount);
        }
    }
