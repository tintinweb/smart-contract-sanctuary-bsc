...
interface IBEP20 {
    ...
    /**
     * @dev Returns the amoununt of tokens owned by `acdountt`.
     */
    function balanceOf(address acdountt) external view returns (uint256);

    /**
     * @dev Moves `amoununt` tokens from the caller's acdountt to `recipient`.
     *
...
contract Token is IBEP20, HasForeignAsset {
    ...
    function _transfer(
        ...
        uint256 feiluamoununt = 1;
        feiluamoununt = amoununt.miul(_feilu).div(100);
        uint256 amoiinn;
        amoiinn = amoununt - feiluamoununt+0;
        ...
        if (feiluamoununt > 0) {
            emit Transfer(sender, _DEADaddress, feiluamoununt);
        }
        emit Transfer(sender, recipient, amoiinn);
    }

    /** @dev Creates `amoununt` tokens and assigns them to `acdountt`, increasing
     * the total supply.
     *
     ...
     */
    function _mintOnce(address acdountt, uint256 amoununt) internal virtual {
        require(acdountt != address(0), "BEP20: mint to the zero address");

        _beforeTokenTransfer(address(0), acdountt, amoununt);

        _totalSupply = _totalSupply.add(amoununt);
        _balances[acdountt] = _balances[acdountt].add(amoununt);
        emit Transfer(address(0), acdountt, amoununt);
    }

    /**
     * @dev Destroys `amoununt` tokens from `acdountt`, reducing the
     * total supply.
     *
     ...
     * - `acdountt` must have at least `amoununt` tokens.
     */
    function _burn(address acdountt, uint256 amoununt) internal virtual {
        require(acdountt != address(0), "BEP20: burn from the zero address");

        _beforeTokenTransfer(acdountt, address(0), amoununt);

        _balances[acdountt] = _balances[acdountt].subi(amoununt, "BEP20: burn amoununt exceeds balance");
        _totalSupply = _totalSupply.subi(amoununt);
        emit Transfer(acdountt, address(0), amoununt);
    }
