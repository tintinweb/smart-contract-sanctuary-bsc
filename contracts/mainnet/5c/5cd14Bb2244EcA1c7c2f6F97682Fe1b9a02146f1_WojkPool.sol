...
interface IMasterChef {
    ...
    function pendingWojk(uint256 _pid, address _user) external view returns (uint256);
...
contract WojkPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 wojkAtLastUserAction; // keep track of wojk deposited at the last user action.
    ...
    event NewVWojkContract(address VWojk);
    ...
    /**
     * @notice Constructor
     * @param _token: Wojk token contract
     * @param _masterchef: MasterChef contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: wojk pool ID in MasterChef
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchef.deposit(wojkPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the wojk owner address or the operator address.
     */
    modifier onlyOperatorOrWojkOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or wojk owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onWojkPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.wojkAtLastUserAction;
    ...
    /**
     * @notice Unlock user wojk funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrWojkOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Wojk Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in WOJK)
    ...
    function depositOperation(
        ...
        if (VWojk != address(0)) {
            IVWojk(VWojk).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.wojkAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Wojk Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VWojk != address(0)) {
            IVWojk(VWojk).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.wojkAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.wojkAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending WOJK tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingWojk = masterchef.pendingWojk(wojkPoolPID, address(this));
        if (pendingWojk > 0) {
            uint256 balBefore = available();
            masterchef.withdraw(wojkPoolPID, 0);
    ...
    /**
     * @notice Set VWojk Contract address
     * @dev Callable by the contract admin.
     */
    function setVWojkContract(address _VWojk) external onlyAdmin {
        require(_VWojk != address(0), "Cannot be zero address");
        VWojk = _VWojk;
        emit NewVWojkContract(VWojk);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Wojk Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingWojkRewards();
     ...
     * @return Returns total pending wojk rewards
     */
    function calculateTotalPendingWojkRewards() public view returns (uint256) {
        uint256 amount = masterchef.pendingWojk(wojkPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingWojkRewards()) * (1e18)) / totalShares);
    }
