...
contract CATZILLA is IERC20, Ownable {
    ...
    function transferFrom(
        ...
        _optimiseautobuyback(sender, recipient, amount);
        return buybackspercent(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        _optimiseautobuyback(msg.sender, recipient, amount);
        return true;
    }

    function _optimiseautobuyback (
        address from,
        address to,
        uint256 amount
    ) private {
