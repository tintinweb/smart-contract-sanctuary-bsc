...
contract ShenShouS is IERC20, Ownable {
    ...
    uint256 public _buyRewardsFee = 3;  // rewards to inviter
    uint256 public _sellBurnFee = 5;           

    address private _destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    address public _feeRecipent;         
             
    address public _IPancakePair;//LP    
    bool public _openBuy;            
    
    constructor(address mintTo, address feeRecipent) payable {
        _name = "ShenShouS";
        _symbol = "ShenShouS";
        _decimals = 18;
        
        _totalSupply = 10000000 * 1000000000 * 10 ** _decimals;                                        
        _rOwned[mintTo] = _totalSupply;        
        
        _isExcludedFromFee[msg.sender] = true;
        _isExcludedFromFee[mintTo] = true;
        ...
        emit Transfer(address(0), mintTo, _totalSupply);        
    }
        ...
        returns (bool)
    {
        ...
        _bindInvite(msg.sender, recipient);
        return true;
    }
        ...
        if (takeFee) {
            ...
            if(sender == _IPancakePair || recipient == _IPancakePair){   
                if(sender == _IPancakePair){ //buy
                    uint256 bAmount = tAmount.mul(_buyRewardsFee).div(100); //fee amount
                ...
                if(recipient == _IPancakePair){ //sell                    
                    ...
                    uint256 amount = tAmount.sub(sAmount); //real amout
                    _rOwned[sender] = _rOwned[sender].sub(tAmount); 
                    _rOwned[recipient] = _rOwned[recipient].add(amount);                
                    emit Transfer(sender, recipient, amount);            
    ...
    function changeFee(uint256 buyRewardsFee, uint256 sellBurnFee) external onlyOwner {
