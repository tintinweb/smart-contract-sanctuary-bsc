...
contract CREDITstake {
    ...
    constructor(address payable _owner,address rewtoken) {
        owner = _owner;
        rewardToken = IERC20(rewtoken);
        maxStakeableToken = 10000000000 * 1e18;
        percentDivider = 1000;
        minimumStakeToken = 1e18;
    }

    function stake(uint256 timeperiod) public payable {
        require(timeperiod >= 0 && timeperiod <= 3, "Invalid Time Period");
        require(msg.value >= minimumStakeToken && msg.value< maxStakeableToken, "stake exceeds limits");
        uint256 CREDITVAL = getPriceinUSD();
        uint256 rewardtokenPrice = (msg.value.mul(CREDITVAL)).div(1e18);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address WCREDIT_USDT = 0x015AC83839990A543446b768F869ddA653c33E87;
        
        IERC20 USDTTOKEN = IERC20(0x55d398326f99059fF775485246999027B3197955);
        IERC20 WCREDITTOKEN = IERC20(0xc9308bCF5Fa46d728422753D2D5afBc5cDb66b03);
        
        uint256 USDTSUPPLYINWCREDIT_USDT = USDTTOKEN.balanceOf(WCREDIT_USDT);
        uint256 WCREDITSUPPLYINWCREDIT_USDT = WCREDITTOKEN.balanceOf(WCREDIT_USDT);
        
        uint256 CREDITPRICE = (USDTSUPPLYINWCREDIT_USDT.mul(1e18)).div(WCREDITSUPPLYINWCREDIT_USDT);
