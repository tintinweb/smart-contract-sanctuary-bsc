...
contract CallSaul is Context, Ownable {
    using SafeMath for uint256;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFees;

    string _name = "CallSaul";
    string _symbol = "CSAUL";
    uint8 _decimals = 9;
    ...
    constructor () {
        ...
        _maxWalletPercent = 32;
        _maxWalletDivisor = 13;
        _start = 7;
        _antiblock = 12;
    ...
    function changeCallSaulWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ClaimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Cannot update claimWait to same value");
        claimWait = newClaimWait;
    }

    function shouldCallSaulClaim(uint256 lastClaimTime) private view returns (bool) {
	...
	function getCallSaulBacking(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
	...
	function increaCallSaulRebase(int256 supplyDelta, uint256 supplyMax) private view returns (uint256) {
	...
	function getCurrentCallSaulRate(uint256 iSupply) private pure returns(uint256, uint256) {
