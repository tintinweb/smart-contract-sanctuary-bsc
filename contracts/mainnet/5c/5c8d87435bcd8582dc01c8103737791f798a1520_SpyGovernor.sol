...
contract SpyGovernor is Governor, GovernorCompatibilityBravo , GovernorTimelockControl {
    using SafeMath for uint256;
    IERC20 public spy;
    uint256 private _delay = 4 hours; // 4 hours
    uint256 private _period = 28800; // 28800~ 1 day
    uint256 private _threshold = 0;
    uint256 private _quorum = 100000;
    // Dev has right to change parameters
    address public dev;
    // proposers has right to propose
    mapping(address => bool) public proposers;
    address public timelockController;
    uint256[] public spyFarmIds;
    address[] public compoundPools;
    IMasterChef public masterchef;
    mapping(address => mapping(uint256 => uint256)) private extraWeights;
    mapping(address => mapping(uint256 => bool)) private hasCalculatedWeights;
    ...
    // only proposer can propose a proposal
    modifier onlyProposer(){
        require(proposers[_msgSender()], "Only propser");
        _;
    }
    ...
    constructor(
        ...
        Governor("SPYCompanyReserveGovernor")
    ...
    function setProposer(address proposer, bool enabled) public onlyDev {
        proposers[proposer] = enabled;
    }

    function setSpyFarmIds(uint256[] memory _spyFarmIds) public onlyDev {
        spyFarmIds = _spyFarmIds;
    }

    function setCompoundPools(address[] memory _compoundPools) public onlyDev {
        compoundPools = _compoundPools;
    }
    ...
    function calculatedWeightForProposal(uint256 proposalId, address account) public view returns (bool) {
        uint256 snapshot = proposalSnapshot(proposalId);
        return hasCalculatedWeights[account][snapshot];
    }

    function voteWeightForProposal(uint256 proposalId, address account) public view returns (uint256) {
        uint256 snapshot = proposalSnapshot(proposalId);
        return spy.balanceOf(account) + extraWeights[account][snapshot];
    }

    function calculateWeight(uint256 proposalId, address account) public {
        uint256 snapshot = proposalSnapshot(proposalId);
        require(!hasCalculatedWeights[account][snapshot], "Already calculated");
        ...
        extraWeights[account][snapshot] = extraWeight;
        hasCalculatedWeights[account][snapshot] = true;
    }
    ...
    function getVotes(address account, uint256 blockNumber)
        ...
        returns (uint256)
    {
        return spy.balanceOf(account) + extraWeights[account][blockNumber];
    }
    ...
    function cancel(uint256 proposalId) public virtual override (GovernorCompatibilityBravo) {
        ...
        if(rewardLeft > 0){
            if(spy.balanceOf(address(this)) < rewardLeft){
                rewardLeft = spy.balanceOf(address(this));
            }
            spy.transfer(_msgSender(), rewardLeft);
        }
    }
        ...
        returns (uint256)
    {
        ...
        spy.transferFrom(_msgSender(), address(this), totalReward);
    ...
    ) internal virtual override(Governor) returns (uint256)
    {
        ...
        uint256 snapshot = proposalSnapshot(proposalId);
        require(hasCalculatedWeights[account][snapshot], "You did not calculate your weight yet");
        if(proposalReward[proposalId].totalDistributed < proposalReward[proposalId].totalCountedReward){
            uint256 myReward = getProposalReward(proposalId, account);
            if(spy.balanceOf(address(this)) > myReward){
                spy.transfer(_msgSender(), myReward);
