...
contract BEP20 is IBEP20, Context, Ownable {
    ...
    uint256 constant MAXCAPSUPPLY = 100000000000000 ether; //100 trillion
...
// SaveShibToken.
contract SaveShibToken is BEP20, MinterRole {

    using SafeMath for uint256;
    using Address for address payable;

    IUniswapV2Router02 public shbSwapRouter;
    address public shbSwapPair;
    ...
    constructor() BEP20('Save Shib', 'SHB') {

        IUniswapV2Router02 _shbSwapRouter = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E); 
        // for testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        // for mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E

        // Create a uniswap pair for this new token
        address _shbSwapPair = IUniswapV2Factory(_shbSwapRouter.factory())
            .createPair(address(this), _shbSwapRouter.WETH());

        shbSwapRouter = _shbSwapRouter;
        shbSwapPair = _shbSwapPair;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        if (
            ...
            && _inSwapAndLiquify == false
            && address(shbSwapRouter) != address(0)
            && shbSwapPair != address(0)
            && sender != shbSwapPair
            && sender != owner()
            && recipient != owner()
        ) {
        ...
        if(transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
            ...
            require(amount == taxAmount + sendAmount, "SHB:: transfer: Tax value invalid");
    ...
    function BuybackBurn(uint256 tokens) private {
        ...
        path[0] = address(this);
        path[1] = shbSwapRouter.WETH();
        path[2] = XMINE;

        _approve(address(this), address(shbSwapRouter), tokens);

        // make the swap
        shbSwapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokens,
            0,
            path,
            deadWallet,
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        _approve(address(this), address(shbSwapRouter), tokenAmount);

        shbSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        (,,uint256 liquidity) = shbSwapRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "SHB: Account is already the value of 'excluded'");
    ...
    function updateXHASHSwapRouter(address _router) public onlyOwner {
        shbSwapRouter = IUniswapV2Router02(_router);
        shbSwapPair = IUniswapV2Factory(shbSwapRouter.factory()).getPair(address(this), shbSwapRouter.WETH());
        require(shbSwapPair != address(0), "SHB::updateXHASHSwapRouter: Invalid pair address.");
        emit XHASHSwapRouterUpdated(_router, address(shbSwapRouter), shbSwapPair);  
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "SHB: Total fees can not be bigger than max value");
    }
    ...
    /// @notice The EIP-712 typeshb for the contract's domain
    bytes32 public constant DOMAIN_TYPEHASH = keccak256("EIP712Domain(string name,uint256 chainId,address verifyingContract)");

    /// @notice The EIP-712 typeshb for the delegation struct used by the contract
    ...
    */
        ...
        external
    {
        ...
        require(block.timestamp <= expiry, "SHB::delegateBySig: signature expired");
