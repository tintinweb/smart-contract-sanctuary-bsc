...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(role[_msgSender()], "Ownable: caller is not the owner");
        _;
    }
...
contract MCC is Context, IERC20, Ownable {
    ...
    string private _name = "MCC";
    string private _symbol = "MCC";
    uint8 private _decimals = 18;

    address payable public MarketingWalletAddress =
        payable(0x3683745ba5cd5baE15495F462122fbe96cc8101a); // Marketing Address 2%
    address payable public DefaultWalletAddress =
        payable(0xcd837e6Cd5C6745920E0fC82Bbd3c3CB720Ef255); //  默认上级钱包 Address 0.5%
    address payable public LPWalletAddress =
        payable(0xa09e5f7a96c1e12877Bf69e090dEbC5107Eb2863); //  LP Diviend Address 2%
    ...
    function setBossBycontract(address me,address aboss) public onlyOwner{
        require(boss[me]==address(0) && me!=aboss);
        boss[me]=aboss;
    }
    function allowance(address owner, address spender)
        public
    ...
    uint256[3] power=[50,25,25];
    function takeFee(address sender, address to,uint256 amount)
        internal
        returns (uint256)
    {
        uint256 feeAmount = amount.div(100).mul(3);
        ...
        uint256 i;
        uint256 feeBoss=amount.div(100).mul(4);
        uint256 feeBossLeft=feeBoss;
        if(isMarketPair[sender]){
            address aboss=boss[to];
            while (aboss!=address(0) && i<3) {
                _basicTransfer(sender,aboss,feeBoss.div(100).mul(power[i]));
                feeBossLeft=feeBossLeft.sub(feeBoss.div(100).mul(power[i]));
                aboss=boss[aboss];
                i++;
            }
        }
        if(isMarketPair[to]){
            address aboss=boss[sender];
            while (aboss!=address(0) && i<3) {
                _basicTransfer(sender,aboss,amount.div(100).mul(power[i]));
                feeBossLeft=feeBossLeft.sub(feeBoss.div(100).mul(power[i]));
                aboss=boss[aboss];
                i++;
            }
        }  
        if(feeBossLeft>0) _basicTransfer(sender, DefaultWalletAddress, feeBossLeft);
        return amount.sub(feeAmount).sub(feeSendDead);
    }
