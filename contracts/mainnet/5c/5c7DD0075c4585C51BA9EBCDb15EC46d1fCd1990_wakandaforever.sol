...
contract wakandaforever is ERC20, Ownable {
    ...
    wakandaforeverDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Wakanda Forever", "WKDF") {
        marketingWallet = payable(0xCe9b36aF851894377c37942A7B65B091B6d00394);
        devWallet = payable(0x2cceA47F9c256e133d2ff53CF6305e453b1199Bc);
        ...
        dividendTracker = new wakandaforeverDividendTracker(
            payable(this),
            router,
            0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56,
            "wakandaforeverTRACKER",
            "WKDFTRACKER"
        );
    ...
    // set max wallet, can not be lower than 0.000005% of supply
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 200000, "max wallet cannot be set to less than 0.000005%");
        maxWallet = value;
    }
...
contract wakandaforeverDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    wakandaforever public wakandaforeverContract;
    ...
    constructor(
        ...
        defaultToken = token;
        wakandaforeverContract = wakandaforever(mainContract);
    ...
    function _transfer(
        ...
        require(false, "wakandaforever_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "wakandaforever_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main wakandaforever contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(wakandaforeverContract);

            uint256 prevBalance = wakandaforeverContract.balanceOf(address(this));
            ...
            {
                uint256 received = wakandaforeverContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    wakandaforeverContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
