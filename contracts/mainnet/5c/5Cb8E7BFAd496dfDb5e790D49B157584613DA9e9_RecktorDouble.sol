...
contract RecktorDouble {
  ...
  function dualDexTrade(
    ...
    uint256 bnbInitalBalance = address(this).balance;
    ...
    if( finalAAmount > tradeableAAmount + tokenAInitialBalance ){
      { // added scope to avoid 'Stack too dip'
        tempPath[0] = path[1];
        tempPath[1] = path[0];
        IRouter(router).swapTokensForETH( finalAAmount, 0, tempPath, pairs[3], address(this), fees[3] );
      }
      require( address(this).balance > bnbInitalBalance + _amountIn, "Trade Reverted, No Profit Made BNB");
      owner.transfer(address(this).balance);
    } else {
      if( activeRevert ){   
        require( finalAAmount > tradeableAAmount + tokenAInitialBalance, "Trade Reverted, No Profit Made TOKENA");
      }
    }
