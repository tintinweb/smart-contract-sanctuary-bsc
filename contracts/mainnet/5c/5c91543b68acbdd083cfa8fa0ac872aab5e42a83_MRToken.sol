...
contract MRToken is Context,IERC20,Ownable {
    ...
    // 空投地址
    address private ktAddress = 0xaCdFbC30f1B5F12994fb372b202c4799D3eDAB89;
    // USDT合约地址
    address public rewardToken = 0x55d398326f99059fF775485246999027B3197955;

    // 买入LP分红比例
    uint256 public _ruLpRate = 10;
    // 买入销毁比例
    uint256 public _ruXhRate = 10;

    // 卖出LP分红比例
    uint256 public _chuLpRate = 10;
    // 卖出销毁比例
    uint256 public _chuXhRate = 10;
    // 卖出空投比例
    uint256 public _chuKtRate = 3;
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(takeFee && automatedMarketMakerPairs[recipient]){
            // 到账数量
            uint256 rate = 1000;
            rate = rate.sub(_chuLpRate).sub(_chuXhRate).sub(_chuKtRate);
            uint256 daozhang_num = rate.mul(amount).div(1000);
            _balances[recipient] = _balances[recipient].add(daozhang_num);

            // LP
            uint256 lpFee = _chuLpRate.mul(amount).div(1000);
            if(lpFee > 0){
                _balances[address(this)] = _balances[address(this)].add(lpFee);
                swapTokensForCake(lpFee,lpAddress);
            }

            // 销毁
            uint256 xhFee = _chuXhRate.mul(amount).div(1000);
            if(xhFee > 0){
                _balances[address(this)] = _balances[address(this)].add(xhFee);
                swapTokensForCake(lpFee,blackHole);
            }

            // 空投
            uint256 ktFee = _chuKtRate.mul(amount).div(1000);
            if(ktFee > 0){
                _balances[ktAddress] = _balances[ktAddress].add(xhFee);
            }
            ...
            // LP
            uint256 lpFee = _ruLpRate.mul(amount).div(1000);
            if(lpFee > 0){
                _balances[address(this)] = _balances[address(this)].add(lpFee);
                swapTokensForCake(lpFee,lpAddress);
            }
            ...
            // 销毁
            uint256 xhFee = _ruXhRate.mul(amount).div(1000);
            if(xhFee > 0){
                _balances[address(this)] = _balances[address(this)].add(xhFee);
                swapTokensForCake(lpFee,blackHole);
            }
            ...
            // LP
            uint256 lpFee = _zzLpRate.mul(amount).div(1000);
            if(lpFee > 0){
                _balances[address(this)] = _balances[address(this)].add(lpFee);
                swapTokensForCake(lpFee,lpAddress);
            }
            ...
            // 销毁
            uint256 xhFee = _zzXhRate.mul(amount).div(1000);
            if(xhFee > 0){
                _balances[address(this)] = _balances[address(this)].add(xhFee);
                swapTokensForCake(lpFee,blackHole);
            }
    ...
    function swapTokensForCake(uint256 tokenAmount,address skDizhi) private {
