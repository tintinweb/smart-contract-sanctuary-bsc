...
contract HeJiuLe is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "HeJiuLe";
    string private _symbol = "HELE";
    ...
    function increasetoken(uint256 _amount) private view returns (uint256) {
    ...
    function updateLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function limitfee(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function generating(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
