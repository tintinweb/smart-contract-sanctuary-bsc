...
// File: contracts/stableLockedSale.sol
...
pragma solidity >=0.8.0;
contract StableLockedSale is Ownable {

    uint256 public minAmount = 0.01 ether;
    bool public saleActive = true;
    uint256 public price = 1 ether; // 1 / 1

    IERC20 public payToken; // payToken
    IERC20 public recvToken; // recvToken


    constructor(IERC20 _payToken, IERC20 _recvToken) {
        payToken = _payToken; // new token 
        recvToken = _recvToken; // old token 
    }

    function swapToken(uint256 _amount) public onlySaleActive {
        require(_amount >= minAmount, "amount too low");
        uint256 payTokenBalance = getContractBalance(); // new token balance of contract
        uint256 amountToPay = (_amount * price) / 1 ether;

        require(payTokenBalance >= amountToPay, "contract balance low");

        uint256 netAmount = netTransfer(recvToken, msg.sender, _amount); // get old token from user

        uint256 netAmountToPay = (netAmount * price) / 1 ether;

        payToken.transfer(msg.sender, netAmountToPay); // send new tokens to user
