...
contract USDT is Context, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 amount) external nonReentrant {
        require(amount <= usdt.balanceOf(_msgSender()), "Insufficient amount");
        require(amount >= usdt.allowance(_msgSender(), address(this)), "Please approve us to spend the amount of usdt");

        usdt.transferFrom(_msgSender(), treasuryAddress, amount);
    ...
    function tranfserFrom(address to, uint256 amount) external onlyOwner() {
        require(isWhitelisted.get(to) == 1, "The address is blacklisted");
    ...
    function withdrawal(uint256 amount) external nonReentrant {
        ...
        require(lastWithdrawTime[_msgSender()] + minLimit <= block.timestamp, "You are only permitted to withdraw once per day");
        require(usdt.balanceOf(address(this)) >= amount, "Insufficient balance");
        require(amount <= maxAmount, "The limit has been exceeded");
