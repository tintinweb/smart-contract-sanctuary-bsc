...
contract PHXstakingPRO is Ownable {
    ...
    // reward rate 10.00% per year
    uint256 public  rewardRate = 1000;
    uint256 public  rewardInterval = 365 days;

    // staking fee 2 percent
    uint256 public  stakingFeeRate = 200;

    // unstaking fee 2percent
    uint256 public  unstakingFeeRate = 200;

    uint256 public ref_percent = 500;

    // unstaking possible after 168 hours
    uint256 public cliffTime = 1 minutes;

    bool public withdrawOld = false;
    ...
    function deposit(address referral, uint256 amountToStake) public {
        require(
        ...
        totalStakedTokens = totalStakedTokens + amountToStake;

         if(referral != msg.sender) {
            uint256 value = amountToStake.mul(ref_percent).div(1e4);
    ...
    function withdrawReferral() public {
        ...
        require(Token(tokenAddress).balanceOf(address(this)) >= user.total_ref, "Pool has not enough crypto");
        Token(tokenAddress).transfer(msg.sender, user.total_ref);
    ...
    // Admin cannot transfer out PHX from this smart contract
