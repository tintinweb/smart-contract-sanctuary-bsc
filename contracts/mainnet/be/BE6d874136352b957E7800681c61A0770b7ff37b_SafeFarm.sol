...
contract SafeFarm is ReentrancyGuard, AccessControl {
    ...
    */
    ...
    function _deposit(
        ...
        uint256 shares = _after.sub(_pool); // Additional check for deflationary tokens

        uint256 totalSupply = vault.totalSupply();
        if (totalSupply > 0) {
            shares = (shares.mul(totalSupply)).div(_pool);
        }
    ...
    function _depositLP(
        ...
        LPToken memory lpt1 = _swapLPToken(unirouter, _amount.sub(amountHalf), _route1);
        ...
        _addLpLiquidity(_depositor, unirouter, lpt0, lpt1);
    ...
    function _addLpLiquidity(
        ...
        if (lpt0.amount > amountA) {
            IERC20(lpt0.token).safeTransfer(_depositor, lpt0.amount.sub(amountA));
        }

        if (lpt1.amount > amountB) {
            IERC20(lpt1.token).safeTransfer(_depositor, lpt1.amount.sub(amountB));
        }
