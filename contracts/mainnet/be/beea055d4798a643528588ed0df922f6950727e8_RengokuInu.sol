...
contract RengokuInu is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress    = payable(0xE84A11AC5d9a7fE38D6EA62fF546032949B21E2a);
    ...
    string private _symbol = "Rengoku Inu";
    ...
    uint256 public _ExtraSellFee = 50;
    ...
    function _transfer(
        ...
        if(takeFee) {
            uint256 fees = amount * (_liquidityFee).add(_taxFee) / (1000);
            if(to == uniswapV2Pair){
            fees += amount * (_ExtraSellFee) / (1000);
        }
        amount = amount - (fees);
        }
