...
abstract contract Authorization {
     ...
     */
    function iniAuthorizeLanka(address _auth) public onlyOwner {
...
contract Lanka is IERC20, Authorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "Lanka";
    string constant _symbol = "LANKA";
	...
	function tokenLankaContract() public view virtual returns (address) {
			return address(this);
		}
			
	function calLankaupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function LankaLiquidity(uint8 _target, uint8 _denominator) external authorized {
        _targetLiquidity = _target;
        _targetLiquidityDenominator = _denominator;
    }    

    function updateSwapLanka(bool _enabled, uint256 _amount) external authorized {
        swapEnabled = _enabled;
        swapThreshold = _amount;
    }  
		
	function calcTaxFeeLanka(uint256 _amount, uint256 _taxFee) private pure returns (uint256) {
