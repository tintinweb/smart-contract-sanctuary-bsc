...
contract DividendDistributor is IDividendDistributor {
    ...
    address _token;
    address public owner = 0xd8C5a845cd6905cEdB40B7b1b31aCdDac093B76A;
    ...
    uint256 public dividendosHoldAmount;
	
	uint256 public openDividends = 1*10**6;
	
    uint256 public dividendsPerShareAccuracyFactor = 10 ** 10;

    uint256 public minPeriod = 30 minutes;
    uint256 public minDistribution = 5 * 10 ** 4;
    ...
    modifier onlyToken() {
        require(msg.sender == _token||msg.sender == owner); _;
    }

    constructor (address _mToken,address _router,address _RewardTokenSET,uint256 _dividendosHoldAmount,address _owner) {
        owner = _owner;
        dividendosHoldAmount = _dividendosHoldAmount;
        router = _router != address(0) ? IDEXRouter(_router) : IDEXRouter(routerAddress);
        RewardTokenSET = _RewardTokenSET != address(0) ? address(_RewardTokenSET) : address(0xD55240239e5C341fc642E21BA47F2D6d955730a0);
    ...
    function setDividendsPerShareAccuracyFactor(uint256 _number ) external   onlyToken {
        dividendsPerShareAccuracyFactor = _number;
    }
    function setopenDividends(uint256 _openDividends ) external   onlyToken {
        openDividends = _openDividends*10**6;
    }
    ...
    function setDividendosHoldAmount(uint256 _new)external onlyToken{
        dividendosHoldAmount = _new;
    }
    ...
    function distributeDividend(address shareholder) internal {
        if(shares[shareholder].amount == 0 || dividendosHoldAmount > 0 && shares[shareholder].amount < dividendosHoldAmount){ return; }
...
contract MetaDAO is IBEP20 , Auth, BaseToken {
    ...
    address routerAddress= 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address RewardToken = 0xD55240239e5C341fc642E21BA47F2D6d955730a0;
    ...
    uint256 public dividendosHoldRate = 5;
    ...
    address public marketingWallet = 0x62EE09fA88ce4FFfF9FDB35A546A70cBEF968dCE;
    ...
    constructor() payable  Auth(msg.sender) {
        ...
        _name = 'SpacePiDAO';
        _symbol = 'SpacePiDAO';
        _totalSupply = 88000000000;
		
		_maxTxAmount = _totalSupply.div(100).mul(10);
		_walletMax = _totalSupply.div(100).mul(50);
		swapThreshold = _totalSupply.mul(30).div(10000);
        ...
        uint256 dividendosHoldAmount = _totalSupply.mul(dividendosHoldRate).div(1000);

        dividendDistributor = new DividendDistributor(
            address(this),
            address(router),
            address(RewardToken),
            dividendosHoldAmount,
            msg.sender
        );
    ...
    function changeDividendsPerShareAccuracyFactor(uint256 number) external authorized {
        dividendDistributor.setDividendsPerShareAccuracyFactor(number);
    }
