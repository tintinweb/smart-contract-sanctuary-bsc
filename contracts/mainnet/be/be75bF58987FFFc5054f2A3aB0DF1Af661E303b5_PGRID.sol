...
contract PGRID is ERC20, ERC20Burnable, Pausable, Ownable, ERC20Permit, ERC20Snapshot {
    ...
    uint256 public maxTokenPurchasableGlobalWei;
    uint256 public maxTokenPurchasablePerAddressWei;
    ...
    constructor() ERC20("Presale GRID-ID Token", "PGRID") ERC20Permit("PGRID") {
        whitelistedAddresses[msg.sender] = true;
        whitelistedAddresses[address(0)] = true;
        buyFee = 18 * 10 ** 3; // -> 18000 = 18%
        whitelistEnabled = false;
        presaleOpen = false;
        fundAutoTransfer = true;
        fundDestination = payable(msg.sender);
        priceForOneTokenInWei = 1 * 10 ** 18;
        maxTokenPurchasableGlobalWei =  10800000 * 10 ** 18;
        maxTokenPurchasablePerAddressWei = 180000 * 10 ** 18;
...
// ---------- Helper functions  ---------- //
    ...
    function setPresalePriceForOneTokenInWei(uint256 _priceForOneTokenInWei) external onlyOwner{
    ...
    function setMaxTokenPurchasableGlobalWei(uint256 _max) external onlyOwner{
        maxTokenPurchasableGlobalWei = _max;
    }

    function setMaxTokenPurchasablePerAddressWei(uint256 _max) external onlyOwner{
        require(_max <= maxTokenPurchasableGlobalWei, "Max purchasable per address cannot be greater than global max");
        maxTokenPurchasablePerAddressWei = _max;
    }
    ...
    function validateBalanceOf(address account) public view returns (uint256) {
        if (!isAddressBlacklisted(account)) {
...
// ---------- Helper functions  ---------- //
    ...
    returns (bool) {
        require(presaleOpen, "Presale is currentely closed");
        require(_beneficiary != address(0), "Zero address cannot buy");
        require(_coinAmountInWei != 0, "Buy amount must be greater than zero");
        require(totalSupply() < maxTokenPurchasableGlobalWei, "All available token already sold");

        return true;
    }

    function postValidatePurchase(address _beneficiary, uint256 _tokenToBuy) public view returns (bool) {
        require(
            (totalSupply() + _tokenToBuy) < maxTokenPurchasableGlobalWei,
            "Buy amount to high (global)"
        );
        require(
            balanceOf(_beneficiary) < maxTokenPurchasablePerAddressWei || _msgSender() == owner(),
            "Max ownable token reached"
        );
        require(
            balanceOf(_beneficiary).add(_tokenToBuy) < maxTokenPurchasablePerAddressWei || _msgSender() == owner(),
            "Buy amount to high (per address)"
        );
    ...
    function buyTokens() public payable {
        ...
        if (preValidatePurchase(msg.sender, msg.value)) {
            ...
            if (postValidatePurchase(msg.sender, tokenToBuy)) {
                // mint tokens to buyer address
                _mint(msg.sender, tokenToBuy);
                ...
                emit TokenPurchase(msg.value, feeValue, remainingCoin, priceForOneTokenInWei, tokenToBuy, fundToTransfer, fundAutoTransfer, totalSupply());
            }
        }
    }
}
