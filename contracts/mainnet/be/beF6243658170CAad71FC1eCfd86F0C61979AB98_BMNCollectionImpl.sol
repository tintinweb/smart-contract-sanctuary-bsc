/**
 * @notice Submitted for verification at bscscan.com on 2022-07-30
 */
...
/**
 * @title A collection of NFTs based on BMNCollectionImpl template
 * @author BlockMelon
 * @notice All NFTs from this contract are minted by the same creator.
 * @dev A 10% royalty to the creator is included
 * A 2% royalty to the creator is included
 * based on the collection contract of Foundation: 0xe38f942db7a1b4213d6213f70c499b59287b01f1 `CollectionContract`
 */
contract BMNCollectionImpl is ICloneInitializer, CollectionMinter {
    using AddressUpgradeable for address;

    /// @notice Emitted when the collection contract is created
     ...
     */
    ICloneFactory public immutable collectionFactory;
    ...
    constructor(address _collectionFactory) {
        require(
            ...
            "collectionFactory is not a contract"
        );
        collectionFactory = ICloneFactory(_collectionFactory);
    }
    ...
    function initialize(
        ...
        require(
            _msgSender() == address(collectionFactory),
            "collection must be created via the factory"
        );
        ...
        __CollectionTokenBase_init_unchained();
        __CollectionCreator_init_unchained();
        __CollectionLockedContent_init_unchained();
        __CollectionFirstOwners_init_unchained();
        __CollectionRoyaltyInfo_init_unchained();
        __CollectionMinter_init_unchained();
        _transferOwnership(creator);
        _setDefaultApprovedMarket(market);
        emit CollectionCreated(creator, contractName, tokenSymbol);
    }
}
 ...
 */
abstract contract CollectionTokenBase is
    OwnableUpgradeable,
    ERC721URIStorageUpgradeable
{
    ...
    mapping(string => bool) private _cidToMinted;

    function __CollectionTokenBase_init_unchained() internal onlyInitializing {}

    modifier isNotMinted(string memory tokenCID) {
        require(!_cidToMinted[tokenCID], "NFT was already minted");
        _;
    }
     ...
     */
    function getHasMintedCID(string memory tokenCID)
        public
        view
        returns (bool)
    {
        return _cidToMinted[tokenCID];
    }

    function _setMintedCID(string memory cid) internal virtual {
        _cidToMinted[cid] = true;
    }

    function _burn(uint256 tokenId) internal virtual override {
        super._burn(tokenId);
        delete _cidToMinted[tokenURI(tokenId)];
    }
...
import "./CollectionRoyaltyInfo.sol";

abstract contract CollectionSupply is CollectionRoyaltyInfo {
    event MaxTokenIdUpdated(uint256 indexed _maxTokenId);
    /**
    ...
    /**
     * @dev Tracks the token supply.
     */
    uint256 private _supply;

    function __CollectionSupply_init_unchained() internal onlyInitializing {}

    /**
     * @dev For compliance with {IERC721Enumerable-totalSupply}.
     */
    ...
    function updateMaxTokenId(uint256 _maxTokenId) external onlyOwner {
        require(_maxTokenId > 0, "Max token ID may not be cleared");
        require(
            maxTokenId == 0 || _maxTokenId < maxTokenId,
            "Max token ID may not increase"
        );
        require(
            latestTokenId + 1 <= _maxTokenId,
            "Max token ID must be greater than last mint"
        );
...
import "./CollectionFirstOwners.sol";
 ...
 */
abstract contract CollectionRoyaltyInfo is
    IHasSecondarySaleFees,
    IGetRoyalties,
    IERC2981Upgradeable,
    CollectionFirstOwners
{
    using AddressUpgradeable for address;
    /*
    ...
    uint256 private constant CREATOR_ROYALTY_IN_BASIS_POINTS = 1000;
    uint256 private constant FIRST_OWNER_ROYALTY_IN_BASIS_POINTS = 200;
    uint256 private constant ROYALTY_RATIO = 10;

    function __CollectionRoyaltyInfo_init_unchained()
        internal
        onlyInitializing
    {}
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(CollectionFirstOwners, IERC165Upgradeable)
        returns (bool)
    {
        return
            ...
            CollectionFirstOwners.supportsInterface(interfaceId);
    }
        ...
        returns (address payable[] memory)
    {
        ...
        result[0] = tokenCreator(tokenId);
        result[1] = firstOwner(tokenId);
        return result;
    }
    ...
    function getFeeBps(
        ...
        uint256[] memory feesInBasisPoints = new uint256[](2);
        feesInBasisPoints[0] = CREATOR_ROYALTY_IN_BASIS_POINTS;
        feesInBasisPoints[1] = FIRST_OWNER_ROYALTY_IN_BASIS_POINTS;
        ...
        )
    {
        ...
        recipients[0] = tokenCreator(tokenId);
        recipients[1] = firstOwner(tokenId);
...
import "./CollectionSupply.sol";

abstract contract CollectionMinter is CollectionSupply {
    event Minted(
        address indexed creator,
    ...
    function __CollectionMinter_init_unchained() internal onlyInitializing {}
    ...
    /**
     * @notice Sets the default approved market contract address for all the token IDs.
     */
    function setDefaultApprovedMarket(address operator) public onlyOwner {
        _setDefaultApprovedMarket(operator);
    }

    /**
     * @dev Creates a new NFT at the address of the owner
     ...
     *       - maxTokenId must be zero or the new token Id must be lower than that
     * @return tokenId The newly minted token ID
     */
    function mint(string memory tokenUri, string memory lockedContent)
        public
        onlyOwner
        isNotMinted(tokenUri)
        returns (uint256 tokenId)
    {
        tokenId = _incrementTokenId();
        require(
            maxTokenId == 0 || tokenId <= maxTokenId,
            "Max token count has already been minted"
        );
        _safeMint(_msgSender(), tokenId);
        _setTokenURI(tokenId, tokenUri);
        _setMintedCID(tokenUri);
...
abstract contract CollectionLockedContent is ILockedContent, CollectionCreator {
    ...
    function __CollectionLockedContent_init_unchained()
        internal
        onlyInitializing
    {}
...
import "./CollectionLockedContent.sol";

abstract contract CollectionFirstOwners is
    IFirstOwner,
    CollectionLockedContent
{
    ...
    function __CollectionFirstOwners_init_unchained()
        internal
        onlyInitializing
    {}
...
import "./CollectionTokenBase.sol";

abstract contract CollectionCreator is ITokenCreator, CollectionTokenBase {
    /// @dev bytes4(keccak256('tokenCreator(uint256)')) == 0x40c1a064
    bytes4 private constant _INTERFACE_ID_TOKEN_CREATOR = 0x40c1a064;

    function __CollectionCreator_init_unchained() internal onlyInitializing {}
