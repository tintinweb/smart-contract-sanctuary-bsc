...
contract Akainu is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"AKAINU サカズキ"; 
    string private constant _symbol = "SAKAZUKI";    
    ...
    function setExcludedFromMaxBalance(address account,bool isExcluded) public onlyOwner {
    ...
    function swapAndLiquify() private lockTheSwap{
        uint tokenAutoLP = balanceOf(address(this)) * 2 / 10;
        uint tokenTreasury = balanceOf(address(this)) * 8 / 10;

        uint balTreasury = swapTokensForEth(tokenTreasury);
        if (balTreasury > 0) payable(marketingWallet).transfer(balTreasury);
