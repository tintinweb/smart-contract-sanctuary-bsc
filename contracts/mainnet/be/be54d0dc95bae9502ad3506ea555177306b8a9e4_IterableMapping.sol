...
contract BitcoinAfrica is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    address public _dividendToken = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c; // BTC
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;

    bool private swapping;
    bool public tradingIsEnabled = false;
    bool public rewardEnable = true;
    bool public sobActive = false;
    bool public coolDownActive = false;

    BTCATokenDividendTracker public dividendTracker;

    address public marketingWallet;
    address public devWallet;

    uint256 public maxSellTransactionAmount = 6500000 * (10**9); // 0.5%
    uint256 public maxWalletBalance = 26000000 * (10**9); // 2%
    uint256 public swapTokensAtAmount = 26000000 * (10**9); // 2%
    ...
    uint256 private abSellCoolDown;
    ...
    constructor() ERC20("Bitcoin Africa", "BTCA") {
        dividendTracker = new BTCATokenDividendTracker();
        ...
        */
        _mint(owner(), 1_300_000_000 * (10**9));
    }

    receive() external payable {}

    // Activate trading on the contract
    function activateContract() public onlyOwner {
        tradingIsEnabled = true;
        sobActive = true;
        coolDownActive = true;
        abSellCoolDown = 30 seconds;
        startBlockNumber = block.number;
    }

    function whitelistExchange(address exchangeAddress) public onlyOwner {
        dividendTracker.excludeFromDividends(exchangeAddress);
        excludeFromFees(exchangeAddress, true);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BTCA: The dividend tracker already has that address"
        );

        BTCATokenDividendTracker newDividendTracker = BTCATokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BTCA: The new dividend tracker must be owned by the BTCA token contract"
        );
    ...
    function updateBuyFees(
        ...
        buyTotalFees = reward
            ...
            .add(marketing)
            .add(development);
        require(buyTotalFees <= 25, "Fees too high");
    }
    ...
    function updateSellFees(
        ...
        sellTotalFees = reward
            ...
            .add(marketing)
            .add(development);
        require(sellTotalFees <= 40, "Fees too high");
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            newAddress != address(uniswapV2Router),
            "BTCA: The router already has that address"
        );
    ...
    function updateMaxTxn(uint256 maxTxnAmount) external onlyOwner {
        ...
        maxSellTransactionAmount = maxTxnAmount * (10**9);
    }

    function updateMaxWalletBalance(uint256 newMaxWalletBalance) external onlyOwner {
        require(newMaxWalletBalance >= 10, "You can't set max wallet balance below 10.");
        maxWalletBalance = newMaxWalletBalance * (10**9);
    }
    ...
    function setABActive(bool active) external onlyOwner() {
        sobActive = active;
    }

    function setCoolDownActive(bool active) external onlyOwner() {
        coolDownActive = active;
    }

    function setBlocksToWait(uint256 numberofBlocks) external onlyOwner {
        blocksToWait = numberofBlocks;
    }

    function setABSellCoolDown(uint256 coolDownTime) external onlyOwner {
        require(coolDownTime <= 300, "Sell cooldown is too long");
        abSellCoolDown = coolDownTime;
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "BTCA: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BTCA: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BTCA: Cannot update gasForProcessing to same value"
        );
    ...
    function markBot(address user, bool value)
        public
        virtual
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (sobActive) {
            require(!_bot[from] && !_bot[to], "No bots allowed");
        }
        ...
        if (automatedMarketMakerPairs[to] && 
            ...
            require(
                ...
                "Exceeds max sell amount"
            );
        }

        if (sobActive) {
            if (automatedMarketMakerPairs[from] && 
                ((block.number < startBlockNumber + blocksToWait) || (lastTrade[to] == block.number)) &&
                !_isExcludedFromFees[to]) {
                _bot[to] = true;
        ...
        if (coolDownActive) {
            if (automatedMarketMakerPairs[to] && !_isExcludedFromFees[from]) {
                require(lastTrade[from] + abSellCoolDown < block.timestamp, 
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        // swap tokens for BNB
        swapTokensForBNB(half); // <- this breaks the BNB -> BTCA swap when swap+liquify is triggered
...
contract BTCATokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("BTCA_Dividend_Tracker", "BTCA_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BTCA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BTCA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BTCA contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BTCA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BTCA_Dividend_Tracker: Cannot update claimWait to same value"
        );
