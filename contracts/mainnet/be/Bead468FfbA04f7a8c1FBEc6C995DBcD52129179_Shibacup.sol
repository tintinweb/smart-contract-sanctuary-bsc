...
contract Shibacup is IERC20 {
    ...
    string constant private _name = "Shibacup";
    string constant private _symbol = "SHIBACUP";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        treasury: payable(0x038FBf5cCE4Fb668965301aa4f8D84e8e7300e3d),
        insurance: payable(0x1d1c2b917daB9Ab26Bd2038C4C937EcB7E40d963)
    });
...
//===============================================================================================================
    ...
    function setWallets(address payable treasury, address payable insurance) external onlyOwner {
    ...
    function setRatios(uint16 treasury, uint16 insurance, uint16 firePit) external onlyOwner {
        _ratios.treasury = treasury;
        _ratios.insurance = insurance;
        _ratios.firePit = firePit;
        _ratios.total = treasury + insurance;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.total + _ratios.firePit <= total, "Cannot exceed sum of buy and sell fees.");
    }
                ...
                if (autoRebaseEnabled
                   && block.timestamp >= autoRebaseLastTriggered + rebaseMinutes
                    ...
                    if (deltaTimeFromInitialization < (365 days)) {
                        rebaseRate = 3078;
                    } else if (deltaTimeFromInitialization >= (365 days)) {
                        rebaseRate = 13; 
                    } else if (deltaTimeFromInitialization >= ((15 * 365 days) / 10)) {
                        rebaseRate = 24;
                    } else if (deltaTimeFromInitialization >= (7 * 365 days)) {
                        rebaseRate = 146;
                    }
                    ...
                    for (uint256 i = 0; i < rebaseAmounts; i++) {
                        tTotal = (tTotal * ((10**_rateDecimals) + _rRate)) / (10**_rateDecimals);
                        if (tTotal >= maxSupply) {
                            tTotal = maxSupply;
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        bool success;
        uint256 treasuryAmount = (amtBalance * ratios.treasury) / ratios.total;
        uint256 insuranceAmount = amtBalance - treasuryAmount;
        if (ratios.treasury > 0) {
            (success,) = _taxWallets.treasury.call{value: treasuryAmount, gas: 35000}("");
        }
        if (ratios.insurance > 0) {
            (success,) = _taxWallets.insurance.call{value: insuranceAmount, gas: 35000}("");
        }
    }
    ...
    function takeTaxes(address from, bool buy, bool sell, uint256 amount, uint256 aSupply) internal returns (uint256) {
        ...
        uint256 firePitAmount = (feeAmount / (ratios.total + ratios.firePit)) * ratios.firePit;
        uint256 swpAmt = feeAmount - firePitAmount;
        if (firePitAmount > 0) {
            _tOwned[DEAD] += firePitAmount;
            emit Transfer(from, DEAD, firePitAmount / aSupply);
        }
        if (swpAmt > 0) {
            _tOwned[address(this)] += swpAmt;
            emit Transfer(from, address(this), swpAmt / aSupply);
        }
