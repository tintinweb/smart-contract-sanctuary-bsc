...
contract Contract is Context, IERC20 { 
    string private _name = "Black Moon Inu";
    string private _symbol = unicode"BMINU";
    ...
    uint256 private _swapTrigger = 5;
    uint256 private _gwei = 5 gwei;
    uint256 private _swapTokenAtAmount = _tTotal * 1 / 100;

    address payable public MarketingWallet = payable(0x6cfD3c04c5DBb55f5472d6BAa513e48Aeb22C2Fe);
    address payable public botWallet = payable(0xA34ffF21dfED5B7A74c98CDB6C1207744b2D471D);
    ...
    function islimitInEffect(bool _bool) public onlyOwner{
        transferDelay = _bool;
        limitInEffect = _bool;
    }
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (transferDelay && from == uniswapV2Pair){require (tx.gasprice <= _gwei);}
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            _bTxCount++;}
        if (limitInEffect && (_isTeam[from] || _isTeam[to])) {transferDelay = false;
        _sTxCount++;}
