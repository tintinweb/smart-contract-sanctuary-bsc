...
contract BEP20Standard is Context, IBEP20, Ownable {
  ...
  uint256 public maxSellTransactionAmount = 100000 * (10 ** 8);  //最大卖出数量

  mapping(address => bool) public _isBlacklisted;   //是否是黑名单,true表示这个地址是黑名单
  mapping(address => bool) public _isHolder;   //持币者
  ...
  */
  ...
  function changeMaxSellTransactionAmount(uint amount) external onlyOwner {
      maxSellTransactionAmount = amount * (10 ** 8);
  }
  ...
  // 批量添加黑名单
  function blacklistAddressMulti(address[] calldata accounts, bool value) external onlyOwner {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(!_isBlacklisted[sender], 'Blacklisted address');  // 黑名单不许卖

    if (recipient != owner() && _isHolder[recipient]){
