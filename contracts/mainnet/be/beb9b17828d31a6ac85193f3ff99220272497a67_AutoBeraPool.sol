...
// File: contracts/AutoBeraPool.sol

pragma solidity 0.6.12;

contract AutoBeraPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 beraAtLastUserAction; // keeps track of cake deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Bera token
    ...
    /**
     * @notice Constructor
     * @param _token: Bera token contract
    ...
    /**
     * @notice Deposits funds into the Bera Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in BERA)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.beraAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Bera Vault
     */
     ...
     * @return Expected reward to collect in BERA
     */
    function calculateHarvestBeraRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBera(0, address(this));
     ...
     * @return Returns total pending bera rewards
     */
    function calculateTotalPendingBeraRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingBera(0, address(this));
        amount = amount.add(available());
