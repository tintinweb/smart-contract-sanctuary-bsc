...
contract LFWRouterV1 {
    ...
    constructor (ILFWDEXUtils _utils) {
        utils = _utils;
    }
    ...
    function swapTokenToToken(uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256[] calldata dexPath, address to) external {
        ...
        uint256 amountOut = utils.getAmountsOut(amountIn, path, dexPath)[1];
        _swap(path, dexPath, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "LFWRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        
        emit SwapTokenToToken(msg.sender, path[0], path[1], dexPath[0], amountIn, amountOut);
    }

    function swapBNBToToken(uint256 amountOutMin, address[] calldata path, uint256[] calldata dexPath, address to) external payable {
        require(path[0] == WBNB, "LFWRouter: INVALID_PATH");
        ...
        emit SwapBNBToToken(msg.sender, path[0], path[1], dexPath[0], amountIn, amountOut);
    }

    function swapTokenToBNB(uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256[] calldata dexPath, address to) external {
        require(path[path.length - 1] == WBNB, "LFWRouter: INVALID_PATH");
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, utils.pairFor(dexPath[0], path[0], path[1]), amountIn
        );
        uint256 amountOutBNB = utils.getAmountsOut(amountIn, path, dexPath)[1];
        _swap(path, dexPath, address(this));
        uint256 amountOut = IBEP20(WBNB).balanceOf(address(this));
        require(amountOut >= amountOutMin, "LFWRouter: INSUFFICIENT_OUTPUT_AMOUNT");
        IWBNB(WBNB).withdraw(amountOut);
        TransferHelper.safeTransferBNB(to, amountOut);
        emit SwapTokenToBNB(msg.sender, path[0], path[1], dexPath[0], amountIn, amountOutBNB);
    }
}