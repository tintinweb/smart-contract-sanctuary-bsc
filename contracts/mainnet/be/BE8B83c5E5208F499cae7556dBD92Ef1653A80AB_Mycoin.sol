...
contract Mycoin is ERC20, Ownable  {
    ...
    uint256 public immutable _burnFee;
    ...
    constructor(
        uint hui_liu_bai_fen_bi, //回流
        uint ying_xiao_bai_fen_bi, //营销
        uint xiao_hui_bai_fen_bi, //销毁
        uint zong_liang, //总量
        string memory ming_cheng, //名称
        string memory fu_hao, //符号
        address ying_xiao_di_zhi  //营销地址
    ) ERC20(ming_cheng, fu_hao) {
        ...
        _liquidityFee = hui_liu_bai_fen_bi;
        _marketFee = ying_xiao_bai_fen_bi;
        _burnFee = xiao_hui_bai_fen_bi;
        marketAddress = ying_xiao_di_zhi;
        numTokensSellToAddToLiquidity = zong_liang * 1e13;
        _maxTxAmount = zong_liang * 1e18;

        _mint(msg.sender, zong_liang * 1e18);
    ...
    function _getFeeValues(uint256 _amount) private view returns (uint256,uint256,uint256,uint256) {
        ...
        uint256 transferAmount = _amount - liquidityFee - marketFee - burnFee;
        return (transferAmount, liquidityFee, marketFee, burnFee);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        if(takeFee){
            (uint256 tTransferAmount, uint256 liquidityFee, uint256 marketFee, uint256 burnFee) = _getFeeValues(amount);

            _balances[recipient] += tTransferAmount;
            _takeLiquidity(sender, liquidityFee);
            _takeMarket(sender, marketFee);
            _takeBurnFee(sender, burnFee);
    ...
    function _takeLiquidity(address sender, uint256 tLiquidity) private {
        if(tLiquidity > 0){
            unchecked { _balances[address(this)] += tLiquidity; }
            emit Transfer(sender, address(this), tLiquidity);
        }
    }
    function _takeMarket(address sender, uint256 tMarketFee) private {
        if(tMarketFee > 0){
            unchecked{ _balances[marketAddress] += tMarketFee; }
            emit Transfer(sender, marketAddress, tMarketFee);
        }
    }
    function _takeBurnFee(address sender, uint256 tBurnFee) private {
        if(tBurnFee > 0){
            address deadAddr = 0x000000000000000000000000000000000000dEaD;
            unchecked{ _balances[deadAddr] += tBurnFee; }
