...
contract BNBMiner is Ownable, ReentrancyGuard {
	...
	uint256 public referralIncome = 2000; 
    ...
    function deposit(address sponsor) external nonReentrant payable{
		...
		if(user.sponsor == address(0)) 
		{
			...
			uint256 vip = sponsorInfo.team;
            
            if (vip < 3) {
                uint256 referralReward = msg.value * 5000 / 10000; 
    ...
    function updateTotalStaked(uint256 _amount) public onlyOwner {

        totalStaked = _amount;
    }
    function updatetoTalClaimed(uint256 _amount) public onlyOwner {

        totalClaimed = _amount;
    }

    function updateTotalReferred(uint256 _amount) public onlyOwner {
