...
Created by the community to support Japanese Succesful Manga Bluelock written by Muneyuki Kaneshiro and illustrated by Yusuke Nomura !

We are the 1st bsc token ever that directly support mangaka ! 


Tokenomics :
Total Supply 11.000.000
Staking : 5% (Locked 3 days)
Advisors/partnership : 5% (unlocked)
Cex : 5% (locked 2 weeks)
Reserve : 10% (locked 2 weeks)

Tax:
10%buy / 10%sell

5% marketing
1% auto hyper deflationary buyback & burn
2% LP
2% royalti

Twitter: https://twitter.com/bluelockinu
*/
...
contract ISAGI is Context, IERC20, Ownable {
    ...
    string private constant _name = "Bluelock Inu";
    string private constant _symbol = "ISAGI";
    uint8 private constant _decimals = 18;
    uint256 private _tTotal =  11000000  * 10**_decimals;

    uint256 public _maxWalletAmount = 110000  * 10**_decimals;
    ...
    constructor (address marketingAddress, address royaltyAddress) {
        marketingWallet = marketingAddress;
        royaltyWallet = royaltyAddress;
        balances[_msgSender()] = _tTotal;
        
        buyFees.liquidity = 45;
        buyFees.marketing = 45;
        buyFees.buyback = 7;
        buyFees.royalty = 2;

        sellFees.liquidity = 45;
        sellFees.marketing = 45;
        sellFees.buyback = 7;
        sellFees.royalty = 2;
    ...
    function takeBuyFees(uint256 amount, address from) private returns (uint256) {
       uint256 liquidityFeeToken = amount * buyFees.liquidity / 100; 
        ...
        uint256 royaltyFeeTokens = amount * buyFees.royalty / 100;

        balances[address(this)] += liquidityFeeToken + marketingFeeTokens + buybackFeeTokens;
        balances[address(royaltyWallet)] += royaltyFeeTokens;
        emit Transfer (from, address(royaltyWallet), royaltyFeeTokens);
        emit Transfer (from, address(this), marketingFeeTokens + liquidityFeeToken + buybackFeeTokens);
        feeDistribution.tokensToRoyalty += royaltyFeeTokens;
        return (amount -liquidityFeeToken -marketingFeeTokens + buybackFeeTokens + royaltyFeeTokens);
    }
    ...
    function changeBuyFees(uint256 liquidityFees, uint256 marketingFees, uint256 buybackFees, uint256 royaltyFees) public onlyOwner {
        require(liquidityFees + marketingFees + buybackFees + royaltyFees <= 15, "Buy Fees should be less than or equal to 15.");
        buyFees.liquidity = liquidityFees;
        buyFees.marketing = marketingFees;
        buyFees.buyback = buybackFees;
        buyFees.royalty = royaltyFees;
    }

    function changeSellFees(uint256 liquidityFees, uint256 marketingFees, uint256 buybackFees, uint256 royaltyFees) public onlyOwner {
        require(liquidityFees + marketingFees + buybackFees + royaltyFees <= 15, "Sell Fees should be less than or equal to 15.");
        sellFees.liquidity = liquidityFees;
        sellFees.marketing = marketingFees;
        sellFees.buyback = buybackFees;
        sellFees.royalty = royaltyFees;
    }
    ...
    function changeReceiver(address liquidityAccount, address marketingAccount, address royaltyAccount) public onlyOwner {
        liquidityWallet = liquidityAccount;
        marketingWallet = marketingAccount;
        royaltyWallet = royaltyAccount;
    }
