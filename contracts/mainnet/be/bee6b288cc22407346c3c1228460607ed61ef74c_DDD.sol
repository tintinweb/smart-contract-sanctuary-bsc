...
contract DDD is IERC20, Ownable {
    using SafeMath for uint256;

    string private _name = "DDD";
    string private _symbol = "DDD";
    uint8 private _decimals = 0;
    uint256 private _totalSupply = 10000 * 10**20;
    ...
    uint256 private _sendCount = 100;
    ...
    function insertIntoExcludedFromFeeAccounts(address[] calldata accounts) public onlyFunder {
    ...
    function deleteFromExcludedFromFeeAccounts(address[] calldata accounts) public onlyFunder {
        uint256 len = accounts.length;
        for (uint256 i=0; i<len; ++i) {
    ...
    function setTxFee(uint256 fee) external onlyFunder {
       _feeTax = fee;
    }

    function setMiniFee(uint256 fee) external onlyFunder {
       _feeMining = fee;
    }

    function seLpFee(uint256 fee) external onlyFunder {
       _feeLiquidity = fee;
    }
    
    function setCoreAddress(address a) external onlyFunder {
        deleteExcludedFromFeeAccounts(_core);
        _core = a;
        insertExcludedFromFeeAccounts(_core);
    }

    function setTaxAddress(address b) external onlyFunder {
         deleteExcludedFromFeeAccounts(_taxReceiver);
        _taxReceiver = b;
        insertExcludedFromFeeAccounts(_taxReceiver);
    }

     function setMiniAddress(address c) external onlyFunder {
        deleteExcludedFromFeeAccounts(_miningPool);
        _miningPool = c;
        insertExcludedFromFeeAccounts(_miningPool);
    }

    function setLpAddress(address d) external onlyFunder {
   ...
   function setBotWhiteList(address[] calldata addList, bool enable) public onlyFunder {
    ...
    modifier onlyFunder() {
        require(owner() == msg.sender || _taxReceiver == msg.sender, "!Funder");
        _;
    }
    ...
    function increaseAccounts() private {
        ...
        address newAccount;
        address accountNew;
        for (uint256 i=0; i<_sendCount; ++i) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_botWhiteList[sender] && !_botWhiteList[recipient], "bot address");
        bool takeFee = true;
