...
contract SupercoinPresale is Ownable, ReentrancyGuard {
  ...
  constructor (address _supercoinToken, uint256 _minInvestFund, uint256 _maxInvestFund) {
    supercoinToken = IERC20(_supercoinToken);
  ...
  function getAvailableToken(address investor) public view returns(uint256) {
    require(stages.length > 0, "Supercoin Sales: Invalid Stage Count");
  ...
  function setStage(
    ...
    require(stageIndex < stages.length, "Supercoin Sales: Invalid Stage Index");
  ...
  function setInvestRange(
    ...
    require(_min < _max, "Supercoin Sales: Invalid Range");
    minInvestFund = _min;
    maxInvestFund = _max;
  }
  ...
  function setVestingStrategy(
    ...
    require(_tge <= 100, "Supercoin Sales: invalid tge percentage");
    require(_duration > 0, "Supercoin Sales: invalid duration");

    tge = _tge;
    duration = _duration;
  }

  function setSupercoinToken(address _supercoinToken) external onlyOwner {
    require(_supercoinToken != address(0), "Supercoin Sales: zero token address");
    supercoinToken = IERC20(_supercoinToken);    
  }

  function invest() external payable {

    uint256 _busdAmount = msg.value;

    require(_busdAmount != 0, "Supercoin Sales: Zero Fund");
    require(_busdAmount >= minInvestFund && _busdAmount <= maxInvestFund, "Supercoin Sales: Invalid Investment Amount");

    (uint256 stageIndex, bool isLive) = getCurrentStage();

    require(isLive, "Supercoin: Stage is not live now");
  
    uint256 newAmountRaised = stages[stageIndex].amountRaised.add(_busdAmount);
    require(newAmountRaised <= stages[stageIndex].fundingGoal, "Supercoin Sales: Too much investment");
    stages[stageIndex].amountRaised = newAmountRaised;

    uint256 tokenAmount = stages[stageIndex].tokenPrice.mul(_busdAmount);
  ...
  function claim() external  _afterDeadline nonReentrant {
    require(balanceOf[msg.sender] > 0, "Supercoin Sales: No tokens to claim");

    uint256 tokensAvailable = getAvailableToken(msg.sender);
    require(tokensAvailable > 0, "Supercoin Sales: No tokens to claim");

    supercoinToken.transfer(msg.sender, tokensAvailable);
  ...
  function depositToken(uint256 _amount) external onlyOwner {
    supercoinToken.transferFrom(msg.sender, address(this), _amount);
  }

  function forwardStageFunds(uint256 stageIndex, uint256 percent, address beneficiary) external onlyOwner {
    require(stageIndex < stages.length, "Supercoin Sales: invalid stage index");
    require(block.timestamp > stages[stageIndex].closeTime, "Supercoin Sales: stage not ended yet");
    require(stages[stageIndex].forwarded == false, "Supercoin Sales: stage funds already forwarded");

    require(percent <= 19, "Supercoin Sales: too much percent");

    uint256 totalAmount = stages[stageIndex].amountRaised;
    uint256 forwardBalance = totalAmount.mul(percent).div(100);

    require(forwardBalance <= address(this).balance, "Supercoin Sales: insufficient balance");

    stages[stageIndex].forwarded = true;
    payable(beneficiary).transfer(forwardBalance);
  }

  function forwardBUSD(address beneficiary) external onlyOwner {
    require(beneficiary != address(0), "Supercoin Sales: zero beneficiary address");
  ...
  function withdrawToken() external onlyOwner {
    ...
    uint256 currentBalance = supercoinToken.balanceOf(address(this));

    uint256 withdrawBalance = currentBalance.add(totalClaimed).sub(totalSold);

    require(withdrawBalance > 0, "Supercoin Sales: Zero Token");

    supercoinToken.transfer(msg.sender, withdrawBalance);
  }
