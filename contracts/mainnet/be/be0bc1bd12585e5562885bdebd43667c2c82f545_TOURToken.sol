...
contract TOURToken is ERC20, SafeOwnable {
    using SafeMath for uint256;
    ...
    uint256 public fundFeePercent = (1 * BASE_RATIO) / 100;
    uint256 public liquidityFeePercent = (3 * BASE_RATIO) / 100;
    uint256 public nftPoolFeePercent = (3 * BASE_RATIO) / 100;
    uint256 public shareholderFeePercent = (2 * BASE_RATIO) / 100;
    uint256 public burnFeePercent = (3* BASE_RATIO) / 100;
    uint256 public currentAllFee =
        fundFeePercent + liquidityFeePercent + nftPoolFeePercent+shareholderFeePercent+burnFeePercent;

    bool private inSwapAndLiquify;
    IERC20 public usdtToken;
    address public smartVault;
    address public nftDividendAddress=0x7d8aba82aD296CEF204ebC67e693a6c6566F4F79;
    address public blackHoleAddress=0x000000000000000000000000000000000000dEaD;
    address public shareholderAddress=0x2c222A6c40666786429a55A3cBD3F19fcF510679;
    address public fundAddress=0xF2707109e5EFa22F965f905fF9C60DaD53D210F6;

    address public usdtAddress=0x55d398326f99059fF775485246999027B3197955;
    address public factoryAddress=0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
    address public routerAddress=0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public subCoinAddress;
    ...
    constructor() ERC20("TOUR", "TOUR") {
        _setupDecimals(18);
        usdtToken = IERC20(usdtAddress);
        IFactory _factory=IFactory(factoryAddress);
        IRouter _router=IRouter(routerAddress);
        liquidity = _factory.createPair(usdtAddress, address(this));
        router = _router;
        rewardEndTime = block.timestamp.add(300 days);
        managerList[_msgSender()]=true;
        liquidityList[liquidity]=true;
        setRewardBlacklist(liquidity, true);
        setRewardBlacklist(blackHoleAddress, true);
        setRewardBlacklist(nftDividendAddress, true);
        setRewardBlacklist(shareholderAddress, true);
        setRewardBlacklist(fundAddress, true);
        setRewardBlacklist(address(this), true);
        setRewardBlacklist(_msgSender(), true);
        subCoinAddress=_msgSender();
    }
   ...
   modifier onlyManager() {
    ...
    function setManager(address account,bool enable) public onlyOwner{
        managerList[account]=enable;
    }
    ...
    function setMinner(address _minner, bool enable) external onlyManager {
        minner[_minner] = enable;
    }


    function setLiquidityList(address account,bool enable) public onlyManager{
        require(account!=address(0),"error");
        liquidityList[account]=enable;
    }
    ...
    function setNFTPool(address _nftPool) external onlyManager {
        nftDividendAddress = _nftPool;
    }

    function setFund(address _fund) external onlyManager {
        fundAddress = _fund;
    }
    ...
    function _transfer(
        ...
        if(swapAndLiquifyEnabled&&!inSwapAndLiquify&&balanceOf(address(this))>0){
        ...
        uint256 amount
          ...
          if(excludeFeeList[from]||excludeFeeList[to]){
              return amount;
               ...
               if (nftDividendAddress != address(0) && nftFee > 0) {
                   realAmount = realAmount.sub(nftFee);
                   super._transfer(account, nftDividendAddress, nftFee);
                }
                uint256 fundFee = amount.mul(fundFeePercent).div(BASE_RATIO);
                if (fundAddress != address(0) && fundFee > 0) {
                    realAmount = realAmount.sub(fundFee);
                    super._transfer(account, fundAddress, fundFee);
                }
                ...
                uint256 burnFee = amount.mul(burnFeePercent).div(BASE_RATIO);
                if(liquidityList[to]){
                   uint256 fee= checkFee();
                   if(fee>0){
                       burnFee=burnFee.add(amount.mul(fee).div(100));
                   }
                }
                if (burnFee > 0) {
                    realAmount = realAmount.sub(burnFee);
                    super._transfer(account, blackHoleAddress, burnFee);
                }
                uint256 shareHolderFee = amount.mul(shareholderFeePercent).div(BASE_RATIO);
                if (shareHolderFee > 0) {
                    realAmount = realAmount.sub(shareHolderFee);
                    super._transfer(account, shareholderAddress, shareHolderFee);
                }
    ...
    function checkFee() public view returns(uint256 fee){
      ...
      uint256 price=reserveU.mul(10**10).div(reserveT);
        ...
        if(price>1000000000){
            return 0;
        }else if(price>500000000){//0.005*10**10
            return 5;    
        }else if(price>100000000){//0.001*10**10
            return 10;    
        }else{
            return 15;
        }
    ...
    function setSubCoinAddress(address account) public onlyManager{
        require(account!=address(0),"error");
    ...
    function setExcludeFeeList(address account,bool enable)public onlyManager{
          excludeFeeList[account]=enable;  
    }
    function setRewardBlacklist(address account, bool enable) public onlyManager {
