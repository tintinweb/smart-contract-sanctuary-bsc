...
contract BomberzillaGame is AccessControl, ERC721Holder, ERC1155Holder, Rescueable {
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("BomberzillaGame")),
                keccak256(bytes("1")),
                chainId,
                address(this)
            )
    ...
    function cancelNonce(uint256 nonce) public notBlacklisted {
        _cancelNonce(nonce, msg.sender);
    }

    function cancelNonce(uint256 nonce, address account) public onlyOwner {
        _cancelNonce(nonce, account);
    }

    function _cancelNonce(uint256 nonce, address account) internal {
        require(!claimed[account][nonce], "BomberzillaGame: Nonce already claimed");
        cancelled[account][nonce] = true;
    ...
    function cancelNonceBatch(uint256[] calldata nonces) public notBlacklisted {
        ...
        for (uint256 i = 0; i < nonces.length; i++) {
            _cancelNonce(nonces[i],account);
        }
    }

    function cancelNonceBatch(uint256[] calldata nonces,address[] calldata accounts) public onlyOwner {
        for (uint256 i = 0; i < nonces.length; i++) {
            _cancelNonce(nonces[i],accounts[i]);
        }
    }
