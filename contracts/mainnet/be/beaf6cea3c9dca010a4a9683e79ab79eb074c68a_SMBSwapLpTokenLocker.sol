...
contract SMBSwapLpTokenLocker is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(address _feesCalculator, address payable _feesReceiver,
        address _feeToken) {
        feesCalculator = IFeesCalculator(_feesCalculator);
        feeReceiver = _feesReceiver;
        feeToken = IERC20(_feeToken);
        //keyToken = new ERC20("SMBSwap Key Token","SMBSK");
    }
    
    function subString(string memory str, uint256 startIndex, uint endIndex)public pure returns (string memory )
    {
    ...
    *                       1 - pay fees in SELF + LP token,
    *                       2 - pay fees fully in BNB,
    *                       3 - pay fees fully in SELF
    */
    function lockTokens(address lpToken, uint256 amount, uint256 unlockTime,
        ...
        string memory token_name = IERC20(lpToken).name();
        require(bytes(token_name).length < 2 || keccak256(bytes(subString(token_name,bytes(token_name).length-3,bytes(token_name).length-1))) != keccak256(bytes("LP")), "INVALID LP Token");
