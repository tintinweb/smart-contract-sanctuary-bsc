...
contract Recktor is Ownable {
  ...
  function dualDexTrade(
    address[2] memory routers, // r0, r1
    address[2] memory tokens, // A, B
    uint256[2] memory slippage, // [ slippage A -> B, slippage B -> A ] 
    uint256 _amountIn
  ) external onlyOwner {
    uint startBalance = IERC20(tokens[0]).balanceOf(address(this));
    uint token2InitialBalance = IERC20(tokens[1]).balanceOf(address(this));
    swap(routers[0],tokens[0], tokens[1],_amountIn, slippage[0]);
    uint token2Balance = IERC20(tokens[1]).balanceOf(address(this));
    uint tradeableAmount = token2Balance - token2InitialBalance;
    swap(routers[1],tokens[1], tokens[0],tradeableAmount, slippage[1]);
  ...
  function instaTrade(
    ...
    if(  path[0] != 0x0000000000000000000000000000000000000000 ) { 
      ...
      swap(routers[0], path[0], path[1], _amountIn, slippage[0] ); // STABLE -> A
    ...
    // swap(routers[4], path[1], path[0], tradeableAAmountFinal, slippage[4]); // A -> STABLE
