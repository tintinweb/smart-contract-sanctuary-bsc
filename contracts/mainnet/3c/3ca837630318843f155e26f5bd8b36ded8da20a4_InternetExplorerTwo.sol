...
library Address{
 ...
 contract InternetExplorerTwo is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0xF5ec0B0458c847bDc4f2F2B0BfDA02F5B21ed6D6;
    address public ExplorerWallet = 0xbbd43cEb267F46CDD3247E86D53cD168496cE8Fe;

    string private constant _name = "Internet Explorer 2.0";
    string private constant _symbol = "EXP2";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[ExplorerWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[ExplorerWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Explorer) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Explorer);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Explorer) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Explorer);
        emit FeesChanged();
    }
    ...
    function _takeExplorer(uint256 rExplorer, uint256 tExplorer) private {
        totFeesPaid.Explorer +=tExplorer;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tExplorer;
        }
        _rOwned[address(this)] +=rExplorer;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rExplorer) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tExplorer = tAmount*temp.Explorer/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tExplorer;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rExplorer = s.tExplorer*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rExplorer;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rExplorer) {

        if(!takeFee) {
          return(0);
        }

        rExplorer = s.tExplorer*currentRate;
        return (rExplorer);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tExplorer);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rExplorer > 0 || s.tExplorer > 0) _takeExplorer(s.rExplorer, s.tExplorer);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Explorer) * 2;
        ...
        uint256 ExplorerAmt = unitBalance * 2 * temp.Explorer;
        if(ExplorerAmt > 0){
            payable(ExplorerWallet).sendValue(ExplorerAmt);
        }
    }
    ...
    function updateExplorerWallet(address newWallet) external onlyOwner{
        ExplorerWallet = newWallet;
    }
