 ...
 * owner.
 */
contract FinoVesting is Ownable {
     ...
     */
    constructor (address beneficiary_, IERC20 finoToken_, uint256 start_, uint256 cliffDuration_, uint256 duration_, bool revocable_) {
    ...
    function released() public view returns (uint256) {
        return _released[address(_finoToken)];
    }
    ...
    function revoked() public view returns (bool) {
        return _revoked[address(_finoToken)];
    }
    ...
    function release() public {
        uint256 unreleased = _releasableAmount(_finoToken);

        require(unreleased > 0, "TokenVesting: no tokens are due");

        _released[address(_finoToken)] = _released[address(_finoToken)].add(unreleased);

        _finoToken.safeTransfer(_beneficiary, unreleased);

        emit TokensReleased(address(_finoToken), unreleased);
    }
    ...
    function revoke() public onlyOwner {
        ...
        require(!_revoked[address(_finoToken)], "TokenVesting: token already revoked");

        uint256 balance = _finoToken.balanceOf(address(this));

        uint256 unreleased = _releasableAmount(_finoToken);
        uint256 refund = balance.sub(unreleased);

        _revoked[address(_finoToken)] = true;

        _finoToken.safeTransfer(owner(), refund);

        emit TokenVestingRevoked(address(_finoToken));
    }
