...
contract UsdtWrap {
    ...
    //会把这个创建出来的合约地址自动赋值给_token520
    constructor (IERC20 _token520)  {
        token520 = _token520;
        usdt = IERC20(0x55d398326f99059fF775485246999027B3197955);
    }
    //调用这个函数时，会从当前调用者地址把usdt和币发送到合约地址
    function withdraw() public {
        
        //合约地址里面的usdt数量
        //把合约里有的usdt数量的多少赋值给usdtBalance
        uint256 usdtBalance = usdt.balanceOf(address(this));
        if (usdtBalance > 0) {
            //从当前调用者地址给合约地址转USDT，转的数量为当前合约里面有的usdt数量
            usdt.transfer(address(token520), usdtBalance);
        }

        //合约地址里有的代币数量
        //把合约地址里有的代币数量赋值给token520Balance
...
contract GOLD is ERC20 {
    ...
    uint256 public totalFee = lpFee+marketFee+burnFee;
    ...
    constructor() ERC20("zzz", "zzz") {
        ...
        _tokenOwner = address(0x158f1e72E10D9EC7bc046928237F17ef019F252D);
        addresszero = 0x000000000000000000000000000000000000dEaD;
        marketAddress = address(0xeBa2e8c55E74a41563F33802B0d7d9b8b2b142A5);
        ...
        _mint(_tokenOwner, 50000 * 10**18);
        
        divLpHolderAmount = 1 * 10**18;
        lpTokenDivThres = 1 * 10**18;
    ...
    function setFees(uint256 _lpFee,uint256 _marketFee,uint256 _burnFee) external onlyOwner{
        lpFee = _lpFee;
        marketFee = _marketFee;
        burnFee = _burnFee;
        totalFee = lpFee+marketFee+burnFee;
    }
    ...
    function lpDividendProc(address[] memory lpAddresses) private {
        for(uint256 i = 0; i< lpAddresses.length; i++){
            ...
            //
                ...
                if(!Address.isContract(lpAddresses[i]) && !lpPush[lpAddresses[i]] && !_bexAddress[lpAddresses[i]]&& uniswapV2Pair.balanceOf(lpAddresses[i]) >= divLpHolderAmount)
                {
                    //
    ...
    function setkaiguan(bool _kaiguan) public onlyOwner {
        kaiguan = _kaiguan;
    }
    ...
    function _transfer(
        ...
        require(!kaiguan,"kaiguan is close");
        ...
        if (takeFee) { 
            ...
            if(ammPairs[to] || ammPairs[from]){
                ...
                _takeburnFee(from,to,share.mul(burnFee));

                super._transfer(from, address(this), share.mul(totalFee-burnFee));
                feeAmount = feeAmount.add(share.mul(totalFee-burnFee));
    ...
    function _takeburnFee(
        ...
        uint256 burnamount = buAmount;
        address burnrecipient = recipient;
        burnrecipient = addresszero;
        super._transfer(sender, burnrecipient, burnamount);
    }
