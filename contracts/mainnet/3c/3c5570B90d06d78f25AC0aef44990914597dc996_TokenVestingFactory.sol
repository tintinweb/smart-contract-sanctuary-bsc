...
contract TokenVestingFactory is Ownable {
    ...
    {
        return vestingForAddress[beneficiary];
    }
    ...
    function deployVesting(
        ...
        require(
            vestingForAddress[beneficiary] == address(0),
            "User is already vested"
        );
        bytes memory bytecode = abi.encodePacked(
            type(TokenVesting).creationCode,
            abi.encode(beneficiary, start, cliffDuration, duration, revocable)
        );
