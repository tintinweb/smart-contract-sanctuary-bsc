...
contract amyfootball_lottery_result {
    ...
    function checklotteryFromHash(uint256 _blockHash,uint256 lottoType)  public pure returns(uint256, uint256,uint256) {
        uint256 crew = uint256(_blockHash) % lottoType;
            return (crew, _blockHash,lottoType);
    }
}