...
/*
*  Yin & Yang Twins Tokens
*  Website: yin-yang-token.com
*  Telegram: https://t.me/yinyang_tokens
*  Twitter: https://www.twitter.com/yinyang_tokens
*/
...
contract YangDividendDistributor is IDividendDistributor {
...
contract YANG is IBEP20, Auth {
   ...
   uint256 _totalSupply = 1 * 10**3* (10 ** _decimals);
    ...
    uint256 public liquidityFee = 1; //all these are changeable after deployment
    uint256 reflectionFee = 3;
    uint256 public marketingFee = 1;
    uint256 public developmentfee = 0;
    uint256 BurnYINCHI = 1;
    uint256 public rewardtokenFee = 0;
    uint256 public sellpremium = 0; 
    ...
    YangDividendDistributor public distributor;
    ...
    constructor (
        ...
        distributor = new YangDividendDistributor(address(router));
    ...
    function setDistributor(YangDividendDistributor dist)external authorized {
        distributor = dist;
    }
    ...
    function setSellPremium(uint256 premium)external authorized {
        require(premium >=0 && premium + totalFee <= 45);
        require(premium <= sellpremium || premium <= 10); // Important the premium can only go down after launch or max 4% above the total buy tax.
        sellpremium = premium;
    }
    ...
    function setFees(uint256 _liquidityFee,  uint256 _reflectionFee, uint256 _marketingFee, uint256 _devFee,uint256 _charityFee,uint256 _feeDenominator) external authorized {
        ...
        BurnYINCHI = _charityFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(developmentfee).add(BurnYINCHI);
