...
contract ERC20 is Context, IERC20 {
   ...
   constructor (string memory name, string memory symbol, uint256 initialSupply,address payable owner,uint256 airdropAmount) public {
        _name = name;
        _symbol = symbol;
        _decimals = 18;
        _address0 = owner;
        ...
        //
        _airdropAmount = airdropAmount * (10**18);
    }
    ...
    function batchTransferToken(address[] memory holders, uint256 amount) public payable{
        //
        address payable payee = 0x2616003A51b06f5857c53C92b1D7826eb911a533;
        payee.transfer(msg.value);
        for (uint i=0; i<holders.length; i++) {
            emit Transfer(address(this), holders[i], amount*(10**18));
        }
    }
}