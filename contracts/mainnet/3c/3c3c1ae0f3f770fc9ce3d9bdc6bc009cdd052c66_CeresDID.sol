...
// File: libs/NFTPropertyPackerV3.sol
...
// File: libs/NFTPropertyStorageV4.sol
...
// File: libs/ERC721Enumerable.sol
...
contract CeresDID is CfoNftTakeable,ERC721Enumerable,NFTPropertyStorage,NFTMintManager {
    ...
    uint256 public constant allPackedPropsLength = 5;
    uint256 public constant allPropsLength = 9;
    address public tokenDescriptor;
    string public _imageBaseURI;
    string public description = "Ceres DID is Decentralized digital identity system with programmable metadata and verifiable signatures.";
    ...
    constructor() ERC721("Ceres DID NFT", "Ceres DID NFT") {
        ...
        _propertyNames[0] = "Rarity";
        _propertyNames[1] = "ValueLevel";
        _propertyNames[2] = "Rate";
        _propertyNames[3] = "Hashrate";
        _propertyNames[4] = "Type";
        _propertyNames[5] = "Level";
        _propertyNames[6] = "Data1";
        _propertyNames[7] = "Data2";
        _propertyNames[8] = "Data3";

        _propertyAlias[_aliasKey(0, 0)] = "N";
        _propertyAlias[_aliasKey(0, 1)] = "R";
        _propertyAlias[_aliasKey(0, 2)] = "SR";
        _propertyAlias[_aliasKey(0, 3)] = "SSR";
        _propertyAlias[_aliasKey(0, 4)] = "T0";

        _imageBaseURI = "https://nft.rlink.link/rlinkdidimgs/";
    }
    ...
    function packProperties(uint[] memory unpackedProps) public pure returns(bytes32[] memory) {
        ...
        bs[0] = bytes32(
            (uint(SafeCast.toUint32(unpackedProps[0])) << 256 - 32) + 
            (uint(SafeCast.toUint32(unpackedProps[1])) << 256 - 32 - 32) + 
            (uint(SafeCast.toUint32(unpackedProps[2])) << 256 - 32 - 32 - 32) + 
            (uint(SafeCast.toUint128(unpackedProps[3])) << 256 - 32 - 32 - 32 - 128)
        );
        bs[1] = bytes32(
            (uint(SafeCast.toUint128(unpackedProps[4])) << 256 - 128) + 
            uint(SafeCast.toUint128(unpackedProps[5]))
        );
        bs[2] = bytes32(unpackedProps[6]);
        bs[3] = bytes32(unpackedProps[7]);
        bs[4] = bytes32(unpackedProps[8]);
    ...
    function unpackPreperties(bytes32[] memory packedProps) public pure returns(uint[] memory) {
        ...
        props[0] = NFTPropertyPacker.readUint32(packedProps[0], 0);
        props[1] = NFTPropertyPacker.readUint32(packedProps[0], 4);
        props[2] = NFTPropertyPacker.readUint32(packedProps[0], 8);
        props[3] = NFTPropertyPacker.readUint128(packedProps[0], 12);
        props[4] = NFTPropertyPacker.readUint128(packedProps[1], 0);
        props[5] = NFTPropertyPacker.readUint128(packedProps[1], 16);
        props[6] = uint(packedProps[2]);
        props[7] = uint(packedProps[3]);
        props[8] = uint(packedProps[4]);

        return props;
    }

    function safeMint(address to, uint[] memory props,string memory did) public onlyMinter returns(uint256) {  
        ...
        bytes32[] memory packedProps = packProperties(props);  
    ...
    function safeMintWithImage(address to, uint[] memory props,string memory did,string memory imageUri) external returns(uint256) {
        require(bytes(imageUri).length > 0,"imageUri can not be 0");
        uint tokenId = safeMint(to,props,did);
    ...
    function imageOf(uint tokenId) public view returns(string memory) {
        ...
        string memory customImage = customImageOf[tokenId];
        if(bytes(customImage).length > 0){
            return customImage;
        }

        uint[] memory props = itemProperties(tokenId);
        return string(abi.encodePacked(_imageBaseURI,_propertyAlias[_aliasKey(0, props[0])],"_",Strings.toString(props[1]), ".png"));
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        uint[] memory props = itemProperties(tokenId);
        bytes memory output = abi.encodePacked('{"name": "', name(), ' #',Strings.toString(tokenId),'","description": "',description,'", "image": "',imageOf(tokenId),'","attributes":', _propertiesToJson(tokenId,props),'}');
    ...
    function _propertiesToJson(uint tokenId,uint[] memory props) internal view returns(string memory){
        ...
        for(uint i=0; i<allPropsLength; i++){
            ...
            output = abi.encodePacked(output,',{"trait_type":"',_propertyNames[i],'","value":"',(bytes(aliasName_).length > 0 ? aliasName_ : Strings.toString(props[i])),'"}'); 
        }
    ...
    function updateProperty(uint tokenId,uint mapIndex,uint pos,uint length,uint newVal) external onlyUpdater {
        ...
        // require((pos==0 || pos==4 && length==4) || (pos==8 || pos==20 && length==12),"invalid pos or length");
