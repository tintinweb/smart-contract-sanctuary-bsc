...
contract MintverseMysteryBoxIssuer is AccessControlEnumerable {
    ...
    function setManager(address value) external onlyRole(MANAGER_ROLE) {
        manager = DragonaireNFTManager(value);
    }

    function setFundsRecipient(address payable value) external onlyRole(MANAGER_ROLE) {
        fundsRecipient = value;
    }

    function purchase(uint256 amount) external payable {
        require(currentSupply + amount <= maxSupply, "MintverseMysteryBoxIssuer: max supply reached");

        uint256 totalPrice = price * amount;
        currentSupply += amount;
        paymentToken.transferFrom(msg.sender, fundsRecipient, totalPrice);

        manager.mintBatch(msg.sender, 1, amount);
    }
}
