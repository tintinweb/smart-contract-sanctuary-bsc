...
contract LPTokenWrapper is IPool,Governance {
    ...
    address public usdt = 0xC7f3a226295e45f79807bb6172E4F11457f5952b;

    address public _stakeToken; 

    uint256 public _oneRate = 1;
    uint256 public _twoRate = 1;

    uint256 public _totalSupply;
    uint256 private _totalSupplytwo;
    mapping(address => uint256) public _balanceusdt;
    mapping(address => uint256) public _balancesBCC;

    function totalSupply() public view returns (uint256) {
        return _totalSupply;
    }
    
    function set_token(address new_token) public  onlyGovernance {
        _stakeToken = new_token;
    }

    function balanceOfBCC(address account) public view returns (uint256) {
        return _balancesBCC[account];
    }

    function balanceOf(address account) public view returns (uint256) {
        return _balanceusdt[account];
    }

    function stake(uint256 amountone, uint256 amounttwo) internal {
        _totalSupply = _totalSupply.add(amountone);
        _balanceusdt[msg.sender] = _balanceusdt[msg.sender].add(amountone);
        _balancesBCC[msg.sender] = _balancesBCC[msg.sender].add(amounttwo);

        IERC20( usdt).safeTransferFrom(msg.sender, address(this), amountone);
        IERC20( _stakeToken).safeTransferFrom(msg.sender, address(this), amounttwo);
    }
}
...
contract UniswapReward is LPTokenWrapper{
    ...
    constructor(uint _lockperiod, address new_stakeToken,uint new_powerrate, address[] memory _route,address new_rewardToken) public {
    ...
    function set_pwr_address(address _pwrCtrl)public onlyGovernance{
        pwrCtrl = _pwrCtrl;
    }
    ...
    function seize(address token, uint256 amount) external onlyGovernance{
        require(token != usdt, "reward");
        require(token != _stakeToken, "stake");
        IERC20( token).safeTransfer(_governance, amount);
    }
        ...
        checkStart
    {
        ...
        amount= amount.mul(200*1e18);         //todo
        uint[] memory tokennum = uniswap(swapAddr).getAmountsOut(amount,route);
        super.stake(amount,tokennum[tokennum.length-1]);
    ...
    function withdraw(uint256 amount)   //usdt
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    {
        require(_lastStakedTime[msg.sender] - block.timestamp >= lockperiod,"locked");
        require(amount > 0, "Cannot withdraw 0");

        uint wrate = amount.mul(1e18).div(_balanceusdt[msg.sender]);
        uint bccnum = _balancesBCC[msg.sender].mul(wrate).div(1e18);

        _totalSupply = _totalSupply.sub(amount);
        _balanceusdt[msg.sender] = _balanceusdt[msg.sender].sub(amount);
        _balancesBCC[msg.sender] = _balancesBCC[msg.sender].sub(bccnum);
        
       IERC20(usdt).safeTransfer( msg.sender, amount);
       IERC20(_stakeToken).safeTransfer( msg.sender, bccnum);
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish) {
            _rewardRate = totalSupply().mul(2).mul(_powerrate).div(100).div(DURATION);
            _periodFinish = block.timestamp.add(DURATION);

            tokenCon(address(_rewardToken)).mint(address(this),totalSupply().mul(2).mul(_powerrate).div(100));
        ...
        updateReward(address(0))
    {
        ...
        tokenCon(address(_rewardToken)).mint(address(this),_initReward);
