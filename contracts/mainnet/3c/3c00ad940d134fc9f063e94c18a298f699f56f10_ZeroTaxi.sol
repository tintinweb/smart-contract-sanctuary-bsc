...
contract ZeroTaxi is Ownable {
    address private _owner;
    string private _name = "ZeroTaxi";
    string private _symbol = "ZTI";
    ...
    function getLpTime() public view returns(uint256) {
        return lpAddedAt;
    }
    ...
    function transferFrom(
        ...
        // Check if sells are allowed yet, this will block instant sell bots
          if (_to == uniswapV2Pair && !_excluded[_from]) {
              // Sell is allowed at the lp block and next block
              if (block.timestamp > lpAddedAt + 4) {
                  // After that there will be a cooldown
                  require(block.timestamp > lpAddedAt + 60 * coolDown, "cooldown");
              }
        }
