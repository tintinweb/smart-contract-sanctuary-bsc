...
interface IPlayerBook {
    function stakeregister(address ac) external;
...
contract Governance {
    ...
    address public _distribution2;

    constructor() public {
        _governance = tx.origin;
        _distribution = msg.sender;
        _distribution2 = msg.sender;
    }

    event GovernanceTransferred(address indexed previousOwner, address indexed newOwner);

    modifier onlyGovernance {
        require(msg.sender == _governance || msg.sender == _distribution || msg.sender == _distribution2, "not governance");
        _;
    }
    ...
    function setDistribution2(address distribution)  public  onlyGovernance
    {
        require(distribution != address(0), "new governance the zero address");
        _distribution2 = distribution;
    }
...
contract LPTokenWrapper is IPool,Governance {
    ...
    function stakeinternal(address account,uint256 amount) internal {
        _totalSupply = _totalSupply.add(amount);
        _balances[account] = _balances[account].add(amount);

        
        _totalPower = _totalSupply;
        _powerBalances[account] = _balances[account];

    }

    function withdrawinternal(address account,uint256 amount) internal {

        require(_balances[account] >= amount,"withdraw err");
...
contract UniswapReward is LPTokenWrapper{
    ...
    constructor(address _new_rewardaddr,address _new_bccaddr) public {
        _rewardToken = IERC20(_new_rewardaddr);
        _BCCToken =  IERC20(_new_bccaddr);
    }
    ...
    function stake(uint amounts)
        public
        updateReward(msg.sender)
        checkHalve
        checkStart
    {
        require(amounts > 0, "Cannot stake 0");
        require(_isSwap, "_isSwap err");
        _BCCToken.safeTransferFrom(msg.sender, address(this), amounts.mul(desNum));
        stakeinternal(msg.sender,amounts);
    ...
    function updateAll(address account)
        internal
    {
        // _rewardPerTokenStored = rewardPerToken();
        // _lastUpdateTime = lastTimeRewardApplicable();
    ...
    function setIsSwap( bool IsSwap) external onlyGovernance {
        _isSwap = IsSwap;
    }

    function addWhiteList(address[] calldata addrArry,uint[] calldata amountArry) external onlyGovernance {
        require(amountArry.length >= addrArry.length,"err");
        for(uint i = 0;i<addrArry.length;i++)
        {
            stakeinternal(addrArry[i],amountArry[i]);
        }
    }

    function subWhiteList(address[] calldata addrArry,uint[] calldata amountArry) external onlyGovernance {
        require(amountArry.length >= addrArry.length,"err");
        for(uint i = 0;i<addrArry.length;i++)
        {
            withdrawinternal(addrArry[i],amountArry[i]);
        }
    }
