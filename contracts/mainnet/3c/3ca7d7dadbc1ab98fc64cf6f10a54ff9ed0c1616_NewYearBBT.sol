...
// Project description: Auto buyback New Year token - trade for get more price at 2023
// https://newyear-bbt.xyz
// https://t.me/NYBBT
...
contract NewYearBBT is ERC20, Ownable, Pausable {
    ...
    uint256 private swapThreshold = 0.001 ether; // The contract will only swap to ETH, once the fee tokens reach the specified threshold
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if (antiBot) {
                    require(_lastBuyBlock[from] != block.number, "CoinToken: Bad bot!");
                }
    ...
    /**
     * @dev Burns tokens from charity address.
     */
    ...
    /**
     * @dev Show current buy taxes.
     */
    function showBuyTaxes() public view returns(uint256, uint256, uint256, uint256) {
        return (buyTaxes["dev"], buyTaxes["marketing"], buyTaxes["liquidity"], buyTaxes["charity"]);
    }

    /**
     * @dev Show current sell taxes.
     */
    function showSellTaxes() public view returns(uint256, uint256, uint256, uint256) {
        return (sellTaxes["dev"], sellTaxes["marketing"], sellTaxes["liquidity"], sellTaxes["charity"]);
    }
