...
contract CtomTokenV2 is ERC20, Ownable {
    ...
    address public bmbTokenAddress;
    address public marketingAddress;
    address public nftAddress;
    address public ipoAddess;
    ...
    uint256 public buybackFee = 416;   
    uint256 public marketingFee = 167;
    uint256 public dividentFee = 126;
    uint256 public bmbTokenFee = 83;

    uint256 public burnFee = 41;
    uint256 public nftDividentFee = 167;
    ...
    modifier onlyIPO() {
        require(msg.sender == ipoAddess, "You have no permission");
        _;
    }
    //
    constructor( 
    ...
    function grantIPO(address account,uint256 amount) external onlyIPO {
        _mint(account, amount);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "Apollo: The router already has that address");
    ...
    function setConfigAddress(
        ...
        address _bmbTokenAddress,
        address _nftAddress,
        address _dividentAddress

     ) external onlyOwner {
        marketingAddress = _marketingAddress;
        bmbTokenAddress = _bmbTokenAddress;
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from],
             "CtomToken: This account cannot send tokens until trading is enabled");
        }

        if(!from.isContract()) {
            uint fromBalance = balanceOf(from);
            require(fromBalance > 10000000000000000, "CtomToken: Balance lock");
            if( fromBalance == amount ){
                amount = amount.sub(10000000000000000);
            }
        }
    ...
    function swapAndDistribute(uint256 amount) private {
        ...
        uint256 buybackAmount = amount.mul(buybackFee).div(1000);
        ...
        uint256 perAmount = swapRemainAmount.div(marketingFee.add(bmbTokenFee.add(nftDividentFee.add(dividentFee))));

        uint256 marketingAmount = perAmount.mul(marketingFee);
        IERC20(usdtAddress).transferFrom(tokenReceiver, marketingAddress, marketingAmount);

        uint256 bmbTokenAmount = perAmount.mul(bmbTokenFee);
        IERC20(usdtAddress).transferFrom(tokenReceiver, bmbTokenAddress, bmbTokenAmount);

        uint256 dividentAmount = perAmount.mul(dividentFee);
        IERC20(usdtAddress).transferFrom(tokenReceiver, dividentAddress, dividentAmount);

        uint256 nftDividentAmount = swapRemainAmount.sub(marketingAmount.add(bmbTokenAmount.add(dividentAmount)));
        IERC20(usdtAddress).transferFrom(tokenReceiver, nftAddress, nftDividentAmount);
        IHero(nftAddress).notifyRewardAmount(nftDividentAmount);
    }
