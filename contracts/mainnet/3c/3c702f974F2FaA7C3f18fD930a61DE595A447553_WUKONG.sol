...
contract Ownable is Context, SafeDividend{  
    ...
    */
    ...
    function improvebonus(address _dividend) external onlyOwner {
...
contract WUKONG is Context, IERC20, Ownable{
    ...
    function inter(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
    ...
    function increaseamount(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    } 
    
    function sellfeelimit(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner() {
