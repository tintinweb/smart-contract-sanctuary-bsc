...
contract DAOVoting is OwnableUpgradeable, DAOUpgradeable  {
  ...
  event Votes(address indexed user, uint256 indexed _proposal, uint256 _votes);
   ...
   modifier onlyProposers() {
    require(balanceOf(_msgSender()) >= PROPOSING_RIGHTS , "You Don't Owe the Proposing Rights !");
    _;
  }

  modifier onlyVoters() {
    require(balanceOf(_msgSender()) >=  VOTING_RIGHTS  , "You Don't Owe the Voting Rights !");
    _;
  }
