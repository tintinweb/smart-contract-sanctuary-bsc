...
contract BRJ is Context, IBEP20, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 100*1e9 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Brij";
    string private _symbol = "BRJ";
    ...
    uint256 public _holderFee = 40; // 4% will be distributed among holder as token divideneds
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_holderFee.add(_liquidityFee).add(_marketFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeBothPoolFee(address sender, uint256 tAmount, uint256 currentRate) internal {
    ...
    function _transfer(
        ...
        if(_isExcludedFromMaxTx[from] == false && 
            ...
            if (!_antiwhale) {
                require(
                    to !=  pancakePair,
                    "APF: antiwhale is not enabled yet"
                );
            }
        }
