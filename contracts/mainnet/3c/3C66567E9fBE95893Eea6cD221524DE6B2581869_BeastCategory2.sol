...
contract BeastCategory2 is ERC721BurnableUpgradeable, SafeOwnable, Verifier {
    using StringsUpgradeable for uint256;
    using ECDSAUpgradeable for bytes32;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint128 public minted;
    uint128 public upgraded;
    ...
    function verifyMintParam(address account, uint256 tokenId, uint8 v, bytes32 r, bytes32 s) private view returns (bool) {
    ...
    function verifyBatchMintParam(address[] calldata accounts, uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s) private view returns (bool) {
        bytes32 hash = keccak256(abi.encodePacked(accounts, "_", tokenIds, "_", block.chainid, "_", address(this)));
        bytes32 cHash = convertHash(hash);
        return verifier == cHash.recover(v, r, s);
    }

    function mint(address to, uint256 tokenId, uint8 v, bytes32 r, bytes32 s) external {
        require(verifyMintParam(to, tokenId, v, r, s), "Mint: verify failed");

        minted += 1;
        _mint(to, tokenId);
    }

    function batchMint(address[] calldata tos, uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s) external {
        require(tos.length == tokenIds.length, "BatchMint: param not enough");
        require(verifyBatchMintParam(tos, tokenIds, v, r, s), "BatchMint: verify failed");
    ...
    function batchBurn(uint256[] calldata tokenIds) external {
        require(tokenIds.length > 0, "BatchBurn: len(ids) == 0");
