...
contract WomSwapDepositor is Ownable {
    ...
    function getWmxWomAmountOut(uint256 _amountIn) external returns (uint256 amountOut) {
        (amountOut, ) = ISwapRouter(swapRouter).getAmountOut(getTokensPath(), getPoolsPath(), int256(_amountIn));
    }

    function getTokensPath() public returns (address[] memory tokens) {
        tokens = new address[](2);
        tokens[0] = wom;
        tokens[1] = wmxWom;
    }

    function getPoolsPath() public returns (address[] memory pools) {
        pools = new address[](1);
        pools[0] = pool;
    }
