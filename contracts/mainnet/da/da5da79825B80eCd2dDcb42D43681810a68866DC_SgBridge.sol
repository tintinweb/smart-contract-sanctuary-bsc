...
contract SgBridge is Initializable, UUPSUpgradeable, OwnableUpgradeable, IStargateReceiver, WardedLiving, ISgBridge {
    ...
    function createPayload(address destAddress, address destToken, bytes calldata receiverPayload) private returns (bytes memory) {
        bytes memory data;
        {
            data = abi.encode(destAddress, destToken, receiverPayload);
        }
        return data;
    }
    ...
    function bridgeInternal(
        ...
        bytes memory payload = createPayload(params.destinationAddress, params.destinationToken, receiverPayload);
        ...
        router.swap{value: params.fee }(
            ...
            params.amount,
            0, //FIXME!!!
            getLzParams(params.destinationAddress),
    ...
    function bridge(address token,
        ...
        address destinationToken,
        bytes calldata receiverPayload
    ) external override live payable {
        ...
        if (currentChainId == destChainId) {
            ...
            if (receiverPayload.length > 0) {
                externalCall(token, destinationAddress, amount, currentChainId, receiverPayload);
            }
            return;
        }
