...
contract Treasury is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function reinit_SplitFeeTokensToDevReserve(address _devReserve) external reinitializer(1) {
        setDevReserve(_devReserve);
        devReserveRatio = 20;
        for (uint256 i = 0; i < feeTokens.length;) {
            address _token = feeTokens[i];
            uint256 _balance = IERC20(_token).balanceOf(address(this));
            uint256 _amountToDevReserve = _balance * devReserveRatio / RATIO_PRECISION;
            _transferToDevReserve(_token, _amountToDevReserve);
            unchecked {
                ++i;
            }
        }
    ...
    function setDevReserve(address _devReserve) public onlyRole(CONTROLLER_ROLE) {
        require(_devReserve != address(0), "INVALID ADDRESS");
        devReserve = _devReserve;
        emit DevReserveSet(_devReserve);
    }
    ...
    function withdrawFee() external {
        for (uint256 i = 0; i < feeTokens.length;) {
            ...
            if (_feeReserve > 0) {
                ...
                uint256 _amountToDevReserve = (_balanceAfter - _balanceBefore) * devReserveRatio / RATIO_PRECISION;
                _transferToDevReserve(_token, _amountToDevReserve);
            }
            unchecked {
                ++i;
    ...
    function _transferToDevReserve(address _token, uint256 _amount) internal {
        if (devReserve != address(0) && _amount > 0) {
            IERC20(_token).safeTransfer(devReserve, _amount);

            emit DevReserveReceived(_token, _amount);
        }
    }
    ...
    event DevReserveReceived(address indexed _token, uint256 _amount);
    event DevReserveSet(address indexed _address);
}