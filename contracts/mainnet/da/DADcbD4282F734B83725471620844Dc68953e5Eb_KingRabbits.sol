...
contract KingRabbits is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // Lists 
  uint256[250] public remainingIds;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0xe512AF112A1Bf27401D1771eB7E91468Df467A9a;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "KingRabbits: minting has not started yet");
    require(_mintAmount > 0, "KingRabbits: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "KingRabbits: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "KingRabbits: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "KingRabbits: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "KingRabbits: zero address");
    require(_amount > 0, "KingRabbits: you have to mint at least one");
    require(lastSupply != 0,  "KingRabbits: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"KingRabbits: URI query for nonexistent token");
