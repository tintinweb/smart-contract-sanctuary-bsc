...
contract PRESENT4LIFE is Context, IBEP20, Ownable {
    ...
    uint256 public maxBuyLimit                      = 2_000_000 * 10**9;    //2% of the total supply
    uint256 public maxSellLimit                     = 1_000_000 * 10**9;    //1% of the total supply
    uint256 public maxWalletLimit                   = 2_000_000 * 10**9;    //2% of the total supply
    
    uint256 public genesis_block;
    
    address public prizepromoWallet                 = 0xa58eBa1211f5FA95f905E579B145205d226f3a04;
    address private devWallet 				        = 0xFe45674eb8FFF0013477527BFF9f6507EfCE50DF;

    string private constant _name                   = "PRESENT4LIFE";
    string private constant _symbol                 = "4LIFE";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[prizepromoWallet]        = true;
        _isExcludedFromFee[devWallet]          	    = true;
        
        allowedTransfer[address(this)]              = true;
        allowedTransfer[owner()]                    = true;
        allowedTransfer[pair]                       = true;
        allowedTransfer[prizepromoWallet]           = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _prizepromo, uint256 _liquidity, uint256 _dev) public onlyOwner {
       taxes = Taxes(_rfi,_prizepromo,_liquidity,_dev);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _prizepromo, uint256 _liquidity, uint256 _dev) public onlyOwner {
       sellTaxes = Taxes(_rfi,_prizepromo,_liquidity,_dev);
        emit FeesChanged();
    }
    ...
    function _takePrizepromo(uint256 rPrizepromo, uint256 tPrizepromo) private {
        totFeesPaid.prizepromo +=tPrizepromo;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tPrizepromo;
        }
        _rOwned[address(this)] +=rPrizepromo;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rPrizepromo, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tPrizepromo = tAmount*temp.prizepromo/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tDev = tAmount*temp.dev/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tPrizepromo-s.tLiquidity-s.tDev;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rPrizepromo, uint256 rLiquidity){
        ...
        rPrizepromo = s.tPrizepromo*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rDev = s.tDev*currentRate;
        rTransferAmount =  rAmount-rRfi-rPrizepromo-rLiquidity-rDev;
        return (rAmount, rTransferAmount, rRfi,rPrizepromo,rLiquidity);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 3) {
            require(to != pair, "Sells are not allowed for the first 3 blocks.");
        }
        ...
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            ...
            if(to != pair){
                require(balanceOf(to) + amount <= maxWalletLimit, "The cooldown is enabled.");
            }
            if(coolDownEnabled){
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tPrizepromo + s.tDev);
        }
        if(s.rPrizepromo > 0 || s.tPrizepromo > 0) _takePrizepromo(s.rPrizepromo, s.tPrizepromo);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.prizepromo + temp.dev) * 2;
        ...
        uint256 prizepromoAmt = unitBalance * 2 * temp.prizepromo;
        if(prizepromoAmt > 0){
            payable(prizepromoWallet).sendValue(prizepromoAmt);
        }
    ...
    function updatePrizepromoWallet(address newWallet) external onlyOwner{
        prizepromoWallet = newWallet;
    }
