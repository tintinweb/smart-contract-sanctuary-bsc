...
// File: contracts/YFIH/YFIHMAIN.sol
...
contract YFIHTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    uint256 public tokenRewardsNum;
    uint256 public liquidityNum;
    uint256 public marketingNum;
    uint256 public fundNum;
    uint256 public burnNum;
    uint256 public refNum;
    uint256 public nftNum;

    bool public  penaltyMode; 
    bool public canswaping; 
    bool public cantransfer; 
    uint256 public lasterPrice; 
    ...
    constructor(
        ...
        address[5] memory addrs, // reward, router, marketing wallet, dividendTracker, fund
        uint256[7] memory feeSettings, // rewards, liquidity, marketing , burn , nft , fund ,ref
        uint256 minimumTokenBalanceForDividends_
        ...
        nftFee = feeSettings[4];
        fundFee = feeSettings[5];
        refFee = feeSettings[6];
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(nftFee).add(fundFee).add(refFee);
        ...
        */
        ...
        nextTime = block.timestamp + 5 minutes;
    ...
    function getPrice() public view  returns(uint256){
        ...
        uint256 wbnbBalance = IERC20(address(uniswapV2Router.WETH())).balanceOf(uniswapV2Pair);
   
        return wbnbBalance.mul(10 ** 18).div(tokenBalance);
    }

    function getUntilNextTime() public view  returns(uint256){
    ...
    function setCanSwapIng(bool _able) external onlyOwner {
        canswaping = _able;
    }
    ...
    function setRefFee(uint256 value) external onlyOwner {
        refFee = value;
         totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee).add(nftFee).add(fundFee).add(refFee);
        require(totalFees <= 25, "Total fee is over 25%");
    }
    ...
    function _transfer(
        ...
        if(block.timestamp >= nextTime){
            ...
            if(price > 0 && price <= lasterPrice * 7 / 10){
                penaltyMode = true;
            }
            lasterPrice = price;
            nextTime = nextTime + 5 minutes;
        }

    
        if(penaltyMode && from == _fundWalletAddress){
            penaltyMode = false;
        }
        ...
        if (
            ...
            // super._transfer(from, address(_marketingWalletAddress), marketingNum);
            IERC20(address(this)).transfer(_marketingWalletAddress, marketingNum);
            marketingNum = 0;

         
            if(fundNum>0){
                IERC20(address(this)).transfer(_fundWalletAddress, fundNum);
            }

            fundNum = 0;

     
            IERC20(address(this)).transfer(_burnAddress, burnNum);
            burnNum = 0;


            uint256 swapTokens = liquidityNum;
            swapAndLiquify(swapTokens);
            liquidityNum = 0;

   
            uint256 sellTokens = tokenRewardsNum;
        ...
        if (takeFee) {
            ...
            uint256 fees = amount.mul(marketingFee.add(nftFee).add(burnFee).add(liquidityFee).add(tokenRewardsFee)).div(100); //运营1 nft2 burn1 回流1 持币2
            tokenRewardsNum = tokenRewardsNum.add(amount.mul(tokenRewardsFee).div(100));
            marketingNum = marketingNum.add(amount.mul(marketingFee).div(100));
            nftNum = nftNum.add(amount.mul(nftFee).div(100));
            burnNum = burnNum.add(amount.mul(burnFee).div(100));
            liquidityNum = liquidityNum.add(amount.mul(liquidityFee).div(100));

     
            if (automatedMarketMakerPairs[to]) {
              
                if(penaltyMode){
                    fees += amount.mul(fundFee.add(15)).div(100);
                    fundNum = fundNum.add(amount.mul(fundFee.add(15)).div(100));
                }else{
                    fees += amount.mul(fundFee).div(100);
                    fundNum = fundNum.add(amount.mul(fundFee).div(100));
                }



                refNum = refNum.add(amount.mul(refFee).div(100));
                // super._transfer(from, address(this), refNum);
                ...
                if(parent!=address(0)){
                    shareAmount += refNum.mul(4).div(10);
                    super._transfer(from, parent , refNum.mul(4).div(10));
                    amount = amount.sub(refNum.mul(4).div(10));

               
                    parent = _parent[parent];
                    if(parent!=address(0)){
                        shareAmount += refNum.mul(3).div(10);
                        super._transfer(from, parent , refNum.mul(3).div(10));
                        amount = amount.sub(refNum.mul(3).div(10));
                ...
                uint256 remainAmount = refNum - shareAmount;
                if(remainAmount > 0){
           
                    // IERC20(address(this)).transfer(_fundWalletAddress,  remainAmount);
                    super._transfer(from, _fundWalletAddress , remainAmount);
                    amount = amount.sub(remainAmount);
                }
            ...
            // }
                ...
                if(_fromToAmount[to][from] == 0){                  
                    if(_fromToAmount[from][to] == 0 && amount >= 1 * 10 ** (decimals() - 2) && _parent[to] == address(0))  {  
                        _fromToAmount[from][to] = amount; 
                        _parent[to]=from;
                }}

               
                fees += amount.mul(fundFee.add(1)).div(100);
                fundNum = fundNum.add(amount.mul(fundFee.add(1)).div(100));
            }
