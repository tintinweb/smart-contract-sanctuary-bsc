...
interface IBondCalculator {
    function getPrice( address _pair ) external view returns ( uint _price);
...
contract DepositHelper is Ownable {
    ...
    IBondCalculator public immutable calculator;


    constructor (address _deposit, ISwapV2Router _router, address _LPAddress, address _Token, address _BUSD, IBondCalculator _calculator) {
        deposit = _deposit;
        Token = _Token;
        BUSD = _BUSD;
    ...
    function depositHelper(
        ...
        if (_tokenAddress == LPAddress) {
            ...
            BUSD.safeApprove(address(router), uint256(- 1));
            Token.safeApprove(address(router), uint256(- 1));
            calAndSwap();
            (,, uint256 moreLPAmount) = router.addLiquidity(BUSD, Token, BUSD.myBalance(), Token.myBalance(), 0, 0, address(this), block.timestamp);
            LPAddress.safeApprove(address(deposit), moreLPAmount);
            if (BUSD.myBalance() > 0) {
                BUSD.safeTransfer(msg.sender, BUSD.myBalance());
            }
            if (Token.myBalance() > 0) {
    ...
    function depositValue(uint256 _amount) public view returns (uint256 value_) {
        ISwapV2Pair lpToken = ISwapV2Pair(factory.getPair(BUSD, Token));
        (uint256 token0Reserve, uint256 token1Reserve,) = lpToken.getReserves();
        (uint256 debtReserve, uint256 relativeReserve) = BUSD ==
        ...
        if (swapAmt > 0) {
            ...
            (path[0], path[1]) = isReversed ? (Token, BUSD) : (BUSD, Token);
            uint[] memory amounts = router.getAmountsOut(swapAmt, path);
            (uint256 amount0, uint256 amount1) = BUSD == lpToken.token0() ? (_amount.sub(swapAmt), amounts[1]) : (amounts[1], _amount.sub(swapAmt));
            uint256 _totalSupply = lpToken.totalSupply();
            uint256 lpAmount = Math.min(amount0.mul(_totalSupply) / token0Reserve, amount1.mul(_totalSupply) / token1Reserve);
            uint256 value = calculator.getLPValue(LPAddress, lpAmount );
    ...
    function calAndSwap() internal {
        ...
        (uint256 debtReserve, uint256 relativeReserve) = BUSD ==
        lpToken.token0() ? (token0Reserve, token1Reserve) : (token1Reserve, token0Reserve);
        (uint256 swapAmt, bool isReversed) = optimalDeposit(BUSD.myBalance(), Token.myBalance(),
            debtReserve, relativeReserve);
        if (swapAmt > 0) {
            address[] memory path = new address[](2);
            (path[0], path[1]) = isReversed ? (Token, BUSD) : (BUSD, Token);
