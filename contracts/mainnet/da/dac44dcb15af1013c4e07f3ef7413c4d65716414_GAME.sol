...
contract GAME is Owner {
	...
	address public seed_addr = 0xfC01E36aB6e5d3D9e7bbf44d4d459Ad579e13c1b;
	address public game_addr = 0xdCBf271F4048F177BE9956c3D84A747124aF671D;
	...
	mapping(address =>uint) public day_base_out_lists;
 	...
 	uint256 public reward_1=600 * 10 **18;
	uint256 public reward_2=360 * 10 **18;
	uint256 public reward_3=240 * 10 **18;
	uint256 public reward_seed=1 * 10 **18;
	...
	address[] public raward_address_lists;
 ...
 function set_game_num(uint _val) external onlyOwner {
	...
	function set_buy_game_max(uint _val) external onlyOwner {
        buy_game_max = _val;
    }
    ...
    function base_output(uint cur_id) external   lock returns(bool) {
		...
		if(game_index % game_num==0){
			...
			uint r_1_1=r_1+(game_id-1)*game_num-1;
			uint r_2_1=r_2+(game_id-1)*game_num-1;
			uint r_3_1=r_3+(game_id-1)*game_num-1;
			
			raward_address_lists.push(game_address_lists[r_1_1]);
			raward_address_lists.push(game_address_lists[r_2_1]);
			raward_address_lists.push(game_address_lists[r_3_1]);
			
			 
			amount_token[game_address_lists[r_1_1]]=amount_token[game_address_lists[r_1_1]].add(reward_1);
			amount_token[game_address_lists[r_2_1]]=amount_token[game_address_lists[r_2_1]].add(reward_2);
			amount_token[game_address_lists[r_3_1]]=amount_token[game_address_lists[r_3_1]].add(reward_3);
   ...
   function get_address(uint ii,uint max_i) public view returns (address[] memory) {
	...
	function get_raward_lists(uint ii,uint max_i) public view returns (uint256[] memory) {
        ...
        uint256[] memory lists = new uint256[]((max_i-i));
		 ...
		 function get_raward_address(uint ii,uint max_i) public view returns (address[] memory) {
		uint i=ii;
        ...
        address[] memory lists = new address[]((max_i-i));
		 while (i <max_i) {
					...
					lists[iii]=raward_address_lists[i];
