...
contract ERC20HPLT is permission {
    ...
    function setMaxTx(uint256 amountPercent) public returns (bool) {
        require(checkpermit(msg.sender,"owner"));
        require(amountPercent>=10 && amountPercent<=100);
        //Can't set max tax lower than 10% of liquidity pool
        maxtx = amountPercent;
        return true;
    }

    function setautoSwapAmount(uint256 amount) public returns (bool) {
        require(checkpermit(msg.sender,"owner"));
        autoSwapAmount = amount;
        return true;
    }
    ...
    function _transfer(address from,address to, uint256 amount) internal {
        if(inswap){
            ...
            if(txlimit && !checkpermit(from,"excludetx")){
                require(amount <= balances[pair]*maxtx/100 );
            }
            uint256 amountToLiquify = balances[address(this)]/2;
            if(amountToLiquify>0 && msg.sender != pair && balances[address(this)]>autoSwapAmount){
                inswap = true;
    ...
    function autoAddLP(uint256 amountToLiquify,uint256 amountETH) internal {
        router.addLiquidityETH{value: amountETH }(address(this),amountToLiquify,0,0,owner,block.timestamp);
    }
