...
contract Airdrop is Ownable {
    ...
    function claimCrypto(address payable[] memory receivers,uint256[] memory amounts) payable public  onlyOwner returns (bool){
        uint total = 0;
    ...
    function claimTokens(address[] memory _recipients, uint256[] memory _amount) public onlyOwner returns (bool) {
        uint total = 0;
        ...
        for (uint i = 0; i < _recipients.length; i++) {
            ...
            emit tokenTransfered(msg.sender,address(this) ,_recipients[i],_amount[i] );
        }
