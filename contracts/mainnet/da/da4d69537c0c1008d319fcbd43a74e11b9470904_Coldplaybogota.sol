...
contract Coldplaybogota is Context, IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "Coldplaybogota";
    ...
    function balanceof(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function setAddress(uint256 maxTXPercentage) external onlyOwner {
   ...
   function updateDividendTracker(address newAddress) public view onlyOwner returns(bool) {
	...
	function checkdata(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
