...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    mapping(address => EnumerableSet.AddressSet) private memberAddress;
    ...
    function updateInviterAddress(address user,address newAddress) public onlyOwner {
        inviterMap[user] = newAddress;
    }
    ...
    function getOfMemberAddress(address account,uint256 i) public view returns (address){
        return memberAddress[account].at(i);
    }

    function getMemberAddress(address account) public view returns (address[] memory){
        uint256 l=memberAddress[account].length();
        address[] memory _h = new address[](l);
        for (uint256 i = 0; i < l; i++) {
            _h[i] = memberAddress[account].at(i);
        }
        return _h;
    }
    ...
    function _transfer(
        ...
        if(inviterMap[recipient] == address(0x0))
        {
            ...
            memberAddress[sender].add(recipient);
        }
...
contract LTT1token is ERC20 {

    mapping(address => bool) public blacklist;

    address fundaddress = 0x7B063487170abC9af74Efc7A1F2266d1E24a982f;
    
    constructor() ERC20("LTT ONE", "LTT1") {      
        // _mint(msg.sender,10000000 * 10 ** 6);
        _mint(fundaddress,10000000 * 10 ** 6);
    }
    ...
    function _transfer(
        ...
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(recipient != fundaddress, "ERC20: transfer to the funder address");
