...
contract FarmBooster is Ownable {
    ...
    /// @notice CST pool.
    address public immutable CST_POOL;
    ...
    /// @notice CST pool BOOST_WEIGHT precision
    ...
    /// @param _CST CST token contract address.
    /// @param _CSTPool CST Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the CST pool.
    modifier onlyCSTPool() {
        require(msg.sender == CST_POOL, "onlyCSTPool: Not CST pool");
        _;
    }
    ...
    /// @notice CSTpool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in CST pool.
    /// @param _lockedDuration user locked duration in CST pool.
    /// @param _totalLockedAmount Total locked CST amount in CST pool.
    /// @param _maxLockDuration maximum locked duration in CST pool.
    function onCSTPoolUpdate(
        ...
        emit UpdateCSTPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice CST pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IBEP20(CST).balanceOf(CST_POOL);

        uint256 totalLockedAmount = ICSTPool(CST_POOL).totalLockedAmount();

        uint256 pricePerFullShare = ICSTPool(CST_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((ICSTPool(CST_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = ICSTPool(CST_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = ICSTPool(CST_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration CST pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = ICSTPool(CST_POOL)
            .userInfo(_user);
