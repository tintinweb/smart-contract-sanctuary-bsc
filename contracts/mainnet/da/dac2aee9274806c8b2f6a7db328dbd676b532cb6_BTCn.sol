...
contract BTCn is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 21000000 * 10**16;  
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Bitachon V.3";
    ...
    address public developmentWallet = 0x303d5Dc231A6053959e6b0CB0A5BD9d4bDfe78F7;
    ...
    uint256 public _maxTxAmount = 21000000 * 10**16;  
    uint256 public _maxWalletSize = 21000000 * 10**16;  
    uint256 private numTokensSellToAddToLiquidity = 110000 * 10**16;  
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(developmentAmount).sub(burnAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(developmentAmount).sub(burnAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(developmentAmount).sub(burnAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(developmentAmount).sub(burnAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(developmentAmount).sub(burnAmount));
        }

        //Temporarily remove fees to transfer to burn and development address
        removeAllFee();
