...
//    ___         __  ___              
//   / _ \___ ___/ / / _ )___ ___ _____
//  / , _/ -_) _  / / _  / -_) _ `/ __/
// /_/|_|\__/\_,_/ /____/\__/\_,_/_/   

// https://t.me/redbearofficial
// https://red-bear.co
...
contract RedBear is IERC20, Context, Ownable {
    ...
    uint256 public maxSellTx = 200000000000 * 10**9;
    uint256 public maxWalletSize = 400000000000 * 10**9;

    uint256 public G_block;

    uint256 public maxBuyTx = 400000000000 * 10**9;
    uint256 public swapTokensAtAmount = 2000000000 * 10**9;
    
    address public RBearTeam = 
        0x26F6534bdDD928748123604b936b7FBc57ABF4f0;

    address public RBearMarketing =
        0x089dbF65E1178cb332D942983F775e8F8C6628b5;
    ...
    constructor(address routerAddress) {
        ...
        _isExcludedFromFee[RBearMarketing] = true;
        _isExcludedFromFee[RBearTeam] = true;

        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[RBearMarketing] = true;
        allowedTransfer[RBearTeam] = true;
    ...
    function setTradingStatus(bool state) external onlyOwner {
        ...
        swapEnabled = state;
        if (state == true && G_block == 0) G_block = block.number;
    }
    ...
    function _transfer(
        ...
        if (from == pair && !_isExcludedFromFee[to] && !swapping) {
            ...
            require(
                ...
                "You are exceeding maxWalletSize"
            );
        }
        ...
        lockTheSwap
    {
        ...
        if (marketingAmt > 0) {
            payable(RBearMarketing).sendValue(marketingAmt);
        }
        uint256 TeamAmt = unitBalance * 2 * temp.Team;
        if (TeamAmt > 0) {
            payable(RBearTeam).sendValue(TeamAmt);
        }
    }
    ...
    function updateRBearMarketing(address newWallet) external onlyOwner {
        RBearMarketing = newWallet;
    }

    function updateRBearTeam(address newWallet) external onlyOwner {
        RBearTeam = newWallet;
    }
