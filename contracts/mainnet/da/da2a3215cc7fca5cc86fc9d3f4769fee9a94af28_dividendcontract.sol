...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        require(_totalSupply > 15000000 * (10**9), "Max 15000000");
...
contract dividendcontract is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 totalSupply = totalSupply_ * (10**9); // decimal: 9
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IBEP20(rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens);
        uint256 newBalance = (IBEP20(rewardToken).balanceOf(address(this))).sub(initialCAKEBalance);
        IBEP20(rewardToken).transfer(_marketingWalletAddress, newBalance);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        bool success = IBEP20(rewardToken).transfer(address(dividendTracker), dividends);
        if (success) {
