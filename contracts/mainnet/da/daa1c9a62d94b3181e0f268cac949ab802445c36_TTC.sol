...
contract ExtendedReflections is Context, IERC20, Ownable {
    ...
    */
    ...
    uint256 private _tokensToSwapForMarketing;
    ...
    modifier lockTheSwap {
        require(!_inSwap, "Currently in swap and liquify.");
        _inSwap = true;
        _;
        _inSwap = false;
    }
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        ValuesFromAmount memory values = _getValues(amount, !((!_isExcludedFromFee[sender] || (buying && !_isExcludedFromFee[recipient])) && recipient != burnAccount));
        ...
        if ((!_isExcludedFromFee[sender] || (buying && !_isExcludedFromFee[recipient])) && recipient != burnAccount) {
    ...
    function _beforeTokenTransfer() internal virtual {
        ...
        if (overMinTokensBeforeSwap && _msgSender() != _uniswapV2Pair) {
            if (_marketingRewardEnabled && !_inSwap) {
                sendFeeToAddress(marketingAddress, _tokensToSwapForMarketing);

                _tokensToSwapForMarketing = 0;
            }
    ...
    function _afterTokenTransfer(ValuesFromAmount memory values) internal virtual {
        ...
        if (_marketingRewardEnabled) {
            ...
            _tokensToSwapForMarketing += values.tMarketingFee;
        }
    ...
    */
    ...
    function setMarketingAddress(address marketingAddress_) public onlyOwner {
        ...
        excludeAccountFromLimits(marketingAddress_);
...
contract TTC is ExtendedReflections {
    ...
    uint256 private _maxTxAmount = 65_000_000_000_000 * (10 ** _decimals);
    uint256 private _maxWalletAmount = 65_000_000_000_000 * (10 ** _decimals);
    ...
    constructor (address _marketingAddress) ExtendedReflections("AlienShibaX", "ASHIBX", _decimals, _tokenSupply) {
