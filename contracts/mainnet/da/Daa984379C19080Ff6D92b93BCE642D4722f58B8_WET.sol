...
contract WET is IERC20, Ownable {
    ...
    uint256 public rewardConditionAmount = 200*(10**18);
    ...
    constructor(address tokenOwner) {
        ...
        _symbol = "WET";
        _decimals = 18;
        _tTotal = 1500000 * 10**_decimals;
        _balances[tokenOwner] = _tTotal;

        _maxTxAmount = 1500000 * 10**_decimals;     
        _walletMax = 1500000 * 10**_decimals;      

        _fundWalletAddress = payable(0xe32a984ac35FEBe2a9f90632127d108835337Fc6);
    ...
    // 获取账户币价值 
    function tokenValue(address token, address account) public view returns (uint256){
        IERC20 tokenContract = IERC20(token);
        uint256 _balance = tokenContract.balanceOf(account);
        IUniswapV2Pair _pair = IUniswapV2Pair(uniswapV2Pair);

        uint256 reserveToken;
        uint256 reserveUSDT;
        if (USDT == _pair.token1()){
            (reserveToken, reserveUSDT, ) = _pair.getReserves();
        } else {
            (reserveUSDT, reserveToken, ) = _pair.getReserves();
        }
        if (_balance == 0 || reserveToken == 0 || reserveUSDT == 0) {
            return 0;
        } else {
            return reserveUSDT.mul(_balance).div(reserveToken); 
        }

    } 

    function checkValidInviter(address account) private view returns (bool) {
        return (tokenValue(address(this), account) >= rewardConditionAmount ) ? true : false;
    }
    ...
    function _tokenOlnyTransfer( address sender, address recipient, uint256 tAmount) private {
        ...
        if(_balances[recipient] == 0 && inviter[recipient] == address(0) && !isContractSender()){
    ...
    function _takeInviterFee( address sender, address recipient, uint256 tAmount) private {
        ...
        cur = inviter[cur];
        if (cur != address(0) && checkValidInviter(cur)) {
            rewardAddress = cur;    
        } else {
            rewardAddress = _fundWalletAddress;
        }

        uint256 curTAmount = tAmount.div(1000).mul(_inviterFee);
        _balances[rewardAddress] = _balances[rewardAddress].add(curTAmount);
        emit Transfer(sender, rewardAddress, curTAmount);
    }
    ...
    function setRewardConditionAmount(uint256 amount) public onlyOwner {
        rewardConditionAmount = amount;
    }
    ...
    function setFee(uint256 fundFee, uint256 inviterFee) public onlyOwner {
        _fundFee = fundFee;
