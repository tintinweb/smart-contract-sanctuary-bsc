...
// SPDX-License-Identifier: Unlicensed
        ...
        contract TOKEN is BEP20,LockToken {
            using SafeMath for uint256;
            ...
            address private _teamwallet = 0x9887b00f5eD1f08c03D9dD997c7DEEb8Fa57C917;


            // Buy tax
            uint256 public _buyLiqudityPoolFee = 2;
            uint256 public _buyMarketingPoolFee = 4;
            uint256 public _buyDevFee = 2;

            // Sell tax    
            uint256 public _sellLiqudityPoolFee = 2;
            uint256 public _sellMarketingPoolFee = 5;
            ...
            constructor () BEP20("OneBabyPearl", "ONEP"){


                 numTokensSellToAddToLiquidity = 30000 * 10 ** decimals();
                _maxWalletBalance = 200000 * 10 ** decimals();
                _marketingWalletAddress = 0x6330ff4e44C13d3ac13523617dE82D49AAB5085F;
                _devAddress = 0xA914AECF06da6304F5a1e217612e159035e79A54;
               ...
               _isExcludedFromMaxWallet[_teamwallet] = true;
            ...
            function setBuyFeePercent(uint256 lFee, uint256 mFee, uint256 dFee) external onlyOwner {
                _buyLiqudityPoolFee = lFee;
                _buyMarketingPoolFee = mFee;
                _buyDevFee = dFee;
            }

            function setSellFeePercent(uint256 lFee, uint256 mFee, uint256 dFee) external onlyOwner {
                _sellLiqudityPoolFee = lFee;
                _sellMarketingPoolFee = mFee;
                _sellDevFee = dFee;
            } 
            ...
            function _transfer(
                ...
                {   if (from == uniswapV2Pair)
                    { // buy
                    uint256 lpTokens = amount.mul(_buyLiqudityPoolFee).div(100);
                    uint256 marketingTokens = amount.mul(_buyMarketingPoolFee).div(100);
                    uint256 devTokens = amount.mul(_buyDevFee).div(100);
                    amount= amount.sub(lpTokens).sub(marketingTokens).sub(devTokens);
                    super._transfer(from, address(this), lpTokens.add(devTokens).add(marketingTokens));
                    super._transfer(from, to, amount);
                    }
                     else if (to == uniswapV2Pair)
                    { // sell
                    uint256 lpTokens = amount.mul(_sellLiqudityPoolFee).div(100);
                    uint256 marketingTokens = amount.mul(_sellMarketingPoolFee).div(100);
            ...
            function swapBack(uint256 contractBalance) private {
                ...
                uint256 tokensForLiquidity = contractBalance.mul(_sellLiqudityPoolFee).div(100);
                uint256 marketingTokens = contractBalance.mul(_sellMarketingPoolFee).div(100);
                uint256 floorTokens = contractBalance.mul(_sellDevFee).div(100);
                ...
                (success,) = address(_devAddress).call{value: ethForFloor}("");
