...
contract PLTLottery is ReentrancyGuard, IPLTLottery, Ownable, ERC1155Holder {
    ...
    IERC1155 public pltNFT;
    uint256 public pltNFTFreeTicketId = 1;
    ...
    event NewPltNFT(address indexed pltNFT);
    event TicketsPurchase(address indexed buyer, uint256 indexed lotteryId, uint256 numberTickets, uint256 amountPLTToTransfer, uint256 amountNftToTransfer, string affiliateId);
     ...
     */
    constructor(address _pltTokenAddress, address _randomGeneratorAddress, address _pltNFT) {
        pltToken = IERC20(_pltTokenAddress);
        randomGenerator = IPLTRandomNumberGenerator(_randomGeneratorAddress);
        pltNFT = IERC1155(_pltNFT);
    ...
    function buyTicketsWithAffiliateId(uint256 _lotteryId, uint32[] calldata _ticketNumbers, bool _useFreeTickets, string memory _affiliateId)
        external
        override
        notContract
        nonReentrant {
        ...
        (uint256 amountPLTToTransfer, uint256 amountNftToTransfer) = _calculateTotalPriceForBulkTickets(
        ...
        if(amountPLTToTransfer > 0) {
            pltToken.safeTransferFrom(msg.sender, address(this), amountPLTToTransfer);
        } 
        if(amountNftToTransfer > 0) {
            pltNFT.safeTransferFrom(msg.sender, address(this), pltNFTFreeTicketId, amountNftToTransfer, "");
        }
        ...
        emit TicketsPurchase(msg.sender, _lotteryId, _ticketNumbers.length, amountPLTToTransfer, amountNftToTransfer, _affiliateId);
    }
    ...
    function claimTickets(
        ...
        if(rewardInPLTNftToTransfer > 0) {
            ...
            pltNFT.safeTransferFrom(address(this), msg.sender, pltNFTFreeTicketId, rewardInPLTNftToTransfer, "");
        }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(uint256 _lotteryId, bool _autoInjection)
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPLT) * (PPTT - _lotteries[_lotteryId].treasuryFee))
        ) / PPTT;
    ...
    function changePltNFT(address _pltNFT) external onlyOwner {
        pltNFT = IERC1155(_pltNFT);
        emit NewPltNFT(_pltNFT);
    }
  
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        pltToken.safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function setPltNFTFreeTicketId(uint256 _pltNFTFreeTicketId) external onlyOwner {
        pltNFTFreeTicketId = _pltNFTFreeTicketId;
    }
    ...
    function _calculateTotalPriceForBulkTickets(
        ...
        if(_useFreeTickets) {
            uint256 freeTicketsBalance = pltNFT.balanceOf(msg.sender, pltNFTFreeTicketId);
