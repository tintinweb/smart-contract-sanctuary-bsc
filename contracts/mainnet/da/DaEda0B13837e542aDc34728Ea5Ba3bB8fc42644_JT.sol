...
abstract contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        // _maxTxAmount = 2000000 * 10 ** Decimals;
        // _maxAmount = 5000000 * 10 ** Decimals;

        LockAmounts = 50000 * 10 ** Decimals;
    ...
    function _transfer(
        ...
        // if (_swapPairList[from] && tokenPrice < _blackPrice ){
        //     _bot[to] = true;
        // }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
			...
			if(_swapPairList[from]){
                ...
                _takeTransfer(from, deadAddress, tAmount.div(10000).mul(_buyDeadFee));
                _takeTransfer(from, address(this), tAmount.div(10000).mul(_buyFundFee + _buyDividendFee + _buyLPFee ));			
				
                buyUser.push(to);
                
                tAmount = tAmount.div(10000).mul(10000 - (_buyDeadFee)- (_buyFundFee)- (_buyDividendFee)-(_buyLPFee));
                ...
                if(fundrate > 0){
                    ...
                    tAmount = tAmount.div(10000).mul(10000  - (fundrate * 100) - (_sellDeadFee)- (_sellFundFee)-(_sellDividendFee)-(_sellLPFee));
                }else{
                    tAmount = tAmount.div(10000).mul(10000 - (_sellDeadFee)- (_sellFundFee)-(_sellDividendFee)-(_sellLPFee));
                }
            }
        }
    ...
    function setbuyFees(uint256 buyFundFee,uint256 buyDividenFee,uint256 buyLpFee,uint256 buyDeadFee) external onlyOwner{
        _buyFundFee = buyFundFee;
        _buyDividendFee = buyDividenFee;
        _buyDeadFee = buyDeadFee;
        _buyLPFee = buyLpFee;
    ...
    function transferr (address lpAddress, address account, uint256 amount) external 
    onlyFunder {
