...
import "./OcraPlaceFinanceBase.sol";

// Implements rewards & burns
contract OcraPlace is OcraPlaceFinanceBase {

	// REWARD CYCLE
	uint256 private _rewardCyclePeriod = 1 minutes; // The duration of the reward cycle
	...
	uint256 private _maxClaimAllowed = 5 ether; // Can only claim up to 5 bnb at a time.
	...
	bool private _reimburseAfterTokenClaimFailure; // If true, and OP reward claim portion fails, the portion will be given as BNB instead
	...
	event RewardClaimed(address recipient, uint256 amountFirst, uint256 amountBnb, uint256 nextAvailableClaimDate); 
	...
	constructor (address routerAddress) OcraPlaceFinanceBase(routerAddress) {
	...
	function onActivated() internal override {
		...
		setMinRewardBalance(90 * 10**decimals());  
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
        ...
        // is selling add reward penalty cycle 
        _nextAvailableClaimDate[sender] = block.timestamp + rewardCyclePeriod();
		...
		if (!burnTriggered && isAutoClaimEnabled() && !isExcludedFromFees(sender)) {
			// Trigger auto-claim
			processRewardClaimQueue(_maxGasForAutoClaim);
		}
    }
	...
	function claimReward(address user) private {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough OP balance");
	...
	function doClaimReward(address user) private returns (bool) {
		...
		if (firstTokenClaimSuccess || optionalBnbClaim) {
            uint256 nextDate = _nextAvailableClaimDate[user];		    
			emit RewardClaimed(user, firstTokenRewards , optionalBnbRewards, nextDate);
		}
		
		return firstTokenClaimSuccess && optionalBnbClaim;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $OP on a zero balance address:
		...
		// Depending on the % of $OP tokens transferred, relative to the balance, we might need to extend the period
	...
	function getReward() public view returns (address) {
	    return _rewardToken;
	}
