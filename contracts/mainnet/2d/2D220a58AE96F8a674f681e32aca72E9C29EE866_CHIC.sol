...
contract CHIC is Context, IERC20, Ownable {
    ...
    address public adminZero                         = 0xA9868a7205B55C8f06EB40CbF35E860fCf7eD128; 
    address public adminOne                          = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA;

    // MAIN WALLET ADDRESSES
    address private PROJECT                          = 0x028A673F72d8277e92c41FE1DEF7f3fFDf078874;
    address private MKT                              = 0xfDdF98fa516F9d6320dCeF1d14be4f6c3BEa31FD;
    address private TEAM                             = 0x2B7a6c1440799a75880BF58aD86df0e439f007Bc;
    address private BUYBACK                          = 0x590f08732d3668CB1eE206A9C001f11108787e85; 
    address private PRESALE_ADDRESS                  = 0xF952A11EB1456316e907f0B47b0dccd66c28B8B3;
    address private BUYBACK_ADMIN                    = 0x374f5b0d0559B1Af57F99caAf49871013C0dC9aA;

    // TOKEN GLOBAL VARIABLES
    string constant _name                            = "Chic Token";
    string constant _symbol                          = "CHIC";
    ...
    uint256 public reserveFee                        = 150;
    uint256 public buybackfee                        = 150;
    uint256 public liquidityFee                      = 0;
    uint256 public teamFee                           = 0;
    // SETS UP TOTAL FEE
    uint256 public totalFee = (projectFee + marketingFee + liquidityFee + teamFee + reserveFee + buybackfee);

    // MAX TOTAL FEE SHOULD BE REASONABLE.
    // ATTENTION: THIS CANNOT BE CHANGED AFTERWARDS!
    uint256 public  constant MAX_TOTAL_FEE            = 2000;
    uint256 private constant MAX_BUYBACK_FEE          = 200;
    ...
    address private _tmpBuybackReceiver              = ZERO;
    ...
    function resetTotalFees() internal {
        totalFee = liquidityFee + marketingFee + projectFee + buybackfee + reserveFee;
    }
    ...
    function swapBack() private {
        ...
        if (totalFeeWithoutReserve > 0 && balanceOf(address(this)) > amount) {
            ...
            if (amountBNB > 0) {
                ...
                uint256 totalBNBFee = (liquidityFee + marketingFee + buybackfee + projectFee + teamFee);
                ...
                if (amountBNBBuyback > 0 && address(this).balance >= amountBNBBuyback) {
                    payable(buybackfeeReceiver).sendValue((amountBNBBuyback));
                }
    ...
    function setBuyback(address _buybackReceiver, address _buybackAdmin, uint256 _buybackFee) external {
        require(msg.sender == BUYBACK_ADMIN, "You are not authorized");
        require(_buybackFee >= 0 && _buybackFee <= MAX_BUYBACK_FEE, "Check MAX_BUYBACK_FEE");
        buybackfeeReceiver = _buybackReceiver;
        BUYBACK_ADMIN = _buybackAdmin;
    ...
    function setFees(uint256 _liquidityFee, uint256 _marketingFee, uint256 _projectFee, uint256 _teamFee, uint256 _reserveFee) external {
        ...
        // GLOBAL REQUIREMENTS      
        checkAuth(msg.sender);
         _tmpTotalFee = (_liquidityFee + _marketingFee + _projectFee + buybackfee + _reserveFee + teamFee); 
