...
contract Firefoxes is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 public cost1 = 0.2 ether;
  uint256 public cost2 = 0.3 ether;
  uint256 public cost3 = 0.4 ether;
  uint256 public cost4 = 0.5 ether;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x0E626635099fe334dc290d9F02FD79eA3CecDEcd;
  ...
  function cost() public view returns (uint256){
      if (totalSupply() <= 250){
          return cost1;
      } else if(totalSupply() > 250 && totalSupply() <= 500){
          return cost2;
      } else if(totalSupply() > 500 && totalSupply() <= 750){
          return cost3;
      } else {
          return cost4;
      }
  ...
  function setCost(uint256 _newCost1, uint256 _newCost2, uint256 _newCost3, uint256 _newCost4) public onlyOwner {
    cost1 = _newCost1;
    cost2 = _newCost2;
    cost3 = _newCost3;
    cost4 = _newCost4;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Firefoxes: minting has not started yet");
    require(_mintAmount > 0, "Firefoxes: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Firefoxes: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Firefoxes: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost() * _mintAmount, "Firefoxes: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Firefoxes: zero address");
    require(_amount > 0, "Firefoxes: you have to mint at least one");
    require(lastSupply != 0,  "Firefoxes: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Firefoxes: URI query for nonexistent token");
