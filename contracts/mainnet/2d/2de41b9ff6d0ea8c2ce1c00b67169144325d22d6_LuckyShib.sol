...
// Root file: contracts/baby/LuckyShib.sol
...
contract LuckyShib is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    // exchange pool 
    uint8[10]  public  pool_bonuses = [30, 20, 10, 10, 5, 5, 5, 5, 5, 5];
    uint256 public  pool_balance;  // 3 => 3%
	mapping(uint256 => mapping(address => uint256)) public allTokenBuyers;
	mapping(uint256 => address) public topTokenBuyers;
	uint256 public drawRound;      // number of times the lottery has been drawn
	uint256 public nextDrawTime;   // the time of next draw
	uint256 public tokenBuyersNum; // number of Token buyers in the current round
    ...
    event PoolPayout(address indexed addr, uint256 amount, uint256 pos, uint256 indexed round);
    event DrawPool(uint256 drawtime, uint256 amount, uint256 n, uint256 indexed round);

    constructor(
        string memory name_,
        string memory symbol_,
        uint256 totalSupply_, 
        address[4] memory addrs, // rewardToken, router , marketing wallet, dividendTracker
        uint256[4] memory feeSettings, // rewards, liquidity, marketing, pool 
        uint256 minimumTokenBalanceForDividends_
        ...
        */
        ...
        nextDrawTime = block.timestamp + 86400; // 24 hours later
        drawRound = 0;
        tokenBuyersNum = 0;
        pool_balance = 0;
    }
    ...
    function calcTotalFees() private {
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(poolFee);
    ...
    function setPoolFee(uint256 value) external onlyOwner { 
        poolFee = value;
        calcTotalFees();
    }
    ...
    function getTopTokenBuyersNum () public view returns (uint256) { 
	    return tokenBuyersNum < pool_bonuses.length ? tokenBuyersNum : pool_bonuses.length;
	}
    ...
    function _drawPool() private {
        ...
        uint256 n = getTopTokenBuyersNum();
        uint256 total = 0;
        uint256 award;

        emit DrawPool(nextDrawTime, pool_balance, n, drawRound);

        uint256 draw_amount = pool_balance.div(10);

        for (uint256 i = 0; i < n; ++i) { 
		    award = draw_amount.mul(pool_bonuses[i]).div(100);
            super._transfer(address(this), topTokenBuyers[i], award);
			total += award;

            emit PoolPayout(topTokenBuyers[i], award, i, drawRound);
		}
    ...
    function _swapTokenFee(address from, address to) private {
        uint256 contractTokenBalance = balanceOf(address(this));//current balance
		
		require(contractTokenBalance >= pool_balance, "ERC20: self-hold token balance < pool balance");
		contractTokenBalance -= pool_balance; 
        ...
        if (
            ...
            //poolToken
            uint256 poolTokens = contractTokenBalance
                .mul(poolFee)
                .div(totalFees);
			pool_balance += poolTokens;
    ...
    function _updateTokenBuyers(address to, uint256 amount) private { 
        uint256 n = getTopTokenBuyersNum();

        amount += allTokenBuyers[drawRound][to]; 

        uint256 i = 0;
        for (; i < n; ++i) { 
            if (allTokenBuyers[drawRound][topTokenBuyers[i]] < amount) {
			    break;
            }
        }
        ...
        uint256 j = i;
        for (; j < n; ++j) { 
            if (topTokenBuyers[j] == to) {
                break;
            }
		}

        for (; j > i; --j) { 
            topTokenBuyers[j] = topTokenBuyers[j - 1];
        }

        topTokenBuyers[i] = to;

        if (allTokenBuyers[drawRound][to] == 0) {
            tokenBuyersNum += 1;
        }

        allTokenBuyers[drawRound][to] = amount;
    }
    ...
    function _transfer(
        ...
        else if (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to]) { //only uniswap
            takeFee = !swapping;
        }
        // else
        // {
        ...
        // be sure from Uniswap and update the buyers info
        if (automatedMarketMakerPairs[from]) { 
           _updateTokenBuyers(to, amount);
        }
    ...
    // half self and half tokens to eth that add the liquify 
    function swapAndLiquify(uint256 tokens) private {
        // split the contract balance into halves
        uint256 half = tokens.div(2);//50%
        uint256 otherHalf = tokens.sub(half);//50%
    ...
    // token for swap to reward token
    ...
    function claimDrawPool() external {
        _swapTokenFee(address(this), uniswapV2Pair);
    }

    function poolTopInfo() view external returns(address[10] memory addrs, uint256[10] memory deps, uint256 n) {    
        n = getTopTokenBuyersNum();       
        for(uint256 i = 0; i < n; i++) {
            addrs[i] = topTokenBuyers[i];
            deps[i] = allTokenBuyers[drawRound][topTokenBuyers[i]];
        }
    }
}