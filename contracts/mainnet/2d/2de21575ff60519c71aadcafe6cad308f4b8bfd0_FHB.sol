...
contract FHB is IERC20, Ownable {
    ...
    address public gdAddress = address(0x64494f54169CdfC29db0e0f5ba1f539B913DB17C);

    uint256 public _inviterFee = 500;
    address public inviterAddress = address(0x64494f54169CdfC29db0e0f5ba1f539B913DB17C);
    ...
    constructor(address tokenOwner) {
        ...
        uint256 leftAmount = 210000 * 10**17;
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            rate =_liquidityFee + _destroyFee + _inviterFee + _destroyFee;
        } else {
            // 转账
            _takeTransfer(
                sender,
            ...
            _takeInviterFee(sender, recipient, tAmount, currentRate);
            rate = _destroyFee + _inviterFee + _destroyFee;
        }
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 8; i++) {
            ...
            rec_balance = balanceOf(reciver);
            if(rec_balance >= 5 ** 14) {
                _rOwned[reciver] = _rOwned[reciver].add(curRAmount);
                emit Transfer(sender, reciver, curTAmount);
            } else {
                _rOwned[inviterAddress] = _rOwned[inviterAddress].add(curRAmount);
                emit Transfer(sender, inviterAddress, curTAmount);
            } 
        }
    }
