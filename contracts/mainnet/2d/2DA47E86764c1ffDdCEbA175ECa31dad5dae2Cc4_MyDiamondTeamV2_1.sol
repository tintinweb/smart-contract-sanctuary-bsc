...
contract MyDiamondTeamV2_1 {
    ...
    function payoutOf(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            ...
            uint256 timepoint = users[_addr].checkpoint;
            if(timepoint < block.timestamp.sub(48 hours)){
            //    timepoint = block.timestamp.sub(48 hours);
            }

            payout = (users[_addr].deposit_amount.mul(BASE_PERCENT).div(PERCENTS_DIVIDER))
                    .mul(block.timestamp.sub(timepoint))
                    .div(TIME_STEP);
    ...
    function payoutOf_CUTOFF(address _addr) view external returns(uint256 payout, uint256 max_payout) {
        ...
        if(users[_addr].deposit_payouts < max_payout) {
            ...
            if( timestamp_user_action < timestamp_cutoff_release){ // last action was before cut off upgrade
