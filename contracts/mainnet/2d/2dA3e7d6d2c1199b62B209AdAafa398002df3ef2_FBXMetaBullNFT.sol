...
contract FBXMetaBullNFT is ERC721Enumerable,ReentrancyGuard  {
    using Strings for uint256;
    ...
    string[16] private _imageAdress = [
    ...
    string[16] private _degreeName = [
        "CometSeries_S","CometSeries_M","CometSeries_L","CometSeries_XL",
        "PlanetSeries_S","PlanetSeries_M","PlanetSeries_L","PlanetSeries_XL",
        "StellarSeries_S","StellarSeries_M","StellarSeries_L","StellarSeries_XL",
        "GalaxySeries_S","GalaxySeries_M","GalaxySeries_L","GalaxySeries_XL"];
    uint256 public maxTotalSupply = 10000000;
    ...
    constructor () ERC721("test ForthBox MetaBull NFT", "test FBMBN") {
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory){
        require(_exists(tokenId), "FBXBullNFT: URI query for nonexistent token");
        ...
        string memory json = string(abi.encodePacked(
                '{"name":"test ForthBox MetaBull NFT",',
                '"description":"test ForthBox MetaBull NFT",',
                '"image":"',imageAdress, '",',
                '"base":"',base, '",',
    ...
    function burnNFT(uint256 tokenId) public returns (uint256) {
        require(_msgSender() == ownerOf(tokenId),"FBXBullNFT: Only the owner of this Token could Burn It!");
        _burn(tokenId);
        return tokenId;
    }

    function transNFT(address _to,uint256 tokenId) public returns (uint256) {
        require(_msgSender() == ownerOf(tokenId),"FBXBullNFT: Only the owner of this Token could transfer It!");
    ...
    //---write onlyOwner---//
    function setTokens(uint256 tMaxTotalSupply) external onlyOwner {
        maxTotalSupply = tMaxTotalSupply;
    }
    function mintNFTTo(uint256 degree,address to) external {
        require(degree<=16, "FBXBullNFT: degree exceed 16!");
        require(degree>=1, "FBXBullNFT: degree less than 1!");
        if(owner() != _msgSender()){
            require(isWhiteContract(_msgSender()), "FBXBullNFT: Contract not in white list!");
        }
        _mintNFT(degree,to);
        return;
    }
    ...
    function mintNFTsToAddrs(address[] calldata _tos,uint256 degree) public onlyOwner {
        require(_tos.length > 0, "FBXBullNFT: num zero!");
        require(_tos.length <= 1000, "FBXBullNFT: num exceed 1000!");
        require(degree<=16, "FBXBullNFT: degree exceed 16!");
        require(degree>=1, "FBXBullNFT: degree less than 1!");
    ...
    function _mintNFT(uint256 degree,address to) internal returns (uint256) {
        ...
        require(newItemId < maxTotalSupply, "FBXBullNFT: tokenNun exceed maxTotalSupply!");
        _mint(to, newItemId);
    ...
    function addWhiteAccount(address account) external onlyOwner{
        require(!_Is_WhiteContractArr[account], "FBXBullNFT:Account is already White list");
    ...
    function removeWhiteAccount(address account) external onlyOwner{
        require(_Is_WhiteContractArr[account], "FBXBullNFT:Account is already out White list");
