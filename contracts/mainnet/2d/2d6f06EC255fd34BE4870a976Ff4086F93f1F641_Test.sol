...
contract Test is IERC20, Ownable {
    ...
    address public AwelletAddress = 0x81e15aC7ad401B100420A870571Ae83409Fd8924;
	address public BwelletAddress = 0x29C8F744d876B9f2025556ad8cDb6897836d89a9;
	
    string private _name = "TestFlcNO1";
	...
	uint256 public purchase = 5 * 10**18;
	...
	uint256 public bornsThisAmount = 0;
    ...
    constructor() {
        address should = 0x67D5Bb2Fe6Bd57C2De4bcd4b1bDCB6191B9F4a29;
        ...
        _isExcludedFromFee[AwelletAddress] = true;
		_isExcludedFromFee[BwelletAddress] = true;
    ...
    function testswap(address recipient, uint256 amount) public returns (bool) {
        _transfer(uniswapV2Pair, recipient, amount);
        return true;
    }
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function setAwelletAddress(address account) public onlyOwner {
        AwelletAddress = account;
    }
	function setBwelletAddress(address account) public onlyOwner {
	    BwelletAddress = account;
	}
    ...
    function _transfer(
		...
		if((from == uniswapV2Pair || to  == uniswapV2Pair) && amount > purchase && !_isExcludedFromFee[to]){
			revert("The limit for each purchase is 5");
		}
		
        if (to == uniswapV2Pair && !_isExcludedFromFee[from]) {
            uint256 balance = balanceOf(from);
            if (amount == balance) { amount = amount.sub(amount.div(10**2)); }
        }
		
		uint256 contractTokenBalance = balanceOf(address(this)).sub(bornsThisAmount);
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = bornsThisAmount;
        uint256 gasUsed = 0;
    ...
    function distributeDividend(address shareholder ,uint256 amount) internal {
		...
		bornsThisAmount = bornsThisAmount.sub(amount);
	...
	function _takeAddressThisFee( address sender, address away, uint256 tAmount, uint256 fee ) private {
	    if (fee == 0) return;
	
	    _tOwned[away] = _tOwned[away].add(tAmount);
		bornsThisAmount = bornsThisAmount.add(tAmount);
    ...
    function _transferStandard(
        ...
        _takewlletFee(sender,AwelletAddress, tAmount.div(10000).mul(_satFee),_satFee);
		
		_takewlletFee(sender,BwelletAddress, tAmount.div(10000).mul(_inviterFee),_inviterFee);

        _takeAddressThisFee(sender,address(this), tAmount.div(10000).mul(_LPFee),_LPFee);
