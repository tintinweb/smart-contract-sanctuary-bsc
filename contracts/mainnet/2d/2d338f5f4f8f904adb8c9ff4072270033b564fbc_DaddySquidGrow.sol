...
// TG - https://t.me/DaddySquidGrow_Portal
...
contract DaddySquidGrow is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress  = payable(0x83E42CFb69Bf5aA6E7b840A784C6590ecE896053);
    ...
    uint256 private _totalFee = _marketingFee.add(_liquidityFee);
    ...
    function removeAllFee() private {
        if(_reflectionFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _totalFee == 0) return;
    ...
    function _transfer(
        ...
        if(!isWalletLimitExempt[to])
            require(balanceOf(to) + amount <= _maxWallet, "You are exceeding maxWallet");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {           
        ...
        if(_marketingFee > 0)
        {
            uint256 marketingAmount = contractTokenBalance.mul(_marketingFee).div(_totalFee);
    ...
    function transferForMarketingETH(address payable to, uint256 amount) private {
        require(to != address(0), "BEP20: Marketing to the zero address");
        to.transfer(amount);
