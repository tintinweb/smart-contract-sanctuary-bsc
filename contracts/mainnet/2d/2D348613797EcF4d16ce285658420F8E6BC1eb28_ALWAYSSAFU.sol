...
contract ALWAYSSAFU is ERC20Detailed, Auth {
    ...
    uint256 public swapThreshold = ( INITIAL_FRAGMENTS_SUPPLY * 80 ) / 10000;
    uint256 public minAmounttoSwap = ( INITIAL_FRAGMENTS_SUPPLY * 2 ) / 10000;
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        if(sender == pair && token.balanceOf(recipient) > minDiscount ||
            sender == pair && tAmount >= _totalSupply.mul(minPercentage).div(1000)){
    ...
    function setNFTDiscount(address _address, uint256 _minHoldings, uint256 _minPercentage) external authorized {
