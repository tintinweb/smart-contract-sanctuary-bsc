...
contract Ownable {
    ...
    constructor() {
       // _owner = msg.sender;
       _owner = 0xD74CB25e777960b93d64327A0342EE123328650C;
    }
...
contract CHRONODAO is ERC20Detailed, Ownable {
    ...
    string public _name = "CHRONODAO";
    string public _symbol = "CHRD";
    ...
    uint256 public totalFee =
        liquidityFee.add(marketingFee).add(
            firePitFee
        );      
    ...
    uint256 private constant MAX_SUPPLY = 500000 * 10**3 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("CHRONODAO", "CHRD", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xe9a47bF80730c5D60da09d067C6e37Cd42E7fb4e;
        marketingReceiver = 0xF454f2D52036BB88320CD787515a26330871d143; 
    
        firePit = 0x65fcE048C1253c61Ff4EFB80ca3D060212F34Fe1;
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (365 days)) {
            ...
            rebaseRate = 2;
        } else if (deltaTimeFromInit >= ((15 * 365 days) / 8)) {
            rebaseRate = 15;
        ...
        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply
                .mul((8**RATE_DECIMALS).add(rebaseRate))
                .div(8**RATE_DECIMALS);
        }
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            _totalFee =
             ...
             .add(sellFeel);
            _marketingFee = marketingFee.add(sellFeet);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_marketingFee)
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToMarketingAndSIF = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(marketingReceiver).call{
            value: amountETHToMarketingAndSIF.mul(marketingFee).div(
                marketingFee
            ),
            gas: 30000
        }("");
   ...
   antiBotEnabled = _enable;
    ...
    function withdrawAllToMarketing() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no CHRD token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function shouldAddLiquidity() internal view returns (bool) {
        return
            ...
            !inSwap && 
            msg.sender != pair &&
            block.timestamp >= (_lastAddLiquidityTime + 8 minutes);
    }
