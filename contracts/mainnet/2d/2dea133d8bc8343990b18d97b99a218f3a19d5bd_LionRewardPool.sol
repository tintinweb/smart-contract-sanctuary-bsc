...
// File: contracts/LionRewardPool.sol
...
contract LionRewardPool is IRewardPool, ReentrancyGuard {
    ...
    uint256 public lionPerSecond = 0.0016534391 ether; 
    ...
    constructor(address _lion, uint256 _poolStartTime) {
        ...
        lionPerSecond = TOTAL_REWARDS.div(runningTime);
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "lionRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOperatorOrTreasury() {
        require(
            ...
            "lionRewardPool: caller is not the operator/treasury"
        );
        _;
    }
    ...
    // Update the pool's start time. Can only be called by the owner.
    function setStartTime(uint256 _startTime) external onlyOperator {
        require(_startTime > block.timestamp, "start must be after now");
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(lionPerSecond);
            return poolEndTime.sub(_fromTime).mul(lionPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(lionPerSecond);
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _lionReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accLionPerShare = accLionPerShare.add(
                _lionReward.mul(1e18).div(tokenSupply)
            );
        }
        return
            user.amount.mul(accLionPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accLionPerShare = pool.accLionPerShare.add(
                _lionReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                .amount
                .mul(pool.accLionPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (_pending > 0) {
                _safeLionTransfer(msg.sender, _pending);
    ...
    function _withdraw(
        ...
        uint256 _pending = user
            ...
            .sub(user.rewardDebt);
        if (_pending > 0) {
            _safeLionTransfer(_account, _pending);
    ...
    function _safeLionTransfer(address _to, uint256 _amount) internal {
        ...
        onlyOperatorOrTreasury
    {
        ...
        uint256 _oldRate = lionPerSecond;
        massUpdatePools();
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (lion or lps) if less than 90 days after pool ends
