...
contract TierTokenLock is Ownable {
    ...
    function lockAndClaimTier(uint256 amount, uint256 lockDays) external {
        require(
            amount >= 2500 * 10**18,
            "OrbitLock: You should lock at least 2500 tokens!"
        );
        require(
            lockDays >= FOURTEEN_DAYS,
            "OrbitLock: Lock period should be at least 14 days!"
        );
        uint256 tokenBalance = IERC20(tokenAddress).balanceOf(msg.sender);
        require(
            tokenBalance >= amount,
            "OrbitLock: You don't have enough tokens to lock!"
        );
        LockInfo memory lockInfo = userLockInfo[msg.sender];
        lockInfo.amount += amount;
        lockInfo.lockTimestamp = block.timestamp;

        lockInfo.unlockTimestamp = block.timestamp + 60 * lockDays;
        // TODO: Uncomment
    ...
    function extendLockTime(uint256 newLockDays) external {
        ...
        require(
            lockInfo.amount > 0,
            "OrbitLock: Additional Days should be at least 14 days!"
        );

        // TODO: need to change for the actual days
        if (lockInfo.unlockTimestamp > block.timestamp) {
            require(
                lockInfo.unlockTimestamp - block.timestamp < 60 * newLockDays,
                "OrbitLock: New Lock Days Should be bigger than your current lock"
            );
        }
        lockInfo.unlockTimestamp = block.timestamp + 60 * newLockDays;
    ...
    function increaseTier(uint256 amount) external {
        ...
        require(
            lockInfo.amount > 0,
            "OrbitLock: You don't have available tier yet!"
        );
        ...
        //         FOURTEEN_DAYS;
        // }
        if (lockInfo.unlockTimestamp - block.timestamp < 60 * FOURTEEN_DAYS) {
            // Increase user's unlock time if it's smaller than 14 days
            lockInfo.unlockTimestamp = block.timestamp + 60 * FOURTEEN_DAYS;
        }
    ...
    function unlockToken() external {
        ...
        require(
            lockInfo.unlockTimestamp <= block.timestamp,
            "OrbitLock: You tokens are still locked!"
        );
