...
contract XIEZHEN is IERC20, Context, Ownable {
    ...
    address private XZ__Marketing_Wallet = 0xEb9B96c58A37c9e37434F2A28bD02AEF8fBCf268;
    address private XZ__Team_Wallet = 0xEb9B96c58A37c9e37434F2A28bD02AEF8fBCf268;
    
    string private constant _symbol = "XZ";
    string private constant _name = "XIEZHEN";

    uint256 public XZ__max_BuyLimit = 2000000 * 10**9;
    uint256 public XZ__max_SellLimit = 500000 * 10**9;

    uint256 public XZ__max_WalletLimit = 2000000  * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[XZ__Marketing_Wallet] = true;
        _isExcludedFromFee[XZ__Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[XZ__Marketing_Wallet] = true;
        allowedTransfer[XZ__Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= XZ__max_BuyLimit, "You are exceeding XZ__max_BuyLimit");
            require(balanceOf(to) + amount <= XZ__max_WalletLimit, "You are exceeding XZ__max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= XZ__max_SellLimit, "You are exceeding XZ__max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= XZ__max_WalletLimit, "You are exceeding XZ__max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(XZ__Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(XZ__Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function XZ__Update_MarketingWallet(address newWallet) external onlyOwner{
        XZ__Marketing_Wallet = newWallet;
    }
    
    function XZ__Update_TeamWallet(address newWallet) external onlyOwner{
        XZ__Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        XZ__max_BuyLimit = maxBuy * 10**decimals();
        XZ__max_SellLimit = maxSell * 10**decimals();

    }
    
    function XZ__Max_Walletlimit(uint256 amount) external onlyOwner{
        XZ__max_WalletLimit = amount * 10**decimals();
    }
