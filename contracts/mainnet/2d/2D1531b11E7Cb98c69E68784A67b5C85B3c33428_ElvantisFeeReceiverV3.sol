...
contract ElvantisFeeReceiverV3 is Ownable, IElvantisFeeReceiver {
    ...
    event SwapAndLiquify(uint256 indexed amount, uint256 indexed newBalance, uint256 indexed otherHalf);
    event SwapTokensForEth(uint256 amount);

    modifier onlyElvantis() {
        require(msg.sender == address(elvantis) || msg.sender == owner(), "ElvantisFeeReceiver: Only Elvantis!");
        _;
    }
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountToSwap = contractTokenBalance / 2;
        uint256 tokenAmount = contractTokenBalance - amountToSwap;

        uint256 initialBalance = address(this).balance;

        // get some eth to liquidate
        if (amountToSwap > swapThreshold) {
            address[] memory path = new address[](2);
            path[0] = address(elvantis);
            path[1] = router.WETH();

            elvantis.approve(address(router), amountToSwap);
            ...
            {
                // the amount of eth we got from swap
                ...
                {
                    emit SwapAndLiquify(amountToSwap, ethAmount, tokenAmount);
                } catch {}
            } catch {}
        }
    }
    ...
    function _swapTokensForETH(uint256 amount, address recipient) private lockTheSwap {
        if (amount > swapThreshold) {
            ...
            try router.swapExactTokensForETHSupportingFeeOnTransferTokens(amount, 0, path, recipient, block.timestamp) {
                emit SwapTokensForEth(amount);
            } catch {}
        }
    }
