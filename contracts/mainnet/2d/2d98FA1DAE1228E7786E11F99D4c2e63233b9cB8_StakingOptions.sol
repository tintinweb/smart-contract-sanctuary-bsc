...
contract StakingOptions is Owned, ReentrancyGuard {
    ...
    bool public stakebreaker = false; // withdraw can be lock,, default unlocked
    ...
    function setStakeBreaker(bool _breaker) external onlyOwner {
        stakebreaker = _breaker;
    }
       ...
       internal
   {
    ...
    function STAKE(uint256 tokens) external nonReentrant returns(bool) {
        require(stakebreaker == false, "Admin Restricted Stake");
        ...
        require(IERC20(LockPayV2).transferFrom(msg.sender, address(this), tokens), "Tokens cannot be transferred from user for locking");
            ...
            uint256 owingl = pendingRewardLockPayV2(msg.sender);
            stakers[msg.sender].remainderLockPayV2 += owingl;

            stakers[msg.sender].lastDividendsLockPayV2 = owingl;
            stakers[msg.sender].fromTotalDividendLockPayV2 = totalDividendsLockPayV2;
            stakers[msg.sender].roundLockPayV2 =  roundLockPayV2;
    ...
    function LockByOwner(uint256 tokens, address staker) external nonReentrant onlyOwner returns(bool){
        //require(breaker == false, "Admin Restricted WITHDRAW");
        //require(block.timestamp >= UnlockTime(msg.sender), "ERR: Funds are Locked");
        

        require(stakers[staker].stakedTokens >= tokens && tokens > 0, "Invalid token amount to withdraw");
        totalStakes = totalStakes.sub(tokens);
        
        // add pending rewards to remainder to be claimed by user later, if there is any existing stake
        uint256 owing = pendingReward(staker);
        stakers[staker].remainder += owing;
                
        stakers[staker].stakedTokens = stakers[staker].stakedTokens.sub(tokens);
        stakers[staker].lastDividends = owing;
        stakers[staker].fromTotalDividend= totalDividends;
        stakers[staker].round =  round;

        uint256 owingl = pendingRewardLockPayV2(staker);
        stakers[staker].remainderLockPayV2 += owingl;
                
        stakers[staker].lastDividendsLockPayV2 = owingl;
        stakers[staker].fromTotalDividendLockPayV2= totalDividendsLockPayV2;
        stakers[staker].roundLockPayV2 =  roundLockPayV2;

        if(stakers[staker].stakedTokens == 0){
            farmTime[staker] =  0;
        }
        
        require(IERC20(LockPayV2).transfer(owner, tokens), "Error in un-staking tokens");
        emit UNSTAKED(staker, tokens);
