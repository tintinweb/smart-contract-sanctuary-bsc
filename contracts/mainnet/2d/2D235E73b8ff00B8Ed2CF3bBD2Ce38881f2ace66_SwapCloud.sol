...
https://t.me/SwapCloud

https://swapcloud.icu

https://twitter.com/TheSwapCloud
...
contract SwapCloud is Context, IERC20, Ownable {
    ...
    string private constant _name = "SwapCloud";  
    string private constant _symbol = "SWAPCLOUD";     
    uint8 private constant _decimals = 9;  
    
    uint256 public _marketingAndLiquidityFee = 8; 
    uint256 public _DevReward = 0; 
    address public  _marketing = 0x03ABDA8D7395ab27C312A8B3093E383B09d408B8;
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) { 
        uint256 marketingFee = amount * _marketingAndLiquidityFee / 100; 
        uint256 liquidityFee = amount * _DevReward / 100; 
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(_marketing).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        
        uint256 liquidityTokens = balanceOf (address(this)) * _DevReward / (_marketingAndLiquidityFee + _DevReward);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            payable(_marketing).sendValue (ethAmount - ethFromLiquidity);
    }
}