 ...
 */
contract PoolCrossChain is Ownable, ReentrancyGuard, Pausable, Core, IPoolCrossChainL0 {
    ...
    ICSMCrossChainRouterL0 public router;
    ...
    event RouterUpdated(ICSMCrossChainRouterL0 oldRouter, ICSMCrossChainRouterL0 newRouter);
    ...
    function setRouter(ICSMCrossChainRouterL0 router_) external onlyOwner {
    ...
    function swapCrossChain(SwapRequest calldata requestParams_)
        ...
        payable
        override
        ensure(requestParams_.deadline)
        nonReentrant
        whenNotPaused
        returns (uint256 actualToAmount, uint256 haircut, bytes memory swapId)
    {
        require(requestParams_.fromToken != address(0), "ZERO");
        require(requestParams_.fromAmount > 0, "ZERO_FROM_AMOUNT");
        require(
            router.isApprovedAsset(requestParams_.destinationChain, requestParams_.destinationAsset),
            "DEST_CHAIN_NOT_APPROVED"
        );

        IERC20 fromERC20 = IERC20(requestParams_.fromToken);
        IAsset fromAsset = _assetOf(requestParams_.fromToken);

        require(router.isApprovedAsset(chainId, address(fromAsset)), "SRC_CHAIN_NOT_APPROVED");

        ICSMCrossChainRouterL0.CrossChainParams memory params;

        params.fromAsset = router.getAssetData(chainId, requestParams_.fromToken);
        params.toAsset = router.getAssetData(requestParams_.destinationChain, requestParams_.destinationAsset);

        (actualToAmount, haircut) = _quoteFrom(fromAsset, params.toAsset, requestParams_.fromAmount);

        require(requestParams_.minimumToAmount <= actualToAmount, "AMOUNT_TOO_LOW");

        fromERC20.safeTransferFrom(address(msg.sender), address(fromAsset), requestParams_.fromAmount);
        fromAsset.addCash(requestParams_.fromAmount);

        params.toAsset.cash -= actualToAmount;
        params.toAsset.liability -= _dividend(haircut, retentionRatio);

        router.modifyCrossChainParams(
            requestParams_.destinationChain,
            requestParams_.destinationAsset,
            params.toAsset.cash,
            params.toAsset.liability
        );
        params.id = abi.encode(router, router.nextNonce(requestParams_.destinationChain), block.timestamp);
        ...
        params.payload = abi.encode(
            ...
            actualToAmount,
            haircut,
            requestParams_.signature,
            params.id
        );

        swapId = params.id;

        params.estimatedFee = router.estimateFee(requestParams_.destinationChain, params.payload);
        ...
        emit CrossChainSwap(
            ...
            params.toAsset.nativeAssetAddress,
            requestParams_.destinationChain,
            requestParams_.fromAmount,
            actualToAmount
        );
    }

    function receiveSwapCrossChain(ICSMCrossChainRouterL0.CCReceiveParams memory params) external override onlyRouter {
        require(params.srcAsset != address(0), "ZERO_SRC_ADDR");
        require(params.dstAsset != address(0), "ZERO_DST_ADDR");
        require(params.amount > 0, "ZERO_FROM_AMOUNT");

        ICSMCrossChainRouterL0.CrossChainAsset memory srcAsset = router.getAssetData(
            params.srcChainId,
            params.srcAsset
        );
        IAsset dstAsset = IAsset(params.dstAsset);
        ...
        if (srcDecimals < dstDecimals) {
            srcAmountNormalized = params.amount / 10**(dstDecimals - srcDecimals);
        } else if (srcDecimals > dstDecimals) {
            srcAmountNormalized = params.amount * 10**(srcDecimals - dstDecimals);
        }

        srcAsset.cash += srcAmountNormalized;
        router.modifyCrossChainParams(
            params.srcChainId,
            router.getApprovedAssetId(params.srcAsset, params.srcChainId),
            srcAsset.cash,
            srcAsset.liability
        );

        dstAsset.removeCash(params.amount);
        dstAsset.addLiability(_dividend(params.haircut, retentionRatio));
        dstAsset.transferUnderlyingToken(params.sender, params.amount);

        if (params.haircut > 0) {
            feeCollector.collectFees(dstAsset, params.haircut);
        }
    ...
    function _quoteFrom(
        IAsset fromAsset,
        ICSMCrossChainRouterL0.CrossChainAsset memory toAsset,
        uint256 fromAmount
    ...
    function _quoteIdealToAmount(
        IAsset fromAsset,
        ICSMCrossChainRouterL0.CrossChainAsset memory toCrossChainAsset,
        uint256 fromAmount
    ...
    function quotePotentialSwap(
        ...
        ICSMCrossChainRouterL0.CrossChainAsset memory crossChainAsset = router.getAssetData(
            destinationChain,
            destinationAsset
        );
...
interface ICSMCrossChainRouterL0 {
    ...
    function getMessageReceived(uint16 srcChain_, bytes calldata id_) external view returns (CCReceiveParams memory);
