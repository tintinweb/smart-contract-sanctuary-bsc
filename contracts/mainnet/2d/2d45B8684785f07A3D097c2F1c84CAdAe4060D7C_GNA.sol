...
contract GNA is Context, IERC20, Ownable {
    ...
    string private _name = "GNA2.0";
    string private _symbol = "GNA2.0";
	...
	// fee list end
	address public ownerAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);
    
	address public marketAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);

    address public marketAddress2 = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);
	
	address public lpAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);

    address public lpAddress2 = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);
	
	address public nftAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);

    address public oskAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);

    address public backAddress = address(0xd212F954caA4C95489a923EFcfD4B7ffa98d945F);
    ...
    function setOskAddress(address adr) public onlyOwner {
        oskAddress = adr;
    }
    ...
    constructor ()  {
        ...
        IW3swapRouter02 _uniswapV2Router = IW3swapRouter02(0xF29acE1FE5f36389d0dDe450a0195A30c3770245);
        uniswapV2Pair = IW3swapFactory(_uniswapV2Router.factory()).createPair(address(this), oskAddress);
    ...
    function _toLpPool2(address sender, uint amount) private{
        if (amount > 0){
    ...
    function _toDao2(address sender, uint amount) private{
        if (amount > 0){
    ...
    function _toNft(address sender, uint amount) private{
        if (amount > 0){
			_rOwned[nftAddress] = _rOwned[nftAddress].add(amount);
			emit Transfer(sender, nftAddress, amount);
		}
	}
	...
	private {
		if (amount > 0) {
            uint256 fee = amount.div(3);
            if (type2 == 0){
                _toPair(sender, fee);
                _toLpPool(sender, fee);
                _toDao(sender, fee);
            } else {
                _toLpPool2(sender, fee);
                _toDao2(sender, fee);
                _toNft(sender, fee);
            }
        }
    ...
    function setMarketAddress2(address adr) public onlyOwner {
        marketAddress2 = adr;
    }

    function setNftAddress(address adr) public onlyOwner {
        nftAddress = adr;
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = IERC20(oskAddress).balanceOf(address(this));

        swapTokensForToken(half); 

        uint256 newBalance = IERC20(oskAddress).balanceOf(address(this)).sub(initialBalance);
    ...
    function swapTokensForToken(uint256 tokenAmount) private {
        ...
        wrap.transfer(oskAddress);
    }

    function addLiquidity(uint256 tokenAmount, uint256 otherAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IERC20(oskAddress).approve(address(uniswapV2Router), otherAmount);
        // add the liquidity
