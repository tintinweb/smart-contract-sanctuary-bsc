    ...
    ReentrancyGuardUpgradeable
{
    ...
    function recoverFunds() external onlyRole(FUNDS_RECOVERY_ROLE) {
        // sale state needs to be finished
        require(saleState == SaleState.FINISHED, "SF");

        uint256 ethToRecover = address(this).balance;

        if (ethToRecover > 0) {
            (bool transferSuccess, ) = payable(msg.sender).call{
                value: ethToRecover
            }("");
            require(transferSuccess, "Recover transfer failed");
        }
    }
