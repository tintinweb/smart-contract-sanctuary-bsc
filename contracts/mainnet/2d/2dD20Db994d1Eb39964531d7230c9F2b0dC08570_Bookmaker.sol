...
contract Bookmaker is ReentrancyGuard, Pausable, Ownable {
    ...
    event Refund(uint256 indexed amount);
    event Bet(address indexed staker, uint256 indexed pool, uint256 indexed amount);
    event Closed(uint256 indexed winningPool);
    ...
    */
    function close(uint256 pool_) external onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            recipients[i].wallet.sendValue(address(this).balance);
            fee += amount;
        }
        reward -= fee;
    ...
    /// @notice Refunds tokens to users.
    function refund() external onlyOwner {
        ...
        for (uint256 i = 0; i < numberOfPools; i++) {
            uint256 stakersLength = _stakers[i].length();
            for (uint256 j = 0; j < stakersLength; j++) {
                address staker = _stakers[i].at(j);
                IERC20(stakingToken).safeTransfer(staker, balances[i][staker]);
                balances[i][staker] = 0;
            }
            supplies[i] = 0;
        }
