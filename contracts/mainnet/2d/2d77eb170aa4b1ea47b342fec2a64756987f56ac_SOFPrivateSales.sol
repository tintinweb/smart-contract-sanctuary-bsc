...
/**
 PS.SOLARFINANCE.IO - SOLARFINANCE.IO - BUSDSTACKING.COM
  ____     ___    _____             ____           _                   _            ____            _        
 / ___|   / _ \  |  ___|           |  _ \   _ __  (_) __   __   __ _  | |_    ___  / ___|    __ _  | |   ___ 
 \___ \  | | | | | |_       _____  | |_) | | '__| | | \ \ / /  / _` | | __|  / _ \ \___ \   / _` | | |  / _ \
  ___) | | |_| | |  _|     |_____| |  __/  | |    | |  \ V /  | (_| | | |_  |  __/  ___) | | (_| | | | |  __/
 |____/   \___/  |_|               |_|     |_|    |_|   \_/    \__,_|  \__|  \___| |____/   \__,_| |_|  \___|
...
contract SOFPrivateSales {
    ...
    address busdtoken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;  /** BUSD token **/
    uint256 transactionCount;
    uint256 public AMOUNT_TO_CHECK = 38;  
    bool public contractStarted = true;
    address public owner;

    event TransferBlock(string email, uint256 timestamp, uint amount);
    ...
    constructor() {
        ...
        token_BUSD = BUSDToken(busdtoken);
    }
    ...
    function Deposit(string memory email, uint amount) public{
       ...
       require(user.initialDeposit.add(amount) <= MAX_DEPOSIT_LIMIT, "Max deposit limit reached.");
            token_BUSD.transfer(addReceive,amount);
            ...
            transactions.push(TransferStruct(email, block.timestamp, amount ));
           emit TransferBlock( email, block.timestamp, amount);
