...
contract CthulhuStaking_Tether {
    ...
    function withdrawTether(address to, uint256 amount) external onlyOwner {
        uint256 balance = _token.balanceOf(address(this));
        if (amount > balance) {
    ...
    function depositTether(uint256 amount) external onlyOwner {
