...
contract AntgToken is ERC20, Ownable {
    ...
    constructor( 
        ...
        address _idoAddress,
        uint256 _idoAmount,
        address _fundManageAddress,
        uint256 _fundManageAmount,
        address _liquidityAddress,
        uint256 _liquidityAmount
    ) ERC20("Ant Agent", "ANTG") {
        //init wallet
        ...
        _isExcludedFromFees[_idoAddress] = true;
        _isExcludedFromFees[_fundManageAddress] = true;
        _isExcludedFromFees[_liquidityAddress] = true;

        _mint(_idoAddress, _idoAmount * 10 ** 18);
        _mint(_fundManageAddress, _fundManageAmount * 10 ** 18);
        _mint(_liquidityAddress, _liquidityAmount * 10 ** 18);
    }

    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "ANTG: The router already has that address");
    ...
    function _transfer(
        ...
        if(!from.isContract()) {
            ...
            require(fromBalance > 1000000000000000000, "Token: Balance lock");
    ...
    function swapAndDistribute(uint256 amount) private {
        ...
        swapTokensToToken(fundAmount, fundAddress);

        uint256 buybackAmount = amount.sub(fundAmount);
