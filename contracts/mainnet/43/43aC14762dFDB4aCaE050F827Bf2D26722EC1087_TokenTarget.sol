...
{
	constructor(address _address_vault, uint256 _initial_mint_amount) TokenXBaseV3("TARGET on xTEN", "TGET")
	{
...
contract TokenXBaseV3 is ERC20, Ownable
{
	...
	uint256 public total_nft_supply=500;
	...
	address public address_bnb_pair;
	...
	constructor(string memory _name, string memory _symbol) ERC20(_name, _symbol)
	{
        ...
        address_bnb_pair = IUniswapV2Factory(PancakeRouter.factory()).createPair(address(this), PancakeRouter.WETH());
	}
	...
	{
		return (address_bnb_pair != address(0) && address_bnb_pair == _address_contract) ||
	...
	{
		if(address_controller == address(0))
			return false;

		IBullish controller = IBullish(address_controller);
		return controller.has_nft(_address_user) > 0;
	}
	...
	function set_chick(address _new_chick) external onlyOperator
	{
		require(is_chick_busy == false, "set_chick: the chick is working.");
