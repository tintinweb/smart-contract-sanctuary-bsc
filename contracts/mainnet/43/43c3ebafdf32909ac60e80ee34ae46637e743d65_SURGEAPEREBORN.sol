...
// SURGE APE REBORN THE COMMUNITY WAS ABANDONED AND ITSTIMETOMOON SAW HOW MANY WERE SO PASSIONATE ABOUT THE PROJECT HE IS REBRANDING AND TRYING TO HELP THEM BY REVIVING IT WITH A SAFU TEAM
...
contract SURGEAPEREBORN is IERC20, Context, Ownable, ReentrancyGuard {
    event Bought(
    ...
    // token data
    string private constant _name = "SURGE APE REBORN";
    ...
    // Total Supply
    uint256 public constant _totalSupply = 10**6*1* _DECMULTIPLIER;// 1M supply 
    ...
    //Tax wallets
    address public developmentwallet = 0x27fD7C8Da34F35C6416440664cdec8BE52d85cbC;
    address public treasuryWallet = 0x3FBbF210c4BB5a7A6727c542C6d5b6418161d849;

    // Tax Split
    uint256 public development = 300;
    uint256 public treasuryShare = 200;//Staking pool
    ...
    //trading parameters
    uint256 public liquidity = 200000 * 10**_srgDecimals;
    ...
    function changeTaxDistribution(
        ...
        require(
            newdevelopment + newtreasuryShare + newBurnShare + newRewardShare ==
                SHAREDIVISOR,
    ...
    function changeFeeReceivers(
        ...
        require(
            newdevelopmentwallet != address(0) && newTreasuryWallet != address(0),
    ...
    function withdrawTaxBalance() external nonReentrant onlyOwner{
        if(development>0){
        bool temp1 = SRGIE.transfer(developmentwallet, (taxBalance * development) /(development + treasuryShare));
        require(temp1,"failed to send funds");
        }

        if(treasuryShare>0){
        bool temp2 = SRGIE.transfer(treasuryWallet, (taxBalance * treasuryShare) /(development + treasuryShare));
