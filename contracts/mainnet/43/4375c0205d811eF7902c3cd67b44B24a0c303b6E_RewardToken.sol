...
contract MasterBuilder is Ownable {
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, bool _withUpdate,uint256 _harvestInterval) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate,uint256 _harvestInterval) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
...
function canHarvest(uint256 _pid, address _user) public view returns (bool) {
    ...
    function harvestFarms(uint256 _pid) public {

        require (_pid != 0, 'harvest DexToken by staking');
...
function harvestStaking() public {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[0][msg.sender];
        require(user.nextHarvestUntil<block.timestamp,"harvest interval not come");
