...
contract Miner is ERC20Burnable, AccessControl {
    ...
    constructor() ERC20("MINER", "Excavate Miner") {}
    ...
    function setTotalMinted(uint256 _totalMinted)
        external
        checkRole(msg.sender, DEFAULT_ADMIN_ROLE)
    {
    ...
    function setTaxAddress(address _tax_address, uint256 _taxId)
        external
        checkRole(msg.sender, OPERATOR_ROLE)
    {
        isTaxed[_tax_address] = true;
        ...
        returns (uint256, uint256)
    {
        ...
        if (tax_info.endTime >= block.timestamp) {
            newTaxPercent = tax_info.startRate.sub(
                (tax_info.endRate.div(endTime)).mul(currentTime)
            );
        } else {
