...
contract UbtStake is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    mapping(uint => uint) public dailyBurn;

    uint public lastClearTime;

    struct RankInfo {
        mapping(address => uint) addressToRank;
        mapping(uint => address) rankToAddress;
        mapping(address => uint) referCardAmount;
    }

    mapping(uint => RankInfo) rankInfo;
    ...
    modifier checkMarket(){
        ...
        if (block.timestamp > market.lastTime + 86400 && sell.amount != 0) {
            ...
            if (sell.claimed < sell.amount) {
                token.transfer(sell.addr, sellRew);
            }
            if (market.amount <= market.claimed) {
    ...
    function _processRefer(address addr, uint power_, uint amount) internal {
        ...
        if (left > 0) {
            token.transfer(0x000000000000000000000000000000000000dEaD, left);
            dailyBurn[getCurrentTime()] += left;
        }

    }

    function checkDailyBurn() public view returns (uint){
        return (dailyBurn[getCurrentTime()]);
    }

    function _processReferCard(address addr) internal {
        uint tempAmount = rankInfo[lastClearTime].referCardAmount[addr];
        uint rank = rankInfo[lastClearTime].addressToRank[addr];
        if (rank == 0 || rank >= 10) {
            rank = 10;
        }
        if (rank == 1) {
        ...
        if (tempAmount <= rankInfo[lastClearTime].referCardAmount[rankInfo[lastClearTime].rankToAddress[10]]) {
            return;
        }
        uint oldRank = rank;
        uint newRank = 1;
        for (uint i = rank - 1; i > 0; i--) {
            if (tempAmount >= rankInfo[lastClearTime].referCardAmount[rankInfo[lastClearTime].rankToAddress[i]] || rankInfo[lastClearTime].rankToAddress[i] == address(0)) {
                continue;
            }
            if (tempAmount <= rankInfo[lastClearTime].referCardAmount[rankInfo[lastClearTime].rankToAddress[i]]) {
                newRank = i + 1;
            }
        }
        if (newRank == oldRank) {
        ...
        for (uint j = oldRank; j >= newRank; j--) {

            rankInfo[lastClearTime].rankToAddress[j] = rankInfo[lastClearTime].rankToAddress[j - 1];
            rankInfo[lastClearTime].addressToRank[rankInfo[lastClearTime].rankToAddress[j]] = j;
        }
        rankInfo[lastClearTime].addressToRank[addr] = newRank;
        rankInfo[lastClearTime].rankToAddress[newRank] = addr;
    }

    function checkRank() public view returns (address[10] memory addrs, uint[10] memory cardAmount){
        for (uint i = 1; i <= 10; i++) {
            addrs[i - 1] = rankInfo[lastClearTime].rankToAddress[i];
            cardAmount[i - 1] = rankInfo[lastClearTime].referCardAmount[rankInfo[lastClearTime].rankToAddress[i]];
        }
    }

    function stake(uint[] memory cardId, address invitor) checkMarket external {
        ...
        rankInfo[lastClearTime].referCardAmount[userInfo[msg.sender].invitor] += cardId.length;
        _processReferCard(userInfo[msg.sender].invitor);
    ...
    function claimReward() checkMarket external {
