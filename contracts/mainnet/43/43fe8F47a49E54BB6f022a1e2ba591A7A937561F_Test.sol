...
constructor (){
    ...
    function staking(uint256 ordertype, uint256 orderamount) public {
        ...
        pushorder(orderid,ordertype,orderSTime,orderOTime,amount,0,0);
    }
   ...
   function getwebdate()  view public returns (uint256 power,uint256 relieve,uint256 ordertype,uint256 orderamount,uint256 nowamount,uint256 endtime) {
    ...
    function pushorder(uint256 orderid,uint256 ordertype,uint256 orderSTime,uint256 orderOTime,uint256 amount,uint256 orderamount,uint256 status) private {
        orderlist.push([orderid,ordertype,orderSTime,orderOTime,amount,orderamount,status]);
    }
    ...
    function getorderlistdate(uint256 orderid) view public returns (uint256,uint256,uint256,uint256,uint256,uint256) {
        if(orderid <= orderlist.length && orderid > 0){
            return (orderlist[orderid-1][1],orderlist[orderid-1][2],orderlist[orderid-1][3],orderlist[orderid-1][4],orderlist[orderid-1][5],orderlist[orderid-1][6]); 
        }else{
            return (0,0,0,0,0,0);
        }
    }
