...
contract DynamicUnderlyingStakingRewardsWithLock is IStakingRewards, Ownable, Readable {
    ...
    constructor(address _owner, IERC20 _rewardsToken, IERC20 _stakingToken, IERC20 _underlyingToken, uint _withdrawTimeout) {
    ...
    function balanceOf(address account) external override view returns (uint) {
        uint stake = users[account].stake;
        if (stake == 0) {
            return 0;
        }
        return uint(users[account].underlyingStake).mul(stakingBalanceOf(account)).div(stake);
    }

    function stakingBalanceOf(address account) public view returns (uint) {
    ...
    function earned(address account) public override view returns (uint) {
        return uint(users[account].underlyingStake).mul(rewardPerToken().sub(userRewardPerTokenPaid[account])).div(1e18).add(rewards[account]);
    }
    ...
    function stake(uint amount) external override updateReward(msg.sender) {
        ...
        IUniswapPair(address(stakingToken)).skim(address(this));
        require(balanceBefore == underlyingToken.balanceOf(address(this)), 'Staking manipulation');
        uint underlyingStake = underlyingToken.balanceOf(address(stakingToken)).mul(amount).div(stakingToken.totalSupply());
        _totalSupply = _totalSupply.add(underlyingStake);
        ...
        users[msg.sender].underlyingStake = uint(users[msg.sender].underlyingStake).add(underlyingStake).toUInt88();
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        emit Staked(msg.sender, underlyingStake, amount);
    }

    function restake() external {
        User storage user = users[msg.sender];
        uint requested = user.requested;
        uint underlyingStake = uint(user.underlyingStake).mul(user.requested).div(user.stake);
        user.readyAt = 0;
        user.requested = 0;
        emit Restaked(msg.sender, underlyingStake, requested);
    }
    ...
    function requestWithdrawal(uint amount) external {
        ...
        uint underlyingStake = uint(user.underlyingStake).mul(amount).div(user.stake);
        emit WithdrawRequested(msg.sender, underlyingStake, amount, newReadyAt);
    }
    ...
    function withdraw(uint amount) public override updateReward(msg.sender) {
        ...
        if (user.stake == amount) {
            underlyingStake = user.underlyingStake;
        }
        user.stake = uint(user.stake).sub(amount).toUInt88();
        user.requested = uint(user.requested).sub(amount).toUInt88();
        user.underlyingStake = uint(user.underlyingStake).sub(underlyingStake).toUInt88();
        users[msg.sender] = user;
        _totalSupply = _totalSupply.sub(underlyingStake);
        stakingToken.safeTransfer(msg.sender, amount);
        emit Withdrawn(msg.sender, underlyingStake, amount);
    }
    ...
    event Staked(address indexed user, uint amount, uint stakingAmount);
    event Restaked(address user, uint amount, uint stakingAmount);
    event WithdrawRequested(address user, uint amount, uint stakingAmount, uint readyAt);
    event Withdrawn(address indexed user, uint amount, uint stakingAmount);
    event RewardPaid(address indexed user, uint reward);
    event Recovered(IERC20 token, uint amount);
    event StakingCapSet(uint newCap);
}

contract BULKStakingRewardsV2Test is DynamicUnderlyingStakingRewardsWithLock(
...
contract BULKStakingRewardsV2 is DynamicUnderlyingStakingRewardsWithLock(
    0xE6f9422826DFdd43382d754e4111408242b39355,
    IERC20(0xa143ac515dCA260A46C742C7251eF3B268639593),
    IERC20(0xcc0b4B2E5185f8977089a989A662155742EF5263),
    IERC20(0xa143ac515dCA260A46C742C7251eF3B268639593),
    15 days) {}
