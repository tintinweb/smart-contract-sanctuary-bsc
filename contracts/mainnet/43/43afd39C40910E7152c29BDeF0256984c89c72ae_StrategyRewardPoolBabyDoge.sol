...
import "./StrategyRewardPool.sol";


contract StrategyRewardPoolBabyDoge is StrategyRewardPool {
  ...
  function balanceOfPool() public view override virtual returns (uint256) {
    (uint256 _amount,, ) = IERC20Farm(rewardPool).userInfo(address(this));
    return _amount * IERC20Farm(rewardPool).pricePerShare();
  }
...
// INTERNAL FUNCTIONS
  ...
  function _poolDeposit(uint256 _amount) internal override virtual {
    if (_amount > 0) {
      IERC20Farm(rewardPool).deposit(_amount);
    }
    else { // harvest
      _poolWithdraw(0);
    }
  ...
  function _poolWithdraw(uint256 _amount) internal override virtual {
    ...
    if (shares > 0) {
      shares = _amount / IERC20Farm(rewardPool).pricePerShare();
    }

    IERC20Farm(rewardPool).withdraw(shares);
  }

  function _emergencyWithdraw() internal override virtual {
    uint256 poolBal = balanceOfPool();
    if (poolBal > 0) {
      IERC20Farm(rewardPool).emergencyWithdraw();
    }
  }
  ...
  function _outputBalance() internal override returns (uint256) {
    ...
    uint256 outputAmount = (allBal * (MAX_FEE - poolFee) / MAX_FEE);

    if (output == BABYDOGE) {
      uint256 coinFee = IBabyDogeCoin(output)._taxFee();
      coinFee+= IBabyDogeCoin(output)._liquidityFee();

      outputAmount = outputAmount * (MAX_BABYDOGE_FEE - coinFee) / MAX_BABYDOGE_FEE;
    }

    if (outputAmount == 0) return 0;

    uint256 minAmount = IERC20Farm(rewardPool).defaultStakePPS();
    uint256 amount = _checkOutput(outputAmount);
...
// INTERNAL FUNCTIONS
    ...
    function _poolDeposit(uint256 amount) internal override virtual {
    ...
    function _swapRewards(uint256 toWant) internal virtual {
        if (output != want) {
            // uint256 toWant = IERC20(output).balanceOf(address(this));
            _swapToken(toWant, outputToWantRoute, address(this));
        }
    }
}
