...
contract NFTMeta is ERC721,Ownable {
    ...
    // function preSubSalemint(uint256 cardId,uint256 tokenQuantity) public payable
    // {  
    //     cardToSubNft[cardId].preSalemint(tokenQuantity,msg.sender);
    // }

    // function addSub(uint256 tokenId,NFTMetaSub subNftAddr) public
    // {
    //     cardToSubNft[tokenId]=subNftAddr;
    // }


    // function getSubNftList(uint256 cardId, address owner,uint256 pageSize) public  view  returns (uint256[] memory)  
    // {
    //     return cardToSubNft[cardId].getAllToken(owner,pageSize);
    // }
...
import "hardhat/console.sol";
 ...
 contract NFTMetaSub is ERC721,Ownable {
    ...
    function _mintNicMeta(uint256 tokenQuantity) internal {
        for (uint256 i = 0; i < tokenQuantity; i++) {
            ...
            if (totalSupply() < MAX_SUPPLY) {
                ...
                cardNft[cardNftConfig.token_id].push(mintIndex);
                subNftToCard[mintIndex]=cardNftConfig.token_id;
            }
        }
    }
    ...
    function setCardOwner(address addr) public  onlyOwner
    {
        cardOwner=addr;
    }
    //合成
    function composeNft(uint256 cardId) public
    {
        uint256 len= getNftLengthByCardId(cardId, msg.sender,1000);
        if(len>9)
        {
            //进行合成的操作,卡片是否授权
            require(isApprovedForAll(cardOwner,address(this)));
            transferFrom(cardOwner, msg.sender, cardId);
