...
// File contracts/interfaces/IEllipsisChef.sol

pragma solidity >=0.5.0;

interface IEllipsisChef {
    // ===== Write =====
...
// File contracts/StrategyEllipsisBase.sol

pragma solidity =0.6.12;


contract StrategyEllipsisBase is BaseStrategyBSC {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using AddressUpgradeable for address;
    using SafeMathUpgradeable for uint256;

    address public ellipsisSwap; // Ellipsis renBtc Swap
    address public lpComponent; // single component to be added on harvests

    address public constant eps = 0xA7f552078dcC247C2684336020c03648500C6d9F; // EPS token
    address public constant weth = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // WBNB Token, used for eps <> weth <> btcb route
    address public constant btcb = 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c; // BTCB Token
    address public constant renbtc = 0xfCe146bF3146100cfe5dB4129cf6C82b0eF4Ad8c; // renBTC token

    address public constant chef = 0xcce949De564fE60e7f96C85e55177F8B9E4CF61b; // Master staking contract
    uint256 public pid;

    uint256 public keepEPS;

    event EllipsisHarvest(
        uint256 epsHarvested,
        uint256 keepEps,
        uint256 epsRecycled,
        uint256 lpComponentDeposited,
    ...
    function getName() external override pure returns (string memory) {
        return "StrategyEllipsis";
    }

    function balanceOfPool() public override view returns (uint256) {
        (uint256 balance, ) = IEllipsisChef(chef).userInfo(pid, address(this));
        return balance;
    }
    ...
    function getProtectedTokens() external view override returns (address[] memory) {
        ...
        protectedTokens[2] = eps;
        return protectedTokens;
    }

    /// ===== Permissioned Actions: Governance =====
    function setKeepEPS(uint256 _keepEPS) external {
        _onlyGovernance();
        keepEPS = _keepEPS;
    }
    ...
    function _onlyNotProtectedTokens(address _asset) internal override {
        ...
        require(eps != _asset, "eps");
    }

    function _deposit(uint256 _want) internal override {
        IEllipsisChef(chef).deposit(pid, _want);
    }

    function _withdrawAll() internal override {
        (uint256 toWithdraw, ) = IEllipsisChef(chef).userInfo(pid, address(this));
        IEllipsisChef(chef).withdraw(pid, toWithdraw);
    }

    function _withdrawSome(uint256 _amount) internal override returns (uint256) {
        IEllipsisChef(chef).withdraw(pid, _amount);
        return _amount;
    }
    ...
    function harvest() external whenNotPaused returns (HarvestData memory) {
        ...
        uint256 _beforeLp = IERC20Upgradeable(want).balanceOf(address(this));
        uint256 _beforeEps = IERC20Upgradeable(eps).balanceOf(address(this));

        // Note: Deposit of zero harvests rewards balance, but go ahead and deposit idle want if we have it
        IEllipsisChef(chef).deposit(pid, _beforeLp);

        uint256 _afterEps = IERC20Upgradeable(eps).balanceOf(address(this));

        harvestData.epsHarvested = _afterEps.sub(_beforeEps);
        uint256 _eps = _afterEps;

        // Transfer EPS to keep to Rewards
        harvestData.keepEps = _eps.mul(keepEPS).div(MAX_FEE);
        if (harvestData.keepEps > 0) {
            IERC20Upgradeable(eps).safeTransfer(IController(controller).rewards(), harvestData.keepEps);
        }

        harvestData.epsRecycled = _eps.sub(harvestData.keepEps);

        // Convert remaining EPS to lpComponent
        if (harvestData.epsRecycled > 0) {
            address[] memory path = new address[](3);
            path[0] = eps;
            path[1] = weth;
            path[2] = lpComponent;
            _swap(eps, harvestData.epsRecycled, path);
        }

        // Deposit into Ellipsis to increase LP position
        ...
        if (harvestData.lpComponentDeposited > 0) {
            _safeApproveHelper(lpComponent, ellipsisSwap, harvestData.lpComponentDeposited);
            _add_liquidity_ellipsis(harvestData.lpComponentDeposited);
        }
        ...
        emit EllipsisHarvest(
            harvestData.epsHarvested,
            harvestData.keepEps,
            harvestData.epsRecycled,
            harvestData.lpComponentDeposited,
            harvestData.wantProcessed,
    ...
    /// @dev Handle the particular function variant for EllipsisSwap
    function _add_liquidity_ellipsis(uint256 _amount) internal virtual {
        // e.g. ICurveFi(ellipsisSwap).add_liquidity([0, _amount, 0], 0);
    }
}


// File contracts/APY_Helper_BSC.sol
...
contract APY_Helper is Ownable {
    ...
    address public constant cakeChef = 0x73feaa1eE314F8c655E354234017bE2193C9E24E; // Master staking contract
    address public constant banana = 0x603c7f932ED1fc6575303D8Fb018fDCBb0f39a95; // BANANA token
    address public constant bananaChef = 0x5c8D727b265DBAfaba67E050f2f739cAeEB4A6F9; // Master staking contract
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant epsChef = 0xcce949De564fE60e7f96C85e55177F8B9E4CF61b; // Master staking contract
    address public constant eps = 0xA7f552078dcC247C2684336020c03648500C6d9F; // EPS token
    address public constant pancakeswapRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public quai_controller;
    //using target of 3 seconds/block
    ...
    function getYearlyCakeRewards(address strategy) public view returns(uint256) {
        uint256 pid = StrategySushiLpOptimizer(strategy).pid();
        (uint256 lpDeposited, ) = IPancakeChef(cakeChef).userInfo(pid, strategy);
        uint256 cakePerBlock = IPancakeChef(cakeChef).cakePerBlock();
        uint256 totalAllocPoint = IPancakeChef(cakeChef).totalAllocPoint();
        (address lpToken, uint256 poolAllocPoint, , ) = IPancakeChef(cakeChef).poolInfo(pid);
        uint256 lpInChef = IERC20Upgradeable(lpToken).balanceOf(cakeChef);
        uint256 cakeRewardsPerYear = (cakePerBlock * blocksPerYear * poolAllocPoint * lpDeposited) / (totalAllocPoint * lpInChef);
        return cakeRewardsPerYear;
    }

    function getYearlyBananaRewards(address strategy) public view returns(uint256) {
        uint256 pid = StrategySushiLpOptimizer(strategy).pid();
        (uint256 lpDeposited, ) = IPancakeChef(bananaChef).userInfo(pid, strategy);
        uint256 cakePerBlock = IPancakeChef(bananaChef).cakePerBlock();
        uint256 totalAllocPoint = IPancakeChef(bananaChef).totalAllocPoint();
        (address lpToken, uint256 poolAllocPoint, , ) = IPancakeChef(bananaChef).poolInfo(pid);
        uint256 lpInChef = IERC20Upgradeable(lpToken).balanceOf(bananaChef);
        uint256 bananaRewardsPerYear = (cakePerBlock * blocksPerYear * poolAllocPoint * lpDeposited) / (totalAllocPoint * lpInChef);
        return bananaRewardsPerYear;
    }

    function getYearlyEpsRewards(address strategy) public view returns(uint256) {
        uint256 pid = StrategySushiLpOptimizer(strategy).pid();
        (uint256 lpDeposited, ) = IEllipsisChef(epsChef).userInfo(pid, strategy);
        uint256 rewardsPerSecond = IEllipsisChef(epsChef).rewardsPerSecond();
        uint256 totalAllocPoint = IEllipsisChef(epsChef).totalAllocPoint();
        (address lpToken, uint256 poolAllocPoint, , ,) = IEllipsisChef(epsChef).poolInfo(pid);
        uint256 lpInChef = IERC20Upgradeable(lpToken).balanceOf(epsChef);
        uint256 epsRewardsPerYear = (rewardsPerSecond * secondsPerYear * poolAllocPoint * lpDeposited) / (totalAllocPoint * lpInChef);
        return epsRewardsPerYear;
    }

    //price of 1e18 cake in WBNB
    function getCakePriceInWbnb() public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = cake;
        path[1] = WBNB;
        uint256[] memory amountsOut = IUniswapRouterV2(pancakeswapRouter).getAmountsOut(1e18, path);
        uint256 wethOut = amountsOut[amountsOut.length - 1];
        return wethOut;
    }

    //price of 1e18 banana in WBNB
    function getBananaPriceInWbnb() public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = banana;
        path[1] = WBNB;
        uint256[] memory amountsOut = IUniswapRouterV2(pancakeswapRouter).getAmountsOut(1e18, path);
    ...
    //price of 1e18 CRV in WBNB
    function getEpsPriceInWbnb() public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = eps;
        path[1] = WBNB;
        uint256[] memory amountsOut = IUniswapRouterV2(pancakeswapRouter).getAmountsOut(1e18, path);
    ...
    function getSLPPriceInWeth(address lpToken) public view returns(uint256) {
        ...
        if(token0 == WBNB) {
            ...
            path[0] = token0;
            path[1] = WBNB;
            uint256[] memory amountsOut = IUniswapRouterV2(pancakeswapRouter).getAmountsOut(token0PerLP, path);
    ...
    function getScaledCakeAPY(address lpToken) public view returns(uint256) {
        ...
        uint256 cakeRewardsPerYear = getYearlyCakeRewards(strategy);
        uint256 yearlyWbnbRewards = cakeRewardsPerYear * getCakePriceInWbnb();
        uint256 lpPriceInWeth = getSLPPriceInWeth(lpToken);
        uint256 totalLpValueInWeth = totalLpInvested * lpPriceInWeth;
        return (yearlyWbnbRewards * 1e18) / totalLpValueInWeth;
    }

    //result is scaled up by 1e18
    function getScaledBananaAPY(address lpToken) public view returns(uint256) {
        address vault = IQUAI_Controller(quai_controller).vaults(lpToken);
        address strategy = IQUAI_Controller(quai_controller).strategies(lpToken);
        uint256 totalLpInvested = getTotalInvested(vault);
        uint256 cakeRewardsPerYear = getYearlyBananaRewards(strategy);
        uint256 yearlyWbnbRewards = cakeRewardsPerYear * getBananaPriceInWbnb();
        uint256 lpPriceInWeth = getSLPPriceInWeth(lpToken);
        uint256 totalLpValueInWeth = totalLpInvested * lpPriceInWeth;
        return (yearlyWbnbRewards * 1e18) / totalLpValueInWeth;
    }

    //price of 1e18 Curve LP in WBNB
    function getCurveSwap(address lpToken) public view returns(address) {
        address strategy = IQUAI_Controller(quai_controller).strategies(lpToken);
        address curveSwap = StrategyEllipsisBase(strategy).ellipsisSwap();
        return curveSwap;
    }

    function getCurveSwaptoken0PerLP(address curveSwap) public view returns(address, uint256) {
        address token0 = ICurveSwap(curveSwap).coins(0);
        uint256 token0PerLP = ICurveSwap(curveSwap).calc_withdraw_one_coin(1e18, 0);
        return (token0, token0PerLP);
    }

    function getCurveLPPriceInWbnb(address lpToken) public view returns(uint256) {
        address curveSwap = getCurveSwap(lpToken);
        (address token0, uint256 token0PerLP) = getCurveSwaptoken0PerLP(curveSwap);
        address[] memory path = new address[](2);
        path[0] = token0;
        path[1] = WBNB;
        uint256[] memory amountsOut = IUniswapRouterV2(pancakeswapRouter).getAmountsOut(token0PerLP, path);
    ...
    function getScaledCurveAPY(address lpToken) public view returns(uint256) {
        ...
        address strategy = IQUAI_Controller(quai_controller).strategies(lpToken);
        uint256 totalLpInvested = getTotalInvested(vault);
        uint256 epsRewardsPerYear = getYearlyEpsRewards(strategy);
        uint256 yearlyWbnbRewards = epsRewardsPerYear * getEpsPriceInWbnb();
        uint256 lpPriceInWeth = getCurveLPPriceInWbnb(lpToken);
        uint256 totalLpValueInWeth = totalLpInvested * lpPriceInWeth;
        return (yearlyWbnbRewards * 1e18) / totalLpValueInWeth;
    }
