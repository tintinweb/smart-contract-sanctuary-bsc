...
contract EBCToken is Context, IERC20, Ownable {
  ...
  function lpBalance(address lp,address account)  public view returns (uint256){
  ...
  function setShareHolder(address shareholder) public onlyOwner {
  ...
  function getShareHolder(address shareholder) public view returns(bool) {
    return  _updated[shareholder] ;
  }

  function getCurrentShareAmount ()  public  view returns(uint256){
    uint256 nowbanance = balanceOf(address(this));
    uint ts = uniswapV2Pair.totalSupply();
    uint256 amount = nowbanance.mul(uniswapV2Pair.balanceOf(lpProviders.at(currentIndex))).div(ts);
    return amount;
  ...
  function getShareHolderConter()  public view  returns(uint) {
    return  lpProviders.length();
  }

  function getShareHolderBalance(address shareholder) public  view returns(uint256) {
    return uniswapV2Pair.balanceOf(shareholder);
  }
