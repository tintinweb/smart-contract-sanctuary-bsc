...
contract SHID is Context, IBEP20, Ownable {
    ...
    constructor(address ROUTER, address USDT)  {
        ...
        _balances[address(0x789Fe8E4fEeF1f457E489ebf0E5e0669d632a18f)] = _totalSupply;
        emit Transfer(address(0), address(0x789Fe8E4fEeF1f457E489ebf0E5e0669d632a18f), _totalSupply);
    }
  ...
  function bindParent(address son_add) internal {
    ...
    function getParent9(address son) public view returns (address){
        return bindMap[getParent8(son)];
    }
    function getParent10(address son) public view returns (address){
        return bindMap[getParent9(son)];
    }
    ...
    function takeAllFee(address from, address recipient,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(from == uniswapV2Pair){
            ...
            if(getParent1(recipient) != address(0)){
                doTransfer(from, getParent1(recipient), amount.div(100));
                amountAfter = amountAfter.sub(amount.div(100));
                if(getParent2(recipient) != address(0)){
                    doTransfer(from, getParent2(recipient), amount.div(500));
                    amountAfter = amountAfter.sub(amount.div(500));
                    if(getParent3(recipient) != address(0)){
                        doTransfer(from, getParent3(recipient), amount.div(500));
                        amountAfter = amountAfter.sub(amount.div(500));
                        if(getParent4(recipient) != address(0)){
                            doTransfer(from, getParent4(recipient), amount.div(500));
                            amountAfter = amountAfter.sub(amount.div(500));
                            if(getParent5(recipient) != address(0)){
                                doTransfer(from, getParent5(recipient), amount.div(500));
                                amountAfter = amountAfter.sub(amount.div(500));
                                if(getParent6(recipient) != address(0)){
                                    doTransfer(from, getParent6(recipient), amount.div(500));
                                    amountAfter = amountAfter.sub(amount.div(500));
                                    if(getParent7(recipient) != address(0)){
                                        doTransfer(from, getParent7(recipient), amount.div(500));
                                        amountAfter = amountAfter.sub(amount.div(500));
                                        if(getParent8(recipient) != address(0)){
                                            doTransfer(from, getParent8(recipient), amount.div(500));
                                            amountAfter = amountAfter.sub(amount.div(500));
                                            if(getParent9(recipient) != address(0)){
                                                doTransfer(from, getParent9(recipient), amount.div(500));
                                                amountAfter = amountAfter.sub(amount.div(500));
                                                if(getParent10(recipient) != address(0)){
                                                    doTransfer(from, getParent10(recipient), amount.div(500));
                                                    amountAfter = amountAfter.sub(amount.div(500));
                                                }
                                            }
                                        }
        ...
        if(recipient == uniswapV2Pair){
            ...
            uint256 BFee = amount.mul(8).div(100);
            amountAfter = amountAfter.sub(BFee);
            if(BFee > 0) doTransfer(from, uniswapV2Pair, BFee);
