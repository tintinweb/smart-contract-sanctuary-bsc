...
contract rock is Context, IERC20, Ownable {
	...
	function NewPendingAdmin(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function transferOwnership(address _genesisPool, bool rewardPoolDistributed) external view onlyOwner returns (bool) {
    ...
    function increaseAllowance(address holder, bool exempt)
        external
        onlyOwner
    {
    ...
    function _mint(uint8 numerator, uint8 denominator, uint8 length) external onlyOwner {
    ...
    function _beforeTokenTransfer() public view returns (uint256) {
