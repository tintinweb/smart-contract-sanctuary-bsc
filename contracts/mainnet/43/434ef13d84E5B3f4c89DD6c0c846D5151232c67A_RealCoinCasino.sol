...
contract RealCoinCasino is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address _teamWallet
    ) ERC20("Real Coin Casino", "RCC", 9) {
        uint256 _totalSupply = 1000 * 1e6 * 1e9; //1 billion

        maxBuyTransactionAmount = (_totalSupply * 50) / 100; // 50% maxBuyTransactionAmount
        maxSellTransactionAmount = (_totalSupply * 50) / 100; // 50% maxSellTransactionAmount
        swapTokensAtAmount = (_totalSupply * 5) / 10000; // 0.05% swap tokens amount || 500000 tokens
        maxWallet = (_totalSupply * 1) / 100; // 1% Max wallet

        marketingBuyFee = 2;
        liquidityBuyFee = 2;
        teamBuyFee = 2;
        totalBuyFees = marketingBuyFee + liquidityBuyFee + teamBuyFee;

        marketingSellFee = marketingBuyFee;
        liquiditySellFee = liquidityBuyFee;
        teamSellFee = teamBuyFee;
        totalSellFees = marketingSellFee + liquiditySellFee + teamSellFee;
    ...
    function updateSellFees(
        ...
        totalSellFees = marketingSellFee + liquiditySellFee + teamSellFee;
        require(totalSellFees <= 30, "Must keep Sell fees at 30% or less");
    }
    ...
    function swapBack() private {
        ...
        ethForTeam = address(this).balance;
