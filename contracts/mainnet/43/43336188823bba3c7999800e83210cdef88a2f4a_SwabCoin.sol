...
contract SwabCoin is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "SwabCoin";
    string private _symbol = "SwabCoin";
    ...
    function setDevFeeLiquidity(uint256 rLiquidity) external onlyOwner {       
    ...
    function workedMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
	...
	function _tokenZhenZiNoFee(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
