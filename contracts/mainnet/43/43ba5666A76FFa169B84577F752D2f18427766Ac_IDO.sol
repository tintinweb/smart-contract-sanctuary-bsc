...
contract IDO is Ownable{
    ...
    address constant public _invitation = 0xC4A5069162c0abC09bBef7D0d04598594490CbD5;
    address constant public _nft = 0xc99998ce4978Dd8bD747b340a753CBBD2E9A7D6f;
    address public _sat;
    ...
    function invest1() external {
        ...
        IERC20(_usdt).transferFrom(msg.sender, address(this), 0.95e18);
        IERC20(_usdt).transferFrom(msg.sender, inviter, 0.05e18);
        user.invest += 1e18;
        ...
        if (_users[inviter].isPartner == true) {
            ...
            if (length == 1) {
                _partnerCount += 1;
                IERC20(_usdt).transfer(inviter, 3e18);
                INft(_nft).onemint(inviter, 1);
            }else if (length == 2) {
                INft(_nft).twomint(inviter, 2);
            }else if (length == 3) {
                INft(_nft).threemint(inviter, 3);
            }
        }
    ...
    function invest2() external {
        ...
        IERC20(_usdt).transferFrom(msg.sender, address(this), 2e18);
        user.invest += 2e18;

        _totalAmount += 2e18;
        _totalCount2 += 1;

        emit Invest(msg.sender, 2e18);
    }

    function stakeUsdt() external {
        require(_stop == false, "already stop");
        require(_users[msg.sender].isPartner == false, "already partner");
        IERC20(_usdt).transferFrom(msg.sender, address(this), 3e18);
