...
contract RabbitCoin is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('RabbitCoin', 'RBC') {
        require(
            feeAccount != address(0),
            'RabbitCoin: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'RabbitCoin: incorrect fee percent');
        require(owner != address(0), 'RabbitCoin: incorrect owner address');
        require(mintToAddr != address(0), 'RabbitCoin: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'RabbitCoin: transfer from the zero address');
        require(recipient != address(0), 'RabbitCoin: transfer to the zero address');
