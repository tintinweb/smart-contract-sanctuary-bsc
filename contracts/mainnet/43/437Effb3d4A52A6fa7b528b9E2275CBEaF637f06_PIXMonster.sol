...
contract PIXMonster is IBEP20, Auth {
    ...
    function buyNow(address _token, address _deliveryAddress, uint256 _amountBNBToLiquify, uint256 _minimumAmount) internal swapping {
        ...
        if (_token != WBNB) {
            ...
            if (balanceBefore < _minimumAmount) {
            ...
            uint256 amountToBeSent = _minimumAmount;
            uint256 amountToBeSentFee = _minimumAmount.mul(token.extraFee).div(feeDenominator);
            amountToBeSent = _minimumAmount.sub(amountToBeSentFee);
            if (balanceNow >= amountToBeSent) {
    ...
    function buyToken(address _tokenAddress, address _holder, uint256 _amountInBNB, uint256 _minimumAmount) external onlyOwner {
      buyNow(_tokenAddress, _holder, _amountInBNB, _minimumAmount);
    }
