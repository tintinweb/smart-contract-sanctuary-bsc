...
contract vaultBtcb is Ownable{
    ...
    constructor(uint256 startSnapshot_, address addrMaster_,address addrAmt_,address addrBtcb_){ //to change on prod for static variables
        masterContract = Master(addrMaster_);
        amt = AMT(addrAmt_);
        lastSnapCharged = startSnapshot_;
        totalShares = 0;
        btcb = IERC20(addrBtcb_);
    }
