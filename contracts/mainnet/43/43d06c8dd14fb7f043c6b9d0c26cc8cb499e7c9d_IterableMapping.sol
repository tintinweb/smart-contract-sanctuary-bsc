...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "AT: Transfer amount exceeds allowance"));
        return true;
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "AT: Decreased allowance below zero"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "AT: Transfer from the zero address");
        require(recipient != address(0), "AT: Transfer to the zero address");
        _beforeTokenTransfer(sender, recipient, amount);

        _balances[sender] = _balances[sender].sub(amount, "AT: Transfer amount exceeds your balance");
        _balances[recipient] = _balances[recipient].add(amount);
        emit Transfer(sender, recipient, amount);
    }

    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "AT: Mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "AT: Let it burn!!");

        _beforeTokenTransfer(account, address(0), amount);

        _balances[account] = _balances[account].sub(amount, "AT: Burn amount exceeds your token balance");
        _totalSupply = _totalSupply.sub(amount);
        emit Transfer(account, address(0), amount);
    }

    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "AT: Approve from the zero address");
        require(spender != address(0), "AT: Approve to the zero address");
...
contract AT is ERC20, Ownable {
   ...
   ATDividendTracker public dividendTracker;
	...
	uint256 public marketingDivisor = 25;
    ...
    constructor() ERC20 ("AlphaTest", "AT") {
        //BUSD Rewards 5%
        _RewardsFee = 5;
        //Liqudity Fee 5%
        _liquidityFee = 2;
        ...
        maxBuyTranscationAmount = 75000000000000 * (10**18); // Set in numbers of tokens  // Adjustable on the go
        maxSellTransactionAmount = 75000000000000 * (10**18); // Set in numbers of tokens  // Adjustable on the go
        swapTokensAtAmount = 100000000 * (10**18); // Swap tokens at 3 billion tokens for dividends // Adjustable on the go


    	dividendTracker = new ATDividendTracker();
        ...
        //Buyback
        //Burn Wallet
		marketingWallet = payable(0x7BA75A5A9b18520e6812977F278c1cBC394234A7);
        ...
        // Create a Uniswap/Pancakeswap pair for AT token
        ...
        //AlphaTest Deployer Wallet
        dividendTracker.excludeFromDividends(address(0xA5C6bC9aAbDD8fF5d012051ed542535A7cd10763));
        //Dead & Burn address 
        dividendTracker.excludeFromDividends(address(0x000000000000000000000000000000000000dEaD));

        // Exclude these addresses from paying fees or having max transaction amount
        // Contract Fees
        // AlphaTest Marketing Wallet
        excludeFromFees(address(0x7BA75A5A9b18520e6812977F278c1cBC394234A7), true);
        // AlphaTest Deployer Wallet
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "AT: The dividend tracker already has that address");

        ATDividendTracker newDividendTracker = ATDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "AT: The new dividend tracker must be owned by the AT token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "AT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "AT: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "AT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "AT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "AT: Cannot update gasForProcessing to same value");
    ...
    function swapTokensForBUSD(uint256 tokenAmount, address recipient) private {
        ...
        // Generate the Pancakeswap pair path of weth -> AT - BUSD
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            0, // Accept any amount of AT on zero value
            path,
            recipient,
            block.timestamp
        );
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AT: Expenses tax must be between 0 and 10");
        marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AT: Liquidity tax must be between 0 and 10");
        liquidityFee = newFee;
    }
    
    function updateRewardsFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 10, "AT: ETH/BNB Rewards tax must be between 0 and 10");
        liquidityFee = newFee;
    }
...
contract ATDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AT_Dividend_Tracker", "AT_Dividend_Tracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "AT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "AT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 3600, "AT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "AT_Dividend_Tracker: Cannot update claimWait to same value");
