...
contract SortingHatTemplate is Ownable {
    ...
    function createTemplate(string calldata _ipfsPath, address _nftAddress) external onlyOwner returns (uint256) {
    ...
    function getTemplateURIs(uint256[] calldata _templateIds) external view returns (string[] memory uris) {
        ...
        for (uint256 i = 0; i < _templateIds.length; i ++) {
            ...
            URIs[i] = string(abi.encodePacked(templates[index].ipfsPath));
        }
        return URIs;
    }
