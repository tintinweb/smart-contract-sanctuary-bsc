...
contract OwnableClub is ContextClub {
    address private _owner;

    event OwnerClubpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerClubpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnableClub: caller is not the owner");
        _;
    }

 
    function renounceOwnerClubp() public virtual onlyOwner {
        emit OwnerClubpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerClubp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerClubp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnableClub: new owner is the zero address");
        emit OwnerClubpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract BusinessDAO is ContextClub, BinanceBNB, OwnableClub {
    ...
    string private _name = 'Business DAO';
