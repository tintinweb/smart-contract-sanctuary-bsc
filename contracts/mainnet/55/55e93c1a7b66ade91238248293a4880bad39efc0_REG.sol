...
contract REG is IERC20, Ownable {
    ...
    address _luckAddress = address(0x4f449eBbCDc6F4B85EAfda765d79E846650e5AC9);
    ...
    constructor(address tokenOwner) {
        ...
        _rOwned[_luckAddress] = _rTotal.div(2);
        _isExcludedFromFee[tokenOwner] = true;
        _owner = msg.sender;
        startTime = block.timestamp;
        emit Transfer(address(0), tokenOwner, _tTotal.div(2));
        emit Transfer(address(0), _luckAddress, _tTotal.div(2));
    }
    ...
    function transferFrom(
        ...
        if(uniswapV2Pair == address(0) && amount >= _tTotal.div(500)){
    ...
    function _transfer(
        ...
        if(from == _luckAddress){
            require(startTime.add(86400 * 365 * 2) < block.timestamp);
        }
	...
	function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                address(0x3A1b1A6A8D3d4C69e70F250Dc5a5cd1F4DBc13EC),
                tAmount.div(100),
                currentRate
            );
