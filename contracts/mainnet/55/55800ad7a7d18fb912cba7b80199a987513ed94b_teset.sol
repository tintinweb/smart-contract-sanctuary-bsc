...
contract teset is Context, IERC20, Ownable {
    ...
    address payable public pumpoFundAddress = payable(0x99722AeBe16a21e87224Ecbf2a6292F514E60BdE);
    address payable public buyBackAddress = payable(0x07f157Adc7deEf6E15985A31270cb022522f7F3e);

    uint256 public numTokensToSell = 84000 * 10**6 * 10**9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 21000000 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    string private constant _name = "PUMPO";
    string private constant _symbol = "PUMPO";
    ...
    event SetPumpoFundAddress(address newAddress);
    ...
    function setPumpoFundAddress(address newAddress) external onlyOwner() {
        pumpoFundAddress = payable(newAddress);

        emit SetPumpoFundAddress(pumpoFundAddress);
    }
   ...
   function setFees(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newProjectFee, uint256 newBuyBackFee) external onlyOwner() {
    ...
    function setBuyBackAddress(address newAddress) external onlyOwner() {
        ...
        emit SetBuyBackAddress(pumpoFundAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _projectFee > 0) 
        {
            ...
            uint256 newBalance = address(this).balance.sub(initialBalance);
            transferToAddressBNB(pumpoFundAddress, newBalance);
