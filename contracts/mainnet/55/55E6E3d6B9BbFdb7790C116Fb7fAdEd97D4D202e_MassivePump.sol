...
Tax for first 1 hour after launch: 0% Buy, 18% Sell ( to fill Liquidity and Marketing Wallet )
Tax normally: 0% Buy, 8% Sell ( 2% LP | 6% Marketing )


Max Tx: 1%
Max Bag: 2%

tg: https://t.me/MassivePumpOfficial
*/
...
contract MassivePump is IERC20, Ownable {
    ...
    constructor () {
        ...
        teamFeeReceiver = 0x246FEAa85aD3b9506fd401be22482c124CbbaFe6;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (taxFeeVar, _marketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 1000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(taxFeeVar).div(_totalFee).div(2);
        ...
        uint256 totalBNBFee = _totalFee.sub(taxFeeVar.div(2));
    ...
    function setFees(uint256 _taxFeeVar, uint256 _reflectionFee, uint256 __marketingFee, uint256 _giveawayFee) external onlyOwner {
        taxFeeVar = _taxFeeVar;
        reflectionFee = _reflectionFee;
        _marketingFee = __marketingFee;
        giveawayFee = _giveawayFee;
        totalFee = taxFeeVar.add(reflectionFee).add(_marketingFee).add(giveawayFee);
