...
contract PTC is ERC20Burnable, OperationWithSigner {
    ...
    uint256 private _burnPercent = 87;
        ...
        ERC20("Perfect Curve", "PTC")
    {
        ...
        _mint(address(this), _STAKE_INCOME + _CARD_REWARD);
        _feeWhitelist[address(this)]=true;
        _feeWhitelist[_foundationAddress]=true;
    }
    ...
    function setFeeWhitelists(address[] memory addresses, bool value) external onlyOperators {
    ...
    function _transfer(
        ...
        if (
            ...
            _feeWhitelist[from]==false 
            && from!=pancakePairAddress){
            //disallow emptying wallet except from pancakePair or this contract
            if (amount<MIN_BALANCE) return;
            amount = amount - MIN_BALANCE;
        }
        if (
            !_feeEnabled ||  from== pancakePairAddress ||_feeWhitelist[from] || _feeWhitelist[to]
        ) {
            //without fee
