...
contract BTDog is Context, IERC20, IERC20Metadata,Ownable {
    ...
    function _dividendFee() private{
        ...
        if(reserve0 >0 && reserve1 >0){
            ...
            IFeeDistributor(nftFeeDistributerAddress).distribute();
        }
    ...
    function _caculateFee(address sender,address to,uint256 tAmount) private view returns(uint ){
        ...
        if(_feeWhiteList[sender] || _feeWhiteList[to]){
            ...
            if(_swapPairList[to]){
                ...
                if(priceNow < _protectionP * 75/100){
                    fee = tAmount * 30 /100;
                }else if(priceNow < _protectionP * 80/100){
                    fee = tAmount * 25 /100;
                }else if(priceNow < _protectionP * 85/100){
                    fee = tAmount * 20 /100;
                }else if(priceNow < _protectionP * 90/100){
                    fee = tAmount * 15 /100;
                }
            }
