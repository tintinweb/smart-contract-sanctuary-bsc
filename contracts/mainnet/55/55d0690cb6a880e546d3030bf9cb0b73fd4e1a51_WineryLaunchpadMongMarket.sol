...
// File contracts/WineryLaunchpad.sol
...
interface IWineryMongMarketNFT {
...
contract WineryLaunchpadMongMarket is
  Initializable,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  IWineryMongMarketNFT public wineryMongMarketNFT;
  ...
  function initialize(IWineryMongMarketNFT _wineryMongMarketNFT, address _treasuryAddress)
    public
    initializer
  {
    ...
    wineryMongMarketNFT = _wineryMongMarketNFT;
  ...
  function unFreezeNFT(uint256 nftId) external nonReentrant whenNotPaused {
    ...
    wineryMongMarketNFT.tokenUnfreeze(nftId);
  }
    ...
    returns (uint256)
  {
    ...
    uint256 nftId = wineryMongMarketNFT.launchpadMint(
      msg.sender,
      _launch.level,
      _launch.robiBoost,
      true
  ...
  function checkLimits(address user, uint256 launchIndex) internal view returns (bool) {
    ...
    return boughtCount[user] < maxToUser && launch.soldCount + launch.sendCount < launch.totalCount;
  }
