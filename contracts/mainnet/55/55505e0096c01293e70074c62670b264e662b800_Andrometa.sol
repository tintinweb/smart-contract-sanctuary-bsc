...
contract Andrometa is IERC20, Ownable {
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (liq_fee, _marketingFee, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function changeFees(uint256 _liq_fee, uint256 _reflectionFee, uint256 __marketingFee, uint256 _giveawayFee) external onlyOwner {
        liq_fee = _liq_fee;
        reflectionFee = _reflectionFee;
        _marketingFee = __marketingFee;
        giveawayFee = _giveawayFee;
        totalFee = liq_fee.add(reflectionFee).add(_marketingFee).add(giveawayFee);
