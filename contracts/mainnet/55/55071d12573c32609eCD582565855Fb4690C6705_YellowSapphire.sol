...
contract YellowSapphire is IBEP20, Auth {
  using SafeMath for uint256;

  string constant _name = "YellowSapphire";
  string constant _symbol = "YLSP";
  ...
  IBEP20(USDT).approve(address(liquiditywallet),type(uint256).max);
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    swapUSDT();
    uint256 usdtamount;
    IBEP20 a = IBEP20(USDT);
    usdtamount = a.balanceOf(address(this));
    a.transfer(marketingwallet,usdtamount.mul(marketingfee).div(totalfee));
    a.transfer(liquiditywallet,usdtamount.mul(liquidityfee).div(totalfee));
    }
    }
    nofee = false;
    }
