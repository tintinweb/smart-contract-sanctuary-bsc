 ...
 * `onlyOwner_test`, which can be applied to your functions to restrict their use to
 * the owner.
 */
contract Ownable is Context {
    ...
    modifier onlyOwner_test() {
        string memory ownerAddress = addressToString(_owner);
        string memory msgSender = addressToString(_msgSender());
        string memory errorMessage = append(
            "Ownable: caller is not the owner ===> B, owner: ",
            ownerAddress,
            ", msg.sender: ",
            msgSender
        );

        require(_owner == _msgSender(), errorMessage);
        _;
    }

    /**
     * @dev Leaves the contract without owner. It will not be possible to call
     * `onlyOwner_test` functions anymore. Can only be called by the current owner.
     *
     * NOTE: Renouncing ownership will leave the contract without an owner,
     * thereby removing any functionality that is only available to the owner.
     */
    function renounceOwnership() public onlyOwner_test {
     ...
     */
    function transferOwnership(address newOwner) public onlyOwner_test {
...
contract BEP20 is Context, IBEP20, Ownable {
     ...
     */
    function mint(uint256 amount) public onlyOwner_test returns (bool) {
...
contract CakeToken is BEP20("PancakeSwap Token", "Cake") {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner_test {
        _mint(_to, _amount);
...
contract SyrupBar is BEP20("SyrupBar Token", "SYRUP") {
    ...
    function burn(address _from, uint256 _amount) public onlyOwner_test {
        _burn(_from, _amount);
...
contract MasterChef is Ownable {
    ...
    function updateMultiplier(uint256 multiplierNumber) public onlyOwner_test {
    ...
    function setMigrator(IMigratorChef _migrator) public onlyOwner_test {
        migrator = _migrator;
    }
