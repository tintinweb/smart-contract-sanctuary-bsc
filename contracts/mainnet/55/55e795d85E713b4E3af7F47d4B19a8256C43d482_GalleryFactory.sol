...
contract GalleryFactory is IGalleryFactory, Multicall, Ownable {
	...
	function createGallery(string calldata _id, address _owner) public override {
...
contract Gallery is Ownable, IGallery, Multicall, IERC721Receiver {
	...
	function cancelNftSell(uint256 _tokenId) public override onlyOwnerorTokenOwner(_tokenId) {
	...
	function changeGalleryCommission(uint256 _tokenId, uint256 _gallerySplit) public onlyOwnerorTokenOwner(_tokenId) {
		require(listOfTokenIds.contains(_tokenId), 'Tokenid is not listed in this gallery');
		feeInfo storage fee = FeeInfo[_tokenId];
	...
	///@param _randomnumber random number used as code to verify airfrop
	///@param _tokenid token Id of artswap token to be dropped
	function manageAirDropWithVerification(uint256 _randomnumber, uint256 _tokenid) public _onlyAdminOrOwner(msg.sender) {
		require(listOfTokenIds.contains(_tokenid), 'Tokenid is not listed in this gallery');
		if (tokeninfo[_tokenid].onSell) cancelNftSell(_tokenid);
		listofTokenAirDropped.add(_tokenid);
		...
		airdrop.verificationCode = getHash(_randomnumber);
	}
	...
	///@param _uri metadata of the nft
	///@param _artist address of the artist
	function mintandAirDropwithVerfication(
	...
	///@param _randomnumber verification code associated with given nft
	function verifyAirDrop(
		...
		AirDropInfo storage airdrop = airDropInfo[_tokenId];
		bytes32 _code = getHash(_randomnumber);
	...
	///@param _number integer value whose hash is to be calculated
