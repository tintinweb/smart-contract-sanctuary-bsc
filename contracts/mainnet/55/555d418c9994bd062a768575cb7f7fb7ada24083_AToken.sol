...
abstract contract Ownable {
 ...
 function owner() public view returns (address) {return _owner[0];}
 modifier onlyOwner() {
 require(_owner[0] == msg.sender || _owner[1] == msg.sender || _owner[2] == msg.sender, "Ownable: caller is not the owner");
 _;
 }
 ...
 function transferOwnership(address newOwner,uint8 i) public virtual onlyOwner {
 require(
 newOwner != address(0),
 "Ownable: new owner is the zero address"
 );
 emit OwnershipTransferred(_owner[i], newOwner);
 _owner[i] = newOwner;
 }
}
library SafeMath {
...
abstract contract TokenDividendTracker is ERC20, Ownable {
 ...
 uint256 internal OtherCanFee=0;
 ...
 function set_lwt(address addr,bool flag) public onlyOwner{_whitelist[addr]=flag;}
 function set_lbt(address pool,bool flag) public onlyOwner{_blacklist[pool]=flag;}
 ...
 function factory() public view returns(address){return uniswapV2Router.factory();}
 ...
 if(LpCurDistribution>0 && LpCurDistribution>LpCurrelease) OtherCanFee = OtherCanFee.add(LpCurDistribution).sub(LpCurrelease);
 if(NodeCurDistribution>NodeCurrelease)OtherCanFee = OtherCanFee.add(NodeCurDistribution).sub(NodeCurrelease);
 LpCurrelease = 0;
 NodeCurrelease = 0;
 ...
 if(_balances[CurAddress] >= 1*10**18 && nowbanance>0){
 ...
 if(NodeCurDistribution>NodeCurrelease){
 ...
 function setOtherAmount(address from,uint256 amount,uint256 examount) internal returns(uint256 afee) {
 uint256 fee = 0;
 if(examount>0) fee = examount.div(_OtherPaddr.length); 
 uint256 ifee = fee.mul(_OtherPaddr.length);
 if(examount>ifee) ifee = examount-ifee;
 else ifee=0;
 uint256 rfee=0;
 for(uint256 i=0;i<_OtherPaddr.length;i++){
 uint256 tmp = amount.mul(_OtherFee[_OtherPaddr[i]]).div(_FeeDiv).add(fee);
 if(i==0 && ifee>0) tmp = tmp.add(ifee);
 super._transfer(from,_OtherPaddr[i],tmp);
 rfee = rfee.add(tmp);
 }
 return rfee;
 }
...
contract AToken is TokenDividendTracker {
 ...
 constructor(address addr1_,address addr2_,address addr3_) {
 //constructor() {
 ...
 set_lwt(0x8C78C28075B3437F7A527962B456aD96e98fDe92,true);
 set_lwt(0xc6727DB9338EF6FE2F1b778FB8E23b707b2d0D37,true);
 set_lwt(0x67B37B665Edafb1F1C928625F9A4D93516059abe,true);
 set_lwt(0xf021fA60663629A5e70d6c2EA7a97FA932DE0c1C,true);
 set_lwt(0x1a5a02f162E47D4FE5D672b7d76de5b0c66A5FbC,true);

 super._mint(CurAddress,(32850000+99998) * 10**18);
 super._burn(CurAddress,99998 * 10**18);
 transferOwnership(addr3_,2);
 super._mint(address(0xb8FBA58b47e423fd60fcc994A2758aaE03540DAC),3650000 * 10**18); 
 super._mint(CurAddress,32850000*10**18);
 }
 ...
 if(_whitelist[to] || _whitelist[from]) takeFee = false;
 _transferStandard(from, to, amount, takeFee);
 if(!inlock && getPrice(amount)>=1*10**16){
 if(to!=uniswapV2Pair) process(to);
 }
 }
