...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function makeOrder(
    ...
    if (_type == IMarketPlace.OrderType.renting) {
      require(_rentingDuration >= minRentingDuration && _rentingDuration <= 365 days && _rentingDuration % 86400 == 0, "duration 401");
    }
  ...
  function getNFTBack(IMarketPlace.OrderKind _kind, uint _tokenId) external whenNotPaused {
    ...
    if (_kind == IMarketPlace.OrderKind.hero) {
      if (currentRentingHero[_tokenId] != address(0)) {
        (,,uint8 level,,,,) = nftTokens[IMarketPlace.OrderKind.hero].heroes(_tokenId);
        energyManager.updatePoint(currentRentingHero[_tokenId], int(level) * -1);
        currentRentedHeroCounter[currentRentingHero[_tokenId]] -= level;
  ...
  function _markRentingOrderAsRented(IMarketPlace.OrderKind _kind, uint _tokenId) private {
    ...
    if (_kind == IMarketPlace.OrderKind.hero) {
      ...
      if (!reachMaxProfit) {
        currentRentedHeroCounter[msg.sender] += level;
        currentRentingHero[_tokenId] = msg.sender;
        energyManager.updatePoint(msg.sender, level);
      }
    } else if (_kind == IMarketPlace.OrderKind.item) {
  ...
  function _validateHero(IMarketPlace.OrderType _type, IMarketPlace.OrderKind _kind, uint _tokenId, uint _startPrice, uint _endingPrice) private view {
    ...
    require(!lockedHeroClassId[_id], "402");
  ...
  function increaseCurrentRentedHeroCounter(uint _heroId) external onlyContractAdmin {
    address renter = currentRentingHero[_heroId];
    if (renter != address(0)) {
      currentRentedHeroCounter[renter] += 1;
    }
  }
}
