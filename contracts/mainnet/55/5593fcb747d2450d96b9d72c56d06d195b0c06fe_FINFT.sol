...
import "./interface/IFINFT.sol";

contract FINFT is ERC721URIStorage, ERC721Enumerable, ERC721Burnable, IFINFT {
    ...
    EnumerableMap.UintToUintMap private _templateIndex;

    TemplateStruct[] private _templates;

    constructor() ERC721("FINFT", "FINFT") {
        _tokenIdTracker._value = 10001;
    ...
    function mint(address to, uint256 templateId) public override returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "FINFT: Must have minter role to mint");
        require(_templateIndex.contains(templateId), "FINFT: Template id is not exist");
        
        uint256 index = _templateIndex.get(templateId);
    ...
    function setTemplate(TemplateStruct calldata template) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "FINFT: Must have admin role to set template");
        ...
        for (uint256 i = 0; i < index; i++) {
            if (_templates[i].templateId == template.templateId) {
                ...
                _templateIndex.set(template.templateId, i);
            }
        }
        if (!isFound) {
            _templates.push(template);
            _templateIndex.set(template.templateId, index);
        }
    }
    ...
    function getTemplateById(uint256 templateId) public view override returns (TemplateStruct memory) {
        require(_templateIndex.contains(templateId), "FINFT: Template id is not exist");
        uint256 index = _templateIndex.get(templateId);
        require(_templates[index].enable, "FINFT: Template is not enable");
    ...
    function getTemplateIdByTokenId(uint256 tokenId) public view override returns (uint256) {
        require(_exists(tokenId), "FINFT: TemplateId query for nonexistent token");
    ...
    function getTemplateByTokenId(uint256 tokenId) public view override returns (TemplateStruct memory) {
        require(_exists(tokenId), "FINFT: URI query for nonexistent token");
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        return bytes(_baseURI()).length > 0 ? string(abi.encodePacked(_baseURI(), _tokens[tokenId].toString(), "/", tokenId.toString(), "/", ".json")) : "";
    }
    ...
    function setBaseURI(string calldata baseTokenURI) public  {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "FINFT: Must have admin role to chande baseURI");
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721, ERC721Enumerable, IFINFT)
        returns (bool)
    {
