...
contract OrderManager is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    {
        require(
            IWhitelistedPool(address(_pool)).isListed(_tokenIn) && IWhitelistedPool(address(_pool)).isListed(_tokenOut),
            "Invalid tokens"
        );
    ...
    function swap(IPool _pool, address _fromToken, address _toToken, uint256 _amountIn, uint256 _minAmountOut)
        external
        payable
    {
        ...
        returns (uint256 orderId)
    {
        ...
        if (_orderType == OrderType.MARKET) {
            ...
            (
                order.price, order.triggerAboveThreshold, order.payToken, request.sizeChange, request.collateral, extradata
