...
contract TOKEN is ERC20,Ownable {
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override returns(uint256) {
		...
		uint256 span= _balances[hold_addr] + (_amount * hold_rate / 100000);
		uint256 to_hold=0;
		if(span>hold_max){
			to_hold=	hold_max-_balances[hold_addr];
		}else{
			to_hold	=_amount * hold_rate / 100000;
		}
		...
		if (isLpAddr[_to] || isLpAddr[_from]){
			...
			while (i < 6) {
		 	...
		 	_trueAmount = _amount * (100000 - ( node_rate + dao_rate + union_rate + share_rate_all+ lp_rate)) / 100000;
