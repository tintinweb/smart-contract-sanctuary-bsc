...
contract DexSell {
  ...
  uint public _totalTokenAirDrop;
  ...
  function totalTokenAirDrop() external view returns (uint256) {
    return _totalTokenAirDrop;
  }
  ...
  function updateEndTime(uint min, uint max) external {
    require(msg.sender == _owner, 'only owner');
    _minBNB = min;
    _maxBNB = max;
  }
