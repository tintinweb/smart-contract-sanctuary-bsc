...
contract ETToken is ERC20("ET", "ET"), Ownable {
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 1000000 * (10**_decimals);

    uint256 public burnFee = 450;
    uint256 public aFee = 10;
    uint256 public bFee = 10;
    uint256 public cFee = 30;
    ...
    address public feeAddressA = 0x540197bAB3Ea874F8EDB918AeD3AA38Ef7166157;
    address public feeAddressB = 0x09727fCA03F0FC2183CAFe0E4d5aE6e7846fD052;
    address public feeAddressC = 0x78203BA1003E3a778D53eAcFdfd6888cEe0104Cd;
    ...
    constructor(address router_, address usdt_) {
        ...
        liquidityWallet = 0x6a7A33f053760803f29581dbEd2428Eb9407cb11;
    ...
    function _takeFees(address sender, address recipient, FeeInfo memory _feeInfo) private {
       ...
       if (_feeInfo.tBurn > 0) {
            uint256 endTotal =  10000 * 10**decimals();
        ...
        if (_feeInfo.tFeeA > 0) {
            _distributionFees(feeAddressA,  _feeInfo.tFeeA);
        }  
        if (_feeInfo.tFeeB > 0) {
            _distributionFees(feeAddressB,  _feeInfo.tFeeB);
        }  
        if (_feeInfo.tFeeC > 0) {
            _distributionFees(feeAddressC,  _feeInfo.tFeeC);
        }  
    }
        ...
        )
    {
        ...
        if (_isBuy ){
            ...
            _feeInfoTmp.tFeeA = tAmount * (aFee) / (1000);
            _feeInfoTmp.tFeeB = tAmount * (bFee) / (1000);
            _feeInfoTmp.tFeeC = tAmount * (cFee) / (1000);
        }
        
        _feeInfoTmp.tAmount = tAmount;
        tTransferAmount = tTransferAmount - (_feeInfoTmp.tBurn) - (_feeInfoTmp.tFeeA) - (_feeInfoTmp.tFeeB) - (_feeInfoTmp.tFeeC);
