...
contract ShellContract is Context, IERC20, Ownable {
    ...
    address public MarketingWallet = 0x8c9956cb73c735331088c5B4ECF3a9D04613cC53;
    address public DevelopmentWallet = 0xb9BdB878127d2185ae3512A17209bC86b2579c24;
    address public BurnWallet = 0x000000000000000000000000000000000000dEaD;

    string private constant _name = "TESTING88";
    string private constant _symbol = "TESTING88";
    ...
    Taxes public buyTaxes = Taxes(0, 100, 50, 50, 20);
    Taxes public sellTaxes = Taxes(0, 100, 50, 50, 20);
    ...
    function Enabletrading(
        ...
        tradingEnabled = true;
        swapEnabled = true;
        require (_deadline <= 10, "Deadline cannot exceed 10 Blocks"); 
        deadline = _deadline;
        launchtax = Taxes(Reflections, Marketing, Liquidity, Development, Burn);
        if (tradingEnabled == true) genesis_block = block.number;
    }
    ...
    //@Development kept original Reflections naming -> "reward" as in reflection
    ...
    function setBuyTaxes(uint256 _Reflections, uint256 _Marketing, uint256 _Liquidity, uint256 _Development, uint256 _Burn) public onlyOwner {
        require(_Reflections+(_Marketing)+(_Liquidity)+(_Development)+(_Burn) <=22, "Buy Taxes cannot be higher than 22%");
        buyTaxes = Taxes(_Reflections,_Marketing,_Liquidity,_Development,_Burn);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _Reflections, uint256 _Marketing, uint256 _Liquidity, uint256 _Development, uint256 _Burn) public onlyOwner {
        require(_Reflections+(_Marketing)+(_Liquidity)+(_Development)+(_Burn) <=22, "Sell Taxes cannot be higher than 22%");
       
       sellTaxes = Taxes(_Reflections,_Marketing,_Liquidity,_Development,_Burn);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rReflections, to_return.rMarketing, to_return.rLiquidity, to_return.rDevelopment, to_return.rBurn) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tReflections-s.tMarketing-s.tLiquidity-s.tDevelopment-s.tBurn;
        return s;
    }

    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rReflections,uint256 rMarketing, uint256 rLiquidity, uint256 rDevelopment, uint256 rBurn){
        ...
        rTransferAmount =  rAmount-rReflections-rMarketing-rLiquidity-rDevelopment-rBurn;
        return (rAmount, rTransferAmount, rReflections,rMarketing,rLiquidity,rDevelopment,rBurn);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 8) {
            require(to != pair, "You are not allowed");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        _reflectReflections(s.rReflections, s.tReflections);
        _takeMarketing(s.rMarketing,s.tMarketing);
        _takeDevelopment(s.rDevelopment,s.tDevelopment);
        _takeLiquidity(s.rLiquidity,s.tLiquidity);
        _takeBurn(s.rBurn, s.tBurn);
        emit Transfer(sender, recipient, s.rAmount);
        emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDevelopment);
        emit Transfer(sender, BurnWallet, s.tBurn);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.Liquidity + temp.Marketing + temp.Development + temp.Burn) * 2;
    ...
    function updateMarketingWallet(address newWallet) external onlyOwner{
        ...
        _isExcluded[MarketingWallet];
        _isExcludedFromFee[MarketingWallet];
    }
    
    function updateDevelopmentWallet(address newWallet) external onlyOwner{
        DevelopmentWallet = newWallet;
        _isExcluded[DevelopmentWallet];
        _isExcludedFromFee[DevelopmentWallet];
    }
    
    function updateBurn(address newWallet) external onlyOwner{
        BurnWallet = newWallet;
        _isExcluded[BurnWallet];
        _isExcludedFromFee[BurnWallet];
    }
