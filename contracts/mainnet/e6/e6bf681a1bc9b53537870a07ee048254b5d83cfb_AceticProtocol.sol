...
//  Acetic Protocol - Deflationary token powered by the Binance Smart Chain platform.
...
contract AceticProtocol is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x834eDEC16a2ed099D7A1c6858A9d60329016dc9e); // Marketing Address
    address payable public charityAddress = payable(0x834eDEC16a2ed099D7A1c6858A9d60329016dc9e); // Charity/Dev Address
    ...
    uint256 private _tTotal = 950000000 * 10**18; // 1 Qud
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Acetic Protocol";
    ...
    uint256 public _maxTxAmount = 950000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    uint256 private minimumTokensBeforeSwap = 950 * 10**18;  //allow maximum sell amount(anti-dump setup)
  ...
  function setALLFeePercent(uint256 charity,uint256 marketting,uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee,uint256 sellLiquidityFee,uint256 sellTaxFee,uint256 buyLiquidityFee,uint256 buyTaxFee) external onlyOwner() {
    ...
    uint256 private salePrice = 1500000;// 0.01*1500000=15,000 token for 0.01 bnb (set as per requirements)
