...
contract AvionToken is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    function _transferFrom(
        ...
        if (usersInfo[recipient].lastTimeSold == 0) usersInfo[recipient].lastTimeSold = block.timestamp - 1 days;
        ...
        if (shouldRebase(sender) && autoRebase) {
            _rebase();
        }
    ...
    function shouldRebase(address from) internal view returns (bool) {
        return nextRebase <= block.timestamp && msg.sender != address(router) && from != address(router);
    }
    ...
    function setSwapBackSettings(
        bool _enabled,
        uint256 _gonSwapThreshold
    ) external onlyOwner {
        swapEnabled = _enabled;
        gonSwapThreshold = _gonSwapThreshold.mul(getYield(address(this)));
        emit SetSwapBackSettings(_enabled, _gonSwapThreshold, _gonSwapThreshold);
    }
    ...
    function takeFee(
        ...
        if (recipient == address(router)) {
            updateAvailableAmount(sender, gonAmount.div(gonsPerFragment));
        }
    ...
    function setRouter(address _router) external onlyOwner {
        ...
        _allowedFragments[address(this)][_router] = type(uint256).max;
        IERC20(busdToken).approve(_router, type(uint256).max);
    }
    ...
    function updateAvailableAmount (address sender, uint256 amount) internal {
        if (block.timestamp - usersInfo[sender].lastTimeSold >= 1 days && block.timestamp - usersInfo[sender].lastTimeSold < 7 days) {
            usersInfo[sender].amountAvailable = (balanceOf(sender) * percentageForLessThanSevenDays) / 1000;
            usersInfo[sender].lastTimeSold = block.timestamp;
        } else if (block.timestamp - usersInfo[sender].lastTimeSold >= 7 days) {
            usersInfo[sender].amountAvailable = (balanceOf(sender) * percentageForMoreThanSevenDays) / 1000;
            usersInfo[sender].lastTimeSold = block.timestamp;
        }

        require(amount <=  usersInfo[sender].amountAvailable, "Avion: you sell more than you can");

        usersInfo[sender].amountAvailable = (usersInfo[sender].amountAvailable).sub(amount);
    }

    function getMaxAvailableAmount (address sender) external view returns(uint256) {
        if (block.timestamp - usersInfo[sender].lastTimeSold < 7 days) {
            return balanceOf(sender) * percentageForLessThanSevenDays / 1000;
        } else {
            return balanceOf(sender) * percentageForMoreThanSevenDays / 1000;
        }
    ...
    function buyWithReferral (address referral, uint256 amount) external {
        IERC20(busdToken).transferFrom(msg.sender, address(this), amount);

        require(referral != msg.sender, "Referral can't be yourself");
        require(balanceOf(referral) > 0, "Referral is not token holder");
        ...
        uint256 amountReceived = balanceOf(msg.sender) - amountBefore;

        uint256 gonAmount = amountReceived.mul(5).div(100).mul(getYield(referral));
        _gonBalances[referral] = _gonBalances[referral].add(gonAmount);

        emit Transfer(address(this), referral, amountReceived.mul(5).div(100));
    }
    ...
    function clearStuckBalanceAvion(address _receiver) external onlyOwner {
        _basicTransfer(address(this), _receiver, balanceOf(address(this)));
        emit ClearStuckBalance(_receiver);
    }
    ...
    function burn(
        ...
        uint256 gonAmount = amount.mul(getYield(from));
