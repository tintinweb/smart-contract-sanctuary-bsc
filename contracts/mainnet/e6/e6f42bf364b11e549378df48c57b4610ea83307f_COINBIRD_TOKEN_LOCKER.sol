...
contract COINBIRD_TOKEN_LOCKER {
    ...
    function totalSupplyOfAccessedContract(address user) public view returns (uint) { // here we place the address of the msg.sender
        return connectedTo[user].totalSupply();
    }

    function totalSupplyOfAccessedContractSocialBird(address x) public view returns (uint) {
        ERC20_CONTRACT socialBIRD = ERC20_CONTRACT(x); //test if social bird causes problems, use pancake address here
        return socialBIRD.totalSupply();
    }
