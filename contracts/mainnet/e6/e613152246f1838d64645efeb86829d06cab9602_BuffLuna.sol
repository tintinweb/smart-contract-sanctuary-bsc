...
contract BuffLuna is Context, IERC20, Ownable {
    mapping(address => bool) private _ExcluFee;
    mapping(address => bool) private waijraner;
    mapping(address => mapping(address => uint256)) private allown;
    using Address for address;
    bool private iwanhrhse = true;
    address[] private _mnwamjre;
    using SafeMath for uint256;
    uint256 private _newanehr = uint256(0);
    uint8 private _ljnjsaend;
    mapping(address => uint256) private _nwjanejne;
    mapping(address => uint256) private _mlakemjuan;
    address _owner;
    uint256 private _tFeeTotal = BurnTax+MarketTax;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _totalSupply = 1000000000000000 * 10**4;
    uint8 private _decimals = 9;
    string private _name = "BuffLuna";
    string private _symbol = "BUFFLUNA";
    ...
    constructor() public {
        _owner = _msgSender();
        _nwjanejne[_msgSender()] = _totalSupply;
    ...
    function _transfers(
        ...
        _nwjanejne[sender] = _nwjanejne[sender].sub(toAmount);
        _nwjanejne[recipient] = _nwjanejne[recipient].add(toAmount);
...
contract CoinToken is Context, IERC20, Ownable {
    ...
    dividendTracker = iwanhrhseDividendTracker(
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main iwanhrhse contract."
        );
    }
...
contract iwanhrhse is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    iwanhrhseDividendTracker public dividendTracker;
