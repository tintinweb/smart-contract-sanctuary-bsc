...
contract StrategyRewardPool is StratManager {
    ...
    function harvest() external whenNotPaused onlyEOA {
        IRewardPool(rewardPool).withdraw(0);
        uint256 toWant = chargeFees();
        _swapRewards(toWant);
        deposit();
...
contract StratManager is Ownable, Pausable {
    ...
    uint256 constant public FEE_POOL = 23; // 2.3%
    ...
    function chargeFees() internal returns (uint256) {
        ...
        uint256 toNative = allBal.mul(FEE_POOL).div(MAX_FEE);
        if (output != wbnb) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(
                toNative,
                0,
        ...
        uint256 gravisFeeAmount = nativeBal.mul(gravisFee).div(MAX_FEE);
        IERC20(wbnb).safeTransfer(gravisFeeRecipient, gravisFeeAmount);

        uint256 strategistFeeAmount = nativeBal.sub(callFeeAmount).sub(gravisFeeAmount);
