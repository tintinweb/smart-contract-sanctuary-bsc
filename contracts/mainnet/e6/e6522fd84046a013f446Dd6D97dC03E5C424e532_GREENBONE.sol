...
contract GREENBONE is ERC20, Ownable {
    ...
    constructor(
        ...
        address owner,
        address mintToAddr
    ) ERC20('GREEN BONE', 'GBONE') {
        require(
            feeAccount != address(0),
            'GREENBONE: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'GREENBONE: incorrect fee percent');
        require(owner != address(0), 'GREENBONE: incorrect owner address');
        require(mintToAddr != address(0), 'GREENBONE: incorrect mintToAddr address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'GREENBONE: transfer from the zero address');
        require(recipient != address(0), 'GREENBONE: transfer to the zero address');
