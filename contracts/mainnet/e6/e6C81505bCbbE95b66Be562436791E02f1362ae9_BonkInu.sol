...
contract BonkInu is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "BONKINU";
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0x84DB01041B87c0caA2e5f781D901566B3165f51a;

        marketingWallet = 0x84DB01041B87c0caA2e5f781D901566B3165f51a;        
        ...
        totalFeeIfSelling = totalFee*sellFeeMult;
    ...
    function blacklistSniper(address holder, bool isASniper) external authorized {
    ...
    function changeFees(uint256 newLiqFeeThou, uint256 newRewardFeeThou, uint256 newmarketingFee) external onlyOwner {
    ...
    function changeSellFeeMult(uint256 newSellFeeMult) external authorized {
        sellFeeMult = newSellFeeMult;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (pair == recipient) {
            feeApplicableThou = totalFeeIfSelling;
        } else {
            feeApplicableThou = totalFee;
        }
	...
	function setAntisniperGas(bool active, uint256 quantity) external authorized {
		require(!active || antiSniperGasLimit >= 7 gwei, "Needs to be at least 7 gwei.");
		gasLimitActive = active;
		antiSniperGasLimit = quantity * 1 gwei;
	}
