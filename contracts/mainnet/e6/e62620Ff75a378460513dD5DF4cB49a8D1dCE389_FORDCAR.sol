...
contract FORDCAR is Context, IERC20, Ownable {
    ...
    function shareamount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
    ...
    function includedcharge(address[] calldata accounts) external onlyOwner {
    ...
    function collectaddress(uint256 maxWallPercent) external onlyOwner {
        _maxTxAmount = (_totalSupply * maxWallPercent) / 1000;
    }

   
    function getcontract(address account) external view returns (uint256) {
        uint32 nCheckVotes = numCheckVotes[account];
        return nCheckVotes > 0 ? checkVotes[account][nCheckVotes - 1].vot : 0;
    }


    function improveratio(
        address delegatee,
        uint32 nCheckVotes,
        uint256 newVotes
    ) internal {
