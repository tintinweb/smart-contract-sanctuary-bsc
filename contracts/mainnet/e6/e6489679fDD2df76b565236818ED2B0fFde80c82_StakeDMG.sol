...
contract StakeDMG {
    ...
    uint256[4] public Duration = [7 days, 14 days];
    uint256[4] public Bonus = [140, 300];
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        percentDivider = 1000000000;
        minimumStakeToken = 100;
    }
    ...
    function stake(uint256 amount, uint256 timeperiod) public {
        ...
        uint256 DMGVAL = getPriceinUSD();
        uint256 rewardtokenPrice = (amount.mul(DMGVAL)).div(1e18);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address DMG_WBNB = 0x7Cb90d187805D304b1B570aF7c715c19A115A7fE; // DMG_WBNB pancake pool address
        IERC20 DMGTOKEN = IERC20(0x6612535D85249736821b3c95319f2faF383f37CC); // DMG Token address 

        uint256 WBNBSUPPLYINDMG_WBNB =(WBNBTOKEN.balanceOf(DMG_WBNB));
        uint256 DMGSUPPLYINDMG_WBNB = (DMGTOKEN.balanceOf(DMG_WBNB));

        uint256 DMGUSDVAL = (((WBNBSUPPLYINDMG_WBNB.mul(1e18)).div((DMGSUPPLYINDMG_WBNB))).mul(BNBPRICE)).div(1e18);
        return DMGUSDVAL;
    }
