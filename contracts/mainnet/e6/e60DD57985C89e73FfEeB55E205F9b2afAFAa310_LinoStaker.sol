...
contract LinoStaker is Ownable {
    ...
    //Setting
    uint256 public PercentOfReward = 3; //7 %
    uint256 public PeriodTimeBySecond=60*60*24*30;//one Month is 60*60*24*30
    ...
    function AddRewardByChangeSetting(uint256 _amount,uint256 _percent,uint256 _days) onlyOwner public{
        ...
        require(rewardToken.transferFrom(msg.sender,address(this),_amount),"Staker: transfer faild");
    ...
    public {
        ...
        ///////////////////
        uint256 reward = CalcaluateReward(msg.sender);
        uint256 PlusAdded = _amount;
        if(reward>0) {
            PoolReward = PoolReward.sub(reward);
            PlusAdded = PlusAdded.add(reward);
            user.reward = 0;
        }
        user.deposited = user.deposited.add(PlusAdded);
        totalStaked = totalStaked.add(PlusAdded);
    ...
    public {
        ...
        user.lastTimeStaked = block.timestamp;
        require(depositToken.transfer(msg.sender,_amount),"Staker: Error Withdraw");
    ...
    function claimReward() public{
        ...
        require(rewardToken.transfer(msg.sender,user.reward),"Staker: Error Withdraw");
        PoolReward = PoolReward.sub(Rewarded);
        user.reward = 0;
        user.lastTimeStaked = block.timestamp;
