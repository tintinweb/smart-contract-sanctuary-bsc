...
contract BPoolsReferral is AccessControl {
  ERC20 public immutable _Token = ERC20(0x57a0c4c868EaB8b5C605562Cc70597E94108195C); // Token to withdraw

  bytes32 public constant ADMIN = keccak256("ADMIN");
  bytes32 public constant SIGNER = keccak256("SIGNER");

  address public _treasuryAddress;
  uint public _tokenPerPointAmount = 10;
  uint public _minSwapPointAmount = 50;
  uint public _maxSwapPointAmount = 100;
  ...
  // Constructor
  constructor(uint tokenPerPointAmount, uint minSwapPointAmount, uint maxSwapPointAmount, address signer) {
    _treasuryAddress = msg.sender;
    _tokenPerPointAmount = tokenPerPointAmount; // 1 Point = ? Token
    _minSwapPointAmount = minSwapPointAmount; // Min point to withdraw
    _maxSwapPointAmount = maxSwapPointAmount; // Max point to withdraw
  ...
  function setTokenPerPointAmount(
    uint tokenPerPointAmount
  ) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    _tokenPerPointAmount = tokenPerPointAmount;
    return true;
  }
  ...
  function setMinSwapPointAmount(
    ...
    function setMaxSwapPointAmount(
    uint maxSwapPointAmount
  ) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    _maxSwapPointAmount = maxSwapPointAmount;
    return true;
  }
  ...
  function ClaimReferral(
    ...
    require(
      ...
      "Point amount have to greater than or equal min swap point"
        ...
        require(
      pointAmount <= _maxSwapPointAmount * 1e18,
      "Point amount have to less than or equal max swap point"
    );

    uint tokenAmount = pointAmount * _tokenPerPointAmount;

    _Token.transferFrom(_treasuryAddress, msg.sender, tokenAmount);
