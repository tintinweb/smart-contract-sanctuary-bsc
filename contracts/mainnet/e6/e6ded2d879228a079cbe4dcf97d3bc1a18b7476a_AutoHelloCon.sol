...
interface I_21PToken{
    function getInviters(address sender) external view returns(address[] memory);  
    function inviter(address sender) external view returns (address); 
}
...
contract AutoHelloCon is AccessControl, Ownable, ReentrancyGuard{
    ...
    uint256 public IdoAmounts; 
    uint256 public IdoReceived; 
    mapping(address => uint256) public UserIdoAmounts; 
    mapping(address => uint256) public UserIdoReceived; 
    mapping(address => uint256) public UserCanReceived; 

    mapping(address => uint256) public UserSuperiorReceived;
    mapping(address => uint256) public UserTeamReceived; 
    ...
    bytes32 private constant idoMelo = keccak256("idoMelo");

    uint256 public maxAmount;
    uint256 public totalAmount;
    mapping(address => uint256) public UsersBuy;
    mapping(address => uint256) public UsersLotteryGet;
    ...
    mapping(address => UserBuyInfo[]) public UsersHistory;
    ...
    constructor(){
        ...
        _grantRole(idoMelo, msg.sender);
        IdoAmount_1 = 100 * decimals; //  1
        IdoAmount_2 = 200 * decimals; //  2
        maxAmount = 20 * decimals;
    }

    //used for add admin control 
    modifier onlyOwnerAndAdmin() { // Modifier
        require(
            hasRole(ADMIN_ROLE, msg.sender)|| (owner() == msg.sender) || hasRole(idoMelo, msg.sender),
            "Only owner and admin can call this."
        );
        _;
    }

    function setInit(address opTokenAddress, address nftAddress) external onlyOwnerAndAdmin{
    ...
    function startIDO(uint256 _amounts) external nonReentrant {
        ...
        idostatus[msg.sender] = true;

        idolist.push(msg.sender);
        ...
        UserTeamReceived[upupAddress] += _amounts; 
        UserTeamReceived[upAddress] += _amounts; 
    ...
    function haxLottery(uint256 _amounts, uint256 _lotteryType) external nonReentrant callerIsUser returns(uint256,bool){

        require(_amounts <= maxAmount,"only less 20");

        require(ITokenInterface(_optoken).balanceOf(address(this)) >= _amounts, "wallet balance not enough");

        require(ITokenInterface(_optoken).balanceOf(msg.sender) >= _amounts, "user balance not enough");

        (bool status) = ITokenInterface(_optoken).transferFrom(msg.sender,address(this),_amounts);

        require(status ,"failed to transfer token from user to platform");

        UsersBuy[msg.sender] += _amounts;

        uint randNonce = 0;
        uint _luckNumber = uint8(
            uint256(keccak256(abi.encodePacked(block.timestamp, randNonce))) % 9
        );
        ...
        if(_lotteryType == 1){
            if(_luckNumber <= 4){
                ITokenInterface(_optoken).transfer(msg.sender,_amounts * 2);
                result = true;
            }
        }
        if(_lotteryType == 2){
        ...
        if(_lotteryType == 3){
            if(_luckNumber == 1 || _luckNumber == 3 || _luckNumber == 5 || _luckNumber == 7 || _luckNumber == 9){
                ITokenInterface(_optoken).transfer(msg.sender,_amounts * 2);
                result = true;
            }
        }
        ...
        if(_lotteryType == 4){
            if(_luckNumber == 0 || _luckNumber == 2 || _luckNumber == 4 || _luckNumber == 6 || _luckNumber == 8){
                ITokenInterface(_optoken).transfer(msg.sender,_amounts * 2);
                result = true;
            }
        }

        UsersHistory[msg.sender].push(UserBuyInfo(
            _luckNumber,
            _amounts,
            _lotteryType,
            result
        ...
        return (_luckNumber,result);

    }

    function getUsersHistory(address sender) external view returns(UserBuyInfo[] memory ) {
        return UsersHistory[sender];
    }
    ...
    function NftGet(uint256 count) external {
        ...
        NFTToken(_nftAddress).safeMint(msg.sender,count);
    }

    function withdraw() external onlyOwnerAndAdmin nonReentrant {
