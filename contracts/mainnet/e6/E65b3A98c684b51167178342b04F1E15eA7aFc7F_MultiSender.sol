...
contract MultiSender {
    ...
    function sendERC20(IERC20 token, address[] calldata addresses, uint256 [] calldata values) external returns (uint256) {
      uint256 i = 0;
    ...
    function sendERC1155(IERC1155 token, address[] calldata addresses, uint256 [] calldata values, uint256 [] calldata ids) external returns (uint256) {
      uint256 i = 0;

      while (i < addresses.length) {

        token.safeTransferFrom(msg.sender, addresses[i],  values[i], ids[i], "");
        i += 1;
      }
