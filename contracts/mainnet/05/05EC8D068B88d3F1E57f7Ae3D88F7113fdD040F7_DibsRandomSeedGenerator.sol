    ...
    AccessControl
{
    ...
    mapping(uint256 => SeedRequestStatus) public seedRequests; // roundId => SeedRequestStatus
    ...
    constructor(
        ...
        address setter_,
        address dibs_,
        address VRFConsumerBaseV2_,
        uint64 subscriptionId_,
        bytes32 keyHash_
    ) VRFConsumerBaseV2(VRFConsumerBaseV2_) ConfirmedOwner(admin_) {
        dibs = dibs_;
    ...
    function requestRandomSeed(uint32 roundId)
        external
        returns (uint256 requestId)
    {
        // round must be over
        if (IDibs(dibs).getActiveLotteryRound() <= roundId) {
            revert LotteryRoundNotOver();
        }

        // if request for current roundId already exists, revert
        if (seedRequests[roundId].exists) {
            revert SeedExists();
        }
        ...
        seedRequests[roundId] = SeedRequestStatus({
            fulfilled: false,
            exists: true,
            roundId: roundId,
            requestId: requestId,
    ...
    function getRequestStatus(uint256 requestId_)
        public
        view
        returns (bool fulfilled, uint256 seed)
    {
        if (!seedRequests[requestId_].exists) revert RequestIdNotExists();
        SeedRequestStatus memory request = seedRequests[requestId_];
        return (request.fulfilled, request.seed);
    }

    /// @notice query the status of a seed request
    /// @param roundId_ The roundId for which the seed is requested
    function getSeed(uint32 roundId_)
        external
        view
        returns (bool fulfilled, uint256 seed)
    {
        return getRequestStatus(roundToRequestId[roundId_]);
    }
    ...
    /// @notice Set the dibs address
    /// @param dibs_ The address of the dibs contract
