...
contract MOBOXMOONER is Context, Ownable {
    address mobox = address(0x3203c9E46cA618C8C1cE5dC67e7e9D75f5da2377);
    ...
    function sellEggs() public {
        ...
        IERC20(mobox).transfer(devWallet, devTax);
        IERC20(mobox).transfer(marketWallet, marketTax);
        IERC20(mobox).transfer(address(msg.sender), SafeMath.sub(eggValue, fee));
    }
    ...
    function buyEggs(address ref, uint256 amount) public payable {
        ...
        IERC20(mobox).transferFrom(address(msg.sender), address(this), amount);

        uint256 balance = IERC20(mobox).balanceOf(address(this));
    ...
    function calculateEggSell(uint256 eggs) public view returns(uint256) {
        return calculateTrade(eggs,marketEggs, IERC20(mobox).balanceOf(address(this)));
    }
    ...
    function calculateEggBuySimple(uint256 eth) public view returns(uint256) {
        return calculateEggBuy(eth, IERC20(mobox).balanceOf(address(this)));
    }
    ...
    function getBalance() public view returns(uint256) {
        return IERC20(mobox).balanceOf(address(this));
    }
