...
contract TimeLockPool is BasePool, ITimeLockPool {
    ...
    uint8 public constant aFactor0_3 = 0;
    uint8 public constant aFactor3_6 = 1;
    uint8 public constant aFactor6_12 = 2;
    uint8 public constant aFactor12_24 = 3;
    uint8 public constant aFactor24_n = 4;
    ...
    event Deposited(uint256 amount, uint256 start, uint256 end, uint256 duration, address indexed receiver, address indexed from);
    ...
    function deposit(uint256 _amount, uint256 _duration, address _receiver) external override {
        ...
        uint256 mintAmount = _amount * getMultiplier(_duration);
        totalLocked += _amount;

        _mint(_receiver, mintAmount);
        emit Deposited(_amount, block.timestamp, block.timestamp + _duration, _duration, _receiver, _msgSender());
    }
    ...
    function withdraw(uint256 _depositId, address _receiver) external {
        ...
        uint256 shareAmount = userDeposit.amount * getMultiplier(uint256(userDeposit.end - userDeposit.start));
        totalLocked -= userDeposit.amount;
        ...
        emit Withdrawn(_depositId, _receiver, _msgSender(), userDeposit.amount, block.timestamp);
    }

    function getMultiplier(uint256 _lockDuration) public view returns(uint256 factor) {
        if (_lockDuration < (monthInSeconds * 3)) {
            factor = aFactor0_3;
        } else if ((monthInSeconds * 3) <= _lockDuration && _lockDuration < (monthInSeconds * 6)) {
            factor = aFactor3_6;
        } else if ((monthInSeconds * 6) <= _lockDuration && _lockDuration < (monthInSeconds * 12)) {
            factor = aFactor6_12;
        } else if ((monthInSeconds * 12) <= _lockDuration && _lockDuration < (monthInSeconds * 24)) {
            factor = aFactor12_24;
        } else {
            factor = aFactor24_n;
        }
...
abstract contract BasePool is ERC20Votes, AbstractRewards, IBasePool, TokenSaver {
    ...
    event RewardsClaimed(address indexed _from, address indexed _receiver, uint256 _amount, uint256 timestamp);
    ...
    function claimRewards(address _receiver) external {
        ...
        emit RewardsClaimed(_msgSender(), _receiver, rewardAmount, block.timestamp); 
    }
...
/* ========  Constants  ======== */
  // uint128 public constant POINTS_MULTIPLIER = 10**18;
...
/* ========  Public View Functions  ======== */
  ...
  function cumulativeRewardsOf(address _account) public view override returns (uint256) {
    // return (pointsPerShare * getSharesOf(_account)) / POINTS_MULTIPLIER;
