...
contract XENTHIUMCOIN is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x7804e54DA98b7cF4673aC7AC8CD606d01998956D); 
    address payable public teamAddress = payable(0xf5A0b785D8aD6492af5200720b1C4a0333A3dD7C); 
    address payable public environmentAddress = payable(0xf409DCd3d26f16b57A286D2c82F681abbF7e43FB);
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) 
        {
            if (to == uniswapV2Pair) 
            {
                if (overMinimumTokenBalance) 
                {
                    ...
                    uint256 marketingDevelopmentAndTeamToken = contractTokenBalance.mul(_sellMarketingFee+_sellEnvironmentFee+_sellTeamFee).div(_sellAllSwapableFees);
                    uint256 liquidityToken = contractTokenBalance.sub(marketingDevelopmentAndTeamToken);

                    addLiquidityToToken(liquidityToken);

                    swapTokens(marketingDevelopmentAndTeamToken);    
    ...
    function SwapAndLiquifyBNBtoBusd(uint256 bnbLiquifyAmount) private lockTheSwap {
        ...
        swapBNBForTokensToBusd(bnbLiquifyAmount);
        ...
        uint256 teamFunds = newTokenBalance.mul(_sellTeamFee).div(_sellAllSwapableFees);
        uint256 environmentFunds = newTokenBalance.sub(marketingFunds).sub(teamFunds);

        require(IERC20(busdAddress).transfer(marketingAddress,marketingFunds));
        require(IERC20(busdAddress).transfer(teamAddress,teamFunds));
        require(IERC20(busdAddress).transfer(environmentAddress,environmentFunds));
    ...
    function addLiquidityToToken(uint256 tokenLiquifyAmount) private lockTheSwap{
        ...
        uint256 half = tokenLiquifyAmount.div(2);
        uint256 otherHalf = tokenLiquifyAmount.sub(half);
