...
contract PigeUp is IERC20 {
  ...
  string private constant _name = "PigeUp";
  string private constant _symbol = "$PigeUp";
  ...
  uint256 public constant maxBuyTaxes = 3000;
  uint256 public constant maxSellTaxes = 3000;
  uint256 public constant maxTransferTaxes = 9000;
  uint256 public constant maxRoundtripFee = 3000;
  ...
  TaxWallets public _taxWallets = TaxWallets({
    marketing: payable(0x8E4BB87C51f4A5Fd092A237600A633A38CE29370)
  });
  ...
  uint256 public dailySellPercent = 100;
  ...
  function setTaxes(
    ...
    require(
      buyFee + sellFee <= maxRoundtripFee,
  ...
  function setContractSwapEnabled(bool swapEnabled, bool priceImpactSwapEnabled)
    external
    onlyOwner
  {
  ...
  function getSecondsToNextSellReset(address account)
    external
    view
    returns (uint256)
  {
        ...
        if (!_isExcludedFromLimits[from] && !_isExcludedFromLimits[to]) {
          if (dailySellCooldownEnabled) {
            if (user[from].sellStamp + dailySellCooldown > block.timestamp) {
              ...
              require(
                addition <= user[from].dailyLimit,
                "Sell amount exceeds daily limit."
              );
              user[from].sold = addition;
            } else {
              user[from].dailyLimit =
                (balanceOf(from) * dailySellPercent) /
                masterTaxDivisor;
              require(
                amount <= user[from].dailyLimit,
                "Sell amount exceeds daily limit."
              );
  ...
  function multiSendTokens(address[] memory accounts, uint256[] memory amounts)
    external
    onlyOwner
  {
