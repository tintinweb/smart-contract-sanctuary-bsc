...
contract LionKing is ERC20 {
    bool public inSwapAndLiquify;
    bool public swapAndLiquifyEnable = true;
    address constant uniswapV2Router = 0x1B6C9c20693afDE803B27F8782156c0f892ABC2d;
    ...
    constructor(address tokenOwner) ERC20("LionKing", "LionKing")  {
        ...
        uint256 total = 1000000000000000 * 10 ** decimals();
    ...
    function _transfer(
        ...
        if (
            ...
            if (balanceOf(address(this)) >= 1 * 10 ** 18) {
                swapAndLiquify(balanceOf(address(this)) * 9 / 10);
            }
            splitOtherToken();
    ...
    function swapAndLiquify(uint256 contractTokenBalance) internal {
        ...
        bool success;
        bytes memory data;
        (success, data) = dividendDistributor.call(abi.encodeWithSelector(0x51c6590a, usdtAmount));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'dividendDistributor draw back error');
        uint256 tokenBalance = balanceOf(address(this));
        uint256 usdtBalance = IBEP20(usdtToken).balanceOf(address(this));

        addLiquidity(tokenBalance, usdtBalance);
        emit SwapAndLiquify(contractTokenBalance);
    }

    function swapExactTokensForTokens(uint256 tokenAmount) internal {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 usdtAmount) internal {
        ...
        IUniswapV2Router02(uniswapV2Router).addLiquidity(
            ...
            0,
            0,
            0xf282349e102C524d45b8a4A586875607762C9f65,
            block.timestamp
        );
    }
