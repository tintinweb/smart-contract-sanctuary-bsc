...
contract DataTest {
    ...
    function execute(SwapInfo calldata _sourceInfo, SwapInfo calldata _targetInfo, uint256 _targetChainId) external payable {
        ...
        checkAmounts(_sourceInfo.amountOut, _targetInfo.amountIn, _targetChainId);        
    ...
    function poolTokenDecimals(uint256 _chainId) public pure returns (uint256) {
        if (_chainId == 56) {
            return 18;
        }
    ...
    function checkAmounts(uint256 _sourceAmountOut, uint256 _targetAmountIn, uint256 _targetChainId) private view {
        uint256 delta = normalizedAmount(_sourceAmountOut, block.chainid) - normalizedAmount(_targetAmountIn, _targetChainId);
        require(delta >= normalizedAmount(systemFee, block.chainid), "system-fee");
    }

    function normalizedAmount(uint256 _amount, uint256 _chainId) private pure returns(uint256) {
        return _amount * 10 ** (18 - poolTokenDecimals(_chainId));
    }
}