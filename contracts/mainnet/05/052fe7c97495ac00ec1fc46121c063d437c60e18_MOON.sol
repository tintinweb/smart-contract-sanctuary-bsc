...
contract MOON is ERC20, Ownable {
    ...
    event SwapFeeEnabledUpdated(bool enabled);
    ...
    constructor(
        ...
        _isSwapWhiteList[msg.sender] = true;
    ...
    function setSwapFeeEnabled(bool _enabled) public onlyOwner {
        ...
        emit SwapFeeEnabledUpdated(_enabled);
    }
    ...
    function includeSwapWhiteListBatch(address[] memory accounts) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
   ...
   function setDeadFee(uint256 val) public onlyOwner {
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount && swapFeeEnabled;
        if( canSwap &&
            !swapping &&
            from != uniswapV2Pair &&
            from != owner() &&
