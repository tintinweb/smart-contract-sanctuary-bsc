...
contract TokenVesting is Ownable, ReentrancyGuard, Pausable {
    ...
    /* Mappings */
    mapping(address => uint256) public balanceBeneficiary; 
    mapping(address => uint256) public totalTokensAssignedBeneficiary; 
    mapping(address => uint256) public totalTokensClaim; 
    mapping(address => uint256) public tokensPerClaim; 

    mapping(address => uint256) public firstClaimDate; 
    mapping(address => uint256) public cooldownTimePerWallet; 
    ...
    /*
     * Contract Address Token that will be used in Vesting
     */
    ...
    constructor(
        ...
        TokenSmartContract = IERC20(address(tokenAddress)); 
    ...
    function assignTokensToWallet(
        uint256[] calldata cantidadesBBCNAAsignar, 
        address[] calldata walletsUsuarios, 
        uint256[] calldata _firstClaimDates, 
        uint256[] calldata _cooldownTimePerWallets 
        ...
        for (uint256 i; i < walletsUsuarios.length; ++i) {
            ...
            //Asignamos la nueva cantidad de tokens a la variable que no cambia
            totalTokensAssignedBeneficiary[walletUsuario] = balanceBeneficiary[
                walletUsuario
            ];
    ...
    function cancelVesting(
        ...
        totalTokensAssignedBeneficiary[walletUsuario] = 0;
        firstClaimDate[walletUsuario] = 0;
        ...
        nonReentrant
    {
        ...
        for (uint256 i; i < (frequency); ++i) {
            ...
            if ((i != 0) && (fechaClaim <= block.timestamp))
                fechaClaim += cooldownTimePerWallet[_investor];
