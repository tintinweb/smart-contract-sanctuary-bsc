...
library Address{
 ...
 contract DOGEWHALE is Context, IERC20, Ownable {
    ...
    address public DOGEWHALEWallet = 0x68f8965fb4979c23130c2C6464d6C204Ab404989;
    address public donationWallet = 0x68f8965fb4979c23130c2C6464d6C204Ab404989;

    string private constant _name = "DOGEWHALE";
    string private constant _symbol = "DOGEWHALE";
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[DOGEWHALEWallet] = true;
        _isExcludedFromFee[donationWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[DOGEWHALEWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _DOGEWHALE, uint256 _liquidity, uint256 _donation) public onlyOwner {
       taxes = Taxes(_rfi,_DOGEWHALE,_liquidity,_donation);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _DOGEWHALE, uint256 _liquidity, uint256 _donation) public onlyOwner {
       sellTaxes = Taxes(_rfi,_DOGEWHALE,_liquidity,_donation);
        emit FeesChanged();
    }
    ...
    function _takeDOGEWHALE(uint256 rDOGEWHALE, uint256 tDOGEWHALE) private {
        totFeesPaid.DOGEWHALE +=tDOGEWHALE;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tDOGEWHALE;
        }
        _rOwned[address(this)] +=rDOGEWHALE;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rDOGEWHALE, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tDOGEWHALE = tAmount*temp.DOGEWHALE/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tDonation = tAmount*temp.donation/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tDOGEWHALE-s.tLiquidity-s.tDonation;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rDOGEWHALE, uint256 rLiquidity){
        ...
        rDOGEWHALE = s.tDOGEWHALE*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rDonation = s.tDonation*currentRate;
        rTransferAmount =  rAmount-rRfi-rDOGEWHALE-rLiquidity-rDonation;
        return (rAmount, rTransferAmount, rRfi,rDOGEWHALE,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tDOGEWHALE + s.tDonation);
        }
        if(s.rDOGEWHALE > 0 || s.tDOGEWHALE > 0) _takeDOGEWHALE(s.rDOGEWHALE, s.tDOGEWHALE);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.DOGEWHALE + temp.donation) * 2;
        ...
        uint256 DOGEWHALEAmt = unitBalance * 2 * temp.DOGEWHALE;
        if(DOGEWHALEAmt > 0){
            payable(DOGEWHALEWallet).sendValue(DOGEWHALEAmt);
        }
    ...
    function updateDOGEWHALEWallet(address newWallet) external onlyOwner{
        DOGEWHALEWallet = newWallet;
    }
