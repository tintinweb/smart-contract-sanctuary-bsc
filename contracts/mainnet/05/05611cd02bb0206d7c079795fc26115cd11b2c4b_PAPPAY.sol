...
contract OwnablePAY is ContextPAY {
    address private _owner;

    event OwnerPAYpTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor () {
        ...
        _owner = msgSender;
        emit OwnerPAYpTransferred(address(0), msgSender);
    }
     ...
     */
    modifier onlyOwner() {
        require(_owner == _msgSender(), "OwnablePAY: caller is not the owner");
        _;
    }

 
    function renounceOwnerPAYp() public virtual onlyOwner {
        emit OwnerPAYpTransferred(_owner, address(0));
        _owner = address(0);
    }

    /**
     * @dev Transfers ownerPAYp of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     */
    function transferOwnerPAYp(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "OwnablePAY: new owner is the zero address");
        emit OwnerPAYpTransferred(_owner, newOwner);
        _owner = newOwner;
    }
}

contract PAPPAY is ContextPAY, BinanceBNB, OwnablePAY {
    ...
    string private _name = 'PAP PAY';
    string private _symbol = 'PAP PAY';
