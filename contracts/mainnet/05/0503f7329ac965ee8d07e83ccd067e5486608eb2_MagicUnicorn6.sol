...
contract MagicUnicorn6 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn6DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn6", "MU6") {
        ...
        dividendTracker = new MagicUnicorn6DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn6TRACKER", "MU6TRACKER");
...
contract MagicUnicorn6DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn6 public MagicUnicorn6Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn6Contract = MagicUnicorn6(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn6 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn6Contract);

            uint256 prevBalance = MagicUnicorn6Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn6Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn6Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
