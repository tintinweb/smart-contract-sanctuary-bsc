...
contract MonstropolyNFTAirdrop is
    UUPSUpgradeableByRole,
    ERC721HolderUpgradeable
{
    bytes32 public constant NFT_AIRDROP_ADMIN_ROLE =
        keccak256("NFT_AIRDROP_ADMIN_ROLE");

    event FailedAirdrop(address receiver);
    ...
    function mintAirdrop(address token, address[] calldata receivers)
        external
        onlyRole(NFT_AIRDROP_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < receivers.length; i++) {
            (bool success, ) = token.call(
                abi.encodeWithSignature("mint(address)", receivers[i])
            );
            if (!success) emit FailedAirdrop(receivers[i]);
        }
    }
    ...
    ) external onlyRole(NFT_AIRDROP_ADMIN_ROLE) {
        require(
            receivers.length == tokenIds.length,
            "MonstropolyNFTAirdrop: lengths dont match"
        );
        for (uint256 i = 0; i < receivers.length; i++) {
            (bool success, ) = token.call(
