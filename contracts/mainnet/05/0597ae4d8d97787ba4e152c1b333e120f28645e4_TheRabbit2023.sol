...
The Rabbit 2023 - A memecoin to celebrate the year of the rabbit!
兔年快乐！

Join us: https://t.me/therabbit2023portal
...
contract TheRabbit2023 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "TheRabbit2023";
    string constant _symbol = "TheRabbit";
    ...
    bool public rabbitholeMode = true;
    mapping (address => bool) public israbbitholed;
    ...
    bool public ApeRabbitOpen = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient]){
            require(ApeRabbitOpen,"Trading not open yet");
        }

        // Rabbithole (Bots)
        if(rabbitholeMode){
            require(!israbbitholed[sender] && !israbbitholed[recipient],"Rabbithole");    
        }
    ...
    // switch Tradingonlaunch
    function ApeRabbitStatus(bool _status) public onlyOwner {
        ApeRabbitOpen = _status;
    }
    ...
    function enable_rabbithole(bool _status) public onlyOwner {
        rabbitholeMode = _status;
    }

    function manage_rabbithole(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            israbbitholed[addresses[i]] = status;
        }
    }
