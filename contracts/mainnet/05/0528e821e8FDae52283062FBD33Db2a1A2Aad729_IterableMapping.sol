    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            if(rewardToken == weth) {
                ...
                success = user.balance.sub(initBalance) == _withdrawableDividend;
            } else {
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor(address rewardToken_, address weth_, uint256 minimumTokenBalanceForDividends_) DividendPayingToken(
        rewardToken_, weth_
    ) {
        claimWait = 3600;
...
contract BabyCorgiCeo is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
    ...
    ) ERC20("BabyCorgiCeo", "BCC") {
        rewardToken = rewardToken_;
        uint256 totalSupply_ = 1 * (10**9) * (10**18);
        swapTokensAtAmount = totalSupply_.mul(5).div(10000); // 0.05%
        maxTransaction = totalSupply_.mul(5).div(1000); // 0.5%
        ...
        dividendTracker = new DividendTracker(rewardToken, weth, swapTokensAtAmount.mul(5));
    ...
    function setFee(uint256 rewardsFee_, uint256 marketingFee_, uint256 buyFee_, uint256 sellFee_) external onlyOwner {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(marketingFee).div(marketingFee+rewardsFee);
    ...
    function swapTokensForReward(uint256 tokenAmount) private {
        ...
        if (rewardToken == weth) {
            ...
            uint256 receiveAmount = address(this).balance.sub(initBalance);
            IWETH(weth).deposit{value: receiveAmount}();
        } else {
