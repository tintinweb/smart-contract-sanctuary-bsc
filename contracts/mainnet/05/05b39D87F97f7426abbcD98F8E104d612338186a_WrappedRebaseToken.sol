...
contract WrappedRebaseToken is IERC20, Ownable {
    ...
    // token data
    string private constant _name = "Wrapped-PTX";
    string private constant _symbol = "wPTX";
    uint8 private constant _decimals = 5;

    // Token We Wrap Around
    IERC20 public constant PTX = IERC20(0x5Ec500C5C6f0a270b633d5D5f0c3b9eB9b041455);
    ...
    /**
        Wrap `amount` of PTX into wPTX
     */
    function wrap(uint256 amount) external {
        require(amount > 0, "Zero Amount");
        require(
            amount <= PTX.balanceOf(msg.sender),
            "Amount Exceeds Senders PTX Balance"
        );

        // wPTX To Mint
        uint256 wPTXToMint = _totalSupply == 0
            ? amount
            : getWrappedQuantity(amount);

        // Transfer In PTX
        uint256 before = PTX.balanceOf(address(this));
        bool s = PTX.transferFrom(msg.sender, address(this), amount);
        uint256 received = PTX.balanceOf(address(this)).sub(before);
        require(s && received > 0 && received <= amount, "Error TransferFrom");

        // Mint wPTX To Sender
        _mint(msg.sender, wPTXToMint);
    }

    /**
        Unwrap `amount` of wPTX into PTX
     */
    function unwrap(uint256 amount) external {
        require(amount > 0, "Zero Amount");
        require(amount <= balanceOf(msg.sender), "Amount Exceeds Balance");

        // calculate PTX to redeem for wPTX
        uint256 PTXToSend = getUnwrappedQuantity(amount);

        // burn tokens from balance and supply
        _burn(msg.sender, amount);

        // release PTX for caller
        require(
            PTX.transfer(msg.sender, PTXToSend),
            "Error On PTX Transfer"
        );
    }

    function withdraw(address token) external onlyOwner {
        require(token != address(0), "Zero Address");
        require(token != address(PTX), "Cannot Withdraw PTX");
        require(
    ...
    function getRedeemRate() public view returns (uint256) {
        return PTX.balanceOf(address(this)).mul(precision).div(_totalSupply);
    }

    /**
        For an `amount` of wPTX, Returns the amount of PTX to receive when unwrapping
     */
        ...
        returns (uint256)
    {
        ...
        uint256 PTXBalance = PTX.balanceOf(address(this));
        return unwrapQTY < PTXBalance ? unwrapQTY : PTXBalance;
    }

    /**
        For an `amount` of PTX, Returns the amount of wPTX to receive when wrapping
     */
    ...
    /**
        Burns `amount` wPTX from `account`
     */
    ...
    /**
        Mints `amount` of wPTX to `account`
     */
