...
contract HighestFinance is ERC20Detailed, Ownable {
    ...
    string public _name = "Highest Finance";
    string public _symbol = "HGT";
    ...
    uint256 public actionFundFee = 20;
    uint256 public referralFundFee = 20;

    uint256 public sellFee = actionFundFee.add(referralFundFee);
    ...
    address public actionFundReceiver;
    address public referralFundReceiver;
    ...
    constructor() ERC20Detailed("Highest Finance", "HGT", uint8(DECIMALS)) Ownable() {
        ...
        lpReceiver = 0xB89e8f600779C3be29dE6C9e713CEa7Fc25C1a95;
        treasuryReceiver = 0x4e9371Cb1Cb00B20bE5999c050bDc7163c420c9C;
        insuranceReceiver = 0xB468021b0189f16ece1f749C2f1268eDef93A70F;
        actionFundReceiver = 0xCDc06838433458BcbBDa36802E5b879e91E1282C;
        referralFundReceiver = 0x7A4991a0F60523d02f67D62e2849519098b1BD81;
    ...
    function setZxFees() external onlyOwner {
        liquidityFee = 0;
        treasuryFee = 0;
        insuranceFee = 0;
        actionFundFee = 40;
        ...
        sellFee = actionFundFee.add(referralFundFee);
    ...
    function rebase() internal {
        ...
        for (uint idx = 0; idx < blockCount.mod(15); idx++) { // 4 sec rebase
            // S' = S(1+p)^r
            tmp = tmp.mul(deno.mul(100).add(rebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(15).mod(60); idx++) { // 1 min rebase
            // S' = S(1+p)^r
            tmp = tmp.mul(deno.mul(100).add(minuteRebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(15 * 60).mod(24); idx++) { // 1 hour rebase
            // S' = S(1+p)^r
            tmp = tmp.mul(deno.mul(100).add(hourRebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(15 * 60 * 24); idx++) { // 1 day rebase
            // S' = S(1+p)^r
    ...
    function swapBack() internal swapping {
        ...
        uint256 _currentFee = treasuryFee.add(insuranceFee).add(actionFundFee).add(referralFundFee);
        ...
        (success, ) = payable(actionFundReceiver).call{
        value: currentBalance.mul(actionFundFee).div(
            _currentFee
        ),
        gas: 30000
        }("");

        (success, ) = payable(referralFundReceiver).call{
        value: currentBalance.mul(referralFundFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
    ...
    function setFeeReceivers(
        ...
        address _referralFundReceiver,
        address _actionFundReceiver
    ) external onlyOwner {
        treasuryReceiver = _treasuryReceiver;
        insuranceReceiver = _insuranceReceiver;
        referralFundReceiver = _referralFundReceiver;
        actionFundReceiver = _actionFundReceiver;
