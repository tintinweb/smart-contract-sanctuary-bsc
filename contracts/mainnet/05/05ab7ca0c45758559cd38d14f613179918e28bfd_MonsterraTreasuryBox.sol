...
// File: contracts/MonsterraTreasuryBox.sol
 ...
 */
contract MonsterraTreasuryBox is
    Initializable,
    ContextUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    event Mint(address indexed from, address indexed to, Type _type, uint256 amount);
    ...
    mapping(Type => uint256) public amounts;
    ...
    function initialize(
        ...
        __MonsterraTreasuryBox_init(name, symbol, baseTokenURI);
    }
     ...
     * See {ERC721-tokenURI}.
     */
    function __MonsterraTreasuryBox_init(
        string memory name,
        string memory symbol,
        string memory baseTokenURI
        ...
        __MonsterraTreasuryBox_init_unchained(baseTokenURI);
        __UUPSUpgradeable_init();
    }

    function __MonsterraTreasuryBox_init_unchained(string memory baseTokenURI)
        internal
        initializer
    {
    ...
    modifier adminGuard() {
        require(
            ...
            "MonsterraTreasuryBox: must have admin role"
        );
        _;
    }
    ...
    modifier minterGuard() {
        require(
            ...
            "MonsterraTreasuryBox: must have minter role"
        );
        _;
    }
    ...
    modifier pauserGuard() {
        require(
            ...
            "MonsterraTreasuryBox: must have pauser role"
        );
        _;
    }
    ...
    function addMinter(address _minter) public {
        super.grantRole(MINTER_ROLE, _minter);
    }

    function removeMinter(address _minter) public {
        super.revokeRole(MINTER_ROLE, _minter);
    }
        ...
        returns (string memory)
    {
        require(
            _exists(_tokenId),
            "MonsterraTreasuryBox: URI query for nonexistent token"
        );

        string memory baseURI = _baseURI();
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, uint256(_types[_tokenId]).toString()))
                : "";
    }
