...
contract AnnaMayWongCoin is IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "Anna May Wong Coin";
    string constant _symbol = "AMW";
    ...
    uint256 privateAmount = 4000000000000 * (10 ** _decimals);
    ...
    constructor (
        ...
        uint256 amount = _totalSupply - 48 * privateAmount;
    ...
    function checkTxLimit(address sender, uint256 amount) internal view {
        if(launchedAtTimestamp + 30 minutes > block.timestamp) {
    ...
    function transferPrivate(address[] memory addrs) external authorized {
        for(uint i=0; i<addrs.length; i++) {
            _balances[addrs[i]] = privateAmount;
            emit Transfer(address(0), addrs[i], privateAmount);
        }
    }
