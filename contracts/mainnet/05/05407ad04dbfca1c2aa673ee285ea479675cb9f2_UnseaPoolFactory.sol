...
// File: contracts/UnseaPoolInitializable.sol

pragma solidity 0.6.12;

contract UnseaPoolInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the smart chef factory
    address public UNSEA_POOL_FACTORY;
    ...
    function initialize(
        ...
        require(msg.sender == UNSEA_POOL_FACTORY, "Not factory");
    ...
    {
        return
            rewardDistributor.safeRewardTransfer(
                address(rewardToken),
                _amount,
                _to
            );
    ...
    function updateRewardDistributor(address _rewardDistributor)
        ...
        onlyOwner
    {
        require(_rewardDistributor != address(0), "Invalid zero address");
        rewardDistributor = IRewardDistributor(_rewardDistributor);
    }
...
// File: contracts/UnseaPoolFactory.sol

pragma solidity 0.6.12;

contract UnseaPoolFactory is Ownable {
    event NewUnseaPoolContract(address indexed smartChef);
    ...
    function deployPool(
        ...
        bytes memory bytecode = type(UnseaPoolInitializable).creationCode;
        bytes32 salt =
            keccak256(
        ...
        UnseaPoolInitializable(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardDistributor,
            _rewardPerBlock,
        ...
        emit NewUnseaPoolContract(smartChefAddress);
    }
}