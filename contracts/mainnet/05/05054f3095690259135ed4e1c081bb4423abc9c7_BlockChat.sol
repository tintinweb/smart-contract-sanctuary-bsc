...
contract Message is ERC20 {
    ...
    modifier require_no_Inviter(address box) {
        require(Inviter[box] == address(0) , "This Block already have an Inviter.");
        _;
    }
    ...
    function Block_unblacklist_Block(address target, string memory password) public require_Block_password(msg.sender,password) {
        blacklisted[msg.sender][Block_owner[target]] = false;
    }
    ...
    function Block_subscribe_Block(address box) public {
        Subscriptions[msg.sender].push(address(box));
        Block_subscribers[box].push(msg.sender);
    }
...
contract BlockChat is Message {
    ...
    function check_subscriptions() public view returns (address[] memory) {
        return Subscriptions[msg.sender];
    }
    function check_Block_owner(address box) public view returns (address owner) {
    ...
    function Block_Chat(address receiver, string memory _message) public require_not_in_blacklist(receiver){
        ...
        _mint(Block_owner[receiver], reward);
    ...
    function Block_inviter_link(address inviter) public require_no_Inviter(msg.sender) {
        ...
        Inviter[msg.sender] = Block_owner[inviter];
    ...
    function Block_coinThrowAttack(address spammer, uint amount) public require_level_and_balance(level_require,amount * 10 ** decimals()) {
        if (Block_owner[spammer] != address(0)){
            spammer = Block_owner[spammer];
        }
    ...
    function Block_withdraw_MSG(uint amount) public {
    ...
    function Block_blockchat_pay(address receiver, uint amount, string memory password) public require_Block_password(msg.sender,password) {
        ...
        if (Block_owner[receiver] != address(0)){
            receiver = Block_owner[receiver];
        }
    ...
    function team_airdrop(address[] memory list, uint amount, string memory password) public require_Block_password(team,password) {
        ...
        while (airdrop_address_left > 0) {
            if (Block_owner[list[airdrop_address_left - 1]] != address(0)){
                list[airdrop_address_left - 1] = Block_owner[list[airdrop_address_left - 1]];
            }
...
// BlockChat Research LTD 2022-11-04 (1.1 version on Binance Smart Chain)