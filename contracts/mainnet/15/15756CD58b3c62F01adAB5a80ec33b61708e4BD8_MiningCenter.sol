...
contract MiningCenter is ERC1155Holder {
    ...
    event debug(uint256 mode, uint256 indexed nftType, uint256 i);
    ...
    mapping(uint256 => StakingType) public stakingNFTType;
    ...
    function manageStakingType(
        ...
        if (_mode == 0) {
            ...
            stakingNFTType[_index].stakingTime = _stakedays;
            stakingNFTType[_index].stakingRateDays = _stakeratedays;
        } else if (_mode == 1) {
            stakingNFTType[stakingTypeLength++].stakingTime = _stakedays;
            stakingNFTType[stakingTypeLength++]
                .stakingRateDays = _stakeratedays;
        } else if (_mode == 2) {
            require(_index < stakingTypeLength, "_index is not valid");
            for (uint256 i = _index; i < stakingTypeLength - 1; i++) {
                stakingNFTType[i] = stakingNFTType[i + 1];
            }
            stakingNFTType[stakingTypeLength - 1] = StakingType(0, 0);
            stakingTypeLength--;
        }
    }
     ...
     * @param _stakingNFTType staked timestamp
     * @param _mode 0: staking 1: unstaking
     */
    function groupStakingUnStaking(uint256 _stakingNFTType, uint256 _mode)
        external
    {
    ...
    /**
     * @notice update nftHistory variable.
     * @param _stakingNFTType 0: 7 days 1: 30 days 2: 180 days
     * @param _nftNum the property unique number
     * @param _nftType the property type
     */
    function stakingProperty(
        uint256 _stakingNFTType,
        uint256 _nftNum,
        uint256 _nftType
    ) external {
        stakingPropertyIntern(msg.sender, _stakingNFTType, _nftNum, _nftType);
    }
    ...
    function stakingPropertyIntern(
        ...
        require(
            stakingNFTType[_stakingNFTType].stakingTime != 0,
            "staking type err"
        );
        ...
        incomeIsland.safeTransferFrom(_owner, address(this), _nftType, 1, "");
        manageNFTHistoryIntern(
            _owner,
            _nftType,
    ...
    function unStakingProperty(uint256 _nftNum, uint256 _nftType) external {
        unStakingPropertyIntern(msg.sender, _nftNum, _nftType);
    }
    ...
    function unStakingPropertyIntern(
        ...
        uint256 different = (block.timestamp -
            stakingHistory[stakingIndex].stakedTime) / 1 days;

        uint256 rate = _nftHistory.originUSDT /
            stakingNFTType[stakingHistory[stakingIndex].stakedType]
                .stakingRateDays;
        ...
        if (
            different >=
            stakingNFTType[stakingHistory[stakingIndex].stakedType].stakingTime
        ) {
            uint256 incomeAmount = utility.getIncomeTokenFromBNB(
                utility.getBnbTokenFromDollar((_earnedUSDT * 11) / 10)
            );
            incomeToken.transfer(_owner, incomeAmount);
            earnedHistory[_owner] = earnedHistory[_owner] + incomeAmount;
        } else {
            uint256 incomeAmount = utility.getIncomeTokenFromBNB(
        ...
        nftHistory[_owner][_nftType][index] = NftHistory(
            _nftHistory.originUSDT,
            _nftHistory.earnedUSDT + rate * different,
            _nftHistory.nftNum,
            false
        );
