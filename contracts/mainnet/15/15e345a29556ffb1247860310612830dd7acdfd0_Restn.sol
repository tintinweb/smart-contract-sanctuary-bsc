...
contract Restn is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    uint256 public sellFee2 = 14;
    uint256 public sellFee3 = 16;
    uint256 public sellFee4 = 18;
    uint256 public sellFee5 = 20;
    ...
    constructor() ERC20Detailed("Restn", "RST", uint8(DECIMALS)) Ownable() {
        ...
        address _owner = 0xdB85EB773b9ddB7A492dc4BdF7965F75fAe29c52;
        ...
        _isFeeExempt[safuDevAddress] = true;
    ...
    function takeFee(address sender, address recipient, uint256 gonAmount) internal returns (uint256){
        ...
        if (automatedMarketMakerPairs[sender]){
            ...
            else if (_percentage < 500) _realFee = sellFee4;
            else _realFee = sellFee5;
        }else {
    ...
    function setFeeExempt(address _addr, bool _value) external isAuth {
        require(_isFeeExempt[_addr] != _value, "Not changed");
        _isFeeExempt[_addr] = _value;
    }

    function setTargetLiquidity(uint256 target, uint256 accuracy) external isAuth {
        targetLiquidity = target;
        targetLiquidityDenominator = accuracy;
    }

    function setSwapBackSettings(bool _enabled) external isAuth {
    ...
    function setAllFees (uint256 _transferFee, uint256 _buyFee, 
        uint256 _sellFee1, uint256 _sellFee2, uint256 _sellFee3, 
        uint256 _sellFee4, uint256 _sellFee5) public isAuth {
            
        require (_transferFee <= 12 && _buyFee <= 4 &&
            _sellFee1 <= 20 && _sellFee2 <= 20 && _sellFee3 <= 20 && _sellFee4 <= 20 && 
            _sellFee5 <= 20, "Fees too high");
    ...
    function setFeeShare(uint256 _liquidityFee, uint256 _burnFee, uint256 _treasuryFee, uint256 _RFVFee ) external isAuth {
    ...
    function clearStuckBalance(address _receiver) external isAuth {
        uint256 balance = address(this).balance;
        payable(_receiver).transfer(balance);
    }

    function rescueToken(address tokenAddress, uint256 tokens) external isAuth returns (bool success){
        return ERC20Detailed(tokenAddress).transfer(msg.sender, tokens);
    }

    function setAutoRebase(bool _autoRebase) external isAuth {
        require(autoRebase != _autoRebase, "Not changed");
        autoRebase = _autoRebase;
    }

    function setRebaseFrequency(uint256 _rebaseFrequency) external isAuth {
        require(_rebaseFrequency <= MAX_REBASE_FREQUENCY, "Too high");
        rebaseFrequency = _rebaseFrequency;
    }

    function setRewardYield(uint256 _rewardYield, uint256 _rewardYieldDenominator) external isAuth {
        rewardYield = _rewardYield;
        rewardYieldDenominator = _rewardYieldDenominator;
    }

    function setFeesOnNormalTransfers(bool _enabled) external isAuth {
        require(feesOnNormalTransfers != _enabled, "Not changed");
        feesOnNormalTransfers = _enabled;
    }

    function setIsLiquidityInBnb(bool _value) external isAuth {
        require(isLiquidityInBnb != _value, "Not changed");
        isLiquidityInBnb = _value;
    }

    function setNextRebase(uint256 _nextRebase) external isAuth {
