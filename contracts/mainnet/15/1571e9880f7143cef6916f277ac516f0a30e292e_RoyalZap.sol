 ...
 * Implemented and modified by RoyalSwap teams.
 */
contract RoyalZap is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // RoyalRouter interface
    IRoyalRouter02 public royalRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from RoyalSwap)
    ...
    // Address RoyalRouter
    address private royalRouterAddress;
     ...
     * @param _royalRouter: address of the RoyalRouter
    ...
    constructor(
        ...
        royalRouterAddress = _royalRouter;
     ...
     * @param _lpToken: LP token address (e.g. ROYAL/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. ROYAL) to receive in the intermediary swap (e.g. BNB --> ROYAL)
     */
     ...
     * @param _lpToken: LP token address (e.g. ROYAL/USDC)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ROYAL) in the intermediary swap (e.g. USDC --> ROYAL)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. ROYAL/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. ROYAL --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. ROYAL/USDC)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. ROYAL or USDC)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. ROYAL) in the intermediary swap (e.g. USDC --> ROYAL)
     */
    ...
    {
        address token0 = IRoyalPair(_lpToken).token0();
        address token1 = IRoyalPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IRoyalPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = royalRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = royalRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IRoyalPair(_lpToken).token0() || _token0ToZap == IRoyalPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IRoyalPair(_lpToken).token0() || _token1ToZap == IRoyalPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IRoyalPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IRoyalPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IRoyalPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = royalRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = royalRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IRoyalPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "RoyalRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "RoyalRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        royalRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), royalRouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(royalRouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(royalRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = royalRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = royalRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = royalRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = royalRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = royalRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
