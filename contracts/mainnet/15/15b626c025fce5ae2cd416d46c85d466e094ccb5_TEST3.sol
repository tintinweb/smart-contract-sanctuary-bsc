...
contract TEST3 is ERC20, Ownable {
   ...
   TEST3DividendTracker public dividendTracker;
    ...
    constructor() ERC20("TEST3", "TESTERY") {
...
contract TEST3DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(
        ...
        defaultToken = token;
        TEST3Contract = TEST3(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "TEST3_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BitBurnReflect contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(TEST3Contract);

            uint256 prevBalance = TEST3Contract.balanceOf(address(this));
            ...
            {
                uint256 received = TEST3Contract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    TEST3Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
