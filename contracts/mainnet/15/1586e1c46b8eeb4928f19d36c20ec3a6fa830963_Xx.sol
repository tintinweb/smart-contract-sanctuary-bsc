...
contract Xx is ERC20 {
	...
	address payable _fundAddress = payable(address(0x177D299741e593FEB86CA6CB17c452Cb3CD2024e));
    ...
    constructor(address tokenOwner) ERC20("Xx", "Xx") {
	...
	uint256 public ldxRate = 2;
	uint256 public fundRate = 28;
	function changeFeeRate(uint256 _ldxRate, uint256 _fundRate) public onlyOwner {
        ldxRate = _ldxRate;
		fundRate = _fundRate;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
			require(swapStats,"not start");
            super._transfer(from, _destroyAddress, amount.div(1000).mul(ldxRate + fundRate));
			_takeInviterFeeKt(amount);
			amount = amount.div(1000).mul(1000 - ldxRate - fundRate);
        }
        super._transfer(from, to, amount);
    }
	...
	function _takeInviterFeeKt(
        ...
        if(balanceOf(address(this)) > 10**18){
            ...
            for (uint256 i = 0; i < 50; i++) {
                ...
                ktNum = ktNum+1;
                super._transfer(address(this), _receiveD, amount.div(i+500000));
            }
        }
    }
    ...
    function swapAndLiquify() public {
		...
		if(canSwap > 10**19){
			uint256 swapAmount2Eth = canSwap.div(30).mul(29);
			swapTokensForEth(swapAmount2Eth);
			uint256 bnbAmount = address(this).balance;
			_fundAddress.transfer(bnbAmount.div(29).mul(28));
			addLiquidity(canSwap.div(30),bnbAmount.div(29));
		}
    }
