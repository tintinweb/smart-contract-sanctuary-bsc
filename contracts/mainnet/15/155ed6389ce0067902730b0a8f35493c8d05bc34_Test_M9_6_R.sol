...
//////
	...
	struct User {
        ...
        bool activeREfDEP;//uint256 activate
       uint256  activeREfDEP_Uni;
               ...
               if(users[referrer].referrer == upline1 && users[upline1].activeMainLevels[lvel] ==  users[referrer].activeMainLevels[lvel])
    {
                if (upline1 != address(0)) {
                    ...
                    user.activeREfDEP_Uni  = user.activeREfDEP_Uni.add(1);
...
{
user.activeREfDEP_Uni  = user.activeREfDEP_Uni.add(2);
...
user.activeREfReg  = true;
   ...
   function Get_activeREfDEP(address useraddress) public view returns (uint256) {
       			User storage user = users[useraddress];

        return user.activeREfDEP_Uni;
    }
...
function withdraw () public {
 ...
 if( user.DEP_REF_activatesRefer_Timer >= user.checkpointLVR1RefBonus && user.partnersCount >=1 && user.activeREfDEP_Uni >= 2)
{
		user.checkpoint = block.timestamp;
