...
/*
    Telegram : https://t.me/catfoodbsc
*/
...
contract CatFoodContract is Context, IERC20, Ownable {
    ...
    event SetCharityFee(uint256 amount);
    
    string private _name = "Cat Food";
    string private _symbol = "CTF";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 100000 * 10**9 * 10**_decimals;
    
    address payable public marketingAddress = payable(0xeCFe96b9A55364D41f9237e08c4a3C522881bE5F);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant charityAddress = 0xeCFe96b9A55364D41f9237e08c4a3C522881bE5F;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFeeCatF;
    mapping (address => bool) private _isExcludedFromMaxBalancesCatF;

    uint256 private constant _maxFeeCatF = 15;
    uint256 private _totalFeeCatF;
    uint256 private _totalFeeCatFToContract;
    uint256 private _liquidityFeeCatF;
    uint256 private _charityFeeCatF;
    uint256 private _marketingFeeCatF;
    ...
    constructor () {
        ...
        _isExcludedFromFeeCatF[owner()] = true;
        _isExcludedFromFeeCatF[address(this)] = true;
        
        _isExcludedFromMaxBalancesCatF[owner()] = true;
        _isExcludedFromMaxBalancesCatF[address(this)] = true;
        _isExcludedFromMaxBalancesCatF[uniswapV2Pair] = true;

        _liquidityFeeCatF = 2;
        _marketingFeeCatF = 12;
        _charityFeeCatF = 1;
        _totalFeeCatF = _liquidityFeeCatF.add(_marketingFeeCatF).add(_charityFeeCatF);
        _totalFeeCatFToContract = _liquidityFeeCatF.add(_marketingFeeCatF);
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_charityFeeCatF).add(_marketingFeeCatF) <= _maxFeeCatF, "Fees are too high.");
        _liquidityFeeCatF = newLiquidityFee;
        _totalFeeCatF = _liquidityFeeCatF.add(_marketingFeeCatF).add(_charityFeeCatF);
        _totalFeeCatFToContract = _liquidityFeeCatF.add(_marketingFeeCatF);
        emit SetLiquidityFee(_liquidityFeeCatF);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_liquidityFeeCatF.add(_charityFeeCatF).add(newMarketingFee) <= _maxFeeCatF, "Fees are too high.");
        _marketingFeeCatF = newMarketingFee;
        _totalFeeCatF = _liquidityFeeCatF.add(_marketingFeeCatF).add(_charityFeeCatF);
        _totalFeeCatFToContract = _liquidityFeeCatF.add(_marketingFeeCatF);
        emit SetMarketingFee(_marketingFeeCatF);
    }

    function setCharityFeePercent(uint256 newCharityFee) external onlyOwner() {
        require(_liquidityFeeCatF.add(newCharityFee).add(_marketingFeeCatF) <= _maxFeeCatF, "Fees are too high.");
        _charityFeeCatF = newCharityFee;
        _totalFeeCatF = _liquidityFeeCatF.add(_marketingFeeCatF).add(_charityFeeCatF);
        emit SetCharityFee(_charityFeeCatF);
    }
    ...
    function isExcludedFromFees(address account) public view returns(bool) {
        return _isExcludedFromFeeCatF[account];
    }
    
    function excludeFromFees(address account) public onlyOwner {
        _isExcludedFromFeeCatF[account] = true;
    }
    
    function includeInFees(address account) public onlyOwner {
        _isExcludedFromFeeCatF[account] = false;
    }

    function isExcludedFromMaxBalance(address account) public view returns(bool) {
        return _isExcludedFromMaxBalancesCatF[account];
    }
    
    function excludeFromMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalancesCatF[account] = true;
    }
    
    function includeInMaxBalance(address account) public onlyOwner {
        _isExcludedFromMaxBalancesCatF[account] = false;
    }

    function totalFees() public view returns (uint256) {
        return _totalFeeCatF;
    }

    function liquidityFee() public view returns (uint256) {
        return _liquidityFeeCatF;
    }

    function marketingFee() public view returns (uint256) {
        return _marketingFeeCatF;
    }

    function CharityFee() public view returns (uint256) {
        return _charityFeeCatF;
    }
    ...
    function _transfer(
        ...
        if(
            ...
            !_isExcludedFromMaxBalancesCatF[to]  // is excludedFromMaxBalance
        ){
            require(
        ...
        if(
            ...
            _totalFeeCatFToContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromFeeCatF[from] || _isExcludedFromFeeCatF[to])
            && _totalFeeCatF > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_totalFeeCatFToContract).div(100);
            uint256 toBurnAmount = amount.mul(_charityFeeCatF).div(100);
            
        	amount = amount.sub(feesToContract.add(toBurnAmount)); 

            transferToken(from, address(this), feesToContract);
            transferToken(from, charityAddress, toBurnAmount);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_liquidityFeeCatF).div(_totalFeeCatFToContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeCatF).div(_totalFeeCatFToContract);
