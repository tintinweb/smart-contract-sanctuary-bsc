...
contract FlashBotsUniswapQueryV2 {
    ...
    function getTokensReservesByPairs(IUniswapV2Pair[] calldata _pairs)
        external
        view
        returns (address[2][] memory, uint256[3][] memory)
    {
        address[2][] memory tokens = new address[2][](_pairs.length);
        uint256[3][] memory result = new uint256[3][](_pairs.length);
        for (uint i = 0; i < _pairs.length; i++) {
            tokens[i][0] = _pairs[i].token0();
            tokens[i][1] = _pairs[i].token1();
            (result[i][0], result[i][1], result[i][2]) = _pairs[i]
                .getReserves();
        }
