...
contract SoulMetaRouter is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    function setContracts(address[] calldata addrs_ , bool isOpenCast_, bool isOpenExtract_) public onlyOwner {
        ...
        a3dNft = addrs_[6];
        aSoulPair = addrs_[7];
        aSoulMinter = addrs_[8];
        aReceiveAddr = addrs_[9];
    ...
    function drawBox(address caller_, uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns (bool){
        ...
        cSoulToken.transferFrom(address(cSoulController), player, s * amount_);
        ...
        uint random = randomCeil(2);
        uint count;
        if(random == 1){
            count = _getOneKey(player, theme_, group_, amount_, recommender_);
            if(count > 0){
                count = _getOrdinary(player, theme_, group_, (amount_ - count), recommender_);
            }
        }else{
            count = _getOrdinary(player, theme_, group_, amount_, recommender_);
            if(count > 0){
                count = _getOneKey(player, theme_, group_, (amount_ - count), recommender_);
            }
        }
    ...
    function _getOneKey(address player, uint8 theme_, uint8 group_, uint amount_, address recommender_) private returns (uint){
        uint8 soulCount;
        uint poolId;
        uint[] memory ids;
        uint[] memory counts;
    ...
    function _getOrdinary(address player, uint8 theme_, uint8 group_, uint amount_, address recommender_) private returns(uint){
        uint8 soulCount;
        uint poolId;
        uint[] memory ids;
        uint[] memory counts;
     ...
     *  一键mint
     **/
    function oneKeyCast(uint8 theme_, uint8 group_, uint soulCount_, address recommender_) public{
        address player = _msgSender();
        require(isOpenCast, "not open");
        require(recommender_ != player, "not own addr");
        require(theme_ == 1 && (group_ == 1 || group_ == 2), "inv param");
        ...
        uint amount = getBuildCount();

        cSoulController.divide(player, recommender_, address(0), 1);

        //新池子  10111 - 10115
        uint poolId = getPoolId(theme_, group_, uint8(soulCount_ /5 + 11));

        uint tokenId = cSoulController.incrTokenId();
        cSoul721.mint(player, tokenId);
        cSoul1155.mint(address(cSoulStore), tokenId, amount);

        cSoulStore.createBox(tokenId, amount, poolId);
        emit Cast(player, recommender_, amount, poolId);
    }
...
interface ISoulBonus{
    function addBonus(uint time, uint add) external;
}
