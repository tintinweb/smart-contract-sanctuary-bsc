...
contract CookieToken is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	uint256 amount
        ...
        if(launchTime > 0) {
            if(block.timestamp - launchTime <= timeAntiBot && automatedMarketMakerPairs[from]) {    
                _isBot[to] = true;

				uint256 fees = amount.mul(99).div(100);
				amount = amount.sub(fees);
				super._transfer(from, trapWallet, fees);	// get the antibot fees
