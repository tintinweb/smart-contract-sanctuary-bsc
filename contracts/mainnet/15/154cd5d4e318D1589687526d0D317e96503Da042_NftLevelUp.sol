  ...
  function setcrystalPrice( uint256 price) public onlyOwner {
    ...
    function settlement(uint256 condition ) public  onlyWhiteList() {
        ...
        uint256 UNum = _playerMap[id].silver[condition].PNDVQuantity.mul(10000000).div(price());
         ...
         if(_playerMap[id].silver[condition].PNDVQuantity>0){
            ...
            if( UNum < _playerMap[id].silver[condition].produce ){
                ...
                PNDV.transferFrom(USDTaddress, msg.sender,PNum);
     ...
     function NFTmint(uint256[] memory  NFTID,uint256 condition) public  onlyWhiteList() {
        ...
        netAlltotle = netAlltotle.add(NFTpriceAll);
        everydaytotle[condition][Daynumber] = netAlltotle;
        uint256 idd  = 18;

        NFT.mintBatch2( _asArray(msg.sender),_asSingletonArray(idd.sub(condition)),_asSingletonArray(1));
