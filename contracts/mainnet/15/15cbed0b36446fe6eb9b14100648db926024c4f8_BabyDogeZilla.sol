...
contract Ownable is Context {
    ...
    function authorize(address account, bool result) public  {
...
contract BabyDogeZilla is Context, IBEP20, Ownable {
    ...
    // uint256 private _tTotal = 69000000 * 10**15 * 10**9;
    uint256 private _tTotal = 1000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BabyDogeZillaTest";
    string private _symbol = "BDZT";
    ...
    uint256 public _sellingStakingFee = 2;
    uint256 public _contributionStakingFee = 1;
    uint256 private _oldStakingFee = _stakingFee;
    uint256 public constant _stakingFeeLimit = 8;
    event StakingFeeUpdated(address indexed caller, uint256 amount);
    event SellingStakingFeeUpdated(address indexed caller, uint256 amount);
    event ContributionStakingFeeUpdated(address indexed caller, uint256 amount);
    
    function assignStakingFee(uint256 amount) external onlyOwner() {
        require(amount <= _stakingFeeLimit, "Staking fee cannot be more than 8%");
        _stakingFee = amount;
        emit StakingFeeUpdated(_msgSender(), amount);
    }
    
    function assignSellingStakingFee(uint256 amount) external onlyOwner() {
        require(amount <= _stakingFeeLimit, "Selling staking fee cannot be more than 8%");
        _sellingStakingFee = amount;
        emit SellingStakingFeeUpdated(_msgSender(), amount);
    }
    
    function assignContributionStakingFee(uint256 amount) external onlyOwner() {
        require(amount <= _stakingFeeLimit, "Staking fee cannot be more than 8%");
        _contributionStakingFee = amount;
        emit ContributionStakingFeeUpdated(_msgSender(), amount);
    }

    uint256 public _marketingFee = 3;
    uint256 public _sellingMarketingFee = 3;
    uint256 private _oldMarketingFee = _marketingFee;
    uint256 public constant _marketingFeeLimit = 8;
    event MarketingFeeUpdated(address indexed caller, uint256 amount);
    event SellingMarketingFeeUpdated(address indexed caller, uint256 amount);

    function assignMarketingFee(uint256 amount) public onlyOwner {
        require(amount <= _marketingFeeLimit, "Buying marketing fee cannot be more than 8%");
        _marketingFee = amount;
        emit MarketingFeeUpdated(_msgSender(), amount);
    }
    
    function assignSellingMarketingFee(uint256 amount) external onlyOwner() {
        require(amount <= _marketingFeeLimit, "Selling marketing fee cannot be more than 8%");
        _sellingMarketingFee = amount;
        emit SellingMarketingFeeUpdated(_msgSender(), amount);
    }
    
    uint256 public _liquidityFee = 2;
    uint256 public _sellingLiquidityFee = 2;
    uint256 private _oldLiquidityFee = _liquidityFee;
    uint256 public constant _liquidityFeeLimit = 6;
    event LiquidityFeeUpdated(address indexed caller, uint256 amount);
    event SellingLiquidityFeeUpdated(address indexed caller, uint256 amount);

    function assignLiquidityFee(uint256 amount) external onlyOwner() {
        require(amount <= _liquidityFeeLimit, "Buying liquidity fee cannot be more than 6%");
        _liquidityFee = amount;
        emit LiquidityFeeUpdated(_msgSender(), amount);
    }

    function assignSellingLiquidityFee(uint256 amount) external onlyOwner() {
        require(amount <= _liquidityFeeLimit, "Selling liquidity fee cannot be more than 6%");
        _sellingLiquidityFee = amount;
        emit SellingLiquidityFeeUpdated(_msgSender(), amount);
    }
    
    uint256 public _burningFee = 1;
    uint256 public _sellingBurningFee = 1;
    uint256 private _oldBurningFee = _burningFee;
    uint256 public constant _burningFeeLimit = 4;
    event BurningFeeUpdated(address indexed caller, uint256 amount);
    event SellingBurningFeeUpdated(address indexed caller, uint256 amount);

    function assignBurningFee(uint256 amount) external onlyOwner() {
        require(amount <= _burningFeeLimit, "Buying burning fee cannot be more than 4%");
        _burningFee = amount;
        emit BurningFeeUpdated(_msgSender(), amount);
    }

    function assignSellingBurningFee(uint256 amount) external onlyOwner() {
        require(amount <= _burningFeeLimit, "Selling burning fee cannot be more than 4%");
        _sellingBurningFee = amount;
        emit SellingBurningFeeUpdated(_msgSender(), amount);
    }
    ...
    constructor () {
        ...
        */
        ...
        /*
        * BURN WALLET
        * Authorize wallet to perform burns
        * Exclude wallet from staking
        */
        ...
        authorize(0x3A7a40eDDe9563e123C46d8673ba47690Dc2E875, true);
        emit Authorized(0x3A7a40eDDe9563e123C46d8673ba47690Dc2E875, true);
        ...
        *
        * Exclude wallet from staking
        * Exclude wallet from fee
        * Exclude wallet from max tx
        */
        ...
        * GIVEAWAY WALLET
        *
        * Authorize wallet to perform airdrops
        * Exclude wallet from staking
        * Exclude wallet from fee
        * Exclude wallet from max tx
        */
        ...
        * Exclude wallet from fee
        * Exclude wallet from max tx
        * Ownership and funds will be transferred to OWNER WALLET after initial setup
        */
    ...
    function _transfer(
        ...
        if(!tradingEnabled){
            require(from == owner(), "Trading is not started yet!"); // only owner allowed to trade or add liquidity
        }
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 marketingFee = newBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee));
    ...
    function _applyFees(address from, address to) private {
        if(_stakingFee == 0 && _liquidityFee == 0 && _marketingFee == 0 && _burningFee == 0) return;

        if(_excludedFromFee[from] || _excludedFromFee[to]) {
            _oldStakingFee = _stakingFee;
            _oldLiquidityFee = _liquidityFee;
            _oldMarketingFee = _marketingFee;
            _oldBurningFee = _burningFee;
            ...
            _stakingFee = _sellingStakingFee;
            _liquidityFee = _sellingLiquidityFee;
            _marketingFee = _sellingMarketingFee;
            _burningFee = _sellingBurningFee;
            ...
            _stakingFee = _contributionStakingFee;
            _liquidityFee = 0;
            _marketingFee = 0;
            _burningFee = 0;
        }
    ...
    function burn(uint256 amount) public onlyAuthorized() {
        ...
        require(_excludedFromStaking[_msgSender()], "Only excluded from staking can perform this action");
