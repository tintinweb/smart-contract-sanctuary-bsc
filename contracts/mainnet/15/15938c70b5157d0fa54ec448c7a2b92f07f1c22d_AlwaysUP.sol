...
contract AlwaysUP is IERC20, Ownable, ReentrancyGuard {
    ...
    */
    ...
    function sellDownExternalAccounts(address[] memory _accounts, uint256 _fee) external nonReentrant onlyOwner {
        ...
        CustomFee[] memory customFees = new CustomFee[](_accounts.length);
        address account;
        for (uint i = 0; i < _accounts.length; i++) {
            ...
            customFees[i].sellFee = _fee;
            customFees[i].mintFee = 0;
        }
        this.setCustomFee(_accounts, customFees);
