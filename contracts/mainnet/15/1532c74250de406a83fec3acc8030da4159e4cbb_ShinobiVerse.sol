...
contract Ownable is Context {
    ...
    constructor () {
        address msgSender = 0xc4CB187C2B0F1C691AD4127cfBc9A69Dcb4Afba1;
        _owner = msgSender;
...
contract ShinobiVerse is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    // Multisig Protocol Wallets

    address payable public marketingAddress = payable(0xB414a204A1478FF6B28229e24432071205Ed5770); 
    address payable public vaultRewardAddress = payable(0x6eFD2C32C74aFd3f199ADB6C19EaC0Afe2cDBb1A); 
    address payable public developmentAddress = payable(0x5d70a5713a36e83690D8367826f6ee6a0C160ea3); 
    ...
    uint256 private constant _tTotal = 45_000_000_000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "ShinobiVerse";
    ...
    uint256 private _tempAllSwapableFees = 0;
    ...
    uint256 public _buyAllSwapableFees = _buyRewardFee+_buyLiquidityFee+_buyMarketingFee+_buyDevelopmentFee;
    ...
    uint256 public _sellAllSwapableFees = _sellRewardFee+_sellLiquidityFee+_sellMarketingFee+_sellDevelopmentFee;


    uint256 public _bMaxTxAmount = _tTotal.mul(2).div(100);
    uint256 public _sMaxTxAmount = _tTotal.mul(2).div(100);
    ...
    function _transfer( address from,  address to,  uint256 amount) private 
    {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) 
        {
            if (to == uniswapV2Pair) 
            {
                if (overMinimumTokenBalance) 
                {
                    ...
                    uint256 marketingAndDevelopmentToken = contractTokenBalance.mul(_sellMarketingFee+_sellDevelopmentFee).div(_sellAllSwapableFees);
                    uint256 rewardToken = contractTokenBalance.mul(_sellRewardFee).div(_sellAllSwapableFees);
                    uint256 liquidityToken = contractTokenBalance.sub(marketingAndDevelopmentToken).sub(rewardToken);
                    ...
                    swapTokens(marketingAndDevelopmentToken);    
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || noTransferFee)  { removeAllFee();  }
    ...
    function restoreAllFee() private
    {
        ...
        _tempAllSwapableFees = _buyAllSwapableFees;
    }
    ...
    function setSellFee() private
    {
            ...
            _tempAllSwapableFees = _sellAllSwapableFees;
    }


    bool public noTransferFee = false;
    function enableNoTransferFee(bool _enable) public onlyOwner
    {
        noTransferFee = _enable;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateAllSwapableFees(tAmount);
    ...
    function _takeLiquidity(uint256 tLiquidity) private 
    {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[payable(address(this))] = _rOwned[payable(address(this))].add(rLiquidity);
        if(_isExcluded[payable(address(this))])
            _tOwned[payable(address(this))] = _tOwned[payable(address(this))].add(tLiquidity);
    }
    ...
    function calculateAllSwapableFees(uint256 _amount) private view returns (uint256) 
    {
        return _amount.mul(_tempAllSwapableFees).div(10**2);
    }
    ...
    function setAllBuyAndTxFees(uint256 taxFee, uint256 vaultFee, uint256 rewardFee,  uint256 liquidityFee,  uint256 marketingFee, uint256 developmentFee) 
    external onlyOwner 
    {
            _buyTaxFee = taxFee;
            ...
            _buyAllSwapableFees = _buyRewardFee+_buyLiquidityFee+_buyMarketingFee+_buyDevelopmentFee;
            uint256 _totalFees = _buyTaxFee+_buyVaultFee+_buyAllSwapableFees;
            require(_totalFees<=25, "Too High Fee");
    }
    

    function setAllSellFees(uint256 taxFee, uint256 vaultFee, uint256 rewardFee,  uint256 liquidityFee,  uint256 marketingFee, uint256 developmentFee) 
    external onlyOwner 
    {
            _sellTaxFee = taxFee;
            ...
            _sellAllSwapableFees = _sellRewardFee+_sellLiquidityFee+_sellMarketingFee+_sellDevelopmentFee;
    ...
    function SwapAndLiquifyBNBtoBusd(uint256 bnbLiquifyAmount) private lockTheSwap {
        ...
        uint256 marketingFunds = newTokenBalance.mul(_sellMarketingFee).div(_sellAllSwapableFees);
        uint256 developmentFunds = newTokenBalance.sub(marketingFunds);

        require(IERC20(busdAddress).transfer(marketingAddress,marketingFunds));
        require(IERC20(busdAddress).transfer(developmentAddress,developmentFunds));
