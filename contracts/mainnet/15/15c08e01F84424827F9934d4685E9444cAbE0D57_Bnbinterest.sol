...
contract Bnbinterest {
    ...
    function getBoxesByDepositAmount(uint8 plan, uint256 depositedAmount) public pure returns(uint256) {
        if(depositedAmount >= NFT_BOXES_STEP) {
            ...
            if(plan == 3) {
                uint256 boxes =  depositedAmount.div(NFT_BOXES_STEP); // 1 box per 1000 BNB
            ...
            if(plan == 4) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(2); // 2 boxes per 1000 BNB
            ...
            if(plan == 5) {
                uint256 boxes = depositedAmount.div(NFT_BOXES_STEP).mul(3); // 3 boxes per 1000 BNB
