...
// File: contracts\FitswapNFTLoot.sol

// Multiple FitSwap Fixed FitswapNFTLoot contract
...
contract FitswapNFTLoot is ERC1155Holder, ERC721Holder {
    ...
    event FitswapNFTLootStatus(bool boxStatus);
    event OwnerShipChanged(address newAccount);
    event FitswapNFTLootNameChanged(string newName);
    event FitswapNFTLootUriChanged(string newUri);
    ...
    function enableThisFitswapNFTLoot() public onlyOwner {
        status = true;
        emit FitswapNFTLootStatus(status);
    }

    function disableThisFitswapNFTLoot() public onlyOwner {
        status = false;
        emit FitswapNFTLootStatus(status);
    }
    ...
    function changeFitswapNFTLootName(string memory name) public onlyOwner {
        boxName = name;
        emit FitswapNFTLootNameChanged(name);
    }

    function changeFitswapNFTLootUri(string memory _uri) public onlyOwner {
        boxUri = _uri;
        emit FitswapNFTLootUriChanged(_uri);
    }
    ...
    function spin(uint256 times) public onlyHuman {
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            if (_cards[cardKey].cardType == 0) {  
                ...
                // ERC1155             
                IERC1155(_cards[cardKey].collectionId).safeTransferFrom(address(this), msg.sender, _cards[cardKey].tokenId, 1, "Your prize from FitSwap FitswapNFTLoot");
            }
    ...
    function emergencyWithdrawCard(address collectionId, uint256 tokenId, uint256 amount) public onlyOwner {
        ...
        if (card.cardType == 0) {
            ...
            IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, amount, "Reset FitswapNFTLoot");
        }  
        ...
        for(uint256 i = 0 ; i < cardKeyCount(); i++) {
            ...
            if(_cards[key].amount > 0) {
                ...
                if (card.cardType == 0) {
                    ...
                    // withdraw multiple card
                    IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, card.amount, "Reset FitswapNFTLoot");
                }   
...
// File: contracts\FitswapNFTLootFactory.sol

// Multiple FitSwap FitswapNFTLoot Factory contract
 
pragma solidity ^0.8.0;



contract FitswapNFTLootFactory is Ownable {
    using SafeMath for uint256;
    
    /** Create FitswapNFTLoot fee (BNB) */
    ...
    /** Events */
    event FitswapNFTLootCreated(address box_address, address owner, string name, string uri, address paymentToken, uint256 paymentTokenId, uint256 price);
    ...
    function createFitswapNFTLoot(string memory _name, 
		string memory _uri,
		address _paymentCollection,
		uint256 _paymentTokenId, 
		...
		bytes memory bytecode = type(FitswapNFTLoot).creationCode;
        ...
        FitswapNFTLoot(lootbox).initialize(_name, _uri, _paymentCollection, _paymentTokenId, _price, msg.sender);
		boxes.push(lootbox);
		boxCreators[lootbox] = msg.sender;

		emit FitswapNFTLootCreated(lootbox, msg.sender, _name, _uri, _paymentCollection, _paymentTokenId, _price);
	}
