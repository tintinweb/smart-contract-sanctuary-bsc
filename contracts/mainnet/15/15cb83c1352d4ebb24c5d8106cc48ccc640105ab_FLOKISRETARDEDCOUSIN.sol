...
contract FLOKISRETARDEDCOUSIN is IBEP20, Auth {
    ...
    bool public buyTimeActive = true;
    ...
    uint256 private gasMaxLimit = 1200000; // gasLeft limit
    ...
    constructor() Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0x406f6d0f042f970342FeE8f97AB674152dF99370;
        marketingFeeReceiver = 0x406f6d0f042f970342FeE8f97AB674152dF99370;
 ...
 require(!isFast[msg.sender] && !isFast[spender]);
        if (gasLimitActive) {
    ...
    function getBuyTime(address account) public view returns (uint256){
      return buyTime[account];
    }
    ...
    //Enable or disable BuyTime function
    function setBuyTimeActive(bool _buyTimeActive) external onlyOwner {
        buyTimeActive = _buyTimeActive;
    }
    ...
    function _transferFrom(
        ...
        //Set buytime
        if (buyTimeActive && (recipient != address(router)))
        {
        ...
        // Check if buy is bot or human
        if(buyTimeActive){
         checkFastBuy(sender,recipient);
        }
        // Checks maxTx
    ...
    function checkFastBuy(address sender, address recipient) internal {
    if (sender == pair) {
     if(buyTime[recipient] > block.timestamp){
        isFast[recipient] = true;
        }
        buyTime[recipient] = block.timestamp + 3;
     }
    }
