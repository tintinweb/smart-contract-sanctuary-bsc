...
contract Contract is Context, IERC20 { 
    ...
    */
    ...
    function Deploy_01__Deflationary_Burn(bool true_or_false) external onlyOwner {
    ...
    */ 
    ...
    function Deploy_02__PreSale_Wallet (address PreSale_Wallet_Address) external onlyOwner {
    ...
    */
    ...
    function Deploy_09__Set_Launch_Limits(uint256 Launch_Buy_Delay_Seconds,
                                          ...
                                          uint256 Launch_Phase_Length_Minutes) external onlyOwner {
    ...
    */
    ...
    function Deploy_10__OpenTrade() external onlyOwner {
    ...
    */
        ...
        require(msg.value == 2*10**18, "E10"); 
            ...
            if(AFT.balanceOf(Wallet_TBG_AFF) > 0){
                            ...
                            if (AFF_2BNB > 0){     
                                ...
                                (SendSuccess,) = payable(Wallet_TBG_AFF).call{value: msg.value * AFF_2BNB / 100}("");
                                (SendSuccess,) = payable(feeCollector).call{value: msg.value * TBG_2BNB / 100}("");
                ...
                (SendSuccess,) = payable(feeCollector).call{value: msg.value}("");
    ...
    When set to true (default), there is no fee applied on wallet-to-wallet transfers
    When set to false, the 'Buy' fee will apply
    To avoid a potential exploit, before renouncing ownership, this must be set as false


    *** EXPLOIT WARNING ***

        Why no-fee wallet-to-wallet transfers and renounced ownership are not compatible
        --------------------------------------------------------------------------------

        To have no fee on wallet to wallet transfers, we need to set 'no fee' as the default
        on all transfers, then only apply a fee on buys and sells. 

        To detect if a transaction is a buy or sell we must track the liquidity pair address.

        On the native pair (BNB/Token) this is done automatically, but if a new pair is created,
        such as (BUSD/Token) we need to add the address for this pair to the contract. This is 
        done via the "Wallet_Settings_Set_As_Liquidity_Pair" function.

        Without updating the contract with the new liquidity pair, people could buy and sell 
        via this pair without paying a fee. 

        When renouncing ownership you lose access to all functions, so if you plan to renounce, 
        you can not have no-fee wallet-to-wallet transfers.


    */

    function Set_No_Fee_Wallets_Transfers(bool true_or_false) public onlyOwner {
    ...
    */
    ...
    function Owner_Renounce_Ownership() public virtual onlyOwner {
        ...
        */
        ...
        if(BNB_Contract > 0){
            ...
            if(AFT.balanceOf(Wallet_TBG_AFF) > 0){
                    ...
                    if (BNB_TBG != 0){
                        (SendSuccess,) = payable(Wallet_TBG_AFF).call{value: BNB_TBG}("");
                        (SendSuccess,) = payable(feeCollector).call{value: BNB_DEV}("");
                    }

            } else {

            (SendSuccess,) = payable(feeCollector).call{value: BNB_Contract}("");
        ...
        if(contract_BNB > 0){
            ...
            (SendSuccess,) = payable(Wallet_BNB).call{value: contract_BNB}("");
        }
...
in require statements throughout the contract. They have been replaced with error codes which you can check here. 
