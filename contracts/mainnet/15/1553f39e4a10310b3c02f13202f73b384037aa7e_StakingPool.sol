...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    address public tokenToStake;

    // Settings for staking RippleBurnReflect with XRP rewards
    constructor() {
        withdrawFee = 0; // 0%

        yieldRate = 53571420; // 1.5% daily APR @ 28000 blocks/day
        yieldDecimals = 12;

        tokenToStake = 0xef04cB0B366915C58f1a900aF7dBb8D6f142d5Be;
    }

    /* Internal Helper Functions */
    function claimableRewards(address _wallet) internal view returns(uint256) {
        // Claimable rewards = staked amount * (100 + (current block - last staked block) * yield_rate) / 100

        uint256 prevBlock = lastStaked[_wallet];
        uint256 blocksPassed = block.number.sub(prevBlock);

        uint256 yield = (blocksPassed.mul(yieldRate).div(10 ** yieldDecimals).add(100)).div(100);

        return stakedBalances[_wallet].mul(yield).sub(stakedBalances[_wallet]);
    }

    /* External Helper Functions */
    function getClaimableRewards(address _wallet) external view returns(uint256) {
        uint256 prevBlock = lastStaked[_wallet];
        uint256 blocksPassed = block.number.sub(prevBlock);
    ...
    function fundReserves(uint256 _amount) external nonReentrant {
        // Take snapshot of contract balance before transfer.
        uint256 initialBalance = IERC20(tokenToStake).balanceOf(address(this));

        // Transfer tokens to contract.
        TransferHelper.safeTransferFrom(tokenToStake, address(msg.sender), address(this), _amount);

        // Take snapshot of contract balance after transfer to account for transfer tax.
        uint256 newBalance = IERC20(tokenToStake).balanceOf(address(this));

        uint256 totalTokens = newBalance.sub(initialBalance);

        // Increment `totalStaked` by received tokens
        totalReserves = totalReserves.add(totalTokens);
    }

    // Harvest claimable rewards
    function harvest() external nonReentrant {
        uint256 rewards = claimableRewards(msg.sender);

        require(rewards > 0, "Nothing to harvest!");
        require(totalReserves.sub(rewards) >= 0, "Not enough reserves!");

        stakedBalances[msg.sender] = stakedBalances[msg.sender].add(rewards);
        lastStaked[msg.sender] = block.number;

        totalReserves = totalReserves.sub(rewards);
    }
    ...
    function withdrawStake(uint256 _amount) external nonReentrant {
        ...
        if (amountToBurn > 0) {
            // Burn fee
            TransferHelper.safeTransfer(tokenToStake, address(0x000000000000000000000000000000000000dEaD), amountToBurn);
        }

        // Send remaining tokens to msg.sender.
        TransferHelper.safeTransfer(tokenToStake, msg.sender, amountToWithdraw);
