...
contract SUShareRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public suSharePerSecond = 0.0008127861 ether; 
    ...
    constructor(address _sushare, uint256 _poolStartTime) public {
        ...
        suSharePerSecond = TOTAL_REWARDS.div(runningTime);
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "SUShareRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOperatorOrTreasury() {
        require(
            ...
            "SUShareRewardPool: caller is not the operator/treasury"
        );
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(
                ...
                "SUShareRewardPool: existing pool?"
            );
        }
    }
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(suSharePerSecond);
            return poolEndTime.sub(_fromTime).mul(suSharePerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(suSharePerSecond);
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _sushareReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accSUSharePerShare = accSUSharePerShare.add(
                _sushareReward.mul(1e18).div(tokenSupply)
            );
        }
        return
            user.amount.mul(accSUSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accSUSharePerShare = pool.accSUSharePerShare.add(
                _sushareReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                .amount
                .mul(pool.accSUSharePerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (_pending > 0) {
                _safeSUShareTransfer(msg.sender, _pending);
    ...
    function _withdraw(
        ...
        uint256 _pending = user
            ...
            .sub(user.rewardDebt);
        if (_pending > 0) {
            _safeSUShareTransfer(_account, _pending);
    ...
    function _safeSUShareTransfer(address _to, uint256 _amount) internal {
        ...
        onlyOperatorOrTreasury
    {
        ...
        uint256 _oldRate = suSharePerSecond;
        massUpdatePools();
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 180 days) {
            // do not allow to drain core token (SUShare or lps) if less than 180 days after pool ends
