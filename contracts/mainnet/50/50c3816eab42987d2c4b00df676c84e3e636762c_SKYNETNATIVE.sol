...
contract SKYNETNATIVE is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    SKYNETNATIVEDividendTracker public dividendTracker;
    ...
    constructor() BEP20("SKYNET NATIVE", "SKYNET") 
    {
        dividendTracker = new SKYNETNATIVEDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "SKYNETNATIVE: The dividend tracker already has that address");
        SKYNETNATIVEDividendTracker newDividendTracker = SKYNETNATIVEDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "SKYNETNATIVE: The new dividend tracker must be owned by the SKYNETNATIVE token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SKYNETNATIVE: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SKYNETNATIVE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SKYNETNATIVE: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SKYNETNATIVE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SKYNETNATIVE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SKYNETNATIVE: Cannot update gasForProcessing to same value");
...
contract SKYNETNATIVEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SKYNETNATIVE_Dividend_Tracker", "SKYNETNATIVE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SKYNETNATIVE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SKYNETNATIVE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SKYNETNATIVE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SKYNETNATIVE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SKYNETNATIVE_Dividend_Tracker: Cannot update claimWait to same value");
