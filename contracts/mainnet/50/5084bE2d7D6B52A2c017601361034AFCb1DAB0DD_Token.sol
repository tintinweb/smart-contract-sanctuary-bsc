...
contract Token is IERC20, Ownable {
    ...
    string private _name = "Marbleheroes Token Sub";
    string private _symbol = "MBHS";
    uint8 private _decimals = 18;
    uint256 private _totalSupply = 550000000 * 10**18;
    ...
    function setDividendExempt(address account, bool state) public onlyOwner {
        isDividendExempt[account] = state;
    }
    ...
    function _transfer(
        ...
        if(!isDividendExempt[fromAddress] && !_marketPairs[from]) setShare(fromAddress);
        if(!isDividendExempt[toAddress] && !_marketPairs[to]) setShare(toAddress);
        
        fromAddress = from;
        toAddress = to;  
        if(_balances[address(this)] >= 10 && from != address(this) && LPRewardLastSendTime.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(shareholders[currentIndex])).div(lpTotalSupply);
    ...
    function _tokenTransfer(
        ...
        if(takeFee) {                      
            if(_marketPairs[sender]) {
                ...
                uint256 taxAmount = _takeTaxFee(recipient, buyTaxFee, amount);
                amountAfter = amountAfter.sub(taxAmount);

                uint256 marketAmount = _takeFee(recipient, marketAddress, marketFee, amount);
                ...
                uint256 taxAmount = _takeTaxFee(sender, sellTaxFee, amount);
                amountAfter = amountAfter.sub(taxAmount);

                uint256 devAmount = _takeFee(sender, devAddress, devFee, amount);
                amountAfter = amountAfter.sub(devAmount);
    ...
    function _takeTaxFee(
        ...
        if (tAmount == 0 || fee == 0) return 0; 

        feeAmount = tAmount.mul(fee).div(100);
    ...
    function _takeFee(
        ...
        feeAmount = tAmount.mul(fee).div(100);
        _balances[address(recipient)] = _balances[address(recipient)].add(feeAmount);
