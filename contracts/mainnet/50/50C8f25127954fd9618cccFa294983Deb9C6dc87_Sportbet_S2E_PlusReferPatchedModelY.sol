...
// File: contracts/SportbetPlusRefer_S2E_Patched.sol
...
contract Sportbet_S2E_PlusReferPatchedModelY is Ownable {
    ...
    constructor(IERC20 _chipToken, uint _earningsFeeBP, uint _losingsFeeBP, uint _referralRewardsFeeBP) {
        chipToken = _chipToken;
        earningsFeeBP = _earningsFeeBP;
        losingsFeeBP  = _losingsFeeBP;
    ...
    external onlyOwner {
        require(_aTeamOdds >= 1*earningsFeeDP && _aTeamOdds <= 10*earningsFeeDP && _drawOdds >= 1*earningsFeeDP && _drawOdds <= 10*earningsFeeDP && _bTeamOdds >= 1*earningsFeeDP && _bTeamOdds <= 10*earningsFeeDP, "createGambling: Odds should be between 1 and 10!"); ///Odds no more than 10 to control risk///
        require(_maxPoolSize >= 10 && _maxPoolSize <= 1000*1e9 , "createGambling: _maxPoolSize should be between 10 S2K and 4 BILLION S2K (ATTENTION: the decimal digits in this contract is 9)!");
        ...
        leagues[_leagueId].gamblings.push(Gambling({aTeam: _aTeam, bTeam: _bTeam, startTime: _startTime, bullseye: 99, stakes: [uint(0), 0, 0], isFinished: false, 
          ...
          maxPoolSize: _maxPoolSize*1e9    //Add the 9 decimals here for future use of the maxPoolSize limit
        }));
    }
    ...
    function drawLottery(uint _leagueId, uint _gamblingId, uint _ballot) external onlyOwner onlyNotFinished(_leagueId, _gamblingId) {
        ...
        /*//BURN A PERCENTAGE OF THE USERS' TOTAL LOSS IN THIS GAMBLING//*/
        //Note: losingStakesTotal = totalStakes - winnerStakes//
        uint losingStakesTotal = leagues[_leagueId].gamblings[_gamblingId].stakes[0] + leagues[_leagueId].gamblings[_gamblingId].stakes[1] + leagues[_leagueId].gamblings[_gamblingId].stakes[2] - leagues[_leagueId].gamblings[_gamblingId].stakes[_ballot] ; /////////USING 9 DECIMALS IN THIS CONTRACT/////////
        uint losingFee = losingStakesTotal.fromUint().mul(losingsFeeBP.fromUint()).div(earningsFeeDP.fromUint()).toUint();
        if (losingFee > 0) { chipToken.safeTransfer(nullAddress, losingFee); } //Did not use require, so losingFee could equal to zero here. Same for the fee in claimLottery.//
    }
    ...
    function createBet(uint _leagueId, uint _gamblingId, uint _target, uint _stake, address _referParent) external onlyNotFinished(_leagueId, _gamblingId) { // _referParent DEFAULT VALUE SHOULD BE undef IN JAVASCRIPT IF NO REFERER
        ...
        require(_stake >= 10*1e9, "createBet: Minimum Bet $10 in 9 decimals!"); /////////NEED A BETTER WAY TO MULTIPLY THE UINT8 DECIMALS/////////
    ...
    function claimBet(uint _leagueId, uint _gamblingId, uint _index) external {
        ...
        bet.reward = reward;
        if (fee > 0) { chipToken.safeTransfer(nullAddress, fee); }
