...
contract PopeyeInu is Context, ERC20, Ownable {
    using SafeMath for uint256;

    IPancakeSwapV2Router private _pancakeSwapV2Router;
    ...
    uint256 private constant _tSupply = 1e12 * (10**18);
    ...
    event SetBuyFee(uint256 buyMktgFee, uint256 buyDevFee, uint256 buyLiqFee);
	event SetSellFee(uint256 sellMktgFee, uint256 sellDevFee, uint256 sellLiqFee);
    
    constructor (address payable mktgWallet, address payable devWallet, address payable liqWallet) ERC20("Popeye Inu", "POP") {
        require(mktgWallet != ZERO, "mktgWallet address cannot be 0");
        require(devWallet != ZERO, "devWallet address cannot be 0");
        require(liqWallet != ZERO, "liqWallet address cannot be 0");

        _pancakeSwapV2Router = IPancakeSwapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        _approve(address(this), address(_pancakeSwapV2Router), _tSupply);
        _pancakeSwapV2Pair = IPancakeSwapV2Factory(_pancakeSwapV2Router.factory()).createPair(address(this), _pancakeSwapV2Router.WETH());
        IERC20(_pancakeSwapV2Pair).approve(address(_pancakeSwapV2Router), type(uint).max);

        _mktgWallet = mktgWallet;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        bool takeFee = true;
        TransactionType txType = (from == _pancakeSwapV2Pair) ? TransactionType.BUY : (to == _pancakeSwapV2Pair) ? TransactionType.SELL : TransactionType.TRANSFER;
        if (from != owner() && to != owner() && to != ZERO && to != DEAD && !_swapping) {
            require(!_isBlacklisted[from] && !_isBlacklisted[to], "Blacklisted.");

            if (cooldownEnabled) {
                if (to != address(_pancakeSwapV2Router) && to != address(_pancakeSwapV2Pair)) {
            ...
            if (txType == TransactionType.BUY && to != address(_pancakeSwapV2Router) && !_isExcludedMaxTransactionAmount[to]) {
            ...
            if (txType == TransactionType.SELL && from != address(_pancakeSwapV2Router) && !_isExcludedMaxTransactionAmount[from]) {
    ...
    function _swapBack(uint256 contractBalance) internal {
        ...
        uint256 bnbForMktg = bnbBalance.mul(_tokensForMktg).div(totalTokensToSwap);
        uint256 bnbForDev = bnbBalance.mul(_tokensForDev).div(totalTokensToSwap);
        uint256 bnbForLiq = bnbBalance.sub(bnbForMktg).sub(bnbForDev);
        
        _tokensForMktg = 0;
        _tokensForDev = 0;
        _tokensForLiq = 0;

        if(liquidityTokens > 0 && bnbForLiq > 0) _addLiquidity(liquidityTokens, bnbForLiq);

        (success,) = address(_devWallet).call{value: bnbForDev}("");
    ...
    function _sendBNBToFee(uint256 amount) internal {
    ...
    function setBlacklisted(address[] memory accounts, bool isBL) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
            if((accounts[i] != _pancakeSwapV2Pair) && (accounts[i] != address(_pancakeSwapV2Router)) && (accounts[i] != address(this))) _isBlacklisted[accounts[i]] = isBL;
        }
    }

    function setBuyFee(uint256 _buyMktgFee, uint256 _buyDevFee, uint256 _buyLiqFee) public onlyOwner {
        require(_buyMktgFee.add(_buyDevFee).add(_buyLiqFee) <= 120, "Must keep buy taxes below 12%");
        buyMktgFee = _buyMktgFee;
        buyDevFee = _buyDevFee;
        buyLiqFee = _buyLiqFee;
        emit SetBuyFee(buyMktgFee, buyDevFee, buyLiqFee);
    }

    function setSellFee(uint256 _sellMktgFee, uint256 _sellDevFee, uint256 _sellLiqFee) public onlyOwner {
        require(_sellMktgFee.add(_sellDevFee).add(_sellLiqFee) <= 120, "Must keep sell taxes below 12%");
        sellMktgFee = _sellMktgFee;
        sellDevFee = _sellDevFee;
        sellLiqFee = _sellLiqFee;
        emit SetSellFee(sellMktgFee, sellDevFee, sellLiqFee);
    }
    ...
    function unclog() public onlyOwner {
        uint256 contractBalance = balanceOf(address(this));
        _swapTokensForBNB(contractBalance);
    }
    
    function distributeFees() public onlyOwner {
        uint256 contractBNBBalance = address(this).balance;
        _sendBNBToFee(contractBNBBalance);
    }
