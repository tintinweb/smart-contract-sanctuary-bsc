...
♻️ AXERIUM CHAIN ♻️
 ...
 🚀 STEALTH LAUNCH 4th July 2022 at 14:00 - 14:30 Pm Utc

HUGE MARKETING BEFORE LAUNCH AND AFTER LAUNCH.
PREVIOUS PROJECT IS AWESOME

AXERIUM CHAIN
Public Blockchain and Smartcontract Platform That Enable Developers to Create Many Projects based on Blockchain technology

AXERIUM CHAIN is the fastest blockchain in the world and the fastest growing ecosystem in crypto, with thousands of projects spanning DeFi, NFTs, Web3 and more.

AXERIUM CHAIN is an open-source (permissionless) public blockchain protocol and smart contract platform that enables developers (startups, corporates, and businesses).

As the third generation blockchain protocol, Axerium is born to support DApps (Decentralized Applications), DeFi (Decentralized Finance) and many real use cases of blockchain applications for startups, businesses, and corporates. Axerium is going to make blockchain technology accessible and applicable to enhance various industries.


💎 TOKENOMIC 💎

Total Supply :  180.000.000
Token name :  AXERIUM CHAIN
Symbol          : AXERIUM
Decimal         : 18

Tax 8% for Buy and Sell
...
♻️ AXERIUM SWAP
http://swap.axeriumchain.com
(live)

♻️ AXERIUM DEFI STAKING
http://staked.axeriumchain.com
(live)

♻️ AXERIUM CHAIN  BETA TESTNET EXPLORER
http://betatestnet.axeriumchain.com/
(Live)

🔥 JOIN AXERIUM CHAIN COMMUNITY

👥 Telegram
https://t.me/axeriumchainnews

📣 Twitter
https://twitter.com/axeriumchain

🌏 Website 
https://www.axeriumchain.com
*/
...
contract Axerium_Chain is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingAddress =
        payable(0xd037db44b9c61642eF2dD5Bd27ea15DE237b1e55); // Marketing Address

    address payable public devAddress =
        payable(0xd037db44b9c61642eF2dD5Bd27ea15DE237b1e55); // Buyback Address

    address payable public liquidityAddress =
        payable(0xd037db44b9c61642eF2dD5Bd27ea15DE237b1e55); // Liquidity Address
    ...
    uint256 private constant _tTotal = 180000000 * 1e18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    bool public limitsInEffect = true;

    string private constant _name = "AXERIUM CHAIN";
    string private constant _symbol = "$AXERIUM";
    ...
    constructor() {
        ...
        maxTransactionAmount = _tTotal * 1000 / 1000; // 2% maxTransactionAmountTxn
    ...
    function setMinimumTokensBeforeSwapAmount(uint256 _amount) external onlyOwner {
    ...
    function clearStuckBNB(uint256 amountPercentage) external onlyOwner {
        ...
        payable(devAddress).transfer(amountBNB * amountPercentage / 100);
    }
    ...
    function _transfer(
     ...
     if(limitsInEffect){
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _teamTokensToSwap + _buyBackTokensToSwap;
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                removeAllFee();
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyTDFee + _buyBuyBackFee;
                buyOrSellSwitch = BUY;
            }
            // Sell
                ...
                removeAllFee();
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellTDFee + _sellBuyBackFee;
    ...
    function swapBack() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap.add(_teamTokensToSwap).add(_marketingTokensToSwap).add(_buyBackTokensToSwap);
        ...
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForTeam).sub(bnbForBuyBack);
        ...
        (success,) = address(buyBackAddress).call{value: bnbForBuyBack}("");
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _teamTokensToSwap += tLiquidity * _buyTDFee / _liquidityFee;
            ...
            _teamTokensToSwap += tLiquidity * _sellTDFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyDevFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBuyBackFee)
        external
        onlyOwner
    {
        ...
        require(_buyDevFee + _buyLiquidityFee + _buyMarketingFee + _buyBuyBackFee <= 30, "Must keep taxes below 30%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyDevFee);
    }

    function setSellFee(uint256 sellDevFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBuyBackFee)
        external
        onlyOwner
    {
        ...
        require(_sellDevFee + _sellLiquidityFee + _sellMarketingFee + _sellBuyBackFee <= 30, "Must keep taxes below 30%");
