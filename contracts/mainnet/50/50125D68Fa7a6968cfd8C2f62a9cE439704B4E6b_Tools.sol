...
contract Tools is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function disperseToken(address[] calldata recipients, uint256[] calldata values, IERC20Upgradeable token) external {
        uint total = 0;
    ...
    function disperseTokenSimple(address[] calldata recipients, uint256[] calldata values, IERC20Upgradeable token) external {
        for (uint256 i = 0; i < recipients.length; i++)
            require(token.transferFrom(msg.sender, recipients[i], values[i]));
    }
    
    function rainDrop(address[] calldata addresses, uint256[] calldata tokens, address _tokenAddress) external onlyOwner {
        ...
        for (uint i = 0; i < addrLength; i++) {
            tokenAddress.transferFrom(from, addresses[i], tokens[i]);
        }
    }

    function rainDropSimple(address[] calldata addresses, uint256[] calldata tokens, address _tokenAddress) external onlyOwner {
        address from = msg.sender;
        uint256 addrLength = addresses.length;
        uint256 tokenLength = tokens.length;
        if (addrLength != tokenLength) revert MismatchLength(addrLength, tokenLength);
