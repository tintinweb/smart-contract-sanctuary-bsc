...
contract WomensInu is IERC20, Context, Ownable {
    ...
    string private constant _name = "Womens Inu";

    uint256 public Winu_max_BuyLimit = 4000000 * 10**9;
    uint256 public Winu_max_SellLimit = 2000000 * 10**9;

    address private Winu_Only_Wallet = 0x4CB4532640BA2E92df505318792c76dAafe2db05;
    address private Winu_Team_Wallet = 0xbac6D1fa0ac599feeF7251865Bf4049C23a81639;

    uint256 public Winu_max_WalletLimit = 4000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Winu_Only_Wallet] = true;
        _isExcludedFromFee[Winu_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Winu_Only_Wallet] = true;
        allowedTransfer[Winu_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Winu_max_BuyLimit, "You are exceeding Winu_max_BuyLimit");
            require(balanceOf(to) + amount <= Winu_max_WalletLimit, "You are exceeding Winu_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Winu_max_SellLimit, "You are exceeding Winu_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Winu_max_WalletLimit, "You are exceeding Winu_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Winu_Only_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Winu_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Winu_Update_MarketingWallet(address newWallet) external onlyOwner{
        Winu_Only_Wallet = newWallet;
    }
    
    function Winu_Update_TeamWallet(address newWallet) external onlyOwner{
        Winu_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Winu_max_BuyLimit = maxBuy * 10**decimals();
        Winu_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Winu_Max_Walletlimit(uint256 amount) external onlyOwner{
        Winu_max_WalletLimit = amount * 10**decimals();
    }
    ...
    function reBNB(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient BNB balance");
        payable(msg.sender).transfer(weiAmount);
    }
    

    function reAnyBEP20Tokens(address _tokenAddr, address _to, uint _amount) public onlyOwner {
