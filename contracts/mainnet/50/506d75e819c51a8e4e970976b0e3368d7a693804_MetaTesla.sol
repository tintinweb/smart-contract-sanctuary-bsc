...
contract MetaTesla is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    MetaTeslaDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x01338D0D33D890F5884c5923883632768A9f5543);
    ...
    constructor() BEP20("MetaTesla", "mTESLA") 
    {
        dividendTracker = new MetaTeslaDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "MetaTesla: The dividend tracker already has that address");
        MetaTeslaDividendTracker newDividendTracker = MetaTeslaDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "MetaTesla: The new dividend tracker must be owned by the MetaTesla token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "MetaTesla: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "MetaTesla: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "MetaTesla: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MetaTesla: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MetaTesla: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MetaTesla: Cannot update gasForProcessing to same value");
...
contract MetaTeslaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MetaTesla_Dividend_Tracker", "MetaTesla_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "MetaTesla_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "MetaTesla_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MetaTesla contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MetaTesla_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MetaTesla_Dividend_Tracker: Cannot update claimWait to same value");
