...
contract TOURToken is ERC20, SafeOwnable {
    ...
    uint256 public guardFee=5;
    uint256 private guardPoolFeePercent = (guardFee* BASE_RATIO) / 100;
   ...
   modifier onlyManager() {
    ...
    function setGuardFee(uint256 fee) public onlyManager{
        guardFee=fee;
    }
          ...
          if(excludeFeeList[from]||excludeFeeList[to]){
              return amount;
               ...
               if (nftDividendAddress != address(0) && nftFee > 0) {
                ...
                if(guardPool&&liquidityList[to]){
                    burnFee=burnFee.add(amount.mul(guardPoolFeePercent).div(BASE_RATIO));
                }
                if (burnFee > 0) {
    ...
    function setGuadPool(bool _enabled) public onlyManager {
        guardPool = _enabled;
    }
