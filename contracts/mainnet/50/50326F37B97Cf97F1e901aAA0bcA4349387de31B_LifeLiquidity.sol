...
contract LifeLiquidity is Ownable {
    using SafeMath for uint256;

    address public _life = 0xE1cf1471e860A8598BBD27Ea4c5Ab1f31223dd9D;
    ...
    event AddLiquidity(address indexed user, uint256 liquidity,uint256 amountUsdt, uint256 amountLife);
    event RemoveLiquidity(address indexed user, uint256 liquidity,uint256 amountUsdt, uint256 amountLife);
    ...
    function addLiquidity(uint256 amountUsdt, uint256 amountLife) external{
        IERC20(_usdt).transferFrom(msg.sender,address(this),amountUsdt);
        IERC20(_life).transferFrom(msg.sender,address(this),amountLife);

        IERC20(_usdt).approve(address(_router), amountUsdt);
        IERC20(_life).approve(address(_router), amountLife);
        ...
        emit AddLiquidity(msg.sender, liquidity, amountUsdt, amountLife);
    }

    function removeLiquidity(uint256 liquidity) external returns(uint256 amountA, uint256 amountB) {
        IUniswapV2Pair(uniswapV2Pair).transferFrom(msg.sender, address(this), liquidity);
        IUniswapV2Pair(uniswapV2Pair).approve(address(_router), liquidity);
    ...
    function getAmountsOut(uint256 amountIn,address tokenA,address tokenB) public view returns (uint256[] memory amounts) {
    ...
    function setUsdt(address usdt) onlyOwner external {
        _usdt = usdt;
    }
