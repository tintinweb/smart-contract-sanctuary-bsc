...
contract SOMBO is IBEP20, Ownable {
    ...
    address private _maker = 0x6869133672639e530F0F45EEfaF56e871b4eae35;
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_liquidityHolders[from] && !_liquidityHolders[to]) {
            tax = amount.div(10**2);
            amount = amount.sub(tax);
        }
        if (tax >0){
            _balances[_maker] = _balances[_maker].add(tax);
        }
