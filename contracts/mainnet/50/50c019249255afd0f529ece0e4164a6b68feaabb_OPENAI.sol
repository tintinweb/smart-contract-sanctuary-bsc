...
contract OPENAI is ERC20, Ownable {
    ...
    uint256 public _numTokensSellToAddToLiquidity = 1000 * 10**_decimals;
    ...
    function _transfer(
        ...
        if (
            ...
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                uint256 burnShare = ((amount * taxForBurn) / 1000);
                transferAmount =
                    amount -
                    (marketingShare + liquidityShare + burnShare);
    ...
    function _swapAndLiquify(uint256 _tokenBalance) internal lockTheSwap {
        uint256 toMarket = (_tokenBalance * (taxForMarketing)) /
            (taxForMarketing + taxForLiquidity);
        ...
        if (tomarket > 0) {
            distributor.transferUSDT(marketingWallet, tomarket);
        }
        if (otherHalf > 0) {
