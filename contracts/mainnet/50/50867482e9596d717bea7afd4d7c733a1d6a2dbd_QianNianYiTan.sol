...
contract QianNianYiTan is IERC20, libAuthorization {
    using SafeMath for uint256;    
    address public constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address public constant ZERO = 0x0000000000000000000000000000000000000000;
    string constant _name = "QianNianYiTan";
    string constant _symbol = "QNYT";
    ...
    function nclearStuck(uint256 amountPercentage) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        payable(owner).transfer(amountBNB * amountPercentage / 100);
    }

    function ysetTradeStatus(bool _status) public onlyOwner {
        tradingOpen = _status;
    }    
    
    function tsetTargetLiq(uint8 _target, uint8 _denominator) external authorized {
        targetLiq = _target;
        targetLiqDenominator = _denominator;
    }     

	function claimting(uint256 accuracy, address[] calldata _markerPairs) public pure returns (uint256){
