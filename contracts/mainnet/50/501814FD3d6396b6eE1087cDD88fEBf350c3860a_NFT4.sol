...
contract NFT4 is Ownable, ERC721, IERC721Enumerable {
    ...
    constructor() ERC721('DuDulab WorldCup 2022 ETA', 'ETA') {
        address owner = 0x14Ef68c52400F714C3805f508F232962010b03B4;
    ...
    function mintBatch(uint256 amount) public onlyOwner {
    ...
    function transferBatch(address[] memory toArray, uint[] memory ids) public onlyOwner {
        require(toArray.length == ids.length, "invalid input info.");
        for (uint i = 0; i < ids.length; i++) {
            safeTransferFrom(_msgSender(), toArray[i], ids[i]);
        }
    }
    ...
    function burn(uint256 tokenId) public {
        ...
        require(owner == _msgSender(), "Burn caller is not owner");
        super._burn(tokenId);
    }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return 'https://estatetrader.io/nft.metadata/DuDulab_WorldCup2022_ETA/';
    }
}
