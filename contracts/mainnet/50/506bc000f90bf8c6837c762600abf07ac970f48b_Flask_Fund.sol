...
contract Flask_Fund is Base, Adressable, Math, Payable, AccessControl, With_DAORole, Terminatable, With_UpdaterRole, With_TradeManagerRole, With_TradeRole,
                          With_PWRToken, With_BUSDToken, With_WBNBToken, With_PancakeRouter, With_TradeManagerContract, With_DAOContract, I_Flask_Fund{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function setBaseWithdrawFees(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Flask_Fund: Denominator cannot be null");
    ...
    function getBNBForGasRefund(uint256 amountBNB) external onlyRole(UPDATER_ROLE){
        ...
        require(BUSDToken.balanceOf(thisAddr) >= amountBUSD, "Flask_Fund: BUSD reserve too small");
    ...
    function getFundBUSD() public view returns (uint256){
        ...
        nbBUSD += tradeManager.initBUSDBlanceInTrades();
        
        return nbBUSD;
    }

    function sendBUSDToTrade(I_Flask_Trade_Abstract trade, uint256 nbBUSD) external onlyRole(TRADE_MANAGER_ROLE){
    ...
    function buyPWR(uint256 amountBUSD) external onlyNotTerminated{
        require(BUSDToken.balanceOf(msg.sender) >= amountBUSD, "Flask_Fund: Not enough BUSD");
        
        if (DAO.investorWhitelistEnabled()){
            require(DAO.getUserData(msg.sender).isInvestor, "Flask_Fund: Investors have to be whitelisted");
        }
        
        uint256 nbPWRToMint = getPWRAmountFromBUSD(amountBUSD);

        require(nbPWRToMint > 0, "Flask_Fund: Too small amount of BUSD sent");
    ...
    function beforePWRTransfer(address from, address to, uint256 amount) external onlyRole(PWR_TOKEN_ROLE){
        ...
        require(nbFreePwrFrom >= amount, "Flask_Fund: Cannot transfer locked PWR tokens.");
    ...
    function sellPWR(uint256 amountPWR) external{
        ...
        require(nbFreePwr >= amountPWR, "Flask_Fund: Not enough PWR.");
        ...
        require(nbBUSDTowithdraw > 0, "Flask_Fund: Too small amount of PWR sent.");
        require(nbBUSDTowithdraw <= BUSDToken.balanceOf(thisAddr), "Flask_Fund: All BUSD are locked in trades.");
    ...
    function removeLockedPWRTokens(address addr, uint256 amountPWR) external onlyRole(WITHDRAWER_ROLE){
        ...
        require(nbLockedPwr >= amountPWR, "Flask_Fund: Not enough locked PWR.");
        require(nbFreePwr + nbLockedPwr >= amountPWR, "Flask_Fund: Not enough total PWR.");
    ...
    function getDataPWRLock(address addr) public view returns (uint256, uint256, uint256, uint256, uint256[2][] memory){
        ...
        uint256 l = tradeManager.getNbOpenRealTrades();
        
        uint256[2][] memory lockDatas = new uint256[2][](l);
        
        for(uint256 i=0; i<l; i++){
            (I_Flask_Trade_Manager.TradeData memory data, uint256 id) = tradeManager.getOpenRealTrade(i);
