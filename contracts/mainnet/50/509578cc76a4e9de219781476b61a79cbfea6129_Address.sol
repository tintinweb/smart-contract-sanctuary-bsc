...
contract HOMELESSCAT is Context, IERC20, Ownable {
    ...
    string private constant _name = "HOMELESS CAT";
    string private constant _symbol = "HOCT";
    ...
    uint256 public _sellStakingFee = 100;
    ...
    uint256 public _stakingTokensToSend;
    ...
    event SetBuyFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 devFee, uint256 stakingFee);
    
    event SetSellFee(uint256 marketingFee, uint256 liquidityFee, uint256 reflectFee, uint256 devFee, uint256 stakingFee);
    ...
    event UpdatedStakingAddress(address staking);
    ...
    constructor() {
        ...
        marketingAddress = payable(0xD1E7E3cc574B84d7C3829fAdeeD95F3C4bC43512); // Marketing Address
        devAddress = payable(0xa3C799A2a6F9cA5E7e0840fC09af09b762faA489);
        stakingAddress = payable(0xD1E7E3cc574B84d7C3829fAdeeD95F3C4bC43512); // Staking Address Placeholder
        
        excludeFromFee(newOwner);
        excludeFromFee(address(this));
        excludeFromFee(stakingAddress);
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyStakingFee + _buyLiquidityFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            } 
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellStakingFee + _sellLiquidityFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        if(_stakingTokensToSend > 0 && balanceOf(address(this)) >= _stakingTokensToSend) {
            _transfer(address(this), stakingAddress, _stakingTokensToSend);
        }
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyDevFee, uint256 buyStakingFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyDevFee + _buyStakingFee <= 1500, "Must keep buy taxes below 15%");
        emit SetBuyFee(buyMarketingFee, buyLiquidityFee, buyTaxFee, buyDevFee, buyStakingFee);
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellDevFee, uint256 sellStakingFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellDevFee + _sellStakingFee <= 3000, "Must keep sell taxes below 30%");
        emit SetSellFee(sellMarketingFee, sellLiquidityFee, sellTaxFee, sellDevFee, sellStakingFee);
    }
