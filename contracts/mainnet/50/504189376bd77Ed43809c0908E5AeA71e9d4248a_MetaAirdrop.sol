...
contract MetaAirdrop is AccessControlEnumerableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function newPending(address addr) external view returns (uint256 result) {
        if (claims[addr]) {
            uint40 nowMetaCount = IMeta(meta).accounts(addr).point;
            if (nowMetaCount > nowPoints[addr]) {
                result = (nowMetaCount - nowPoints[addr]) * newBack;
            }
        }
    }
    ...
    function setAccount(address addr, uint40 point) external {
        accounts[addr].point = point;
    }

    function clearClaim(address addr) external {
        claims[addr] = false;
        nowPoints[addr] = 0;
    }
}
