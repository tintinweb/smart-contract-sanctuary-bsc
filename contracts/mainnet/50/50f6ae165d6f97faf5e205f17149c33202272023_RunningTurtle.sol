...
contract MktCap is Ownable {
    ...
    constructor(address ceo_, address baseToken_, address router_, address initMarketingAddress) {
        ceo = ceo_;
        ...
        allot = Allot(2500, 0, 500, 3000);

        address[] memory marketingAddress = new address[](1);
        marketingAddress[0] = initMarketingAddress;
...
contract RunningTurtle is ERC20, ERC20Burnable, Ownable {
    using SafeMath for uint;
    MktCap public mkt;
    ...
    constructor() ERC20("RunningTurtle", "RunningTurtle") {
        ceo = _msgSender();
        ...
        address _marketingAddress = 0x300a8C17E64A4E5Db2e21F3E1f92C4A8c615e3e0;
        mkt = new MktCap(ceo, _baseToken, _router, _marketingAddress);
        _approve(address(mkt), _router, uint256(2**256-1));

        address _initHolder = 0x958531159abC913A2915ABf67651125d95284071;
        uint256 total = 100000000000000;
        ...
        doAddToWhiteList(address(this));
        doAddToWhiteList(_initHolder);
        doAddToWhiteList(ceo);
        doAddToWhiteList(_marketingAddress);
    ...
    function setOnlyWhiteListSwap(bool state) public {
        require(ceo == _msgSender(), "only CEO set only white list swap");
    ...
    function getWhiteListCount() public view returns (uint256) {
        return listWhiteList.length;
    }

    function doAddToWhiteList(address newAddress) private returns (bool) {
        require(!isInWhiteList(newAddress), "already in white list");

        mapWhiteList[newAddress] = true;
        listWhiteList.push(newAddress);
        return true;
    }

    function addToWhiteList(address newAddress) public returns (bool) {
        require(ceo == _msgSender(), "only CEO can add member to white list");
        return doAddToWhiteList(newAddress);
    }

    function removeFromWhiteList(address tgtAddress) public returns (bool) {
        require(ceo == _msgSender(), "only CEO can remove member from white list");
        require(isInWhiteList(tgtAddress), "not in white list");

        delete mapWhiteList[tgtAddress];
        uint256 countWL = listWhiteList.length;
        bool found = false;
        uint256 tgtIndex = 0;
        for(uint i = 0; i < countWL; i++) {
            if(listWhiteList[i] == tgtAddress) {
                tgtIndex = i;
                found = true;
            }
        }
        ...
        if(found) {
            listWhiteList[tgtIndex] = listWhiteList[countWL-1];
            listWhiteList.pop();
        }
