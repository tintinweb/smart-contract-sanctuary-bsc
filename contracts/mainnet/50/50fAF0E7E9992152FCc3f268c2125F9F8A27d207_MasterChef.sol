...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        bool governance; // mint xGov if true
    }

    // MetaVill TOKEN!
    IMetaVillToken public metaVillToken;
    // Governance Token
    xGovToken public xGov;
    // Dev address.
    address public dev1;
    address public dev2;
    // Deposit Fee address
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                if(pool.governance) {
                    xGov.mint(msg.sender, _amount.sub(depositFee)); // xGov mint
                }
            } else {
                user.amount = user.amount.add(_amount);

                if(pool.governance) {
                    xGov.mint(msg.sender, _amount);
                }
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(pool.governance) {
            xGov.burn(msg.sender, _amount); // if user transfer their xGov it causes error.
        }
        user.rewardDebt = user.amount.mul(pool.accTokenPerShare).div(1e12);
