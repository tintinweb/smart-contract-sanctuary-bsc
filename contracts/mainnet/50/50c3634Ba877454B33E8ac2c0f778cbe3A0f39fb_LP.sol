...
contract LP is Context, Ownable {
    ...
    function setKLNAddress(address tAddress) public onlyOwner {
        klnAddress = tAddress;
        kln = IERC20(tAddress);
    }

    function setQSQAddress(address tAddress) public onlyOwner {
        qsqAddress = tAddress;
        qsq = IERC20(tAddress);
    }
    ...
    function getUAmount(uint256 tAmount) public view returns (uint256) {
    ...
    function setKAmount(uint256 _kAmount) public onlyOwner {
        kAmount = _kAmount;
    } 

    function getTokenAmount(uint256 uAmount) public view returns (uint256) {
    ...
    function swapKLNForU(uint256 tAmount) public {
        require(
            klnAddress != address(0) && qsqAddress != address(0),
            "Not Set Token"
        );
        require(kAmount > 0 && kAmount >= tAmount, "No Enough KLN");
        ...
        if (kln.balanceOf(burnAddress) < 90000 * 10**18) {
            ...
            kln.transferFrom(_msgSender(), address(this), tAmount);
        }
        kAmount = kAmount.sub(tAmount);
    ...
    function swapUForQSQ(uint256 tokenAmount) private {
    ...
    function getKLN(address recipient, uint256 amount) public {
        ...
        uint256 tAmount = getTokenAmount(amount.div(2));
