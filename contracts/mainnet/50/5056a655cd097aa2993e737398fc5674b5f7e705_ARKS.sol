...
contract ARKS is IERC20, MinterRole {
   

    constructor() IERC20('CTY Constelary', 'CTY') {
        IUniswapV2Router02 _router = IUniswapV2Router02(0x0B0aa8f1c7d368508e1cb2BEc7283480CF723567); 
    ...
    address public marketingFeeReceiver = 0x3B4b42cF2F597117542Ab7a12CF1f44d1fd1A7F9;
    ...
    address public buyBackToken = address(0x4C3D5aF5c43dBecee525327E93d51eB4D6ddabec); 
        ...
        if (tradingOpen || sender == owner()) {
            if (
                ...
                if (transferTaxRate == 0 || _excludedFromFees[sender] || _excludedFromFees[recipient]) {
                    ...
                    require(amount == taxAmount + sendAmount, "CTY:: transfer: Tax value invalid");
 ...
 function setmkt(address _marketingFeeReceiver) public onlyOwner {
    ...
    function setExcludeFromFees(address _account, bool _excluded) public onlyOwner {
        require(_excludedFromFees[_account] != _excluded, "CTY: Account is already the value of 'excluded'");
    ...
    function updateCTYSWAPSwapRouter(address _router) public onlyOwner {
        ...
        require(routerPair != address(0), "CTY::updateCTYSWAPSwapRouter: Invalid pair address.");
    ...
    function setTxFees(uint256 _transactionFee) external onlyOwner{
        ...
        require(transferTaxRate <= MAX_TX_RATE, "CTY: Total fees can not be bigger than max value");
    }
    ...
    function setSellFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee, uint256 _burnFee) external onlyOwner{
        ...
        require(sellTaxRate <= MAX_TX_RATE, "CTY: Total fees can not be bigger than max value");
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CTY::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CTY::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "CTY::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CTY::getPriorVotes: not yet determined");
        uint32 nCheckpoints = numCheckpoints[account];
        if (nCheckpoints == 0) {
            return 0;
        }
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying CTYs (not scaled);
    ...
    function _writeCheckpoint( address delegatee, uint32 nCheckpoints, uint256 oldVotes, uint256 newVotes)
        internal
    {
        uint32 blockNumber = safe32(block.number, "CTY::_writeCheckpoint: block number exceeds 32 bits");
