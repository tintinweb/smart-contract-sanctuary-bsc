...
contract SeaOfColdBSC {
    ...
    constructor() {
        ...
        owner = msg.sender;                     //  
        PercsKey[10] = DivPercs(864000, 10);    // 10天 1%
        PercsKey[20] = DivPercs(1728000, 20);   // 20天 2%
        PercsKey[30] = DivPercs(2592000, 30);   // 30天 3%
        PercsKey[40] = DivPercs(3456000, 40);   // 40天 4%
        PercsKey[50] = DivPercs(4320000, 50);   // 50天 5%

        FeesKey[10] = FeesPercs(864000, 100);   // 10        
        FeesKey[20] = FeesPercs(1728000, 80);   // 20
        FeesKey[30] = FeesPercs(3456000, 50);   // 40
    ...
    function redeemInitial (uint256 num ) public {
        ...
        if (currDays < FeesKey[10].daysInSeconds){          // LESS THAN 10 DAYS STAKED ; 
            minusAmt = initialAmt.mul(FeesKey[10].feePercentage).div(percentdiv);  
            ...
            minusAmt = initialAmt.mul(FeesKey[20].feePercentage).div(percentdiv);  
            ...
            minusAmt = initialAmt.mul(FeesKey[30].feePercentage).div(percentdiv);  
            ...
            minusAmt = initialAmt.mul(FeesKey[40].feePercentage).div(percentdiv);  
