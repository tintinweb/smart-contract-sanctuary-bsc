...
import "./interfaces/IAvatarToken.sol";
...
contract AvatarTokenV2 is ERC20, Ownable,IAvatarToken {
    ...
    uint256 private _swapInterval = 30 minutes;
    ...
    function _takeFee(address _from,address _to, uint256 amount)
        private
        returns (uint256 amountAfter)
    {
        amountAfter = amount;
        uint256 LPFee;
        if(_to != uniswapV2Pair && _from != uniswapV2Pair){//transfer
            LPFee = amount.mul(transferFee).div(100);
        }else{
            LPFee = amount.mul(tradeFee).div(100);
        }
        if (LPFee > 0) {
            AmountLpFee = AmountLpFee.add(LPFee);
            super._transfer(_from, address(this), LPFee);
    ...
    function setLpReceiveWallet(address _address) external onlyOwner{
        lpReceiveWallet = _address;
    }
...
interface IAvatarToken is IERC20 {
