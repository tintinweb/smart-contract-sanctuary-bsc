...
// File: contracts/ JollyCakeDividendPayingToken.sol
pragma solidity ^0.7.6;
...
// File: contracts/ JollyCake.sol
pragma solidity ^0.7.6;
contract  JollyCake is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool private swapping;
     ...
     JollyCakeDividendTracker public dividendTracker;
    ...
    constructor() public ERC20("JollyCake", " JollyCake") {
        ...
        dividendTracker = new  JollyCakeDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), " JollyCake: The dividend tracker already has that address");

         JollyCakeDividendTracker newDividendTracker =  JollyCakeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), " JollyCake: The new dividend tracker must be owned by the  JollyCake token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), " JollyCake: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, " JollyCake: Account is already the value of 'excluded'");
   ...
   function blacklistAddress(address account, bool excluded) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, " JollyCake: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, " JollyCake: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, " JollyCake: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, " JollyCake: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], " JollyCake: This account cannot send tokens until trading is enabled");
        }
...
contract  JollyCakeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken(" JollyCake_Dividend_Tracker", " JollyCake_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, " JollyCake_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, " JollyCake_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main  JollyCake contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, " JollyCake_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, " JollyCake_Dividend_Tracker: Cannot update claimWait to same value");
