...
contract USDTMine {
	...
	uint256 constant public INVEST_MINE1 = 100e18;
	uint256 constant public INVEST_MINE2 = 500e18;
	uint256 constant public INVEST_MINE3 = 1000e18;
	uint256 constant public INVEST_MINE4 = 3000e18;
	uint256 constant public INVEST_MINE5 = 5000e18;
	uint256[] public REFERRAL_PERCENTS 	= [1000, 600, 400, 200, 200, 200, 200, 200, 200, 200];
	uint256[] public USDTMINE_PERCENTS 	= [500, 300, 200, 100, 100, 100, 50, 50, 50, 50];
	...
	struct User {
		...
		uint256 bonus;
		uint256 totalBonus;
		uint256 usdtmineincome;
		uint256 withdrawn;
		uint256 withdrawnusdtmine;
	}
	...
	event UsdtmineIncome(address indexed referrer, address indexed referral, uint256 indexed level, uint256 amount);
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(30, 400));
		plans.push(Plan(30, 410));
		plans.push(Plan(30, 417));
        plans.push(Plan(30, 433));
		plans.push(Plan(30, 466));
	}
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		require(plan == 0&&amounts == INVEST_MINE1 || plan == 1&&amounts == INVEST_MINE2 || plan == 2&&amounts == INVEST_MINE3 || plan == 3&&amounts == INVEST_MINE4 || plan == 4&&amounts == INVEST_MINE5, "Error invest");
        require(plan < 5, "Error plan");
	...
	function withdraw() public {
		...
		uint256 usdtmineAmount = getcurrentusdtmineincome(msg.sender);
		...
		totalAmount = totalAmount.add(usdtmineAmount);
		user.withdrawnusdtmine = user.withdrawnusdtmine.add(usdtmineAmount);
	...
	function getUserUsdtmineIncome(address userAddress) public view returns (uint256){
	
		uint256 totalUsdtmineAmount;
		uint256 usdtmineshare;
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			for (uint256 i = 0; i < downline.deposits.length; i++) {
				...
				if (downline.deposits[i].start < finish) {
					...
					uint256 to = finish < block.timestamp ? finish : block.timestamp;
					//usdtmine income
                    usdtmineshare = share.mul(USDTMINE_PERCENTS[level-1]).div(PERCENTS_DIVIDER);
					
					if (from < to) {
					
							totalUsdtmineAmount = totalUsdtmineAmount.add(usdtmineshare.mul(to.sub(from)).div(TIME_STEP));	
		...
		return totalUsdtmineAmount;		
	
	} 
	
	
	function getcurrentusdtmineincome(address userAddress) public view returns (uint256){
	    User storage user = users[userAddress];
	    return (getUserUsdtmineIncome(userAddress).sub(user.withdrawnusdtmine));
	    
	}
	
	function getUserTotalUsdtmineWithdrawn(address userAddress) public view returns (uint256) {
		return users[userAddress].withdrawnusdtmine;
	}
