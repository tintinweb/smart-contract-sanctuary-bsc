 ...
 * @dev Allows users to lock their tokens to participate in deals
 */
contract Locker is ILocker, AdminableUpgradeable {
    ...
    function __Locker_init(IERC20Upgradeable _blpToken, ITierCalculator _tierCalculator) public initializer {
        __Adminable_init();
    ...
    function withdraw(uint256 _amount) external override {
        ...
        if (feeAmount > 0) {
            blpToken.safeTransfer(owner(), feeAmount);
        }

        emit Withdraw(msg.sender, _amount, feeAmount, penaltyBP);
    }

    function transferToPool(address _poolAddress, uint256 _amount) external override {
        require(_amount <= userAmounts[msg.sender], "transferToPool: amount exceed user amount");

        userAmounts[msg.sender] = userAmounts[msg.sender].sub(_amount);

        blpToken.approve(_poolAddress, _amount);
        IStakingPool(_poolAddress).stakeFromLocker(_amount);

        emit TransferToPool(msg.sender, _poolAddress, _amount);
    }
...
interface IStakingPool {
    ...
    function stakeFromLocker(uint256 _amountToStake) external;
...
interface ILocker {
    ...
    /// @dev Transfer to staking pool
    /// @param _amount amount to transfer
    function transferToPool(address _poolAddress, uint256 _amount) external;
}
