 ...
 */
contract GemuniLottery is ReentrancyGuard, IGemuniLottery, Ownable {
    ...
    uint256 public maxPriceTicketInGENI = 5000 ether;
    uint256 public minPriceTicketInGENI = 0.0000000005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInGENI;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] geniPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInGENI;
        uint32 finalNumber;
    }
     ...
     * @param _GemuniTokenAddress: address of the GENI token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _GemuniTokenAddress, address _randomGeneratorAddress) {
        GemuniToken = IERC20(_GemuniTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of GENI to this contract
        uint256 amountGENIToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInGENI,
            _ticketNumbers.length
        );

        // Transfer geni tokens to this contract
        GemuniToken.safeTransferFrom(address(msg.sender), address(this), amountGENIToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInGENI += amountGENIToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInGENIToTransfer
        uint256 rewardInGENIToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInGENIToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        GemuniToken.safeTransfer(msg.sender, rewardInGENIToTransfer);

        emit TicketsClaim(msg.sender, rewardInGENIToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in GENI per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInGENI) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in GENI for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].geniPerBracket[j] =
                ...
                // A. No GENI to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].geniPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInGENI - amountToShareToWinners);

        // Transfer GENI to treasury address
        GemuniToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in GENI token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        GemuniToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInGENI += _amount;
     ...
     * @param _priceTicketInGENI: price of a ticket in GENI
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInGENI >= minPriceTicketInGENI) && (_priceTicketInGENI <= maxPriceTicketInGENI),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInGENI: _priceTicketInGENI,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            geniPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInGENI: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(GemuniToken), "Cannot be GENI token");
    ...
    /**
     * @notice Set GENI price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInGENI: minimum price of a ticket in GENI
     * @param _maxPriceTicketInGENI: maximum price of a ticket in GENI
     */
    function setMinAndMaxTicketPriceInGENI(uint256 _minPriceTicketInGENI, uint256 _maxPriceTicketInGENI)
        external
        onlyOwner
    {
        require(_minPriceTicketInGENI <= _maxPriceTicketInGENI, "minPrice must be < maxPrice");

        minPriceTicketInGENI = _minPriceTicketInGENI;
        maxPriceTicketInGENI = _maxPriceTicketInGENI;
    }
     ...
     * @param _priceTicket price of a ticket (in GENI)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].geniPerBracket[_bracket];
        } else {
            return 0;
        }
    }
