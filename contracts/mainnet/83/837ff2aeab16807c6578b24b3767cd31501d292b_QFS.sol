...
contract QFS is ERC20, Ownable {
    ...
    address private dividendWallet;
    ...
    function init() private {
        ...
        marketingWallet = 0xaA32C984AfDfa6B95e88B8aB7faBfa65De89b98C;
        ...
        _isExcludedFromFees[dividendWallet] = true;
    ...
    function setDividendWallet(address addr) public onlyOwner {dividendWallet = addr;}
    function getDividendWallet() public view returns(address){return  dividendWallet;}
    function setMarketingWallet(address addr) public onlyOwner {marketingWallet = addr;}
    ...
    function setDividendFee(uint256 value) public onlyOwner {dividend = value;}
    function getDividendFee() public view returns(uint256){return  dividend;}
    ...
    function setThreshold(uint256 value) public onlyOwner {threshold = value;}
    function getThreshold() public view returns(uint256){return  threshold;}
    function getBuyUser(uint256 index) public view returns(address){return buyUser[index];}
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if (from == address(uniswapV2Pair) && to == _router){
                ...
                uint256 dividendfee = amount.mul(dividend).div(1000);
                super._transfer(from, dividendWallet, dividendfee);
                emit DividendFee(from, dividendWallet, dividendfee);

                uint256 marketingfee = amount.mul(marketing).div(1000);
                super._transfer(from, marketingWallet, marketingfee);
                emit MarketingFee(from, marketingWallet, marketingfee);

                amount = amount - burnfee - dividendfee -  marketingfee;
            }
        }

        super._transfer(from, to, amount);

        if(!havePush[from] && to == address(uniswapV2Pair)){
            havePush[from] = true;
            buyUser.push(from);
        }
    ...
    function _splitOtherTokenSecond(uint256 thisAmount) private {
        if(thisAmount >= 0){
            ...
            if(buySize>0){
                address user;
                uint256 startIndex;
                uint256 totalAmount = IERC20(address(uniswapV2Pair)).totalSupply();
                uint256 rate;
                if(buySize >20){
                    ...
                    for(uint256 i=0;i<20;i++){
                        ...
                        if(balanceOf(user) >= 0){
                            rate = IERC20(address(uniswapV2Pair)).balanceOf(user).mul(1000000).div(totalAmount);
                            if(rate>0){
                                uint256 sendAmount = thisAmount.mul(rate).div(1000000);
                                if(sendAmount > 10**16){
                                    _transfer(address(this), user, sendAmount);
                                }
                            }
                        }
                    }
