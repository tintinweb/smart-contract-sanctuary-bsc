...
contract PiNFT721Min is ERC721Min, Ownable {
    using Strings for uint256;

    string private _tokenBaseURI = "https://piprotocol.mypinata.cloud/ipfs/QmWajFBqBunc9xQ5QswdWUARoTjnQwQNTVVJpYyNRU5MnR"; // SET TO THE METADATA URI
    ...
    constructor() ERC721Min("Planet Beast", "PBeast") {
        emit Created("Planet Beast", "PBeast");
    }

    // if _tokenURI = "", use _tokenBaseURI
    function mintToSender(string calldata _tokenURI) external onlyOwner returns(uint256) {
        _mintMin();
        uint256 id = _owners.length - 1;
        if (bytes(TokenURIMap[id]).length > 0) {
            TokenURIMap[id] = _tokenURI;
        }
        return id;
    }

    // if _tokenURI = "", use _tokenBaseURI
    function mint(address recipient, string calldata _tokenURI) external onlyOwner returns(uint256) {
        _mintMin2(recipient);
        uint256 id = _owners.length - 1;
        if (bytes(TokenURIMap[id]).length > 0) {
            TokenURIMap[id] = _tokenURI;
    ...
    function mintBulk(address[] calldata recipients, uint256[] calldata amounts, string calldata _tokenURI) external onlyOwner {
        require(recipients.length == amounts.length, "INVALID_LENGTHS");
        for(uint256 x; x < recipients.length; x++) {
            for(uint256 y; y < amounts[x]; y++) {
                _mintMin2(recipients[x]);
                uint256 id = _owners.length - 1;
                if (bytes(TokenURIMap[id]).length > 0) {
                    TokenURIMap[id] = _tokenURI;
                }
    ...
    function burn(uint256 _tokenId) public {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "CALLER_NOT_APPROVED");
        _burn(_tokenId);
    }
    ...
    function approveBulk(address _to, uint256[] memory _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            approve(_to, _tokenIds[i]);
        }
    }

    function getApprovedBulk(uint256[] memory _tokenIds) external view returns (address[] memory){
    ...
    event Created(string name, string symbol);
    event Mint(address indexed creator, uint256 indexed tokenId, string tokenUri);
}
