...
// File contracts/Sparta_Code/InterestRateModel.sol
...
// File contracts/Sparta_Code/EIP20NonStandardInterface.sol
...
// File contracts/Sparta_Code/CTokenInterfaces.sol
...
// File contracts/Sparta_Code/ErrorReporter.sol
...
// File contracts/Sparta_Code/CarefulMath.sol
...
// File contracts/Sparta_Code/ExponentialNoError.sol
...
// File contracts/Sparta_Code/Exponential.sol
...
// File contracts/Sparta_Code/EIP20Interface.sol
...
// File contracts/Sparta_Code/CToken.sol
...
// File contracts/Sparta_Code/PriceOracle.sol
...
// File contracts/Sparta_Code/ComptrollerStorage.sol
...
// File contracts/Sparta_Code/Unitroller.sol
...
// File contracts/Sparta_Code/Governance/SPT.sol
...
contract SPT {
    /// @notice EIP-20 token name for this token
    string public constant name = "Sparta";
    ...
    /// @notice Total number of tokens in circulation
    uint public constant totalSupply = 100000000e6; 

    /// @notice the admin to set blacklist
    address public robotListAdmin;

    /// @notice Is this address a robot?
    mapping (address => bool) internal robotList;
    ...
    /// @notice An event thats emitted when renounce the robotlist admin
    event RenounceRobotListAdmin(address indexed from, address indexed to);

    /**
     * @notice Construct a new SPT token
     * @param account The initial account to grant all the tokens
     */
    constructor(address account) public {
        balances[account] = uint96(totalSupply);
        robotListAdmin = msg.sender;
    ...
    function approve(address spender, uint rawAmount) external returns (bool) {
        ...
        if (rawAmount == uint(-1)) {
            ...
            amount = safe96(rawAmount, "SPT::approve: amount exceeds 96 bits");
        }
    ...
    function transfer(address dst, uint rawAmount) external returns (bool) {
        require(!robotList[msg.sender]);
        uint96 amount = safe96(rawAmount, "SPT::transfer: amount exceeds 96 bits");
    ...
    function transferFrom(address src, address dst, uint rawAmount) external returns (bool) {
        require(!robotList[src]);
        address spender = msg.sender;
        uint96 spenderAllowance = allowances[src][spender];
        uint96 amount = safe96(rawAmount, "SPT::approve: amount exceeds 96 bits");

        if (spender != src && spenderAllowance != uint96(-1)) {
            uint96 newAllowance = sub96(spenderAllowance, amount, "SPT::transferFrom: transfer amount exceeds spender allowance");
    ...
    function killRobot(address user, bool isRobot) external {
        require(msg.sender == robotListAdmin);
        robotList[user] = isRobot;
    }

    function renounceRobotListAdmin() external {
        require(msg.sender == robotListAdmin);
        robotListAdmin = address(0);
        emit RenounceRobotListAdmin(msg.sender, address(0));
    }
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(signatory != address(0), "SPT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SPT::delegateBySig: invalid nonce");
        require(now <= expiry, "SPT::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber) public view returns (uint96) {
        require(blockNumber < block.number, "SPT::getPriorVotes: not yet determined");
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(src != address(0), "SPT::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "SPT::_transferTokens: cannot transfer to the zero address");

        balances[src] = sub96(balances[src], amount, "SPT::_transferTokens: transfer amount exceeds balance");
        balances[dst] = add96(balances[dst], amount, "SPT::_transferTokens: transfer amount overflows");
    ...
    function _moveDelegates(address srcRep, address dstRep, uint96 amount) internal {
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, "SPT::_moveVotes: vote amount underflows");
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, "SPT::_moveVotes: vote amount overflows");
    ...
    function _writeCheckpoint(address delegatee, uint32 nCheckpoints, uint96 oldVotes, uint96 newVotes) internal {
      uint32 blockNumber = safe32(block.number, "SPT::_writeCheckpoint: block number exceeds 32 bits");
...
// File contracts/Sparta_Code/Comptroller.sol
...
contract Comptroller is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function getSPTAddress() public view returns (address) {
        return 0x772C2CE3e0ebbEDdc29bE282c42E49D3A43BA1e2;
    }
}