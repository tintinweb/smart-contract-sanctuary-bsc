...
// Author: @janedoe
contract Tesla is ERC20, SimpleAccess {
    ...
    constructor() ERC20("TeslaCoin", "Tesla") {
        ...
        buyTaxPtgBase = 0;
        sellLimitTimePeriod = 86400;
        sellLimitActive = false;
        sellLimitAmount = 10000000000000000000;
    }


    function getSellTax(address user, uint256 amount) external view returns (uint256) {
        console.log("buyTaxPtgBase %s amount: %s",buyTaxPtgBase, amount);
        return amount * buyTaxPtgBase / 100;
    }
    ...
    function _transfer(
        ...
        console.log("Transferring from %s to ", from , to);
        if(!isWhiteListed[msg.sender]){
            if (liquidityPair[from] && !noFee[to])
                transferAmount = transferAmount;
            else if (liquidityPair[to] && !noFee[from]){
                SellLimit storage limit = userToSellLimit[msg.sender];
                uint256 selledToday;
                uint256 diffSinceLastSell = block.timestamp - limit.lastSell;
                console.log("diffSinceLastSell %s ", diffSinceLastSell);
                if (diffSinceLastSell >= sellLimitTimePeriod) {
                    selledToday = amount;
                    console.log("selledToday %s ", selledToday);
                } 
                else {
                    ...
                    console.log("nowModule %s ", nowModule);
                    console.log("lastModulo %s ", lastModulo);
                    if (nowModule <= lastModulo) {
                        selledToday = amount;
                        console.log("1 nowModule %s ", nowModule);
                    } else { // if how far we are in the day now is greater than how far we were in the day at last buy then its the same day
                        selledToday = amount + limit.sold;
                        console.log("2 selledToday %s ", selledToday);
                    }
                }
                console.log("sellLimitAmount %s ", sellLimitAmount);
                if(sellLimitAmount != 0){
                    require(selledToday <= sellLimitAmount, "Sell exceeds buy limit");
                }
                console.log("Pass minimum check");
                limit.lastSell = uint128(block.timestamp);
                limit.sold = uint128(selledToday);
                console.log("lastSell %s sold: %s",limit.lastSell, limit.sold);
                uint256 currTax = this.getSellTax(from, transferAmount);
                console.log("currTax %s ", currTax);
                transferAmount -= currTax;
        ...
        if (tax > 0) 
            super._transfer(from, teamWallet, tax);
    ...
    //     require(msg.sender == mintPass, "Only mint pass can burn Zipper");
    //     _burn(user, amount);
    // }
    ...
    function setSellLimitTimePeriod(uint256 tp) external onlyOwner { 
        sellLimitTimePeriod = tp;
    }

    function setSellLimitActive(bool sl) external onlyOwner {
        sellLimitActive = sl;
    }

    function setSellLimitAmount(uint256 sl) external onlyOwner { 
        sellLimitAmount = sl;
    }
