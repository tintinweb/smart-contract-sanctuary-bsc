...
contract LosMuertos is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      // Save dev wallet
      _wallet = 0x8960E5Acb841DBEe5431CC0A5c1076EB717a6C0F;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "LosMuertos: minting has not started yet");
    require(_mintAmount > 0, "LosMuertos: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "LosMuertos: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "LosMuertos: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "LosMuertos: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "LosMuertos: zero address");
    require(_amount > 0, "LosMuertos: you have to mint at least one");
    require(lastSupply != 0,  "LosMuertos: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"LosMuertos: URI query for nonexistent token");
