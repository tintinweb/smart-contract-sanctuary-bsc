...
contract OborMembershipPurchase is Ownable {
    Membership  public membership;

    mapping(uint256 => MembershipPricing) public pricingTable;

    address public cardHolder;
    Membership  public membershipNft;

    constructor() public {

    }

    function setPrice(uint256 level, IERC20 currency, uint256 amount,  Membership _membershipNft, uint256 _minTokenId, uint256 _maxTokenId) public onlyOwner {
        pricingTable[level] = MembershipPricing({
            currency: currency,
            value: amount,
            membershipNft: _membershipNft,
            minTokenId: _minTokenId,
            maxTokenId: _maxTokenId,
            currentTokenId: _minTokenId
        });
    }
  ...
  function setMembershipNft(address _membershipNft) external onlyOwner {
     ...
     function stake(uint256 level) external {
        MembershipPricing storage pricing = pricingTable[level];

       // require(mintCount[msg.sender][membership] < maximumMintCount, "Reached max mint count");
        IERC20(pricing.currency).transferFrom(
            _msgSender(),
            address(this),
            pricing.value
        );
        uint256 tokenId = pricing.currentTokenId;
        pricing.currentTokenId = pricing.currentTokenId +1;

        require (pricing.currentTokenId < pricing.maxTokenId, "Token id maximum");

         pricing.membershipNft.mint(
                        msg.sender,
                        tokenId
                    );
