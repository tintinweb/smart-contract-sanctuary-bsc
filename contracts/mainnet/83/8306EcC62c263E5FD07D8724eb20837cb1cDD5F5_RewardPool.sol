...
contract HHB is IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "HHB Token";
    string private _symbol = "HHB";
...
contract RewardPool is Ownable, UserRegistry {
    ...
    address private constant HHB_ADDRESS = address(0x29C10E2d41e9D868840AAb5C1A4a4653087A853b);
    address private constant PAIR_ADDRESS = address(0x67A901d872CbCCe5D079cF6d8a7AD5BbE59e8f1D);
    address private constant BURN_ADDRESS = address(0x000000000000000000000000000000000000dEaD);
    address private constant RECEIVE_ADDRESS = address(0x51cA1A51cA2b118C1B7063316DEA310D72Ef3971);
    // bsc-testnet
    ...
    // address private constant HHB_ADDRESS = address(0xAb1E81126060e08AB6805ae89812F01e2117F481);
    ...
    function getPriceHHB(uint256 amount) public view returns(uint256) {
        return getPrice(HHB_ADDRESS, USDT_ADDRESS, amount);
    }

    function getPriceUSDT(uint256 amount) public view returns(uint256) {
        return getPrice(USDT_ADDRESS, HHB_ADDRESS, amount);
    }
    ...
    function depositHHB(uint256 amount, address recommender) external {
        uint256 usdtAmount = getPriceHHB(amount) * 5;
        IERC20(HHB_ADDRESS).transferFrom(msg.sender, BURN_ADDRESS, amount);
    ...
    function depositUSDT(uint256 amount, address recommender) external {
        uint256 hhbAmount = getPriceUSDT(amount) / 5;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, address(this), amount);
        IERC20(HHB_ADDRESS).transferFrom(msg.sender, BURN_ADDRESS, hhbAmount);
