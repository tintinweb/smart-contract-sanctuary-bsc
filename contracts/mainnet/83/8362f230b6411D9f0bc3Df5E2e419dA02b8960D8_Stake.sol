...
interface IERC20 {
    function name() external view returns (string memory);    function symbol() external view returns (string memory);    function decimals() external view returns (uint8);    function totalSupply() external view returns (uint256);    function balanceOf(address account) external view returns (uint256);    function transfer(address to, uint256 amount) external returns (bool);    function allowance(address owner, address spender)
        external
        view
        returns (uint256);    function approve(address spender, uint256 amount) external returns (bool);    function transferFrom(
        address from,
        address to,
        uint256 amount
    ) external returns (bool);    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(
...
contract Stake is Ownable {
    ...
    uint256 public userTotal;    uint256 private _perBlockDiff = 12056327160;
    ...
    uint256 private _endRewardBlock;    IERC20 private _LP;
    ...
    event Withdraw(address user, uint256 amount);    receive() external payable {}    function withdrawETH() public onlyAdmin {
        payable(msg.sender).transfer(address(this).balance);
    }    function withdrawToken(IERC20 token) public onlyAdmin {
        token.transfer(msg.sender, token.balanceOf(address(this)));
    }    constructor() {
        _lastRewardBlock = block.number;        _LP = IERC20(0xE877E2F70BD980b2adA86f636401F220Df3B8686);
        ...
        returns (address[] memory invites)
    {
        ...
        require(msg.sender != refer, "Not Self");        if (refer != address(0) && !userInfos[refer].isExist) {
        ...
        user.balance = 0;
        user.inviteRewardBalance = 0;        if (amount > 0 && user.refer != address(0)) {
        ...
        if (_startRewardBlock == 0) {
            ...
            _endRewardBlock = _startRewardBlock.add(28800 * 60);
        }
        if (lpSupply == 0) {
        ...
        uint256 multiplier = _getMultiplier();
        uint256 cakeReward;        if (multiplier >= 30 * 28800) {
