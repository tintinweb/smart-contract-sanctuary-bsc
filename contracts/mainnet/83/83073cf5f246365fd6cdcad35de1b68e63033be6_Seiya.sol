...
contract Seiya is ERC20 {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address _tokenOwner;
	address _baseToken = address(0x55d398326f99059fF775485246999027B3197955);
    address _bornToken = address(0x2287C04a15bb11ad1358BA5702C1C95E2D13a5E0);
    IERC20 public USDT;
    GogezillaWarp warp;
	...
	address public _fundAddress = address(0x49837Dc0cC3699dDe2Fd6DCC762551F1C941E55a);
    ...
    uint256 public bornAmount;
    uint256 public bornFishAmount;
	uint256 public ldxFishAmount;
    uint160 public constant MAXADD = ~uint160(0);
    uint256 total;
    constructor(address tokenOwner) ERC20("Seiya", "Seiya") {
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to]) {
			...
			if(from == uniswapV2Pair){}else if(to == uniswapV2Pair){}else{
                takeFee = false;
            }
        }
        ...
        if (takeFee) {
			...
			bornAmount = bornAmount.add(amount.div(100));
	...
	function swapAndLiquify() public {
        uint256 fhAmount;
        if(bornAmount > bornFishAmount){
            fhAmount = bornAmount.sub(bornFishAmount);
        }
		uint256 ldxAmount;
		if(bornAmount > ldxFishAmount){
            ldxAmount = bornAmount.sub(ldxFishAmount);
        }
		uint256 allAmount = balanceOf(address(this));
        if(allAmount > fhAmount.add(ldxAmount).add(10**27)){
            uint256 canswap = allAmount.sub(fhAmount).sub(ldxAmount);
		    swapTokensForOther(canswap.div(5).mul(4));
			uint256 usdtAmount = USDT.balanceOf(address(this));
			USDT.transfer(_fundAddress,usdtAmount.div(4).mul(3));
			addLiquidityUsdt(canswap.div(5),usdtAmount.div(4));
			ldxFishAmount = ldxFishAmount.add(canswap.div(5));
        }
    }
        ...
        if(buySize>0 && sendAmount > 0){
            ...
            if(buySize >10){
                for(uint256 i=0;i<10;i++){
                    ...
                    if(amountUsdt>10**10){
                        bornFishAmount = bornFishAmount.add(amountUsdt);
