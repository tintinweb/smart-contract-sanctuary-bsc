...
contract LaunchPadPool is ILaunchPadPool, ReentrancyGuard, Ownable {
  ...
  // function distributePurchasedTokens(uint256 start, uint256 end) external onlyOwner {
  ...
  function withdrawContribution() external {
    ...
    //require((block.timestamp+900) < poolStates.finishTime,"illegal operation");
    ...
    poolStates.totalVolumePurchased -= _amount.mul(poolSettings.rate).mul(10**decimalsToken).div(10**18);
    //if(block.timestamp+900 < poolSettings.endTime && poolStates.state != LaunchPadStorage.State.Cancelled) { 
    if((block.timestamp+900) < poolSettings.endTime && poolStates.state != LaunchPadStorage.State.Cancelled) { 
