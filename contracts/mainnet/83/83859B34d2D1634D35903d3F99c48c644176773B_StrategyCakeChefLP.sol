...
contract StrategyCommonChefLP is StratManager, FeeManager {
  ...
  constructor(
    ...
    public
    StratManager(_keeper, _strategist, _unirouter, _vault, _plutusFeeRecipient)
  {
    want = _want;
    poolId = _poolId;
    chef = _chef;
  ...
  function chargeFeesNative() internal {
    uint256 nativeFees = (IERC20(native).balanceOf(address(this)) * 45) / 1000;
    transferFees(nativeFees);
  }

  function transferFees(uint256 amount) internal {
    uint256 callFeeAmount = (amount * callFee) / MAX_FEE;
    IERC20(native).safeTransfer(msg.sender, callFeeAmount);

    uint256 plutusFeeAmount = (amount * plutusFee) / MAX_FEE;
    IERC20(native).safeTransfer(plutusFeeRecipient, plutusFeeAmount);

    uint256 strategistFee = (amount * STRATEGIST_FEE) / MAX_FEE;
    IERC20(native).safeTransfer(strategist, strategistFee);
  }

  // Adds liquidity to AMM and gets more LP tokens.
  function addLiquidityFrom(address yield) internal {
    uint256 yieldHalf = IERC20(yield).balanceOf(address(this)) / 2;
  ...
  function _giveAllowancesArguments(
    ...
    IERC20(_want).safeApprove(_chef, type(uint256).max);
    IERC20(_output).safeApprove(_unirouter, type(uint256).max);

    IERC20(_lpToken0).safeApprove(_unirouter, 0);
    IERC20(_lpToken0).safeApprove(_unirouter, type(uint256).max);

    IERC20(_lpToken1).safeApprove(_unirouter, 0);
    IERC20(_lpToken1).safeApprove(_unirouter, type(uint256).max);
  }
...
contract StrategyCakeChefLP is StrategyCommonChefLP {
  address private immutable chefAddress; // MasterChef address
  ...
  function harvest() public override(StrategyCommonChefLP) {
    super.harvest();
  }
}