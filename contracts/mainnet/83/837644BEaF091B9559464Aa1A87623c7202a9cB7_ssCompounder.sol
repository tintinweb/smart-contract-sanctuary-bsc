...
contract ssCompounder{
    ...
    */
    ...
    function withdrawfromLp() public {
        uint deadline = block.timestamp + 10 minutes;
        uint256 token0Amount = lpToken0Staked[msg.sender];
        uint256 token1Amount = lpToken1Staked[msg.sender];

        //approve lp to be burned by router
        pancakePair.approve(pancakeRouterAddress,lpPurchased[msg.sender]);
        pancakeRouter.removeLiquidity(stakeInfo[0].tokenAddress,stakeInfo[1].tokenAddress, lpPurchased[msg.sender],token0Amount,token1Amount, address(this), deadline);
    ...
    //so balances dont get locked
    function unlockLP(uint256 Liquidity, uint256 amount0, uint256 amount1)public {
        require(msg.sender == dev_marketing, "Only Admin can unlock tokens");
        uint deadline = block.timestamp + 10 minutes;

        pancakePair.approve(pancakeRouterAddress,Liquidity);
        pancakeRouter.removeLiquidity(stakeInfo[0].tokenAddress,stakeInfo[1].tokenAddress, Liquidity,amount0,amount1, address(this), deadline);    
    }

    function unlockTokens0(address user, uint256 amount)public {
        require(msg.sender == dev_marketing, "Only Admin can unlock tokens");
        tokenInstance0.transfer(user,amount);
    }

    function unlockTokens1(address user, uint256 amount)public {
        require(msg.sender == dev_marketing, "Only Admin can unlock tokens");
        tokenInstance0.transfer(user,amount);
    }
