...
contract DividendPayingToken is BEP20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function withdrawDividend() public virtual override {
    ...
    if (_withdrawableDividend > 0) {
      ...
      bool success = IBEP20(ETH).transfer(user, _withdrawableDividend);
...
contract BNBDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BNB_Dividend_Tracker", "BNB_Dividend_Tracker", 18) {
    	claimWait = 3600; 
        minimumTokenBalanceForDividends = 10000 * (10**18); //0.01%
    }
...
contract BRB is BEP20, Ownable{
   ...
   using SafeMath for uint256;
    ...
    uint256 public _TotalFees = _ETHRewardsFee.add(_LiquidityFee).add(_MarketingFee).add(_BurnFee);
    
    
    uint256 public maxWalletToken = 200 * (10**5) * (10**18); //2%

    address public _marketingWalletAddress = 0x2eA1286d358663b0AB7443d94827C25639ef4D75;
    ...
    constructor() BEP20("Black Rabbit TEST", "BRBT", 18) {
        ...
        excludeOrIncludeFromMaxTx(deadWallet, true);
    ...
    function updatePancakeswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeswapV2Router), "BRB: The router already has that address");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "BRB: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != pancakeswapV2Pair, "BRB: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialETHBalance = IBEP20(ETH).balanceOf(address(this));

        swapTokensForETH(tokens);
        uint256 newBalance = (IBEP20(ETH).balanceOf(address(this))).sub(initialETHBalance);
        ...
        IBEP20(ETH).transfer(_marketingWalletAddress, marketingFeeBalance);
        IBEP20(ETH).transfer(_developmentWalletAddress, teamFeeBalance);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        swapTokensForETH(tokens);
        uint256 dividends = IBEP20(ETH).balanceOf(address(this));
        bool success = IBEP20(ETH).transfer(address(dividendTracker), dividends);
     ...
     function calculateFee() private {
        ...
        if(numWBNB >= 500 * (10**18) && numWBNB < 800 * (10**18)){

            _ETHRewardsFee = 4;
            _LiquidityFee = 0;
            _MarketingFee = 2;
            _BurnFee = 2;
            _TotalFees = _ETHRewardsFee.add(_LiquidityFee).add(_MarketingFee).add(_BurnFee);//8%
       }

        if(numWBNB >= 800 * (10**18) && numWBNB < 1600 * (10**18)){

            _ETHRewardsFee = 2;
            _LiquidityFee = 0;
            _MarketingFee = 1;
            _BurnFee = 1;
            _TotalFees = _ETHRewardsFee.add(_LiquidityFee).add(_MarketingFee).add(_BurnFee);//4%
       }

         if(numWBNB >= 1600 * (10**18) && numWBNB < 8000 * (10**18)){

            _ETHRewardsFee = 2;
            _LiquidityFee = 0;
            _MarketingFee = 0;
            _BurnFee = 0;
            _TotalFees = _ETHRewardsFee.add(_LiquidityFee).add(_MarketingFee).add(_BurnFee);//2%
       }

          if(numWBNB >= 8000 * (10**18)){

            _ETHRewardsFee = 0;
            _LiquidityFee = 0;
            _MarketingFee = 0;
            _BurnFee = 0;
            _TotalFees = _ETHRewardsFee.add(_LiquidityFee).add(_MarketingFee).add(_BurnFee);//0%
