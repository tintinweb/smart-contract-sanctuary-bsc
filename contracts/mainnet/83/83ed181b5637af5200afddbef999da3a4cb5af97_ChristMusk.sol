...
// Telegram : https://t.me/Christ_MuskPortal
// Website : https://christmusk.day/
...
library Address{
 ...
 contract ChristMusk is IERC20, Context, Ownable {
    ...
    string private constant _name = "ChristMusk";

    uint256 public CM_max_BuyLimit = 30000000 * 10**9;
    uint256 public CM_max_SellLimit = 30000000 * 10**9;

    address private CM_Wallet = 0x10BF7C929C360dFA5147C7f88f417aE6f2254dC0;
    address private CM_Team_Wallet = 0xAbaE683E0E454600a5DbCF48c1d406371916299e;

    uint256 public CM_max_WalletLimit = 30000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[CM_Wallet] = true;
        _isExcludedFromFee[CM_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[CM_Wallet] = true;
        allowedTransfer[CM_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= CM_max_BuyLimit, "You are exceeding CM_max_BuyLimit");
            require(balanceOf(to) + amount <= CM_max_WalletLimit, "You are exceeding CM_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= CM_max_SellLimit, "You are exceeding CM_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= CM_max_WalletLimit, "You are exceeding CM_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(CM_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(CM_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function CM_Update_MarketingWallet(address newWallet) external onlyOwner{
        CM_Wallet = newWallet;
    }
    
    function CM_Update_TeamWallet(address newWallet) external onlyOwner{
        CM_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        CM_max_BuyLimit = maxBuy * 10**decimals();
        CM_max_SellLimit = maxSell * 10**decimals();
    }
    
    function CM_Max_Walletlimit(uint256 amount) external onlyOwner{
        CM_max_WalletLimit = amount * 10**decimals();
    }
