...
Elon Musk: Fun times ahead!!

TG: https://t.me/Parler_Ye
...
contract PARLER is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PARLERDividendTracker public dividendTracker;
    ...
    constructor() ERC20("PARLER", "PARLER") {
        marketingWallet = payable(0x4F1D6DeFe553A063E3E8218408A4E8cD38D4E803);
        devWallet = payable(0xb6B556B5b64945d6A7Ce416344Df10b08dDBD024);
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new PARLERDividendTracker(
            payable(this),
            router,
...
contract PARLERDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        PARLERContract = PARLER(mainContract);
    ...
    function _transfer(
        ...
        require(false, "PARLER_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "PARLER_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PARLER contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(PARLERContract);

            uint256 prevBalance = PARLERContract.balanceOf(address(this));
            ...
            {
                uint256 received = PARLERContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    PARLERContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
