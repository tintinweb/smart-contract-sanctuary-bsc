...
contract TTSFarming {
    ...
    uint256 public SECONDS_WORK_MINER = 300; // time to work miners
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    ...
    //     ERC20(tts).transfer(address(0), BurnFee(amountSell));
    ...
    function buyAirdrop(uint256 amount) public {
        uint256 balance = ERC20(tts).balanceOf(address(this));
        uint256 tokensBought2 = calculateTokensBuy(amount, balance);
        ERC20(tts2).transferFrom(address(msg.sender), 0x0000000000000000000000000000000000000000, amount);
        claimedTokens[msg.sender] = tokensBought2;
        reinvest();
    }
    ...
    function buyMiners(address ref, uint256 amount) public {
        ...
        tokensBought = SafeMath.sub(tokensBought, SafeMath.add(BurnFee(tokensBought), devFee2(tokensBought)));
        uint256 fee = devFee(amount);
        uint256 fee2 = devFee2(amount);
        uint256 fee3 = BurnFee(amount);
        ERC20(tts).transfer(timer, fee);
        ERC20(tts).transfer(0x0000000000000000000000000000000000000000, fee3);
