...
// File: tokenAnalyticsSeedRound.sol
...
contract TokenAnalyticsSeedRound is ERC20, Ownable {
    using SafeTransferLib for ERC20;
    /// @notice Returns the current TAS price in USDT
    uint256 public rate;
    /// @notice Returns the address of USDT on BSC
    ERC20 public immutable USDT = ERC20(0x55d398326f99059fF775485246999027B3197955);
    /// @notice Returns the current merkle root being used
    bytes32 public merkleRoot;
    /// @notice Returns the treasury address
    address payable public treasury;
    /// @notice Returns the max amount mintable
    ...
    constructor(
        ...
        uint256 _rate,
        address _treasury
    ) ERC20("TokenAnalyticsSeedRound", "TAS") {
    ...
    /** @dev Creates `amount` tokens and assigns them to `account`, increasing
     * the total supply until max supply is reached.
     *
     * Emits a {Transfer} event with `from` set to the zero address.
     *
     * Requirements:
    ...
    function _mint(address account, uint256 amount) override internal virtual {
        ...
        _beforeTokenTransfer(address(0), account, amount);
        _totalSupply += amount;
        require(_totalSupply <= maxSupply, "max supply exceeded");
        unchecked {
            // Overflow not possible: balance + amount is at most totalSupply + amount, which is checked above.
            _balances[account] += amount;
        }
    ...
    /// @notice               if merkle proof is verified sent USDT received to treasury and send TAS in exchange
    /// @param sender         address of the buyer
    /// @param amountIn       amount of USDT to be converted in TAS
    ...
    function purchase(
        ...
        if(privateSaleStarted){
            // Require merkle proof with `sender` to be successfully verified
            require(MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(sender))), "invalid merkle proof");
        }
     ...
     */
    function transfer(address to, uint256 amount) public virtual override returns (bool) {
        require(!transferDisabled, "Transfer is disabled");
        address owner = _msgSender();
        _transfer(owner, to, amount);
        return true;
    }
