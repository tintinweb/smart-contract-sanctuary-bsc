...
contract BallondOrToken is ERC20, Ownable {
    ...
    event walletsChanged(address wallet1);
    ...
    constructor(
        address wallet1_, address wallet2_, address wallet3_, address wallet4_
    )
        payable
        ERC20("Ballon dOr Token", "BDO")
    {
        ...
        wallet2 = address(wallet2_);
        wallet3 = address(wallet3_);
        wallet4 = address(wallet4_);
    ...
    function changeWallets(
        ...
        onlyOwner
    {
        wallet1 = address(_wallet1);
        wallet2 = address(_wallet2);
        wallet3 = address(_wallet3);
        wallet4 = address(_wallet4);
        emit walletsChanged(wallet1);
    }
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from] && autoSwapEnable) {
            ...
            swapAndSendMarketing(swapAmount);
            swapping = false;
        }
    ...
    function swapAndSendMarketing(uint256 tokenAmount) private {
        ...
        uint256 amount1 = newBalance / 4;
        uint256 amount2 = newBalance / 4;
        uint256 amount3 = newBalance / 4;
        uint256 amount4 = newBalance - amount1 - amount2 - amount3;

        sendBNB(payable(wallet1), amount1);
        sendBNB(payable(wallet2), amount2);
        sendBNB(payable(wallet3), amount3);
        sendBNB(payable(wallet4), amount4);
    }
