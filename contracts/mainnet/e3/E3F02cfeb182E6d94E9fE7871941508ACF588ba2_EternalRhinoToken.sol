...
import "./EternalRhinoTokenStorage.sol";
import {List} from  "../struct/RhinoTokenStructs.sol";


contract EternalRhinoToken is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, EternalRhinoTokenStorage {
    ...
    constructor(string memory baseURI_, address payee_) ERC721("MiddleLevelEternalRhinoToken","MERT") {
        baseURI = baseURI_;
        payee = payee_;
    }
    ...
    function withdraw() external nonReentrant returns(uint _mintTokenId){
        ...
        List memory list = lotteryList[_recipient];
        ...
        List storage lottery = lotteryList[_recipient];
        lottery.reward += 1;
        return newTokenId;
    }
    ...
    function withdrawList(uint256 mintAmount) external nonReentrant returns(uint256[] memory){
        ...
        List storage lottery = lotteryList[_recipient];
        lottery.reward += mintAmount;
        return id;
    }

    function synthesisMint(address recipient) external onlyMerge returns(address, uint256){
    ...
    function subscribeList(address[] calldata account, uint256[] calldata amount) external onlyOwner {
        ...
        for (uint i = 0; i < account.length; i++) {
            ...
            List storage list = lotteryList[account[i]];
    ...
    function oneSubscribeList(address account, uint256 amount) external onlyOwner {
        ...
        List storage list = lotteryList[account];
...
contract EternalRhinoTokenStorage {

    uint256 internal subscribeAmount;

    uint256 public constant MAX_SUPPLY = 50;

    mapping(address => List) public lotteryList;
