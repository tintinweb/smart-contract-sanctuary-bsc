...
contract RewardTreasury is Ownable {
    ...
    constructor(IBEP20 _roxy, IMasterChef _masterChef, uint256 _masterChefPoolId, IBEP20 _lockedPoolToken) {
        roxy = _roxy;
    ...
    function depositToMasterChef(uint256 _amount) external onlyOwner {
        lockedPoolToken.approve(address(masterChef), _amount);
        lockedPoolToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdrawFromMasterChef(uint256 _amount) external onlyOwner {
         ...
         lockedPoolToken.safeTransfer(address(msg.sender), _amount);
