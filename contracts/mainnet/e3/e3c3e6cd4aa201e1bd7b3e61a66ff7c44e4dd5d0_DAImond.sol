      ...
      "Yb,   `"Y8ad'      "Yb,8,dP"      `ba8P"'   ,dP"
        "Yb,    `"8,        "Y8P"        ,8"'    ,dP"
          "Yb,    `8,         8         ,8'    ,dP"
            "Yb,   `Ya        8        aP'   ,dP"
              "Yb,   "8,      8      ,8"   ,dP"
                "Yb,  `8,     8     ,8'  ,dP"   Innovating Stable Coins:
                  "Yb, `Ya    8    aP' ,dP"     DAImond.gg
                    "Yb, "8,  8  ,8" ,dP"
                      "Yb,`8, 8 ,8',dP"
                        "Yb,Ya8aP,dP"
                          "Y88888P"
...
contract DAImond is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 DAImond-WAVAX LP -> DAImond pool
    // uint256 public constant INITIAL_DAImond_POOL_DISTRIBUTION = 0 ether;
    ...
    address public DAImondOracle;
    // Address of the Tax Office
    // Stuff
    address public taxOffice;
    string public discord = "https://discord.gg/P3stJsKe";
    string public website = "daimond.gg";
    string public twitter = "DAImondsBSC";
    string public telegramAnnouncement = "https://t.me/DAImondBSCchat";
    // Current tax rate
    ...
    // Address of the DAImond-Avax LP
    ...
    /**
     * @notice Constructs the DAImond ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress, uint256 _sellStartTime) public ERC20("DAImond Token", "DAImond") {
        // Mints 1 DAImond to contract creator for initial pool setup
    ...
    function _getDAImondPrice() internal view returns (uint256 _DAImondPrice) {
        try IOracle(DAImondOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("DAImond: failed to fetch DAImond price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _DAImondPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_DAImondPrice >= taxTiersTwaps[tierId]) {
    ...
    function setDAImondOracle(address _DAImondOracle) public onlyOperatorOrTaxOffice {
        require(_DAImondOracle != address(0), "oracle address cannot be 0 address");
        DAImondOracle = _DAImondOracle;
    }
    ...
    /**
     * @notice Operator mints DAImond to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of DAImond to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentDAImondPrice = _getDAImondPrice();
            currentTaxRate = _updateTaxRate(currentDAImondPrice);
            if (currentDAImondPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        // address _DAImondPool
        // address _airdropWallet
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_genesisPool != address(0), "!_genesisPool");
        // require(_DAImondPool != address(0), "!_DAImondPool");
        // require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        // _mint(_DAImondPool, INITIAL_DAImond_POOL_DISTRIBUTION);
    ...
    function getRemainingTimeForDAImondSellingEnable() public view returns(uint256) {
        ...
        /// @dev overrides transfer function to meet tokenomics of DAImond
