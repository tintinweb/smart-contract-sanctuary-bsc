...
contract XVSVault is XVSVaultStorage, ECDSA {
    ...
    /**
     * @dev This is a temporary function to recompute the reward debt for two affected
     *   users. It will be removed in the next implementation contract.
     */
    function recomputeRewardDebt() external onlyAdmin {
        address xvsAddress_ = xvsAddress;

        address user1Address = address(0x37d768C8FC5A0F54754A6bDB8b8469e6fF8CaD07);
        UserInfo storage user1 = userInfos[xvsAddress_][0][user1Address];
        uint256 user1AccRewardPerShare = 57427560299109; // At block 25458953
        user1.rewardDebt = user1.amount.mul(user1AccRewardPerShare).div(1e12);

        address user2Address = address(0x3C7eA3ae7C47bD817C63c47e9ecEe89452471A9E);
        UserInfo storage user2 = userInfos[xvsAddress_][0][user2Address];
        uint256 user2AccRewardPerShare = 57427562417997; // At block 25459184
        user2.rewardDebt = user2.amount.mul(user2AccRewardPerShare).div(1e12);
    }
