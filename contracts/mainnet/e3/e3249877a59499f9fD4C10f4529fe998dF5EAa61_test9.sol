...
contract test9 is Context, IERC20, Ownable {
    ...
    string private _name = "test9";
    string private _symbol = "test9";
    ...
    address public charityWallet = 0xa17819e8A62038161C9BCE008493894C047b6931;
    ...
    constructor () public {
        ...
        _isExcludedMaxTransactionAmount[charityWallet] = true;
        
        _isExcludedMaxWallet[owner()] = true;
        _isExcludedMaxWallet[address(this)] = true;
        _isExcludedMaxWallet[charityWallet] = true;

        _maxTxAmount = totalSupply() * 10 / 1000; // 1% maxTx
        _maxWalletSize = totalSupply() * 10 / 1000; // 1% maxWallet
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        if(!_isExcludedMaxTransactionAmount[sender] && !_isExcludedMaxTransactionAmount[recipient]) {
            require(amount <= _maxTxAmount, "Buy transfer amount exceeds the _maxTxAmount.");
            if(!_isExcludedMaxWallet[sender] && !_isExcludedMaxWallet[recipient]) {
                require(amount + balanceOf(recipient) <= _maxWalletSize, "Max wallet exceeded.");
            }
        }
    ...
    function updateMaxTxnAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 1000)/1e18, "Cannot set _maxTxAmount lower than 0.1%");
