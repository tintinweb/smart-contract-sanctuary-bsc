...
contract FIFA is IERC20, Ownable {
    ...
    address private _fundAddress = address(0x1F6737dcBb133fF5CaeCB89D3493374f4eF022ae);
    address private _airdropAddress = address(0x636BAFA8216c9e00Aac6Adb18a4a4966E2f51A9c);
    ...
    constructor() {
        ...
        uint256 leaveAmount = 5000000 * 10**_decimals;
        _tTotalFeeMax = _tTotal.sub(leaveAmount);
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[tokenOwner] = _rTotal;
        _owner = tokenOwner;
        whiteLists[_owner] = true;
        whiteLists[_fundAddress] = true;
        whiteLists[_airdropAddress] = true;
        ...
        if (_endBlock >= block.number) {
            if (to == uniswapV2Pair || from == uniswapV2Pair) {
                if (!whiteLists[from] || !whiteLists[to]) {
                    ...
                    uint256 rAmount = amount.sub(txFee);
                    emit Transfer(from, _fundAddress, txFee);
