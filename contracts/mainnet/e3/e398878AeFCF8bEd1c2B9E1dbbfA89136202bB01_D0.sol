...
interface IMining {
    function withdrawBonus(uint256 _pid,uint256 _nftid, address _addr) external;
	function buyAndBurn() external;

}
interface INft {
    function mintDFC(address _useraddr) external;
	function DFCBloodSub(uint256 tokenId,uint256 _value) external;
	function DFCBloodAdd(uint256 tokenId,uint256 _value) external;
	function DFCBloodRe(uint256 tokenId) external;
	function totalSupply() external view returns (uint256);
	function balanceOf(address owner) external view returns (uint256 balance);
	function ownerOf(uint256 tokenId) external view returns (address owner);
	function nftsBlood(uint256 tokenId) external view returns (uint256 balance);
	function DFCtransfer(address from, address to, uint256 tokenId) external;
	
	
}


contract D0 is Context, IERC20, Ownable {
	...
	address public nftMining;
	uint256 public nftMintNum = 100000000000 * 10**18; 
    ...
    address private fomoReceiver;
	address public dividendsAddr;
    ...
    uint256 public feeLiquidityall = 10;
    uint256 public feeLiquidity = 20;
	uint256 public feeOtherToken = 80;
	...
	function setNfttoken(address _nfttoken) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        nfttoken = _nfttoken;
    }
	
	function setNftMining(address _nftMining) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        nftMining = _nftMining;
    }
	
	function setNftfee(uint256 _nftfee) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        nftfee = _nftfee;
    }
	
	function setNftMintNum(uint256 _nftMintNum) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        nftMintNum = _nftMintNum;
    }
	function setIsMint(bool _value) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        isMint = _value;
    }

    
    function setDividendsAddr(address _dividendsAddr) public {
        require(_msgSender() == devReceiver || _msgSender() == owner(), "fail");
        dividendsAddr = _dividendsAddr;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tFee) {
		...
		if (launchedAtTime + 10 > block.timestamp) {
			tLiquidity = tAmount.mul(feeLiquidityall).div(100);
		}else{
			tLiquidity = tAmount.mul(feeLiquidityall).div(100);
		}
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && from != owner() && to != owner() && to != fomoReceiver){
			...
			if(amount >= nftMintNum && isMint && !isContract(to)){
				INft(nfttoken).mintDFC(to);
			}
			uint256 nfttotal = INft(nfttoken).totalSupply();
			if(isMint && !isContract(to) && nfttotal > 0){
				uint rid = randType(to,nfttotal);
				address nftaddr = INft(nfttoken).ownerOf(rid);
				IMining(nftMining).withdrawBonus(0,rid,nftaddr);
				if(rid >= nftfee && nftaddr != to ){
					uint256 nowblood = INft(nfttoken).nftsBlood(rid);
					if(amount >= nowblood){
						INft(nfttoken).DFCtransfer(nftaddr,to,rid);
						INft(nfttoken).DFCBloodRe(rid);
					}else{
						INft(nfttoken).DFCBloodSub(rid,amount);
					}
				}
        ...
        if (
            ...
            //.mul(feeLiquidityall).div(100);
            uint256 lptmp = balanceOf(address(this)).mul(feeLiquidity).div(100);
            //add liquidity
            swapAndLiquify(lptmp);
			swapAndSendDividends(balanceOf(address(this)).sub(lptmp));
			inSwapAndLiquify = false;
        }
	...
	function randType(address _addr, uint _modulus) internal returns(uint){
        randNonces++;
        return uint(keccak256(abi.encodePacked(now,_addr,randNonces))) % _modulus;
    }
	
	
	
	function lotterize(address _addr,uint256 _modulus) private returns(uint randomNumber) {
		uint256 randomNumber = randType(_addr,_modulus);
