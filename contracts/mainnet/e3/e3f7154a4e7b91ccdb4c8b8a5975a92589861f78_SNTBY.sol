...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _rewardToken) DividendPayingToken("Reward Tracker", "DividendTracker", _rewardToken) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = minBalance * 10 ** 8;
    }
...
contract SNTBY is ERC20, Ownable {
    ...
    event TransferFeesUpdated(uint256 fee1);
    ...
    constructor() payable ERC20("Santa Bunny", "SNTBY") {
        ...
        treasuryWallet = 0x92EAeC51919f655615d74966Edd4227bd774B01c;
        ...
        _mint(owner(), 10_000_000 * (10 ** 8));

        swapTokensAtAmount          = totalSupply() / 5000;
        maxTransactionAmountBuy     = totalSupply() * 4 / 100; // 4%
        maxTransactionAmountSell    = totalSupply() * 2 / 100; // 2%
    }
    ...
    function updateTransferFees(uint256 _transferFee) external onlyOwner {
        transferFee = _transferFee;

        require(transferFee <= 2, "Transfer fee cannot be more than 2%");

        emit TransferFeesUpdated(totalSellFee);
    }
