...
contract SKToken is IERC20, Ownable {
    ...
    // share
    uint256 public minLpUsdt = 50 * 10 ** 18;
    ...
    constructor(address _router, IERC20 _usdt, address _projectAddress, address _nftPoolAddress) {
        usdt = _usdt;
    ...
    function setAddress(address addr1, address addr2) public onlyOwner {
        ...
        nftPoolAddress = addr2;
        _isExcludedFromFee[projectAddress] = true;
        _isExcludedFromFee[nftPoolAddress] = true;
    }
    ...
    function _transfer(
        ...
        if (block.timestamp.sub(startTime) < oneDay && ammPairs[to]) {
            require(userBuy[from] >= minBuy || _isExcludedFromFee[from], "must buy token first");
        }
        ...
        if (ammPairs[from]) {
            userBuy[to] = userBuy[to].add(amount);
        }
    ...
    function _tokenTransfer(
        ...
        uint256 nftPoolFee = _nftPool(sender, amount);
        ...
        uint256 recipientAmount = amount.sub(nftPoolFee + projectFee + lpFee + burnFee);
    ...
    function _nftPool(
        ...
        uint256 tFee = amount.mul(400).div(baseRate);
        _tOwned[nftPoolAddress] = _tOwned[nftPoolAddress].add(tFee);
        emit Transfer(sender, nftPoolAddress, tFee);
        return tFee;
    }
