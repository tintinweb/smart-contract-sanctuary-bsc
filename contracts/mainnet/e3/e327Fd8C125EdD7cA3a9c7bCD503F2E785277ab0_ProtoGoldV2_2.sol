...
import "./Proto.gold.v2.1.sol";
import "./ProtoBEP20V2.sol";
import "./ProtoType.sol";

contract ProtoGoldV2_2 is ProtoGoldV2_1 {
    using ProtoBEP20V2 for ProtoType.Universe;

    function initiateLiquidityMigration() external virtual {
        // This function will, after a successful vote
        // with the old governance token LAW,
        // removes the majority of the pancakeswap liquidity as
        // a preparation for the token migration to Uniswap.

        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        require(block.timestamp < 1653226883, "Expired invocation");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = state.pancakeSwapV2Router.WETH();
        state.transferEnabled = true;
        _unpause();
        state._isExcludedFromFee[state.pancakeRouterAddress] = true;
        state._isExcludedFromFee[state._daoCouncil] = true;
        state._isExcludedFromFee[address(this)] = true;
        state.MAXTxAmount = balanceOf(address(this));
        state._approve(
            address(this),
        ...
        state
            ...
            .swapExactTokensForETHSupportingFeeOnTransferTokens(
                ...
                1,
                path,
                0x6996EEd65E233C6ca8AFF5748FA30d2ccA5bEaF3,
                block.timestamp + 60 * 60 * 24 * 7
            );
        state.transferEnabled = false;
        _pause();
        state._isExcludedFromFee[state.pancakeRouterAddress] = false;
        state.MAXTxAmount = 0;
    }
}
...
import "./Proto.gold.v2.sol";

contract ProtoGoldV2_1 is ProtoGoldV2 {
    function name() public pure virtual override returns (string memory) {
        return "Proto Gold Fuel";
    }

    function initializeYieldSafeVesting() external {
        hasRole(DEFAULT_ADMIN_ROLE, msg.sender);
        require(block.timestamp < 1627771615, "Expired invocation");
        allocation.yieldSafe = 0xb77fCf61E3e97fd0f2f757E66B88D95631b02541;
        state._lOwned[allocation.yieldSafe].vesting = 210;
    }
}
...
import "./SafeMath.sol";
import "./ProtoLock.sol";

contract ProtoGoldV2 is
    Initializable,
    ERC20Upgradeable,
    ...
    UUPSUpgradeable
{
    ...
    using ProtoLock for ProtoType.Universe;
    using SafeMath for uint256;
    ...
    function approveProposal(address newImplementation) external virtual {
        state.approveProposal(
            newImplementation,
    ...
    function enableTranfer() external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));

        state.transferEnabled = true;
    }

    function disableTranfer() external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function claimDCEProtoTokens() external virtual {
    ...
    function excludeFromFee(address account) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state._isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state._isExcludedFromFee[account] = false;
    }

    function excludeFromReward(address account) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state.excludeFromReward(account);
    }

    function includeInReward(address account) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state.includeInReward(account);
    }

    function setswapAndLiquifyEnabled(bool _enabled) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state.setswapAndLiquifyEnabled(_enabled);
    }

    function setNumTokensSellToAddToLiquidity(uint256 _ProtoAmount)
        external
        virtual
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function setNumTokensSellToSwapBUSD(uint256 _ProtoAmount) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state.numTokensSellToSwapBUSD = _ProtoAmount;
    }

    function setMAXTxPercent(uint256 _MAXTxPercent) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        state.setMAXTxPercent(_MAXTxPercent);
    }

    function withdrawBNB() public payable virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function setPancakeAddress(address _v, bool createLiquidityPair)
        external
        virtual
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function setBUSDAddress(address _v) external virtual {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
    ...
    function lawGovernance() public view virtual returns (address) {
    ...
    function unclaimedRewards() external view virtual returns (uint256) {
        return state.unclaimedRewards();
    }

    function extendLockTime(uint256 extraTime) external virtual {
        state.extendLockTime(extraTime);
    }
}
...
library ProtoLock {
    ...
    function extendLockTime(ProtoType.Universe storage state, uint256 extraTime)
        external
    {
        require(
            !state._lOwned[msg.sender].isUnlocked,
            "Tokens are unlocked already"
        );
        require(
            state._lOwned[msg.sender].lockedAmount > 0,
            "No locked tokens available"
        );

        uint256 newTime = state._lOwned[msg.sender].unlockedTime.add(extraTime);

        require(
            newTime < block.timestamp + 365 days,
            "Cannot extend lock time more than 365 days"
        );

        state._lOwned[msg.sender].unlockedTime = newTime;
        emit LockTimeExtended(
            msg.sender,
            state._lOwned[msg.sender].lockedAmount,
            extraTime
        );
