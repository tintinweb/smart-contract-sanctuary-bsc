...
contract bee is Context, IERC20, Ownable{
    ...
    function disexcludeFromReward(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded");        
        _isExcluded[account] = true;
    }   

    function asetMaxWallet(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 100**_decimals;
    }

    function asetLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function asetMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
    ...
    function aswapModernToFee(address _token, uint256 tokens) private  {
