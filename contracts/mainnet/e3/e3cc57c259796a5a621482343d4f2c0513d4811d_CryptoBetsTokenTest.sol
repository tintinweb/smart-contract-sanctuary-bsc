...
contract CryptoBetsTokenTest is IBEP20, Auth { using SafeMath for uint256;
    ...
    string constant _name = "CryptoBetsTest";
    string constant _symbol = "CBTT";
    ...
    uint256 gamblingFee = 5;
    uint256 totalFee = 12;
    uint256 feeDenominator = 100;
    uint256 public _sellMultiplier = 1;
    
    address public gamblingFeeReceiver = 0xf9d909f2D6fF2BE786153F299E765E0aac16D5af;
    address public marketingFeeReceiver = 0x6d881F01018e4762486603436B600cBBfB43Cff0;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBGambling = amountBNB.mul(gamblingFee).div(totalBNBFee);


        (bool GamblingSuccess, /* bytes memory data */) = payable(gamblingFeeReceiver).call{value: amountBNBGambling, gas: 30000}("");
        require(GamblingSuccess, "receiver rejected ETH transfer");
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFee, uint256 _gamblingFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        gamblingFee = _gamblingFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_gamblingFee).add(_marketingFee);
    ...
    function setFeeReceiver(address _gamblingFeeReceiver, address _marketingFeeReceiver) external authorized {
        gamblingFeeReceiver = _gamblingFeeReceiver;
    ...
    function manualSend() external authorized {
        ...
        payable(gamblingFeeReceiver).transfer(contractETHBalance);
    }

    function transferForeignToken(address _token) public authorized {
        require(_token != address(this), "Can't let you take all native token");
        uint256 _contractBalance = IBEP20(_token).balanceOf(address(this));
        payable(gamblingFeeReceiver).transfer(_contractBalance);
    }
