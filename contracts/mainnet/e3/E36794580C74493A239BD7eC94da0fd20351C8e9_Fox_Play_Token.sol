...
contract Fox_Play_Token is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x3574fe645fE40C6b37BF7Ef5023148Cc4038AEca);
    address public marketingWalletToken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    address public constant deadAddress = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludeFromBalancesMax;
    mapping (address => bool) private _isExcludedFromMaxFees;

    uint256 private constant _feeMaxFox = 10;
    uint256 private _feeTotalForFox;
    uint256 private _feeTotalForFoxContract;
    uint256 private _feeForLiquidityFox;
    uint256 private _marketingFeeFox;
    uint256 private _burnFeeFox;
    ...
    constructor () {
        ...
        _isExcludedFromMaxFees[owner()] = true;
        _isExcludedFromMaxFees[address(this)] = true;
        
        _isExcludeFromBalancesMax[owner()] = true;
        _isExcludeFromBalancesMax[address(this)] = true;
        _isExcludeFromBalancesMax[uniswapV2Pair] = true;

        _feeForLiquidityFox = 3;
        _marketingFeeFox = 7;
        _burnFeeFox = 0;
        _feeTotalForFox = _feeForLiquidityFox.add(_marketingFeeFox).add(_burnFeeFox);
        _feeTotalForFoxContract = _feeForLiquidityFox.add(_marketingFeeFox);

        _liquidityThreshhold = 300 * 10**9 * 10**_decimals;
        _maxBalanceWalletFox = 10 * 10**9 * 10**_decimals;
    ...
    function setLiquidityFeePercent(uint256 newLiquidityFee) external onlyOwner() {
        ...
        require(newLiquidityFee.add(_burnFeeFox).add(_marketingFeeFox) <= _feeMaxFox, "Fees are too high.");
        _feeForLiquidityFox = newLiquidityFee;
        _feeTotalForFox = _feeForLiquidityFox.add(_marketingFeeFox).add(_burnFeeFox);
        _feeTotalForFoxContract = _feeForLiquidityFox.add(_marketingFeeFox);
        emit SetLiquidityFee(_feeForLiquidityFox);
    }

    function setMarketingFeePercent(uint256 newMarketingFee) external onlyOwner() {
        require(!inSwapAndLiquify, "inSwapAndLiquify");
        require(_feeForLiquidityFox.add(_burnFeeFox).add(newMarketingFee) <= _feeMaxFox, "Fees are too high.");
    ...
    function setBurnFeePercent(uint256 newBurnFee) external onlyOwner() {
        require(_feeForLiquidityFox.add(newBurnFee).add(_marketingFeeFox) <= _feeMaxFox, "Fees are too high.");
        _burnFeeFox = newBurnFee;
        _feeTotalForFox = _feeForLiquidityFox.add(_marketingFeeFox).add(_burnFeeFox);
        emit SetBurnFee(_burnFeeFox);
    }
    ...
    function _transfer(
        ...
        // Make sure that: Balance + Buy Amount <= _maxBalanceWalletFox
        if(
            from != owner() &&              // Not from Owner
            to != owner() &&                // Not to Owner
            !_isExcludeFromBalancesMax[to]  // is excludedFromMaxBalance
        ){
            require(
        ...
        if(
            ...
            _feeTotalForFoxContract > 0 &&                         // LiquidityFee + MarketingFee > 0
            from != owner() &&                                  // Not from Owner
            to != owner()                                       // Not to Owner
        ) {
            collectFees();
        ...
        // Take Fees 
        if(
            !(_isExcludedFromMaxFees[from] || _isExcludedFromMaxFees[to])
            && _feeTotalForFox > 0
        ) {
            
        	uint256 feesToContract = amount.mul(_feeTotalForFoxContract).div(100);
            uint256 toBurnAmount = amount.mul(_burnFeeFox).div(100);
    ...
    function collectFees() private lockTheSwap {
        ...
        uint256 liquidityTokensToSell = balanceOf(address(this)).mul(_feeForLiquidityFox).div(_feeTotalForFoxContract);
        uint256 marketingTokensToSell = balanceOf(address(this)).mul(_marketingFeeFox).div(_feeTotalForFoxContract);
