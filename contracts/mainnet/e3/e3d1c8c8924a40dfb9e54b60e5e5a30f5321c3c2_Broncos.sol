...
contract Broncos is IERC20, Ownable, IDistributorShare{
    
    using SafeMath for uint256;

    string _name = "Broncos";
    string _symbol = "Broncos";
    uint8 _decimals = 9;
    ...
    function setAutoExempt(address holder) external onlyOwner {
        require(holder != address(this));
        initializeDistribution(holder);   
        _isFeeExempt[holder] = true;              
    } 

	function getBroncosRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}

	function getBroncosMaxHold(uint256 _maxHold) public view returns(uint256) {
