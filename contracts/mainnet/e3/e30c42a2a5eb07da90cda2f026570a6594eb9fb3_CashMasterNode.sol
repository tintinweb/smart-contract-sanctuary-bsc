...
// File contracts/distribution/CashMasterNode.sol
...
contract CashMasterNode {
    ...
    IERC20 public constant CASH = IERC20(0xbFF6a376F54335919BC5332a16A81a07E8bDc06a);
    uint256[] public tierAllocPoints = [1 ether, 1 ether, 1 ether, 1 ether, 1 ether];
    uint256[] public tierAmounts = [50 ether, 500 ether, 1000 ether, 2000 ether, 4000 ether];
    struct User {
    ...
    function claimTreasuryRewards() external {
        if (treasury_rewards > 0) {
            CASH.safeTransfer(dev, treasury_rewards);
            treasury_rewards = 0;
        }   
    }
    ...
    function create(uint256 nodeTier, uint256 numNodes) external {
        ...
        require(CASH.balanceOf(_sender) >= tierPrice, "Insufficient balance");
        require(CASH.allowance(_sender, address(this)) >= tierPrice, "Insufficient allowance");
        CASH.safeTransferFrom(_sender, address(this), tierPrice);
    ...
    function claim() public {
        ...
        if (_rewards > 0) {
            ...
            IERC20(CASH).safeTransfer(_sender, _rewards);
    ...
    function getRewardDrip() public view returns (uint256) {
        if (lastDripTime < block.timestamp) {
            uint256 poolBalance = getCashBalancePool();
    ...
    function getCashBalance() public view returns (uint256) {
        return IERC20(CASH).balanceOf(address(this));
    }

     function getCashBalancePool() public view returns (uint256) {
        return getCashBalance().sub(total_rewards).sub(treasury_rewards);
    }
