...
interface IPearlFarm {
    function userStacks(uint256) external view returns (IUserStack memory);
    function userStacksLength() external view returns (uint256);
}
...
contract PearlToken is ERC20, Ownable {
    ...
    uint256 public lpHoldFee = 5;
    uint256 public lpStakeFee = 5;

    address public mkAddress = 0xCbb0b2e8ea6B330214c4828e037Da5d07aAE7885;
    address public vcAddress = 0x56467CD0D9Fe46ab2109e003d5a1F76C456744Eb;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;

    address public USDT = 0x55d398326f99059fF775485246999027B3197955;
    address public SHELL = 0xd5b121F7723B9e63c60809388FfA284E2d893d40;
    ...
    LpAwardCondition public lpAwardCondition = LpAwardCondition(10 ** 18, 10 ** 18);

    bool private swapping;
    FeeHelper public lpHoldFeeHelper = new FeeHelper();
    FeeHelper public lpStakeFeeHelper = new FeeHelper();
    ...
    mapping(uint256 => uint256) public burnTotalAmount;
    ...
    constructor() ERC20(name_, symbol_)  {
        ...
        excludeFromFees(address(lpHoldFeeHelper), true);
        excludeFromFees(address(lpStakeFeeHelper), true);
    ...
    function setMiningStartTime(uint _miningStartTime) public onlyOwner {
        miningStartTime = _miningStartTime;
    }

    function setPearlFarm(address pearlFarm_) public onlyOwner {
        pearlFarm = IPearlFarm(pearlFarm_);
    }
    ...
    function getMiningId() public view returns (uint256) {
        if(miningStartTime >= block.timestamp || miningStartTime == 0) return 0;
        return block.timestamp.sub(miningStartTime).div(30 days).add(1);
    }
    ...
    function _transfer(
        ...
        if(
            amount == 0 || 
            from == address(lpHoldFeeHelper) ||
            from == address(lpStakeFeeHelper) ||
            swapping
        ) { 
        ...
        if(takeFee) {
            ...
            bool canSwap = contractTokenBalance >= swapTokensAtAmount && from != uniswapV2Pair;
            if(canSwap) {
                swapping = true;
   ...
   function processHoldLpReward() private {
        uint256 shareholderCount = lpInterest.tokenHolder.length();
        if (shareholderCount == 0) return;

        uint256 nowbanance = balanceOf(address(lpHoldFeeHelper));
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            if ( balanceOf(address(lpHoldFeeHelper)) < amount ) break;

            if(amount > 0 && block.timestamp.sub(lastBuyAt[shareholder]) <= 24 * 60 * 60){
                lpHoldFeeHelper.transferToken(address(this), shareholder, amount);
            }
   ...
   function processStakeLpReward() private {
        uint256 shareholderCount = pearlFarm.userStacksLength();
        if (shareholderCount == 0) return;

        uint256 nowbanance = balanceOf(address(lpStakeFeeHelper));
        if (nowbanance == 0) return;
        uint256 iterations = 0;
        uint index = lpStakeIndex;
        uint sendedCount = 0;
        uint sendCountLimit = lpInterest.sendCount;

        uint256 lpTotalSupply = lpInterest.token.totalSupply();
        uint256 tokenAmountPair = balanceOf(uniswapV2Pair);

        uint ts = lpInterest.token.balanceOf(address(pearlFarm));
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            IUserStack memory userStake = pearlFarm.userStacks(index);
            address shareholder = userStake.account;
            uint256 amount = nowbanance.mul(userStake.amount).div(ts);
            if ( balanceOf(address(lpStakeFeeHelper)) < amount ) break;

            if(amount > 0 && tokenAmountPair.mul(userStake.amount).div(lpTotalSupply) >= 10**18){
                lpStakeFeeHelper.transferToken(address(this), shareholder, amount);
            }
            sendedCount ++;
            iterations++;
            index ++;
    ...
    function takeAllFee(address from, address to, uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(feeRate > 0){
            ...
            if(burnFeeAmount > 0){
                uint256 miningId = getMiningId();
                if(miningId > 0) burnTotalAmount[miningId] += burnFeeAmount;
                super._transfer(from, burnAddress, burnFeeAmount);
            }

            uint256 lpHoldFeeAmount = feeAmount.mul(lpHoldFee).div(initFeeRate);
            if(lpHoldFeeAmount > 0) super._transfer(from, address(lpHoldFeeHelper), lpHoldFeeAmount);

            uint256 lpStakeFeeAmount = feeAmount.mul(lpStakeFee).div(initFeeRate);
            if(lpStakeFeeAmount > 0) super._transfer(from, address(lpStakeFeeHelper), lpStakeFeeAmount);

            uint256 otherFeeAmount = feeAmount.sub(burnFeeAmount).sub(lpHoldFeeAmount).sub(lpStakeFeeAmount);
