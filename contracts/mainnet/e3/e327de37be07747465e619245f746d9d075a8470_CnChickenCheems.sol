...
contract CnChickenCheems is Context, Ownable {
    using Address for address;
    
    string private _name = unicode"Chinese Chicken Cheems";
    string private _symbol = unicode"CHCHCH";
    ...
    function addToBotList(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _isBotlisted[addresses[i]] = true;
      }
    }

    function removeFromBotList(address account) external onlyOwner {
        _isBotlisted[account] = false;
    }
    ...
    function _transfer(
        ...
        require(!_isBotlisted[from] && !_isBotlisted[to], "This address is botlisted");
