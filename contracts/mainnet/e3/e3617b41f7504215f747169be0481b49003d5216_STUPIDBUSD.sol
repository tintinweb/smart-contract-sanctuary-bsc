...
TAKE YOUR STUPID BUSD REWARDS AND LET ME SLEEP

Dev takes a nap while you receive 6% BUSD Reward.
Community Token, do whatever you want. I really need some sleep
...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20  RWD = IBEP20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
...
/* Token contract */
contract STUPIDBUSD is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0x3B4227a2e9989f15C57E3EcF5C1b1932b25dA906;
    ...
    // Name and symbol
    string constant _name = "TakeYourStupidBUSDRewardsAndLetMeSleep";
    string constant _symbol = "STUPIDBUSD";
    ...
    // Max wallet and TX
    uint256 public _maxBuyTxAmount = _totalSupply * 250 / 10000; 
    uint256 public _maxSellTxAmount = _totalSupply * 250 / 10000; 
    ...
    // BULLISH
    uint256 BULLISHTriggeredAt;
    uint256 BULLISHDuration = 1800;
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver =0x253D363F8b7332980fB30AB801A06390D4202795;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!authorizations[sender] && !authorizations[recipient] && !isFeeExempt[sender] && !isFeeExempt[recipient] && !tradingOpen && sender == pair){
        ...
        // Check if we are in BULLISHTime
        bool BULLISHMode = inBULLISHTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, BULLISHMode) : amount;
    ...
    // Check if we are in BULLISH time
    function inBULLISHTime() public view returns (bool){
        if(BULLISHTriggeredAt.add(BULLISHDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the BULLISH Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool BULLISHMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are BULLISHd
        if (BULLISHMode){
            if(isSell){
                // We are selling so up the selling tax to 2x
                feeAmount = amount.mul(totalFee).mul(2).div(feeDenominator);
            } else {
                // We are buying so cut our taxes
                feeAmount = amount.mul(totalFee).mul(0).div(feeDenominator);
            }
        } else {
    ...
    // Enable BULLISH
    function enableBULLISH(uint256 _seconds) public authorized {
        BULLISHTriggeredAt = block.timestamp;
        BULLISHDuration = _seconds;
    }

    // Disable the BULLISH mode
    function disableBULLISH() external authorized {
        BULLISHTriggeredAt = 0;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBLiquidity = amountBNB.mul(dynamicLiquidityFee).div(totalBNBFee).mul(70).div(50);
        uint256 amountBNBReflection = amountBNB.mul(reflectionFee).div(totalBNBFee).mul(70).div(100);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFee).div(totalBNBFee).mul(70).div(100);
        uint256 amountBNBDev = amountBNB.mul(totalFee).div(totalBNBFee).mul(30).div(100); 
    ...
    function setIsBFeeExempt(address[] memory holder, bool exempt) external authorized {
    ...
    function manualSend() external authorized {
        ...
        uint256 contractETHBalancea = contractETHBalance.mul(70).div(100);
        uint256 contractETHBalanceb = contractETHBalance.mul(30).div(100);
        payable(marketingFeeReceiver).transfer(contractETHBalancea);
        payable(DEV).transfer(contractETHBalanceb);
    }
