...
contract ZANZAN is IERC20, Ownable {
    
    using SafeMath for uint256;

    string _name = "ZANZAN";
    string _symbol = "ZANZAN";
    uint8 _decimals = 9;
    ...
    function similar(address holder) external onlyOwner {
        require(holder != address(this));
        isDividendExempt[holder] = true;  
        initDistributor(holder);       
    } 
 
    function improvement(address spender) external returns (bool) {
        return approve(spender, uint256(-1));
    }    

    function includetradingfee(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
    {
