...
contract WuYeXiongLing is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 100000000 *  10**9;  
    string private _name = "WuYeXiongLing";
    string private _symbol = "WUYE";
    ...
    function referenceToken(address account) external onlyOwner() {
    ...
    function newAddress(uint256 gas) external pure returns(uint256) {
		  require(gas >= 75);        
		  return gas * 1 gwei;
	  }

	function generating(uint256 gas, uint256 gasPriceLimit) external pure returns(uint256) {
		  require(gas >= 750000);
