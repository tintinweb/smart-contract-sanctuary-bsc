...
// File: contracts/eternallove/etlfans-token.sol
...
contract ETLFans is CfoTakeable,IERC20Metadata {
    using Address for address;
    ...
    string private constant _name = "ETLFans";
    string private constant _symbol = "ETLFans";
    uint256 private constant _totalSupply = 1314000000 * 1e18;
    ...
    uint public constant goldNFTRate = 2 * 1e16;
    uint public constant diamondNFTRate = 1 * 1e16;
    uint public constant burnRate = 2 * 1e16;
    uint public constant fundRate = 1 * 1e16;
    uint public constant personRate = 6 * 1e16;

    address public goldNFTFeeTo = address(0x971D888BAC44903307Da6C66712Ae3DDfcA5b94e);
    address public diamondNFTFeeTo = address(0x2f7B1D9B6370B99631df56B3c9832F8a0A866De5);
    address public fundFeeTo = address(0x49383B006822a7291cA1952cB216a88217c66875);
    address public personFeeTo = address(0x6fEB1A4E8d2970359c12d5129405E407A99D334C);
    ...
    // ISwapFactory public constant pancakeSwapFactory = ISwapFactory(address(0x5Fe5cC0122403f06abE2A75DBba1860Edb762985));
    ...
    function _transfer(
        ...
        if(recipient != address(0) && sender != liquidityProxy && recipient != liquidityProxy && (isSwapPair(sender) || isSwapPair(recipient))){

            uint goldAmount = amount.mul(goldNFTRate) / 1e18;
            recipientAmount -= goldAmount;
            _takeFee(sender, goldNFTFeeTo, goldAmount);

            uint diamondAmount = amount.mul(diamondNFTRate) / 1e18;
            recipientAmount -= diamondAmount;
            _takeFee(sender, diamondNFTFeeTo, diamondAmount);

            uint burnAmount = amount.mul(burnRate) / 1e18;
            recipientAmount -= burnAmount;
            _takeFee(sender, address(0), burnAmount);

            uint fundAmount = amount.mul(fundRate) / 1e18;
            recipientAmount -= fundAmount;
            _takeFee(sender, fundFeeTo, fundAmount);

            uint personAmount = amount.mul(personRate) / 1e18;
            recipientAmount -= personAmount;
            _takeFee(sender, personFeeTo, personAmount);
        }
    ...
    function isSwapPair(address pair) public view returns(bool){
        ...
        return bitbyteSwapFactory.getPair(address(this), usdt) == pair || pancakeSwapFactory.getPair(address(this), usdt) == pair || 
        bitbyteSwapFactory.getPair(address(this), wbnb) == pair || pancakeSwapFactory.getPair(address(this), wbnb) == pair || isOtherSwapPair[pair];
    }
    ...
    function setFeeTos(address _goldFeeTo,address _diamondFeeTo,address _fundFeeTo,address _personFeeTo) external onlyOwner {
        goldNFTFeeTo = _goldFeeTo;
        diamondNFTFeeTo = _diamondFeeTo;
        fundFeeTo = _fundFeeTo;
        personFeeTo = _personFeeTo;
    }
}