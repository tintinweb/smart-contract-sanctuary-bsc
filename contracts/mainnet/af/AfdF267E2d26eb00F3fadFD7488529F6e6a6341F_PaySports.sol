...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x4983eF5E31e7A9b7a434DeA27C4B2AEfBF75267F;
...
contract PaySports is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x7f14444a5295b28d7863085f3Acb4B49498254c9;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1520000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Pay Sports";
    ...
    function payment(
      ...
      uint256[] calldata royalties
        ...
        require(paymentId > 0, "Payment id is invalid. (ERROR-0001)");

        require(nftContract != address(0), "NFT contract address is zero address. (ERROR-0002)");
        require(nftId > 0, "NFT id is invalid. (ERROR-0003)");

        require(payer != address(0), "Payer address is zero address. (ERROR-0004)");
        require(payer == msg.sender, "Payer address is invalid (ERROR-0005)");

        uint count = recipients.length;
        require(count > 0, "Receivers addresses is invalid (ERROR-0006)");
        require(recipients.length == royalties.length, "Royalties values is invalid (ERROR-0007)");

        require(amount > 0, "Payment amount must be greater than zero (ERROR-0008)");
        require(amount <= balanceOf(msg.sender), "Insufficient balance to complete payment transaction. (ERROR-0009)");

        uint i = 0;
        uint256 totalRoyalties = 0;
        for (i; i < count; i++) {
            require(recipients[i] != address(0), "Recipient address is zero address. (ERROR-0010)");
            require(royalties[i] >= 0, "Royalties value is invalid. (ERROR-0011)");

            totalRoyalties = totalRoyalties + royalties[i];
        }

        require(totalRoyalties == 100, "Total royalties is invalid. (ERROR-0012)");

        i = 0;
        for (i; i < count; i++) {
            _transfer(msg.sender, recipients[i], amount * royalties[i] / 100);
        }
