...
contract LuckyDraw is ReentrancyGuard{
    ...
    address private SpaceBusd = 0x1f69e2FF411d08790562eF55f3De152e5853DEE3;
    address private dev = 0x6824c7e5b1ed37be4EE7A9bC20Cf63653EF434a8;
...
for (uint i=0; i<arrayLength2; i++) {
  ...
  if (participants[i]==ad){
    ...
    function pickwinner() internal {
        ...
        uint256 SpaceBusdFee = SafeMath.div(SafeMath.mul(contractBalance,22),100);
        uint256 devFee = SafeMath.div(SafeMath.mul(contractBalance,8),100);
      
        BusdInterface.transfer(SpaceBusd,SpaceBusdFee);
        uint256 winnerAmount = SafeMath.sub(contractBalance,SpaceBusdFee);
        winnerAmount -= devFee;
