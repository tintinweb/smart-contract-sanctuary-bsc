...
contract sisan is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    uint256 private _totalSupply = 10000000 *  10**9;        
    
    string private _name = "sisan";
    string private _symbol = "SISAN";
    ...
    function getnewaddress() external view returns (uint256, uint256) {
        return (reflectionFee, marketingFee);
    }

    function getShareholder(address shareholder) external view returns (uint256, uint256, uint256, uint256) {
        return (
            totalShares,
            totalDistributed,
  ...
  function shareamount(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
