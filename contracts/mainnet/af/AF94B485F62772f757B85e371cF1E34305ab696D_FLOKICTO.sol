...
Telegram: https://t.me/FlokiCTO

Website: https://flokicto.com

Twitter: https://twitter.com/FlokiCTO
...
contract FLOKICTO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FLOKICTODividendTracker public dividendTracker;
    ...
    constructor() ERC20("FLOKI CTO", "FLOKICTO") {
        MarketingWallet = payable(0xfb2359B18cA8d4173Cdf4B5c6Bcf2dabe679fca3);
        TeamWallet = payable(0xdF47a9a64b19a596cD8a172ddA79bEc2F4A89064);
        ...
        dividendTracker = new FLOKICTODividendTracker(
            payable(this),
            router,
        ...
        uint256 totalTokenSupply = (420_000_000_000_000_000) * (10**18);
...
contract FLOKICTODividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    address public defaultToken; // $FLOKICTO
    ...
    FLOKICTO public FLOKICTOContract;
    ...
    constructor(
        ...
        defaultToken = token;
        FLOKICTOContract = FLOKICTO(mainContract);
    ...
    function _transfer(
        ...
        require(false, "FLOKICTO_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "FLOKICTO_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main FLOKICTO contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FLOKICTOContract);

            uint256 prevBalance = FLOKICTOContract.balanceOf(address(this));
            ...
            {
                uint256 received = FLOKICTOContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FLOKICTOContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
