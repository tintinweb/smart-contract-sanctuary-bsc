...
contract StrategyHonorChefLP is StratManager, FeeManager, GasThrottler {
    ...
    uint256 public maxHarvestInterval = 3600 * 24;
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            IMasterChef(chef).deposit(poolId, wantBal, address(this));
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(chef).withdraw(poolId, _amount.sub(wantBal), address(this));
    ...
    function beforeDeposit() external virtual override {
        if (harvestOnDeposit && block.timestamp > lastHarvest + maxHarvestInterval ) {
            require(msg.sender == vault, "!vault");
            _harvest(tx.origin);
        }
    }
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IMasterChef(chef).harvest(poolId, address(this));
        ISimpleStaking(staking).emergencyWithdraw();
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 feeAmount = IERC20(output).balanceOf(address(this)).mul(callFee).div(CALL_MAX);
        IERC20(output).safeTransfer(callFeeRecipient, feeAmount);
    ...
    function setHarvestInterval(uint256 interval) external onlyManager {
        maxHarvestInterval = interval;
    }
    ...
    function retireStrat() external {
        ...
        IMasterChef(chef).emergencyWithdraw(poolId, address(this));
...
abstract contract FeeManager is StratManager {
    uint constant public CALL_FEE_FAP = 100;
    uint constant public CALL_MAX = 10000;
...
contract GasThrottler {
    ...
    address public gasprice = address(0xDbD5D51EB42fE276cde6B97be8a3b6D6F4412730);
