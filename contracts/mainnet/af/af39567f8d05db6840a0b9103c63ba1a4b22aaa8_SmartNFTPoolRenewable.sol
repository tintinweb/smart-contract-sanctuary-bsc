...
// Farmageddon Renewable NFT staking Contract
...
contract SmartNFTPoolRenewable is IERC721Receiver ,Ownable, ReentrancyGuard {
    ...
    uint256 public subFee =  250000000000000000;    
    uint256 public subEndBlock = 0;
    uint256 public subLength = 1209600;
    ...
    // Vault for token Fee Collection
    
    uint256 public tokenFee = 0;
    address public payToken = 0xd51237A6F3219d186f0C8d8Dd957b1Bcb3Ce5d48;
   ...
   // Subscription
    ...
    function setSubFees(uint256 _subFee, uint256 _subLength) external onlyOwner {
        require(_subFee <= 100000000000000000, "Max 1 bnb Subscription Fee");
        subFee = _subFee;
    ...
    function RenewOrExtendSubscriptionSixWeeks() external payable onlySub {
        address treasury = NFTPoolFactory(SMART_CHEF_FACTORY).treasury();
    ...
    */
    ...
    function startNewPoolOrRound( uint256 _startInDays, uint256 _poolLengthDays ) external onlySub {
        ...
        totalNewReward = 0;
        if(subEndBlock == 0) subEndBlock = bonusEndBlock;
        isInitialized = true;
    ...
    function setNextRewardToken(IBEP20 _rewardToken) external onlySub {
        ...
        roundInfo[currentRound].rwdToken = address(rewardToken);
        RoundInfo storage round = roundInfo[currentRound];
        round.accTokenPerShare = 0;
    ...
    function stake(uint256[] memory tokenIds) public {
        ...
        address vault = NFTPoolFactory(SMART_CHEF_FACTORY).vault();
        require(PAY.transferFrom(msg.sender, vault, tokenFee * tokenIds.length),"Not Enough Tokens");
        ...
        usersRewardDebt[msg.sender][currentRound] = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR);
    }
    ...
    */
    ...
    // Claim rewards or Harvest rewards
     function claim() internal {
        ...
        uint256 pending = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][currentRound]);
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 adjustedTokenPerShare =
                round.accTokenPerShare.add(cakeReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
            return user.amount.mul(adjustedTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][currentRound]);
        } else {
            return user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][currentRound]);
        }
    }
    ...
    function prevPendingRewards(address _user) public view returns (address[] memory _rwdToken, uint256[] memory _amount, bool hasRewards) {
        UserInfo storage user = usersInfo[_user];
        _rwdToken = new address[](currentRound);
        _amount = new uint256[](currentRound);
        hasRewards = false;
        for (uint i = 0; i < currentRound; i++) {
            RoundInfo memory round = roundInfo[i];
            _rwdToken[i] = round.rwdToken;
            _amount[i] = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][i]);
    ...
    function claimPrevRewards() public {
        ...
        for (uint i = 0; i<currentRound; i++){
            ...
            uint256 pending = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][i]);
            
            if (pending > 0) {
                IBEP20 rwd = IBEP20(round.rwdToken);
                try rwd.transfer(address(msg.sender), pending) {} catch {}
                usersRewardDebt[msg.sender][i] = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR);
            }
        }
    }
    ...
    function getMainPoolInfo() external view returns (
        ...
        uint256 _tokenFee
          ...
          return (rewardPerBlock, totalStaked, startBlock, bonusEndBlock, payToken, tokenFee);  
        }

    function _updateNewRewards() internal {
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(rewardPerBlock);
        round.accTokenPerShare = round.accTokenPerShare.add(cakeReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
    ...
    function setPayToken(address newPayToken) public onlyOwner {
...
contract NFTPoolFactory is Ownable {
    ...
    address public vault = 0xDF7ca526F053a661684a6FE41d53aC758351e051;
    address public treasury = 0xd8d95A915efd4b4D0400215bA594945C2aaf7959;
    ...
    function createNewBlanks(uint howMany) external onlyOwner {
    require(howMany < 4, "4 Max at a time");
        for (uint i = 0; i < howMany; i++){
            SmartNFTPoolRenewable pool = new SmartNFTPoolRenewable();
            _SmartPools.push(pool);
            SmartNFTPoolRenewable(pool).transferOwnership(Controller);
        }
    }
    
    function createNFTPool(IFarmageddonNFT nftContract, IBEP20 partnerToken,address PayToken , uint256 payTokenAmount) external onlyOwner {
            SmartNFTPoolRenewable pool = new SmartNFTPoolRenewable();
            _SmartPools.push(pool);
            SmartNFTPoolRenewable(pool).setupTokens(nftContract,partnerToken);
            SmartNFTPoolRenewable(pool).setPayToken(PayToken);
            SmartNFTPoolRenewable(pool).setTokenFee(payTokenAmount);
            SmartNFTPoolRenewable(pool).transferOwnership(Controller);
