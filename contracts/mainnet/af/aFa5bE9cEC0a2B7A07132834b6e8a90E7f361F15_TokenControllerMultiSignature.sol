...
contract TokenControllerMultiSignature is AccessControl {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _signaturer.length; ++i) {
            require(
                !hasRole(SIGNER_ROLE, _signaturer[i]),
                "address already signature"
            );
    ...
    function acceptAddMinter(
        ...
        require(
            block.timestamp >= _requestAt && block.timestamp <= _expireAt,
            "illegal time"
        );
        require(
            hasRole(SIGNER_ROLE, _msgSender()),
        ...
        require(
            !signatureRecord[hash][_msgSender()],
            "signature already accepted"
        );
        signatureNum[hash] = signatureNum[hash] + 1;
        signatureRecord[hash][_msgSender()] = true;
        emit AcceptAddMinter(
            _requester,
            _minter,
            _maxAmount,
            _requestAt,
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(
                ...
                bytes32(0),
                _salt,
                timeLocker.getMinDelay()
            );
        }
    }

    function executeAddMinter(
        address _minter,
        uint _maxAmount,
        bytes32 _salt
    ) external {
    ...
    function applyDelMinter(
        address minter,
        uint expireAt,
        bytes32 salt
    ) external {
        bytes32 hash = keccak256(
            abi.encode(_msgSender(), minter, block.timestamp, expireAt, salt)
        );
        require(!transactions[hash], "apply already request");
        transactions[hash] = true;
        signatureNum[hash] = 0;
    ...
    function acceptDelMinter(
        ...
        bytes32 hash = keccak256(
            abi.encode(_requester, _minter, _requestAt, _expireAt, _salt)
        );
        require(transactions[hash], "apply not exists");
        require(
            !signatureRecord[hash][_msgSender()],
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(
                address(token),
                0,
                abi.encodeWithSignature("delMinter(address)", _minter),
                bytes32(0),
                _salt,
                timeLocker.getMinDelay()
            );
    ...
    function applySetPendingOwner(
        address owner,
        uint expireAt,
        bytes32 salt
    ) external {
        bytes32 hash = keccak256(
            abi.encode(_msgSender(), owner, block.timestamp, expireAt, salt)
        );
        require(!transactions[hash], "apply already request");
        transactions[hash] = true;
        signatureNum[hash] = 0;
        emit ApplySetPendingOwner(
            msg.sender,
            owner,
            block.timestamp,
            expireAt,
    ...
    function acceptSetPendingOwner(
        address _requester,
        address _owner,
        uint _requestAt,
        uint _expireAt,
        ...
        bytes32 hash = keccak256(
            abi.encode(_requester, _owner, _requestAt, _expireAt, _salt)
        );
        require(transactions[hash], "apply not exists");
        require(
            !signatureRecord[hash][_msgSender()],
        ...
        if (signatureNum[hash] >= SIGNATURE_MIN_NUM) {
            timeLocker.schedule(
                address(token),
                0,
                abi.encodeWithSignature("setPendingOwner(address)", _owner),
                bytes32(0),
                _salt,
                timeLocker.getMinDelay()
            );
