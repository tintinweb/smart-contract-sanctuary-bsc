...
contract Monteno is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = 'Monteno';
    string public _symbol = 'MTO';
    ...
    uint256 public MTOInsuranceFundFee = 50;
    uint256 public firePitFee = 10;
    uint256 public totalFee = liquidityFee.add(gameTreasuryFee).add(MTOInsuranceFundFee).add(firePitFee);
    ...
    address public MTOInsuranceFundReceiver;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 700000 * 10**DECIMALS;
    uint256 private constant MAX_SUPPLY = 700000000 * 10**DECIMALS;
    ...
    constructor(
        ...
        MTOInsuranceFundReceiver = msg.sender;
        infernoPit = ZERO;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_gameTreasuryFee.add(MTOInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(gameTreasuryReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(gameTreasuryFee).div(gameTreasuryFee.add(MTOInsuranceFundFee)),
            gas: 30000
        }('');
        (success, ) = payable(MTOInsuranceFundReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(MTOInsuranceFundFee).div(
                gameTreasuryFee.add(MTOInsuranceFundFee)
            ),
            gas: 30000
        }('');
    }
    ...
    function setFeeReceivers(
        ...
        address _MTOInsuranceFundReceiver,
        address _infernoPit
    ) external onlyOwner {
        gameTreasuryReceiver = _gameTreasuryReceiver;
        MTOInsuranceFundReceiver = _MTOInsuranceFundReceiver;
    ...
    function setFee(
        ...
        uint256 _MTOInsuranceFundFee,
        uint256 _firePitFee,
        uint256 _sellFee
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        gameTreasuryFee = _gameTreasuryFee;
        MTOInsuranceFundFee = _MTOInsuranceFundFee;
        firePitFee = _firePitFee;
        totalFee = liquidityFee.add(gameTreasuryFee).add(MTOInsuranceFundFee).add(firePitFee);
        sellFee = _sellFee;
