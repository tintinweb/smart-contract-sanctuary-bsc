...
// PYEChef is the master of Cherry. He can make Cherry and he is a fair guy.
//
...
contract CherryChefV2 is Ownable {
    using SafeMath for uint256;
    ...
    function updateCherryPerBlock(uint256 _cherryPerBlock) external onlyOwner {
        cherryPerBlock = _cherryPerBlock;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cherryReward = multiplier.mul(cherryPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        cherry.mint(devaddr, cherryReward.mul(devfee).div(10000));
