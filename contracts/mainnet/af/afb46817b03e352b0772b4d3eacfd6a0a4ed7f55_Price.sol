...
contract Price {
    ...
    constructor() {
        ...
        commonDEXes.push(0x0841BD0B734E4F5853f0dD8d7Ea041c241fb0Da6);
    }
    ...
    function overflowHandler(address poolAddr) public view returns (uint256,uint256){
        ...
        if(token0Decimals > token1Decimals){
            k0 = token0Decimals - token1Decimals;
        }
        else if(token1Decimals > token0Decimals){
            k0 = token1Decimals - token0Decimals;
        }
            
        if(Q6496 < 10 ** 38 && Q6496 > 10 ** 14){

            if(Q6496 > 10 ** 33){

                if(token0Decimals > token1Decimals){
                    priceToken0 = Q6496 ** 2 / 2 ** 192 * 10 ** 18 * 10 ** k0;
                    priceToken1 = 2 ** 192 * 10**18 / Q6496 ** 2 / 10 ** k0;
                }
                else if(token1Decimals > token0Decimals){
                    ...
                    priceToken0 = Q6496 ** 2 / 2 ** 192 * 10 ** 18;
                    priceToken1 = 2 ** 192 * 10**18 / Q6496 ** 2;
                }
            }
            else if(Q6496 > 10 ** 27){

                if(token0Decimals > token1Decimals){
                    priceToken0 = Q6496 ** 2 * 10 ** 9 / 2 ** 192 * 10 ** 9 * 10 ** k0;
                    priceToken1 = 2 ** 192 * 10**18 / Q6496 ** 2  / 10 ** k0;
                }
                else if(token1Decimals > token0Decimals){
                    priceToken0 = Q6496 ** 2 * 10 ** 9 / 2 ** 192 * 10 ** 9 / 10 ** k0;
                    ...
                    priceToken0 = Q6496 ** 2 * 10 ** 9 / 2 ** 192 * 10 ** 9;
                    priceToken1 = 2 ** 192 * 10**18 / Q6496 ** 2;
                }
            }
            else{
                ...
                if(token0Decimals > token1Decimals){
                    priceToken0 = Q6496 ** 2 * 10**18 / 2 ** 192 * 10 **k0;
                    priceToken1 = 2 ** 192 * 10**18 /  Q6496 ** 2 / 10 ** k0;
                }
                else if(token1Decimals > token0Decimals){
                    priceToken0 = Q6496 ** 2 * 10**18 / 2 ** 192 / 10 **k0;
                    ...
                    priceToken0 = Q6496 ** 2 * 10**18 / 2 ** 192;
                    priceToken1 = 2 ** 192 * 10**18 /  Q6496 ** 2;
                }
    ...
    function returnV3Prices(address poolAddr) public view returns (uint256,uint256,uint256,uint256,uint256,uint256){
        ...
        (priceToken0,priceToken1) = overflowHandler(poolAddr);
            ...
            LP0BalanceToUSD = IERC20(pool.token0()).balanceOf(address(pool)) * 10 **(18 -  IERC20(pool.token0()).decimals()) * priceToken0USD / 10 ** 18;

            priceToken1USD = priceToken1 * getBasicPrice(pool.token0()) / 10 ** 18;
            LP1BalanceToUSD = IERC20(pool.token1()).balanceOf(address(pool)) * 10 **(18 -  IERC20(pool.token1()).decimals()) * priceToken1USD / 10 ** 18;
        ...
        for (uint8 i = 0; i < commonTokens.length; i++) {
            for (uint8 j = 0; j < feesV3.length; j++){
                ...
                if (pairAddress != address(0)) {
                    ...
                    if (tokenAddress == pool.token0() && IERC20(pool.token0()).balanceOf(address(pool)) != 0) {

                        (,,pairPrice,,,) = returnV3Prices(address(pool));
                        (,,,,,pairLiquidity) = returnV3Prices(address(pool));
                        if(pairLiquidity < 600 * 10 ** 18){
                            pairLiquidity = 0;
                        }
                        totalLiquidity += pairLiquidity;

                    } else if (IERC20(pool.token1()).balanceOf(address(pool)) != 0) {
        ...
        for (uint8 i = 0; i < commonTokens.length; i++) {
            ...
            if (pairAddress != address(0)) {
                ...
                if (tokenAddress == pair.token0() && reserve0 != 0) {
                    reserve0 = reserve0 * 10 ** (18 - IERC20(pair.token0()).decimals());
                    reserve1 = reserve1 * 10 ** (18 - IERC20(pair.token1()).decimals());
        ...
        returns (uint256 price)
    {
        ...
        if(address(pair) != address(0)){
            ...
            uint8 decimal0 = IERC20(pair.token0()).decimals();
            uint8 decimal1 = IERC20(pair.token1()).decimals();

            if (tokenAddress == pair.token0()) {
                return (reserve1 * 10 ** 18 * 10 ** (18 - decimal1))  / (reserve0 * 10 ** (18 - decimal0));
            } else {
                return (reserve0 * 10 ** 18 * 10 ** (18 - decimal0)) / (reserve1 *  10 ** (18 - decimal1));
            }
        }
        else{
            return 0;
