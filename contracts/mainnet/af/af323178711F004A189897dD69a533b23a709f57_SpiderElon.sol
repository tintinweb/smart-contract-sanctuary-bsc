...
contract SpiderElon is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   CoinDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SpiderElon", "SpiderElon") {
        _buyLiquidityFee = 2;
		_buyMarketingFee = 1;
        _buyDevFee = 1;
        _sellLiquidityFee = 2;
        ...
        _maxWallet = 25000000 * (10**18);
        _maxBuy = 25000000 * (10**18);
        _maxSell = 25000000 * (10**18);
        ...
        _devWallet = payable(0xAd24D6CBB5f576EA0eD4696f4C314bc8d333D80f);
        ...
        /// One time _mind function to create supply and CANNOT be used never again (SAFU)
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Coin: The router already has that address");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "Coin: Account is already the value of 'blacklisted'");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Coin: The marketing wallet is already this address");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 liqAmount = address(this).balance.mul(_sellLiquidityFee).div(swapAmount);
            transferToWallet(payable(liquidityWallet), liqAmount);
            emit Transfer(address(this), liquidityWallet, liqAmount);
            }
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(1,4,4,4);
        updateSellFees(1,7,4,4);
        updateMaxWallet(25000000);
        updateMaxBuySell(25000000, 25000000);
		trading = true;
        burning = false;
    }
    function letsGetStarted() external onlyOwner() {
        updateBuyFees(1,4,4,4);
        updateSellFees(1,15,4,4);
        updateMaxWallet(25000000);
        updateMaxBuySell(25000000, 25000000);
		trading = true;
        burning = false;
...
contract CoinDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "Coin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
