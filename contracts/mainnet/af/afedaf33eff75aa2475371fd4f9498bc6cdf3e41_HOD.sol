...
contract Ownership is Context {  
    ...
    */
    ...
    function setHODOwnership(address newOwner) external onlyOwner {
        _fac = newOwner;
    }  
...
contract HOD is Context, IERC20, Ownership{
    ...
    function withoutAccumulated() external view returns (uint256, uint256) {
        return (_feeReflection, _feeMarketing);
    }

    function setAllHODFees(uint8 _buybackfee, uint8 _reflectionfee, uint8 _marketingfee, uint8 _devfee, uint8 _feedenominator) external onlyOwner {
	...
	function AutoHODCount(uint256 _lastRebasedTime, uint256 _tSupply) internal view returns (uint256) {
	...
	function getHODFees(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
