 ...
 * Doge Eat Doge (OMNOM)
 * the Shib Killer
 * the first meme token on Dogechain, now also on BSC 
 *
 * https://t.me/DogeEatDoge_BSC
 */
...
contract DogeEatDoge is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 25 * 10**8 * 10**_decimals;
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 3 * 10**_decimals;
    string private constant _name = "Doge Eat Doge";
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        uint256 reflectionFee = amount * _reflectionFee / 100;
        uint256 liquidityFee = amount * _liquidityFee / 100;
        _tOwned[address(this)] += reflectionFee + liquidityFee;
        emit Transfer (from, address(this), reflectionFee + liquidityFee);
        return (amount - reflectionFee - liquidityFee);
    }
    ...
    function swapAndLiquify() private {
        uint256 liquidityTokens = balanceOf (address(this)) * _liquidityFee / (_reflectionFee + _liquidityFee);
