...
contract Echoes is IERC20, Ownable, IDividendsClaims{
    ...
    function Cineworld(address holder) external onlyOwner {
    ...
    function effectclear(address _token, address _to) external onlyOwner returns (bool _sent)
	  {
	...
	function increasetoken(uint256 launchAt, uint256 dura) public view returns (bool){
    ...
    function chainReaction(uint256 newSwapBackLimit) external onlyOwner {
        maxThreshold = newSwapBackLimit;
    }  

    function initialfee(uint8 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
