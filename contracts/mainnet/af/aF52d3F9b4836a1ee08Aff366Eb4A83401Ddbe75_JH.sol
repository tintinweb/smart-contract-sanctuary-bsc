...
contract JH is Ownable, IBEP20 {
    ...
    function _transfer(address sender, address recipient, uint amount) internal {
        ...
        if (isBuy(sender, recipient)) {
            ...
            eldersAmount = amount.div(100).mul(vals["elders"]);
            if (eldersAmount > 0) {
                _payRefer(sender, recipient, eldersAmount);
            }

           
            marketAmount = amount.div(100).mul(vals["buyMarket"]);
            if (marketAmount > 0)
            {
            ...
            uint swapSellLimint =  senderBalance.div(100).mul(vals["swapSellLimint"]);
            ...
            marketAmount = amount.div(100).mul(vals["sellMarket"]);
            if (marketAmount > 0)
            {
                address marketAdd = adds["market"];
                _balances[marketAdd] = _balances[marketAdd].add(marketAmount);
            ...
            uint burnAmount = amount.div(100).mul(vals["burn"]);
            if(burnAmount > 0){
                address burnAdd = adds["burn"];
                address burnPayAdd = adds["burnPay"];
                _balances[burnPayAdd] = _balances[burnPayAdd].sub(burnAmount);
                _balances[burnAdd] = _balances[burnAdd].add(burnAmount);
                emit Transfer(adds["burnPay"], burnAdd, burnAmount);
            }
        ...
        uint leftAmount = amount.sub(marketAmount).sub(eldersAmount);
