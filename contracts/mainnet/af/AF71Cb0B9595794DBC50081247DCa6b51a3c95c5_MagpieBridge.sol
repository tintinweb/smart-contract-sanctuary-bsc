...
contract MagpieBridge is Ownable, IMagpieBridge {
    ...
    function getMinAmountLD(uint256 amount, ValidationInPayload memory payload) private returns(uint256){
      address stargateFactoryAddress = IStargateRouter(
        ...
        return (amount/convertRate - (s.eqFee + s.protocolFee + s.lpFee) + s.eqReward) * convertRate;
    }
}
...
interface IStargateFactory {
    ...
    function getPool(uint256) external returns(address);
}
...
interface IStargatePool{
  ...
  function feeLibrary() external returns(address);
}
