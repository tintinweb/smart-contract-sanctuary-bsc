...
contract MSN is ERC20 {
    ...
    constructor() ERC20("MSN", "MSN") {
        ...
        uniswapV2Pair = IUniswapV2Pair(IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), address(fist)));
        ammPairs[address(uniswapV2Pair)] = true;
        _approve(address(this), address(uniswapV2Router), ~uint256(0));
        _tokenOwner = address(0x5780F00117272d30CB8a13AE7B81c05975839fe0);
        ...
        lpToken = fist;
        fistWrap = new FistWrap(IERC20(address(this)));
        _mint(_tokenOwner, 5000 * 10**18);
        lpDivAmount = 1 * 10**12;
        leaveAmount = 1 * 10**13;
        inviteAmount = 1 * 10**13;
        lpTokenDivThres = 50 * 10**6;
        thresRefAmount = 1 * 10**17;
    }
    ...
    function setThresRef(uint256 _thres) public onlyOwner {
        thresRefAmount = _thres;
    }
    ...
    function donateDust(address addr, uint256 amount) external onlyDever {
        require(addr != address(this) && addr != address(lpToken) , "KM: We can not withdrawal (KM,lpToken)");
    ...
    function _transfer(
        ...
        if (takeFee) {     
                ...
                uint256 dFee = 0;
                if(balanceOf(_destroyAddress) < 4500 * 10**18){
                    super._transfer(from, _destroyAddress, share.mul(5));
                    dFee = 5;
                }
                super._transfer(from, address(this), share.mul(15));
                lpAmount = lpAmount.add(share.mul(15));
                _takeInviterFee(from,to,amount);
                amount = amount.sub(share.mul(25 + dFee));
    ...
    function proc() private {
        ...
        if(balanceOf(address(this)) >= canfundAmount && canfundAmount >= amountT){
            ...
            swapTokensFor(canfundAmount,fundAddress);  
        }
    ...
    function swapTokensFor(uint256 tokenAmount,address to) private{
         // generate the uniswap pair path of token -> weth
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = address(fist);
