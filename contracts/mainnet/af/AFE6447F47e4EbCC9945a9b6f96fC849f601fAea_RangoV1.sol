...
contract RangoV1 is BaseContract, RangoCBridgeProxy, RangoThorchainProxy {
    // TODO: check cbridge executor security-groups
    // TODO: make cbridge executor a systemd service
    ...
    function initialize() public initializer {
        ...
        rangoThorchainAddress = NULL_ADDRESS;
...
contract BaseContract is Pausable, Initializable, Ownable, ReentrancyGuard {
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        // Get Platform fee
        if (request.feeIn > 0) {
            _sendToken(request.fromToken, request.feeIn, feeContractAddress, isSourceNative, false);
...
contract RangoThorchainProxy is BaseContract {
    ...
    function swapInToThorchain(
        ...
        require(rangoThorchainAddress != NULL_ADDRESS, 'Thorchain wrapper address in Rango contract not set');

        (, uint out) = onChainSwapsInternal(request, calls);
        approve(request.toToken, rangoThorchainAddress, out);

        IRangoThorchain(rangoThorchainAddress).swapInToThorchain{value: msg.value}(
            request.toToken,
            out,
            tcRouter,
            tcVault,
