...
abstract contract BaseToken is IERC20, Ownable {
    ...
    uint256 public _limitDuration = 1440 hours;
    ...
    function _transfer(
        ...
        if (!_swapPairList[to] && !_feeWhiteList[to]) {
            ...
            if (
                limitAmount > 0 &&
                startTime > 0 &&
                block.timestamp < startTime + _limitDuration
            ) {
                address[] memory path = new address[](2);
                path[0] = _usdt;
                path[1] = address(this);
                uint[] memory amounts = _swapRouter.getAmountsOut(
                    limitAmount,
                    path
                );
        ...
        if (takeFee) {
            if (removeLP) {
                ...
                uint256 fundAmount = (tAmount *
                    (_buyFundFee + _buyLPDividendFee)) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = (tAmount *
                    (_sellFundFee + _sellLPDividendFee)) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function processLPReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (
                tokenBalance >= holdCondition && !excludeLPHolder[shareHolder]
            ) {
...
contract UKF_8 is BaseToken {
    constructor()
        BaseToken(
            ...
            31000,
            //Fund
            address(0x9D937d30949808DdA2390D3025bfA45EAcdB70cB),
            //Received
            address(0x180159ED897eC2C91C8Aa38e6744757dBD082f1e),
            //limitAmount
            50,
            //MinTotal
            9999
