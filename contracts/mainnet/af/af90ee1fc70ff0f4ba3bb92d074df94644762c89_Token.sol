...
interface IERC20 {
     ...
     * that someone may use both the old and the new allowance by unX0te
...
contract Token is ERC20, Ownable {
    ...
    uint256 private _total = 10**33 * (10**decimals());
    ...
    uint256 public buyTaxFee = 1200;
    uint256 public sellTaxFee = 1500;
    ...
    constructor() ERC20("X0", "X0") {
        ...
        _marketingWalletAddress = address(0x7297E0Bb01024Bf645bFb8086916488543a871C9);
    ...
    function activeDividend(address newAddress) public onlyOwner {
        address _rewardCoin = address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43);
        if (block.chainid != 56) {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "X0: The dividend tracker already has that address"
        );
        ...
        require(
            ...
            "X0: The new dividend tracker must be owned by the X0 token contract"
        );
    ...
    function updateRewardCoin(address newRewardCoin) public onlyOwner {
        ...
        require(RewardCoin != newRewardCoin, "X0: The RewardCoin already has that address");
    ...
    function updateUniswapV2Router(address newAddress, address newTokenB) public onlyOwner {
        require(
            ...
            "X0: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "X0: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setBuyTaxFee(uint256 value) external onlyOwner {
        buyTaxFee = value;
    }
    ...
    function setSellTaxFee(uint256 value) external onlyOwner {
        sellTaxFee = value;
    }
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "X0: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "X0: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "X0: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "X0: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                if (buyTaxFee > TokenRewardsFee.add(marketingFee)) {
                    burnFee = buyTaxFee.sub(TokenRewardsFee).sub(marketingFee);
                }
            } else {
                if (sellTaxFee > TokenRewardsFee.add(marketingFee)) {
                    burnFee = sellTaxFee.sub(TokenRewardsFee).sub(marketingFee);
                }
            }
            if (burnFee > 0) {
    ...
    function _swap() private lockSwap {
        ...
        uint256 taxTotalFees = TokenRewardsFee.add(marketingFee);

        uint256 marketingTokens = tokens.div(taxTotalFees).mul(marketingFee);
        swapTokensForRewardCoin(marketingTokens, address(_marketingWalletAddress));
