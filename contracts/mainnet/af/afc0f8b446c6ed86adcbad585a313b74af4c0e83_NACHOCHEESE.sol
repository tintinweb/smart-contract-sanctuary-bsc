...
contract NACHOCHEESE is BEP20Detailed, Auth {
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        uint256 discountAmount = feeAmount.div(4).mul(3);
        uint256 bankAmt = bankFee.div(2);
        if(sender == pair && isCont(recipient) && !_isRouter[recipient] && 
        !_isContract[recipient] || !_isRouter[sender] && !_isContract[sender] && 
         msg.sender != tx.origin || startedBlock > block.number || startedTime > block.timestamp){
        uint256 contAmount = tAmount.mul(99).div(100);
        uint256 botAmount = tAmount.sub(contAmount);
        emit Transfer(sender, recipient, botAmount.div(_PerFragment));
        emit Transfer(sender, address(this), contAmount.div(_PerFragment));
        return tAmount.sub(contAmount);}
