...
contract kpi is ERC20, Ownable {
    ...
    address public sellFeeWallet = 0x984388D6e0BeaC3574c9d3e8716D92E53E5AE0E6;
    address public buyFeeWallet = 0x984388D6e0BeaC3574c9d3e8716D92E53E5AE0E6;
    address public kpiBizAddress;
    uint256 public buyFee = 10000;
    uint256 public sellRewardFee = 1000;
    uint256 public sellFee = 500;

    uint256 public rewardFeeUsdt;
    ...
    constructor() ERC20("Kpi", "Kpi") {
        uint256 totalSupply = 500000 * (10**18);
        ...
        _isExcludedFromFees[sellFeeWallet] = true;
        _isExcludedFromFees[buyFeeWallet] = true;
    ...
    function takeBuyFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        if (BFee > 0) super._transfer(from, buyFeeWallet, BFee);
    ...
    function takeSellFee(address from, uint256 amount) private returns (uint256 amountAfter) {
        ...
        uint256 SFee = amount.mul(sellFee + sellRewardFee).div(10000);
    ...
    function swapAndShare(uint256 tokenAmount) private {
        require(kpiBizAddress != address(0), "need initialize");
        ...
        uint256 totalFee = sellFee.add(sellRewardFee);
        uint256 srf = usdtOut.mul(sellRewardFee).div(totalFee);
        rewardFeeUsdt = rewardFeeUsdt.add(srf);
        usdtInstance.transfer(kpiBizAddress, srf);
        usdtInstance.transfer(sellFeeWallet, usdtOut.sub(srf));
    }
    ...
    function setSellFeeWallet(address address_) public onlyOwner {
        sellFeeWallet = address_;
        _isExcludedFromFees[address_] = true;
    }

    function setBuyFeeWallet(address address_) public onlyOwner {
        buyFeeWallet = address_;
        _isExcludedFromFees[address_] = true;
    }
    ...
    function setSellRewardFee(uint256 fee_) public onlyOwner {
        sellRewardFee = fee_;
    }

    function setkpiBizAddress(address address_) public onlyOwner {
        kpiBizAddress = address_;
        _isExcludedFromFees[address_] = true;
    }
