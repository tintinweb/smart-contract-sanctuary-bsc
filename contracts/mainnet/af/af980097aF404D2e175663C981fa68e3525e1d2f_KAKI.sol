...
contract KAKI is Context, IERC20, Ownable {
    ...
    mapping(uint256 => address ) private _addressOfTokenId;
    
    address[] private _excluded;
   
    uint8 private _decimals = 18;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 202000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address public fundAddress;
    address public nftAddress;
    address public divAddress;  
    string private _name = "BSC-KAKI"; 
    string private _symbol = "BSC-KAKI";
    ...
    uint256 public nftPool = 10;
    uint256 public nftSigle = 10;

    uint256 public maxTxAmount = 20200 * 10**18;
    ...
    mapping (address => bool) public isExcludeSingleLimit;
    ...
    constructor (address _usdt, address _router, address _team, address _wrap, address _fundAddress) {
        TEAM = _team;
        USDT = _usdt;
        wrap = _wrap;
        ...
        isExcludeSingleLimit[uniswapV2Pair] = true;
        isExcludeSingleLimit[address(this)] = true;
    ...
    function addressTokenId(uint256 tokenId) public view returns (address) {
        return _addressOfTokenId[tokenId];
    }
    ...
    function batchExcludeFromFee(address[] memory accounts) public onlyOwner {
    ...
    function batchIncludeInFee(address[] memory accounts) public onlyOwner {
        for(uint256 i= 0; i < accounts.length; i++) {
    ...
    function setNftPool(uint256 _nftPoolFee) external onlyOwner {
        nftPool = _nftPoolFee;
    }

    function setNftPSigle(uint256 _nftSigleFee) external onlyOwner {
        nftSigle = _nftSigleFee;
    }
    ...
    function setDivAddress(address _divAddress) public onlyOwner{
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (Param memory param) {
        ...
        uint256 currentRate = _getRate();
        if(takeFee && isSell){
            param.tLiquidity = tAmount * liquidityFee / 100;
            param.tBurn = tAmount * burnFee / 100;
            param.tTax = tAmount * taxFee / 100;
            
            //直接扣除10%给钱包地址
            // transferFrom(msg.sender, _fundAddress, tAmount.div(10));
            //  require(tAmount < balanceOf(msg.sender)*90/100,"sell amount must less than 90%");
            param.toFund =  tAmount * nftSigle / 100;
            param.toPool = tAmount * nftPool / 100;
        
            //nft指定分红
            uint _totalFee = liquidityFee + burnFee + taxFee + nftPool + nftSigle;
        ...
        if(takeFee && !isSell) {
            param.tLiquidity = 0;
            param.tTax = 0;
            param.tBurn = tAmount * buyFee / 100;
             //nft分红
            uint tokenId = ((tAmount)%(10**18) + 10**14)/(10**14);
            if(tokenId > 999 && tokenId < 10000) {
                uint tokenIdAc = tokenId - 1;
                address nftOwner = _addressOfTokenId[tokenIdAc]; 
                //transferFrom(_fundAddress,nftAddess,tAmount*10/100);
                param.fundTo = tAmount.div(10);
                param.nftOwner = nftOwner;
            }
            
            tFee = tAmount * buyFee / 100;
            rFee = tFee.mul(currentRate);
        }
    ...
    function setDivNft(address account,uint tokenId) public {
        require(msg.sender == nftAddress,"sender is not owner");
        //require(_addressOfTokenId[tokenId] == msg.sender,"tokenId is other address");
        _addressOfTokenId[tokenId] = account;
    }

    function addDivNft(address account, uint tokenId) public {
        require(msg.sender == nftAddress,"sender is not owner");
        _addressOfTokenId[tokenId] = account;
    }
    ...
    function excludeSingleLimit(address account, bool excluded) public onlyOwner {
        require(isExcludeSingleLimit[account] != excluded, " Account is already the value of 'excluded'");
        isExcludeSingleLimit[account] = excluded;
    }
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && !isExcludeSingleLimit[from]) {
            uint256 balance = balanceOf(from);
            require(amount <= balance.div(100).mul(90), "Exceed Single Max Sell");
        }
        ...
        if(uniswapV2Pair != address(0) && contractTokenBalance >= maxTxAmount){
            ...
            // 卖触发加池
            if (!inSwapAndLiquify && from != uniswapV2Pair && to == uniswapV2Pair && from != address(this)) {
    ...
    function _takeFee(Param memory param, address from) private {
        ...
        if(param.toFund >0) {
            _rOwned[fundAddress] = _rOwned[fundAddress].add(param.toFund.mul(currentRate));
            emit Transfer(msg.sender, fundAddress, param.toFund);
        }

        if(param.fundTo>0) {
            _rOwned[fundAddress] = _rOwned[fundAddress].sub(param.fundTo.mul(currentRate));
            _rOwned[param.nftOwner] = _rOwned[param.nftOwner].add(param.fundTo.mul(currentRate));
            emit Transfer(fundAddress, param.nftOwner, param.fundTo); 
        }

        if(param.toPool>0) { 
            _rOwned[divAddress] = _rOwned[divAddress].add(param.toPool.mul(currentRate));
            if (IDIVC(divAddress).totalSupply() > 0) {
                IDIVC(divAddress).distributeDividends(param.toPool);
           
            }
            emit Transfer(msg.sender, divAddress, param.toPool);
    ...
    function claim() external {
	    IDIVC(divAddress).processAccount(msg.sender, false);
    }
