...
contract Token is IERC20, Ownable {
    ...
    mapping (address => uint256) public _addressTime;
    uint256 private _xzTime = 24 * 60 * 60;
    ...
    address private _destroyAddress =0xCf0dC809692fa2851ce110c47eB2ED0593b2CB2e;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && to == uniswapV2Pair){
            _addressTime[from] = block.timestamp;
        }

        if(!_isExcludedFromFee[from] && to == uniswapV2Pair){
            if(block.timestamp <= _addressTime[from] + _xzTime)
            require(amount <= balanceOf(from).mul(10));
        }
