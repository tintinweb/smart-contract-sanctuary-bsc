...
contract StarlinkBSCStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The staked token
    IBEP20 public bridgeAddress;

    // Info of each user that stakes tokens (bridgeAddress)
     ...
     * @param _bridgeAddress: staked token address
     * @param _tokenAddress: reward token address
     * @param _rewardPerBlock: reward per block (in tokenAddress)
    ...
    function initialize(
        ...
        uint256 decimalsRewardToken = uint256(tokenAddress.decimals());
     ...
     * @param _amount: amount to withdraw (in tokenAddress)
     */
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                tokenAddress.safeTransfer(address(msg.sender), pending);
            }
        }
        ...
        if (_amount > 0) {
            ...
            bridgeAddress.safeTransferFrom(address(msg.sender), address(this), _amount);
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            bridgeAddress.safeTransfer(address(msg.sender), _amount);
        }
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            ...
            bridgeAddress.safeTransfer(address(msg.sender), amountToTransfer);
        }
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(bridgeAddress), "Cannot be staked token");
        require(_tokenAddress != address(tokenAddress), "Cannot be reward token");
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 bridgeAddressSupply = bridgeAddress.balanceOf(address(this));
        if (block.number > lastRewardBlock && bridgeAddressSupply != 0) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
            uint256 forwardReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(forwardReward.mul(PRECISION_FACTOR).div(bridgeAddressSupply));
    ...
    function getStakersList(uint startIndex, uint endIndex) 
        ...
        uint[] memory _bridgeAddresss = new uint[](length);
        ...
        for (uint i = startIndex; i < endIndex; i = i.add(1)) {
            ...
            _bridgeAddresss[listIndex] = userInfo[staker].amount;
        }
        
        return (_stakers, _stakingTimestamps, _bridgeAddresss, _lockTime);
    }
    /*
    ...
    function _updatePool() internal {
        ...
        uint256 bridgeAddressSupply = totalStaked;

        if (bridgeAddressSupply == 0) {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 forwardReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(forwardReward.mul(PRECISION_FACTOR).div(bridgeAddressSupply));
        lastRewardBlock = block.number;
    }
