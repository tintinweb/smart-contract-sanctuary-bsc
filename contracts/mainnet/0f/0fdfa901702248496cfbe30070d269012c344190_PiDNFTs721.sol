...
PiD NFT @custom:security-contact [email protected] 
*/
...
contract PiDNFTs721 is ERC721Mochi, ReentrancyGuard {
        ...
        ERC721Mochi(_owner, name, symbol, _anyoneCanMint) 
    {
        ...
        require(_commissionRate<=100, "PiDERC721: Commission rate has to be between 0 and 100");
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) external {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "PiDNFTs721: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "PiDNFTs721: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) external nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "PiDNFTs721: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "PiDNFTs721: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) external {
        ...
        require(sellBidPrice[tokenId]==0, "PiDNFTs721: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "PiDNFTs721: The selected NFT already has an auction");
        require(ownerOf(tokenId)==msg.sender, "PiDNFTs721: Only owner can auction this item");
    ...
    function bid(uint256 tokenId, uint256 bid_value) external nonReentrant {
        ...
        address owner = ownerOf(tokenId);
        require(msg.sender!=owner, "PiDNFTs721: The owner cannot bid his own collectible");
