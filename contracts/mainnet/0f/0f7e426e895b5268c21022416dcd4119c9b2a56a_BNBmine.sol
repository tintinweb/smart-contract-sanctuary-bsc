...
contract BNBmine is ERC20 {
	...
	address payable _marketAddress = payable(address(0x3aF3C2ec1Cbacc86994f575a2E3229DEb4b737E0));
    ...
    constructor(address tokenOwner) ERC20("BNBmine", "BNBmine") {
	...
	function swapTokensAnd2ldx() private {
		...
		if(totalAmount > 10**20){
			uint256 bnbOldAmount = address(this).balance;
			swapTokensForEththis(totalAmount.div(60).mul(55));
			uint256 bnbAmount = address(this).balance;
			if(bnbAmount > bnbOldAmount){
				uint256 newAmount = bnbAmount - bnbOldAmount;
				addLiquidity(totalAmount.div(60).mul(5),newAmount.div(55).mul(5));
				_marketAddress.transfer(newAmount.div(55).mul(20));
			}
		}
	}
		...
		if(buySize>0){
            ...
            if(buySize >20){
                for(uint256 i=0;i<20;i++){
                    ...
                    if(balanceOf(user) >= 10**23){
                        rate = super.balanceOf(user).mul(thisAmount).div(totalAmount);
                        if(rate>10**13){
                            payable(user).transfer(rate);
                        }
                    }
                    startIndex += 1;
                }
