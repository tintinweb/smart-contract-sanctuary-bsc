...
FTM MAKER
  ...
  ~ 2% of fees will go to FTM MAKER.
  ...
  ~ 50 tickets max per user. 150 FTM = 50 tickets.
  ~ 2.5 FTM deposit = 1 per ticket. based on user deposit/compound.
  ~ 90% of the pot will be converted to miners and be sent to the winning address.
  ~ 10% fee will go to FTM MAKER

  Whale Control Feature:
  ~ 36 hours cut off time.
  ~ 12 hours withdraw cooldown.
  ~ 6,000 FTM max withdrawal.
  ~ 30,000 FTM max deposits per wallet.
...
contract FTMMAKER {
    ...
    IERC20 public token_FTM;
	address erctoken = 0xAD29AbB318791D579433D831ed122aFeAf29dcfe; /** mainnet FTM **/
    ...
    uint256 public MIN_INVEST = 1 ether; /** 1 FTM  **/
	uint256 public WITHDRAW_COOLDOWN = 12 * 60 * 60; /** 12 hours  **/
    uint256 public WITHDRAW_LIMIT = 6000 ether; /** 6000 FTM  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 30000 ether; /** 30,000 FTM  **/
    ...
    function sellEggs() public{
        ...
        token_FTM.transfer(msg.sender, eggsPayout);
    ...
    function buyEggs(address ref, uint256 amount) public payable{
        ...
        token_FTM.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** referral rewards will be in FTM **/
                ...
                token_FTM.transfer(upline, refRewards);
                /** referral rewards will be in FTM value **/
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_FTM.transfer(project, projectFee);
		token_FTM.transfer(partner1, partnerFee);
		token_FTM.transfer(partner2, partnerFee);
        token_FTM.transfer(buyback, buybackFee);
    ...
    function payFeesSell(uint256 eggValue) internal returns(uint256){
        ...
        token_FTM.transfer(project, prj);
		token_FTM.transfer(marketing, mkt);
        token_FTM.transfer(buyback, bbk);
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    token_FTM.transfer(buyback, back);
    ...
    function getBalance() public view returns (uint256) {
        return token_FTM.balanceOf(address(this));
	}
