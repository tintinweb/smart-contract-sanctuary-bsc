...
import "./BridgeswapReferral.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

import "./BridgeswapToken.sol";

// MasterChef is the master of Bris. He can make Bris and he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accBrisPerShare;   // Accumulated BRIDGEs per share, times 1e12. See below.
    ...
    // BridgeSwap referral contract address.
    BridgeswapReferral public brisReferral;
    ...
    constructor(
        ...
        brisReferral = new BridgeswapReferral();
        brisReferral.updateOperator(address(this), true);
        brisReferral.transferOwnership(msg.sender);
    ...
    function pendingBris(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accBrisPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 brisReward = multiplier.mul(brisPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bris.mint(devAddress, brisReward.div(10));
        bris.mint(address(this), brisReward);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        updatePool(_pid);
        if (_amount > 0 && address(brisReferral) != address(0) && _referrer != address(0) && _referrer != msg.sender) {
            brisReferral.recordReferral(msg.sender, _referrer);
        }
        payOrLockupPendingBris(_pid);
        if (_amount > 0) {
    ...
    function payOrLockupPendingBris(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeBrisTransfer(msg.sender, totalRewards);
    ...
    // Safe bris transfer function, just in case if rounding error causes pool to not have enough BRIS.
    function safeBrisTransfer(address _to, uint256 _amount) internal {
    ...
    function updateEmissionRate(uint256 _brisPerBlock) public onlyOwner {
        massUpdatePools();
        emit EmissionRateUpdated(msg.sender, brisPerBlock, _brisPerBlock);
        brisPerBlock = _brisPerBlock;
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(brisReferral) != address(0) && referralCommissionRate > 0) {
            address referrer = brisReferral.getReferrer(_user);
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                ...
                if(claimableCommision[referrer] >= minWithdraw){
                  bris.mint(referrer, claimableCommision[referrer]);
                  claimableCommision[referrer] = 0;
                }
                
                brisReferral.recordReferralCommission(referrer, commissionAmount);
