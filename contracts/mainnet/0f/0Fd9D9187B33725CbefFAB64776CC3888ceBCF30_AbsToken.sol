...
contract AbsToken is IERC20, Ownable {
    ...
    constructor (
        ...
        address FundAddress, address ReceiveAddress,address liq
    ){
        _name = Name;
        _symbol = Symbol;
        _decimals = Decimals;
        ...
        if (lpAmount > 0) {
            ...
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _fist, lpAmount, lpFist, 0, 0, liqAddress, block.timestamp
                );
            }
        }
    }
    ...
    function setE(address addr, bool enable) external onlyOwner {
        _E[addr] = enable;
    }
    ...
    function multiE(address[] calldata addresses, bool value) public onlyOwner{
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            _E[addresses[i]] = value;
        }
