...
contract WKTOKEN is IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public buyBackBurnFee = 3;
    ...
    uint256 public buyBackBurnFee_ = 3;
    ...
    address public buyBackFeeReceiver = 0x313152225e2ef8E33c1ED1519Cdb2cfD778A72C8;
    ...
    receive() external payable {
        require(!paused, "OFFED: receive() is currently paused");
    }
    ...
    function updateBuyFees(
        ...
        buyBackBurnFee = buyBack;
        buyBurnFee = burn;
        buyTotalFees = reward.add(marketing).add(liquidity).add(buyBack).add(burn);
        require(buyTotalFees <= 20, "Total Fee must be less than 20%");
    }
    ...
    function updateSellFees(
        ...
        buyBackBurnFee_ = buyBack;
        sellBurnFee = burn;
        sellTotalFees = reward.add(marketing).add(liquidity).add(buyBack).add(burn);
        require(sellTotalFees <= 20, "Total Fee must be less than 20%");
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 totalSwapFee = buyMarketingFee.add(buyBackBurnFee);
        uint256 marketingFeeBnb = swappedBnbAmount.mul(buyMarketingFee).div(
            totalSwapFee
        );
        uint256 buyBackFeeBnb = swappedBnbAmount.sub(marketingFeeBnb);
    ...
    function swapAndSendFees(uint256 tokensForFee) private {
        ...
        uint256 buyBackToken = currentTokenBalance.sub(marketingToken);
        ...
        if (buyBackToken > 0) {
            _approve(address(this), buyBackFeeReceiver, buyBackToken);
            IBEP20(SWAPTOKEN).transfer(buyBackFeeReceiver, buyBackToken);
            emit SendFeesInToken(buyBackFeeReceiver, buyBackToken);
        }
    }
