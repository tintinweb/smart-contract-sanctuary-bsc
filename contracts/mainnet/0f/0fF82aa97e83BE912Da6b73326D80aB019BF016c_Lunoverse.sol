...
interface IERC20 {
    function transfer(address to, uint lunoverses) external returns (bool success);
    function transferFrom(address from, address to, uint lunoverses) external returns (bool success);
    function balanceOf(address lunoverseOwner) external view returns (uint balance);
    function approve(address spender, uint lunoverses) external returns (bool success);
    function allowance(address lunoverseOwner, address spender) external view returns (uint remaining);
    function totalSupply() external view returns (uint);
    event Transfer(address indexed from, address indexed to, uint lunoverses);
    event Approval(address indexed lunoverseOwner, address indexed spender, uint lunoverses);
}
...
contract Lunoverse is Owned {
    
    using SafeMath for uint;

    address public lunoverse;
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(lunoverse).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough  to pay registration fee.");
        require(IERC20(lunoverse).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(lunoverse).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(lunoverse).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient  balance in pool');
        stakeRewards[msg.sender] = 0;
        referralRewards[msg.sender] = 0;
        referralCount[msg.sender] = 0;
        uint remainder = (now.sub(lastClock[msg.sender])).mod(86400);
        lastClock[msg.sender] = now.sub(remainder);
        IERC20(lunoverse).transfer(msg.sender, totalReward);
        emit OnWithdrawal(msg.sender, totalReward);
        return true;
    }

    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(lunoverse).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        require((IERC20(lunoverse).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient  balance in pool');
        IERC20(lunoverse).transfer(msg.sender, _amount);
