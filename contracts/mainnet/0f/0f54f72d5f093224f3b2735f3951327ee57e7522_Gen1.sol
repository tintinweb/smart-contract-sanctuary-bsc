...
contract Gen1 {
 ...
 function withdraw() public {//address userAddress
        User storage user = users[msg.sender];
    //if ( users[userAddress].deposits.length > MIN_Deposits){
        ...
        if (contractBalance < totalAmount) {
         ...
         //  if(user.withdrawn>MIN_Deposits_amount){
        user.checkpoint = block.timestamp;
        ...
        token.transfer(msg.sender, totalAmount);
        emit Withdrawn(msg.sender, totalAmount);
         uint256 fee= SafeMath.div(SafeMath.mul(totalAmount,0),100);
         uint256 fee2=fee;
