...
contract DistContract is  Context,  Ownable{
    ...
    string constant public Version = "BASEDIST V1.1.2";
...
/************************************owner function *********************************/
    ...
    function SetDaoConfig(address _daoReward, address _powerAddr, address _daoAddr, uint256 _eff) public onlyOwner {
    ...
    function MintAndDist(uint256 _amount) public onlyOwner {
        ...
        uint price = getPrice();
        distProfit(reward, _amount, price);
    }

    function MintAndDistHasDao(uint256 _amount) public onlyOwner {
        IBEP20(token).mint(_amount);
        uint price = getPrice();
        uint256 daoDist = getDaoDist(_amount, price);
        distProfit(daoReward, daoDist, price);
        _amount = _amount.sub(daoDist);
        distProfit(reward, _amount, price);
    }
...
/***************************************public function *********************************************/
    ...
    function DistHasDao(uint256 _amount) public returns (bool){
        IBEP20(token).transferFrom(_msgSender(), address(this), _amount);
        uint price = getPrice();
        uint256 daoDist = getDaoDist(_amount, price);
        distProfit(daoReward, daoDist, price);
...
/********************************************** view function*****************************/
    function GetDaoConfig() public view returns (
        address _daoReward,
        address _powerAddr,
        address _daoAddr,
        uint256 _eff
    ) { 
        return (daoReward, powerAddr, daoAddr, eff);
    }
...
/***********************************************internal function*****************************/
    ...
    function getDaoDist(uint256 _amount, uint256 _price) internal view returns (uint256) {
        uint dp = getDeadPower(_price);
        return _amount.mul(dp).div(IPowerPool(powerAddr).tatolpower().add(dp));
    }
/********************************private function******************************/    
