...
contract DDNNFTMarketplace is Ownable, Pausable, IMarketplace, FeeManager, ERC721Holder {
    ...
    constructor() public Ownable(){
        isAcceptedToken[nativeToken] = true;
    }
    ...
    function depositTokens(address _token, uint256 _amount) external onlyOwner {
        ...
        totalRewardBalance = totalRewardBalance.add(_amount);
    }
    ...
    function withdrawTokens(address _token, uint256 _amount) external onlyOwner {
        ...
        totalRewardBalance = totalRewardBalance.sub(_amount);
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public payable whenNotPaused {
        ...
        if(order.currency == 0x0000000000000000000000000000000000000000) {
            ...
            uint256 _discountAmount = order.price.mul(10).div(100);
            
            if(totalRewardBalance >= _discountAmount) {
                IERC20(nativeToken).transfer(msg.sender, _discountAmount);
                totalRewardBalance = totalRewardBalance.sub(_discountAmount);
            }
        }
