...
contract Vault {
    ...
    function rewardWithdraw(address account, uint256 deadline, uint256 interest, bytes memory signature) external {
        if (block.timestamp > deadline) revert();
        uint256 nonce = rewardNonces[account];
        bytes memory message = abi.encode(nonce, account, deadline, interest, 1);
        bytes32 hash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(bytes(message).length), message)
        );
        if (SignatureChecker.isValidSignatureNow(signer, hash, signature) == false) revert();
    ...
    function achievementRewardWithdraw(
        ...
        bytes memory message = abi.encode(nonce, account, deadline, interest, 2);
        bytes32 hash = keccak256(
            abi.encodePacked("\x19Ethereum Signed Message:\n", Strings.toString(bytes(message).length), message)
        );
        if (SignatureChecker.isValidSignatureNow(signer, hash, signature) == false) revert();
