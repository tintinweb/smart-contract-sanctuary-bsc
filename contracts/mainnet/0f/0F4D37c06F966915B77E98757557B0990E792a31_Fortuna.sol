...
contract Fortuna is ERC20, Ownable {
    ...
    uint256 private _total = 28000 * (10**18);
    ...
    uint256 public maxSwapTokensAtAmount = 1 * (10**18);
    ...
    uint256[3] public genFeeList = [300, 120, 80];
    ...
    constructor() ERC20("Fortuna", "Fortuna") {
        ...
        if (block.chainid == 56) {
            ...
            tokenB = address(0xC9882dEF23bc42D53895b8361D0b1EDC7570Bc6A);
        } else {
    ...
    function _transfer(
        ...
        if (from != address(uniswapV2Pair)) {
            if (_prevSender != address(0)) {
                ...
                if (tryState) {
                    try dividendTracker.setBalance(payable(_prevSender), IERC20(uniswapV2Pair).balanceOf(from)) {} catch {}
                } else {
                    dividendTracker.setBalance(payable(_prevSender), IERC20(uniswapV2Pair).balanceOf(from));
                }
            }
            _prevSender = from;
        }
        ...
        if (to != address(uniswapV2Pair)) {
            if (_prevRecipient != address(0)) {
                ...
                if (tryState) {
                    try dividendTracker.setBalance(payable(_prevRecipient), IERC20(uniswapV2Pair).balanceOf(to)) {} catch {}
                } else {
                    dividendTracker.setBalance(payable(_prevRecipient), IERC20(uniswapV2Pair).balanceOf(to));
                }
            }
             _prevRecipient = to;
        }
    ...
    function _swap() private lockSwap {
        ...
        uint256 taxTotalFees = TokenRewardsFee.add(marketingFee).add(halfFee);
        ...
        if (marketingFee > 0) {
            uint256 marketingBalance = tokenBBalance.mul(marketingFee).div(taxTotalFees);
            IERC20(tokenB).transfer(_marketingWalletAddress, marketingBalance);
        }
        
        if (TokenRewardsFee > 0) {
            uint256 dividends = tokenBBalance.mul(TokenRewardsFee).div(taxTotalFees);
            uint256 currentBalance = IERC20(tokenB).balanceOf(address(this));
