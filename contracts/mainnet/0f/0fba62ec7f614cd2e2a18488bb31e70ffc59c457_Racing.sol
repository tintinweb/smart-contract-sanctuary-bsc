...
contract Racing {
    ...
    function finish()
        public
    {       
        require(block.timestamp > timeStampStop[racIter] || (!(someoneBuy)));
        require(!racingEnded);        
        ...
        if(maxIter > 1)
        {
            timeStampStop[racIter] = block.timestamp + (5 minutes);
        }
        else
        {
        ...
        returns (uint256[] memory result)
    {
        ...
        {
            if(userChoice[a][i] == winners[i] && bids[i][a] > 0)
                j++;
        }
