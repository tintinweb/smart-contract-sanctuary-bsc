...
contract BUSDPrinter {
    ...
    modifier initialized() {
        require(init, "DApp has not started yet");
        _;
    }

    constructor(address _owner, address _partner, address _marketer) {
            owner = _owner;
            partner = _partner;
            marketer = _marketer;
        ...
        function Renounce_Ownership(address value) external {
        require(msg.sender == owner, "Owner use only.");
        owner = value;
    }
    
    function stakeStablecoins(uint256 amtx, address ref)  external payable initialized {
        ...
        uint256 userStakePercentAdjustment = 1000 - partnerPercentage;
        uint256 adjustedAmt = amtx.mul(userStakePercentAdjustment).div(percentdiv); 
        uint256 stakeFee = amtx.mul(partnerPercentage).div(percentdiv); 
        ...
        BUSD.safeTransfer(owner, stakeFee * 3/8);
        BUSD.safeTransfer(partner, stakeFee * 3/8);
        BUSD.safeTransfer(marketer, stakeFee * 2/8);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        uint256 adjustedPercent = 1000 - partnerPercentage;
        uint256 adjustedAmt = x.mul(adjustedPercent).div(percentdiv); 
        uint256 withdrawFee = x.mul(partnerPercentage).div(percentdiv);

        main.ovrTotalWiths += x;
        user.lastWith = block.timestamp;

        BUSD.safeTransfer(owner, withdrawFee * 3/8);
        BUSD.safeTransfer(partner, withdrawFee * 3/8);
        BUSD.safeTransfer(marketer, withdrawFee * 2/8);
    ...
    function withdrawRefBonus() external payable initialized {
        User storage user = UsersKey[msg.sender];
    ...
    function stakeRefBonus() external payable initialized {
        User storage user = UsersKey[msg.sender];
        Main storage main = MainKey[1];
    ...
    function compound() external payable initialized {
        User storage user = UsersKey[msg.sender];
        Main storage main = MainKey[1];
