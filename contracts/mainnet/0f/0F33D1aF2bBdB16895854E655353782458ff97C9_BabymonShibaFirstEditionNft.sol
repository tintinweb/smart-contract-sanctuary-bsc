...
contract BabymonShibaFirstEditionNft is ERC721Full, ReentrancyGuard {
    ...
    */
    ...
    function mint(address who) private {
        ...
        string memory uri = string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/Qmcf77PqhrXNJvBkpHbgeiku7Vi2zqN75HDZ6ihVusXamg/", uint2str(roll), ".json"));
        remaining--;
    ...
    function setFreedrops(address[] calldata who, bool free) external onlyOwner {
        for (uint256 i = 0; i < who.length; i++) {
            freeDrop[who[i]] = free;
        }
    }
