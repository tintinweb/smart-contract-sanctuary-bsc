...
contract SmartRewardInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SmartRewardInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("SmartRewardInu", "$MARTINU") {
        marketingWallet = payable(0x0a4b06b51Cf9eaF4c9633Af8C013505DD0bcB253);
        devWallet = payable(0xE7Ef37bE5410ce98b258B1f444C9Bb1E8303FDA4);
        ...
        dividendTracker = new SmartRewardInuDividendTracker(
            payable(this),
            router,
            0xA8D172141A13fa20cBeA81B9B7FbB165908BCD20,
            "SmartRewardInuTRACKER",
            "SmartRewardInuTRACKER"
        );
...
contract SmartRewardInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SmartRewardInu public SmartRewardInuContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SmartRewardInuContract = SmartRewardInu(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SmartRewardInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SmartRewardInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SmartRewardInu contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SmartRewardInuContract);

            uint256 prevBalance = SmartRewardInuContract.balanceOf(address(this));
            ...
            {
                uint256 received = SmartRewardInuContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SmartRewardInuContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
