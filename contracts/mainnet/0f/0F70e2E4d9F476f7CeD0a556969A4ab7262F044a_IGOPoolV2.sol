...
contract IGOPoolV2 is Ownable, ReentrancyGuard, structItem, p {
    ...
    function igo(uint256 idoNum, uint256 _cosoID) external payable nonReentrant {
        ...
        uint256 additionalAmount = (fk1.price).mul(idoNum);
        ...
        if (takeTX) {
            if (fk1.erc20Token == address(0))
            {
                ...
                swapConfig.routerAddress2.swapExactTokensForTokens(additionalAmount, 0, swapConfig.swapErc20Path, msg.sender, block.timestamp);
            }
