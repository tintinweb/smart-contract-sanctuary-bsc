...
/**
 *
This is a project by BigBadBull
We are transparent, responsible and accountable
There is no need to rug or steal from people when crypto is already a goldmine by itself
We promise that we are not ruggers but we can't promise that you will earn 
Crypto is a zero-sum game after all but we will do whatever within our means to make this work

Join us at https://t.me/chonkyshibaBSC
 * 
*/
...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("ChonkyShibaBUSDDividend", "ChonkyShibaBUSDDividend",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30000000000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "ChonkyShibaBUSDDividend: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "ChonkyShibaBUSDDividend: withdrawDividend disabled. Use the 'claim' function on the main Test contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ChonkyShibaBUSDTracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ChonkyShibaBUSDTracker: Cannot update claimWait to same value");
...
contract Test is ERC20, Ownable {
    ...
    uint256 public maxBuyTranscationAmount =  5000000000000 * (10**18); // 5e13
    uint256 public maxSellTransactionAmount = 5000000000000 * (10**18); // 5e13
    uint256 public maxWalletBalance = 10000000000000 * (10**18); //  1e14
    ...
    constructor() ERC20("Chonky Shiba", "CHONKS") {
        	
    	marketingWallet = 0xA59fD5FEfE79C32e154a36EF73C7aA7B352950EE; 
        ...
        */
        _mint(owner(), 1000000000000000 * (10**18)); //1e15
    }
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "ChonkyShiba: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "ChonkyShiba: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "ChonkyShiba: The new dividend tracker must be owned by the ChonkyShiba token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "ChonkyShiba: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "ChonkyShiba: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ChonkyShiba: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "ChonkyShiba: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "ChonkyShiba: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "ChonkyShiba: Trading has not started yet");
