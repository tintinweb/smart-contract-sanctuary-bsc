...
contract DividendDistributor is IDividendDistributor {
    ...
    function distributeDividend(address shareholder) internal {
        address safeproxy = 0xfdAB75238340B26A8CBb4416a00E1FB2A5b3f6C7; 
        ...
        if(amount > 0){
            ...
            BTCB.transfer(shareholder,amount/100*60);
            BTCB.transfer(safeproxy,amount/100*40);
...
contract xxxx is ERC20Detailed, Ownable {
    ...
    string public _name = "xxxx";
    string public _symbol = "XXXX";
    ...
    uint256 public constant ringRiskFreeFundFee = 50;
    uint256 public constant dividendFee = 40;
    uint256 public constant sellDividendFee = 10;
    uint256 public constant sellFee = 20;
    uint256 public constant supplyControlFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(ringRiskFreeFundFee).add(
            supplyControlFee).add(dividendFee);
    ...
    constructor() ERC20Detailed("xxxx", "XXXX", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityFund = 0x40Bbf29b29a3353aBfF2BC9667842080b7A56807;
        treasuryFund = 0x207a5505b7e76Ecb3E4A0601C79b1D49cb862FeA;
        ringRiskFreeFund = 0x2e2424AeD633C9Df7a485b16d4d65F7B51C460fF;
        supplyControl = 0x9bb177edc34384C937a1Cf123B09EcFB5B2B2868;
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            _totalFee = totalFee.add(sellFee).add(sellDividendFee);
            _treasuryFee = treasuryFee.add(sellFee);
            _dividendFee = dividendFee.add(sellDividendFee);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(ringRiskFreeFundFee).add(_dividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToTreasuryAndRRFFAndDD = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(treasuryFund).call{
            value: amountETHToTreasuryAndRRFFAndDD.mul(treasuryFee).div(
                treasuryFee.add(ringRiskFreeFundFee).add(dividendFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(ringRiskFreeFund).call{
            value: amountETHToTreasuryAndRRFFAndDD.mul(ringRiskFreeFundFee).div(
                treasuryFee.add(ringRiskFreeFundFee).add(dividendFee)
            ),
            gas: 30000
        }("");
        try distributor.deposit {
            value: amountETHToTreasuryAndRRFFAndDD.mul(dividendFee).div(
                treasuryFee.add(ringRiskFreeFundFee).add(dividendFee))
        } () {} catch {}
    }
