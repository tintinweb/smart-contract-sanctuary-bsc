...
contract TestDropHashLotteryV4 is VRFElementRoundIdUint256 {
    ...
    uint256 internal constant BET_AMOUNT = 1000 ether;
    uint256 internal constant SETTLE_AMOUNT = 0.001 ether;
    ...
    uint256 internal constant REWARD_PERCENT = 20;

    uint256 internal constant ROUND_LENGTH = 300; //5 min;

    uint256 internal constant CLIFF_ROUND_LENGTH = 60; //1 min;

    uint256 internal constant DAY_LENGTH = 300; // 5 min
    uint256 internal constant VESTING_DAY_LENGTH = 3; //3 days vesting
    ...
    event SettleBet(uint256 roundId, bytes32 hashMap);
    ...
    /// @notice place bet or buy ticket for round. if previous bet is not settled, new round will created. All bet have hashMap - sorted 2 or 3 symbols from block hash

    function placeBet() external virtual nonReentrant whenNotPaused {
        require(!_msgSender().isContract(), "HashLottery: sender cannot be a contract");
        require(tx.origin == _msgSender(), "HashLottery: msg sender is not original user");

        uint256 currentRoundStarted = rounds[lastRoundIndex].roundStarted;
        uint256 currentRoundEnded = rounds[lastRoundIndex].roundEnded;

        //current round was not settled
        ...
        if (lastRoundIndex > 0) {
            previousLastIndex = --lastRoundIndex;

            require(
                (block.timestamp > rounds[previousLastIndex].roundEnded + CLIFF_ROUND_LENGTH) && (block.timestamp < currentRoundStarted),
                "HashLottery: the round has not started yet"
            );
        } else {
            require(block.timestamp < currentRoundStarted, "HashLottery: the round has not started yet");
        }

        rounds[lastRoundIndex].totalBets = rounds[lastRoundIndex].totalBets + BET_AMOUNT;

        rounds[lastRoundIndex].numberOfBets = ++rounds[lastRoundIndex].numberOfBets;

        _beforeProcessBet(BET_AMOUNT);

        Bet memory bet = Bet({blockNumber: block.number - 1, amount: BET_AMOUNT, roundId: lastRoundIndex, hash: "", hashMap: ""});
    ...
    function _processBet(address _gamer, uint256 _id) internal virtual {
        ...
        require(!roundMaps[lastRoundIndex][hashMap].contains(_msgSender()), "HashLottery: already exist bet in this map");
    ...
    /// @notice settle bet and get hash map for winners
    function settleBet(uint256 _vrfFee) external payable virtual nonReentrant whenNotPaused {
        require(
            (block.timestamp > rounds[lastRoundIndex].roundStarted) && (block.timestamp < rounds[lastRoundIndex].roundEnded + DAY_LENGTH),
            "HashLottery: round has not started or already ended"
        );

        require(rounds[lastRoundIndex].roundSettled == 0, "HashLottery: round settled");

        uint256 settleAmount = msg.value - _vrfFee;

        require(settleAmount >=  SETTLE_AMOUNT, "HashLottery: cannot enough BNB for draw");

        if (settleAmount > SETTLE_AMOUNT) {
            //refund extra
            _deliverFunds(_msgSender(), settleAmount - SETTLE_AMOUNT, "HashLottery: failed transfer BNB to address");
        }

        _deliverFunds(platformAddress, settleAmount, "Can't deliver funds");
        ...
        RandomnessRequestData memory data = RandomnessRequestData({roundId: lastRoundIndex, number: 0, fullfilled: false, processed: false});
    ...
    function _processBetWithRandomness(RandomnessRequestData storage _data, uint256 _randomness) internal virtual override {
        ...
        rounds[roundId].settledHash = b;
        rounds[roundId].hashMap = hashMap;
        rounds[roundId].roundSettled = block.timestamp;

        rounds[roundId].reward = (jackpotAmount * REWARD_PERCENT * PRECISION) / 100 / PRECISION;

        emit SettleBet(roundId, hashMap);
    ...
    /// @param _roundIds array of rounds with rewards
    function claimBets(uint256[] memory _roundIds) public virtual nonReentrant whenNotPaused {
        for (uint256 i = 0; i <= _roundIds.length; i++) {
            claimBet(_roundIds[i]);
        }
    }

    /// @notice claim bets witn reward with vesting (90 days)
    /// @param _roundId round with reward
    function claimBet(uint256 _roundId) public virtual nonReentrant whenNotPaused {
        Round memory round = rounds[_roundId];

        require(roundMaps[_roundId][round.hashMap].contains(_msgSender()), "HashLottery: have no reward in this round");
        ...
        uint256 claimable = vested - win.claimed;
