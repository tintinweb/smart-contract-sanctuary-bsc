...
contract USDTStake is Ownable, ReentrancyGuard {
    ...
    function _takeInviterFee(address _address,uint256 amount) private {
        ...
        for (int256 i = 0; i < 4; i++) {
            ...
            if (cur == address(0)) {
              if (amount != count) {
                IERC20(USDT).safeTransferFrom(_address, mk, amount.sub(count));
              }
              break;
            }
            if (_aced[cur] && i == 0) {
              count = count.add(amount.mul(4).div(10));
              IERC20(USDT).safeTransferFrom(_address, cur, amount.mul(4).div(10));
            }
            if (_aced[cur] && i == 1) {
              count = count.add(amount.mul(3).div(10));
              IERC20(USDT).safeTransferFrom(_address, cur, amount.mul(3).div(10));
            }
            if (_aced[cur] && i == 2) {
              count = count.add(amount.mul(2).div(10));
              IERC20(USDT).safeTransferFrom(_address, cur, amount.mul(2).div(10));
            }
            if (_aced[cur] && i == 3) {
              count = count.add(amount.mul(1).div(10));
              IERC20(USDT).safeTransferFrom(_address, cur, amount.mul(1).div(10));
            }
        }
    }
