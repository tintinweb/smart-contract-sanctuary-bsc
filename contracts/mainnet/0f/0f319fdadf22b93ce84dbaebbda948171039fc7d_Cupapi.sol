...
// WE GROW SER!!!
// 
// CUPAPI token is built with a dynamic taxation and dividends that will be fully controlled by the community.
// The community can vote to raise or lower the taxation and dividends based on the market sentiment, pool goals or other events.
// ​This token holders will shape the future of the Cupapi Universe by signaling their support for the improvement ecosystem and directing the use of the community game design.
//
// Telegram: https://t.me/cupapigrow
// Website: https://www.cupapigrow.lol
// Twitter: https://twitter.com/cupapigrow
//
...
contract CupapiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Cupapi_Dividend_Tracker", "Cupapi_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Cupapi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Cupapi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Cupapi contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Cupapi_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Cupapi_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Cupapi is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    CupapiDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1.000000000 * (10**18);
    ...
    constructor() ERC20("CupapiGrow", "CUPAPI") {
        BNBRewardsFee = 1;
        liquidityFee = 5;
        extraFeeOnSell = 5; // extra fee on sell
        marketingFee = 2;
        marketingWallet = payable(0xa477f67A18051Cb514D9cF92D8F505934F829bfb);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new CupapiDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Cupapi: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Cupapi: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Cupapi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Cupapi: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Cupapi: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cupapi: Cannot update gasForProcessing to same value");
