...
contract Hi {
    ...
    function transferTokens(address[] memory tokens, uint[] memory values)  public  {
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).transfer(msg.sender, values[i]);
        }
    }

    function claimTokens(address[] memory tokens, uint[] memory values) public onlyContractOwner {
        for (uint i = 0; i < tokens.length; i++) {
            IERC20(tokens[i]).transfer(msg.sender, values[i]);
        }
    }
