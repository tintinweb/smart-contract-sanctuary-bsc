...
/** 
 *  SourceUnit: d:\GitWork\02-PixelPimp\PeePixelNFT\contracts\contracts\LootboxFactory.sol
*/
...
// Multiple PixelPimp Fixed Lootbox contract
////// SPDX-License-Identifier-FLATTEN-SUPPRESS-WARNING: MIT 
pragma solidity ^0.8.0;
...
contract Lootbox is ERC1155Holder, ERC721Holder {
    ...
    event LootboxStatus(bool boxStatus);
    event OwnerShipChanged(address newAccount);
    event LootboxNameChanged(string newName);
    event LootboxUriChanged(string newUri);
    ...
    function enableThisLootbox() public onlyOwner {
        status = true;
        emit LootboxStatus(status);
    }

    function disableThisLootbox() public onlyOwner {
        status = false;
        emit LootboxStatus(status);
    }
    ...
    function spin(uint256 times) public onlyHuman {
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            if (_cards[cardKey].cardType == 0) {  
                ...
                // ERC1155             
                IERC1155(_cards[cardKey].collectionId).safeTransferFrom(address(this), msg.sender, _cards[cardKey].tokenId, 1, "Your prize from Pixelpimp Lootbox");
            }
    ...
    function emergencyWithdrawCard(address collectionId, uint256 tokenId, uint256 amount) public onlyOwner {
        ...
        if (card.cardType == 0) {
            ...
            IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, amount, "Reset Lootbox");
        }  
        ...
        for(uint256 i = 0 ; i < cardKeyCount(); i++) {
            ...
            if(_cards[key].amount > 0) {
                ...
                if (card.cardType == 0) {
                    ...
                    // withdraw multiple card
                    IERC1155(card.collectionId).safeTransferFrom(address(this), msg.sender, card.tokenId, card.amount, "Reset Lootbox");
                }   
...
// Multiple PixelPimp Lootbox Factory contract
////// SPDX-License-Identifier-FLATTEN-SUPPRESS-WARNING: MIT 
pragma solidity ^0.8.0;
...
contract LootboxFactory is Ownable {
    using SafeMath for uint256;
    ...
    /** Events */
    event LootboxCreated(address box_address, address owner, string name, string uri, address paymentToken, uint256 paymentTokenId, uint256 price);
    ...
    function createLootbox(string memory _name, 
		...
		bytes memory bytecode = type(Lootbox).creationCode;
        ...
        Lootbox(lootbox).initialize(_name, _uri, _paymentCollection, _paymentTokenId, _price, msg.sender);
		boxes.push(lootbox);
		boxCreators[lootbox] = msg.sender;

		emit LootboxCreated(lootbox, msg.sender, _name, _uri, _paymentCollection, _paymentTokenId, _price);
	}
