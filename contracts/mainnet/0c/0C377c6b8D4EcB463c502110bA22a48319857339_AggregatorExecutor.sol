...
contract AggregatorExecutor is Ownable {
    ...
    uint256 private constant _FROM_ETH = 1;
    uint256 private constant _TO_ETH = 2;
    ...
    modifier ensure(uint256 _deadline) {
        require(_deadline >= block.timestamp, 'SwapRouter: EXPIRED');
        _;
    }

    //=========== external functions ============//
    function swap(SwapStep[] calldata _swaps, uint256 _amountIn, uint256 _amountOutMin, address payable _receiver, uint256 _flags, uint256 _deadline)
    external payable
    ensure(_deadline)
    {
        //check input eth
        if(_flags == _FROM_ETH && msg.value > 0) {
        ...
        //send eth to user
        if (_flags == _TO_ETH && _swaps[_length - 1].toToken == address(WBNB)) {
