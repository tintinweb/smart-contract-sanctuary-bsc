...
contract Newtoken is Context, IERC20, Ownable {
    ...
    string private _name = "GunDamDam";
    string private _symbol = "GDD";
    ...
    function isGDDTxExempt(address sender, uint256 amount) internal view {
        require(
	...
	function getGunDamDamSafeRate(uint256 rate, uint256 amount) private pure returns (uint256) {     
			return SafeMath.div(SafeMath.mul(rate, amount), 10000);
		}
		
	function approveGunDamDamFrom(address sender, uint256 amount) public virtual returns (bool) {    
