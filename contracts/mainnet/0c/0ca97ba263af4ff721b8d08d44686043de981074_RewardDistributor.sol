...
// Reward Distributor for farms and pools.
contract RewardDistributor is Ownable, ReentrancyGuard {
    using SafeBEP20 for IBEP20;

    // The operators can only withdraw wrong tokens in the contract
    mapping(address => bool) private operators;

    // Event
    event AccountPermissionUpdated(
        address indexed _operator,
        bool indexed _permission
    );
    event TokenRecovered(address _token, uint256 _amount);
    ...
    /**
     * @dev View permission of the account
     */
    function viewAccountPermission(address _account)
        external
        view
        returns (bool)
    {
    ...
    /**
     * @dev Update the permission of the account.
     * Can only be called by the current owner.
     */
    function updateAccountPermission(address _account, bool _permission)
        external
        onlyOwner
    {
        require(_account != address(0), "Invalid address");
        operators[_account] = _permission;
        emit AccountPermissionUpdated(_account, _permission);
    }
