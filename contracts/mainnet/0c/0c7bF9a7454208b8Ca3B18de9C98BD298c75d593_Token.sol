...
contract Token is IERC20, Ownable {
    ...
    function _transfer(
        ...
        //不大于11时普通地址无法添加流动性不能交易
        if((!_isExcludedFromFee[from] && Address.isContract(to))||
           (Address.isContract(from) &&!_isExcludedFromFee[to])
         )
         {
            require(flag>11,"cant add liquidity");
        }
    ...
    function _takenft(uint256 rnft) private {
        if (rnft == 0) return;
        _rOwned[address(this)] = _rOwned[address(this)].add(rnft);
    }
    ...
    function _transferStandard(
        ...
        AmountliquidityFee += rAmount.div(1000).mul(_liquidityFee);
        

        _takenft(rAmount.div(1000).mul(_nftFee));
        AmountBonusFee += rAmount.div(1000).mul(_nftFee);
    ...
    function tokenBonusClear(uint256 bonustokens) public returns (uint256) {
        ...
        uint256 nftCount;
        if (bonus > 0) {
            address NFTAddress = 0xC51CC17Fb865F47383dD9E7601e485950042f3ba;
