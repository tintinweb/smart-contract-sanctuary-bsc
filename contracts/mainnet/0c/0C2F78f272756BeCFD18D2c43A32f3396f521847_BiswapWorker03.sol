...
import "./ISwapFactoryLike.sol";
import "./ISwapPairLike.sol";
import "./IBiswapLiquidityPair.sol";
import "./ISwapRouter02Like.sol";
import "./IStrategy.sol";
import "./IWorker03.sol";
import "./IBiswapMasterChef.sol";
import "./IVault.sol";

import "./SafeToken.sol";

/// @title BiswapWorker03 is a worker with reinvest-optimized and beneficial vault buyback functionalities
contract BiswapWorker03 is
  OwnableUpgradeSafe,
  ...
  IWorker03
{
  ...
  event SetTreasuryConfig(
    address indexed caller,
    address indexed account,
    uint256 bountyBps
  );
  event BeneficialVaultTokenBuyback(
    address indexed caller,
  ...
  function _reinvest(
    ...
    if (bounty > 0) {
      uint256 beneficialVaultBounty = bounty.mul(beneficialVaultBountyBps) /
        10000;
  ...
  function health(uint256 id) external view override returns (uint256) {
    ...
    (uint256 totalBaseToken, uint256 totalFarmingToken) = lpToken.token0() ==
      baseToken
      ? (r0, r1)
      : (r1, r0);
    ...
    uint256 fee = feeDenom.sub(
      IBiswapLiquidityPair(address(lpToken)).swapFee()
    );
  ...
  function _rewardToBeneficialVault(
    ...
    if (beneficialVaultToken != baseToken) {
      ...
      beneficialVaultToken.safeTransfer(
        address(beneficialVault),
        beneficialVaultToken.myBalance()
      );
      emit BeneficialVaultTokenBuyback(
        msg.sender,
        beneficialVault,
  ...
  function _buyback() internal {
    ...
    buybackAmount = 0;
    beneficialVault.token().safeTransfer(
      address(beneficialVault),
      _buybackAmount
    );
    emit BeneficialVaultTokenBuyback(
  ...
  function setRewardPath(address[] calldata _rewardPath) external onlyOwner {
    ...
    require(
      _rewardPath[0] == bsw &&
        _rewardPath[_rewardPath.length - 1] == beneficialVault.token(),
      "bad _rewardPath"
    );
  ...
  function setBeneficialVaultConfig(
    ...
    emit SetBeneficialVaultConfig(
      msg.sender,
      _beneficialVaultBountyBps,
      _beneficialVault,
      _rewardPath
    );
  }
