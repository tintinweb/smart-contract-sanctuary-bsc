...
contract Cryptopump is ContextUpgradeable, OwnableUpgradeable {
    ...
    function payForLevel(uint256 _flag, uint256 _level, address _userAddress, uint256 _adminPrice, uint256 _amt) internal {
        ...
        if (users[referer[0]].currentLevel >= _level) {
            ...
            if (_level == 2 || _level == 4) {
                ...
                if (_level == 2) {
                    users[msg.sender].level2Claim = true;
                }
                if (_level == 4) {
                    users[msg.sender].level4Claim = true;
                }
            }
    ...
    function safeWithdrawToken(address _admin, uint256 _amount)
    public
    returns(bool)
    {
        require(msg.sender == ownerAddress, "only OwnerWallet");
    ...
    function claimLevelReward(uint256 _level)
    public
    returns(bool)
    {
        require(users[msg.sender].isExist, "User not exist");
        require(_level == 2 || _level == 4, "Invalid level");
        require(users[msg.sender].currentLevel >= _level, "Invalid level");

        if (_level == 2) {
            require(!users[msg.sender].level2Claim, "Alreacy  purchased");
            tokenAddress.safeTransfer(msg.sender, TOKEN_PRICE[2]);
            users[msg.sender].claimedamount += TOKEN_PRICE[2];
            totalclaimed += TOKEN_PRICE[2];
            users[msg.sender].level2Claim = true;
        } else {
            require(!users[msg.sender].level4Claim, "Alreacy  purchased");
            tokenAddress.safeTransfer(msg.sender, TOKEN_PRICE[4]);
            users[msg.sender].claimedamount += TOKEN_PRICE[4];
            totalclaimed += TOKEN_PRICE[4];
            users[msg.sender].level4Claim = true;
        }
    ...
    function distributeUsers(address[4] memory _poolUsers, uint[4] memory percentage)
    public
    returns(bool)
    {
        require(msg.sender == owner(), "only OwnerWallet");
        for (uint p = 0; p < _poolUsers.length; p++) {
            eligibleUsers.push(_poolUsers[p]);
            isEligible[_poolUsers[p]] = true;
            users[_poolUsers[p]].rewardPercentage = percentage[p];
            totalPoolUsers = totalPoolUsers.add(1);
        }
        lastPoolTimestamp = block.timestamp;
        endPoolTime = lastPoolTimestamp + 1 weeks;
