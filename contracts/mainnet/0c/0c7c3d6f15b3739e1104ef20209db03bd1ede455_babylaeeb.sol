...
contract babylaeeb is ERC20 {
	...
	address payable _marketAddress = payable(address(0x2Eb31654eBCC922672691440eA6fAac37E24ED23));
    ...
    constructor(address tokenOwner) ERC20("babylaeeb", "babylaeeb") {
	...
	function swapAndLiquifyLDX() public {
        ...
        if(allAmount  > 10**19 ){
			uint256 half12 = allAmount.div(10).mul(9);
			uint256 otherHalf = allAmount.sub(half12);        
			swapTokensForEth(half12);
			uint256 thisBalance = address(this).balance;
			_fundAddress.transfer(thisBalance.div(9).mul(4));
			_marketAddress.transfer(thisBalance.div(9).mul(4));
			addLiquidity(otherHalf, thisBalance.div(9));
		}
    }
