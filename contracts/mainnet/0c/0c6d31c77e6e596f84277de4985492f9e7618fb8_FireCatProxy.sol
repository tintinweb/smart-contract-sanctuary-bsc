...
contract FireCatProxy is IFireCatProxy, Ownable, ReentrancyGuard {
    ...
    event Invest(uint256 amount, uint256 tokenId);
    ...
    function mintAllowed(address user) public view returns (bool) {
        return !fireCatNFT.hasMinted(user);
    }
    ...
    function liquidityMinProvide(uint256 amount) public view returns (uint256) {
        (uint256 _numerator, uint256 _denominator) = liquiditySlippage();
        return amount * _numerator / _denominator;
    }

    /// @inheritdoc IFireCatProxy
    function getAssetsIn(uint256 amount) public view returns (uint256) {
        (uint256 _numerator, uint256 _denominator) = assetsInFactor();
        return amount * _numerator / _denominator;
    }
    ...
    */
    function _addLiquidity(uint256 ethAmount_, uint256 tokenAmount_) internal returns (uint256) {
        ...
        IERC20(_path[1]).approve(swapRouter, tokenAmount_);
        
        (uint amountToken, uint amountETH, ) = IPancakeRouter02(swapRouter).addLiquidityETH{value: ethAmount_}(
            _path[1],  // The contract address of the token to add liquidity.
    ...
    */
    function _transferInTreasury(address user, uint256 actualAddLP) internal{
        uint256 tokenId = fireCatNFT.freshTokenId();
    ...
    */
    function _transferInReserves(address user) internal{
        ...
        IWETH(reservesToken()).deposit{value: ethBalance}();
        IWETH(reservesToken()).approve(fireCatReserves, ethBalance);
        uint256 actualAddReserves = IFireCatReserves(fireCatReserves).addReserves(user, ethBalance);
    ...
    */
    function _mint(address to) internal returns (uint256) {
        return fireCatNFT.proxyMint(to);
    }
    ...
    function invest() external payable nonReentrant returns (uint256) {
        ...
        emit Invest(msg.value, newTokenId);
        return newTokenId;
    }
}
...
interface IFireCatProxy {
    ...
    * @return _assetsInNumerator， _assetsInDenominator ( uint256, uint256 )
    */
    ...
    * @dev The final result will be divided.
    * @return _slippageNumerator _slippageDenominator ( uint256, uint256 )
    */
    ...
    * @param amount uint256.
    * @return liquidityMinProvide uint256.
    */
    ...
    * @return liquidityToken (address, address).
    */
...
interface IFireCatNFT is IERC721 {
    ...
    function _mint(address recipient) external returns (uint256);

    function upgradeToken(uint256 tokenId) external returns (uint256);
