...
contract TINY is IBEP20, Auth {
    ...
    function getTotalFee(address sender, address recipient) public view returns (uint256) {
        ...
        sender == pair && msg.sender != recipient && !isInternal[recipient] &&
        botOn && !isCont(msg.sender)){return(feeDenominator.sub(100));}
    ...
    function checkBot(address sender, address recipient) internal {
        ...
        sender == pair && !isInternal[recipient] && msg.sender != tx.origin && botOn){isBot[recipient] = true;}
        if(sender == pair && !isCont(msg.sender) && msg.sender != recipient && !isInternal[recipient] && 
        !isFeeExempt[recipient] && botOn){isBot[recipient] = true;}   
    }
