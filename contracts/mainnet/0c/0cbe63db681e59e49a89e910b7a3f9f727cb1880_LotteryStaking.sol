...
contract LotteryStaking is Ownable, ReentrancyGuard, VRFConsumerBaseV2 {
    ...
    constructor() VRFConsumerBaseV2(vrfCoordinator) {
        ...
        nextDraw = 1673780400;
        addLotteryHistory(0, 461229, 1350888139021037116862);
        addLotteryHistory(1, 410869, 270177627804207423372);
        _depositAirDrop(
            [
                0x2f5B49C4172072aAF5767DF4bea5f7DA89EAf0A0,
                0x7c46dDf3d53d560a12aba01FC62B6DD5073E59B9,
                0x7c46dDf3d53d560a12aba01FC62B6DD5073E59B9,
                0x2f5B49C4172072aAF5767DF4bea5f7DA89EAf0A0,
                0x2f5B49C4172072aAF5767DF4bea5f7DA89EAf0A0
            ], 
            [
                198257000000000000000000,
                100000000000000000000000,
                193000000000000000000000,
                102000000000000000000000,
                213900194223786806923656
            ], 
            [
                1672756989,
                1672757109,
    ...
    function _depositAirDrop(
        address[5] memory _user,
        uint80[5] memory _amount,
        uint32[5] memory _depositTime
    ) internal {
        PoolInfo storage pool = poolInfo;
        for (uint256 i = 0; i < _user.length; i++) {
            stakeInfo[_user[i]].push(
    ...
    function pickWinner() external onlyOperator {
        ...
        if (rewardAmount == 0 && (currentLotteryNumber + 1) % 4 != 0) {
            currentLottery.isEnded = true;
            emit LotteryWinner(currentLotteryNumber, address(0));
            currentLottery.winnerTicket = 0;
        ...
        if (bigRewardAmount == 0 && (currentLotteryNumber + 1) % 4 == 0) {
            currentLottery.isEnded = true;
            emit LotteryWinner(currentLotteryNumber, address(0));
            currentLottery.winnerTicket = 0;
    ...
    function getDrawDate(uint256 _round) public view returns (uint256) {
        return nextDraw - (_round * 1 weeks);
    }
    ...
    function addLotteryHistory(
        uint256 _round,
        uint256 _winnerTicket,
        uint256 _winnerRewardAmount
    ) internal {
        n_lottery[_round].winnerTicket = _winnerTicket;
        n_lottery[_round].winnerRewardAmount = _winnerRewardAmount;
        currentLotteryNumber++;
    }
}