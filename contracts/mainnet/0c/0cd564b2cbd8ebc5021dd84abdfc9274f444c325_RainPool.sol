...
contract RainPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    address public constant feeWallet = 0x537e4527C7B22d4f1D67642b0ca22fFF60ed859E;
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            uint256 _depositFeeAmount = _amount.mul(depositFee).div(1000);
            stakedToken.safeTransfer(feeWallet,_depositFeeAmount);
            user.amount = user.amount.add(_amount.sub(_depositFeeAmount));
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(block.number > lastRewardBlock,"Pool not Ended");
        UserInfo storage user = userInfo[msg.sender];
