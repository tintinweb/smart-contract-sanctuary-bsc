...
contract AcaiToken is IBEP20, Auth {
    ...
    uint256 public sellBurnFee            = 180;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (recipient == pair) {
            ...
            burnedAmount = amount.mul(burnFee).div(feeDenominator);
        }
        if (burnedAmount > 0) {         
            _transferFrom(sender, DEAD, burnedAmount);
            if (realBurn) {
    ...
    function setFeeDistribution(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _projectFee,  uint256 _operationFee) external authorized {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        projectFee = _projectFee;
        operationFee = _operationFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_projectFee).add(_operationFee).add(sellBurnFee);
