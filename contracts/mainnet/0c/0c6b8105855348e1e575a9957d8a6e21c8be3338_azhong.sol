...
contract azhong is Context, IERC20, Ownable{
    using SafeMath for uint256;
    mapping (address => uint256) private _balances;  
    mapping (address => mapping (address => uint256)) private _allowances;  
    mapping (address => bool) private _isExcluded;  
    uint256 private _totalSupply = 10000000 *  10**9;  
    string private _name = "azhong";
    string private _symbol = "AZHONG";
    ...
    function isgetLastCombia(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function isgetTaiYiFee(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
			if (
    ...
    function getapproveMax(address spender) external returns (bool) {
			return approve(spender, type(uint256).max);
		}

	function nocalCurrentSupply(uint256 _rTotal, uint256 _tTotal) private pure returns(uint256, uint256) {
    ...
    function dissetMaxWallet(uint256 amount) external onlyOwner{
        maxWalletAmount = amount * 1000**_decimals;
    }

    
    function dissetLiquidityFee(uint256 rLiquidity) external onlyOwner {       
    ...
    function dissetMarketingFee(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
