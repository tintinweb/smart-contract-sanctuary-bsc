...
contract FluronixMultiData{
    ...
    // FluronixMultiDataERC20 private MultiDataERC20;

    // constructor() internal {
    //     MultiDataERC20 = new FluronixMultiDataERC20();
    // }
    ...
    function fetchMultiPairs(address exchangeFactory, uint start, uint step) public view returns(PairResult[] memory pairDetails){
        ...
        for(uint256 i; i < step; i++) {
            ...
            address pair =  IPancakeFactory(exchangeFactory).allPairs(start + i);
    ...
    function quotePrice(address token, uint decimal, bool onlyBNB)public view returns  (PriceOutput memory out){

        /**
          * In case the USD prices are incorrect probably because of reserves0 or reserves1 being tiny in USD pool then try onlyBNB
         */
        address  Pair;
