...
contract MdcToken is ERC20, Ownable {

    address public SWAP_PAIR;
    address immutable public DESTROY;
    address public LP;

    //0 normal, 1 free, 2 exchange
    ...
    constructor(string memory name_, string memory symbol_, uint256 total_, address lp_) ERC20(name_, symbol_){
        
        DESTROY = address(1);
        LP = lp_;

        SWAP_PAIR = IUniswapV2Factory(IUniswapV2Router01(ROUTER).factory()).createPair(address(this), WBNB);
       
        setExtConfig(_msgSender(), 1);
        setExtConfig(SWAP_PAIR, 2);
    ...
    function _handleTransferFeeWhenTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override virtual returns (uint256 receiveAmount) {
        ...
        if(s1 == 0 && s2 == 0){
            //normal transfer
            _standardTransfer(from, DESTROY, fee * 2);
            return amount - fee * 2;
        }

        //pancake swap
        _standardTransfer(from, DESTROY, fee);
        _standardTransfer(from, LP, fee * 2);
        if(s2 == 2){
            //sell or add
            _standardTransfer(from, to, fee * 2);
            return amount - fee * 5;
        }
        //buy or remove
        return amount - fee * 3;
    ...
    function setExtConfig(address account_, uint state_) public onlyOwner() {
        extConfig[account_] = state_;
    }

    function setLP(address lp_) public onlyOwner() {
        LP = lp_;
    }
}