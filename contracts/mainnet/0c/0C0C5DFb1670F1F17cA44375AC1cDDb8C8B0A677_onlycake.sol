...
contract onlycake is IERC20, Context, Ownable {
    ...
    string private constant _symbol = "OCK";
    string private constant _name = "Only Cake";

    uint256 public Cake_max_BuyLimit = 60000000 * 10**9;
    uint256 public Cake_max_SellLimit = 50000000 * 10**9;

    address private Cake_Only_Wallet = 0x9591aAe4D0a8196B9D1AbdED82eDbC5B7822C495;
    address private Cake_Team_Wallet = 0x5b286b812d6963D6Ca75b8d07a7519cAb04b26Ad;

    uint256 public Cake_max_WalletLimit = 100000000 * 10**9;
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Cake_Only_Wallet] = true;
        _isExcludedFromFee[Cake_Only_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Cake_Only_Wallet] = true;
        allowedTransfer[Cake_Only_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Cake_max_BuyLimit, "You are exceeding Cake_max_BuyLimit");
            require(balanceOf(to) + amount <= Cake_max_WalletLimit, "You are exceeding Cake_max_WalletLimit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Cake_max_SellLimit, "You are exceeding Cake_max_SellLimit");
            if(to != pair){
                require(balanceOf(to) + amount <= Cake_max_WalletLimit, "You are exceeding Cake_max_WalletLimit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Cake_Only_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Cake_Only_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function Cake_Update_MarketingWallet(address newWallet) external onlyOwner{
        Cake_Only_Wallet = newWallet;
    }
    
    function Cake_Update_TeamWallet(address newWallet) external onlyOwner{
        Cake_Team_Wallet = newWallet;
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Cake_max_BuyLimit = maxBuy * 10**decimals();
        Cake_max_SellLimit = maxSell * 10**decimals();
    }
    
    function Bnbb_Max_Walletlimit(uint256 amount) external onlyOwner{
        Cake_max_WalletLimit = amount * 10**decimals();
    }
