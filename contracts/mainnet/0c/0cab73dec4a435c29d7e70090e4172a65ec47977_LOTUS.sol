 ...
 *  Telegram : https://t.me/GoldenLotusBSC
 *  Website : 
 *  
 *  
...
contract LOTUS is Context, IERC20, Ownable {
    ...
    string private _name = "GoldenLotus";
    ...
    address  charityAdd = address(0x14B6Be8b1d7746d5BE46B245EB3783FC99D7d9C8);
 ...
 function Launch() public onlyOwner {
    ...
    function setTokenToSell(uint256 maxTxPercent) external onlyOwner() {
        numTokensSellToAddToLiquidity = maxTxPercent;
    }
...
require(!_isLockwallet[from],"locked");
        ...
        if(from != ownerAddres && to != ownerAddres && !_isExcludedFromFee[from]) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        if(from != owner() && to != owner() && to != address(0) && to != burnAddress && to != uniswapV2Pair && !_isExcludedFromFee[from]){
