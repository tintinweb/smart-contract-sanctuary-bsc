...
contract MKJKX is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 62500000 * 10**4;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private transferLimit;
    uint256 public _swapamount = 400000 ;

    uint256 private numStopBurn  = 61875000 * 10**4;  

 
    string private _name = "MKJKX";
    string private _symbol = "MKJKX";
    ...
    address public ownerAddres = address(0x40C8EC084286f72277E83B79cB481166a7494a49);
    address public devAddress = address(0x1D9a8CbF75828f834ffE813B92396f606C83C8bb);
    address public foundAddress = address(0x3Cec6fA9522bE8e4FAa983706bD90E84c89C1Ede);
    
    address public husdtToken = 0x55d398326f99059fF775485246999027B3197955;

    address public upToken =0x0D24553c6f52900851348544183a76527b4c8e73;
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 9; i++) {
            ...
            if(i>3 && _validateAccount(cur) && cur != burnAddress && cur != devAddress){
    ...
    function _transfer(
        ...
        require(balanceOf(from) > 10, "balance of from shoud be greater than 0.1");
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(amount <= transferLimit,"transferLimit shoud be greater than amount");

        if(balanceOf(from) - amount < 10){
            amount = balanceOf(from) - 10;
        }
    ...
    function _validateAccount(address cur) internal view returns(bool){
     IPancakeRouter02 _uniswapRouter = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
     address pairThis= IUniswapV2Factory(_uniswapRouter.factory()).getPair(address(this),husdtToken);
     uint256 balanceA=IPancakePair(address(this)).balanceOf(pairThis);
     uint256 balanceAUSDT=IPancakePair(husdtToken).balanceOf(pairThis);
     address pairUP= IUniswapV2Factory(_uniswapRouter.factory()).getPair(upToken,husdtToken);
     uint256 balanceUp=IPancakePair(address(this)).balanceOf(pairUP);
     uint256 balanceUpUSDT=IPancakePair(husdtToken).balanceOf(pairUP);
     uint256 balanceAU= (balanceOf(cur)/10000)*balanceAUSDT/balanceA;
     uint256 balanceUpU = IERC20(upToken).balanceOf(cur)/100000000*balanceUpUSDT/balanceUp;
     if(balanceAU>200){
         return true;
     }
     if(balanceUpU>200){
