...
contract NfterriumNet is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function buyVip(uint8 _count) external payable whenNotPaused {
        ...
        require(msg.value >= vipPrice.mul(_count).mul(10**26).div(rate), "insufficientAmount");
        ...
        uint256 toFightPool = vipPriceNomad.mul(_count);
        toFightPool = toFightPool.add(users[userId].terraCount.mul(vipPriceTerra.div(5)));
    ...
    function buyPlace(uint256 _id) external payable whenNotPaused {
        ...
        uint256 rate = uint256(getLatestPrice());
        require(msg.value >= usersForSale[_id].price.mul(10**26).div(rate), "insufficientAmount");
        if (usersForSale[_id].forWallet != address(0)) {
            require(usersForSale[_id].forWallet == msg.sender, "placeNotForWallet");
        }

        uint256 cms = msg.value.div(20);
        _addressToCom.transfer(cms);
        users[_id].wallet.transfer(msg.value.sub(cms));
    ...
    function activate(address _refWallet) external payable whenNotPaused {
        ...
        uint256 rate = uint256(getLatestPrice());
        require(msg.value >= nomadPrice.mul(10**26).div(rate), "insufficientAmount");
        ...
        _addressToCom.transfer(msg.value);
    ...
    function buyTerra(uint8 _count) external payable whenNotPaused {
        ...
        uint256 rate = uint256(getLatestPrice());
        require(msg.value >= terraPrice.mul(_count).mul(10**26).div(rate), "insufficientAmount");
        ...
        _addressToCom.transfer(msg.value.div(5));
        processRef(userId, msg.value);
        processUplines(userId, msg.value);
        fightPool = fightPool.add(msg.value.div(10));
