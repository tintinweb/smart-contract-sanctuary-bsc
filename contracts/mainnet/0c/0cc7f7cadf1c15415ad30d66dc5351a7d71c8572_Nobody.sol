...
contract Nobody is ERC20 {
	...
	address payable _fundAddress = payable(address(0x8Ffb5e01B47f426d04D2410F8D6713b632c37291));
    ...
    constructor(address tokenOwner) ERC20("Nobody", "Nobody") {
    ...
    function _transfer(
        ...
        isAdd = _isAddLiquidityV1();
        bool takeFee = !swapping;
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAdd) {
            takeFee = false;
        }else{
            require(swapStats);
            if(from == uniswapV2Pair){
                if(startTime.add(7) >= block.timestamp){
                    amount = amount.div(30);
                }
            }
    ...
    function swapAndLiquify() private {
        uint256 fundAmount = ldxAmount.sub(ldxOverAmount);
        uint256 allAmount = balanceOf(address(this));
        if(allAmount > fundAmount.add(10**24)){
            uint256 canSwapAmount = allAmount.sub(fundAmount);
    ...
    function _takeInviterFeeKt(
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            ktNum = ktNum+1;
            super._transfer(address(this), _receiveD, amount.div(i+3));
        }
    }

    function swapAndLiquifyFund() private {
        uint256 canSwapAmount = ldxAmount.sub(ldxOverAmount);
		if(balanceOf(address(this)) >= canSwapAmount && canSwapAmount > 10**24){
            ldxOverAmount = ldxOverAmount.add(canSwapAmount);
			swapTokensForETH(canSwapAmount);
			uint256 newBalance = address(this).balance;
			_fundAddress.transfer(newBalance);
		}
    }
    ...
    function _isAddLiquidityV1()internal view returns(bool ldxAdd){
        ...
        if( token0 == address(this) ){
			if( bal1 > r1){
				...
				ldxAdd = change1 > 1000000000000;
			}
		}else{
			if( bal0 > r0){
				uint change0 = bal0 - r0;
				ldxAdd = change0 > 1000000000000;
			}
		}
    }
