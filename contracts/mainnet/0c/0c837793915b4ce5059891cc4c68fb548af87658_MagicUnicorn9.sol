...
contract MagicUnicorn9 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn9DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn9", "MU9") {
        ...
        dividendTracker = new MagicUnicorn9DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn9TRACKER", "MU9TRACKER");
    ...
    function enableSellPhaseBuyBackWallet(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        ...
        if (buybackBNB > 0) {
            ...
            _burn(buyBackWallet, balanceOf(buyBackWallet));
        }
    }
...
contract MagicUnicorn9DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn9 public MagicUnicorn9Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn9Contract = MagicUnicorn9(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn9 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn9Contract);

            uint256 prevBalance = MagicUnicorn9Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn9Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn9Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
