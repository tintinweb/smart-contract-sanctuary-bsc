...
contract NFTSales is Ownable, ERC721Min, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public NFTToReward;
    mapping(uint256 => uint256) public NFTToClaimed;
    ...
    mapping(address => mapping(uint256 => nftToken)) public NFTTokens;    
    ...
    function burn(uint256 tokenId) external {
        require(_owners[tokenId] == _msgSender(), "NOT_OWNER");
        _burn(tokenId);
    }

    // ** - PROXY - ** //

    function mintOne(address receiver) external onlyApproved {
        require(_owners.length < maxMintForOne, "EXCEEDS_MAXMINT");
        _mintMin2(receiver);
    }

    function mintThree(address receiver) external onlyApproved {
        require(_owners.length < maxMintForThree, "EXCEEDS_MAXMINT");
        _mintMin2(receiver);
        _mintMin2(receiver);
        _mintMin2(receiver);
    }   

    function mint(address receiver, uint16 amount) external onlyApproved {
        require(_owners.length + amount < maxMintForOne, "EXCEEDS_MAXMINT");
        for (uint256 i = 0; i < amount; i++) {
            _mintMin2(receiver);
        }
    }    
    ...
    function depositBNB() public payable onlyApproved {
        uint256 depositPerNFT = msg.value / _owners.length;
        for(uint256 x; x < _owners.length; x++) {
            NFTToReward[x] += depositPerNFT;
        }
        emit DepositBNB(_msgSender(), msg.value, depositPerNFT);
    }
    
    function depositERC20(address token, uint256 amount) public onlyApproved {
        uint256 depositPerNFT = amount / _owners.length;
        ...
        for(uint256 x; x < _owners.length; x++) {
            NFTTokens[token][x].earned += depositPerNFT;
        }
        emit DepositERC20(_msgSender(), token, amount, depositPerNFT);        
    }

    function claim() external payable nonReentrant {
        uint256 balance = balanceOf(_msgSender());
        require(balance > 0, "NO_NFTS");
        uint256 amount;
        for(uint256 x; x < _owners.length; x++) {
            if (_owners[x] == _msgSender()) {
                amount += NFTToReward[x] - NFTToClaimed[x];
                NFTToClaimed[x] = NFTToReward[x];
                balance--;
                if (balance == 0) break;
            }        
        }
    ...
    function claimERC20(address token) external payable nonReentrant {
        uint256 balance = balanceOf(_msgSender());
        require(balance > 0, "NO_NFTS");
        uint256 amount;
        for(uint256 x; x < _owners.length; x++) {
            if (_owners[x] == _msgSender()) {
                amount += NFTTokens[token][x].earned - NFTTokens[token][x].claimed;
                NFTTokens[token][x].claimed = NFTTokens[token][x].earned;
                balance--;
                if (balance == 0) break;
            }        
        }
        ...
        emit ClaimERC20(_msgSender(), token, amount);
    }
    ...
    event DepositERC20(address indexed user, address indexed token, uint256 indexed amount, uint256 depositPerNFT);
    event ClaimERC20(address indexed user, address indexed token, uint256 amount);
}
