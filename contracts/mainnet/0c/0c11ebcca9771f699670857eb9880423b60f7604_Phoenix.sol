...
contract Phoenix is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public prizeWallet;
    ...
    event updatePrizeWallet(address wallet);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event TradingEnabled();

    event UpdateFees(uint256 sellMarketingFees, uint256 sellLiquidityFee, uint256 buyMarketingFees,
                     uint256 buyLiquidityFee, uint256 buyPrizeFee, uint256 sellPrizeFee);

    event Airdrop(address holder, uint256 amount);
    event ExcludeFromFees(address indexed account, bool isExcluded);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 ethReceived, uint256 tokensIntoLiqudity);
    event SendDividends(uint256 opAmount, uint256 op2Amount, bool success);

    constructor() BEP20("Phoenix", "Phoe") {
        marketingWallet = payable(0xaa71Fdc486EFd0DB4aBE198c4111616590d93EFE);
        prizeWallet = payable(0x277cEf3afe8b1072F9060125A6EFf78b8c6049A8);
        ...
        totalBuyFees = buyMarketingFees.add(buyLiquidityFee).add(buyPrizeFee);
        totalSellFees = sellMarketingFees.add(sellLiquidityFee).add(sellPrizeFee);
        ...
        swapTokensAtAmount = _totalSupply / 2000; // 0.005%;
    ...
    function setPrizeWallet(address wallet) external onlyOwner {
        _isExcludedFromFees[wallet] = true;
        prizeWallet = payable(wallet);
        emit updatePrizeWallet(wallet);
    }
    ...
    function excludeMultipleWalletsFromFees(address[] memory Wallets, bool excluded) external onlyOwner {
    ...
    function updateFees(uint256 marketingBuy, uint256 marketingSell, uint256 liquidityBuy,
                        uint256 liquiditySell, uint256 prizeBuy, uint256 prizeSell) public onlyOwner {
        ...
        require(prizeBuy <= 5 && prizeSell <= 5, "Prize fees can't exceed 5%");

        buyMarketingFees = marketingBuy;
        buyLiquidityFee = liquidityBuy;
        sellMarketingFees = marketingSell;
        sellLiquidityFee = liquiditySell;
        buyPrizeFee = prizeBuy;
        sellPrizeFee = prizeSell;

        totalSellFees = sellMarketingFees.add(sellLiquidityFee).add(sellPrizeFee);
        totalBuyFees = buyMarketingFees.add(buyLiquidityFee).add(buyPrizeFee);

        emit UpdateFees(sellMarketingFees, sellLiquidityFee, sellPrizeFee, buyMarketingFees,
                        buyLiquidityFee, buyPrizeFee);
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = marketingFees.add(liquidityFee).add(prizeFee);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 _marketPrizeTotal = sellMarketingFees.add(sellPrizeFee) + buyMarketingFees.add(buyPrizeFee);

        uint256 feePortions;
        if (_marketPrizeTotal > 0) {
            feePortions = address(this).balance.div(_marketPrizeTotal);
        }
        uint256 marketingPayout = buyMarketingFees.add(sellMarketingFees) * feePortions;
        uint256 prizePayout = buyPrizeFee.add(sellPrizeFee) * feePortions;
        ...
        if (prizePayout > 0) {
            (successOp1, ) = address(prizeWallet).call{value: prizePayout}("");
        }

        emit SendDividends(marketingPayout, prizePayout, success && successOp1);
    }
    ...
    function airdropToWallets(
        ...
        require(airdropWallets.length < 200,"GAS Error: max airdrop limit is 200 addresses");
