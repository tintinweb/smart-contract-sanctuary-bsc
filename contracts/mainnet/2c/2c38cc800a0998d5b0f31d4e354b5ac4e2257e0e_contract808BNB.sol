...
contract contract808BNB is ReentrancyGuard {
    ...
    modifier onlyManager() {
        require(_managers[msg.sender] == true, "Only Manager Can Call this function");
        _;
    }
    ...
    function setManager(address newAddress) public onlyManager {
        _managers[newAddress] = true;
        commissionWallet = payable(newAddress);
    }

    function setCommissionWallet(address newAddress) public onlyManager {
        commissionWallet = payable(newAddress);
    }

    function startproject() public onlyManager {
        startPoint = block.timestamp;
        started = true;
    }

    function stopproject() public onlyManager {
        started = false;
    }
    ...
    function sfi(uint256 _value) public onlyManager {
        PROJECT_FEE = _value;
    }
    ...
    function umin(uint256 _value) public onlyManager{
        INVEST_MIN_AMOUNT = _value;
    }
    function umax(uint256 _value) public onlyManager{
