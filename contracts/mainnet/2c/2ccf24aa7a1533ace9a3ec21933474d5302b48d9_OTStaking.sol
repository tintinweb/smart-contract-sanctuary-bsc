...
contract OTStaking {
    ...
    uint256[4] public Reward = [50, 75, 100, 200]; //gives: 60%, 90%, 120 and 240% in 1 year
    ...
    function withdrawRewards(uint256 index) public {
        ...
        token.transferFrom(
            owner,
            msg.sender,
            stakersRecord[msg.sender][index].reward*10**9
        );
        stakedTokens[msg.sender] = stakedTokens[msg.sender].sub(_amount);
    ...
    function realTimeUserRewardPerPlan(address user, uint256 planIndex) public view returns (uint256) {
        uint256 ret;
            if (
                !stakersRecord[user][planIndex].withdrawan &&
                !stakersRecord[user][planIndex].unstaked
            ) {
                uint256 val;
                val = block.timestamp - stakersRecord[user][planIndex].staketime;
                val = val.mul(stakersRecord[user][planIndex].persecondreward);
                if (val < stakersRecord[user][planIndex].reward) {
                    ret = val;
                } else {
                    ret = stakersRecord[user][planIndex].reward;
                }
            }
        return ret;
    }
