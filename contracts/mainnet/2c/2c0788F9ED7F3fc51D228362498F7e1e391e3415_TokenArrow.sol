...
contract TokenArrow is TokenXBaseV3
{
	...
	{
		address_nft = 0xF1120281d48F30Ce580116273a536612Caed600e;
...
contract TokenXBaseV3 is ERC20, Ownable
{
	...
	uint256 public total_nft_supply=1000;
	...
	mapping(address => bool) private is_address_lp; // cake lp address for sell, buy tax
	...
	event SetLPAddressCB(address indexed operator, address _lp_address, bool _is_enabled);
	...
	function _transfer(address sender, address recipient, uint256 amount) internal virtual override
	{
		...
		if(is_intenal_address[sender] == false && is_intenal_address[recipient] == false)
			_make_juice_by_chick(); // tax processing

		_check_send_limit(sender, recipient, amount);

		uint256 cur_tax_rate_e6 = _get_tax_rate_e6(sender, recipient); // sell, buy tax		
		if(cur_tax_rate_e6 == TAX_FREE)
	...
	function _make_juice_by_chick() internal
	{
		if(address_chick == address(0) || chick_work == false || is_chick_busy == true)
			return;
	...
	{
		require(is_address_lp[_to] == true && _amount <= send_limit_amount[_from], "_check_send_limit: limit exceed");
	}

	function _has_xnft(address _address_user) internal view returns(bool)
	{
		if(address_nft == address(0))
			return false;

		IERC1155 nft = IERC1155(address_nft);
		for(uint256 grade=0; grade<10; grade++)
		{
			for(uint256 serial=0; serial<total_nft_supply; serial++)
			{
				uint256 id = grade * 1e6 + serial;
				if(nft.balanceOf(_address_user, id) > 0)
					return true;
			}
		}
	...
	{		
		...
		// Pancake LP에 들어오고 나간다는 것이 sell, buy를 말함
		// LP에서 들어오고 나가는건 무조건 택스를 떼되, 
		if(is_address_lp[_from] == true) // LP -> User(LPTool) // Buy
		{
			if(is_tax_free_send[_to] == false)
			{
				// 우리 사이트에서 하는건 텍스를 안 떼게
				// (중간에 넘기는 놈 LPTool을 두고 그 놈을 is_tax_free_send 처리)
				tax_rate_e6 = tax_rate_send_e6;
			}
			else if(address_nft != address(0))
			{
				if(_has_xnft(_to) == true)
					tax_rate_e6 = tax_rate_send_with_nft_e6;
			}
		}
		else if(is_address_lp[_to] == true) // User(LPTool) -> LP // Sell
		{
			if(is_tax_free_recv[_from] == false)
			{
	...
	function _set_internal_address(address _address, bool _is_set) private
	{
		if(_address != address(0))
		{
			...
			is_tax_free_send[_address] = _is_set;
			is_tax_free_recv[_address] = _is_set;
		}
	}
	...
	// Setters
	//---------------------------------------------------------------
	function set_operator(address _new_operator) public onlyOperator
	{
		require(_new_operator != address(0), "set_operator: Wrong address");

		_set_internal_address(address_operator, false);
		_set_internal_address(_new_operator, true);
	...
	function set_controller(address _new_controller) public onlyOperator
	{
		require(_new_controller != address(0), "set_controller: Wrong address");

		_set_internal_address(address_operator, false);
		_set_internal_address(_new_controller, true);
		
		address_controller = _new_controller;

		emit SetControllerCB(msg.sender, _new_controller);
	}

	function set_chick(address _new_chick) external onlyController
	{
		require(!is_chick_busy, "set_chick: the chick is working.");
		require(_new_chick != address(0), "set_chick: Wrong address");
	...
	function set_total_supply_limit(uint256 _amount) external onlyOperator
	{
		total_supply_limit = _amount;
	}

	function set_send_tax_free(address _to, bool _is_free) public onlyOperator
	{
		require(_to != address(0), "set_send_tax_free: Wrong address");
		is_tax_free_send[_to] = _is_free;

		emit SetSendTaxFreeCB(msg.sender, _to, _is_free);
	}

	function set_recv_tax_free(address _from, bool _is_free) public onlyOperator
	{
		require(_from != address(0), "set_recv_tax_free: Wrong address");
		is_tax_free_recv[_from] = _is_free;
		
		emit SetRecvTaxFreeCB(msg.sender, _from, _is_free);
	}
	...
	function set_lp_address(address _lp_address, bool _is_enabled) public onlyOperator
	{
		...
		emit SetLPAddressCB(msg.sender, _lp_address, _is_enabled);
	}
	...
	function set_nft(address _address_nft) public onlyOperator
	{
		address_nft = _address_nft;
	}

	function set_total_nft_supply(uint256 _total_supply) public onlyOperator
	{
		total_nft_supply = _total_supply;
	}
}