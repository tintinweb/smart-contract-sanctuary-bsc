...
interface IPrivateSale{
    ...
    function getUserInfo(address _sender) external view returns (UserInfo memory _userInfo);
}
...
contract PrivateSale is Ownable {
    ...
    constructor(IBEP20 _usdtToken, IBEP20 _xgToken, IReferral _referral, address _ref,address _ps,uint256 _totalUsdt) public {
        referral = _referral;
        ...
        saleInfo.push(SaleInfo({
            amtAvl: 2052503356445590,
            price: 800000000,
            startDate: 1650154500,
            endDate: 0,
    ...
    function setTotalUsdt(uint256 _totalUsdt) public onlyOwner {
        totalUsdt = _totalUsdt;
    }

    function setXgToken(address _xgToken) public onlyOwner {
        xgToken = IBEP20(_xgToken);
    }
    ...
    function addUsers(address _user) internal{
        if(userInfo[_user].amount1==0 && userInfo[_user].amount2==0 &&
            userInfo[_user].amount3==0 && userInfo[_user].amount4==0){
             users.push(_user);
        }   
    }
    ...
    function importUser() public{
        UserInfo storage user = userInfo[msg.sender];
        if(user.amount1==0){
            addUsers(msg.sender);

            IPrivateSale.UserInfo memory olduser = ps.getUserInfo(msg.sender);
            
            user.amount1 = olduser.amount1;
            user.amount2 = olduser.amount2;
            user.amount3 = olduser.amount3;
            user.amount4 = olduser.amount4;
        }
    }
    ...
    function buyFromSC(uint256 _amount,address _buyer) public onlyMasterChef{
