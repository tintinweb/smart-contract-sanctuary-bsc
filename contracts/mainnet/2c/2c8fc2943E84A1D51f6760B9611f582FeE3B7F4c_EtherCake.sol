...
contract EtherCake is Context, IERC20, Ownable {
    ...
    string private _name = "Ether Cake";
    string private _symbol = "ETHCAKE";
    uint8 private _decimals = 4;

    address public marketingWallet = 0x7a086c08FbE62249638d77F8232566F5bb8B85a0;
    address public charityWallet = 0x7a086c08FbE62249638d77F8232566F5bb8B85a0;
    ...
    uint256 public _maxTxAmount =  _totalSupply.mul(20).div(denominator);     
    uint256 public _walletMax = _totalSupply.mul(20).div(denominator);    
    ...
    constructor () {
        ...
        totalBuy = _buyLiquidityFee.add(_buyMarketingFee).add(_buyCharityFee);
        totalSell = _sellLiquidityFee.add(_sellMarketingFee).add(_sellCharityFee);
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        uint256 amountBNBCharity = amountReceived.sub(amountBNBLiquidity).sub(amountBNBMarketing);

        if(amountBNBMarketing > 0)
            transferToAddressETH(marketingWallet, amountBNBMarketing);

        if(amountBNBCharity > 0)
            transferToAddressETH(charityWallet, amountBNBCharity);
    ...
    function setBuyFee(uint _newLP , uint _newMarket , uint _newCharity) public onlyOwner {     
        _buyLiquidityFee = _newLP;
        _buyMarketingFee = _newMarket;
        _buyCharityFee = _newCharity;
        totalBuy = _buyLiquidityFee.add(_buyMarketingFee).add(_buyCharityFee);
    }

    function setSellFee(uint _newLP , uint _newMarket, uint _newCharity) public onlyOwner {        
        _sellLiquidityFee = _newLP;
        _sellMarketingFee = _newMarket;
        _sellCharityFee = _newCharity;
        totalSell = _sellLiquidityFee.add(_sellMarketingFee).add(_sellCharityFee);
    }

    function setWallets(address _market,address _liquidityRec,address _charityW) public onlyOwner {
        marketingWallet = _market;
        liquidityReciever = _liquidityRec;
        charityWallet = _charityW;
    }
