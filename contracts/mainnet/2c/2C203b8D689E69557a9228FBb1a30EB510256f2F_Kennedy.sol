...
interface INftDividend{
    function swapAllToken() external;
    function perDayCheckBonus() external;
}

contract Kennedy is Context, IERC20, IERC20Metadata, Ownable, Operator {
    ...
    string private _name = "test Ken"; // test Veaglse Kennedy
    string private _symbol = "ttK"; // KENNEDY
    ...
    uint256 public _maxTxAmount = 100 * 10**18; // prod
    ...
    address public sellFeeAddress = 0x395a6fC02119a23d6d37596300A2e0fdb85cF47b;
    address public communityAddress = 0x44d349eAfF38De0DDD124fD8f2a9C4c4cB0F43FA;
    address public lpAddAddress;
    ...
    function excludeFromFees(address[] memory accounts) public onlyOperator{
    ...
    function setNftFeeAddress(address account) public onlyOperator{
        require(account != nftFeeAddress, "Nft fee address must be a new address");
        _isExcludedFromFee[account] = true;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if( contractTokenBalance >= _maxTxAmount
            && !inSwapAndLiquify 
            && !ammPairs[from] 
            && IERC20(uniswapV2Pair).totalSupply() > 20 * 10**18 ){
        ...
        if( takeFee ){
            ...
            if(!ammPairs[from] && !ammPairs[to]){
                param.takeFee = false;
                param.tTransferAmount = amount;   //common transfer
            }
        } else {
            param.takeFee = false;
    ...
    function _takeFee(Param memory param, address from) private {
        ...
        if( param.tNftFee > 0 ){
            ...
            if (nftFeeAddress != address(0) && nftFeeAddress.isContract()) {
                try INftDividend(nftFeeAddress).swapAllToken() {} catch {}
            }
        }
    }

    event _param(address indexed sender,uint256 tBurnFee,
    uint256 tNodeFee,uint256 tCommunityFee,uint256 tLpFee,uint256 tNftFee, uint256 tTransferAmount,string a);
