...
/**
 🏅⚔️Mini  Age Of GODS ⚔️🏅
🔥($Mini AOG) FIRST EVER HYPER THRUST AUTO AOG HODLING.🔥
 ⚔️2% $AOG Reward Token⚔️
 ⚔️LOW TAX Reflection Token

👨‍💻 RENOUNCED
🔒 LP LOCK 1 Year

⚔️🔖Max:            ⚔️Wallet 2% |  Max Txs: 2%
⚔️🔖 Fee:           ⚔️ Buy: 7%  |  Sell: 7%
⚔️🔖 Slippage:      ⚔️ Buy: 7   |  Sell: 12


⚔️network - Binance Smart Chain
⚔️2%Rewards As $AOG
⚔️2%Liquidity
⚔️1%Marketing

💌⚔️Telegram :https://t.me/MiniAOG
🌎⚔️Website https://babyaog.wixsite.com/token
*/
...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 public rewardToken = IERC20(0x40C8225329Bd3e28A043B029E0D07a5344d2C27C);
    IRouter02 router;
...
contract MiniAOGSale is ERC20, Ownable {
    ...
    address payable public teamWallet = payable(0xE213a73b8Bd76EDd9d65A1D89465E8624860dD1b);
    ...
    uint256 public maxWalletLimit = 2_200_000 * 10**18; // 2.2%
    
    IRouter02 public dexRouter;
    address public dexPair;
    uint256 public swapThreshold = 500_000 * 10**18; // 0.5%
    ...
    constructor() ERC20("Mini AOG", "Mini AOG") {
        // Create supply
    ...
    function excludeFromReward(address account, bool excluded) public onlyOwner {
        require((account != address(this) && account != dexPair) || excluded, "Mini AOG: Main pair and contract's addresses cannot be included from rewards");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Mini AOG: Account has already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }

    function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxWalletLimit[account] != excluded, "Mini AOG: Account has already the value of 'excluded'");
        _isExcludedFromMaxWalletLimit[account] = excluded;
        emit ExcludeFromMaxWalletLimit(account, excluded);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != dexPair, "Mini AOG: The main pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Mini AOG: Automated market maker pair is already set to that value");
    ...
    function setFees(uint8 newFee) external onlyOwner {
        require(newFee <= 9 && newFee >=0,"Mini AOG: Total fees must be between 0 and 9");
        totalFees = newFee;
        emit FeesUpdated(newFee);
    }

    function setMaxWalletLimit(uint256 amount) external onlyOwner {
        require(amount >= 500_000 && amount <= 2_000_000, "Mini AOG: Amount must be bewteen 500,000 and 2,000,000");
        maxWalletLimit = amount *10**18;
        emit MaxWalletLimitUpdated(amount);
    }

    function setSwapThreshold(uint256 amount) external onlyOwner {
        require(amount >= 1 && amount <= 1_000_000, "Mini AOG: Amount must be bewteen 1 and 1 000 000");
    ...
    function setLaunchTimestamp(uint256 timestamp) external onlyOwner {
        require(launchStartedTimestamp > block.timestamp, "Mini AOG: Updating the timestamp is not allowed if the launch is already started");
    ...
    function addPresaleAddresses(address account) public onlyOwner {
        require(!_presaleAddresses[account],"Mini AOG: This account is already added");
    ...
    function setLiquidityWallet(address newWallet) public onlyOwner {
        require(newWallet != liquidityWallet, "Mini AOG: The liquidity wallet has already this address");
        emit LiquidityWalletUpdated(newWallet, liquidityWallet);
        liquidityWallet = newWallet;
    }
    function setMarketingWallet(address payable newWallet) external onlyOwner {
        require(newWallet != marketingWallet, "Mini AOG: The marketing wallet has already this address");
        emit MarketingWalletUpdated(newWallet,marketingWallet);
        marketingWallet = newWallet;
    }

    function setTeamWallet(address payable newWallet) external onlyOwner {
        require(newWallet != teamWallet, "Mini AOG: The team wallet has already this address");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "Mini AOG: Transfer from the zero address");
        require(to != address(0), "Mini AOG: Transfer to the zero address");
        require(amount >= 0, "Mini AOG: Transfer amount must be greater or equals to zero");

        bool launchAlreadyStarted = block.timestamp >= launchStartedTimestamp;

        if(!launchAlreadyStarted) {
            require(_presaleAddresses[from], "Mini AOG: This account is not allowed to send tokens before launch is started");
        }
        ...
        if(launchAlreadyStarted && !isSellTransfer && !isBuyTransfer && !_isExcludedFromMaxWalletLimit[to]) {
            require(balanceOf(to) + amount <= maxWalletLimit, "Mini AOG: Amount exceeds the maxWalletLimit.");
        }
        ...
        // Buy
        if(isBuyTransfer){
            if(!_isExcludedFromMaxWalletLimit[to]) require(balanceOf(to) + amountWithoutFees <= maxWalletLimit, "Mini AOG: Amount exceeds the maxWalletLimit.");
        }
    ...
    function batchTokensTransfer(address[] calldata _accounts, uint256[] calldata _amounts) external onlyOwner {
        require(_accounts.length <= 200, "Mini AOG: 200 addresses maximum");
        require(_accounts.length == _amounts.length, "Mini AOG: Account array must have the same size as the amount array");
        require(_isExcludedFromReward[_msgSender()], "Mini AOG: Owner must be excluded from reward");
    ...
    function getStuckBNBs(address payable to) external onlyOwner {
        require(address(this).balance > 0, "Mini AOG: There are no BNBs in the contract");
        to.transfer(address(this).balance);
    } 

    function getStuckTokens(address payable to, address token, uint256 amount) external onlyOwner {
        require(IERC20(token).balanceOf(address(this)) > 0, "Mini AOG: There are tokens in the contract");
        require(token != address(this),"Mini AOG: Mini AOGSale tokens cannot be got from the contract");
        IERC20(token).transfer(to,amount);
    }


    function setDistributorSettings(uint256 newGas) external onlyOwner {
        require(newGas >= 100000 && newGas <= 900_000, "Mini AOG: Gas must be between 100,000 and 900,000");
        require(newGas != distributorGas, "Mini AOG: DistributorGas is already this value");
