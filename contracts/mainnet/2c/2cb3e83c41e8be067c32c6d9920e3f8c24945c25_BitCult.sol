...
contract BitCult is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BITCULTDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("BitCult", "BITCULT") {
        _buyRewardsFee = 5;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 2;
        _buyDevFee = 1;
    	...
    	dividendTracker = new BITCULTDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0xd02E35235cb9E03842830EDbf8ef2F9161e142CA);
        _devWallet = payable(0x26f7fd17E1dDA38e5519350917c876a6Ef71F7c8);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BITCULT: The dividend tracker already has that address");

        BITCULTDividendTracker newDividendTracker = BITCULTDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BITCULT: The new dividend tracker must be owned by the BITCULT token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "BITCULT: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BITCULT: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BITCULT: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "BITCULT: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BITCULT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BITCULT: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BITCULT: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "BITCULT: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BITCULT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BITCULT: Cannot update gasForProcessing to same value");
...
contract BITCULTDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BITCULT_Dividend_Tracker", "BITCULT_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "BITCULT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BITCULT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "BITCULT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BITCULT_Dividend_Tracker: Cannot update claimWait to same value");
