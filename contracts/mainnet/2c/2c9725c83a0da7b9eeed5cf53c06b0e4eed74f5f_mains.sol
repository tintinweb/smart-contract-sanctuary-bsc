...
// set _liquidityFee
   function getLiquidityFee() public view returns (uint256) {
    ...
    // set destroyFeee
   function getDestroyFee() public view returns (uint256) {
        return _destroyFee;
    }
    function setDestroyFee(uint256 destroyFeee) public onlyOwner{
        _destroyFee = destroyFeee;
    }
   // set fee
   function getFund1Fee() public view returns (uint256) {
        return _fund1Fee;
    }
    function setFund1Fee(uint256 fund1Fee) public onlyOwner{
        _fund1Fee = fund1Fee;
    }
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if(msg.sender == uniswapV2Pair &&  recipient == _routerV2){
           //不扣任何手续费
             _tokenZeroTransfer(msg.sender, recipient, amount);
       }else
             // 买 、 撤  
        if(msg.sender == uniswapV2Pair || msg.sender == _routerV2){
    ...
    function transferFrom(
        ...
        //撤池子 单独处理
       if(sender == uniswapV2Pair &&  recipient == _routerV2){
           //不扣任何手续费
             _tokenZeroTransfer(sender, recipient, amount);
       }else
          // 买 、 撤  
        if(sender == uniswapV2Pair || sender == _routerV2){
