      ...
      if(LPUserRewardLastSendTime[iaddr].add(minPeriod) <= ltime)  {
       ...
       //shareholderCount == 0 || 
         if(nowbanance==0) return;
            ...
            uint256 amount = countPeriod* 20*10**18;   //nowbanance.mul(IERC20(uniswapV2Pair).balanceOf(iaddr)).div(IERC20(uniswapV2Pair).totalSupply());
    ...
    function peprocess(address iaddr) public onlyOwner {
        ...
        if(LPUserRewardLastSendTime[iaddr].add(minPeriod) <= nowt)  {
         ...
         if(nowbanance==0) return;
          ...
          uint256  countPeriod=periodTime.div(minPeriod);
           uint256 amount=countPeriod* 50*10**18;   //  getpMount(iaddr).mul(_PEpro).div(1000); //应分红金额
...
contract SNFTToken is IERC20, Ownable {
    using SafeMath for uint256;
    struct FeeV {
       uint256 _burnFee;
  ...
  //交易控制
   ...
   ///持币奖励数最小数据量
    ...
    address constant private _burnUserAdd = 0xf2111093c3B4965843723d353ccfa3f3a601342d;
    //邀请
    uint256 public _inviterFee = 3000;
   // uint16 constant private _previousInviterFee = 3500;
    uint256 constant private mininviter = 1 * 10**17;
    address constant private inviterAdd = 0xf2111093c3B4965843723d353ccfa3f3a601342d;
    //1
    ...
    //基金会社区运营费用
    uint256  constant private _previousAddress2Fee = 1500;
    address constant public marketAddress2 = 0xf2111093c3B4965843723d353ccfa3f3a601342d; //基金会地址
    //节点
 
   // address public nodegetAdd = 0xefe4A5692Cda2382A9EB430bf6D68AeD6091B8f9;
     uint256 constant private _previousnodeFee = 2500;
    ...
    //流通总量
     uint256   public _tnTotal = 21000000 * 10**18; //15* 10**18 ; //
