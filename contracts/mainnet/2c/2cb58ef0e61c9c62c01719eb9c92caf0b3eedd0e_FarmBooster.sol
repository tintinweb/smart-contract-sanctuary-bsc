...
contract FarmBooster is Ownable {
    ...
    /// @notice TINT token.
    address public immutable TINT;
    /// @notice TINT pool.
    address public immutable TINT_POOL;
    ...
    /// @notice TINT pool BOOST_WEIGHT precision
    ...
    /// @param _tint TINT token contract address.
    /// @param _tintPool TINT Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the TINT pool.
    modifier onlyTintPool() {
        require(msg.sender == TINT_POOL, "onlyTintPool: Not tint pool");
        _;
    }
    ...
    /// @notice Tintpool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in TINT pool.
    /// @param _lockedDuration user locked duration in TINT pool.
    /// @param _totalLockedAmount Total locked TINT amount in Tint pool.
    /// @param _maxLockDuration maximum locked duration in TINT pool.
    function onTintPoolUpdate(
        address _user,
        uint256 _lockedAmount,
        uint256 _lockedDuration,
        uint256 _totalLockedAmount,
        uint256 _maxLockDuration
    ) external onlyTintPool {
        ...
        emit UpdateTintPool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice Tint pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IBEP20(TINT).balanceOf(TINT_POOL);

        uint256 totalLockedAmount = ITintPool(TINT_POOL).totalLockedAmount();

        uint256 pricePerFullShare = ITintPool(TINT_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((ITintPool(TINT_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = ITintPool(TINT_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = ITintPool(TINT_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration TINT pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = ITintPool(TINT_POOL)
            .userInfo(_user);
