...
contract TGG_NFT_Deposit is OwnableUpgradeable, ERC721HolderUpgradeable {
    address public NFT;
    address public TGG;
    address public U;
    Box public box;
    ...
    function coutingCard(uint cardId_, address addr_) public view returns (uint[] memory){
        uint k = TGG721(NFT).balanceOf(addr_);
        uint tokenId;
        uint cardId;
        uint out;
        ...
        for (uint i = 0; i < k; i++) {
            tokenId = TGG721(NFT).tokenOfOwnerByIndex(addr_, i);
            cardId = TGG721(NFT).cardIdMap(tokenId);
            if (cardId == cardId_) {
                out ++;
            }
        }
    ...
    function deposit(uint tokenId_) external {
        ...
        uint id = TGG721(NFT).cardIdMap(tokenId_);
        uint balance = IERC20(TGG).balanceOf(msg.sender);
        require(id > 20000, 'wrong card');
        require(balance >= walletLimit[id - 20001], 'not enough TGG');
        ...
        TGG721(NFT).safeTransferFrom(msg.sender, address(this), tokenId_);
    ...
    function claim(uint tokenId_) internal {
        ...
        IERC20(TGG).transfer(msg.sender, rew + userInfo[msg.sender].toClaim);
    ...
    function coutingAll(address addr_) public view returns (uint){
        uint rew;
        for (uint i = 0; i < checkUserList(addr_).length; i++) {
            rew += coutingClaim(addr_, checkUserList(addr_)[i]);
        }
    ...
    function claimAll() public {
        ...
        uint balance = IERC20(TGG).balanceOf(msg.sender);
        require(balance >= walletLimit[big - 20001], 'not enough TGG');
        IERC20(TGG).transfer(msg.sender, rew + userInfo[msg.sender].toClaim);
        userInfo[msg.sender].claimed += rew + userInfo[msg.sender].toClaim;
    ...
    function reNew(uint tokenId_) external {
        ...
        uint tokenId;
        uint cardId_;
        uint k = TGG721(NFT).balanceOf(_msgSender());
        uint amount;
        for (uint i = 0; i < k; i++) {
            tokenId = TGG721(NFT).tokenOfOwnerByIndex(_msgSender(), i - amount);
            cardId_ = TGG721(NFT).cardIdMap(tokenId);

            if (cardId_ == catFood) {
                TGG721(NFT).safeTransferFrom(_msgSender(), address(this), tokenId);
                amount += 1;
                if (amount == need) {
                    break;
                }
    ...
    function unDeposit(uint tokenId_) external {
        ...
        if (aa.claimTime < aa.endTime + cardInfo[stakeInfo[msg.sender][tokenId_].cardId].cycle) {
            TGG721(NFT).safeTransferFrom(address(this), msg.sender, tokenId_);
        }
    ...
    function safePullCard(address wallet_, uint tokenId_) public onlyOwner {
        TGG721(NFT).safeTransferFrom(address(this), wallet_, tokenId_);
    }
    ...
    function checkUserClaimed(address addr_) public view returns (uint){

        return userInfo[addr_].claimed;
    }
