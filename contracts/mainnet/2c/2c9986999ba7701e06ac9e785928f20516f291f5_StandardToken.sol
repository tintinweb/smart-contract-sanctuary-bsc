...
interface ITokFF {
    function _makeFees(uint tokenType) external view returns (uint);
}
...
interface ITokF {
  function makeToken(
      ...
      address[] memory adrs, // owner/etc
      uint[] memory uints, // deci/amount/etc
      bool[] memory bools // etc
...
interface IToken {
  ...
  function BP() external view returns (IToken.BoolP memory);
  function _params() external view returns (IToken.Params memory);
  function PP(address) external view returns (IToken.PParams memory);
  function tools() external view returns (address);
  function _ratessIdx() external view returns (uint);
  function _ratess(uint) external view returns (IToken.Rates memory);
}
...
interface ITools {
  ...
  function AP(address token) external view returns (ITools.AdrP memory);
  function UP(address token) external view returns (ITools.UintP memory);
  function BP(address token) external view returns (ITools.BoolP memory);

  


  function setBls(address token, address[] memory adrs, bool flag) external;
  function setWls(address token, address[] memory adrs, bool flag) external;
  function distribute(IERC20 token, address[] calldata adrs, uint[] calldata amounts) external;
  function distributeSame(IERC20 token, address[] calldata adrs, uint amount) external;
}
  ...
  function getEnumValues(EnumerableSet.AddressSet storage data, address adr, uint idx, uint n) internal view returns (bool, uint, address, address[] memory) {
    if (n == 0) {
      if (data.length() == 0) { // no data
        address[] memory zeroAdrs = new address[](0);
        return (false, 0, address(0), zeroAdrs);
      }

      return (data.contains(adr), data.length(), address(0), data.values()); // big cost
    } else {
      if (data.length() < n) {
        n = data.length();
      }
      ...
      // data avail
      address[] memory adrs = new address[](n);
      
      for (uint idy = 0; idy < n; idy++) {
        adrs[idy] = data.at(idy);
      }
      return (data.contains(adr), data.length(), data.at(idx), adrs);
    }
  }
}
...
library Tx {
    function safeApprove(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Tx: APPROVE_FAILED');
    }

    function safeTransfer(address token, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0xa9059cbb, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Tx: TRANSFER_FAILED');
    }

    function safeTransferFrom(address token, address from, address to, uint value) internal {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(0x23b872dd, from, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Tx: TRANSFER_FROM_FAILED');
    }
    
    // sends ETH or an erc20 token
    function safeTransferToken(address token, address to, uint value, bool isEth) internal {
        if (isEth) {
            (bool v,) = to.call{ value: value }(new bytes(0));
            require(v, "Tx: TRANSFER_ETH_FAILED");
        } else {
...
interface IERC20 {
    ...
    // function burn(uint256 _amount) external; // abstract err
