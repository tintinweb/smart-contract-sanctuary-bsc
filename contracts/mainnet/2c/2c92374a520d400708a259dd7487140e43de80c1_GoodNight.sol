...
contract GoodNight is Context, IERC20, Ownable{
    ...
    function initialfee(uint256 iSupply) private pure returns(uint256, uint256) {
    ...
    function tokenRemove(uint256 rDev) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
            totalFees.dev +=rDev;
        }        
    ...
    function updateprovideBonus(uint256 rMarketing) external onlyOwner {       
        if(_isExcluded[address(this)])
        {
