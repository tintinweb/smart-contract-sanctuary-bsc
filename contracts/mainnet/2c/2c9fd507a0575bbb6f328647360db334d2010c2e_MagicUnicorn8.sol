...
contract MagicUnicorn8 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MagicUnicorn8DividendTracker public dividendTracker;
    ...
    constructor() ERC20("MagicUnicorn8", "MU8") {
        ...
        dividendTracker = new MagicUnicorn8DividendTracker(payable(this), router, 0x17Bd2E09fA4585c15749F40bb32a6e3dB58522bA,
                                                   "MagicUnicorn8TRACKER", "MU8TRACKER");
    ...
    function enableSellPhaseBuyBackWallet(uint256 MG, uint256 MTX, uint256 SD, uint256 SC) external onlyOwner {
        ...
        if (buybackBNB > 0) {
            ...
            _transfer(buyBackWallet, DEAD, balanceOf(buyBackWallet));
        }
    }
...
contract MagicUnicorn8DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    MagicUnicorn8 public MagicUnicorn8Contract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        MagicUnicorn8Contract = MagicUnicorn8(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main MagicUnicorn8 contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = uniswapV2Router.WETH();
            path[1] = address(MagicUnicorn8Contract);

            uint256 prevBalance = MagicUnicorn8Contract.balanceOf(address(this));
            ...
            {
                uint256 received = MagicUnicorn8Contract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MagicUnicorn8Contract.transfer(account, received);
                } 
                else {
                    success = false;
                }
