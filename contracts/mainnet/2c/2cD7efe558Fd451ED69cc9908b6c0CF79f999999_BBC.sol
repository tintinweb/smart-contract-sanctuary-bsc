...
abstract contract AbsToken is IERC20, Ownable {
    ...
    uint256 public _buyLPDividendLPFee = 30;
    ...
    uint256 public _sellLPDividendLPFee = 170;
    ...
    constructor (
        ...
        holderRewardLPCondition = 1 ether / 1000;
    }
    ...
    function _transfer(
        ...
        if (from != address(this)) {
            ...
            processRewardLP(500000);
            if (progressRewardLPBlock != block.number) {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyFundFee2 + _buyLPDividendFee + _buyLPFee + _buyLPDividendLPFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellFundFee2 + _sellLPDividendFee + _sellLPFee + _sellLPDividendLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 totalLPFee = lpFee + lpDividendLPFee;
        uint256 totalFee = fundFee + fundFee2 + lpDividendFee + totalLPFee;
        totalFee += totalFee;
        ...
        if (lpUsdt > 0) {
            ...
            uint256 lpFeeLP = liquidity * lpFee / totalLPFee;
            if (lpFeeLP > 0) {
                IERC20(_mainPair).transfer(_receiveAddress, lpFeeLP);
            }
        }
    }
    ...
    function setBuyFee(
        ...
        uint256 lpDividendFee, uint256 lpFee, uint256 lpDividendLPFee
    ) external onlyOwner {
    ...
    uint256 public holderCondition = 1000;
    ...
    uint256 public progressRewardLPBlock;
    uint256 public progressRewardLPBlockDebt = 100;

    function processRewardLP(uint256 gas) private {
        uint256 blockNum = block.number;
        if (progressRewardLPBlock + progressRewardLPBlockDebt > blockNum) {
            return;
        }

        IERC20 usdt = IERC20(_mainPair);
        ...
        progressRewardLPBlock = blockNum;
    }
    ...
    function setProgressRewardLPBlockDebt(uint256 blockDebt) external onlyOwner {
        progressRewardLPBlockDebt = blockDebt;
    }
}

contract BBC is AbsToken {
    constructor() AbsToken(
        ...
        6,
        10000,
        address(0xF3aE0e4e374DdCc36EB452b1Baa381E361c585F3),
        address(0xF3aE0e4e374DdCc36EB452b1Baa381E361c585F3),
        address(0xE799F34aeB5B5eF7A0d4502C165f59280209fe84),
        0
    ){
