...
contract Moomins is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address payable public moomWallet = payable(0x7E729bD73Eda64C917Da69EE80dBe508620d76bF);

  // Supply
  uint256 public mintReward = 70;
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "Moomins: minting has not started yet");
    require(_mintAmount > 0, "Moomins: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "Moomins: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "Moomins: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "Moomins: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "Moomins: zero address");
    require(_amount > 0, "Moomins: you have to mint at least one");
    require(lastSupply != 0,  "Moomins: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"Moomins: URI query for nonexistent token");
  ...
  */
  ...
  function handleMintReward(uint256 _amount) private {
      ...
      // Send owner share
      payable(moomWallet).transfer(devShare);
  }
