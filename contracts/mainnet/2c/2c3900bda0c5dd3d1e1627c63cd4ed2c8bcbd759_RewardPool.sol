...
// File: contracts/distribution/RewardPool.sol
...
// Note that this pool has no minter key of TRI (rewards).
// Instead, the governance will call TRI distributeReward method and send reward to this pool at the beginning.
contract RewardPool {
    using SafeMath for uint256;
    ...
    // The time when TRI mining starts.
    uint256 public poolStartTime;

    // The time when TRI mining ends.
    ...
    function init() public onlyOperator {
        require(rewardPerSecond == 0, "already initialized");
        rewardPerSecond = reward.balanceOf(address(this)) / runningTime;
    }
    ...
    // View function to see pending TRI on frontend.
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        uint256 _rewardBal = reward.balanceOf(address(this));
        if (_rewardBal > 0) {
            if (_amount > _rewardBal) {
                reward.safeTransfer(_to, _rewardBal);
            } else {
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        // do not allow to drain core token (TRI or lps)
