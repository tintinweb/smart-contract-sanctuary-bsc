...
contract SUPERRATS is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public RewardToken = 0xAf98e6c2D3b520F4e9b3D7ea22ad7AeFFFDbc2fd;
    address public FeeReceiver = 0xED7d83bf6209a0259A3CEcDd4d87409590cF0Ab4;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "SUPERRATS: minting has not started yet");
        require(_mintAmount > 0, "SUPERRATS: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "SUPERRATS: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "SUPERRATS: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "SUPERRATS: total cost isn't match"
            );
        }
        // Minting
    ...
    {
        // Checks
        require(_winner != address(0), "SUPERRATS: zero address");
        require(_amount > 0, "SUPERRATS: you have to mint at least one");
        require(lastSupply != 0, "SUPERRATS: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "SUPERRATS: URI query for nonexistent token");
