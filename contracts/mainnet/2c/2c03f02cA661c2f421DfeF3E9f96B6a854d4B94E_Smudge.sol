...
contract Smudge is Ownable, ERC721Enumerable {
    ...
    uint256 private _mintPrice = 0.5 ether;
    string private _baseTokenURI = "";

    constructor(address _marketingWallet) ERC721("SmudgeNFT", "SMCN") {
    ...
    function mintPrice() public view returns (uint256) {
        ...
        if (supply < 250) {
            return _mintPrice.div(100).mul(20);
        }
        if (supply < 500) {
            return _mintPrice.div(100).mul(40);
        }
        if (supply < 750) {
            return _mintPrice.div(100).mul(60);
        }
        return _mintPrice;
    }
    ...
    function increaseMaxMints(uint256 count) external onlyOwner {
        MAX_MINTS += count;
        require(MAX_MINTS <= 2000, "Total mints exceeds allowance");
    }
