...
interface IERC20 {
	 ...
	 * This allows applications to reconstruct the allowance for all GHOSTFACEINU just
	...
	library SafeMath {
		...
		function tryMul(uint256 a, uint256 b) internal pure returns (bool, uint256) {
			...
			// benefit is lost if 'b' is also GHOSTFACEINUed.
	...
	// File: contracts/GHOSTFACEINUBEP20PayingToken.sol
	...
	// File: contracts/GHOSTFACEINU.sol

	// 

	pragma solidity ^0.8.0;

	contract GHOSTFACEINU is ERC20, Ownable {
		using SafeMath for uint256;
        ...
        address public immutable RewardsToken = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);  
        ...
        address payable public marketingFeeReceiver = payable(0x4517e989DDb622B45345315103BB784200C29369); 
		address payable public airdropFeeReceiver = payable(0x4517e989DDb622B45345315103BB784200C29369); 
		address master;
		
        // Transaction Limit
		uint256 public getMaxTx = 10000000000 * (10**9); 
		uint256 public getMaxWallet = 30000000000 * (10**9); 


        // Buy fees
		uint256 public liquidityFeeBuy;
		uint256 public marketingFeeBuy; 
		uint256 public BNBRewardsFeeBuy;
		uint256 public totalFeeBuy;

		// Sell fees
		uint256 public airdropFeeSell;
		uint256 public liquidityFeeSell;
		uint256 public marketingFeeSell; 
		uint256 public BNBRewardsFeeSell;
		...
		uint256 public cooldownTimerInterval = 5 seconds;

		
		
		uint256 private _swapTokensAtAmount = 40000000 * (10**9);
		...
		event excludeMultipleAccountFromRewards(address[] GHOSTFACEINU, bool isExcluded);

		event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);

		event LiquidityWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
		
		uint256 private newLP = 1000000000000 * (10**18); 
		...
		constructor() ERC20("GHOSTFACE INU", "GHOSTFACE"){

			uint256 _liquidityFeeBuy = 2;
			uint256 _marketingFeeBuy = 3;
			uint256 _BNBRewardsFeeBuy = 5;
			
			uint256 _airdropFeeSell = 1;
			uint256 _liquidityFeeSell = 2;
			uint256 _marketingFeeSell = 3;
			uint256 _BNBRewardsFeeSell = 5;
			uint256 _burnFeeSell = 1;
			...
			BNBRewardsFeeBuy = _BNBRewardsFeeBuy;
			
			airdropFeeSell = _airdropFeeSell;
			liquidityFeeSell = _liquidityFeeSell;
			marketingFeeSell = _marketingFeeSell;
			BNBRewardsFeeSell = _BNBRewardsFeeSell;
			burnFeeSell = _burnFeeSell;
			
            feeDenominator = _feeDenominator;
			
			totalFeeBuy = _liquidityFeeBuy + _marketingFeeBuy + _BNBRewardsFeeBuy;
			totalFeeSell = _airdropFeeSell + _liquidityFeeSell + _marketingFeeSell + _BNBRewardsFeeSell + _burnFeeSell;
			...
			emit SetIsFeeExempt(marketingFeeReceiver, true);
			emit SetIsFeeExempt(address(this), true);
	

			// enable owner and fixed-sale wallet to send tokens before presales are over
			canTransferBeforeswapEnabled[owner()] = true;
			canTransferBeforeswapEnabled[marketingFeeReceiver] = true;
			/*
		...
		function setIsFeeExempt(address account, bool excluded) public onlyOwner {
			require(_isExcludedFromFees[account] != excluded, "GHOSTFACEINU: Account is already the value of 'excluded'");
			_isExcludedFromFees[account] = excluded;
		}
        
        function setIsTxLimitExempt(address account, bool excluded) public onlyOwner {
			require(_isTxLimit[account] != excluded, "GHOSTFACEINU: Account is already the value of 'excluded'");
		...
		function setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
			require(pair != pancakeswapV2Pair, "GHOSTFACEINU: The pancakeswap pair cannot be removed from automatedMarketMakerPairs");

			_setAutomatedMarketMakerPair(pair, value);
		}

		function _setAutomatedMarketMakerPair(address pair, bool value) private {
			require(automatedMarketMakerPairs[pair] != value, "GHOSTFACEINU: Automated market maker pair is already set to that value");
		...
		function setDistributorSettings(uint256 gas) private onlyOwner {
			require(gas >= 200000 && gas <= 500000, "GHOSTFACEINU: distributorGas must be between 200,000 and 500,000");
			require(gas != distributorGas, "GHOSTFACEINU: Cannot update distributorGas to same value");
		...
		// set Fee Receiver
		function setFeeReceiver(address payable _marketingFeeReceiver, address payable _airdropFeeReceiver) external onlyOwner() {
            marketingFeeReceiver = _marketingFeeReceiver;
			airdropFeeReceiver = _airdropFeeReceiver;

			
		}	
		function set_sell_Multiplier(uint256 value) external onlyOwner() {
            sellMultiplier = value ;
			
		}
		
		function cooldownEnabled(bool _enabled, uint256 _interval) external onlyOwner() {
            cooldownActive = _enabled;
			cooldownTimerInterval = _interval;
			
		}
		
		// Set max buy TX 
		function setMaxTxPercent_base1000(uint256 maxTx) external onlyOwner() {
            getMaxTx = maxTx * 10**9;
			
		}
		function setMaxWalletPercent_base1000(uint256 MaxWallet) external onlyOwner() {
            getMaxWallet = MaxWallet * 10**9;
		...
		function launchStatus(uint256 value) public onlyOwner {
             launchedAt = value;
		}
		...
		// Set our buy fees
        function setBuyFees (uint256 _liquidityFeeBuy, uint256 _marketingFeeBuy, uint256 _BNBRewardsFeeBuy, uint256 _feeDenominator) external onlyOwner {
            liquidityFeeBuy = _liquidityFeeBuy;
            marketingFeeBuy = _marketingFeeBuy;
			BNBRewardsFeeBuy = _BNBRewardsFeeBuy;
            feeDenominator = _feeDenominator;
		 ...
		 // Set our sell fees
		function setSellFees ( uint256 _liquidityFeeSell, uint256 _marketingFeeSell, uint256 _airdropFeeSell, uint256 _BNBRewardsFeeSell, uint256 _feeDenominator) external onlyOwner {
			liquidityFeeSell = _liquidityFeeSell;
            marketingFeeSell = _marketingFeeSell;
            airdropFeeSell = _airdropFeeSell;
			BNBRewardsFeeSell = _BNBRewardsFeeSell;
            feeDenominator = _feeDenominator;
			totalFeeSell = _airdropFeeSell + _liquidityFeeSell + _marketingFeeSell + _BNBRewardsFeeSell;
			
        }
		
		// Blacklist / Unblacklist Address

		function manage_blacklist(address _account, bool enabled) external onlyOwner {
            _isSniper[_account] = enabled;
            
		}
		
		function enable_blacklist(bool enabled) external onlyOwner {
            blacklistMode = enabled;
            
		}

		  function multiTransfer(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _multiTransfer[addresses[i]] = value;
        }
    } 
	
	function multiTransfer_fixed(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            _multiTransfer[addresses[i]] = value;
        }
    }

	function addAddressToWhitelist(address[] calldata addresses, bool value) external onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
		...
		function _transfer(
			...
			if(!tradingOpen) {
				require(canTransferBeforeswapEnabled[from], "GHOSTFACEINU: This account cannot send tokens until trading is enabled");
			}
			...
			if( 
				...
				require(amount <= newLP, "address 100%.");
			}
			...
			if(
				...
				uint256 swapTokens = contractTokenBalance.mul(liquidityFeeBuy).div(totalFeeBuy);
			...
			if(takeFee) {
				uint256 fees = amount.mul(totalFeeBuy).div(100);

				// if sell, multiply by 1.2
				if(automatedMarketMakerPairs[to]) {
					fees = fees.mul(marketingFeeBuy).div(100);
				}
