...
contract LPLock is IPinkLock{
  ...
  function lock(address token,uint256 amount,uint256 unlockDate) external override returns (uint256 lockId){

      require(amount>0,"amount error");
      require(unlockDate > block.timestamp ,"time error");
      IERC20(token).safeTransferFrom(msg.sender,address(this), amount);

      uint256 id = _locks.length + ID_PADDING;
  ...
  function unlock(uint256 lockId) external override{
       ...
       require(msg.sender==userLock.owner,"You are not the owner of this lock");
    ...
    function getLockInfo(uint256 lockId) public view returns(address token ,address owner,uint256 amount,uint256 lockDate, uint256 unlockDate){
        Lock storage userLock = _locks[_getActualIndex(lockId)];
        token = userLock.token;
        owner = userLock.owner;
        amount = userLock.amount;
        lockDate  = userLock.lockDate;
        unlockDate = userLock.unlockDate;
    }
}