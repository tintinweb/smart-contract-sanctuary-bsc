...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x8b84931b8b81C1De88Cd31Ca379D6F62A4130a84);
...
contract ShiBalloon is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   SBLNDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("ShiBalloon", "SBLN") {
        _buyRewardsFee = 15;
        _buyLiquidityFee = 15;
		_buyMarketingFee = 0;
        _buyDevFee = 0;
    	...
    	dividendTracker = new SBLNDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x5B6252f90069effddBfE6c81E9eca2C4e1fc7fFc);
        _devWallet = payable(0xc862841f52e6D8617186C5A7E0f411C1a7fE3DF9);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "SBLN: The dividend tracker already has that address");

        SBLNDividendTracker newDividendTracker = SBLNDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SBLN: The new dividend tracker must be owned by the SBLN token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "SBLN: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SBLN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SBLN: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "SBLN: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SBLN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SBLN: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "SBLN: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "SBLN: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SBLN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SBLN: Cannot update gasForProcessing to same value");
...
contract SBLNDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("SBLN_Dividend_Tracker", "SBLN_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "SBLN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SBLN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "SBLN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SBLN_Dividend_Tracker: Cannot update claimWait to same value");
