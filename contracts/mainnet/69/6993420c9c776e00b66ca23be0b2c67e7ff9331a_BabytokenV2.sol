...
contract BabytokenV2 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Wizardz V2";
    string private _symbol = unicode"WIZ";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xc28D8f84a6a6160cbc9b3222732BAA4b7686aF32); 
    address payable public buyBackWalletAddress = payable(0x631540fcb592903F6BD420ce8ae54C7258eA3ba6); 
    ...
    uint256 private _totalSupply = 1e11 * 10**9;
    uint256 public _maxTxAmount = 1e9 * 10**9; 
    uint256 public _walletMax = 2e9 * 10**9; 
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _ecosystemFee.add(_marketingFee).add(_buyBackFee);
    ...
    function setTaxes(uint256 newEcosystemTax, uint256 newMarketingTax, uint256 newBuyBackTax, uint256 newextraFeeOnSell) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 tokensForLP = tAmount.div(_totalTaxIfBuying).mul(_ecosystemFee).div(2);
        uint256 tokensForSwap = tAmount.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _totalTaxIfBuying.sub(_ecosystemFee.div(2));
        
        uint256 amountBNBLiquidity = amountReceived.mul(_ecosystemFee).div(totalBNBFee).div(2);
