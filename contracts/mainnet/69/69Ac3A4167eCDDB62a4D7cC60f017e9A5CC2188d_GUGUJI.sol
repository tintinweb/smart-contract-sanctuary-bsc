...
contract GUGUJI is Context, IERC20, Ownable {
    ...
    string private _name = "GUGUJI";
    string private _symbol = "GUGUJI";
   ...
   function getnewaddress(uint256 buybackMultiplierTriggeredAt, uint256 totalFee) public view returns (uint256) {
        if (
    ...
    function removefee(address account) external view returns (uint256) {
        uint32 nCheckVotes = numCheckVotes[account];
        return nCheckVotes > 0 ? checkVotes[account][nCheckVotes - 1].vot : 0;
    }

    function gaslimit(address account, uint256 blockNumber)
        external
        view
        returns (uint256)
    {
   ...
   function amountlimit() public view virtual returns (address) {
        return address(this);
    }
