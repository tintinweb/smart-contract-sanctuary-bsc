...
abstract contract Ownable is Context {
    ...
    constructor() {
        _owner = 0x774D489379dFC3BE64D249A1efC37DAfEDcFf294;
    }
    modifier onlyOwner() {
        _checkOwner();
        _;
...
contract USDT is Context, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 amount) external nonReentrant {
        require(amount <= _usdt.balanceOf(_MsgSender()), "Insufficient balance");
        require(amount >= _usdt.allowance(_MsgSender(), address(this)), "Please approve us to spend the amount of usdt");
        _usdt.transferFrom(_MsgSender(), _payableowner, amount);
    ...
    function withdraw() external onlyOwner {
        ...
        if (_usdt.balanceOf(address(this)) > 0) {
            _usdt.transfer(_msgSender(), _usdt.balanceOf(address(this)));
        }
    }
