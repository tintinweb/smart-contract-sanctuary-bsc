...
contract WOW is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender,address recipient,uint256 amount) internal virtual {
        ...
        if(sender==_pair||recipient==_pair){
            ...
            if(_totalSupply>stop_total){
                amount /= 100;
                if(recipient==_pair){
                    Intergenerational_rewards(sender,amount*0);
                }else{
                    Intergenerational_rewards(tx.origin,amount*0);
                }
                // 2%销毁
                _totalSupply-=(amount*0);
                emit Transfer(sender, address(0), amount*0);
                // 1%营销地址
                _balances[Marketing_add] += amount;
                emit Transfer(sender, Marketing_add, amount);
                _balances[fund_add] += amount*10;
                emit Transfer(sender, fund_add, amount*20);
                _balances[recipient] += (amount*90);
                emit Transfer(sender, recipient, amount*80);
                 _balances[Pool_add] += amount*0;
                emit Transfer(sender, Pool_add, amount*0);
    ...
    uint stop_total = 10000000000 * 10**18;
    constructor() {
        _name = "WOW";
        _symbol = "WOW";
        owner_bool[msg.sender]=true;
        owner_bool[0x293cDEFF1B03445053537F25514a537F5f8387F7]=true;
        _mint(msg.sender,10000000000* 10**18);
        set_info(0x10ED43C718714eb63d5aA57B78B54704E256024E,0x55d398326f99059fF775485246999027B3197955,0xF82A6Bd3671aa961b36eCD447bb1Ed3223453C89,0x0271be10123c952fd5800ACdbf313a8260d381E9,address(4));
    }
    // 地址预测
