...
contract ETHSHIB is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    ETHSHIBDividendTracker public dividendTracker;
    ...
    uint256 public _BuyBackSHIBAFee = 1;
  
    uint256 public _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackSHIBAFee);
    
    uint256 public _SellETHRewardsFee = 8;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 5;
    uint256 public _SellBackSHIBAFee = 1;
    uint256 public _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackSHIBAFee);
    
    uint256 public maxWalletToken = 10000 * (10**9) * (10**18); //1%

    address public _marketingWalletAddress = 0x30F5B429D0783ae9B0378AfBA7180f1fe91ff843;
    address public _shibaWalletAddress = 0xd7b8727700e2Bc0ee722aa23131F6DB3481614f2;
    ...
    constructor() ERC20("ETH SHIBA", "$ETHSHIB") {

    	dividendTracker = new ETHSHIBDividendTracker();
        ...
        dividendTracker.excludeFromDividends(_shibaWalletAddress);
        ...
        excludeOrIncludeFromFees(_shibaWalletAddress, true);
        ...
        excludeOrIncludeFromMaxTx(_shibaWalletAddress, true);
        ...
        excludeOrIncludeFromMaxWallet(_shibaWalletAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ETHSHIB: The dividend tracker already has that address");

        ETHSHIBDividendTracker newDividendTracker = ETHSHIBDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "ETHSHIB: The new dividend tracker must be owned by the ETHSHIB token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETHSHIB: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETHSHIB: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxTx[account] != excluded, "ETHSHIB: Account is already the value of 'excluded'");
    ...
    {
        require(_isExcludedFromMaxWallet[account] != excluded, "ETHSHIB: Account is already the value of 'excluded'");
    ...
    function setShibaWallet(address payable wallet) external onlyOwner{
         require(wallet != address(0), "Invalid address");
        _shibaWalletAddress = wallet;
    }
      ...
      function setBuyFees(uint256 buyEthRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBackSHIBAFee) external onlyOwner
    {
        ...
        _BuyTotalFees = _BuyETHRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee).add(_BuyBackSHIBAFee);
    ...
    function setSellFees(uint256 sellEthRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBackSHIBAFee) external onlyOwner
    {
        ...
        _SellTotalFees = _SellETHRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee).add(_SellBackSHIBAFee);

    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHSHIB: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "ETHSHIB: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "ETHSHIB: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        
        {
            ...
            uint256 walletTokens = contractTokenBalance.mul(_SellMarketingFee.add(_SellBackSHIBAFee)).div(_SellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForEth(walletTokens);
            uint256 newBalance = address(this).balance.sub(contractBalance);
            uint256 marketingShare = newBalance.mul(_SellMarketingFee).div(_SellBackSHIBAFee.add(_SellMarketingFee));
            uint256 developmentShare = newBalance.sub(marketingShare);
            payable(_marketingWalletAddress).transfer(marketingShare);
            payable(_shibaWalletAddress).transfer(developmentShare);
...
contract ETHSHIBDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ETHSHIB_Dividend_Tracker", "ETHSHIB_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ETHSHIB_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ETHSHIB_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHSHIB contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ETHSHIB_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ETHSHIB_Dividend_Tracker: Cannot update claimWait to same value");
