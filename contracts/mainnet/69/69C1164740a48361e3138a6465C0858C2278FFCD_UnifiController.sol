...
interface IEventBroadcaster {
    function broadcastEvent(bool isDeposit, bool isWithdrawl, bool isClaim, uint currentLiquidity, uint liquidity, address user, address pool,address receipient) external;
}
interface IUP {
...
interface IOldFeeController {
  

 
    function lp_RewardPerToken(address pool) external view returns( uint );
    function lp_userState(address _pool , address _user) external view returns(uint);
...
contract UnifiController {
    ...
    //remember to update UNIFIVAULT ATTRIBUTE AFTER DEPLOYING IT
    ...
    IUP public uptoken;
    IOldFeeController public oldFeeController;
    ...
    constructor(address _wbnb, address _up_address, address _oldFeeController) {
        ...
        oldFeeController = IOldFeeController(_oldFeeController);
    ...
    function _mintNativeUP(address _from, uint _ethAtm) internal returns(uint) {
        ...
        uint upMinted = uptoken.balanceOf(address(this))  - beforeUpBalance;
        
        uint upMintedForPool = upMinted * this.getMintRate(_from) / 10000;
        if(upMinted >upMintedForPool ){
            uptoken.justBurn(upMinted - upMintedForPool);           
        }

        return (upMintedForPool);
    ...
    function _mintUPinBulk(address _pool ) internal returns(uint) {
        ...
        uptoken.mint {
            value: lp_nativeTokensAccumulated[_pool] 
        }(address(this), lp_nativeTokensAccumulated[_pool] );
        uint upMinted = uptoken.balanceOf(address(this))  - beforeUpBalance;
        uint upMintedForPool = upMinted * this.getMintRate(_pool) / 10000;
        if(upMinted >upMintedForPool ){
            uptoken.justBurn(upMinted - upMintedForPool);           
        }
    ...
    if(oldFeeController.lp_RewardPerToken(_pool)>0 && lp_RewardPerToken[_pool] == 0 ){
        lp_RewardPerToken[_pool]  = oldFeeController.lp_RewardPerToken(_pool);
    }
    ...
    function pendingUpRewards(address _user, address _pool) external view returns(uint256) {
            //if(oldSCREwardPerToken > 0 && current rewardToken is 0  retrieve and use old balance)
            //else use new one
            ...
            if(oldFeeController.lp_RewardPerToken(_pool)>0 && lp_RewardPerToken[_pool] == 0 ){
                 return (oldFeeController.lp_RewardPerToken(_pool) - (oldFeeController.lp_userState(_pool,_user))) * (IUnifiPair(_pool).balanceOf(_user)) / (1e18); // up decimal places      
    ...
    function broadcastEvent(bool isDeposit, bool isWithdrawl, bool isClaim, uint currentLiquidity, uint liquidity, address user, address pool,address receipient) internal returns(bool) {
        this.claimPlatformUPFees();
        if (broadcastEnabled) {
            ieb.broadcastEvent(isDeposit, isWithdrawl, isClaim, currentLiquidity, liquidity, user, pool,receipient); //
        }
        return true;
    }
    ...
    function claimUP(address _user, address _receipient, uint _liquidity, bool _isDeposit, bool _isWithdrawl, bool _isClaim) public returns(uint) {
        ...
        broadcastEvent(_isDeposit, _isWithdrawl, _isClaim, currentLiquidity, _liquidity, _user, msg.sender,_receipient);
        return upRewards;
    }
    ...
    function claimUP(address _user, address _upRecipient, address _pool) public returns(uint) {
        ...
        broadcastEvent(false, false, true, currentLiquidity, currentLiquidity, _user, _pool,_user);
        return upRewards;
    }
   ...
   //if(current rewardToken is 0  && oldSCREwardPerToken > 0 &&  )
            // retieve and store the pool details 

   //if(current user pool rewardToken is 0  &&  old details is  > 0 &&  )
            // retieve and store the user details 

    if(lp_RewardPerToken[_pool] == 0 && oldFeeController.lp_RewardPerToken(_pool)>0  ){
        lp_RewardPerToken[_pool]  = oldFeeController.lp_RewardPerToken(_pool);
    }

    if(lp_userState[_pool][_user] == 0 && oldFeeController.lp_userState(_pool,_user)>0){
        lp_userState[_pool][_user] = oldFeeController.lp_userState(_pool,_user);
    }
    ...
    function justBurnUP(uint amount ) public onlyOwner{
        require(uptoken.balanceOf(address(this)) >= amount);
        uptoken.justBurn(amount);
    }
