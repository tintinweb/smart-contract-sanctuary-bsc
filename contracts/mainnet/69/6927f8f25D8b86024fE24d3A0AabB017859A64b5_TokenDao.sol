...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        // 各种费率
        _cPercent = 50;   // 转账买卖添加移除扣费万份之50  交易转账中扣取
        lotteryR = 10;    // 抽奖费率  万份之10           交易中扣取
        rewardR = 3000;   // 每次只分30%  万分之3000    lp 分红每次只分
        managerR = 500;    // 基金费率 5%  万份之500 从用户lp分红中扣取
        divBase = 10000;     //费率 万份比
        ...
        tradeLockInterval = 60; // 卖出间隔 600 为600秒 或者10分钟
        ...
        lottery.interval = 86400; // 抽奖的时间间隔
        lottery.epochLength = 300;  
        lottery.rewardNum = 10;   
        _updateLottery(_current);

        lockRe.start = _current;
        lockRe.lockInterval = 86400;  // 交易对限制移除的时长 ， 从合约部署开始计算     
        lockRe.openInterval = 43200;  // 可以移除的时长
