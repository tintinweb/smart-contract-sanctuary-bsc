...
contract UNC is Context, IBEP20, Ownable {
    ...
    uint256 private finalTotalToken = 210 * 10**4 * 10**18;
    string private _name = "UNC";
    string private _symbol = "UNC";
    uint8 private _decimals = 18;
    
    uint256 public _buyFee = 11;
    uint256 public _saleFee = 11;
    uint256 public _transferFee =0;
    uint256 public _cancelFee =0;

    uint256 public _backFee = 19;
    uint256 public _burnFee = 45;
    uint256 public _lpShareFee = 27;
    uint256 public _fundingFee = 9;

    uint256 public _oneBnbToCdao=1;
    ...
    constructor () {
       IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        pancakeswapV2PairCdao = pancakeswapV2Pair;
        pancakeswapV2RouterCdao = 0x10ED43C718714eb63d5aA57B78B54704E256024E;

        addressCdao = _pancakeswapV2Router.WETH();
        backAddress = _msgSender();
        fundingAddress = _msgSender();
    ...
    function setFundingAddress(address _fundingAddress) external onlyOwner() {
        require(fundingAddress != _fundingAddress);
    ...
    function dealWithShareBonus(uint256 tokenBalance) private   {
        ...
        uint256 fundingBonusNum = tokenBalance.mul(_fundingFee).div(100);
        if(fundingBonusNum>0){
            _balances[fundingAddress] = _balances[fundingAddress].add(fundingBonusNum);
            leftBouns=leftBouns.sub(fundingBonusNum);
        }
    ...
    function doShareBonusCdao(uint256 shareBonus) private lockTheSwap {
        ...
        if(newBalance>0){
             dividendTracker.process(distributorGas,newBalance);           
        }
    }
...
contract TokenDividendTracker is Ownable {
    ...
    function process(uint256 gas,uint256 doCdaoNum) external onlyOwner {
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            payable( shareholders[currentIndex] ).call{value: amount};
