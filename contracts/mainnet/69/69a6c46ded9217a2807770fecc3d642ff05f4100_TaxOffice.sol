...
contract TaxOffice is MultipleOperator {
    ...
    */
    ...
    function calculateMainTokenTax() external view virtual returns(uint256 taxRate){
    ...
    function calculateShareTokenTax() external view virtual returns(uint256 taxRate){
        uint256 mainTokenPrice = _getMainTokenPrice();
...
contract TaxOfficeV5 is TaxOffice {
    ...
    struct UserInfo {
        ...
        uint256 mainTokenReferralAmount; //Tax Referrals earnt in kraken.
        uint256 shareTokenReferralAmount; //Tax Referrals earnt in kshare.
    ...
    event ReferralPaid(address indexed referrer, address indexed referree, address indexed token, uint256 amount, uint256 timeStamp);
    ...
    function calculateMainTokenTax() external view override returns(uint256 taxRate){
    ...
    function calculateShareTokenTax() external view override returns(uint256 taxRate){
    ...
    function handleMainTokenTax(uint256 _amount) external override onlyOperator {
        ...
        //Calculate the referral weight.
        uint256 referrerAmount;
        (address _upline, uint256 _referrerWeight, ) = referralManager.calculateTaxOfficeReferralWeights(
            tx.origin,
            0,
            _amount
        );

        //Handle referral amount if ignoring swap.
        if (mainTokenReferralFollowsSwap == false) {
            if (_upline != address(0) && _referrerWeight > 0) {
                //Calculate referral amount.
                referrerAmount = _amount.mul(_referrerWeight).div(BASIS_POINTS_DENOM);
                mainToken.transfer(_upline, referrerAmount);
                _amount = _amount.sub(referrerAmount);

                //Add referral amount to the upline.
                userInfo[_upline].mainTokenReferralAmount = userInfo[_upline].mainTokenReferralAmount.add(referrerAmount);

                //Emit event
                emit ReferralPaid(_upline, tx.origin, address(mainToken), referrerAmount, block.timestamp);
            }
        }
        
        //Swap for main token tax.
        if (mainTokenTaxPath.length > 1) {
            IERC20(mainToken).safeIncreaseAllowance(router, _amount);
            ...
            _amount = swapAmounts[swapAmounts.length-1];
        }

        //Handle referral amount if following swap.
        if (mainTokenReferralFollowsSwap == true) { 
            if (_upline != address(0) && _referrerWeight > 0) {
                referrerAmount = _amount.mul(_referrerWeight).div(BASIS_POINTS_DENOM); 
                mainTokenTaxPathEndpoint.transfer(_upline, referrerAmount);

                //Add referral amount to the upline.
                if (mainTokenTaxPathEndpoint == mainToken) {
                    userInfo[_upline].mainTokenReferralAmount = userInfo[_upline].mainTokenReferralAmount.add(referrerAmount);
                } else if (mainTokenTaxPathEndpoint == shareToken) {
                    userInfo[_upline].shareTokenReferralAmount = userInfo[_upline].shareTokenReferralAmount.add(referrerAmount);
                } else {
                    userInfo[_upline].daiReferralAmount = userInfo[_upline].daiReferralAmount.add(referrerAmount);
                }

                //Emit event
                emit ReferralPaid(_upline, tx.origin, address(mainTokenTaxPathEndpoint), referrerAmount, block.timestamp);
            }
        }
    ...
    function handleShareTokenTax(uint256 _amount) external override onlyOperator {
        ...
        //Handle referral amount if ignoring swap.
        if (shareTokenReferralFollowsSwap == false) {
            if (_upline != address(0) && _referrerWeight > 0) {
                //Calculate referral amount.
                referrerAmount = _amount.mul(_referrerWeight).div(BASIS_POINTS_DENOM);
                shareToken.transfer(_upline, referrerAmount);
                _amount = _amount.sub(referrerAmount);
                ...
                //Emit event
                emit ReferralPaid(_upline, tx.origin, address(shareToken), referrerAmount, block.timestamp);
            }
        }
        
        //Swap for share token tax.
        if (shareTokenTaxPath.length > 1) {
        ...
        //Handle referral amount if following swap.
        if (shareTokenReferralFollowsSwap == true) { 
            if (_upline != address(0) && _referrerWeight > 0) {
                referrerAmount = _amount.mul(_referrerWeight).div(BASIS_POINTS_DENOM); 
                shareTokenTaxPathEndpoint.transfer(_upline, referrerAmount);

                //Add referral amount to the upline.
                if (shareTokenTaxPathEndpoint == mainToken) {
                    userInfo[_upline].mainTokenReferralAmount = userInfo[_upline].mainTokenReferralAmount.add(referrerAmount);
                } else if (shareTokenTaxPathEndpoint == shareToken) {
                    userInfo[_upline].shareTokenReferralAmount = userInfo[_upline].shareTokenReferralAmount.add(referrerAmount);
                } else {
                    userInfo[_upline].daiReferralAmount = userInfo[_upline].daiReferralAmount.add(referrerAmount);
                }
                ...
                //Emit event
                emit ReferralPaid(_upline, tx.origin, address(shareTokenTaxPathEndpoint), referrerAmount, block.timestamp);
            }
        }
    ...
    function setTaxPaths(
        ...
        mainTokenTaxPathEndpoint = _mainTokenTaxPath.length > 1 ? IERC20(_mainTokenTaxPath[_mainTokenTaxPath.length-1]) : mainToken;
        shareTokenTaxPath = _shareTokenTaxPath;
        shareTokenTaxPathEndpoint = _shareTokenTaxPath.length > 1 ? IERC20(_shareTokenTaxPath[_shareTokenTaxPath.length-1]) : mainToken;
    }
