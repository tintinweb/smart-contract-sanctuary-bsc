...
// File: contracts/NftLauchPad.sol
...
contract NftMintyFactory is Ownable {
   ...
   mapping (address => MyCollection) public nftToCollections;
   ...
   MyCollection[] public theCollections;
    ...
    function setCollectionFeeAddress(address _wallet) public onlyOwner {
        collectionFeeAddress = _wallet;
    }

    function withdrawMaticFees() public {
        require(msg.sender == collectionFeeAddress, "only collection owner can withdraw");
        uint256 cBalance = payable(address(this)).balance;
        payable(collectionFeeAddress).transfer(cBalance);
    }
    ...
    function createCollection(
        ...
        theCollections.push(MyCollection(newCollection, _name, _symbol, _cBanner, _description, msg.sender, _website, _twitterLink, _discordLink, _maxSupply, _revealed, _fee));
        nftToCollections[newCollection] = MyCollection(newCollection, _name, _symbol, _cBanner, _description, msg.sender, _website, _twitterLink, _discordLink, _maxSupply, _revealed, _fee);
    }
    ...
    function getCollectionProps(uint256 index) public view returns(address, string memory, string memory, string memory, string memory, address, string memory, string memory, string memory, uint256, bool, uint256){
    ...
    function getCollectionInfo(address _collection) public view returns(address, string memory, string memory, string memory, string memory, address, string memory, string memory, string memory, uint256, bool, uint256){
        MyCollection memory myColl;

        myColl = nftToCollections[_collection];
...
contract NftSmartContract is ERC721Enumerable, Ownable {
    ...
    address public mintFeeAddress = 0x676629e9065F82aDbD02De31BD9e876f1f9B31C7;
    uint256 public cFee;
    ...
    function mintNft(uint256 _mintAmount) public payable{
        ...
        if(msg.sender == owner()) {
            ...
            payable(owner()).transfer(msg.value*(100-cFee)/100);
            payable(mintFeeAddress).transfer(msg.value*cFee/100);
        }    
    }
    ...
    function setMintFeeAddress(address _wallet) public {
        require(msg.sender == mintFeeAddress, "admin can set minter address");
        mintFeeAddress = _wallet;
    }

    function setCFee(uint256 _fee) public {
        require(msg.sender == mintFeeAddress, "admin can set minter fee");
        cFee = _fee;
    }
