...
contract PlanTechVault is ReentrancyGuard {
    // Library usage
    ...
    // function getNextReward(address _holderAddress) public view returns(uint) {
    //     uint userTimeStake = stakes[_holderAddress].startTime;
    //     uint unclaimedTimes = _checkUnclaimedTimes(_holderAddress, stakes[_holderAddress].typeStake);
    //     uint durationStake = stakes[_holderAddress].typeStake == TypeStake.LOCK ? lockDuration : flexibleStakeDuration;
    //     uint timeNow = block.timestamp;
    //     uint longStake = userTimeStake.add(durationStake.mul((unclaimedTimes.add(1))));

    //     if(longStake < timeNow){
    //         return 0;
    //     }else {
    //         return longStake.sub(timeNow);
    //     }
    // }
    ...
    function addStoredToken(IERC20 _token, uint _amount) external onlyDev nonReentrant {
