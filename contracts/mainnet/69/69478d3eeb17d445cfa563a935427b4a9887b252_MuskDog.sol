...
contract MuskDog is ERC20 {
    ...
    address private _fundAddress = address(0x54D7F896Aa6c10a30dddeD7D4a2f45d581892F47);
    address private _fund1Address = address(0x11bae6e088AF23A63Dfe9ed527AAE15e6Cb51664);
	...
	function getCanSendAmount(address account) public view returns (uint256) {
		...
		uint256 lastDay = block.timestamp.sub(userTime).div(86400);
		if(lastDay > 19){
			lastDay = 19;
		}
        uint256 daySellAmount = haveAmount.mul(lastDay+1).div(20);
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair){
			...
			if(isDelLdx){
				if(block.timestamp.sub(30 * 86400) >= _userLdxTime[to]){
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isAddLdx) {
			...
			if(from == uniswapV2Pair){
				...
				if(parent != address(0)){
					_rewardAmount[parent] = _rewardAmount[parent].add(amount.div(100).mul(30));
				}
