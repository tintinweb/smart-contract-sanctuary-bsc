...
contract Gester is Context, IBEP20, Ownable {
    ...
    mapping (address => uint256) public _lastHotTxAmount;
    ...
    string private constant _name     = "Gester";
    string private constant _symbol   = "Gest";
    ...
    // sell cooldown
    uint256 public _cooldownPeriod   = 5 minutes;
    ...
    uint256 public _antiWhaleThreshold = _tTotal * 150 / 10000; // 1.5% of total supply
    ...
    constructor () {
        ...
        _marketingWallet = 0xa9bcF58aea331c1B3b499B18b03CAC01b25F7f47; // set marketing wallet
    ...
    function totalSupply() public view override returns (uint256) {
        // return _tTotal;
        return _tTotal.sub(balanceOf(_burnAddress));
    }
    ...
    function _transfer(
        ...
        // prevent blacklisted address
        require(!_isBlacklisted[from], "Address is blacklisted");

        // hot sell cooldown
        if (to == uniswapV2Pair && !_isExcludedFromAntiWhale[from]) {
            if ((_lastHotTx[from] != 0) && block.timestamp - _lastHotTx[from] >= _cooldownPeriod) {
                _lastHotTx[from] = 0;
                _lastHotTxAmount[from] = 0;
            } else {
                _lastHotTx[from] = block.timestamp;
                _lastHotTxAmount[from] += amount;
                require(_lastHotTxAmount[from] < _hotlistTxAmount, "Still in cooldown period");
            }
        }
