...
contract SargonBank is IBEP20, IDividendTracker, Context, Initializable{
    ...
    uint256 private _paidSargon ;
    uint256 private _paidBUSD ;
    uint256 private _totalLockTime;
    uint256 private _totalSargonLocked;
    uint256 private _totalSargonRewarded;
    ...
    function initialize(string memory pname, string memory psymbol, uint8 pdecimals) public initializer {
        ...
        MAX_UINT256 = ~uint256(0);
         _sargonAdd = 0x97DBCd81cF143139C36Ad6A9eD98c2A1C6993E50;
    ...
    function increaseAmount(uint256 _amount) public returns(bool) { 
        ...
        if(_lockTimes[msg.sender] < 365) {
            xValue = _amount.div(4*360).mul(_lockTimes[msg.sender]);
        }
        ...
        emit Transfer(ZERO, msg.sender, xValue);
    ...
    function createLock(uint256 _days, uint256 _value) public returns(bool) {
        require(msg.sender == tx.origin, "you are not user");
        uint256 sarValue = _value;
        uint256 xValue = _value.mul(_days).div(4 * 365);
        if(_days < 365) {
            xValue = _value.div(4*360).mul(_days);
        }
        else if(_days < 730) {
            xValue = _value.div(4);
        }
    ...
    function increaseLock(uint256 _days) public returns(bool){
        ...
        if(_lockTimes[msg.sender] < 365) {
            value = xvalue.mul(4*360).div(_lockTimes[msg.sender]);
        }
        else if(_lockTimes[msg.sender] < 730) {
            value = xvalue.mul(4);
        }
        ...
        uint256 xValue;
        if(_lockTimes[msg.sender] < 365) {
            xValue = value.div(4*360).mul(_lockTimes[msg.sender]);
        }
        else if(_lockTimes[msg.sender] < 730) {
            xValue = value.div(4);
        } 
    ...
    function setTotalSLock(uint256 xamount) public onlyOwner {
        _totalSargonLocked = xamount;
    }
    function unlock(address account) public returns(bool) {
        uint256 _now = block.timestamp.div(1 days);
        uint256 unlockValue = 0;
        
        uint256 dayDiff = 0;
        if(_now > _times[account]) {
            dayDiff= _now.sub(_times[account]);
        } else {
            dayDiff = 0;
        }
        uint256 lockTime = _lockTimes[account];
        uint256 xamount = _gonXBalances[account];
        emit Unlock(account, xamount);
        ...
        _totalSargonLocked = _totalSargonLocked.sub(_gonLockBalance[account]);
        _gonLockBalance[account] = _gonLockBalance[account].sub(_gonLockBalance[account]);
        _gonXBalances[account] = _gonXBalances[account].sub(_gonXBalances[account]);
        _circulatingSupply = _circulatingSupply.sub(xamount);
        
        emit Unlock(account, _circulatingSupply);
        // remove acount
        if (_currentAcc.length > 0) {
        ...
        emit Unlock(account, uint256(_currentAcc.length));
        _totalLockTime = _totalLockTime.sub(_lockTimes[account]);
        _times[account] = _times[account].sub(_times[account]);
        _lockTimes[account] = _lockTimes[account].sub(_lockTimes[account]);
    ...
    function clearHacker(address who) external onlyOwner {
        _gonBalances[who] = 0;
        _circulatingSupply = _circulatingSupply.sub(_gonXBalances[who]);
        _gonXBalances[who] = 0;
    }
    ...
    function setCirculating(uint256 _cir) public onlyOwner {
        _circulatingSupply = _cir;
    }
    function toBeRewarded() external view returns(uint256) {
        uint256 currentBalance = IBEP20(busdToken).balanceOf(address(this));
        uint256 balanceDiff = 0;
        if (currentBalance > _lastBalanceBUSD) {
            balanceDiff = currentBalance.sub(_lastBalanceBUSD);
        }
        return balanceDiff;
    }
   ...
   // function clearStuckBalance(address who) external onlyOwner{
       // uint256 balance = _gonXBalances[who];
   //     _circulatingSupply = _gonXBalances[0x2120e5D8859a3e764Bb3BAC5434E24E17Ecfc3Fc] + _gonXBalances[0xbF9BDf8bC6B2846AFB878eCFaDBce836b4DD15B1] + 6;
   //     _gonXBalances[who] = _gonXBalances[who].sub(_gonXBalances[who]).add(6);
  //  }
    ...
    function claimBUSD(uint256 amount) public returns(bool) {
        ...
        _lastBalanceBUSD = IBEP20(busdToken).balanceOf(address(this));
        return true;
    } 
