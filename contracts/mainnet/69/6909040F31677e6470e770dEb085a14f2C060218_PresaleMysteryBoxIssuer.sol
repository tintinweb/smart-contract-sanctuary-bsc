...
import "./lib/TimeMockable.sol";
import "./DragonaireNFTManager.sol";
import "./lib/AccessControlEnumerable.sol";
import "./lib/Token/IERC20.sol";

contract PresaleMysteryBoxIssuer is TimeMockable, AccessControlEnumerable {
    ...
    uint256 public constant PURCHASEMENT_LIMIT = 3;
    uint256 public constant MAX_BILL_EXEMPTION = 10;

    IERC20 public immutable paymentToken;
    uint256 public immutable launchTime;
    uint256 public immutable discountEndTime;
    uint256 public immutable limitationEndTime;
    uint256 public immutable saleEndTime;

    DragonaireNFTManager public nftManager;
    address public fundsRecipient;

    uint256 public currentSupply;
    mapping(address => uint256) public purchasedAmountOf;
    uint256 public billExemptedCount;

    event Purchased(address indexed account, uint256 price, uint256 amount, bool billExempted, uint256 totalPrice);

    constructor(
        IERC20 _paymentToken,
        DragonaireNFTManager _nftManager,
        address _fundsRecipient,
        uint256 _launchTime,
        bool _currentTimeMocked
    ) TimeMockable(_currentTimeMocked) {
        paymentToken = _paymentToken;
        nftManager = _nftManager;
        fundsRecipient = _fundsRecipient;
        launchTime = _launchTime;
        discountEndTime = _launchTime + 30 minutes;
        limitationEndTime = _launchTime + 24 hours;
        saleEndTime = _launchTime + 48 hours;
    ...
    function setNftManager(DragonaireNFTManager value) external onlyRole(MANAGER_ROLE) {
        nftManager = value;
    }

    function setFundsRecipient(address value) external onlyRole(MANAGER_ROLE) {
        fundsRecipient = value;
    }

    function grantRoleInBatch(bytes32 role, address[] calldata accounts) public {
        for (uint256 i = 0; i < accounts.length; i++) {
            grantRole(role, accounts[i]);
        }
    }

    function _decideBillExemption() internal returns (bool) {
        if (billExemptedCount >= MAX_BILL_EXEMPTION) {
            return false;
        }
        // possibility of 1%
    ...
    function purchase(uint256 amount) external {
        require(_currentTime() >= launchTime, "PresaleMysteryBoxIssuer: sale not start yet");
        require(_currentTime() < saleEndTime, "PresaleMysteryBoxIssuer: sale end");
        require(currentSupply + amount <= MAX_SUPPLY, "PresaleMysteryBoxIssuer: supply is reaching maximum");
        require(amount > 0, "PresaleMysteryBoxIssuer: amount must be greater than zero");
        if (_currentTime() < limitationEndTime) {
            require(purchasedAmountOf[msg.sender] + amount <= PURCHASEMENT_LIMIT, "PresaleMysteryBoxIssuer: purchasement exceeds the limit");
        }

        uint256 actualPrice = PRICE;

        if (_currentTime() < discountEndTime) {
            if (hasRole(OG_ROLE, msg.sender)) {
                actualPrice = PRICE * 90 / 100;
            } else if (hasRole(WHITELIST_ROLE, msg.sender)) {
                actualPrice = PRICE * 95 / 100;
            }
        }
        uint256 totalPrice = actualPrice * amount;

        bool billExempted = _decideBillExemption();
        if (billExempted) {
            totalPrice -= actualPrice;
        }

        currentSupply += amount;
        purchasedAmountOf[msg.sender] += amount;
        paymentToken.transferFrom(msg.sender, fundsRecipient, totalPrice);
        nftManager.mintBatch(msg.sender, 3, amount);

        emit Purchased(msg.sender, actualPrice, amount, billExempted, totalPrice);
    }
}
...
abstract contract TimeMockable {
    bool public immutable currentTimeMocked;
    uint256 public mockingCurrentTime;

    constructor(bool _currentTimeMocked) {
        currentTimeMocked = _currentTimeMocked;
    }

    function _currentTime() internal view returns (uint256) {
        if (currentTimeMocked) {
            return mockingCurrentTime;
        } else {
            return block.timestamp;
        }
    }

    // Intended no restriction. Used in debug and tests only
    function setMockingCurrentTime(uint256 value) external {
        mockingCurrentTime = value;
    }
}
...
interface IRandomGenerator {
    ...
    enum RandomnessGenerationStage {
        UNKNOWN,
        REQUESTING,
        FULFILLED
    }
    ...
    function requestRandomNumbers(uint256[] memory tokenIds) external;

    function getStageOf(uint256 tokenId) external view returns (RandomnessGenerationStage);
...
import "./DragonaireNFTCore.sol";

contract DragonaireNFTManager is
    AccessControlEnumerable,
    RandomConsumerBase,
    ItemFactoryManager
{
    ...
    DragonaireNFTCore public immutable nftCore;
    ...
    bool private _useChainLinkRandomGen = true;
    ...
    modifier onlyTokenOwner(uint256 tokenId_) {
        require(
            ...
            "DragonaireNFT: caller is not owner"
        );
        _;
    }
    ...
    modifier onlyMysteryBox(uint256 tokenId_) {
        ...
        require(_dna == 0, "DragonaireNFT: token is already unboxed");
        _;
    }
    ...
    modifier onlyExistedToken(uint256 tokenId_) {
        require(
            ...
            "DragonaireNFT: token does not exists"
        );
        _;
    }

    function getUseChainLinkRandomGen() public view returns (bool) {
        return _useChainLinkRandomGen;
    }

    function setUseChainLinkRandomGen(bool value)
        public
        onlyRole(MANAGER_ROLE)
    {
        _useChainLinkRandomGen = value;
    }
        ...
        onlyRole(CONTRACT_UPGRADER)
    {
        require(
            ...
            "DragonaireNFT: itemFactory_ is the zero address"
        );
        _updateItemFactory(itemFactory_);
    }
        ...
        onlyRole(CONTRACT_UPGRADER)
    {
        require(
            ...
            "DragonaireNFT: randomGenerator_ is the zero address"
        );
        _updateRandomGenerator(randomGenerator_);
    }
    ...
    {
        _mint(to_, boxType_, false);
    }

    function safeMint(address to_, uint256 boxType_)
        public
        onlyRole(MINTER_ROLE)
        onlySupportedBoxType(boxType_)
    {
        _mint(to_, boxType_, true);
    }
    ...
    function mint(
        ...
        uint256 artifacts_,
        bytes32 dna_
    ) public onlyRole(GAME_MASTER_ROLE) returns (uint256 tokenId) {
        tokenId = nftCore.mint(to_);
        nftCore.updateTokenMetaData(tokenId, artifacts_, dna_);
    }

    function burn(uint256 tokenId) public onlyRole(GAME_MASTER_ROLE) {
        nftCore.burn(tokenId);
    }
    ...
    function _mint(
        ...
        uint256 _tokenId;
        if (safe) {
            _tokenId = nftCore.safeMint(to_);
        } else {
            _tokenId = nftCore.mint(to_);
        }
    ...
    function _doUnbox(uint256 tokenId_)
        internal
    ...
    {
        require(
            unboxStageOf(tokenId_) == UnboxStage.INITIAL,
            "DragonaireNFTManager: token unboxing or unboxed"
        );
    ...
    function unbox(uint256 tokenId_) public payable {
        ...
        if (_boxType > 100) {
            ...
            for (uint256 i = 1; i < boxTypes.length; i++) {
                _mint(_msgSender(), boxTypes[i], false);
            }
        } else {
            _doUnbox(tokenId_);

            if (_useChainLinkRandomGen) {
                _takeRandomFee();
                randomGenerator.requestRandomNumber(tokenId_);
            }

            if (!_useChainLinkRandomGen) {
                fulfillRandomness(tokenId_, block.timestamp);
            }
        }
    }

    function unboxStageOf(uint256 tokenId) public view returns (UnboxStage) {
        return UnboxStage(uint256(randomGenerator.getStageOf(tokenId)));
    }

    function isUnboxing(uint256 tokenId) public view returns (bool) {
        return unboxStageOf(tokenId) == UnboxStage.UNBOXING;
    }

    function unboxBatch(uint256[] calldata tokenIds) external {
        require(
            tokenIds.length > 0 && tokenIds.length <= 20,
            "DragonaireNFTManager: unboxing amount of tokens should be in 1-20"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _doUnbox(tokenIds[i]);
        }

        randomGenerator.requestRandomNumbers(tokenIds);
    }
     ...
     * 24~39: DragonaireId, len=16
     * 40~47: ItemType, len=8
     * 48~55: star, len=8
     * ...
     *
     * dna format:
    ...
    function addBoxType(uint256 boxType_) external onlyRole(MANAGER_ROLE) {
        ...
        require(success, "DragonaireNFT: box type is already supported");
    }
    ...
    function updateCapability(uint256 tokenId, uint256 capability)
        external
        onlyRole(GAME_MASTER_ROLE)
    {
        nftCore.updateCapability(tokenId, capability);
    }

    function updateCapabilityInBatch(
        DragonaireNFTCore.CapabilityUpdateRequest[] calldata requests
    ) external onlyRole(GAME_MASTER_ROLE) {
        nftCore.updateCapabilityInBatch(requests);
    }
}
    ...
    ERC721Pausable
{
    ...
    modifier notExceedingTotalSupply() {
        require(
    ...
    function _mint(address to_, uint256 tokenId_) internal virtual override notExceedingTotalSupply {
        super._mint(to_, tokenId_);
    }

    function _safeMint(address to_, uint256 tokenId_) internal virtual override notExceedingTotalSupply {
    ...
    function _doMint(address to_) internal returns (uint256) {
    ...
    function _doSafeMint(address to_) internal returns (uint256) {
        uint256 _tokenId = _tokenIdTracker.current();
    ...
    function safeMint(address to_) public onlyRole(MANAGER_ROLE) returns (uint256) {
        return _doSafeMint(to_);
    }

    function safeMintBatch(address to_, uint256 amount_) public onlyRole(MANAGER_ROLE) {
        require(amount_ > 0, "DragonaireNFT: amount_ is 0");
        require(totalSupply() + amount_ <= cap(), "cap exceeded");
