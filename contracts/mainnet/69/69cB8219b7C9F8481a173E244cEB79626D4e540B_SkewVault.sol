...
import {ISkew} from "./intf/ISkew.sol";
...
contract SkewVault is Ownable {
    ...
    uint256 withdrawFee;
    uint256 withdrawFeeBase = 10000;
    address public treasuryAddress;
    uint256 normalMaxQueueLength;
    uint256 fastMaxQueueLength;
    uint256 currentNormalQueueLength;
    uint256 currentFastQueueLength;
    address Skew;
    address vToken;

    constructor(address _token, address _vToken, address _sToken, uint256 _minWithdrawAmount, address _treasury) {
        vaultToken = _token;
        ...
        normalMaxQueueLength = 1000;
        fastMaxQueueLength = 1000;
    }
    ...
    function setWithdrawFee(uint256 _newFee) external onlyOwner {
        require( _newFee <= withdrawFeeBase, "Can't set 100% fee");
        withdrawFee = _newFee;
    }
    ...
    function setNormalQueueLength(uint256 _newNormalQueue) external onlyOwner {
        normalMaxQueueLength = _newNormalQueue;
    }

    function setFastQueueLength(uint256 _newFastQueue) external onlyOwner {
        fastMaxQueueLength = _newFastQueue;
    }

    function setSkewAddress(address _skew) external onlyOwner {
        Skew = _skew;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        totalSToken += mintAmount;
        totalDeposit += _amount;
    }


    function depositReward(uint256 _amount) external {
        require(_msgSender() == Skew, "Caller must be strategy address");
        IERC20(vaultToken).transferFrom(msg.sender, address(this),_amount);
        uint256 fee = _amount * withdrawFee / withdrawFeeBase;
        IERC20(vaultToken).transfer(treasuryAddress, fee);
        totalReward += (_amount-fee);
    }

    function getExchangeRate() public view returns(uint256) {
        if(totalDeposit == 0) return exchangeDecimal;
        return (totalDeposit+totalReward)*exchangeDecimal/totalSToken;
    }
    ...
    function withdrawUnderlying(
        ...
        totalSToken -= sTokenAmount;
        require(_amount >= minWithdrawAmount, "amount should bigger than minWithdraw");
        // add to queue
        if( _bFast ) {
            require(currentFastQueueLength < fastMaxQueueLength, "Fast Queues are full");
            ...
            currentFastQueueLength++;
        }
        else {
            require(currentNormalQueueLength < normalMaxQueueLength, "Normal Queues are full");
    ...
    function botDepositToVenus(uint256 _amount) external onlyOwner {
        require(Skew.isContract(),"Skew Address is in correct");
        IERC20(vaultToken).approve( Skew ,_amount);
        ISkew(Skew).depositToVenus(vToken, _amount);
    }
    ...
    function fastWithdraw() external onlyOwner {
        ...
        for(uint256 i = 0; i < length; i++) {
            ...
            uint256 fee = amount * withdrawFee / withdrawFeeBase;
            uint256 actualAmount = amount - fee; 
            totalFee += fee;
            IERC20(vaultToken).transfer( user, actualAmount);
            delete fastWithdrawalQueue[user];
        }
        // Transfer fee to treasury wallet
        IERC20(vaultToken).transfer( treasuryAddress, totalFee);
