...
contract BenfaNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, Whitelist {
...
contract BenfaFactory is UUPSUpgradeable, ERC721HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function initialize(address _paymenToken, address _feeAddress) public initializer {
		...
		createCollection("BenfaNFT", "BENFANFT");
    }
    ...
    function createCollection(string memory _name, string memory _symbol) public returns(address collection) {
		bytes memory creationCode = type(BenfaNFT).creationCode;
        ...
        IBenfaNFT(collection).initialize(msg.sender);
    ...
    function list(address _collection, address owner, uint256 _token_id, address _currency,  uint256 _price) public {
        ...
        IBenfaNFT(_collection).safeTransferFrom(owner, address(this), _token_id, "List");

        address creator = IBenfaNFT(_collection).creatorOf(_token_id);
        uint256 royalties = IBenfaNFT(_collection).royalties(_token_id);
    ...
    function delist(address _collection, uint256 _token_id) public returns (bool) {
        ...
        IBenfaNFT(_collection).safeTransferFrom(address(this), msg.sender, _token_id, "DeList");
    ...
    function buy(address _collection, uint256 _token_id) external payable nonReentrant {
        ...
        IBenfaNFT(item.collection).safeTransferFrom(address(this), _newOwner, item.token_id, "Purchase Item");
