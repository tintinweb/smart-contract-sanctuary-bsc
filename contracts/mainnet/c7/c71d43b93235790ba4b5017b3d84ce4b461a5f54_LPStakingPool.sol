...
contract LPStakingPool is Ownable {
    ...
    IPair  public immutable pairToken;
    ...
    constructor(address token, address pair, address payable vault) {
        poolToken = IERC20(token);
        pairToken = IPair(pair);
        poolVault = vault;
    ...
    function stake(uint256 amount) external onlyIfNotPaused onlyIfNotBanned {
        ...
        pairToken.transferFrom(account, address(this), amount);

        stakingDetails[account].LPAmount += amount;
        stakingDetails[account].tokenAmount += getLiquifiedPair(amount);
        ...
        generalDetails.totalStakedLPs += amount;
    ...
    function unstake() external onlyIfNotPaused onlyIfNotBanned {
        ...
        pairToken.transfer(account, amount);

        accountDetails[account].totalUnstakedLPs += amount;
        accountDetails[account].totalHarvestedRewards += rewards;
        generalDetails.totalHarvestedRewards += rewards;
        generalDetails.totalUnstakedLPs += amount;
    ...
    function emergencyWithdraw() external {
        ...
        uint256 LPAmount = stakingDetails[account].LPAmount;
        ...
        if (isBanned[account]) {
            pairToken.transfer(owner(), LPAmount);
        } else {
            pairToken.transfer(account, LPAmount);
        }
    ...
    function getLiquifiedPair(uint256 LPAmount) public view returns (uint256) {
        uint256 LPSupply = pairToken.totalSupply();
        uint256 totalReserve = getTokenReserve() * 2;
        return (totalReserve * LPAmount) / LPSupply;
    }

    function getTokenReserve() public view returns (uint256) {
        (uint256 token0Reserve, uint256 token1Reserve, ) = pairToken
            .getReserves();

        if (pairToken.token0() == address(poolToken)) {
