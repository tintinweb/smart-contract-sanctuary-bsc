...
// File: contracts/Compound/interfaceIBEP20.sol
...
contract AutoCompound is Ownable {
    ...
    */
    constructor(IBEP20 _token) {
        tokenReward = _token; // Define Endereço BEP20
    }
    /*
    * -------------------------------------------
    * |               Unitarios                 |
    ...
    */
    ...
    uint256 private feeTax = 2; // Taxa em ARC
    ...
    */
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            rewards[account] = compound(account);
    ...
    */
    ...
    function totalPoolRewards() public view returns(uint256) {
        return IBEP20(tokenReward).balanceOf(address(this));
    }
    function harvestUser(address account) public view returns(uint256) {
        uint256 currentTimes = block.timestamp;
        uint256 userHarv = harvestTime[account];
    ...
    function addManualCompound(address account, uint256 tAmount) external  onlyOwner {
        totalSupply += tAmount;
        totalLiquidity += tAmount;
        balanceUser[account] += tAmount;
        lastUpdateTime = lastTimeRewardApplicable();
    ...
    function initCompound(uint256 tAmount) external payable updateReward(_msgSender()) {
        ...
        IBEP20(tokenReward).transferFrom(_msgSender(), address(this),tAmount);
        totalSupply += tAmount;
    ...
    function exit() external payable updateReward(_msgSender()) {
        ...
        uint256 balance = balanceUser[_msgSender()];
        require(balance > 0, "Voce nao tem saldo");
        balanceUser[_msgSender()] -= balance;
        IBEP20(tokenReward).transfer(_msgSender(), balance);
    }
    ...
    function takeMyRewards() external payable updateReward(_msgSender())  {
        ...
        require(harvestUser(_msgSender()) == 0, "Tempo de Colheita nao liberado");
        blockHarvest();
        ...
        if (reward > 0) {
            ...
            IBEP20(tokenReward).transfer(_msgSender(), reward);
        }
        else {
    ...
    function changeToken(address _tokenReward) external onlyOwner {
