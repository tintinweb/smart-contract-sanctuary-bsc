...
contract MetaZombie is IERC20 {
    ...
    string constant private _name = "MetaZombie";
    ...
    struct TaxWallets {
        ...
        address payable alpha;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xfd832866e2e1D7B654168a4dB0F44C48e4651789),
        development: payable(0x3A41f3Ef20b4a692225414153699d0d40F4A254b),
        alpha: payable(0xdD411F9B37DC2Ace4408fEcbB581c2c38f9713f3)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 development, uint16 alpha) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.development = development;
        _ratios.alpha = alpha;
        _ratios.totalSwap = liquidity + marketing + development + alpha;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable development, address payable alpha) external onlyOwner {
        require(marketing != address(0) && development != address(0) && alpha != address(0), "Cannot be zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.development = payable(development);
        _taxWallets.alpha = payable(alpha);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        uint256 alphaBalance = (amtBalance * ratios.alpha) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (developmentBalance + alphaBalance);
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 55000}("");
        }
        if (ratios.alpha > 0) {
            (success,) = _taxWallets.alpha.call{value: alphaBalance, gas: 55000}("");
        }
