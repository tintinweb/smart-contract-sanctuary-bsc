...
contract AAAS is IFlashBorrower, IXFlashBorrower{
    ...
    /** MDB+ */
    address constant DUMP = 0x6b8a384DDe6FC779342Fbb2E4a8EcF73eD18D151;
    ...
    function handleSwaps(
        ...
        if (_token0 == XUSD && _token1 == DUMP) {
            // Mint MDB+
            _mintDUMP(amount);
        } else if(_token0 == DUMP && approvedStables[_token1]) {
            // Sell MDB+
            _sellDUMP(amount);
    ...
    function _sellDUMP(uint256 amount) internal {
        IMDBP(DUMP).sell(amount);
    }

    function _mintDUMP(uint256 amount) internal {
        IERC20(XUSD).approve(DUMP, amount);
        IMDBP(DUMP).mintWithBacking(amount, address(this));
    }
