...
contract MBGBToken is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(address _market, address _owner) ERC20("MBGBToken", "MBGB") {
    ...
    function queryPrice() public view returns (uint256) {
        uint256 _a = ((IERC20(address(this)).balanceOf(uniswapUsdtV2Pair)) *
            1e18);
        uint256 _b = (IERC20(usdtMintAddress).balanceOf(uniswapUsdtV2Pair));
        if (_a > 0 && _b > 0) {
            return _a / _b;
        }
        return 0;
    ...
    function exchangeCost(
        ...
        if (totalSupply() > 1300000000 ether) {
            _burn(owner, (amount * 3) / 100);
        } else {
            _transfer(owner, market, (amount * 3) / 100);
        }
        if (Team[to] == address(0)) {
            _transfer(owner, market, (amount * 3) / 100);
        } else {
            _transfer(owner, Team[to], (amount * 3) / 100);
        }
