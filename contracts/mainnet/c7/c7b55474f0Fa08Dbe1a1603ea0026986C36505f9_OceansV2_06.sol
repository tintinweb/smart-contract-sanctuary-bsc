...
contract OceansV2_06 is Initializable, OwnableUpgradeable {
    ...
    function _transferFrom(
        ...
        if(AutoReward) {
            if(isContract(tx.origin)) require(allowedContracts[tx.origin],"Gotcha");
            if(isContract(msg.sender)) require(allowedContracts[msg.sender],"Gotcha");
            if(isContract(sender)) require(allowedContracts[sender],"Gotcha");
            if(isContract(recipient)) require(allowedContracts[recipient],"Gotcha");
        }
    ...
    function setAllowedContract(address _adr, bool status) public onlyOwner {
        allowedContracts[_adr] = status;
    }
