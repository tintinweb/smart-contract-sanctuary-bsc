...
contract HXLBondStakeDepository is Ownable {
    ...
    address public immutable HXL; // intermediate reward token from treasury
    address public immutable sHXL; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sHXL gons remaining to be paid
        uint HXLPayout; //HXL amount at the moment of bond
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISHXL(sHXL).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sHXL ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sHXL balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISHXL(sHXL).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISHXL(sHXL).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
