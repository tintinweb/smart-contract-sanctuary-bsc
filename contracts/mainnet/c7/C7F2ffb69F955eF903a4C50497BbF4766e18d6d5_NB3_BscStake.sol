...
contract NB3_BscStake is Owned {
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(NB3).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough  to pay registration fee.");
        require(IERC20(NB3).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        lastClock[msg.sender] = now;
         lockTime[msg.sender] = block.timestamp + 1600 minutes;
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(NB3).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(NB3).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient  balance in pool');
        stakeRewards[msg.sender] = 0;
        referralRewards[msg.sender] = 0;
        referralCount[msg.sender] = 0;
        uint remainder = (now.sub(lastClock[msg.sender])).mod(86400);
        lastClock[msg.sender] = now.sub(remainder);
        IERC20(NB3).transfer(msg.sender, totalReward);
        emit OnWithdrawal(msg.sender, totalReward);
        return true;
    }

    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(NB3).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        require((IERC20(NB3).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient  balance in pool');
        IERC20(NB3).transfer(msg.sender, _amount);
