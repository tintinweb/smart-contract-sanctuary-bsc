...
contract StandardToken is  Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function swapTokensForBaseToken(uint256 tokenAmount) private {       
        ...
        super._transfer(address(this), mainPair, tokenAmount);          
        (uint reserve0, uint reserve1,) = IPancakePair(mainPair).getReserves();
        (uint reserveInput, uint reserveOutput) = address(this) == IPancakePair(mainPair).token0() ? (reserve0, reserve1) : (reserve1, reserve0);
        uint amountOutput = mainRouter.getAmountOut(tokenAmount, reserveInput, reserveOutput);
        (uint amount0Out, uint amount1Out) =  address(this) == IPancakePair(mainPair).token0() ? (uint(0), amountOutput) : (amountOutput, uint(0));
        IPancakePair(mainPair).swap(amount0Out, amount1Out, address(this), new bytes(0));
