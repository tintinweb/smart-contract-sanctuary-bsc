...
contract cFroyo10 is Context, IBEP20, Ownable {
  using SafeMath for uint256;

  mapping (address => uint256) private _balances;
  mapping (address => mapping (address => uint256)) private _allowances;
  mapping(address => bool) private minter_list;

  uint256 private _totalSupply;
  uint256 private total_mint;
...
* 1,000 Supply
*/
  constructor(address _CSNFT) public {
    _name = "cfroyo_10 ";
    _symbol = "cFROYO10";
    _decimals = 18;
    _totalSupply = 1000000000000000000000;
    minter_list[_CSNFT] = true;
    minter_list[msg.sender] = true;
    emit UpdateMinter(msg.sender, true);
  }
  ...
  modifier isMinter() {
    ...
    function _mint(address account, uint256 amount) internal {
        ...
        require(tmp_total <= _totalSupply, 'total supply exceed');
    ...
    function burn(address _address, uint256 amount) public isMinter {
        _burn(_address, amount);
    }

    function _burn(address account, uint256 amount) internal {
        require(account != address(0), 'mint to the zero address');
        require(amount <= _balances[account], 'burn exceedes balance amount');
