...
contract MulticallTool is Multicall {
    function functionCall(address target, bytes calldata data) external returns (bytes memory) {
        return Address.functionCall(target,data);
    }

    function functionStaticCall(address target, bytes calldata data) external view returns (bytes memory) {
