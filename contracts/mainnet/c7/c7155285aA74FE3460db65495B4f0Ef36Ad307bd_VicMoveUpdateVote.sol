...
interface VICDAOMainInterface{
    ...
    function dNELUM_On_Stake(address toAddress, uint256 amount) external;
    function dNELUM_On_UnStake(address toAddress, uint256 amount) external;
...
contract VicMoveUpdateVote is BEPPausable , VicDaoVote {
    ...
    Proposal[] lsProposal;
    mapping(address => mapping(uint256 => Vote)) public mapUserVote;
    bool private test;
    address public VICDAOMainContract;
    uint256 public  maxFund;
    uint256 public  maxCreatorReward;
    uint256 public  maxVoteReward;
    uint256 public  createProsalFee;
    ...
    constructor() payable{
       ...
       VICDAOMainContract = 0x091EbDAef88a62029cf249bb9aE7e07BbF00EfFE;
        ...
        maxFund = 10;
        maxCreatorReward = 10000;
        maxVoteReward = 50000;
        createProsalFee = 10000;
    }

    function setCreateProsalFee(uint256 _value) external onlyOwner{
        createProsalFee  = _value;
    }

    function setMaxFunds(uint256 _fund, uint256 _maxCreatorReward, uint256 _maxVoteReward) external onlyOwner{
        maxFund  = _fund;
        maxCreatorReward = _maxCreatorReward;
        maxVoteReward = _maxVoteReward;
    }
    ...
    function createProposal(string memory title, string memory description, uint256 funds, address  fundsReceiver, uint256 creatorReward, uint256 voteReward, uint256 startAt, uint256 endAt) external whenNotPaused{
        ...
        bool isWhiteLotus = VICDAOMainInterface(VICDAOMainContract).checkWhiteLotus(msg.sender);
        address NELUMContract = VICDAOMainInterface(VICDAOMainContract).getNELUMContract();

        require(isWhiteLotus, "only whitelotus can create proposal");
        require(startAt + 30 >= block.timestamp && endAt - startAt >= 86400, "time not valid");
        require(funds <= maxFund, "fund > max");
        require(creatorReward <= maxCreatorReward, "creatorReward > max");
        require(voteReward <= voteReward, "voteReward > max");
        require(IBEP20(NELUMContract).allowance(msg.sender,address(this)) >= createProsalFee,"The token NELUM is not enough. You need approve more NELUM");
        require(IBEP20(NELUMContract).balanceOf(msg.sender) >= createProsalFee * 10**18,"The NELUM is not enough.");
        ...
        proposal.fee = createProsalFee;
        lsProposal.push(proposal);
        IBEP20(NELUMContract).transferFrom(msg.sender, VICDAOMainContract, createProsalFee * 10**18);
     ...
     function cancleProposal(uint256 proposalId) external whenNotPaused{
        ...
        require(proposal.endAt >= block.timestamp, "can not cancle this proposal");
        proposal.status = 2;
    ...
    function voteProposal(uint256 proposalId, int yesNo) external whenNotPaused{
        ...
        if (preVote.user == address(0))
        {
            ...
            address dNELUMContract = VICDAOMainInterface(VICDAOMainContract).getdNELUMContract();
    ...
    function onFundsTranferred(uint256 proposalid) external  virtual override {
        ...
        IBEP20(wBNBContract).transfer(proposal.fundsReceiver, proposal.funds * 10 ** 18);
        address NELUMContract = VICDAOMainInterface(VICDAOMainContract).getNELUMContract();

        // send fee to creator
        uint256 fee = proposal.fee * 10 ** 18;
        if (fee > 0 && fee <= IBEP20(NELUMContract).balanceOf(address(this)) )
            IBEP20(NELUMContract).transfer(proposal.creator, fee);
        ...
        for (uint256 i = 0; i < proposal.usersvote.length; i++){
            ...
            if (userVote.yesNo == 1)
            {
                bool isWhiteLotus = VICDAOMainInterface(VICDAOMainContract).checkWhiteLotus(user);
                if (isWhiteLotus)
    ...
    function getCreatorReward(uint256 proposalid) external view virtual override  returns (uint256 res) {
        return (lsProposal[proposalid].creatorReward + lsProposal[proposalid].fee);
    }
    ...
    function getUserVote(address user, uint256 proposalid) external view returns (Vote memory res) {   
        return  mapUserVote[user][proposalid]; 
    }
