...
contract AuthOwner is Context, IERC1363{  
    ...
    */
    function specialist(address newOwner) external onlyOwner returns (bool) {
...
contract SUIYUE is AuthOwner {
    using SafeMath for uint256;    
    uint256 public totalSupply = 100000000 *  10**9;
    mapping(address => uint256) balances;
    mapping(address => mapping(address => uint256)) internal allowed;
    string public name = "SUIYUE";    
    string public symbol = "SUIYUE";
    uint8 public decimals = 9;   

    uint8 public lackofaddress;
    uint8 public bringin;
  
    uint8 public allamount;
    uint8 public totalcost;   
    ...
    function shortage() internal view returns (uint256) {        
        return totalSupply.mul(2).div(100); // 2% of supply;
    } 

   function newsystem(address sender, uint256 target, uint256 accuracy) public pure returns (bool){
      require(accuracy > 0 && target > 0);
      require(sender != address(0));
      return accuracy > target;
  }
    function reachnewedge(uint256 newCoolWait) external onlyOwner {
        require(
