...
contract Polo is Context, IERC20, Ownable{
    ...
    function AutoFromReward(address account) external onlyOwner() {
        require(!_isExcluded[account], "Account is already excluded");        
        _isExcluded[account] = true;
    }   

    function setPoloEnabled(bool _enabled) external onlyOwner{
        swapEnabled = _enabled;
    }

    function setLiquidityFees(uint256 rLiquidity) external onlyOwner {       
	...
	function _tokenPoLoNoFee(address sender, address recipient, uint256 amount) private { 
		  if (amount > 0) {
