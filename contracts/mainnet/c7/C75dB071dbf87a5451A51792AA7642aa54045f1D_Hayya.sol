...
contract Hayya is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x6704F4f32aC6bf157d562139CDD0120de6229870;
    address private devWallet = 0x012e2736Fa4F659FB4BF683042d5819e468Da6Ca;
    address public buyBurn = 0xD9DB3A73a3aC3535AE4Ac9c2E9E0ADB54e032Fd5;
    ...
    Taxes public sellTaxes = Taxes(1, 1, 0, 0, 1);
    ...
    constructor () {
        ...
        _isExcludedFromFee[buyBurn] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(balanceOf(address(this)), sellTaxes);
            else  swapAndLiquify(balanceOf(address(this)), taxes);
        }
        bool takeFee = true;
        bool isSell = false;
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        uint256 tokensToBuyback = contractBalance * temp.buyback / denominator*2;
        uint256 tokensToDev= contractBalance * temp.dev / denominator*2;
        uint256 toSwap = contractBalance - tokensToAddLiquidityWith-tokensToBuyback-tokensToDev;
        ...
        uint256 marketingAmt = deltaBalance-bnbToAddLiquidityWith;
        if(marketingAmt > 0){
            payable(marketingWallet).sendValue(marketingAmt);
        }
        if(tokensToDev> 0){
            _tokenTransfer(address(this), devWallet, tokensToDev, false, false);
        }
        
        if(tokensToBuyback > 0){
            _tokenTransfer(address(this), buyBurn, tokensToBuyback, false, false);
        }
