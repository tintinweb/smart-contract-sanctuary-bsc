...
contract MccToken is ERC20, Ownable {
    ...
    uint256 public  lpRate = 20; //lp fee 

    uint256 public  daoRate = 10; // dao address 

    uint256 public  defiRate = 80; //defi address

    uint256 public extraDestroyFee = 120;

    uint256 public dropRate = 3;
    ...
    address public receiveAddress1 = 0xe68a7B6a4A36cD20B9d62767c23Cd60A7e0d338b; //30%

    address public receiveAddress2 = 0x73F9A15F76f58C7aC29818A99E32CFcE3f4C5346; //55%

    address public daoWalletAddress = address(0);
    ...
    constructor(
        ...
        lpAddress = 0x424082CE7B617371CdbbBC7b53FC1057B3E29C85;

        daoAddress = 0xb0f455da0Bf9786b89a9DEcd3403959e13Acac7F; //1%
        defiContractAddress = 0xdDD58B525DF7e42B5A452C36f78Eca796b6B7892; //8% 

        excludeFromFees(owner(), true);
        excludeFromFees(address(this), true);
        // excludeFromFees(angelAddress, true);
        excludeFromFees(lpAddress, true); // 2%
        ...
        excludeFromFees(daoWalletAddress, true); // 10% of total
    ...
    function setlplpaddress(address addr) public onlyOwner {
        lpAddress = addr;
    }
    ...
    function setFeeRate(uint256 _tradeDestroy,
        uint256 _lpRate, uint256 _daoRate, uint256 _defiRate) public onlyOwner {
        tradeDestroy = _tradeDestroy;
        lpRate = _lpRate;
        daoRate = _daoRate;
        defiRate = _defiRate;
        tradeFee = tradeDestroy.add(lpRate).add(daoRate).add(defiRate);
    }
    ...
    function takeAllFee(address from, address to, uint256 amount) internal returns (uint256 amountAfter) {
        ...
        } else if(to == deadAddress){//dead
            
            destroyAmount = amount.mul(tradeFee).div(maxRate);
            tradeFeeAmount = destroyAmount;
            _totalSupply = _totalSupply.sub(amount - destroyAmount);
