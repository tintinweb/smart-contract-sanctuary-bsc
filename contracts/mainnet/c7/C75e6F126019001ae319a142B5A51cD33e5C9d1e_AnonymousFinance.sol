...
// File: contracts/ArcaneOfc/interfaceIUniswapV2Router02.sol
...
// File: contracts/ArcaneOfc/interfaceIUniswapV2Pair.sol
...
// File: contracts/ArcaneOfc/interfaceIUniswapV2Factory.sol
...
// File: contracts/ArcaneOfc/Context.sol
...
// File: contracts/ArcaneOfc/Ownable.sol
...
// File: contracts/ArcaneOfc/Address.sol
...
// File: contracts/ArcaneOfc/SafeMath.sol
...
// File: contracts/ArcaneOfc/interfaceIBEP20.sol
...
// File: contracts/ArcaneOfc/AnonymousFinance.sol
...
contract AnonymousFinance is IBEP20, Ownable  {
    ...
    */
    ...
    address public  uniswapV2Pair; // Par ANF/BNB
    /*
    ---------------------------------
    -         Modifier              -
    ---------------------------------
    ...
    */
    ...
    ---------------------------------
    */
    string private _name = "Anonymous Finance";
    ...
    ---------------------------------
    */
    uint256 public buyTax = 10; // Taxa de Compra
    uint256 public sellTax = 10; // Taxa de Venda
    ...
    uint256 private _maxTotalSupplyToken = 1000000000 * _decimalPlace;
    uint256 public launchBlock;
    uint256 public maxSellLimit = 1000000000 * 10 ** 18; // Controle a Quantidade de Tokens que podem ser Vendidos
    uint256 public maxBuyLimit = 1000000000 * 10 ** 18; // Controle a Quantidade de Tokens que podem ser Vendidos
    uint256 public firstBuy = 1000000000 * 10 ** 18; // Limita Primeira Compra
    uint256 public tokenLiquidityPercent = 10; // 10% Dos Tokens do Contrato vai para Liquidez
    uint256 public maxWalletBalance = 1000000000 * 10 ** 18; // Limite de Wallet
    ...
    */
    constructor (uint256 _fisrMint, address _addressDistribute) {
        ...
        address pairCreated = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); // Gera o Par ANF/BNB
    ...
    */
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns(bool) {
        ...
        require(currentAllowance >= subtractedValue, "ANF: reducao do subsidio abaixo de zero");
    ...
    */
    function _approve(address owner, address spender, uint256 amount) internal {
        require(owner != address(0), "ANF:Owner nao pode ser Address 0");
        require(spender != address(0), "ANF:Owner nao pode ser Address 0");
    ...
    function _spendAllowance(address owner, address spender, uint256 amount) internal {
        ...
        if(currentAllowance != type(uint256).max) {
            require(currentAllowance >= amount, "ANF: subsidio insuficiente");
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        require(from != address(0), "ANF:From nao pode ser Address 0");
        require(amount > 0, "ANF: Montante precisa ser maior do que 0");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = _balance[from];
        require(fromBalance >= amount,"ANF: o valor da transferencia excede o saldo" );
        ...
        if(blackEnabled && !blackUser) {
           require(!_isBlackFrom(from) && !_isBlackTo(to), "ANF:Voce esta na Lista Negra");
        }
        ...
        if(!takeFee) removeAllFee(); // Remove todas as Taxa
            ...
            if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                ...
                if (amount > maxSellLimit && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to)) {
                    revert("ANF: Montante de Venda nao pode ultrapassar limite"); 
                }
            }
            ...
            if(automatedMarketMakerPairs[from]) {
                ...
                if(buyCooldownEnabled && !isTimelockExempt[to] && !coolDownUser) {
                    require(lockToBuyOrSellForTime(getFromLastPurchaseBuy(to), cooldownTimerInterval), "ANF:Por favor, aguarde o cooldown entre as compras");
                    buyCoolDown(to);
                }
                ...
                if(launchPhase && amount > firstBuy && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to)) {
                    revert("ANF: Montante de Compra nao pode ultrapassar limite"); // Define o Valor de compra para o padrão após 1:30 Minutos
                }
                ...
                if(sellCooldownEnabled && !isTimelockExempt[from] && !coolDownUser) {
                    require(lockToBuyOrSellForTime(getFromLastSell(from), cooldownTimerInterval), "ANF:Por favor, aguarde o cooldown entre as vendas");
                    sellCoolDown(from);
                }
            ...
            if(maxWalletBalance > 0 && !_isUnlimitedSender(from) && !_isUnlimitedRecipient(to) && !automatedMarketMakerPairs[to]) {
                ...
                require(recipientBalance.add(amount) <= maxWalletBalance, "ANF:Nao pode Ultrapassar o limite por Wallet");
            }
                ...
                if(!stopSniper) {
                    ...
                    if(isActive) {
                        ...
                        if(launchBlock == block.number) {
                            require(block.number > launchBlock, "ANF:Bad Bot");
                        }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ANF: O par de AutomatedMarketMakerPair ja esta definido para esse valor");
    ...
    */
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "ANF:uniswapV2Pair nao pode ser removido de AutomatedMarketMakerPair");
    ...
    function setDeactiveSniperBot(bool _launchPhase) external onlyOwner {
        require(launchPhase, "ANF:Anti-Bot ja esta desativado");
    ...
    function withdrawANF(uint256 rAmount) external onlyOwner {
