...
contract GAMBLERTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main GAMBLERTOKEN contract."
        );
    }
...
contract GAMBLERTOKEN is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    GAMBLERTOKENDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        uint256 miniDividends_ //mini mum token balance for dividends_
        ...
        swapTokensAtAmount = totalSupply_.mul(8).div(10**3); // 8%

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = GAMBLERTOKENDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "GAMBLERTOKEN: The dividend tracker already has that address"
        );

        GAMBLERTOKENDividendTracker newDividendTracker = GAMBLERTOKENDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "GAMBLERTOKEN: The new dividend tracker must be owned by the GAMBLERTOKEN token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "GAMBLERTOKEN: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "GAMBLERTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "GAMBLERTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "GAMBLERTOKEN: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "GAMBLERTOKEN: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "GAMBLERTOKEN: Cannot update gasForProcessing to same value"
        );
