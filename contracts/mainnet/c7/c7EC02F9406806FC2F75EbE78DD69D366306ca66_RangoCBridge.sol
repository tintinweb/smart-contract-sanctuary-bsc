...
contract RangoCBridge is IRangoCBridge, MessageSenderApp, MessageReceiverApp, BaseContract {
    ...
    event CBridgeIMStatusUpdated(bytes32 id, address token, uint256 outputAmount, OperationStatus status);
    ...
    function computeCBridgeSgnFee(RangoCBridgeModels.RangoCBridgeInterChainMessage memory imMessage) external view returns(uint) {
        bytes memory msg = abi.encode(imMessage);
        return IMessageBus(messageBus).calcFee(msg);
    }
    ...
    function cBridgeIM(
        ...
        TransferHelper.safeTransferFrom(_fromToken, msg.sender, address(this), _inputAmount);
        ...
        bytes32 id = _computeSwapRequestId(msg.sender, chainId, _dstChainId, message);
        emit CBridgeIMStatusUpdated(id, _fromToken, _inputAmount, OperationStatus.Created);
    }
    ...
    function executeMessageWithTransferRefund(
        ...
        TransferHelper.safeTransfer(_token, m.originalSender, _amount);

        bytes32 id = _computeSwapRequestId(m.originalSender, uint64(block.chainid), m.dstChainId, _message);
        emit CBridgeIMStatusUpdated(id, _token, _amount, OperationStatus.RefundInSource);
    ...
    function executeMessageWithTransfer(
        ...
        if (m.path.length > 1) {
            ...
            if (ok) {
                _sendToken(m.toToken, dstAmount, m.recipient, m.nativeOut);
                status = OperationStatus.Succeeded;
                receivedToken = m.toToken;
            } else {
                // handle swap failure, send the received token directly to receiver
                _sendToken(_token, _amount, m.recipient, false);
                dstAmount = _amount;
            ...
            _sendToken(m.path[0], _amount, m.recipient, m.nativeOut);
            dstAmount = _amount;
            status = OperationStatus.Succeeded;
            receivedToken = m.path[0];
        }
        emit CBridgeIMStatusUpdated(id, receivedToken, _amount, status);
    ...
    function executeMessageWithTransferFallback(
        ...
        RangoCBridgeModels.RangoCBridgeInterChainMessage memory m = abi.decode((_message), (RangoCBridgeModels.RangoCBridgeInterChainMessage));
        bytes32 id = _computeSwapRequestId(m.originalSender, _srcChainId, uint64(block.chainid), _message);
        TransferHelper.safeTransfer(_token, m.recipient, _amount);

        emit CBridgeIMStatusUpdated(id, _token, _amount, OperationStatus.RefundInDestination);
    ...
    function _sendToken(
        ...
        if (_nativeOut) {
            require(_token == nativeWrappedAddress, "token mismatch");
            IWETH(nativeWrappedAddress).withdraw(_amount);
    ...
    function _trySwap(RangoCBridgeModels.RangoCBridgeInterChainMessage memory _swap, uint256 _amount) private returns (bool ok, uint256 amountOut) {
        require(whitelistContracts[_swap.dexAddress] == true, "Dex address is not whitelisted");
...
contract BaseContract is Pausable, Initializable, Ownable, ReentrancyGuard {
    ...
    function onChainSwaps(
        ...
        (bytes[] memory result, uint outputAmount) = onChainSwapsInternal(request, calls, false);
    ...
    function onChainSwapsInternal(
        ...
        uint balanceBefore = ercToken.balanceOf(address(this));

        bytes[] memory result = callSwapsAndFees(request, calls, isNative);

        uint balanceAfter = ercToken.balanceOf(address(this));
    ...
    function callSwapsAndFees(
        ...
        // validate
        require(feeContractAddress != NULL_ADDRESS, "Fee contract address not set");
        ...
        if (isNative) {
            require(request.fromToken == nativeWrappedAddress, "You should set from=WETH when asking for native swap");
            IWETH(nativeWrappedAddress).deposit{value: totalInputAmount}();
        } else {
