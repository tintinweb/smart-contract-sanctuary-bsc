...
contract SellBox is Ownable {
    ...
    mapping(address => bool) public claimed; // user => is claimed
    struct ChildBuyBox {
    ...
    constructor(IRefferal _refer, IERC1155 _box, IERC20 _tokenBuy, address _ceo, IPools _pool) {
        refer = _refer;
        box = _box;
        tokenBuy = _tokenBuy;
        ceo = _ceo;
    ...
    function claim() external {
        require(remains[0] > 0, "SellBox::claim: remain not enough");
        require(!claimed[_msgSender()], "SellBox::claim: claimed");

        box.safeTransferFrom(ceo, _msgSender(), 0, 1, '0x');
    ...
    function buy(uint _tokenId, uint _amount) external {
        ...
        uint feeSys = totalBill / 10;
        tokenBuy.transferFrom(_msgSender(), address(commTreasury), feeSys);

        uint comm = totalBill * 15 / 100;
        uint process = totalBill - comm - feeSys;
        ...
        tokenBuy.transferFrom(_msgSender(), address(pool), process);
        ...
        if(refer.isReferrer(_msgSender())) {
            ...
            tokenBuy.transferFrom(_msgSender(), ceo, comm);
        }
    ...
    function setReffer(IRefferal _refer) public onlyOwner {
        refer = _refer;
    }
