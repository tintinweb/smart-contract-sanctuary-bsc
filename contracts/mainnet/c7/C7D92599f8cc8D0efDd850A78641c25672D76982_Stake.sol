...
contract Stake is Ownable, DateTime {
    ...
    constructor() {
        _FIST = IERC20(0x689faD88202B4E40b0e9bDCBDC11129e7e53fB0F);
    ...
    function deposit(
        ...
        require(_FIST.balanceOf(msg.sender) >= fist, "Insufficient FIST");
        _handleUserAndRefer(msg.sender, refer);
        _FIST.transferFrom(msg.sender, address(this), fist);
    ...
    function withdrawOrder(uint256 index) public {
        ...
        _FIST.transfer(msg.sender, fist);
    ...
    function withdrawInvite() public {
        ...
        if (user.inviteBalance > 0) {
            _FIST.transfer(msg.sender, user.inviteBalance);
            emit WithdrawInvite(msg.sender, user.inviteBalance);
            user.inviteBalance = 0;
        }
    }
    function withdrawNode() public {
        ...
        if (user.nodeBalance > 0) {
            _FIST.transfer(msg.sender, user.nodeBalance);
