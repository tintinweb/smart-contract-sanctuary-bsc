...
https://t.me/NewCommunityBinaryX
...
library Address{
 ...
 contract cBNX is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x067a77Db8AD94a56332D1Cb2B47024EC109D1Ce3;
    address public developmentWallet = 0x067a77Db8AD94a56332D1Cb2B47024EC109D1Ce3;

    string private constant _name = "New Community BinaryX";
    string private constant _symbol = "cBNX";
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferbuyback) public view returns(uint256) {
        require(tAmount <= _tTotal, "Amount must be less than supply");
        if (!deductTransferbuyback) {
    ...
    function setTaxes(uint256 _buyback, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       taxes = Taxes(_buyback,_marketing,_liquidity,_development);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _buyback, uint256 _marketing, uint256 _liquidity, uint256 _development) public onlyOwner {
       sellTaxes = Taxes(_buyback,_marketing,_liquidity,_development);
        emit FeesChanged();
    }

    function _reflectbuyback(uint256 rbuyback, uint256 tbuyback) private {
        _rTotal -=rbuyback;
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rbuyback, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tbuyback-s.tMarketing-s.tLiquidity-s.tdevelopment;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rbuyback,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rbuyback-rMarketing-rLiquidity-rdevelopment;
        return (rAmount, rTransferAmount, rbuyback,rMarketing,rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rbuyback > 0 || s.tbuyback > 0) _reflectbuyback(s.rbuyback, s.tbuyback);
