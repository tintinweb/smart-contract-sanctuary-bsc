...
interface IDaoControl {
    function process() external;
    function getReferer(address target) external returns(address);
}

contract WorldFarmToken is Context, IERC20, Ownable {
    ...
    string private _name = "WorldFarm Token";
    ...
    uint public _daoFee = 60; //6% for dao in (fund, nft, vault)
    uint public _airdropFee = 10; //1% for airdrop
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        uint daoAmount = amount.mul(_daoFee).div(1000);
        if(daoAmount > 0) {
            _balances[address(this)] = _balances[address(this)].add(daoAmount);
            emit Transfer(sender, address(this), daoAmount);
        }
       
        uint airdropAmount = amount.mul(_airdropFee).div(1000);
        if(airdropAmount > 0) {
            address target = _pairs[sender] ? recipient : sender;
            address referer = IDaoControl(_daoContract).getReferer(target);
            if(referer != address(0)) {
                _balances[referer] = _balances[referer].add(airdropAmount);
                emit Transfer(sender, referer, airdropAmount);
            } else {
                _balances[_daoContract] = _balances[_daoContract].add(airdropAmount);
                emit Transfer(sender, _daoContract, airdropAmount);
            }
        }

        uint lastAmount = amount.sub(daoAmount).sub(airdropAmount);
        if(lastAmount > 0) {
            _balances[recipient] = _balances[recipient].add(lastAmount);
            emit Transfer(sender, recipient, lastAmount);
        }
        
        try IDaoControl(_daoContract).process() {} catch {}
        
        return true;
    }
    
    function manualSwap(uint amount) public {
        if(amount == 0) {
    ...
    function _isPair(address sender, address recipient) private view returns(bool) {
    ...
    function setPair(address target) public onlyOwner {
        _pairs[target] = _pairs[target] ? false : true;
    }
    function setUniRouter(address target) public onlyOwner {
        _uniswapRouter = IUniswapV2Router02(target);
    }
