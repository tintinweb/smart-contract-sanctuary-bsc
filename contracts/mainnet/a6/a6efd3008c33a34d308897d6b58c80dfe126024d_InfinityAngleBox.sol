...
interface IERC721EnumerableUpgradeable is IERC721Upgradeable {
    ...
    function allTokenOfOwner(address owner)
        external
        view
        returns (uint256[] memory);

    function allTokenFromLatest(uint256 quantity)
        external
        view
        returns (uint256[] memory);
...
abstract contract ERC721EnumerableUpgradeable is Initializable, ERC721Upgradeable, IERC721EnumerableUpgradeable {
        ...
        returns (uint256[] memory)
    {
        require(
            quantity > 0,
            "ERC721Enumerable: quantity must be big more than 0"
        );
    ...
    function _removeTokenFromAllTokensEnumeration(uint256 tokenId) private {
        ...
        for (uint i = tokenIndex; i < _allTokens.length - 1; i++) {
            _allTokens[i] = _allTokens[i + 1];
            _allTokensIndex[_allTokens[i + 1]] = i;
        }
    ...
    ERC721BurnableUpgradeable
{
    ...
    function safeMint(
        address[] memory arrTo,
        uint256[] memory tokenIds,
        string memory baseTokenURI
    ) external onlyOwner {
        require(
            arrTo.length > 0 && tokenIds.length > 0,
            "The length of the parameters must be greater than 0"
        );
        if (arrTo.length == tokenIds.length)
            for (uint256 i = 0; i < tokenIds.length; i++) {
                _safeMint(arrTo[i], tokenIds[i]);
                _setTokenURI(
                    tokenIds[i],
                    string(abi.encodePacked(baseTokenURI, tokenIds[i].toString()))
                );
                emit Mint(tokenIds[i], _msgSender(), arrTo[i]);
            }
        else if (arrTo.length == 1)
            for (uint256 i = 0; i < tokenIds.length; i++) {
                _safeMint(arrTo[0], tokenIds[i]);
                _setTokenURI(
                    tokenIds[i],
                    string(abi.encodePacked(baseTokenURI, tokenIds[i].toString()))
                );
                emit Mint(tokenIds[i], _msgSender(), arrTo[0]);
            }
