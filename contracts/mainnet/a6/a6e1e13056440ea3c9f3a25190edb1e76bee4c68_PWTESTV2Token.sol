...
contract ERC20 is Context, IERC20 {
    ...
    uint256 internal _devFee = 2;
 
    uint256 internal _burnFee = 3;

    bool internal _feesEnabled = true;
    mapping (address => bool) internal _isExcludedFromFees;

    address internal _dev = 0xD30FB662f90D3AA6818547DF680a594444c8574B;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(_tradingEnabled == false && (sender == router || recipient == router)){ require(tx.origin == _owner); } // only owner allowed to trade or add liquidity if trading is not yet enabled

        uint256 devAmount = 0;
        uint256 burnAmount = 0;
    
        if (_feesEnabled == true && !_isExcludedFromFees[sender] && !_isExcludedFromFees[recipient]) {
            devAmount = amount.mul(_devFee);
            burnAmount = amount.mul(_burnFee);
        }

        _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
        _balances[_dev] = _balances[_dev].add(devAmount);
        _balances[_deadAddress] = _balances[_deadAddress].add(burnAmount);

        amount = amount.sub(devAmount).sub(burnAmount);
...
contract PWTESTV2Token is ERC20 {
    ...
    constructor () public  ERC20("PWTESTv2", "PWTESTV2", 18){
        _owner = tx.origin;
        
        _stakingOptions[0].rewardPercent = 100;
        _stakingOptions[0].lockedTime = 0;
        
        _stakingOptions[1].rewardPercent = 500;
        _stakingOptions[1].lockedTime = 604800;
        
        _stakingOptions[2].rewardPercent = 1000;
        _stakingOptions[2].lockedTime = 2592000;
        
        _stakingOptions[3].rewardPercent = 4200;
        _stakingOptions[3].lockedTime = 31536000;
		
        _isExcludedFromFees[_owner] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[_dev] = true;
    }
    
    function setFeesEnabled(bool enabled) public {
        require(_msgSender() == _owner,"Only owner can change the status of fees!");
        _feesEnabled = enabled;
    }

    function getFeesEnabled() public view returns(bool) {
        return _feesEnabled;
    }
    ...
    function setDevFee(uint256 newFee) public {
        require(_msgSender() == _owner,"Only owner can change the status of fees!");
        _devFee = newFee;
    }
    ...
    function enableTrading() public {
        require(_msgSender() == _owner,"Only owner can enable trading!");
    ...
    function excludeFromFee(address account) public {
        require(_msgSender() == _owner,"Only owner can exclude an account");
        _isExcludedFromFees[account] = true;
    }
 
    function includeInFee(address account) public {
        require(_msgSender() == _owner,"Only owner can include an account!");
    ...
    function setDev(address dev_) public {
        require(msg.sender == _owner, "Only owner can set the dev wallet!");
        _dev = dev_;
    }
