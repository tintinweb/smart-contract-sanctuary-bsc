...
contract Staker is Ownable{
    ...
    uint256 constant public rate = 25;
    uint256 constant public minStakingAmount = 50 * 1e18;
    uint256 constant public waitPeriodSecconds = 90 * (60*60*24);
    uint256 constant public slicePeriodSeconds = 30 * (60*60*24);
    uint256 constant public totalSlices = 3;
    ...
    constructor(address _rewardToken, address[] memory _stakingTokens){
        ...
        for (uint256 i = 0; i < _stakingTokens.length; i++) {
            require(_stakingTokens[i] != address(0x0), "Invalid item in _stakingTokens");
    ...
    function stake(address stakingToken, uint256 amount) public {
        ...
        require(IERC20(stakingToken).allowance(msg.sender, address(this)) >= amount
                ,"Need more allowance of stakingToken");

        uint256 amtTotal = amount + ((((amount * 1e18) / 100) * rate) / 1e18);
        require((rewardToken.balanceOf(address(this)) - _lockedRewardTokens) >= amtTotal
                ,"Not enough supply of rewarding token");

        _lockedRewardTokens += amtTotal;

        _stakings[msg.sender].push(StakingData(amtTotal, 0, block.timestamp));

        ERC20Burnable(stakingToken).burnFrom(msg.sender, amount);
    }

    function computeMyTokens() public view returns(uint256){
        return _computeAllWithdrawableAmt(msg.sender);
    }

    function getMyStakingData() public view returns(StakingData[] memory){
    ...
    function withdrawMyTokens() public{
        ...
        rewardToken.transfer(holder, totAmount);
    }    

    function withdraw() public onlyOwner{
        uint256 amount = rewardToken.balanceOf(address(this)) - _lockedRewardTokens;
        require(amount > 0 , "No available tokens to withdraw");
        rewardToken.transfer(owner(), amount);
    }

    function lockedRewardTokens() public view onlyOwner returns(uint256){
        return _lockedRewardTokens;
    }

    function addStakingToken(address token) public onlyOwner {
    ...
    function removeStakingToken(address token) public onlyOwner {
        require(token != address(0x0), "Invalid token address");
        require(validStakingTokens[token], "Token already listed");
        validStakingTokens[token] = false;
    }
    
    function getStakingData(address holder) public view onlyOwner returns(StakingData[] memory){
        require(holder != address(0x0), "Invalid holder address");
        return _stakings[holder];
    }

    function computeTokens(address holder) public view onlyOwner returns(uint256){
        return _computeAllWithdrawableAmt(holder);
    }

    function _computeAllWithdrawableAmt(address holder) private view returns(uint256){
        uint256 amount;
        for (uint256 i = 0; i < _stakings[holder].length; i++) {
            amount += _computeAmt(_stakings[holder][i]);
        }
    ...
    function _computeAmt(StakingData storage stakingData) private view returns(uint256) {
        uint256 amount;
        uint256 currentTime = block.timestamp;

        if(stakingData.amtWithdrawn < stakingData.amtTotal &&
            currentTime >= (waitPeriodSecconds + stakingData.started))
        {                  
            uint256 slices = (currentTime - (stakingData.started + waitPeriodSecconds)) / slicePeriodSeconds;
            if(slices==0){
                return 0;
            }

            uint256 amtPerSlice = ((stakingData.amtTotal * 1e18) / totalSlices) /1e18;

            uint256 amtCalc = (amtPerSlice * slices) - stakingData.amtWithdrawn;

            uint256 availableTokens = stakingData.amtTotal - stakingData.amtWithdrawn;

            amount = (amtCalc > availableTokens ? availableTokens : amtCalc);

            if(amount < stakingData.amtTotal){
                uint256 leftOver = stakingData.amtTotal - (amount + stakingData.amtWithdrawn);
