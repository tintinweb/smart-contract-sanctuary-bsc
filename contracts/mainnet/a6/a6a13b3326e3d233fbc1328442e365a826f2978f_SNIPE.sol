...
//Development Team
/**
   _____ _   _ _____ _____  ______ ______ _____ _   _          _   _  _____ ______   _____ ____  __  __ 
  / ____| \ | |_   _|  __ \|  ____|  ____|_   _| \ | |   /\   | \ | |/ ____|  ____| / ____/ __ \|  \/  |
 | (___ |  \| | | | | |__) | |__  | |__    | | |  \| |  /  \  |  \| | |    | |__   | |   | |  | | \  / |
  \___ \| . ` | | | |  ___/|  __| |  __|   | | | . ` | / /\ \ | . ` | |    |  __|  | |   | |  | | |\/| |
  ____) | |\  |_| |_| |    | |____| |     _| |_| |\  |/ ____ \| |\  | |____| |____ | |___| |__| | |  | |
 |_____/|_| \_|_____|_|    |______|_|    |_____|_| \_/_/    \_\_| \_|\_____|______(_)_____\____/|_|  |_|
...
// File: contracts/SNIPEDividendPayingToken.sol
pragma solidity ^0.7.6;
...
contract SNIPE is ERC20, Ownable {
    ...
    SNIPEDividendTracker public dividendTracker;
    ...
    address payable public treasury2Address;
    address payable public buyBackAddress;
    address public treasuryTokenAddressForFee;
    address public treasury2TokenAddressForFee;
    address public buyBackTokenAddressForFee;
    // transfer fees
    uint256 public treasuryFee = 1;
    uint256 public treasury2Fee = 1;
    uint256 public buyBackFee = 1;
    uint256 public tokenRewardsFee = 1;
    uint256 public liquidityFee = 1;
    uint256 public totalFees = tokenRewardsFee.add(liquidityFee).add(treasuryFee).add(treasury2Fee).add(buyBackFee);
    // buy fees
    uint256 public buyTreasuryFee = 1;
    uint256 public buyTreasury2Fee = 1;
    uint256 public buyBuyBackFee = 1;
    uint256 public buyTokenRewardsFee = 1;
    uint256 public buyLiquidityFee = 1;
    uint256 public buyTotalFees = buyTokenRewardsFee.add(buyLiquidityFee).add(buyTreasuryFee).add(buyTreasury2Fee).add(buyBuyBackFee);
    // sell fees
    uint256 public sellTreasuryFee = 1;
    uint256 public sellTreasury2Fee = 1;
    uint256 public sellBuyBackFee = 1;
    uint256 public sellTokenRewardsFee = 1;
    uint256 public sellLiquidityFee = 1;
    uint256 public sellTotalFees = sellTokenRewardsFee.add(sellLiquidityFee).add(sellTreasuryFee).add(sellTreasury2Fee).add(sellBuyBackFee);
    // fee tracker
    uint256 private previousTreasuryFee = treasuryFee;
    uint256 private previousTreasury2Fee = treasury2Fee;
    ...
    function restoreFees() internal {
            ...
            treasury2Fee = previousTreasury2Fee;
    ...
    function updateTreasury2Address(address payable newAddress) external onlyOwner {
        treasury2Address = newAddress;
        _isExcludedFromFees[newAddress] = true;
    ...
    function updateTreasury2TokenFeeAddress(address newAddress) external onlyOwner {
        treasury2TokenAddressForFee = newAddress;
    }    
    function updateBuyBackTokenFeeAddress(address newAddress) external onlyOwner {
        buyBackTokenAddressForFee = newAddress;
    }
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _treasury2Fee, uint256 _buyBackFee) external onlyOwner {
        tokenRewardsFee = _tokenRewardsFee;
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
        treasury2Fee = _treasury2Fee;
        buyBackFee = _buyBackFee;
        totalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_treasury2Fee).add(_buyBackFee);
    }       
    function updateBuyFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _treasury2Fee, uint256 _buyBackFee) external onlyOwner {
        buyTokenRewardsFee = _tokenRewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyTreasuryFee = _treasuryFee;
        buyTreasury2Fee = _treasury2Fee;
        buyBuyBackFee = _buyBackFee;
        buyTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_treasury2Fee).add(_buyBackFee);
    }        
    function updateSellFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _treasuryFee, uint256 _treasury2Fee, uint256 _buyBackFee) external onlyOwner {
        sellTokenRewardsFee = _tokenRewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellTreasuryFee = _treasuryFee;
        sellTreasury2Fee = _treasury2Fee;
        sellBuyBackFee = _buyBackFee;
        sellTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_treasuryFee).add(_treasury2Fee).add(_buyBackFee);
    }
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "SNIPE: The dividend tracker already has that address");

        SNIPEDividendTracker newDividendTracker = SNIPEDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SNIPE: The new dividend tracker must be owned by the SNIPE token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SNIPE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        //require(_isExcludedFromFees[account] != excluded, "SNIPE: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "SNIPE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SNIPE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SNIPE: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SNIPE: Cannot update gasForProcessing to same value");
    ...
    function swapAndLiquify(uint256 tokens) private  {
        ...
        uint256 treasury2FeeFull = tokens.mul(treasury2Fee).div(100);
        uint256 buyBackFeeFull = tokens.mul(buyBackFee).div(100);
        uint256 tokensAfterFee = tokens - treasuryFeeFull - treasury2FeeFull - buyBackFeeFull;
        ...
        uint256 treasury2FeeBalance = newBalance.mul(treasury2Fee).div(100);
        uint256 buyBackFeeBalance = newBalance.mul(buyBackFee).div(100);
        uint256 finalBalance = newBalance - treasuryFeeBalance - treasury2FeeBalance - buyBackFeeBalance;
        ...
        if(treasury2TokenAddressForFee != address(0)){
            swapEthForTokens(treasury2FeeBalance, treasury2TokenAddressForFee, treasury2Address);
        }else{
            (bool sent,) = treasury2Address.call{value: treasury2FeeBalance}("");
        }        
        if(isBuyBackActive){
            swapEthForTokens(buyBackFeeBalance, address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56), buyBackAddress);
        }else{
    ...
    function _transfer(
        ...
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SNIPE: This account cannot send tokens until trading is enabled");
        }
        if(amount == 0) {
...
// buy
        ...
        if (
            ...
            previousTreasury2Fee = treasury2Fee;
...
contract SNIPEDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("SNIPE_Dividend_Tracker", "SNIPE_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens to get the dividends
    }
    ...
    function _transfer(address, address, uint256) internal override {
        require(false, "SNIPE_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "SNIPE_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SNIPE contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SNIPE_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SNIPE_Dividend_Tracker: Cannot update claimWait to same value");
