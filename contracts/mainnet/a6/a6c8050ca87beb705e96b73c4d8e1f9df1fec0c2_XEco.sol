...
abstract contract Initializable {
    ...
    function _setInitializedVersion(uint8 version) private returns (bool) {
        ...
        if (_initializing) {
            ...
            require(_initialized < version, "contract is already initialized");
...
contract XEco is IBEP20, OwnableUpgradeable {
    ...
    string private _name;//     = "XECO Token";
    string private _symbol; //  = "XEco";
    ...
    uint256 private _percentageOfLiquidityForDev;// = 64; 
    uint256 private _percentageforOperationsAndMarket;
    uint256 private _percentageforLiquidityTax;
    ...
    function initialize() initializer public {
        ...
        _transferTax   = 3;
        _liquidityFee  = 8; 
        _percentageOfLiquidityForDev = 50;
        _percentageforOperationsAndMarket = 40;
        _percentageforLiquidityTax = 10;
        _maxTxAmount     = 10000000000 * 10**18;
        _minTokenBalance = 0.5 * 10**18;
    ...
    function setDEVandOwner(address cOwner, address devWallet, address marketingWallet)public onlyOwner{
        _devWallet = devWallet;
        _marketingWallet = marketingWallet;
        _transferOwnership(cOwner);
        _rOwned[cOwner] = _rTotal;
        _tOwned[cOwner] = _tTotal; 
        // emit Transfer(address(0), cOwner, _tTotal);
    }
    
    // constructor (address cOwner, address devWallet, address marketingWallet) Ownable(cOwner) {
    //     _devWallet = devWallet;
    //     _marketingWallet = marketingWallet;
    ...
    //     // _rOwned[_devWallet] = (_rTotal *15)/100;
    //     // _rOwned[_marketingWallet] = (_rTotal *4)/100;
    //     // _rOwned[cOwner] = _rTotal-(_rOwned[_devWallet]+_rOwned[_marketingWallet]);
    ...
    function transfer(address recipient, uint256 amount) public override isTradeable returns (bool) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override isTradeable returns (bool) {
    ...
    //     require(tAmount <= _tTotal, "Must be less than supply");
    //     (, uint256 tFee, uint256 tLiquidity) = _getTValues(tAmount);
    //     uint256 currentRate = _getRate();
    ...
    function setPercentageForOperationsAndMarket(uint256 Fee) external onlyOwner {
        _percentageforOperationsAndMarket = Fee;
    }

    function setPercentageForLiquidityTax(uint256 Fee) external onlyOwner {
        _percentageforLiquidityTax = Fee;
    }
    ...
    function setTradeable(bool tradeable) public onlyOwner {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokenShareForDev = (contractTokenBalance * _percentageOfLiquidityForDev)/100;
        uint256 tokenShareForMarketing = (contractTokenBalance * _percentageforOperationsAndMarket)/100;
        uint256 tokenShareForLiquidity = (contractTokenBalance * _percentageforLiquidityTax)/100;
        uint256 initialBalance = address(this).balance;
        swapTokensForBnb(tokenShareForDev);
        payable(_devWallet).transfer(address(this).balance - initialBalance);
        initialBalance = address(this).balance;
        swapTokensForBnb(tokenShareForMarketing);
        payable(_marketingWallet).transfer(address(this).balance - initialBalance);
        ...
        */
        ...
        // uint256 marketingFee = newBalance.mul(_percentageforOperationsAndMarket).div(100);
        // uint256 LiquidityTaxFee = newBalance.mul(_percentageforLiquidityTax).div(100);
        // uint256 feeAdded = devFee.add(marketingFee);
        //uint256 bnbForLiquidity = newBalance.sub(feeAdded.add(LiquidityTaxFee));
        ...
        // }
        // if(LiquidityTaxFee > 0){
        //     payable(_liquidityWallet).transfer(LiquidityTaxFee);
        // }
    ...
    function _feeTransfer(uint256 fee) private{
        uint256 devFee = calculateFee(fee, 50);
        uint256 marketingFee = calculateFee(fee , 40);
        // uint256 autoLiquidateFee = calculateFee(fee , 10);
        uint256 testDevFee = (MAX - (MAX % devFee));
        uint256 testMarketingFee = (MAX - (MAX % marketingFee));
        _rOwned[_devWallet] = _rOwned[_devWallet].add(testDevFee);
        _rOwned[_marketingWallet] = _rOwned[_marketingWallet].add(testMarketingFee);
        // _rOwned[_devWallet] += autoLiquidateFee;
    }
}