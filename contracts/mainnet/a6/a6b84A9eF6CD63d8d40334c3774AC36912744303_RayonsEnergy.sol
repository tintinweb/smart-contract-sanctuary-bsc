...
contract RayonsEnergy is IBEP20, Ownable  {
    ...
    */
    ...
    address public  uniswapV2Pair; // Par MRRAYONSURAL/BNB
    /*
    ---------------------------------
    -         Modifier              -
    ---------------------------------
    ...
    */
    ...
    ---------------------------------
    */
    string private _name = "Contrato Teste";
    ...
    */
    ...
    uint256 public transferFee; // Taxa de Transferencia
    ...
    uint256 private limitBurn = 200000000 * _decimalPlace;
    uint256 private _tTotal = 20000000000 * _decimalPlace;
    uint256 public maxSellLimit = 200000000 * _decimalPlace; // Controle a Quantidade de Tokens que podem ser Vendidos
    uint256 public maxBuyLimit = 200000000 * _decimalPlace; // Controle a Quantidade de Tokens que podem ser Vendidos
    ...
    */
    constructor () {
        ...
        address pairCreated = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()); // Gera o Par RAYONS/BNB
    ...
    */
    function _approve(address owner, address spender, uint256 amount) internal {
        ...
        require(spender != address(0), "Spender nao pode ser Address 0");
    ...
    function _isUnlimitedAddress(address account) public view returns(bool) {
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        require(from != address(0), "From nao pode ser Address 0");
        require(amount > 0, "Montante precisa ser maior do que 0");
        _beforeTokenTransfer(from, to, amount);
        require(tradingOpen, "Trading precisa estar ativo");

 
        if(blackEnabled && !blackUser) {
           require(!_isBlackFrom(from) && !_isBlackTo(to), "Voce esta na Lista Negra");
        }
        ...
        require(fromBalance >= amount,"o valor da transferencia excede o saldo" );
        ...
        if(!takeFee) removeAllFee(); // Remove todas as Taxa
            ...
            if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
                fees = amount.mul(transferFee).div(100); // Define taxa de Venda
            }

            if(automatedMarketMakerPairs[from]) {
                fees = amount.mul(buyFee).div(100); // Define taxa de Compra

                if (amount > maxBuyLimit && !_isUnlimitedAddress(from) && !_isUnlimitedAddress(to)) {
                    revert("Montante de Compra nao pode ultrapassar limite"); 
                }
            }
            else if(automatedMarketMakerPairs[to]) {
                fees = amount.mul(sellFee).div(100); // Define taxa de Compra

                if (amount > maxSellLimit && !_isUnlimitedAddress(from) && !_isUnlimitedAddress(to)) {
    ...
    function changeFees(uint256 _buyFee, uint256 _sellFee, uint256 _transferFee, uint256 _tokenLiquidityPercent) external onlyOwner {
        buyFee = _buyFee;
        sellFee = _sellFee;
    ...
    function setBurn(uint256 _limitBurn) external onlyOwner {
        limitBurn = _limitBurn * _decimalPlace;
    }
    ...
    function withdrawRayons(uint256 rAmount) external onlyOwner {
        _transfer(address(this), _msgSender(), rAmount);
    }
    function initSwapLiquify(bool _startSwap) external onlyOwner {
        startSwap = _startSwap;
    }
    function setLimitsContract(uint256 _maxBuyLimit, uint256 _maxSellLimit) external onlyOwner {
        maxBuyLimit = _maxBuyLimit * _decimalPlace; // Limite de Compra
        maxSellLimit = _maxSellLimit * _decimalPlace; // Limite de Venda
    }
}