...
abstract contract DividendFee is Owned, DexBase, ERC20 {
    ...
    address public constant marketingAddr = 0x8B50D786482AB0EB807bD822356f901fA5EFB866;
    address public constant buybackAddr = 0x483e8E5eF657c4a483D584Eed37d0e8cA2E78b98;
...
contract CoonGold is ExcludedFromFeeList, DividendFee {
    uint256 private constant _totalSupply = 818 * 1e18;
    ...
    uint256 public _maxHavAmount = 10 * 1e18;
    uint256 public _maxTxSellAmount = 5 * 1e18;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    constructor() ERC20("CoonGold", "COINGOLD", 18) DividendFee() {
    ...
    function setIsTxLimitExempt(address holder, bool txExempt, bool havExempt) external onlyOwner {
    ...
    function _transfer(
        ...
        require(recipient == uniswapV2Pair || (balanceOf[recipient] + amount) <= _maxHavAmount || isHavLimitExempt[recipient], "HAV Limit Exceeded");

        if(recipient == uniswapV2Pair){require(amount <= _maxTxSellAmount || isTxLimitExempt[sender], "TX Limit Exceeded");}

        if(launchedAt + 20 <= block.number && block.number <= launchedAt + 40){
