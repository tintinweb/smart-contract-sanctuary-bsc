...
/**
    The Shopdi Protocol is a decentralized finance (DeFi) token.
...
contract Token is ERC20, Ownable {
    ...
    constructor(address _TokenStrategy, address _gnosisSafeWallet, string memory __name, string memory __symbol, uint _maxSupply, uint _mintPercent, uint _maxBurn)
    ERC20(__name, __symbol) {
        require(_gnosisSafeWallet != address(0));
        MAX_SUPPLY = _maxSupply;
        _mint(_gnosisSafeWallet, _mintPercent * MAX_SUPPLY / 100);
        MAX_BURN = _maxBurn;
     ...
     * Requirements
     *
     * - `msg.sender` must be the gnosisSafeWallet
     */
