...
contract Ownable is Context {
     ...
     */
    constructor() {
        _owner = address(0x49c41D44b77628778c09b1382a4bc729E997E502);
...
contract SHIBAMOVE is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet =
        payable(0x450E5abBE32053CBeb50dd5F7EFb30dB77CA7894);
    address public AppWallet = 0xDd84714Fa4812a94aC0ddcf4A3625a3fb4Defd0a;
    address public immutable DeadWallet =
        0x000000000000000000000000000000000000dEaD;

    uint256 private constant MAX = ~uint256(0);
    uint256 private constant _tTotal = 50 * 10**9 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "SHIBAMOVE";
    ...
    uint256 private _AppFee = 2;
    uint256 private _previousAppFee = _AppFee;
    ...
    event CoolDownUpdated(bool CoolDownUpdated);
    ...
    constructor() {
        ...
        _isExcludedFromFee[AppWallet] = true;
    ...
    function UpdateWallets(
        ...
        address newAppWallet
    ) external onlyOwner {
        require(
            newMarketingWallet != address(0) && newAppWallet != address(0),
    ...
    function UpdateCoolDownStatus(bool status) external onlyOwner {
        coolDownEnabled = status;

        emit CoolDownUpdated(coolDownEnabled);
    }
    ...
    function UpdateBuyTaxFeePercentage(
        ...
        require(
            liquidityFee.add(burnFee).add(MarketingFee).add(AppFee) <=
    ...
    function _transfer(
        ...
        if (
            from != owner() && !_isAtomaticMarketMaker[from] && coolDownEnabled
        ) {
            if (blockNo[from] == 0) {
                blockNo[from] = block.number.add(1);
            } else {
                require(blockNo[from] < block.number);
                blockNo[from] = block.number.add(1);
            }
        }
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takeApp(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
    ...
    function takeApp(
        ...
        uint256 tApp = tAmount.mul(_AppFee).div(feeDenominator);
        uint256 rApp = tApp.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rApp);
        tTransferAmount = tTransferAmount.sub(tApp);
        _rOwned[AppWallet] = _rOwned[AppWallet].add(rApp);
        emit Transfer(sender, AppWallet, tApp);
        return (tTransferAmount, rTransferAmount);
    }
