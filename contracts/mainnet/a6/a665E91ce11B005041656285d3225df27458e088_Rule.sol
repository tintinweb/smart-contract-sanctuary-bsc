...
contract Rule is Context, Ownable {
    ...
    uint256 public _tTotalMaxOne=20000 * 10**18;
    uint256 public _stopBurn=20000 * 10**18;
    uint256 public _threshold=0 * 10**18; 
    uint256[] public _rate=[10,10,10,20,10,20,10,6,3,1];

    address public _tokenAddr=address(0xcd1141c3FF3649682852D9b59bf3B9F629fAf1ac);
  ...
  function buy( address u, uint256 a) private returns(uint256 sy) {
        ...
        if(_rate[1]>0){ sy-=inviterFee(u,_fundAddress,a*_rate[1]/1000);}
    ...
    function sell( address u, uint256 a) private returns(uint256 sy) {
        sy=a;
        if(_rate[3]>0){ sy-=addBalanceWithLog(u,_bonusAddress,a*_rate[3]/1000);}
        if(_rate[4]>0){ sy-=inviterFee(u,_fundAddress,a*_rate[4]/1000);}
        if(_rate[5]>0){ sy-=addBalanceWithLog(u,_lpFundAddress,a*_rate[5]/1000);}
    }
    ...
    function inviterFee(address u,address t,uint256 am) private returns(uint256 x){
        uint256 outx=0; address cur=u;
        for (uint256 i = 1; i <= 3; i++) {
            cur = Token(_tokenAddr)._inviter(cur);
            if (cur == address(0)) {
                break;
            }
            if(Token(_tokenAddr).balanceOf(cur)>=10000000000000000){
                outx += addBalanceWithLog(u,cur,am*_rate[6+i]/1000);
            }
        }
        addBalanceWithLog(u,t,am-outx);
        return am;
    }
