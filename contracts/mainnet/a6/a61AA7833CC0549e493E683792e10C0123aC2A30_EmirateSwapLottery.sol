...
interface IEmirateSwapLottery {
     ...
     */
    function buyTickets(uint256 _lotteryId, uint32[] calldata _ticketNumbers) payable external;
...
// File: contracts/EmirateSwapLottery.sol
 ...
 */
contract EmirateSwapLottery is ReentrancyGuard, IEmirateSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBNB = 50 ether;
    uint256 public minPriceTicketInBNB = 0.005 ether;
        ...
        payable
    {
        ...
        // Calculate number of BNB to this contract
        uint256 amountBNBToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBNB,
            _ticketNumbers.length
        );

        //  msg.value must be equal or greater than amountBNBToTransfer
        require(msg.value >= amountBNBToTransfer, "BNB not enough");

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBNB += amountBNBToTransfer;
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBNB) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBNB - amountToShareToWinners);
     ...
     */
    function injectFunds(uint256 _lotteryId) external override onlyOwnerOrInjector payable {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        _lotteries[_lotteryId].amountCollectedInBNB += msg.value;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBNB >= minPriceTicketInBNB) && (_priceTicketInBNB <= maxPriceTicketInBNB),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBNB: _priceTicketInBNB,
            ...
            amountCollectedInBNB: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    /**
     * @notice Set BNB price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBNB: minimum price of a ticket in BNB
     * @param _maxPriceTicketInBNB: maximum price of a ticket in BNB
     */
    function setMinAndMaxTicketPriceInBNB(uint256 _minPriceTicketInBNB, uint256 _maxPriceTicketInBNB)
        external
        onlyOwner
    {
        require(_minPriceTicketInBNB <= _maxPriceTicketInBNB, "minPrice must be < maxPrice");

        minPriceTicketInBNB = _minPriceTicketInBNB;
        maxPriceTicketInBNB = _maxPriceTicketInBNB;
    }
