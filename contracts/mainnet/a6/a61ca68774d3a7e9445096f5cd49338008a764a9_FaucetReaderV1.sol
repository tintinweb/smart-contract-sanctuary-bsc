...
contract FaucetReaderV1 {
    ...
    function getUserRefClaimPos(address _addr) internal view returns (uint256){
        (address upline, uint256 referrals, uint256 total_structure, uint256 direct_bonus, uint256 match_bonus, uint256 deposits, uint256 deposit_time, uint256 payouts, uint256 rolls, uint256 ref_claim_pos, uint256 accumulatedDiv) = faucet.users(_addr);
        return ref_claim_pos;
    ...
    function getFullPlayerDetail(address _addr) external view returns (uint256 claimsAvailable, uint256 br34pBalance, uint256 ref_claim_pos, uint256[] memory userStats, address[] memory uplines, bool[] memory uplinesRewardsAllowed, address nextUplineRewarded) {
        ...
        ref_claim_pos = getUserRefClaimPos(_addr);
