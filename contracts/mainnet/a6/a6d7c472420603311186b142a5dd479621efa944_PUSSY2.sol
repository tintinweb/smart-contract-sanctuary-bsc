...
contract PUSSY2 is Context, IERC20, Ownable {
  ...
  string private _name = "PUSSY2";
  string private _symbol = "PUS2";

  uint256 private _decimals = 9;
  uint256 private _totalSupply = 10000 * 10**_decimals;
  uint256 private _maxBuy = _totalSupply.mul(10).div(1000);
  uint256 private _maxSell = _totalSupply.mul(10).div(1000);
  ...
  address payable private _buybackWallet = payable(0x09D3153E747885A708b85B78527DB840fBA32a51);
  address payable private _marketingWallet = payable(0xa3ad44739B28Fb84FcA64fc5f3a71E484632c56e);
  ...
  uint256 private _numTokensForSwap = 1000 * 10**_decimals;
  ...
  function _transfer(
    ...
    if (_stopSnipe && isBuy && !isOwnerTransfer && !_inSwapAndLiquify && !isLiquidityTransfer && !isInnerTransfer) {
      ...
      if ( !blackListInstead ){
        ...
        if ( block.number <= deadBlockNumber )  _isBlackListed[recipient] = true;
         _tokenTransfer(isBuy, sender, recipient, amount, takeFee, isInnerTransfer);
    }
  ...
  function diableStopSnipe(uint256 _deadBlocks) public onlyOwner {
    _stopSnipe = false;
  ...
  function withdrawBNBEmergency(address payable account, uint256 amount) public onlyOwner {
