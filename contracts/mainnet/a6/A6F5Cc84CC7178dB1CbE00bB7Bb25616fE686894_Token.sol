...
interface LP {
    function getKLN(address, uint256) external;
}
...
contract Token is Context, IERC20, Ownable {
    ...
    string private _name = "QSQ";
    string private _symbol = "QSQ";
    ...
    address public ownerAddress =
        address(0x3a3d5F7d433090B3D87805b7d93A6c15b845F15F);
    address public usdtAddress =
        address(0x55d398326f99059fF775485246999027B3197955);
    address public lpAddress =
        address(0x8746BA5186F35e35A9f754Fb61ca792101Ed4C7c);
    ...
    constructor() public {
        ...
        _balances[ownerAddress] = 1000000 * 10**18;
        _totalSupply = 1000000 * 10**18;

        transferOwnership(ownerAddress);

        emit Transfer(address(0), ownerAddress, 1000000 * 10**18);
    }
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        if (_balances[burnAddress] >= 900000 * 10**18) {
            return 0;
        }
    ...
    function removeAllFee() private {
        if (_liquidityFee == 0 && _burnFee == 0 && _rewardFee == 0) return;
    ...
    function _takeReward(uint256 rewardFee) private {
        ...
        // swap tokens for BNB
        swapTokensForU(rewardFee, address(this));

        // how much BNB did we just swap into?
        uint256 newBalance = usdt.balanceOf(address(this)).sub(
            initialBalance
        );

        uint256 pairReward = newBalance.div(2);
        uint256 lpReward = newBalance.sub(pairReward);

        usdt.transfer(swapPair, pairReward);
        usdt.transfer(lpAddress, lpReward);
    ...
    function _takeLiquidity(address sender, uint256 liquidityFee) private {
        ...
        // swap tokens for BNB
        swapTokensForU(liquidityFee, address(this));
        ...
        usdt.transfer(lpAddress, newBalance);
        LP(lpAddress).getKLN(sender, newBalance);
        inSwapAndLiquify = false;
    }

    function swapTokensForU(uint256 tokenAmount, address recipient) public {
