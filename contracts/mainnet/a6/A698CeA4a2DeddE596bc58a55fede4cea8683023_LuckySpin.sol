...
contract LuckySpin is Ownable {
    ...
    function spin(uint256 _nonce, bytes memory _sign) external {
        ...
        if (prize == GOLD_CHEST || prize == PLATINUM_CHEST || prize == HERALD_CHEST || prize == RUBY_CHEST || prize == DIAMOND_CHEST) {
            ...
            emit SpinSuccess(user, info.name, 1, 0);
    ...
    function updatePrize(bytes32 _prize, string memory _name, uint256 _limit, uint256 _percent) external onlyOwner {
        Prize storage prize = prizeInfo[_prize];
        prize.name = _name;
        prize.limit = _limit;
        prize.percent = _percent;
        _calculateAcc();
    }
    ...
    function setLimitSpin(uint256 _limitSpin) external onlyOwner {
        limitSpin = _limitSpin;
    }
