...
contract Switcc is ERC20 {
    using SafeMath for uint256;
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    address _tokenOwner;
	address _baseToken = address(0x55d398326f99059fF775485246999027B3197955);
	address _ethToken = address(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
	...
	address private _maketAddress = address(0xF8cdf806318E1733C37a9F26d025eb94B1a545A9);
    ...
    constructor(address tokenOwner) ERC20("Swit-cc", "Swit-cc") {
	...
	function swapToken2btcethusdt() public {
		uint256 allAmount = balanceOf(address(this));
		uint256 canLdxAmount = ldxAmount.sub(ldxOverAmount);
		if(allAmount > canLdxAmount.add(5 * 10**18)){
			uint256 canSwapAmount = allAmount.sub(canLdxAmount);
			swapTokensForUsdt2Fund(canSwapAmount.div(10).mul(2));
			swapTokensForUsdt2Mark(canSwapAmount.div(10).mul(2));
			swapTokensForBTC(canSwapAmount.div(10).mul(3));
			swapTokensForETH(canSwapAmount.div(10).mul(3));
		}
	}
	...
	function swapAndLiquifyLdx() public {
		uint256 allAmount = balanceOf(address(this));
		uint256 canLdxAmount = ldxAmount.sub(ldxOverAmount);
		if(allAmount >= canLdxAmount){
			ldxOverAmount = ldxOverAmount.add(canLdxAmount);
			uint256 canSwapAmount = canLdxAmount.div(2);
			uint256 otherAmount = canLdxAmount.sub(canSwapAmount);
			swapTokensForOther(canSwapAmount);
			uint256 usdtAmount = USDT.balanceOf(address(this));
			addLiquidityUsdt(usdtAmount,otherAmount);
		}
    }
	...
	function swapTokensForUsdt2Fund(uint256 tokenAmount) internal {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
	...
	function swapTokensForUsdt2Mark(uint256 tokenAmount) internal {
		address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(_baseToken);
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            if(totalAmount>0){
                ...
                if(buySize >10){
                    for(uint256 i=0;i<10;i++){
                        ...
                        if(rate>0){
                            BTC.transfer(user,btcPerAmount.mul(rate).div(1000000));
                            ETH.transfer(user,ethPerAmount.mul(rate).div(1000000));
                        }
                        startIndex += 1;
                    }
                }else{
	...
	uint256 public btcPerAmount = 10**14;
	uint256 public ethPerAmount = 10**15;
	function _splitOtherToken() private {
        btcPerAmount = BTC.balanceOf(address(this));
		ethPerAmount = ETH.balanceOf(address(this));
        if(btcPerAmount >= 10**14 && ethPerAmount >= 10**15){
