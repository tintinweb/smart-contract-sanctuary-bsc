...
contract DeltaNeutralVault02 is
  ERC20Upgradeable,
  ...
  OwnableUpgradeable
{
  ...
  // --- Events ---
  event LogInitializePositions(
    address indexed _from,
    uint256 _stableVaultPosId,
    uint256 _assetVaultPosId
  );
  event LogDeposit(
    address indexed _from,
  ...
  error DeltaNeutralVault_UnsafeOutstanding(
    address _token,
    uint256 _amountBefore,
    uint256 _amountAfter
  );
  error DeltaNeutralVault_PositionsIsHealthy();
  error DeltaNeutralVault_InsufficientTokenReceived(
    address _token,
    uint256 _requiredAmount,
    uint256 _receivedAmount
  );
  error DeltaNeutralVault_InsufficientShareReceived(
    uint256 _requiredAmount,
    uint256 _receivedAmount
  );
  error DeltaNeutralVault_UnTrustedPrice();
  error DeltaNeutralVault_PositionValueExceedLimit();
  error DeltaNeutralVault_WithdrawValueExceedShareValue(
  ...
  modifier onlyReinvestors() {
    if (!config.whitelistedReinvestors(msg.sender))
      revert DeltaNeutralVault_Unauthorized(msg.sender);
    _;
  }
  ...
  function initialize(
    ...
    if (address(alpacaToken) == address(0))
      revert DeltaNeutralVault_InvalidInitializedAddress();
  ...
  function _checkAndMint(
    ...
    uint256 _depositValue = _calculateEquityChange(
      _positionInfoAfter,
      _positionInfoBefore
    );
    ...
    if (_sharesToUser < _minShareReceive) {
      revert DeltaNeutralVault_InsufficientShareReceived(
        _minShareReceive,
        _sharesToUser
      );
    }
  ...
  function withdraw(
    ...
    uint256 _withdrawalFeeBps = config.feeExemptedCallers(msg.sender)
      ? 0
      : config.withdrawalFeeBps();
    uint256 _shareToWithdraw = ((MAX_BPS - _withdrawalFeeBps) * _shareAmount) /
      MAX_BPS;
  ...
  function _checkAndTransfer(
    ...
    if (_stableTokenBack < _minStableTokenAmount) {
      revert DeltaNeutralVault_InsufficientTokenReceived(
        stableToken,
        _minStableTokenAmount,
        _stableTokenBack
      );
    ...
    uint256 _withdrawValue = _calculateEquityChange(
      _positionInfoBefore,
      _positionInfoAfter
    );

    if (_withdrawShareValue < _withdrawValue) {
      revert DeltaNeutralVault_WithdrawValueExceedShareValue(
        _withdrawValue,
        _withdrawShareValue
      );
    }
    ...
    IController _controller = IController(config.controller());
    if (address(_controller) != address(0))
      _controller.onWithdraw(tx.origin, _shareAmount);
  ...
  function rebalance(
    ...
    uint256 _assetPositionValue = _positionInfoBefore.assetPositionEquity +
      _positionInfoBefore.assetPositionDebtValue;
    uint256 _equityBefore = _positionInfoBefore.stablePositionEquity +
      _positionInfoBefore.assetPositionEquity;
    uint256 _rebalanceFactor = config.rebalanceFactor(); // bps

    // 1. check if positions need rebalance
    if (
      _stablePositionValue * _rebalanceFactor >=
      _positionInfoBefore.stablePositionDebtValue * MAX_BPS &&
      _assetPositionValue * _rebalanceFactor >=
      _positionInfoBefore.assetPositionDebtValue * MAX_BPS
    ) {
  ...
  function reinvest(
    ...
    IFairLaunch(_fairLaunchAddress).harvest(
      IVault(stableVault).fairLaunchPoolId()
    );
    IFairLaunch(_fairLaunchAddress).harvest(
      IVault(assetVault).fairLaunchPoolId()
    );
    uint256 _alpacaAmount = IERC20Upgradeable(alpacaToken).balanceOf(
      address(this)
    );
    ...
    if (_beneficiaryShare > 0)
      IERC20Upgradeable(alpacaToken).safeTransfer(
        config.alpacaBeneficiary(),
        _beneficiaryShare
      );
    IERC20Upgradeable(alpacaToken).safeTransfer(
      config.alpacaReinvestFeeTreasury(),
      _bounty - _beneficiaryShare
    );
  ...
  function _depositHealthCheck(
    ...
    uint256 _expectedStableEqChange = (_depositValue * (_leverageLevel - 2)) /
      ((2 * _leverageLevel) - 2);
    // The equity allocation of short side should be equal to _depositValue * _leverageLevel / ((2*_leverageLevel) - 2)
    uint256 _expectedAssetEqChange = (_depositValue * _leverageLevel) /
      ((2 * _leverageLevel) - 2);

    uint256 _actualStableDebtChange = _positionInfoAfter
      .stablePositionDebtValue - _positionInfoBefore.stablePositionDebtValue;
    ...
    uint256 _expectedStableDebtChange = (_expectedStableEqChange *
      (_leverageLevel - 1));
    // The debt allocation of short side should be equal to _expectedAssetEqChange * (_leverageLevel - 1)
    uint256 _expectedAssetDebtChange = (_expectedAssetEqChange *
      (_leverageLevel - 1));
  ...
  function _withdrawHealthCheck(
    ...
    uint256 _totalEquityBefore = _positionInfoBefore.stablePositionEquity +
      _positionInfoBefore.assetPositionEquity;
    ...
    if (
      !Math.almostEqual(
        (_stableLpWithdrawValue -
          (_positionInfoBefore.stablePositionDebtValue -
            _positionInfoAfter.stablePositionDebtValue)) * _totalEquityBefore,
    ...
    if (
      !Math.almostEqual(
        (_assetLpWithdrawValue -
          (_positionInfoBefore.assetPositionDebtValue -
            _positionInfoAfter.assetPositionDebtValue)) * _totalEquityBefore,
    ...
    uint256 _totalStablePositionBefore = _positionInfoBefore
      .stablePositionEquity + _positionInfoBefore.stablePositionDebtValue;
    uint256 _totalStablePositionAfter = _positionInfoAfter
      .stablePositionEquity + _positionInfoAfter.stablePositionDebtValue;
    ...
    uint256 _totalassetPositionBefore = _positionInfoBefore
      .assetPositionEquity + _positionInfoBefore.assetPositionDebtValue;
  ...
  function _outstandingCheck(
    Outstanding memory _outstandingBefore,
    Outstanding memory _outstandingAfter
  ) internal view {
  ...
  function positionInfo() public view returns (PositionInfo memory) {
    ...
    uint256 _stableDebtValue = _positionDebtValue(
      stableVault,
      stableVaultPosId,
      stableTo18ConversionFactor
    );
    uint256 _assetDebtValue = _positionDebtValue(
      assetVault,
      assetVaultPosId,
      assetTo18ConversionFactor
    );

    return
      PositionInfo({
        stablePositionEquity: _stablePositionValue > _stableDebtValue
          ? _stablePositionValue - _stableDebtValue
          : 0,
        stablePositionDebtValue: _stableDebtValue,
        stableLpAmount: _stableLpAmount,
        assetPositionEquity: _assetPositionValue > _assetDebtValue
          ? _assetPositionValue - _assetDebtValue
          : 0,
  ...
  function totalEquityValue() public view returns (uint256) {
    uint256 _totalPositionValue = _lpToValue(
      IWorker02(stableVaultWorker).totalLpBalance() +
        IWorker02(assetVaultWorker).totalLpBalance()
    );
    uint256 _totalDebtValue = _positionDebtValue(
      stableVault,
      stableVaultPosId,
      stableTo18ConversionFactor
    ) +
  ...
  function setDeltaNeutralOracle(IDeltaNeutralOracle _newPriceOracle)
    external
    onlyOwner
  {
    // sanity call
  ...
  function setDeltaNeutralVaultConfig(
    IDeltaNeutralVaultConfig02 _newVaultConfig
  ) external onlyOwner {
    // sanity call
  ...
  function _positionDebtValue(
    ...
    if (_vaultDebtShare == 0) {
      return
        (_positionDebtShare * _18ConversionFactor).mulWadDown(
          _getTokenPrice(_token)
        );
    }
    uint256 _vaultDebtValue = IVault(_vault).vaultDebtVal() +
      IVault(_vault).pendingInterest(0);
    uint256 _debtAmount = FullMath.mulDiv(
      _positionDebtShare,
      _vaultDebtValue,
      _vaultDebtShare
    );
    return
      (_debtAmount * _18ConversionFactor).mulWadDown(_getTokenPrice(_token));
  }
  ...
  function _lpToValue(uint256 _lpAmount) internal view returns (uint256) {
    (uint256 _lpValue, uint256 _lastUpdated) = priceOracle.lpToDollar(
      _lpAmount,
      lpToken
    );
    if (block.timestamp - _lastUpdated > 86400)
      revert DeltaNeutralVault_UnTrustedPrice();
    return _lpValue;
  }
  ...
  function _calculateEquityChange(
    ...
    uint256 _lpChange = (_greaterPosition.stableLpAmount +
      _greaterPosition.assetLpAmount) -
      (_lesserPosition.stableLpAmount + _lesserPosition.assetLpAmount);

    uint256 _debtChange = (_greaterPosition.stablePositionDebtValue +
      _greaterPosition.assetPositionDebtValue) -
      (_lesserPosition.stablePositionDebtValue +
        _lesserPosition.assetPositionDebtValue);
  ...
  function _execute(
    ...
    if (_actions.length != _values.length || _actions.length != _datas.length)
      revert DeltaNeutralVault_BadActionSize();
  ...
  function _doWork(bytes memory _data) internal {
    ...
    if (
      OPENING != 1 &&
      !((_vault == stableVault && _posId == stableVaultPosId) ||
        (_vault == assetVault && _posId == assetVaultPosId))
    ) {
      revert DeltaNeutralVault_InvalidPositions({
        _vault: _vault,
        _positionId: _posId
      });
    }
  ...
  function _valueToShare(uint256 _value, uint256 _totalEquity)
    internal
    view
    returns (uint256)
  {
  ...
  function _to18ConversionFactor(address _token)
    internal
    view
    returns (uint256)
  {
