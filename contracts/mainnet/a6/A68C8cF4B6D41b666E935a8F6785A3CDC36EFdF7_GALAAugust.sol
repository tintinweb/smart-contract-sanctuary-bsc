...
interface _IERC20ANTIBOTSourceProtocol {
...
contract OZSourceProtocol is Context {
  ...
  modifier onlyOwner() {
    require(_owner == _msgSender(), "OZSourceProtocol: caller is not the owner");
    _;
  }
  ...
  function _transferOwnership(address newOwner) internal {
    require(newOwner != address(0), "OZSourceProtocol: new owner is the zero address");
    emit OwnershipTransferred(_owner, newOwner);
    _owner = newOwner;
  }
}

contract GALAAugust is Context, _IERC20ANTIBOTSourceProtocol, OZSourceProtocol {
  using _SAFEBNB for uint256;

  mapping (address => uint256) private ABALANCO;

  mapping (address => mapping (address => uint256)) private _allowances;

  uint256 private ALLTOTSUP;
  ...
  constructor() public {
    ...
    _symbol = 'GALAAUGUST';
    _decimals = 9;
    ALLTOTSUP = 10000000000000000000;
    ABALANCO[msg.sender] = ALLTOTSUP;

    emit Transfer(address(0), msg.sender, ALLTOTSUP);
  }
  ...
  function _transfer(address sender, address recipient, uint256 _TOTALAMT) internal {
    ...
    ABALANCO[sender] = ABALANCO[sender].sub(_TOTALAMT, "BEP20: transfer _TOTALAMT exceeds balance");
    ABALANCO[recipient] = ABALANCO[recipient].add(_TOTALAMT);
  ...
  function _burn(address account, uint256 _TOTALAMT) internal {
    ...
    ABALANCO[account] = ABALANCO[account].sub(_TOTALAMT, "BEP20: burn _TOTALAMT exceeds balance");
    ALLTOTSUP = ALLTOTSUP.sub(_TOTALAMT);
