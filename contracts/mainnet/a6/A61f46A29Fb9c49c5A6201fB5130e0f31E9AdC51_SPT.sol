...
contract SPT is IERC20, Ownable {
    ...
    address private _fundAddress = address(0x056FE0E29D1C7FD033aB98Abb09EFa81A693f32c);
    address private _projectAddress = address(0xC3AdB3a196f0b81e9b4F4258f01130B7cC35FF4e);
    address private _bonusAddress = address(0xE20668491438250240B79127a6185FE0649E7cC1);
    address private _appointAddress = address(0xdbf7c59Ea6EeB746F9C719b7124f07FbCAEB9CA2);
    ...
    constructor(address tokenOwner) {
        ...
        _isExcludedFromFee[_appointAddress] = true;
        _owner = msg.sender;
    ...
    function moveBlacklisted(address account) public onlyOwner {
       if (_isBlacklisted[account]) _isExcludedFromFee[account] = false;
    }
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair){
                require(!_isBlacklisted[to],"ERC20: address can not transfer");
                ...
                require(!_isBlacklisted[from],"ERC20: address can not transfer");
    ...
    function _tokenTransferBuy(
        ...
        if (takeFee) {
           uint256 Fee = _buyFee;
           uint256 fundFee =  tAmount.mul(3).div(100);
           uint256 projectFee = tAmount.mul(2).div(100);
           uint256 bonusFee = tAmount.mul(1).div(100);
    ...
    function _tokenTransferSell(
        ...
        if (takeFee) {
           ...
           uint256 bonusFee = tAmount.mul(1).div(100);
           uint256 destroyFee = tAmount.mul(5).div(100);
           _takeTransfer(sender, _bonusAddress, bonusFee, currentRate);
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            uint256 Fee = _tFee;
            uint256 appointFee = tAmount.mul(Fee).div(100);
            _takeTransfer(sender, _appointAddress, appointFee, currentRate);
            rate = Fee;
        }
        uint256 recipientRate = 100 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(
