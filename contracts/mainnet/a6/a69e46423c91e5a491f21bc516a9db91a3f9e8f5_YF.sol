...
contract YF is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount = 8888 * 10 **18;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        if (IERC20(fistToken).balanceOf(address(this)) >= 10) {
            IERC20(fistToken).transfer(address(devAddress), IERC20(fistToken).balanceOf(address(this)));
        }
        uint256 addHl = uint256(100).mul(_liquidityFee).div(_liquidityFee.add(_devFee));
        uint256 addNumber = contractTokenBalance.mul(addHl).div(100).div(2);
        uint256 devNumber = contractTokenBalance.sub(addNumber);
        uint256 initialBalance = IERC20(fistToken).balanceOf(address(this));
        swapTokensForDividendToken(devNumber);
        uint256 newBalance =IERC20(fistToken).balanceOf(address(this)).sub(initialBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 tokenBmount) public {
        ...
        IERC20(fistToken).approve(address(uniswapV2Router), tokenBmount * 100);
        // add the liquidity
