...
// File: contracts/interfaces/IBSExchangeRouter.sol


pragma solidity 0.6.12;

/// @dev an simple interface for integration dApp to swap
interface IBSExchangeRouter {
...
// File: contracts/interfaces/IBSLiquidityRouter.sol


pragma solidity 0.6.12;

/// @dev an simple interface for integration dApp to contribute liquidity
interface IBSLiquidityRouter {
    /**
...
interface IBSRouter01 is IBSExchangeRouter, IBSLiquidityRouter {
...
// File: contracts/periphery/BSRouter02.sol
...
contract BSRouter02 is IBSRouter02 {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB, uint256 vReserveA, uint256 vReserveB, ) = BSLibrary
            .getTradeInfo(pool, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = BSLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, "BSRouter: INSUFFICIENT_B_AMOUNT");
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = BSLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, "BSRouter: INSUFFICIENT_A_AMOUNT");
                (amountA, amountB) = (amountAOptimal, amountBDesired);
            }
            uint256 currentRate = (vReserveB * Q112) / vReserveA;
            require(
                currentRate >= vReserveRatioBounds[0] && currentRate <= vReserveRatioBounds[1],
                "BSRouter: OUT_OF_BOUNDS_VRESERVE"
            );
        }
    }
        ...
        )
    {
        ...
        liquidity = IBSPool(pool).mint(to);
    }
        ...
        )
    {
        ...
        if (ampBpsAndFeeBps[0] == BPS) {
            pool = IBSFactory(factory).getUnamplifiedPool(tokenA, tokenB);
        }
        if (pool == address(0)) {
            pool = IBSFactory(factory).createPool(
                tokenA,
                tokenB,
                ampBpsAndFeeBps[0],
        ...
        )
    {
        address pool;
        if (ampBpsAndFeeBps[0] == BPS) {
            pool = IBSFactory(factory).getUnamplifiedPool(token, weth);
        }
        if (pool == address(0)) {
            pool = IBSFactory(factory).createPool(
                token,
    ...
    function removeLiquidity(
        ...
        (uint256 amount0, uint256 amount1) = IBSPool(pool).burn(to);
        (IERC20 token0, ) = BSLibrary.sortTokens(tokenA, tokenB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (IERC20 token0, ) = BSLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? poolsPath[i + 1] : _to;
            IBSPool(poolsPath[i]).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = BSLibrary.getAmountsOut(amountIn, poolsPath, path);
        require(
    ...
    function swapTokensForExactTokens(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = BSLibrary.getAmountsIn(amountOut, poolsPath, path);
    ...
    function swapExactETHForTokens(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = BSLibrary.getAmountsOut(msg.value, poolsPath, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "BSRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (IERC20 input, IERC20 output) = (path[i], path[i + 1]);
            (IERC20 token0, ) = BSLibrary.sortTokens(input, output);
            IBSPool pool = IBSPool(poolsPath[i]);
            uint256 amountOutput;
            {
                ...
                (
                    ...
                    uint256 vReserveOut,
                    uint256 feeInPrecision
                ) = BSLibrary.getTradeInfo(poolsPath[i], input, output);
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(amountOut >= amountOutMin, "BSRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function getAmountsOut(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return BSLibrary.getAmountsOut(amountIn, poolsPath, path);
    }
    ...
    function getAmountsIn(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return BSLibrary.getAmountsIn(amountOut, poolsPath, path);
    }

    function verifyPoolsPathSwap(address[] memory poolsPath, IERC20[] memory path) internal view {
        require(path.length >= 2, "BSRouter: INVALID_PATH");
        require(poolsPath.length == path.length - 1, "BSRouter: INVALID_POOLS_PATH");
    ...
    function verifyPoolAddress(
        ...
        require(IBSFactory(factory).isPool(tokenA, tokenB, pool), "BSRouter: INVALID_POOL");
    }
}