...
// File: fc108/token/ERC20/IERC20.sol
...
// File: fc108/token/ERC20/FFCERC20.sol
...
contract FFCERC20 is Context, Ownable, IERC20 {
    ...
    bool public isOnlyWhitelistBuy = true;
    bool public isLimitTokenAmount = true;
    uint256 public limitTokenAmount = 200 * 10**18;
    ...
    constructor (address mineAddress_, address lpAddress_, address marketAddress_) public {
        _name = "Fans Club";
        _symbol = "FC";
        _decimals = 18;
        ...
        _isExcludedFromFee[mineAddress_] = true;
        _isExcludedFromFee[lpAddress_] = true;
        _isExcludedFromFee[marketAddress_] = true;
        _marketAddress = marketAddress_;

        _mint(mineAddress_, 1000000 * 10**18);
        _mint(lpAddress_, 80000 * 10**18);
        
        pancakeRouter = IPancakeRouter(_router);
        pair = IPancakeFactory(pancakeRouter.factory()).createPair(address(this), pancakeRouter.WETH());
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (to == pair && !isExcludedFromFee(from)) { //sell, add
            _takeFee(from, _marketAddress, amount.div(100).mul(2));  //marketing
            _takeFee(from, address(0), amount.div(100).mul(5));      //burn 
            amount = amount.div(100).mul(93);
        }   

        if (from == pair) { //buy, sub
            require(!isOnlyWhitelistBuy || isExcludedFromFee(to), "only whitelist buy");
            
            if (isLimitTokenAmount && !isExcludedFromFee(to)) {
                uint256 balance = balanceOf(to);
                require(balance+amount <= limitTokenAmount, "limit token amount");
            } 
        }
    ...
    function setIsOnlyWhitelistBuy(bool isOnlyWhitelistBuy_) public onlyOwner {
        isOnlyWhitelistBuy = isOnlyWhitelistBuy_;
    }

    function setIsLimitTokenAmount(bool isLimitTokenAmount_) public onlyOwner {
        isLimitTokenAmount = isLimitTokenAmount_;
    }

    function setLimitTokenAmount(uint256 limitTokenAmount_) public onlyOwner {
        limitTokenAmount = limitTokenAmount_ * 10**18;
    }
