...
contract WordlCupMiner {
    ...
    uint256 public GOALS_TO_HIRE_1MINERS = 972000; // 8%
    uint256 public GOALS_BUSD = 100; // ammount of goals for 1 busd
    uint256 public goalsValue = GOALS_TO_HIRE_1MINERS * GOALS_BUSD; //GOALS_TO_HIRE_1MINERS * GOALS_BUSD
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public FEE = 60;   // 6%
    uint256 public BONUS = 50; //5%
    ...
    uint256 public max_pool_balance = 10000 ether; /** 10,000 BUSD  **/
     ...
     uint256 public marketGoals = 259200000000;
     uint256 PSN = 10000;
     uint256 PSNH = 5000;
    ...
    function distributeWinnerRewards(string memory winCountry) public nonReentrant {
        ...
        for(uint8 i = 0; i < addresses.length; i++) {
            ...
            if(compareStrings(user.selectedWorldCupWinner, winCountry)){
              uint256 initialGoalsBought = calculateGoalsBuy(user.initialDeposit);  //get initial goals
              uint256 initialMiners = initialGoalsBought.div(GOALS_TO_HIRE_1MINERS);
              user.miners += (initialMiners); // + 100%
    ...
    function distributeDailyRewards(string memory winMatchCountry) public nonReentrant {
        ...
        for(uint8 i = 0; i < addresses.length; i++) {
            ...
            if(compareStrings(user.selectedWorldCupWinner, winMatchCountry)){
              uint256 initialGoalsBought = calculateGoalsBuy(user.initialDeposit);  //get initial goals
              uint256 initialMiners = initialGoalsBought.div(GOALS_TO_HIRE_1MINERS);
              user.miners += ((initialMiners * BONUS) / PERCENTS_DIVIDER); //  +5%
             }
        }
    ...
    function VAR_GOALS_BUSD(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 100 && value <= 1000); /** min 100 max 1000 **/
        GOALS_BUSD = value;
        goalsValue = GOALS_TO_HIRE_1MINERS * GOALS_BUSD;
    }
