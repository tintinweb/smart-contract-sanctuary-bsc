...
contract CATKING is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   DividendTracker public dividendTracker;
    ...
    uint256 private _buyKusFee;

    uint256 private _sellRewardsFee;
    uint256 private _sellLiquidityFee;
	uint256 private _sellMarketingFee;
    uint256 private _sellKusFee;
    ...
    event KusWalletUpdated(address indexed newKusWallet, address indexed oldKusWallet);
    ...
    constructor() ERC20 ("CatKing", "$CKING") {
        _buyRewardsFee = 3;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 6;
        _buyKusFee = 1;
        ...
        _maxWallet = 10000000000000 * (10**18);
        _maxBuy = 10000000000000 * (10**18);
        _maxSell = 10000000000000 * (10**18);

        totalBuyFees = _buyRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee).add(_buyKusFee);
        totalSellFees = _sellRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellKusFee);

    	dividendTracker = new DividendTracker();

    	liquidityWallet = owner();
		marketingWallet = payable(0x9BE51c4c910d5b31d9E4f9fC4841F90cd51F0372);
        kusWallet = payable(0x8C2B94ff8A8186608D02230c76a05e77f2b91418);
        ...
        excludeFromFees(kusWallet, true);
    ...
    function updatedividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "CatKing The dividend tracker already has that address");

        DividendTracker newdividendTracker = DividendTracker(payable(newAddress));

        require(newdividendTracker.owner() == address(this), "CatKing: The new dividend tracker must be owned by the CatKing token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "CatKing: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "CatKing: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "CatKing: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "CatKing: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CatKing: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "CatKing: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "CatKing: The marketing wallet is already this address");
        excludeFromFees(newMarketingWallet, true);
        emit MarketingWalletUpdated(newMarketingWallet, marketingWallet);
        marketingWallet = newMarketingWallet;
    }

     function updateKusWallet(address newKusWallet) public onlyOwner {
        require(newKusWallet != kusWallet, "CatKing: The kus wallet is already this address");
        excludeFromFees(newKusWallet, true);
        emit KusWalletUpdated(newKusWallet, kusWallet);
        kusWallet = newKusWallet;
    }
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "CatKing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "CatKing: Cannot update gasForProcessing to same value");
    ...
    function updateIsElon(address account, bool elon) public onlyOwner {
        require(_isElon[account] != elon, "CatKing: Account is already the value of 'elon'");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
            ...
            uint256 kusAmount = address(this).balance.mul(_sellKusFee).div(swapAmount);
            transferToWallet(payable(kusWallet), kusAmount);

            emit Transfer(address(this), kusWallet, address(this).balance.mul(_sellKusFee).div(swapAmount));
            }
	...
	function letsGetStarted() external onlyOwner() {
        updateBuyFees(2,5,3,2);
        updateSellFees(2,5,3,2);
    ...
    function updateBuyFees(uint8 newBuyLiquidityFee, uint8 newBuyMarketingFee, uint8 newBuyRewardsFee, uint8 newBuyKusFee) public onlyOwner {
        _buyLiquidityFee = newBuyLiquidityFee;
        _buyMarketingFee = newBuyMarketingFee;
        _buyRewardsFee = newBuyRewardsFee;
        _buyKusFee = newBuyKusFee;
        
        totalFees();
    }

    function updateSellFees(uint8 newSellLiquidityFee, uint8 newSellMarketingFee, uint8 newSellRewardsFee, uint8 newSellKusFee) public onlyOwner {
        _sellLiquidityFee = newSellLiquidityFee;
        _sellMarketingFee = newSellMarketingFee;
        _sellRewardsFee = newSellRewardsFee;
        _sellKusFee = newSellKusFee;
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("Dividend_Tracker", "Dividend_Tracker", DividendToken) {
        ...
        isAuth[owner()] = true;
        isAuth[address(0x6cd1EFa71f74a53E9497B015Ebe37949A4622642)];
    }
