...
require(_BUSD.allowance(msg.sender, address(this)) >= price.mul(amount), "Error : BUSD Not Approved !");
safeTransferFrom(_BUSD, msg.sender, address(this), price.mul(amount));
...
_orderIdToDetail[orderID]['TOTAL'] = price.mul(amount);
return true;
}
...
_BUSD.transfer(_orderIdToOwner[orderID], _orderIdToDetail[orderID]['PRICE'].mul(_orderIdToDetail[orderID]['AMOUNT']));
...
require(_orderIdToDetail[orderID]['AMOUNT'] > 0, "Error : Invalid Amount !");
require(keccak256(abi.encodePacked(_orderIdToType[orderID])) == keccak256(abi.encodePacked("SELL")), "Error : Invalid Type !");
require(_BUSD.allowance(msg.sender, address(this)) >= _orderIdToDetail[orderID]['PRICE'].mul(_orderIdToDetail[orderID]['AMOUNT']), "Error : BUSD Not Approved !");
safeTransferFrom(_BUSD, msg.sender, address(this), _orderIdToDetail[orderID]['PRICE'].mul(_orderIdToDetail[orderID]['AMOUNT']));
_BUSD.transfer(_orderIdToOwner[orderID], _orderIdToDetail[orderID]['PRICE'].mul(_orderIdToDetail[orderID]['AMOUNT']));
...
_ETPOS.transfer(_orderIdToOwner[orderID], _orderIdToDetail[orderID]['AMOUNT']);
_BUSD.transfer(msg.sender, _orderIdToDetail[orderID]['PRICE'].mul(_orderIdToDetail[orderID]['AMOUNT']));
_orderIdToDetail[orderID]['AMOUNT'] = 0;
_lastPrice = _orderIdToDetail[orderID]['PRICE'];
