...
// File contracts/pools/ALU/Common/StrategyManagerNative.sol

pragma solidity 0.8.14;
contract StrategyManagerNative is Ownable, Pausable {
  /**
   * @dev Beefy Contracts:
   ...
   */
  function setPlutusFeeRecipient(address _plutusFeeRecipient)
    external
    onlyOwner
  {
    plutusFeeRecipient = _plutusFeeRecipient;
...
// File contracts/pools/ALU/Common/FeeManagerNative.sol

pragma solidity 0.8.14;
abstract contract FeeManagerNative is StrategyManagerNative {
...
// File contracts/pools/ALU/StrategyNative.sol
...
contract StrategyNative is StrategyManagerNative, FeeManagerNative {
  ...
  constructor(
    ...
    public
    StrategyManagerNative(_keeper, _unirouter, _vault, _plutusFeeRecipient)
  {
    poolId = _poolId;
    wrapped = _wrapped;
    want = _want;
    masterchef = _chef;
    wantToWrappedRoute = [_want, _wrapped];
    _giveAllowances();
  }
  ...
  function chargeFees() internal {
    uint256 toWrapped = (IERC20(want).balanceOf(address(this)) * 10) / 1000;
    ...
    uint256 beefyFeeAmount = (wrappedBal * beefyFee) / MAX_FEE;
