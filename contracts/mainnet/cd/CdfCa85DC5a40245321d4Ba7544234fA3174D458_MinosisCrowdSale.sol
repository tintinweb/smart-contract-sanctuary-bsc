...
abstract contract Crowdsale is Initializable {
   ...
   * @param _token Address of the token being sold
   */
  function __Crowdsale_init_unchained(uint256 _rate, address _wallet, IERC20Upgradeable _token) internal {
    require(_rate > 0, "Rate cant be 0");
    require(_wallet != address(0), "Address cant be zero address");
...
abstract contract TimedCrowdsale is Crowdsale {
   ...
   * @param _closingTime Crowdsale closing time
   */
  function __TimedCrowdsale_init_unchained(uint256 _openingTime, uint256 _closingTime) internal {
    // solium-disable-next-line security/no-block-members
    require(_openingTime >= block.timestamp, "OpeningTime must be greater than current timestamp");
    require(_closingTime >= _openingTime, "Closing time cant be before opening time");
...
contract MinosisCrowdSale is Crowdsale, PausableUpgradeable, FinalizableCrowdsale, UUPSUpgradeable {
    ...
    function initialize(uint256 rate, address payable wallet, IERC20Upgradeable token, uint256 openingTime,  uint256 closingTime) public initializer {
      __TimedCrowdsale_init_unchained(openingTime, closingTime);
      __Crowdsale_init_unchained(rate, wallet, token);
