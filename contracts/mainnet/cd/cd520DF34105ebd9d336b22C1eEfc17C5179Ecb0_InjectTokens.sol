...
// File contracts/InjectTokens.sol


pragma solidity ^0.8.0;
contract InjectTokens
{
    ...
    event DistributePLC(address account, uint256 amount, uint256 amountBurn, address[] receivers, uint256[] amounts, string orderId, string gameId, uint256 timestamp);

    constructor(address _plcToken, address[] memory _plcReceiveAddrs, uint256[] memory _plcPercents) {
    ...
    /// plcToken percents
    function updatePlcReceiveAddrs(address[] memory receiveAddrs) external onlyAdmin {
        require(receiveAddrs.length == plcPercents.length, "length error");
        plcReceiveAddrs = receiveAddrs;
    }
    function updatePlcPercents(uint256[] memory percents) external onlyAdmin {
    ...
    function injectPLC(uint256 amount, string memory orderId, string memory gameId) external {
        require(PLCToken(plcToken).balanceOf(msg.sender) >= amount, "no enough plc");
        
        uint256[] memory amounts = new uint256[](plcPercents.length);
        for (uint256 j = 0; j < plcPercents.length; ++j) {
            amounts[j] = amount*plcPercents[j]/100;
        }

        PLCToken(plcToken).transferBatch(plcReceiveAddrs, amounts, msg.sender);
        PLCToken(plcToken).burnByUser(amount*plcPercentBurn/100, msg.sender);

        emit DistributePLC(msg.sender, amount, amount*plcPercentBurn/100, plcReceiveAddrs, amounts, orderId, gameId, block.timestamp);
    }

    function collectPLC() external onlyAdmin {
        if (PLCToken(plcToken).balanceOf(address(this)) > 0) {
            PLCToken(plcToken).transfer(adminAddress, PLCToken(plcToken).balanceOf(address(this)));
        }
    }
}