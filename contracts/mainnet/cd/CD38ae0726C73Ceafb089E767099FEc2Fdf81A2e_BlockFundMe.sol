...
contract BlockFundMe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    BFMDividendTracker public dividendTracker;
    ...
    uint256 public _buyUSDTRewardsFee = 4;
    uint256 public _buyLiquidityFee = 1;
    uint256 public _buyMarketingFee = 4;
    uint256 public _buyBurnFee = 1;

    uint256 public _buyTotalFees = _buyUSDTRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee);
    
    uint256 public _sellUSDTRewardsFee = 5;
    uint256 public _sellLiquidityFee = 2;
    uint256 public _sellMarketingFee = 4;
    uint256 public _sellBurnFee = 1;
    uint256 public _sellTotalFees = _sellUSDTRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee);
    uint256 public _paneltyFee = 40;
    ...
    constructor() ERC20("BlockFundMe", "BFM") {
    ...
    function removeToWhitelist(address _add) external onlyOwner
    {
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BFM: The dividend tracker already has that address");

        BFMDividendTracker newDividendTracker = BFMDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BFM: The new dividend tracker must be owned by the BFM token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BFM: The router already has that address");
    ...
    function excludeOrIncludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BFM: Account is already the value of 'excluded'");
    ...
    function excludeOrIncludeFromMaxTx(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTx[account] != excluded, "BFM: Account is already the value of 'excluded'");
    ...
    function excludeOrIncludeFromMaxWallet(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxWallet[account] != excluded, "BFM: Account is already the value of 'excluded'");
    ...
    function setBuyFees(uint256 buyUSDTRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyBurnFee) external onlyOwner {
        _buyUSDTRewardsFee = buyUSDTRewardFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyMarketingFee = buyMarketingFee;
        _buyBurnFee = buyBurnFee;
        _buyTotalFees = _buyUSDTRewardsFee.add(_buyLiquidityFee).add(_buyMarketingFee);
        require(_buyTotalFees >= 0 && _buyTotalFees <= 20, "Fees are exceeding the limits");//question to client
    }

    function setSellFees(uint256 sellUSDTRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellBurnFee) external onlyOwner {
        _sellUSDTRewardsFee = sellUSDTRewardFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellMarketingFee = sellMarketingFee;
        _sellBurnFee = sellBurnFee;
        _sellTotalFees = _sellUSDTRewardsFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellBurnFee);
        require(_sellTotalFees >= 0 && _sellTotalFees <= 20, "Fees are exceeding the limits");
    }
      function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BFM: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "BFM: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "BFM: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        {           
            ...
            uint256 walletTokens = contractTokenBalance.mul(_sellMarketingFee).div(_sellTotalFees);
            uint256 contractBalance = address(this).balance;
            swapTokensForBnb(walletTokens);
        ...
        if(takeFee) {
           if(automatedMarketMakerPairs[from]) {
                ...
                burnFees = amount.mul(_buyBurnFee).div(100);
           } else {
    ...
    function manualSwapTokensForBnb(uint256 tokenAmount) public onlyOwner {
...
contract BFMDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BFM_Dividen_Tracker", "BFM_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BFM_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BFM_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BFM contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BFM_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BFM_Dividend_Tracker: Cannot update claimWait to same value");
