...
/*  CYFtoken - Experimental Community Owned MeMeCoin.
    ...
    */
    ...
    contract CYFSmartMinter is ReentrancyGuard, PriceConsumerV3 {
        ...
        CYFpreminterclaims immutable Preminter2 = CYFpreminterclaims(0x2254FFa60a3414123071Ece3FAC9E6fF282Ddf0d);
        ...
        function mintCYFT() public {
            ...
            require(!address(_user).isContract(), 'NotAllowed');
            // require(users[_user].minter.pclaimed > 0, 'NothingToMint');
            ...
            // Verify if User has preminter v1
            ...
            if(_allocation > 0){
                ...
                // Verify if User has preminter v2
                (, , , , , , , uint256 _allocation2, uint256 _dateClaimed2) = Preminter2.minters(_user);
                // Withdrawals are allowed only once per 12hrs
                if(_allocation == 0 && _allocation2 > minterClaimed){
                    uint256 _amountToClaim = _allocation2 - minterClaimed;
                    uint256 _mintAmountP = getMintAmount(_amountToClaim, _dateClaimed2);
                    tokenSold += _amountToClaim;
                    _mintAmount += _mintAmountP;
                    users[_user].minter.preminter += _amountToClaim;
                    users[_user].minter.pclaimed += _amountToClaim;
