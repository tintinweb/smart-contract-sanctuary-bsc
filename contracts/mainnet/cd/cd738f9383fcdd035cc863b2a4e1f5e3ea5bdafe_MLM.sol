...
// Telegram  : https://t.me/messiahlaunch
...
contract MLM is IBEP20, Ownable {

    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "Messiah Launch Moon"; // 
    string constant _symbol = "MLM"; // 
    ...
    uint256 public pepsienergyFee;
    uint256 public totalFee;

    uint256 public BuyliquidityFee    = 0;
    uint256 public BuycharityFee   = 0;
    uint256 public BuymarketingFee    = 0;
    uint256 public BuypepsienergyFee      = 0;
    uint256 public BuytotalFee        = BuyliquidityFee + BuycharityFee + BuymarketingFee + BuypepsienergyFee;

    uint256 public SellliquidityFee    = 0;
    uint256 public SellcharityFee   = 0;
    uint256 public SellmarketingFee    = 0; 
    uint256 public SellpepsienergyFee      = 0;
    uint256 public SelltotalFee        = SellliquidityFee + SellcharityFee + SellmarketingFee + SellpepsienergyFee;

    // Max wallet & Transaction
    uint256 public _maxBuyTxAmount = _totalSupply / (100) * (30); // 30%
    uint256 public _maxSellTxAmount = _totalSupply / (100) * (30); // 30%
    uint256 public _maxWalletToken = _totalSupply / (100) * (30); // 30%

    // Fees receivers
    address public autoLiquidityReceiver = address(this);
    address public marketingFeeReceiver = 0x851682F7ac336Da36214398F61eabAb0222F9f1E;
    address public devFeeReceiver = 0x851682F7ac336Da36214398F61eabAb0222F9f1E;
    address public pepsienergyFeeReceiver = 0x851682F7ac336Da36214398F61eabAb0222F9f1E;
    ...
    function buyFees() internal{
        ...
        pepsienergyFee      = BuypepsienergyFee;
    ...
    function sellFees() internal{
        ...
        pepsienergyFee      = SellpepsienergyFee;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBpepsienergy = amountBNB * (pepsienergyFee) / (totalBNBFee);
        uint256 amountBNBMarketing = amountBNB * (marketingFee) / (totalBNBFee);
        uint256 amountBNBDev = amountBNB - amountBNBLiquidity - amountBNBpepsienergy - amountBNBMarketing;

        (bool MarketingSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        require(MarketingSuccess, "receiver rejected ETH transfer");
        (bool pepsienergySuccess,) = payable(pepsienergyFeeReceiver).call{value: amountBNBpepsienergy, gas: 30000}("");
        require(pepsienergySuccess, "receiver rejected ETH transfer");
    ...
    // Only Owner allowed
    function setBuyFees(uint256 _liquidityFee, uint256 _pepsienergyFee, uint256 _marketingFee, uint256 _devFee) external onlyOwner {
        BuyliquidityFee = _liquidityFee;
        BuypepsienergyFee = _pepsienergyFee;
        BuymarketingFee = _marketingFee;
        BuycharityFee = _devFee;
        BuytotalFee = _liquidityFee + (_pepsienergyFee) + (_marketingFee) + (_devFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _pepsienergyFee, uint256 _marketingFee, uint256 _devFee) external onlyOwner {
        SellliquidityFee = _liquidityFee;
        SellpepsienergyFee = _pepsienergyFee;
        SellmarketingFee = _marketingFee;
        SellcharityFee = _devFee;
        SelltotalFee = _liquidityFee + (_pepsienergyFee) + (_marketingFee) + (_devFee);
    }
    
    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _pepsienergyFeeReceiver, address _devFeeReceiver) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        pepsienergyFeeReceiver = _pepsienergyFeeReceiver;
