...
contract Coin is ERC20,Ownable{
    ...
    uint public decimalVal = 1e18;
     ...
     address public nftAddr;//NFT地址

     address public lpAddr;//lp地址
     uint base = 1000;
     uint marketPropor = 10;//社区治理
     uint nftPropor = 20;//NFT分红
   
     uint lpPropor = 10;//LP分红
    ...
    constructor (address addr_)  ERC20("SAF", "SAF") {
        _mint(addr_, 21000*decimalVal);
        isFeeAddress[addr_] = true;
        isFeeAddress[router] = true;
        isFeeAddress[address(this)] = true;
    }
    
    function setAddr(address marketAddr_,address nftAddr_,address lpAddr_) public onlyOwner {
    ...
    function common(address addr,address to,uint value) internal returns (uint){
        uint transferAmount = value;
           if(!isFeeAddress[addr] && !isFeeAddress[to]){
            ...
            uint amount = nftAmount.add(lpAmount);
