...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            bool success = IERC20(usdtAddress).transfer(user, _withdrawableDividend);
            if(!success) {
...
contract RichCoinDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(address _usdtAddress) DividendPayingToken("RichCoinDividendTracker", "RichCoin_DividendTracker") {
        claimWait = 3600;
    ...
    function withdrawDividend() public pure override {
        require(false, "RichCoinDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main richCoin contract.");
    }
...
contract YlcToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public deflationFee = 2;
    uint256 public sellFee = 13;
    uint256 public buyFee = 12;

    uint256 public buybackFee = 12;   
    uint256 public burnFee = 12;
    uint256 public marketingFee = 20;   
    uint256 public tokenDividendFee = 8; 
    uint256 public stakeDividendFee = 48;   
                    
    uint256 public maxSupply = 10 ** 11 * (10 ** decimals());

    uint256 public deflationLimit  = 10 ** 10 * (10 ** decimals());
    ...
    constructor( 
        ...
        dividendTracker = new RichCoinDividendTracker(usdtAddress);
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "YlcToken: The router already has that address");
    ...
    function setBuyFees(
        ...
        uint256 _tokenDividendFee, 
        uint256 _stakeDividendFee, 
        uint256 _burnFee
    )  external onlyOwner {
        buybackFee = _buybackFee;
        marketingFee = _marketingFee;
        tokenDividendFee = _tokenDividendFee;
        stakeDividendFee = _stakeDividendFee;
        burnFee = _burnFee;
    }

    function setConfigAddress(address _marketingWallet, address _stakeAddress) external onlyOwner {
        marketingWallet = _marketingWallet;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "YlcToken: The dividend tracker already has that address");

        RichCoinDividendTracker newDividendTracker = RichCoinDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "YlcToken: The new dividend tracker must be owned by the token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "YlcToken: Account is already the value of 'excluded'");
    ...
    function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromMaxTxLimit[account] != excluded, "YlcToken: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "YlcToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "YlcToken: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if(!tradingEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "YlcToken: This account cannot send tokens until trading is enabled");
        }
        ...
        if(takeFee) {
            if(to == pancakePair || from == pancakePair) {
                uint256 totalFees = 0;
                if(to == pancakePair) {
                    totalFees = amount.mul(sellFee).div(100);
                } else if(from == pancakePair) {
                    totalFees = amount.mul(buyFee).div(100);
                }
                amount = amount.sub(totalFees);
                super._transfer(from, address(this), totalFees);
            } else {
                uint256 deflationAmount = (maxSupply * (10 ** decimals())).sub(balanceOf(address(burnAddress)));
                if(deflationAmount > deflationLimit) {
                    uint256 burnAmount = amount.mul(deflationFee).div(100);
    ...
    function swapAndDistribute(uint256 tokens) private {
        ...
        forBurn = tokens.mul(burnFee).div(100);
        super._transfer(address(this), burnAddress, forBurn);
        
        uint256 remainAmount = tokens.sub(forBuyback.add(forBurn));
        ...
        uint256 perAmount = swapRemainAmount.div(marketingFee.add(tokenDividendFee).add(stakeDividendFee));
        ...
        uint256 swapTokenDividendsAmount = perAmount.mul(tokenDividendFee);
        if(swapTokenDividendsAmount > 0) {
            IERC20(usdtAddress).approve(address(dividendTracker), swapTokenDividendsAmount);
            dividendTracker.distributeDividends(swapTokenDividendsAmount);
            emit SwapAndSendTo(swapTokenDividendsAmount, "TOKENDIVIDENDS");
        }
        uint256 swapStakeDividendsAmount = swapRemainAmount.sub(swapMarketAmount.add(swapTokenDividendsAmount));
        if(swapStakeDividendsAmount > 0) {
            emit SwapAndSendTo(swapStakeDividendsAmount, "STAKEDIVIDENDS");
            //to noti
            IERC20(usdtAddress).approve(address(this), swapStakeDividendsAmount);
            IERC20(usdtAddress).transferFrom(address(this), stakeAddress, swapStakeDividendsAmount);
            IStakePool(stakeAddress).notifyRewardAmount(swapStakeDividendsAmount);
        }
    }
