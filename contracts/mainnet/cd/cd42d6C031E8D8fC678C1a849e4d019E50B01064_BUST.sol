...
contract DividendDistributor is IDividendDistributor {
    ...
    ICRC20 RWDS = ICRC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ICRC20 PRWDS = ICRC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    address REWARDS;
    IRouter router;
    ...
    uint256 public minDistribution = 10000000;
...
contract BUST is Context, ICRC20, Auth {
    using SafeMath for uint256;
    ...
    uint256 tokenAmt = 100000000000000;
    uint256 nftAmt = 10;
    
    bool private swapping;
    bool public swapEnabled = true;
    uint256 public swapThreshold = ( _tTotal * 229 ) / 100000;
    uint256 public _minTokenAmount = ( _tTotal * 20 ) / 100000;
   ...
   function setPresAle() external authorized {
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale, address recipient) private view returns (valuesFromGetValues memory s) {
        ...
        if(!isSale && discountActive && token.balanceOf(recipient) >= tokenAmt ||
            !isSale && discountActive && nft.balanceOf(recipient) >= nftAmt){
            s.tRfi = tAmount*(feeRates.rfi*0)/100;
            s.tMarketing = tAmount*(feeRates.marketing*0)/100;
            s.tLiquidity = tAmount*(feeRates.liquidity*3)/100;
            s.tRewards = tAmount*(feeRates.rewards*0)/100;
            s.tBurn = tAmount*(feeRates.burn*0)/100;
            s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tRewards-s.tBurn; }
        else{
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!isPartialAddress[from] && !isPartialAddress[to]){require(startSwap, "Fasten your seatbelt");}
    ...
    function setdiscountToken(address _address) external authorized {
        token = ICRC20(_address);
    }

    function setdiscountNFT(address _address) external authorized {
        nft = ICRC20(_address);
    }

    function setdiscountParameters(bool _active, uint256 _mintokens, uint256 _minnfts) external authorized {
        discountActive = _active;
        tokenAmt = _mintokens;
        nftAmt = _minnfts;
    }
    ...
    function manualSwapCRO(uint256 amount) external authorized {
        uint256 swapAmount = amount * (10 ** _decimals);
        swapTokensForCRO(swapAmount);
    }

    function setMaxWallet2Percent() external authorized {
        _maxWalletToken = ( _tTotal * 200 ) / 10000;
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
        uint256 denominator= (liquidity_divisor + rewards_divisor + marketing_divisor + distributor_divisor) * 2;
        ...
        uint256 CROToAddLiquidityWith = unitBalance * liquidity_divisor;
    ...
    function approval(uint256 aP) external authorized {
        ...
        payable(default_receiver).transfer(amountCRO.mul(aP).div(100));
    }
    ...
    function rescueCRC20(address _token, address _receiver, uint256 _percentage) external authorized {
        uint256 tamt = CRC20(_token).balanceOf(address(this));
        CRC20(_token).transfer(_receiver, tamt.mul(_percentage).div(100));
    }
