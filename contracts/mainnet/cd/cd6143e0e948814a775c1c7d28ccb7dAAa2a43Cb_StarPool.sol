...
contract StarPool is TokenFreed {
    ...
    function onERC721ExReceived(
        ...
        for (uint256 index = 0; index != tokenIds.length; index++) {
            require(card._cardType(tokenIds[index]) == 3, "not type");
            nftOwner[from].push(tokenIds[index]);
            nftIndex[tokenIds[index]] = nftOwner[from].length;
        }
    ...
    function hanleWithdraw(uint256 _index) internal {
        uint256 cardIndex = nftOwner[msg.sender].length - 1;
        uint256 newCard = nftOwner[msg.sender][cardIndex];
        nftOwner[msg.sender][_index] = newCard;
        nftIndex[newCard] = _index + 1;
        nftOwner[msg.sender].pop();
    }

    function withdraw(uint256[] memory cardId)
        public
        updateReward(msg.sender)
        checkhalve
        checkStart
    {
        require(nftOwner[msg.sender].length > 0, "not amount");
        uint256 total = cardId.length;
        require(total > 0, "not length");
        uint256[] memory returnCard = new uint256[](total);
        for (uint256 index = 0; index != total; index++) {
            require(cardId[index] > 0, "index > 0");
            uint256 returnCardIndex = nftIndex[cardId[index]];
            require(returnCardIndex > 0, "cardIndex > 0");
            hanleWithdraw(returnCardIndex - 1);
            nftIndex[cardId[index]] = 0;
        }

        uint256 amount = returnCard.length;
        require(amount != 0, "nft does not exist");

        card.safeBatchTransferFrom(address(this), msg.sender, cardId);
    ...
    function getReward() public updateReward(msg.sender) checkhalve checkStart {
        ...
        freed(msg.sender, reward.mul(70).div(100));
