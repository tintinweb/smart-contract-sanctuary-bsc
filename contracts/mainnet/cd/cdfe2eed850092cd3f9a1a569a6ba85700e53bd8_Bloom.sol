...
contract Bloom is IERC20 {
    ...
    string constant private _name = "Bloom";
    string constant private _symbol = "BLM";
    ...
    struct TaxWallets {
        ...
        address payable BFund;
        address payable LP;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x331E3d7C80322B3b467a28E523D4d4934FbfB2D5),
        BFund: payable(0x9A495A0B910415b77A492ba984039746Be536Fe5),
        LP: payable(0xEffBe115F9019DF69D92E4aF55b2bE79764803E0)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 marketing, uint16 BFund, uint16 LP) external onlyOwner {
        _ratios.marketing = marketing;
        _ratios.BFund = BFund;
        _ratios.LP = LP;
        _ratios.totalSwap = marketing + BFund + LP;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable BFund, address payable LP) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.BFund = payable(BFund);
        _taxWallets.LP = payable(LP);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        bool success;
        uint256 BFundBalance = (amtBalance * ratios.BFund) / ratios.totalSwap;
        uint256 LPBalance = (amtBalance * ratios.LP) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (BFundBalance + LPBalance);
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 35000}("");
        }
        if (ratios.BFund > 0) {
            (success,) = _taxWallets.BFund.call{value: BFundBalance, gas: 35000}("");
        }
        if (ratios.LP > 0) {
            (success,) = _taxWallets.LP.call{value: LPBalance, gas: 35000}("");
        }
    }
