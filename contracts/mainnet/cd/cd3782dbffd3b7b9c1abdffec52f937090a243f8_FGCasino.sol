...
// Dice Roll
    ...
    function blackJackUser(address _user) external view returns (
        uint256[] memory _playerCardsSuit,
        uint256[] memory _playerCardsNumber,
        uint256[] memory _houseCardsSuit,
        uint256[] memory _houseCardsNumber,
        uint256 _currentBet,
        uint256 _total,
        uint256 _Dtotal,
        bool _gameStarted
        ...
        BlackJack storage user = blackJack[_user];
         _playerCardsSuit = new uint256[](user.playerCardsSuit.length);
        _playerCardsSuit = user.playerCardsSuit;
         _playerCardsNumber = new uint256[](user.playerCardsSuit.length);
        _playerCardsNumber = user.playerCardsNumber;
         _houseCardsSuit = new uint256[](user.houseCardsSuit.length);
        _houseCardsSuit = user.houseCardsSuit;
         _houseCardsNumber = new uint256[](user.playerCardsSuit.length);
         _houseCardsNumber = user.houseCardsNumber;
    ...
    function resetBlackJack() internal {
        BlackJack storage user = blackJack[msg.sender];
            bool[] memory cards = new bool[](52);
            user.cards = cards;
            uint256[] memory reset = new uint256[](0);
            user.playerCardsSuit = reset;
            user.playerCardsNumber = reset;
            user.houseCardsNumber = reset;
            user.houseCardsSuit = reset;
            user.currentBet = 0;
            user.total = 0;
            user.Dtotal = 0;
            user.gameStarted = false;
    ...
    function blackJackHitMe() external pausable  {
       BlackJack storage user = blackJack[msg.sender];
        ...
        emit BlackJackGame(msg.sender, user.playerCardsSuit ,user.playerCardsNumber, user.houseCardsSuit, user.houseCardsNumber, currentBet, total, user.Dtotal, bust, false);
