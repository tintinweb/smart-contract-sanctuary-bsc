...
contract WBLOTTERY {
    ...
    constructor() {
        ...
        nonce = 0;
        token = Itoken(0xD04A8f36a59A4Ec8C3f362e27Eb9620CFf8F9247);
        duration = 1 hours;
        ...
        if (lotteries[nonce].participants.length > 0) {
            for (
                ...
                token.transfer(
                    ...
                    (lotteries[nonce].winner_reward[i] *
                        withdraw_refferal_percentage) / divider
                );
    ...
    function set_withdraw_refferal_percentage(
        uint256 _withdraw_refferal_percentage
    ) external onlyAdmin {
