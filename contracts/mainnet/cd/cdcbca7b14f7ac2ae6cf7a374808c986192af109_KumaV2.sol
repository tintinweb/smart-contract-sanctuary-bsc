...
contract KumaV2 is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // Lists
    uint256[2500] public remainingIds;
    mapping(uint256 => uint256) public lastDividendAt;
    mapping(uint256 => address) public minters;

    uint256 public balanceToBurn; 
    ...
    address public RewardToken = 0x8ab7Ef0eB25AAd36DfF0661f81Fb81B144AF5b87;
    ...
    function mint(uint256 _mintAmount) public payable {
        // Checks
        require(!paused, "KumaV2: minting has not started yet");
        require(_mintAmount > 0, "KumaV2: you have to mint at least one");
        require(
            _mintAmount <= maxMintAmount,
            "KumaV2: you can only mint 10 at a time"
        );
        require(
            lastSupply >= _mintAmount,
            "KumaV2: the collection is sold out"
        );
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _mintAmount,
                "KumaV2: total cost isn't match"
            );
        }
    ...
    {
        // Checks
        require(_winner != address(0), "KumaV2: zero address");
        require(_amount > 0, "KumaV2: you have to mint at least one");
        require(lastSupply != 0, "KumaV2: the collection is sold out");
    ...
    function tokenURI(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), "KumaV2: URI query for nonexistent token");
    ...
    function handleMintReward(uint256 _amount) private {
        ...
        uint256 burnShare = _amount.sub(mintShare);
        // Save dividend for community
        reflectDividend(mintShare);
        // Send owner share
        balanceToBurn += burnShare;
    } 
    ...
    function triggerBurn() public {
        ...
        uint256 amount = balanceToBurn;
        balanceToBurn = 0;
        buyTokenAndBurn(amount);
    }

    function buyTokenAndBurn(uint amount) internal {
        // Buy Token
