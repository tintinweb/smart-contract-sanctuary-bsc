...
contract Minter is AccessControl, IMinter {
    ...
    OneNFT private nftContract;
    sLogReporting private logger;

    constructor(address _minter, address _contractAddr) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(MINTER_ROLE, _minter);
        logger = sLogReporting('Minter');
        nftContract = OneNFT(_contractAddr);
    }
    ...
    function mint(address to, uint256[] memory tokenIds) public _onlyMinter {
        require(to != address(0), logger.reportError('Address must be different than 0x'));
        for (uint256 i = 0; i < tokenIds.length; i++) {
            nftContract.mint(to, tokenIds[i]);
        }
    }
}
...
interface IMinter {
    /**
     * @dev Mint as many tokens passed in `tokenIds`
     *
     * Requirements:
     *
