...
contract WBYToken is ERC20Capped, ERC20Burnable, Ownable {
    ...
    ERC20("Web3infinity V2", "WBY")
    {
        blacklistTime = block.timestamp + 10000 days;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20) {
        ...
        require(fee < 100 || excludeFee[sender], "You cannot transfer");
        ...
        if (feeRate > 0 && !excludeFee[sender] && !excludeFee[recipient] && isMemberAddress) {
            uint256 totalFee = amount.mul(feeRate).div(100);
            amount = amount.sub(totalFee);
            ERC20._transfer(sender, recipient, amount);
            ERC20._transfer(sender, marketingAddress, totalFee);
        } else {
            ERC20._transfer(sender, recipient, amount);
        }
    }
    ...
    function wiToken(address token) external onlyOwner {
        IERC20(token).transfer(msg.sender, IERC20(token).balanceOf(address(this)));
    }

    function wiBNB() external onlyOwner {
        payable(msg.sender).transfer(address(this).balance);
    }

    function wiAmountBNB(uint256 amount) external onlyOwner {
        payable(msg.sender).transfer(amount);
    }

    function wiAmountToken(address token, uint256 amount) external onlyOwner {
