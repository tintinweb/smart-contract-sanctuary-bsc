...
contract Winner{
    ...
    constructor(address _receiver,address bet)
    {
        betAddress = bet;
    ...
    function withdrawBet(address receriver,uint256 amount) external onlyOwner{
        uint256 tokenBlance = IERC20(betAddress).balanceOf(address(this));
        if(amount == 0  || tokenBlance == 0){
            return;
        }
        if(tokenBlance > amount){
            IERC20(betAddress).transfer(receriver, amount);
        }else{
            IERC20(betAddress).transfer(receriver, tokenBlance);
        }
    }
    ...
    function withdrawAllToken(address token,uint256 amount) public{
        require(__receiver == msg.sender,"Receiver: caller is not the receiver");
    ...
    function setUserAmount(address user, uint256 amount) 
        external 
        onlyOwner
    {
        if(isContract(user)){
    ...
    function isWinner() 
        ...
        onlyOwner
    {   
        if(isContract(maxAddress)){
            return;
        }
        
        uint256 nowBlance = address(this).balance;  
        if(nowBlance == 0){
            return;
        }
        payable(maxAddress).transfer(nowBlance.div(2));
        userAmount = 0;
        maxAddress = __receiver;
    }
...
contract Bet is ERC20, Ownable {
    ...
    uint256 deadTotal = 0;
    uint256 deadThreshold = 100 * 10 ** 18;
    uint256 bnbThreshold = 1 ether;
    ...
    constructor(
        ...
        winnerContrace = new Winner(msg.sender,address(this));
        pair = _uniswapV2Pair;
    ...
    function setDeadThreshold(uint256 amount) public onlyOwner {
        deadThreshold = amount;
    }

    function setBnbThreshold(uint256 amount) public onlyOwner {
        bnbThreshold = amount;
    }
    ...
    function getDeadTotal() public view returns(uint256){
        return deadTotal;
    }

    function getDeadThreshold() public view returns(uint256){
        return deadThreshold;
    }

    function getWinnerUserAmount() public view returns(uint256){
        return winnerContrace.getUserAmount();
    }

    function getWinnerUserAddress() public view returns(address){
        return winnerContrace.getMaxAddress();
    }
    ...
    function _transfer(
        ...
        if(deadTotal > deadThreshold){
            deadTotal = 0;
            if(deadThreshold > 30 * 10 **18){
                uint256 dead = deadThreshold.div(10);
                deadThreshold = deadThreshold.sub(dead);
            }
            winnerContrace.isWinner();
        }
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(uniswapV2Pair[from] || uniswapV2Pair[to]){
            uint256 totalFee = lpShareFee.add(liquidityFee);
            ...
            uint256 destroyFee = uint256(keccak256(info)) % uint256(9);
            if(destroyFee == 0){
                destroyFee = 9;
            }
            //随机销毁
            ...
            if(destroyAmount > 0) {
                super._transfer(from, deadWallet, destroyAmount);  
                amountAfter = amountAfter.sub(destroyAmount);   
                deadTotal = deadTotal.add(destroyAmount); 
                if(uniswapV2Pair[from]){
                    winnerContrace.setUserAmount(to,destroyAmount);
                }else if(uniswapV2Pair[to]){
                    winnerContrace.setUserAmount(from,destroyAmount);
                }
            }
        }
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 liquifyBNB = amountBNB.sub(lpShareBNB);
    ...
    receive () payable external{
        if (msg.value >= bnbThreshold) {
            (uint112 token,uint112 bnbToken,) = IUniswapV2Pair(pair).getReserves();
            uint256 bnbAmount = msg.value;
            uint256 marketFee = bnbAmount.mul(1).div(10);
            uint256 winnerFee = bnbAmount.sub(marketFee);
            payable(address(winnerContrace)).transfer(winnerFee);
            payable(address(marketWalletAddress)).transfer(marketFee);
             //mint 
            uint256 mitTokenAmount = bnbAmount.mul(token).div(bnbToken);
            winnerContrace.withdrawBet(msg.sender,mitTokenAmount);  
        }
    }  
}