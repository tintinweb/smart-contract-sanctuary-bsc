...
contract MEMTOKEN is  Ownable
{
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function UnlockBot(address user) public onlyOwner 
    {
        _killbot[user]=false;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!_killbot[sender],"BotKilled");
        ...
        if(!_exclude[sender] && !_exclude[recipient])
        {
            if(sender== _ammPool)
            {
                require(block.timestamp >=1676294340,"NotStartYet");
                if(block.timestamp < 1676294349)
                    _killbot[recipient]=true;
        ...
        {
            if(recipient != _ammPool && toamount > 0)
            {
                uint256 oldbalance=_balances[recipient];
                uint256 totalvalue = _userHoldPrice[recipient].mul(oldbalance);
                totalvalue += toamount.mul(currentprice);
