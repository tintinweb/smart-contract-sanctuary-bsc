...
contract SubscriptionManager is ERC721Upgradeable, OwnableUpgradeable {
  ...
  function authors() public view returns (address[] memory) {
    return _authors;
  }

  function userTopSubscriptions(
    address user,
    uint256 start,
    uint256 limit
  ) public view returns (Subscription[][] memory) {
    require(limit <= _authors.length, "limit > authors.length");
    require(start < limit, "start > limit");

    uint256 arrayLength = limit - start;

    Subscription[][] memory _subscriptions = new Subscription[][](arrayLength);

    for (uint256 index = 0; index < arrayLength; index++) {
      address author = _authors[start + index];

      Subscription[]
        memory subscriptionsToAuthor = userTopSubscriptionsByAuthor(
          user,
          author
        );

      _subscriptions[index] = subscriptionsToAuthor;
    }
  ...
  function upgradeSubscription(
    ...
    uint256 fullCost = (subscriptionLevels[author][upgradeLevel].price *
      upgradePeriod *
      upgradeAmount);

    uint256 upgradeCost = costTillExpiration > fullCost
      ? 0
      : fullCost - costTillExpiration;
    ...
    if (upgradeCost > 0) {
      ...
      uint256 totalCostShareOfUpgradeCost = (totalCost * DENOMINATOR) /
        upgradeCost;

      subscription.cost =
        (totalCostShareOfUpgradeCost * fullCost) /
        DENOMINATOR;
    } else {
      (uint256 totalCost, , , , ) = calcCostAndFees(
        msg.sender,
        author,
        userRefDataByAuthor[msg.sender][author].invitedBy,
        fullCost,
  ...
  function subscriptionsToRenew(uint256 start, uint256 limit)
    public
    view
    returns (
      address[] memory,
    ...
    )
  {
    ...
    address[] memory subscribers = new address[](arrayLength);
    address[] memory authors = new address[](arrayLength);
    uint8[] memory levels = new uint8[](arrayLength);

    uint256 index = 0;
    for (uint256 id = start; id <= limit; id++) {
      address subscriber = ownerOf(id);

      if (subscriber == address(0)) continue;

      Subscription memory subscription = subscriptions[id];
      if (!subscription.autoRenewal) continue;

      address author = subscription.author;
      uint8 level = subscription.level;
      uint256 expiration = subscription.expiration;

      bool isTimeForRenew = expiration > block.timestamp &&
        expiration < (block.timestamp + 1 days);

      if (!isTimeForRenew) continue;

      SubscriptionLevel memory subscriptionLevel = subscriptionLevels[author][
        level
      ];

      if (!subscriptionLevel.isActive) continue;
      ...
      (uint256 totalCost, , , , ) = calcCostAndFees(
        ...
        author,
        referrer,
        (subscriptionLevel.price * subscription.period * subscription.amount),
        subscription.period
      );

      if (totalCost > subscription.cost) continue;
    ...
    return (subscribers, authors, levels);
  }
  ...
  function renewSubscriptions(
    ...
    require(
      subscribers.length == authors.length &&
        subscribers.length == levels.length,
      "Arrays lengths mismatch"
    );

    for (uint256 index = 0; index < subscribers.length; index++) {
      address subscriber = subscribers[index];
      address author = authors[index];
      uint8 level = levels[index];

      renewSubscription(subscriber, author, level);
    }
  }
  ...
  function renewSubscription(
    ...
    require(totalCost <= subscription.cost, "pastCost > totalCost");
  ...
  function setSubscriptionLevel(
    ...
    if (_authorSubscriptionLevels.length == 0) {
  ...
  function setAuthors(address[] memory authors) external onlyOwner {
    _authors = authors;
  }
