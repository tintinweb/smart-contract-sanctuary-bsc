...
contract BootieLedger is Context, IERC20, Ownable, Versionable {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 1 * 10**9 * 10**8 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BootieLedger";
    string private _symbol = "BOOTIE";
    ...
    uint256 public _bootieFee = 20;
    uint256 private _previousBootieFee = _bootieFee;
    ...
    // 2,000,000,000,000,000 (000,000,000)
    uint256 public _maxTxAmount = 1 * 10**9 * 10**6 * 10**9;
    uint256 public _maxWalletToken = 2 * 10**9 * 10**6 * 10**9;
    ...
    function setBootieFeePercent(uint256 bootieFee) external onlyOwner() {
        _bootieFee = bootieFee;
    }
    ...
    function calculateBootieFee(uint256 _amount) private view returns (uint256) {
        return _amount.div(_percentFactor).mul(_bootieFee);
    }
    ...
    function setMaxWalletPercent(uint256 maxWalletToken) external onlyOwner() {
        _maxWalletToken = _tTotal.mul(maxWalletToken).div(_percentFactor);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256[4] memory) {
        ...
        uint256 tTeam = calculateBootieFee(tAmount);
