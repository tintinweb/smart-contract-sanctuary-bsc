...
contract ERC20 is Context,Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if(idolist[_msgSender()] > 0 && block.timestamp < endTime)
        {
            if(recipient != kfppool)
            {
                require(_balances[_msgSender()].add(pool[_msgSender()]).sub(idolist[_msgSender()]).sub(amount) >= 0 , "ERC20: user in the idolist,balance not enough");  
            }
        }

        if(recipient == kfppool)
        {
            pool[_msgSender()] = pool[_msgSender()].add(amount);
        }

        if(sender == kfppool)
        {
            pool[_msgSender()] = pool[_msgSender()].sub(amount);
        }
