...
contract FatToken is ERC20, Ownable {
    ...
    function multi_bclist(address[] calldata addresses, bool value)
        ...
        onlyOwner
    {
        require(enableBlacklist, "enableBlacklist false");
    ...
    function _transfer(
        ...
        require(!_blackList[from], "bclisted address");
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance
                .mul(marketingFee)
                .div(totalFees);
            if (aa) swapAndSendToFee(marketingTokens);

            uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(
                totalFees
            );
            if (bb) swapAndLiquify(swapTokens);

            uint256 sellTokens = balanceOf(address(this));
            if (cc) swapAndSendDividends(sellTokens);
