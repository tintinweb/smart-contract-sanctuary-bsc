...
contract RewardPool is Ownable {
    ...
    constructor(address tokenAdd, uint subsequentClaims, uint timestamp) {
        require(timestamp > 0, "RewardPool : timestamp > 0");
...
/// add distribution mention address, amount and local timestamp
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 250,"RewardPool : length < 250");
        require((account.length == amount.length) && (startTime.length == amount.length),"RewardPool : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
        require ( account[i] != 0x0000000000000000000000000000000000000000, "remove the zero address");
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "RewardPool : insufficient balance to allocate");
...
/// revoke distribution from beneficiary address , amount credit in admin wallet
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        require(account.length < 30,"RewardPool : length < 30");

        for(uint i=0; i< account.length; i++) {  
            userStruct memory user_ = user[account[i]];   
            require(user_.totalClaimed < user_.balance, "RewardPool : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "RewardPool : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"RewardPool : revoke transfer failed");
            }
            emit revokewallet(
                account[i]
            );
...
///only beneficiary wallet will claim after vesting start
    function claim() external {
        ...
        require(user_.balance > 0, "RewardPool  : all claim received");
        require(user_.totalClaimed < user_.balance, "RewardPool : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "RewardPool : lastClaim < block.timestamp");
...
/// extra token withdraw by admin
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            ...
            require(payable(owner()).send(amount), "RewardPool : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "RewardPool : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "RewardPool : transfer failed");
        }
    }
}