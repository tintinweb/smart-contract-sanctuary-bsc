...
contract DEPLOY is Context, IERC20 { 
    ...
    // Feesa
    uint256 public _Tax__Buy_Burn;
    uint256 public _Tax__Buy_Liquidity;
    uint256 public _Tax__Buy_Team;
    uint256 public _Tax__Buy_Tokens;
    uint256 public _Tax__Buy_Rewards;

    uint256 public _Tax__Sell_Burn;
    uint256 public _Tax__Sell_Liquidity;
    uint256 public _Tax__Sell_Team;
    uint256 public _Tax__Sell_Tokens;
    uint256 public _Tax__Sell_Rewards;
    ...
    Wallet_Dev          = payable(_OwnerWallet);
    ...
    function Token_Information() external view returns(address Owner_Wallet,
        ...
        uint256 Total_buy =  _Tax__Buy_Burn         +
                             _Tax__Buy_Liquidity    +
                             _Tax__Buy_Team          +
                             _Tax__Buy_Rewards      +
                             _Tax__Buy_Tokens       ;

        uint256 Total_sell = _Tax__Sell_Burn        +
                             _Tax__Sell_Liquidity   +
                             _Tax__Sell_Team         +
                             _Tax__Sell_Rewards     +
                             _Tax__Sell_Tokens      ;
    ...
    */
    ...
    // Buy Fees
    function A_Contract_Initialized_01__BuyTax(
        ...
        require (Dev          + 
                 ...
                 Tokens       +
                 Rewards
                 <= 10, " Buy fees are limited of 10% max to protect buyers"); 

        // Update Fees
        _Tax__Buy_Team        = Dev;
        _Tax__Buy_Liquidity  = Liquidity;
        _Tax__Buy_Burn       = Burn;
        _Tax__Buy_Tokens     = Tokens;
        _Tax__Buy_Rewards    = Rewards;

        // Fees For Processing
        _SwapFeeTotal_Buy    = _Tax__Buy_Team + _Tax__Buy_Liquidity + _Tax__Buy_Rewards;

        emit updated_Buy_fees(_Tax__Buy_Team, _Tax__Buy_Liquidity, _Tax__Buy_Burn, _Tax__Buy_Tokens, _Tax__Buy_Rewards);
    }

    // Sell Fees
    function A_Contract_Initialized_02__SellTax(
        ...
        require (Dev        + 
                 ...
                 Tokens     +
                 Rewards 
                  <= 10, " Sell fees are limited of 10% max to protect buyers "); 

        // Update Fees
        _Tax__Sell_Team        = Dev;
        _Tax__Sell_Liquidity  = Liquidity;
        _Tax__Sell_Burn       = Burn;
        _Tax__Sell_Tokens     = Tokens;
        _Tax__Sell_Rewards    = Rewards;


        // Fees For Processing
        _SwapFeeTotal_Sell    = _Tax__Sell_Team + _Tax__Sell_Liquidity + _Tax__Sell_Rewards;

        emit updated_Sell_fees(_Tax__Sell_Team, _Tax__Sell_Liquidity, _Tax__Sell_Burn, _Tax__Sell_Tokens, _Tax__Sell_Rewards);
    }
    ...
    */
    ...
    function A_Contract_Initialized_05__Open_Trade() external onlyOwner {
        ...
        _SwapFeeTotal_Buy    = _Tax__Buy_Liquidity + _Tax__Buy_Team + _Tax__Buy_Rewards ;
        _SwapFeeTotal_Sell   = _Tax__Sell_Liquidity + _Tax__Sell_Team + _Tax__Sell_Rewards;
    ...
    */
    ...
    function D_Update___Wallets(
        ...
        Wallet_Dev = payable(Dev_Fee_Wallet);
    ...
    */
    ...
    function G_Rewards__Auto_Process(bool true_or_false) external onlyOwner {
    ...
    function G_Rewards__Process_Now (uint256 Percent_of_Tokens_to_Process) external onlyOwner {
    ...
    function G_Rewards__Swap_Trigger_Count(uint256 Transaction_Count) external onlyOwner {
    ...
    function G_Rewards__Remove_Random_Tokens(
    ...
    */
    ...
    // Exclude From Fees
    function E_WalletsOp__Exclude_From_Fees(
    ...
    function E_WalletsOp__Exempt_From_Limits(
    ...
    */
    ...
    function _transfer(
        ...
        if (!Trade_Open){
        require(_isWhitelisted[from] || _isWhitelisted[to], "Trade is not open, only whitelisted wallets can trade");
        }
    ...
    */
    ...
    function swapAndLiquify(uint256 Tokens) private {
        ...
        uint256 LP_Tokens       = Tokens * (_Tax__Buy_Liquidity + _Tax__Sell_Liquidity) / _FeesTotal / 2;
        ...
        uint256 fee_Split       = _FeesTotal * 2 - (_Tax__Buy_Liquidity + _Tax__Sell_Liquidity);

        // Calculate BNB Values
        uint256 BNB_Liquidity   = returned_BNB * (_Tax__Buy_Liquidity     + _Tax__Sell_Liquidity)       / fee_Split;
        uint256 BNB_Rewards     = returned_BNB * (_Tax__Buy_Rewards       + _Tax__Sell_Rewards)     * 2 / fee_Split; 
        ...
        if (contract_BNB > 0){
            ...
            send_BNB(Wallet_Dev, contract_BNB);
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool Fee) private {
        ...
        if (Fee){
            ...
            if(_isPair[recipient]){
                ...
                // Sell Fees
                tBurn           = tAmount * _Tax__Sell_Burn       / 1000;
                tTokens         = tAmount * _Tax__Sell_Tokens     / 1000;
                tSwapFeeTotal   = tAmount * _SwapFeeTotal_Sell    / 1000;

            } else {

                // Buy Fees
                tBurn           = tAmount * _Tax__Buy_Burn        / 1000;
                tTokens         = tAmount * _Tax__Buy_Tokens      / 1000;
