...
contract YING is ERC20, Ownable {
    ...
    FREDDYYINGREFLECTDividendTracker public dividendTracker;
    ...
    constructor() ERC20("YING", "YING") {
        ...
        totalSellFees = sellRewardsFee
         ...
         dividendTracker = new FREDDYYINGREFLECTDividendTracker(
            payable(this),
            router,
...
contract FREDDYYINGREFLECTDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        YINGContract = YING(mainContract);
    ...
    function _transfer(
        ...
        require(false, "YING_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "YING_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main YING contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(YINGContract);

            uint256 prevBalance = YINGContract.balanceOf(address(this));
            ...
            {
                uint256 received = YINGContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    YINGContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
