...
// SPDX-License-Identifier: MIT
    ...
    contract XTYToken is Context, IERC20, Ownable {
        ...
        uint256 private _tTotal = 5680000 * 10**18;
        ...
        uint256[] public _perTierFee = [3,2,1,1,1,1,1,1];

        address public  uniswapV2Pair;
            
        uint256 public _maxTradeAmount = 50000 * 10**18;
        uint256 public _recommerAmount = 1 * 10**18;
        uint256 public MAX_STOP_FEE = 568000 * 10**18;
        ...
        constructor (address _holder, address _marketAddress1, address _marketAddress2, address _fundAddress) public {
        ...
        function setMaxStopFee(uint256 amount) external onlyOwner() {
            MAX_STOP_FEE = amount;
        }

        function setRecommerAmount(uint256 amount) external onlyOwner() {
            _recommerAmount = amount;
        }
        ...
        function _getValues(uint256 tAmount,bool takeFee,address user) private view returns (Param memory param) {
            ...
            if(takeFee){
                ...
                uint tier = getUserTierCount(user);
                uint _shareFee = 0;
                if( tier > 0 ){
                    _shareFee = getTotalShareFee();
                    param.tShare = tAmount * _shareFee / 100;
                }
                uint _totalFee = _liquidityFee  + _burnFee   + _marketFee  + _shareFee;
        ...
        function _takeLiquidity(uint256 tLiquidity,uint256 currentRate,address from) private {
            ...
            _rOwned[uniswapV2Pair] = _rOwned[uniswapV2Pair].add(rLiquidity);
            if(_isExcluded[uniswapV2Pair]){
                _tOwned[uniswapV2Pair] = _tOwned[uniswapV2Pair].add(tLiquidity);
            }
            emit Transfer(from, uniswapV2Pair, tLiquidity);
        }
        ...
        function _takeMarket(uint tMarket,uint256 currentRate,address from) private {
            ...
            _rOwned[marketAddress1] = _rOwned[marketAddress1].add(rMarket.div(2));
            _rOwned[marketAddress2] = _rOwned[marketAddress2].add(rMarket.div(2));
            if(_isExcluded[marketAddress1]){
                _tOwned[marketAddress1] = _tOwned[marketAddress1].add(tMarket.div(2));
            }
            if(_isExcluded[marketAddress2]){
                _tOwned[marketAddress2] = _tOwned[marketAddress2].add(tMarket.div(2));
            }    
        ...
        function _takeShare(uint tShare,uint256 currentRate,address from,address user) private {
            ...
            if( tShare > 0 ){
                ...
                if( tier > 0 ){
                    address[] memory farthers = getForefathers(user,tier);
        ...
        function getUserTierCount(address addr) public view returns (uint) {
            uint256 count = 0;
            address recommer = _recommerMapping[addr];
            while(true) {
                if(recommer == rootAddress || recommer == address(0) ) break;
                count = count.add(1);
                if (count >= _perTierFee.length) break;
                recommer = _recommerMapping[recommer];
            }
            return count;
        }
        ...
        function _transfer(
            ...
            if(uniswapV2Pair != address(0) && _tTotal> MAX_STOP_FEE) {
                if( from == uniswapV2Pair &&  !_isExcludedFromFee[to]   ){
                    takeFee = true;
                    user = to;
                }
        ...
        function setRecommer(address addr, address recommer) external onlyOwner {
            require(recommer != address(0), 'zero address');
            require(addr != address(0), 'zero address');
            require(_recommerMapping[recommer] != address(0), 'bad recommer1');
            require(_checkRecommer(addr, recommer) == true, 'bad recommer2');

            _recommerMapping[addr] = _recommerMapping[recommer];
        }

        function _checkRecommer(address addr, address recommer) internal view returns(bool) {
