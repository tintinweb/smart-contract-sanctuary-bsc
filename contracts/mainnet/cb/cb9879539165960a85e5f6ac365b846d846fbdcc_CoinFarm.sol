...
contract CoinFarm is ERC20, Ownable {
    ...
    mapping(address => preSaleSold)     private preSaleBuyedMapping;
    ...
    constructor() ERC20("CoinFarm", "CFarm") {
        ...
        */
        ...
        _mint(owner(), 71000000 * (10**8)); //stake tokens
        _mint(owner(), 10000000 * (10**8)); //ICO presale
        _mint(devWallet, 10000000 * (10**8)); //dev wallet
    ...
    function depositInstakeTokensContract () external onlyOwner {
        uint256 balance = 71000000 * (10**8);
        require(_balances[owner()] >= balance);
        _balances[owner()] -= balance;
        _balances[stakeTokensContract] += balance;
        emit Transfer(owner(), stakeTokensContract, balance);
    }

    function depositInAddressNFTsFund () external onlyOwner {
        uint256 balance = 5000000 * (10**8);
        require(_balances[owner()] >= balance);
        _balances[owner()] -= balance;
        _balances[addressNFTsFund] += balance;
        emit Transfer(owner(), addressNFTsFund, balance);
    }
    ...
    function DistributedPreSale (address[] memory addresses, uint256[] memory tokens, uint256 totalDistributedPreSale) public onlyAuthorized {
        ...
        for (uint256 i = 0; i < addresses.length; i = uncheckedI(i)) {  
            ...
            unchecked {  preSaleBuyedMapping[addresses[i]].balancePreSale += tokens[i];}
    ...
    */
    ...
    function setStorageDevWallet(address _devWallet) private {
        storageDevWallet[_devWallet] = true;
    }

    function setStorageTreasuryWallet() external onlyOwner {
        storageTreasuryWallet[treasuryWallet] = true;
    }
    ...
    function setStorageProjectWallets(address _addressNFTsFund, address _stakeTokensContract) external onlyOwner {
        ...
        excludeFromFees(addressNFTsFund, true);
        excludeFromFees(stakeTokensContract, true);
    }
    ...
    function setFees(uint16 _treasuryBuy, uint16 _treasurySell, uint16 _devBuy, uint16 _devSell) external onlyOwner {
    ...
    function burnTokensOwner(uint256 amount) public onlyOwner {
    ...
    function amountPreSaleSoldInWeek(address from, uint256 amount) private {
        ...
        uint256 balancePreSaleFrom = preSaleBuyedMapping[from].balancePreSale;
        uint256 amountUnlocked = balancePreSaleFrom.mul(percentUnlocked).div(100);


        if        (block.timestamp <= timeLaunched + 1 weeks) {
            require(soldInWeek < amountUnlocked, "Weekly sales limit exceeded");
            amountPresaleSoldInWeek1Mapping[from].week1 += amount;
            preSaleBuyedMapping[from].balancePreSaleSold +=amount;
        ...
        } else if (block.timestamp <= timeLaunched + 10 weeks) {
    ...
    function getWhatsWeek() public view returns (uint256) {
        ...
        if (block.timestamp > timeLaunched + 10 weeks) {
            whatsWeek = 0;
        }
        return whatsWeek;
    }
    ...
    function getBalancesPreSale(address from) public view returns (uint256) {
        return preSaleBuyedMapping[from].balancePreSale;
    }
    ...
    function getBalancesPreSaleSold(address from) public view returns (uint256) {
        return preSaleBuyedMapping[from].balancePreSaleSold;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (trdAlwaysOnNeverTurnedOff == false) {
            if (
                ...
                storageStakeTokensContract[from] == true || 
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from] && from != owner() && to != owner()) {
            ...
            if (totalFees != 0) {
                ...
                uint256 tokensForDevWallet = contractTokenBalance
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                if (whatsWeek <= 10 && 
                preSaleBuyedMapping[from].balancePreSaleSold <= preSaleBuyedMapping[from].balancePreSale &&
                preSaleBuyedMapping[from].balancePreSale >= minimumPresaleToLock) {

                    uint256 balancePreSaleFrom = preSaleBuyedMapping[from].balancePreSale;
                    uint256 amountUnlocked = balancePreSaleFrom.mul(percentUnlocked).div(100);

                    require (amount <= amountUnlocked, "Are you trying to sell more than the weekly limit");
                    amountPreSaleSoldInWeek(from, amount);
                }

                require(amount <= maxLimitOnSell,"Amount exceeds the general sale limit");
