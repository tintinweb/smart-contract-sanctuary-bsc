...
contract StakingBitscrowNewEntry {
    ...
    mapping(address => uint256) private stakingPeriodEnd;

    // Token amount variables
    mapping(address => uint256) private alreadyWithdrawnRewards;
    mapping(address => uint256) private balances;
    uint private _tokenaavailableforrewards = 0;
    uint private _TotalStakedTokens = 0;
    uint private _willWithdraw;
    uint private _seatsLeft;
    

    //rewards
    uint private rewardsrate;
    uint256 private minPeriodBetweenWithdraws = 7 days;
    uint256 private countdownEnd;
    mapping(address => uint) private PersonalRewards;
    uint private cost;
    uint private apy;
    uint private restakeBonus;

      /**
     * @dev allows the owner to achange the cost of the entry
     */
    function changeCost(uint newCost) external  onlyOwner{
        cost = newCost;
        _adjustSeats();
    }
     ...
     */
    function changeApy(uint newApy) external  onlyOwner{
        apy = newApy;
    ...
    constructor(IERC20 _erc20_contract_address, uint _apy, uint _cost) {
        // Set contract owner
        owner = msg.sender;
        // Set staking period
        timePeriod = 90 days;
    ...
    modifier checkPeriod(){
        ...
        require(stakingPeriodEnd[msg.sender] != 0, "stakingPeriodEnd cannot be zero");
        require(block.timestamp >= stakingPeriodEnd[msg.sender], 
        "Tokens are only available after correct time period has elapsed");
        _;
    }

    /**
     * @dev allows the user to stake their tokens
     */

    function stakeTokens() external  {
        uint amount = cost;
        PersonalRewards[msg.sender] = amount * rewardsrate / 1000;
        require(_tokenaavailableforrewards >=  PersonalRewards[msg.sender], "not enough BTCSCRW in fromrewards");
        ...
        require(balances[msg.sender] == 0,  "you already have tokens in staking");
        stakingPeriodEnd[msg.sender] = block.timestamp + timePeriod;       
    ...
    function unstakeTokens() external checkPeriod  {
        ...
        uint rewards = PersonalRewards[msg.sender];
     ...
     * this will restake the tokens orignally staked and send the rewards to the user, the rewards will 
    ...
    function restakeTokens() external checkPeriod {
        ...
        uint rewards = PersonalRewards[msg.sender];
        alreadyWithdrawnRewards[msg.sender] += rewards;
        PersonalRewards[msg.sender] = _amount * (rewardsrate + restakeBonus) / 1000;
        require(_tokenaavailableforrewards >=  PersonalRewards[msg.sender], 
     ...
     * this will send the only the tokens orignally staked to the user, resulting in the user losing his rewards
     */
     ...
     */
    function changeBonus(uint newBounus) external onlyOwner{
        restakeBonus = newBounus;
    }
    
    /**
     * @dev allows the owner to withdraw the locked funds, note that can only be used if 
     * the correct time has passed since the beginning the countdown for the withdraw, note that even
     * if all the funds in ('_tokenaavailableforrewards') are withdrawn the contract would still retain 
     * enough  funds to pay back all the tokens that are currently staked and their respective rewards
     */

    function withdrawlockedfunds() external onlyOwner returns(bool success){
        require(countdownEnd != 0, "countdownEnd cannot be zero");
    ...
    function initiateCountdownForWithdraaw(uint amount) external onlyOwner returns(bool success){
        require(amount <= _tokenaavailableforrewards, "not enough funds in contract");
    ...
    /**
     * @dev allows the owner to lock the tokens in order for them to be used to pay the rewards
     */
     /// @param amount amount of tokens to be locked
    function lockTokens(uint256 amount) external onlyOwner {
        require(amount <= token.balanceOf(msg.sender), "Not enough BTSCRW tokens in your wallet");
        _tokenaavailableforrewards += amount;
     ...
     */
    function changeSeats(uint amount) external onlyOwner{
        require(_tokenaavailableforrewards * 1000 / rewardsrate / cost >= amount, "not enough funds");
        _seatsLeft = amount;
    }

    /**
     * @dev allows the owner to change the time period
     */
    function changeTimePeriod(uint newTP) external onlyOwner{
        timePeriod = newTP;
        rewardsrate = apy / 12 * (timePeriod / 30 days);
    }
    ...
    function timeLeftTillUnstake(address user) public  view returns(uint256){
        if(block.timestamp >= stakingPeriodEnd[user] || balances[user] == 0) {
            return 0;
        }else {
            return stakingPeriodEnd[user] - block.timestamp;
        }
    }
    ...
    function personalRewards(address user) external  view returns(uint){
        return PersonalRewards[user];
    }

    function seeAlreadyWithdrawnRewards(address user) public view returns(uint){
    ...
    function  seeapy()  external  view returns(uint){
        return apy / 10;
    }

    function  seeCountdownEnd()  external  view returns(uint){
        return countdownEnd;
    }
    ...
    function contractInfo()external view returns(uint poolCost, uint seats, uint Stakingperiod, uint Apy) {
        return (cost, _seatsLeft, timePeriod, apy/10);
    }


    function userInfo(address user)external view  returns(uint Rewards, uint balance, uint timeleft, uint allowance) {
        return (PersonalRewards[user], balances[user], timeLeftTillUnstake(user), token.allowance(user, address(this)));
    }

    function _adjustSeats() private {
        if(_tokenaavailableforrewards * 1000 / rewardsrate / cost < _seatsLeft) {
            _seatsLeft = _tokenaavailableforrewards * 1000 / rewardsrate / cost;
        }
    }
