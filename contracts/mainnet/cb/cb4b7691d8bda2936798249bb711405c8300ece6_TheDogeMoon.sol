...
// WELCOME TO THE DOGE MOON
//Release the Doge !  is an open source peer-to-peer cryptocurrency, favored by Dogecoin worldwide. 
//Elon Musk thinks we're pretty cool.
//https://twitter.com/Thedogemoonn
//https://t.me/Thedogemoongr
//https://t.me/thedogemoonchannel
...
contract TheDogeMoon is Context, IBEP20, Ownable {
    ...
    address public marketingWallet = 0x281B4a0D0d566a086377322c60c5D59Bcf681D99;
    address public devWallet = 0x8487Ff6B5835e656b3277e339615805e6D71d06F;

    string private constant _name = "The Doge Moon";
    ...
    Taxes public buyTaxes = Taxes(3, 3, 0, 3, 0);
    Taxes public sellTaxes = Taxes(3, 3, 0, 3, 0);
    ...
    modifier isDev() {
        ...
        require(owner() == msg.sender || msg.sender == devWallet, "Dev: caller is not the owner or dev");
        _;
    }
    ...
    function EnableTrading() external isDev {
    ...
    function excludeFromReward(address account) public isDev {
    ...
    function includeInReward(address account) external isDev {
    ...
    function excludeFromFee(address account) public isDev {
        _isExcludedFromFee[account] = true;
    }

    function includeInFee(address account) public isDev {
    ...
    function bulkExcludeFee(address[] memory accounts, bool state) external isDev {
        for (uint256 i = 0; i < accounts.length; i++) {
            _isExcludedFromFee[accounts[i]] = state;
        }
    }

    function updateMarketingWallet(address newWallet) external isDev {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        marketingWallet = newWallet;
    }

    function updateDevWallet(address newWallet) external isDev {
        require(newWallet != address(0),"Fee Address cannot be zero address");
        devWallet = newWallet;
    }

    function updateSwapTokensAtAmount(uint256 amount) external isDev {
        require(amount <= 1e15, "Cannot set swap threshold amount higher than 1% of tokens");
        swapTokensAtAmount = amount * 10**_decimals;
    }

    //Use this in case BNB are sent to the contract by mistake
    function rescueBNB(uint256 weiAmount) external isDev {
        require(address(this).balance >= weiAmount, "insufficient BNB balance");
        payable(msg.sender).transfer(weiAmount);
    }

    //Use this in case BEP20 Tokens are sent to the contract by mistake
    function rescueAnyBEP20Tokens(address _tokenAddr,address _to, uint256 _amount) public isDev {
