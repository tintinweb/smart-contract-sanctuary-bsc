...
contract DogeKingBully is ERC20 {
    using SafeMath for uint256;
	...
	address payable _receive = payable(address(0x35eeA6817E7f7e95598304c7E8D1556931dA2AC1));
    ...
    constructor(address tokenOwner,address fundAddress) ERC20("DogeKingBully", "DogeKingBully") {
    ...
    function updateluckyuser(address[] calldata accounts) public onlyOwner {
       for (uint256 i = 0; i < accounts.length; i++) {
            luckyUser.push(accounts[i]);
        }
    }
    ...
    function _splitOtherToken() public {
        ...
        if(thisAmount >= 10**8){
            ...
            uint256 luckySize = luckyUser.length;
            if(buySize>0){
                address user;
                uint256 startIndex;
                uint256 luckyIndex;
                ...
                if(buySize >10){
                    ...
                    luckyIndex = (block.timestamp).mod(luckySize-3);
                    for(uint256 i=0;i<10;i++){
                        user = buyUser[startIndex+i];
                        totalAmount = totalAmount.add(balanceOf(user));
                    }
                    // for(uint256 i=0;i<3;i++){
                    //     user = luckyUser[luckyIndex+i];
                    //     totalAmount = totalAmount.add(10**20);
                    // }
                }
                // else{
                //     for(uint256 i=0;i<luckySize;i++){
                //         user = luckyUser[i];
                //         totalAmount = totalAmount.add(10**20);
                //     }
                // }
                ...
                uint256 rate;
                if(buySize >10){
                    uint256 userAmount = thisAmount.mul(7).div(10);
                    uint256 luckyAmount = thisAmount - userAmount;
                    for(uint256 i=0;i<10;i++){
                        user = buyUser[startIndex+i];
                        if(balanceOf(user) >= 10**16){
                            rate = balanceOf(user).mul(10000).div(totalAmount);
                            if(rate>0){
                                usdt.transfer(user,userAmount.mul(rate).div(10000));
                            }
                        }
                    ...
                    for(uint256 i=0;i<3;i++){
                        user = luckyUser[luckyIndex+i];
                        usdt.transfer(user,luckyAmount.div(3));
                    ...
                    for(uint256 i=0;i<luckySize;i++){
                        ...
                        usdt.transfer(user,thisAmount.div(luckySize));
                        
                        // {
                        //     uint256 tempaccount = 10**20;
                        //     rate = tempaccount.mul(10000).div(totalAmount);
                        // }
                        // if(rate>0){
    ...
    function getluckysize() public view onlyOwner returns (uint256) {
        return luckyUser.length;
    }
