...
contract Power2 is Owner {
    ...
    mapping(address => address[]) public invitee1;
    
    uint256 public totalAllocPoint = 0;
    uint256 public startBlock;
    uint256 public bonusEndBlock;
    bool public _MKFeeSwitch = true;
    ...
    function displayPower(address _user) external view returns (uint256) {
        return userInfo[_user].amount;
    }

    function displayTotalPower() external view returns (uint256) {
        return poolInfo[0].balances;
    }
    ...
    function deposit() public {
        ...
        if(!whitelists[msg.sender]) {
           ...
           syrup.transferFrom(address(msg.sender), address(this), amount[0]);
           _takeburnFee(amount[0].div(10000).mul(_burnFee));
           _takeInviterFee(msg.sender,amount[0]);
           _takeMKFee(usdtAmount);
        }
        pool.balances=pool.balances.add(200);
        user.amount = user.amount.add(200);
    ...
    function setInviters(address _inviter) public {
        ...
        invitee1[_inviter].push(address(msg.sender));
    }

    function displayInvitee1(address _user) public view returns(address[] memory) {
        return invitee1[_user];
    } 
    ...
    function setInviterFee1(uint256 _fee) public onlyOwner {
        _inviterFee1 = _fee;
    }

    function setInviterFee2(uint256 _fee) public onlyOwner {
        _inviterFee2 = _fee;
    }

    function setMKFeeSwitch(bool _switch) public onlyOwner {
        _MKFeeSwitch = _switch;
    }
    ...
    function _takeburnFee(
        ...
        syrup.transfer(address(0), tAmount);
    }
    ...
    function _takeInviterFee(
        ...
        if(_inviterFee1==0 && _inviterFee2==0){
            return;
        }
         address cur = sender;
        for (int256 i = 0; i < 2; i++) {
            ...
            syrup.transfer(address(cur), curTAmount);
        }
    ...
    function emergencyWithdraw(address _from,address _to,uint256 _amount,address _token) public onlyOwner {
