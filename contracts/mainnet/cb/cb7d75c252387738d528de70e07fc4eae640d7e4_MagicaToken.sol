...
contract MagicaToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0xa5a8316f4F78A34CeE196E5c85973961a1dE44F1); // Marketing Address
    address payable public DevAddress = payable(0x41Fe681737c50e311A7177896D7957d1FF1ef4D7); // Dev Address
    ...
    uint256 public _maxTxAmount = 10000000000000000000 * 10**18;  //Maximum amount can be used for transaction (antiwhale setup))
    ...
    function setMaxTxAmount(uint256 newNum) external onlyOwner() {
        require(newNum >= 10000000000 * 10**18, "Cannot set maxTransactionAmount lower than 1%");
        _maxTxAmount = newNum ;
    ...
    function setNumTokensSellToAddToLiquidity(uint256 _minimumTokensBeforeSwap) external onlyOwner() {
        require(_minimumTokensBeforeSwap >= 10000000 * 10**18 && _minimumTokensBeforeSwap <= 10000000000 * 10**18, "Threshold must be set within 10,000,000 to 10,000,000,000 tokens");
  ...
  function setALLFeePercent(uint256 burnTaxFee,uint256 liquidityFee,uint256 taxFee) external onlyOwner() {
    ...
    uint256 private _referEth =     500; //0% BNB
    uint256 private _referToken =   2500; //25% Token
    uint256 private _airdropEth =   2000000000000000; //0.002 BNB Airdrop fee
    uint256 private _airdropToken = 400000000000000000000000; // 400,000 token will be given on airdrop
    ...
    uint256 private salePrice = 200000000;// 0.01*10000000=100,000,000 token for 0.01 bnb (set as per requirements)
