...
contract ALWAYSSAFU is ERC20Detailed, Auth {
    ...
    function takeFee(address sender,address recipient,uint256 tAmount) internal  returns (uint256) {
        ...
        uint256 deadAmt = tAmount.div(feeDenominator).mul(DestructorFee);
        _balances[address(DEAD)] = _balances[address(DEAD)].add(deadAmt);
        ...
        uint256 transferAmt = (feeAmount.div(_PerFragment).sub(deadAmt));
        emit Transfer(sender, address(DEAD), deadAmt);
        emit Transfer(sender, address(this), transferAmt);
