...
contract FeeDistributor is Ownable {
    ...
    function _distribute() internal {
        ...
        uint dividendFee = _totalBuyFee == 0 ? 0 : collectedBuyFees.mul(dividendBuyFee).div(_totalBuyFee) + 
                            _totalSellFee == 0 ? 0 : collectedSellFees.mul(dividendSellFee).div(_totalSellFee);
        uint reflectFee = _totalBuyFee == 0 ? 0 : collectedBuyFees.mul(reflectBuyFee).div(_totalBuyFee) + 
                            _totalSellFee == 0 ? 0 : collectedSellFees.mul(reflectSellFee).div(_totalSellFee);
        uint marketingFee = _totalBuyFee == 0 ? 0 : collectedBuyFees.mul(marketingBuyFee).div(_totalBuyFee) + 
                            _totalSellFee == 0 ? 0 : collectedSellFees.mul(marketingSellFee).div(_totalSellFee);
        uint[] memory reservedFees = new uint[](reservedBuyFees.length);
        for (uint i = 0; i < reservedBuyFees.length; i++) {
            reservedFees[i] = _totalBuyFee == 0 ? 0 : collectedBuyFees.mul(reservedBuyFees[i]).div(_totalBuyFee) + 
                            _totalSellFee == 0 ? 0 : collectedSellFees.mul(reservedSellFees[i]).div(_totalSellFee);
        }
