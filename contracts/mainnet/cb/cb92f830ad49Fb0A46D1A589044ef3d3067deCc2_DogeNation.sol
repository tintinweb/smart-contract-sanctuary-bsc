...
library Address{
 ...
 contract DogeNation is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletLimit = 2000000000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x9CF660acAe0178214a5A78B5b9cBb54007c013ae;
    address public UtilityWallet = 0x9CF660acAe0178214a5A78B5b9cBb54007c013ae;

    string private constant _name = "DogeNation";
    ...
    Taxes public sellTaxes = Taxes(0, 5, 2, 1);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Utility) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_Utility);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _Utility) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_Utility);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rUtility) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.trfi-s.tMarketing-s.tLiquidity-s.tUtility;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rrfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rrfi-rMarketing-rLiquidity-rUtility;
        return (rAmount, rTransferAmount, rrfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rUtility) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tUtility);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.Utility) * 2;
