...
contract SaleGovernance is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "Sale Governance";
    string private _symbol = "SGNC";
    ...
    address payable public charityWalletAddress =
        payable(0x1a3786a7bB59434938550e2E4f8c302e61D1E036);
    ...
    constructor() {
        ...
        _totalTaxIfBuying = _buyLiquidityFee.add(_buyCharityFee).add(
            _buyTeamFee
        );
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellCharityFee).add(
            _sellTeamFee
        );
        _totalDistributionShares = _liquidityShare.add(_charityShare).add(
            _teamShare
        );
    ...
    function setBuyTaxes(
        ...
        require(
            newLiquidityFee + newCharityFee + newTeamFee <= 10**4 / 4,
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (
                ...
                uint256 _sellReserveFeeAmount = amount.div(100).mul(
                    _sellReserveFee
                );
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountBNBCharity = amountReceived.sub(amountBNBLiquidity).sub(
            amountBNBTeam
        );
