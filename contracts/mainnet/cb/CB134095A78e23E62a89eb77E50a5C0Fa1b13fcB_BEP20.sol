...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    contract BEP20 is ERC20, Ownable, Pausable {
    uint256 private initialSupply = 1 * (10**12);
    ...
    uint256 private backbuyburnTaxBuy = 0;
    ...
    address private devTaxWallet = 0xbd0A13314596D8191f1bbCe2FE916298b2953B59;
    address private marketingTaxWallet = 0xbd0A13314596D8191f1bbCe2FE916298b2953B59;
    address private charityTaxWallet = 0xbd0A13314596D8191f1bbCe2FE916298b2953B59;
    address private backbuyburnTaxWallet = 0xbd0A13314596D8191f1bbCe2FE916298b2953B59;
    ...
    constructor() ERC20("WORLD WAR", "WORLD WAR")
    {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if(ethValue >= swapThreshold) {
                    ...
                    uint256 liquidityETH = (ethGained * ((liquidityTokens / 2 * 10**12) / taxSum)) / 10**12;
                    
                    uint256 marketingETH = (ethGained * ((marketingTokens * 10**12) / taxSum)) / 10**12;
                    uint256 devETH = (ethGained * ((devTokens * 10**12) / taxSum)) / 10**12;
                    uint256 charityETH = (ethGained * ((charityTokens * 10**12) / taxSum)) / 10**12;
    ...
    function _transfer(
        ...
        require(!paused(), "WW3: token transfer while paused");
        require(!isBlacklisted(msg.sender), "WW3: sender blacklisted");
        require(!isBlacklisted(recipient), "WW3: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "WW3: sender blacklisted");
    ...
    function pause() public onlyOwner {
        require(!paused(), "WW3: Contract is already paused");
        _pause();
    }

    function unpause() public onlyOwner {
        require(paused(), "WW3: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "WW3: Account is already blacklisted");
        blacklist[account] = true;
    }
    
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "WW3: Account is not blacklisted");
        blacklist[account] = false;
    }
    
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "WW3: Account is already excluded");
        excludeList[account] = true;
    }
    
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "WW3: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "WW3: Tax is already enabled");
        taxStatus = true;
    }
    function disableTax() public onlyOwner {
        require(taxStatus, "WW3: Tax is already disabled");
        taxStatus = false;
    }
