...
// File: contracts/True.sol

pragma solidity ^0.8.0;

contract BGENFTBox is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("BgeNFTBox", "BgeNFTBox") {}

    modifier canMint() {
        require(isOwner() || minter == msg.sender, "no permission");
        _;
    }
    ...
    function mintBatch(address to, uint256[] memory tokenIds) public canMint {
    ...
    function mintBatch2(address to, uint256 amount) public canMint {
        uint256[] memory tokenIds = new uint256[](amount);
        for (uint256 index = 0; index < amount; index++) {
            tokenIds[index] = maxTokenId + index + 1;
        }
    ...
    function burn(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "not your own tokenId");
        super._burn(tokenId);
    }
    ...
    function setEndUri(string memory uri) public onlyOwner {
        enduri = uri;
    }
