...
contract AgentManager is Ownable {
    ...
    address[] routers = [0x10ED43C718714eb63d5aA57B78B54704E256024E];
    ...
    mapping (uint256 => mapping (uint256 => int256)) public swapInfo;
    ...
    function _createNewAgent(uint256 _tid, uint256 _amount) internal {
        ...
        if(available > 0) {
            ...
            IAgent(address(agent)).deposit(available);
        }
    ...
    function _swap(uint256 _from, uint256 _to, uint256 _amount) internal returns (uint256) {
        ...
        for (uint256 index = 0; index < routers.length; index++) {
            ...
            if(amounts[amounts.length] > amountOut) {
                amountOut = amounts[amounts.length];
    ...
    function swap(uint256 _from, uint256 _to, uint256 _amount) public onlyOwner {
        massUpdatePools();
        uint256 amountOut = _swap(_from, _to, _amount);
        PoolInfo storage from = poolInfo[_from];
        PoolInfo storage to = poolInfo[_to];
        from.balance = from.balance.sub(_amount);
        to.balance = to.balance.add(amountOut);
        swapInfo[_from][_to] = swapInfo[_from][_to] - int256(_amount);
        swapInfo[_to][_from] = swapInfo[_to][_from] + int256(amountOut);
    }
...
contract InsurAceAgent is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) public onlyOwner {
        ...
        available = available > balance ? balance : available;
        IERC20(depositToken).approve(target, available);
        IStakingV2Controller(target).stakeTokens(available, depositToken);
    }
    ...
    function harvest() public onlyOwner {
        uint256 available = availableHarvest();
        uint256 balanceBefore = IERC20(rewardToken).balanceOf(address(this));
        IRewardController(rewardController).withdrawReward(available);
    ...
    function availableWithdraw(uint256 _amount) public view returns (uint256) {
        ...
        uint256 totalStakedAmount = IStakersPoolV2(stakersV2).getStakedAmountPT(depositToken);
        uint256 available = burnable.mul(totalStakedAmount).div(IERC20(lpToken).totalSupply());
        return available;
    }
