...
contract Farm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accFonvityPerShare; // Accumulated Roselle per share, times 1e12. See below.
    }
    ...
    uint256 public fonvityPerBlock = 1e11;
    // Bonus muliplier for early fonvity makers.
    ...
    function pendingFonvity(uint256 _pid, address _user)
        external
        view
        ...
        returns (uint256)
    {
        ...
        uint256 accFonvityPerShare = pool.accFonvityPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 fonvityReward = multiplier
                .mul(fonvityPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint)
                .mul(85)
                .div(100);
            accFonvityPerShare = accFonvityPerShare.add(
                fonvityReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accFonvityPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public validatePoolByPid(_pid) {
        ...
        uint256 fonvityReward = multiplier
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        pool.accFonvityPerShare = pool.accFonvityPerShare.add(
            fonvityReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
        ...
        nonReentrant
    {
        ...
        user.rewardDebt = user.amount.mul(pool.accFonvityPerShare).div(1e12);
    ...
    // Safe fonvity transfer function, just in case if rounding error causes pool to not have enough Roselle.
