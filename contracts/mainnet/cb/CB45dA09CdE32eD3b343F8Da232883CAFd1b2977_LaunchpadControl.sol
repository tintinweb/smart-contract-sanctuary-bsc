...
contract LaunchpadControl is 
  ...
  function buyNFT(uint256 _id, uint256 _amount) external payable {
    ...
    require((allocations[msg.sender].add(aluAmount)).mul(10**18) <= maxAllocation, "exceeded max allocation.");
...
contract PancakePairPrice {
  ...
  // Get pair price
  function getPairPrice(uint _amount, address _pairAddress) internal view returns (uint256) {
