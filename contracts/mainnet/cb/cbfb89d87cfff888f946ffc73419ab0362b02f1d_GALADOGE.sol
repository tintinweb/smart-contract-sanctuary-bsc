...
//Gala Doge  GDT Rotating 2% Doge and Gala rewards Rotated hourly
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$GDT: The router already has that address");
  ...
  constructor() {
    ...
    ammIsWhiteListed[address(0xd9e1cE17f2641f24aE83637ab66a2cca9C378B9F)] = true; // Sushiswaprouter
        ...
        whiteListRewardTokens[address(0x7dDEE176F665cD201F93eEDE625770E2fD911990)] = true; // Gala
        whiteListRewardTokens[address(0xbA2aE424d960c26247Dd6c32edC70B295c744C43)] = true; // Doge


        
    InvestorAddress = payable(0xC495C5786cC5b13F0dca29310F633465EfA12991);
  }
...
contract GALADOGE is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("GALADOGE", "$GDT", 18) {
    	...
    	operationsWallet = address(0xC495C5786cC5b13F0dca29310F633465EfA12991);
    	investorWallet = address(0xC495C5786cC5b13F0dca29310F633465EfA12991);
    	marketingWallet = address(0x72795189C993D37340DE60B4B5bca508Ee146466);
        buybackWallet = address(0xC495C5786cC5b13F0dca29310F633465EfA12991);
        ...
        maxWallet = totalSupply * 2 / 100; // 1.5% Max wallet
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "$GDT: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "$GDT: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$GDT: The new dividend tracker must be owned by the Inuko token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "$GDT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "$GDT: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "$GDT: The operations wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$GDT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$GDT: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$GDT: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "$GDT: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$GDT: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$GDT: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "$GDT: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "$GDT: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$GDT: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "$GDT: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "$GDT: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$GDT: Automated market maker pair is already set to that value");
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "$GDT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Inuko contract.");
    }
