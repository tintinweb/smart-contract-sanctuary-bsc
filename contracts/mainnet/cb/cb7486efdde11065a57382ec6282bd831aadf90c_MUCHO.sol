...
contract MUCHO is IERC20, Ownable {
    ...
    struct status{bool isFeesExempt; uint256 swapTime; 
    bool isBot; bool isInternal; bool isDividendExempt;}
    ...
    function checkCooldown(address sender, address recipient, uint256 amount) internal view {
        ...
        if(sender != pair && cooldownEnabled && bigBuyerWinningsCooldown[sender] && coolWinnings && coolBigBuy &&
        ...
        if(sender != pair && cooldownEnabled && bigBuyerWinningsCooldown[sender] && coolWinnings && 
        !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[sender].isFeesExempt){
            require(amount <= _balances[sender].sub(bigBuyWinningsCooldown[sender]), "Cooldown not reach in order to sell Big Buy Winnings");}
        if(sender != pair && coolBigBuy && bigBuyerWinningsCooldown[sender] && cooldownEnabled &&
        !isFeeExempt[recipient].isFeesExempt && !isFeeExempt[sender].isFeesExempt){
