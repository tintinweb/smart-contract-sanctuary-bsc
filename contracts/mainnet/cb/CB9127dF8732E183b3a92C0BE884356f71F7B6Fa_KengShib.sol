...
contract ShibDividendTracker is DividendPayingToken, Ownable {
    ...
    function withdrawDividend() public pure override {
        require(false, "ShibDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main KengShib contract.");
    }
...
contract KengShib is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("KengShib", "KengShib") {
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "KengShib: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "KengShib: The dividend tracker already has that address");

        ShibDividendTracker newDividendTracker = ShibDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KengShib: The new dividend tracker must be owned by the KengShib token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KengShib: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KengShib: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KengShib: Cannot update gasForProcessing to same value");
