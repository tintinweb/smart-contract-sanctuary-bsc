...
contract Bnbm is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the BNBM ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("bnbm.finance", "BNBM") {
        // Mints 1 BNBM to contract creator for initial pool setup
    ...
    function _getBnbmPrice() internal view returns (uint256 _bnbmPrice) {
        try IOracle(bnbmOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Bnbm: failed to fetch BNBM price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _bnbmPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_bnbmPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBnbmOracle(address _bnbmOracle) public onlyOperatorOrTaxOffice {
        require(_bnbmOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints BNBM to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BNBM to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBnbmPrice = _getBnbmPrice();
            currentTaxRate = _updateTaxRate(currentBnbmPrice);
            if (currentBnbmPrice < burnThreshold) {
                burnTax = true;
            }
        }
