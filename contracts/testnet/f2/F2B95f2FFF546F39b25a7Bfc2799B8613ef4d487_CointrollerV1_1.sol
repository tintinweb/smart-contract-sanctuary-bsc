 ...
 * @author Rifi
 */
contract CointrollerV1_1 is CointrollerStorage, CointrollerInterface, CointrollerErrorReporter, ExponentialNoError {
    ...
    //         return fail(Error.UNAUTHORIZED, FailureInfo.SET_PRICE_ORACLE_OWNER_CHECK);
    //     }
    //     bytes memory signature = this._setPriceOracle.selector;
    //     bytes memory data = abi.encodePacked(newOracle);
	// 	uint256 eta = block.timestamp + timelock.delay();
        
    //     timelock.queueTransaction(address(this), 0, signature, data, eta);
    // }
    ...
    function _setMintPaused(RToken rToken, bool state) public onlyTimeLock returns (bool) {
    ...
    function _setBorrowPaused(RToken rToken, bool state) public onlyTimeLock returns (bool) {
        require(markets[address(rToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");
    ...
    function _setTransferPaused(bool state) public onlyTimeLock returns (bool) {
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");
    ...
    function _setSeizePaused(bool state) public onlyTimeLock returns (bool) {
        require(msg.sender == pauseGuardian || msg.sender == admin, "only pause guardian and admin can pause");
        require(msg.sender == admin || state == true, "only admin can unpause");
    ...
    function _become(Unitroller unitroller) public onlyTimeLock {
     ...
     */
    function adminOrInitializing() internal view onlyTimeLock returns (bool) {
     ...
     */
    function _setRifiSpeed(RToken[] memory rTokens, uint[] memory supplySpeeds, uint[] memory borrowSpeeds) public onlyTimeLock {
     ...
     */
    function _setContributorRifiSpeed(address contributor, uint rifiSpeed) public onlyTimeLock {
