...
contract BscJungle {
    ...
    uint16 constant CAN_SET_MAX_REF_LEVELS = 5; 
    ...
    function setLaunchDate(uint40 _launch_time) external {
        require(msg.sender == contractOwner, "Owner only");
        require(LAUNCH_TIME == 0 || uint40(block.timestamp) < LAUNCH_TIME, "Contract has already been launched");
        require(isContractParamsSet, "Parameters must be set before setting launch time");
        require(isSealed, "Contract must be sealed before launch date is set");
    ...
    function _deposit(address _investor, uint8 _tarif, address _upline, uint256 amount) private {
        require(uint40(block.timestamp) >= LAUNCH_TIME, "Contract is not launched yet");
        require(_tarif >= TARIF_MIN_DAYS && _tarif <= TARIF_MAX_DAYS, "Tarif not found");
        ...
        require(MAXIMUM_DEPOSIT == 0 || amount <= MAXIMUM_DEPOSIT, "Must deposit maximum amount at most");
    ...
    function depositCoin(uint8 _tarif, address _upline) external payable {
        require(useNativeCoin, "Contract uses a token");
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[CAN_SET_MAX_REF_LEVELS] memory structure) {
        Player storage player = players[_addr];
        uint256 payout = this.payoutOf(_addr);
        for (uint8 i = 0; i < BONUS_LINES_COUNT; i++) {
