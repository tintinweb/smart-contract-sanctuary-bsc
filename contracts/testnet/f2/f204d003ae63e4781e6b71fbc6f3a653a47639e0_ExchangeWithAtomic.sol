...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function withdrawToPool(
        ...
        if(useUserBalance == true){
            amountA = LibUnitConverter.baseUnitToDecimal(assetAddressA,amountADesired);
            amountB = LibUnitConverter.baseUnitToDecimal(assetAddressBNoEth,amountBDesired);
            int deltaA = int(amountA) - int(assetBalances[msg.sender][assetAddressA]);
            int deltaB = int(amountB) - int(assetBalances[msg.sender][assetAddressBNoEth]);

            if(deltaA > 0){
                if (assetAddressA != address(0)) {
                    IERC20(assetAddressA).safeTransferFrom(
                        msg.sender,
                        address(this),
                        uint(deltaA)
                    );
            ...
            if(deltaB > 0){
                IERC20(assetAddressBNoEth).safeTransferFrom(
                    msg.sender,
                    address(this),
                    uint(deltaB)
                );
