...
contract Stakes is Owner, ReentrancyGuard {
    ...
    function end(uint256 _recordId) external nonReentrant {
        Record memory _stakeRecord = _findRecord(msg.sender, _recordId);

        require(_stakeRecord.ended==false, "Invalid stake");
        
        // penalization
        if(block.timestamp - _stakeRecord.from < maturity) {
            uint256 _penalization = _stakeRecord.amount * penalization / 100;
            require(asset.transfer(msg.sender, _stakeRecord.amount - _penalization));
            require(asset.transfer(getOwner(), _penalization));
            _stakeRecord.penalization = _penalization;
            _stakeRecord.to = block.timestamp;
            _stakeRecord.ended = true;
            emit StakeEnd(msg.sender, _stakeRecord.amount, _penalization, 0, _stakeRecord.id);
        // interest gained
        } else {
            uint256 _interest = get_gains(_stakeRecord.id);
    ...
    function get_gains(uint256 _recordId) public view returns (uint256) {
        Record memory _stakeRecord = _findRecord(msg.sender, _recordId);
    ...
    function ledger_records() public view returns (Record[] memory) {
        Record[] memory _stakeRecords = ledger[msg.sender];

        for (uint i = 0; i < _stakeRecords.length; i++) {
            _stakeRecords[i].gain = get_gains(_stakeRecords[i].id);
        }
