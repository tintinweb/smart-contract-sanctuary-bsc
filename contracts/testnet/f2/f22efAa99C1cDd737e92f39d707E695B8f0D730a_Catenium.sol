...
contract Catenium is AccessControl, ReentrancyGuard {
    ...
    constructor(uint256 feePercent_) {
    ...
    function placeTokens(uint256 nonce, uint256 price, IERC20Metadata token, uint256 initialVolume, bytes memory signature) public {
        ...
        require(msg.sender.code.length == 0, "Catenium: Deposits from contracts are prohibited");
        require(!nonces[nonce], "Catenium: Invalid nonce");
        require(!placedTokens[token].isActive, "Catenium: This token was already placed");
        require(initialVolume > 0, "Catenium: initial Volume must be >0");

        address signer = keccak256(abi.encodePacked(sender, address(token), initialVolume, price, nonce))
        .toEthSignedMessageHash().recover(signature);

        require(hasRole(SIGNER_ROLE, signer), "Catenium: Invalid signature");
    ...
    function _sendCollectedFunds(address sender, IERC20Metadata token) private {
        ...
        require (sender == placedToken.owner, "Catenium: You are not the owner of this token");

        bool success = payable(placedToken.owner).send(placedToken.collectedAmount);
        require(success, "Catenium: Transfer failed");
    ...
    function buyTokens(IERC20Metadata token) public payable nonReentrant {
        address sender = msg.sender;
        uint256 volume = msg.value;

        PlacedToken storage placedToken = placedTokens[token];

        require(placedToken.isActive == true, "Catenium: Round isn't active");

        bool success = payable(address(this)).send(volume);
        require(success, "Transfer failed");

        uint256 tokensAmount = volume * token.decimals() / placedToken.price;
        require(tokensAmount <= placedToken.volume, "Catenium: Not enough volume");
    ...
    function withdrawFees() public payable onlyRole(ADMIN_ROLE) {
