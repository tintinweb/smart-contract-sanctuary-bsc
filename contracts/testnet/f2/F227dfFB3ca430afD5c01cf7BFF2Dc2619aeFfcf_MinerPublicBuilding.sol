...
contract MinerPublicBuilding is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyMunicipality() {
        require(msg.sender == municipalityAddress, "MinerNFT: Only municipality is allowed to mint");
        _;
    }
    ...
    function mintBundle(address user, Municipality.Parcel[] memory parcels) external onlyMunicipality {
        ...
        for(uint256 i = 0; i < parcels.length; ++i) {
            ...
            for(uint256 j = 0; j < 4; ++j) {
                ...
                IMunicipality(municipalityAddress).attachMinerToParcel(miner, parcelId);
            }
        }
    }
}
...
interface IMunicipality {
    function attachMinerToParcel(uint256 minerId, uint256 parcelId) external;
}
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    address private ambassadorWalletAddress;
    address private reserveWalletAddress;
    address private companyWalletAddress;

    // Addresses of Gymstreet smart contracts
    address private standardParcelNFTAddress;
    address private businessParcelNFTAddress;
    address private minerV1NFTAddress;
    address private backendAddress;
    address private nftReflectionPoolAddress;
    address private netGymStreetAddress;
    address private miningAddress;
    address private nftRankRewardsAddress;
    address private minerPublicBuildingAddress;

    // Addresses of GymNet smart contracts
    address private gymTurnoverPoolAddress;
    address private buyAndBurnAddress;
    address private gymNetworkAddress;

    // Token addresses for interacint with liquidity pools
    address private vbtcAddress;
    address private busdAddress;
    address private vbtcBusdLpAddress;
    ...
    address private vBTCBuyAndBurnWallet;
    ...
    mapping(uint8 => uint256[10]) private operationTypeToPercentagesMapping;
    ...
    event SetBundles(BundleInfo[6] _bundles);
    event SetNFTContractAddresses(address[11] _nftContractAddresses);
    event SetDistributionReceiverAddresses(address[10] _nftContractAddresses);
    event SetParcelsSlotsCount(
        uint8 indexed _standardParcelSlotsCount,
        uint8 indexed _upgradedParcelSlotsCount
    );
    event SetPurchasePrices(
        uint256 _upgradePrice,
        uint256 _minerPrice,
        uint256 _minerRepairPrice,
        uint256 _electricityVoucherPrice
    );
    event SetSaleActivation(bool indexed _saleActivation);

    /// @notice Modifier for access by the whitelisted addresses only
    modifier onlyWhitelisted() {
        require(
            whitelistedAddresses[msg.sender] == true,
            "Municipality: Only whitelisted addresses allowed to access"
        );
        _;
    }
    ...
    modifier onlyMinerPublicBuilding() {
        require(msg.sender == minerPublicBuildingAddress, "Municipality: This functionn is available only to a miner public building");
        _;
    }
    ...
    function setBundles(BundleInfo[6] calldata _bundles) external onlyOwner notZeroAddress {
        ...
        emit SetBundles(_bundles);
    }

    /// @notice Set contract addresses for all NFTs we currently have
    function setNFTContractAddresses(address[11] calldata _nftContractAddresses) external onlyOwner {
        standardParcelNFTAddress = _nftContractAddresses[0];
        ...
        backendAddress = _nftContractAddresses[4];
        routerAddress =  _nftContractAddresses[5];
        busdAddress = _nftContractAddresses[6];
        wbnbAddress = _nftContractAddresses[7];
        vbtcAddress = _nftContractAddresses[8];
        gymNetworkAddress = _nftContractAddresses[9];
        minerPublicBuildingAddress = _nftContractAddresses[10];

        emit SetNFTContractAddresses( _nftContractAddresses);
    }

    /// @notice Set addresses to all smart contract and wallet addresses we currently distribute the amounts to
    function setDistributionReceiverAddresses(address[10] calldata _distributionReciverAddresses) external onlyOwner {
        ...
        vBTCBuyAndBurnWallet = _distributionReciverAddresses[8];
        liquidityWallet = _distributionReciverAddresses[9];

        emit SetDistributionReceiverAddresses(_distributionReciverAddresses);
    }
    ...
    function setParcelsSlotsCount(uint8[2] calldata _parcelsSlotsCount) external onlyOwner {
        ...
        emit SetParcelsSlotsCount(_parcelsSlotsCount[0], _parcelsSlotsCount[1]);
    }
    ...
    function setSaleActivation(bool _saleActivation) external onlyOwner {
        ...
        emit SetSaleActivation(_saleActivation);
    }
    ...
    function mintParcels(ParcelsMintSignature calldata _mintingSignature, uint256 _referrerId)
        ...
        notZeroAddress
    {
        _verifySigner(_mintingSignature);
    ...
    function attachMinerToParcel(uint256 minerId, uint256 parcelId) external onlyMinerPublicBuilding {
...
/// @notice Buy and burn previously distributed amount
    ...
    function _verifySigner(ParcelsMintSignature memory mintParcelSignature) private view {
        require(
            mintParcelSignature.parcels.length == mintParcelSignature.signatures.length,
            "Municipality: Number of signuatures does not match number of parcels"
        );
        for (uint256 index; index < mintParcelSignature.parcels.length; index++) {
            ...
            require(signer == backendAddress, "Municipality: Unauthorized signer");
        }
    }
