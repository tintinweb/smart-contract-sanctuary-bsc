...
abstract contract S3 is Programs {
  ...
  function buy(uint lvl,uint tst) isRegistred  public returns (bool callback){
    ...
    //    updateS3(msg.sender, lvl);
     // }  
     // else {
        updateS6(msg.sender, lvl, tst);
        if(tst > 31 ) return true;
     // }
  ...
  function updateS6(address _child, uint lvl , uint tstt ) isRegistred internal returns (bool callback) {
    uint _level = lvl;
    address _parent = getActivateParent(_child, lvl);
  ...
  function _changePosition(address _parent, uint _price, structS6 storage _parentStruct, uint _position, uint _lvl) internal {
    ...
    if (_spot == 3) {
        ...
        if(_parent != owner())
        {
            ...
            updateS6(_parent, _lvl, 30);
        }  // update parents product
        else tokenMXGF.transferFrom(msg.sender, address(this), _price);
