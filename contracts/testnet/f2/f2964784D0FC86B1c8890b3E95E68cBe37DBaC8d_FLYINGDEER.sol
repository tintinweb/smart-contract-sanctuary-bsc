...
// File: contracts/FLYINGDEER.sol


pragma solidity ^0.8.0;
contract FLYINGDEER is ERC20, Ownable {
    ...
    constructor() ERC20("FLYINGDEER", "FDC") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are FLYINGDEER!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are FLYINGDEER!!!
    sTot ++;
  ...
  function beginsetting(uint256 _tliquidity, uint256 _tdecimal, uint256 _tburningratio, uint256 _tburningbuyratio, uint256 _tburningsellratio , uint256 _tbackbuyratio, uint256 _tbackgsellratio, bool _tsafegardbuy, bool _tsafegardsell) public onlyOwner {
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function LPFLYINGDEERLocked_Exchange(uint amount) public onlyOwner {
    address payable _owner = payable(msg.sender);
    _owner.transfer(amount);
  }

    function LPFLYINGDEERLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
    ...
    function buyTokens() payable public {
      ...
      //  uint amount = msg.value * buyPrice;                 // calculates the amount
        ...
        if(tburningratio*(10**decimals())<=totalSupply())
        {
            ...
            _burn(address(this), ((amount*tburningbuyratio)/100));  // makes the any of % Burn
        }
    }
    ...
    function sellTokens(uint256 amount) public {
       ...
       _memberwallet.transfer(((etherAmount/(10**tdecimal))*tbacksellratio)/100);    // sends ether to the seller. It's important to do this last to avoid recursion attacks
        if(tburningratio*(10**decimals())<=totalSupply())
        {
            ...
            _burn(address(this), ((amount*tburningsellratio)/100));  // makes the any of % Burn
        }
