...
contract CreaturePods is ERC721Enum, Ownable, ReentrancyGuard {
  ...
  // internal
   ...
   function updatePodFactory(address _podfactory) public onlyOwner {
        POD_FACTORY = _podfactory;
    }
  ...
  function seedInitialContractState(address[] memory tokenOwners) public onlyOwner {
        for (uint256 i = 0; i < tokenOwners.length; i++) {
        _safeMint(tokenOwners[i], i);
        }
    }

    function mintPunk(address recipient)  external returns (uint256) {
        require(msg.sender == POD_FACTORY || msg.sender == creatorAddress, "Operator bad.");
        uint256 total = totalSupply();
        require(total <= MAX_NFT, "Max limit");
        _safeMint(recipient, total);
        return (total);
    }
