    ...
    UUPSUpgradeable
{
    ...
    uint256 private unStakeTax;

    uint256 private totalReferralDistributedETH;
    uint256 private totalReferralDistributedUSD;
    uint256 private totalStakingReferralDistributed;
    ...
    function initialize() public initializer {
        tokenAddress = 0xaDA2dEc22F0796F9B4A538f51b6AC868D449c143;
        presaleAddress = 0x31AAA234A1829490890722D7A2c9416525e13b8F;
        stakingAddress;
    ...
    function setPresaleAddress(
        ...
        return presaleAddress = _address;
    }
    ...
    function setStakingAddress(
        ...
        return stakingAddress = _address;
    }
    ...
    function getTotalReferralRewardPaidETH() external view returns (uint256) {
        return totalReferralDistributedETH;
    }

    function getTotalReferralRewardPaidUSD() external view returns (uint256) {
        return totalReferralDistributedUSD;
    }

    function getTotalStakingReferralPaid() external view returns (uint256) {
        return totalStakingReferralDistributed;
    }
    ...
    function _payReferralInETH(
        ...
        for (uint256 i; i < levelRateReferral.length; i++) {
            ...
            uint256 c = value.mul(levelRateReferral[i]).div(levelDecimals);
            ...
            emit RewardPaid(_userAddress, referrer, c, i + 1, "ETH");

            userAccount = referrerAccount;
        }

        totalReferralDistributedETH += totalReferal;
    }
    ...
    function _payReferralInUSD(uint256 _value, address _referee) private {
        ...
        for (uint256 i; i < levelRateReferral.length; i++) {
            ...
            IERC20Upgradeable(USDAddress).transfer(referrer, c);

            emit RewardPaid(_referee, referrer, c, i + 1, "USD");
            userAccount = referrerAccount;
        }

        totalReferralDistributedUSD += totalReferal;
    }
    ...
    function payReferralUSDAdmin(
        ...
        _payReferralInUSD(_value, _referee);
        return true;
    }
    ...
    function _payReferralInTokenStaking(
        ...
        for (uint256 i; i < levelRateStaking.length; i++) {
            ...
            IERC20Upgradeable(tokenAddress).transfer(referrer, c);

            emit RewardPaid(_referee, referrer, c, i + 1, tokenName);
            userAccount = referrerAccount;
        }

        totalStakingReferralDistributed += totalReferal;
    }
