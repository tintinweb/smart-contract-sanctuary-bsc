    ...
    TransformAmount
{
    ...
    /// @dev withdraw amount of tokens
    // mapping(uint256 => uint256) public _withdrawPerTokenId;

    mapping(address => mapping(uint256 => uint256)) public _withdrawPerTokenId;

    uint256 public _monthsOfUtility = 37;
    ...
    function stakeNft(uint256 _StakeId, uint256 _tokenId) external noReentrant {
        ...
        /// @dev get the address of the NFT
        uint256 currentRetirement = _withdrawPerTokenId[stake.addressNft][
            _tokenId
        ];

        require(
            currentRetirement <= _monthsOfUtility,
            "Stake NFT: Can't do more withdrawal"
        );
    ...
    function withdrawNFTStake(uint256 index) external noReentrant {
        ...
        uint256 currentRetirement = _withdrawPerTokenId[addressNft][idNft];

        require(
            currentRetirement <= _monthsOfUtility,
            "Withdraw NFT Stake: not available"
        );
        ...
        /// @dev add count of withdrawals
        _withdrawPerTokenId[addressNft][idNft] = currentRetirement.add(1);
    }

    /// @dev set Months Of Utility
    function setMonthsOfUtility(uint _amount) external onlyUser {
        _monthsOfUtility = _amount;
    }

    /// @dev set current retirement for a token
    function setCurrentRetirement(
        address _scNft,
        uint _amount,
        uint _idNft
    ) public {
        _withdrawPerTokenId[_scNft][_idNft] = _amount;
    }
}
...
contract FactoryStakeNft is Administered {
    ...
    function EditStaked(
        ...
        if (_type == 1) {
            _Stake[_id].addressNft = _address;
        } else if (_type == 2) {
            _Stake[_id].addressTokenReward = _address;
        } else if (_type == 3) {
            _Stake[_id].decimalsTokenReward = _amount;
        } else if (_type == 4) {
            _Stake[_id].rewardTotal = _amount;
        } else if (_type == 5) {
            _Stake[_id].day = _amount;
        } else if (_type == 6) {
            _Stake[_id].min = _amount;
        } else if (_type == 7) {
            _Stake[_id].max = _amount;
        } else if (_type == 8) {
