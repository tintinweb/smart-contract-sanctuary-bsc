...
contract AvionCollection is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    uint256 public whitelistPresalePeriodLimit = 1650;
    uint256 public publicPresalePeriodLimit = 3150;
    uint256 public currentSupply;
    uint256 mintFeeTreasuryPercent = 700;
    uint256 mintFeeDevWalletPercent = 300;
    string BASE_URI;
    bool public allowedToMint;
    bool public whitelistPresalePeriod;
    bool public publicPresalePeriod;

    mapping(uint256 => uint256) totalNftMinted;
    mapping(address => uint256) userNftMinted;
    mapping(address => uint256) userNftMintedWhitelist;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        if (random <= 750 && totalNftMinted[3] < 750 ) mintInternal(3, VRFRequests[requestId]);
        if (random <= 3000 && totalNftMinted[2] < 2250 ) mintInternal(2, VRFRequests[requestId]);
    ...
    function mintAtPublicPresalePeriod(uint256 _amount) external {
        ...
        require(userNftMinted[msg.sender]  + _amount <= 3, "AC: maximum amount of nft per wallet exceeded");
        _mint(_amount, 290 ether * _amount);
        userNftMinted[msg.sender] += _amount;
    }
    ...
    function mintAtWhitelistPresalePeriod(bytes32[] calldata merkleProof, uint256 _amount) external {
        ...
        require(userNftMintedWhitelist[msg.sender]  + _amount <= 3, "AC: maximum amount of nft per wallet exceeded");
