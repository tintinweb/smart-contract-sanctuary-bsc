    ...
    ERC721URIStorageUpgradeable
{
    ...
    mapping(uint256 => uint256) public arcusPackIds;

    event ArcusPackMinted(
    ...
    function initialize(string memory baseURI_) public initializer {
        __ERC721_init("Arcus Packs", "ARCPACKS");
        __Pausable_init();
    ...
    function mintN(
        ...
        for (uint256 i = 0; i < _amount; i++)
            tokenIds[i] = mint(_minter, _packId);
    }

    function mint(address _minter, uint256 _packId)
        public
        gameMasterOnly
        whenNotPaused
        returns (uint256 tokenId)
    {
        ...
        arcusPackIds[tokenId] = _packId;

        emit ArcusPackMinted(
            tokenId,
            _packId,
            _minter,
    ...
    function getPackId(uint256 _tokenId) public view returns (uint256) {
        return arcusPackIds[_tokenId];
    }
...
interface IArcusNFT is IERC721Upgradeable {
    ...
    function mint(address _minter, uint256 _edition) external;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    CountersUpgradeable.Counter private _packIdCounter;
    EnumerableSet.AddressSet private allowedPaymentTokens;
    EnumerableSet.AddressSet private allowedNFTs;
    EnumerableSet.UintSet private activePackIds;

    address public treasury;
    ArcusPack public arcusPack;
    SeedGenerator public seedGenerator;

    mapping(address => TokenType) tokenTypes;
    mapping(address => uint256) lastBlockNumberCalled;
    mapping(uint256 => NFTPack) public nftPacks;
    mapping(uint256 => uint256) public nftPacksType; // 1 = package, 2 = gacha
    mapping(uint256 => PackItem[]) nftPackItems;
    ...
    function _getRandomItems(uint256 _seed, uint256 _packId)
        private
        view
        returns (uint256[] memory)
    {
        PackItem[] memory packItems = nftPackItems[_packId];
        uint256[] memory selectedItems = new uint256[](
            nftPacks[_packId].maxItems
        );
        uint256 roll;
        uint256 cumulativeWeight;
        for (uint256 i = 0; i < nftPacks[_packId].maxItems; i++) {
            roll =
                Random.combineSeeds(_seed, i) %
                nftPacks[_packId].totalWeight;
            cumulativeWeight = 0;
            for (uint256 j = 0; j < packItems.length; j++) {
                cumulativeWeight += packItems[j].chance;
        ...
        returns (uint256[] memory)
    {
        ...
        uint256[] memory itemIds = new uint256[](packItems.length);
        for (uint256 i = 0; i < packItems.length; i++) {
    ...
    function _mintSelectedItems(
        ...
        for (uint256 i = 0; i < _selectedItems.length; i++) {
            IArcusNFT(nftPackItems[_packId][_selectedItems[i]].nftAddress)
                .mintFromPack(
                    _minter,
                    nftPackItems[_packId][_selectedItems[i]].edition,
                    nftPackItems[_packId][_selectedItems[i]].tradeable
                );
        }
    }
    ...
    function getCurrentPackId() public view returns (uint256) {
        return _packIdCounter.current();
    }

    function getNftPackItems(uint256 _packId)
        public
        view
        returns (PackItem[] memory)
    {
        return nftPackItems[_packId];
    }

    function getNftPackIds()
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory packIds = new uint256[](getCurrentPackId());
        for (uint i = 0; i < getCurrentPackId(); i++) {
            packIds[i] = i.add(1);
        }
        return packIds;
    }

    function getActivePackIds() public view returns (uint256[] memory) {
        EnumerableSet.UintSet storage set = activePackIds;
        uint256[] memory packIds = new uint256[](set.length());
    ...
    function newNftPack(
        ...
        require(_packType > 0, "E7001");
        _packIdCounter.increment();
        uint256 packId = _packIdCounter.current();
        NFTPack storage nftPack = nftPacks[packId];
        nftPack.price = _price;
        nftPack.paymentToken = _paymentToken;
        nftPack.maxItems = _maxItems;
        nftPack.active = _active;
        nftPacksType[packId] = _packType;
    ...
    function addPackItem(uint256 _packId, PackItem memory _packItem)
        public
        gameMasterOnly
    {
        require(allowedNFTs.contains(_packItem.nftAddress), "E4003");
        require(_packItem.quantity > 0, "E7003");
        nftPackItems[_packId].push(_packItem);
        nftPacks[_packId].totalWeight = nftPacks[_packId].totalWeight.add(
            _packItem.chance
        );
    }

    function mintNftPack(uint256 _packId) public nonReentrant {
        require(nftPacks[_packId].active, "E7004");
        require(
            IERC20(nftPacks[_packId].paymentToken).balanceOf(msg.sender) >=
                nftPacks[_packId].price,
            "E3002"
        );
        arcusPack.mint(msg.sender, _packId);
        if (nftPacks[_packId].price > 0) {
            IERC20(nftPacks[_packId].paymentToken).transferFrom(
                msg.sender,
                treasury,
                nftPacks[_packId].price
            );
    ...
    function openNftPack(uint256 _tokenId) public nonReentrant {
        require(arcusPack.ownerOf(_tokenId) == msg.sender, "E7006");
        uint256 packId = arcusPack.getPackId(_tokenId);
        require(packId > 0, "E7005");
        uint256[] memory selectedItems;
        if (nftPacksType[packId] == 1) {
            selectedItems = _getPackItems(packId);
        } else {
            selectedItems = _getRandomItems(
                seedGenerator.popSingleSeed(
                    address(this),
                    Random.combineSeeds(PACK_RANDOM_SEED, _tokenId),
                    true,
                    true
                ),
                packId
        ...
        _mintSelectedItems(selectedItems, packId, msg.sender);
        arcusPack.burn(_tokenId);
        emit NFTPackOpened(
            packId,
            _tokenId,
            msg.sender,
    ...
    function generateSeed(uint256 _tokenId) public nonReentrant {
        seedGenerator.requestSingleSeed(
            address(this),
            Random.combineSeeds(PACK_RANDOM_SEED, _tokenId)
        );
    }

    function hasSeedRequested(uint256 _tokenId) public view returns (bool) {
        return
            seedGenerator.hasSingleSeedRequest(
                address(this),
                Random.combineSeeds(PACK_RANDOM_SEED, _tokenId)
            );
    }

    function updateNftPack(
        uint256 _packId,
        address _paymentToken,
        uint256 _price,
        uint256 _packType,
        ...
        require(_packId > 0, "E7005");
        require(_packType > 0, "E7001");
        if (nftPacks[_packId].paymentToken != _paymentToken) {
            nftPacks[_packId].paymentToken = _paymentToken;
        }
        if (nftPacks[_packId].price != _price) {
            nftPacks[_packId].price = _price;
        }
        if (nftPacksType[_packId] != _packType) {
            nftPacksType[_packId] = _packType;
        }
        if (nftPacks[_packId].maxItems != _maxItems) {
            nftPacks[_packId].maxItems = _maxItems;
        }
        if (nftPacks[_packId].active != _active) {
            nftPacks[_packId].active = _active;
            if (!_active) {
                activePackIds.remove(_packId);
            } else {
                activePackIds.add(_packId);
            }
        }
    }

    function updatePackItem(uint256 _packId, uint256 _itemIndex, PackItem memory _packItem) public gameMasterOnly {        
        require(_packId > 0, "E7005");
        require(_itemIndex < nftPackItems[_packId].length, "E7008");
        if (nftPackItems[_packId][_itemIndex].nftAddress != _packItem.nftAddress) {
            require(allowedNFTs.contains(_packItem.nftAddress), "E4003");
            nftPackItems[_packId][_itemIndex].nftAddress = _packItem.nftAddress;
        }
        if (nftPackItems[_packId][_itemIndex].chance != _packItem.chance) {
            uint256 totalWeight = nftPacks[_packId].totalWeight;
            totalWeight = totalWeight.sub(nftPackItems[_packId][_itemIndex].chance);
            totalWeight = totalWeight.add(_packItem.chance);
            nftPackItems[_packId][_itemIndex].chance = _packItem.chance;
            nftPacks[_packId].totalWeight = totalWeight;
        }        
        if (nftPackItems[_packId][_itemIndex].quantity != _packItem.quantity) {
            nftPackItems[_packId][_itemIndex].quantity = _packItem.quantity;
        }        
        if (nftPackItems[_packId][_itemIndex].edition != _packItem.edition) {
            nftPackItems[_packId][_itemIndex].edition = _packItem.edition;
        }        
        if (nftPackItems[_packId][_itemIndex].tradeable != _packItem.tradeable) {
            nftPackItems[_packId][_itemIndex].tradeable = _packItem.tradeable;
        }
    }
}
