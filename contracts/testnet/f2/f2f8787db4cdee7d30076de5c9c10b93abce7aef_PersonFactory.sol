...
contract PersonFactory is  Ownable {
    ...
    function getDaySumAmount(uint _version) public view returns(uint){
        require(_version>=0,"day must bigger than 0.");
        uint _amountSum = initSumAmount.mul(oneU);
        for(uint i = 0;i<_version;i++){
            _amountSum = _amountSum.add(_amountSum.mul(createPercent).div(100));
        }
        return _amountSum;
    }
    ...
    function getVersionSumAmount(uint _pLevel,uint _version) public view returns(uint){
        ...
        for(uint i = 0 ; i<_version;i++){
            _initAmount = _initAmount.add(_initAmount.mul(13).div(10));
        }
        return _initAmount;
    }
