...
contract TikiTakaFinance is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Tiki Taka Token";
    string private constant _symbol = "TTT";
    uint8 private constant _decimals = 9;
    uint256 private constant MAX = 15000000000 * 10**18;
    uint256 private _tTotal = 1500000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _tFeeTotal;
    uint256 public _TikiTakaTokenBurned;
    ...
    uint256 private _reflection = 10;
    uint256 private _contractFee = 5;
    uint256 private _TikiTakaTokenBurn = 5;
    ...
    /// @notice address set to this mapping, would not be paying fees on transfers 
   ...
   /// @notice this function would be used to blacklist an address, addresses that are blacklisted cannot perfolrm transfers 
    ...
    function addLiquidity() external onlyOwner() {
        ...
        feeEnabled = true;
        limitTX = true;
        _maxTxAmount = 1500000000 * 10**18;
        _maxBuyAmount = 1500000000 * 10**18; //100% buy cap
   ...
   function manualSwapTokensForEth() external onlyOwner() {
    ...
    function removeAllFee() private {
        if (_reflection == 10 && _contractFee == 5 && _TikiTakaTokenBurn == 5) return;
        _reflection = 0;
        _contractFee = 0;
        _TikiTakaTokenBurn = 0;
    }

    function restoreAllFee() private {
        _reflection = 10;
        _contractFee = 5;
        _TikiTakaTokenBurn = 5;
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 amount) private {
        (uint256 tAmount, uint256 tBurn) = __TikiTakaFinanceEthBurn(amount);
   ...
   function __TikiTakaFinanceEthBurn(uint amount) private returns (uint, uint) {
        uint orgAmount = amount;
        uint256 currentRate = _getRate();
        uint256 tBurn = amount.mul(_TikiTakaTokenBurn).div(100);
        uint256 rBurn = tBurn.mul(currentRate);
        _tTotal = _tTotal.sub(tBurn);
        _rTotal = _rTotal.sub(rBurn);
        _TikiTakaTokenBurned = _TikiTakaTokenBurned.add(tBurn);
