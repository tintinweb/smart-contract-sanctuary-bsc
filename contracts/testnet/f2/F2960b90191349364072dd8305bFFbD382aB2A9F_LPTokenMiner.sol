...
contract LPTokenMiner is Ownable {
    ...
    function bindRelationShip(address _one, address _two) internal {
    ...
    function deposit(uint256 amount, uint256 option, address one_, address two_) external { //需要先授权
        if (!getRelationExist(msg.sender)){
            bindRelationShip(one_, two_);
        }
