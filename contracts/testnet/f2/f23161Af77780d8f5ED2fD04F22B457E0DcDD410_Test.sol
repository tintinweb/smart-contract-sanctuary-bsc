...
constructor (){
    ...
    function getblanceOflp (address lpaddress) public {
        if(_lplocked[msg.sender][lpaddress] > 0 && block.timestamp >= _lplockedtime[msg.sender][lpaddress].add(_lplockeddays.mul(60))){
            IERC20(lpaddress).transfer(msg.sender,_lplocked[msg.sender][lpaddress]);
