...
contract provideHelp is Context, Ownable {
    ...
    event provideHelpEvent(address indexed user,uint256 oderId,uint256 _helpPoolId,uint256 amount,uint256 paidAmount,uint256 userId,uint256 time);
    ...
    constructor()  {
        for (uint256 i = 0; i < 10; i++) {
            ...
            _currentUserID++;
            users[ownerWallets[i]].totalHelp.push(HelpPoolStruct({id:_helpPoolId,oderId:_helpPoolId,amount: amounts[i],paidAmount: amounts[i],isPaid: true,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
            users[ownerWallets[i]].totalHelp.push(HelpPoolStruct({id:_helpPoolId,oderId:_helpPoolId,amount: amounts[i],paidAmount: amounts[i],isPaid: true,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
            _helpPoolId++;
             emit provideHelpEvent(ownerWallets[i],_helpPoolId,_helpPoolId,amounts[i],amounts[i],users[ownerWallets[i]].userId,block.timestamp);
    ...
    function provideHelpFunction(uint256 amount,uint256 oderId,uint256 helpId) public payable {
        ...
        users[_msgSender()].totalHelp.push(HelpPoolStruct({oderId:oderId,id:_helpPoolId,amount: amount,paidAmount: msg.value,isPaid: false,userId:users[_msgSender()].userId,userAddress:_msgSender(),isWithdrawal:false,time:block.timestamp}));
        emit provideHelpEvent(_msgSender(),oderId,_helpPoolId,amount,msg.value,users[_msgSender()].userId,block.timestamp);
        _helpPoolId++;
        clearGetHelp(helpId,msg.value);
    }

    //pay 80% amount
    function completeHelp(uint256 helpPoolId,uint256 getHelpId) public payable{
        ...
        users[_msgSender()].totalInvested=users[_msgSender()].totalInvested+msg.value;
        clearGetHelp(getHelpId,msg.value);
    ...
    function percentage(uint256 price, uint256 per) internal pure returns (uint256) {
     ...
     function clearGetHelp(uint256 helpId,uint256 amount) public payable{
