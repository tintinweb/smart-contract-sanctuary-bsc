    ...
    IERC721ReceiverUpgradeable
{
    ...
    // whenNotPaused
    {
        ...
        uint256 _points = _reBalance(_amount);

        //update the allocation point of the staker in storage
        allocPoints[msg.sender] += _points;
    ...
    function _reBalance(uint256 _amount) public view returns (uint256 _points) {
        for (uint256 i; i < tiers.length; i++) {
            if (_amount >= tiers[i].stake) {
