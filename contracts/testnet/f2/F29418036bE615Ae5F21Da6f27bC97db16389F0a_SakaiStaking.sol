...
contract SakaiStaking is ERC20, Ownable {
    ...
    uint256 public minimumStakeEqualtoUSDT;
    uint256 public interestRate;
    uint256 public interestRateDenominator;

    uint256 public bnbFee;
    address public gasWallet = 0x95469E2B56b0aD585EE3E88CBb7f45F888267945;
    address public treasuryWallet = 0x6b269c07e8f94f0fa1769CBD362879AFeA0206dB;
    ...
    event BNBFeeUpdated(uint256 amount);
    event StakingTokenUpdated(address indexed stakingToken);
    event GasWalletUpdated(address indexed gasWallet);
    event PoolStarted(uint256 timeStart, uint256 timeEnd);
    event RewardClaimed(address[] account, uint256[] amount);
    event DepositStaking(address indexed account, uint256 amount);
    event WithdrawStaking(address indexed account, uint256 balance);

    constructor() ERC20("Sakai Staking Protocol", "Sakai-SP") {
        address routerAddress = getRouterAddress();
        uniswapV2Router = IUniswapV2Router02(routerAddress);

        minimumStakeEqualtoUSDT = 1 * (10**18);
        interestRate = 8;
    ...
    function claimStuckTokens(address token) external onlyOwner {
        ...
        if (token == stakingToken && isStart) {
            ...
            IERC20 ERC20StakingToken = IERC20(token);
    ...
    function getAvailableRefundToken() public view returns (uint256) {
        require(stakingToken != address(0), "Staking Token is not set yet");
    ...
    function getRewardPerDay(uint256 amount) internal view returns (uint256) {
        uint256 rewardPerDay = (amount * interestRate) /
    ...
    function _getForecastRewardByAmount(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 rewardPerDay = getRewardPerDay(amount);
        uint256 availableDay = (timeEnd - block.timestamp) / 1 hours;
        uint256 forecastReward = rewardPerDay * availableDay;
    ...
    function setBNBFee(uint256 amount) external onlyOwner {
        require(amount != bnbFee, "BNB Fee already on that amount");
        require(
            amount <= 2 * (10**16),
            "Cannot set BNB Fee more than 0.02 BNB"
        );

        bnbFee = amount;

        emit BNBFeeUpdated(amount);
    }
    ...
    function setGasWallet(address _gasWallet) external {
        require(
            msg.sender == address(gasWallet),
            "Only current gas wallet can change gas wallet"
        );
        require(_gasWallet != gasWallet, "Gas wallet is already that address");
        require(
            _gasWallet != address(0),
            "Gas wallet cannot be the zero address"
        );

        gasWallet = _gasWallet;

        emit GasWalletUpdated(gasWallet);
    }
    ...
    function startPool() external onlyOwner {
        ...
        timeEnd = timeStart + 1 days;

        emit PoolStarted(timeStart, timeEnd);
    }
    ...
    function claimReward(address[] calldata account, uint256[] calldata amount)
        external
    {
        require(
            msg.sender == address(gasWallet),
            "Only gas wallet can claim reward for user"
        );
        ...
        for (uint256 i = 0; i < account.length; i++) {
            ...
            require(
                thisAmount <= totalAllocationReward,
            ...
            totalAllocationReward -= thisAmount;
    ...
    function deposit(uint256 amount) external payable {
        ...
        uint256 forecastAmount = _getForecastRewardByAmount(amount);

        require(
            availableTokenLeft > forecastAmount,
            "Reward Amount not enough for deposit amount, please contact owner for Deposit Reward Allocation"
        );

        require(
            msg.value >= bnbFee && msg.value <= 2 * (10**16),
            "msg.value must be between bnbFee and 0.2"
        );
        ...
        if (_stakers[msg.sender].balance == 0) {
            ...
            require(
                amountsOut[1] >= minimumStakeEqualtoUSDT,
                "Minimum amount to stake is equal to 100 USDT"
            );
            ...
            _stakers[msg.sender] = Stake(msg.sender, 0, timeNow, timeNow, 0);
        }
        ...
        emit DepositStaking(msg.sender, amount);
    }
    ...
    function withdraw() external payable {
        ...
        emit WithdrawStaking(msg.sender, balance);
    }
}