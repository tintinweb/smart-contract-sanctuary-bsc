...
contract vPrototype {
    ...
    function deposit(uint256 tier_index, address ref) public payable {
        ...
        uint256 watts_amount = convertToWatts(after_tax).div(1 ether);
    ...
    function convertToWatts(uint256 _bnb_amount) public view returns(uint256){
        return _bnb_amount.mul(exchangeMultiplier);
    }

    function convertToBnb(uint256 _watts_amount) public view returns(uint256){
        return _watts_amount.div(exchangeMultiplier).mul(1 ether);
    }
