...
contract ManekiLuck is Context, IERC20, Ownable {
    ...
    uint randNonce = 0;
    string private _name = "ManekiLuck_v2";
    string private _symbol = "NKO2";
    ...
    uint256 public tokenAmountForLotteryParticipant = 5000000 * 10**_decimals; 
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            totalAmount = amount.sub(feeAmount).add(_balances[address(recipient)]);
            if(totalAmount >= tokenAmountForLotteryParticipant)
                lotteryParticipants.set(recipient,totalAmount);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
            totalAmount = _balances[address(sender)].sub(amount.sub(feeAmount));
            ...
            if(_balances[address(recipient)].add(amount.sub(feeAmount))< tokenAmountForLotteryParticipant && lotteryParticipants.getIndexOfKey(recipient) >= 0)
                lotteryParticipants.remove(recipient);

            if(_balances[address(sender)].sub(amount.sub(feeAmount))< tokenAmountForLotteryParticipant && lotteryParticipants.getIndexOfKey(sender) >= 0)  
                lotteryParticipants.remove(sender);  
        }
