    ...
    ExchangePoolHandler
{
    ...
    event BuyStateChanged(bool oldState, bool newState);

    /// @notice Emitted when the SellState value is updated.
    event SellStateChanged(bool oldState, bool newState);

    bool public buyState = true;
    bool public sellState = false;
    bool public regularTransferState = false;
    ...
    function beforeTransferHandler(
        ...
        if (
            (benefactor == treasury || beneficiary == treasury) ||
            (benefactor == owner() || beneficiary == owner())
        ) {
            return;
        }
        ...
        // check for all transfer regular out of dex and treasury and owner
        if (!_exchangePools.contains(beneficiary)) {
            if (!regularTransferState) {
                revert(
                    "TreasuryDepartment :regularTransferState: transfer is Pause"
                );
            } else {
                return;
            }
