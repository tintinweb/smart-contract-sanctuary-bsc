...
contract GOTSToken is Context, IERC20, Ownable {
    ...
    address public buyFeeAddress = 0xA41d4e94861D5415B9070F964f98D290E3CbD95B;
    address public sellFeeAddress = 0xA41d4e94861D5415B9070F964f98D290E3CbD95B;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(lpPoolAddress != address(0) && !whiteList[sender] && (lpPoolAddress == recipient || lpPoolAddress == sender)){
            if(lpPoolAddress == recipient){ // sell 
                uint256 sellFeeAmount = amount.mul(8).div(100);
                _balances[sellFeeAddress] = _balances[sellFeeAddress].add(sellFeeAmount);
                emit Transfer(sender, sellFeeAddress, sellFeeAmount);
                realRecipientAmount=realRecipientAmount.sub(sellFeeAmount);
            }else{ // buy
                uint256 buyFeeAmount = amount.mul(13).div(100);
                _balances[buyFeeAddress] = _balances[buyFeeAddress].add(buyFeeAmount);
                emit Transfer(sender, buyFeeAddress, buyFeeAmount);
                realRecipientAmount=realRecipientAmount.sub(buyFeeAmount);
            }
        }
