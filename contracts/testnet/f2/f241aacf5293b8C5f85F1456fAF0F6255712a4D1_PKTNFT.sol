...
contract PKTNFT is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    // A mapping of the "approved" address for each token
    mapping (uint256 => address) internal tokensForSale;
    ...
    constructor(address creator_, string memory customBaseURI_, uint256 MAX_SUPPLY_, uint256 MAX_MULTIMINT_) ERC721("PKTNFT", "PKTN") {
        _creator = creator_;
        customBaseURI = customBaseURI_;
        MAX_SUPPLY = MAX_SUPPLY_;
        MAX_MULTIMINT = MAX_MULTIMINT_;
    }

    /** MINTING **/
    // uint256 public constant PRICE = 2000000000000000;
    function safeMint(uint256 tokenId) public nonReentrant onlyOwner {
        require(MAX_SUPPLY !=0 && totalSupply() + 1 < MAX_SUPPLY, "Exceeds max supply");
        // require(msg.value >= PRICE, "Insufficient payment, 0.002 ETH per item");

        _safeMint(msg.sender, tokenId);
    }
    function safeMultiMint(uint256[] calldata tokenIds) public nonReentrant onlyOwner {
        require(MAX_SUPPLY !=0 && totalSupply() + tokenIds.length - 1 < MAX_SUPPLY, "Exceeds max supply");
        require(tokenIds.length <= MAX_MULTIMINT, "Mint at most 20 at a time");
        // require(msg.value >= PRICE * tokenIds.length, "Insufficient payment, 0.002 ETH per item");
    ...
    /**
     * @dev Returns the address of the nft owner.
     */
    ...
    function withdraw() public nonReentrant {
        ...
        Address.sendValue(payable(creator()), balance);
    }

    /** ROYALTIES **/
    // uint256 private ownerFeePercentage = 1;
    // function royaltyInfo(uint256, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
    //     return (address(this), (salePrice * (100 * ownerFeePercentage)) / 10000);
    // }
    ...
    //     return _tokenId != 0 && _tokenId <= MAX_SUPPLY && !burned[_tokenId];
    // }
