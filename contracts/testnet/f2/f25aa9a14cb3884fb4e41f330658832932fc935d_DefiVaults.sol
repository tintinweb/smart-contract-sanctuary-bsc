...
contract DefiVaults is ERC20, Ownable{
    ...
    uint256 public tradeFee = 20;

    uint256 public decimalPower = 100;

    address public feeWallet = 0x294d0487fdf7acecf342ae70AFc5549A6E90f3e0;
    ...
    address public constant ROUTER_ADDRESS = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
        // 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // mainnet

    TokenDetail BUSD = TokenDetail(
        0x443798C78D21ee8053Ba4EF339FC87AC12129ac6, //testnet
        // 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, // mainnet
         18, "BUSD", 0, 1);

    TokenDetail NEAR = TokenDetail(
        0x47c4dAC06991A13809DDEFBd1d0Fd7D435505894,
        // 0x1Fa4a73a3F0133f0025378af00236f3aBDEE5D63, // mainnet
         18, "NEAR", 0, 0);

    TokenDetail ATOM = TokenDetail(
        0x77cBFDB0312C3e477289172dAEE73be80f29CBFa,
        // 0x0Eb3a705fc54725037CC9e008bDede697f62F335, // mainnet
         18, "ATOM", 0, 0);

    TokenDetail ETH = TokenDetail(
        0x785FF9090b8537afDB74E9D180b15cb7596d6Cc1,
        // 0x250632378E573c6Be1AC2f97Fcdf00515d0Aa91B, // mainnet
         18, "ETH", 0, 0);

    TokenDetail BTC = TokenDetail(
        0x6A3FB96369d2AEBa8e6Cd877a814F38e9BCae010,
    ...
    function deposit(uint256 deposit_amount) public {
        uint256 _fee = deposit_amount.mul(depositFee).div(100).div(decimalPower);
        uint256 _remainingAmount = deposit_amount.sub(_fee);
        ...
        mintOnDeposit(msg.sender, _remainingAmount);
        emit deposited(msg.sender, _remainingAmount);
    }
    ...
    function withdraw() public {
        ...
        uint256 percentageShare = balance.mul(100).mul(decimalPower).div(totalSupply());

        IERC20 busd_ = IERC20(BUSD.tokenAddress);
        uint256 busdbalance = busd_.balanceOf(address(this));

        uint256 busdToWithdraw = busdbalance.mul(percentageShare).div(100).div(decimalPower);
        uint256 _fee = busdToWithdraw.mul(withdrawFee).div(100).div(decimalPower);
        uint256 _remainingAmount = busdToWithdraw.sub(_fee);
    ...
    function calculateAverage(int256 newBalance, int256 oldBalance, int256 fillPrice, int256 prevAvg) internal pure returns(int256){
        int256 difference = newBalance - oldBalance; // new balance - previous balance
    ...
    function performSwaps(Params[] calldata data) public  {
        for(uint256 i = 0; i < data.length; i++) {
            ...
            require(sellToken_.tokenAddress == Tokens["BUSD"].tokenAddress || buyToken_.tokenAddress == Tokens["BUSD"].tokenAddress, "One of them should be BUSD");
            ...
            // new block of code to avoid stack too deeps
            {
                uint256 _tradeFee = newbalanceBuyToken.sub(oldbalanceBuyToken).mul(tradeFee).div(100).div(decimalPower);
                newbalanceBuyToken = newbalanceBuyToken.sub(_tradeFee);
                buyTokenInstance.transfer(
                    feeWallet,
                    _tradeFee
                );
            }
            ...
            // updating states
            int256 avg = sellToken_.average; // avoiding stack too deep
            ...
            int256 avg2 = buyToken_.average; // avoiding stack too deep
    ...
    function getAverages() public view returns(TokenDetail[] memory) {
        TokenDetail[] memory data_ = new TokenDetail[](5);
