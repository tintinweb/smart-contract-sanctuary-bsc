...
contract Marshorse is ERC721("Marshorse", "MHorse"), ERC721Enumerable, Ownable {
    ...
    uint256 private bnb_price = 250000000000000000;      //0.25 BNB
    ...
    mapping(address => stakeTokenInfo[]) public stakeTokens;
    ...
    function mintNFT(uint256 _numOfTokens, uint256 _mintType) public payable {
        ...
        if(_mintType == 2) {       //mint with LHC
            ERC20(coinAddress).transferFrom(msg.sender, address(this), lhc_price.mul(_numOfTokens));
        }
    ...
    // function stakeToken(uint256 _tokenId, uint256 _balance) public {
    //     require(ERC721.ownerOf(_tokenId) == msg.sender, "Token owner does not match");
    //     _transfer(msg.sender, address(this), _tokenId);
    //     ERC20(coinAddress).transferFrom(msg.sender, address(this), _balance);
    //     stakeTokens[msg.sender].push(stakeTokenInfo(_tokenId, _balance, 0, block.timestamp, block.timestamp, ERC721.ownerOf(_tokenId)));
    // }

    // function unstakeToken(uint256 _index) public {
    //     require(stakeTokens[msg.sender][_index].owner == msg.sender, "Token owner does not match");
    //     uint256 balance = stakeTokens[msg.sender][_index].balance;
        
    //     if (block.timestamp - stakeTokens[msg.sender][_index].createdTime < 10 days) {
    //         balance = balance * 70 / 100;
    //     }
    //     else if (block.timestamp - stakeTokens[msg.sender][_index].createdTime < 20 days) {
    //         balance = balance * 90 / 100;
    //     }

    //     _transfer(address(this), msg.sender, stakeTokens[msg.sender][_index].tokenId);
    //     ERC20(coinAddress).transferFrom(address(this), msg.sender, balance);
    //     delete stakeTokens[msg.sender][_index];
    // }       
