...
contract LuckyDraw is Ownable {
    ...
    function addPool(uint _dailySpin, uint _price, uint _burnPercent, address _spentToken, uint _guaranteedSpin, string memory _hash) external onlyOwner {
        require(_price > 0, "LuckyDraw: _price invalid");
        require(_spentToken != address(0), "LuckyDraw: _spentToken invalid");
        pools.push(Pool(_dailySpin, _price, _burnPercent, true, _spentToken));
