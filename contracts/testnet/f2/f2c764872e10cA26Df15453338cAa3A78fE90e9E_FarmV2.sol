...
contract FarmV2 {
    ...
    function getACount(address owner) public view returns(uint8) {
        return contSaveDay[owner];
    }
    function getASave(address owner) public view returns(bool) {
        return saveTime[owner];
    }

    function getATime(address owner) public view returns(uint40) {
    ...
    function sync(Event[] memory _events) public hasFarm  returns (Farm memory) {
        ...
        if(varCont > 3){
            require(block.timestamp >=  varTime + 7 minutes , "TIME_IN_FISHING");
