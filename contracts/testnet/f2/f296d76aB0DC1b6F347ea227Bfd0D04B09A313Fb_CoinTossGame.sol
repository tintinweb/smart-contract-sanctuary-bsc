...
contract CoinTossGame is Ownable {
    ...
    uint256 public contractTreasury;
    enum GAME_STATE {
        OPEN,
        START,
        CLOSED,
    ...
    error LowBalance(string _msg, uint256 requested, uint256 available);

    function fundTheTreasury() public payable onlyOwner {
        require(
            contractTreasury == 0,
            "Contract treasury should be zero to fund"
        );
        contractTreasury += msg.value;
        game_state = GAME_STATE.OPEN;
    }

    function playGame() public payable {
        require(game_state == GAME_STATE.OPEN, "The game is not opened yet!");
        if (msg.value >= contractTreasury) {
            revert LowBalance({
                _msg: "Entry value should be lower than contract treasury",
                requested: msg.value,
                available: contractTreasury
            });
        }
        contractTreasury += msg.value;
    ...
    function calculateWinner() internal {
        ...
        if (randomness <= 499) {
            ...
            contractTreasury += user.entryValue;
