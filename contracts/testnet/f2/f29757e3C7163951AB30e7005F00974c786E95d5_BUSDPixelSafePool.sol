...
/*
A Safe Pool that helps users stake in PIXEL farms and pools more simply.
Supporting auto compound in Single Staking Pool.
*/

contract BUSDPixelSafePool is Initializable, ReentrancyGuard, Ownable {
    ...
    IPixelReferral public pixelReferral = IPixelReferral(0xE6f9c256A6F3f2dddd4B20401627450c2d3E5d10);
    uint256 public constant PIXEL_BUSD_PID = 1;
      ...
      /*  // TESTNET
       ...
       //referraltreseury: 0xFE6cb444245547A4fDc21b6FFc5Bff9C7C9767bF
    ...
    event UpdateReferralCommissionRate(uint256 rate);
    event UpdatePercentFeeForCompounding(uint256 rate);
    ...
    function initialize(address _pixel, address _busd, address _router, address _factory, address _pixelStakingManager, address _safePoolReferralTreasury) public initializer {
    ...
    function canCompound() public view returns (bool) {
        return pixelStakingManager.canHarvest(PIXEL_BUSD_PID, address(this));
    }

    function nearestCompoundingTime() public view returns (uint256 time) {
        (,,,time) = pixelStakingManager.userInfo(PIXEL_BUSD_PID, address(this));
    }
    ...
    function totalPoolPendingRewards() public view returns (uint256) {
        ...
        return pixelStakingManager.pendingPosition(PIXEL_BUSD_PID, address(this)).mul(99).div(100);
    }
    ...
    function getSwappingPair() internal view returns (IUniswapV2Pair) {
        return IUniswapV2Pair(
            factory.getPair(address(pixel), address(busd))
        );
    }
    ...
    function updateReferralCommissionRate(uint256 _rate) external onlyOwner {
        require(_rate <= 2000, "max 20% - BUSD Safe Pool");
        referralCommissionRate = _rate;
        emit UpdateReferralCommissionRate(_rate);
    }

    function updatePercentFeeForCompounding(uint256 _rate) external onlyOwner {
        require(_rate <= 100, "max 10% - BUSD Safe Pool");
        percentFeeForCompounding = _rate;
        emit UpdatePercentFeeForCompounding(_rate);
    }
    ...
    */
    ...
    function deposit(uint256 amount, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract {
        ...
        uint256 pixelOut = swapBusdToPixel(amountToSwap);
    ...
    function depositTokenPair(uint256 amountPixel, uint256 amountBusd, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract {
        ...
        uint256 balanceOfPixelAfterTransfer = pixel.balanceOf(address(this));
        uint256 amountPixelReceived = balanceOfPixelAfterTransfer - balanceOfPixelBeforeTrasnfer;
        // note pixelAdded is might reduced by ~1%
        (uint256 pixelAdded, uint256 busdAdded, uint256 liquidityAmount) = router.addLiquidity(
            address(pixel),
            address(busd),
            amountPixelReceived,
            amountBusd,
    ...
    function _depositLP(address account, uint256 liquidityAmount, address referrer) internal {
       /*  if (
        ...
        require(address(pixelReferral) != address(0),"set pixelReferral contract address");
        require(referrer != address(0),"zero/null address not allowed as a referral");
        require(referrer != account,"sender address not allowed as a referral");
    ...
    function withdraw(uint256 amount, bool isReceiveBusd) external updateReward(msg.sender) nonReentrant noCallFromContract {
        ...
        pixelStakingManager.withdraw(PIXEL_BUSD_PID, lpAmountNeeded);
        (uint256 amountA,uint256 amountB) = removeLiquidity(lpAmountNeeded);
        if(isReceiveBusd){
            // send as much as we can
            // doesn't guarantee enough $amount
            busd.transfer(msg.sender, swapPixelToBusd(amountA).add(amountB));
        }else{
    ...
    function withdrawLP(uint256 lpAmount) external updateReward(msg.sender) nonReentrant {
        ...
        pixelStakingManager.withdraw(PIXEL_BUSD_PID, lpAmount);
    ...
    function harvest(bool isReceiveBusd) external updateReward(msg.sender) nonReentrant {
        ...
        if (reward > 0) {
            ...
            pixelStakingManager.withdraw(PIXEL_BUSD_PID, lpNeeded);
            (uint256 amountPixel,uint256 amountBusd) = removeLiquidity(lpNeeded);
            if(isReceiveBusd) {
                ...
                pixel.transfer(msg.sender, amountPixel);
            }
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 busdOut = swapPixelToBusd(pixelAmountToSwap);
            (,, uint256 liquidityAmount) = router.addLiquidity(
                address(pixel),
                address(busd),
                rewardForPool.sub(pixelAmountToSwap),
                busdOut,
                0,
                0,
                address(this),
    ...
    function pendingPositionNextCompound() public view returns (uint256){
        return pixelStakingManager.pendingPosition(PIXEL_BUSD_PID, address(this));
    }
    ...
    function swapBusdToPixel(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function swapPixelToBusd(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function removeLiquidity(uint256 lpAmount) internal returns (uint256 amountPixel, uint256 amountBusd){
        uint256 pixelBalanceBefore = pixel.balanceOf(address(this));
    ...
    function depositStakingPool(uint256 amount) internal {
        pixelStakingManager.deposit(PIXEL_BUSD_PID, amount, address(safePoolReferralTreasury));
    }


    function getBusdPixelRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(busd);
        paths[1] = address(pixel);
    }

    function getPixelBusdRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
