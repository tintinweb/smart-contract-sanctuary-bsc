...
$$ |  $$ |$$\   $$ |$$ |  $$\       $$ |      $$ |        $$ |  $$ |            
$$$$$$$  |\$$$$$$  |\$$$$$$  |      $$ |      $$$$$$$$\ $$$$$$\ $$ |            
\_______/  \______/  \______/       \__|      \________|\______|\__|   PVP         
                                                                                
This contract provides the root functionality for PvP coin flips to take place,
...
// File: contracts/Games/BSCFlipPvP.sol
...
contract BSCCoinFlipPvP is Ownable {
    ...
    address public _houseFeeReceiver = address(0x74dC4C095a3fb35298924b707B9d3CF77Ff7A4Bf);
    ...
    mapping (uint256 => uint256) public _readyGames; // Ready Game ID -> Game ID
    uint32[] public _waitingGameIds; // GameIDs waiting to be flipped against
    Queue private _queuedReadyGames;
    ...
    uint256 private _readyQueueSize;
    uint32 private _globalGameCounter = 0;
    mapping (address => mapping (address => uint256)) public _winnings;
    mapping (address => address) public _referrer;
    mapping (address => bytes32) public _usernames;
    ...
    function createGame(uint256 wager, uint8 outcome, address token, address referrer) external payable {
        ...
        emit GameCreated(_msgSender(), token, wager, outcome, _globalGameCounter);
        _waitingGameIds.push(_globalGameCounter);
        _games[_globalGameCounter++] = Game({creator: _msgSender(), opposer: address(0x0), winner: address(0x0), token: token, id: _globalGameCounter, creatorPredictedOutcome: outcome, finished: false, wager: wager, startBlock: block.number});
    }

    function enterGame(uint256 gameId, address referrer) external payable {
        require(_gameEnabled, "Game is currently paused");
        require(!_isBlacklisted[_msgSender()], "This user is blacklisted");
        require(!(_msgSender().isContract()), "Contracts are not allowed to play the game");
        require(msg.sender == tx.origin, "Sender must be the same as the address that started the tx");
        ...
        require(!game.finished, "This game is already finished");
        require(game.opposer == address(0x0), "This game already has an opposer");

        if (game.token != address(0x0)) {
            gameToken.transferFrom(_msgSender(), address(this), game.wager);
        } else {
            require(msg.value == game.wager, "Must send same amount as specified in wager");
        }
        ...
        game.opposer = _msgSender();

        _readyGames[_queuedReadyGames.end++] = game.id;
        _readyQueueSize++;
    ...
    function completeQueuedGames() internal {
        while (_readyQueueSize > _queueResetSize) {
            Game storage game = _games[_readyGames[_queuedReadyGames.start]];

            // If this game is finished, the user must have cancelled it.
            if (!game.finished) {
                if (block.number < game.startBlock.add(_blockWaitTime) ||
                    game.creator == _msgSender() || game.opposer == _msgSender()) {
                    // Wait _blockWaitTime before completing this game, to avoid exploits.
                    // Don't allow someone to complete their own game
                    break;
                }

                address winner = (rand() % 2) == game.creatorPredictedOutcome ? game.creator : game.opposer;
                address loser = winner == game.creator ? game.opposer : game.creator;

                _winnings[winner][game.token] += (game.wager * 2);

                game.finished = true;



                emit GameFinished(winner, loser, game.token, game.wager, game.id);
            }

            _queuedReadyGames.start++;
            _readyQueueSize--;
        }
    }
    ...
    function rand() public view returns(uint256)
    {
        uint256 seed = uint256(keccak256(abi.encodePacked(
            ...
            block.number + _readyQueueSize
        )));

        return seed;
    }

    function findAndDeleteWaitingGame(uint256 gameId) internal {
        uint32 index = 0;
        bool found = false;
        for (uint32 i = 0; i < _waitingGameIds.length; i++) {
            if (_waitingGameIds[i] == gameId) {
                index = i;
                found = true;
                break;
            }
        ...
        _waitingGameIds[index] = _waitingGameIds[_waitingGameIds.length-1];
        _waitingGameIds.pop();
    }
    ...
    function cancelGame(uint32 gameId) public {
        Game storage game = _games[gameId];
        require(game.creator == _msgSender(), "This address is not the creator of the game");
        IERC20 token = IERC20(game.token);

        findAndDeleteWaitingGame(gameId);
        game.finished = true;

        if (game.token != address(0x0)) {
            token.transfer(_msgSender(), game.wager);
        } else {
            _msgSender().call{value: game.wager};
        }
