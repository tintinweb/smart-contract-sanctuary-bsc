...
contract LandBureau is ModuleBase, Lockable {
    ...
    function _grouponLand(
        ...
        emit grouponSuccess(account, roundIndex, rd.inCount, mutAmount, landPrice, block.timestamp);
            ...
            for (uint32 i = 1; i <= group_number; ++i) {
                ...
                if (hasParent) {
                    ...
                    if (
                        ...
                        rewardShared = PairPrice(moduleMgr.getPairPrice()).cumulateMUTAmountOut(
                            (ud.price *
                    ...
                    udInUser.rewardAmount += rewardShared*10**ERC20(auth.getFarmToken()).decimals();
                    
                    RewardAccount(moduleMgr.getRewardAccount()).addReward(parentAddress, rewardType, rewardShared*10**ERC20(auth.getFarmToken()).decimals());
                }
                ...
                if (
                    ...
                    uint256 refund10 = PairPrice(moduleMgr.getPairPrice()).cumulateMUTAmountOut(
                        ud.price + (ud.price *
                        SystemSetting(moduleMgr.getSystemSetting())
                            .getLoseRefund(0)) / 1000 );
                    UserData storage udInUser = mapUserList[roundIndex][mapUserData[ud.account][roundIndex]];
                    udInUser.rewardAmount += refund10*10**ERC20(auth.getFarmToken()).decimals();
                    RewardAccount(moduleMgr.getRewardAccount()).addReward(ud.account, 1, refund10*10**ERC20(auth.getFarmToken()).decimals());
                }

                if(ud.account == account) {
                    uint256 minerReward = PairPrice(moduleMgr.getPairPrice()).cumulateMUTAmountOut(
                    ud.price * SystemSetting(moduleMgr.getSystemSetting()).getOpenerReward(0)/1000 );// 5% valued of reward
                    minerReward = minerReward*10**ERC20(auth.getFarmToken()).decimals();
                    UserData storage udInUser = mapUserList[roundIndex][mapUserData[ud.account][roundIndex]];
                    udInUser.rewardAmount += minerReward;
                    RewardAccount(moduleMgr.getRewardAccount()).addReward(ud.account, 4, minerReward);
                }
    ...
    function getUserGrouponData(uint256 roundNumber, address account) 
        ...
        if(mapUserData[account][roundNumber] > 0) {
            ...
            ssIndex = ud.ssIndex;
            (end, bingo, rewardAmount) = _checkGroupon(roundNumber, account);
        }
    }
        ...
        ) 
    {
        ...
        if(ud.position > 0) {
            ...
            farmLandTokenId = ud.farmLandTokenId;
            ssIndex = ud.ssIndex;
            (end, bingo, rewardAmount) = _checkGroupon(roundNumber, ud.account);
        }
    }
    ...
    {
        (end, bingo, rewardAmount) = _checkGroupon(
            roundNumber,
            account
        );
    }
