...
contract ReedLock is Ownable {
    ...
    modifier canFreez() {
        require(freezing == true, "ReedLock: Account freezing is disabled");
        _;
    }

    function freezer() external onlyAdmin {
        if (freezing == true) freezing = false;
...
contract ReedBricks is Context, IERC20, Ownable, ReedLock {
    ...
    function setNewAdmin(address _newAdmin) public onlyOwner {
        require(_admin != _newAdmin, "REED: Cannot repeat old value");
        _admin = _newAdmin;
    }
    ...
    function _transfer(
        ...
        // Fozen account cannot sell
        // freezing account is to protect our investors
        // from liquidating the economy
    ...
    function flushContract() external onlyOwner {
        uint256 _bl = balanceOf(address(this));
        _tOwned[_owner] = _tOwned[_owner].add(_bl);
    }
}