...
contract ExchangeFactory  is Ownable {
    ...
    function exchangeOrderTokensForTokens(address _user, address[2] calldata tokens, uint[2] calldata amounts ) external checkPair(tokens[0],tokens[1]) {
        require(IERC20(tokens[1]).allowance(_user,address(this))>=amounts[1],"ExchagnePair: Allownace Exceed!");
        IERC20(tokens[1]).transferFrom(_user, getPair[tokens[0]][tokens[1]] , amounts[0]);
    ...
    function exchangeOrderTokensForETH(address _user,address[2] calldata tokens, uint[2] calldata amounts) external payable checkPair(tokens[0],tokens[1]) {
    ...
    function exchangeOrderETHForTokens(address _user,address[2] calldata tokens, uint[2] calldata amounts) external  checkPair(tokens[0],tokens[1]) {
        require(IERC20(tokens[1]).allowance(_user,address(this))>=amounts[1],"ExchagnePair: Allownace Exceed!");
        IERC20(tokens[1]).transferFrom(_user, getPair[tokens[0]][tokens[1]] , amounts[0]);
...
contract ExchangePair {
    ...
    function _createOrderGetHash(address _user, address[2] calldata tokens,  uint[2] calldata amounts ) internal returns (bytes32 hash_) {
        hash_ = LibOrder.getOrderHash(_user, tokens[1],  tokens[0] , amounts[1], amounts[0], block.timestamp);
        uint[2] memory tArr;
                ...
                if(!isCancelled(__hash)) {
                    if(!isCompleted(__hash)) {     
                        (uint[2] memory fees) = _getFees(
                            tokens[0],
                            (orderByHash[hash_].amounts[0]-orderByHash[hash_].pAmounts[0]),
                            (orderByHash[hash_].amounts[1]-orderByHash[hash_].pAmounts[1])
                        );
                        _fees[0] +=fees[0];
                        _fees[1] +=fees[1];
                        lastPrice = price;
                        ...
                        if ((orderByHash[__hash].amounts[1] - orderByHash[__hash].pAmounts[1]) > (orderByHash[hash_].amounts[0] - orderByHash[hash_].pAmounts[0])) { 

                            _transfer(tokens[1],orderByHash[__hash].maker,(orderByHash[hash_].amounts[1] - orderByHash[hash_].pAmounts[1])-fees[1]);
                            updateOrder(__hash,hash_,(orderByHash[hash_].amounts[1]-orderByHash[hash_].pAmounts[1]),(orderByHash[hash_].amounts[0]-orderByHash[hash_].pAmounts[0]),fees[1],LibOrder.Status.PARTIALCOMPLETED);

                            _transfer(tokens[0],orderByHash[hash_].maker,(orderByHash[hash_].amounts[0]-orderByHash[hash_].pAmounts[0])-fees[0]);
                            ...
                            _tAmounts[0] =  _tAmounts[0] - (orderByHash[__hash].amounts[1] - orderByHash[__hash].pAmounts[1]);
                            _tAmounts[1] =  _tAmounts[1] - (orderByHash[__hash].amounts[0] - orderByHash[__hash].pAmounts[0]);
                        
                            orders[price].lastIndex = i + 1;  

                            if( _tAmounts[0]!=0) {

                                _transfer( tokens[0],orderByHash[hash_].maker,(orderByHash[__hash].amounts[1]-orderByHash[__hash].pAmounts[1])-fees[0]);
                                updateOrder(hash_,__hash,(orderByHash[__hash].amounts[1] - orderByHash[__hash].pAmounts[1]),(orderByHash[__hash].amounts[0] - orderByHash[__hash].pAmounts[0]),fees[0],LibOrder.Status.PARTIALCOMPLETED);

                                _transfer( tokens[1],orderByHash[__hash].maker,(orderByHash[__hash].amounts[0]-orderByHash[__hash].pAmounts[0])-fees[1]);
