...
contract IERC721 is IERC165 {
    ...
    event Approval(address indexed owner, address indexed approved, uint32 indexed tokenId);
    event ApprovalForAll(address indexed owner, address indexed operator, bool approved);

    function balanceOf(address owner) public view returns (uint32 balance);
    function ownerOf(uint32 tokenId) public view returns (address owner);
    function safeTransferFrom(address from, address to, uint32 tokenId) public;
    function transferFrom(address from, address to, uint32 tokenId) public;
    function approve(address to, uint32 tokenId) public;
    function getApproved(uint32 tokenId) public view returns (address operator);
    function setApprovalForAll(address operator, bool _approved) public;
    function isApprovedForAll(address owner, address operator) public view returns (bool);
    function safeTransferFrom(address from, address to, uint32 tokenId, bytes memory data) public;
}
...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    function ownerOf(uint32 tokenId) public view returns (address) {
    ...
    function getApproved(uint32 tokenId) public view returns (address) {
    ...
    function safeTransferFrom(address from, address to, uint32 tokenId, bytes memory _data) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _safeTransferFrom(from, to, tokenId, _data);
    }

    function _safeTransferFrom(address from, address to, uint32 tokenId, bytes memory _data) internal {
        _transferFrom(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _exists(uint32 tokenId) internal view returns (bool) {
        address owner = _tokenOwner[tokenId];
        return owner != address(0);
    }

    function _isApprovedOrOwner(address spender, uint32 tokenId) internal view returns (bool) {
    ...
    function _safeMint(address to, uint32 tokenId, bytes memory _data) internal {
        _mint(to, tokenId);
        require(_checkOnERC721Received(address(0), to, tokenId, _data), "ERC721: transfer to non ERC721Receiver implementer");
    }

    function _mint(address to, uint32 tokenId) internal {
    ...
    function _burn(address owner, uint32 tokenId) internal {
    ...
    function _burn(uint32 tokenId) internal {
        _burn(ownerOf(tokenId), tokenId);
    }

    function _transferFrom(address from, address to, uint32 tokenId) internal {
    ...
    function _transferFromInternal(address from, address to, uint32 tokenId) internal {
        require(ownerOf(tokenId) == from, "ERC721: transfer of token that is not own");
        require(to != address(0), "ERC721: transfer to the zero address");
    ...
    function _checkOnERC721Received(address from, address to, uint32 tokenId, bytes memory _data)
    ...
    function _clearApproval(uint32 tokenId) private {
...
contract IERC721Enumerable is IERC721 {
    ...
    function tokenOfOwnerByIndex(address owner, uint32 index) public view returns (uint32 tokenId);

    function tokenByIndex(uint32 index) public view returns (uint32);
}
...
contract ERC721Enumerable is Context, ERC165, ERC721, IERC721Enumerable {
    ...
    mapping(uint32 => uint32) private _allTokensIndex;
    ...
    function getAllTokens() external view returns (uint32[] memory _tokens) {
        _tokens = _allTokens;
    }
    ...
    function _tokensOfOwner(address owner) internal view returns (uint32[] storage) {
    ...
    function _addTokenToAllTokensEnumeration(uint32 tokenId) private {
        _allTokensIndex[tokenId] = _allTokens.length.safe32();
        _allTokens.push(tokenId);
    }

    function _removeTokenFromOwnerEnumeration(address from, uint32 tokenId) private {
        uint32 lastTokenIndex = _ownedTokens[from].length.sub(1).safe32();
    ...
    function _removeTokenFromAllTokensEnumeration(uint32 tokenId) private {
        uint32 lastTokenIndex = _allTokens.length.sub(1).safe32();
        uint32 tokenIndex = _allTokensIndex[tokenId];

        uint32 lastTokenId = _allTokens[lastTokenIndex];
...
contract IERC721Metadata is IERC721 {
    ...
    function tokenBase(uint32 tokenId) external view returns (uint16 _level, uint8 _quality, uint32 _hashrate, uint8 _status, uint16 _type);
}
...
contract ERC721Metadata is Context, ERC165, ERC721, IERC721Metadata {
    ...
    function tokenBase(uint32 tokenId) external view returns (uint16 _level, uint8 _quality,uint32 _hashrate, uint8 _status, uint16 _type) {
        ...
        // _tokenId = (_quality * 10 ** 9) + (ttype * (10 ** 6)) + tokenId;
        _level = _tokenBase.u16Level;
        _quality = SafeMath.safe8(tokenId / (10 ** 9));
        _hashrate = _tokenBase.u32Hashrate;
        _status = _tokenBase.status;
        _type = SafeMath.safe16((tokenId % (10 ** 9)) / (10 ** 6));
    }

    function _setTokenBase(uint32 tokenId, uint16 _level,  uint32 _hashrate, uint8 _status) internal {
    ...
    function _upLevel(uint32 tokenId) internal {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenBases[tokenId].u16Level++;
    }

    function _setHashrate(uint32 tokenId, uint32 _hashrate) internal {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenBases[tokenId].u32Hashrate = _hashrate;
    }

    function _setStatus(uint32 tokenId, uint8 _status) internal {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        _tokenBases[tokenId].status = _status;
    }
    ...
    function removeAtTokenID(uint32 _tokenID) private {
        bool isFind = false;
        uint index = 0;
...
contract YiBoxNFT is ERC721Full, Governance {
    ...
    function getLevelsByToken(uint32 _token) external view returns (uint16[] memory, uint64[] memory, uint32[] memory) {
        require(_exists(_token), "can't find token");
        return (Levels[_token], UpTimes[_token], HashRates[_token]);
    }

    // tokenId 起点 是 1
    function mint(address to, uint32 tokenId, uint8 _quality, uint32 _hashrate,uint16 ttype) external returns(uint32 _tokenId) {
        require(_minters[msg.sender], "!minter");
        _tokenId = (_quality * 10 ** 9) + (ttype * (10 ** 6)) + tokenId;
        _mint(to, _tokenId);
        _setTokenBase(_tokenId, 1,_hashrate,4);
        Levels[tokenId].push(1);
        UpTimes[tokenId].push(now.safe64());
 ...
 */
    function upLevel(uint32 tokenId, uint32 _hashrate) external {
        require(_minters[msg.sender], "!minter");
        uint16 _level;
...
*/
    function setStatus(address _s, uint32 tokenId, uint8 _status) public {
        require(_minters[msg.sender], "!minter");
...
*/
    function burnIn(uint32 tokenId) external {
        //solhint-disable-next-line max-line-length
        require(_minters[msg.sender], "!minter");
        _burn(tokenId);
    }
    ...
    function getHashrateByAddress(address _target) external view returns (uint256 pTotal) {
        uint32[] memory _tokens = _tokensOfOwner(_target);
    ...
    function getAdditionParam(address _target, address _setting) external view returns (uint256 suit, uint256 lv4, uint256 lv5) {
        ...
        for (uint i=0; i < _tokens.length; i++) {
            uint8 _qu = SafeMath.safe8(_tokens[i] / (10 ** 9));
            uint16 _ty = SafeMath.safe16((_tokens[i] % (10 ** 9)) / (10 ** 6));
            if (_qu > 0) {
                _qb[_ty].nums[_qu - 1]++;
                if (_qu == 4) {
                    lv4++;
    ...
    function getTokensByStatus(address _owner, uint8 _status) external view returns (uint32[] memory tokens) {
        uint32[] memory _tokens = _tokensOfOwner(_owner);
        uint sum = 0;
    ...
    function tokensOfOwner(address owner) public view returns (uint32[] memory) {
