...
////////////////////////////////////////// StakingStorage - AmountInBase view methods
    ...
    function getCurrentAmountInBaseWrite(address _account) public
    {
        getCurrentAmountInBase(_account);
    }
...
library CompoundPercent {
    ...
    function compound(uint256 amount, uint256 yearPercent, uint256 period)
        internal
        pure
        returns (uint)
    {
        return ABDKMath64x64.mulu(
            ABDKMath64x64.add(
                ...
                ABDKMath64x64.divu(yearPercent, 100 * 365 * 24 * 60 * 60)
        ...
        returns (uint)
    {
        return ABDKMath64x64.mulu(
            ABDKMath64x64.add(
                ABDKMath64x64.fromUInt(1),
                ABDKMath64x64.divu(yearPercent.mantissa, DoubleMath.doubleScale * 100 * 365 * 24 * 60 * 60)
            ).pow(period).sub(ABDKMath64x64.fromUInt(1)),
            amount
        );
    }
...
////////////////////////////////////////// methods for full balance
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        _rewardInDelay = CompoundPercent.compound(approximateFullBalanceInBase, baseRewardInPercent + timedRewardInPercent, _period);

        if (rewardDelayPeriod >= _period) {
            return (_rewardAllowedToClaim, _rewardInDelay);
        }

        _rewardAllowedToClaim = CompoundPercent.compound(approximateFullBalanceInBase, baseRewardInPercent + timedRewardInPercent, _period - rewardDelayPeriod);
...
////////////////////////////////////////// methods for calculate individual pending
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        uint256 _period = block.timestamp - _periodBegin;
        uint256 _nonSuspensionPeriod = _calculateNonSuspensionPeriod(_periodBegin, block.timestamp);

        if (_nonSuspensionPeriod == 0) {
            return (_rewardAllowedToClaim, _rewardInDelay);
        }

        uint256 _medianOfBalance = _cumulativeFullBalance / _period;
        DoubleMath.Double memory _medianOfPercent = DoubleMath.fraction(_cumulativeFullPercent, _nonSuspensionPeriod);

        _rewardInDelay = CompoundPercent.compound(_medianOfBalance, DoubleMath.add_(_medianOfPercent, baseRewardInPercent), _nonSuspensionPeriod); // _compoundInterestOfFull


        if (rewardDelayPeriod >= _nonSuspensionPeriod) {
            return (_rewardAllowedToClaim, _rewardInDelay);
        }

        _period -= rewardDelayPeriod;
        _nonSuspensionPeriod = _calculateNonSuspensionPeriod(_periodBegin, block.timestamp - rewardDelayPeriod);

        _medianOfBalance = _cumulativeAvailableBalance / _period;
        _medianOfPercent = DoubleMath.fraction(_cumulativeAvailablePercent, _nonSuspensionPeriod);

        _rewardAllowedToClaim = CompoundPercent.compound(_medianOfBalance, DoubleMath.add_(_medianOfPercent, baseRewardInPercent), _nonSuspensionPeriod); // _compoundInterestOfAvailable
        _rewardInDelay -= _rewardAllowedToClaim;
