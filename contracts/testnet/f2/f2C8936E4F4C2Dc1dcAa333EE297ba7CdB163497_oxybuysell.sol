...
contract oxybuysell is Ownable {
    ...
    uint256 public tokenBuyprice = 5000000000000000000;
    ...
    event TokenPurchase(address indexed purchaser, uint256 rate, uint256 totaltoken ,uint256 amount);
    ...
    function buyoxy(address _receiver,uint256 _amount) public payable {
        ...
        emit TokenPurchase(msg.sender, tokenBuyprice, totalbtoken, _amount);
        transaction.push(
            Transaction({
                _address : _receiver,
                _value : totalbtoken,
        ...
        userstate[_receiver]._totalbuytoken = userstate[_receiver]._totalsendbusd.add(_amount);
    ...
    function _calTotalbusd(uint256 _amount) public view returns(uint256, uint256)
    {
        ...
        // if(userstate[_receiver]._totaltransaction > 0)
        // {
        //     uint256 findoddeven = (userstate[_receiver]._totaltransaction % 2);
        //     if(findoddeven == 0){
        //         uint256 countrate = (tokenBuyprice.sub(tokenBuyprice.mul(tokenSellprice).div(100)));
        //         uint256 totaltoken = ((_amount * decimal) / countrate);
        //         return (totaltoken, countrate);
        //     }
        //     else{
        //         uint256 countrate = (tokenBuyprice.sub(tokenBuyprice.mul(even).div(100)));
        //         uint256 totaltoken = ((_amount * decimal) / countrate);
        //         return (totaltoken, countrate);
        //     }
        // }
        // else
        // {
        //     uint256 countrate = (tokenBuyprice.sub(tokenBuyprice.mul(odd).div(100)));
        //     uint256 totaltoken = ((_amount * decimal) / countrate);
        //     return (totaltoken, countrate);
        // }
    }
