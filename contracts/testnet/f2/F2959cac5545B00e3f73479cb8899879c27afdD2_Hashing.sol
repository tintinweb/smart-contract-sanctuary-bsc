...
contract Hashing {
    ...
    function encode(uint256 timestamp, string memory input, address addr) public pure returns (string memory) {
        return b64encode(abi.encodePacked(timestamp, input, addr));
    }
    ...
    function b64encode(bytes memory _str) internal pure returns (string memory) {
        return Base64.encode(_str);
    }

    function b64decode(string memory _str) internal pure returns (string memory) {
