...
contract TableclothAwardsPool is Initializable, AccessControlUpgradeable,ITableclothAwardsPool{
    ...
    TableclothERC1155 public tableclothERC1155;
    ...
    function initialize(address _tableclothERC1155Address, address chiCoinAddress) public initializer{
        tableclothERC1155 = TableclothERC1155(_tableclothERC1155Address);
    ...
    function addAwards(
        ...
        uint256 chiAmount
    ) onlyRole(AWARDSSENDER_ROLE) external override {
        //need approval at before
        chiCoin.transferFrom(sender, address(this), chiAmount);
     ...
     */
    function withdraw(uint256 tableclothId) external override{
        require(tableclothERC1155.balanceOf(_msgSender(), tableclothId) >= 1, "TableclothAwardsPool error: not the owner");
        uint128 typeId = tableclothERC1155.getTableclothType(tableclothId);
        _withdraw(tableclothId, typeId, _msgSender());
    }

    /** internal function
     * @dev Withdraw the token's unaccalimed awards amount in pool.
     */
    function _withdraw(uint256 tableclothId, uint128 typeId, address to) internal{
        uint256 amounts = _getUnaccalimedAmount(tableclothId, typeId);
        require(amounts > 0, "TableclothAwardsPool error: no awards anymore");
        //refresh indexPoolAmount
        tokenRecordMapping[tableclothId] = awardsPoolMapping[typeId].totalAmount;
        //reduce AwardsPool amount
        awardsPoolMapping[typeId].currentAmount -= amounts;
            
        chiCoin.transfer(to, amounts);
        emit Withdraw(tableclothId, to, amounts);
    }
...
contract TableclothERC1155 is ERC1155Upgradeable, AccessControlUpgradeable, ITableclothERC1155 {
    ...
    mapping(uint256 => TableclothType) public types;
     ...
     *
     * Requirements:
     * - `_attr` set gold, wood, water, fire and earth, five attribute values
     */
    ...
    function buyTablecloth(
        ...
        require(types[typeId].soldQuantity.add(1) <= types[typeId].maximum, "Quantity sold exceeds limit");
     ...
     *  @dev Get tablecloth type details by type id
     */
    function getTypeDeyails(uint256 typeId) public view override returns (
        string memory tableclothName,
        string memory tableclothDescribe,
        uint256 tableclothPrice,
        uint256 maximum,
    ...
    function setPrice(uint128 typeId, uint256 price) onlyRole(DEFAULT_ADMIN_ROLE) external {
