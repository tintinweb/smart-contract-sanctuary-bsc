...
contract FASTBNB {
    ...
    uint256 public withdrawLimit = 1e18; // 1 BNB
    ...
    struct User {
        ...
        uint256 withdrawold;
        uint256 withdrawnew;
        uint256 totalReferralCountOld;
        uint256 migrateDataTrue;
    ...
    function migrateData(uint256 _totalReferralCountOld, uint256 _withdrawold,
    uint256 _depositamount, uint8 _plan, uint256 _timestamp, address _referrer) public {
        User storage user = users[msg.sender];
        
        require(user.migrateDataTrue == 0);
        
        user.migrateDataTrue = 1;   
        user.totalReferralCountOld = _totalReferralCountOld;
        user.withdrawn = _withdrawold;
        user.withdrawold = _withdrawold;
        user.referrer = _referrer;
        
        user.deposits.push(Deposit(_plan, _depositamount, _timestamp));
        

    }


    function migrateDataReferral(uint256 _reflevel0, uint256 _reflevel1, uint256 _reflevel2, uint256 _reflevel3, uint256 _reflevel4,
    uint256 _reflevel5, uint256 _reflevel6, uint256 _reflevel7, uint256 _reflevel8, uint256 _reflevel9) public {
        // User storage user = users[msg.sender];
        
        users[msg.sender].levels[0] = users[msg.sender].levels[0].add(_reflevel0);
        users[msg.sender].levels[1] = users[msg.sender].levels[1].add(_reflevel1);
        users[msg.sender].levels[2] = users[msg.sender].levels[2].add(_reflevel2);
        users[msg.sender].levels[3] = users[msg.sender].levels[3].add(_reflevel3);
        users[msg.sender].levels[4] = users[msg.sender].levels[4].add(_reflevel4);
        users[msg.sender].levels[5] = users[msg.sender].levels[5].add(_reflevel5);
        users[msg.sender].levels[6] = users[msg.sender].levels[6].add(_reflevel6);
        users[msg.sender].levels[7] = users[msg.sender].levels[7].add(_reflevel7);
        users[msg.sender].levels[8] = users[msg.sender].levels[8].add(_reflevel8);
        users[msg.sender].levels[9] = users[msg.sender].levels[9].add(_reflevel9);
        

    }
        
    function migrateDepositData(address _userAddress, uint8 _plan, uint256 _depositamount, uint256 _timestamp) public {
        User storage user = users[_userAddress];
        
        user.deposits.push(Deposit(_plan, _depositamount, _timestamp));
    }
    
    function withdrawNew(uint256 _amount) public {
        User storage user = users[msg.sender];
        
        uint256 contractBalance = address(this).balance;
        require(contractBalance < _amount && withdrawLimit <= _amount);
        
        require(now >= (user.checkpoint + 86400));
        
        user.withdrawnew = user.withdrawnew.add(_amount);
    ...
    function changeWithdrawLimit(uint256 amount) public{
        if (msg.sender == commissionWallet) {
    ...
    function getUserTotalWithdrawnNew(address userAddress) public view returns (uint256, uint256, uint256, uint256) {
        return (users[userAddress].withdrawn, users[userAddress].withdrawold, users[userAddress].withdrawnew, users[userAddress].totalDividends);
    }

    function getUserStruct(address userAddress) public view returns (uint256, address, uint256, uint256, 
    uint256, uint256, uint256) {
        return (users[userAddress].checkpoint, users[userAddress].referrer, users[userAddress].bonus,
        users[userAddress].totalBonus, users[userAddress].seedincome, 
        users[userAddress].withdrawnseed,
        users[userAddress].totalReferralCountOld);
