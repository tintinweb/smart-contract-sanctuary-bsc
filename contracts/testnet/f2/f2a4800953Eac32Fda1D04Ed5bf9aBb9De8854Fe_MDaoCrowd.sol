...
interface IMetaCrowdV2 {
    ...
    function pending(address account, uint256 regionIndex, uint256 round) external view returns (uint256[] memory amounts, uint16 state, uint16 version, bool restart);

    function pendingForBrokerReward(address account, uint256 regionIndex, uint256[] calldata roundIds) external view returns (uint256 amount, uint16[] memory states);
...
interface ICrowdManager {
    ...
    function takeCrowdProfit(uint256 amount) external;
}
...
contract MDaoCrowd is AccessControl {
    ...
    address public /*constant*/ CROWD_ACCOUNT = 0xc6A55f07B588157aB2a8250c138784Fc799c3A4b;
    address public /*constant*/ META_CROWD = 0xCf533Df3846B1065E1E6174257c396a71c53C527;
    address public /*constant*/ META_POINT = 0x8347A810ff4d856A1571A5Ecc4672C0d0A52fA6c;
    address public /*constant*/ FUND_ALLOC = 0xb886073a77E0FdF466E635F89344B48Aa5d1E671;
    ...
    function harvestAll(uint256 regionIndex) external {
        ...
        for (uint256 i = 0; i < roundIds.length; i++) {
            (uint256[] memory amounts, , , ) = IMetaCrowdV2(META_CROWD).pending(address(this), regionIndex, roundIds[i]);
            if (amounts[2] > 0) {
                profit += amounts[2];
            }
        }
        if (profit > 0) ICrowdManager(mamager).takeCrowdProfit(profit);
    }
    //提取经纪人奖
    function withdrawBrokerReward(uint256 regionIndex) external {
        ...
        (uint256 amount,) = IMetaCrowdV2(META_CROWD).pendingForBrokerReward(address(this), regionIndex, roundIds);
        if (amount > 0) {
            IMetaCrowdV2(META_CROWD).withdrawBrokerReward(regionIndex, roundIds);
            ICrowdManager(mamager).takeCrowdProfit(amount);
        }
    }
    //启用杠杆
