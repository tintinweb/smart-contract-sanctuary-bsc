...
contract StakingV2 is Ownable {
    ...
    constructor(IERC20 _crace, address _oldAddress) {
        crace = _crace;
        g_oldContract = IOldStaking(_oldAddress);
    }

    function aTest(uint256 _pid) external view returns(uint256, uint256, uint256) {
        uint256 amount = 0;
        uint256 rewards = 0;
        uint256 timestamp = 0;
        (amount, rewards, timestamp) = g_oldContract.userInfo(_pid, msg.sender);
        return (amount, rewards, timestamp);
    }
