...
contract SusumiCrowdfunding is Ownable, ReentrancyGuard, ISusumiCrowdfunding {
    ...
    uint64 public receiverFeeSUSU; // receiver fee while a user creates a custom campaign
    uint64 public price; // in USDT
    uint32 public defaultMaturityPeriod;
    address public BUSDAddress; //binance
    ...
    mapping(uint128 => uint32) public campaignReceiverCommission; // Commission sent to receiver for withdrawing
    ...
    function initialize(
        ...
        price = 10**5; // In USDT (10 ** 8)
        defaultMaturityPeriod = 60 minutes;
        receiverAddress = _receiverAddress;
        receiverFeeSUSU = uint64(100 * (10**susu.decimals())); // Fee in SUSU

        // For Testing
        setSusuValueInfo(1, 100 * (10**8), 500 * (10**8), 2, 2, 8300); // For level 1
        setSusuValueInfo(2, 500 * (10**8), 2000 * (10**8), 2, 10, 5800); // For level 2
        setSusuValueInfo(3, 1000 * (10**8), 10000 * (10**8), 2, 20, 2300); // For level 3

        addUpdateVestingInfo(500 * (10**8), 30 minutes, 1); //; Vesting Counter 1
        addUpdateVestingInfo(2000 * (10**8), 30 minutes, 2); // Vesting Counter 2
        addUpdateVestingInfo(10000 * (10**8), 30 minutes, 3); // Vesting COunter 3
    }
    ...
    function setSusuValueInfo(
        uint8 _userLevel,
        uint64 _minSusuValue,
        uint64 _maxSusuValue,
        uint32 _minKeyValue,
        uint32 _maxKeyValue,
        uint32 _receiverFee
    ) public onlySubOwner("SUSU_VALUE_INFO") {
    ...
    // Owner sets or update the vesting information
    function addUpdateVestingInfo(
        uint128 _maxSusuValue,
        uint32 _vestingPeriod,
        uint128 vestingCounter
    ) public onlySubOwner("VESTING_INFO") {
        // Maximum vesting counter is 3
        require(vestingCounter <= 3, "Counter cannot be greater than 3");

        vestingMapping[vestingCounter] = SusuLibrary.VestingInfo({
            maxSusuValue: _maxSusuValue,
            vestingPeriod: _vestingPeriod
        });
    }

    // Returns Vesting period index for the Susu Value
    function getVestingValue(uint128 _susuValue) internal view returns (uint8) {
        if (_susuValue <= vestingMapping[3].maxSusuValue) {
            if (_susuValue <= vestingMapping[2].maxSusuValue) {
                if (_susuValue <= vestingMapping[1].maxSusuValue) {
                    return 1;
                }
                return 2;
            }
    ...
    function newCampaignInternal(
        ...
        SusuLibrary.Campaign memory newCampaign = SusuLibrary.Campaign({
            ...
            vestingIndex: getVestingValue(_SusuValue),
            expirationTime: 0,
            levelValue: _level,
        ...
        campaignReceiverCommission[totalCampaigns] = susuValueMapping[_level]
            .receiverFee;
    ...
    function requestCustomCampaign(
        ...
        // Receiver fee check
        require(
            susu.balanceOf(msg.sender) >= receiverFeeSUSU,
            "User doesn't have enough balance to pay Custom Fund fees"
        );
        require(
            susu.allowance(msg.sender, address(this)) >= receiverFeeSUSU,
            "User doesn't have enough allowance to pay Custom Fund fees"
        );
        ...
        validateCampaign(_campaignIndex)
    {
        ...
        require(
            Token(BUSDAddress).balanceOf(msg.sender) >= cInfo.slotValue,
            "Check your balance."
        );
        require(
        ...
        SusuLibrary.User memory uInfo = SusuLibrary.User({
            ...
            vestingPeriod: vestingMapping[cInfo.vestingIndex].vestingPeriod,
            claimFlag: false
        });
        ...
        validateCampaign(_campaignIndex)
    {
        ...
        uint64 receiverShare = uint64(
            (campaignReceiverCommission[_campaignIndex] *
                cInfo.susuValueRaised) / 10**5
        );
        ...
        isCampaignMaturityOver(_campaignIndex)
    {
        ...
        campaignInfo[_campaignIndex].susuValueRaised -= uInfo.amountDonated;
        totalAmountRaised -= uInfo.amountDonated;
        uInfo.userClaimed = true;
        ...
        emit CampaignAfterDonationWithdrawal(
            _campaignIndex,
            campaignInfo[_campaignIndex].susuValueRaised
        );
    }
    ...
    function updateDefaultMaturity(uint32 _newTime)
        external
        onlySubOwner("DEFAULT_MATURITY_PERIOD")
    {
        defaultMaturityPeriod = _newTime;
    }

    // receiver can update receiver fee in usdt
    function updateReceiverFeeUSDT(uint64 _receiverFeeSUSU)
        external
        onlySubOwner("RECEIVER_FEE_USDT")
    {
...
library SusuLibrary {
    ...
    struct Campaign {
        ...
        uint8 vestingIndex; // 1 or 2 or 3
        uint8 levelValue;
