...
//        setContracts(addrs_, true, true);
    ...
    function setOldData(address[] calldata addr_, address[] calldata referrer_, uint[] calldata wallet_, uint8 dataType_) public onlyOwner {
        if (dataType_ == 1) {
            require(addr_.length == referrer_.length, "invalid input");
            for (uint i = 0; i < addr_.length; i++) {
                teams[addr_[i]] = referrer_[i];
            }
        } else if (dataType_ == 2) {
            require(addr_.length == referrer_.length, "invalid input");
            for (uint i = 0; i < addr_.length; i++) {
                referrers[addr_[i]] = referrer_[i];
            }
        } else if (dataType_ == 3) {
            require(addr_.length == wallet_.length, "invalid input");
            for (uint i = 0; i < addr_.length; i++) {
                castWallet[addr_[i]] = wallet_[i];
            }
        } else if (dataType_ == 4) {
            require(addr_.length == wallet_.length, "invalid input");
            for (uint i = 0; i < addr_.length; i++) {
                referWallet[addr_[i]] = wallet_[i];
            }
        } else {
            revert("wrong type");
        }
...
//    }
    ...
    function castApi(uint8 theme_, uint8 group_, uint soulCount_, uint amount_, address recommender_) public {
        ...
        //用户每铸造100个需要支付4u
        cUsdt.safeTransferFrom(player, address(this), amount_ * 4 * U_ACC);
...
//   *****  开启->前端调用api  *****
     ...
     *  group_ : 群体： 1-女，2-男
     **/
    function batchBuyBox(address caller_, uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns(bool){
        address player;
        ...
        uint b1 = cSoulToken.balanceOf(aSoulPair);
        uint b2 = cUsdt.balanceOf(aSoulPair) * 1 ether / U_ACC;
        if (b1 != 0 && b2 != 0 && (b2 * 1 gwei / b1 >= 4 gwei)) {
            s = 4.2 gwei * 1 ether / s;
        }
        ...
        uint sCount = _extractAll(player, recommender_, tokenIds, amounts, poolId);
        if(amount_ > sCount){
            sCount = amount_ - sCount;
            //未开完
            poolId = getPoolId(theme_, group_, 1);
            (uint[] memory ids, uint[] memory counts) = cSoulStore.extractBoxBatch(poolId, sCount);
            uint cCount = _extractAll(player, recommender_, ids, counts, poolId);
            require(cCount == sCount, "quantity");
        }
        return true;
    }
    ...
    function _divide(address caller_, address recommender_, address caster_, uint amount_) private {
        ...
        // 存在骑士
        if (teams[player] != address(0)) {
            uint communityAdd = amount_ * U_ACC / 10;
    ...
    **/
    function reward() public {
        address player = _msgSender();
        require(referWallet[player] > 0 || castWallet[player] > 0, "no credit");
        uint amount = referWallet[player] + castWallet[player];
