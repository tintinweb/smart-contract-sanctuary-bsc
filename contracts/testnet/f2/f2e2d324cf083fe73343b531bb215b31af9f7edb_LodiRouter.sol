...
interface ILodiRouter02 is ILodiRouter01 {
...
library LodiLibrary {
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'LodiLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'LodiLibrary: ZERO_ADDRESS');
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = ILodiPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'LodiLibraryR: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'LodiLibrary: INSUFFICIENT_LIQUIDITY');
...
contract LodiRouter is ILodiRouter02 {
    using SafeMath for uint;
struct depositor{
    uint amountA;
    uint amountB;
    ...
    address public immutable override WETH;
     ILodiController public iuc ;
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'LodiRouter: EXPIRED');
        _;
    }
       ...
       constructor(address _factory,address _weth , address _iuc) public{
        factory = _factory;
        WETH = _weth;
        iuc = ILodiController(_iuc);
        owner = msg.sender;
    }

    function setUC(address _uc) public {
        if(msg.sender == owner){
            iuc = ILodiController(_uc);
        }
    }
    ...
    function _addLiquidity(
        ...
        if (ILodiFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            ILodiFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) =  getReserves( tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = LodiLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'LodiRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = LodiLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'LodiRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = (ILodiFactory(factory).getPair(tokenA, tokenB));
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = ILodiPair(pair).mint(to);
    }
    ...
    function getReserves(        address tokenA,      address tokenB) public view returns(uint reserveA, uint reserveB){
        ...
        address pair = ILodiFactory(factory).getPair(token0, token1) ;
        (uint reserve0, uint reserve1,) = ILodiPair(pair).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    ...
    function withdrawEth() external {
        ...
        require(iuc.owner() != address(0),"LodiRouter: Not found");
    ...
    function addLiquidityETH(
        ...
        address pair =ILodiFactory(factory).getPair( token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = ILodiFactory(factory).getPair( tokenA, tokenB);
        ILodiPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = ILodiPair(pair).burn(to);
        (address token0,) = LodiLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'LodiRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'LodiRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function removeLiquidityWithPermit(
        ...
        ILodiPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = LodiLibrary.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? ILodiFactory(factory).getPair( output, path[i + 2]) : _to;
            ILodiPair(ILodiFactory(factory).getPair( input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'LodiRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ILodiFactory(factory).getPair( path[0], path[1]), amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'LodiRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ILodiFactory(factory).getPair( path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'LodiRouter: INVALID_PATH');
        amounts = getAmountsOut( msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'LodiRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ILodiFactory(factory).getPair( path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'LodiRouter: INVALID_PATH');
        amounts = getAmountsIn( amountOut, path);
        require(amounts[0] <= amountInMax, 'LodiRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ILodiFactory(factory).getPair( path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'LodiRouter: INVALID_PATH');
        amounts = getAmountsIn( amountOut, path);
        require(amounts[0] <= msg.value, 'LodiRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ILodiFactory(factory).getPair(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = LodiLibrary.sortTokens(input, output);
            ILodiPair pair = ILodiPair(ILodiFactory(factory).getPair( input, output));
            uint amountInput;
            uint amountOutput;
            uint fee= iuc.getPairFee(ILodiFactory(factory).getPair(path[i],path[i+1]));
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ILodiFactory(factory).getPair( path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'LodiRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(ILodiFactory(factory).getPair(path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'LodiRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return LodiLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        require(path.length >= 2, 'LodiLibrary: INVALID_PATH');
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut,uint fee)       public
        ...
        require(amountIn > 0, 'LodiLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'LodiLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut,uint fee)       public
        ...
        require(amountOut > 0, 'LodiLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'LodiLibrary: INSUFFICIENT_LIQUIDITY');
