...
contract Treenvest is ITreenvest, Plan {
  ...
  IUserData private constant userData =
    IUserData(0x423085e9405917609f213936a55168063643F0d7);
  ...
  function invest3Month(address referrer)
    ...
    returns (uint256)
  {
    uint256 value = _deposit(referrer, msg.value, REWARD_PERIOD / 2);
  ...
  function invest6Month(address referrer)
    ...
    returns (uint256)
  {
    uint256 value = _deposit(referrer, msg.value, REWARD_PERIOD);
  ...
  function withdrawToInvest(bool six)
    external
    override
    nonReentrant
    returns (bool)
...
interface ITreenvest {
  ...
  function invest3Month(address referrer) external payable returns (uint256);

  function invest6Month(address referrer) external payable returns (uint256);
