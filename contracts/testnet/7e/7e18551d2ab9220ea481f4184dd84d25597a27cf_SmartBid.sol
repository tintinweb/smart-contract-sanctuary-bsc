...
contract SmartBid is  Ownable{
    ...
    function _fundOthers(uint256 pid, bool status, uint256 rem, uint256 num) internal {
        ...
        if (status) {
            ...
            for(uint256 i = projLength.sub(3 + 1); i >= projLength.sub(3 + all); i--) {
                others[i] = projBidders[pid][i];
            }
            address[] memory without = new address[](others.length);

            uint256 individual = rem.div(others.length);
            uint256 share = (individual.mul(all).div(1E18));
            // uint256 remnt;
            for (uint256 x; x < others.length; x++) {
                address wallet = others[x];
                bool iHave = getUri(pid, wallet);
                if (iHave) {
                    temporaryOtherLast3Address.push(wallet);
                } else {
                    without[x] = wallet;
                }
            }
    ...
    function fundRand(uint256 pid, uint256 remt) internal {
        ...
        uint256 share = (individual.mul(info.multiplier).div(1E18));
        if (iHave.length > 0 ) {
    ...
    function getUri(uint256 pid, address _user) public view returns(bool WIHave) {
        tokenInfo memory info = request_token_info[pid];        
        biddersInfo memory bidder = bidders[pid][_user];
        address tContract = bidder.tokenAddress;
        if(tContract != address(0)) {
            string memory token = IERC1155(tContract).uri(bidder.tokenOwned);
    ...
    function bTransfer(uint256 pid) public view returns(uint256 nPOne, uint256 nPTwo,  uint256 nPThree) {
        ...
        uint256 share = (info.multiplier).div(1E18);      
        if((getUri(pid, FirstTBidder))) {
            nPOne = positionOne.mul(share);
        } else {nPOne = positionOne;}
        if((getUri(pid, secondTBidder))) {
            nPTwo = positionTwo.mul(share);
        } else {nPTwo = positionTwo;}
        if((getUri(pid, thirdTBidder))) {
