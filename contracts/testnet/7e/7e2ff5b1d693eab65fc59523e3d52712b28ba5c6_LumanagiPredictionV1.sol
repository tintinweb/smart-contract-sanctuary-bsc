...
contract LumanagiPredictionV1 is Ownable, Pausable, ReentrancyGuard, AutomationCompatibleInterface  {
    ...
    bool public needExecuteRound = false;
    
    uint256 public _nextGenesisLockOnceExecuteTimeStamp;
    uint256 public _nextExecuteRoundTimeStamp;
    ...
    {
        if (genesisStartOnce && block.timestamp == _nextGenesisLockOnceExecuteTimeStamp && needGenesisLock) {
            //needGenesisLock = true;
            upkeepNeeded = true;
        }
        else if (genesisLockOnce && block.timestamp == _nextExecuteRoundTimeStamp && needExecuteRound) {
            //needExecuteRound = true;
            upkeepNeeded = true;  
        }
    }
...
rounds[epoch - 2].closeTimestamp
    */
     ...
     * @dev Callable by operator
     */
    function genesisLockRound() internal {   //whenNotPaused onlyOperator
        ...
        _nextExecuteRoundTimeStamp = block.timestamp + intervalSeconds + 3;
    ...
    function genesisStartRound() external whenNotPaused onlyOperator {
        ...
        _nextGenesisLockOnceExecuteTimeStamp = block.timestamp + intervalSeconds + 3;
        needGenesisLock = true;
    }
