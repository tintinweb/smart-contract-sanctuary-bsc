...
contract NFTRanking is Ownable {
    ...
    constructor (UserInfo[] memory infos) {
        for (uint256 i = 0; i < infos.length; i++) {
            userInfos.push(infos[i]);
        }
    }
    ...
    function quickSort(UserInfo[] storage arr, int left, int right) internal {
        int i = left;
        int j = right;
        if (i == j) return;
        UserInfo memory pivot = arr[uint(left + (right - left) / 2)];
        while (i <= j) {
            UserInfo memory userI = arr[uint(j)];
            UserInfo memory userJ = arr[uint(j)];

            while (userI.power > pivot.power) i++;
            while (pivot.power > userJ.power) j--;
            if (i <= j) {
                (arr[uint(i)], arr[uint(j)]) = (userJ, userI);
                i++;
                j--;
            }
        }
    ...
    // function burnDroidBot(uint256 _droidBotId) external rankingNotEnd {
    //     require(droidBot.ownerOf(_droidBotId) == msg.sender, 'NFTRanking: not owner of bot');
    //     uint256 power = userPowers[msg.sender];
    ...
    //     }
    //     userPowers[msg.sender] += droidBot.power(_droidBotId);
    // }

    // function setRank(uint256 n) external {
    //     require(nUser >= n, 'NFTRanking: n overflow');

    //     quickSort(rank, 0, n);
    // }

    function setRank() external {
        quickSort(userInfos, int(0), int(userInfos.length - 1));
    }
