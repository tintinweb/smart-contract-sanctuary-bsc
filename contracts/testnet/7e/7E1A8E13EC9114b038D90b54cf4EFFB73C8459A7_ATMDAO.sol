...
contract PancakeTool {
    ...
    function initIRouter(address _router,address quoteToken) internal {
...
contract ATMDAO is IBEP20, Ownable, PancakeTool {
    ...
    address private _defaultReco;
    address[] private _lockAddress;
    address private _making;

    uint8 private _tPercent = 3;

    uint256 private _maxDeals = 10 ether;
    uint256 private _maxHold = 30 ether;
    ...
    constructor(address defaultReco_) {
        _name = "ATM 500 DAO";
        _symbol = "ATMDAO";
        _decimals = 18;
        _totalSupply = 10000 ether;
        _finalSupply = 500 ether;
        _balances[msg.sender] = _totalSupply;
        tokenHold[msg.sender] = true;
        _making = msg.sender;
        

        _lockAddress.push(0x000000000000000000000000000000000000dEaD);
        _defaultReco = defaultReco_;
    ...
    function _beforeTransfer(
        ...
        if (sender == PancakePair && recipient == _PancakeRouter) {
            ...
            require(aBalance <= _maxHold,"The maximum number of holdings is 30");
    ...
    function _tokenReward() internal returns (bool) {
        ...
        for (uint256 i = 0; i < tokenHolders.length; i++) {
            bool isLock = false;
            for (uint256 lockIndex = 0;lockIndex < _lockAddress.length;lockIndex++) {
