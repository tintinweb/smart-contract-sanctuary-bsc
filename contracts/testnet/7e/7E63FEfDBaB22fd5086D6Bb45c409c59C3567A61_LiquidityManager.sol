...
contract LiquidityManager is Ownable, ILiquidityManager {
    ...
    ISwapFactory private uniSwapFactory;
    ...
    constructor(address router, address sheepContract, address tokenContract) {
        SHEEP = IERC20(sheepContract);
        TOKEN = IERC20(tokenContract);
        
        uniSwapRouter = ISwapRouter02(router);
        uniSwapFactory = ISwapFactory(uniSwapRouter.factory());

        swapPairAddr = uniSwapFactory.getPair(address(SHEEP), tokenAddr);
    ...
    function _swapSHEEPForTokens(address destination, uint256 sheepAmount, uint256 amountOutMin) private {
        ...
        path[1] = tokenAddr;
        bool success = SHEEP.approve(address(uniSwapRouter), sheepAmount);
