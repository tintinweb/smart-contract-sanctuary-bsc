...
interface IPerpetualTreasury {
    function addLiquidity(uint256 _iPoolIndex, int128 _fTokenAmount) external;
    ...
    function getTokenAmountToReturn(uint256 _poolId, int128 _fShareAmount) external view returns (int128);

    function getAmountForPeriod(uint256 _poolId, address _user) external view returns (int128);
}
 ...
 */
pragma solidity 0.8.13;
pragma experimental ABIEncoderV2; // to pass structs in public function of AMMPerpModule
...
contract PerpetualTreasury is PerpetualRebalanceFunctions, IFunctionList, IPerpetualTreasury {
     ...
     */
    function addLiquidity(uint256 _poolId, int128 _fTokenAmount) external override nonReentrant updateFundingAndPrices(_poolId) {
    ...
    ) external override nonReentrant updateFundingAndPrices(_poolId) {
     ...
     */
    function _validateLiquidityData(uint256 _poolId, int128 _fAmount) internal view {
    ...
    function _addCheckpoint(
        ...
        checkpoints[_pool.id][_user].push(Checkpoint(uint128(block.timestamp), _fShareAmount));
    }
    ...
    function _getAmountForPeriod(LiquidityPoolData storage _pool, address _user) internal view returns (int128) {
        ...
        for (uint256 i = length; i > 0; i--) {
            ...
            if (block.timestamp - checkpoint.timestamp > _pool.iPnLparticipantWithdrawalPeriod) {
                break;
            }
    ...
    function getTokenAmountToReturn(uint256 _poolId, int128 _fShareAmount) external view override returns (int128) {
        PerpStorage.LiquidityPoolData storage pool = liquidityPools[_poolId];
        return _getTokenAmountToReturn(pool, _fShareAmount);
    }

    function getAmountForPeriod(uint256 _poolId, address _user) external view override returns (int128) {
        PerpStorage.LiquidityPoolData storage pool = liquidityPools[_poolId];
