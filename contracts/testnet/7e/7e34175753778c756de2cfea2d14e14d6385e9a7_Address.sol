...
/**
 *Submitted for verification at BscScan.com on 2022-29-05
*/
...
contract LEMU is Ownable, IERC20 {
    string private _name = "LemurX INU";
    string private _symbol = "LEMU";
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "LEMU: decreased allowance below zero");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(currentAllowance >= amount, "LEMU: transfer amount exceeds allowance");
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }    

    function _transfer(address sender, address recipient, uint256 amount) internal {
        require(sender != address(0), "LEMU: transfer from the zero address");
        require(recipient != address(0), "LEMU: transfer to the zero address");
        if(antiBotEnabled) {
        ...
        require(senderBalance >= amount, "LEMU: transfer amount exceeds balance");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "LEMU: approve from the zero address");
        require(spender != address(0), "LEMU: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "LEMU: mint to the zero address");
        _totalSupply += amount;
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "LEMU: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "LEMU: burn amount exceeds balance");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "LEMU: account is already set to requested state");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "LEMU: DEX pair can not be removed");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LEMU: automated market maker pair is already set to that value");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "LEMU: the router is already set to the new address");
    ...
    function compound() public {
        require(compoundingEnabled, "LEMU: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    string private _name = "LEMU_DividendTracker";
    string private _symbol = "LEMU_DividendTracker";  
    ...
    constructor(address _tokenAddress) {
        minTokenBalanceForDividends = 150000 * (10**18);
    ...
    function excludeFromDividends(address account, bool excluded) external onlyOwner {
        require(excludedFromDividends[account] != excluded, "LEMU_DividendTracker: account already set to requested state");
    ...
    function _mint(address account, uint256 amount) private {
        require(account != address(0), "LEMU_DividendTracker: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "LEMU_DividendTracker: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "LEMU_DividendTracker: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
        magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account]
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("LEMU_DividendTracker: method not implemented");
    }
