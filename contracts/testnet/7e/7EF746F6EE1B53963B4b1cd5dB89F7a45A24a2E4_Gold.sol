...
contract Gold is ERC20, Pausable, AccessControl {
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override whenNotPaused {
        require(_blackList[_from] == false, "Gold: account sender was on blacklist!");
        require(_blackList[_to] == false, "Gold: account recipient was on blacklist!");
        super._beforeTokenTransfer(_from, _to, _amount);
    }

    function addToBlacklist(address _account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_account != msg.sender, "Gold: must not add sender to backlist!");
        require(_blackList[_account] == false, "Gold: account was on blacklist!");
        _blackList[_account] = true;
        emit BlacklistAdded(_account);
    }

    function removeToBlacklist(address _account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_blackList[_account] == true, "Gold: account was on blacklist!");
