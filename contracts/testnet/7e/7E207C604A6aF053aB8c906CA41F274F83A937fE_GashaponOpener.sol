...
contract GashaponOpener is Ownable {
    ...
    event ChangePoolStartTime(uint256 time);
    event ChangePoolEndTime(uint256 time);
    event ChangePoolGashaPrice(uint256 price);
    ...
    uint256 public ltdStartTime = 0;
    uint256 public ltdEndTime = 0;
    ...
    function changeRandomRateWeap(address _address) public onlyOwner {
        randomRateAddress[randomRateType.WEAP] = _address;
        emit ChangeRandomRateContract(_address);
    }

    function changeRandomRateSuite(address _address) public onlyOwner {
        randomRateAddress[randomRateType.SUITE] = _address;
        emit ChangeRandomRateContract(_address);
    }

    function changePoolStartTime(uint256 newtime) public onlyOwner {
        ltdStartTime = newtime;
        emit ChangePoolStartTime(newtime);
    }

    function changePoolEndTime(uint256 newtime) public onlyOwner {
        ltdEndTime = newtime;
        emit ChangePoolEndTime(newtime);
    }

    function changePoolWeapPrice(uint256 price) public onlyOwner {
        weapGashaPrice = price;
        emit ChangePoolGashaPrice(price);
    }
    ...
    function openGasha(randomRateType _RandomType) public {
        if (_RandomType == randomRateType.WEAP) {
        ...
        } else if (_RandomType == randomRateType.SUITE) {
            uint256 _balance = IERC20(lakrimaTokenContract).balanceOf(
                msg.sender
            );
            require(
        ...
        returns (string memory)
    {
        ...
        //create SW
        if(_RandomType == randomRateType.WEAP) {
            partCode = createSwWeap(_randomNumber, _RandomType);
        } else if(_RandomType == randomRateType.SUITE) {
            partCode = createSwSuit(_randomNumber, _RandomType);
        }
    ...
    function createSwSuit(uint256 _randomNumber, randomRateType _RandomType)
        private
        view
        returns (string memory)
    {
    ...
    function createSwWeap(uint256 _randomNumber, randomRateType _RandomType)
        private
        view
        returns (string memory)
    {
