...
contract PreSale is Validatable {
    ...
    /**
     *  @notice mapping user address => bool is white list of user
     */
    ...
    event SetMaximum(uint256 oldValue, uint256 newValue);
    event SetWhitelist(address[] users, bool allow);
    ...
    /**
     * @notice Set max nft can buy
     * @dev    Only admin can call this function
     * @param _maximum max nft can buy
     *
     * emit {SetMaximum} events
     */
    function setMaximum(uint256 _maximum) external onlyAdmin notZero(_maximum) {
        uint256 _oldValue = maximum;
        maximum = _maximum;
        emit SetMaximum(_oldValue, maximum);
    }
     ...
     */
    function setWhitelist(address[] memory receivers, bool allow) external onlyAdmin {
        require(receivers.length > 0, "Invalid length");
        for (uint256 i = 0; i < receivers.length; i++) {
            require(receivers[i] != address(0), "Invalid address");
            isWhiteList[receivers[i]] = allow;
        }

        emit SetWhitelist(receivers, allow);
    }
    ...
    function buy(uint256 times) external payable notZero(times) {
        require(isWhiteList[_msgSender()], "User is not white list");
