...
contract ProjectIDO is Ownable {
	struct IdoConfig {
		uint256 startTime;
		uint256 endTime;
		uint256[3] quota; //vns,cpd,usdt
		uint256[3] quotaPerShare; //vns,cpd,usdt
	}
	...
	IdoConfig public config;
	uint256 public constant CLAIM_EXPIRATION_TIME = 7 days;
	...
	constructor(
		...
		tokenAddress[0] = _vns;
		tokenAddress[1] = _cpd;
		tokenAddress[2] = _usdt;
		UNI_V2_FACTORY = _uniswapV2Factory;
	}

	function getUserMaxIdoAmount(address user)public view returns (
			uint256 vnsQuota,
			uint256 cpdQuota,
			uint256 usdtQuota
		)
	{
		UserInfo memory userInfo = IVNSMemberShip(memberShip).getUserInfo(user);
		...
		vnsQuota = config.quotaPerShare[0] * coefficient;
		cpdQuota = config.quotaPerShare[1] * coefficient;
		usdtQuota = config.quotaPerShare[2] * coefficient;
	}
	...
	function calQuota() external onlyOwner {
		uint256 level1User = IVNSMemberShip(memberShip).getLevelLength(1);
		uint256 level2User = IVNSMemberShip(memberShip).getLevelLength(2);
		uint256 level3User = IVNSMemberShip(memberShip).getLevelLength(3);
		uint256 denominator = (level1User + level2User * 5 + level3User * 10);

		config.quotaPerShare[0] = config.quota[0] / denominator;
		config.quotaPerShare[1] = config.quota[1] / denominator;
		config.quotaPerShare[2] = config.quota[2] / denominator;
	}

	function getUsdtAmount(uint256 vnsAmount) internal view returns (uint256 usdtAmount) {
		if (vnsPrice == 0) {
			address VNS=tokenAddress[uint(TokenType.VNS_TOKEN)];
			address USDT=tokenAddress[uint(TokenType.VNS_TOKEN)];

			address lpAddress = IUniswapV2Factory(UNI_V2_FACTORY).getPair(VNS, USDT);
	...
	function ido(TokenType tokentype, uint256 amount) external {
		require(config.quota[uint256(tokentype)] > 0, "invalid tokentype");
		...
		address tAddress = tokenAddress[index];
	...
	function redeem(TokenType tokentype, uint256 amount) external {
		...
		require(block.timestamp < startClaimTime + CLAIM_EXPIRATION_TIME, "invalid time3");
