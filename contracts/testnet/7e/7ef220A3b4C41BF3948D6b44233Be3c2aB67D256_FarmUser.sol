...
contract FarmUser is IFarmUser, Initializable, Ownable {
    ...
    function incrementInvestAmount(address userAddr, uint256 usdtAmount) external override onlyFarm returns (Model.User memory) {
        userMap[userAddr].totalInvestAmount = SafeMath.add(userMap[userAddr].totalInvestAmount, usdtAmount);
        ...
        if (userMap[userAddr].totalYieldAmount < userMap[userAddr].outAmount && userMap[userAddr].out == 1) {
    ...
    {
        uint256 outAmount = SafeMath.div(SafeMath.mul(investAmount, appConf.getOutMultiple()), 1000);
        return outAmount;
    }
