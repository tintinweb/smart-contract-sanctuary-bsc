...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function updateCharacter(address owner,uint256 nftId, uint256 characterType, uint256 rank, uint256  star,uint256 exp,uint256 isDeleted) public onlyOwner returns (uint256) {
        characters[owner][nftId] = encode(Character(nftId,characterType,rank,star,exp,isDeleted));
        return nftId;
    }
...
function transfer(uint256 _nftId, address _target)
        external whenNotPaused
    {
        ...
        characters[ownerOf(_nftId)][_nftId]= encode(Character(_nftId,0,0,0,0,1));
