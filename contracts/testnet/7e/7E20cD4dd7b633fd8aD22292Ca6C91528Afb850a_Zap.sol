...
contract Zap is Ownable, Pausable, Initializable {
    ...
    */
    ...
    function getEstimatedLpToken(address fromToken, uint256 fromTokenAmount, address toToken) external view returns (uint256 liquidity) {
        if(isLpToken(toToken)) {
            ...
            if(fromToken == token0 || fromToken == token1) {
                uint256 halfFromTokenAmount = fromTokenAmount.div(2);
                ...
                uint256 otherTokenAmount = getPoolAddedAmount(fromToken, otherToken, halfFromTokenAmount);
                ...
                uint256 amount0 = token0 != WBNB ? getPoolAddedAmount(WBNB, token0, halfWbnbAmount): halfWbnbAmount;
                uint256 amount1 = token1 != WBNB ? getPoolAddedAmount(WBNB, token1, halfWbnbAmount): halfWbnbAmount;
    ...
    function getPriceImpactWhenZapIn(address fromToken, uint256 amountIn, address toToken) public view returns (uint256[] memory priceImpacts) {
        ...
        if (isLpToken(toToken)) {
            ...
            if (fromToken == token0 || fromToken == token1) {
                ...
                uint256 WBNBAmount = fromToken != WBNB ? getPoolAddedAmount(fromToken, WBNB, amountIn) : amountIn;

                if (fromToken != WBNB) {
                    priceImpacts[0] = calculatePriceImpact(fromToken, amountIn, WBNB);
                }

                if (token0 != WBNB) {
                    priceImpacts[1] = calculatePriceImpact(WBNB, WBNBAmount.div(2), token0);
                }

                if (token1 != WBNB) {
                    priceImpacts[2] = calculatePriceImpact(WBNB, WBNBAmount.div(2), token1);
                }
            }
    ...
    function getPriceImpactWhenZapOut(address fromToken, address toToken, uint256 amountIn) public view returns (uint256 priceImpact) {
        if (!isLpToken(fromToken)) {
            priceImpact = calculatePriceImpact(fromToken, amountIn, toToken);
        }
    }
    ...
    function zapInToken(address fromToken, uint256 amountIn, address toToken, uint256 amountOutMin) external whenNotPaused {
        ...
        require(amountOut >= amountOutMin, 'Zap: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function zapOut(address fromToken, address toToken, uint256 amountIn, uint256 amountOutMin) external whenNotPaused {
        IERC20(fromToken).safeTransferFrom(msg.sender, address(this), amountIn);
        _approveTokenIfNeeded(fromToken);

        if (!isLpToken(fromToken)) {
            uint256 amountOut = _swap(fromToken, amountIn, toToken, msg.sender);
            require(amountOut >= amountOutMin, 'Zap: INSUFFICIENT_OUTPUT_AMOUNT');
        } else {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                (uint256 amountToken, uint256 amountETH) = ROUTER.removeLiquidityETH(token, amountIn, 1, 1, msg.sender, block.timestamp);
                if (token != toToken) {
                    amountOut = _swap(token, amountToken, toToken, msg.sender);
                } else {
                    amountOut = amountToken;
                }
                if (WBNB != toToken) {
                    amountOut = amountOut.add(_swapBNBForToken(toToken, amountETH, msg.sender));
                } else {
                    amountOut = amountOut.add(amountETH);
                }
                require(amountOut >= amountOutMin, 'Zap: INSUFFICIENT_OUTPUT_AMOUNT');
            } else {
                (uint256 amountA, uint256 amountB) =  ROUTER.removeLiquidity(token0, token1, amountIn, 1, 1, msg.sender, block.timestamp);
                if (token0 != toToken) {
                    amountOut = _swap(token0, amountA, toToken, msg.sender);
                } else {
                    amountOut = amountA;
                }
                if (token1 != toToken) {
                    amountOut = amountOut.add(_swap(token1, amountB, toToken, msg.sender));
                } else {
        ...
        emit ZapOut(fromToken, toToken, amountIn, amountOutMin);
    }
    ...
    */
    ...
    function _safeSwapToBNB(uint256 amount) private returns (uint256) {
        ...
        require(WBNB != address(0), 'Zap: IWBNB is not set');
