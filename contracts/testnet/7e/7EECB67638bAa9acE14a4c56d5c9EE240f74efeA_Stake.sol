...
contract Stake is Ownable {
    ...
    event Staked(address staker, uint256 poolId, uint256 amount, address referral, uint256 time);
    event UnStaked(address staker, uint256 poolId, uint256 amount, uint256 time);
    event Rewards(address[] staker, uint256[] amount, uint256 time);
    event PoolCreation (address tokenAddress, uint256 rewardPer, uint256 lockPeriod, uint256 time);
    ...
    function createPool(address _tokenAddress, uint256 _rewardPer, uint256 _lockPeriod, uint256 _referralPer) public onlyOwner returns (bool){
        ...
        pool.push(poolInfo);
        emit PoolCreation(_tokenAddress, _rewardPer, _lockPeriod, block.timestamp);
        return true;
    }

    function updatePool(uint256 _poolId, uint256 _rewardPer, uint256 _referralPer) public onlyOwner returns (bool){
    ...
    function stake(uint256 _amount, uint256 _poolId, address _referral) public  returns(bool) {
        ...
        if(!userDetails[msg.sender].isExist){
            ...
            if(_referral != address(0)){
                IBEP20(pool[_poolId].tokenAddress).transfer(_referral, _amount * pool[_poolId].referralPer / 100);
            }
        }
    ...
    function rewards(address[] memory _account, uint256[] memory _amount, uint256 _totalSend) public onlyOwner{
        require(_account.length == _amount.length, "Both length must be same");
        require(token.allowance(msg.sender, address(this)) >= _totalSend, "Tokens not Approved");
        for(uint i = 0; i < _account.length; i++) {
            require(userDetails[_account[i]].isExist, "Address is not a staker");
            token.transferFrom(msg.sender, _account[i], _amount[i]);
            userDetails[_account[i]].harvested += _amount[i];
        }
        emit Rewards(_account, _amount, block.timestamp);
    }
    ...
    function viewROI(uint256 _stakingId) public view returns(uint256 ROI){
        return (((block.timestamp - stakeDetails[_stakingId].stakeTime) * stakeDetails[_stakingId].amount * pool[stakeDetails[_stakingId].poolId].rewardPer / 100) / 1 days);
    }
