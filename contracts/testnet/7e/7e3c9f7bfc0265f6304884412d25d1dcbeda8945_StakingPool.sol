...
contract Upgradable is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256[] configs; // startDate(0), endDate(1), duration(2), endStakeDate(3), exchangeRateRewardToStaking(4)
        uint256 poolNFT; // 0: pool Token, 1: pool 721, 2: pool 1155
    }
}
...
contract StakingPool is Upgradable {
    ...
    */
    function createPool(string[] memory strs, address[] memory addr, uint256[] memory data, uint256[] memory configs) external onlyAdmins {
        ...
        if(data[4]!=0){
            pool = PoolInfo(addr[0], addr[1], 0, 0, data[0], data[0], data[1], 0, data[2], data[3], 1, data[4], flexData, configs, data[5]);
        } else {
            uint256 poolDuration = configs[1] - configs[0];
            uint256 MaxTVL = getMaxTVL(poolDuration,data[0]);
            pool = PoolInfo(addr[0], addr[1], 0, 0, data[0], data[0], data[1], 0, data[2], MaxTVL, 1, data[4], flexData, configs, data[5]);
        }
     ...
     * @param strs: poolId(0), internalTxID(1)
     * @param newConfigs: startDate(0), endDate(1), endStakingDate(2), stakingLimit(3), exchangeRateRewardToStaking(4)
    */
    ...
    //             newRewardFund  = newConfigs[3] * pool.apr * pool.configs[4]/ 1e4;
    ...
    //         pool.stakingLimit = getMaxTVL(poolDuration, pool.initialFund);
    //     }
    ...
    /**
     * @dev Return config of PoolInfo
     * @param poolId: Pool id
    */
    function showMaxTVL(string memory poolId) external view returns(uint256) {
        PoolInfo memory pool = poolInfo[poolId];
        return pool.stakingLimit;
    }
    ...
    */
    function setAdmin(address _address, bool _value) external onlySuperAdmin { 
