...
contract launchpad is Ownable {
    ...
    function updateIDOInfoLaunch(
        ...
        idoTransfer[_user] =
            idoTransfer[_user] +
            FixedStaking(updateContract).getIDOTransferred(_user);
    ...
    uint256 public tempAmount = 0;
    address public tempAddress;

    function transferIDOTokenToFixed(
        address _fixedContract,
        address _user,
        uint256 _amount
    ) external {
        tempAmount = _amount;
        tempAddress = _fixedContract;
        require(
            idoTokenTransferredFromFixed[_fixedContract][_user] >= _amount,
            "No sufficient tokens"
        );
        require(
            idoToken.transfer(address(_fixedContract), _amount),
            "Transfer Failed"
        );
        totalIDOTokenTransferred -= _amount;
        tokenTransferredToPool[_user] -= _amount;
