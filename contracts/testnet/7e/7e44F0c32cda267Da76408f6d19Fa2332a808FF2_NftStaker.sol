...
contract NftStaker is Ownable {
    ...
    constructor(address[] memory _address,uint[] memory price) {
        for(uint i = 0; i< _address.length;i++)
        {
            ...
            // nftstruct[i].nftaddress = _address[i];
    ...
    function stake(address nftId,uint _tokenId) public payable{
        if(nftstruct[nftId].tokenid[_tokenId].tokenid == 0)
        {
            ...
            require(nftstruct[nftId].tokenid[_tokenId].status == 0, "Already Staked");
            stakes.push(Stake({
                _address : msg.sender,
                tokenId : _tokenId,
                starttime : block.timestamp,
    ...
    function unstake(address nftId,uint _tokenId) public payable{
        ...
        uint256 price = (nftstruct[nftId].tokenid[_tokenId].price * 5) / 100;
    ...
    function getTokenPrice() private view returns(uint){
        ...
        path[0] = 0xe405B8148d731E4E1117e00542264daC5375BC97;   // Arma token
        path[1] = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;   // Busd token
