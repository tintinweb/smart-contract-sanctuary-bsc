...
/**
 * @title BCF Bulk Sender, support ETH and ERC20 Tokens
*/
...
contract BulkSender is Ownable {
  ...
  modifier hasPermission() {
    require(msg.sender == owner || users[msg.sender]);
    _;
  }
  ...
  */
   ...
   */
  function sendSameValue(address[] memory _to, uint _value) private {
  ...
  function sendDifferentValue(address[] memory _to, uint[] memory _value) private {
  ...
  function coinSendSameValueFromAddress(address _tokenAddress, address[] memory _to, uint _value, address _fromAddress) private {
    ...
    StandardToken token = StandardToken(_tokenAddress);		
		for (uint8 i = 0; i < _to.length; i++) {
			token.transferFrom(_fromAddress, _to[i], _value);
		}
    emit LogTokenMultiSent(_tokenAddress,sendAmount);
  }

  function coinSendSameValue(address _tokenAddress, address[] memory _to, uint _value)  private {
		uint sendValue = msg.value;		
		address from = msg.sender;
		uint256 sendAmount = _to.length * _value;
    require(_to.length <= 255);
	...
	function coinSendDifferentValue(address _tokenAddress, address[] memory _to, uint[] memory _value)  private  {
		require(_to.length == _value.length);
		require(_to.length <= 255);
   ...
   */
  function bulkSend(address[] memory _to, uint _value) hasPermission payable public {
   ...
   */
	function bulkSendToken(address _tokenAddress, address[] memory _to, uint _value) hasPermission payable public {
   ...
   * Send coin with the same value by a implicit call method
   */
	function bulkSendTokenFromAddress(address _tokenAddress, address[] memory _to, uint _value, address _fromAddress) hasPermission payable public {
    coinSendSameValueFromAddress(_tokenAddress, _to, _value, _fromAddress);
	}

  /*
   * Send ether with the different value by a explicit call method
   */
  function bulkSendDifferentValue(address[] memory _to, uint[] memory _value) hasPermission payable public {
   ...
   */
  function bulkSendTokenWithDifferentValue(address _tokenAddress, address[] memory _to, uint[] memory _value) hasPermission payable public {
