...
contract singleStake is Ownable {
    ...
    function stake(address stakeUserAddress, uint256 amount) public returns (bool) {
        bytes memory data = abi.encodeWithSelector(IERC20.transferFrom.selector, stakeUserAddress, address(this) , amount);
    ...
    function unStake(uint256 amount) public returns (bool) {
        uint256 stakeAmt = stakeAmts[_msgSender()];
        stakeAmts[_msgSender()] = stakeAmt.sub(amount);
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, _msgSender() , amount);
        tokenContract.functionCall(data,"TransferHelper: TRANSFER_FAILED");
        return true;
    }


    function getStakeAmt(address stakeUserAddress) public view returns (uint256) {
        uint256 stakeAmt = stakeAmts[stakeUserAddress];
        return stakeAmt;
    }


     function getStakeTime(address stakeUserAddress) public view returns (uint256) {
    ...
    function claim() public returns (uint256) {
        ...
        uint256 stakeTime = stakeTimes[_msgSender()];
        uint256 myNow = getBlockTimestamp();
        uint256 betweenTime = myNow.sub(stakeTime);
        require(betweenTime >= minStakeTime,"Less than min stake time, no income");
        ...
        bytes memory data = abi.encodeWithSelector(IERC20.transfer.selector, _msgSender() , rewardAmt);
        tokenContract.functionCall(data,"TransferHelper: TRANSFER_FAILED");
        stakeTimes[_msgSender()] = myNow;
        return rewardAmt;
    }


    function getClaim(address stakeUserAddress,uint myNow) public view returns (uint256) {
        uint256 stakeAmt = stakeAmts[stakeUserAddress];
        require(stakeAmt >= minStakeAmt,"Less than min stake amount, no income");
        uint256 stakeTime = stakeTimes[stakeUserAddress];
        uint256 betweenTime = myNow.sub(stakeTime);
    ...
    function getClaim1(address stakeUserAddress,uint myNow) public view returns (uint256) {
        uint256 stakeAmt = stakeAmts[stakeUserAddress];
        require(stakeAmt >= minStakeAmt,"Less than min stake amount, no income");
        uint256 stakeTime = stakeTimes[stakeUserAddress];
        uint256 betweenTime = myNow.sub(stakeTime);
    ...
    function getClaim2(address stakeUserAddress,uint myNow) public view returns (uint256) {
         uint256 stakeAmt = stakeAmts[stakeUserAddress];
        require(stakeAmt >= minStakeAmt,"Less than min stake amount, no income");
        uint256 stakeTime = stakeTimes[stakeUserAddress];
        uint256 betweenTime = myNow.sub(stakeTime);
    ...
    function getClaim3(address stakeUserAddress,uint myNow) public view returns (uint256) {
        uint256 stakeAmt = stakeAmts[stakeUserAddress];
        require(stakeAmt >= minStakeAmt,"Less than min stake amount, no income");
        uint256 stakeTime = stakeTimes[stakeUserAddress];
        uint256 betweenTime = myNow.sub(stakeTime);
