...
contract FarmBooster is Ownable {
    ...
    /// @notice Adjust the totalLockedAmount.
    ...
    function updateTotalLockedAmount(address _user) internal {
        ...
        if (!initialization) {
            initialization = true;
            // Initialize the latest total locked amount.
    ...
    /// @param _user User address.
    /// @param _tokenId Token Id of position NFT.
    // This is just for test , will remove soon after updating MCV3 interface.
    // function removeBoostMultiplier(address _user, uint256 _tokenId) external onlyMasterChefV3 {
    //     // In order to save gas, no need to check the farms which have never beed boosted.
    //     // if(everBoosted[pid]){
    //     ItMap storage itmap = userInfo[_user];
    //     if (itmap.contains(_tokenId)) {
    //         itmap.remove(_tokenId);
    //         emit DeactiveFarmPool(_user, 0, _tokenId);
    //     }
    //     // }
    // }
