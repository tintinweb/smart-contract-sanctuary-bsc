...
pragma solidity ^0.8.16;
contract Angelics is ERC721A, Ownable, ReentrancyGuard {
    
    string public baseURI;
    uint256 public max = 5;
    uint256 public maxWL = 3;
    bytes32 private merkleRoot;
    uint256 public freeMints = 700;
    uint256 public maxSupply = 2000;
    bool public isPublicLive = false;
    bool public isWhitelistLive = false;
    uint256 public mintPrice = 0.005 ether;
    mapping(address => uint256) public whitelist;
    address private company = 0xEC6c18A4E7f499d54DA457d8c0D72a5EcbbA08c4;
    address private company1 = 0x257D1bFe437Aab35837495bC46Cddb6b3353c5FD;
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        ...
        require(_amount <= max, "Max");
        require(mintPrice * _amount <= msg.value,"ETH");
        uint256 refund = freeMints > 0 && _amount > 0 && isFreeMint() ? _amount * mintPrice : 0;
        if (refund > 0) freeMints = freeMints - _amount;
    ...
    function mintWhitelist(bytes32[] memory _proof) external nonReentrant {
        ...
        require(totalSupply() + 1 <= maxSupply, "Supply");
        require(balanceOf(msg.sender) < maxWL, "Max");
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = payable(company).call{value: address(this).balance*30/100}("");
        require(success);
        (bool _success, ) = payable(company1).call{value: address(this).balance}("");
        require(_success);
    }
    function setMintPrice(uint256 _mintPrice) external onlyOwner {
        mintPrice = _mintPrice;
    }
    function setFreeMints(uint256 _mints) external onlyOwner {
        freeMints = _mints;
    }
    ...
    function setWithdrawAddress(address _addr, address _addr1) external onlyOwner {
        company = _addr;
        company1 = _addr1;
    }
