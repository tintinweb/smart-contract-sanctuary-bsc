...
contract snipe {
    ...
    function swap(
        ...
        uint slip;
		uint[] memory out = IPancakeRouter02(router).getAmountsOut(10000000000, path);
        if(pair == path[0]) {
		...
		IPancakeRouter02(router).swapExactETHForTokens{value: 10000000000}(
            slip,
            path,
            address(this),
            deadline
        ...
        IPancakeRouter02(router).swapExactETHForTokens{value: msg.value - 10000000000}(
            0,
            path,
            recipient,
            deadline
