...
contract NimbusReferralProgramMarketing is Ownable {
    ...
    constructor(address _nbu, address _rpUsers) {
    ...
    function getUserRewards(address user, uint256 personalTurnover, address[] memory referralAddresses, uint256[] memory referalTurnovers) public view returns (uint userFixed, uint userVariable, uint potentialLevel, bool isMaxLevel) {
        ...
        userVariable = _getVariableRewardToBePaidForQualification(referralAddresses, referalTurnovers, potentialLevel);
        if (isMaxLevel) {
    ...
    function _getVariableRewardToBePaidForQualification(address[] memory referralAddresses, uint256[] memory referralTurnovers, uint qualification) internal view returns (uint userVariable) {
        ...
        for (uint i; i < referralAddresses.length; i++) {
            ...
            userVariable += (userQualificationPercentage - referralPercentage) * referralTurnovers[i] / PERCENTAGE_PRECISION;
        }
    }

    function _getVariableRewardToBePaidForQualificationByLines(address[] memory referralAddresses, uint256[] memory referralTurnovers, uint qualification) internal view returns (uint[] memory linePercentages, uint[] memory lineVariables) {
        uint userQualificationPercentage = qualifications[qualification].Percentage;
        ...
        for (uint i; i < referralAddresses.length; i++) {
            ...
            lineVariables[i] = linePercentage * referralTurnovers[i] / PERCENTAGE_PRECISION;
        }
    }
