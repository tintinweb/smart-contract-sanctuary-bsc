...
import "../contracts/BranchOfPools.sol";

contract RootOfPools is Initializable, OwnableUpgradeable {
    ...
    function initialize(address usdtAddress, address rankingAddress)
        external
        initializer
    {
        __Ownable_init();
...
import "../contracts/RootOfPools.sol";

contract BranchOfPools is Ownable {
    ...
    uint256 private _CURRENT_COMMISSION;
    ...
    constructor(
        ...
        _root = Root;
        _usdt = RootOfPools(_root).getUSDTAddress();
        _VALUE = VALUE;
        _stepValue = Step;
        _priceToken = price;
    ...
    function init() public onlyOwner returns (uint256) {
    ...
    function depositUSDT(uint256 amount) public onlyState(State.Fundrasing) {
        ...
        if (
            Ranking(RootOfPools(_root).getRanks()).getParRankOfUser(tx.origin)[
                2
            ] != 0
        ) {
            commission =
                (amount *
                    Ranking(RootOfPools(_root).getRanks()).getParRankOfUser(
                        tx.origin
                    )[2]) /
    ...
    //Доделать доступ только разрабам + распределение
    ...
    function isClaimable(address user)
        public
        view
        onlyState(State.TokenDistribution)
        returns (bool)
    {
