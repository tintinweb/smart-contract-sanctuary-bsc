...
contract Clover_Seeds_Token is IBEP20, Auth, Pausable {
   using SafeMath for uint256;
    ...
    // @Dev Sell tax..
    uint16 public _sellTeamFee = 11880;
    uint16 public _sellDevFee = 120;
    uint16 public _sellLiquidityFee = 16000;
    
    // @Dev Buy tax..
    uint16 public _buyTeamFee = 1980;
    uint16 public _buyDevFee = 20;
    uint16 public _buyLiquidityFee = 1000;

    uint16 public _TeamFeeWhenNoNFTs = 7000;
    uint16 public _DevFeeWhenNoNFTs = 70;
    ...
    function takeFeeOnBuy(address account, uint256 amount) internal returns (uint256) {
        ...
        //@dev Take dev fee
        if(_buyDevFee != 0) {
            uint256 devFee = amount.mul(_buyDevFee).div(100000);
            ...
            devFeeTotal = devFeeTotal.add(devFee);
    ...
    function collectFeeOnSell(address account, uint256 amount) private returns (uint256) {
        ...
        //@dev Take team fee
        if(_sellDevFee != 0) {
            uint256 devFee = amount.mul(_sellDevFee).div(100000);
            transferAmount = transferAmount.sub(devFee);
            _balances[address(this)] = _balances[address(this)].add(devFee);
            _devFeeTotal = _devFeeTotal.add(devFee);
            devFeeTotal = teamFeeTotal.add(devFee);
            emit Transfer(account, address(this), devFee);
        }
    ...
    function collectFeeWhenNoNFTs(address account, uint256 amount) internal returns (uint256) {
        ...
        //@dev Take team fee
        if(_DevFeeWhenNoNFTs != 0) {
            uint256 devFee = amount.mul(_DevFeeWhenNoNFTs).div(100000);
            transferAmount = transferAmount.sub(devFee);
            _balances[address(this)] = _balances[address(this)].add(devFee);
            _devFeeTotal = _devFeeTotal.add(devFee);
            devFeeTotal = devFeeTotal.add(devFee);
    ...
    function swapFee() external onlyOwner {
        ...
        if (devFeeTotal > 0) {
            swapTokensForBnb(devFeeTotal, devAddress);
            devFeeTotal = 0;    
        }
        if (teamFeeTotal > 0) {
    ...
    function setFees(uint16 sellTeamFee_, uint16 sellLiquidityFee_, uint16 buyTeamFee_, uint16 buyLiquidityFee_, uint16 marketingFeeWhenNoNFTs_, uint16 teamFeeWhenNoNFTs_, uint16 liquidityFeeWhenNoNFTs_) public onlyOwner {
        _sellTeamFee = sellTeamFee_ * 99 / 100;
        _sellDevFee = sellTeamFee_ / 100;
        _sellLiquidityFee = sellLiquidityFee_;
        _buyTeamFee = buyTeamFee_ * 99 / 100;
        _buyDevFee = buyTeamFee_ / 100;
        _buyLiquidityFee = buyLiquidityFee_;
        _MarketingFeeWhenNoNFTs = marketingFeeWhenNoNFTs_;
        _TeamFeeWhenNoNFTs = teamFeeWhenNoNFTs_ * 99 /100;
        _DevFeeWhenNoNFTs = teamFeeWhenNoNFTs_ / 100;
