...
FORTUNE HUNTERS - ELK OF FORTUNE

  ~ 3% daily interest contract basis.
  ~ 4% Referral Bonus, rewards wil go straight to the wallet.
  ~ 2.5% stacking compound bonus every 24 hours, max of 7 days.
  ~ 4% of fees will go to elk pools.
  ~ 60% Tax on early withdrawals.
    *60% Tax will stay in the contract.
    *This will be implemented from day 1.
  ~ Madatory 5 day compound

  Community Lottery:
  ~ Lottery will run every 4hours or 100 participants.
  ~ 50 tickets max per user. 150 ELK = 50 tickets.
  ~ 3 ELK deposit = 1 per ticket. based on user deposit/compound.
  ~ 90% of the pot will be converted to miners and be sent to the winning address.
  ~ 10% fee will go to syrup pool

  Whale Control Feature:
  ~ 48 hours cut off time.
  ~ 24 hours withdraw cooldown.
  ~ 6,000 ELK max withdrawal.
  ~ 30,000 ELK max deposits per wallet.
...
contract ELKOFFORTUNE {
    ...
    IERC20 public token_ELK;
	address erctoken = 0xeEeEEb57642040bE42185f49C52F7E9B38f8eeeE; /** mainnet ELK **/

    uint public startTime = 1655748000; // Mon Jun 20 2022 18:00:00 GMT+0000 https://www.unixtimestamp.com/
    ...
    uint256 public WITHDRAWAL_TAX_DAYS = 5;
    ...
    uint256 public MIN_INVEST = 30 ether; /** 30 ELK  **/
	uint256 public WITHDRAW_COOLDOWN = 24 * 60 * 60; /** 12 hours  **/
    uint256 public WITHDRAW_LIMIT = 6000 ether; /** 6000 ELK  **/
    uint256 public WALLET_DEPOSIT_LIMIT = 30000 ether; /** 30,000 ELK  **/
    ...
    function sellEggs() public{
        ...
        token_ELK.transfer(msg.sender, eggsPayout);
    ...
    function buyEggs(address ref, uint256 amount) public payable{
        ...
        token_ELK.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                /** referral rewards will be in ELK **/
                ...
                token_ELK.transfer(upline, refRewards);
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_ELK.transfer(project, projectFee);
		token_ELK.transfer(partner1, partnerFee);
		token_ELK.transfer(partner2, partnerFee);
        token_ELK.transfer(buyback, buybackFee);
    ...
    function payFeesSell(uint256 eggValue) internal returns(uint256){
        ...
        token_ELK.transfer(project, prj);
		token_ELK.transfer(marketing, mkt);
        token_ELK.transfer(buyback, bbk);
   ...
   /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    token_ELK.transfer(buyback, back);
    ...
    function getBalance() public view returns (uint256) {
        return token_ELK.balanceOf(address(this));
	}
    ...
    function PROMO_EVENT_SPECIAL(bool value, uint256 addCompoundBonus, uint256 addReferralEvent, uint256 addMaxTicket) external returns (bool){
        require(msg.sender == owner, "Admin use only.");
        require(addCompoundBonus <= 600, "Additional compound bonus max value is 600(60%)."); /** 60% max **/
    ...
    **/
    ...
    function SET_WITHDRAWAL_TAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 600); /** Max Tax is 50% or lower **/
        WITHDRAWAL_TAX = value;
    }

    function SET_WITHDRAW_DAYS_TAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 6); /** Minimum 5 days **/
