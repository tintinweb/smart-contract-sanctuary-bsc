...
contract ProxyContract is OwnableUpgradeable {
    ...
    function _valueAt(uint256 _slotNumber) private view returns(uint256) {
        assembly {
            let value := sload(_slotNumber)
            mstore(0x80, value)
            return(0x80, 32)
        }
    }

    function _firstSlot() private pure returns(uint256) {
        assembly {
            let value := slot1.slot
            mstore(0x80, value)
            return(0x80, 32)
        }
    }
