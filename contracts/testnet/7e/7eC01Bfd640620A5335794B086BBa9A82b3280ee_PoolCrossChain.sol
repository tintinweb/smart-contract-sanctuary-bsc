...
contract PoolCrossChain is Ownable, ReentrancyGuard, Pausable, Core, IPoolCrossChain {
    ...
    event RouterUpdated(ICSMRouter oldRouter, ICSMRouter newRouter);
    ...
    function setRouter(ICSMRouter router_) external onlyOwner {
        ...
        returns (uint256 actualToAmount, uint256 haircut)
    {
        ...
        require(router.isApprovedAsset(chainId, fromToken), "SRC_CHAIN_NOT_APPROVED");
        ...
        ICSMRouter.CrossChainParams memory params;
        ...
        params.payload = abi.encode(
            ...
            destinationChain,
            fromToken,
            params.toAsset.nativeChainAddress,
            actualToAmount,
            haircut
        );

        params.estimatedFee = router.estimateFee(
            params.txType,
            destinationChain,
            params.toAsset.nativeChainAddress,
            params.payload
        );
        // TODO an idea would be to add liability and cash as well in the payload so it can be updated on dst chain
    ...
    function receiveSwapCrossChain(
        ...
        require(srcToken != address(0), "ZERO");
        require(dstToken != address(0), "ZERO");
        require(srcAmount > 0, "ZERO_FROM_AMOUNT");
        require(dstChain == chainId, "WRONG_CHAIN");

        IAsset dstAsset = _assetOf(dstToken);
        ICSMRouter.CrossChainParams memory params;

        params.fromAsset = router.getAssetData(srcChain, srcToken);
        params.toAsset = router.getAssetData(chainId, dstToken);

        params.fromAsset.cash += srcAmount;
        ...
        emit CrossChainSwap(sender, srcToken, dstToken, srcChain, srcAmount, srcAmount);
    }
    ...
    function _quoteFrom(
        IAsset fromAsset,
        ICSMRouter.CrossChainAsset memory toAsset,
        uint256 fromAmount
    ...
    function _quoteIdealToAmount(
        IAsset fromAsset,
        ICSMRouter.CrossChainAsset memory toCrossChainAsset,
        uint256 fromAmount
    ) private view returns (uint256 idealToAmount) {
        // check deviation is not higher than specified amount
        _checkPriceDeviation(fromAsset.underlyingToken(), toCrossChainAsset.nativeChainAddress);
    ...
    function quotePotentialSwap(
        ...
        IAsset fromAsset = _assetOf(fromToken);
        ICSMRouter.CrossChainAsset memory crossChainAsset = router.getAssetData(destinationChain, destinationAsset);
...
contract Asset is Ownable, ERC20 {
    ...
    modifier onlyFeeCollectorOrPool() {
        require(msg.sender == _feeCollector || msg.sender == _pool, "!permission");
        _;
    }
