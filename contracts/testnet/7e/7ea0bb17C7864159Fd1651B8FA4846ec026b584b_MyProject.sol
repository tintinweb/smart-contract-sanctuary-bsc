...
contract Main is BEP20, SharedOwnable, SafeToken {
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimal_, address marketing_, address team_, address multiuse_, uint256 supply_, uint256 dispatch_, uint8 maxtaxes_) BEP20(name_, symbol_) {
        ...
        MaxTaxes = maxtaxes_;
 		_SellFee = _LiquidityFee.add(_MarketingFee).add(_TeamFee).add(_MultiUseFee).add(_token1DividendRewardsFee).add(_BurnFee);//17%
    ...
    function GetFees() public view returns(uint Buy, uint Sell, uint Wallet2Wallet, uint Liquidity, uint Marketing, uint Team, uint MultiUse, uint Dividend, uint Burn){return (_BuyFee, _SellFee, _Wallet2WalletFee, _LiquidityFee, _MarketingFee, _TeamFee, _MultiUseFee, _token1DividendRewardsFee, _BurnFee);}
    function GetLimits() public view returns(uint256 SellMax, uint256 WalletMax, uint256 TaxMax, uint256 Gas, uint256 MinSwap, uint256 SwapMax, bool SwapLiq, bool ENtrading, bool RewardAuto){return (MaxSell, MaxWallet, MaxTaxes, gasForProcessing, SwapMin, MaxSwap, swapAndLiquifyEnabled, tradingEnabled, AutoReward);}
    function GetContractAddresses() public view returns(address marketing, address team, address multiuse, address reward, address Dead, address LP){return (address(MarketingWallet), address(TeamWallet), address(MultiUseWallet), address(dividendTracker), address(DeadWallet), address(uniswapV2Pair));}
    ...
    function SetTaxes(uint256 newBuyTax, uint256 wallet2walletfee, uint256 newLiquidityTax, uint256 newBurnTax, uint256 newMarketingTax, uint256 newReward1Tax) external onlySharedOwners {
        require(newBuyTax <= MaxTaxes && newBuyTax >= newBurnTax, "Total Tax can't exceed MaxTaxes. or be lower than burn tax");
        require(newMarketingTax.add(newReward1Tax).add(newLiquidityTax).add(newBurnTax) <= MaxTaxes, "Total Tax can't exceed MaxTaxes.");
        require(newMarketingTax >= 0 && newBuyTax >= 0 && newReward1Tax >= 0 && newLiquidityTax >= 0 && newBurnTax >= 0,"No tax can be negative");
        ...
        _SellFee = _LiquidityFee.add(_MarketingFee).add(_token1DividendRewardsFee).add(_BurnFee);
    } 

    function SetLimits(uint256 _maxWallet, uint256 _maxSell, uint256 _minswap, uint256 _swapmax, uint256 newMinimumBalance, uint256 NewGas, uint256 claimWait, uint256 MaxTax, bool _swapAndLiquifyEnabled, bool autoreward) public onlySharedOwners {
        uint256 supply = totalSupply ();
    ...
    function _transfer(address from, address to, uint256 amount)  internal override {
       ...
       // preparation of launch LP and token dispatch allowed even if trading not allowed
        ...
        if(canSwap && !inSwapAndLiquify && !automatedMarketMakerPairs[from] && swapAndLiquifyEnabled && !_isWhitelisted[from] && !_isWhitelisted[to] && TotalFees != 0) {swapAndLiquify(contractTokenBalance);}
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 TotalFees = _SellFee.sub(_BurnFee);
        //int256 NoRewardFees = TotalFees;
        uint256 initialBalance = address(this).balance;
        uint256 tokensForLP = contractTokenBalance.div(TotalFees).mul(_LiquidityFee.div(2));
        ...
        uint256 totalBNBFee = TotalFees.sub(_LiquidityFee.div(2));
        //uint256 amountBNBLiquidity = amountReceived.mul(_LiquidityFee).div(totalBNBFee).div(2);
        //uint256 amountBNBRewards = amountReceived.mul(_token1DividendRewardsFee).div(totalBNBFee);
        ...
        uint256 liquidityBNBPortion = 0;
        uint256 rewardBNBPortion = 0;
        
        if(_MarketingFee != 0) {
            marketingBNBPortion = BNBBalance.div(totalBNBFee).mul(_MarketingFee);
            MarketingWallet.transfer(marketingBNBPortion);
        }
        if(_TeamFee != 0) {
            teamBNBPortion = BNBBalance.div(totalBNBFee).mul(_TeamFee);
            TeamWallet.transfer(teamBNBPortion);
        }
        if(_MultiUseFee != 0) {
            multiuseBNBPortion = BNBBalance.div(totalBNBFee).mul(_MultiUseFee);
            MultiUseWallet.transfer(multiuseBNBPortion);
        }
        if(_LiquidityFee != 0) {
            liquidityBNBPortion = BNBBalance.div(totalBNBFee).mul(_LiquidityFee.div(2));
            addLiquidity(tokensForLP, liquidityBNBPortion);
            //emit SwapAndLiquify(half, liquidityBNBPortion, half);
        }
        if(_token1DividendRewardsFee != 0) {
            rewardBNBPortion = BNBBalance.div(totalBNBFee).mul(_token1DividendRewardsFee);
            swapAndLiquifyDividends (rewardBNBPortion);
        }     
