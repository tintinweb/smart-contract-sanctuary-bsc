...
contract GENIPassSaleV2 is IGENIPassSaleV2, IERC721ReceiverUpgradeable, PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function putOnSale(uint passId, uint price, uint startTime, uint expirationTime) public override whenNotPaused onlyOperator {
        ...
        saleInfo.price = getPricePass(passId, price, IGENIPass.PriceType.GENI);
    ...
    function convertPricePass(uint passId, uint price) public view returns(uint pricePass) {
        if(price != 0) {
    ...
    function getDefauftPrice (uint passId) public view returns (uint pricePass) {
    ...
    function purchase(uint passId, uint buyPrice) external payable override whenNotPaused nonReentrant {
        ...
        uint salePrice = convertPricePass(passId, busdPrice);
    ...
    function purchaseWithReferral(uint passId, uint buyPrice, address referralAddress, uint nonce, bytes memory signature) external payable override whenNotPaused nonReentrant {
        ...
        // uint busdPrice = saleInfo.price;
        uint salePrice = convertPricePass(passId, saleInfo.price);
        uint payPrice = geniPass.balanceOf(buyer) > 0 ? (salePrice - (salePrice * discountRate / 100 / decimalRate)) : salePrice;
