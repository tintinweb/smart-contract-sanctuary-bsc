    ...
    ILaunchpadCrowdsaleNftNative
{
    ...
    modifier onlyBy(address account) {
        require(
            msg.sender == account,
            "LaunchpadCrowdsaleNftNative: sender unauthorized"
        );
        _;
    }
    ...
    function nativeBuyTokens() external payable override {
        _nativeBuyTokensFor(msg.sender, "");
    }
     ...
     * @param beneficiary Recipient of the token purchase
     */
    function nativeBuyTokensFor(address beneficiary) external payable override {
        _nativeBuyTokensFor(beneficiary, "");
    }
        ...
        onlyBy(governanceAccount)
    {
        require(
            account != address(0),
            "LaunchpadCrowdsaleNftNative: zero account"
        );
        ...
        isWhitelisted(beneficiary)
    {
        ...
        if (
            ...
            revert("LaunchpadCrowdsaleNftNative: beneficiary cap exceeded");
        }
...
/**
 * @title ILaunchpadCrowdsaleNftNative
 * @author Enjinstarter
 */
interface ILaunchpadCrowdsaleNftNative is ICrowdsale {
    function buyTokens(address paymentToken, uint256 lots) external;

    function nativeBuyTokens() external payable;
    ...
    function nativeBuyTokensFor(address beneficiary) external payable;
