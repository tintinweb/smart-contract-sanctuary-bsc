...
contract CandaoPriorityPass is Ownable, Pausable {
    ...
    // @dev: EVENTS
    event PriorityPassBought(address buyer, string reservationToken, uint256 packageIndex, uint256 price, uint256 fromUser, uint256 fromDeposit, uint256 domainLength);
    event DomainBought(address buyer, string[] reservationToken, uint256 price, uint256 fromUser, uint256 fromDeposit);
    event CDOBought(address buyer, uint256 amount, uint256 fromUser, uint256 fromDeposit);
    ...
    function buyPriorityPassAdmin(address _wallet, string memory domain, uint256 packagePrice) external onlyOwner {
        ...
        _userInfo[_wallet] = ProrityPass(packagePrice, characterCount, block.number, packagePrice, true);
        emit PriorityPassBought(_wallet, "", packagePrice, packagePrice, 0, 0, characterCount);
        emit CDOBought(_wallet, depositRest, 0, 0);
    }

    function buyPriorityPass(uint256 packagePrice, string memory domain, string memory reservationToken) external whenNotPaused returns (ProcessedPricing memory) {
        ...
        ProcessedPricing memory priceHistory = transferFromUser(totalPrice);
        emit PriorityPassBought(msg.sender, reservationToken, packagePrice, totalPrice, priceHistory.fromUser, priceHistory.fromDeposit, characterCount);
        return priceHistory;
    }
    ...
    function buyCDO(uint256 amount) external returns (ProcessedPricing memory) {
        require(_userInfo[msg.sender].valid, "CandaoCoordinator: Address isn't allowed to buy CDO tokens.");

        ProcessedPricing memory priceHistory = transferFromUser(amount);
        emit CDOBought(msg.sender, amount, priceHistory.fromUser, priceHistory.fromDeposit);
        return priceHistory;
    }

    function buyAdditionalDomain(string[] memory domains, string[] memory reservationTokens) external returns (ProcessedPricing memory) {
        ...
        ProcessedPricing memory priceHistory = transferFromUser(totalPrice);
        emit DomainBought(msg.sender, reservationTokens, totalPrice, priceHistory.fromUser, priceHistory.fromDeposit);
        return priceHistory;
    }
    ...
    function transferFromUser(uint256 amount) private returns (ProcessedPricing memory) {
        uint256 balance = _balances[msg.sender];
        if (balance == 0) {
            token.transferFrom(msg.sender, wallet, amount);
            return ProcessedPricing(amount, 0, amount);
        } else {
            if (balance < amount) {
                uint256 missing = amount - balance;
                _balances[msg.sender] -= balance;
                token.transferFrom(msg.sender, wallet, missing);
                token.transfer(wallet, balance);
                return ProcessedPricing(amount, balance, missing);
            } else {
                uint256 rest = balance - amount;
                _balances[msg.sender] = rest;
                token.transfer(wallet, amount);
                return ProcessedPricing(amount, amount, 0);
            }
        }
    }
