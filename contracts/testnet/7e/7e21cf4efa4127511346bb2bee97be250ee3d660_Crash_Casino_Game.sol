...
contract Crash_Casino_Game is SignVerify , Ownable, Pausable, ReentrancyGuard{
    ...
    function internalBet(uint256 _cycleID) private{
        ...
        betAmountAfterTax[_msgSender()][_cycleID] = _betamount;
    }

    function Return(uint256 _cycleID, uint256 _amount, uint256 _nonce, bytes memory signature)
    external
    whenNotPaused
    nonReentrant
    {
        require(msg.sender == tx.origin,"invalid caller!");
        require(betHolders[_msgSender()][_cycleID] == true, "Sorry, you are not eligible for a reward.");
        ...
        usedHash[hash] = true;
        require(betAmount[_msgSender()][_cycleID] == _amount, "Error");
    ...
    nonReentrant
    {
        ...
        _reward = _reward - betAmount[_msgSender()][_cycleID];
        ...
        _reward = betAmountAfterTax[_msgSender()][_cycleID] + _reward;
