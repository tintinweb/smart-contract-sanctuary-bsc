...
contract SquadGameBasic {
   // address public impl;
    ...
    address public SecContractOwner;
...
contract SquadGame is SquadGameBasic {
    ...
    modifier onlyContractOwner() { 
        require(msg.sender == contractOwner||msg.sender == SecContractOwner, "onlyOwner"); 
        _; 
    }
    ...
    function init(address _ownerAddress, address _SecOwnerAddress, address _secWallet, address _regWallet, IERC20 _depositTokenAddress) public onlyContractOwner {
        SecContractOwner = _SecOwnerAddress;
        RegFee = 3e15;
        BASIC_PRICE = 5e15;
        LAST_LEVEL = 16;
        ...
        levelPrice[9] = 120e15;
        levelPrice[10] = 180e15;
        levelPrice[11] = 250e15;
        levelPrice[12] = 360e15;
        levelPrice[13] = 490e15;
        levelPrice[14] = 640e15;
        levelPrice[15] = 810e15;
        levelPrice[16] = 1000e15;
    ...
    function sendETHDividends(uint8 level) private returns (bool) {
        ...
        {
            //800000000000000 3000000000000  650000000000000
        }else
        {
    ...
    {
        uint32 count = TotolLevelUserCount(level);
    ...
    function TotolLevelUserCount(uint8 level) public view returns (uint32 count)
    {
    ...
    function UserRewards(address userAddress)public view returns (uint64[] memory){
        ...
        for (uint8 i=0; i < LAST_LEVEL; i++) 
        {
            uint32 level_length = TotolLevelUserCount(i+1);
    ...
    function ProgressID(address userAddress)public view returns (uint64[] memory){
        ...
        for (uint8 i=0; i < LAST_LEVEL; i++) 
        {
            uint64 level_length = TotolLevelUserCount(i+1);
            uint32 levelid =LevelPlace[i];
            DownPos=0;
            if(level_length>0)
            {                   
