...
contract NFT721 is Ownable, AccessControl, ERC721, ERC721Enumerable, ERC721Burnable, ERC721Pausable, ERC2981PerTokenRoyalties {
    ...
    constructor() ERC721("NFT721", "ONE") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ROLE_CREATOR, msg.sender);
    ...
    function create(address _to) external onlyRole(ROLE_CREATOR) returns (uint256) {
	...
	function create(address _to, address royaltyRecipient, uint256 royaltyValue) external onlyRole(ROLE_CREATOR) returns (uint256) {
		uint256 _id = _getNextTokenID();
		_incrementTokenTypeId();
    ...
    function mint(address _to, uint256 _id) public onlyRole(ROLE_MINTER) {
        _mint(_to, _id);
    }

    function mintBatch(address _to, uint256[] memory _ids) public onlyRole(ROLE_MINTER) {
        for (uint256 i = 0; i < _ids.length; i++) {
            _mint(_to, _ids[i]);
        }
    }

    function burn(uint256 _id) public onlyRole(ROLE_BURNER) override {
        super.burn(_id);
    }

    function burnBatch(uint256[] memory _ids) public onlyRole(ROLE_BURNER) {
        for (uint256 i = 0; i < _ids.length; i++) {
            super.burn(_ids[i]);
        }
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControl, ERC721, ERC721Enumerable, ERC2981Base) returns (bool) {
