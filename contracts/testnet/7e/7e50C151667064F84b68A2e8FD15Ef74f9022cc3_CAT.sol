...
/*
Crypto Arbitrage Team official smart contract.

🌐 Official website: mycat.team
*/
...
contract CAT is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256[10] public PERCENTAGES = [120, 80, 60, 40, 20, 20, 10, 10, 5, 5];

    mapping(address => bool) public pendingWithdrawal;
    ...
    function withdraw(uint256 amount) external whenStarted whenNotPaused nonReentrant {
        require(amount > 0, "Zero amount");
        require(!pendingWithdrawal[_msgSender()], "Current pending withdrawal");
        _updateNotWithdrawn(_msgSender());
        require(amount <= stake[_msgSender()].notWithdrawn, "Balance too low");
    ...
    function editPendingWithdrawal(address[] calldata account, bool[] calldata _pendingWithdrawal) external onlyOwner {
        require(account.length == _pendingWithdrawal.length, "Non-matching length");
        for (uint256 i; i < account.length; i++) {
            pendingWithdrawal[account[i]] = _pendingWithdrawal[i];
        }
    }
