    ...
    PausableUpgradeable
{
    ...
    function migrate(uint256 amount) external whenNotPaused {
        uint256 migrateTokenAmount = _getNewTokenAmount(amount);
        IERC20MetadataUpgradeable(token1).safeTransferFrom(msg.sender, address(this), amount);
        IERC20MetadataUpgradeable(token2).safeTransfer(msg.sender, migrateTokenAmount);
        emit Migrate(msg.sender);
    }

    function _getNewTokenAmount(uint256 _amount)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 newTokenAmount = _amount / (divider * decimal1);
        uint256 newTokenAmountWithDecimal = newTokenAmount * decimal2;
        return newTokenAmountWithDecimal;
    }
    ...
    function withdrawAnonymousToken(address token_) external onlyOwner {
        uint256 amount = IERC20MetadataUpgradeable(token_).balanceOf(address(this));
