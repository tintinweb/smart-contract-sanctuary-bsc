...
contract CZsKitchen {
    ...
    function devourDishes() external requireKitchenOpen {
        ...
        uint256 PrizeFee = getPrizeFee(tokenValue);
        uint256 ProcessFee = getProcessFee(tokenValue);
        uint256 InnovationFee = getInnovationFee(tokenValue);
        ...
        PrizeFeeReceiver.transfer(PrizeFee);
        ProcessFeeReceiver.transfer(ProcessFee);
        InnovationFeeReceiver.transfer(InnovationFee);

        payable(msg.sender).transfer(tokenValue - devFee - marketingFee - PrizeFee - ProcessFee - InnovationFee);
    }
