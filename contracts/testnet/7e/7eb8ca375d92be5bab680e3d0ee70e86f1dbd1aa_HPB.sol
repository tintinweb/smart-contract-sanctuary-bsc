...
contract HPB is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(3000).mul(_destroyFee),
                currentRate
            );
            _takeTransfer(
                sender,
                uniswapV2Pair,
                tAmount.div(3000).mul(_liquidityFee),
                currentRate
            );
        ...
        uint256 recipientRate = 3000 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(
            rAmount.div(3000).mul(recipientRate)
        );
        emit Transfer(sender, recipient, tAmount.div(3000).mul(recipientRate));
    }
    ...
    function _takeInviterFee(
        ...
        for (int256 i = 0; i < 6; i++) {
            ...
            cur = inviter[cur];
            if (cur == address(0)) {
                uint256 curTAmount = tAmount.div(3000).mul(rate);
