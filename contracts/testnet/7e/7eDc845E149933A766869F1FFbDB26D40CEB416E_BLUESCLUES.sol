...
contract BLUESCLUES is Context, IERC20, ERC20Ownable {
    ...
    string private constant _name = "BLUESCLUES";
    ...
    constructor() payable {
        ...
        MarketingWallet = payable(0xaA93f395d7195bB3ACCE7251E7b06f3029fA6FCC);
        CharityWallet = payable(0x8cB6520aF3e5A3cAC64c1fDCBcF3173Dbbb126D6);
        DevWallet = payable(0x8cB6520aF3e5A3cAC64c1fDCBcF3173Dbbb126D6);
        ...
        _isMaxWalletExclude[CharityWallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(_initiateTrades == true) {
            ...
            _isMaxWalletExclude[address(pcsV2Pair)] = true;
            _isMaxWalletExclude[address(pcsV2Router)] = true;
            _isExcludedMaxTxnAmount[address(pcsV2Router)] = true;
            _isExcludedMaxTxnAmount[address(pcsV2Pair)] = true;
            limitsOn = true;
            maxWalletOn = true;
        ...
        uint256 totalTokensToSwap = tokensForLiquidity.add(tokensForMarketing).add(tokensForCharity);
        uint256 contractTokenBalance = balanceOf(address(this));
        bool overMinimumTokenBalance = contractTokenBalance >= _minTaxSwap;
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(pcsV2Pair) > 0 && totalTokensToSwap > 0 && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && to == pcsV2Pair && overMinimumTokenBalance) {
            swapTokens();
            }
        bool takeFee = true;
    ...
    function swapTokens() private lockTheSwap {
        ...
        uint256 totalTokensToSwap = tokensForMarketing + tokensForCharity + tokensForLiquidity;
        uint256 swapLiquidityTokens = tokensForLiquidity.div(2);
        uint256 amountToSwapForBNB = contractBalance.sub(swapLiquidityTokens);
        ...
        uint256 bnbForCharity = bnbBalance.mul(tokensForCharity).div(totalTokensToSwap);
        uint256 bnbForLiquidity = bnbBalance.sub(bnbForMarketing).sub(bnbForCharity);
        ...
        addLiquidity(swapLiquidityTokens, bnbForLiquidity);
    ...
    function removeAllFee() private {
        if (_reflectionsTax == 0 && _liquidityTax == 0 && _marketingTax == 0 && _charityTax == 0) return;
    ...
    function setBuyTax(uint256 _buyLiquidityTax, uint256 _buyReflectionsTax, uint256 _buyMarketingTax, uint256 _buyCharityTax) external onlyOwner {
        ...
        require(_buyLiquidityTax + _buyReflectionsTax + _buyMarketingTax + _buyCharityTax <= 25);
    }
    function setSellTax(uint256 _sellLiquidityTax, uint256 _sellReflectionsTax, uint256 _sellMarketingTax, uint256 _sellCharityTax) external onlyOwner {
        ...
        require(_sellLiquidityTax + _sellReflectionsTax + _sellMarketingTax + _sellCharityTax <= 25);
    }
