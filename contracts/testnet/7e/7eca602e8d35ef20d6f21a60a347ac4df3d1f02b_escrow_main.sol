...
contract escrow_main is Ownable,  ReentrancyGuard, AccessControl {
    ...
    IERC20 coin = IERC20(0xb883C5E72AC27c5f0B8A5233C6b9c8cf034C5371);
    ...
    event GigApprovedByBuyer(
        uint256 gigIndex,
        address indexed approvedByBuyer
    );
    event GigStatusChanged(
        uint256 gigIndex,
        bool activeStatus
    );
...
*/
    function seller_makeGig( string memory _title, string memory _description, address _buyer, string memory _milestones, uint256[] memory _pricePerMilestone , uint256 deadLineTimeInSeconds, string memory _feePaidBy ) public returns(uint256) {

        _gigCounter++;
        gigMap[_gigCounter] = gig( msg.sender, _buyer, true, false, true, _pricePerMilestone.length, _milestones, block.timestamp, deadLineTimeInSeconds, _title, _description, _feePaidBy );
    ...
    function seller_flipGigStatus(uint256 _id) public {
        ...
        emit GigStatusChanged(
            _id,
            gigMap[_id].gigActive
        );
    ...
    function buyer_approveGig(uint256 _id) public nonReentrant() {
        ...
        for(uint256 i=0; i<gigMap[_id].numberOfMilestones; i++){
            totalFee += pricePerMilestone[_id][i];
        }

        require ( coin.allowance(msg.sender, address(this)) >= totalFee , "Allowance not given");

        coin.transferFrom( msg.sender, platformFeeAccount , platformFee ); // Taking platform Fee from buyer
        coin.transferFrom( msg.sender, address(this), totalFee - platformFee  ); // Taking money into Escrow
        

        gigMap[_id].approvedByBuyer = true;

        emit GigApprovedByBuyer(
            _id,
            msg.sender
        );
...
*/
    ...
    function buyer_makeGig( string memory _title, string memory _description, address _seller, string memory _milestones, uint256[] memory _pricePerMilestone, uint256 deadLineTimeInSeconds, string memory _feePaidBy ) public nonReentrant() returns(uint256) {

        _gigCounter++;
        gigMap[_gigCounter] = gig( _seller, msg.sender, false, true, true, _pricePerMilestone.length, _milestones, block.timestamp, deadLineTimeInSeconds, _title, _description, _feePaidBy);
...
*/
    ...
    function buyer_approveMilestone(uint256 _gigId, uint256 _milestoneId) public nonReentrant() {
        ...
        require( _milestoneId >= 0 &&  _milestoneId < gigMap[_gigId].numberOfMilestones, "Undefined milstone id");
        ...
        coin.transfer( gigMap[_gigId].seller , pricePerMilestone[_gigId][_milestoneId] );
    ...
    function getPriceOfMilestonesOfGig(uint256 _id) public view returns(
        uint256[] memory
    ){
