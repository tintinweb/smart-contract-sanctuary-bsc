...
import "./../proxies/BHOLaunchpadProxy.sol";

interface ILaunchpad is SharedConstants {
    function initialize(
...
contract BHOLaunchpadFactory is Initializable, SharedConstants {
    ...
    function initialize(IBHOStaking _staking, IAddressDecentralization _addrDec, address _launchpadLogic, address _proxyAdmin)
        public
        initializer
    {
        staking = _staking;
        addrDec = _addrDec;
        launchpadLogic = _launchpadLogic;
    ...
    function createLaunchpad(
        ...
        bytes memory data = abi.encodeWithSelector(
            ILaunchpad(launchpadLogic).initialize.selector,
            _launchpadData,
            _vestingTimeline,
            staking,
            addrDec
        );
        BHOLaunchpadProxy launchpad = new BHOLaunchpadProxy(
            launchpadLogic,
            proxyAdmin,
            data
        );
    ...
    /**
     * @dev update launchpad logic
     */
    function updateLaunchpad(address _launchpadLogic)
        public
        onlyRole("manager")
    {
        launchpadLogic = _launchpadLogic;
...
contract BHOLaunchpadProxy is TransparentUpgradeableProxy {
    constructor(
        address _logic,
        address admin_,
        bytes memory _data
