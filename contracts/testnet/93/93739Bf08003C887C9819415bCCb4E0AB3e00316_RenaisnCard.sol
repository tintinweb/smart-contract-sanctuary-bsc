...
contract RenaisnCard is Ownable, ERC721 {
    uint256 private _cardId;

    string private _boxUrl;
    mapping(uint256 => string) private cardUrl;
    mapping(address => bool) public miners;

    mapping(uint256 => uint256) public cardLevel;
    mapping(uint256 => string) public cardUuid;
    mapping(uint256 => bool) public isActivated;

    event ActivatedCard(address indexed account_, uint256 indexed tokenId_);

    constructor() ERC721("RenaisnBox", "RenaisnBox") {}
    ...
    function setBoxUrl(string memory url) external onlyOwner {
        _boxUrl = url;
    }

    function setCardUrl(uint256 lv, string memory url) external onlyOwner {
        cardUrl[lv] = url;
    }

    function mintTo(address account_) external onlyMiner returns (uint256) {
        _cardId += 1;
        uint256 newItemId = _cardId;
        _mint(account_, newItemId);
        return newItemId;
    }
    ...
    function activateCard(
        ...
        require(ownerOf(cardId) == account, "Card owner error");

        cardLevel[cardId] = lv;
        cardUuid[cardId] = uuid;
        isActivated[cardId] = true;
        emit ActivatedCard(account, cardId);
    }
    ...
    function _transfer(
        ...
        require(!isActivated[cardId], "Disable Transfer error");
        super._transfer(from, to, cardId);
    }
        ...
        returns (string memory)
    {
        ...
        if (!isActivated[cardId]) {
            return _boxUrl;
        }

        string memory _url = cardUrl[cardLevel[cardId]];
