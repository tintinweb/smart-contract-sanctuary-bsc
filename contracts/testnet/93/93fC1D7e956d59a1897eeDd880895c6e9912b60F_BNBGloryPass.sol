 ...
 */
contract BNBGloryPass is Ownable, ERC165, IERC721, IERC721Metadata, IStarNFT {
    ...
    event EventUpdateOwnerMintable(
        bool oldOwnerMintable,
        bool newOwnerMintable
    );
    ...
    modifier allowMint(address account, uint256 amount) {
        if (!_ownerMintable) {
            require(
                _balances[account] == 0 && amount == 1,
                "StarNFT: owner can not mint new nft"
            );
        }
        _;
    }
