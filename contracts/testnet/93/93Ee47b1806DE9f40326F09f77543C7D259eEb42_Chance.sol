...
contract Chance is ERC721URIStorage, ERC721Enumerable, Ownable{
    ...
    // mapping(uint256 => uint256) public jackpotMap;
    ...
    function drop_winner_token_id() private returns (uint256){
        // start 
        ...
        require(end >= start, "not enough lottery conditions");

        // test seed
        // uint256 _seed = uint256(keccak256(abi.encode(block.difficulty, block.timestamp)));
        // uint256 _seed = VRFv2Consumer(VRF_ADDR).s_randomWords(round%24);
        ...
        // address winner_addr = ownerOf(winner_id);
        // add winner jackpot
    ...
    function withdrawByWinner(uint256 _round, address payable _to) public {
