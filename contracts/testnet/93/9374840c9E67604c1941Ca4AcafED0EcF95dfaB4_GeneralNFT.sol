...
contract GeneralNFT is ERC721, Ownable, SignatureChecker {
    ...
    function mintPass(Pass memory pass, uint256 nonce, uint256 tokenId) public checker(pass,nonce,tokenId){
...
contract SignatureChecker {
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => bool))) internal _mintedAddresses;

    constructor() {
    }


    function _isVerifiedPass(bytes32 digest, Pass memory pass)
        internal
        view
        returns (bool)
    {
    ...
    function _createMessageDigest(address _address, uint256 nonce, uint256 tokenId)
        internal
        pure
        returns (bytes32)
    {
    ...
    modifier checker(Pass memory pass, uint256 nonce,uint256 tokenId) {
        require(
            _isVerifiedPass(_createMessageDigest(msg.sender, nonce,tokenId), pass),
            "Pass is not valid."
        );
        require(
            !_mintedAddresses[msg.sender][nonce][tokenId],
            "Wallet has already minted."
        );
        _mintedAddresses[msg.sender][nonce][tokenId] = true;
        _;
    }
