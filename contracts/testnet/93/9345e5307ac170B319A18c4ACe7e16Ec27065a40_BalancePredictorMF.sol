...
contract BalancePredictorMF {
    ...
    address public MoonForce = 0x2144eB7DDD44a79c102d34A9565db5Fafbd12666;
    address public Gravity = 0x3D3e4eA1f554d9ee1a632abe83c256AE86e3C5A5;


    function predictMFBalance() external view returns (uint256) {
        uint256 weightedBalanceMF = (IBEP20(MoonForce).balanceOf(address(msg.sender)) / totalSupplyMF);
        uint256 weightedBalanceGV = (IBEP20(Gravity).balanceOf(address(msg.sender)) / totalSupplyGV);
    ...
    function predictBalanceIfStaked(uint256 _amountMF, uint256 _amountGV) external view returns (uint256) {
