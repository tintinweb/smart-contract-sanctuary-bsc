...
import "./libraries/CompileStrings.sol";
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    baseTokenURI(),
                    CompileStrings.toString(_tokenId)
                )
            );
    }
...
library CompileStrings {
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        ...
        uint256 digits;
        while (temp != 0) {
            digits = SafeMath.add(digits, 1);
            temp = SafeMath.div(temp, 10);
        }
        bytes memory buffer = new bytes(digits);
        uint256 index = SafeMath.sub(digits, 1);
        temp = value;
        while (temp != 0) {
            buffer[SafeMath.sub(index, 1)] = bytes1(uint8(48 + (temp % 10)));
            temp = SafeMath.div(temp, 10);
            index = SafeMath.sub(index, 1);
        }
