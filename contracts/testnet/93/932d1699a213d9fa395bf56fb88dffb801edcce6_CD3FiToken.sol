...
contract CD3FiToken is Context, IERC20, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity = _tTotal.mul(1).div(1e4);
    uint256 private buyBackUpperLimit = 1 * 10**18;
    
    uint256 private constant maxBracketTax = 750;                       // max bracket is holding 75%
    uint256 private constant taxBracketMultiplier = 10;
    ...
    uint8 private _stabilityFee = 0;
    uint8 private _previousStabilityFee = _stabilityFee;

    uint8 private _MarketingFee = 0;
    uint8 private _previousMarketingFee = _MarketingFee;

    uint8 private _buybackburnFee = 0;
    uint8 private _previousBuybackburnFee = _buybackburnFee;
    ...
    struct BuyFee{
        uint8 setTaxFee;
        uint8 setRewardFee;
        uint8 setStabilityFee;
        uint8 setMarketingFee;
        uint8 setBuyBackBurnFee;       
    }
    ...
    returns (uint256)
    {
        ...
        uint256 totalCap = (userTotal * (1000)) / (getTokensInLPCirculation());
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _rewardFee == 0 && _stabilityFee == 0 && _buybackburnFee == 0) return;  //_MarketingFee == 0
    ...
    function setDynamicTax(uint _addTax) private {   
        ...
        uint8 adder = uint8(_addTax.mul(250).div(1000));           
