...
contract SmartChef is Ownable {
    ...
    function stopReward() public onlyOwner {
        bonusEndBlock = block.number.div(100);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number.div(100) > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number.div(100));
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.number.div(100) <= pool.lastRewardBlock) {
            return;
        }
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (lpSupply == 0) {
            pool.lastRewardBlock = block.number.div(100);
            return;
        }
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number.div(100));
        uint256 cakeReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
