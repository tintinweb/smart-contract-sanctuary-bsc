...
contract Atlas is ERC20Detailed, Ownable {
    ...
    uint256 private _rebaseRate = 2432;
    ...
    constructor() ERC20Detailed("Atlas", "ATL", uint8(DECIMALS)) Ownable() {
    ...
    function addLiquidity(uint256 _liqTokens) internal swapping {
        
        uint256 amountToLiquify = _liqTokens.div(2);
        uint256 amountToSwap = _liqTokens.sub(amountToLiquify);
    ...
    function swapBack() internal swapping {
        ...
        uint256 burnTokens = contractTokenBalance.div(totalFee).mul(firePitFee);
        uint256 liqTokens = contractTokenBalance.div(totalFee).mul(liquidityFee);
        uint256 treasuryTokens = contractTokenBalance.sub(liqTokens).sub(burnTokens);
        
        _basicTransfer(address(this), DEAD, burnTokens);
        addLiquidity(liqTokens);
        ...
        uint256 amountBNBToTreasury = address(this).balance.sub(
            balanceBefore
        );

        payable(treasuryReceiver).transfer(amountBNBToTreasury);
    }
    ...
    function setRebaseRate(uint256 rebaseRate) external onlyOwner {
