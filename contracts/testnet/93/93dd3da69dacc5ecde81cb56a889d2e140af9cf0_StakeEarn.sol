...
contract StakeEarn is Ownable, ReentrancyGuard {
    ...
    uint256 public Bonus = 125;

    // Min stake limit per user
    uint256 public minStake = 20 * 10**18;
    ...
    function deposit(uint256 amount) external nonReentrant {
		...
		uint256 reward    = amount.mul(Bonus).div(10000) ;
	...
	function withdraw(uint256 depositNumber) public {
	   ...
	   totalStaked = totalStaked.sub(user.deposits[depositNumber].amount);
	   totalReward = totalReward.sub(user.deposits[depositNumber].reward);
    ...
    function SetStakeBonus(uint256 bonus) external onlyOwner {
        Bonus = bonus;
    }
