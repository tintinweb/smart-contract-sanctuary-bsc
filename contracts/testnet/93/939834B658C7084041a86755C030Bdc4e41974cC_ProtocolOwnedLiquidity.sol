 ...
 */
contract ProtocolOwnedLiquidity is AccessControl, Initializable{
    ...
    Note Need to call setBUSDProtocolAddress function once we deployed BUSD_Protocol contract
     */
    function initialize(address _owner,address _Admin) external initializer {
        Admin=IAdmin(_Admin);
        require(Admin.Treasury() != address(0), 'ProtocolOwnedLiquidity:: initialize: Treasury Address can not be null');
        require(Admin.BUSD() != address(0), 'ProtocolOwnedLiquidity:: initialize: Address can not be null');
        require(Admin.swapPage() != address(0), 'ProtocolOwnedLiquidity:: initialize: Address can not be null');
        _setupRole(ADMIN_ROLE, _owner);
        _setupRole(WITHDRAW_ROLE, Admin.swapPage());
    }

    /**
    @dev this function is only called by address having withdraw roles i.e. BUSD_Protocol and swappage

    @param _receiver address who receive that amount
    @param _amount amount to transfer

    Note if caller is BUSD_Protocol then receiver should be treasury
        if caller is swappage then receiver should be user
     */
    function withdrawFromPOL(address _receiver, uint _amount) external onlyRole(WITHDRAW_ROLE) {
        require(_amount > 0, 'ProtocolOwnedLiquidity: Amount can not be zero');
        if(msg.sender == Admin.BUSDProtocol()){
            require(_receiver == Admin.Treasury(),"ProtocolOwnedLiquidity: Invalid receiver");
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(),_amount);
        } else {
            IERC20(Admin.BUSD()).transfer(_receiver,_amount);
        }
    }

    /**
        @dev this function in only called by address having Admin Role
        @param amount amount to transfer
        Note caller is BUSD_Protocol then receiver should be SwapPage
     */

    function convertedBUSDForbYSL(uint amount) external onlyRole(ADMIN_ROLE) {
        IERC20(Admin.BUSD()).transfer(Admin.swapPage(),amount);        
    }
}
