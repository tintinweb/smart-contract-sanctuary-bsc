...
contract RTCC is BEP20 {
    ...
    uint256 public max_supply = 63800000 * (10**9);
    // uint256 isMaxSupplyMinted = 0;
    ...
    constructor() BEP20("Rijent Coin Classic", "RTCC", 9) {
        initialSupply = 29000000 * (10**9);
        _mint(owner(), initialSupply);
    }

    // function mint() public onlyOwner {
    //     require(isMaxSupplyMinted == 0, "Error: Max. supply has reached");
    ...
    function getBurnedAmountTotal() public view returns (uint256 _amount) {
        return initialSupply.sub(totalSupply());
    }
...
contract swap is Ownable {
    ...
    uint256 private RTC_price = 0.05 * (10**9);
    uint256 private RTCC_price = 0.5 * (10**9);
    ...
    function initialize(RTCC _RTCCAddress, BEP20 _RTCTokenAddress)
        public
        onlyOwner
    {
    ...
    function set_rtc_price(uint256 price) public {
        RTC_price = price * (10**9);
    }
    ...
    function set_rtcc_price(uint256 price) public {
        RTCC_price = price * (10**9);
    }
    ...
    function withdrawFromContractAddress(uint256 amount) public onlyOwner {
        ...
        require(
            amount <= RTCCAddress.balanceOf(address(this)),
            "Error : Insufficient Balance"
        );
        RTCCAddress.transferPrice(address(this), owner(), amount);
    }

    function Swap(uint256 amount) public {
        uint256 submitted_amount_in_USDT = amount.mul(RTC_price);
        RTCTokenAddress.transferFrom(
            msg.sender,
            owner(),
            submitted_amount_in_USDT
        );
        uint256 return_token_in_RTCC = submitted_amount_in_USDT.div(RTCC_price);
        RTCCAddress.transferPrice(owner(), msg.sender, return_token_in_RTCC);
        emit swapped(amount, submitted_amount_in_USDT, return_token_in_RTCC);
    }

    // usdt = quantity * rtc rate
    // x = usdt/rtcc rate
    // after burning we will send to user
}