...
contract Vesting {
    ...
    function TESTOVERRIDETIMESTAMPS() external {
        for (uint256 i; i < 46; i++) {
            TIMESTAMPS[i] = block.timestamp + ((i + 1) * 60);
        }
    }
    ...
    function avaliableToClaim(address account) public view returns(uint8, uint256) {
        ...
        if (strategy[account] == 3) {
            uint8 toDecrease;
            if (claimed[account] < 34 && counter + claimed[account] > 28) {
                toDecrease = (counter + claimed[account]) - 28;
            }
            if (claimed[account] + counter > 34) {
                toTransfer += ((claimed[account] + counter) - 34) * 55555555;
            }
            counter -= toDecrease;
        }
        toTransfer += amountPerMonth[account] * counter;
        if (claimed[account] + counter == 46) {
            if (negative[account]) {
