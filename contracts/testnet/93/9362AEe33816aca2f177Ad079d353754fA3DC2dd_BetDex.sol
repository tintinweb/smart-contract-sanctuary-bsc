...
contract BetDex is AccessControl, ReentrancyGuard, EIP712 {
    ...
    mapping(bytes32 => mapping(uint256 => mapping(uint16 => bytes32[])))
        private gameBetsArr;
    ...
    function getUserBalance(bytes32 roomId, address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function lockUserBalance(bytes32 roomId, address userAddress) internal {
        require(
    ...
    function unlockUserBalance(bytes32 roomId, address userAddress) internal {
        require(
            userBalanceMutexes[roomId][userAddress],
            "FAILED_TO_UNLOCK_USER_BALANCE"
        );
        delete userBalanceMutexes[roomId][userAddress];
    }
    ...
    {
        require(
            (input.timestamp - SIGNATURE_TIMESTAMP_THRESHOLD) < block.timestamp,
            "INVALID_TIMESTAMP"
        );
        return _validateAuthorization(hash, input.v, input.r, input.s);
    }
    ...
    function batchSignedBet(Input calldata input) external {
        require(
            usedNonce[input.bets[0].bettor][input.nonce] == false,
            "Nonce already used"
        );
        ...
        _validateSignatures(input, betSignHash);
        (
            uint256 totalBetAmount,
            uint256 totalPotentialWinningAmount
        ) = _preparePlaceBets(input, betHashes);
        _transferFees(input.roomId, input.bets[0].bettor, totalBetAmount);
        _transferRoomReverse(
            input.roomId,
            totalBetAmount,
            totalPotentialWinningAmount
        );
    }

    function _preparePlaceBets(Input calldata input, bytes32[] memory betHashes)
        internal
        returns (uint256, uint256)
    {
        ...
        uint256 betLength = input.bets.length;
        for (uint256 i = 0; i < betLength; i = _uncheckedInc(i)) {
            require(input.bets[i].amount > 0, "Amount must be nonzero");
            require(
                bettorAddress == input.bets[i].bettor ||
                    bettorAddress == address(0),
                "Not allow multiple bettor"
            );
            uint256 potentialWinningAmount = (input.bets[i].amount *
                input.bets[i].odds) / (10**ODDS_DECIMAL);
            ...
            gameBetsArr[input.roomId][input.bets[i].matchId][
                input.bets[i].gameType
            ].push(betHashes[i]);
            if (
                !gameBetRoomsArrState[input.bets[i].matchId][
                    input.bets[i].gameType
                ][input.roomId]
            ) {
                gameBetRoomsArrState[input.bets[i].matchId][
                ...
                gameBetRoomsArr[input.bets[i].matchId][input.bets[i].gameType]
                    .push(input.roomId);
            }
    ...
    function _transferFees(
        ...
        if (userBalance == 0) {
            ...
            emit TransferToRoom(roomId, contractAddress, userAddress, amount);
        } else if (userBalance < amount) {
            lockUserBalance(roomId, userAddress);
            transferAmount = amount - userBalance;
            userBalances[roomId][userAddress] = 0;
            unlockUserBalance(roomId, userAddress);
            ...
            lockUserBalance(roomId, msg.sender);
            userBalances[roomId][userAddress] = userBalance - amount;
            unlockUserBalance(roomId, msg.sender);
        }
    }

    function _transferRoomReverse(
        bytes32 roomId,
        uint256 betAmount,
        uint256 potentialWinningAmount
    ) internal {
        lockRoomReverse(roomId);
        rooms[roomId].reverse += betAmount;
        require(
            roomAvailableReserve(roomId) >= potentialWinningAmount,
            "FAILED_TO_BET"
        );
        rooms[roomId].lockedReverse += potentialWinningAmount;
        unlockRoomReverse(roomId);
    }
    ...
    function resolveBet(
        ...
        uint256 roomLength = gameBetRoomsArr[matchId][gameType].length.min(
            maxRoomLength
        );
        uint256 targetLength = _targets.length;
        require(roomLength > 0, "NO_BETS");
        for (uint256 i = 0; i < targetLength; i = _uncheckedInc(i)) {
            targets[_targets[i]] = true;
        }
        for (uint256 i = 0; i < roomLength; i = _uncheckedInc(i)) {
            bytes32 roomId = gameBetRoomsArr[matchId][gameType][i];
            uint256 length = gameBetsArr[roomId][matchId][gameType].length.min(
                maxBetLength
            );
            for (uint256 j = 0; j < length; j = _uncheckedInc(j)) {
                bytes32 betSlipId = gameBetsArr[roomId][matchId][gameType][i];
            ...
            delete gameBetRoomsArr[matchId][gameType][i];
        }
        for (uint256 i = 0; i < targetLength; i = _uncheckedInc(i)) {
    ...
    function numberOfBets(uint256 matchId, uint16 gameType)
        public
        view
        virtual
        returns (uint256)
    {
        uint256 count = 0;
        uint256 roomLength = gameBetRoomsArr[matchId][gameType].length;
        for (uint256 i = 0; i < roomLength; i = _uncheckedInc(i)) {
            bytes32 roomId = gameBetRoomsArr[matchId][gameType][i];
            uint256 length = gameBetsArr[roomId][matchId][gameType].length;
            count += length;
        }
        return count;
    }
...
contract EIP712 {
    ...
    bytes32 public constant BET_TYPEHASH =
        keccak256(
            "Bet(uint16 gameType,uint16 target,uint32 odds,uint256 amount,address bettor,uint256 matchId)"
        );
    ...
    {
        bytes32[] memory betHashes = new bytes32[](bets.length);
        ...
        return
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
                    DOMAIN_SEPARATOR,
                    keccak256(abi.encode(ROOT_TYPEHASH, root))
                )
            );
    }
}
