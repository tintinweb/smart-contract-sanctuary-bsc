...
contract HoldersPartition is UUPSUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyRFTXAdmin() {
        require(rftxAdmin == address(msg.sender), "Not a RFTX admin!");
        _;
    }
    ...
    function process() external onlyRFTXAdmin {
        _process();
    }
    ...
    function recordTransactionHistory(address payable account, uint256 amount, bool isSell) external onlyRFTXAdmin {
    ...
    function setClaimWait(uint256 _newvalue) external onlyRFTXAdmin {
        require(claimWait != _newvalue, "alredy same value");
        claimWait = _newvalue;
    }

    function setEligiblePeriod(uint256 _newvalue) external onlyRFTXAdmin {
        require(eligiblePeriod != _newvalue, "alredy same value");
        eligiblePeriod = _newvalue;
    }

    function setEligibleMinimunBalance(uint256 _newvalue) external onlyRFTXAdmin {
        require(minimumBalance != _newvalue, "alredy same value");
        minimumBalance = _newvalue;
    }

    function setTierPercentage(uint256 _tierIndex, uint256 _newvalue) external onlyRFTXAdmin {
    ...
    function updateTotalBUSD() external onlyRFTXAdmin {
        totalBUSD = IERC20(REWARD).balanceOf(address(this));
    }

    function updateRFTXDividendTrackerToken(address _rftx_div_token) external onlyOwner() {
