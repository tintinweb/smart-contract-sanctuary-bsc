...
contract NFTAuction {
    ...
    // ERC20 citrus = ERC20(0xAe67Cf598a349aFff89f6045108c6C1850f82839);  // Mainnet
    ERC20 citrus = ERC20(0x7f629f02e0E9529887146d04efa633f2219Bb5b4);  // Testnet
    ...
    struct Auction {
        ...
        uint citrusBuyNowPrice; // Citrus token price
        uint nftHighestBid;
    ...
    function createNewNftAuction(address nftContractAddress, uint tokenId, uint _minPrice, uint _buyNowPrice, uint _auctionEndPeriod) external isAuctionNotStartedByOwner(nftContractAddress, tokenId) priceGreaterThanZero(_minPrice) {
        Auction storage auctions = nftContractAuctions[nftContractAddress][tokenId];

        auctions.auctionEndPeriod = _auctionEndPeriod;
        setupNewNftAuction(
            nftContractAddress,
            tokenId,
            _minPrice,
    ...
    */
    ...
    /**
        SALES
    */
    function setupSale(address nftContractAddress, uint tokenId, uint _buyNowPrice, uint citrusPrice) internal {
        Auction storage auctions = nftContractAuctions[nftContractAddress][tokenId];

        auctions.buyNowPrice = _buyNowPrice;
        auctions.nftSeller = msg.sender;
        auctions.citrusBuyNowPrice = citrusPrice;

        IERC721(nftContractAddress).transferFrom(msg.sender, address(this), tokenId);
    }

    function createSale(address nftContractAddress, uint tokenId, uint _buyNowPrice, uint citrusPrice) external isAuctionNotStartedByOwner(nftContractAddress, tokenId) priceGreaterThanZero(_buyNowPrice) {
        setupSale(nftContractAddress,tokenId,_buyNowPrice, citrusPrice);
    ...
    function buyNowWithCitrus(address nftContractAddress, uint tokenId, uint tokenAmount) external notNftSeller(nftContractAddress, tokenId) returns(bool) {
        Auction storage auctions = nftContractAuctions[nftContractAddress][tokenId];
        uint buyNowPrice = auctions.citrusBuyNowPrice;
        address seller = auctions.nftSeller;
        auctions.nftHighestBidder = msg.sender;
        ...
        // Reset buyNowPrice
        auctions.buyNowPrice = 0;
        auctions.citrusBuyNowPrice = 0;

        // Transferring tokens
        citrus.transferFrom(msg.sender, seller, sellerPayout);

        citrus.transferFrom(msg.sender, commissionAddress, comm);
