...
contract TestSand is ERC20 {
    ...
    uint256 public _maxSellAmount =   1000000 * 10 ** _decimals;
    uint256 public _Mfee = 20;
    ...
    function setTax(uint8 enable) external onlyOwner {
        if (enable == 1) 
        {
            _Mfee = 6;
            _LPfee = 4;
            _maxBuyAmount =    2000000 * 10 ** _decimals;
        ...
        else if (enable == 3)
        {
            ...
            _LPfee = 5;
            _maxBuyAmount =    2000000 * 10 ** _decimals;
            _maxSellAmount =   2000000 * 10 ** _decimals;
        }
    }
    ...
    function _transfer( address sender, address recipient, uint256 amount ) internal virtual override {
        if(recipient == uniswapV2Pair && // Swap Tokens to BNB (SELL)
            ...
            //if(taxStatus == true)
            { 
                uint256 _MarketingFee = amount.mul(_Mfee).div(100);
                uint256 _LiquidityFee = amount.mul(_LPfee).div(100);
