...
contract GEMUNIStakingGeniPass is GEMUNIStaking {
    ...
    function getPriceNFT (address stakedNFT, uint passId) public override view returns (uint) {
        ...
        if (stakedNFT == geniPass) {
            pricePass = IGENIPassSaleV2(geniPassSale).getPricePass(passId, 0, IGENIPass.PriceType.GENI);
        } else {
    ...
    function getTotalPriceNFT (uint[] memory passIds, uint tokenAmount) public view returns (uint) {
        uint totalAmountNFT;
        ...
        return totalAmountNFT.add(tokenAmount);
    }
}
