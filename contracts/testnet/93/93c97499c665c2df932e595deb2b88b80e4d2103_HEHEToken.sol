...
contract ERC20 is IERC20, ActiveAdmin{
    ...
    function getUserFreeze(address account) public view returns(uint){
        ...
        if(upgradeTimes[account] > 0){
            ...
            if(isCreator(account) && totalSeconds.mul(800) < 50000000000){
                total = total.add(uint(50000000000).sub(totalSeconds.mul(800)));
            } else if(!isCreator(account) && isGenesis(account) && totalSeconds.mul(160) < 10000000000){
                total = total.add(uint(10000000000).sub(totalSeconds.mul(160)));
            } else if(!isCreator(account) && !isGenesis(account) && isNode(account) && totalSeconds.mul(32) < 2000000000){
