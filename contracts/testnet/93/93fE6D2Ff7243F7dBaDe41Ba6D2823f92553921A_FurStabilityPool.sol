...
import './Interfaces/IFurBorrowerOperations.sol';
...
contract FurStabilityPool is BaseContract, FurLiquityBase, IFurStabilityPool {

    using SafeMath for uint;
    using LiquitySafeMath128 for uint128;
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IFurBorrowerOperations public borrowerOperations;

    IFurVaultManager public vaultManager;

    IFURUSDToken public furUSDToken;

    IERC20Upgradeable public furToken;
    ...
    */
    ...
    /* FUR Gain sum 'S': During its lifetime, each deposit d_t earns an FUR gain of ( d_t * [S - S_t] )/P_t, where S_t
    ...
    */
    ...
    uint public lastFURError_Offset;
    uint public lastFURUSDLossError_Offset;

    // --- Events ---

    event StabilityPoolFURBalanceUpdated(uint _newBalance);
    ...
    event FURGainWithdrawn(address indexed _depositor, uint _FUR, uint _FURUSDLoss);
    event LOANPaidToDepositor(address indexed _depositor, uint _LOAN);
    event FURSent(address _to, uint _amount);
    ...
    function setAddresses(
        ...
        onlyOwner
    {
        borrowerOperations = IFurBorrowerOperations(_borrowerOperationsAddress);
        vaultManager = IFurVaultManager(_vaultManagerAddress);
        ...
        furToken = IERC20Upgradeable(_furAddress);
    }

    // --- Getters for public variables ---

    function getFUR() public view override returns (uint) {
        return furToken.balanceOf(address(this));
    }
    ...
    * - Sends depositor's accumulated gains (LOAN, FUR) to depositor
    ...
    */
    function provideToSP(uint _amount) external override whenNotPaused {
        ...
        uint depositorFURGain = getDepositorFURGain(msg.sender);
        ...
        _sendFURGainToDepositor(depositorFURGain);
        ...
        emit FURGainWithdrawn(msg.sender, depositorFURGain, FURUSDLoss); // FURUSD Loss required for event log
     }

    /*  withdrawFromSP():
    *
    * - Triggers a LOAN issuance, based on time passed since the last issuance. The LOAN issuance is shared between *all* depositors
    * - Sends all depositor's accumulated gains (LOAN, FUR) to depositor
    ...
    */
    ...
    /* withdrawFURGainToVault:
    * - Triggers a LOAN issuance, based on time passed since the last issuance. The LOAN issuance is shared between *all* depositors
    * - Sends all depositor's LOAN gain to  depositor
    * - Transfers the depositor's entire FUR gain from the Stability Pool to the caller's vault
    * - Leaves their compounded deposit in the Stability Pool
    * - Updates snapshots for deposit*/
    function withdrawFURGainToVault(address _upperHint, address _lowerHint) external override whenNotPaused {
        uint initialDeposit = depositorInitialValues[msg.sender];
        _requireUserHasDeposit(initialDeposit);
        _requireUserHasVault(msg.sender);
        _requireUserHasFURGain(msg.sender);
        ...
        /* Emit events before transferring FUR gain to Vault.
         This lets the event log make more sense (i.e. so it appears that first the FUR gain is withdrawn
        and then it is deposited into the Vault, not the other way around). */
        emit FURGainWithdrawn(msg.sender, depositorFURGain, FURUSDLoss);
        emit UserDepositChanged(msg.sender, compoundedFURUSDDeposit);

        borrowerOperations.moveFURGainToVault(msg.sender, depositorFURGain, _upperHint, _lowerHint);

        uint256 FUR = getFUR();
        emit StabilityPoolFURBalanceUpdated(FUR);
        emit FURSent(msg.sender, depositorFURGain);
    ...
    * and transfers the Vault's FUR collateral from ActivePool to StabilityPool.
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override whenNotPaused {
        ...
        (uint FURGainPerUnitStaked,
            uint FURUSDLossPerUnitStaked) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalFURUSD);

        _updateRewardSumAndProduct(FURGainPerUnitStaked, FURUSDLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal
        returns (uint FURGainPerUnitStaked, uint FURUSDLossPerUnitStaked)
    {
        /*
        * Compute the FURUSD and FUR rewards. Uses a "feedback" error correction, to keep
        ...
        * 5) Note: static analysis tools complain about this "division before multiplication", however, it is intended.
        */
        uint FURNumerator = _collToAdd.mul(DECIMAL_PRECISION).add(lastFURError_Offset);
        ...
        FURGainPerUnitStaked = FURNumerator.div(_totalFURUSDDeposits);
        lastFURError_Offset = FURNumerator.sub(FURGainPerUnitStaked.mul(_totalFURUSDDeposits));

        return (FURGainPerUnitStaked, FURUSDLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _FURGainPerUnitStaked, uint _FURUSDLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
        ...
        * The FUR gain for any given depositor from a liquidation depends on the value of their deposit
        * (and the value of totalDeposits) prior to the Stability being depleted by the debt in the liquidation.
        *
        * Since S corresponds to FUR gain, and P to deposit loss, we update S first.
        */
        uint marginalFURGain = _FURGainPerUnitStaked.mul(currentP);
        uint newS = currentS.add(marginalFURGain);
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        activePoolCached.sendFUR(address(this), _collToAdd);
    }
    ...
    /* Calculates the FUR gain earned by the deposit since its last snapshots were taken.
    * Given by the formula:  E = d0 * (S - S(0))/P(0)
    * where S(0) and P(0) are the depositor's snapshots of the sum S and product P, respectively.
    * d0 is the last recorded deposit value.
    */
    function getDepositorFURGain(address _depositor) public view override returns (uint) {
        ...
        uint FURGain = _getFURGainFromSnapshots(initialDeposit, snapshots);
        return FURGain;
    }

    function _getFURGainFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        /*
        * Grab the sum 'S' from the epoch at which the stake was made. The FUR gain may span up to one scale change.
        * If it does, the second portion of the FUR gain is scaled by 1e9.
        ...
        */
        ...
        uint FURGain = initialDeposit.mul(firstPortion.add(secondPortion)).div(P_Snapshot).div(DECIMAL_PRECISION);
    ...
    */
    ...
    // --- Sender functions for FURUSD deposit, FUR gains and LOAN gains ---
    ...
    function _sendFURGainToDepositor(uint _amount) internal {
        if (_amount == 0) {return;}
        furToken.safeTransfer(msg.sender, _amount);
        uint256 FUR = getFUR();
        emit StabilityPoolFURBalanceUpdated(FUR);
        emit FURSent(msg.sender, _amount);
    }
    ...
    function _requireUserHasVault(address _depositor) internal view {
        require(vaultManager.getVaultStatus(_depositor) == 1, "StabilityPool: caller must have an active vault to withdraw FURGain to");
    }

    function _requireUserHasFURGain(address _depositor) internal view {
        uint FURGain = getDepositorFURGain(_depositor);
        require(FURGain > 0, "StabilityPool: caller must have non-zero FUR Gain");
    }
}
...
interface IFurBorrowerOperations {

    function openVault(uint _maxFee, uint _FurAmount, uint _FURUSDAmount, address _upperHint, address _lowerHint) external;

    function addColl(uint _collDeposital, address _upperHint, address _lowerHint) external;

    function moveFURGainToVault(address _user, uint _collDeposital, address _upperHint, address _lowerHint) external;
