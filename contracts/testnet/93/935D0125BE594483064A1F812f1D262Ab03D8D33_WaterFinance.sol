...
contract WaterFinance is ERC20Detailed, Ownable, MinterRole {
    ...
    constructor() ERC20Detailed() {
        ...
        DevReceiver = 0xeC6712BE6d4f80aBb375E33C5024e10ff844d99f;
    ...
    function swapBuyBack() internal swapping {
        ...
        IERC20(address(this)).approve(address(router), amountToSwap);
        ...
        uint256 amountBuyTreasury = amountBUSD.mul(BuyTreasuryFee).div(BuyTotalFee);
        uint256 amountBuyMarketing = amountBUSD.mul(BuyMarketingFee).div(BuyTotalFee);
        uint256 amountBuyDev = amountBUSD.mul(BuyDevFee).div(BuyTotalFee);

        BUSD.transfer(TreasuryReceiver, amountBuyTreasury);
        BUSD.transfer(MarketingReceiver, amountBuyMarketing);
        BUSD.transfer(DevReceiver, amountBuyDev);
    ...
    function swapSellBack() internal swapping {
        ...
        uint256 amountSellTreasury = amountBUSD.mul(SellTreasuryFee).div(SellTotalFee);
        uint256 amountSellMarketing = amountBUSD.mul(SellMarketingFee).div(SellTotalFee);
        uint256 amountSellDev = amountBUSD.mul(SellDevFee).div(SellTotalFee);

        BUSD.transfer(TreasuryReceiver, amountSellTreasury);
        BUSD.transfer(MarketingReceiver, amountSellMarketing);
        BUSD.transfer(DevReceiver, amountSellDev);
    }
