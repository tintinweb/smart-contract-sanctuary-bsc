...
interface INFT{
    ...
    function getStarAttributes(uint256 _tokenID) external view returns(starAttributesStruct memory attr);
...
contract MPUpdateCard is Initializable, ERC721,Member {
    ...
    uint256[] public cardPowerPrice;
    uint8 public maxGrade;
    ...
    function initialize(INFT _nft,IERC20 _usdt, IERC20 _mp, IUniswapV2Pair _pair, IPancakeRouter02 _pancakeSwap, IERC20 _wbnb) public initializer {
        __initializeERC721("MetaPirate Upgrade Card", "MetaPirate Upgrade Card");
        __initializeMember();
        init();
    ...
    function init() internal initializer {
        cardPowerPrice = [315,630,1260,2520,5040];
        maxGrade = 5;
    }

    function setPair(IUniswapV2Pair _pair, IPancakeRouter02 _pancakeSwap, IERC20 _wbnb) external {
        require(msg.sender == manager.members("owner"));
        pair = _pair;
        pancakeSwap = _pancakeSwap;
        wbnb = _wbnb;
    ...
    function mintCard(uint256 grade) public {
            ...
            uint256 needPay = cardPowerPrice[grade-2] * (1e36) / (mp_price);

            // check user balance
            uint256 amount = IERC20(mp).balanceOf(address(msg.sender));
            require(amount >= needPay, "blanace not enough");

            IERC20(mp).transferFrom(msg.sender,address(this),needPay);
            distribute(needPay);

            card_Id++;
            cardAttributes[card_Id].grade = grade;
            _mint(msg.sender, card_Id);
            emit Mintcard(msg.sender,grade,card_Id, needPay);
    }

    function updateNFT(uint256 nftId,uint256 cardId) public{
        require(nft.ownerOf(nftId) == msg.sender,"It's not your nft");
        INFT.starAttributesStruct memory attr = nft.getStarAttributes(nftId);
        uint256 nftGrade = attr.power;
    ...
    function distribute(uint256 needpay) internal{
        ...
        IERC20(mp).transfer(address(manager.members("PreacherAddress")),SuperPirateAmount); //   6/105 超級海盗将领池 + 1/105 海盜大將池
    ...
    function tokenURI(uint256 NftId) external view override returns(string memory) {
        return uriPrefix.concat("nft/update/").concat(NftId.toString().concat(".json"));
    }
    ...
    function getPrice() public view returns(uint256){
        ...
        uint256 token_price = usd_balance * (1e18) / (mp_balance);
        return token_price;
    }
