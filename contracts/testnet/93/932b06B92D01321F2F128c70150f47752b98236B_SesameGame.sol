...
contract SesameGame is IProduct, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 toWinner = currentFund - currentFees;
        uint256 toCollect = currentFees;
        totalFundEmitted += toWinner;
        totalFeesEmitted += currentFees;

        lastWinner = _winner;
        uint256 debit = getCredit(toWinner);
        uint256 feeDebit = getCredit(toCollect);
        emit DeclareWinner(_winner, debit, _round, toWinner);
        emit CollectFee(feeDebit, _round, toCollect);
        ...
        if (_native) {
            ...
            rewardToken.deposit{value: toCollect}();
            rewardToken.transfer(collector, toCollect);
        } else {
            gameToken.safeTransfer(_winner, toWinner);
            gameToken.safeTransfer(collector, toCollect);
        }
