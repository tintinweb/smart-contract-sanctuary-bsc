...
contract FarmContract1216V1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function getDeservedRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalRefferalReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            ...
            uint256 pendingReward = availableReffReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedReffReward[_pid][InviteeAddress[i]][_msgSender()]);
            deservedPendingReward = deservedPendingReward.add(pendingReward);
    ...
    function getDeservedNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns (uint256 totalNodeReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            if(getLevel(_pid, InviteeAddress[i]) < getLevel(_pid, _user)) {
                ...
                uint256 pendingReward = availableNodeReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedNodeReward[_pid][InviteeAddress[i]][_msgSender()]);
                deservedPendingReward = deservedPendingReward.add(pendingReward);
    ...
    function claimRefferalReward(uint256 _pid) internal returns (uint256 totalRefferalReward) {
        ...
        for (uint256 i = 0; i < InviteeAddress.length; i++) {
            ...
            deservedPendingReward = deservedPendingReward.mul(getRefferalRewardPercent(_pid, _msgSender())).div(rateDenominator);
            uint256 pendingReward = availableReffReward[_pid][InviteeAddress[i]][_msgSender()].sub(claimedReffReward[_pid][InviteeAddress[i]][_msgSender()]);
            claimedReffReward[_pid][InviteeAddress[i]][_msgSender()] = claimedReffReward[_pid][InviteeAddress[i]][_msgSender()].add(pendingReward);
            deservedPendingReward = deservedPendingReward.add(pendingReward);
            totalRefferalReward = totalRefferalReward.add(deservedPendingReward);
        }
    }
