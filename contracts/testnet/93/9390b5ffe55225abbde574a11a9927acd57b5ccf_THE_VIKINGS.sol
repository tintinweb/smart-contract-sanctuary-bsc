...
abstract contract Context {
 ...
 library SafeMath {
  ...
  contract THE_VIKINGS is ERC721A,Ownable{
    ...
    // 1st sale
    function RoyalVikingsSale(uint256 _count) public payable
    {
        require(msg.value>= mintPrice.mul(_count), "LOW_AMOUNT_ENTERED");
        require(Royal_Sale == true, "MINT_NOT_STARTED");
        require(totalMintedInFirstSale+_count<=firstSaleMintingLimit,"Max Limit Reached");
        require(userMintedNFT[msg.sender]+_count<=2,"User Max Limit Reached");
    ...
    //  2nd sale
    function JarlsVikingsSale(uint256 _count) public payable
    {
        require(msg.value>= mintPrice.mul(_count), "LOW_AMOUNT_ENTERED");
    ...
    //  3rd sale
    function KarlsVikings_Sale(uint256 _count) public payable
    {
        require(msg.value>= mintPrice.mul(_count), "LOW_AMOUNT_ENTERED");
    ...
    function addWhitelist(address[]memory _users) public onlyOwner {
      for(uint256 i; i< _users.length; i++){
          whiteListAddress[_users[i]] = true;
        }
    }
