...
contract ExenoBridgeNode is Ownable, Pausable, ERC1363Payable {
    ...
    // Amount of fees in native currency accumulated in this contract
    ...
    /**
     * When fee is paid in cash, validate the following aspects of a deposit:
        ...
        internal
    {
        ...
        if (_isNative(d.token[0])) {
            ...
            IERC20(d.token[0]).transferFrom(msg.sender, address(this), d.amount[0]);
        }
        ...
        internal
    {
        ...
        if (_isCore(d.token[0])) {
            ...
            require(IERC20(d.token[0]).allowance(sender, address(this)) >= d.amount[0],
                "ExenoBridgeNode: before depositing user must approve amount");
            IERC20(d.token[0]).transferFrom(sender, address(this), d.amount[0]);
        }
    ...
    /**
     * When fee is paid in cash, validate the following aspects of a withdrawal:
        ...
        internal
    {
        if (_isMintable(d.token[1])) {
            ...
            IERC20(d.token[1]).transfer(d.beneficiary[1], d.amount[1]);
