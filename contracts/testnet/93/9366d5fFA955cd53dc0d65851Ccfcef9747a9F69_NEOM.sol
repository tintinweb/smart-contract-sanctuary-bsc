...
contract NEOM  {
    ...
    mapping(address => payAmount) public payAmounts;
    ...
    function updateUser(address userAddress, uint refferID) private {
        ...
        payAmounts[idToAddress[refferID]].user = idToAddress[refferID];
        payAmounts[idToAddress[refferID]].refferAmount =  payAmounts[idToAddress[refferID]].refferAmount.add(refferPrice);
        ...
        if(users[refferID].refferal.length == 2){
    ...
    function updateReferrer(uint refferID) private {
        if (refferID != 1) {
            ...
            if (refferID != freeReferrerID) {
                 ...
                 payAmounts[idToAddress[refferID]].refferAmount =  payAmounts[idToAddress[refferID]].refferAmount.sub(price);
    ...
    function withDraw() external onlyUnlocked() {
        require(payAmounts[msg.sender].user == msg.sender,"Invalid User");
        require(payAmounts[msg.sender].refferAmount > 0 , "Invalid Amount");
        uint amount;
        amount = payAmounts[msg.sender].refferAmount;
        payAmounts[msg.sender].refferAmount = 0;
    ...
    function calculateFee(uint amount)private view returns(uint _adminfee, uint _refferfee){
        _adminfee = amount.mul(adminCommission).div(100e18);
        _refferfee = amount.sub(_adminfee);
    }
