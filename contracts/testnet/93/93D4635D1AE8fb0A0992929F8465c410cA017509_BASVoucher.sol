...
contract BASVoucher is Context, Ownable, ERC721, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;

    IERC20 public busd;
    uint256 public immutable CAP = 5000;
    uint256 public immutable PRICE = 50 * 1e18;   // 50 BUSD
    // sale max per transaction
    uint256 public maxPerTime = 9; // max 9， 1+1+1+1+1+4

    uint256 private _totalSold = 0;
    // address => purchased or not
    mapping(address => bool) public isPurchased;
    address private _validator;
    Counters.Counter private _currentId;
    uint256 public startTimestamp;
    uint256 public salePeriod; // 1days == 86400

    event BuyBASVoucher(address indexed account, uint256 indexed tokenId);

    constructor (address busdAddress, address validator, uint256 _startTimestamp, uint256 _salePeriod) ERC721("Dracoo Master BAS IDO Voucher", "BASVoucher") public {
    ...
    modifier onlyStarted() {
        require(block.timestamp >= startTimestamp, "Sale has not started yet");
        require(block.timestamp <= startTimestamp + salePeriod, "Sale has ended");
        _;
    }
    ...
    function checkRemaining() external view returns (uint256) {
        return CAP.sub(_totalSold);
    }

    function setMaxPerTime(uint256 _maxPerTime) external onlyOwner {
        maxPerTime = _maxPerTime;
    }

    function setBusdAddress(address newBusdAddress) external onlyOwner {
        busd = IERC20(newBusdAddress);
    }

    function setSalePeriod(uint256 newSalePeriod) external onlyOwner {
        salePeriod = newSalePeriod;
    }

    // busd: approve first
    function buyVoucher(uint256 amounts,
                               bytes memory signature) external onlyStarted nonReentrant {
        require(_totalSold.add(amounts) <= CAP, "can not exceed max cap");
        require(!isPurchased[_msgSender()], "you can only purchase ONE time");
        require(amounts <= maxPerTime, "can not excced maxPerTime limit");
        // check signature
        address signer = tryRecover(whitelistHash(_msgSender(), amounts), signature);
        require(signer == _validator, "signature error");

        isPurchased[_msgSender()] = true;
        _totalSold = _totalSold.add(amounts);

        busd.safeTransferFrom(_msgSender(), address(this), amounts.mul(PRICE));

        _mintVouchers(_msgSender(), amounts);
    }

    function whitelistHash(address account, uint256 amounts) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(account, amounts, "BASVoucher"));
    }
    ...
    function _mintVouchers(address to, uint256 amounts) internal  {
        for (uint256 i = 0; i < amounts; ++i) {
            _currentId.increment();
            uint256 tokenId = _currentId.current();
            _safeMint(to, tokenId);

            emit BuyBASVoucher(to, tokenId);
        }
    }
}
