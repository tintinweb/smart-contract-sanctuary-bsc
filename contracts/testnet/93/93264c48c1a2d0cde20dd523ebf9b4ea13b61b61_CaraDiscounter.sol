...
import "./lib/CaraTypes.sol";
import "./lib/CaraSupportAsset.sol";
import "./lib/CaraSupportAssetStaked.sol";

contract CaraDiscounter is CaraSupportAsset, CaraSupportAssetStaked {
    using EnumerableSet for EnumerableSet.AddressSet;

    struct Discount {
        uint8 min; uint16 rates; address product; address asset; bool stakedOnly;
    }

    struct Status {
        uint8 version; address[] products; address[] assets;
    }

    address SANCTUM;
    EnumerableSet.AddressSet private PRODUCTS;
    mapping(address => mapping(address => Discount)) private DISCOUNTS; // {[product]: {[asset]: Discount, ...}, ...}

    constructor(address sanctum) {
    ...
    //===[ onlyRole(OPERATOR) ]===//
    function setProduct(address product, address asset, uint8 min, uint16 rates, bool stakedOnly) external onlyRole(OPERATOR) {
        DISCOUNTS[product][asset] = Discount({
            min: min > 0 ? min: 1, rates: rates, product: product, asset: asset, stakedOnly: stakedOnly
        });
    ...
    //===[ views ]===//
    function productDiscounts() external view returns (Discount[] memory) {
        uint _productsCount = PRODUCTS.length();
        uint _assetsCount = SUPPORTED_ASSETS.length();
        uint _count = _productsCount * _assetsCount;
        Discount[] memory _discounts = new Discount[](_count);

        uint8 _dIdx = 0; uint8 _aIdx; address _product;
        for (uint8 _idx = 0; _idx < _productsCount; _idx++) {
            _product = PRODUCTS.at(_idx);
            for (_aIdx = 0; _aIdx < _assetsCount; _aIdx++) {
                _discounts[_dIdx] = DISCOUNTS[_product][SUPPORTED_ASSETS.at(_aIdx)];
                _dIdx++;
            }
        }
    ...
    function accountDiscounts(address product, address account) external view returns (Discount[] memory) {
        uint _count = SUPPORTED_ASSETS.length();
        address[] memory _foundAssets = new address[](_count);

        uint8 _min; uint8 _idx; uint8 _sIdx; uint8 _found; address _asset;
        uint _inWallet; uint _staked;
        for (_idx = 0; _idx < _count; _idx++) {
            _staked = 0;
            _asset = SUPPORTED_ASSETS.at(_idx);
            _min = DISCOUNTS[product][_asset].min;

            if (DISCOUNTS[product][_asset].stakedOnly) {
                _staked += ICaraStaking(SANCTUM).balanceOf(account, _asset);
                if (_staked >= _min) {
                    _foundAssets[_idx] = _asset;
                    _found++;
                    break;
                }

                for (_sIdx = 0; _sIdx < SUPPORTED_STAKING[_asset].length(); _sIdx++) {
                    _staked += ICaraStaking(SUPPORTED_STAKING[_asset].at(_sIdx)).balanceOf(account, _asset);
                    if (_staked >= _min) {
                        _foundAssets[_idx] = _asset;
                        _found++;
                        break;
                ...
                _inWallet = ICaraNFT(_asset).balanceOf(account);
                if (_inWallet >= _min) {
                    _foundAssets[_idx] = _asset;
                    _found++;
                } else {
                    _staked += ICaraStaking(SANCTUM).balanceOf(account, _asset);
                    if ((_inWallet + _staked) >= _min) {
                        _foundAssets[_idx] = _asset;
                        _found++;
                        break;
                    }
        ...
        Discount[] memory _discounts = new Discount[](_found);
        uint8 _dIdx = 0;
        for (_idx = 0; _idx < _count; _idx++) {
            if (_foundAssets[_idx] != address(0)) {
                _discounts[_dIdx] = DISCOUNTS[product][_foundAssets[_idx]];
                _dIdx++;
            }
        }
    ...
    function discountRates(address product, address account) external view returns (uint16) {
        uint8 _min; uint8 _sIdx; uint16 _finalRates; uint16 _rates; address _asset;
        uint _inWallet; uint _staked;
        for (uint8 _idx = 0; _idx < SUPPORTED_ASSETS.length(); _idx++) {
            _staked = 0;
            _asset = SUPPORTED_ASSETS.at(_idx);
            _min = DISCOUNTS[product][_asset].min;
            _rates = DISCOUNTS[product][_asset].rates;
        ...
        return _finalRates >= 1e4 ? 1e4 : _finalRates;
    }

    function status() public view returns (Status memory) {
        return Status({version: 1, products: PRODUCTS.values(), assets: CaraSupportAsset.supportedAssets()});
    }
}
...
contract CaraSupportAssetStaked is Ownable, AccessControlEnumerable {
    using EnumerableSet for EnumerableSet.AddressSet;

    mapping(address => EnumerableSet.AddressSet) SUPPORTED_STAKING;

    //===[ onlyRole(OPERATOR) ]===//
    function addStaking(address asset, address staking) external onlyRole(OPERATOR) {
        SUPPORTED_STAKING[asset].add(staking);
    }

    function removeStaking(address asset, address staking) external onlyRole(OPERATOR) {
        SUPPORTED_STAKING[asset].remove(staking);
    }

    //===[ checks ]===//
    function stakingSupported(address asset, address staking) public view returns (bool) {
        return SUPPORTED_STAKING[asset].contains(staking);
    }

    //===[ views ]===//
    function supportedStaking(address asset) public view returns (address[] memory) {
        return SUPPORTED_STAKING[asset].values();
    }
}
