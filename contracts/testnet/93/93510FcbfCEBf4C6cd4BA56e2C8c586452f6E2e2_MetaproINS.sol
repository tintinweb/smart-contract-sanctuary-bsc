...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    // @dev: Ins token contract address
    ...
    // @dev: Ins token contract address
    mapping(uint256 => address) private insContractAddress;

    // @dev: insId => Referral.ReferralFees
    ...
    // @dev: dictionary with insIds - wallet address => insIds
    ...
    function create(
        ...
        // Check if providd tokenId is valid
        require(
            _tokenId > 0,
        ...
        insContractAddress[currentInsId] = _tokenContractAddress;
    ...
    function saveInsRoyaltyFees(uint256 _insId, uint256 _tokenId) private {
        Royalty.RoyaltyTeamMember[] memory teamMembers = metaproRoyalty
            .getTeamMembers(_tokenId, insContractAddress[_insId]);
    ...
    function deposit(
        ...
        require(
            configuration.valid,
            "INS: tokenId configuration not found or ins is ended"
        );
        ...
        if (configuration.maxCapInBusd != 0) {
            ...
            require(
                ...
                "INS: amount exceeds available cap"
            );
        }
        ...
        require(block.number <= configuration.endBlock, "INS: ins is ended");
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        require(
            insConfiguration.insId != 0,
            "INS: insId configuration not found"
        );

        require(!alreadyCompletedIns[insId], "INS: ins is already completed");

        require(
            block.number >= insConfiguration.endBlock ||
                insConfiguration.maxCap == insConfiguration.currentCap ||
                block.number < insConfiguration.startBlock,
            "INS: Ins is still in progress or max cap is not reached"
        );
        ...
        bool isInsOperator = msg.sender == insConfiguration.operator;

        if (insConfiguration.minCap != 0) {
            // Ins is not completed - deposited tokens will be returned to all depositers
            if (insConfiguration.currentCap < insConfiguration.minCap) {
                if (isInsOperator)
                    processOperatorGiveBack(insConfiguration, _data);
                else
                    processDepositorGiveBack(insConfiguration, deposits[insId]);
        ...
        finalizeInsByWallet(insId, msg.sender);

        finalizeTokenId(
            insConfiguration.tokenId,
            insContractAddress[insId],
            insId
        );

        if (
            insTokenQuantity[insConfiguration.insId] == 0 &&
            insBalance[insId] == 0
        ) {
    ...
    function emergencyInsWithdraw(uint256 _insId, bytes memory _data)
        ...
        onlyOwner
    {
        INSConfiguration storage insConfiguration = availableIns[_insId];
        ...
        if (busdBalance > 0) busd.transfer(msg.sender, busdBalance);
        if (tokenBalance > 0)
            IERC1155(insContractAddress[_insId]).safeTransferFrom(
                address(this),
                msg.sender,
                insConfiguration.tokenId,
                tokenBalance,
    ...
    function addInsToWallet(uint256 _insId, address _walletAddress) private {
        WalletIns memory newWalletIns = WalletIns({
            insId: _insId,
            finalized: false
        });
        bool insAdded = false;
        ...
        if (!insAdded) walletIns[_walletAddress].push(newWalletIns);
    }
    ...
    function getInsTokenContractAddress(uint256 _insId)
        ...
        returns (address)
    {
        return insContractAddress[_insId];
    }
    ...
    function processOperatorGiveBack(
        ...
        IERC1155(insContractAddress[_insConfiguration.insId]).safeTransferFrom(
            address(this),
    ...
    function processOperatorWithdraw(
        ...
        for (uint256 i = 0; i < _insDeposits.length; i++) {
            ...
            if (singleWalletDeposit.amount != 0) {
        ...
        if (operatorTokens != 0) {
            IERC1155(insContractAddress[_insConfiguration.insId])
                .safeTransferFrom(
                    address(this),
                    _insConfiguration.operator,
                    _insConfiguration.tokenId,
    ...
    function processDepositorGiveBack(
        INSConfiguration memory _insConfiguration,
        INSDeposit[] memory _insDeposits
    ) private {
        require(
