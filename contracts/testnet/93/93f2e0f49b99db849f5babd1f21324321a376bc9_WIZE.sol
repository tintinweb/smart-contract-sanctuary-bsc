...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("WIZE_Busd_Dividend_Tracker", "WIZE_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "WIZE_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "WIZE_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
...
contract WIZE is ERC20, Ownable {
    using SafeMath for uint256;
 
    IUniswapV2Router02 public uniswapV2Router;
    address private immutable uniswapV2Pair;
 
    address private busdDividendToken;
    ...
    bool private busdDividendEnabled = true;
    ...
    uint256 private maxBuyTranscationAmount = 5000000000 * (10**18);
    uint256 private maxSellTransactionAmount = 5000000000 * (10**18);
    uint256 private maxWalletBalance = 5000000000 * (10**18);
    uint256 private swapTokensAtAmount = 20 * 10**6 * 10**18;
    ...
    uint256 public _busdDividendRewardsFeeBuy = 3;
    uint256 public _marketingFeeBuy = 3;
    uint256 public _LpFeeBuy = 2;
    
    uint256 public _busdDividendRewardsFeeSell=3;
    uint256 public _LpFeeSell=2;
    uint256 public _marketingFeeSell=3;  
    
 
    uint256 private busdDividedRewardsInContract;
    uint256 private tokensForLpInContract;

    uint256 private sellFeeIncreaseFactor = 130;
 
    uint256 private gasForProcessing = 600000;
 
    mapping (address => bool) private isExcludedFromFees;
     mapping (address => bool) private isIncludedinFees;
    ...
    event IncludedinFees (address indexed account, bool isIncluded) ;
    ...
    constructor() ERC20("WIZEFROG", "WIZE") {
        	
    	marketingWallet = 0xb4773795580666EA67ED8a230989e04e5f2587AE; 
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "WIZE: The marketing wallet is already this address");
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "WIZE: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "WIZE: The new dividend tracker must be owned by the Boda token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "WIZE: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "WIZE: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WIZE: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "WIZE: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "WIZE: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "WIZE: Trading has not started yet");
    ...
    function transferToWallet(address payable recipient, uint256 amount) private {
     ...
     function setBuyTaxes(uint256 newLpFeeBuy, uint256 newmarketingFeeBuy, uint256 newbusdDividendRewardsFeeBuy ) external onlyOwner() {
        _LpFeeBuy = newLpFeeBuy;
        _marketingFeeBuy = newmarketingFeeBuy;
        _busdDividendRewardsFeeBuy = newbusdDividendRewardsFeeBuy;
       

    }

    function setSellTaxes(uint256 newLpFeeSell, uint256 newmarketingFeeSell, uint256 newbusdDividendRewardsFeeSell) external onlyOwner() {
        _LpFeeSell = newLpFeeSell;
        _marketingFeeSell = newmarketingFeeSell;
        _busdDividendRewardsFeeSell = newbusdDividendRewardsFeeSell;
    }
