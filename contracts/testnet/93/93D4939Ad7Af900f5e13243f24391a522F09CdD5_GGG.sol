...
contract GGG is Context, IBEP20, Ownable {
    ...
    uint256 private _totalSupply = 100 * 1e6 * 1e18;
    ...
    uint256 public maxTxAmount = _totalSupply.mul(1).div(100); // should be 0.2% percent per transaction
    uint256 public maxHoldingAmount = _totalSupply.mul(2).div(100); // maximum Holding limit is 1% percent of total supply
    ...
    uint256 public devFeeOnBuying = 40; // 4% will be added to the development address
    uint256 public liquidityFeeOnBuying = 40; // % will be added to the liquidity

    uint256 public marketFeeOnSelling = 40; // 4% will be added to the market address
    uint256 public devFeeOnSelling = 40; // 4% will be added to the development address
    ...
    constructor( 
        ...
        // Create a pancake pair for this new GGG
        ...
        _isExcludedFromMaxHoldLimit[deadAddress] = true;
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "GGG: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "GGG: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function startTrading() external onlyOwner {
        require(!_tradingOpen, "GGG: Already enabled");
        _tradingOpen = true;
    ...
    function addSniperInList(address _account) external onlyOwner {
        require(
            ...
            "GGG: We can not blacklist dexRouter"
        );
        require(!_isSniper[_account], "GGG: sniper already exist");
        _isSniper[_account] = true;
    }

    function removeSniperFromList(address _account) external onlyOwner {
        require(_isSniper[_account], "GGG: Not a sniper");
    ...
    function totalBuyFeePerTx(uint256 amount) public view returns (uint256) {
        uint256 fee = amount
            .mul(
                devFeeOnBuying.add(marketFeeOnBuying).add(
    ...
    function _approve(
        ...
        require(owner != address(0), "GGG: approve from the zero address");
        require(spender != address(0), "GGG: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "GGG: transfer from the zero address");
        require(to != address(0), "GGG: transfer to the zero address");
        require(amount > 0, "GGG: Amount must be greater than zero");
        require(!_isSniper[to], "GGG: Sniper detected");
        require(!_isSniper[from], "GGG: Sniper detected");

        if (
            _isExcludedFromMaxTx[from] == false &&
            _isExcludedFromMaxTx[to] == false // by default false
        ) {
            require(amount <= maxTxAmount, "GGG: amount exceeded max limit");

            if (!_tradingOpen) {
                require(
                    from != dexPair && to != dexPair,
                    "GGG: Trading is not enabled yet"
                );
            }
    ...
    function _tokenTransfer(
        ...
        if (sender == dexPair && takeFee) {
            ...
            _takeFeeOnBuying(sender, amount);
            ...
            _takeFeeOnSelling(sender, amount);
        } else {
            _balances[sender] = _balances[sender].sub(amount);
    ...
    function _takeFeeOnBuying(address sender, uint256 amount) internal {
    ...
    function _takeFeeOnSelling(address sender, uint256 amount) internal {
