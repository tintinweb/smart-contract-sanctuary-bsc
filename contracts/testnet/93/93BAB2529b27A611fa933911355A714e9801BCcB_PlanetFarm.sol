...
interface IStrategy {
    ...
    // Autocompound farm reward by supplying them to green planet
    function earnGammaProfitsiGamma() external;
...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    struct UserInfo {
        ...
        // gamma deposit and withdraw tally
        uint256 deposit; 
        uint256 withdraw;
    ...
    function updatePool(uint256 _pid) public{
        ...
        if (pool.iToken == iGammaAddress){
            ...
            IERC20(GAMMA).transfer(pool.strat, GAMMAReward);
            IStrategy(pool.strat).earnGammaProfitsiGamma();
    ...
    // function stakeGammaOnInfinityvault(uint256 _gammaAmount) internal {
    ...
    function depositAuthorized(address _user,uint256 _wantAmt, bool takeWantFromMsgsender) internal  {
        ...
        if (_wantAmt != 0) {
            ...
            uint256 mintAmount = IStrategy(pool.strat).deposit(_wantAmt, false);
            ...
            user.deposit += _wantAmt;
            user.lifetimeRewards += _wantAmt; 
    ...
    function claimAllPoolsPendingGamma() external nonReentrant {
        ...
        for(uint256 i = 0; i < length; ++i) {
            ...
            if(pool.isInfinity){
                user.rewardDebt = (IERC20(pool.iToken).balanceOf(_msgSender()) - user.iTokenToBeUnstaked)*pool.accGAMMAPerShare;
            }
            else{
                user.rewardDebt = (user.shares*pool.accGAMMAPerShare + user.factor*pool.accGAMMMAPerFactorPerShare)/1e12;
            }
        ...
        if(totalPending != 0) {
            if(autoStakeGamma)
                ...
                depositAuthorized(_msgSender(),totalPending, false);
            else
    ...
    function unstakeInstantly(uint256 _pid, uint256 unstakeAmount ) external notContract{
        ...
        if(pool.iToken == iGammaAddress ) {
            ...
            user.withdraw += (stratWantTokensRemoved*gToken_exchange_rate)/1e18; 
        }
    ...
    function unstakeAfterMinWithdrawTime(uint256 _pid) external notContract{        
        ...
        if (pool.iToken == iGammaAddress){
            IERC20(GAMMA).transfer(pool.strat, lostRewards);
            IStrategy(pool.strat).earnGammaProfitsiGamma();
        }
        else{
    ...
    */
    ...
    function _notContract() internal view {
        ...
        require(_msgSender() == tx.origin, "proxy");
    }
