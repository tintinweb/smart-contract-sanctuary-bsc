...
contract GREENTRANSMIT is Context, Ownable {
    ...
    address _sender = address(0xB5cB1568E7B8Dc5e8AaEba6BDD712DdD8dde5E0E);//test 钱包地址
    ...
    constructor() {
        ...
        // _transmitTime.push(1663902620);
        // _transmits[0].push(transmitInfo(true,0,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmits[0].push(transmitInfo(true,1,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmits[0].push(transmitInfo(true,2,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmits[1].push(transmitInfo(true,0,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmits[1].push(transmitInfo(true,0,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmits[1].push(transmitInfo(true,1,address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D),1663902620));
        // _transmitCount+=6;
    }
    ...
    function countdownTime() public view returns(uint res){
        if(_transmitTime.length>0){
            for(uint i;i<_transmitTime.length;i++){
                if(res==0){
                    res=_transmitTime[i]+_params.completeTime;
                }else{
                    if((_transmitTime[i]+_params.completeTime)-res>=(_params.completeTime-_params.alarmTime+_params.insertTime)){
                        res+=_params.insertTime;
                    }
                }
            }
        }else{
            res=block.timestamp+_params.completeTime;
        }
    }
    //爆仓
    function complete() external onlyOwner{
        require(_transmitCount>0&&(transmitLastTime()+_params.alarmTime)<block.timestamp,"error");
        //爆仓
        uint buyLevel=getBuyLevel(false);//当前级别
        uint floor=getFloor(false);//当前层数
