...
contract LendingPool is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint16 public interestPercent; //国库利息分配比例
    uint16[] public interestAllocRates;
    address[] public interestAllocAddrs;
    EnumerableSetUpgradeable.AddressSet collateralTokens; //支持的抵押token
    ...
    struct LoanData {
        uint256 pid; //借贷池
        uint256 loanAmount; //借贷金额 musd
        uint256 collateralAmount; //抵押金额 mai
        uint256 dueTime; //到期时间
        uint8 status; //0:借贷中, 1:已还款, 2:被清算
        address collateralToken; //抵押token
        address borrower; //借款人
    }
    mapping(uint256 => LoanData) public loans;
    mapping(address => EnumerableSetUpgradeable.UintSet) loanIds;
    mapping(address => uint16) public tokenCollateralRates; //token抵押率
    mapping(address => uint16) public tokenLiquidateRates; //token清算比例
    mapping(address => mapping(address => uint16)) addrCollateralRates; //address抵押率
    mapping(address => uint256) public liquidatedAmounts; //被清算的token数量
    ...
    function initialize() public initializer {
        ...
        treasury = 0x0c1F886792e3E8cc5a4b3532b0c75ab572c9B31E;
        priceOracle = 0x29AcbB7Be64e572EA38B7e7C1BE8726E19476434;
    ...
    //清算 条件:违约金+(贷款金额*抵押率)>抵押token价值*(清算比例)
    function liquidate(uint256 loanId) external onlyRole(LIQUIDATOR_ROLE) {
        ...
        //有权限控制，且支持mai以外的抵押token，可以不用限制时间
        //require(block.timestamp > dueTime, "liquidate time limited");
