...
function storeSeedPhrase(uint256[] memory tokenIds, string memory seedPhrase) public {
    ...
    bytes memory encryptedSeedPhrase = bytes(seedPhrase);
...
function decryptSeedPhrase(uint256[] memory tokenIds, bytes memory signature) public returns (string memory) {
    require(tokenIds.length >= 3 && tokenIds.length <= 15, "You must specify between 3 and 15 token IDs");
    require(approvedWalletsIndices[msg.sender] > 0, "You are not an approved wallet");
