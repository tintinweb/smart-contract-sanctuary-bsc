...
abstract contract WLBModel is IERC20, Ownable, Context {
    ...
    mapping(address => bool) public mapBigDaoArr;
    //小道主 用户
    address[] private smallDaoArr;
    mapping(address => bool) public mapSmallDaoArr;

    //万分之 持币者
    address[] private tenThousandArr;
    mapping(address => bool) public mapTenThousandArr;
    ...
    //wlb 兑U 汇率
    uint256 private wlbRate = 150000;
    ...
    //设置wlb对U的价格
    function setWlbRate(uint256 wlbRate_) public returns (bool) {
        wlbRate = wlbRate_;
        return true;
    }

    function setLpFee(uint256 fee) public returns (bool) {
        lpFee = fee;
        return true;
    }

    function setBigDaoFee(uint256 fee) public returns (bool) {
        bigDaoFee = fee;
        return true;
    }

    function setSmallDaoFee(uint256 fee) public returns (bool) {
        smallDaoFee = fee;
        return true;
    }

    function setTenThousandFee(uint256 fee) public returns (bool) {
        tenThousand = fee;
        return true;
    }

    function setTenThousandDynamicFee(uint256 fee) public returns (bool) {
        tenThousandDynamic = fee;
        return true;
    }

    function setPlatformFee(uint256 fee) public returns (bool) {
        platformFee = fee;
        return true;
    }

    function setTenThousandDynamicAddr(address addr) public returns (bool) {
        tenThousandDynamicAddr = addr;
        return true;
    }

    function setLpAddr(address addr) public returns (bool) {
        lpFeeAddr = addr;
        return true;
    }

    function setPlatformFeeAddr(address addr) public returns (bool) {
        platformFeeAddr = addr;
        return true;
    }


    function deleteBigDao(address big) public returns (bool)  {
    ...
    function deleteSmallDao(address small) public returns (bool)  {
    ...
    function deleteTenThousand(address addr) public returns (bool)  {
    ...
    function addBigDao(address addr) public returns (bool)  {
        bigDaoArr.push(addr);
        mapBigDaoArr[addr]=true;
        return true;
    }

    function addSmallDao(address addr) public returns (bool)  {
        smallDaoArr.push(addr);
        mapSmallDaoArr[addr] = true;
        return true;
    }

    function addTenThousand(address addr) public returns (bool)  {
        tenThousandArr.push(addr);
        mapTenThousandArr[addr] = true;
        return true;
    }

    function setBigDao(address[] memory addr) public returns (bool)  {
        for (uint i = 0; i < bigDaoArr.length; i++) {
    ...
    function setSmallDao(address[] memory addr) public returns (bool)  {
    ...
    function setTenThousand(address[] memory addr) public returns (bool)  {
        for (uint i = 0; i < tenThousandArr.length; i++) {
    ...
    // 分润    产生分润的用户       分润金额
    function dividend(address sender, uint256 amount) public {
        if (amount == 0) return;
    ...
    function withdrawUSDT(uint256 amount) onlyOwner public returns (bool){
        if (amount == 0) {
            amount = IERC20(usdtAddress).balanceOf(msg.sender);
        }
        return IERC20(usdtAddress).transfer(msg.sender, amount);
    }
    ...
    function _dividendBigDao(address sender, uint256 amount) private {
        ...
        //平均没人分红
        uint256 div = amount.div(wlbRate).div(bigDaoArr.length);
        for (uint i = 0; i < bigDaoArr.length; i++) {
    ...
    function _dividendTenThousand(address sender, uint256 amount) private {
        ...
        //平均没人分红
        uint256 div = amount.div(wlbRate).div(tenThousandArr.length);
        for (uint i = 0; i < tenThousandArr.length; i++) {
