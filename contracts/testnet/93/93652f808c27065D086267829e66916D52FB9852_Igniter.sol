...
contract Igniter is IERC20, IERC20Metadata, Context, Ownable, ReentrancyGuard {
    ...
    string private _NAME = "Incinerator Factory";
    string private _SYMBOL = "LFLF";
    ...
    bool burnSwapState = false;

    struct KEY_ADDRESSES {
        address burnerContract;
    }

    KEY_ADDRESSES contractAddresses;
    ...
    constructor(
        ...
        contractAddresses = KEY_ADDRESSES({burnerContract: _contractToIgnite});
    ...
    function totalSupply() external view override returns (uint256) {
        return IERC20(contractToIgnite).totalSupply();
    }

    function balanceOf(address account) public view override returns (uint256) {
        return IERC20(contractToIgnite).balanceOf(account);
    }
    ...
    modifier onlyAuthorized() {
        parent = Incinerator(contractToIgnite);

        require(
            parent.authorized(msg.sender) ||
                contractOwner == msg.sender ||
                contractToIgnite == msg.sender,
            "Not Authorized"
        );
        _;
    }
    ...
    modifier isWhitelisted() {
        ...
        require(
            parent.getWhitelisted(msg.sender) ||
                contractOwner == msg.sender ||
                contractToIgnite == msg.sender,
    ...
    modifier isNotBlacklisted() {
        ...
        require(parent.getBlacklisted(msg.sender) == false, "Blacklisted");
        _;
    }
    ...
    function lp_EtherPerLPToken(
        ...
        _tokensPerLPToken =
            ((IWETH(pairedToken).balanceOf(address(checkpair)) *
                (IIncinerator(sourceContract).defaultMultiplier() *
    ...
    function lp_TotalTokensInLPOwnedByProject(
        ...
        uint256 defaultMultiplier = IIncinerator(sourceContract)
            .defaultMultiplier();
        ...
        uint256 tokenbalanceOfPair = IIncinerator(sourceContract).balanceOf(checkpair);
        tokensinlpControlledByProject = (tokenbalanceOfPair * percentControlled) / (1 ether);
    }
        ...
        onlyOwner
    {
        ...
        contractAddresses.burnerContract = _burnerAddress;
        contractToIgnite = _burnerAddress;
        contractOwner = _burnerAddress;
        _transferOwnership(_burnerAddress);
    }

    function burnSwap(address swapAddress, uint256 value)
        external
        nonReentrant
    {
        ...
        swapToken.safeApprove(address(dexRouter), type(uint256).max);
            ...
            )
        {
            ...
            revert("Something went wront swapping your tokens");
        }
    }

    function checkBurnSwapClaimValue() external view returns (uint256 balance) {
        require(burnSwapState, "Burn swap is currently not available");
        balance = claimBalance[msg.sender];
    }
    ...
    function claimBurnSwap(address to)
        external
        nonReentrant
    {
        require(burnSwapState, "Burn swap is currently not available");
        ...
        path[1] = contractAddresses.burnerContract;

        uint[] memory amountOut = dexRouter.getAmountsOut(claimable, path);
        uint256 truAmountOut = amountOut[amountOut.length - 1];
        uint256 amountUpdated = ((truAmountOut * 9)/100) + truAmountOut;        
        
        IIncinerator _incinerator = IIncinerator(contractAddresses.burnerContract);

        _incinerator.removeAllFees();
        try
            _incinerator.transfer(to, amountUpdated)
        {
            _incinerator.restoreAllFees();
        } catch {
           _incinerator.restoreAllFees();
            revert("Burn Swap transfer failed");
        }
    }
    ...
    {
        require(0 < amount, "Zero Tokens");
        require(IERC20(contractAddresses.burnerContract).balanceOf(address(this)) >= amount, "Not enough tokens to send");
        require(
            IERC20(contractAddresses.burnerContract).transfer(destination, amount),
            "transfer failed"
        );
    }
    ...
    ReentrancyGuard
{
    ...
    struct FEES {
        ...
        uint256 xfrFee;
        uint256 tokenBurn;
        uint256 burnTokenOnXfr;
        uint256 xfrBurnLP;
        uint256 xfrBurnOverride;
        uint256 burnBypassFee;
        uint256 userSellBurn;
        uint256 userBuyFee;
        uint256 buyBurnFee;
        uint256 buyBurnLP;
        // Trackoriginal fees to bypass fees for charity account
    }
    ...
    mapping(burnType => uint256) public burnAmounts;
    mapping(uint256 => DAILY_BURN) public dailyBurn;

    string private _NAME = "Lunar Flare Labs";
    string private _SYMBOL = "LFLR";
    ...
    uint256 private pendingTokensToBurnFromLP = 0;
    uint256 private queuedDailyBurn = 0;
    uint256 private timeSinceLastPair = 0;
    uint256 public defaultMultiplier = 1000000000000000;

    uint256 houseBalance = 0;
    uint256 houseEthBalance = 0;
    ...
    address contractOwner;
    address peggedPair;
    address payable igniterSupport;

    bool InitialLiquidityRan = false;
    bool stateOfLPInteraction = true;
    bool globalFeeState = false;
    ...
    constructor(
        ...
        address _payloadWallet,
        address _burnWallet,
        address _RouterAddress,
        address payable _igniterSupport,
        address[] memory _tokensToPegTo
    ) payable {
        igniterSupport = _igniterSupport;
        ...
        contractAddresses = KEY_ADDRESSES({
            ...
            payloadWallet: _payloadWallet,
            burnWallet: _burnWallet,
            igniterContract: _igniterSupport
        });

        _projectWhitelist[contractOwner] = true;
        _projectWhitelist[_payloadWallet] = true;
        _projectWhitelist[contractAddresses.marketingWallet] = true;
        _projectWhitelist[contractAddresses.burnWallet] = true;
        _projectBlacklist[address(0)] = false;
        _projectBlacklist[DEAD] = false;

        balances[address(0)] = 0;
        balances[address(this)] = (_TotalSupply * 70) / 100;
        balances[contractOwner] = (_TotalSupply * 15) / 100;
        balances[_igniterSupport] = (_TotalSupply * 15) / 100;
        ...
        emit Transfer(
            address(0),
            address(_igniterSupport),
            balances[address(_igniterSupport)]
        );

        setDexRouter(_RouterAddress);

        for (uint256 index = 0; index < _tokensToPegTo.length; index++) {
            address thisAddress = _tokensToPegTo[index];
            createPair(thisAddress, false);
        }
    ...
    {
        _tokenFee = ((tokenAmount * tokenFee) / _GRANULARITY) / (100);
        _tokenBurn = ((tokenAmount * tokenBurn) / _GRANULARITY) / (100);
        _secondTokenBurnTally =
            ((tokenAmount * tokenAdditionalLPBurn) / _GRANULARITY) /
            (100);
    }

    function _getTokenTransferAmount(
        uint256 tokenAmount,
        uint256 _baseFee,
        uint256 _tokenBurn
    ) private pure returns (uint256) {
        return tokenAmount - _baseFee - _tokenBurn;
    }
        ...
        )
    {
        ...
        if (_getTokensToRemove > 0) {
            ...
            uint256 percent = ((_getTokensToRemove *
                (defaultMultiplier * _DECIMALFACTOR)) /
                LPContractTokenBalance) / (defaultMultiplier);

            uint256 lpAmount = IERC20(primePair).balanceOf(address(this));

            lpToUnpair = (lpAmount * percent) / (100 * _DECIMALFACTOR);
            ethToExtract =
                (uint256(LPWethBalance) * percent) /
                (100 * _DECIMALFACTOR);
    ...
    function pendingBurn() external view returns (uint256) {
        return pendingTokensToBurnFromLP;
    }
    ...
    function lp_contractTokensPerLPToken(address checkpair)
        external
        view
        returns (uint256 _tokensPerLPToken)
    {
        uint256 lpTokenSupply = IERC20(checkpair).totalSupply();

        _tokensPerLPToken = balances[checkpair] / lpTokenSupply;
    }
    ...
    function _actualTransfer(
        ...
        if (_transferAmount > 0) {
            unchecked {
                balances[sender] = balances[sender] - (_transferAmount);
            }

            unchecked {
                balances[receiver] = balances[receiver] + (_transferAmount);
            }
    ...
    function _burnToken(
        burnType _burnType,
        address addressBurning,
        uint256 amountToBurn
    ) private returns (bool) {
        balances[addressBurning] = balances[addressBurning] - amountToBurn;
        burnAmounts[_burnType] = burnAmounts[_burnType] + amountToBurn;
        balances[address(0)] = balances[address(0)] + amountToBurn;

        totalTokensBurned = totalTokensBurned + amountToBurn;
    ...
    function _controlDailyBurn() private {
        ...
        if (dailyBurn[_dayCounter.current()].startTimestamp != _timestamp) {
            ...
            dailyBurn[_dayCounter.current()] = DAILY_BURN({
                ...
                startingBalance: startingTokensInLP,
                amountTobeBurned: startingTokensInLP / 100,
                amountBurned: 0
            });
        }

        if (dailyBurns() > 0) {
            queuedDailyBurn = dailyBurns();
        }
    }

    function dailyBurns() public view returns (uint256 _tokensToBurn) {
        uint256 tickSinceStart = block.timestamp -
            dailyBurn[_dayCounter.current()].startTimestamp;

        // console.log("tickSinceStart", tickSinceStart);

        uint256 totalTicks = dailyBurn[_dayCounter.current()].endTimestamp -
            dailyBurn[_dayCounter.current()].startTimestamp;

        // console.log("totalTicks", totalTicks);
        // console.log("amountTobeBurned", dailyBurn[_dayCounter.current()].amountTobeBurned);
        uint256 _toBeBurnedPerTick = dailyBurn[_dayCounter.current()]
            .amountTobeBurned / totalTicks;

        // console.log("_toBeBurnedPerTick", _toBeBurnedPerTick);
    ...
    {
        _queuedDailyBurn =
            dailyBurn[_dayCounter.current()].startingBalance -
            dailyBurn[_dayCounter.current()].amountBurned;
    }

    function removeAllFees() public onlyAuthorized {
        contractFees.baseTxnFee = 0;
        contractFees.xfrFee = 0;
        contractFees.tokenBurn = 0;
        contractFees.burnTokenOnXfr = 0;
        contractFees.xfrBurnLP = 0;
        contractFees.xfrBurnOverride = 0;
        contractFees.burnBypassFee = 0;
        contractFees.userSellBurn = 0;
        contractFees.userBuyFee = 0;
        contractFees.buyBurnFee = 0;
        contractFees.buyBurnLP = 0;
    }

    function _removeLiquidityETHForRequestedTokens(
        uint256 getTokensToRemove,
        uint256 _dailyBurnCount
    )
        internal
        returns (
            bool,
    ...
    {
        (uint256 tokensToExtract, , uint256 lpToUnpair) = calculateLPtoUnpair(
            getTokensToRemove + _dailyBurnCount
        );

        if (tokensToExtract > 0 && lpToUnpair >= (defaultMultiplier)) {
            uint256 priorBalance = balances[address(this)];

            uint256 amountToken = 0;
            uint256 priorEthBalance = address(this).balance;
            uint256 txETHAmount = 0;

            globalFeeState = false;

            _transferOwnership(msg.sender);
            approveContract(primePair, address(dexRouter), lpToUnpair);
            ...
            {
                txETHAmount = address(this).balance - priorEthBalance;
                amountToken = balances[address(this)] - priorBalance;
                getHouseFees();
            ...
            return (true, amountToken, txETHAmount);
        }

        return (false, 0, 0);
    }

    function getHouseFees() private onlyOwner {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = dexRouter.WETH();

        // uint256 _houseBalance = houseBalance;
        if (
            houseBalance > 0 &&
            balances[address(this)] >= houseBalance &&
            ...
            try
                dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    houseBalance, // accept as many tokens as we can
                    0,
                    path,
            ...
            {
                console.log("House Sent Eth");
                houseBalance = 0;
                houseEthBalance = address(this).balance - priorHouseEthBalance;
            } catch {
                console.log("House Sent Eth Failed");
                _actualTransfer(
                    address(this),
                    contractAddresses.marketingWallet,
                    houseBalance / 2
                );
                _actualTransfer(
                    address(this),
                    contractAddresses.payloadWallet,
                    houseBalance / 2
                );
            }
        }
    ...
    function restoreAllFees() public onlyAuthorized {
        contractFees.baseTxnFee = uint256(ogFees.baseTxnFee);
        contractFees.xfrFee = uint256(ogFees.xfrFee);
        contractFees.tokenBurn = uint256(ogFees.tokenBurn);
        contractFees.burnTokenOnXfr = uint256(ogFees.burnTokenOnXfr);
        contractFees.xfrBurnLP = uint256(ogFees.xfrBurnLP);
        contractFees.xfrBurnOverride = uint256(ogFees.xfrBurnOverride);
        contractFees.burnBypassFee = uint256(ogFees.burnBypassFee);
        contractFees.userSellBurn = uint256(ogFees.userSellBurn);
        contractFees.userBuyFee = uint256(ogFees.userBuyFee);
        contractFees.buyBurnFee = uint256(ogFees.buyBurnFee);
        contractFees.buyBurnLP = uint256(ogFees.buyBurnLP);
    }
        ...
        isNotBlacklisted(sender, recipient)
    {
        ...
        //Review this code
        if (!tradingEnabled && !authorized[sender]) {
        ...
        if (
            ...
            // (lpPairs[sender] == true &&
            //     recipient == contractAddresses.routerAddress) ||
            // (lpPairs[recipient] == true &&
            //     recipient == contractAddresses.routerAddress)
        ) {
            takeFee = false;
        }
        ...
        uint256 tokenBurn = 0;
        uint256 additionalBurnFromLP = 0;

        //BUY - Tokens coming from DEX
        if (lpPairs[sender]) {
            tokenBurn = contractFees.buyBurnFee;
            additionalBurnFromLP = contractFees.buyBurnLP;
            // console.log("Buy");
        }
        //SELL - Tokens going to DEX
        else if (lpPairs[recipient]) {
            tokenBurn = contractFees.userSellBurn;
            additionalBurnFromLP = 0;
        }
        //TRANSFER
        else {
            tokenBurn = contractFees.xfrFee;
            additionalBurnFromLP = contractFees.xfrBurnLP;
        }
        ...
        (
            ...
            uint256 secondTokenBurnTally
                ...
                amount,
                contractFees.baseTxnFee,
                tokenBurn,
                additionalBurnFromLP
            );

        uint256 tTransferAmount = _getTokenTransferAmount(
            amount,
            _baseFee,
            tokensToBurn
        );
        ...
        if (swapFeesimmediately && balances[primePair] > 0) {
            houseBalance = _baseFee;
            _actualTransfer(sender, address(this), _baseFee);
        } else {
            _actualTransfer(
                sender,
                contractAddresses.marketingWallet,
        ...
        if (tokensToBurn > 0) {
            _burnToken(burnType.General, sender, tokensToBurn);
        }

        if (secondTokenBurnTally > 0) {
            pendingTokensToBurnFromLP += secondTokenBurnTally;
        }

        if (takeFee && balances[primePair] > 0) {
            if (
                msg.sender != address(this) &&
                ...
                if (pendingTokensToBurnFromLP > 0) {
                    _unpairBakeAndBurn(
                        burnType.LP,
                        pendingTokensToBurnFromLP,
                        queuedDailyBurn
                    );
                }
            }
    ...
    function _unpairBakeAndBurn(
        burnType thisBurnType,
        uint256 tokensBeingTransacted,
        uint256 _dailyBurnPortion
    ) private nonReentrant {
        if (tokensBeingTransacted > 0) {
            (
                bool goBurn,
                uint256 tokenAmount,
                uint256 nativeETHReceived
            ) = _removeLiquidityETHForRequestedTokens(
                    tokensBeingTransacted,
                    _dailyBurnPortion
                );

            if (goBurn && tokenAmount > 0 && nativeETHReceived > 0) {
                ...
                if (success) {
                    ...
                    if (thisBurnType == burnType.LP) {
                        queuedDailyBurn = 0;
                        pendingTokensToBurnFromLP = 0;
                        dailyBurn[_dayCounter.current()]
                            .amountBurned = _dailyBurnPortion;

                        _burnToken(burnType.LP, address(this), amount);
                    }

                    // if (thisBurnType == burnType.Daily) {
                    //     uint256 BurnFromDaily = dailyBurns();
                    //     dailyBurn[_dayCounter.current()]
                    //         .amountBurned += BurnFromDaily;

                    //     _burnToken(burnType.LP, address(this), BurnFromDaily);
                    // }
                }
    ...
    function burnContractReservesADDValueToLP()
        external
        payable
        onlyOwner
        nonReentrant
        returns (bool)
    {
        IWETH(dexRouter.WETH()).deposit{value: address(this).balance}();

        uint256 myWETH = IERC20(dexRouter.WETH()).balanceOf(address(this));

        bool thisTransferResult = IERC20(dexRouter.WETH()).transfer(
            primePair,
            myWETH
        );

        return thisTransferResult;
    }
    ...
    function toggleLPBurn() external onlyOwner {
        stateOfLPInteraction = !stateOfLPInteraction;
    }
    ...
    function setGlobalFeeState(bool _thisGlobalStateValue)
        external
        onlyAuthorized
        returns (bool)
    {
        globalFeeState = _thisGlobalStateValue;

        return globalFeeState;
    }
    ...
    function unpairBakeAndBurn(
        uint256 tokensBeingTransacted,
        uint256 dailyBurnPortion
    ...
    function updateFee(FEES memory FeeStruct) external onlyOwner {
        require(
            FeeStruct.baseTxnFee < 100 &&
                FeeStruct.xfrFee < 100 &&
                FeeStruct.tokenBurn < 100 &&
                FeeStruct.burnTokenOnXfr < 100 &&
                FeeStruct.xfrBurnLP < 100 &&
                FeeStruct.xfrBurnOverride < 100 &&
                FeeStruct.burnBypassFee < 100 &&
                FeeStruct.userSellBurn < 100 &&
                FeeStruct.userBuyFee < 100 &&
                FeeStruct.buyBurnFee < 100 &&
                FeeStruct.buyBurnLP < 100
        );

        contractFees.baseTxnFee = FeeStruct.baseTxnFee * 100;
        contractFees.xfrFee = FeeStruct.xfrFee * 100;
        contractFees.tokenBurn = FeeStruct.tokenBurn * 100;
        contractFees.burnTokenOnXfr = FeeStruct.burnTokenOnXfr * 100;
        contractFees.xfrBurnLP = FeeStruct.xfrBurnLP * 100;
        contractFees.xfrBurnOverride = FeeStruct.xfrBurnOverride * 100;
        contractFees.burnBypassFee = FeeStruct.burnBypassFee * 100;
        contractFees.userSellBurn = FeeStruct.userSellBurn * 100;
        contractFees.userBuyFee = FeeStruct.userBuyFee * 100;
        contractFees.buyBurnFee = FeeStruct.buyBurnFee * 100;
        contractFees.buyBurnLP = FeeStruct.buyBurnLP * 100;

        ogFees.baseTxnFee = uint256(contractFees.baseTxnFee);
        ogFees.xfrFee = uint256(contractFees.xfrFee);
        ogFees.tokenBurn = uint256(contractFees.tokenBurn);
        ogFees.burnTokenOnXfr = uint256(contractFees.burnTokenOnXfr);
        ogFees.xfrBurnLP = uint256(contractFees.xfrBurnLP);
        ogFees.xfrBurnOverride = uint256(contractFees.xfrBurnOverride);
        ogFees.burnBypassFee = uint256(contractFees.burnBypassFee);
        ogFees.userSellBurn = uint256(contractFees.userSellBurn);
        ogFees.userBuyFee = uint256(contractFees.userBuyFee);
        ogFees.buyBurnFee = uint256(contractFees.buyBurnFee);
        ogFees.buyBurnLP = uint256(contractFees.buyBurnLP);
    }
    ...
    function updateIgniterContract(address _igniterAddress) external onlyOwner {
        igniterSupport = payable(_igniterAddress);
        contractAddresses.igniterContract = _igniterAddress;
    }

    function withDrawFees() public onlyOwner nonReentrant {
        require(houseEthBalance > 0, "Nothing to Withdraw");
        uint256 etherToTransfer = houseEthBalance;
        houseEthBalance = 0;

        address payable marketing = payable(contractAddresses.marketingWallet);
        address payable payload = payable(contractAddresses.payloadWallet);

        marketing.transfer(etherToTransfer/2);
        payload.transfer(etherToTransfer/2);
        ...
        //     uint256 _houseBalance = houseBalance;
        //     if (_houseBalance > 0 && balances[address(this)] >= _houseBalance) {
        //         houseBalance = 0;
        ...
        //         // _transferOwnership(msg.sender);
        //         dexRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
        //             _houseBalance, // accept as many tokens as we can
        //             0,
        //             path,
        //             address(this), //contractAddresses.marketingWallet, // Send To Recipient
        //             block.timestamp + 15
        //         );
        //     }
        // }
