...
contract BrewLabsStakingFactory is Ownable, ReentrancyGuard {
    ...
    function establishLock(
        ...
        for (uint256 i = 0; i < _locks.length; i++)
            IBrewLabsLockup(staking).addLockup(
                _locks[i]._duration,
                _locks[i]._depositFee,
                _locks[i]._withdrawFee,
                _locks[i]._rate
            );
    ...
    function addStakings(address[] memory _stakings) public onlyOwner {
        for (uint256 i = 0; i < _stakings.length; i++)
            stakings.push(_stakings[i]);
    }

    function removeStakings(address[] memory _stakings) public onlyOwner {
        for (uint256 i = 0; i < stakings.length; i++)
            for (uint256 j = 0; j < _stakings.length; j++) {
                if (stakings[i] == _stakings[j]) remove(i);
            }
    }

    function remove(uint256 index) internal {
        if (index >= stakings.length) return;

        for (uint256 i = index; i < stakings.length - 1; i++) {
            stakings[i] = stakings[i + 1];
        }
        delete stakings[stakings.length - 1];
        stakings.pop();
    }
