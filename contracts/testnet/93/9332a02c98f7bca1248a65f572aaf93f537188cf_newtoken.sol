...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20Extended public reward_token;
    ...
    uint256 public minDistribution = 1 * (10**reward_token.decimals());
    ...
    constructor(address router_, address _reward_token) {
        _token = msg.sender;
        router = IDexRouter(router_);
        reward_token = IBEP20Extended(_reward_token);
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        uint256 amount = reward_token.balanceOf(address(this))-(
            balanceBefore
        );
    ...
    function withdraw(address __Owner) external onlyToken {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(reward_token);
        ...
        reward_token.transfer(__Owner, reward_token.balanceOf(address(this))); // transfer all reward_token to Owner before Changing to new Reward Token Address
    }
...
contract newtoken is IBEP20Extended {
    ...
    address public reward_token = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    bool initializedpair;
    modifier initializerpair() {
        require(!initializedpair);
        _;
        initializedpair = true;
    }
    bool initializeddistributor;
    modifier initializerdistibutor() {
        require(!initializeddistributor);
        _;
        initializeddistributor = true;
    }
    ...
    function initDistributor() external initializerdistibutor onlyOwner {
        distributor = new DividendDistributor(address(router), reward_token);
        isDividendExempt[pair] = true;
        isDividendExempt[address(this)] = true;
        isDividendExempt[DEAD] = true;
    }
    function initPair(address _pair) external initializerpair onlyOwner {
    ...
    function setNewRewardToken(address newrewardToken) external onlyOwner {
        distributor.withdraw(msg.sender);
