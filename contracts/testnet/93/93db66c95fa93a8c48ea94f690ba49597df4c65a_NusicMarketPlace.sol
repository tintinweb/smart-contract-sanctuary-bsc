...
contract NusicMarketPlace {
    ...
    // address => (listing => biddingAmount)
    mapping(address => mapping(uint256 => uint256)) public biddings;
    ...
    function list(
        ...
        Listing memory listing = Listing({
            ...
            creator: msg.sender,
            isAuction: isAuction,
            auctionExpiresOn: auctionExpiresOn
        });
    ...
    function buyListing(
        ...
        if (msg.value < listing.askPrice) revert WrongValueSent();

        delete getListing[listingId];

        for (uint256 i = 0; i < owners.length; ++i) {
            (bool hs, ) = payable(owners[i]).call{value: amounts[i]}("");
        }
    ...
    function claimAuction(uint256 listingId, address winnerAddress)
        public
        payable
    {
        Listing memory listing = getListing[listingId];
        ...
        require(msg.sender == winnerAddress, "Not a winner");
