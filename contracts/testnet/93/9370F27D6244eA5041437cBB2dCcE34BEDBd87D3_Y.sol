...
contract Y is Ownable, LockToken, IERC20 {
    ...
    uint8   public  constant yieldDecimals = 7;
    uint8   public  constant DECIMALS = 9;

    uint256 public  constant startingSupply = 1_000_000_000 * 10 ** DECIMALS;
    uint256 public  constant maxSupply = startingSupply * 10;

    uint24  private constant capPerTx = 6000;
    uint256 private start;

    uint256 public  yieldPeriod = 3 hours;
    uint256 public  baseYieldRate = 40;
    ...
    constructor() {
        ...
        //balances[0x85A5B19f158c8beFE8b0c7A1241C0eDA50815064] = 1e5 * 10**decimals();
    ...
    function getNewBalance(address _address) public view returns(uint256 newBalance) {
        ...
        for (uint24 i = 0; i < times; i++) {
            newBalance = newBalance * (10 ** yieldDecimals + tier * baseYieldRate) / 10 ** yieldDecimals;
        }
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal  returns (uint256) {
        ...
        uint256 tier = yieldTier(sender);
        uint256 bonus = tier == 1 ? 1 : 0;
        uint256 toSwapBack = amount * (_treasuryFee + bonus) / feeDenominator;
        uint256 toLiquidity = amount * (_liquidityFee + bonus) / feeDenominator;
