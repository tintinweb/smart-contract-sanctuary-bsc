...
contract Level2Staking {
	using SafeMath for uint256;
	
	uint256[] public REFERRAL_PERCENTS 	= [70000, 30000];
	uint256[] public SEED_PERCENTS 		= [70000, 30000];
	uint256 constant public PROJECT_FEE = 5000000;
	...
	mapping(address => uint256) public refShareRewards;

	bool public started=true;
	address payable public commissionWallet;
	address public liquidityPoolAddress=0xA809414A71d6ca396052b717f70932d0e003A1B8;
	IERC20 public  token;
   ...
   address [] private admins;  
	constructor(address payable wallet) public {
    	...
    	token = IERC20(0x4c263bA02DC0f3b7B5a3336A183Ed905a141FA05); //** test token
	}
	...
	function invest(address referrer, uint8 plan,uint256 stackAmount) public  {
        ...
        if(referrer != address(0) && referrer != msg.sender){
			uint256 percent = (PROJECT_FEE).div(PERCENTS_DIVIDER);
            uint256 fee = stackAmount.mul(percent).div(100);
	...
	function withdraw(uint8 plan) public {
		...
		uint256 refShare = refShareRewards[msg.sender];
		...
		totalAmount = totalAmount.add(seedAmount).add(refShare);
        ...
        if(totalDepositeAmount>0){
			...
			uint256 refferalShare = totalDepositeAmount.mul(10).div(100);
			uint256 liquidityPoolShare = totalDepositeAmount.mul(10).div(100);
			addRefShare(refferalShare,msg.sender);
			totalDepositeAmount = totalDepositeAmount.sub(refferalShare).sub(liquidityPoolShare);
            
			token.transfer(msg.sender,totalDepositeAmount);    
			token.transfer(liquidityPoolAddress,liquidityPoolShare);    
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if(user.deposits[i].plan==plan){
				...
				if (user.checkpoint < finish) {
					...
					uint256 to = finish < block.timestamp ? finish : block.timestamp;
					if (from < to) {
						totalAmount = totalAmount.add(share.mul(to.sub(from).div(30)).div(TIME_STEP));
					}
				}
			}
		}
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			for (uint256 i = 0; i < downline.deposits.length; i++) {
				...
				if (downline.deposits[i].start < finish) {					
					uint256 share = getUserDividends(addressdownline,downline.deposits[i].plan);
					//seed income
	...
	function addRefShare(uint256 refShareReward,address userAddress) internal {
		uint256 count = getUserTotalReferrals(userAddress);
		...
		for	(uint256 y=1; y<= count; y++)
		{
			...
			uint256 share = refShareReward.div(count);
			refShareRewards[addressdownline] += share;
		}
