...
contract fBurnStake is Ownable, ReentrancyGuard {
    ...
    function getElapsedTime(uint256 _stakeId) public view returns(uint256){
        ...
        if(block.timestamp > userStakeDetails.unlockTime && userStakeDetails.lockedTime < block.timestamp) {
    ...
    function testGetUnclaimedRewards(uint256 _stakeBalance, uint256 _rarityApplied, uint256 _lockDuration, uint256 _lockedDays) public view returns(uint256){
        ...
        uint unclaimedRewards = applicableRewards * testGetElapsedTime(_lockedDays);

        return unclaimedRewards; 
    }

    function testGetElapsedTime(uint256 _lockedDays) public view returns(uint256){
    ...
    function claimRewards(uint256 _stakeId) nonReentrant unpaused public returns (bool){
        ...
        require(userStakeDetails.unlockTime <= block.timestamp,"You can not withdraw");
