...
contract Bonjour is ContextUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _ownerAddress, address _externalAddress) public initializer {
        ...
        reinvestExternalAmount = 15000;
        minReinvestment = 10 * 1e18;
        __Ownable_init();
    }
    ...
    function claimOrRetopop(bool isClaim,uint256 _claimAmount) public {
        ...
        if (totalearned.add(users[msg.sender].referupto) >= maxlimit && msg.sender != ownerAddress) {
            ...
            if(_claimAmount >= users[msg.sender].earnedAmount){
              _claimAmount = users[msg.sender].earnedAmount;
            }else{
              users[msg.sender].earnedAmount = users[msg.sender].earnedAmount - _claimAmount;
              isreset = false;
            }
            if (isClaim && _claimAmount > 0) {
                //require(users[msg.sender].earnedAmount >= minWithdrawAmount, "User should reach minimum withdraw");
                require(_claimAmount >= minWithdrawAmount, "User should reach minimum withdraw");
                BusdAddress.safeTransfer(msg.sender, _claimAmount);
                totalGainAmount[msg.sender] += _claimAmount;
                if(isreset){
                  users[msg.sender].investmentAmount = 0;
                  users[msg.sender].referupto = 0;
                }
            ...
            if (isClaim && _claimAmount > 0) {
                ...
                BusdAddress.safeTransfer(msg.sender, _claimAmount);
                totalGainAmount[msg.sender] += _claimAmount;
                users[msg.sender].referupto += _claimAmount;
            }
            users[msg.sender].investmentDate = block.timestamp;
        }
        if(isreset){
    ...
    function checkIsEarn(address _user) public view returns(bool) {
        ...
        uint diff = (endDate - startDate) / 60 / 30;
    ...
    function setminreinvestExternalAmount(uint256 amount) public onlyOwner {
        reinvestExternalAmount = amount;
    }
}