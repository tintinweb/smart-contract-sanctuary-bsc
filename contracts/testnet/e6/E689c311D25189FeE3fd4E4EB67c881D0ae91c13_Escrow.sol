...
contract Escrow is Ownable,Initializable {
    ...
    ///@param allocated market allocation amount in wei
    ///@param allocated market start time
    function initialize(Dividend memory dividentParams, address marketAddress, uint allocated, uint64 startTime) external onlyOwner initializer {
        require(!isZero(uint160(dividentParams.token)), "the dividend token must not be zero");
        require(!isZero(uint160(marketAddress)), "the market address must not be zero");
        ...
        require(!isZero(allocated), "the market allocation must not be zero");
        
        dividend = dividentParams;
        market.account = marketAddress;
        market.allocated = allocated;
        market.lastClaim = market.startTime = startTime;
    }
    ...
    ///@dev update the market address
    ///@param marketAddress market address
    function updateMarket(address marketAddress) external onlyOwner {
        require(!isZero(uint160(marketAddress)), "updateMarket: the market address must not be zero");
        market.account = marketAddress;
    }
    ...
    ///@dev market address claim its dividends from here
    function claim() external {
        require(market.account == _msgSender(), "claim: the market wallet is not a caller");
        require(!isZero(market.startTime), "claim: start time must not be zero");
        require(!isZero(market.allocated), "claim: allocation must not be zero");
        require(market.released <= market.allocated, "claim: the market wallet has claimed all dividends");

        if(!dividend.isInitialReleased) {
            _claimInitial();
        } else {
            require(
                market.lastClaim + dividend.period <= blockTimestamp(),
    ...
    function _claimInitial() private {
        ...
        market.released += (market.allocated * dividend.initialRelease) / DIVISOR;
        
        if(market.lastClaim + dividend.period <= blockTimestamp()) {
            _claim();
        }
    }
    ...
    function _claim() private {
        ...
        if(!isZero(dividends)) {
            market.released += dividends;
            market.lastClaim += (dividend.period * claims).toUint64();
    ...
    function viewDividend() public view returns(uint dividends, uint claims) {        
        if(!isValidClaim(market)) {   
            return (0,0);
        }

        uint months = (blockTimestamp() - market.lastClaim) / dividend.period;

        if(!isZero(months)) {
            uint divAmount = (market.allocated * dividend.percent) / DIVISOR;
            dividends = divAmount * months;
            claims = months;

            if((market.released + dividends) > market.allocated) {
                dividends = market.allocated - market.released;
            }
        }
    }

    ///@dev returns true if market is allowed to claim
    ///@param wallet market info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Market memory wallet) private view returns(bool) {
        return !(
