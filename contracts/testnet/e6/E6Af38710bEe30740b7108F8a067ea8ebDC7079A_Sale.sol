...
contract Sale is AccessControl{
    ...
    function initialize(uint256 _startingRate,uint256 _slabTotalMinerAvailableToPurchase,uint256 _totalRaised, uint256 _totalMinerSold) public {
    ...
    function addNewTier(
        ...
        tierBundles.push(TierBundleInformation({
    ...
    function removeTier(uint _index) external onlyRole(OPERATOR_ROLE) {
    ...
    function setTreasuryAddress(address _treasury) external onlyRole(OPERATOR_ROLE) {
        treasury = _treasury;
    }
    function getTier(uint256 amount) view public returns (uint){
        uint index = 0;
        uint256 index_cost = 0;
        for(uint i = 0; i < tierBundles.length; i++){
            //available bundle eligible for reward
            if(amount >= tierBundles[i].costInBusd && tierBundles[i].slabRemaining>0)
            {
    ...
    function getAllTier() view public returns (TierBundleInformation[] memory){
        return tierBundles;
    }

    function purchase(uint256 amountIn) external {
        uint _index= getTier(amountIn);
        TierBundleInformation storage tier = tierBundles[_index];
        require(tier.slabRemaining > 0, "No more left in this tier.");
        require(IERC20(Busd).balanceOf(msg.sender) >= amountIn, "Not enough Busd in your wallet!");
        uint256 minerToGive = amountIn / currentRate;
        // Giving back based on Max Purchasable Miner
        uint256 amountOut = min((slabTotalMinerAvailableForPurchase - slabCounter), minerToGive);
        amountIn = amountOut * currentRate;
        require(presaleOpen, "Presale is not open!");
        require(slabCounter < slabTotalMinerAvailableForPurchase, "All Miner sold out. Wait for the next round on Miner.");
        //to do
        require(amountIn <= maxPerWalletBUSDLimit, "Exceeded Buy Limit. Ask a discord admin on how to buy more.");
        require(amountOut <= IERC20(SMiner).balanceOf(address(this)), "Not enough Miner to sell!");
        require(IERC20(Busd).balanceOf(msg.sender) >= amountIn, "Not enough Busd in your wallet!");
    ...
    function transferOut(address _token, uint256 value, address to) public onlyRole(TRANSFEROUT_ROLE){
        require(value >= IERC20(_token).balanceOf(address(this)), "Requested Value Exceeds Balance.");
        IERC20(_token).safeTransfer(to, value);
    }

    function setNewSlab(uint256 _newSlab, uint256 _newPrice) external onlyRole(OPERATOR_ROLE) {
        slabCounter = 0;
    ...
    function setPerWalletMaxBuyLimitBUSD(uint256 _limit) external onlyRole(OPERATOR_ROLE) {
