...
contract RouterPayToken is EIP712, Ownable {
    ...
    event CrosschainTokenPaymentEvent(address indexed userFrom, address indexed worker, uint256 executionPrice, address token);
    event CreditCrosschainTokenPaymentEvent(address sender, address worker, uint256 executionPrice);
    ...
    function _checkWorkerSignature(
        ...
        bytes32 workerStructHash = keccak256(
            abi.encodePacked(
                keccak256(
                    "DelegatedCallWorkerPermit(address from,uint256 chainIdTo,uint256 executionPrice,bytes32 executionHash,address uniswapRouter,uint256 aggregationFee,uint256 nonce,uint256 deadline,address token)"
                ),
                msg.sender,
                chainIdTo,
    ...
    function _proceedFees(uint256 executionPrice, address worker, address token) internal {
        SafeERC20.safeTransferFrom(IERC20(token), msg.sender, worker, executionPrice);

        emit CrosschainTokenPaymentEvent(msg.sender, worker, executionPrice, token);
    }
    ...
    function synthesizeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTHESIZE_REQUEST_SIGNATURE_HASH, token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, token);
    ...
    function synthesizeRequestWithPermitPayNative(
        ...
        IERC20WithPermit(token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (amount + receipt.executionPrice),
            permitData.deadline,
            permitData.v,
            permitData.r,
            permitData.s
    ...
    function synthTransferRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_TRANSFER_REQUEST_SIGNATURE_HASH, tokenSynth, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenSynth);
    ...
    function unsynthesizeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _UNSYNTHESIZE_REQUEST_SIGNATURE_HASH, tokenSynth, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenSynth);
        SafeERC20.safeTransferFrom(IERC20(tokenSynth), msg.sender, address(this), amount);
    ...
    function unsynthesizeWithMetaExchangePayNative(
        ...
        address worker = _checkWorkerSignature(_synthParams.chainId, _UNSYNTHESIZE_WITH_META_EXCHANGE_REQUEST, _tokenParams.token, address(0), receipt);
        ...
        emit CreditCrosschainTokenPaymentEvent(msg.sender, worker, receipt.executionPrice);
    ...
    function unsynthesizeWithMetaExchangeWithPermitPayNative(
        ...
        IERC20WithPermit(_tokenParams.token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (_tokenParams.amount + receipt.executionPrice),
            permitData.deadline,
            permitData.v,
            permitData.r,
            permitData.s
...
/////////////////////////////
    function synthBatchAddLiquidity3PoolMintEUSDRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_MINT_EUSD_REQUEST_SIGNATURE_HASH, tokenParams.token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
    ...
    function synthBatchMetaExchangeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_META_EXCHANGE_REQUEST_SIGNATURE_HASH, tokenParams.token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
        SafeERC20.safeTransferFrom(IERC20(tokenParams.token), msg.sender, _portal, tokenParams.amount);
    ...
    function synthBatchAddLiquidity3PoolMintEUSDRequestWithPermitPayNative(
        ...
        IERC20WithPermit(tokenParams.token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (tokenParams.amount + receipt.executionPrice),
            permitData.deadline,
            permitData.v,
            permitData.r,
            permitData.s
    ...
    function synthBatchMetaExchangeWithSwapRequestWithPermitPayNative(
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_META_EXCHANGE_SWAP_REQUEST_SIGNATURE_HASH, tokenParams.token, address(0), receipt);
        IERC20WithPermit(tokenParams.token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (tokenParams.amount + receipt.executionPrice),
    ...
    function synthBatchMetaExchangeWithUnwrapRequestWithPermitPayToken(
        ICurveProxy.TokenInput calldata tokenParams,
        IPortal.SynthParamsMetaSwap memory synthParams,
        ICurveProxy.MetaExchangeParams memory metaParams,
        ISynthesis.PermitData calldata permitData,
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _SYNTH_BATCH_META_EXCHANGE_UNWRAP_REQUEST_SIGNATURE_HASH, tokenParams.token, address(0), receipt);
        IERC20WithPermit(tokenParams.token).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (tokenParams.amount + receipt.executionPrice),
    ...
    function synthBatchMetaExchangeWithUnwrapRequestPayToken(
        ICurveProxy.TokenInput calldata tokenParams,
        IPortal.SynthParamsMetaSwap memory synthParams,
        ICurveProxy.MetaExchangeParams memory metaParams,
        DelegatedCallReceipt calldata receipt
    ...
    function metaExchangeRequestVia3poolPayNative(
        ...
        address worker = _checkWorkerSignature(params.chainId, _LOCAL_META_EXCHANGE_REQUEST_SIGNATURE_HASH, tokenParams.token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
    ...
    function mintEusdRequestVia3poolPayToken(
        ICurveProxy.MetaMintEUSD calldata params,
        ICurveProxy.TokenInput calldata tokenParams,
        uint256 chainId,
        DelegatedCallReceipt calldata receipt
    ) external payable {
        address worker = _checkWorkerSignature(chainId, _MINT_EUSD_REQUEST, tokenParams.token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, tokenParams.token);
        SafeERC20.safeTransferFrom(IERC20(tokenParams.token), msg.sender, _curveProxy, tokenParams.amount);
        ICurveProxy(_curveProxy).addLiquidity3PoolMintEUSD(params, tokenParams);
    }

    function mintEusdRequestVia3poolWithPermitPayToken(
        ICurveProxy.MetaMintEUSD calldata params,
        ICurveProxy.PermitData calldata permitData,
        ICurveProxy.TokenInput calldata tokenParams,
        uint256 chainId,
    ...
    function redeemEusdRequestPayNative(
        ...
        address worker = _checkWorkerSignature(chainId, _REDEEM_EUSD_REQUEST_SIGNATURE_HASH, payToken, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, payToken);
    ...
    function redeemEusdRequestWithPermitPayNative(
        ...
        IERC20WithPermit(payToken).permit(
            msg.sender,
            address(this),
            permit.approveMax ? uint256(2**256 - 1) : (params.tokenAmountH + receipt.executionPrice),
            permit.deadline,
            permit.v,
            permit.r,
            permit.s
    ...
    function tokenSwapWithMetaExchangeRequestPayNative(
        ...
        address worker = _checkWorkerSignature(_synthParams.chainId, _TOKEN_SWAP_META_EXCHANGE_REQUEST_SIGNATURE_HASH, _exchangeParams.tokenToSwap, address(0), receipt);
        ICurveProxy.FeeParams memory feeParams = ICurveProxy.FeeParams(
            worker,
            receipt.executionPrice,
            _coinIndex
    ...
    function tokenSwapWithMetaExchangeRequestWithPermitPayNative(
        ...
        IERC20WithPermit(_exchangeParams.tokenToSwap).permit(
            msg.sender,
            address(this),
            permitData.approveMax ? uint256(2**256 - 1) : (_exchangeParams.amountToSwap + receipt.executionPrice),
            permitData.deadline,
            permitData.v,
            permitData.r,
            permitData.s
    ...
    function tokenSwapPayNative(
        ...
        address worker = _checkWorkerSignature(finalSynthParams.chainId, _TOKEN_SWAP_HASH, tokenToSwap, uniswapRouterV2, receipt);
        if(stable){
            _proceedFees(receipt.executionPrice, worker, tokenToSwap);
            ...
            SafeERC20.safeTransferFrom(IERC20(tokenToSwap), msg.sender, _curveProxyV2, amount);
            ICurveProxy(_curveProxyV2).tokenSwapLite(tokenToSwap, to, amountOutMin, tokenToReceive, deadline, from, amount, receipt.executionPrice, uniswapRouterV2, finalSynthParams);
        }
    ...
    function tokenSwapWithPermitPayNative(
        ...
        if(stable){
            ...
            SafeERC20.safeTransferFrom(IERC20(tokenToSwap), msg.sender, _curveProxyV2, amount);
            ICurveProxy(_curveProxyV2).tokenSwapLite(tokenToSwap, to, amountOutMin, tokenToReceive, deadline, from, amount, receipt.aggregationFee, uniswapRouterV2, finalSynthParams);
        }
    }


    function removeLiquidityPayToken(
        address remove,
        int128 x,
        ...
        address worker = _checkWorkerSignature(synthParams.chainId, _REMOVE_LIQUIDITY_HASH, token, address(0), receipt);
        _proceedFees(receipt.executionPrice, worker, token);
    ...
    function removeLiquidityWithPermitPayToken(
        address remove,
        int128 x,
        uint256 expectedMinAmount,
        address to,
    ...
    function tokenSwapUnwrapPayToken(
        address tokenToSwap,
        address to,
        uint256 amountOutMin,
        address tokenToReceive,
        ...
        address worker = _checkWorkerSignature(block.chainid, _TOKEN_SWAP_UNWRAP_HASH, tokenToSwap, uniswapRouterV2, receipt);
        _proceedFees(receipt.executionPrice, worker, tokenToSwap);
        SafeERC20.safeTransferFrom(IERC20(tokenToSwap), msg.sender, _curveProxyV2, amount);
    ...
    function tokenSwapUnwrapWithPermitPayToken(
        address tokenToSwap,
        address to,
        uint256 amountOutMin,
        address tokenToReceive,
