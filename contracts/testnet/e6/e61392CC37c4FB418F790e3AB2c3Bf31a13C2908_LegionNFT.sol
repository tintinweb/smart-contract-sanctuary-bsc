...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    mapping (address => uint256) public taxStartDay;
    ...
    event SetLiquidtyPool(address indexed _addr);
    constructor() {
        bloodstone = IERC20(0xab2F797028b05B938aaA7636FCb57d083817628a);
        ...
        //BUSD.approve(liquidityPool, BUSD.totalSupply());
    }
    ...
    function hunt(uint256 tokenId, uint8 monsterId) public {
        ...
        BUSD.transferFrom(msg.sender, liquidityPool, reward*feehandler.getFee(1)*10**10);
    ...
    function setLiquidityPool(address _lpAddr) external onlyOwner {
        require(_lpAddr!=address(0), "You can not set liquidity pool zero address");
        liquidityPool = _lpAddr;
        emit SetLiquidtyPool(_lpAddr);
    }
...
contract FeeHandler is IFeeHandler, Ownable {
    ...
    */
    ...
    address constant BLST = 0xab2F797028b05B938aaA7636FCb57d083817628a;
    ...
    function getSummoningPrice(uint256 _amount) external view override returns(uint256) {
        ...
        uint256 amountIn;
        if(_amount==1) {
            amountIn = UsdValue*10**18;
        } else if(_amount==10) {
            amountIn = UsdValue*10*99*10**16;
        } else if(_amount==50) {
            amountIn = UsdValue*50*98*10**16;
        } else if(_amount==100) {
            amountIn = UsdValue*100*97*10**16;
        } else if(_amount==150) {
            amountIn = UsdValue*150*95*10**16;
        } else {
            amountIn = UsdValue*_amount*10**18;
        }
    ...
    function getTrainingCost(uint256 _count) external view override returns(uint256) {
        ...
        path[0] = BUSD;
        path[1] = BLST;
        return router.getAmountsOut(_count*(10**18)/2, path)[1];
    }
    ...
    function getCostForAddingWarrior(uint256 _warriorCount, uint256 _remainingHunts) external view override returns(uint256) {
        ...
        path[0] = BUSD;
        path[1] = BLST;
        return router.getAmountsOut((_remainingHunts*_warriorCount)*10**18+_warriorCount*10**18/2, path)[1];
    }
    ...
    function getBLSTAmount(uint256 _usd) internal view returns(uint256) {
        ...
        path[0] = BUSD;
        path[1] = BLST;
        return router.getAmountsOut(_usd*10**18, path)[1];
    }
    ...
    function getBLSTReward(uint256 _reward) external view override returns(uint256) {
        ...
        path[0] = BUSD;
        path[1] = BLST;
        return router.getAmountsOut(_reward*10**14, path)[1];
    }
    function getExecuteAmount() external view override returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = BUSD;
        path[1] = BLST;
        return router.getAmountsOut(fees[3]*10**18*2/10, path)[1]; // 20% will return back to player
    }
}
