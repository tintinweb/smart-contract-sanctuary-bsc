...
contract RecruitCoinTest is ERC20, Ownable, ReentrancyGuard {
   ...
   */
  function manualDeposit(address[] calldata depositors,uint256[] calldata paymentAmounts, bool[] calldata preSale,bool[] calldata publicSale3M,bool[] calldata publicSale6M) public onlyOwner {
    require(depositors.length > 0,"EMPTY LIST NOT ALLOWED");
    for (uint256 i = 0; i < depositors.length; i++) {
      _deposit(depositors[i], paymentAmounts[i], 0, preSale[i], publicSale3M[i], publicSale6M[i]);
    }
  }
   ...
   * @param pTIndex : payment token index
   */
  function deposit(bytes32[] calldata merkleProof,uint256 paymentAmount, uint8 pTIndex) public {
    ...
    _deposit(msg.sender, paymentAmount, pTIndex, false, false, false);
  }
  ...
  function _deposit(
    ...
    if (getCurrentTime() >= dates[0] && getCurrentTime() < dates[1] || preSale) {
      ...
      _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(10),ONE_MINUTE.mul(120), ONE_MINUTE.mul(5), false, purchasedTokens.mul(1e18));
    }
    /*PUBLIC SALE*/
