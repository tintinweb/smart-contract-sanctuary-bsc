...
contract TokenVesting is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(address => uint256) public tokensPerClaim; // Total tokens por cada claim. Si se asignan 100 tokens. El total, serán 25 en Wei. Esto no se cambia en ningún momento, a menos,que se asignen nuevos.
    ...
    /**
     * @notice Función que permite cambiar el contrato del token usado en el vesting
     */
     ...
     * @notice Debe pasársele la cantidad en Wei
     */
    ...
    function assignTokensToWallet(
        uint256[] calldata cantidadesBBCNAAsignar, // Cantidad de tokens que se le asignarán a una wallet (es una lista)
        ...
        uint256 cantidadBBCNAAsignar; // Cantidad de tokens que se le asignarán a una única wallet
        ...
        for (uint256 i; i < walletsUsuarios.length; ++i) {
            ...
            // Convierto la cantidad a asignar en base 18
            //cantidadBBCNAAsignar = cantidadesBBCNAAsignar[i];
            // cantidadBBCNAAsignar = cantidadesBBCNAAsignar[i] * 10**18;
    ...
    function cancelVesting(
        ...
        tokensPerClaim[walletUsuario] = 0;
    }
        ...
        nonReentrant
    {
        ...
        tokensFromOneClaim = tokensPerClaim[_investor];
        ...
        require(
            TokenSmartContract.balanceOf(address(this)) >=
                balanceBeneficiary[_investor],
        ...
        if (balanceBeneficiary[_investor] == 0) {
            nextClaimDate[_investor] = 0;
            cooldownTimePerWallet[_investor] = 0;
            totalTokensClaim[_investor] = 0;
            tokensPerClaim[_investor] = 0;
        } else _triggerCooldown(_investor); // Se vuelve a inicializar el cooldown.
