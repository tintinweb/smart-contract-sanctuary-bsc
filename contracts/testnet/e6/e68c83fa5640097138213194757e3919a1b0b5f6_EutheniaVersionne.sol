...
contract EutheniaVersionne is ERC20Detailed, Ownable {
    ...
    string public _name = "Euthenia versionne ";
    string public _symbol = "EniaVersione";
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        3333 * 10**3 * 10**DECIMALS;

    uint256 public liquidityFee = 40;
    uint256 public treasuryFee = 40;
    uint256 public riskControlFee = 40;
    uint256 public sellFee = 60;
    uint256 public sControlFee = 10;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(riskControlFee).add(
            sControlFee
        );
    ...
    address public autoLiquidityFund;// diff Lalune
    address public treasuryFund;// diff Lalune
    address public riskControlFund;
    address public sControl;// diff Lalune
    address public pairAddress;
    bool public swapEnabled = true;// diff Lalune is false
    ...
    uint256 private constant MAX_SUPPLY = 3333 * 10**9 * 10**DECIMALS;// diff Lalune for MAX_SUPPLY = 100 * 10**9 * 10**DECIMALS;

    uint256 public INDEX;// diff lalune not define
    ...
    uint public ownerRebaseRate;// diff lalune not define
    bool public isOwnerRebaseEnabled;// diff lalune not define

    mapping(address => uint256) private _gonBalances;
    mapping(address => mapping(address => uint256)) private _allowedFragments;
    mapping(address => bool) public blacklist; //diff lalune not define

    constructor() ERC20Detailed("Euthenia versionne", "EniaVersione", uint8(DECIMALS)) Ownable() {
        router = IPancakeSwapRouter(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // test pancakSwap
        pair = IPancakeSwapFactory(router.factory()).createPair(
            router.WETH(),
            address(this)
        );
      
        autoLiquidityFund = 0xe405EA8f1B4E26BF94F8216aF0D2B29E1e096F0d;
        treasuryFund = 0x1Bbc45592f4e51CD044BEc4ABdfC718182F1199c;
        riskControlFund = 0xB73Bc63D3eC61419fb619EB0B54F0Cb64D84d3ce;
        sControl =  0x643fF1cA4A8eF6342BA66a83326639e254c8A620;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(riskControlFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryFund).call{
            value: amountETHToTreasuryAndIRF.mul(treasuryFee).div(
                treasuryFee.add(riskControlFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(riskControlFund).call{
            value: amountETHToTreasuryAndIRF.mul(riskControlFee).div(
                treasuryFee.add(riskControlFee)
            ),
            gas: 30000
        }("");
    ...
    function setFeeReceivers(
        ...
        address _riskControlFund,
        address _sControl
    ) external onlyOwner {
        autoLiquidityFund = _autoLiquidityFund;
        treasuryFund = _treasuryFund;
        riskControlFund = _riskControlFund;
        sControl = _sControl;
    }
