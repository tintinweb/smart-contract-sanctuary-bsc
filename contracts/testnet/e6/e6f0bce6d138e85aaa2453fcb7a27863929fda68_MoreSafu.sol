...
contract MoreSafu is ERC20Detailed, Ownable {
    ...
    uint256 public msafuDividendFee = 70;
    uint256 public sellFee = 20;
    uint256 public autofirePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(msafuDividendFee).add(
            autofirePitFee
        );
    ...
    address public msafuDividendReceiver;
    ...
    constructor() ERC20Detailed("MoreSafu", "MSU", uint8(DECIMALS)) Ownable() {
        ...
        msafuDividendReceiver = address(distributor);
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(msafuDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(msafuDividendFee)
            ),
            gas: 30000
        }("");
        if(success){}
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(msafuDividendFee).div(treasuryFee.add(msafuDividendFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no moreSafu token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
