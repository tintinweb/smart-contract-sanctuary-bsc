...
contract CryptoWealth {
	...
	constructor(address payable _master,address _token) { 
	    
	    owner = msg.sender;
       	master_wallet = _master;
       
		tokenbsc =IBEP20(_token);	
	...
	function getTokenBalance() public view returns (uint256) {
       return tokenbsc.balanceOf(address(this));
    }	
    ...
    function register(uint uplineid,uint256[] memory parametrs) public payable {
       ...
       if(users[msg.sender].id <= 0){
		...
		uint256 totalAmount=package_amount.add(activation_fees);
		
		tokenbsc.safeTransferFrom(msg.sender,address(this),totalAmount);
	...
	function buycredits(uint256 amount) public payable {
		...
		tokenbsc.safeTransferFrom(msg.sender,address(this),amount);  
	...
	function _tranefertosponsor(address receiver,address sender,uint256 amount,uint256 level) internal{ 
	    tokenbsc.safeTransfer(receiver,amount);
    ...
    function withdrawLostTokens() external OnlyOwner {        
        require(tokenbsc.balanceOf(address(this)) > 0, 'cannot withdraw deposit token');
        tokenbsc.transfer(owner,tokenbsc.balanceOf(address(this)));        
    }

    function setTokenContract(address _contract) OnlyOwner external {     
        tokenbsc = IBEP20(_contract);
    } 
