    ...
    PausableUpgradeable
{
     ...
     * @param amount Amount of token
     * @param token Address of token
     * @param logic Address of logic
     */
    function takeToken(uint256 amount, address token, address logic)
        external
        onlyMultiLogicProxy
        isUsedToken(token)
    {
        uint8 decimals = AggregatorV3Interface(token).decimals();
        uint256 amountExp18 = amount * 10**(18 - decimals);
        IERC20Upgradeable(token).safeTransfer(logic, amount);
     ...
     * @param logic Address of logic
     */
    function returnToken(uint256 amount, address token, address logic)
        external
        onlyMultiLogicProxy
        isUsedToken(token)
    {
     ...
     * @param logic Address of logic
     */
    function addEarn(uint256 amount, address logic) 
        external
        onlyMultiLogicProxy
        {
