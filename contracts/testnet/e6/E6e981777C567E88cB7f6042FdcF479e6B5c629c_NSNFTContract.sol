...
contract NSNFTContract is ERC721Enumerable, Ownable, ERC721Burnable {
    ...
    event JoinFace(uint256[] indexed ids);
    event SetPricesEvent(uint256[] indexed ids);
    event SetURIEvent(uint256 indexed id);
    event UpdateURIEvent(uint256 indexed id);
    event RemoveURIEvent(uint256 indexed id);

    constructor() ERC721("NSNFTContract", "NSN") {
        pause(true);
    }
    ...
    function updateURI(uint256 id, string memory uri) public onlyOwner {
        require(!_mintStates[id], "Already Minted - Cannot Update");
        require(id <= totalUpload, "Invalid Id");
        updateTokenURI(id, uri);
        emit UpdateURIEvent(id);
    }

    function removeURI(uint256 id) public onlyOwner {
        require(!_mintStates[id], "Already Minted - Cannot Remove");
        require(id <= totalUpload, "Invalid Id");
        removeTokenURI(id);
        emit RemoveURIEvent(id);
    }

    function getTotalUpload() public view returns (uint256) {
        return totalUpload;
    }
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function setTokenURI(uint256 _tokenId, string memory _token_uri) internal {
        require(_tokenId == totalUpload + 1, "TokenId mismatch");
        _tokenURI[_tokenId] = _token_uri;
        totalUpload += 1;
    }

    function updateTokenURI(uint256 _tokenId, string memory _token_uri) internal {
        _tokenURI[_tokenId] = _token_uri;
    }

    function removeTokenURI(uint256 _tokenId) internal {
        for(uint i = _tokenId; i < totalUpload; i++) {
            _tokenURI[i] = _tokenURI[i+1];
        }
        totalUpload -= 1;
    }
