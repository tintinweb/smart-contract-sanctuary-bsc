...
contract ALEX is Context, IERC20, Ownable {
   ...
   //=======General=======//
    ...
    uint256 public marketingAndDevelopmentFee=5;
    uint256 private _previousmarketingAndDevelopmentFee = marketingAndDevelopmentFee;
    address public marketingAndDevelopmentWallet=0xe4B560ba6C86c8809cFA77969195B7E45eD2E3B5;

    uint256 public totalFees = taxFee + liquidityFee + marketingAndDevelopmentFee;
    ...
    uint256 private maxTransactionRateBuy=1;//deminator 1000
    uint256 private maxTransactionRateSell=1;//deminator 1000
    ...
    constructor(address _newOwner){ 
        require(taxFee + liquidityFee + marketingAndDevelopmentFee <= 25, "Total fees cannot be more than 25%");
        require(maxTransactionRateBuy >= 1 && maxTransactionRateSell >= 1, "Max transfer rates must be greater than 0.1%");
        require(maxWalletLimitRate  >= 10, "Max wallet limit rate must be greater than 1%");
        ...
        _isExcludedFromFees[marketingAndDevelopmentWallet] = true;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingAndDevelopment) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketingAndDevelopment, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tMarketingAndDevelopment);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tMarketingAndDevelopment = calculateMarketingAndDevelopmentFee(tAmount);
        uint256 tTransferAmount = tAmount - tFee - tLiquidity - tMarketingAndDevelopment;
        return (tTransferAmount, tFee, tLiquidity, tMarketingAndDevelopment);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingAndDevelopment, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rLiquidity = tLiquidity * currentRate;
        uint256 rMarketingAndDevelopment = tMarketingAndDevelopment * currentRate;
        uint256 rTransferAmount = rAmount - rFee - rLiquidity - rMarketingAndDevelopment;
    ...
    function _takeMarketingAndDevelopment(uint256 tMarketingAndDevelopment) private {
        if (tMarketingAndDevelopment > 0) {
            uint256 currentRate =  _getRate();
            uint256 rMarketingAndDevelopment = tMarketingAndDevelopment * currentRate;
            _rOwned[address(this)] = _rOwned[address(this)] + rMarketingAndDevelopment;
            if(_isExcluded[address(this)])
                _tOwned[address(this)] = _tOwned[address(this)] + tMarketingAndDevelopment;
        }
    }
    ...
    function calculateMarketingAndDevelopmentFee(uint256 _amount) private view returns (uint256) {
        return _amount * marketingAndDevelopmentFee / 100;
    }
    
    function removeAllFee() private {
        if(taxFee == 0 && liquidityFee == 0 && marketingAndDevelopmentFee == 0) return;
        
        _previousTaxFee = taxFee;
        _previousLiquidityFee = liquidityFee;
        _previousmarketingAndDevelopmentFee = marketingAndDevelopmentFee;
        taxFee = 0;
        marketingAndDevelopmentFee = 0;
        liquidityFee = 0;
    }
    ...
    function restoreAllFee() private {
       ...
       marketingAndDevelopmentFee = _previousmarketingAndDevelopmentFee;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 taxForSwap = liquidityFee + marketingAndDevelopmentFee;
            if(taxForSwap > 0) {
                if(liquidityFee > 0) {
                ...
                if(marketingAndDevelopmentFee > 0) {
                    uint256 marketingTokens = contractTokenBalance * marketingAndDevelopmentFee / taxForSwap;
    ...
    function swapAndSendMarketing(uint256 tokenAmount) private {
        ...
        payable(marketingAndDevelopmentWallet).transfer(newBalance);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingAndDevelopment) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender] - rAmount;
        _rOwned[recipient] = _rOwned[recipient] + rTransferAmount;
        _takeMarketingAndDevelopment(tMarketingAndDevelopment);
    ...
    function changeMarketingWallet(address _marketingAndDevelopmentWallet) public onlyOwner {
        require(_marketingAndDevelopmentWallet != marketingAndDevelopmentWallet, "Marketing wallet is already that address");
        marketingAndDevelopmentWallet = _marketingAndDevelopmentWallet;
        emit MarketingWalletChanged(marketingAndDevelopmentWallet);
    }

    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        taxFee = _taxFee;
        totalFees = taxFee + marketingAndDevelopmentFee + liquidityFee;
        require(totalFees <= 25, "Total fees must be less than 25%");
    }

    function setMarketingAndDevelopmentFee(uint256 _marketingAndDevelopmentFee) external onlyOwner {
        marketingAndDevelopmentFee = _marketingAndDevelopmentFee;
        totalFees = taxFee + marketingAndDevelopmentFee + liquidityFee;
        require(totalFees <= 25, "Total fees must be less than 25%");
    }
