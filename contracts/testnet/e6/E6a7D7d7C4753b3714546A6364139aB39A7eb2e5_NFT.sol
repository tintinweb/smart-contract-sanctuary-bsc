...
contract NFT is ERC721,Ownable{
    ...
    uint public heroSupply = 4000;
    uint public equipSupply = 4000;
    ...
    mapping (uint=>Attr) attrs;
    uint heroNum;
    uint equipNum;
    string public uri;
    mapping  (address=>bool) public whiteList;
    constructor() ERC721("ROR NFT", "ROR") {

    }

    function setWhiteList(address addr,bool b) onlyOwner external{
        whiteList[addr] = b;
    }
    ...
    function mint(address recipient, Attr memory attr) external returns(uint256) {
        require(whiteList[msg.sender], "get out");
        if (attr.types == 1) {
            require(heroNum<= heroSupply,"max 4000");
            heroNum++;
        } else if (attr.types == 2){
            require(equipNum<= heroSupply,"max 4000");
            equipNum++;
        }
    ...
    function setHeroSupply(uint total) external onlyOwner {
        heroSupply = total;
    }
     function setEquipSupply(uint total) external onlyOwner {
        equipSupply = total;
    }
