...
contract Enigma is IERC20, Ownable {
    ...
    IUniswapV2Router02 public constant router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    mapping(address => Permissions) public permissions;

    address public sellFeeRecipient;
    address public buyFeeRecipient;
    address public transferFeeRecipient;
    uint256 public constant minTokensToLiquidate = 10000000;
    ...
    function _transferFrom(
        ...
        require( tradingEnabled || permissions[recipient].isLiquidityPool , "Trading Not Enabled");
        // tax liquidation and distribution
        if (IERC20(address(this)).balanceOf(address(this)) >= minTokensToLiquidate){
            _trigger();
        }
    ...
    function _liquidateToken(address token_) internal returns (bool) {
        ...
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(balance, 0, swapPath, address(this), block.timestamp + 30000);
        delete swapPath;
        return true;
    }

    function _trigger() internal {
        bool l = _liquidateToken(address(this));
        uint256 each = address(this).balance.div(3);
        _send(addr0, each);
        _send(addr1, each);
        _send(addr2, each);
        require(l, "Failure On Liquidate");
    }
