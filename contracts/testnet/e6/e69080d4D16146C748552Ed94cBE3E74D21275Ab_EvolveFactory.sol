...
contract EvolveFactory is Context, Ownable, EvolveAccessControll, IEvolveFactory {
    ...
    function addToken(IERC20 _tokenAddress,uint256 _stablePrice) external onlyAdmin {
        tokens.push(Token( _tokenAddress,_stablePrice,0,0));
    }

    function removeToken(uint256 _index) external onlyAdmin {
        require(_index < tokens.length, "Index out of range");
        delete tokens[_index];
    }

    function depositToken(uint256 _index, uint256 _amount) external onlyAdmin {
        require(_index < tokens.length, "Index out of range");
        tokens[_index].tokenAddress.transferFrom(_msgSender(), address(this), _amount);
        tokens[_index].totalBalance = tokens[_index].totalBalance.add(_amount);
    }

    function withdrawToken(uint256 _index, uint256 _amount) external onlyAdmin {
        require(_index < tokens.length, "Index out of range");
        uint256 totalBalance = tokens[_index].totalBalance;
        uint256 unlockedBalance = totalBalance.sub(tokens[_index].lockedBalance);
        require(unlockedBalance >= _amount, "Not enough unlocked balance");
        tokens[_index].tokenAddress.transfer(_msgSender(), _amount);
        totalBalance = totalBalance.sub(_amount);
        tokens[_index].totalBalance = totalBalance;
    }

    function readTokens() external view onlyAdmin returns (Token[] memory) {
        return tokens;
    }
    ...
    function addNewCompetion(uint256 _presetId, Team[] memory _teams, uint256 _createAt) external override ownerOrAdmin returns(uint256 competitionId){
        ...
        for (uint8 i = 1; i < _teams.length; i++) {
            require(_teams[i].competitors.length == teamMemberLength, "Teams length must be same");
        }

        (uint entryFeeInUSD,uint playerNumber,  uint[] memory supportedTokens ) = evolveStorage.getPreset(_presetId);
        require(teamMemberLength == playerNumber, "team member need to be equal with preset team number!");

        competitionId = evolveStorage.addNewCompetionInternal(_presetId, _teams, _createAt);

        for(uint8 i = 0 ; i < _teams.length ; i++){
            for(uint8 j = 0 ; j < teamMemberLength ; j++){
                uint256 tokenIndex = _teams[i].competitors[j].tokenIndex;
                require(tokenIndex < supportedTokens.length, "Token index out of range");
                require(isTokenSupportedByPreset(supportedTokens, tokenIndex), "Token is not supported by preset");

                uint256 payableInUSD = _teams[i].competitors[j].payableInUSD;
                require(payableInUSD <= entryFeeInUSD, "Payable amount cannot be more than entry fee");

                address competitorAddress = _teams[i].competitors[j].account;
                
                transferDesiredTokenFromCompetitor(tokenIndex, competitorAddress, payableInUSD);
            }
        }
    }

    function transferDesiredTokenFromCompetitor(uint256 _tokenIndex, address _competitorAddress, uint256 entryFeeInUSD) internal returns(bool){
        ...
        uint256 competitorBalanceInUSD= token.balanceOf(_competitorAddress).mul(stablePrice);
        require(competitorBalanceInUSD >= entryFeeInUSD, "Competitor balance is not enough");
        uint256 entryFeeInToken = entryFeeInUSD.div(stablePrice);
        token.transferFrom(_competitorAddress, address(this), entryFeeInToken);
        uint256 accessibleBalance =   DesiredToken.totalBalance - DesiredToken.lockedBalance;
        require(accessibleBalance >= entryFeeInToken, "Not enough unlocked balance");
        DesiredToken.lockedBalance = DesiredToken.lockedBalance.add(entryFeeInToken);
        return true;
    }

    function isTokenSupportedByPreset(uint256[] memory supportedTokens, uint256 tokenIndex) internal pure returns(bool){
    ...
    function setCompetionWinner(uint256 _competionId, uint8 _winnerTeam) external override  ownerOrAdmin returns(bool){
       (uint256 presetPrice,Competitor[] memory teamA, Competitor[] memory teamB, uint _competionStatus, uint _competionWinner, uint rakeAmountInUSD) = evolveStorage.getCompetion(_competionId);
    ...
    //    Competitor[] memory _winnerCompetitors = competion.teams[_winnerTeam].competitors;
    ...
    function pay(Competitor[] memory winnerCompetitors, uint256 rakeAmountInUSD, bool areTeamsEqual) internal {
        for(uint8 i = 0 ; i < winnerCompetitors.length ; i++){
            ...
            uint256 stablePrice = desiredToken.stablePrice;
            IERC20 token = IERC20(desiredToken.tokenAddress);
            uint256 entryFeeInToken = payableInUSD.div(stablePrice);
            uint256 rakeAmountInToken =  rakeAmountInUSD.div(stablePrice);
            uint payableInToken;
            if(areTeamsEqual){
            ...
            unchecked {
                desiredToken.lockedBalance = desiredToken.lockedBalance - entryFeeInToken;
            }
            unchecked {
                desiredToken.totalBalance = desiredToken.totalBalance - entryFeeInToken;
            }
        }
    }
...
interface IEvolveFactory{
    ...
    function addNewCompetion(uint256 _presetId, Team[] memory _teams, uint256 _createAt) external returns(uint256 competitionId);
