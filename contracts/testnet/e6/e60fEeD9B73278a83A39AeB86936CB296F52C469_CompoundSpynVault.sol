...
contract CompoundSpynVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    ISpynPool public vaultReferralTreasury;
    ...
    function initialize(
        ...
        vaultReferralTreasury = ISpynPool(_vaultReferralTreasury);
    ...
    function updateReferralTreasury(ISpynPool _treasury) external onlyOwner {
        vaultReferralTreasury = _treasury;
    }
    ...
    */
    ...
    function withdraw(uint256 amount) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (remainingAmount > 0) {
            spyn.transfer(treasury, remainingAmount);
        }
        /*
        if(isReceiveToken1){
    ...
    function takeHarvestFee(address _user, uint256 _pending, address _token) internal returns (uint256 feeTaken) {
        ...
        if (harvestFeeAmount > 0) {
            IERC20(_token).transfer(treasury, harvestFeeAmount);
        }
    ...
    function takeDepositFee(address _user, uint256 _depositedAmount, address _token) internal returns (uint256 feeTaken) {
        ...
        if (depositFeeAmount > 0) {
            IERC20(_token).transfer(treasury, depositFeeAmount);
        }
    ...
    function depositStakingPool(uint256 amount) internal {
        masterchef.deposit(farmPid, amount, address(vaultReferralTreasury));
    }
