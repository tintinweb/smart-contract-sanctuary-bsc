...
abstract contract AllTheFees is Auth {
    ...
    uint256 public BuyFeeNewcomerShares = 1;      // NewComer FIX 1%
    ...
    uint256 public totalBuyFee = BuyFeeMarketing.add(BuyFeeLP).add(BuyFeeBB).add(BuyFeeWinnersReward).add(BuyFeeJackpotReward).add(BuyFeeLuckyShares).add(BuyFeeNewcomerShares).add(BuyFeeHouse);
    ...
    uint256 public SellFeeNewcomerShares = 1;      // NewComer FIX 1%
    function ActualFeeOfSellFeeNewcomerShares() external view returns (uint256) { return SellFeeNewcomerShares; }


    uint256 public QuittersPenalty = 2;      // Quitters Penalty 2-6% If someone sells more than 90% of their tokens, some extra fee is applied to the sell! (Don't leave us if you wanna become rich)
    ...
    uint256 public totalSellFee = SellFeeMarketing.add(SellFeeLP).add(SellFeeBB).add(SellFeeWinnersReward).add(SellFeeJackpotReward).add(SellFeeLuckyShares).add(SellFeeNewcomerShares).add(SellFeeHouse);
...
contract LuckToken1 is Token {
    ...
    function transferFrom2(address sender, address recipient, uint256 amount) external returns (bool) {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        ...
        if(!takeFee){basicTransferFrom(sender, recipient, amount);}

        if(sender == pair){ //checks if it is buy
            uint256 amountAfterTax = amount - totalBuyFee.mul(amount).div(100);
            //uint256 taxPayed = totalBuyFee.mul(amount).div(100);


            UnpayedMarketing += BuyFeeMarketing.mul(amount).div(100);
            TotalMarketing += BuyFeeMarketing.mul(amount).div(100);
            
            UnpayedBB += BuyFeeBB.mul(amount).div(100);
            TotalBB += BuyFeeBB.mul(amount).div(100);
            
            UnpayedWinnersReward += BuyFeeWinnersReward.mul(amount).div(100);
            TotalWinnersReward += BuyFeeWinnersReward.mul(amount).div(100);
            
            UnpayedJackpotReward += BuyFeeJackpotReward.mul(amount).div(100);
            TotalJackpotReward += BuyFeeJackpotReward.mul(amount).div(100);

            UnpayedLuckyShares += BuyFeeLuckyShares.mul(amount).div(100);
            TotalLuckyShares += BuyFeeLuckyShares.mul(amount).div(100);

            UnpayedNewcomerShares += BuyFeeNewcomerShares.mul(amount).div(100);
            TotalNewcomerShares += BuyFeeNewcomerShares.mul(amount).div(100);

            UnpayedFeeHouse += BuyFeeHouse.mul(amount).div(100);
            TotalFeeHouse += BuyFeeHouse.mul(amount).div(100);
    ...
    function basicTransferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
