...
contract xMooneyVault is Ownable, ReentrancyGuard {
    ...
    function releaseTokens() external nonReentrant {
        ...
        if (contractBalance > amountOfTokens) {
            ...
            require(swapTokenForEth(amountToSell), "Swap for Token Failed");
    ...
    function swapTokenForEth(uint256 amount) private returns (bool) {
        uint256 deadline = block.timestamp + 15; // using 'now' for convenience, for mainnet pass deadline from frontend!
    ...
    function swapTokenForEthPub(uint256 amount)
        public
        nonReentrant
        returns (bool)
    {
        require(swapTokenForEth(amount), "Swap for Eth Failed");
