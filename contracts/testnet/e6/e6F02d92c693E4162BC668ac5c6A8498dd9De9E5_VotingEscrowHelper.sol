...
contract VotingEscrowHelper is IVotingEscrowCallback, Ownable {
    IVotingEscrowCallback[] public rebatePool;
    ...
    constructor(
        ...
        rebatePool.push(IVotingEscrowCallback(rebatePool_) );
    }

    function syncWithVotingEscrow(address account) external override {
        emission.syncWithVotingEscrow(account);
        for(uint256 i = 0; i< rebatePool.length; i++) {
            rebatePool[i].syncWithVotingEscrow(account);
        }
    }

    function addRebatePool(address addressPool) public onlyOwner isContract(addressPool) {
        rebatePool.push(IVotingEscrowCallback(addressPool));
    }
