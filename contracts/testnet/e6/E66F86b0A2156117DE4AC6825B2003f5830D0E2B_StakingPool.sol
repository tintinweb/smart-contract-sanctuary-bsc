...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accTokenPerShare;   // Accumulated rewardToken per share, times 1e18. See below.
    ...
    // Update the given pool's rewardToken allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingToken(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.lpSupply != 0) {
            ...
            accTokenPerShare = accTokenPerShare.add(reward.mul(1e18).div(pool.lpSupply));
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accTokenPerShare = pool.accTokenPerShare.add(reward.mul(1e18).div(pool.lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Deposit LP tokens to TimeLockPool for rewardToken allocation.
