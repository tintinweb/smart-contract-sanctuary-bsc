...
contract shera is Context, IERC20, LockToken {
    ...
    uint256 private _tTotal = 2e12 *10**9;
    ...
    uint256 public maxWalletToken = 150000000001 * 10**9;
    uint256 public buyLimit = 40000000001 *10**9;
    ...
    function _transfer(
        ...
        // Set buy fees
        if(from == uniswapV2Pair || from.isContract() && to != owner()) {
            _taxFee = _buyTaxFee;
        ...
        // Set sell fees
        if(to == uniswapV2Pair || to.isContract() &&  from !=owner()) {
            _taxFee = _sellTaxFee;
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        ...
        _buyBurnFee = burnFee;
        require(_buyTaxFee + _buyMarketingFee + _buyLiquidityFee + _buyBurnFee <= 11, "Buy Fee should be 11% or less");
    }
