...
/**
@dev This contract is for USDs token.
     Make sure Treasury, BUSD contracts are deployed before this contract is being initialized.
     Admin.whitelist();  address of whitelist contract.
     Admin.Treasury();  address of Treasury contract.
     Admin.BUSD();  address of BUSD token.
     Admin.WBNB(); address of WBNB. 
     Admin.BshareBUSDVault();  address of BshareBUSDVault contract.
     Admin.ApeswapRouter(); address of Router.
     _admin address of Admin Contract.
 */
contract BShare is ERC20, AccessControl, Pausable {
    ...
    uint public priceImpactProtection = 10; //Price impact protection is multiply by 10 for testing, For deployment 10 = 1
    ...
    uint [] public BShare_Tax_Allocation = [1000, 500]; // index 1 => YSL Vault, index 2 => treasury contract
    IAdmin public Admin;
   ...
   **/
    ...
    **/
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        // require(false,"BShare: User can't transfer USDs");
        console.log('################################');
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        require(restrictTransfer[sender] != block.number,"BShare: you can't interact in same block");
        ...
        if(isContract(sender) || isContract(recipient)){
            ...
            "BShare: No external contract interact with bshare");
        }
        if(sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB()) || recipient == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.WBNB())){
            ...
            if(IWhitelist(Admin.whitelist()).getAddresses(user)){
                ...
                if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[2] >= BShare_ThresholdLimit){
                    ...
                    IERC20(Admin.BUSD()).transfer(Admin.BShareBUSDVault(), (convertedBUSD * BShare_Tax_Allocation[0])/BShare_Tax);
                    IERC20(Admin.BUSD()).transfer(Admin.Treasury(), ((convertedBUSD * BShare_Tax_Allocation[1])/BShare_Tax));
                }
                amount -= taxAmount;
                ...
                require(prevAmount + ((prevAmount * priceImpactProtection)/1000) >= currentAmount && prevAmount - ((prevAmount * priceImpactProtection)/1000) <= currentAmount,
                    "BShare: The difference b/w prev and current is greater than or less than the priceImpactProtection");
            }
    ...
    */
    ...
    Note allocation point should be equal to tax
        ...
        only admin can call 
     */
    function setBShareAndAllocationTax(uint tax, uint[] memory allocationTax) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tax > 0, 'BShare: Tax should be greater than zero');
        ...
        require(total == tax, 'BShare: Tax not equal to sum of allocation tax');
        BShare_Tax = tax;
     ...
     */
    function setYSLThresholdLimit(uint _BShare_ThresholdLimit) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_BShare_ThresholdLimit > 0, 'YSL: Threshold limit for YSL can not be zero');
