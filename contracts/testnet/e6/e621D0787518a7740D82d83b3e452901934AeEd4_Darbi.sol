...
contract Darbi is AccessControl, Safe {
  ...
  function arbitrage() public onlyMonitor {
    (bool aToB, uint256 amountIn, uint256 reserves0, uint256 reserves1, uint256 backedValue) = moveMarketBuyAmount();
    ...
    // If Buying UP
    if (aToB) {
      arbitrageBuy(balances, amountIn, backedValue, reserves0, reserves1);
    } else {
      arbitrageSell(balances, amountIn, backedValue);
    }
  }

  function arbitrageBuy(uint256 balances, uint256 amountIn, uint256 backedValue, uint256 reserves0, uint256 reserves1) public onlyMonitor {
  ...
  struct ArbitrageSellVars {
    address[] path;
    uint256 darbiBalanceMaximumUpToMint;
    uint256 actualAmountIn;
    uint256 nativeToMint;
    uint256 up2Balance;
    uint256 diffBalances;
    bool success;
  }

  function arbitrageSell(uint256 balances, uint256 amountIn, uint256 backedValue) public onlyMonitor {
    ArbitrageSellVars memory vars;

      vars.darbiBalanceMaximumUpToMint = (balances * 1e18) / backedValue; // Amount of UP that we can mint with current balances
      vars.actualAmountIn = amountIn <= vars.darbiBalanceMaximumUpToMint
        ? amountIn
        : vars.darbiBalanceMaximumUpToMint; // Value in UP
      vars.nativeToMint = (vars.actualAmountIn * backedValue) / 1e18;
      DARBI_MINTER.mintUP{value: vars.nativeToMint}();

      vars.path[0] = address(UP_TOKEN);
      vars.path[1] = WETH;

      vars.up2Balance = UP_TOKEN.balanceOf(address(this));
      UP_TOKEN.approve(address(router), vars.up2Balance);
      ...
      vars.diffBalances = address(this).balance - balances;
      (vars.success, ) = address(UP_CONTROLLER).call{value: vars.diffBalances}("");
      require(vars.success, "FAIL_SENDING_BALANCES_TO_CONTROLLER");
      // This Sells UP
  }

  function moveMarketBuyAmount() public view returns (bool aToB, uint256 amountIn, uint256 reserves0, uint256 reserves1, uint256 upPrice) {
    address[2] memory swappedTokens = [address(UP_TOKEN), address(WETH)];
    (reserves0, reserves1) = UniswapHelper.getReserves(
      factory,
      swappedTokens[0],
      swappedTokens[1]
    );
