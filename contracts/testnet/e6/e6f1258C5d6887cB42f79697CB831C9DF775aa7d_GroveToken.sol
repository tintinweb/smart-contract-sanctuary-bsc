...
contract GroveToken is IERC20, Ownable {
    ...
    CustomTaxPeriod private _default =
        CustomTaxPeriod("default", 0, 0, 1, 1, 3, 3, 1, 1, 1, 1, 2, 2);
    CustomTaxPeriod private _base =
        CustomTaxPeriod("base", 0, 0, 1, 1, 3, 3, 1, 1, 1, 1, 2, 2);
    ...
    event WalletChange(
        string indexed walletIdentifier,
    ...
    event CustomTaxPeriodChange(
        uint8 indexed newValue,
        uint8 indexed oldValue,
    ...
    event Mint(uint256 indexed tokenAmount);
    event Burn(uint256 indexed tokenAmount);
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "GroveToken: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        ...
        onlyOwner
    {
        require(
            ...
            "GroveToken: Account is already the value of 'excluded'"
        );
        _isExcludedFromFee[account] = excluded;
        ...
        if (excluded) {
            ...
            for (uint256 i = 0; i < _excludedFromDividends.length; i++) {
                if (_excludedFromDividends[i] == account) {
                    _excludedFromDividends[i] = _excludedFromDividends[
                        _excludedFromDividends.length - 1
                    ];
                    _tOwned[account] = 0;
    ...
    function setWallets(
        ...
        if (liquidityWallet != newLiquidityWallet) {
            require(
                ...
                "GroveToken: The liquidityWallet cannot be 0"
            );
            emit WalletChange(
                "liquidityWallet",
                newLiquidityWallet,
        ...
        if (marketingWallet != newMarketingWallet) {
            require(
                ...
                "GroveToken: The marketingWallet cannot be 0"
            );
            emit WalletChange(
                "marketingWallet",
                newMarketingWallet,
        ...
        if (buyBackWallet != newBuyBackWallet) {
            require(
                ...
                "GroveToken: The buyBackWallet cannot be 0"
            );
        ...
        onlyOwner
    {
        require(
            ...
            "GroveToken: The selected wallet is already set to the value "
        );
        _feeOnSelectedWalletTransfers[account] = value;
    ...
    function setUniswapRouter(address newAddress) external onlyOwner {
        require(
            ...
            "GroveToken: The router already has that address"
        );
    ...
    function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
        require(
            ...
            "GroveToken: Cannot update maxTxAmount to same value"
        );
    ...
    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(
            ...
            "GroveToken: Cannot update maxWalletAmount to same value"
        );
    ...
    function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
        require(
            ...
            "GroveToken: Cannot update minimumTokensBeforeSwap to same value"
        );
        emit MinTokenAmountBeforeSwapChange(newValue, minimumTokensBeforeSwap);
        minimumTokensBeforeSwap = newValue;
    }

    function setBridgeController(address newAddress) external onlyOwner {
        require(
            bridgeController != newAddress,
            "GroveToken: Cannot update bridgeController to same address"
        );
        emit BridgeControllerChange(newAddress, bridgeController);
        bridgeController = newAddress;
        _isExcludedFromFee[bridgeController] = true;
        excludeFromDividends(bridgeController, true);
        _isAllowedToTradeWhenDisabled[bridgeController] = true;
        _isExcludedFromMaxTransactionLimit[bridgeController] = true;
        _isExcludedFromMaxWalletLimit[bridgeController] = true;
    }

    function claimETHOverflow(uint256 amount) external onlyOwner {
        require(
            amount < address(this).balance,
            "GroveToken: Cannot send more than contract balance"
        );
    ...
    function totalSupply() external view override returns (uint256) {
        return _tTotal + mintamount - burnamount;
    }
        ...
        returns (uint256)
    {
        require(
            rAmount <= _rTotal,
            "GroveToken: Amount must be less than total reflections"
        );
        ...
        returns (uint256)
    {
        require(
            tAmount <= _tTotal,
            "GroveToken: Amount must be less than supply"
        );
        uint256 currentRate = _getRate();
    ...
    function _transfer(
        ...
        require(
            ...
            "GroveToken: Cannot transfer more than balance"
        );
        ...
        if (
            ...
            require(
                isTradingEnabled,
                "GroveToken: Trading is currently disabled."
            );
            if (
                ...
                require(
                    amount <= maxTxAmount,
                    "GroveToken: Buy amount exceeds the maxTxBuyAmount."
                );
            }
            if (!_isExcludedFromMaxWalletLimit[to]) {
                require(
                    (balanceOf(to) + amount) <= maxWalletAmount,
                    "GroveToken: Expected wallet amount exceeds the maxWalletAmount."
                );
            }
        }
    ...
    function _tokenTransfer(
        ...
        (
            ...
            uint256 rOther,
            uint256 rBurn
        ) = _getRValues(tAmount, tFee, tOther, tBurn, _getRate());
        ...
        )
    {
        if (!takeFee) {
            ...
            uint256 tOther = (tAmount *
                (_liquidityFee + _marketingFee + _buyBackFee)) / 100;
    ...
    function _adjustTaxes(
        ...
        if (
            !isSelltoLp &&
            !isBuyFromLp &&
            (_feeOnSelectedWalletTransfers[from] ||
                _feeOnSelectedWalletTransfers[to])
        ) {
            _liquidityFee = _base.liquidityFeeOnBuy;
            _marketingFee = _base.marketingFeeOnBuy;
            _buyBackFee = _base.buyBackFeeOnBuy;
    ...
    function _swapAndLiquify() private {
        ...
        uint256 totalEthFee = totalFeePrior -
            (liquidityFeePrior / 2) -
            (burnFeePrior + holdersFeePrior);
        uint256 amountETHLiquidity = (ETHBalanceAfterSwap * liquidityFeePrior) /
            totalEthFee /
            2;
        uint256 amountETHMarketing = (ETHBalanceAfterSwap * marketingFeePrior) /
            totalEthFee;
    ...
    function mint(uint256 _tokenAmount) external {
        require(
            msg.sender == bridgeController,
            "GroveToken: Only bridgeController can mint"
        );
        mintamount += _tokenAmount;
        emit Mint(_tokenAmount);
    }

    function burn(uint256 _tokenAmount) external {
        require(
            msg.sender == bridgeController,
            "GroveToken: Only bridgeController can burn"
        );
        burnamount += _tokenAmount;
        emit Burn(_tokenAmount);
    }
}