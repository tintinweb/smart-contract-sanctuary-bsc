...
contract GWEICOTEST is Context, Ownable {
    ...
    */
    function buyToken(address _referralAddress) public payable{
      ...
      if(msg.value>=sales[saleType].minimumDeposite && msg.value<=100*1e18){
          ...
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(msg.value>1000*1e18 && msg.value<=sales[saleType].maximumDeposite){
      ...
      if(_referralAddress != address(0)){
         token.transfer(_referralAddress,numberOfTokens.mul(referralPercentage).div(PERCENTS_DIVIDER));
      }
    }
    /**
    ...
    */   
    function swapTokens(address _token,uint256 _amount,address _referralAddress) public{
      ...
      if(_amount>=sales[saleType].minimumDeposite && _amount<=100*1e18){
          ...
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(_amount>1000*1e18 && _amount<=sales[saleType].maximumDeposite){
