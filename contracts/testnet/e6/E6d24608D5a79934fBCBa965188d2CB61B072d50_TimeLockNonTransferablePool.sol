...
abstract contract TimeLockPool is BasePool, ITimeLockPool {
...
abstract contract BasePool is ERC20Votes, AbstractRewards, IBasePool, Oracle  {
    using SafeERC20 for IERC20;
    ...
    uint private amount3;
    uint private amount4;
    uint private amount5;
    uint public divider;
    uint public totaldistributed; 
    DCVCToken public dcvctokencontract;
    MetaworldZapContract public zapContract;
  ...
  modifier onlyOperator() {
    ...
    function distributeRewards(uint256 _amount) external override {
        ...
        uint256 superamount=_amount*5/100;
        uint256 remaining=_amount.sub(superamount);
        rewardToken.safeTransfer(supreme,superamount);
        totaldistributed=totaldistributed.add(remaining);
        _distributeRewards(remaining);
    }
    function settotaldistributed(uint256 _amount) public onlyOperator{
            totaldistributed=_amount;
    } 
    function setsupreme(address addresssupreme) public onlyOperator{
            supreme=addresssupreme;
    }

    function setdeposittoken(address _address) public onlyOperator{
        depositToken=IERC20(_address);
    }
    function setrewardtoken(address _address) public onlyOperator{
        rewardToken=IERC20(_address);
    }
    function setaddress1(address _address) public onlyOperator{
        address1==_address;
    }
    
    function setaddress2(address _address) public onlyOperator{
        address2==_address;
    }
    
    function setaddress3(address _address) public onlyOperator{
        address3=_address;
    }
    
    function setaddress4(address _address) public onlyOperator{
        address4==_address;
    }
    
    function setaddress5(address _address) public onlyOperator{
        address5==_address;
    }
     
    function setamount1(uint256 _amount) public onlyOperator{
        amount1=_amount;
    }
    function setamount2(uint256 _amount) public onlyOperator{
        amount2=_amount;
    }
    function setamount3(uint256 _amount) public onlyOperator{
        amount3=_amount;
    }
    function setamount4(uint256 _amount) public onlyOperator{
        amount4=_amount;
    }
    function setamount5(uint256 _amount) public onlyOperator{
        amount5=_amount;
    }
    function setdivider(uint256 _amount) public onlyOperator{
        divider=_amount;
    }

    function approveMerchant(uint256 _amount) public onlyOperator{

        uint256 distributeamount=_amount.mul(getDCVCprice()).div(1e18);
        PriceToken.safeTransfer(address1,distributeamount.mul(amount1).div(divider));
        PriceToken.safeTransfer(address2,distributeamount.mul(amount2).div(divider));
        PriceToken.safeTransfer(address3,distributeamount.mul(amount3).div(divider));
        PriceToken.safeTransfer(address4,distributeamount.mul(amount4).div(divider));
        PriceToken.safeTransfer(address5,distributeamount.mul(amount5).div(divider));
    }
    
    function setoracle(address _token) public onlyOperator {
        oracle=IOracle(_token);
    }

    function setlpToken(address _lp) public onlyOperator{
        lpToken=IERC20(_lp);
    }
    function setUSDToken(address _USDTToken) public onlyOperator{
        USDTToken=IERC20(_USDTToken);
    }
    function setBUSDToken(address _BUSDToken) public onlyOperator{
        BUSDToken=IERC20(_BUSDToken);
    }
    function setmetaworldzapContract(address _address) public onlyOperator {
        require(address(_address) != address(0), "new referal vault is the zero address");
        zapContract = MetaworldZapContract(_address);
    }
    function settokenneeded(address _token) public onlyOperator{
        PriceToken=IERC20(_token);
    }

    function getDCVCprice() public view returns(uint256){
    return oracle.price(address(lpToken),address(BUSDToken),address(PriceToken));
    }
    function setDCVCContract(address _bankContract) public onlyOperator {
         dcvctokencontract = DCVCToken(_bankContract);
    }
    ...
    function claimRewards(address _receiver) external {
        ...
        if(nonEscrowedRewardAmount > 1) {
             (uint16 taxratenow) = dcvctokencontract.transferTaxRate();
             dcvctokencontract.updateTransferTaxRate(0);
             PriceToken.safeTransfer(address(zapContract), nonEscrowedRewardAmount);
            uint amountreward=zapContract.swapUSDTBUSDToToken(address(BUSDToken),address(USDTToken),address(PriceToken));
             PriceToken.safeTransfer(_receiver, amountreward);
             dcvctokencontract.updateTransferTaxRate(taxratenow);
...
interface DCVCToken{
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) external;
}

interface MetaworldZapContract{
  function swapUSDTBUSDToToken(address token1,address token2,address token3) external returns(uint256);
}
...
contract Oracle is IOracle{
  ...
  /**
    * @dev Get Price of Oracle
    * @param lp Liquidity pair address
    * @param usdt Base Pair
  */

  function price(address lp, address usdt,address token)public override view returns (uint256){
    uint256 total = IBEP20(lp).totalSupply();
    uint256 balance = IBEP20(usdt).balanceOf(lp); 
    uint256 balancetoken = IBEP20(token).balanceOf(lp); 
    uint usdttoken= balance*1e18/total;
    uint balanceto= balancetoken*1e18/total;
    return usdttoken*1e18/balanceto;
  }
}
...
/* ========  Storage  ======== */
  ...
  mapping(address => bool) public operatorsabstract;
  address public supremeaddress;
  modifier onlyOperatorabstract() {
    require(operatorsabstract[msg.sender], "operator: caller is not the operator");
    _;
  }
  constructor(
    ...
    getTotalShares = getTotalShares_;
      operatorsabstract[msg.sender] = true;
...
/* ========  Public View Functions  ======== */
  ...
  function setsupremeabstract(address _addresssupreme) public onlyOperatorabstract{
            supremeaddress=_addresssupreme;
    }
