...
contract ve is IERC721, IERC721Metadata, Ownable {
    ...
    uint public constant MAXTIME = 42 weeks;
    ...
    string constant public name = "veDCF";
    string constant public symbol = "veDCF";
    string constant public version = "1.0.0";
    uint8 constant public decimals = 18;

    // NFT
    address public darkNFT;
    mapping(address => uint256) private _depositedNFT;
    mapping(uint256 => uint256) private _idOfVeNFT;
    ...
    // NFT
    function setDarkNFT(address _darkNFT) external onlyOwner {
        require(_darkNFT != address(0), "zero");
        darkNFT = _darkNFT;
    }
    ...
    function depositNFT(uint256 _tokenId) external nonreentrant {
        require(IERC721(darkNFT).ownerOf(_tokenId) == msg.sender, "user does not have specified NFT");
        require(_depositedNFT[msg.sender] == 0, "unstake first");
        IERC721(darkNFT).transferFrom(msg.sender, address(this), _tokenId);
        _depositedNFT[msg.sender] = _tokenId;
    }

    // Withdrawing of NFT
    function withdrawNFT(uint256 _tokenId) external nonreentrant {
        require(_depositedNFT[msg.sender] == _tokenId, "invalid tokenID");
        IERC721(darkNFT).transferFrom(address(this), msg.sender, _tokenId);
    ...
    // get DarkNFT id
    function getIdOfVeNFT(uint256 _tokenVeId) external view returns (uint256) {
        return _idOfVeNFT[_tokenVeId];
    }

    function getBoostedAmount(uint256 amount, uint256 _tokenVeId) external view returns (uint256) {
        uint256 _level = IDarkNFT(darkNFT).getTokenLevel(_idOfVeNFT[_tokenVeId]);
        return amount * boostedPercent[_level - 1] / 10000;
    }
    ...
    function allTokensOfOwner(address _owner) external view returns (uint[] memory _tokenIds) {
        uint _length = ownerToNFTokenCount[_owner];
        _tokenIds = new uint[](_length);
        for (uint _tokenIndex = 0; _tokenIndex < _length; _tokenIndex++) {
    ...
    function abstain(uint _tokenId) external onlyVoter {
    ...
    function _create_lock(uint _value, uint _lock_duration, address _to) internal returns (uint) {
        ...
        _mint(_to, _tokenId);
        _idOfVeNFT[_tokenId] = _depositedNFT[_to];
    ...
    function create_lock_for(uint _value, uint _lock_duration, address _to) external nonreentrant returns (uint) {
        require(_depositedNFT[_to] != 0, "stake first");
    ...
    function emergencyWithdraw(uint _tokenId) external nonreentrant {
        _withdraw(_tokenId, true);
    }

    function _withdraw(uint _tokenId, bool _isEmergency) internal {
        assert(_isApprovedOrOwner(msg.sender, _tokenId));
        require(attachments[_tokenId] == 0 && !voted[_tokenId], "attached");
    ...
    function _balanceOfNFT(uint _tokenId, uint _t) internal view returns (uint) {
        ...
        if (_epoch == 0) {
            ...
            return this.getBoostedAmount(uint(int256(last_point.bias)), _tokenId);
        }
    }
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', toString(_tokenId), '", "description": "Firebird Aggregator locks, can be used to vote on token emission, and receive sharing profits", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
