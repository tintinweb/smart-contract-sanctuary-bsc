...
contract MultiSend {
    ...
    function multiSendDiffEth(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            newSumAfterFee[i] = amounts[i] - amounts[i].mul(percent).div(10000);  // 100 - (100 * 10 / 10000) = 99.9
            taxes += amounts[i].div(10000).mul(percent);  // (100 * 10 / 10000) = 0.1
    ...
    function multiSendDiffToken(
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            newSumAfterFee[i] = amounts[i] - amounts[i].mul(percent).div(10000);  // 100 - (100 * 10 / 10000) = 99.9
            taxes += amounts[i].div(10000).mul(percent);  // (100 * 10 / 10000) = 0.1
            ERC20(token).safeTransferFrom(msg.sender, recipients[i], newSumAfterFee[i]);
        }
