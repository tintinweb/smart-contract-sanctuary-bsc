...
contract FixedPeriodPool is SafeOwnable, ReentrancyGuard {
    ...
    function withdraw(uint256 depositId) external nonReentrant {
        ...
        uint256 totalStaked = totalUserStaked[msg.sender];
        totalUserStaked[msg.sender] = totalStaked - userDeposit.stakeShares;
     ...
     * @param lockDuration Number of blocks the deposit should be locked for
     */
    function redeposit(uint256 depositId, uint32 lockDuration) external nonReentrant {
    ...
    function closeDeposits(DepositPath[] calldata depositPaths) external nonReentrant {
        ...
        for (uint i = 0; i < depositPaths.length; i++) {
            if (deposits[depositPaths[i].account].length <= depositPaths[i].depositId) {
                revert InvalidPath(i, depositPaths[i].account, depositPaths[i].depositId);
            }
            DepositInfo memory userDeposit = deposits[depositPaths[i].account][depositPaths[i].depositId];
            if (!userDeposit.active) {
                revert NotActive(i, depositPaths[i].account, depositPaths[i].depositId);
            }

            deposits[depositPaths[i].account][depositPaths[i].depositId].active = false;
            if (userDeposit.unlockBlock + freeWithdrawalPeriod > block.number) {
                revert NotUnlocked(i, depositPaths[i].account, depositPaths[i].depositId);
            }

            uint256 stakeAmount = userDeposit.stakeShares * stakePPS();

            uint256 pendingRewards = userDeposit.pendingRewards + userDeposit.rewardShares
                * (_accTokenPerShare - userDeposit.lastUpdateAccTokenPerShare) / PRECISION_FACTOR;

            uint256 totalStaked = totalUserStaked[depositPaths[i].account];
            totalUserStaked[depositPaths[i].account] = totalStaked - userDeposit.stakeShares;
            stakeTotalShares -= userDeposit.stakeShares;
            rewardTotalShares -= userDeposit.rewardShares;

            uint256 eliminationPeriodStart = userDeposit.unlockBlock + freeWithdrawalPeriod;
            uint256 _eliminationPeriodDuration = eliminationPeriodDuration;
            uint256 rewardForDepositClosing = block.number > (eliminationPeriodStart + _eliminationPeriodDuration)
                ? pendingRewards
                : pendingRewards * (block.number - eliminationPeriodStart) / _eliminationPeriodDuration;
            ...
            IRewardMinter(address(rewardToken)).mint(depositPaths[i].account, pendingRewards - rewardForDepositClosing);

            emit Withdrawal(
                depositPaths[i].account,
                uint32(depositPaths[i].depositId),
                stakeAmount,
                userDeposit.stakeShares,
    ...
    returns (
        ...
        for(uint i = 0; i < deposits[account].length; i++) {
            depositsData[index] = _getDepositData(
                account,
                i,
                _stakePPS,
                newAccTokenPerShare,
            ...
            totalStakeShares += depositsData[index].stakeShares;
            totalRewardShares += depositsData[index].rewardShares;
            totalPendingRewards += depositsData[index].pendingRewards;
            totalRewardsPerDay += depositsData[index].rewardsPerDay;
            totalTreatTokensLocked += depositsData[index].locked;
            index++;
        }
    ...
    /*
     * @notice Estimates rewards per day, that user will receive for deposit with specific values
     * @param amount Amount to deposit (in stakedToken)
     * @param lockDuration Number of blocks the deposit should be locked for
     * @return Expected rewards per day for the deposit
     * @dev For FrontEnd use
     */
    function estimateRewardsPerDayForDeposit(
        uint256 amount,
        uint256 lockDuration
    ) external view returns(uint256) {
        uint256 expectedStakeShares = amount / stakePPS();
        uint256 expectedRewardShares = expectedStakeShares * _getRewardMultiplier(lockDuration) / 10_000;
        uint256 expectedRewardTotalShares = rewardTotalShares + expectedRewardShares;

        return expectedRewardShares * rewardPerBlock / expectedRewardTotalShares * 1200 * 24;
    }


    /*
     * @notice Estimates rewards per day, that user will receive for redepositing for specific duration
     * @param amount Amount to deposit (in stakedToken)
     * @param lockDuration Number of blocks the deposit should be locked for
     * @return Expected rewards per day for the deposit
     * @dev For FrontEnd use
     */
    function estimateRewardsPerDayForRedepositing(
        address account,
        uint256 depositId,
        uint256 lockDuration
    ) external view returns(uint256) {
        ...
        uint256 expectedRewardTotalShares = rewardTotalShares - userDeposit.rewardShares;
        uint256 expectedRewardShares = userDeposit.stakeShares
            * _getRewardMultiplier(lockDuration) / 10_000;
        expectedRewardTotalShares += expectedRewardShares;
    ...
    /*
     * @notice Estimates rewards per day that user will receive for replenishing or extending the deposit
     ...
     * @dev For FrontEnd use.
     */
    function estimateRewardsPerDayForReplenishAndExtendDeposit(
        address account,
        uint256 depositId,
        ...
        uint256 expectedRewardTotalShares = rewardTotalShares;
        if (additionalLockDuration > 0) {
            ...
            expectedRewardTotalShares += rewardSharesToAdd;
        ...
        if (additionalAmount > 0) {
            ...
            require(
                additionalAmount > PPS
                &&
                (maxDepositAmount == 0
                || totalUserStaked[account] * PPS + additionalAmount <= maxDepositAmount),
                "invalid amount"
            );
            uint256 lockDuration = userDeposit.unlockBlock - block.number;

            // calculate shares
            uint256 stakeSharesToAdd = additionalAmount / PPS;
        ...
        return userDeposit.rewardShares * rewardPerBlock / expectedRewardTotalShares * 1200 * 24;
    }


    /*
     * @notice Estimates rewards for closing expired deposits
     * @param depositPaths Array of deposit paths: Address of deposit owner + ID index of deposit
     * @return totalRewards Total amount of reward tokens that will be received for closing these deposits
     * @return individualRewards Array of individual reward amounts for closing specific deposits
     * @dev For FrontEnd use. getUserInfo function will provide much more info, so use it when you need it
     */
    function estimateRewardForClosingDeposits(DepositPath[] calldata depositPaths)
    external view returns(uint256 totalRewards, uint256[] memory individualRewards) {
        totalRewards = 0;
        individualRewards = new uint256[](depositPaths.length);
        ...
        for (uint i = 0; i < depositPaths.length; i++) {
            ...
            totalRewards += rewardForDepositClosing;
            individualRewards[i] = rewardForDepositClosing;
        }
    }
     ...
     * @return depositId Deposit id
     * @return stakeAmount Stake amount to withdraw
     * @return _vestingDuration Vesting duration for the deposit
     * @return totalStaked Total user stake shares
     * @return totalStaked Total user stake shares
     */
    function _executeWithdrawal(
        ...
        if (stakeTreatToken) {
            ...
            uint256 lockedToTransfer = locked * deposits[account][depositId].stakeShares
                / totalStaked;
            tokenVesting[account][depositId].amount = locked - lockedToTransfer;
