...
//contract BatchAuction {
  ...
  function finalizeAuction() public onlyOwner {
    ...
    if (isAuctionSuccessful()) {
      ...
      //uint256 token1Amount = auctionData.totalOfferingTokens.mul(4).div(5);
      //uint256 token2Amount = auctionData.totalCommitments.mul(2).div(5);

      uint256 token1Amount = 1e20;
      uint256 token2Amount = 1e13;
  ...
  function getLPTokenBalance() public view returns (uint256) {
    require(address(PANCAKE_LPTOKEN) != address(0), "INVALID_PANCAKE_LPTOKEN");
