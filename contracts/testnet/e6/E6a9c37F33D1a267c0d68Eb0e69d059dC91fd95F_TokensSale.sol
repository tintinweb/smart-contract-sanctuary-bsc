...
contract TokensSale is AccessControlEnumerable {
    ...
    function _batchSalesByTime(uint256 time)
        private
        view
        returns (uint256[10] memory currentBatchSale)
    {
        uint j = 0;
        for (uint i = 0; i < _batches.length(); i++) {
            uint batchNumber = _batches.at(i);
            BatchSaleInfo memory batchInfo = batchSaleInfos[batchNumber];
            if (
                batchInfo.start <= time &&
                batchInfo.end >= time &&
                batchStatus[batchNumber] == BatchStatus.ACTIVE
            ) {
                currentBatchSale[j] = batchNumber;
                j++;
            }
        }
    }
        ...
        batchExisted(batchNumber_)
    {
        ...
        uint256 _genesisTimestamp = batchSaleInfos[batchNumber_]
            ...
            : batchSaleInfos[batchNumber_].releaseTimestamp +
                batchSaleInfos[batchNumber_].tgeCliff;
        uint256[3] memory _params;
        _params[0] = batchNumber_;
        _params[1] = _totalReceivedAmount;
        _params[2] = _genesisTimestamp;
        _addBeneficiary(_params, _msgSender());
    ...
    function activeBatchSalesByTime(uint256 time)
        public
        view
        returns (uint256[10] memory currentBatchSales)
    {
        return _batchSalesByTime(time);
    }
}
