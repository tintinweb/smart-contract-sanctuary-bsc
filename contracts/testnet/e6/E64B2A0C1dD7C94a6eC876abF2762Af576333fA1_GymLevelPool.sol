...
contract GymLevelPool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
     ...
     *         level 14 pool will also include wallets from level 15 and 16
     *         level 15 pool will include wallets from level 16
     ...
     *         level 15 pool will contain only candidates qualified to level 15
     *         same for level 16
     * @return QualifiedCandidates struct with total candidates per each pool
     */
    ...
    function getPendingRewardsTotal() external view returns (uint256) {
        return address(this).balance - totalUnclaimed;
    }
