...
contract IDOFactory is OwnableUpgradeable {
    ...
    function test(uint i) public view returns(address  tokens, string memory names, uint[8] memory infos) {
            (address token, string memory name, uint mode, uint buyAmount, uint target, uint price, uint privateTime, uint publicTime, uint endTime, uint releaseTime) = idoInfo(getIdoList(2)[i]);
            tokens = token;
            names = name;
