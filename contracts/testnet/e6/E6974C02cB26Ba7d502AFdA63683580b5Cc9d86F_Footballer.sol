...
// File: contracts/FootBallerNFT.sol
...
contract Footballer is ERC721, Ownable {
    ...
    function getNFTInformation(uint256 nftID) external view returns (uint256 Attribute, uint256 Energy, uint256 lastestUpdate) {
        ...
        if (characters[nftID].lastestUpdate + 8 hours > block.timestamp) { // 0 Energy
          ...
          } else if (characters[nftID].lastestUpdate + 24 hours < block.timestamp) { // 3 Energy
        ...
        if (Energy == 2) {
            ...
            if (characters[nftID].lastestUpdate + 8 hours > block.timestamp) {

                lastestUpdate = 8 hours + characters[nftID].lastestUpdate - block.timestamp;

            } else if ((characters[nftID].lastestUpdate + 8 hours < block.timestamp) && (characters[nftID].lastestUpdate + 16 hours > block.timestamp)) {
            ...
            if ((characters[nftID].lastestUpdate + 8 hours < block.timestamp) && (characters[nftID].lastestUpdate + 16 hours > block.timestamp)) {
