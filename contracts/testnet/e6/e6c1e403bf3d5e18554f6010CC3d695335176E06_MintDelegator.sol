...
contract MintDelegator is Member, SignHelper, OwnableOperatorRole {
    function mint(
        address to_,
        uint256 id_,
        uint256 tax_,
        ...
        uint256 _nonceValue = _nonce.getNonce(address(this), to_);
        string memory _signMessage = prepareMessageTwo(
            address(this),
            to_,
            id_,
            _nonceValue,
        ...
        _nonce.setNonce(address(this), to_, _nonceValue + 1);

        IAgoraAsset asset = IAgoraAsset(getMember("asset"));
        asset.mint(to_, id_, tax_);
    }
}
