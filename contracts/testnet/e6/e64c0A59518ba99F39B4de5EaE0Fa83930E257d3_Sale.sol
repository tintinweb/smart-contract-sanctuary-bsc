...
contract Sale is Ownable, ReentrancyGuard {
    ...
    // max size for whitelist buy private sale
    uint256 private maxSizeWhitelist = 300;
    ...
    function buyTokens(uint256 _amount) external nonReentrant hasSaleRunning {
        require(isWhitelist[msg.sender], "Not be in whitelist!");
    ...
    function setWhitelist(address[] memory listAddress) external onlyOwner {
        require(listAddress.length > 0 && listAddress.length <= maxSizeWhitelist, "Invalid whitelist");
        for (uint256 i; i < listAddress.length; i++){
            isWhitelist[listAddress[i]] = true;
        }
    }
