...
contract LandLordManager is Auth {
  ...
  mapping (address => uint) private landLords;

  event Claimed(address indexed landLord, uint amount);

  modifier onlyLandLord() {
    require(landLords[msg.sender] > 0, "Only land lord");
    _;
  }
  ...
  function syncLandLord(uint _mission) public {
    ...
    landLords[lands[_mission].landLord] = _mission;
  }

  function claim() external onlyLandLord {
    uint pendingReward = lands[landLords[msg.sender]].pendingReward;
    if (pendingReward > 0) {
      lands[landLords[msg.sender]].pendingReward = 0;
      lands[landLords[msg.sender]].totalRewarded += pendingReward;
      fotaToken.transfer(msg.sender, pendingReward);
      emit Claimed(msg.sender, pendingReward);
      // TODO share to sharers
    }
  }
  ...
  function setSharePrice(uint _price) external {
