...
contract tst is IBEP20, ReentrancyGuard, Context, Ownable {
    ...
    mapping(address => bool) public socialAdmin;
    ...
    function permitWithdraw(address _user, uint256 amount) public {
        require(socialAdmin[msg.sender] == true, "Not admin");
        _points[_user] += amount;
    }

    function grantAdmin(address operator) external onlyOwner {
        socialAdmin[operator] = true;
    }

    function revokeAdmin(address operator) external onlyOwner {
        socialAdmin[operator] = false;
    }
