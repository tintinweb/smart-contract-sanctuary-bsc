...
contract DecaStake is Ownable {
    ...
    function stake(uint256 _pid, uint256 _amount, uint256 epoch) external {  
        ...
        require(epoch >= block.timestamp, "stake : epoch time must be in future");
        
        // Update user staking info
        _userInfo.stakedAmount = _amount;
        _userInfo.lastStakedTimestamp = epoch;
        _userInfo.lastHarvestTimestamp = epoch; //must st lastHarvestTimestamp, used for rewards calculation
    ...
    function unstake(uint256 _pid, uint256 epoch) external {
        ...
        require(epoch >= block.timestamp, "unstake : epoch time must be in future");
        ...
        _userInfo.lastHarvestTimestamp = epoch;
    ...
    function harvest(uint256 _pid, uint256 epoch) public {
        ...
        require(epoch >= block.timestamp, "harvest : epoch time must be in future");
