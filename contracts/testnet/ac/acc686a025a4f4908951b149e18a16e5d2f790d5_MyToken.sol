...
contract MyToken is IERC20, Ownable {
  ...
  uint256 private numTokensSellToAddToLiquidity = 8 * 10**(_decimals-1);
  ...
  uint256 public _startTimeForSwap = 0;
  ...
  function _transfer(address from, address to, uint256 amount) private {
    ...
    if (_isExcludedFromFee[from] || _isExcludedFromFee[to]
      //todo:生产系统测试这样是不是撤池不扣费
      || (from == uniswapV2Pair && to == routerAddress)
      ) {
        takeFee = false;
    }
    ...
    //add as LP if from/to is, remove if not any more    
    if (!_isExcludedFromReward[from] && from != uniswapV2Pair) setInviter(from);
    if (!_isExcludedFromReward[to] && to != uniswapV2Pair) setInviter(to);
  }
  ...
  function swapTokensForEth(uint256 tokenAmount) private {
    ...
    function setInviter(address shareholder) private {
        if (_updated[shareholder]){
            if (IERC20(uniswapV2Pair).balanceOf(shareholder) == 0) quitInviter(shareholder);
            return;
        }
    ...
    function quitInviter(address shareholder) private {
