    ...
    PausableUpgradeable
{
    ...
    function unstaking(uint256 _tokenId) public {
        ...
        totalBagSize -= getLockedAmountByTokenId(_tokenId);
    ...
    function getStakingStatusByAddress(address _address)
        public
        view
        returns (Stake memory stakeInfo, string memory tokenUri)
    {
        uint256[] memory tokenIds = walletOfOwner(_address);
    ...
    function getBagWeight(address _address) private view returns (uint256) {
        if (balanceOf(_address) == 0 || getLockedAmountByAddress(_address) == 0)
            return 0;
        return (getLockedAmountByAddress(_address) * (10**18)) / totalBagSize;
    }

    function getTimeWeight(address _address) private view returns (uint256) {
        if (balanceOf(_address) == 0 || getLockedAmountByAddress(_address) == 0)
            return 0;
