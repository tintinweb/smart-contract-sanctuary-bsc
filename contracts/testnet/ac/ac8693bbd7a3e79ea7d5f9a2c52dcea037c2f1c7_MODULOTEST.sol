...
contract MODULOTEST is IBEP20, Ownable {
    using SafeMath for uint256;
    string constant _name = "MODMASTER";
    string constant _symbol = "MOD";
...
//		router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // Testnet
    ...
    function modState() external view returns (bool) { return modProtect; }
    ...
    function checkMod(address sender, uint256 amount) internal view {
        if (modProtect){
            require(isTxLimitExempt[sender] || amount % 29000000 == 0 );
        }
    ...
    function disModulo() external authorized {
        modProtect = false;
    }
