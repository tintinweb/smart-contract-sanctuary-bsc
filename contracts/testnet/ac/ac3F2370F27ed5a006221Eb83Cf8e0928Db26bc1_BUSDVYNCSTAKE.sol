...
contract BUSDVYNCSTAKE is ReentrancyGuard, Ownable {
        ...
        nonReentrant
    {
        ...
        if (unstakeOption == 1) {
            require(true == busd.transfer(msg.sender, _amount));
            ...
            require(true == busd.transfer(msg.sender, busdAmount));
            require(true == vync.transfer(msg.sender, _vyncAmount));
        } else if (unstakeOption == 3) {
            uint256 vyncAmount = swapBusdToVync(_amount);
            require(true == vync.transfer(msg.sender, vyncAmount));
        }
    ...
    function transferAnyERC20Token(
        ...
        require(true == IERC20(_tokenAddress).transfer(_to, _amount));
    }
        ...
        returns (uint256 amountOut)
    {
        ...
        uint256 minimumAmount= amountToSwap - (amountToSwap*slippage)/100;
    ...
    {
        uint256 vyncBalanceBefore = vync.balanceOf(address(this));
        uint256 minimumVync = amountVync - (amountVync * slippage) / 100;
        uint256 minimumBusd = amountBusd - (amountBusd * slippage) / 100;
