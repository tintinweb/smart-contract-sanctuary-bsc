...
Santa is in his Chad form this Yuletide season. He comes bearing gifts as always but this time he extends his generous hands beyond the Christmas and presents up to 7% in FlokiNY; the one true New Year token.
...
✅ Clean CA
✅ Strong Community 🫡
✅ Massive Shilling / Raid
🚀 FlokiNY Whales Watching
...
Max Buy -2%(20,000)
Max sell -0.5%(5,000)
Telegram: https://t.me/+EzAQN4-p1LlmZmNk
...
contract SantaChad is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    SantaChadDividendTracker public dividendTracker;
    ...
    // Anti Dump //
    uint256 private maxSellAmountPerCycle = 5000 * 10**18;
    uint256 private antiDumpCycle = 10 minutes;
    ...
    // staking variables
    mapping(address => uint256) private stakingBonus;
    mapping(address => uint256) private stakingUntilDate;
    mapping(uint256 => uint256) private stakingAmounts;
    ...
    constructor() ERC20("SantaChad", "SanChad") {
        marketingWallet = payable(0x1bAdCFF19078FD2516FbBd21277178C8d0C1a3fd);
        devWallet = payable(0xCcF1C2D07Cc65006A661DAd7F6ebD1f23fE9B9a4);
        address router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // testnet router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 
        ...
        dividendTracker = new SantaChadDividendTracker(
            payable(this),
            router,
        ...
        // FlokiNY: 0x4A01D24Aa894530f83Fa3764c584Fc75885762B4
        ...
        _isExcludedMaxTransactionAmount[address(marketingWallet)] = true;
        _isExcludedMaxTransactionAmount[address(router)] = true;
        _isExcludedMaxTransactionAmount[uniswapV2Pair] = true;
        _isExcludedMaxTransactionAmount[address(dividendTracker)] = true;
        _isExcludedMaxTransactionAmount[address(devWallet)] = true;
        _isExcludedMaxTransactionAmount[msg.sender] = true;

        // exclud from maxWallet
        ...
        _isExcludedFromMaxWalletLimit[address(devWallet)] = true;
        ...
        maxTransactionAmount = totalTokenSupply / 50; // 2%
        swapTokensAtAmount = totalTokenSupply / 200; // 0.5%
    ...
    // use for presale wallet, adds all exclusions to it
    ...
    // set max wallet, can not be lower than 0.01% of supply & higher than 5%
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 10000, "max wallet cannot be set to less than 0.01%");
        maxWallet = value;
    }

    // set max Tx, can not be lower than 0.005% of supply & higher than 2.5%
    function setmaxTX(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 20000, "max TX cannot be set to less than 0.005%");
    ...
    // set ANTI-DUMP Function
     function updateAntiDump(uint256 _maxSellAmountPerCycle, uint256 timeInMinutes) external onlyOwner{
        require(_maxSellAmountPerCycle >= 100, "Amount must be >= 100 Tokens");
        antiDumpCycle = timeInMinutes * 1 minutes;
        maxSellAmountPerCycle = _maxSellAmountPerCycle * 10**18;
    }

    function updateTransferFee(uint256 newTransferFee) public onlyOwner {
        require (newTransferFee <= 40, "transfer fee cannot exceed 40%");
        transferFee = newTransferFee;
        emit UpdateTransferFee(transferFee);
    }

    function updateBuyFees(uint256 _marketingFee, uint256 _rewardFee, uint256 _liquidityFee, uint256 _deadFee, uint256 _devFee) external onlyOwner {
        buyMarketingFee = _marketingFee;
        buyRewardsFee = _rewardFee;
        buyLiquidityFee = _liquidityFee;
        buyDeadFee = _deadFee;
        buyDevFee = _devFee;
        totalBuyFees = buyMarketingFee + buyRewardsFee + buyLiquidityFee + buyDeadFee + buyDevFee;
        require(totalBuyFees <= 30, "total buyfees cannot exceed 30% buy");
    }

    function updateSellFees(uint256 _marketingFee, uint256 _rewardFee, uint256 _liquidityFee, uint256 _deadFee, uint256 _devFee) external onlyOwner {
        sellMarketingFee = _marketingFee;
        sellRewardsFee = _rewardFee;
        sellLiquidityFee = _liquidityFee;
        sellDeadFee = _deadFee;
        sellDevFee = _devFee;
        totalSellFees = sellMarketingFee + sellRewardsFee + sellLiquidityFee + sellDeadFee + sellDevFee;
        require(totalSellFees <= 30, "total sellfees cannot exceed 30% sell");
    }
    ...
    function _transfer(
        ...
        require(!bots[from] && !bots[to], "You are a bot, so you cant sell.");
        uint256 RewardsFee;
        uint256 deadFees;
        ...
        if (amount == 0) {
            ...
            else if (isSelling && !_isExcludedMaxTransactionAmount[from]) {
                RewardsFee = sellRewardsFee;
                deadFees = sellDeadFee;
                marketingFees = sellMarketingFee;
            ...
            } else if (isBuying && !_isExcludedMaxTransactionAmount[to]) {
                RewardsFee = buyRewardsFee;
                deadFees = buyDeadFee;
                marketingFees = buyMarketingFee;
                liquidityFee = buyLiquidityFee;
                devFees = buyDevFee;

                if (limitsInEffect) {
                require(block.number > launchblock,"you shall not pass");
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (buyRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(buyRewardsFee)
            .div(buyRewardsFee + buyMarketingFee + buyDevFee);
        }
        if (sellRewardsFee > 0) {
            ...
            .div(totalAmount)
            .mul(sellRewardsFee)
            .div(sellRewardsFee + sellMarketingFee + sellDevFee);
        }
        ...
        uint256 _completeFees = sellMarketingFee.add(sellDevFee) +
            buyMarketingFee.add(buyDevFee);
...
contract SantaChadDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    SantaChad public SantaChadContract;
    ...
    constructor(
        ...
        defaultToken = token;
        SantaChadContract = SantaChad(mainContract);
    ...
    function _transfer(
        ...
        require(false, "SantaChad_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "SantaChad_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SantaChad contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(SantaChadContract);

            uint256 prevBalance = SantaChadContract.balanceOf(address(this));
            ...
            {
                uint256 received = SantaChadContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    SantaChadContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
