...
contract NextEarlyAdaptersSale is ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public minPurchaseLimit = 1 ether;
    ...
    function updateParticipant(address participant, uint256 amount) internal {
        ...
        require(
            (totalAmount * 0.06 ether) / 10**18 <= maxPurchaseLimit,
        ...
        whenInSale
    {
        ...
        Participant memory particpant = addressToParticipant[user];

        // Require for whitelist state
        require(
            whitelist[user],
            "Only whitelisted wallets can use this page. Please connect with your whitelisted wallet."
        );

        // Checks on only first purchase
        if (particpant.vestedAmount == 0)
            require(
        ...
        // Transfer payment token (BUSD) to fund address
    ...
    function getInfo(address user) public view returns (Info[] memory) {
        ...
        //todo: p yok ise require ile no alloc mesage dön
        Participant storage p = addressToParticipant[user];

        require(p.vestedAmount != 0, "has no allocation");
