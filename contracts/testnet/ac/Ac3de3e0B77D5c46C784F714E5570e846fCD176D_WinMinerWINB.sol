...
contract WinMinerWINB {
    ...
    // Staking token
	address erctoken = 0xD7C1b1bE2e0D6568F152a1B8BA40BF7CCfD3EC4f; /** WINB Testnet **/
    // address erctoken = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; /** WINB Mainnet **/
    
    /// WINB contract
    IBEP20 winb = IBEP20(0xD7C1b1bE2e0D6568F152a1B8BA40BF7CCfD3EC4f);
    ...
    function sellQubic() public{
        ...
        token_WINB.transfer(msg.sender, qubicPayout);
    ...
    function sellAutoQubic(address adr) internal {
        ...
        token_WINB.transfer(adr, qubicPayout);
        user.totalWithdrawn = user.totalWithdrawn.add(qubicPayout);
        totalWithdrawn = totalWithdrawn.add(qubicPayout);
    }
    ...
    function payFees(uint256 qubicValue) internal returns(uint256){
        ...
        token_WINB.transfer(bbadr, qubicValue.mul(BBTAX).div(PERCENTS_DIVIDER));
        token_WINB.transfer(devadr, qubicValue.mul(DEVTAX).div(PERCENTS_DIVIDER));
        token_WINB.transfer(mktadr, qubicValue.mul(MKTTAX).div(PERCENTS_DIVIDER));
        return tax.mul(1);
    }

    function payAuto(uint256 qubicValue) internal returns(uint256){
        uint256 tax = qubicValue.mul(AUTOTAX).div(PERCENTS_DIVIDER);
         token_WINB.transfer(autoadr, qubicValue.mul(AUTOTAX).div(PERCENTS_DIVIDER));
         return tax.mul(1);
    }
