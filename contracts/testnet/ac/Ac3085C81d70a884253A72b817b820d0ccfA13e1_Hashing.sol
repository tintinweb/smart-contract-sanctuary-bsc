...
contract Hashing {
    ...
    function encode(uint256 timestamp, string memory input, address addr) public pure returns (string memory) {
        return string(abi.encodePacked(timestamp, input, addr));
    }

    function decode(string memory hash) public pure returns (uint256, string memory, address) {

        (uint timestamp, string memory input, address addr) = abi.decode(bytes(b64decode(hash)), (uint256, string, address));
