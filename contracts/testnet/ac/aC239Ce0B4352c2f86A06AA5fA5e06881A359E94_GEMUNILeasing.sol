...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing {
    ...
    function registerLeasing(LeasingRegister[] memory inputs, uint rentalPrice) external override {
        ...
        for( uint i = 0; i < inputs.length; i++) {
            ...
            totalRental += register(nftToken, tokenId, duration, minDuration, maxDuration, pricePerWeek, mortgageAmount);
        }
    ...
    function register(address nftToken, uint tokenId, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek, uint mortgageAmount) internal returns (uint rentalAmount){
        ...
        emit RegisterLeasing(nftToken, tokenId, duration, minDuration, maxDuration, pricePerWeek, mortgageAmount, rentalAmount);
    }
    ...
    function rentMore(address nftToken, uint tokenId, uint addedDuration, uint addedRentalPrice) external override {
        ...
        if (msg.sender == renter.renter && guild.gameGuild != address(0)) {
            ...
            require(
                ...
                renter.duration + addedDuration <= guild.maxDuration,
            ...
            if (guild.gameGuild != address(0)) {
                ...
                if (leasing.lender != address(0)) {
                    if (nftToken == geniPass) {
                        ...
                        checkTransferNFT(nftToken, tokenId, guild.mortgageAmount, msg.sender, guild.gameGuild);
                    }
    ...
    function checkTransferNFT(address nftToken, uint tokenId, uint amount, address lender, address renter) internal {
        ...
        RenterInfo memory renting = renterInfo[nftToken][tokenId];
        LeasingInfo memory leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo memory guild = leasingGuildInfo[nftToken][tokenId];

        try IERC721(nftToken).transferFrom(renter, address(this), tokenId) {
            if (msg.sender == leasing.lender && renting.renter != address(0) && guild.gameGuild != address(0)) {
    ...
    function guildRetrieveNFT(address nftToken, uint tokenId, RenterInfo storage renter, LeasingGuildInfo storage guild) internal {
        ...
        if (nftToken == geniPass) {
            ...
            checkTransferNFT(nftToken, tokenId, guild.mortgageAmount, renter.renter, guild.gameGuild);
        }
        delete renterInfo[nftToken][tokenId];
    }
...
interface IGEMUNILeasing {
    ...
    event RegisterLeasing(address nftToken, uint indexed tokenId, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek, uint mortgageAmount, uint rentalPrice);
