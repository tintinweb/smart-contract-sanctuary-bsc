...
contract Campaign{
    ...
    address public tokenAddress = 0xBf357322D671D168bA8b4600D427Db4726421A5a; // sample token 
    ...
    function getReplaceRequests() public view 
    returns(bool check, ReplaceRequest[] memory replaceRequestLists){
        require(!pause, "project is paused.");
        check = false;
        replaceRequestLists = replaceRequests;
        if (isFundingFinished() && isFundingSuccessful() && !projectComplete){
            if (replaceRequestLists.length > 0){
                for (uint256 i=0; i < replaceRequestLists.length; i++){
                    if (replaceRequestLists[i].active && !replaceRequestLists[i].replaced){
                        check = true;    
                    }
                }
            }
