...
/**
 * @title BNBBets_Prediction
 */
contract XXXXXX is Ownable, Pausable, ReentrancyGuard {
    ...
    address public devAddress; // address of the developer
    ...
    uint256 public treasuryFee = 900; // 9%
    uint256 public devFee = 100; // 1%
    uint256 public totalBet;

    uint256 public houseBetPot; // pot for the house bet
    uint256 public houseBetAmount = 10 ether; // house bet amount
    bool public enableHouseBet = true;

    uint256 public protectionCheckpoint;
    //uint256 public constant PROTECT_STEP = 10 days; // 10 day        mainnet 
    uint256 public constant PROTECT_STEP = 300;// 5 min                testnet

    //uint constant public TIME_STEP = 1 days;  //                     mainnet
    uint constant public TIME_STEP = 600;    //                         testnet

    uint256 public prizePool;
    uint256 public lastDistribute;
    uint256 public prizeCycle;
    //uint256 public giveawayPrize = 0.25 ether;                 // mainnet
    uint256 public giveawayPrize = 0.1 ether;                 // testnet
    //uint256 public giveawayStep = 100;                       // mainnet
    uint256 public giveawayStep = 4;                         // testnet
    //uint256 public dailyMinCount = 10;                       // mainnet
    uint256 public dailyMinCount = 2;                         // testnet
    //uint256[3] public dailyPrize = [3 ether, 2 ether, 1 ether];   //mainnet
    uint256[3] public dailyPrize = [0.03 ether, 0.02 ether, 0.01 ether];     //testnet
    ...
    event NewDailyPrize(uint256 indexed cycle, uint256 index, uint256 prize);
    ...
    modifier onlyDev() {
        require(msg.sender == devAddress, "Not developer");
        _;
    }
     ...
     */
    constructor() {
        oracle = AggregatorV3Interface(address(0xD276fCF34D54A926773c399eBAa772C12ec394aC));
        //operatorAddress = address(0x7bDCf26c9edB87b0682f2EB6827F2b82A02ef3FB);    //mainnet
        operatorAddress = address(0x04f61498aaf25A376ee44114Ad4CE2a22825317E);      //testnet
        //intervalSeconds = 600;     //mainnet
        intervalSeconds = 180;      // testnet
        bufferSeconds = 60;
        minBetAmount = 0.01 ether;
        oracleUpdateAllowance = 300;
        lastDistribute = block.timestamp;
        //devAddress = address(0x5bf42673dCe99bA4f3Fa2F4aE552ef79a65840eB);          //mainnet
        devAddress = address(0x04f61498aaf25A376ee44114Ad4CE2a22825317E);            //testnet
        //adminAddress = address(0xCc2F8C92E20866D7c7b555676e56e48ad00fAF78);        //mainnet
        adminAddress = address(0x04f61498aaf25A376ee44114Ad4CE2a22825317E);          //testnet
    }
    ...
    function betBear(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        uint256 devAmt = (amount * devFee) / 10000;
        _safeTransferBNB(adminAddress, treasuryAmt);
        _safeTransferBNB(devAddress, devAmt);

        // Update round data
        amount = amount - (treasuryAmt + devAmt);
        ...
        if(profileDay.betsCount >= dailyMinCount){
            _updateLeaderRank(msg.sender);
        }

        protectionCheckpoint = block.timestamp;
        emit BetBear(msg.sender, epoch, amount, (treasuryAmt + devAmt));
    ...
    function betBull(uint256 epoch) external payable whenNotPaused nonReentrant notContract {
        ...
        protectionCheckpoint = block.timestamp;
        emit BetBull(msg.sender, epoch, amount, (treasuryAmt + devAmt));
    ...
    function executeRound() external whenNotPaused onlyOperator {
        ...
        //(uint80 currentRoundId, int256 currentPrice) = _getPriceFromOracle(); //testnet - uncomment on mainnet
        uint256 currentRoundId = currentEpoch; // testnet - delete on mainnet

        int256 currentPrice = 202.83 * 1e8; // testnet - delete on mainnet
        uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.difficulty))) % 100; // testnet - delete on mainnet
        if(randomnumber > 50){ // testnet - delete on mainnet
            currentPrice += 1e8; // testnet - delete on mainnet
        }else{ // testnet - delete on mainnet
            currentPrice -= 1e8; // testnet - delete on mainnet
        } // testnet - delete on mainnet
        currentPrice += int256(randomnumber); // testnet - delete on mainnet
    ...
    function genesisLockRound() external whenNotPaused onlyOperator {
        ...
        require(!genesisLockOnce, "Can only run genesisLockRound once");
         ...
         // (uint80 currentRoundId, int256 currentPrice) = _getPriceFromOracle(); // testnet - should uncomment on mainnet

        uint256 currentRoundId = currentEpoch; // testnet - delete on mainnet

        int256 currentPrice = 202.83 * 1e8; // testnet - delete on mainnet
        uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, block.difficulty))) % 900; // testnet - delete on mainnet
        if(randomnumber > 50){ // testnet - delete on mainnet
            currentPrice += 1e8; // testnet - delete on mainnet
        }else{ // testnet - delete on mainnet
            currentPrice -= 1e8; // testnet - delete on mainnet
    ...
    function claimHousePot(uint256 _amount) external nonReentrant onlyAdmin {
        ...
        _safeTransferBNB(adminAddress, currentHousePot);
    ...
    function protectFromLock() external onlyAdmin {
        ...
        _safeTransferBNB(adminAddress, finalAmount);
        protectionCheckpoint = block.timestamp;
    ...
    function setMinBetAmount(uint256 _minBetAmount) external whenPaused onlyAdmin {
        require(_minBetAmount >= 0.01 ether && _minBetAmount <= 0.1 ether, "only between 0.01 ether to 0.1 ether");
     ...
     * @dev Callable by admin
     */
    function setDailyPrize(uint256 _index, uint256 _prizeAmount) external onlyAdmin {
        dailyPrize[_index] = _prizeAmount;
        emit NewDailyPrize(prizeCycle, _index, _prizeAmount);
    }
    ...
    function claimPrize() external nonReentrant notContract {
        ...
        require(claimableAmount >= prizePool, "Not enough BNB in prize pool");
        profile.claimable = 0;
        profile.totalClaimed += claimableAmount;
        prizePool -= claimableAmount;

        _safeTransferBNB(address(msg.sender), claimableAmount);
