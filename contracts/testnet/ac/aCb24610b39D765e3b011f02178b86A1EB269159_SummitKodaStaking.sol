...
contract SummitKodaStaking is Ownable, ReentrancyGuard {
  ...
  mapping(address => mapping(address => uint256)) claimedTokenAmount; // token => user => amount
  ...
  mapping(address => uint256) public tokenPerRatingPoint;
  ...
  mapping(uint256 => uint256) public kCounter;
  ...
  constructor(
    ...
    penalties[7889229] = 2000;
    penalties[15778458] = 3000;
    penalties[31556916] = 5000;
  }
  ...
  function setK(uint256 _lockFor, uint256 _k) external onlyOwner {
    k[_lockFor] = _k;
  }
  ...
  function sharePremium(address premiumToken, uint256 amount) external {
    ...
    tokenPerRatingPoint[premiumToken] += (amount * PREMIUM_PER_RATING_POINT_BASE) / totalRating;
    emit SharePremium(msg.sender, amount, tokenPerRatingPoint[premiumToken], totalRating);
    IERC20(premiumToken).safeTransferFrom(msg.sender, address(this), amount);
  }

  function claimPremium(address premiumToken) external {
    claimedTokenAmount[premiumToken][msg.sender] = _claimPremium(premiumToken);
  }

  function _claimPremium(address premiumToken) private returns (uint256) {
    UserAccount storage account = accounts[msg.sender];
    uint256 total = (account.rating * tokenPerRatingPoint[premiumToken]) / PREMIUM_PER_RATING_POINT_BASE;
    uint256 _claimedAmount = claimedTokenAmount[premiumToken][msg.sender];
  ...
  function premiumOf(address premiumToken, address user) external view returns (uint256) {
    ...
    uint256 total = (account.rating * tokenPerRatingPoint[premiumToken]) / PREMIUM_PER_RATING_POINT_BASE;
    uint256 _claimedAmount = claimedTokenAmount[premiumToken][user];
    if (total <= _claimedAmount) {
      return 0;
    }
    uint256 rest = total - _claimedAmount;
  ...
  function deposit(
    ...
    uint256 deltaRating = amount * k[lockFor];
    UserAccount storage account = accounts[beneficiary];
    uint256 _rating = account.rating;
    claimedTokenAmount[kodaToken][beneficiary] =
      ((_rating + deltaRating) * tokenPerRatingPoint[kodaToken]) /
      PREMIUM_PER_RATING_POINT_BASE;
    claimedTokenAmount[kapexToken][beneficiary] =
      ((_rating + deltaRating) * tokenPerRatingPoint[kapexToken]) /
      PREMIUM_PER_RATING_POINT_BASE;
  ...
  function withdrawDeposit(uint256 depositId, uint256 amount) external {
    ...
    uint256 deltaRating = amount * k[deposit.lockFor];
    UserAccount storage account = accounts[msg.sender];
    account.totalDepositAmount -= amount;
    uint256 _rating = account.rating;
    claimedTokenAmount[kodaToken][msg.sender] =
      ((_rating + deltaRating) * tokenPerRatingPoint[kodaToken]) /
      PREMIUM_PER_RATING_POINT_BASE;
    claimedTokenAmount[kapexToken][msg.sender] =
      ((_rating + deltaRating) * tokenPerRatingPoint[kapexToken]) /
      PREMIUM_PER_RATING_POINT_BASE;
    account.rating = (_rating - deltaRating);
    totalRating -= deltaRating;
