...
contract BNBTesting is Context, IERC20, Ownable {  
    ...
    string  private _name = "BNB Reward";
    string  private _symbol = "BNR";
    ...
    function RewrdDistri(uint256 contractTokenBalance) private  {
        ...
        if (isRewardTake)
        {
            SplitBNBBalance = Balance.div(_marketingPer.add(_RewardPer));
        }
        else
        {
            SplitBNBBalance = Balance.div(_marketingPer);
        }
            ...
            if (isRewardTake)
           {
                MarketingBNB=SplitBNBBalance*_marketingPer;
                emit comments ("RDMarketingBNB", MarketingBNB);
                RewardBNB=SplitBNBBalance*_RewardPer;
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        emit comments ("sETH" , tokenAmount);
