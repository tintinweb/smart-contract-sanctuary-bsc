...
contract Initializable {
    ...
    modifier ownerinitializer() {
        require(!ownerinited, "Owner already inited");
        _;
        ownerinited = true;
    }
}
...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder, Initializable {
     ...
     */
    function initialize(address _acceptedToken, uint256 _commissionFee) onlyOwner external initializer {
        require(_acceptedToken.isContract(),"The accepted token address must be a deployed contract");
        acceptedToken = IERC20(_acceptedToken);
        commissionFee = _commissionFee;

    }
    function setContractOwner() external ownerinitializer
    {
        address msgSender = _msgSender();
        _owner = _msgSender();
        emit OwnershipTransferred(address(0), msgSender);
