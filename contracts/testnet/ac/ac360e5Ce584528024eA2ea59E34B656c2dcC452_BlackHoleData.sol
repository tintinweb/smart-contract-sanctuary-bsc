...
contract BlackHoleData is Ownable {
    ...
    function addBondAddress(address _address) public onlyOwner {
    ...
    function getAllTeamInfo() public view returns (TeamInfo []memory){
        ...
        for (uint i = 0; i < teamAddress.length; i++) {
            ...
            infos[i].BHOAmount = support.tokenOfTeam(teamAddress[i]);
            infos[i].sBHOAmount = sBHO.gonsForBalance(support.bondOfTeam(teamAddress[i], sBHOAddress));
    ...
    function getUserHoldingInfo(address [] memory _users) public view returns (UserInfo [] memory){
        ...
        for (uint i = 0; i < _users.length; i++) {
            ...
            infos[i].sBHOAmount = sBHO.gonsForBalance(support.userStakingAmount(_users[i], sBHOAddress));
        }
        return infos;
    }
}