...
// File contracts/DAIDAIUpgradeableMarket.sol
...
contract DAIDAIUpgradeableMarket is OwnableUpgradeable, ReentrancyGuardUpgradeable, Signature {
  ...
  function _verifyCreatorFee(CreatorFee[] memory _fees, address _collection, uint256 _closeBefore, bytes memory _signature) internal view returns (bool) {
    ...
    uint len = _fees.length;
        ...
        return verifySignature(message, _signature, owner());
  }
  ...
  function acceptListing721(Listing721 memory _listing,
    ...
    require(_listing.globalNonce == globalNonce[msg.sender], "global nonce expired");
    require(_listing.listingNonce == listingNonce721[msg.sender][_listing.collection][_listing.tokenId], "listing nonce expired");
    //verification
  ...
  function acceptOffer721(Offer721 memory _offer,
    ...
    require(_offer.globalNonce == globalNonce[msg.sender], "global nonce expired");
    require(_offer.offerNonce == offerNonce721[msg.sender][_offer.collection][_offer.tokenId], "listing nonce expired");
    //verification
    bytes32 message = prefixed(keccak256(abi.encodePacked(
      _offer.buyer,
      _offer.collection,
