...
contract PreSaleFactory is Ownable {
    ...
    uint256 public deployFee = 0 ether;
    uint256 public minAirdropAmount = 1e4;
    uint256 public minVote = 100;
    ...
    * @param params IPresale.InitParams
    * @dev
    */
    function createSale(
        ...
        require(voteParams.minVote >= minVote, "Minimum vote amount is too low");

        uint256 decimals = IERC20Metadata(params.mainToken).decimals();
        require(voteParams.airdropAmount.div(10**decimals) >= minAirdropAmount, "Airdrop amount is too low");
        address newSaleAddress = Clones.clone(implementation);
        IPresale newSale = IPresale(newSaleAddress);
        newSale.initialize(
            params,
            voteParams,
            _stageTimes,
        ...
        _addSale(newSaleAddress, msg.sender, params.mainToken);
    ...
    */
    ...
    function setMinAirdropAmount(uint256 minAirdropAmount_) external onlyOwner {
        minAirdropAmount = minAirdropAmount_;
    }

    function setMinVote(uint256 minVote_) external onlyOwner {
        minVote = minVote_;
    }
