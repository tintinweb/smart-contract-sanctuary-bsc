...
contract PancakeFlashSwap{
    ...
    function startArbitrage(address _tokenBorrow, uint256 _amount) external {
        IERC20(BUSD).approve(PANCAKE_ROUTER, MAX_INT);
        IERC20(CAKE).approve(PANCAKE_ROUTER, MAX_INT);
        IERC20(USDT).approve(PANCAKE_ROUTER, MAX_INT);
        IERC20(CROX).approve(PANCAKE_ROUTER, MAX_INT);
        ...
        require(pair != address(0), "Pool is not found");
        ...
        // passing data as bytes so that "swap" function knows it is a flashloan ; this data can be anything
    ...
    function pancakeCall(
        ...
        // Calculate the amount to repay at the end ; i think the 997 to gives extra fees to the platform variable plus 1 fixed cost.
        ...
        // Place Trades
        uint256 trade1AcquiredCoin = placeTrade(BUSD, USDT, loanAmount);
        require(trade1AcquiredCoin > 0 , "first trade is failed");
        uint256 trade2AcquiredCoin = placeTrade(USDT, CAKE, trade1AcquiredCoin);
        require(trade2AcquiredCoin > 0 , "second trade is failed");
        uint256 trade3AcquiredCoin = placeTrade(CAKE, BUSD, trade2AcquiredCoin);
        require(trade3AcquiredCoin > 0 , "Third trade is failed");
