...
contract Info is OwnableUpgradeable{
    ...
    function userCenter(address player) external view returns(uint[10] memory info){
        ...
        (info[8],info[9]) = coutingCoin(player);
    }
    ...
    function compoundList(uint[] memory list1, uint[] memory list2, uint[] memory list3) internal pure returns(uint[] memory){
        uint[] memory list = new uint[](list1.length + list2.length + list3.length);
        ...
        for(uint i = 0; i < list3.length; i ++){
            list[list1.length + list2.length + i] = list3[i];
        }
        return list;
    }
    function getUserProfilePhoto(address addr_) public view returns(string[] memory) {
        ...
        uint[] memory list3 = checkCreation(market.getSellingList(1,addr_));
        // uint creationAmoun
        
        uint []memory bovineHeroPhoto = compoundList(list1,list2,list3);
