...
contract TokenSale is Initializable, ITokenSale {
        ...
        initializer
    {
        ...
        busd = IERC20D(0xD631C7CFc2b44ec1ef60fc9D2d61e5cCfC0C0E8b);
    }

    //TODO set require for claim on first vesting points
        ...
        external
    {
        ...
        (params.vestingPoints,) = admin.descendingSort(_vestingPoints);
    }

    // function getVestingPoints()
    //     external
    //     view
    //     returns (uint128[2][] memory)
    // {
    //     return params.vestingPoints;
    // }
    ...
    function checkingEpoch() public {
        ...
        if (
            epoch != Epoch.Private &&
            block.timestamp >= params.privateStart &&
            block.timestamp <= params.privateEnd
        ) {
            epoch = Epoch.Private;
            return;
        }
        // if (
        //     epoch != Epoch.Public &&
        //     block.timestamp >= params.publicStart &&
        //     block.timestamp <= params.publicEnd &&
        //     _overcomeThreshold()
        // ) {
        //     epoch = Epoch.Public;
        //     return;
        // }
        if (
            (epoch != Epoch.Finished && (block.timestamp > params.privateEnd))
        ) {
        ...
        //     (block.timestamp > params.privateEnd && block.timestamp < params.publicStart)
        // ) {
        //     epoch = Epoch.Waiting;
        //     return;
        // }
    ...
    function deposit(uint256 _amount) external {
        ...
        //     privatePrice = (params.privateTokenPrice * PCT_BASE) / (uint96(state.exchangeRate) * ORACLE_MUL);
        // }
    ...
    //     _onlyAdmin();
    //     require(_tokenAddress != address(token), "can not withdraw IDO token");
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        if (only) {
            ...
            if (
                uint256(tokensaleTiers[_sender]) >
                uint256(stakingContract.getTierOf(_sender))
            ) {
                t = tokensaleTiers[_sender];
            } else {
        ...
        // uint256 maxFiatInDecimals = maxInFiat * PCT_BASE;
        ...
        require(_amount <= max, "upto Allocation");
        ...
        // console.log("s point", uint8(s.point));
        ...
        // if (left > 0) {
        //     busd.safeTransfer(_sender, left);
        // }
    }
    ...
    //     uint256 forContract = state.totalSupplyDecimals -
    ...
    function claim() external {
        checkingEpoch();
        // console.log("timestamp claim", block.timestamp);
        // console.log("private end", params.privateEnd);
        // console.log("epoch", uint8(epoch));
        require(
            uint8(epoch) > 1 && !admin.blockClaim(address(this)),
            "incorrect time"
        );
        // uint256 lastElement = params.vestingPoints.length -1 ;
        // console.log("length", lastElement);
        // require(block.timestamp >= params.vestingPoints[lastElement][0], "can't claim yet");
        // console.log("ddd", params.vestingPoints[0][0]);
        address sender = msg.sender;
        ...
        if (left > 0) {
            busd.safeTransfer(sender, _shift(left));
    ...
    function _claim(Staked memory _s) internal view returns (uint256, uint256) {
        // uint256 supply = state.totalSupplyDecimals;
        if (state.totalPrivateSold > state.totalSupplyDecimals) {
            uint256 rate;
            if (state.totalSupplyDecimals > state.freePrivateSold && !_s.free) {
                rate =
                    ((state.totalSupplyDecimals - state.freePrivateSold) *
                        PCT_BASE) /
                    (state.totalPrivateSold - state.freePrivateSold);
            } else if (
                state.totalSupplyDecimals <= state.freePrivateSold && _s.free
            ) {
                rate =
                    (state.totalSupplyDecimals * PCT_BASE) /
                    state.freePrivateSold;
            } else {
                rate = PCT_BASE;
            }
            ...
            return (
                _s.share,
                ((_s.amount - _s.share) * params.privateTokenPrice) / PCT_BASE
            );
        } else {
            return (_s.amount, 0);
        }
    ...
    {
        _newPoint = _curPoint;
        // console.log("curpoint",uint8(_curPoint));
        for (uint8 i = 0; i <= uint8(_curPoint); i++) {
            // require((_now >= params.vestingPoints[i][0]), "Can't claim yet");
    ...
    function takeLocked() external override {
        _onlyAdmin();
        require(block.timestamp >= (params.privateEnd + 2592e3), "Not ended");
        uint256 amountTkn = token.balanceOf(address(this));
        uint256 amountBUSD = busd.balanceOf(address(this));
        if (amountTkn > 0) {
            token.safeTransfer(admin.wallet(), amountTkn);
...
contract Staking is IStaking, Initializable {
    ...
    function unstake(uint256 _amount) external override {
        ...
        if (stateOfUser[msg.sender].lock > 1) {
            IAirdrops(admin.airdrop()).userPendingEBSC(msg.sender);
        }
        ...
        if (s.lock > 1) {
            IAirdrops(admin.airdrop()).setShareForEBSCReward(msg.sender,_amount);
        }
    }
