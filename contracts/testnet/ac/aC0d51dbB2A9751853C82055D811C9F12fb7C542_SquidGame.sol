        ...
        for (uint8 i=0; i < LAST_LEVEL; i++) 
        {
            ...
            if(level_length>0)
            {
                if(i==15){
                    ...
                    if((!users[userAddress].activeMainLevels[i+2]) && (users[userAddress].CheckPayment[i].firstPayment) && (users[userAddress].CheckPayment[i].secPayment)){
                        rewardCount = 2;
                    }
                    else{
                        uint32 tempCount=levelid;
