...
contract SafeSwapTradeRouter is Initializable {
     ...
     */
    function getTokenInfoDetails(
        address _tokenAddress,
        TransactionType _transactionType
    ) public view returns (TokenFee memory) {
     ...
     */
    function getTotalSwapFees(
        uint256 amountIn,
        address[] memory path
    ) public view returns (uint256 feeAmount, uint256 portalFee) {
        if (whitelistFfsFee[msg.sender]) {
            return (0, 0);
        }
        require(path.length >= 2, "SafeswapFee: INVALID_PATH");
    ...
    function _claimTokenFee(address _assetIn, TransactionType _transactionType, uint256 _feeAmount) private {
    ...
    function _claimTokenFees(address[] memory _path, uint256 _amountIn) private {
        ...
        for (uint256 i; i < _path.length - 1; i++) {
            if (_path[i] != ISafeSwapRouter(swapRouter).WETH()) {
            ...
            if (_path[i + 1] != ISafeSwapRouter(swapRouter).WETH()) {
