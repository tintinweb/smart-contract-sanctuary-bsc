...
contract StandartStaking is Ownable {
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 syrupSupply = pool.balance;
        if (syrupSupply == 0) {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare + ((tokenReward * 1e12) / syrupSupply);
        pool.lastRewardBlock = block.number;
    }
    ...
    function _deposit(address userAddress,uint256 _pid,uint256 _amount)internal{
        ...
        if(pool.depositFee>0){
            ...
            _amount = _amount - (_amount * pool.depositFee) / 10_000;
            
        }
        user.rewardPending = ((user.amount *  pool.accRewardPerShare) / 1e12) - user.rewardDebt + user.rewardPending;
        user.amount = user.amount + _amount;
        pool.balance += _amount;
        user.rewardDebt = (user.amount *  pool.accRewardPerShare) / 1e12;
        user.lastDepositTime = block.number;
    ...
    function withdraw(uint256 _pid,uint256 _amount) public {
        ...
        if(pool.withdrawFee>0){
            ...
            _amount = _amount - (_amount * pool.withdrawFee) / 10_000;
        }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        if(pool.emergencyWithdrawFee>0){
            ...
            _amount = _amount - (_amount * pool.emergencyWithdrawFee) / 10_000; 
        }
        pool.stakedToken.safeTransfer(address(msg.sender), _amount);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function fixedDeposit(uint256 _pid,uint256 _amount) public {
        require (_amount > 0, 'amount 0');
    ...
    function fixedWithdraw(uint256 _pid) public {
    ...
    function fixedEmergencyWithdraw(uint256 _pid) public {
    ...
    function fixedSetDepositFee(uint256 _pid,uint depFee,uint emFee,uint wFee) external onlyOwner {
        FixedPoolInfo storage pool = fixedPoolInfo[_pid];
    ...
    function transferPoolNativeToOther(uint256 _pid,uint256 _newPid) external{
        FixedPoolInfo storage pool = fixedPoolInfo[_pid];
        ...
        user.amount+=pending;
        _deposit(msg.sender, _newPid, user.amount);
        user.amount = 0;
    }
}