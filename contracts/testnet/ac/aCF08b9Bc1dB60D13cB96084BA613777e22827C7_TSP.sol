...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            if (isSell) {
                ...
                swapFee = tAmount.mul(5).div(100);
                _takeTransfer(sender, address(this), swapFee);               
                tAmount = tAmount.sub(swapFee).sub(tAmount.mul(1).div(1000));
            }
    ...
    function _takeTransfer(
        ...
        if(checkWalletLimit && !isWalletLimitExempt[to])
        require(balanceOf(to).add(tAmount) <= _walletMax);
