...
interface TokenLock {
    function lockToken(address to, uint256 lock) external;
}

contract BDCToken is ERC20Capped, Ownable {
    using SafeMath for uint256;
    using Address for address;

    IERC20 public BDToken;
    address public lockTokenPool;
    mapping(address => address) public referrers;
    mapping(address => address[]) private fans;

    constructor(IERC20 BDToken_)
        ERC20("Brain Dance Computing", "BDC")
        ERC20Capped(6000000000 * 10**18)
    {
        BDToken = BDToken_;

        _mint(address(this), 6000000000 * 10**18);
    }

    function setLockTokenPool(address lockTokenPool_) external onlyOwner {
        lockTokenPool = lockTokenPool_;
    }

    function mint(uint256 bdValue) external {
        BDToken.transferFrom(msg.sender, address(this), bdValue);
        uint256 mintValue = bdValue.mul(3);
        require(
            balanceOf(address(this)) >= mintValue,
            "isufficient BDC reserves"
        );
        _approve(address(this), lockTokenPool, bdValue);
        TokenLock(lockTokenPool).lockToken(msg.sender, bdValue);
    }
    ...
    function getFans(address account) public view returns (address[] memory) {
        return fans[account];
    }
}
