...
contract SesameGame is IProduct, ReentrancyGuard {
    ...
    event Return(address indexed player, uint256 round, uint256 tickets);
    ...
    constructor(
        ...
        distributor = ITokenDistributor(_distributor);
    ...
    function enter(uint256 ticket) public payable nonReentrant {
        ...
        if (ticket + tickets[round].length > ticketPerRound) {
            ...
            if (_native) {
                ...
                emit Return(msg.sender, round, extra);
            }
        }
        ...
        nonReentrant
    {
        ...
        lastWinner = _winner;
        emit DeclareWinner(_winner, _round, toWinner);
        accountant.declareWinner(_winner, _round, toWinner);
    ...
    /** @notice Governance can issue refund for current round */
    function refund() external override onlyGovernance {
        _refund(round);
        _closeRound(round);
        _startRound();
    }
    ...
    function _refund(uint256 _round) internal nonReentrant {
        for (uint256 i = 0; i < tickets[_round].length; i++) {
            ...
            if (amount > 0) {
                accountant.refund(player, _round, ticketMap[_round][player]);
                emit Refund(player, _round, ticketMap[_round][player]);
