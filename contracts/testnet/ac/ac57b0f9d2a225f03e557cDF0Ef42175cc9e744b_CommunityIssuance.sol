...
import "../Interfaces/IVSTAToken.sol";
...
contract CommunityIssuance is ICommunityIssuance, Ownable, CheckContract, BaseMath {
    ...
    using SafeERC20 for IVSTAToken;
    ...
    */
    ...
    /* 
    * The community VSTA supply cap is the starting balance of the Community Issuance contract.
    * It should be minted to this contract by VSTAToken, when the token is deployed.
    * 
    * Set to 32M (slightly less than 1/3) of total VSTA supply.
    */
    uint constant public VSTASupplyCap = 32e24; // 32 million

    IVSTAToken public vstaToken;

    address public stabilityPoolAddress;

    uint public totalVSTAIssued;
        ...
        override 
    {
        ...
        vstaToken = IVSTAToken(_vstaTokenAddress);
        stabilityPoolAddress = _stabilityPoolAddress;

        // When VSTAToken deployed, it should have transferred CommunityIssuance's VSTA entitlement
        uint VSTABalance = vstaToken.balanceOf(address(this));
        require(VSTABalance >= VSTASupplyCap, "setAddresses: balance must be less than supplycap");
    ...
    function issueVSTA() external override returns (uint) {
        _requireCallerIsStabilityPool();

        uint latestTotalVSTAIssued = VSTASupplyCap.mul(_getCumulativeIssuanceFraction()).div(DECIMAL_PRECISION);
        uint issuance = latestTotalVSTAIssued.sub(totalVSTAIssued);

        totalVSTAIssued = latestTotalVSTAIssued;
        emit TotalVSTAIssuedUpdated(latestTotalVSTAIssued);
    ...
    function sendVSTA(address _account, uint _VSTAamount) external override {
        _requireCallerIsStabilityPool();

        vstaToken.safeTransfer(_account, _VSTAamount);
    }
