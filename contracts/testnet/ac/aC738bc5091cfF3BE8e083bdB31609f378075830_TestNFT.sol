...
contract TestNFT is ERC721Enumerable, Ownable {
    ...
    // airdrop claim
    function claimAirdrop(address _to, uint256 _mintAmount) public payable {
        uint256 supply = totalSupply();
        require(!paused);
        require(onlyOnce);
        require(_mintAmount > 0);
        ...
        if (msg.sender != owner()) {
            if (hasMint[msg.sender] != true) {
                if (airdropWallets[msg.sender] != true) {
                    require(msg.value >= cost * _mintAmount);
                } else {
                ...
                revert("ERROR: token already minted");
            }
    ...
    function setOnlyOnce(bool _state) public onlyOwner {
        onlyOnce = _state;
    }
    ...
    function addAirdropUser(address _user) public onlyOwner {
        airdropWallets[_user] = true;
    }

    function removeAirdropUser(address _user) public onlyOwner {
        airdropWallets[_user] = false;
    }
