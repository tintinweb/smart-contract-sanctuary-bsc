...
contract SushiRouter is Context {
    ...
    address public UniswapV2Factory_addr;
    address public WETH;
    ...
    constructor () public {
        ...
        UniswapV2Factory_addr = uniswapV2Router.factory();
        WETH = uniswapV2Router.WETH();
    }

    function viewTest() public view returns(uint, uint, uint) {
    ...
    function addLiquidity(
        ...
        require(IERC20(tokenA).allowance(msg.sender, address(this)) >= amountADesired, "Check the token1 allowance");
        require(IERC20(tokenB).allowance(msg.sender, address(this)) >= amountBDesired, "Check the token2 allowance");
        ...
        TransferHelper.safeApprove(tokenA, UniswapV2Router_addr, amountADesired);
        TransferHelper.safeApprove(tokenB, UniswapV2Router_addr, amountBDesired);
    ...
    function addLiquidityETH(
        ...
        require(IERC20(token).allowance(msg.sender, address(this)) >= amountTokenDesired, "Check the token allowance");
        TransferHelper.safeTransferFrom(token, msg.sender, address(this), amountTokenDesired);
        TransferHelper.safeApprove(token, UniswapV2Router_addr, amountTokenDesired);
        ...
        address pair = IUniswapV2Factory(UniswapV2Factory_addr).getPair(token, WETH);
        IERC20(pair).transfer(msg.sender, liquidity); // send liquidity to user
        (test1, test2, test3) = (amountToken, amountETH, liquidity);
        // // balanceOfLP[msg.sender][token][WETH] = balanceOfLP[msg.sender][token][WETH].add(liquidity);
        if(msg.value > amountETH) TransferHelper.safeTransferETH(msg.sender, msg.value - amountETH);
        if(amountTokenDesired > amountToken) TransferHelper.safeTransfer(token, msg.sender, amountTokenDesired - amountToken);
    }
 ...
 // **** REMOVE LIQUIDITY ****
    function removeLiquidity(
        ...
        address pair = IUniswapV2Factory(UniswapV2Factory_addr).getPair(tokenA, tokenB);
        IERC20(pair).transferFrom(msg.sender, address(this), liquidity); // send liquidity to user
        IERC20(pair).approve(UniswapV2Router_addr, liquidity);
    ...
    function removeLiquidityETH(
        ...
        (amountToken, amountETH) = uniswapV2Router.removeLiquidityETH(
            token, liquidity, 0, 0, payable(address(this)), block.timestamp + 20 minutes
        );
        test2 = amountToken;
        test3 = amountETH;
    ...
    function removeLiquidityETH1(
        ...
        test1 = liquidity;
        (amountToken, amountETH) = uniswapV2Router.removeLiquidityETH(
            token, liquidity, 0, 0, msg.sender, block.timestamp + 20 minutes
        );
        test2 = amountToken;
        test3 = amountETH;
    }
