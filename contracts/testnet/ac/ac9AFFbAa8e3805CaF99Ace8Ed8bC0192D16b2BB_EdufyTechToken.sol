...
contract EdufyTechToken is Context, IERC20, Ownable {
    BPContract public BP;
    bool public bpEnabled;
    bool public BPDisabledForever; //Line 787 to 798 will trigger this function as True/ False (Default is False)
    ...
    address payable public devmarketingWallet = payable(0x4508b5993B8CdD7fC876cac5dA235aE4fc684966);
    address payable public charityWallet = payable(0x2C8A16Aed972626c029C9EeB48C0bDde24ACc26D); 
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000000000000000000000 *10**18;
    uint256 public _maxTxAmount = 100000000000000000000000000000000 *10**18;
...
//
// Set Bot Protection Enabled Forever - added by KD
    function setBotProtectionEnabledForever() external onlyOwner{
    require(BPDisabledForever == true);
...
//
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_devFee.add(_liquidityFee).add(_marketingFee)); 

        uint256 tokensForDev = contractTokenBalance.mul(_devFee).div(_liquidityFee.add(_marketingFee).add(_devFee));
        
        uint256 tokensForCharity = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_devFee));
        ...
        swapTokensForWallets(tokensForCharity, charityWallet);
        swapTokensForWallets(tokensForDev, devmarketingWallet);
    }
    ...
    function setdevmarketingWallet(address payable newWallet) external onlyOwner() {
        devmarketingWallet = newWallet;
    }
    
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyMarketingFee = MarketingFee;
        _buyLiquidityFee = liquidityFee;
        _buyDevFee = devFee;
        require(_buyTaxFee + _buyDevFee + _buyLiquidityFee + _buyMarketingFee <= 15, "Must keep at 15% or less");
    }

    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 devFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellMarketingFee = MarketingFee;
        _sellLiquidityFee = liquidityFee;
        _sellDevFee = devFee;
        require(_sellTaxFee + _sellDevFee + _sellLiquidityFee + _sellMarketingFee<= 20, "Must keep at 20% or less");
    }
    ...
    //Withdraw leftover BNB gas - KD
    function withdraw() public onlyOwner {
    payable(msg.sender).transfer(address(this).balance); 
    }
    //End

    //New router address? - OLD CODES
