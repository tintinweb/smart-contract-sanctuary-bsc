...
contract BnbOctopus {
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buyoctopushiminers();}
    ...
    function BuyOctopus(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startOctopus(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many Octopus per day user will receive based on BNB deposit */
    ...
    function buyoctopushiminers() private{
