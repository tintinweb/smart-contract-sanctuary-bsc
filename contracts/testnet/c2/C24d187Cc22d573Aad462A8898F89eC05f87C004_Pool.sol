...
contract Pool is Ownable {
    ...
    function depositToken(address token, uint256 amount, string[] calldata data) public{    
        if (amount > 0) {
            LDC(token).transferFrom(_msgSender(), address(this), amount);
        }
        emit DepositToken(_msgSender(), token, amount, data[0], uint64(block.timestamp));
    }
    ...
    function withdrawToken(
        ...
        uint256 gameContractBalance = LDC(token).balanceOf(address(this));
        if (gameContractBalance < amount) {
            return;
        }
        LDC(token).transfer(_msgSender(), amount);
...
contract LDC is ERC20 {
  uint256 public constant cap = 300000000*10**18;
  address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
  uint256 public _burnAmount;

  constructor () ERC20("Lost Dungeon", "LDC"){
