...
contract StakePoolTokenForToken is States, Ownable {
    ...
    function setTokens(uint256 _earlyUstakeFee, IBEP20 _stakingToken, IBEP20 _rewardingToken, address _rewardingTokenWallet) public onlyOwner {
    ...
    function setData(uint256 _apy, uint256 _days, address _rewardingTokenWallet, uint256 _maxReward, uint256 _earlyUstakeFee) public onlyOwner {
        apyTier = _apy;
        lockPeriod = _days;
        rewardingTokenWallet = _rewardingTokenWallet;
        earlyUstakeFee = _earlyUstakeFee;
    ...
    function _unstakeTokens() internal {
        ...
        rewardingToken.transfer(_msgSender(),wUser.amount);
    ...
    function _claim() internal {
        ...
        if( info.reward > 0 && info.reward <= pendingRewards() ){
            rewardingToken.transferFrom(rewardingTokenWallet , _msgSender() , info.reward);
        } else{
...
contract FactoryStakingPool{
    ...
    function createTokenForToken(address ownerAddress, uint256 _earlyUstakeFee, IBEP20 _stakingToken, IBEP20 _rewardingToken, address _rewardingTokenWallet) public returns (address)
    {
        StakePoolTokenForToken toDeploy = new StakePoolTokenForToken();
        toDeploy.setTokens(_earlyUstakeFee, _stakingToken, _rewardingToken, _rewardingTokenWallet);
