...
contract DDBNFT is ERC721AQueryable, Ownable {
    ...
    error DDBNFT__ContractIsPaused();
    error DDBNFT__NftSupplyLimitExceeded();
    error DDBNFT__InvalidMintAmount();
    error DDBNFT__MaxMintAmountExceeded();
    error DDBNFT__InsufficientFunds();
    error DDBNFT__QueryForNonExistentToken();
    ...
    function mint(uint256 _mintAmount) external payable {
        if (paused == 1) revert DDBNFT__ContractIsPaused();
        if (_mintAmount == 0) revert DDBNFT__InvalidMintAmount();
        if (_mintAmount > maxMintAmountPerTx)
            revert DDBNFT__MaxMintAmountExceeded();
        uint256 supply = totalSupply();
        if (supply + _mintAmount > maxSupply)
            revert DDBNFT__NftSupplyLimitExceeded();

        if (msg.sender != owner()) {
            if (msg.value < cost * _mintAmount)
                revert DDBNFT__InsufficientFunds();
        }
    ...
    {
        if (!_exists(tokenId)) revert DDBNFT__QueryForNonExistentToken();
