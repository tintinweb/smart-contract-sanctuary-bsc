...
contract MLONTOMGM is ReentrancyGuard, Ownable{
   using SafeMath for uint256; 
    ...
    constructor(address _mlon, address _mgm) public{
        mlon = IBEP20(_mlon);
        mgm = IBEP20(_mgm);
    }
    ...
    function currentPrice() public view returns(uint256){
        return mgmSupply.div(mlonSupply);
    }

    function swapMLONToMGM(uint256 _amountIn, uint256 _amountOutMin) public nonReentrant{ 
        require(mlon.balanceOf(msg.sender) >= _amountIn, "MLONTOMGM: insufficient mlon");
        require(mgm.balanceOf(address(this)) >= _amountOutMin, "MLONTOMGM: insufficient mgm");
        uint256 amountOut = _getAmountOut(_amountIn);
        require(amountOut >= _amountOutMin, "MLONTOMGM: slippage too low");
        mgm.transfer(msg.sender, amountOut);
        mlon.transferFrom(msg.sender, address(this), _amountIn);
        
        
    }

    function getAmountOut(uint256 _amountIn) public view returns(uint256 amountOut){
        uint256 mSupply = mlonSupply;
        mSupply = mSupply.add(_amountIn);
        amountOut = mgmSupply.sub((k.div(mSupply)));
    }

    function _getAmountOut(uint256 _amountIn) internal returns(uint256 amountOut){

        mlonSupply = mlonSupply.add(_amountIn);
        amountOut = mgmSupply.sub((k.div(mlonSupply)));
        mgmSupply = mgmSupply.sub((k.div(mlonSupply)));
        return amountOut;
    }

    function addLiquiduity(uint256 _mgmAmount, uint256 _mlonAmount) public onlyOwner{
        require(mgm.balanceOf(msg.sender) >= _mgmAmount, "MLONTOMGM: insufficient mgm");
        require(mlon.balanceOf(msg.sender) >= _mlonAmount, "MLONTOMGM: insufficient mlon");

        mlonSupply = mlonSupply.add(_mlonAmount);
        mgmSupply = mgmSupply.add(_mgmAmount);
        mgm.transferFrom(msg.sender, address(this), _mgmAmount);
        mlon.transferFrom(msg.sender, address(this), _mlonAmount);

        k = mlonSupply.mul(mgmSupply);
    }

    function removeLiquiduity(uint256 _mgmAmount, uint256 _mlonAmount) public onlyOwner{
        require(mgm.balanceOf(address(this)) >= _mgmAmount, "MLONTOMGM: insufficient mgm");
        require(mlon.balanceOf(address(this)) >= _mlonAmount, "MLONTOMGM: insufficient mlon");

        mlonSupply = mlonSupply.sub(_mlonAmount);
        mgmSupply = mgmSupply.sub(_mgmAmount);
        mgm.transfer(msg.sender, _mgmAmount);
        mlon.transfer(msg.sender, _mlonAmount);
