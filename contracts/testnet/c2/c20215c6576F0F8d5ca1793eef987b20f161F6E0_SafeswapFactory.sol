...
contract SafeswapFactory is ISafeswapFactory, Initializable {
    bytes32 public constant INIT_CODE_PAIR_HASH =
        keccak256(abi.encodePacked(type(SafeswapPair).creationCode));
    ...
    uint256 public buyBackFeePercent;
    ...
    function initialize(
        ...
        uint256 _companyFeePercent,
        uint256 _buyBackFeePercent,
        uint256 _lpFeePercent
    ...
    function setCompanyFeePercent(uint256 _companyFeePercent) external {
        require(msg.sender == feeToSetter, "Safeswap: FORBIDDEN");
        companyFeePercent = _companyFeePercent;
    }

    function setBuyBackFeePercent(uint256 _buyBackFeePercent) external {
        require(msg.sender == feeToSetter, "Safeswap: FORBIDDEN");
        buyBackFeePercent = _buyBackFeePercent;
    }

    function setLpFeePercent(uint256 _lpFeePercent) external {
        require(msg.sender == feeToSetter, "Safeswap: FORBIDDEN");
        lpFeePercent = _lpFeePercent;
    }
    ...
    function getFeeConfig() public view returns (FeeConfig memory) {
        ...
        _feeConfig.feeTo = feeTo;
        _feeConfig.buyBackWallet = buyBackWallet;
        _feeConfig.companyFeePercent = companyFeePercent;
        _feeConfig.buyBackFeePercent = buyBackFeePercent;
        _feeConfig.lpFeePercent = lpFeePercent;
        _feeConfig.precision = PRECISION;
    }

    function getTotalFee() public view returns (uint256, uint256) {
        return (
            companyFeePercent + buyBackFeePercent + lpFeePercent,
            PRECISION
        );
    }
}
