...
contract BNBRagnarok is ERC20Detailed, Ownable {
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(gamePoolFee).add(
            burnFee
        );
    ...
    uint256 private constant MAX_SUPPLY = 100 * 10**6 * 10**DECIMALS; //100M Total Supply
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(gamePoolFee)).div(feeDenominator)
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(gamePoolFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(gamePoolReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(gamePoolFee).div(
                treasuryFee.add(gamePoolFee)
            ),
            gas: 30000
        }("");          
