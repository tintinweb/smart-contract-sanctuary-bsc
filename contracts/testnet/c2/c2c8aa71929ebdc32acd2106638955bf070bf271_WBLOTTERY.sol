...
contract WBLOTTERY {
    ...
    struct lotterydata {
        mapping(address => draw) participants;
    ...
    function setwinners() external onlyAdmin {
        ...
        if (lotteries[nonce].prize > 0) {
            ...
            uint256 pLenght = lotteries[nonce].uniqueaddress.length;
            //uint256 pp = 0;
            uint256 number = 0;
            ...
            //mapping(address => draw) participants;
            for (uint256 i = 0; i < pLenght; i++) {
                player storage user = players[lotteries[nonce].uniqueaddress[i]];
                lotteries[nonce].participants[lotteries[nonce].uniqueaddress[i]].first = number;
                number += user.lotteries_bought[nonce];
                lotteries[nonce].participants[lotteries[nonce].uniqueaddress[i]].last = number;
                number++;
            }
            address winner1 = getwinners(0, lotteries[nonce].participants, tickets, lotteries[nonce].uniqueaddress);
            address winner2 = getwinners(1, lotteries[nonce].participants, tickets, lotteries[nonce].uniqueaddress);
            address winner3 = getwinners(2, lotteries[nonce].participants, tickets, lotteries[nonce].uniqueaddress);
    ...
    function getwinners(uint256 index, mapping(address => draw) storage _part, uint256 tickets, address[] storage uniques) internal view returns (address winner) {
        uint256 winner_number = random(
            tickets,
            index
        );
        address winner_address;
        for(uint256 i = 0; i < uniques.length; i++){
            if(winner_number >= _part[uniques[i]].first && winner_number <= _part[uniques[i]].last){
                winner_address = uniques[i];
            }
        }
