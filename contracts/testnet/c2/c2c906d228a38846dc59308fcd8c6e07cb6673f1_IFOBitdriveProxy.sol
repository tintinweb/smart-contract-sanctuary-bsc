...
contract IFOBitdriveProxy is ReentrancyGuard, Initializable {
  ...
  uint256 public instantpercentage=15;
  ...
  function deposit(uint256 _amount) public {
    ...
    require (_amount > 0 &&_amount >=STAKING_PRICE[1], 'need _amount > 0');
  ...
  function availableMcar(address _user) public view returns (uint256) {
      ...
      uint256 available = 0;
      if( block.timestamp > userInfo[_user].round3withdrawtime && diff > 1 ){
            uint calcMcar = diff / 1;
            if(calcMcar >= 4){
                calcMcar = balance;
            }
  ...
  function startNextRound(uint256 round,uint _endBlock) public onlyAdmin {
