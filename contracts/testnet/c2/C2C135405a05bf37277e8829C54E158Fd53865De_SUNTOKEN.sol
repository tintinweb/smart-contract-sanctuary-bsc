...
contract SUNTOKEN {
    ...
    bool public _enableMaxPriceImpact;
    ...
    address WBNB_address = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; //ETHtest: 0xc778417e063141139fce010982780140aa0cd5ab BSCtest: 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd  BSC: 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c
    address wallet_team;
    ...
    function doThaTaxTing(address from, address to, uint amount) internal returns (bool) {
        ...
        require(priceImpactOfAmount(amount) <= maxPriceImpact);
    ...
    function priceImpactOfAmount(uint amount) public view returns (uint) {

        if(_enableMaxPriceImpact == true){
            uint pooledTokens = balanceOf(uniswapV2Pair);
            uint pooledETH = WBNB.balanceOf(uniswapV2Pair);
            uint price = div(pooledETH,pooledTokens);
            uint ETHvalue = amount*price;

            uint new_pooledTokens = add(pooledTokens,amount);
            uint new_pooledETH = sub(pooledETH,ETHvalue);
            uint new_price = div(new_pooledETH,new_pooledTokens);

            uint priceDiff;
            if(price > new_price){
                priceDiff = sub(price,new_price);
            }
            else if(price < priceDiff){
                priceDiff = sub(new_price,price);
            }
            else{
                priceDiff = 0;
            }
            ...
            uint priceImpact = div(priceDiff,price); //Gives the price impact in percentage*100
        return priceImpact;
        }
        else{
        return 0;
    ...
    function enableMaxPriceImpact() public owner returns(bool){
        _enableMaxPriceImpact = true;
    return true;
    }

    function disableMaxPriceImpact() public owner returns(bool){
        _enableMaxPriceImpact = false;
    return true;
    }
