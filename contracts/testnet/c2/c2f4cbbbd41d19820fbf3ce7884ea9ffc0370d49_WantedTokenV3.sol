...
contract WantedTokenV3 is ERC20Upgradeable, WantedStorageV1, ReentrancyGuardUpgradeable{
    ...
    /**@dev Buy Wanted Tokens.
      ...
      *@return bool
      */
    function buyToken() payable external nonReentrant returns (bool) {
        ...
        _transfer(owner(), LMSWallet, _amount);
        amount -= _amount;
        ...
        _amount = _feeCalculation(TokenomicDetails[1].nftSweepstakePercent, tempAmount);
        _transfer(owner(), nftSweepstakeWallet, _amount);
        amount -= _amount;
        ...
        returns (bool)
    {
        ...
        uint256 amount;
        uint256 length = addressess.length;
        ...
        if(SweepstakeType == 1) {
            ...
            tokenRewardPerUser = tokenRewardAmount / length;
            ...
            nftRewardPerUser = nftRewardAmount / length;
            amount = nftRewardPerUser;
        }
        ...
        for(uint256 i = 0; i < length; i++) {
            ...
            if(SweepstakeType == 1 /*&& data.tokenSweepstakeTimestamp < block.timestamp*/) {
                data.tokenSweepstakeAmount = amount;
                data.tokenSweepstakeTimestamp = block.timestamp + 7 days;    
            }else if(SweepstakeType == 2 /*&& data.nftSweepstakeTimestamp < block.timestamp*/) {
                data.nftSweepstakeAmount = amount;
        ...
        emit RewardReceived(tempAddresses, amount, SweepstakeType);
    ...
    /**@dev For claiming the winninng amount in Sweepstake Lottery.
      ...
      *@return bool.
      */
    function claimReward(uint8 SweepstakeType) external returns (bool) {
        ...
        returns (bool) 
    {
        ...
        if(pairAddress[to] == true) {
            ...
            _amount = _feeCalculation(TokenomicDetails[3].tokenSweepstakePercent, tempAmount);
            _transfer(from, tokenSweepstakeWallet, _amount);
            amount -= _amount;

            _amount = _feeCalculation(TokenomicDetails[3].nftSweepstakePercent, tempAmount);
            _transfer(from, nftSweepstakeWallet, _amount);
            amount -= _amount;
            emit call(1);
        }else if(pairAddress[from] == true){
            tempAmount = amount;
            ...
            _amount = _feeCalculation(TokenomicDetails[2].tokenSweepstakePercent, tempAmount);
            _transfer(from, tokenSweepstakeWallet, _amount);
            amount -= _amount;
            emit call(3);
        }   
...
abstract contract WantedStorageV1 is OwnableUpgradeable {
    ...
    // address burnWallet; // delete this
    // address LMSWallet;
    // address nftSweepstakeWallet;
    // address tokenSweepstakeWallet;
