...
contract LoyaltyRedeemProgram is Ownable, Pausable, ReentrancyGuard {
    ...
    function redeem(address _to, uint256 _amount) external whenNotPaused nonReentrant {
        ProgramInfo memory program = programs[activeProgramId];
        ...
        require(
            program.startTime <= block.timestamp && block.timestamp < program.endTime, "LoyatyProgramRedeem: Invalid time"
        );
...
interface ILyLevelToken is IERC20 {
