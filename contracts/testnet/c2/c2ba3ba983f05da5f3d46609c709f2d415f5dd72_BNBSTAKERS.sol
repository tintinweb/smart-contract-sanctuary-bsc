...
contract BNBSTAKERS {
    ...
    uint256 public old = 0;
  
    uint8 constant BONUS_LINES_COUNT = 5;

    uint16 constant PERCENT_DIVIDER = 1000; 
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [40, 20, 10, 5, 1]; 
    ...
    constructor() {
        market = msg.sender;
        market1 = 0x583646c6148c55c281e5567ee102b3424da25fbA;
        holders = 0x64bCE470877d0b593A874Baa91E419A2C1a36885; //Pool funds for v1
        owner = address(0);
        uint8 tarifPercent = 155;
        for (uint8 tarifDuration = 30; tarifDuration <= 40; tarifDuration++) {
    ...
    function withdraw() external {
        ...
        withdrawn += amount;
        uint256 oll = amount/13;
        uint256 withamount = amount - oll;
        if(old <=500) {
            payable(holders).transfer(oll);
            old = old + oll;
    ...
    function reinvest(uint8 _tarif) external {
      Player storage player = players[msg.sender];
        ...
        require(amount >= 0.01 ether, "Minimum deposit amount is 0.01 BNB");
        require(amount <= 5 ether, "Maximum deposit amount is 5 BNB");
        require(tarifs[_tarif].life_days > 0, "Tarif not found");
