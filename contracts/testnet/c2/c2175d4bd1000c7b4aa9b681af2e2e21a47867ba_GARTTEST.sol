...
contract GARTTEST is ERC20, Ownable {
    ...
    constructor () ERC20("GARTTEST", "GARTTEST") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        buyBackUpperLimit = 1 * 10**18;  // set this
        balanceMinimumLimit = 1 * 10**18;  // set this
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            uint256 totalFee = _totalFeesOnBuy + _totalFeesOnSell; // 10
            uint256 liquidityShare = liquidityFeeOnBuy + liquidityFeeOnSell; // 2
            uint256 otherHalf;

            if (liquidityShare > 0) {
                uint256 liquidityTokens = swapTokensAtAmount * liquidityShare / totalFee; // 20
                uint256 half = liquidityTokens / 2; // 10
                otherHalf = liquidityTokens - half; // 10
            }

            swapTokensAndSend(swapTokensAtAmount - otherHalf, otherHalf); // 90    
    ...
    function swapTokensAndSend(uint tokensAmount, uint256 otherHalf) private{
        ...
        uint256 liquidityShare = (liquidityFeeOnBuy + liquidityFeeOnSell) * newBalance / totalFee; // 2 * 1 / 10 = 0.2 BNB
        uint256 marketingShare = (marketingFeeOnBuy + marketingFeeOnSell) * newBalance / totalFee; // 2 * 1 / 10 = 0.2 BNB
