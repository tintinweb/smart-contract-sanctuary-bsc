...
contract BNB_V3 {
    ...
    uint256 public WITHDRAWAL_LIMIT = 5 * 1e17;
    uint256 public WITHDRAWAL_TAX = 800;
    uint256 public COMPOUND_FOR_NO_TAX_WITHDRAWAL = 2;
    ...
    function sellCrops() public{
        ...
        if(eggValue <= WITHDRAWAL_LIMIT){
            ...
            //withdrawal limit payout if egg value is equals too or less
            uint256 eggsPayout = eggValue.sub(payFees(eggValue));
            payable(address(msg.sender)).transfer(eggsPayout);
            user.totalWithdrawn = user.totalWithdrawn.add(eggsPayout);
            totalWithdrawn = totalWithdrawn.add(eggsPayout);            
        }else{
            uint256 eggPrice = WITHDRAWAL_LIMIT;

            if(getBalance() < eggValue) {
                eggValue = getBalance();
            }

            //withdrawal limit payout if egg value is greater than limit set
            
            uint256 eggsPayout = eggPrice.sub(payFees(eggValue));
            payable(address(msg.sender)).transfer(eggsPayout);
            user.totalWithdrawn = user.totalWithdrawn.add(eggsPayout);
            totalWithdrawn = totalWithdrawn.add(eggsPayout); 
            ...
            uint256 dailyCompoundBonus = getDailyCompoundBonus(msg.sender, eggsForCompound);
            eggsForCompound = eggsForCompound.add(dailyCompoundBonus);
            uint256 eggsUsedVal = calculateEggSell(eggsForCompound);
            uint256 eggsUsedValue = eggsUsedVal.sub(eggPrice);
            user.userDeposit = user.userDeposit.add(eggsUsedValue);
            totalCompound = totalCompound.add(eggsUsedValue);
...
**/    
    ...
    function SET_WITHDRAWAL_LIMIT(uint256 value) external {
        require(msg.sender == owner, "Admin use only");
        require(value >= 3);
        WITHDRAWAL_LIMIT = value * 1e18;
    }
