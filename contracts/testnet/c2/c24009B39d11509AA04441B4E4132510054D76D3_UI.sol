...
contract UI {
    ...
    uint256 private constant oneWarriorArraySize = 25;
...
// *** SET ***
    function UpgradeUI(address eternalStorageAddr_) external {
...
// *** GET EXTERNAL IN-GAME ***
    ...
    */
    ...
    function _EX_WarriorDetails(uint256 warriorId_) private view returns (uint256[] memory) {
        ...
        (uint256 lastFightedTime_, uint256 enemyRarityIndex_, uint256 enemyPersonaIndex_,) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 10, 11, 12, 99);
        (values[0], values[1], values[2], values[3]) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 1, 2, 3, 4);
        (values[7], values[22], values[23],) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 13, 5, 6, 99);
        if (values[7] > 0) {
            (values[4] , values[5], values[6],) = eternalStorage.GET_NFT_DATA_UINT256(warriorId_, 0, 7, 8, 9, 99);
        }
        values[8] = ((values[2] / 10) + 1) * 1000;
        if (block.timestamp > lastFightedTime_) values[9] = block.timestamp - lastFightedTime_;
        values[10] = values[21] * values[20];

        if (lastFightedTime_ + values[20] < block.timestamp) {
            if ((block.timestamp - lastFightedTime_) / values[20] >= values[21]) values[11] = values[21];
            else values[11] = (block.timestamp - lastFightedTime_) / values[20];
        }
