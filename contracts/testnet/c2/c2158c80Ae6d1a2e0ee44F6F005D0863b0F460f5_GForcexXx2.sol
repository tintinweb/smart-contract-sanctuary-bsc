...
contract GForcexXx2 is Ownable {
    ...
    function pay(uint256 level, address id, uint256 percentage, address[] memory usersArr) private {
    ...
    function doFurtherLevel1(uint256 level, address userId, address uplineId, string memory slot, address mySlotUplineFoundId, address mySlotUplineFoundUplineId) internal {
        ...
        //      //a emit Log("dd");
        //       //a emit Log(toAsciiString(mySlotUplineFound.Id));
        //      //a emit Log(myBonusSlot);
        //      //a emit Log(slot);
        ...
        //a emit Log("newdd");
        //a emit Log(toAsciiString(myBonusSlotUpline));
        ...
        if (bestBonusSlot3.toSlice().len() <= 4) {
            ...
            //a emit Log("pay70");
            //a emit Log(toAsciiString(myBonusSlotUplineFound.Id));

            pay(level, myBonusSlotUplineFound.Id, 70, myBonusSlotUplineFound.UserList);
        ...
        if (GlobalUserExists[level][upline1.UplineId]) {
            ...
            if (mySlot.toSlice().len() > 1) {
                ...
                if (bestSlot2.toSlice().len() <= 4) {
                    ...
                    pay(level, mySlotUplineFound.Id, 30, findImmediateUpline.UserList);
                }
    ...
    function doLevel3(uint256 level, address userId, address uplineId, string memory slot) internal {
        ...
        checkAndRefresh(level, upline2.Id, upline1.UserList, upline2.UserExists);
        checkAndRefresh(level, upline3.Id, upline1.UserList, upline3.UserExists);
    ...
    function doLevel2(uint256 level, address userId, address uplineId, string memory slot) internal {
        ...
        //a emit Log("2mySlot found");
        //a emit Log(toAsciiString(upline2.Id));
        //a emit Log(mySlot);
        ...
        if (mySlot.toSlice().len() > 1) {
            ...
            //a emit Log("2myslotupline found");
            //a emit Log(toAsciiString(mySlotUplineFound.Id));
    ...
    private {
        if (currentUserDownlines.length == 15) {
            ...
            if (GlobalUserExists[level][user.UplineId]) {
                ...
                join(level, newRandomAddress, user.UplineId);
    ...
    function findUplineInverse2(
        ...
        if (bestSlot3.toSlice().len() <= 4) {
            ...
            pay(level, mySlotUplineFound.Id, payPercentage, findImmediateUpline.UserList);
        }
    ...
    function findUpline(
        ...
        if (bestSlot2.toSlice().len() <= 4) {
            ...
            if (payPercentage > 0) {
                pay(level, upline2.Id, payPercentage, findImmediateUpline.UserList);
            }
        }
        return upline2;
    }
