 ...
 */
  ...
  */
  ...
  constructor(
    ...
    require(
      ...
      "sale start time should be some time later"
    );
  ...
  */
    ...
    _isWhitelisted(msg.sender)
  {
    ...
    require(_numberOfAllocations > 0, "number of allocations not in range");
    require(
  ...
  */
  function claimGameTokens() external {
    ...
    // IERC20(gameTokenAddress).transferFrom(
    //   projectOwner,
    //   msg.sender,
    //   claimableAmount
    // );
  ...
  */
  function buyTickets(uint256 _bracketNumber)
    ...
    _isPoolActive
  {
    require(lotteryStatus == LotteryStatus.STARTED, "lottery is not active");
    ...
    investorsInfo[msg.sender].ticketCount += ticketBrackets[_bracketNumber]
      .numberOfTickets;
  ...
  */
  function drawWinners(string memory _seed) external onlyOwner {
  ...
  */
  function claimBackBUSD() external {
    ...
    require(investorsInfo[msg.sender].allocationLost > 0, "nothing to claim");
    require(
      lotteryStatus == LotteryStatus.FINISHED,
  ...
  */
  function setXPandBUSD(XpToken _xpToken, IERC20 _busd) external onlyOwner {
    XPToken = _xpToken;
    BUSD = _busd;
  }
}
...
contract XpToken is ERC20, AccessControl {
  ...
  /**
    @notice function to mint new XP tokens
    @notice can be called by minter only (which will be XPFarm)
    @param _recipient address of the reciever
  ...
  */
  ...
  /**
    @notice function to burn XP tokens
    @notice it will burn if the caller is allowed to burn the tokens
    @param _account address whose tokens are to be burned
  ...
  */
  ...
  /**
    @notice function to add minter (which can only be XPFarm contract)
    @param _minter address of the minter (XPFarm in this case)
  */
  function addMinter(address _minter) external onlyRole(OWNER_ROLE) {
    require(_isContract(_minter), "minter should be a contract");
    require(!hasRole(MINTER_ROLE, _minter), "already a minter");
    _grantRole(MINTER_ROLE, _minter);
  }

  /**
    @notice function to remove minter
    @param _minter address of the minter
  */
  function removeMinter(address _minter) external onlyRole(OWNER_ROLE) {
    _revokeRole(MINTER_ROLE, _minter);
  }

  /**
    @notice function to check whether the address is contract or not
...
contract Vesting {
  ...
  */
  function initVestingStages() internal {
    ...
    stages.push(VestingStages(2 * vestingPeriod, 3000));
    stages.push(VestingStages(4 * vestingPeriod, 4500));
    stages.push(VestingStages(6 * vestingPeriod, 6000));

    stages.push(VestingStages(8 * vestingPeriod, 10000));
