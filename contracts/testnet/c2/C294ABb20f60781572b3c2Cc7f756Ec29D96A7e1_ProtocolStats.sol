...
interface IPriceChecker {
    ...
    function getDarkPriceToUsd() external view returns (uint256);
}
...
interface ITreasury is IEpoch {
    ...
    function getDarkgUpdatedPrice() external view returns (uint256);

    function getNextExpansionRate() external view returns (uint256);

    function getNextExpansionAmount() external view returns (uint256);

    function previousEpochDarkgPrice() external view returns (uint256);
...
contract ProtocolStats is OwnableUpgradeSafe {
    ...
    address public skyg;
    address public lightg;
    address public wcro;
    address public dark;
    address public usdc;
    address public pairDarkgDark;
    address public pairSkygWcro;

    address public darkgPool;
    address public skygPool;
    ...
    function getDarkgPriceToUsd() public view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(darkg);
    }

    function getDarkPriceToUsd() public view returns (uint256) {
        return IPriceChecker(priceChecker).getDarkPriceToUsd();
    }
    ...
    function _accountTotalDollarValue(address _account) internal view returns (uint256) {
        ...
        uint256 _darkgBalance = IERC20(darkg).balanceOf(_account);
        uint256 _skygBalance = IERC20(skyg).balanceOf(_account);
        uint256 _wcroBalance = IERC20(wcro).balanceOf(_account);
        uint256 _darkBalance = IERC20(dark).balanceOf(_account);
        uint256 _usdcBalance = IERC20(usdc).balanceOf(_account);
        uint256 _darkgUsdValue = (_darkgBalance > 0) ? _priceChecker.getTokenPriceToUsd(darkg).mul(_darkgBalance).div(1e18) : 0;
        uint256 _skygUsdValue = (_skygBalance > 0) ? _priceChecker.getTokenPriceToUsd(skyg).mul(_skygBalance).div(1e18) : 0;
        uint256 _wcroUsdValue = (_wcroBalance > 0) ? _priceChecker.getTokenPriceToUsd(wcro).mul(_wcroBalance).div(1e18) : 0;
        uint256 _darkUsdValue = (_darkBalance > 0) ? _priceChecker.getTokenPriceToUsd(dark).mul(_darkBalance).div(1e18) : 0;
        return _darkgUsdValue.add(_skygUsdValue).add(_wcroUsdValue).add(_darkUsdValue).add(_usdcBalance);
    }

    function addresses() external view returns (
        address darkgAddress, address skygAddress, address lightgAddress, address wcroAddress, address pairDarkgDarkAddress, address pairSkygWcroAddress,
        address darkgPoolAddress, address skygPoolAddress,
        address treasuryAddress, address boardroomAddress, address daoAddress, address insuranceAddress) {

        darkgAddress = darkg;
        skygAddress = skyg;
        lightgAddress = lightg;
        wcroAddress = wcro;

        pairDarkgDarkAddress = pairDarkgDark;
        pairSkygWcroAddress = pairSkygWcro;

        darkgPoolAddress = darkgPool;
        skygPoolAddress = skygPool;
    ...
    function tokenStats() external view returns (
        uint256 darkgPrice, uint256 skygPrice, uint256 lightgPrice, uint256 darkPrice, uint256 darkgDarkLpPrice, uint256 skygWcroLpPrice,
        uint256 darkgSafePrice, uint256 darkgCirculation, uint256 skygCirculation, uint256[] memory tokenSupplies
    ) {
        tokenSupplies = new uint256[](3);
        {
            tokenSupplies[0] = IERC20(darkg).totalSupply();
            tokenSupplies[1] = IERC20(skyg).totalSupply();
            tokenSupplies[2] = IERC20(lightg).totalSupply();
            darkgCirculation = tokenSupplies[0].sub(IERC20(darkg).balanceOf(darkgPool));
            skygCirculation = tokenSupplies[1].sub(IERC20(skyg).balanceOf(skygPool));
        }
        {
            IPriceChecker _priceChecker = IPriceChecker(priceChecker);
            darkgPrice = getDarkgPriceToUsd();
            skygPrice = _priceChecker.getTokenPriceToUsd(skyg);
            darkPrice = getDarkPriceToUsd();
            darkgDarkLpPrice = _priceChecker.getLpPriceToUsd(pairDarkgDark);
            skygWcroLpPrice = _priceChecker.getLpPriceToUsd(pairSkygWcro);
        }
        uint256 _lightgPremiumRate;
        {
            ITreasury _treasury = ITreasury(treasury);
            _lightgPremiumRate = _treasury.getBondPremiumRate();
            darkgSafePrice = _accountTotalDollarValue(_treasury.insuranceFund()).mul(1e18).div(darkgCirculation);
        }
        lightgPrice = darkgPrice;
        if (_lightgPremiumRate > 1e18) {
            lightgPrice = darkgPrice.mul(_lightgPremiumRate).div(1e18);
        }
    }
    ...
    function poolStats(address _pool) public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        ...
        for (uint i = 0; i < poolLength; i++) {
            ...
            {
                if (_pool == darkgPool) reward = darkg;
                else reward = skyg;
    ...
    function darkgPoolStats() public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        address[] memory lps, uint256[] memory allocPoints, uint256[] memory tvls, uint256[] memory aprs, uint256[] memory lpPrices) {
        return poolStats(darkgPool);
    }

    function skygPoolStats() public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        address[] memory lps, uint256[] memory allocPoints, uint256[] memory tvls, uint256[] memory aprs, uint256[] memory lpPrices) {
        return poolStats(skygPool);
    }

    function boardroomStat() external view returns (
        uint256 epoch, uint256 nextEpochPoint, uint256 twap,
        address share, uint256 sharePrice, uint256 totalShare, uint256 tvl, uint256 nextExpansionRate, uint256 nextExpansionAmount, uint256 darkgEarnPerShare, uint256 apr) {
        ITreasury _treasury = ITreasury(treasury);
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        epoch = _treasury.epoch();
        nextEpochPoint = _treasury.nextEpochPoint();
        twap = _treasury.getDarkgUpdatedPrice();
        ...
        if (_treasurySharedPercent > 0) {
            ...
            if (totalShare > 0) {
                darkgEarnPerShare = nextExpansionAmount.mul(1e18).div(totalShare);
                if (sharePrice > 0) {
                    // = (darkgEarnPerShare * totalShare * darkgPrice) / TVL
                    // = (darkgEarnPerShare * darkgPrice * 1460) / sharePrice
                    apr = darkgEarnPerShare.mul(1460).mul(_priceChecker.getTokenPriceToUsd(darkg)).div(sharePrice);
                }
            }
        }
    }

    function tvls() external view returns (uint256 darkgPoolTvl, uint256 skygPoolTvl, uint256 boardroomTvl,
        uint256 daoTvl, uint256 insuranceTvl, uint256 lendingTvl, uint256 totalTvl) {
        ITreasury _treasury = ITreasury(treasury);
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        ITvlStats _tvlStats = ITvlStats(tvlStats);

        (, darkgPoolTvl, , , , , , ,) = poolStats(darkgPool);
        (, skygPoolTvl, , , , , , ,) = poolStats(skygPool);
        totalTvl = darkgPoolTvl.add(skygPoolTvl);

        uint256 _sharePrice = _priceChecker.getTokenPriceToUsd(skyg);
    ...
    function initialize(
        ...
        address _dark,
        address _usdc,
        address _pairDarkgDark,
        address _pairSkygWcro,
        address _darkgPool,
        address _skygPool,
        address _treasury,
        ...
        pairDarkgDark = _pairDarkgDark;
        pairSkygWcro = _pairSkygWcro;

        darkgPool = _darkgPool;
        skygPool = _skygPool;
