...
contract DeuxStake is Context, Ownable, ReentrancyGuard {
    ...
    function _addStakeholder(address _user) internal returns (uint256) {
        ...
        stakes[_user] = stakeholders.length;
        return userIndex;
    }
    ...
    function _findStake(uint256 _id) internal view returns (Stake storage) {
        uint256 user_index = stakes[_msgSender()];
        return stakeholders[user_index].address_stakes[_id];
    }

    function _deleteStake(uint256 _id) internal {
        uint256 user_index = stakes[_msgSender()];
        delete stakeholders[user_index].address_stakes[_id];
    }

    function _withdraw(uint256 _id) internal returns (uint256) {
        Stake storage current_stake = _findStake(_id);
        require(
            block.timestamp >
    ...
    function emergencyWithdraw(uint256 _id) external {
        ...
        uint256 feeAmount = (current_stake.amount * emergencyFee) / 1000;
        uint256 afterFee = current_stake.amount - feeAmount;
