...
//Property of Gross Mininng
//DO NOT COPY!!!

//https://grossmining.com
//https://t.me/gross_mining
//https://twitter.com/gross_mining

pragma solidity ^0.8.10;

contract GROSSMINING {
    ...
    uint256 ELIT_COMPESATION_FACTOR = 10;
    ...
    function getHashRate(address miner, uint256 tier_index) public view returns(uint256) {
        ...
        uint256 since_last_withdraw = block.timestamp - Miners[miner].lastWithdrawTime;

        if((since_last_withdraw/24 hours) > MIN_ABSENT_DAYS){
            uint256 lwf = (since_last_withdraw/24 hours) / MIN_ABSENT_DAYS;
            hash_rate = hash_rate - (HASH_RATE_COMPENSATOR * lwf);
        }

        if(Miners[miner].downLinesCount >= 5){
            if(Miners[miner].downLinesCount >= ELIT_COMPESATION_FACTOR){
                uint256 rcf = Miners[miner].downLinesCount / ELIT_COMPESATION_FACTOR;
                hash_rate = hash_rate + (HASH_RATE_COMPENSATOR * rcf);
            }else{
