...
contract HoksageMatrix is Ownable {
  ...
  function setTokenAddress(address account) public onlyOwner returns (bool) {
    HOKCOIN = account;
    return true;
  }
  ...
  function payment(address account,address invitor,uint256 level) internal returns (bool) {
    ...
    if(HOKCOIN != address(0)){
      safeTransferETH(invitor,per(amount,5));
  ...
  function getMatrix32(address viewer,uint256 level,uint256 viewcycle) public view returns (address[] memory){
    ...
    path[1] = viewer;
    if(viewcycle>0){
      if(users[viewer].matrix[level].cycle[viewcycle].child.length==0){
        path[2] = address(0);
        path[3] = address(0);
      }else
      if(users[viewer].matrix[level].cycle[viewcycle].child.length==1){
  ...
  function getMatrixTree(address account,uint256 level,uint256 branch) internal view returns (address) {
    ...
    if(branch==0){
      ...
      }else if(users[account].matrix[level].referee.length>0){
        return users[account].matrix[level].referee[branch];
      }else{ return address(0); }
    }else if(branch==1){
      if(users[account].matrix[level].reinvestcount>0){
        return users[account].matrix[level].cycle[0].child[branch];
      }else if(users[account].matrix[level].referee.length==2){
        return users[account].matrix[level].referee[branch];
      }else{ return address(0); }
    }else{
      revert("branch was out of range");
    }
  }
