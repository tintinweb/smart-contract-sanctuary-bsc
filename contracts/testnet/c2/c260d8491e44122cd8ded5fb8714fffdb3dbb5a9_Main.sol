...
interface IBEP20 {
    ...
    function approve(address spender, uint256 addedValue) external returns (bool);
...
contract Main is IBEP20, Auth, Initializable {
    ...
    string constant _name = "H1T9";
    string constant _symbol = "H1T9";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!whitelisted[sender] && !whitelisted[recipient] && !authorizations[sender] && !authorizations[recipient]){
            require(tradingOpen,"Trading not open yet");
            require(!blacklisted[sender] && !blacklisted[recipient],"black listed");
        }
    ...
    function clearStuckBalance(uint256 amountPercentage, address adr) external onlyOwner {
        ...
        (bool success, bytes memory data) = payable(adr).call{value: address(this).balance}("");
        data;
        require(success);
