...
/* Please visit https://radefi.finance/ to learn more! */
contract Radefi {
	using SafeERC20 for IERC20;
	
	uint256 public constant TIME_STEP = 60; // seconds in 1 day
	...
	address private constant tokenAddress = 0x6a343A206700E6131F4cf9DF8E8b00B113c71cCe;
	uint256 public initBlock = 167328360; // start time: 9th January 5PM GMT
	...
	function boost(uint256 _booster, uint256 _amount) public {
		require(block.timestamp >= initBlock);
		require(users[msg.sender].lastAction > 0);
		require(_booster < 3);
	...
	function getUserProfit(address _addr) public view returns(uint256) {
		...
		uint256 _excessBlocks = _totalBlocks % TIME_STEP;
		uint256 _days = (_totalBlocks - _excessBlocks) / TIME_STEP;
		uint256 _totalRoi = _days * _dailyRoi;
		_totalRoi += _dailyRoi * _excessBlocks / TIME_STEP;
		uint256 _totalInvestment = getUserTotalInvestment(_addr);
		uint256 _totalProfit = _totalInvestment * _totalRoi / RATE_DIVISOR;
		return _totalProfit;
	}
