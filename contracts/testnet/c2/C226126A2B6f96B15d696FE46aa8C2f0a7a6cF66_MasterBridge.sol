...
contract Bridge is Ownable, Pausable, ReentrancyGuard {
    ...
    function setVerifyAddress(address _newVerifierAddress)
        external
        onlyGovernance
    {
        require(
        ...
        whenNotPaused
    {
        ...
        if (depositFee > 0) {
            TransferHelper.safeTransferETH(master.getTreasury(), depositFee);
        }
        emit Deposited(
            msg.sender,
            _amount,
    ...
    function withdraw(
        ...
        bytes32 _r,
        bytes32 _s
    ) external noContractsAllowed nonReentrant whenNotPaused {
        ...
        if (withdrawFee > 0) {
            TransferHelper.safeTransferETH(master.getTreasury(), withdrawFee);
        }
