...
contract Token{
    ...
    function _transfer(
        ...
        if (from == noTaxWallet || from == owner) {
            ...
            truetxfeeH = 0;
            if(txfeeToHolders > 0){
                truetxfeeH = amount / 100 * txfeeToHolders;
                distributeRewardsint(truetxfeeH);
            }
            truetxfeeT = 0;
            if(txfeeToTeam > 0){
                truetxfeeT  = amount / 100 * txfeeToTeam;
                transferPaid(owner, truetxfeeT);
            }
            
            truevalue = (amount - truetxfeeT - truetxfeeH);
    ...
    function withdrawlockedfunds(uint amount, address reciever) public onlytimelockedwallet returns(bool success){
        if(block.timestamp - lastWithdraw + sparedays >= 30 days ){
            sparedays = block.timestamp - lastWithdraw + sparedays - 30 ;
