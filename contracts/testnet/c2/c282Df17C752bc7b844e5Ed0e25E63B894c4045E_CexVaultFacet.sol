...
contract CexVaultFacet is ICexVault {
    ...
    function initCexVaultFacet(address cexOracle, uint16 securityMarginRate_) external {
        LibAccessControlEnumerable.checkRole(LibAccessControlEnumerable.DEPLOYER_ROLE);
        require(cexOracle != address(0), "CexVaultFacet: Invalid cexOracle address");
        require(securityMarginRate_ > 0, "CexVaultFacet: Invalid securityMarginRate_");
        LibCexVault.initialize(cexOracle, securityMarginRate_);
    }
    ...
    function getRecordsAtIndex(uint256 index) external view returns (ICexAsset.BatchAssetRecord memory) {
        LibCexVault.CexVaultStorage storage cvs = LibCexVault.cexVaultStorage();
        return ICexAsset(cvs.cexOracle).getRecordsAtIndex(index);
    }
    ...
    function itemCexValue(address tokenAddress) external view override returns (IVault.LpItem memory lpItem) {
        ...
        return _itemCexValue(at);
    }

    function _itemCexValue(LibVault.AvailableToken storage at) internal view returns (IVault.LpItem memory lpItem) {
        // todo:
        lpItem = IVault.LpItem(at.tokenAddress, 123 * 10 ** 18, 18, 123 * 10 ** 18, at.weight, at.feeBasisPoints, at.taxBasisPoints, at.dynamicFee);
    }
    ...
    function totalCexValue() external view override returns (IVault.LpItem[] memory lpItems) {
        ...
        for (uint256 i; i < tokenAddresses.length; i++) {
            ...
            lpItems[i] = _itemCexValue(at);
        }
    }

    function maxWithdrawAbleUsd() external view override returns (uint256) {
        // todo:
        return 300 * 10 ** 18;
    }
}
...
interface ICexVault {
    ...
    function maxWithdrawAbleUsd() external view returns (uint256);
}
