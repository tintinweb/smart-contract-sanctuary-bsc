...
contract Project is IProject, Validatable, ReentrancyGuardUpgradeable {
     ...
     */
    function setMaxCollectionInProject(uint256 _maxCollectionInProject)
        external
        onlyAdmin
        notZero(_maxCollectionInProject)
    {
    ...
    function createProject(
        ...
        for (uint256 i = 0; i < _collections.length; i++) {
            require(
                ...
                "Invalid address collection or collection is already in use"
            );
    ...
    function removeProject(uint256 _projectId) external validProjectId(_projectId) onlyProjectOwner(_projectId) {
        ...
        for (uint256 i = 0; i < collectionAddress[_projectId].length(); i++) {
            ...
            uint256 _amount = IClaimPool(projects[_projectId].claimPool).getReward(collectionAddress[_projectId].at(i));
            if (_amount > 0) {
                IClaimPool(projects[_projectId].claimPool).withdrawCollection(
    ...
    function addCollections(
        ...
        require(isProjectActive(_projectId), "Project deleted");
        require(_collections.length > 0, "Invalid collection length");
        require(
    ...
    function removeCollection(
        ...
        uint256 _amount = IClaimPool(projects[_projectId].claimPool).getReward(_collectionAddress);
        if (_amount > 0) {
            IClaimPool(projects[_projectId].claimPool).withdrawCollection(_collectionAddress, _msgSender(), _amount);
        }
        ...
        notZero(_amount)
    {
        ...
        IClaimPool(projectInfo.claimPool).addBudget(_collection, _amount);
     ...
     */
    function updatePercent(uint256 _projectId, uint256[] memory percents)
        public
        nonReentrant
        validProjectId(_projectId)
        onlyProjectOwner(_projectId)
    ...
    function _splitBudget(ProjectInfo storage _projectInfo, uint256 _amount) private {
        for (uint256 i = 0; i < collectionAddress[_projectInfo.projectId].length(); i++) {
            ...
            IClaimPool(_projectInfo.claimPool).addBudget(collectionAddress[_projectInfo.projectId].at(i), newReward);
        }
    }
    ...
    function getRewardOf(address collection) external view returns (uint256) {
        uint256 projectId = collectionToProjects[collection];
        if (projects[projectId].claimPool == address(0) || !isProjectActive(projectId)) {
            return 0;
        }
        return IClaimPool(projects[projectId].claimPool).getReward(collection);
    }
}
