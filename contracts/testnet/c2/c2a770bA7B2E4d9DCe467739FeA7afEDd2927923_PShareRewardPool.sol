...
contract PShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PShare to distribute per block.
        uint256 lastRewardTime; // Last time that pshare distribution occurs.
        uint256 accPSharePerShare; // Accumulated pshare per share, times 1e18. See below.
    ...
    uint256 public psharePerSecond = 0.00128253 ether; // 41000 pshare / (370 days * 24h * 60min * 60s)
    ...
    // View function to see pending PShare on frontend.
    ...
    // Safe pshare transfer function, just in case if rounding error causes pool to not have enough pshare.
