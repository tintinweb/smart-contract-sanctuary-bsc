...
contract ZwangLocker is Ownable, ReentrancyGuard {
    ...
    uint256 public EthFee = 10000000000000000;
    ...
    // withdrawable values
    uint256 public remainingFees = 0;
    ...
    function lockTokens(
        ...
        if (_feeInEth) {
            ...
            remainingFees = remainingFees.add(msg.value);
        } else {
    ...
    function withdrawFees(address payable withdrawalAddress) external onlyOwner {
        if (remainingFees > 0) {
            withdrawalAddress.transfer(remainingFees);
            remainingFees = 0;
        }
