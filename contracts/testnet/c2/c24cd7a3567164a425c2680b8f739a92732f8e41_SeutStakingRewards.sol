...
interface IStakingRewards {
    ...
    function stakingToken(uint i) external  view returns (address);

    function rewardsDistribution() external  view returns (address);

    function balanceOf(address account) external  view returns (uint256[2] memory);

    function withdrawTime(address account, uint i) external  view returns (uint256);

    function getRewardTime(address account, uint i) external  view returns (uint256);

    function userRewarded(address account, uint i) external  view returns (uint256);
    
    function earned(address account) external view returns (uint256[2] memory);
...
contract SeutStakingRewards is RewardsDistributionRecipient {
    ...
    uint[2] withdrawCycle = [0 , 0];
    mapping(address => uint256[2]) public withdrawTime;

    uint[] getRewardCycle = [0, 0];
    mapping(address => uint256[2]) public getRewardTime;

    // address destroyAddress = address(0x000000000000000000000000000000000000dEaD);
    ...
    constructor(
        ...
        stakeReward = IStakingRewards(_stake);
        rewardsToken = IERC20(stakeReward.rewardsToken());
        stakingToken = [IERC20(stakeReward.stakingToken(0)), IERC20(stakeReward.stakingToken(1))];
        rewardsDistribution = stakeReward.rewardsDistribution();
    ...
    function getEarned(address account) external view returns (uint256[2] memory) {
        ...
        if(!isOthers(account)){
            uint rate = 100;
            // rate = rate.sub(blackRate).sub(otherRate);
            rate = rate.sub(otherRate);
            return [
                _earned[0].mul(rate).div(100),
                _earned[1].mul(rate).div(100)
            ];
        }else{
            return [
                _earned[0],
    ...
    function migrate() external  {
        ...
        require(!isMigrate[sender], 'already migrated');

        _balances[sender] = stakeReward.balanceOf(sender);
        withdrawTime[sender] = [stakeReward.withdrawTime(sender, 0), stakeReward.withdrawTime(sender, 1)];
        getRewardTime[sender] = [stakeReward.getRewardTime(sender, 0), stakeReward.getRewardTime(sender, 1)];

        rewards[sender] = stakeReward.earned(sender);

        userRewarded[sender] = [stakeReward.userRewarded(sender, 0), stakeReward.userRewarded(sender, 1)];

        _totalSupply = [_totalSupply[0].add(_balances[sender][0]), _totalSupply[1].add(_balances[sender][1])];
    ...
    function stake(uint256 amountLp, uint256 amountSeut) external updateReward(msg.sender) {
        ...
        if(0 < amountLp){
            ...
            if(0 == getRewardTime[msg.sender][0]){
                getRewardTime[msg.sender][0] = block.timestamp.add(getRewardCycle[0]);
            }
        }
        ...
        if(0 < amountSeut){
            ...
            if(0 == getRewardTime[msg.sender][1]){
                getRewardTime[msg.sender][1] = block.timestamp.add(getRewardCycle[1]);
            }
        }
    ...
    function getReward(uint i) public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            if(!isOthers(msg.sender)){
                // uint destoryAmount = reward.mul(blackRate).div(100);
                // rewardsToken.safeTransfer(destroyAddress, destoryAmount);

                uint rewardOthersAmount = rewardOthers(i, reward);

                reward = reward.sub(rewardOthersAmount);
            }
            
            rewardsToken.safeTransfer(msg.sender, reward);

            userRewarded[msg.sender][i] = userRewarded[msg.sender][i].add(reward);
    ...
    function rewardOthers(uint tokenNum, uint _earned) private returns(uint) {
        if(0 < others.length){
            uint rewardAmounts = _earned.mul(otherRate).div(100);
