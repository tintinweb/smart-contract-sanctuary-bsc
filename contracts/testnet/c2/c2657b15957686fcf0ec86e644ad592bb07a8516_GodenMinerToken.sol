...
contract GodenMinerToken is Context, IERC20, Ownable{
    ...
    function setFees(uint256 liquidityFee, uint256 marketingFee) public onlyOwner{
        require(liquidityFee + marketingFee <=25, "Too High Fee");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal{
        ...
        if(inSwapAndLiquify || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient] || recipient != pair){
            return _basicTransfer(sender,recipient,amount);
        }

        //transfer and take fee 
        uint256 feeAmount = amount.div(100).mul(_totalFee);
        uint256 liquidityAmount = amount.div(100).mul(_liquidityFee);
    ...
    function shouldAddLiquidity() internal view returns (bool) {
        return _autoAddLiquidity && !inSwapAndLiquify && _msgSender() != pair &&
         block.timestamp >= (_lastAddLiquidityTime + 1 minutes);
    }

    function addLiquidity() internal lockTheSwap {
        uint256 autoLiquidityAmount = _balances[_autoAddLiquidityReceiver];
        _balances[address(this)] = _balances[address(this)].add(
            _balances[_autoAddLiquidityReceiver]
        );
