...
contract UpdateHero {
  ...
  function updateHero (uint[] calldata _tokenIds, uint _tokenId, uint8 _level, uint32 _experience, uint[3] calldata _skills) external {
    require(msg.sender == owner, "UpdateHero: Invalid caller.");
    heroNFT.updateHeroInfo(_tokenId, _level, _experience, _skills);
