...
contract SRDController is Ownable {
    ...
    event DrawNFTProfits(address account, uint256 NFTType, uint256 profit);
    ...
    constructor() {
        ...
        SRD = ERC20(0xC9CCbd6f07Ce5aC9Bf1803Ae1378D3A8D92E828B);

        // USDT = ERC20(0x55d398326f99059fF775485246999027B3197955);
        USDT = ERC20(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);

        SRDN = SRDNFT(0x86187C68cB29054F73B694f656Bd1C59ab59d9cE);

        pair = address(0x6da4ae47989e19eA32Dd2eB3ec0D89C299a92672);
    ...
    function drawNFTProfits() public {
        ...
        for(i = 1; i < tokenIds.length; i++){
            ...
            if(SRDN.tokenIdToDrawTimes(tokenIds[i]) < SRDN.typeToDrawTimesLimit(types)){
                (profit0, drawTimes) = queryNFTProfit(tokenIds[i]);

                profit += profit0;

                typeToNFTProfits[types] += profit0;

                SRDN.addNFTProfit(tokenIds[i], profit0);

                SRDN.addNFTDrawTimes(tokenIds[i], drawTimes);

                SRDN.setNFTDrawTime(tokenIds[i], SRDN.tokenIdToDrawTime(tokenIds[i]) + drawTimes.mul(SRDN.typeToDrawTime(types)));
            }
        }
        ...
        emit DrawNFTProfits(msg.sender, types, profit);
    }
    ...
    function drawNFTProfit(uint256 tokenId) public {
        ...
        (uint256 profit,uint256 drawTimes) = queryNFTProfit(tokenId);
    ...
    function queryNFTProfit(uint256 tokenId) public view returns(uint256 profit, uint256 times){
    ...
    function queryNFTProfit(address user) public view returns(uint256 profit) {
        ...
        for(i = 1; i < tokenIds.length; i++){
            ...
            if(SRDN.ownerOf(tokenIds[i]) == user && SRDN.tokenIdToDrawTimes(tokenIds[i]) < SRDN.typeToDrawTimesLimit(types)){
                (profit0,) = queryNFTProfit(tokenIds[i]);
    ...
    // function queryNodeProfit(address user) public view returns(uint256 profit) {
    ...
    function queryNodeProfit(address user) public view returns(uint256 nodeProfit, uint256 profit) {
        uint256 usernodeID = nodeIDs[user];
        nodeProfit = nodeIDToProfits[usernodeID];
        profit = nodeIDToProfits[usernodeID] - drawNodeProfits[usernodeID];
    }
    ...
    uint256[] memory nodeIds, address[] memory users) {
        ...
        bindTimes = new uint256[](uint256(amount));
        nodeIds = new uint256[](uint256(amount));
        ...
        if(nodeType == 1){
            for(i = 0; i < amount; i++){
                users[i] = invitees[user][i];
            }
        }

        if(nodeType == 2){
            uint256 amount1 = queryNodeInviteeAmount(user, 1);

            uint256 j;

            for(i = 0; i < amount1; i++){
                for(j = 0; j < amount; j++){
                    users[i] = invitees[invitees[user][i]][j];
                }
            }
        }
        ...
        if(nodeType == 3){
            ...
            uint256 amount1 = queryNodeInviteeAmount(user, 1);
            uint256 amount2 = queryNodeInviteeAmount(user, 2);
            ...
            for(i = 0; i < amount1; i++){
                for(j = 0; j < amount2; j++){
                    for(k = 0; k < amount; k++){
                        user0 = invitees[invitees[user][i]][j];
                        users[i] = invitees[user0][k];
                    }
                }
            }
        }
    ...
    uint256[] memory nodeIds, address[] memory users, uint256[] memory status) {
        ...
        uint256 counter = SRDN.counter();

        address[] memory users0 = new address[](uint256(node1Amount));
    
        uint256 i;
        uint256 j;
        uint256 count;
        uint256[] memory tokenIds0;
        uint256[] memory tokenIds1 = new uint256[](uint256(counter));

        for(i = 0; i < node1Amount; i++){
            users0[i] = invitees[user][i];

            tokenIds0 = SRDN.queryUserNFTIDs(users0[i]);
        ...
        buyTimes = new uint256[](uint256(count));
        tokenIds = new uint256[](uint256(count));
        nodeIds = new uint256[](uint256(count));
        users = new address[](uint256(count));
        status = new uint256[](uint256(count));
    ...
    function queryMyData(address user) public view returns(uint256 nftAmount, uint256 prNftAmount, uint256 grNftAmount, uint256 unclaimedAmount, uint256 usdtAmount) {
        ...
        unclaimedAmount = queryNFTProfit(user).div(10 ** 14);
        usdtAmount = bnbEqualsToUsdt(tokenEqualsToBnb(unclaimedAmount));
    }
