...
contract StakingPools is Context, Ownable {
    using SafeMath for uint256;
    ...
    struct LockCount {
        uint256 noLockCount;
        uint256 lockCountHalf;
        uint256 lockCountFull;
        uint256 noLockTotal;
        uint256 lockHalfTotal;
        uint256 lockFullfTotal;
        uint256 totalInPool;
    }
    ...
    uint256 public minThreshold = 100 * 10**_decimals; // 6k token
    ...
    modifier onlyStakingIsOpen() {
        require(openStaking, "Staking Pool: staking is not open yet.");
        _;
    }
    ...
    function deployNewPool(uint8 poolNo_, string memory name,  uint256 minStake_, uint256 apyNoLock, uint256 apyHalfLock, uint256 apyFullLock, uint256 maxStakers) public onlyOwner {
        require(pools[poolNo_].pool == 0, "Pool already present.");
        pools[poolNo_] = Pool(poolNo_, name, maxStakers, minStake_, apyNoLock, apyHalfLock, apyFullLock);
    ...
    function reinvestReturn(uint poolNo) public  {
        require(canReinvest, "StakingPool: Reinvesting is disabled.");
        Staker memory staker = stakers[_msgSender()][poolNo];
        require(staker.amount > 0, "Sorry! you have not staked anything.");
        //require(block.timestamp >= staker.stakeTime.add(_getLockPeriod(staker.apyTime)), "StakingPool: staking period is not over yet.");
    ...
    function updatePool(uint8 poolNo_, string memory rarity_, uint256 stakeSize_, uint256 minStake_, uint256 apyNoLock_, uint256 apyHalfLock_, uint256 apyFullLock_) public onlyOwner {
        ...
        pool.apyHalfLock = apyHalfLock_;
        pool.apyFullLock = apyFullLock_;
    ...
    function updatePoolApys(uint8 poolNo_, uint256 apyNoLock_, uint256 apyHalfLock_, uint256 apyFullLock_) external onlyOwner {
        Pool memory pool = pools[poolNo_];
        pool.apyNoLock = apyNoLock_;
        pool.apyHalfLock = apyHalfLock_;
        pool.apyFullLock = apyFullLock_;
    ...
    function updateStakingPeriods(uint256 halfLock_, uint256 fullLock_) external onlyOwner {
        require(halfLock_ <= 30 days, "StakingPool: should be less than 30 days");
        require(fullLock_ <= 60 days, "StakingPool: should be less than 60 days");
        halfLock = halfLock_;
        fullLock = fullLock_;
    }
    ...
    function addLockCount(uint poolNo, uint256 apy, uint256 amount) internal {
        ...
        if(apy == 1) { lockCount.lockCountHalf += 1; lockCount.lockHalfTotal += amount;}
        if(apy == 2) { lockCount.lockCountFull += 1; lockCount.lockFullfTotal += amount;}
    ...
    function subLockCount(uint poolNo, uint256 apy, uint256 amount) internal {
        ...
        if(apy == 1) { lockCount.lockCountHalf = (lockCount.lockCountHalf != 0) ? lockCount.lockCountHalf.sub(1): 0; lockCount.lockHalfTotal = (lockCount.lockHalfTotal >= amount) ? lockCount.lockHalfTotal.sub(amount): 0; }
        if(apy == 2) { lockCount.lockCountFull = (lockCount.lockCountFull != 0) ? lockCount.lockCountFull.sub(1) : 0; lockCount.lockFullfTotal = (lockCount.lockFullfTotal >= amount) ? lockCount.lockFullfTotal.sub(amount): 0; }
    ...
    function getTotalLockCount() public view returns(LockCount memory lockCountTotal) {
        ...
        for(uint i = 1; i <= 5; i++) {
            ...
            lockCountTotal.lockCountHalf += lockCount.lockCountHalf;
            lockCountTotal.lockCountFull += lockCount.lockCountFull;
            lockCountTotal.noLockTotal += lockCount.noLockTotal;
            lockCountTotal.lockHalfTotal += lockCount.lockHalfTotal;
            lockCountTotal.lockFullfTotal += lockCount.lockFullfTotal;
    ...
    function relock(uint256 apyTime_, uint poolNo_) external {
        Staker memory staker = stakers[_msgSender()][poolNo_];
        require(staker.amount != 0, "StakingPool: caller has no staking in this pool");
    ...
    function approveTESTTOKEN(address spender, uint256 amount) external onlyOwner {
