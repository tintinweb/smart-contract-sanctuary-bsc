...
interface LMSTracker {
    ...
    function updateLmsToken(address newLmsToken) external;
...
contract ZIBU is ERC20, Ownable {
    ...
    uint256 public gasForLMS = 200000; //! test
    ...
    function swapAndSendLMS(uint256 amount) private{
        ...
        uint256 balanceLMSToken = IERC20(lmsToken).balanceOf(address(this));
        bool success = IERC20(lmsToken).transfer(address(lmsTracker), balanceLMSToken);

        if (success) {
            lmsTracker.deposit(balanceLMSToken);
            emit DepositLMS(balanceLMSToken);
        }
    }
    ...
    function setExcludedFromLMS(address account, bool value) external onlyOwner {
        _isExcludedFromLMS[account] = value;
        lmsTracker.removeFromLMS(account);
    }

    function setLMSToken(address _newLMSToken) external onlyOwner {
        lmsTracker.updateLmsToken(_newLMSToken);
    }

    function activateNewLmsToken(address _newLMSToken) external {
        require(msg.sender == address(lmsTracker), "Only owner or LMS tracker can activate new LMS token");
        lmsToken = _newLMSToken;
    }
    ...
    function updateGasForLMS(uint256 newGas) external onlyOwner {
        require(newGas >= 100000 && newGas <= 300000, "gasForLMS must be between 100,000 and 300,000");
