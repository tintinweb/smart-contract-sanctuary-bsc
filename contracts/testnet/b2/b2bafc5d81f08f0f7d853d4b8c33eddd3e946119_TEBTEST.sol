...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    uint256 public teamSellStart = 	1651551780; //Team tokens can be transferred after specific time has passed
    ...
    function _transfer(
        ...
        if (_isTeamWallet[from] && to != owner())
            ...
            require (block.timestamp > TokenTransfer[from] + sellLimit && TokenTransfer[from] > 0,
                        "Team Wallet can only use transfer once every 5 minutes."
            );
