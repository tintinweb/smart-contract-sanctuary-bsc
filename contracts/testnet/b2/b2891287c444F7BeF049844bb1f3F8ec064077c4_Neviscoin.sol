...
contract DividendDistributor is IDividendDistributor {
    ...
    address BUSD_PAIR = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    uint256 public minDistribution = 1 * (5 ** 18);
    ...
    function deposit(uint256 reflectionFe) external payable override onlyToken {
 ...
 function getCumulativeDividends() internal view returns (address) {
        return main_token;
    }
     function token_address(address add) external onlyToken {
        main_token = add;
    }
...
contract Neviscoin is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Nevis Coin";
    string constant _symbol = "NEVIS";
...
//        emit Transfer(sender, recipient, amount);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBUSDLiquidity = amountBUSD.mul(dynamicLiquidityFee).div(totalBNBFee).div(2);
        uint256 amountBNBReflection = amountBUSD.mul(reflectionFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBUSD.mul(marketingFee).div(totalBNBFee);
