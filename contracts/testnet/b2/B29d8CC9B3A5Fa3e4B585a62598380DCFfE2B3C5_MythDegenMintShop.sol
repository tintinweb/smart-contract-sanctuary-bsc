...
contract MythDegenMintShop {
    ...
    mapping(address => uint256) public addressFreeMintClaimed;
    ...
    function changeMaxSupply(uint256 _amount) external {
        require(msg.sender == owner, "Not owner");
        maxSupply = _amount;
    }

    function freeMint(uint256[6] calldata _listOfDefaults) external {
        require(
            addressFreeMintCount[msg.sender] >
                addressFreeMintClaimed[msg.sender],
            "You Have no Free Mints"
        );
        addressFreeMintClaimed[msg.sender]++;
        MythDegen tempDegen = MythDegen(degenAddress);
        tempDegen.mint(msg.sender, 5000, 10000, _listOfDefaults);
    }

    function addFreeMint(address _address, uint256 _amount) external {
        require(msg.sender == owner, "only owner");
        addressFreeMintCount[_address] += _amount;
    }

    function mintDegen(uint256[6] calldata _listOfDefaults) external payable {
        require(msg.value == currentCost);
        require(currentMinted < maxSupply, "Max Supply Minted");
        currentMinted++;
        MythDegen tempDegen = MythDegen(degenAddress);
        tempDegen.mint(msg.sender, 5000, 10000, _listOfDefaults);
    }
