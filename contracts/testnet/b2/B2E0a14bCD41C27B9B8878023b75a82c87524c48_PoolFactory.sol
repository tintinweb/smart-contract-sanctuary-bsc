...
contract PoolLogic is ReentrancyGuardUpgradeable {
    ...
    address public immutable BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    /// @dev PoolFactory set PoolManagerLogic
    ...
    function previewWithdrawal(address _investor, bool _isStablecoin, uint256 _unitOut)
        external
        view
        returns (previewWithdrawnAsset[] memory withdrawnAssets)
    {
        require(_unitOut > 0, "require number of unit");

        uint256 numberOfUnit = units[_investor];
            ...
            if (withdrawnAmount > 0) {
                ...
                if (_isStablecoin) {
                    ...
                    if (!IPoolManagerLogic(poolManagerLogic).isDepositAsset(asset)) {
                        address[] memory path = _getPathSwap(asset, BUSD);
                        uint256[] memory amountOuts = IPancakeRouter02(pancakeswapRouter).getAmountsOut(withdrawnAmount, path);
            ...
            if (withdrawnAmount > 0) {
                if (!_isStablecoin) {
                    ...
                    if (IPoolManagerLogic(poolManagerLogic).isDepositAsset(asset)) {
                        ...
                        // swap to stable coin
                        _swapForWithdrawal(asset, BUSD, withdrawnAmount, msg.sender);
                    }
                }
    ...
    function _swap(
        ...
        IPancakeRouter02(pancakeswapRouter).swapExactTokensForTokens(_amountIn, _amountOutMin, _path, _to, block.timestamp);
...
contract PoolManagerLogic is Initializable, IPoolManagerLogic, ISupportedAsset {
    ...
    /// @dev harcode - cannot update any asset
