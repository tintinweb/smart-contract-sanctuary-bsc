...
contract RewardsRankingFound is Ownable, ReentrancyGuard {
    ...
    function updateAddresses() external onlyOwner {
        rankingContract = gameAddresses.getRankingContract();
        pvpRewardAddress = gameAddresses.getRewardsPvPAddress();
        pveRewardAddress = gameAddresses.getWinRewardsAddress();
    }
    ...
    function balancerToWinPveReward() external onlyOwner {
        require(pveRewardAddress != address(0x0),"Not setted");
        require(block.timestamp >= unlockBalancerTimestamp, "too soon");
        require(BZAI.transfer(pveRewardAddress, BZAI.balanceOf(address(this)) / 10));
    }
