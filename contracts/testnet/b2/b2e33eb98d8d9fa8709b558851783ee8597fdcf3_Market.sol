...
interface IMarket {
    ...
    event DeleteOrders(uint256[] orderIds);
...
contract Market is Ownable, IMarket {
    ...
    mapping(uint256 => bool) public tokenSellingStatuses;
    ...
    modifier NFTContractMustSet() {
      ...
      modifier ERC20TokenAddressMustSet() {
        require(address(TokenAddress) != address(0), "Address NFT must set");
        _;
    }
    ...
    function sellToken(uint256 tokenId, uint256 price, uint32 expireTime) override NFTContractMustSet public {
        ...
        require(NFTAddress.ownerOf(tokenId) == msg.sender, "You are not owner of this token");
        totalOrders += 1;
    ...
    function isApproved(uint256 tokenId) NFTContractMustSet view public returns(bool) {
        require(NFTAddress.getApproved(tokenId) == address(this), "Has not approved yet");

        return true;
    }

    function isAllowance(address owner, address spender, uint256 amount) ERC20TokenAddressMustSet view public returns(bool) {
        require(TokenAddress.allowance(owner, spender) >= amount, "Has not allowed");
    ...
    function buyTokenBySelectedToken(uint256 orderId) ERC20TokenAddressMustSet public {
        Order memory order = orders[orderId];
        uint256 tokenId = order.tokenId;
        require(isApproved(tokenId), "Token has not been approved");
        require(isAllowance(msg.sender, address(this), order.price), "Token has not allowed");
        require(TokenAddress.balanceOf(msg.sender) >= order.price, "You must pay greater or equal selling price");
        require(block.timestamp <= order.expireTime, "This token is not available");
        require(order.status == OrderStatus.SELLING, "Token has not been sold");
        ...
        orders[orderId].updatedTime = block.timestamp;
        orders[orderId].receiver = msg.sender;
        TokenAddress.transferFrom(msg.sender, owner(), order.price);
        NFTAddress.safeTransferFrom(owner(), msg.sender, tokenId);
    }
    ...
    function cancelSellToken(uint256 orderId) override NFTContractMustSet public {
        ...
        orders[orderId].status = OrderStatus.CANCEL;
        orders[orderId].updatedTime = block.timestamp;
        tokenSellingStatuses[tokenId] = false;
        NFTAddress.safeTransferFrom(owner(), msg.sender, tokenId);
    ...
    function deleteOrderIds(uint256[] memory orderIds) public onlyOwner {
        for (uint256 i = 0; i < orderIds.length; i++) {
            delete orders[orderIds[i]];
        }

        emit DeleteOrders(orderIds);
    }
}