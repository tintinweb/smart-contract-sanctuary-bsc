...
contract Marketplace is ReentrancyGuard {
    ...
    uint256 private listingFee = 1 gwei;
    ...
    /**
     * @dev Creates a market item listing, requiring a listing fee and transfering the ISTEPNft token from
    ...
    function createMarketItem(
        ...
        address creator = ISTEPNft(nftContractAddress).getTokenCreatorById(tokenId);
    ...
    /**
     * @dev Creates a market sale by transfering msg.sender money to the seller and ISTEPNft token from the
...
contract ISTEPNft is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable {
    ...
    function setBaseURI(string memory baseTokenURI) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ISTEPNft admin role required");
        _baseTokenURI = baseTokenURI;
    }

    function setupMinterRole(address account, bool _enable) external {
        require(account != address(0), "ISTEPNft account cann't equal address 0x");
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ISTEPNft admin role required");
        if (_enable) {
    ...
    function mint(address toAddress) external returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ISTEPNft minter role required");
        require(toAddress != address(0), "ISTEPNft cann't mint zero address");
