...
function topUp(uint256 _amount) public{
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
        ...
        if (sent) {
            ...
            if (_level < 3 && users[referer].referrerID >= 1) {
      ...
      function withdrawROI() public {
       ...
       uint256 reward = users[userList[users[msg.sender].referrerID]].stakedToken + ((users[userList[users[msg.sender].referrerID]].stakedToken * numDays * 1 ) / 10000 );
       token.unfreezeToken(msg.sender,reward);
       users[userList[users[msg.sender].referrerID]].stakedToken=users[userList[users[msg.sender].referrerID]].stakedToken-reward;
       ...
       function withdrawableROI(address add) public view returns(uint256 reward) {

        uint256 numDays = (block.timestamp - freezeTime[add]);
              // Increase the reward by 0.01% per day
        reward = users[userList[users[add].referrerID]].stakedToken + ((users[userList[users[add].referrerID]].stakedToken * numDays * 1 ) / 10000 );
