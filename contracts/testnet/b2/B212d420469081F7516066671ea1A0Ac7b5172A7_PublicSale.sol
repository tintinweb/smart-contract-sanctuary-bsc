...
contract PublicSale is Ownable, Pausable {
	...
	IERC20 public paymentToken = IERC20(0x55d398326f99059fF775485246999027B3197955);
	...
	uint256 public tokenPriceRate = 19000000000000000;
	uint256 public minBuy = 10000000000000000000; // $10
	uint256 public maxBuy = 50000000000000000000; // $50
	uint256 public openSale = 1682899200; // 2023-05-01 00:00:00
	uint256 public endSale = 1684195199; // 2023-05-15 23:59:59
	...
	function buy(uint256 _paymentAmount) public whenNotPaused {
		require(totalBuy(msg.sender) + _paymentAmount >= minBuy && totalBuy(msg.sender) + _paymentAmount <= maxBuy, "limit min/max buy token");
		require(block.timestamp >= openSale && block.timestamp <= endSale, "can not buy at this time");

		// check balance payment token before buy token
		require(paymentToken.balanceOf(msg.sender) >= _paymentAmount, "your balance not enough");

		uint256 totalToken = _paymentAmount.mul(1e18).div(tokenPriceRate);
		require(token.balanceOf(address(this)) >= totalToken, "contract not enough balance");
	...
	function getITransferInvestment(address _wallet) external view returns (uint256){
		...
		if (otherInvestmentContract != address(0)) {
			totalLock = totalLock.add(IToken(otherInvestmentContract).getITransferInvestment(_wallet));
		}
		totalLock = totalLock.add(userLockDetail[_wallet]);

		// unlock at TGE
		if (block.timestamp >= timeTGE) {
			totalLock = totalLock.sub(userLockDetail[_wallet].div(100).mul(tgeUnlock));
		}

		if (block.timestamp >= startUnlock) {
			uint256 unlockAmount = userLockDetail[_wallet].mul(receivePercentage).div(divPercentage).mul(
				block.timestamp.sub(startUnlock).div(timePeriod)
			);
			if (unlockAmount > 0) {
				totalLock = unlockAmount >= totalLock ? 0 : totalLock.sub(unlockAmount);
			}
		}
		return totalLock;
	}
