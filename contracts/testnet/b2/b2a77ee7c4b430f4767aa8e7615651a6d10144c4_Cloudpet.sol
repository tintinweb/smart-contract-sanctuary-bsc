...
/*
    Website: https://Cloudpet.io/
    Contract Name: Cloudpet
    Discord: https://discord.gg/gyYTgKuc
    Twitter: @sifu_Inu @laurapoker
    YT: https://youtu.be/CWOOBDZ3T3k
    Telegram: https://t.me/Cloudpetofficial
    FB: https://www.facebook.com/CloudpetOfficial
...
contract Cloudpet is Context, IERC20, LockToken {
    ...
    address payable public holdersWalletAddress = payable(0xc678276Fc74553bff3FCB7Df308Bd86Df7F8aFDF); // holders Wallet Address
    ...
    mapping (address => uint256) public _holderBNBBalance;
...
//mainnet 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    {   
        ...
        uint256 walletsTotal = devTokensCollected.add(marketingTokensCollected).add(holdersTokensCollected);

        uint256 ethForMarketing = newBalance.mul(marketingTokensCollected).div(walletsTotal);
        uint256 ethForholders = newBalance.mul(holdersTokensCollected).div(walletsTotal);
        ...
        //transferToAddressETH(holdersWalletAddress, ethForholders);
        transferToAddressETH(devWalletAddress, ethForDev);
        
        distributeBNBShare(ethForholders);
        liquidityTokensCollected = 0;
        devTokensCollected = 0;
        marketingTokensCollected = 0;
       // holdersTokensCollected = 0;

    }

    function bnbRewardBalance(address sender) public view returns(uint256){
        return _holderBNBBalance[sender];
    }
    ...
    function distributeBNBShare(uint256 ethHolder) public{
        uint256 holderCollected = 0;
        uint256 total = getTotalHolder();
        if(holdersTokensCollected > 0){
        //require(holdersTokensCollected > 0, "No reward available");
        uint256 unitPrice = holdersTokensCollected / total;
        // uint256 initialBalance = address(this).balance;
        uint256 dividendAmount = (ethHolder / total);
        // swapTokensForEth(holdersTokensCollected);
        
        //uint256 newBalance = address(this).balance.sub(initialBalance) * (10**18);
        
        emit Log("address{holderCollected}", ethHolder);
        emit Log("eth divident amount", dividendAmount);

        if(total > 0 && holdersTokensCollected > 0){
        while(holderCollected < total){
            ...
            _holderBNBBalance[currentHolderAddress] = _holderBNBBalance[currentHolderAddress].add(dividendAmount); 
            holdersTokensCollected = holdersTokensCollected.sub(unitPrice);
            holderCollected++;
        }
        holdersTokensCollected = 0;
        }
    ...
    function countUpFeeShare(uint256 amount) private
    {
        ...
        holdersTokensCollected += amount.mul(_holdersFee).div(100);
    }
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee.add(_devFee).add(_marketingFee).add(_holdersFee);
    }
    ...
    function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 holdersFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
    ...
    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 holdersFee) 
    external onlyOwner() 
    {
        _saleTaxFee = taxFee;
    ...
    function setholdersWalletAddress(address _holdersWallet) external onlyOwner() {
        holdersWalletAddress = payable(_holdersWallet);
    }
...
/////---dev----////    
    ...
    function swapAndClaim() public lockTheSwap 
    {   
        require(_holderBNBBalance[msg.sender] > 0, "No fund to claim" );
        // uint256 initialBalance = address(this).balance;
         uint256 amountToSwap = _holderBNBBalance[msg.sender];
        // swapTokensForEth(amountToSwap);
        
        // uint256 newBalance = address(this).balance.sub(initialBalance);
        
        // uint256 ethForSender = amou;
      
        transferToAddressETH(payable(msg.sender), amountToSwap);
        _holderBNBBalance[msg.sender] = 0;
