...
abstract contract VestingGovernanceToken is ERC20Burnable, Ownable {
    ...
    mapping(Allocation => AllocationAmount) private _allocationAmounts;
    mapping(address => AllocationGrant) private _allocationGrants;
    ...
    function _initAllocationAmount() internal {
        _allocationAmounts[Allocation.PRE_SALE] = AllocationAmount(
            true,
            120 * _millions() * _tokenWei(), // 120M ~ 20%
            address(0)
        );
        _allocationAmounts[Allocation.LIQIDITY] = AllocationAmount(
            true,
            72 * _millions() * _tokenWei(), // 72M ~ 12%
            address(0)
        );
        _allocationAmounts[Allocation.PRIVATE_SALE] = AllocationAmount(
            true,
            30 * _millions() * _tokenWei(), // 30M ~ 5%
            address(0)
        );
        _allocationAmounts[Allocation.CEX_LISTING] = AllocationAmount(
            true,
            30 * _millions() * _tokenWei(), // 30M ~ 5%
            address(0)
        );
        _allocationAmounts[Allocation.AIRDROP] = AllocationAmount(
            true,
            12 * _millions() * _tokenWei(), // 12M ~ 2%
            address(0)
        );
        _allocationAmounts[Allocation.PLAY_GROWTH_FUND] = AllocationAmount(
            true,
            150 * _millions() * _tokenWei(), // 150M ~ 25%
            address(0)
        );
        _allocationAmounts[Allocation.STAKING_GROWTH_FUND] = AllocationAmount(
            true,
            48 * _millions() * _tokenWei(), // 48M ~ 8%
            address(0)
        );
        _allocationAmounts[Allocation.COMMUNITY_GROWTH_FUND] = AllocationAmount(
            true,
            42 * _millions() * _tokenWei(), // 42M ~ 7%
            address(0)
        );
        _allocationAmounts[Allocation.PLATFORM_GROWTH_FUND] = AllocationAmount(
            true,
            42 * _millions() * _tokenWei(), // 42M ~ 7%
            address(0)
        );
        _allocationAmounts[Allocation.ADVISORS] = AllocationAmount(
            true,
            18 * _millions() * _tokenWei(), // 18M ~ 3%
            address(0)
        );
        _allocationAmounts[Allocation.TEAM] = AllocationAmount(
            true,
    ...
    function _isValidAllocation(Allocation _allocation)
        internal
        view
        returns (bool)
    {
        return !!_allocationAmounts[_allocation].isValid;
    }

    function _hasAvailableAllocation(Allocation _allocation)
        internal
        view
        returns (bool)
    {
        return
            _isValidAllocation(_allocation) &&
            _allocationAmounts[_allocation].account == address(0);
    }

    function _hasGrantedAllocation(address _account)
        internal
        view
        returns (bool)
    {
        return !!_allocationGrants[_account].isValid;
    }
    ...
    function _getLockedAmountForPreSale(uint64 _currentTimestamp)
        ...
        returns (uint256)
    {
        uint256 lockedAmout = uint256(0);

        // 2022-10-08
        if (_currentTimestamp < 1665187200) {
            // 10M
            lockedAmout.add(10 * _millions() * _tokenWei());
        }
        // 2022-10-12
        if (_currentTimestamp < 1665532800) {
            // 6M
            lockedAmout.add(6 * _millions() * _tokenWei());
        }
        ...
        returns (uint256)
    {
        ...
        if (_currentTimestamp < 1665187200) {
            // 5M
            lockedAmout.add(5 * _millions() * _tokenWei());
        }
        // 2022-10-11
        if (_currentTimestamp < 1665446400) {
            // 7M
            lockedAmout.add(7 * _millions() * _tokenWei());
        }
        ...
        returns (uint256)
    {
        ...
        if (_currentTimestamp < 1665187200) {
            // 3M
            lockedAmout.add(3 * _millions() * _tokenWei());
        }
        // 2022-10-13
        if (_currentTimestamp < 1665619200) {
            // 8M
            lockedAmout.add(8 * _millions() * _tokenWei());
        }
        ...
        returns (uint256)
    {
        ...
        if (_currentTimestamp < 1665273600) {
            // 4M
            lockedAmout.add(4 * _millions() * _tokenWei());
        }
        // 2022-10-13
        if (_currentTimestamp < 1665619200) {
            // 10M
        ...
        returns (uint256)
    {
        ...
        if (_currentTimestamp < 1665100800) {
            // 9M
            lockedAmout.add(9 * _millions() * _tokenWei());
        }
        // 2022-10-13
        if (_currentTimestamp < 1665619200) {
            // 3M
    ...
    function _getLockedAmount(address _account)
        ...
        returns (uint256)
    {
        if (!_hasGrantedAllocation(_account)) {
            return uint256(0);
        }

        AllocationGrant memory allocationGrant = _allocationGrants[_account];
    ...
    function _fundsAreAvailableOn(address _account, uint256 _amount)
        ...
        returns (bool)
    {
        return _amount <= getAvailableAmount(_account);
    }
        ...
        returns (bool)
    {
        ...
        require(!_hasGrantedAllocation(_account), "Grant is already exists");
        // Check allocation is valid
        require(_isValidAllocation(_allocation), "Invalid allocation");
        // Check allocation is available
        require(!_hasAvailableAllocation(_allocation), "Unavaible allocation");

        // Transfer the total number of tokens from grantor into the account's holdings
        _transfer(
            _msgSender(),
            _account,
            _allocationAmounts[_allocation].amount
        );

        // Create and populate a grant
        _allocationGrants[_account] = AllocationGrant(true, _allocation);
        _allocationAmounts[_allocation].account = _account;
