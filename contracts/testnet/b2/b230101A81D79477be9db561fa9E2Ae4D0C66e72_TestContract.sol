...
contract TestContract is Context, Ownable {
    using SafeMath for uint256;

    uint256 private DOUGH_TO_HATCH_1MINERS = 1080000;//for final version should be seconds in a day
    ...
    function sellDough() public {
        ...
        uint256 fee = devFee(doughValue);
        claimedDough[msg.sender] = 0;
        lastKnead[msg.sender] = block.timestamp;
        marketDough = SafeMath.add(marketDough,hasDough);
        splitFees(fee);
        payable (msg.sender).transfer(SafeMath.sub(doughValue,fee));
    }
    ...
    function splitFees(uint256 fees) public payable {
    ...
    function buyDough(address ref) public payable {
        ...
        doughBought = SafeMath.sub(doughBought,devFee(doughBought));
