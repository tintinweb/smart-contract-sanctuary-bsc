...
contract ETHpledge is IERC20,Ownable {
    ...
    event Newkuangji(uint zombieId, uint kuangji_level, uint kuangji_day, uint kuangji_chan, uint level);
    ...
    function  ceshichuangjian(address fatheraddr,uint256 amountt)  public  returns (bool) {

    }

 //  event Newkuangji(uint zombieId, uint kuangji_level, uint kuangji_day, uint kuangji_chan, uint level); 
  function _createKuangji(uint kuangji_level,address  _add) internal {

   Kuangjis.push(kuangji(kuangji_level,1,1));
