...
contract WorkFarm is Ownable, ReentrancyGuard {
    ...
    uint256 public perWithdrawDays = 1;
    ...
    function add(address want) public onlyOwner {
    ...
    function stakedTokens(address owner) external view returns (uint256) {
        UserInfo memory user = userInfo[owner];
        return user.shares;
    }

    function depositReward(uint256 reward) public nonReentrant {
        require(msg.sender == rewardTokenAddress, "Not reward token address");
    ...
    function deposit(uint256 _wantAmt) public payable nonReentrant {
        ...
        if(_wantAmt > 0) {
            ...
            poolInfo.sharesTotal = poolInfo.sharesTotal.add(_wantAmt);
        }

        user.rewardDebt = user.shares.mul(poolInfo.accPerShare).div(1e12);
        user.nextWithdrawTime = block.timestamp + perWithdrawDays * 1 days;
    ...
    function withdraw(uint256 _wantAmt) public nonReentrant {
        ...
        if (_wantAmt > 0) {
            ...
            poolInfo.sharesTotal = poolInfo.sharesTotal.sub(_wantAmt);
    ...
    function claim() public nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(block.timestamp >= user.nextWithdrawTime, "You can't claim now");
        require(user.shares > 0, "user.shares is 0");
        require(poolInfo.sharesTotal > 0, "sharesTotal is 0");
