...
contract SocialMusifySale {
    ...
    uint256 internal openingTime = block.timestamp;
    uint256 internal totalleft = openingTime.add(180);
    SocialMusify public token;
    address payable public wallet;
    uint256 public rate;
    uint256 public weiRaised;
    uint256 internal _timeleft;
    ...
    function endSale() public {
       ...
       // uint256 remaining = token.balanceOf(address(this));
        //token.transfer(admin, remaining);
    ...
    function secondsLeft () external view returns (uint256) {
     ...
     modifier timeleft (uint256) {
        _timeleft = totalleft.sub(block.timestamp);
        _;
    }
}