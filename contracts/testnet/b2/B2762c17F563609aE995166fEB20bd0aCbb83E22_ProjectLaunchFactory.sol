...
contract Sale is ReentrancyGuard {
            ...
            if(block.timestamp > sale_info[_round].end) {
                ...
                if(sale_details.hasVesting == 0) {
                    ...
                    if(buyer.TGE_claimed == 0) {
                        tge_claim_amount = (buyer.sale * sale_vesting[_round].TGE_amount / 100);
                    ...
                    if(block.timestamp >= sale_vesting[_round].cliff){
                        claim_amount = (buyer.sale * (sale_vesting[_round].period_amount * ((block.timestamp - sale_vesting[_round].cliff) / sale_vesting[_round].period)) / 100) - buyer.claimed;
    ...
    function changeVestingAdmin(uint256 _round, uint256[] memory _vesting) public {
        ...
        require(sale_vesting[_round].cliff > sale_info[_round].end, "Vesting too early");
        require(sale_vesting[_round].cliff >= sale_vesting[_round].TGE, "Cliff is less than TGE");
