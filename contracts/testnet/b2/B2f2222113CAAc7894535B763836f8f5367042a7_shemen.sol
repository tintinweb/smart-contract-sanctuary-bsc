...
contract shemen is Ownable{
    ...
    address public bftAddr =  0x60a25C82d256E04C12e84e26514De1400B1248d3;
   ...
   // mapping(uint => Result) private result;
    ...
    event BetEvent(address indexed sender,bytes32 reqId);
    event ResultDataEvent(address indexed sender,bytes32 reqId,uint betNum,uint randomNum,uint amount,uint income,address inviter,uint inviterIncome,uint timestamp);
    constructor()  {
        suppliers.push(gasAddr);
        supplier[gasAddr] = true;
    }
    ...
    function bet(uint amount,uint num) external payable{
       ...
       if (bnbBalance >=1e15){
        ...
        emit BetEvent(msg.sender,reqId);
    ...
    function betResult(bytes32 reqId, uint randomNum) internal{
        ...
        if (request[reqId].betNum >= randomNum){
            ...
            if (whiteAddrs[request[reqId].addr]) {
                ...
                if (inviter ==address(0)){
                    IERC20(bftAddr).transfer(feeAddr,reward * 5 /100);
                } else {
                    IERC20(bftAddr).transfer(inviter,reward * 5 /100);
                    inviterIncome = reward * 5 /100;
                }
        ...
        emit ResultDataEvent(request[reqId].addr,reqId,request[reqId].betNum,randomNum,request[reqId].amount,income,inviter,inviterIncome,block.timestamp);
