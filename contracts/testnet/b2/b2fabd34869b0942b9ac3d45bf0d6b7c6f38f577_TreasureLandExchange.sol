...
struct AdvanceOrder {
    MakerOrder orderParameter; // origin maker order
    ...
    ReentrancyGuardUpgradeable
{
    ...
    bytes32 internal constant TYPE_HASH = keccak256("MakerOrder(bool side,address signer,address policy,address payment,uint256 nonce,uint256 startTime,uint256 endTime,bytes params)");
    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(EIP712DOMAIN_TYPEHASH, keccak256("TreasureLandExchange"), keccak256("1"), block.chainid, address(this))
        );
    }
    ...
    /**
     * Constructor ERC1967 required by the openzeppelin UUPS module
     */
    function initialize(
    ...
    function matchAskWithTakerBid(
        ...
        // TODO:改成从policyManager获取
        bool isExecutionValid = IPolicy(makerAsk.policy)
        ...
        AdvanceOrder memory advanceOrder = _convertOrderToAdvancedOrder(makerAsk, askHash);

        require(takerBid.offerComponents.length == 1, "Order: taker item out of range");
        uint256 itemIndex = takerBid.offerComponents[0].itemIndex;
        require(itemIndex < advanceOrder.items.length, "Order: item index out of range");
        uint256 itemNonce = makerAsk.nonce + itemIndex;

        require(!_isUserOrderNonceExecutedOrCancelled[makerAsk.signer][itemNonce],
            "Order: order nonce is executed or cancelled"
        );

        _isUserOrderNonceExecutedOrCancelled[makerAsk.signer][itemNonce] = true;

        if (makerAsk.payment == address(0)) {
            require(msg.value >= advanceOrder.items[itemIndex].price, "Taker: increase price");
            payable(makerAsk.signer).transfer(advanceOrder.items[itemIndex].price);
        } else {
            // Execution part 1/2
    ...
    function matchBidWithTakerAsk(
        ...
        bool isExecutionValid = IPolicy(makerBid.policy)
            .canExecuteTakerAsk(takerAsk, makerBid);

        require(isExecutionValid, "Policy: Execution invalid");
        AdvanceOrder memory advanceOrder = _convertOrderToAdvancedOrder(makerBid, bidHash);

        require(takerAsk.offerComponents.length == 1, "Order: taker item out of range");
        uint256 itemIndex = takerAsk.offerComponents[0].itemIndex;
        require(itemIndex < advanceOrder.items.length, "Order: item index out of range");
        uint256 itemNonce = makerBid.nonce + itemIndex;

        require(!_isUserOrderNonceExecutedOrCancelled[makerBid.signer][itemNonce],
            "Order: order nonce is executed or cancelled"
        );
    ...
    function batchMatchAskWithTakerBid(
        ...
        for (uint256 i = 0; i < orderLength; i++) {
            ...
            _validateOrder(makerAsks[i], askHash);
            advanceOrders[i] = _convertOrderToAdvancedOrder(makerAsks[i], askHash);
        }
        ...
        for (uint256 i = 0; i < takerBid.offerComponents.length; i++) {
            ...
            require(itemIndex < advanceOrders[orderIndex].items.length, "Order: itemIndex out of range");
            uint256 itemNonce = advanceOrders[orderIndex].orderParameter.nonce + itemIndex;
            require(!_isUserOrderNonceExecutedOrCancelled[advanceOrders[orderIndex].orderParameter.signer][itemNonce],
                "Order: order is executed or cancelled"
            );
            _isUserOrderNonceExecutedOrCancelled[advanceOrders[orderIndex].orderParameter.signer][itemNonce] = true;

            if (advanceOrders[orderIndex].orderParameter.payment == address(0)) {
                require(msg.value >= advanceOrders[orderIndex].items[itemIndex].price, "Taker: increase price");
                payable(advanceOrders[orderIndex].orderParameter.signer)
                    .transfer(advanceOrders[orderIndex].items[itemIndex].price);
            } else {
                // Execution part 1/2
                _transferFeesAndFunds(
                    advanceOrders[orderIndex].orderParameter.policy,
                    advanceOrders[orderIndex].items[itemIndex].collection,
                    advanceOrders[orderIndex].items[itemIndex].tokenId,
                    advanceOrders[orderIndex].orderParameter.payment,
                    takerBid.taker,
                    advanceOrders[orderIndex].orderParameter.signer,
                    advanceOrders[orderIndex].items[itemIndex].price
                );
            }

            // Execution part 2/2
            _transferNonFungibleToken(
                advanceOrders[orderIndex].items[itemIndex].itemType,
                advanceOrders[orderIndex].items[itemIndex].collection,
                advanceOrders[orderIndex].orderParameter.signer,
                takerBid.taker,
                advanceOrders[orderIndex].items[itemIndex].tokenId,
                advanceOrders[orderIndex].items[itemIndex].amount
            );

            emit TakerBid(
                advanceOrders[orderIndex].orderHash,
                itemNonce,
                takerBid.taker,
                advanceOrders[orderIndex].orderParameter.signer,
                advanceOrders[orderIndex].orderParameter.policy,
    ...
    function batchMatchBidWithTakerAsk(
        ...
        for (uint256 i = 0; i < orderLength; i++) {
            ...
            _validateOrder(makerBids[i], bidHash);
            advanceOrders[i] = _convertOrderToAdvancedOrder(makerBids[i], bidHash);
        }
    ...
    function batchTransferNFT(
        ...
        for (uint256 i = 0; i < erc721nfts.length; i++) {
            ...
            for (uint256 j = 0; j < erc721nfts[i].tokenIds.length; j++) {
                erc721.safeTransferFrom(msg.sender, to, erc721nfts[i].tokenIds[j]);
            }
        }

        for (uint256 i = 0; i < erc1155nfts.length; i++) {
            IERC1155 erc1155 = (IERC1155)(erc1155nfts[i].collection);
            erc1155.safeBatchTransferFrom(msg.sender, to, erc1155nfts[i].tokenIds, erc1155nfts[i].amounts, "");
        }
    }
    ...
    function batchCancelOrders(uint256[] calldata orderNonces) external override {
        for (uint256 i = 0; i < orderNonces.length; i++) {
            cancelOrder(orderNonces[i]);
        }
    }

    function hashStruct(MakerOrder memory makerOrder) public pure returns (bytes32) {
        return keccak256(abi.encode(TYPE_HASH, makerOrder.side, makerOrder.signer, makerOrder.policy, makerOrder.payment, makerOrder.nonce, makerOrder.startTime, makerOrder.endTime, keccak256(bytes(makerOrder.params))));
    }
    ...
    function _validateOrder(MakerOrder calldata makerOrder, bytes32 orderHash) internal view {
        // Verify whether order nonce has expired
        require(makerOrder.nonce >= userMinOrderNonce[makerOrder.signer], "Order: Matching order expired");

        // Verify the signer is not address(0)
        require(makerOrder.signer != address(0), "Order: Invalid signer");

        // Verify the validity of the signature
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, hashStruct(makerOrder)));
        require(ecrecover(digest, makerOrder.v, makerOrder.r, makerOrder.s) == makerOrder.signer, "Signature: Invalid");

        // TODO:Verify whether the currency is whitelist

        // TODO:Verify whether strategy can be executed
    }
     ...
     */
    function _convertOrderToAdvancedOrder(MakerOrder calldata order, bytes32 orderHash)
        internal
        pure
        returns (AdvanceOrder memory advanceOrder)
    {
