...
contract MythDegen is ERC721 {
    ...
    function emitCosmeticAdded(
        uint256 _layerType,
        uint256 _layerId,
        string memory _imageURL
    ) external isDegenWhitelisted {
        emit cosmeticAdded(_layerType, _layerId, msg.sender, _imageURL);
    }
    ...
    function changeDegenCosmeticAddress(
        address _cosmeticAddress,
        uint256 _degenId
    ) external isWhitelisted {
        emit degenUnlockedCosmeticAddress(
            _cosmeticAddress,
            _degenId,
            !unlockedCosmeticAddressForDegen[_degenId][_cosmeticAddress]
        );
        unlockedCosmeticAddressForDegen[_degenId][
            _cosmeticAddress
        ] = !unlockedCosmeticAddressForDegen[_degenId][_cosmeticAddress];
    }

    function changeDegenCosmetic(
        uint256 _degenId,
        address _cosmeticAddress,
        uint256 _itemId,
        uint256 _layerId
    ) external {
        require(_layerId >= 0 && _layerId <= 6, "Enter valid layer type");
        require(
            degenStats[_degenId].owner == msg.sender,
            "Only the owner of the degen can change the Cosmetic"
        );
        require(
        ...
        require(
            unlockedCosmetics[_degenId][_cosmeticAddress][_layerId][_itemId] ||
                freeCosmetics[_cosmeticAddress][_layerId][_itemId] ||
                unlockedCosmeticAddressForDegen[_degenId][_cosmeticAddress],
            "This degen does not have this cosmetic unlocked"
        );
        emit cosmeticChange(_degenId, _layerId, _cosmeticAddress, _itemId);
        if (_layerId == 1) {
    ...
    function getDegenImage(uint256 _id) public view returns (string[9] memory) {
        ...
        tempList[5] = getMouthURL(_id);
        tempList[6] = getNoseURL(_id);
    ...
    function getImageLayers(uint256 _id) public view returns (string memory) {
        ...
        string[9] memory tempList = getDegenImage(_id);
    ...
    function setFreeCosmetics(
        ...
        for (uint256 i = 0; i < _length; i++) {
            emit cosmeticFreeAdded(_layerType, _itemIds[i], _cosmeticAddress);
            freeCosmetics[_cosmeticAddress][_layerType][_itemIds[i]] = true;
        }
    }
...
contract MythCosmetic {
    ...
    function changeEyeUrl(string[10] calldata _url, uint256[10] calldata _id)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 10; i++) {
    ...
    function changeMouthUrl(string[10] calldata _url, uint256[10] calldata _id)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 10; i++) {
    ...
    function changeNoseUrl(string[10] calldata _url, uint256[10] calldata _id)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 10; i++) {
    ...
    function changeBodyUrl(string[10] calldata _url, uint256[10] calldata _id)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 10; i++) {
    ...
    function changeHeadUrl(string[10] calldata _url, uint256[10] calldata _id)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 10; i++) {
...
contract MythCityWeapons is ERC721 {
    ...
    constructor() ERC721("Myth City Weapons", "MYTHWEP") {
        tokenCount = 1;
        owner = msg.sender;
        whitelistedAddresses[msg.sender] = true;
    }
...
contract MythCityWeaponSkins is ERC721 {
    ...
    constructor() ERC721("Myth City Weapon Skins", "WEPSKIN") {
        tokenCount = 1;
        owner = msg.sender;
        whitelistedAddresses[msg.sender] = true;
    }
