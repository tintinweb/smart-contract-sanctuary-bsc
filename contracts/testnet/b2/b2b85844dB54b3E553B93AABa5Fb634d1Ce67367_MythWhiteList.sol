...
contract MythWhiteList {
    ...
    function getRoundOfGameLobby(uint256 _lobbyId, uint256 _roundId)
        external
        view
        returns (roundStruct memory)
    {
        return roundOfGameLobby[_lobbyId][_roundId];
    }
        ...
        returns (bool)
    {
        ...
        emit gameRoundAdded(_side, _gameCount, 0, msg.value, coinflipAddress);
        if (!_pvp) {
        ...
        returns (bool)
    {
        ...
        emit gameRoundAdded(_side, _gameCount, 0, msg.value, diceduelAddress);
        if (!_pvp) {
            gameIdOfBlockNumberAndBlockNonce[block.number][
                gameCountsPerBlock[block.number]
            ] = _gameCount;
        ...
        returns (bool)
    {
        ...
        emit gameRoundAdded(_side, _gameCount, 0, msg.value, deathrollAddress);
        if (!_pvp) {
            gameIdOfBlockNumberAndBlockNonce[block.number][
                gameCountsPerBlock[block.number]
            ] = _gameCount;
        ...
        while (counter < gameCountOfBlock) {
            ...
            if (currentGame.betterWinner) {
                ...
                if (currentGame.caller == address(0)) {
                    bobBalance -= currentGame.winningAmount;
                }
            } else {
                if (currentGame.caller != address(0)) {
                    rewardBalances[currentGame.caller] += currentGame
                        .winningAmount;
                } else {
                    bobBalance += currentGame.winningAmount;
                }
