...
library Address{
 ...
 contract Test is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 200_000_000_000_000 * 10**9;
    uint256 public maxSellLimit = 100_000_000_000_000 * 10**9;
    uint256 public maxWalletLimit = 200_000_000_000_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xA47Ec2B3FD34540642050f42AB74Bcaec9B55D4e;
    address public ecosystemWallet = 0x4028fb39eEDE6baF6D3FB891e2081C62D5De98B1;
    ...
    Taxes public taxes = Taxes(0, 2, 1, 2);
    Taxes public selltax = Taxes(0, 2, 1, 2);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[ecosystemWallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[ecosystemWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _ecosystem) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_ecosystem);
        emit FeesChanged();
    }
    
    function setselltax(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _ecosystem) public onlyOwner {
       selltax = Taxes(_rfi,_marketing,_liquidity,_ecosystem);
        emit FeesChanged();
    }
    ...
    function _takeecosystem(uint256 recosystem, uint256 tecosystem) private {
        totFeesPaid.ecosystem +=tecosystem;

        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tecosystem;
        }
        _rOwned[address(this)] +=recosystem;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.recosystem) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        Taxes memory temp;
        if(isSell) temp = selltax;
        else temp = taxes;
        
        s.tRfi = tAmount*temp.rfi/100;
        s.tMarketing = tAmount*temp.marketing/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tecosystem = tAmount*temp.ecosystem/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tecosystem;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 recosystem = s.tecosystem*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-recosystem;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 recosystem) {

        if(!takeFee) {
          return(0);
        }

        recosystem = s.tecosystem*currentRate;
        return (recosystem);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && swapEnabled && canSwap && from != pair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            if(to == pair)  swapAndLiquify(swapTokensAtAmount, selltax);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tecosystem);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.recosystem > 0 || s.tecosystem > 0) _takeecosystem(s.recosystem, s.tecosystem);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.ecosystem) * 2;
        ...
        uint256 ecosystemAmt = unitBalance * 2 * temp.ecosystem;
        if(ecosystemAmt > 0){
