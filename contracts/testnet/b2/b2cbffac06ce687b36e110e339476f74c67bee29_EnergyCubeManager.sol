...
interface IWQWS{
    function buyEQforWQC(address player, uint256 wqc_amount) external;
    function sellEQforWQC(address player, uint256 EQ_id) external;
    function mintFee(address player, uint256 wqc_amount) external;
    function improveEQforWQC(address player, uint256 EQ_id, uint256 wqc_amount) external;
}
...
contract EnergyCubeManager is SafeMath {
  ...
  function buyEnergyQube(address player, uint256 amount, uint256 WQCAmount) public {
      WQWSContract.buyEQforWQC(player, WQCAmount);
  ...
  function sellEnergyQube(address player, uint256 EQ_id) public {
      //TO CLARIFY SELLING SCHEME
  }

  function mintEnergyQube(address player, uint256 amount, uint256 WQCAmount) public {
      uint256 maxMintedId = EnergyQubeContract.getMaxMintedId();
      EnergyQubeContract.mint(player, maxMintedId+1, amount);
  ...
  function addEffEnergyQube(address player, uint256 EQ_id, uint256 WQCAmount, uint256 energyAmount) public {
      WQWSContract.improveEQforWQC(player, EQ_id, WQCAmount);
      EnergyQubeContract.addEfficiency(EQ_id, energyAmount);
      WQCContract.burn(mul(WQCAmount,Z));
  }

  function addCapEnergyQube(address player, uint256 EQ_id, uint256 WQCAmount, uint256 capAmount) public {
      WQWSContract.improveEQforWQC(player, EQ_id, WQCAmount);
      EnergyQubeContract.addCapacity(EQ_id, capAmount);
      WQCContract.burn(mul(WQCAmount,Z));
  }
}