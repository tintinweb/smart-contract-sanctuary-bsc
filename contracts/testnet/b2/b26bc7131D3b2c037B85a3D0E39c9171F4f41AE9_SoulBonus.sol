...
contract SoulBonus is DivestorUpgradeable {
    ...
    uint constant DAY = 15 minutes; // 天

    uint constant BEGIN_SEARCH_DAY = 1652745600 / DAY; // 北京时间 2022-05-17 08:00:00
    ...
    // 用户上一次操作的天数
    mapping(address => uint) public sameDayQuotient;

    function initialize(address usdt_, address soulToken_, address soul1155_, address soulController_) public initializer {
        __Divestor_init();
        addr = AddrCollection({
    ...
    function _getSameDay() private returns (uint) {
        ...
        if (tempQuotient != sameDayQuotient[_msgSender()]) { // 不是当天
            _settleDay(sameDayQuotient[_msgSender()], tempQuotient); // 结算
            sameDayQuotient[_msgSender()] = tempQuotient;
        }
        return tempQuotient;
    }
    ...
    function _settleDay(uint fromDay_, uint toDay_) private {
        uint notGet = _searchUserBonus(fromDay_, toDay_); // 未领取

        DayToken storage dayToken = dayToDayToken[toDay_][_msgSender()];
        dayToken.accumulate = dayToDayToken[fromDay_][_msgSender()].accumulate + notGet;
        dayToken.cumulative = dayToDayToken[fromDay_][_msgSender()].cumulative;
    }
    ...
    // 用户查询分红
    function _searchUserBonus(uint fromDay_, uint toDay_) view private returns(uint) {
        uint total;
        fromDay_ = (fromDay_ < BEGIN_SEARCH_DAY) ? BEGIN_SEARCH_DAY : fromDay_;
        for (uint i = fromDay_; i < toDay_; i++) {
            ...
            if (totalToken.nft > 0) {
                total += dayToken.nft * (bonus * weightBonus[0]) / totalToken.nft;
            }

            if (totalToken.soul > 0) {
                total += dayToken.soul * (bonus * weightBonus[1]) / totalToken.soul;
            }
            
            if (totalToken.lp > 0) {
                total += dayToken.lp * (bonus * weightBonus[2]) / totalToken.lp;
            }
        }
    ...
    // 设置领取状态
    function _dealGetBonus(uint toDay_) private {
        DayToken storage dayToken = dayToDayToken[toDay_][_msgSender()];
        dayToken.cumulative += dayToken.accumulate;
        dayToken.accumulate = 0;
    }
    ...
    function getUserBonusApi() public {
        ...
        DayToken storage dayToken = dayToDayToken[sameDay][_msgSender()];
        uint accumulate = dayToken.accumulate;

        require(accumulate > 0, "not coin err");
        _dealGetBonus(sameDay);
        addr.usdt.safeTransfer(_msgSender(), accumulate);
    }

    // 查询数据
    function searchUserBonusApi() view public returns(DayToken memory, DayTokenTotal memory, uint) {
        uint sameDay = sameDayQuotient[_msgSender()];
        uint tempQuotient = block.timestamp / DAY;

        uint notGet = _searchUserBonus(sameDay, tempQuotient);
        DayToken memory dayToken = dayToDayToken[tempQuotient-1][_msgSender()];

        dayToken.accumulate = dayToDayToken[sameDay][_msgSender()].accumulate + notGet;
        dayToken.cumulative = dayToDayToken[sameDay][_msgSender()].cumulative;

        return (dayToken, totalDayToken[tempQuotient-1], bonusPool[tempQuotient-1]);
    }
}
