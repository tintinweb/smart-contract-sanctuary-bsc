...
contract Treasury is ContractGuard {
   ...
   function init(
      ...
      if (initialized == false) {
         initialize(_tomb, _tbond, _tshare, _tombOracle, _masonry, _startTime);
      } else {
         if (_tomb != address(0)) tomb = _tomb;
         if (_tbond != address(0)) tbond = _tbond;
         if (_tshare != address(0)) tshare = _tshare;
         if (_tombOracle != address(0)) tombOracle = _tombOracle;
         if (_masonry != address(0)) masonry = _masonry;
