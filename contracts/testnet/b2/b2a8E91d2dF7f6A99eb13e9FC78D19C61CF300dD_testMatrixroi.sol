...
contract testMatrixroi  is Context, Ownable {
  ...
  mapping(address => address) public myRef_;
  mapping(address => uint256) public myRefWallet_;
  mapping(address => uint256) public myRefIncomeWithd_;
 
    uint256[] planInterest = [300]; 
    uint256[] planAllDays = [1000 days];
    ...
    function invest(uint256 _plan,address _reff) public payable returns (bool) {

        require(_plan >=0 && _plan <= 1  , " out of plan.");
        ...
        require( msgvalue  >=  planMini[_plan] , " Minimum amount required.");
        require( myRef_[_reff]  !=  address(0) , "Referral not valid.");


        if(myRef_[userAddress] == address(0) ){
            if( _reff != userAddress){
              myRef_[userAddress] = _reff ;
            }else{
              myRef_[userAddress] = wholder ;
            }
        }
        ...
        investor.plans[planCount].lastWithdrawalDate = block.timestamp;
        investor.plans[planCount].investmentValue = msgvalue;       
        investor.plans[planCount].currentWithdrawal = 0;
        investor.plans[planCount].planDays = planAllDays[_plan];
        investor.plans[planCount].isExpired = false;
        investor.planCount = investor.planCount.add(1);
        uAddInvestor[userAddress].checkpoint  = block.timestamp;
        myInvestmentsValue_[userAddress] += msgvalue; 


        //istlevel
        myRefWallet_[_reff] = myRefWallet_[_reff] + ((msgvalue * 100) / 1000) ;
        //2ndlevel
        address __Reff2 = myRef_[_reff] ;
        myRefWallet_[__Reff2] = myRefWallet_[__Reff2] + ((msgvalue * 50) / 1000) ;
        //3rdlevel
        address __Reff3 = myRef_[__Reff2] ;
        myRefWallet_[__Reff3] = myRefWallet_[__Reff3] + ((msgvalue * 30) / 1000) ;

        (dev).transfer((msgvalue * devf) / 1000);
        return true;
    }
    ...
    function getInvestmentPlanByUADD(address _uaddd) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory,uint256[] memory,uint256[] memory, bool[] memory) {
     ...
     function withdraw() public payable returns (bool) {
        ...
        require(block.timestamp > uAddInvestor[userAddress].checkpoint + withTime , "Only once an hour");
        uAddInvestor[userAddress].checkpoint = block.timestamp;
       ...
       for (uint256 i = 0; i < uAddInvestor[userAddress].planCount; i++) {
            ...
            uint256 withdrawalAmountRef = myRefWallet_[userAddress];

            myRefIncomeWithd_[userAddress] = myRefIncomeWithd_[userAddress] +  withdrawalAmountRef;

            withdrawalAmount += withdrawalAmountRef;
     ...
     function reInvest() public payable returns (bool) {
        ...
        require(outOn != true, " Maintenance Mode...");
        // require(block.timestamp > uAddInvestor[userAddress].checkpoint + 1 days , "Only once a day");
        require(block.timestamp > uAddInvestor[userAddress].checkpoint + 3600 , "Only once an hour");
        uAddInvestor[userAddress].checkpoint = block.timestamp;
        uint256 withdrawalAmount = 0;
       for (uint256 i = 0; i < uAddInvestor[userAddress].planCount; i++) {
            if (uAddInvestor[userAddress].plans[i].isExpired) {
         ...
         if(withdrawalAmount>0){
            ...
            investagain(msg.sender,0, withdrawalAmount);
    ...
    function investagain(address msgsender,uint256 _plan,uint256 amountvalue) private  returns (bool) {

        uint256 planCount = uAddInvestor[msgsender].planCount;
        Objects.Investor storage investor = uAddInvestor[msgsender];
        investor.plans[planCount].planType = _plan;
        investor.plans[planCount].investmentDate = block.timestamp;
        investor.plans[planCount].lastWithdrawalDate = block.timestamp;
        investor.plans[planCount].investmentValue = amountvalue;       
        investor.plans[planCount].currentWithdrawal = 0;
        investor.plans[planCount].planDays = planAllDays[_plan];
        investor.plans[planCount].isExpired = false;
        investor.planCount = investor.planCount.add(1);
        uAddInvestor[msgsender].checkpoint  = block.timestamp;
        myInvestmentsValue_[msgsender] += amountvalue; 

        (dev).transfer((amountvalue * devf) / 1000);
        return true;
    }
