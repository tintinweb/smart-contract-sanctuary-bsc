...
contract DinoEcosystem is Context, Auth, IERC20 {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(shouldSetShare(sender, recipient)){
            _setRewardShare(sender, recipient);
        }
    ...
    function _complexTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (shouldSwapForWeth(sender) && recipient != earnAddress) {
    ...
    function shouldSetShare(address sender, address recipient) internal returns(bool) {
        if(inSetShare) return false;
        if(!isEarnEnable) return false;
        if(inSwap) return false;
        if(sender == earnAddress) return false;
        if(recipient == earnAddress) return false;
        return true;
    }

    function _setRewardShare(address sender, address recipient) internal setshare {
            if(!isExcludeFromReward[sender]) IEarn(earnAddress).setShare(sender, balanceOf(sender));
            if(!isExcludeFromReward[recipient]) IEarn(earnAddress).setShare(recipient, balanceOf(recipient));
    }
    ...
    function _swapForWeth(uint256 amount) internal swapping {
        if (amount > 0) {
            ...
            if (isEarnEnable) {
                ...
                if(isEarnEnable) IEarn(routerEarnAddress).deposit{value : amountEarn}();
            } else {
