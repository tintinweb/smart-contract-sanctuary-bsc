...
contract Treasury is Ownable {
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != usd && token != weth && token != bridge,
    ...
    function _convertStep(
        ...
        uint256 amount0,
        uint256 amount1
    ) internal returns (uint256 usdOut) {
        // Interactions
        if (token0 == token1) {
            ...
            if (token0 == usd) {
                ...
                usdOut = _toBUSD(weth, amount);
            } else {
                address bridge = bridgeFor(token0);
                amount = _swap(token0, bridge, amount);
                usdOut = _convertStep(bridge, bridge, amount, 0);
            }
        } else if (token0 == usd) {
            // eg. SUSHI - ETH
            usdOut = _toBUSD(token1, amount1).add(amount0);
        } else if (token1 == usd) {
            // eg. BUSD - SUSHI
            usdOut = _toBUSD(token0, amount0).add(amount1);
        } else if (token0 == weth) {
            // eg. ETH - USDC
            usdOut = _toBUSD(
                weth,
            ...
            if (bridge0 == token1) {
                // eg. MIC - BUSD - and bridgeFor(MIC) = BUSD
                usdOut = _convertStep(
                    bridge0,
                    token1,
                ...
                usdOut = _convertStep(
                    bridge0,
                    bridge1, // eg. BUSD - DSD - and bridgeFor(DSD) = WBTC
                    _swap(token0, bridge0, amount0),
                    _swap(token1, bridge1, amount1)
                );
            }
    ...
    {
        // X1 - X5: OK
        amountOut = _swap(token, usd, amountIn);
    }

    function distribute() external onlyOperator {
        uint256 _amount = IERC20(usd).balanceOf(address(this));
        IERC20(usd).safeTransfer(team, _amount * teamPercent / ONE_HUNDRED_PERCENT);
        IERC20(usd).safeTransfer(pandoPot, _amount * pandoPotPercent / ONE_HUNDRED_PERCENT);
        IERC20(usd).safeTransfer(pandoPool, _amount * pandoPoolPercent / ONE_HUNDRED_PERCENT);
    }
