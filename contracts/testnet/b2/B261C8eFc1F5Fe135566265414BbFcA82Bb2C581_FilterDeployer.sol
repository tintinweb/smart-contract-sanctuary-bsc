...
contract FilterDeployer {
    ...
    function createToken(uint _tokenType, string memory _tokenName, string memory _tokenSymbol, uint[] memory _tokenArgs, address _baseTokenAddress, uint _baseTokenAmount, uint _ownerShare, uint _liquidityLockTime) public returns (address deployedTokenAddress) {
    ...
    function createTokenETH(uint _tokenType, string memory _tokenName, string memory _tokenSymbol, uint[] memory _tokenArgs, uint _ownerShare, uint _liquidityLockTime) public payable returns (address deployedTokenAddress) {
        require(filterManager.tokenCreationAllowed(), "FilterDeployer: TOKEN_CREATION_NOT_ALLOWED");
        require(_ownerShare <= filterManager.maxOwnerShare(), "FilterDeployer: DEV_SHARE_TOO_HIGH");
        require(filterManager.tokenTemplateAddresses(_tokenType) != address(0), "FilterDeployer: INVALID_TOKEN_TYPE");
    ...
    function createPresaleToken(uint _tokenType, string memory _tokenName, string memory _tokenSymbol, uint[] memory _tokenArgs, address _ownerAddress) public returns (address deployedTokenAddress) {
