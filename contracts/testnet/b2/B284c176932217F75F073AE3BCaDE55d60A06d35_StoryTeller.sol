...
contract StoryTeller is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
    ...
    string public  contractURIstorefront = '{ "name": "StoryTeller", "description": "Storyteller is an Artificial Intelligence Based Storytelling Experience where storytellers are able to create stories through the minting of letters and putting the letters together to create an image story using Stable Diffusion", "image": "https://storyteller.ai", "external_link": "https://storyteller.ai", "seller_fee_basis_points": 300, "fee_recipient": "0x42A1DE863683F3230568900bA23f86991D012f42"}'; 
    string public _tokenURI = '{"attributes":[{"trait_type":"length","value":1}],"description":"StoryTeller NFT","external_url":"http://storyteller.ai","image":"","name":"" ,"seller_fee_basis_points": 500, "fee_recipient": "0x42A1DE863683F3230568900bA23f86991D012f42" }';

    uint256 public RequestCount;
    uint256 public RequestProcessed;
    ...
    function mintLetter () public {
        RequestCount++;
        Requests[RequestCount]._owner = msg.sender;
        Requests[RequestCount]._type = 1;
    }

    function merge ( uint256 _a, uint256 _b ) public {
        RequestCount++;
        Requests[RequestCount]._owner = msg.sender;
        Requests[RequestCount]._type = 2;
        Requests[RequestCount]._a = _a;
        Requests[RequestCount]._b = _b;
        transferFrom ( msg.sender, address(this), _a ) ;
        transferFrom ( msg.sender, address(this), _b ) ;

    }

    function process ( uint256 _request , string memory _json  ) public onlyEngine {
  
        RequestProcessed++;
        require ( Requests[_request]._processed == false , "Already processed" );
        Requests[_request]._processed = true;
        _tokenIds++;
        uint256 newTokenId = _tokenIds;
        _safeMint( Requests[_request]._owner , newTokenId);
        _setTokenURI(newTokenId, _json);

    }

    function setEngineEcosystemContract ( address _EngineEcosystemContractAddress ) public onlyEngine {
         _enginecontract =  EngineEcosystemContract ( _EngineEcosystemContractAddress );
