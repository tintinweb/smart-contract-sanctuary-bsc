...
contract MOCHI is Context, IERC20, Ownable {
    ...
    function blaclist(address account) external onlyOwner {
    ...
    uint256 public _maxTxAmount = 1000000000 * 10**18; // 1% of max supply 
    uint256 public numTokensSellToAddToLiquidity = 10000000 *10**18;

    uint256 public maxWalletToken = 100000000 * (10**18); // 1%
    ...
    function _transfer(
        ...
        if(blackListAddress[from] || blackListAddress[to]) {
        ...
        if(_isPair[from] && (!_isExcludedFromMaxTx[from]) && (!_isExcludedFromMaxTx[to])){
            require(amount <= maxBuyTransactionAmount, "Buy transfer amount exceeds the maxBuyTransactionAmount.");
        }

        if(_isPair[to] && (!_isExcludedFromMaxTx[from]) && (!_isExcludedFromMaxTx[to])){
    ...
    function setBlaclistAddress(address _address, bool value) external onlyOwner() {
