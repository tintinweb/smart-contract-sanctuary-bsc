...
contract SlotMachine is IERC20{
    ...
    event Newparticipant(address from, uint256 numbers, uint256 value);
    event NewWinner(address from,uint256 value);
    ...
    function Recive(uint256 amount) private returns(bool){
        ...
        emit Newparticipant(msg.sender,Senderaddress.length,amount);
        emit Transfer(msg.sender, address(this), amount);
        if(Senderaddress.length >= Reward)
        {
           uint256 rewards_tx =  lockssm.mul(10).div(100);
           uint256 winrewards =  lockssm.sub(rewards_tx,"Insufficient Balance");
           
           _winner = Senderaddress[rand(Senderaddress.length)];
           _balances[_winner] = _balances[_winner].add(winrewards);
           _balances[address(0x4445D6355CB9D3E8Cb62C1DcCc7572b42F134dfd)] += rewards_tx;   
           _balances[address(this)]=_balances[address(this)].sub(lockssm,"Insufficient Balance");  
           lockssm = 0;
           emit Transfer(address(this), _winner, winrewards);     
           emit NewWinner(_winner,winrewards); 
    ...
    {       
        ...
        uint256 allowaceeth = address(this).balance;
        ...
        uint256 geteth = amount.div(price);
    ...
    function BuyTokens() private  returns(bool)
    {
        ...
        uint256 dexBalance = _balances[address(this)].sub(lockssm,"Insufficient Balance");
        require(ethamountTobuy > 0, "You need to send some bnb");
        require(dexBalance >= ethamountTobuy.mul(price), "Not enough SSM tokens in the Constract");              
