...
contract TINAup is ContextUpgradeable, ERC20BurnableUpgradeable , OwnableUpgradeable {
    ...
    uint256 public BTCBdivisor;
    uint256 public BETHdivisor;
    uint256 public BNBdivisor;
    ...
    function initialise(IUniswapV2Router02 _router, 
                        address _beth, 
                        address _btcb, 
                        DividendTrackerBNB _forBNB, 
                        DividendTracker1 _forBTCB, 
                        DividendTracker1 _forBETH
                    ) external initializer{
        __Context_init();
        __ERC20Burnable_init();
        __ERC20_init("TINA Token","TINA");
        __Ownable_init();
    ...
    function swapAndSendDividends(uint256 _flag, uint256 tokens) private {
        if(_flag == 0){
            ...
            uint256 dividends = IERC20Upgradeable(BTCB).balanceOf(address(this));
            bool success = IERC20Upgradeable(BTCB).transfer(address(dividendTrackerBTCB) ,dividends);
            ...
            uint256 dividends = IERC20Upgradeable(BETH).balanceOf(address(this));
            bool success = IERC20Upgradeable(BETH).transfer(address(dividendTrackerBETH) ,dividends);
            ...
            uint256 initialBalance = address(this).balance;
            swapTokenforBNB(tokens);

            uint256 swappedBNB = address(this).balance.sub(initialBalance);
            // uint256 dividends = swapTokenforBNB(tokens);
            (bool success,) = address(dividendTracker).call{value: swappedBNB}(""); 
            
            if(success) {
                dividendTracker.distributeRewardTokenDividends(swappedBNB);
                // forDividends = forDividends.sub(tokens);
                // forBNB = 1;
            }
        }
    ...
    function claimBETH() external {
        dividendTrackerBETH.processAccount(payable(_msgSender()), false);
    }
    
    function claimBTCB() external {
        dividendTrackerBTCB.processAccount(payable(_msgSender()), false);
    }
   ...
   //Setter functions
    ...
    function excludeFromMaxTxAmount(address account, bool status) public onlyOwner {
        _isExcludedFromMaxTx[account] = status;
    }

    function excludeFromMaxBalance(address account, bool status) public onlyOwner {
        _isExcludedFromMaxBal[account] = status;
    } 
...
contract DividendTrackerBNB is DividendPayingTokenBNB {
    ...
    function withdrawDividend() public pure override {
        require(false, "DividendTrackerBNB: withdrawDividend disabled. Use the 'claim' function on the main Honey contract.");
    }
