...
contract WorldCupViewer is DivestorUpgradeable {
    ...
    // 获取用户记录
    function getPlayerRecord(address player_) public view returns(IWorldCup.Instant[] memory) {
        uint recordlen = worldCup.mapPlayerRecordLen(player_);
        IWorldCup.Instant[] memory record = new IWorldCup.Instant[](recordlen);
        for (uint i = 0; i < recordlen; i++) {
            record[i] = worldCup.mapPlayerRecord(player_, i);
        }
        return record;
    }

    // 获取用户冠军赛记录
    function getPlayerChampion(address player_) public view returns(IWorldCup.Champion[] memory) {
        uint recordlen = worldCup.mapPlayerChampionLen(player_);
        IWorldCup.Champion[] memory championRecord = new IWorldCup.Champion[](recordlen);
        for (uint i = 0; i < recordlen; i++) {
            championRecord[i] = worldCup.mapPlayerChampion(player_, i);
        }
    ...
    function getEarningsData(address player) public view returns(IWorldCup.PlayerEarningInfo memory) {
        ...
        if (worldCup.mapPlayerRecordLen(player) != 0) {
            ...
            IWorldCup.Instant[] memory record = getPlayerRecord(player);
        ...
        IWorldCup.Champion[] memory championRecord = getPlayerChampion(player);
