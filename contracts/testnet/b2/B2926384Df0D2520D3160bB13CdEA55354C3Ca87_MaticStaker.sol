...
contract MaticStaker {
	...
	constructor(IERC20 _token, uint256 startDate){
		require(startDate > 0);
        token = _token;
		startUNIX = startDate;

        plans.push(Plan(3, 80));
        plans.push(Plan(5, 65));
	...
	function invest(uint8 plan) public {
		...
		// kmemit FeePayed(msg.sender, fee);
		...
		totalStaked = totalStaked.add(amount-fee);
