...
contract MainBridge is Pausable, Ownable {
    ...
    constructor() {
        ownToken = IERC20(0x7665CB7b0d01Df1c9f9B9cC66019F00aBD6959bA);
    ...
    function claim(uint chainId, uint itemId, address account, uint amount, bytes memory signature) public onlyOwner whenNotPaused {
        ...
        require(verify(chainId, itemId, account, amount, signature) == true, "Signature is invalid.");
    ...
    function getMessageHash(uint chainId, uint itemId, address account, uint amount) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(chainId, itemId, account, amount));
    }
    ...
    function verify(uint chainId, uint itemId, address account, uint amount, bytes memory signature) public view virtual returns (bool) {
        bytes32 messageHash = getMessageHash(chainId, itemId, account, amount);
