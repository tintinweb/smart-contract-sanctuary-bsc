...
contract ProductMarketing is Ownable {
    ...
    function registerUser(uint referralId) public returns (bool) {
        ...
        uint decimal = 10 ** stablecoincontract.decimals();
        uint registerprice = configs.registerfee * decimal;
        ...
        stablecoincontract.transfer(
            ...
            configs.ownercommission * decimal
        );
        ...
        for (uint lv = 1; lv <= configs.levels; lv++) {
            ...
            stablecoincontract.transfer(_prnt.wallet, comssion * decimal);
        ...
        for (uint pt = 0; pt < configs.partners.length; pt++) {
            stablecoincontract.transfer(
                configs.partners[pt],
    ...
    function setConfigs(
        Settings memory newConfigs
    ) public onlyOwner returns (bool) {
        // not allow chnage config
        newConfigs.partners = configs.partners;
