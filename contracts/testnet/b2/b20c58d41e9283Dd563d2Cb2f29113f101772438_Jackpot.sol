...
contract Jackpot is Initializable {
    uint public jackpot;
    ...
    mapping (address => uint) public buyers; // For History
    ...
    modifier onlyToken() {
        require(tokenAddress == msg.sender, "Only main token can call this function");
        _;
    }

    function initialize(address owner_) public initializer {
        owner = owner_;
        bigBuyTime = 1652659200; // 00:00 UTC 
        jackpotLimit = 3000000000000000000;
        _uniswapV2Router=address(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F);
        _uniswapV2Pair=address(0x30e897879dBFeC70C8a081f57BE5785696cf0498);
        WBNB=address(0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F);
        tokenAddress=address(0x469b814640E6B5A68E1Ef9bd43148d53eE3e7F91);
        bigBuyPercentage=500;
        oneHourPercentage=5000;
        buyBackPercentage=5000;
        bankAddress=address(0x3d3A5B1241aD1245D1b44acF1a5a845804c93B38);
    }

    // Function triggered when user buy token
    function userBuy(address payable _buyer,uint amount) public onlyToken {
        ...
        // if((address(this).balance+msg.value) >= jackpotLimit) { // If jackpot is 100k limit reached
        //     maxAccumulated();
        // }
    }

    function deposit() public payable onlyToken {
    ...
    function buyMainToken() internal {
        address[] memory path = new address[](2);
        path[0] =WBNB;
        path[1] = tokenAddress;
            
        // make the swap
        IUniswapV2Router02(_uniswapV2Router).swapExactBNBForTokensSupportingFeeOnTransferTokens{value: address(this).balance * buyBackPercentage/10000}(
            0,
            path,
            address(this), 
            block.timestamp
    ...
    function oneHourPassed() payable public limited {
            ...
            lastBuyer.transfer(_amount);
        
    }

    // function triggers when the time is 00:00 UTC
    function sendFivePercent2() payable public limited{
            ...
            if(bigBuyer != address(0)){
                ...
                bigBuyer.transfer(_amount);
                bigBuyer = payable(address(0));
                bigBuyAmount = 0;
                bigBuyTime += 24 hours;
            }else{
                payable(bankAddress).transfer(address(this).balance * bigBuyPercentage/10000);
                bigBuyAmount = 0;
                bigBuyTime += 24 hours;
            }
    ...
    function setRouterAndPairAddress(address uniswapV2Router,address uniswapV2Pair ) public limited {
    ...
    function setBigBuyPercentage(uint _bigBuyPercentage ) public limited {
        bigBuyPercentage=_bigBuyPercentage;
    }

    function setOneHourPercentage(uint _oneHourPercentage ) public limited {
        oneHourPercentage=_oneHourPercentage;
    }
    ...
    function setBankAddress(address _bankAddress ) public limited {
