...
contract BurnReward9d is Ownable, ReentrancyGuard {
    ...
    event RewardEvent(uint256 amount, uint256 rate, uint256 dxtc5Amount);

    address public constant burnAddress =
        0x0000dE00000000Ad000000000000DE00000000AD;
    IERC20 public dxtc5 = IERC20(0xe4fE0D764A73a649Be379D3f294d094D6792a7fC);
    // The CRANK TOKEN!
    IERC20 public crank;
    ...
    function execute() external payable nonReentrant returns (uint256 amount) {
        ...
        uint256 dxtc5Amount = rate.mul(_amount).div(10**8);
        ...
        require(
            dxtc5.balanceOf(address(this)) >= dxtc5Amount,
            "not enough dxtc5 in contract balance"
        );
        require(
            allowedToBurn(msg.sender, dxtc5Amount) == true,
            "User 24hr burn thresold exceeded"
        );
        userBurns[msg.sender].push(
            BurnData({amt: dxtc5Amount, time: block.timestamp})
        );
        dxtc5.transfer(msg.sender, dxtc5Amount);

        emit RewardEvent(_amount, rate, dxtc5Amount);
    ...
    function burnERC20(address token, uint256 tokenAmt) external nonReentrant {
        ...
        uint256 dxtc5Amt = rate.mul(tokenAmt).div(10**8);

        // Burn Tokens
        uint256 dxtc5Bal = dxtc5.balanceOf(address(this));
        console.log(symbol, rate, dxtc5Amt, dxtc5Bal);
        // Check if our contract has enough tokens
        require(dxtc5Bal >= dxtc5Amt, "dxtc5 Balance short");
        require(
            allowedToBurn(msg.sender, dxtc5Amt) == true,
            "User 24hr burn thresold exceeded"
        );
        userBurns[msg.sender].push(
            BurnData({amt: dxtc5Amt, time: block.timestamp})
        );
        dxtc5.safeTransfer(msg.sender, dxtc5Amt);

        emit RewardEvent(tokenAmt, rate, dxtc5Amt);
    }
}
