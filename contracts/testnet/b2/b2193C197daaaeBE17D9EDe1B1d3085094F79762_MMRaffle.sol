...
contract MMRaffle is Ownable, VRFConsumerBase {
    ...
    uint256 public maxTax = 300; // first tax maximum, in % * 10
    uint256 public taxRate = 25; // tax difference, in % * 10
    ...
    {
        return
            (Raffles[_id].ticketBalances[user] / Raffles[_id].ticketCount) *
            10**18; // must divide 10 ** 16 in frontend
    }
        ...
        override
    {
        ...
        IERC20(winOnePrize).transfer(
            ...
            amountWinOnePrize * (1 - stakerOne.tax) // must divide 1000 in frontend
        );
        ...
        for (uint256 i = 0; i < numOfWinTwo; i++) {
            ...
            if (winnerTwo != Raffles[raffleId].winnerOne) {
                ...
                IERC20(winTwoPrize).transfer(
                    winnerTwo,
                    amountWinTwoPrize * (1 - stakerTwo.tax) // must divide 1000 in frontend
                );
            }
        }
        ...
        for (uint256 i = numOfWinTwoThree; i < numOfWinTwo; i--) {
            ...
            if (
                ...
                IERC20(winThreePrize).transfer(
                    winnerThree,
                    amountWinThreePrize * (1 - stakerThree.tax) // must divide 1000 in frontend
                );
            }
        }
