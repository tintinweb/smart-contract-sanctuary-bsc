...
contract CollectionFactory is AccessControl {
    ...
    event ERC721ACollectionDeployed(
        address addr,
        string name,
        string symbol,
        address owner,
    ...
    constructor(
        ...
        address _ERC721AImplementation,
    ...
    function createERC721ACollection(
        address _owner,
        address[] memory _minters,
        string memory _name,
        string memory _symbol,
        ...
        clone = Clones.clone(erc721AImplementation);
        Collection(clone).initialize(
            _owner,
            _minters,
            _name,
        ...
        emit ERC721ACollectionDeployed(
            clone,
            _name,
            _symbol,
            _owner,
    ...
    function updateERC721AImplementation(address _newERC721AImplementation)
        external
    {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) ||
                hasRole(ROLE_ADMIN, _msgSender()),
            "MediaEyeCollectionFactory: must have owner or admin role to change 721A implementation."
        );
        erc721AImplementation = _newERC721AImplementation;
    }
