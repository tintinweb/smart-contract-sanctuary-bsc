...
contract GymVaultsStrategyAlpaca is Ownable, ReentrancyGuard, Pausable {
    ...
    /// Alpaca Vault config address
    address public immutable vaultConfigAddress;
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        if (isAutoComp && !strategyStopped) {
            ...
            if (vault.token() == IVaultConfig(vaultConfigAddress).getWrappedNativeAddr()) {// address(this).balance > 0
                IWETH(wbnbAddress).deposit{value : _wantAmt}();
            }
        }
