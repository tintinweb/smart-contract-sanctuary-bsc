...
contract Launchpad {
  ...
  event AuctionTokenTransferredFromVault(uint256 _pid);
  ...
  event AllowlistSignerChanged(uint256 indexed _pid, address _address);

  /* ========== INITIALIZE ========== */
  constructor() {
    OPERATOR = 0x89AC0D2c7aB17e93F833062d8F6c3f4980f374ED; // main
  }
  ...
  /**
   * @dev Change Operator
   * @param _address new opertor address
   */
  function changeOperator(address _address) external onlyOperator {
    OPERATOR = _address;
  ...
  function initProject(
    ...
    AUCTION_TOKEN_VAULT = 0x89AC0D2c7aB17e93F833062d8F6c3f4980f374ED;
    AUCTION_TREASURY = 0x7E71ebB8E268b6aD9f41Ade15aB40445aC309777;
    ...
    PROJECTS[_pid].auction = _auction;
  ...
  function getAuctionTokenBalance(uint256 _pid) external view returns (uint256) {
    require(address(PROJECTS[_pid].auction) != address(0), 'project_should_be_initialized');
    return PROJECTS[_pid].auction.getAuctionTokenBalance();
  }

  function cancelAuctionBeforeStart(uint256 _pid) external onlyOperator {
    require(address(PROJECTS[_pid].auction) != address(0), 'project_should_be_initialized');
  ...
  function getAuctionData(uint256 _pid) external view returns (AuctionData memory) {
    require(address(PROJECTS[_pid].auction) != address(0), 'PROJECT_SHOULD_BE_INITIALIZED');
    return PROJECTS[_pid].auction.getAuctionData();
  }
  ...
  function withdrawUnclaimedAuctionToken(uint256 _pid) external onlyOperator {
    require(address(PROJECTS[_pid].auction) != address(0), 'PROJECT_SHOULD_BE_INITIALIZED');
    PROJECTS[_pid].auction.withdrawUnclaimedAuctionToken();
    emit UnclaimedAuctionTokenWithdrawn(_pid);
  }

  function withdrawUnclaimedLPToken(uint256 _pid) external onlyOperator {
    require(address(PROJECTS[_pid].auction) != address(0), 'PROJECT_SHOULD_BE_INITIALIZED');
    PROJECTS[_pid].auction.withdrawUnclaimedLPToken();
    emit UnclaimedLPTokenWithdrawn(_pid);
  }

  function setAllowlistSigner(uint256 _pid, address _signer) external onlyOperator {
    require(address(PROJECTS[_pid].auction) != address(0), 'PROJECT_SHOULD_BE_INITIALIZED');
...
//contract BatchAuction {
  ...
  /// @notice 
  mapping(address => uint256) public VESTED_LPTOKEN_CLAIMED;
  ...
  /// @notice Withdraw cap 
  /// https://www.notion.so/planetarium/Launchpad-Commit-Withdraw-9909fac5ff404e7aa21d17b3fb7c03c5
  // BNB case
  uint256 private WITHDRAW_CAP_MIN = 5e18; // 5 BNB
  uint256 private WITHDRAW_CAP_MAX = 250e18; // 250 BNB
  uint256 private WITHDRAW_CAP_LIMIT = 75e18; // 75 BNB
  uint256 private WITHDRAW_CAP_INTERCEPT = 14; 
  uint256 private WITHDRAW_CAP_INTERCEPT_PLUS = 175e18; 
  uint256 private WITHDRAW_CAP_INTERCEPT_DIV = 49;
  ...
  /// @dev to check commit currency is ETH or ERC20 token
  address public COMMIT_CURRENCY;

  // TODO:
  // BSC Testnet
  /// @dev The placeholder ETH address.
  address private constant ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; // ETH
  address private constant WETH = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // WETH

  // https://docs.pancakeswap.finance/code/migration/masterchef-v2
  address private constant PancakeFactoryAddress = 0x6725F303b657a9451d8BA641348b6761A6CC7a17;
  address private constant PancakeRouterV2Address = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
  IPancakeFactoryV2 private pancakeFactoryV2 = IPancakeFactoryV2(PancakeFactoryAddress);
  IPancakeRouterV2 private pancakeRouterV2 = IPancakeRouterV2(PancakeRouterV2Address);
  ...
  constructor() {
    ...
    uint256 TEST_DIV = 1000;
    WITHDRAW_CAP_MIN = WITHDRAW_CAP_MIN.div(TEST_DIV); // 0.005
    WITHDRAW_CAP_MAX = WITHDRAW_CAP_MAX.div(TEST_DIV); // 0.25
    WITHDRAW_CAP_LIMIT = WITHDRAW_CAP_LIMIT.div(TEST_DIV); // 0.075
    WITHDRAW_CAP_INTERCEPT_PLUS = WITHDRAW_CAP_INTERCEPT_PLUS.div(TEST_DIV); //0.175 
  ...
  function transferAuctionTokenFromVault() external onlyOwner {
   ...
   */
  //function cancelAuctionBeforeStart() external onlyOwner nonReentrant {
  function cancelAuctionBeforeStart() external onlyOwner {
    require(!_isAuctionFinalized(), "AUCTION_SHOULD_NOT_BE_FINALIZED"); 
   ...
   * @notice Commit ETH
   */
  //function commitETH(bytes calldata signature) external payable {
   ...
   */
  //function commitERC20Token(uint256 _amount, bytes calldata signature) external {
  function commitERC20Token(uint256 _amount) external {
    if (!DEBUG) {
      require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
  ...
  function withdrawERC20Token(uint256 _amount) external {
    if (!DEBUG) {
      require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
      require(_amount > 0, "INVALID_AMOUNT");
      require(auctionData.startTime < block.timestamp 
  ...
  function claimInstantToken() external {
    if (!DEBUG) {
      ...
      require(_isAuctionSuccessful(), "AUCTION_SHOULD_BE_SUCCESSFUL");
      require(_isAuctionFinalized(), "AUCTION_SHOULD_BE_FINALIZED");
      require(!USER_RECEIVED_INSTANT_TOKEN[msg.sender], "USER_ALREADY_CLAIMED_INSTANT_TOKEN");
    }

    uint256 amount = getActualInstantTokenAmount(msg.sender);
  ...
  function claimVestedLPToken() external {
    if (!DEBUG) {
      require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED");
      require(isValidClaimablePeriod(), "CLAIMABLE_PERIOD_ENDED");
      require(COMMITMENTS[msg.sender] > 0, "NO_COMMITMENTS");
  ...
  function withdrawUnclaimedAuctionToken() external onlyOwner {
    if(!DEBUG) {
  ...
  // TODO:: TODO:: TODO onlyOwner
  function withdrawUnclaimedLPToken() external onlyOwner {
    if(!DEBUG) {
      require(!isValidClaimablePeriod(), "CLAIMABLE_PERIOD_NOT_ENDED");
    }
  ...
  //function getCommitmentBack() external nonReentrant {
  function getCommitmentBack() external {
    if(!DEBUG) {
      require(block.timestamp > auctionData.endTime, "AUCTION_NOT_ENDED");
      require(!_isAuctionSuccessful(), "AUCTION_SHOULD_BE_FAILED");
      require(_isAuctionFinalized(), "AUCTION_SHOULD_BE_FINALIZED"); 
      require(COMMITMENTS[msg.sender] > 0, "NO_COMMITMENTS");
    ...
    auctionData.totalCommitments = auctionData.totalCommitments.sub(userCommited);
    COMMITMENTS[msg.sender] = 0; 

    if(COMMIT_CURRENCY == ETH_ADDRESS) {
      _safeTransferETH(payable(msg.sender), userCommited);
    } else { // ERC20 Token
      IERC20(COMMIT_CURRENCY).safeTransfer(msg.sender, userCommited);
    }

    emit GotCommitmentBack(msg.sender, userCommited);
  }
  ...
  function finalizeAuction() external onlyOwner {
    ...
    if (_isAuctionSuccessful()) {
      ...
      emit AuctionFinalizedWithSuccess(
        transferAmount,
        token1Amount,
        token2Amount,
        PANCAKE_LPTOKEN,
   ...
   * user can claim 70% of tokens instantly, without vesting
   */
  function getActualInstantTokenAmount(address _user) public view returns (uint256) {
    if(!DEBUG) {
      require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED");
    }

    if(!isValidClaimablePeriod()) return 0;
    if(USER_RECEIVED_INSTANT_TOKEN[_user]) return 0;
   ...
   */
  function getTotalVestedLPTokenAmount(address _user) public view returns (uint256) {
  ...
  function getActualVestedLPTokenAmount(address _user) public view returns (uint256) {
    require(COMMITMENTS[_user] > 0, "NO_COMMITMENTS");
    require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED");
    require(_isAuctionSuccessful(), "AUCTION_SHOULD_BE_SUCCESSFUL");
    require(_isAuctionFinalized(), "AUCTION_SHOULD_BE_FINALIZED");
  ...
  /**
   * @dev Calculate the amount of Commited Token that can be withdrawn by user
   */
  ...
  /**
   * @dev Get total commited amount of a user
   */
  function getCommitedAmount(address _user) external view returns (uint256) {
  ...
  function getLockedAmountAfterDeposit(address _user, uint256 _amount) external view returns (uint256) {
    ...
    return total.sub(_withdrawableAmountPerUser(_user));
  }
  ...
  function _addCommitment(address _user, uint256 _amount) internal {
    if (!DEBUG) {
      require(_amount > 0, "INVALID_COMMITMENT_VALUE");
      require(auctionData.startTime < block.timestamp 
              && block.timestamp < auctionData.endTime, "INVALID_AUCTION_TIME");
    ...
    if (COMMITMENTS[_user] >= MAX_COMMITMENTS[_user]) {
       MAX_COMMITMENTS[_user] = MAX_COMMITMENTS[_user].add(_amount);
    }
  ...
  function _withdrawCommitment(address _user, uint256 _amount) internal {
    if (!DEBUG) {
      require(_amount > 0, "INVALID_AMOUNT");
   ...
   */
    ...
    require(COMMIT_CURRENCY != address(0), "INVALID_COMMIT_CURRENCY");
    uint256 max_withdraw = _withdrawableAmountPerAmount(MAX_COMMITMENTS[_user]);
    return max_withdraw - ACCUMULATED_WITHDRAW[_user];
  }

   function get1(address _user) external view returns (uint256) {
    uint256 max_withdraw = _withdrawableAmountPerAmount(MAX_COMMITMENTS[_user]);
    return max_withdraw;
  }

   function get2(address _user) external view returns (uint256) {
    return ACCUMULATED_WITHDRAW[_user];
  }

   function get3(address _user) external view returns (uint256) {
    return MAX_COMMITMENTS[_user];
  }

   /**
   * @dev Calculate Withdrawable Amount based on amount
   */
    function _withdrawableAmountPerAmount(uint256 _amount) internal view returns (uint256) {
    require(_amount > 0, "INVALID_AMOUNT");
    if (_amount <= WITHDRAW_CAP_MIN) {
      return _amount;
    } else if (_amount < WITHDRAW_CAP_MAX) {
      return (_amount.mul(WITHDRAW_CAP_INTERCEPT).add(WITHDRAW_CAP_INTERCEPT_PLUS)).div(WITHDRAW_CAP_INTERCEPT_DIV);
    } else { 
      return WITHDRAW_CAP_LIMIT;
    }
  }

  function setupPancakeSwapPool(
    address _token1,
    address _token2,
    uint256 _token1Amount,
    uint256 _token2Amount
    ...
    uint256 deadline = block.timestamp + 300; // current + 5 minutes
    address lpAddress;

    if(_token2 == ETH_ADDRESS) {
      _token2 = WETH; // token2 should be WETH (WETH is WBNB in BSC)
    }
   ...
   */
  function _isAuctionSuccessful() internal view returns (bool) {
   ...
   */
  function _isAuctionFinalized() internal view returns (bool) {
