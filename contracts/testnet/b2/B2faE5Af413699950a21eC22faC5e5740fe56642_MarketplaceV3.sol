...
contract MarketplaceV3 is Ownable, IERC721Receiver {
  ...
  function selling(
    ...
    require(!sellingTokens[_id].isExist, 'the token is selling');
    require(
  ...
  function cancelSelling(uint256 _id) public {
    require(sellingTokens[_id].isExist, 'the token is sold or already bought');
    // check current ownership
    NftSold memory nftSold = sellingTokens[_id];
   ...
   */
  function buy(uint256 _id, uint256 _price) external returns (bool) {
    require(sellingTokens[_id].isExist, 'the token is not sold');
    NftSold memory nftSold = sellingTokens[_id];
