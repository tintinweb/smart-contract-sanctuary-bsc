...
// NFT交易
contract NFTExchange is INFTExchange, Caller, Fallback {
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;

    IConfig public config; //配置合约
    ...
    function getOrderCount() external view override returns (uint256) {
        return orderIdSet.lengthUintSet();
    }
    ...
    // NFT挂单
    function make(
        uint8 type_, //1:固定价格,2:拍卖
        address token, //NFT代币合约
        uint256 tokenId, //NFT代币ID
        address quoteToken, //报价代币合约
        uint256 quoteAmount, //报价代币总包数量
        uint256 ts //结束时间
    ) external override {
        require(tokens.containsAddressSet(token), "1185");
        require(1 == type_ || 2 == type_, "1138");
        if (2 == type_) {
            require(block.timestamp < ts, "1137");
        } else {
            ts = block.timestamp;
        }
        require(0 < tokenId, "1176");
        config.getApproveProxy().transferFromERC721(
            token,
            msg.sender,
            address(config.getAsset()),
            tokenId
        );
        orderId = orderId.add(1, "1141");
        orderIdSet.addUintSet(orderId);
    ...
    // NFT一口价吃单
    function takeFixPrice(uint256 orderId_) external payable override {
        OrderInfo memory order_ = order[orderId_];
        require(address(0) != order_.token, "1122");
        require(1 == order_.type_, "1132");
        orderIdSet.removeUintSet(orderId_);
        delete order[orderId_];
    ...
    // NFT拍卖吃单
    function takeAuction(uint256 orderId_, uint256 amount)
        external
        payable
        override
    {
        OrderInfo storage order_ = order[orderId_];
        require(address(0) != order_.token, "1136");
        require(2 == order_.type_, "1131");
        require(block.timestamp < order_.ts, "1128");
        require(order_.quoteAmount < amount, "1127");
        TransferV1.transferFromFlush(
            config.getApproveProxy(),
            order_.quoteToken,
            msg.sender,
        ...
        order_.quoteAmount = amount;
        order_.bidder = msg.sender;
        if (order_.ts < block.timestamp.add(300000, "1146")) {
            order_.ts = order_.ts.add(300000, "1156");
        }
        emit BidEvent(
            orderId_,
            order_.type_,
    ...
    // NFT撤单
    function cancel_(uint256 orderId_, bool isPlatform) private {
        OrderInfo memory order_ = order[orderId_];
        require(address(0) != order_.token, "1150");
        if (!isPlatform) {
            require(order_.maker == msg.sender, "1157");
        }
        orderIdSet.removeUintSet(orderId_);
        delete order[orderId_];
        if (address(1) != order_.bidder) {
    ...
    // NFT撤单
    function cancelUser(uint256 orderId_) external override {
        cancel_(orderId_, false);
    }

    // NFT撤单
    function cancelPlatform(uint256 orderId_) external override onlyCaller {
        cancel_(orderId_, true);
    }

    // NFT系统撤单
    function cancelAll(uint256 count) external override onlyCaller {
        if (orderIdSet.lengthUintSet() < count) {
            count = orderIdSet.lengthUintSet();
        }
        for (uint256 i = 0; i < count; i++) {
            cancel_(orderIdSet.atUintSet(0), true);
        }
    }

    // NFT拍卖系统交割
    function delivery(uint256[] memory orderId_) external override onlyCaller {
        uint256 orderId__;
        OrderInfo memory order_;
        for (uint256 i = 0; i < orderId_.length; i++) {
            orderId__ = orderId_[i];
            order_ = order[orderId__];
            require(address(0) != order_.token, "1125");
            require(2 == order_.type_, "1126");
            require(order_.ts < block.timestamp, "1151");
            if (address(1) == order_.bidder) {
                cancel_(orderId__, true);
            } else {
                orderIdSet.removeUintSet(orderId__);
                delete order[orderId__];
                delivery_(orderId__, order_);
                emit AcceptEvent(
                    orderId__,
                    order_.type_,
                    order_.token,
    ...
    function delivery_(uint256, OrderInfo memory order_) private {
        //orderId_
        address platform;
        uint256 platformFee;
        address creator = INFT(order_.token).getCreator(order_.tokenId);
        uint256 creatorFee;
        (platformFee, platform, creatorFee) = config
            .getNFTTradePlatformFeeAndPlatformFeeRateAndCreatorFeeRate();
        platformFee = order_.quoteAmount.mul(platformFee, "1158").div(
            100,
            "1143"
        );
        creatorFee = order_.quoteAmount.mul(creatorFee, "1148").div(
            100,
            "1306"
        );
        uint256 makerFee = order_.quoteAmount.sub(
            platformFee.add(creatorFee, "1159"),
            "1155"
        );
        IAsset asset = config.getAsset();
        asset.claimMainOrERC20(order_.quoteToken, platform, platformFee);
        asset.claimMainOrERC20(order_.quoteToken, creator, creatorFee);
        asset.claimMainOrERC20(order_.quoteToken, order_.maker, makerFee);
        asset.claimERC721(order_.token, order_.bidder, order_.tokenId);
    }

    // NFT拍卖接受报价
    function acceptOffer(uint256 orderId_) external override {
        OrderInfo memory order_ = order[orderId_];
        require(address(0) != order_.token, "1154");
        require(2 == order_.type_, "1145");
        //require(order_.ts < block.timestamp, "1173");
        require(order_.maker == msg.sender, "1165");
        require(address(1) != order_.bidder, "1169");
        orderIdSet.removeUintSet(orderId_);
        delete order[orderId_];
        delivery_(orderId_, order_);
        emit AcceptEvent(
    ...
    function setTokenExchange(address token, bool state)
        external
        override
        onlyCaller
    {
        if (state) {
            tokens.addAddressSet(token);
        } else {
            tokens.removeAddressSet(token);
        }
    }

    function getTokenExchangeLength() external view override returns (uint256) {
        return tokens.lengthAddressSet();
    }

    function getTokenExchangeAll()
        external
        view
        override
        returns (address[] memory)
    {
        uint256 len = tokens.lengthAddressSet();
        address[] memory token = new address[](len);
        for (uint256 i = 0; i < len; i++) {
            token[i] = tokens.atAddressSet(i);
        }
        return token;
    }

    function getTokenExchange(uint256 index)
        external
        view
        override
        returns (address)
    {
        return tokens.atAddressSet(index);
    }
}
...
interface INFTExchange {
    ...
    event MakeEvent(
        ...
        address maker,
        address bidder,
        uint256 ts //拍卖单：订单截止时间，一口价单：当前时间
    );
    ...
    function setTokenExchange(
        address token, //NFT合约地址
        bool state //是否开启交易
    ) external;

    function getTokenExchangeLength() external view returns (uint256);

    function getTokenExchangeAll() external view returns (address[] memory);

    function getTokenExchange(uint256 index) external view returns (address);
}
