...
contract Staking is AccessControl, ReentrancyGuard {
     ...
     */
    function setReward(uint256 amount) external onlyRole(ADMIN_ROLE) {
        pastProduced = _produced();
        produceTime = block.timestamp;
        rewardsPerEpoch = amount;

        emit SetReward(amount);
    }
    ...
    function stake(uint256 _amount) external {
        require(lockStake, "Staking: stake is not available now");
        require(
    ...
    function unstake(uint256 amount) external nonReentrant {
        require(islockUnstake, "Staking: unstake is not available now");
        Staker storage staker = stakers[msg.sender];
    ...
    function getUserInfo(address user) external view returns (Staker memory) {
        ...
        staker.rewardAllowed = getRewardInfo(user);
        return staker;
    }
