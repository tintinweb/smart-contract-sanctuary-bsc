...
contract USDTBeeStake {
    ...
    uint256 constant public POOLBNBBEEGAME_FEE = 20;
    ...
    address payable public poolbnbbeegameWallet;
    ...
    constructor(address payable ceoAddr, address payable devAddr, address payable markAddr, address payable bnbbeegameAddr) {
        require(!isContract(ceoAddr) && !isContract(devAddr) && !isContract(markAddr) && !isContract(bnbbeegameAddr));
		ceoWallet = ceoAddr;
		devWallet = devAddr;
        markWallet = markAddr;
        poolbnbbeegameWallet = bnbbeegameAddr;
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external {
        ...
        require(amount >= 5 ether, "Minimum deposit amount is 5 USDT");
        ...
        uint256 pFee = amount.mul(POOLBNBBEEGAME_FEE).div(PERCENT_DIVIDER);
		USDT.safeTransfer(ceoWallet, ceo);
		USDT.safeTransfer(devWallet, dFee);
        USDT.safeTransfer(markWallet, mFee);
        USDT.safeTransfer(poolbnbbeegameWallet, pFee);
