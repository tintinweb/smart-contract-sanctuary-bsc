...
contract DutchAuctionFactory is Ownable {
    ...
    uint256 public constant MIN_AUCTION_DURATION = 0.5 days;
    ...
    constructor(address _auctionToken, address _payToken, address _auctionTreasury, address _auctionAdmin) {
        setAuctionToken(_auctionToken);
        setPayToken(_payToken);
        setAuctionTreasury(_auctionTreasury);
        setAuctionAdmin(_auctionAdmin);
    }
    ...
    function createAuction(
        ...
        DutchAuction _newAuction = new DutchAuction();
        IERC20(auctionToken).safeTransferFrom(auctionTreasury, address(this), _totalTokens);
        IERC20(auctionToken).safeIncreaseAllowance(address(_newAuction), _totalTokens);
        // 2. initialize
        _newAuction.initialize(
            auctionToken,
            payToken,
            _totalTokens,
            _startTime,
    ...
    function setAuctionTreasury(address _treasury) public onlyOwner {
        require(_treasury != address(0), "Invalid address");
        auctionTreasury = _treasury;
        emit AuctionTreasuryUpdated(_treasury);
    }

    function setAuctionAdmin(address _admin) public onlyOwner {
