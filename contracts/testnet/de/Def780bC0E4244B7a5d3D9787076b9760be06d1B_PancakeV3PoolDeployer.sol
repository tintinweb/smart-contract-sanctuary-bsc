...
contract PancakeV3Pool is IPancakeV3Pool {
    ...
    function swap(
        ...
        uint256 fee0X128 = feeGrowthGlobal0X128;
        uint256 fee1X128 = feeGrowthGlobal1X128;

        // update fee growth global and, if necessary, protocol fees
        // overflow is acceptable, protocol has to withdraw before it hits type(uint128).max fees
        if (zeroForOne) {
            feeGrowthGlobal0X128 = state.feeGrowthGlobalX128;
            fee0X128 = feeGrowthGlobal0X128 - fee0X128;
            if (state.protocolFee > 0) protocolFees.token0 += state.protocolFee;
        } else {
            feeGrowthGlobal1X128 = state.feeGrowthGlobalX128;
            fee1X128 = feeGrowthGlobal1X128 - fee1X128;
        ...
        emit Swap(msg.sender, recipient, amount0, amount1, state.sqrtPriceX96, state.liquidity, state.tick, fee0X128, fee1X128);
        slot0.unlocked = true;
    }
