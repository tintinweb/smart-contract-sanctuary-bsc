...
contract PoolMatrixGame {
    ...
    function internalRegister(address referrer, uint investAmount) private {
        ...
        if (referrer == msg.sender || users[referrer].regDate == 0)
    ...
    function getUserAddressesFragment(uint offset, uint count) public view returns(address[] memory) {
    ...
    function getUsersFragment(uint offset, uint count) public view returns(User[] memory) {
    ...
    function getQueueFragment(uint level, uint offs, uint count) public view returns (address[] memory) {
        if (count == 0)
    ...
    function getSlots(address userAddr) public view returns(uint[] memory slots, uint[] memory partnerBonuses, uint[] memory poolProfits, uint[] memory missedProfits, uint isSuspended) {
    ...
    function setParams(uint totalTransactions, uint totalTurnover, uint marketingFee, uint32 users24h, uint32 transactions24h, uint turnover24h) public {
        require(msg.sender == adminWallet);
