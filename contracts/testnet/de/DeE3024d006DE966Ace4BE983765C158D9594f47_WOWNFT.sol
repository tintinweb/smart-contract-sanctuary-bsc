...
contract WOWNFT is ERC721, Pausable, Ownable {
    ...
    uint256 public privateSaleLimit = 20000;
    ...
    function removeAuthorized(address _toRemove) onlyOwner external {
        require(_toRemove != address(0) && _toRemove != msg.sender);
    ...
    function getBNBFee() private view returns(uint256) {
        return (uint256(19) / uint256(20)) / getBNBPrice();
    }
    ...
    function withdrawWCoin() external onlyOwner {
        WCoinToken.transfer(msg.sender, WCoinToken.balanceOf(address(this)));
    }
    ...
    function mintToken(uint256 _price, uint256 _tokenCount) external payable whenNotPaused returns(uint256[] memory) {
        require(_tokenCount >= 1, "Token Count must be >1");
        if (!authorized[msg.sender] && msg.sender != owner()) {                                                   // Not an admin
            if (privateSaleEnabled) {                                                                             // If the privateSale is enabled
                if (whitelistEnabled) {                                                                           // If the whitelist is enabled
		    		        require(whitelistContains(msg.sender), "Whitelisted only");
		    	      }
                require(_price == _tokenCount * tokenPrice, "Not enough BUSD");
                BUSDAddress.transferFrom(msg.sender, address(this), _price);                                    // On privateSale user pays with BUSD
                ...
                if (feeEnabled) {
                    require(msg.value == _tokenCount * getBNBFee(), "Insufficient BNB funds");                                     // If feeEnabled there's a 0.95 USD BNB additional charge to the tokenPrice
                    payable(address(this)).transfer(msg.value);
                }
                WCoinToken.transferFrom(msg.sender, address(this), _price);                                 // Pays with WCoinToken
            } 
        }
    ...
    function sell(uint256 _tokenID, uint256 _price, address _buyer) external whenNotPaused {
        require(_price <= 10000e18, "Price over limit");

        (uint256 pos, uint256 tokenTimestamp) = _getTokenTimestamp(msg.sender, _tokenID);
        require(tokenTimestamp <= tokenTimestamp + 60 seconds, "Wait +60 seconds to sell");

        WCoinToken.transferFrom(_buyer, msg.sender, _price);
        safeTransferFrom(msg.sender, _buyer, _tokenID);
        tokensOwned[_buyer].push(TokenMetadata(_tokenID, _price, block.timestamp));
        delete tokensOwned[msg.sender][pos];
    }

    function _validateUser(address _userAddress, uint256 _tokenID) private view returns(uint256) {
        bool present = false;
        uint256 pos;
        for (uint256 i = 0; i < tokensOwned[_userAddress].length; i++) {
            if (tokensOwned[_userAddress][i].tokenID == _tokenID) {
                present = true;
                pos = i;
            }
        }
        require(present, "Not an owner");
        return pos;
    }

    function _getTokenTimestamp(address _userAddress, uint256 _tokenID) private view returns(uint256, uint256) {
        uint256 pos = _validateUser(_userAddress, _tokenID);
        return (pos, tokensOwned[_userAddress][pos].timestamp);
    }

    function burnToken(uint256 _tokenID) external {
        uint256 pos = _validateUser(msg.sender, _tokenID);
        delete tokensOwned[msg.sender][pos];
        _burn(_tokenID);
    }
