...
/**
 ~ PLANS:
    plan 1 -   3% daily, 600% for 200 days (500% ROI)
    plan 2 -   3.5% daily, 385% for  110 days (285% ROI)
    plan 3 -   4% daily, 200% for  50 days (100% ROI)
...
contract BNBYieldCropsFarmer {
	...
	uint256 public MKT_BUYBACK_FEE = 30;
    ...
    uint public startTime = 1661893200; // Fri Aug 30 2022 21:00:00 GMT+0000 https://www.unixtimestamp.com/
	...
	constructor(address payable _dev1, address payable _dev2, address payable _dev3) {
        ...
        plans.push(Plan(200, 30,  1 * 1e17, 25 ether, 0, 0, 0, 0, true));
        plans.push(Plan(110, 35,  2 * 1e17, 30 ether, 0, 0, 0, 0, true));
        plans.push(Plan(50,  40,  3 * 1e17, 35 ether, 0, 0, 0, 0, true));
	}
	...
	function withdraw() public {
        ...
        totalAmount = totalAmount.sub(totalAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER)); /* 10% of withdrawable amount goes back to the project. */
