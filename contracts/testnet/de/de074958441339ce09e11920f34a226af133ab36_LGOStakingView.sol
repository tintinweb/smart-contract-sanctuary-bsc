...
contract LGOStakingView is Ownable, ILGOStakingView {
    ...
    uint256 public constant LGO_MAX_SUPPLY = 30_000_000 ether;
    ...
    function addEmission(uint256 _rewardsPerSecond, uint256 _startTimestamp, uint256 _endTimestamp)
        ...
        onlyOwner
    {
        uint256 length = emissions.length;
        if (length != 0) {
            EmissionInfo storage _emission = emissions[length - 1];
            if (!_emission.removed) {
                require(_startTimestamp >= _emission.startTimestamp, "LGOStakingView::addEmission: invalid time");
                _emission.endTimestamp = _startTimestamp;
            }
        }
        ...
        emit EmissionAdded(emissions.length - 1, _rewardsPerSecond, _startTimestamp, _endTimestamp);
    }

    function removeEmission(uint256 _emissionId) external onlyOwner {
        require(emissions.length > _emissionId, "LGOStakingView::removeEmission: invalid id");
