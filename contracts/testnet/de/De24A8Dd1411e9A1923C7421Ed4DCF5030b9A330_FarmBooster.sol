...
contract FarmBooster is Ownable {
    ...
    /// @notice floatie token.
    address public immutable FLOATIE;
    /// @notice floatie pool.
    address public immutable FLOATIE_POOL;
    ...
    /// @notice Floatie pool BOOST_WEIGHT precision
    ...
    /// @param _floatie FLOATIE token contract address.
    /// @param _floatiePool Floatie Pool contract address.
    ...
    /// @notice Checks if the msg.sender is the floatie pool.
    modifier onlyFloatiePool() {
        require(msg.sender == FLOATIE_POOL, "onlyFloatiePool: Not floatie pool");
        _;
    }
    ...
    /// @notice Floatiepool operation(deposit/withdraw) automatically call this function.
    /// @param _user user address.
    /// @param _lockedAmount user locked amount in floatie pool.
    /// @param _lockedDuration user locked duration in floatie pool.
    /// @param _totalLockedAmount Total locked floatie amount in floatie pool.
    /// @param _maxLockDuration maximum locked duration in floatie pool.
    function onFloatiePoolUpdate(
        ...
        emit UpdateFloatiePool(_user, _lockedAmount, _lockedDuration, _totalLockedAmount, _maxLockDuration);
    }
    ...
    /// @notice floatie pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IERC20(FLOATIE).balanceOf(FLOATIE_POOL);

        uint256 totalLockedAmount = IFloatiePool(FLOATIE_POOL).totalLockedAmount();

        uint256 pricePerFullShare = IFloatiePool(FLOATIE_POOL).getPricePerFullShare();
        ...
        uint256 boostedRatio = ((IFloatiePool(FLOATIE_POOL).totalShares() - flexibleShares) * BOOST_RATIO_PRECISION) /
            originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = IFloatiePool(FLOATIE_POOL).BOOST_WEIGHT();
        uint256 maxLockDuration = IFloatiePool(FLOATIE_POOL).MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    /// @param _pid pool id.
    /// @param _duration floatie pool average locked duration.
    ...
    function _boostCalculate(
        ...
        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = IFloatiePool(FLOATIE_POOL)
            .userInfo(_user);
