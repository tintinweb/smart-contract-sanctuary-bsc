...
contract BVStake is OwnableUpgradeable, DivestorUpgradeable {
	...
	function coutingApy() public view returns (uint256[3] memory apys) {
		...
		for (uint256 i; i < 3; i++) {
			uint256 stakeAmount = soltInfo[i].stakeAmount;
			if (stakeAmount == 0) continue;
			uint256 reward = ((2544 ether / 15) * stakeAmount) / pool.tvl;
			apys[i] = (reward * rPrice * 365) / (soltInfo[i].stakeAmount * sPrice);
		...
		)
	{
		...
		for (uint256 i; i < 3; i++) {
			...
			poolInfo[i] = [lockTime, soltInfo[i].rate];

			uint256 stakeAmount = uInfo.slots[i].stakeAmount;
			uint256 lastStakeTime = uInfo.slots[i].lastStakeTime;
			// stake 	releaseTIme
			uPoolInfo[i] = [stakeAmount, lastStakeTime > 0 ? lastStakeTime + lockTime : 0];
			info[2] += stakeAmount;
		}
	}
	...
	function verifyInviter(address account_, address inviter_) public view returns (bool) {
		if (userInfo[inviter_].stakeValue == 0) {
			return false;
		}
	...
	function viewReward(address account_) public view returns (uint256 reward, uint256 newDepted) {
		User memory uInfo = userInfo[account_];
		reward = uInfo.toClaim;
		newDepted = coutingDebt();
		if (uInfo.stakeValue == 0) {
			return (reward, newDepted);
		}
		reward += ((newDepted - uInfo.debted) * uInfo.stakeValue) / (ACC * 1e10);
	}
	...
	function stake(
		...
		uint256 newDebt;
		if (uInfo.stakeValue > 0) {
			(uInfo.toClaim, newDebt) = viewReward(msg.sender);
		} else {
			sInfo.stakeCount += 1;
			newDebt = coutingDebt();
		}
		...
		// uInfo.stakeValue += stakeValue;
		// uInfo.slots[slotId_].stakeAmount += stakeAmount_;
		// uInfo.slots[slotId_].lastStakeTime = block.timestamp;
	...
	function claimReward() public check(0) {
		(uint256 totalReward, uint256 newDepted) = viewReward(msg.sender);
		require(totalReward > 0, "not reward");

		uint256 refReward = (totalReward * refTotalRate) / 100;
		uint256 reward = totalReward - refReward;

		_takeInviterReward(totalReward, refReward);

		User storage uInfo = userInfo[msg.sender];
		uInfo.toClaim = 0;
		uInfo.debted = newDepted;
	...
	function _updateUserAndPool(
		...
		if (isAdd_) {
			...
			uInfo.stakeValue -= stakeValue_;
			uInfo.slots[slotId_].stakeAmount = 0;
			uInfo.slots[slotId_].lastStakeTime = 0;
		}

		pool.debted = newDebted_;
		pool.lastTime = block.timestamp;

		uInfo.debted = uInfo.stakeValue > 0 ? newDebted_ : 0;
	}
	...
	function unStake(uint256 slotId_) public check(slotId_) {
		...
		uint256 stakeValue = (stakeAmount * sInfo.rate) / 100;
		...
		// uInfo.stakeValue -= stakeValue;
		// uInfo.debted = uInfo.stakeValue > 0 ? newDebt: 0;
		// uInfo.slots[slotId_].lastStakeTime = 0;
		// uInfo.slots[slotId_].stakeAmount = 0;
