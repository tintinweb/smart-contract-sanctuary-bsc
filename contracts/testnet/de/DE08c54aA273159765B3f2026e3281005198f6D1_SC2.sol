...
contract SC2 is Ownable, OverflowToken {
    ...
    event stakeEnd(address owner, uint256 Amount, uint256 tokenType, uint256 stakeType, uint256 claim, uint256 date);
    event stakeReal(address owner, uint256 Amount, uint256 tokenType, uint256 stakeType, uint256 claim, uint256 date);
    ...
    function getStakeData() public view returns(address[] memory owners, uint256[] memory amounts, uint256[] memory date, uint256[] memory tokenTypes){
        ...
        return (owners, amounts, date, tokenTypes);
    }
        ...
        for(uint i=0; i<stakeNum; i++){
            if(stake_data[i].tokenType > 0){
                ...
                if(stake_data[i].stakedDate + staking_period[stake_data[i].stakeType] <= block.timestamp){
                    ...
                    emit stakeEnd(stake_data[i].owner, stake_data[i].amount, stake_data[i].tokenType, stake_data[i].stakeType, block.timestamp, stake_data[i].claimedAmount);
                    stake_data[i].tokenType = 0;
                }
                else{
                    emit stakeReal(stake_data[i].owner, stake_data[i].amount, stake_data[i].tokenType, stake_data[i].stakeType, stake_data[i].stakedDate + staking_period[stake_data[i].stakeType], stake_data[i].claimedAmount);
                }
            }
