...
contract MoniwarJackpotSpin is Ownable, ReentrancyGuard {
    ...
    function changeDevAdrr(address _dev) public onlyOwner {
        dev = _dev;
    }
    ...
    function setPercentFeeSpin(uint256 _feeSpin) public onlyDev {
        feeSpin = _feeSpin;
    }

    function setPercentFeeJackpot(uint256 _feeJackpot) public onlyDev {
        feeJackpot = _feeJackpot;
    }

    function setMowaFeeSpin(uint256 _mowaSpin) public onlyDev {
    ...
    function setQtyReward(uint256 qty3, uint256 qty4, uint256 qty5, uint256 qty6) public onlyDev {
    ...
    function spin(uint256 _tokenId, uint256 time) public nonReentrant {
        if(_tokenId > 0){
            ...
            mowaItemsCore.burnNFT(_msgSender(), _tokenId, 1);
