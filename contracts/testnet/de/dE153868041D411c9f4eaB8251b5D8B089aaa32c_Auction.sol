...
contract Auction is ReentrancyGuard, Ownable {
    ...
    //f.
    function MinterListNFT(uint256 _tokenId, uint256 _price, uint256 _amount, uint256 _period, bool _isERC721) public returns(uint256) {
    ...
    //g.
    function BuyerListNFT(uint256 _tokenId, uint256 _price, uint256 _amount, uint256 _period, bool _isERC721) public returns(uint256) {
        if(_isERC721){
            require(ylnft721.ownerOf(_tokenId) == msg.sender, "You haven't this token");
            require(ylnft721.getApproved(_tokenId) == address(this), "NFT must be approved to market");
