...
contract MintSynTest is Ownable {
    ...
    function getHealth(uint256 positionId) public view returns(uint256 assetValueInCollateral,uint256 colleteral,uint256 minRatio) {
        ...
        colleteral = pos.collateralAmount;
        minRatio = config.getMinCollateralRatio(pos.asset);
    }
    ...
    function withdraw(uint256 positionId,uint256 withdrawAmount) external {
        ...
        if((assetValueInCollateral.mul(config.getMinCollateralRatio(pos.asset))).div(1e20) > collateralAmountAfterSub){
    ...
    function mint(uint256 positionId,uint256 mintAmount) external {
        ...
        if((assetValueInCollateral.mul(config.getMinCollateralRatio(pos.asset))).div(1e20) > collateralAmount){
    ...
    function auction(uint256 positionId,uint256 burnAmount) external {
        ...
        if(assetValueInCollateral.mul(config.getMinCollateralRatio(pos.asset)).div(1e20) <  pos.collateralAmount){
        ...
        uint256 assetValueInCollateralDiscount = pos.mintAmount.mul(discountPrice).div(1e18);
