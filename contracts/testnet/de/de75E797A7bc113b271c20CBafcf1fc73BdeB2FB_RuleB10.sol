...
contract RuleB10 is IBEP20 {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "RULE : This function is only accesible by owner.");
        _;
    }
    ...
    function burnToken(uint256 amount) public returns (bool) {
        _wallets[msg.sender].balance = _wallets[msg.sender].balance.sub(amount * 10**_decimals, "RULE : Burn Amount Exceeds Balance");
        _total_supply = _total_supply.sub(amount * 10**_decimals);
        emit Burn(msg.sender, amount * 10**_decimals);
        emit Transfer(msg.sender, address(0), amount * 10**_decimals);
        return true;
    }

    function addDirectorWallet(address account) public returns (bool) {
        require(_wallets[account].balance > 0, "RULE : Wallet Is Empty To Create Director.");
        require(_wallets[account].is_director == false, "RULE : Wallet Is Already Director.");
        require(_wallets[account].is_investor == false, "RULE : Wallet Is Already Investor.");
    ...
    function addInvestorWallet(address account) public onlyOwner returns (bool) {
        require(_wallets[account].balance > 0, "RULE : Wallet Is Empty To Create Investor.");
        require(_wallets[account].is_investor == false, "RULE : Wallet Is Already Investor.");
        require(_wallets[account].is_director == false, "RULE : Wallet Is Already Director.");
        _wallets[account].total_lock_amount = _wallets[account].balance;
        _wallets[account].total_release_amount = 0;
    ...
    function _approve( address owner, address spender, uint256 amount ) private {
        require(owner != address(0), "RULE : Approve from the zero address");
        require(spender != address(0), "RULE : Approve to the zero address");
        require(_wallets[owner].balance >= amount, "RULE : Can not allow more than balance.");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, msg.sender, _allowances[sender][msg.sender].sub(amount, "RULE :transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(msg.sender, spender, _allowances[msg.sender][spender].sub(subtractedValue, "RULE :decreased allowance below zero"));
        return true;
    }
        ...
        for (uint256 i = 0; i < _total_directors; i++) {
            if ( _wallets[_directors[i]].current_release_iteration < Variables._director_total_release_iteration ) {
                if ( _wallets[_directors[i]].next_release_time <= block.timestamp ) {
                    // get multiplier
                    uint256 multiplier = (((block.timestamp - _wallets[_directors[i]].next_release_time) / 60) / Variables._director_release_every_days_after_locking) + 1;
        ...
        for (uint256 i = 0; i < _total_investors; i++) {
            if ( _wallets[_investors[i]].current_release_iteration < Variables._investor_total_release_iteration ) {
                if ( _wallets[_investors[i]].next_release_time <= block.timestamp ) {
                    uint256 multiplier = (((block.timestamp - _wallets[_investors[i]].next_release_time) / 60) / Variables._investor_release_every_days_after_locking) + 1;
    ...
    function _transfer(
        ...
        require(sender != address(0), "RULE :transfer from the zero address");
        require(recipient != address(0), "RULE :transfer to the zero address");
        if (sender.isContract()) {
            require(
                _wallets[sender].balance >= amount,
                "RULE : transfer amount exceeds balance"
            );
        } else {
            require(
        ...
        if ( _wallets[sender].is_director || _wallets[sender].is_investor ) {
            ...
            require(
                ...
                "RULE : transfer amount exceeds allowed balance"
            );
        }
    ...
    function _transferOwnership(address newOwner) internal {
        require(
            newOwner != address(0),
            "RULE : Can not transfer ownership to ZERO address."
        );
