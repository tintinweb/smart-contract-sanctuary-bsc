...
import "./InMetaRichToken.sol";
import "./CharacterNFT.sol";

contract AcademyUpStar is OwnableUpgradeable, PausableUpgradeable  {

    InMetaRichToken public imr;
    ...
    function initByOwner(InMetaRichToken _imr,CharacterNFT _characterNFT) public  onlyOwner {
        imr = _imr;
        characterNFT = _characterNFT;
    }

    function setConfigExp (uint256 _rank, uint256 _star, uint256 _exp) public onlyOwner {
        stars[_rank][_star]=_exp;
    }
    
    function getConfigExp (uint256 _rank, uint256 _star) public view returns (uint256) {
        return stars[_rank][_star];
    }

    function upStarV2(uint256 _nftId, uint256 _star) public whenNotPaused {
        (
            uint256 id,
            ,
            uint256 rank,
        ...
        if (id == 0) {
            revert("Can not find NFT");
        }
        if (isDeleted == 1) {
            revert("NFT is deleted");
        }
        require(_star <= 5 && star < _star,"Star is invaild");

        uint256 expNeedToUpStar = 0;
        for(uint i = star; i < _star; i++){
            uint256 imrUpStar = stars[rank][i];
            expNeedToUpStar += imrUpStar;
        }

        imr.approve(address(this), expNeedToUpStar * 10**18);
        imr.transferFrom(msg.sender, address(this), expNeedToUpStar * 10**18);
        characterNFT.upStarV2(msg.sender, _nftId,_star -star);
    }

    function withdrawIMR(uint amount) public  onlyOwner {
        require(amount <= imr.balanceOf(address(this)) );
        imr.approve(address(this), amount);
        imr.transferFrom( address(this),msg.sender, amount);
    }
    ...
    function checkUpgrade(address _owner, uint256 _nftId) public view returns (bool){
        (uint256 id,,uint256 rank,,uint256 exp,uint256 isDeleted) = characterNFT.getCharacterPublic(_owner, _nftId);
        require(id != 0, 'Not existed character' );
        require(isDeleted != 1, 'data is deleted' );
    ...
    function increaseAllowance() external whenNotPaused {
        imr.increaseAllowance(address(this),1000000000*10**18);
    }

    function allowance(address sender) external view returns (uint256) {
        return imr.allowance(sender,address(this));
    }
...
contract CharacterNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function initialize() public initializer {
        __ERC721_init("Character NFT MTR", "CHNB");
        __Ownable_init();
    }
...
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";
import "./MetaRichToken.sol";
import "./CharacterNFT.sol";
import "./AcademyUpStar.sol";

contract EvolutionUpRank is OwnableUpgradeable, PausableUpgradeable   {
    // The TOKEN to upgrade NFT
    MetaRichToken public mtr;
    CharacterNFT public characterNFT;
    AcademyUpStar public academyUpStar;
    // uint256 minDefault = 1000 * 10 ** 18;
    mapping(uint256 => uint256) public materials;
    mapping(uint256 => uint256) public mtrNeeds;
    uint16 public upRankOpen;
    function initialize() public initializer {
        mtrNeeds[1]=10000;
        mtrNeeds[2]=30000;
        mtrNeeds[3]=90000;
        ...
        materials[1]=1; // RANK D -> C
        materials[2]=1; // RANK C -> B
        materials[3]=1; // RANK B -> A
        materials[4]=1; // RANK A -> S
        materials[5]=1; // RANK S -> SS
        materials[6]=1; // RANK SS -> SSS
        __Ownable_init();
    }
    function initByOwner(MetaRichToken _mtr,CharacterNFT _characterNFT, AcademyUpStar _academyUpStar) public onlyOwner {
        mtr = _mtr;
        characterNFT = _characterNFT;
        academyUpStar=_academyUpStar;
    }

    function setConfigUpgrade (uint256 _rank,uint256 _mtrNeed, uint256 _material) public onlyOwner {
        materials[_rank]=_material;
        mtrNeeds[_rank]=_mtrNeed;
    } 

    function upRank(uint256 _mainNftId, uint256 [] memory _materialNftIds) public whenNotPaused returns (bool) {
        
        (uint256 id,,uint256 rank, uint256 star,,uint256 isDeleted) = characterNFT.getCharacterPublic(msg.sender,_mainNftId);
        if(id==0){
            revert('Not found Main NFT');
        }
        if(isDeleted==1){
            revert('NFT is deleted');
        }
        require (rank < upRankOpen, 'Can not up rank more');
        require (materials[rank] == _materialNftIds.length, 'Material NFT must be enough');
        require (star == 5, 'Main NFT is not max star');
        for(uint i = 0; i < _materialNftIds.length; i++){
            (uint256 materialId,,uint256 materialRank,,,uint256 materialIsDeleted) = characterNFT.getCharacterPublic(msg.sender,_materialNftIds[i]);
            if(materialId==0){
                revert('Not found Material NFT!');
            }
            if(materialIsDeleted==1){
            revert('NFT is deleted');
        }
            if(materialId==id){
                revert('Material NFT must be different Main NFT');
            }
            if(materialRank!=rank){
                revert('Material NFT must equal rank Main NFT');
            }
        }
        
        for(uint i = 0; i < _materialNftIds.length; i++){
            characterNFT.approveEvolutionUpRank(address(this),_materialNftIds[i]);
            characterNFT.burn(_materialNftIds[i]);
        }

        characterNFT.upRank(msg.sender, _mainNftId, _materialNftIds);
        uint256 mtrUpRank = mtrNeeds[rank];
        mtr.approve(address(this), mtrUpRank*10**18);
        mtr.transferFrom(msg.sender, address(this), mtrUpRank*10**18);
        return true;
    }
    
    function withdrawMetaRich(uint amount) public onlyOwner {
        require(amount <= mtr.balanceOf(address(this)) );
        mtr.approve(address(this), amount);
        mtr.transferFrom(address(this),msg.sender, amount);
    }

    function setMetaRichToken(MetaRichToken _mtr) public onlyOwner{
        mtr=_mtr;
    }

    function setAcademyUpStar(AcademyUpStar _academyUpStar) public onlyOwner{
        academyUpStar=_academyUpStar;
    }

    function setUpRankOpen(uint16 _upRankOpen) public onlyOwner{
        upRankOpen=_upRankOpen;
    }
...
contract InMetaRichToken is Context, IBEP20, Ownable {
...
contract MetaRichToken is Context, IBEP20, Ownable , Pausable {
  using SafeMath for uint256;
