...
contract SwapNFT is Accept {
  ...
  // purchase any order listed on fixed price
  function purchaseOrder(uint256 orderNumber) external payable {
    ...
    if (keccak256(bytes(tokenOf[tokenAddress])) == keccak256(bytes("bnb"))) {
      order.seller.transfer(order.price.sub(_fee.add(_royality))); // - (_fee + _royality)
  ...
  // Cancel order Whose listed order on market place
  ...
  function addOrder(
    ...
    require(!isOrderExist[orderNumber] ,"OrderNumber Exist");
    require(IERC721(nftAddress).ownerOf(tokenId) == msg.sender, "Should be owner");
  ...
  function addMultiOrder(
    ...
    require(IERC721(nftAddress).isApprovedForAll(msg.sender, address(this)) == true, "NOT_APPROVED_FOR_ALL");
    require(tokens[tokenType] != address(0), "Invalid Token");
    require(tokenId.length == orderNumber.length,"Token and Order length mismatch");
    for (uint256 i = 0; i < tokenId.length; i++) {
      require(!isOrderExist[orderNumber[i]] ,"OrderNumber Exist");
      ...
      emit AddOrder(orderNumber[i], tokenId[i], nftAddress, msg.sender, price, 0, IERC721(nftAddress).creator(tokenId[i]), royalityPercent);
    }
  }

  // Add multiple nfts as a bundle
...
// Make Discount offer on specific order of owner
  ...
  // Buyer make Offer to user whose token is not listed
  function makeOffer(
    address nftAddress,
    uint256 tokenId,
    uint256 price,
    ...
    require(!isTokenListed[tokenId], "Token is already on sale");
    require(tokens[tokenType] != address(0), "Payment Method is Invalid");
    require(IERC721(nftAddress).ownerOf(tokenId) != address(0), "Token is not minted");
    require(!isOrderExist[orderNumber] ,"OrderNumber Exist");
    if (keccak256(bytes(tokenType)) == keccak256(bytes("bnb"))) {
      require(msg.value > 0, "Please send a Valid Offer");
      pendingOrders[orderNumber].price = msg.value;
    } else {
      require(price > 0, "Please send a Valid Offer");
  ...
  function acceptOffer(uint256 orderNumber) external {
    Order storage order = pendingOrders[orderNumber];
    require(IERC721(order.nftAddress).ownerOf(order.tokenId) == msg.sender, "Not owner of this nft");
    require((IERC721(order.nftAddress).getApproved(order.tokenId) == address(this)) || (IERC721(order.nftAddress).isApprovedForAll(order.seller, address(this))), "Token is not Approved");
    IERC721(order.nftAddress).safeTransferFrom(msg.sender, order.buyer, order.tokenId);
    isTokenListed[order.tokenId] = false;
    address tokenAddress = order.tokenAddress;
    uint256 _fee = _computeFee(order.price);
    uint256 _royality = computeRoyality(order.price, order.royalityPercent);
  ...
  // Detail of All token ids in bundle and creators of that nfts
  ...
  // Cancel Offer send by buyer
  function cancelOffer(uint256 orderNumber) external {
    Order memory order = pendingOrders[orderNumber];
    require(order.buyer == msg.sender, "ONLY_OFFER_SENDER_CALL");
    cancelledOrders[orderNumber] = order;
    order.isOffer = false;
    isOrderExist[orderNumber] = false;
    delete pendingOrders[orderNumber];
