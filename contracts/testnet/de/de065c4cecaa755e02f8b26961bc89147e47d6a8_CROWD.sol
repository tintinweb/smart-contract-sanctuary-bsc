...
contract CROWD is BEP20, Ownable {
	...
	function setInvestorRewardsFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(communityRewardsFee[0].add(buy)  <= 3000 , "Max fee limit reached for 'BUY'");
		require(communityRewardsFee[1].add(sell) <= 3000 , "Max fee limit reached for 'SELL'");
		require(communityRewardsFee[2].add(p2p)  <= 3000 , "Max fee limit reached for 'P2P'");
	...
	function setCommunityRewardsFee(uint256 buy, uint256 sell, uint256 p2p) external onlyOwner {
	    require(investorRewardsFee[0].add(buy)  <= 3000 , "Max fee limit reached for 'BUY'");
		require(investorRewardsFee[1].add(sell) <= 3000 , "Max fee limit reached for 'SELL'");
		require(investorRewardsFee[2].add(p2p)  <= 3000 , "Max fee limit reached for 'P2P'");
	...
	function setCommunityRewardsWallet(address payable newWallet) external onlyOwner() {
	...
	function setInvestorRewardsWallet(address payable newWallet) external onlyOwner() {
        require(newWallet != address(0), "zero-address not allowed");
	...
	function _transfer(address from, address to, uint256 amount) internal override {
		...
		if (!inSwapping && canSwap && swapEnable && automatedMarketMakerPairs[to]) 
		{
			 ...
			 if(tokenToSwap >= swapTokensAtAmount) 
			 {
				...
				uint256 communityRewardsPart = newBalance.mul(tokenToCommunityRewards).div(tokenToSwap);
				uint256 investorRewardsPart  = newBalance.mul(tokenToInvestorRewards).div(tokenToSwap);
				
				if(communityRewardsPart > 0) {
				    IBEP20(BUSD).transfer(communityRewardsWallet, communityRewardsPart);
					communityRewardsFeeTotal = communityRewardsFeeTotal.sub(swapTokensAtAmount.mul(tokenToCommunityRewards).div(tokenToSwap));
				}
				
				if(investorRewardsPart > 0) {
				    IBEP20(BUSD).transfer(investorRewardsWallet, investorRewardsPart);
					investorRewardsFeeTotal = investorRewardsFeeTotal.sub(swapTokensAtAmount.mul(tokenToInvestorRewards).div(tokenToSwap));
				}
			 }
		}
	...
	function collectFee(uint256 amount, bool sell, bool p2p) private returns (uint256) {
		...
		uint256 communityReward = amount.mul(p2p ? communityRewardsFee[2] : sell ? communityRewardsFee[1] : communityRewardsFee[0]).div(10000);
		        communityRewardsFeeTotal = communityRewardsFeeTotal.add(communityReward);
		
		uint256  investorReward = amount.mul(p2p ? investorRewardsFee[2] : sell ? investorRewardsFee[1] : investorRewardsFee[0]).div(10000);
	...
	function swapTokenForBUSD(uint256 tokenAmount) private lockTheSwap{
        ...
        pancakeSwapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAmount, 0, path, address(this), block.timestamp);
    }
