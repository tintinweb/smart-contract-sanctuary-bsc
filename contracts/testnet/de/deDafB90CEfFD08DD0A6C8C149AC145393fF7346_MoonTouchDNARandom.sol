...
contract MoonTouchDNARandom is  SecurityBase{
    ...
    function setIRepeater(address repeater) public {
    ...
    function getMoonTouchDNA()public returns(string memory){
        uint256 seed = getDraw(500);
        // uint256 seed = rand(_seed);
        DNA memory dna;
        dna.batch="A01";
        dna.level="00";
        dna.mood="3";
    ...
    function getGene(DNA memory dna)public view {
        getGeneDate(dna,1);
        getGeneDate(dna,2);
    }

    function getGeneDate(DNA memory dna,uint256 typeNumber)public view {
        string memory date;
        if(typeNumber == 1){
    ...
    function getImage(DNA memory dna)public view {
        (dna.image,dna.seed)=weightNmb(dna.seed,ImageWeight,ImageWeightSum);
    }

    function getDurability(DNA memory dna)public view {
    ...
    function getAttribute(DNA memory dna)public view {
    ...
    function getAttributeRandom(DNA memory dna, RandomCtx memory ctx )public view  {
    ...
    function getAttributeNumber(RandomCtx memory ctx,uint256 ctxSeed,uint256 attributeNumber,uint256 AttributeType,uint256 residueStat) public view  returns (uint256 rewardDate,uint256 seeddate){
    ...
    function getRandNumber(uint256 randDate,uint256 scope)public pure returns(uint256 rt){
    ...
    function getDraw(uint256 gross)public     returns (uint256 win){
        win =IRepeater(IRepeaterAddress).get_random_int(gross,0,msg.sender);
    }

    function weightNmb(uint256 seed,uint256[] memory weight,uint256 gross)public view  returns (uint256 win ,uint256 seeddate ){
        uint256 newdate = 0;
        uint256 draw;
