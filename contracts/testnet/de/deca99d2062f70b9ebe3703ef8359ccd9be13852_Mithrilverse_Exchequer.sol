                                   ...
                                   Mithrilverse_Permissioned
{
    ...
    /// Set taxes for new Ore taxers.
    ///
    /// @param _taxers as addressess of taxing contracts.
    /// @param amounts as quantities of ore to burn.
    function createOreTaxes(address[] memory _taxers, uint256[] memory amounts) external onlyOwner {
        uint256 length = _taxers.length;

        for (uint256 i = 0; i < length; i++) {
            address taxer = _taxers[i];
            taxers[taxers.length] = taxer;
            taxRates[taxer] = amounts[i];
            isPermissioned[taxer] = true;
        }
    }
