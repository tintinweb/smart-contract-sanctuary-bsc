...
contract DarkReferral is OwnableUpgradeSafe, IReferral {
    using SafeMath for uint256;
    ...
    mapping(address => uint256) public rewards; // account => reward
    mapping(address => mapping(address => uint256)) public tokenRewards; // account => token => reward
    mapping(address => address[]) public referralOf;

    uint256 public lastClaim;
    mapping(address => uint256) public lastTokenClaim; // token => block_number
    uint256 public referralPercent = 250; // 2.5%
    ...
    function setReferralPercent(uint256 _referralPercent) external onlyOwner {
        require(_referralPercent <= 2000, "_referralPercent is over 20%");
        referralPercent = _referralPercent;
    }

    function set(address from, address to) external override isAuthorised {
        if (from != address(0x0) && to != address(0x0) && from != to && referral[to] == address(0x0)) {
            referral[to] = from;
    ...
    function numberReferralOf(address add) external view override returns (uint256) {
    ...
    function onCommission(
        ...
        referralEarned[_from][_to] = referralEarned[_from][_to].add(_amount);
        totalEarned[_from] = totalEarned[_from].add(_amount);
        emit Commission(_from, _to, _amount);
    }

    function reward(address addr) external payable override isAuthorised {
        if (addr != address(0x0)) {
            address ref = refOf(addr);
            if (ref != address(0x0)) {
                uint256 amountForRef = (msg.value * referralPercent) / 10000;
    ...
    function rewardToken(
        ...
        if (addr != address(0x0)) {
            address ref = refOf(addr);
            if (ref != address(0x0)) {
                uint256 amountForRef = (amount * referralPercent) / 10000;
    ...
    function claimReward() external {
        uint256 _transferAmount = rewards[msg.sender];
    ...
    function claimTokenReward(address token) external {
        uint256 _transferAmount = tokenRewards[token][msg.sender];
        require(_transferAmount > 0);
    ...
    function withdrawAll() external onlyOwner {
        ...
        require(block.number - lastClaim > BLOCKS_PER_DAY * 180);
        msg.sender.transfer(address(this).balance);
    }

    function withdrawAllToken(address token) external onlyOwner {
        // Owner can withdraw all balance if in 6 months there is not any claim
        require(block.number - lastTokenClaim[token] > BLOCKS_PER_DAY * 180);
