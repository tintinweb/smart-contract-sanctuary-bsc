    ...
    Ownable
{
    ...
    uint256 public price = 0.15 * 10**18;

    uint256 public buy_limit_per_address = 4;

    uint256 public sell_begin_time = block.timestamp;
    ...
    constructor() {
        _name = "DreamFarmPond";
        _symbol = "DFP";
        setBaseURL("https://www.dreamfarm.io/Pond/token/");
    }
    ...
    function buy(uint256 amount, uint256 adv_time) public payable {
        require(
            block.timestamp >= SafeMath.sub(sell_begin_time, adv_time),
            "Purchase_Not_Enabled"
        );
        require(
            SafeMath.add(balanceOf(msg.sender), amount) <=
                buy_limit_per_address,
    ...
    function setSale(uint256 _tokenId, uint256 setType) public {
        ...
        require(
            owner() == sender || ownerOf(_tokenId) == sender,
            "Invalid_Owner"
        );
        require(setType == 1 || setType == 2, "Unknow_setType");
    ...
    function offload(uint256 _tokenId, address receiver) public {
        ...
        address sender = _msgSender();
        require(
            _onSaleList[_tokenId] == sender || owner() == sender,
            "Invalid_Owner"
        );
