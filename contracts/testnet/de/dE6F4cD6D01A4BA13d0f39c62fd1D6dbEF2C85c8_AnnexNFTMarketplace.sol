...
contract AnnexNFTMarketplace is Initializable, OwnableUpgradeable, Verification, UUPSUpgradeable {
    address wbnbAddress;
    ...
    function initialize(address _wbnb, address _ann) public initializer {
        __Ownable_init();
    ...
    function acceptOfferBid(acceptOfferBidData memory _transferData) external payable onlyProxy {
    ...
    function buyNFT(buyNFTData memory _buyData) external payable onlyProxy{
        ...
        if(_buyData.percent > 0) {
            ...
            if(_buyData.currency==1) {
                ...
                transferERC20ToOwner(msg.sender, address(this), royaltyPercent, annexTokenAddress);
                uint256 amount = royaltiesANN[_buyData.collectionId];
                royaltiesANN[_buyData.collectionId] = amount + royaltyPercent;
            }
        }
    ...
    function transferForFree(transferNFTData memory _transferData) public onlyProxy {
        require(!seenNonces[msg.sender][_transferData.nonce], "Invalid request");
        seenNonces[msg.sender][_transferData.nonce] = true;
        require(verify(msg.sender, msg.sender, _transferData.amount, _transferData.encodeKey, _transferData.nonce, _transferData.signature), "invalid signature");
        uint256 tokenId = _transferData.tokenId;
        ...
        NFT nftToken = NFT(_transferData.nft);
        nftToken.safeTransferFrom(_transferData.currentOwner, _transferData.newOwner, tokenId);
    }
    ...
    function checkNFTOwner(address cAddress, uint256 token) public onlyProxy returns (address) {
        NFT nftToken = NFT(cAddress);
        return nftToken.ownerOf(token);
    }
    fallback () payable external {}
    receive () payable external {}
    function updatePlatformSharePercent(uint256 percent) public onlyOwner onlyProxy {
    ...
    function withdrawANN() public onlyOwner onlyProxy {
    ...
    function withdrawWBNB() public onlyOwner onlyProxy {
        ERC20Token wbnb = ERC20Token(wbnbAddress);
        uint256 balance = wbnb.balanceOf(address(this));
        require(balance >= 0, "insufficient balance" );
        wbnb.transfer(owner(), balance);
    }
    function transferRoyalties(uint256 collection, address payout) public onlyOwner onlyProxy {
