...
contract INOBami is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 public amountFcfsStaking;
    ...
    mapping(address => bool) private userIsBought;
    ...
    event fcfsClaimNftComplete(
        address claimer, 
        bool isClaim, 
        address devWallet,
        bool fcfsWhiteList
    );
    event priorClaimNftComplete(
        address claimer, 
        bool isClaim, 
        address devWallet,
        bool priorWhiteList
    ...
    function setPriorLevelStake(uint256[] calldata _amountOfLevelStake, uint256[] calldata _maxNumberOfNftToBuy) public onlyOwner {
    ...
    function fcfsClaimNft() external nonReentrant{
        ...
        require(userIsBought[msg.sender] == false, "You must buy first");
        require(fCFSWhiteList[msg.sender]==true, "You are not in white list");
        ...
        emit fcfsClaimNftComplete(msg.sender, isClaimed[msg.sender], devWallet, priorWhiteList[msg.sender]);
    }
    function priorClaimNft() external nonReentrant{
        require(block.timestamp >= startTimeClaim, "Claim time is not started yet");
        require(block.timestamp <= endTimeClaim,"Claim time ended");
        require(userIsBought[msg.sender] == false, "You must buy first");
        require(priorWhiteList[msg.sender]==true, "You are not in white list");
        ...
        for(uint256 index = NftQuantityPerUser[msg.sender]-1; index >= 0; index--)
        {
            uint256 priorNftId = ERC721Enumerable(nftAddess).tokenOfOwnerByIndex(address(this), index);
            IERC721(nftAddess).transferFrom(address(this),msg.sender,priorNftId);
        }
        isClaimed[msg.sender] = true;
        
        emit priorClaimNftComplete(msg.sender, isClaimed[msg.sender], devWallet, priorWhiteList[msg.sender]);
    }
    ...
    function priorStaking(uint256 _levelStakeId) external nonReentrant{
        require(priorWhiteList[msg.sender] == false, "You are already on the Prior whitelist");
        require(fCFSWhiteList[msg.sender] = false, "You have been in FCFS whitelist");
        ...
        emit priorStakingComplete(_levelStakeId, amountOfLevelStake, maxNumberOfNftToBuy, msg.sender);
    }

    function setAmountFcfsStaking (uint256 _amountFcfsStaking) public onlyOwner {
        amountFcfsStaking = _amountFcfsStaking;
    }

    function fcfsStaking() external nonReentrant {
        require(fCFSWhiteList[msg.sender] == false, "You are already on the FCFS whitelist");
        require(priorWhiteList[msg.sender] = false, "You have been in prior whitelist");
        require(IERC20(tokenAddress).balanceOf(msg.sender) >= amountFcfsStaking, "Not enough balance");

        IERC20(tokenAddress).safeTransferFrom(msg.sender, address(this), amountFcfsStaking);
