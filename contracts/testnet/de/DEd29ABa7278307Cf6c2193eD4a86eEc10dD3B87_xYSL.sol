...
@dev this contract is for xYSL token.
     ...
     Admin.WBNB(); address of WBNB token.
     Admin.whitelist(); address of whitelist contract address.
     Admin.Treasury();  address of Treasury contract.
     Admin.POL();  address of POL contract.
     Admin.temporaryHolding(); address of Temporary holding.
     _router address of Router.
     _admin address of Admin contract.
 */
contract xYSL is ERC20, AccessControl, Pausable {
    ...
    uint256 public DAYS_IN_SECONDS = 10; // 1 day = 86400; for testing 1 day = 10s
    ...
    uint256 public lockTransactionTime = 10 seconds; 
    uint256[] public xYSL_Tax_Allocation = [500, 500, 500];//index-1 => BUSD Protocol, index-2 => YSL Vault, index-3 => YSL_BUSD Vault; All index value multiply by 100;
    ...
    */
     ...
     */
    function swapOldXYSL() external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint day = (block.timestamp - deploymentTime)/DAYS_IN_SECONDS;
        ...
        IPancakeRouter02(router).swapExactTokensForTokens(oldXYSL.balanceOf(address(this)),0,path,Admin.TeamAddress(),block.timestamp+1000)[1];
    }
   ...
   /**
    ...
    Note only admin role can call 
     */
    function setMigrateDaysLimit(uint _xYSLMigrateDaysLimit) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool){
    ...
    @param _teamAddress Days limit for migrate old xYSL in to new xYSL token
    ...
    function setxYSLAndAllocationTax(
        ...
        require(tax > 0, "xYSL: Tax should be greater than zero");
        require(
        ...
        require(total == tax, "xYSL: Tax not equal to sum of allocation tax");
        xYSL_Tax = tax;
    ...
    function _transfer(
        ...
        if(isContract(sender) || isContract(recipient)){
            require(IWhitelist(Admin.whitelist()).getProtocolAddresses(sender) || IWhitelist(Admin.whitelist()).getProtocolAddresses(recipient),
            "XYSL: No external contract interact with XYSL");
        } else {
        if (
            !(IWhitelist(Admin.whitelist()).getUserAddresses(sender)) &&
            !(IWhitelist(Admin.whitelist()).getUserAddresses(recipient))
        ) {
        require(
            restrictTransfer[sender] != block.number,
            "bYSL: you can't interact in same block"
        );
            ...
            uint256 taxAmount = (amount *
                (xYSL_Tax_Allocation[0] + xYSL_Tax_Allocation[1])) / 10000;
            _burn(sender, (amount * xYSL_Tax_Allocation[2]) / 10000);
            
            if (
                IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(amount,path)[1] >= xYSL_ThresholdLimit) {     
                IERC20(address(this)).approve(
                    Admin.ApeswapRouter(),
                    taxAmount
                ); //BSC Testnet pancake router address
                uint256 convertedBUSD = IUniswapV2Router02(
                    Admin.ApeswapRouter()
                ...
                IERC20(Admin.BUSD()).transfer(
                    Admin.Treasury(),
                    (convertedBUSD * xYSL_Tax_Allocation[0]) / 1000
                ); // 1% allocation  = 10% of BUSD
                IERC20(Admin.BUSD()).transfer(
                    Admin.temporaryHolding(),
                    (convertedBUSD * xYSL_Tax_Allocation[1]) / 1000
                );
            }
            amount -= (taxAmount + (amount * xYSL_Tax_Allocation[2]) / 10000);
            ...
            if( timeInDays > 2){
                ...
                exitRateAmount = ((amount * exitRateOfDay)/100);
            }
            else{
                exitRateAmount = ((amount * 1)/1000);
            }
            ...
            uint prev = IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(amount ,path)[1];
            super._transfer(sender, recipient, amount);
            uint current = IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(amount ,path)[1];
            require(prev + ((prev * priceImpactProtection)/1000) >= current && prev - ((prev * priceImpactProtection)/1000) <= current,
                "xYSL: The difference b/w prev and current is greater than or less than the priceImpactProtection");
            restrictTransfer[sender] = block.number;
            return;
            }}
        super._transfer(sender, recipient, amount);
     ...
     */
    function setPriceImpactProtection(uint value) public returns(uint) {
        priceImpactProtection = value;
    }
    /**
    setter function for LockTransactionTime
     */
    function setLockTransactionTime(uint time) public {
...
interface IWhitelist {
    function getProtocolAddresses(address _protocol) external view returns(bool);
    function getUserAddresses(address _user) external view returns(bool);
    function addProtocolWhiteList(address[] calldata _protocol) external;
    function addUserWhiteList(address[] calldata _user) external;
    function revokeProtocolWhiteList(address[] calldata _protocol) external;
    function revokeUserWhiteList(address[] calldata _user) external;
}
 ...
 */
contract WhiteList is AccessControl, Initializable{

    mapping(address => bool) public protocolAddresses; // contract addresses => bool
    mapping(address => bool) public userAddresses; //user addresses => bool

    bytes32 constant public ADMIN_ROLE = keccak256("ADMIN_ROLE"); //role byte for admin
     ...
     */
    function getProtocolAddresses(address _protocol) public view returns(bool){
        return(protocolAddresses[_protocol]);
    }

    /**
    @dev get contract address whitlisted or not

    @param _user address of contract
     */
    function getUserAddresses(address _user) public view returns(bool){
        return(userAddresses[_user]);
    }
    ...
    Note only admin can call
     */
    function addProtocolWhiteList(address[] calldata _protocol) external onlyRole(ADMIN_ROLE){
        require(hasRole(ADMIN_ROLE,msg.sender),"WhiteList: don't have access");
        for(uint i = 0; i < _protocol.length; i++){
            require(_isContract(_protocol[i]),"Whitelist: Not a contract address");
            protocolAddresses[_protocol[i]] = true;
        }  
    }

     /**
    @dev function to add user in whitelist

    @param _user is array of address to whitelist

    Note only admin can call
     */
    function addUserWhiteList(address[] calldata _user) external onlyRole(ADMIN_ROLE){
        require(hasRole(ADMIN_ROLE,msg.sender),"WhiteList: don't have access");
        for(uint i = 0; i < _user.length; i++){
            require(!_isContract(_user[i]),"Whitelist: Not a user address");
            require(_user[i] != address(0),"WhiteList: invalid address");
            userAddresses[_user[i]] = true;
        }
    }
    ...
    Note only admin can call
     */
    function revokeProtocolWhiteList(address[] calldata _protocol) external onlyRole(ADMIN_ROLE){
        require(hasRole(ADMIN_ROLE,msg.sender),"WhiteList: don't have access");
        for(uint i = 0; i < _protocol.length; i++){
            require(protocolAddresses[_protocol[i]],"WhitList: not whitelisted");
            protocolAddresses[_protocol[i]] = false;
        }  
    }

    /**
    @dev function to remove user address from whitlist

    @param _user array of address to remove

    Note only admin can call
     */
    function revokeUserWhiteList(address[] calldata _user) external onlyRole(ADMIN_ROLE){
        require(hasRole(ADMIN_ROLE,msg.sender),"WhiteList: don't have access");
        for(uint i = 0; i < _user.length; i++){
            require(userAddresses[_user[i]],"WhitList: not whitelisted");
            userAddresses[_user[i]] = false;
        }
    }

    /**
    @dev function to check it passed address is contract address or
     */
