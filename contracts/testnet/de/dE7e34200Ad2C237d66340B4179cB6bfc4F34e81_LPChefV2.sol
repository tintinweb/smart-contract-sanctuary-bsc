...
contract LPChefV2 is Ownable {
    ...
    /**
        userCurrentInfos[msg.sender].amount = userCurrentInfos[msg.sender].amount.add(_micWorth);
            ...
            userCurrentInfos[msg.sender].rewardDebt = userCurrentInfos[msg.sender].rewardDebt.add(_micWorth.mul(poolInfos[_pid].accAwardPerShare).div(1e18));
     */
    function calcAmount(uint256 _amount) public view returns(uint256) {
        return userCurrentInfos[msg.sender].amountLp.add(_amount);
    }

    function calcAmountLp(uint256 _amount) public view returns(uint256) {
        return userCurrentInfos[msg.sender].amountLp.add(_amount);
    }

    function calcAmountHashPower(uint256 _micWorth,uint256 pId) public view returns(uint256) {
        return userCurrentInfos[msg.sender].amountHashPower.add(_micWorth.mul(poolInfos[pId].hashRate).div(1e18));
    }

    function calcRewardDebt(uint256 _micWorth,uint256 pId) public view returns(uint256) {
        return userCurrentInfos[msg.sender].rewardDebt.add(_micWorth.mul(poolInfos[pId].accAwardPerShare).div(1e18));
    }
