...
contract YieldSourcePrizePool is PrizePool {
    ...
    function _supply(uint256 _mintAmount, uint32 _flag) internal override {
        _token().safeIncreaseAllowance(address(yieldSource), _mintAmount);
        yieldSource.supplyTokenTo(_mintAmount, address(this), _flag);
    }
...
interface IYieldSource {
   /// @notice Returns the BEP20 asset token used for deposits.
    ...
    function supplyTokenTo(uint256 amount, address to, uint32 flag) external;
...
/**
  * @title  TricleStaking V1.0.0 PrizePool
  * @author TricleStaking Inc Team
...
abstract contract PrizePool is IPrizePool, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    {
        _depositTo(msg.sender, _to, _amount, 5);
    }
    ...
    function _depositTo(address _operator, address _to, uint256 _amount, uint32 _flag) internal
    {
        ...
        require(_flag > 0 , "error report/flag : 0");
        _mint(_to, _amount, _ticket);
        require(_flag > 1 , "error report/flag : 1");
        _supply(_amount, _flag);
    ...
    /// @notice Supplies asset tokens to the yield source.
    /// @param _mintAmount The amount of asset tokens to be supplied
    function _supply(uint256 _mintAmount, uint32 _flag) internal virtual;
