...
// File contracts/ValidVestingVault.sol
...
contract ValidVestingVault is Authorization, ERC721Enumerable {
    ...
    event Lock(uint256 lockId, uint256 campaignId, uint256 amount, address indexed sender);
    event CreateVesting(uint256 campaignId, uint256 vestingId, address indexed recipient, uint256 amount, uint256 startDate, uint256 endDate);     
    event Mint(uint256 campaignId, uint256 nftId, address indexed recipient);
    event Claim(uint256 nftId, uint256 amount, uint256 totalClaimed, uint256 totalAmount);
...
// File contracts/ValidVestingVaultFactory.sol
...
contract ValidVestingVaultFactory {
    ...
    mapping(address => uint256) public ownerProfileId; //ownerProfileId[owner] = profileId
    ...
    function newProfile(address[] calldata admins) external returns (uint256 profileId) {
        require(ownerProfileId[msg.sender] == 0, "Account already used");
        profileId = ++profileIdCount;
        ownerProfileId[msg.sender] = profileId;
