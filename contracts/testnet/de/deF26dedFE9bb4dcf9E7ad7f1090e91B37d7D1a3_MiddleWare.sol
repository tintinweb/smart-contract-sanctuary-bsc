...
interface IDoNFT is IComplexDoNFT, IERC721 {
...
contract MiddleWare{
    ...
    struct DoNftMarketInfo{   
        ...
        address owner;               // 9
        address user;                // 10
        address orderPaymentToken;   // 11       
        bool orderIsValid;           // 12
    }

    function getNftOwnerAndUser(address originalNftAddr, uint256 orginalNftId, address doNftAddr) public  view returns (address owner , address user) {
        IBaseDoNFT doNft = IBaseDoNFT(doNftAddr);
        IERC721  oNft =  IERC721(originalNftAddr);
    ...
    function getDoNftMarketInfo(address nftAddr, uint256 nftId, address marketAddr) public view  
        returns (DoNftMarketInfo memory doNftInfo) {

        IDoNFT doNft = IDoNFT(nftAddr);
        IMarket market = IMarket(marketAddr);

        doNftInfo.originalNftAddress = doNft.getOriginalNftAddress();
        doNftInfo.orderFee = uint64(market.getFee()) +  uint64(doNft.getRoyaltyFee());

        if(doNft.exists(nftId)){
            (uint256 oid, , uint64[] memory starts,uint64[] memory ends,) = doNft.getDoNftInfo(nftId);
            ...
            if( doNftInfo.orderIsValid) {
                ...
                doNftInfo.orderPricePerDay = order.pricePerDay;
