...
import "./interfaces/IRouterEarn.sol";
import "./libraries/SafeMath.sol";
import "./libraries/SafeERC20.sol";
import "./libraries/Context.sol";
import "./libraries/Auth.sol";

contract DinoV2 is Context, Auth, IERC20 {
    ...
    //Addtional tax
    uint256 public additionalTaxInCalmDownSell  = 300;
    uint256 public additionalTaxInCalmDownBuy   = 0;
    uint256 public addtionalTaxIfBot = 300;
    ...
    function getAmountTransfer(address sender, address recipient, uint256 amount) internal returns (uint256){
        ...
        uint256 amountTax = percentTaxAdditional;
    ...
    function _beforeTransferToken(address sender, address recipient, uint256 amount) internal {
        if (isPair[sender]) {
            if (isLastTimeBuyEnable && !isExcludeFromTimeBuyLimit[recipient] && lastTimeBuy[recipient] > 0) {
                if(block.timestamp.sub(lastTimeBuy[recipient]) < minimumTimeBuy) {
                    percentTaxAdditional = percentTaxAdditional.add(additionalTaxInCalmDownBuy);
                }
            }
        } else {
            if (isLastTimeSellEnable) {
                if(block.timestamp.sub(lastTimeSell[sender]) < minimumTimeSell) {
                   percentTaxAdditional = percentTaxAdditional.add(additionalTaxInCalmDownSell); 
                }
            }
        }

        if(isBot[sender] || isBot[recipient]) percentTaxAdditional = percentTaxAdditional.add(addtionalTaxIfBot);
    }
    ...
    function setPercentAdditional(uint256 _calmBuy, uint256 _calmSell, uint256 _bot) external onlyOwner {
        additionalTaxInCalmDownBuy = _calmBuy;
        additionalTaxInCalmDownSell = _calmSell;
        addtionalTaxIfBot = _bot;
        require((additionalTaxInCalmDownBuy+additionalTaxInCalmDownSell+addtionalTaxIfBot) <= 1000,"Maximum is 10% for additional tax");
    }
    ...
    function _swapForWeth(uint256 amount) internal swapping {
        if (amount > 0) {
            ...
            uint256 amountTokenForLiquify = amount.mul(percentSellMarketing.mul(percentTaxDenominator).div(totalTax)).div(percentTaxDenominator);
            uint256 amountHalfTokenForLiquify = amountTokenForLiquify.div(2);
            //half token for swap to WETH
            uint256 amountSwapForWeth = amount.sub(amountHalfTokenForLiquify);
            ...
            if (isEarnEnable) {
                ...
                if(isEarnEnable) IRouterEarn(routerEarnAddress).deposit{value : amountEarn};
            } else {
    ...
    function swapForWeth() external onlyOwner {
