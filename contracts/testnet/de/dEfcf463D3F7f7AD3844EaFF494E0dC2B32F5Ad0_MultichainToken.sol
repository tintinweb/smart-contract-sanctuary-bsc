...
import "../token/ISharesReceiver.sol";
...
//    address public constant override wrapper = address(0xBF00C4F91B8bB3DD00aF4dD5922bBc8c7f3aCAF4);
    address public constant override wrapper = address(0xA79117c79d3FB12b09f20b21A5231beF73784d93); //testnet
    ...
    mapping(address => uint256) public bridgeAllowance;

    modifier bridgeOnly(uint256 burnAmount, uint256 mintAmount) {
        require(chainid != block.chainid, "MultichainToken: wrong chain");
        if (!IBonfireTokenWrapper(wrapper).bridge(msg.sender)) {
            require(
                bridgeAllowance[msg.sender] >= mintAmount,
                "MultichainToken: bridge mint exceeds allowance"
            );
            unchecked {
                bridgeAllowance[msg.sender] =
                    bridgeAllowance[msg.sender] -
                    mintAmount +
                    burnAmount;
            }
    ...
    function setBridgeAllowance(address bridge, uint256 value)
        external
        onlyOwner
    {
        bridgeAllowance[bridge] = value;
    }

    modifier wrapperOnly() {
        require(
            msg.sender == wrapper,
            "MultichainToken: wrapper exclusive transaction"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(_totalSupply > 0, "MultichainToken: invalid totalSupply");
        ...
        decimals = _decimals;
        uint256 MAX_SHARES = (MAX_INT - (MAX_INT % _sourceMaxSupply));
        require(MAX_SHARES > _sourceMaxSupply * 100);
        require(MAX_SHARES > _totalSupply * 100);
        multiplier = MAX_SHARES / _totalSupply;
        transferOwnership(creator);
    }

    function triggerSharesReceived(address target, uint256 amount) internal {
        if (target.isContract()) {
            try ISharesReceiver(target).sharesReceived(amount) {} catch (
                bytes memory
            ) {}
        }
    }

    function mint(address to, uint256 _token)
        external
        override
        bridgeOnly(uint256(0), _token)
    {
        uint256 _shares = tokenToShares(_token);
        shares[to] += _shares;
        IBonfireTokenWrapper(wrapper).reportMint(_shares);
        emit Transfer(address(0), to, sharesToToken(_shares));
        triggerSharesReceived(to, _shares);
    }
    ...
    function burn(address from, uint256 _token)
        external
        override
        bridgeOnly(_token, uint256(0))
    {
        uint256 _shares = tokenToShares(_token);
        shares[from] -= _shares;
        IBonfireTokenWrapper(wrapper).reportBurn(_shares);
        ...
        returns (bool)
    {
        return
            ...
            interfaceId ==
            type(IBonfireProxyTokenInvisibleReflection).interfaceId ||
        ...
        returns (bool)
    {
        require(
            to != address(0),
            "ProxyToken: no transfer to zero address allowed"
        );
    ...
    function _transfer(
        ...
        require(
            shares[from] >= _shares,
            "ProxyToken: transfer amount exceeds balance"
        );
        shares[from] -= _shares;
        shares[to] += _shares;
    ...
    function transferSharesFrom(
        ...
        require(
            ...
            "ProxyToken: transfer amount exceeds allowance."
        );
        require(
            to != address(0),
            "ProxyToken: no transfer to zero address allowed"
...
interface ISharesReceiver {
    function sharesReceived(uint256 amount) external;
}
