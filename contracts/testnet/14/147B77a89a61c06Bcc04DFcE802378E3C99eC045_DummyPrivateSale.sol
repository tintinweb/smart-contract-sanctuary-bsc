   ...
   function isOwner(address account) public view returns (bool) {
    ...
    function deposit() public payable    {
      ...
      uint256 price=plans[currentPresaleindex-1].rate;
            ...
            token=price*msg.value.div(1000000000000000000);
    ...
    function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
            ...
            else if(daysv>25 minutes && daysv<30 minutes){
                totalAmount=user.deposits[i].profit.mul(75).div(100);
            }
            else if(daysv>=20  minutes && daysv<25 minutes){
                totalAmount=user.deposits[i].profit.mul(50).div(100);
            }
            else if(daysv>=15  minutes && daysv<20  minutes){
