...
//
// Suuper PROTOCOL COPYRIGHT (C) 2022 
...
contract Suuper is ERC20Detailed, Ownable {
    ...
    uint256 public SuuperInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public burnFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(SuuperInsuranceFundFee).add(
            burnFee
        );
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(SuuperInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(SuuperInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(SuuperInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(SuuperInsuranceFundFee).div(
                treasuryFee.add(SuuperInsuranceFundFee)
            ),
            gas: 30000
        }("");
