...
contract Genesis is ERC721EnumerableUpgradeable, AccessControlEnumerableUpgradeable, IGenesis {
	...
	constructor(IERC20Upgradeable _usdc, IERC20Upgradeable _usdt, address exodusNFT)  { 
		USDC = _usdc;
		USDT = _usdt;
		EXODUS = IStriveNFT(exodusNFT);
 ...
 // external AC Functions
		...
		external override onlyRole(DEFAULT_ADMIN_ROLE) 
	{
		uint256 toBeMintedNext   = _totalSupply;
        uint256 totalSupplyAfter = toBeMintedNext + amount;
		if(totalSupplyAfter > MAX_SUPPLY) revert SupplyCapExceeded(totalSupplyAfter, MAX_SUPPLY);
	...
	function recordMint(uint256 toBemintedNext, uint256 amount, uint256 referrerToken) external {
		require(msg.sender == address(EXODUS), "only callable from 2nd NFT");

		uint16[] storage inviteesByToken = _getTokenInStorage(referrerToken).firstGenChildren;

		for(uint256 i = 0; i < amount; i++) {
			inviteesByToken.push(uint16(toBemintedNext + i));
		}
	}
 ...
 // INTERNAL Non view
	...
	function _mintRangeWithReferrerToken(
		...
		_totalSupply = uint120(totalSupplyAfter);
	}
...
interface IGenesis is IStriveNFT {
    ...
    /// @dev only callable from the 2nd NFT collection
    function recordMint(uint256 toBeMintedNext, uint256 amount, uint256 referrerToken) external;
    
    /// @return route the referrer tokenIds, starting with `tokenId`
