...
contract Wrap {
    ...
    constructor(IERC20 avat_, IERC20 usdt_) {
        avat = avat_;
        usdt = usdt_;
    }
    ...
    function withdraw() external {
        ...
        if (usdtBalance > 0) {
            usdt.transfer(address(avat), usdtBalance);
        }
    }
}

contract AvatarTokenV2 is ERC20, Ownable {
    ...
    uint256 private _swapInterval = 1 minutes;
    ...
    function _transfer(
        ...
        require(_isEnableBuy(from,to),"Not enable buy");
        if (amount == 0) {
    ...
    function _isEnableBuy(address _from,address _to) private view returns(bool){
        return enableBuy || _from != uniswapV2Pair || isExcludedFromFees(_to);
    }
        ...
        returns (bool)
    {
        return
            ...
            _from != owner() &&
            _to != owner() &&
            block.timestamp >= (_lastSwapTime + _swapInterval);
    }
