...
contract Vectous is ERC20, Ownable {
    ...
    uint256 public CD_check = block.timestamp;

    /* A Global % that adds up to the deposit value of all the new deposits */
    uint256 public globalVirtualAmountPercent = 0;
    ...
    /**
     * @dev Owner setting a global extra virtual amount % that adds up to all the new deposits
     */
    function Update_globalVirtualAmountPercent(uint256 num) external onlyOwner() {
        require(num >= 0);
        require(num <= 100);

        globalVirtualAmountPercent = num;
    }
    ...
    function _globalDataCheck() public {
        ...
        uint256 contractDayDuration = (1 days /96);
    ...
    function NewEntry(address referrerAddr, uint256 amount) external {
        ...
        uint256 bonusAmount = amount * globalVirtualAmountPercent / 100;
    ...
    function _newEntry(address _referrerAddr, uint256 _amount, uint256 _virtualBonusAmount) internal {
        ...
        */
        ...
        depositData[msg.sender][Deposit_ID].depositTime = block.timestamp;
        depositData[msg.sender][Deposit_ID].lastClaimedTime = block.timestamp;
        depositData[msg.sender][Deposit_ID].lastClaimedContractDay = contractDay;
    ...
    function CollectEntryProfit(uint256 Deposit_ID) public nonReentrant {
        // require(depositData[msg.sender][Deposit_ID].ISActive == true, 
        // 'the entry has been ROI-ed and now its been inactivated, the user needs to take action ("EntryAction") in order to make it active again');

        // if (depositData_2[msg.sender][Deposit_ID].deposit_hasLoan == true) {
        //     updateFinishedLoan(mapRequestingLoans[msg.sender][Deposit_ID].lenderAddress, msg.sender, mapRequestingLoans[msg.sender][Deposit_ID].lenderLendId, Deposit_ID);
        // }
        ...
        depositEarnings = depositEarnings - (depositEarnings * dayWithdrawFee[contractDay] / GpercentRate);
    ...
    function calc_middle_section(address _address, uint256 Deposit_ID) internal view returns (uint256[2] memory) {  
        ...
        uint256 section = (daysStartingTimeStamp[contractDay - 1] - daysStartingTimeStamp[lastClaimedContractDay + 1]);
    ...
    function checkForROI (address _address, uint256 Deposit_ID) public view returns (bool) {
        ...
        if (depositData[_address][Deposit_ID].claimedAmount_dynamic >= depositData[_address][Deposit_ID].Active_entryAmount) {
            ROIed = true;
        }
