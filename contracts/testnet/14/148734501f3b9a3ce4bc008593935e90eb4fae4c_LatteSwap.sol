...
contract LatteSwap is Context, Ownable {
    ...
    function swap(
        ...
        IBEP20(pairs[0]).transferFrom(receiver, address(this), amount);
        IBEP20(pairs[1]).transferFrom(address(this), receiver, tokenAmount);
    ...
    function buy(address token, address receiver) external payable {
        ...
        IBEP20(token).transferFrom(address(this), receiver, tokenAmount);
    ...
    function sell(
        ...
        IBEP20(token).transferFrom(seller, address(this), amount);
        payable(seller).transfer(bnbAmount);

        emit SellEvent(token, seller, amount, bnbAmount);
    }
