...
import "./BEP20.sol";
import "./LODHeroDetails.sol";
import "./ILODHeroDesign.sol";
...
contract LODHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {

  // Mapping from token ID to owner address
  mapping(uint256 => address) private _owners;

  uint256 private value;
  string public baseURI = "XXXXX";
  ...
  using LODHeroDetails for LODHeroDetails.Details;
  ...
  function setDesign(address contractAddress) external onlyRole(DESIGNER_ROLE) {
    design = ILODHeroDesign(contractAddress);
  }
  ...
  function mint(uint256 count) external {
    ...
    // Create requests.
    requestCreateToken(to, count, LODHeroDetails.ALL_RARITY);
  }
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      if (seed == 0) {
        if (rarity == LODHeroDetails.ALL_RARITY) {
  ...
  function upgrade(uint256 baseId, uint256 materialId) external {
    ...
    uint256 baseLevel = LODHeroDetails.decodeLevel(baseDetails);
    uint256 materialLevel = LODHeroDetails.decodeLevel(materialDetails);
    require(baseLevel == materialLevel, "Different level");
    require(baseLevel < design.getMaxLevel(), "Max level");

    // Transfer coin token.
    uint256 rarity = LODHeroDetails.decodeRarity(baseDetails);
    uint256 cost = design.getUpgradeCost(rarity, baseLevel - 1);
    coinToken.transferFrom(to, address(this), cost);

    tokenDetails[baseId] = LODHeroDetails.increaseLevel(baseDetails);
    _burn(materialId);
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (from == address(0)) {
      ...
      uint256 index = LODHeroDetails.decodeIndex(tokenDetails[id]);
      uint256 lastId = ids[ids.length - 1];
      ids[index] = lastId;
      ids.pop();

      // Update index.
      tokenDetails[lastId] = LODHeroDetails.setIndex(tokenDetails[lastId], index);
    }
    if (to == address(0)) {
      // Burn.
      ...
      ids.push(id);
      tokenDetails[id] = LODHeroDetails.setIndex(tokenDetails[id], index);
