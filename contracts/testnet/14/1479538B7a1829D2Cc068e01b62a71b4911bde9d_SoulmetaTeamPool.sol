...
contract SoulmetaTeamPool is DivestorUpgradeable {
    ...
    function initialize(address usdt_, address soulFrogAddr_) public initializer {
        __Divestor_init();
    ...
    function setMeta(address usdt_, address soulFrogAddr_, bool isOpen_) public onlyOwner {
        cUsdt = IERC20MetadataUpgradeable(usdt_);
        soulFrogAddr = ISoul721(soulFrogAddr_);
    ...
    function getUserBonus() public {
        ...
        uint balance = soulFrogAddr.balanceOf(_msgSender());
        uint get = cUsdt.balanceOf(address(this));
        require(balance > 0, "not nft");
        require(get > 0, "not usdt");

        UserInfo storage userInfo = userInfos[_msgSender()];
        userInfo.cumulative += get;

        cUsdt.transfer(_msgSender(), get);
    }

    // 查询分红
    function searchUserBonus(address player_) view public returns(uint) {
        return soulFrogAddr.balanceOf(player_) > 0 ? cUsdt.balanceOf(address(this)) : 0;
    }
}
