...
contract MetaPots is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

 
    address public Marketing_Wallet =
        0x916c1871efFeC994e06d27a44dd8A028D107Fe9C;
    ...
    string private _name = "MetaPots";
    string private _symbol = "MPOTs";
    ...
    function _transfer(
        ...
        if (takeFee) {
            // 5% of every transfer Marketing
            uint256 MarketingAmount = amount.mul(_marketingShare).div(100);
            // 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(MarketingAmount);
            require(
                amount == sendAmount + MarketingAmount,
                "Marketing value invalid"
            );

            //transfer amount, it will take tax, burn, liquidity fee
            _tokenTransfer(from, Marketing_Wallet, MarketingAmount, false);
