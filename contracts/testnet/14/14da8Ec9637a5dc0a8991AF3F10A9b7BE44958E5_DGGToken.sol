...
//  uint public constant blockInOneMonth = 864000; // 30 * 24 * 60 * 20
  ...
  uint public lastReleaseTreasuryBlock;
  ...
  constructor(address _liquidityPool) TokenAuth(msg.sender, _liquidityPool) {
    // TODO remove
    blockInOneMonth = 200;
  }
  ...
  function startVestingLiquidityPool() onlyOwner external {
    ...
    _mint(liquidityPool, liquidityPoolAllocation * 4 / 100);
  }
  ...
  function startVestingTreasury() onlyOwner external {
    ...
    lastReleaseTreasuryBlock = block.number + blockInOneMonth * 6;
  }
  ...
  function releaseEcosystemAllocation(address _address, uint _amount) external onlyEcosystemContract returns (bool) {
    require(startVestingEcosystemBlock > 0, "VestingEcosystem not started");
  ...
  function releaseLiquidityPoolAllocation() external onlyliquidityPool {
  ...
  function releaseTreasuryAllocation() external onlyTreasuryAddress {
    ...
    require(maxBlockNumber > lastReleaseTreasuryBlock, "Treasury allocation had released");
    uint blockPass;
    if (block.number < maxBlockNumber) {
      blockPass = block.number - lastReleaseTreasuryBlock;
      lastReleaseTreasuryBlock = block.number;
    } else {
      blockPass = maxBlockNumber - lastReleaseTreasuryBlock;
      lastReleaseTreasuryBlock = maxBlockNumber;
    }
    uint releaseAmount = treasuryAllocation * blockPass / (blockInOneMonth * 24);
    _mint(msg.sender, releaseAmount);
  }
