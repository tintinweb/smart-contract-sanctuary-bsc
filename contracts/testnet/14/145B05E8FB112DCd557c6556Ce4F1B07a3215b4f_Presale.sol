...
contract Presale is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256)) public userContributionToken;
    ...
    uint256 public performanceFee = 175; //1.75% of NATIVE raised
    ...
    uint256 public minLiquidityLock = 60 * 60 * 24 * 1; // 1 day
    ...
    function updateRaiseFeeWalletToken(address _raiseFeeWalletForToken) external onlyOwner {
    ...
    function updatePerformanceFeeToken(uint256 _performanceFeeForToken) external onlyOwner {
        ...
        returns (uint256 amount)
    {
        ...
        amount = _hardCap.div(_tier1Rate);
        uint256 _routerAmount = amount
            .mul(_routerRate.add(_defaultRouterRate.add(_liquidityRate)))
            .div(100);
        amount = amount.add(_routerAmount);
        amount = amount.add(amount.mul(performanceFeeForToken).div(10000));
    }
    function tier1Sale(uint256 _saleId) internal {
        bool _outsideContributionAvailable = outsideContribution[msg.sender].length <=outsideContributionCount;
        require((outsideContributionBalance != 0 &&
        ...
        userContributionToken[_saleId][msg.sender] += msg.value.div(rate);
        totalContributionToken[_saleId] += msg.value.div(rate);
    }
    ...
    function addLiquidity(uint256 realAmount, uint256 _saleId, uint256 _tokenFeeAmount) internal {
        address _token = presaleTokens[_saleId];
    ...
    function getPresaleInfo(uint256 _saleId) public view returns(address,uint256[9] memory){
            
            return(presaleTokens[_saleId],
            [minContributeRate[_saleId],
            maxContributeRate[_saleId],
            startTime[_saleId],
            tier1Time[_saleId],
            endTime[_saleId],
            liquidityLockTime[_saleId],
            liquidityRate[_saleId],
            softCap[_saleId],
            hardCap[_saleId]]);
    }
