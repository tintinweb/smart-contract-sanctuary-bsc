...
import "./components/PriceFeed.sol";
...
contract PriceFeed is Initializable, OwnableUpgradeable {
  ...
  function __PriceFeed_init() internal onlyInitializing {
      __PriceFeed_init_unchained();
  }

  function __PriceFeed_init_unchained()
    internal
    onlyInitializing
  {
    __Ownable_init();
...
/*     event whitelistedToken(address indexed tokenaddressToWhitelist);
  ...
  function getbnbusdLatestPrice() public view returns (uint256) {
      (
          /*uint80 roundID*/,
          int price,
          /*uint startedAt*/,
  ...
  function convertBNBUSD(uint _amountInUsd) public view returns (uint) {
    ...
    uint256 BNBUSD = getbnbusdLatestPrice();
