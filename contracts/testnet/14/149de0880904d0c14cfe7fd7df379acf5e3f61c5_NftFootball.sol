...
// File: contracts\NftFootball.sol
...
contract NftFootball is ERC1155PresetMinterPauser{
    ...
    // Events
    event NftCreated(uint256 indexed pieceId, address creator, uint8 _type, uint256 _supplyNumber );
    event MintNFT(uint256 indexed pieceId, address minter, uint256 _amount, address _receiver);
    ...
    function createNft(
        ...
        if (_nftType == nftType.Character) {
            require(_rarity < 4, "CreateNFT: Character rarities should be less than 4.");
            supplyNumber = nftCharacterRarities[_rarity];
        }
        if (_nftType != nftType.Character) {
            require(_supply > 0, "CreateNFT: supply number can not be less than zero.");
            supplyNumber = _supply;
        }

        NftInfo[nonce++] = nftInfo(
            msg.sender,
            _nftType,
            supplyNumber
        );
        ...
        mint(msg.sender, nonce - 1, supplyNumber, "");
    }
    ...
    */
    function mintNFT(
        ...
        require(hasRole(MINTER_ROLE, _msgSender()), "NftFootball: Only minters can Mint NFT");
        nftInfo storage nft = NftInfo[_nftId];
        require(nft.nfttype != nftType.Character, "MintNFT: Character type can not mint");
        mint(msg.sender, _nftId, _mintAmount, "");
    ...
    /**
        ...
        @param _id piece id
     */
    function getNftInfo(uint256 _id) external view returns( address _creator, uint8 _type, uint256 _supplyAmount){
