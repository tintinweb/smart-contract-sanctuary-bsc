...
contract CopyrightRent is Pausable, AccessControl {
    ...
    function setRoyaltyFee(
        ...
        SupportToken memory token = supportedToken[currency];
        require(token.enable, "Currency must be enable");
        require(
            paraArt.copyrightOwner(hashed) == msg.sender,
            "Must be owner of ParaArt"
        );
        Price storage p = price[hashed][currency];
        p.enable = enable;
        p.amount = amount;
        emit SetRoyaltyFee(hashed, currency, enable, amount);
    }
    ...
    function rentByBNB(bytes32 hashed) public payable {
        ...
        emit Rent(msg.sender, hashed, address(0x0), p.amount);
    }
    ...
    function rentByToken(bytes32 hashed, address tokenAddress) public {
        ...
        emit Rent(msg.sender, hashed, tokenAddress, p.amount);
    }
