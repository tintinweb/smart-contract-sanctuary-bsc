...
contract Marketplace is IMarketplace, Ownable, ERC1155Holder {
    ...
    mapping(uint256 => address) public referralCodeUser;
    mapping(address => uint256) public userReferralCode;
    ...
    function buy(
        ...
        require(
            ERC20(currency).transferFrom(msg.sender, address(this), totalValueUsdWithDecimal),
            "MARKETPLACE: FAILED IN TRANSFERING CURRENCY TO MARKETPLACE"
        );
        // Transfer nft from marketplace to buyer
        bytes memory data = abi.encode(msg.sender);
        try AnotherNFT(nft).safeTransferFrom(address(this), msg.sender, _nftId, _amount, data) {
    ...
    function sell(
        ...
        require(priceEachUsd > 0, "MARKETPLACE: WRONG NFT ID TO SELL");
        uint8 decimalCurrency = ERC20(currency).decimals();
        ...
        uint256 totalValueUsdWithDecimal = (priceEachUsd - priceDiscount) *
            ...
            (10 ** decimalCurrency);
        require(
            ERC20(currency).balanceOf(address(this)) >= totalValueUsdWithDecimal,
            "MARKETPLACE: NOT ENOUGH CURRENCY BALANCE TO PAY"
        );
        // Transfer nft from seller to marketplace
        bytes memory data = abi.encode(msg.sender);
        try AnotherNFT(nft).safeTransferFrom(msg.sender, address(this), _nftId, _amount, data) {
            require(
                ERC20(currency).transfer(msg.sender, totalValueUsdWithDecimal),
     ...
     */
    function payReferralCommissions(address payable _receiver, uint256 _amountUsd) internal {
        ...
        require(
            ERC20(token).transfer(_receiver, commissionAmountInToken),
            "MARKETPLACE: UNABLE TO TRANSFER COMMISSION PAYMENT TO RECIPIENT"
        );
    }
    ...
    function withdrawToken(uint256 _amount) public override onlyOwner {
        ...
        require(
            ...
            "MARKETPLACE: BALANCE NOT ENOUGH"
        );
        require(
