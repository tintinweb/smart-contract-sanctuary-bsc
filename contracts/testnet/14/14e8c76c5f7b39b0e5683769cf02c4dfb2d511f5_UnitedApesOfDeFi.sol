...
contract UnitedApesOfDeFi is ERC20Detailed, Ownable {
    ...
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(insuranceFee).add(burnFee).add(USDCRewardsFee);
    ...
    uint256 private constant MAX_SUPPLY = 50 * 10**10 * 10**DECIMALS;
    ...
    function setNormalFees() external onlyOwner {
        ...
        sellFee = treasuryFee;
        totalFee = liquidityFee.add(treasuryFee).add(insuranceFee).add(burnFee).add(USDCRewardsFee);
