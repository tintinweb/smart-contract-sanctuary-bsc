...
contract CronUpkeep is KeeperCompatibleInterface, KeeperBase, ConfirmedOwner, Pausable {
     ...
     */
    function _checkUpkeep() internal view whenNotPaused returns (bool, bytes memory) {
   ...
   */
     ...
     * @return isInRange signals if upkeep is needed, performData is an abi encoding
     * of the id and "next tick" of the eligible cron job
     */
    function checkInRange(uint256 start, uint256 end) private view returns (bool isInRange, bytes memory handler) {
        uint256 id;
        uint256 lastTick;
