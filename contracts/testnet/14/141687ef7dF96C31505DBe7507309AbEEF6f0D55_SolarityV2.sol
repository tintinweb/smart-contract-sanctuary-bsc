...
contract Solarity is Initializable, ERC20Upgradeable, UUPSUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  */
  ...
  {
    // SHEPARD: Validates that safe guards for maxWalletPercent, greater than 0 and less than the allocation, are being met.
    require(maxWalletPercent > 0 && maxWalletPercent <= 1000, "The parameter 'maxWalletPercent' must be greater than 0 and must not exceed 1000.");
    // SHEPARD: Validate Multiply Values for both Buy and Sell Maxes.
    require(maxBuyPercent > 0 && maxBuyPercent <= 1000, "The parameter 'maxBuyPercent' must be greater than 0 and must not exceed 1000.");
    require(maxSellPercent > 0 && maxSellPercent <= 1000, "The parameter 'maxSellPercent' must be greater than 0 and must not exceed 1000.");

    // SHEPARD: Ensure that you cannot buy or sell more than a single wallet can hold.
    require(maxBuyPercent <= maxWalletPercent, "The parameter 'maxBuyPercent' must not exceed 'maxWalletPercent'.");
    require(maxSellPercent <= maxWalletPercent, "The parameter 'maxSellPercent' must not exceed 'maxWalletPercent'.");
  ...
  */
  ...
  {
    require(liquidityWalletAddress != newWalletAddress , "The new Liquidity wallet address is the same as the current Liquidity wallet address.");
  ...
  */
  ...
  {
    require(marketingWalletAddress != newWalletAddress , "The new Marketing wallet address is the same as the current Marketing wallet address.");
  ...
  */
  ...
  {
    require(maxAllocation > 0 && maxAllocation * 10**_decimals > totalSupply(), "The specified maxAllocation must be greater than zero (0) and also greater than the number of tokens already issued.");
  ...
  */
  ...
  {
    require(operationsWalletAddress != newWalletAddress , "The new Operations wallet address is the same as the current Operations wallet address.");
    // SHEPARD: Immediately deny the old wallet and add the new wallet to all bypass lists. 
  ...
  */
  function _canSenderTransfer(address from, address to, uint256 amount, TransactionType transactionType) internal view
  {
    ...
    if (_internalList[from] || _internalList[to] || to == address(0) || to == address(0xdead))
    {
      return;
    }
  ...
  */
  function _canTransfer(address from, address to, uint256 amount) internal view
  {
    ...
    // MASTERJEDI: The Router and therefore the Pair must be present to allow transfers to occurr.
    require(uniswapV2Pair != address(0) && address(uniswapV2Router) != address(0), "The Liquidity Pool Router and Pair must be set in order to complete transactions.");
