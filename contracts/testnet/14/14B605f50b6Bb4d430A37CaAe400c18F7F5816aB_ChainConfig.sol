...
contract ChainConfig is InjectorContextHolder, IChainConfig {
    ...
    event SupportedTokenChanged(address prevValue, address newValue);
    event RewardTokenChanged(address prevValue, address newValue);
    ...
    function initialize(
        ...
        _consensusParams.supportedToken = supportedToken;
        emit SupportedTokenChanged(address(0), supportedToken);
        _consensusParams.rewardToken = rewardToken;
        emit RewardTokenChanged(address(0), rewardToken);
    }
    ...
    function getSupportedToken() external view returns (address) {
        return _consensusParams.supportedToken;
    }

    function setSupportedToken(address newValue) external override onlyFromGovernance {
        address prevValue = _consensusParams.supportedToken;
        _consensusParams.supportedToken = newValue;
        emit SupportedTokenChanged(prevValue, newValue);
    }

    function getRewardToken() external view returns (address) {
        return _consensusParams.rewardToken;
    }

    function setRewardToken(address newValue) external override onlyFromGovernance {
        address prevValue = _consensusParams.rewardToken;
        _consensusParams.rewardToken = newValue;
        emit RewardTokenChanged(prevValue, newValue);
    }
}
