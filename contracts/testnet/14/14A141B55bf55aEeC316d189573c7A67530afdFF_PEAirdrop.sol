...
contract PEAirdrop is ReentrancyGuard, Ownable, Pausable {
    address public AIRDROP_TOKEN = 0xDd088A66Afe9754d569989903d77F7D86e576D0E;

    uint256 public CLAIMED_AIRDROP_BEGIN_AT;
    uint256 public CLAIMED_AIRDROP_END_AT;
    ...
    event ClaimAirdrop(uint256 indexed claimType, address indexed receiver, uint256 indexed amount);
    ...
    function claimAirdrop(uint256 _amount, uint256 _voucher)  external nonReentrant {
        ...
        uint256 remainAmount = user.value - user.receivedVoucher * 5 * 10 ** 18 + user.receivedAmount;
        require(_amount + _voucher * 5 * 10 ** 18 <= remainAmount, "Claim airdrop exceeds allowance");
    ...
    function getUserReceivedVoucher(address userAddress) external view returns (uint256) {
        UserInfo storage user = userInfo[userAddress];
        return user.receivedVoucher;
    }
