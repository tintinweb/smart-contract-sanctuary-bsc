...
contract StakePoolTokenForBNB is Ownable {
    using SafeMath for uint256;
    ...
    function balanc() public view returns (uint256){
...
contract FactoryStakingPool{
    ...
    function createTokenForBNB(address ownerAddress, IBEP20 _stakingToken, uint256 _lockPeriod, uint256 _maxReward, bool _withUpdate, uint256 rewardPerBlock) public returns (address)
    {
        StakePoolTokenForBNB toDeploy = new StakePoolTokenForBNB(_stakingToken, rewardPerBlock, block.number, _lockPeriod);
        toDeploy.set(_maxReward, _withUpdate, rewardPerBlock);
