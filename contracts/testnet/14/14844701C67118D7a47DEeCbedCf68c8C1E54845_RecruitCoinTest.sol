...
contract RecruitCoinTest is ERC20, Ownable, ReentrancyGuard {
  ...
  function _deposit(
    ...
    if (getCurrentTime() >= dates[0] && getCurrentTime() < dates[1] || preSale) {
      ...
      _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(5),ONE_MINUTE.mul(5), ONE_MINUTE.mul(5), false, purchasedTokens.mul(1e18));
    }
    /*PUBLIC SALE*/
      ...
      if (getBonusWeek() <= 2 || publicSale3M) {
        _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(5),1, 1, false, purchasedTokens.mul(1e18));
      }else{
        _createVestingSchedule(depositor, getCurrentTime(), ONE_MINUTE.mul(10),1, 1, false, purchasedTokens.mul(1e18));
      }
    } else {
