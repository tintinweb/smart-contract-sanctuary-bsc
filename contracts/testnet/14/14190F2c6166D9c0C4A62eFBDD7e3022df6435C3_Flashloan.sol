...
contract Flashloan {
  ...
  function pancakeCall(
    ...
    if (address(token) == BRT && address(otherToken) == UST) {
        address[] memory longPath;
        longPath = new address[](3);
        longPath[0] = BRT;
        longPath[1] = BR2;
        longPath[2] = UST;
        ...
        amountReceived = amounts[2];

    } 

    otherToken.transferFrom(tx.origin, msg.sender, amountRequired);
    if (amountReceived - amountRequired > 0)
