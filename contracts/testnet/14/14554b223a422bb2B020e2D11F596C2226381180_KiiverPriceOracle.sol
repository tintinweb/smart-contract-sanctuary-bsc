...
import './interfaces/IKiiverPriceOracle.sol';
...
contract KiiverPriceOracle is IKiiverPriceOracle, Ownable {
    ...
    */
    function update() external override {
        require(tryUpdate(), 'KiiverPriceOracle: PERIOD_NOT_ELAPSED');
    }
    ...
    */
    function consult(address token, uint amountIn) external view override returns (uint amountOut) {
        if (token == token0) {
            ...
            require(token == token1, 'KiiverPriceOracle: INVALID_TOKEN');
    ...
    */
    function changePeriod(uint256 _period) public onlyOwner {
        require(_period > 0, 'KiiverPriceOracle: INVALID_PERIOD');
        period = _period;
        ...
        require(reserve0 != 0 && reserve1 != 0, 'KiiverPriceOracle: NO_RESERVES'); // ensure that there's liquidity in the pair
