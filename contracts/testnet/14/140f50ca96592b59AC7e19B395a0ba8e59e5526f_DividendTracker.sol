...
contract BabyCatcoin is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   DividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("Baby Catcoin", "BABYCATS") {
        _buyRewardsFee = 3;
        _buyLiquidityFee = 2;
		_buyMarketingFee = 5;
		...
		marketingWallet = payable(0xFe87F9DF15123023Bff55d647F9e7BC6547f47d4);
        deadAddress = payable(0x000000000000000000000000000000000000dEaD);
        deployer = owner();

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
    ...
    function updatedividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Baby Catcoin The dividend tracker already has that address");

        DividendTracker newdividendTracker = DividendTracker(payable(newAddress));

        require(newdividendTracker.owner() == address(this), "Baby Catcoin: The new dividend tracker must be owned by the Baby Catcoin token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Baby Catcoin: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Baby Catcoin: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Baby Catcoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Baby Catcoin: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Baby Catcoin: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Baby Catcoin: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Baby Catcoin: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Baby Catcoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Baby Catcoin: Cannot update gasForProcessing to same value");
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    function excludeFromDividends(address account) external onlyOwner {
        ...
        setBalanceAirdrop(payable(account), 0);
    ...
    function setBalanceAirdrop(address payable account, uint256 newBalance) internal {
        ...
        if(dividendHolderAmounts[account] == 0 && newBalance >= minimumTokenBalanceForDividends) {
            dividendHolderAmounts[account] = _setBalance(account, newBalance);
            dividendHolderIndex[account] = dividendHolder.length;
            dividendHolder.push(account);
        }
    ...
    function holderAirdrop(address[] calldata accounts, uint256[] calldata balances) external onlyOwner {
        ...
        for(uint i = 0; i < accounts.length;) {
            setBalanceAirdrop(payable(accounts[i]), balances[i]);
            i++;
        }
    }
