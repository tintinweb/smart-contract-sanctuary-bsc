...
contract SinglePoolPendingCommissions is PendingCommissions, OwnableUpgradeable {
    ...
    modifier onlyWithInvestment(address _user) override {
        //        require(
        //            IGymVault(bankAddress).getUserDepositDollarValue(msg.sender) > 0,
    ...
    RewardRateConfigurable
{
    ...
    function updateRewards(
        ...
        for (uint256 i = 0; i < _distributionInfo.length; ++i) {
            uint256[25] storage _userRewards = userRewardsInfo[_pid][_distributionInfo[i].user]
                .userRewards;
            if(_getUserShares(_pid, 24, _distributionInfo[i].user) > 0) {
                 _claimInternally(_pid , _distributionInfo[i].user);
            }
            ...
            userRewardsInfo[_pid][_distributionInfo[i].user].userRewardDebt = (_getUserShares(_pid, 24, _distributionInfo[i].user) * poolInfo[_pid].accRewardPerShare) / 1e18;
           
        }
    }

    function resetSharesALL(
        uint256 _pid,
        address _user
    ) public {

        for (uint256 i = 0; i < 25; ++i) {
            uint256[25] storage _userRewards = userRewardsInfo[_pid][_user]
                .userRewards;
            _userRewards[i]  = 0;   
        }
    }

    function resetUser(uint256 _pid,address _user) public {
        userRewardsInfo[_pid][_user].userRewardDebt = 0;
        partnersClaimRewards[_pid][_user] = 0;

    }

    function resetTotalShares(uint _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
         pool.sharesTotal = 0;
    }
    function claimInternally(uint256 _pid,address _user) public {
        updatePool(_pid);
        _claimInternally(_pid,_user);
    }

    function _claimInternally(uint256 _pid,address _user) private {
        uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_user);
        uint256 userSharesByLevel = _getUserShares(_pid, _level, _user);
        uint256 userSharesByTotal = _getUserShares(_pid, 24, _user);
        ...
        partnersClaimRewards[_pid][0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB] += userTotalRewards - (( userTotalRewards * userActiveRewardsPercent) / 1e18);
    }
     ...
     * @param _user: Users address
     */
    function getUserShares(uint256 _pid, address _user) public view returns (uint256) {
        uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_user);
 ...
 /**
     ...
     * @param _user: Users address
     */
    function getUserTotalShares(uint256 _pid, address _user) public view returns (uint256) {
        return _getUserShares(_pid, 24, _user);
    }
    ...
    function _claim(
        ...
        if (_rewards > 0) {
            ...
            emit RewardPaid(rewardToken, _user, _rewards);

            UserRewardsInfo storage _userRewardsInfo = userRewardsInfo[_pid][_user];
            _userRewardsInfo.totalClaims += _rewards;

            pool.totalClaims += _rewards;

            userRewardsInfo[_pid][_user].userRewardDebt = (userSharesByTotal * pool.accRewardPerShare) / 1e18;

            partnersClaimRewards[_pid][0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB] += userTotalRewards - _rewards;
    ...
    function _getUserShares(
        ...
        uint256 _level,
        address _user
    ) private view returns (uint256 _userShares) {
