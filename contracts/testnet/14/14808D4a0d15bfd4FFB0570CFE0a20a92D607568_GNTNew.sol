...
contract GNTNew is Ownable{
    ...
    uint256 private constant timeskip = 2 minutes;    //1 days default
    ...
    function depositBySplit(uint256 _packageID) private {
        ...
        require(userInfo[msg.sender].split >= packagePrice[_packageID] / 2, "Invalid Split Package");
        ...
        for(uint256 i = referDepth; i > 0; i--){
            ...
            if(user_up.teamTotalDep >= 50000e18 && user_up.teamCount >= 25 && teamUsers[upline][2].length != 0){
                user_up.level = 1;
                reward = (packagePrice[_packageID] * i * 100) / baseDivider;
            }
            if(user_up.teamTotalDep >= 100000e18 && user_up.teamCount >= 50 && teamUsers[upline][2].length != 0){
                user_up.level = 2;
    ...
    function deposit(uint256 _packageID) public {
        ...
        if(order.start != 0){
            user.withdrawable += packagePrice[order.packid] + ((packagePrice[order.packid] * cyclePercent * 7000) / (baseDivider*baseDivider));
            user.split += (packagePrice[order.packid] * cyclePercent * 3000) / (baseDivider*baseDivider);
            teamDep = 0;
        }
        ...
        if(order.packid < _packageID){
            teamDep = packagePrice[_packageID] - packagePrice[order.packid];
            order.packid = _packageID;
        }
