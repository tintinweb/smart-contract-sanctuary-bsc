...
contract BStaking {
	...
	function stakeInfo(address addr) external view returns (uint112 totalReturn, uint112 totalClaimed, uint112 cps) {
		User memory user = users[addr];
		...
		for(uint256 i = 0; i < length; ++i) {
			...
			if(block.timestamp < stake.startDate + STAKE_LENGTH) {
				cps += stake.amount / (PERCENT_TOTAL / PERCENT_PERDAY) / 24 / 60 / 60;
			}
			if(user.lastClaim >= stake.startDate + STAKE_LENGTH) {
				totalClaimed += stake.amount;
			}
			else {
				totalClaimed += stake.amount * (user.lastClaim - stake.startDate) / STAKE_LENGTH;
			}
		}
	}
