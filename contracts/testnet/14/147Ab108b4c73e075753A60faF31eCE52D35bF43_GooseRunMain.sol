...
contract GooseRunMain is ERC721Upgradeable {
    Beans be = Beans(0x6CE1fb5DF40245f6fF7EB0bea2eD9763d0baC96B);
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            be.transferFrom(be.ownerTokenCfo(), toAddress, 10000000 * be.unit());  
        }
        emit Mint(tokenIds);
    }
    ...
    function airdropBeans(uint256[] memory _tokenId, uint256 beans) external {
        require(msg.sender == _owner, "Only owner");
        for (uint i = 0; i < _tokenId.length; i++) {
            be.transferFrom(be.ownerTokenCfo(), ownerOf(_tokenId[i]), beans * be.unit());  
        }
    }
}