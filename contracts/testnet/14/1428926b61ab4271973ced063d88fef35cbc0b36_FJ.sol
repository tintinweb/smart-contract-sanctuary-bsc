...
contract FJ is Context, IERC20, Ownable {
    ...
    address payable public marketingWallet = payable(0x68cc005fd1A492Cd1E8547938aCB8eAFbF092977);  
    address payable public lpWallet = payable(0x11F7db3A5ECe53FF3B086Aaa89E75eC37196891a);         
    address payable public invitationWallet = payable(0xeA7404E392774Ad8ad922500B261D993cd09d7B5); 
    address payable public fundWallet = payable(0xC32eC3C571327Fde605F493e2E2BF167E1227e26);       
    address payable public repoWallet = payable(0x8E2Ca700CB263dABc555CF48bf18C2aEbc92D939);       
    address payable public devWallet = payable(0xE67b33718f805aC649040412d2b9059Ea5477a5B);        
    ...
    uint256 public marketingTrigger = 300000000 * 10**18;                  
    uint256 public repoTrigger = 3 * 10**18;                               
    uint256 public repoAccumulator;                                        
    uint256 public lpTrigger = 100000000 * 10**18;                         
    ...
    uint256 public txOpenAt;                                                
    uint256 public banInterval;                                             
    ...
    function setLpTrigger(uint256 newValue) external onlyOwner {
        lpTrigger = newValue;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require((pairs[sender] || pairs[recipient]) && (txSwitch || noTxLimit[sender] || noTxLimit[recipient]), "ERC20: transaction not permit");
        
        if (sender == uniswapPair && recipient != owner() && !isTxOpen && !noTxLimit[recipient]) {
            if (txOpenAt == 0) {
                txOpenAt = block.timestamp + banInterval;
            }
            if (txOpenAt > block.timestamp) {
        ...
        if(pairs[sender] && !noFee[sender] && !noFee[recipient]) {
            ...
            _balances[marketingWallet] += amount.mul(buyMarketingFee).div(100);
            emit Transfer(sender, marketingWallet, amount.mul(buyMarketingFee).div(100));
            
            _balances[lpWallet] += amount.mul(buyLpFee).div(100);
            
            emit Transfer(sender, lpWallet, amount.mul(buyLpFee).div(100));
            
            uint256 forOne = amount.mul(buyInviteOneFee).div(100);
            _balances[invitationWallet] += forOne;
            emit Transfer(sender, invitationWallet, forOne);
            uint256 forTwo = amount.mul(buyInviteTwoFee).div(100);
            _balances[invitationWallet] += forTwo;
            emit Transfer(sender, invitationWallet, forTwo);
            
            address superior = superiors[recipient];
            if(superior != address(0)){
                _allowances[invitationWallet][superior] += forOne;
                superior = superiors[superior];
            }
            if(superior != address(0)){
                _allowances[invitationWallet][superior] += forTwo;
            }
        
        }else if(pairs[recipient] && !noFee[sender] && !noFee[recipient]) {
            
            feeEi = amount.mul(sellFee).div(100);
            uint256 feeToBnb = sellFee.sub(sellBurnFee);
            fee = feeEi.mul(feeToBnb).div(100);
            _balances[address(this)] += fee;                                                                     
            
            uint256 bnbBefore = address(this).balance;
            swapTokensForEth(fee);                                                                               
            fee = address(this).balance.sub(bnbBefore).div(feeToBnb); 

            _balances[deadAddress] += amount.mul(sellBurnFee).div(100);
            emit Transfer(sender, deadAddress, amount.mul(sellBurnFee).div(100));
            
            payable(marketingWallet).transfer(fee.mul(sellMarketingFee));
            payable(fundWallet).transfer(fee.mul(sellFundFee));
            payable(devWallet).transfer(fee.mul(sellDevFee));
            ...
            if(_balances[marketingWallet] >= marketingTrigger){
                tokenToEth(marketingWallet);
            }
            
            if(_balances[lpWallet] >= lpTrigger){
                uint256 token = _balances[lpWallet];
                _balances[address(this)] += token;
                _balances[lpWallet] = 0;
                uint256 ethBefore = address(this).balance;
                swapTokensForEth(token);
                lpAccumulator += address(this).balance.sub(ethBefore);
                payable(lpWallet).transfer(address(this).balance.sub(ethBefore));
            }
        }
    ...
    function withdrawInvitation(uint256 amount) public returns (bool) {
        require(_allowances[invitationWallet][_msgSender()] >= amount, "ERC20: withdraw amount exceeds allowance");

        _allowances[invitationWallet][_msgSender()] -= amount;
        _balances[invitationWallet] -= amount;
        _balances[address(this)] += amount;
        uint256 bnbBefore = address(this).balance;
        swapTokensForEth(amount);
        uint256 bnbDiff = address(this).balance.sub(bnbBefore);
        payable(_msgSender()).transfer(bnbDiff);
