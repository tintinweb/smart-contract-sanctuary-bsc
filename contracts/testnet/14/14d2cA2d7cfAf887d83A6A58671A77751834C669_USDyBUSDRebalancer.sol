...
contract USDyBUSDRebalancer is Initializable, ReentrancyGuard, AccessControl {
    ...
    bool public isEnabled = true; // enable or disable the rebalancer
    uint256 public mode = 3; // enable or disable peg directions
    uint256 public impactPercentageLimit = 10; // liquidity impact percentage limit
    ...
    */
    ...
    function rebalance() external _isAdminOrOperator returns (bool) {
        ...
        require(
            block.timestamp - lastTimeStamp >= epoch,
            "USDyBUSDRebalancer: insufficient time passed"
        );
        ...
        if (r1 < r2 && (mode == 2 || mode == 3)) {
            ...
            uint256 x = (sqrt(r1) * sqrt(r2)) - r1; // x is the amount of USDy purchased
            ...
            uint256 x = (sqrt(r2) * sqrt(r1)) - r2; // x is the amount of USDy purchased
            uint256 ratio = r1 / r2;
            x *= sqrt(ratio);
            ...
            if (checkLiquidityImpact(inputAmount, r1)) {
                ...
                IERC20(lp).safeApprove(address(lProvider), amountOfLp);
