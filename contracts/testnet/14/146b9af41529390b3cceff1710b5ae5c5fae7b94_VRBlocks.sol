...
contract VRBlocks is Owned {
    ...
    uint256 public multiplier1 = 1; // 0.25% daily
    uint256 public divider1 = 400;
    ...
    function withdrawReward1() public{   
       uint256 rewards = IntervalRewardsOf1(msg.sender);
        ...
        if(user[msg.sender].rewardsWithdrawn1 > (3*user[msg.sender].stakedBal1)){
            rewards = 3*user[msg.sender].stakedBal1 - (user[msg.sender].rewardsWithdrawn1-rewards);
            user[msg.sender].rewardsWithdrawn1 = 3*user[msg.sender].stakedBal1;
        }
