...
//YEZFIRE: Anti-snipe - 80% tax on selling within 1 hour of purchase
...
contract YEZ is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(from == uniswapV2PairAddr)
        {
            ...
            SELLcooldownTimer[from] = block.timestamp + cooldownTimerInterval;        
        }
        ...
        if(to == uniswapV2PairAddr)
        {
            ...
            SELLcooldownTimer[from] < block.timestamp) 
            {
                _taxFee = 200;
                _liquidityFee = 200;
                _marketingFee = 200;
