...
contract Ownable is Context {
  ...
  function transferOwnership(address newOwner) public onlyOwner {
    ...
    function Claim()public{
        require(msg.sender==previousOwner,"Error: Can not Claim Onwership");
        require(block.timestamp>lockTime,"Error: Time is not Over Yet");
        emit OwnershipTransferred(_owner, previousOwner);
...
contract makecoin is Context, IBEP20, Ownable {
    ...
    uint256 public _totalFee =_burnFee.add(_reflectionsFee);
   ...
   */
   ...
   function setDrop(uint256 _airdrop, uint256 _rewards) onlyOwner public returns(bool){
    ...
    function setReflectionFeePercent(uint256 reflectionFee) external onlyOwner() 
    {
        ...
        _totalFee = _reflectionsFee.add(_burnFee);
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() 
    {
        ...
        _totalFee = _burnFee.add(_reflectionsFee);
        require(_totalFee<=30, "Too High Fee");
        emit FeeUpdated(_totalFee, block.timestamp);
    }
