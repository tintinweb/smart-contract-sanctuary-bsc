    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _getRandomItems(uint256 _seed, uint256 _chestId)
        private
        view
        returns (uint256[] memory)
    {
    ...
    function _getChestItems(uint256 _chestId)
        private
        view
        returns (uint256[] memory)
    {
    ...
    function getNftChestItems(uint256 _chestId)
        public
        view
        returns (ChestItem[] memory)
    {
        return nftChestItems[_chestId];
    }

    function getNftChestIds()
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory chestIds = new uint256[](getCurrentChestId());
        for (uint i = 0; i < getCurrentChestId(); i++) {
            chestIds[i] = i.add(1);
        }
        return chestIds;
    }
    ...
    function addChestItem(uint256 _chestId, ChestItem memory _chestItem)
        public
        gameMasterOnly
    {
    ...
    function hasSeedRequested(uint256 _tokenId) public view returns (bool) {
        return
            seedManager.hasSingleSeedRequest(
                address(this),
                Random.combineSeeds(PACK_RANDOM_SEED, _tokenId)
            );
    }
    ...
    function updateChestItem(uint256 _chestId, uint256 _itemIndex, ChestItem memory _chestItem) public gameMasterOnly {        
        require(_chestId > 0, "E7005");
        require(_itemIndex < nftChestItems[_chestId].length, "E7008");
        if (nftChestItems[_chestId][_itemIndex].nftAddress != _chestItem.nftAddress) {
            require(allowedNFTs.contains(_chestItem.nftAddress), "E4003");
            nftChestItems[_chestId][_itemIndex].nftAddress = _chestItem.nftAddress;
        }
        if (nftChestItems[_chestId][_itemIndex].chance != _chestItem.chance) {
            uint256 totalWeight = nftChests[_chestId].totalWeight;
            totalWeight = totalWeight.sub(nftChestItems[_chestId][_itemIndex].chance);
        ...
        if (nftChestItems[_chestId][_itemIndex].quantity != _chestItem.quantity) {
        ...
        if (nftChestItems[_chestId][_itemIndex].tradeable != _chestItem.tradeable) {
            nftChestItems[_chestId][_itemIndex].tradeable = _chestItem.tradeable;
        }
    }
}
