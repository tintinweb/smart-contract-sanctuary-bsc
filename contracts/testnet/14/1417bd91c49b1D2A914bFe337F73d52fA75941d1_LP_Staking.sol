...
contract LP_Staking {
    ...
    uint256 public BASE_BONUS_REWARD = 8000 * 1e18; //基础每期产出
    ...
    function initToken(address _token, uint8 _decmails) public onlyOwner {
        ...
        BASE_BONUS_REWARD = 8000 * baseToken;
    }
    ...
    function bonusUsdt() private {
        uint256 allUsdtReward = IERC20(usdtAddress).balanceOf(token);
        pendingUsdt = 0;
        if (allUsdtReward >= BASE_BONUS_USDT) {
            uint256 balanceBefore = IERC20(usdtAddress).balanceOf(
                address(this)
            ); //兼容有手续费的代币
            TransferHelper.safeTransferFrom(
                ...
                token,
                address(this),
                IERC20(usdtAddress).balanceOf(token)
            );
