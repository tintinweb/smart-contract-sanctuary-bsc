...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    constructor(string memory _name, string memory _symbol, address _routerAddress, address _receiveTokenAddress)  ERC20(_name, _symbol) {
    ...
    function distributeDividends() public override payable {
        ...
        if (msg.value > 0) {
            uint256 beforeBalance = IERC20(receiveTokenAddress).balanceOf(address(this));
            swapETHForTokens(msg.value);
            uint256 swapBalance = IERC20(receiveTokenAddress).balanceOf(address(this)).sub(beforeBalance);
    ...
    // generate the uniswap pair path of token -> weth
      ...
      path[1] = address(receiveTokenAddress);
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(receiveTokenAddress).transfer(user, _withdrawableDividend);
...
contract HoneyPadUsdt is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _supply, uint256[3] memory _buyFees, uint256[3] memory _sellFees, address[4] memory _addresses, uint256 _minimumTokenBalanceForDividends) ERC20(_name, _symbol) {
        buyback = _buyFees[0];
        marketingFee = _buyFees[1];
        dividendFee = _buyFees[2];

        sellBuybackFee = _sellFees[0];
        sellMarketingFee = _sellFees[1];
        sellDividendFee = _sellFees[2];

        marketingWallet = _addresses[0];
        buybackWallet = _addresses[1];

        dividendTracker = new CoinDividendTracker(_minimumTokenBalanceForDividends, _addresses[2],_addresses[3]);

        updatePancakeRouter(_addresses[2]);
...
contract CoinDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(uint256 _minimumTokenBalanceForDividends, address _routerAddress, address _receiveTokenAddress) DividendPayingToken("CoinDividendTracker", "Coin_DividendTracker", _routerAddress, _receiveTokenAddress) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividends * (10**decimals()); //must hold 10000+ tokens
    }
