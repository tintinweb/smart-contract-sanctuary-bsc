...
contract iFARMS01 is Context, IBEP20, Ownable {
    ...
    address private _testnetRouterAddress =
        0x25CB5E534Ab309874B8c2a2318cCF9906A815274;

    address private _walletAddressMarketing =
        0xC23bB39E89a7AC332f38C0A609acEE901D14A650;
    address private _burnWalletAddress =
        0x000000000000000000000000000000000000dEaD;

    uint8 private _decimals = 9; // 0.000000000000000000
    uint256 private _zeroes = 10**_decimals;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1 * 10**9 * _zeroes;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Infinity Farms";
    string private _symbol = "iFARMS01";
    ...
    uint256 public _maxTxAmount = 25000000000000 * _zeroes;
    uint256 private numTokensSellToAddToLiquidity = 500000000 * _zeroes;
    uint256 private numTokensBurn = 999900000 * _zeroes;
    ...
    function _takeMarketing(uint256 tMarketing) private {
        uint256 currentRate = _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
        _rOwned[_walletAddressMarketing] = _rOwned[_walletAddressMarketing].add(
            rMarketing
        );
        if (_isExcluded[_walletAddressMarketing])
            _tOwned[_walletAddressMarketing] = _tOwned[_walletAddressMarketing]
                .add(tMarketing);
    }
    ...
    function _setRouterAddress(address router) private {
        IPancakeSwapV2Router02 _newPancakeRouter = IPancakeSwapV2Router02(
            router
        );
    ...
    /**
     * @dev if someone (accidentally) sends BNB/BNB directly to the contract.
     *
     */
    function clearBNB(address payable recipient) external onlyOwner {
        uint256 amountBNB = address(this).balance;
        require(
            recipient != address(0),
            "Cannot withdraw the BNB balance to the zero address"
        );
        require(amountBNB > 0, "The BNB balance must be greater than 0");
...
contract INFINITYFARMS is iFARMS01 {
    constructor() iFARMS01(Env.MainnetV2) {
