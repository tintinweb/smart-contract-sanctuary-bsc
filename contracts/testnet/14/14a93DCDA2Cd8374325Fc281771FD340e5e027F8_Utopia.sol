...
contract Utopia is  Ownable {
    ...
    constructor(address utopiaToken_, address utopiaNFT_, address utopiaInsurance_, address utopiaRalationship_, address operatingFund_) {
    ...
    function setRecommender(address recommender) public {
        ...
        require(getRecommender(msg.sender) == address(0), "[Utopia] Referrer has been bound!");
    ...
    function statisticsReward() public onlyDerivation{
        uint256 totalNftCount = utopiaNFT.totalSupply();
        if(totalNftCount > 0){
            lastTime = nowTime();
            uint256 bonus = 0;
