...
contract Referral is ReferralFetchers, ReentrancyGuard {
    ...
    function addAsAFriend(address customerAddress) external nonReentrant {
        ...
        if (friendToCustomer[customerAddress] == msg.sender){
            revert FriendAndCustomerReferringEachOther();
        }
        ...
        if (totalRewardAmount + totalRewardIssuedOut > maximumReward){
            revert MaxReached();
        }
        ...
        friendToCustomer[msg.sender] = customerAddress;
    ...
    function claimReward(address friendAddress) external nonReentrant {
        ...
        uint256 customerFlexvisBalance = flexvisContract.balanceOf(msg.sender);

        uint256 customerNoOfActiveInsuranceInvestment = (
            insuranceInvestmentContract.getNoOfInvestmentsArray(msg.sender)
        )[1];
    
        if (customerFlexvisBalance < minFlexvisAmount || customerNoOfActiveInsuranceInvestment < minInsuranceInvestmentCount){
            revert CallerClaimConditionsNotSatisfied();
        }

        // Make sure that friend is part of the customer's friend
        ...
        // rewardPercentage = [10, 30, 60]
        /// rewardDays = [0, 15days, 30 days]
        ...
        if (rewardCount == 0 && claimCount == 0) {
            ...
            if (flexvisContract.balanceOf(address(this)) < rewardToClaim) {
                revert InsufficientFlexvisContractBalance();
            }
            // Claim the reward
        ...
        int256 customerIndex = checkIfIncluded(msg.sender, allClaimedAddresses);
...
abstract contract ReferralDeclaration is ReferralError   {
    ...
    uint256 public insuranceInvestmentReward = 20E18;
    uint256 public regularInvestmentReward = 10E18;
    ...
    mapping(address => address) public friendToCustomer;
...
abstract contract ReferralError  {
    ...
    error InsufficientFlexvisContractBalance();
    error FriendAndCustomerReferringEachOther();
    error CallerClaimConditionsNotSatisfied();
}
...
abstract contract ReferralFetchers is ReferralEvents, Ownable {
    function getAllFriends(address customerAddress)
        external
        view
        returns (Friend[] memory friends)
    {
        if (customerAddress == address(0)) {
            revert AddressCannotBeZero();
        }
        if (msg.sender == customerAddress || msg.sender == owner()) {
            friends = customerAddressToFriend[customerAddress];
        } else {
    ...
    function getAllClaimedAddresses()
        external
        view
        onlyOwner
