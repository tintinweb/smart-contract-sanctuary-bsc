...
contract Pledge {
    ...
    mapping(address => order[]) public _profitOrder; //质押列表

    mapping(address => order[]) public _financialOrder; //质押列表

    //质押时间变量
    uint256 public _profitDay = 10;
    //理财时间变量
    uint256 public _financialDay = 5;
    //质押利润率
    uint256 public _profitRate = 100;
    //理财利润率
    uint256 public _financialRate = 50;
    //质押利润衰减
    uint256 public _profitDecay = 90;
    //理财每天单数
    uint256 public _financialDayNum = 3;

    mapping(address => uint256) public _userPledgeTotal; //用户质押总表

    mapping(address => uint256) public _userFinancialTotal; //用户理财总表

    //理财订单日统计
    mapping(uint256 => uint256) public _financialDayCount; //理财订单日统计

    //定长数组10 用户利润
    uint256[10] public _userProfitRate = [10, 6, 5, 4, 3, 2, 1, 1, 1, 1];

    //二维mapping用户烧伤表
    mapping(address => mapping(address => uint256)) public _userBurn;

    mapping(address => mapping(address => uint256)) public _userBurnLast;

    //二位数组10个用户的烧伤表
    mapping(address => mapping(address => uint256[10])) public _userTeamBurn;

    mapping(address => mapping(address => uint256[10]))
        public _userTeamBurnLast;

    //用户星级
    mapping(address => uint8) public _userStar;
    //用户星级奖励
    mapping(address => uint256) public _userStarReward;
    //用户推荐奖励
    mapping(address => uint256) public _userRecommendReward;

    uint256 public _totalPledge; //总质押

    uint256 public _totalFinancial; //总理财

    uint256 public _minPledgeAmount = 10000000; //单笔最小质押额度

    uint256 public _minFinancialAmount = 10000000; //单笔最小理财额度

    uint256 public _maxFinancialAmount = 1000000000; //单笔最大理财额度

    mapping(address => address) public _parents; // 记录上级  我的地址 => 我的上级地址

    mapping(address => address[]) public _inviteRecords; //直推用户

    address public _firstAddress = 0x1aBF9A2E66906F13ff2830bc18478405abC68eE0; //创世地址

    uint256 public _totalAddress; //总地址

    address public _token = 0x5b9Ae21Ba7fe45E5440Ec9e85F92BCb47EE69E31; //TOKEN地址

    //市场地址
    address public _marketAddress = 0x29224fC97BcB2Df0ceCB56BC1E7b579708fFa3ea;
    //市场比率
    uint256 public _marketRate = 15;
    //技术地址
    address public _technologyAddress =
        0x43BCc53C9C6F034Cf6F3d3E736e0230133C626D4;
    //技术比率
    uint256 public _technologyRate = 5;
