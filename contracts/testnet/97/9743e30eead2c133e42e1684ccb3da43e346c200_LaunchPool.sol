...
contract LaunchPool {
    ...
    function setEventPeriod(uint256 _startTime, uint256 _endTime)
        ...
        onlyOwner
    {
        require(address(tokenBuy) != address(0), "Setup raised first");
    ...
    function setWBNBAddress(address _newWBNB) external onlyOwner {
        wBNB = _newWBNB;
    }
    ...
    function withdrawToken(uint256 _amount) public onlyOwner withdrawCheck {
        tokenBuy.transfer(msg.sender, _amount);
    }

    // Buy Function
    function getHardFilled() public view returns (bool) {
        return alreadySale >= hardCap;
    }

    function getSoftFilled() public view returns (bool) {
        return alreadySale >= softCap;
    }

    function getBuyTokenAmount(uint256 _amount)
        internal
        view
        returns (uint256)
    {
    ...
    function buy(uint256 _amount) external payable {
        ...
        require(_amount >= minBuy, "Less than min buy");
        require(_amount <= maxBuy, "More than max buy");
        require(
            _amount + alreadySale <= totalTokenSell,
            "amount buy more than total sell"
        );
        uint256 buyTokenNeed = getBuyTokenAmount(_amount);

        if (address(tokenBuy) == wBNB) {
            require(msg.value >= buyTokenNeed, "Please input value");
        } else {
            uint256 allowance = tokenBuy.allowance(msg.sender, address(this));
            uint256 balance = tokenBuy.balanceOf(msg.sender);
            require(balance >= buyTokenNeed, "Not enough tokens");
            require(allowance >= buyTokenNeed, "Check the token allowance");
            tokenBuy.transferFrom(msg.sender, address(this), buyTokenNeed);
        }

        UserInfo memory userInfo = usersTokenBought[msg.sender];

        userInfo.totalToken += _amount;
        userInfo.totalSpent += buyTokenNeed;
    ...
    function claimFailed() external {
        ...
        if (address(tokenBuy) == wBNB) {
            ...
            tokenBuy.transfer(msg.sender, userSpent);
        }
