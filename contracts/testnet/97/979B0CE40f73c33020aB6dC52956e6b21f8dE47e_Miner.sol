...
contract Miner {
    ...
    function launch(uint amount) public onlyOwner {
        ...
        totalMiners = 10 * busdBalance();
        busdMinerRate = totalMiners / busdBalance();
        isMinerRunning = true;
    ...
    function deposit(uint amount) public isRunning {
        busd.transfer(address(this), amount);
        uint currentMiners = (amount * busdMinerRate) / 2;
    ...
    function compound(address _user) private {
        ...
        uint currentMiners = (dailyBusdReward * busdMinerRate) / 2;
        totalMiners += currentMiners;
        user[msg.sender].miners += currentMiners;
    ...
    function claimTokens(address _user) private {
        // claim mined tokens
        // calculate the tokens mined based on the mining rate and multiply by 24hours
        uint tokenMined = ((minerTokenRate * user[_user].miners) / 100) * 24;
