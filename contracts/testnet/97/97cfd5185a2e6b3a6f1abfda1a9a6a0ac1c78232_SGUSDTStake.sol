...
contract SGUSDTStake is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    address public FHToken;
    address public SGCoin;
    address public USDT;
    ...
    uint256 public usdtOne = 1 * 10 ** _decimals;
    uint256 public usdtFive = 5 * 10 ** _decimals;
    uint256 public usdtThousand = 10 * 10 ** _decimals;
    uint256 public usdtFiveThousand = 50 * 10 ** _decimals;

    uint256 public destroySG = 1 * 10 ** _decimals;

    address public _destroyAddress = address(0x000000000000000000000000000000000000dEaD);

    address public mk = 0x22d2e749Cda9596E6c955DB02613c138AC1E80c2;
    ...
    constructor (address _FHToken, address _SGCoin, address _USDT) {
      FHToken = _FHToken;
      SGCoin = _SGCoin;
      USDT = _USDT;
    ...
    function getUserKeep(address _address) public view returns (bool one, bool two, bool three, bool four) {
    ...
    function setSGDestroy(uint256 _dest) external onlyOwner{
        destroySG = _dest * 10 ** _decimals;
    }
    ...
    function deposit(uint256 amount) public updateReward(msg.sender) {
        ...
        if (amount == usdtOne && !user.one) {
            user.one = true;
            IERC20(SGCoin).safeTransferFrom(msg.sender, _destroyAddress, destroySG);
        } else if (amount == usdtFive && !user.two) {
            user.two = true;
            IERC20(SGCoin).safeTransferFrom(msg.sender, _destroyAddress, destroySG.mul(5));
        } else if (amount == usdtThousand && !user.three) {
            user.three = true;
            IERC20(SGCoin).safeTransferFrom(msg.sender, _destroyAddress, destroySG.mul(10));
        } else if (amount == usdtFiveThousand && !user.four) {
            user.four = true;
            IERC20(SGCoin).safeTransferFrom(msg.sender, _destroyAddress, destroySG.mul(50));
        } else {
    ...
    function getReward() public updateReward(msg.sender) {
        ...
        uint256 time = _userTime[msg.sender] + DURATION;
    ...
    function safeTransfer(address _to, uint256 _amount) internal {
        ...
        if(_amount > tokenBalance) {
            IERC20(FHToken).transfer(_to, tokenBalance);
        } else {
