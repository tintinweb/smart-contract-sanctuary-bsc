...
contract DSBStake is Ownable {
    ...
    constructor() {
        ...
        path[1] = 0x5450169A273Fe112CaD88E503758e3416A4A4666;
    }
    ...
    function claim() public onlyEOA returns (bool) {
        ...
        for (uint256 i = 0; i < stakedNft.length; i++) {
            ...
            if (
                sInfo.isFirstDone == false && sInfo.currentOwner == msg.sender
            ) {
                sInfo.isFirstDone = true;
            }
        }
    ...
    function getStakedNFT(address addr) public view returns (uint256[] memory) {
        uint256 arrCount = stakeCount[addr];
        require(arrCount > 0, "address does not stake a single nft");
        uint256[] memory output = new uint256[](arrCount);
        for (uint256 i = 0; i < 500; i++) {
            StakeInfo storage sInfo = stakeTracker[i + 1];
