...
contract TestStakingBSC is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    function getCampaign(address token) public view returns(uint256 percentage, uint256 rate){
        return (
    ...
    function updatePercentage(address token, uint256 percentage) external onlyOwner validToken(token) updateReward(address(0), token) {
