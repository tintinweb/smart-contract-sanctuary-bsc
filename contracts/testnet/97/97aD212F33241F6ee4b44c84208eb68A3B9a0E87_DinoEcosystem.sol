...
contract RouterEarn is Context, Auth {
    ...
    bool public enableReflection = true;
    bool public enableFarming = false;
    bool public enableStaking = false;
    bool public enableLoop = false;

    uint public currentIndex = 1;

    constructor(address earn, address owner) Auth(owner) {
        addressReflection = earn;
        percentReflection = 10000;
    }

    function deposit(uint256 loop) external payable {
        loop = enableLoop ? loop : 1;
        uint256 baseAmount = msg.value / percentTaxDenominator;
        if(enableReflection && percentReflection > 0){
            uint256 amountReflection = percentReflection * baseAmount;
            IEarn(addressReflection).deposit{value:msg.value}(amountReflection);
        }
        if(enableFarming && percentFarming > 0){
            uint256 amountFarming = percentFarming * baseAmount;
            IEarn(addressFarming).deposit{value:msg.value}(amountFarming);
        }
        if(enableStaking && percentStaking > 0){
            uint256 amountStaking = percentStaking * baseAmount;
            IEarn(addressStaking).deposit{value:msg.value}(amountStaking);
        }
    ...
    function setEnable(bool reflection, bool staking, bool farming) external onlyOwner {
        enableReflection = reflection;
        enableStaking = staking;
        enableFarming = farming;
    }
...
contract DinoEcosystem is Context, Auth, IBEP20 {
    ...
    constructor() Auth(msg.sender) {
        ...
        DinoReflection earn = new DinoReflection(address(this), 0x1782AA2B0762B134B0Ba4110099b32a46e61919e, msg.sender);
