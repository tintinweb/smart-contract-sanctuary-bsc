...
// File: contracts/EpicGalleryNFT.sol
...
contract EpicGalleryNFT is ERC721 {
    ...
    constructor() ERC721("EpicGallery","Epic") {    
        factory = msg.sender;
    }
...
// File: contracts/EpicGalleryMarket.sol
...
contract EpicGalleryMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
	...
	uint256 public totalEarning; /* Total EpicGallery Token */
	uint256 public totalSwapped; /* Total swap count */
    
	/** Events */
    event CollectionCreated(address collection_address, address owner);
	...
	constructor (address collAddr) {	
		emit CollectionCreated(collAddr, msg.sender);
	}
    ...
    function createCollection() public returns(address collection) {
		bytes memory bytecode = type(EpicGalleryNFT).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp));
        assembly {
             collection := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IEpicGalleryNFT(collection).initialize(msg.sender, adminAddress);
    ...
    function list(address _collection, uint256 _token_id, address _tokenAdr, uint256 _price) OnlyItemOwner(_collection,_token_id) external {
		...
		IEpicGalleryNFT nft = IEpicGalleryNFT(_collection);        
    ...
    function delist(uint256 _id) external {        
        ...
        IEpicGalleryNFT(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].token_id);        
    ...
    function buy(uint256 _id) external payable {
		...
		if (pairs[_id].tokenAdr == address(0x0)) { //MTV
			...
			if (pair.creatorFee > 0 ){
				governanceToken.transferFrom(msg.sender, pair.creator, totalAmount.mul(pair.creatorFee).div(PERCENTS_DIVIDER));	
			}		
		...
		IEpicGalleryNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
