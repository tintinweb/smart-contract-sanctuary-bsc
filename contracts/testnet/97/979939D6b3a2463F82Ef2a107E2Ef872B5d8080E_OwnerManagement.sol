...
// File: contracts/IDocumentStoreInterface.sol
...
contract DocumentStore is OwnableUpgradeable {
    ...
    function setEmail(string memory _email) private {
        email = _email;
        IDocumentStoreInterface(ownerManager).setEmail(address(this), _email);
    }

    function setLegalReference(string memory _legalReference) private {
        legalReference = _legalReference;
    ...
    function setIntentDeclaration(string memory _intentDeclaration) private {
        intentDeclaration = _intentDeclaration;
    ...
    function setHost(string memory _host) private {
        host = _host;
        IDocumentStoreInterface(ownerManager).setHost(address(this), _host);
    }

    function removePublisher(address _publisher) private onlyOwner {
    ...
    function removeAllPublishers() private onlyOwner {
        for (uint256 i; i < publishers.length; i++) {
            publishers.pop();
        }
    }

    function addPublisher(address _newPublisher) private onlyOwner {
    ...
    function verify(
        ...
        bytes32 root,
        bytes32 leaf
    ) internal onlyIssued(root) onlyNotRevoked(root) returns (bool) {
    ...
    function bulkRevoke(bytes32[] memory documents) internal {
        for (uint256 i = 0; i < documents.length; i++) {
            revoke(documents[i]);
        }
    }
    ...
    modifier onlyVerified(bytes32[] memory proof, bytes32 root, bytes32 leaf) {
        require(
            ...
            "Error: Leaf is not verified");
        _;
    }
}
// File: contracts/OwnerManagement.sol
...
contract OwnerManagement {
    ...
    mapping(address => bool) public instAddable;
    ...
    function addInstitution() public {
        require(instAddable[institutions[institutions.length-1]]);
        ...
        instAddable[_institution] = false;
    }
    ...
    function deploy(
        ...
        instAddable[address(instance)] = true;
        addInstitution();
