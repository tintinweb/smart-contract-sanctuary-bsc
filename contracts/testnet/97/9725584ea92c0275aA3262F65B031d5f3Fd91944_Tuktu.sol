...
abstract contract Balance is IBalance, Account {
	...
	function Withdraw(
		uint _AID,
		uint _Amount
	) public virtual nonReentrant VerifyAmount(_Amount) OnlyAccountOwner(_AID, msg.sender) returns (bool success_) {
	...
	function TransferToken(
		...
		OnlySupportedToken(_Token)
		VerifyAmount(_Amount)
		OnlyAccountOwner(_FromAccount, msg.sender)
...
contract Tuktu is ITuktu, Balance {
	...
	function UpgradeLevelManually(
		...
		uint _LTo,
		IERC20 _Token
	) public payable OnlyAccountExisted(_AID) returns (bool success_) {
		if (_LFrom < 1 || _LFrom > 15 || !XPrograms.isLevelActivated(_AID, _XPro, _LFrom - 1)) {
			for (_LFrom = 2; _LFrom <= 15; ++_LFrom) if (!XPrograms.isLevelActivated(_AID, _XPro, _LFrom)) break;
			if (_LFrom > 15) return false;
		}

		if (_LTo < 1 || _LTo > 15 || _LTo < _LFrom) _LTo = 15;
		uint amountUSD = PirceOfLevelUpgradeManually(_LFrom, _LTo);
		if (!isSupportedToken(_Token)) _Token = DEFAULT_STABLE_TOKEN;

		if (msg.value > 0) {
			if (!DepositETH(_AID, _Token, amountUSD)) revert("Deposit eth fail!");
		} else {
			if (!DepositToken(_AID, _Token, amountUSD)) revert("Deposit token fail!");
		}

		return XPrograms._UpgradeLevelManually(_AID, _XPro, _LFrom, _LTo);
	}

	function PirceOfLevelUpgradeManually(uint _LFrom, uint _LTo) public view returns (uint pirceOfLevelUpgradeManually_) {
		for (uint i = _LFrom; i <= _LTo; ++i) pirceOfLevelUpgradeManually_ += PirceOfLevel[i];
	}
