...
contract MLRFactory is Ownable {
    ...
    constructor(address _depository) {
        require(_depository != address(0), "NA");
    ...
    function createMLR(
        CreateArgs[] memory _createArgs
    ) external {
        for (uint256 i = 0; i < _createArgs.length; i ++) {
            require( _createArgs[i].nftContract != address(0), "NA" );
            require( _createArgs[i].maxRoyaltyOwner > 0, "No Royalty owner" );
            require( getMLR[_createArgs[i].nftContract][_createArgs[i].tokenID] == address(0), "MLR_EXISTS" );
            // Depository
            // depository = address( new Depository() );
            // IDepository(depository).initialize();
            // MLR
            address mlr = address(
                new MultilevelRoyalty(
                    _createArgs[i].nftContract,
                    _createArgs[i].tokenID,
                    _createArgs[i].maxRoyaltyOwner,
                    _createArgs[i].name,
                    _createArgs[i].tokenName,
                    _createArgs[i].tokenSymbol,
                    depository
                )
            );
            IERC721(_createArgs[i].nftContract).safeTransferFrom(msg.sender, mlr, _createArgs[i].tokenID);   // send NFT to MLR
            IMultilevelRoyalty(mlr).initialize(msg.sender);
            
            getMLR[_createArgs[i].nftContract][_createArgs[i].tokenID] = mlr;
            allMLRs.add(mlr);
            
            IMultilevelRoyalty(mlr).createOrder(
                _createArgs[i].buyoutPrice,
                _createArgs[i].royaltyFee,
                _createArgs[i].tokenPrice,
                _createArgs[i].protectionRate,
                _createArgs[i].isFixedProtection,
                _createArgs[i].protectionTime,
                _createArgs[i].acceptOffers,
                _createArgs[i].offerClosingTime
            );
...
interface IMultilevelRoyalty {
	function initialize(address _nftOwner) external;
...
contract MultilevelRoyalty is Ownable, ERC20 {
	...
	struct Order {
		...
		uint256 buyPrice; // previous buy price of seller, to calculate profit
		// protection
	...
	mapping(uint256 => Order) public getOrder;	// identify getOrder by orderID
    ...
    mapping(uint256 => Offer) public getOffer;                     // identify getOffer by offerID
	...
	event ClaimDownsideProtectionInOffer(
		uint256 offerID,
		uint256 statusOrder,
		uint256 soldTime,
		address indexed buyerOrSeller,
	...
	function transfer(address to, uint256 amount) public virtual override returns (bool) {
		...
		require(balanceOf(owner) == amount, "all balance");
	...
	function transferFrom(
		...
		require(balanceOf(from) == amount, "all balance");
	...
	function updateRoyaltyInfo(
		...
		if ( balanceOf(msg.sender) == 0 && _royaltyFee > 0 ) {	// add royalty
			require( currentOwner == payable(msg.sender), "Only owner" );
			require( royaltyHolders.length < maxRoyaltyOwner, "limit exceed" );
			
			_mint( msg.sender, _royaltyFee );
		} else {	//update royalty
			require( _royaltyFee < balanceOf(msg.sender), "not fee up" );
	...
	function buyOut(
		...
		require( royaltyIndex[_previousOwner] > 0 , "only royalty owner" );
		require(buyoutPrices[_previousOwner] > 0, "buyout > 0");
		uint32 realRoyalty = getRealRoyalty(_previousOwner);
		uint256 realPrice = buyoutPrices[_previousOwner] * realRoyalty / 100;
		if (msg.value > 0) {
			require( msg.value >= realPrice, "less price" );
		} else {
			require( _vaultAddress != address(0), "NA" );
            require( IVault(_vaultAddress).owner() == msg.sender, "Invalid owner" );
            IVault(_vaultAddress).withdraw(address(this), realPrice);
		}
		...
		payable(_previousOwner).transfer(realPrice);
	...
	modifier createOrderValidator(
		...
		require( currentOwner == payable(msg.sender) || msg.sender == factory, "only owner" );
		require( _tokenPrice > 0, "Invalid price" );
		if ( _acceptOffers ) {
			require( _offerClosingTime > 0, "Auction closing time" );
		}
		_;
	}

	modifier buyFixedPayOrderValidator( uint256 _orderId ) {
        Order storage order = getOrder[_orderId];
        require( order.statusOrder == OrderStatus.Active, "Invalid status" );   
        require( order.typeOrder == OrderType.FixedPay, "Invalid type" );   // AuctionType getOrder are directly executed by seller
        _;
    }

    modifier onlySeller( uint256 _orderId ) {
        Order storage order = getOrder[_orderId];
        require( payable(msg.sender) == order.sellerAddress, "Only seller" );
        _;
    }

	modifier onlyBuyerOrExpired(uint256 _offerId) {
        Offer storage offer = getOffer[_offerId];
	...
	{
		require(_protectionRate <= 10000 , "Protection rate <= 100%");
		
		orderIdCount ++;
		Order storage order = getOrder[orderIdCount];
		...
		if ( balanceOf(currentOwner) == 0 && _royaltyFee > 0 ) {
			require( royaltyHolders.length < maxRoyaltyOwner, "limit exceed" );
			_mint( currentOwner, _royaltyFee );
		} else {
			require( _royaltyFee < balanceOf(msg.sender), "no fee up" );
	...
	function claimDownsideProtectionAmount( uint256 _orderId ) external {
        Order storage order = getOrder[_orderId];
        require(order.statusOrder == OrderStatus.UnderDownsideProtectionPhase, "Invalid status");   
	...
	function createBid( uint256 _orderId ) external payable {
		...
		require( msg.value > previousMaxOfferAmount, "too low" );
        require( order.statusOrder == OrderStatus.Active || order.statusOrder == OrderStatus.Bidded, "Invalid status" );
        require( order.typeOrder == OrderType.AuctionType, "Invalid type" );
        require( order.offerClosingTime >= block.timestamp, "time over" );
	...
	function executeBid( uint256 _orderId ) external {
        ...
        require(order.typeOrder == OrderType.AuctionType, "Invalid type");
        require(order.statusOrder == OrderStatus.Bidded, "Invalid type");
        require(order.offerClosingTime <= block.timestamp, "bidding");
	...
	function updateBid(uint256 _orderId) external payable {
        ...
        require(order.statusOrder == OrderStatus.Bidded, "Invalid status");
        require(order.typeOrder == OrderType.AuctionType, "Invalid type");
        require(order.buyerAddress == payable(msg.sender), "Only bidder");
        require(order.offerClosingTime >= block.timestamp, "time over");
        require(buyerBidStatus[order.buyerAddress][_orderId] == BidStatus.Pending, "no pending");
	...
	function createOffer(
        ...
        require(_tokenPrice > 0 && _tokenPrice == msg.value, "token price");
        require(_protectionRate <= 10000 , "Protection rate <= 100%");
        
        offerIdCount++;
        Offer storage offer = getOffer[offerIdCount];
	...
	function acceptOffer(uint256 _offerId) external returns (bool) {
        Offer storage offer = getOffer[_offerId];
        require(offer.statusOffer == OfferStatus.Active, "Invalid status");
        require(currentOwner == payable(msg.sender), "only owner");
        
        offer.statusOffer = OfferStatus.UnderDownsideProtectionPhase;
        offer.sellerAddress = payable(msg.sender);

        uint256 downsideAmount = offer.tokenPrice * offer.protectionRate / 10000;    // downsideAmount comes after companyShare
		uint256 allRoyaltyFees = 0;
		if ( buyPrice > 0 && offer.tokenPrice - buyPrice > 0 ) {
			uint256 profit = (offer.tokenPrice - buyPrice) * (10000 - offer.protectionRate) / 10000;
			allRoyaltyFees = _sendRoyaltyFee( profit );
		}
        offer.sellerAddress.transfer(offer.tokenPrice - downsideAmount - allRoyaltyFees);        // Transfer the seller his amount
        uint256 depositId = depository.deposit{value: downsideAmount}();     // Invest the downside in Venus
		
        currentOwner = offer.buyerAddress;     // Transfer the NFT
		if ( balanceOf(offer.sellerAddress) > 0 )  {
			royaltyHolders.push( offer.sellerAddress );
		}
		royaltyIndex[offer.sellerAddress] = royaltyHolders.length;
		offer.buyPrice = buyPrice;
		buyPrice = offer.tokenPrice;
	...
	function claimDownsideProtectionAmountInOffer(uint256 _offerId) external returns (bool) {
        Offer storage offer = getOffer[_offerId];
        require(offer.statusOffer == OfferStatus.UnderDownsideProtectionPhase, "Invalid status");   
            ...
            && offer.soldTime != 0)
        {
            ...
            uint256 allRoyaltyFees = 0;
			if ( offer.tokenPrice > offer.buyPrice ) {
				uint256 profit = value - offer.buyPrice * offer.protectionRate / 10000;
				allRoyaltyFees = _sendRoyaltyFee( profit );
			}
            offer.sellerAddress.transfer( value - allRoyaltyFees );   // Transfer to Seller the whole Yield Amount
            ...
            && offer.soldTime != 0)
        {
			...
			currentOwner = offer.sellerAddress;     // Send NFT back to seller
			_deleteRoyaltyHolder( currentOwner );
	...
	function _proceedPayments(
		...
		if ( balanceOf(order.sellerAddress) > 0 ) {
	...
	function withdrawNFT() external {
		require( currentOwner == payable(msg.sender), "Only owner" );
		require( totalSupply() == 0, "allroyalty == 0" );
