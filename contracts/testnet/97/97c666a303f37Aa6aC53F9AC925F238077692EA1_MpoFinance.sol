...
contract MpoFinance is OwnableUpgradeable {
    ...
    mapping(uint => EventInfo) public eventInfo;
    ...
    // 2.0
    uint public thisRoundTotal;
    uint public thisRoundClaimed;
    ...
    function init() public initializer {
        ...
        setIDO(130, 100000000 ether, 100 ether);
        swSwitch = true;
        wSwitch = true;

        MpoInvite = 0x1CC66756E6015A945eA7e59B0ad5a04B0c5Abe55;
        MPO = 0xB3eA60DE817c2Bd70bFCE46BE2598D7165E2073C;
    ...
    function setThisRoundBonus(uint bonus_) public onlyAdmin {
        thisRoundTotal = bonus_;
        IERC20(MPO).transferFrom(msg.sender, address(this), bonus_);
        thisRoundClaimed = 0;
    }
    ...
    function setIdoSWswitch(bool b_) public onlyAdmin {
        swSwitch = b_;
    }

    function setIdoWswitch(bool b_) public onlyAdmin {
        wSwitch = b_;
    }
    ...
    function preInvite(address user_, address inv_) internal {
        address _inv = IMPOInvite(MpoInvite).checkInviter(user_);
        if (_inv != address(0)) {
            require(_inv == inv_, "inviter wrong");
        } else {
            IMPOInvite(MpoInvite).whoIsYourInviter(user_, inv_);
        }
    ...
    function mutiCheck(address user_)
        external
        view
        returns (uint[4] memory list, bool[2] memory b)
    {
        ...
        returns (uint aa)
    {
        ...
        aa =
            ...
            userInfo[p][user_].claimed;
    }

    function finance_claimBonus() public returns (uint rew) {
        require(thisRoundClaimed < thisRoundTotal, "out of bonus");
        address user_ = msg.sender;
        uint _phase = IMPOT(MPO).checkPhase();
        uint _thisRoundRemaining = thisRoundTotal - thisRoundClaimed;

        rew = finance_checktoClaimBonus(user_);
        if (rew > 0) {
            if (_thisRoundRemaining < rew) {
                rew = _thisRoundRemaining;
            }
            userInfo[_phase][user_].claimed += rew;
            userInfo[_phase][user_].lastClaimTime = block.timestamp;
            thisRoundClaimed += rew;
            IERC20(MPO).transfer(user_, rew);

            emit ClaimBonus(user_, rew);
        }
    }
}