...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        notZeroAddress
    {
        ...
        uint256 userVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        uint256 priceEquivalentVouchersAmount = price / electricityVoucherPrice;
        if(price > 0) {
            if (userVouchers > 0 && userVouchers >= priceEquivalentVouchersAmount) {
                userToElectricityVoucherAmountMapping[msg.sender] -= priceEquivalentVouchersAmount;
            } else {
                price -= userVouchers * electricityVoucherPrice;
                userToElectricityVoucherAmountMapping[msg.sender] = 0;
    ...
    function purchaseBasicBundle(uint8 _bundleType, uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        uint256 userVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        uint256 priceEquivalentVouchersAmount = bundlePurchasePrice / electricityVoucherPrice;
        if (userVouchers > 0 && userVouchers >= priceEquivalentVouchersAmount) {
            userToElectricityVoucherAmountMapping[msg.sender] -= priceEquivalentVouchersAmount;
        } else {
            bundlePurchasePrice -= userVouchers * electricityVoucherPrice;
            userToElectricityVoucherAmountMapping[msg.sender] = 0;
            _transferToContract(bundlePurchasePrice);
            IAmountsDistributor(amountsDistributorAddress).distributeAmounts(bundlePurchasePrice, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        }
    ...
    function repairAllMiners() external onlySaleActive notZeroAddress {
        ...
        uint256 userVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        uint256 priceEquivalentVouchersAmount = _amount / electricityVoucherPrice;
        if (userVouchers > 0 && userVouchers >= priceEquivalentVouchersAmount) {
            userToElectricityVoucherAmountMapping[msg.sender] -= priceEquivalentVouchersAmount;
        } else {
            _amount -= userVouchers * electricityVoucherPrice;
            userToElectricityVoucherAmountMapping[msg.sender] = 0;
    ...
    function upgradeStandardParcelsGroup(uint256[] memory _parcelIds) external onlySaleActive {
        ...
        uint256 userVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        uint256 priceEquivalentVouchersAmount = _totalUpgradePrice / electricityVoucherPrice;
        ...
        if(_totalUpgradePrice > 0) {
            if (userVouchers > 0 && userVouchers >= priceEquivalentVouchersAmount) {
                ...
                _totalUpgradePrice -= userVouchers * electricityVoucherPrice;
                userToElectricityVoucherAmountMapping[msg.sender] = 0;
    ...
    function upgradeStandardParcel(uint256 _parcelId) external onlySaleActive {
        ...
        uint256 userVouchers = userToElectricityVoucherAmountMapping[msg.sender];
        uint256 priceEquivalentVouchersAmount = upgradePrice / electricityVoucherPrice;
        if(usersMintableNFTAmounts[msg.sender].upgrades > 0) {
            usersMintableNFTAmounts[msg.sender].upgrades--;
        } else {
            if (userVouchers > 0 && userVouchers >= priceEquivalentVouchersAmount) {
                ...
                upgradePrice -= userVouchers * electricityVoucherPrice;
                userToElectricityVoucherAmountMapping[msg.sender] = 0;
