...
contract Orderbook is Initializable, IOrderbook, OwnableUpgradeable {
    ...
    function placeSellOrder(uint256 amountOfTradeToken) external {
        ...
        while (busdToSend > 0 && counter <= buyCount) {
            ...
            uint256 mxstToSend;
            if (curBidOrder.remAmount >= busdToSend) {
                mxstToSend = busdToMxst(busdToSend);
                curBidOrder.remAmount = curBidOrder.remAmount.sub(mxstToSend);
                busdToSend = 0;
            } else {
                mxstToSend = busdToMxst(curBidOrder.remAmount);
                busdToSend = busdToSend.sub(curBidOrder.remAmount);
                curBidOrder.remAmount = 0;
            }

            if (curBidOrder.remAmount == 0) {
                curBidOrder.isFilled = true;
                buyStep = counter + 1;
            }
        ...
        if (sellOrders[orderId].remAmount == 0) {
            sellOrders[orderId].isFilled = true;
            sellOrders[orderId].updatedAt = block.timestamp;
            sellStep = orderId + 1;
        }
        ...
        emit PlaceSellOrder(msg.sender, mxstPrice, amountOfTradeToken);
    }
