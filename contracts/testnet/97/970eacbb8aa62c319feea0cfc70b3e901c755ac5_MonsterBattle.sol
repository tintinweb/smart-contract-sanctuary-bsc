...
contract MonsterBattle is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    MonsterAttr public monsterAttr;              // 怪兽属性
    PlayerAttr public playerAttr;                // 玩家属性
    SummonedAttr public summonedAttr;            // 召唤兽属性
    ...
    mapping(uint => Monster) public mapAmount;
    ...
    function setAmount(uint monsterId_, uint amount_) public onlyOwner {
        mapAmount[monsterId_] = Monster({
            amount: amount_,
            bounty: initBounty
        });
    }

    function setAttr(MonsterAttr calldata monster_, PlayerAttr calldata player_, SummonedAttr calldata summoned_) public onlyOwner {
     ...
     * {bool} 是否有召唤兽
     */
    function battle(uint[] memory playGame_, uint monsterId_, bool isSummoned_) public payable returns(uint) {
        address player = _msgSender();

        // 挑战价格
        uint totalPrice = mapAmount[monsterId_].bounty * 1 / 10;
        // 是否使用召唤兽
        if (isSummoned_) {
            totalPrice += summonedPrice;
        }
        require(msg.value == totalPrice, "playerMint: Price_error");
        require(playGame_.length != 0, "battle: No punches thrown");
        require(mapAmount[monsterId_].amount != 0, "battle: amount err");
    ...
    // 获取挑战价格
    function getBattlePrice(uint monsterId_, bool isSummoned_) public view returns(uint) {
        // 挑战价格
        return isSummoned_ ? mapAmount[monsterId_].bounty * 1 / 10 + summonedPrice : mapAmount[monsterId_].bounty * 1 / 10;
    }
