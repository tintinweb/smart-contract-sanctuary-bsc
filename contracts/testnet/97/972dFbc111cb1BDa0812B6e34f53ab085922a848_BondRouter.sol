...
contract BondRouter is Ownable, BondStruct, Pausable, ReentrancyGuard {
    ...
    /* ===================== Events ===================== */
    event UpdateBondPrice(uint256 _batchId, uint256[] _prices, bool _action);
    ...
    event UpdateBeneficiary(address _old, address _new, uint256 _batchId);
    event CreateNewBatch(uint256 _batchId, BatchConfig _config, BackedBond[] _backedBond);
    ...
    /* ===================== Internal functions ===================== */
    function _payment(IERC20 _currency, address _receiver, uint256 _amount) internal {
        _currency.safeTransfer(_receiver, _amount);
    }
    ...
    function _calInterest(uint256 _interest, uint256 _lastHarvest, uint256 _maturity, uint256 _startTime) internal view returns (uint256) {
        ...
        return (_endTime - _lastHarvest) * _interest / (_maturity - _startTime);
    }
    ...
    function _executeRequest(uint256 _requestId) internal {
        ...
        IERC20(batchInfo[_request.batchId].config.currency).safeTransferFrom(msg.sender, _request.to, _request.amount);
    ...
    function _buyBond(uint256 _batchId, uint256 _price, uint256 _quantity) internal {
        ...
        //transfer fund
        IERC20(_batchInfo.config.currency).safeTransferFrom(msg.sender, address(this), _amountTransfer);

        _batchInfo.raised += _amountTransfer;

        //issue bond nft
        _issue(bondNFTAddress, msg.sender, _quantity, _price, _batchInfo.config.maturity, _batchInfo.config.interestRate * _price / ONE_HUNDRED_PERCENT, _batchId);

        emit BuyBond(msg.sender, _price, _quantity, _batchId);
    }

    function _calAmount(uint256 _tokenId, uint256 _batchId, uint256 _startTime) internal returns (uint256 _interest, uint256 _bondAmount, uint256 _amountBack) {
    ...
    function _executeRedeem(uint256 _batchId, uint256[] memory _ids, address _to) internal {
        ...
        if (IERC20(_info.config.currency).balanceOf(address(this)) >= _amountSent) {
            _redeem(bondNFTAddress, _ids);
            _payment(IERC20(_info.config.currency), _to, _amountSent);
        } else {
    ...
    function _executeHarvest(uint256 _batchId, uint256[] memory _ids, address _to) internal {
        ...
        BatchInfo storage _info = batchInfo[_batchId];
        require(block.timestamp >= _info.config.startTime, "BondRouter: can not harvest now");

        uint256 _amountSent = 0;
        for (uint256 i = 0; i < _ids.length; i++) {
            (uint256 _interestAmount,,) = _calAmount(_ids[i], _batchId, _info.config.startTime);
    ...
    function createNewBatch(BatchConfig memory _config, BackedBond[] memory _backedBond) external onlyOperator {
        batchId ++;
        ...
        batchInfo[batchId].bondPrice.add(2000 ether);
        emit CreateNewBatch(batchId, _config, _backedBond);
    }

    function updateBondPrice(uint256 _batchId, uint256[] memory _prices, bool _action) external onlyOperator {
        BatchInfo storage _info = batchInfo[_batchId];
        ...
        emit UpdateBondPrice(_batchId, _prices, _action);
    }

    function executeRequest(uint256 _requestId) external onlyOperator {
        require(listRequestId.contains(_requestId), "BondRouter: request id not in list");
        _executeRequest(_requestId);
    }

    function withdrawFund(uint256 _batchId) external onlyOperator {
        BatchInfo storage _info = batchInfo[_batchId];
        require(_info.config.beneficiary != address(0), "BondRouter: !_batchId");
        IERC20 _currency = IERC20(_info.config.currency);
        _currency.safeTransfer(_info.config.beneficiary, _info.raised);
    }

    function refund(IERC20 _erc20, uint256 _amount) external onlyOperator {
    ...
    function updateBeneficiary(address _value, uint256 _batchId) external onlyOperator {
        BatchInfo storage _info = batchInfo[_batchId];
        require(_value != address(0), "BondRouter: invalid value");
        address _old = _info.config.beneficiary;
        _info.config.beneficiary = _old;
        emit UpdateBeneficiary(_old, _value, _batchId);
    }
...
interface BondStruct {
    ...
    struct BatchConfig {
        ...
        address beneficiary; // address receiver
        address currency; //
    }
...
interface IBondNFT is BondStruct {
    function info(uint256 id) external returns (BondInfo memory _info);
