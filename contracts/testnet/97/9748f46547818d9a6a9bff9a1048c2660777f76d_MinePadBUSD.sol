...
contract MinePadBUSD is Context, Ownable , ReentrancyGuard, Events {
    ...
    mapping(address => vestingTime) public vesting;
    mapping(address => userAllocations) public allocationTokensReward;
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        if(!checkAlready()){
            ...
            weekly[msg.sender] = weeklyWithdraw(msg.sender,block.timestamp,deadline_weekly);
    ...
    function _updateAllocations(uint256 _amount) internal {
        uint256 currentReward = allocationTokensReward[msg.sender].totalWithdrawal;
        
        uint256 currentAllocate = allocationPoint(_amount);

        uint256 sumReward = SafeMath.add(currentReward,currentAllocate);

        vesting[msg.sender] = vestingTime(msg.sender, SafeMath.add(v_period, block.timestamp));
        
        allocationTokensReward[msg.sender] = userAllocations(msg.sender, sumReward);
    ...
    function claimAllocationReward() external noReentrant {
        require(block.timestamp > vesting[msg.sender].vestingPeriod, "You cant withdraw");
        uint256 currentReward = allocationTokensReward[msg.sender].totalWithdrawal;
        require(tokenAllocationInterface.balanceOf(address(this)) > currentReward, "No More Reward.");
        uint256 wFee = withdrawFee(currentReward);
        uint256 totalAmountToWithdraw = SafeMath.sub(currentReward,wFee); // changed from aval_withdraw to aval_withdraw2
        
        allocationTokensReward[msg.sender] = userAllocations(msg.sender, 0);
    ...
    function resetAlloc(uint256 _newAlloc) external onlyOwner {
        alloc = _newAlloc;
    }
