...
// by alex2 Testing Lottery with testing generator
...
// File: contracts/interfaces/IUfxLottery.sol

pragma solidity ^0.8.4;

interface IUfxLottery {
    /**
     ...
     * @param _priceTicketInUfx: price of a ticket in CAKE
 ...
 */
contract UfxLottery is ReentrancyGuard, IUfxLottery, Ownable {
    ...
    uint256 public maxPriceTicketInUfx = 50 ether;
    uint256 public minPriceTicketInUfx = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInUfx;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ufxPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInUfx;
        uint32 finalNumber;
    }
     ...
     * @param _ufxTokenAddress: address of the CAKE token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _ufxTokenAddress, address _randomGeneratorAddress) {
        ufxToken = IERC20(_ufxTokenAddress);
        ...
        nonReentrant
    {
        ...
        uint256 amountUfxToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInUfx,
            _ticketNumbers.length
        );

        // Transfer ufx tokens to this contract
        ufxToken.safeTransferFrom(address(msg.sender), address(this), amountUfxToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInUfx += amountUfxToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInUfxToTransfer
        uint256 rewardInUfxToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInUfxToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        ufxToken.safeTransfer(msg.sender, rewardInUfxToTransfer);

        emit TicketsClaim(msg.sender, rewardInUfxToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInUfx) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].ufxPerBracket[j] =
                ...
                _lotteries[_lotteryId].ufxPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInUfx - amountToShareToWinners);

        // Transfer CAKE to treasury address
        ufxToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ufxToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInUfx += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInUfx >= minPriceTicketInUfx) && (_priceTicketInUfx <= maxPriceTicketInUfx),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInUfx: _priceTicketInUfx,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            ufxPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInUfx: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ufxToken), "Cannot be CAKE token");
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInUfx: minimum price of a ticket in CAKE
     * @param _maxPriceTicketInUfx: maximum price of a ticket in CAKE
     */
    function setMinAndMaxTicketPriceInUfx(uint256 _minPriceTicketInUfx, uint256 _maxPriceTicketInUfx)
        external
        onlyOwner
    {
        require(_minPriceTicketInUfx <= _maxPriceTicketInUfx, "minPrice must be < maxPrice");

        minPriceTicketInUfx = _minPriceTicketInUfx;
        maxPriceTicketInUfx = _maxPriceTicketInUfx;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].ufxPerBracket[_bracket];
        } else {
            return 0;
        }
    }
