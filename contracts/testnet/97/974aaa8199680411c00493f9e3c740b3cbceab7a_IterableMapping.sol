...
/*
    function getminimumTokenBalanceForDividends() public view returns (uint256) {
        uint256 min = minPercentOfSupplyForDiv;
...
contract INFINITYLOTTOV2 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Infinity Lotto V2";
    string constant _symbol = "iLOTTOV2";
    ...
    uint256 public totalFee         = marketingFee + TreasuryFee + reflectionFee + liquidityFee + lotteryFee + burnFee;
    ...
    address public marketingFeeReceiver = 0xEdc13e391D5CA5d8B9d788d8B401863C81135Bf4;
    address private TreasuryFeeReceiver = 0x8E7A2ada58181eCD0A04e14968d15B5257769c39;
    address public lotteryFeeReceiver = 0xAe6d9aE0854875243CFE1FaF7DBDa17f4c22729c;
    address private treasuryDivWallet = 0x1A6891c4C91787E7e21611f9e94aF2A55fbD56F8;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Staking count for Div Tracker
        if(isStakingContract[recipient]) stakedAmount[sender].add(amountReceived);       // staking
        if(isStakingContract[sender]) stakedAmount[recipient].sub(amount);               // unstaking
        if(recipient == treasuryDivWallet) stakedAmount[treasuryDivWallet].add(amountReceived);  // treasury wallet received dividends
        if(sender == treasuryDivWallet) stakedAmount[treasuryDivWallet].sub(amount);             // treasury wallet gets less dividends
        
        
        // Dividend tracker
        if(!isDividendExempt[sender]) {
            try distributor.setBalance(payable(sender), stakedAmount[sender]) {} catch {}
        }
 
        if(!isDividendExempt[recipient]) {
            try distributor.setBalance(payable(recipient), stakedAmount[recipient]) {} catch {}
        }
    ...
    function takeFee(address sender,address recipient, uint256 amount, bool isSell) internal returns (uint256) {
        ...
        // Staking Fees
        if(isStakingContract[recipient] || isStakingContract[sender]) {
          uint256 stakingFeeAmount = amount.div(feeDenominator * 100).mul(stakingFee).mul(100);
          feeAmount += stakingFeeAmount;
    ...
    function ClearMarketingBalance(uint256 amountPercentage) external authorized {
        ...
        payable(lotteryFeeReceiver).transfer(amountBNB * amountPercentage / 100);
            ...
            function CleartreasuryBalance(uint256 amountPercentage) external authorized {
        uint256 amountBNB = address(this).balance;
    ...
    function swapBack() internal swapping {
        ...
        // need to convert BNB TO BUSD and SEND IT  ( amoutnBNBReflection )
    ...
    function swapAndSendToDiv(uint256 amount) internal {
        uint256 beforeAmount = IBEP20(RWRD).balanceOf(address(distributor));
        ...
        uint256 amountBUSDReflection = IBEP20(RWRD).balanceOf(address(distributor)).sub(beforeAmount);

        try distributor.distributeTokenDividends(amountBUSDReflection) {} catch {}
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external authorized {
        ...
        if(exempt){
            distributor.setBalance(payable(holder), 0);
        }else{
            distributor.setBalance(payable(holder), stakedAmount[holder]);
        }
    }
    ...
    function setContractFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _lotteryFee, uint256 _TreasuryFee, uint256 _burnFee, uint256 _stakingFee) external authorized {
        ...
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_lotteryFee).add(TreasuryFee);
        feeDenominator = 100;
        require(totalFee + burnFee < feeDenominator/6, "Max Fees = 16.6%");
        require(_stakingFee < 6, "Staking Fee cannot be more than 6%");
    }
 
    function setFeeContract(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _TreasuryFeeReceiver, address _lotteryFeeReceiver, address _burnFeeReceiver, address _treasuryDivWallet ) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        TreasuryFeeReceiver = _TreasuryFeeReceiver;
        lotteryFeeReceiver = _lotteryFeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        treasuryDivWallet = _treasuryDivWallet;
    }
