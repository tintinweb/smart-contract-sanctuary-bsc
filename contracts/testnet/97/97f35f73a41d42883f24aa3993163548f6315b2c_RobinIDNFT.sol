    ...
    RobinNFTStorageV1
{
    ...
    function buyID(address buyer, uint level, uint howmany) public payable returns (uint256[] memory) {
        // 验资
        require(howmany >= 1, "invalid howmany");
        require(level > 0, "invalid level");
        uint256 buyPrice = levelPrices[level];
        require(buyPrice > 0, "price NOT set");
        if (buyToken != IERC20(address(0))) {
            buyToken.transferFrom(_msgSender(), address(this), buyPrice * howmany);
        } else {
    ...
    function mint(uint256 level) public returns (uint256) {
        require(msg.sender == owner() || msg.sender == admin, "must be owner or admin");
