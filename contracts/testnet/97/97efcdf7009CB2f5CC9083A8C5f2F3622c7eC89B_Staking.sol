	...
	function depositBNB(uint256 _pid) public whenNotPaused payable {
		...
		if (userContract != address(0)) {
			...
			if (_ref != address(0)) {
				...
				// add reward f0
				commissionContract.add(_ref, totalUSDDeposit.mul(refReward[0]).div(100).mul(1e18).div(rewardTokenRateUSD));
				address ref = IUser(userContract).getRef(_ref);
				for (uint256 i = 1; i < refReward.length; i++) {
					if (ref != address(0)) {
						// add reward to Fn
						commissionContract.add(ref, totalUSDDeposit.mul(refReward[i]).div(100).mul(1e18).div(rewardTokenRateUSD));
						ref = IUser(userContract).getRef(ref);
					}
				}
			}
