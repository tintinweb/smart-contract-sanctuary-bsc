...
import "./interfaces/IBinancePool_R4.sol";

contract Aggregator is AccessControl, Multicall {
    IBinancePool_R4 public ankrBnb; // 0x3C9205b5d4B312cA7C4d28110C91Fe2c74718a94
    address payable public beneficiary;
    uint256 public protocolFee;

    event ContractCreated(IBinancePool_R4 indexed ankrBnb);
    event StakedBnbInAnkr(address indexed sender, uint256 indexed amount);

    constructor(address _ankrBnb, address payable _beneficiary, uint256 _protocolFee) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        ankrBnb = IBinancePool_R4(_ankrBnb);
        beneficiary = _beneficiary;
        protocolFee = _protocolFee;
        emit ContractCreated(ankrBnb);
    }

    // Stakes BNB and claims aBNBc for the staked BNB.
    // payableAmount (BNB, required) — amount of BNB to be staked.
    function stakeAnkrBnb() external payable {
        uint256 minimumAnkrStake = IBinancePool_R4(ankrBnb).getMinimumStake();
        uint256 relayerFee = IBinancePool_R4(ankrBnb).getRelayerFee();
        uint256 minStakeValue = minimumAnkrStake + relayerFee + protocolFee;
        console.log("Minimum stake value", minStakeValue);
        require(msg.value >= minStakeValue, "Please add more value");
        console.log("require");
        uint256 payableAmount = msg.value - protocolFee;
        console.log("payableAmount");
        IBinancePool_R4(ankrBnb).stakeAndClaimCerts{value: payableAmount};
        bool sent = beneficiary.send(protocolFee);
        require(sent, "Failed to send BNB");
        emit StakedBnbInAnkr(msg.sender, payableAmount);
    }
