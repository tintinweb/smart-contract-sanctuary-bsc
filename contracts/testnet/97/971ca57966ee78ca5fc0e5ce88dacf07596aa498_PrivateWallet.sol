...
// File: contracts/5_PrivateSale.sol
...
contract PrivateWallet is Ownable, ReentrancyGuard {
    ...
    // Whitelist 1 user type1
    // Whitelist 2 user type2
    // Whitelist 3 user type3: @dev, @team
    ...
    function initialize(
        ...
        _startAt = startAt_;
        _firstLockingTime = startAt_ + firstLockingTime_;
        _tge = tge_;
    ...
    function buy(uint256 amount) external onlyAfterStart nonReentrant {
        require(_maxW1 != 0 && _maxW2 != 0, "Not initial");
        require(
        ...
        require(
            isWhiteList1(msg.sender) ||
                ...
                isWhiteList3(msg.sender),
            "Address not whitelist"
        );
        ...
        if (isWhiteList1(msg.sender)) {
            require(amount >= _minW1, "Less than _minW1");
            require(user.balance + amount <= _maxW1, "More than maxium");
        } else if (isWhiteList2(msg.sender)) {
            require(amount >= _minW2, "Less than _minW2");
            require(user.balance + amount <= _maxW2, "More than maxium");
        }
    ...
    function claimTGE() external nonReentrant {
        User storage user = users[msg.sender];
        require(!user.isClaimTGE, "Already Claim TGE");
    ...
    function getUnlocked(address account) public view returns (uint256) {
        ...
        require(block.timestamp >= _firstLockingTime, "Early for unlocking");
        uint256 times = (block.timestamp - _firstLockingTime) / _vestingPeriod;
        // Vesting right after the locking
        times = times + 1;
        require(times > 0, "Earlyfor vesting");
    ...
    function setWhiteList3(address[] calldata users_) external onlyOwner {
        delete whiteList3;
        whiteList3 = users_;
    }
