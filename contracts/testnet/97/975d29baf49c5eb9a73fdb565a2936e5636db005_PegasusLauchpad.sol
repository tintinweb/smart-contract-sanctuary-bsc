...
contract PegasusLauchpad is ReentrancyGuard, OwnableUpgradeable {
    ...
    event Claim(address indexed user, uint256 receiveAmount, uint256 refundAmount);
    ...
    function lock(uint256 _amount) public onlyActive nonReentrant notContract {
        ...
        uint256 _before = getTotalStakeTokenBalance();
        stakeToken.safeTransferFrom(msg.sender, address(this), _amount);
        require(_amount == getTotalStakeTokenBalance() - _before, "invalid amount");
    ...
    function claim() external nonReentrant onlyFinished notContract{
        ...
        if (refundAmount > 0) {
            safeTransferStakeInternal(msg.sender, refundAmount);
        }
        ...
        emit Claim(msg.sender, receiveAmount, refundAmount);
    }
    ...
    function getReceiveAmount(address user) public view returns (uint256) {
        if (totalWeiCap <= totalRaisingWei) {
            return (userInfo[user].amount * totalWeiCap / totalRaisingWei) * rate / 100;
        } else {
            // Return an offering amount equal to a proportion of the raising amount
            return userInfo[user].amount * rate / 100;
        }
    }
    ...
    function getRefundAmount(address user) public view returns (uint256) {
        ...
        uint256 payAmount = userInfo[user].amount * totalWeiCap / totalRaisingWei;
        return userInfo[user].amount - payAmount;
    }
    ...
    function userTokenStatus(address _user) public view returns (uint256 stakeAmount, uint256 refundAmount, uint256 receiveAmount, bool isClaimed) {
        stakeAmount = userInfo[_user].amount;
        refundAmount = getRefundAmount(_user);
        receiveAmount = getReceiveAmount(_user);
        isClaimed = userInfo[_user].claimed;
        return (stakeAmount, refundAmount, receiveAmount, isClaimed);
    }

    function getInfo() public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        // Return ERC20 balance
        return (totalWeiCap, totalRaisingWei, rate, addressList.length, minStakeAmount, maxStakeAmount, startTime, endTime, releaseTime);
    }
    ...
    function finalWithdraw() external onlyFinished onlyOwner {
        uint256 stakeTokenAmountWithdrawable = totalWeiCap;
        uint256 sellingTokenAmountWithdrawable = 0;
        if (totalRaisingWei < totalWeiCap) {
            stakeTokenAmountWithdrawable = totalRaisingWei;
            sellingTokenAmountWithdrawable = (totalWeiCap - totalRaisingWei) * rate / 100;
        }
        safeTransferStakeInternal(reserveWallet, stakeTokenAmountWithdrawable);
        if (sellingTokenAmountWithdrawable > 0) {
            sellingToken.safeTransfer(msg.sender, sellingTokenAmountWithdrawable);
        }
    }
