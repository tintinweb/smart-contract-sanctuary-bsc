...
//contract BatchAuction {
  ...
  // USDT case
  /*
  uint256 private WITHDRAW_CAP_MIN = 2000e18; //  2000 USDT
  uint256 private WITHDRAW_CAP_MAX = 100000e18; // 10000 USDT
  uint256 private WITHDRAW_CAP_LIMIT = 300000e18; //  30000 USDT
  uint256 private WITHDRAW_CAP_INTERCEPT = 2; // 2
  uint256 private WITHDRAW_CAP_INTERCEPT_PLUS = 10000e18; // 10000 USDT
  uint256 private WITHDRAW_CAP_INTERCEPT_DIV = 7; // 7
  */
  // BNB case
  ...
  function withdrawETH(uint256 _amount) external onlyAuctionWindow {
    if (!DEBUG) {
      require(COMMIT_CURRENCY == ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
  ...
  function withdrawERC20Token(uint256 _amount) external onlyAuctionWindow {
    if (!DEBUG) {
      require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
      require(_amount > 0, "INVALID_AMOUNT");
    }
  ...
  */
  ...
  function _withdrawableAmountPerCommitments(uint256 _amount) internal view returns (uint256) {
   ...
   function get1(uint256 _amount) external view returns (uint256) {
    return _amount.mul(WITHDRAW_CAP_INTERCEPT);
  }

    function get2(uint256 _amount) external view returns (uint256) {
    return _amount.mul(WITHDRAW_CAP_INTERCEPT).add(WITHDRAW_CAP_INTERCEPT_PLUS);
  }

    function get3(uint256 _amount) external view returns (uint256) {
    return  (_amount.mul(WITHDRAW_CAP_INTERCEPT).add(WITHDRAW_CAP_INTERCEPT_PLUS)).div(WITHDRAW_CAP_INTERCEPT_DIV);
  }
