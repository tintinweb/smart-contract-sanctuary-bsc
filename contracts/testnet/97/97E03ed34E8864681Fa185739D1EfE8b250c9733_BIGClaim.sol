...
contract BIGClaim is Ownable {
    ...
    event WithdrawnBIG(address _to, uint256 _amount);
    ...
    function claimBIG(
        ...
        BIG_TOKEN.transfer(_to, _amount * 10**18);

        emit ClaimedBIG(_nonce, _to, _amount);
    }

    function withdrawBIG(address _to) external onlyOwner {
        uint256 amount = BIG_TOKEN.balanceOf(address(this));

        BIG_TOKEN.transfer(_to, amount);
        emit WithdrawnBIG(_to, amount);
    }
