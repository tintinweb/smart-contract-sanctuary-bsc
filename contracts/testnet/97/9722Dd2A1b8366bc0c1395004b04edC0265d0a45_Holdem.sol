...
contract SellTime is Ownable{
    ...
    address public MarcoWallet = 0x3A5d7eFf1FF7835d36CB3A370928F177779A2c18;
    address public ClubWallet = 0x65d98663a0AcB3B1B326DdD30E8b254BD6755C6E;
    address public HoldemWallet = 0xBAbD53c6eFc62Fee4417Bf667C8eaE148D42b049;
    uint public chipsPurchaseTime = 1643544000;// 2022/1/30 20:00
    uint public chipsPurchaseEnd = 9999999999;
    
    function setChipsPurchaseTime(
        uint _chipsPurchaseTime,
        uint _chipsPurchaseEnd
    ) external onlyOwner{
        chipsPurchaseTime = _chipsPurchaseTime;
        chipsPurchaseEnd = _chipsPurchaseEnd;
    }

    modifier chipsPurchase(){
        require(now > chipsPurchaseTime && chipsPurchaseEnd > now,
...
contract Holdem is Pausable, SellTime, IChips, Random, ERC1155{

    using Counters for Counters.Counter;
    mapping(address => bool) _buyer;
    mapping(address => bool) _whitelist;

    uint public totalSellAmount = 100000;
    uint public permitSellAmount = 1000;

    mapping (uint => uint) private _nftCount;

    uint public jpgChips = 990;
    uint public gifChips = 10;
    uint public limit = 3;
    //Contracts

    IERC20 public usdtContract = IERC20(0x55d398326f99059fF775485246999027B3197955);

    uint private _priceForUSDT = 150e18; //decimal = 18
    ...
    constructor() public ERC1155("https://nftclubm.com/metadata/holdem/{id}.json") {
        _baseURI = 'https://nftclubm.com/metadata/holdem/';
        _chipsURI[1] = '1.json';
        _chipsURI[2] = '2.json';
        _name = "Marco Holdem";
        _symbol = "MH";
    ...
    function getChipsIndex() private returns(uint){
        ...
        if(randomNumber > (permitSellAmount.sub(gifChips))){
            gifChips = gifChips.sub(1);
            return 2;
        }else{
            jpgChips = jpgChips.sub(1);
            return 1;
        }
    }
    ...
    ) override external whenNotPaused chipsPurchase{
    ...
    function buyChipsBatchByUsdt(
        uint amount
    ) external whenNotPaused chipsPurchase{
...
//_mint
    ...
    function mint(address recipient, uint chipsIndex, uint amount) public onlyOwner{
...
//onlyOwner
    ...
    function setJpgChips(uint amount) external onlyOwner{
        jpgChips = amount;
    }

    function setGifChips(uint amount) external onlyOwner{
        gifChips = amount;
    }
    ...
    function setNewPermitSell(
        uint _Ppermit, uint _jpgChips, uint _gifChips
    ) external onlyOwner{
        require(_Ppermit==(_jpgChips.add(_gifChips)),"numeric error exception");
    ...
    function setHoldemWallet(address _wallet) external onlyOwner{
        HoldemWallet = _wallet;
    }
    ...
    function withdraw() external onlyOwner{
        payable(BitapeWallet).transfer(address(this).balance);
        usdtContract.transfer(BitapeWallet, usdtContract.balanceOf(address(this)));
    }
