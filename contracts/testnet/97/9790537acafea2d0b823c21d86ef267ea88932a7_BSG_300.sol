...
contract BSG_300 is Ownable{
    ...
    function withdraw() external {
        ...
        lastwithdraw[msg.sender] = Total[msg.sender];

        uint256 _withDraw = remainingReward[msg.sender] * maximunwithdraw;
        if(withdrawable > _withDraw)
        {
            withdrawable = _withDraw;
        }
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 2000e18){
            ...
            if(total >= 5000e18 && user.teamNum >= 20 && maxTeam >= 10000e18 && otherTeam >= 10000e18){
                levelNow = 5;
            }else if(user.teamNum >= 10 && maxTeam >= 5000e18 && otherTeam >= 5000e18){
                levelNow = 4;
            }else{
                levelNow = 3;
            }
    ...
    mapping(address => uint256) public withrawamount;
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        Total[_user] = user.totalDeposit.sub(_amount);

        remainingReward[_user] = Total[_user] - lastwithdraw[_user];

        withrawamount[_user] = remainingReward[_user] * maximunwithdraw;
    ...
    function userRewardINFOs(address _user) public view returns(RewardInfo memory){
