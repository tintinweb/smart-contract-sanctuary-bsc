...
/**
    ...
    - 需导入天数和小时数的幂映射数据
 */
contract FullBack is AccessControlUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => bool) public merchants; // todo 重新部署时 调整到members下方
    ...
    function setMerchantDiscount(uint8 discount) external {
        require(discount >= 10 && discount <= 320, "Invail discount");
    ...
    function trade(address merchant, uint256 amount) external {
        ...
        uint256 merchantContribute = (amount * merchantDiscounts[merchant]) / 1000;
        ...
        processContributeAdd(_msgSender(), (merchantContribute * tradeContributeSelfRate) / 1000);
        processContributeAdd(mdao, (merchantContribute * tradeContributeMdaoRate) / 1000);
    ...
    function setMerchant(address merchant, bool result, uint16 discount) external onlyRole(ORACLE_ROLE) {
    ...
    function setTradeContributeMdaoRate(uint16 rate) external onlyRole(OPERATOR_ROLE) {
        tradeContributeMdaoRate = rate;
    }
    ...
    function tradeContributeReward(
        ...
        if (currentMajorLevel > 0) {
            processSpeedUp(self, (merchantContribute * majorLevelDiffRateMemory[currentMajorLevel]) / 1000);
        }
        address parent = parents[self];
        for (uint8 i = 0; i < 50; i++) {
            if (currentMember < 6 && contributes[parent] >= tradeMemberContributeLimit) {
                processContributeAdd(parent, (merchantContribute * tradeRewardRate[currentMember]) / 1000);
                currentMember += 1;
            }
            if (currentAgentLevel < 6 && agentLevels[parent] > currentAgentLevel) {
                ...
                uint256 speedUpAmount = (merchantContribute * diff) / 1000;
            ...
            if (currentMajorLevel < 3 && majorLevels[parent] > currentMajorLevel) {
                ...
                processSpeedUp(parent, (merchantContribute * diff) / 1000);
            }
        ...
        for (uint8 i = 0; i < 50; i++) {
            ...
            if (contributes[parent] > tradeMemberContributeLimit) {
                processContributeAdd(parent, (merchantContribute * tradeMerchantRewardRate[currentMerchant]) / 1000);
                currentMerchant += 1;
            }
            parent = parents[parent];
            if (parent == address(0)) {
    ...
    function testSetContribute(
        address addr,
        uint256 amount,
        uint32 lastTime
    ) external {
        contributes[addr] = amount;
        contributeLastTimes[addr] = lastTime;
    }

    function testSetPointBalance(address addr, uint256 amount) external {
        pointBalances[addr] = amount;
    }

    function testSetMember(address addr, bool result) external {
        members[addr] = result;
    }

    function testClearParent(address addr) external {
        parents[addr] = address(0);
    }

    function testSetMerchantDiscount(address addr, uint8 discount) external {
        merchantDiscounts[addr] = discount;
    }
}
