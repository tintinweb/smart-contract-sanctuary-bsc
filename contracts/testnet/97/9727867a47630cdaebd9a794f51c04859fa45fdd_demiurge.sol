...
contract demiurge is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed("demiurge", "DEMIURGE", uint8(DECIMALS)) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldRebase() && autoRebase && counter <= accumulator) {
            _rebase();
    ...
    function coreRebase(int256 supplyDelta) private returns (uint256) {
        ...
        accumdif = block.timestamp + rebaseFrequency - inception;
