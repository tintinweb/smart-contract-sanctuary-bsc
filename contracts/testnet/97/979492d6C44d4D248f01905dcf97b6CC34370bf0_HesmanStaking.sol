...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun validStakeTime {
        ...
        amountClaim[msg.sender] += interestCalculate(block.number - stakeBlock[msg.sender]);
    ...
    function interestCalculate(uint256 blockNumber) private view returns(uint256 _interest){
        _interest = blockNumber.mul(nftIdList[msg.sender].length).mul(minInterest);
        return _interest;
    }
