...
interface FarmMint {
    function mintToken(address recipient,uint256 amount) external payable;
}
...
contract FarmContract is ERC20, Ownable{
    ...
    function Claim(address to) external{
        stakingInfo memory _staking = stakingList[msg.sender];
        uint256 stakDay = block.timestamp - _staking.stakingTime;//现在时间 - 质押时间
        uint256 lockDay = _staking.lockTime - _staking.stakingTime;//锁定时间-质押时间
        require(lockDay>=stakDay, "LOCK_STAKING_TIME");
        uint256 Reward = _caclClaim(_staking);
    ...
    //mint 代币合约
    function setMintFarm(
        address _mainToken
    ) external onlyOwner {
        Farm = FarmMint(_mainToken);
    }
    //更换质押代币
