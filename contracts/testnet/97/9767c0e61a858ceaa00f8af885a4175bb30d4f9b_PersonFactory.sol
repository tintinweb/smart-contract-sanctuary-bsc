...
contract PersonFactory is  Ownable {
    ...
    function getUserLevel(address _user) public view returns(uint){
        ...
        //     return 1;
        // }
        if(userInfo[_user].payAmount>=2000*oneU&&getValidUser(_user)>=5&&getValidUser(_user)>=2){
            return 2;
        }
        if(userInfo[_user].payAmount>=1000*oneU&&getValidUser(_user)>=2&&getValidUser(_user)>=1){
            return 1;
        }
        return 0;
    }
