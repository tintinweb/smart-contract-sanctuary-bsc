...
contract RealboxTokenTimelock is Ownable {
    ...
    function unlockedFutureIssuing(uint256 _timestamp) public view returns (uint256 _unlockedAmount) {
        uint256 total = totalSupply.mul(200).div(1000);
        if (_timestamp < tge + 1 * 365 days) {
            return 0;
        } else if (_timestamp < tge + 2 * 365 days) {
            return total.mul(10).div(100);
        } else if (_timestamp < tge + 3 * 365 days) {
            return total.mul(20).div(100);
        } else if (_timestamp < tge + 4 * 365 days) {
            return total.mul(35).div(100);
        } else if (_timestamp < tge + 5 * 365 days) {
            return total.mul(50).div(100);
        } else if (_timestamp < tge + 6 * 365 days) {
            return total.mul(70).div(100);
        }
        return total;
    }
