...
contract BaoNFT is ERC721Enumerable, Ownable {
    ...
    IERC20 public GON = IERC20(0x746750cF3092c26ecDBc322D197b9476D71743D3);
    ...
    uint256[] public maxTypeToken = [43, 43, 25];
    uint256[] private currentTypeToken = [0, 0, 0];

    uint256[] private priceBNB = [0.8 ether, 1 ether, 1.2 ether];
    uint256[] private priceTOKEN = [8000 ether, 10000 ether, 12000 ether];
    ...
    function mintCosmicGauntletsWithBNB(uint256 _typeGBox) public payable {
        require(!paused);
        require(currentType[0] + currentType[1] + currentType[2] + currentTypeToken[0] + currentTypeToken[1] + currentTypeToken[2] < maxSupply, "Can not buy more Cosmic Gauntlets.");
    ...
    function breedGauntlets(uint256 _tokenId) external onlyOwner() {
        ...
        require(gauntlet.lastBreed < (block.timestamp - 30 days), "Each NFT can breed 1 gauntlet/30 days");
        uint256 tokenId_minted = totalSupply() + 1;
    ...
    // show 
    function getMaxType() public view returns (uint256[] memory) {
        return maxType;
    }

    function getCurrentType() public view returns (uint256[] memory) {
        return currentType;
    }

    function getMaxTypeToken() public view returns (uint256[] memory) {
        return maxTypeToken;
    }

    function getCurrentTypeToken() public view returns (uint256[] memory) {
        return currentTypeToken;
    }

    function getPriceBNB() public view returns (uint256[] memory) {
        return priceBNB;
    }

    function getPriceToken() public view returns (uint256[] memory) {
        return priceTOKEN;
    }
