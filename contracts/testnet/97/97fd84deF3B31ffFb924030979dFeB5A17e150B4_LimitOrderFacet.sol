...
contract LimitOrderFacet is ILimitOrder {
    ...
    function executeLimitOrder(KeeperExecution[] memory executeOrders) external override {
        ...
        require(ITradingConfig(address(this)).getTradingConfig().executeLimitOrder, "LimitBookFacet: This feature is temporarily disabled");
        ...
        for (UC i = ZERO; i < uc(executeOrders.length); i = i + ONE) {
            ...
            require(ke.price > 0, "LimitBookFacet: price must be greater than 0");

            LimitOrder memory order = los.limitOrders[ke.hash];
            require(order.amountIn > 0, "LimitBookFacet: Order does not exist");

            (bool result, ITradingChecker.Refund refund) = ITradingChecker(address(this)).executeLimitOrderCheck(order, ke.price);
            LibLimitOrder.executeLimitOrder(ke.hash, result, refund);
        }
    }
...
library LibLimitOrder {
    ...
    function executeLimitOrder(
        ...
        if (!result) {
            ...
            uint executionFee = ITradingConfig(address(this)).getTradingConfig().executionFeeUsd * (10 ** token.decimals) / (token.price * 1e10);
