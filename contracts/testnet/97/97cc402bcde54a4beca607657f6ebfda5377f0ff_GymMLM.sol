...
contract GymMLM is OwnableUpgradeable {
        ...
        while (index < directReferralBonuses.length && addressToId[userToReferrer[_user]] != 1) {
            ...
            if (index <= _level && userDepositDollarValue > 0) {
                ...
                _activateCommission = ICommissionActivation(
                    0x6a4B27E70dbe2Cc0Fc23Bf69f300905371aD3b29
     ...
     */
    function seedUsers(address[] memory _users, address[] memory _referrers, uint256[] memory _levels) external onlyOwner {
        require(_users.length == _referrers.length, "Length mismatch");
        for (uint256 i; i < _users.length; i++) {
            _addUser(_users[i], _referrers[i]);
            INetGymStreet(0x61BbE362a483a9BDB1eb13716654ce18581690DE).seedUserMlmLevel(_users[i],_levels[i],true);
    ...
    function _chUMS(address _newAddr,address _oldAddr,address[] memory _partners) private{
        ...
        INetGymStreet(0x61BbE362a483a9BDB1eb13716654ce18581690DE).seedUserMlmLevel(_newAddr,_level,false);
...
interface INetGymStreet {
    ...
    function seedUserMlmLevel(address _user, uint256 _level, bool _isNewPurchaseDate) external returns(bool);
}
