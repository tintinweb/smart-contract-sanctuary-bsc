...
contract FarmingContract is Ownable {
    ...
    function deposit(bool tokenType) public {
        ...
        if(tokenType)
        {
            value.amount = lpUSCTContract.balanceOf(msg.sender);
            lpUSCTContract.transferFrom(msg.sender, address(this), value.amount);
            // tokenType == 1 lp : usct-stb
        } else {
            value.amount = lpUSDCContract.balanceOf(msg.sender);
    ...
    function calcReward() public view returns(uint256 _reward) {
        _reward = 10 ** 18 * user[msg.sender].amount * apyFee / 100 / (60 * 60 * 24 * 365) / (60 * 60 * 24);
        // _reward = user[msg.sender].amount * apyFee / 100;
        return _reward;    
    }
    function calcReward1() public view returns(uint256 _reward) {
        _reward = 10 ** 18 * user[msg.sender].amount * apyFee / 100 / (60 * 60 * 24 * 365);
        // _reward = user[msg.sender].amount * apyFee / 100;
        return _reward;    
    }
    function calcReward2() public view returns(uint256 _reward) {
        _reward = 10 ** 18 * user[msg.sender].amount * apyFee / 100 ;
        // _reward = user[msg.sender].amount * apyFee / 100;
        return _reward;    
    }
