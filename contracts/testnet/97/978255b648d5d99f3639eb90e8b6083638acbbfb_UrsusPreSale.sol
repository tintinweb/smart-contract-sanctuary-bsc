...
contract UrsusPreSale is Ownable{
    ...
    receive() external payable {
        require(msg.value > 0, "Need more than 0 BNB.");
        if(address(msg.sender) != owner()){
            transferURSUS(msg.sender, convertAmount(msg.value));
        }
    }

    function transferURSUS(address to, uint256 amount) private{
        require(amount <= IERC20(coinContract).balanceOf(address(this)), "Not enough URSUS coin left in the contract.");
    ...
    function takeUrsusFunding() onlyOwner public{
    ...
    function takeBNBFunding() onlyOwner public{
