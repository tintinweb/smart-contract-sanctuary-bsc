...
contract PreSale is Ownable {
	...
	IERC20 public buyToken = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); //endereço da token utilizada para comprar
	//possível incluir no constructor para mais flexibilidade
	IERC20 public sellToken; //endereço da token que será vendida
	IERC20 public taxToken = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); //endereço da token para cobrar a taxa
	...
	uint public tax; //valor da taxa da plataforma em wei (10^9)
	uint public totalTokenSales; //Total de tokens para venda
	uint public price; //quantidade de tokens de venda para cada token de compra em wei(10^9)
	uint public minBuy; //valor mínimo em wei (10^9) de compra por wallet em tokens de compra
	uint public maxBuy; //valor máximo em wei (10^9) de compra por wallet em tokens de compra
	...
	constructor(address sellToken_, uint totalTokenSales_, uint minBuy_, uint maxBuy_, uint softcap_, uint hardcap_,
		...
		* minBuy_ -> valor mínimo em wei (10^9) de compra por wallet em tokens de compra
		* maxBuy_ -> valor máximo em wei (10^9) de compra por wallet em tokens de compra
		...
		* price_ ->  quantidade de tokens de venda para cada token de compra em wei(10^9)
		*/
	...
	function depositTokens() external{
		...
		* Esta função faz quem a executa pagar a quantidade definida no constructor em "tax" (em wei 10^9)
		* na token definida como taxToken.
		* Depois ela cobra a quantidade de tokens a serem vendidas usando a fórmula Hardcap/10^9 * preço
		...
		*/
		...
		sellToken.transferFrom(msg.sender, address(this), totalTokenSales);	
	    liquiditySellToken = hardcap/10**9*price*liquidityPercentage/10**4;
	...
	function buyTokens(uint amount) external {
		...
		/*
		* Função que o usuário utiliza para participar do lançamento comprando uma determinada quantidade de tokens em wei (10^9)
		*/
		...
		tokensBought[msg.sender] += amount*price; // atualiza o total de compras em tokens de venda da carteira
	...
	function setPrice(uint price_) external onlyOwner {
		//somente admin
		//estabelece quantidade de tokens de venda para cada token de compra em wei(10^9)
		price = price_;
	}
