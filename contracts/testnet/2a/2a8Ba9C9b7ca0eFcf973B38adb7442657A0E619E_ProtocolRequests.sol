...
contract ProtocolRequests is OwnedAndParsed{
    ...
    struct record{
        ...
        uint numUint;
        mapping(uint => uint) dataUint;

        uint numAdr;
        mapping(uint => address) dataAdr;
    ...
    function getRecord(uint innerId) public view returns (halfRecord memory){
        record storage ptr = Records[innerId];
        halfRecord memory res;
        res.tp = ptr.tp;
        res.chainId = ptr.chainId;
        res.txnHash = ptr.txnHash;
        res.frm = ptr.frm;
        res.numUint = ptr.numUint;
        res.numAdr = ptr.numAdr;
        return res;
    }

    function getRecordUints(uint innerId) public view returns (uint[] memory){
        record storage ptr = Records[innerId];
        uint[] memory res = new uint[](ptr.numUint);
        for(uint i=0; i<ptr.numUint; i++){
            res[i] = ptr.dataUint[i];
        }
        return res;
    }

    function getRecordAdrs(uint innerId) public view returns (address[] memory){
        record storage ptr = Records[innerId];
        address[] memory res = new address[](ptr.numUint);
        for(uint i=0; i<ptr.numUint; i++){
            res[i] = ptr.dataAdr[i];
        }
        return res;
    ...
    function changeToken(IERC20 token) public onlyOwnerOrParser returns (bool){
    	feeToken = token;
    	return true;
    }
    mapping(uint => mapping(uint => uint)) fees;
    function changeFee(uint chain, uint tp, uint price) public onlyOwnerOrParser returns (bool){
    	fees[chain][tp] = price;
    	return true;
    }
    function getFee(uint chain, uint tp) public view returns (uint){
        return fees[chain][tp];
    }

    function submitRequest(uint tp, uint chainId, uint[] memory dataU, address[] memory dataA) public returns (bool){

        require(fees[chainId][tp] != 0, "This chainId and request type is not allowed now");

        Archives.Action[] memory toArch = new Archives.Action[](1);

    	// pay fee
    	toArch[0].a = fees[chainId][tp];
        toArch[0].b = block.chainid;
        toArch[0].z = msg.sender;
        toArch[0].x = archive.getOwner();
        toArch[0].token = feeToken;

    	archive.stackedActions(toArch);

    	record storage ptr = Records[lastOne];

    	ptr.tp = tp;
    	ptr.chainId = chainId;
    	ptr.frm = msg.sender;
    	ptr.numUint = dataU.length;
    	ptr.numAdr = dataA.length;
    	for(uint i=0; i<dataU.length; i++){
    	    ptr.dataUint[i] = dataU[i];
    	}
    	for(uint i=0; i<dataA.length; i++){
    	    ptr.dataAdr[i] = dataA[i];
    	}
    ...
    function pushHash(uint innerId, string memory status) public onlyOwnerOrParser returns (bool){
        Records[innerId].txnHash = status;
        return true;
    }
