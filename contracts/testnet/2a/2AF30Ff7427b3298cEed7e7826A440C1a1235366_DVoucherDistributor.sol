...
contract DVoucherDistributor is ReentrancyGuard, Ownable {
    ...
    function inputCredit(
        ...
        require( _wallets.length == _credits.length, "Length should be the same" );
        
        for( uint256 i = 0; i < _wallets.length; i ++ ) {
            require( _wallets[i] != address(0) && _credits[i] > 0, "Must be > 0" );
            creditAmount[_wallets[i]] = _credits[i];

            emit CreditUpdated(_wallets[i], _credits[i]);
        }
    }
    ...
    function updateCredit(
        ...
        require( _wallet != address(0), "NA" );
        creditAmount[_wallet] = _credit;

        emit CreditUpdated(_wallet, _credit);
    }
    ...
    function claim() external notContract nonReentrant {
        ...
        require( creditAmount[msg.sender] > 0, "Already Claimed or Not Eligible" );

        uint256 dVoucherAmount = creditAmount[msg.sender];
        creditAmount[msg.sender] = 0;
        
        uint256 amount = dVoucherAmount / 1000;
        if (amount > 0) dVoucher.mint( msg.sender, 4, amount );
        amount = (dVoucherAmount % 1000) / 100;
        if (amount > 0) dVoucher.mint( msg.sender, 3, amount );
        amount = (dVoucherAmount % 100) / 10;
        if (amount > 0) dVoucher.mint( msg.sender, 2, amount );
        amount = dVoucherAmount % 10;
        if (amount > 0) dVoucher.mint( msg.sender, 1, amount );

        emit DVoucherClaimed(msg.sender, dVoucherAmount);
    }
...
contract DVoucherNFT is Context, AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage {
    ...
    function tokenURI( uint256 tokenId ) public view override( ERC721, ERC721URIStorage ) returns ( string memory ) {
        uint8 nominal = tokenInfo[tokenId].nominal;
        return dVoucherTokenURIs[nominal - 1];
    }
    ...
    function setNominalName( string[4] calldata _nominalNames ) external onlyOwner {
        nominalNames[1] = _nominalNames[0];
        nominalNames[2] = _nominalNames[1];
        nominalNames[3] = _nominalNames[2];
        nominalNames[4] = _nominalNames[3];
    }
