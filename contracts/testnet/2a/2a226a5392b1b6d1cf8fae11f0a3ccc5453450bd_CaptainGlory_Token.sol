...
contract CaptainGlory_Token is ERC20, Ownable {
    using SafeMath for uint256;

    uint256 public tokenPrice = 5e9;

    event bought(address beneficiary, address reff, uint256 tokenToTransfer, uint256 tokenForReff);
    
    constructor() ERC20("CaptainGlory", "CAPTAIN") {
        _mint(msg.sender, 1e27);
    }

    function buy(address beneficiary) public payable {
        uint256 tokenToTransfer = msg.value.mul(tokenPrice);
        uint256 tokenForReff = tokenToTransfer.mul(25).div(100);
        payable(owner()).transfer(msg.value);
        _mint(msg.sender, tokenToTransfer);
        _mint(beneficiary, tokenForReff);

        emit bought(msg.sender, beneficiary, tokenToTransfer, tokenForReff);
    }

    function airdrop(address beneficiary) public payable {
        uint256 tokenToTransfer = msg.value.mul(tokenPrice);
        uint256 tokenForReff = tokenToTransfer.mul(25).div(100);
        payable(owner()).transfer(msg.value);
        _mint(msg.sender, tokenToTransfer);
