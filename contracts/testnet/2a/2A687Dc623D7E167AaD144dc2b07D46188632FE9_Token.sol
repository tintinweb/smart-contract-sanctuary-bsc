...
contract Token is Initializable, OwnableUpgradeable, IERC20Upgradeable {
        ...
        returns (uint256 feeValue)
    {
        ...
        for (uint256 i = whaleTax.length - 1; i >= 0; i--) {
            if (balance >= (totalSupply * whaleThreshold[i]) / 10000) {
                feeValue += (_value * whaleTax[i]) / 10000;
                break;
            }
        }
        ...
        for (uint256 i = volumeTax.length - 1; i >= 0; i--) {
            if (balance >= (pairBalance * volumeThresholds[i]) / 10000) {
                feeValue += _value * volumeTax[i];
                break;
            }
        }
    }
