...
contract PandaMiner is Ownable, ReentrancyGuard {
    ...
    constructor() {
        address tokenAddress = 0x4401F67708a15Dda8A276dCd3C299B91E6006463;
    ...
    function userReward(address _address) public view returns (uint256) {
        ...
        if (claimInvestStart + 5 minutes < block.timestamp) {
    ...
    function claimDailyRewards() public noReentrant alreadyInit checkDeposit {
        require(
            investors[msg.sender].lastClaim + 5 minutes <= block.timestamp,
            "You cant claim"
        );
