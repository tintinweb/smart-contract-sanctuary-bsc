...
contract BNBM is ERC20, Ownable {
   ...
   // address public immutable BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //BUSD mainnet
    ...
    uint256 public _BuyBUSDRewardsFee = 7;
    uint256 public _BuyLiquidityFee = 1;
    uint256 public _BuyMarketingFee = 2;
    uint256 public _BuyTotalFees = _BuyBUSDRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee);
    
    uint256 public _SellBUSDRewardsFee = 15;
    uint256 public _SellLiquidityFee = 1;
    uint256 public _SellMarketingFee = 2;
    uint256 public _SellTotalFees = _SellBUSDRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee);

    address public _marketingWalletAddress = 0x49478D67E5f9D327dbEf99728c01a8F02F1A998E;
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner{
      ...
      function setBuyFees(uint256 buyBUSDRewardFee, uint256 buyLiquidityFee, uint256 buyMarketingFee) external onlyOwner
    {
        _BuyBUSDRewardsFee = buyBUSDRewardFee;
        _BuyLiquidityFee = buyLiquidityFee;
        _BuyMarketingFee = buyMarketingFee;
        _BuyTotalFees = _BuyBUSDRewardsFee.add(_BuyLiquidityFee).add(_BuyMarketingFee);
        require(_BuyTotalFees >= 0 && _BuyTotalFees <= 20, "Fees are exceeding the limits");
    }

    function setSellFees(uint256 sellBUSDRewardFee, uint256 sellLiquidityFee, uint256 sellMarketingFee) external onlyOwner
    {
        _SellBUSDRewardsFee = sellBUSDRewardFee;
        _SellLiquidityFee = sellLiquidityFee;
        _SellMarketingFee = sellMarketingFee;
        _SellTotalFees = _SellBUSDRewardsFee.add(_SellLiquidityFee).add(_SellMarketingFee);
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "BNBM: gasForProcessing must be between 200,000 and 800,000");
...
contract BNBMDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BNBM_Dividen_Tracker", "BNBM_Dividend_Tracker") {
    	claimWait = 3600;
