...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    address[] public listPairAddress;
    uint256 public indexForPairFee;
    ...
    event RegisterPairWithFee(address pair);
    event UnregisterPairWithFee(address pair);
    event ChangeIndexForPairFee(uint256 newIndex);
        ...
        isRouter(_msgSender())
    {
        ...
        if (takeFee) {
            ...
            if (isPairWithFee[from] || isPairWithFee[to]) {
                tierIndex = indexForPairFee;
            }
        }
    ...
    function changeIndexForPairFee(uint256 _index) public onlyOwner {
        indexForPairFee = _index;
        emit ChangeIndexForPairFee(_index);
    }

    function registerPairWithFee(address[] memory pairs) external onlyOwner {
        uint256 count = pairs.length;
        for (uint256 i = 0; i < count; i++) {
            _registerPairWithFee(pairs[i]);
        }
    }

    function unregisterPairWithFee(address[] memory pairs) external onlyOwner {
        uint256 count = pairs.length;
        for (uint256 i = 0; i < count; i++) {
            _unregisterPairWithFee(pairs[i]);
        }
    }

    function _registerPairWithFee(address _pair) private {
        require(isPairWithFee[_pair] == false, "Safemoon: added before");

        isPairWithFee[_pair] = true;
        listPairAddress.push(_pair);
        emit RegisterPairWithFee(_pair);
    }

    function _unregisterPairWithFee(address _pair) private {
        require(isPairWithFee[_pair] == true, "Safemoon: removed before");

        isPairWithFee[_pair] = false;
        uint256 listPairLength = listPairAddress.length;
        for (uint256 i = 0; i < listPairLength; i++) {
            if (listPairAddress[i] == _pair) {
                _deletePairAddress(i, listPairLength - 1);
                break;
            }
        }
        emit UnregisterPairWithFee(_pair);
    }

    function _deletePairAddress(uint256 x, uint256 y) private {
        if (x == y) {
            return;
        }
        listPairAddress[x] = listPairAddress[y];
        listPairAddress.pop();
    }
}
