...
contract MeToken is Ownable, IERC20 {
    ...
    uint8 public splitTimesPerTran = 5;//每次分5个
    uint256 public currentSplitIndex;
    uint256 public waitLPHolderTokenNum;//等待分红的ME
    uint256 public waitSellTokenNum;//等待卖出ME
    ...
    function init() private onlyOwner {
        ...
        _feeRate = 10000;
        numTokensToAddToLiquidity = 1000 ether;
        swapTokensAtAmount = 1000 ether;

        _token_trusteeship = address(0xb3eA4141585D2fD6C19C19512f77c5bD39c3F2ab);
        _security_fund = address(0x156225AEB51F0CfaddE62e958e8378a6BDB3DeE4);//保障基金合约
    ...
    function setupLP() private onlyOwner {
        ...
        else if (block.chainid == 97) // bsc testnet
        {
            ...
            //usdt = IERC20(0x09Fdb9D9a28155ce73ABbe43c504B6AEe7a53F1a);测试用
            usdt = IERC20(0x0B7E566590D26Ec368e08e23cEa6B18e618f541c);//测试用
        }
    ...
    function _transferWithSellFee(address from, address to, uint256 amount) internal {
        ...
        _basicTransfer(from, address(this), amountA);
        waitSellTokenNum += amountA;
        if(waitSellTokenNum > 0){
            tokenTrustenshipTransfer(waitSellTokenNum);//币值管理
        }
        _burn(from,amountB);
        _basicTransfer(from, _security_fund, amountC);
    ...
    //币值管理 转换成USDT再转出
    function tokenTrustenshipTransfer(uint256 tokenAmount) private{
        waitSellTokenNum = 0;
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(
            fromBalance >= amount,
            "ERC20: burn transfer amount exceeds balance"
        );
    ...
    function _transfer(
        ...
        if(currentSplitIndex > 0 || balanceOf(address(this)) >= swapTokensAtAmount){
            if(currentSplitIndex == 0){
                waitLPHolderTokenNum = swapTokensAtAmount;
            }
            splitLPHolderToken();
        } else {
            clearBuyUser();
