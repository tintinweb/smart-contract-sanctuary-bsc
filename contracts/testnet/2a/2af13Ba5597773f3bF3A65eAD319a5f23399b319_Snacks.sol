...
contract Snacks is SnacksBase {
    ...
    uint256 public constant FEE_PULSE = 35;
    uint256 public constant D = 1000000;
    uint256 public constant FEE_POOL_REWARD_DISTRIBUTOR = 45;
        ...
        override
    {
        ...
        if (feeBtcSnacks > 0 && totalHoldersBalance_ > 0) {
            ...
            for (uint256 i = 0; i < holdersListLength; i++) {
                address holder = _holderList.at(i);
                uint256 holderFee = (feeBtcSnacks * balanceOf(holder)) /
                    totalHoldersBalance_;
                uint256 btcBalance = btcSnacks.balanceOf(address(this));
                uint256 amount = (btcBalance >= holderFee)
                    ? holderFee
                    : btcBalance;
                btcSnacks.safeTransfer(holder, amount);
            }
        }
        ...
        if (
            feeEthSnacks > 0 &&
            totalHoldersBalance_ > 0 &&
            holdersListLength > 0
        ) {
            // Calculate and give fee of btc snacks to every holder
            for (uint256 i = 0; i < holdersListLength; i++) {
                address holder = _holderList.at(i);
                uint256 holderFee = (feeEthSnacks * balanceOf(holder)) /
                    totalHoldersBalance_;
                uint256 ethBalance = ethSnacks.balanceOf(address(this));
                uint256 amount = (ethBalance >= holderFee)
                    ? holderFee
                    : ethBalance;
                ethSnacks.safeTransfer(holder, amount);
            }
        }
    }
