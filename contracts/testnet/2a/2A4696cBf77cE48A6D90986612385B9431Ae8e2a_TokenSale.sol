...
contract TokenSale is Ownable, Pausable {
    ...
    event Burned(address currency, uint256 amount);
    ...
    function buy(address currency, uint256 amount) external whenNotPaused {
        ...
        _token.safeTransfer(_msgSender(), count);

        emit Bought(currency, count);
    }

    function adminSetCurrency(address currency, uint256 amount) external onlyOwner {
        _currencies[currency] = amount;

        emit CurrencySet(currency, amount);
    }

    function adminWithdraw(address currency, address target, uint256 amount) external onlyOwner whenPaused {
        IERC20(currency).safeTransfer(target, amount);
    }

    function adminBurn(uint256 amount) external onlyOwner whenPaused {
        _token.transfer(DEAD, amount);

        emit Burned(DEAD, amount);
    }

    function adminPause() public virtual onlyOwner {
        _pause();
    }

    function adminUnpause() public virtual onlyOwner {
        _unpause();
    }
