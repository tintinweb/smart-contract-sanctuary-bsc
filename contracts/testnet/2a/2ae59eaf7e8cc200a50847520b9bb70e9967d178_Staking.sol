...
contract Staking is Ownable {
    ...
    uint256 public endPool = 0;
    uint256 public timeEndPool = 0;
    ...
    mapping(address => uint256) public userStakingState;
    mapping(address => uint256) public userAwardClaimed;
    ...
    constructor(
        ...
        maxPool = _maxPool * 10**18;
        minStaking = _minStaking * 10**18;
        adminlist[msg.sender] = 1;
    }
...
Event
 -----------------------------------------------------------------*/
    ...
    event Staked(address receiver, uint256 amount);
    event Unstaked(address receiver, uint256 amount);
    event AddedBlacklist(address account);
    event AddedBatchBlacklist(address[] accounts);
    ...
    event ChangedPoolStatus(uint256 status);
...
Stake
 -----------------------------------------------------------------*/
    function Stake(uint256 amount) public {
        require(poolStatus == 1, "Not time to stake!");
        require(endPool == 0, "Pool closed!");
        require(userStakingState[msg.sender] != 1, "You have already staked!");
        require(totalPool + amount <= maxPool, "Exceed the total pool limit!");
        require(!isBlackList(msg.sender), "Revert blacklist!");
        require(amount >= minStaking, "Not enough token for staking!");

        MATK.transferFrom(msg.sender, address(this), amount);
        totalPool += amount;
        if (userStakingState[msg.sender] != 2) {
            userAccount[numOfAccount] = msg.sender;
            numOfAccount = numOfAccount + 1;
        }
        userAmount[msg.sender] = amount;
        userStakeTime[msg.sender] = block.timestamp;
        userStakingState[msg.sender] = 1;
        userAwardClaimed[msg.sender] = 0;
...
Admin Stake
 -----------------------------------------------------------------*/
    ...
    function AdminStakeForUser(address receiver, uint256 amount)
        public
        onlyAdmin
    {
        require(poolStatus == 1, "Not time to stake!");
        require(userStakingState[receiver] != 1, "You have already staked!");
        require(totalPool + amount <= maxPool, "Exceed the total pool limit!");
        require(!isBlackList(receiver), "Revert blacklist!");
        require(amount >= minStaking, "Not enough token for staking!");

        MATK.transferFrom(msg.sender, address(this), amount);
        totalPool += amount;
        if (userStakingState[receiver] != 2) {
            userAccount[numOfAccount] = msg.sender;
            numOfAccount = numOfAccount + 1;
        }
        userAmount[receiver] = amount;
        userStakeTime[receiver] = block.timestamp;
        userStakingState[receiver] = 1;
        userAwardClaimed[receiver] = 0;
        emit Staked(receiver, amount);
    }

    /*----------------------------------------------------------------
Admin Stake Not Transfer
 -----------------------------------------------------------------*/

    function AdminStakeBackup(address receiver, uint256 amount)
        public
        onlyAdmin
    {
        require(poolStatus == 1, "Not time to stake!");
...
Admin Stake
 -----------------------------------------------------------------*/
    ...
    function AdminStakeForUserWithTime(
        address receiver,
        uint256 amount,
        uint256 stakeTime
    ) public onlyAdmin {
        ...
        userAmount[receiver] = amount;
        userStakeTime[receiver] = stakeTime;
        userStakingState[receiver] = 1;
        userAwardClaimed[receiver] = 0;
        emit Staked(receiver, amount);
    }
...
Admin Stake Not Transfer
 -----------------------------------------------------------------*/
    ...
    function AdminStakeBackupWithTime(
        address receiver,
        uint256 amount,
        uint256 stakeTime
    ) public onlyAdmin {
...
Award
 -----------------------------------------------------------------*/
    function awardCalculation(address receiver, uint256 totalDay)
        public
        view
        returns (uint256)
    {
        uint256 award = ((userAmount[receiver] * stakingRate * totalDay) /365) / 10000;
        return award;
    }

    function getClaimableAward(address receiver) public view returns (uint256) {
        uint256 userEndTime = 0;
        if (endPool == 0) {
            userEndTime = block.timestamp;
        } else {
            userEndTime = timeEndPool;
        }
        // uint256 totalDay = (userEndTime-userStakeTime[receiver])/86400;
        uint256 totalDay = (userEndTime - userStakeTime[receiver]) / 60;
        uint256 award = awardCalculation(receiver, totalDay);
        return award - userAwardClaimed[receiver];
    }

    function claimAwards() public {
        address receiver = _msgSender();
        uint256 claimableAward = getClaimableAward(receiver);
        require(userStakingState[receiver] == 1, "You have not staked yet!");
        require(claimableAward > 0, "Claimable Award is zero");

        MATK.transfer(receiver, claimableAward);
        userAwardClaimed[receiver] += claimableAward;
        emit TokensClaimed(receiver, claimableAward);
    }
...
Unstake
 -----------------------------------------------------------------*/
    function unstake() public {
        ...
        require(userStakingState[receiver] == 1, "You have not staked yet!");
        require(
            block.timestamp > userStakeTime[receiver] + lockTime,
            "It's not time to unstake!"
        );

        uint256 claimAmount = getClaimableAward(receiver);
        uint256 transfer = userAmount[receiver] + claimAmount;
        MATK.transfer(receiver, transfer);
        totalPool -= userAmount[receiver];
        userStakingState[receiver] = 2;
        userAmount[receiver] = 0;
        emit Unstaked(msg.sender, transfer);
    }

    /*----------------------------------------------------------------
Remove from staking list
 -----------------------------------------------------------------*/
    function removeStaking(address receiver) public onlyAdmin {
        require(!isBlackList(msg.sender), "Revert blacklist!");
        require(userStakingState[receiver] == 1, "You have not staked yet!");
 ...
 -----------------------------------------------------------------*/
    ...
    /*----------------------------------------------------------------
Users check their staking amount and award amount
 -----------------------------------------------------------------*/
    function userCheckStakingAmount() external view returns (uint256) {
        uint256 balance = userAmount[msg.sender];
        return balance;
    }

    function userCheckAward() external view returns (uint256) {
        uint256 userEndTime = 0;
        if (endPool == 0) {
            userEndTime = block.timestamp;
        } else {
        ...
        // uint256 totalDay = (userEndTime-userStakeTime[msg.sender])/86400;
        uint256 totalDay = (userEndTime - userStakeTime[msg.sender]) / 60;
        return awardCalculation(msg.sender, totalDay);
    }

    function userCheckClaimableAward() external view returns (uint256) {
        return getClaimableAward(msg.sender);
    }

    /*----------------------------------------------------------------
Users check their staking status
 -----------------------------------------------------------------*/
    function userCheckStakingState() external view returns (uint256) {
        return userStakingState[msg.sender];
    }

    /*----------------------------------------------------------------
Users check staking time
 -----------------------------------------------------------------*/
    function userCheckStakeTime() external view returns (uint256) {
        return userStakeTime[msg.sender];
    }

    /*----------------------------------------------------------------
Admin checks user staking amount and award amount
 -----------------------------------------------------------------*/
    function adminCheckStakingAmount(address user)
        external
        view
        onlyAdmin
        returns (uint256)
    {
        uint256 balance = userAmount[user];
        return balance;
    }

    function adminCheckAward(address user)
        external
        view
        onlyAdmin
        returns (uint256)
    {
        ...
        // uint256 totalDay = (userEndTime-userStakeTime[user])/86400;
        uint256 totalDay = (userEndTime - userStakeTime[user]) / 60;
        return awardCalculation(user, totalDay);
    }

    /*----------------------------------------------------------------
Check Pool
 -----------------------------------------------------------------*/
    function checkTotalPool() external view returns (uint256) {
        return totalPool;
    }

    function checkMaxPool() external view returns (uint256) {
        return maxPool;
    }
...
Change Pool Status
 -----------------------------------------------------------------*/
    function changePoolStatus(uint256 status) public onlyAdmin {
        poolStatus = status;
        emit ChangedPoolStatus(status);
    }

    function EndPool() public onlyAdmin {
        endPool = 1;
        timeEndPool = block.timestamp;
    }

    /*----------------------------------------------------------------
Check Pool Status
 -----------------------------------------------------------------*/
    function checkPoolStatus() external view returns (uint256) {
        return poolStatus;
    }
 ...
 -----------------------------------------------------------------*/
    ...
    function addBatchToBlacklist(address[] memory accounts) public onlyOwner {
        ...
        emit AddedBatchBlacklist(accounts);
        (accounts);
    }
