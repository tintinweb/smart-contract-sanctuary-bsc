...
contract Peach is PeachMathematician, Decorated {
    ...
    function faucet(address _target) external {
        require(
            lastMinted[_target] < block.timestamp - 1 days,
            "You can only do this once a day."
        );
        peachStorage.mint(_target, 1000 * 10**_decimals);
    }

    function getMintableTime(address _target) external view returns (uint256) {
        return lastMinted[_target] + 1 days;
    }
