...
contract Solmit is IBEP20, Auth {
    ...
    // presale
    //
    mapping (address => mapping (uint256 => uint256)) usersBought;
    ...
    constructor () Auth(msg.sender) {
        ...
        roundsPrices = [0 , 100 , 67 , 50 , 40 , 33 , 0 , 0];
    ...
    function startPresale() external onlyOwner{
        ...
        unlockData.push([0 , 0]);
        unlockData.push([currentTime + (roundsPeriod * 6) , 25]);
        unlockData.push([currentTime + (roundsPeriod * 7) , 50]);
        unlockData.push([currentTime + (roundsPeriod * 8) , 75]);
        unlockData.push([currentTime + (roundsPeriod * 9) , 100]);
    ...
    function getRoundMaxAmount(uint256 _round) public view returns(uint256){
        require(_round != 0 && _round <= 7 , "wrong round");
