    ...
    function buy(uint _product) public payable {
      ...
      if (_parent != _referal) {
        if ((msg.value - minCost) != 0) {
          _to.transfer((msg.value - minCost) * _levels * _reward / 10000); // Send parent reward
        }
        newReferal.transfer(minCost * _levels * _reward / 1000); // Send active parent reward
      } else {
        _to.transfer(msg.value * _levels * _reward / 10000); // Send parent reward
      }
