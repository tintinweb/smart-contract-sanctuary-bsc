...
contract CeresStakingCRS is CeresStakingRewards, IMintReceiver, Ownable {
    ...
    function reinvestMint() public override nonReentrant updateMint(msg.sender) {
        ...
        if (mint > 0) {
            ...
            ICeresStaking(stakingASC).notifyReinvest(msg.sender, mint);
        }
    }
    ...
    function notifyMint(uint256 amountMint, uint256 amountBurn) public override onlyStakings {
        ...
        require(crs.balanceOf(address(this)) >= leftStaking, "Staking left balance is not enough!");
    ...
    function notifyReinvest(address account, uint256 amount) external override updateMint(account) updateReward(account) onlyStakings {
        require(_totalStaking + amount <= crs.balanceOf(address(this)), "Notify amount exceeds balance!");
