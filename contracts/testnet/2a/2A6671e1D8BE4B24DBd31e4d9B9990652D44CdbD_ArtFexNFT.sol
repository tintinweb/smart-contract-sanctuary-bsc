...
contract ArtFexNFT is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => string) _UriTokens;
    ...
    constructor(
        ...
        _TransferCost = _transfer_cost;
        _transaction_fee = _transactFee;
        _owner = Owner_;
        _withdOwner = Owner_;
        setBaseURI(_initBaseURI);
        mint(msg.sender, "bafybeihy3wuwxpldh27shubfvl4klzbjioyhgv6hn3gmqyvud6pr6symiq");
    }
    ...
    // public
    function mint(address _to, string memory _uriToken) public payable isPausable(){
        uint256 supply = totalSupply();
        uint256 newId = supply + 1;

        if (msg.sender != owner()) {
            if(whitelisted[msg.sender] != true) {
                require(msg.value >= _costMintValue);
            }
        }
        _safeMint(_to, newId);
    ...
    // public
    function multMint(ReceiversMint[] memory _mintAmount) public payable isPausable(){
        uint256 supply = totalSupply();
        require(_mintAmount.length > 0);
        require(_mintAmount.length <= maxMintAmount);
        require(supply + _mintAmount.length <= maxSupply);

        if (msg.sender != owner()) {
            if(whitelisted[msg.sender] != true) {
                require(msg.value >= _costMintValue * _mintAmount.length);
            }
        }
    ...
    function transferFrom(
        ...
        if (msg.sender != owner() && !inTransfer) {
            if(!whitelisted[msg.sender]) {
                require(msg.value >= _transfer_cost, "it is necessary to add the shipping fee.");
            }
        }
    ...
    function _setUriToken(uint256 tokenId, string memory _UriToken) private {
