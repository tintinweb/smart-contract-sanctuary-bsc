    ...
    function zapOut(address fromToken, address toToken, uint256 amountIn, uint256 amountOutMin) external whenNotPaused {
        ...
        if (!isLpToken(fromToken)) {
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                if (token != toToken) {
                    if (toToken == WBNB) {
                        amountOut = _swapTokenForBNB(token, amountToken, address(this));
                    } else {
                ...
                if (token0 != toToken) {
                    if (toToken == WBNB) {
                        amountOut =_swapTokenForBNB(token0, amountA, address(this));
                    } else {
                ...
                if (token1 != toToken) {
                    if (toToken == WBNB) {
                        amountOut = amountOut.add(_swapTokenForBNB(token1, amountB, address(this)));
                    } else {
