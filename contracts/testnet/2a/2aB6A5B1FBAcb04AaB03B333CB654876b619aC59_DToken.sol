...
contract DToken is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    uint256 public globalMultiplier = 1500000000000000000000;
    ...
    // normal transaction
        ...
        // tx limits
            ...
            //sell
                ...
                bool canSell = updateMultiplier();

                // marketing auto-bnb
                require(canSell,"Selling price target is not reached.");
                if (canSell && balanceOf(pancakeV2Pair) > 0) {
    ...
    function updateMultiplier() public returns (bool allow) {
      if(isMultiEnabled){
        uint256 buyprice = buyPriceNow();
        uint256 sellprice = sellPriceNow();
        uint256 _tokenCount = tokenCount.current();
        uint256 roofprice=checkMultiplier().mul(sellPrice);

         if(basePrice <= buyprice){
          allow = false;
        }
        if(roofprice <= sellprice  && sellPrice<=sellprice){
          lastMultiplierData[(tokenCount.current() == 0 ? 1 : _tokenCount)].lastMulti = wpow(globalMultiplier.div(denominatorMultiplier),(tokenCount.current() == 0 ? 1:_tokenCount));
          tokenCount.increment();
          lastMultiplierData[(tokenCount.current() == 0 ? 1 : _tokenCount)].currentMulti = wpow(globalMultiplier.div(denominatorMultiplier),(tokenCount.current() == 0 ? 1 : _tokenCount));
          editBasePrice(basePrice,checkMultiplier().mul(basePrice));
          editSellPrice(sellPrice,checkMultiplier().mul(sellPrice));
          // basePrice = (wpow(globalMultiplier.div(denominatorMultiplier),(tokenCount.current() == 0 ? 1 : _tokenCount))).mul(basePrice);
          // sellPrice = (wpow(globalMultiplier.div(denominatorMultiplier),(tokenCount.current() == 0 ? 1 : _tokenCount))).mul(sellPrice);
          allow = true;
        }
    ...
    function checkMultiplier() public view returns (uint256) {
      if(isMultiEnabled){
        return readPowerResult(globalMultiplier.div(denominatorMultiplier),readMultiplierLength()).div(WAD);
      }
    }
    // Check User Balance
    ...
    //   uint256 roofprice=checkMultiplier().mul(sellPrice);
      if(isMultiEnabled){
        if(_soldAmount < initialBalance.mul(minPercent).div(1000) && roofprice <= sellprice && sellPrice<=sellprice){
          return true;
        }
      }
      return false;
    ...
    // Set Excluded From Fees users
    function readPower() public view returns(uint256 mul1,uint256 mul2,bool _status1,bool _status2) {
        uint256 price = sellPriceNow();
        mul1 = wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()));
        mul2 = buyPriceNow();
        _status1 = (price == basePrice && readMultiplierLength() == 1);
        _status2 = (price <= (wpow(globalMultiplier.div(denominatorMultiplier),(readMultiplierLength()))).mul(basePrice));
    }
    function readPowerResult(uint256 x, uint256 y)public view returns (uint256 z){
        z = wpow(x,y);
    }
    // Airdrop Function
