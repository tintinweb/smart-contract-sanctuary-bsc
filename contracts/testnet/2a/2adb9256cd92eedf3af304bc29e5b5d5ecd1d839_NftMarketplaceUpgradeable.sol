...
contract NftMarketplaceUpgradeable is Initializable, ReentrancyGuardUpgradeable, IERC721Receiver {
    ...
    function initialize(address _feeReceiver, uint256 _fee) public initializer {
    ...
    function purchaseNft(uint256 _orderId) external payable nonReentrant {
        ...
        payable(listedItem.seller).transfer(msg.value - _fee);
    ...
    function ownerOf(address _nftAddress, uint256 _tokenId) public view returns (address) {
        return IERC721(_nftAddress).ownerOf(_tokenId);
    }

    function ownerOf2(address _nftAddress, uint256 _tokenId) public view returns (address) {
        return IERC721(_nftAddress).ownerOf(_tokenId);
    }

    function isListed(uint256 _orderId) public view returns (bool) {
        uint256 orderBookLength = orderBook.length;
        if(orderBookLength == 0 || _orderId >= orderBookLength)
            return false;

        OrderBook memory listedItem = orderBook[_orderId];
