    ...
    */
    ...
    function poolprofit(address _customerAddress) public view returns(uint256){
        ...
        for	(uint256 y=1; y<= referralCount_[_customerAddress]; y++)
		{
            ...
            uint256 dayscount                   = SafeMath.div(timediff, 60); //86400 Sec for 1 Day
            uint256 roiPercent                  = SafeMath.mul(dayscount, stakePer_);
            uint256 downlineroi                 = SafeMath.percent(stakeBalanceLedger_[downline],roiPercent,100,18);
            uint256 poolpercentage              = SafeMath.div(POOL_PERCENTS[level-1],PERCENTS_DIVIDER);
            uint256 poolshare                   = SafeMath.mul(downlineroi,poolpercentage);
            totalPoolAmount                     = SafeMath.add(totalPoolAmount,poolshare);
        }
