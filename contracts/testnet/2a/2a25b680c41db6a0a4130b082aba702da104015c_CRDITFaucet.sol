...
contract CRDITFaucet is Ownable {
    ...
    function testMint1() public returns(uint256) {
        crdit.issuerMint(_msgSender(), faucetAmount);
        return faucetAmount;
    }

    function testMint2() public returns(uint256) {
        crdit.issuerMint(_msgSender(), faucetAmount);
        emit MintedCRDIT(faucetAmount);
        return faucetAmount;
    }

    function testMint3(address _agent) public returns(uint256) {
        crdit.issuerMint(_msgSender(), faucetAmount);
    ...
    function testMint4(address _agent) public returns(uint256) {
        require(block.timestamp > addressToTime[_msgSender()],"24 hours have not passed.");
    ...
    function nelFaucet() public returns(bool) {
        require(block.timestamp > addressToTime[_msgSender()], "Still in the cooldown time.");
        addressToTime[_msgSender()] = block.timestamp + 1 days;
        ICRDIT token = ICRDIT(CRDITAddress);
    ...
    function nelFaucet2() public returns(bool) {
        require(block.timestamp > addressToTime[_msgSender()], "Still in the cooldown time.");
        addressToTime[_msgSender()] = block.timestamp + 1 days;
        bool value = crdit.issuerMint(_msgSender(), faucetAmount);
        if (value == true) {
            emit MintToken(_msgSender(), faucetAmount);
        } else {
            emit MintToken(_msgSender(), 0);
    ...
    function nelFaucet3() public returns(bool) {
        require(block.timestamp > addressToTime[_msgSender()], "Still in the cooldown time.");
        require(crdit.mintLimitOf(address(this)) > faucetAmount + agentRewards, "This contract has reached its mint limit");
        addressToTime[_msgSender()] = block.timestamp + 1 days;
        bool value = crdit.issuerMint(_msgSender(), faucetAmount);
