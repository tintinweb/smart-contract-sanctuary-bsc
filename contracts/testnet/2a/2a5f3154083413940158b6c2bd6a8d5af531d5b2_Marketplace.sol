...
interface IMarketplace {
    ...
    struct Order {
        ...
        address nftAddress;
        // palPrice (in wei) for the published item
        uint256 palPrice;
        uint256 nagPrice;
    }
    ...
    event OrderCreated(
        ...
        uint256 palPriceInWei,
        uint256 nagPriceInWei
    );
...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
	...
	address public nftAddress  = 0x9CaA9769ee1006fdB0945154961F01184c0b062B;
    ...
    function createOrder(
        uint256 _assetId,
        uint256 _palPriceInWei,
        uint256 _nagPriceInWei
    )
        public whenNotPaused
    {   
        _createOrder(nftAddress, _assetId, _palPriceInWei,_nagPriceInWei);
    }
        ...
        public whenNotPaused
    {
        ...
        require(_palPriceInWei > 0, "Marketplace: palPrice should be bigger than 0");


        orderByAssetId[nftAddress][_assetId].palPrice = _palPriceInWei;
        orderByAssetId[nftAddress][_assetId].nagPrice = _nagPriceInWei;
        

        emit OrderUpdated(orderByAssetId[nftAddress][_assetId].id, _palPriceInWei,_nagPriceInWei);
    }
        ...
        internal
    {
        ...
        orderByAssetId[_nftAddress][_assetId] = Order({
            ...
            palPrice: _palPriceInWei,
            nagPrice:_nagPriceInWei
        ...
        if(nftInfo.level < sellMinLevel){
            require(isExcluded[msg.sender],"Marketplace: No permission to sell");
        }	
    ...
    function setNagTaxAddress(address _nagTaxAddress) public onlyOwner{
        nagTaxAddress = _nagTaxAddress;
    }

    function setSellMinLevel(uint256 _sellMinLevel) public onlyOwner{
        sellMinLevel = _sellMinLevel;
    }

    function seExcluded(address user, bool status) public onlyOwner{
        isExcluded[user] = status;
    }
}