...
//    gameMiningPool = IFOTAToken(0x0A4E1BdFA75292A98C15870AeF24bd94BFFe0Bd4); // TODO use this on mainnet deploy
  ...
  function deposit(uint _amount, PaymentCurrency _paymentCurrency) external {
    ...
    if(_paymentCurrency == PaymentCurrency.fota) {
      _takeFund(_convertUsdToFota(fundAmountInDollar) * (oneHundredPercentageDecimal3 - discounts[0]) / oneHundredPercentageDecimal3, _paymentCurrency);
    } else if(_paymentCurrency == PaymentCurrency.busd) {
      _takeFund(fundAmountInDollar  * (oneHundredPercentageDecimal3 - discounts[1]) / oneHundredPercentageDecimal3, _paymentCurrency);
    } else if(_paymentCurrency == PaymentCurrency.usdt) {
      _takeFund(fundAmountInDollar  * (oneHundredPercentageDecimal3 - discounts[2]) / oneHundredPercentageDecimal3, _paymentCurrency);
    } else {
      require(address(otherPayment) != address(0), "RewardManager: not supported for now");
      _takeFund(fundAmountInDollar  * (oneHundredPercentageDecimal3 - discounts[3]) / oneHundredPercentageDecimal3, _paymentCurrency);
    }
  ...
  function setContracts(address _heroNft, address _fotaToken, address _landLordManager, address _farmAddress, address _gameProxyContract, address _fotaPricer) external onlyMainAdmin {
