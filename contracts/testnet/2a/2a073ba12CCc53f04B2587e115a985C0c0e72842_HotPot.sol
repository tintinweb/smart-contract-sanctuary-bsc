...
contract HotPot is Ownable, ReentrancyGuard {
    ...
    IPancakeRouter01 public rewardConvertor;
    ...
    function setRewardConvertor(address _rewardConvertor) external onlyOwner {
        rewardConvertor = IPancakeRouter01(_rewardConvertor);
        emit NewRewardConvertor(_rewardConvertor);
    }

    /**
     * Exchange WBNB for BNB before repurchasing Sesame from the open market
     */
    function unwrapBNB(uint256 amount) external nonReentrant onlyOwner {
        IWBNB(rewardToken).withdraw(amount);
    }
    ...
    function repurchase(
        ...
        path[0] = rewardConvertor.WETH();
        path[1] = sesameToken;
        uint256[] memory amounts = rewardConvertor.swapExactETHForTokens{
            value: amountToSell
        }(amountToReceive, path, address(this), block.timestamp + timeout);
        emit RepurchaseSesame(amounts[0], amounts[1]);
    }
