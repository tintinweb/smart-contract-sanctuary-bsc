...
contract UniverzMarketplace is ERC721Holder, ReentrancyGuard, AccessControl {
    ...
    bytes32 public constant CREATOR_CONTRACT = keccak256("CREATOR_CONTRACT");

    constructor(address _creationContract) {
        _grantRole(CREATOR_CONTRACT, _creationContract);
    }
    ...
    function addToSell(
        ...
        _addToSell(_tokenAddress, msg.sender, _id, _priceInWei);

        IERC721(_tokenAddress).safeTransferFrom(msg.sender, address(this), _id);
    }

    function addToSellAfterCreation(
        address _tokenAddress,
        address _owner,
        uint256 _id,
        uint256 _priceInWei
    ) external {
        _addToSell(_tokenAddress, _owner, _id, _priceInWei);
    }
    ...
    function _addToSell(
        ...
        bytes32 itemId = encode(_owner, itemsCount[msg.sender]);
        ...
        items[itemId] = MarketItem(
            ...
            _priceInWei,
            _owner, //seller
            IERC721(_tokenAddress).ownerOf(_id), //owner
