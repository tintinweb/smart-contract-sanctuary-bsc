...
contract YINLaunchGaller is AccessControl, ERC721 {
            ...
            revert(
                string(
                    abi.encodePacked(
                        ...
                        Strings.toString(uint160(_msgSender())),
                        " is missing role ",
    ...
    constructor(
        ...
        _setupRole(MINTER_ROLE, _launchpad);
    }
    ...
    function setLaunchMaxSupply(uint256 _launchMaxSupply)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LAUNCH_MAX_SUPPLY = _launchMaxSupply;
    ...
    function mintTo(address to, uint256 size) external onlyRole(MINTER_ROLE) {
        require(
            to != address(0) && !to.isContract(),
            "YINLaunchGaller: can't mint to empty address and can't mint to contract"
        );
        require(size > 0, "YINLaunchGaller: size must greater than zero");
        require(
            LAUNCH_SUPPLY + size <= LAUNCH_MAX_SUPPLY,
            "YINLaunchGaller: max supply reached"
        );

        for (uint256 idx = 0; idx < size; idx++) {
            LAUNCH_SUPPLY = LAUNCH_SUPPLY + 1;
