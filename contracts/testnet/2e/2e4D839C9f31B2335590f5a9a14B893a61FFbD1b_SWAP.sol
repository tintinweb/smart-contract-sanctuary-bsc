    ...
    ShopContract
{
    ...
    mapping(uint256 => uint8) private _shopOrders;
    ...
    function getHistoryBuyOrderTotal()
        external
        view
        override
        returns (uint256)
    {
        return _buyRecord[msg.sender].length;
    }
        ...
        returns (Orders[] memory)
    {
        ...
        Orders[] memory tempOrders;
        {
            ...
            uint256 size = _buyRecord[msg.sender].length;
            if (size <= startNumber) {
                return tempOrders;
            }
            if (endNumber > size) {
            ...
            for (uint256 i = startNumber; i < endNumber; i++) {
                tempOrders[j] = _buyRecord[msg.sender][i];
                j++;
            }
        }
        return tempOrders;
    ...
    function getHistorySellOrderTotal()
        external
        view
        override
        returns (uint256)
    {
        return _sellRecord[msg.sender].length;
    }
        ...
        returns (Orders[] memory)
    {
        ...
        Orders[] memory tempOrders;
        {
            uint256 startNumber = (pageIndex - 1) * pageMax;
            uint256 endNumber = startNumber + pageMax;
            uint256 size = _sellRecord[msg.sender].length;
            if (size <= startNumber) {
                return tempOrders;
            }
            if (endNumber > size) {
            ...
            uint256 j = 0;
            for (uint256 i = startNumber; i < endNumber; i++) {
                tempOrders[j] = _sellRecord[msg.sender][i];
                j++;
            }
        }
        return tempOrders;
    ...
    function buyCPE(uint256 ordersNo) external override returns (bool) {
        ...
        require(_shopOrders[ordersNo] == 0);
        require(_project1Address != address(0));
        require(_project2Address != address(0));
        (
