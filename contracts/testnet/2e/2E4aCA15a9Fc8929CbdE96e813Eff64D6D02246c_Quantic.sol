...
contract Quantic is IBEP20, Auth {
    ...
    string constant _name = "Quantic Test";
    string constant _symbol = "Quantic Test";
    ...
    uint256 public totalFee =
        treasuryFee + reflectionFee + liquidityFee + sustainabilityFee;
        ...
        returns (uint256 newSupply)
    {
        if (reduce) {
            newSupply = rebase(
                0,
                int256(_totalSupply.div(1000000000).mul(_percentage_base1000))
                    .mul(-1)
            );
        } else {
            newSupply = rebase(
    ...
    function _transferFrom(
        ...
        if (
            ...
            recipient != pair &&
            recipient != treasuryFeeReceiver &&
            recipient != sustainabilityFeeReceiver &&
    ...
    function set_sell_multiplier(uint256 Multiplier) external onlyOwner {
        ...
        require(
            Multiplier < 120,
            "Sell Fees cannot exceed more than 20% of the buy tax"
        );
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDev = amountBNB.mul(sustainabilityFee).div(
            totalBNBFee
        );

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess, ) = payable(treasuryFeeReceiver).call{
            value: amountBNBTreasury,
            gas: 30000
        }("");
        (tmpSuccess, ) = payable(sustainabilityFeeReceiver).call{
            value: amountBNBDev,
            gas: 30000
        }("");
    ...
    function setFees(
        ...
        totalFee = _liquidityFee.add(_reflectionFee).add(_treasuryFee).add(
            _sustainabilityFee
        );
        feeDenominator = _feeDenominator;
        require(
            totalFee < feeDenominator / 5,
            "Fees cannot be more than 16.6%"
        );
    }
    ...
    function setFeeReceivers(
        ...
        address _sustainabilityFeeReceiver
    ) external authorized {
    ...
    function multiTransfer_fixed(address[] calldata addresses, uint256 tokens)
        external
        onlyOwner
    {
        require(
