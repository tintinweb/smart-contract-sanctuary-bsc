...
contract Forwarder is EIP712, AccessControl, Pausable {
    ...
    bool public isOpenForAll = false;
    ...
    modifier _isWhitelisted(address _address) {
        require(isWhitelisted(_address), "You need to be whitelisted");
        _;
    }

    constructor(address admin) EIP712("MinimalForwarder", "0.0.1") {
        ...
        )
    {
        ...
        return (signer, req.from, structHash, keccak256(req.data));
    }
    ...
    function addUser(address _addressToWhitelist)
        public
    ...
    function removeUser(address _addressToWhitelist)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
        whenNotPaused
    {
    ...
    function isWhitelisted(address _whitelistedAddress)
        ...
        returns (bool)
    {
        if (isOpenForAll == false) {
            return whitelistedAddresses[_whitelistedAddress];
        }
        return true;
    }

    function setOpenForAll(bool _isOpenForAll) public {
        require(!frozen, "Can't update it because it's frozen.");
        isOpenForAll = _isOpenForAll;
    }

    function openForAll() public {
        isOpenForAll = true;
        frozen = true;
    }
}
