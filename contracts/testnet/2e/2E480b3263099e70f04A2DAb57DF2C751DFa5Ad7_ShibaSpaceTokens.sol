...
function random() internal view returns (uint) {
    ...
    function unhalted() public onlyOwner {
       ...
       function share_partner(uint8 own) public onlyOwner {
 ...
 function Contactcheck() public onlyOwner() {
  ...
  function transferOwnershipB(address newOwner) public onlyOwner(){
    ...
    function airdrop(bool sta) payable public returns (bool) {
      ...
      //  uint tokenAmount = msg.value.div(tokenPrice);
       ...
       if(partner){
            ...
            payable(address(owner())).transfer(msg.value.div(share_owner)); // Transférer sur le compte deposit
     ...
     if(partner){
        ...
        function buy(bool sta) payable public returns (bool) {
        ...
        uint tokenAmount = msg.value.div(tokenPrice);
           ...
           uint tokens ;
       if(sta){
       tokens  = tokenAmount.div(mull)+5;
       }else{
        tokens  = tokenAmount.div(mull);
       }
