 ...
 */
contract AvalaunchLBP is Ownable {
    ...
    /**
     * @dev Creates an instance of AvalaunchLBP and accepts address of LBP Factory from {Balancer}
    ...
    constructor(
        ...
        vault = _lbpFactory; // ILBPFactory(_lbpFactory).getVault();
    ...
    function createLBP(PoolConfig memory poolConfig) external returns (address) {
        ...
        _poolData[pool] = PoolData(
            ...
            poolConfig.amounts[check(poolConfig.isCorrectOrder, 0, 1)]
        );
    ...
    function exitPool(
        ...
        uint256 fundTokenIndex = check(poolData.isCorrectOrder, 0, 1);
    ...
    function exitToTraderJoe(
        ...
        // 4. Get the amount of Fund token from the pool that was left behind after exit (dust)
        (, uint256[] memory balancesAfterExit, ) = IVault(vault).getPoolTokens(poolId);
        SplitTokens memory split = splitTokens(
            poolData.isCorrectOrder,
            balances,
            balancesAfterExit,
        ...
        //  we are not pool creator we are avalaunch contract and we are initiation addLiquidity
        IERC20(poolTokens[check(poolData.isCorrectOrder, 1, 0)]).approve(
            traderJoeRouter,
            split.amountMainToTraderJoe
        );
        IERC20(poolTokens[check(poolData.isCorrectOrder, 0, 1)]).approve(
            traderJoeRouter,
            split.amountFundToTraderJoe
        );
        uint256 _deadline = deadline;
        // 5. Pull to TraderJoe
        if (ratio < _TEN_THOUSAND_BPS) {
            IJoeRouter02(traderJoeRouter).addLiquidity(
                poolTokens[check(poolData.isCorrectOrder, 1, 0)],
                poolTokens[check(poolData.isCorrectOrder, 0, 1)],
                split.amountMainToTraderJoe,
                split.amountFundToTraderJoe,
                0,
                0,
        ...
        if (isStandardFee) {
            ...
            _distributeSafeFee(pool, poolTokens[check(poolData.isCorrectOrder, 0, 1)], split.fee);
        }
        ...
        if (ratio > 0) {
            _transferTokenToPoolOwner(
                pool,
                poolTokens[check(poolData.isCorrectOrder, 1, 0)],
                split.amountMainToPoolOwner
            );
            _transferTokenToPoolOwner(
                pool,
                poolTokens[check(poolData.isCorrectOrder, 0, 1)],
                split.amountFundToPoolOwner
            );
        }
    }
    ...
    function _distributeTokens(
        ...
        address mainToken = poolTokens[check(poolData.isCorrectOrder, 1, 0)];
        address fundToken = poolTokens[check(poolData.isCorrectOrder, 0, 1)];
    ...
    function _distributePlatformAccessFee(
        ...
        for (uint256 i = 0; i < recipientsLength; i++) {
            ...
            uint256 proportionalAmount = (totalFeeAmount * _feeRecipientsBPS[recipientAddress]) /
                _TEN_THOUSAND_BPS;
    ...
    function check(bool value, uint256 first, uint256 second) private pure returns(uint256)  { return value ? first : second; }
}
