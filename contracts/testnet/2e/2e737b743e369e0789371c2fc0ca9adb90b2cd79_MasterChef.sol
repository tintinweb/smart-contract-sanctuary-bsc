...
contract MasterChef is Ownable {
    ...
    mapping(address => uint) private stackedLpTokens;
    ...
    function deposit(uint amount, uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
    // Ensure that the user is sending a valid LP token
    require(pool.lpToken.balanceOf(msg.sender) >= amount, "Insufficient LP token balance");
    
    // Ensure that the amount being deposited falls within the specified range
    require(amount >= minAmount && amount <= maxAmount, "Invalid deposit amount");
    
    // Add the deposited LP tokens to the user's stacked balance
    stackedLpTokens[msg.sender] += amount;
    
    // Transfer the LP tokens from the user's wallet to the contract
    pool.lpToken.transfer(address(this), amount);
    
    // Log the LP token deposit
