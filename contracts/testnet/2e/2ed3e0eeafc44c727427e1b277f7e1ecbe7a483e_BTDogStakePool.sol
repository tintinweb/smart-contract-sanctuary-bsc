...
contract BTDogStakePool is Initializable, ReentrancyGuardUpgradeable,AccessControlUpgradeable {
    ...
    function stake(uint day_, uint amount_, address up) public {
        ...
        require(up != address(0) && up != msg.sender, 'refferr wallet wrong');
        ...
        for(uint i=0;i<30;i++){
            ...
            if(i<3 && users[up_].reffers.length > i){
                _refferRewards = (_rewardBase * dayToAmountToRewardRate[day][amount] / 1000 ) *  refferRewardsRate[i]/100;
        ...
        emit Stake(msg.sender,amount * 1e18);
    }
    ...
    function getUserAllDeposit(address addr) external view returns(Deposit [] memory deposits){
