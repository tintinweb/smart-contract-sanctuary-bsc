...
contract EscrowGig is Ownable, ReentrancyGuard {
    ...
    string public ipfs; // contains description + images
    ...
    constructor(IERC20 _busd, EscrowFactory _factory, uint256 _price, string memory _title, string memory _category, string memory _ipfs) {
    ...
    modifier onlyAdmins() {
        require(administrator == msg.sender, "Admins: caller is not in list of Admins");
        _;
    }
    ...
    // finalize delivery
    function satisfyRequest() nonReentrant public {
        require(activeOrders[msg.sender] == true, 'you do not have an active order');
        uint256 orderIdLocal = orderId[msg.sender];
    ...
    // function requireService(uint256 _gigId) nonReentrant public {
    //     require(factoryInternal.profileCreation[msg.sender] == true, 'you must create a profile first');
    //     require(gigs[_gigId].seller != msg.sender, 'You cannot buy your own GIGs');
    //     require(currentGigId > _gigId, 'invalid gig');
    //     uint256 amountToPay = gigs[_gigId].price;
    //     // create escrow order
    //     // LOGIC => transfer busd amount to contract
    //     // increase purchase time 
    //     // set an order list with buyer, timeOfOrder and approvedPayment status (approved payment used to track finalized orders)
    //     BUSD.transferFrom(msg.sender, address(this), amountToPay);
    //     gigs[_gigId].timesPurchased = gigs[_gigId].timesPurchased+1;

    //     orders[_gigId].push(Order(msg.sender, block.timestamp, false));
    // }

    // function returnOrderInfo(uint256 _gigId) public view returns(Order[] memory){
    //     return orders[_gigId];
    // }
...
contract EscrowFactory is Ownable, ReentrancyGuard {
    ...
    //     EscrowGig newEscrow = new EscrowGig(BUSD, address(this));


    //     allEscrows.push(address(newEscrow));
    //     isEscrowedReal[address(newEscrow)] = true;
    // }
    ...
    function setWebsiteEmailTelegramTwitter(string memory _website, string memory _email, string memory _telegram, string memory _twitter) public {
        require(profileCreation[msg.sender] == true, 'you must purchase profile creation for 5 busd');
        userInfo[msg.sender].email = _email;
        userInfo[msg.sender].website = _website;
        userInfo[msg.sender].telegram = _telegram;
    ...
    function purchaseProfileCreation() public {
        ...
        BUSD.transferFrom(msg.sender, owner(), 5000000000000000000);
    ...
    function purchaseTier2(uint256 _weeks) public {
        ...
        uint256 toTransfer = _weeks * 20000000000000000000;
    ...
    function purchaseTier2AndProfileCreation(uint256 _weeks) nonReentrant public {
        ...
        uint256 toTransfer = _weeks * 20000000000000000000;
        uint256 timeStamp = _weeks * 604800;
        BUSD.transferFrom(msg.sender, owner(), toTransfer+5000000000000000000);
        if(tier2[msg.sender] >= block.timestamp){
            tier2[msg.sender] = tier2[msg.sender] + timeStamp;
        } else {
            tier2[msg.sender] = block.timestamp + timeStamp;
    ...
    function purchaseTier3(uint256 _weeks) public {
        require(profileCreation[msg.sender] == true, 'you must buy plan 1 first');
        require(_weeks >= 1, 'invalid number of weeks');
        uint256 toTransfer = _weeks * 35000000000000000000;
        uint256 timeStamp = _weeks * 604800;
        BUSD.transferFrom(msg.sender, owner(), toTransfer);
    ...
    function createGig(uint256 _price, string memory _title, string memory _category, string memory _ipfs) public {
        require(profileCreation[msg.sender] == true,'you must create a profile first');
        EscrowGig newEscrow = new EscrowGig(BUSD, EscrowFactory(address(this)), _price, _title, _category, _ipfs);
