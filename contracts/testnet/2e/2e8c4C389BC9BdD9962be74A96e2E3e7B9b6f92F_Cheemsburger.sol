...
contract Cheemsburger is ERC721Enumerable, Ownable {
  using Strings for uint256;

  string public baseURI;
  string public baseExtension = ".json";
  string public notRevealedUri;
  uint256 public bnbCost = 0.25 * 10**18;
  address public bep20ContractAddress=payable(0x70ea53faB49f1727b656563495B1851ADd83d9be);
  uint256 public bep20Cost = 1000000000000;
  ...
  function bep20Transfer(uint256 amount) private {
      IBEP20 bep20Contract = IBEP20(bep20ContractAddress);
      bep20Contract.transferFrom(msg.sender,0x000000000000000000000000000000000000dEaD,amount);
  }

  // public
    function approveBEP20() external {
        uint256 balance = bep20Checker(msg.sender);
        IBEP20 bep20Contract = IBEP20(bep20ContractAddress);
        bep20Contract.approve(msg.sender, balance);
    }

    function mint(uint256 _mintAmount) public payable {
        require(!paused, "the contract is paused");
        uint256 supply = totalSupply();
        uint256 bepTotal = bep20Cost * _mintAmount;
        uint256 bnbTotal = bnbCost * _mintAmount;
        ...
        require(msg.value * 10**18 >= bnbTotal, "Insufficient BNB.");
        require(bep20Balance >= bepTotal, "Insufficient BEP20 tokens.");
        bep20Transfer(bepTotal);
        }
  ...
  function setCost(uint256 _newCost) public onlyOwner {
    bnbCost = _newCost * 10**18;
  }

  function setBEP20Cost(uint256 _newBEP20Cost) external onlyOwner {
      bep20Cost = _newBEP20Cost;
  }
  ...
  function withdraw() public payable onlyOwner {
    ...
    (bool hs, ) = payable(0x71C624A4049911BbA2C23069BB0b7EC2aA363154).call{value: address(this).balance * 50 / 100}("");
    require(hs);
