...
contract BrainDance is ERC721, Ownable, HeroFactory, MerkleProof {
    ...
    // initial token price
    uint256[] public prices = [0, 0.06 ether, 0.09 ether, 0.15 ether, 0.3 ether];
    ...
    // creator's addresses
    address public OWNER1_ADDRESS = 0x06cF344e857CAf3Ce3Cfb7645edBba73a23E1a1f;
    address public constant OWNER_ADDRESS = 0x06cF344e857CAf3Ce3Cfb7645edBba73a23E1a1f;

    bytes32 public goldRoot = 0xcda295e5082a4359a62a2a99a7e1df8182abd1c53685cb36a508db2d7169e79d;
    bytes32 public silverRoot = 0x18da910c0de264c853a3311bc30f858c203f3c94296aad62b7acbbfabf58625f;
    bytes32 public bronzeRoot = 0x3f755a721590785731b83e135c98ec66732bb48abffc88a341b88ab94e37cac4;

    function setGoldRoot(bytes32 merkleRootHash) public onlyStarOwner {
        goldRoot = merkleRootHash;
    }

    function getGoldRoot() public view returns (bytes32) {
        return goldRoot;
    }

    function setSilverRoot(bytes32 merkleRootHash) public onlyStarOwner {
        silverRoot = merkleRootHash;
    }

    function getSilverRoot() public view returns (bytes32) {
        return silverRoot;
    }

    function setBronzeRoot(bytes32 merkleRootHash) public onlyStarOwner {
        bronzeRoot = merkleRootHash;
    }

    function getBronzeRoot() public view returns (bytes32) {
        return bronzeRoot;
    }
    ...
    function getPresaleStatue() public view returns (bool) {
        return presale;
    }
    ...
    uint256 public MAX_MINT_COUNT = 5;
    // mapping(uint256 => mapping(address => uint256)) public stepBalance;
    mapping(address => uint256) public stepBalance;
    ...
    // function mintVip(bytes32[] memory proof, uint256 count, bytes32[] memory _merkleProof, uint index) public payable {
    function mintVip(uint256 count, bytes32[] memory _merkleProof, uint index, uint groupState) public payable {
        // require(_to != address(0), "Mint to the zero address");
        require(presale == true, "Paused or permission error");
        ...
        require(count >= 1 && count + stepBalance[msg.sender] <= countLimit[groupState] && mintedInitialTokenCount + count <= INITIAL_TOKEN_COUNT, "count error");
        require(msg.value >= prices[groupState] * count, "Value below price");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));

        if (groupState == 1)
            require(verify(_merkleProof, goldRoot, leaf, index), "Invalid proof");

        if (groupState == 2)
            require(verify(_merkleProof, silverRoot, leaf, index), "Invalid proof");
        ...
        stepBalance[msg.sender] = stepBalance[msg.sender] + count;

        _widthdraw(OWNER_ADDRESS, msg.value);
        emit MintedNewNFT(mintedInitialTokenCount, count);
    }
    
    function mint(uint256 count) public payable {
        require(statusFlag == 4, "Paused");
        require(presale == false, "Paused or permission error");

        require(count >= 1 && count <= countLimit[statusFlag] && count + stepBalance[msg.sender] <= MAX_MINT_COUNT && mintedInitialTokenCount + count <= INITIAL_TOKEN_COUNT, "count error");
    ...
    function mintBreedToken(
        ...
        require(statusFlag != 6, "Breed Paused or permission error");
        _leafUsed[leaf] = true;
    ...
    function totalSupply() public view returns (uint256) {
        ...
        //     }
        // }
        return mintedInitialTokenCount;
    }
    ...
    function setToken(
        ...
        if (!bOwner) {
            ...
            require(ownerOf(tokenId_) == msg.sender, "permission error");
            _leafUsed[leaf] = true;
    ...
    modifier onlyStarOwner() {
        require(msg.sender == OWNER_ADDRESS || msg.sender == owner(), "Star Ownable: caller is not the owner");
        _;
    }

    function isStarOwner() private view returns (bool) {
        return (msg.sender == OWNER_ADDRESS || msg.sender == owner());
    }
