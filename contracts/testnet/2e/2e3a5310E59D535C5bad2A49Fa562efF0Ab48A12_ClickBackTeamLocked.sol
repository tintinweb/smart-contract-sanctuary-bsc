...
contract ClickBackTeamLocked {
   ...
   constructor(address _owner) {
    ...
    function lockteamoken(uint256 _pascode,uint256 _amount, uint256 _deadline) external returns(bool)
    {
        ...
        userToTokenLocks[msg.sender].push(Lock(msg.sender,_pascode,_token, balanceAfter-balanceBefore, _deadline));     
        
        emit Locked(msg.sender,_pascode, _token, _amount, _deadline);
        return true;
    }

    function withdrawteamtoken(uint256 _pascode,uint256 _index) external returns(Lock[] memory)
    {
    ...
    function getLocksByacontractddress(address _user) public view returns(Lock[] memory){
