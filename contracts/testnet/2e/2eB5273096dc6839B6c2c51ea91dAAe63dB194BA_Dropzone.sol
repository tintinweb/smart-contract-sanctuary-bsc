...
contract Dropzone is Initializable,Ownable {
    ...
    event SocialMediaUpdated(SocialMedia[] _oldSocialMedia,SocialMedia[] _newSocialMedia );
    event TimeUpdated(uint oldFrom ,uint oldTo , uint newFrom ,uint newTo);
    event AirdropListed(bytes32 indexed _hash,address indexed _token,address _owner , uint _amount, uint _from, uint _to );
    ...
    bool private locked;
    mapping(bytes32=>mapping(address=>uint)) public userclaimed;
    function initialize(address _newowner) external initializer{
        ownable(_newowner);
    }
   
    function applyforAirdrop(address _token,uint _amount,uint _from ,uint _to ,SocialMedia[] calldata _socialMedia ) external  {
        ...
        for(uint i=0;i<_socialMedia.length;i++) {
            airdrops[_hash].socialMedia[i]=_socialMedia[i];
        }

        allAirdrops.push(_hash);
        emit AirdropListed( _hash,_token, msg.sender, _amount,  _from, _to);
    }

    function removeAirdrop(bytes32 _hash) external onlyCreator(_hash) {
        require(!cancled[_hash],"Order is Cancled!");
        cancled[_hash]=true;
        IERC20(airdrops[_hash].token).transfer(airdrops[_hash].owner, airdrops[_hash].amount-airdrops[_hash].distbutedAmount);
    }

    function changeSocialMedia(bytes32 _hash,SocialMedia[] calldata _socialMedia) external onlyCreator(_hash) {
        require(!cancled[_hash],"Order is Cancled!");
        emit SocialMediaUpdated( airdrops[_hash].socialMedia,_socialMedia );
        for(uint i=0;i<_socialMedia.length;i++) {
            airdrops[_hash].socialMedia[i]=_socialMedia[i];
        }
    ...
    function claimAirdrop(bytes32 _hash , uint _amount) external noReentrant {
        Airdrop memory airdrop =airdrops[_hash];
        ...
        uint claim = getClaimableAmount(_hash);
    ...
    function getClaimableAmount(bytes32 _hash) internal view returns(uint claimAmt){

        for(uint8 i=0;i<airdrops[_hash].socialMedia.length;i++) {
            claimAmt+=airdrops[_hash].socialMedia[i].amount;
        }
    }
}
