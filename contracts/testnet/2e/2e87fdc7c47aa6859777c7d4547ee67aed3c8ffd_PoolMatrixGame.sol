...
contract PoolMatrixGame {
    ...
    function getSlots(address userAddr) public view verified returns(uint[] memory slots, uint[] memory partnerBonuses, uint[] memory poolProfits, uint[] memory missedProfits, uint contractState) {
    ...
    function importCleanQueues(uint startLevel, uint endLevel) public {
        require(msg.sender == adminWallet);
        for (uint level = startLevel; level <= endLevel; level++)
            delete levelQueue[level];
    }

    function importQueues(address[][] memory addrs) public {
        require(msg.sender == adminWallet);
        for (uint level = 0; level < LEVELS_COUNT; level++) {
            uint n = addrs[level].length;
            for (uint i = 0; i < n; i++)
                levelQueue[level].push(addrs[level][i]);
        }
    }
