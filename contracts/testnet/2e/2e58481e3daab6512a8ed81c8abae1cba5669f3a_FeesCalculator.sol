...
contract FeesCalculator is Ownable, IFeesCalculator {
    ...
    uint256 public sbsLockAmount = 134 ether;
    ...
    uint8 public constant PAYMENT_MODE_SBS_LP_TOKEN = 1;
    uint8 public constant PAYMENT_MODE_BNB_MAX = 2;
    uint8 public constant PAYMENT_MODE_SBS_MAX = 3;
    ...
    *                       1 - pay fees in minSBS + LP token,
    *                       2 - pay fees fully in maxBNB,
    *                       3 - pay fees fully in maxSBS
    *                       4 - pay fees by locking SBS
    */
    function calculateFees(
        address /* lpToken */,
        uint256 amount,
        ...
        if(partnerReferralPercent > 0) {
            referralPercent = partnerReferralPercent;
        } else {
            referralPercent = referralPercentScaled;
        }
        if (paymentMode == PAYMENT_MODE_BNB_LP_TOKEN) {
            return (ethMin, 0, liquidityPercent.mul(amount).div(1e4), 0, referralPercent);
        }
        if (paymentMode == PAYMENT_MODE_SBS_LP_TOKEN) {
            return (0, tokenMin, liquidityPercent.mul(amount).div(1e4), 0, referralPercent);
        }
        if (paymentMode == PAYMENT_MODE_BNB_MAX) {
            return (ethMax, 0, 0, 0, referralPercent);
        }
        if (paymentMode == PAYMENT_MODE_LOCK_TOKENS) {
            return (0, 0, 0, sbsLockAmount, referralPercent);
        }
        return (0, tokenMax, 0, 0, referralPercent);
    }
    ...
    */
    ...
    external override view returns (uint256 ethFee, uint256 tokenFee, uint256 lpTokenFee, uint256 tokenAmountToLock)  {
        ...
        if (paymentMode == PAYMENT_MODE_SBS_MAX) {
    ...
    function getFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256)  {
        return (ethMin, tokenMin, ethMax, tokenMax, liquidityPercent, sbsLockAmount);
    }

    function setEthMin(uint256 _ethMin) external onlyOwner {
        ethMin = _ethMin;

        emit OnFeeChanged(ethMin, tokenMin, ethMax, tokenMax, liquidityPercent, sbsLockAmount);
    }
    ...
    function setSbsLockAmount(uint256 _sbsLockAmount) external onlyOwner {
        sbsLockAmount = _sbsLockAmount;
