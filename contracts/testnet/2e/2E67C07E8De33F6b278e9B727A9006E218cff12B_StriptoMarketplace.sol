...
contract StriptoMarketplace is ReentrancyGuard {
    ...
    address payable owner; // The owner of the StriptoMarket contract (transfer and send function availabe to payable addresses)
    ...
    function resellNFT(uint256 itemId, uint256 price) public payable {
        ...
        nonReentrant
    {
        ...
        idToMarketItem[itemId].seller.transfer(msg.value-saleFee);
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
        idToMarketItem[itemId].isSold = true;
        idToMarketItem[itemId].owner = payable(msg.sender);
        _itemsSold.increment();
        payable(owner).transfer(saleFee*2);
    }
        ...
        nonReentrant
    {
        ...
        require(
            msg.value <= price,
            "Price cannot be less than listing price"
        );
        ...
        // transfer stripped amount from Escrow to seller
         escrowContract.withdrawStrippedAmount(itemId,idToMarketItem[itemId].seller);
        ...
        // pay listing price to makertplace owner from buyer
         payable(owner).transfer(saleFee);
    }



    function stripItemPrice(uint256 itemId) public payable nonReentrant returns (uint256)
    {
        uint256 stripedPrice = idToMarketItem[itemId].stripedPrice;
        uint256 price = idToMarketItem[itemId].price;

        uint256 stripAmt=(price*stripPercentage)/100;
        uint256 newstripedPrice=stripedPrice-stripAmt;
 
       //require(newstripedPrice > 0, "Can't strip amount to zero");

       // deposit stripped amount from Escrow Account
        escrowContract.depositToEscrow{ value: msg.value}(itemId);

        uint256 leftAmount=stripedPrice-stripAmt;
...
function depositToEscrow(uint256 _itemId) external payable returns (uint256){
  ...
  function depositPayment(uint256 _itemId) external payable returns (uint256) {
         uint256 _amount=msg.value;
