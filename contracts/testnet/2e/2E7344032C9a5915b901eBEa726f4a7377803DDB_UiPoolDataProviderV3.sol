...
// File @aave/core-v3/contracts/dependencies/openzeppelin/contracts/[email protected]
...
interface IUiPoolDataProviderV3 {
  ...
  struct BaseCurrencyInfo {
    ...
    address marketReferenceCurrency; // 0x0 = USD
  }

  struct UserAcountData {
  ...
  function getUserAccountData(IPoolAddressesProvider provider, address user)
    external
    view
    returns (UserAcountData memory);
}
...
interface IPool {
  ...
  function mintBorrowBalance(address asset, address account) external view returns (uint256);

  function setInterestReceiver(address receiver) external returns (address);
}
...
// File @aave/core-v3/contracts/interfaces/IAaveIncentivesContr[email protected]
...
// File @aave/core-v3/contracts/protocol/libraries/math/[email protected]
...
// File @aave/core-v3/contracts/protocol/libraries/helpers/[email protected]
...
// File @aave/core-v3/contracts/protocol/libraries/configuration/[email protected]
...
library ReserveConfiguration {
  ...
  uint256 internal constant SUPPLYING_MASK =                 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF7FFFFFFFFFFFFFFF; // prettier-ignore 63bit
  ...
  /// @dev bit 63 reserved
  // 63 位 是否允许 supply
  uint256 internal constant SUPPLYING_ENABLED_START_BIT_POSITION = 63;
  ...
  /**
   * @notice Enables or disables supplying on the reserve
   * @param self The reserve configuration
   * @param enabled True if the supplying needs to be enabled, false otherwise
   **/
  function setSupplyingEnabled(DataTypes.ReserveConfigurationMap memory self, bool enabled)
    internal
    pure
  {
    self.data =
      (self.data & SUPPLYING_MASK) |
      (uint256(enabled ? 1 : 0) << SUPPLYING_ENABLED_START_BIT_POSITION);
  }

  /**
   * @notice Gets the supplying state of the reserve
   * @param self The reserve configuration
   * @return The supplying state
   **/
  function getSupplyingEnabled(DataTypes.ReserveConfigurationMap memory self)
    internal
    pure
    returns (bool)
  {
    return (self.data & ~SUPPLYING_MASK) != 0;
  }
}
...
// File @aave/core-v3/contracts/protocol/pool/[email protected]
...
// File @aave/core-v3/contracts/misc/[email protected]
...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
  ...
  uint256 public constant USD_CURRENCY_UNIT = 1e8;
...
console.log(address(poolDataProvider));
...
console.log("4");
      ...
      if(address(poolDataProvider) != address(0)) {
      ...
      reserveData.supplyingEnabled = reserveConfigurationMap.getSupplyingEnabled();
    }

    BaseCurrencyInfo memory baseCurrencyInfo;
    if(address(networkBaseTokenPriceInUsdProxyAggregator) != address(0)) {
    ...
    try oracle.BASE_CURRENCY_UNIT() returns (uint256 baseCurrencyUnit) {
      ...
      baseCurrencyInfo.marketReferenceCurrency = oracle.BASE_CURRENCY();
    } catch (bytes memory /*lowLevelData*/) {  
      baseCurrencyInfo.marketReferenceCurrencyUnit = USD_CURRENCY_UNIT;
      if(address(marketReferenceCurrencyPriceInUsdProxyAggregator) != address(0)) {
    ...
    returns (UserReserveData[] memory, uint8)
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      if (userConfig.isBorrowing(i)) {
        ...
        userReservesData[i].mintBorrowBalance = pool.mintBorrowBalance(userReservesData[i].underlyingAsset, user);
      }
    }
  ...
  function getUserAccountData(IPoolAddressesProvider provider, address user)
    ...
    returns (UserAcountData memory)
  {
    UserAcountData memory uad;

    IPool pool = IPool(provider.getPool());
    require(address(pool) != address(0), "Invalid pool");

    (uad.totalCollateralBase, uad.totalDebtBase, uad.availableBorrowsBase, uad.currentLiquidationThreshold, uad.ltv, uad.healthFactor) = pool.getUserAccountData(user);
