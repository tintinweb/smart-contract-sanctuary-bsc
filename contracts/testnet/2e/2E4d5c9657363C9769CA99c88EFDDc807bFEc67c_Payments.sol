...
contract Payments is Ownable, ReentrancyGuard {
    ...
    event RewardUsed(address indexed user, uint256 amount, address usedOn, uint256 date);
    ...
    function payOwner(address _owner, uint256 _amount) external onlyAuth nonReentrant returns(bool){
        ...
        if(IStaking(_stakingAddress).getMiningStarted()){
            // 5 % for stakers (need mining rewards began)
            IStaking(_stakingAddress).receiveFees(_toDistribute / 4);
            require(BZAI.transfer(_stakingAddress, _toDistribute / 4));
        }else{
            // 5% for winning contract
            require(BZAI.transfer(gameAddresses.getWinRewardsAddress(), _toDistribute / 4));
        }
    ...
    function distributeFees(uint256 _amount) external onlyAuth nonReentrant returns(bool){
        ...
        if(IStaking(_stakingAddress).getMiningStarted()){
            // 80% to stakers (need mining rewards began)
            IStaking(_stakingAddress).receiveFees(toStakers);
            require(BZAI.transfer(_stakingAddress, toStakers));
        }else{
            // 40%  for reward challenge in game (daily and weekly ranking)
            require(BZAI.transfer(gameAddresses.getChallengeRewardsAddress(), toStakers / 2));
            // 40% for winning contract (where each win in game give 0.1% of the win pool to the user)
            return (BZAI.transfer(gameAddresses.getWinRewardsAddress(), toStakers / 2));
        }
    ...
    function payNFTOwner(address _owner, uint256 _amount) external onlyAuth nonReentrant returns(bool){
        uint256 _toOwner = _amount * 9800 / 10000;
        uint256 _toDistribute = _amount - _toOwner;

        // 98% for owner of Nuresery/Training Center or Forge
        _myRevenues[_owner] += _toOwner;
    ...
    function rewardPlayer(address _user, uint256 _amount) external onlyAuth returns(bool){
    ...
    function burnRevenuesForEggs(address _owner, uint256 _amount) external onlyAuth nonReentrant returns(bool){
    ...
    function claimMyCentersRevenues() external returns(bool){
