...
contract FeatHealth is Ownable {
    ...
    event Report(uint256 indexed reportId, uint256 challengeId, string videoPath, address reporter);
     ...
     */
    function createChallenge(uint256 _categoryId, string memory _name, uint256 _targetQuantity, uint256 _rewardPercentage) public onlyOwner() {
     ...
     */
    function updateChallenge(uint256 _id, uint256 _categoryId, string memory _name, uint256 _targetQuantity, uint256 _rewardPercentage, bool _isEnable) public onlyOwner() {
        challengeInfo[_id].categoryId = _categoryId;
        challengeInfo[_id].name = _name;
        challengeInfo[_id].targetQuantity = _targetQuantity;
        challengeInfo[_id].rewardPercentage = _rewardPercentage;
        challengeInfo[_id].isEnable = _isEnable;
    }
     ...
     */
    function completeChallenge(uint256 _id)
        public
        onlyDepositer(true) 
        onlyUnLockUp()
        onlyPossibleChallenge()
    {
        ...
        uint256 rewardTokenAmount = userTokenAmount.div(100000).mul(challengeInfo[_id].rewardPercentage);
     ...
     */
    function getReportList(address _challenger) public view returns (ReportChallengeInfo[] memory) {
        uint totalReportCount = _reportIds.current();
        uint challengerReportCount = 0;
        for (uint i = 0; i < totalReportCount; i++) {
            uint id = i + 1;
            if(reportChallengeInfo[id].challenger == _challenger){
