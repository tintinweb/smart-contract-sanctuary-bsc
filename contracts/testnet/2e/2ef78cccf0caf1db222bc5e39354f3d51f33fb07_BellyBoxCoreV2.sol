...
contract BellyBoxCoreV2 is Initializable, AccessControlUpgradeable {

    event GiveChiToken(uint256 indexed boxId, address indexed to, uint256 amount);

    struct BellyBox {
        // Quantity of CHI used to purchase a single blind box
        uint256 price;
     ...
     */
    function initialize(address _ingredientERC1155Address, address _equipmentERC1155Address, address chiCoinAddress, address cswAddress, address _seedBiulder) public initializer{
        ...
        bellyBoxChiAmount = 20000000000000000000000; // 20000 CHI
    ...
    function createBellyBox(
        ...
        cswToken.transferFrom(recipient, address(this), cswAmount);
        uint256 seed = seedBiulder.getRandomSeed(_msgSender());
        uint8 ingredientType = Random.createRandom(1, 4, seed);
        uint8 equipmentType = Random.createRandom(11, 13, seed, ingredientType);

        uint8 allInOneType = 0;
        if (bellyBoxType == 3) {
            allInOneType = Random.createRandom(1, 3, seed, equipmentType);
        }
        
        //TODO test code, will be removed
        allInOneType = 3;

        if (bellyBoxType == 1 || (allInOneType == 1 && bellyBoxType == 3)) {
            ingredientERC1155.createIngredient(recipient, boxId, ingredientType, _name, _describe, seed);
        } else if (bellyBoxType == 2 || (allInOneType == 2 && bellyBoxType == 3)) {
            equipmentERC1155.createEquipment(recipient, boxId, equipmentType, _name, _describe, seed); 
        } else if (bellyBoxType == 3 && allInOneType == 3 ) {
            chiCoin.transfer(recipient, bellyBoxChiAmount);
            emit GiveChiToken( boxId, recipient, bellyBoxChiAmount);
        }

        bellyBoxes[bellyBoxType].opened ++;
    }
     ...
     */
    function setBellyBox(uint8 bellyBoxType, uint256 price, uint64 supply, string memory describe) onlyRole(DEFAULT_ADMIN_ROLE) external {
     ...
     */
    function getBellyBox(uint256 bellyBoxType) public view returns (
        uint256 price,
        uint64 supply,
        uint64 opened,
        ...
        opened = bellyBoxes[bellyBoxType].opened;
