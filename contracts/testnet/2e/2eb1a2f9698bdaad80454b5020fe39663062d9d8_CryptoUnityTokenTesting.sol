...
contract CryptoUnityTokenTesting is Context, IERC20, Ownable {
    ...
    address payable public vaultRewardAddress = payable(0x317D70A66F7906233EeC3D417961Bbb25159A6F8); 
    address payable public developmentAddress = payable(0x29D3471D301a9C98b5C9ab4BCf2af7Bde5977a21); 
    ...
    string private constant _name = "CryptoUnityTokenTesting";
    string private constant _symbol = "CUTTesting";
    ...
    uint256 public _buyLiqFee = 1;
    uint256 private _buyLiquidityFee = 11;
    ...
    uint256 private _sellLiquidityFee = 14;
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    uint256 remainingLiquidityToken = contractTokenBalance.mul(_sellRewardFee+_sellMarketingFee+_sellDevelopmentFee).div(_sellLiquidityFee);
                    uint256 liquidityToken = contractTokenBalance.sub(remainingLiquidityToken);
    ...
    function setRouterAddressAndCreatePair(address newRouter) external onlyOwner() {
    ...
    function setReawardMarketingDevFee(uint256 _sellHolderPercent,uint256 _sellRewardsPercent,uint256 _sellMarketingPercent, uint256 _sellDevelopmentPercent, uint256 _sellLiquidityPercent, uint256 _buyHolderPercent, uint256 _buyRewardsPercent,uint256 _buyMarketingPercent, uint256 _buyDevelopmentPercent, uint256 _buyLiquidityPercent ) external onlyOwner {
        require((_sellHolderPercent + _sellLiquidityPercent+ _sellMarketingPercent+_sellRewardsPercent+_sellDevelopmentPercent)<=35,"Total Sell Percent Should be less than 35%");
        require((_buyHolderPercent + _buyLiquidityPercent+_buyRewardsPercent+_buyMarketingPercent+_buyDevelopmentPercent)<=35,"Total Buy Percent Should be less than 35%");
    ...
    function setTransferFee(uint256 _holderPercent, uint256 _rewardPercent, uint256 _marketingPercent, uint256 _devPercent, uint256 _liquidityPercent) external onlyOwner{
        require((_holderPercent + _liquidityPercent + _rewardPercent + _marketingPercent + _devPercent)<=35,"Total Tax Percent Should be less than 35%");
        _taxFee = _holderPercent;
        _liqFee = _liquidityPercent;
        _liquidityFee = _liquidityPercent + _rewardPercent + _marketingPercent + _devPercent;
    }
