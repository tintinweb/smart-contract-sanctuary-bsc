...
contract Rewarder is Ownable, Pausable, Initializable {
    ...
    function getUserInfo(address _user) external view returns(uint ,int256, bool) {
        return (_userInfo[_user].amount, _userInfo[_user].rewardDebt, _userInfo[_user].isUpdate);
    }
    ...
    whenNotPaused
    {
        ...
        UserInfo storage user = _userInfo[_user];
        if(!user.isUpdate) {
            staking.update(_user);
        }
        uint256 accumulatedRewards = user.amount * accRewardPerShare / ACC_REWARD_PRECISION;
        uint256 pending = uint256(int256(accumulatedRewards) - user.rewardDebt);
        if (pending > 0) {
            rewardToken.safeTransfer(to, pending);
        }
        user.rewardDebt = int256(lpToken * accRewardPerShare / ACC_REWARD_PRECISION);
...
//        user.rewardDebt += int256(_change * accRewardPerShare / ACC_REWARD_PRECISION);
        user.rewardDebt += (int256(_amount * accRewardPerShare / ACC_REWARD_PRECISION) - int256(user.amount * accRewardPerShare / ACC_REWARD_PRECISION));
        user.amount = _amount;
...
//        user.rewardDebt -= int256(_change * accRewardPerShare / ACC_REWARD_PRECISION);
        user.rewardDebt -= (int256(user.amount * accRewardPerShare / ACC_REWARD_PRECISION) - int256(_amount * accRewardPerShare / ACC_REWARD_PRECISION));
        user.amount = _amount;
    ...
    function onUpdate(address _user, uint256 _amount) external onlyStaking whenNotPaused {
        ...
        UserInfo storage user = _userInfo[_user];
        if (user.amount < _amount) {
            user.rewardDebt += int256((_amount - user.amount) * accRewardPerShare / ACC_REWARD_PRECISION);
        } else {
            user.rewardDebt -= int256((user.amount - _amount) * accRewardPerShare / ACC_REWARD_PRECISION);
        }
        user.amount = _amount;
        user.isUpdate = true;
