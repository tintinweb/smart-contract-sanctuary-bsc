...
contract GAMEGAMEGAME is ReentrancyGuard {
    ...
    address payable public w3;
    address payable public w4;
    address payable public w5;
    ...
    constructor(address payable _marketing) public {
        ...
        w1 = payable(msg.sender);
        w2 = payable(msg.sender);
        w3 = payable(msg.sender);
        ...
        users[w2] = User({
            ...
            turnover : 0
             ...
             usersAddressById[2] = w1;
             usersAddressById[3] = w2;
        ...
        for(uint8 round = 1; round <= totalrounds; round++) {
            ...
            users[w1].rounds[round].maxPayouts = 15;
            users[w2].rounds[round].maxPayouts = 15;
            roundQueue[round].push(owner);
            roundQueue[round].push(w1);
            roundQueue[round].push(w2);
        }
    }
    ...
    function buyround(uint8 round) public payable nonReentrant{
        ...
        if (roundQueue[round][headIndex[round]] != msg.sender) {
            ...
            if (sent) {
                ...
                RoiRounds1[round] = 25 * 31536000 / (now - users[rewardAddress].rounds[round].lastTimeBuyround); 
                emit roundPayout(now, msg.sender, users[rewardAddress].id, round, reward, users[msg.sender].id, RoiRounds1[round]);
            } else {
