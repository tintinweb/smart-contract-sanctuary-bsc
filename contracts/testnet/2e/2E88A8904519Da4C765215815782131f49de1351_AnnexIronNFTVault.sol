...
contract AnnexIronNFTVault is Ownable {
    ...
    constructor() {
        rewardsAllocationTUSD = 1000 ether;
    ...
    function _updateReward(address _user) internal {
        Staker storage staker = stakers[_user];
        uint256 bonusAmount = calculateRewards(_user);
        if(bonusAmount == 0){
            totalRewards = totalRewards.sub(staker.rewardsEarned);
            staker.rewardsEarned = 0;
            if(staker.isAutoCompound){totalRewardsForCompound = 0;}
        }else if(bonusAmount > staker.rewardsEarned){
            uint256 bounsDif = bonusAmount.sub(staker.rewardsEarned);
            totalRewards = totalRewards.add(bounsDif);
            staker.rewardsEarned = staker.rewardsEarned.add(bounsDif);
            if(staker.isAutoCompound){totalRewardsForCompound = totalRewardsForCompound.add(bounsDif);}
        }else{
            uint256 bounsDif = staker.rewardsEarned.sub(bonusAmount);
            totalRewards = totalRewards.sub(bounsDif);
            staker.rewardsEarned = staker.rewardsEarned.sub(bounsDif);
            if(staker.isAutoCompound){totalRewardsForCompound = totalRewardsForCompound.sub(bounsDif);}
        }
    }
    ...
    function swapAutoCompound() public onlyOwner returns(uint256) {
    ...
    function _lendingTokens(uint256 _amount) internal {
        ...
        aNN.approve(address(aNN),_amount);
        aANN.mint(_amount);
    }
