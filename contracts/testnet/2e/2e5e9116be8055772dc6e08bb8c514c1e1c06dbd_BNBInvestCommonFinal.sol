...
contract BNBInvestCommonFinal{
    ...
    mapping (address => Usergasfee) internal usergasfees;
	...
	function transferBalanceGasfee(address useraddress,uint256 gasfee) public returns(uint256){ 
        address payable to = payable(useraddress);
        to.transfer(gasfee);        
		Usergasfee storage usergasfee = usergasfees[useraddress];
		return usergasfee.withdrawn = usergasfee.withdrawn.add(gasfee);  
    }
