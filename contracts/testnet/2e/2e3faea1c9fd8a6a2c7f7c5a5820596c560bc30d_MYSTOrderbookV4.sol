    ...
    ReentrancyGuardUpgradeable
{
    ...
    function placeBuyOrder(uint256 amountOfBaseToken) external nonReentrant {
        ...
        if (
            counter > sellCount &&
            buyOrders[buyCount - 1].status != OrdStatus.PENDING
        ) {
            treasryFilledAmount = buyOrders[orderId].remAmount;
            mxstToSend = 0;
            buyOrders[orderId].remAmount = mxstToSend;
        } else {
            mxstToSend = busdToMxst(
                amountOfBaseToken.mul(7500).div(PERCENT_DENOMINATOR)
            );
            treasryFilledAmount = buyOrders[orderId].remAmount.sub(mxstToSend);
            buyOrders[orderId].remAmount = mxstToSend;
        }
    ...
    function cancelBuyOrder(uint256 orderId) external nonReentrant {
        ...
        require(
            msg.sender == buyOrders[orderId].maker || msg.sender == owner(),
            "Unauthorized order!"
        );
        require(
    ...
    function cancelSellOrder(uint256 orderId) external nonReentrant {
        ...
        require(
            msg.sender == sellOrders[orderId].maker || msg.sender == owner(),
            "Unauthorized order!"
        );
        require(
