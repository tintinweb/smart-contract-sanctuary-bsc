...
contract LFX is ILFX, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = ILFX(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = ILFX(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        ILFX(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        ILFX(_token).transfer(msg.sender, _amount);
    }
