...
/// @notice The (older) MasterChef contract gives out a constant number of BERRY tokens per block.
/// It is the only address with minting rights for BERRY.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accberryPerShare) - user.rewardDebt
    ///
    ...
    ///     BERRY rewards each pool gets.
    /// `accberryPerShare` Accumulated CAKEs per share, times 1e12.
    ...
    ///     the BERRY rewards to all the PancakeSwap products.
    ...
    /// @notice Address of BERRY contract.
    IBEP20 public immutable BERRY;

    /// @notice The only address can withdraw all the burn BERRY.
    ...
    uint256 public constant MASTERCHEF_BERRY_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_BERRY_PRECISION = 1e18;
    ...
    uint256 public constant BERRY_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of BERRY burn action being executed.
    /// @notice BERRY distribute % for burn
    uint256 public berryRateToBurn = 643750000000;
    /// @notice BERRY distribute % for regular farm pool
    uint256 public berryRateToRegularFarm = 62847222222;
    /// @notice BERRY distribute % for special pools
    uint256 public berryRateToSpecialFarm = 293402777778;
    ...
    /// @param _BERRY The BERRY token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of BERRY.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn BERRY reward from current block in MCV1 pool
    ...
    /// only for BERRY distributions within PancakeSwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake BERRY token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != BERRY, "BERRY token can't be added to farm pools");
    ...
    /// @notice Update the given pool's BERRY allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending BERRY rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingBerry(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 berryReward = multiplier
                .mul(berryPerBlock(pool.isRegular))
                .mul(pool.allocPoint)
                .div(
                    (
                        pool.isRegular
            ...
            accberryPerShare = accberryPerShare.add(
                berryReward.mul(ACC_BERRY_PRECISION).div(lpSupply)
            );
        }
        ...
        uint256 boostedAmount = user
            ...
            .div(BOOST_PRECISION);
        return
            boostedAmount.mul(accberryPerShare).div(ACC_BERRY_PRECISION).sub(
                user.rewardDebt
            );
    }
    ...
    /// @notice Calculates and returns the `amount` of BERRY per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function berryPerBlock(bool _isRegular)
        public
        view
        returns (uint256 amount)
    {
        if (_isRegular) {
            amount = MASTERCHEF_BERRY_PER_BLOCK.mul(berryRateToRegularFarm).div(
                    BERRY_RATE_TOTAL_PRECISION
                );
        } else {
            amount = MASTERCHEF_BERRY_PER_BLOCK.mul(berryRateToSpecialFarm).div(
                    BERRY_RATE_TOTAL_PRECISION
                );
        }
    }
    /// @notice Calculates and returns the `amount` of BERRY per block to burn.
    function berryPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_BERRY_PER_BLOCK.mul(berryRateToBurn).div(
            BERRY_RATE_TOTAL_PRECISION
        );
    }
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                uint256 berryReward = multiplier
                    ...
                    .mul(pool.allocPoint)
                    .div(totalAllocPoint);
                pool.accberryPerShare = pool.accberryPerShare.add(
                    (berryReward.mul(ACC_BERRY_PRECISION).div(lpSupply))
                );
            }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingBerry(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user
            ...
            .mul(multiplier)
            .div(BOOST_PRECISION)
            .mul(pool.accberryPerShare)
            .div(ACC_BERRY_PRECISION);
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests BERRY from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send BERRY pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnBerry(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingBerryToBurn = multiplier.mul(berryPerBlockToBurn());

        // SafeTransfer BERRY
        _safeTransfer(burnAdmin, pendingBerryToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of BERRY distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of BERRY to burn each block.
    /// @param _regularFarmRate The % of BERRY to regular pools each block.
    /// @param _specialFarmRate The % of BERRY to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateBerryRate(
        uint256 _burnRate,
        ...
        berryRateToBurn = _burnRate;
        berryRateToRegularFarm = _regularFarmRate;
        berryRateToSpecialFarm = _specialFarmRate;

        emit UpdateBerryRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingBerry(_user, _pid, prevMultiplier);
    ...
    /// @notice Settles, distribute the pending BERRY rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingBerry(
        address _user,
        uint256 _pid,
        ...
        uint256 accCake = boostedAmount
            .mul(poolInfo[_pid].accberryPerShare)
            .div(ACC_BERRY_PRECISION);
    ...
    /// @notice Safe Transfer BERRY.
    /// @param _to The BERRY receiver address.
    /// @param _amount transfer BERRY amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough BERRY. If not, harvest from MCV1.
            if (BERRY.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = BERRY.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
