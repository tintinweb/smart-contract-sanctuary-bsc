...
{   
    ...
    constructor(IERC721 _NFT,IBEP20 _token)
    {
       NFT=_NFT;
       token=_token;
    }
    ...
    uint256 public  pkgNo = 2000;
    uint256 public firstprize = 10;
    uint256 public second_prize = 5;
    uint256 public third_prize = 3;
    ...
    function createBet(uint256 tokenId,address add) external 
    {
        ...
        require(block.timestamp > startTime+timetoRace , "time end" );
    ...
    {
        UserInfo[a].Getreward = firstprize;
        UserInfo[b].Getreward =second_prize ;
        UserInfo[c].Getreward = third_prize;
        ...
        token.transfer(y, firstprize);
        token.transfer(z, firstprize);
    ...
    function reward(address[] memory user) public{
        for(uint i=0;i<user.length;i++){
        token.transfer(user[i],1000000);
        roundedata[rounds].push(user[i]);
        }
