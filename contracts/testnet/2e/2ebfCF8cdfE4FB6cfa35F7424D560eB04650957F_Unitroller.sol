...
contract PriceOracle is PriceOracleProxyStorage, PriceOracleCore, OracleErrorReporter {
    ...
    function getUnderlyingType(address underlying) public returns (uint) {
        if (checkUniswapLP(underlying)) {
            return uint(UnderlyingType.UniswapV2LP);
        } else if (checkAsset(underlying)) {
    ...
    function checkAsset(address underlying) public view returns (bool) {       
        (address oracle,,) = searchPair(underlying);

        return bool(oracle != address(0));
    }

    function checkUniswapLP(address pool) public view returns (bool) {
        for (uint i = 0; i < priceOracles.length; i++) {
            (address pair,) = UniswapCommon(priceOracles[i]).searchPair(pool);

            if (address(pair) == address(pool)) {
                return true;
            }
        }
    ...
    function getPoolLiquidity(address pool) public view returns (uint112) {
        (,, uint112 maxLiquidity) = searchPair(pool);
        return maxLiquidity;
    }
}
...
contract PTokenFactory is PTokenFactoryStorageV1, FactoryErrorReporter {
    ...
    function createPToken(address underlying_) external returns (uint) {
        ...
        uint underlyingType_ = getOracle().getUnderlyingType(underlying_);
    ...
    function checkPair(address asset, uint underlyingType) public view returns (bool) {
        if (underlyingType == uint(PriceOracle.UnderlyingType.RegularAsset)) {
        ...
        } else if (underlyingType == uint(PriceOracle.UnderlyingType.UniswapV2LP)) {
            uint112 totalReserve = getOracle().getPoolLiquidity(asset);

            return bool(totalReserve >= minOracleLiquidity);
        } else {
            return false;
        }
    }
    ...
    function _createPTokenNameAndSymbol(address underlying_, uint underlyingType_) internal view returns (string memory, string memory) {
        if (underlyingType_ == uint(PriceOracle.UnderlyingType.RegularAsset)) {
            return _createPTokenNameAndSymbolForAsset(underlying_);
        } else if (underlyingType_ == uint(PriceOracle.UnderlyingType.UniswapV2LP)) {
