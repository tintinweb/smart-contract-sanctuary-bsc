...
contract Staking is Ownable, ReentrancyGuard {
  ...
  // Top staking
  mapping(address => UserStaking) public topStaking;

  // Unstake
  mapping(uint256 => StakeStatus) public unStaked;
  ...
  bool public isPoolFulled = false;
  bool public enabled = true;

  struct UserStaking {
    uint256 lastDepositeAt;
    uint256 lastDepositeAmount;
    uint256 lastDepositePoint;
  }
  ...
  StakeStatus useStakeStatus;
  /*** CONSTRUCTOR ***/
  constructor(address _token) {
    token = IERC20(_token);
    termToInterestRate[TERM] = 66;
  ...
  /// @dev Update Status Pool
  function setIsPoolFulled(bool _isPoolFulled) external onlyOwner {
    isPoolFulled = _isPoolFulled;
  }
  ...
  function stake(uint256 _amount) external nonReentrant {
    ...
    require(isPoolFulled == false , "The pool is full");
    require(enabled == true , "Staking is disabled");
    require(
      currentTimestamp >= startStakingAt && currentTimestamp <= endStakingAt,
      "Staking is not availabe at this time"
    );
    require(_amount >= 1e18, "Amount must be greater than 0");
    totalDNLStaked = totalDNLStaked.add(_amount);
    uint256 currentId = _tokenIdCounter.current();
    _idToAddress[currentId] = msg.sender;
    idToTerm[currentId] = TERM;
    idToStartTime[currentId] = currentTimestamp;
    ...
    computeTopStaking(currentTimestamp, _amount, msg.sender);
    unStaked[currentId] = StakeStatus.Staking;
    _tokenIdCounter.increment();
    emit Stake(msg.sender, currentId, TERM, currentTimestamp, _amount);
  }

  /// @dev compute deposite point
  function computeTopStaking (uint256 _depositeAt, uint256 _depositeAmount, address _address)
    private
  {
    // this line for test since timestamp local chain not true
    // _depositeAt = block.timestamp + _depositeAt;
    uint256 lastDepositeAmount = topStaking[_address].lastDepositeAmount;
    uint256 lastDepositeAt = topStaking[_address].lastDepositeAt;
    uint256 lastDepositePoint = topStaking[_address].lastDepositePoint;
    // first time staking
    if(lastDepositeAmount == 0) {
      lastDepositeAt = startStakingAt;
      lastDepositePoint = 0;
      lastDepositeAmount = 0;
    }
    if(_depositeAt > endStakingAt) {
      _depositeAt = endStakingAt;
    }
    uint256 durationInSeconds = _depositeAt.sub(lastDepositeAt).div(60 * 60);
    uint256 lastAmount = _depositeAmount.sub(_depositeAmount.mod(100));
    uint256 amount = lastAmount.mul(10).div(100);
    lastDepositePoint = lastDepositePoint.add(durationInSeconds.mul(amount));
    topStaking[_address] = UserStaking(_depositeAt, _depositeAmount, lastDepositePoint);
  }
    ...
    returns (uint256)
  {
    ...
    return idToBalance[_id].mul(TERM).mul(idToInterestRate[_id]).div(ONE_DAY_IN_SECONDS).div(10000);
  }
    ...
    )
  {
    ...
    dailyAPR = idToInterestRate[_id];
    return (amount, currentInterest, startAt, endAt, dailyAPR);
  }
  ...
  function getStakeHolders() external view returns (uint256) {
    ...
    returns (address[] memory, uint256[] memory)
  {
    ...
    uint256[] memory lastDepositePoint = new uint256[](_addresses.length);
    for (uint256 i = 0; i < _addresses.length; ++i) {
      uint256 staked = 0;
    ...
    returns (bool)
  {
    ...
    require(
      totalBalance - _amount >= totalDNLStaked,
