...
contract unisalePresale {
   ...
   function refundTokens(uint _id, address _poolHolder) external payable returns (bool) {
      require(presales[_id].pool == msg.sender, "The caller must be one pool.");
      ...
      uint256 _amount = IERC20(presales[_id].tokenCA).balanceOf(_poolHolder);
      // paying tokens to presales owner 
      bool _pay = IERC20(presales[_id].tokenCA).transferFrom(msg.sender, _poolHolder, _amount);
      // check all steps for sure 
      return _pay;
   }
