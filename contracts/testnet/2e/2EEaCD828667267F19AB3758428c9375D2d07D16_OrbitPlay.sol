...
contract OrbitPlay is Ownable, ReentrancyGuard {
    ...
    event SpinWheelResult(address bidder, uint256 betAmount, string betPlace, string result, uint256 returned);
    ...
    function placeSpinBet(uint256 amount, uint256 betPlace) public { //betPlace 1: Red 2: Yellow: Red 3: Green: Red 4: Blue
		...
		if(destiny == betPlace){ //win (1: heads 2: tails)
			...
			userInfo.returningAmount = 0;
            uint256 burnAmount = amount.mul(spinBurnOnLossPercentage).div(10000);
            IERC20(tokenAddress).safeTransfer(DeadAddress, burnAmount);
            updateSpinInfo(0, burnAmount);
		}        
        
        emit SpinWheelResult(msg.sender, amount, getSpinPlaceFromNo(betPlace), getSpinPlaceFromNo(destiny), userInfo.returningAmount);        
	}
