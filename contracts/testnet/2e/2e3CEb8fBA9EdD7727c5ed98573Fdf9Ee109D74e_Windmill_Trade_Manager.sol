...
contract Windmill_Trade_Manager is Base, Math, Adressable, AccessControl, With_DAORole, Terminatable, With_UpdaterContract, With_CompetitionContract,
                                   With_StackingContract, With_FundContract, With_DAOAddress, With_BUSDToken, With_PWRToken, With_WithdrawerContract, I_Windmill_Trade_Manager{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function claimPaperEnergy() external{
        require(maxPaperEnergy[msg.sender] == 0, "Windmill_Trade_Manager: Paper energy already claimed.");
        require(maxEnergy[msg.sender] == 0, "Windmill_Trade_Manager: Real trader cannot claim paper energy.");
        require(autoPromotionEnabled, "Windmill_Trade_Manager: Autopromotion systemm is not enabled.");
    
        I_Windmill_DAO DAO = I_Windmill_DAO(DAOAddress);
        I_Windmill_DAO.DAOLevel memory data = DAO.getUserData(msg.sender);
        ...
        require(nbPWR >= minPWRToStartPaperTrade, "Windmill_Trade_Manager: Not enough PWR to claim paper energy.");
        require(!investorWhitelistEnabled || (data.isInvestor && investorWhitelistEnabled), "Windmill_Trade_Manager: Only investors can claim paper energy.");
    ...
    function startTrade(uint256 deployerId, uint256 nbEnergy) external onlyNotTerminated returns (uint256){
        ...
        require(!isPaper || autoPromotionEnabled, "Windmill_Trade_Manager: Cannot start paper trade when auto-promotion system disabled.");
        
        require(deployer.enabled, "Windmill_Trade_Manager: Trade deployer disabled.");
        
        if (isPaper){
            require(getMaxEnergy(msg.sender) == 0, "Windmill_Trade_Manager: Real trader cannot start paper trades.");
            require(nbEnergy <= getRemainingPaperEnergy(msg.sender), "Windmill_Trade_Manager: Not enough remaining energy.");
            require(nbEnergy >= 10, "Windmill_Trade_Manager: Not enough energy to start a trade.");
            
            uint256 nbPWR = PWRToken.balanceOf(msg.sender) + stacking.stackedPWR(msg.sender);
            require(nbPWR >= minPWRToStartPaperTrade, "Windmill_Trade_Manager: Not enough PWR to start paper trade.");
            
        }else{
            require(getMaxPaperEnergy(msg.sender) == 0, "Windmill_Trade_Manager: Paper trader cannot start real trades.");
