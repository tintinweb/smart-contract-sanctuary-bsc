...
contract Guild is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public purchaseMarketingFee = 1;
    uint256 public purchaseTreasuryFee = 5;
    ...
    constructor() public ERC20("Guild", "Guild") {
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[from]) {
            uint256 mfee = amount.mul(purchaseMarketingFee).div(100);
            uint256 tfee = amount.mul(purchaseTreasuryFee).div(100);
            uint totalfee = mfee.add(tfee);
        	amount = amount.sub(totalfee);  

            pendingTreasuryFees = pendingTreasuryFees.add(tfee);
            pendingmarketingFees = pendingmarketingFees.add(mfee);
            // super._transfer(from, marketingAddress, mfee);
            // super._transfer(from, treasuryAddress, tfee);
        ...
        if (automatedMarketMakerPairs[to]) {
            ...
            uint256 tfee = amount.mul(sellTreasuryFee).div(100);
            uint totalfee = mfee.add(tfee);
        	amount = amount.sub(totalfee);  
