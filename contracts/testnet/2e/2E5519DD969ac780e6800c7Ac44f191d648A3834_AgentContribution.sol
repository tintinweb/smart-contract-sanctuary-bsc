...
interface IAgentContribution{
  function depositAgent1(address token,address payable user,uint256 amount)external payable;
  function depositAgent2(address token,address payable user,uint256 amount)external payable;
  function depositAgent3(address token,address payable user,uint256 amount)external payable;
  function depositAgent4(address token,address payable user,uint256 amount)external payable;
}
...
contract Referral{
   ...
   */
  function payReferral(uint256 value,address token) internal returns(uint){
    ...
    for (uint i; i < levelRate.length; i++) {
      ...
      if (parent == address(0)) {
        ...
        RewardPool.recorduserreward(parent,c);
    ...
    uint remaining=0;
      remaining=remaining.add(value).sub(totalReferal);
      IBEP20(token).transfer( address(RewardPool),totalReferal);
      IBEP20(token).transfer(management,remaining);

    // updateActiveTimestamp(msg.sender);
    return totalReferal;
  }
  function claimpool(address token) public onlyOperator{
    
    IBEP20(token).transfer(msg.sender,totalpools);
    totalpools=0;
  ...
  function changeuserInfo(address payable user,address payable _referrer, uint _reward, uint _referredCount, uint depositAmount) public onlyOperator{
...
contract AgentContribution is ReentrancyGuard,IAgentContribution,Referral,Pausable{
  using SafeBEP20 for IBEP20;
  ...
  EnumerableMap.UintToAddressMap private holderMap;
  ...
  function addPackage(uint256 amount,address token) public onlyOperator{
  ...
  function addHolderMap(uint256 index,address _user)public returns(bool){
    require(_user!=address(0),'Address cannot be empty');
    return EnumerableMap.set(holderMap,index,_user);
  }

  function removeHolderMap(uint256 index)public onlyOperator returns(bool){
    return EnumerableMap.remove(holderMap,index);
  }

  function holderMapLength()public view returns(uint256){
    return EnumerableMap.length(holderMap);
  }

  function containsHolderMap(uint256 index)public view returns(bool){
    return EnumerableMap.contains(holderMap,index);
  }

  function getHolderMap(uint256 index)public view returns(address holder){
    return EnumerableMap.get(holderMap,index);
  }
  
  function depositAgent1(address token,address payable referrer,uint256 amount)public override payable {
    require(msg.value>=fees,'Fees is zero!');
    payable(feeAddr).transfer(msg.value);
    (bool stat)=checkPackage(amount,token);
    require(accounts[msg.sender].referrer==address(0),'Already Purchased');
    require(stat,'Token is not accepted!');
    IBEP20(token).transferFrom(msg.sender, address(this), amount);
    bool status=addReferrer(referrer);
    if(status==true){
      id++;
      string memory randomID = randomAgentName(id,msg.sender,referrer);
      holderlist[id]=msg.sender;
      accounts[msg.sender].userID = randomID;
      accounts[msg.sender].depositAmount = accounts[msg.sender].depositAmount.add(amount);
      payReferral(amount,token);
      emit AgentRecorded(msg.sender, amount, block.timestamp);
    }
  }
  function depositAgent2(address token,address payable referrer,uint256 amount)public override payable {
    require(msg.value>=fees,'Fees is zero!');
    payable(feeAddr).transfer(msg.value);
    (bool stat)=checkPackage(amount,token);
    require(accounts[msg.sender].referrer==address(0),'Already Purchased');
  ...
  function depositAgent3(address token,address payable referrer,uint256 amount)public override payable {
    require(msg.value>=fees,'Fees is zero!');
    payable(feeAddr).transfer(msg.value);
    (bool stat)=checkPackage(amount,token);
    require(accounts[msg.sender].referrer==address(0),'Already Purchased');
  ...
  function depositAgent4(address token,address payable referrer,uint256 amount)public override payable {
    require(msg.value>=fees,'Fees is zero!');
    payable(feeAddr).transfer(msg.value);
    (bool stat)=checkPackage(amount,token);
    require(accounts[msg.sender].referrer==address(0),'Already Purchased');
    ...
    if(status==true){
      userCount.increment();
      uint256 id = getCount(2);
      addHolderMap(id,msg.sender);
      payReferral(amount,token);
      emit AgentRecorded(msg.sender, amount, block.timestamp);
    }
  }
