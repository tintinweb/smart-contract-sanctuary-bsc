 ...
 * @dev A base perpetual staking with 30 days lock period.
 * It locks the user's $TOKEN token and mint stTOKEN to them. Users may
 * unstake their $TOKEN before lock period, but they need to pay the
 * penalty amount. The penalty amount will be subtracted from user's staked
 * amount and be sent to the treasury.
 **/
// solhint-disable not-rely-on-time
contract BaseStaking is ERC20VotesComp, Ownable, ReentrancyGuard {
    ...
    function stakeFor(address _user, uint128 _amount) external nonReentrant {
        require(msg.sender == distributor, "Only distributor can stake");
     ...
     */
    function unstake(uint128 _stakeIndex) external nonReentrant {
        require(_stakeIndex < stakers[msg.sender].length, "Invalid index");

        Stake memory stakeRef = stakers[msg.sender][_stakeIndex];
        ...
        if (
            stakeRef.stakedTimestamp + stakeRef.penaltyDays * 86400 >
     ...
     * @param amount amount of stakes to return
     * @return values array of stakes
    ...
    function getStakes(
        ...
        uint256 cursor,
        uint256 amount
    ) public view returns (Stake[] memory values, uint256 newCursor) {
        uint256 length = amount;
        Stake[] memory stakes = stakers[_address];

        newCursor = cursor + length;

        if (length > stakes.length - cursor) {
            length = stakes.length - cursor;
            newCursor = 0;
        }

        values = new Stake[](length);
        for (uint256 i = 0; i < length; i++) {
            values[i] = stakes[cursor + i];
        }
    }

    /**
     * @notice Get all stakes for a user. it could brake if
     * too much stakes (probably won't happen)
     * @param _address address of user
    ...
    /**
     * @notice Removes stake from the array
     * @param _staker the address of the beneficiary
     * @param _stakeIndex the index of the stake position
     */
    function _removeStake(address _staker, uint128 _stakeIndex) private {
        Stake[] storage stakes = stakers[_staker];
        uint256 stakesLength = stakes.length;
        Stake memory lastStake = stakes[stakesLength - 1];

        lastStake.stakeIndex = _stakeIndex;
        stakes[_stakeIndex] = lastStake;
        stakes.pop();
    }

    /**
     * @notice Creates stake from the array
    ...
    function _stake(address _staker, uint256 _amount) private {
        ...
        _delegate(_staker, _staker);

        uint256 stakeIndex = stakers[_staker].length;

        stakers[_staker].push(
            Stake({
                amount: _amount,
                stakedTimestamp: uint128(block.timestamp),
                stakeIndex: uint128(stakeIndex),
