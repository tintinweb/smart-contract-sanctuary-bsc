...
contract TreeFiddy is ERC20, Ownable {
        ...
        if (isExcluded) {
            ...
            if (isBuy || isSell) {
                ...
                if (isBuy) {
                    ...
                    require(
                        (balanceOf(to) + amount) <= getMaxWalletSize(),
                        "Balance + amount cannot exceed 1 % of circulating supply."
                    );
                    //May Buy
                    require(
                        amount <= _maxWalletBuy,
                        "Amount cannot exceed max wallet buy of tree fiddy."
                    );
                    lPTaxAmount = (amount * _lpTaxBuy) / 10000;
                    // Reset sell delay
                } else if (isSell) {
                    lPTaxAmount = (amount * _lpTaxSell) / 10000;
                    require(
    ...
    function getMaxWalletSize() public view returns (uint256) {
        return totalSupply() * _maxWalletSize / 10000;
    }
