...
contract Promote is Member {
    ...
    function updateLevel(address user) internal view returns (uint8){
        ...
        uint256 amount = userInfo[user].depositAsUsdt;
         for(uint8 i = 4; i > 0;i--) {
             level1 = amount >= amountThreshould[i - 1]? i : 0;
             level2 = userInfo[user].numSS >= numThresholdSS[i-1] ? i : 0;
              level3 = userInfo[user].numGS >= numThresholdGS[i-1] ? i : 0;
         }
    ...
    function updateUserBouns(address _user) public {
        ...
        for( uint8 i = 0; i < 3; i++) {
            if(_newGenBonusArray[i] != _oldGenBonusArray[i]) {
                ...
                uint256 _newBounsWeight = _newGenWeightArray[i].mul(_newGenWeightArray[i].add(100)).div(100);
