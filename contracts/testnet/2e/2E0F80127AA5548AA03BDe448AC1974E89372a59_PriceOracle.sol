...
contract PriceOracle {
    ...
    function priceOfToken(address token) public view returns (uint256) {
        return ((block.number * 10**16 + uint160(token)) % (5 * 10**18)) + 10**16;
    }

    function priceOfBNB() public pure returns (uint256) {
        return 300 * 10**18;
    }

    function _lpAmountsForToken(address token) internal view returns (uint256, uint256) {
        return (((block.timestamp * 10**18 + uint160(token)) % (5 * 10**18)) + 10**16, ((block.number * 10**16 + uint160(token)) % (5 * 10**18)) + 10**16);
    }
}