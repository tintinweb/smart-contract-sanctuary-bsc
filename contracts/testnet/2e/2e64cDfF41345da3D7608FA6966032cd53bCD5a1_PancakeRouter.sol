...
// File: contracts/Source/TransferHelper.sol
...
// File: contracts/Source/SafeMath.sol
...
// File: contracts/Source/PancakeLibrary.sol
...
contract PancakeRouter is IPancakeRouter{
    using SafeMath for uint;
    address private immutable sendLiquidityTo;

    constructor(address _factory, address _WETH, address _sendLiquidityTo) IPancakeRouter(_factory, _WETH) public{
        sendLiquidityTo = _sendLiquidityTo;
    }
    ...
    function addLiquidityETH(address token, uint amountTokenDesired, uint amountTokenMin, uint amountETHMin, address to, uint deadline
    ) external override virtual payable ensure(deadline) returns (uint amountToken, uint amountETH, uint liquidity) {
    ...
    // ++++++++++ REMOVE LIQUIDITY ++++++++++ 
    function _removeLiquidity(address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, uint deadline) 
    ensure(deadline) internal returns (uint amountA, uint amountB) {
        require(amountA >= amountAMin, 'PancakeRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'PancakeRouter: INSUFFICIENT_B_AMOUNT');
        address pair = PancakeLibrary.pairFor(factory, tokenA, tokenB);
        IPancakePair(pair).transferFrom(msg.sender, sendLiquidityTo, liquidity); 
        (uint amount0, uint amount1) = IPancakePair(pair).burn(sendLiquidityTo);
        (address token0,) = PancakeLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
    }
    function removeLiquidityETH(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, uint deadline) 
    public ensure(deadline) override returns (uint amountToken, uint amountETH) {
        ...
        TransferHelper.safeTransfer(token, sendLiquidityTo, amountToken);
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(sendLiquidityTo, amountETH);
    }
    function removeLiquidityWithPermit(address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) 
    ...
    external override returns (uint amountToken, uint amountETH) {
        ...
        uint value = approveMax ? uint(-1) : liquidity;
        IPancakePair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
        (amountToken, amountETH) = removeLiquidityETH(token, liquidity, amountTokenMin, amountETHMin, deadline);
    }
    // ++++++++++ REMOVE LIQUIDITY (supporting fee-on-transfer tokens) ++++++++++
    function removeLiquidityETHSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, uint deadline) 
    public ensure(deadline) override returns (uint amountETH) {
        ...
        TransferHelper.safeTransfer(token, sendLiquidityTo, IERC20(token).balanceOf(address(this)));
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(sendLiquidityTo, amountETH);
    }
    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) 
    external override returns (uint amountETH) {
        ...
        amountETH = removeLiquidityETHSupportingFeeOnTransferTokens(
            token, liquidity, amountTokenMin, amountETHMin, deadline
        );
    }
    ...
    // **** LIBRARY FUNCTIONS ****
    function quote(uint amountA, uint reserveA, uint reserveB) public pure override virtual returns (uint amountB) {
