    ...
    IStakingProtocol
{
        ...
        initializer
    {
        ...
        __init_SuspensionPeriods(
            _rewardDelayPeriod
        );
        __init_FullAmountCalculator();
...
////////////////////////////////////////// oracle - helpers functions
        ...
        public
    {
        ...
        _updateAllStakingRelatedAssets();
    }
...
// todo вернуть взад onlyStableCoin
    function runStaking()
        public
//        onlyStableCoin
    {
...
//        onlyStableCoin
    ...
    function onStableCoinRebase()
        public
        onlyStableCoin
    {
        getOracle().updateHistoryOnStableCoinRebase();
    }
...
interface IStakingProtocol {
    ...
    function onStableCoinRebase() external;
}
...
abstract contract SuspensionPeriods
{
    ...
    function __init_SuspensionPeriods(
        uint256 _rewardDelayPeriod
    ) internal {
...
////////////////////////////////////////// private methods write
    ...
    function __startNewSuspensionPeriod(uint256 _suspensionPeriodBegin)
        private
    {
    ...
    function __finishLastSuspensionPeriod()
        private
    {
        if (suspensionPeriods.length == 0) {
            return;
...
////////////////////////////////////////// internal methods write
    ...
    function _runStaking()
        internal
    {
        __finishLastSuspensionPeriod();
    }
    function _disableStaking()
        internal
    {
        __startNewSuspensionPeriod(block.timestamp);
    }
    function _rewardAnnulation()
        internal virtual
    {
        __startNewSuspensionPeriod(block.timestamp - rewardDelayPeriod);
    }
}
...
////////////////////////////////////////// structs
    ...
    struct AccountPeriodInfo {
        uint256 timeBegin;
        EnumerableMapUpgradeable.AddressToUintMap assetsAmounts; // syf/stablecoin/lp_token/flat_Nft_Obligation_value
...
////////////////////////////////////////// fields definition
    ...
    // Account address => syf/stablecoin/lp_token => amount
...
////////////////////////////////////////// view methods
        ...
        returns(uint256 _amountInBase)
    {
        ...
        for (uint256 i = 0; i < _accountPeriodInfo.assetsAmounts.length(); ++i) {
            (address _asset, uint256 _amount) = _accountPeriodInfo.assetsAmounts.at(i);
            _amountInBase += getOracle().consultByHistory(_asset, _accountPeriodInfo.timeBegin, block.timestamp)
                * _amount
        ...
        DoubleMath.Double memory smartNFTBoostCoefficient = __getSmartNFTBoostCoefficientForPeriod(_accountPeriodInfo, block.timestamp);
        if (!smartNFTBoostCoefficient.isZero()) {
        ...
        returns(DoubleMath.Double memory)
    {
        ...
        if (
            _accountPeriodInfo.smartNFTCreatedAt == 0
        ) {
        ...
        if (_accountPeriodInfo.smartNFTLastTime >= timeEnd) {
            return DoubleMath.fraction(_accountPeriodInfo.smartNFTBoostCoefficient, PERCENT);
        }

        uint256 periodFull = timeEnd - _accountPeriodInfo.timeBegin;
        if (periodFull == 0) {
            return DoubleMath.Double({mantissa: 0});
        }

        uint256 periodWithBoost = _accountPeriodInfo.smartNFTLastTime - _accountPeriodInfo.timeBegin;
...
////////////////////////////////////////// assets helpers
    ...
    function _updateAllStakingRelatedAssets()
        internal
    {
        address[] memory assets = _getAllAssets();

        uint256 _count = assets.length;
        for (uint256 i = 0; i < _count; ++i) {
            getOracle().updateHistory(assets[i]);
        }
    }
    ...
    function _getAllAssets()
        ...
        returns (address[] memory)
    {
        uint256 countInStaking = assetWithRewardCoefficients.length();
        uint256 count = countInStaking;

        for (uint256 i = 0; i < countInStaking; ++i) {
            (, uint256 _rewardCoefficient) = assetWithRewardCoefficients.at(i);
        ...
        uint256 countNFTBond = _assetsInNFTBond.length;

        count += countNFTBond;
        address[] memory _assets = new address[](count);

        uint256 index = 0;
        for (; index < countNFTBond; ++index) {
            _assets[index] = _assetsInNFTBond[index];
        }
        for (uint256 i = 0; i < countInStaking; ++i) {
...
////////////////////////////////////////// deposit/withdraw
    ...
    function __recalculateFullBalance(address _account) internal virtual {}
}
...
////////////////////////////////////////// methods for full balance
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        _rewardInDelay = 0;
        (uint256 _cumulativeFullBalance, uint256 _cumulativeAvailableBalance, uint256 _periodFull, uint256 _periodAllowedToClaim) = __getCumulativeFullBalancesInBaseAndPeriods();

        if (_periodFull == 0 || _cumulativeFullBalance == 0) {
        ...
        _rewardInDelay = CompoundPercent.compound(_cumulativeFullBalance / _periodFull, _totalPercent, _periodFull);

        if (rewardDelayPeriod >= _periodFull || _cumulativeAvailableBalance == 0 || _periodAllowedToClaim == 0) {
            return (_rewardAllowedToClaim, _rewardInDelay);
        }

        _rewardAllowedToClaim = CompoundPercent.compound(_cumulativeAvailableBalance / _periodAllowedToClaim, _totalPercent, _periodAllowedToClaim);

        require(_rewardInDelay >= _rewardAllowedToClaim, 'PC::2');
        _rewardInDelay -= _rewardAllowedToClaim;
        require(_rewardAllowedToClaim >= claimedTotal, 'PC::3');
...
////////////////////////////////////////// methods for calculate individual pending
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        require(_rewardInDelay >= _rewardAllowedToClaim,
            string.concat(
                'PC::3',
                ' 0: ', _rewardInDelay.toString(),
                ' 1: ', _rewardAllowedToClaim.toString(),
                ''
            )
        );
        _rewardInDelay -= _rewardAllowedToClaim;
...
abstract contract FullAmountCalculator is
    StakingStorage,
    SuspensionPeriods
{
    using SafeERC20Upgradeable for IERC20MetadataUpgradeable;
...
////////////////////////////////////////// initialize
    ...
    function __init_FullAmountCalculator(
    ) internal {
...
////////////////////////////////////////// fields definition
    ...
    uint256 private stakingStartDate;
    uint256 private stakingLastUpdateDate;
    uint256 private stakingPeriod;

    EnumerableMapUpgradeable.AddressToUintMap private cumulativeFullAssetsAmounts;
    EnumerableMapUpgradeable.AddressToUintMap private lastFullAssetsAmounts;
...
////////////////////////////////////////// view methods
    ...
    function __getCumulativeFullBalancesInBaseAndPeriods()
        internal
        view
        returns(uint256 _cumulativeFullBalance, uint256 _cumulativeAvailableBalance, uint256 _periodFull, uint256 _periodAllowedToClaim)
    {
        _cumulativeFullBalance = 0;
        _cumulativeAvailableBalance = 0;
        
        uint256 allowedToClaimStartDate = block.timestamp - rewardDelayPeriod;

        address[] memory assets = _getAllAssets();
        if (stakingLastUpdateDate <= allowedToClaimStartDate) {
            // -----stakingLastUpdateDate---allowedToClaimStartDate------block.timestamp
            uint256 periodBeforeAllowedToClaimDate = _calculateNonSuspensionPeriod(stakingLastUpdateDate, allowedToClaimStartDate);
            uint256 periodAfterAllowedToClaimDate = _calculateNonSuspensionPeriod(allowedToClaimStartDate, block.timestamp);

            _periodAllowedToClaim = stakingPeriod + periodBeforeAllowedToClaimDate;
            _periodFull = _periodAllowedToClaim + periodAfterAllowedToClaimDate;

            if (_periodAllowedToClaim == 0 && _periodFull == 0) {
                return (_cumulativeFullBalance, _cumulativeAvailableBalance, _periodFull, _periodAllowedToClaim);
            }

            for (uint256 i = 0; i < assets.length; ++i) {
                address asset = assets[i];
                uint256 rewardCoefficient = __getRewardCoefficientForAmountInBase(asset);

                uint256 cumulativeAmountInBase = 0;
                uint256 oneToken = 10 ** IERC20MetadataUpgradeable(asset).decimals();


                (, uint256 cumulativeFullAssetAmount) = cumulativeFullAssetsAmounts.tryGet(asset);
                if (cumulativeFullAssetAmount != 0) {
                    cumulativeAmountInBase += cumulativeFullAssetAmount * getOracle().consultByHistory(asset, stakingStartDate, stakingLastUpdateDate);
                }

                (, uint256 lastFullAssetAmount) = lastFullAssetsAmounts.tryGet(asset);
                if (lastFullAssetAmount != 0 && periodBeforeAllowedToClaimDate != 0) {
                    cumulativeAmountInBase += lastFullAssetAmount * periodBeforeAllowedToClaimDate * getOracle().consultByHistory(asset, stakingLastUpdateDate, allowedToClaimStartDate);
                }

                _cumulativeAvailableBalance += cumulativeAmountInBase * rewardCoefficient / PERCENT / oneToken;


                if (lastFullAssetAmount != 0 && periodAfterAllowedToClaimDate != 0) {
                    cumulativeAmountInBase += lastFullAssetAmount * periodAfterAllowedToClaimDate * getOracle().consultByHistory(asset, allowedToClaimStartDate, block.timestamp);
                }
                _cumulativeFullBalance += cumulativeAmountInBase * rewardCoefficient / PERCENT / oneToken;
            }

            return (_cumulativeFullBalance, _cumulativeAvailableBalance, _periodFull, _periodAllowedToClaim);
        }

        // stakingLastUpdateDate > allowedToClaimStartDate
        {
            // -----allowedToClaimStartDate----stakingLastUpdateDate-----block.timestamp
            uint256 periodBeforeLastUpdateDate = _calculateNonSuspensionPeriod(allowedToClaimStartDate, stakingLastUpdateDate);
            uint256 periodAfterLastUpdateDate = _calculateNonSuspensionPeriod(stakingLastUpdateDate, block.timestamp);

            require(stakingPeriod >= periodBeforeLastUpdateDate, 'FAC::1');
            _periodAllowedToClaim = stakingPeriod - periodBeforeLastUpdateDate;
            _periodFull = stakingPeriod + periodAfterLastUpdateDate;
            ...
            for (uint256 i = 0; i < assets.length; ++i) {
                ...
                uint256 cumulativeAmountInBaseForFull = 0;
                uint256 cumulativeAmountInBaseForAvailable = 0;

                {
                    (, uint256 cumulativeFullAssetAmount) = cumulativeFullAssetsAmounts.tryGet(asset);
                    if (stakingPeriod != 0 && cumulativeFullAssetAmount != 0) {
                        uint256 cumulativeAmountBeforeLastUpdateDate = cumulativeFullAssetAmount / stakingPeriod * periodBeforeLastUpdateDate;

                        require(cumulativeFullAssetAmount >= cumulativeAmountBeforeLastUpdateDate,
                            string.concat(
                                'FAC::2',
                                ' cumulativeFullAssetAmount: ', cumulativeFullAssetAmount.toString(),
                                ' cumulativeAmountBeforeLastUpdateDate: ', cumulativeAmountBeforeLastUpdateDate.toString(),
                                ' periodBeforeLastUpdateDate: ', periodBeforeLastUpdateDate.toString(),
                                ''
                            )
                        );
                        cumulativeAmountInBaseForAvailable = (cumulativeFullAssetAmount - cumulativeAmountBeforeLastUpdateDate) * getOracle().consultByHistory(asset, stakingStartDate, allowedToClaimStartDate);
                        cumulativeAmountInBaseForFull = cumulativeAmountInBaseForAvailable + cumulativeAmountBeforeLastUpdateDate * getOracle().consultByHistory(asset, allowedToClaimStartDate, stakingLastUpdateDate);
                    }

                    (, uint256 lastFullAssetAmount) = lastFullAssetsAmounts.tryGet(asset);
                    if (lastFullAssetAmount != 0 && periodAfterLastUpdateDate != 0) {
                        cumulativeAmountInBaseForFull += lastFullAssetAmount * periodAfterLastUpdateDate * getOracle().consultByHistory(asset, stakingLastUpdateDate, block.timestamp);
                    }
                }

                uint256 oneToken = 10 ** IERC20MetadataUpgradeable(asset).decimals();
                _cumulativeFullBalance += cumulativeAmountInBaseForFull * rewardCoefficient / PERCENT / oneToken;
                _cumulativeAvailableBalance += cumulativeAmountInBaseForAvailable * rewardCoefficient / PERCENT / oneToken;
            }
...
////////////////////////////////////////// fullBalance
    ...
    function __recalculateFullBalance(address _account)
        internal virtual override
    {
        ...
        DoubleMath.Double memory _previousOldBoostCoefficient = __getSmartNFTBoostCoefficientForPeriod(_previousAccountPeriodInfo, _previousAccountPeriodInfo.timeBegin);
        DoubleMath.Double memory _previousNewBoostCoefficient = __getSmartNFTBoostCoefficientForPeriod(_previousAccountPeriodInfo, block.timestamp);
        DoubleMath.Double memory _currentBoostCoefficient = __getSmartNFTBoostCoefficientForPeriod(_currentAccountPeriodInfo, _currentAccountPeriodInfo.timeBegin);

        uint256 period = _calculateNonSuspensionPeriod(stakingLastUpdateDate, block.timestamp);
        ...
        for (uint256 i = 0; i < count; ++i) {
            ...
            // prolong previousCumulativeBalance to current time
            (, uint256 lastFullAssetAmount) = lastFullAssetsAmounts.tryGet(asset);
            cumulativeFullAssetsAmounts.add(asset, lastFullAssetAmount * period);

            // apply diff in cumulativeFullAssetsAmounts by previousAmount NewBoostCoefficient change
            (,uint256 previousAmount) = _previousAccountPeriodInfo.assetsAmounts.tryGet(asset);

            uint256 previousOldBoostedAmount = _previousOldBoostCoefficient.isZero()
                ? previousAmount
                : DoubleMath.mul_(previousAmount, _previousOldBoostCoefficient);

            if (_previousOldBoostCoefficient.mantissa != _previousNewBoostCoefficient.mantissa) {
                uint256 previousNewBoostedAmount = _previousNewBoostCoefficient.isZero()
                    ? previousAmount
                    : DoubleMath.mul_(previousAmount, _previousNewBoostCoefficient);

//                require(
//                    previousOldBoostedAmount >= previousNewBoostedAmount,
//                    string.concat(
//                        'FullAmountCalculator::__recalculateFullBalance - '
//                        ' previousOldBoostedAmount: ', previousOldBoostedAmount.toString() , ';',
//                        ' previousNewBoostedAmount: ', previousNewBoostedAmount.toString() , ';',
//                        ''
//                    )
//                );

                cumulativeFullAssetsAmounts.sub(asset, (previousOldBoostedAmount - previousNewBoostedAmount) * period);
            }


            // apply diff in lastFullAssetsAmounts by currentAmount
            (,uint256 currentAmount) = _currentAccountPeriodInfo.assetsAmounts.tryGet(asset);
            uint256 currentBoostedAmount = _currentBoostCoefficient.isZero()
                ? currentAmount
                : DoubleMath.mul_(currentAmount, _currentBoostCoefficient);

            if (previousOldBoostedAmount == currentBoostedAmount) {
                continue;
            }

            lastFullAssetsAmounts.sub(asset, previousOldBoostedAmount);
            lastFullAssetsAmounts.add(asset, currentBoostedAmount);
        }
        ...
        internal virtual override
    {
        ...
        uint256 periodAllowedToClaimStart = block.timestamp - rewardDelayPeriod;

        if (periodAllowedToClaimStart > stakingLastUpdateDate) {
            uint256 period = _calculateNonSuspensionPeriod(stakingLastUpdateDate, periodAllowedToClaimStart);
            if (period > 0) {
                for (uint256 i = 0; i < count; ++i) {
                    address asset = assets[i];
                    // prolong previousCumulativeBalance to current time
        ...
        if (periodAllowedToClaimStart < stakingLastUpdateDate) {
            uint256 period = _calculateNonSuspensionPeriod(periodAllowedToClaimStart, stakingLastUpdateDate);
            ...
            if (period > 0) {
                for (uint256 i = 0; i < count; ++i) {
                    ...
                    // prolong previousCumulativeBalance to current time
                    (, uint256 cumulativeFullAmount) = cumulativeFullAssetsAmounts.tryGet(asset);
                    cumulativeFullAssetsAmounts.sub(asset, cumulativeFullAmount / stakingPeriod * period);
                }
