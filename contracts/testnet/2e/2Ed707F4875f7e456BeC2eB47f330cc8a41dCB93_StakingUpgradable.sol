    ...
    PausableUpgradeable
{
    ...
    function GetStakedAmont(address _address) public view returns (uint256) {
        return StakedAmount[_address];
    }

    function GetReward(address _address) public view returns (uint256) {
        if (StakedAmount[msg.sender] > 0) {
            uint256 CurrentStakedAmount = StakedAmount[_address];
            return
                CurrentStakedAmount +=
                    (_min(block.timestamp, StakedInfoMap[msg.sender].EndTime) -
                        StakedInfoMap[msg.sender].StartTime) *
                    10**TokenDecimals;
        }
    ...
    function UnStake(uint256 _value) external {
        require(_value > 0, "UnStake amount should be greater than 0");
        require(AddressStaked[msg.sender] = true, "You have not staked tokens");
        ...
        StakedInfoMap[msg.sender].Amount -= _value;
        StakedInfoMap[msg.sender].RewardClaimed += _value;
    ...
    function SetRewardRate() external onlyOwner {}

    function SetMaxRewardDuration() external onlyOwner {}
