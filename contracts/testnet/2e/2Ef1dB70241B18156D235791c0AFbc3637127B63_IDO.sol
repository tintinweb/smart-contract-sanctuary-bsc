...
/**
 * @title Token launchpad contract with private and public pools
 */
contract IDO is Ownable {
	// number of pools
	uint public constant POOLS_NUMBER = 2;

	// pool info
    struct Pool {
        uint raisingAmount; // amount of tokens to raise
        uint offeringAmount; // amount of tokens to offer
        uint minLimitPerUserInDealToken; // min limit of tokens per user, set 0 to ignore
        uint maxLimitPerUserInDealToken; // max limit of tokens per user, set 0 to ignore
        uint totalAmount; // total amount of deposited tokens
		uint fixedStakingPoolIndex; // fixed staking pool index where to search for min stake amount
        uint fixedStakingMinStakeAmount; // min user stake amount in fixed staking
    }

	// user info per pool
    struct UserInfo {
        uint amount; // how many tokens the user has provided for pool
        bool refunded; // refund excess funds
        mapping (uint => bool) claimed; // whether the user has claimed each period for a pool
    }

	// fixed staking contract
	...
	// array of available pools
	Pool[POOLS_NUMBER] private _poolInfo;

	// harvest release block timestamps
    uint[] public harvestReleaseBlocks;

    // harvest release rates, 1e12 == 100%
    uint[] public harvestReleaseRates;

	// block number when IDO starts
    uint public startBlock;

	// block number when IDO ends
    uint public endBlock;

	// address to user pool info
	mapping(address => UserInfo[POOLS_NUMBER]) private _userInfo;
     ...
     * @param _dealToken the raise token used
	 ...
	 * @param _fixedStaking fixed staking contract address
     * @param _harvestReleaseBlocks array of timestamps when harvest can be performed
     * @param _harvestReleaseRates array of harvest rates (vesting), 1e12 == 100%
     */
	constructor(
		IERC20 _dealToken,
        ...
        uint _startBlock,
        uint _endBlock,
		FixedStaking _fixedStaking,
        uint[] memory _harvestReleaseBlocks,
        uint[] memory _harvestReleaseRates 
	) {
		require(_dealToken != _offeringToken, "EQUAL_TOKENS");
        require(_startBlock > block.number, "INVALID_START_TIME");
        require(_endBlock > _startBlock, "INVALID_END_TIME");
        require(_harvestReleaseBlocks.length > 0, "EMPTY_RELEASE_BLOCKS");
        require(_harvestReleaseRates.length > 0, "EMPTY_RELEASE_RATES");
        require(_harvestReleaseBlocks.length == _harvestReleaseRates.length, "HARVEST_RELEASE_ARRAYS_NOT_MATCH");
        dealToken = _dealToken;
        offeringToken = _offeringToken;
        startBlock = _startBlock;
        endBlock = _endBlock;
		fixedStaking = _fixedStaking;
        harvestReleaseBlocks = _harvestReleaseBlocks;
        harvestReleaseRates = _harvestReleaseRates;
	}
	...
	/**
     * @notice Calculates offer and refund amounts of tokens
     * @param _user user address
     * @param _pid pool id
     */
    function calculateUserAmounts(address _user, uint _pid)
        public
        view
        returns (uint userOfferingAmount, uint userRefundingAmount)
    {
        Pool memory _pool = _poolInfo[_pid];
        UserInfo storage userInfo = _userInfo[_user][_pid];
     ...
     * @param _pid pool id
     */
    function checkMinStakeAmount(address _user, uint _pid) public view returns (bool) {
        require(_pid < POOLS_NUMBER, "POOL_DOES_NOT_EXIST");
		Pool memory pool = _poolInfo[_pid];
		(uint stakedAmount, , , ) = fixedStaking.userPoolStake(_user, pool.fixedStakingPoolIndex);
		return stakedAmount >= pool.fixedStakingMinStakeAmount;
    }

	/**
     * @notice Allows users to deposit deal tokens to the pool
     * @param _amount the number of deal token used
     * @param _pid pool id
     */
    function deposit(uint _amount, uint _pid) public {
        ...
        // validation
        require(_pool.offeringAmount > 0 && _pool.raisingAmount > 0, "POOL_NOT_SET");
        require(block.number > startBlock, "TOO_EARLY");
        require(block.number < endBlock, "TOO_LATE");
        require(_amount > 0, "ZERO_AMOUNT");
        require(checkMinStakeAmount(msg.sender, _pid), "NOT_ENOUGH_STAKE_TOKENS");
        require(_user.amount + _amount >= _pool.minLimitPerUserInDealToken, "MIN_AMOUNT_THRESHOLD_NOT_REACHED");
        require(_user.amount + _amount <= _pool.maxLimitPerUserInDealToken, "MAX_AMOUNT_THRESHOLD_REACHED");
        ...
        // update total and user's amounts
        _user.amount += _amount;
        _pool.totalAmount += _amount;

		// transfers funds to this contract
        dealToken.transferFrom(msg.sender, address(this), _amount);
    }
     ...
     */
    function harvest(uint _pid, uint _harvestPeriod) public {
		// validation
        require(_harvestPeriod < harvestReleaseBlocks.length, "INVALID_HARVEST_PERIOD");
        require(checkMinStakeAmount(msg.sender, _pid), "NOT_ENOUGH_STAKE_TOKENS");
        require(block.number > harvestReleaseBlocks[_harvestPeriod], "TOO_EARLY_TO_HARVEST");
        require(_pid < POOLS_NUMBER, "POOL_DOES_NOT_EXIST");
        UserInfo storage userInfoPid = _userInfo[msg.sender][_pid];
        require(userInfoPid.amount > 0, "NOT_A_PARTICIPANT");
        require(!userInfoPid.claimed[_harvestPeriod], "ALREADY_CLAIMED");

		// calculate offer and refund amounts
        (uint256 offeringTokenAmount, uint256 refundingTokenAmount) = calculateUserAmounts(msg.sender, _pid);

		// mark harvest period as claimed
        ...
        if (refundingTokenAmount > 0 && !userInfoPid.refunded) {
            ...
            dealToken.transfer(msg.sender, refundingTokenAmount);
        }

		// transfer offer tokens to user
        uint harvestAmount = offeringTokenAmount / 1e12 * harvestReleaseRates[_harvestPeriod];
        if (offeringTokenAmount > 0) {
            offeringToken.transfer(msg.sender, harvestAmount);
        }
    }
	...
	/**
     * @notice Sets fixed staking contract address
     * @param _fixedStakingAddress fixed staking contract address
     */
    function setFixedStakingAddress(address _fixedStakingAddress) public onlyOwner {
        require(_fixedStakingAddress != address(0), "ZERO_ADDRESS");
        fixedStaking = FixedStaking(_fixedStakingAddress);
    }

	/**
     * @notice Sets pool parameters
     * @param _pool pool instance
     * @param _pid pool id
     */
    function setPool(Pool calldata _pool, uint _pid) public onlyOwner {
        require(block.number < startBlock, "IDO_ALREADY_STARTED");
        require(_pid < POOLS_NUMBER, "POOL_DOES_NOT_EXIST");
    ...
    /**
     * @notice Updates harvest release timestamps and rates
     * @param _harvestReleaseBlocks array of timestamps when harvest can be performed
     * @param _harvestReleaseRates array of harvest rates (vesting), 1e12 == 100%
     */
    function updateHarvestReleaseParams(
        uint[] memory _harvestReleaseBlocks, 
        uint[] memory _harvestReleaseRates
    ) 
        public 
	...
	/**
     * @notice Updates start and end blocks
     * @param _startBlock the new start block
     * @param _endBlock the new end block
     */
    function updateStartAndEndBlocks(uint _startBlock, uint _endBlock) public onlyOwner {
        require(block.number < startBlock, "IDO_ALREADY_STARTED");
        require(_startBlock < _endBlock, "START_BLOCK_IS_GREATER_THAN_END_BLOCK");
        require(block.number < _startBlock, "START_BLOCK_IS_LESS_THAN_CURRENT_BLOCK");
        startBlock = _startBlock;
        endBlock = _endBlock;
    }

	/**
     * @notice Withdraws funds by owner
     * @param _dealTokenAmount the number of deal token to withdraw
     * @param _offerAmount the number of offering amount to withdraw
     */
    function withdraw(uint _dealTokenAmount, uint _offerAmount) public onlyOwner {
        require(_dealTokenAmount <= dealToken.balanceOf(address(this)), "NOT_ENOUGH_DEAL_TOKENS");
        require(_offerAmount <= offeringToken.balanceOf(address(this)), "NOT_ENOUGH_OFFERING_TOKENS");

        if (_dealTokenAmount > 0) {
            dealToken.transfer(msg.sender, _dealTokenAmount);
        }

        if (_offerAmount > 0) {
            offeringToken.transfer(msg.sender, _offerAmount);
        }
    }

	/**
     * @notice Withdraws wrong tokens sent to the contract
     * @param _tokenAddress the address of the token to withdraw
     * @param _tokenAmount the number of token amount to withdraw
     */
    function withdrawWrongTokens(address _tokenAddress, uint _tokenAmount) public onlyOwner {
        require(_tokenAddress != address(dealToken), "DEAL_TOKEN_WITHDRAW");
        require(_tokenAddress != address(offeringToken), "OFFERING_TOKEN_WITHDRAW");
...
contract ReferralProgram is Ownable {
        ...
        returns(uint) 
    {
        ...
        // return fee rate
        uint feeRate = refLevels[_refLevelIndex].feeLevels[_sourceRefContract].level1;
        if (_feeLevel == 1) feeRate = refLevels[_refLevelIndex].feeLevels[_sourceRefContract].level2;
        if (_feeLevel == 2) feeRate = refLevels[_refLevelIndex].feeLevels[_sourceRefContract].level3;
        if (_feeLevel == 3) feeRate = refLevels[_refLevelIndex].feeLevels[_sourceRefContract].level4;
        if (_feeLevel == 4) feeRate = refLevels[_refLevelIndex].feeLevels[_sourceRefContract].level5;
        return feeRate;
    }
