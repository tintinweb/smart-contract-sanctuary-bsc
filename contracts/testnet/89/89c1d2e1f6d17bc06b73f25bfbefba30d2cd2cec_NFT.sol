...
contract NFT is ERC721URIStorage {
    ...
    mapping(uint256 => address) private _creators; // 최초 생성한 사람
    mapping(uint256 => uint256) private _category; // TokenId -> category
    
    event TokenMinted(uint256 indexed tokenId, string tokenURI, address marketplaceAddress);

    constructor(address _marketplaceAddress) ERC721("SourceSeaTestNFT", "SSTN") {
    ...
    function mintToken(string memory tokenURI) public returns (uint256) {
        ...
        // ipfs url // metadata.json -> tokenURI.loyalty 
    ...
    function setCategory(uint256 tokenId, uint256 category) public {
        _category[tokenId] = category;
    }

    function getCategory(uint256 tokenId) public view returns (uint256) {
        return _category[tokenId];
    }
    
    // Token 생성한 사람의 주소를 불러옴
    function getTokenCreatorById(uint256 tokenId) public view returns (address) {
        return _creators[tokenId];
    }

    // 자기자신이 만든 NFT 리스트를 가져옴 (loyalty를 받을 수 있는 NFT 목록을 가져옴)
