...
contract GEMUNIStakingGeniPass is GEMUNIStaking {
    ...
    function getPriceNFT (address stakedNFT, uint passId) public override view returns (uint) {
        ...
        if (stakedNFT == geniPass) {
            pricePass = IGENIPassSale(geniPassSale).getPricePass(passId, 0, IGENIPass.PriceType.GENI);
        } else {
...
interface IGEMUNIStaking {
    ...
    event CreatedPoolNft(uint indexed pid, address satkedNFT, address stakedToken, address rewardToken, uint stakePeriod, bool isEarlyWithdrawAllowed, uint rewardSupply, uint _startTime);
    ...
    function depositNFT(uint pid, uint[] memory tokenIds, uint _amountToken) external;
...
abstract contract GEMUNIStaking is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIStaking{
    ...
    function emergencyRewardWithdraw(uint pid, uint256 _amount, address admin) external override onlyOperator {
        ...
        uint currentSupply = IERC20(pool.tokenPoolInfo.rewardToken).balanceOf(address(this));
        require(_amount <= currentSupply, "GS: Amount too high");

        IERC20(pool.tokenPoolInfo.rewardToken).safeTransferFrom(address(this), admin, _amount);
        pool.tokenPoolInfo.rewardBalance = pool.tokenPoolInfo.rewardBalance.sub(_amount);
        _updateReward(pid);
    ...
    function increaseReward(uint pid, uint256 _amount) external override onlyOperator {
        ...
        require(pool.isActive, "GS: already locked");
        require(_amount > 0, "GS: invalid amount");
        IERC20(pool.tokenPoolInfo.rewardToken).safeTransferFrom(msg.sender, address(this), _amount);
        pool.tokenPoolInfo.rewardBalance = pool.tokenPoolInfo.rewardBalance.add(_amount);
        _updateReward(pid);
    ...
    function addPoolNft(
        ...
        for (uint256 i = 0; i < poolInfoNfts.length; i++) {
            ...
            if (pool.tokenPoolInfo.stakedNFT == _tokenStaked && pool.tokenPoolInfo.stakedToken == _tokenStaked && pool.tokenPoolInfo.rewardToken == _rewardToken && pool.timePoolInfo.stakingPeriod == _stakingPeriod) {
                foundToken = true;
                break;
            }
        }
        ...
        PoolInfoNft storage pool = poolInfoNfts[pid];
        IERC20(pool.tokenPoolInfo.rewardToken).safeTransferFrom(msg.sender, address(this), rewardSupply);
        _updateReward(pid);
        emit CreatedPoolNft(pid, _tokenNFT, _tokenStaked, _rewardToken, _stakingPeriod, _isEarlyWithdrawAllowed, rewardSupply, _startTime);
    }
    ...
    // User deposit NFT
    // User deposit NFT
    function depositNFT(uint pid, uint[] memory tokenIds, uint _amountToken) external override onlyTokenOwner(pid, tokenIds) nonReentrant {
        uint maxLengthPool = poolInfoNfts.length - 1;
        require(pid <= maxLengthPool, "GS: invalid pid");
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                pool.tokenPoolInfo.rewardBalance = pool.tokenPoolInfo.rewardBalance.sub(pending);
            }
        }

        if (pool.tokenPoolInfo.stakedToken == address(0)){
            require(_amountToken == 0, "GS: invalid amount");
        } else {
            require(IERC20(pool.tokenPoolInfo.stakedToken).balanceOf(msg.sender) >= _amountToken, "GS: Don't have enough geni");
            if (_amountToken > 0) {
                IERC20(pool.tokenPoolInfo.stakedToken).safeTransferFrom(msg.sender, address(this), _amountToken);
            }
        } 
        uint totalAmountNFT;
        for (uint i = 0; i < tokenIds.length; i++ ) {
            uint amountNFT = getPriceNFT(pool.tokenPoolInfo.stakedNFT, tokenIds[i]);
            totalAmountNFT = totalAmountNFT.add(amountNFT);
            uint amountToken;
            if ( i < tokenIds.length -1) {
                amountToken = _amountToken / tokenIds.length;
            } else {
                amountToken = _amountToken / tokenIds.length + _amountToken % tokenIds.length;
            }
            IERC721(pool.tokenPoolInfo.stakedNFT).transferFrom(msg.sender, address(this), tokenIds[i]);
            user.stakedNFTs.push(tokenIds[i]);
            NFTStaked[pid][tokenIds[i]].user = msg.sender;
            NFTStaked[pid][tokenIds[i]].amount = amountToken + amountNFT;
        }
        user.stakeTime = block.timestamp;
        uint totalAmount = totalAmountNFT + _amountToken;
        ...
        emit DepositNFT(pid, msg.sender, tokenIds, totalAmount);
    }


    // User withdraw
    function withdrawNFT(uint pid, uint256 passId) external override onlyTokenOwnerStaking(pid, passId) nonReentrant {
        uint maxLengthPool = poolInfoNfts.length - 1;
        require(pid <= maxLengthPool, "GS: invalid pid");
        UserInfoNft storage user = userInfoNft[pid][msg.sender];
        PoolInfoNft storage pool = poolInfoNfts[pid];
        ...
        uint _amountNFT = getPriceNFT(pool.tokenPoolInfo.stakedNFT, passId);
        uint _amount = NFTStaked[pid][passId].amount;
        uint _amountToken = _amount.sub(_amountNFT); 
        ...
        if (pending > 0) {
            IERC20(pool.tokenPoolInfo.rewardToken).safeTransfer(address(msg.sender), pending);
            pool.claimedRewards = pool.claimedRewards.add(pending);
            pool.tokenPoolInfo.rewardBalance = pool.tokenPoolInfo.rewardBalance
                .sub(pending)
        ...
        if (_amount > 0) {
            ...
            IERC721(pool.tokenPoolInfo.stakedNFT).safeTransferFrom(address(this), msg.sender, passId);
            if (_amountToken > 0) {
    ...
    function emergencyWithdrawNFT(uint pid) external override {
        ...
        for (uint256 i = 0; i < user.stakedNFTs.length; i++) {
            ...
            if (IERC721(pool.tokenPoolInfo.stakedNFT).ownerOf(user.stakedNFTs[i]) == address(this)) {
                IERC721(pool.tokenPoolInfo.stakedNFT).safeTransferFrom(address(this), msg.sender, user.stakedNFTs[i]);
