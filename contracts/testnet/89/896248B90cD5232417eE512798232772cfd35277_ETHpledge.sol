...
contract ETHpledge is IERC20, Ownable {
    ...
    function  pledgein(address fatheraddr)  public payable returns (bool) {
        ...
        receivetime[msg.sender]=block.timestamp+6;
        return true;
    }
    ...
    function  ETHreceive()  external returns (bool) {
        ...
        bool B2 = receivenumber[msg.sender] <= 20;
        require(B2,"pledgeendtime  is end.");

        bool B1 = receiveamount[msg.sender] < pledgeamount[msg.sender].mul(2) ;
        require(B1,"receiveamount  is double.");

        uint256 _recamount=pledgeamount[msg.sender].div(_baseFee).mul(_profit1);
