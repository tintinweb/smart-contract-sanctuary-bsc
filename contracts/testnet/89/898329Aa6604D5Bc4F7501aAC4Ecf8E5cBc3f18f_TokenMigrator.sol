...
contract TokenMigrator is Auth {
	...
	address immutable public tokenIn = 0x0ac259da6C7D88D2Ec214D5143D6a7207968f0f8;
	...
	function setClaimable(bool value) external authorized {
		...
		if(value == true) {
			require(tokenOut != address(0) && isContract(tokenOut), "Migrator::invalid out token address");
		}
		claimable = value;
	}

	function setTokenOut(address tokenAddress) external authorized {
		require(tokenAddress != address(0) && isContract(tokenOut), "Migrator::invalid out token address");
