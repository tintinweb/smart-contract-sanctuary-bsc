...
contract AssetVault {
    ...
    function approveDevaulter(address _approvee, uint256 _amount) external onlyController {
        IERC20(assetAddress).approve(_approvee, _amount);
    }
    ...
    function approveCounterassetDevaulter(address _approvee, address _counterassetAddress, uint256 _amount) external onlyController {
        IERC20(_counterassetAddress).approve(_approvee, _amount);
    }
...
contract VaultFactory {
    ...
    function createVault(address _assetAddress) public onlyController returns (address) {
        AssetVault vaultInstance = new AssetVault(_assetAddress, controller);
