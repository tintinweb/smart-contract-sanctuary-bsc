    ...
    PriceControl
{
    ...
    mapping(uint8 => address[]) objectsByTier;
    mapping(uint8 => address[]) resourcesByTier;
    mapping(uint8 => uint256) public resourcesQuantityByTierMap;
    mapping(uint8 => SharedStructs.ExoTierConfig) public exoTierConfigMap;
    mapping(uint8 => SharedStructs.ArmorTierConfig) public armorTierConfigMap;
    mapping(uint8 => SharedStructs.WeaponTierConfig) public weaponTierConfigMap;
    mapping(uint8 => SharedStructs.LandVehicleTierConfig)
        public landVehicleTierConfigMap;
    mapping(uint8 => SharedStructs.SpaceVehicleTierConfig)
        public spaceVehicleTierConfigMap;
    ...
    function setResourcesQuantitiesByTier(uint8 tier, uint256 quantity)
        external
        onlyOwner
    {
    ...
    function getLootBoxFreeWhiteList(address _whiteListed) public virtual {
    ...
    /// @param _whiteListed Address of the white listed
    /// @param tier Indicates the tier of the purchase
    function getLootBoxDiscountWhiteList(address _whiteListed, uint8 tier)
        public
        virtual
    {
    ...
    function _exoGeneration(uint8 tier) internal {
        ...
        for (uint8 i = 0; i < exoTier.exoQuantity; i++) {
            IExocredit(exoTier.exoCreditAddress).randomMint(
                msg.sender,
                tier,
                exoTier
            );
    ...
    function _armorGeneration(uint8 tier) internal {
        ...
        for (uint8 i = 0; i < armorTier.armorQuantity; i++) {
            IArmor(armorTier.armorAddress).randomMint(
                msg.sender,
                tier,
                armorTier
            );
    ...
    function _weaponGeneration(uint8 tier) internal {
        ...
        for (uint8 i = 0; i < weaponTier.weaponQuantity; i++) {
            IWeapon(weaponTier.weaponAddress).randomMint(
                msg.sender,
                tier,
                weaponTier
            );
    ...
    function _landVehicleGeneration(uint8 tier) internal {
        ...
        for (uint8 i = 0; i < landVehicleTier.landVehicleQuantity; i++) {
            ILandVehicle(landVehicleTier.landVehicleAddress).randomMint(
                msg.sender,
                tier,
                landVehicleTier
            );
    ...
    function _spaceVehicleGeneration(uint8 tier) internal {
        ...
        for (uint8 i = 0; i < spaceVehicleTier.spaceVehicleQuantity; i++) {
            ISpaceVehicle(spaceVehicleTier.spaceVehicleAddress).randomMint(
                msg.sender,
                tier,
                spaceVehicleTier
            );
...
interface IExocredit is IERC721Upgradeable {
    function randomMint(
        address to,
        uint8 tier,
        SharedStructs.ExoTierConfig memory _exoTierConfig
    ) external;
}
