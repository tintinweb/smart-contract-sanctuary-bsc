...
interface IArcadiaSwap {
  ...
  function rpAddress() external view returns(address);
}
interface IArcadiaPacToken {
  ...
  function currentPacType(uint256 tokenId) external view returns(uint256);
}
...
contract ArcadiaGameEngine is Ownable {
  ...
  mapping(address=>bool) private board;
  ...
  uint256 private tax = 5000;

  enum PacType {Common, UnCommon, Rare, Legendary, Mythical}
  enum Difficulty {None, Easy, Medium, Hard, VeryHard, Extreme}
  /***
  ...
  */
  ...
  struct UserInfo {
    ...
    uint256 lastXpPurchasedTime;
  }
  mapping(address=>UserInfo) public users;
  uint256 private elapsedGamePlayed;
  ...
  XpPriceInfo[] public xp_prices;
  uint256 public waitTime = 48 hours;

  constructor() {
    matchXp[Match.Win] = 10*1e18;
    matchXp[Match.Loss] = 3*1e18;

    rarities[PacType.Common] = PacInfo({ payout: 500, price: 1*1e15 });
    rarities[PacType.UnCommon] = PacInfo({ payout: 1500, price: 3*1e15 });
    rarities[PacType.Rare] = PacInfo({ payout: 4000, price: 8*1e15 });
    rarities[PacType.Legendary] = PacInfo({ payout: 10000, price: 2*1e16 });
    rarities[PacType.Mythical] = PacInfo({ payout: 25000, price: 5*1e16 });

    board[_msgSender()] = true;
    maxPacLimit = 15;

    xp_prices.push(XpPriceInfo({amount: 100*1e18, price: 100*1e18, discount: 0}));
    xp_prices.push(XpPriceInfo({amount: 200*1e18, price: 200*1e18, discount: 1000}));
    xp_prices.push(XpPriceInfo({amount: 500*1e18, price: 500*1e18, discount: 1600}));
    xp_prices.push(XpPriceInfo({amount: 1000*1e18, price: 1000*1e18, discount: 2000}));

    payoutInfos[1] = PayoutInfo({ payoutRate: 500, energy: 8, rechargeCostOfReward: 2500 });
    payoutInfos[2] = PayoutInfo({ payoutRate: 500, energy: 8, rechargeCostOfReward: 2500 });
    payoutInfos[3] = PayoutInfo({ payoutRate: 500, energy: 8, rechargeCostOfReward: 2500 });
    payoutInfos[4] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[5] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[6] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[7] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[8] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[9] = PayoutInfo({ payoutRate: 515, energy: 9, rechargeCostOfReward: 2200 });
    payoutInfos[10] = PayoutInfo({ payoutRate: 550, energy: 10, rechargeCostOfReward: 2000 });
    payoutInfos[11] = PayoutInfo({ payoutRate: 550, energy: 10, rechargeCostOfReward: 2000 });
    payoutInfos[12] = PayoutInfo({ payoutRate: 550, energy: 10, rechargeCostOfReward: 2000 });
    payoutInfos[13] = PayoutInfo({ payoutRate: 550, energy: 10, rechargeCostOfReward: 2000 });
    payoutInfos[14] = PayoutInfo({ payoutRate: 550, energy: 10, rechargeCostOfReward: 2000 });
    payoutInfos[15] = PayoutInfo({ payoutRate: 600, energy: 12, rechargeCostOfReward: 1700 });

    difficultyCoefficients[Difficulty.Easy][Match.Win] = DifficultyCoefficient({xp: 5000, reward: 9000});
    difficultyCoefficients[Difficulty.Easy][Match.Loss] = DifficultyCoefficient({xp: 3000, reward: 5000});
    difficultyCoefficients[Difficulty.Medium][Match.Win] = DifficultyCoefficient({xp: 7500, reward: 9500});
    difficultyCoefficients[Difficulty.Medium][Match.Loss] = DifficultyCoefficient({xp: 5000, reward: 6000});
    difficultyCoefficients[Difficulty.Hard][Match.Win] = DifficultyCoefficient({xp: 10000, reward: 10000});
    difficultyCoefficients[Difficulty.Hard][Match.Loss] = DifficultyCoefficient({xp: 8000, reward: 7000});
    difficultyCoefficients[Difficulty.VeryHard][Match.Win] = DifficultyCoefficient({xp: 12500, reward: 11000});
    difficultyCoefficients[Difficulty.VeryHard][Match.Loss] = DifficultyCoefficient({xp: 10000, reward: 8000});
    difficultyCoefficients[Difficulty.Extreme][Match.Win] = DifficultyCoefficient({xp: 15000, reward: 13000});
    difficultyCoefficients[Difficulty.Extreme][Match.Loss] = DifficultyCoefficient({xp: 11000, reward: 9000});
  ...
  function addPriceInfo(uint256 _amount, uint256 _price, uint256 _discount) public onlyOwner {
    xp_prices.push(XpPriceInfo({amount: _amount, price: _price, discount: _discount }));
  }
  function updatePriceInfo(uint256 idx, uint256 _amount, uint256 _price, uint256 _discount) public onlyOwner {
    require(idx < xp_prices.length, "Error: invalid index.");
    xp_prices[idx] = XpPriceInfo({amount: _amount, price: _price, discount: _discount });
  }
  function removePriceInfo(uint256 idx) public onlyOwner {
    require(idx < xp_prices.length, "Error: invalid index.");
    for(uint256 i=idx;i<xp_prices.length-1;i++) {
      xp_prices[i] = xp_prices[i+1];
    }
    delete xp_prices[xp_prices.length-1];
  }
  function xpPriceLength() public view returns(uint256) {
    return xp_prices.length;
  }
  function changeWaitTimeForXpPurchase(uint256 _waitTime) public onlyOwner {
    waitTime = _waitTime;
  }
  ...
  function setMinter(address _minter, bool _flag) public onlyOwner {
    isMinter[_minter] = _flag;
  }
  function setArcadia(address _arcadia, bool _flag) public onlyOwner {
    isArcadia[_arcadia] = _flag;
  }
  ...
  */
  ...
  function mintPac(uint8 pac) public {
    uint256 estArcAmount = IArcadiaSwap(arcSwap).estArcAmount(rarities[PacType(pac)].price);
    require(IERC20(arcToken).balanceOf(_msgSender()) >= estArcAmount, "Error: Insufficient balance to buy the specific Pac");
    IERC20(arcToken).safeTransferFrom(_msgSender(), address(this), estArcAmount);
    
    IArcadiaPacToken(pacToken).mint(_msgSender());
    ...
    _transferArcToRewardPool();
  }

  event PacLeveledUp(address account, uint256 tokenId, uint8 from, uint8 to);
  function levelupOfPac(uint256 tokenId) public {
    ...
    uint256 estArcAmount = IArcadiaSwap(arcSwap).estArcAmount(arc);
    ...
    IERC20(arcToken).safeTransferFrom(_msgSender(), address(this), arc);
    ...
    emit PacLeveledUp(_msgSender(), tokenId, currentPacLevel, currentPacLevel+1);
  }
  ...
  function dailyPayoutFromPacs(address account) public view returns(uint256, uint256, uint256) {
    ...
    for(uint8 i=0;i<pacCount;i++) {
      PacType pacType = PacType(IArcadiaPacToken(pacToken).currentPacType(pacs[i]));
  ...
  function startGame() public {
    UserInfo storage user = users[_msgSender()];
    require(user.status != ConfirmStatus.progress, "Error: you need to confirm the game result first.");
    require(user.energies > 0, "Error: Insufficient energy.");
    elapsedGamePlayed++;
    user.energies = user.energies - 1;
    user.playedGame = user.playedGame.add(1);
    user.gameDifficulty = uint8(_rand(user.playedGame));
    user.status = ConfirmStatus.progress;
    emit StartedGame(_msgSender(), user.gameDifficulty);
  }
  function _rand(uint256 num) internal view returns(uint256) {
    uint256 seed = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.number, elapsedGamePlayed, num)));
    return (seed % 5 + 1);
  }
  event ConfirmedGameResult(address account, Match gameMatch, uint256 earnedXP, uint256 earnedCoin);
  function confirmGameResult(Match gameMatch) public {
    UserInfo storage user = users[_msgSender()];
    require(user.status == ConfirmStatus.progress, "Error: Something went wrong.");
    ...
    user.gameDifficulty = 0;
    emit ConfirmedGameResult(_msgSender(), gameMatch, earnedXp, earnedReward);
  }
  ...
  event PurchasedXp(address indexed account, uint256 amount);
  function purchaseXp(uint256 idx) public {
    UserInfo storage user = users[_msgSender()];
    require(block.timestamp.sub(user.lastXpPurchasedTime) > waitTime, "Error: You need to wait.");
    require(idx < xp_prices.length, "Error: Invalid price index");
    uint256 price = xp_prices[idx].price.sub(xp_prices[idx].discount);
    uint256 estArcAmount = IArcadiaSwap(arcSwap).estArcAmount(price);
    require(IERC20(arcToken).balanceOf(_msgSender()) >= estArcAmount, "Error: Insufficient balance.");
    IERC20(arcToken).safeTransferFrom(_msgSender(), address(this), estArcAmount);

    _mintXp(_msgSender(), xp_prices[idx].amount);
    emit PurchasedXp(_msgSender(), xp_prices[idx].amount);

    user.lastXpPurchasedTime = block.timestamp;
    _transferArcToRewardPool();
  }

  function _transferArcToRewardPool() internal {
    uint256 amount = IERC20(arcToken).balanceOf(address(this));
    uint256 taxAmount = amount.mul(tax).div(1e4);

    IERC20(arcToken).safeTransfer(address(IArcadiaSwap(arcSwap).rpAddress()), amount.sub(taxAmount));
    IERC20(arcToken).safeTransfer(address(arcSwap), taxAmount);
    IArcadiaSwap(arcSwap).addAccumulativeTax(taxAmount);
  }
}