...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    uint256 public swapTrigger = 16; // Fee swap will be triggered after 15 transactions
    uint256 public swapCounter = 1;  // Start at 1, not zero, to save on gas
    uint256 public tokenSwapMin = 1 * 10**3 * 10**9;
    uint256 public tokenSwapMax = 1 * 10**6 * 10**9;

    uint256 public teamSellStart = 	1651644420; //Team tokens can be transferred after specific time has passed
    ...
    {
        require (
            numTokens >= tokenSwapMin && numTokens <= tokenSwapMax,
            "Tokens for swap should be between 1000 to 1,000,000."
        );
    ...
    function _transfer(
        ...
        if (
            ...
            swapCounter >= swapTrigger // Check that enough transactions have passed since last swap
        ) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapCounter = 1; // Reset transaction counter
