...
contract FLPCrowdSale is Ownable
{
    ...
    // một BNB/USDT sẽ mua được bao nhiêu tokenb
    uint256 public BNB_rate;
    uint256 public USDT_rate;
    uint256 public BTC_rate;
    uint256 public DBZ_rate;

    IERC20 public token;
    IERC20 public usdtToken; // mine
    ...
    constructor( 
        ...
        BNB_rate = bnb_rate;
        USDT_rate = usdt_rate;
        _wallet = wallet; // 0x3674434FAe01D7996e32054932612A1E694b880a
        token = icoToken;
    }
    ...
    function buyTokenByBNB()external payable{
            ...
            uint256 tokenAmount = getTokenAmountBNB(bnbAmount);
            require(tokenAmount > 0, "Amount is zero");
            // balance cua contract ICO phai co token
            require(token.balanceOf(address(this)) >= tokenAmount, "Insufficient account balance");
            ...
            emit BuyTokenByBNB(msg.sender, tokenAmount);
    }
    ...
    function buyTokenByUSDT(uint256 USDTAmount) external{
            ...
            uint256 tokenAmount = getTokenAmountUSDT(USDTAmount);
            require(tokenAmount > 0, "Amount is zero");
            require(token.balanceOf(address(this)) >= tokenAmount, "Insufficient account balance");
 
            SafeERC20.safeTransferFrom(usdtToken, msg.sender, _wallet, USDTAmount);
            SafeERC20.safeTransfer(token, msg.sender, tokenAmount);
            emit BuyTokenByUSDT(msg.sender, tokenAmount);
    }
