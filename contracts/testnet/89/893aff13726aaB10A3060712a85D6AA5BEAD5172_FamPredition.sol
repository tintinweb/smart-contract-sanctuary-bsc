...
contract FamPredition is Ownable {
    ...
    struct Meta {
        ...
        uint8 withdrawMinFeeRate; // withdraw mini rate
    ...
    function betBatch(
        ...
        uint256[2][] calldata infos // uint256[] memory betNumbers_, // uint256[] memory betrate_
    ) public {
        require(positions_.length == infos.length, "length not match");
        require(gameStatus == GameStatus.OPEN, "bet is not open");
        ...
        require(userBets[epoch_][msg.sender].length <= meta.limit, "bet limit");

        if (rounds[epoch_].closeTime == 0) {
            // rounds[epoch_].start = true;
            rounds[epoch_].startTime = block.timestamp;
            rounds[epoch_].closeTime = block.timestamp + meta.interval;
        }
        ...
        for (uint256 i; i < positions_.length; i++) {
            totalBetAmount += infos[i][1];
            bet(epoch_, positions_[i], infos[i][0], infos[i][1]);
        }
        ...
        _takeInviterReward(totalBetAmount, (totalBetAmount * refTotalRate) / 100); // 15
    }
    ...
    function bet(
        ...
        require(betNumber_ > 0, "bet number must be greater than 0");
        if (position_ == BetType.FIVE) {
            require(betNumber_ >= 111 && betNumber_ <= 144, "mode error");
        }

        // BetInfo memory uBetInfo = userBets[epoch_][msg.sende][index_];
    ...
    // function getBetToClaim(uint256 epoch_, address account_) public view returns (uint256) {
    //     (uint256 totalReward, , , ) = getUserRoundInfo(epoch_, account_);
    //     return reward;
    // }

    function withdrawBonus(uint256 epoch_) public {
        // BetInfo memory uBetInfo = userBets[epoch_][msg.sender][index_];
        // BetInfo[] memory uBetInfos = userBets[epoch_][msg.sender];
        ...
        if (reward >= meta.withdrawMinFeeRate) {
            reward *= 1 ether;
            uint256 fee = (reward * meta.withdrawFeeRate) / 100;
    ...
    function withdrawAllBonus() public {
        ...
        if (totalReward >= meta.withdrawMinFeeRate) {
            totalReward *= 1 ether;
            uint256 fee = (totalReward * meta.withdrawFeeRate) / 100;
            addr.fam.transfer(addr.fee, fee);
            totalReward -= fee;
        }
        addr.fam.transfer(msg.sender, totalReward);
    }
    ...
    {
        BetStatus memory uBetStatus = userBetStatus[epoch_][account_];
        if (!uBetStatus.isBet || rounds[epoch_].luckNumber == 0) {
    ...
    function queryAllUnPaidBonus(address account_) public view returns (uint256 totalReward, uint256[] memory epochs) {
        ...
        uint256 i = userInfo[account_].lastClaimedIndex;
        ...
        for (i; i < length; i++) {
            ...
            if (userBetStatus[epoch][account_].claimed) {
                continue;
            }
            epochs[i] = epoch;
    ...
    function getUserRounds(
        ...
        // BetInfo[] memory uBetInfos = new BetInfo[](length);
    ...
    function requestRandomWords() internal {
        ...
        // require(meta.vrfCoordinator.getSubscription(meta.subscriptionId).balance > 0, "Not enough LINK to pay for subscription");
        // require(meta.vrfCoordinator.getSubscription(meta.subscriptionId).owner == address(this), "Subscription not owned by this contract");
        // require(meta.vrfCoordinator.getSubscription(meta.subscriptionId).endAt > block.timestamp, "Subscription expired");
    ...
    function setClubs(address[] calldata clubs_) public onlyOwner {
        for (uint256 i; i < clubs_.length; i++) {
            userInfo[clubs_[i]].club = clubs_[i];
        }
    }
