...
interface IWoolshed {
    ...
    function teamAirdrop(address[] memory recipients_, uint256[] memory amounts_) external returns (bool);
}
...
contract AirdropSystem is Whitelist, Pausable {
    ...
    function getTeamOf(address _addr) public view returns (address[] memory team) {
        team = new address[](_users[_addr].members);

        // Send an airdrop to each qualified account.
        for(uint256 i = 0; i < _users[_addr].members; i ++) {
            team[i] = _users[_addr].index[i];
        }
    }
    ...
    function getDownlineById(address player, uint256 _pos) public view returns (address) {
        return _users[player].index[_pos];
    }
    ...
    function setUpline(address _newUpline) public whenNotPaused() returns (uint256) {
        ...
        _users[_newUpline].index[_users[_newUpline].members] = msg.sender;
