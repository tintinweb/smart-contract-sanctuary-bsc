...
contract Rolls {
    ...
    struct GenInfo {
        uint256 coinType;// 产出类型  1UNICORN  2WOW
        string name;// 币种名称
        address contractAddress;// 产出合约地址
        uint256 decimals;// 产出币精度
        uint256 daily;// 每日产出
    ...
    uint256 PROFIT_STATUS_GENERATING = 0;// 产出中
    uint256 PROFIT_STATUS_TAKED = 1;// 已领取
    struct ProfitRecord {
    ...
    function discardOwner() public onlyOwner {
        owner = address(0);
    }

    // total不带精度
    function setGen(uint256 coinType, string memory name, address contractAddress, uint256 total, uint256 decimals, uint256 dayCount) public onlyOwner {
        uint256 daily = total.mul(uint256(10).power(decimals));
        GenInfo memory g = GenInfo({
            coinType : coinType,
            name : name,
            contractAddress : contractAddress,
            decimals : decimals,
            daily : daily.div(dayCount),
            dayCount : dayCount
        });
        genInfoArray.push(g);
    ...
    // 质押量，已领取收益，待领取收益，能否领取
    function getStakeData(uint256 id, uint256 coinType) public view returns (uint256, uint256, uint256, bool) {
        ...
        return (addressStakeIdMap[msg.sender][id].stake, stakeProfitRecord[msg.sender][id][coinType].taked,
        pendingProfit, checkCanExtract(currentBlockHeight, coinType));
    }

    function checkCanExtract(uint256 currentBlockHeight, uint256 coinType) internal view returns (bool) {
        return currentBlockHeight >= genInfoMap[coinType].dayCount;
    }

    function calculateProfit(address user, uint256 id, uint256 coinType, uint256 limit) internal view returns (uint256) {
        uint256 itemu = 0;
        uint256 t = 0;
        if (addressStakeIdMap[user][id].stakeHeight >= limit) {
            return 0;
        }
        for (uint256 i = addressStakeIdMap[user][id].stakeHeight; i < limit; i++) {
    ...
    function getPendingProfit(address user, uint256 id, uint256 currentBlockHeight, uint256 coinType) internal view returns (uint256) {
        ...
        ProfitRecord memory p = stakeProfitRecord[user][id][coinType];
        if (addressStakeIdMap[user][id].status == STAKE_STATUS_STAKING) {// 质押中
            if (currentBlockHeight >= g.dayCount) {// 此产出币已产完
                if (p.status == PROFIT_STATUS_GENERATING) {
                    return calculateProfit(user, id, coinType, g.dayCount);
                } else {
                    return 0;
                }
            } else {// 此币还处于产出期
                if (p.status == PROFIT_STATUS_GENERATING) {
                    return calculateProfit(user, id, coinType, currentBlockHeight);
                } else {
                    return 0;
                }
            }
        } else {
            if (addressStakeIdMap[user][id].redeemHeight >= g.dayCount) {// 赎回时的高度已经过了产出期
                if (p.status == PROFIT_STATUS_GENERATING) {// 还未领取收益
                    return calculateProfit(user, id, coinType, g.dayCount);
                } else {
                    return 0;
                }
                ...
                if (p.status == PROFIT_STATUS_GENERATING) {
                    return calculateProfit(user, id, coinType, addressStakeIdMap[user][id].redeemHeight);
                } else {
                    return 0;
                }
            }
    ...
    function getTotalPerBlock(uint256 blockHeight, uint256 coinType) internal view returns (uint256) {
        ...
        return blockHeight <= genInfoMap[coinType].dayCount ? genInfoMap[coinType].daily : 0;
    }
    ...
    function stake(uint256 amount) public {
        ...
        for (uint256 j = 0; j < l; j++) {
            ...
            stakeProfitRecord[msg.sender][sid][genInfoArray[j].coinType] = ProfitRecord({
                coinType: genInfoArray[j].coinType,
                taked: 0,
                status: currentBlockHeight <= genInfoArray[j].dayCount ? PROFIT_STATUS_GENERATING : PROFIT_STATUS_TAKED
            });
        }
    ...
    function extract(uint256 id, uint256 coinType) public {
        uint256 currentBlockHeight = getBlockHeight();
        require(checkCanExtract(currentBlockHeight, coinType), "generaing, can not extract");
        uint256 pendingProfit = getPendingProfit(msg.sender, id, currentBlockHeight, coinType);
        require(pendingProfit > 0, "avail less than 0");
        require(TransferHelper.safeTransfer(genInfoMap[coinType].contractAddress, msg.sender, pendingProfit), "asset insufficient");
        ...
        stakeProfitRecord[msg.sender][id][coinType].status = PROFIT_STATUS_TAKED;
    }
