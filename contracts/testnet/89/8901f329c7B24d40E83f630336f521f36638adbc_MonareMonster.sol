...
contract MonareMonster is ERC721Enumerable, Ownable {
    ...
    event welcomeToMonareMonster(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("MonareMonster", "MonsterMonsterNFT"){
        setBaseURI(baseURI);
    }
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToMonareMonster(_tokenId);
    }
    ...
    function withdrawAll(address _owner) public onlyOwner {
