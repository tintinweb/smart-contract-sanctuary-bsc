...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable {
	...
	event OrderCreated(uint256 _orderId, address _tokenAddress, uint256 _tokenId, uint256 _price);
	...
	event AcceptBid(uint256 _orderId, uint256 indexed _bidId);
	 ...
	 * @param orderAmount is amount token to buy item
	 */
	...
	function _match(
		...
		uint256 royaltyFee = _getRoyaltyFee(order.tokenAddress, order.tokenId);

		uint256 _totalEarnings = orderAmount;

		if (royaltyFee > 0 || platformFee > 0) {
			_totalEarnings = (orderAmount * (BASE_DENOMINATOR - royaltyFee - platformFee)) / BASE_DENOMINATOR;
		}

		if (creator != address(0) && royaltyFee > 0) {
			_paid(_paymentToken, creator, (orderAmount * royaltyFee) / BASE_DENOMINATOR);
		}

		if (platformFee > 0) {
			_paid(_paymentToken, treasuryAddress, (orderAmount * platformFee) / BASE_DENOMINATOR);
		}

		_paid(_paymentToken, order.owner, _totalEarnings);

		IERC721Upgradeable(order.tokenAddress).safeTransferFrom(address(this), _buyer, order.tokenId);

		return true;
	}
}

contract MarketDex is Manager, ReentrancyGuardUpgradeable {
	using SafeERC20Upgradeable for IERC20Upgradeable;
	using AddressUpgradeable for address payable;

	function initialize(uint256 _platformFee, address _treasuryAddress) public virtual override initializer {
		Manager.initialize(_platformFee, _treasuryAddress);
	}
	...
	function createOrder(
		...
		require(IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == msg.sender, "Not-item-owner");

		if (!IERC721Upgradeable(_tokenAddress).isApprovedForAll(msg.sender, address(this))) {
			IERC721Upgradeable(_tokenAddress).setApprovalForAll(address(this), true);
		}
		...
		emit OrderCreated(_orderId, _tokenAddress, _tokenId, _price);

		return _orderId;
	}

	function buy(uint256 _orderId, address _paymentToken) external payable whenNotPaused returns (bool) {
		Order memory order = orders[_orderId];
		...
		uint256 totalPaymentAmount = order.price;
		...
		return _match(msg.sender, _paymentToken, _orderId, totalPaymentAmount);
	}
	...
	function createBid(
		...
		newBid.isOwnerAcceptOffer = false;

		IERC20Upgradeable(newBid.paymentToken).safeTransferFrom(newBid.bidder, address(this), _price);

		newBid.status = true;
		newBid.expireTime = _expireTime;
		bids[totalBids] = newBid;

		_bidId = totalBids;
		currentOrder.listBidId.push(_bidId);
		totalBids += 1;
		emit BidCreated(_bidId, _tokenAddress, _tokenId, _price, _paymentToken);
		return _bidId;
	}

	function acceptBid(uint256 _bidId, uint256 _orderId) external whenNotPaused returns (bool isMatchedOrder) {
		Bid memory bid = bids[_bidId];
		Order memory currentOrder = orders[_orderId];
		require(bid.orderId == _orderId, "Order-is-not-match");
		require(block.timestamp < bid.expireTime && bid.status, "Bid-expired-or-cancelled");

		require(currentOrder.owner == msg.sender && currentOrder.isOnsale, "Oops!Wrong-order-owner-or-cancelled");
		require(bid.paymentToken == currentOrder.paymentToken, "Payment-token-invalid");
		require(bid.tokenAddress == currentOrder.tokenAddress && bid.tokenId == currentOrder.tokenId, "Wrong-token");
		require(!bid.isOwnerAcceptOffer, "Bid-accepted");

		uint256 orderAmount = bid.bidPrice;
		bid.isOwnerAcceptOffer = true;
		bid.status = false;

		isMatchedOrder = _match(bid.bidder, bid.paymentToken, bid.orderId, orderAmount);

		emit AcceptBid(_orderId, _bidId);

		return isMatchedOrder;
	}
	...
	function cancelBid(uint256 _bidId) external whenNotPaused nonReentrant returns (uint256) {
		Bid memory bid = bids[_bidId];
		...
		if (bid.paymentToken == address(0)) {
			...
			IERC20Upgradeable(bid.paymentToken).safeTransferFrom(address(this), bid.bidder, payBackAmount);
		}
	...
	function adminMigratePushNFT(
		...
		for (uint256 i = startId; i <= endId; i++) {
			...
			if (IERC721Upgradeable(order.tokenAddress).ownerOf(order.tokenId) == address(this)) {
				IERC721Upgradeable(order.tokenAddress).safeTransferFrom(address(this), newMarket, order.tokenId);
			}
		}
	}
