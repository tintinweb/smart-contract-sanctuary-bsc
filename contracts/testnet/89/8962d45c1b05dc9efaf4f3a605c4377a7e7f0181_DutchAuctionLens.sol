...
contract DutchAuctionLens {
    function getAuctions(address _auctionFactory)
        public
        view
        returns (address[] memory auctions, uint256 totalAuctions)
    {
        totalAuctions = IDutchAuctionFactory(_auctionFactory).totalAuctions();
        if (totalAuctions > 0) {
            auctions = new address[](totalAuctions);
            for (uint256 i = totalAuctions - 1; i >= 0; i--) {
                address auctionAddress =
                    IDutchAuctionFactory(_auctionFactory).auctions(i);
        ...
        returns (AuctionInfo[] memory auctionInfos)
    {
        ...
        for (uint256 i = 0; i < _take; i++) {
            AuctionInfo memory dutchAuction =
                getAuctionInfo(_auctionFactory, IDutchAuction(auctions[_skip + i]));
