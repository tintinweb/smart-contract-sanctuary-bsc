...
contract GetTheWealth is IBEP20, Ownable {
    ...
    // Set our bot fees
    function setBotFees(uint256 _liquidityFeeBot, uint256 _buybackFeeBot, uint256 _reflectionFeeBot, uint256 _marketingFeeBot, uint256 _feeDenominator) external onlyOwner {
    ...
    function transferForPresale(address _address, uint256 _amount) external onlyPresale {
        checkMaxWallet(_address, presaleWallet, _amount);
...
contract RefundVault is Ownable {
  ...
  constructor(address _wallet, address _token) {
    ...
    token = GetTheWealth(payable(_token));
    wallet = _wallet;
    active = true;
  }
...
contract Crowdsale is Ownable {
  ...
  address public wallet = address(0x57C4C6652070d673aAbD4c19D203D209784248CF);
  ...
  constructor(address payable _token, uint256 _startTime, uint256 _endTime, uint256 _rate, uint256 _softCap, uint256 _hardCap) {
    require(_softCap > 0);
    require(_hardCap > 0);
    ...
    token = GetTheWealth(_token);
    vault = new RefundVault(wallet, _token);
  ...
  function buyTokens() public payable {
    ...
    token.transferForPresale(msg.sender, tokens);
  ...
  // Overridden to send to refundable vault instead
  function forwardFunds() internal {
    // payable(token).transfer(msg.value);
