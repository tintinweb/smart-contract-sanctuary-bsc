...
abstract contract ReentrancyGuard {
 ...
 contract PresaleToken {
    ...
    function withdraw(uint amount, address payable recipient) internal {
    ...
    function sendToken(address _tokenAddress, uint256 _amount, address payable recipient) internal {                               
        IERC20(_tokenAddress).transfer(recipient, _amount);
    }
...
contract IDO is Ownable, ReentrancyGuard{
    ...
    function getProjectDetails(uint256 _id) public view returns(projectDetails memory){
        return details[_id];
    }
