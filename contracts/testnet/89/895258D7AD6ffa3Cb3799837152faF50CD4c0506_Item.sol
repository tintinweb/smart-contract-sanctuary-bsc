...
import "../bridge/IBridgeERC721.sol";

abstract contract ExtendERC721 is
    IExtendERC721, IBridgeERC721, ERC721Enumerable, AccessControlEnumerable, Ownable {
        ...
        returns (uint256)
    {
        ...
        _mint(_to, newId);
        _setMetadata(newId, _metadataId, true);
    ...
    function bridgeMint(address _to, string memory _metadataId)
        external
        override(IBridgeERC721)
        allowBridgeMint
        returns (uint256)
    {
        tokenCounter.increment();
        ...
        _mint(_to, newId);
        _setMetadata(newId, _metadataId, false);
        ...
        onlyOwner
    {
        ...
        _setMetadata(tokenId, _metadataId, true);
        emit MetadataChanged(tokenId, _metadataId);
    }

    function _setMetadata(uint256 tokenId, string memory metadataId, bool checkMetadataId) internal {
        require(!checkMetadataId || !executed[metadataId], "Already metadata id");
    ...
    function setBridgeMinterRole(address to) external onlyOwner {
        _setupRole(BRIDGE_ROLE, to);
    }
    ...
    modifier allowBridgeMint() {
        require(
            hasRole(BRIDGE_ROLE, _msgSender()),
            "Must have bridge mint role"
        );
        _;
    }
    ...
    function burn(uint256 _tokenId) external override(IExtendERC721, IBridgeERC721) {
        require(
    ...
    function getMetadataId(uint256 tokenId) external override(IBridgeERC721) view returns(string memory) {
