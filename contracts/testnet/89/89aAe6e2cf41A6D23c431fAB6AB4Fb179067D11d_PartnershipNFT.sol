...
contract PartnershipNFT is ERC721Burnable, Pausable {
    ...
    //event
    event PartnershipNFTCreated(address indexed receiver,uint256 indexed id);
    event Setting(uint[] tiers, uint128[] supply, string[] names);
    ...
    function mint(bytes memory _signature, uint _tier) whenNotPaused external returns(uint256 _tokenId) {
        require(!minted[msg.sender], "PartnershipNFT: user has been minted");
        uint _newTier = userTier[msg.sender];
        require(_tier >= _newTier && _tier <= totalTier, "PartnershipNFT: _tier invalid");
        //supply
        totalSupply++;
        ...
        // get tier of nft
        _newTier = _getTier(msg.sender, _newTier, _tier);
        ...
        //emit event
        emit PartnershipNFTCreated(msg.sender, _tokenId);
        return _tokenId;
    }

    function setting(uint[] memory _tiers, uint128[] memory _supply, string[] memory _name) external onlyOwner {
        ...
        totalTier = _length;
        emit Setting(_tiers, _supply, _name);
    }
    ...
    //================= internal ================== //
    function _getTier(address _user, uint _tier, uint _optionTier) internal returns (uint _newTier) {
        bool checkWhiteList = true;
        if(hasWhiteList) {
            checkWhiteList = whiteList[_tier].contains(_user);
        }
        Status memory _status = tierStatus[_optionTier];
        if (_status.currentMinted < _status.supply) {
            _newTier = _optionTier;
            return _newTier;
        }
        else {
            for (uint i = _tier; i <= totalTier; i++) {
                //check user in white list
                if (checkWhiteList) {
                    Status memory _status = tierStatus[i];
                    if (_status.currentMinted < _status.supply) {
                        _newTier = _tier;
                        break;
                    }
