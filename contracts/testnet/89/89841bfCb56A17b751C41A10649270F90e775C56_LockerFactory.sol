...
contract LockerFactory is Ownable {
    ...
    function createLocker(
        ...
        if (lockType == 0) {
            ...
            require(param4 <= 1e4, "Invalid tge");
        }

        bytes memory bytecode = type(Locker).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token, lockType, msg.sender));
        address locker;
        ...
        Locker(locker).initialize(lockType, param1, param2, param3, param4, token, uri);
...
contract Locker is Ownable, ReentrancyGuard {
    ...
    uint256 public suggestUnlockPercent;
    ...
    function initialize(
        ...
        emit LockerInitialized(lockType, _param1, _param2, _param3, _param4, address(token), uri);
    }
     ...
     */
    function suggestUnlock(uint256 _suggestUnlockPercent) external payable onlyOwner nonReentrant {
        require(_suggestUnlockPercent > unlockedPercent, "Invalid _suggestUnlockPercent");
        suggestUnlockPercent = _suggestUnlockPercent;
        ...
        emit EmergencyUnlockSuggested(suggestUnlockPercent);
    }
    ...
    function approveSuggestedUnlock() external onlyFactory {
        require(suggestUnlockPercent > unlockedPercent, "Invalid");
        unlockedPercent = suggestUnlockPercent;
        suggestUnlockPercent = 0;
    ...
    function _getUnlockedAmount(uint256 amount) internal view returns (uint256) {
        ...
        if (lockType == 0) {
            ...
            if (block.timestamp >= cliffTime + duration) {
                ...
                uint256 tgeAmount = (amount * tge) / MULTIPLIER;
                uint256 restAmount = amount - tgeAmount;
                uint256 periodicityCount = duration / periodicity;
                uint256 periodicityAmount = restAmount / periodicityCount;
                unlockedAmount = tgeAmount + ((block.timestamp - cliffTime) / periodicity) * periodicityAmount;
            }
        }
