...
import "./ERC721MiranaMinimal.sol";
...
contract MiranaCollection is Ownable {
  ...
  function createToken(
    ...
    address beaconProxy = deployProxy(salt);
  ...
  function deployProxy(uint256 salt) internal returns (address proxy) {
  ...
  {
    return
      abi.encodePacked(type(ERC721MiranaMinimal).creationCode, abi.encode());
  }
    ...
    returns (bytes memory)
  {
    return
      abi.encodeWithSelector(
        ERC721MiranaMinimal(address(this)).__MiranaERC721_init.selector,
        _name,
        _symbol
      );
  }
...
contract ERC721MiranaMinimal is
  Initializable,
  ERC721Upgradeable,
  ...
  OwnableUpgradeable
{
  ...
  function __MiranaERC721_init(string memory _name, string memory _symbol)
    external
    initializer
  {
    __MiranaERC721_init_unchained(_name, _symbol);

    emit MiranaERC721Created(_msgSender(), _name, _symbol);
  }

  function __MiranaERC721_init_unchained(
    string memory _name,
    string memory _symbol
  ) internal {
