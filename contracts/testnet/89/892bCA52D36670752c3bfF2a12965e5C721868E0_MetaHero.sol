...
contract MetaHero is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    IERC20Upgradeable public MSHToken;
    ...
    function initialize(
        string memory baseUri, address addMSHToken
    ) public initializer {
        __Ownable_init_unchained();
        __ReentrancyGuard_init_unchained();
        __ERC721_init("Meta1 Hero1 Contract", "MetaHero1");

        _uri = baseUri;
        _setAcceptedTokenContract(addMSHToken);
    ...
    modifier validateBreed(uint256 _pet1Id, uint256 _pet2Id) {
        require(
            ownerOf(_pet1Id) == msg.sender && ownerOf(_pet2Id) == msg.sender,
            "Not the owner of this one"
        );
        require(_pet1Id != _pet2Id, "Use 2 to breed");
        require(
            pets[_pet1Id].breedCount < MAX_BREED_COUNT &&
                pets[_pet2Id].breedCount < MAX_BREED_COUNT,
            "Breed reached limit"
        );

        require(
            block.timestamp >= adulthoodTime + uint256(pets[_pet1Id].birthTime),
            "pet1 is not mature enough to breed"
        );
        require(
            block.timestamp >= adulthoodTime + uint256(pets[_pet2Id].birthTime),
            "pet2 is not mature enough to breed"
        );

        require(
            pets[_pet1Id].matronId != _pet2Id &&
                pets[_pet1Id].sireId != _pet2Id &&
                pets[_pet2Id].matronId != _pet1Id &&
                pets[_pet2Id].sireId != _pet1Id,
            "Can't breed between parent and child"
        );

        if (
            pets[_pet1Id].matronId > 0 ||
            pets[_pet1Id].sireId > 0 ||
            pets[_pet2Id].matronId > 0 ||
            pets[_pet2Id].sireId > 0
        ) {
            // NOT minted one
            require(
                pets[_pet1Id].matronId != pets[_pet2Id].matronId &&
                    pets[_pet1Id].matronId != pets[_pet2Id].sireId &&
                    pets[_pet2Id].matronId != pets[_pet1Id].sireId,
    ...
    function _setAcceptedTokenContract(address addMSHToken) private {
        MSHToken = IERC20Upgradeable(addMSHToken);
    }
    ...
    function setAcceptedTokenContract(address addMSHToken) external onlyOwner {
        _setAcceptedTokenContract(addMSHToken);
    }

    function setBreedCosts(uint256[6] memory _newBreedCosts)
        external
        onlyOwner
    {
    ...
    function calculateServiceFee(uint256 _price)
        private
        view
        returns (uint256)
    {
    ...
    function putOnSale(uint256 _petId, uint256 _price)
        external
        whenNotPaused
        onlyPetOwner(_petId)
        notInBlackList(_petId)
    {
    ...
    function _putOnSale(uint256 _petId, uint256 _price) private {
        //
        heroOnSale[_petId] = _price;
        emit PetListed(_petId, msg.sender, _price);
    }

    function cancelSale(uint256 _petId)
        external
        whenNotPaused
        nonReentrant
        onlyPetOwner(_petId)
    {
        require(heroOnSale[_petId] > 0, "This one is not on sale already!");
        _cancelSale(_petId);
    }
    ...
    function buyHero(uint256 _petId)
        external
        payable
        whenNotPaused
        nonReentrant
        notInBlackList(_petId)
    {
        uint256 price = heroOnSale[_petId];
    ...
    function batchMint(uint8 generation, uint256[] memory listGenId) external {
    ...
    function _makeTransaction(
        ...
        require(
            transferToTreasury,
            "Make Transaction: transfer to treasury error"
        );
    }
