...
contract Coffers is OwnableUpgradeable, ReentrancyGuard {
    ...
    function getTotalPoolValueWeighted() public view returns (uint256 totalPoolAlloc) {
      for (uint256 i=0; i < poolToken.length; i++) { 
        totalPoolAlloc = totalPoolAlloc.add((poolToken[i].lpToken.balanceOf(address(this))).mul((poolToken[i].poolWeight).div(weightPercent)));
      }
    }
    ...
    function calculateRewardPerEra(uint256 _endEra, uint256 _lastRewardBlock, uint256 _blockNumber, uint256 _pid, uint256 _userAmount) private view returns(uint256 unlockedRewards, uint256 lockedRewards, uint256[22] memory rewardsByEra) {
      ...
      for (uint256 i=getEraNumber(_lastRewardBlock); i<=_endEra;i++) {
        ...
        rewardsByEra[i]=(_userAmount.mul((poolToken[_pid].poolWeight).div(weightPercent)));
        rewardsByEra[i]=rewardsByEra[i].mul(totalRewards).div(totalPoolValue);
        _lastRewardBlock = endBlock;
      }

      unlockedRewards = (_userAmount.mul((poolToken[_pid].poolWeight).div(weightPercent))).mul(unlockedRewards).div(totalPoolValue);
      lockedRewards = (_userAmount.mul((poolToken[_pid].poolWeight).div(weightPercent))).mul(lockedRewards).div(totalPoolValue);
