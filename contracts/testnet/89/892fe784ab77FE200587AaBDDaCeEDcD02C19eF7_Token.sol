...
// File contracts/Router/interfaces/uniswap/IUniswapV2Router01.sol
...
// File contracts/Router/interfaces/uniswap/IUniswapV2Router02.sol
...
// File contracts/Router/interfaces/avalanche/IJoeRouter01.sol
...
// File contracts/Router/interfaces/avalanche/IJoeRouter02.sol
...
// File contracts/Router/interfaces/avalanche/IPangolinRouter.sol
...
// File contracts/Router/interfaces/IMidasMultinetworkRouter.sol
...
// File contracts/Router/interfaces/IMidasGoldRouter.sol
...
contract Token is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(
        ...
        lpPair = IUniswapV2Factory(midasGoldRouter.getDexRouter().factory())
            .createPair(address(this), midasGoldRouter.getNativeTokenAddress());
    ...
    function swapNativeForTokens(uint256 nativeAmount, address _to) private {
        address[] memory path = new address[](2);
        path[0] = midasGoldRouter.getNativeTokenAddress();
        path[1] = address(this);

        uint256 amountOutIn = midasGoldRouter.getDexRouter().getAmountsOut(
            nativeAmount,
            path
        )[1];
        ...
        if (chainId == 43114) {
            ...
            midasGoldRouter
                ...
                value: nativeAmount
            }(amountOutIn, path, _to, block.timestamp + 1000);
        } else {
            // make the swap
            midasGoldRouter
                .getDexRouter()
    ...
    function swapTokensForNative(uint256 tokenAmount, address _to) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = midasGoldRouter.getNativeTokenAddress();

        _approve(
            address(this),
            midasGoldRouter.getDexRouterAddress(),
            tokenAmount
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (chainId == 43114) {
            // add the liquidity
            midasGoldRouter.getDexRouter().addLiquidityAVAX{value: ethAmount}(
                address(this),
                tokenAmount,
            ...
            // add the liquidity
            midasGoldRouter.getDexRouter().addLiquidityETH{value: ethAmount}(
                address(this),
                tokenAmount,
                0, // slippage is unavoidable
                0, // slippage is unavoidable
    ...
    function _beforeTransferCheck(
        ...
        if (
            ...
            if (
                from != owner() &&
                to != address(midasGoldRouter.getDexRouterAddress()) &&
                to != address(lpPair) &&
                to != address(this)
            ) {
