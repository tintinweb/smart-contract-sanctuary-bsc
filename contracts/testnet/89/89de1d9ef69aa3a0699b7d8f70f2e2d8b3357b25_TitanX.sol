...
contract TitanX is Context, Ownable, ERC20 {
    ...
    constructor() ERC20("TitanX","TITAN") {
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        //Testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 BSC
    ...
    function setBlacklistState(address _account, bool _state) external onlyOwner {	
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        transferAmount = amount - (fee + _burnFee);

        //If it's a sell we increase the fee 
        uint256 transferAmountAfterFee = transferAmount;

        if(recipient == uniswapV2Pair && !isTaxless[sender]){

            uint256 feeForSelling = ((transferAmount * sellFee) / 100);

            fee += feeForSelling;

            transferAmount = transferAmountAfterFee - feeForSelling;
        }
    ...
    function swapAndDistribute() private lockTheSwap {
        ...
        uint256 amountToMarketing = ((address(this).balance * rewardDivider) / 100);
        uint256 amountToPool = (address(this).balance - amountToMarketing);

        transferToAddressETH(marketingAddress,amountToMarketing);

         (bool sent, bytes memory data) = payable(stakingPool).call{value: amountToPool}("");
