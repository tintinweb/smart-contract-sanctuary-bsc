...
contract ClaimContract is ERC20, SafeOwnable {
    using SafeMath for uint256;
    using Address for address;
    mapping(address=> bool) public managerList;
    mapping(address=> bool) public mapList;


    
    event ClaimEvent(address token,address  recipient,uint256 amount,uint256 timestamp);


    constructor() ERC20("Claim", "Claim") {
         managerList[_msgSender()]=true;
    }
    function setMapList(address _toAddress, bool _eanble) external onlyOwner{
        mapList[_toAddress]=_eanble;
    }
    ...
    function claim(address token,address recipient,uint256 amount) external onlyManager {
        require(!mapList[recipient],"Error: can't claim");
        uint256 tokenBalance=IERC20(token).balanceOf(address(this));
        require (tokenBalance>=amount,"Error :Insufficient balance");
        TransferHelper.safeTransfer(token,recipient,amount);
        emit ClaimEvent(token,recipient,amount,block.timestamp);
    }
