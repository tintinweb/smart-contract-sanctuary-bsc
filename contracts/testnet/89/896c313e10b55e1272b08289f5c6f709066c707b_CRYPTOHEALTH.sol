...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    constructor() payable {
        ...
        _isLimitExempt[Wallet_Prize] = true;
    ...
    function setBuyFee(uint16 prize, uint16 develop, uint16 treasury, uint16 tax) external onlyOwner {
        require(prize + develop + treasury + tax <= 10, "Fees are capped at 10%"); 
        
        buyFee.prizeFee = prize;
    ...
    function setSellFee(uint16 prize, uint16 develop, uint16 treasury, uint16 tax) external onlyOwner {
        require(prize + develop + treasury + tax <= 15, "Fees are capped at 10%"); 

        sellFee.prizeFee = prize;
    ...
    function setTransferFee(uint16 prize, uint16 develop, uint16 treasury, uint16 tax) external onlyOwner {
        require(prize + develop + treasury + tax <= 40, "Fees are capped at 40%"); 

        transferFee.prizeFee = prize;
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(tPrize).sub(tTreasury).sub(tDevelopment);
        return (tTransferAmount, tFee, tPrize, tTreasury, tDevelopment);
    }
    ...
    function _takeTreasuryAndDevelopment(uint256 tTreasury, uint256 tDevelopment, uint256 tPrize)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rTreasury = tTreasury.mul(currentRate);
        ...
        _rOwned[Wallet_Prize] = _rOwned[Wallet_Prize].add(rPrize);
        ...
        _tOwned[Wallet_Prize] = _tOwned[Wallet_Prize].add(rPrize);
    ...
    function calculatePrizeFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_prizeFee).div(10**2);
    }
    ...
    function setTransfer() private {
        ...
        _prizeFee = transferFee.prizeFee;
    ...
    function _transfer(
        ...
        if (to == Wallet_Prize)
            ...
            require ((from == owner() || from == Wallet_Treasury) // Can only receive tokens from Treasury."
                    && (heldTokens + amount) <= _prizeHold, // Can only hold at most 5% of the supply"
