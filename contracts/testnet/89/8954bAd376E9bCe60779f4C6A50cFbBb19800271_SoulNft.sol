...
contract SoulNft is Ownable, ERC721Enumerable {
    using Strings for uint;
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    IERC20 public soulToken;                // soul token 合约地址
    address public wallet;                  // 资金池
    ...
    constructor (address wallet_, address soul_) ERC721("Soul Meta 3D NFT", "SM3D") {
        wallet = wallet_;
        soulToken = IERC20(soul_);
        baseURI = "https://3dnft.soulmeta.io/metadata/";
        freeMaxNums = 200;
    }
    ...
    function setWhiteList(address address_, bool status_) public onlyOwner {
        require(freeMaxNums != 0, "setWhiteList: No_Free_NFT");
        whiteList[address_] = status_;
    }
    ...
    function playerMint(uint orderNumber_) public {
        ...
        if (mapWhiteToken[player] == 0 && freeMaxNums != 0 && whiteList[player]) {
            // 免费数量递减
            if (freeMaxNums != 0) {
                freeMaxNums -= 1;
            }
            ...
            sold += 1;
            // 付费铸造
            soulToken.safeTransferFrom(player, wallet, handlingFee);
        }
