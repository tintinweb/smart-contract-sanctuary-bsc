...
interface ILab20 {
    function userInfo(uint256 _tokenId, address _address) external;
}
...
contract ResearchLab is OwnableUpgradeable, ReentrancyGuard {
    ...
    // get external userInfo
    function getUserInfoExternal (uint256 _pid) public {
      ILab20(convertFromAddress).userInfo(_pid, msg.sender);
    }
    ...
    function getTotalPoolValueWeighted() public view returns (uint256 totalPoolAlloc) {
      for (uint256 i=0; i < poolToken.length; i++) { 
        totalPoolAlloc = totalPoolAlloc.add(poolToken[i].lpToken.balanceOf(address(this)).mul(100).div(100-(poolToken[i].poolWeight)));
      }
    }
    ...
    function getCurrentEraNumber() public view returns(uint256) {
      uint256 eraNumber = block.number.sub(startBlock).div(blocksPerEra).add(1);
      return eraNumber; 
    }
    ...
    // Deposit LP tokens into the pool
    function depositBridge(uint256 _pid, uint256 _amount, address _oAddress, uint256 _oDepositTime, uint256 _oDepositBlock, uint256 _oLastRewardBlock) public nonReentrant {
        UserInfo storage user = userInfo[_pid][msg.sender];
        
        require(user.amount == 0, "User already exists");
        require(_amount > 0, "Amount must be greater than 0");
        require(_oAddress == convertFromAddress, "Incorrect convert from address");
    ...
    function pendingRewardsCalc(uint256 _pid, address _user, uint256 _blockNumber) private {
        ...
        (uint256 rewardValueUnlocked, uint256 rewardValueLocked, uint256 adjVested) = pendingRewards(_pid, _user);
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256 availableUnlockedRewards, uint256 availableLockedRewards, uint256 adjVested) {
        ...
        if (currentEra <= numberOfEras) {
          ...
          (availableUnlockedRewards, availableLockedRewards) = calculateRewardPerEra(currentEra, lastRewardBlock, blockNumber, _pid, user.amount);

        } else {
          uint256 eraCheckStartBlock = startBlock.add(numberOfEras.mul(blocksPerEra));

          if (lastRewardBlock < eraCheckStartBlock) {
            (availableUnlockedRewards, availableLockedRewards) = calculateRewardPerEra(numberOfEras, lastRewardBlock, (eraCheckStartBlock-1), _pid, user.amount);
            adjVested = availableLockedRewards;
          } else {
            (availableUnlockedRewards, availableLockedRewards, adjVested) = (0,0,0);
          }
          ...
          if(blockNumber<(eraCheckStartBlock.add(blocksPerVesting))) {
            availableLockedRewards = eraCheckStartBlock.add(blocksPerVesting-1).sub(blockMax).mul(rewardPerBlock);
          } else {
        ...
        return (availableUnlockedRewards, availableLockedRewards, adjVested);
    }

    // Calculate the rewards per era 
    function calculateRewardPerEra(uint256 _endEra, uint256 _lastRewardBlock, uint256 _blockNumber, uint256 _pid, uint256 _userAmount) private view returns(uint256 unlockedRewards, uint256 lockedRewards) {
      ...
      for (uint256 i=lastRewardEra; i<=_endEra;i++) {
        ...
        unlockedRewards = unlockedRewards.add(totalRewards.mul(i.mul(5)).div(totalPercent));
        lockedRewards = lockedRewards.add(totalRewards.mul(totalPercent.sub(i.mul(5))).div(totalPercent));
        _lastRewardBlock = endBlock.add(1);
      }

      unlockedRewards = (_userAmount.mul(totalPercent).div(totalPercent.sub(poolToken[_pid].poolWeight))).mul(unlockedRewards).div(totalPoolValue);
      lockedRewards = (_userAmount.mul(totalPercent).div(totalPercent.sub(poolToken[_pid].poolWeight))).mul(lockedRewards).div(totalPoolValue);
      
      return(unlockedRewards, lockedRewards);
    }
