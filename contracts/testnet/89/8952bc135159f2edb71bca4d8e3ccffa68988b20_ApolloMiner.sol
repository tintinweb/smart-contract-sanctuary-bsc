...
contract ApolloMiner {
    address busdt = 0x2DB089EbBbefbC5EAB8C00C7BFE4AEBE37552E5d;
    ...
    ITimerPool public timer; //timer Address
    ...
    constructor(ITimerPool _timer, address _partner) public {
        Team = msg.sender;
        Partner = _partner;
        timer = _timer;
    }    
    ...
    function sellTokens(uint256 amountSell) public {
        ...
        uint256 teamFee = getTeamFee(amountSell); //team
        uint256 timerFee = getTimerFee(amountSell); //timer
        uint256 partnerFee = getPartnerFee(amountSell); //partner
        ...
        ERC20(busdt).transfer(Team, teamFee);
        ERC20(busdt).transfer(timer, timerFee);
        ERC20(busdt).transfer(Partner, partnerFee);
        ERC20(busdt).transfer(msg.sender, SafeMath.sub(amountSell, (teamFee + timerFee + partnerFee)));
    ...
    function buyMiners(address ref, uint256 amount) public {
        ...
        tokensBought = SafeMath.sub(tokensBought, SafeMath.add(getTeamFee(tokensBought), getTimerFee(tokensBought) + getPartnerFee(tokensBought)));

        uint256 teamFee = getTeamFee(amount); //team
        uint256 timerFee = getTimerFee(amount); //timer
        uint256 partnerFee = getPartnerFee(amount); //partner

        ERC20(busdt).transfer(timer, timerFee);
        timer.update(timerFee, block.timestamp, msg.sender);
        ERC20(busdt).transfer(Team, teamFee);
        ERC20(busdt).transfer(Partner, partnerFee);
    ...
    function getTimerFee(uint256 amount) public pure returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, 1), 100); //timer 1% 
    }

    function getTeamFee(uint256 amount) public pure returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, 5), 100); //team 9% 
    }

    function getPartnerFee(uint256 amount) public pure returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, 1), 100); //partner 1% 
    }

    function seedMarket() public payable {
        require(msg.sender == Team, "invalid call");
