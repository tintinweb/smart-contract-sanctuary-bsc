    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
        ...
        if(tokenAddress[_tokenId] == address(0)) {
            ...
            if(_fragmentTokens[_tokenId]){
                ...
                uint256 _amountEarn = 0;
                _amountEarn = _subOwnerFee.div(_subOwners[_rootId].length-1 > 0 ? _subOwners[_rootId].length-1 : 1);
...
contract PaybNftPool is IPaybNftPool, Ownable{
    address PaybMKT;
    using Address for address;
    using SafeMath for uint256;
    ...
    function withdraw(address _tokenAddress) external override {
