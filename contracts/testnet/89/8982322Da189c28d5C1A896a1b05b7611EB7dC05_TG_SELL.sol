...
contract TG_SELL is OwnableUpgradeable {
        ...
        for (uint i = 0; i < 10; i ++) {
            ...
            if (level > lastLevel) {
                if (left >= rewardRate[level]) {
                    ...
                    userInfo[refer].groupReward += amount * (rewardRate[level] - rewardRate[lastLevel]) / 100;
                    left -= (rewardRate[level] - rewardRate[lastLevel]);
                    lastLevel = level;
                } else {
    ...
    function claimReward() external{
        require(userInfo[msg.sender].referReward > 0 ||userInfo[msg.sender].referReward > 0, 'no reward');
