...
contract OPT_IDO is OwnableUpgradeable {
    ...
    function buyIDO(uint amount, bool withStm, address invitor) public onlyEOA {
        ...
        if (!withStm) {
            usdt.transferFrom(msg.sender, address(this), IDOPrice * amount);
        } else {
            usdt.transferFrom(msg.sender, address(this), IDOPrice * amount / 2);
            stm.transferFrom(msg.sender, address(this), IDOPrice * 1e18 * amount / 2 / getStmPrice());
        }
        ...
        userRecord[msg.sender].amount.push(amount);
        userRecord[msg.sender].time.push(block.timestamp);
        userRecord[msg.sender].isSTM.push(withStm);
    }
