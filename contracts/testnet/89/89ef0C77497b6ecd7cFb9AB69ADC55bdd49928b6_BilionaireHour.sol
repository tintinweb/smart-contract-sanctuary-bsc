...
contract BilionaireHour is Ownable {
  ...
  address constant firstUser = 0x3c0bC8835c922D41055EBC4eBA3A337dc54E531e;
    ...
    function wTotal() external {
        ...
        require(waitingToReceive(sender) == 0 && (aInfo[sender].lastWithdraw + 10 minutes) < block.timestamp && address(this).balance >= freeToWithdrawl, "Error");
        ntWithdraw += 1;
    ...
    function WaU(uint _amount) external {
        address sender = msg.sender;
        (,uint freeToWithdrawl, uint comulativePercent) = availableForWithdrawal(sender);
        require((aInfo[sender].lastWithdraw + 10 minutes) < block.timestamp && freeToWithdrawl > 0 && freeToWithdrawl >= _amount, "Error");
        aEarning[sender].pAmount = comulativePercent;
        dWa(sender, _amount);
    ...
    function wP(uint _amount) external {
        address sender = msg.sender;
        require((aInfo[sender].lastWithdraw + 10 minutes) < block.timestamp , "Time limit");
