...
contract Chaos is 
    ...
    function setDaoAddress(address newAddr) external onlyOwner {
        daoSafeWallet = newAddr;
    }
    
    function setTeamAddress(address newAddr) external onlyOwner {
        teamWallet = newAddr;
    }

    function setLiquidityAddress(address newAddr) external onlyOwner {
    ...
    function takeTransactionTax(address from, uint256 amountToken, bool sell) internal returns(uint256) {
        ...
        super._transfer(from, daoSafeWallet, daoTax);
