...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getBundles() external view returns (uint256[6][17] memory) {
       return IMunInfo(munInfoAddr).getBundles();
    }
    ...
    function getPriceForSuperBundle(uint8 _bundleType) external returns(uint256,uint256) {
        return _getPriceForSuperBundle(_bundleType, msg.sender);
    }
    ...
    function _purchaseSuperBundle(uint8 _bundleType, uint256 _referrerId, address _user) private {	
        ...
        LastPurchaseData storage lastPurchase = lastPurchaseData[_user];	
        INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);	
        (uint256 bundlePurchasePrice,) = _getPriceForSuperBundle(_bundleType, msg.sender);	
        _transferToContract(bundlePurchasePrice);	
        userToPurchasedAmountMapping[_user] += bundlePurchasePrice;	
        _updateAdditionLevel(_user);	
        lastPurchase.dollarValue += bundlePurchasePrice;	
    ...
    function _getPriceForSuperBundle(uint8 _bundleType, address _user) private returns(uint256, uint256) {
        _validateSuperBundleType(_bundleType);
        SuperBundleInfo memory bundle = superBundlesInfos[_bundleType- BUNDLE_TYPE_SUPER_1];
        ...
        return block.timestamp < (INetGymStreet(netGymStreetAddress).termsAndConditionsTimestamp(_user) +7 days) ?
        (_discountPrice(bundlePrice, _getNewIncentiveDiscount(_bundleType)), bundlePrice) : (_discountPrice(bundlePrice, bundle.discountPct), bundlePrice);
    }

    function _getNewIncentiveDiscount(uint8 _bundleType) private view returns(uint256 _newPercentage) {
        if (_bundleType == 16) {
...
interface INetGymStreet {
    ...
    function termsAndConditionsTimestamp(address _user) external returns (uint256);
}
