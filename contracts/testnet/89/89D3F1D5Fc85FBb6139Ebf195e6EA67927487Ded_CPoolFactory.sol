...
contract CPoolFactory is Ownable {
    ...
    function createCPool(
        ...
        CPool newPool = new CPool(
            ...
            _performanceFee,
            _minAmount,
            address(IERC20(rulesContract.stableToken())),
            address(rulesContract)
        );

        IERC20(rulesContract.stableToken()).safeTransferFrom(
            msg.sender,
            address(this),
            _traderAmount
        );

        IERC20(rulesContract.stableToken()).safeTransfer(
            address(newPool),
            _traderAmount
        );
    ...
    modifier validateCreation(
        ...
        require(
            ...
            "Trader amount must be grater than the defined minimum."
        );
        require(
            _traderAmount >= rulesContract.minAmountToCreatePool(),
            "Trader amount must be grater than the minimum defined in the rules."
        );
...
contract CPool {
    ...
    uint256 public maxSharesPerParticipant;
    ...
    constructor(
        ...
        rulesContract = ICRules(_rulesAddress);
    ...
    function _init(
        ...
        investmentOf[_trader] = ICPool.Invest(
            _traderAmount,
            1 ether,
            block.timestamp + rulesContract.penaltyRange()
        );
    ...
    function closePool() external onlyTrader whenPoolIsOpen {
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            if (tokens[i] != stableCoin) {
                ...
                uint256[] memory amountOutMin = IPancakeRouter(
                    rulesContract.pancakeRouter()
    ...
    function swap(
        ...
        return _swap(_tokenIn, _tokenOut, _amountIn, _amountOutMin);
    }
    ...
    function _swap(
        ...
        IPancakeRouter router = IPancakeRouter(rulesContract.pancakeRouter());
        ...
        whenPoolIsOpen
    {
        ...
        invest.averageCostPerShare = calcAverageCostPerShare(
            oldAverageCostPerShare,
            oldShares,
            shares,
            sharePrice
        );
        invest.penaltyDeadline = block.timestamp + rulesContract.penaltyRange();

        IGlobalStats(rulesContract.globalStatsContract()).computeSuscribe(
            msg.sender,
            shares,
            oldShares,
        ...
        whenPoolIsOpen
    {
        ...
        IGlobalStats(rulesContract.globalStatsContract()).computeUnsuscribe(
            msg.sender,
            shares,
            invest.shares,
            invest.averageCostPerShare,
    ...
    function totalRedeem() external {
        require(msg.sender != trader, "Trader not allowed to total redeem.");
     ...
     *      For example: - 100000 gives a precision of 2 decimal places.
                         - 10000000 gives a precision of 4 decimal places.
     */
    function swapTokensToRedeem(uint256 _shares) private returns (uint256) {
        uint256 percentageDivider = 10000000;
        uint256 percentageOfTotal = (_shares * percentageDivider) / totalShares;
        uint256 balanceBUSD = tokenBalanceOf[address(stableCoin)];
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            if (tokens[i] != stableCoin) {
                path[0] = address(tokens[i]);
                uint256 balance = tokenBalanceOf[address(tokens[i])];
                uint256 toSwap = (balance / percentageDivider) *
                    percentageOfTotal;

                uint256[] memory amountOutMin = IPancakeRouter(
                    rulesContract.pancakeRouter()
                ).getAmountsOut(toSwap, path);
                counter += _swap(
                    address(tokens[i]),
                    address(stableCoin),
                    balance,
    ...
    function _calcPlatformFeeAmount(uint256 _amount) private returns (uint256) {
        ...
        if (msg.sender != trader && !isOpen) return 0;
        if (block.timestamp > invest.penaltyDeadline) {
            return 0;
        } else {
            return
    ...
    /* ========== VIEWS ========== */
    function calcAverageCostPerShare(
        uint256 _averagePriceIn,
        uint256 _currentShares,
        uint256 _newShares,
        uint256 _sharePrice
    ) private pure returns (uint256) {
        uint256 t1 = _averagePriceIn == 0 || _currentShares == 0
            ? 0
            : _averagePriceIn * _currentShares;

        uint256 t2 = _newShares * _sharePrice;

        return (t1 + t2) / (_newShares + _currentShares);
    }

    function getSharePrice() private view returns (uint256) {
        uint256 cPoolValue;
        for (uint256 i = 1; i < tokens.length; i++) {
            uint256 balance = tokenBalanceOf[address(tokens[i])];
            uint256 price = IPriceProvider(rulesContract.priceProvider())
                .getPrice(address(tokens[i]));
    ...
    modifier validateRedeem(uint256 _amount) {
        if (msg.sender == trader) {
    ...
    function _validateMinAmountToTrader(uint256 _amount) private {
        ...
        uint256 currentAmount = (invest.shares * getSharePrice()) / 10**18;
    ...
    function _validateAmountToSubscribe(uint256 _amount) private {
        uint256 stakedTokens = ICopyStaking(rulesContract.stakingContract())
        ...
        ) = IGlobalStats(rulesContract.globalStatsContract())
                .getMaxInvestmentAvailable(msg.sender, stakedTokens);
...
interface ICPool {
    ...
    function investedCapitalOf(address _copierAddress)
        external
        view
        returns (uint256);
