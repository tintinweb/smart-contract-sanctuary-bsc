...
contract Presale is Ownable {
  ...
  function getClaimable(address _user) public view returns (bool) {
    return openClaim == true && boughtTokens[_user] && saleStartTime.add(privateSalePeriod) < block.timestamp;
  }
  ...
  function purchase(uint256 _purchaseAmount) external returns (bool) {
    ...
    IERC20(busd).safeTransferFrom(msg.sender, address(this), _purchaseAmount);
  ...
  function withdraw() external onlyOwner {
    ...
    openIdo = false;
    uint256 blstAmount = IERC20(blstToken).balanceOf(address(this)).sub(sellAmount.mul(rate).mul(10 ** 18).div(100));
