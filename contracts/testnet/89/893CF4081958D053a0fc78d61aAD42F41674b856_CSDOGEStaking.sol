...
contract CSDOGEStaking is Ownable, ReentrancyGuard {
    ...
    {
        return stakers[_address].stakingNFTs;
    }

    function getStakedNFTLength(address _address)  public view returns(uint256)
    {
        return stakers[_address].stakingNFTs.length;
    }
     ...
     function nftStakeDetails(uint256 tokenid) external view returns(uint256,uint256,address,uint256,uint256, uint256)
    {
        ...
        if (stakerAddress[tokenid][msg.sender] == true){
            ...
            return (tokenid, block.timestamp- detail.depositTime,detail.useraddress,detail.depositTime,calculateRewardsPerToken(msg.sender, tokenid), detail.amountStaked);
        }
        else{
    ...
    function withdraw(uint256[] calldata _tokenIds, uint256[] calldata _amounts) external nonReentrant {
        require(
            stakers[msg.sender].TotalAmountStaked > 0,
        ...
        for (uint256 i; i < len; ++i) {
            ...
            require(stakerAddress[_tokenIds[i]][msg.sender] = true);
            stakerAddress[_tokenIds[i]][msg.sender] = false;
            nftCollection.safeTransferFrom(address(this), msg.sender, _tokenIds[i],_amounts[i],"");
            stakers[msg.sender].TotalAmountStaked -=_amounts[i];
        }
        stakers[msg.sender].lastUpdateTime = block.timestamp;
        if (stakers[msg.sender].TotalAmountStaked == 0) {
    ...
    {
        return (stakers[_user].TotalAmountStaked, availableRewards(_user));
    }
        ...
        returns( uint256 _reward)
    {
       ...
       if (rewardPercentage != 0)
        ...
        if (StakeInfo.depositTime < PoolInfostartOfDeposit)
        {
            reward = 0;
        }
        else if (StakeInfo.depositTime < PoolInfoendOfPool)
        {
           reward = PoolInfoTotalReward/PoolInfototalStaked;
           reward = reward * (block.timestamp - StakeInfo.depositTime)/  (MINUTES_IN_DAY * 60 * PoolInfoperiodStaking);
        }
        else{
            reward = PoolInfoTotalReward/PoolInfototalStaked;
        }
