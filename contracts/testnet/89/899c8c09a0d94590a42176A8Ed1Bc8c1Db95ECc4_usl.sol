...
function aauser_choujiang()external  returns(uint256  a){
uint _test = uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender))) % 3+1;
a=_test;
}
...
//ipo_start ipo_amount ipo_count user_level user_yeji
    ...
    function  tixian(uint256 num)  external returns (bool) {
        ...
        token.transfer(msg.sender, num*10**18);
