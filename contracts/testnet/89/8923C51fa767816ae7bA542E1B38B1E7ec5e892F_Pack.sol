...
contract Pack is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function blacklistAddress(address account, bool isBlacklisted) external onlyOwner {
        ...
        emit BlacklistStatusChanged(account, isBlacklisted);
    }
    ...
    /// @notice Sets the fees that will be applied to buys/sells (Total fees can not be greater than 15%)
    /// @param lpFee The fee in % that will be sent to the liquidity wallet
    /// @param devFee The fee in % that will be sent to the marketing wallet
    /// @param divFee The fee in % that will be used to pay dividend rewards
    function setBaseFees(uint256 lpFee, uint256 devFee, uint256 divFee) public onlyOwner {
        require((_liquidityFee + devFee + divFee) <= 15, "taxes !> 15%");
        liquidityFee = lpFee;
        marketingFee = devFee;
        rewardFee = divFee;
        totalFee = lpFee + devFee + divFee;
        emit FeeChange("baseFee", lpFee, devFee, divFee);
    }
    ...
    function _transfer(
        ...
        // Antibot can be removed after upgrade
