...
contract NFTSale is NFTSaleLib, RoyaltyPayer {
    ...
    event SellerWithdraw(uint256 indexed id, address indexed owner);
    ...
    function _calculateFees(
        ...
        // TODO test and if fees are the problem, revert the change on line below
        PlatformFees memory platformFees = _getFees(saleInfo.owner, saleInfo.createdOn);
        ...
        if (saleInfo.nftItemId>0 && _nft.owner()!=saleInfo.owner)
        {
    ...
    function buy(uint256 id, uint256 providedPrice) public whenNotPaused payable returns(uint256) {
    ...
    function bid(uint256 id, uint256 bidPrice) public whenNotPaused payable {
        INFTSaleStructs.SaleInfo memory saleInfo = _storage.getSale(id);
    ...
    function deleteSale(uint256 id) public whenNotPaused payable {
        ...
        if(saleInfo.nftItemId > 0)
            _nftStorage.transferTo(saleInfo.nftAddress, saleInfo.owner, saleInfo.nftItemId);
        emit SaleDeleted(id);
    }

    function sellerWithdraw(uint256 id) public whenNotPaused payable {
        INFTSaleStructs.SaleInfo memory saleInfo = _storage.getSale(id);
        require (saleInfo.id==id, 'not found');
        require (saleInfo.saleType==INFTSaleStructs.SaleType.Auction, 'sale type must be Auction');
        require (saleInfo.status==INFTSaleStructs.SaleStatus.Active, 'must be active');
        ...
        require(saleInfo.owner == _msgSender(), "not auth");
        require(saleInfo.auctionInfo.isSellerWithdrawComplete == false, "already withdrawn");
        ...
        _storage.completeSellerWithdraw(saleInfo.id);
        emit SellerWithdraw(saleInfo.id, saleInfo.auctionInfo.highestBidder);
    }
    ...
    function bidderWithdraw(uint256 id) public whenNotPaused payable {
        address payable bidder = payable(_msgSender());
        INFTSaleStructs.SaleInfo memory saleInfo = _storage.getSale(id);
    ...
    function _buy(INFTSaleStructs.SaleInfo memory saleInfo, uint256 totalPrice, bool isWinnerPayout, address payable buyer) internal returns(uint256) {
       ...
       Fees memory fees = _calculateFees(
        ...
        if (isWinnerPayout && saleInfo.saleType == INFTSaleStructs.SaleType.Auction) {
            ...
            if(!saleInfo.auctionInfo.isSellerWithdrawComplete){
                 _storage.sellerWithdraw(
                        saleInfo.id,
                        payable(saleInfo.auctionInfo.highestBidder),
                        _feeWallet,
