...
contract AltSwitch is ERC20, Ownable {
    ...
    event ExcludeFromFees(address indexed account, bool included);
    ...
    event AntiDumpUpdated(uint256 increaseFactor);
    ...
    constructor() ERC20("AltSwitch", "ALTX", 9) {
        ...
        updateMinimumRebate(0); // @note disabled
    ...
    // Register wallet/contract as enterprise.
     ...
     */
    function setRebate(address addr, uint16 percentRebate)
        external
        onlyEnterprise
    {
        ...
        if (owner() == msg.sender) {
            ...
            _isWithCustomRebate[addr] = true;
            _percentRebate[msg.sender] = percentRebate;
        }
    }
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 rebate = amount.mul(getPercentRebate(to)).div(10000);
    ...
    function setAntiDump(uint256 _sellIncreaseFactor) external onlyOwner {
        require(_sellIncreaseFactor <= 10, "Max Sell increase factor is 10%");

        sellIncreaseFactor = _sellIncreaseFactor;
        emit AntiDumpUpdated(_sellIncreaseFactor);
    }
