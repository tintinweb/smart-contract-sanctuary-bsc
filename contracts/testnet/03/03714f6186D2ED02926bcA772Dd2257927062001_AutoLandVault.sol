...
interface ILandTokenStakingV2 {
    ...
    function _userInfo(address _user) external view returns (uint256, uint256);
...
contract AutoLandVault is Ownable, Pausable {
    ...
    ILandTokenStakingV2 public immutable masterchef;
    ...
    constructor(
        ...
        ILandTokenStakingV2 _masterchef,
        address _admin,
        address _treasury
    ) public {
        token = _token;
    ...
    function harvest() external notContract whenNotPaused {
        ILandTokenStakingV2(masterchef).withdraw(0);
    ...
    function emergencyWithdraw() external onlyAdmin {
        ILandTokenStakingV2(masterchef).emergencyWithdraw();
    }
    ...
    function calculateHarvestCakeRewards() external view returns (uint256) {
        uint256 amount = ILandTokenStakingV2(masterchef).pendingReward(address(this));
    ...
    function withdraw(uint256 _shares) public notContract {
        ...
        if (bal < currentAmount) {
            ...
            ILandTokenStakingV2(masterchef).withdraw(balWithdraw);
    ...
    function balanceOf() public view returns (uint256) {
        (uint256 amount, ) = ILandTokenStakingV2(masterchef)._userInfo(address(this));
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            ILandTokenStakingV2(masterchef).deposit(bal);
        }
    }
