...
contract Game_NFT is Ownable {
    ...
    function StartGame(uint256 number_table) public payable { ///////////////5 метод
       // require(msg.value == min_price*(number_table+1), "Not enough BNB provided");
        ...
        require(time_now >= (time_now + 86400)*number_table, "Table closed");
    ...
    function Check_Progress(uint256 number_table) public view returns(uint256) {
       return all_tables[number_table].progress_game_percent[msg.sender];
    }

    function Check_InGame(uint256 number_table) public view returns(bool) {
       return all_tables[number_table].in_game[msg.sender];
    }

    function Check_Zarabotok(uint256 number_table) public view returns(uint256) {
       return all_tables[number_table].zarabotok[msg.sender];
    }
    ...
    function Add_Inviter(address inviter) public{   //////////////////////////1 метод
        ...
        if(my_inviter[msg.sender] == address(0) && inviter != msg.sender){
