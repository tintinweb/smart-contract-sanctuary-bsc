    ...
    ReentrancyGuard
{
    ...
    constructor(string memory name_, string memory symbol_) payable ERC721WithPermit(name_, symbol_) {
        ...
        _setType(1, address(0), 11000000000000000000, 10, 2);
        _setType(2, address(0), 12000000000000000000, 10, 2);
        _setType(3, address(0), 13000000000000000000, 10, 2);
        _setType(4, address(0), 14000000000000000000, 10, 3);
        _setType(5, address(0), 15000000000000000000, 10, 1);
        _setPriceFeeds(address(0), 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526); // BNB
        _idCounter = 1;
    }
    ...
    function withdrawToken(address token_, uint256 amount_) external onlyOwner {
        address sender = _msgSender();
        if (token_ == address(0)) {
            CurrencyTransferLib.safeTransferNativeToken(sender, amount_);
        } else {
            CurrencyTransferLib.safeTransferERC20(token_, address(this), sender, amount_);
        }
    }
    ...
    function buy(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
        ...
        uint256 paymentAmount = _getTokenAmount(typeInfo.paymentToken, total);

        if (total == 0) revert NFT__InvalidType();

        if (typeInfo.paymentToken == address(0)) {
            if (msg.value < paymentAmount) revert NFT__InsufficientBalance();
            CurrencyTransferLib.safeTransferNativeToken(_recipient, msg.value);
        } else {
            CurrencyTransferLib.safeTransferERC20(typeInfo.paymentToken, sender, _recipient, paymentAmount);
        }
    ...
    function getTokenAmount(address paymentToken, uint256 usdAmount) external view returns (uint256) {
        return _getTokenAmount(paymentToken, usdAmount);
    }
