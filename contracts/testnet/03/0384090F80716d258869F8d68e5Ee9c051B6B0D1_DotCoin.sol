...
contract DotCoin is Context, IERC20, Ownable{
    ...
    uint256 public maxTxAmount = 1_000_000e18;
    uint256 private minTokensSellToAddToLiquidity = 10e18;
    ...
    constructor (string memory name_,string memory symbol_,address[] memory wallets_,bool isMainNetwork_){
        ...
        excludeFromReward(address(_pancakeswapV2Router));
        excludeFromReward(pancakeswapV2Pair);
     

        //initlize wallets
        _initilizeWallets(wallets_);

        //Initilize tokenomics
         _initilizeTokenomics();


        
    }

    function _initilizeTokenomics() private{

         // ICO - 5% (5, 00, 00, 000) – Single Wallet
            transfer(icoWallet, _tTotal.mul(50).div(1000));
    ...
    function _initilizeWallets(address[] memory wallets_) private{

      //Wallets
      icoWallet=wallets_[0];
      _isExcludedFromFee[icoWallet] = true;
      excludeFromReward(icoWallet);

      airDropWallet=wallets_[1];
      _isExcludedFromFee[airDropWallet] = true;
      excludeFromReward(airDropWallet);

      bufferWallet=wallets_[2];
      _isExcludedFromFee[bufferWallet] = true;
      excludeFromReward(bufferWallet);

      managementWallet=wallets_[3];
      _isExcludedFromFee[managementWallet] = true;
      excludeFromReward(managementWallet);

      rewardWallet=wallets_[4];
      _isExcludedFromFee[rewardWallet] = true;
      excludeFromReward(rewardWallet);

      creatorsWallet=wallets_[5];
      _isExcludedFromFee[creatorsWallet] = true;
      excludeFromReward(creatorsWallet);

      burnWallet=wallets_[6];
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >= minTokensSellToAddToLiquidity;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquify &&
            ...
            contractTokenBalance = minTokensSellToAddToLiquidity;
            //add liquidity
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount,uint256 rTransferAmount,uint256 rRefelctionFee,,,,uint256 tTransferAmount,uint256 tReflectionFee,uint256 tLiquidityFee,uint256 tRewardFee,uint256 tBurnFee) = _getValues(tAmount);
...
(uint256 rAmount,uint256 rTransferAmount,uint256 rRefelctionFee,,,,uint256 tTransferAmount,uint256 tReflectionFee,uint256 tLiquidityFee,uint256 tRewardFee,uint256 tBurnFee) = _getValues(tAmount);
    ...
    function changeTax(uint256 newLiquidityFee_,uint256 newBurnFee_,uint256 newRewardFee_,uint256 newReflectFee_) external returns(bool) {
        
        require(_msgSender()==owner(),"You are not Authorised");
        require((newLiquidityFee_).add(newBurnFee_).add(newRewardFee_).add(newReflectFee_)<=7);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256,uint256, uint256, uint256, uint256, uint256) {
         uint256[] memory values=new uint256[](11);
        (values[0],values[1],values[2],values[3],values[4]) = _getTValues(tAmount);
        (values[5],values[6],values[7],values[8],values[9],values[10]) = _getRValues( tAmount,  values[1],  values[2], values[3] , values[4], _getRate());
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256,uint256,uint256) {
        ...
        values[0] = calculateReflectionFee(tAmount);
        values[1] = calculateLiquidityFee(tAmount);
        values[2]=calculateRewardFee(tAmount);
        values[3]=calculateBurnFee(tAmount);

        uint256 tTransferAmount = tAmount.sub(values[0]).sub(values[1]).sub(values[2]).sub(values[3]);
        return (tTransferAmount, values[0], values[1],values[2],values[3]);
    }

    function _getRValues(uint256 tAmount, uint256 tReflectionFee, uint256 tLiquidityFee,uint256 tRewardFee ,uint256 tBurnFee, uint256 currentRate) private pure returns (uint256, uint256, uint256,uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rRefelctionFee = tReflectionFee.mul(currentRate);
    ...
    function calculateReflectionFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(reflectionsFee).div(
            10**2
        );
    }
    ...
    function calculateRewardFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(rewardsFee).div(
            10**2
        );
    }
 ...
 //Locking
    ...
    uint256 public constant FREQUENCY_OF_UNLOCK = 10; // This value should be in day
        ...
        returns (uint256)
    {
        ...
        if (locked[recipient_].amount > 0) {
            ...
            unlockDate =
                ...
                (60 *  locked[recipient_].frequencyOfUnlock);
    ...
    function unLock(address recipient_) public  returns (bool) {
        if (locked[recipient_].amount > 0) {
            ...
            if (isModify) {
                   if(burnWallet == recipient_){
                    if(locked[recipient_].unlockAmount + unlockAmount > locked[recipient_].amount){
                        _tokenBurn(burnWallet,locked[recipient_].amount - locked[recipient_].unlockAmount);
                        delete locked[recipient_];  // delete record from struct
                        _removeLockAddress(recipient_);
                    }
                    else{
                        _tokenBurn(burnWallet,unlockAmount);
                        locked[recipient_].unlockAmount += unlockAmount;
    ...
    /// @return true
    function _tokenBurn(address _sender, uint256 amount) private returns (bool){
        bool takeFee = true;
