...
interface BTCF {
  ...
  */
  ...
  event TransferOperationFee(address indexed from, address indexed to, uint256 value);
...
contract BITCOINFUTURE is Context, BTCF, Ownable {
  ...
  */
   ...
   *
   * Emits a {Transfer} event.
   * Emits a {TransferOperationFee} event if fee is applicable
   *
   * Requirements:
   *
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(_feeAmount == 0) {
      ...
      emit TransferOperationFee(sender, _feeWallet, _feeAmount);
    }
  }
