...
import "../../../libs/atlantis/AtlantisComptroller.sol";  
import "../../../libs/atlantis/ExponentialNoError.sol"; 
 
import "../../interfaces/ILendingAdapter.sol";

contract AtlantisAdapter is ILendingAdapter, ExponentialNoError {

    AtlantisComptroller public immutable comptroller;

    address constant NATIVE_TOKEN_ADDRESS = address(0x1E1e1E1E1e1e1e1e1e1E1E1E1E1e1e1E1e1e1E1E);

    constructor(AtlantisComptroller account) {
        comptroller = account;
    }
    ...
    function getMarket(address asset) external view override returns(address platformToken) {
        AToken[] memory tokens = comptroller.getAllMarkets();
        for (uint i = 0; i < tokens.length; i++) {
            AToken aToken = tokens[i];
            if (aToken.underlying() == asset)
    ...
    function underlying(address platformToken) public view override returns(address asset) {
        AToken aToken = AToken(platformToken);
        return _underlying(aToken);
    }

    function _underlying(AToken aToken) internal view returns(address asset) {
        if (compareStrings(aToken.symbol(), "aBNB"))  
            return NATIVE_TOKEN_ADDRESS;
        return aToken.underlying();
    }

    function amountUnderlying(address platformToken, uint amount) public view override returns(uint) {
        AToken aToken = AToken(platformToken);
        Exp memory exchangeRate = Exp({mantissa: aToken.exchangeRateStored()});
    ...
    function deposit(address platformToken, uint amount) external override {
        AToken token = AToken(platformToken);
        token.mint(amount);
    }
        ...
        returns(uint totalDeposit, uint totalCollateral, uint totalBorrow, uint healthFactor) 
    {
        ...
        uint oErr;
        AToken[] memory assets = comptroller.getAssetsIn(account);
...
interface AtlantisComptroller {

    function oracle() external view returns(AtlantisPriceOracle); 

    function getAllMarkets() external view returns (AToken[] memory);

    function markets(address aToken) external view returns(bool isListed, uint collateralFactorMantissa);
    ...
    function atlantisAccrued(address holder) external view returns(uint);
...
interface AToken  {
    ...
    function comptroller() external view returns(AtlantisComptroller);
