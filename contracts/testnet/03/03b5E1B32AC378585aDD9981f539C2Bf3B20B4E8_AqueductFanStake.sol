...
contract AqueductFanStake is
    ...
    mapping(uint256 => uint256) public poolToTokensAmount;
    ...
    modifier poolShouldExist(uint256 poolId_) {
        require(poolId_ <= _poolId.current() &&  poolId_ != 0, "Pool doesn't exist");
        _;
    }
    ...
    modifier stageShouldBeFinished(uint256 poolId_) {
        require(stages[pools[poolId_].stageId].endAt <= block.timestamp, "Stage should be finished");
        _;
    }
    ...
    event SectionCreated(string sectionName, uint256 sectionId, uint256 ednDate);
    ...
    initializer
    {
        ...
        VRFConsumerBaseV2Upgradable.init(0x6A2AAd07396B36Fe02a22b33cf443582f682c82f);
    ...
    override(VRFConsumerBaseV2Upgradable)
    {
        ...
        for (
            ...
            i < requestIdToGoldAmount[requestId] + requestIdToSilverAmount[requestId];
            i++
        ) {
    ...
    function reveal(uint256 poolId_) public onlyAdmin stageShouldBeFinished(poolId_) poolShouldExist(poolId_) {
    ...
    function createStake(
    ...
    ) public poolShouldExist(poolId_) stageShouldNotBeFinished(poolId_) whenNotPaused {
        ...
        for (uint256 i = 0; i < amount_; i++) {
            uint256 stakeId = _poolIdToStakeId[_poolId.current()];
            poolIdToStakesIdToTokenId[poolId_][stakeId] = _tokenId
            .current();
        ...
        emit StakeCreated(
            ...
            _tokenId.current(),
            referralAddress_,
            tokenToMetadata[_tokenId.current()]
        );
    }
    ...
    function revealToken(uint256 tokenId_, string memory matadata) public onlyAdmin shouldBeUsersToken(betIdToPoolId[tokenId_], tokenId_) {
    ...
    {
        require(sectionId_ <= _sectionId.current(), "Section doesn't exist");
        require(sections[sectionId_].endDate <= block.timestamp, "Section is not finished");
    ...
    shouldBeUsersToken(betIdToPoolId[tokenId_], tokenId_)
    {
        ...
        require(
            ifStringsEqual(bet_.stakeOn, sectionToWinner[pools[poolId].sectionId]),
