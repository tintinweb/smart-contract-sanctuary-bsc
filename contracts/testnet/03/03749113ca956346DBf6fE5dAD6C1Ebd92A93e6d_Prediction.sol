...
contract Prediction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function addPool(uint256 _eventId, address _token) public {
        EDataTypes.Event memory _event = eventData.info(_eventId);
        if (predictStats[_eventId][_token].options.length == 0) {
            predictStats[_eventId][_token].options = new uint256[](_event.odds.length);
        }
    }
