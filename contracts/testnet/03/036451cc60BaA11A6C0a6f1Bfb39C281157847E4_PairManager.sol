...
contract PairManager is Initializable, IPairManager, Block, PairManagerStorage {
    ...
    modifier onlyCounterParty() {
        require(counterParty == _msgSender() || liquidityPool == _msgSender(), Errors.VL_ONLY_COUNTERPARTY);
        _;
    }
    ...
    modifier onlyLiquidityPool() {
        require(liquidityPool == _msgSender(), Errors.VL_ONLY_LIQUIDITY_POOL);
        _;
    }
    ...
    // TODO change to cancel and claim the needed amount
    function claimAmountFromLiquidityPool(
        uint256 quoteAmount,
        uint256 baseAmount,
        address user
    ) external override onlyLiquidityPool {
        address pairManagerAddress = address(this);

        if (quoteAmount > 0) {
            _transfer(quoteAsset, user, quoteAmount);
        }


        if (baseAmount > 0) {
            _transfer(baseAsset, user, baseAmount);
        }

    }


    function collectFund(IERC20 token, address to, uint256 amount) external override onlyLiquidityPool {
        _transfer(token, to, amount);
    }
    function cancelGridOrders(bytes32[] memory _orderIds) public override onlyLiquidityPool returns (uint256 base, uint256 quote) {
        uint256 _basisPoint = basisPoint;
        for (uint256 i = 0; i <_orderIds.length; i++){
            (uint128 _pip, uint64 _orderId, bool _isBuy) = _orderIds[i].unpack();
            uint256 filledSize;
            (
                bool isFilled,
                ,
            ...
            if(isFilled){
                ...
                // cancelget
                (uint256 sizeLeft, uint256 partialFilled) = _internalCancelLimitOrder(_pip, _orderId);
                if(_isBuy){
        ...
        )
    {
        ...
        if (user == liquidityPool) {
            ...
            if(baseAmountUsed > uint256(claimableBase)){
                ...
                uint256 baseClaimed = uint256(claimableBase) - baseAmountUsed;
                baseAmountUsed = 0;
            ...
            if(quoteAmountUsed > uint256(claimableQuote)) {
                ...
                uint256 quoteClaimed = uint256(claimableQuote) - quoteAmountUsed;
                quoteAmountUsed = 0;
        ...
        returns (IPairManager.ExchangedData memory, bool isFilled)
    {
        ...
        if (isFilled) {
            ...
            if(isBuy){
                ...
                exData.quoteAmount += partialFilled.baseToQuote(_pip, basisPoint);
            }
    ...
    function _internalCancelLimitOrder(uint128 pip, uint64 orderId) internal returns (uint256 remainingSize, uint256 partialFilled) {
        bool isBuy;
        (remainingSize, partialFilled, isBuy) = tickPosition[pip].cancelLimitOrder(orderId);
...
//            }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            ...
            // if pip == 0
            if (
                (_order.pip == _singleSlot.pip && hasLiquidity && _singleSlot.isFullBuy != (isBuy ? 1 : 2)) ||
    ...
    function _openMarketWithMaxPip(
        ...
        uint128 _maxFindingWordsIndex = (isBuy ? maxPip - singleSlot.pip : singleSlot.pip - maxPip)/250;
        return _internalOpenMarketOrderWithMaxFindingWord(size, isBuy, maxPip, address(0), true, _maxFindingWordsIndex);
    }
    ...
    function _internalOpenMarketOrder(
        ...
        return _internalOpenMarketOrderWithMaxFindingWord(
            _size,
            _isBuy,
            _maxPip,
            _trader,
    ...
    function _internalOpenMarketOrderWithMaxFindingWord(
        ...
        }
        {
            ...
            if (_maxPip != 0) {
                ...
                singleSlot.pip = state.remainingSize != 0 ? _maxPip : state.lastMatchedPip;
            } else {
    ...
    returns (
        ...
        {
            if (isBuy) {
                int128 maxPip = int128(_singleSlot.pip) - int128(maxWordRangeForLimitOrder * 250);
                if (maxPip > 0) {
                ...
                require(_pip >= 1, Errors.VL_MUST_CLOSE_TO_INDEX_PRICE_LONG);
                require(
                    _pip <= (_singleSlot.pip + maxWordRangeForLimitOrder * 250), Errors.VL_MUST_CLOSE_TO_INDEX_PRICE_SHORT
                );
            }
        bool hasLiquidity = liquidityBitmap.hasLiquidity(_pip);
        //save gas
        {
            bool canOpenMarketWithMaxPip = (isBuy && _pip >= _singleSlot.pip)
            || (!isBuy && _pip <= _singleSlot.pip);
            if (
...
//                }
        ...
        if (size > sizeOut) {
            ...
            emit LimitOrderCreated(orderId, _pip, remainingSize, isBuy);
        }
    }
    }
    ...
    function clearCurrentPip() public onlyOwner{
