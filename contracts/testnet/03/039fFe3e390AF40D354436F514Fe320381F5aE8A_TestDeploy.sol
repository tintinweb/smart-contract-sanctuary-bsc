...
contract TestDeploy is Context, IERC20, Ownable {
    ...
    string  private constant _name = "TANATA"; 
    ...
    address payable public   BuyBackWalletAddress =payable(0x26f07c9657BcE24B4416E0072cF20e47c89DD171);
    address payable public   _devWallet           =payable(0x4eb195eE5C338Ec4A306cDEAbE2FC7A5899BCd59);
...
if (takeFee)
{
  ...
  function swapAndLiquify(uint256 _yehaiCtrctkakhata) private {
           ...
           uint256 tokens_to_Burn = _yehaiCtrctkakhata.mul(_brnTx).div(100);
            ...
            uint256 tokens_to_LP_Half = _yehaiCtrctkakhata.mul(_liqTx).div(200);
            ...
            swapTokensForBNB(tokens_to_LP_Half.add(tokens_to_M).add(tokens_to_D));
             ...
             uint256 split_D = _dvTx.mul(100).div(_dvTx.add(_promTx).add(_dvTx));
             uint256 BNB_D = BNB_Total.mul(split_D).div(100);

             emit _split(split_M,BNB_M , split_D , BNB_D);
             emit _addLiquidity(tokens_to_LP_Half , (BNB_Total - BNB_M - BNB_D));
      ...
      address[] memory path = new address[](2);
       ...
       emit _swapTokenETH(address(this) , address(uniswapV2Router) ,tokenAmount ,   BuyBackWalletAddress);
