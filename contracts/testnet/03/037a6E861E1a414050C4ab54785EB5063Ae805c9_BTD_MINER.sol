...
contract BTD_MINER {
    ...
    function hatchBTD(address ref) public payable {
        ...
        if(lastHatched[msg.sender] > lastCliamed[msg.sender]){
            unpaidedBtd[msg.sender] = getMyBTD();
        }else{
    ...
    function getMyBTD() public view returns(uint256) {
        return SafeMath.add(unpaidedBtd[msg.sender],getBTDSinceLastHatch(msg.sender));
    }
    ...
    function getPrice() internal view returns (uint256){
        ...
        uint256 addition = 0;
        if(son[msg.sender] > 0){
            addition = addition +son[msg.sender] * 2;
        }

        if(grandsons[msg.sender] > 0){
            addition = addition +grandsons[msg.sender] * 1;
        }
