...
contract Web3ShotPassportLevel {
    ...
    function setPassportLevelByUser(
        ...
        // check level up
        uint32 level = 1 + learningPoints / 10 + connexConnections;
        if (!info.connexProfile && connexProfile) {
            level += 10;
        }
        require(info.level < level, "level is less than or equal to prev");

        _PASSPORT_LEVEL_UP_INFO[owner] = PassportLevelUpInfo(tokenId, learningPoints, connexProfile, connexConnections, level);
