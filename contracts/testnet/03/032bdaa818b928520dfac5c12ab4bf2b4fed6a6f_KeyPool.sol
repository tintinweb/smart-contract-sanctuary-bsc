...
contract KeyPool is Ownable, AYiBoxBase {
    ...
    function purchase(address _staker, uint256 _keyNum) external lock onlyOwner isOpen returns (bool) {
        ...
        uint256 _incomeNow = getIncome(_ks,_now); //calcIncome(stake[_idx].Amount, now - stake[_idx].lastTime);
    ...
    */
    ...
    function recordStake(uint256 _time) internal returns (uint256 base) {
        ...
        if (stakeTimeStamp == 0) {
            // uint256 startTime = getStartTime();
            YiBoxBase.SetParam(YiBoxType.KeyStakeTimeStamp, _time);
        } else {
            uint256 _delay;
    ...
    function calcIncome(uint256 _stakePrice, uint256 _delay , uint256 BusdCount, uint _lastAirDrop) private view returns(uint256) {
        ...
        //计算累计收益
    function getIncome(KeyStake memory _ks, uint256 _time) internal returns (uint256 _income) {
