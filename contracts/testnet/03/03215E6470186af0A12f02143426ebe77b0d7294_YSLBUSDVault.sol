...
@dev This contract is for YSL-BUSD vault.
     Admin.tokenSwap(); address of TokenSwap Contract.
     Admin.YSL(); address of YSL token.
     Admin.BUSD(); address of BUSD token.
     Admin.WBNB(); address of WBNB.
     Admin.masterNTT(); address of masterNTT.

 */
contract YSLBUSDVault is AccessControl, Initializable {
    ...
    address public yslBUSD_A;
    IUniswapV2Pair yslBUSD;
    ...
    function initialize(
        ...
        Admin = IAdmin(_admin); 
        yslBUSD = IUniswapV2Pair(_yslBUSD);
        startTime = block.timestamp;
        router = _router;
        yslBUSD_A = Clones.clone(Admin.masterNTT());
        INTT(yslBUSD_A).initialize(address(this), "YSL-BUSD-A", "YSL-BUSD-A");
    ...
    function deposit(address user, uint amountLp) external {
        ...
        IERC20(address(yslBUSD)).transferFrom(user, address(this), amountLp);
        INTT(yslBUSD_A).mint(address(this),(amountLp * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        exchangeRatio = (IERC20(address(yslBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(yslBUSD_A).totalSupply());
    ...
    function withdraw(address user, bool isReciept,uint _amount) external{
        ...
        if(isReciept){
            IERC20(yslBUSD_A).safeTransfer(user,_amount);
        }
        else{
            uint balance = (_amount * exchangeRatio)/10 ** 18;
            IERC20(address(yslBUSD)).safeTransfer(user,balance);
            INTT(yslBUSD_A).burn(address(this),balance);
        }
    ...
    */
    function claimReward() external{
        require(UserDeposit[msg.sender] > 0, "YSLBUSDVault: Deposit first");
        uint balance = (UserDeposit[msg.sender] * exchangeRatio)/(IERC20(address(yslBUSD)).balanceOf(address(this)) * 10 ** 18);
    ...
    */
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        IERC20(address(yslBUSD)).safeTransferFrom(user, address(this),amount);
        uint balance = purchaseOf(amount);
        require(balance > minAmount, "YSLBUSD: Amount is less than the minAmount");
        INTT(yslBUSD_A).mint(user, balance);
        exchangeRatio = (IERC20(address(yslBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(yslBUSD_A).totalSupply());
        return(balance);
    }
/**
    ...
    */
    function sell(address user, uint amount, uint minAmount) external returns(uint){
        INTT(yslBUSD_A).burn(user,amount);
