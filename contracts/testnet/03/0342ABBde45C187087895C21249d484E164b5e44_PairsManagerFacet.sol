...
contract PairsManagerFacet is IPairsManager {
    ...
    event Fix(uint indexed i, uint indexed i2, address indexed base, uint i3);

    function fix() external {
        LibPairsManager.PairsManagerStorage storage pms = LibPairsManager.pairsManagerStorage();
        address[] memory bases1 = pms.slippageConfigPairs[0];
        for(uint i; i<bases1.length;i++) {
            emit Fix(111, i, bases1[i], pms.pairs[bases1[i]].slippagePosition);
            pms.pairs[bases1[i]].slippagePosition = uint16(i);
        }

        LibFeeManager.FeeManagerStorage storage fms = LibFeeManager.feeManagerStorage();
        address[] memory bases2 = fms.feeConfigPairs[0];
        for(uint i; i<bases2.length;i++) {
            emit Fix(222, i, bases2[i], pms.pairs[bases2[i]].feePosition);
            pms.pairs[bases2[i]].feePosition = uint16(i);
        }
    }
