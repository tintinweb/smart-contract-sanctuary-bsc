 ...
 * in IPFS
 */
contract BlockToWin is Initializable, AccessControlUpgradeable {
  ...
  mapping(address => uint256[]) public idDocumentsApprovals;
  ...
  modifier onlyRole(bytes32 _role) {
    require(hasRole(_role, msg.sender), "BlockToWin: must have a role");
    _;
  }
   ...
   */
  function register(TypeOfOwner typeOfOwner, string memory name) public {
    approvals[msg.sender].user = msg.sender;
    approvals[msg.sender].typeOfOwner = typeOfOwner;
    approvals[msg.sender].name = name;
    approvalsCount++;
    emit Registered(msg.sender, typeOfOwner);
  }
   ...
   */
  function approve(address approval) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(approval != address(0), "BlockToWin: ZERO_ADDRESS");
    require(approvals[approval].user == approval, "BlockToWin: APPROVAL_ERROR");
    grantRole(APPROVAL_ROLE, approval);

    approvals[approval].approved = true;
    emit Approved(msg.sender, approval);
  }
   ...
   */
  function reject(address approval) public onlyRole(DEFAULT_ADMIN_ROLE) {
    require(approval != address(0), "BlockToWin: ZERO_ADDRESS");

    bool success;

    if (approvals[approval].user == approval) {
      delete approvals[approval];
      success = true;
      emit Rejected(msg.sender, approval);
    }
  ...
  function submitDocument(
    ...
    idDocumentsApprovals[msg.sender].push(documentsCount);
    documentsCount += 1;
    emit DocumentSubmitted(msg.sender, name, promo, hash, uploadedAt, ext);
  }
  ...
  {
    require(
      documents[documentIndex].owner == msg.sender,
  ...
  function isPending(address _user) public view returns(bool) {
    if (approvals[_user].user==_user && approvals[_user].approved==false) {
      return true;
    }
    return false;
  }
  ...
  {
    return idDocumentsApprovals[msg.sender];
  }

  /**
   *  @notice Cancel a document at `index` in the mapping of documents
   */
  function cancelDocument(uint256 index)
    ...
    returns (bool)
  {
    require(msg.sender == documents[index].owner, "Not document's owner");
  ...
  function removeApproval(address _approval)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(_approval != address(0), "BlockToWin: ZERO_ADDRESS");
    revokeRole(APPROVAL_ROLE, _approval);
    if (approvals[_approval].user == _approval) {
      approvals[_approval].approved = false;
    }
  }
