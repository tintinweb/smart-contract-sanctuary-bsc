...
contract FOFPlayerRaceData is Ownable {
    ...
    mapping(uint256 => address[3]) public clubRaceMembers;//俱乐部参赛会员tokenid-address
    ...
    mapping(uint256 => mapping(uint256 => uint256[])) public raceVehicles;//比赛类型-比赛场次-tokenid
...
contract FOFPlayerRaceQuery is Ownable {
    ...
    constructor() {
        ...
        FPRD = FOFPlayerRaceData(payable(0x4a017320Ac65649C922aa67354cA410B94564807));
    }
    ...
    uint256[] memory clubIds, string[] memory cardNicknames,uint256[] memory vehicleTypes,uint256[] memory rewards,uint256[] memory fofRewards) {
        ...
        //     (rewards[i],fofRewards[i]) = raceReward(raceType,FPRD.raceIds(raceType) + 1 - index,i);
        // } 
    }
    ...
    function checkCanSignUp(address user, uint256 raceType, uint256 raceId,uint256 vehicleId) public view returns(uint256) {
        ...
        if(FPRD.getRaceVehicles(raceType, raceId).length >= 10) return 6;//"Registration for this race is full"
