...
// 
contract Tokam is ERC20, ERC20Permit, ERC20Burnable, Ownable, Pausable, ReentrancyGuard {
    ...
    constructor() ERC20("Tokam", "TKM") ERC20Permit("Tokam") {
     
        _mint(msg.sender, 100 * (10**7) * 10 ** decimals());
        planExpired = block.timestamp + _planExpired;
        totalStakers = 0;
        token = ERC20(address(this));
    }

    /*transfers a specified amount of tokens to an address. 
    This function basically deducts an amount (_value) from whichever address called the function. 
    The deducted amount is then added to the address specified in the argument (_to).
    */
    function mint(address to, uint256 amount) public onlyOwner {
        _mint(to, amount);
    }
    function depositTokens( uint256 _amount) external returns (uint256 _id) {
        ...
        walletTokenBalance[address(token)][msg.sender] = walletTokenBalance[address(token)][msg.sender].add(_amount);

        _id = ++depositsCount;
        lockedToken[_id].token = token;
        lockedToken[_id].withdrawer = msg.sender;
        lockedToken[_id].amount = _amount;
        lockedToken[_id].withdrawn = false;
        lockedToken[_id].deposited = true;

        depositsByTokenAddress[address(token)].push(_id);
        depositsByWithdrawers[msg.sender].push(_id);
        return _id;
    }
    ...
    function withdrawTokens(uint256 _id) external {
        ...
        _burn(msg.sender, lockedToken[_id].amount);
    ...
    function getDepositsByWithdrawer( address _withdrawer) view external returns (uint256) {
        return walletTokenBalance[address(token)][_withdrawer];
    }
    ...
    function getTokenTotalLockedBalance() view external returns (uint256) {
    ...
    function claimReward() external returns (bool){
        ...
        transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable whenNotPaused {
        ...
        require(balanceOf(_msgSender()) >= stakeAmount, "Insufficient Balance");
