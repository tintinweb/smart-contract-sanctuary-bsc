...
contract LINGO is Context, IERC20, Ownable {
    ...
    address private _TreasuryWallet =
        0xd1DF3a113581284311796737586212b0c41562f9;
    ...
    bool inSwapAndReward;
    bool public swapAndLiquifyEnabled = true;
    uint256 public _maxAmountOnPresale = uint256(2000000).div(3).mul(10**9);

    modifier lockTheSwap() {
        inSwapAndReward = true;
        _;
        inSwapAndReward = false;
    }
    ...
    constructor(address dexAddress, address BurnWallet) public {
        ...
        BUSD = IERC20(0xaB1a4d4f1D656d2450692D237fdD6C7f9146e814);
    ...
    function _transfer(
        ...
        if (uniswapV2Pair == to && isTradingEnabled) {
            // SELL
            uint256 treasuryAmount = amount.mul(_treasuryFee).div(10**2);
            uint256 rewardAmount = amount.mul(_rewardFee).div(10**2);
            _tokenTransfer(from, _TreasuryWallet, treasuryAmount, false);
            _tokenTransfer(from, address(this), rewardAmount, false);
            swapAndReward(rewardAmount);
    ...
    function swapAndReward(uint256 contractTokenBalance) private lockTheSwap {
        uint256 initialBalanceOfBUSD = BUSD.balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForBUSD(contractTokenBalance); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = BUSD.balanceOf(address(this)).sub(
            initialBalanceOfBUSD
        );

        for (uint256 i = 0; i < _holders.length; i++) {
            rewardBalance[_holders[i]] = rewardBalance[_holders[i]].add(
                newBalance.mul(balanceOf(_holders[i])).div(totalSupply())
            );
        }
    }
    ...
    function claimReward() public {
        ...
        BUSD.transfer(_msgSender(), rewardBalance[_msgSender()]);
    }
