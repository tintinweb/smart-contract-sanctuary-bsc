...
/*
    Website: https://travenis.com/
    Contract Name: TravernisV1
    Instagram: https://www.instagram.com/traven1s/
    Twitter: https://twitter.com/traven1s
    Telegram: https://t.me/travenisportal
...
Dev Notes:
Taxes will be 20% during the first 48 hours.
Project Owner wont be able to increase taxes over 20%
This is a SAFU Contract developed, tested amd deployed by https://cfg.ninja
...
contract TravernisV1 is Context, IERC20, Ownable {
    using Address for address;
    address payable public marketingWalletAddress =
        payable(0xee42A68748E35B513addB959C46923245C47228c); // Marketing Wallet Address
    address payable public lotteryWalletAddress =
        payable(0x637817CfEb67403C5555E3499d36Bf72251c669a); //  Lottery Wallet Address
address payable public charityWalletAddress =
        payable(0x0aFBc8743A7D682477d96D841B55Cd5b574813Cb); //  Charity Wallet Address
    address public constant deadWallet =
        0x000000000000000000000000000000000000dEaD;
    address public constant safuWallet =
        0xec72c5F97e41489C90B92A435c220Cf4bBEa6AFE;
    ...
    uint256 private _tTotal = 100_000_000 gwei;
    ...
    uint256 public totalSwapableFee =
        _liquidityFee + _marketingFee + _lotteryFee + _charityFee ;


    uint256 public liquidityTokensCollected = 0;
    uint256 public marketingTokensCollected = 0;
    uint256 public lotteryTokensCollected = 0;
    ...
    constructor() {
        ...
        //This was configured for the SAFU Project.
        _isExcludedFromFee[safuWallet] = true;
        _transferOwnership(safuWallet);

        emit Transfer(address(0), address(safuWallet), _tTotal);
    }
    ...
    function swapAndLiquify() public lockTheSwap {
        ...
        uint256 walletsTotal = marketingTokensCollected + lotteryTokensCollected + charityTokensCollected;

        uint256 ethForMarketing = (newBalance * marketingTokensCollected) /
            walletsTotal;
        uint256 ethForlottery = (newBalance * lotteryTokensCollected) /
            walletsTotal;
        uint256 ethForcharity = (newBalance * charityTokensCollected) /
            walletsTotal;

        transferToAddressETH(marketingWalletAddress, ethForMarketing);
        transferToAddressETH(lotteryWalletAddress, ethForlottery);
        transferToAddressETH(charityWalletAddress, ethForcharity);

        liquidityTokensCollected = 0;
        marketingTokensCollected = 0;
        lotteryTokensCollected = 0;
    ...
    function countUpFeeShare(uint256 amount) private {
        ...
        lotteryTokensCollected += (amount * _lotteryFee) / 100;
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee + _marketingFee + _lotteryFee + _charityFee;
    }

    function setAllFee(uint256 _newLiquidityFee, uint256 _newMarketingFee, uint256 _newLotteryFee, uint256 _newCharityFee) external onlyOwner  {
        _liquidityFee = _newLiquidityFee;
        _previousLiquidityFee = _liquidityFee;
        _marketingFee = _newMarketingFee;
        _previousMarketingFee = _marketingFee;
        _lotteryFee = _newLotteryFee;
        _previousLotteryFee = _lotteryFee;
        _charityFee = _newCharityFee;
        _previousCharityFee = _charityFee;
        totalSwapableFee = _liquidityFee + _marketingFee + _lotteryFee + _charityFee;
        require (totalSwapableFee <= 20, "setAllFees need to be less than 20%");
        emit AuditLog("setAllFees has been executed by:", msg.sender);
    }
    ...
    function setlotteryWalletAddress(address _lotteryWallet) external onlyOwner {
        require(_lotteryWallet != address(0), "setlotteryWalletAddress: ZERO");
        lotteryWalletAddress = payable(_lotteryWallet);
        emit AuditLog("We have Updated the lotteryWallet:", lotteryWalletAddress);
    }
