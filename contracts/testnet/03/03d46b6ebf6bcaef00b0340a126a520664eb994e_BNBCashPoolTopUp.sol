...
abstract contract Blacklist is Ownable {
    ...
    function addBlacklist(address account) public onlyOwner {
        require(!isBlacklisted(account), "Already in blacklist");
    ...
    function removeBlacklist(address account) public onlyOwner {
        require(isBlacklisted(account), "Already removed from blacklist");
        blacklist[account] = false;
    }

    function isBlacklisted(address account) public view returns (bool blacklisted) {
...
contract BNBCashPoolTopUp is Blacklist {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    IBEP20 public BCP;
    mapping(address => uint256) deposited;
    mapping(address => bool) signerRole;
    ...
    modifier onlySinger() {
        require(isSigner(_msgSender()), "only signer");
        _;
    }

    constructor(IBEP20 bcp) {
        BCP = bcp;
        signerRole[_msgSender()] = true;
    }
    ...
    function addSigner(address account) external onlyOwner {
        require(!isSigner(account), "addSigner: role already enabled");
        signerRole[account] = true;
    }

    function removeSigner(address account) external onlyOwner {
        require(isSigner(account), "addSigner: role already disabled");
        signerRole[account] = false;
    }

    function deposit(uint256 amount) public onlyNonBlacklist {
        require(address(BCP) != address(0), "deposit: BCP address is zero");
        require(amount != 0, "deposit: amount is zero");
        address caller = _msgSender();
        BCP.safeTransferFrom(caller, address(this), amount);
    ...
    function inCaseTokensGetStuck(address _tokenAddress, uint256 _tokenAmount) public onlySinger {
    ...
    function inCaseBNBGetStuck(uint256 _tokenAmount) public onlySinger {
        require(address(this).balance >= _tokenAmount, "inCaseBNBGetStuck: insufficient to recover");
        payable(msg.sender).transfer(_tokenAmount);
    }

    function user(address account) public view returns (uint totalDeposit) {
