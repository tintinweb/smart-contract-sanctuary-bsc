...
contract LiquidityGeneratorToken is Context, IERC20, Ownable {
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner {
        require(taxFee >= 0 && taxFee <= 10, "taxFee out of range"); //change the max to 10%
        _taxFee = taxFee;
    }

    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(
            liquidityFee >= 0 && liquidityFee <= 10, //change the max to 10%
    ...
    // uint256 public minMxTxPercentage = 50;
    // https://ethereum.stackexchange.com/a/101582
    // Limits the percentage of how many tokens a person can purchase per transaction.
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        require(
            maxTxPercent >= minMxTxPercentage && maxTxPercent <= 100,
            "maxTxPercent out of range"
        );
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(10 ** 2); // == _tTotal*(50/100) == _tTotal(0.5%)
    }
