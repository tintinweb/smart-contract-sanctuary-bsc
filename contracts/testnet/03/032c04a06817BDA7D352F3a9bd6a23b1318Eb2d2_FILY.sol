...
// 这是 0.8.7版本的USDT测试用合约，部署函数 await usdt.new(accounts[0], accounts[0], "USDT", "USDT", 6);
...
contract FILY is ERC20 {
    ...
    mapping(address => bool) BlackList; // 当LP减少，就封杀收益和转账
    ...
    mapping(address => uint256) public UserLastStakeWithDrawTimestamp; // 玩家上次提质押收益时间
    ...
    uint256 public AccFarmReward; // 每人分红
    ...
    function updateTEAMAndReward(
        ...
        if (_upper != address(0) && !BlackList[_upper]) {
            if (UpDown) {
                TEAMStakeAmount[account] += stakeAmount;
            } else {
                TEAMStakeAmount[account] -= stakeAmount;
            }
            // 级差
            ...
            UserTEAMRewardNoPaid[_upper] +=
                (block.timestamp - UserLastTEAMWithDrawTimestamp[account]) *
    ...
    function stake(address _upper, uint256 amount) external EOA {
        ...
        updateTEAMAndReward(msg.sender, amount, true); // 刷新上级的团队质押业绩
    }
    ...
    // 增删 决策委员会
    // function setManager(address to) external OnlyOperator {
    //     Manager[to] = !Manager[to];
    // }
