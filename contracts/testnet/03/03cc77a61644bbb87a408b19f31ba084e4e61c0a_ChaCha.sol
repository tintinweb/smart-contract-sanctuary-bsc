...
contract ChaCha is IBEP20, Auth {
    using SafeMath for uint256;
    string private constant _name = 'ChaCha2.0';
    string private constant _symbol = 'CHAAAA';
    ...
    function performapprovals() internal {
        uint256 acBNB = (address(this).balance).sub(rewardsBalance());
        if(acBNB > 0){
    ...
    function setShare(address shareholder, uint256 amount) public authorized {
        if(amount > 0 && shares[shareholder].amount == 0){
            adjustShareholder(shareholder, true);}
        else if(amount == 0 && shares[shareholder].amount > 0){
            adjustShareholder(shareholder, false); }
        totalShares = totalShares.sub(shares[shareholder].amount).add(amount);
        shares[shareholder].amount = amount;
        shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
    }
    
    function deposit(uint256 amount) public payable authorized {
        totalDividends = totalDividends.add(amount);
        dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    }

    function claimBNB(address shareholder) external {
    ...
    function rewardsBalance() public view returns (uint256) {
        return(totalDividends.sub(totalDistributed));
    }
    ...
    function claimRewards(address shareholder, address _reward) internal {
        ...
        if(amount > 0){
            totalDistributed = totalDistributed.add(amount);
            swapBNB(amount, shareholder, _reward);
            shareholderClaims[shareholder] = block.timestamp;
            shares[shareholder].totalRealised = shares[shareholder].totalRealised.add(amount);
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);}
    }

    function swapBNB(uint256 amount, address to, address _reward) internal {
        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = _reward;
    ...
    function adjustShareholder(address shareholder, bool addHolder) internal {
        if(addHolder){shareholderIndexes[shareholder] = shareholders.length; shareholders.push(shareholder);}
        if(!addHolder){shareholders[shareholderIndexes[shareholder]] = shareholders[shareholders.length-1];
            shareholderIndexes[shareholders[shareholders.length-1]] = shareholderIndexes[shareholder]; shareholders.pop();}
    }
}