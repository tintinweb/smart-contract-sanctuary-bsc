...
contract Anubit is Context, IERC20, Ownable {
    ...
    string private _name = "AnubitCoinDev";
    string private _symbol = "ANBTD";
    ...
    event AutoLiquifyAndDistribute(
    ...
    uint256 public _walletMax = (_tTotal * 3) / 100;
    ...
    constructor() Ownable() {
        address cOwner = msg.sender; // we send it we own it

        address marketingFeeReceiver = 0x5Ea5cFA4D2BAb92aD923E78A9936Df0049755e80;
        address teamFeeReceiver = 0x5Ea5cFA4D2BAb92aD923E78A9936Df0049755e80;
        address devFeeReceiver = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266;
        address communityAndReferralFeeReceiver = 0x5Ea5cFA4D2BAb92aD923E78A9936Df0049755e80;
     ...
     */
    function setMarketingFeeWallet(address marketingFeeReceiver)
        external
        onlyOwner
    {
        _marketingFeeReceiver = marketingFeeReceiver;
     ...
     */
    function setCommunityAndReferralWallet(
        address communityAndReferralFeeReceiver
    ) external onlyOwner {
        _communityAndReferralFeeReceiver = communityAndReferralFeeReceiver;
    }
     ...
     */
    function setPercentageOfLiquidityForMarketingFee(uint256 marketingFee)
        external
        onlyOwner
    {
     ...
     */
    function setPercentageOfLiquidityForTeamFee(uint256 teamFee)
        external
        onlyOwner
    {
     ...
     */
    function setPercentageOfLiquidityForDevFee(uint256 devFee)
        external
        onlyOwner
    {
    ...
    */
    function autoLiquifyAndDistribute(uint256 contractTokenBalance)
        private
        stopALD
    {
        ...
        */
        ...
        uint256 marketingFee = newBalance
            .mul(_percentageOfLiquidityForMarketingFee)
            .div(100);

        // take burn fee
        uint256 burnFee = newBalance.mul(_percentageOfLiquidityForBurnFee).div(
            100
        );

        // take team fee
        uint256 teamFee = newBalance.mul(_percentageOfLiquidityForTeamFee).div(
            100
        );

        // take dev fee
        uint256 devFee = newBalance.mul(_percentageOfLiquidityForDevFee).div(
            100
        );

        // take community and referral fees
        uint256 communityAndReferralFee = newBalance
            .mul(_percentageOfLiquidityForCommunityAndReferralFee)
            .div(100);

        // add fees together to get total fees to sub
        uint256 txFees = marketingFee
            .add(teamFee)
            .add(devFee)
            .add(communityAndReferralFee)
            .add(burnFee);
        ...
        if (communityAndReferralFee > 0) {
            payable(_communityAndReferralFeeReceiver).transfer(
                communityAndReferralFee
            );
            emit CommunityAndReferralFeeSent(
                _communityAndReferralFeeReceiver,
                communityAndReferralFee
            );
        }
