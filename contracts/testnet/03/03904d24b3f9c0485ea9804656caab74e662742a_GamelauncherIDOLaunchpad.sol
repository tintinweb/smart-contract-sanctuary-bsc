...
contract GamelauncherIDOLaunchpad {
    ...
    //Get Estimated BNB For Token
    function getEstimatedBNBNeedForToken(string memory _projectId,string memory _phaseId,uint256 _numberOfTokens) public view returns(uint256 _tokenPrice){  
        if (_numberOfTokens == 0) {
            return 0;
        } else {
            uint256 _tokenprice=getProjectPhaseDetails[_phaseId].tokenPrice;
            uint256 safeDivison=uint256(10) ** getProjectDetails[_projectId].decimals;
            return SafeMath.safeMultiply(_numberOfTokens, _tokenprice)/safeDivison;
        }
    }

    function getEstimatedTokenNeedForBNB(string memory _projectId,string memory _phaseId,uint256 _tokenprice) public view returns(uint256 _numberOfTokens){ 
        if (_tokenprice == 0) {
            return 0;
        } else {
            _tokenprice=getProjectPhaseDetails[_phaseId].tokenPrice;
            uint256 safeDivison=uint256(10) ** getProjectDetails[_projectId].decimals;
            return SafeMath.safeMultiply(_tokenprice, _numberOfTokens)/safeDivison;
        }
    }
    ...
    function ParticipateIDO(string memory _phaseId,string memory _projectId,string memory _orderId,uint256 _numberOfTokens) public payable returns (bool) {
       uint256 _tokenprice=getProjectPhaseDetails[_phaseId].tokenPrice;
       uint256 safeDivison=uint256(10) ** getProjectDetails[_projectId].decimals;
       require(msg.value == (SafeMath.safeMultiply(_numberOfTokens, _tokenprice)/safeDivison),"Invalid No of Token ! Mismatch With Price !");
