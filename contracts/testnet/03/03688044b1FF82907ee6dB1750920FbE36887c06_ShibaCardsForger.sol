...
import "./abstracts/ShibaCardsMintable.sol";
import "./interfaces/IBank.sol";
import "./interfaces/IMinter.sol";

contract ShibaCardsForger is
  ShibaCardsPayable,
  ShibaCardsMintable,
  ShibaCardsDealable
{
  ...
  event CardCreated(uint256[] tokenIds);
  event CardForged(address account, uint256 tokenId);

  /**
   * @dev stores the components by tokenId required by a forgeable card
   */
  mapping(uint256 => uint256[]) private componentsOf;

  function getComponentsOf(uint256 tokenId) public view returns (uint256[] memory) {
    return componentsOf[tokenId];
  }

  /**
   * @dev Combines specific cards into a new one.
   */
  function forge(uint256 tokenId) public payable {
    uint256[] memory components = componentsOf[tokenId];
    require(components.length > 0, "Not forgeable");

    bank.makePayment(_msgSender(), getFees());

    for (uint256 i = 0; i < components.length; i++) {
      minter.burn(_msgSender(), components[i], 1);
    }
    ...
    minter.mintSpecific(_msgSender(), ids, amounts);

    emit CardForged(_msgSender(), tokenId);
  }

  /**
   * @dev Creates a new forgeable token.
   */
  function create(
    uint256[][3] memory allComponents
  ) public onlyWhitelistedOrAdmin {
      uint256[] memory newIds = dealer.create(true);
       
      for (uint256 index = 0; index < newIds.length; index++) {
        uint256[] memory components = allComponents[index];
        uint256 newId = newIds[index];
        cards.push(newId);
        componentsOf[newId] = components;
      }

      emit CardCreated(newIds);
  }

  /**
   * @dev Get forgeable cards as array.
   */
...
interface IDealer {
  ...
  function getRandomId(uint256 randomNumber) external returns (uint256 id);
  function getRandomIds(uint256 randomNumber, uint256 amount) external returns (uint256[] memory);
  ...
  function create(bool platinum) external returns (uint256[] memory);
}
...
abstract contract ShibaCardsMintable is ShibaCardsAccessible {
  IMinter minter;
