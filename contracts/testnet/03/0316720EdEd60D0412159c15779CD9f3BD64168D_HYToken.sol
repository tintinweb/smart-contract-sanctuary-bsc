 ...
 contract HYToken is IERC20, Ownable {
    ...
    function _tokenTransfer(
        ...
        if (_swapPairList[sender]) {//Buy
            uint256 NFTAmount = tAmount * 10 / 100;
        ...
        //1%销毁
            ...
            //3%营销运营
            uint256 YXAmount = tAmount * 3/ 100;
            if (YXAmount > 0) {
                _takeTransfer(sender, _YXAddress, YXAmount);
            }
            //2:3 兑换为USDT给子币赋值和直推奖励
            uint256 sellSwapAmount = tAmount * 5/ 100;
            _takeTransfer(sender, address(this), sellSwapAmount);
            //到账
            _takeTransfer(sender, recipient,tAmount*90/100);

            if (!inSwap) {
                swapTokenForFund(sender);
            }
        } else {//Transfer
    ...
    function swapTokenForFund(address sender) private lockTheSwap {
        ...
        if (tokenBalance >= 10) {
            ...
            uint256 ZBFZAmount = usdtBalance * 60/100;
            ...
            if(invitor != address(0)){
                ...
                USDT.transferFrom(TokenDistributor,_YXAddress,RewardAmount);
            }
