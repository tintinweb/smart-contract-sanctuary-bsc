...
contract RewardManager is Ownable, ReentrancyGuard {
    ...
    // TESTING
    uint256 public constant oneWeek = 1800; // 30 mins
    ...
    function setLockDuration(uint256 _durationInSeconds) public onlyOwner {
    ...
    function unlockedBalance(address user) external view returns (uint256 amount) {
        ...
        amount = bal.unlocked;
        Lock[13] storage loc = locks[user];
        for (uint256 i; i < 13; i++) {
            if (loc[i].unlockTime <= block.timestamp) {
                amount = amount + loc[i].amount;
            }
    ...
    function withdrawableBalance(address user) public view returns (uint256 totalAmount, uint256 penalizedAmount) {
        Balance storage bal = balances[user];
