...
interface I_Windmill_Trade_Abstract is I_Math, IAccessControl, I_With_DAORole, I_With_UpdaterRole, I_With_TradeManagerRole,
	...
	event Action(uint256 id, uint256 timestamp, uint256 data0, uint256 data1, address dataAddr);
...
contract Windmill_Royalty is Base, Math, Adressable, AccessControl, With_DAORole,
                             With_UpdaterRole, ERC20Capped, With_PWRToken, With_FundContract, I_Windmill_Royalty{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    Fraction public percentPWRRoyaltyMint;

    ////
    ////
    ////
    //////////////// Private variables ////////////////
    mapping(address=>uint256) internal addressToholderId;

    ////
    ////
    ////
    //////////////// Constructor & Modifiers ////////////////
    constructor() ERC20("Windmill_Royalties", "ROY") ERC20Capped(100){
        _initholders();

        percentPWRRoyaltyMint.numerator = 100;
        percentPWRRoyaltyMint.denominator = 10000;
    }
    ...
    function setRoyaltyRatio(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        ...
        percentPWRRoyaltyMint.numerator = numerator;
        percentPWRRoyaltyMint.denominator = denominator;
    }

    function processRoyalties() external onlyRole(UPDATER_ROLE){
        uint256 ROYTotal = totalSupply();
        uint256 PWRTotal = fund.getTotalPWR();

        for (uint256 i=1; i<holders.length; i++){
            address addr = holders[i];
            uint256 ROYBalance = balanceOf(addr);
            uint256 PWRToMint = (PWRTotal * percentPWRRoyaltyMint.numerator * ROYBalance) / (percentPWRRoyaltyMint.denominator * ROYTotal);
    ...
    ////
    //////////////// Private functions ////////////////
    function _initholders() internal{
        holders.push(address(0x0));
    }

    function _isHolder(address addr) internal view returns (bool){
        return addressToholderId[addr] != 0;
    }

    function _addHolder(address addr) internal{
        addressToholderId[addr] = holders.length;
        holders.push(addr);
    }

    function _deleteHolder(address addr) internal{
        uint256 i = addressToholderId[addr];
        uint256 lastI = holders.length-1;
        if (i != lastI){
            address lastAddr = holders[lastI];
            holders[i] = lastAddr;
            addressToholderId[lastAddr] = i;
        }
        holders.pop();
    }
