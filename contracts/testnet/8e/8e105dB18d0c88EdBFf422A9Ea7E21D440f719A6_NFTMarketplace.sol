...
contract NFTMarketplace is Ownable {
    ...
    function offerAll(OfferData[] calldata a)
    public 
    payable
    returns(bool)
    {   
        ...
        {
            require(checkLock(a[i]._token, a[i].tokenId), "token is locked");
            MockNFT(a[i]._token).transferFrom(msg.sender, address(this), a[i].tokenId);
            userOffers[msg.sender].push(OfferData({tokenId: a[i].tokenId, minTime: a[i].minTime, maxTime: a[i].maxTime, startDiscountTime: a[i].startDiscountTime, price: a[i].price, discountPrice: a[i].discountPrice, endTime: 0, _token: a[i]._token}));
        }
        return true;
    }
