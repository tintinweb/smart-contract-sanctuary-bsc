...
contract Redistribution is Ownable {
    ...
    uint256 private nftAdminShare = 20;
    ...
    function setNFTShare(uint256 _holderShare, uint256 _adminShare, uint256 _charityShare) external onlyOwner {
        require((_holderShare + _adminShare + _charityShare) <= 100, "INVALID");
        nftHolderShare = _holderShare;
        nftAdminShare = _adminShare;
    ...
    function deposit (uint256 amount) external returns (bool) {
        ...
        uint256 l = admin.length;
        uint256 m = charity.length;
        for (uint256 i = 0; i <= n-1; i++) {
            // 60% to 20 NFT holders
            RWX.transfer(holder[i], (amount * nftHolderShare)/(100 * n));
        }
        // transfer to admin wallets

        for (uint256 k=0; k<= l-1; k++) {
            // 20% to charity holders
            RWX.transfer(charity[k], (amount * nftAdminShare)/(100 * k));
        }
        // transfer to charity
