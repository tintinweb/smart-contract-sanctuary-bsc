    ...
    IZuna
{
    ...
    function bulkMint(
        ...
        Shared.Offer[] calldata offers
    ) external {
        require(
            tokenIds.length == vouchers.length &&
                tokenIds.length == offers.length,
            "Invalid Parameters"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            NFTVoucher memory voucher = vouchers[i];
        ...
        IMarket(marketAddress).bulkPriceSet(offers);
    }
