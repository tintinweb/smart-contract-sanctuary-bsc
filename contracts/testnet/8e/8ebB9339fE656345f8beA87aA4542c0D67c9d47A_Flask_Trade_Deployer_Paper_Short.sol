...
abstract contract Flask_Trade_Deployer_Abstract is Base, Initializable, Adressable, AccessControl, With_DAORole, With_UpdaterRole,
                                                      With_TradeManagerRole, With_BUSDToken, I_Flask_Trade_Deployer_Abstract{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
	...
	function deployNewTrade(address owner, address manager, address fund, address updater, address withdrawer, address competition) external onlyRole(TRADE_MANAGER_ROLE) onlyInitialized(0) returns (I_Flask_Trade_Abstract){
        I_Flask_Trade_Abstract trade =  I_Flask_Trade_Abstract(_deployNewTrade());
    ...
    function _initTrade(I_Flask_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal virtual{
...
abstract contract Flask_Trade_Deployer_Swap is Base, Flask_Trade_Deployer_Abstract, With_PancakeRouter, I_Flask_Trade_Deployer_Swap{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    uint256 public constant nbProposals = 2;
	...
	function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public virtual onlyRole(DAO_ROLE){
        if (id == 0){
            ...
            authorizedTokens[paramsUint256].disabled = true;
            delete routes[authorizedTokens[paramsUint256].routeId];
        }
    }
	...
	function getRoute(uint256 routeId) external view returns (address[] memory){
        ...
        require(route.length > 0, "Flask_Trade_Deployer_Swap: Route id not found.");
    ...
    ////
    //////////////// Private functions ////////////////
    function _initTrade(I_Flask_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal virtual override{
        I_Flask_Trade_Swap tradeDerived = I_Flask_Trade_Swap(address(trade));
    ...
    function _unpackRoute(uint256 data, address token) internal view returns (address[] memory){
        ...
        for (uint i = 1; i<=size; i++){
            ...
            require(tokenId < authorizedTokens.length, "Flask_Trade_Deployer_Swap: Route token id not found.");
...
abstract contract Flask_Trade_Deployer_Borrow is Base, Math, Flask_Trade_Deployer_Swap, I_Flask_Trade_Deployer_Borrow{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    ////
    //////////////// Private variables ////////////////
    AuthorizedSetup[] internal authorizedSetup;
    ...
    //////////////// Public functions ////////////////
	function getAuthorizedSetupWithCheck(uint256 setupId) external view returns(AuthorizedSetup memory){
		require(setupId < authorizedSetup.length, "Flask_Trade_Deployer_Borrow: setup not found.");
		
		AuthorizedSetup memory setup = authorizedSetup[setupId];
		
		require(!authorizedSetup[setupId].disabled, "Flask_Trade_Deployer_Borrow: setup disabled.");
    ...
    function getNbAuthorizenSetup() external view returns (uint256){
        return authorizedSetup.length;
    }
	...
	function setSecurityFactor(uint256 numerator, uint256 denominator) external onlyRole(DAO_ROLE){
        require(denominator>0, "Flask_Trade_Deployer_Borrow: Denominator cannot be null");
     ...
     *
     */
	function checkProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public view virtual override(I_Flask_Trade_Deployer_Abstract, Flask_Trade_Deployer_Swap) onlyRole(DAO_ROLE) returns (bool){
	...
	function applyProposal(uint256 id, uint256 paramsUint256, address paramsAddress) public override(I_Flask_Trade_Deployer_Abstract, Flask_Trade_Deployer_Swap) onlyRole(DAO_ROLE){
		...
		if (id == 10){
			...
			authorizedSetup[paramsUint256].disabled = true;
    ...
    ////
    //////////////// Private functions ////////////////
    function _initTrade(I_Flask_Trade_Abstract trade, address owner, address manager, address fund, address updater, address withdrawer, address competition) internal override(Flask_Trade_Deployer_Swap){
        I_Flask_Trade_Borrow tradeDerived = I_Flask_Trade_Borrow(address(trade));
...
interface I_Flask_Trade_Deployer_Paper_Short is I_Flask_Trade_Deployer_Borrow{
}

contract Flask_Trade_Deployer_Paper_Short is Base, Flask_Trade_Deployer_Borrow, I_Flask_Trade_Deployer_Paper_Short{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
