...
// File: contracts\Capverse\CapverseMarket.sol

// SPDX-License-Identifier: MIT
pragma solidity ^0.6.0;
contract CapverseMarket is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;

    string public name = "Capverse Market";
    ...
    // receive order fee address
    address public orderFeeTo;

    address[] public nftList;

    constructor() public {
        adminList.push(msg.sender);
        orderFeeTo = _msgSender();
        BASETOKENADDRESS = address(1);
    }

    // add token allow
    function addAllowedPayTokens(address token) public nonReentrant {
        require(onlyAdmin(msg.sender), "Only administrators can operate");
        require(token != address(0), "token is zero address");
        require(payTokenIsAllowed(token) == false, "token is allowed");
    ...
    // get pay token list
    function getAllowedPayTokens() public view returns (address[] memory) {
        return allowedPayTokens;
    }

    // set BASE token address
    function setBaseTokenAddress(address token) public onlyOwner nonReentrant {
        require(token != address(0), "token is zero address");
        BASETOKENADDRESS = token;
    }

    // set fee to address
    function setOrderFeeToAddress(address _orderFeeTo) public onlyOwner nonReentrant {
        require(_orderFeeTo != address(0), "orderFeeTo is zero address");
        orderFeeTo = _orderFeeTo;
    }

    // set pay token fee
    function setPayTokenFee(address token, uint256 amount) public nonReentrant {
        require(onlyAdmin(msg.sender), "Only administrators can operate");
        require(token != address(0), "token is zero address");
        require(payTokenIsAllowed(token) == true, "token is not allowed");
        require(amount < 10000, "amount overflow");
        // amount = 100; //this mean 0.01 (1%)
    ...
    // (nftToken > tokenID > orderID)
    mapping(address => mapping(uint256 => uint256)) nftTokenOfOrder;

    // order
    event SoldOrder(address nftToken, uint256 tokenID, address payToken, uint256 payAmount, uint256 newOrderId);
    event SoldOutOrder(address nftToken, uint256 tokenID, uint256 orderID);
    event PurchaseOrder(address nftToken, uint256 tokenID, uint256 orderID, address payToken, uint256 payAmount, address ownerAddress);
    ...
    // get orderID by NFTToken
    function getOrderIDByNFTToken(address nftToken, uint256 tokenID) public view returns (uint256) {
        return nftTokenOfOrder[nftToken][tokenID];
    }
    ...
    function soldOrder(address nftToken, uint256 tokenID, address payToken, uint256 payAmount) public nonReentrant {
        ...
        require(onlyNFT(nftToken), "nft address is not allowed");
        ...
        require(orderIdIsExisted(newOrderId) == false, "orderId is existed");

        IERC721(nftToken).transferFrom(msg.sender, address(this), tokenID);

        orderList[newOrderId] = OrderInfo(newOrderId, msg.sender, nftToken, tokenID, payToken, payAmount, 1);

        ordersIdList.push(newOrderId);
        orderOfOwner[newOrderId] = msg.sender;
        nftTokenOfOrder[nftToken][tokenID] = newOrderId;

        emit SoldOrder(nftToken, tokenID, payToken, payAmount, newOrderId);
    }

    // sold out order
    function soldOutOrder(address nftToken, uint256 tokenID) public nonReentrant {
        require(nftToken != address(0), "nft address is zero address");
        require(onlyNFT(nftToken), "nft address is not allowed");

        uint256 orderID = getOrderIDByNFTToken(nftToken, tokenID);
        require(orderID > 0, "order id is error");
        ...
        nftTokenOfOrder[nftToken][tokenID] = 0;
        ...
        emit SoldOutOrder(nftToken, tokenID, orderID);
    }
    ...
    // purchase order
    function purchaseOrder(address nftToken, uint256 tokenID) public payable nonReentrant {
        require(nftToken != address(0), "nft address is zero address");
        require(onlyNFT(nftToken), "nft address is not allowed");
        ...
        address _payToken = orderList[orderID].PayToken;
        ...
        if (_payToken == BASETOKENADDRESS) {
            ...
            if ((tokenFee > 0) && (rate > 0)) {
                address payable _orderFeeTo = address(uint160(orderFeeTo));
                _orderFeeTo.transfer(tokenFee);
            }
            ...
            if ((tokenFee > 0) && (rate > 0)) {
                IERC20(_payToken).safeTransferFrom(msg.sender, orderFeeTo, tokenFee);
            }
        ...
        emit PurchaseOrder(nftToken, tokenID, orderID, _payToken, _payAmount, _OrderOwner);
    }


    function setNFTList(address[] memory _nftList) public nonReentrant {
        require(onlyAdmin(msg.sender), "Only administrators can operate");
        require(_nftList.length > 0, "_nftList is empty");

        for (uint256 nIndex = 0; nIndex < _nftList.length; nIndex++) {
            require(_nftList[nIndex] != address(0), "NFT address is empty");
        }
    ...
    function onlyNFT(address token) internal view returns (bool) {
        for (uint256 nIndex = 0; nIndex < nftList.length; nIndex++) {
            if (nftList[nIndex] == token) {
                return true;
            }
        }
        return false;
    ...
    function onlyAdmin(address _adminAddress) internal view returns (bool) {
