...
contract Utopia is  Ownable {
    ...
    constructor(address utopiaToken_, address utopiaNFT_, address admin_) {
    ...
    function setRecommender(address recommender) public {
        ...
        require(recommender == owner() || IDCARD[recommender] != 0, "Recommender address is invalid!");
    ...
    function updateToken(bool flag) public onlyOwner{
        if(flag == true){
            require(utopiaToken.pair() != address(0), "Unable to set(pair is null)!");
        }
    ...
    function join() public {
        ...
        uint256 amount;
        if(utopiaToken_usdt){
            uint join_price_ut = getUTForUsdt(join_price);
            require(utopiaToken.balanceOf(msg.sender) >= join_price_ut, "Your balance is insufficient balance to mint NFT!");
            utopiaToken.transferFrom(msg.sender, address(this), join_price_ut);//入金
            amount = (join_price_ut*10)/100;//直推奖励10%
            utopiaToken.transfer(recommender_address,amount);
            totalBonus += join_price_ut;//增加奖金
        }else{
    ...
    //结算奖励
    function statisticsReward() public onlyAdmin {
        require(totalBonus > 0, "not enough totalBonus!");
        uint256 totalNftCount = utopiaNFT.totalSupply();
        require(totalNftCount > 0, "not enough user!");
        for(uint256 i=0;i<8;i++){
            if(all_grade_num[i] > 0){
                rewardLadderAmount[i] = totalBonus*rewardLadder[i]/100/all_grade_num[i];
            }
        }
    ...
    function queryReward(address account) public view returns(uint256){
        require(referrer[account] != address(0), "You are not qualified!");
        require(numberOfTimes > numberOfIndividuals[account], "Unable to receive rewards!");
        uint256 amount = 0;
        if(statisticsRewardTime <= createTime[account]){
            return amount;
        }
    ...
    function getUTForUsdt(uint256 usdtAmount) public view returns(uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(utopiaToken.pair());
        (uint256 amountUTInPair, uint256 amountUsdtInPair) = getLiquidityPairAmount(pair);
        return (amountUTInPair * usdtAmount) / amountUsdtInPair;
    }

    function getUsdtAmountForUT(uint256 UTAmount) public view returns(uint256) {
        IUniswapV2Pair pair = IUniswapV2Pair(utopiaToken.pair());
        (uint256 amountUTInPair, uint256 amountUsdtInPair) = getLiquidityPairAmount(pair);
        return (amountUsdtInPair * UTAmount) / amountUTInPair;
    }
    ...
    function getLiquidityPairAmount(IUniswapV2Pair pair) public view returns(uint256 amountTk, uint256 amountUsdt)  {
        ...
        if (pair.token0() == address(utopiaToken)) {
            amountTk = token0;
            amountUsdt = token1;
        }
        else {
    ...
    modifier onlyAdmin() {
        require(admin != address(0), "The utopia contract is not set!");
        require(msg.sender == admin || msg.sender == owner(), "This function can be called only from admin!");
        _;
    }
}