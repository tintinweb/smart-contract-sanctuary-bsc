...
contract SwapPool is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IWETH public wrappedToken;
    IABNBC public abnbcToken;
    ILP public lpToken;

    uint256 public nativeTokenAmount;
    uint256 public ABNBCTokenAmount;
//    uint256 public maxFlashLoanAmount;
//    uint256 public flashLoanFee; // In basis points
    ...
    FeeAmounts public flashLoanFeeCollected;
    ...
    function initialize(
        ...
        wrappedToken = IWETH(_wrappedToken);
        abnbcToken = IABNBC(_abnbcToken);
        lpToken = ILP(_lpToken);
        integratorLockEnabled = _integratorLockEnabled;
        providerLockEnabled = _providerLockEnabled;
    }

    function setMaxFlashLoan(uint128 amount) external onlyOwner {
    ...
    function _addLiquidity(
        ...
        uint256 ratio = abnbcToken.ratio();
        ...
        if (useEth) {
            wrappedToken.deposit{ value: amount0 }();
            uint256 diff = msg.value - amount0;
            if (diff != 0) {
                _sendValue(msg.sender, diff);
            }
        } else {
            wrappedToken.transferFrom(msg.sender, address(this), amount0);
        }
        abnbcToken.transferFrom(msg.sender, address(this), amount1);
        if (nativeTokenAmount == 0 && ABNBCTokenAmount == 0) {
            require(amount0 > 1e18, "cannot add first time less than 1 token");
            nativeTokenAmount = amount0;
            ABNBCTokenAmount = amount1;

            lpToken.mint(msg.sender, (2 * amount0) / 10**8);
        } else {
            uint256 allInNative = nativeTokenAmount + (ABNBCTokenAmount * 1e18) / ratio;
        ...
        emit LiquidityChange(msg.sender, amount0, amount1, nativeTokenAmount, ABNBCTokenAmount, true);
    }
    ...
    function _removeLiquidity(uint256 removedLp, bool useEth) internal virtual {
        ...
        uint256 amount1Removed = (removedLp * ABNBCTokenAmount) / totalSupply;

        nativeTokenAmount -= amount0Removed;
        ABNBCTokenAmount -= amount1Removed;

        if (useEth) {
            wrappedToken.withdraw(amount0Removed);
            _sendValue(msg.sender, amount0Removed);
        } else {
            wrappedToken.transfer(msg.sender, amount0Removed);
        }
        abnbcToken.transfer(msg.sender, amount1Removed);
    ...
    function swap(
        ...
        return _swap(wrappedToABNBC, amountIn, receiver, false);
    }
    ...
    function _swap(
        ...
        if (nativeToCeros) {
            if (useEth) {
                ...
                wrappedToken.transferFrom(msg.sender, address(this), amountIn);
            }
            if (!excludedFromFee[msg.sender]) {
                ...
                if (integratorLockEnabled) {
                    ...
                    if (integratorFeeAmt > 0) {
                        wrappedToken.transfer(msg.sender, integratorFeeAmt);
                    }
                }
                nativeTokenAmount +=
                amountIn +
            ...
            require(ABNBCTokenAmount >= amountOut, "Not enough liquidity");
            ABNBCTokenAmount -= amountOut;
            abnbcToken.transfer(receiver, amountOut);
            emit Swap(msg.sender, receiver, nativeToCeros, amountIn, amountOut);
        } else {
            abnbcToken.transferFrom(msg.sender, address(this), amountIn);
            if (!excludedFromFee[msg.sender]) {
                ...
                if (integratorLockEnabled) {
                    ...
                    if (integratorFeeAmt > 0) {
                        abnbcToken.transfer(msg.sender, integratorFeeAmt);
                    }
                }
                ABNBCTokenAmount +=
                amountIn +
            ...
            if (useEth) {
                ...
                wrappedToken.transfer(receiver, amountOut);
            }
            emit Swap(msg.sender, receiver, nativeToCeros, amountIn, amountOut);
        }
    }
    ...
    function getAmountOut(
        ...
        if (wrappedToABNBC) {
            ...
            amountOut = (amountIn * ratio) / 1e18;
            enoughLiquidity = ABNBCTokenAmount >= amountOut;
        } else {
            if (!isExcludedFromFee) {
                uint256 unstakeFeeAmt = (amountIn * unstakeFee) / FEE_MAX;
                amountIn -= unstakeFeeAmt;
    ...
    function _withdrawOwnerFee(
        ...
        if (amount0 > 0) {
            ...
            if (useEth) {
                wrappedToken.withdraw(amount0);
                _sendValue(msg.sender, amount0);
            } else {
                wrappedToken.transfer(msg.sender, amount0);
            }
        }
        if (amount1 > 0) {
            ownerFeeCollected.abnbcFee -= amount1;
            abnbcToken.transfer(msg.sender, amount1);
        }
    }
    ...
    function _withdrawManagerFee(address managerAddress, bool useNative) internal virtual {
        ...
        if (feeRewards.nativeFee > 0) {
            ...
            if (useNative) {
                wrappedToken.withdraw(feeRewards.nativeFee);
                _sendValue(managerAddress, feeRewards.nativeFee);
            } else {
                wrappedToken.transfer(managerAddress, feeRewards.nativeFee);
            }
        }
        if (feeRewards.abnbcFee > 0) {
            ...
            abnbcToken.transfer(managerAddress, feeRewards.abnbcFee);
        }
    }
    ...
    function triggerRebalanceAnkr() external virtual nonReentrant onlyManager {
        ...
        uint256 amountBInNative = (ABNBCTokenAmount * 1e18) / ratio;
        ...
        if (isStake) {
            ...
            bnbPool.stake{ value: amount }(false);
        } else {
            uint256 abnbcAmount = (amount * ratio) / 1e18;
            uint256 commission = bnbPool.unstakeCommission();
            ABNBCTokenAmount -= abnbcAmount;
            nativeTokenAmount -= commission;
            wrappedToken.withdraw(commission);
            bnbPool.unstake{ value: commission }(abnbcAmount, false);
        }
    }

    function approveToMaticPool() external virtual {
        abnbcToken.approve(address(bnbPool), type(uint256).max);
    }

    function skim() public virtual {
        uint256 contractBal = address(this).balance;
        uint256 contractWNativeBal = wrappedToken.balanceOf(address(this)) -
        ownerFeeCollected.nativeFee -
        (managerFeeCollected.nativeFee - _claimedManagerFees.nativeFee);
        uint256 contractABNBCBal = abnbcToken.balanceOf(address(this)) -
        ...
        if (contractABNBCBal > ABNBCTokenAmount) {
            ABNBCTokenAmount = contractABNBCBal;
        }

        if (contractBal > nativeTokenAmount) {
            wrappedToken.deposit{ value: contractBal }();
            nativeTokenAmount += contractBal;
        }

        // 0.5 BNB
        if (contractABNBCBal >= 5e17) {
            uint256 commission = bnbPool.unstakeCommission();
            bnbPool.unstake{ value: commission }(contractABNBCBal, false);
        }
    }
    ...
    function flashFee(address token, uint256 amount) public view returns (uint128) {
        require(uint128(amount) == amount, "SwapPool: uint128 overflow");
        
        require(token == address(abnbcToken) || token == address(wrappedToken), "SwapPool: token_unsupported");

        return flashLoanFixedFee + (uint128(amount) * flashLoanFee / 10000);
    }
    ...
    function flashLoan(
        ...
        require(flashLoanFee > 0 && flashLoanFixedFee > 0, "SwapPool: fees not set");
        require(token == address(abnbcToken) || token == address(wrappedToken), "SwapPool: token_unsupported");
        ...
        if (token == address(abnbcToken)) {
            flashLoanFeeCollected.abnbcFee += fee;
        }

        if (token == address(wrappedToken)) {
            flashLoanFeeCollected.nativeFee += fee;
        }
        ...
        IERC20(token).transferFrom(address(receiver), address(this), total);

        return true;
    }

    // TODO: unstake if aBNBc >= 0.5 for rebalancing - DONE
    // TODO: flash loan fee = fixed fee + percent - DONE
    // TODO: FeeType <- flash loan fee, flash loan fixed fee - DONE
