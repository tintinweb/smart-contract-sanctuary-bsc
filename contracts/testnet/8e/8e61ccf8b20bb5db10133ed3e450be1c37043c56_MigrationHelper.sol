...
contract MigrationHelper is Ownable {
    address public immutable MasterChefV1Addr;

    address public immutable MasterChefV1OriginOwner;

    uint256 public immutable MasterChefV2Pid;
    ...
    constructor(address _MasterChefV1Addr, uint256 _MasterChefV2Pid) public {
    ...
    function addPoolToMigrate(uint256[] memory pools) external onlyOwner {
        for (uint256 i; i < pools.length; i++) {
            ...
            (, uint256 allocPoint, , ) = IMasterChef(MasterChefV1Addr).poolInfo(
                pid
            );
    ...
    function transferOwnershipForMasterChefV1() external onlyOwner {
        IMasterChef(MasterChefV1Addr).transferOwnership(
            MasterChefV1OriginOwner
        );
    }
    ...
    function recover(uint256 _pid) external onlyOwner {
        ...
        if (prevAllocPoints[_pid] > 0) {
            IMasterChef(MasterChefV1Addr).set(
                _pid,
                prevAllocPoints[_pid],
                false
            );
        }
    }
    ...
    function kill() external onlyOwner {
        require(
            MasterChefV1OriginOwner == IMasterChef(MasterChefV1Addr).owner(),
