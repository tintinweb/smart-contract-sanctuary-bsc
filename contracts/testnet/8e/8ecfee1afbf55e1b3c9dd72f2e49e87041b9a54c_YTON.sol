...
contract YTON is Ownable{
	...
	uint256 constant public UP_MIN_AMOUNT = 0.001 ether;    //BNB最小质押数量
	...
	uint256 constant public MARKETING_FEE = 50;  //yingxiao
	uint256 constant public PERCENTS_DIVIDER = 1000;  //base lv
	uint256 constant public BASE_FEE = 100;  //收益分母
	uint256 constant public CONTRACT_BALANCE_STEP = 100000 * 10 ** 18; //U
	...
	function withdraw() public {
		...
		user.checkpoint = block.timestamp;
         ...
         //给用户转账实际到账数量
		usdt.transfer(msg.sender, totalAmount.mul(PERCENTS_DIVIDER.sub(MARKETING_FEE)).div(PERCENTS_DIVIDER)); // 95%
    ...
    function ext() public returns (bool){
		User storage user = users[msg.sender];
        //充值后30分钟内不能体现
        if(now.sub(user.deposits[user.deposits.length-1].start) < 5){
            return  false;
        }
