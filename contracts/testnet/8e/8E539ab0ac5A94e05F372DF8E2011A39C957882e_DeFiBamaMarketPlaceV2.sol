...
contract DeFiBamaMarketPlaceV2 is ReentrancyGuard, IERC721Receiver {
    ...
    function makeABid(uint256 id) nonReentrant payable public {
        ...
        uint256 numberOfHours = auctionListing[a._auctionItem.nft.adr][a._auctionItem.nft.id].auctionDurationBasedOnHours * 1 seconds;
        ...
        uint256 minimumBidAllowed = (auctionListing[a._auctionItem.nft.adr][a._auctionItem.nft.id].currentBidPrice) + ((auctionListing[a._auctionItem.nft.adr][a._auctionItem.nft.id].currentBidPrice * minimumBidIncreasePercentage) / 100);
    ...
    function checkAuctions() nonReentrant external payable {
        ...
        for (uint256 i = 0; i < currentId; i++) {
            ...
            if (m._isDeleted == false) {
                uint256 numberOfHours =  m._auctionItem.auctionDurationBasedOnHours * 1 seconds;
    ...
    function listNFTForSaleOnMarket(uint256 price, address addressOfNFT, uint256 tokenId,
     uint8 category, uint256 prefession) isOwner(addressOfNFT, tokenId) nonReentrant public payable returns(uint256) {
        ...
        return _buyNowtokenIds.current() - 1;
        //emit buyNowMarketItemCreated(tokenId, payable(msg.sender), payable(msg.sender), price, category);
    }

    // Call this function before lisiting on marketplace so marketplace is allowed to transfer nft
    function setApproval(address addressOfNFT, uint256 tokenId) isOwner(addressOfNFT, tokenId) nonReentrant public {
        IERC721 nftContract = IERC721(addressOfNFT);
    ...
    function listNFTForSaleOnAuction(uint256 price, address addressOfNFT, uint256 tokenId,
     uint8 category, uint auctionDurationInHours, uint256 profession) nonReentrant isOwner(addressOfNFT, tokenId) public payable returns(uint256) {
