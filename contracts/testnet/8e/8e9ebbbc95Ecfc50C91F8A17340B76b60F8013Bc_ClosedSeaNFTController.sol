...
interface IclosedSeaNft {
  ...
  function getOM(uint256 _tokenId) external view returns(address);
...
contract ClosedSeaNFTController is ERC721Holder, Ownable, Pausable {
    ...
    IclosedSeaNft public closedSeaNFT;
    ...
    function getRoyaltyInfo(uint _tokenId, uint _price) public view returns(address recipient, uint royaltyAmount){
    ...
    function buyToken(address _tokenAddr, uint256 _tokenId) public payable whenNotPaused notBanned(_tokenAddr, _tokenId) {
        ...
        if (_tokenAskedWithEther[key]) {
            // Charge 2% (constant tax) if user doesn't own at least 500 Sea Token
            // if(quoteErc20.balanceOf(_msgSender()) <  seaAmountForExemptFee) {
            //     require(msg.value >= price.add(price.mul(200).div(MAX_FEE_PERCENT)), 'pay amount insufficient');
            // }
            // else {
            //     require(msg.value >= price, 'pay amount insufficient');
            // }
            ...
            safeTransferQuoteToken(_msgSender(), seller, price);
            // caller: address
            // Token trnasfer: msg.sender => seller
        }
