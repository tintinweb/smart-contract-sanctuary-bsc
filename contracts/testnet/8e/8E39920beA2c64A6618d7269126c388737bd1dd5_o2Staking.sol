...
contract o2Staking is Context, Ownable {
    ...
    address public _o2=address(0x806EA413C35e3a6AFEC0afe8706cd8D99DC7fC61);//test o2
    ...
    function stake(uint256 amount,address _token,uint256 interestId) public{
        ...
        require(_buyAmountEveryDay[today]+amount<=_maxBuyAmountEveryDay,"error3");
        _buyAmountEveryDay[today]=_buyAmountEveryDay[today].add(amount);
        interestInfo memory _info=_interestInfos[interestId];
        if(_stakeCharge>0){
            //扣rbl手续费
            IBEP20(_rbl).transferFrom(_msgSender(),_recipientRblAddress,_amountOut(amount.mul(_stakeCharge).mul(_info.stakeTime.div(24*3600)).div(1000),_usdt,_rbl));
        }
        uint inAmount;
        if(_usdt!=_token){
    ...
    function unStake(uint256 stakeId) external{
        ...
        if(info.status==false&&info.waitReceiveAmount>0){
            ...
            if(_breachOpen==true&&block.timestamp<info.stakeEndTime&&_breachRatio>0){
                ...
                require(info.status==true,"error3");
            }
