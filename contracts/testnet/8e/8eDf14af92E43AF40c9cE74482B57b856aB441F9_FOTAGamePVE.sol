...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  uint public finishGameTimeOut; //todo delete when release
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature) external {
    bool firstCompleteMission = _validateFinishGame(_mission, _heroIds, _itemIds, _signature);
    uint energy;
    uint from;
    uint to;
  ...
  function _validateFinishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature) private returns (bool) {
