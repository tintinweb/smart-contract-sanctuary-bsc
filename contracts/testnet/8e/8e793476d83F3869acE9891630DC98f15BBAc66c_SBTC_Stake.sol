...
contract SBTC_Stake is OwnableUpgradeable {
    ...
    function checkAllToken() public view returns(uint[] memory index,string[] memory names,uint[] memory decimals){
        uint temp;
    ...
    function _calculateReward(address addr) internal view returns (uint, bool){
        ...
        bool out;
        if (quota >= user.totalPower) {
            rew = rew - countingSbtc(quota - user.totalPower, price);
            out = true;
        }
        return (rew, out);
    }
    ...
    function _processRefer(address addr, uint price, uint debt_,uint tokenIndex) internal {
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            if (isOut) {
                ...
                uint finalRew = countingSbtc(userInfo[temp].totalPower - userInfo[temp].toClaimQuota, sbtcPrice) + userInfo[temp].toClaim;
