...
contract chronoxia is Context, IBEP20, Ownable {
    ...
    string public constant name     = "Chronoxia";
    ...
    uint256 public _sellPercentageOfLiquidityForMarketing = 0;

    uint256 public _maxTxAmount     = _tTotal / 2;
    uint256 public _minTokenBalance = _tTotal / 500000;


    event SetTransferFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee);
    event SetBuyFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee);
    event SetSellFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee);
    ...
    function setTransferFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        require(
            taxFee + liquidityFee + marketingFee <= 25
        );
        _taxFeeTransfer       = taxFee;
        _liquidityFeeTransfer = liquidityFee;
        _marketingFeeTransfer = marketingFee;

        _tFeeTotal   = taxFee + liquidityFee + marketingFee;
        emit SetTransferFee(taxFee, liquidityFee, marketingFee);
    }
    function setBuyFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        ...
        _tFeeTotal   = taxFee + liquidityFee + marketingFee;
        emit SetBuyFee(taxFee, liquidityFee, marketingFee);
    }
    function setSellFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee) external onlyOwner {
        ...
        _tFeeTotal   = taxFee + liquidityFee + marketingFee;
        emit SetSellFee(taxFee, liquidityFee, marketingFee);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        uint256 bnbForMarketing = newBalance.mul(_sellPercentageOfLiquidityForMarketing).div(100);
