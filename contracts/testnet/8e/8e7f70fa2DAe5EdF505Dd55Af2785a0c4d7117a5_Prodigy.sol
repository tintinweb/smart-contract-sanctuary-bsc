...
contract Prodigy is IERC20Metadata, Ownable {
    ...
    uint256 internal constant _totalSupply = 2000000000000 * (10**9);
    ...
    constructor () {
        ...
        storeTransferData(address(0), owner(), _totalSupply);
    }
     ...
     */
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee, bool isBuying) public view returns(uint256) {
    ...
    // holders transfer data storage
    function storeTransferData(address sender, address recipient, uint256 amount) private {
        setHolderBuying(recipient, amount);
        setHolderSelling(sender, amount);
    }

    function setHolderBuying(address holderAddress, uint256 amount) private {
        holders[holderAddress].tokens_bought =  holders[holderAddress].tokens_bought.add(amount);
    }

    function setHolderSelling(address holderAddress,  uint256 amount) private {
        holders[holderAddress].tokens_sold =  holders[holderAddress].tokens_sold.add(amount);
    }

    function calculateHoldersRFI(address holderAddress) external view returns (uint256) {
        uint256 currentBal = balanceOf(holderAddress);
        uint256 lifetimeBal = holders[holderAddress].tokens_bought - holders[holderAddress].tokens_sold;

        if(currentBal > lifetimeBal) {
            return currentBal - lifetimeBal;
        }
    ...
    function processReflectedBal (address sender, address recipient, uint256 amount, uint256 sumOfFees, bool isBuying) private {
        ...
        storeTransferData(sender, recipient, tTransferAmount); // store the holders transfers
