...
contract PriceFeedSwapModifier is INimbusRouter, Ownable {
    ...
    function setUseBNBSwapForTokens(address _tokenA, address _tokenB, bool _usePriceFeedBNB) external onlyOwner {
    ...
    function rateConversion(uint256 amount, address sourceTokenAddress, address destTokenAddress, bool isOut) public view returns (uint256 result) {
        ...
        if (useBNBSwapForTokens[sourceTokenAddress][destTokenAddress]) {
            uint result1 = _swapResult(amount, sourceTokenAddress, address(baseToken), isOut);
            result = _swapResult(result1, address(baseToken), destTokenAddress, isOut);
            return result;
        }
