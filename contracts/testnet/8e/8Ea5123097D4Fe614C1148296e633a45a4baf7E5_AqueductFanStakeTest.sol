...
contract AqueductFanStakeTest is
ERC721Enumerable,
ERC721Burnable,
VRFConsumerBaseV2
{
    ...
    Counters.Counter public _sectionId;
    Counters.Counter public _stageId;
    Counters.Counter public _poolId;
    mapping(uint256 => uint256) public _poolIdToStakeId;
    ...
    // STORAGE //
    mapping(uint256 => string[]) public stageToResults;
    ...
    {
        treasury = 0x74B376b0232B0f704A8A561527F2E9d2306161cB;
        erc20 = ITreasury(treasury).getErc20Address();
        s_subscriptionId = 2076;
        _tokenId.increment();
        _poolId.increment();
    ...
    function createFristSection() public {
        createEvent("Event", 1000);
        createSection("Section1", block.timestamp);
        createStage(
            "Stage1",
            block.timestamp,
        ...
        createPool(0, 0, 2000000000000000000, 4000, 1000);
    }

    function createSecondSection(uint256 silverChance, uint256 goldChance)
    public
    {
        createSection("Section2", block.timestamp);
        createStage(
            "Stage2",
            block.timestamp,
            block.timestamp + 1000000,
        ...
        createPool(1, 1, 3000000000000000000, silverChance, goldChance);
    }

    function createThirdThird(uint256 silverChance, uint256 goldChance) public {
        createSection("Section3", block.timestamp);
        createStage(
            "Stage3",
            block.timestamp,
            block.timestamp + 1000000,
        ...
        createPool(2, 2, 1000000000000000000, silverChance, goldChance);
    }
    ...
    override(VRFConsumerBaseV2)
    {
        ...
        for (uint256 i = 0; i < requestIdToSilverAmount[requestId]; i++) {
            silverArray[i] =
            uint(keccak256(abi.encodePacked(randomWords[i]))) %
    ...
    function createPool(
        ...
        require(silverTicketChance_ + goldTicketChance_ < 9999, "Sum of chances should be less then 10000(100%)");
    ...
    ) public whenNotPaused poolShouldExist(poolId_) stageShouldNotBeFinished(poolId_) {
    ...
    //         poolIdToTokenIdToBet[poolId_][betId_].betOn
    //     ] -= poolIdToTokenIdToBet[poolId_][betId_].amount;
    //     poolToStakesAmount[poolId_] -= poolIdToTokenIdToBet[poolId_][betId_]
    //         .amount;
