...
contract Crowdsale is Ownable {
    ...
    function unstakeToken() external {
        ...
        for(uint i=0; i< _length; i++) {
            ...
            stakedData[msg.sender][i].interestAmount = calculateInterestAmount(stakedData[msg.sender][i].stakedAmount, stakedData[msg.sender][i].stakedTimestamp);
        }
    ...
    function claimStakedInterest() external {
        ...
        for(uint i=0; i< _length; i++) {
            if(stakedData[msg.sender][i].interestAmount > 0 && stakedData[msg.sender][i].stakedAmount == 0 && stakedData[msg.sender][i].claimTimestamp == 0) {
                totalStakedInt += stakedData[msg.sender][i].interestAmount; // Unstake occured
            } else if(stakedData[msg.sender][i].interestAmount > 0 && stakedData[msg.sender][i].stakedAmount > 0 && stakedData[msg.sender][i].claimTimestamp == 0) {
                stakedData[msg.sender][i].interestAmount = 0; //Claim occured earlier but principal still staked
                totalStakedInt += calculateInterestAmount(stakedData[msg.sender][i].stakedAmount, stakedData[msg.sender][i].claimTimestamp); // Claim occured earlier
            } else if(stakedData[msg.sender][i].claimTimestamp == 0){
                stakedData[msg.sender][i].interestAmount = calculateInterestAmount(stakedData[msg.sender][i].stakedAmount, stakedData[msg.sender][i].stakedTimestamp);
                totalStakedInt += stakedData[msg.sender][i].interestAmount;
            }
