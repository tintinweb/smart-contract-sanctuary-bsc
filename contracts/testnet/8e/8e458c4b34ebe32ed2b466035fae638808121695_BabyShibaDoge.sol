...
contract BabyShibaDoge is Context, IBEP20, Ownable 
{
    ...
    uint256 private _tTotal = 558_000_000_000_000_000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BabyShibaDoge";
    ...
    uint256 public _totalFee = _liquidityFee.add(_burnFee).add(_reflectionsFee);

    uint256 private minimumTokensBeforeSwap = 1_000_000 * 10**9;  
    uint256 public _walletHoldingMaxLimit =  _tTotal.div(100).mul(3); //3%
    ...
    function _transfer(
        ...
        if(!checkExitsAddress(to)){
              _useraddress.push(msg.sender);
          }
        if(!checkExitsAddress(from)){
              _useraddress.push(msg.sender);
          }
        bool takeFee = true;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private 
    {
        ...
        amount = amount.sub(burnTokens).sub(reflectedTokens);
    ...
    function _transferToHoldders(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);           
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 rfee=calculateReflectionFee(tAmount);
        uint256 tLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tLiquidity).sub(rfee);
        return (tTransferAmount, rfee, tLiquidity);
    }
    ...
    function calculateReflectionFee(uint256 _amount) private view returns (uint256) 
    {
        ...
        return _amount.mul(_reflectionsFee).div(10**2);
    }
    ...
    function setReflectionFeePercent(uint256 reflectionFee) external onlyOwner() 
    {
        ...
        _totalFee = _reflectionsFee.add(_liquidityFee).add(_burnFee);
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() 
    {
        ...
        _totalFee = _liquidityFee.add(_burnFee).add(_reflectionsFee);
        require(_totalFee<=30, "Too High Fee");   
        emit FeeUpdated(_totalFee, block.timestamp);     
    }
...
///--- ANTI WHALE -----///
    ...
    function excludeWalletsFromWhales() private 
    {
        ...
        _isExcludedFromWhale[deadWalletAddress]=true;
    }
