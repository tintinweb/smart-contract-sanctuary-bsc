...
contract FridayLottery is Initializable{
    ...
    mapping(uint256 => historyOfPrizeWinners) public historyOfLotteryPrizeWinners;
    mapping(uint256 => historyOfPrizePerPersonAmount) public historyOfPrizePerPersonAmounts;
    mapping(uint256 => historyOfPrizeTotalAmount) public historyOfPrizeTotalAmounts;
    ...
    mapping(uint256 => mapping(address => roundInfo[])) public yourHistoryPerRound;
    ...
    event TicketBought(address indexed buyer, uint  tickets);
    event TicketGuessed(address indexed buyer, guessRandomNumberAndStars  guesses);
    event TicketHistoryPerRound(uint indexed roundNumber,address indexed buyer, roundInfo  info);
    ...
    function initialize(
        ...
        distributionFee = //distribution fee for each prize winner
    ...
    function buyTicket() external checkLotteryStarted {
        require(block.timestamp > (lotteryStartTime + cooldownTime), "The Lottery Is Not Started Yet!" );
    ...
    function addGuessNumber(
        ...
        //     )
        // );
        emit TicketHistoryPerRound(roundNumber, msg.sender, roundInfo(
                guessRandomNumberAndStars(
                    _numbersAndStars.number1,
                    _numbersAndStars.number2,
                    _numbersAndStars.number3,
    ...
    function addWinningNumber(
        ...
        require(winningNumbersAndStars.number1 == 0, "Winning Numbers Already Added");
        ...
        onlyOwner
    {
        ...
        uint256 maintenanceStakeAndGoodCausesAmount = (poolPrize * maintenanceStakeAndGoodCausesFee) / 10000;
        ...
        historyOfPrizePerPersonAmounts[roundNumber] = historyOfPrizePerPersonAmount(
        ...
        guessCount = 0;   
        lotteryStartTime = _lotteryEndTime;
        lotteryEndTime = (_lotteryEndTime + 20 minutes);
        winningNumbersAndStars = guessRandomNumberAndStars(0, 0, 0, 0, 0, 0, 0);
        roundNumber++;
    }
    ...
    function distributePrize(
        ...
        for (uint256 i = 0; i < _totalWinner; i++) {
            roundPersonInfo=yourHistoryPerRound[roundNumber][winners[i]];
            roundPersonInfo = yourHistoryPerRound[roundNumber][winners[i]];
            
            for (uint256 j = 0; j < roundPersonInfo.length; j++) {
                if (
                    roundPersonInfo[j].winningIndex == i &&
                    roundPersonInfo[j].wininnigCategory == _wininnigCategory
                ) {
                    roundPersonInfo[j] = roundInfo(
                        roundPersonInfo[j].guessNumbersAndStars,
                        true,
                        amountPerWinner,
                        _wininnigCategory,
                        roundPersonInfo[j].ticketNumber,
                        roundPersonInfo[j].winningIndex
                    );
                    emit TicketHistoryPerRound(roundNumber, msg.sender, roundInfo(
                        roundPersonInfo[j].guessNumbersAndStars,
                        true,
                    ...
                    yourHistoryPerRound[roundNumber][guessInfos[i].user] = roundPersonInfo;
                    break;
                }
            }
        ...
        )
    {
...

        {
            for (uint256 i = 0; i < totalTickets; i++) {
                ...
                if (
                    guessInfos[i].guess.number1 == winningNumbersAndStars.number1 ||
                    guessInfos[i].guess.number1 == winningNumbersAndStars.number2 ||
                    guessInfos[i].guess.number1 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number1 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number1 == winningNumbersAndStars.number5
                ) {
                    matchedNumber++;
                }
                if (
                    guessInfos[i].guess.number2 == winningNumbersAndStars.number1 ||
                    guessInfos[i].guess.number2 == winningNumbersAndStars.number2 ||
                    guessInfos[i].guess.number2 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number2 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number2 == winningNumbersAndStars.number5
                ) {
                    matchedNumber++;
                }
                if (
                    guessInfos[i].guess.number3 == winningNumbersAndStars.number1 ||
                    guessInfos[i].guess.number3 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number3 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number3 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number3 == winningNumbersAndStars.number5
                ) {
                    matchedNumber++;
                }
                if (
                    guessInfos[i].guess.number4 == winningNumbersAndStars.number1 ||
                    guessInfos[i].guess.number4 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number4 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number4 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number4 == winningNumbersAndStars.number5
                ) {
                    matchedNumber++;
                }
                if (
                    guessInfos[i].guess.number5 == winningNumbersAndStars.number1 ||
                    guessInfos[i].guess.number5 == winningNumbersAndStars.number5 ||
                    guessInfos[i].guess.number5 == winningNumbersAndStars.number3 ||
                    guessInfos[i].guess.number5 == winningNumbersAndStars.number4 ||
                    guessInfos[i].guess.number5 == winningNumbersAndStars.number5
                ) {
                    matchedNumber++;
                }
                if (matchedNumber == 5) {
                    if (
                        guessInfos[i].guess.star1 == winningNumbersAndStars.star1 ||
                        guessInfos[i].guess.star1 == winningNumbersAndStars.star2
                    ) {
                        matchedStar++;
                    }
                    if (
                        guessInfos[i].guess.star2 == winningNumbersAndStars.star1 ||
                        guessInfos[i].guess.star2 == winningNumbersAndStars.star2
                    ) {
                        matchedStar++;
                    }
                }
                address _userAddress = guessInfos[i].user;
                if (matchedNumber == 5 && matchedStar == 2) {
                    uint256 _length=yourHistoryPerRound[roundNumber][_userAddress].length;
                    roundPersonInfo=yourHistoryPerRound[roundNumber][_userAddress];
                    
                    for (uint256 j = 0; j < _length; j++) {
                        if (roundPersonInfo[j].ticketNumber == i) {
                            roundPersonInfo[j] = roundInfo(
                                roundPersonInfo[j].guessNumbersAndStars,
                                true,
                                0,
                            ...
                            yourHistoryPerRound[roundNumber][_userAddress] = roundPersonInfo;
                            break;
                        }
                    }
                    prize1Winner[prizeWinner.totalPrize1Winner] = _userAddress;
                    prizeWinner.totalPrize1Winner++;
                    ...
                    prize2Winner[prizeWinner.totalPrize2Winner] = _userAddress;
                    prizeWinner.totalPrize2Winner++;
                    ...
                    prize3Winner[prizeWinner.totalPrize3Winner] = _userAddress;
                    prizeWinner.totalPrize3Winner++;
                    ...
                    prize4Winner[prizeWinner.totalPrize4Winner] = _userAddress;
                    prizeWinner.totalPrize4Winner++;
                    ...
                    prize5Winner[prizeWinner.totalPrize5Winner] = _userAddress;
                    prizeWinner.totalPrize5Winner++;
                    ...
                    prize6Winner[prizeWinner.totalPrize6Winner] = _userAddress;
                    prizeWinner.totalPrize6Winner++;
                }
            }
        }
    ...
    function getMaintenanceStakeAndGoodCausesFee() external view returns(uint16) {
        return maintenanceStakeAndGoodCausesFee ;
    }

    function changeMaintenanceStakeAndGoodCausesAddress( address _maintenanceStakeAndGoodCausesAddress ) external onlyOwner {
        maintenanceStakeAndGoodCauses = _maintenanceStakeAndGoodCausesAddress;
    }

    function getDistributionFee() external view returns( distributionPercentage memory) {
    ...
    function setIsTicketCanBeSold(bool _isTicketCanBeSold) external onlyOwner checkLotteryStarted {
        isTicketCanBeSold = _isTicketCanBeSold;
    }

    function setLotteryStartTime(uint256 _lotteryStartTime) external onlyOwner checkLotteryStarted {
        require( block.timestamp < _lotteryStartTime, "Lottery Start Time Cannot Be In The Past!" );

        lotteryStartTime = _lotteryStartTime;
        lotteryEndTime = lotteryStartTime + 20 minutes;
    }

    function setLotteryEndTime(uint256 _lotteryEndTime) external onlyOwner checkLotteryStarted {
        require( block.timestamp < _lotteryEndTime, "Lottery End Time Cannot Be In The Past!" );
    ...
    function stopLottery() external onlyOwner checkLotteryStarted {
        require( guessCount == 0, "Please Annouce Winner Before Stopping The Lottery" );

        require( block.timestamp >= lotteryEndTime, "Lottery Cannot Be Stopped Before The Lottery End Time" );
    ...
    function setTuesdayLotteryAddress(address _tuesdayLotteryAddress) external onlyOwner {
