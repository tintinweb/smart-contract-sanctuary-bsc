...
contract VariableTaxController is OwnableUpgradeable {
    ...
    function TEST_setAirdrops(uint256 _airdrop_amount) external {
        address _addr = msg.sender;
    ...
    function TEST_setDeposit(uint256 _deposit_amount) external {
        address _addr = msg.sender;
    ...
    function _getUserStats(address _addr, uint256 _amount_to_claim, uint256 _amount_to_nitro, uint256 _amount_to_deposit) internal view returns (uint256 nitro_amount, uint256 claim_amount, uint256 airdrop_amount, uint256 claim_backlog, uint256 nitro_lead){
        ...
        uint256 claimAmount = userStats[_addr].claim_amount.add(_amount_to_claim);
        uint256 nitroAmount = userStats[_addr].nitro_amount.add(userStats[_addr].airdrop_amount);
        nitroAmount = nitroAmount.add(_amount_to_nitro);
        nitroAmount = nitroAmount.add(_amount_to_deposit);

        if(claimAmount > nitroAmount){
            claim_backlog = claimAmount.sub(nitroAmount);
        }else if(nitroAmount > claimAmount.add(_amount_to_nitro)){
            nitro_lead = nitroAmount.sub(claimAmount);
        }
