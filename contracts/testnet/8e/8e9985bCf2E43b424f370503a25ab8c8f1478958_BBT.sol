...
contract BBT is ERC20, Ownable {
    ...
    uint256 public constant MAX_BUY_FEE = 300;
    ...
    uint256 public buyFee = MAX_BUY_FEE;
    uint256 public sellFee = MAX_SELL_FEE;
    uint256 public topHoldersAmount = MAX_TOP_HOLDERS_AMOUNT;
    uint256 public thresholdAmount;
    uint256 public tradingEnabledBlockTimestamp;
    uint256 private _buyFeeAmount;
    uint256 private _sellFeeAmount;
    uint256 private _holdersFeeAmount;
    ...
    function setFees(uint256 buyFee_, uint256 sellFee_) external onlyOwner {
        require(
            buyFee_ <= MAX_BUY_FEE && sellFee_ <= MAX_SELL_FEE,
            "BBT: fees exceed the maximum permissible values"
        );
        buyFee = buyFee_;
        sellFee = sellFee_;
    }
    ...
    function distributeTokensBetweenHolders(address[] calldata accounts_) external onlyOwner {
        ...
        uint256 holdersFeeAmount = _holdersFeeAmount;
        if (holdersFeeAmount != 0) {
            uint256 amountToDistributeBetweenEachOfTopHolder = holdersFeeAmount / (2 * topHoldersAmount);
            for (uint256 i = 0; i < accounts_.length; i++) {
                _transfer(address(this), accounts_[i], amountToDistributeBetweenEachOfTopHolder);
            }
            uint256 amountToDistributeBetweenNftHolders = holdersFeeAmount / 2;
        ...
        override
    {
        ...
        if (block.timestamp - tradingEnabledBlockTimestamp <= 15) {
        ...
        uint256 amountToReceive = takeFee ? _takeFees(from_, amount_, buy, sell) : amount_;
    ...
    function _takeFees(address from_, uint256 amount_, bool buy_, bool sell_) private returns (uint256) {
        uint256 feeAmount;
        if (buy_) {
            feeAmount = amount_ * buyFee / BASE_PERCENT;
            if (feeAmount > 0) {
                uint256 buyFeeAmount = feeAmount / 3;
                _buyFeeAmount += buyFeeAmount;
                _holdersFeeAmount += feeAmount - buyFeeAmount;
            ...
            if (feeAmount > 0) {
                ...
                _balances[address(this)] += feeAmount;
                emit Transfer(from_, address(this), feeAmount);
                if (_sellFeeAmount + _buyFeeAmount >= thresholdAmount) {
                    _swap();
                }
            }
        } else {
