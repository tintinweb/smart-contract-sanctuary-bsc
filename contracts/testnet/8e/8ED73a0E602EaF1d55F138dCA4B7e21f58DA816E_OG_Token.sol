...
contract OG_Token is Context, IERC20, Ownable {
    ...
    uint256 private _taxFee = 40;            //4% for reflect to holders at Buy the tokens.
    uint256 private _sellTaxFee = 40;        //4% for reflect to holders at sell the tokens.
    ...
    uint256 private _developementFee = 5;        //0.5% of tokens will goes to the developement wallet at buy the tokens.
    uint256 private _sellDevelopementFee = 10;   //1% of tokens will goes to the developement wallet at sell the tokens.
    ...
    // wallet[0] -- Future Burn Wallet              -  25%
    // wallet[1] -- teamWallet                      -  10%
    // wallet[2] -- MarketWallet                    -  7%
    // wallet[3] -- developementWallet              -  5%
    // wallet[4] -- investor & partner wallets      -  10%
    // wallet[5] -- pancakeswap wallet              -  10%
    // wallet[6] -- Exchange wallet                 -  16%
    // wallet[7] -- launchPad                       -  15.04%
    // wallet[8] -- privateSale                     -  1.96%
    
    constructor (address _BUSD,address[] memory wallets, address _buybackWallet)  {
        require(wallets.length == 9,"invalid wallets");
    ...
    function SellFees() external view returns(uint256 TaxFee,uint256 TeamFee,uint256 MarketingFee,uint256 DevelopementFee,uint256 BuybackFee){
        return (_sellTaxFee, _sellTeamFee, _sellMarketFee, _sellDevelopementFee, _sellBuybackFee);
    }

    function BuyFees() external view returns(uint256 TaxFee,uint256 TeamFee,uint256 MarketingFee,uint256 DevelopementFee,uint256 BuybackFee){
        return (_taxFee, _teamFee, _marketFee, _developementFee, _buybackFee);
    }
    ...
    function Burn(uint256 amount) external onlyOwner {
        ...
        emit BurnTokens(msg.sender, deadAddress, amount);
    }
