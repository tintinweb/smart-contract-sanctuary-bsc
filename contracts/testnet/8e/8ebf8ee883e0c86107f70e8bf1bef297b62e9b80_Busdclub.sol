...
contract Busdclub {
    ...
    address owner;
    uint[20] level_bonuses = [1000, 200, 100, 100, 100, 40, 40, 40, 40, 40, 30, 30, 30, 30, 30, 30, 30, 30, 30, 30];
    ...
    function packageInfo(uint256 _pkg) pure private  returns(uint8 p) {
        if(_pkg == 10e18){
            ...
            p=2;
        }else if(_pkg == 10000e18){
            p=3;
        }else if(_pkg == 100000e18){
            p=4;
        }else{
            p=0;
        }
    ...
    function _setReferral(address _referral, uint256 _refAmount) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            players[_referral].levelIncome[i]+=_refAmount.mul(level_bonuses[i]).div(baseDivider);
    ...
    function globalclub(uint256 _amount) public {
