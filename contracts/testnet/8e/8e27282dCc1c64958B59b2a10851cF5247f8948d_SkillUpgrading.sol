...
contract SkillUpgrading is BaseUpgrading {
  ...
  event Upgraded(address indexed user, uint indexed tokenId, uint8 acceptedRatio, UpgradingInfo upgradingInfo, IMarketPlace.PaymentCurrency _paymentCurrency, bool success);
  ...
  function skillUp(
    ...
    require(_acceptedRatio <= requirements[section].successRatio, "Upgrading: invalid ratio");
    _upgradeSkill(_tokenId, _acceptedRatio, _seed, upgradingInfo, _paymentCurrency, eatherPrice);
  ...
  function _upgradeSkill(
    ...
    (uint ownPrice, uint failedUpgradingAmount) = heroNft.getHeroPrices(_tokenId);
    if (success) {
      heroNft.updateOwnPrice(_tokenId, ownPrice + _eatherPrice);
      heroNft.skillUp(_tokenId, _upgradingInfo.skillIndex);
    } else {
      uint _upgradingFee = requirements[_upgradingInfo.section].fee * uint(_acceptedRatio) / uint(requirements[_upgradingInfo.section].successRatio);
      takeFund(_paymentCurrency, _upgradingFee);
      heroNft.updateFailedUpgradingAmount(_tokenId, failedUpgradingAmount + _upgradingFee + _eatherPrice);
    }

    emit Upgraded(msg.sender, _tokenId, _acceptedRatio, _upgradingInfo, _paymentCurrency, success);
  }
