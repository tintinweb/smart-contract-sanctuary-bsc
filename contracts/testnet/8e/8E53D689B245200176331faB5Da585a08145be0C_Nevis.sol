...
contract Nevis is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal =  17000* 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private totalWithdraw;

    string private _name = "nevis";
    string private _symbol = "nevis";
    uint8 private _decimals = 9;
    
    
    uint256 public _redistributionFee = 15;
...
//(Physical Reinvestment Trust Fund) 
    uint256 public _NevisbankFee = 0;
    address public _NevisbankWallet = 0x7Aab0EFa276b2F5FfaA5b16e37a34977698Aa63e;
    uint256 private _previousNevisbankFee = _NevisbankFee;
    ...
    event dividentDistribution(address user, uint256 amount);
    ...
    function _distributerewards(address depositor, uint256 fees) internal{
        uint256 dividentShare;
        for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
          if (!_isExcluded[investors[i]]) {
             dividentShare = _tOwned[investors[i]].mul(100).div(totalHoldings);
            user.rewards+=fees.mul(dividentShare).div(100);
          }
        }
        emit dividentDistribution(depositor,fees);
    }

   
    function removeAllFee() private {
        if(_redistributionFee == 0 && _liquidityFee == 0 && _NevisbankFee==0 && _burnFee==0) return;
        
        _previousredistributionFee = _redistributionFee;
        _previousLiquidityFee = _liquidityFee;
        _previousBurnFee = _burnFee;
        _previousNevisbankFee = _NevisbankFee;
    ...
    function restoreAllFee() private {
       ...
       _NevisbankFee = _previousNevisbankFee;
    }
...
function claimdivident() external payable {

        require(balanceOf(uniswapV2Pair) > 0, "Insufficient liquidity");

        User storage user = users[msg.sender];
        uint256 _amount;
        _amount=user.rewards;
		...
		emit widthdraw(msg.sender, _amount);
	}
    ...
    function swaptoken (uint256 _amount) private {
		...
		IERC20(BUSD).approve(address(uniswapV2Router),_amount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate _NevisbankFee amount
     
        uint256 OwnerAmt = amount.mul(_NevisbankFee).div(100);
       
                if (!_isExcludedFromFee[sender] && !_isExcludedFromFee[recipient]) {
                
                    _transferStandard(sender, recipient, (amount.sub(OwnerAmt)));
                } else if (_isExcludedFromFee[sender] && _isExcludedFromFee[recipient]) {
                    _transferBothExcluded(sender, recipient, (amount.sub(OwnerAmt)));
                }else{
                    _transferToExcluded(sender, recipient, (amount.sub(OwnerAmt)));
                } 
                    if(includeIninvestor(recipient)!=true)
                    {
        ...
        function includeIninvestor(address recipient) private view returns(bool) {
                for (uint256 i = 0; i < investors.length; i++) {
                    if (investors[i] == recipient || DEAD == recipient ||  ZERO == recipient ) {
                        return true;
                    }
                }
            }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
          if(owner()==sender){
            totalHoldings+=tTransferAmount;
         }else{
              totalHoldings-= tFee;
         }
          _distributerewards(sender,tFee);
