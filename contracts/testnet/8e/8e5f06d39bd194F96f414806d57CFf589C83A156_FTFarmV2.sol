...
contract FTFarmV2 is IFTFarm, AccessControl {
    ...
    uint256 public maxRewardHash = 1000*1e18;
    ...
    function stake(address _owner, uint256 _amount) external onlyOwner
    {
        _stake1(_owner, _amount);
    }

    function batchStake(uint256 _id,address[] calldata _addresses,uint256[] calldata _amounts) external onlyOwner{
        require(_addresses.length == _amounts.length,"error");
        for(uint256 i=0;i<_addresses.length;i++){
            _stake1(_addresses[i], _amounts[i]);
        }
        emit BatchStake(_id);
    }
    ...
    function getReward(uint256 _reward) external updateReward(msg.sender) {
        ...
        uint256 balance = getHash(user).min(maxRewardHash);
    ...
    function _stake1(address _owner, uint256 _amount) private updateReward(_owner)
    {
    ...
    function setMaxRewardHash(uint256 _max) external onlyOwner{
        maxRewardHash = _max;
    }
