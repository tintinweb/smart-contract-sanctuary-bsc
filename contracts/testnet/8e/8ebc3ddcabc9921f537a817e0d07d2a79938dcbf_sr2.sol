...
contract sr2 is Ownable {
    using SafeMath for uint;

    address public constant factory = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
    address public constant WBNB = 0x214467FBF5b6026F1Dc6FaC6346a5689d7ef2462;
    // bsc variables 
    ...
    function sandwichExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], address(this), PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    //================== 只为一种代币服务============================================
    //买入
    function In_onetoken(uint amountIn, uint amountOutMin, address _onetoken) external onlyOwner() returns(bool success) {
        
        require(IERC20(WBNB).balanceOf(address(this)) >= amountIn, "in: not enough wbnb on the contract");  //wbnb必须大于交易值
        address[] memory path;
        path = new address[](2);
        path[0] = WBNB;     //0为wbnb地址   //1为out地址
        path[1] = _onetoken;
        
        sandwichExactTokensForTokens(
            amountIn,       //wbnb使用数量
            amountOutMin,       //新token最少数量
            path,               //地址对
            address(this),      //本合约地址
            block.timestamp + 120       //时间戳
        );
        return true;
    }

    //卖出
    function Out_onetoken(uint amountOutMin, address _onetoken) external onlyOwner() returns(bool success) {
        
        uint amountIn = IERC20(_onetoken).balanceOf(address(this));
        amountIn = amountIn * 999 / 1000;
        require(amountIn >= 0, "out: empty balance for this token");        //确保合约拥有代币
  ...
  //================== owner functions=====================
    ...
    //bnb to WBNB   bnb转化WBNB
    function bnb_to_WBNB() external payable onlyOwner{
