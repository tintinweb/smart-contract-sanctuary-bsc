...
contract ShinobiVerse is Context, IERC20, Ownable {
    ...
    uint256 public _bMaxTxAmount = _tTotal.mul(15).div(100);
    uint256 public _sMaxTxAmount = _tTotal.mul(15).div(100);
    ...
    function _transfer(
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    uint256 busdLiquidityToken = contractTokenBalance.mul(_sellMarketingFee+_sellDevelopmentFee).div(_sellLiquidityFee);
        ...
        //for presale purpose only. 
        if(noTransferFee==true)
        {
            _tempTaxFee = 0;
            _tempVaultFee = 0;
            _tempLiquidityFee = 0;
    ...
    function setReawardAndMarketingFee(uint256 _sellRewardsPercent,uint256 _sellMarketingPercent, uint256 _sellDevelopmentPercent, uint256 _sellLiquidityPercent, uint256 _buyRewardsPercent,uint256 _buyMarketingPercent, uint256 _buyDevelopmentPercent, uint256 _buyLiquidityPercent ) external onlyOwner {
        ...
        _liquidityFee = _buyLiquidityPercent+_buyRewardsPercent+_buyMarketingPercent+_buyDevelopmentPercent;
    }
