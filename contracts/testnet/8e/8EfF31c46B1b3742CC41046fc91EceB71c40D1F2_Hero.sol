...
contract Hero is ERC721Enumerable, Ownable,AccessControlEnumerable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;
    string private _url;
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    event Mint(address to,uint256 hero_type);
    ...
    function mint(address to,uint256 hero_type) public onlyMinter returns (uint256) {
        _tokenIdTracker.increment();
        uint256 token_id = _tokenIdTracker.current();
        _mint(to, token_id);
        emit Mint(to,hero_type);
        return token_id;
    }

    function listTokenIds(address owner)public view returns (uint256[] memory tokenIds){
