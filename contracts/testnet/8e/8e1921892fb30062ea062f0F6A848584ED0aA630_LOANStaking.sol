...
contract LOANStaking is BaseContract, ILOANStaking, BaseMath {
    ...
    uint public F_FUR;  // Running sum of FUR fees per-LOAN-staked
    ...
    address public furfiActivePoolAddress;
    address public furFiAddress;
    address public furVaultManagerAddress;
    address public furBorrowerOperationsAddress;
    address public furActivePoolAddress;
    address public furAddress;

    // --- Events ---

    event StakeChanged(address indexed staker, uint newStake);
    event StakingGainsWithdrawn(address indexed staker, uint FURUSDGain, uint FURFIGain, uint FURGain);
    event F_FURFIUpdated(uint _F_FURFI);
    event F_FURUpdated(uint _F_FUR);
    ...
    event FURSent(address _account, uint _amount);
    event StakerSnapshotsUpdated(address _staker, uint _F_FURFI, uint _F_FUR, uint _F_FURUSD);
    ...
    function setTokenAddresses(address _loanTokenAddress, address _furUSDTokenAddress) external onlyOwner {
    ...
    function setFurFiLiquityAddresses(
        ...
        address _furfiActivePoolAddress
    ) 
        external 
        onlyOwner 
    {
        ...
        furfiActivePoolAddress = _furfiActivePoolAddress;
        ...
        onlyOwner 
    {
        ...
        furActivePoolAddress = _furActivePoolAddress;
    ...
    function getFUR() external view override returns (uint) {
    ...
    function stake(uint _LOANamount) external override whenNotPaused {
        ...
        uint FURGain;
        uint FURUSDGain;
        // Grab any accumulated FURFI, FUR and FURUSD gains from the current stake
        if (currentStake != 0) {
            FURFIGain = _getPendingFURFIGain(msg.sender);
            FURGain = _getPendingFURGain(msg.sender);
       ...
       _updateUserSnapshots(msg.sender);
        ...
        emit StakingGainsWithdrawn(msg.sender, FURUSDGain, FURFIGain, FURGain);
    ...
    function unstake(uint _LOANamount) external override whenNotPaused {
        ...
        uint FURGain = _getPendingFURGain(msg.sender);
    ...
    function increaseF_FUR(uint _FURFee) external override whenNotPaused {
        _requireCallerIsVaultManager();
        uint FURFeePerLOANStaked;
     
        if (totalLOANStaked > 0) {FURFeePerLOANStaked = _FURFee.mul(DECIMAL_PRECISION).div(totalLOANStaked);}

        F_FUR = F_FUR.add(FURFeePerLOANStaked);
        emit F_FURUpdated(F_FUR);
    }
    ...
    function getPendingFURGain(address _user) external view override returns (uint) {
        return _getPendingFURGain(_user);
    }

    function _getPendingFURGain(address _user) internal view returns (uint) {
        uint F_FUR_Snapshot = snapshots[_user].F_FUR_Snapshot;
        uint FURGain = stakes[_user].mul(F_FUR.sub(F_FUR_Snapshot)).div(DECIMAL_PRECISION);
        return FURGain;
    }
    ...
    function _updateUserSnapshots(address _user) internal {
        ...
        snapshots[_user].F_FUR_Snapshot = F_FUR;
        snapshots[_user].F_FURUSD_Snapshot = F_FURUSD;
        emit StakerSnapshotsUpdated(_user, F_FURFI, F_FUR, F_FURUSD);
    }
    ...
    function _sendFURGainToUser(uint FURGain) internal {        
        IERC20Upgradeable FurToken = IERC20Upgradeable(furAddress);
        FurToken.safeTransfer(msg.sender, FURGain);
        emit FURSent(msg.sender, FURGain);
    }
    ...
    function _requireCallerIsVaultManager() internal view {
        require(
            ...
            "LOANStaking: caller is not VaultM");
    }
    ...
    function _requireCallerIsBorrowerOperations() internal view {
        require(
            ...
            "LOANStaking: caller is not BorrowerOps");
    }

     function _requireCallerIsActivePool() internal view {
        require(
            msg.sender == furfiActivePoolAddress ||
            msg.sender == furActivePoolAddress,
            "LOANStaking: caller is not ActivePool");
    }
