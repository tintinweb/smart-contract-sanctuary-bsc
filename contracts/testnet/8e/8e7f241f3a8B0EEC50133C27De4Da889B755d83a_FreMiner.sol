...
contract FreMiner is ReentrancyGuard {
    ...
    // testnet
    address private _freaddr = address(0x54E455C29B0967652A3754555742Ac2772811637);
    address private _fretrade = address(0x5EC6160c636B4a2A5B28513C3B0C2FdDF5D20C7D);
    address private _bnbtradeaddr = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
    address private _wrappedbnbaddr = address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    address private _usdtaddr = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    address private _destroyaddr = address(0x000000000000000000000000000000000000dEaD);
    ...
    function getCurrentLineUsers(uint8 c) public view returns (address[] memory) {
    ...
    function SetParentByAdmin(address user, address parent, uint8 cline) public {
        ...
        if (c != 0) {
            _currentLine[msg.sender] = c;
            _currentLineAddresses[c].push(msg.sender);
        } else {
    ...
    function getPendingCoin(address user) public view returns (uint256) {
        ...
        if (_maxcheckpoint > 0) {
            ...
            if (lastblock < block.number && lastblock > 0) {
                ...
                if (_nowtotalhash > 0) {
                    uint256 get = blockcount.mul(_currentMulitiper).mul(mytotalhash).div(
                        _nowtotalhash
                    );
                    total = total.add(get);
                }
    ...
    function WithDrawCredit() public nonReentrant returns (bool) {
        ...
        if (_userInfos[msg.sender].earnCapped == _userInfos[msg.sender].earned) return true;
        uint256 lAmount = _userInfos[msg.sender].earnCapped - _userInfos[msg.sender].earned;
        ...
        returns (bool)
    {
        ...
        if (_userInfos[msg.sender].earnCapped + getPendingCoin(msg.sender) > amountb * rangeEarned(amountb.mul(1e18).div(getExchangeCountOfOneUsdt(_freaddr)))) {
            _userInfos[msg.sender].earnCapped += getPendingCoin(msg.sender) - amountb * rangeEarned(amountb.mul(1e18).div(getExchangeCountOfOneUsdt(_freaddr)));
        } else {
    ...
    function cLineSort() internal view returns (uint[] memory) {
        uint[] memory h = new uint[](3);
    ...
    function quickSort(uint[] memory arr, uint left, uint right) public pure returns (uint[] memory) {
        uint i = left;
        uint j = right;
        if (i == j) return arr;
