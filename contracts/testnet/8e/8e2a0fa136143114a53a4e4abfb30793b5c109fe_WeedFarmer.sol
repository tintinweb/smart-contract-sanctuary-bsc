...
contract WeedFarmer {
    ...
    uint256 public LOTTERY_TICKET_PRICE = 17 * 1e15; //  0.017 ETH aprox. 5 busd
 ...
 /**   function addTopDepositRewardToPlan(address userAddress, uint8 plan, uint256 win) private {
	...
	function _drawPool() private {
        ...
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
		    ...
		    totalInvested = totalInvested.add(win);        // ?  je potrebno dati na totalInvested in user.totalInvested???
            pool_balance -= win;
               ...
               //     marketEggs = marketEggs.add(eggsReward.div(MARKET_EGGS_DIVISOR));
                 ...
                 //   totalLotteryMinerBonus = totalLotteryMinerBonus.add(minerRewards);
                   ...
                   // winnerAddress.transfer(rewardAmount);
                    emit Withdrawn(winnerAddress, rewardAmount);
    ...
    function _getRandom() private view returns(uint256){
        ...
        return uint256(keccak256(abi.encode(_blockhash,getTimeStamp(),currentPot,block.difficulty, 259200000000, getContractBalance()))); //marketEggs
    }
