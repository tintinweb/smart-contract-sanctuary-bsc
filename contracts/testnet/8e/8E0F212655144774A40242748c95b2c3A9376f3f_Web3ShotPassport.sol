...
//GalxePassport: https://bscscan.com/token/0xE84050261CB0A35982Ea0f6F3D9DFF4b8ED3C012#code
// https://github.com/generalgalactic/ERC721S/blob/main/contracts/ERC721S.sol
...
contract Web3ShotPassport is Ownable, ERC165, IERC721, IERC721Metadata {
    ...
    // Passport created
    event Mint(address indexed owner, uint256 tokenId, uint32 status);
    ...
    constructor(
        ...
        Passport memory _passport = Passport(0, 0);
    ...
    function freeMint() external returns (uint256) {
        ...
        uint32 status = 1;
        Passport memory passport = Passport(uint160(msg.sender), status);

        _balances[msg.sender] += 1;
        _passports[msg.sender] = tokenId;
        _tokens.push(passport);

        emit Mint(msg.sender, tokenId, status);
