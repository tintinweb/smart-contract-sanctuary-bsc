...
interface ISyrupBar {
    ...
    function safeDxbTransfer(address _to, uint256 _amount) external;
}
...
contract MasterChef is Ownable, ReentrancyGuard, Whitelist {
    ...
    // Bonus muliplier for early DXB makers.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. DXB to distribute per block.
        uint256 lastRewardBlock;  // Last block number that DXB distribution occurs.
        uint256 accDxbPerShare;   // Accumulated DXB per share, times 1e12. See below.
    }
    ...
    // The DXB TOKEN!
    IDxbToken public dxbToken;
    // The SYRUP TOKEN!
    ISyrupBar public syrup;

    // DXB tokens created per block.
    uint256 public dxbPerBlock;
    ...
    // The block number when DXB mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 dxbPerBlock);
    ...
    constructor(
        ...
        ISyrupBar _syrup,
        address _devAddress,
        uint256 _dxbPerBlock,
        uint256 _startBlock
    ) public {
        dxbToken = _dxb;
        dxbPerBlock = _dxbPerBlock;
    ...
    // View function to see pending DXB on frontend.
    function pendingDxb(uint256 _pid, address _user) external view returns (uint256) {
        
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDxbPerShare = pool.accDxbPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 dxbReward = multiplier.mul(dxbPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accDxbPerShare = accDxbPerShare.add(dxbReward.mul(1e12).div(lpSupply));
        }
        uint pendingRewards = user.amount.mul(accDxbPerShare).div(1e12).sub(user.rewardDebt);
        if (dxbToken.isMintable(pendingRewards)) {
    ...
    // Update the given pool's DXB allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (dxbToken.isMintable(dxbReward)) {
            dxbToken.mint(address(syrup), dxbReward);
        }
        if (dxbToken.isMintable(dxbReward.div(10))) {
            dxbToken.mint(devAddr, dxbReward.div(10));
        }

        pool.accDxbPerShare = pool.accDxbPerShare.add(dxbReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for DXB allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant nonContract {
        require(dxbToken.isMintable(0), "Farming has been ended.");
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDxbPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeDxbTransfer(msg.sender, pending, _pid);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accDxbPerShare).div(1e12);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accDxbPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                syrup.safeDxbTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Safe DXB transfer function, just in case if rounding error causes pool to not have enough DXB.
    function safeDxbTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        uint256 boost = 0;
        syrup.safeDxbTransfer(_to, _amount);
        if (enableNFTBoost) {
            boost = getBoost(_to, _pid).mul(_amount).div(100);
            if (boost > 0) {
                if (dxbToken.isMintable(boost)) {
                    dxbToken.mint(_to, boost);
                }
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _dxbPerBlock) public onlyOwner {
        massUpdatePools();
        dxbPerBlock = _dxbPerBlock;
        emit UpdateEmissionRate(msg.sender, _dxbPerBlock);
    }
