...
contract Chameleon is Context, IERC20, Ownable, LockToken 
{
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 markettingFee = newBalance.div(allFee).mul(_marketingFee);
        uint256 buyBackFee = newBalance.div(allFee).mul(_BuyBackFee); 
        sendETHToCharity(markettingFee,marketingWallet);
        sendETHToCharity(buyBackFee,BuyBackWallet);
        //marketingWallet.transfer();

        //BuyBackWallet.transfer(newBalance.div(allFee).mul(_BuyBackFee));
    }

    function sendETHToCharity(uint256 amount, address payable wallet) public { 
