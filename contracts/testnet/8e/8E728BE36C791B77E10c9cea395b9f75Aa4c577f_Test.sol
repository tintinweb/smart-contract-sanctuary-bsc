...
contract Test {
    ...
    function callFunction(string memory _coin, uint256 amount) public view returns(uint256 _amount,uint256 _buy_amt)
	{
        if(keccak256(abi.encodePacked(_coin)) == keccak256(abi.encodePacked("BNB")))
        {
           ...
           uint256 buy_amt=(amount.mul(1e18)).div(pairs[_coin].buyPrice);
           return(amount,buy_amt);
        }
        else
        {
