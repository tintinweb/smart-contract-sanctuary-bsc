...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Manager, LpLock {
    ...
    function Dividend() public onlyManager {
        ...
        for (uint i = 0; i < DividendAdds.length; i++) {
            uint256 userBalance = LiquidityWallet.balanceOf(DividendAdds[i]).add(LockBalances[DividendAdds[i]].lockBalance);
...
contract SourceDate {
    ...
    uint256 public DayBurnAmount;
    address public WithdrawalAddr;
    address public MintAddr;
    bool public PairPaused = false;
    uint256 public sellMarketingRates;
    uint256 public buyMarketingRates;
    ...
    mapping(address => UserLocks) public LockBalances;
}
...
import "@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/utils/math/SafeMathUpgradeable.sol";
import "./SourceDate.sol";

contract LpLock is SourceDate {
    using SafeMathUpgradeable for uint256;
    function addLpLock(uint256 amount, uint finishTime) public {
        IERC20Upgradeable LPErc20Pair = IERC20Upgradeable(LiquidityAddr);
        require(LPErc20Pair.balanceOf(msg.sender) >= amount, "lp amount < actionAmount");
        require(LPErc20Pair.allowance(msg.sender, address(this)) >= amount, "lp allowance amount < actionAmount");
        if (LPErc20Pair.transferFrom(msg.sender, address(this), amount)) {
            LockBalances[msg.sender].lockBalance.add(amount);
            LockBalances[msg.sender].finishTime = finishTime;

            DividendAdds.push(msg.sender);
            DividendAddsIndex[msg.sender] = DividendAdds.length;
        }
    }

    function cancelLpLock() public {
        require(LockBalances[msg.sender].lockBalance > 0, "lock lp < 0");
        require(LockBalances[msg.sender].finishTime < block.timestamp, "finishTime < currentTime");
        IERC20Upgradeable LPErc20Pair = IERC20Upgradeable(LiquidityAddr);
        if (LPErc20Pair.transfer(msg.sender, LockBalances[msg.sender].lockBalance)) {
            LockBalances[msg.sender].lockBalance = 0;
            delete DividendAdds[DividendAddsIndex[msg.sender] - 1];
        }
    }

    function getLockBalanceOf(address value) public view returns (uint256) {
        return LockBalances[value].lockBalance;
    }
