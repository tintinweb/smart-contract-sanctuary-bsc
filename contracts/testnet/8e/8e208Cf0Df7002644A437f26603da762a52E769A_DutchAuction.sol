 ...
 */
    ...
    /// @param startPrice The minimum price for the auction
    /// @param endTime Is the percentage to increase between bids
    ...
    ) internal isNFTOwner(nftContractAddress, tokenId, msg.sender)
    {
        ...
        nftContractAuctionTimmings.deadlineTime = (block.timestamp + endTime);
        nftContractAuctions[nftContractAddress][tokenId].aucState == AuctionState.OPEN;
    ...
    {
        uint256 auctionTime = nftContractAuctions[nftContractAddress][tokenId].auctionPeriod = (block.timestamp + endTime);
    ...
    {
        nftContractAuctions[nftContractAddress][tokenId].payedPrice = msg.value;
        _setCurrentPriceBid(
            nftContractAddress,
            tokenId,
            msg.value
    ...
    onlyOwner
{
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint){
        ...
        require(nftContractAuction.aucState == AuctionState.OPEN,"Auction's closed");
        uint decreaseAmount = nftContractAuction.decreaseRate*block.timestamp;
        uint decreasecurrentPrice = nftContractAuction.startPrice - decreaseAmount;

        if(decreasecurrentPrice >= nftContractAuction.minPrice){
            return decreasecurrentPrice;
        }else{
    ...
    function _setupAuction(
        ...
        nftContractAuction.auctionPeriod =  (block.timestamp + endTime);
