...
contract BeinBit is IBeinBit, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade() {
        require(!_upgraded, "BeinBit: Already upgraded");
        _;
        _upgraded = true;
    }
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "BeinBit: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __BeinBit_init_unchained(_router);
    }
    function __BeinBit_init_unchained(address _router) internal initializer {
        _name = "BeinBit";
        _symbol = "$Bebit";
        _decimals = 18;
        ...
        __BeinBit_tiers_init();
        emit Transfer(address(0), _msgSender(), _tTotal);
    }
    function __BeinBit_tiers_init() internal initializer {
    ...
    function whitelistAddress(address _account, uint256 _tierIndex)
        ...
        preventBlacklisted(_account, "BeinBit: Selected account is in blacklist")
    {
        require(_account != address(0), "BeinBit: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner {
        require(_account != address(0), "BeinBit: Invalid address");
        require(_accountsTier[_account] > 0, "BeinBit: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "BeinBit: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_ecoSystem != address(0), "BeinBit: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner checkTierIndex(_tierIndex) {
        require(_owner != address(0), "BeinBit: Address Zero is not allowed");
    ...
    function _approve(
        ...
        private
        preventBlacklisted(owner, "BeinBit: Owner address is blacklisted")
        preventBlacklisted(spender, "BeinBit: Spender address is blacklisted")
    {
    ...
    function _transfer(
        ...
        private
        preventBlacklisted(_msgSender(), "BeinBit: Address is blacklisted")
        preventBlacklisted(from, "BeinBit: From address is blacklisted")
        preventBlacklisted(to, "BeinBit: To address is blacklisted")
        isRouter(_msgSender())
    {
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        ...
        uint256 initialBalance = IBeinBit(address(this)).balanceOf(msg.sender);
        // swap BNB for Tokens
        swapBnbForTokens(half);

        // how much BNB did we just swap into?
        uint256 newBalance = IBeinBit(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);

        _approve(msg.sender, address(this), swapeedToken);
        IBeinBit(address(this)).transferFrom(msg.sender, address(this), swapeedToken);
    ...
    function migrate(address account, uint256 amount)
        external
        override
        preventBlacklisted(account, "BeinBit: Migrated account is blacklisted")
    {
        require(migration != address(0), "BeinBit: Migration is not started");
        require(_msgSender() == migration, "BeinBit: Not Allowed");
    ...
    function withdrawToken(address _token, uint256 _amount) public onlyOwner {
        IBeinBit(_token).transfer(msg.sender, _amount);
    }
