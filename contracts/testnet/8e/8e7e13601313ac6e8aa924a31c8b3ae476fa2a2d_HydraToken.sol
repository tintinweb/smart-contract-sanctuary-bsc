...
contract HydraToken is Context, IERC20, Ownable 
{
    ...
    address payable public marketingAddress = payable(0x9279dbbd11F23b9d9385f0a48E692e12B9EdD7C0);
    ...
    uint256 public _developmentFee = 25; //2.5%
    uint256 private _previousDevelopmentFee = _developmentFee;
    
    uint256 public _marketingFee = 25; //2.5%
    uint256 private _previousMarketingFee = _marketingFee;

    uint256 _saleDevelopmentFee = 75;  // 7.5 developement fee
    uint256 _saleMarketingFee = 75; //7.5% marketing Fee
    ...
    event SwapAndSendFeeToWallets(uint256 tokensSwapped, uint256 ethReceived);
    ...
    constructor () 
    {
        ...
        _isExemptFromTxLimit[developmentAddress] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != uniswapV2Pair) {
            if (overMinimumTokenBalance) 
            {
                ...
                swapAndSendFeeToWallets(contractTokenBalance);
            }
        }
    ...
    function swapAndSendFeeToWallets(uint256 contractTokenBalance) private lockTheSwap 
    {
        uint256 _totalFee = _marketingFee.add(_developmentFee);
        swapTokensForEth(contractTokenBalance); 
        uint256 newBalance = address(this).balance;
        uint256 bnbForMarketing = newBalance.mul(_marketingFee).div(_totalFee);
        uint256 bnbForDevelopement = newBalance.sub(bnbForMarketing);
        marketingAddress.transfer(bnbForMarketing);
        developmentAddress.transfer(bnbForDevelopement);
        emit SwapAndSendFeeToWallets(contractTokenBalance, newBalance);
    }
    ...
    event UpdatedBuyFee(uint256 totalFee, uint256 timestamp);
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee) 
    external onlyOwner()
    {
        _developmentFee = liquidityFee;
        _previousDevelopmentFee = liquidityFee;
        ...
        emit UpdatedBuyFee(totalFee, block.timestamp);
    }
    ...
    external onlyOwner()
    {
        _saleDevelopmentFee = liquidityFee;
    ...
    uint256 public _walletHoldingMaxLimit =  _tTotal.div(1000).mul(2); //0.2%
    ...
    function excludeWalletsFromWhales() private 
    {
        ...
        _isExcludedFromWhale[developmentAddress]=true;
    }
