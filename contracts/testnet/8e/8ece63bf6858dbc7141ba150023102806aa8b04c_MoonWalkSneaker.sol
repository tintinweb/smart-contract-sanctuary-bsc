...
contract MoonWalkSneaker is ERC721, Ownable {
  ...
  function blackListToken(uint256 _id) external onlyOwner{
      _blackListToken(_id);
  }
  ...
  function _blackListToken(uint256 _id) private{
    ...
    function changeMarketingTransferState(bool state) external onlyOwner{
      _shouldSendToMarketing = state;
        ...
        function setCustomTokenAddress(address _address) external onlyOwner{
      customToken = _address;
    }

    function setCustomTokenDecimals(uint256 _decimals) external onlyOwner{
      customTokenDecimals = _decimals;
    }
