...
contract DividendDistributorBSC is IDividendDistributor {
...
contract TokenCutterBSC is Context, IERC20, IERC20Metadata {
    using SafeMath for uint256;
    ...
    constructor(string memory tokenName, string memory tokenSymbol, uint256 initialSupply, address owner, address reflectionToken
                ,address routerAddress, address initialHldAdmin, Fees.allFees memory fees) {    
        _name = tokenName;
        _symbol = tokenSymbol;
        ...
        dividendDistributor = new DividendDistributorBSC(routerAddress, reflectionToken, address(this));
        ...
        totalFeeIfSelling = devFeeOnSell.add(lpFeeOnSell).add(reflectionFeeOnSell).add(revenueFee);         
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 amountEthReflection = amountETH.sub(devBalance).sub(revenueBalance).sub(amountEthLiquidity);
...
contract proofTokenFactoryBSC is Ownable {
    ...
    constructor(address initialRouterAddress, address initialLockerAddress,uint256 initialBurnThreshold) {
    ...
    function createToken(string memory tokenName, string memory tokenSymbol, uint256 initialSupply, address reflectionToken, 
        ...
        TokenCutterBSC newToken = new TokenCutterBSC(tokenName, tokenSymbol, initialSupply, msg.sender, reflectionToken, routerAddress, hldAdmin, fees);
