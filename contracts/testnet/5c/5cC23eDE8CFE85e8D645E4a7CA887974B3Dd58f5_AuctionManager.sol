...
contract Auction {
    ...
    // Auction constructor
    constructor(IERC20 _acceptedToken, address _creator,uint _endTime,uint _minIncrement,uint _directBuyPrice, uint _startPrice,address _nftAddress,uint _tokenId){
    ...
    function placeBid(uint _bid) external {
        ...
        require(_bid > startPrice); // The bid must be higher than the starting price
        require(_bid > maxBid + minIncrement); // The bid must be higher than the current bid + the minimum increment
         //transfer bid to auction ???????
        require(acceptedToken.transferFrom(msg.sender, address(this), _bid), "Failed to place bid");
        ...
        maxBid = _bid; // The new highest bid
        maxBidder = msg.sender; // The address of the new highest bidder
        if(_bid >= directBuyPrice){ // If the bid is higher than the direct buy price
            isDirectBuy = true; // The auction has ended
        }
        bids.push(Bid(msg.sender,_bid)); // Add the new bid to the list of bids

        if(lastHighestBid != 0){ // if there is a bid
            //transfer bid to previous bidder ???????
            require(acceptedToken.transfer(payable(address(uint160(lastHightestBidder))), lastHighestBid), "Failed to transfer bid to previous bidder");
    ...
    function withdrawToken() external returns(bool success){
    ...
    function withdrawFunds() external returns(bool success){ 
        require(getAuctionState() == AuctionState.ENDED || getAuctionState() == AuctionState.DIRECT_BUY); // The auction must be ended by either a direct buy or timeout
        require(msg.sender == creator); // The auction creator can only withdraw the funds
        require(acceptedToken.transfer(payable(address(uint160(creator))), maxBid), "Failed to transfer bid to owner");
    ...
    function cancelAuction() external returns(bool success){ // Cancel the auction
...
contract AuctionManager is ReentrancyGuard {
    ...
    uint256 public LISTING_FEE = 0.0000 ether;
    ...
    function setListingFee(uint256 _listingFee) external {
        require(msg.sender == _marketOwner,"Owner only is allowed to change fee");
        LISTING_FEE = _listingFee;
        emit ChangedListingFee(_listingFee);
    }
    ...
    function createAuction(uint _endTime, uint _minIncrement, uint _directBuyPrice,uint _startPrice,address _nftAddress,uint _tokenId) external returns (bool){
        ...
        require(_endTime > 5 minutes); // end time must be greater than 5 minutes (setting it to 5 minutes for testing you can set it to 1 days or anything you would like)
         ...
         //transfer listing fee to auction owner ???????
         if(LISTING_FEE > 0){
            require(acceptedToken.transferFrom(msg.sender,_marketOwner, LISTING_FEE),"Transferring the listing fee to the Auction creatord failed");
         }

        uint auctionId = _auctionIdCounter; // get the current value of the counter
        _auctionIdCounter++; // increment the counter
        Auction auction = new Auction(acceptedToken, msg.sender, _endTime, _minIncrement, _directBuyPrice, _startPrice, _nftAddress, _tokenId); // create the auction
    ...
    {
        allAuctions = new Auction[](_auctionsList.length); 
