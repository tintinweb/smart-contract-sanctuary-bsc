...
contract BSG is Ownable{
    ...
    uint256 public constant dayPerCycle = 25 minutes;

    // uint256 private constant timeStep = 1 minutes;
    // uint256 public constant dayPerCycle = 5 minutes;

    uint256 private constant dayReward2Percents = 540000000000000000000;
    uint256 private constant dayRewardPercents = 440000000000000000000;
    ...
    function checkDeposit(address _user) public view returns(uint256) {
        UserInfo storage user = userInfo[_user]; 
        uint256 total = (user.totalDeposit).mul(maxiWithdraw);
        return total;
    }
    ...
    function withdraw() external {
        ...
        // uint256 cycleId = getUserCycleId(msg.sender);
        ...
        ////////////////////////////////////////////////////////////////////
        uint256 total = checkDeposit(msg.sender);

        if(total<= withdrawable){
            withdrawable = total;
        }
        else{
            withdrawable = total.sub(userRewards.totalWithdrawls);
        }
        ///////////////////////////////////////////////////////////////////
        ...
        userRewards.totalWithdrawls = (userRewards.totalWithdrawls).add(withdrawable);
    ...
    // function getUserCycleId (address _user) public view returns(uint256){
    //     return userCycle[_user];
    // }
    ...
    function gett(address user_) public view returns(uint256){
        uint256 index = getOrderLength(user_).sub(1);
        OrderInfo storage order = orderInfos[user_][index];
        return order.unfreeze;
    }
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 200e18){
            ...
            if(total >= 250e18 && user.teamNum >= 3 && maxTeam >= 300e18 && otherTeam >= 300e18){
                levelNow = 5;
            }else if(user.teamNum >= 2 && maxTeam >= 200e18 && otherTeam >= 200e18){
                levelNow = 4;
            }else{
                levelNow = 3;
            }
    ...
    mapping (address => uint256) public maxRewardTime;
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        // uint256 previousTime = maxRewardTime[_user];
        // if(getCurDay() > previousTime.add(dayPerCycle)){
        //     userCycle[msg.sender] = userCycle[msg.sender].add(1);
        // }
