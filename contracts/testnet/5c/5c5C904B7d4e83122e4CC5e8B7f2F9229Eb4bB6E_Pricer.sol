...
contract Pricer {
    ...
    */
    ...
    function getLpShare(uint amount, address _pair) external view returns(uint lpShare_){ // 12 decimals precision
        require(amount > basis, "not enough LP");
        IUniswapV2Pair pair = IUniswapV2Pair(_pair); // define pair

        uint lpSupply = pair.totalSupply(); // get pair total supply of LP tokens
        uint lpShare = (basis).mul(((amount).mul(1e12)).div(lpSupply)).div(basis); // get the lp share for amountLp input / % in basis
        lpShare_ = lpShare;
        if (lpShare == 0){
            revert();
        }
