...
contract Escrow {
 ...
 mapping(uint256 => Payment) private payments;
  ...
  constructor(IERC20 _tokenAddress, address _routerAddress) {
     owner  =  payable(msg.sender);
     StriptoToken  = _tokenAddress; // Strip token address
  ...
  function depositPayment(uint256 _itemId, uint256 _amount) onlyOwner external returns (uint256) {
         address[] memory path = new address[](2);
        path[0] = address(StriptoToken);
        path[1] = uniswapV2Router.WETH();

        StriptoToken.approve(address(uniswapV2Router), _amount);
       // make the swap
        uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
          ...
          payments[paymentId] = Payment(address(msg.sender), _amount,_itemId,block.timestamp,PaymentStatus.Pending);
