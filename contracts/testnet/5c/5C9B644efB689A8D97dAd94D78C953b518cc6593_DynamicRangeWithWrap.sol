...
contract DynamicRangeWithWrap is BaseWithWrap {
    ...
    function deposit(
        ...
        IiZiSwapLiquidityManager.MintParam
            memory mintParam = iZiSwapCallingParams.mintParams(
                rewardPool.tokenX, rewardPool.tokenY, rewardPool.fee, amountXLim, amountYLim, leftPoint, rightPoint, type(uint256).max
            );
        ...
        if (tokenXIsETH) {
            if (actualAmountX < msg.value) {
                _safeTransferETH(msg.sender, msg.value - actualAmountX);
            }
        } else {
            if (actualAmountX < amountXLim) {
                _transferTokenToUser(rewardPool.tokenX, rewardPool.tokenXIsWrap, msg.sender, amountXLim - actualAmountX);
            }
        }

        if (tokenYIsETH) {
            if (actualAmountY < msg.value) {
                _safeTransferETH(msg.sender, msg.value - actualAmountY);
            }
        } else {
            if (actualAmountY < amountYLim) {
                _transferTokenToUser(rewardPool.tokenY, rewardPool.tokenYIsWrap, msg.sender, amountYLim - actualAmountY);
            }
        }
        ...
        if (newTokenStatus.nIZI > 0) {
            ...
            _recvTokenFromUser(address(iziToken), false, newTokenStatus.nIZI);
        }
    ...
    function withdraw(uint256 tokenId, bool noReward) external nonReentrant {
        ...
        _transferTokenToUser(rewardPool.tokenX, rewardPool.tokenXIsWrap, msg.sender, refundAmountX);
        _transferTokenToUser(rewardPool.tokenY, rewardPool.tokenYIsWrap, msg.sender, refundAmountY);
        ...
        _transferTokenToUser(rewardPool.tokenX, rewardPool.tokenXIsWrap, msg.sender, amountX);
        _transferTokenToUser(rewardPool.tokenY, rewardPool.tokenYIsWrap, msg.sender, amountY);
...
abstract contract BaseWithWrap is Ownable, Multicall, ReentrancyGuard {
