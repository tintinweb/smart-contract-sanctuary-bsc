    ...
    IERC721ReceiverUpgradeable
{
    ...
    event ChangeDefaultTimeToReducePrice(uint32 defaultTimeToReducePrice);
    ...
    modifier doesBidMeetBidRequirements(
        ...
        require(payedPrice >= getCurrentPrice(nftContractAddress, tokenId) &&
        getCurrentPrice(nftContractAddress, tokenId) >= nftContractAuctions[nftContractAddress][tokenId].minPrice,
    ...
    function initialize(
        ...
        uint32 defaultTimeToReducePrice_,
        ...
        isPercentageInBounds(defaultTimeToReducePrice_)
    {
        __Ownable_init();
        ...
        defaultTimeToReducePrice = defaultTimeToReducePrice_;
        initialized = true;
    }
    ...
    {
        require(nftContractAuctions[nftContractAddress][tokenId].ERC20Token != address(0), "Only ERC20 Payment");
    ...
    {
        require(nftContractAuctions[nftContractAddress][tokenId].ERC20Token == address(0), "Only BNB Payment");
    ...
    /// Change the default Time to reduce Price
    /// @param newDefaultTimeToReducePrice New default Time to reduce
    function changeDefaultTimeToReducePrice(uint32 newDefaultTimeToReducePrice)
    external
        onlyOwner
        isPercentageInBounds(newDefaultTimeToReducePrice)
    {
        defaultTimeToReducePrice = newDefaultTimeToReducePrice;
        emit ChangeDefaultTimeToReducePrice(defaultTimeToReducePrice);
    }
    ...
    /// Get funtion to get decreased Price
    /// @param nftContractAddress Contract address that will be put on auction
    /// @param tokenId Identifier of the NFT
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        ...
        AuctionDecreasePeriod memory auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        uint256 decreasesteps = (nftContractAuctionTimmings.deadlineTime - block.timestamp)/auctionPeriod.decreasePeriod;
        uint256 amountToDecrease = (nftContractAuction.startPrice - nftContractAuction.minPrice) / decreasesteps;
    ...
    /// Internal function for updating the bids data
    /// @param nftContractAddress Contract address auctioned
    /// @param tokenId Identifier of the NFT
    /// @param payedPrice Amount of tokens for the bid
    function _setPriceBid(
        address nftContractAddress,
        uint256 tokenId,
        uint256 payedPrice
    ) internal {
        Auction memory auction = nftContractAuctions[nftContractAddress][
            tokenId
        ];
