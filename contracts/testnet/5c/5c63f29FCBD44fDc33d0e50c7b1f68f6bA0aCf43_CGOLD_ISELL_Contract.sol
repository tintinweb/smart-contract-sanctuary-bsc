...
contract CGOLD_ISELL_Contract is ReentrancyGuard {
	...
	uint256 public minimumBuyAmount = DECIMAL_18 / 100; //0.01 BNB
	uint256 public maximumBuyAmount = DECIMAL_18 / 10; //0.1 BNB
	uint256 public maximumBuyToken;
	...
	constructor(address _idoToken) public {
		...
		tokenRate = 54500000000;
		//f1_rate   = PERCENTS_DIVIDER/6;
		f1_rate = 0;
		if (tokenRate > 0)
		{
			maximumBuyToken = maximumBuyAmount * DECIMAL_18 / tokenRate;
		}
	}
   ...
   */
	function updateAmoi(uint256 _amountClaimAirDrop) public onlyIDOAdmin {
   ...
   */
	function updateRate(uint256 rate) public onlyIDOAdmin {
		tokenRate = rate;
		if (tokenRate > 0 && maximumBuyAmount > 0)
		{
			maximumBuyToken = maximumBuyAmount * DECIMAL_18 / tokenRate;
		}
	}
   ...
   */
	...
	function AirDrop(address _referrer) public payable ifNotPaused returns (uint256) {
		...
		updateAddLock(msg.sender, amountClaimAirDrop);
