...
contract HBEP20Factory is IBEP20Factory {
    function createToken(string memory name, string memory symbol, uint8 decimals) external override returns (address token){
        bytes memory bytecode = type(HBEP20Token).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token));
        assembly {
            token := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        HBEP20Token(token).initialize(msg.sender, name, symbol, decimals);
