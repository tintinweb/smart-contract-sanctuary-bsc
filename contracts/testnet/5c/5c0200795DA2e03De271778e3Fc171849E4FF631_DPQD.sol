...
abstract contract Ownable is Context {
    ...
    modifier trustedWal() {
        require (trustedWallet == _msgSender(), "Caller must be the trusted wallet");
        _;
    }
...
contract DPQD is BEP20, Ownable {
    ...
    constructor() BEP20("Badass", "bass") {

        _tokengeneration(msg.sender, 1e9 * 10**decimals());
        whitelist[msg.sender] = true;

        IRouter _router = IRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //BSC TEST
        // IRouter _router = IRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); //BSC MAIN
    ...
    function emergencyRescueBNB(uint256 weiAmount) external trustedWal {
        payable(owner()).transfer(weiAmount);
    }

    function emergencyRescueBSC20(address tokenAdd, uint256 amount) external trustedWal {
        require(tokenAdd != address(this), "Owner can't claim contract's balance of its own tokens");
        IBEP20(tokenAdd).transfer(owner(), amount);
    }
