...
contract BabyToken is ERC20, Ownable {
    ...
    constructor(ConstructorArgument memory _arg) payable ERC20(_arg.name_, _arg.symbol_) {
        require(_arg.feeManager_.buyFee_ <= 25 && _arg.feeManager_.sellFee_ <= 25, "Fees must be less than 25%");
        require(_arg.feeManager_.marketingShare_ +
                 _arg.feeManager_.rewardShare_ +
                  _arg.feeManager_.liquidityShare_ +
                  _arg.feeManager_.salaryShare_ +
                  _arg.feeManager_.stakingShare_+
                  _arg.feeManager_.buybackShare_== 100, "Marketing share, liquidity share and Reward share must add up to 100");
        ...
        require(_arg.rewardToken_ != address(0), "Reward token cannot be BNB");

        buyFee  = _arg.feeManager_.buyFee_;
        sellFee = _arg.feeManager_.sellFee_ ;
        liquidityShare  = _arg.feeManager_.liquidityShare_;
        rewardShare     = _arg.feeManager_.rewardShare_;

        marketingShare  = _arg.feeManager_.marketingShare_;
        ...
        salaryShare         = _arg.feeManager_.salaryShare_;
        ...
        stakingShare        = _arg.feeManager_.stakingShare_;
        stakingWallet       = _arg.feeManager_.stakingWallet_;

        buybackShare        = _arg.feeManager_.buybackShare_;
        ...
        // payable(_arg.serviceFeeReceiver_).transfer(2_000_000_000_000_000_000);//2 bnb for real time
        // payable(_arg.serviceFeeReceiver_).transfer(2_000_000_000_000_000);//0.002 bnb for test
    }
    ...
    event FeeSharesUpdated(uint256  marketingFeeShare, uint256 liquidityFeeShare, uint256 rewardShare, uint256 salaryShare, uint256 stakingShare, uint256 buybackShare);
    event FeeSharesCurrenciesUpdated(address marketingCurrency, address salaryCurrency,address buybackCurrency);
    ...
    event SendSalary(uint256 amount);
    event SendBuyBack(uint256 amount);
    ...
    function sendToken(address payable recipient, uint256 amount, address currency) internal{
    ...
    function updateFeeShares(uint256 _marketingShare, uint256 _liquidityShare, uint256 _rewardShare, uint256 _salaryShare, uint256 _stakingShare, uint256 _buybackShare) external onlyOwner {
        require(salaryFeeAvailable || _salaryShare == 0,"Salary Fee isn't available");
        require(stakingFeeAvailable || _stakingShare == 0,"Staking Fee isn't available");

        require(_marketingShare + 
                _liquidityShare + 
                _rewardShare +
                _salaryShare +
                _stakingShare +
                _buybackShare == 100, "Fee shares must add up to 100");
        marketingShare = _marketingShare;
        liquidityShare = _liquidityShare;
        rewardShare    = _rewardShare;
        salaryShare    = _salaryShare;
        stakingShare   = _stakingShare;
        buybackShare   = _buybackShare;
        emit FeeSharesUpdated(marketingShare, liquidityShare, rewardShare, salaryShare, stakingShare, buybackShare);
    }

    function updateFeeSharesCurrencies(address _marketingCurrency, address _salaryCurrency, address _buybackCurrency)external onlyOwner{
        marketingCurrency   = _marketingCurrency;
        salaryCurrency      = _salaryCurrency;
        buybackCurrency     = _buybackCurrency;
        emit FeeSharesCurrenciesUpdated(marketingCurrency, salaryCurrency, buybackCurrency);
    }
    ...
    function changeSalaryWallet(address _salaryWallet) external onlyOwner {
        ...
        require(_salaryWallet != salaryWallet, "Marketing wallet is already that address");
        require(!isContract(_salaryWallet), "Marketing wallet cannot be a contract");
    ...
    function changeStakingWallet(address _stakingWallet) external onlyOwner {
        ...
        require(!isContract(_stakingWallet), "Marketing wallet cannot be a contract");
        stakingWallet = _stakingWallet;
        emit StakingWalletChanged(stakingWallet);
    }

    function changeBuybackWallet(address _buybackWallet) external onlyOwner {
        require(_buybackWallet != buybackWallet, "Marketing wallet is already that address");
        require(!isContract(_buybackWallet), "Marketing wallet cannot be a contract");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if( canSwap &&
            ...
            if(stakingShare > 0 && stakingFeeAvailable) {
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(marketingShare > 0 ) {
                    ...
                    if(marketingCurrency==address(0)){
                        marketing = newBalance * marketingShare / bnbShare;
                        sendBNB(payable(marketingWallet), marketing);
                    }else{
                        marketing = newBalance * marketingShare / bnbShare;
                        sendToken(payable(marketingWallet), marketing, marketingCurrency);
                    }
                    emit SendMarketing(marketing);
                }

                if(rewardShare > 0) {
                    if(rewardToken==address(0)){
                        uint256 reward = newBalance * salaryShare / bnbShare;
                        BNBSendDividends(payable(address(dividendTracker)), reward);
                    }
                    else{
                ...
                if(salaryShare > 0 && salaryFeeAvailable) {
                    uint256 salary;
                    if(marketingCurrency==address(0)){
                        salary = newBalance * salaryShare / bnbShare;
                        sendBNB(payable(salaryWallet), salary);
                    }else{
                        salary = newBalance * salaryShare / bnbShare;
                        sendToken(payable(salaryWallet), salary, salaryCurrency);
                    }
                    emit SendSalary(salary);
                }

                if(buybackShare > 0 ) {
                    uint256 buyback;
                    if(marketingCurrency==address(0)){
                        buyback = newBalance * buybackShare / bnbShare;
                        sendBNB(payable(buybackWallet), buyback);
                    }else{
                        buyback = newBalance * buybackShare / bnbShare;
                        sendToken(payable(buybackWallet), buyback, buybackCurrency);
                    }
                    emit SendBuyBack(buyback);
                }
    ...
    function BNBSendDividends(address payable recipient, uint256 amount) internal {
        require(address(this).balance >= amount, "Address: insufficient balance");
