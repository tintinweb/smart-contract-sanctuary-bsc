...
contract FarmBooster is Ownable {
    ...
    /// @notice The boost user mapping(proxy => user).
    ...
    constructor(
        ...
        require(_max > 0 && _cA > 0 && _cB > 0, "constructor:Invalid parameter");
        CAKE = _cake;
        CAKE_POOL = _cakePool;
        MASTER_CHEF = _v2;
        MAX_BOOST_POOL = _max;
    ...
    modifier onlyFactory() {
        require(msg.sender == BOOSTER_FACTORY, "Operations: Not factory");
        _;
    }

    /// @notice Checks if the msg.sender is the FarmBooster Proxy.
    modifier onlyProxy(address _user) {
        require(msg.sender == ProxyContract[_user], "Operations: Not proxy");
        _;
    }

    /// @notice Checks if the msg.sender is the cake pool.
    modifier onlyCakePool() {
        require(msg.sender == CAKE_POOL, "Operations: Not cake pool");
        _;
    }

    /// @param _max Maximum allowed boosted pool numbers.
    function setMaxBoostPool(uint256 _max) external onlyOwner {
        require(MAX_BOOST_POOL > 0, "setMaxBoostPool: Maximum boost pool should greater than 0");
        MAX_BOOST_POOL = _max;
        emit UpdateMaxBoostPool(_max);
    }

    /// @param _factory boost factory contract.
    ...
    function setProxy(address _user, address _proxy) external onlyFactory {
        require(_proxy != address(0), "setBoostProxy: Invalid proxy address");
        ProxyContract[_user] = _proxy;
        ProxyUser[_proxy] = _user;
    ...
    function setCA(uint256 _cA) external onlyOwner {
        require(_cA > 0, "setCA: cA should greater than 0");
        uint256 temp = cA;
        cA = _cA;
    ...
    function setCB(uint256 _cB) external onlyOwner {
        require(_cB > 0, "setCB: cB should greater than 0");
        uint256 temp = cB;
        cB = _cB;
        emit UpdateCB(temp, cB);
    }

    /// @notice Cakepool operation automatically call this function
    ...
    function onCakePoolUpdate(
        ...
        uint256 avgDuration;
        for (uint256 i = itmap.iterateStart(); itmap.iterateValid(i); i = itmap.iterateNext(i)) {
            (uint256 pid, ) = itmap.iterateGet(i);
            // only call once
            avgDuration = avgDuration > 0 ? avgDuration : _avgLockedDuration();
            // ignore none boosted farm pool
            if (!itmap.contains(pid)) continue;
            _updateBoostMultiplier(_user, ProxyContract[_user], pid, avgDuration);
        }
    ...
    function updatePoolBoostMultiplier(address _user, uint256 _pid) public onlyProxy(_user) {
        _updateBoostMultiplier(_user, msg.sender, _pid, _avgLockedDuration());
    }
    ...
    function activate(uint256 _pid) external {
        ...
        require(itmap.size < MAX_BOOST_POOL, "activate: Boosted farms reach to MAX");

        _updateBoostMultiplier(msg.sender, proxy, _pid, _avgLockedDuration());
    ...
    function deactive(uint256 _pid) external {
        ...
        if (itmap.get(_pid) > BOOST_PRECISION) {
    ...
    /// @notice Any user can refesh sepecific user boost multiplier
    /// @param _user user address.
    /// @param _pid pool id(MasterchefV2 pool).
    function refresh(address _user, uint256 _pid) external {
        address proxy = ProxyContract[_user];
        ItMap storage itmap = userInfo[proxy];
        require(itmap.contains(_pid), "refresh: None boost user");

        _updateBoostMultiplier(_user, proxy, _pid, _avgLockedDuration());

        emit Refresh(_user, proxy, _pid);
    }

    /// @notice Whether user active specific booster farm pool
    /// @param _user user address.
    /// @param _pid pool id(MasterchefV2 pool).
    ...
    /// @param _user user address.
    function activedPools(address _user) external view returns (uint256[] memory ret) {
        ItMap storage itmap = userInfo[ProxyContract[_user]];
        if (itmap.size == 0) return ret;

        ret = new uint256[](itmap.size);
        // solidity for-loop not support multiple variables initializae by ',' separate.
        uint256 i;
        for (uint256 index = itmap.iterateStart(); itmap.iterateValid(index); index = itmap.iterateNext(index)) {
            (uint256 pid, ) = itmap.iterateGet(index);
            ret[i] = pid;
            i++;
        }
    }
    ...
    function _boostCalculate(
        ...
        if (lpBalance == 0) return BOOST_PRECISION;

        (, , , , uint256 lockStartTime, uint256 lockEndTime, , , uint256 userLockedAmount) = ICakePool(CAKE_POOL)
            .userInfo(_user);
        if (userLockedAmount == 0) return BOOST_PRECISION;

        uint256 totalLockedAmount = ICakePool(CAKE_POOL).totalLockedAmount();

        uint256 dB = (cA * lpBalance) / CA_PRECISION;
        // todo if dB = 0 => division by zero
        if (dB == 0) return BOOST_PRECISION;

        IBEP20 lp = IBEP20(IMasterChefV2(MASTER_CHEF).lpToken(_pid));
        // todo if totalLockedAmount = 0 || avgDuration = 0, division by zero
        uint256 aB = ((lp.balanceOf(MASTER_CHEF) * userLockedAmount * (lockEndTime - lockStartTime)) / cB) /
            (totalLockedAmount * _duration);
        // todo if lpBalance = 0, division by zero
        return (lpBalance < (dB + aB) ? lpBalance : (dB + aB)) / dB;
    }

    /// @notice calculate cake pool average locked duration
    function _avgLockedDuration() internal view returns (uint256) {
        uint256 totalStakedCakeAmount = IBEP20(CAKE).balanceOf(CAKE_POOL);

        uint256 totalLockedAmount = ICakePool(CAKE_POOL).totalLockedAmount();

        uint256 pricePerFullShare = ICakePool(CAKE_POOL).getPricePerFullShare();

        // todo if pricePerFullShare = 0, division by zero
        uint256 flexibleCakeShares = (totalStakedCakeAmount - totalLockedAmount) / pricePerFullShare;

        uint256 fixedTermCakeOriginalShares = totalLockedAmount / pricePerFullShare;
        // todo if totalLockedAmount = 0 => fixedTermCakeOriginalShares = 0, division by zero
        uint256 avgBoostRatio = (ICakePool(CAKE_POOL).totalShares() - flexibleCakeShares) / fixedTermCakeOriginalShares;

        // todo avgBoostRatio must be > 1 ?
        // if avgBoostRatio = 1, return value =  0, division by zero
        return ((avgBoostRatio - 1) * 365) / 20;
    }
}
