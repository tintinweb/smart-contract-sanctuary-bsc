...
contract Dalchand {
    ...
    function registration(address refAddress)external payable{
        require(msg.value==LEVEL_PRICE[1],"Invalid amount");
        regUser(refAddress);
    }
   
    
       function regUser(address refAddress) internal{
           address payable userAddress = msg.sender;

        require(users[refAddress].isExist, 'Incorrect referrer Id');
        require(!users[userAddress].isExist, 'Incorrect referrer Id');
