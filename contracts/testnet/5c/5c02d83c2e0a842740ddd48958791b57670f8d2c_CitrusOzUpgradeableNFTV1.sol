...
/**
 * @notice Citrus mint contract for Cactus NFT marketplace
 * @dev This contract is upgradeable
 * @dev Initializable needs to be the first to be derived from 
 * @dev currently supports ERC721 token standard
 * @dev todo: support other tokens like ERC1155? hard to add later?
 */
contract CitrusOzUpgradeableNFTV1 is    Initializable,
                                    ERC721Upgradeable, 
                                    OwnableUpgradeable, 
    ...
    event CitrusOzUpgradeableNFTContractDeployed( 
    ...
    /// @dev Always add new variables only at end of the layout if this contract is upgradeable
    ...
    mapping(uint256 => string) private citrus_tokenIdTodata;
    ...
    /// @dev  Don't use constructor if contract is upgradeable. Instead use initializer
    ...
    */
    ...
    ) public initializer 
    {
        require(
            ...
            "platformFeeRecipient is zero address"
        );
        require( 
            platformFeePercent >=0, 
            "platformFeePercent should be >=0"
        );
        _platformFeeRecipient = platformFeeRecipient;
        _platformFeePercent = platformFeePercent;

        emit CitrusOzUpgradeableNFTContractDeployed(
    ...
    /**
     @notice Mints/Creates a new NFT and returns it Id
     @param data UUID of the item
     @param to address of receiver of new NFT minted 
     @param royaltyRecipient address of the party recieving royalty on secondary sale
     @param royaltyValue percentage of royalty for royaltyRecipient 
     */
    ...
    )   external returns (uint256)
    {
        ...
        /// @dev todo: check use of safeMint instead of mint
        _mint(to, newItemId);
        citrus_tokenIdTodata[newItemId] = data;
        if (royaltyValue > 0) {
    ...
    ///////////////////////////////////////
    /// Contract owner/admin methods    ///
    /// @dev for later if fee required  ///
    ///////////////////////////////////////

    /**
     @notice for updating platform fee
     @dev Only admin
     @param platformFeePercent uint256 the platform fee to set
     */
    function updatePlatformFeePercent(uint256 platformFeePercent) external onlyOwner {
        _platformFeePercent = platformFeePercent;
        emit PlatformFeePercentUpdated(platformFeePercent);
    }

    /**
     @notice for updating platform fee address
     @dev Only admin
     @param platformFeeRecipient payable address the address to sends the funds to
     */
    function updatePlatformFeeRecipient(address payable platformFeeRecipient) external onlyOwner {
        _platformFeeRecipient = platformFeeRecipient;
