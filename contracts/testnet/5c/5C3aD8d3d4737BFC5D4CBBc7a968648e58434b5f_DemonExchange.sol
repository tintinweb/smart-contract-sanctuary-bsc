...
import "./utils/SafeMath.sol"; // bingo
import "./Member.sol"; // done
import "./Card.sol"; // done
import "./interface/IERC20.sol"; // done
import "./shop/VerifySignature.sol";

contract DemonExchange is Member {
    ...
    // 删除订单
    event CancelOrder(bytes32 orderid, uint256 sellCont, uint256 logeType); //6
    // 交易订单
    event TradeOrder(
        bytes32 sellOrderId,
        bytes32 orderid,
    ...
    struct tradeOrdersMsg {
        uint256[] cardid; //卡片id
        uint256 price; //单价
        uint256 tradeTime; //交易时间
        address maker; //制定者
        address payToken; //支付币
        address taker;
    }

    // mapping(uint256 => KKOrder) makerOrders;  //创建单
    mapping(bytes32 => orderMsg) sellOrders; //创建单
    orderMsg[] allSellOrder;
    mapping(bytes32 => tradeOrdersMsg) tradeOrders; //交易单
    tradeOrdersMsg[] allTradeOrder;
    mapping(address => mapping(address => uint256)) userBalances; //用户余额
    uint256 private fees; //手续费

    function setFees(uint256 _fees) external ContractOwnerOnly {
        fees = _fees;
    }
    ...
    function createOrder(
        ...
        // 判断token是否接受
        (, uint256[] memory cardidList) = VerifySignature(
            manager.members("verify")
        ).creatOrder(msg.sender, data);

        uint256 length = cardidList.length;
        require(length > 0, "cardId error");

        bytes32 orderid = keccak256(
            abi.encode(cardidList[0], msg.sender, block.number)
        );
        ...
        orderMsg storage newOrder = sellOrders[orderid];
        newOrder.maker = msg.sender;
        newOrder.price = price;
        newOrder.payToken = token;
        newOrder.createTime = block.timestamp;
        newOrder.cardid = cardidList;
        newOrder.isUsed = true;
        allSellOrder.push(newOrder);
    }

    function takeOrder(bytes32 sellOrderId, uint256 buyCont) external {
        // 赋值
        orderMsg storage order = sellOrders[sellOrderId];
        // 判断有无创建单
        require(order.isUsed, "Not Exists Order!");
        require(order.maker != msg.sender, "buy user error");
        require(
            order.sellCont + buyCont <= order.cardid.length,
            "Error buyCont"
        );

        uint256 payAmount = order.price.mul(buyCont);
        // 千三的手续费
        uint256 tradeFees = payAmount.mul(fees).div(1000);
        // 给财务转手续费
        IERC20(order.payToken).transferFrom(
            msg.sender,
            manager.members("funder"),
            tradeFees
        );
        // 除去手续费
        ...
        // 发布者在合约的存款增加
        userBalances[order.maker][order.payToken] += sendAmount;
        // 压缩交易信息
        bytes32 orderid = keccak256(
            abi.encode(
                order.cardid[0],
        ...
        tradeOrdersMsg storage fOrder = tradeOrders[orderid];
        fOrder.price = order.price;

        // 卡片转给你
        for (uint256 i = order.sellCont; i != buyCont; i++) {
            Card(manager.members("card")).transferFrom(
                address(this),
                msg.sender,
                order.cardid[i]
            ...
            fOrder.cardid.push(order.cardid[i]);
        }

        order.sellCont += buyCont;

        fOrder.maker = order.maker;
        fOrder.payToken = order.payToken;
        fOrder.taker = msg.sender;
        fOrder.tradeTime = block.timestamp;
        allTradeOrder.push(fOrder);
        if (order.sellCont == order.cardid.length) {
    ...
    function cancelOrder(bytes32 orderid) external {
        // copy
        orderMsg memory order = sellOrders[orderid];
        // 验证是否是自己发布的
        require(order.maker == msg.sender, "invalid card");
        // 还我卡
        for (uint256 i = order.sellCont; i < order.cardid.length; i++) {
            Card(manager.members("card")).transferFrom(
                address(this),
                msg.sender,
                order.cardid[i]
        ...
        emit CancelOrder(orderid, order.sellCont, 6);
    }

    // 撤销  取钱
    function withdraw(address token) external {
        // 验证
        require(userBalances[msg.sender][token] > 0, "not enough balance");
        // 还钱
        IERC20(token).transfer(msg.sender, userBalances[msg.sender][token]);
        // 重置
        userBalances[msg.sender][token] = 0;
    }

    // 查询存款
    function calcWithdraw(address user, address token)
        external
        view
        returns (uint256)
    {
    ...
    function getAllOrder() public view returns (orderMsg[] memory) {
        return allSellOrder;
    }

    function getAllTradeOrder() public view returns (tradeOrdersMsg[] memory) {
        return allTradeOrder;
    }

    function getSellOrderInfo(bytes32 orderid)
        public
        view
        returns (orderMsg memory)
    {
        return sellOrders[orderid];
    }

    function getTradeOrderInfo(bytes32 orderid)
        public
        view
        returns (tradeOrdersMsg memory)
    {
...
import "./interface/IERC20.sol"; // bingo
import "./lib/String.sol"; //bingo
import "./lib/Util.sol"; //about
import "./ERC721Ex.sol"; //about
...
contract Card is ERC721Ex {
    ...
    event Mint(address, address, uint256, uint256 logeType);
    event SyntherisCard(address, address,  uint256, uint256,uint256,uint256); /// 1 3
    event SyntherisCardBurn(address, address, uint256, uint256,uint256,uint256);/// 2 3


    // 包
    mapping(address => bool) public packages;
    mapping(address => bool) public syntherisAddress;
    mapping(address => bool) public miningAddress;
    ...
    function setSyntheris(address package, bool enable)
        external
        CheckPermit("Config")
    {
        syntherisAddress[package] = enable;
    }

    function setMining(address package, bool enable)
        external
        CheckPermit("Config")
    {
        miningAddress[package] = enable;
    }
    ...
    function mint(address to, uint256 cardIdPre) external {
        ...
        // 集成的721铸造方法
        _mint(to, cardId);
        emit Mint(address(0), to, cardId, 2);
    }
    ...
    function syntherisCard(
        address to,
        uint256[] memory cardsOld,
        uint256 conts,
        uint256 sLogeType
    ) external {
        // 验证我的包
        require(syntherisAddress[msg.sender], "syntheris only");

        for (uint256 i = 0; i < cardsOld.length; i++) {
            _burn(cardsOld[i]);
            emit SyntherisCardBurn(to, address(0),cardsOld[i], sLogeType,2,3);
        }
        for (uint256 i = 0; i != conts; ++i) {
            uint256 cardIdPres = (uint256(uint32(cardsOld[i])) << 224);
            // 利用位数拼接生成卡片id
            uint256 cardId = NFT_SIGN_BIT |
                (cardIdPres & ID_PREFIX_MASK) |
                (block.timestamp << 64) |
                uint64(totalSupply + i);
            // 集成的721铸造方法
            _mint(to, cardId);
            emit SyntherisCard(address(0), to, cardId, sLogeType,1,3);
        }
    }

    function miningBurnCard(uint256 cardsOld) external {
        // 验证我的包
        require(miningAddress[msg.sender], "Mining only");
        _burn(cardsOld);
    }
