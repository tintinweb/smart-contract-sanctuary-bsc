...
contract MARKETPLACE {
    ...
    constructor(address busd_addr) {
        tokenBUSD = BUSD(busd_addr);
    }

    mapping(uint256 => Listing) ListItems;
    mapping(uint256 => Listing) TokenList;
    ...
    function listItem(
        ...
        if (TokenList[tokenId].owner == address(0)) {
            ...
            ListItems[TokenList[tokenId].listId].price = price;
            ListItems[TokenList[tokenId].listId].owner = msg.sender;
            ListItems[TokenList[tokenId].listId].sold = false;

            TokenList[tokenId].price = price;
            TokenList[tokenId].owner = msg.sender;
            TokenList[tokenId].sold = false;
        }
    }
    ...
    function buy(address nftAddress, uint256 tokenId) external {
        require(
            tokenBUSD.balanceOf(msg.sender) >= TokenList[tokenId].price,
            "Not enough busd"
        ); //checks that enough busd
        require(
            tokenBUSD.transferFrom(
                msg.sender,
                TokenList[tokenId].owner,
        ...
        ListItems[TokenList[tokenId].listId].owner = msg.sender;
        ListItems[TokenList[tokenId].listId].sold = true;

        TokenList[tokenId].owner = msg.sender;
        TokenList[tokenId].sold = true;
    }
