    ...
    function migrateDistributorStates(
        ...
        require(
            distributorState.holders.length ==
                ...
                distributorState.claims.length ==
                distributorState.amounts.length &&
                distributorState.amounts.length ==
                distributorState.totalExcluded.length &&
                distributorState.totalExcluded.length ==
                distributorState.totalRealised.length,
            "Unequal length"
        );

        for (uint256 i = 0; i < distributorState.holders.length; i++) {
            shareholders[i] = distributorState.holders[i];
            shareholderIndexes[distributorState.holders[i]] = distributorState
                .indexes[i];
            shares[distributorState.holders[i]] = Share(
                distributorState.amounts[i],
                distributorState.totalExcluded[i],
                distributorState.totalRealised[i]
            );
            shareholderClaims[distributorState.holders[i]] = distributorState
                .claims[i];
        }
