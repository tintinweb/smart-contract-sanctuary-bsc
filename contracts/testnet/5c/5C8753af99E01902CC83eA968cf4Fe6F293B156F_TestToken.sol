...
contract TestToken is ERC20, Ownable, Distributor {
    using Address for address payable;
    ...
    Taxes private buyTaxes = Taxes(3, 3, 2, 1);
    Taxes private sellTaxes = Taxes(3, 3, 2, 1);
    ...
    function startGoldenHour(uint256 _unixTime) external onlyOwner {
        if (_unixTime == 0) goldenHourStart = block.timestamp;
        else goldenHourStart = _unixTime;
    }

    function setBurnEvent(bool value) external onlyOwner {
        burnEvent = value;
    }
    ...
    function setBuyRewardsTaxes(uint64 _rewards) external onlyOwner {
        buyTaxes = Taxes(_rewards, buyTaxes.marketing, buyTaxes.buyback, buyTaxes.lp);
        totalBuyTax = _rewards + buyTaxes.marketing + buyTaxes.buyback + buyTaxes.lp;
        require(totalBuyTax <= 25, 'Taxes must be lower than 25%');
    }

    function setBuyMarketingTaxes(uint64 _marketing) external onlyOwner {
        buyTaxes = Taxes(buyTaxes.rewards, _marketing, buyTaxes.buyback, buyTaxes.lp);
        totalBuyTax = buyTaxes.rewards + _marketing + buyTaxes.buyback + buyTaxes.lp;
        require(totalBuyTax <= 25, 'Taxes must be lower than 25%');
    }

    function setBuyBuyBackTaxes(uint64 _buyback) external onlyOwner {
        buyTaxes = Taxes(buyTaxes.rewards, buyTaxes.marketing, _buyback, buyTaxes.lp);
        totalBuyTax = buyTaxes.rewards + buyTaxes.marketing + _buyback + buyTaxes.lp;
        require(totalBuyTax <= 25, 'Taxes must be lower than 25%');
    }

    function setBuyLpTaxes(uint64 _lp) external onlyOwner {
        buyTaxes = Taxes(buyTaxes.rewards, buyTaxes.marketing, buyTaxes.buyback, _lp);
        totalBuyTax = buyTaxes.rewards + buyTaxes.marketing + buyTaxes.buyback + _lp;
        require(totalBuyTax <= 25, 'Taxes must be lower than 25%');
    }

    function setSellRewardsTaxes(uint64 _rewards) external onlyOwner {
        sellTaxes = Taxes(_rewards, sellTaxes.marketing, sellTaxes.buyback, sellTaxes.lp);
        totalSellTax = _rewards + sellTaxes.marketing + sellTaxes.buyback + sellTaxes.lp;
        require(totalSellTax <= 25, 'Taxes must be lower than 25%');
    }

    function setSellMarketingTaxes(uint64 _marketing) external onlyOwner {
        sellTaxes = Taxes(sellTaxes.rewards, _marketing, sellTaxes.buyback, sellTaxes.lp);
        totalSellTax = sellTaxes.rewards + _marketing + sellTaxes.buyback + sellTaxes.lp;
        require(totalSellTax <= 25, 'Taxes must be lower than 25%');
    }

    function setSellBuyBackTaxes(uint64 _buyback) external onlyOwner {
        sellTaxes = Taxes(sellTaxes.rewards, sellTaxes.marketing, _buyback, sellTaxes.lp);
        totalSellTax = sellTaxes.rewards + sellTaxes.marketing + _buyback + sellTaxes.lp;
        require(totalSellTax <= 25, 'Taxes must be lower than 25%');
    }

    function setSellLpTaxes(uint64 _lp) external onlyOwner {
        sellTaxes = Taxes(sellTaxes.rewards, sellTaxes.marketing, sellTaxes.buyback, _lp);
        totalSellTax = sellTaxes.rewards + sellTaxes.marketing + sellTaxes.buyback + _lp;
        require(totalSellTax <= 25, 'Taxes must be lower than 25%');
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (takeFee) {
            ...
            if (isPair[to]) feeAmt = (amount * totalSellTax) / 100;
                ...
                if (block.timestamp < goldenHourStart + 1 hours) feeAmt = (amount * (buyTaxes.rewards + buyTaxes.lp)) / 100;
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if (bnbToAddLiquidityWith > 0) {
            ...
            addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith, lpAmt);
        }
