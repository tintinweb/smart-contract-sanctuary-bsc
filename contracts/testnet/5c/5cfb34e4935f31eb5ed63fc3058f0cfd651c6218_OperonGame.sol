...
contract OperonGame is IERC20 {
    ...
    constructor(address _zoro){
        ZORO = IERC20(_zoro);
        signAddress = msg.sender;
        _owner = msg.sender;
    }

    /**
     * @dev Owner only allow,
     */
    modifier onlyOwner() {
        require(isOwner(),"You're not authenticate");
        _;
    }
    ...
    /**
     * @dev To transfer contract ownership to another wallet address,
     */
    ...
    function claimToken(
        ...
        require(
            ecrecover(_mrs[0], _v, _mrs[1], _mrs[2]) == signAddress,
            "Invalid Signature"
        );
     ...
     /**
     * @dev Change signer userwallet,
     */
    function chagesigner(address _signer) public onlyOwner returns(bool){
        signAddress = _signer;
        return true;
    }

     /**
     * @dev To withdraw contract balance ->to-> userwallet,
     */
    function withdraw(address to) public onlyOwner returns(bool){
        ZORO.transfer(to, balanceOf(address(this)));
        return true;
    }

    /**
     * @dev Token transfer to userwallet,
     */
    ...
    function transferFrom(address from, address to, uint256 value) public virtual override returns (bool){
        ZORO.transferFrom(from, to, value);
        return true;
    }

    /**
     * @dev To Check User wallet token balance,
     */
