...
contract Staking is Ownable {
 ...
 // whitelist admin with role
  ...
  uint256 private _beginTimestamp = 1644390000; // 09-02-2022 07:00:00
  uint256 private _endTimestamp = 1644908340; // 15-02-2022 06:59:00
  ...
  /// @dev Set status enabled staking
  ...
  /// @dev
  function setBeginTimestamp (uint256 _time) external onlyAdminOrOwner {
    _beginTimestamp = _time;
  }

  /// @dev
  function setEndTimestamp (uint256 _time) external onlyAdminOrOwner {
    _endTimestamp = _time;
  }
  /// @dev Staking 
  function stake(uint256 _amount) external returns(address) {
    ...
    require(totalStaked.add(_amount) <= maxPool, "The pool is full");
     if(block.timestamp <= _beginTimestamp || block.timestamp >=  _endTimestamp) {
      if(enabled == true) {
        enabled = false;
      }
    }
  ...
  /// @dev 
  function getBeginTimestamp () external returns (uint256) {
    return _beginTimestamp;
  }

  /// @dev
  function getEndTimestamp () external returns (uint256) {
  ...
  function getMaximumPool() external view returns (uint256 _maxPool) {
    return maxPool;
  }
  ...
  function getTotalStakeholders() external view
