...
contract LlamaLandOrigin is Context, AccessControl {
    ...
    constructor(address _llamaLand, string memory _furCid) {
        ...
        _setRoleAdmin(DRAW_ROLE, ADMIN_ROLE);
    }
    ...
    function draw(uint startTokenId, string[] memory cidList, string memory seed) onlyRole(DRAW_ROLE) external {
        uint amount = cidList.length;
        uint start = getCuttingIndex(seed, amount);
        uint iCid = start;
        uint tokenId = startTokenId;
        do {
            string memory cid = cidList[iCid];
            llamaLand.upgrade(tokenId, cid);
            iCid++;
            tokenId++;
            if (iCid == amount) {
                iCid = 0;
