...
contract DigipadStaking is Ownable {
    ...
    uint public totalRewardPending = 0;
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration ) external {
        ...
        uint userReward = ((_amount * pool.returnPer / 10000) / 365) * _lockupDuration;
        uint requiredToken = (totalStake - totalWithdrawal) + totalRewardPending + userReward;
        ...
        totalStake += _amount;
        totalRewardPending +=  ((_amount * pool.returnPer / 10000) / 365) * _lockupDuration;
    ...
    function emergencyWithdraw(uint256 orderId) external {
        ...
        totalRewardsDistribution -= ((orderInfo.amount * orderInfo.returnPer / 10000) / 365) * orderInfo.lockupDuration;
    ...
    {
       uint contract_balance = token.balanceOf(address(this));
       uint total = contract_balance - (totalStake - totalWithdrawal) + totalRewardPending;
