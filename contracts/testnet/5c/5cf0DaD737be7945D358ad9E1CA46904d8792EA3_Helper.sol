...
contract Helper is HelperBase {
        ...
        )
    {
        ...
        uint256[] memory currentLTVs = new uint256[](loanIndex);
        uint256 count = 0;
        for (uint256 i = 1; i <= loanIndex; i++) {
            if (IAPHCore(aphCoreAddress).loanExts(nftId, i).active) {
                ...
                currentLTVs[count] = IAPHCore(aphCoreAddress).getLoanCurrentLTV(i, nftId);
                count++;
            }
        }
        return (count, activeLoanIds, activeLoans, currentLTVs);
    }
}
