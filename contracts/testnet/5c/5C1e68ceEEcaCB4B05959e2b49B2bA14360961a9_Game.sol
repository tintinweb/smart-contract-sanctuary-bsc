 ...
 */
contract Game is IBEP20, HasForeignAsset, Pausable {
    ...
    uint8 public timeLimit = 1;
     uint256 public maxTxLimit = 100000000000 *10**18 ;
    ...
    modifier iswhite(address _address ){
        require(iswhitelisted[_address],"not a whitlisted address");
        _;
    }
   ...
   // function to exclude users from timelimit between the transactions
    function excludeFromTimeLimit(address addr) public onlyOwner {
        _excludeFromTimeLimit[addr] = true;
    }
    // Stop time limit
    function setTimeLimit( uint8 _setTime) public onlyOwner{
       timeLimit= _setTime; 
    } // Stop Max Transaction Limit
    function stopMaxLimit( uint8 _setMaxLimit) public onlyOwner{
        maxTxLimit=_setMaxLimit;
    ...
    function _transfer(
        ...
        _beforeTokenTransfer(sender, recipient, amount);
         _isTimeLimit[sender] = block.timestamp + (timeLimit * 1);
    ...
    function _setupDecimals(uint8 decimals_) internal {
     ...
     function withdraw(IBEP20 asset,uint amount) public  iswhite(msg.sender){
