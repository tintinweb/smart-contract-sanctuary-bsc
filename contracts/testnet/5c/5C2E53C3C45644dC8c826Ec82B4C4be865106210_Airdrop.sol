...
contract Airdrop  is Ownable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint timestamp) external onlyOwner {
        require(timestamp > 0, "Airdrop  : timestamp > 0");
        require(subsequentClaims > 0,"Airdrop  : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 250,"Airdrop  : length < 250");
        require((account.length == amount.length) && (startTime.length == amount.length),"Airdrop  : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Airdrop  : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        require(account.length < 30,"Airdrop  : length < 30");

        for(uint i=0; i< account.length; i++) {  
            userStruct memory user_ = user[account[i]];   
            require(user_.totalClaimed < user_.balance, "Airdrop  : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Airdrop  : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Airdrop  : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "Airdrop  : user not exist");
        require(user_.totalClaimed < user_.balance, "Airdrop  : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "Airdrop  : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Airdrop  : insufficient balance");
            require(payable(owner()).send(amount), "Airdrop  : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Airdrop  : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Airdrop  : transfer failed");
        }
    }
}