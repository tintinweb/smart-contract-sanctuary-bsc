...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  address public DefaultUserToken = address(0);
  ...
  mapping(address => bool) public _isDividendSet;
      ...
      withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
        ...
        if(_isDividendSet[user]){
            ...
            if(DefaultUserToken == address(0)){
                ...
                success = swapEthForTokens(_withdrawableDividend,DefaultUserToken,user);
                //success = IERC20(DividendToken).transfer(user, _withdrawableDividend);
                if(success){
                emit DividendWithdrawn(user, _withdrawableDividend, DividendToken);
                }
...
contract BFR is ERC20, Ownable {
    ...
    constructor() ERC20("Baby Fenrir", "BFR") {
        // LP burn address
    ...
    function updateDefaultUserToken(address _newToken) public onlyOwner {
        dividendTracker.updateDefaultUserToken(_newToken);
    }  
    ...
    function getDefaultUserToken() external view returns(address){
        return dividendTracker.getDefaultUserToken();
    } 
    ...
    function updateFees(uint256 _tokenRewardsFee, uint256 _liquidityFee, uint256 _marketFee, uint256 _treasuryFee, uint256 _buyBackFee, uint256 _burnFee)external onlyOwner{
        ...
        previousTreasuryFee = _treasuryFee;
        ...
        previousTotalFees = _tokenRewardsFee.add(_liquidityFee).add(_marketFee).add(_treasuryFee).add(_buyBackFee).add(_burnFee);
    }    
...
contract BFRDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("BFR_Dividend_Tracker", "BFR_Dividend_Tracker") {
        claimWait = 3600;
