...
contract ArbitrageBot is Ownable {
    ...
    function CrossExchangeArbitrage(
        ...
        if (token0 == WETH) {
            ...
            uint256 tokenAcquired = address(this).balance - PreviousBalance;
            Checktrade(tokenAcquired, CalculatePercentage(msg.value));
        } else if (token1 == WETH) {
            uint256 PreviousBalance = IERC20(token0).balanceOf(address(this));
            ...
            uint256 tokenAcquired = IERC20(token0).balanceOf(address(this)) -
                PreviousBalance;
            ...
            swapTokenForToken(router0, token0, token1, amount);
            IERC20(token1).approve(
                router1,
                IERC20(token1).balanceOf(address(this))
            );
    ...
    function Trianglestrategy(
        ...
        if (token0 == WETH) {
            ...
            swapTokenForToken(router, token0, token1, amount);
            IERC20(token1).approve(
                router,
                IERC20(token1).balanceOf(address(this))
            );
