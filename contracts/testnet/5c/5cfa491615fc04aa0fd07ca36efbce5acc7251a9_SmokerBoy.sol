...
contract SmokerBoy is ERC721 {
  ...
  // define crypto boy struct
   ...
   mapping (uint => address) public smokerToOwner;
  ...
  function mintCryptoBoy(string memory _name, string memory _tokenURI, uint256 _price, string[] calldata _colors) external {
    ...
    smokerToOwner[cryptoBoyCounter] = msg.sender;
  }
function smokeAndIncrement(uint _smokerId) public payable {
     require(msg.sender == smokerToOwner[_smokerId]);
      CryptoBoy memory cryptoboy = allCryptoBoys[_smokerId];
      cryptoboy.smokePoints = cryptoboy.smokePoints.add(10);
  }
