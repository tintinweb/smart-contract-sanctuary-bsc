...
//    uint256 public waitAutoLPTokenNum;//等待自动LP
    ...
    uint256 public waitTrustenshipTokenNum;//等待卖出ME
    ...
    function splitLPHolderToken() private {
        ...
        if(currentSplitIndex >= buySize){
            ...
            if(processingLPHolderDividendTokenNum > 0){
                ...
                processingLPHolderDividendTokenNum = 0;
            }
        }
    }
...
//        waitAutoLPTokenNum += amountB;
        ...
        if(waitTrustenshipTokenNum > 0){ //处理币值管理
            tokenTrustenshipTransfer(waitTrustenshipTokenNum);//币值管理
        }
    ...
    function autoLP() private {
        ...
        uint256 waitAutoLPTokenNum = contractTokenBalance - waitLPHolderDividendTokenNum - processingLPHolderDividendTokenNum - waitTrustenshipTokenNum;//等待自动LP
    ...
    function _transfer(
        ...
        if(currentSplitIndex > 0 || waitLPHolderDividendTokenNum >= swapTokensAtAmount){
            if(currentSplitIndex == 0){
                processingLPHolderDividendTokenNum = waitLPHolderDividendTokenNum;
                waitLPHolderDividendTokenNum = 0;
            }
            splitLPHolderToken();
        } else {
            clearBuyUser();
