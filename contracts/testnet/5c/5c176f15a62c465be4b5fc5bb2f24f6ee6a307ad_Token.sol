...
contract Token is Context, IERC20, Ownable {
    ...
    address payable private marketingAddress; // Marketing Address
    ...
    uint256 private _tTotal = 10 * 10 ** 17;
    ...
    uint public tradingEnabledTimestamp = 1627653600; //10:00pm
    ...
    uint256 public _maxTxAmount = 10 * 10 ** 17;
    uint256 private minimumTokensBeforeSwap = 10 * 10 ** 13;
    ...
    constructor (address payable _marketingAddress,string memory name1,string memory symbol1) {
    ...
    function _transfer(
        ...
        // Sell tokens for ETH
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && from != liquidAddress &&
        to != liquidAddress && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance && _startTimeForSwap + _intervalMinutesForSwap <= block.timestamp) {
                    _startTimeForSwap = block.timestamp;
                    uint backs = contractTokenBalance.div(2);
                    swapTokens(backs);
                    swapAndLiquify(contractTokenBalance.sub(backs));
                }
            }
    ...
    function updateLiquidAddress(address _liquidAddress) external onlyOwner{
    ...
    function setMaxTxAmountPercent(uint256 percent) external onlyOwner {
        _maxTxAmount = _tTotal.mul(percent).div(100);
    }
