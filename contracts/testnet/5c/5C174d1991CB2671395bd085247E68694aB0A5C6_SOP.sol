...
contract SOP is IERC20,Ownable{
    ...
    event Auction(uint256 id,address take,uint256 fee);
    event Take(uint256 id);
    event DestroyCard(uint256 id);
    event AuctionTransfer(uint256 id,uint256 amount,uint256 ddaAmount,uint256 sopAmount);
    ...
    function auctionTransfer(uint256 id,address recipient,uint256 amount) public returns (bool) {
        ...
        uint256 ddaAmount = amount.mul(100-_fee).div(100);
        uint256 sopAmount = amount.mul(_fee).div(100);
        require(DDA.transferFrom(msg.sender,recipient,ddaAmount),"DDA transfer error");
        _transfer(msg.sender, _master, sopAmount);
        emit AuctionTransfer(id,amount,ddaAmount,sopAmount);
        return true;
    }
