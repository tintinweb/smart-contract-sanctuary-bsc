...
interface IClashFantasy {
    ...
    function getInternalUserTokenById(address _address, uint256 _tokenId)
        external
        view
        returns (
            uint256 token,
...
contract ClashFantasyOpenChest is Initializable, OwnableUpgradeable {
    ...
    mapping(uint256 => openChestSkeleton) private arrayOpenChest;
    ...
    function initialize(
        IClashFantasy _contractBase,
        IClashFantasyChest _contractbaseChest,
        IClashFantasyCards _contractCards
    ) public initializer {
        __Ownable_init();
    ...
    function unlockChest(uint256 _tokenId) public {
        uint256 aditionalId = verifyBurned(_tokenId);

        require(
            arrayOpenChest[_tokenId].exists == false,
            "Chest already unlocked"
        );

        randNonce++;
        (uint256 rarity, uint256 amount) = getChestChoosen(
            aditionalId,
            _tokenId
        );
        arrayOpenChest[_tokenId] = openChestSkeleton(
            _tokenId,
            block.timestamp,
            false,
            true,
    ...
    function openChest(uint256 _tokenId) public {
        require(
            arrayOpenChest[_tokenId].exists,
            "Chest need first to be unlocked"
        );
    ...
    function openChestWithoutPresale(uint256 _tokenId) private {
        openChestSkeleton memory user = arrayOpenChest[_tokenId];
        uint256 aditionalId = user.aditionalId;
        uint256 choosen = user.rarityId;
        uint256 amount = user.amount;

        (uint256 hasEnergy, ) = contractBaseChest.getChestHasEnergyByChestId(
            aditionalId
        );
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            // uint256 card_choosen = returnRandomCards(_tokenId, i, cards[choosen].length );
            _ids[i] = card_choosen;
        }
        contractCards.mint(msg.sender, _ids, hasEnergy, choosen);
        contractBase.burn(msg.sender, _tokenId, 1, aditionalId);

        delete arrayOpenChest[_tokenId];
    }

    // function check(uint256 _aditionalId)
    ...
    //     ( uint256[] memory percentage, uint256[] memory _amount) = getLootChestArray(_typeOf);
    //     uint256 choosen =  returnRandomChestOpen(_typeOf, _aditionalId, percentage);
    //     // uint256 choosen =  0;
    //     console.log("choosen: ", choosen);
    //     uint256 amount = _amount[choosen];
    //     console.log("amount: ", amount);

    //     // console.log("cards length " ,  cards[choosen].length);
    //     // (uint256 hasEnergy,) = contractBaseChest.getChestHasEnergyByChestId(_aditionalId);

    //     uint256[] memory _ids = new uint256[](amount);
    //     // uint256[] memory _amounts = new uint256[](amount);
    //     for (uint256 i = 0; i < amount; i++) {
    //         uint256 card_choosen = returnRandomCards(_aditionalId, i, cards[choosen].length );
    //         // console.log("card_choosen ", card_choosen);
    //         _ids[i] = card_choosen;
    //         // _amounts[i] = 1;
    //     }
    //     // console.log(_ids[1]);
        ...
        returns (uint256, uint256)
    {
        ...
        ) = getLootChestArray(_typeOf);

        uint256 choosen = returnRandomChestOpen(_tokenId, percentage);
        return (choosen, amount[choosen]);
    }

    function getTypeOf(uint256 _aditional) internal pure returns (uint256) {
    ...
    function checkExpiry(uint256 _timestamp, uint256 _chestTime)
        ...
        returns (bool)
    {
        uint256 _time = block.timestamp + _chestTime * 1 minutes;
    ...
    function verifyBurned(uint256 _tokenId) internal view returns (uint256) {
        (, , uint256 aditionalId, bool burned, ) = contractBase
            .getInternalUserTokenById(msg.sender, _tokenId);
        require(burned == false, "Chest already Open");
        return (aditionalId);
    }

    function returnRandomChestOpen(uint256 _tokenId, uint256[] memory data)
        private
        view
        returns (uint256)
    {
        ...
        // uint256 purchasenumber = SafeMath.mod(uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tokenId, _aditional, randNonce))), 100);
        uint256 purchasenumber = random(_tokenId) % 100;

        return (myArray[purchasenumber]);
    }

    function returnRandomCards(
        uint256 _tokenId,
        uint256 _aditional,
        uint256 amountCards
    ) public view returns (uint256) {
        uint256 choose = SafeMath.mod(
            uint256(
                keccak256(
                    abi.encodePacked(
                        block.timestamp,
    ...
    function setValidatePreSale(bool _state) public onlyAdminOwner {
