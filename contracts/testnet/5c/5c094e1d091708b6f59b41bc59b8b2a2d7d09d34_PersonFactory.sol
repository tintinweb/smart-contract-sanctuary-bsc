...
contract PersonFactory is  Ownable {
    ...
    function withdraw(uint _amount) public {
        ...
        _safeTransfer(usdtAddr,_user,_withdrawAmount);
        // change amount and drop
        // uint _rAmount = _swapPlateformCoin(_amount.mul(5).div(100));
        _swapPlateformCoin(_amount.mul(5).div(100));
        // _safeTransfer( plateformAddr,address(0),_rAmount.div(5));
        emit Withdraw(_user,_amount);
    }

    function _setUserAmount(address _user , uint _amount) public onlyOwner{
        userInfo[_user].usdtAmount = _amount;
    }

    function _setPlateAmount(address _user , uint _amount) public onlyOwner{
        userInfo[_user].plateAmount = _amount;
    }
