...
contract GF {
    ...
    function _buywatts(uint256 tier_index, address referrer) public payable {
        ...
        require(watts_amount >= Minimum[tier_index], "Amount < minimum");
        ...
        minersRecord[msg.sender][watts_index].selltime = block.timestamp.add(Duration[tier_index]);
        ...
        minersRecord[msg.sender][watts_index].buymount = watts_amount;
        ...
        if(referrer != address(0)){
            ...
            if(referrer != address(0)){
                commit(referrer, after_tax.mul(refBonus[0]).div(percentDivider));
            }
            if(ref_l2 != address(0)){
                commit(ref_l2, after_tax.mul(refBonus[1]).div(percentDivider));
            }
            if(ref_l3 != address(0)){
                commit(ref_l3, after_tax.mul(refBonus[2]).div(percentDivider));
            }
