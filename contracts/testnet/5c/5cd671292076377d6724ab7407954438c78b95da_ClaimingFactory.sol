...
contract ClaimingFactory is Initializable, ContextUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct DistributorSettings {
        ...
        uint256 lockAmount; // LockAmount=10000000;
    ...
    event TokenSettingsChanged(address indexed _distributorAddress, uint256 indexed _lockAmount);
   ...
   function setRoot(address _distributorAddress, bytes32 _merkleRoot) public onlyOwner {
   ...
   function withdrawTokens(address _distributorAddress, uint256 amount) public onlyOwner {
    ...
    function lock(
        ...
        bytes32 _merkleRoot,
        uint256 _lockAmount,
        UnlockPeriod[] calldata _periods) payable public nonReentrant {

        require(_periods.length > 0, "cannot lock up zero amount");

        IERC20Upgradeable _token = IERC20Upgradeable(_distributorAddress);
        require(_token.balanceOf(_msgSender()) >= _lockAmount, "insufficient balance to lock");

        _setTokenSettings(_distributorAddress, _merkleRoot, _lockAmount);
        _fillUnlockPeriods(_distributorAddress, _periods);

        _token.transferFrom(_msgSender(), address(this), _lockAmount);
    }
    ...
    function _setTokenSettings(
        address _distributorAddress,
        bytes32 _merkleRoot,
        uint256 _lockAmount) private {
        distributors[_distributorAddress].claimingWalletsMerkleRoot = _merkleRoot;
        distributors[_distributorAddress].lockAmount = _lockAmount;

        emit TokenSettingsChanged(_distributorAddress, _lockAmount);
    }
