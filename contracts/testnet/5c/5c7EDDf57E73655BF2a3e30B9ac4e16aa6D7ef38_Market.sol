...
contract Market is LzApp {
  ...
  function checkSafeLHS(address _user) public view returns (uint256) {
    uint256 _collateralFactor = marketConfig.collateralFactor(address(this), _user);
    require(_collateralFactor <= 9500 && _collateralFactor >= 5000, "bad collateralFactor");
    uint256 _userCollateralShare = userCollateralShare[_user];
    return
      (cashier.toAmount(collateral, _userCollateralShare, false) * collateralPrice * _collateralFactor) /
      (BPS_PRECISION);
  }

  function checkSafeRHS(address _user) public view returns (uint256) {
    uint256 _collateralFactor = marketConfig.collateralFactor(address(this), _user);
    require(_collateralFactor <= 9500 && _collateralFactor >= 5000, "bad collateralFactor");
    uint256 _userDebtShare = userDebtShareLocal[_user];
    return (_userDebtShare * totalDebtValueLocal * debtPrice) / totalDebtShareLocal;
  }
