...
contract Escrow {
  ...
  _;
    ...
    constructor(address payable _owner,uint256 _timeLimitInDays,address payable _admin,address payable _liquidityPool,address payable _burnAddress,uint8 fees,uint8 _adminBNB,uint8 _adminLKN,uint8 _poolCut,uint8 _burnCutBNB,uint8 _burnCutLKN,uint8 _buyerRef) {
        owner = _owner;
        ...
        poolInfo.push(PoolInfo({token: IERC20(address(0x75b089c50cbfD125Fd36F2d944a3c940d05C3a4b))}));
        poolInfo.push(PoolInfo({token: IERC20(address(0xC4144bA713d2dc77D9452DfaAF7E3d39A0EbC204))}));
    ...
    function requestPayment(uint256 _id) public {
        instance memory temp = getEscrow[_id];
        require(block.timestamp >= temp.start + (temp.timeInDays) * 1 days );
    ...
    function raiseDispute(uint256 id) public{
     ...
     function approveForWithdraw(uint256 id,bool withdrawParty) public isWhitelisted(msg.sender) { //onlyOwner function
    ...
    function transferOwnership(address setAddress) public virtual onlyOwner{
        owner = setAddress;
    }
