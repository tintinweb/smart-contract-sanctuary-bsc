...
contract SynthMixPoolV1 is Admin {
        ...
        returns (uint256[] memory, uint256[] memory)
    {
        ...
        uint256[] memory rewardsReferrer = new uint256[](5);

        if (userInfo.user != address(0)) {
            address stakedToken = userInfo.tokenAddress[stakeId];
            // State updation
            uint256 stakedAmount = userInfo.stakedAmount[stakeId];
            ...
            interval = endOfProfit.sub(userInfo.startTime[stakeId]);
            ...
            // Reward calculation
            if (interval >= HOURS) {
                (rewardsToken, rewardsReferrer) = _UserRewardCalculation(
                    userAddress,
                    stakeId,
                    stakeData,
                    interval
        ...
        return (rewardsToken, rewardsReferrer);
    }
    ...
    function _UserRewardCalculation(
        ...
        address referrerMain = userInfo.referrer[stakeId] != address(0)
            ? userInfo.referrer[stakeId]
            : address(0);

        address tokenMain = userInfo.tokenAddress[stakeId] != address(0)
            ? userInfo.tokenAddress[stakeId]
            : address(0);
        ...
        uint256[] memory tokensRefEarned = new uint256[](5);
        ...
        rewardsEarned = tokenMain != address(0)
            ? noOfDays[1].mul(
                getOneDayReward(
                    StakingDataZero,
                    tokenMain,
        ...
        while (i < intervalDays.length) {
            if (noOfDays[0] >= intervalDays[i]) {
                ...
                address rewardToken = tokenMain != address(0)
                    ? tokensSequenceList[tokenMain][i]
                    : address(0);
                ...
                if (rewardToken != address(0)) {
                    if (
                        ...
                        if (referrerMain != address(0)) {
                            uint256 rewarsOnTime = rewardsEarned;
                            refEarned = (rewarsOnTime.mul(refPercentage)).div(
                                100 ether
                            );
                            rewardsEarned = rewarsOnTime.sub(refEarned);
                        }
                    }
                }
                tokensEarned[i] = rewardsEarned;
                tokensRefEarned[i] = refEarned;
        ...
        return (tokensEarned, tokensRefEarned);
    }
