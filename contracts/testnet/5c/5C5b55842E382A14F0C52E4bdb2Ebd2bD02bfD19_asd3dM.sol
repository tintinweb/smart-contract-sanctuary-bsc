...
contract asd3dM is VRFConsumerBase,ReentrancyGuard {
	using SafeMath for uint256;
	...
	IERC20 usd = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
	IERC20 btc = IERC20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);
    IERC20 doge = IERC20(0x8a9424745056Eb399FD19a0EC26A14316684e274);
    IERC20 eth = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    address dogeTrading = 0x8a9424745056Eb399FD19a0EC26A14316684e274;
    address ethTrading = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;

    address internal btcContract = 0xca90A4e46c9eE5757236640f66e7BAAdA61B1AA6;
    address internal dogeContract = 0xe36c86828A37a65a374B3424cf100f573bB4E53B;
    address internal ethContract = 0xca90A4e46c9eE5757236640f66e7BAAdA61B1AA6;
    ...
    uint internal TICKET_VAL = 5000000000000000000;  // Amount (in wei) for getting registered on list
    ...
    // sustain jackpots
    uint256 internal sustainingJackpotsAmount = 1100000000000000000; // 475000000000000000000;
    uint256 internal internalJackpot = jackpotFeed.sub(sustainingJackpotsAmount);
    uint256 internal btcJackpot = sustainingJackpotsAmount.mul(70).div(100);
    uint256 internal dogeJackpot = sustainingJackpotsAmount.mul(20).div(100);
    uint256 internal ethJackpot = sustainingJackpotsAmount.mul(10).div(100);

	uint public JACKPOT_CHANCE = 3; //the chance to hit the jackpot in percent after selected on raffle


	uint internal raffle; //number which picks the winner from registered List
	uint256 internal jackpot = address(this).balance * 80/100; // current jackpot - 20% accumulates for next jackpot
	...
	event LotteryLog(address adrs, string message,uint number,string message2,uint choosennumber1,uint choosenNumber2,uint choosenNumber3);
    ...
    function swapETHToUSD(uint256 tokenAmount) private {
        // generate the swap pair path of tokens
        address[] memory path = new address[](2);
        ...
        // make the swap
        pancakeRouter.swapETHForExactTokens{value: address(this).balance}(
            tokenAmount,
            path,
            address(this), // The contract
            block.timestamp
    ...
    function sendMatic(address payable _to,uint256 value) public payable onlyVRFCoordinator {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override onlyVRFCoordinator {
        ...
        if(jackpotNumber == choosenNumber1) {
            ...
            sendMatic(payable(lastJackpotWinner),jackpot);
        
            clearRegisteredElement(raffle);
            delete addressMapping[lastJackpotWinner];
            delete registereds[lastJackpotWinner];
        
            jackpot = address(this).balance * 80/100;

            emit LotteryLog(lastJackpotWinner, "Jackpot is hit! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
        
        } else {
            // Send Raffle Prize and add a draw on ticket
            usd.safeTransfer(lastWinner,halfTicket);
            if(totalDraws < 2){
                registereds[userWallet].amountOfDraws += 1;
            } else {
                clearRegisteredElement(raffle);
                delete addressMapping[lastWinner];
                delete registereds[userWallet];
            }
            jackpot = address(this).balance * 80/100;
            emit LotteryLog(lastWinner, "We have a new raffle Winner but no jackpot! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1,choosenNumber2,choosenNumber3);
        }
    }
    ...
    // Play Function
    function Bur(uint256 ticketValue,uint number1,uint number2, uint number3) public nonReentrant {
        require(ticketValue == TICKET_VAL,"Failed: not enough USDC sent!");
        require(usd.balanceOf(msg.sender) >= TICKET_VAL,"You don't have enough USDC in the wallet!");
        require(addressMapping[msg.sender] == false,"Failed: already joined!"); //--> cheaper access through map instead of a loop
        require(number1 != number2 && number1 != number3 && number2 != number3 && number1 <= 100 && number1 > 0 && number2 <= 100 && number2 > 0 && number3 <= 100 && number3 > 0 ,"Failed: number has to be between 1 and 100 and not be equal");

        addressMapping[msg.sender] = true;
        Registered memory registered = Registered(0,number1,number2,number3);
        ...
        usd.transferFrom(payable(msg.sender),deployer,sustainingFee);

        // sustain jackpots swaps
        swapTokens(btcTrading,btcJackpot,btcContract);
        swapTokens(dogeTrading,dogeJackpot,dogeContract);
        swapTokens(ethTrading,ethJackpot,ethContract);

        swapTokensForETH(internalJackpot);

        emit LotteryLog(msg.sender, "Ticket Bought, Good luck!",0,"Ticket Numbers :",number1,number2,number3);
    ...
    // Users Forfeit Withdraw function get Back a % of ticket value and fires a reward
    function Fur(uint256 index) public nonReentrant {
       ...
       require(registereds[msg.sender].amountOfDraws < 3,"You already drawed 3 and can't forfeit.");     // Paranoic checking
        ...
        // Refund user
        swapETHToUSD(refundValue);
        usd.safeTransfer(payable(msg.sender),refundValue);         
        
        // Fires a new Draw
        swapETHToUSD(halfTicket);
        getRandomNumber();
