  ...
  Ownable
{
  ...
  //max amount available for PRESALE
  uint256 private constant maxPresalePurchasesTotal = 5000;
  //max amount available for PUBLIC sale
  uint256 private constant maxPublicPurchasesTotal = 2777;

  //max amount available per user for PUBLIC sale
  uint256 private constant maxPublicPurchasesPerUser = 2;
  //max amount available per user for PRESALE
  uint256 private constant maxPresalePurchasesPerUser = 1;

  //total presales sold
  uint256 private totalPresales;

  //cost of each nft
  uint256 private constant mintPrice = 0.03 ether;

  //admin addresses
  address private constant address1 = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
  address private constant address2 = 0xdD870fA1b7C4700F2BD7f44238821C26f7392148;
  address private constant address3 = 0x00000000002870670871555dA8E9686fAaBf712F;
  ...
  mapping (address => bool) private usersForPresale;

  //address[] private usersForPresale;
  ...
  /**
   * @dev Adds a user to presale
   * @param _user address for presale
   */
  function addPresaleUser(address _user) external onlyOwner {
    usersForPresale[_user] = true;
  }

  /**
   * @dev Removes a user from presale
   * @param _user address for presale removal
   */
  ...
  /**
   * @dev Adds a list of users to presale
   * @param _users list of addresses of the presale users
   */
  function addPresaleUsers(address[] memory _users) external onlyOwner {
    for (uint i; i < _users.length; i++) {
      usersForPresale[_users[i]] = true;
    }
  }

  /**
   * @dev Removes a list of users to presale
   * @param _users list of addresses of the presale users to remove
   */
  function removePresaleUsers(address[] memory _users) external onlyOwner {
    for (uint i; i < _users.length; i++) {
      usersForPresale[_users[i]] = false;
    }
  }
   ...
   */
  function mintCrim(uint256 _mintCount) external payable{
    require(isMintable, "Minting is not available");
    require(usersTotalMints[msg.sender].add(_mintCount) <= 11, "Max possible is 11");
    if (tokens.length <= 888) {
        //free mint
        _mintFree();
        
    } else {
        _mintPaid(_mintCount);
    }
  }
  
  /**
   * @dev Mints a paid nft
   * @notice This function is internal and to be called by the mintCrim function
   * @param _mintCount amount of nfts to mint
   */
  function _mintPaid(uint256 _mintCount) internal {
    require(_mintCount <= 5, "Too many");
    if (_mintCount <= 0) {
        _mintCount = 1;
    }
    require(tokens.length.add(_mintCount) <= maxSupply, "Not enough left to mint");
  ...
  /**
   * @dev Mints a free nft
   * @notice This function is internal and to be called by the mintCrim function
   */
  function _mintFree() internal {
    require(tokens.length.add(1) <= 888, "Not enough free to mint");
  ...
  */
  function withdraw() external onlyOwner {
    ...
    require(payable(address1).send(balance.mul(25).div(100)), "address1 couldn't withdraw ");

    require(payable(address2).send(balance.mul(25).div(100)), "address2 couldn't withdraw");

    require(payable(address3).send(balance.mul(25).div(100)), "address3 couldn't withdraw");

    require(payable(address4).send(balance.mul(25).div(100)), "address4 couldn't withdraw");
  }
