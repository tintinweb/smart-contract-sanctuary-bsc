...
contract launchpad is Ownable {
    ...
    mapping(address => uint) public tokenTransferredToPool;
    mapping(address => mapping(address => uint)) public idoTokenTransferredFromFixed;
    ...
    function withdrawRewardToken() public onlyOwner {
        require(totalRewardTokens > 0, "There are zero Reward tokens");
        require(contractEnd < block.timestamp, "Contract is not ended yet!");
        require(rewardToken.transfer(msg.sender, totalRewardTokens), "Failed to transfer");
        totalRewardTokens = 0;
    }
    ...
    function getClaim(address _user) public view returns(uint) {
        ...
        uint256 tempAPR = currentAPR(msg.sender);
        uint _feePercent = tempAPR;
    ...
    function updateIDOInfoLaunch(address updateContract, address _user, uint256 _amount) external {
    ...
    function transferIDOTokenToFixed(address _fixedContract, uint256 _amount) external {
        address _user = msg.sender;
        require(idoTokenTransferredFromFixed[_fixedContract][_user] >= _amount, "No sufficient tokens");
        require(idoToken.transfer(_fixedContract, _amount), "Transfer Failed");
        totalIDOTokenTransferred -= _amount;
        tokenTransferredToPool[_user] -= _amount;
        idoTransfer[_user] = idoTransfer[_user] - _amount;
        idoTransferTemp[_user] = _amount;
    ...
    function currentAPR(address _user) public view returns(uint256) {
        uint256 tempAPR = per * 1e18;
        uint256 tempTier = getTier(_user);
         if (tempTier != 0) {
             if (tempTier == 1) {tempAPR = tierPercent1;}
             else if (tempTier == 2) {tempAPR = tierPercent2;}
             else {tempAPR = tierPercent3;}
         }
         return tempAPR;
    }
