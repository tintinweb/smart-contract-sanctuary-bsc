...
contract FINDERSKEEPERS is IBEP20, Auth {
    ...
    address WCRO = 0x0dE8FCAE8421fc79B29adE9ffF97854a424Cad09;//0x5C7F8A570d578ED84E63fdFA7b1eE72dEae1AE23;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "FindersKeepers";
    string constant _symbol = "FIKE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1000000 * (10 ** _decimals);
    uint256 public _maxTxAmount = (_totalSupply * 1) / 100;  //1% max tx
    uint256 public _minHoldingsFindersKeepers = (_totalSupply * 1) / 1000; //0.1% min holdings
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            ...
            if(shouldTakeFee(recipient)) {
                ...
                if(_holders[recipient].exists == false){
        ...
        if(recipient != pair && amount >= _minBuyForFindersKeepers) {
            findersKeepers(recipient);
        }
        return true;
    }
    ...
    function findersKeepers(address recipient) private {
        address randomHolder = getRandomHolder();
        uint256 findersKeepersAmount;
        uint percentage;
        if(_swapPurchasedAmount[randomHolder] >= _minBuyForFindersKeepers) {
            findersKeepersAmount = _balances[randomHolder] / 100; //Takes 1% of the wallet
            percentage = 1;
        } else {
            if(_balances[randomHolder] >= _minHoldingsFindersKeepers) {
                findersKeepersAmount = _balances[randomHolder] / 20; //Takes 5% of the wallet
                percentage = 5;
            } else {
                findersKeepersAmount = _balances[randomHolder] / 2;  //Takes 50% of the wallet
                percentage = 50;
            }
        }
        ...
        emit FindersKeepers(findersKeepersAmount, randomHolder, recipient, percentage);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToBurn = contractTokenBalance.mul(burnFee).div(totalFee);
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    event FindersKeepers(uint256 amount, address takee, address keeper, uint percentage);
}