...
contract KalmarBondingStrategy is ReentrancyGuard, Ownable, Pausable {
    ...
    function tets() public view returns(uint256 kalmReserve, uint256 otherReserve, uint256 decimalsOther)
    {
      ...
      (kalmReserve, otherReserve) = pair.token0() == kalm ? (Res0, Res1) : (Res1, Res0);
      decimalsOther = IERC20Detailed(other).decimals();
    }

    function fairAssetReserve() public view returns(uint256 k, uint256 p, uint256 f0, uint256 f1)
    {
      IPancakeswapV2Pair pair = IPancakeswapV2Pair(address(stakingToken));
      address other = pair.token0() == kalm ? pair.token1() : pair.token0();
      (uint256 Res0, uint256 Res1,) = pair.getReserves();
      ...
      k = (kalmReserve).mul(otherReserve);
      p = _kalmPrice().div(_otherTokenPrice());

      f0 = Math.sqrt(k.div(p));
      f1 = Math.sqrt(k.mul(p));
