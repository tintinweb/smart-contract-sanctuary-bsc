...
contract Bonjour is Ownable,ReentrancyGuard {
    ...
    function regUser(address _referrer,uint256 _amount) public {
        ...
        if(users[_referrer].referralCount == 5 && _referrer!= ownerAddress){
          users[_referrer].levelIncomeEligible = 2;  
        }else if(users[_referrer].referralCount == 15 && _referrer!= ownerAddress){
          users[_referrer].levelIncomeEligible = 5;    
        }
        totalParticipants++;
    ...
    function reInvest(uint256 _amount) public {
         ...
         if(totalearned > users[msg.sender].investmentAmount.mul(earnLimit).div(100)){
             users[msg.sender].earnedAmount =  earnLimit * 1e18;
    ...
    function checkIsEarn(address _user) public view returns(bool) {
        ...
        uint256 totalearned = uptoroi.add(users[_user].earnedAmount);
