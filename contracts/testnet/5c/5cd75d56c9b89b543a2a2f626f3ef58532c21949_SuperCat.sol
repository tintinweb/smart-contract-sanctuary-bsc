...
contract SuperCat is Context, Ownable{
    ...
    mapping (address => bool) public nft_pledge_info;
    ...
    constructor() public {
        ...
        SuperCatVault_addr = 0x15A0944D8d796e33E22f82F2123D3968f66FC9F0;
    ...
    function addLPLiquidity(uint256 usdt_amount,uint256 _nftId,address _nft_contract) public{
        ...
        IERC20(catToken).transferFrom(msg.sender,address(this), _quote);
        IERC20(usdtToken).transferFrom(msg.sender,address(this), usdt_amount);
        IERC20(catToken).approve(address(Router), _quote);
        IERC20(usdtToken).approve(address(Router), usdt_amount);
        Router.addLiquidity(usdtToken,catToken,usdt_amount,_quote,usdt_amount.mul(90).div(100),_quote.mul(90).div(100),msg.sender,block.timestamp);
        // IERC20(catToken).increaseWeight(msg.sender, usdt_amount);
        lp_weight[msg.sender] = lp_weight[msg.sender].add(usdt_amount);
        if(nft[_index].lp_amount>lp_weight[msg.sender]&&nft_pledge[_nft_contract][_nftId].miner_time==0){
            nft_pledge[_nft_contract][_nftId].miner_time = block.timestamp;
        }
        nft_pledge_info[msg.sender] = true;
        lp_weight_total = lp_weight_total.add(usdt_amount);
        SuperCatVault(SuperCatVault_addr).addLPLiquidity(msg.sender,usdt_amount);
        emit AddLiquidity(usdt_amount,_quote);
    }
    ...
    function swap(uint256 _amount,bool _Is) public{
        ...
        if(_Is){
            ...
            IERC20(catToken).approve(_router, _amount); 
            ...
            IERC20(usdtToken).approve(_router, _amount); 
    ...
    function receiveProfit(uint256 _nftId,address _nft_contract) public {
        ...
        uint256 _index = nft_index[_nft_contract];
        uint256 _amount = (lp_release /86400) * (nft[_index].lp_amount / lp_weight_total) * _time_interval;
        if(_amount>0){
            IERC20(catToken).transfer(msg.sender, _amount);
            nft_pledge[_nft_contract][_nftId].miner_time = block.timestamp;
            emit ReceiveProfit(msg.sender,_amount,_time_interval,block.timestamp,nft[_index].lp_amount,lp_weight_total);
        }
    }
    ...
    function nftBuy(address contractAddress)  public payable{
        ...
        // uint256 nft_id = IERC721(contractAddress).officalMint(str,1);
        // IERC721(contractAddress).transferFrom(address(this),msg.sender,nft_id);
    }
