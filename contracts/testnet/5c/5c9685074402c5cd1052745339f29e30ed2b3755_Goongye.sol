...
contract Goongye is Ownable, Pausable, ERC721Enumerable{

    using Strings for uint256;
    using SafeMath for uint256;

    uint256 public constant MAX_SUPPLY = 40;
    uint256 constant public init_supply = 17;
    ...
    uint256 preSaleprice1 = 0.001 ether ;
    uint256 preSaleprice2 = 0.002 ether ;
    uint256 preSaleprice3 = 0.003 ether ;
    uint256 preSaleprice4 = 0.004 ether ;
    uint256 kingprice = 0.01 ether;
    ...
    constructor(
        // address payable _collectionWallet
        ) ERC721("G00NGYE", "KLAYTN") { 
    ...
    function InitMinting() internal  {
        require(totalInitialMinted<init_supply,"INIT Total Minted Reached");
    ...
    function mint(uint256 _count) public payable  {
        ...
        require(totalSupply() + _count <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value >= (gPRice(_count)), "INVALID_ETH_SENT");
    ...
    // Function to get the price of NFT according to price set 
    function gPRice(uint256 _count) public view returns(uint256){
        ...
        (totalSupply()+i > 20 && totalSupply()+i <= 25)
        {
            Price += preSaleprice2;
        }else if
        (totalSupply()+i > 25 && totalSupply()+i <= 30)
        {
            Price += preSaleprice3;
        }else if
        (totalSupply()+i > 30 && totalSupply()+i <= 40)
        {
            Price += preSaleprice4;
        }
    }
    ...
    uint256 public upgradesupply = 40;
    uint256 public Max_upgradesupply = 60;
    function UpgradeKing(uint256 _id) public payable{

        // require(totalSupply() >= 8000,"Common Card minting hasn't completed");
        // require(totalSupply() + _count <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        // require(msg.value == kingprice, "INVALID_ETH_SENT");
        require(msg.value>= 0.01 ether,"Enter required Kalytn Coin");
        require(upgradesupply <= Max_upgradesupply, "MAX_SUPPLY_REACHED");
        ERC721._burn(_id);
        
        _safeMint(msg.sender, (upgradesupply + 1));
        upgradesupply += 1 ;

        totalPublicMinted[msg.sender]+=_id;
        addToMintData(_id); 
    }
