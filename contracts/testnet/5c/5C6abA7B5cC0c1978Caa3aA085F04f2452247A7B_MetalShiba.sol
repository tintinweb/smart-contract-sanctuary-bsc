...
All new created liquidity is sent to the contract address and not in any wallet controlled by anyone. 
These LP are withdrawable from the contract only after a given TimeLock is passed (fixed at 180 days). 
...
contract MetalShiba is IBEP20, Ownable, TimeLock {
    ...
    function farmTokens(address friend) public {
    ...
    // launch
    function launch(uint256 _deadBlocks_antisniper, uint256 _deadBlocks_highslippage) public onlyOwner {
    ...
    function set_farming_state(bool status) public onlyOwner {
        is_farmable = status;
    }
    ...
    function set_ReferTreshold(uint256 amount) public onlyOwner {
        require(amount >= _totalSupply / 1000);
    ...
    function get_pool_details(address addy) public view returns (uint256, uint256, uint256) {
    ...
    function getprice_BNB() public view returns (uint256) {
