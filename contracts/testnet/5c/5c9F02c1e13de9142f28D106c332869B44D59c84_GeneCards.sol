...
contract GeneCards is ERC1155, AccessControl {
  ...
  function RealizeOne(uint16 kind) public {
    uint256[9] memory balance;
    uint256 minCountOfFrag = 1000000000000;
    for(uint8 i=0; i<9; i++) {
    ...
    if(minCountOfFrag > 0) {
      _mint(msg.sender, CARD_BASE + kind, minCountOfFrag, "");
      cardSupply[kind] -= minCountOfFrag;
      emit CardFragMint(msg.sender, CARD_BASE + kind, minCountOfFrag);
