...
contract StakePoolV2 is StakePool {
    ...
    function withdrawNFT(uint256 _tokenId, uint256 _feeAmount) external {
        require(_nftOwners[_tokenId] == _msgSender(), "!Owner");

        uint256 _weight = nft.getWeight(_tokenId);

        uint256 _amount = _values[_weight];
        uint256 _fee = calculateWithdrawFees(_amount);
        require(_feeAmount >= _fee, "!Fee");

        stakeToken.transferFrom(_msgSender(), address(this), _fee);
...
contract StakePoolFee is Ownable {
    ...
    function calculateWithdrawFees(uint256 _amount)
        public
        view
        returns (uint256 _fee)
    {
        uint256 _timeLock = timeLocks[msg.sender];
