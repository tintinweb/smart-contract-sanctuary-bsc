...
contract XoomVirtualStake {
    ...
    function claim(uint stakeID) external validateAllowed validWallet activeStake(stakeID) validRequirements(FUNCTION_ID_CLAIM) claimValidate(stakeID)
    {
        ...
        {
            internalSwap(platformToken, rewardToken, accumulated, reward, msg.sender);
        }
    }
    ...
    function internalSwap(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOutMin, address to) private 
    {
        ...
        if (tokenIn == chainWrapToken || tokenOut == chainWrapToken) 
        {
            path = new address[](2);
            path[0] = tokenIn;
            path[1] = tokenOut;
