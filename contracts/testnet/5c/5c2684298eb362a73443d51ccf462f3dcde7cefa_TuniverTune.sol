    ...
    ITuniverTune
{
    ...
    ITunipass public tunipassContract;

    Tune[] private _tunes;
    mapping(uint256 => bool) public mintFrom;

    uint256 public amountTunePerTunipass;
    ...
    function setTunipassContract(ITunipass _tunipassContract) external onlyRole(CONTROLLER_ROLE) {
        require(address(_tunipassContract) != address(0));
        tunipassContract = _tunipassContract;
    }
    ...
    function levelUpTunipass(uint256 tuneId, uint256 tunipassId) external {
        require(address(tunipassContract) != address(0));
