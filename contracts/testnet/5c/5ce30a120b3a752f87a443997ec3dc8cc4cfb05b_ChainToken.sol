...
contract ChainToken is Context, IERC20, Ownable {
    ...
    constructor (
        ...
        swapPair = IFactory(swapRouter.factory())
            .createPair(address(this), swapRouter.WETH());


        //exclude from reward
        _isExcluded[swapPair] = true;
        _excluded.push(swapPair);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!swapping && swapEnabled && canSwap && from != swapPair && !_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
