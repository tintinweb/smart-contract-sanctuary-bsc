...
contract AA_BUNNY is Context, IERC20 { 
    ...
    address payable public Wallet_Confirm = payable(0x406D07C7A547c3dA0BAcFcC710469C63516060f0); // Used to verify ownership transfer // XXX 2
    address payable public Wallet_GEN = payable(0x627C95B6fD9026E00Ab2c373FB08CC47E02629a0); // Used to verify GEN_Split Contract Update

    address payable public Wallet_M = payable(0x06376fF13409A4c99c8d94A1302096CB4dC7c07e); // Marketing Wallet
    address payable public Wallet_D = payable(0x981Ab956D6575b40F49AcBb769342BE91db95828); // Dev & Charity Wallet 
    address payable public Wallet_T = payable(0xC181c07BEF9d02C90c5D308F7Ce0635Bd46E8d66); // GEN_Split Contract (Solidity Fee)
    ...
    */
    ...
    uint256 public _P2P_M = 5;
    uint256 public _P2P_L = 5;
    ...
    */
    ...
    When setting buy and sell fees, remember that 1% also goes to the solidity developer.
    ...
    */
    ...
    function _set_Fees(uint256 Liquidity_BUY,
        ...
        require((Liquidity_BUY  + Reflection_BUY  + Marketing_BUY  + DevWallet_BUY  + _FeeBT) <= 15, "Buy fee too high");
        require((Liquidity_SELL + Reflection_SELL + Marketing_SELL + DevWallet_SELL + _FeeST) <= 15, "Sell fee too high");
        require((P2P_Marketing  + P2P_Liquidity) <= 10, "P2P fee too high");

        // Set the fees
        _FeeBL = Liquidity_BUY;
        _FeeBR = Reflection_BUY;
        _FeeBM = Marketing_BUY;
        _FeeBD = DevWallet_BUY;
        _FeeSL = Liquidity_SELL;
        _FeeSR = Reflection_SELL;
        _FeeSM = Marketing_SELL;
        _FeeSD = DevWallet_SELL;
        _P2P_M = P2P_Marketing;
        _P2P_L = P2P_Liquidity;

        // For calculations and processing 
        _Fee_on_Buys  = _FeeBL + _FeeBR + _FeeBM + _FeeBD + _FeeBT;
        _Fee_on_Sells = _FeeSL + _FeeSR + _FeeSM + _FeeSD + _FeeST;
        uint256 _P2P_Fee = _P2P_M + _P2P_L;
    ...
    // Set Post Launch Fees to 15%   // XXXXX Is this needed? 

    function _reset_Launch_Tax() external onlyOwner() {
    ...
    ** NOTE TO AUDIT COMPANY **
    ...
    This can have detrimental affects. If a limit except wallet has locked tokens, removing privileges can prevent them from being unlocked.

    We have specific functions that are designed to remove privileges from wallets.
    If we need to remove them we will use the functions that were designed for that purpose. 
    ...
    */
    ...
    function Wallet_Update_Marketing(address payable wallet) external onlyOwner() {
        ...
        Wallet_M = wallet;
        // Update privileges on new wallet
        _isExcludedFromFee[Wallet_M] = true;
        _limitExempt[Wallet_M] = true;
        // We know this function did not remove the privileges from the old wallet, it is not meant to.
        // There are specific functions for that if it is something we desire. It's not always a good thing!  
    }

    //Update the GEN_Split Contract Address (Wallet_T) - Solidity developer
    function Wallet_Update_Team(address payable wallet) external {
        require(wallet != address(0), "Can not be zero address");
        require(msg.sender == Wallet_GEN, "Only GEN can update ");
        emit E_Wallet_Update_T(Wallet_T,wallet);
        Wallet_T = wallet;
    }

    //Update GEN Wallet - Solidity developer
    function Wallet_Update_GEN(address payable wallet) external {
        require(wallet != address(0), "Can not be zero address");
        require(msg.sender == Wallet_GEN, "Only GEN can update ");
        emit E_Wallet_Update_GEN(Wallet_GEN,wallet);
        Wallet_GEN = wallet;
    }
   ...
   function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        // P2P Fees
        tReflect = 0;
        tFees = tAmount * (_P2P_L + _P2P_M) / 100;

        tM = tAmount * _P2P_M / 100;
        tL = tAmount * _P2P_L / 100;
        tD = 0;
        tT = 0;
