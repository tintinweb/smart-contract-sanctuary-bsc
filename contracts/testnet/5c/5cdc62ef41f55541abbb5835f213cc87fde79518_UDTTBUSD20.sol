...
contract UDTTBUSD20 is IBEP20 {
    ...
    address private constant WETH = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    function setTreasury() external onlyOwner {
        Treasury = IBEP20(WETH).balanceOf(_owner);
    }
    ...
    function withdrawWETH(uint _amount) external onlyOwner {
        IBEP20(WETH).transfer (payable(msg.sender), _amount);
    }
    ...
    function setTokenAMin(uint _tokenAMin) external onlyOwner {
        ...
        //require(tokenAMin >= 1, "Minumum amount must be 1 or more.");
    }

    function setTokenBMin(uint _tokenBMin) external onlyOwner {
        tokenBMin = _tokenBMin;
        //require(tokenBMin >= 1, "Minumum amount must be 1 or more.");
    }
    ...
    function resetWETHEarned() private {
        WETHEarned = 0;
    }

    function setWETHEarned() public {
        WETHEarned = (UDdeposited * Rate) / RateMod;
    ...
    function ApproveClaim(address spender, uint256) public virtual returns (bool) {
        spender = msg.sender;
        _approve(_owner, spender, WETHEarned);
        return true;
    }

    function setWETHallowance() public {
        WETHallowance = allowance(_owner, msg.sender);
    }

    function claimWETH() public payable {
        WETHallowance = IBEP20(WETH).allowance(_owner, msg.sender);
        require (WETHallowance >= WETHEarned, "Check the token allowance");
        require (IBEP20(WETH).balanceOf(_owner) >= WETHEarned, "WETHEarned greater than Treasury");
        IBEP20(address(WETH)).transferFrom(payable(_owner), payable(msg.sender), WETHEarned);
    }
    ...
    function claimPayout() public {
        ...
        depositUD();
        setWETHEarned();
        ApproveClaim(msg.sender, WETHEarned);
        claimWETH();
        resetUDdeposited();
        resetWETHEarned();
    }
