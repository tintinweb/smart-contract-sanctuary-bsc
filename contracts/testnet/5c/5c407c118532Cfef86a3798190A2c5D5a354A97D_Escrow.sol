...
contract Escrow is Ownable,Initializable {
    ...
    mapping(address => Grants) public grants;
    ...
    ///@dev add grants to the grant list
    ///@param accounts address of grants
    ///@param amounts allocation amount of grants
    function addGrants(
        ...
        onlyOwner 
    {
        require(accounts.length == amounts.length, "onlyGrants: mismatch array length");
        require(accounts.length < 30, "onlyGrants: length must be lesser than 30");

        uint64 blockTime = blockTimestamp().toUint64();

        for(uint i=0;i<accounts.length;i++) {
            grants[accounts[i]] = Grants({
               allocated : amounts[i],
               released : 0,
               startTime : blockTime,
               lastClaim : blockTime
    ...
    ///@dev remove grants from the grant list
    ///@param accounts address of grants
    function removeGrants(
        ...
        onlyOwner 
    {
        require(accounts.length < 30, "removeGrants: length must be lesser than 30");

        for(uint i=0;i<accounts.length;i++) {
            delete grants[accounts[i]];
        }
    }    
    ...
    function updateDivPeriod(uint64 period) external onlyOwner {
        require(!isZero(period), "updateDividend: the dividend period must not be zero");
        dividend.period = period;
    }

    ///@dev grants claim their dividends from here
    function claim() external {
        Grants storage grant = grants[_msgSender()];
        require(!isZero(grant.startTime), "claim: start time must not be zero");
        require(!isZero(grant.allocated), "claim: allocation must not be zero");
        require(grant.released <= grant.allocated, "claim: the market wallet has claimed all dividends");
        require(grant.lastClaim + dividend.period <= blockTimestamp(),"claim: time hasn't passed for the next dividend");

        (uint dividends, uint claims) = viewDividend(_msgSender());

        if(!isZero(dividends)) {
            grant.released += dividends;
            grant.lastClaim += (dividend.period * claims).toUint64();
    ...
    function viewDividend(address account) public view returns(uint dividends, uint claims) {
        Grants storage grant = grants[account];
        
        if(!isValidClaim(grant)) {   
            return (0,0);
        }

        uint months = (blockTimestamp() - grant.lastClaim) / dividend.period;

        if(!isZero(months)) {
            uint divAmount = (grant.allocated * dividend.percent) / DIVISOR;
            dividends = divAmount * months;
            claims = months;

            if((grant.released + dividends) > grant.allocated) {
                dividends = grant.allocated - grant.released;
            }
        }
    }

    ///@dev returns true if grant is allowed to claim
    ///@param grant grant info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Grants memory grant) private view returns(bool) {
        return !(
            isZero(grant.startTime) ||
            isZero(grant.allocated) ||
            grant.released >= grant.allocated ||
            (grant.startTime + (dividend.cliffTime + 1 days)) < blockTimestamp()
        );
    }
