...
import "./LimitedEditionCrates.sol";
import "./LimitedEditionSamurai.sol";


contract LimitedEditionCratesOpening {
    LimitedEditionCrates public immutable crates;
    LimitedEditionSamurai public immutable samurais;
    
    address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

    mapping(uint => uint) public samuraiIdToCrateId;

    constructor(LimitedEditionCrates crates_, LimitedEditionSamurai samurais_) {
        crates = crates_;
        samurais = samurais_;
    }
    ...
    function openCrates(uint[] calldata crateIds) external {
          for (uint i = 0; i < crateIds.length; i++) {
            ...
            uint samuraiId = samurais.mint(msg.sender);

            samuraiIdToCrateId[samuraiId] = crateId;

            emit CrateOpened(msg.sender, crateId, samuraiId);
        }
    }

    event CrateOpened(address indexed user, uint crateId, uint samuraiId);
}
...
contract LimitedEditionCrates is ERC721Enumerable, AccessControl {
     ...
     */
    function mintCrate(address user) external onlyRole(MINTER_ROLE) returns (uint) {
...
contract LimitedEditionSamurai is ERC721Enumerable, AccessControl {
    using Counters for Counters.Counter;
