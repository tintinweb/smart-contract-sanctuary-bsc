...
contract GEMUNILeasing is PermissionGroup {
    ...
    mapping(address => uint[]) public guildInfo;
    ...
    event LendPass(uint indexed tokenId, uint price,uint startLending, uint duration);
    event UpdateLeasingPass(uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerSecond, bool newTransferWallet, LeasingType _type);
    event RemoveFromLending(uint indexed tokenId);
    ...
    event WithdrawPass(uint indexed tokenId);
    ...
    modifier onlyPassOwner(uint tokenId) {
        require(geniPass.ownerOf(tokenId) == msg.sender, "Leasing: not pass owner");
        _;
    }

    modifier onlyLender(address nftToken, uint tokenId) {
        LeasingInfo memory leasing = leasingInfo[nftToken][tokenId];
        require(leasing.lender == msg.sender, "Lending: not pass owner");
        _;
    }

    modifier onlyGameGuild(address guild) {
        require(gameGuild[guild], "Lending: not game guild");
        _;
    }

    function validatePass(uint tokenId) public view returns(bool) {
        bool isExistedToken = IGENIPass(address(geniPass)).exists(tokenId);
    ...
    function adminPutOnLeasing(address nftToken, uint tokenId, uint minDuration, uint maxDuration, uint pricePerWeek, bool transferWallet, LeasingType _type) public onlyOperator {
        require(validatePass(tokenId));
    ...
    function guildPutOnLeasing(address nftToken, uint tokenId, uint minDuration, uint maxDuration, uint pricePerWeek, bool transferWallet) public onlyGameGuild(msg.sender) {
        require(validatePass(tokenId));
        require(nftToken != address(0), "Leasing: invalid address");
        require(minDuration > 0 && maxDuration > minDuration && pricePerWeek > 0, "Leasing: invalid input");
    ...
    function guildRegisteredLeasing(address nftToken, uint[] memory tokenIds, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek, uint rentalPrice) external onlyGameGuild(msg.sender) {
        uint[] storage guild = guildInfo[msg.sender];
        require(guild.length + tokenIds.length >= minPasses, "Leasing: invalid number of passes");
        uint totalRental;
    ...
    function updateLeasingPass(address nftToken, uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerSecond, bool newTransferWallet, LeasingType _type) external {
        require(validatePass(tokenId));
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        require(leasing.price > 0, "Leasing: not on leasing");
        require(geniPass.ownerOf(tokenId) == address(this), "Lending: already rented");
        require(newMinDuration > 0 && newMaxDuration > newMinDuration && newPricePerSecond > 0, "Leasing: invalid input");
        ...
        leasing.leasingType = _type;
        emit UpdateLeasingPass(tokenId, newMinDuration, newMaxDuration, newPricePerSecond, newTransferWallet, _type);
    }
    
    function removeFromLending(address nftToken, uint tokenId) external onlyLender(nftToken, tokenId) {
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        require(leasing.price > 0, "Lending: not on lending");
        require(geniPass.ownerOf(tokenId) == address(this), "Lending: already rented");
 
        geniPass.transferFrom(address(this), leasing.lender, tokenId);
        emit RemoveFromLending(tokenId);

        delete leasingInfo[nftToken][tokenId];
    }

    function setNumberPassesOfThirdParty(uint _minPasses) external onlyOperator {
        require (_minPasses > 0, "invalid min max");
        minPasses = _minPasses;
    }

    function addListGameGuild(address[] memory users) external onlyOperator {
    ...
    function removeFromGuilds(address[] calldata users) external onlyOperator {
        require(users.length > 0, "Leasing: list user is empty");
    ...
    function lendPass(address nftToken, uint tokenId, uint duration, uint price, bytes memory sig) external {
        ...
        if (leasing.leasingType == LeasingType.GameGuild) {
            ...
            rentPassFromGameGuild(guild, duration, price, renter);
        } else {
            rentPassFromAdmin(leasing, duration, price, renter);
        }
        IERC721(nftToken).transferFrom(address(this), msg.sender, tokenId);

        if (nftToken == address(geniPass)) {
            require(validatePass(tokenId));
            
            IGENIPass(address(geniPass)).permit(msg.sender, address(this), tokenId, sig);

            IGENIPass(address(geniPass)).setActive(tokenId, true);

            if(!IGENIPass(address(geniPass)).isLocked(tokenId)) {

                IGENIPass(address(geniPass)).lockPass(tokenId);
            }
        } else {
        ...
        emit LendPass(tokenId, renter.amount, renter.startLending, duration);
    }

    function rentPassFromGameGuild(LeasingGuildInfo storage guild, uint duration, uint price, RenterInfo storage renter) internal {
        require(guild.price > 0, "Leasing: not on game guild");
        require(duration >= guild.minDuration && duration <= guild.minDuration, "Leasing: invalid duration");
        ...
        renter.amount = rentalPriceGeni;
    }

    function rentPassFromAdmin(LeasingInfo storage leasing, uint duration, uint price, RenterInfo storage renter) internal {
        require(duration >= leasing.minDuration && duration <= leasing.minDuration, "Leasing: invalid duration");
    ...
    function increaseMortgage(address nftToken, uint amount, uint tokenId) external {
        ...
        if (leasing.leasingType == LeasingType.GameGuild && guild.price > 0 && msg.sender == renter.renter) {
    ...
    function userReturnPass(address nftToken, uint tokenId) external {
        ...
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo storage guild = leasingGuildInfo[nftToken][tokenId];
        require(renter.amount > 0,"GEMUNILending: not found lending info");
        require(block.timestamp <= renter.startLending + renter.duration, "GEMUNILending: can not return pass");
        if (msg.sender == guild.gameGuild) {
            require(renter.renter == address(0), "Leasing: need to retrieve pass form user");
            guildReturnNFT(leasing, nftToken, guild);
            delete leasingGuildInfo[nftToken][tokenId];
        } else if (msg.sender == renter.renter) {
            userReturnNFT(renter, leasing, nftToken, guild);
            delete renterInfo[nftToken][tokenId];
        } else {
            revert("Leasing: invalid lender");
        }
        if (nftToken == address(geniPass)) {
            IGENIPass(address(geniPass)).unLockPass(tokenId);
            IGENIPass(address(geniPass)).setActive(tokenId, false);
    ...
    function userReturnNFT(RenterInfo storage renter, LeasingInfo storage leasing, address nftToken, LeasingGuildInfo storage guild) internal {
        ...
        if (block.timestamp > endLending) {
            ...
            if (nftToken == address(geniPass)) {
                ...
                if (leasing.leasingType == LeasingType.GameGuild) {
                    ...
                    currentInterest = guild.price * (block.timestamp - endLending);
                } else {
                    debtAmount = (block.timestamp - endLending) * leasing.price;
                    currentInterest = leasing.price * (block.timestamp - endLending);
                }
                geni.transferFrom(msg.sender, address(this), debtAmount);
            } else {
                if (leasing.leasingType == LeasingType.GameGuild) {
                    currentInterest = renter.addedDuration * guild.price;
                    geni.transfer(guild.gameGuild, currentInterest + leasing.depositAmount);
                } else {
                    currentInterest = renter.addedDuration * leasing.price;
                    geni.transfer(treasury, currentInterest + leasing.depositAmount);
                }
            }
        } else if (block.timestamp < endLending) {
            uint returnAmount;
            if (leasing.leasingType == LeasingType.GameGuild) {
                returnAmount = (endLending - block.timestamp) * guild.price;
            } else {
                returnAmount = (endLending - block.timestamp) * leasing.price;
            }
            geni.transfer(msg.sender, returnAmount);
        }
    }

    function guildReturnNFT(LeasingInfo storage leasing, address nftToken, LeasingGuildInfo storage guild) internal {
        ...
        if (block.timestamp > endLending) {
            ...
            if (nftToken == address(geniPass)) {
                ...
                currentInterest = guild.addedDuration * leasing.price;
                geni.transfer(guild.gameGuild, leasing.depositAmount + currentInterest);
            }
        } else if (block.timestamp < endLending) {
            uint returnAmount = (endLending - block.timestamp) * leasing.price;
            geni.transfer(msg.sender, returnAmount);
        }
    }

    function guildRetrieveNFT(address nftToken, uint tokenId, address to) public {
        RenterInfo storage renter = renterInfo[nftToken][tokenId];
        // LeasingInfo memory leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo storage guild = leasingGuildInfo[nftToken][tokenId];
    ...
    function adminRetrieveNFT(address nftToken,uint tokenId, address to) public onlyOperator {
        require(validatePass(tokenId));
        RenterInfo storage renter = renterInfo[nftToken][tokenId];
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo storage guild = leasingGuildInfo[nftToken][tokenId];
        ...
        if (nftToken == address(geniPass)) {
            ...
            //guild rent pass, retrieve from renter
                ...
                transferPass(tokenId, renter.renter, to) ;
    ...
    //     }
    // }
    function transferPass(uint tokenId, address from, address to) internal {
        IGENIPass(address(geniPass)).unLockPass(tokenId);
        IGENIPass(address(geniPass)).setActive(tokenId, false);
        LeasingInfo storage leasing = leasingInfo[address(geniPass)][tokenId];
        if(leasing.isTransferWallet) {
            geniPass.transferFrom(from, to, tokenId);
            delete leasingInfo[address(geniPass)][tokenId];
        } else {
            geniPass.transferFrom(from, address(this), tokenId);
        }
    }
    ...
    function withdrawPass(address nftToken, uint tokenId) external onlyOperator {
        require(validatePass(tokenId));
        require(geniPass.ownerOf(tokenId) == address(this) && leasingInfo[nftToken][tokenId].lender == address(0), "GEMUNILending: not found token in contract");
