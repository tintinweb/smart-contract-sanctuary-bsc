...
contract TEOMB is ERC20Burnable, Operator {
     ...
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TEOMB.cash", "TEOMB") {
    ...
    function _getHelioPrice() internal view returns (uint256 _helioPrice) {
        try IOracle(helioOracle).consult(address(this), 1e18) returns (uint144 _price) {
    ...
    function _updateTaxRate(uint256 _helioPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_helioPrice >= taxTiersTwaps[tierId]) {
    ...
    function setHelioOracle(address _helioOracle) public onlyOperatorOrTaxOffice {
        require(_helioOracle != address(0), "oracle address cannot be 0 address");
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentHelioPrice = _getHelioPrice();
            currentTaxRate = _updateTaxRate(currentHelioPrice);
            if (currentHelioPrice < burnThreshold) {
                burnTax = true;
            }
        }
