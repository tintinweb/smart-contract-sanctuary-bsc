...
contract RouterEarn is Context, Auth {
    ...
    function deposit() external payable {
        ...
        uint256 amountReflection = getAmountPercent(baseAmount, percentReflection);
        uint256 amountFarming = getAmountPercent(baseAmount, percentReflection);
        uint256 amountStaking = getAmountPercent(baseAmount, percentStaking);
        if(addressReflection != address(0)) IEarn(addressReflection).deposit{value:amountReflection}();
        if(addressStaking != address(0)) IEarn(addressStaking).deposit{value:amountStaking}();
        if(addressFarming != address(0)) IEarn(addressFarming).deposit{value:amountFarming}();
    }
