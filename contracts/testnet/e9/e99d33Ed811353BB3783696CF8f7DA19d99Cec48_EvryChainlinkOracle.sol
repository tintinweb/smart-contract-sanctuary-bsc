...
contract EvryChainlinkOracle is PriceOracle {
    ...
    function getUnderlyingPrice(EToken eToken) public view override returns (uint) {
    ...
    function getPrice(EToken eToken) internal view returns (uint price) {
        BEP20Interface token = BEP20Interface(EBep20(address(eToken)).underlying());
    ...
    function setUnderlyingPrice(EToken eToken, uint underlyingPriceMantissa) external onlyAdmin() {
        address asset = address(EBep20(address(eToken)).underlying());
...
abstract contract PriceOracle {
    ...
    /**
      ...
      */
    function getUnderlyingPrice(EToken eToken) external view virtual returns (uint);
}
...
contract EBep20 is EToken, EBep20Interface {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function liquidateBorrow(address borrower, uint repayAmount, ETokenInterface vTokenCollateral) external override returns (uint) {
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, ETokenInterface vTokenCollateral) internal nonReentrant returns (uint, uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, ETokenInterface vTokenCollateral) internal returns (uint, uint) {
