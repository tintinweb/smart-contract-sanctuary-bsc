...
interface IKepler22bToken is IERC20 {
...
contract KeplerFarmStake is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    IERC721 public keplerFarmToken;
    IKepler22bToken public kepler22bToken;
    ...
    modifier requireKepler22bToken() {
        require(address(kepler22bToken) != address(0), "kepler22bToken not set");
        _;
    }

    modifier requireKeplerFarmToken() {
        require(address(keplerFarmToken) != address(0), "keplerFarmToken not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setKepler22bTokenAddress(address kepler22bTokenAddr) public onlyOwner() {
      kepler22bToken = IKepler22bToken(kepler22bTokenAddr);
    }

    function setKeplerFarmTokenAddress(address keplerFarmAddr) public onlyOwner() {
      keplerFarmToken = IERC721(keplerFarmAddr);
    }
    ...
    function depositsOf(address account)
      external 
      view 
      requireKepler22bToken
      returns (uint256[] memory)
    {
      EnumerableSet.UintSet storage depositSet = _deposits[account];
      uint256[] memory depositSetOtherContract = kepler22bToken.depositsOf(account);
    ...
    {
      uint256[] memory tokensInOldContract = kepler22bToken.depositsOf(account);
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        uint256 lastBlock = arrayContains(tokensInOldContract, tokenId) ? 
          kepler22bToken._depositBlocks(account, tokenId) 
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused requireKepler22bToken {
      uint256 reward; 
      uint256 blockCur = Math.min(block.number, expiration);
      uint256[] memory tokensInOldContract = kepler22bToken.depositsOf(msg.sender);
      ...
      kepler22bToken.claimRewards(tokensInOldContract);

      if (reward > 0) {
        kepler22bToken.transfer(msg.sender, reward);
      }
    }

    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireKeplerFarmToken requireKepler22bToken {
        require(msg.sender != address(kepler22bToken), "Invalid address");
        claimRewards(tokenIds);
        for (uint256 i; i < tokenIds.length; i++) {
            keplerFarmToken.safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                ""
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireKeplerFarmToken requireKepler22bToken nonReentrant() {
        claimRewards(tokenIds);
    ...
    function withdrawTokens() external onlyOwner {
        uint256 tokenSupply = kepler22bToken.balanceOf(address(this));
        kepler22bToken.transfer(msg.sender, tokenSupply);
    }
