...
contract Test1 is Context, IERC20, Ownable {
    ...
    string private _name = "TESTTOKEN"; //name of token
    string private _symbol = "TSTKN"; //symbol of your token 
    ...
    uint256 public _buyLiquidityFee = 300; //300 means 3% 
    ...
    address payable public _devAddress =payable( 0x000000000000000000000000000000000000dEaD);
    ...
    // 2% of Total Supply
    uint256 private numTokensSellToAddToLiquidity = (_tTotal * 2) / 100;
    ...
    function setBuyTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 devFee) external onlyOwner() {
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 marketingFee, uint256 devFee) external onlyOwner() {
     require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(marketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
        require(devFee <= maxDevFee); // Prevents owner from abusing fees.
    ...
    function setMarketingWallet(address payable newMarketingWallet) external onlyOwner {
        require(_marketingWallet != newMarketingWallet, "Wallet already set!");
        _marketingWallet = payable(newMarketingWallet);
    }

     function setDevWallet(address payable newDevWallet) external onlyOwner {
        require(_devAddress != newDevWallet, "Wallet already set!"); 
        _devAddress  = payable(newDevWallet);
    }
    ...
    function _takeDev(address sender, uint256 tDev) private {
        ...
        emit Transfer(sender, _devAddress, tDev); // Transparency is the key to success.
    }
