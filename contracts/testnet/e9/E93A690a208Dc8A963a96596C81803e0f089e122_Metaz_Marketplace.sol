...
contract Metaz_Marketplace{
    ...
    address public addressReceiveTax=0x37E58b4882F5Cc4Af6a2470c26606c7Aa92DB320;
    ...
    event event_item_sold(uint collectionPos);
    ...
    function buy_Item(uint _posItemsOnSell) public payable{
        // check tokenId is onSell
        require(_posItemsOnSell<itemsOnSell.length, "Wrong item on sell position");
        require(itemsOnSell[_posItemsOnSell].status==0, "Item is not on sell");
        require(Collections[itemsOnSell[_posItemsOnSell].collectionPos].Status==1, "Collection is disable.");
        IERC721 currentCollection = IERC721(Collections[itemsOnSell[_posItemsOnSell].collectionPos].Address);
        require(currentCollection.getApproved(itemsOnSell[_posItemsOnSell].tokenId)==address(this));
        require(currentCollection.ownerOf(itemsOnSell[_posItemsOnSell].tokenId)==itemsOnSell[_posItemsOnSell].seller, "Item is not exist.");
        // check allownwance for market.  require(currentCollection.ownerOf());

        if(itemsOnSell[_posItemsOnSell].currencyPos==0){ // CET
            require(msg.value>=itemsOnSell[_posItemsOnSell].price, "Not enough CET");
            if(tax_market>0){
                payable(addressReceiveTax).transfer(itemsOnSell[_posItemsOnSell].price*tax_market/100);
            }
        }else{
            require(itemsOnSell[_posItemsOnSell].currencyPos<currenciesAddress.length, "Wrong currencies address position.");
            IERC20 currentToken = IERC20(currenciesAddress[itemsOnSell[_posItemsOnSell].currencyPos]);
            require(currentToken.balanceOf(msg.sender)>=itemsOnSell[_posItemsOnSell].price,"You dont have token enough.");
            require(currentToken.allowance(msg.sender, address(this))>=itemsOnSell[_posItemsOnSell].price, "Please approve token");
            currentToken.transferFrom(msg.sender, address(this), itemsOnSell[_posItemsOnSell].price*(100-tax_market)/100);
            if(tax_market>0){
                currentToken.transferFrom(msg.sender, addressReceiveTax, itemsOnSell[_posItemsOnSell].price*tax_market/100);
            }
        }

        //transfer token ERC721
        currentCollection.transferFrom(itemsOnSell[_posItemsOnSell].seller, msg.sender, itemsOnSell[_posItemsOnSell].tokenId);

        // update status success after check $ is enough
        itemsOnSell[_posItemsOnSell].buyer = msg.sender;
        itemsOnSell[_posItemsOnSell].start_to_buy = block.timestamp;
        itemsOnSell[_posItemsOnSell].status = 1;

        emit event_item_sold(_posItemsOnSell);
    }

    // for administrator
    function update_Collections_status(uint collectionPos, uint newStatus) public checkOwner{
        require(collectionPos<Collections.length, "Weong collections position");
        Collections[collectionPos].Status = newStatus;
    }

    function add_new_Collections(address newCollection) public checkOwner{
        require(newCollection!=address(0), "Wrong collection address");
        Collection memory newCol = Collection(newCollection, 1);
        Collections.push(newCol);
    }

    function update_Tax(uint _tax_market) public checkOwner{
