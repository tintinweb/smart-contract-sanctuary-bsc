...
contract Token is INEW, Context, Ownable {
    using SafeMath for uint256;
    ...
    struct Fees {
        ...
        uint256 goldmineFee;
        uint256 metagamedevFee;
        address marketingAddress;
        address developmentAddress;
        address liquidityAddress;
        address goldmineAddress;
        address metagamedevAddress;
    }
    ...
    address public WBNB;
    uint256 public _maxTxAmount = 1 * 10**16; // 10,000,000 (10 million, e.g., 1% of the total supply)
    ...
    constructor(address _router, address _rewardAdmin) {
        ...
        _defaultFees = Fees(
            ...
            0x0E714CF5E1b5CE80F18Bc96b232E06916c3343E0, // development
            0x069854a3fEF5eF4D0079386483B05081E5728370, // liquidity
            0x62779D0272415B82367e9bC3b070efc7a29034C4, // goldmine 
            0x6e96b5f23ab2e556551b5D2f88a073d955988168  // MGD-Admin
        );

        // Set base token in the pair as WBNB, which acts as the tax token
        INEWSwapPair(newSwapPair).setBaseToken(WBNB);
        INEWSwapPair(newSwapPair).updateTotalFee(1500); // sum of the defaultFees
    ...
    function setGoldminePercent(uint256 _goldmineFee) external onlyOwner {
        _defaultFees.goldmineFee = _goldmineFee;
        _updatePairsFee();
    }
    function setmetagamedevPercent(uint256 _metagamedevFee) external onlyOwner {
        _defaultFees.metagamedevFee = _metagamedevFee;
        _updatePairsFee();
    }
    ...
    function setmetadgamedevAddress(address _metagamedev) external onlyOwner {
        require(_metagamedev != address(0), "NEW: Address Zero is not allowed");
        _defaultFees.metagamedevAddress = _metagamedev;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        FeeValues memory values = FeeValues(
            ...
            calculateFee(tAmount, _defaultFees.metagamedevFee)
        );
        

        values.transferAmount = tAmount.sub(values.reflection).sub(values.marketing).sub(values.liquidity).sub(values.goldmine).sub(values.metagamedev);
        return values;
    }
    ...
    function getTotalFee() internal view returns (uint256) {
        return _defaultFees.reflectionFee
            ...
            .add(_defaultFees.metagamedevFee);
    }
    ...
    function depositLPFee(uint256 amount, address token) public onlyExchange {
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                uint256 developmentFeeAmount = amount.mul(_defaultFees.marketingFee).div(totalFee).div(2);
                ...
                uint256 metagamedevFeeAmount = amount.mul(_defaultFees.metagamedevFee).div(totalFee);
                ...
                IERC20(token).transfer(_defaultFees.metagamedevAddress, metagamedevFeeAmount);
