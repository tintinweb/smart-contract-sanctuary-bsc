...
contract AwardPool {
    ...
    function pendingAward(address _user) external view returns(uint256){
        ...
        if(userInfo[_user].trueReward.add(
            ...
            ) >= userInfo[_user].maxReward){
            return userInfo[_user].maxReward.sub(userInfo[_user].trueReward);
        }
    ...
    function pledge(uint256 _amount, address _leader) public notPause {
		...
		userInfo[msg.sender].maxReward = _amount.div(poolInfo.price).mul(poolInfo.multiple);
    ...
    function handling(uint256 _amount, address _from, address _leader)internal{
        ...
        if(userInfo[_from].leader !=address(0)){
			...
			// share 30%
			userInfo[currentLeader].amount = userInfo[currentLeader].amount.add(_amount.mul(poolInfo.shareRate).div(100));
			...
			for(uint256 i = 2; i > 0; i--){
				...
				if(userInfo[currentLeader].performance > guildMap[i].minPerformance  && userInfo[currentLeader].inviteNum > guildMap[i].minInviteNum){
					userInfo[currentLeader].amount = userInfo[currentLeader].amount.add(_amount.mul(guildMap[i].awardRate).div(100));
					break;
				}
			}
		}
    ...
    function withdraw() public  notPause {
        ...
        if(userInfo[msg.sender].trueReward.add(pending) >= userInfo[msg.sender].maxReward){
            pending = userInfo[msg.sender].maxReward.sub(userInfo[msg.sender].trueReward);
