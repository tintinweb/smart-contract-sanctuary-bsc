 ...
 * ```
 */
contract cecMigrator is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  */
  ...
  function initialize(address legacyToken, address newToken, cecVestingVault  vesting) public initializer {
  ...
  function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    /**
     * @dev Addding a account to Blacklisting
  ...
  function migrate(address account, uint256 amount) public {
    require(!_blacklist[account], "blacklist: Your Account has been blacklisted");
    ...
    vestingToken.addTokenGrant(account, balanceAmount, vestingMonths, 1);
  ...
  */
  ...
  /**
    ...
    * @param months update the number of months
    */
  function updateVestingMonths(uint256 months) external onlyOwner whenNotPaused {
...
contract cecVestingVault is Initializable, OwnableUpgradeable, UUPSUpgradeable  {
        ...
        external
    {
        ...
        if(tokenGrants[_recipient].amount == 0){
            ...
            if(tokenGrant.monthsClaimed < tokenGrant.vestingDuration){
                tokenGrant.amount = uint256(tokenGrant.amount.add(_amount));
                emit GrantUpdateAmount(_recipient, tokenGrant.amount, _amount);
            } else if(tokenGrant.monthsClaimed == tokenGrant.vestingDuration) {
                tokenGrant.startTime = currentTime().add(_lockDurationInMonths.mul(intervalTime));
                tokenGrant.amount = _amount;
                tokenGrant.vestingDuration = _vestingDurationInMonths;
