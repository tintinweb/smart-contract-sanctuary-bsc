...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function distributeTokenDividends() external onlyOwner {
    console.log("in dtddddddddddddddddd",totalBalance);
    if(totalBalance > 0){
        console.log("axasssssssxaxasxxxxxxxxxxxxxxxx");
        uint256 tokensToAdd;
        console.log("address tokens USDC",address(token));
        uint256 balance = IERC20(token).balanceOf(address(this));
        console.log("balance",balance);
        console.log("totalDividendsWaitingToSend",totalDividendsWaitingToSend);
        ...
        if (tokensToAdd > 0) {
          console.log("zqqwqeqed", tokensToAdd);
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    console.log("_withdrawableDividend",_withdrawableDividend);
  ...
  function withdrawableDividendOf(address _owner) public view override returns(uint256) {
    console.log("aaccumulative", accumulativeDividendOf(_owner),   withdrawnDividends[_owner]  );
  ...
  function accumulativeDividendOf(address _owner) public view override returns(uint256) {
    console.log("holderBalance[_owner]",holderBalance[_owner]);
...
contract EHXLPStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public unstakeLockDuration = 10 days;
...
////// CHANGES
    ...
    function totalStakeEarned(address _user) public view returns(uint256){
        UserInfo storage user = userInfo[_user];
        return user.unstakeAmount;
    }
    
    function viewRewardsEarned(address _user) public view returns(uint256){
        uint256 pending = pendingReward(_user);
        return rewardsClaimed[_user] + pending;
    }

    function viewRewardsCollected(address _user) public view returns(uint256){
        return rewardsClaimed[_user];
    }

        function collectEHX(address _user) public{
        // UserInfo storage user = userInfo[_user];
        rewardsClaimed[_user] += rewardsPending[_user];                     ////////////////////////////////
        rewardToken.transfer(address(_user), rewardsPending[_user]);
    }
////////
