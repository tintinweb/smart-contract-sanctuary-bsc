...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        finalizeTokenId(insConfiguration.tokenId, insId);
    ...
    function activateTokenId(uint256 _tokenId) private {
        ...
        for (uint256 i = 0; i < activeInsTokenIds.length; i++) {
            if (activeInsTokenIds[i] == _tokenId) {
                enableToActivate = false;
            }
        }

        if (enableToActivate) {
            activeInsTokenIds.push(_tokenId);
        }

        // We need to back remove _tokenId from finished
        for (uint256 i = 0; i < finishedInsTokenIds.length; i++) {
            if (finishedInsTokenIds[i] == _tokenId) {
                delete finishedInsTokenIds[i];
            }
        }
    }

    function finalizeTokenId(uint256 _tokenId, uint256 _insId) private {
        INSConfiguration[] memory tokenIns = getTokenIns(_tokenId);

        bool allInsForTokenIdFinalized = true;

        for (uint256 i = 0; i < tokenIns.length; i++) {
            if (tokenIns[i].valid && tokenIns[i].insId != _insId) {
        ...
        if (allInsForTokenIdFinalized) {
            for (uint256 i = 0; i < activeInsTokenIds.length; i++) {
                if (activeInsTokenIds[i] == _tokenId) {
                    ...
                    finishedInsTokenIds.push(_tokenId);
                }
            }
        }
    }
    ...
    function getFinishedInsTokenIds() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < finishedInsTokenIds.length; i++) {
            if (finishedInsTokenIds[i] != 0) {
                correctArraySize += 1;
            }
        }

        uint256[] memory finishedIns = new uint256[](correctArraySize);
