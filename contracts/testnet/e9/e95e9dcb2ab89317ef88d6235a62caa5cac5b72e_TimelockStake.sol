...
contract TimelockStake
{
    ...
    /* *****************************
        POOL MANAGEMENT
    *  *****************************/
    function createPool(uint offchainId, address stakingToken, uint period, uint256 apr, uint locked) external onlyOwner
    {
        require(stakingToken != address(0), "ZERO_ADDRESS"); // validate address
    ...
    function setPoolDisabledDeposit(uint poolID, uint disabledValue) external onlyOwner
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
        poolDepositDisabled[poolID] = disabledValue;
    }

    function setPoolOffchainId(uint poolID, uint offchainId) external onlyOwner
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
        poolOffchainId[poolID] = offchainId;
    }

    function destroyPool(uint poolID) external onlyOwner
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
    ...
    *  *****************************/
    ...
    function getInterestReward(uint256 stakedAmount, uint256 apr, uint stakePeriod) public pure returns (uint256, uint256, uint256)
    {
    ...
    *  *****************************/
    ...
    function forcePlayerToWithdraw(uint poolID, uint poolRecordIndex, address player) external onlyOwner
    {
        _poolStakeWithdraw(poolID, poolRecordIndex, player);
    }

    function forcePlayerToWithdrawWholePool(uint poolID, address player) external onlyOwner
    {
    ...
    function transferNetworkCoinFund(address to, uint256 amountInWei) external onlyOwner
    {
        //Withdraw Network Coin
        payable(to).transfer(amountInWei);
    }

    function erasePlayerDataInPool(uint poolID, address player) external onlyOwner
    {
        ...
        if(poolSize > 0)
        {
            for(uint ixRecord = 0; ixRecord < poolSize; ixRecord++)
            {
                ...
                uint256 stakePeriod = SafeMath.safeSub(occuredEndTime, startTime);
                (, , uint256 redeemable) = getInterestReward(playerStakeRecordAmount[poolID][player][ixRecord], poolAPR[poolID], stakePeriod);
                emit PlayerErase(ixRecord, playerStakeRecordAmount[poolID][player][ixRecord], redeemable, startTime, occuredEndTime);
            }
        }
