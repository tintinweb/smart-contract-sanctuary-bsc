...
// ERC721
// ERC721A
contract NFTComposite is ERC721, INFTComposite, Caller, Fallback {
    ...
    mapping(uint256 => bytes) private propertyScore;
    ...
    function _setURI(string memory uri_) private {
        ...
        bytes memory str = new bytes(3 + data.length * 2);
        str[0] = "0";
        str[1] = "x";
        str[str.length - 1] = "/";
        ...
        uri = string(abi.encodePacked(uri_, string(str)));
    }
    ...
    function batchMintNFT(
        ...
        bytes[] memory propertyScore_
    ) external override onlyCaller {
        require(maxId.add(1, "") == startTokenId, "");
        require(0 != propertyScore_.length, "");
        startTokenId = maxId;
        bytes memory propertyScore__;
        for (uint256 i = 0; i < propertyScore_.length; i++) {
            startTokenId = startTokenId.add(1, "");
            propertyScore__ = propertyScore_[i];
            require(0 == propertyScore__.length % 12, "");
            propertyScore[startTokenId] = propertyScore__;
            _safeMint(to, startTokenId);
        }
        maxId = startTokenId;
        if (address(0) != address(token)) {
            token.mint(to, propertyScore_.length);
        }
    }
    ...
    function getPropertyScore(uint256 tokenId)
        external
        view
        returns (bytes memory)
    {
        return propertyScore[tokenId];
    }
}
...
interface INFTComposite {
    // uint64,uint32,uint64,uint32,uint64,uint32...[]
