...
contract EXPONA_RBCF {
    ...
    function getTKNrwrd() public view returns (uint) {
        return REGESTRATION_FESS / tokenPrice() * 1e15;
    }
    ...
    function Registration(uint _referrerID, uint _coreferrerID,uint256 _amount) public payable {
        ...
        if (!isTokenAcceptance) {
            ...
            tokenReward =  _amount / tokenPrice() * 1e15;
            }
        } else {
            
            require(_amount == REGESTRATION_FESS/(sqrt(currUserID*1e30)*1e15), "Incorrect Value");
