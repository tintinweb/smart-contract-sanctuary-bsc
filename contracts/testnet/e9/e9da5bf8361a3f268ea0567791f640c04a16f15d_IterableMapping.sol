...
contract SPDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("PUNKS_Dividend_Tracker", "PUNKS_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = totalSupply() / 10000000; //must hold some tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SP_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SP_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SP_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SP: Cannot update claimWait to same value");
...
contract StreetPunks is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SPDividendTracker public dividendTracker;
    ...
    constructor(uint256 initialSupply, address _MarketingWallet, address _router) ERC20("Street Punks", "PUNKS") {
        BNBRewardsFee = 5;
        liquidityFee = 4;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 3;
        marketingWallet = payable(_MarketingWallet);
	    maxSellTransactionAmount = initialSupply / 100000;
    	swapTokensAtAmount = initialSupply / 50000;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new SPDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SP: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SP: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SP: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SP: Cannot update gasForProcessing to same value");
