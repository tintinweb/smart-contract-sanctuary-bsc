...
contract LOTTERY{
  ...
  uint256 [5] public winners_amounts = [1000,500,300,100,50];
  ...
  constructor(){
    ...
    for(uint256 i = 0; i < winners_amounts.length; i++){
      winners_amounts[i] = winners_amounts[i] * (10 ** token.decimals());
    }
  ...
  function setwinners() external onlyAdmin{
    ...
    lotteries[nonce].winner_reward[i] = lotteries[nonce].prize;
