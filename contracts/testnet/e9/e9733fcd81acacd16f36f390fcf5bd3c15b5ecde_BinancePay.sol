...
contract BinancePay is Ownable {
    ...
    uint256 public currUserId = 0 ;
    uint256 public  distributefallbackstatus =0;
    uint256 public  distributereceivestatus =0;
    ...
    fallback() external  payable {
         distributefallbackstatus=1;
     }

    receive() external payable {
         distributereceivestatus = 1;
    ...
    function payForLevel(uint _level, address _user) internal {
     ...
     function distribute() internal{
           ...
           if(royalparticipants.length > 0){
               uint256 royalityshare = (royalityAmount/royalparticipants.length);
                for(uint i=0; i < royalparticipants.length;i++){
                    users[royalparticipants[i]].earnedAmount += royalityshare;
                    users[royalparticipants[i]].royalityincome = royalityshare;
                }
           }
