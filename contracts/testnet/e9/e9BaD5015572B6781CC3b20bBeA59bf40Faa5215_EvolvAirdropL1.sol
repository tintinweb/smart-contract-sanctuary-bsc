...
import "./token/EvolvSettleable.sol";

/// @dev This module is supposed to be used in layer 1 (settlement layer).

contract EvolvAirdropL1 is Ownable, EvolvSettleable, Pausable {
    ...
    ) EvolvSettleable(bridgeAddress) ERC721(name, symbol) {
...
import "../../core/Evolv.sol";
import "../utils/rollup/Settleable.sol";

import "../utils/rollup/RollupTypes.sol";

abstract contract EvolvSettleable is Evolv, Settleable, AccessControl {
...
import "./interfaces/IEvolv.sol";
import "hardhat/console.sol";

abstract contract Evolv is ERC721, IEvolv {
    ...
    mapping (uint256 => mapping( bytes32 => bool)) private _utilities;

    function setBaseURI(string memory URI, string memory ext) internal virtual{
        _URI = URI;
        _extension = ext;

        emit URIUpdated(_URI, ext);
    }
    ...
    function baseExt() internal view returns (string memory) {
        return _extension;
    }
    ...
    function setLevel(uint256 tokenId, uint256 level) public virtual {
    ...
    function levelUp(uint256 tokenId) public virtual {
        // require(_exists(tokenId), "Evolv: Level Up of nonexistent token");
        _levels[tokenId] += 1;

        emit NFTLeveledUp(tokenId, _levels[tokenId]);
    }

    function unlockUtility(uint256 tokenId, bytes32 utilitySlug) internal virtual {
        require(_exists(tokenId), "Evolv: Utility unlock of nonexistent token");

        _utilities[tokenId][utilitySlug] = true;

        emit UtilityUnlocked(tokenId, utilitySlug);
    }

    function levelUpNFTWithUtility(uint256 tokenId, bytes32 utilitySlug) internal virtual {
        require(_exists(tokenId), "Evolv: Level Up and Utility unlock of nonexistent token");
        _levels[tokenId] += 1;
        _utilities[tokenId][utilitySlug] = true;

        emit NFTUtilityLevelUp(tokenId, _levels[tokenId], utilitySlug);
    }

    function toggleUtility(uint256 tokenId, bytes32 utilitySlug) internal virtual {
        require(_exists(tokenId), "Evolv: Toggle utility of nonexistent token");
        _utilities[tokenId][utilitySlug] = _utilities[tokenId][utilitySlug] ? false : true;

        emit ToggledUtility(tokenId, utilitySlug);
    }

    function levelOf(uint256 tokenId) public view virtual override returns (uint256) {
        // require(_exists(tokenId), "Evolv: Level of nonexistent token");

        return _levels[tokenId];
    }

    function isUtilityUnlocked(uint256 tokenId, bytes32 utilitySlug) public view virtual override returns (bool) {
        require(_exists(tokenId), "Evolv: Level of nonexistent token");

        return _utilities[tokenId][utilitySlug];
    }
}
...
/**
 * @dev Required interface of an Evolv compliant contract.
 */
interface IEvolv {
    ...
    event ToggledUtility(uint256 tokenId, bytes32 utilitySlug);

    /**
     * @dev Get the level of the NFT
     */
    function levelOf(uint256 tokenId) external returns (uint256);

    /**
     * @dev Check if the utility is unlocked for the NFT
     */
    function isUtilityUnlocked(uint256 tokenId, bytes32 utilitySlug) external returns (bool);
}
