...
contract WoolFactory is Whitelist, QueueHelper, ReentrancyGuard {
    ...
    // _itemTier[tierId].saleItems is the 'q'
    // whatever else, is the data
    ...
    function getTierItemsForSale(uint256 _tier) public view returns (uint256[] memory) {
        uint256[] storage _items = (getSaleItemsOfTier(_tier));
        return (_items);
    }
    ...
    function _nextItemOf(uint256 _tier) internal view returns (address, uint256) {
        uint256[] memory saleItems = getSaleItemsOfTier(_tier);
        uint256 _saleId = saleItems[0];
    ...
    function _buyItem(address _recipient, uint256 _tier) internal returns (uint256) {
        ...
        if (_mintable > 0) {
            ...
            QueueHelper.pop(_itemTier[_tier].saleItems);
    ...
    function _listItem(address nftContract, uint256 tokenId, uint8 markupLevel) internal {
        ...
        QueueHelper.push(_itemTier[_tier].saleItems, itemId);
    ...
    function getSaleItemsOfTier(uint _tier) internal view returns (uint256[] storage) {
        return (_itemTier[_tier].saleItems.data);
    }
}