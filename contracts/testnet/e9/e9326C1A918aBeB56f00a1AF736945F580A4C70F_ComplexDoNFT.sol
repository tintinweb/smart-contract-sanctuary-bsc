...
contract Royalty is IRoyalty {
    ...
    function setTempRoyaltyAdmin(address tempAdmin_) internal virtual{
...
contract ComplexDoNFT is WrapDoNFT,Royalty,IComplexDoNFT{
    ...
    function initialize(string memory name_, string memory symbol_,address nftAddress_,address market_,address tempRoyaltyAdmin_) public virtual initializer {
        super._BaseDoNFT_init(name_,symbol_,nftAddress_,market_);
        setTempRoyaltyAdmin(tempRoyaltyAdmin_);
    }

    function mintXNft(uint256 oid) public nonReentrant override(WrapDoNFT,IBaseDoNFT) returns (uint256 tid){
        require(oid2xid[oid] == 0, "already warped");
        address lastOwner;
