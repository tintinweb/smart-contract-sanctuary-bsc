...
// File: contracts\interfaces\IFarmageddonRouter01.sol

pragma solidity >=0.6.2;

interface IFarmageddonRouter01 {
...
// File: contracts\interfaces\IFarmageddonRouter02.sol

pragma solidity >=0.6.2;

interface IFarmageddonRouter02 is IFarmageddonRouter01 {
...
library FarmageddonLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                // hex'00fb7f630766e6a796048ea87d01acd3068e8ff67d078148a3fa3f4a84f69bd5' // live
                hex'912052b8e092c1393c3fc186236fa46fc8e76089dd7717441eb9459d70cbdab8' // init code hash testnet
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(9965);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        ...
        uint denominator = reserveOut.sub(amountOut).mul(9965);
...
contract FGRouter is IFarmageddonRouter02, Ownable {
    using SafeMath for uint;
    ...
    function _addLiquidity(
        ...
        if (IFarmageddonFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IFarmageddonFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = FarmageddonLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = FarmageddonLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'FarmageddonRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = FarmageddonLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'FarmageddonRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = FarmageddonLibrary.pairFor(factory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IFarmageddonPair(pair).mint(to);
    }
    function addLiquidityETH(
        address token,
        uint amountTokenDesired,
        ...
        address pair = FarmageddonLibrary.pairFor(factory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = FarmageddonLibrary.pairFor(factory, tokenA, tokenB);
        IFarmageddonPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IFarmageddonPair(pair).burn(to);
        (address token0,) = FarmageddonLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'FarmageddonRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'FarmageddonRouter: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityETH(
        address token,
        uint liquidity,
    ...
    function removeLiquidityWithPermit(
        ...
        IFarmageddonPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = FarmageddonLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return FarmageddonLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return FarmageddonLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return FarmageddonLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return FarmageddonLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return FarmageddonLibrary.getAmountsIn(factory, amountOut, path);
    }
}