...
contract BitcoinUpDown is Ownable {
    ...
    function getIsExist(address _addr) public view returns (uint256) {
        if (players_bet[_addr].isExist == true) {
            return players_bet[_addr].bet;
        }
        return 0;
    }

    function getUpDown(address _addr) public view returns (UP_DOWN) {
        if (players_bet[_addr].isExist == true) {
            return players_bet[_addr].up_down;
        }
    }

    function _startBet() private {
        prize_pool.push(0);
        prize_pool_up.push(0);
        prize_pool_down.push(0);
        price_open = getPrice();
        ts_start = block.timestamp;
        emit eventStartBet(round, ts_start, price_open);
    }
