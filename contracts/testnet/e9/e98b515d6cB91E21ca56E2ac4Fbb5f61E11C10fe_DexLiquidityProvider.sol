...
contract DexLiquidityProvider is DexBase, IDexLiquidityProvider, Initializable, ReentrancyGuardUpgradeable {
    ...
    function deleteLiquidityProvider(address lpIn, address lpOut) external onlyOwner {
        ...
        if (amt > 0) {
            IERC20Upgradeable(compensateToken).safeTransfer(lpOut, amt);
        }
...
contract DexBase {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "DexLiquidityProvider: caller is not the owner");
        _;
    }

    modifier onlyLiquidityProvider() {
        require(liquidityProviders[msg.sender] > 0, "DexLiquidityProvider: caller is not the liqidity provider");
        _;
    }

    modifier onlyBabtUser() {
        require(!babSwitch || babToken.balanceOf(msg.sender) > 0, "DexLiquidityProvider: caller is not a BABToken holder");
        _;
    }
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "DexLiquidityProvider: new owner is the zero address");
        require(newOwner != _owner, "DexLiquidityProvider: new owner is the same as the current owner");
    ...
    function acceptOwnership() external {
        require(msg.sender == _pendingOwner, "DexLiquidityProvider: invalid new owner");
    ...
    function setBabTokenAddress(address newBabToken) external onlyOwner {
        require(newBabToken != address(0), "DexLiquidityProvider: new BAB token is the zero address");
