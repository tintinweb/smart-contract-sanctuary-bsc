...
contract DV2TokenMine is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      uint256 currentBasePrice = checkMultiplier().mul(basePrice).div(100);
      uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice).div(100);
      if(isMultiEnabled){
    ...
    // Set Excluded From Fees users
    function readPower() public view returns(uint256 mul1,uint256 mul2,uint256 mul3,uint256 mul4,bool _status1,bool _status2,bool _status3) {
        uint256 buyprice = buyPriceNow();
        uint256 sellprice = sellPriceNow();
        uint256 currentBasePrice = checkMultiplier().mul(basePrice).div(100);
        uint256 currentSellTargetPrice = checkMultiplier().mul(sellTargetPrice).div(100);
        ...
        _status3 = timeStart <= block.timestamp;
    }
