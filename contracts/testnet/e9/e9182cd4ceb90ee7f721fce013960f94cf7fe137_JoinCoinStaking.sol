...
contract JoinCoinStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public numVestingPeriods;
    IBEP20 public immutable joinToken;
    ...
    constructor(
        ...
        joinToken = IBEP20(0xaEF6C981F821B529a3b074Bf8Bbee65803381969);
        rewardToken = joinToken;
        rewardPerBlock = 50 * 1e18;
        startBlock = block.number;
        bonusEndBlock = 99999999;

        // staking pool
        poolInfo.push(PoolInfo({
            lpToken: joinToken, // JoinCoin
            allocPoint: 1000,
        ...
        vestingPeriod = 5 minutes; // Update to correct time period
        numVestingPeriods = 9; // 9 periods, because 10% is given initially
    ...
    function userAbleToWithdraw(address userWallet) public view returns (bool){
        UserInfo storage user = userInfo[userWallet];
        return block.timestamp >= user.nextWithdraw;
    }

    // Withdraw primary tokens from STAKING. One withdraw per unlock.
    function withdraw() public nonReentrant {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        uint256 _amount = user.amountPerUnlock;
        require(userAbleToWithdraw(msg.sender), "Unable to withdraw tokens.  Check nextWithdraw timing.");
        user.nextWithdraw += vestingPeriod;
        user.lastWithdraw = block.timestamp;
        require(user.amount >= _amount, "withdraw: attempting to withdraw too many tokens");
        require(_amount <= rewardsRemaining(), "Cannot withdraw other people's staked tokens.  Contact an admin.");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accTokensPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            rewardToken.safeTransfer(address(msg.sender), pending);
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        require(_amount <= rewardToken.balanceOf(address(this)) - totalStaked, 'not enough token');
    ...
    function stakeForVesters(address[] calldata wallets, uint256[] calldata amount) external onlyOwner() {
        PoolInfo storage pool = poolInfo[0];
        updatePool(0);
        
        uint256 totalAirdropAmount =  0;
        require(wallets.length == amount.length, "stakeForVesters:: Arrays must be the same length");
        for(uint256 i = 0; i < wallets.length; i++){
            address wallet = wallets[i];
            UserInfo storage user = userInfo[wallet];
            uint256 airdropAmount = amount[i];
            user.amount = user.amount.add(airdropAmount);
            user.lastWithdraw = block.timestamp;
            user.nextWithdraw = block.timestamp + vestingPeriod;
            user.amountPerUnlock += airdropAmount / numVestingPeriods;
            totalStaked += airdropAmount;
        ...
        // ensure owner approval on the JoinCoin token.
        pool.lpToken.transferFrom(msg.sender, address(this), totalAirdropAmount);
    }
}