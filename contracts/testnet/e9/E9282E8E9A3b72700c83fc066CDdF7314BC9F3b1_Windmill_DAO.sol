...
contract Windmill_DAO is Base, Math, Adressable, AccessControl, With_DAORole, Terminatable, With_UpdaterRole,
    ...
    ////
    //////////////// Public variables ////////////////
    uint256 public constant nbProposals = 49;
    ...
    bool public investorWhitelistEnabled;
    ...
    modifier onlyGovernor(){
        require(users[msg.sender].level == 4, "Windmill_DAO: Only governor are allowed to call this function.");
        _;
    }
    ...
    function addUser(address addr, uint8 level) external onlyRole(DAO_ROLE){
        require(level <= 4, "Windmill_DAO: the max level is 4");
        ...
        updater.setUserGovernorStatus(addr, level == 4);
    }
    ...
    function checkProposal(uint16 id, uint256 paramsUint256, address paramsAddress) external view returns(bool){
        ...
        return _checkProposalParameters(proposal);
    }
    ...
    function _updateProposal(Proposal storage proposal, uint256 voteLevel) internal{
        ...
        if (percentYes >= data.majorityPercent){
            if (totalVotes >= data.quorum || (data.quorumTrigger > 0 && totalVotes >= data.quorumTrigger)){
                    ...
                    if (id<20){
                        if (id<18){
                            if (id==16){
                                ...
                                if (users[proposal.paramsAddress].level < 3){
                            ...
                            if (id==18){
                                tradeManager.setTraderLevel(proposal.paramsAddress, 4);
                                users[proposal.paramsAddress].level = 4;
                    ...
                    if (id<44){
                        ...
                        if (id<46){
                            ...
                            if (id==46){
                                ...
                                stacking.setMaxCycleLock(proposal.paramsUint256);
                            }
                        }
                    }
                }
            }else{ // id==48
                tradeManager.disableTradeDeployer(proposal.paramsUint256);
            }
        }
    }
    ...
    function _checkProposalParameters(Proposal memory proposal) internal view returns (bool){
        ...
        if (id<32){
           ...
           if (id<48){
                ...
                if (   proposal.paramsAddress != address(0x0)
                    || proposal.paramsUint256 >= tradeManager.getNbTradeDeployers()){
                    return false;
                }
            }
        }
