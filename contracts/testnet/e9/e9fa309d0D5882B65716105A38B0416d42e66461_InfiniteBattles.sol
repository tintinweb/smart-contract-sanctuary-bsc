...
contract InfiniteBattles is Ownable {
    ...
    uint256 public blockIds = 0;
    IERC20 public blsToken;

    event EmergencySweepWithdraw(address indexed user, IERC20 indexed token, uint256 amount);
    event BattleInitiated(address indexed user, uint256 indexed blockId, uint256 attackerId_);
    ...
    event BlockCreated(uint256 indexed blockId, address rewardToken, uint256 amountRewards, uint256 runsForSeconds);

    constructor(IERC20 blsToken_) {
        blsToken = IERC20(blsToken_);
    }

    function initiateBattle(uint256 blockId_, uint256 attackerId_) external {
        ...
        blocks[blockId_].attacker = attackerId_;
        blocks[blockId_].attackerAddress = msg.sender;
    ...
    function battleWinner(uint256 blockId_, address winner_) external {
    ...
    function createBlock(IERC20 rewardToken_, uint256 amountRewards_, uint256 runsForSeconds_) external onlyOwner {
        ...
        emit BlockCreated(blockIds, address(rewardToken_), amountRewards_, runsForSeconds_);
        ++blockIds;
    }

    function depositRewards(IERC20 token, uint256 amountOfTokens) external {
