...
contract Fairlaunch is Ownable
{
    ...
    function SetParams(uint256 rewardRatio,address rewardToken, 
    address vaultRouter,address dev,address auction,address chainlink,address spareUtilityVault ) public onlyOwner
    {
        ...
        Dev = dev;
        Auction = auction;
        Chainlink =  PriceOracle(chainlink);
    ...
    function SetPool(address dev, address stakeToken, uint256 allocPoint, uint256 poolId) public onlyOwner{
    ...
    function UpdatePool(uint256 poolId)  public onlyRegisteredUsers
    {
        ...
        uint256 reward = 0;
        {
            (uint256 price,) = Chainlink.getPrice(IUtilityToken(pool.StakeToken).BaseToken(),
        address(0));
        emit ChainlinkPrice(price);
        uint256 totalReward = 
            pool.StakeBalance.mul(RewardRatio).div(ACC_REWARDRATIO_PRECISION)
            .mul(block.timestamp - pool.LastRewardTime).div(365).div(86400);
            reward = totalReward.mul(price).div(1e18).mul(pool.AllocPoint).div(TotalAllocPoint);
        }
        {
    ...
    function PendingUtility(uint256 workerId, address to)  public view returns(uint256)
    {
        WorkerInfo storage worker = Workers[workerId];
        UserInfo storage user = WorkerUsers[workerId][to];
        uint256 useramount = IERC20(IVaultRouter(VaultRouter).ShareTokens(workerId)).balanceOf(to);
        uint256 accumulatedUtility = useramount.mul(worker.AccRewardPerShare).div(ACC_REWARD_PRECISION);
        return accumulatedUtility.sub(user.RewardDebt);
    }
