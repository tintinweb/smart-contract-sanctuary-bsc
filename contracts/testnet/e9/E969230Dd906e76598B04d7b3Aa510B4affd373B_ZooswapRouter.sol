...
// File: bitbyte-swap/libs/Ownable.sol
...
// File: bitbyte-swap/libs/TransferHelper.sol
...
// File: bitbyte-swap/libs/SafeMath.sol
...
// File: bitbyte-swap/interfaces/IWETH.sol
...
// File: bitbyte-swap/interfaces/IERC20.sol
...
// File: bitbyte-swap/interfaces/ISwapMining.sol
...
// File: bitbyte-swap/interfaces/IZooswapRouter.sol
...
// File: bitbyte-swap/interfaces/IZooswapPair.sol
...
// File: bitbyte-swap/interfaces/IZooswapFactory.sol
...
// File: bitbyte-swap/ZooswapRouter.sol
...
contract ZooswapRouter is IZooswapRouter, Ownable {
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            IZooswapPair(pairFor(input, output)).swap(
    ...
    returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'ZooswapRouter: INVALID_PATH');
        amounts = IZooswapFactory(factory).getAmountsIn(amountOut, path);
        require(amounts[0] <= msg.value, 'ZooswapRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value : amounts[0]}();
        assert(IWETH(WETH).transfer(pairFor(path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = IZooswapFactory(factory).sortTokens(input, output);
            IZooswapPair pair = IZooswapPair(pairFor(input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'ZooswapRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
