...
contract JedNFTs is IERC721, ERC721URIStorage, Ownable, VRFConsumerBaseV2 {
  ...
  CharProperties[] private charProps;
  ...
  constructor(string memory _name, string memory _ticker, address _vrfCoordinator, uint64 _vrfSubscriptionId) ERC721(_name, _ticker) VRFConsumerBaseV2(_vrfCoordinator) {
    ...
    _counter = 1;
    //The first base set of characteristics for each character
    CharProperties memory initProps;
    initProps.propertyCount = 5;
    initProps.tierFreeAssigns = [uint16(5),4,3,2,1];
    charProps.push(initProps);
  }
  ...
  function mint(uint8 _characterId, uint8 _tierId, address _coin, address _to) public returns (uint256) {
    ...
    require(coinCostPerTier[_coin][_tierId] > 0, "Coin is not supported for this tier");
    ...
    myCharacter.propertyInt =uint256(0);
   ...
   function fulfillRandomWords(
     ...
     uint256[] memory randomWords
      ...
      tokenIdToCharacter[vrfIdToTokenId[_requestId]].propertyInt = randomWords[0];
  ...
  function getCharacterProperties(uint256 _tokenID) public view returns (uint256[] memory) {
    require(_tokenID < _counter, "Token ID not recognised");
    uint16 totalProps = 0;
    uint16 propsCompleted = 0;
    for (uint16 k = 0; k < charProps.length; k++){
      totalProps += charProps[k].propertyCount;
    }
    bool[] memory assigned = new bool[](totalProps);
    uint256[] memory _fullCharacterProperties = new uint256[](totalProps);

    for (uint16 j = 0; j < charProps.length; j++){

      uint16 props = charProps[j].tierFreeAssigns[tokenIdToCharacter[_tokenID].characterTier];
      uint16 i = 0;

      while (props > 0 && i < propertyIterationMax){
          uint256 arrPos = uint256(propsCompleted) + uint256(tokenIdToCharacter[_tokenID].propertyInt/(10 ** i)) % charProps[j].propertyCount;
          uint256 digit_a = uint256(tokenIdToCharacter[_tokenID].propertyInt/(10 ** (i+1))) % 10;
          uint256 digit_b = uint256(tokenIdToCharacter[_tokenID].propertyInt/(10 ** (i+2))) % 10;
          if (!assigned[arrPos]){
              assigned[arrPos] = true;
              _fullCharacterProperties[arrPos] = uint256(tokenIdToCharacter[_tokenID].propertyInt/(10 ** ((i + propsCompleted + digit_a + digit_b) % 70))) % 100;
              props--;
          }
          i++;
      }
      propsCompleted += charProps[j].propertyCount;
    }
    return _fullCharacterProperties;
  }
