...
contract YiBoxBase is Governance,YiBoxStandard {
    ...
    function AddHeroTypeCount(address id, uint256[] memory _tys, uint256[] memory _qus) public returns (uint256 suit, uint256 suits, uint256 l4, uint256 l5) {
    ...
    function SubHeroTypeCount(address id, uint256[] memory _tys, uint256[] memory _qus) public returns (uint256 suit, uint256 suits, uint256 l4, uint256 l5) {
        require(_tys.length == _qus.length, "length error");
        // uint256 _ho = heroOwners[id];
    ...
    function GetHeroGroups(uint256 _gid) public view returns (uint256[] memory _ids, uint256[] memory _types, uint256[] memory _nums, uint256 _len) {
        ...
        if (_len == 0) {
            ...
            for (uint i = 0; i < _len; i++) {
                ...
                _ids[i] = HeroGroups[_gid][i].Id;
                _types[i] = HeroGroups[_gid][i].Type;
                _nums[i] = HeroGroups[_gid][i].Num;
            }
        }
    }
