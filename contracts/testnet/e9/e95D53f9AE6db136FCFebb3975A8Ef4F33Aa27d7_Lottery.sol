...
contract Lottery is ReentrancyGuard, ILottery, Ownable {
    ...
    uint256 public maxPriceTicket = 5 * (10 ** 8);
    uint256 public minPriceTicket = 0.005 * (10 ** 8);
    ...
    function getRewardInfo(uint256 _lotteryId) external view returns (uint256[6] memory rewardsBreakdown, uint256[6] memory tokenPerBracket, uint256[6] memory countWinnersPerBracket) {
        LotteryItem memory lottery = _lotteries[_lotteryId];
        return (
            lottery.rewardsBreakdown,
            lottery.tokenPerBracket,
