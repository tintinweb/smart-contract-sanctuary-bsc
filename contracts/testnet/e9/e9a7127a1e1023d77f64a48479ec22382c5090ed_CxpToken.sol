...
contract Faucet {
    ...
    uint256 public vndtAmount = 2288100000000000000000000;
...
contract CxpToken is ERC20, ERC20Snapshot, AccessControl, Pausable, ERC20Permit {
    ...
    function setFaucetAddress(address payable _faucet)
    public
    onlyRole(ADMIN_ROLE)
    {
    ...
    function mint(uint256 amount) public onlyRole(MINTER_ROLE) {
        _mint(address(faucet), amount);
    }

    function getMintData(uint256 amount) public pure returns (bytes memory) {
        return abi.encodeWithSignature("mint(uint256)", amount);
    }

    /**
     * @dev Destroys `amount` tokens from the faucet.
     *
     * See {BEP20-_burn}.
     */
    function burn(uint256 amount) public onlyRole(MINTER_ROLE) {
        _burn(address(faucet), amount);
    }

    function getBurnData(uint256 amount) public pure returns (bytes memory) {
        return abi.encodeWithSignature("burn(uint256)", amount);
    }
