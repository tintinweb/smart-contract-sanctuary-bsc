...
// File: contracts\libs\MerchantSharedProperty.sol
...
abstract contract MerchantSharedProperty is Ownable {
    ...
    IBEP20 private web3Token;

    IUniswapV2Router02 private swapRouter; // Pancake V2 router
    ...
    function viewWeb3Token() public view virtual returns (IBEP20) {
        if (
            MERCHANT_FACTORY == address(0) ||
    ...
    function viewSwapRouter() public view virtual returns (IUniswapV2Router02) {
        if (
            MERCHANT_FACTORY == address(0) ||
            propSelfUpdates[SharedProperty.SWAP_ROUTER]
        ) {
            return swapRouter;
        }
        return MerchantSharedProperty(MERCHANT_FACTORY).viewSwapRouter();
    }
    ...
    function updateWeb3TokenAddress(address _tokenAddress) public onlyOwner {
        ...
        web3Token = IBEP20(_tokenAddress);
    ...
    {
        require(
            address(_affiliatePool) != address(0),
            "Invalid pool"
        );
    ...
    function updateSwapRouter(address _router) public onlyOwner {
        ...
        emit SwapRouterUpdated(address(swapRouter), _router);
        swapRouter = IUniswapV2Router02(_router);
        propSelfUpdates[SharedProperty.SWAP_ROUTER] = true;
    }
...
contract Merchant is Pausable, MerchantSharedProperty {
    using SafeMath for uint256;
    ...
    function initialize(
        ...
        IBEP20(_receiveToken).balanceOf(address(this)); // Function just confirming
    ...
    function handleFee(uint256 donationFeeAmount) internal {
        ...
        if (feeProcessingMethod == FeeMethod.SIMPLE) {
            ...
            IBEP20 web3Token = viewWeb3Token();
    ...
    {
        IUniswapV2Router02 swapRouter = viewSwapRouter();
        IBEP20 web3Token = viewWeb3Token();
    ...
    function predictAmount(
        ...
        if (_payingTokenAddress == receiveToken) {
            return knownAmount;
        }
        ...
        if (getInAmount) {
            uint256[] memory amounts = swapRouter.getAmountsIn(
                knownAmount,
                path
            );
            return amounts[0];
    ...
    function getAmountIn(
        ...
        return predictAmount(_payingTokenAddress, amountOut, path, true);
    }
    ...
    function getAmountOut(
        ...
        return predictAmount(_payingTokenAddress, amountIn, path, false);
    }
    ...
    function getDonationFeeAmount(uint256 _amountIn)
        private
        view
        returns (uint256)
    {
        return _amountIn.mul(viewDonationFee()).div(10000);
    }
    ...
    function getFeeAmount(
        ...
        uint256 donationFeeAmount = getDonationFeeAmount(_amountIn);
    ...
    function submitTransaction(
        ...
        // doMerchantDeposit(_payingTokenAddress, _amountIn, path);
        {
            ...
            payingToken.approve(address(swapRouter), _amountIn);

            if (_payingTokenAddress == receiveToken) {
                IBEP20(receiveToken).safeTransfer(merchantWallet, _amountIn);
            } else {
                // swap path should be valid one
                if (
                    !(path.length >= 2 &&
                    ...
                    if (_payingTokenAddress == swapRouter.WETH()) {
                        ...
                        path[0] = _payingTokenAddress;
                        path[1] = receiveToken;
                    } else if (_payingTokenAddress != swapRouter.WETH()) {
                        path = new address[](3);
                        path[0] = _payingTokenAddress;
                        path[1] = swapRouter.WETH();
                        path[2] = receiveToken;
