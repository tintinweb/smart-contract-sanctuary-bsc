...
contract OrbiterBridge is Ownable, ReentrancyGuard, Multicall {
    ...
    function transferERC20(address token, address to, uint256 value) private {
        bool success = IERC20(token).transferFrom(msg.sender, to, value);
        require(success, "TOKEN ERROR");
    }
    ...
    function swapFail(
        ...
        if (token == address(0)) {
            to.transfer(value);
            emit SwapFailEvent(tradeId, address(0), to, value);
        } else {
    ...
    function swapOK(
        ...
        if (token == address(0)) {
            to.transfer(value);
            emit SwapOKEvent(tradeId, address(0), to, value);
        } else {
            transferERC20(token, to, value);
