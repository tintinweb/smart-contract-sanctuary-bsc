...
contract COINGIOCO is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    COGTokenDividendTracker public dividendTracker;

    address public teamWallet;
    address public marketingWallet;
    address public gameprojectWallet;

    uint256 public swapTokensAtAmount = 100000000000 * (10**9);

    // buy fees
    uint256 public buyDividendRewardsFee = 3;
    uint256 public buyGameprojectFee = 0;
    ...
    uint256 public sellGameprojectFee = 0;
    ...
    constructor() ERC20("COINGIOCO", "COG") {
        dividendTracker = new COGTokenDividendTracker();

        teamWallet = 0x9A24337BbF6A553182692b5B9d20BC6818b1d853;
        gameprojectWallet = 0x9cEc888042De4BF5941608B30A3dA50A39FCF9a5;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "COG: The dividend tracker already has that address"
        );

        COGTokenDividendTracker newDividendTracker = COGTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "COG: The new dividend tracker must be owned by the COG token contract"
        );
    ...
    function updateSellFees(
        ...
        sellGameprojectFee = gameproject;
        selTeamFee = team;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "COG: The router already has that address"
        );
    ...
    function updateGameprojectAddress(address newAddress) public onlyOwner {
        excludeFromFees(gameprojectWallet, false);
        gameprojectWallet = newAddress;
        excludeFromFees(newAddress, true);
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "COG: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "COG: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "COG: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "COG: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "COG: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = buyGameprojectFee.add(buyTeamFee).add(
                    buyMarketingFee
                );
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(gameprojectWallet),
            address(this).balance.div(totalFee).mul(buyGameprojectFee)
        );
        transferToWallet(
...
contract COGTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("COG_Dividend_Tracker", "COG_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "COG_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "COG_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main COG contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "COG_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "COG_Dividend_Tracker: Cannot update claimWait to same value"
        );
