...
contract AttachBoss is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address => AttackInfo[]) attacks;
    ...
    function attackBoss(
        ...
        require(
            !isAttack(_msgSender(), typeBoss, bossId),
        ...
        attacks[_msgSender()].push(AttackInfo(typeBoss, attackDamage, bossId));
    ...
    function isAttack(
        ...
        AttackInfo[] memory attackInfos = attacks[userAddress];
        for (uint256 index = 0; index < attackInfos.length; index++) {
            if(attackInfos[index].typeBoss == typeBoss && attackInfos[index].bossId == bossId) {
                return true;
            }
        }
        return false;
