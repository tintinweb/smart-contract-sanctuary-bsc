...
contract MultiSignature is AccessControl {
    ...
    function applyToken(
        ...
        bytes32 hash = keccak256(
            abi.encode(_msgSender(), amount, block.timestamp, expireAt, salt)
        );
    ...
    function acceptApplyToken(
        ...
        bytes32 hash = keccak256(
            abi.encode(_requester, _amount, _requestAt, _expireAt, _salt)
        );
    ...
    function applySetReceiver(
        address _receiver,
        uint expireAt,
        bytes32 salt
    ) external {
        require(
            _receiver != address(0) && _receiver != receiver,
            "receiver is zero or the same"
        );
        bytes32 hash = keccak256(
            abi.encode(_msgSender(), _receiver, block.timestamp, expireAt, salt)
        );
        require(!transactions[hash], "apply already request");
        transactions[hash] = true;
        signatureNum[hash] = 0;
        emit ApplySetReceiver(
            msg.sender,
            _receiver,
            block.timestamp,
            expireAt,
    ...
    function acceptApplySetReceiver(
        address _requester,
        address _receiver,
        uint _requestAt,
        uint _expireAt,
        ...
        bytes32 hash = keccak256(
            abi.encode(_requester, _receiver, _requestAt, _expireAt, _salt)
        );
        require(transactions[hash], "apply not exists");
        require(
            !signatureRecord[hash][_msgSender()],
