...
contract Game3  is Ownable  {
    ...
    modifier ownerNft(uint256 _tokenId) {
        ...
        if(block.timestamp - timeOfToken[_tokenId] >= 5 minutes && turnOfToken[_tokenId] == 0){
    ...
    function getTurnOfToken(uint256 _tokenId) external view returns(uint256) {
        if(turnOfToken[_tokenId] == 0 && (timeOfToken[_tokenId] == 0 || block.timestamp - timeOfToken[_tokenId] >= 1 days)){
            return 6;
        }
        if(turnOfToken[_tokenId] != 0){
