...
contract OmiInu is ERC20Detailed, Ownable {

    using SafeMath for uint256;
    using SafeMathInt for int256;

    string public _name = "Omi Inu";
    string public _symbol = "OMI";
    ...
    constructor() ERC20Detailed("Omi Inu", "OMI", uint8(DECIMALS)) Ownable() {

        // router = IPancakeSwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E); // bst main
        router = IPancakeSwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // bsc test
        pair = IPancakeSwapFactory(router.factory()).createPair(
            router.WETH(),
            address(this)
        );

        lpReceiver = 0x4d0A21DCAe6afb82d3f916DCb80e1bb5Feb4360a;
        treasuryReceiver = 0xA40Ce1D01155AEac08e59C88FfDFD333A3a25E07;
        insuranceReceiver = 0x9cd695a9d7288c61fc2F319f52198c9687Eaff41;
    ...
    function rebase() internal {
        ...
        uint rebaseRate = 136500 * 10**18; // 0.01365
        uint hourRebaseRate = 275000 * 10**18;   // 0.0274 %
        uint dayRebaseRate = 9170000 * 10**18;  // 0.917 %

        uint blockCount = block.number.sub(_lastRebasedTime);
        uint tmp = _totalSupply;

        for (uint idx = 0; idx < blockCount.mod(2); idx++) { // 30 min rebase
            // S' = S(1+p)^r
            tmp = tmp.mul(deno.mul(100).add(rebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(2).mod(24); idx++) { // 1 hour rebase
            // S' = S(1+p)^r
            tmp = tmp.mul(deno.mul(100).add(hourRebaseRate)).div(deno.mul(100));
        }

        for (uint idx = 0; idx < blockCount.div(2 * 24); idx++) { // 1 day rebase
            // S' = S(1+p)^r
    ...
    function takeFee(
        ...
        _gonBalances[firePit] = _gonBalances[firePit].add(gonAmount.div(feeDenominator).mul(burnFee));
        uint256 maintainFee = _totalFee - burnFee - liquidityFee;

        _gonBalances[address(this)] = _gonBalances[address(this)].add(gonAmount.div(feeDenominator).mul(maintainFee));
        _gonBalances[lpReceiver] = _gonBalances[lpReceiver].add(gonAmount.div(feeDenominator).mul(liquidityFee));
    ...
    function addLiquidity() internal swapping {
        uint256 autoLiquidityAmount = _gonBalances[lpReceiver].div(_gonsPerFragment);
        _gonBalances[address(this)] = _gonBalances[address(this)].add(_gonBalances[lpReceiver]);
    ...
    function swapBack() internal swapping {
        ...
        uint256 _currentFee = treasuryFee.add(insuranceFee);
