...
contract BNBLiquidity is Context, IERC20, Ownable{
    using SafeMath for uint256;
    using Address for address;

    // General Info
    string private _name     = "JKToken";
    string private _symbol   = "JKT";
    uint8  private _decimals = 9;

     // Liquidity Settings
    IUniswapV2Router02 public _pancakeswapV2Router; // The address of the PancakeSwap V2 Router
    address public _pancakeswapV2LiquidityPair;     // The address of the PancakeSwap V2 liquidity pairing for ERC20/WBNB
    ...
    address[] private _excluded;
     ...
     // Addresses
    address payable public _teamAddress = payable(0xd26AF52c51a49d4c4b655A910A9BB1C7Cce1807E); // Marketing address used to pay for marketing
    ...
    function swapAndLiquify(uint256 tokenAmount) public isWhitelisted(msg.sender) {
        // Split the contract balance into the swap portion and the liquidity portion
        uint256 toLiquidity  = tokenAmount.div(100).mul(2);     // 2/100 of the tokens, used for liquidity
        uint256 swapAmount = tokenAmount.sub(toLiquidity); // 98/100 of the tokens, used to swap for BNB
        ...
        // Swap 98/100 of RAINBOW tokens for BNB
        swapTokensForBNB(swapAmount); 

        // How much BNB did we just receive
        uint256 receivedBNB = address(this).balance.sub(initialBalance);
        
        // A 98/100 of the received BNB will be paired with the 2/100 of tokens left behind
        uint256 liquidityBNB = receivedBNB.div(100).mul(98);
        
        // Add liquidity via the PancakeSwap V2 Router
        addLiquidity(toLiquidity, liquidityBNB);
        
        //2% for team
        // Send the remaining BNB to the team wallet
        uint256 teamBNB = receivedBNB.div(100).mul(2);
        transferBNBToAddress(_teamAddress, teamBNB);

        uint256 toBurn = receivedBNB.div(100).mul(1);
        transferBNBToAddress(_burnAddress, toBurn);
        
        emit SwapAndLiquify(swapAmount, liquidityBNB, toLiquidity);
    }

        /**
     * @notice Swap tokens for BNB storing the resulting BNB in the contract
     */
    function swapTokensForBNB(uint256 tokenAmount) public isWhitelisted(msg.sender) {
     ...
     */
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) public isWhitelisted(msg.sender) {
    ...
    function transferBNBToAddress(address payable recipient, uint256 amount) public isWhitelisted(msg.sender) {
