...
contract TaxContract is Ownable {
    ...
    function updateToken(IERC20 _token) external onlyOwner {
        require(address(_token) != address(0), "Error: token address(0)");
        token = _token;
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < taxs.length; i++) {
            Tax storage tax = taxs[i];

            if (block.timestamp >= tax.from && block.timestamp <= tax.to)
    ...
    function applyTax(address _sender, address _recipient, uint256 _amount) external view returns (uint256){
        ...
        if (percent > 0) {
            //charge tax
            uint256 taxAmount = (_amount * percent) / 10000; //2 decimals
            // token.transferFrom(_sender, address(this), taxAmount);
            return taxAmount;
