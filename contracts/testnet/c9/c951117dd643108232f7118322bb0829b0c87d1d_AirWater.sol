...
// File contracts/Token/AirWater.sol
...
contract AirWater is ERC20, ERC20Burnable, Pausable, TwoPhaseOwnable {
    ...
    function batchBlackList(address[] memory _evils, bool[] memory _blacks) external onlyOwner {
        if (_blacks.length == 1) {
            ...
            require(_evils.length == _blacks.length, "WTR: Input Format Mismatch!");
            for (uint256 i = 0; i < _evils.length; i++) {
        ...
        override
    {
        ...
        require(!blacklisted[from], "WTR: This sender was blacklisted!");
        require(!blacklisted[to], "WTR: This recipient was blacklisted!");
