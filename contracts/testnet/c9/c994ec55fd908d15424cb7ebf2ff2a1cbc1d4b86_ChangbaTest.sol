...
contract ChangbaTest is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;

    address private constant PANCAKE_ROUTER_ADDRESS = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    address private constant WBNB_ADDRESS = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address private constant USDT_ADDRESS = 0x337610d27c682E347C9cD60BD4b3b107C9d34dDd;
    address private constant ALICE_ADDRESS = 0x84102Df4B6Bcb72114532241894B2077428a7f86;
    ...
    uint256 public timeDuration = 600;
    uint256 public rewards_rate = 300;

    mapping(address => uint256) public usdtBalances;


    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("Changba Test", "CT") {
    ...
    IERC20(WBNB_ADDRESS).approve(PANCAKE_ROUTER_ADDRESS, type(uint).max);
    ...
    function mint(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            uint256 swapBNB = totalCost * rewards_rate / 1000;
            require(msg.value >= totalCost, "Not Enough BNB Sent");  

            swapETHForWUSDT(swapBNB);
    ...
    function swapETHForWUSDT(uint256 bnb_amount_in) public payable {
        address[] memory path = new address[](2);
        path[0] = WBNB_ADDRESS;
        path[1] = USDT_ADDRESS;

        pancakeRouter.swapExactETHForTokens{value: bnb_amount_in}(
            0,
            path,
            address(this),
        ...
        uint256 wusdt_balance = IERC20(USDT_ADDRESS).balanceOf(address(this));

        for (uint i = 0; i < totalSupply(); i++) {
        address holder = ownerOf(i);
        usdtBalances[holder] += wusdt_balance / totalSupply();
    }
    ...
    function claimUSDT() public {
        ...
        require(usdtBalances[msg.sender] > 0, "No USDT balance to claim");
        uint256 amount = usdtBalances[msg.sender];
    ...
    function setRewards_rate(uint256 _rewards_rate) external onlyOwner {
        rewards_rate = _rewards_rate;
    }

    function setTimeDuration(uint256 _timeDuration) external onlyOwner {
