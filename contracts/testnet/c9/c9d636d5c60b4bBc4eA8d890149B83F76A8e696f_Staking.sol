...
address public MBcontract = 0xD9fae0dC81A58bfA43EA8b35904eCDDcCf1c9B7B;
address public MCcontract = 0xa90E4cDC17a29C6A3f035a36bB32924E9fec5CBa;
...
function storeMBTokens(uint _number ) internal {
      ...
      MBstakeOwners[ msg.sender].rewardtokens = (MBstakeOwners[ msg.sender].tokens  * multiplier) / 10;
 ...
 delete tokenID;
   ...
   function MBtopup(uint tokens) external {
    ...
    require( !periodOver , "Staking Period is still over");
...
function checkStakedtokenIDs(address _address) external view returns(uint [] memory){
  return stakeOwners[_address].tokens;
   ...
   function getMBStakingEndTime(address _address) public view returns(uint64){
         ...
         return MBstakeOwners[_address].stakingstartingtime + 180  ;
 ...
 function calculateMBReward(address _address) public view returns (uint) {
    ...
    return ((MBstakeOwners[_address].rewardtokens /86400 ) * checkMBTime(_address));
...
function checkIfMBPeriodOver(address _address) public view returns (bool)
 {
     ...
     bool period = false;
          ...
          uint64 time =MBstakeOwners[_address].stakingstartingtime + 180;
          if( endtime >=  time )
          {
              period = true;
          }
