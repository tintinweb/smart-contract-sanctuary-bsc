...
contract Lazy1155 is Initializable,BEP1155Upgradeable, OwnableUpgradeable, BEP1155BurnableUpgradeable {
   event Approve(
    ...
    function minting(string memory ipfsmetadata, uint256 royal, address from, address to, uint256 nooftoken) internal returns(uint256){
         _tid = _tid.add(1);
        uint256 id_ = _tid.add(block.timestamp);
        _mint(to, id_, nooftoken, "");
        _setTokenURI(id_, ipfsmetadata);
        _creator[id_]= from;
        _royal[id_]=royal.mul(1e18);
         return id_;
    }
    function lazyMint(uint256 royal, uint256 nooftoken, string memory ipfsmetadata, address payable from,uint256 amount, string memory bidtoken, string memory status) public payable{
        uint256 id_ = 0;
        if(keccak256(abi.encodePacked((status))) == keccak256(abi.encodePacked(("Buy")))){
            id_ = minting(ipfsmetadata, royal, from, msg.sender, nooftoken);
            _saleToken(from, id_, amount, bidtoken);
        }   
        else{
            id_ = minting(ipfsmetadata, royal, msg.sender, from, nooftoken);
