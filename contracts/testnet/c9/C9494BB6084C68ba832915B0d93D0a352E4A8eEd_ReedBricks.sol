...
contract ReedBricks is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _isFozen;
    ...
    function freezMe() external {
        require(!_isFozen[_msgSender()], "Account is already frozen");
        _isFozen[_msgSender()] = true;
        emit Freez(_msgSender());
    }

    function unFreezMe(address account) external onlyOwner {
        require(!_isFozen[account], "Account is already excluded");
        _isFozen[account] = false;
