    ...
    AccessControl
{
    ...
    constructor() ERC721("Paragon", "PRG") {
    ...
    {
        ParagonStruct memory pg = paragonDNA[tokenId];
        return (
            ownerOf(tokenId),
            pg.originalArt,
            pg.rawHashed,
        ...
        onlyRole(HYDRA_ROLE)
    {
        for (uint256 i = 0; i < tokenId.length; i++) {
            ...
            paragonDNA[tokenId[i]].status = status[i];
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(paragonDNA[tokenId].status == 1, "Paragon must be active");
