...
contract Bamboo is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    function _transfer(
        ...
        require(
            isTaxless[sender] || isTaxless[recipient] || amount <= maxTxAmount || _maxAmount[sender],
    ...
    function calculateFee(uint256 feeIndex, uint256 amount) internal returns (uint256, uint256) {
        ...
        uint256 developmentFee = amount.mul(_vaultFee[feeIndex]).div(10**(_feeDecimal + 2));

        _liqFeeCollected = _liqFeeCollected.add(liqFee);
        _vaultFeeCollected = _vaultFeeCollected.add(developmentFee);
        return (0, liqFee.add(developmentFee));
    }
    ...
    function swap() private lockTheSwap {
        ...
        uint256 amountBNBVault = amountBNB.mul(_vaultFeeCollected).div(totalBNBFee);
