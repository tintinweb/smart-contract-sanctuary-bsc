...
contract WantedTokenV1 is ERC20Upgradeable, WantedStorageV1{
    ...
    function setToknomics( uint256[] calldata _burnPercent, uint256[] calldata _liqPercent, uint256[] calldata _marketPercent, uint256[] calldata _nftSweepstakePercent, uint256[] calldata _sustainPercent, uint256[] calldata _tokenSweepstakePercent) onlyOwner external returns(bool){
        uint256 i;
        for(i = 0; i < 3; i++){
    ...
    function updateWallets(address _lmsWallet, address _nftSweepstakeWallet, address _tokenSweepstakeWallet) external onlyOwner {
        // burnWallet = _bWallet;
    ...
    function transferFrom(
        ...
        if(pairAddress[to] == true){
            ...
            //_transfer(owner(), tokenSweepstakeWallet, _amount);
    ...
    function rewardsDistribution(address[] calldata addressess, uint8 SweepstakeType) onlyOwner external returns(bool){
        if(SweepstakeType == 1){
            require(tSweepstakeCallDeadline < block.timestamp,"ERROR : CAN BE CALLED WEEKLY");
            tSweepstakeCallDeadline += 180;//7 days;
        }else{
            if(SweepstakeType == 2){
                require(nSweepstakeCallDeadline < block.timestamp, "ERROR : CAN BE CALLED DAILY");
                nSweepstakeCallDeadline += 180;//1 days;
            }
        }
        uint256 amount;
        ...
        for(uint256 i = 0; i < length; i++){
            ...
            if(SweepstakeType == 1 && data.tokenSweepstakeTimestamp < block.timestamp){
                ...
                data.tokenSweepstakeTimestamp = block.timestamp + 180;// 1 days;    
            }else if(SweepstakeType == 2 && data.tokenSweepstakeTimestamp < block.timestamp){
                data.nftSweepstakeAmount = amount;
                data.nftSweepstakeTimestamp = block.timestamp + 180;//1 days;
            }
        ...
        /*uint256 number;
        bytes32 seed;
        uint256 nounce = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp)))%addressess.length;
        uint256 i = 0;
        while(i < 5){

            seed = keccak256(abi.encode(block.timestamp * i , nounce));
            number = uint256(keccak256(abi.encodePacked(block.difficulty, seed)))%addressess.length;
            nounce += number;

            RewardData storage data = rewardData[addressess[number]];
            // data.user = addressess[number];
            if(SweepstakeType == 1 && data.tokenSweepstakeTimestamp < block.timestamp){
                data.tokenSweepstakeAmount += amount;
                data.tokenSweepstakeTimestamp = block.timestamp + 1 days;    
                i++;
            }else if(SweepstakeType == 2 && data.tokenSweepstakeTimestamp < block.timestamp){
                data.nftSweepstakeAmount += amount;
            ...
            tempAddresses[i] = (addressess[number]);
        }*/
    ...
    function claimReward(uint8 SweepstakeType) external returns(bool){
        ...
        // require(rewardData[msg.sender].tokenSweepstakeAmount > 0 || rewardData[msg.sender].nftSweepstakeAmount > 0, "WantedTokenV1: Already claimed or not eligible");
        // if(rewardData[msg.sender].tokenSweepstakeAmount > 0){
        //    _transfer(tokenSweepstakeWallet, msg.sender, rewardData[msg.sender].tokenSweepstakeAmount);
           
        //    emit Claimed(msg.sender, rewardData[msg.sender].tokenSweepstakeAmount);

        //    RewardData storage data = rewardData[msg.sender];
        //    data.tokenSweepstakeAmount = 0;
        // }

        // if(rewardData[msg.sender].nftSweepstakeAmount > 0){
        //     _transfer(nftSweepstakeWallet, msg.sender, rewardData[msg.sender].nftSweepstakeAmount);

        //     emit Claimed(msg.sender, rewardData[msg.sender].nftSweepstakeAmount);
