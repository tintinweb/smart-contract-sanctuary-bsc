...
contract SuperCat is Context, Ownable{
    ...
    function receiveProfit() public {
        ...
        uint256 _amount = (lp_release /86400) * (lp_pool[msg.sender].usdt * _time_interval) /lp_weight_total;
        if(_amount>0){
            IERC20(catToken).transfer(msg.sender, _amount);
            lp_pool[msg.sender].receive_time = block.timestamp;
    ...
    function receiveTeamProfit() public{
        ...
        uint256 _amount = (re_release /86400) * (lp_pool[msg.sender].team_usdt *_time_interval)/lp_weight_total ;
        if(_amount>0){
            IERC20(catToken).transfer(msg.sender, _amount);
    ...
    function team_usdt(uint256 _amount) private{
        if(_amount>0){
            ...
            for (uint i = 1; i <= 15; i++) {
                ...
                if(lp_pool[pre].team_usdt==team_amount){
