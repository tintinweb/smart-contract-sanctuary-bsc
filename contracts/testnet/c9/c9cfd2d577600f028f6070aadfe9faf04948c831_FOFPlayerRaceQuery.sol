...
contract FOFPlayerRaceQuery is Ownable {
    ...
    function getClubMemberLists(address user) public view returns(string[] memory nicknames, address[] memory members, uint256[] memory vehicleNums) {

        uint256[] memory clubIds = FPC2.getUserNFTIDsByType(user, 4);

        // return getClubMemberLists(clubIds[0]);

        (nicknames,members,vehicleNums) = getClubMemberLists(clubIds[0]);
    ...
    function getNFTByUserAndType(address user, uint256 types) public view returns(uint256[] memory ids, string[] memory cardNicknames, uint256 index) {
        ...
        uint256[] memory clubIds = FPC2.getUserNFTIDsByType(user, types);
