...
contract BitdriveLaunchpad is Owned {
    ...
    function claimToken() public {
        ...
        if (!isVest) {
            ...
            // );
            require(
                block.timestamp > (lastClaimed + 3 minutes),
                "Vesting period error"
            );
            ...
            userInfo[msg.sender].claimableTokens = userInfo[msg.sender]
                .claimableTokens
                .sub(available);
            userInfo[msg.sender].claimedTokens = userInfo[msg.sender]
                .claimedTokens
                .add(available);
        ...
        )
    {
        ...
        uint256 vestingPeriod = 3 minutes;
        diff = (block.timestamp > end) ? block.timestamp - end : 0;
        ...
        if (userInfo[_user].claimableTokens > 0 && diff > 0) {
            ...
            if (totalVesting > 10000) {
                ...
                uint256 claimedAmt = userInfo[_user].claimedTokens;
                available = available - claimedAmt;
                uint256 totalAmt = available + claimedAmt;
                if (totalAmt > invest) {
                    available = invest - claimedAmt;
                    Percentage = 10000 - userInfo[msg.sender].totalVesting;
                }
            }
        }
