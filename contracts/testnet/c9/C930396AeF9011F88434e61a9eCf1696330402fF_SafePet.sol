...
contract SafePet is AccountChangable, Utils {
    ...
    function upgrade(uint256 level, uint256 numberOfAnims) public mustBeActiveInvestor {
        ...
        Transaction memory transaction = Transaction(investorAddress, level, decreaseCount, ANIM_CHANGE_REASON_UPGRADE_DECREASE, block.number);
        ...
        if (successCount > 0) {
            ...
            transaction = Transaction( investorAddress, level.add(1), successCount, ANIM_CHANGE_REASON_UPGRADE_INCREASE, block.number);
            transactions.push(transaction);
        }
    ...
    function transferAnim(
        ...
        Transaction memory transaction = Transaction(from, level, numberOfAnims, ANIM_CHANGE_REASON_SEND_TRANSFER, block.number);
        transactions.push(transaction);
        emit AnimChange(from, level, numberOfAnims, ANIM_CHANGE_REASON_SEND_TRANSFER);
        increaseAnim(to, level, numberOfAnims);
        transaction = Transaction(to, level, numberOfAnims, ANIM_CHANGE_REASON_RECEIVE_TRANSFER, block.number);
        transactions.push(transaction);
    ...
    function sellAnim(uint256 level, uint256 numberOfAnims) public mustBeActiveInvestor {
        ...
        Transaction memory transaction = Transaction(investorAddress, level, numberOfAnims, ANIM_CHANGE_REASON_SELL, block.number);
        transactions.push(transaction);
        pay(fundAdmin, fee);
