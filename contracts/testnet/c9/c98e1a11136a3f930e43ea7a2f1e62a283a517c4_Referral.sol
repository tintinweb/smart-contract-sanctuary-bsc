...
// File @openzeppelin/contracts/access/IAccessControl.so[email protected]
...
interface IReferral  {
  ...
  event CodeCreated(string indexed code, address indexed referrer, uint256 rate);
...
contract Referral is IReferral, ReentrancyGuard, AccessControl  {
  ...
  function RefPoint(address sender, uint256 point) external override view returns(uint256, uint256, address) {
  ...
  function createCode(string calldata code, uint256 rate) external {
    ...
    refOwner[code].ref = msg.sender;
    refOwner[code].rate = rate;
    userOfCodes[msg.sender].push(code);
    emit CodeCreated(code, msg.sender, rate);
  }

  function setExtra(uint256 extral) external onlyRole(DEFAULT_ADMIN_ROLE) {
