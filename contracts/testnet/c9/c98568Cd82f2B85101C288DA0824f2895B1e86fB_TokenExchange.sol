...
interface ITokenExchange {
    ...
    function takeFixPrice(uint256 orderId_) external payable;

    function takeAuction(uint256 orderId_, uint256 amount) external payable;

    function cancelUser(uint256 orderId_) external;

    function cancelPlatform(uint256 orderId_) external;

    function cancelAll(uint256 count) external;

    function delivery(uint256[] memory orderId_) external;

    function acceptOffer(uint256 orderId_) external;
}
