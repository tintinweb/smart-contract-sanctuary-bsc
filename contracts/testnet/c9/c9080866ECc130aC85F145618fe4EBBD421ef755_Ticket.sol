...
contract Ticket is ITicket, ERC721, ERC721Enumerable, SafeTransfer, Ownable, Pausable {
	...
	event SetFloorPrice(uint256 newFloorPrice);

	modifier onlyOperator() {
		require(metaspacecyAccessControls.hasOperatorRole(_msgSender()), "Ticket: only operator");
		_;
	}
	...
	function setFloorPrice(uint256 _floorPrice) public onlyOwner {
		floorPrice = _floorPrice;
		emit SetFloorPrice(_floorPrice);
	}
	...
	function claimCard() public payable whenNotPaused {
		...
		if (senderInfo.isMember && !senderInfo.claimed) {
			...
			_tokenIdAirdrop++;
			emit ClaimCard(_msgSender(), currentIdAirdrop);
		} else {
			require(currentIdSell <= (MAX_SUPPLY - SUPPLY_RESERVE), "Ticket: sell ended");
			require(msg.value >= cardPrice, "Ticket: price invalid");

			_safeMint(_msgSender(), currentIdSell);
			if (!_members.contains(_msgSender())) {
				_members.add(_msgSender());
				senderInfo.isMember = true;
				senderInfo.claimed = true;
			}
			_tokenIdSell++;
			emit ClaimCard(_msgSender(), currentIdSell);
		}
	}
