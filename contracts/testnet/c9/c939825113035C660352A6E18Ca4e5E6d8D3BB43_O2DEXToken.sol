...
contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0), "newOwner must not be address-zero");
        _owner = newOwner;
    }
}
...
contract O2DEXToken is Ownable, ERC20Burnable {
    ...
    uint256 internal _offTimestamp;
    uint256 internal _transMaxVol;
    ...
    function offTimestamp() public view returns (uint256) {
        return _offTimestamp;
    }

    function transMaxVol() public view returns (uint256) {
        return _transMaxVol;
    }
    ...
    function freeAirdrop(address account) public view returns (bool) {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        require(!_blackList[_msgSender()], "O2DEXToken: sender address is in blacklist");
		require(!_blackList[recipient], "O2DEXToken: recipient address is in blacklist");
		
        bool transable = block.timestamp < _offTimestamp && _offTimestamp > 0 && _transMaxVol > 0  && !_whiteList[_msgSender()] && !_whiteList[recipient];
		if ( transable ) {
			 require(amount <= _transMaxVol, "O2DEXToken: amount is overflow");
		}
		 
		bool isInviter = _inviter[recipient] == address(0) && !_freeAirdrop[_msgSender()] && amount>=_inviteMinVol;
        if( isInviter ) {
    ...
    function setOffTimestamp(uint256 timestamp) public onlyOwner {
		_offTimestamp = timestamp;
    }

    function setTransMaxVol(uint256 amount) public onlyOwner {
		_transMaxVol = amount;
    }

    function setInviteMinVol(uint256 amount) public onlyOwner {
    ...
    function setFreeAirdrop(address account,bool value) public onlyOwner {
