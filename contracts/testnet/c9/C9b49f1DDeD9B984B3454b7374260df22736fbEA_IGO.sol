...
contract IGO is Auth, PausableUpgradeable {
  ...
  event PricesUpdated(uint id, uint price);
  ...
  function addOrganization(
    ...
    organizations[totalOrganizations].price = _price;
    emit OrganizationAdded(totalOrganizations, _name, _nftClassIds, _price, _quantityPerNFT, _maxBuyPerUserPerClassId);
  }

  function updatePrice(uint _id, uint _price) external onlyMainAdmin {
    require(_id <= totalOrganizations, "IGO: id invalid");
    organizations[_id].price = _price;
    emit PricesUpdated(_id, _price);
  }
  ...
  function claim(uint _id, uint _index, bytes32[] calldata _path, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    uint amount = _takeFund(_id, _paymentCurrency);
  ...
  function getClassIds(uint _id) external view returns (uint16[] memory) {
    require(_id <= totalOrganizations, "IGO: id invalid");
    return organizations[_id].nftClassIds;
  }
  ...
  function _takeFund(uint _id, IMarketPlace.PaymentCurrency _paymentCurrency) private returns (uint) {
    uint amount = organizations[_id].price;
  ...
  function _mintNFT(uint _id, uint16[] memory _ids) private returns (uint16[] memory, uint[] memory){
    ...
    uint price = organizations[_id].price / _ids.length;
