...
// main contract
contract test is IERC20Extended, Auth {
    using SafeMath for uint256;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToSwap = swapThreshold.sub(
            amountToLiquify.add(_reflectionFeeCount).add(_burnFeeCount)
        );
        ...
        try distributor.deposit(_reflectionFeeCount) {
            _basicTransfer(
                address(this),
                address(distributor),
                _reflectionFeeCount
        ...
        _basicTransfer(address(this), DEAD, _burnFeeCount);
