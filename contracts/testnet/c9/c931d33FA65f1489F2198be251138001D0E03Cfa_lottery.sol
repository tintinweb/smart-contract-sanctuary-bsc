        ...
        for(uint i=0;i<lotteryIds.length;i++){            
            ...
            if(_gameGameReult == 4){
                ...
                if(_gameResult == _gameGameReult){//买对了,并且未领取，既有收益
                    uint256 lAmount = 0;
                    if(_gameResult == 0){
                          lAmount = _lotteryAmount.mul(game.threeOdds).add(_lotteryAmount).div(10000); 
                    }else if(_gameResult == 1){
                        lAmount = _lotteryAmount.mul(game.oneOdds).add(_lotteryAmount).div(10000); 
                    }else if(_gameResult == 2){
                        lAmount = _lotteryAmount.mul(game.twoOdds).add(_lotteryAmount).div(10000); 
                    }
                    if(!_drawBool){
        ...
        if(lotteryIds.length == 0){            
            ...
            for(uint i=0;i<lotteryIds.length;i++){
                ...
                if(_gameResult != _gameGameReult){
                    ...
                    if(_gameResult == 0){
                          totalAmount = totalAmount.add(_lotteryAmount.mul(game.threeOdds).add(_lotteryAmount).div(10000)); 
                    }else if(_gameResult == 1){
                        totalAmount = totalAmount.add(_lotteryAmount.mul(game.oneOdds).add(_lotteryAmount).div(10000)); 
                    }else if(_gameResult == 2){
                        totalAmount = totalAmount.add(_lotteryAmount.mul(game.twoOdds).add(_lotteryAmount).div(10000)); 
                    }                    
                }
