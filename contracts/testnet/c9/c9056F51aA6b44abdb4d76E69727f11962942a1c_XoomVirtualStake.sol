...
contract XoomVirtualStake {
    ...
    function registerStake(uint256 apr, address token, address rewardToken, uint256 minRequiredBalanceToKeep, uint checkpointFrequency, uint checkpointWaitTime, uint lockTime) external onlyOwner noReentrancyForOwner validateAllowed validAddress(token) validAddress(rewardToken) validWallet
    {
        ...
        setStake(stakeID, apr, token, rewardToken, minRequiredBalanceToKeep, checkpointFrequency, checkpointWaitTime, lockTime);
    ...
    function setStake(uint stakeID, uint256 apr, address token, address rewardToken, uint256 minRequiredBalanceToKeep, uint checkpointFrequency, uint checkpointWaitTime, uint lockTime) public onlyOwner noReentrancyForOwner validateAllowed validAddress(token) validAddress(rewardToken) validWallet
    {
