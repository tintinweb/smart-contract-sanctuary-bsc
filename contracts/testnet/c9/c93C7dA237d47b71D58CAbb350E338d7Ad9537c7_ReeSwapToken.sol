...
contract ReeSwapToken is BEP20 {
    ...
    // Min amount to liquify. (default 500 RETs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to RET's router when our own AMM release
    IUniswapV2Router02 public RETRouter;
    // The trading pair
    address public RETPair;
    // In swap and liquify
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "RET::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
     ...
     */
    constructor() public BEP20("ReeSwap Tokens", "RETs") {
        ...
        uint256 TOTAL_SUPPLY = 10 * 10**9 * 10**18; // 10,000,000,000 RET
        _mint(address(this), TOTAL_SUPPLY);
        _moveDelegates(address(0), _delegates[address(this)], TOTAL_SUPPLY);
    }

    /// @dev overrides transfer function to meet tokenomics of RET
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(
                ...
                "RET::transfer: Burn value invalid"
            );
            ...
            require(
                ...
                "RET::transfer: Tax value invalid"
            );
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the RET pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = RETRouter.WETH();

        _approve(address(this), address(RETRouter), tokenAmount);

        // make the swap
        RETRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        RETRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from RETRouter when swapping
        ...
        onlyOperator
    {
        require(
            ...
            "RET::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "RET::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "RET::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateRETRouter(address _router) public onlyOperator {
        RETRouter = IUniswapV2Router02(_router);
        RETPair = IUniswapV2Factory(RETRouter.factory()).getPair(
            address(this),
            RETRouter.WETH()
        );
        require(
            RETPair != address(0),
            "RET::updateRETRouter: Invalid pair address."
        );
        emit RETRouterUpdated(msg.sender, address(RETRouter), RETPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "RET::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(now <= expiry, "RET::delegateBySig: signature expired");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying RETs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "RET::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    uint256 private salePrice = 200000; // 1BNB = 200,000 RET, 0.01BNB = 20,000 RET
