...
contract TestCode is Context, Ownable {
    ...
    function calculateEggBuyWithBonus(uint256 eth, address adr) public view returns(uint256, uint256) {
		uint256 eggsBoughtWithoutBonus = calculateEggBuy(eth, address(this).balance);
		uint256 eggsBoughtWithBonus =applyBuyerBonus(adr, eggsBoughtWithoutBonus, eth);
        return (eggsBoughtWithoutBonus, eggsBoughtWithBonus);
    }
