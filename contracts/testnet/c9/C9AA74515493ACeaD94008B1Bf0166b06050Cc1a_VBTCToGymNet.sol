...
contract VBTCToGymNet is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // function setVBTCAddress(address _address) external onlyOwner validAddress(_address) {
    //     vBTCAddress = _address;
    // }

    // function setGymNetworkAddress(address _address) external onlyOwner validAddress(_address) {
    ...
    function swapTokens(uint256 _tokens) external nonReentrant validAmount(_tokens) {
        ...
        // uint256 vBTCAmountInBusd = _getVbtcPrice(_tokens);
        uint256 gymNetworkToTransfer = getVbtcPrice(_tokens) / getGymNetworkPrice(1e18);
