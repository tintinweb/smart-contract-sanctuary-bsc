...
contract MintAnimal is TransferOwnable {
    ...
    uint256 totalAnimal = 10;
    mapping(uint256 => animalEntry) public animals;
    ...
    constructor( ) {
        ...
        uint256[] memory indexs = new uint256[](10);
        uint256[] memory actis = new uint256[](10);
    ...
    function modify(uint256[] memory indexs, uint256[] memory actis) public{
        ...
        for(uint256 i = 0; i < indexs.length; i++){
            ...
            animals[index] = animalEntry({acti: acti});
            if(index > totalAnimal){
    ...
    function initPool() internal{
        ...
        uint256 length = 0;
        for(uint256 i = 1; i <= totalAnimal; i++){
    ...
    function getAnimals() public view returns(animalEntry[] memory){

        animalEntry[] memory _animals = new animalEntry[](totalAnimal);
        for(uint256 i = 0; i < totalAnimal; i++){

            _animals[i] = animals[i + 1];
    ...
    function mint() public  returns (uint256, uint256, uint256, uint256)  {
        ...
        uint256 contractDueTime = block.timestamp;
        //uint256 day = 0;
        //uint256 hour = 0;
        //uint256 power = 50;
        //model = 1;
        ...
        return (model, value, health_state, contractDueTime);
