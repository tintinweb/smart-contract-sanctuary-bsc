...
contract GymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice Change lp address of the pool. Can only be called by the owner
     * @param _poolId: id of the pool that is changed
     * @param _allocPoint: allocPoint for new pool
     * @param _lpToken: address of lpToken for new pool
     * @param _withUpdate: if true, update all pools
     */
    function updatePool(
        ...
        require(_poolId < poolInfo.length, "GymFarming: Pool with that id does not exist");
        require(_isSupportedLP(_lpToken), "GymFarming: Unsupported liquidity pool");
        ...
        totalAllocPoint -= poolInfo[_poolId].allocPoint;
        totalAllocPoint += _allocPoint;
        isPoolExist[address(poolInfo[_poolId].lpToken)] = false;

        poolInfo[_poolId].lpToken = _lpToken;
