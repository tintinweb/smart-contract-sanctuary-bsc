...
contract MythWhiteList {
    ...
    function mintMythForBet(
        ...
        mythContract.mint(owner, mintUnit * 20);
        uint256 marketShare = 10;
        if (getAffiliatePercent(better)) {
            mythContract.mint(better, mintUnit * 6);
            mythContract.mint(affiliateAddress[better], mintUnit * 4);
        } else {
            mythContract.mint(better, mintUnit * 5);
            marketShare += 5;
        }
        if (caller == address(0)) {
            marketShare += 10;
        } else {
            if (getAffiliatePercent(caller)) {
                mythContract.mint(caller, mintUnit * 6);
                mythContract.mint(affiliateAddress[caller], mintUnit * 4);
            } else {
                mythContract.mint(caller, mintUnit * 5);
                marketShare += 5;
            }
        }
        mythContract.mint(marketingWallet, mintUnit * marketShare);
        return true;
    }
...
contract MythCoinFlip {
    ...
    uint256 public minBet = 10**16;
    uint256 public maxBet = 1 * (10**17);
    ...
    function placeBet(bool _pvp, uint8 _side) external payable {
        ...
        if (!_pvp) {
            ...
            bool mintedTokens = whitelist.mintMythForBet(
                placedBets[betCount].amount,
                placedBets[betCount].better,
                address(0)
            );
            require(mintedTokens, "Failed to mint Myth");
            betCount++;
        } else {
            placedBets[betCount] = betStructure(
                msg.sender,
