...
contract TokieGovernor is Governor, GovernorCompatibilityBravo , GovernorTimelockControl {
    ...
    constructor(
        ...
        address _tokie, 
        address _masterchef,
        uint256[] memory _tokieFarmIds, 
        address[] memory _compoundPools)
        Governor("TOKIECompanyReserveGovernor")
    ...
    function setTokieFarmIds(uint256[] memory _tokieFarmIds) public onlyDev {
        tokieFarmIds = _tokieFarmIds;
    }
    ...
    function voteWeightForProposal(uint256 proposalId, address account) public view returns (uint256) {
        uint256 snapshot = proposalSnapshot(proposalId);
        return tokie.balanceOf(account) + extraWeights[account][snapshot];
    }
    ...
    function calculateWeight(uint256 proposalId, address account) public {
        ...
        for (uint256 i = 0; i < tokieFarmIds.length; i ++) {
            (address lpToken,,,,) = masterchef.poolInfo(tokieFarmIds[i]);
            (uint256 amount,,,) = masterchef.userInfo(tokieFarmIds[i], account);
            uint256 totalSupply = IUniswapV2Pair(lpToken).totalSupply();
            uint256 balance0 = tokie.balanceOf(lpToken);
    ...
    function getVotes(address account, uint256 blockNumber)
        ...
        returns (uint256)
    {
        return tokie.balanceOf(account) + extraWeights[account][blockNumber];
    }
    ...
    function cancel(uint256 proposalId) public virtual override (GovernorCompatibilityBravo) {
        ...
        if(rewardLeft > 0){
            if(tokie.balanceOf(address(this)) < rewardLeft){
                rewardLeft = tokie.balanceOf(address(this));
            }
            tokie.transfer(_msgSender(), rewardLeft);
        }
    }
        ...
        returns (uint256)
    {
        ...
        tokie.transferFrom(_msgSender(), address(this), totalReward);
    ...
    ) internal virtual override(Governor) returns (uint256)
    {
        ...
        if(proposalReward[proposalId].totalDistributed < proposalReward[proposalId].totalCountedReward){
            ...
            if(tokie.balanceOf(address(this)) > myReward){
                tokie.transfer(_msgSender(), myReward);
