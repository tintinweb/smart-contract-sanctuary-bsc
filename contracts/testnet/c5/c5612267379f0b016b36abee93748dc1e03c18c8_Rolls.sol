...
contract Rolls {
    ...
    // 设置每日产出[给ROLLS用]
    function setGen(uint256 coinType, uint256 daily, uint256 startHeight, uint256 dayCount) public onlyOwner {
        uint256 l = genInfoArray.length;
        for (uint256 i = 0; i < l; i++) {
            if (genInfoArray[i].coinType != coinType) continue;
            genInfoArray[i].daily = daily;
            genInfoArray[i].startHeight = startHeight;
            genInfoArray[i].dayCount = dayCount;
            break;
        }
        genInfoMap[coinType].daily = daily;
        genInfoMap[coinType].startHeight = startHeight;
