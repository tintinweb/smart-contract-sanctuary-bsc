...
contract BtcSnacks is SnacksBase {
    ...
    uint256 public constant D = 100000000;
    uint256 public constant FEE_PULSE = 15;
    uint256 public constant FEE_POOL_REWARD_DISTRIBUTOR = 35;
    ...
    function _beforeDistributeFee(
        ...
        if (feeBtcSnacks_ > 0) {
            // % BtcSnacks holders
            uint256 feeToHolders = (feeBtcSnacks_ * feeBTCETHSnacksHolders) /
                100;
            ...
            if (totalBtcHoldersBalance_ > 0) {
                uint256 holdersListLength = _holderList.length();
                for (uint256 i = 0; i < holdersListLength; i++) {
                    address holder = _holderList.at(i);
                    uint256 holderFee = (feeToHolders * balanceOf(holder)) /
                        totalBtcHoldersBalance_;
                    IERC20(address(this)).transfer(holder, holderFee);
                }
            }
            // % Snacks holders
            if (feeSnacksHolders > 0) {
                IERC20(address(this)).transfer(
                    snacks,
                    (feeBtcSnacks_ * feeSnacksHolders) / 100
                );
            }
        }
    }

    function _D() internal pure override returns (uint256) {
        return D;
    }
}
...
abstract contract SnacksBase is ERC20, Ownable {
    ...
    address public immutable poolRewardDistributor; // Pool reward distributor Pool contract address (Pool reward distributor)
    ...
    uint256 public immutable feePoolRewardDistributor; // Pool reward distributor
    ...
    // All Snacks holders
    EnumerableSet.AddressSet internal _holderList;
    ...
    constructor(
        ...
        address seniorageAddress_,
        uint256 feePulse_,
        uint256 feePoolRewardDistributor_,
        ...
        // Set fee
        feePulse = feePulse_;
        feePoolRewardDistributor = feePoolRewardDistributor_;
        ...
        _blockedHolders[poolRewardDistributor] = true;
        _blockedHolders[address(this)] = true;
    }

    // ================= Methods: Publics/External ======================================================

    function holderList(uint256 index) public view returns (address) {
        return _holderList.at(index);
    }
    ...
    function calculateRedeemAmount(
        ...
        uint256 snacksValue = (amountSnacksToBurn *
            (currentTotalSupply + end)) / 2;
        uint256 zoinksToReturn = _unDecimals(snacksValue * multiplier, 2);
    ...
    function calculateBuyAmount(
        ...
        uint256 snacksValue = ((newLastSnack + nextFirstSnack) *
            (newLastSnack - nextFirstSnack + ONE_SNACK)) / 2;

        uint256 zoinksToPay = _unDecimals(snacksValue * multiplier, 2);
        return zoinksToPay;
    }
    ...
    function buy(uint256 amountSnacksToMint) external {
        ...
        uint256 snacksFeeAmount = (amountSnacksToMint * TAX_PERCENT_FOR_BUY) /
            100;
    ...
    function redeem(uint256 amountSnacksToBurn) external {
        ...
        uint256 snacksFeeAmount = (amountSnacksToBurn *
            TAX_PERCENT_FOR_REDEEM) / 100;
        // Step 2: Move Snacks fee to contract like undistributed fee
        _transfer(msg.sender, address(this), snacksFeeAmount);
    ...
    function distributeFee() external onlyOwner {
        ...
        uint256 totalHoldersBalance = getTotalHoldersBalance();
        ...
        if (feeSnacks > 0) {
            ...
            if (feePulse > 0) {
                IERC20(address(this)).transfer(
                    pulse,
                    (feeSnacks * feePulse) / 100
                );
            }
            // % Snacks staking pool
            if (feePoolRewardDistributor > 0) {
                IERC20(address(this)).transfer(
                    poolRewardDistributor,
                    (feeSnacks * feePoolRewardDistributor) / 100
                );
            }
    ...
    function _afterTokenTransfer(
        ...
        if (amount > 0) {
            ...
            if (
                from != address(0) &&
                balanceOf(from) == 0 &&
                !_blockedHolders[from]
            ) {
                // This address not has any balance
                require(_holderList.remove(from), "cannotRemoveHolder");
            }
            // Step 2: Check to account
            if (to != address(0) && !_holderList.contains(to)) {
    ...
    function _addHolder(address holder_) private {
        ...
        if (!_blockedHolders[holder_]) {
            require(_holderList.add(holder_), "cannotAddNewHolder");
        }
    }

    // Description:
    //  Get balance of all holders
    function getTotalHoldersBalance() public view returns (uint256) {
        uint256 total = 0;
        uint256 holdersLength = _holderList.length();
        for (uint32 i = 0; i < holdersLength; i++) {
            total += balanceOf(_holderList.at(i));
        }
        return total;
    }
    ...
    /// @notice Different values of D in subcontracts (childs) for internal use by formula in calculateBuyTotal
    /// @dev Anton P.
    /// @return value D for formula in calculateBuyTotal
    function _D() internal pure virtual returns (uint256) {}

    /// @notice Calculate how many Snacks/ETHSnacks/BTCSnacks we can buy with this amount
    /// @dev Stepan
    /// @param payTokenAmount_ How many tokens we wont to pay to get Snacks/BTCSnacks/ETHSnacks
    /// @return How many Snacks/BTCSnacks/ETHSnacks can you buy with Zoinks/BTC/ETH
    function calculateBuyTotal(uint256 payTokenAmount_)
        public
        view
        returns (uint256)
    {
        uint256 firstInRange = multiplier + totalSupply() / _D();
        require(payTokenAmount_ >= firstInRange, "Insufficient tokens amount");
        uint256 a = multiplier;
        uint256 b = 2 * firstInRange - a;
        uint256 c = 2 * payTokenAmount_;
        uint256 discriminant = (b**2) + 4 * a * c;
        uint256 squareRoot = Math.sqrt(discriminant);
        uint256 result = PRBMathUD60x18.div((squareRoot - b), (2 * a));
        return result;
    }

    // Description:
    //  Buy max X Snacks by Zoinks/ETH/BTC amount
    function buyTotal(uint256 payTokenAmount_) external {
        require(
            payToken.balanceOf(msg.sender) >= payTokenAmount_,
            "ERROR: need many tokens to pay Snacks"
        );
        uint256 amountSnacksToMint = calculateBuyTotal(payTokenAmount_);
        payToken.safeTransferFrom(msg.sender, address(this), payTokenAmount_);
        uint256 snacksFeeAmount = (amountSnacksToMint * TAX_PERCENT_FOR_BUY) /
            100;
        _mint(address(this), snacksFeeAmount);
        _mint(address(msg.sender), amountSnacksToMint - snacksFeeAmount);
