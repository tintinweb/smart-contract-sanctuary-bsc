...
contract GYM is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant MAX_SUPPLY = 10_200;
    ...
    mapping(address => uint256[5]) private childs;
    ...
    function refund(address to, uint256 val) private {
        if (val > 0) {
    ...
    function rewardingRef(address user, uint256 totalPrice) private {
        uint256 i;
        address father = user;
        for(i=0; i<refPercents.length; i++){
             father = fathers[father];
             if( father == address(0) ) break;
            refund(father, totalPrice / 100 * refPercents[i]);
        }
    ...
    function getChilds(address a) public view returns(uint256[5] memory){
        return childs[a];
    }
    ...
    function getBNBperNFT(uint256 count) public view returns (uint256) {
        return getBNBper1USD()/(_sale.priceUsd/1e15) * count * 1e3;
    }
    ...
    function buyForUSD(address to,uint256 mintAmount,address ref ) external whenNotPaused nonReentrant cooldown {
        ...
        for(i=0; i<refPercents.length; i++){
             father = fathers[father];
             if(father==address(0)) break;
             uint256 bonusUsd = priceBUSD / 100 * refPercents[i];
            _tokenBUSD.transferFrom(msg.sender, father, bonusUsd);
            refAmount+=bonusUsd;
        }
        _tokenBUSD.transferFrom( msg.sender, address(this), priceBUSD - refAmount);
