...
contract PYESwap_Base is IPYE, Context, Ownable {
    ...
    string constant _symbol = "PSWAPBASE";
    ...
    uint256 public swapThreshold = 1 * 10**16; // 0.001 WBNB
    bool inSwap;
    ...
    constructor(address _marketing, uint256 _marketingFee, uint256 _liquifyFee) public {
    ...
    function liquifyThreshold() public view returns (uint256) {
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.marketing).sub(values.liquify);
        return values;
    }
    ...
    function _takeFees(FeeValues memory values) private {
        ...
        _takeFee(values.liquify, address(this));
    }
    ...
    function shouldBuyAndLiquify() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && IERC20(WBNB).balanceOf(address(this)) >= swapThreshold;
    }
    ...
    function buyAndLiquify() internal swapping {
        ...
        uint256 amountTokenToAdd = IERC20(address(this)).balanceOf(address(this));

        _approve(address(this), address(pyeSwapRouter), amountTokenToAdd);
        _approve(address(this), address(pyeSwapRouter), amountWBNBtoAdd);
