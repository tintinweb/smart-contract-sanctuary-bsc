...
contract ImplementationV1 is Implementation {
     ...
     * @param _data (0) token id, (1) totalCopies, (2) onSaleQuantity, (3) price, (4) amount, (5) token type
     * @param _addr (0) buyer, (1) seller, (2) signer, (3) collectionAddress, (4) tokenAddress
     * @param _internalTxId (0) saleOrderId, (1) transactionId
     * @param _signatures (0) saleOrderSignature, (1) mintRequestSignature
    ...
    function handleMintRequest(
        ...
        string[] memory _str
    ) public payable notBlocked notAdmins nonReentrant {
        (bool success, ) = payable(mintHandler).call{value: msg.value}(
            abi.encodeWithSignature(
                "handleMintRequest(uint256[],address[],bytes[],bytes[],string[])",
                _data,
                _addr,
                _internalTxId,
                _signatures,
        ...
        require(success, "Implementation: Failed to handle mint request");
    }

    /**
     * @dev Handler buy request
     * @param _data (0) token id, (1) onSaleQuantity, (2) price, (3) token type, (4) amount
     * @param _addr (0) buyer, (1) seller, (2) signer,(3) tokenAddress
     * @param _internalTxId (0) saleOrderId, (1) transactionId
     * @param _signatures (0) saleOrderSignature, (1) buyRequestSignature
     */
    function handleBuyRequest(
        ...
        (bool success, ) = payable(buyHandler).call{value: msg.value}(
            abi.encodeWithSignature(
                "handleBuyRequest(uint256[],address[],bytes[],bytes[],string[])",
                _data,
                _addr,
                _internalTxId,
                _signatures,
        ...
        require(success, "Implementation: Failed to handle buy request");
    }

    /**
     * @dev Handle cancel sale order request
     * @param _data (0) onSaleQuantity, (1) price, (2) tokenType
     * @param _addr (0) seller, (1) caller
     * @param _str (0) saleOrderId
    ...
    function handleCancelOrder(
        ...
        (bool success, ) = cancelHandler.call(
            abi.encodeWithSignature(
                "handleCancelOrder(uint256[],address[],bytes[],bytes[],string[])",
                _data,
                _addr,
                _internalTxId,
                _signatures,
        ...
        require(success, "Implementation: Failed to handle cancel request");
    ...
    function handleMintRequestByAdmin(
        ...
        bytes memory _data
    ) public onlyAdmins nonReentrant {
        (bool success, ) = payable(mintHandler).call(
            abi.encodeWithSignature(
                "handleMintRequestByAdmin(address,bytes,uint256[],uint256,uint256[],bytes)",
                _account,
                _nftId,
                _tokenIds,
                _tokenType,
...
contract Implementation is Upgradeable {
     ...
     * @param _value true/false
     */
    function setSignerList(address _account, bool _value) public onlyAdmins {
        signerList[_account] = _value;
    }
