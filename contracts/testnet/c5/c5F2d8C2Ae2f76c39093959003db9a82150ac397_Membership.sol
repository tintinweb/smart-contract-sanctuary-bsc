...
contract Membership is ERC721Enumerable, ERC721Pausable, Ownable {
     ...
     */
    function setNewPool(address newPool) external onlyOwner whenNotPaused {
        currentPool = newPool;
     ...
     */
    function setBaseURI(string memory baseTokenURI) external onlyOwner whenNotPaused {
        _baseTokenURI = baseTokenURI;
    }
     ...
     */
    function getDefaultMembership(address account) external view returns (uint256) {
    ...
    function mint() external whenNotPaused returns (uint256) {
        require(msg.sender == tx.origin, "Membership/do-not-support-smart-contract");
    ...
    function _usableTokenId(address owner, uint256 tokenId) internal view returns (uint256) {
        ...
        if (tokenId == 0) {
            ...
            require(ownerOf(tokenId) == owner, "Membership/caller-is-not-card-owner");
        }
        return tokenId;
    }

    function _setFirstOwnedDefaultMembership(address account, uint256 tokenId) internal {
