...
contract Purgatory is Ownable, ERC20 {
    ...
    function claimNFTsSoulToken(uint nft) public {
        address a = LRT.ownerOf(nft);  
        require(a == _msgSender());
        require(NFTs[nft].Soultoken > 0); 
        uint value = NFTs[nft].Soultoken*SoulValue();
        _mint(_msgSender(), value);
        if(NFTs[nft].Rarity < 4) Soulsupply -= NFTs[nft].Soultoken;
        NFTs[nft].Soultoken = 0;
        Soulpool -= value;     
    }

    function claimCommonNFT(uint nft) public {
        address a = LRT.ownerOf(nft);
        require(a == _msgSender());
        require(NFTs[nft].Rarity == 1);
        _mint(_msgSender(), NFTs[nft].RHT);
        NFTs[nft].RHT = 0;
        SupplyNFTs[nft] = SupplyNFTs[SupplyNFTs.length - 1];
        SupplyNFTs.pop();
    }

    function checkNFT (uint nft) public view returns(bool passed) {
        passed = false;
        for(uint i = 0; i < SupplyNFTs.length; i++) {
            uint x = SupplyNFTs[i];
            if(nft == x) passed = true;
        }
    }
    ...
    function nftRarity(uint nft) public view returns(uint rar) {
        rar = NFTs[nft].Rarity;
    }

    function nftRHT(uint nft) public view returns(uint nRHT) {
        nRHT = NFTs[nft].RHT;
    }

    function nftSoultoken(uint nft) public view returns(uint token) {
        token = NFTs[nft].Soultoken;
    }

    function nftUsedToken(uint nft) public view returns(uint utoken) {
        utoken = NFTs[nft].usedToken;
    }

    function nftScore(uint nft) public view returns(uint score) {
        score = NFTs[nft].Score;
    }
}