...
contract TSLVault is Initializable, OwnableUpgradeable, ReentrancyGuard {
    ...
    //inviter info
    mapping(address => uint256) public userInvitedValue;
    //inviter Referral reward
    mapping(address => uint256) public referralReward;
    mapping(address => uint256) public userRefClaimed;
    ...
    function deposit(
        ...
        require(userLevel[msg.sender] >= level_, "unauthorized user level");
        require(
        ...
        userDepositInfo[msg.sender].push(userInfo);
    }
    ...
    function claimRefReward() public nonReentrant {
        ...
        IERC20(usdt).transferFrom(address(this), msg.sender, amount);
    ...
    function setLevelNumber(uint256[] memory nums) public onlyOwner {
        for (uint256 i = 0; i < nums.length; i++) {
            LevelNum[i + 1] = nums[i];
        }
    }

    function setLevelBalance(uint256[] memory bal_) public onlyOwner {
