...
contract Launchpad is Ownable, ReentrancyGuard {
    ...
    function mintWhitelisted(
        ...
        require(currentSupply + batchSize <= campaign.maxSupply + _campaignsNormal[contractAddress].maxSupply, "reach campaign max supply");
    ...
    function mint(address contractAddress, uint256 batchSize) external payable nonReentrant {
        ...
        // NFT contract must impl ERC721Enumerable to have this totalSupply method
        uint256 currentSupply = ILaunchpadNFT(contractAddress).getLaunchpadSupply();
        require(currentSupply + batchSize <= campaign.maxSupply + _campaignsWhitelisted[contractAddress].maxSupply, "reach campaign max supply");
        uint256 totalPrice = campaign.price * batchSize;
        require(msg.value >= totalPrice, "value not enough");
    ...
    function getLaunchpadMaxSupply(address contractAddress) external view returns (uint256) {
        return ILaunchpadNFT(contractAddress).getMaxLaunchpadSupply();
    }

    function getLaunchpadSupply(address contractAddress) external view returns (uint256) {
    ...
    function addCampaign(
        ...
        campaign = Campaign(contractAddress_, payeeAddress_, price_, maxSupply_, listingTime_, expirationTime_, maxBatch_, maxPerAddress_, validator_);
        if (mode == CampaignMode.normal) {
