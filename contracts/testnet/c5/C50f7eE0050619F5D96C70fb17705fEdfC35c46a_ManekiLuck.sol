...
contract ManekiLuck is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    uint randNonce = 0;
    string private _name = "ManekiLuck_v1";
    string private _symbol = "NKO1";
    uint8 private _decimals = 9;
    bool public market_active;
    mapping (address => bool) public _isBlacklisted;
    mapping (address => bool) public premarket_user;

    address payable public marketingWalletAddress = payable(0x2686fa6e4F3E78EBEb988ea534a607F28868ab18);
    address payable public teamWalletAddress = payable(0x48a52e3bF0d472b43b1Afe3C33E9a0a01Ad71aA3);
    ...
    uint256 public _buyJackPotFee = 4;
    uint256 public _buyTeamFee = 4;
    
    uint256 public _sellLiquidityFee = 1;
    uint256 public _sellMarketingFee = 4;
    uint256 public _sellJackPotFee = 4;
    ...
    uint256 public _JackPotShare = 30;
    ...
    uint256 private lotteryBalanceLimit=1;
    uint256 private tokenAmountForLotteryParticipant = 5000000 * 10**_decimals; 
    ...
    function isPartecipant(address account) public view returns(bool){
        return lotteryParticipants.getIndexOfKey(account) >=0 ? true : false;
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            totalAmount = amount.add(_balances[address(recipient)]);
            if(totalAmount >= tokenAmountForLotteryParticipant)
                lotteryParticipants.set(recipient,totalAmount);
        }
        else if(isMarketPair[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
            totalAmount = _balances[address(sender)].sub(amount);
            if(totalAmount < tokenAmountForLotteryParticipant && lotteryParticipants.getIndexOfKey(sender) >= 0)
                lotteryParticipants.remove(sender);
        }else{

            if(_balances[address(recipient)].add(amount)< tokenAmountForLotteryParticipant && lotteryParticipants.getIndexOfKey(recipient) >= 0)
                lotteryParticipants.remove(recipient);
            if(_balances[address(sender)].sub(amount)< tokenAmountForLotteryParticipant && lotteryParticipants.getIndexOfKey(sender) >= 0)  
                lotteryParticipants.remove(sender);  
        }
    ...
    function lotteryDraw() public onlyOwner{
        ...
        if(amountJackpot > lotteryBalanceLimit)
            amountJackpot = amountJackpot.sub(lotteryBalanceLimit);
