...
Buy tax
Tax: 10% (slippage 11%)
4% Payouts in $BTCB, $ETH, $BUSD, $MATIC, $ADA, $BNB & $SLAKT, more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $Bnb
0.6% admin 
...
Payouts
→ all the sellers tax is given back in SLAKTple tokens → no $SLAKT price dump from selling the sell tax
Out of the seling tax:
50% holders in $BTCB, $ETH, $ADA, $MATIC, $BNB, $BUSD & $SLAKT
...
interface DAPP {
    ...
    function ChooseSLAKTplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
...
// slaktern Network contract

contract slaktern is ERC20, ReentrancyGuard {
    string public constant symbol = "SLAKT"; //SLAKT
    string public constant name = "slaktern"; //slaktern Network
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
   ...
   // address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor() {
        ...
        _enableTransfer = true; //defaSLAKT
...
//        __balanceOf[address(this)] = __totalSupply/2;
     ...
     //launch_time = block.timestamp;  //now as defaSLAKT - used for tax calculation
   AddTokentoPayoutList(_pancakeRouter.WETH(), "BNB"); //BNB added as defaSLAKT
   AddTokentoPayoutList(address(this), "SLAKT"); //this contract token
   
   
   //defaSLAKT testnet tokens for debugging
 ...
 //contract addresses are as defaSLAKT excluded from auto payouts of rewards and have
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as defaSLAKT
         N_holders++;
         _excludedfromTax[recipient] = ExcludedfromTax;
         
         _SplitTokenRewards[recipient] = true;
         _RewardTokens[recipient].Slice1 = 75; //defaSLAKT 75% bnb
         _RewardTokens[recipient].Slice1 = 25; //25% SLAKT
...
function checkRewards(address addr) public view returns (uint256)
{
   return _earnings[addr];
        ...
        if(LotteryPot > 0)
        {
             if(random(100) < 5) //won - buyamount SLAKTplied 1/20 probability
