...
contract DeFiAIMultiStrat is IDeFiAIMultiStrat, ReentrancyGuard, Ownable, Pausable {
    ...
    function updateBalance(address user, uint256 balance) external onlyGovernance {
    ...
    function _convertWantToLp() internal {
        ...
        path[0] = wantAddress;
        path[1] = pairAddress;
        IERC20(wantAddress).safeIncreaseAllowance(farmInfo[0].routerAddress, wantAmt / 2);
        _safeSwap(
        ...
        uint _tokenA = IERC20(wantAddress).balanceOf(address(this));
        uint _tokenB = IERC20(pairAddress).balanceOf(address(this));
        IERC20(wantAddress).safeIncreaseAllowance(farmInfo[0].routerAddress, _tokenA / 2);
        IERC20(pairAddress).safeIncreaseAllowance(farmInfo[0].routerAddress, _tokenB / 2);
        ...
        _tokenA = IERC20(wantAddress).balanceOf(address(this));
        _tokenB = IERC20(pairAddress).balanceOf(address(this));

        IERC20(wantAddress).safeIncreaseAllowance(farmInfo[1].routerAddress, _tokenA);
        IERC20(pairAddress).safeIncreaseAllowance(farmInfo[1].routerAddress, _tokenB);
    ...
    function _convertLpToWant() internal {
        ...
        path[0] = pairAddress;
        path[1] = wantAddress;
        uint _pair = IERC20(pairAddress).balanceOf(address(this));
        IERC20(pairAddress).safeIncreaseAllowance(farmInfo[0].routerAddress, _pair);
        _safeSwap(
            farmInfo[0].routerAddress, 
            _pair, 
            9900,
