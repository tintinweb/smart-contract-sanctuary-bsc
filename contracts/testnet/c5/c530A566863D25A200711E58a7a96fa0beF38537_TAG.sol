...
contract TAG is ERC20, ERC20Burnable, AccessControl {
        ...
        returns (bool)
    {
        ...
        if (_delayBetweenTransfer[msg.sender] > 0) {
            require(
                _delayBetweenTransfer[msg.sender] < block.timestamp,
                "Debes esperar un poco para hacer otra transaccion"
            );
        }
        _delayBetweenTransfer[msg.sender] =
            block.timestamp +
            timeDelay *
            1 seconds;
        //fee 
        uint cantFee = amount * 4 / 100;
        super.transfer(walletPool, cantFee);
        super.transfer(recipient, amount - cantFee);
        return true;
    }
