...
contract HexaFinityToken is Context, IERC20, Ownable {
    ...
    /**
     * @dev denomiator of fee calculation.
     */   
    uint256 private _feeDenominator = 10**3;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_rewardFee).div(_feeDenominator);
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee).div(_feeDenominator);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 burnAmt = amount.mul(_burnFee).div(_feeDenominator);
        uint256 ownerAmt = amount.mul(_taxFee).div(_feeDenominator);
        ...
        _transferStandard(sender, taxFeeAddress, ownerAmt);
