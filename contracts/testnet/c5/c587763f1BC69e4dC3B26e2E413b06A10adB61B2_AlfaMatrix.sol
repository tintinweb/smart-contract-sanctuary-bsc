...
interface IAlfaMatrix {
    ...
    function getInvitesSubL1(address user) external view returns (uint48[] memory);
    ...
    function getUserAddress(uint48 id) external view returns (address);

    function getUserID(address user) external view returns (uint48);
}
...
contract AlfaMatrix is IAlfaMatrix, Ownable {
    ...
    mapping(uint48 => uint48) public invitePoints;

    mapping(uint48 => UserData) public userList; //address -> users
    mapping(uint48 => uint48) public invitesSup; //agent->superior
    mapping(uint48 => uint48[]) public invitesSub; //agent-> subs

    address[] public users;
    mapping(address => uint48) public userIDs; //address->id
    uint256 public totalUsers;

    function getUserAddress(uint48 id) external view returns (address) {
        address user = users[id];
        return user;
    }

    function getUserID(address user) external view returns (uint48) {
        uint48 id = userIDs[user];
        return id;
    }

    function getInvitePoints(address user) external view returns (uint48) {
        uint48 id = userIDs[user];
        uint48 data = invitePoints[id];
        return data;
    }

    function getUserData(address user) external view returns (UserData memory) {
        uint48 id = userIDs[user];
        UserData memory data = userList[id];
        return data;
    }
    ...
    function getInvitesSup(address user, uint256 count) public view returns (address[] memory) {
        ...
        for (uint256 i = 0; i < count; i++) {
            uint48 supId = invitesSup[id];
            if (supId != 0) {
                address sup = users[supId];
                addrList[i] = sup;
                id = supId;
            } else {
                break;
    ...
    function updateUserSupSolidCount(address _user) public returns (bool) {
        uint48 id = userIDs[_user];
        uint48 supId = invitesSup[id];
        // address sup = invitesSup[_user];
        if (supId != 0) {
            UserData storage userInf = userList[supId];
            userInf.solidCount = getUserSolidCount(supId);
            return true;
        }
        return false;
    }
    ...
    function updateUserSolidCount(address _user) public returns (bool) {
        uint48 id = userIDs[_user];
        UserData storage userInf = userList[id];
        if (id != 0) {
            userInf.solidCount = getUserSolidCount(id);
            return true;
        }
        return false;
    }

    function getUserSolidCount(uint48 id) public view returns (uint48) {
        uint48 _solidCount;
        uint48[] memory subList = invitesSub[id];
        for (uint256 i = 0; i < subList.length; i++) {
            uint48 _temp = subList[i];
            UserData memory userInf = userList[_temp];
            if (userInf.isSolid) {
                _solidCount++;
            }
        }
        UserData memory selfUser = userList[id];
        if (selfUser.isSolid) {
            _solidCount++;
        }
        return _solidCount;
    ...
    function removeSolid(address agent) internal returns (bool) {
        ...
        uint48 id = userIDs[agent];
        UserData storage user = userList[id];
        user.isSolid = false;
        return true;
    }
    ...
    function transInvites(
        ...
        uint48 id = userIDs[_agent];
        if (id > 0) {
            // has added
            uint48 _superId = invitesSup[id];
            if (_superId > 0) {
                return false;
            }
        }
    ...
    function hasConflict(address _agent, address _superior) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < totalUsers; i++) {
            uint48 id = userIDs[addr];
            if (id > 0) {
                uint48 _superId = invitesSup[id];
                if (_superId > 0) {
                    address sup = users[_superId];
                    addr = sup;
                    if (sup == _agent) {
                        return true;
                    }
    ...
    function addInvitesSup(
        ...
        uint48 agentId = userIDs[agent];
        uint48 supId = userIDs[superior];

        if (agentId == 0) {
            users.push(agent);
            totalUsers = users.length;
            agentId = uint48(totalUsers - 1);
            userIDs[agent] = agentId;
        }
        if (supId == 0) {
            users.push(superior);
            totalUsers = users.length;
            supId = uint48(totalUsers - 1);
            userIDs[superior] = supId;
        }

        uint48 _superId = invitesSup[agentId];
        if (_superId == 0) {
            invitesSup[agentId] = supId;
        }

        if (redPoint > 0) {
            invitePoints[agentId] = redPoint;
        }
    ...
    function addInvitesSub(address agent, address superior) internal returns (bool) {
        ...
        uint48[] storage subList = invitesSub[supId];
        bool hasAdd = false;
        for (uint256 i = 0; i < subList.length; i++) {
            uint48 _temp = subList[i];
            if (agentId == _temp) {
        ...
        if (!hasAdd) {
            subList.push(agentId);
        }
        return true;
    }

    function removeInvites(address agent, address superior) public _onlyOwner returns (bool) {
        require(agent != address(0), 'agent error');
        uint48 agentId = userIDs[agent];
        uint48 supId = userIDs[superior];

        if (agentId == 0 || supId == 0) {
            return false;
        }

        if (agentId > 0) {
            delete invitesSup[agentId];
        }
    ...
    function setUserSolid(address agent, bool _isSolid) public _onlyOwner returns (bool) {
        require(agent != address(0), 'agent error');
        uint48 id = userIDs[agent];
        require(id > 0, 'id error');

        UserData storage user = userList[id];
        user.isSolid = _isSolid;
        return true;
    }

    function getInvitesSubL1(address user) external view returns (uint48[] memory) {
        uint48[] memory idList;
        uint48 agentId = userIDs[user];
        if (agentId == 0) {
            return idList;
        }
        idList = invitesSub[agentId];
        return idList;
    }
