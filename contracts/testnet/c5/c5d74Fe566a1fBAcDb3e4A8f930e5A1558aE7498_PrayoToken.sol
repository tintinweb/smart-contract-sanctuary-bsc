...
Every transactions:
5% tax (where 80% goes to development and 20% goes to liquidity)
5% burned (after total supply less than 80,000,000 the burned stops, the 5% will goes to liquidity)

- Burned address cannot be changed
- Owner can change the taxes
- Owner can lock wallet address (for vesting purpose)
...
contract PrayoToken is ERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x4AE32fA28343dd8a7233e8E0fe450aD6AaF4AEa7;
    address private _burnAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 private constant MAX = ~uint256(0);
    uint256 private  _burnCap = 80000000 * 10**18; //burn cap
    uint256 private _tTotal = 800000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tBurnTotal;
    
    uint8 private _decimals = 18;
    uint256 public _developmentFee = 40; //4% for development team
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _liquidityFee = 10; //1% for liquidity / project continuity
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public _burnFee = 50; //auto-burn supply 5% until 80million supply left.
    ...
    function totalSupply() public view override returns (uint256) {
        (,uint256 total) = _getCurrentSupply();
        return total;
    }
    ...
    (uint256 rAmount, uint256 rTransferAmount, uint256 tTransferAmount, uint256 tLiquidity, uint256 tDevelopment, uint256 tBurn) = _getValues(tAmount);
    ...
    //we use these function for unit testing purpose at the moment
    function setBurnLimit(uint256 limit) public onlyOwner {
        require(limit > 0,'burn cap must > 0');
        _burnCap = limit * 10**18;
    }
    //we use these function for unit testing purpose at the moment
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 tTransferAmount, uint256 tLiquidity, uint256 tDevelopment, uint256 tBurn) = _getTValues(tAmount);


        //get values to be reflected
        (uint256 rAmount, uint256 rTransferAmount,) = _getRValues(tAmount, tLiquidity, tDevelopment, tBurn, _getRate());

        return (rAmount, rTransferAmount, tTransferAmount, tLiquidity, tDevelopment, tBurn);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256,uint256, uint256) {
        ...
        return (tTransferAmount, tLiquidity, tDevelopment, tBurn);
    }

    function _getRValues(uint256 tAmount, uint256 tLiquidity,uint256 tDevelopment,uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
