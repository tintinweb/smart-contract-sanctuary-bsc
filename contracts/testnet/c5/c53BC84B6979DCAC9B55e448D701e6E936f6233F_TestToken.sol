...
contract TestToken is ERC20, Ownable {
    ...
    function _transfer(
        ...
        // NEW VERSION
        if (takeFee) {
            uint256 feesBusd = (amount * BUSDRewardsFee) / 100;
            uint256 feesDev = (amount * DevFee) / 100;
            uint256 feesBurn = (amount * BurnFee) / 100;
            uint256 feesMkt = (amount * MarketingFee) / 100;

            amount -= feesBusd + feesDev + feesMkt + feesBurn;

            super._transfer(from, address(this), feesBusd);
            super._transfer(from, DevWallet, feesDev);
            super._transfer(from, BurnAddress, feesBurn);
            super._transfer(from, MarketingWallet, feesMkt);
        }
