 ...
 */
contract RVL is IERC20, Ownable {

    using SafeMath for uint256;

    // total supply
    uint256 private _totalSupply = 500_000 * 10**18;

    // token data
    string private constant _name = 'REVIVALx';
    string private constant _symbol = 'RVLx';
    ...
    // 0.01569634703 RVLx Per Block is 165,000 tokens per year or 33% APY from initial supply
    uint256 public emission_rate = 1569634703 * 10**7;
    ...
    function setEmissionRate(uint256 newRate) external onlyOwner {
        require(
            newRate < emission_rate,
            'Cannot Raise Emission Rate'
        );
