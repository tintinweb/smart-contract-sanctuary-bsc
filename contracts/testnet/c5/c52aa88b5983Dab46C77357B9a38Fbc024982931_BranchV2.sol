...
contract BranchV2 is ReentrancyGuard, AccessControlEnumerable {
    ...
    function userFinalDeposit(uint256 _index) external nonReentrant {
        ...
        // require(!userCheckBlacklist(msg.sender), "You are in blacklist");
        _lossCompensationClaim(msg.sender);
        IERC20(usdt).safeTransferFrom(msg.sender, address(this), deposit.finalAmount);
        ...
        brokers[msg.sender].depositTotal += deposit.finalAmount;
        brokers[msg.sender].depositCurrent += deposit.finalAmount;
        brokers[msg.sender].brokerBonusMax += deposit.finalAmount * 3;
        brokers[msg.sender].profitAmount -= int256(deposit.finalAmount);
        depositAmountDays[today()] += deposit.finalAmount;
        allocAmount(deposit.finalAmount);
    }

    function userWithdraw(uint256 _index) external nonReentrant {
        require(!userCheckBlacklist(msg.sender), "You are in blacklist");
        Deposit memory deposit = deposits[msg.sender][_index];
        require(deposit.status == 20, "Deposit can`t withdraw");
    ...
    uint256 public restartTimes = 10; // 重启次数
