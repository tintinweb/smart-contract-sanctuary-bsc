...
// File contracts/extensions/IERC20MetadataUpgradeable.sol
...
contract AloraNode is ERC20Upgradeable {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        bool _isBuying =  from==address(uniswapV2Pair);
        bool _isSelling =  to==address(uniswapV2Pair);  
        uint256 _amount = amount;
                
        if (!isExcludedFromFee[from] && !isExcludedFromFee[to]) {
            uint256 taxAmount = 0;
            if(_isSelling && checkNodeBeforeSell && nodeManagerAddress != address(0) ) {
    ...
    function claimTransfer(address from, address to, uint256 amount) public  {
    ...
    //     require(uniswapV2Pair != address(0), "Token::updateRouter: Invalid pair address.");
    ...
    // ReserveAllocation (RA)
    uint256 private _maxCapReserveAllocation  = 75000e18;
    uint256 public reserveAllocationAmount = 0; 
    ...
    // RewardPool (RP)
    uint256 private _maxCapRewardPool  = 700000e18;
    uint256 public rewardPoolAmount = 0; 
    ...
    //   Team Allocation (TA)
    uint256 private _maxCapTeamAllocation  = 75000e18;
    uint256 public teamAllocationAmount = 0; 
    ...
    //   Liquidity Pool (LP)
    uint256 private _maxCapLiquidityPool   = 150000e18;
    uint256 public liquidityPoolAmount = 0; 
    ...
    function whiteLister(bytes32[] calldata merkleProof) public returns(bool) {
    ...
    function depositToken(bytes32[] calldata merkleProof) public {
        require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Invalid WhiteLister");
        IERC20(stableToken).transfer(address(this), 1000e18);
        depositAmount[stableToken] += 1000e18;
    }
