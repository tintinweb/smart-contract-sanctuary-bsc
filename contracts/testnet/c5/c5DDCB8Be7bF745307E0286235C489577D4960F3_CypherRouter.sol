...
import "../app/node_modules/@openzeppelin/contracts/token/ERC1155/IERC1155.sol";
import "../app/node_modules/@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
contract CypherRouter is Ownable {
    ...
    event depositNFT(address indexed sender, uint256 _tokenId);
    event WithdrawNFT(address indexed sender, uint256 _tokenId);
    ...
    function executeNewSale(uint256 _idToken, uint256 _priceToken) public returns (bool) {
        ...
        cypherCollectionable.safeTransferFrom(msg.sender, address(cypherMarket), _idToken, 1, "");
    ...
    function executeSell(address _seller, uint256 _tokenId, uint256 _priceToken, bytes32 _hashId) public returns (bool) {
        ...
        require(cypherCollectionable.balanceOf(address(cypherMarket), _tokenId) > 0);
        ...
        cypherCollectionable.safeTransferFrom(address(cypherMarket), msg.sender, _tokenId, 1, "");
    ...
    function executeDepositToken(uint256 _tokenId) public returns (bool) {
        ...
        cypherCollectionable.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "");
        _deposits[msg.sender].push(_tokenId);
        emit depositNFT(msg.sender, _tokenId);
        return true;
    }

    function executeExtractToken(address _sender, uint256 _tokenId) public onlyOwner returns (bool) {
        //requiere que haya dejado un token en el contrato
        require(_tokenId != 0, "Invalid id");
        (bool checkWithdraw, uint256 pos) = canWithdraw(_sender, _tokenId);
        require(checkWithdraw, "No puedes retirar en este momento");
        require(cypherCollectionable.balanceOf(address(this), _tokenId) > 0, "No hay tokens para transferir");

    
        _removeItemOfArray(_deposits[_sender], pos);
        cypherCollectionable.safeTransferFrom(address(this), _sender, _tokenId, 1, "");
        emit WithdrawNFT(_sender, _tokenId);
        return true;
    }
