...
contract HeroNFT is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => string) public notRevealedUri;
    string public baseExtension = ".json";
   
    mapping(uint256 => string) private _tokenURIs;
    mapping(uint256 => uint256) public heroType;
   
    constructor(string memory name,string memory symbol,string memory initBaseURI)
        ERC721(name, symbol)
    {
        setBaseURI(initBaseURI);
    } 
    function mint(address owner,uint256 _level,uint256 tokenId) public {
        ...
        returns (string memory)
    {
        ...
        if (_revealed == false) {
            if(level[tokenId]==1){
                return notRevealedUri[1];
            }
            if(level[tokenId]==2){
                return notRevealedUri[2];
            }
            if(level[tokenId]==3){
                return notRevealedUri[3];
            }
    ...
    function setNotRevealedURI(string memory first,string memory second,string memory third) public onlyOwner {
        notRevealedUri[1] = first;
        notRevealedUri[2] = second;
        notRevealedUri[3] = third;
    }
