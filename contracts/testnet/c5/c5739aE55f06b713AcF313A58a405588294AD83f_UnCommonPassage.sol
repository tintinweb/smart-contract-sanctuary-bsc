    ...
    Ownable
{
    ...
    // xKeys
    address public immutable xKeys;
    uint256 public upgradeCost;

    constructor(
        address xKeys_,
        address rewardToken_,
        address CommonPassage_
    ) {
        require(xKeys_ != address(0), "Zero Address");
        xKeys = xKeys_;
    ...
    function mint(uint256[] calldata commonTokenIds) external {
        require(address(CommonPassage) != address(0), "Uncommon Not Paired");

        uint256 len = commonTokenIds.length;
        uint256 cost = upgradeCost * len;
        require(len > 0, "Zero IDs");
        require(
