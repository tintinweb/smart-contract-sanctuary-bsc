...
contract LabBunnies is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public minClaim = 1594 * 10**18;
    uint256 public maxClaim = 6376 * 10**18;
    ...
    function mint(uint256 _quantity) public {
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            bunnies[totalSupply()].attributes.image = generateImageName(
                totalSupply()
            );
            tokenId++;
    ...
    function claim(uint256 _poolId, uint256 bunnyId) public nonReentrant {
        ...
        if (claimedRewards >= maxClaim && lockedRewards >= maxClaim) {
            researchLab.safeLockedTransfer(msg.sender, _poolId, maxClaim);
        } else if (claimedRewards >= minClaim && claimedRewards < maxClaim && lockedRewards >= claimedRewards) {
            researchLab.safeLockedTransfer(msg.sender, _poolId, claimedRewards);
        } else if (claimedRewards < minClaim && lockedRewards >= minClaim) {
            researchLab.safeLockedTransfer(msg.sender, _poolId, minClaim);
        } else {
            researchLab.safeLockedTransfer(msg.sender, _poolId, lockedRewards);
        }
    ...
    function generateImageName(uint256 _tokenId)
        internal
        view
