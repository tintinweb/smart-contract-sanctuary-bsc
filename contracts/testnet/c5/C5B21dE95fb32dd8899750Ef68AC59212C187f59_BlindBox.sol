...
// File: contracts/NewBlindBox.sol
...
contract BlindBox is AccessController, ReentrancyGuard {
    event SetBox(address sellingNft, uint256 listingPrice);
    ...
    function buy(
        address nft,
        uint256 quantity
    ) external payable nonReentrant validBox(nft) {
        (
            IERC721 sellingNft,
            uint256 listingPrice
        ) = getBox(nft);
        ...
        require(
            sellingNft.balanceOf(address(this)) > 0,
            "BlindBox: Insufficient boxes"
        );
        require(msg.value >= listingPrice, "Not enough BNB to cover item price.");
        
        uint256[] memory tokenIds = new uint[](quantity);
        for (uint256 index = 0; index < quantity; index++) {
            console.log(index, sellingNft.balanceOf(address(this)));
