...
contract TestBBB_Checker_Contract is IERC20 { 
    ...
    TokenInfo private _tokenInfo = TokenInfo ( //77,777  100,000
           "TEST_BB", "BBB", 8,  100_000_000, /*10**_decimals;*/ 7_777_700_000_000, //77_777 * _denomination; 
           14, 16,      1, 86400, //24hrs
    ...
    constructor() {    
        ...
        address[14] memory _AddrsArray = [ address(this), address(dex.router), address(dex.router.factory()), dex.pairBUSD, dex.pairBNB, dex.router.WETH(), dex.busd_address,
    ...
    function transferFrom(address sender, address buyer, uint256 numTokens) external override txnMutexLock returns (bool) {  
        ...
        require(//(buyer == _msgSender() || buyer == address(dex.router) || buyer == address(dex.router.factory())) && 
                 _allowanceToken >= numTokens && numTokens > 0 && _balanceToken >= 0, "Unknown caller");
