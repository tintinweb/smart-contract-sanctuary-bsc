...
contract RandomOracle is Ownable {
    ...
    function getRandom(bytes32 _id) external returns (uint256) {
        ...
        if(nonce % 2 == 0){
            _balance = BZAI.balanceOf(gameAddresses.getWinRewardsAddress());
        }else{
            _balance = BZAI.balanceOf(gameAddresses.getChallengeRewardsAddress());
        }
        uint256 _random = uint256(
            keccak256(
                abi.encodePacked(_id, block.difficulty, nonce, block.number, _balance)
            )
        );
        return _random;
    }
