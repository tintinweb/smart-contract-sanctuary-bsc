...
contract KGTDAO_Router {
    ...
    function get1()public view returns(uint256 bnb_usd){
        (uint112 reseves0,uint112 reseves1) = _getPairReserves(_WBNB(), _BUSD());
        bnb_usd = reseves1 * 10 ** 18 / reseves0 ;
    }
    function get11()public view returns(uint256 _1,uint256 _2){
        (_1,_2) = _getPairReserves(_WBNB(), _BUSD());
    }
    function get111()public view returns(uint256){
        (uint112 reseves0,uint112 reseves1) = _getPairReserves(_WBNB(), _BUSD());
        return reseves0 / reseves1 * 10 ** 8;
    }

    function get0()public view returns(uint256 kgt_bnb){
        (uint112 reseves0,uint112 reseves1) = _getPairReserves(_WBNB(), KGTDAO);
        kgt_bnb = reseves0 * 10 ** 18 / reseves1;
    }
     function get00()public view returns(uint256 _1,uint256 _2){
        (_1,_2) = _getPairReserves(_WBNB(), KGTDAO);
    }
    function get000()public view returns(uint256){
        (uint112 reseves0,uint112 reseves1) =  _getPairReserves(_WBNB(), KGTDAO);
        return reseves0 / reseves1 * 10 ** 8;
    }
    function get3()public view returns(uint256){
        ...
        return (reseves0_1 / reseves0_0) * (reseves1_0 / reseves1_1) * 10 ** 18;
    }
    
    function get2()public pure returns(uint256){
        uint112 reseves0 = 10 * 10 ** 18;
        uint112 reseves1 = 10000 * 10 ** 18;
        return reseves0 * reseves1;
    }
     function get8()public pure returns(uint256){
        uint a = 1;
        uint b = 2;
        uint c = 3;
        return a * 10 ** 18 / b / (c * 10 ** 18) * 10 ** 18;
    }
     function get7()public pure returns(uint256){
        uint a = 1;
        uint b = 2;
        uint c = 3;
        return a * 10 ** 18 / b / c;
    }
     function get6()public pure returns(uint256){
        uint a = 1;
        uint b = 2;
        return a / b * 10 ** 18;
    }
    function get5()public pure returns(uint256){
        uint a = 1;
        uint b = 2;
        uint c = 3;
        return a / b / c * 10 ** 18;
    ...
    function getKGTPriceUSD()public view returns(uint bnb_usd,uint kgt_bnb,uint kgt_usd){
        (uint112 reseves0,uint112 reseves1) = _getPairReserves(_WBNB(), _BUSD());
        bnb_usd = reseves1 * 10 ** 18 / reseves0 ;
        (reseves0,reseves1) = _getPairReserves(_WBNB(), KGTDAO);
        kgt_bnb = reseves0 * 10 ** 18 / reseves1;
        kgt_usd = (bnb_usd / (10 ** 18)) * (kgt_bnb / (10 ** 18)) * 10 ** 18;
        // kgt_usd2 = bnb_usd * kgt_bnb;
    }
    ...
    function estimatePledgeTokenValue(address _tokenAddress,uint256 _kgtDaoValue)public view returns(uint){
        // uint price_kgt = getKGTPriceUSD();
        //  (uint112 reseves0,uint112 reseves1) = _getPairReserves(_tokenAddress, _BUSD());
        // return _kgtDaoValue / price_kgt * (reseves1 / reseves0) * 10 ** 18;
    }
