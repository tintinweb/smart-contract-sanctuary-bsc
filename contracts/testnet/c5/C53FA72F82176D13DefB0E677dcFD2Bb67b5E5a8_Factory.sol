...
contract Factory is Ownable, IERC721Receiver, Pausable {
    ...
    // orc earn 800 $GENE per day
    uint256 public constant DAILY_GENE_RATE = 1 ether;
    // orc must have 3 days worth of $GENE to unstake or else it's too cold
    ...
    uint256 public constant MAXIMUM_GLOBAL_GENE = 44000 ether;
    // claim price
    uint256 public claimPrice = 0.003 ether;
     ...
     */
    function addManyToFactoryAndPack(
        address account,
        uint16[] calldata tokenIds
    ) external {
        require(
            account == _msgSender() || _msgSender() == address(warrior),
            "DONT GIVE YOUR TOKENS AWAY"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (_msgSender() != address(warrior)) {
                // dont do this step if its a mint + stake
                require(
                    warrior.ownerOf(tokenIds[i]) == _msgSender(),
                    "AINT YO TOKEN"
                );
            ...
            if (isOrc(tokenIds[i])) _addOrcToFactory(account, tokenIds[i]);
            else _addWarriorToPack(account, tokenIds[i]);
        }
    }
     ...
     */
    function _addOrcToFactory(address account, uint256 tokenId)
        internal
        whenNotPaused
        _updateEarnings
    {
     ...
     * to unstake a Orc it will require it has 3 days worth of $GENE unclaimed
     * @param tokenIds the IDs of the tokens to claim earnings from
     * @param unstake whether or not to unstake ALL of the tokens listed in tokenIds
     */
    function claimManyFromFactoryAndPack(
        uint16[] calldata tokenIds,
        bool unstake
    ) external payable whenNotPaused _updateEarnings {
        require(claimPrice == msg.value, "Invalid claim price");

        uint256 owed = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (isOrc(tokenIds[i]))
                owed += _claimOrcFromFactory(tokenIds[i], unstake);
            else owed += _claimWarriorFromPack(tokenIds[i], unstake);
        }
        if (owed == 0) return;
     ...
     */
    function _claimOrcFromFactory(uint256 tokenId, bool unstake)
        internal
        returns (uint256 owed)
    {
        ...
        require(
            ...
            "GONNA BE COLD WITHOUT TWO DAY'S GENE"
        );
        if (totalGeneEarned < MAXIMUM_GLOBAL_GENE) {
            owed = ((block.timestamp - stake.value) * DAILY_GENE_RATE) / 1 minutes;
        } else if (stake.value > lastClaimTimestamp) {
            owed = 0; // $GENE production stopped already
        } else {
            owed =
                ((lastClaimTimestamp - stake.value) * DAILY_GENE_RATE) /
                1 minutes; // stop earning additional $GENE if it's all been earned
        }
        if (unstake) {
            if (random(tokenId) & 1 == 1) {
                // 50% chance of all $GENE stolen
                _payWarriorTax(owed);
                owed = 0;
            }
     ...
     * @return owed - the amount of $GENE earned
     */
    function _claimWarriorFromPack(uint256 tokenId, bool unstake)
        internal
        returns (uint256 owed)
    {
        require(
            warrior.ownerOf(tokenId) == address(this),
    ...
    // set claim price
    function setClaimPrice(uint256 pPrice) external onlyOwner {
        claimPrice = pPrice;
    }
    ...
    /**
     * allows owner to withdraw funds from claim
     */
    function withdraw() external onlyOwner {
        payable(owner()).transfer(address(this).balance);
    }
