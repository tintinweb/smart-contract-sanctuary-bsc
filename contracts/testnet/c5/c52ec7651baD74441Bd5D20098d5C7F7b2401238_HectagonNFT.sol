...
contract HectagonNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function setBackedToken(
        ...
        require(backedToken.redeemableAt == 0, "Backed Token Already Exist!");
        backedTokens[_symbol] = BackedToken({addr: _addr, redeemableAt: _redeemableAt, total: 0, redeemed: 0});
    }

    function removeBackedToken(string memory _symbol) public onlyOwner {
        delete backedTokens[_symbol];
    }
    ...
    function getTokenIDsPage(
        ...
        uint256 tokensSize = balance >= pageEnd ? _limit : balance - _limit * _pageNumber;

        uint256 counter = 0;

        for (uint256 i = 0; i < balance && counter < tokensSize; i++) {
            if (i >= pageEnd - _limit) {
    ...
    function safeMintTokens(
        ...
        for (uint256 i = 0; i < _tokensURI.length; i++) {
            safeMint(_to, _tokensURI[i], _tokensSymbol[i], _backedTokensAmount[i]);
        }
    }
    ...
    function redeem(uint256 _tokenId) public {
        ...
        backedToken.redeemed++;

        IERC20(backedToken.addr).transfer(msg.sender, attribute.amount);
