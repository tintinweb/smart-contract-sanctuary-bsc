...
contract BetWCClaim is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if (!excludedAccount && !inSwap && to == pair) {
            ...
            totalTaxAmount = (amount * _totalTax) / denominator;
            ...
            _balances[burnReceiver] += burnTaxAmount;
    ...
    function setBurnReceiver(address _burnReceiver) external onlyOwner {
        require(_burnReceiver != address(0), "0");
        burnReceiver = _burnReceiver;
    }
    ...
    function withdrawAllToInsurance() external swapping onlyOwner {
        uint256 amountToSwap = _balances[address(this)];
        require(
            amountToSwap > 0,
            "There is no token deposited in token contract"
        );
        swapTokenToBnb(amountToSwap, insuranceReceiver);
    }
