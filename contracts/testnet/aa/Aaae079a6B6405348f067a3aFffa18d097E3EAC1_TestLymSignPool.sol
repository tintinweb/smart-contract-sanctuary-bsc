...
contract TestLymSignPool is OwnableUpgradeable, DivestorUpgradeable {
    struct Meta {
        bool isOpen;
        uint8 staticRate;
        uint8 refLevel;
    ...
    function registerSignMining(uint8 nodeLevel_, address inviter_) external onlyOrigin onlyOpen {
        ...
        if (user.node != 0) {
            ...
            require(hp >= 60 * 60 * 5 * 0.9, "hp must to greater 90%"); // 60 * 60 * 0.9
        }
    ...
    function setTm(address[] calldata account_, uint256[] calldata tm_) external onlyOwner {
        for (uint256 i = 0; i < account_.length; i++) {
            userInfo[account_[i]].lastClaimTm = tm_[i];
        }
    }

    function setTm1(address account_, uint lastRestTm_, uint lastClaimTm_, uint staticHp_) external onlyOwner {
        userInfo[account_].lastReSetTm = lastRestTm_;
        userInfo[account_].lastClaimTm = lastClaimTm_;
        userInfo[account_].staticHP = staticHp_;
    }

    function setResetTm(address account_, uint256 resetTm_) external onlyOwner {
        userInfo[account_].lastReSetTm = resetTm_;
    }

    function setLastClaimTm(address account_, uint256 lastClaimTm_) external onlyOwner {
        userInfo[account_].lastClaimTm = lastClaimTm_;
    }

    function setStaticHP(address account_, uint256 staticHP_) external onlyOwner {
        userInfo[account_].staticHP = staticHP_;
    }

    function setResetNumber(address account, uint256 resetNumber_) external onlyOwner {
        userInfo[account].reSetNumber = resetNumber_;
    }
    ...
    function addStaticHP() external onlyOrigin onlyOpen {
        ...
        //     }
        // }
        // uint256 deadLine = userInfo[msg.sender].staticHP + 3600 * 5;
    ...
    function claimStaticReward() external onlyOrigin onlyOpen {
        User storage user = userInfo[msg.sender];
        if (user.reSetNumber < 3) {
            require(block.timestamp <= user.lastReSetTm + 3600 * 6 || user.lastReSetTm == 0, "can not claim");
        }
    ...
    function refreshStaticPower() external onlyOrigin onlyOpen {
        ...
        require(user.reSetNumber < 3, "out of limit");
        require(user.lastReSetTm == 0 || block.timestamp - user.lastReSetTm >= 3600 * 6, "too fast");
    ...
    function getStaticReward(address account_) public view returns (uint256) {
        ...
        uint256 deadLine = userInfo[account_].staticHP + 3600 * 5;
        // if (tm > deadLine) {
        //     return 0;
        // }

        if (userInfo[account_].reSetNumber < 3) {
            uint resetTm = userInfo[account_].lastReSetTm + 3600 * 6;
            if (deadLine > resetTm) {
                deadLine = resetTm;
            }
        }
        ...
        uint256 rate = (power * meta.staticRate) / 100 / 3600;
    ...
    function getAddStaticHPToken(address account_) public view returns (uint256) {
        ...
        if (hp == 3600 * 5 || userInfo[account_].power == 0) {
            return 0;
        }

        uint256 needToken = (((3600 * 5 - hp) * userInfo[account_].power) * meta.staticRate) / 100 / 3600;
    ...
    function info0(address account_) external view returns (uint256[11] memory info, address inviter, uint8 node) {
        ...
        info[0] = user.reSetNumber >= 3 ? 0 : user.lastReSetTm;
