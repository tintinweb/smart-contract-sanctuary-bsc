    ...
    PaymentUpgradeable
{
    ...
    function mint(
        ...
        address treasury = payment.getTreasury();
        if (!hasRole(MINTER_ROLE, sender)) {
            if (whitelisted[sender] != true) {
                uint256 amountExchanged = payment.exchange(baseToken, _token, _mintAmount * cost);
                if(address(_token) == address(0)){
                    _safeNativeTransfer(treasury, amountExchanged);
                } else {
                    _safeERC20TransferFrom(_token, sender, treasury, amountExchanged);
                }
            }
        }
