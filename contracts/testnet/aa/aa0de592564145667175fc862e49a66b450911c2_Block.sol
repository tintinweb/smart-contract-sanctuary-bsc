...
contract Block is ERC721, Ownable{
    ...
    constructor(string memory name_, string memory symbol_,address _tokenaddress) ERC721(name_, symbol_){
    ...
    function mint(string memory uri,uint256 _tokenamount) public returns(bool){
        require(validcid[uri],"invaild cid url");
        address _nftcreater = nftaddress[uri];
        require(_nftcreater != address(0x0) && nftvalue[uri] > 0 );
    ...
    function mint(string memory uri) public payable returns(bool){
        require(validcid[uri],"invaild cid url");
        address _nftcreater = nftaddress[uri];
        require(_nftcreater != address(0x0) && nftvalue[uri] > 0 );
    ...
    function devlopermint(string memory cid_,uint256 _royalty,uint256 _value)public returns(bool){
        require(isMinter[_msgSender()],"not minter !!!");
        require(_royalty <= 500,"Royalty should be less than 5%");

        nftvalue[cid_] = _value.sub(1);
        tokenId.increment();
        uint256 id = tokenId.current();

        _uri[id] = cid_;
        royalty[id] = _royalty;
        creator[id] = nftaddress[cid_] ;
