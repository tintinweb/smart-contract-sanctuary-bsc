...
contract BUSDVault is Initializable,ReentrancyGuard, Pausable, IEvents, AccessControl {
    ...
    **/
    ...
    function getReceiptToken() public view returns(address) {
        return BUSDs;
    }
    ...
    function _claimRebaseReward(address user) internal whenNotPaused() {
        uint userAmount = ((UserDeposit[user])/ IERC20(address(Admin.USDy())).totalSupply());
        uint reward =((userAmount * (currentReward - ss[user]))/ 10 ** 18);
