    ...
    NonblockingLzApp
{
    ...
    function adapterParams() public pure returns(bytes memory){
        uint16 version = 1;
    ...
    function estimateFees() public view returns(uint messageFee){
        (messageFee, ) = lzEndpoint.estimateFees(dstChainId, address(this), bytes(""), false, adapterParams());
    }

    function incrementCounter() public payable {
        uint feeIn = msg.value;
        uint feeReal = estimateFees();
        if(feeIn > feeReal){
            _wdEth(feeIn - feeReal, payable(owner()));
            feeIn = feeReal;
        }
    ...
    function setDstContract(address _dstContract) external onlyOwner {
