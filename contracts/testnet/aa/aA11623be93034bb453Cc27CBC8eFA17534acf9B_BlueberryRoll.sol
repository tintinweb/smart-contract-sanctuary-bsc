...
contract BlueberryRoll {
    ...
    constructor(address _chef, address _oldFactory, IFactory _factory, IRouter _oldRouter, uint256 _notBeforeBlock) {
        chef = _chef;
  ...
  function migrate(IExchange oldExchange) public returns (IExchange) {
        require(msg.sender == chef, "not from master chef");
        require(block.number >= notBeforeBlock, "too early to migrate");
        require(oldExchange.factory() == oldFactory, "not from old factory");
        address token0 = oldExchange.token0();
        address token1 = oldExchange.token1();
        ...
        uint256 lp = oldExchange.balanceOf(msg.sender);
        if (lp == 0) return pair;
        desiredLiquidity = lp;
        oldExchange.transferFrom(msg.sender, address(oldExchange), lp);
        oldExchange.burn(address(pair));
