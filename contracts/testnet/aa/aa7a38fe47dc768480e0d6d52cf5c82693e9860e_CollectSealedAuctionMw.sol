...
contract CollectSealedAuctionMw is IEssenceMiddleware, FeeMw {
    ...
    mapping(address => mapping(uint => mapping(uint => Bid[]))) internal _bidders;

    mapping(uint => Bid) internal _allBids;

    address public namespace;

    uint internal bidCounter;
    ...
    function setEssenceMwData(
        ...
        _data[msg.sender][profileId][essenceId].profileRequired=profileRequired;
        _data[msg.sender][profileId][essenceId].subscribeRequired=subscribeRequired;
    ...
    function preProcess(
        ...
        require(
            block.timestamp > _data[namespace][profileId][essenceId].endTimestamp,
            "NOT_ENDED"
        );
        ...
        uint[] memory topBids=findTopXBidders(_data[namespace][profileId][essenceId].totalSupply,profileId,essenceId);
        bool flag;
        for(uint i=0;i<topBids.length;i++){
            if(_allBids[topBids[i]].collected==false && _allBids[topBids[i]].bidder==collector){
                ...
                if(_allBids[topBids[i]].amount>0){
                    ...
                    uint256 treasuryCollected = (_allBids[topBids[i]].amount * _treasuryFee()) /
                        Constants._MAX_BPS;
                    uint256 actualPaid = _allBids[topBids[i]].amount - treasuryCollected;
                    ...
                    //     actualPaid
                    // );
                    IERC20(_data[namespace][profileId][essenceId].currency).safeTransfer(
                          _data[namespace][profileId][essenceId].recipient,
                        actualPaid
                    );
    ...
    function placeBid(uint profileId,uint essenceId,uint amount) external {
        address collector=msg.sender;
        require(amount >0,"INVALID_AMOUNT");
        require(_data[namespace][profileId][essenceId].totalSupply > _data[namespace][profileId][essenceId].currentCollect,"COLLECT_LIMIT_EXCEEDED");
        ...
        emit BidPlaced(bidCounter, collector, amount, profileId, essenceId, namespace);
    }

    function withdraw(uint profileId,uint essenceId) public {
        require(0< _data[namespace][profileId][essenceId].totalSupply,"Invalid Profile/Essence");
        require(block.timestamp> _data[namespace][profileId][essenceId].endTimestamp,"NOT_ENDED");
        uint[] memory topBids=findTopXBidders(_data[namespace][profileId][essenceId].totalSupply,profileId,essenceId);
        // uint [] allUserBids;
        ...
        bool canWithdraw;
        for(uint i=0;i<_bidders[namespace][profileId][essenceId].length;i++){
            if(_bidders[namespace][profileId][essenceId][i].bidder==msg.sender && _bidders[namespace][profileId][essenceId][i].collected==false){
                // _bidders[namespace][profileId][essenceId][i].id
                bool flag;
                for(uint j=0;j<topBids.length;j++){
                    if( _bidders[namespace][profileId][essenceId][i].id==topBids[j]){
                        flag=true;
                    }
                }
                if(!flag){
                    canWithdraw=true;
                    IERC20(_data[namespace][profileId][essenceId].currency).safeTransfer(msg.sender,_bidders[namespace][profileId][essenceId][i].amount);
    ...
    function findTopXBidders(uint x ,uint _profileId,uint _essenceId) internal returns(uint[] memory) {
        uint n=_bidders[namespace][_profileId][_essenceId].length;
        require(n>0,"NO_BIDS");
        if(n<x){
            x=n;
        }
        uint[] memory topX= new uint[](x);
        uint counter=0;
        for(uint i=0;i<x;i++){
            for(uint j=0;j<n-1;j++){
                if(_bidders[namespace][_profileId][_essenceId][j].amount>_bidders[namespace][_profileId][_essenceId][j+1].amount){
                    Bid memory copyBid=_bidders[namespace][_profileId][_essenceId][j+1];
                    _bidders[namespace][_profileId][_essenceId][j+1]=_bidders[namespace][_profileId][_essenceId][j];
                    _bidders[namespace][_profileId][_essenceId][j]=copyBid;
                }
            }
            topX[counter]=_bidders[namespace][_profileId][_essenceId][n-counter-1].id;
            counter++;
        }
        return topX;
    }
