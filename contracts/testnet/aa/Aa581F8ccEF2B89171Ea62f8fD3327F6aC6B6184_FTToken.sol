...
contract FTToken is ERC20, Ownable {
    ...
    TokenDividendTracker immutable public dividendTracker;

    bool private _swapping;
    uint256 private _lastSwapTime;
    uint256 immutable public lpRewardFee = 1;
    uint256 public AmountLpRewardFee;
    uint256 immutable public marketingFee = 1;
    uint256 immutable public sellBurnFee = 2;
    address immutable public burnAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 immutable public maxBurn;
    ...
    function _isGtMaxHold(address _from,address _to) private view returns(bool){
        return owner() != address(0) && _to != uniswapV2Pair && balanceOf(_to) > maxHold && (!_isExcludedFromFees[_to] || !_isExcludedFromFees[_from]);
    }

    function _shouldBan(address _from,address _to) private view returns(bool){
        uint256 current = block.timestamp;
        return swapStartTime > 0 &&_from == uniswapV2Pair && !_isExcludedFromFees[_to] && current >= swapStartTime && current <  swapStartTime.add(10 minutes);
    }
        ...
        returns (uint256 amountAfter)
    {
        amountAfter = amount;
        if(from != uniswapV2Pair){
            uint256 BFee = amount.mul(sellBurnFee).div(100);
            uint256 isBurn = balanceOf(burnAddress);
            if(isBurn >= maxBurn)BFee =0;
            if(BFee > 0 && isBurn.add(BFee) > maxBurn)BFee = maxBurn.sub(isBurn);
            if(BFee > 0) super._transfer(from, burnAddress, BFee);
            amountAfter = amountAfter.sub(BFee);
        }
        uint256 MFee = amount.mul(marketingFee).div(100);
        if(MFee > 0) super._transfer(from, marketingAddress, MFee);
        ...
        if (LPFee > 0) super._transfer(from, address(this), LPFee);
