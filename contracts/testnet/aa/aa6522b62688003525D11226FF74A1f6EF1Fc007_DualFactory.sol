...
contract DualFactory is AccessControl, IDualFactory {
  ...
  function _close(
    ...
    dual.closedPrice = priceFeed.historicalCrossPrice(tariff.baseToken, baseRoundId, tariff.quoteToken, quoteRoundId, finishAt);
    emit DualClaimed(id);
  }
  ...
  function deleteTariff(uint256 id) public onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function updateStable(address stablecoin, bool isStable) public onlyRole(DEFAULT_ADMIN_ROLE) {
    stablecoins[stablecoin] = isStable;
  }
}
...
interface IDualFactory {
  ...
  function deleteTariff(uint256 id) external;
  ...
  function updateStable(address stablecoin, bool isStable) external;
}
