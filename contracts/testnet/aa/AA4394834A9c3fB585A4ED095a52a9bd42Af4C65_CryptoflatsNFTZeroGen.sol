...
contract CryptoflatsNFTZeroGen is DefaultOperatorFilterer, ERC721rPass, Ownable, ERC2981 {

    using Strings for uint256;

    uint256 public constant PRICE = 0.06 ether;
    uint256 public constant SALE_PRICE = 0.036 ether;

    address payable private _teamWallet;

    // Merkle roots
    bytes32 private _freeListRoot;
    bytes32 private _saleListRoot;

    // mappings for tracking mint
    mapping(address => bool isMint) private _freeList;
    mapping(address => uint256 countMinted) private _saleList;

    constructor(
        address payable teamWallet
    ) ERC721rPass("Cryptoflats-Gen0", "CNRS-0", 1_111) {
        _setDefaultRoyalty(msg.sender, 700);
        _teamWallet = teamWallet;
    }

    //setters
    function setTeamWallet(address payable _newTeamWallet) external onlyOwner {
        _teamWallet = _newTeamWallet;
    }

    function setFreeListRoot(bytes32 _root) external onlyOwner {
        _freeListRoot = _root;
    }

    function setSaleListRoot(bytes32 _root) external onlyOwner {
        _saleListRoot = _root;
    }
    ...
    function baseURI() public pure returns (string memory) {
        return "ipfs://QmT39SCsDvSKekjLvoHq8UruesiciUrxsBEshh4SXTKdXB/";
    }
    ...
    // mint
    function mint(
        bytes32[] calldata _proofFreeList,
        bytes32[] calldata _proofSaleList
    ) external payable {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (_isMemberFreeList(_proofFreeList, leaf)) {
            _freeList[msg.sender] = true;
        } else if (_isMemberSaleList(_proofSaleList, leaf)) {
            require(msg.value >= SALE_PRICE, "CNRS: Insufficient funds");
            _saleList[msg.sender]++;
        } else {
            require(msg.value >= PRICE, "CNRS: Insufficient funds");
        }
        _mintRandom(msg.sender, 1);
    }

    function addDiamond(uint256[] calldata _diamondIds) external onlyOwner {
    ...
    // private methods
    function _isMemberFreeList(
        bytes32[] calldata _proof,
        bytes32 _leaf
    ) internal view returns (bool) {
        return MerkleProof.verify(_proof, _freeListRoot, _leaf) && !_freeList[msg.sender];
    }

    function _isMemberSaleList(
        bytes32[] calldata _proof,
        bytes32 _leaf
    ) internal view returns (bool) {
        return MerkleProof.verify(_proof, _saleListRoot, _leaf) && _saleList[msg.sender] < 3;
    }

    // Defefault operator filters Opensea
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC2981, ERC721rPass) returns (bool) {
