...
contract RematicVoteV1 is ReentrancyGuard, Ownable {
    ...
    function registerManager() external payable {
        require(
            isManager[_msgSender()] == false,
            "Already registered as a manager"
        );
        require(msg.value >= REGISTER_PRICE, "Payment amount is not enough.");
    ...
    function createPoll(
        ...
        require(
            isManager[_msgSender()] == true,
            "You are not an election manager."
        );
        require(
            _tokenAddr != address(0x0),
            "Token contract address can't be ZERO."
        );
        require(msg.value >= POLL_PRICE, "Payment amount is not enough.");
        require(_openTime < _closeTime, "Vote period is not valid.");
    ...
    function updateWhitelist(
        ...
        uint256 curNumberOfVoters = polls[_pollId].sizeOfWhitelist;
        uint256 newNumberOfVoters = _hasWhitelist
        ...
        require(
            IERC20(polls[_pollId].tokenAddress).allowance(
                ...
                TOKEN_AMOUNT_PER_VOTER *
                    (
                        newNumberOfVoters > curNumberOfVoters
                            ? newNumberOfVoters - curNumberOfVoters
                            : 0
                    ),
        ...
        if (newNumberOfVoters > curNumberOfVoters) {
            uint256 amount = TOKEN_AMOUNT_PER_VOTER *
                (newNumberOfVoters - curNumberOfVoters);
            IERC20(polls[_pollId].tokenAddress).transferFrom(
                _msgSender(),
                address(this),
                TOKEN_AMOUNT_PER_VOTER * (newNumberOfVoters - curNumberOfVoters)
            );
            polls[_pollId].tokenBalance += amount;
        } else if (newNumberOfVoters < curNumberOfVoters) {
            uint256 amount = TOKEN_AMOUNT_PER_VOTER *
                (curNumberOfVoters - newNumberOfVoters);
            IERC20(polls[_pollId].tokenAddress).transfer(_msgSender(), amount);
            polls[_pollId].tokenBalance -= amount;
        }

        polls[_pollId].sizeOfWhitelist = newNumberOfVoters;
    }
    ...
    function createQuestion(
        ...
        polls[_pollId].questions.push(numOfQuestions);

        return polls[_pollId].numOfQuestions++;
    }
    ...
    function updateQuestion(
        ...
        questions[_questionId].text = _text;
        questions[_questionId].kind = _kind;

        if (_choices.length > questions[_questionId].numOfChoices) {
            uint256 i = 0;
            while (i < questions[_questionId].numOfChoices) {
                questions[_questionId].choices[i] = _choices[i++];
            }
            while (i < _choices.length) {
                questions[_questionId].choices.push(_choices[i++]);
            }
        } else {
        ...
        questions[_questionId].numOfChoices = _choices.length;
    }
    ...
    function claim(uint256 _pollId, uint256 _amount) public {
        require(
            _msgSender() == polls[_pollId].manager,
            "You are not the poll manager."
        );
        require(
            polls[_pollId].etherBalance >= _amount,
            "Claim amount is bigger than balance."
        );
    ...
    function withdraw(address payable _to, uint256 _amount) public onlyOwner {
        require(_safeTransferETH(_to, _amount), "Withdraw failed.");
    }

    function claimToken(uint256 _pollId, uint256 _amount) public {
        require(
            _msgSender() == polls[_pollId].manager,
            "You are not the poll manager."
        );
        require(
            polls[_pollId].tokenBalance >= _amount,
            "Claim amount is bigger than balance."
        );
        IERC20(polls[_pollId].tokenAddress).transfer(_msgSender(), _amount);
        polls[_pollId].tokenBalance -= _amount;
    }
