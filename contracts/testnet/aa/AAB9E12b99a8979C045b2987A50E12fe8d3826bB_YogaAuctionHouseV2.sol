...
contract YogaAuctionHouseV2 is Pausable, ERC721, ReentrancyGuard, ERC2981, Ownable {
    using Strings for *;
    ...
    constructor(
        ...
        _setDefaultRoyalty(contractConfig.mikeAddress, contractConfig.sellerFeeBasisPoints);
    }
    ...
    function bidOnMultipleApes(uint[] calldata apeIds) external payable whenNotPaused nonReentrant {
        ...
        for (uint i; i < apeIds.length; i++) {
            (bool success, uint amountSpent) = bidOnApe(apeIds[i], availableEth);
    ...
    function bidOnApe(uint apeId, uint availableEth) internal returns (bool success, uint amountSpent) {
    ...
    function withdraw() external {
        ...
        emit Withdraw(ethAvailableToWithdraw);
        
        ethAvailableToWithdraw = 0;
        
        _safeTransferETHWithFallback(contractConfig.withdrawAddress, ethAvailableToWithdraw);
    }
