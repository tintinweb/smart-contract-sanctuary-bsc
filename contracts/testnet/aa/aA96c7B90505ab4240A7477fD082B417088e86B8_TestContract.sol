...
contract TestContract is OwnableUpgradeable
{
    ...
    struct UserData // Needs memory packing
    {
        uint solarPanels;
        uint storedPower;
        uint allowance;
    ...
    function computeExchangeRate(uint amount) public view returns (uint)
    {
    ...
    function buyPanels(address referrer) external payable
    {
        ...
        else
        {
            ...
            uint newFreshValue = user.amountDeposited - user.amountSold;
    ...
    function sellPower(uint amount) external
    {
        ...
        if (user.allowance - amountRequested == 0)
            if (user.allowance > 0)
