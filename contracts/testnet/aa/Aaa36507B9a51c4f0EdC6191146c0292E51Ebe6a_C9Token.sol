...
contract TokenDividendTracker is Ownable {
    ...
    uint256 public minSharePercent = 10; //1%
    ...
    function shareHolderLength() external view returns (uint256 res) {
        res = shareholders.length;
    }
    ...
    function setMinSharePercent(uint256 percent) public onlyOwner {
    ...
    function process(uint256 gas) external onlyOwner {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if(amount.mul(1000).div(nowbanance) < 10) {
                currentIndex++;
                iterations++;
                return;
            }
...
contract C9Token is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public deadFee = 10; //1%
    uint256 public buyFee = 0;
    uint256 public sellFee = 0;
    uint256 public lpRewardFee = 30; //3%
    address public lpRewardToken;
    ...
    address public lpManager = 0x099682740e2BAaB4ec7d5f0760AA6ba79AadD944;

    address private fromAddress;
    address private toAddress;

    mapping(address => bool) public isExcludedFromFees; //address dont give fee
    mapping(address => bool) public isDividendExempt; //address dont get reward fee
    mapping(address => bool) public ammPairs;
    mapping(address => bool) public startPair;
    mapping(address => uint256) public pairStartBlock;

    uint256 public minPeriod = 120; //send reward fee for every single day by default
    ...
    constructor() payable ERC20("CRISPR-Cas9", "C9") {
        uint256 totalSupply = 500000000 * (10**18);
        burnToAmount = 499999000 * (10**18); //burn total 1000 for test
        swapTokensAtAmount = 200 * (10**18); // swap C9 to reward token every 2000 amount
        lpRewardToken = 0x8dCF611214047CbfE117268fa7C72c0E8128C289;
        uniswapV2Router = IUniswapV2Router02(
            0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        );
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(
                address(this),
                0x8dCF611214047CbfE117268fa7C72c0E8128C289
            );
    ...
    function setFeeAddress(address _buyFeeAddr, address _sellFeeAddr)
        external
        onlyOwner
    {
    ...
    function setFee(
        ...
        deadFee = _deadFee; //1% by default
        buyFee = _buyFee;   //0 by default
        sellFee = _sellFee; //0 by default
        lpRewardFee = _lpRewardFee; //3% by default
    }
    ...
    function _transfer(
        ...
        //lpManager add liquidity at the first time, to == uniswapV2Pair and from == lpManager
        //now then we set startPair[uniswapV2Pair] = true, from then on, people can swap tokens
        ...
        bool takeFee = !swapping; //if _transfer is called from swapLPRewardToken itself, don't take fee, else take it fee
        if (//if from or to address is excluded from fees, don't take fee neither
            isExcludedFromFees[from] ||
            isExcludedFromFees[to] ||
            isEqual("transfer", tradeType) //don't take fee if transfer happen between normal wallet addresses
        ) {
            takeFee = false;
        }
        uint256 amountAfter = amount;
        if (takeFee) {
            uint256 totalBurnAmount = totalSupply().sub(burnToAmount);
            if (deadFee > 0 && burntAmount < totalBurnAmount) { //burn to dead address
                uint256 dfAmount = amount.mul(deadFee).div(1000);   
                uint256 canBurnAmount = totalBurnAmount.sub(burntAmount);          
                dfAmount = canBurnAmount >= dfAmount ? dfAmount : canBurnAmount;
                if (dfAmount > 0) {
                    super._transfer(from, deadAddr, dfAmount);
                }
                amountAfter = amountAfter.sub(dfAmount);
                burntAmount = burntAmount.add(dfAmount);
            }

            if (lpRewardFee > 0) { //take reward fee as this->token, and transfer to address of this
            ...
            if (
                buyFee > 0 &&
                (isEqual("buy", tradeType) || isEqual("contract", tradeType))
            ) {
