...
contract Chickly is Ownable {
	...
	function withdraw(uint256 bnb, uint256 busd) public {
        User storage user = users[msg.sender];
		...
		require(user.reserved[0] > 0 || user.reserved[1] > 0, "User has no profit");

		user.timestamps[1] = uint32(block.timestamp);

        if (user.reserved[0] < bnb) {
            bnb = user.reserved[0];
        }

        if (user.reserved[1] < busd) {
            busd = user.reserved[1];
        }

        if (user.reserved[0] >= bnb) {
            user.reserved[0] -= bnb;
            payable(msg.sender).transfer(bnb);
            marketingAddress.transfer(bnb * MARKETING_FEE / PERCENTS_DIVIDER);
        }

        if (user.reserved[1] >= busd) {
            user.reserved[1] -= busd;
            BUSD.transfer(msg.sender, busd);
            BUSD.transfer(marketingAddress, busd * MARKETING_FEE / PERCENTS_DIVIDER);
        }

        emit Withdrawn(msg.sender, bnb, busd);
	}
