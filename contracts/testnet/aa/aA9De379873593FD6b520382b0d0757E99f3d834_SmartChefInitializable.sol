...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when HEXA mining ends.
    ...
    // The block number when HEXA mining starts.
    ...
    function initialize(
        ...
        rewardToken = IHexaToken(_rewardToken);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(depositFeeBP > 0){
                uint256 feeAmount = (_amount * depositFeeBP) / 10000;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            user.amount = user.amount - _amount;
            if(withdrawFeeBP > 0){
                uint256 feeAmount = (_amount * withdrawFeeBP) / 10000;
                stakedToken.safeTransfer(feeAddress, feeAmount);
                stakedToken.safeTransfer(address(msg.sender), (_amount - feeAmount));
            } else {
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            if(withdrawFeeBP > 0){
                uint256 feeAmount = (amountToTransfer * withdrawFeeBP) / 10000;
                stakedToken.safeTransfer(feeAddress, feeAmount);
                stakedToken.safeTransfer(address(msg.sender), (amountToTransfer - feeAmount));
            } else {
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        rewardToken.mint(address(msg.sender), _amount);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 hexaReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (hexaReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 hexaReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (hexaReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
