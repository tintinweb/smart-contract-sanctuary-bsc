...
contract TokenWEFI is IERC20, Ownable {
    ...
    address public constant marketingWallet = 0xDA616Cf8f1114dcC4acfb76Efc9b23DCF2DeB54a;
    ...
    function _transfer(address from, address to, uint256 amount) private
    {
        ...
        require(from != address(0), "TokenWEFI: transfer from the zero address");
        ...
        if (overMinTokenBalance && !_inSwapAndLiquify && from != uniswapV2Pair)
        {
        ...
        bool overMinBalanceBNB = balance >= minimumBNBsBeforeSwap;
        if (overMinBalanceBNB && !_inSwapAndLiquify && from != uniswapV2Pair){
            _buyBackBurnTokens(balance);
        }

        bool takeFee = true;

        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] || (from != uniswapV2Pair && to != uniswapV2Pair) || (from == uniswapV2Pair)) {
            takeFee = false;
        }
    ...
    function _calculateBuyBackFee(uint256 _amount) private view returns (uint256)
    {
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "TokenWEFI: approve from the zero address");
        require(spender != address(0), "TokenWEFI: approve to the zero address");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
        require(allowance(sender, _msgSender()) >= amount, "TokenWEFI: insufficient allowance");
    ...
    function reflectToHolders(uint256 tAmount) external {
        ...
        require(balanceOf(sender) >= tAmount, "TokenWEFI: insufficient balance");
        require(!_isExcludedFromReward[sender], "TokenWEFI: excluded addresses cannot call this function");
