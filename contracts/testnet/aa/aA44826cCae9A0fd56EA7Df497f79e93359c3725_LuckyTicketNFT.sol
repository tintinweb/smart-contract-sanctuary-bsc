    ...
    VRFConsumerBase
{
    ...
    uint8 _ownerTicketMaxCount = 250;
    ...
    mapping(address => uint256[]) ownerTicket;
    ...
    function setTodayTokenId() external onlyOwner {
    ...
    function resetAllTicket() internal {
    ...
    function buyTicket(uint8 _quantity) external nonReentrant {
        uint256[] storage _ownerTicket = ownerTicket[msg.sender];
        ...
        for (uint8 i = 0; i < _quantity; i++) {
            _ownerTicket.push(block.timestamp);
            // store all user ticket
            allTicketSoldPerDay.push(block.timestamp);
    ...
    function randomWinner() external onlyOwner isPeriod {
        getRandomNumber();
