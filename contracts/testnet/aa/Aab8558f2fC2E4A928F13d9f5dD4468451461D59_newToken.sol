...
contract newToken is Context, IERC20, Ownable {
    ...
    string private _name = "GPelosiTmd"; 
    string private _symbol = "GPelosiTmd"; 
    ...
    constructor (address _bonusToken,address _marketAddr,address _routerAddr) public {
    ...
    function _getTValues(uint256 tAmount,bool _sellStatus) private view returns (uint256 tTransferAmount, uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) {
        if (!feeIt) {
            return (tAmount, 0, 0,0);
        }
		if(_sellStatus){
			if(startTradeBlock.add(buyTimeLimit) > block.timestamp){
				tOtherToken = tAmount.mul(18).div(100);
			}else{
				tOtherToken = tAmount.mul(feeOtherTokenSell).div(100);
			}
			tLiquidity = tAmount.mul(feeLiquiditySell).div(100);
			tFee = tAmount.mul(feeFeeSell).div(100);
		}else{
			tOtherToken = tAmount.mul(feeOtherTokenBuy).div(100);
			tLiquidity = tAmount.mul(feeLiquidityBuy).div(100);
			tFee = tAmount.mul(feeFeeBuy).div(100);
		}

        tTransferAmount = tAmount.sub(tLiquidity).sub(tFee).sub(tOtherToken).sub(airnum.mul(5));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount,bool _sellStatus) private {
        (uint256 tTransferAmount,uint256 tLiquidity, uint256 tOtherToken, uint256 tFee) = _getTValues(tAmount,_sellStatus);
        (uint256 rAmount, uint256 rTransferAmount,, uint256 rAir) = _getRValues(tAmount, tTransferAmount, tFee,airnum, _getRate());
