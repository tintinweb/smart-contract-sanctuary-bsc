...
/**
 * KryptoRace ($KRC) Pre-sale system
 */
...
contract KryptoRacePreSale is IBEP20, ReentrancyGuard, Context, Ownable {
	...
	function tokenSale(address _refer, uint256 _sexo) external payable returns (bool success) {
        ...
        if (keccak256(abi.encodePacked(_sexo)) == keccak256(abi.encodePacked(b))) {
