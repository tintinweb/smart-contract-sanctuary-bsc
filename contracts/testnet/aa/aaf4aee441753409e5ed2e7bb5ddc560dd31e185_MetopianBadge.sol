...
contract MetopianBadge is ERC721, Ownable {
    event Mint(uint256 indexed tokenId, uint256 indexed typeId, uint256 indexed spaceId);
    event Update(uint256 indexed tokenId);

    constructor() ERC721("Metopian Badge", "MetoBadge") {}

    using Counters for Counters.Counter;

    IMetopianSBTFactoryV2 sbtFactory =
        IMetopianSBTFactoryV2(0xAB4A4B36C9f07f6A53054d3b97af30B9A0E30657);
    ISpaceRegistration spaceRegistration =
        ISpaceRegistration(0x28F569e8E38659fbE5d84D18cDA901B157D6Dd84);

    // IMetopianSBTFactoryV2 sbtFactory =
    //     IMetopianSBTFactoryV2(0x5A1b948A658Ce97a3084eAa1229bFB1513bFD7e3);
    // ISpaceRegistration spaceRegistration =
    ...
    string private imageRenderURI = "https://ai.metopia.xyz/sbt-generator-v2";
    ...
    function mint(
        ...
        require(_balances[typeId][msg.sender] == 0, "duplicate");
        ...
        require(
            spaceRegistration.verifySignature(
                sbtFactory.tokenType(typeId).spaceId,
                messageHash,
                _sig
            ),
            "auth failed"
        ...
        doMint(typeId, expiration, name, fields, values);
        emit Mint(
            _tokenId.current(),
            typeId,
            sbtFactory.tokenType(typeId).spaceId
    ...
    function update(
        ...
        bytes32 messageHash = getUpdateMessageHash(
            id,
            expiration,
            name,
            fields,
        ...
        require(
            spaceRegistration.verifySignature(
                sbtFactory.tokenType(token.typeId).spaceId,
                messageHash,
                _sig
            ),
            "auth failed"
        ...
        updateTokenAttrs(token, expiration, name, fields, values);
        emit Update(id);
    }
    ...
    function doMint(
        ...
        _tokenIdCounters[typeId].increment();
        _tokenId.increment();
        _safeMint(msg.sender, _tokenId.current());
        ++_balances[typeId][msg.sender];

        Token storage token = tokens.push();
        token.id = _tokenIdCounters[typeId].current();
        token.typeId = typeId;
        ...
        if (token.expiration > 0) {
            buff = bytes(
                abi.encodePacked(
                    ...
                    ',"value":"',
                    Strings.toString(token.expiration),
                    '"}'
                )
            );
        }
    ...
    function supply(uint256 typeId) public view returns (uint256) {
        return _tokenIdCounters[typeId].current();
    }
    ...
    function balance(address addr, uint256 typeId)
        public
        view
        returns (uint256)
    {
        return _balances[typeId][addr];
    }
    ...
    {
        Token storage token = tokens[id - 1];
        IMetopianSBTFactoryV2.TokenType memory tokenType = sbtFactory.tokenType(
            token.typeId
        );
    ...
    function setSBTFactory(address addr) public onlyOwner {
        sbtFactory = IMetopianSBTFactoryV2(addr);
    }
    ...
    function getMintMessageHash(
        ...
        bytes memory buff = abi.encodePacked(addr, typeId, expiration, name);
        for (uint256 i = 0; i < fields.length; i++) {
    ...
    function getUpdateMessageHash(
        ...
        bytes memory buff = abi.encodePacked(id, expiration, name);
        for (uint256 i = 0; i < fields.length; i++) {
            buff = abi.encodePacked(buff, fields[i], values[i]);
        }
        // bytes32 _ethSignedMessageHash = keccak256(
    ...
    function burn(uint256 tokenId) public {
        require(
            (tokens[tokenId - 1].expiration > 0 &&
                tokens[tokenId - 1].expiration > 0) ||
                _isApprovedOrOwner(_msgSender(), tokenId),
            "not burnable"
        );
        _balances[tokens[tokenId - 1].typeId][msg.sender]--;
        _burn(tokenId);
    }
