...
contract StakingPool is Ownable, ReentrancyGuard {
    ...
    function pendingRewards(address _user) external view returns (uint256) {
        ...
        uint256 totalStaked = pool.currentDepositAmount;
        if (block.number > pool.lastRewardBlock && totalStaked != 0) {
            ...
            if (remainingRewards > 0) {
                ...
                accRewardPerShare = accRewardPerShare.add(totalRewards.mul(1e12).div(totalStaked));
            }
        }
    ...
    function updatePool() public {
        ...
        pool.accRewardPerShare = pool.accRewardPerShare.add(totalRewards.mul(1e12).div(totalStaked));
        pool.lastRewardBlock = block.number;
    }
    ...
    function income(uint256 _amount) external nonReentrant {
        updatePool();
        uint256 balanceBefore = rewardStorage.rewardBalance();
        rewardToken.safeTransferFrom(address(msg.sender), address(rewardStorage), _amount);
        uint256 balanceAfter = rewardStorage.rewardBalance();
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        rewardStorage.safeRewardTransfer(_to, _amount);
    }
    ...
    //A specific user address is provided to facilitate aggregating harvests on multiple contracts
    function harvestFor(address _user) public nonReentrant{
        //Limit to self or delegated harvest
...
contract RewardStorage is Ownable {
    ...
    address constant public wrappedNativeAddress = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c);
    ...
    function safeRewardTransfer(address _to, uint256 _amount) external onlyOwner {
        uint256 transferAmount = Math.min(rewardBalance(), _amount);

        if(address(reward) == wrappedNativeAddress){
            ...
            reward.safeTransfer(_to, transferAmount);
        }
    }
