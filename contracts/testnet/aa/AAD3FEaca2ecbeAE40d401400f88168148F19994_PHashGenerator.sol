...
//function to calculate any key(word) as Pedersen Hash on the Baby-Jubjub Elliptic Curve
 function zeroValueGenerator(uint _key) external view returns(uint256) {
  return uint256(keccak256(abi.encodePacked(_key))) % _babyjubjubcurvep ; 
