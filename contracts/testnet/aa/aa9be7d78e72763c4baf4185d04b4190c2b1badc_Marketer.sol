...
contract Marketer is Swap{
    ...
    function buyToken(address[] calldata path, uint256 slippage)external payable vipVerify{
        ...
        if(tokenU != WETH){
			...
			require( amountIn >= predicted.mul(100-slippage),"did not get the corresponding amount[1]");	
        ...
        require( tokenAmountOut >= predicted.mul(100-slippage),"did not get the corresponding amount[2]");	
    ...
    function buyTokenForToken(address[] calldata path, uint256 amountIn, uint256 slippage)external vipVerify{
		...
		require( tokenAmountOut >= predicted.mul(100-slippage),"did not get the corresponding amount");				
