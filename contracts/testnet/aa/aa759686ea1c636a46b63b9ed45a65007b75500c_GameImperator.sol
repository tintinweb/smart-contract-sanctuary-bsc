...
contract GameImperator is BEP20Token {
    ...
    ///плата рефералу///   
         if (my_inviter[msg.sender] != address(0)){
           ...
           emit Referalsum(my_inviter[msg.sender],all_game[a].referal_gift,a,block.timestamp);
       }
    ///////////////////  
        ...
        if (all_game[a].adr.length > 1)
        {
            tokenUSDT.transfer(all_game[a].adr[all_game[a].count], (all_game[a].price - (all_game[a].price - all_game[a].referal_gift)) * 10 ** 18  );
            emit EmperorLog(all_game[a].adr[all_game[a].count], all_game[a].price - (all_game[a].price - all_game[a].referal_gift),a,block.timestamp);

            all_game[a].progress[all_game[a].adr[all_game[a].count]] += 1;
            emit MyLog(all_game[a].adr[all_game[a].count],all_game[a].price - (all_game[a].price - all_game[a].referal_gift),a,block.timestamp);


        if ( all_game[a].progress[all_game[a].adr[all_game[a].count]] == 8){
    ...
    function Second_Game() public{
       ...
       require(all_game[a].in_game[msg.sender] == false, "You are in the game");
       require(second_game[msg.sender] == true, "Big circle not active");
    ...
    function Check_Number(address adr,uint256 number) public In_game(adr,number) view returns(uint256) {
        return all_game[number].number[adr] - all_game[number].count;
    }

     function Check_status(address adr,uint256 number) In_game(adr,number) public view returns(string memory) {
         if (all_game[number].number[adr] - all_game[number].count <=  8 * all_game[number].count && all_game[number].count > 0 )
        return "Emperor";
        else
        return "Legion";
    }
    ...
    function Check_Kol_Viplat(address adr,uint256 number) In_game(adr,number) public view returns(uint256){
        if (number >=1 && number <=4)
        return all_game[number].progress[adr];
        else
        revert("Error number");
    }
 ...
 /*   function TEST() public view returns(uint256){
        ...
        return  game1.progress[game1.adr[game1.count]];
    }
    */
