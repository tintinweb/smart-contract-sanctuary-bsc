...
contract DeFiTrinity is IERC20, Context, Ownable {
    ...
    // fees
    uint256 public pendingTotalFees = 0;
    ...
    function getBUSDValue(uint256 amount) public view returns (uint256) {
        ...
        path[0] = router.WETH();
        path[1] = _busd;
        uint256 value = router.getAmountsOut(amount, path)[1];
        return value;
    }
    ...
    function _transferFrom(
        ...
        if (
            ...
            inSwap = true;
            uint256 lpTokens = contractTokenBalance.sub(_jackpotTokens);
            swapAndLiquify(lpTokens);
            uint256 jackpotTokens = contractTokenBalance.sub(lpTokens);
            uint256 busdBalanceBefore = IERC20(_busd).balanceOf(address(this));
            swapTokensForTokens(jackpotTokens, _busd);
            uint256 busdBalanceAfter = IERC20(_busd).balanceOf(address(this));
            uint256 busdReceived = busdBalanceAfter.sub(busdBalanceBefore);
            _jackpotTokens = 0;
            _pendingJackpotBalance = _pendingJackpotBalance.add(busdReceived);
            inSwap = false;
        }
        ...
        returns (bool)
    {
        ...
        _lastSellBUSDValue = getBUSDValue(bnbAmount);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        if (successful) {
            ...
            _transfer(msg.sender, address(0), tokenAmount.sub(jackpotTokens));
