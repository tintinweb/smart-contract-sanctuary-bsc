...
contract LezmeWalletContract is Ownable {
	...
	*/
	function createWallet() public {
		require(keccak256(abi.encodePacked(msg.sender)) != keccak256(abi.encodePacked("")), "Lezme: username not empty");
		require(userWallet[msg.sender] == address(0), "Lezme: the user already exists a wallet on the system");
		
		userWallet[msg.sender] = address(new LezmeUserWallet(pool, owner(), msg.sender, address(this), lezmeToken));
	...
	*/
	function withdrawToken(address _to, uint256 _amount, string memory _message, uint256 _expiredTime, bytes memory _signature) public {
		...
		emit Withdraw(msg.sender, _amount, pool, lezmeToken, block.timestamp, _message);
	}
...
contract LezmeUserWallet is Ownable {
	...
	constructor (address _pool, address _owner, address _username, address _lezmeWallet, address _lezmeToken) {
		pool = _pool;
		USERNAME = _username;
