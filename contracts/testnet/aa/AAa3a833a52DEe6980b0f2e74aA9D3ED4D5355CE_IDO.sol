...
//  uint public constant blockInOneMonth = 864000; // 30 * 24 * 60 * 20
  ...
  function updateMaxAllocation(uint _idoAllocation) onlyMainAdmin external {
    idoAllocation = _idoAllocation;
  }
  ...
  function setUserAllocations(address[] calldata _buyers, uint[] calldata _amounts, uint[] calldata _prices) external onlyMainAdmin {
    require(_buyers.length == _amounts.length && _amounts.length == _prices.length, "IDO: invalid data input");
    address buyer;
    uint amount;
    uint price;
    ...
    require(totalAllocated <= idoAllocation, "IDO: amount invalid");
  }

  function removeBuyerAllocation(address _buyer) external onlyMainAdmin {
    require(buyers[_buyer].allocated > 0, "IDO: User have no allocation");
    totalAllocated -= buyers[_buyer].allocated;
    require(buyers[_buyer].boughtAtBlock == 0, "IDO: User have bought already");
    delete buyers[_buyer];
  }

  function buy(USDCurrency _usdCurrency) external {
    Buyer storage buyer = buyers[msg.sender];
    require(buyer.allocated > 0, "IDO: You have no allocation");
    require(buyer.boughtAtBlock == 0, "IDO: You had bought");
