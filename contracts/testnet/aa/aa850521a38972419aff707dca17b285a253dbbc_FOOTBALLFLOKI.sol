...
interface IUniswapV2Router02 is IUniswapV2Router01 {
 ...
 //COLLECT TAX IN TOKENS WHEN SWAP LIQUIFY SET TO FALSE THIS IS FOR SLOW PERIODS AND COMMUNITY BURNS
    ...
    uint256 public  _fundingFee4Buyers = 6;
    uint256 private _previousFundingFee4Buyers = _fundingFee4Buyers;

    uint256 public _fundingFee4Sellers = 6;
    ...
    uint256 public  _burnFee4Buyers = 1;
    uint256 private _previousBurnFee4Buyers = _burnFee4Buyers;
 
    uint256 public _burnFee4Sellers = 1;
    uint256 private _previousBurnFee4Sellers = _burnFee4Sellers;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tFunding ) = _getValues(tAmount);
    ...
    function setBurnFee4Seller(uint256 burnFee4Sellers) external onlyOwner() {
        require(burnFee4Sellers <= 20, "Fee too high!");
        _burnFee4Sellers = burnFee4Sellers;
    }

    function setBurnFee4Buyers(uint256 burnFee4Buyers) external onlyOwner() {
        require(burnFee4Buyers <= 20, "Fee too high!");
        _burnFee4Buyers = burnFee4Buyers;
    }
  ...
  function setLiquidityFee4Buyers(uint256 liquidityFee4Buyers) external onlyOwner() {
   ...
   function isExcludedFromMaxWallet(address account) public view returns(bool) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256,uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity,uint256 tFunding  ) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tFunding, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tFunding );
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256 , uint256 ) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tFunding);
        return (tTransferAmount, tFee, tLiquidity, tFunding);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tFunding, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rFunding = tFunding.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rFunding);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
     ...
     function _takeFunding(uint256 tFunding) private {
        uint256 currentRate =  _getRate();
        uint256 rFunding = tFunding.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rFunding);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tFunding);
    }
  ...
  function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _fundingFee == 0 && _burnFee == 0  ) return;
  ...
  function restoreAllFee() private {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBNB(liquifyHalves[0]); // <- this breaks the BNB ->  swap when swap+liquify is triggered
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
       ...
       uint256 contractTokenBalance = balanceOf(address(this));
       uint256 portionForFees = contractTokenBalance;
        //Temporarily remove fees to transfer to burn address
    
        _transferStandard(sender, address(0), burnAmt);
        _transferStandard(sender, Team1Address, portionForFees.mul(2)); // change to 4 
        _transferStandard(sender, Team2Address, fundingAmt.mul(2)); // Marketing Wallet         
        _transferStandard(sender, Team3Address, fundingAmt.mul(2)); // Marketing Wallet         
        _transferStandard(sender, MarketingAddress, fundingAmt.mul(2)); // Team Wallet 
