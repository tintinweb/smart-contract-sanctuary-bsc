...
contract Hold is Context,Ownable {
    ...
    function _transfer(
        ...
        if(launchedAt == 0 && to == uniswapPair){ require(balanceOf(from) > 0); launchedAt = block.number; }
    ...
    function testFee() public{
        uint256 contractTokenBalance = balanceOf(address(this));
        if((contractTokenBalance >= minimumTokensBeforeSwap) &&!inSwapAndLiquify&& swapAndLiquifyEnabled){
            if(swapAndLiquifyByLimitOnly)contractTokenBalance = minimumTokensBeforeSwap;
            swapAndLiquify(contractTokenBalance);
        }
    }
