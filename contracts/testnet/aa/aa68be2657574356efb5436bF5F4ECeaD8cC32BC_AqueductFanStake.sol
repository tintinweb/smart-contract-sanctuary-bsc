...
VRFConsumerBaseV2
{
    ...
    Counters.Counter private _sectionId;
    Counters.Counter private _stageId;
    ...
    mapping(uint256 => uint256) public poolToBetsAmount;
    mapping(uint256 => uint256) public poolToStakesAmount;
    mapping(string => uint256) public resultToStakesAmount;
    bool public isPaused;

    mapping(uint256 => mapping(uint256 => Bet)) public poolIdToTokenIdToBet;
    mapping(uint256 => mapping(address => uint256[]))
    public poolIdToAccountToTokenIds;
    ...
    mapping(uint256 => mapping(string => uint256)) public poolToResultToPoint;
    mapping(uint256 => uint256) public betIdToPoolId;
    mapping(uint256 => string) public poolIdToWinner;

    mapping(uint256 => mapping(uint256 => uint256))
    public poolIdToStakesIdToTokenId;
    mapping(uint256 => mapping(uint256 => uint256[]))
    public poolIdToBetIdToStakes;

    // MODIFIERS //
    modifier poolShouldExist(uint256 poolId_) {
        require(poolId_ <= _poolId.current(), "Pool doesn't exist");
        _;
    }
    ...
    modifier stageShouldBeFinished() {
        _;
    }
    ...
    modifier onlyAdmin() {
        ITreasury(treasury).isAdmin(msg.sender);
        _;
    }
    ...
    event BetModified(uint256 stakeId, uint256 newAmount);
    event StakeWithdrawn(address user, uint256 stakeId, uint256 poolId);

    // CHAINLINK
    VRFCoordinatorV2Interface COORDINATOR;
    uint64 s_subscriptionId = 2076;
    bytes32 keyHash =
    ...
    mapping(uint256 => uint256) public requestIdToGoldAmount;
    mapping(uint256 => uint256) public requestIdToSilverAmount;
    uint256[] public result;
    uint256[] public goldRes;
    uint256[] public silRes;

    constructor(address payable treasury_, string memory baseURI_)
    ERC721("Aqueduct Fan Stake", "AFS")
    ...
    returns (string memory)
    {
        ...
        return string(abi.encodePacked(_baseUri, "/", "1", _baseExtension));
    }
    ...
    override(VRFConsumerBaseV2)
    {
        ...
        for (
            ...
            i < requestIdToGoldAmount[requestId];
            i++
        ) {
        ...
        goldRes = goldArray;
        silRes = silverArray;
        revealToken(requestIdToPoolId[requestId], goldArray, silverArray);
    }

    function reveal(uint256 poolId_) public onlyAdmin {
        uint256 goldAmount = (poolToBetsAmount[poolId_] *
        pools[poolId_].goldTicketChance) / 10000;
        uint256 silverAmount = (poolToBetsAmount[poolId_] *
        pools[poolId_].silverTicketChance) / 10000;
        uint32 numWords = convert(goldAmount + silverAmount);
        ...
        if (numWords > 0) {
            ...
            revealToken(poolId_, new uint256[](0), new uint256[](0));
        }
    }
    ...
    function createSection(string memory sectionName_)
    public
    onlyAdmin
    {
        sections[_sectionId.current()] = Section(sectionName_);
    ...
    function createStage(
        ...
        uint256 startAt_,
        uint256 endAt_,
        uint256 resultSource_,
        string[] memory results_
    ) public onlyAdmin {
        require(resultSource_ < 2, "Source result ids are 0 and 1");
    ...
    function createStake(
    ...
    ) public poolShouldExist(poolId_) whenNotPaused {
        ...
        uint256 toTreasury = (pools[poolId_].betPrice * CurrentEvent.commission) / 10000;
        uint256 toContract = pools[poolId_].betPrice -
        (pools[poolId_].betPrice * CurrentEvent.commission) /
        10000;
        ...
        poolIdToTokenIdToBet[poolId_][_tokenId.current()].betOn = result_;
        ...
        poolToBetsAmount[poolId_] += amount_;
        ...
        for (uint256 i = 0; i < amount_; i++) {
            poolIdToStakesIdToTokenId[poolId_][_stakeId.current()] = _tokenId
            .current();
            poolIdToBetIdToStakes[poolId_][_tokenId.current()].push(
                _stakeId.current()
            );
            _stakeId.increment();
        }
    ...
    function withdrawStake(uint256 poolId_, uint256 betId_)
    public
    poolShouldExist(poolId_)
    shouldBeUsersToken(poolId_, betId_)
    {
        require(
            block.timestamp <= stages[pools[poolId_].stageId].endAt,
            "This stage is finished"
        );
        uint256 amountToWithdraw = (pools[poolId_].betPrice -
        (pools[poolId_].betPrice * CurrentEvent.commission) /
        10000) * poolIdToTokenIdToBet[poolId_][betId_].amount;

        IERC20(erc20).transfer(msg.sender, amountToWithdraw);

        delete poolIdToAccountToTokenIds[poolId_][msg.sender][
        ...
        uint256[] memory stakes = poolIdToBetIdToStakes[poolId_][betId_];

        for (uint256 i = _tokenId.current(); i > 0; i--) {
            uint256[] memory lastTokenStakes = poolIdToBetIdToStakes[poolId_][
            i
            ];

            for (
                uint256 f = 0;
                (f < lastTokenStakes.length || f < stakes.length);
                f++
            ) {
                poolIdToStakesIdToTokenId[poolId_][stakes[f]] = i;
                _stakeId.decrement();
                delete poolIdToBetIdToStakes[poolId_][betId_][f];
                delete stakes[f];
            }
        }

        poolToResultToAmount[poolId_][
        poolIdToTokenIdToBet[poolId_][betId_].betOn
        ] -= poolIdToTokenIdToBet[poolId_][betId_].amount;
        poolToStakesAmount[poolId_]--;
        resultToStakesAmount[
        poolIdToTokenIdToBet[poolId_][betId_].betOn
        ] -= poolIdToTokenIdToBet[poolId_][betId_].amount;
        poolToBetsAmount[poolId_] -= poolIdToTokenIdToBet[poolId_][betId_]
        .amount;
        betIdToPoolId[betId_] = 0;
        poolToAssets[poolId_] -= amountToWithdraw;

        poolIdToTokenIdToBet[poolId_][betId_] = Bet(0, 0, 0, 0, "");
        burn(betId_);
        emit StakeWithdrawn(msg.sender, betId_, poolId_);
    }
    ...
    function revealToken(
    ...
    ) internal stageShouldBeFinished {
        poolToCommonStakesAmount[poolId_] =
        poolToBetsAmount[poolId_] -
        ...
        uint256 betPrice = pools[poolId_].betPrice -
        (pools[poolId_].betPrice * CurrentEvent.commission) /
        10000;
        ...
        for (uint256 i = 0; i < silverRandomNumbers_.length; i++) {
            ...
            Bet memory currentBet = poolIdToTokenIdToBet[poolId_][tokenId];

            if (currentBet.amount != currentBet.silverAmount) {
                poolIdToTokenIdToBet[poolId_][tokenId].ticketType = 1;
                poolIdToTokenIdToBet[poolId_][tokenId].silverAmount++;
                poolIdToResultToSilverAmount[poolId_][currentBet.betOn]++;
            }
        }
        ...
        for (uint256 i = 0; i < goldRandomNumbers_.length; i++) {
            ...
            if (
                ...
                poolIdToTokenIdToBet[poolId_][tokenId].silverAmount--;
                poolIdToResultToGoldAmount[poolId_][currentBet.betOn]++;
                poolIdToResultToSilverAmount[poolId_][currentBet.betOn]--;
                poolIdToTokenIdToBet[poolId_][tokenId].ticketType = 2;
            } else {
                poolIdToTokenIdToBet[poolId_][tokenId].goldAmount++;
                poolIdToResultToGoldAmount[poolId_][currentBet.betOn]++;
                poolIdToTokenIdToBet[poolId_][tokenId].ticketType = 2;
        ...
        for (uint256 i = 0; i < results.length; i++) {
            ...
            if (
                ...
                (poolToBetsAmount[poolId_] * (10**18)) /
                (commonStakesAmount +
                silverStakesAmount *
                5 +
                goldStakesAmount *
            ...
            (poolToResultToPoint[poolId_][results[i]] * betPrice) /
            (10**18);
            poolIdToBetOnToTicketTypeToProfit[poolId_][results[i]][2] =
            (poolToResultToPoint[poolId_][results[i]] * betPrice * 5) /
            (10**18);
            poolIdToBetOnToTicketTypeToProfit[poolId_][results[i]][3] =
            (poolToResultToPoint[poolId_][results[i]] * betPrice * 25) /
            (10**18);
        }
    }

    function setWinner(uint256 poolId_, string memory winner_)
    public
    onlyAdmin
    {
        poolIdToWinner[poolId_] = winner_;
    }
    ...
    shouldBeUsersToken(betIdToPoolId[tokenId_], tokenId_)
    {
        ...
        Bet memory bet_ = poolIdToTokenIdToBet[poolId][tokenId_];
        require(
            ifStringsEqual(bet_.betOn, poolIdToWinner[poolId]),
        ...
        IERC20(erc20).transferFrom(address(this), msg.sender, profit);
    }
