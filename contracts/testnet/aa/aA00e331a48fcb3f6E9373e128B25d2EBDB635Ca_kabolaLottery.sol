...
// File: contracts/kabolaLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title PankabolaSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract kabolaLottery is ReentrancyGuard, IkabolaLottery, Ownable {
    ...
    uint256 public maxPriceTicketInkabola = 50 ether;
    uint256 public minPriceTicketInkabola = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInkabola;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] kabolaPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInkabola;
        uint32 finalNumber;
    }
     ...
     * @param _kabolaTokenAddress: address of the kabola token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _kabolaTokenAddress, address _randomGeneratorAddress) {
        kabolaToken = IBEP20(_kabolaTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of kabola to this contract
        uint256 amountkabolaToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInkabola,
            _ticketNumbers.length
        );

        // Transfer kabola tokens to this contract
        kabolaToken.safeTransferFrom(address(msg.sender), address(this), amountkabolaToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInkabola += amountkabolaToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInkabolaToTransfer
        uint256 rewardInkabolaToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInkabolaToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        kabolaToken.safeTransfer(msg.sender, rewardInkabolaToTransfer);

        emit TicketsClaim(msg.sender, rewardInkabolaToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInkabola) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in kabola for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].kabolaPerBracket[j] =
                ...
                // A. No kabola to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].kabolaPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInkabola - amountToShareToWinners);

        // Transfer kabola to treasury address
        kabolaToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        kabolaToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInkabola += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInkabola >= minPriceTicketInkabola) && (_priceTicketInkabola <= maxPriceTicketInkabola),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInkabola: _priceTicketInkabola,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            kabolaPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInkabola: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(kabolaToken), "Cannot be kabola token");
    ...
    /**
     * @notice Set kabola price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInkabola: minimum price of a ticket in kabola
     * @param _maxPriceTicketInkabola: maximum price of a ticket in kabola
     */
    function setMinAndMaxTicketPriceInkabola(uint256 _minPriceTicketInkabola, uint256 _maxPriceTicketInkabola)
        external
        onlyOwner
    {
        require(_minPriceTicketInkabola <= _maxPriceTicketInkabola, "minPrice must be < maxPrice");

        minPriceTicketInkabola = _minPriceTicketInkabola;
        maxPriceTicketInkabola = _maxPriceTicketInkabola;
    }
     ...
     * @param _priceTicket price of a ticket (in kabola)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].kabolaPerBracket[_bracket];
        } else {
            return 0;
        }
    }
