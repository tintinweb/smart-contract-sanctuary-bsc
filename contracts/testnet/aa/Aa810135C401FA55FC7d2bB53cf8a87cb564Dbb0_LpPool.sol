...
contract LpPool is OwnableUpgradeable, DivestorUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    event ClaimLpReward(address indexed account, uint indexed value);
    ...
    function claimLpReward(uint total_, uint amount_, uint timestamp_, bytes32 r, bytes32 s, uint8 v) external returns (uint) {
        require(block.timestamp < timestamp_, 'signature expired');
        bytes32 hash = keccak256(abi.encodePacked(total_, amount_, timestamp_, _msgSender(), address(this)));
        address a = ecrecover(hash, v, r, s);
        require(a == banker, "invalid signature");

        totalClimed += amount_;
        userClaim[_msgSender()].claim += amount_;
        userClaim[_msgSender()].claimNumber += 1;
        require(userClaim[_msgSender()].claim <= total_, 'wrong amount');

        token.transfer(_msgSender(), amount_);

        emit ClaimLpReward(_msgSender(), amount_);
        return amount_;
    }
}