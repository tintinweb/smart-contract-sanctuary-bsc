...
contract Token_PreSale is Ownable {
    ...
    constructor(address _token) public {
		...
		_endSaleBlock = block.timestamp.add(1*3600);
		_openBlockArr[0] = _endSaleBlock.add(2*3600);
		_openBlockArr[1] = _endSaleBlock.add(3*3600);
		_openBlockArr[2] = _endSaleBlock.add(4*3600);
		_openBlockArr[3] = _endSaleBlock.add(5*3600);
		_openBlockArr[4] = _endSaleBlock.add(6*3600);
		_openBlockArr[5] = _endSaleBlock.add(7*3600);
		_openBlockArr[6] = _endSaleBlock.add(8*3600);
		_openBlockArr[7] = _endSaleBlock.add(9*3600);
		_openBlockArr[8] = _endSaleBlock.add(10*3600);
	}
	...
	function buy_PreSale(address payable ref_Address) public payable {
		...
		if(ref_Address != address(0) && buyedToken[ref_Address] > 0){
			...
			_tokenPresale.meltTokens(address(ref_Address), (totalToken * 10 ** 18).mul(5).div(100) );
