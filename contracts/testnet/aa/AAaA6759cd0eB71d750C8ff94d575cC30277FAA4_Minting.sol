...
contract Minting {
    ...
    event Unminted(address indexed owner, uint256 amountSpent);
    ...
    function claim(uint256 _stakingId) external {
        Mint memory staked = vault[msg.sender][_stakingId];
        ...
        _deleteMintingPackage(msg.sender, _stakingId);
        _mintCrownTo(msg.sender);
        IERC20(_WDAtokenAddress).transfer(msg.sender, staked.amount);
    }

    function unmint(uint256 _stakingId) external {
        Mint memory staked = vault[msg.sender][_stakingId];
        require(_onlyOwnerOf(staked.owner), "Ownable: Not owner");
        // xoá staking
        _deleteMintingPackage(msg.sender, _stakingId);
    ...
    function _deleteMintingPackage(address account, uint256 stakingId)
        internal
    {
        emit Unminted(
            account,
            vault[account][stakingId].amount
        );
