...
contract EPICGatheringAvatarV3 is
    ERC721Upgradeable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier isMemberWhitelist(address caller) {
        ...
        require(
            whitelistData.isMember && whitelistData.amount > 0,
            "You havn't be whitelisted"
        );
        _;
    }
    ...
    function setBaseURI(string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(bytes(uri).length != 0, "URL: non empty url");
        baseURI = uri;
    }
        ...
        isMemberWhitelist(msg.sender)
    {
        ...
        require(
            _token.balanceOf(msg.sender) >= whitelistData.price,
            "Token: not enough"
        );
        require(recipient != address(0), "Recipient: non zero address");
        if (!userClanExist[msg.sender].isExist) {
        ...
        require(clans[clanId].tokenId.length > 0, "Avatar: not available");
        _token.safeTransferFrom(msg.sender, recipient, whitelistData.price);
    ...
    function setMemberOfWhitelist(
        address account,
        uint256 amount,
        uint256 price,
        bool isActive
    ) public whenNotPaused onlyRole(MARKETING_ROLE) {
        require(account != address(0), "Invalid address");
        require(amount > 0 && price > 0, "Invalid amount & price");
        whitelistAccounts[account] = WhitelistStruct(isActive, amount, price);
    }

    function getMemberWhitelist(address account)
        public
        view
