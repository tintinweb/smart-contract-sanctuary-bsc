    ...
    FragNFTStorageV1
{
    ...
    function mint(uint256 property, address to) public onlyOwner returns (uint256) {
    ...
    /// @param property NFT token property
    /// @param to NFT token owner
    function mintMany(uint256 property, address to, uint256 count) public onlyOwner returns (uint256[] memory) {
        require(property != 0, "param property should NOT be 0");
    ...
    function synthesize(uint256[] memory from,
        uint256 rsgcAmt,
        uint256 propsKey) public onlyOwner returns (uint256) {
        address sender = _msgSender();

        return _synthesize(from, rsgcAmt, sender, propsKey);
    }
    ...
    function _synthesize(uint256[] memory from,
        uint256 rsgcAmt,
        address sender,
        uint256 prop) private returns (uint256) {
        ...
        uint256 tokenId = propAddr.mint(prop, sender);
