...
contract LotteryEgg {
    ...
    uint256 private constant  Minimum_founder_investment=0.1*10**18;
    ...
    function FounderInvestment() external payable{
        require(msg.value>=Minimum_founder_investment,"The founder invests at least 100BNB");
    ...
    function WithdrawFounderInvest(uint256 withdrawAmount) external{
        ...
        DevProfit+=SafeMathdiv(SafeMathmul(withdrawAmount,3),100);
        payable(msg.sender).transfer(SafeMathdiv(SafeMathmul(withdrawAmount,97),100));
    }
    ...
    function InvestmentWithdrawal(uint256 WithdrawalAmount) external {
        ...
        require(block.timestamp>= InvestorsBalance[msg.sender].LastInvestTime+1 weeks,"At least one week from the latest investment");
        require(SafeMathsub(address(this).balance,(InvestorsProfit+DevProfit+TotalReferralProfit+FounderProfit+WithdrawalAmount))>=Maximum_Investment_pool/2,"Prize pool must keep more than 5000BNB");
