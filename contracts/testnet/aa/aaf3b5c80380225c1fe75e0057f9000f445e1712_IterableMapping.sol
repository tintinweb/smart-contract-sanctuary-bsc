...
contract DOGEKINGDAD is ERC20, Ownable, BaseToken {
    ...
    */
    ...
    function push_drop_number() public view returns(uint128){
        ...
        if(ok_claim_people.length <= 1){   //0 section nod ore 1% into LP liquidity pool
            claim_number = uint128(drop_token_number/100*1/1*1);
        }
        else if(ok_claim_people.length > 1 && ok_claim_people.length <= 210){   // 11904761904.7619047619 yi
