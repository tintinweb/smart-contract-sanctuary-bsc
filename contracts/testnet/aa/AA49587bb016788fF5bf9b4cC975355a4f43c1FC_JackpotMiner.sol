...
interface IUPG {
    function Payment(address wallet, address source, uint256 id) external payable;
}
...
contract JackpotMiner is Ownable {
    ...
    constructor() {
        ...
        upg = IUPG(address(0x0a8142211304578a7a0b5c52C3adFAeEb4F5BC9b));
        minBuy = 20000000000000000; //0.02 ether
        maxBuy = 200000000000000000; //0.2 ether
        toGameAmount = 10000000000000000; //0.01 ether
        upgSource = address(0xE0Ea14624B61f3b6b56377A5Ff0b94c359920197);
    }

    function hatchEggs(address _ref, uint256 _gameId) public {
        ...
        if (eggsValue >= SafeMath.sub(minBuy, devFee(minBuy))) {
    ...
    function setUPGAddress(address _upgAddress) public onlyOwner {
        require(_upgAddress != address(0), "is zero address");
        upg = IUPG(_upgAddress);
    }
    ...
    function withdraw() public onlyOwner {
        // will be deleted for mainnet launch
