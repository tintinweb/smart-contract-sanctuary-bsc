...
contract KingsCastleFactory is Ownable {
    ...
    function createKingsCastle(uint256 _rewardRate, address _BUSD) external onlyOwner {
    ...
    function removeKingsCastle(address _kingsCastle) external onlyOwner {
        require(
            ...
            "KingsCastleFactory: kings castle not found"
        );
...
contract KingsCastle is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        mapping (uint256 => EnumerableSet.UintSet) tokenPerLottery;
        uint256 rewards;
    ...
    mapping(uint256 => address) public stakerPerLottery;
    mapping(uint256 => uint256) private winningTokenPerLottery;
    ...
    function updatePool(uint256 _tokenId) external onlyLottery {
        if (!firstLottery) {
            nonce++;
        } else {
            firstLottery = false;
        }
        winningTokenPerLottery[nonce] = _tokenId;
    }
    ...
    function stake(uint256 _tokenId) external nonReentrant {
        ...
        require(
            winningTokenPerLottery[nonce] == _tokenId,
            "KingsCastle: outdated or not a winning token"
        );
        ILottery(lottery).burn(_tokenId);
        UserInfo storage user = userInfo[msg.sender];
        user.tokenPerLottery[nonce].add(_tokenId);
        user.lastTimeRewardClaimed = block.timestamp;
        stakerPerLottery[nonce] = msg.sender;
        emit Stake(msg.sender, _tokenId);
    }
    
    function claim() external nonReentrant {
        require(
            msg.sender == stakerPerLottery[nonce],
            "KingsCastle: caller is not a staker of current lottery"
        );
        UserInfo storage user = userInfo[msg.sender];
        ...
        )
    {
        ...
        uint256 amountOfTokens = user.tokenPerLottery[_nonce].length();
        if (amountOfTokens == 0) {
            tokens = new uint256[](0);
        } else {
            tokens = new uint256[](amountOfTokens);
            for (uint256 i = 0; i < amountOfTokens; i++) {
                tokens[i] = user.tokenPerLottery[_nonce].at(i);
            }
        }
    }
    ...
    function pendingRewards(address _user) public view returns (uint256) {
        ...
        uint256 amount = timeBetween * userStakedNFTCountPerLottery(nonce, _user) * rewardRate;
        ...
        returns (uint256)
    {
        require(
            _nonce <= nonce,
            "KingsCastle: invalid nonce"
        );
        return userInfo[_user].tokenPerLottery[_nonce].length();
    }
