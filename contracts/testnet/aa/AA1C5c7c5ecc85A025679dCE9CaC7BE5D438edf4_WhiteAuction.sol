...
contract AuctionTime is Ownable, Random{
    ...
    address public BitapeWallet;
    address public MomoBearWallet = 0x00EcE78d9080f80baA5dA7730EDdbe50751E8472;
    uint public auctionStartTime = 1645444800;// 2022/1/30 20:00
...
contract WhiteAuction is Pausable, AuctionTime, IMomoBear{
    ...
    uint private _priceForUSDT = 200e18; //decimal = 18
    ...
    function bearAuctionAmount() public view returns(uint){
    ...
    function winTheBidByUsdt(
        ...
        require(isCustomer(msg.sender), "You aren`t a customer");
...
//_winTheBid
    ...
    function _winTheBid(uint lastPrice) private{
        uint heroIndex = 905;
        uint allAmount = 20 - bearAuctionAmount() - specialAuctionAmount();
        uint randomNumber = (_getRandomNumber() % allAmount).add(1);
        if(randomNumber > (19 - bearAuctionAmount())){
            heroIndex = 906;
            auctionAmount[2].increment();
        }else{
            auctionAmount[1].increment();
        }
        _mintHero(msg.sender, heroIndex);
        emit WinTheBid(msg.sender, now, heroIndex, lastPrice);
    }
...
//onlyOwner
    ...
    function setMomoBearWallet(address _wallet) external onlyOwner{
        MomoBearWallet = _wallet;
    }
