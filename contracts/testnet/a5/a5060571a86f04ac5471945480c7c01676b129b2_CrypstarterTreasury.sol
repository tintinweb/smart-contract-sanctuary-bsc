...
// File contracts/interfaces/ICST.sol


pragma solidity >=0.8.2;

interface ICST is IERC20 {
...
contract CrypstarterTreasury is OwnableUpgradeable, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    function initialize(address _cst, uint256 _timelock) public initializer {
        __Ownable_init();
    ...
    function __init_messages() internal onlyInitializing {
    ...
    /**
     * @notice allow approved address to deposit an asset for CST
    ...
    function deposit(
        ...
        // mint CST needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        CST.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn CST for reserves
     * @param _amount uint256
     * @param _token address
     */
    ...
    /**
     * @notice mint new CST using excess reserves
        ...
        for (uint256 i = 0; i < reserveToken.length; i++) {
            if (permissions[STATUS.RESERVETOKEN][reserveToken[i]]) {
                reserves = reserves.add(
                    tokenValue(
                        reserveToken[i],
                        IERC20Upgradeable(reserveToken[i]).balanceOf(
                            address(this)
                        )
                    )
                );
        ...
        for (uint256 i = 0; i < liquidityToken.length; i++) {
            if (permissions[STATUS.LIQUIDITYTOKEN][liquidityToken[i]]) {
                reserves = reserves.add(
                    tokenValue(
                        liquidityToken[i],
                        IERC20Upgradeable(liquidityToken[i]).balanceOf(
                            address(this)
                        )
                    )
                );
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SCST) {
            // 9
            sCST = IsCST(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(CST.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns CST valuation of asset
     * @param _token address
     * @param _amount uint256
    ...
    function tokenValue(address _token, uint256 _amount)
        ...
        returns (uint256 value_)
    {
        value_ = _amount.mul(10**IERC20Metadata(address(CST)).decimals()).div(
    ...
    function baseSupply() external view override returns (uint256) {
        return CST.totalSupply() - cstDebt;
    }
}