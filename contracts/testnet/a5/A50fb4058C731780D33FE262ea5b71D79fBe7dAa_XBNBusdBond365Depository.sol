...
import "./EthBondDepository.sol";

contract XBNBusdBond365Depository is TimeBondDepository {}
...
contract TimeBondDepository is Initializable, OwnableUpgradeable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public XBN; // token given as payment for bond
    ...
    function stakeOrSend(address _recipient, uint256 _amount) internal returns (uint256) {
        IERC20(XBN).transfer(_recipient, _amount); // send payout
        return _amount;
    }

    function swap(uint256 _value, address to) internal {
        // generate the pancake pair path of principle -> XBN
    ...
    function liquidify(uint256 _value) internal {
        if (_value > 0) {
            if (IERC20(XBN).allowance(address(this), address(pancakeRouter)) == 0) {
                IERC20(XBN).approve(address(pancakeRouter), ~uint256(0));
            }
            ...
            uint256 oldBalance = IERC20(XBN).balanceOf(address(this));
            swap(_value.div(2), address(this));
            uint256 newBalance = IERC20(XBN).balanceOf(address(this));

            pancakeRouter.addLiquidity(XBN, principle, newBalance.sub(oldBalance), _value.div(2), 0, 0, treasury, block.timestamp + 360);
        }
    }
    ...
    function distributeReferral(address _referrer, uint256 _value) internal {
        if (_referrer != address(0)) {
            ...
            IERC20(XBN).safeTransfer(_referrer, payout);
        }
    }
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(XBN).totalSupply().mul(terms.maxPayout).div(100000);
    }
