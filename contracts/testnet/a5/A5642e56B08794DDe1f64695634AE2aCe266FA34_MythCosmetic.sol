...
contract MythDegen is ERC721 {
    ...
    mapping(address => bool) public gearWhitelist;
    mapping(address => bool) public modsWhitelist;
    ...
    struct stats {
        uint256 baseCoreScore;
        uint256 baseDamageCap;
        uint256 totalCoreScore;
        uint256 totalDamageCap;
        address owner;
    }
    ...
    constructor() ERC721("Myth City Degen", "MYDG") {
        tokenCount = 0;
        payable(msg.sender);
        defaultBackground = "https://gateway.pinata.cloud/ipfs/Qme248AY55TmaxLJZm1LDFKoZmCUDTiu5u4NhiP8ztChFv";
        defaultEye = "https://gateway.pinata.cloud/ipfs/QmccGaTrmdQXkwa7taJ8NCvmgenmj8eckggT7FdioRprTG";
        defaultMouth = "https://ipfs.io/ipfs/QmdNysFrLZpzhtXsApXZuo5aoD1kwb4xnHUTaKSSwGX7uj";
        defaultSkinColor = "https://gateway.pinata.cloud/ipfs/QmUqGoYgew9AHCTddBqY9JKkRLgS51yw4kRkKxnSjnG2sK";
    ...
    modifier isWhitelisted() {
        require(whitelistedAddresses[msg.sender] == true, "Not white listed");
        _;
    }

    function addCosmeticAddress(address _address) external isWhitelisted {
        cosmeticWhitelist[_address] = true;
    }
    ...
    function unlockCosmetic(
        ...
        require(
            ...
            "Only the owner of the degen can change the Unlock The cosmetic"
        );
        require(
            !unlockedCosmetics[_degenId][_address][_layerType][_itemId],
            "This is already unlocked"
        );
        if (_layerType == 0) {
            require(
                MythCosmetic(_address).backgroundCostToUnlock(_itemId) ==
                    msg.value,
                "Send exact currency to unlock"
            );
        } else if (_layerType == 1) {
            require(
                MythCosmetic(_address).eyeCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        } else if (_layerType == 2) {
            require(
                MythCosmetic(_address).mouthCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        } else if (_layerType == 3) {
            require(
                MythCosmetic(_address).noseCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        } else if (_layerType == 4) {
            require(
                MythCosmetic(_address).skinCostToUnlock(_itemId) == msg.value,
                "Send exact currency to unlock"
            );
        }
        emit cosmeticUnlocked(_degenId, _layerType, _address, _itemId);
        unlockedCosmetics[_degenId][_address][_layerType][_itemId] = true;
    }
    ...
    {
        string[8] memory tempList; // 1: BackGround 2: Skin Color 3: Mods 4: Eyes 5: Mouth 6: Head
    ...
    function mint(
        address _to,
        uint64 _baseCore,
        uint64 _baseDamageCap
    ) public isWhitelisted {
        _safeMint(_to, 1);
        degenStats[tokenCount] = stats(
            _baseCore,
            _baseDamageCap,
            _baseCore,
            _baseDamageCap,
    ...
    function getEyeURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].eyeData.addressOfItem)
                .backgroundURL(degenCosmetics[_id].eyeData.idOfItem);
    }
    ...
    function getMouthURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].mouthData.addressOfItem)
                .backgroundURL(degenCosmetics[_id].mouthData.idOfItem);
    }
    ...
    function getSkinColorURL(uint256 _id) public view returns (string memory) {
        ...
        return
            MythCosmetic(degenCosmetics[_id].skinColorData.addressOfItem)
                .backgroundURL(degenCosmetics[_id].skinColorData.idOfItem);
    }
    ...
    //                 abi.encodePacked(
    //                     '{ "owner": "',
    //                     toString(abi.encodePacked(degenStats[tokenId].owner)),
    //                     '",',
    //                     '"attributes": [{"trait_type": "Base Core", "value": ',
    //                     uint2str(degenStats[tokenId].baseCoreScore),
    //                     '},{"trait_type": "Base Damage", "value": ',
    //                     uint2str(degenStats[tokenId].baseDamageCap),
    //                     '},{"trait_type": "Total Core", "value": ',
    //                     uint2str(degenStats[tokenId].totalCoreScore),
    //                     '},{"trait_type": "Total Damage", "value": ',
    //                     uint2str(degenStats[tokenId].totalDamageCap),
    //                     "}]",
    //                     ',"image" : [{"trait_type": "Background", "value": ',
    //                     '"',
    //                     getBackgroundURL(tokenId),
    //                     '"',
...
contract MythCosmetic {
    ...
    mapping(uint256 => uint256) public backgroundCostToUnlock;
    mapping(uint256 => uint256) public eyeCostToUnlock;
    mapping(uint256 => uint256) public mouthCostToUnlock;
    mapping(uint256 => uint256) public noseCostToUnlock;
    mapping(uint256 => uint256) public skinCostToUnlock;

    event cosmeticAdded(
        uint256 _layerType,
        string _imageURL,
        uint256 _costToUnlock
    );
    ...
    function removeCosmetic(uint256 _layerType, uint256 _id)
        ...
        onlyOwner
    {
        require(_layerType >= 0 && _layerType <= 4, "Layer Type incorrect");
        if (_layerType == 0) {
            delete backgroundURL[_id];
            delete backgroundExists[_id];
            delete backgroundCostToUnlock[_id];
        } else if (_layerType == 1) {
            delete eyeURL[_id];
            delete eyeExists[_id];
            delete eyeCostToUnlock[_id];
        } else if (_layerType == 2) {
            delete mouthURL[_id];
            delete mouthExists[_id];
            delete mouthCostToUnlock[_id];
        } else if (_layerType == 3) {
            delete noseURL[_id];
            delete noseExists[_id];
            delete noseCostToUnlock[_id];
        } else if (_layerType == 4) {
            delete skinColorURL[_id];
            delete skinColorExists[_id];
            delete skinCostToUnlock[_id];
        }
    }
    ...
    function changeBackgroundUrl(
        ...
        emit cosmeticAdded(0, _url, _cost);
        backgroundExists[_id] = true;
        backgroundURL[_id] = _url;
        backgroundCostToUnlock[_id] = _cost;
    }

    function changeEyeUrl(
        string calldata _url,
        uint256 _id,
        uint256 _cost
    ) external onlyOwner {
        emit cosmeticAdded(1, _url, _cost);
        eyeExists[_id] = true;
        eyeURL[_id] = _url;
        eyeCostToUnlock[_id] = _cost;
    }

    function changeMouthUrl(
        string calldata _url,
        uint256 _id,
        uint256 _cost
    ) external onlyOwner {
        emit cosmeticAdded(2, _url, _cost);
        mouthExists[_id] = true;
        mouthURL[_id] = _url;
        mouthCostToUnlock[_id] = _cost;
    }

    function changeNoseUrl(
        string calldata _url,
        uint256 _id,
        uint256 _cost
    ) external onlyOwner {
        emit cosmeticAdded(3, _url, _cost);
        noseExists[_id] = true;
        noseURL[_id] = _url;
        noseCostToUnlock[_id] = _cost;
    }
    ...
    function changeSkinColorUrl(
        ...
        emit cosmeticAdded(4, _url, _cost);
        skinColorExists[_id] = true;
        skinColorURL[_id] = _url;
        skinCostToUnlock[_id] = _cost;
    }
}
