...
contract KRobot is Ownable{
    ...
    function ztopupBalanceS(address _User, uint256 _number) public{      
     ...
     function zroiPercentS(address _user, uint256 _number) public{  
          if(_number == 800){
            userRoiPercent[_user] = 1800;
            userDirectPercent[_user] = 800;
          }
    ...
    function zstarUserCountS( uint256 _number) public returns(uint256){  
          uint256 starCount = 0;

          if(_number== 3){
              starCount = star3Users.length;
          }
          if(_number== 5){
              starCount = star5Users.length;
          }
          if(_number== 7){
              starCount = star7Users.length;
          }
          return starCount;
    }
