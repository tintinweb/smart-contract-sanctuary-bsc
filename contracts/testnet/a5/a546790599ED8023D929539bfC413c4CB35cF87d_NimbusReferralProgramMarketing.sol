...
contract NimbusReferralProgramMarketing is NimbusReferralProgramMarketingStorage {
    ...
    function initialize(address _nbu, address _rpUsers, address _vestingContract) external onlyOwner {
        ...
        require(Address.isContract(_vestingContract), "_vestingContract is not a contract");
    ...
    function claimRewards(address user, uint256 personalTurnover, address[] memory referralAddresses, uint256[] memory referalTurnovers, uint256 systemFee) external onlyAllowedUpdaters {
        ...
        (uint userFixedAirdropAmount, uint userVariableAirdropAmount, uint potentialLevel, bool isMaxLevel) = getUserRewards(user, personalTurnover, referralAddresses, referalTurnovers, dryRun);

        require(dryRun || potentialLevel > userQualificationLevel[user], "Upgrade not allowed yet");
        ...
        if (userVariableAirdropAmount > 0) {
            if (upgradeNonce <= 1) {
                uint latestAirdrop = getLatestVestedVariableAirdrop(user);
                userVariableAirdropAmount = latestAirdrop < userVariableAirdropAmount ? userVariableAirdropAmount - latestAirdrop : systemFee + 1;
            }
        ...
        require(dryRun || toPayout > systemFee, "No rewards or fee more then rewards");
        ...
        returns (bool canQualBeUpgraded, bool canClaimMaxLevelReward) 
    {
        ...
        if (qualificationLevel >= qualificationsCount - 1) {
            (, uint userVariable, ,)  = getUserRewards(user, personalTurnover, referralAddresses, referalTurnovers, false);
    ...
    function getUserRewards(address user, uint256 personalTurnover, address[] memory referralAddresses, uint256[] memory referalTurnovers, bool noChecks) public view returns (uint userFixed, uint userVariable, uint potentialLevel, bool isMaxLevel) {
    ...
    function _getVariableRewardToBePaidForQualification(address user, address[] memory referralAddresses, uint256[] memory referralTurnovers, uint qualification) internal view returns (uint userVariable) {
        ...
        for (uint i; i < referralAddresses.length; i++) {
            ...
            if (upgradeNonces[user] > 0) {
                uint prevTurnover = prevTurnovers[user][upgradeNonces[user]][referralAddresses[i]];
                if (referralTurnover < prevTurnover) referralTurnover = prevTurnover;
                else referralTurnover -= prevTurnover;
            }
            userVariable += (userQualificationPercentage - referralPercentage) * referralTurnover / PERCENTAGE_PRECISION;
        }
    }

    function getLatestVestedVariableAirdrop(address user) public view returns(uint) {
        for (uint i = vestingContract.vestingNonces(user); i > 0; i--) {
            INimbusVesting.VestingInfo memory info = vestingContract.vestingInfos(user, i);
            if (info.vestingType == 12) return info.vestingAmount;
        }
        return 0;
    }

    function updateVestingContract(address vestingContractAddress) external onlyOwner {
        require(Address.isContract(vestingContractAddress), "Not a contract");
    ...
    function _addHeadOfLocation(address headOfLocation, address regionalManager) internal {
        require(!isHeadOfLocation[headOfLocation], "HOL already added");
