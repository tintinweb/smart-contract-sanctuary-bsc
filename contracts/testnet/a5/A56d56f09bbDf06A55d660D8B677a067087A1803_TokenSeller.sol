...
interface MintableERC721 is IERC721Enumerable {
...
contract TokenSeller is Ownable, UserRegistry {
    ...
    uint256 mintCounts;
    uint256 mintCounts1Percent;
    uint256 mintCounts3Percent;
    uint256 mintCounts5Percent;
    uint256 mintCounts30Percent;
    ...
    event ClaimNft(address indexed account, bool success);

    constructor(IERC20 _usdt, MintableERC721 _nft, address _receiveAddr) {
        usdt = _usdt;
        nft = _nft;
    ...
    function join(address recommender_) external {
        require(recommender(msg.sender) == address(0), "Already has recommender");
        _join(msg.sender, recommender_);
        users[msg.sender].airdropBalance += 20 * 10**18;
        initialSupply += 20 * 10**18;
    }
    ...
    function claim() external {
        ...
        uint256 airdropBalance = users[msg.sender].airdropBalance;
        users[msg.sender].receiveBalance = 0;
        users[msg.sender].inviteBalance = 0;
        users[msg.sender].airdropBalance = 0;

        det.transfer(msg.sender, receiveBalance + inviteBalance + airdropBalance);

        emit Claim(msg.sender, receiveBalance + inviteBalance + airdropBalance);
    }

    function claimNft() external {
        require(!users[msg.sender].nftClaimed, "Claimed");
        require(users[msg.sender].joinBalance > 0, "Not joined");
        users[msg.sender].nftClaimed = true;

        uint256 random = uint256(keccak256(abi.encodePacked(block.coinbase, block.timestamp, block.difficulty, blockhash(block.number - 32)))) % 100;
        if (users[msg.sender].isFounder) {
            if (random >= 50 && (mintCounts30Percent * 100) / ((mintCounts / 100 + 1) * 100) < 30) {
                mintCounts30Percent++;
                nft.mint(msg.sender);
                emit ClaimNft(msg.sender, true);
            }
        } else if (users[msg.sender].joinBalance >= 86 * 10**18) {
            if (random >= 15 && (mintCounts5Percent * 100) / ((mintCounts / 100 + 1) * 100) < 5) {
                mintCounts5Percent++;
                nft.mint(msg.sender);
                emit ClaimNft(msg.sender, true);
            }
        } else if (users[msg.sender].joinBalance >= 56 * 10**18) {
            if (random >= 10 && (mintCounts3Percent * 100) / ((mintCounts / 100 + 1) * 100) < 3) {
                mintCounts3Percent++;
                nft.mint(msg.sender);
                emit ClaimNft(msg.sender, true);
            }
        } else {
            if (random >= 5 && (mintCounts1Percent * 100) / ((mintCounts / 100 + 1) * 100) < 1) {
                mintCounts1Percent++;
                nft.mint(msg.sender);
                emit ClaimNft(msg.sender, true);
            }
        ...
        mintCounts++;
        emit ClaimNft(msg.sender, false);
    }
...
contract DETNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("DET NFT", "DET") {
    }

    function mint(address account) external onlyOperator {
