...
contract CBDVendor is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ICBDVendor, EIP712{
    ...
    constructor(
        ...
        require(newVendor != address(0), "0 vendorAddr!");  
        require(owner != address(0), "O OwnerAddr!");  
    ...
    function setNewVendor(address newVendor) external onlyOwner{
        require(newVendor != address(0), "0 vendorAddress!");
    ...
    function safeMintWithPermit( 
        ...
        require(blockExpiry > block.number, "Expired!");
        require(isMintable, "Mintable not enabled yet!");
        require(signer != address(0), "Zero address!");
        require(!_nonces[hash], "Nonce already exist!");
        require(factory.hasMinterRole(signer), "Only minter can mint!");
...
contract CBDVendorFactory is Ownable, Pausable, ICBDVendorFactory {
    ...
    function setTreasuryAddress(address newTreasuryWallet) public onlyOwner {
        treasuryWallet = newTreasuryWallet;
    }

    function setNewManager(address newManager) public onlyOwner {
        require(newManager != address(0), "0 addr!");
        manager = ICBDManager(newManager);
    }

    function setTreasuryFee(uint newFee) external onlyOwner {
        treasuryFee = newFee;
    }
    ...
    function createNewVendor(
        ...
        if(treasuryFee != 0 && treasuryWallet != address(0)) {
            require(msg.value >= treasuryFee, "Invalid treasury fee!");

            treasuryWallet.safeTransferETH(msg.value);
        }

        CBDVendor newVendor = new CBDVendor(name,symbol,nftOwner,nftOwner);
        manager.setNewVendor(address(newVendor));
