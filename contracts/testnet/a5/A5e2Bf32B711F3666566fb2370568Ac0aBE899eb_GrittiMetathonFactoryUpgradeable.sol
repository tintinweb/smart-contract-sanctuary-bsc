...
import "./base/NoDelegateCallUpgradeable.sol";
import "./interfaces/IGrittiMetathonFactory.sol";
import "./GrittiMetathonDeployer.sol";

/// @title Canonical Gritti Metathon Factory
/// @notice Deploys Gritti Metathon events and manages ownership
contract GrittiMetathonFactoryUpgradeable is
    GrittiMetathonDeployer,
    Initializable,
    OwnableUpgradeable,
    NoDelegateCallUpgradeable,
    UUPSUpgradeable,
    IGrittiMetathonFactory
{
    ...
    /// @inheritdoc IGrittiMetathonFactory
    function owner() public view virtual override(IGrittiMetathonFactory, OwnableUpgradeable) returns (address) {
        return super.owner();
    }

    /// @inheritdoc IGrittiMetathonFactory
    mapping(string => address) public override getEvent;

    string[] private _allEvents;
    ...
    function createEvent(
    ...
    ) external noDelegateCall onlyOwner returns (address addr) {
        ...
        require(!isEmpty(name), "Invalid name");
        require(!isEmpty(symbol), "Invalid symbol");
        require(!isEmpty(baseURI), "Invalid baseURI");
        require(getEvent[eventSlug] == address(0), "Event already created");

        addr = deploy(address(this), eventSlug, maxSupply, eventName, rootHash, name, symbol, baseURI);

        getEvent[eventSlug] = addr;
        _allEvents.push(eventSlug);

        emit EventCreated(eventSlug, addr);
    }

    function countEvent() public view virtual returns (uint256) {
        return _allEvents.length;
    }

    function listEvent(uint256 offset, uint256 limit) public view virtual returns (string[] memory eventList) {
        eventList = new string[](limit);
        uint256 index = 0;
        for (uint256 i = offset; i < offset + limit; i++) {
            if (i < _allEvents.length) {
                eventList[index] = _allEvents[i];
            } else {
                eventList[index] = "";
            }
            index++;
    ...
    function listEventAddr(uint256 offset, uint256 limit) public view virtual returns (address[] memory addrList) {
        addrList = new address[](limit);
        uint256 index = 0;
        for (uint256 i = offset; i < offset + limit; i++) {
            if (i < _allEvents.length) {
                addrList[index] = getEvent[_allEvents[i]];
            } else {
                addrList[index] = address(0);
            }
            index++;
        }
    }
...
abstract contract NoDelegateCallUpgradeable is Initializable {
...
import "./GrittiMetathon.sol";

// https://github.com/Uniswap/v3-core/blob/main/contracts/UniswapV3PoolDeployer.sol
contract GrittiMetathonDeployer is IGrittiMetathonDeployer {
    struct Parameters {
        address factory;
        string eventSlug;
        uint256 maxSupply;
    ...
    /// @inheritdoc IGrittiMetathonDeployer
    Parameters public override parameters;

    /// @dev Deploys a event with the given parameters by transiently setting the parameters storage slot and then
    ...
    function deploy(
        ...
        addr = address(new GrittiMetathon{salt: keccak256(abi.encode(eventSlug))}());
        delete parameters;
    }
}
...
contract GrittiMetathon is ERC721, ERC721Enumerable, NoDelegateCall, IGrittiMetathon {
    ...
    function mint(address to, uint256 tokenId) public onlyFactoryOwner onlyFactoryOwner {
...
interface IGrittiMetathon is IERC721Enumerable {
    ...
    /// @notice Returns the minter of the `tokenId`
