    ...
    ReentrancyGuardUpgradeable
{
    ...
    function startLiquidate() external onlyBotWallet nonReentrant {
        ...
        require(
            IERC20(rematicAddress).balanceOf(address(this)) > 0,
        ...
        //add liquidity
        _addLiquidity(buyTax, sellTax, tax);

        // distribute BNB
        _distributeBNB(buyTax, sellTax, tax);

        _distibuteREWARD(buyTax, sellTax, tax);
    ...
    function _addLiquidity(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {

        uint256 totalPerc =_getTotalPercentage(buyTax, sellTax, tax);
        uint256 liquidityRFTX = (buyTax.liquidity
            .add(sellTax.liquidity)
            .add(tax.liquidity)).mul(IRematic(rematicAddress).balanceOf(address(this))).div(totalPerc);
    ...
    function _getRFTXAmountForBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal pure returns(uint256 bnbRFTX){
        ...
        bnbRFTX = bnbRFTX.add(sellTax.pension);
        bnbRFTX = bnbRFTX.add(tax.pension);
        bnbRFTX = bnbRFTX.add(buyTax.legal);
        bnbRFTX = bnbRFTX.add(sellTax.legal);
        bnbRFTX = bnbRFTX.add(tax.legal);
        bnbRFTX = bnbRFTX.add(buyTax.team);
        bnbRFTX = bnbRFTX.add(sellTax.team);
        bnbRFTX = bnbRFTX.add(tax.team);
    }

    function _distributeBNB(Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {

            uint256 totalPerc =_getTotalPercentage(buyTax, sellTax, tax);
            uint256 bnbPec = _getRFTXAmountForBNB(buyTax, sellTax, tax);
            if(bnbPec > 0){
                uint256 bnbRFTX = IRematic(rematicAddress).balanceOf(address(this)).mul(bnbPec).div(totalPerc);
                uint256 bnbAmount = swapTokensForEth(rematicAddress, bnbRFTX);

                //send some rewardBNB to Pension wallet
                uint256 pAmount = bnbAmount
                    .mul(
                        buyTax.pension.add(sellTax.pension).add(
                            tax.pension
                        )
                    )
                    .div(bnbRFTX);
                ...
                uint256 tAmount = bnbAmount
                    .mul(
                        buyTax.team.add(sellTax.team).add(
                            tax.team
                        )
                    )
                    .div(bnbRFTX);
    ...
    function _distibuteREWARD (Tax memory buyTax, Tax memory sellTax, Tax memory tax) internal {

        uint256 totalPerc =_getTotalPercentage(buyTax, sellTax, tax);
        //distrubute BUSD on Default Token Tracker
        uint256 hsAmount = (buyTax.divtracker
            .add(sellTax.divtracker)
            .add(tax.divtracker)).mul(IRematic(rematicAddress).balanceOf(address(this))).div(totalPerc);

        _distributeRewardDividends(hsAmount);

        uint256 pRewardRFTX = buyTax.partition
            .add(sellTax.partition)
            .add(tax.partition)
            .add(buyTax.k401)
            .add(sellTax.k401)
            .add(tax.k401);
        ...
        uint256 rewardAmount = swapTokensForREWARD(pRewardRFTX.mul(IRematic(rematicAddress).balanceOf(address(this))).div(totalPerc));

        uint256 hsRewardAmount = rewardAmount
            .mul(
                buyTax.partition.add(sellTax.partition).add(
                    tax.partition
                )
            )
            .div(pRewardRFTX);
    ...
    function _getTotalPercentage(Tax memory buyTax, Tax memory sellTax,Tax memory tax) internal returns (uint256 totalPerc) {
        totalPerc = buyTax.stake;
        totalPerc = totalPerc.add(buyTax.burn);
        totalPerc = totalPerc.add(buyTax.liquidity);
        totalPerc = totalPerc.add(buyTax.pension);
        totalPerc = totalPerc.add(buyTax.legal);
        totalPerc = totalPerc.add(buyTax.team);
        totalPerc = totalPerc.add(buyTax.divtracker);
        totalPerc = totalPerc.add(buyTax.partition);
        totalPerc = totalPerc.add(buyTax.k401);

        totalPerc = totalPerc.add(sellTax.burn);
        totalPerc = totalPerc.add(sellTax.liquidity);
        totalPerc = totalPerc.add(sellTax.pension);
        totalPerc = totalPerc.add(sellTax.legal);
        totalPerc = totalPerc.add(sellTax.team);
        totalPerc = totalPerc.add(sellTax.divtracker);
        totalPerc = totalPerc.add(sellTax.partition);
        totalPerc = totalPerc.add(sellTax.k401);

        totalPerc = totalPerc.add(tax.burn);
        totalPerc = totalPerc.add(tax.liquidity);
        totalPerc = totalPerc.add(tax.pension);
        totalPerc = totalPerc.add(tax.legal);
        totalPerc = totalPerc.add(tax.team);
        totalPerc = totalPerc.add(tax.divtracker);
        totalPerc = totalPerc.add(tax.partition);
        totalPerc = totalPerc.add(tax.k401);
    }
}
