...
contract StakingVaultV6_0_0 is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    uint256 private MIN_LOCK_DURATION = 1 weeks; // 1 week
    ...
    uint256 private BOOST_WEIGHT = 100 * 1e10; // 100%
    ...
    function depositOperation(
        ...
        if (user.shares[_slot] > 0 && user.locked[_slot]) {
        ...
        returns (uint256, uint256)
    {
        UserInfo storage user = userInfo[_user];
        return (
            ((balanceOf() * (user.shares[_slot])) / totalShares) -
    ...
    function updateUserShareAndSendDualToken(address _user, uint256 _slot) internal {
        ...
        if (user.shares[_slot] > 0) {
            ...
            if (!feeDistributor.getFreePerformanceFeeUsers(_user)) {
                ...
                uint256 totalAmount = (user.shares[_slot] * balanceOf()) / totalShares;
    ...
    function distributeRewards() internal whenNotPaused {
        ...
        if (address(feeDistributor) != address(0x0)) {
            if (rewardsPerBlock > 0) {
                tokenHarvest = feeDistributor.withdraw(
                    blocksPassed * rewardsPerBlock,
                    false
                );
            }
            if (dualRewardsPerBlock > 0 && dualTokenVault == true) {
                dualTokenHarvest = feeDistributor.withdraw(
     ...
     * @dev Only possible when contract not paused.
     */
    function setDisableDeposits(bool _disableDeposits) external onlyOwner {
    ...
    function setMinMaxLockDuration(
        ...
        require(_minLockDuration * 1 days >= 7 days, '<min');
        require(_minBondingLockDuration * 1 days >= 7 days, '<bmin');
    ...
    {
        //IUniswapV2Router02
        return (partnerAdmin, bondingContract, feeDistributor); //uniswapV2Router todo add more?
    }
        ...
        returns (uint256)
    {
        ...
        uint256 amount;
        if (_dualToken == false) {
            amount = feeDistributor.pendingRewards(
                blocksPassed,
                rewardsPerBlock,
                _dualToken
            );
        ...
        )
    {
        ...
        return (
            user.lastDepositedTime[_slot],
            user.tokensAtLastUserAction[_slot],
            user.lastUserActionTime[_slot],
            user.dualTokenDebt[_slot]
        );
    }
...
contract StakingFactoryV6 {
    ...
    function createStaking(
        ...
        // require(stakings[token_] == address(0)); TODO
        require(msg.sender == creator);
        StakingVaultV6_0_0 _staking = new StakingVaultV6_0_0(
            IERC20(token_),
            IERC20(dualToken_),
            rewardsPerBlock_,
            dualRewardsPerBlock_,
            tokenDecimals_,
            dualTokenDecimals_,
            partnerAdmin
        );
...
interface IFeeTracker {
    function checkFeeToken(IERC20 token) external returns (bool withoutFee);

    function transferTokenToDead(IERC20 token) external;

    function getWithoutFee() external view returns (bool);
...
contract FeeTracker {
    using SafeERC20 for IERC20;
    mapping(address => mapping(address => bool)) private excludedFromFee;

    function checkVaultExcludedFromFee(IERC20 token, address vault) external {
        console.log(1000 ether);
        console.log(1000 * 10**18);
        uint256 allowance = token.allowance(msg.sender, address(this));
        console.log(allowance);
        token.safeTransferFrom(msg.sender, address(this), 1000 ether);
        uint256 balanceBefore = token.balanceOf(address(this));
        if (balanceBefore < 1000 ether) {
            excludedFromFee[address(token)][vault] = false;
            token.safeTransfer(msg.sender, token.balanceOf(address(this)));
            return;
        }
        token.approve(vault, 1000 ether);
        StakingVaultV6_0_0(payable(vault)).checkFee();
        if (balanceBefore > token.balanceOf(address(this))) {
            excludedFromFee[address(token)][vault] = false;
            token.safeTransfer(msg.sender, token.balanceOf(address(this)));
            return;
        }
        if (balanceBefore == token.balanceOf(address(this))) {
            excludedFromFee[address(token)][vault] = true;
            token.safeTransfer(msg.sender, token.balanceOf(address(this)));
            return;
        }
    }

    function isVaultExcludedFromFee(address token, address vault)
        external
        view
        returns (bool)
    {
        return excludedFromFee[token][vault];
    }
}
