...
contract PlearnFeeManager is Ownable {
    ...
    function processAllFee(bool ignoreError) external onlyOwner {
        ...
        IPlearnFeeHandler.RemoveLiquidityInfo[] memory liquidityList = new IPlearnFeeHandler.RemoveLiquidityInfo[](_getPairCountForProcessFee());
        IPlearnFeeHandler.SwapInfo[] memory swapList = new IPlearnFeeHandler.SwapInfo[](_getPairCountForProcessFee());
        uint liquidityCounter = 0;
        for (uint256 i = 0; i < pairCount; ++i) {
            ...
            (uint amountAMin, uint amountBMin) = _getLiquidityTokenMinAmount(reserve0, reserve1, lpBalance, totalSupply);
            uint plearnAmountOutMin = pair.token0() == address(plearn) ? amountAMin : amountBMin;
            if (plearnAmountOutMin >= minimumPlearn) {
                liquidityList[liquidityCounter] = IPlearnFeeHandler.RemoveLiquidityInfo({
                    pair: pair,
                    amount: lpBalance,
        ...
        for (uint256 i = 0; i < pairCount; ++i) {            
            ...
            if (amountIn > 0) {
                swapList[swapCounter] = IPlearnFeeHandler.SwapInfo({
                    amountIn: amountIn,
                    amountOutMin: 0,
                    path: path
                });
    ...
    function _getPairCountForProcessFee() internal view returns (uint liquidityCount) {
        uint pairCount = getPairCount();
        uint counter = 0;
        for (uint256 i = 0; i < pairCount; ++i) {
            address pairAddress = EnumerableSet.at(_pairs, i);
        ...
        liquidityCount = counter;
    }

    function _getLiquidityTokenMinAmount(uint _reserve0, uint _reserve1, uint _lpBalance, uint _totalSupply) internal view returns (uint _amountAMin, uint _amountBMin) {
        uint tokenAAmountPerLP = _reserve0 / _totalSupply;
        ...
        uint tokenBAmountPerLP = _reserve1 / _totalSupply;
