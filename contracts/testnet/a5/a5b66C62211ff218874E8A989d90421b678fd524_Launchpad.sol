...
contract PresalePool is Ownable, Pausable, ReentrancyGuard {
    ...
    INovationRouter02 public router = INovationRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function initialize(
        ...
        if (_args.maxInvest > 0) {
            maxInvestable = _args.maxInvest;
            require (_args.minInvest <= _args.maxInvest, "!min investable");
        }
        minInvestable = _args.minInvest;
        ...
        publicMode = _whitelist.length > 0 ? false : true;
    ...
    function enableClaim() external onlyTokenOwner {
        ...
        uint toSale = salePrice.mul(raised).div(1e18);
    ...
    function claim() external nonReentrant {
        ...
        uint claimAmount = salePrice.mul(invests[msg.sender]).div(1e18);
    ...
    function multiSend() external onlyTokenOwner {
        ...
        for (uint i = 0; i < investors.length(); i++) {
            ...
            uint claimAmount = salePrice.mul(invests[investor]).div(1e18);
    ...
    function claimable(address _user) external view returns(uint) {
        return salePrice.mul(invests[_user]).div(1e18);
    }
    ...
    function saleAmount() external view returns (uint amount) {
        amount = salePrice.mul(raised).div(1e18);
    }
    ...
    function unclaimed() external view returns (uint) {
        ...
        for (uint i = 0; i < investors.length(); i++) {
            ...
            amount += salePrice.mul(invests[investors.at(i)]).div(1e18);
        }
...
contract Launchpad is Ownable {
    ...
    function addPool(address _pool, string memory _urls) external onlyOwner {
        require (!pools.contains(_pool), "already existing");
        require (bytes(_urls).length <= 700, "invalid social urls size");
