...
contract YTON is Ownable{
	...
	event InvestUsdt(address indexed user, uint256 totalAAmount, uint256 totalBAmount);
    ...
    //IDO
	function investUsdtAndHds(uint256 tokenAAmount,uint256 tokenBAmount) public returns (bool) {

        usdt.transferFrom(msg.sender, address(this),tokenAAmount);
        //90%
        token.transferFrom(msg.sender,address(this),tokenBAmount);
		emit InvestUsdt(msg.sender, tokenAAmount, tokenBAmount);
