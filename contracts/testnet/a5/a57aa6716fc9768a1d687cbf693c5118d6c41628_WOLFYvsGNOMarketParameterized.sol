...
contract WOLFYvsGNOMarketParameterized is Ownable, ReentrancyGuard {
    ...
    IERC20 marketCurrencyency = IERC20(marketCurrency);
    ...
    function changemarketCurrencyency(address _newCurr) external onlyOwner {
    ...
    function predict(bool option, uint256 timeframe, uint256 leverage, uint256 _stake) external returns(uint256 id) {
        ...
        marketCurrencyency.safeTransferFrom(_msgSender(), address(this), _stake);
    ...
    function calcAndDistributePredictionFee(uint256 _fee) private {
        ...
        marketCurrencyency.safeTransferFrom(address(this), operator, operatorDividend);
    ...
    function withdrawPredictionPosition(uint256 id, uint256 amount) external nonReentrant {
        ...
        require(amount > 0 && amount <= predictionBalance[_msgSender()][tf][opt], "withdraw prediction: insufficient balance");
        ...
        if (amount == predictionBalance[_msgSender()][tf][opt]) {  
            delete riskOnOption[_msgSender()][tf][opt];           
            delete predictionBalance[_msgSender()][tf][opt];
            removePrediction(id);
        }
        else {
            riskOnOption[_msgSender()][tf][opt] -= riskOn;
            predictionBalance[_msgSender()][tf][opt] -= amount;
        }
        totalRisk[tf][opt] -= riskOn;
        optionSize[tf][opt] -= sz;
        totalPredicted -= amount;

       // marketCurrencyency.safeTransferFrom(address(this), _msgSender(), amount);
    ...
    function addLiquidity(uint256 amount) external {
        ...
        liquidity += amount;
        marketCurrencyency.safeTransferFrom(_msgSender(), address(this), amount);
    ...
    function withdrawLiquidity(uint256 amount) external nonReentrant {
        ...
        liquidity -= amount;
        lpBalances[_msgSender()] -= amount;
        marketCurrencyency.safeTransferFrom(address(this), _msgSender(), withdrawal);
    ...
    function calcAndDistributeLiquidityWithdrawalFee(uint256 fee) private {
        ...
        marketCurrencyency.safeTransferFrom(address(this), operator, dividend);
        marketCurrencyency.safeTransferFrom(address(this), WPACKStakingManagerAddr, dividend);
    }
    ...
    function notifyWOLFPACKStakingManager(uint256 distribution) private {
        ...
        marketCurrencyency.safeTransferFrom(address(this), WPACKStakingManagerAddr, distribution);
    }
