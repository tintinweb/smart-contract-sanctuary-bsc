...
contract CryptoEnergyCoin is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    uint256 private previousTotalTaxFee;
    ...
    function initialize() public initializer {
        ...
        totalTaxFee = marketingFee + developmentFee + charityFee;
        previousTotalTaxFee = totalTaxFee;
    ...
    function distributeFee() internal {
        ...
        if(contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            if(marketingFeeEnabled) transferBNBToAddress(payable(marketingWalletAddress), (transferredBalance * marketingFee) / totalTaxFee);

            // Send eth to development  
            if(developmentFeeEnabled) transferBNBToAddress(payable(developmentWalletAddress), (transferredBalance * developmentFee) / totalTaxFee);

            // Send eth to Charity address
            if(charityFeeEnabled) transferBNBToAddress(payable(charityWalletAddress), (transferredBalance * charityFee) / totalTaxFee);
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee) = getTValues(tAmount); // 99 ,1
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = getRValues(tAmount, tFee);  // 100, 99, 1
        {
            address from = sender;
            address to = recipient;
            _tOwned[from] = _tOwned[from] - tAmount;
    ...
    function calculateTaxFee(uint256 _amount) internal view returns (uint256) {
        return (_amount * totalTaxFee) / 10**2;
    }

    function takeReflectionFee(uint256 rFee) internal {
        _rTotal = _rTotal - rFee;
    }

    function removeAllFee() internal {
        if(totalTaxFee == 0) return;
    ...
    function restoreAllFee() internal {
        totalTaxFee = previousTotalTaxFee;
    }
