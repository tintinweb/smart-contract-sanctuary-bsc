...
import './interfaces/IProfitSender.sol';
...
contract HuukMarket is Manager, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using Address for address payable;

    uint256 public constant ZOOM_HUUK = 10**18;
	 ...
	 * @param _paymentToken is payment method (USDT, HUUK, BNB, ...)
    ...
    function _match(
        ...
        if (buyerRef != address(0)) {
            uint256 amountToBuyerRef = orderAmount.mul(xUser).mul(ZOOM_FEE - yRefRate).div(ZOOM_FEE**2);
                ...
                if (isFarmingNFTs[order.tokenAddress] && (order.owner != creator) && farmingAmount[order.owner][keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))] > 0) {
                    ...
                    if (a >= _quantity) {
                    ...
                    } else {
                        {
                            ...
                            amountToCreator =
                            amountToCreator +
                            IProfitSender(profitSender).profitSend(
                                order.tokenAddress,
                                _paymentToken,
                                order.tokenId,
                                a,
    ...
    function createOrder(
        ...
        if (
            ...
            IHuukNFT(_tokenAddress).getCreator(_tokenId) == msg.sender &&
        ...
        if (
            isFarmingNFTs[_tokenAddress] && (msg.sender != IHuukNFT(_tokenAddress).getCreator(_tokenId)) && (lastBuyPriceInUSDT[msg.sender][keccak256(abi.encodePacked(_tokenAddress, _tokenId))] == 0)
        ) {
    ...
    function buy(
        ...
        if (_paymentToken == huuk && discountForHuuk > 0) {
            exactPaymentAmount = exactPaymentAmount.sub(orderAmount.mul(discountForHuuk).div(ZOOM_FEE));
        }
    ...
    function acceptBid(uint256 _bidId, uint256 _quantity) external returns (bool) {
        Bid memory bid = bids[_bidId];
...
contract Manager is Ownable, Pausable {
    ...
    uint256 public discountForHuuk = 100; // discount for user who made payment in huuk
    ...
    function setSystemFee(
        ...
        _setSystemFee(_xUser, _xCreator, _yRefRate, _zProfitToCreator, _discountForBuyer, _discountForHuuk);
    }
    ...
    function setHuukExchangeContract(address _huukExchangeContract) public onlyOwner returns (bool) {
        huukExchangeContract = _huukExchangeContract;
        return true;
    }

    function setProfitSenderContract(address _profitSender) public onlyOwner returns (bool) {
