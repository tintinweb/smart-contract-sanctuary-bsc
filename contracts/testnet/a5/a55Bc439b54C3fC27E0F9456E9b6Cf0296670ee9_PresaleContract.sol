...
contract PresaleContract is Ownable, ReentrancyGuard {
    ...
    {
        require(
            timestampStarts[round - 1] > 0,
            "Contract has not started yet."
        );

        uint256 currentRound = this.getCurrentRound();

        if (round > currentRound) return 0; // TODO: fix loi bi hoai khi round chua activate chưa test
        ...
        if (
            round > 1 &&
            block.timestamp - timestampStarts[round - 1] >=
            tokenReleaseRounds[round - 2]
        ) {
            currentReleaseMonth =
                (block.timestamp -
                    timestampStarts[round - 1] -
                    tokenReleaseRounds[round - 2]) /
                oneMonth;
        }
        ...
        if (round > 1)
            maxMonths =
                (tokenReleaseRounds[round - 1] -
                    tokenReleaseRounds[round - 2]) /
                oneMonth;
        else maxMonths = tokenReleaseRounds[round - 1] / oneMonth;
    ...
    function getRoundIsFinished(uint256 round) external view returns (bool) {
        ...
        if (
            timestampStarts[round - 1] + roundLengths[round - 1] <=
            block.timestamp
        ) return true;
    ...
    function start() external onlyOwner {
        require(timestampStarts[0] <= 0, "Presale started");
    ...
    /* ========== MODIFIERS ========== */
    modifier isStarted() {
        require(timestampStarts[0] > 0, "The round has not started yet");
