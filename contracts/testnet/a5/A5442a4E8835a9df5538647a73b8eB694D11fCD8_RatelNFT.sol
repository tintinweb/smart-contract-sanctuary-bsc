...
contract RatelNFT is ERC721A, VRFConsumerBase, Ownable, ReentrancyGuard {
  ...
  uint internal constant FIVE_DAY_SECONDS = 300; //TODO，上线改为 432000; //幼兽成熟时间是5天

  // price for pre sale stage (0.2 BNB)
  uint internal preSalePrice =20 * 10**14; //TODO，上线改为  20 * 10**16;

  // price for publicsale stage (0.25 BNB)
  uint internal publicSalePrice = 25 * 10**14;  //TODO，上线改为 25 * 10**16;

  // fee for take grownup rate (0.01 BNB)
  uint internal takeGrownupFee = 1 * 10**16;

  // fee for breed , 游戏币 
  uint internal breedFee = 125000;

  // max airdrop  quota allowed
  // uint internal maxAirdropCount = 1000;
  ...
  // flag for pre sale
  bool internal preSale = true; //false
  ...
  uint public randomResult; //private
  bytes32 requestId_;
  //tokenId与随机数的映射
  ...
  IERC20 internal ratelGameToken= IERC20(_ratelGameTokenAddress);//游戏代币合约
   ...
   //构造方法，先部署游戏代币合约及市场合约,每次交易只能铸造10张
    constructor() 
  ...
  */
  modifier mintable(uint _quantity) {
      if(publicSale) {
  ...
  /**
    ...
    * @notice only owner can call this method
    */
  function togglePreAndPublicSale(uint flag) public onlyOwner returns (bool) {
   ...
   */
  function _mint(MintMode mode, address _to, uint _level, uint _quantity, uint babyId) internal {
    //TODO 必须给合约转1个link
    require(randomResult>0, "randomResult error");

    if (mode == MintMode.AIRDROP) {
      _totalAirdrop += _quantity;
    ...
    for (uint i=0; i< _quantity; i++) {
      tokenId = _generateCharacters(mode, _level, babyId);
    }
  ...
  function _generateCharacters(MintMode mode, uint level, uint babyId) 
    internal 
    returns(uint)
  {
   //从0开始 
    ...
    } else if (mode == MintMode.AIRDROP){
        randomResult = uint(keccak256(abi.encode(block.timestamp, randomResult)));
        base = (level+1) * 20;
    ...
    // randomResult = uint(keccak256(abi.encode(block.timestamp, randomResult)));
    uint strength = uint(keccak256(abi.encode(randomResult, 1))) % 20 + base ;// 力量
    uint dexterity = uint(keccak256(abi.encode(randomResult, 2))) % 20 + base ; //敏捷
    uint luck = uint(keccak256(abi.encode(randomResult, 3))) % 20 + base ;     //运气
    uint defense = uint(keccak256(abi.encode(randomResult, 4))) % 20 + base ; //防御
    uint wisdom = uint(keccak256(abi.encode(randomResult, 5))) % 20 + base ;  //智慧
    uint attack = uint(keccak256(abi.encode(randomResult, 6))) % 20 + base ;   //攻击
  ...
  //铸造方法，要根据预售或公开销售状态支付不同价格的BNB, 当同一区块内或游戏online之后，无法调用此方法
    ...
    mintable(_quantity) 
  {
    ...
    if (getMintMode() == MintMode.PRESALE) {
      require(_quantity<=2, "Max two can mint in one block");
    ...
    return _mint(getMintMode(), _msgSender(), 0, _quantity, 0);

  }

  function mintA(uint256 quantity) external payable {
    // _safeMint's second argument now takes in a quantity, not a tokenId.
    _safeMint(msg.sender, quantity);
  }

  //空投, 只允许部署者调用,指定属性及数量
  function airdrop(uint level, address[] memory receivers, uint[] memory counts)
    external
    onlyOwner
    returns (bool) {
    require(level > 0, "level must be 1 or 2 or 3");
    ...
    for(uint i = 0; i < receivers.length; i++) {
        _mint(MintMode.AIRDROP, receivers[i], level, counts[i], 0);
    }
    return true;
  }
  ...
  function getRatelFamily(uint tokenId) public view returns(uint, uint, uint) {
     return  (
  ...
  //获取用户拥有的幼兽总数
  function getBabyOwnCount(address addr) public view returns(uint) {
  ...
  //获取繁殖需要的HRGT代币
  function getBreedFee(uint maleTokenId, uint femaleTokenId) public view returns(uint){
  ...
  function breed(uint maleTokenId, uint femaleTokenId)
    ...
    // step 2, 检测雌雄的父母是否相同 
    //雄性的父亲
    uint father1 = parents[maleTokenId][FATHER];
    //雌性的父亲
    uint father2 = parents[femaleTokenId][FATHER]; 
    require(father1 != father2, "Same father error.");

    //雄性的母亲
    uint mother1 = parents[maleTokenId][MOTHER];
    //雌性的母亲
    uint mother2 = parents[femaleTokenId][MOTHER];
    ...
    online
  {
    ...
    _mint(MintMode.BREED, _msgSender(), 0, 1, babyId);
  ...
  */
  ...
  //撤单
  function cancelOrder(uint itemId) public nonReentrant returns (bool) {
      require(itemId <= _itemIds.current(), "item not exists" );
