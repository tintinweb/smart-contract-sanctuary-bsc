...
// AUCTION TOKEN: HOTBODY, commit token: TBNB (-> WBNB)
...
//contract BatchAuction {
  ...
  function finalizeAuction() public onlyOwner {
    ...
    if (isAuctionSuccessful()) {
      ...
      //auctionData.totalLPTokenAmount = setupBalancerPool(
      //  AUCTION_TOKEN, token80Amount, COMMIT_CURRENCY, token20Amount);
      // TODO
      // BAL BPT -> Pancake Swap BPT
      auctionData.totalLPTokenAmount = 0;

      require(auctionData.totalLPTokenAmount > 0, "INVALID_AMOUNT");

      emit AuctionFinalizedWithSuccess(transferAmount, token80Amount, token20Amount, auctionData.totalLPTokenAmount);
