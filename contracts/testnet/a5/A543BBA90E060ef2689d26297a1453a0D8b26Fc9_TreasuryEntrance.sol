...
/**
@dev this contract used for TreasuryEntrance 
 */
contract TreasuryEntrance is AccessControl, Initializable {
    using SafeERC20 for IERC20;

    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE"); // byte role for owner
     ...
     */
    function initialize(address _owner, address _teamAddress,address _Admin) external initializer{
        owner = _owner;
        Admin= IAdmin(_Admin);
    ...
    /**
    @dev this function used to withdraw whole amount to owner address 
     */
    function withdraw() external onlyRole(OWNER_ROLE){
        IERC20(Admin.BUSD()).transfer(teamAddress,IERC20(Admin.BUSD()).balanceOf(address(this)));
    }
