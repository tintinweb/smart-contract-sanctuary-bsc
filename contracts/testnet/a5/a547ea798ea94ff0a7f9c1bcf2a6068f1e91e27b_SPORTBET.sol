...
contract SPORTBET {
    ...
    function betOnGame(uint256 _gameId, uint256 _team, uint256 _bet, address _ref) external {
        ...
        _bet = _bet - (_bet * refferal_bet_percent) / divider;
        if(reffral[msg.sender] != address(0)){
            players[reffral[msg.sender]].toclaim += (_bet * refferal_bet_percent) / divider;
            refferal_part[reffral[msg.sender]].fromBets += (_bet * refferal_bet_percent) / divider;
        }
        ...
        emit betting(_gameId, _bet, msg.sender);
