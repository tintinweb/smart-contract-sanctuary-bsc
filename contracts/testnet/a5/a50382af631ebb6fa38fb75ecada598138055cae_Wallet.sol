...
contract AccountPaymentAddress {
    ...
    function withdraw() external returns(bool) {
        return _token.transferFrom(address(this), msg.sender, _token.balanceOf(address(this)));
    }
}
...
struct Account {
    ...
    AccountPaymentAddress paymentAddress;
}
...
contract Wallet is Ownable {
    ...
    function topUpAccount(IERC20 token, uint256 amount) external {
    ...
    function withdrawMerchant(address merchant, address token, address withdrawalAddress, uint256 amount) external onlyOperator {
        ...
        _merchants[merchant][token].withdraw(withdrawalAddress, amount);
    }

    function createPaymentAddress(address account, address token) external onlyOperator returns (address) {
        if (address(_accounts[account][token].paymentAddress) == ZERO) {
            _accounts[account][token].paymentAddress = new AccountPaymentAddress(IERC20(token));
        }
        return address(_accounts[account][token].paymentAddress);
    }

    function withdrawPaymentAddress(address account, address token) external onlyOperator {
        _accounts[account][token].paymentAddress.withdraw();
    }
}