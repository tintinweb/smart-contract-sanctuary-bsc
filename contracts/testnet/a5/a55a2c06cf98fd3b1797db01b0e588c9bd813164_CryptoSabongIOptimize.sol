...
// The address of the player and => the user info
   ...
   function() public payable {}
    ...
    function testAllocate1(uint96 _gameId, uint96 teamWinner)public onlyAgent{
        Game storage game = gameInfo[_gameId];
        require(bettingActive == false);
        address[] memory winners = players;
        address[] memory playersAdd = players;
     ...
     function testAllocate2(uint96 _gameId, uint96 teamWinner)public onlyAgent{
        Game storage game = gameInfo[_gameId];
        require(bettingActive == false);
        address[] memory winners = players;
        address[] memory playersAdd = players;
    ...
    function testAllocateMeron(uint96 _gameId)public onlyAgent{
        Game storage game = gameInfo[_gameId];
        require(bettingActive == false);
        address[] memory playersAdd = players;
        //We have to create a temporary in memory array with fixed size
        ...
        for(i = 0; i < playersAdd.length; i++){
            ...
            //If the player selected the winner team
            //We add his address to the winners array
            if(playerInfo[playerAddress].teamSelected == 1){
                totalBets = totalBetsOne + totalBetsTwo;
                ads =  (totalBets * fee) / totalBetsOne;
                bets = playerInfo[playerAddress].amountBet;
                rewards = (bets * ads) / 10000;
                _pendingBalance[playerAddress] += rewards;
            }
            playerInfo[playerAddress]._state = PlayerStatus.Not_Joined;
        }
