...
contract KingToken is ERC20, Ownable {
    ...
    function setMarketingWalletAddress(address tempAddress_)
        external
        onlyOwner
    {
        _marketingWalletAddress = tempAddress_;
    }
    ...
    constructor(
        ...
        address kswappAddress_,
        address usdtAddress_
    ) payable ERC20("King Token", "K") {
        ...
        uint256 tokenBalanceForReward_ = 1;
    ...
    function _transfer(
        ...
        bool canSwap = AmountMarketingFee > swapTokensAtAmount ||
            AmountLiquidityFee > swapTokensAtAmount ||
            AmountTokenRewardsFee > swapTokensAtAmount;
        if (
            canSwap &&
            !swapping &&
    ...
    function swapAndSendToFee() private {
        uint256 tokens = balanceOf(address(this)).sub(AmountLiquidityFee).sub(
        ...
        IERC20(rewardToken).transfer(_marketingWalletAddress, tokens);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        IERC20(address(this)).transfer(address(uniswapV2Pair), tokens);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _approve(usdtAddress, address(uniswapV2Router), ethAmount);
        // add the liquidity
