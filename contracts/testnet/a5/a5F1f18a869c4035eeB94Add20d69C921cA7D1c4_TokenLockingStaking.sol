...
contract TokenLockingStaking is TokenWrapper, OwnableUpgradeable {
    ...
    function outputParams() public view returns (uint256, uint256) {
        uint256 ratio = fffExchange.fff2ffftRatio();
        return ((totalOutput - rewardsGloble) * ratio / 1e18, rewardsGloble * ratio / 1e18);
    }

    function getAPY() public view returns (uint256[3] memory) {
        uint256[3] memory weigth = getWeight();
        ...
        for(uint i=0; i< weigth.length; i++){
            if (_totalSupply == 0) {
                ...
                apys[i] = weigth[i] * dayOut * 365 / _totalSupply;
            } 
        }
        return apys;
    }

    function getWeight() public view returns (uint256[3] memory) {
