...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
	...
	constructor(
        ...
        IERC721 _contract_ceekland, address _contract_decoder, address _swapRouter,
    ...
    function getPriceInCurrency(uint256 tokenId, EIP20 payment_token) public view returns (uint256) {
        ...
        if (payment_token == contract_payment_token) {
            ...
            //bytes memory resultGetAmountsOut = Address.functionStaticCall(swapRouter, abi.encodeWithSignature("getAmountsOut(uint256,address[])", 1000000000000000000, [address(payment_token), address(contract_payment_token)]));
            //(uint256[] memory exRates) = abi.decode(resultGetAmountsOut, (uint256[]));
            //token_price = sellBidPrice[tokenId].mul(exRates[1]).div(1000000000000000000);
            token_price = sellBidPrice[tokenId].mul(exRate).div(1000000000000000000);
    ...
    function buy(uint256 tokenId, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        ...
        require(ptOk1, "CEEKLANDGATE_030");
        ...
        require(ptOk1, "CEEKLANDGATE_031");
    ...
    function updateAdmin(EIP20 _payment_token, EIP20[] memory _payment_tokens, IERC721 _contract_ceekland,
                         ...
                         address _wbnbToken, address _swapRouter, uint256 _exRate) public {
    ...
    function changeExRate(uint256 _exRate) public {
        require(_msgSender()==contract_owner, "CEEKLANDGATE_024");
        exRate = _exRate;
    }
