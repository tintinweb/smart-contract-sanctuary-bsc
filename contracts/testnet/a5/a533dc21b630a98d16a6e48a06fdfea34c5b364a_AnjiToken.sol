...
contract BNBDistributor is IDividendDistributor {
    ...
    event ShareUpdated(address account, uint256 newShare, uint256 oldShare);
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        ...
        emit ShareUpdated(shareholder, amount, oldShare);
    }
...
contract AnjiToken is Context, IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!isDividendExempt[sender]) {
            bnbDistributor.setShare(sender, _balances[sender]);
        }

        if(!isDividendExempt[recipient]) {
            bnbDistributor.setShare(recipient, _balances[recipient]);
        }
