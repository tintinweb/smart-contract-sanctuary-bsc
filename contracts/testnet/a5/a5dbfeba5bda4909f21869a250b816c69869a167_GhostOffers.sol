...
contract GhostOffers is Auth, ERC721Holder {
    ...
    function removeNFTOffer(
        ...
        if (_offerIndex >= _nftOffersByCollection[_nftAddress][_nftId].length)
            return;
    ...
    function rejectOffer(
        ...
        if (isToken) {
            removeTokenOffer(_nftAddress, _nftId, _offerIndex, _offerId);
        } else {
            removeNFTOffer(_nftAddress, _nftId, _offerIndex, _offerId);
        }
    }
    ...
    function createNFTOffer(
        ...
        IERC721 offeredNftAddress = IERC721(_offeredNFT);
        IERC721 desiredNftAddress = IERC721(_nftAddress);

        require(
            isAcceptableNFT(_offeredNFT),
            "You can not offer this token currently."
        );
        require(
            isAcceptableOfferedNFT(_nftAddress),
        ...
        for (uint256 i = 0; i < _offeredIds.length; i++) {
            require(
                offeredNftAddress.ownerOf(_offeredIds[i]) == msg.sender,
                "You do not own one of the nft's you are offering."
            );
            offeredNftAddress.transferFrom(
                msg.sender,
                address(this),
                _offeredIds[i]
            );
