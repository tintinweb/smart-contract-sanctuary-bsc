...
contract Trusteeship is Ownable, ReentrancyGuard {
    ...
    function retrieve(address _user, address _nftToken, uint256 _tokenId) public nonReentrant onlyAdmin {
        require((_user != address(0)) && (_nftToken != address(0)), "NONEMPTY_ADDRESS");
        require((trustList[_nftToken][_tokenId] == _user), "TOKEN_IS_NOT_USER'S");

        IERC721(_nftToken).transferFrom(address(this), _user, _tokenId);
        trustList[_nftToken][_tokenId] = address(0);

        emit Retrieve(_user, _nftToken, _tokenId, block.timestamp);
    }
}