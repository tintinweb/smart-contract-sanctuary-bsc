...
contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  function requestRandomWords() external onlyAuthorized returns (uint256) {
    if(newRequestNeeded()){
  ...
  function fulfillRandomWords(
    ...
    randomNumber[requestId] = randomWords;
  }

  function newRequestNeeded() public view returns (bool){
      if(s_requestId == 0){
          return true;
      }
      return randomNumber[s_requestId][0] != 0 || block.number >= lastRequestBlock + requestConfirmations;
  }
  ...
  function getResult(uint256 requestId) external view returns (uint256){
      return randomNumber[requestId][0];
  }
}
