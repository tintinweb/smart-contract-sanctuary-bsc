    ...
    ReentrancyGuard
{
    ...
    using MediaEyeOrders for MediaEyeOrders.ListingInput;
     ...
     */
    constructor( MediaEyeOrders.ListingConstructor memory _listingConstructor ) {
        require(_listingConstructor._treasuryWallet != address(0));
        require(_listingConstructor._basisPointFee <= 500, "Max fee");

        _setupRole(DEFAULT_ADMIN_ROLE, _listingConstructor._owner);
        for (uint256 i = 0; i < _listingConstructor._admins.length; i++) {
            _setupRole(ROLE_ADMIN, _listingConstructor._admins[i]);
        }

        treasuryWallet = _listingConstructor._treasuryWallet;
        feeContract = _listingConstructor._feeContract;

        basisPointFee = _listingConstructor._basisPointFee;
        mediaEyeMarketplaceInfo = IMarketplaceInfo(_listingConstructor._mediaEyeMarketplaceInfo);

        chainlink = _listingConstructor._chainlink;
    ...
    function updateConstantsByAdmin(
        ...
        subscriptionCheckActive = _subscriptionCheck;
     ...
     *
     */
    function setChainlink(MediaEyeOrders.Chainlink memory _chainlink) external {
        require(
            hasRole(ROLE_ADMIN, msg.sender) ||
                hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "admin"
     ...
     */
    function createListing(MediaEyeOrders.ListingInput memory _listingInput)
        external
        payable
        nonReentrant
    {
    ...
    function _sendPayments(
        ...
        if (IERC165(_tokenAddress).supportsInterface(type(IMediaEyeSplitInfo).interfaceId)) {
            split = IMediaEyeSplitInfo(_tokenAddress).getSplitInfo();
            if (split.royaltyBasisPoint > 0) {
                payoutToCreator = (price * split.royaltyBasisPoint) / 10000;
            }
        }
        else if (IERC165(_tokenAddress).supportsInterface(type(IERC2981).interfaceId)) {
            (royaltyArtist, payoutToCreator) = IERC2981(_tokenAddress)
                .royaltyInfo(_tokenId, price);
            split.royaltyArtist = payable(royaltyArtist);
        }

        // payout to Charity/sellers
        uint256 remainingPayout = (price - payoutToTreasury - payoutToCreator);
        if (split.charityBasisPoint > 0 && split.charity != address(0)) {
            payoutToCharity =
                (remainingPayout * split.charityBasisPoint) /
                10000;
        }
        if (split.splitBasisPoint > 0 && split.splitRecipient != address(0)) {
        ...
        uint256 payoutToSeller = (remainingPayout *
            (10000 - split.charityBasisPoint - split.splitBasisPoint)) /
            10000;
