...
contract GovernanceRedemptionPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant SUPPLY_PRECISION = 1 ether;
    ...
    function initialize(address _lgo, address _lgoView, address _pool, address _weth, address _ethUnwrapper)
        external
        initializer
    {
        ...
        require(_pool != address(0), "GovernanceRedemptionPool::initialize: invalid address");
        require(_weth != address(0), "GovernanceRedemptionPool::initialize: invalid address");
        require(_ethUnwrapper != address(0), "GovernanceRedemptionPool::initialize: invalid address");
    ...
    modifier onlyRedemptionActive() {
        require(isRedemptionActive(), "GovernanceRedemptionPool::onlyRedemptionActive: redeemption is not active");
        _;
    }
    ...
    function startNextBatch() external onlyRole(CONTROLLER_ROLE) {
        require(liquidTokens.length > 0, "GovernanceRedemptionPool::startNextBatch: no liquid tokens");
    ...
    function redeem(address _to, uint256 _amount) external onlyRedemptionActive nonReentrant {
        (address[] memory _tokens, uint256[] memory _claims) = getUserClaims(_amount);
        for (uint256 i = 0; i < _tokens.length;) {
            if (_claims[i] != 0) {
                if (_tokens[i] == address(weth)) {
                    treasury.transfer(_tokens[i], address(this), _claims[i]);
                    _safeUnwrapETH(_claims[i], _to);
                } else {
                    treasury.transfer(_tokens[i], _to, _claims[i]);
                }
            }
            unchecked {
                ++i;
        ...
        emit Redeemed(msg.sender, _to, _amount);
    }
        ...
        nonReentrant
    {
        ...
        uint256 _amountOut;
        for (uint256 i = 0; i < _tokens.length;) {
            uint256 _claimableAmount = _claims[i];
            if (_tokens[i] != _tokenOut) {
                uint256 _beforeBalance = IERC20(_tokenOut).balanceOf(address(this));
                treasury.transfer(_tokens[i], address(pool), _claimableAmount);
                pool.swap(_tokens[i], _tokenOut, 0, address(this), abi.encode(_to));
                _amountOut += IERC20(_tokenOut).balanceOf(address(this)) - _beforeBalance;
            } else {
                treasury.transfer(_tokens[i], address(this), _claimableAmount);
                _amountOut += _claimableAmount;
            }
            unchecked {
                ++i;
            }
        }
        require(_amountOut >= _minimumAmountOut, "GovernanceRedemptionPool::redeemToToken: !slippage");

        if (_tokenOut == address(weth)) {
            _safeUnwrapETH(_amountOut, _to);
        } else {
        ...
        emit RedeemToToken(msg.sender, _to, _amount, _tokenOut, _amountOut);
    }

    // =============== VIEW FUNCTIONS ===============

    function getUserClaims(uint256 _lgoAmount)
        public
        view
        returns (address[] memory _tokens, uint256[] memory _balances)
    {
        if (isRedemptionActive() && snapshot.lgoSupply > 0 && _lgoAmount <= snapshot.lgoSupply) {
            ...
            _balances = new uint256[](snapshot.balances.length);
            for (uint256 i = 0; i < snapshot.balances.length;) {
                _balances[i] = _lgoAmount * snapshot.balances[i] / snapshot.lgoSupply;
                unchecked {
                    ++i;
                }
            }
    ...
    function getSnapshot() external view returns (Snapshot memory _snapshot) {
        return snapshot;
    }

    function getNextSnapshot() public view returns (Snapshot memory _snapshot) {
        (address[] memory _tokens, uint256[] memory _balances) = getTreasuryBalances(liquidTokens);
        _snapshot = Snapshot({
    ...
    function getTreasuryBalances(address[] memory _tokens) public view returns (address[] memory, uint256[] memory) {
        uint256[] memory _balances = new uint256[](_tokens.length);
        for (uint256 i = 0; i < _tokens.length;) {
            _balances[i] = IERC20(_tokens[i]).balanceOf(address(treasury));
            unchecked {
                ++i;
            }
        }
    ...
    function setTreasury(address _treasury) external onlyRole(ADMIN_ROLE) {
        require(_treasury != address(0), "GovernanceRedemptionPool::setTreasury: invalid address");
        treasury = ITreasury(_treasury);
        emit TreasurySet(_treasury);
    }

    function setPool(address _pool) external onlyRole(ADMIN_ROLE) {
        require(_pool != address(0), "GovernanceRedemptionPool::setPool: invalid address");
        pool = IPool(_pool);
        emit PoolSet(_pool);
    }

    function setEthUnwrapper(address _ethUnwrapper) external onlyRole(ADMIN_ROLE) {
        require(_ethUnwrapper != address(0), "GovernanceRedemptionPool::setEthUnwrapper: invalid address");
        ethUnwrapper = IETHUnwrapper(_ethUnwrapper);
        emit EthUnwrapperSet(_ethUnwrapper);
    }
    ...
    event RedemptionStoped();
    event LiquidTokensUpdated();
    event TreasurySet(address indexed _addr);
    event PoolSet(address indexed _addr);
    event EthUnwrapperSet(address indexed _addr);
    ...
    event Redeemed(address indexed _from, address indexed _to, uint256 _amount);
    event RedeemToToken(
