...
contract fBurnStake is Ownable, ReentrancyGuard {
    ...
    address nftTokenAddress = 0x328fA466A35635bC428D184258b995d808dE7187;
    ...
    function getUnclaimedRewards(uint256 _stakeId) public view returns (uint256){
        ...
        uint unclaimedRewards = (applicableRewards * getElapsedTime(_stakeId, stakePoolId));

        return unclaimedRewards; 
    }

    function getElapsedTime(uint256 _stakeId, uint256 _stakePoolId) public view returns(uint256){
        userStake memory userStakeDetails = userStakesById[_stakeId];
        uint256 lapsedDays;
    ...
    //Testing function
    function testSetLockTime(uint256 _stakeId, uint256 _days) nonReentrant unpaused external returns (bool){
        userStake memory userStakeDetails = userStakesById[_stakeId];
