...
contract PurchasaBNBEscrow is Ownable {
    ...
    IBEP20 public tokenPCA = IBEP20(0xe700BA35998faD8e669e3CCa7b3A350F1FDcACF8);
    ...
    uint256 ESCROW_TIME = 14 days;
    uint256 public commission = 5;
 

    function lockBNBToken(uint256 amount, address reciever,string memory orderId) payable public {
        require(msg.sender.balance >= amount, "Insufficient Balance");
        require(msg.value == amount, "Invalid Balance");
        ...
        escrowOrderList.end = currentTime + ESCROW_TIME;
        escrowOrderList.orderId = orderId;
        escrowOrderList.totalCommission = ((amount / 100) * commission);
        escrowOrderList.isDispute = false;
        Escrow memory orderData = Escrow(amount,reciever,msg.sender,currentTime,currentTime + ESCROW_TIME,orderId,((amount / 100) * commission),false);
        userOrders.push(orderData);
    }


    function payoutBNB() public {
        uint256 totalAmount = 0;
        uint256 currentTime = block.timestamp;
        for(uint i=0; i < userOrders.length; i++){
            if(userOrders[i].reciever == msg.sender && currentTime >= userOrders[i].end && escrowList[userOrders[i].orderId].isDispute == false) {
    ...
    function disputeBNBOrder(string memory orderId) public view returns (bool) {
         uint256 currentTime = block.timestamp;
    ...
    function ownerChangeCommission(uint256 commissions) public onlyOwner{
