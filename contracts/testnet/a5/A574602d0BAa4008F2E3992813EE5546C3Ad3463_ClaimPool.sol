...
contract ClaimPool is IClaimPool, Adminable, ERC165Upgradeable, ReentrancyGuardUpgradeable {
    ...
    event TransferredReward(address indexed collectionAddress, address indexed rewardAddress, uint256 totalAmount);
    ...
    function transferReward(
        ...
        uint256 _totlAmount
    ) external onlyAdmin notZeroAddress(_collectionAddress) {
        require(_totlAmount > 0 && collectionClaimPool[_collectionAddress] >= _totlAmount, "Invalid amount");
        collectionClaimPool[_collectionAddress] = collectionClaimPool[_collectionAddress] - _totlAmount;
        TransferHelper._transferToken(paymentToken, _totlAmount, address(this), projectOwner);

        emit TransferredReward(_collectionAddress, address(this), _totlAmount);
    }

    function remove(address _collectionAddress) external onlyProject notZeroAddress(_collectionAddress) {
