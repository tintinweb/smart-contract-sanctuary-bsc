...
abstract contract CBB is IERC20, Ownable {
    //用于存储每个地址的余额数量
    ...
    constructor (string memory Name, string memory Symbol, uint8 Decimals, uint256 Supply, address FundAddress, address DividendAddress){
        ...
        usdt = address(0x9f37055A4BA925E0678E7447e2164CD833bf3F18);
        ...
        IERC20(usdt).approve(address(this), MAX);
    ...
    uint256 public CBBS = 1 ether;
    uint256 public startTime = block.timestamp;
    uint256 public daysPassed;

    function adjustPrice() public {
        // require(msg.sender == wallet, "Only owner can adjust price.");

        uint256 elapsed = block.timestamp - startTime;
        daysPassed = elapsed / oneDay;
        price = daysPassed;
    }

    function buyTokens(address from, uint256 num) public {        
        // 从购买者账户中扣除相应的USDT代币
        // IERC20(USDT).transferFrom(_msgSender(), address(this), amount);
        // 支付USDT
        // IERC20(usdt).approve(msg.sender,MAX);
        IERC20(usdt).transferFrom(from, address(this), num * price );

        // 将代币发送到购买者账户
        transfer(from,num * CBBS);
    }
...
contract AddUsdtLP is CBB {
    ...
    //总量 10亿
        10 * 10 ** 8,
    //营销钱包
        address(0x8C5728a98F16d04B88A18f574fAD74f566f423eD),
    //分红钱包,
        address(0x8C5728a98F16d04B88A18f574fAD74f566f423eD)
    ){
