...
/**
Xol Foundation $XOL
Restoring Trust in Blockchain Security

Xol adds a new layer of blockchain transaction security, shielding selected projects and communities from malicious exploits and financial loss

https://t.me/xolfoundation
https://xolfoundation.com/
*/
...
contract XolFoundation is Context, IBEP20, Ownable {
    ...
    string private constant _name = "Xol Foundation";
    string private constant _symbol = "XOL";
    ...
    constructor (address marketingAddress, address liquidityAddress, address buybackContract) {
        ...
        _isExcludedFromFee[buyback] = true;
    ...
    function changeFee(uint256 _buyMarketingFee, uint256 _buyLiquidityFee, uint256 _buyBurnFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee, uint256 _sellBurnFee) public onlyOwner {
        require(_buyMarketingFee + _buyLiquidityFee < 20 || _sellLiquidityFee + _sellMarketingFee < 20, "Can't change fee higher than 19%");
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if(from != uniswapV2Pair){
                if(balanceOf(address(buyback)) > 1 * 10**_decimals) {
                    IBuyback(buyback).swapTokenForETH(address(this), balanceOf(address(buyback)));
                }
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxnsAmount");
