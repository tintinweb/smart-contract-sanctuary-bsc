...
contract TRACEDIGITAL is Context, IERC20, Ownable {
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        uint256 treasuryFee = amount  * _treasuryFee / 100;
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            ...
            payable(_treasurryWallet).transfer(ethToSend);
    }

    function swapAndLiquify() private {
        // split the contract balance into halves
        uint256 liquidityTokens = balanceOf (address(this)) *_treasuryFee + _liquidityFee / (_treasuryFee + _marketingFee + _liquidityFee);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            ...
            payable(_treasurryWallet).sendValue (ethAmount - ethFromLiquidity);
    }
}