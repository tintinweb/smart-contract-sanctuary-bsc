...
contract QuickLiquidityProvider is DexBase, IQuickLiquidityProvider, Initializable, ReentrancyGuardUpgradeable {
    ...
    function swapRequest(bytes calldata message, bytes calldata signature) external payable nonReentrant onlyBabtUser isCircuitBreaker {
        ...
        secure.freezePending(lpOut, params.compensateAmount);
    ...
    function _settle(string calldata quoteId) internal {
        ...
        require(quoteStorage[quoteId].status == 0, "settlement error, status is not active");
    ...
    function _removeRequest(string memory quoteId) internal {
        quoteStorage[quoteId].status = 2;
    }

    function _declineRequest(string memory quoteId) internal {
        quoteStorage[quoteId].status = 1;
    }
...
contract SecurePool is ReentrancyGuard {
   ...
   function queryPending(address lpOut) external view returns (uint256 amount) {
    ...
    function freezePending(address lpOut, uint256 amount_) external nonReentrant onlyDex {
