...
contract LPChefV4 is Ownable {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        IERC20(aimMicPair).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(
        ...
        IERC20(aimMicPair).safeTransfer(msg.sender, _amount);
    ...
    function calcMicWorth(uint256 lpAmount) public view returns (uint256) {
        uint256 aimMicLpTotalSupply  = IERC20(aimMicPair).totalSupply();
        (uint112 _aimMicReserve0, uint112 _aimMicReserve1,) = IPancakePair(aimMicPair).getReserves();
        uint256 aimAmount = lpAmount.mul(uint256(_aimMicReserve1)).div(aimMicLpTotalSupply);

        (uint112 _aimUsdtReserve0, uint112 _aimUsdtReserve1,) = IPancakePair(aimUsdtPair).getReserves();
        uint256 aimPrice = uint256(_aimUsdtReserve0).mul(1e18).div(_aimUsdtReserve1);
        return aimAmount.mul(aimPrice).div(1e18);
    }
