...
contract XaveMarket is AccessControlEnumerable, Ownable, ReentrancyGuard {
    ...
    // Stores a counter of overriden prices per currency, to save on gas calculations
    // nftContract =>  EnumerableMap( key = currency, value = counter)
    mapping(address => EnumerableMap.AddressToUintMap)
        private _overridenPricesCounter;
    ...
    constructor(address[] memory _validCurrencies) {
        ...
        for (uint256 i = 0; i < _validCurrencies.length; i++) {
            require(
                Address.isContract(_validCurrencies[i]),
    ...
    function buyToken(
        ...
        uint256 _tokenId,
        address _currency
    ) external onlyIfNftExists(_nft) nonReentrant {
        ...
        require(price > 0, "No price found for the currency");
        ...
        bool isApproved = (erc721.isApprovedForAll(tokenOwner, address(this)) ||
            erc721.getApproved(tokenId) == address(this));
        require(isApproved, "Need approval from token owner");

        IERC20 erc20 = IERC20(currency);
        require(
            erc20.allowance(sender, address(this)) >= price,
            "Need more allowance from sender"
        );

        // transfer (currency  price) from sender to this contract
        erc20.transferFrom(sender, address(this), price);
        // transfer tokenId from owner to senderª
        erc721.safeTransferFrom(tokenOwner, sender, tokenId);

        emit NftPurchase(sender, nft, tokenId);
    }

    function addValidCurrency(address currency) external onlyRole(NFT_ADMIN) {
        require(Address.isContract(currency), "Invalid currency address");
        require(
    ...
    function addNft(address nft) public onlyRole(NFT_ADMIN) {
        require(Address.isContract(nft), "Invalid nft address");
    ...
    //Also removes the overriden prices attached to the tokenIds[], if any
        ...
        onlyIfNftExists(nft)
    {
        ...
        uint256 nftOverridenCount = _overridenPricesCounter[nft].length();
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (nftOverridenCount > 0) {
                uint256 counter;
                address currency;
                for (uint256 j = 0; j < nftOverridenCount; j++) {
                    (currency, counter) = _overridenPricesCounter[nft].at(j);
                    if (_overridenPrices[nft][currency][tokenIds[i]] > 0) {
                        // remove overiden price
                        _overridenPrices[nft][currency][tokenIds[i]] = 0;
                        // decrease the counter
                        _overridenPricesCounter[nft].set(currency, counter - 1);
                    }
                }
            }
    ...
    function overridePrice(
        ...
        //if overriding a price that was > 0, do not count it to increase the counter
        uint256 prevPriceCounter;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(amounts[i] > 0, "Amount cannot be 0");
            if (_overridenPrices[nft][currency][tokenIds[i]] > 0) {
                prevPriceCounter++;
        ...
        // Increase counter
        (, uint256 counter) = _overridenPricesCounter[nft].tryGet(currency);
        _overridenPricesCounter[nft].set(
            currency,
            counter + (tokenIds.length - prevPriceCounter)
        );
    }
    ...
    function removeOverridePrice(
        ...
        (bool found, uint256 counter) = _overridenPricesCounter[nft].tryGet(
            currency
        );
        require(found, "No overriden prices found for the nft");
        require(tokenIds.length <= counter, "tokenIds[] length is too big");
        ...
        _overridenPricesCounter[nft].set(currency, counter - tokenIds.length);
    }
    ...
    function withdraw() external onlyRole(WITHDRAW) nonReentrant {
        ...
        for (uint256 i = 0; i < _validCurrenciesSet.length(); i++) {
            ...
            if (bal > 0) {
                currency.transfer(msg.sender, bal);
                withdrawn = true;
            }
        }
