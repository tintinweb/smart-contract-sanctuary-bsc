...
contract OruToken is Context, IERC20, Ownable {
    ...
    uint256 public maxBuyLimit = 5_000_000 * 10**9;
    uint256 public maxSellLimit = 5_000_000 * 10**9;
    uint256 public maxWalletLimit = 20_000_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0x4dC807300d36329689BDB2B58F30f5c463696E4C;
    address public operationsWallet = 0x57Aa0419472f90aB087BF7F58391bc4e6CFB66a7;
    address public communityWallet = 0x24C9B31887098e57041B20c76ABEA145cc754D7B;

    string private constant _name = "OruToken";
    string private constant _symbol = "ORTK";
    ...
    Taxes public taxes = Taxes(0, 2, 1, 3, 1);
    Taxes public sellTaxes = Taxes(0, 2, 1, 3, 1);
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[communityWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _community) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_operations,_community);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _operations, uint256 _community) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_operations,_community);
        emit FeesChanged();
    }
    ...
    function _takecommunity(uint256 rcommunity, uint256 tcommunity) private {
        ...
        if(_isExcluded[address(this)])
        {
            _tOwned[address(this)]+=tcommunity;
        }
        _rOwned[address(this)] +=rcommunity;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.roperations, to_return.rcommunity) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.toperations-s.tcommunity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rcommunity = s.tcommunity*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-roperations-rcommunity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 roperations,uint256 rcommunity) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Get rekt");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.toperations+ s.tcommunity);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rcommunity > 0 || s.tcommunity > 0) _takecommunity(s.rcommunity, s.tcommunity);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.operations + temp.community) * 2;
        ...
        uint256 communityAmt = unitBalance * 2 * temp.community;
        if(communityAmt > 0){
