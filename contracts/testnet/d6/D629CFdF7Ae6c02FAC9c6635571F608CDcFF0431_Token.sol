...
contract Token is Context, IERC20, Ownable {
    ...
    string public _name = "Testing";
    string public _symbol = "THIS";
    uint8 private _decimals = 18;
    
    uint8 public _marketingFee = 5;
    uint8 private _previousMarketingFee = _marketingFee;

    // diğer yerlere de ekle
    uint8 public _teamFee = 5;
    uint8 private _previousTeamFee = _teamFee;
    
    uint8 public _holderFee = 3;
    uint8 private _previousHolderFee = _holderFee;
    ...
    constructor (address tokenOwner,uint8 setMxTxPer, uint8 setMxWalletPer,
        address payable _teamWallet, address payable _marketingWallet
        )  {
        ...
        // max percentage of the total tokens someone can transfer
        _maxTxAmount = _tTotal.mul(setMxTxPer).div(
            10**2
        );
        // max percentage of the total tokens someone can hold (excluding owner)
    ...
    function setAllFeePercent(uint8 marketingFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 buybackFee) external onlyOwner() {
        require(marketingFee >= 0 && marketingFee <=maxMarketingFee,"TF err");
    ...
    function _getBuyValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketing, uint256 tBurn) = _getTBuyValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRBuyValues(tAmount, tFee, tMarketing, tBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMarketing, tBurn);
    }

    // burn ekle
    function _getTBuyValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
    ...
    // burn ekle
    function _getRBuyValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _getSellValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam, uint256 tBurn) = _getTSellValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRSellValues(tAmount, tFee, tTeam, tBurn, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTeam, tBurn);
    }

    function _getTSellValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
    ...
    function _getRSellValues(uint256 tAmount, uint256 tFee, uint256 tMarketing, uint256 tBurn, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rMarketing= tMarketing.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
    ...
    function removeAllFee() private {
        if(_marketingFee == 0 && _liquidityFee == 0 && _burnFee == 0 && _walletFee == 0 && _buybackFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        if(!takeFee && (sender != owner() || recipient != owner()))
            removeAllFee();
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if(sender == owner() && recipient != owner()){
            uint256 tMarketing;
            uint256 tBurn;
            (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tMarketing, tBurn) = _getBuyValues(tAmount);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
            _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
           
            _rOwned[marketingWallet] = _rOwned[marketingWallet].add(tMarketing.mul(_getRate()));
            _tokenTransferNoFee(address(this), dead, tBurn);
        }

        else if (sender != owner() && recipient == owner()){
            uint256 tTeam;
            uint256 tBurn;
            (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tTeam, tBurn) = _getSellValues(tAmount);
            _rOwned[sender] = _rOwned[sender].sub(rAmount);
            _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);

             _rOwned[teamWallet] = _rOwned[teamWallet].add(tTeam.mul(_getRate()));
            _tokenTransferNoFee(address(this), dead, tBurn);
        }
