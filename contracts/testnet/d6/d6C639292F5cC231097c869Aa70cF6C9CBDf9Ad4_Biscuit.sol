...
contract Biscuit is ERC20, Ownable {
    ...
    event BiscuitBurned(address _account, address destination, uint256 _amount);
    event BiscuitMinted(address _account, address destination, uint256 _amount);
    ...
    constructor(address _stakingContract) ERC20("PyeBiscuit", "Biscuit") {
    ...
    function transfer(address to, uint256 amount) public view override returns (bool) {
        revert("Biscuit is non-transferrable");
    }
    ...
    function burnBiscuit(address _staker, uint256 _amount) external onlyStakingContract() {
    ...
    function _burn(address account, uint256 amount) internal override {
        ...
        _tTotal -= amount;
        emit Transfer(account, address(0), amount);
        emit BiscuitBurned(account, address(0), amount);
        _afterTokenTransfer(account, address(0), amount);
    }

    // --------- Modified Mint Fxns ----------
   
    function mintBiscuit(address _depositor, uint256 amount) external onlyStakingContract() {
       _mint(_depositor, amount);
    }

    function _mint(address account, uint256 amount) internal override {
        require(account != address(0), "ERC20: cannot mint to the zero address");
        require(Balances[account] == 0, "Already has biscuit"); 
        _beforeTokenTransfer(address(0), account, amount);

        _tTotal += amount;
        Balances[account] += amount;
        emit Transfer(address(0), account, amount);
        emit BiscuitMinted(address(0), account, amount);
