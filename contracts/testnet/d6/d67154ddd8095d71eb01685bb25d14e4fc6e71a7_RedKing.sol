...
contract RedKing is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !swapping && from != address(uniswapV2Router) && to != address(uniswapV2Router)){
            require(amount <= balanceOf(address(from)).mul(500).div(1000),"sale limit 50%");
        }
