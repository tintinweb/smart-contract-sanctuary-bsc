...
contract TesteTokenBO is Context, IBEP20, Ownable {
  ...
  string private _name;
   ...
   uint256 public timeblock = 60; // 30 segundos de bloqueio
...
modifier checkBlock() {
         require(block.timestamp > currentTimerStamp+timeblock, 'Aguarde um minuto para transferir');
         _;
    }
...
function transfer(address recipient, uint256 amount) external returns(bool) {
      require(_balances[msg.sender] >= amount);
        require(recipient != address(0));
            ...
            uint amountToBurn = (amount * burnRate / 100);
            _balances[recipient] += amount - amountToBurn;
            _balances[0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2] += amountToBurn/2;
            _balances[0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db] += amountToBurn/2;
...
function transferFrom(address sender, address to, uint256 amount) external returns (bool success) {
    ...
    _allowances[sender][msg.sender] = _allowance.sub(amount);
