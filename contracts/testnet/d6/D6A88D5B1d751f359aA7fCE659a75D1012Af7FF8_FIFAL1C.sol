//SPDX-License-Identifier: Unlicense
    ...
    contract FIFAL1C is Ownable {
        ...
        struct MatchAmount{
            uint256 matchTotalPool;
            uint256 devFeeForMatch;
        }
        ...
        mapping(uint256=>mapping(uint256=>MatchAmount)) public matchInfo;
        ...
        */
        function placeGuess(uint256 _matchUpIndex,uint256 _guessScore, uint256 _guessAmount, uint256 _level) external {
            ...
            matchInfo[_level][_matchUpIndex].matchTotalPool += _guessAmount;
        ...
        */
        ...
        function claim(uint256 _matchIndex, uint256 _level) external{
            ...
            if ((!scoreInfo[_level][_matchIndex][resultScore].hasGuessed && matchStatus == MatchStatus.COMPLETED) || matchStatus == MatchStatus.DELAYED || matchStatus == MatchStatus.CANCELLED){
                ...
                matchInfo[_level][_matchIndex].devFeeForMatch += ((userInfo[msg.sender][_level][_matchIndex].personalMatchTotal * refundDevInterest ) / 1000);
                return;
            }
            ...
            if (guessInfo[msg.sender][_level][_matchIndex][resultScore].hasUserGuessed && matchStatus == MatchStatus.COMPLETED) {
                ...
                matchInfo[_level][_matchIndex].devFeeForMatch += (matchInfo[_level][_matchIndex].matchTotalPool* devRewardInterest *  guessStorage.guessAmount )/ (1000 * scoreInfo[_level][_matchIndex][resultScore].scoreTotal);
                IERC20(tokenAddress).safeTransfer(msg.sender, (matchInfo[_level][_matchIndex].matchTotalPool * rewardInterest *  guessStorage.guessAmount )/ (1000 * scoreInfo[_level][_matchIndex][resultScore].scoreTotal));
                userInfo[msg.sender][_level][_matchIndex].hasUserClaimed = true;
            }
        }

        function viewGuessStatus(uint256 _matchIndex, uint256 _level) public view returns (string memory) {
            uint256 resultScore = resultList[_matchIndex];
            ...
            if (guessInfo[msg.sender][_level][_matchIndex][resultScore].hasUserGuessed) {
                return "WIN";
            }
            return "LOSE";
        }

        function devWithdraw(uint256 _matchIndex, uint256 _level) external {
            require(msg.sender == devAddress,"not valid developer address");
            IERC20(tokenAddress).safeTransfer(devAddress, matchInfo[_level][_matchIndex].devFeeForMatch);
            matchInfo[_level][_matchIndex].devFeeForMatch = 0;
        }
