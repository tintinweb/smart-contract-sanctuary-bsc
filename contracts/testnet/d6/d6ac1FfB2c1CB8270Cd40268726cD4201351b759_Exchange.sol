...
contract ExchangeCore is Pausable {
    ...
    event ExecuteEvent(  // Exchange event
    ...
    struct AssetType {
        ...
        uint assetID;  /* token id need if bid is ERC721/1155. 0 for ERC20 and ETH */   
    }

    struct Asset {
        AssetType assetType; /* Asset info */
        bytes4 askAsset; /* ask token class same as asset class*/
        uint value; /* bid amount need if bid is ERC20/1155 or ETH. 0 for ERC721 */
        uint orderType; /* 0- buy 1- sell */
        uint askAssetValue; /* ask amount if ask is ERC20/1155 or ETH. 0 for ERC721 */
        uint askAssetID; /* ask token ID if ask is ERC721/1155. 0 for ERC20 and ETH*/
        uint8 v;
        bytes32 r;
        bytes32 s;
        uint salt;
    ...
    bytes32 constant public ASSET_TYPE_TYPEHASH = keccak256(
        "AssetType(bytes4 assetClass,address asset,uint assetID)"
    );

    bytes32 constant public ASSET_TYPEHASH = keccak256(
    ...
    function hashkey(Order memory order) public view returns (bytes32) {
        return keccak256(abi.encodePacked( /* Create fill key */
                order.maker,
                order.taker,
    ...
    function hash(Order memory order) public pure returns (bytes32) {
        return keccak256(abi.encodePacked( /* Create main order key */
                ORDER_TYPEHASH,
                order.maker,
    ...
    function hash(AssetType memory assetType) public pure returns (bytes32) {
        return keccak256(abi.encodePacked( /* Create asset type key */
                ASSET_TYPE_TYPEHASH,
    ...
    function hash(Asset memory asset) public pure returns (bytes32) {
        return keccak256(abi.encodePacked( /* Create asset key */
                ASSET_TYPEHASH,
    ...
    function validate(Order memory order) internal view {
        require(order.makeAsset.end > block.timestamp, "Order end validation failed");
        require(order.makeAsset.askAsset == order.takeAsset.assetType.assetClass, "maker bid asset is not taker ask asset");
        require(order.takeAsset.askAsset == order.makeAsset.assetType.assetClass, "taker bid asset is not maker ask asset");
    }
     ...
     */
    function _executeInternal(Order calldata order, uint8 _type) internal whenNotPaused {
        ...
        if(_type == 0){ // buy
            ...
            address orderSigner = hash(order.takeAsset).recover(order.takeAsset.v, order.takeAsset.r, order.takeAsset.s); // validates signed message.
            require(orderSigner == order.taker, "In correct signature, validation failed signer is not a taker");

            address orderMaker = hash(order.makeAsset).recover(order.makeAsset.v, order.makeAsset.r, order.makeAsset.s);
            require(orderMaker == order.maker, "In correct signature, validation failed maker");

            require(((order.makeAsset.value == order.takeAsset.askAssetValue) && (order.makeAsset.askAssetValue == order.takeAsset.value)), "In correct value token");
            require(((order.makeAsset.assetType.assetID == order.takeAsset.askAssetID) && (order.makeAsset.askAssetID == order.takeAsset.assetType.assetID )), "In correct token ID");
        }
        else { // sell
            require(msg.sender == order.maker, "Tx signer must be a maker");
            
            address orderSigner = hash(order.makeAsset).recover(order.makeAsset.v, order.makeAsset.r, order.makeAsset.s); // validates signed message.
            require(orderSigner == order.maker, "In correct signature, validation failed signer is not a maker");

            address orderTaker = hash(order.takeAsset).recover(order.takeAsset.v, order.takeAsset.r, order.takeAsset.s);
            require(orderTaker == order.taker, "In correct signature, validation failed taker");
    ...
    function transferTakerAsk(Order calldata _order) internal {
        if(_order.makeAsset.assetType.assetClass == ERC721_ASSET_CLASS) {// execute taker ask order 
            IERC721(_order.makeAsset.assetType.asset).safeTransferFrom( _order.maker, _order.taker,_order.makeAsset.assetType.assetID);
        }else if(_order.makeAsset.assetType.assetClass == ERC1155_ASSET_CLASS) {
            IERC1155(_order.makeAsset.assetType.asset).safeTransferFrom(_order.maker, _order.taker, _order.makeAsset.assetType.assetID, _order.makeAsset.value, "0x");
        }else 
     ...
     * @param _type 0 if buy, else sell.
     */
    function transferMakerAsk(Order calldata _order, uint _type) internal {
        if(_order.takeAsset.assetType.assetClass == ETH_ASSET_CLASS){ // execute maker ask order
            require((msg.value == _order.takeAsset.value) && (_type != 1), "msg.value is invalid or bnb not accepted on sell exec, wbnb is acceptable");
            transferBNB(_order.takeAsset.value, _order.maker);
        } else if(_order.takeAsset.assetType.assetClass == ERC20_ASSET_CLASS)
            transferErc20(IERC20(_order.takeAsset.assetType.asset), _order.takeAsset.value, _order.maker, _order.taker);
        else 
            revert("Token is not acceptable");
    }
    ...
    function transferBNB(uint256 amount, address _maker) internal {
        (uint256 _DAO, uint256 _burn, uint _revenue, uint _tValue) = computeProtocolFee(amount);
     ...
     */
    function transferErc20(IERC20 _token, uint256 _amount, address  _maker, address _taker) internal{
        require(isAllowed[address(_token)] , "Not authorized token");
        
        (uint256 _DAO, uint256 _burn, uint _revenue, uint _tValue) = computeProtocolFee(_amount);
        
        if(_DAO > 0) SafeERC20.safeTransferFrom(_token,_taker,beneficiaryAddress[0],_DAO);
        
        if(_burn > 0) SafeERC20.safeTransferFrom(_token,_taker,beneficiaryAddress[1],_burn);
        
        if(_revenue > 0) SafeERC20.safeTransferFrom(_token,_taker,beneficiaryAddress[2],_revenue);
        
        if(_tValue > 0) SafeERC20.safeTransferFrom(_token,_taker,_maker,_tValue);
    }
     ...
     * @param order Order tuple
     */
    function getFills(Order calldata order) view external returns (bool) {
     ...
     */
    function computeProtocolFee(uint256 amount ) public view returns(uint _DAO, uint _burn, uint _revenue, uint _tValue){
        if(feeRate <= 0) {
...
contract Exchange is ExchangeCore, AccessControl{
     ...
     */
    constructor( address[3] memory _beneficiaryAddress, uint[3] memory _protocolFee) {
     ...
     */
    function setDAOFee( uint _DAO) external onlyRole(DEFAULT_ADMIN_ROLE) {
        protocolFee[0] = _DAO;
    }
     ...
     */
    function setBurnFee( uint _burn) external onlyRole(DEFAULT_ADMIN_ROLE) {
        protocolFee[1] = _burn;
    }
     ...
     */
    function setRevenueFee( uint _revenue) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setFeeRate(uint _feeRate) public onlyRole(DEFAULT_ADMIN_ROLE) {
        feeRate = _feeRate;
    }
     ...
     */
    function setRevenueAdd( address _revenue) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     */
    function setTokenStatus(address token, bool status) external onlyRole(DEFAULT_ADMIN_ROLE) {
