...
contract TreeDefiTokenSwapV1x2 is Ownable, ReentrancyGuard {
    ...
    //uint256 public constant FOUR_MONTHS = 120 days;
    ...
    struct KV{
        uint tS;            //Key
        uint amount;        //Value
        uint8 legacyCode;   // 0 SEED, 1 TCO2, 2 TREE
    ...
    //
    // Utility functions to manage swap data structure
    //
    ...
    function setDeposit(address _of, uint _amount, uint8 _legacyCode) private {
        //withdrawn counter is set to 0 at the first deposit 
        swapHistory[_of].push(KV(block.timestamp, _amount, _legacyCode, uint8(0)));
    }


    // Exchange Rates
    function setRatioTREEtoCAR(uint256 _ratio) public onlyOwner {
        // 10 ** 18 is ratio 1:1
        ratioTREEtoCAR = _ratio;
    }

    function setRatioSEEDtoCAR(uint256 _ratio) public onlyOwner {
        // 10 ** 18 is ratio 1:1
        ratioSEEDtoCAR = _ratio;
    }
    
    function setRatioTCO2toCAR(uint256 _ratio) public onlyOwner {
        // 10 ** 18 is ratio 1:1
        ratioTCO2toCAR = _ratio;
    }
    
    function getRatioTREEtoCAR() public view returns (uint256) {
        return ratioTREEtoCAR;
    }

    function getRatioSEEDtoCAR() public view returns (uint256) {
        return ratioSEEDtoCAR;
    }
    
    function getRatioTCO2toCAR() public view returns (uint256) {
    ...
    function depositSEED(uint256 _amountInSEED) public nonReentrant returns (uint256) {
        ...
        setDeposit(msg.sender, amountOut, 0);
    ...
    function depositTCO2(uint256 _amountInTCO2) public nonReentrant returns (uint256) {
        ...
        // stores Exchanged amount and timestamp 
        setDeposit(msg.sender, amountOut, 1);
    ...
    function depositTreeDefi(uint256 _amountInTree) public nonReentrant returns (uint256) {
        ...
        // stores Exchanged amount and timestamp 
        setDeposit(msg.sender, amountOut, 2);
    ...
    function terminateSwap() public payable onlyOwner {
        // Selfdestruct works by erasing the contract bytecode from the chain, sending any liquidity to a specified address, and then, 
        // refunding a portion of the gas fees to developers. 
        // (ref. https://docs.soliditylang.org/en/v0.8.17/introduction-to-smart-contracts.html?highlight=selfdestruct#deactivate-and-self-destruct)
