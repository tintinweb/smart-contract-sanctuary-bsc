...
contract GDeal is ERC20, ERC20Detailed {
    ...
    uint256 public coin_rate = 1;

    constructor ()  ERC20Detailed("Great Deal Coin", "GDS", 8) {
        deployer = msg.sender;
        owner = deployer;
    ...
    function _participate(address _addr) private {
        ...
        uint256 latestPrice = priceFeed.latestAnswer() / 1e8;
        uint256 usd_amount = latestPrice * (msg.value / 1e18);

        require(users[_addr].total_deposit + usd_amount < users[_addr].limit, "can not buy more than limit");
        
        if(users[_addr].deposit_time == 0){
            users[_addr].deposit_time = block.timestamp;
            users[_addr].limit = 1500;
        }

        users[_addr].no_of_deposits++;

        uint256 final_coin_rate = coin_rate / 10;

        uint256 tokens = usd_amount / final_coin_rate;
    ...
    function getInvestments(address _addr, uint _level) public view returns(uint256, uint, uint) { 
        return (users[_addr].investment[_level].deposit_amount, users[_addr].investment[_level].time, users[_addr].investment[_level].claim_period);
    }
    function getLatestPrice(uint256 _value) public view returns(uint256, uint256) { 
        uint256 latestPrice = priceFeed.latestAnswer() / 1e8;
        uint256 usd_amount = latestPrice * (_value / 1e18);
        return (latestPrice, usd_amount);
    }
