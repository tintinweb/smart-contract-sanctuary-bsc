...
contract HeroNFT is BaseNFT {
  ...
  function updateTotalRewarded(uint _tokenId, uint _totalRewarded) onlyMintAdmin external {
    Hero storage hero = heroes[_tokenId];
    require(hero.ownPrice * rewardRate / 100 <= _totalRewarded, "400");
    hero.totalRewarded = _totalRewarded;
    emit TotalRewardedUpdated(_tokenId, _totalRewarded);
  }

  function updateRewardRate(uint _rewardRate) onlyMainAdmin external {
