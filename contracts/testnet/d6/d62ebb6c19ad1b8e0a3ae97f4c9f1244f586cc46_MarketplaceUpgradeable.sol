...
contract AuctionUpgradeable is FactoryUpgradeable {
    ...
    function placeBid(uint256 auctionItemIndex, uint256 price) external payable nonReentrant whenNotPaused {
        ...
        BidHistory memory lastBid;
        if (_bidHistories[auctionItemIndex].length > 0) {
            lastBid = _bidHistories[auctionItemIndex][_bidHistories[auctionItemIndex].length - 1];
            highestBid = lastBid.price;
        }

        require(price > highestBid, "Error: Price must be greater than last price");
        ...
        if (auctionItem.paymentToken != address(0)) {
            ...
            if (lastBid.price != 0) {
                require(
                    IERC20(auctionItem.paymentToken).transferFrom(address(this), lastBid.bider, lastBid.price),
            ...
            if (lastBid.price != 0) {
                lastBid.bider.transfer(lastBid.price);
            }
        }

        BidHistory memory bidHistory = BidHistory(payable(_msgSender()), price);
    ...
    function withdraw(uint256 auctionItemIndex) external nonReentrant {
        ...
        if (_bidHistories[auctionItemIndex].length > 0) {
            BidHistory memory lastBid = _bidHistories[auctionItemIndex][_bidHistories[auctionItemIndex].length - 1];

            // winner the auction should be allowed ti withdraw the auction item
            if (_msgSender() == lastBid.bider) {
                _transferAsset(
                    auctionItem.nftContractAddress,
                    address(this),
                    _msgSender(),
...
contract ExchangeUpgradeable is FactoryUpgradeable {
    ...
    function createExchangeSale(uint256 exchangeItemIndex, uint256 amount) external payable nonReentrant {
        ...
        SaleHistory memory saleHistory = SaleHistory(_msgSender(), totalPrice, amount, block.timestamp);
        _saleHistories[exchangeItemIndex].push(saleHistory);

        emit ExchangeItemSale(exchangeItemIndex, _msgSender(), totalPrice, amount, feePercent, block.timestamp);
    }
