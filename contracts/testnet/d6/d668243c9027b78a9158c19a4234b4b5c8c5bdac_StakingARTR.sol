...
// File: contracts/StakingARTR.sol
...
contract StakingARTR is Ownable {
    ...
    function Stake(uint months, uint amount, address refferral, uint tier) external {
        ...
        holders[refferral].teamAmount++;
        holders[refferral].team[holders[refferral].teamAmount] = holder;

        stakeDetails[holder].date = block.timestamp;
        stakeDetails[holder].month = months;
        stakeDetails[holder].stakeFinishTime = block.timestamp + months * 2629743;
        stakeDetails[holder].amount += amount;

        if (months > stakeMonths[1]) {
            holders[holder].dailyReward = (stakeDetails[holder].amount * percents[months])/30000;
    ...
    function Unstake() external {
        ...
        require(amount != 0, "you do not have stake");

        stakeDetails[holder].amount = 0;
        stakeDetails[holder].month = 0;
        
        stakeDetails[holder].freezedFinishTime = block.timestamp + 1814400;
    ...
    function countReward(address holder) internal returns (uint) {
        ...
        if (!holders[holder].rewardFinished) {
            uint readyRewards = (block.timestamp - stakeDetails[holder].date)/86400 - holders[holder].numOfPayments;
    ...
    function activatePromo(address holder, uint coins) external onlyOwner returns(uint) {
        require (coins > 0, "invalid promocode");
        holders[holder].virtualCoins += coins*10**18;
        emit PromoActivated(holder, coins);
        return coins;
    }

    function setStakingToken(address _newAddress) external onlyOwner {
        ARTR = IERC20(_newAddress);
    }
