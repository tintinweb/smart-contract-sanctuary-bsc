...
import "./TestKBv2Game.sol";



contract KBTestFinal is ERC20, Ownable {
    ...
    TestKBv2Game public TestKBv2GameInstance;
    ...
    constructor() ERC20("KBTestFinal", "TKBF", 18) {
        ...
        totalFees = rewardsFee + operationsFee + liquidityFee + buybackFee + marketingFee;

    	dividendTracker = new DividendTracker();

    	liquidityWallet = owner();
    	operationsWallet = owner();
    	marketingWallet = operationsWallet;
        buyBackWallet = owner();
    ...
    receive() external payable {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "KBTestFinal: setWhiteListAMM:: AMM is a wallet, not a contract");
        ...
        returns (bool)
    {
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "KBTestFinal: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KBTestFinal: The new dividend tracker must be owned by the KBTestFinal token contract");
    ...
    function setTestKBv2GameAddress(address _TestKBv2GameAddress) public onlyOwner {
        TestKBv2GameInstance = TestKBv2Game(_TestKBv2GameAddress);
        excludeFromFees(_TestKBv2GameAddress, true);
    }
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "KBTestFinal: The router already has that address");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "KBTestFinal: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "KBTestFinal: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "KBTestFinal: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "KBTestFinal: The operations wallet is already this address");
    ...
    function updatemarketingWallet(address newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != marketingWallet, "TestKBv4AR: The marketing wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "TestKBv4AR: The buyback wallet is already this address");
    ...
    function updateFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee) external onlyOwner {
        operationsFee = _operationsFee;
        rewardsFee = _rewardsFee;
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        marketingFee = _marketingFee;
        totalFees = operationsFee + rewardsFee + liquidityFee + buybackFee + marketingFee;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KBTestFinal: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KBTestFinal: Cannot update gasForProcessing to same value");
    ...
    function updateSellFeeIncreaseFactor(uint256 newSellFeeIncreaseFactor) public onlyOwner {
        require(newSellFeeIncreaseFactor <= 550, "Sell Increase factor can't be more than 550%");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "KBTestFinal: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "KBTestFinal: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "KBTestFinal: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "KBTestFinal: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "KBTestFinal: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "KBTestFinal: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(!isBlacklistedToken(rewardTokenAddress), "KBTestFinal: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "KBTestFinal: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "KBTestFinal: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KBTestFinal: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        TestKBv2GameInstance.endUserGames(msg.sender);
        }           
    }
