...
contract CCMRouterV2 is TaxableRouter, UUPSUpgradeable {
    ...
    function calculateSlippage(address[] calldata path, uint amountIn) external returns(uint slippage) {
        (uint[] memory swapAmounts,,) = _getSwapInfos(amountIn, path);
        uint expectedToGet = IPancakeRouter02(pcsRouter).getAmountsOut(amountIn, path)[path.length - 1];
        uint reallyGotAfterTaxes = swapAmounts[swapAmounts.length - 1];
        slippage = reallyGotAfterTaxes * 100 / expectedToGet;
    }
