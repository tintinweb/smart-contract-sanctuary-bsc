...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    */
    ...
    function startStacking(uint256 amount) public returns(bool) {
        
        require(stackingEnabled, "Error: Stacking disabled");
        require(nextAvailableClaimDate[msg.sender] <= block.timestamp, "Error: next available not reached");
        require(balanceOf(msg.sender) > 0, "Error: No HODL found");
        require(amount <= balanceOf(msg.sender), "Error: not enough HODL");
        require(!rewardStacking[msg.sender].enabled, "Stacking already enabled");
        
        uint256 reward = calculateBNBReward(msg.sender);
        require(reward < bnbStackingLimit, "Reward higher than stacking limit");

        stacking memory tmpStack;
        tmpStack.enabled = true;
        tmpStack.amount = amount;
        tmpStack.stackingCounter = getStackingCounter(rewardCycleBlock);
        tmpStack.hardcap = rewardHardcap;
        tmpStack.cycle = rewardCycleBlock;
        tmpStack.stackingLimit = bnbStackingLimit; //.div(reward);

        rewardStacking[msg.sender] = tmpStack;

        _tokenTransfer(msg.sender, stackingWallet, amount, false);
