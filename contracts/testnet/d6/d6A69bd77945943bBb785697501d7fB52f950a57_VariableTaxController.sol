...
contract VariableTaxController is OwnableUpgradeable {
    ...
    struct user_stats {
        ...
        /**REMOVE IN MAINNET!!! */user_booster[] boosters;/**REMOVE IN MAINNET!!! */
        user_booster[3] boosterslots;
    }
    ...
    struct booster_template {
        uint256 boost; // 5 = 0.5%
        uint256 durationDays;
    }

    /**REMOVE IN MAINNET!!! */
    booster_template[] available_boosters;
    /**REMOVE IN MAINNET!!! */
    ...
    address constant BUSD = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); // TESTNET
    ...
    function upgrade141122() external onlyOwner {

        booster_maxDays = 30;
        booster_maxBoostSum = 10;
        booster_maxBoostSingle = 5;
        booster_pointCost = 40;
        booster_increaseRatio = 70000000000000000; // 0.07

        paymentReceiver = address(0x4ff8C8741256d729A8cC7F1Cc02330F8103CCD28); // should be the LMS contract on mainnet
    }
    ...
    function getBoosterRate( address _addr ) public view returns (uint256){
        ...
        for(uint256 i = 0; i < 3; i++){
            user_booster memory booster = userStats[_addr].boosterslots[i];
    ...
    function getBoosterSlotInfo(address _addr) public view returns (uint256 _slot0_percent, uint256 _slot1_percent, uint256 _slot2_percent, uint256 _slot0_endtime, uint256 _slot1_endtime, uint256 _slot2_endtime ){
        _slot0_percent = userStats[_addr].boosterslots[0].boost;
        _slot1_percent = userStats[_addr].boosterslots[1].boost;
        _slot2_percent = userStats[_addr].boosterslots[2].boost;

        _slot0_endtime = userStats[_addr].boosterslots[0].end_time;
        _slot1_endtime = userStats[_addr].boosterslots[1].end_time;
        _slot2_endtime = userStats[_addr].boosterslots[2].end_time;
    }

    function buyBooster( uint256 _slotIndex, uint256 _boost, uint256 _days ) external {
        //@TODO: check if user is in race
        ...
        require(userStats[_addr].boosterslots[_slotIndex].end_time == 0 || userStats[_addr].boosterslots[_slotIndex].end_time < block.timestamp, "slot not available available" );
        ...
        userStats[_addr].boosterslots[_slotIndex] = user_booster(_boost, block.timestamp.add(_days.mul(1 days)));        
    }
