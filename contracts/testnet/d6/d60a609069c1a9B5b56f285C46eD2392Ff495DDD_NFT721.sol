 ...
 */
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base, ExchangeDomain {

    ERC20TransferProxy public erc20TransferProxy;

    mapping(address => bool) public _whitelists;

    mapping(address => uint) public _claims;

    mapping(address => uint) public _historyClaim;
    ...
    constructor (string memory name, string memory symbol, address signer, string memory contractURI, string memory tokenURIPrefix, ERC20TransferProxy _erc20TransferProxy) ERC721Base(name, symbol, contractURI, tokenURIPrefix) {
        _registerInterface(bytes4(keccak256('MINT_WITH_ADDRESS')));
        _addSigner(signer);
        erc20TransferProxy = _erc20TransferProxy;
        _startTime = 1637676000;
        _endTime = 1703277345;
        _endClaimTime = 1677098145;
    ...
    function mint(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI)  public onlySigner   {        
    ...
    function purchase(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI, uint price, AssetType assetType, address addressToken)  payable public {
        require(isSigner(ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this, tokenId, price, addressToken)))), v, r, s)), "owner should sign tokenId");
        require( _endTime > block.timestamp || _startTime <= block.timestamp || (_startTime >= block.timestamp && _whitelists[msg.sender] == true), 'Can not purchase this time');
        transfer(owner(), assetType, price, addressToken);
        _mint(msg.sender, tokenId, _fees);
        _setTokenURI(tokenId, tokenURI);    
    }

    function claim(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI) public  {
        require(_endClaimTime > block.timestamp || _claims[msg.sender] > 0, 'Can not claim this time');
        require(isSigner(ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this, tokenId)))), v, r, s)), "owner should sign tokenId");        
        _mint(msg.sender, tokenId, _fees);
        _setTokenURI(tokenId, tokenURI);
        _claims[msg.sender]--;
        _historyClaim[msg.sender]++;
    }
    ...
    function transfer( address to, AssetType assetType, uint price, address addressToken) internal { 
        ...
        if (assetType == AssetType.ETH) {
            ...
            erc20TransferProxy.erc20safeTransferFrom(IERC20(addressToken), msg.sender , to, price);
        }        
    }
    ...
    function addToClaim(address[] memory accounts) public onlyOwner {
        require(accounts.length > 0, "Invalid input");
        for(uint256 index = 0; index < accounts.length; index++){
            _claims[accounts[index]] = 1;
        }
    }

    function getClaimAddress(address _addr) public view returns  (uint numb) {
        return _claims[_addr];
    }

    function getWhiteAddress(address _addr) public view returns  (bool status) {
        return _whitelists[_addr];
    }

    function getHistoryClaim(address _addr) public view returns (uint numb) {
        return _historyClaim[_addr];
    }

    function setTimePurchase(uint256 startTime, uint256 endTime) public onlyOwner {
        _startTime = startTime;
        _endTime = endTime;
    }

    function setTimeEndClaim(uint256 endClaimTime) public onlyOwner {
