...
contract OMEA is Ownable, ReentrancyGuard {
    ...
    {
        lastCalculatedBlock = block.timestamp - _investor.lastCalculatedBlock;
        uint256 _totalLocked = _investor.totalInvested;

        uint16 _hrp = getHPR(_totalLocked);

        claimables =
            ((_totalLocked * _hrp) * (lastCalculatedBlock / REWARD_PERIOD)) /
            (10000);
    }
