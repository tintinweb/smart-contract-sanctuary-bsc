 ...
 * Implemented and modified by Evoc_Swap teams.
 */
contract Evoc_ZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // Evoc_Router interface
    IEvoc_Router02 public Evoc_Router;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from Evoc_Swap)
    ...
    // Address Evoc_Router
    address private Evoc_RouterAddress;
     ...
     * @param _Evoc_Router: address of the Evoc_Router
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _Evoc_Router,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        Evoc_RouterAddress = _Evoc_Router;
        Evoc_Router = IEvoc_Router02(_Evoc_Router);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. EVOC/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. EVOC) to receive in the intermediary swap (e.g. BNB --> EVOC)
     */
     ...
     * @param _lpToken: LP token address (e.g. EVOC/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. EVOC) in the intermediary swap (e.g. BUSD --> EVOC)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. EVOC/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. EVOC --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. EVOC/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. EVOC or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. EVOC) in the intermediary swap (e.g. BUSD --> EVOC)
     */
    ...
    {
        address token0 = IEvoc_Pair(_lpToken).token0();
        address token1 = IEvoc_Pair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IEvoc_Pair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = Evoc_Router.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = Evoc_Router.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IEvoc_Pair(_lpToken).token0() || _token0ToZap == IEvoc_Pair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IEvoc_Pair(_lpToken).token0() || _token1ToZap == IEvoc_Pair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IEvoc_Pair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IEvoc_Pair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IEvoc_Pair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = Evoc_Router.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = Evoc_Router.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IEvoc_Pair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "Evoc_Router: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "Evoc_Router: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        Evoc_Router.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token, uint256 _swapAmountIn) private {
        if (IERC20(_token).allowance(address(this), Evoc_RouterAddress) < _swapAmountIn) {
            // Reset to 0
            IERC20(_token).safeApprove(Evoc_RouterAddress, 0);
            // Re-approve
            IERC20(_token).safeApprove(Evoc_RouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = Evoc_Router.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = Evoc_Router.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = Evoc_Router.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = Evoc_Router.quote(
                token0AmountToSell,
            ...
            uint256 nominator = Evoc_Router.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
