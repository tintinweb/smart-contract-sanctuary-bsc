...
contract Miticoin is ERC20,Ownable,IMiticoin {
    IContracts_TITIMITI Contr;
    ICollections Collection;
   
    bool public Primary;
    uint256 _decimals_ = 1000000;
    uint256 private XMITI = 100;

    constructor() ERC20("Miticoin","MITI"){
        Contr=IContracts_TITIMITI(0xB9b84a5bd43FAE99e55bF3374ddB9E206150b538);
        update();
    }

    function BypassMINT (uint256 Max_world) public virtual override{
        require(Primary==false);
        address Coll = Contr.getCollections();
        address Sale = Contr.getSaleMITI();
        require(msg.sender==Coll);
       _mint(Sale,Max_world*_decimals_*XMITI);
    }

    function PrimaryMINT (uint256 PrimaryTotalWorld) public virtual override {
        require(Primary==false);
        address Coll = Contr.getCollections();
        require(msg.sender==Coll);
         Collection=ICollections(address(Coll));

        uint256 PrimaryMITI = PrimaryTotalWorld*_decimals_;
        _mint(Contr.getZoomLoupe(),PrimaryMITI*10);
        _mint(Contr.getTeam(),PrimaryMITI*10);
        _mint(Contr.getCashback(),PrimaryMITI*35);
        _mint(Contr.getRsearchers(),PrimaryMITI*5);
        _mint(Contr.getSaleMITI(),PrimaryMITI*40);
    } 

    function Add_collection (uint256 MaxWorld) public virtual override  {
        require(Primary==true);
        address Coll = Contr.getCollections();
        require(msg.sender==Coll);
        Collection=ICollections(address(Coll));
        require(MaxWorld<=49551);
        address POOL=Contr.getMiningPool();
        uint256 MITI = MaxWorld*_decimals_*XMITI;
        _mint(POOL,MITI);
    }

    function withdrawToken(address payable adr) public onlyOwner{
        uint256 balance = balanceOf(address (this));
        transfer(adr,balance);
    }
    ...
    function  update()  public virtual override{
       Collection=ICollections(address(Contr.getCollections()));
    } 
    }
