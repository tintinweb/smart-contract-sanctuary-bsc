...
contract GFarmTradingStorageV5MODIFIED {
    ...
    function storeTradeByParameters(address trader, bool buy, uint[8] calldata uintData) external onlyTrading{
        uint index = firstEmptyTradeIndex(trader, uintData[0]);
        openTrades[trader][uintData[0]][index] =
        Trade(
            trader,
            uintData[0],
            index,
        ...
        openTradesCount[trader][uintData[0]]++;
        tradesPerBlock[block.number]++;

        if(openTradesCount[trader][uintData[0]] == 1){
            pairTradersId[trader][uintData[0]] = pairTraders[uintData[0]].length;
            pairTraders[uintData[0]].push(trader);
        }

        TradeInfo memory _tradeInfo = TradeInfo(
            0, uintData[7],
            uintData[2] * uintData[4],
            0,
            0,
            false
        );

        openTradesInfo[trader][uintData[0]][index] = _tradeInfo;
        updateOpenInterestDai(uintData[0], uintData[2] * uintData[4], true, buy);
    }
