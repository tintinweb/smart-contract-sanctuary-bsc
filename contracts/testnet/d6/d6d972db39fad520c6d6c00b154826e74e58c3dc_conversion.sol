...
contract conversion is Ownable{
  ...
  constructor(address _router,address _treasury,address _Ustd) {
  ...
  function getAmountsIn(uint _amountOut, address[] calldata _path) external view returns (uint[] memory amounts){
    ...
    function swapTokensForExactTokens(uint _amountIn, address[] calldata _path)
        ...
        uint tax = calculateTax(Ustd.balanceOf(address(this)));
        Ustd.transfer(msg.sender,Ustd.balanceOf(address(this))-tax);
        Ustd.transfer(vault,tax);
        }
   ...
   function setUstdAddress(address _amount) external onlyOwner {
       Ustd = IERC20(_amount);
   }
