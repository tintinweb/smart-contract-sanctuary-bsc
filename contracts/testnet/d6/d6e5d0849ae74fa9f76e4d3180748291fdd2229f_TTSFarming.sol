...
contract TTSFarming {
    ...
    uint256 public SECONDS_WORK_MINER = 1728000; // time to work miners
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    ...
    function buyMinersAirdrop(address ref, uint256 amount) public {
        require(amount >= 50000000000000000000);
        require(initialized);
        countUsers += 1;
        deals += 1;
        ...
        ERC20(tts2).transferFrom(address(msg.sender), address(this), amount);
        uint256 balance = ERC20(tts).balanceOf(address(this));
        uint256 tokensBought = calculateTokensBuy(amount, SafeMath.sub(balance, amount));
        ...
        tokensBought = SafeMath.sub(tokensBought, SafeMath.add(BurnFee(tokensBought), devFee2(tokensBought)));
        ERC20(tts2).transfer(0x0000000000000000000000000000000000000000, amount);
