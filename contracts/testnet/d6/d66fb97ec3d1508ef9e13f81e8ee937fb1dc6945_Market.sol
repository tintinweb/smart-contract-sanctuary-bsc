...
contract Market is Context, Ownable {
    ...
    function getSellList(uint256 total) public view returns(uint256[] memory){
        ...
        for (uint256 i = 1; i <= SellList.length; i++) {
            if(isSell[i]){
                SellList[i] = i;
            }else{
                SellList[i] = 0;
    ...
    function getSellPriceList(uint256 total) public view returns(uint256[] memory){
        ...
        for (uint256 i = 1; i <= SellPriceList.length; i++) {
            if(isSell[i]){
    ...
    function getMySelling(uint256 total) public view returns(uint256[] memory){
        uint256[] memory MySellingList = new uint256[](total+1);
        for (uint256 i = 1; i <= MySellingList.length; i++) {
            if(isSell[i] && idAddr[i]==_msgSender()){
                MySellingList[i] = i;
            }else{
                MySellingList[i] = 0;
            }
