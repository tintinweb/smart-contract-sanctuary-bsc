...
abstract contract Adminable is Context {
     ...
     * @dev Returns the address of the current owner.
     */
    function adminaaa() public view virtual returns (address) {
        return _admin;
    }
     ...
     */
    modifier onlyAdmin() {
        require(adminaaa() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
library Counters {
    ...
    function reset(Counter storage counter,uint256 defaultValue) internal {
        counter._value = defaultValue;
    }
}


// File contracts/nftprice1000.sol
 ...
 */
contract NFT22s is ERC721Enumerable, Ownable, Adminable {
    function safeTransferFrom(
        address token,
        address from,
        address to,
    ...
    uint256 public oneToken = 1e18;
    uint256 public mintMax = 10000;
    uint256 constant changePriceNum = 1000;
    uint256 public singleMintMax = 10;
    ...
    mapping(address => uint256) public singleMintCounter;
    ...
    function setTokenAddress(address _addr) external onlyMaster {
        tokenAddress = _addr;
    }
    ...
    function setTokenAmount2(uint256 _amount) external onlyMaster {
        tokenAmount2 = _amount;
    }
    ...
    function _mintInternal(uint256 _num) internal {
    ...
    function mintFree(uint256 _num) external onlyMaster {
        _mintInternal(_num);
    }

    function mint(uint256 _num) external checkTime {
        require(totalSupply() <= mintMax, "mint max");
        require(
            singleMintCounter[msg.sender] + _num <= singleMintMax,
            "user max"
        );
        singleMintCounter[msg.sender] += _num;
        uint256 totalAmount;
        if (totalSupply() > changePriceNum) {
            totalAmount = tokenAmount2 * _num;
        } else if (totalSupply() + _num <= changePriceNum) {
            totalAmount = tokenAmount * _num;
        } else {
            uint256 mn = changePriceNum - totalSupply();
            totalAmount = tokenAmount * mn;
            totalAmount += tokenAmount2 * (_num - mn);
        }
        safeTransferFrom(
            tokenAddress,
            msg.sender,
