...
contract DexGoRentAndKm is Ownable {
    ...
    function rent(uint256 _tokenId) public payable {
        require(msg.value == IDexGoStorage(storageContract).getFixedAmountOwner() + IDexGoStorage(storageContract).getFixedAmountProject(), "Incorrect fixed amount");
        ...
        Address.sendValue(payable(IDexGoStorage(storageContract).getNftContract()), IDexGoStorage(storageContract).getFixedAmountProject());
        IDexGoNFT(IDexGoStorage(storageContract).getNftContract()).distributeMoney(msg.sender, IDexGoStorage(storageContract).getFixedAmountProject());
    ...
    function repair(uint256 _tokenId, bool isSpeedUp) public payable {
        if (isSpeedUp) {
            require(msg.value == IDexGoStorage(storageContract).getFixedRepairAmountProject() * 4, "Incorrect amount");
            Address.sendValue(payable(IDexGoStorage(storageContract).getNftContract()), msg.value);
            IDexGoNFT(IDexGoStorage(storageContract).getNftContract()).distributeMoney(msg.sender, msg.value);
...
//            repairFinishTime[_tokenId] = block.timestamp + 60 * 60 * 2; // 2 hours
            ...
            require(msg.value == IDexGoStorage(storageContract).getFixedRepairAmountProject(), "Incorrect amount");
            Address.sendValue(payable(IDexGoStorage(storageContract).getNftContract()), msg.value);
            IDexGoNFT(IDexGoStorage(storageContract).getNftContract()).distributeMoney(msg.sender, msg.value);
...
//        repairCount[_tokenId] = repairCount[_tokenId] + 1;
        IDexGoStorage(storageContract).setKmForId(_tokenId,IDexGoStorage(storageContract).getPriceInitialForType(IDexGoStorage(storageContract).getTypeForId(_tokenId)) * IDexGoStorage(storageContract).getRepairCount(_tokenId) / 100);
//        kmLeavesForId[_tokenId] = IDexGoStorage(storageContract).getPriceInitialForType(IDexGoStorage(storageContract).getTypeForId(_tokenId)) * repairCount[_tokenId] / 100;
