...
contract xYSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        uint reservexYSL = IERC20(Admin.YSL()).balanceOf(address(xyslBUSD));
        ...
        uint USDyAmount;
        if(IERC20(Admin.YSL()).balanceOf(address(this)) >= xYSLRequire){
            percentYSL = 30;
            USDyAmount = ((2 * amount * (10 ** 18))/USDyPrice);
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + USDyAmount);
            convertBUSDToLP(amount * percentYSL/100, (reservexYSL * (amount * percentYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.YSL(), address(xyslBUSD));
        }
        else{
            percentYSL = 15;
            ...
            reservexYSL = IERC20(Admin.YSL()).balanceOf(address(xyslBUSD));
            ...
            convertBUSDToLP(reservesBUSD * IERC20(Admin.YSL()).balanceOf(address(this))/ reservexYSL, 
                IERC20(Admin.YSL()).balanceOf(address(this)), Admin.BUSD(), Admin.YSL(), address(xyslBUSD));
