...
contract Staking is Ownable{
    ...
    event DepositFunds(uint256 ethAmount, uint256 usdtAmount);

    constructor (address _byzAddress) {
        byzToken = _byzAddress;
        usdtAddress = 0xDFcbd790C5612b2d1C3AbE9A26159cD38484DBd7;
    } 
    ...
    function deposit(uint256 _amount) public {
        ...
        updatePool();
         ...
         (uint256 etherBalance, uint256 usdtBalance) = getVaultBalance();

        if (user.amount > 0) {
            uint256 pending = user.amount.mul(accPerShare).div(1e18).sub(user.rewardDebt);
            uint256 usdtPending = pending * usdtBalance / etherBalance;
            if(pending > 0) {
                payable(msg.sender).transfer(pending);
                IERC20(usdtAddress).transfer(msg.sender, usdtPending);
            }
        }
    ...
    function setByzAddress(address _byzAddress) public onlyOwner {
        require(
    ...
    function getVaultBalance() public view returns (uint256, uint256) {
        ...
        return (etherBalance, usdtAmount);
    }
    ...
    function pendingReward(address _user) external view returns (uint256, uint256) {
        ...
        if (block.timestamp > lastRewardTime && totalSupply != 0) {
            uint256 multiplier = getMultiplier(lastRewardTime, block.number);
        ...
        (uint256 etherBalance, uint256 usdtBalance) = getVaultBalance();
        uint256 usdtReward = etherReward * usdtBalance / etherBalance;

        return (etherReward, usdtReward);
    }
    ...
    function depositFunds(uint256 usdtAmount) public payable onlyOwner{
        ...
        emit DepositFunds(msg.value, usdtAmount);
    }
