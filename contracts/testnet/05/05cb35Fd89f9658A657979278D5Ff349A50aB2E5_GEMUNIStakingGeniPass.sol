...
abstract contract GEMUNIStaking is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIStaking{
    ...
    modifier onlyTokenOwner(uint pid, uint[] memory tokenIds) {
        ...
        for (uint i; i < tokenIds.length; i ++) {
            require(IERC721(pool.stakedNFT).ownerOf(tokenIds[i]) == msg.sender, "GS: not pass owner");
        }
        _;
    }
    ...
    function addPoolNft(
        ...
        for (uint256 i = 0; i < poolInfoNfts.length; i++) {
            ...
            if (pool.stakedNFT == _tokenStaked && pool.rewardToken == _rewardToken && pool.timePoolInfo.stakingPeriod == _stakingPeriod) {
                foundToken = true;
                break;
            }
        }
    ...
    function removePoolNft(uint pid) external onlyOwner {
        ...
        IERC20(pool.rewardToken).safeTransfer(msg.sender, pool.rewardBalance);
    ...
    function depositNFT(uint pid, uint[] memory tokenIds, uint _amountToken) external override onlyTokenOwner(pid, tokenIds) nonReentrant {
        ...
        if (pool.stakedToken == address(0)){
            require(_amountToken == 0, "GS: invalid amount");
        } else {
            require(IERC20(pool.stakedToken).balanceOf(msg.sender) >= _amountToken, "GS: Don't have enough geni");
            if (_amountToken > 0) {
                IERC20(pool.stakedToken).safeTransferFrom(msg.sender, address(this), _amountToken);
            }
        } 
        uint totalAmountNFT;
        for (uint i = 0; i < tokenIds.length; i++ ) {
            uint amountNFT = getPriceNFT(pool.stakedNFT, tokenIds[i]);
            ...
            IERC721(pool.stakedNFT).transferFrom(msg.sender, address(this), tokenIds[i]);
    ...
    function withdrawNFT(uint pid, uint256 passId) external override onlyTokenOwnerStaking(pid, passId) nonReentrant {
        ...
        uint _amountNFT = getPriceNFT(pool.stakedNFT, passId);
        ...
        if (_amount > 0) {
            ...
            IERC721(pool.stakedNFT).safeTransferFrom(address(this), msg.sender, passId);
            if (_amountToken > 0) {
                IERC20(pool.stakedToken).safeTransfer(msg.sender, _amountToken);
            }
    ...
    function emergencyWithdrawNFT(uint pid) external override {
        ...
        for (uint256 i = 0; i < user.stakedNFTs.length; i++) {
            uint priceNFT = getPriceNFT(pool.stakedNFT, user.stakedNFTs[i]);
            _totalAmountNFT = _totalAmountNFT.add(priceNFT);
            if (IERC721(pool.stakedNFT).ownerOf(user.stakedNFTs[i]) == address(this)) {
                IERC721(pool.stakedNFT).safeTransferFrom(address(this), msg.sender, user.stakedNFTs[i]);
