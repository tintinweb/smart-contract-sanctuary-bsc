...
contract LYMLiquidityPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    // function registerInfo(uint256 round_, address user_) public view returns (UserRegister) {
    //     return registerInfo[round_][user_];
    // }

    function info(address account_) public view returns (uint256[6] memory uInfo) {
    ...
    function registerNumber() public view returns (uint256) {
        return meta.registerNumber;
    }
    ...
    function register(uint256 amount_) public onlyOpen distributionReward {
        ...
        uint256 round;
        if (
            block.timestamp > roundInfo[meta.currentRound].endTm &&
            roundInfo[meta.currentRound].endTm != 0
        ) {
    ...
    function claimReward() public onlyOpen distributionReward {
        uint256 lastRound = meta.currentRound - 1;
    ...
    function unRegister() public distributionReward {
        User storage uInfo = userInfo[msg.sender];
        require(uInfo.registerAmount > 0, "no stake");

        uint256 registerAmount = registerInfo[meta.currentRound][msg.sender].registerAmount;
        require(registerAmount == uInfo.registerAmount, "no stake");

        uInfo.registerAmount = 0;
        _removeRegister(meta.currentRound, msg.sender, registerAmount);

        meta.pair.transfer(msg.sender, registerAmount);
    }
}
