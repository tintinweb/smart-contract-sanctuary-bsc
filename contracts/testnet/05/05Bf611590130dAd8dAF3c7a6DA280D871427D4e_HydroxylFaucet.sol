...
contract HydroxylFaucet is Ownable {
    ...
    function bind() external {
        uint64 preClaimTime = bindGiftTime[ _msgSender() ] ;
        uint64 current = uint64(block.timestamp) ;
        require(current - preClaimTime >= bindGiftTimeSpace, "Operations are too frequent") ;
        bindGiftTime[ _msgSender() ] = current ;
        if(preClaimTime == 0) {
            hydroxylToken.transfer(_msgSender(), bindGift) ;
        } else {
            hydroxylToken.transfer(_msgSender(), everydayBindGift) ;
        }
    }

    function holder() external {
        uint64 preClaimTime = holderGiftTime[ _msgSender() ] ;
        uint64 current = uint64(block.timestamp) ;
        require(current - preClaimTime >= holderGiftTimeSpace, "Operations are too frequent") ;
        holderGiftTime[ _msgSender() ] = current ;

        uint256 amount = 0 ;
        if(preClaimTime == 0) {
            // first claim
            uint256 balance = mitToken.balanceOf(_msgSender()) ;
            require(balance >= minHolderBalance, "The Mit token balance is insufficient") ;
            amount = holderGift ;
        } else {
            // last claim
            if(isEveryCheck) {
