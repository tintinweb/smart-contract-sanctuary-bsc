...
contract BTSAuthRouter {
    ...
    function buyTier(uint tierID, address refAddress) public payable {
        ...
        if (refAddress != address(0)) {
            ...
            payable(address(this)).transfer(userBuyPrice);
    ...
    function upgradeTier(uint tierID, address refAddress) public payable {
        ...
        require(newTier > currentTier, "E3: Downgrading tier not permitted");
        require(newTier > 0, "E4: Tier does not exist");
        require(newTier <= 4, "E5: Invalid tier choice");
        ...
        if (refAddress != address(0)) {
            ...
            payable(address(this)).transfer(userUpgradePrice);
            payable(refAddress).transfer(referrerCommission);
            payable(ownerWalletAddress).transfer(address(this).balance);
        }
    ...
    function registerAsReferrer() public {
        require(isReferrer[msg.sender] != true, "E06: Referrer already registered");
