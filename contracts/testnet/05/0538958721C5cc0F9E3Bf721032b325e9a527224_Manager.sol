...
contract Manager is Context, Ownable {
    ...
    event CreateTokenSale(uint256 locktime, uint256 priceType, uint256 amount, uint256 noRounds);
    event Delivered(uint256 amountPair);
    event Finalized(uint256 amountStarSwap, uint256 amountETH);
    ...
    function create_TokenSale(
        ...
        emit CreateTokenSale(lockTime, price_Type, amount, rounds.length);
    }
...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    uint256 public price_0 = 0.0035 ether;      // 0.035 ether
    uint256 public price_1 = 0.0040 ether;      // 0.04  ether
    uint256 public price_2 = 0.0045 ether;      // 0.045 ether
    ...
    constructor (uint256 buyTime_, uint256 lockTime, uint256 claimTime_, uint8 price_type, address payable manager_, 
        IERC20 token_, address payable wallet_, uint256 _min) {
        ...
        claimeTime = block.timestamp + (buyTime_ + lockTime + claimTime_);
   ...
   function buyTokens(uint256 amount) public nonReentrant {
        ...
        require(amount >= minBuy && amount <= maxBuy,"please approve USDT according to limit");
    ...
    function finalize() public nonReentrant returns(bool) {
