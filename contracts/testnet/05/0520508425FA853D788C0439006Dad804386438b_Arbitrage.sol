...
contract Arbitrage is AccessControl {
    ...
    // create a custom modifier to save some stack
    ...
    function swap(
        ...
        for (uint256 i; i < _pairs.length; i++) {
            IPair currentPair = IPair(_pairs[i]);
            {
                ...
                (uint256 reserveInput, uint256 reserveOutput) = nextInputToken == currentPair.token0()
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
            ...
            (uint256 amount0Out, uint256 amount1Out) = nextInputToken == currentPair.token0()
    ...
    function arbitrage(
        ...
        // solhint-disable-next-line reason-string
        require(swap(_startToken, _pairs, _routers, address(this)) >= _minOutAmount);
    }
