...
contract NFTCards is ERC721, Ownable {
    ...
    address public gateKeeper;          // creator of this NFT
    ...
    constructor() ERC721("Chadinu VIP", "Card") {
    ...
    function swapTokensForUSDC(uint256 tokenAmount, address to) private {
        ...
        path[0] = tokenAddress;
        path[1] = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // address of wbnb
    ...
    function payToMintWithReferrer(address creator, string memory usercode, string memory referrerCode) internal returns (bool) {
        ...
        if (_ref.isOutDeadline != true && bytes(_ref.referrerCode).length > 1)
        {
            ...
            if (block.timestamp - _ref.creationTime < referralDeadline) {
                IBEP20(stableCoinAddress).transferFrom(primaryDevAddress, _parentRef.account, _ref.rewardAmount);
        ...
        uint256 _balanceOfStableCoin = IBEP20(stableCoinAddress).balanceOf(primaryDevAddress);
        swapTokensForUSDC( escrowAmount * 10**12, primaryDevAddress );
        uint256 balanceOfStableCoin_ = IBEP20(stableCoinAddress).balanceOf(primaryDevAddress);
