...
contract Token is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    ...
    string private constant _name = "Feets";
    string private constant _symbol = "FEET";
    ...
    modifier onlyLola() {
        require(isLola(msg.sender), "!LOLA");
        _;
    }
    modifier onlyECO() {
        require(isECO(msg.sender), "!POS");
        _;
    }
    ...
    function transferFrom(
        ...
        _approve(
            sender,
            _msgSender(),
            _allowances[sender][_msgSender()].sub(amount, "M280")
    ...
    function decreaseAllowance(
        ...
        _approve(
            _msgSender(),
            spender,
            _allowances[_msgSender()][spender].sub(subtractedValue, "MC281")
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            (uint256 rAmount,,,,,) = _getValues(tAmount,deductTransferFee);
            return rAmount;
        } else {
    ...
    function _getTValues(
        uint256 tAmount,bool takeFee
            ...
            uint256 tFee = takeFee ? calculateTaxFee(tAmount) : 0;
            uint256 tLiquidity = takeFee ? calculateLiquidityFee(tAmount) : 0 ;
            uint256 tTransferAmount = takeFee ? (tAmount.sub(tFee).sub(tLiquidity)) : tAmount;
    ...
    function doChecks(address from, address to, uint256 amount) public virtual {
        ...
        if (_antiWhaleProtection && (from != uniswapV2Pair) && !_isExcludedFromFee[from]) {
                    
                    //sell or transfer
                    if (_sellIntervalStart[from] != 0) {
                        if (
                            _sellIntervalStart[from].add(_sellTimeLimit) <
                            block.timestamp
                        ) {
                    ...
                    if (amount > _allowedTxAmount[from]) {
                        ...
                        if (_allowedTxAmount[from].sub(amount) <= 0) {
                            ...
                            _allowedTxAmount[from] = _allowedTxAmount[from].sub(
                                amount
                            );
                        }
                    }
 ...
 */
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(15).div(10000);
    }

    function calculateLiquidityFee(
        uint256 _amount
    ) private view returns (uint256) {
        return _amount.mul(85).div(10000);
    }
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        emit SwapAndLiquify(
            ...
            BNBToAddLiquidityWith
        );
    }

    function transferToAddressBnb(
 ...
 */
    ...
    */
    ...
    function increaseLockTime(
        address _lockAccount,
        uint256 _secondsToIncrease
    ) public onlyOwner {
        _lockTime[_lockAccount] = _lockTime[_lockAccount].add(
            _secondsToIncrease
        );
    }
    ...
    */
    ...
    function lolaMintSwap(
        uint256 amount,
        address bnbRecipient
    ) external onlyLola lockTheSwap {
