...
interface IPosiStakingManager{
    ...
    function poolInfo(uint256 pid) external view returns(address lpToken, uint256 allocPoint, uint256 lastRewardBlock, uint256 accPositionPerShare, uint16 depositFeeBP, uint256 harvestInterval);
    function positionPerBlock() external view returns(uint256);
}

interface IPositionLiquidityPool {
    function liquidityInfo(uint256 tokenId) external view returns(bytes32 poolId, uint128 baseAmount, uint128 quoteAmount, int128 rewardDebt, uint128 quoteDeposited);
}
    ...
    OwnableUpgradeable
{
    ...
    IPositionLiquidityPool public positionLiquidityPool;
    ...
    function initialize(
        ...
        IPositionLiquidityPool _positionLiquidityPool,
    ...
    function getPositionPerBlock() public view returns (uint256) {
        (,uint256 allocPoint,,,,) = posiStakingManager.poolInfo(posiStakingPid);
    ...
    //------------------------------------------------------------------------------------------------------------------
    function updatePositionLiquidityPool(address _newLiquidityPool) public onlyOwner {

        positionLiquidityPool =IPositionLiquidityPool(_newLiquidityPool);
    ...
    function approvePositionStakingManager() public onlyOwner {
        IERC20(address(this)).approve(address(posiStakingManager), type(uint256).max);
    }

    function stakePositionStakingManager() public onlyOwner {
        posiStakingManager.deposit(posiStakingPid, balanceOf(address(this)), address(0));
    }
    ...
    function harvestPositionStakingManager() public {
        ...
        if  ( _balanceAfter > _balanceBefore ){
            uint256 shareRewardAmount = ((_balanceAfter - _balanceBefore) * harvestFeeShareRate ) / MAXIMUM_HARVEST_FEE_SHARE;
    ...
    function stakeWithReferral(uint256 _nftId, address _referrer) public nonReentrant {
        _stake(_nftId, _referrer);
    }

    function _stake(uint256 _nftId, address _referrer) internal {
        (bytes32 _pid,,,,uint128 _amount) = positionLiquidityPool.liquidityInfo(_nftId);
        uint256[] storage nftIds = userNft[msg.sender][_pid];
        if (nftIds.length == 0) {
            nftIds.push(0);
            nftOwnedIndex[0][_pid] = 0;
        }
        nftIds.push(_nftId);
        nftOwnedIndex[_nftId][_pid] = nftIds.length - 1;
        ...
        user.rewardDebt = uint128(user.amount.mul(pool.accPositionPerShare).div(1e12));
...
//        _harvest(pid);
    ...
    function payOrLockupPendingPosition(bytes32 _pid) internal {
        ...
        if (user.nextHarvestUntil == 0) {
            user.nextHarvestUntil = uint128(block.timestamp)+pool.harvestInterval;
        }
        ...
        if (canHarvest(_pid, msg.sender)) {
            ...
            user.rewardLockedUp = uint128(user.rewardLockedUp.add(uint128(pending)));
    ...
    function removeNftFromUser(uint256 _nftId, bytes32 _pid) internal {
        ...
        nftOwnedIndex[_nftId][_pid] = nftIndex;
