...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    isRouter(_msgSender())
    {
        ...
        bool overMinTokenBalance = contractBnbBalance >= numOfBnbToSwapAndEvolve;
        ...
        if (
            ...
            //add liquidity
           // swapAndEvolve(contractBnbBalance);
        }
    ...
    function collectBNB() public onlyOwner{
        ...
        uint256 contractTokenBalance = balanceOf(address(this));
         bool overNumTokensToCollectBNB = contractTokenBalance >= numTokensToCollectBNB ;
         require(overNumTokensToCollectBNB,"Token balance is not reach for threshold");

        if(overNumTokensToCollectBNB ) {
            contractTokenBalance = numTokensToCollectBNB;
            swapTokensForBnb(contractTokenBalance);
        }
    }

    function swapAndEvolve() public onlyOwner  {
        ...
        addLiquidity(otherHalf, swapeedToken);
    ...
    function addLiquidity(uint256 bnbAmount, uint256 tokenAmount) public {
    ...
    function withdrawToken(uint256 _amount) public onlyOwner{
        ISafemoon(address(this)).transfer(msg.sender,_amount);
    }

    function setNumberOfTokenToCollectBNB(uint256 _numToken) public onlyOwner {
        numTokensToCollectBNB = _numToken * 10**9;
    }
