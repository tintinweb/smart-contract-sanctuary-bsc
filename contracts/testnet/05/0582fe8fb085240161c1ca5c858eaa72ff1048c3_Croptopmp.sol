...
contract Croptopmp is Context, IBEP20, Ownable {
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool) {
    ...
    //if onlyCommunity is set to true, then only people that have been approved as part of the GEN Community can buy 
        if (onlyCommunity){
        require(_isCommunity[recipient], "Only Community Can Buy!");}
  ...
  function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    ...
    function removeFromCommunitylist(address account) external onlyOwner {
        _isCommunity[account] = false;
    }

        
    //adding people to the Community - these people are the only ones that will be able to buy at launch if Community is set to true!
