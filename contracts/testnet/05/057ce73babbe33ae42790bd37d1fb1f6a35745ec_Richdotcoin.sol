...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    modifier updateReward(address _account) {
        ...
        if (_account != address(0)) {
            treasuryeOf[_account] = _treasuryAmount[_account] * treasuryRATE/(100*duration) * duration;
        }
    ...
    function CheckTreasuryPerformance(address _account) public view returns (uint256) {
        return _treasuryAmount[_account] * treasuryRATE/(100*duration) * (block.timestamp - startAt[_account]);
    }

    function withdrawTreasury(uint256 _amount) external updateReward(msg.sender) {
        require(_amount <= treasuryClaim[msg.sender]);
        uint256 rewardearned = _treasuryAmount[msg.sender]*treasuryRATE/(100*duration) * (block.timestamp - startAt[msg.sender]);
