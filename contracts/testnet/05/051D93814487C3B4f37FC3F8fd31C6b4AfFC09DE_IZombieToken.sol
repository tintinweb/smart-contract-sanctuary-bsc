...
contract iZombieERC20 is Ownable, ERC20 {
    ...
    uint256 public amountPlayToEarn = 100 * 10**6 * 10**18;
    ...
    uint256 public tokenForBosses = 2 * 10**6 * 10**_decimals;

    address public addressForBosses;
    uint256 public sellFeeRate = 3;
    uint256 public buyFeeRate = 2;

    constructor(string memory name, string memory symbol, uint8 decimals, uint256 maxSupply) ERC20(name, symbol, decimals) {
        _decimals = decimals;
    ...
    modifier onlyFarmOwners() {
        require(manager.farmOwners(_msgSender()), "Error: Caller is not the farmer");
        _;
    }

    modifier onlyEvolver() {
        require(manager.evolvers(_msgSender()), "Error: Caller is not the evolver");
        _;
    }
    ...
    function setMinTokensBeforeSwap(uint256 _tokenForBosses)
        ...
        onlyOwner
    {
        require(_tokenForBosses < 20 * 10**6 * 10**_decimals, "Error: Too much tokens for Bosses");
        tokenForBosses = _tokenForBosses;
    }

    function farm(address recipient, uint256 amount) external onlyFarmOwners {
        require(amountFarm != farmReward, "Error: All Tokens for Farming Minted");
        require(recipient != address(0), "Error: Farmer Address can't be Zero Address");
        require(amount > 0, "Error: Faming ammount can't be Zero Value");
    ...
    function win(address winner, uint256 reward) external onlyEvolver {
        require(playToEarnReward != amountPlayToEarn, "Error: All Tokens for Play To Earn Minted");
        require(winner != address(0), "Error: Winner can't be Zero Address");
        require(reward > 0, "Error: Reward can't be Zero Value");
...
contract IZombieToken is iZombieERC20, ReentrancyGuard {
    ...
    constructor(string memory name, string memory symbol, uint8 decimals, uint256 maxSupply_)
        iZombieERC20(name, symbol, decimals, maxSupply)
    {
    ...
    function setAddressForBosses(address _addressForBosses) external onlyOwner {
        require(_addressForBosses != address(0), "Error: Boss Address can't be Zero Address");

        addressForBosses = _addressForBosses;
    }

    function antiBot(uint256 amount) external onlyOwner {
        require(amount > 0, "Error: Bot amount can't be zero");
