...
contract Treasury is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    bytes32 public constant WHITELISTED_PANEL_ROLE =
        keccak256("WHITELISTED_PANEL_ROLE");

    mapping(bytes32 => address[]) public roleToAddresses;
    event TreasuryFundsWithdrawn(address to, uint256 amount);
    event FundsReceived(address from, uint256 amount);

    // constructor() {
    //     _setRoleAdmin(SUPER_ADMIN_ROLE, SUPER_ADMIN_ROLE);
    //     _setRoleAdmin(TREASURER_ROLE, SUPER_ADMIN_ROLE);
    //     _setRoleAdmin(OPERATOR_ROLE, SUPER_ADMIN_ROLE);
    //     _setRoleAdmin(WHITELISTED_PANEL_ROLE, SUPER_ADMIN_ROLE);

    //     _setupRole(SUPER_ADMIN_ROLE, _msgSender());
    //     _setupRole(TREASURER_ROLE, _msgSender());
    //     _setupRole(OPERATOR_ROLE, _msgSender());
    //     _setupRole(WHITELISTED_PANEL_ROLE, _msgSender());
    // }
    ...
    function initialize() public initializer {
        ...
        // _setupRole(OPERATOR_ROLE, _msgSender());
        // _setupRole(WHITELISTED_PANEL_ROLE, _msgSender());
        roleToAddresses[SUPER_ADMIN_ROLE].push(_msgSender());
        // roleToAddresses[SUPER_ADMIN_ROLE].push(_msgSender());
        // roleToAddresses[SUPER_ADMIN_ROLE].push(_msgSender());
        // roleToAddresses[SUPER_ADMIN_ROLE].push(_msgSender());
    }
    ...
    {
        _grantRole(role, account);
        roleToAddresses[role].push(account);
    }
    ...
    {
        _revokeRole(role, account);
        uint256 len = roleToAddresses[role].length;
        address[] storage accounts = roleToAddresses[role];
    ...
    function setUpRoleAdmin(bytes32 _role, bytes32 _adminRole)
        external
        checkRole(SUPER_ADMIN_ROLE)
    {
        _setRoleAdmin(_role, _adminRole);
    }

    function getAllAdminsOfRole(bytes32 role)
        external
        view
        returns (address[] memory)
    {
        return roleToAddresses[role];
    }
...
contract Head2Head is Initializable, UUPSUpgradeable, PausableUpgradeable {
    /**
    ...
    //modifier for access control, through treasury contract
    modifier checkRole(bytes32 role) {
        require(
            treasuryContract.hasRole(SUPER_ADMIN_ROLE, _msgSender()) ||
                treasuryContract.hasRole(role, _msgSender()),
            "AccessControl: No Access"
        );
        _;
    }
    ...
    struct Head2HeadGame {
        ...
        uint8 roundId; // The round Id of championship games, in a particular day
    ...
    //MIN and MAX Multiplier Limits
    uint256[2] public MIN_MAX_MUL_LIMITS; // 0 index = MIN_LIMIT, 1st index = MAX_LIMIT
     ...
     */
    mapping(uint256 => mapping(address => Head2HeadBet[])) private bets;

    // /**
    //  * @notice no. of bets placed by particular user in particular game
    //  */
    // mapping(uint256 => mapping(address => uint256)) public noOfBetsOfUserInGame;

    /**
     * @notice gameIds of all the games, the better has participated in head2head game ( betterAddress -> gameId[] )
     */
    mapping(address => uint256[]) private betterGameIds;
    // /**
    //  * @notice betters address in game
    //  */
    // mapping(uint256 => address[]) public bettersAddress; //Done
    mapping(address => uint256) private totalUserBets;
    ...
    event Head2HeadMinMulLimitModified(uint256 _minMulLimit);
    event Head2HeadMaxMulLimitModified(uint256 _maxMulLimit);

    function initialize(
        address payable treasury,
        address bnbUsdOracle,
        uint256[2] memory minMaxMulLimits
    ) public initializer {
        treasuryContract = Treasury(treasury);
        priceFeed = AggregatorV3Interface(bnbUsdOracle);
        MIN_MAX_MUL_LIMITS = minMaxMulLimits;
        emit Head2HeadMinMulLimitModified(MIN_MAX_MUL_LIMITS[0]);
        emit Head2HeadMaxMulLimitModified(MIN_MAX_MUL_LIMITS[1]);
    ...
    ) external checkRole(OPERATOR_ROLE) whenNotPaused {
        require(
    ...
    function createGame(
    ...
    ) public checkRole(OPERATOR_ROLE) whenNotPaused {
        require(
        ...
        require(
            MIN_MAX_MUL_LIMITS[0] <= _initialMultiplierInWei &&
                MIN_MAX_MUL_LIMITS[1] >= _initialMultiplierInWei,
            "initMul out of limits"
        );
        require(_stockIds[0] != _stockIds[1], "Same stockIds");
        ...
        whenNotPaused
    {
        ...
        } else if (getGameStatus(_gameId) != GameStatus.CREATED) {
        ...
        // noOfBetsOfUserInGame[_gameId][_msgSender()] += 1;
        totalUserBets[_msgSender()]++;

        uint256 totalBetsPooled = head2headGames[_gameId].totalBetsPooled;
        uint256 bnbPrice = uint256(this.getLatestPrice());
    ...
    ) external checkRole(OPERATOR_ROLE) {
        // CHECKS FOR UPDATING THE RESULTS

        // if (head2headGames[_gameId].isGameDraw) {
        //     revert Head2Head__GameDraw();
        // }
        require(_winningStockId != bytes12(0), "winStock!=0");
        if (getGameStatus(_gameId) != GameStatus.ENDED) {
    ...
    // the min value is 18 decimals value, Ex: min limit 2 should be passed as(2 => 2*10**18)
    function setMinMulLimit(uint256 _minMulLimit)
        external
        checkRole(SUPER_ADMIN_ROLE)
    {
        require(_minMulLimit >= 11e17, "Too low");
        MIN_MAX_MUL_LIMITS[0] = _minMulLimit;
        emit Head2HeadMinMulLimitModified(_minMulLimit);
    }

    function setMaxMulLimit(uint256 _maxMulLimit)
        external
        checkRole(SUPER_ADMIN_ROLE)
    {
        require(_maxMulLimit <= 3e18, "Too high");
        MIN_MAX_MUL_LIMITS[1] = _maxMulLimit;
        emit Head2HeadMaxMulLimitModified(_maxMulLimit);
    }
        ...
        checkRole(SUPER_ADMIN_ROLE)
    {
        ...
        require(balance >= amount, "Head2Head: Insufficient Balance");
        (bool success, ) = to.call{value: amount}("");
        require(success, "Head2Head: Transfer failed");
    }
    ...
    function pause() external checkRole(SUPER_ADMIN_ROLE) {
        _pause();
    }

    function unpause() external checkRole(SUPER_ADMIN_ROLE) {
        _unpause();
    }
    ...
    // /**
    //  * @notice Return start and end game stock price of given stockId
    //  * @param _gameId Game ID to return status
    //  * @param _stockId Id of the stock
    //  * @return Return start and end game stock price of given stockId
    //  */
    // function getStartAndEndGameStockPriceOfStockId(
    //     uint256 _gameId,
    //     bytes12 _stockId
    // ) external view returns (uint256, uint256) {
    //     return (
    //         head2headGames[_gameId].startGameStockPricesInWei[_stockId],
    //         head2headGames[_gameId].endGameStockPricesInWei[_stockId]
    //     );
    // }
    ...
    function noOfBetsOfUserInGame(uint256 _gameId, address _better)
        external
        view
        returns (uint256)
    {
    ...
    function getBetterGameIdsLength(address _better)
        external
        view
        returns (uint256)
    {
        return betterGameIds[_better].length;
    }

    /**
     * @notice Returns gameIds of betters epoch wise
     * @param _better: better address
     * @param cursor: cursor
     * @param size: size
     */
    function getBetterGameIds(
        address _better,
        uint256 cursor,
        uint256 size
    ) external view returns (uint256[] memory, uint256) {
        uint256 length = size;
        if (length > betterGameIds[_better].length - cursor) {
            length = betterGameIds[_better].length - cursor;
        }
        uint256[] memory gameIds = new uint256[](size);
        for (uint256 i = 0; i < length; ++i) {
            gameIds[i] = betterGameIds[_better][cursor + i];
        }
        return (gameIds, cursor + length);
    }
    ...
    /**
     * @notice Returns better addresses in epochs
    ...
    function getGameBetters(
        ...
        if (length > head2headGames[_gameId].betters.length - cursor) {
            length = head2headGames[_gameId].betters.length - cursor;
        }

        address[] memory values = new address[](length);

        for (uint256 i = 0; i < length; i++) {
            values[i] = head2headGames[_gameId].betters[cursor + i];
        }

        return (values, cursor + length);
    }

    function getTotalUserBets(address _better) external view returns (uint256) {
        return totalUserBets[_better];
    }

    /**
     * @notice Returns round epochs and game id information
     * @param better: better address
     * @param cursorGameId: cursor
     * @param betCursor: cursor
     * @param size: size
     */
    function getAllBetsOfBetter(
        address better,
        uint256 cursorGameId,
        uint256 betCursor,
        uint256 size
    ) external view returns (bytes memory) {
        Head2HeadBet[] memory values = new Head2HeadBet[](size);
        bytes[] memory extraParams = new bytes[](size); // to store consecutively gameId,rewarddistributed,startgametime,endgametime
        uint256 k;

        cursorGameId = (cursorGameId == type(uint256).max)
            ? betterGameIds[better].length - 1
            : cursorGameId;

        for (uint256 i = cursorGameId; i >= 0; i--) {
            uint256 _gameId = betterGameIds[better][i];
            uint256 noOfBets = bets[_gameId][better].length;
            betCursor = (betCursor == type(uint256).max)
                ? noOfBets - 1
                : betCursor;
            for (uint256 j = betCursor; j >= 0; j--) {
                // console.log(i, j);
                {
                    address _better = better;
                    extraParams[k] = abi.encode(
                        j,
                        _gameId,
                        head2headGames[_gameId].rewardsDistributed,
                        head2headGames[_gameId].timestamps[1],
                        head2headGames[_gameId].timestamps[2]
                    );
                    values[k++] = bets[_gameId][_better][j];
                }

                if ((i == 0) && (j == 0)) {
                    //all entries fetched
                    return abi.encode(extraParams, values, true, 0, 0);
                } else if (k == size) {
                    if (j == 0) {
                        {
                            address _better = better;
                            return
                                abi.encode(
                                    ...
                                    false,
                                    --i,
                                    bets[betterGameIds[_better][i]][_better]
                                        .length - 1
                                );
                        }
                    } else
                        return abi.encode(extraParams, values, false, i, --j);
                } else if (i > 0 && j == 0) {
                    betCursor = type(uint256).max;
                    break;
                }
            }
        }
    ...
    // function getTotalGamesOfBetter(address _better)
    //     external
    //     view
    //     returns (uint256)
    // {
