...
contract EAPool is Ownable, ReentrancyGuard {
    ...
    function DepoEAProfits(uint256 pamt) public {
        ...
        p.eaendtime = block.timestamp + 180;
        // p.eaptotal = ;
    ...
    function WithdrawMiscProfits() public {
        ...
        require (user.pendingWithdraw > 0);
    ...
    function WithdrawEAProfits(uint256 pnum) public {
        ...
        require (profit.eaendtime > block.timestamp);
    ...
    function calcEAProfits(address calcaddy, uint256 profnum) public view returns (uint256) {
        ...
        uint256 perc = newamt.mul(thousand).div(profit.eatotalbased);
    ...
    // function loopThrough() public returns (EAProf [] memory LE) {

    //     EAProf [] memory LE;

    //     for (uint256 i = 0;  i < EAProfitCount; i++){

    //         LE.push(EAProf(internalFunc(i)});
    ...
    // function internalFunc(uint256 x) public returns (EAProf memory LE) {

    //     EAProf storage e = EProf[x];
