...
contract PrivateSale is Auth {
  ...
  function joinPool() public payable {
    ...
    investorInfos[msg.sender] = InvestorInfo(block.timestamp, 0, totalJoinedBNB, total, 0, 0, 0); //joineddate, _cliff, bnb, total, current,notreceived, state
  ...
  function withdrawBNB(uint256 _amount, address _to) external onlyOwner {
    ...
    require(ownerBalance >= _amount && _amount > 0, 'PrivateSale:withdrawBNB - No BNB to withdraw');
  ...
  function withdrawMAMP(uint256 _amount, address _to) external onlyOwner {
    ...
    require(ownerBalance >= _amount && _amount > 0, 'PrivateSale:withdrawMAMP - No MAMP to withdraw');
  ...
  function withdrawTGE() external {
    ...
    uint256 receivingAmountAfterListing = investorInfos[msg.sender].totalReceivableAmount.mul(TGE).div(percDenominator);
    uint256 notReceivedMamps = _claimMamp(receivingAmountAfterListing);

    if (notReceivedMamps > 0) {
      investorInfos[msg.sender].notReceivedAmount = notReceivedMamps;
    }
  ...
  function withdrawPerMonth() external {
    ...
    uint256 totalAmount = investorInfos[msg.sender].totalReceivableAmount;
    uint256 currentAmount = investorInfos[msg.sender].currentAmount;
    require(currentAmount <= totalAmount, 'PrivateSale:withdrawPerMonth - You are all received Amount of Private Sale');

    uint256 amountToReceivePerMonth = totalAmount.mul(PercPerMonth).div(percDenominator);
    require(amountToReceivePerMonth > 0, 'no receivable amount');

    uint256 notReceivedMamps;

    if (amountToReceivePerMonth > totalAmount.sub(currentAmount))
      notReceivedMamps = _claimMamp(amountToReceivePerMonth);
    else notReceivedMamps = _claimMamp(totalAmount.sub(currentAmount));
  ...
  function withdrawNotReceivedMamps() external {
    uint256 amountNotReceived = investorInfos[msg.sender].notReceivedAmount;
    require(amountNotReceived > 0, 'you have no received mamps');
    require(MambaContract.balanceOf(address(this)) >= amountNotReceived, 'Now not enough mamba in contract');

    uint256 result = _claimMamp(amountNotReceived);
    investorInfos[msg.sender].notReceivedAmount = result;

    emit withdrawn(address(this), amountNotReceived);
  }

  function addTotalAmountForPrivateSale(uint256 _addingAmount) external onlyOwner {
    require(_addingAmount > 0, 'adding amount must not be zero');
    totalAmountForPrivateSale = totalAmountForPrivateSale.add(_addingAmount);
  }

  function setListedonPancakeswap() external onlyOwner {
    require(isListedonPancakeswap == false, 'is already set true');
    isListedonPancakeswap = true;
  }

  function setStartTime(uint256 _startTime) external onlyOwner {
    require(_startTime >= block.timestamp, 'start Time must be ahead of current time');
    require(_startTime < end_time, "start time can't be exceed end time");
    start_time = _startTime;
  }

  function setEndTime(uint256 _endTime) external onlyOwner {
    require(_endTime >= block.timestamp, 'end time must be ahead of current time');
    require(_endTime > start_time, "end time can't be short starttime");
    end_time = _endTime;
  }

  function setCliff(uint256 _cliff) external onlyOwner {
    require(_cliff > 0, 'cliff must not be zero');
    cliff = _cliff;
  }
  ...
  function setPercPerMonth(uint256 _perc) external onlyOwner {
    require(_perc > 0 && _perc <= 50, "can't be 0 or bigger 50");
    PercPerMonth = _perc;
  }

  function setTGE(uint256 _tge) external onlyOwner {
    require(_tge > 0 && _tge <= 50, "can't be 0 or bigger 50");
    TGE = _tge;
  }
  ...
  function getInvestorInfo(address _beneficiary) external view returns (InvestorInfo memory) {
    require(_beneficiary != address(0), "can't be zero address");
  ...
  function _claimMamp(uint256 amountToBuy) private returns (uint256) {
    if (MambaContract.balanceOf(address(this)) >= amountToBuy) {
      MambaContract.transfer(msg.sender, amountToBuy);
      return 0;
    } else {
      uint256 notReceivedMamps = amountToBuy.sub(MambaContract.balanceOf(address(this)));
      MambaContract.transfer(msg.sender, MambaContract.balanceOf(address(this)));
      return notReceivedMamps;
    }
  }
