...
contract TheChosenOne is ERC721HolderUpgradeable, DivestorUpgradeable, RandomGeneratorUpgradeable{
    ...
    event ChoiceUser(uint indexed tokenId, address indexed user);

    function Choice() public onlyOwner returns (address){
        require(ChosenUser == address(0), "The results have appeared");
        uint count = RBA.getLatestId();
        uint index = randomCeil(count);
        ChosenUser = RBA.ownerOf(index);
        emit ChoiceUser(index, ChosenUser);
        return ChosenUser;
    }
