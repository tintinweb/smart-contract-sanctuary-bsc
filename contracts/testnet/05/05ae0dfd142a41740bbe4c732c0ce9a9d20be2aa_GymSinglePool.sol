...
contract GymSinglePool is ReentrancyGuardUpgradeable, OwnableUpgradeable, RewardRateConfigurable {
    ...
    function setUsersRightShares(address _user, uint256 _userShareggym) external onlyOwner {
        UserInfo storage user = userInfo[_user];
        user.totalGGYMNET = _userShareggym;
    }

    function RESETUserCompletely(address _user) external onlyOwner {
        UserInfo storage user = userInfo[_user];
        ...
        UserDeposits storage depositDetails = user_deposits[_user][0];
       
        depositDetails.depositTokens = 0;
        depositDetails.depositDollarValue = 0;
        depositDetails.stakePeriod = 0;
        depositDetails.depositTimestamp = 0;
        depositDetails.withdrawalTimestamp = 0;
        depositDetails.rewardsGained = 0;
        depositDetails.is_finished = true;
        depositDetails.rewardsClaimt = 0;
        depositDetails.rewardDebt = 0;
        depositDetails.ggymnetAmt = 0;
        depositDetails.is_unlocked = true;
        totalGymnetLocked = 0;
        totalGGymnetInPoolLocked =0;
    }
    ...
    function deposit(
        ...
        if (
            !ICommissionActivation(0x6a4B27E70dbe2Cc0Fc23Bf69f300905371aD3b29)
    ...
    function _activatePendingCommissions() private {
        ...
        ICommissionActivation(0x6a4B27E70dbe2Cc0Fc23Bf69f300905371aD3b29).activateCommissions(
            3,
            msg.sender
        );
