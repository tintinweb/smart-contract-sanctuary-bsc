...
contract ClaimStakeAll is ReentrancyGuard, Initializable{
    ...
    function stakeAll(uint32 _level) external nonReentrant {
        for(uint i; i < singleVaults.length; i++){
            uint balance = IERC20(singleVaults[i].token).balanceOf(msg.sender);
            console.log(balance,"balance");
            console.log(ISingleVault(Admin.customVaultMaster()).checkVaultStatus(singleVaults[i].vault));
            console.log(singleVaults[i].vault);
            if(balance > 0 && ISingleVault(Admin.customVaultMaster()).checkVaultStatus(singleVaults[i].vault)) {
                ISingleVault(singleVaults[i].vault).deposit(msg.sender, balance, _level, false);
            }
        }
    }
    ...
    function claimAll() external nonReentrant {
        for(uint i; i < singleVaults.length; i++) {
            if(ISingleVault(Admin.customVaultMaster()).checkVaultStatus(singleVaults[i].vault)) {
                ISingleVault(singleVaults[i].vault).claimRebaseReward(msg.sender);
            }
        }
    }
