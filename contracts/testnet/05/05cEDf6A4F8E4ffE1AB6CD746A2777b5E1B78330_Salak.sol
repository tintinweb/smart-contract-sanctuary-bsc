...
contract Salak is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xE8E98a435576D94870739B0de03236182d1cbD34);
    address payable public rndAddress = payable(0xE8E98a435576D94870739B0de03236182d1cbD34);
    address payable public stakingAddress = payable(0xE8E98a435576D94870739B0de03236182d1cbD34);
    ...
    function _rnd(address account, uint256 amount) internal {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        if(_rndFee > 0 && _taxFee > 0) _rnd(sender, bFee);
    ...
    function setMarketingFeePercent(uint256 Fee) external onlyOwner() {
        ...
        function setRndFeePercent(uint256 Fee) external onlyOwner() {
        _rndFee = Fee;
        _liquidityFee = _crctLiquidityFee.add(_rndFee);
    }

        function setStakingFeePercent(uint256 Fee) external onlyOwner() {
        _stakingFee = Fee;
        _liquidityFee = _crctLiquidityFee.add(_stakingFee);
    }
    ...
    function setMarketingAddress(address payable _marketingAddress) external onlyOwner() {
        ...
        function setRndAddress(address payable _rndAddress) external onlyOwner() {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        uint256 tFee = calculateStakingFee(tAmount);
        uint256 bFee = calculateRndFee(tAmount);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _rndFee == 0 && 
        _marketingFee == 0 && _stakingFee == 0 && _crctLiquidityFee == 0) return;
        
        _pcrctLiquidityFee = _crctLiquidityFee; 
        _previousRndFee = _rndFee; // RND fee
        _previousMarketingFee = _marketingFee; // marketing fee
        _previousStakingFee = _stakingFee; //Staking fee
    ...
    function restoreAllFee() private {
        ...
        _rndFee = _previousRndFee; // RND fee
        _marketingFee = _previousMarketingFee; // Marketing fee
        _stakingFee = _previousStakingFee; //Staking fee
...
/** SET MINIMUM TOKEN SEND TO MARKETING WALLET
 ...
 */
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, RND, liquidity fee
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        if(_marketingFee > 0)
        {
            ...
            marketingAmount = marketingAmount.div(_liquidityFee);
            liquidityAmount = tokenBalance.sub(marketingAmount);
            swapToMarketingETH(marketingAddress, marketingAmount);
    ...
    function swapToMarketingETH(address payable recipient, uint256 tokenAmount) private {
