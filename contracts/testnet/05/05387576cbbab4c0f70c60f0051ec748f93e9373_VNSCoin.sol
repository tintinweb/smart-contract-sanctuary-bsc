...
contract VNSCoin is ERC20, Ownable {
    ...
    address constant public _usdc = 0x33B28c697Ad6b0685db05eD8A1aB806f0577AcFC;
    address constant public _busd = 0x33B28c697Ad6b0685db05eD8A1aB806f0577AcFC;
    struct CoinConfig {
        //用于储存用于ido的余额
    ...
    //执行IDO-usdc
    function idoUSDC(address pid,uint256 amount) external  {
        require(amount >= _coinConfig._commonValueMin, "amount need > min");
        // require(
        //     _coinConfig._idoBalanceSupply > 0,
        //     "_idoBalanceSupply need greater than 0"
        ...
        uint256 idoAmountUsdt  = amount / s3 * s1  ;
        uint256 PidAmountUsdt  = amount / s3 * s2  ;
        IERC20(_usdc).transferFrom(msg.sender, address(this), idoAmountUsdt);
        IERC20(_usdc).transferFrom(msg.sender, address(pid), PidAmountUsdt);
        //先铸币给用户
        _mint( msg.sender, idoAmount);
        _mint( address(this) , idoAmount * 5);
        // _transfer(address(this), msg.sender, idoAmount);
    ...
    //执行IDO-usdc
    function idoBUSD(address pid,uint256 amount) external  {
        require(amount >= _coinConfig._commonValueMin, "amount need > min");
        // require(
        //     _coinConfig._idoBalanceSupply > 0,
        //     "_idoBalanceSupply need greater than 0"
        ...
        uint256 idoAmountUsdt  = amount / s3 * s1  ;
        uint256 PidAmountUsdt  = amount / s3 * s2  ;
        IERC20(_busd).transferFrom(msg.sender, address(this), idoAmountUsdt);
        IERC20(_busd).transferFrom(msg.sender, address(pid), PidAmountUsdt);
        //先铸币给用户
        _mint( msg.sender, idoAmount);
        _mint( address(this) , idoAmount * 5);
        // _transfer(address(this), msg.sender, idoAmount);
