...
// File: contracts/main/libraries/Math.sol
...
// File: contracts/Parity/TokenParity.sol
...
mapping(address => Amount) public userAmount;
mapping(address => Amount) public userWeights;
mapping(address => uint256) public usersDeposiTotal;
mapping(address => uint256) public userRisk;
...
event UpdateBaseURI( string _baseURI);
    ...
    function setUserAmount(address _user, uint256 _amount, uint256 _id) 
        external onlyProxy {
        require(
            _user != address(0),
            "Formation.Fi: zero address"
        ...
        if ( _id==0){
        userAmount[_user].alpha += _amount;
        }
        else if ( _id==1) {
        userAmount[_user].beta += _amount;
        }
        else {
            userAmount[_user].gamma += _amount;
        }
    }     
    function setUserAmountDeposit(address _user, uint256 _amount, uint256 _id) external onlyProxy {
        require(
            _user!= address(0),
            "Formation.Fi: zero address"
        );
    ...
    function setUserAmountWithdraw(address _user, uint256 _amount, uint256 _id) 
        external onlyProxy {
        require(
            _user!= address(0),
            "Formation.Fi: zero address"
    ...
    function burn( address _user, uint256 _tokenId) external onlyProxy {
        ...
        require(userAmount[_user].alpha == 0 && userAmount[_user].beta == 0 
        && userAmount[_user].gamma == 0, "Formation.Fi: withdraw tokens");
        deleteUser(_user);
        _burn(_tokenId); 
    ...
    function updateDepositData(address _account, uint256 _amount, uint256 _risk,
        ...
        if ((userWeights[_account].alpha!=0) || (userWeights[_account].beta!=0) 
        || (userWeights[_account].gamma!=0) ){
            require((userWeights[_account].alpha ==  _weightAlpha) && 
            (userWeights[_account].beta == _weightBeta) &&
            (userWeights[_account].gamma == _weightGamma) &&
            (userRisk[_account] == _risk), "Formation.Fi: no user's risk");
        }
        else {
            userWeights[_account].alpha = _weightAlpha;
            userWeights[_account].beta = _weightBeta;
            userWeights[_account].gamma = _weightGamma;
            userRisk[_account] = _risk;
        }
        ...
        usersDeposiTotal[_account] += _amount;
        amountDepositTotal += _amount;

    }
    function updateRisk(address _account, uint256 _weightAlpha ,uint256 _weightBeta ,
            uint256 _weightGamma, uint256 _risk, uint256[3] memory _price ) public onlyProxy { 
            require(
            _account!= address(0),
            "Formation.Fi: zero address"
             );
            ...
            if ( (userWeights[_account].alpha != _weightAlpha) 
            || (userWeights[_account].beta != _weightBeta) ||
            (userWeights[_account].gamma != _weightGamma)){
            //Amount memory _userAmount = userAmount[_account];
            Amount memory  _userDeposit = usersDeposit[_account];
            Amount memory  _userWithdraw = usersWithdraw[_account];
            Amount memory _value;
            _value.alpha = ( userAmount[_account].alpha
            - _userWithdraw.alpha) * _price[0] + _userDeposit.alpha;
            _value.beta =   ( userAmount[_account].beta - _userWithdraw.beta) * _price[1] 
            + _userDeposit.beta;
            _value.gamma = (userAmount[_account].gamma - _userWithdraw.gamma) * _price[2] 
            + _userDeposit.gamma;
    ...
    function updateWithdrawtData(address _account, uint256 _rate) 
        ...
        Amount memory _userAmount = userAmount[ _account];
        if (_userAmount.alpha >0 ){
            uint256 _alpha = (_userAmount.alpha * _rate)/FACTOR_PRICE_DECIMALS;
            usersWithdraw[_account].alpha += _alpha;
            amountWithdraw.alpha += _alpha;
        }

        if (_userAmount.beta >0 ){
            uint256 _beta = (_userAmount.beta * _rate)/FACTOR_PRICE_DECIMALS;
            usersWithdraw[_account].beta += _beta;
            amountWithdraw.beta += _beta;

        }

        if (_userAmount.gamma >0 ){
            uint256 _gamma = (_userAmount.gamma * _rate)/FACTOR_PRICE_DECIMALS;
    ...
    function deleteUser(address _account) internal {
        ...
        uint256 _index = userIndex[_account];
        uint256 size = users.length -1;
        users[_index] = users[size];
        users.pop();
        delete usersDeposit[_account];
        delete usersWithdraw[_account];
        delete userWeights[_account];
        delete usersDeposiTotal[_account];
        delete userRisk[_account];
        delete userIndex[_account];
        delete tokenIdPerAddress[_account];    
        delete userAmount[_account];
    ...
    function _beforeTokenTransfer(
       ...
       uint256 tokenId
        ...
        if ((to != address(0)) && (from != address(0))){
            ...
            userAmount[to] = userAmount[from];
            usersDeposit[to] = usersDeposit[from];
            usersWithdraw[to] =  usersWithdraw[from];
            userWeights[to] = userWeights[from];
            usersDeposiTotal[to] = usersDeposiTotal[from];
            userRisk[to] = userRisk[from];
            userIndex[to] = userIndex[from];
            users[userIndex[to]] = to;
            tokenIdPerAddress[to] = tokenId;
            delete usersDeposit[from];
            delete usersWithdraw[from];
            delete userWeights[from];
            delete usersDeposiTotal[from];
            delete userIndex[from];
            delete userRisk[from];
            delete userAmount[from];
