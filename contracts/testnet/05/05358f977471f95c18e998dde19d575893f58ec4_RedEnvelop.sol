...
contract RedEnvelop {
    ...
    uint256 private constant BONUS_MAMB = 2.5 ether;    // 赠送CBB币数量定义：2.5个（1 ether = 10的18次方）
    uint256 private constant LUCKY_DRAW_MAMBA = 15 ether;     // 打开转卖福袋需要CBB币数量定义：15个

    uint256 public counter;             // 福袋编号（查询福袋详细信息的时候需要传入）
    address public mambaCoinAddr;        //CBB地址   
    ...
    function initBonusCoin(address initTokenAddr) external {
        require(mambaCoinAddr == address(0), "Already Initialized");        // 只允许初始化一次就不允许任何人再修改这个地址了
...
function _calculateRandomAmount(uint256 remainMoney, uint remainCount) private view returns (uint256) {
    ...
    function luckydraw(uint256 redEnvelopId) external returns (uint256) {
        ...
        require(IERC20(mambaCoinAddr).allowance(msg.sender, address(this)) >= LUCKY_DRAW_MAMBA, "Require 24 MAMB");
        ...
        require(IERC20(mambaCoinAddr).transferFrom(msg.sender, address(this), LUCKY_DRAW_MAMBA), "Insufficient MAMB");
