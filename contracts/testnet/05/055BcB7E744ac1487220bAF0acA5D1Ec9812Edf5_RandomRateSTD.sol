...
contract RandomRateSTD is Ownable {
    ...
    function initial() public onlyOwner {
        ...
        for (uint16 p = 0; p < SWPoolPercentage[GEAR].length; p++) {
            uint256 qtyItem = (100 * SWPoolPercentage[GEAR][p]) /
                10000;
            for (uint16 i = 0; i < qtyItem; i++) {
                SWPoolValues[GEAR].push(
                    SWPoolResults[GEAR][p]
                );
            }
        }
        ...
        for (uint16 p = 0; p < SWPoolPercentage[DRO].length; p++) {
            uint256 qtyItem = (100 * SWPoolPercentage[DRO][p]) /
                10000;
            for (uint16 i = 0; i < qtyItem; i++) {
                SWPoolValues[DRO].push(
                    SWPoolResults[DRO][p]
                );
            }
        }
        ...
        for (uint16 p = 0; p < SWPoolPercentage[BOT].length; p++) {
            uint256 qtyItem = (100 * SWPoolPercentage[BOT][p]) /
                10000;
            for (uint16 i = 0; i < qtyItem; i++) {
                SWPoolValues[BOT].push(
                    SWPoolResults[BOT][p]
                );
            }
        }
