...
/*  Tako Token / $TAKO
Telegram: https://t.me/TakoToken
Website:  https://www.takotokenbsc.com/
Twitter:  https://twitter.com/takotoken
Facebook: https://www.facebook.com/takotoken
...
contract TAKO is IBEP20, Auth{
    ...
    uint8 _decimals; //Decimales del ERC20
        ...
        //uint
    uint8 internal buyTax = 1;
    uint8 internal sellTax = 1;
    uint8 internal transferTax = 1;
    ...
    uint256 maxBagBalance; //4% max wallet
    uint256 maxBallenaSplash; //1% max wallet
    ...
    constructor(
        ...
        MARKETING = 0x0e2D036c6D8121aB0b6E11B1C57D9618150E3940;
        DEV = 0x0192FAE52fb00E64EdA1a2B06d7666f96FC1Ff13;
        ...
        PAIR = IDEXFactory( dexRouter.factory() ).createPair( address(this), dexRouter.WETH() );
        ...
        maxBagBalance = _totalSupply.mul(4).div(100);
        maxBallenaSplash = _totalSupply.mul(1).div(100);
    ...
    //BlackList
    function addAntibot( address[] calldata _addresses ) public onlyOwner {
        for ( uint256 i = 0; i < _addresses.length; i++ ) {
            Antibot[_addresses[i]] = true;
        }
    }

    function removeAntibot( address _addresses ) public onlyOwner { Antibot[_addresses] = false; }
    ...
    function _transferConTax(address sender, address recipient, uint amount) private{
        ...
        if( block.timestamp < LaunchTimestamp + antiSniper || LaunchTimestamp <= 0 ){
            ...
            require( balanceOf(recipient) + amount <= maxBagBalance, "Exceeds maxBagBalance" );
    ...
    function _swapContractToken( ) private swapping{
        ...
        uint256 liquifyBNB = balanceBNB.mul( liquidityFee ).div( totalBNBFee ).div(2);
        uint256 BUSDBNB = balanceBNB.mul( rewardBUSDFee ).div( totalBNBFee );
        uint256 marketingBNB = balanceBNB.mul( marketingFee ).div( totalBNBFee );
        uint256 devBNB = balanceBNB.sub( liquifyBNB ).sub( BUSDBNB ).sub( marketingBNB );
    ...
    function SetTaxes( uint8 _buyTax, uint8 _sellTax, uint8 _transferTax, uint256 _maxBagBalance, uint256 _maxBallenaSplash ) public onlyOwner{
        uint maxTax = 100;
        require( _buyTax <= maxTax && _sellTax <= maxTax && _transferTax <= maxTax && _maxBagBalance <= maxTax && _maxBallenaSplash <= maxTax, "Honeypot" );
        
        buyTax = _buyTax;
        sellTax = _sellTax;
        transferTax = _transferTax;
        maxBagBalance = _totalSupply.mul( _maxBagBalance ).div(100);
        maxBallenaSplash = _totalSupply.mul( _maxBallenaSplash ).div(100);
    }
