 ...
 * EverSAFU Token
 * The First Multi-Reward with Sustainable Auto-Staking & Insurance Protocol
 *
 * EverSAFU Protocol
 * 
 * Website : eversafu.com
 * Dapps : dapp.eversafu.com 
...
interface IDividendDistributor {
    ...
    function setWhiteListToken(address tokenAddress, bool whitelisted) external;
    function setRewardToken(address holder, address rewardTokenAddress) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    mapping(address => bool) public tokenIsWhiteListed; // only allow whitelisted token as dividend reward
    ...
    function setWhiteListToken(address tokenAddress, bool whitelisted) external override onlyToken {
        tokenIsWhiteListed[tokenAddress] = whitelisted;
    }
    function setRewardToken(address holder, address rewardTokenAddress) external override onlyToken {
        require(tokenIsWhiteListed[rewardTokenAddress], "Reward token is not whitelisted!");
        userHasCustomRewardToken[holder] = true;
        userCurrentRewardToken[holder] = rewardTokenAddress;
    }

    function unsetRewardToken(address holder) external override onlyToken {
    ...
    // Swap BUSD -> WBNB -> REWARDTOKEN
    function swaptokenfortoken(uint amountIn, address reward, address holder)
        internal
    {
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            if(userHasCustomRewardToken[shareholder]){    
            swaptokenfortoken(amount,userCurrentRewardToken[shareholder],shareholder);
            }
            else{
...
contract eeeddddd is ERC20Detailed, Ownable {
    ...
    //total tax 12% 14%
    uint256 public insuranceFee = 25;
    uint256 public treasuryFee = 15;
    uint256 public safuDividendFee = 60;
    uint256 public sellFee = 20;
    uint256 public autofirePitFee = 20;
    uint256 public totalFee =
        insuranceFee.add(treasuryFee).add(safuDividendFee).add(
            autofirePitFee
        );
    ...
    constructor() ERC20Detailed("eeeddddd", "eeeddddd", uint8(DECIMALS)) Ownable() {
        ...
        insuranceReceiver = 0xa9494e3fF124B319D391AC5c2dC8c33820F0C69C;
        treasuryReceiver = 0xd6eAD72735d3C07638b14b0b640ff3C768c67BDd;   
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(safuDividendFee).add(insuranceFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(safuDividendFee).add(insuranceFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(insuranceReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(insuranceFee).div(
                treasuryFee.add(safuDividendFee).add(insuranceFee)
            ),
            gas: 30000
        }("");
        ...
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(safuDividendFee).div(treasuryFee.add(safuDividendFee).add(insuranceFee))
        } () {} catch {}        
    }
    ...
    function getUserCurrentRewardToken(address holder) public view returns (address){
        return distributor.userCurrentRewardToken(holder);
    }

    function getUserHasCustomRewardToken(address holder) public view returns (bool){
        return distributor.userHasCustomRewardToken(holder);
    }
    ...
    function setWhiteListToken(address tokenAddress, bool whitelisted) external onlyOwner {
        distributor.setWhiteListToken(tokenAddress, whitelisted);
    }

    function setRewardToken(address rewardTokenAddress) external {
        distributor.setRewardToken(msg.sender, rewardTokenAddress);
    }

    // Unset the reward token back to BUSD.  Call from here.
    function unsetRewardToken() public returns (bool){
        distributor.unsetRewardToken(msg.sender);
        return true;
    }

    function setMinimumSharesRequired(uint256 _minSharesRequired) external onlyOwner {
        distributor.setMinimumSharesRequired(_minSharesRequired);
