...
contract NFTHome is Context, ERC721Enumerable, Ownable, MinterAccessControl {
   ...
   */
    ...
    */
    function safeMint(address to_, uint256 tokenId_,uint8 _level) external virtual onlyMinter {
        _safeMint(to_, tokenId_,_level);
    }
    ...
    * @param tokenId_ a number of id to be minted
    */
    function mint(address to_, uint256 tokenId_,uint8 _level) external virtual onlyMinter {
        _mint(to_, tokenId_,_level);
    }
    /**
      * @notice Service function to mint nft
    *
    ...
    */
    function safeBatchMint(address to_, uint256[] memory ids_,uint8 _level) external virtual onlyMinter {
        for (uint256 index; index < ids_.length; index++) {
            _safeMint(to_, ids_[index],_level);
        }
    }

    function batchMint(address to_, uint256[] memory ids_,uint8 _level) external virtual onlyMinter {
        for (uint256 index; index < ids_.length; index++) {
            _mint(to_, ids_[index],_level);
        }
    }
    ...
    */
    function batchTransferFrom(
        ...
        for (uint256 index; index < ids_.length; index++) {
            transferFrom(from_, to_, ids_[index]);
        }
    }
    ...
    function updateClosed(bool flag) external  onlyOwner {
        closed=flag;      
    }

    function upgrade(uint256 tokenId) external onlyMinter {
        level[tokenId]+=1;
    }
    /**
