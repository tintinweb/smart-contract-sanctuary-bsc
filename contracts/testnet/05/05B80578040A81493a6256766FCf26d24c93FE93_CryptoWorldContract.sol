...
contract CryptoWorldContract is Ownable, ReentrancyGuard {
    ...
    constructor() ReentrancyGuard() {
        bonusLevel[0] = 7; // 1º nível
        bonusLevel[1] = 4; // 2º nível
        bonusLevel[2] = 3; // 3º nível
        bonusLevel[3] = 3; // 4º nível
        bonusLevel[4] = 3; // 5º nível
        bonusLevel[5] = 2; // 6º nível
        bonusLevel[6] = 2; // 7º nível
        bonusLevel[7] = 2; // 8º nível
        bonusLevel[8] = 1; // 9º nível
        bonusLevel[9] = 1; // 10º nível
        bonusLevel[10] = 1; // 11º nível
        bonusLevel[11] = 1; // 12 nível
    ...
    function _withdraw(uint256 withdrawValue) private {
        ...
        uint256 totalAcumuladoAReceber = accMapping[msgSender]
        ...
        uint256 totalSacadoCom110 = totalSacado + ((totalSacado / 100) * 10);
        if (contractSecurityLock)
            require(totalSacadoCom110 < (valorTotalRecebivelSemTaxas - ((valorTotalRecebivelSemTaxas / 100) * 2)) && bonusFidelidade == 0, "Saques temporariamente limitados");

        uint256 valorATransferir;

        uint256 fee;

        uint256 totalRecebivelLiquido100Dias = (valorTotalRecebivelSemTaxas -
            ((valorTotalRecebivelSemTaxas / 100) * 2));

        fee = (totalRecebivelLiquido100Dias / 100) * 6;

        totalRecebivelLiquido100Dias = totalRecebivelLiquido100Dias - fee;

        if (withdrawValue == 0) {
            if (
                totalRecebivelLiquido100Dias >
                (totalAcumuladoAReceber - ((totalAcumuladoAReceber / 100) * 6))
            ) {
                valorATransferir =
                    totalAcumuladoAReceber -
                    ((totalAcumuladoAReceber / 100) * 6);
                fee = (totalAcumuladoAReceber / 100) * 6;
            } else {
                uint256 valorTotalLiquidoRecebivel = _valorTotalLiquidoRecebivel(
                        valorDepositado,
                        bonusFidelidade
                    );
                fee = (valorTotalLiquidoRecebivel / 100) * 6;
                valorATransferir = valorTotalLiquidoRecebivel - fee;
            }
        } else {
        ...
        require(
            (valorATransferir + totalSacado) <= totalRecebivelLiquido100Dias,
            "Valor nao liberado, consulte o regulamento"
        );
        require(
            valorATransferir <= totalRecebivelLiquido100Dias,
            "Esta conta nao possui o valor disponivel para saque"
        );
    ...
    function _gerarBonusDiario() private {
        //bonus diário para caso a conta atinja 200%
        address msgSender = _msgSender();
        uint256 valorDepositado = accMapping[msgSender].valorDepositado;
        uint256 bonusFidelidade = accMapping[msgSender].bonusFidelidade;
    ...
    function _valorTotalRecebivelSemTaxas(
        uint256 valorDepositado,
        uint256 bonusFidelidade
    ...
    function setRenewAccount() external payable nonReentrant {
        ...
        uint256 valueReservedForGames = accMapping[msgSender]
            .valueReservedForGames;
        uint256 totalAcumuladoAReceber = accMapping[msgSender]
            .totalAcumuladoAReceber;
        uint256 totalSacado = accMapping[msgSender].totalSacado;

        uint256 tetoTotal = totalAcumuladoAReceber +
            valueReservedForGames +
            totalSacado;
    ...
    function valorControle(address _address) external view returns (uint256 totalRecebido, uint256 totalHistorico)
    {
        totalRecebido = accMapping[_address].valorControleAtual;
        totalHistorico = accMapping[_address].valorControleHistorico;
    }

    //IMPORTATE
    //mÃ©todos para controle nos testes devem ser excluÃ­dos antes de publicar
    function recuperarBnbDoContratoDuranteTestes()
        external
        payable
        onlyOwner
        nonReentrant
