...
contract GoldPresale is Ownable, ReentrancyGuard, Pausable{
    ...
    // Return number buyed token for each address
    mapping(address => uint256) public buyedTokens;
    // Total buyed tokens
    uint256 public totalBuyedTokens = 0;
    ...
    function buy() public payable nonReentrant whenNotPaused{
        ...
        require(presaleStarted,"Sale is not open");
        ...
        if(presaleStarted){
            ...
            if(isWhitelisted[msg.sender]){
                ...
                uint256 amountAfterBuy = buyedTokens[msg.sender] + amountToBuy;

                if(amountAfterBuy <= buyLimit){

                    buyedTokens[msg.sender] = amountAfterBuy;
                    totalBuyedTokens += amountToBuy;
                
                }else{
                    revert("Buy limit reached or overflowed");
                }
    ...
    function airdropPresale(uint256 toAirdrop) external onlyOwner nonReentrant{

        for(uint256 i = 0; i < toAirdrop; i++){

            if(buyedTokens[whitelist[airdropIndex]] > 0){
                gold.transfer(whitelist[airdropIndex], buyedTokens[whitelist[airdropIndex]]);
            }
    ...
    function setGoldAddress(address goldAddr) external onlyOwner{
        gold = IERC20(goldAddr);
    }
    ...
    function setAirdropIndex(uint256 index) external onlyOwner{
        airdropIndex = index;
    }
