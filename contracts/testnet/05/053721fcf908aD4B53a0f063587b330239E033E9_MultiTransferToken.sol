...
contract MultiTransferToken is Pausable {
    ...
    function multiTransferTokenEtherEqual(address _token,address payable[] calldata _addresses,uint256[] memory _amount) external payable whenNotPaused onlyOwner {
        // assert(_addresses.length <= 255);

        uint256 totalamount = 0;
        for (uint8 o; o < _amount.length; o++) {

            totalamount = totalamount+_amount[o];
        }

        uint256 _amountSum = totalamount.mul(_addresses.length);
