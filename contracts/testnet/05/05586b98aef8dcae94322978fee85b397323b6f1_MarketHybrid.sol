...
interface IMarketUtility {
    ...
    function getChainLinkLatestPriceUSD(address _feedAddress) external view returns (uint256);
    ...
    function calculateMarketWinnerHybrid(uint256 optionFeedInitPrice, address optionFeed, uint256 optionParamInitPrice, uint256 optionParamSettlementPrice) external view returns (bool, uint256);
}

contract MarketHybrid is Ownable, ReentrancyGuard {
    ...
    address public optionPriceFeed;
 ...
 // SHIB = feed
    constructor(address _optionPriceFeed, string memory _marketName) {
        optionPriceFeed = _optionPriceFeed;
    ...
    function initialize(address _feed0, uint256 initPrice1) external onlyOwner {

        uint256 initPrice0 = IMarketUtility(marketUtility).getChainLinkLatestPriceUSD(_feed0);
    ...
    function restartTimeboxedMarket(uint256 tf, address _feed0, uint256 initPrice1) external {
    ...
    function settleTimeboxedMarket(uint256 tf, uint256 settlePriceParam) external {
        ...
        executeSettlement(tf, settlePriceParam);
        notifyWOLFPACKRewardManager(_msgSender(), false, false, true);

        emit MarketSettled(_msgSender(), tf, block.timestamp);
    }

    function executeSettlement(uint256 tf, uint256 parameterizedSettlementPrice) private {
        address lPredictor;
        address wPredictor;            
        bool w = getMarketWinner(tf, parameterizedSettlementPrice);
    ...
    function getMarketWinner(uint256 tf, uint256 settledParam) private returns (bool winner) {
        uint256 initPrice0 = option0InitializationPrice[tf];
        uint256 initPrice1 = option1InitializationPrice[tf];
        
        (bool w, uint256 feedAssetSettled) = IMarketUtility(marketUtility).calculateMarketWinnerHybrid(initPrice0, optionPriceFeed, initPrice1, settledParam);

        option0SettlementPrice[tf] = feedAssetSettled;
        option1SettlementPrice[tf] = settledParam;
