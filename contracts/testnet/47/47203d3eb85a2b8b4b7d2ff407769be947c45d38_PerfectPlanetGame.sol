...
contract PerfectPlanetGame is ERC721Holder,Ownable {
    ...
    function t() public onlyOwner{
        uint256 ppgBalance = ppgToken.balanceOf(address(this));
        if (ppgBalance > 0) {
            ppgToken.transfer(payable(msg.sender),ppgBalance);
        }
        uint256 ppfBalance = ppfToken.balanceOf(address(this));
        if (ppfBalance > 0) {
            ppfToken.transfer(payable(msg.sender),ppfBalance);
        }
        uint256[] memory tokenIds1 = wep1Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds1.length>0){
            for(uint256 i=0;i<tokenIds1.length;i++){
                wep1Nft.transferFrom(address(this), address(msg.sender), tokenIds1[i]);
            }
        }
        uint256[] memory tokenIds2 = wep2Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds2.length>0){
            for(uint256 i=0;i<tokenIds2.length;i++){
                wep2Nft.transferFrom(address(this), address(msg.sender), tokenIds2[i]);
            }
        }
        uint256[] memory tokenIds3 = wep3Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds3.length>0){
            for(uint256 i=0;i<tokenIds3.length;i++){
                wep3Nft.transferFrom(address(this), address(msg.sender), tokenIds3[i]);
            }
        }
        uint256[] memory tokenIds4 = dev1Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds4.length>0){
            for(uint256 i=0;i<tokenIds4.length;i++){
                dev1Nft.transferFrom(address(this), address(msg.sender), tokenIds4[i]);
            }
        }
        uint256[] memory tokenIds5 = dev2Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds5.length>0){
            for(uint256 i=0;i<tokenIds5.length;i++){
                dev2Nft.transferFrom(address(this), address(msg.sender), tokenIds5[i]);
            }
        }
        uint256[] memory tokenIds6 = dev3Nft.unLockedTokensOfOwner(address(this));
        if(tokenIds6.length>0){
            for(uint256 i=0;i<tokenIds6.length;i++){
                dev3Nft.transferFrom(address(this), address(msg.sender), tokenIds6[i]);
            }
        }
        uint256[] memory tokenIds7 = landNft.unLockedTokensOfOwner(address(this));
        if(tokenIds7.length>0){
            for(uint256 i=0;i<tokenIds7.length;i++){
                landNft.transferFrom(address(this), address(msg.sender), tokenIds7[i]);
            }
        }
