...
contract GameAntz is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) public _maxAmountExempt;
    ...
    uint256 public maxTxAmount = 500_000_000_000e18;
    uint256 public maxWalletAmount = 2_000_000_000_000e18;
    uint256 public maxFeeLimit = 3000; // 30%
    ...
    modifier ensureMaxFeeLimit() {
        _;
        require(_taxFee[0].add(_gameFee[0]).add(_marketingFee[0]) < maxFeeLimit, "Buy Fee too high");
        require(_taxFee[1].add(_gameFee[1]).add(_marketingFee[1]) < maxFeeLimit, "Sell Fee too high");
        require(_taxFee[2].add(_gameFee[2]).add(_marketingFee[2]) < maxFeeLimit, "P2P Fee too high");
    }
    ...
    constructor(
        ...
        //Exempt maxTxAmount
        _maxAmountExempt[_owner] = true;
        _maxAmountExempt[address(this)] = true;
        _maxAmountExempt[marketingWallet] = true;
        _maxAmountExempt[gameWallet] = true;

        //Exempt maxWalletAmount
        _maxWalletExempt[_owner] = true;
        _maxWalletExempt[address(this)] = true;
        _maxWalletExempt[marketingWallet] = true;
        _maxWalletExempt[gameWallet] = true;
    ...
    function _transfer(
        ...
        require(
            isTaxless[sender] ||
                isTaxless[recipient] ||
                amount <= maxTxAmount ||
                _maxAmountExempt[sender] ||
                _maxAmountExempt[recipient],
            "Max Transfer Limit Exceeds!"
        );

        if (!_maxWalletExempt[recipient]) {
            uint256 heldTokens = balanceOf(recipient);
            require(
                (heldTokens + amount) <= maxWalletAmount,
    ...
    function setTaxFee(
        ...
        uint256 sell,
        uint256 p2p
    ) external onlyOwner ensureMaxFeeLimit {
        _taxFee[0] = buy;
        _taxFee[1] = sell;
        _taxFee[2] = p2p;
    }
    ...
    function setMaxTxAmount(uint256 amount) external onlyOwner {
        maxTxAmount = amount;
        require(maxTxAmount > 100_000e18, "Too low");
    }

    function setMaxWalletAmount(uint256 _amount) external onlyOwner {
        maxWalletAmount = _amount;
        require(maxWalletAmount > 100_000e18, "Too low");
    }
    ...
    function exemptFromMaxTx(address _address, bool value) external onlyOwner {
        _maxAmountExempt[_address] = value;
    }

    function exemptFromMaxWallet(address _address, bool value) external onlyOwner {
        _maxWalletExempt[_address] = value;
    }
