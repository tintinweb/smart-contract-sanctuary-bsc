...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    require(ownerOf(_tokenId) != address(0),"Owner of token id mismatch");
    _;
  }
...
contract SPARCEDependency {
  ...
  modifier onlyWhitelistSetter() {
    require(msg.sender == whitelistSetterAddress,"Only whitelist setter");
    _;
  }
  ...
  modifier whenSpawningAllowed(uint256 _genes, address _owner) {
    require(
      address(spawningManager) == address(0) ||
        spawningManager.isSpawningAllowed(_genes, _owner),"only when spawning allowed"
    );
    _;
  }
  ...
  modifier whenRebirthAllowed(uint256 _sparceId, uint256 _genes) {
    require(
      address(spawningManager) == address(0) ||
        spawningManager.isRebirthAllowed(_sparceId, _genes),"Only when rebirth allowed"
    );
    _;
  }
  ...
  modifier whenRetirementAllowed(uint256 _sparceId, bool _rip) {
    require(
      address(retirementManager) == address(0) ||
        retirementManager.isRetirementAllowed(_sparceId, _rip),"only when retirement allowed"
    );
    _;
  }
  ...
  modifier whenEvolvementAllowed(uint256 _sparceId, uint256 _newGenes) {
    require(
      address(geneManager) == address(0) ||
        geneManager.isEvolvementAllowed(_sparceId, _newGenes),"Only when evolvement allowed"
    );
    _;
  }

  modifier onlySpawner() {
    require(whitelistedSpawner[msg.sender],"Only White List spawnner");
    _;
  }

  modifier onlyByeSayer() {
    require(whitelistedByeSayer[msg.sender],"only bye sayer");
    _;
  }

  modifier onlyGeneScientist() {
    require(whitelistedGeneScientist[msg.sender],"Only whitelist gene scientist");
    _;
  }
