...
contract TestPTP is ERC20 {
    ...
    function updateDailyData(uint256 beginDay,uint256 endDay) internal{
        dailyData memory dailyRecord = dailyDataUpdation[lastUpdatedDay];
        if(lastUpdatedDay == 0){
            beginDay = 0;
        }
        for(uint256 i = beginDay; i<= endDay; i++){
            uint256 dailyLimit =getDailyShare(i) ;
        dailyData memory myRecord = dailyData({dayPayout:dailyLimit,stakeShareTotal:dailyRecord.stakeShareTotal,unclaimed:dailyRecord.unclaimed});
