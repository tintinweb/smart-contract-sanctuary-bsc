...
interface IMYTESTTOKEN is IBEP20 {
...
contract MYTESTTOKEN is ReentrancyGuard, IMYTESTTOKEN {
	...
	// Underlying Asset
	address public _tokenPEG = 0x7B8085ce388E360232a1a51F59b52427852b80a5; // CHANGE THIS AFTER PEG UPLOAD

	// Balances
	mapping (address => uint256) _userBalance;
	mapping (address => mapping (address => uint256)) _userAllowance;
	mapping (address => uint256) public _volumeFor;
	...
	constructor () {
		...
		// Send 1 myTestToken to DEAD, Ensure Never 0 _supplySELF
    ...
    function getMintFee() external view returns (uint256) {
        uint256 token = 100;
        return token.mul((feeDivisor.sub(_mintFee)).div(feeDivisor));
    }

    // Get Burn Fee
    function getBurnFee() external view returns (uint256) {
        uint256 token = 100;
        return token.mul((feeDivisor.sub(_burnFee)).div(feeDivisor));
    }

    // Get Market Fee
    function getMarketFee() external view returns (uint256) {
        uint256 token = 100;
        return token.mul((feeDivisor.sub(_marketFee)).div(feeDivisor));
    }
	...
	// Calculate Current Price Of 1 myTestToken Token
	...
	// Stake Underlying Asset, Get myTestToken To Different Wallet : Needs Approval
	...
	function _stakeUnderlying(uint numTokens, address recipient) internal returns (bool) {
		...
		// Move Peg From User To myTestToken Contract
    ...
    // Sells myTestToken and Sends Back Underlying Asset
	...
	// Update PEG Address
    function updatePegAddress(address newPeg) external onlyOwner {
        _tokenPEG	= newPeg;
        emit UpdatedMarketAddress(newPeg);
    }
    ...
    function setMintFee(uint256 newMintFee) external onlyOwner {
        ...
        require(newMintFee > 90000 && newMintFee < 100001, "New Fee Out Of Range");
    ...
    function setBurnFee(uint256 newBurnFee) external onlyOwner {
        ...
        require(newBurnFee > 90000 && newBurnFee < 100001, "New Fee Out Of Range");
    ...
    function setMarketFee(uint256 newMarketFee) external onlyOwner {
        ...
        require(newMarketFee > 90000 && newMarketFee < 100001, "New Fee Out Of Range");
