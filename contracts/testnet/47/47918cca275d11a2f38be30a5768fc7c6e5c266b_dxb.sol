...
contract dxb is Context, IERC20, Ownable {
    ...
    string private _symbol = "dubai123";
    ...
    address public buybackWallet = 0x5AA1dCA1EF60afBc2DDb215de890aD4b50615b2C;
    uint256 private _previousbuybackFee = _buybackFee;
    
    uint256 public _DevelopmentFee = 2;
    address public DevelopmentWallet = 0x15119e6C8aEc26ed729716b91A38f180C0209Aa5;
    uint256 private _previousDevelopmentFee = _DevelopmentFee;

    uint256 public _marketingFee = 5;
    address public marketingWallet = 0xD15fD685ca92ba5E7d8440a9569D052f3Fee457d;
    ...
    uint256 public maxWalletToken = 3450000000000 * (10**9);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _buybackFee == 0 && _DevelopmentFee == 0 && _marketingFee == 0 && _BurnFee == 0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate Development and buyback amount
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(buybackAmt).sub(DevelopmentAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(buybackAmt).sub(DevelopmentAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(buybackAmt).sub(DevelopmentAmt).sub(marketingAmt).sub(BurnAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(buybackAmt).sub(DevelopmentAmt).sub(marketingAmt).sub(BurnAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(buybackAmt).sub(DevelopmentAmt).sub(marketingAmt).sub(BurnAmt)));
        }
        
        //Temporarily remove fees to transfer to Development and buyback wallet
        _taxFee = 0;
        _liquidityFee = 0;

        //Send transfers to Development and buyback wallet
