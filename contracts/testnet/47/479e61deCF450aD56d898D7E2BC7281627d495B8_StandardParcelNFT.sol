    ...
    ReentrancyGuardUpgradeable
{
    ...
    return  this.tokensOf(_user)[0];//(tests, upgradedParcelsMapping[this.tokensOf(_user)[1]]);
    }
    ...
    function getUpgradedParcelIds(address _user) external view returns(uint256[] memory) {
       ...
       for (uint256 i = 0; i < this.tokensOf(_user).length; i++) {
            if (upgradedParcelsMapping[this.tokensOf(_user)[i]]) {
                upgradedParcesIds[i] = this.tokensOf(_user)[i];
            }
       }
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mintMiners(uint256 _count, uint256 _referrerId) external onlySaleActive notZeroAddress returns(uint256, uint256)
    {
        ...
        require(IERC721Base(standardParcelNFTAddress).balanceOf(msg.sender) > 0, "Municipality: At first it is necessary to have a parcel");
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
