...
contract SMT is Context, IBEP20, Ownable {
    ...
    uint8 public buyMainLPRewardsFee = 3;
    uint8 public buyDeadFee = 4;

    address public minerAddress = 0xDE9D0B8980Fd815b3b4C0253eb0730eFf4EB03f5;//minerAddress to claim coins
    uint256 public mineStartTime =0;//mine starting timestamp(seconds
    address public powerAddress = 0xECA9feAa7324DF2B54931d95B75e953E8E394eD4;//power Address to do withdraw jobs
    
    address public marketingWalletAddress = 0x0000000000000000000000000000000000000001;//marketing wallet
    address public mainLPRewardsWalletAddress = 0x0000000000000000000000000000000000000001;//LP rewards wallet
    ...
    address public contractUSDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;//test 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684 main 0x55d398326f99059fF775485246999027B3197955
    address public contractSME = 0xD7a9a43459F79E090951F4F43508F9A865e194ec;//tPDT 0xD7a9a43459F79E090951F4F43508F9A865e194ec  sme test 0xe30ED8556C97b5095eaDe548BC2C6b984f4545cf 
    ...
    function setBuyMainLPRewardsFee(uint8 num) external  returns (uint8){
        require(owner() == msg.sender,"SMT: owner only");
        buyMainLPRewardsFee = num;
        return buyMainLPRewardsFee;
    }
    ...
    function setMainLPRewardsWalletAddress(address add) external  returns (address){
        require(owner() == msg.sender,"SMT: owner only");
        mainLPRewardsWalletAddress = add;
        return mainLPRewardsWalletAddress;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(sender == liquidityPoolAddress){
            //if transfer from liquidity pool check buy switch 
            require(buyOnOff == 1, "SMT: can not by");
        }
        ...
        if(transferFeeOnOff == 2 || whiteList[sender] || whiteList[recipient]){
            ...
            uint256 LPFee;//LP rewards fee

            //LP/swap 
            if(sender == liquidityPoolAddress || recipient == liquidityPoolAddress){
                LPFee = amount.mul(buyMainLPRewardsFee).div(100);
                MFee = amount.mul(buyMarketingFee).div(100);
                DFee = amount.mul(buyDeadFee).div(100);
                fees = MFee.add(DFee).add(LPFee);

                amount = amount.sub(fees);
                if(DFee > 0) emit Transfer(sender, deadWallet, DFee);
                if(MFee > 0) emit Transfer(sender, marketingWalletAddress, MFee);
                if(LPFee > 0) emit Transfer(sender, mainLPRewardsWalletAddress, LPFee);
