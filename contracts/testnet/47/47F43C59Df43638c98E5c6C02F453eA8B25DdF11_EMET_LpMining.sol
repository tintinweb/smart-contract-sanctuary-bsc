...
contract EMET_LpMining is ERC721Holder, Ownable, RandomGenerator{
    ...
    struct UserInfo_Pos {
        ...
        uint depositTime;
        uint lastLockRewardTime;
        uint lastClaimtTime;
        uint rewardEndLockTime;
    }
    ...
    function updataLockReward_Pos(uint pid_, address user_) view public returns (uint _temp) {
        ...
        if(user.lockReward != 0) {
            if (block.timestamp < user.rewardEndLockTime){
                _temp = user.lockReward * (block.timestamp - user.lastLockRewardTime) / sustainedReleaseTime;
            } if (block.timestamp >= user.rewardEndLockTime){
                if (user.lastLockRewardTime < user.rewardEndLockTime) {
                    _temp = user.lockReward * (user.rewardEndLockTime - user.lastLockRewardTime) / sustainedReleaseTime;
                } else {
                    _temp = 0;
                }
            }
    ...
    function claim_Pos(uint256 pid_) public {
        ...
        uint _reward;
        if (user.amount > 0) {
            _reward = updataUserReward_Pos(pid_, msg.sender);
        }         
        uint reward = _reward;
        ...
        uint _temp1;
        if (user.lockReward != 0) {
            _temp1 = updataLockReward_Pos(pid_, msg.sender);
            _reward += _temp1;
            reward = _reward;
        }
        ...
        user.lastLockRewardTime = block.timestamp;
        // user lock
        if (user.amount != 0) {
            user.rewardEndLockTime = block.timestamp + sustainedReleaseTime;
    ...
    function withdraw_Pos(uint256 pid_) public {
        PoolInfo_Pos storage pool = poolInfo_Pos[pid_];
        UserInfo_Pos storage user = userInfo_Pos[pid_][msg.sender];     
        ...
        if (amountOut > 0 ){
            ICnnPair(pool.lpToken).transfer(address(msg.sender), amountOut);
        }
        updatePool_Pos(pid_);
        user.amount = 0;
    ...
    function updataLockReward_Share(uint pid_, address user_) view public returns(uint _temp){
        UserInfo_Share storage user = userInfo_Share[pid_][user_];
        // if (user.lockReward != 0){
        //     _temp = user.lockReward * (block.timestamp - user.lastClaimtTime) / (user.rewardEndLockTime - user.lastClaimtTime);
        // }
        if(user.lockReward != 0) {
            if (block.timestamp < user.rewardEndLockTime){
                _temp = user.lockReward * (block.timestamp - user.lastLockRewardTime) / sustainedReleaseTime;
    ...
    function claim_Share(uint256 pid_)public returns(bool){
        ...
        uint _temp1;
        if (user.lockReward != 0) {
            _temp1 = updataLockReward_Share(pid_, msg.sender);
            _reward += _temp1;
            reward = _reward;
        }
    ...
    // Withdraw LP tokens from Contract.
    function withdraw_Share(uint256 pid_) public returns(bool){
        UserInfo_Share storage user = userInfo_Share[pid_][msg.sender];
        PoolInfo_Share storage pool = poolInfo_Share[pid_];   
        require(user.amount > 0, "withdraw: amount not good");
        uint amountOut = user.amount;
    ...
    event ClaimBoxAll(address indexed user, uint256 indexed pid, uint256 indexed amount);
    ...
    function checkUserReward(uint type_, uint pid_, address user_) public view returns(uint _re) {
        ...
        if (type_ == 0 ) {
            ...
            if (user_Pos.lockReward != 0) {
                _temp1 = user_Share.lockReward * (block.timestamp - user_Pos.lastClaimtTime) / (user_Pos.rewardEndLockTime - user_Pos.lastClaimtTime);
                _re += _temp1;
            }
        } else if (type_ == 1 ) {
            _re = updataUserReward_Share(pid_, user_) + user_Share.toClaim;
            if (user_Share.lockReward != 0) {
                _temp1 = user_Share.lockReward * (block.timestamp - user_Share.lastClaimtTime) / (user_Share.rewardEndLockTime - user_Share.lastClaimtTime);
                _re += _temp1;
            }
        } else if (type_ == 2) {
