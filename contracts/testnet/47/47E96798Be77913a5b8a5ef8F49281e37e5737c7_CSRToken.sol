...
contract CSRToken is Context, IERC20, Ownable {
    ...
    uint256 private _totalSupply = 33300 * 10**uint256(_decimals);
    ...
    constructor (address receiveAddress, address routerAddress_, address usdtAddress_) public {
        ...
        _isExcludedFromFee[_lpAddr] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if( _isSwapPair[sender] &&  !_isExcludedFromFee[recipient] ){
            require(_openBuy == true, "Trade not open");
        }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        !_isExcludedFromFee[recipient]
            ...
            if (tLiquidity > 0) {
                ...
                lpFeeTotal = lpFeeTotal.add(tLiquidity);
                emit Transfer(sender, _lpAddr, tLiquidity);
            }

            if (tBurn > 0) {
                _balances[_burnAddr] = _balances[_burnAddr].add(tBurn);
    ...
    function updateBurnAddr(address addr) public onlyOwner {
        _burnAddr = addr;
    }
    ...
    function setBuyStatus(bool result) public onlyOwner {
        _openBuy = result;
    }
