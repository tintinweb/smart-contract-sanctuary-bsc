...
contract CCMRouterV2 is TaxableRouter, UUPSUpgradeable {
    ...
    /// Used to return the slippage needed.
    /// @param sentIn Sent amount.
    /// @param successful Whether we were able to determine a slippage or not.
    /// @param slippage Slippage needed to execute the swap.
    error EstimatedSlippage (uint sentIn, bool successful, uint8 slippage);
    function estimateSlippage(
        uint exactAmountIn,
        address[] calldata path
    ) external returns(uint8 slippage) {
        uint[] memory amounts = IPancakeRouter02(pcsRouter).getAmountsOut(exactAmountIn, path);
        (uint[] memory swapAmounts, ,) = _getSwapInfos(exactAmountIn, path);
        uint shouldGet = amounts[amounts.length - 1];
        uint reallyGot = swapAmounts[swapAmounts.length - 1];

        slippage = uint8(reallyGot * 100 / shouldGet);
    }
