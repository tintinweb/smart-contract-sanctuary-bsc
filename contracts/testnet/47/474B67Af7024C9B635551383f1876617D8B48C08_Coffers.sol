...
contract Coffers is OwnableUpgradeable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public oldPoolTotals;
    ...
    mapping (uint => uint256) public stakedFeeRates;
    ...
    function init(
        ...
        unlockedAllocated[1] = 500000000000000000000;
        unlockedAllocated[2] = 500000000000000000000;
        unlockedAllocated[3] = 495000000000000000000;
        unlockedAllocated[4] = 500000000000000000000;
        unlockedAllocated[5] = 500000000000000000000;
        unlockedAllocated[6] = 495000000000000000000;
        unlockedAllocated[7] = 497000000000000000000;
        unlockedAllocated[8] = 500000000000000000000;
        unlockedAllocated[9] = 495000000000000000000;
        unlockedAllocated[10] = 500000000000000000000;
        unlockedAllocated[11] = 495000000000000000000;
        unlockedAllocated[12] = 498000000000000000000;
        unlockedAllocated[13] = 494000000000000000000;
        unlockedAllocated[14] = 497000000000000000000;
        unlockedAllocated[15] = 495000000000000000000;
        unlockedAllocated[16] = 500000000000000000000;
        unlockedAllocated[17] = 493000000000000000000;
        unlockedAllocated[18] = 495000000000000000000;
        unlockedAllocated[19] = 494000000000000000000;
        unlockedAllocated[20] = 500000000000000000000;
        unlockedAllocated[21] = 25912000000000000000000;
        ...
        feeBlock[0] = 200;//28800
        feeBlock[1] = 1400;//201600
        feeBlock[2] = 2600;//374400
        feeBlock[3] = 3000;//403200
    }
    ...
    // Update the old pool deposited totals
    function updateOldPoolTotals(uint256 _pid, uint256 _amount) public onlyOwner {
        oldPoolTotals[_pid] = _amount;
    }

    // Calculate the pool value
    function getTotalPoolValueWeighted() public view returns (uint256 totalPoolAlloc) {
      for (uint256 i=0; i < poolToken.length; i++) { 
        totalPoolAlloc = totalPoolAlloc.add((poolToken[i].lpToken.balanceOf(address(this)).add(oldPoolTotals[i])).mul(weightPercent).div(weightPercent.sub(poolToken[i].poolWeight)));
      }
    }
    ...
    function getUserInfo(uint256 _pid, address _address) public view returns (IERC20 lpToken,uint256 amount,uint256 depositTime,uint256 depositBlock,
        uint256 pendingUnlocked,uint256 pendingLocked,uint256 lockedAmount,uint256 vestedAmount,uint lastRewardBlock) {
        UserInfo memory user = userInfo[_pid][_address];

        return (user.lpToken,user.amount,user.depositTime,user.depositBlock,
        user.pendingUnlocked,user.pendingLocked,user.lockedAmount,user.vestedAmount,user.lastRewardBlock);
    }

    function updateUnlockedAllocated(uint256 _era, uint256 _amount) public onlyOwner {
        unlockedAllocated[_era] = _amount;
    }

    function updateUnlockedClaimed(uint256 _era, uint256 _amount) public onlyOwner {
        unlockedClaimed[_era] = _amount;
    }
    ...
    function updateUser(uint256 _pid, address _userAddress, uint256 _oDepositTime, uint256 _oDepositBlock, uint256 _oLastRewardBlock) public onlyOwner {
    ...
    // Function to verify the staker
    function verifyStaker(address _staker, bool _verify) public onlyOwner {
	  verified[_staker] = _verify;
	}

    // Update the withdraw fee percentage
    function updateStakedFeeRates(uint256 _item, uint256 _amount) public onlyOwner {
        stakedFeeRates[_item] = _amount;
    }

    // Update the withdraw fee blocks
    function updateFeeBlock(uint256 _item, uint256 _block) public onlyOwner {
        feeBlock[_item] = _block;
    }
    ...
    function getWithdrawFeePercent(uint256 _blockNumber) public view returns(uint _feePercent) {
        uint256 currentEra = getEraNumber(_blockNumber);
        uint256 distance = 0;
        
        if(currentEra > numberOfEras) { 
            _feePercent = 0;
        } else {
            distance = _blockNumber.sub(startBlock.add(currentEra.sub(1).mul(blocksPerEra)));
            
            if(distance <= feeBlock[0]) {
                _feePercent = stakedFeeRates[0];
            } else if(distance <= feeBlock[1]) {
                _feePercent = stakedFeeRates[1];
            } else if(distance <= feeBlock[2]) {
                _feePercent = stakedFeeRates[2];
            } else {
                _feePercent = stakedFeeRates[3];
            }
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        require(verified[msg.sender]==true, "You have not been verified");
        ...
        uint withdrawFeePercent = getWithdrawFeePercent(blockNumber);
        uint256 currentEra = getEraNumber(blockNumber);
        ...
        for(uint256 i=1; i<=(numberOfEras+1);i++) {
            ...
            if (_amount==0) {
                unlockedClaimed[i] = unlockedClaimed[i] + rewardsByEra[i];
            }
        }
