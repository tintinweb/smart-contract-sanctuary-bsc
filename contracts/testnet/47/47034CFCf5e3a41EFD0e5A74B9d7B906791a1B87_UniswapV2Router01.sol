...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'UniswapV2Router01: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'UniswapV2Router01: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = UniswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'UniswapV2Router01: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'UniswapV2Router01: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'UniswapV2Router01: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            IUniswapV2Pair(UniswapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'UniswapV2Router01: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'UniswapV2Router01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, UniswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'UniswapV2Router01: INVALID_PATH');
        amounts = UniswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'UniswapV2Router01: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, UniswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]);
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'UniswapV2Router01: INVALID_PATH');
        amounts = UniswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'UniswapV2Router01: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(UniswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) public view override returns (uint amountOut) {
        return UniswapLibrary.getAmountOut(amountIn, reserveIn, reserveOut, IUniswapV2Factory(factory).feeBP());
    }

    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public view override returns (uint amountIn) {
        return UniswapLibrary.getAmountOut(amountOut, reserveIn, reserveOut, IUniswapV2Factory(factory).feeBP());
    }
...
interface IUniswapV2Factory {
    ...
    function feeBP() external view returns (uint);
    function feeToAddress() external view returns (address);
    function feeToPercent() external view returns (uint);
    ...
    function setFeeBP(uint) external;
    function setFeeToAddress(address) external;
    function setFeeToPercent(uint) external;
}
...
library UniswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'bb86ccf1efbbe65250ace00b63971069a20f6d5fd6f3d195d4cfd49e51903176' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint feeBP) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(uint(10000).sub(feeBP));
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint feeBP) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'UniswapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'UniswapLibrary: INSUFFICIENT_LIQUIDITY');
        uint numerator = reserveIn.mul(amountOut).mul(10000);
        uint denominator = reserveOut.sub(amountOut).mul(uint(10000).sub(feeBP));
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        amounts[0] = amountIn;
        uint feeBP = IUniswapV2Factory(factory).feeBP();
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, feeBP);
        }
    }
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        for (uint i = path.length - 1; i > 0; i--) {
            ...
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, feeBP);
        }
    }
}
