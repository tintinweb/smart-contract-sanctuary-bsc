...
contract PawSwap is Ownable {
    ...
    function addTokenTax (uint256 amount) private view returns (uint256) {
        ...
        if (_taxes.liquidityTax != 0) {
            amountPlusTax += amount * (_taxes.liquidityTax / 2) / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.burnTax != 0) {
            amountPlusTax += amount * _taxes.burnTax / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.tokenTax != 0) {
            amountPlusTax += amount * _taxes.tokenTax / 10**(_taxes.feeDecimal + 2);
        }
    ...
    function addEthTax (uint256 amount) private view returns (uint256) {
        uint256 amountPlusTax = amount;
        if (_taxes.tax1 != 0) {
            amountPlusTax += amount * _taxes.tax1 / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.tax2 != 0) {
            amountPlusTax += amount * _taxes.tax2 / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.tax3 != 0) {
            amountPlusTax += amount * _taxes.tax3 / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.tax4 != 0) {
            amountPlusTax += amount * _taxes.tax4 / 10**(_taxes.feeDecimal + 2);
        }
        if (_taxes.liquidityTax != 0) {
            amountPlusTax += amount * (_taxes.liquidityTax / 2) / 10**(_taxes.feeDecimal + 2);
        }
