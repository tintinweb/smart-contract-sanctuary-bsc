...
contract contractB {
  ...
  function deposit(uint tokens) public {
      ...
      require(time - last_time_deposit >= 30*60, 'wait (30-time/60) minutes to deposit');
    last_time_deposit = time;
  ...
  function getBalance (address token) public view returns(uint256) {
      return ERC20(tracker_0x_address).balanceOf(token);
  }

  function getBalanceDepositOfSender() public view returns (uint256) {
