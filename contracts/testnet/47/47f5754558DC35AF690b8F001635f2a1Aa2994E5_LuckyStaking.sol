...
contract LuckyStaking {
    ...
    function getEstimatedPayout(address user) public view returns (uint256){
            uint256 reward=currentClaimableDividendsNoFees(user);
    ...
    function currentClaimableDividendsNoFees (address user) public view returns (uint256){
            ...
            if(reward>(dailyDividendsAvailable())){
                uint256 newReward=(1*(((CNR.balanceOf(address(this))-totalDeposits)*(stake[user]/(totalDeposits/10000)))/divideByDays))/10000;
                return newReward;
            } else {   
            return reward;
            }
    ...
    function claimDividends() public{
        if((lastClaimed[msg.sender]+waitBlocks)<block.number){
            uint256 reward=currentClaimableDividendsNoFees(msg.sender);
            uint256 rewardPostFees=reward-((reward/100)*(ownerFee+dividendFee));
