...
/**
 * @title XplusETHBridge
 *
 * @dev An upgradeable contract for moving XPT gas token and XPT ERC20 tokens between XSC 
 * and other blockchains. An off-chain relayer is responsible for signing proofs of deposits 
 * to be used on destination chains of the transactions. A multi-relayer set up can be used 
 ...
 */
contract XplusETHBridge is OwnableUpgradeable {
    ...
    event NativeTokenCodeUpdated(bytes32 _nativeTokenCode);
    event RelayerChanged(address oldRelayer, address newRelayer);
    event ChainSupportForXptUpdated(uint256 chainId, bool supported);

    bytes32 public nativeTokenCode;
    ...
    function __XplusEthBridge_init(address _relayer, bytes32 _nativeTokenCode) public initializer {
        __Ownable_init();
        ...
        nativeTokenCode = _nativeTokenCode;
    }

    receive() external payable {}

    function updateNativeTokenCode(bytes32 _nativeTokenCode) external onlyOwner {
        nativeTokenCode = _nativeTokenCode;
        emit NativeTokenCodeUpdated(_nativeTokenCode);
    }

    function updateChainSupportForToken(uint256 chainId, bool supported) external onlyOwner {
        tokenSupportedOnChain[nativeTokenCode][chainId] = supported;
        emit ChainSupportForXptUpdated(chainId, supported);
    }
    ...
    function deposit(
        ...
        require(amount > 0, "XplusETHBridge: amount must be positive");
        require(destChainId != currentChainId, "XplusETHBridge: dest must be different from src");
        require(
            isTokenSupportedOnChain(nativeTokenCode, destChainId),
            "XplusETHBridge: token not supported on dest chain"
        );
        require(recipient != 0, "XplusETHBridge: zero address");
    ...
    function withdraw(
        ...
        require(destChainId == currentChainId, "XplusETHBridge: wrong chain");
        require(!withdrawnDeposits[srcChainId][depositId], "XplusETHBridge: already withdrawn");
        require(recipient != 0, "XplusETHBridge: zero address");
        require(amount > 0, "XplusETHBridge: amount must be positive");
        require(currency == nativeTokenCode, "XplusETHBridge: currency must be native token");
        ...
        require(recoveredAddress == relayer, "XplusETHBridge: invalid signature");

        withdrawnDeposits[srcChainId][depositId] = true;

        address decodedRecipient = address(uint160(uint256(recipient)));

        require(payable(decodedRecipient).send(amount), "XplusETHBridge: transfer failed");
    ...
    function _setRelayer(address _relayer) private {
        require(_relayer != address(0), "XplusETHBridge: zero address");
        require(_relayer != relayer, "XplusETHBridge: relayer not changed");
