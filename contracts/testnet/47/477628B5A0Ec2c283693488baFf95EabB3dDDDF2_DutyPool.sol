...
import "./interfaces/IVDuty.sol";

contract DutyPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 dutyAtLastUserAction; // keep track of duty deposited at the last user action.
    ...
    uint256 public performanceFee = 100; // 2%
    uint256 public performanceFeeContract = 100; // 2%
    ...
    event NewVDutyContract(address VDuty);
     ...
     * @param _pid: duty pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(dutyPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the duty owner address or the operator address.
     */
    modifier onlyOperatorOrDutyOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or duty owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onDutyPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user duty funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrDutyOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Duty Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VDuty != address(0)) {
            IVDuty(VDuty).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.dutyAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Duty Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VDuty != address(0)) {
            IVDuty(VDuty).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.dutyAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
    ...
    /**
     * @notice Harvest pending DUTY tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingDuty = masterchefV2.pendingDuty(dutyPoolPID, address(this));
        if (pendingDuty > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(dutyPoolPID, 0);
    ...
    /**
     * @notice Set VDuty Contract address
     * @dev Callable by the contract admin.
     */
    function setVDutyContract(address _VDuty) external onlyAdmin {
        require(_VDuty != address(0), "Cannot be zero address");
        VDuty = _VDuty;
        emit NewVDutyContract(VDuty);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Duty Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingDutyRewards();
     ...
     */
    function calculateTotalPendingDutyRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingDuty(dutyPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingDutyRewards()) * (1e18)) / totalShares);
    }
