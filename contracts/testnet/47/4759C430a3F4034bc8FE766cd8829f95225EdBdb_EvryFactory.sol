...
contract EvryPair is IEvryPair, EvryERC20 {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        require(
            amount0 > 0 && amount1 > 0,
            "Evry: INSUFFICIENT_LIQUIDITY_BURNED"
        );
    ...
    function swap(
        ...
        FeeConfiguration memory feeConfiguration;
        {
            ...
            (
                address _feeToPlatform,
                uint256 _feePlatformBasis,
                uint256 _feeLiquidityBasis
            ) = IEvryFactory(factory).getFeeConfiguration();
        ...
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        require(
            feeConfiguration.amount0Out < _reserve0 &&
                feeConfiguration.amount1Out < _reserve1,
        ...
        uint256 balance1;
        {
            ...
            if (feeConfiguration.amount0Out > 0)
                _safeTransfer(_token0, to, feeConfiguration.amount0Out); // optimistically transfer tokens
            if (feeConfiguration.amount1Out > 0)
                _safeTransfer(_token1, to, feeConfiguration.amount1Out); // optimistically transfer tokens
            if (data.length > 0)
                IEvryCallee(to).evryCall(
                    msg.sender,
                    feeConfiguration.amount0Out,
                    feeConfiguration.amount1Out,
                    data
                );
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        uint256 amount0In = balance0 > _reserve0 - feeConfiguration.amount0Out
            ? balance0 - (_reserve0 - feeConfiguration.amount0Out)
            : 0;
        uint256 amount1In = balance1 > _reserve1 - feeConfiguration.amount1Out
            ? balance1 - (_reserve1 - feeConfiguration.amount1Out)
            : 0;
        uint256 amount0Out = amountOut[0] | 0;
        uint256 amount1Out = amountOut[1] | 0;
        require(
...

        {
            ...
            uint256 totalFee = feeConfiguration.feePlatformBasis.add(
                feeConfiguration.feeLiquidityBasis
            );
            uint256 balance0Adjusted = balance0.mul(10000).sub(
                amount0In.mul(totalFee)
            );
            uint256 balance1Adjusted = balance1.mul(10000).sub(
                amount1In.mul(totalFee)
            );
            require(
    ...
    function sendFeeToPlatform(
        address token,
        uint256 amount,
        uint256 feePlatformBasis,
        address feeToPlatform
    ) private {
        uint256 denominator = amount.mul(feePlatformBasis);
        uint256 platformFeeAmount = denominator / 10000;
    ...
    function getBasisTotalFee() internal view returns (uint256 totalFee) {
        (
            ,
            uint256 _feePlatformBasis,
            uint256 _feeLiquidityBasis
