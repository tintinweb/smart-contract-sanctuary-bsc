...
contract Catpay is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if(!takeFee && isActiveFees){
            ...
            uint256 takeLiquidity_fee = _takeLiquidity(receivedAmount);
            uint256 takeMarketing_fee = _takeMarketing(receivedAmount);
            uint256 takeBurn_fee = _takeBurn(receivedAmount);
            uint256 total_fee = takeLiquidity_fee.add(takeMarketing_fee).add(takeBurn_fee);

            _balances[recipient] = _balances[recipient].add(receivedAmount.sub(total_fee));
