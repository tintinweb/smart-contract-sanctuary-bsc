...
abstract contract HVSBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	// MAIN TOKEN PROPERTIES
	string private constant NAME = "Hallvers";
	string private constant SYMBOL = "HVS";
    ...
    address private _marketingWallet = 0x1176b05f6E73c84Ab75A82f5e4411f24188FcbAc;
    address private _devAddress = 0x1176b05f6E73c84Ab75A82f5e4411f24188FcbAc;
	...
	constructor (address routerAddress, address pinkAntiBot_) {
		...
		// Initialize PancakeSwap V2 router and HVS <-> BNB pair.
		setPancakeSwapRouter(routerAddress);

		_maxHoldAmount = 1200000 * 10**DECIMALS;

		// 2% liquidity fee, 2% reward fee, 2% buyback fee, 2% marketing fee, 1% dev fee
		setFees(2, 2, 2, 2, 1);
		_highBuyFee = 99;
	...
	function doTransfer(address sender, address recipient, uint256 amount) internal virtual {
		require(sender != address(0), "HVS: Transfer from the zero address is not allowed");
		require(recipient != address(0), "HVS: Transfer to the zero address is not allowed");
		// Only use PinkAntiBot if this state is true
		if (antiBotEnabled) {
		pinkAntiBot.onPreTransferCheck(sender, recipient, amount);
		}
		require(amount > 0, "HVS: Transfer amount must be greater than zero");
		require(!isPancakeSwapPair(sender) || _isBuyingAllowed, "HVS: Buying is not allowed before contract activation");

		if (_isSwapEnabled) {
			// Ensure that amount is within the limit in case we are selling
			if (isSellTransferLimited(sender, recipient)) {
				require(amount <= _transactionSellLimit, "HVS: Sell amount exceeds the maximum allowed");
			}

			// Ensure that amount is within the limit in case we are buying
			if (isPancakeSwapPair(sender)) {
				require(amount <= _transactionBuyLimit, "HVS: Buy amount exceeds the maximum allowed");
			}
		}
		...
		if (applyTokenHold) {
			require(_balances[recipient] + transferAmount < _maxHoldAmount, "HVS: Cannot hold more than Maximum hold amount");
		}
	...
	function doApprove(address owner, address spender, uint256 amount) private {
		require(owner != address(0), "HVS: Cannot approve from the zero address");
		require(spender != address(0), "HVS: Cannot approve to the zero address");
	...
	function executeSwap(uint256 amount) private {
		...
		uint256 tokensReservedForReward = amount - tokensReservedForLiquidity - tokensReservedForBuyback - tokensReservedForMarketing - tokensReservedForDev;
		...
		uint256 tokensToSwap = tokensReservedForReward + tokensToSwapForLiquidity + tokensReservedForBuyback + tokensReservedForMarketing + tokensReservedForDev;
		...
		require(sent, "HVS: Failed to send BNB to marketing wallet");
		...
		require(sent, "HVS: Failed to send BNB to dev wallet");
		
		emit Swapped(tokensToSwap, bnbSwapped, tokensToAddAsLiquidity, bnbToBeAddedToLiquidity);
	}

	// This function swaps a {tokenAmount} of HVS tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for HVS -> WBNB
	...
	function swapBNBForCustomeTokens(address token, uint256 bnbAmount, address to) internal returns(bool) {
		// Generate pair for WBNB -> selected Token
		address[] memory path = new address[](2);
		path[0] = _pancakeswapV2Router.WETH();
		path[1] = token;
	...
	// Returns how many more $HVS tokens are needed in the contract before triggering a swap
	...
	function setPancakeSwapRouter(address routerAddress) public onlyOwner {
		require(routerAddress != address(0), "HVS: Cannot use the zero address as router address");
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 devFee) public onlyOwner {
		require(liquidityFee >= 0 && liquidityFee <= 5, "HVS: Liquidity fee must be between 0% and 5%");
		require(rewardFee >= 0 && rewardFee <= 5, "HVS: Reward fee must be between 1% and 5%");
		require(buybackFee >= 0 && buybackFee <= 5, "HVS: Buyback fee must be between 0% and 5%");
		require(marketingFee >= 0 && marketingFee <= 3, "HVS: Marketing fee must be between 0% and 3%");
		require(devFee >= 0 && devFee <= 2, "HVS: Dev fee must be between 1% and 2%");
		...
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee + _devFee;
	}
	...
	function transactionSellLimit() external view returns (uint256) {
	...
	function transactionBuyLimit() external view returns (uint256) {
		return _transactionBuyLimit;
	}

	function setHoldLimit(uint256 limit) external onlyOwner {
	...
	function setTokenSwapThreshold(uint256 threshold) external onlyOwner {
		require(threshold > 0, "HVS: Threshold must be greater than 0");
		_tokenSwapThreshold = threshold;
	}

	function tokenSwapThreshold() external view returns (uint256) {
	...
	function autoLiquidityWallet() external view returns (address) {
	...
	function setDevWallet(address devWalletAddress) external onlyOwner {
		_devAddress = devWalletAddress;
	}

	function totalFeesPooled() external view returns (uint256) {
		return _totalFeesPooled;
	}

	function totalBNBLiquidityAddedFromFees() external view returns (uint256) {
	...
	function isTokenHoldEnabled() external view returns (bool) {
	...
	function isExcludedFromHold(address addr) external view returns(bool) {
		return _addressesExcludedFromHold[addr];
	}


	function setExcludedFromHold(address addr, bool value) external onlyOwner {
...
contract HVS is HVSBase {
	...
	uint256 private _totalBNBAsHVSClaimed; // The total number of BNB that was converted to HVS and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsHVSClaimed; // The amount of BNB converted to HVS and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy HVS automatically
	...
	bool private _reimburseAfterHVSClaimFailure; // If true, and HVS reward claim portion fails, the portion will be given as BNB instead
	...
	constructor (address routerAddress, address pinkAntiBot_) HVSBase(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterHVSClaimFailure(true);
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		require(!_isBlacklisted[sender], "HVS: Account is blacklisted");
		require(!_isBlacklisted[recipient], "HVS: Account is blacklisted");
	...
	function claimReward(address user) public isHuman nonReentrant {
		require(msg.sender == user || isClaimApproved(user, msg.sender), "HVS: You are not allowed to claim rewards on behalf of this user");
		require(isRewardReady(user), "HVS: Claim date for this address has not passed yet");
		require(isIncludedInRewards(user), "HVS: Address is excluded from rewards, make sure there is enough HVS balance");

		bool success = doClaimReward(user);
		require(success, "HVS: Reward claim failed");
	}
	...
	function doClaimReward(address user) private returns (bool) {
        ...
        // Claim HVS tokens
		if (!claimHVS(user, claimBnbAsTokens)) {
			// If token claim fails for any reason, award whole portion as BNB
			if (_reimburseAfterHVSClaimFailure) {
	...
	function claimHVS(address user, uint256 bnbAmount) private returns (bool) {
		if (bnbAmount == 0) {
			return true;
		}
		...
		_bnbAsHVSClaimed[user] += bnbAmount;
		_totalBNBAsHVSClaimed += bnbAmount;
		return true;
	}
	...
	function processRewardClaimQueue(uint256 gas) public nonReentrant {
		require(gas > 0, "HVS: Gas limit is required");
	...
	function processRewardClaimQueueAndRefundGas(uint256 gas) external {
		require(_whitelistedExternalProcessors[msg.sender], "HVS: Not whitelisted - use processRewardClaimQueue instead");
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $HVS on a zero balance address:
		...
		// Depending on the % of $HVS tokens transferred, relative to the balance, we might need to extend the period
	...
	function buyAndBurn(uint256 bnbAmount) external onlyOwner {
		require(bnbAmount <= address(this).balance / 100, "HVS: Manual burn amount is too high!");
		require(bnbAmount > 0, "HVS: Amount must be greater than zero");
	...
	function totalAmountOfTokensHeld() public view returns (uint256) {
  	...
  	function bnbRewardClaimed(address byAddress) external view returns (uint256) {
		return _bnbRewardClaimed[byAddress];
	}

  	function bnbRewardClaimedAsHVS(address byAddress) external view returns (uint256) {
		return _bnbAsHVSClaimed[byAddress];
	}

  	function totalBNBClaimed() external view returns (uint256) {
		return _totalBNBClaimed;
	}

  	function totalBNBClaimedAsHVS() external view returns (uint256) {
		return _totalBNBAsHVSClaimed;
	}
	...
	function setRewardCyclePeriod(uint256 period) external onlyOwner {
		require(period >= 3600 && period <= 86400, "HVS: RewardCycle must be updated to between 1 and 24 hours");
	...
	function nextAvailableClaimDate(address ofAddress) external view returns (uint256) {
		return _nextAvailableClaimDate[ofAddress];
	}

	function maxClaimAllowed() external view returns (uint256) {
		return _maxClaimAllowed;
	}

	function setMaxClaimAllowed(uint256 value) external onlyOwner {
		require(value > 0, "HVS: Value must be greater than zero");
		_maxClaimAllowed = value;
	}

	function minRewardBalance() external view returns (uint256) {
	...
	function maxGasForAutoClaim() external view returns (uint256) {
	...
	function isExcludedFromRewards(address addr) external view returns (bool) {
		return _addressesExcludedFromRewards[addr];
	}

	// Will be used to exclude unicrypt fees/token vesting addresses from rewards
	function setExcludedFromRewards(address addr, bool isExcluded) external onlyOwner {
		_addressesExcludedFromRewards[addr] = isExcluded;
		updateAutoClaimQueue(addr);
	}

	function globalRewardDampeningPercentage() external view returns(uint256) {
		return _globalRewardDampeningPercentage;
	}

	function setGlobalRewardDampeningPercentage(uint256 value) external onlyOwner {
		require(value <= 90, "HVS: Cannot be greater than 90%");
		_globalRewardDampeningPercentage = value;
	}

	function approveClaim(address byAddress, bool isApproved) external {
		require(byAddress != address(0), "HVS: Invalid address");
	...
	function isRewardAsTokensEnabled() external view returns(bool) {
	...
	function gradualBurnMagnitude() external view returns (uint256) {
		return _gradualBurnMagnitude;
	}

	function setGradualBurnMagnitude(uint256 magnitude) public onlyOwner {
		require(magnitude <= 100, "HVS: Must be equal or less to 100");
		_gradualBurnMagnitude = magnitude;
	}

	function gradualBurnTimespan() external view returns (uint256) {
		return _gradualBurnTimespan;
	}

	function setGradualBurnTimespan(uint256 timespan) external onlyOwner {
		require(timespan >= 5 minutes, "HVS: Cannot be less than 5 minutes");
		_gradualBurnTimespan = timespan;
	}

	function claimRewardAsTokensPercentage(address ofAddress) external view returns(uint256) {
		return _claimRewardAsTokensPercentage[ofAddress];
	}

	function setClaimRewardAsTokensPercentage(uint256 percentage) external {
		require(percentage <= 100, "HVS: Cannot exceed 100%");
		_claimRewardAsTokensPercentage[msg.sender] = percentage;
	}

	function mainBnbPoolSize() external view returns (uint256) {
		return _mainBnbPoolSize;
	}

	function setMainBnbPoolSize(uint256 size) external onlyOwner {
		require(size >= 10 ether, "HVS: Size is too small");
	...
	function reimburseAfterHVSClaimFailure() external view returns(bool) {
		return _reimburseAfterHVSClaimFailure;
	}

	function setReimburseAfterHVSClaimFailure(bool value) public onlyOwner {
		_reimburseAfterHVSClaimFailure = value;
	}

	function lastBurnDate() external view returns(uint256) {
	...
	function isWhitelistedExternalProcessor(address addr) external view returns(bool) {
		return _whitelistedExternalProcessors[addr];
	}

	function setWhitelistedExternalProcessor(address addr, bool isWhitelisted) external onlyOwner {
		 require(addr != address(0), "HVS: Invalid address");
	...
	function setExcludeNonHumansFromRewards(bool exclude) external onlyOwner {
		_excludeNonHumansFromRewards = exclude;
	}
	
	function setBlacklistAccount(address account) external onlyOwner {
		require(!_isBlacklisted[account], "HVS: Account is already blacklisted");
		_isBlacklisted[account] = true;
		emit BlacklistChange(account, true);
	}

	function unsetBlacklistAccount(address account) external onlyOwner {
		require(_isBlacklisted[account], "HVS: Account is not blacklisted");
		_isBlacklisted[account] = false;
    emit BlacklistChange(account, false);
	}

	function setAntiBotEnabled(bool _isEnabled) external onlyOwner {
	...
	function updateBlockNum(uint256 _blockNum) external onlyOwner {
