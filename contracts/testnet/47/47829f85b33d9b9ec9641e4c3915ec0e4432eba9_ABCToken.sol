...
contract ABCToken is Context, IERC20, Ownable {
    ...
    uint256 public _devCollected=0;
    uint256 public _mktCollected=0;
    uint256 public _wildlifeCollected=0;
    ...
    function sendBNBToFee(uint256 _amount) private {
        ...
        _devAddress.transfer(_devCollected);
        _mktAddress.transfer(_mktCollected);
        _wildlifeAddress.transfer(_wildlifeCollected);
        _devCollected=0;
        _mktCollected=0;
        _wildlifeCollected=0;
    }
    
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if (!takeFee) removeAllFee();
        //_countUpFeeShare(amount);
        _devCollected += (amount * _devFee)/100;
        _mktCollected += (amount * _mktFee)/100;
        _wildlifeCollected += (amount * _wildlifeFee)/100;
