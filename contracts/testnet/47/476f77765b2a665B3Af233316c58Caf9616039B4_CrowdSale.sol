...
contract CrowdSale is Ownable {
    ...
    event Pledge(address indexed caller, address indexed referral, uint256 amount, uint256 accrued);
    ...
    function setTask(uint task) public onlyOwner {
        REFUND_TASK = task;
    }
    ...
    function pledge(uint256 amount) external beforeDeadline returns (bool) {
        ...
        require(pledgedAmount[msg.sender] <= MAX_STAKE_AMOUNT, "over max amount");
        usdt.safeTransferFrom(msg.sender, address(this), amount);
        amountRaised += amount;

        (uint8 count, address[] memory relations) = freeDao.getRelations(msg.sender);
        address firstReferral = count > 0 ? relations[0] : address(0);
        emit Pledge(msg.sender, firstReferral, amount, pledgedAmount[msg.sender]);

        // caculte referral task and send reword to referral
        if (count > 0 && firstReferral != address(0)) {
            Referral storage ref = referrals[firstReferral];
            ref.teamMembers.push(msg.sender);
            // only msg.sender peledge arrive maxamount
            if (pledgedAmount[msg.sender] >= MAX_STAKE_AMOUNT) {
                ref.count += 1;
                ref.maxStakers.push(msg.sender);
            }
            // hold more than 1000 hash free
            // not yet claim
            // already invite 10 people which pledge maxamount
            uint256 bal = pledgedAmount[firstReferral];
            if (
                bal >= MAX_STAKE_AMOUNT &&
                ref.count >= REFUND_TASK &&
                !ref.claimed
            ) {
                reFound += MAX_STAKE_AMOUNT;
                ref.claimed = true;
                // refund usdt
                usdt.safeTransfer(firstReferral, MAX_STAKE_AMOUNT);
                // send nft
                medal.safeMint(firstReferral);
                // freecoin send, remember approve
                IERC20(address(freeDao)).safeTransferFrom(
                    finance,
                    firstReferral,
                    REWARD_FREE_COIN
                );
                emit MissionComplete(firstReferral);
            }
        }
    }
    ...
    function completeMission() external {
        ...
        require(
            pledgedAmount[msg.sender] >= MAX_STAKE_AMOUNT,
            "not enough pledged"
        );
        reFound += MAX_STAKE_AMOUNT;
        ref.claimed = true;
        // refund usdt
        usdt.safeTransfer(msg.sender, MAX_STAKE_AMOUNT);
        // send nft
        medal.safeMint(msg.sender);
        // freecoin send
        IERC20(address(freeDao)).safeTransferFrom(
            finance,
            msg.sender,
        ...
        emit MissionComplete(msg.sender);
    }

    // get claim hashfree though merkleproof
    ...
    function getTeamMembers(address leader) public view returns (address[] memory members, address[] memory maxStaker){
        Referral memory ref = referrals[leader];
        return (ref.teamMembers, ref.maxStakers);
    }
