...
contract CatCoin is Context, IERC20, Ownable {
    ...
    function _takeLiquidity(address sender,uint256 amount_) private returns(uint256){
        uint256 fee = liquidityFee.mul(amount_).div(100);
        _balances[address(this)] = _balances[address(this)].add(fee);
        emit Transfer(sender, address(this), fee);
        return fee;
    }
    function _takeMarketing(address sender, uint256 amount_) private returns(uint256){
        uint256 fee = 0;
        if(marketingAddress != 0x000000000000000000000000000000000000dEaD){
            fee = marketingFee.mul(amount_).div(100);
            _balances[marketingAddress] = _balances[marketingAddress].add(fee);
            emit Transfer(sender, marketingAddress, fee);
            return fee;
        }
        else{
            return fee;
    ...
    function _takeBurn(address sender, uint256 amount_) private returns(uint256){
        uint256 fee = burnFee.mul(amount_).div(100);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if(!takeFee && isActiveFees){
            ...
            liquidityFeeAmount = _takeLiquidity(sender,tAmount);
            marketingFeeAmount = _takeMarketing(sender,tAmount);
            burnAmount = _takeBurn(sender,tAmount);
            receivedAmount= tAmount.sub(liquidityFeeAmount.add(marketingFeeAmount.add(burnAmount)));
    ...
    function tglburn() external onlyOwner{
        if(burnFee == 0){
            burnFee = 2;
        }
        else{
