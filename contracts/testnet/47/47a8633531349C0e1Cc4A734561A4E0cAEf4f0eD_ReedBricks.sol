...
contract ReedBricks is Context, IERC20, Ownable {
    ...
    // done from funds in this wallet
    ...
    // percentage of the fee we send to Liquidity Pool
    uint256 public _liquidityFee;
    uint256 private _previousLiquidityFee;

    // Protect fee from team getting too greedy
    // only (_treasuryFee + _rewardFee + _liquidityFee) <= 20%
    uint256 constant feeCap = 20;
        ...
        whenSigned
    {
        ...
        require(
            ...
            "REED: Can only increase maxTxPercent"
        );
    ...
    function _transfer(
        ...
        if (automatedMarketMakerPairs[to] && !_isExcludedFromFee[from]) {
            uint256 maxTxAmount = _tTotal.mul(maxTxPercent).div(100);
            require(
                amount <= maxTxAmount,
                "REED: amount exceeds [maxTxPercent]"
            );
        }
    ...
    // Enable buyback
    function setBuyBackEnabled(bool _enabled) external onlyOwner whenSigned {
        require(_enabled != buyBackEnabled, "REED: Value is same as old");
    ...
    function setAutoLPBurn(bool _Enabled)
        external
        onlyOwner
        whenSigned
        returns (bool)
    ...
    // send all token to Treasury
    // send trapped BNB to Treasury
    ...
    // Get all funds to the treasury
    function fundTreasury() external onlyOwner whenSigned {
        payable(treasurer).transfer(address(this).balance);
    }

    // calculate the total reeds accruing to holder
    // based on the rate on local sales
    function settleReed(address _account, uint256 _rate)
        external
        onlyOwner
        returns (uint256)
    {
        uint256 _deposit = deposits[_account];
        uint256 _reedTotal = _deposit.mul(_rate);
        _tokenTransfer(address(this), _account, _reedTotal, false);
        deposits[_account] = deposits[_account].sub(_deposit);
        return _reedTotal;
    }
}