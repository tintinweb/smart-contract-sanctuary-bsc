...
contract Terrarium is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public rumDividendToken;
    ...
    bool public sendRUMInTx = true;

    USDTDividendTracker public usdtDividendTracker;
    RUMDividendTracker public rumDividendTracker;
    ...
    uint256 public previousUSDTRewardsFee;
    
    uint256 public buybackRewardsFee = 2;
    uint256 public previousBuyBackRewardsFee;

    uint256 public marketingFee = 3;
    uint256 public previousMarketingFee;
    uint256 public LiquidityFee = 2;
    uint256 public previousLiquidityFee;
    uint256 public totalFees = USDTRewardsFee.add(buybackRewardsFee).add(marketingFee).add(LiquidityFee);

    uint256 public sellFeeIncreaseFactor = 110;
    uint256 public usdtDividendPriority = 80;
    ...
    event UpdateRUMDividendTracker(address indexed newAddress, address indexed oldAddress);
    ...
    event ProcessedRUMDividendTracker(
        uint256 iterations,
        uint256 claims,
        uint256 lastProcessedIndex,
        bool indexed automatic,
    ...
    constructor() ERC20("Terrarium", "RUM") {

        usdtDividendTracker = new USDTDividendTracker();
        rumDividendTracker = new RUMDividendTracker();

        marketingWallet = 0x72f79b8CEA58fBd5135aE1be9BD9E52B7781D4Fc;
        liquidityWallet = 0x96A4dDfDF1694D715B0f1CA36a2231c33D5C96c2;

        usdtDividendToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
        usdtDividendTracker.setDividendTokenAddress(usdtDividendToken);

        rumDividendToken = address(this);
        rumDividendTracker.setDividendTokenAddress(rumDividendToken);
        ...
        excludeFromDividend(address(rumDividendTracker));
        ...
        excludeFromFees(address(rumDividendTracker), true);
    ...
    function updateMinRUMBeforeSendDividend(uint256 _newAmount) external onlyOwner {
        rumDividendTracker.setMinTokenBeforeSendDividend(_newAmount);
    }

    function getMinRUMBeforeSendDividend() external view returns (uint256) {
        return rumDividendTracker.minTokenBeforeSendDividend();
    }
    ...
    function setUSDTDividendPriority(uint256 _newAmount) external onlyOwner {
        require(_newAmount >= 0 && _newAmount <= 100, "Error amount");
        usdtDividendPriority = _newAmount;
    }
    ...
    function setLiquifyEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = LiquidityFee.add(marketingFee).add(USDTRewardsFee).add(buybackRewardsFee);
            LiquifyEnabled = _enabled;
        }
    ...
    function setUSDTDividendEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = USDTRewardsFee.add(marketingFee).add(buybackRewardsFee).add(LiquidityFee);
            usdtDividendEnabled = _enabled;
        }
    ...
    function setBuyBackEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            previousBuyBackRewardsFee = buybackRewardsFee;
            buybackRewardsFee = 0;
            buybackEnabled = _enabled;
        } else {
            buybackRewardsFee = previousBuyBackRewardsFee;
            totalFees = buybackRewardsFee.add(marketingFee).add(USDTRewardsFee).add(LiquidityFee);
            buybackEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(USDTRewardsFee).add(buybackRewardsFee).add(LiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateUSDTDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newUSDTDividendTracker.owner() == address(this), "must be owned by Main Token Contract");
    ...
    function updateRUMDividendTracker(address newAddress) external onlyOwner {
        RUMDividendTracker newRUMDividendTracker = RUMDividendTracker(payable(newAddress));

        require(newRUMDividendTracker.owner() == address(this), "must be owned by Main Token Contract");

        newRUMDividendTracker.excludeFromDividends(address(newRUMDividendTracker));
        newRUMDividendTracker.excludeFromDividends(address(this));
        newRUMDividendTracker.excludeFromDividends(address(uniswapV2Router));
        newRUMDividendTracker.excludeFromDividends(address(deadAddress));

        emit UpdateRUMDividendTracker(newAddress, address(rumDividendTracker));

        rumDividendTracker = newRUMDividendTracker;
    }

    function updateUSDTRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        USDTRewardsFee = newFee;
        totalFees = USDTRewardsFee.add(marketingFee).add(buybackRewardsFee).add(LiquidityFee);
    }

    function updateRUMDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        buybackRewardsFee = newFee;
        totalFees = buybackRewardsFee.add(USDTRewardsFee).add(marketingFee).add(LiquidityFee);
    }
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        require(newFee <= MAX_FEE_RATE, "wrong");
        LiquidityFee = newFee;
        totalFees = LiquidityFee.add(USDTRewardsFee).add(buybackRewardsFee).add(marketingFee);
    }
    ...
    function excludeFromDividend(address account) public onlyOwner {
        ...
        rumDividendTracker.excludeFromDividends(address(account));
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        ...
        if(value) {
            ...
            rumDividendTracker.excludeFromDividends(pair);
        }
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance) external onlyOwner {
        ...
        rumDividendTracker.updateMinimumTokenBalanceForDividends(newMinimumBalance);
    }

    function updateClaimWait(uint256 claimWait) external onlyOwner {
        usdtDividendTracker.updateClaimWait(claimWait);
        rumDividendTracker.updateClaimWait(claimWait);
    }
    ...
    function getRUMClaimWait() external view returns(uint256) {
        return rumDividendTracker.claimWait();
    }

    function getTotalRUMDividendsDistributed() external view returns (uint256) {
        return rumDividendTracker.totalDividendsDistributed();
    }
    ...
    function withdrawableRUMDividendOf(address account) external view returns(uint256) {
        return rumDividendTracker.withdrawableDividendOf(account);
    }
    ...
    function RUMDividendTokenBalanceOf(address account) external view returns (uint256) {
        return rumDividendTracker.balanceOf(account);
    }
    ...
    function getAccountRUMDividendsInfo(address account)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return rumDividendTracker.getAccount(account);
    }
    ...
    function getAccountRUMDividendsInfoAtIndex(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return rumDividendTracker.getAccountAtIndex(index);
    }
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        ...
        (uint256 rumIterations, uint256 rumClaims, uint256 rumLastProcessedIndex) = rumDividendTracker.process(gas);
        emit ProcessedRUMDividendTracker(rumIterations, rumClaims, rumLastProcessedIndex, false, gas, tx.origin);
    }
    ...
    function claim() external {
        ...
        rumDividendTracker.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastRUMDividendProcessedIndex() external view returns(uint256) {
        return rumDividendTracker.getLastProcessedIndex();
    }
    ...
    function getNumberOfRUMDividendTokenHolders() external view returns(uint256) {
        return rumDividendTracker.getNumberOfTokenHolders();
    }
    ...
    function _transfer(
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (usdtDividendEnabled) {
                    uint256 sellTokens = contractTokenBalance.div(totalFees).mul(USDTRewardsFee);
                    swapAndSendUSDTDividends(sellTokens);
                }

                if (buybackEnabled) {
                    uint256 buybackTokens = contractTokenBalance.div(totalFees).mul(buybackRewardsFee);
                    swapTokensForBuybackDividends(buybackTokens);
                    ...
                    buyBackForDividends(buyBackBalanceBnb);
                 ...
                 if (marketingEnabled) {
                    ...
                    transferToWallet(payable(marketingWallet), marketingBNB);
        ...
        try rumDividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
        try usdtDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
        try rumDividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}

        if(!swapping) {
            uint256 gas = gasForProcessing;

            if(rand() <= usdtDividendPriority) {
                ...
                if(buybackEnabled && sendRUMInTx ){
                    try rumDividendTracker.process(gas) returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {
                        emit ProcessedRUMDividendTracker(iterations, claims, lastProcessedIndex, true, gas, tx.origin);
                    }
                    catch {
    ...
    function swapTokensForBuybackDividends(uint256 tokenAmount) private {
    ...
    function buyBackForDividends(uint256 amount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);

        uint256 initialBalance = balanceOf(address(rumDividendTracker));

        // make the swap
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(
            0, // accept any amount of Tokens
            path,
            address(rumDividendTracker), // receiver address
            block.timestamp.add(300)
        );

        uint256 swappedBalance = balanceOf(address(rumDividendTracker)).sub(initialBalance);
        distributeRUMDividends(rumDividendTracker, swappedBalance);
       
    }

    function manualBuyBackAndDistributeDividends(uint256 _amount) public onlyOwner {
    ...
    function distributeRUMDividends(DividendPayingToken dividendPayingTracker, uint256 amount) private {
...
contract RUMDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    using IterableMapping for IterableMapping.Map;
    ...
    constructor() DividendPayingToken("RUM_Dividend_Tracker", "RUM_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }
