...
import '../interfaces/ITalienNft.sol';
 ...
 */
	...
	event BuySaleNFT(address indexed buyer, uint256 indexed tokenId, uint256 saleId);
	event BuyAuctionNFT(address indexed buyer, uint256 indexed tokenId, uint256 auctionId);
	event PlaceBid(
		uint256 auctionId,
 ...
 */
	 ...
	 * @param _erc721Address indicates the ERC721 NFT contract address
	 */
	function initialize(address _erc721Address) external virtual initializer {
		require(_erc721Address != address(0), 'Market: INVALID_NFT_CONTRACT');
		...
		erc721 = ITalienNft(_erc721Address);
 ...
 */
	...
	modifier onlyValidTokenId(uint256 _tokenId) {
		require(_tokenId > 0 && _tokenId <= erc721.getCurrentTokenId(), 'Market:INVALID_PROFILE_ID');
		_;
	}
 ...
 */
	...
	function sellProfileNFT(
	...
	) external virtual onlyValidTokenId(_tokenId) nonReentrant returns (uint256 saleId) {
		...
		saleId = _sellNFT(_tokenId, _nftPrice, _tokenAddress);
	}
	...
	function createProfileNFTAuction(
		...
		address _tokenAddress,
		uint256 _duration
	) external virtual onlyValidTokenId(_tokenId) nonReentrant returns (uint256 auctionId) {
		require(_initialPrice > 0, 'Market: INVALID_INITIAL_NFT_PRICE');

		//get nft copy from sender and put it in auction
		erc721.transferFrom(msg.sender, address(this), _tokenId);

		auctionId = _createAuction(_tokenId, _initialPrice, _tokenAddress, _duration);
	}
		...
		nonReentrant
	{
		...
		erc721.transferFrom(address(this), msg.sender, _sale.tokenId);
		...
		nonReentrant
	{
		...
		erc721.transferFrom(address(this), msg.sender, _auction.tokenId);
		...
		returns (uint256 saleId)
	{
		...
		//create sale
		saleId = _sellNFT(_auction.tokenId, _sellingPrice, _auction.currency);
	}
   ...
   */
	function buyNFT(uint256 _saleId) external virtual onlyValidSaleId(_saleId) nonReentrant {
		...
		emit BuySaleNFT(msg.sender, _sale.tokenId, _saleId);
	}
		...
		nonReentrant
	{
		...
		erc721.transferFrom(address(this), bid[_auction.winningBidId].bidderAddress, _auction.tokenId);

		//close auction
		_auction.status = 0;
		_auction.buyTimestamp = block.timestamp;

		emit BuyAuctionNFT(bid[_auction.winningBidId].bidderAddress, _auction.tokenId, _auctionId);
	}
 ...
 */
	 ...
	 */
	function checkRoyalties(address _contract) external view virtual returns (bool) {
 ...
 */
	...
	function _sellNFT(
		...
		sale[saleId] = SaleInfo(msg.sender, address(0), _tokenId, _nftPrice, _tokenAddress, 0, 0);

		userSaleIds[msg.sender].push(saleId);
		userTotalSales[msg.sender] += 1;
	...
	function _createAuction(
		...
		userTotalAuctions[msg.sender] += 1;
