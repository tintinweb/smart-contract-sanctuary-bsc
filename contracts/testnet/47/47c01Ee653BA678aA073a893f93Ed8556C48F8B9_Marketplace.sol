...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    mapping(string => Concept) private s_concepts;
    ...
    modifier notListed(string memory concept, uint256[] memory tokenIds) {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            Concept memory _concept = s_concepts[concept];
            if (
                listing.price > 0 &&
                _concept.start_date < block.timestamp &&
                _concept.end_date > block.timestamp
            ) {
    ...
    function listItem(
        ...
        s_concepts[concept] = Concept(start_date, end_date);
    ...
    function getConeptDate(string memory concept)
        external
        view
        returns (Concept memory)
    {
        return s_concepts[concept];
    }
