...
contract DogyRace_1 is ERC721Enumerable, Ownable {
  ...
  function LobbyTier1(uint256 IdTrack, uint256 _ID) public payable {
    ...
    require( _Metadata[_ID].Tier == 1,"Your token has the wrong Tier 1" );

    if(numTier1<LobbyAmount){
        BasePlayer memory newBasePlayer;    
        newBasePlayer.place = numTier1;
        newBasePlayer.Address = payable(msg.sender);
        newBasePlayer.ID = _ID;    
        newBasePlayer.Time = Formula(_Metadata[_ID].Weight,_Metadata[_ID].Endurance,
        _Metadata[_ID].Speed,_Metadata[_ID].Strength);  

        _BasePlayer[IdTrack][numTier1] = newBasePlayer;
        numTier1++;
  ...
  function Formula (uint256 Weight, uint256 Endurance, uint256 Speed,
    uint256 Strength) internal view returns (uint256 Time){
    uint256 _Time =Track/(Speed - Strength + Endurance - Weight);
    return (_Time);
  }   
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= 2,"You can mint a maximum of 2 pieces");
  ...
  function addMetadata(
    ...
    uint256 Endurance2,
    uint256 Speed2,
    uint256 Strength2) public onlyOwner {

    Metadata memory newMetadata; 
    newMetadata.ID = ID;
    newMetadata.Tier = Tier;
    newMetadata.Weight = Weight;
    newMetadata.Endurance =Endurance;
    newMetadata.Speed = Speed;
    newMetadata.Strength = Strength;

    _Metadata[ID]=newMetadata;

    Metadata memory newMetadata2; 
    newMetadata2.ID = ID2;
    newMetadata2.Tier = Tier2;
    newMetadata2.Weight = Weight2;
    newMetadata2.Endurance =Endurance2;
    newMetadata2.Speed = Speed2;
    newMetadata2.Strength = Strength2;

    _Metadata[ID2]=newMetadata2;
  }

  function setCommission(address payable _newCommission) public onlyOwner {
