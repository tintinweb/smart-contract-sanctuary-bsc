...
contract CommunityPoolClubSwaps is Ownable{
    ...
    address tax_distribution;
        ...
        constructor(address cpc, address _tax_distribution, uint256 _tax_rate, uint256 _conversion_rate){
                token = IERC20(cpc);
                tax_distribution = _tax_distribution;
                tax_rate = _tax_rate;
        ...
        function swapCPC(uint256 value) public onlyRegistered {
                require(failsafe[msg.sender] == false, "CommunityPoolClub : User Blacklisted, reach out the support");
                cpc_swapped[msg.sender].add(value);
                require(token.transferFrom(msg.sender, address(this), value), "CommunityPoolClub : Unable to proceed to the transfer");
                require(token.transfer(tax_distribution, value.div(conversion_rate)), "CommunityPoolClub : Impossible to transfer the taxes");
          ...
          function setConversionRate(uint256 _rate) public onlyManager {
            ...
            function setTaxWallet(address wallet) public onlyManager {
              ...
              function recoverLostToken(address _token) onlyManager external returns (bool) {
                  require(IERC20(_token) != token, "CommunityPoolClub : Cannot recover CPC tokens");
  ...
  modifier onlyRegistered() {
        require(registered_users[msg.sender] == true, "CommunityPoolClub : This address is not registered");
        _;
    }
