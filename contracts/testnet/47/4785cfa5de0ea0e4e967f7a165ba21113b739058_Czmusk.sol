...
contract Czmusk {
    ...
    function _deposit(address _user, uint256 _amount, bool isSplitDept) private {
        ...
        if(_amount>=2000e18 && userInfo[_user].incomeArray[10]>=2000e18){
            userInfo[_user].totalIncome+=2000e18;
            userInfo[_user].incomeArray[10]-=2000e18;
        }else{
            unfreezeDepts(_user);
        }
    }
            ...
            if(last20==false){
                ...
                if(i==0){
                    ...
                    if(userInfo[_referral].isStar==1 && i < 3){
                        userInfo[_referral].incomeArray[10]+=levelOn.mul(level_bonuses[i]).div(baseDivider);
    ...
    function unfreezeDepts(address _addr) private {
        ...
        for(uint i=0;i<userDepts[_addr].length;i++){
            ...
            if(pl.isUnfreezed==false && block.timestamp>=pl.unfreeze && isdone==0 && pl.depTime>last70Time){
                ...
                userInfo[_addr].totalIncome+=pl.amount.mul(195).div(1000);
                userInfo[_addr].incomeArray[0]+=pl.amount;
                userInfo[_addr].incomeArray[1]+=pl.amount.mul(195).div(1000);
                isdone=1;
    ...
    function withdraw(uint256 _amount) public{
        ...
        uint256 bonus;
        uint256 dirSplit;
        if(player.incomeArray[10]>=1000e18){
            player.totalIncome+=player.incomeArray[10];
            dirSplit=player.incomeArray[10].mul(30).div(100);
        }
