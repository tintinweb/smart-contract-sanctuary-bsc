...
contract Tree is Ownable {
    ...
    function addTree(address ref) public returns(bool){
        ...
        countPlayers ++;
        push(usersArr, msg.sender);
        return true;
    }

    function getdAllUsers() view public returns (Users [] memory) {
        uint256 length = countPlayers;
        Users[] memory myNft = new Users[](length);
        uint count = 0;

        for (uint256 index = 0; index < countPlayers; index++) {
            myNft[count++] = _users[usersArr[index]];
        }
        return myNft;
    }
