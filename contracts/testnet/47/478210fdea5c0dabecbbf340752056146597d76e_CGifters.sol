...
contract CGifters {
    ...
    IBEP20 public tokenTrx;
    address public owner;
    address payable public master_wallet;
    
    uint256[] public moveup = [1,3];
    uint256 constant public higher_level=50;  
	...
	function getTokenBalance() public view returns (uint256) {
       return tokenTrx.balanceOf(address(this));
    }	
    ...
    function depositFund(uint256 amount) external {     
      tokenTrx.safeTransferFrom(msg.sender,address(this),amount);
      emit TransferredToken(msg.sender,amount);      
    }  
    ...
    function register(uint uplineid,uint256[] memory parametrs) public payable {
       ...
       if(users[msg.sender].id <= 0){
		...
		users[master_wallet].id = userId;	
		users[master_wallet].sponsorid=uplineid;	
		users[master_wallet].pposition = refcount;
        users[master_wallet].cur_level = cur_level;
	    users[master_wallet].depositTime =block.timestamp;	
		...
		tokenTrx.transferFrom(msg.sender,address(this),totalAmount);
		
		if(refcount==moveup[0] || refcount==moveup[1]){
		    _tranefertoupline(upline,msg.sender,package_amount,cur_level); 
		} else {
	...
	function _tranefertoupline(address receiver,address sender,uint256 amount,uint256 level) internal{ 
	    tokenTrx.transfer(receiver,amount);
    ...
    function withdrawLostTokens() external OnlyOwner {        
        require(tokenTrx.balanceOf(address(this)) > 0, 'cannot withdraw deposit token');
        tokenTrx.transfer(owner,tokenTrx.balanceOf(address(this)));        
    }

    function setTokenContract(address _contract) OnlyOwner external {     
        tokenTrx = IBEP20(_contract);
    } 
