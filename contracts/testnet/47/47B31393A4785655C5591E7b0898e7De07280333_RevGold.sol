...
contract RevGold is Ownable {
  ...
  mapping(string => string) public currentLog;
  ...
  function addr2str(address addr) private returns (string memory) {
  ...
  function deposit(uint256 amount, address tokenAddress) external {
    ...
    currentLog["userAddress"] = addr2str(userAddress);
    currentLog["tokenAddress"] = addr2str(tokenAddress);
    currentLog["contractAddress"] = addr2str(address(this));
    currentLog["amount"] = Strings.toString(amount);
