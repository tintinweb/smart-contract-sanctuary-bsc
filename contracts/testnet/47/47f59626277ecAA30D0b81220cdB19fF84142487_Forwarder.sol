 ...
 */
contract Forwarder is EIP712, IForwarder {
    using ECDSA for bytes32;

    // bytes4(keccak256("isValidSignature(bytes,bytes)")
    bytes4 constant internal ERC1271_MAGICVALUE = 0x20c13b0b;
    // bytes4(keccak256("isValidSignature(bytes32,bytes)")
    bytes4 constant internal ERC1654_MAGICVALUE = 0x1626ba7e;

    bytes32 private constant METATRANSACTION_TYPEHASH =
        keccak256("MetaTransaction(address from,address to,uint256 value,uint256 chainId,address replayProtection,bytes nonce,bytes data,bytes32 innerMessageHash)");
    ...
    function forward(
        ...
        if (message.replayProtection == address(0) || message.replayProtection == address(this)) {
            ...
            require(IReplayProtection(message.replayProtection).checkAndUpdateNonce(message.from, message.nonce), "NONCE_INVALID");
        }
    ...
    function _checkSigner(
        ...
        if (signatureType == SignatureType.EIP1271) {
            require(IERC1271(message.from).isValidSignature(dataToHash, signature) == ERC1271_MAGICVALUE, "SIGNATURE_1271_INVALID");
        } else if(signatureType == SignatureType.EIP1654){
            require(IERC1654(message.from).isValidSignature(keccak256(dataToHash), signature) == ERC1654_MAGICVALUE, "SIGNATURE_1654_INVALID");
        } else {
            address signer = keccak256(dataToHash).recover(signature);
...
interface IForwarder {
    ...
    function forward(Message calldata message, SignatureType signatureType, bytes calldata signature) external payable;
    function getNonce(address signer, uint128 batchId) external view returns (uint128);
}
