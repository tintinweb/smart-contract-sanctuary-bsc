...
contract Stake is Initializable,OwnableUpgradeable{
   ...
   // mapping (uint256=>uint256) TheDaysAllAmount;
    ...
    function getStakePoolEndTime(uint256 project) public view returns(uint256){
     ...
     function DailyRewardsPerTokenInReward(uint256 project,uint256 theday) public view  returns(uint256){
        ...
        if(getProjectInforByProjectId[project].theStakeProjectTokenAmountByTheday[theday]==0){
            dailyRewardsPerTokenInReward=0;
        }else{
    ...
    function getRewardinfor( uint256 project, address account) public virtual view returns(uint256){
        ...
        if(stakingaccount.stakeids.length==1){
            ...
            for(uint256 i=0;i<k;i++){
                ...
                for (uint256 j = stratday; j < endday; j++) {
                    getRewardonceStake+=_stakeinfor.stakeAmount*(10**20*_projectinfor.MaxReward/365/_projectinfor.theStakeProjectTokenAmountByTheday[j]);
    ...
    function getReward(uint256 project) public {
        ...
        require(_getReward!=0,"you have no reward");

        if(testtime>_projectinfor.StakePoolEndTime){
            
            IERC20 satketoken=IERC20(_projectinfor.stakeToken);
        
        satketoken.transfer(msg.sender, stakingaccount.allAmount);
        stakingaccount.allAmount=0;
        }
        // uint256 _getUSTD =a;
    ...
    function beforestake(uint256 project) internal {
        uint256 staketime=(testtime-getProjectInforByProjectId[project].StakePoolStartTime)/86400+1;
         ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        if(_projectinfor.today==0){
            _projectinfor.today=staketime;
        }else{
            if(staketime!=_projectinfor.today){
                for (uint256 i = _projectinfor.today; i < staketime; i++) {
                    _projectinfor.theStakeProjectTokenAmountByTheday[i]=_projectinfor.TotalStakeTokens;
                }
    ...
    function getTheTodayBytheProject(uint256 project) public view returns(uint256){
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        return _projectinfor.today;
    }

     function setTheTodayBytheProject(uint256 project,uint256 amount) public onlyOwner {
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        _projectinfor.today=amount;
    }
}
// File: contracts/StakeV1.sol
...
contract StakeV1 is Stake{
     function getRewardinfor( uint256 project, address account) public view override returns(uint256){
        require(testtime>getProjectInforByProjectId[project].StakePoolStartTime,"stake is not start");
        ProjectInfor storage _projectinfor = getProjectInforByProjectId[project];
        stakingAccount memory stakingaccount=_projectinfor.inforOfAddress[account];
        require(stakingaccount.stakeids.length!=0,"you have no satke");
        uint256[] memory _stakeids=stakingaccount.stakeids;
        ...
        stakeInfor memory lastprojectstakeinfor=_projectinfor.stakeInforById[_projectinfor.stakeId];
            uint256 lastprojectstakeday=gettheday(project, lastprojectstakeinfor.stakeTime);

             stakeInfor memory laststakeinfor= _projectinfor.stakeInforById[_stakeids[k]];
                (stratday,endday)=getthestrartandendday(project,laststakeinfor.stakeTime,getRewardtime);
                uint256 getLastReward;
                for (uint256 m = stratday; m < lastprojectstakeday; m++) {
                     getLastReward+=laststakeinfor.stakeAmount*(10**20*_projectinfor.MaxReward/365/_projectinfor.theStakeProjectTokenAmountByTheday[m]);
                }
                for (uint256 n = lastprojectstakeday; n < endday; n++) {
                     getLastReward+=laststakeinfor.stakeAmount*(10**20*_projectinfor.MaxReward/365/_projectinfor.TotalStakeTokens);
                }
