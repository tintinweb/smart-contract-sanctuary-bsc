...
contract OrbMarket is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    */
    ...
    event ChangeReceiveFundAddress(address indexed newReceiveFundAddres);
    ...
    */
    ...
    function setReceiveFundAddress(address _address) external onlyOwner {
        receiveFun = _address;
        emit ChangeReceiveFundAddress(_address);
    }
    function addWhiteListToken(address _address)external onlyOwner {
        whiteListTokens.add(_address);
    }
    function removeWhiteListToken(address _address)external onlyOwner {
        whiteListTokens.remove(_address);
    }
    ...
    function buy(uint256 orderId, uint256 _amount, uint256 price) external nonReentrant {
        ...
        sellOrder.tokenFundAddress.transferFrom(
            msg.sender,
            sellOrder.seller,
            _amount * price - marketFee
        );
        sellOrder.tokenFundAddress.safeTransferFrom(
            msg.sender,
            receiveFun,
            marketFee
        );
        ...
        emit OrderMatching(
            ...
            msg.sender,
            sellOrder.tokenId,
            address(sellOrder.tokenFundAddress),
            _amount,
            price
        );
        sellOrder.amount -= _amount;
        ...
        nonReentrant
    {
        ...
        buyOrder.tokenFundAddress.safeTransfer(msg.sender, buyOrder.price * _amount);
        emit OrderUpdate(
            orderId,
            _amount,
            buyOrder.amount,
            UpdateOrderType.CANCEL_OFFER,
            buyOrder.price,
            buyOrder.price,
            buyOrder.buyer,
            address(0)
    ...
    function takeOffer(uint256 orderId, uint256 _amount) external nonReentrant {
        ...
        buyOrder.tokenFundAddress.safeTransfer(
            msg.sender,
            _amount * buyOrder.price - marketFee
        );
        buyOrder.tokenFundAddress.safeTransfer(receiveFun, marketFee);
        emit OrderMatching(
            orderId,
            msg.sender,
            buyOrder.buyer,
            buyOrder.tokenId,
            address(buyOrder.tokenFundAddress),
            _amount,
            buyOrder.price
        );
        buyOrder.amount -= _amount;
