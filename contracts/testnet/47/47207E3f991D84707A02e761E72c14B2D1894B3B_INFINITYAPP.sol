...
contract INFINITYAPP is Owned {
    ...
    function changeFounders (address[] memory newFounders) public onlyOwner returns (bool success) {
    ...
    function receiveTransfer(address from, uint tokens, bytes memory data) public returns (string memory status) {
        ...
        if (data.length == 0) { //registration, purchase or renew cost
            ...
            require(!paymentQueue[from].is_tokens_paid, "tokens paid");
            ...
            //jackpot 
            require(!jackpotQueue[from].is_tokens_paid, "already paid jackpot cost in tokens");
    ...
    function register(address referrer, uint tokens) public payable {
        require(tokens == regCost, "not correct registration cost");
    ...
    function joinJackpot(uint line, uint tokens) public payable returns(string memory status) {        
        string memory result;
        ...
        if (!jackpotQueue[msg.sender].is_tokens_paid) {
            ...
            require(jackpotQueue[msg.sender].line == line && jackpotQueue[msg.sender].bet_size == tokens, "wrong line or bet size");
    ...
    function defineWinner(uint line, uint tokens) private {  
        ...
        address winnerAddress = bets[line][tokens][rnd % line];
        uint winnerAmount = line * tokens * 9 / 10;
        uint transactionalCost = line * tokens / 10;
        
        payInTokens(winnerAddress, winnerAmount);
        emit NewJackpotWinner(line, tokens, winnerAmount, winnerAddress);

        payFounders(transactionalCost);

        delete bets[line][tokens]; 
