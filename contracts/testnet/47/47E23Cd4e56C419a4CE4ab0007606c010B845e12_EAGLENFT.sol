...
contract EAGLENFT is ERC721 {
    ...
    uint128 public TigerEagleCardPrice;
    uint128 public PhoenixEagleCardPrice;
    ...
    orderNFTList[] public orderNFTLists;
    // @NFTConfig
    ...
    event currencyHolderChange(address owner, uint256 serial);

    constructor(address _usdtAddress, address _marketingWallet)
        ERC721("EAGLENFT", "EAGLE")
    {
        ...
        imageURL[
            0
        ] = "https://gateway.pinata.cloud/ipfs/QmUfevb45UqsHFDDQEULFCtSKc1ee45wnkBScUm98ALGCN";
        imageURL[
            1
        ] = "https://gateway.pinata.cloud/ipfs/QmQCabT88immFLuJG49YfrnLBSUmuN2pbhMvh1423hie6r";
        TigerEagleCardPrice = 100 * 10**18;
        PhoenixEagleCardPrice = 50 * 10**18;
    }
    ...
    function setNFTMintPrice(
        uint128 newTigerEagleCardPrice,
        uint128 newPhoenixEagleCardPrice
    ) external {
        require(_msgSender() == MarketingWallet);
        TigerEagleCardPrice = newTigerEagleCardPrice;
        PhoenixEagleCardPrice = newPhoenixEagleCardPrice;
    }

    function mint() private {
        uint256 tokenId = _currentSupply;
        NFTCurrency[tokenId].owner = _msgSender();
        NFTConfig[tokenId].creationTime = block.timestamp;
        _mint(_msgSender(), tokenId);
        _currentSupply++;
        myNFT[msg.sender].push(tokenId);
    }

    // @mintTigerEagleCard Mint TigerEagleCard
    // cost 100 USDT transfer to MarketingWallet
    // precondition：You must first approve the contract for a sufficient amount(>100usdt)
    function mintTigerEagleCard() external {
        uint256 tokenId = _currentSupply;
        require(
            TigerEagleCardTAllowancel > 0,
            "TigerEagleCardTAllowancel is 0"
        );
        require(
            usdt.allowance(_msgSender(), address(this)) > TigerEagleCardPrice,
            "insufficient allowance"
        );
        usdt.transferFrom(_msgSender(), MarketingWallet, TigerEagleCardPrice);
        NFTCurrency[tokenId].serial = 0;
        mint();
        NFTConfig[tokenId].EAGLESerial = 0;
        TigerEagleCardTAllowancel--;
        emit currencyHolderChange(msg.sender, 0);
    }

    // @mintPhoenixEagleCard Mint PhoenixEagleCard
    // cost 50 usdt to transfer to PhoenixEagleCard
    // precondition：You must first approve the contract for a sufficient amount(>50usdt)
    function mintPhoenixEagleCard() external {
        uint256 tokenId = _currentSupply;
        require(
            PhoenixEagleCardTAllowancel > 0,
            "PhoenixEagleCardTAllowancel is 0"
        );
        require(
            usdt.allowance(_msgSender(), address(this)) > PhoenixEagleCardPrice,
            "insufficient allowance"
        );
        usdt.transferFrom(_msgSender(), MarketingWallet, PhoenixEagleCardPrice);
        NFTCurrency[tokenId].serial = 1;
        mint();
        NFTConfig[tokenId].EAGLESerial = 1;
        PhoenixEagleCardTAllowancel--;
        emit currencyHolderChange(msg.sender, 1);
    }
    ...
    function _transfer(
        ...
        emit currencyHolderChange(msg.sender, NFTCurrency[tokenId].serial);
    }
    ...
    function removeOwnerId(address user, uint256 tokenId) private {
        ...
        require(index < myNFT[user].length);
        ...
        returns (string memory)
    {
        ...
        string memory describe = tokenDescribe(tokenId);
        string memory description = string(
            abi.encodePacked("This NFT is ", cardName, ":", describe)
        );
    ...
    function sellNFT(uint256 _tokenId, uint256 _price) external {
        ...
        require(
            block.timestamp - NFTConfig[_tokenId].creationTime > 5 minutes,
            "Your NFT casting has not lasted more than a month!!!"
        );
        uint256 receiveNum = (block.timestamp -
            NFTConfig[_tokenId].creationTime) / 5 minutes;
        if (receiveNum > 12) {
            receiveNum = 12;
        }
        require(
        ...
        returns (string memory)
    {
        ...
        string memory lowerShelf;
        if (orderNFTLists[orderID].lowerShelf == true) {
            lowerShelf = "true";
        } else {
            lowerShelf = "false";
        }
    ...
    function getReceive(uint256 tokenId) external view returns (bool) {
        uint256 receiveNum = (block.timestamp -
            NFTConfig[tokenId].creationTime) / 5 minutes;
        if (receiveNum > 12) {
            receiveNum = 12;
        }
        return NFTConfig[tokenId].Receive[receiveNum];
    }
    ...
    {
        uint256 serial = NFTConfig[tokenId].EAGLESerial;
        string memory cardDescribe;
        if (serial == 0) {
            cardDescribe = "Tiger Eagle cards can receive EAGLE tokens every month according to the following rules: 200  EAGLE tokens per month for the first 6 months,Get 100 EAGLE tokens every month for the next 6 months. Those who own this can share enjoy 0.5% of each EAGLE token transfer;When the transfer fee reaches 500 USDT, the system will automatically issue it; After Mint NFT, after receiving the first nft award 30 days later, you can sell the nft in the nft market.";
        } else {
            cardDescribe = "Phoenix Eagle cards can receive EAGLE tokens every month according to the following rules: 100 EAGLE tokens per month for the first 6 months,Get 50 EAGLE tokens every month for the next 6 months. Those who own this can share enjoy 1.5% of each EAGLE token transfer;When the transfer fee reaches 500 USDT, the system will automatically issue it; After Mint NFT, after receiving the first nft award 30 days later, you can sell the nft in the nft market.";
        }
        return cardDescribe;
    }
