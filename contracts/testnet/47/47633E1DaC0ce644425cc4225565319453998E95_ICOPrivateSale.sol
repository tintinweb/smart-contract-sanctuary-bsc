...
contract ICOPrivateSale is Initializable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    //     }
     ...
     function initialize(address _AqcTokenContractAddr, address _USDTContractAddress, uint256 _presaleRate, address _liquidityAddr, address pinkAntiBot_) initializer public {
        __Ownable_init();
 ...
 // SaleICOToken function that will use to sell the SoldOut Token
    function SaleICOToken(uint256 _usdtAmount) public returns (bool) {
        require((_usdtAmount)/presaleRate >= 100000 * 10**9,"Token Buy less than 100000 not allowed.");
        ...
        pinkAntiBot.onPreTransferCheck(msg.sender, address(this), _usdtAmountAfterFee);
