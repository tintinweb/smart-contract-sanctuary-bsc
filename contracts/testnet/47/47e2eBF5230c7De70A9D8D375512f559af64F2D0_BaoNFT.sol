...
contract BaoNFT is ERC721Enumerable, Ownable {
    ...
    IERC20 public GON = IERC20(0xa3DE968553eE82194366b5cc676646FA8A4057B9);
    IERC20 public ODR = IERC20(0xa3DE968553eE82194366b5cc676646FA8A4057B9);

    uint256[] public maxType = [400, 400, 200];
    uint256[] private currentType = [0, 0, 0];
    uint256[] private priceBNB = [0.8 ether, 1 ether, 1.2 ether];
    uint256[] private priceTOKEN = [100, 200, 300];

    uint256[] private breedPrice = [2000, 3000, 5000];
    uint256[] private fusionPrice = [2000, 3000, 5000];

    
    uint256 public maxSupply = 3333;
    uint256 public maxMintAmount = 5;
    bool public paused = false;
    bool public isPayWithToken = false;

    address private shop = 0x27A2b2CE8aa4c169a58a788Bd66cA479c43242D5;
    ...
    function mintCosmicGauntletsWithBNB(uint256 _typeGBox) public payable {
        require(!paused);
        require(currentType[0] + currentType[1] + currentType[2] <= maxSupply, "Can not buy more Cosmic Gauntlets.");
        require(_typeGBox <= 3 && _typeGBox >= 1, "Wrong type of gauntlet.");
        require(totalSupply() + 1 <= maxSupply);
        require(currentType[_typeGBox - 1] < maxType[_typeGBox - 1], "This type is sold out");
        uint256 cost;
        // buy with ETH
        cost = priceBNB[_typeGBox - 1];
    ...
    function mintCosmicGauntletsWithToken(uint256 _typeGBox) public payable {
        require(!paused);
        require(isPayWithToken, "Can not pay with token at the moment.");
        ...
        cost = priceTOKEN[_typeGBox - 1];
        // require(msg.value >= cost, "Not enough balance to complete transaction.");

        bool success = GON.transferFrom(msg.sender, shop, cost);
    ...
    function updateSoulStone(uint256 __tokenId, bool _state) public onlyOwner() {
        GauntletTemplate storage gauntlet = SNAPNgauntlet[__tokenId];
        require(gauntlet.canBreed != _state, "Can not set Soul Stone state.");
        gauntlet.canBreed = _state;
        emit setSoulStone(__tokenId, _state);
    }

    function breedGauntlets(uint256 _tokenId) external onlyOwner() {

        GauntletTemplate storage gauntlet = SNAPNgauntlet[_tokenId];
        require(gauntlet.breedCount < 3, "Each NFT can breed maximum 3 gauntlets.");
        require(gauntlet.lastBreed < (block.timestamp - 30 days), "Each NFT can breed 1 gauntlet per 30 days.");
        uint256 tokenId_minted = totalSupply() + 1;
    ...
    function sellGBoxWithTokenStatus(bool _state) public onlyOwner() {
        isPayWithToken = _state;
    } 
        
    function withdrawToken(IERC20 _tokenAddress) public payable onlyOwner() {
        bool transfer = _tokenAddress.transfer(msg.sender, _tokenAddress.balanceOf(address(this)));
        require(transfer, "Withdraw token success");
    }
