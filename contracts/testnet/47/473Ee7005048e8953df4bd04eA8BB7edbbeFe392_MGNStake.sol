...
contract MGNStake is Pausable, Ownable {
    ...
    mapping(uint256 => uint256[]) public mStakeIdByZone;
    ...
    function stakeLand(uint256[] memory _tokenId, uint256 _month, uint256 _zone)
        public
        whenNotPaused
    {
        require(
        ...
        mStakeIdByZone[_zone].push(stakeId);
        ...
        mStake[msg.sender][stakeId] = sStake(
            ...
            block.timestamp,
            true,
            (block.timestamp + _month * 30 seconds),
            _benefits
        );

        _stakeIdCounter.increment();
        emit staked(stakeId, _zone, msg.sender, _tokenId.length, _month, roi[_month]);
    }
    ...
    function unstakeLand(uint256 _stakeId, uint256 _zone) public whenNotPaused {
        require(
        ...
        for(uint256 i = 0; i< mStakeId[msg.sender].length; i++){
            if(_stakeId == mStakeId[msg.sender][i]){
                delete mStakeId[msg.sender][i];
            }
        }

        for(uint256 i = 0; i< mStakeIdByZone[_zone].length; i++){
            if(_stakeId == mStakeIdByZone[_zone][i]){
                delete mStakeIdByZone[_zone][i];
            }
        }
    ...
    function getTokenId(uint256 _stakeId)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory _token = mTokenId[_stakeId];
        uint256[] memory _tokenId = new uint256[](_token.length);
        for (uint256 i = 0; i < _token.length; i++) {
            _tokenId[i] = (mTokenId[_stakeId][i]);
        }
        return _tokenId;
    }
