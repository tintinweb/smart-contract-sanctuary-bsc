...
contract Hero is BEP721 {
    // variables
    address public owner;
    string public baseURI;
    uint256 public circulatingSupply;
    address[] public whitelistedPeople; // array  ✅
    uint256 public silverBox = 1;
    uint256 public goldenBox = 1;
    uint256 public platinumBox = 1;
    uint256 public diamondBox = 1;
    //missing total supply of box
    IBEP20 public HBG;

    uint256 public silverPrice = 250000000000000000000;
    uint256 public goldPrice = 1100000000000000000000;
    uint256 public platinumPrice = 5000000000000000000000;
    uint256 public diamondPrice = 17000000000000000000000;
    uint256 public totalSupplySilverBox = 325;
    uint256 public totalSupplyGoldBox = 100;
    uint256 public totalSupplyPlatinumBox = 50;
    uint256 public totalSupplyDiamondBox = 25;
    uint256 public timeToBurn = 31536000;

    // structs
    // Events
    event buyNewBox(address user, string boxType, uint256 tokenId);
    // mappings
    mapping(uint256 => address) public ownerOfSilverBox;
    mapping(uint256 => address) public ownerOfGoldenBox;
    mapping(uint256 => address) public ownerOfPlatinumBox;
    mapping(uint256 => address) public ownerOfDiamondBox;
    mapping(uint256 => uint256) public start;

    // constructor
    constructor(IBEP20 _HBG) BEP721("HERO Token", "HERO") {
        owner = msg.sender;
        HBG = IBEP20(_HBG); //0xA9Afb65805e143980C4FC231E008DA764FD29b53
    }
    ...
    // Readable Functions
    function checker() public view returns (address[] memory) {
        address[] memory add = new address[](whitelistedPeople.length);
        for (uint256 i = 0; i < whitelistedPeople.length; i++) {
            add[i] = whitelistedPeople[i];
        }
        return add;
    }
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            whitelistedPeople.push(addresses[i]);
        }
    }

    // hàm này gọi để kết thúc Claim Hero cho các địa chỉ được thêm vào whitelist
    function endClaimHero() external onlyOwner {
    ...
    // hàm này tất cả users đều có thể gọi để mua box
    function buyBoxMulti(uint256 _type, uint256 _quantity) public {
        require(_quantity > 0, "Quantity at least greater than 1");
        require(_quantity < 1000, "Quantity must be less than 1000");
        if (_type == 1) {
            require(silverBox < totalSupplySilverBox, "Out of silver Box");

            require(
                HBG.balanceOf(msg.sender) >= silverPrice * _quantity,
                "invalid Price "
            );

            HBG.transferFrom(msg.sender, owner, silverPrice * _quantity);
            for (uint256 i = 0; i < _quantity; i++) {
                ownerOfSilverBox[silverBox] = msg.sender;
                silverBox += 1;

                circulatingSupply += 1;
                totalSupplySilverBox -= 1;
                _mint(msg.sender, circulatingSupply);
                start[circulatingSupply] = block.timestamp;
                emit buyNewBox(msg.sender, "Silver", circulatingSupply);
            }
        } else if (_type == 2) {
            require(goldenBox < totalSupplyGoldBox, "Out of gold Box");
            require(
                HBG.balanceOf(msg.sender) >= goldPrice * _quantity,
                "invalid Price "
            );

            HBG.transferFrom(msg.sender, owner, goldPrice * _quantity);
            for (uint256 i = 0; i < _quantity; i++) {
                ownerOfGoldenBox[goldenBox] = msg.sender;
                goldenBox += 1;
                circulatingSupply += 1;
                totalSupplyGoldBox -= 1;
                _mint(msg.sender, circulatingSupply);
                start[circulatingSupply] = block.timestamp;
                emit buyNewBox(msg.sender, "Gold", circulatingSupply);
            }
        } else if (_type == 3) {
            require(
                platinumBox < totalSupplyPlatinumBox,
                "Out of platinum Box"
            );
            require(
                HBG.balanceOf(msg.sender) >= platinumPrice * _quantity,
                "invalid Price "
            );

            HBG.transferFrom(msg.sender, owner, platinumPrice * _quantity);
            for (uint256 i = 0; i < _quantity; i++) {
                ownerOfPlatinumBox[platinumBox] = msg.sender;
                platinumBox += 1;
                circulatingSupply += 1;
                totalSupplyPlatinumBox -= 1;
                _mint(msg.sender, circulatingSupply);
                start[circulatingSupply] = block.timestamp;
                emit buyNewBox(msg.sender, "Platinum", circulatingSupply);
            }
        } else if (_type == 4) {
            require(diamondBox < totalSupplyDiamondBox, "Out of diamond Box");
            require(
                HBG.balanceOf(msg.sender) >= diamondPrice * _quantity,
                "invalid Price "
            );

            HBG.transferFrom(msg.sender, owner, diamondPrice * _quantity);
            for (uint256 i = 0; i < _quantity; i++) {
                ownerOfDiamondBox[diamondBox] = msg.sender;
                diamondBox += 1;
                circulatingSupply += 1;
                totalSupplyDiamondBox -= 1;
                _mint(msg.sender, circulatingSupply);
                start[circulatingSupply] = block.timestamp;
                emit buyNewBox(msg.sender, "Diamond", circulatingSupply);
            }
        }
    }

    //          Only Owner
    // hàm này để burn hero NFT khi đã tạo ra ít nhất 1 năm
    function BurnNFT(uint256 token_No) external onlyOwner {
        require(
            block.timestamp > (start[token_No] + timeToBurn),
            "Token's age must be older than 1 year."
        );
        _burn(token_No);
        start[circulatingSupply] = 0;
    }

    //thay time to burn boi onlyOwner
    function changeTimeToBurn(uint256 _timeToBurn) external onlyOwner {
        timeToBurn = _timeToBurn;
    }

    //hàm này để admin thay đổi giá bán box
    function SetPrice(
        uint256 _silver,
        uint256 _gold,
        uint256 _platinum,
    ...
    //hàm này để admin thay đổi total box
    function setTotalSupplyBox(
        ...
        totalSupplySilverBox = _totalSupplySilver;
        totalSupplyGoldBox = _totalSupplyGold;
        totalSupplyPlatinumBox = _totalSupplyPlatinum;
        totalSupplyDiamondBox = _totalSupplyDiamond;
    }

    function setTotalBox(
        uint256 _silverBox,
        uint256 _goldenBox,
        uint256 _platinumBox,
        uint256 _diamondBox
    ) public onlyOwner {
        silverBox = _silverBox;
        goldenBox = _goldenBox;
        platinumBox = _platinumBox;
        diamondBox = _diamondBox;
    }
    ...
    // Hàm này để admin gọi tạo ra Hero NFT mới cho user
