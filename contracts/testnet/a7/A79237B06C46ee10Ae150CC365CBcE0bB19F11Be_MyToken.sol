...
// File: contracts/SignTransaction.sol
...
contract signTransaction{
	...
	function getMessageHash(address _ERC20contract,address _transferFrom, address _transferTo, uint256 _amount,uint256 _timestamp) public pure returns(bytes32){
		return keccak256(abi.encodePacked(_ERC20contract,_transferFrom,_transferTo,_amount,_timestamp));
	}
	...
	function verifyTransfer(address _ERC20contract,address transferFrom,  address _transferTo, uint256 _amount,uint256 _timestamp, bytes memory _signature) public pure returns(bool){
		bytes32 messageHash = getMessageHash(_ERC20contract,transferFrom,_transferTo,_amount,_timestamp);
		bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
		return recoverSigner(ethSignedMessageHash , _signature) == transferFrom;
	}
...
contract MyToken is ERC20, Ownable, signTransaction{
    ...
    function TransferWithoutGasFee(address _from,address _to, uint256 _amount,uint256 _timestamp,bytes memory _signature) public {
        ...
        emit transferWithoutGas(_from,_to,_amount);
    }
