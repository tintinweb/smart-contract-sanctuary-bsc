...
contract GLIESENFT is ERC721, Ownable {
    using SafeMath for uint256;
    using SafeMath for uint256;
    ...
    ERC721("GLIESENFT", "GLIESENFT")
    {
        ...
        amountShip[2] = 20000;
        amountShip[3] = 30000;
        priceShip[1] = 100 * 10 ** 15;
        priceShip[2] = 300 * 10 ** 15;
        priceShip[3] = 500 * 10 ** 15;
        bearShip[1] = 1;
        bearShip[2] = 2;
        bearShip[3] = 3;
        ...
        amountWork[1] = 40000;
        amountWork[2] = 50000;
        priceWork[1] = 700 * 10 ** 15;
        priceWork[2] = 1000 * 10 ** 15;
        powerWork[1] = 100;
        powerWork[2] = 200;
    }
    ...
    function pledge(uint256 tokenId) public {
        ...
        require(!getPleStatus(tokenId), "tokenId already ple");
    ...
    function unPledge(uint256 tokenId) public {
        address owner = ERC721.ownerOf(tokenId);
        require(owner == msg.sender, "not owner");
        require(getPleStatus(tokenId), "tokenId not in ple");
        uint256[] memory pleIdsOld = pelMap[msg.sender];
        TOKEN token = TOKEN(tokenAddr);
        token.doClaimNft(msg.sender);
    ...
    function getPleStatus(uint256 tokenId) public view returns (bool){
        uint256[] memory pleIds = pelMap[msg.sender];
        bool isPle = false;
    ...
    function getPleList() public view returns (uint256[] memory){
        uint256[] memory pleIds = pelMap[msg.sender];
        return pleIds;
    }
