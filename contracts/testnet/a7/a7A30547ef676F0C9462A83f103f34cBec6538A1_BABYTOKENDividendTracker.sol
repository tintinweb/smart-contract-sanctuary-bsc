    ...
    DividendPayingTokenOptionalInterface
{
    ...
    // About dividendCorrection: >关于股息校正
    // If the token balance of a `_user` is never changed, the dividend of `_user` can be computed with:
    //    > 如果 `_user` 的代币余额从未改变，则 `_user` 的股息可以通过以下公式计算：
    //   `dividendOf(_user) = dividendPerShare * balanceOf(_user)`.
    // When `balanceOf(_user)` is changed (via minting/burning/transferring tokens),
    //    > 当 `balanceOf(_user)` 改变时（通过 mintingburningtransferring 代币）
    //   `dividendOf(_user)` should not be changed,
    //   but the computed value of `dividendPerShare * balanceOf(_user)` is changed.
    //    > 但 `dividendPerShare balanceOf(_user)` 的计算值会改变
    // To keep the `dividendOf(_user)` unchanged, we add a correction term:
    //    > 为了保持 `dividendOf(_user)` 不变，我们添加了一个修正项：
