...
contract Foo {
    ...
    function receiveCall(Structs.VM memory _vm) external {
        (MessageType _type, address _to, bytes memory _data) = verifyVm(_vm);
        require(_type == MessageType.Call, "!type");
        (bool success, bytes memory _returndata) = _to.call(_data);
        require(success, "!call");
        sendResult(_vm.nonce, _vm.emitterChainId, bytes32ToAddress(_vm.emitterAddress), _returndata);
    }
    ...
    function receiveResult(Structs.VM memory _vm) external {
        (MessageType _type, address _to, bytes memory _data) = verifyVm(_vm);
        require(_type == MessageType.Result, "!type");
        require(_to == address(this), "!to");
        responses[_vm.nonce] = _data;
    }

    function verifyVm(Structs.VM memory _vm) internal view returns (MessageType, address, bytes memory) {
        (bool _valid, string memory _reason) = endpoint.verifyVM(_vm);
        require(_valid, _reason);
        (MessageType _type, uint16 _destination, address _to, bytes memory _data) = abi.decode(_vm.payload, (MessageType, uint16, address, bytes));
        require(_destination == chainId, "!destination");
        return (_type, _to, _data);
    }
