...
contract iconz is ERC721, ERC721Enumerable, Ownable, Clubs {
    ...
    uint256 public numberOfIconzBurned = 0;
    ...
    function mint(uint256 _nbTokens) external payable whenSaleStarted override{
        ...
        require(lastIdMinted + _nbTokens <= maxSupply - _reserved, "Not enough Tokens left.");
        require(_nbTokens * _price <= msg.value, "Inconsistent amount sent!");

        for (uint256 i; i < _nbTokens; i++) {
            _safeMint(msg.sender, lastIdMinted + i + numberOfIconzBurned);
            
        }
        
        lastIdMinted = lastIdMinted + _nbTokens;
    }
    ...
    function fusion(uint256 _idOne, uint256 _idTwo) external whenSaleStarted {
        numberOfIconzBurned = numberOfIconzBurned + 2;
