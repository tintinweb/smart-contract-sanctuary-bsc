    ...
    function _transfer(
        ...
        if (!swapLiquidity && from != uniswapV2Pair && enableSwapAndLiquify) {
            ...
            if (overMaxTokenBalance) {
                ...
                uint256 swapToken = swapableLiquidityFeeAmount;
        ...
        bool overMaxMarketingBalance = swapableMarketingFeeAmount >=
            cappedSwapableMarketingAmount;

        if (!swapMarketingFee && from != uniswapV2Pair ) {
            swapMarketingFee = true;

            if (overMaxMarketingBalance) {
                if (enableFee) {
                    enableFee = false;
                    taxDisableInLiquidity = true;
                }
                uint256 swapToken = swapableMarketingFeeAmount;
                swapTokensForEthToMarketingWallet(swapToken);
                if (taxDisableInLiquidity) {
                    enableFee = true;
                    taxDisableInLiquidity = false;
                }
            }
            swapMarketingFee = false;
