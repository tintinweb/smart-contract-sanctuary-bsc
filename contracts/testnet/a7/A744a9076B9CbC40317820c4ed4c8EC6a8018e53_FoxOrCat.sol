...
contract FoxOrCat is ERC721Enumerable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdTracker;

    string private baseURI;
    uint256 private currentMaxSupply = 10;
    uint256 immutable private MAX_SUPPLY = 666;
    ...
    function mint() external payable {
        require(totalSupply() < currentMaxSupply, "Max supply reached");
    ...
    function setCurrentMaxSupply(uint256 _value) external onlyOperator {
        require(_value != 0, "value zero");
        require(_value <= MAX_SUPPLY, "Exceeded max supply");

        currentMaxSupply = _value;

        emit SetCurrentMaxSupply(_value);
    }

    function setWhiteList(address[] memory _accounts, bool _bool) external onlyOperator {
    ...
    function setRecipientAddress(address _newAddress) external onlyOperator {
    ...
    function setMintPrice(uint256 _value) external onlyOperator {
    ...
    function setStartEndTime(uint256 _startTimestamp, uint256 _endTimestamp) external onlyOperator {
    ...
    event SetCurrentMaxSupply(uint256 _value);
