...
import "./ChainLinkOracles/TykheFortuneDistributor.sol";
import "./Commerce/interfaces/IMercuryMultiNetworkRouter.sol";
import "./Commerce/interfaces/uniswap/IUniswapV2Factory.sol";
import "./Commerce/interfaces/uniswap/IUniswapV2Pair.sol";
...
contract Token is IERC20 {
    ...
    TykheFortuneDistributor private tykheFortuneDistributor;
...
event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    ...
    constructor(address tokenOwner,
        ...
        tykheFortuneDistributor = new TykheFortuneDistributor();
    ...
    function totalSupply() public view virtual  returns (uint256) {return _totalSupply - balanceOf(address(deadAddress));}
    ...
    function balanceOf(address account) public view  returns (uint256) {
        if (tykheFortuneDistributor.isExcludedFromRewards(account)) return _tokenBalance[account];
        return tokenFromReflection(_reflectionBalance[account]);
    }

    /// @notice Function inherited from BEP20
    function allowance(address tokenOwner, address spender) public view virtual  returns (uint256) {return _allowances[tokenOwner][spender];}
    ...
    function _transfer(
        ...
        if (transferDelayEnabled && block.timestamp < (initialDelayTime + totalDelayTime)) {
            ...
            if (from != _owner && to != address(mercuryMultiNetworkRouter) && to != address(lpPair)) {
        ...
        if (tykheFortuneDistributor.isExcludedFromFee(from) || tykheFortuneDistributor.isExcludedFromFee(to) && !lpPairs[from] || !lpPairs[to]) {
            takeFee = false;
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (tykheFortuneDistributor.isExcludedFromRewards(sender) && !tykheFortuneDistributor.isExcludedFromRewards(recipient)) {
            _transferFromExcluded(sender, recipient, amount);
        } else if (!tykheFortuneDistributor.isExcludedFromRewards(sender) && tykheFortuneDistributor.isExcludedFromRewards(recipient)) {
    ...
    function includeInReward(address account) external onlyOwner() {
        require(tykheFortuneDistributor.isExcludedFromRewards(account), "Account is not excluded");
...
contract TykheFortuneDistributor {
    string constant public TOO_MUCH_GREED = "too much greed, fees cannot be higher than 45% (maximum in pancake swap)";
    ...
    function addFeesReceiver(FeeReceiver memory _feeReceiver)
        internal
        virtual
    {}
        ...
        virtual
    {
        for (uint256 index = 0; index < membersCounter; index++) {
            IERC20(_sendFeeTokenAddress).transferFrom(
                from,
                feeReceivers[index].receiver,
                feeReceivers[index].percentage
            );
        }
    }
    ...
    function setFees(
        ...
        // fees cant be higer than 45% (pancake max slipagge)
        require(buyFee <= 4500, TOO_MUCH_GREED);
        require(sellFee <= 4500, TOO_MUCH_GREED);
        require(transferFee <= 4500, TOO_MUCH_GREED);
        require(buyFee + sellFee + transferFee <= 4500, TOO_MUCH_GREED);
