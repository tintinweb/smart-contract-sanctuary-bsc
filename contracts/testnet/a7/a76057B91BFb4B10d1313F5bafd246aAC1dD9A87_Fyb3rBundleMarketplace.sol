...
import "./interfaces/IFyb3rMarketplace.sol";
import "./interfaces/IFyb3rAddressRegistry.sol";
import "./interfaces/IFyb3rTokenRegistry.sol";

contract Fyb3rBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IFyb3rAddressRegistry public addressRegistry;
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IFyb3rTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        uint256 feeAmount = (price * platformFee) / MULTIPLIER;
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "Fyb3rMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "Fyb3rMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IFyb3rMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IFyb3rMarketplace(addressRegistry.marketplace()).getPrice(
                _payToken
            ),
            price
        );
    ...
    /**
     @notice Update Fyb3rAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IFyb3rAddressRegistry(_registry);
    }
...
interface IFyb3rAddressRegistry {
