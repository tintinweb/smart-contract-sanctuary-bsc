...
contract LegionStake is Context, Ownable {
    ...
    IBEP20 token;
    uint256 tiers;
    uint256 public lockPeriod1 = 90 seconds;
    uint256 public lockPeriod2 = 180 seconds; 
    uint256 public lockPeriod3 = 365 seconds;
    ...
    function stakeTokens(uint256 _amount, uint256 _lockPeriod, uint256 _tier) public {
        ...
        if(_lockPeriod == 1){
        deposit[_msgSender()] = User(_tier, _lockPeriod + lockPeriod1, _amount, now, now);
        }
        else if(_lockPeriod == 2){
        deposit[_msgSender()] = User(_tier, _lockPeriod + lockPeriod2, _amount, now, now);
        }
        else if(_lockPeriod == 3){
        deposit[_msgSender()] = User(_tier, _lockPeriod + lockPeriod3, _amount, now, now);
        }
    ...
    function UnstakeTokens() public {
        User memory wUser = deposit[_msgSender()];
        uint256 totalAmount = wUser.amount;

        require(totalAmount > 0, "deposit first");
        //   require(now > wUser.lockPeriod, "Token locked");
        if(now > wUser.lockPeriod){
        // require(now > wUser.lockPeriod, "Token locked");
        ...
        uint256 _fee = totalAmount.mul(earlyUstakeFee).div(100);

        token.transfer(_msgSender(),totalAmount.sub(_fee));
