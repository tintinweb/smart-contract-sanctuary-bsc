...
contract Erecs1 is ERC20, Ownable {
    using SafeMath for uint256;
    using Address for address payable;

    Erecs1DividendTracker public dividendTracker;
    ...
    uint256 public totalFees = liquidityFee.add(BUSDRewardsFee).add(marketingFee).add(burnFee).add(buybackFee);
    ...
    constructor() public ERC20("Erecs1 big", "Erecs1") {

    	dividendTracker = new Erecs1DividendTracker();
    ...
    function setTxLimitExempt(address account, bool excluded) external onlyOwner {
        require(isTxLimitExempt[account] != excluded, "Erecs1::setTxLimitExempt: Account is already the value of 'excluded'");
    ...
    function setExcludedFromAntiWhale(address account, bool excluded) external onlyOwner {
        require(isExcludedFromAntiWhale[account] != excluded, "Erecs1::setExcludedFromAntiWhale: Account is already the value of 'excluded'");
    ...
    function setExcludedFromTransactionCooldown(address account, bool excluded) public onlyOwner {
        require(isExcludedFromTransactionlock[account] != excluded, "Erecs1::setExcludedFromTransactionCooldown: Account is already the value of 'excluded'");
    ...
    function setTransactionCooldownTime(uint256 transactiontime) public onlyOwner {
        require(transactiontime <= 86400, "Erecs1::setTransactionCooldownTime: Can not to exceed max amount");
        emit TransactionCooldownTimeUpdated(transactionLockTime, transactiontime);
        transactionLockTime = transactiontime;
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Erecs1::excludeFromFees: Account is already the value of 'excluded'");
    ...
    function setBlacklist(address account, bool excluded) public onlyOwner {
        require(_isBlacklisted[account] != excluded, "Erecs1::setBlacklist: Account is already the value of 'excluded'");
    ...
    function enableTrading(bool value) external onlyOwner {
        require(!trading, "Erecs1::setTrading: Trading is already set.");
        trading = value;
    }

    function enableImpactEffect(bool value) external onlyOwner {
        require(value != impactEnabled, "Erecs1::enableTrading: ImpactEnabled is already set.");
        impactEnabled = value;
    }

    function setMarketingWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "Erecs1::setMarketingWallet: Please use vaild payable address.");
     ...
     function setBuybackWallet(address payable wallet) external onlyOwner{
        require(wallet != address(0), "Erecs1::setBuybackWallet: Please use vaild payable address.");
        emit BuybackWalletUpdated(msg.sender, _buybackWalletAddress, wallet);
        _buybackWalletAddress = wallet;
    }

    function setLiquidityFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(marketingFee).add(burnFee).add(buybackFee);
        require(totalFees <= 30, "Erecs1::setLiquidityFee: Too high Fees");
        emit LiquidityFeeUpdated(msg.sender, liquidityFee, value);
        liquidityFee = value;
    }

    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        totalFees = value.add(liquidityFee).add(marketingFee).add(burnFee).add(buybackFee);
        require(totalFees <= 30, "Erecs1::setBUSDRewardsFee: Too high Fees");
        emit BUSDRewardsFeeUpdated(msg.sender, BUSDRewardsFee, value);
        BUSDRewardsFee = value;
    }

    function setMarketingFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(liquidityFee).add(burnFee).add(buybackFee);
        require(totalFees <= 30, "Erecs1::setMarketingFee: Too high Fees");
        emit MarketingFeeUpdated(msg.sender, marketingFee, value);
        marketingFee = value;
    }

    function setBurnFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(marketingFee).add(liquidityFee).add(buybackFee);
        require(totalFees <= 30, "Erecs1::setBurnFee: Too high Fees");
        emit BurnFeeUpdated(msg.sender, burnFee, value);
        burnFee = value;
    }

    function setBuybackFee(uint256 value) external onlyOwner{
        totalFees = value.add(BUSDRewardsFee).add(marketingFee).add(burnFee).add(liquidityFee);
        require(totalFees <= 30, "Erecs1::setBuybackFee: Too high Fees");
        emit BuybackFeeUpdated(msg.sender, buybackFee, value);
        buybackFee = value;
    }

    function setSellFee(uint256 value) external onlyOwner{
        require(sellFee <= 10, "Erecs1::setSellFee: Too high Fees");
        emit SellFeeUpdated(msg.sender, sellFee, value);
        sellFee = value;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Erecs1::setAutomatedMarketMakerPair: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Erecs1::_setAutomatedMarketMakerPair: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Erecs1::updateGasForProcessing: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Erecs1::updateGasForProcessing: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(from != address(0), "Erecs1: transfer from the zero address");
        require(to != address(0), "Erecs1: transfer to the zero address");
        require(amount <= maxTxAmount || isTxLimitExempt[from], "Erecs1: transfer amount limit exceeded");
        require(!_isBlacklisted[to] && !_isBlacklisted[from], 'Erecs1: Blacklisted address');
        require(antiWhaledisabled || isExcludedFromAntiWhale[to] || balanceOf(to) + amount <= maxTokensPerAddress, "Erecs1: Max tokens limit for this account exceeded. Or try lower amount");
        ...
        if( !trading && 
            ...
            require( !automatedMarketMakerPairs[to], "Erecs1: Trading disabled");
            require( !automatedMarketMakerPairs[from], "Erecs1: Trading disabled");
        }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        try IERC20(BUSD).transfer(address(dividendTracker), dividends){
            ...
            revert("Erecs1::BUSD transfer reverted because of insufficient BUSD balance");
        }
    }
...
contract Erecs1DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("Erecs1_Dividen_Tracker", "Erecs1_DTker") {
    	claimWait = 21600;
        minimumTokenBalanceForDividends = 100000 * (10**18); //must hold 100000+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        revert("Erecs1_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        revert("Erecs1_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Erecs1 contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Erecs1_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Erecs1_Dividend_Tracker: Cannot update claimWait to same value");
        emit ClaimWaitUpdated(newClaimWait, claimWait);
        claimWait = newClaimWait;
    }

    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        require(newMinimumTokenBalanceForDividends >= 100 * (10**18) && newMinimumTokenBalanceForDividends <= 10000000 * (10**18), "Erecs1_Dividend_Tracker: MinimumTokenBalanceForDividends must be updated to between 100 and 10000000");
        require(newMinimumTokenBalanceForDividends != minimumTokenBalanceForDividends, "Erecs1_Dividend_Tracker: Cannot update MinimumTokenBalanceForDividends to same value");
