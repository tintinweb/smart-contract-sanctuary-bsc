...
contract FOFPlayerRaceData is Ownable {
    ...
    mapping(uint256 => address[]) public clubMembers;//俱乐部会员tokenid-address

    mapping(address => uint256) public memberToClubIds;

    mapping(uint256 => address[3]) public clubRaceMembers;//俱乐部会员tokenid-address
    ...
    mapping(uint256 => mapping(uint256 => uint256[])) public raceVehicles;//排位赛类型-比赛场次-tokenid

    mapping(uint256 => mapping(uint256 => uint256)) public raceReward;//比赛类型-比赛场次-奖金
    mapping(uint256 => mapping(uint256 => uint256)) public fofRaceReward;//比赛类型-比赛场次-奖金
    ...
    constructor() {
        ...
        // raceTime[1][1] = 1659319200;
        // raceTime[1][2] = 1659333600;
        // raceTime[1][3] = 1659348000;
        raceTime[1][1] = 7200;
        raceTime[1][2] = 14400;
        raceTime[1][3] = 21600;
        ...
        raceTime[2][1] = 1692090000;//2023-8-15 17:00:00
        raceTime[2][1] = 1692093600;//2023-8-15 18:00:00
    ...
    function addRaceVehicles(uint256 raceType, uint256 tokenId) public onlyController {
        raceVehicles[raceType][raceIds[raceType]].push(tokenId);
    }
    ...
    function setSettlementTimesToDividends(uint256 dividends) public onlyController {
        settlementTimesToDividends[settlementTimesToTime.length][1] = dividends;
        todayDividends = 0;
    }
...
contract FOFPlayerController1 is Ownable {
    ...
    constructor() {
        ...
        FPN = FOFPlayerNFT(0xC646368BdE58F069031b39Fbd1e268e995b85e62);
    }
...
contract FOFPlayerController2 is Ownable {
    ...
    constructor() {
        ...
        FPN = FOFPlayerNFT(0xC646368BdE58F069031b39Fbd1e268e995b85e62);
        FPC = FOFPlayerController1(payable(0x31C682f7B7739ac9Ca9FeC6317d3700e4faF2881));
...
contract FOFPlayerRaceQuery is Ownable {
    ...
    address public pancakeswapV2Pair = 0x5da88e4aeA67Be3Fa8D75BB32dCE23F3c50B04b9;
    ...
    constructor() {
        ...
        FPN = FOFPlayerNFT(0xC646368BdE58F069031b39Fbd1e268e995b85e62);
        FPC2 = FOFPlayerController2(payable(0x70b0D840179162a2c407c32FB3AeFF96128d2F1a));
        FPRD = FOFPlayerRaceData(payable(0x50f71db8cC297dfBd1DD7feE9dcE6B1b9668e73c));
    }
    ...
    function rankRaceRewards(uint256 reward) public pure returns(uint256[] memory rewards) {

        rewards = new uint256[](uint256(7));

        rewards[0] = reward.mul(70 * 40).div(10000);
        rewards[1] = reward.mul(70 * 35).div(10000);
        rewards[2] = reward.mul(70 * 25).div(10000);

        uint256 reward2 = BUSDEqualsToToken(1 * 10 ** 18);
        rewards[3] = reward2;
        rewards[4] = reward2;
        rewards[5] = reward2;
        rewards[6] = reward2;
    ...
    function BUSDEqualsToToken(uint256 BUSDAmount) public pure returns(uint256) {
        ...
        uint256 BUSDOfPair = 200;

        if(BUSDOfPair == 0){
            return 0;
        }

        uint256 amount = BUSDAmount.mul(tokenOfPair).div(BUSDOfPair);
    ...
    function fuelDivided(uint256 fuelPrices) public pure returns(uint256,uint256,uint256){
        uint256 maket = fuelPrices.mul(10).div(100);
        uint256 union = fuelPrices.mul(5).div(100);
        uint256 burn = fuelPrices.sub(maket * 3).sub(union * 2);
        
        return (burn,maket,union);
    }
    ...
    function checkIsRaceTime(uint256 raceType) public view returns(bool) {
        ...
        if(raceType == 2 || raceType == 3){
            if((FPRD.raceTime(raceType, raceType) < time && time < FPRD.raceTime(raceType, raceType) + FPRD.raceDurations(raceType))){
                return true;
            }
        }
    ...
    function getClubMemberLists(uint256 clubId) public view returns(string[] memory nicknames, address[] memory members, uint256[] memory vehicleNums) {
        ...
        return (nicknames,members,vehicleNums);
    }
    ...
    function raceRankingLists(uint256 raceType,uint256 index) public view returns(address[] memory racers, string[] memory nicknames,
    uint256[] memory clubIds, string[] memory cardNicknames,uint256[] memory vehicleTypes,uint256[] memory rewards) {

        uint256[] memory vehicleIds = raceRanking(raceType,FPRD.raceIds(raceType) + 1 - index);
        ...
        return (racers,nicknames,clubIds,cardNicknames,vehicleTypes,rewards);
    }
    ...
    function raceRanking(uint256 raceType,uint256 raceId) public view returns(uint256[] memory vehicleIds) {
        ...
        uint256[] memory arr = new uint256[](uint256(vehicleIds.length));
...
contract FOFToken is ERC20,Ownable {
    ...
    address public receiveAddress = 0xC5a89985D57FdddEFCE3E9f26faB99EbF471516A;
    ...
    constructor() ERC20("FOFToken", "FOF") {
    ...
    function _transfer(
        ...
        if(isExcludedFromLimit[sender] || isExcludedFromLimit[recipient] || sender == pancakeswapV2Pair){
            return super._transfer(sender, recipient, amount);
        }

        if(!isExcludedFromLimit[sender] && recipient == pancakeswapV2Pair){
            require(isOpen, "ERC20: not yet open");

            uint256 fee = amount.mul(sellFee).div(100);

            amount = amount - fee;

            super._transfer(sender, receiveAddress, fee);
        ...
        if(!isExcludedFromLimit[recipient] && sender == pancakeswapV2Pair){
            require(isOpen, "ERC20: not yet open");
    ...
    function swapAndBurn() public payable onlyController lockTheSwap {
        ...
        //授权如何处理，一次授权？安全性
        // ERC20(USDT).approve(address(pancakeswapV2Router), BNBAmount);

        // pancakeswapV2Router.swapExactTokensForTokens(//swapExactETHForTokens
        pancakeswapV2Router.swapExactETHForTokens{value:msg.value}(
        ...
        emit SwapAndCharged(initialBalance, newBalance, balanceOf(address(this)));
    }

    // function swapTokensForEth() public payable {
...
// import "./FOFPlayerController2.sol";

// import "./FOFPlayerController4.sol";
...
contract FOFPlayerController5 is Ownable {
    ...
    FOFPlayerController1 FPC1;

    FOFPlayerController2 FPC2;

    FOFPlayerRaceData FPRD;

    FOFPlayerRaceQuery FPRQ;
    ...
    constructor() {
        ...
        // FPC2 = FOFPlayerController2(payable(0xE3547B03613503261C7649ad36Fd159C9A2651b2));
        FPN = FOFPlayerNFT(0xC646368BdE58F069031b39Fbd1e268e995b85e62);
        FPC1 = FOFPlayerController1(payable(0x31C682f7B7739ac9Ca9FeC6317d3700e4faF2881));
        FPC2 = FOFPlayerController2(payable(0x70b0D840179162a2c407c32FB3AeFF96128d2F1a));
        FPRD = FOFPlayerRaceData(payable(0x50f71db8cC297dfBd1DD7feE9dcE6B1b9668e73c));
        FPRQ = FOFPlayerRaceQuery(0xD6Aa74a88b45FD56B5eA2bC50B6C96c195e5Ade4);
        FOF = FOFToken(payable(0x586B69166a9Aa17F574B6E3Af89CE90583728026));
    }
    ...
    function seFOFPlayerRaceQuery(address _FOFPlayerRaceQuery) public onlyController {
    ...
    function signUpRace(uint256 raceType, uint256 tokenId) public payable {

        require(FPRQ.checkIsRaceTime(raceType), "Now is not the time of the race");
        ...
        FPRD.addRaceVehicles(raceType, tokenId);

        if(raceType == 1){
            require(FPRD.userRaceLeftTimes(msg.sender) > 0,"You have run out of race times");

            require(FPRD.getRaceVehicles(1, FPRD.raceIds(1)).length < 8, "Registration for this race is full");
            ...
            payable(FPC1.marketingAddress()).transfer(repo);
            ...
            // raceReward[1][raceIds[1]] += rank;
            FPRD.addRaceReward(1,FPRD.raceIds(1),rank);
        }else{
            require(FPRQ.checkRaceMember(FPRD.memberToClubIds(msg.sender), msg.sender),"You are not eligible");

            require(FPRD.getRaceVehicles(raceType, FPRD.raceIds(raceType)).length < 151, "Registration for this race is full");

            require(msg.value == 0,"No need to pay for tickets");

            // if(raceType == 2 && FPRD.getRaceVehicles(2, FPRD.raceIds(2)).length >= 150){
            if(raceType == 2 && FPRD.getRaceVehicles(2, FPRD.raceIds(2)).length >= 10){
                settlement(2,FPRD.raceIds(2));

                // raceIds[2]++;
                FPRD.addRaceIds(2,1);

                // raceTime[2][raceIds[2]] = raceTime[2][raceIds[2]] + 30 days;
                FPRD.setRaceTime(2, FPRD.raceIds(2), 30 days);
            }

            // if(raceType == 3 && FPRD.getRaceVehicles(3, FPRD.raceIds(3)).length >= 150){
            if(raceType == 3 && FPRD.getRaceVehicles(3, FPRD.raceIds(3)).length >= 10){
                settlement(3,FPRD.raceIds(3));

                FPRD.addRaceIds(3,1);

                FPRD.setRaceTime(3, FPRD.raceIds(3), 90 days);
            }
        }
    }

    function settlement(uint256 raceType,uint256 raceId) public {
        ...
        if(raceType == 1){
            ...
            uint256[] memory rewards = FPRQ.rankRaceRewards(FPRD.raceReward(1,FPRD.raceIds(1)));
            ...
            if(raceType == 2){
                ...
                // FPRD.raceReward(3,FPRD.raceIds(3)) += FPC1.leagueAddress().balance.mul(48).div(100);
                FPRD.addRaceReward(3, FPRD.raceIds(3), FPC1.leagueAddress().balance.mul(48).div(100));
                uint256 kickback = FPC1.leagueAddress().balance.sub(reward).sub(FPRD.raceReward(3,FPRD.raceIds(3)));

                fofReward = FOF.balanceOf(FPC1.leagueAddress()).mul(32).div(100);
                // FPRD.fofRaceReward(3,FPRD.raceIds(3)) += FOF.balanceOf(FPC1.leagueAddress()).mul(48).div(100);
                FPRD.addFofRaceReward(3, FPRD.raceIds(3), FOF.balanceOf(FPC1.leagueAddress()).mul(48).div(100));
                uint256 fofKickback = FOF.balanceOf(FPC1.leagueAddress()).sub(fofReward).sub(FPRD.fofRaceReward(3,FPRD.raceIds(3)));
            ...
            if(raceType == 3){
                reward = FPRD.raceReward(3,FPRD.raceIds(3));
                fofReward = FPRD.fofRaceReward(3,FPRD.raceIds(3));
            }
    ...
    function _settlement() internal virtual {
        ...
        if(clubNftNum > 0){
            // settlementTimesToDividends[settlementTimesToTime.length][1] = todayDividends.div(clubNftNum);
    ...
    function chioceclubRaceMembers(uint256 raceType, address[] memory members) public {
        ...
        require(FPRQ.findMemberIndex(clubIds[0], members[0]) > 0 && FPRQ.findMemberIndex(clubIds[0], members[1]) > 0 && FPRQ.findMemberIndex(clubIds[0], members[2]) > 0 , "The user you selected does not belong to the club");

        // clubRaceMembers[clubIds[0]][0] = members[0];
        // clubRaceMembers[clubIds[0]][1] = members[1];
        // clubRaceMembers[clubIds[0]][2] = members[2];
        FPRD.setClubRaceMembers(clubIds[0],0,members[0]);
        FPRD.setClubRaceMembers(clubIds[0],0,members[0]);
        FPRD.setClubRaceMembers(clubIds[0],0,members[0]);
    }
    ...
    function buyFuel(uint256 tokenId) public {
        ...
        require(FPN.NFTTypes(tokenId) == 5, "The petrol stations ID is wrong");

        require(FPRD.userRaceLeftTimes(msg.sender) == 0, "The times of race has not been used up");

        require(FPRQ.getUserVehicleNum(msg.sender) > 0, "You don't have a vehicle");

        address owner = FPN.ownerOf(tokenId);

        uint256 price = FPRD.fuelPrice();

        if(block.timestamp < FPRD.discountEndTime()){
            if(FPRQ.getUserNFTNumByType(msg.sender, 1) > 0){
            ...
            }else if(FPRQ.getUserNFTNumByType(msg.sender, 3) > 0){
                price = FPRD.fuelPrice().mul(FPRD.fuelPriceDiscount(3)).div(100);
            }
        }

        (uint256 burn,uint256 maket,uint256 union) = FPRQ.fuelDivided(price);
        ...
        FPC1.addTodayDividends(union);
        FOF.transferFrom(msg.sender, FPRD.clubAddress(), union);
        FOF.transferFrom(msg.sender, FPC1.marketingAddress(), maket);
        FOF.transferFrom(msg.sender, FPC1.leagueAddress(), maket);
       
        // uerRaceLeftTimes[msg.sender] = uerRaceLeftTimes[msg.sender].add(10);
        // uerRaceTotalTimes[msg.sender] = 10;
        FPRD.addUserRaceLeftTimes(msg.sender, 10);
        FPRD.setUerRaceTotalTimes(msg.sender, 10);

        // todayDividends = todayDividends.add(union);
        FPRD.addTodayDividends(union);
    ...
    function getFuelForFree() public {
        ...
        // uerGetFuelForFree[msg.sender] = true;
        FPRD.setUerGetFuelForFree(msg.sender, true);

        if(block.timestamp < 1661961600){
            // uerRaceLeftTimes[msg.sender] = uerRaceLeftTimes[msg.sender].add(20);
            // uerRaceTotalTimes[msg.sender] = 20;
            FPRD.addUserRaceLeftTimes(msg.sender, 20);

            FPRD.setUerRaceTotalTimes(msg.sender, 20);
        }else{
            FPRD.addUserRaceLeftTimes(msg.sender, 10);
