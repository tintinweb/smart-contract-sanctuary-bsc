...
contract DewinBet is AccessControl, ReentrancyGuard {
    ...
    event Bet(address account, uint256 id, uint256 betPairIndex, uint256 optionPairIndex, uint256 amount);
    ...
    mapping (uint256 => bool) private _idUsed;
   ...
   constructor() {
    ...
    function bet(uint256 id, uint256 betPairIndex, uint256 optionPairIndex, uint256 amount) external nonReentrant notContract {
        ...
        uint256 totalPairAmount = 0;
        for (uint256 i = 0; i < betPair.pairs[betPairIndex].amounts.length; i++) {
            totalPairAmount += betPair.pairs[betPairIndex].amounts[i];
        }
        require(totalPairAmount + amount <= betPair.pairs[betPairIndex].cap, "over pair cap");
        require(betInfo.totalBetAmount + amount <= match_.maxBetAmountForEach, "over each person cap");
        uint256 betNumKind = betInfo.betNumKind;
        if (!betInfo.kindBet[betPairIndex]) {
            betNumKind++;
        }
        require(betNumKind <= match_.maxBetNumKindForEach, "over each person bet kinds");
    ...
    function batchCreateMatch(uint256[] memory ids, Match[] memory matchs, BetPair[] calldata betPairs) external onlyRole(COMMITER_ROLE) {
        for (uint i = 0; i < matchs.length; i++) {
            _createMatch(_msgSender(), ids[i], matchs[i], betPairs[i]);
        }
    }
    ...
    function reveal(uint256 id, uint256[] calldata optionIndexs_) external onlyRole(COMMITER_ROLE) {
        ...
        for (uint256 i = 0; i < optionIndexs_.length; i++) {
            ...
            uint256 payFee = totalAmount * match_.fee / DECIMAL;
    ...
    function _refund(address account, uint256 id, uint256 betPairIndex, uint256 optionPairIndex) internal {
        ...
        uint256 amount = betInfo.usrBet[betPairIndex][optionPairIndex];
        require(amount > 0, "not bet");

        uint256 leftTotal = 0;
        for (uint256 i = 0; i < betPair.pairs[betPairIndex].amounts.length; i++) {
            leftTotal += betInfo.usrBet[betPairIndex][i];
        }
        if (leftTotal == amount) {
        ...
        IERC20(match_.betToken).transfer(account, amount);
    ...
    function getOdds(uint256 id) external view returns (BetOdds[] memory) {
        BetPair memory betPair = pairInfos[id];
        uint256 fee = matchInfos[id].fee;
        BetOdds[] memory odds = new BetOdds[](betPair.pairs.length);
        for (uint256 i = 0; i < betPair.pairs.length; i++) {
            uint256 totalAmount = 0;
            for (uint256 j = 0; j < betPair.pairs[i].amounts.length; j++) {
                totalAmount += betPair.pairs[i].amounts[j];
            }
            uint256 payFee = totalAmount * fee / DECIMAL;
            uint256[] memory amounts = new uint256[](betPair.pairs[i].amounts.length);
            uint256[] memory rewardPerShares = new uint256[](betPair.pairs[i].amounts.length);
            for (uint256 j = 0; j < betPair.pairs[i].amounts.length; j++) {
                amounts[j] = betPair.pairs[i].amounts[j];
                rewardPerShares[j] = (totalAmount - payFee) * DECIMAL / betPair.pairs[i].amounts[j];
            }
            odds[i] = BetOdds({amounts: amounts,rewardPerShares: rewardPerShares});
        }
        return odds;
    }

    function getUsrBetAmount(address[] memory accounts, uint256[] memory ids, uint256[] memory betPairIndex, uint256[] memory optionPairIndex) external view returns(uint256[] memory) {
        uint256[] memory amounts = new uint256[](accounts.length);
        for (uint256 i = 0; i < accounts.length; i++) {
            amounts[i] = userBetInfos[accounts[i]][ids[i]].usrBet[betPairIndex[i]][optionPairIndex[i]];
        }
        return amounts;
    }

    function getUsrClaimAmount(address[] memory accounts, uint256[] memory ids, uint256[] memory betPairIndex, uint256[] memory optionPairIndex) external view returns(uint256[] memory) {
        uint256[] memory amounts = new uint256[](accounts.length);
        for (uint256 i = 0; i < accounts.length; i++) {
            amounts[i] = userBetInfos[accounts[i]][ids[i]].usrClaim[betPairIndex[i]][optionPairIndex[i]];
        }
        return amounts;
    }
