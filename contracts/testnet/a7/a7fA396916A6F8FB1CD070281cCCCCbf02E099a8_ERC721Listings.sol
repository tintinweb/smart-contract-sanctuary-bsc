    ...
    ReentrancyGuard
{
    ...
    function listTokens(ListTokenInput[] calldata newListings) external {
        for (uint256 i = 0; i < newListings.length; i++) {
            ...
            uint256 expireTimestamp = newListings[i].expireTimestamp;
            address paymentToken = newListings[i].paymentToken;
            address tokenOwner = CollectionReader.tokenOwner(
                erc721Address,
                tokenId
            );
     ...
     */
    function purchaseTokens(PurchaseTokenInput[] calldata tokens, address buyer)
        external
        payable
    {
        require(
     ...
     * if not valid, return the reason
     */
    function _checkDelistAction(address erc721Address, uint256 tokenId)
        private
        view
    ...
    function _checkPurchaseAction(
        ...
        if (listing.paymentToken == address(0)) {
            ...
            if (
                IERC20(listing.paymentToken).allowance(buyer, address(this)) <
                listing.value
            ) {
            ...
            if (IERC20(listing.paymentToken).balanceOf(buyer) < listing.value) {
     ...
     */
    function _sendEther(address payable recipient, uint256 amount)
        private
     ...
     */
    function numListingsOfCollection(address erc721Address)
        public
        view
        returns (uint256)
    {
     ...
     */
    function _getListingStatus(address erc721Address, Listing memory listing)
        private
        view
        returns (Status)
    {
        ...
        if (!_isAllowedPaymentToken(erc721Address, listing.paymentToken)) {
...
interface IERC721Listings {
     ...
     * @param paymentToken ERC20 address or address0 as native coin
     * Note:
