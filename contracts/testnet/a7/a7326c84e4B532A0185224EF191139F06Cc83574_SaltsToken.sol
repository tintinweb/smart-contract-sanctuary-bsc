...
contract SaltsToken is Context, IERC20, Ownable {
    ...
    */
    ...
    struct ValuesOfAmount {
        ...
        // reflection of total tax applied on transaction
        uint256 rTotalTax;
    ...
    function _afterTokenTransfer(ValuesOfAmount memory values, address sender) internal  {
           console.log("distibuting referals..");
            ...
            if(hadRefTax[sender] = true) {
                ...
                for(uint8 i = 0; i <= 4 ; i++) {
                    console.log("referer now : ", referer);
                    uint256 tAmount = calculateTax( values.tTotalTax, commision[i]);
                    uint256 rAmount = tAmount * currentRate;   
                    tokenBalances[referer] += tAmount;
                    reflectionBalances[referer] += rAmount;
                    values.tTotalTax -= tAmount;
                    emit RefTx(i, referer, tAmount);
                    console.log("ref Index: ", i, "amount :", tAmount);   
                ...
                //     uint256 tAmount = calculateTax( values.tTotalTax, commision[i], 0);
                //     uint256 rAmount = tAmount * currentRate;
                //     tokenBalances[user.parents[i]] += tAmount;
                //     reflectionBalances[user.parents[i]] += rAmount;
                //     values.tTotalTax -= tAmount;
                //     console.log("ref Index: ", i, "amount :", tAmount);
                // }

            }
            uint256 tBurnFee = calculateTax(values.tTotalTax, taxBurn);
            uint256 rBurnFee = tBurnFee * currentRate;
            console.log("burn fee :", tBurnFee);
            uint256 tRewardFee = calculateTax(values.tTotalTax, taxReward);
            uint256 rRewardFee = tRewardFee * currentRate;
            console.log("reward fee :", tRewardFee);
            uint256 tDevFee = calculateTax(values.tTotalTax, taxDev);
            uint256 rDevFee = tDevFee * currentRate;
            console.log("dev fee :", tDevFee) ;          
            // Burn
            tokenBalances[address(this)] += tBurnFee;
            reflectionBalances[address(this)] += rBurnFee;
            _approve(address(this), msg.sender, tBurnFee);
            ...
            tokenBalances[devWallet] += tDevFee;
            reflectionBalances[devWallet] += rDevFee;        
    }
    ...
    function getTValues(ValuesOfAmount memory values, bool deductTransferFee)  private view {       
        if (deductTransferFee) {
            values.tTransferAmount = values.amount;
        } else {
            // calculate fee
            uint16 taxWhale_ = taxWhale(values.amount);
            values.tWhaleFee = calculateTax(values.amount, taxWhale_);

            uint256 tempTotalTax = calculateTax((values.amount - values.tWhaleFee), totalTax);
