...
contract EAGLENFT is ERC721 {
    ...
    event currencyHolderChange(address owner,uint serial);
    ...
    function setNFTMintPrice(uint128 newTigerEagleCardPrice,uint128 newPhoenixEagleCardPrice)external{
        require(_msgSender() == MarketingWallet);
    ...
    function removeOwnerId(address user,uint256 tokenId)private{
        ...
        for (uint i = index;i<myNFT[user].length-1;i++){
    ...
    function getorderIdUseTokenId(uint256 _tokenId)public view returns(uint256){
    ...
    function getReceive(uint256 tokenId)external view returns(bool){
        uint256 receiveNum = (block.timestamp - NFTConfig[tokenId].creationTime) / 5 minutes;
        if (receiveNum > 12) {
            receiveNum = 12;
        }
