...
contract StiTigerDaoFactory is OwnableUpgradeable {
    ...
    function createTigerDao(uint[3] calldata quotas_, address[2] calldata tokens_, uint[4] calldata times_) public onlyBanker returns (address) {
    ...
    function getTigerDaoInfo(address dao_, address account_) public view  returns(address daoToken, string memory name, uint totalSupply,  uint[8] memory infos, uint[6] memory userData) {
...
interface ITigerDao {
    function viewInfo(address account_) external view returns(address daoToken, string memory name, uint totalSupply,  uint[8] memory infos, uint[6] memory userData);
    ...
    function init(uint totalSupply, uint[3] calldata quotas_, address[2] calldata tokens_, uint[4] calldata times_) external;
}
...
contract TigerDao  {
    ...
    // quiotas_[1] 目标
    // quiotas_[2] 单地址额度
    function init(uint totalSupply_, uint[3] calldata quotas_, address[2] calldata tokens_, uint[4] calldata times_) public onlyBanker {
    ...
    function viewInfo(address account_) public view returns(address daoToken, string memory name, uint totalSupply,  uint[8] memory infos, uint[6] memory userData) {
        daoToken = address(meta.daoToken);
        name = meta.daoToken.symbol();
    ...
    function projectInfo() public view returns(uint nextTm, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota, uint cycle)  {
        swapQuota = meta.buyAmount;

        if (meta1.claimCycle < times.releaseCycle) {
            cycle = meta1.lastClaimTm == 0 ? times.end : meta1.lastClaimTm;
            cycle = block.timestamp > cycle ? block.timestamp - cycle : 0;
            cycle /= times.cycleTm;

            // release done 
            if ((cycle + meta1.claimCycle) >= times.releaseCycle) {
                cycle = times.releaseCycle - meta1.claimCycle;
            } else {
                nextTm = meta1.lastClaimTm + times.cycleTm; 
                nextTm = meta1.lastClaimTm == 0 ? times.end : meta1.lastClaimTm;
                nextTm +=  (cycle +1) * times.cycleTm; 
            }

            claimQuota = cycle * (meta.buyAmount / times.releaseCycle);
        }
        claimedQuota = meta1.claimed;
    }
    ...
    function viewUserInfo(address account_) public view returns(uint nextTm, uint quota, uint swapQuota, uint claimQuota, uint claimedQuota, uint cycle) {
        ...
        quota = (meta.quota - uInfo.quota) ;

        // uint quotaWithSti = quota * 100000 / meta.price  / idoDecimals;        
        swapQuota = uInfo.quota ;
        // swapQuota = swapQuota  * 100000 / meta.price;

        (claimedQuota, nextTm, cycle) = viewReleaseQuota(account_);
        claimedQuota = uInfo.claimed;
    }

    function viewReleaseQuota(address account_) public view returns(uint quota, uint nextTm, uint cycle) {
        User memory uInfo = userInfo[account_];
        if (uInfo.claimedCycle >= times.releaseCycle) {
            return (0, 0, 0);
        }
        ...
        // release done 
        if ((cycle + uInfo.claimedCycle) >= times.releaseCycle) {
            cycle = times.releaseCycle - uInfo.claimedCycle;
        } else {
            nextTm = uInfo.lastClaimTm == 0 ? times.end : uInfo.lastClaimTm;
            nextTm +=  (cycle +1) * times.cycleTm; 
        }
        
        quota = cycle * (uInfo.quota / times.releaseCycle);
    }

    function _withdraw() private {
        (, , , uint claimQuota, , uint cycle) = projectInfo();
        require(cycle > 0, "not release");

        claimQuota = cycle * (meta.buyAmount / times.releaseCycle);

        meta1.claimed += claimQuota;
        meta1.claimCycle += cycle;
        meta1.lastClaimTm = block.timestamp;

        meta.sti.transfer(msg.sender, claimQuota);
    }
