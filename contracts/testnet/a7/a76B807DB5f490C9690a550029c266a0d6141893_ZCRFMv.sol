...
contract ZCRFMv is ERC777 {
    ...
    function getprice(address _recipient) external view returns (uint256)
    {
        uint nfts = users[_recipient].nfts;
        uint256 tokens = users[_recipient].tokens;
        uint256 price = nfts.div(tokens);
        return price;
    }
