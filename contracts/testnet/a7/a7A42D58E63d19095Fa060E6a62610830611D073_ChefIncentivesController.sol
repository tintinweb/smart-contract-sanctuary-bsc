...
contract ChefIncentivesController is Initializable, OwnableUpgradeable {
    ...
    function claim(address _user, address[] memory _tokens) public {
        if (ELIGIBILITY_ENABLED) {
            // We identify market dq by checking lastEligibleStatus
            // So, must refresh after DQ.processUser
            eligibleDataProvider.updatePrice();
            if (!eligibleDataProvider.isEligibleForRewards(_user)) {
                disqualifier.processUser(_user, true);
            }
        ...
        uint256 lowReserveBal = block.timestamp + 5 days;
        if (endRewardTime() < lowReserveBal) {
    ...
    function handleActionAfter(
        ...
        if (ELIGIBILITY_ENABLED) {
            ...
            if (
                ...
                eligibleDataProvider.getDqTime(_user) == 0
            ) {
                disqualifier.processUser(_user, true);
            }
            eligibleDataProvider.refresh(_user);
    ...
    function beforeLockUpdate(address _user) external {
        ...
        if (ELIGIBILITY_ENABLED) {
            ...
            bool isRelock = !_getLpMfd().hasAutoRelockDisabled(_user);
            if (userBounty != 0 && !isRelock) {
                disqualifier.processUser(_user, false);
            }
        }
    }
                ...
                for (uint256 i = 0; i < registeredTokens.length; i++) {
                    ...
                    if (newBal != 0) {
                        _handleActionAfterForToken(
                            ...
                            _user,
                            newBal,
                            poolInfo[registeredTokens[i]].totalSupply.add(
                                newBal
                            ).sub(userInfo[registeredTokens[i]][_user].amount)
                        );
                    }
                }
            }
    ...
    function disqualifyUser(address _user, address _hunter)
        external
        returns (uint256 bountyPaid)
    {
        require(ELIGIBILITY_ENABLED, "!EE");
        require(msg.sender == address(disqualifier), "!dq");
        require(
            !eligibleDataProvider.isEligibleForRewards(_user),
            "user is still eligible"
        );

        uint256 bounty = bountyForUser(_user);
        ...
        if (bounty > 0) {
            bountyPaid = _payBounty(_hunter, _user, bounty);
        }
    }
    ...
    function _payBounty(
        ...
        require(ELIGIBILITY_ENABLED, "!EE");
        if (_bounty < userBaseClaimable[_prey]) {
            userBaseClaimable[_prey] = userBaseClaimable[_prey].sub(_bounty);
        } else {
            _bounty = userBaseClaimable[_prey];
            userBaseClaimable[_prey] = 0;
        }
        return _sendRadiant(address(disqualifier), _bounty);
    }
        ...
        returns (uint256)
    {
        address rdntToken = rewardMinter.getRdntTokenAddress();
        if (_amount != 0) {
            if (IERC20(rdntToken).balanceOf(address(this)) < _amount) {
                _amount = 0; //IERC20(rdntToken).balanceOf(address(this));
            }
    ...
    function endRewardTime() public view returns (uint256 timestamp) {
        ...
        for (uint256 i; i < registeredTokens.length; i++) {
            if (
                ...
                extra = extra.add(poolInfo[registeredTokens[i]]
                    .lastRewardTime
