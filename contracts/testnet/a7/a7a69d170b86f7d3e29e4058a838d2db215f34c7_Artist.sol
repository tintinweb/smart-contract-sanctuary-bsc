...
contract Artist is ERC3525, AccessControl {
    ...
    {
        return
            ERC3525.supportsInterface(interfaceId) ||
            AccessControl.supportsInterface(interfaceId);
    }

    mapping(uint256 => string) private _slotURIs;
    mapping(uint256 => bool) public verifyed;

    constructor() ERC3525("Artist", "Artist", 0) {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    function mint(
        address to,
        string memory _slotURI
    ) public onlyAdmin {
        uint256 tokenId_ = _getNewTokenId(ERC721Enumerable.totalSupply());
        _slotURIs[tokenId_] = _slotURI;
        ERC3525._mintValue(to, tokenId_, tokenId_, 0);
    }
    ...
    {
        return _slotURIs[tokenId];
    }

    function setTokenURI(uint256 tokenId_, string memory tokenURI_) public onlyAdmin {
        _slotURIs[tokenId_] = tokenURI_;
    }

    function verify(uint256 tokenId_, bool verifyed_) public onlyAdmin {
        verifyed[tokenId_] = verifyed_;
    }

    function addValue(uint256 tokenId_, uint256 value_) public onlyAdmin {
        require(tokenId_ != 0, "ERC3525: cannot mint zero tokenId");
        require(_exists(tokenId_), "ERC3525: token not minted");
        require(verifyed[tokenId_], "ERC3525: token not verifyed");
