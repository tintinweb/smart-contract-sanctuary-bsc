 ...
 */
contract Marketplace is AccessControl, Pausable, ERC1155Holder {
    ...
    constructor(
        ...
        test1155 = IERC1155(_nftContract);
  ...
  */
     ...
     */
    function listFixedPrice(uint256[] memory itemId, uint256 basePrice)
        external
        whenNotPaused
        notZero(basePrice)
    {
        ...
        for (uint8 i = 0; i < len; i++) {
            _addOrder(itemId[i], basePrice, 0, OrderType.FixedPrice);
        }
    }
    ...
    function listAuction(
        ...
        uint256 len = itemId.length;
        for (uint8 i = 0; i < len; i++) {
            _addOrder(itemId[i], basePrice, bidStep, OrderType.Auction);
        }
    }
        ...
        whenNotPaused
    {
        ...
        if (_tokenUsed) {
            ...
            IERC20(_token).transferFrom(msg.sender, order.maker, amount);
        } else {
            require(order.basePrice == msg.value, "Not enough amont paid");
    ...
    function makeBid(
        ...
        if (_tokenUsed) {
            ...
            require(msg.value == bidAmount, "Not enough amont paid");
            bids[orderId][order.numBids] = Bid({
                amount: bidAmount,
                bidder: msg.sender,
                isToken: false
        ...
        if (numBids > 0) {
            if (lastBid.isToken) {
                ...
                payable(lastBidder).transfer(lastBid.amount);
            }
        }
        ...
        if (numBids > 2) {
            ...
            if (lastBid.isToken) {
                ...
                uint256 amount = tAmount >
                    ...
                    ? IERC20(_token).balanceOf(address(this))
                    : tAmount;
                IERC20(_token).transfer(order.maker, amount);
            } else {
                payable(order.maker).transfer(lastBid.amount);
            }
            _cancelOrder(orderId, true);
        } else {
            ...
            if (numBids > 0) {
                if (lastBid.isToken) {
                    ...
                    uint256 amount = tAmount >
                        ...
                        ? IERC20(_token).balanceOf(address(this))
                        : tAmount;
                    IERC20(_token).transfer(lastBid.bidder, amount);
                } else {
    ...
    function _transferItem(
        ...
        test1155.safeTransferFrom(from, to, itemId, 1, "");
    }
