...
contract GForcexXx2 is Ownable {
 ...
 function joinWithBnb(address uplineId) public payable {
        join(1, msg.sender, uplineId, true, false);
    }
    function join(address uplineId) public {
        join(1, msg.sender, uplineId, true, true);
    }

 fallback() external {
     
    }
    function defaultPayable() external payable {
