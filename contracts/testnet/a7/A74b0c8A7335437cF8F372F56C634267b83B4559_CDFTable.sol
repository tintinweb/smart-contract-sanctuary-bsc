...
import "./ICDFTable.sol";

contract CDFTable is Ownable, ICDFTable {
    using ABDKMath64x64 for int128;
    using SafeMath for uint256;
    int128 public constant ONE_64x64 = 0x10000000000000000; // 2^64

    int128[] public fKeys;
    mapping(int128 => int128) public fValues;

    event RowsAdded(address indexed sender, uint256 count);

    /**
     * @notice Table population.
     * @dev Keys should be sorted.
     */
    function addRows(int128[] calldata _fKeys, int128[] calldata _fValues) external override onlyOwner {
        require(_fKeys.length == _fValues.length, "arrays mismatch");

        uint256 uiLength = _fKeys.length;

        int128 fPreviousKey;
        if (fKeys.length > 0) {
            fPreviousKey = fKeys[fKeys.length - 1];
        } else {
            fPreviousKey = type(int128).min;
        }

        for (uint256 i = 0; i < uiLength; i++) {
            int128 fKey = _fKeys[i];
            require(fKey > fPreviousKey, "keys aren't sorted");

            fKeys.push(fKey);
            fValues[fKey] = _fValues[i];
            fPreviousKey = fKey;
        }
        emit RowsAdded(msg.sender, _fKeys.length);
    }

    /**
     * @notice Get normal-CDF function value for x=_fKey.
     *
     */
    function getValue(int128 _fKey) public view virtual override returns (int128) {
        int128 fValue;
        bool isGTZero = _fKey > 0;
        if (isGTZero) {
            /* Using Phi(-x) = 1 - Phi(x) for values >0,
               So we only have to store (x,f(x))-pairs for x<0 */
            _fKey = _fKey.neg();
        }
        (int128 fStartKey, int128 fEndKey) = _getKeyRange(_fKey);
        if (fStartKey == fEndKey) {
            fValue = fValues[fStartKey];
        } else {
            int128 fStartValue = fValues[fStartKey];
            int128 fEndValue = fValues[fEndKey];
            int128 fStartDiff = _fKey.sub(fStartKey);
            int128 fEndDiff = fEndKey.sub(_fKey);
            int128 fTotalDiff = fStartDiff.add(fEndDiff);

            /// @dev Calculates an average based on weight for both values.
            fValue = fStartValue.mul(fEndDiff).div(fTotalDiff).add(fEndValue.mul(fStartDiff).div(fTotalDiff));
        }
        if (isGTZero) {
            fValue = ONE_64x64.sub(fValue);
        }
        return fValue;
    }
     ...
     */
    function _getKeyRange(int128 _fKey) internal view returns (int128, int128) {
        uint256 uiLength = fKeys.length;
        int128 fFirstKey = fKeys[0];
        int128 fLastKey = fKeys[uiLength - 1];
        if (_fKey <= fFirstKey) {
            return (fFirstKey, fFirstKey);
        } else if (_fKey >= fLastKey) {
            return (fLastKey, fLastKey);
        }

        uint256 uiStartIndex = 0;
        uint256 uiEndIndex = uiLength - 1;
        int128 fStartKey;
        int128 fEndKey;
        while (uiEndIndex.sub(uiStartIndex) > 1) {
            uint256 centerIndex = uiStartIndex.add((uiEndIndex.sub(uiStartIndex)).div(2));
            if (centerIndex == uiLength - 2) {
                fStartKey = fKeys[uiLength - 2];
                fEndKey = fKeys[uiLength - 1];
            } else {
                fStartKey = fKeys[centerIndex - 1];
                fEndKey = fKeys[centerIndex];
            }

            if (_fKey == fStartKey) {
                return (fStartKey, fStartKey);
            } else if (_fKey == fEndKey) {
                return (fEndKey, fEndKey);
            } else if (fStartKey < _fKey && _fKey < fEndKey) {
                return (fStartKey, fEndKey);
            } else if (_fKey < fStartKey) {
                uiEndIndex = centerIndex;
            } else {
                uiStartIndex = centerIndex;
            }
        }
        return (fKeys[uiStartIndex], fKeys[uiEndIndex]);
    }
}
