...
contract Staking is  OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, GlobalsAndUtils {
    ...
    function setGlobals(address _oracle) public onlyOwner {
		
        oracle = _oracle;
         MIN_STAKE_DAYS = 10 minutes;
         EARLY_UNSTAKE_PENALTY = 18;
         STAKING_TOKEN_DECIMALS = 1;
     ...
     */
    function stake() whenNotPaused external payable nonReentrant CampaignOnGoing {
        uint256 stakedAmount = msg.value;
        if (stakedAmount <= 0 || stakedAmount.mod(MIN_STAKE_TOKENS) != 0) {
            (bool success, ) = msg.sender.call{value: stakedAmount}("");
            require(success, "couldn't transfer tokens");
            revert("staked tokens must be multiple of 100");
        }
     ...
     */
    function claimOrEmergencyUnstak(uint256 _stakeId) whenNotPaused nonReentrant public {
        ApplyUnstake memory usrStake = applyUnstake[msg.sender][_stakeId];
    ...
    function appplyForUnstake(uint256 _stakeId)  whenNotPaused external {
        ...
        require(usrStake.stakedAmount > 0, "stake doesn't exist");
        _unStake(_stakeId);
        emit AppliedUnstake(msg.sender, _stakeId, usrStake.stakedAmount);
    }
    ...
    function getUserStakesIds(address stakeOwner) external view returns (uint256[] memory) {
     ...
     /**
     * @dev PUBLIC FACING: Returns an array of ids of user's applied unstakes
     * @param stakeOwner Address of the user to get the stake ids
     * @return Stake Ids
     */
    function getUserAppliedIds(address stakeOwner) external view returns (uint256[] memory) {
        return userApplyUnstakeIds[stakeOwner];
    }
    ...
    /**
     * @dev PUBLIC FACING: Returns information about a specific application
     * @param stakeOwner Address of the stake owner
     * @param stakeId ID of the stake
     * @return User stake
     */
    function getApplyUnStake(address stakeOwner, uint256 stakeId) external view returns(ApplyUnstake memory) {
         
        return applyUnstake[stakeOwner][stakeId];
    }
...
/**
     ...
     * @return User stake
     */
    function getIsClaimableOrEmergency(address stakeOwner, uint256 stakeId) external view returns(bool) {
        ApplyUnstake memory usrStake = applyUnstake[stakeOwner][stakeId];
        (, uint256 penaltyAmount) = _calcPayoutAndPenalty(usrStake);
 ...
 /**
    ...
    function claimTaxBonus() whenNotPaused external nonReentrant  {
        ...
        //  uint256 campaignId = campaigns[latestCampaignId].startTime > block.timestamp && latestCampaignId > 1 ? latestCampaignId - 1 : latestCampaignId;
         uint256 campaignId = campaigns[latestCampaignId].startTime <= block.timestamp && (campaigns[latestCampaignId].endTime + CLAIM_X_TICKET_DURATION) > block.timestamp && latestCampaignId > 1 ? latestCampaignId - 1 : 0;
        ...
        (bool success, ) = msg.sender.call{value: stakerPenaltyBonus}("");
        require(success, "couldn't transfer tokens");
    }
    ...
    function startLootBox(uint256 rewardCount) external onlyOwner campaignEnded {
        ...
        Campaign storage temp = campaigns[latestCampaignId];
    ...
    function setRewardCanBeClaim(uint256 campaignId, bool status) onlyOwner public {
        require(winningTicketIds[campaignId].length  == campaigns[campaignId].rewardCount, "reward not distributed");
...
//             if(_rangesStart[i] <= _rangesEnd[i]){
//                 userXTicketRange[_stakers[i]][_campaignId].start = _rangesStart[i];
//                userXTicketRange[_stakers[i]][_campaignId].end = _rangesEnd[i];

//             totalUserXTickets[_stakers[i]][_campaignId] = _rangesEnd[i] - _rangesStart[i] + 1;
//             emit RuffleJoined(_stakers[i], _campaignId, _rangesStart[i], _rangesEnd[i]);
//            }
//         }
//     }

//     function migratePenaltyHistory(
//         address[] calldata stakers, 
//         uint256[] calldata releasedAtTimes,
//         uint256[] calldata penaltyIds,
//         uint256[] calldata totalStakedAmounts,
//         uint256[] calldata userStakedAmounts
//     ) external onlyOwner {
//         for (uint256 i=0; i<stakers.length; i++) {
//             penaltyHistoryIds[stakers[i]].push(penaltyIds[i]);
//             penaltyHistories[stakers[i]][penaltyIds[i]] = penaltyHistory(releasedAtTimes[i], totalStakedAmounts[i], userStakedAmounts[i], 0, 0, 0);
//         }
//     }

//     function migratePenaltyHistoryOtherHalf(
//         address[] calldata stakers,
//         uint256[] calldata penaltyIds,
//         uint256[] calldata totalPenaltyAmounts,
//         uint256[] calldata penaltiesClaimed,
//         uint256[] calldata campaignIds
//     ) external onlyOwner {
//         for (uint256 i=0; i<stakers.length; i++) {
//             penaltyHistories[stakers[i]][penaltyIds[i]].totalPenalty = totalPenaltyAmounts[i];
//             penaltyHistories[stakers[i]][penaltyIds[i]].penaltyClaimed = penaltiesClaimed[i];
//             penaltyHistories[stakers[i]][penaltyIds[i]].campaignId = campaignIds[i];
//             userClaimedPenalty[stakers[i]][campaignIds[i]] = true;
//         }
//     }

//     function migrateRewardsReceived(address[] memory _stakers, uint256[] memory _campaignIds, uint256[] memory _rewardCount) external onlyOwner {
//         for (uint256 i = 0; i < _stakers.length; i++) {
//             rewardsReceived[_stakers[i]][_campaignIds[i]] = _rewardCount[i];
//         }
//     }

//     function migrateTotalUserTickets(address[] memory _stakers, uint256[] memory _campaignIds, uint256[] memory _ticketsCount) external onlyOwner {
//         for (uint256 i = 0; i < _stakers.length; i++) {
//             totalUserXTickets[_stakers[i]][_campaignIds[i]] = _ticketsCount[i];
//         }
//     }

//     function migrateWinningTickets(uint256 _campaignId, uint256[] memory _winningTickets, bool reset) external onlyOwner {
//         if(reset) {
//             delete winningTicketIds[_campaignId];
//         }

//         for (uint256 i = 0; i < _winningTickets.length; i++) {
//             winningTicketIds[_campaignId].push(_winningTickets[i]);
//             winningTixketIdExist[_campaignId][_winningTickets[i]] = true;
//         }
//     }

    

//     function migrateStateVariables(uint256 _latestCampaignId, uint256 _totalStakedAmount, uint256 _totalClaimableTickets, uint256 _latestStakeId, uint256 _totalPenaltiesClaimed) external onlyOwner {
//     latestCampaignId = _latestCampaignId;
//     totalStakedAmount = _totalStakedAmount;
//     totalClaimableTickets = _totalClaimableTickets;
//     latestStakeId = _latestStakeId;
//     totalPenaltiesClaimed = _totalPenaltiesClaimed;
// }
    
//     function migratePenaltyData(uint256 _campaignId, uint256 _penaltyAccumulated) external onlyOwner {
//         penaltyAccumulated[_campaignId] = _penaltyAccumulated;
//     }

//     function resetUserPenaltiesIds(address[] calldata _addresses) external onlyOwner {
//         for (uint256 index = 0; index < _addresses.length; index++) {
//             delete penaltyHistoryIds[_addresses[index]];
//         }
//     }

    function resetWinningTicketIds(uint256 _campaignId, uint256 end) external onlyOwner {
        require(winningTicketIds[_campaignId].length > 0);
        for(uint256 i; i< end; i++){
             winningTicketIds[_campaignId].pop();
        }
    }
    ...
    function _unStake(uint256 _stakeId) private {
        ...
        userApplyUnstakeIds[msg.sender].push(_stakeId);

        userStakedAmount[msg.sender] -= stakes[msg.sender][_stakeId].stakedAmount;

        applyUnstake[msg.sender][_stakeId] = ApplyUnstake(stakes[msg.sender][_stakeId].stakedAt,block.timestamp,stakes[msg.sender][_stakeId].stakedAmount);
    ...
    function _removeAppliedFor(uint256 _stakeId) private {
        // Remove stake id from users' stakIdArray
        if (userApplyUnstakeIds[msg.sender].length > 1) {
            for (uint256 x = 0; x < userApplyUnstakeIds[msg.sender].length; x++) {
                // find the index of stake id in userStakes
                if (userApplyUnstakeIds[msg.sender][x] == _stakeId) {
                    if (userApplyUnstakeIds[msg.sender].length > 1) {
                        userApplyUnstakeIds[msg.sender][x] = userApplyUnstakeIds[msg.sender][userApplyUnstakeIds[msg.sender].length.sub(1)];
                        userApplyUnstakeIds[msg.sender].pop();
                    } else {
                        userApplyUnstakeIds[msg.sender].pop();
                    }
                }
        ...
        unStakes[msg.sender][_stakeId] = UnStake(applyUnstake[msg.sender][_stakeId].stakedAt, applyUnstake[msg.sender][_stakeId].appliedAt,block.timestamp,applyUnstake[msg.sender][_stakeId].stakedAmount, 0);

        // Remove user's stake values
        delete applyUnstake[msg.sender][_stakeId];
    }
     ...
     */
    function _getUserStakedTime(ApplyUnstake memory usrStake) private view returns (uint256 unixStakedTime) {
     ...
     * @return penalty
     */
    function _calcPayoutAndPenalty(ApplyUnstake memory usrStake) private view returns(uint256 payout, uint256 penalty) {
    ...
    function _stakerPenaltyBonus(address _staker, uint256 campaignId) allowedClaim(campaignId) private view returns (uint256) {
        ...
        uint256 unixStakedTime = block.timestamp;
        for (uint256 x=0; x < userStakeIds[msg.sender].length; x++) {
            if (stakes[msg.sender][userStakeIds[msg.sender][x]].stakedAt < unixStakedTime) {
                unixStakedTime = stakes[msg.sender][userStakeIds[msg.sender][x]].stakedAt;
            }
        }
        unixStakedTime = block.timestamp.sub(unixStakedTime);
...
abstract contract GlobalsAndUtils {
    ...
    event AppliedUnstake(address staker, uint256 stakeIndex, uint256 stakeAmount);
    ...
    modifier allowedClaim(uint256 campaignId) {
        require(campaigns[latestCampaignId].startTime < block.timestamp && (campaigns[latestCampaignId].endTime + CLAIM_X_TICKET_DURATION) > block.timestamp && latestCampaignId > 1, "not valid time");
    ...
    /// @notice Mapping to store user unstakes.
    mapping(address => mapping(uint256 => ApplyUnstake)) internal applyUnstake;

     /// @notice Mapping to store user unstake ids in array
    mapping(address => uint256[]) internal userApplyUnstakeIds;
