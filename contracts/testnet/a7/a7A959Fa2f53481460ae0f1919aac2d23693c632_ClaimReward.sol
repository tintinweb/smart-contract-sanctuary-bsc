...
contract ClaimReward is Pausable, Ownable {
    ...
    event CashRewarded(address recipient, uint256 rewardType);

    event FoodItemRewarded(address recipient, uint256 rewardType);

    constructor(address signer, address _lootBoxAddress) {
        _signer = signer;
    ...
    function openLootBox(
        ...
        uint256 randomNumber = _validateRequest(requestData, signature);
        IERC1155Burnable(lootBoxAddress).burn(msg.sender, tokenId, 1);
        if (randomNumber >= 1 && randomNumber <= 5) {
            emit CashRewarded(msg.sender, 1);
        } else {
            emit FoodItemRewarded(msg.sender, 1);
        }
    }
    ...
    function _validateRequest(
        ...
        uint256 randomNumber = request.randomNumber;
    ...
    function setLootBoxAddress(address newLootBoxAddress) external onlyOwner {
        lootBoxAddress = newLootBoxAddress;
    }
}
