    ...
    OwnableUpgradeable
{
    ...
    address private constant BNB_TO_USD = 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;
    ...
    function getTokenPrice() public payable returns (uint256) {
        (uint256 token, uint256 wbnb, ) = UNISWAP_PAIR.getReserves();

        //get token price in wbnb
        uint256 wbnbToToken = wbnb/1^18/token/1^9 * 1^8;

        int bnbPrice = _getLatestPriceBNBTOUSD();
