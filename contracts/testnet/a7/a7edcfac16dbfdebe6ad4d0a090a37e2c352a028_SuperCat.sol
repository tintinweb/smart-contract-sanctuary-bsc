...
contract SuperCat is Context, Ownable{
    ...
    function bindNFT(uint256 usdt_amount,uint256 cat_amount,uint256 _nftId,address _nft_contract) public{
        ...
        if(nft_pledge[_nft_contract][_nftId].exp_time>block.timestamp){
        ...
        if(amountA>amountB){
            if(usdt_amount>amountA){
                uint256 usdt_return = amountA - usdt_amount;
                IERC20(usdtToken).transfer(msg.sender, usdt_return);
                usdt_amount = amountA;
            }
            if(cat_amount>amountB){
                uint256 cat_return = amountB - cat_amount;
                IERC20(catToken).transfer(msg.sender, cat_return);
                cat_amount = amountB;
            }
        }else{
            if(usdt_amount>amountB){
                uint256 usdt_return = amountB - usdt_amount;
                IERC20(usdtToken).transfer(msg.sender, usdt_return);
                usdt_amount = amountB;
            }
            if(cat_amount>amountA){
                uint256 cat_return = amountA - cat_amount;
                IERC20(catToken).transfer(msg.sender, cat_return);
                cat_amount = amountA;
            }
        }
        ...
        team_usdt(lp_pool[msg.sender].usdt);
    ...
    function addLPLiquidity(uint256 usdt_amount,uint256 cat_amount) public{
        ...
        (uint256 amountA, uint256 amountB, uint256 liquidity) = Router.addLiquidity(usdtToken,catToken,usdt_amount,cat_amount,usdt_amount.mul(90).div(100),cat_amount.mul(90).div(100),msg.sender,block.timestamp);
        // if(amountA>amountB){
        //     if(usdt_amount>amountA){
        //         uint256 usdt_return = amountA - usdt_amount;
        //         IERC20(usdtToken).transfer(msg.sender, usdt_return);
    ...
    function receiveProfit() public {
        ...
        if(_amount>0){
            ...
            emit ReceiveProfit(msg.sender,_amount,_time_interval,block.timestamp,lp_pool[msg.sender].usdt,lp_weight_total,lp_release);
        }
    }
    ...
    function receiveTeamProfit() public{
        ...
        uint256 _amount = (re_release /86400) * (lp_pool[msg.sender].team_usdt / lp_weight_total) * _time_interval;
        if(_amount>0){
            IERC20(catToken).transfer(msg.sender, _amount);
            lp_pool[msg.sender].team_receive_time = block.timestamp;
            nft_pledge_info[msg.sender].miner_cat = nft_pledge_info[msg.sender].miner_cat + _amount;
            emit ReceiveProfit(msg.sender,_amount,_time_interval,block.timestamp,lp_pool[msg.sender].team_usdt,lp_weight_total,re_release);
        }
    }
    ...
    function mintNFT(address to,address contractAddress,bool _is)  public onlyOwner {
        ...
        if(_is){
            nft_pledge[contractAddress][nft_id].is_mine = true;
    ...
    //添加团队业绩
    function team_usdt(uint256 _amount) private{
        if(_amount>0){
            uint256 team_amount;
            ...
            for (uint i = 1; i <= 15; i++) {
                ...
                lp_pool[pre].team_usdt =  lp_pool[pre].team_usdt.add(team_amount);
                pre = relation[pre];
            }
        }
    }
