...
contract Auction is Ownable{
    ...
    event CreatedAuction(uint indexed auctionID, address _token, uint _tokenAmount, uint _tokenType, uint _endTime, uint256 time, uint _bidPrice);
    event DeletedAuction(uint _auctionId, uint256 time);
    event Bid(address indexed _bidder, uint indexed _auctionId, uint256 endtime, address _lastBidder, uint _remainingBid, uint _bidCount);
    ...
    function bid(uint _auctionID) payable public {
        ...
        uint _endTime = block.timestamp + 120;
        if(auctionInfo[_auctionID].endTime < _endTime){
            auctionInfo[_auctionID].endTime = _endTime;
        }
        emit Bid(msg.sender, _auctionID, auctionInfo[_auctionID].endTime , msg.sender, auctionInfo[_auctionID].remainingBid, auctionInfo[_auctionID].bidCount);
    } 
    ...
    function claim(uint _auctionID) payable public {     
        ...
        if(auctionInfo[_auctionID].tokenType == 0){
            ...
            IERC721(auctionInfo[_auctionID].token).transferFrom(address(this),auctionInfo[_auctionID].lastBidder, auctionInfo[_auctionID].tokenAmount);
        }
