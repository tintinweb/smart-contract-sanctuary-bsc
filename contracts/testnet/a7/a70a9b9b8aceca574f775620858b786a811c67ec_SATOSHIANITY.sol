...
abstract contract WalletSizeLimit is Ownable {

    error WalletLimitReached();

    uint internal walletLimit;
    mapping(address => bool) excludedFromWalletSizeLimit;
    ...
    function setExcludedFromWalletSizeLimit(address addr, bool excluded) public onlyOwner {
        excludedFromWalletSizeLimit[addr] = excluded;
    }

    function walletSizeCheck(address recipient, uint balance, bool isSell) internal {
        if(!isSell && balance > walletLimit && !excludedFromWalletSizeLimit[recipient]){
...
contract SATOSHIANITY is Context, IERC20, Values {
    ...
    modifier debugger(address sender, address recipient, uint256 amount){
        console.log("Sender balace before: ", sender,  balanceOf(sender));
        _;
        console.log("Sender balace after: ", sender,  balanceOf(sender));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private debugger(sender, recipient, amount) {
        ...
        if((sender == pairAddress || recipient == pairAddress) && !(excludedFromTaxes[sender] || excludedFromTaxes[recipient])){
        ...
        walletSizeCheck(recipient, balanceOf(recipient), recipient == pairAddress);
...
abstract contract Values is Ownable, WalletSizeLimit {
