		...
		if(automatedMarketMakerPairs[to]) 
		{
            ...
            if (!inSwapAndLiquify && canSwap && swapAndLiquifyEnabled) {
				if(!liquifyAll)
				{
					...
					uint256 totalFee = shillToEarnFee.add(marketingFee).add(dynamicLiquidityFee);
					
					uint256 fromLiquidityFee = swapTokensAtAmount.div(totalFee).mul(dynamicLiquidityFee);
					...
					uint256 liquidityPart   = newBalance.div(totalFee).mul(dynamicLiquidityFee);
				...
				{
					swapTokensAtAmount = balanceOf(address(this));
					uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : liquidityFee;
					uint256 totalFee = shillToEarnFee.add(marketingFee).add(dynamicLiquidityFee);
					
					uint256 fromLiquidityFee = contractTokenBalance.div(totalFee).mul(dynamicLiquidityFee);
