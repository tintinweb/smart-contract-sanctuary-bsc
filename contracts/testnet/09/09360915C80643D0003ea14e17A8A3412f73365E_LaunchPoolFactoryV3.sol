...
contract LaunchPool {
    ...
    constructor(
        ...
        require(
            _cap[0] >= _cap[1] * 25 / 100,
            "Softcap must be greater than 25% of hardcap"
        );
    ...
    function setVestingPeriodAndClaim(
        ...
        _setVestingPeriodAndClaim(_isVesting, _vests, _claims);
    }
    ...
    function _setVestingPeriodAndClaim(
        ...
        if (_isVesting) {
            ...
            require(
                ...
                "Raising period already started"
            );
            uint256 total;
    ...
    function _finalize() internal {
        ...
        if (IERC20(tokenB) == IERC20(wBNB)) {
            ...
            uint256 forFee = (balance * 5) / 100;
            uint256 forLiquidity = ((balance - forFee) *
                addToLiquidityPercentage) / 100;
            uint256 forOwner = balance - forFee - forLiquidity;

            payable(feeReceiver).transfer(forFee);
            ...
            IERC20(tokenB).transfer(feeReceiver, forFee);
        ...
        if (IERC20(tokenB) == IERC20(wBNB)) {
            IERC20(tokenB).deposit{value: amountB}();
        }
        ...
        uint256 refundAmount = IERC20(tokenA).balanceOf(address(this)).sub(
            presaleAmount
        );
    ...
    function setRelease(Release _release) external onlyOwner {
        ...
        require(release == Release.NOT_SET, "Already released");
        if (isCheckSoftCap) {
        ...
        checkPeriod(uint256(_claim))
    {
        ...
        if (isVesting == false) {
            require(claimInPeriod[msg.sender][0] == false, "already claim");
            ...
            amountClaim = userInfo.totalToken * vestingClaim[_claim] / 100;
            tokenSell.transfer(msg.sender, amountClaim);
...
contract LaunchPoolFactoryV3 is Ownable {
    ...
    function setAmountFee(uint256 _feeAmount) public onlyOwner {
        amountFee = _feeAmount;
    }

    function withdrawBNB(uint256 _amount) public onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > _amount, "does not have any balance");
    ...
    function createNewLaunchPool(
        ...
        uint256 forListing = (((_cap[1] * 95) / 100) * _rate[1] * _rate[2]) /
            100;
