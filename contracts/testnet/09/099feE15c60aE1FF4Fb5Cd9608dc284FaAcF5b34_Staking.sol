    ...
    ERC20Upgradeable
{
    ...
    function calculateReward(address _account) external view returns (uint){  
        uint256 totalReward = (balanceOf(_account).mul(ectAddress.balanceOf(address(this)).add(IRewardPool(rewardPoolAddress).waveAmount())).div(
            totalSupply()
        ));
    ...
    function unstake(uint256 _share) external isRewardSet {
        IRewardPool(rewardPoolAddress).wave();
        uint256 totalShares = totalSupply();
        uint256 what = _share.mul(ectAddress.balanceOf(address(this))).div(totalShares);
        require(_share<= balanceOf(msg.sender),"ECT-Staking: Insufficient Balance");
