...
import "../interfaces/IBUSDProtocol.sol";
 ...
 */
contract BUSDProtocol is AccessControl, Initializable {
    using SafeERC20 for IERC20;

    uint256[] reBalancePercentage = [9000, 1000]; // Ratio between treasury and POL
    uint256 coff = 100; //cofficient for optimise calculation
    IAdmin public  Admin; 
    ...
    function initialize(
        ...
        require(
            ...
            "BUSDProtocol:: initialize: Treasury address can not be null"
        );
    ...
    Note Need to call setReBalanceValues first for 
        ...
        only admin can call 
     */
    function reBalancePool() external onlyRole(ADMIN_ROLE) {
        uint256 netBalance;
        ...
        uint256 treasuryReBalance = (total * reBalancePercentage[0]) / (100 * coff);
        uint256 POLReBalance = ((total * reBalancePercentage[1]) / (100 * coff));
        if ((treasuryBalance > treasuryReBalance) && (POLBalance < POLReBalance)) {
            netBalance = treasuryBalance - treasuryReBalance;
            IBUSDProtocol(Admin.Treasury()).withdraw(netBalance);
        } else if ((treasuryBalance < treasuryReBalance) && (POLBalance > POLReBalance)) {
            netBalance = POLBalance - POLReBalance;
            IBUSDProtocol(Admin.POL()).withdrawFromPOL(Admin.Treasury(), netBalance);
        }
    }
     ...
     */
    function setReBalanceValues(uint256[] memory _reBalancePercentage)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            _reBalancePercentage.length == 2,
            "BUSDProtocol:: setReBalanceValues: Array length must be equal to 2"
        );
