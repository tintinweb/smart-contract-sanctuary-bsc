...
contract Treasury is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public accRwardPerShare;
    ...
    function addPool(uint256 allocPoint, uint256 lockSecond) external onlyRole(OPERATOR_ROLE) {
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lockSecond: lockSecond}));
    ...
    function initRewardTime(uint256 _nextRewardTime) external onlyRole(OPERATOR_ROLE) {
        require(nextRewardTime == 0, "already init");
    ...
    function mint(uint256 pid, uint256 amount) external {
        ...
        uint256 dueTime = block.timestamp + poolInfo[pid].lockSecond;
        bondsInfo[tokenId] = BondsInfo({amount: amount, shareAmount: shareAmount, dueTime: dueTime, rewardDebt: (shareAmount * accRwardPerShare) / 1e18});
    ...
    function redeem(uint256 tokenId) external {
        ...
        totalBondsAmount -= amount;
        // totalShareAmount -= data.shareAmount;
    ...
    function processReward() external {
        ...
        if (totalShareAmount > 0) {
            accRwardPerShare += (rewardAmount * 1e18) / totalShareAmount;
            totalRewardAmount -= rewardAmount;
        }
        //处理已到期的债券
        uint256 interval = nextRewardTime / rewardInterval;
        if (expireAmounts[interval] > 0) {
            accCheckpoints[interval] = accRwardPerShare;
    ...
    function getAccRwardPerShare(uint256 dueTime) internal view returns (uint256) {
        ...
        return expireAcc > 0 ? expireAcc : accRwardPerShare;
    }

    function getInterval(uint256 time) internal view returns (uint256) {
        return (time + rewardInterval - 1) / rewardInterval;
    }
