...
contract StoreBase is ERC165, Ownable, ERC1155Metadata_URI,  ERC1155Base {
    ...
    // Creates a batch of new token types and assings supply to minter
    function _mintBatch(address from, address to,uint256[] memory _id, uint256[] memory _supply, string[] memory _uri) internal {

        for (uint256 i = 0; i < _id.length; ++i) {
            creators[_id[i]] = from;
            _balances[_id[i]][to] = _balances[_id[i]][to].add(_supply[i]);
            _setTokenURI(_id[i], _uri[i]);
            emit TransferSingle(msg.sender, address(0x0), to, _id[i], _supply[i]);
            emit URI(_uri[i], _id[i]);
        }
    }
...
// File: contracts/QubeMintableStore.sol

pragma solidity 0.6.12;
pragma experimental ABIEncoderV2;

contract WQMintableStore is Ownable, SignerRole, StoreBase {
    string public name;
    ...
    function setBlockLimit(uint256 newAmt) public onlyOwner {
        blockLimit = newAmt;
    }
    ...
    function mintBatch(
        ...
        _mintBatch(from, to, id, supply, uri);
