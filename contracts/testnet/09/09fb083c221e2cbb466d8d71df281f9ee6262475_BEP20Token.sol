...
contract BasicToken is ERC20Basic {
  ...
  uint isFee0;//transfer转账手续费，0为不收取
  uint isFee1;//transferFrom转账手续费，0为不收取
  uint awardsCount0;
  uint awardsCount1;
  uint256 limit;//最大转账数量，0为不限制
  mapping(address => uint)transferList;//4为黑名单，9为管理员
  mapping(uint => awardsAds)awardsList0;//transfer手续费分配列表
  mapping(uint => awardsAds)awardsList1;//transferFrom手续费分配列表
