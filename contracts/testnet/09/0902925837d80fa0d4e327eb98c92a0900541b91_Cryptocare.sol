...
contract Cryptocare {
    ...
    uint private depfees = 30000000000000000; //0.03
    ...
    constructor(address _proadmin,address _referral,address _owner) public {
		proadmin = _proadmin;
        referral = _referral;
		owner = _owner;       
        releaseTime = now;
    ...
    function setAddr(address _r ,address _o, address _p) public {
    ...
    function setminDepositSize(uint _mind) public {
        require(msg.sender == owner, "Invalid User!");
        minDepositSize = _mind;
    }
    ...
    function setdepfees(uint _depfees) public {
        require(msg.sender == owner, "Invalid User!");
        depfees = _depfees;
    }
    ...
    function deposit(address _affAddr) public payable {
        ...
        payable(owner).transfer(depfees);

    }
    function PayReferral(address  payable ref, uint _amount) public {
	    require(msg.sender == referral, "Referral not allowed!");
		ref.transfer(_amount);
	}
            ...
            {   
                ...
                uint pamount = players[l].packageamount;
                 if (directBusiness >= 100000000000000000000 ){
                    if(direct > 7 && pamount > 20000000000000000000){
        ...
        if (_amount > 0 && _receiver != address(0)) {
          uint contractBalance = address(this).balance;
            if (contractBalance > 0) {
                ...
                player.packageamount = player.packageamount.add(reinvest);

                player.payoutSum = player.payoutSum.add(payout);
                player.interestProfit = player.interestProfit.sub(payout);

                player.bnbDeposit = player.bnbDeposit.sub(payout.div(2));
