...
contract RootOfPools_v2 is Initializable, DoubleSignature {
    ...
    function createPool(string calldata name, address pool) public dSign {
    ...
    function changeStepValue(string calldata name, uint256 step) public dSign {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );
    ...
    function startFundraising(string calldata name) public dSign {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );
    ...
    function collectFunds(string calldata name) public dSign {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );
    ...
    function stopFundraising(string calldata name) public dSign {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );
    ...
    function stopEmergency(string calldata name) public dSign {
        require(
            _poolsTable[name] != address(0),
            "ROOT: Selected pool does not exist!"
        );
...
contract DoubleSignature is Initializable {
    bytes4 public _firstSign;
    address public _owner1;
    address public _owner2;
    uint8 private flag; //0 - null 1 - owner1 2 - owner2

    function _dSign_init(address owner1, address owner2) public initializer {
        _owner1 = owner1;
        _owner2 = owner2;
    }
    ...
    modifier dSign() {
        require(
            ...
            "DSignature: caller is not the owner"
        );

        if (msg.sender == _owner1) {
            if (flag == 2) {
                if (msg.sig == _firstSign) {
                    _firstSign = 0;
                    _;
                }
            }
