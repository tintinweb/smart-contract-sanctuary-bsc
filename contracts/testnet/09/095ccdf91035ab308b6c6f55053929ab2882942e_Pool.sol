...
contract Pool is Initializable, PoolStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool {
    ...
    function _calcDecreasePayout(
        ...
        // clean user input
        vars.sizeChanged = _min(_position.size, _sizeChanged);
        ...
        if (vars.pnl.isNeg()) {
            // cap the value of PnL to collateral value after fee in case of lost
            vars.pnl.abs = _min(vars.pnl.abs, MathUtils.zeroCapSub(_position.collateralValue, vars.feeValue));
        }
