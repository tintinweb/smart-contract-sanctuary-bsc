...
contract CardSale is Member {
    ...
    mapping(uint256 => uint256) public uintPrice;

    IERC20 public tokenContract;

    constructor(uint256[] memory _price) {
        for (uint256 index = 0; index < _price.length; index++) {
            uintPrice[index + 1] = _price[index];
        }
    }

    function openSale() external CheckPermit("Admin") {
        if (isOpen) {
            isOpen = false;
        } else {
            isOpen = true;
    ...
    function setCardPrice(uint256[] memory _price)
        external
        CheckPermit("Admin")
    {
        for (uint256 index = 0; index < _price.length; index++) {
    ...
    function setSaleToken(address _token) external CheckPermit("Admin") {
        tokenContract = IERC20(_token);
    }

    function buy(uint8 cardType) external {
        require(isOpen, "Sale not open");
        require(uintPrice[cardType] > 0, "not price");
        ...
        NFTCard(manager.members("card")).mint(msg.sender, cardType);
    }
}
