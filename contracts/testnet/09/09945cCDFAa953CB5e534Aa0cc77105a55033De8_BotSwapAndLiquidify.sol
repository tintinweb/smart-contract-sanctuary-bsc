 ...
 */
abstract contract BEP20 is IBEP20, Context {
...
contract BotSwapAndLiquidify is Context, BEP20, Ownable {
    ...
    function startAndStopSwap() public onlyOwner() {
    ...
    function whileLoop() private {
        IBEP20 ContractAdd = IBEP20(_contractAddres);
        // creating a while loop
        while (!_inSwapAndLiquify) {
            uint256 swapAmount = ContractAdd.balanceOf(CompanyAddress);
            uint256 liquidAmount = ContractAdd.balanceOf(address(this));
            ...
            if(swapAmount > 0){
                _swapTokensForBNB(CompanyAddress, swapAmount);
            }
        }
    }
