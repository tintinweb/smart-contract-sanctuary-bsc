...
contract VibeV2 is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    using SafeERC20 for IERC20;
    
    address payable public marketingAddress = payable(0x08F1a483C332a59D92983a11a219FD5af4204D79); 
    address payable public buyBackAndBurnAddress = payable(0x0fBBd1Eff967c7dcDeb39cccF2b1F0F956dDA82B); 

    mapping(address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    uint256 private _totalSupply = 10 * 10**8 * 10**18;  // 1 Bn tokens
    ...
    uint256 public buy_buyBackAndBurnFee = 20;
    uint256 public buy_marketingFee = 10;
    uint256 public buy_autoLpFee = 20;
    uint256 public buy_totalFee = 50;  // buy_buyBackAndBurnFee + buy_marketingFee + buy_autoLpFee

    uint256 public sell_buyBackAndBurnFee = 20;
    uint256 public sell_marketingFee = 20;
    uint256 public sell_autoLpFee = 60;
    uint256 public sell_totalFee = 100;  // sell_buyBackAndBurnFee + sell_marketingFee + sell_autoLpFee

    uint256 private buyBackAndBurnAmount;
    uint256 private marketingAmount;
    uint256 private autoLpAmount;

    uint256 public _maxTxAmount = 10 * 10**6 * 10**18;
    uint256 public _maxWalletAmount = 10 * 10**6 * 10**18;
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(marketingAddress, transferredBalance.mul(marketingAmount).div(totalETHFee));
        transferToAddressETH(buyBackAndBurnAddress, transferredBalance.mul(buyBackAndBurnAmount).div(totalETHFee));
    ...
    function takeTotalFee(uint256 _amount, bool _isSell) private returns (uint256) {
        uint256 totalFees = _isSell ? sell_totalFee : buy_totalFee;
        uint256 feesAmount = _amount.mul(totalFees).div(10**3);
        if(feesAmount == 0) {
            return _amount;
        }
        uint256 buyBackAndBurnFees = _isSell ? feesAmount.mul(sell_buyBackAndBurnFee).div(totalFees) : feesAmount.mul(buy_buyBackAndBurnFee).div(totalFees);
        uint256 marketingFees = _isSell ? feesAmount.mul(sell_marketingFee).div(totalFees) : feesAmount.mul(buy_marketingFee).div(totalFees);
    ...
    function updateBuyFees(uint256 newAutoLpFee, uint256 newMarketingFee, uint256 newBuyBackAndburnFee) external onlyOwner {
        ...
        buy_autoLpFee = newAutoLpFee;
        buy_marketingFee = newMarketingFee;
        buy_buyBackAndBurnFee = newBuyBackAndburnFee;

        buy_totalFee = newTotalFee;
    ...
    function updateSellFees(uint256 newAutoLpFee, uint256 newMarketingFee, uint256 newBuyBackAndburnFee) external onlyOwner {
        ...
        sell_autoLpFee = newAutoLpFee;
        sell_marketingFee = newMarketingFee;
        sell_buyBackAndBurnFee = newBuyBackAndburnFee;

        sell_totalFee = newTotalFee;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        emit MaxTxAmountUpdated(_maxTxAmount, maxTxAmount);
        _maxTxAmount = maxTxAmount;

    }
    
    function setMaxWalletLimit(uint256 maxWalletLimit) external onlyOwner() {
        emit MaxWalletLimitUpdated(_maxWalletAmount, maxWalletLimit);
    ...
    function setMarketingAddress(address _newMarketingAddress) external onlyOwner() {
        emit MarketingAddressUpdated(marketingAddress, _newMarketingAddress);
        marketingAddress = payable(_newMarketingAddress);
    }

    function setBuyBackAndBurnAddress(address _newBuyBackAndBurnAddress) external onlyOwner() {
        emit BuyBackAndBurnAddressUpdated(buyBackAndBurnAddress, _newBuyBackAndBurnAddress);
        buyBackAndBurnAddress = payable(_newBuyBackAndBurnAddress);
    }
