...
contract ProjectIDO is Ownable {
	...
	address public projectToken; //ido token
	uint256 public projectTokenAmount;
	uint256 public projectTokenClaimed;

	uint256 public claimNumerator;
	uint256 public claimDenominator;

	uint256 public rewardVNSAmount;
	uint256 public vnsPrice;

	mapping(address => ProjectIdoUser) public users;
	...
	function getUserMaxIdoAmount(address user)
		public
		view
		returns (
			uint256 vnsQuota,
	...
	function setVnsPrice(uint256 price) external onlyOwner {
		vnsPrice = price;
	}
	...
	function getIdoConfig()
		...
		returns (
			...
			address uniV2Factory,
			address vnsAddress,
			uint256 vnsTotalQuota,
			address cpdAddress,
			uint256 cpdTotalQuota,
			address usdtAddress,
			uint256 usdtTotalQuota
		)
	{
		startTime = config.startTime;
		endTime = config.endTime;
		claimExpTime = config.claimExpTime;
		memberShip = config.memberShip;
		uniV2Factory = config.uniV2Factory;
		vnsAddress = config.tokenAddress[0];
		vnsTotalQuota = config.tokenTotalQuota[0];
		cpdAddress = config.tokenAddress[1];
		cpdTotalQuota = config.tokenTotalQuota[1];
		usdtAddress = config.tokenAddress[2];
		usdtTotalQuota = config.tokenTotalQuota[2];
	}
	...
	function injectProjectToken(address token, uint256 amount) external onlyOwner {
		require(block.timestamp > config.endTime, "invalid time1");
		require(startClaimTime == 0, "invalid time2");
		projectToken = token;
		projectTokenAmount = amount;
		IERC20(projectToken).transferFrom(msg.sender, address(this), amount);
	}

	function startClaim(uint256 numerator, uint256 denominator) external onlyOwner {
		require(block.timestamp > config.endTime, "invalid time0");
		require(startClaimTime == 0, "invalid time1");
		claimNumerator = numerator;
		claimDenominator = denominator;
	...
	function claim(TokenType tokentype, uint256 amount) external {
		...
		uint256 pending = (uamount * claimNumerator) / claimDenominator;

		IERC20(projectToken).transfer(msg.sender, pending);

		emit Claim(msg.sender, projectToken, pending);

		user.projectTokenClaimed = pending;
		projectTokenClaimed += pending;
