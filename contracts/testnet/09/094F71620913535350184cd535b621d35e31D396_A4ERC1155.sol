...
contract A4ERC1155 is ERC1155Burnable, IERC2981, Ownable {

    using SafeMath for uint256;

    event Minted(uint256 tokenId, uint256 tokenQty, string tokenUri, address minter);
    ...
    function mint(string calldata _uri, uint256 _tokenQty, uint8 _royaltyPercentage, bytes memory data) public onlyOwner() {
        ...
        _mint(tx.origin, tokenId, _tokenQty, data);
        royaltyReceiver[tokenId] = tx.origin;
        royaltyPercentage[tokenId] = _royaltyPercentage;
        _tokenUri[tokenId] = _uri;

        emit Minted(tokenId, _tokenQty, _uri, _msgSender());
    }
