...
contract vestingClaims is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function claimTest(uint _nftId,uint8 _matchId) public {
        ...
        if(_startTime < getCurrentTime()){
            ...
            if(!nftData[_nftId][_matchId].initialized){
                ...
                uint256 _amount= getunreleasedAmounts(_nftId,_matchId);
    ...
    function getunreleasedAmounts(uint _nftId,uint8 _matchId) public  view returns(uint256 _claimAmount){
        ...
        if(timeLeft/slicePeriodSeconds > 0){
         claimAmount = ((_amount*slicePeriod)/totalVestingDays)*_decimal*(timeLeft/slicePeriodSeconds) ;
        }
    ...
    function getVestingDetails(uint _nftId,uint8 _matchId) public view returns(uint _claimAmount) {
        // uint256 currentTime = getCurrentTime();
        uint256 timeLeft = 0;
        // uint _decimal = getDecimal();
        uint slicePeriodSeconds = slicePeriod * timeUnit;
        ...
        if(!nftData[_nftId][_matchId].initialized){
            ...
            if(timeLeft/slicePeriodSeconds > 0){
                claimAmount = ((_amount*slicePeriod)/totalVestingDays)*getDecimal()*(timeLeft/slicePeriodSeconds) ;
            }
            ...
            if(claimAmount > _amount){
                ...
                return (claimAmount);
                // return (_tokenAmount,_totalClaimed,(_amount/getDecimal()),_tokenAmount.sub(_amount/getDecimal()),_lastClaimedAt);
            }
            
            return (claimAmount);

            // return (_tokenAmount,_totalClaimed,(claimAmount/getDecimal()),_tokenAmount.sub(_amount/getDecimal()),_lastClaimedAt);
        
        }else{

            if(getCurrentTime()>nftData[_nftId][_matchId].lastClaimedAt){
                timeLeft = getCurrentTime().sub(nftData[_nftId][_matchId].lastClaimedAt);
            }else{
                timeLeft =  nftData[_nftId][_matchId].lastClaimedAt.sub(getCurrentTime());
            }

            _amount = nftData[_nftId][_matchId].totalEligible;
            claimAmount =0;
            if(timeLeft/slicePeriodSeconds > 0){
             claimAmount = ((_amount*slicePeriod)/totalVestingDays)*getDecimal()*(timeLeft/slicePeriodSeconds) ;
            }
            uint _lastReleaseAmount = nftData[_nftId][_matchId].totalClaimed;
            uint temp = nftData[_nftId][_matchId].totalClaimed.add(claimAmount/getDecimal());
            if(temp > nftData[_nftId][_matchId].totalEligible){
                _amount = nftData[_nftId][_matchId].totalEligible.sub(_lastReleaseAmount);

                return ((_amount/getDecimal()));
