...
contract MEDEX is MedexERC20 {
    ...
    function addLiquidity(uint256 amountA, uint256 amountB) public {
        ...
        uint256 amount = (busdPerMedex * amountA);
        amount = amount / 1e6;
        require(amount == amountB, "Invalid Input");
    ...
    function buy(uint256 tokenIN, uint256 amountIn) external {
        ...
        uint256 balance1 = BEP20(tokenB).balanceOf(address(this));
        if (tokenIN == 1) {
            uint256 amount = (busdPerMedex * amountIn);
            amount = amount / 1e6;
            require(amount <= balance1, "Balance is too low A");

            uint256 fees = (amount * liqudityProviderFees) / 1e8;
            uint256 feeamount = amount - fees;

            BEP20(tokenA).transferFrom(msg.sender, address(this), amountIn);
            BEP20(tokenB).transfer(msg.sender, feeamount);
        } else {
            uint256 amount = (amountIn / busdPerMedex);
            amount = amount * 1e6;
            require(amount <= balance0, "Balance is too low B");
            uint256 fees = (amount * liqudityProviderFees) / 1e8;
            uint256 feeamount = amount - fees;

            BEP20(tokenB).transferFrom(msg.sender, address(this), amountIn);
            BEP20(tokenA).transfer(msg.sender, feeamount);
        }
    }
