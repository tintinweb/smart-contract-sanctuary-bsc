...
/**
* Contract: Crypto Peso “USDP” Powered by Ekilibrio Tech
 *  Stablecoin Inspired by Copyright @ xSurge, @ MDB

 */
contract CryptoPeso is IERC20, Ownable, Pausable,ReentrancyGuard {
    ...
    address[] public stables;
      address public _resourceCollector;
    // Fees
    ...
    uint256 public min_max_holdings = 20_000 * 10**18;
    ...
    function setresourceCollector(address resourceCollector)public onlyOwner{
    ...
    */
    function sell(uint256 tokenAmount, address recipient) external whenNotPaused nonReentrant returns (uint256) {
    ...
    function exchange(address tokenIn, address tokenOut, uint256 tokenInAmount, address recipient) external whenNotPaused nonReentrant {
        require(
            tokenIn != address(0) && 
            tokenOut != address(0) && 
            recipient != address(0) &&
    ...
    */
    ...
    function _mintWithNative(address recipient, uint256 minOut) internal whenNotPaused nonReentrant returns (uint256) {        
    ...
    function _sell(address seller, uint256 tokenAmount, address recipient) internal whenNotPaused returns (uint256) {
    ...
    function sellDownAccountToMaximumHoldings(address account) external nonReentrant onlyOwner {
        ...
        // convert to USDP
        uint256 usdpToSell = amtToSellBUSD.mul(precision).div(_calculatePrice());
    ...
    */
    function setFees(uint256 _mintFee, uint256 _transferFee, uint256 _sellFee, uint256 _stableSwapFee) external onlyOwner {
              require(_mintFee >= 97000 &&_mintFee <= 300000);       // capped at 3% fee
        require(_transferFee >= 97000 && _transferFee <= 300000);   // capped at 3% fee
        require(_sellFee >= 97000 && _sellFee <= 300000);       // capped at 3% fee
        require(_stableSwapFee >= 97000 && _stableSwapFee <= 300000); // capped at 3% fee         
        mintFee = _mintFee;
