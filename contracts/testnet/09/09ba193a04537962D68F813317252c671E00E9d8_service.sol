...
contract service is Ownable, Pausable, ReentrancyGuard {
    ...
    event ApproveProjects(address indexed caller, Service ProjectData, uint256 executionTime);
    ...
    function approveProjects(address _devAddress, address _tokenAddress, uint256 _tokenAmount) external payable tokenApproved(_tokenAddress) whenNotPaused nonReentrant{
        ...
        emit ApproveProjects(msg.sender, serviceInfo[projectCount], block.timestamp);
    }
    ...
    function submitProvider(uint256 _projectID) external {
        ...
        if(project.tokenAddress == address(0x0)){
            require(payable(project.developer).send( project.amount - (project.amount * project.devlevelPercentage / 1000)),"Invalid amount");
        }else {
    ...
    function sendToProvider(uint256 _projectID) external onlyOwner {
        ...
        if(project.tokenAddress == address(0x0)){
            require(payable(project.provider).send( project.amount - (project.amount * project.devlevelPercentage / 1000)),"Invalid amount");
        }else {
