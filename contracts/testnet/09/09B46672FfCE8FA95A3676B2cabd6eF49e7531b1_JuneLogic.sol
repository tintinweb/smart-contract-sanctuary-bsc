...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Console, Manager {
    using SafeMathUpgradeable for uint256;
    uint8 private _decimals;
    address public leftToken;
    address public rightToken;
    ...
    event ev_swapLeftToRight(address indexed from, uint256 amount);
    event ev_swapRightToLeft(address indexed from, uint256 amount);

    function initializeToData(address _fromTo, string memory _name, string memory _symbol, uint8 __decimals, uint256 initialSupply, address _MarketingWalletAddr, uint256 _DayMintAmount, uint256 _DayBurnAmount, uint256 _WithdrawBurnFee, address _WithdrawalAddr, address _MintAddr) public view returns (bool){
        return true;
    }
    ...
    function swapLeftToRight(uint256 amount) public {
        IERC20Upgradeable tokenA = IERC20Upgradeable(leftToken);
        IERC20Upgradeable tokenB = IERC20Upgradeable(rightToken);
        uint256 _before = tokenA.balanceOf(address(this));
        tokenA.transferFrom(msg.sender, address(this), amount);
        uint256 _received = tokenA.balanceOf(address(this)).sub(_before);
        uint256 _balanceB = tokenB.balanceOf(address(this));
        uint256 _provide = _received.div(ratio).mul(PERCENTS_DIVIDER);
        require(_balanceB >= _provide, "swap:Sorry, your credit is running low");
        tokenB.transfer(msg.sender, _provide);
        emit ev_swapLeftToRight(msg.sender, amount);
    }

    function swapRightToLeft(uint256 amount) public {
        IERC20Upgradeable tokenA = IERC20Upgradeable(leftToken);
        IERC20Upgradeable tokenB = IERC20Upgradeable(rightToken);
        require(
            tokenA.balanceOf(address(this)) >= amount,
            "Swap:Insufficient quantity available"
        );
        uint256 _before = tokenB.balanceOf(address(this));
        tokenB.transferFrom(msg.sender, address(this), amount);
        uint256 _received = tokenB.balanceOf(address(this)).sub(_before);
        tokenA.transfer(
            msg.sender,
            _received.div(PERCENTS_DIVIDER).div(ratio)
        );
        emit ev_swapRightToLeft(msg.sender, amount);
    }
    // Token withdrawal
    function drawBalance(address token, address _to, uint256 amount) public onlyDraw {
        IERC20Upgradeable _token = IERC20Upgradeable(token);
        require(
            _token.balanceOf(token) > amount,
            "draw:Insufficient quantity available"
        );
        _token.transfer(_to, amount);
    }
    // Token withdrawal all
    function drawBalanceLocal(address _to, uint256 amount) public onlyDraw {
        ...
        _transfer(address(this), _to, tokenBalance);
    }

    function getLeftToRight(uint256 amount) public view returns (uint256) {
        //Amount ÷ ratio * PERCENTS
        return amount.div(ratio).mul(PERCENTS_DIVIDER);
    }

    function getRightToLeft(uint256 amount) public view returns (uint256) {
        //Amount ÷ PERCENTS * ratio
        return amount.div(PERCENTS_DIVIDER).mul(ratio);
    }

    function updateRatio(uint256 amount) public onlyOwner {
        ratio = amount;
    }
    ...
    function setLeftToken(address _leftToken) public onlyManager {
        leftToken = _leftToken;
    }
    ...
    function setRightToken(address _rightToken) public onlyManager {
        rightToken = _rightToken;
    }
    ...
    // Set Batch Config
    function setBatchSettings(address _marketing, uint256 _rate, uint256 _mintAmount, uint256 _burnAmount, address _WithdrawalAddr, address _MintAddr) public onlyManager {
        MarketingWalletAddr = _marketing;
        WithdrawalAddr = _WithdrawalAddr;
        MintAddr = _MintAddr;
        WithdrawBurnFee = _rate;
    ...
    function approveToToken(address token, uint256 amount) public onlyManager virtual returns (bool){
        IERC20Upgradeable _token = IERC20Upgradeable(token);
    ...
    function mintTo() public onlyMint {
        require(DayMintAmount > 0, "DayMintAmount:Less than Or Equal to 0");
        require(MintAddr != address(0), "mintTo:this MintAddr not address");
        _mint(MintAddr, DayMintAmount);
    }

    function burn(uint256 amount) public onlyBurn {
        _burn(MarketingWalletAddr, amount);
    }

    function burnTo() public onlyBurn {
        require(DayBurnAmount > 0, "burnTo:Less than Or Equal to 0");
        require(MintAddr != address(0), "burnTo:this MarketingWalletAddr not address");
