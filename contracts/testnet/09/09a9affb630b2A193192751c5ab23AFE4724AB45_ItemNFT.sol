...
contract ItemNFT is BaseNFT {
  ...
  function mintItemsForTest(address[] calldata _owners, uint16[] calldata _classIds, uint _price, uint _quantity) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      for(uint j = 0; j < _classIds.length; j++) {
        for(uint k = 0; k < _quantity; k++) {
          uint8 gene = uint8(_classIds[j] / 100);
          mintItem(_owners[i], gene, _classIds[j], _price, k);
        }
      }
    }
  }
