...
contract CPoolFactory {
    ...
    function createCPool(
        ...
        uint256 _maxParticipationPerParticipan,
        uint256 _performanceFee,
        uint256 _minAmount
    ) external validateCreation(_traderAmount, _performanceFee, _trader) {
        ...
        stableCoin.safeTransferFrom(_trader, address(this), _traderAmount);
...
// pensar rescates parciales y totales
contract CPool {
    using SafeERC20 for IERC20;
    ...
    event PartialUnsubscribe(address indexed _copier, address indexed _cPool);
    event TotalUnsubscribe(address indexed _copier, address indexed _cPool);
    ...
    constructor(
        ...
        totalParticipations += _traderAmount;
        participants[_trader] = _traderAmount;
        internalBalances[_stableCoin] = _traderAmount;
    ...
    function swap(
        ...
        internalBalances[_tokenIn] -= _amountIn;
        internalBalances[_tokenOut] += result[result.length - 1];
    }
    ...
    {
        uint256 participations = (_amount * (10**18)) / getParticipationPrice();

        stableCoin.safeTransferFrom(_copierAddress, address(this), _amount);
        participants[_copierAddress] += participations;
        totalParticipations += participations;

        emit Subscribe(_copierAddress, address(this), _amount, participations);
    }

    function partialUnsubscribe(uint256 _amount, address _copierAddress)
        external
        validateUnsubscribe(_amount, _copierAddress)
    {
        uint256 amount = (participants[_copierAddress] *
            getParticipationPrice()) / 10**18;

        require(_amount <= amount, "Insufficient balance in pool.");

        uint256 performanceFeeAmount = (amount / PERCENTS_DIVIDER) *
            performanceFee;
        ...
        totalParticipations -= participants[_copierAddress];
        participants[_copierAddress] = 0;

        emit PartialUnsubscribe(_copierAddress, address(this));
    }

    function totalUnsubscribe(address _copierAddress) external {
        uint256 amount = (participants[_copierAddress] *
            getParticipationPrice()) / 10**18;
        uint256 performanceFeeAmount = calcPerformanceFee(amount);
        stableCoin.safeTransfer(
            _copierAddress,
            (amount - performanceFeeAmount)
        );
        totalParticipations -= participants[_copierAddress];
        delete participants[_copierAddress];

        emit TotalUnsubscribe(_copierAddress, address(this));
    }

    /* ========== VIEWS ========== */

    function calcPerformanceFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return (_amount / PERCENTS_DIVIDER) * performanceFee;
    }
    ...
    modifier validateSubscribe(uint256 _amount, address _copierAddress) {
        ...
        _validateAmountToSubscribe(_amount, _copierAddress);
        _;
    }
    ...
    modifier validateTokens(address _tokenA, address _tokenB) {
        require(internalBalances[_tokenA] > 0, "Token not allowed to operate.");
        require(
    ...
    function _validateAmountToSubscribe(uint256 _amount, address _copierAddress)
        private
        view
    {
        uint256 stakedTokens = stakingContract.balanceOf(_copierAddress);
        require(
        ...
        if (participants[_copierAddress] == 0) {
