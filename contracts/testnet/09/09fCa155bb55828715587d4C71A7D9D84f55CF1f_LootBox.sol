    ...
    PriceControl
{
    ...
    function lootBoxFreeWhiteList(address _whiteListed) external {
        FreeWhiteList storage freeWhiteList = mapOfFreeWhiteList[_whiteListed];
    ...
    /// @param _whiteListed Address of the white listed
    function lootBoxDiscountWhiteList(address _whiteListed) external {
        DiscountWhiteList storage discountWhiteList = mapOfDiscountWhiteList[
            _whiteListed
        ];
        require(discountWhiteList.status, "Not in white list");
        uint256 price = (getPriceByTier(discountWhiteList.tier) *
            discountWhiteList.discount) / 10000;
        _paymentWithBUSD(price);
        _objectsGeneration(discountWhiteList.tier);
        _generateResources(discountWhiteList.tier);
