...
contract NFTSGARDEN is ERC721Enumerable, Ownable {
  using Strings for uint256;
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function _mint(uint256 tokenId,uint256 salePrice, uint256 royalty,SellType sellType, uint256 auctionStart, uint256 auctionExpire, address _collaborator) public payable returns(uint256){
    ...
    function multiMint(uint256[] memory tokenId,uint256[] memory salePrice, uint256[] memory royalty, SellType[] memory sellType, address[] memory _collaborator) public payable {
        ...
        require(
            sellType[i] == SellType.FIXED_PRICE || sellType[i] == SellType.AUCTION,
            "Wrong sell type"
        );
        
        _safeMint(msg.sender, tokenId[i]);
        payable(settlementFeeAddress).transfer(msg.value);
         tokenDetails[tokenId[i]] = _tokenDetails(msg.sender,_collaborator[i],msg.sender,royalty[i],0,0,salePrice[i],sellType[i],true,true,false);
        }
    ...
    function setCollaboratorOfNft(uint256 tokenId,address _collaborator) public {
    ...
    function buy(uint256 tokenId) payable public {
        ...
        if(tokenDetails[tokenId].royalty > 0 && tokenDetails[tokenId].firstSale){
                ...
                payable(tokenDetails[tokenId].collaborator).transfer(royaltyAmt);        
        }
