...
contract SC2 is Ownable {
    ...
    address public _SC1ADDRESS = 0x2164956dCA4d9ff201f800FFCAe029B61722eb65;

    uint256[] public staking_period = [3600 * 7, 3600 * 14, 3600 * 30, 3600 * 90, 3600 * 180]; // plan A, B, C, D, E
    uint256[] public staking_weight = [1, 3, 8, 36, 100];                                      // plan A, B, C, D, E

    uint256 public reward_eABCD_amount = 10;
    uint256 public reward_BUSD_amount = 10;
    ...
    struct stakeItem {
        uint256 amount;
        uint256 dateStaked;
        uint256 tokenType; //0 : unstaked, 1 : BUSD, 2 : eABCD
        uint256 stakeType; //0 : planA, 1 : planB, 2 : planC ...
    ...
    function stakeEABCD(uint256 amount, uint256 _type) public {
        ...
        require((amount + IBEP20(_eABCDADDRESS).balanceOf(address(this))) < _MAX_eABCD * 10 ** eABCD_decimal, "overflow with eABCD");
    ...
    function stakeBUSD(uint256 amount, uint256 _type) public {
        ...
        require((amount + IBEP20(_BUSDADDRESS).balanceOf(address(this))) < _MAX_BUSD * 10 ** BUSD_decimal, "overflow with BUSD");
    ...
    function getRewardInTHALESForOwner( address addr) internal view returns(uint256){
        ...
        for(uint i=0; i<stake_data[msg.sender].stakeElement.length; i++){
            ...
            uint256 stakedAmount = stake_data[msg.sender].stakeElement[i].amount;
            uint256 stakeType = stake_data[msg.sender].stakeElement[i].stakeType;
            uint256 stakedDate = stake_data[msg.sender].stakeElement[i].dateStaked;
            if(stake_data[msg.sender].stakeElement[i].tokenType == 2){
                totalReward += stakedAmount * reward_eABCD_amount * staking_weight[stakeType] * (block.timestamp - stakedDate) * ( 10 ** THALES_decimal) / staking_period[stakeType] / (10 ** eABCD_decimal);
            }
            if(stake_data[msg.sender].stakeElement[i].tokenType == 1){
                totalReward += stakedAmount * reward_BUSD_amount * staking_weight[stakeType] * (block.timestamp - stakedDate) * ( 10 ** THALES_decimal) / staking_period[stakeType] / ( 10 ** BUSD_decimal);
            }
        }
        return totalReward - stake_data[msg.sender].claimedAmount;
    }
    ...
    function claimRewardInTHALES(uint256 amount) public {
        ...
        stake_data[msg.sender].claimedAmount += amount;
    }
    function sellOrderForEABCD(uint256 amount) public {
        ...
        uint256 BUSD_For_eABCD = amount * tokenPriceWithOracle * (10 ** BUSD_decimal) * (100 * 1000 - feeForSeller) / 100 / 1000 / (10 ** eABCD_decimal);
