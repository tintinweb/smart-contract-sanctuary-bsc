    ...
    function takeFee(
        ...
        if (from == mocPair && to != mocPair) {
            ...
            if (IERC20(usdtAddress).balanceOf(mocPair) >= reserve0 + amountA) {
                ...
                if (_parentInviters.length > 0) {
                    for (uint8 i = 0; i < _parentInviters.length; i++) {
                        ...
                        if (_isTokenAccount(_parentInviters[i])) {
                            uint256 addInviteAmount = _balances[
                                _parentInviters[i]
                            ...
                            if (
                                ...
                                _gonBalances[
                                    _parentInviters[i]
                                ] = addInviteAmount.mul(_gonsPerFragment);
                            } else {
                ...
                emit InviteReward(from, DEAD, tempFee);
