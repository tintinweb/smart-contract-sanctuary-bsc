...
contract CrossFarmingReceiver is MessageSenderApp, MessageReceiverApp {
    ...
    function executeMessage(
        ...
        if (request.msgType == MessageTypes.Deposit) {
            ...
            // we can't accoring nonce == 0 to sent BNB CHANGE, if current contract balance < BNB_CHANGE in exetremely situation
        ...
        } else if (request.msgType == MessageTypes.Harvest) {
...
contract CrossFarmingProxy is ReentrancyGuard {
    ...
    /// @notice user's cross-chain tx can only be called one by one, can't skip.
    /// @dev if nonce=2 tx arrived before nonce=1 to current contract(Celer SGN or messagebus can't guarantee the message order),
    ///  nonce=2 tx should be reverted, only after nonce=1 execute success, executor can re-execute nonce=2 tx.
    modifier onlyNextNonce(uint256 _pid, uint64 _nonce) {
        //user 1st tx nonces[_pid] == _nonce == 0
        require(nonces[_pid] + 1 == _nonce || _nonce == 0, "Can't skip nonce");
        _;
    }
    ...
    function deposit(
        ...
        uint256 _amount,
        uint64 _nonce
    ) external nonReentrant onlyFactory onlyNextNonce(_pid, _nonce) {
    ...
    function emergencyWithdraw(
        ...
        require(userInfo[_pid] > 0, "No token left");
