...
contract BoredInu is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "Bored Inu";
    string constant _symbol = "BOR";
    ...
    constructor () Auth(msg.sender) {
        ...
        isWalletHoldingLimitExempt[msg.sender] = true;
    ...
    function checkWalletHoldingLimit(address sender, address recipient, uint256 amount) internal view {
        if(!isWalletHoldingLimitExempt[recipient] && sender != owner && recipient != owner){
            require( balanceOf(recipient).add(amount) <= _maxWalletHoldingAmount, "Wallet holding Limit Exceeded");
        }
    }

    function isWalletLimitExempt(address account)external view onlyOwner() returns(bool){
    ...
    function getMultipliedFee() public view returns (uint256) {
        if (launchedAtTimestamp + 1 days > block.timestamp) {
            return totalFee.mul(20000).div(feeDenominator);
        }
        return totalFee;
    }
    ...
    function setWalletHoldingLimitExempt(address holder, bool exempt) external authorized {
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator/4 ,"You can't set more than 25%");
        emit UpdatedTaxFeePercentages(totalFee);
    }
