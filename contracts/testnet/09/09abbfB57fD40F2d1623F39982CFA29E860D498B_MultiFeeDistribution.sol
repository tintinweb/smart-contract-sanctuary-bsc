            ...
            rewardData[_rewardsToken].rewardPerTokenStored.add(
                lastTimeRewardApplicable(_rewardsToken).sub(
                    rewardData[_rewardsToken].lastUpdateTime).mul(
                        rewardData[_rewardsToken].rewardRate).mul(1e9).div(_supply)
            );
    }
// making public for testing -> change to internal back
    function _earned(
        address _user,
        ...
        ).div(1e9).add(rewards[_user][_rewardsToken]);
    }
    ...
    function unstake(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        require(bal.total >= amount, "Insufficient staked balance");
