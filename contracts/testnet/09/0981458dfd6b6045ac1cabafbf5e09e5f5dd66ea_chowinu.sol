...
contract chowinu is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0x261B05cAeC74c14748b3F2CFcf6a7aD0D1747583);
    address payable public GiveawayWalletAddress = payable(0x1de8Ba5e171bCa482dBFa5bEF7E70d37bB9201E1);
    ...
    uint256 public _buyGiveawayFee = 4;
    
    uint256 public _sellLiquidityFee = 1;
    uint256 public _sellMarketingFee = 4;
    uint256 public _sellGiveawayFee = 4;

    uint256 public _liquidityShare = 20;
    uint256 public _marketingShare = 45;
    uint256 public _GiveawayShare = 35;
    ...
    event GiveawayGetBnb(uint256 amount);
    ...
    constructor () {
        ...
        _totalTaxIfBuying = _BuyLpFee.add(_BuyMarketingFee).add(_buyGiveawayFee);
        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellGiveawayFee);
        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_GiveawayShare);
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newGiveawayTax) external onlyOwner() {
        _BuyLpFee = newLiquidityTax;
        _BuyMarketingFee = newMarketingTax;
        _buyGiveawayFee = newGiveawayTax;

        _totalTaxIfBuying = _BuyLpFee.add(_BuyMarketingFee).add(_buyGiveawayFee);
    }

    function setSelTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newGiveawayTax) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellMarketingFee = newMarketingTax;
        _sellGiveawayFee = newGiveawayTax;

        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellGiveawayFee);
    }
    
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newGiveawayShare) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _marketingShare = newMarketingShare;
        _GiveawayShare = newGiveawayShare;
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBGiveaway = amountReceived.mul(_GiveawayShare).div(totalBNBFee);
        uint256 amountBNBMarketing = amountReceived.sub(amountBNBLiquidity).sub(amountBNBGiveaway);

        emit GiveawayGetBnb(amountBNBGiveaway);
