...
contract OpenBiSea is Ownable  {
    ...
    function claimFreeTokens() public returns (bool, uint256, uint256) {
...
//
//        (, amountTotalForCustomer, ,)= checkTokensForClaim(msg.sender,priceMainToUSD.div(10 ** uint256(decimals)));


        uint256 delta = initialBalance.sub(tokensaleTotalSold);
        uint256 newPriceOBS = initialPrice.mul(initialBalance).div(delta);
        uint256 tokensToPay = IOpenBiSeaAuction(auction).revenueFor(msg.sender).mul(10 ** 18).div(newPriceOBS).div(100); // can't reward more than 1% of customer income
//        if (tokensToPay > incomeInOBSfromUser) tokensToPay = incomeInOBSfromUser / 100; // can't reward more than 1% of customer income
        ...
        return (result, newPriceOBS, tokensToPay);
    }
