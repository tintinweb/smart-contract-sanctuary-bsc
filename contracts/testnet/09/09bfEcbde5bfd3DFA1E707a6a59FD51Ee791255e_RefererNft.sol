...
contract RefererNft is Initializable, AccessControl,ReentrancyGuard,IEvents {
    ...
    uint[3] public referrerPercentage;
 ...
 */
    function rewardDistribution(address user, uint _reward, uint volume) public nonReentrant onlyRole(REWARD_ROLE) returns(uint amount, uint leftAmount) {   
        uint t = volume;
        // uint totalRewardPercentage = referrerPercentage[0] + referrerPercentage[1] + referrerPercentage[2];
        ...
        for(uint i = 1; i <= 3; i++){
            if(userReferrer != address(0)){
                ...
                console.log("RREEWWAARRDD", _reward);
                console.log(t * referrerPercentage[i-1]/1000,"_reward * referrerPercentage[i-1]/1000");
                users[userReferrer].totalReferralReward += (t * referrerPercentage[i-1])/1000;
                console.log(leftAmount ,t ,referrerPercentage[i-1],1000);   
                leftAmount -= (t * referrerPercentage[i-1])/1000;
                emit Reward_Earning(user,userReferrer,(t * referrerPercentage[i-1])/1000,i,volume, block.number, block.timestamp);
            }else{
                // break;
                emit Reward_Earning(user,Admin.TeamAddress(),(t * referrerPercentage[i-1])/1000,i,volume, block.number, block.timestamp);

            }
                userReferrer = users[userReferrer].referrer;
        }
        console.log(_reward,"reward",leftAmount,"leftAmount");
        return ((t * 9/100) - leftAmount, leftAmount);
    }
/**
