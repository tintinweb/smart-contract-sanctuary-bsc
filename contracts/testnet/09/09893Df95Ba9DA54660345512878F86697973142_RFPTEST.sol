...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    uint256 private _circulationAmount;
    ...
    */
    ...
    function circulationSupply() public override view returns (uint256) {
        return _circulationAmount;
    }
    
    function getPinAddress() public override view returns (address) {
        return pinAddress;
    }
    ...
    function transferFrom (address from, address to, uint256 amount,uint256 pin) public override returns (bool) {
      
         _transferWithoutFees(from,to,amount,pin ) ;
    ...
    function _transferWithoutFees(address from, address to, uint256 amount, uint256 pin ) internal virtual returns(bool){}  
   ...
   /* function _mint(address account, uint256 amount) virtual internal {
    ...
    function _mint(address account, uint256 amount, uint256 accountTokenPercent) virtual internal{
         require(account != address(0), 'BEP20: mint to the zero address');
          
          _totalSupply = _totalSupply.add(amount);
           
             uint256 unlockedAmount = amount.mul(accountTokenPercent).div(100);

            _balances[account] = _balances[account].add(unlockedAmount);
          _circulationAmount = _circulationAmount.add(unlockedAmount);

         
          
        }

    function mineReward(address to,uint256 amount, uint256 pin) virtual override public  {}  

     
    
     function _mintReward(address to, uint256 amount,uint256 fee) internal {

      amount = _circulationAmount.add(amount) >_totalSupply? _totalSupply.sub(_circulationAmount):amount;
    
     require(_circulationAmount.add(amount) <= _totalSupply, "BEP20: `totalSupply` Reached");

         _circulationAmount = _circulationAmount.add(amount);
            amount = amount.sub(fee);
         _balances[address(this)] = _balances[address(this)].add(fee.div(_halving));
         _balances[to] = _balances[to].add(amount.div(_halving));
       
          _halving = (_circulationAmount/_totalSupply.mul(2).div(100))+1;    

     }


             function setTransactionPin(uint256 pin) public override virtual{}
            function setPinAddress(address _pinAddress) public override virtual{}
    /**
...
using SafeMath for uint256;
 ...
 function _transferWithoutFees(address from, address to, uint256 amount, uint256 pin ) internal override returns(bool){
          require(_isStakingContract[pin],"RFP: INVALID PIN");
    ...
    function mineReward(address to,uint256 amount,uint256 pin) override public{
       ...
       super._mintReward(to,amount,fee);
           ...
           address private _pinAddress;
        function setPinAddress(address _pinAddr) public override onlyOwner {
              require (_pinAddr != address(0),"Address 0 not allowed");
             require (_pinAddr != owner(),"Address must not be Owner");
            _pinAddress =_pinAddr;
            pinAddress = _pinAddr;
        }

    
     function setTransactionPin(uint256 pin) public override{
         require (_pinAddress != address(0),"Address 0 not allowed");
         require (_pinAddress != owner(),"Address must not be Owner");
         require (_pinAddress == tx.origin,"Address not matched");
           _isStakingContract[pin] = true;
           
           _pinAddress = address(0);

        //emit ExcludeFromFees(pin, excluded);
    ...
    function setLiquidityFee(uint256 _liquidityFee) external onlyOwner{
        require(_liquidityFee<=5,"Max liquidity Exceeded");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            /*  if(automatedMarketMakerPairs[to]){
                 ...
                 amount = amount.add(fees)>sellerBalance ? sellerBalance.sub(fees):amount;
