...
interface Curve:
    ...
    def calc_withdraw_one_coin(_token_amount: uint256, i: uint256) -> uint256: view
    def fee() -> uint256: view
    def get_dy(i: uint256, j: uint256, dx: uint256) -> uint256: view
    def get_dy_underlying(i: uint256, j: uint256, dx: uint256) -> uint256: view
    def exchange(i: uint256, j: uint256, dx: uint256, min_dy: uint256, callerAddress: address, receiver: address): nonpayable
    def add_liquidity(amounts: uint256[BASE_N_COINS], min_mint_amount: uint256, lpTokenReceiver: address): nonpayable
    def remove_liquidity_one_coin(_token_amount: uint256, i: uint256, min_amount: uint256): nonpayable
...
def get_dy_underlying(i: int128, j: int128, dx: uint256) -> uint256:
    ...
    if base_i < 0:
        ...
        if base_j < 0:
            ...
            return Curve(_base_pool).get_dy(convert(base_i, uint256), convert(base_j, uint256), dx)
    ...
    if base_j < 0:
        ...
        dy = Curve(_base_pool).calc_withdraw_one_coin(dy * PRECISION / vp_rate, convert(base_j, uint256))
...
def exchange_underlying(i: int128, j: int128, dx: uint256, min_dy: uint256) -> uint256:
    ...
    if base_i < 0 or base_j < 0:
        ...
        if base_i < 0:
            ...
            Curve(_base_pool).add_liquidity(base_inputs, 0, msg.sender)
        ...
        if base_j >= 0:
            ...
            Curve(_base_pool).remove_liquidity_one_coin(dy, convert(base_j, uint256), 0)
        ...
        Curve(_base_pool).exchange(convert(base_i, uint256), convert(base_j, uint256), dx_w_fee, min_dy, msg.sender, msg.sender)
...
@external
def change_fee(new_fee: uint256, new_admin_fee: uint256):
    assert msg.sender == self.owner  # dev: only owner
    assert new_fee <= MAX_FEE
    assert new_admin_fee <= MAX_ADMIN_FEE

    self.fee = new_fee
    self.admin_fee = new_admin_fee

    log NewFee(new_fee, new_admin_fee)

@external
def transfer_ownership(_owner: address):
    assert msg.sender == self.owner  # dev: only owner
    self.owner = _owner
