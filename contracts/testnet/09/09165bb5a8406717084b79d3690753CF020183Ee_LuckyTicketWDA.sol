...
contract LuckyTicketWDA is ReentrancyGuard, VRFConsumerBase {
    ...
    uint24 public totalTicketSoldPerPeriod = 0; // maximum 50000 * 5 days = 250000
    ...
    mapping(address => TicketReward[]) public ownerWinningTicket5000WDA;
    // player address => ticket reward amount
    mapping(address => TicketReward[]) public ownerWinningTicket100000WDA;
    ...
    function withdrawWDAToDAOTreasury() external onlyOwner {
        ...
        IERC20(_token).transfer(DAOTreasuryWallet, amount);
    }
        ...
        returns (uint256)
    {
        ...
        for (
            ...
            if (
                ownerWinningTicket5000WDA[msg.sender][i].claimableDate <=
                block.timestamp
            ) {
                amount++;
            }
        ...
        returns (uint256)
    {
        ...
        for (
            ...
            if (
                ownerWinningTicket100000WDA[msg.sender][i].claimableDate <=
                block.timestamp
            ) {
                amount++;
            }
    ...
    function setTicket5000WDA() external onlyValidTarget isPeriod {
        ...
        if (totalTicketSoldPerPeriod >= minimumPlayer) {
            ...
            for (uint256 i = 0; i < _winnerCount; i++) {
                uint256 _newIndexWinningTicket5000WDA = _generateRandomNum(i);
                _payWinner(_newIndexWinningTicket5000WDA, 5000);
                _removeTicketWin(_newIndexWinningTicket5000WDA);
            }
        }
    }

    function setTicket100000WDA() external onlyValidTarget isPeriod {
        // if too few player => return cost of ticket
        if (totalTicketSoldPerPeriod >= (minimumPlayer - 5)) {
            // random 1 user reward 100000 WDA;
            uint256 indexWinningTicket = _generateRandomNum(100000);
            _payWinner(indexWinningTicket, 100000);
        }
    }

    function _removeTicketWin(uint256 index) internal {
        allTicket[index] = allTicket[totalTicketSoldPerPeriod - 1];
        allTicket.pop();
        ownerOfTicket[index] = ownerOfTicket[totalTicketSoldPerPeriod - 1];
        delete ownerOfTicket[totalTicketSoldPerPeriod - 1];
        totalTicketSoldPerPeriod--;
    }

    function _payWinner(uint256 index, uint256 _rewardAmount) internal {
        uint256 _reward = _rewardAmount * 10**18;
        Ticket memory _ownerTicket = allTicket[index];
        if (_ownerTicket.proposalPercentage != 0) {
            uint256 proposalToWDA = (_reward *
                _ownerTicket.proposalPercentage) / 100;
            if (_ownerTicket.proposalAction == 0) {
                _reward -= proposalToWDA;
            } else {
                _reward += proposalToWDA;
            }
        }
        if (_rewardAmount == 5000) {
            ownerWinningTicket5000WDA[ownerOfTicket[index]].push(
                TicketReward(_reward, _ownerTicket.purchaseDate + lockTime)
            );
        } else {
            ownerWinningTicket100000WDA[ownerOfTicket[index]].push(
                TicketReward(_reward, _ownerTicket.purchaseDate + lockTime)
            );
        }
        emit TicketWin(ownerOfTicket[index], _reward, index);
    }
    ...
    function resetAllTicket() external onlyValidTarget isPeriod {
        for (uint256 i = 0; i < totalTicketSoldPerPeriod; i++) {
            ownerTicketReward[ownerOfTicket[i]].push(
                allTicket[i].purchaseDate + lockTime
            );
            ownerTicketCount[ownerOfTicket[i]] = 0;
    ...
    function _generateRandomNum(uint256 _more) internal view returns (uint256) {
        ...
        return randomNumber % totalTicketSoldPerPeriod;
    }
    ...
    function buyTicket(uint256 _quantity) external payable nonReentrant {
        ...
        require(
            ownerTicketCount[msg.sender] + _quantity <= _ownerTicketMaxCount,
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            allTicket.push(Ticket(block.timestamp, p.percent, p.action));
            // store address owner of ticket
            ownerOfTicket[totalTicketSoldPerPeriod] = msg.sender;
            totalTicketSoldPerPeriod++;
        }
        ownerTicketCount[msg.sender] += _quantity;
        uint256 currentDate = (block.timestamp - lastTimeCheck) / 1 days;
    ...
    function _calculateReward(uint8 ticketType) internal returns (uint256) {
        ...
        if (ticketType == 0) {
            for (
                ...
                if (reward.claimableDate <= block.timestamp) {
    ...
    function claimReward() external {
        ...
        require(availableCount > 0, "No ticket is available");
