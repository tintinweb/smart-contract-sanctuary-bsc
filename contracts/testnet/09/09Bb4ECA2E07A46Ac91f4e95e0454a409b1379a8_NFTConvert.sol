...
contract ITRC721 {
    ...
    function typeNFTs(uint256 _typeNFT)
        public
        view
        returns (
            bool isBox,
    ...
    function creater() public view returns (address creator);
...
contract TRC21 is ITRC21 {
    ...
    // Token name
    string public _name = "NFTConvert";

    // Token symbol
    string public _symbol = "C1";
    using SafeMath for uint256;
...
contract NFTConvert is TRC21, ERC721Full("NFTConvert", "C1"), Ownable {
    event _register(address user, uint256[] _tokenIds);

    uint256 public tokenIdd = 0;
    TRC21 public feeToken = TRC21(0x0358d8b092450B85E69aDe64058b801F8e0C18F0);
    ITRC721 public nftPackage = ITRC721(0x2E0Afa30adf05873212c9A87981F21e58Ac99d39);
    ...
    function register(
        ...
        for (uint256 j = 0; j < _numItems; j++) {
            uint256 tokenId = create(user, _creator, _types[j]);
            tokenIds.push(tokenId);
        }
        emit _register(user, tokenIds);
    }

    function convert(uint256 _tokenId, uint256[] _cardTypes) public {
        uint256 _type;
        address _creator;
        bool isBox;
        uint256 price;
        uint256 numItems;
        uint256 length = _cardTypes.length;
        (_creator, _type) = nftPackage.metadata(_tokenId);
        (isBox, price, numItems, ) = nftPackage.typeNFTs(_type);
        require(!isBox);
        require(length == numItems);
        register(msg.sender, length, _creator, _cardTypes);
        nftPackage._burnItem(msg.sender, _tokenId);
    }

    function getNFTPackageMetadata(uint256 _tokenId) public view returns (address _creator, uint256 _type) {
        (_creator, _type) = nftPackage.metadata(_tokenId);
    }
    ...
    function setNFTPackage(ITRC721 _NFTPackage) public onlyManager {
        nftPackage = _NFTPackage;
    }
    ...
    function transfer(address to, uint256 value) public returns (bool) {
        require(feeToken.transferFrom(msg.sender, taker, minFee()));
