...
/// @author geric
/// @title SaverStaking
contract SaverStaking is Ownable {
     ...
     * @param _withdrawRate New withdrawal rate to be set
     */
    ...
    function stake(uint256 _amount) external {
        require(_amount >= 1000000000, "Minimum stake amount is 1000000000 wei");
        reinvest(_amount);
    ...
    function unstake(uint256 _amount) external {
        ...
        stakingToken.transfer(msg.sender, _amount + earnedRewards);
    }
    ...
    function computeRewardPerSecond(uint256 principal, uint256 rate) public pure returns (uint256) {
        // scaled computation of balance * rate / 1 year
        uint256 maxReward = (principal * rate * 1e18) / 100e18;
    ...
    /**
     * @notice Computes the percentage of the given amount
     * @param _percent The percentage value 1%, 2%, 3%, 15% (no decimals)
     * @param _amount The base value
     */
    function computePercentage(uint256 _percent, uint256 _amount) public pure returns (uint256) {
        uint256 percentOf = (_percent * _amount * 1e18) / 100e18;
        return percentOf;
    }
    ...
    function withdrawalLimit(address _address) public view returns (uint256) {
        uint256 withdrawableLimitPerDay = computePercentage(withdrawRate, stakers[_address].balance);
        uint256 timeStaked = block.timestamp - stakers[_address].stakedTime;
        return (withdrawableLimitPerDay * timeStaked) / 1 days;
    }
