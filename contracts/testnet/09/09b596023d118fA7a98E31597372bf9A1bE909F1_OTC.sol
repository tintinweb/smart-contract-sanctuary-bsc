...
contract OTC is Ownable {
    ...
    // order event
    event CreateOrder(address indexed token, uint256 indexed adId, uint256 indexed orderId, uint256 amount, uint256 price, uint256 fiat, uint256 timestamp, address nextAddress);
    ...
    function createOrder(uint8 _orderType, uint256 _adId, uint256 _amount) public returns (uint256) {
        ...
        emit CreateOrder(_order.tokenAddress, ad.id, _order.id, _order.amount, _order.price, _order.fiat, block.timestamp, _order.nextAddress);
