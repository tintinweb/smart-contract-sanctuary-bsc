...
contract ForsagePrime is Ownable {
  ...
  function pair(uint _referrerID) external payable {
        ...
        usdt.safeTransferFrom(msg.sender, address(this), levelPrice[1]);
        require(_referrerID > 0 && _referrerID <= userCurrentId, "Invalid REFID");
       ...
       function liquidity(uint8 _level) external payable {
        ...
        require(users[msg.sender].isExist && autoPoolUniqueUsers[_userId].isExist, "No User"); 
        ...
        require(_level > 0 && _level <= 5, "Invalid Pack");
        require(msg.value == levelPrice[_level], "Invalid Pack");
    ...
    function failSafe(address payable _toUser) onlyOwner external returns (bool) {
        require(_toUser != address(0), "Invalid Address");
        uint256 remaining = usdt.balanceOf(address(this));
        usdt.transfer(_toUser,remaining);
        return true;
    }
    ...
    function _workPlanPay(uint8 _flag, uint8 _level, address _userAddress) internal {
        ...
        address referer;
         uint _share = 0;
            ...
            if(_level == 1){
                 _share = (levelPrice[_level]).mul(25).div(100);
            }
            if(_level == 2){
                 _share = (levelPrice[_level]).mul(5).div(100);
            }
            if(_level == 3){
                 _share = (levelPrice[_level]).mul(4).div(100);
            }
            if(_level == 4){
                 _share = (levelPrice[_level]).mul(7).div(100);
            }
            if(_level == 5){
                 _share = (levelPrice[_level]).mul(9).div(100);
            }
        ...
        if(users[referer].levelStatus[_level] == true && users[referer].blocked ==false && users[referer].totalDirects >= _level) {
           
            if(_share>0){
                require(usdt.transfer(referer, _share),"Tx Failure");
            }
            ...
            if(users[sponsor].levelStatus[_level] == true) {   
             ...
             _share = (levelPrice[_level]).div(2);
    ...
    function _autoPoolPay(uint8 _flag, uint8 _level, uint _userId) internal {
        ...
        if(autoPoolUniqueUsers[autoPoolId[autoPoolUsers[_level][refId].userAddress]].levelStatus[_level] = true|| _flag == 1) {
            ...
            require(usdt.transfer(refererAddress, _share),"Tx Failure");
