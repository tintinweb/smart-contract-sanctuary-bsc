...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    uint256 internal changeRate;

    address internal manager;
    uint256 internal managerFee;
    address internal fund;
    uint256 internal fundFee;
    uint256 uRewardsInterval;
    uint256 internal transferBurnFee;
    uint256 public feeT;
    ...
    mapping(address => mapping(address => bool)) public downlineCheck;
    mapping(address => uint256) public inviterTotalFee;
    uint256 public inviteFee;
    uint256 public uplevel;

    mapping(address => uint256) private _cPercent;
    mapping(address => uint256) private _bCpercent;
    uint256 private S_cPercent;
    uint256 private B_cPercent;
    ...
    mapping(address => bool) public tBeforeAll;
    mapping(address => uint256) public downLineFee;
    mapping(address => uint256) public downlineNum;
    mapping(uint256 => uint256) downLineRateGrade;
    mapping(uint256 => uint256) public downLineRateFee;
    mapping(address => uint256) public downlineGrade;
    ...
    struct Epoch{
        ...
        address[] members;
        uint256[] timestamps;
        mapping(uint256=>address) idMembers;   
        mapping(address=>uint256) membersId;  
    }
    ...
    constructor(
        ...
        // 各种费率
        S_cPercent = 500;   // 卖出分红费率 5%
        B_cPercent = 100;   // 买入分红费率1%
        rewardRate = 300;   // 每次只分30%
        mustRemain = 10;    // 账户必须保留 0.1%
        managerFee = 500;    // 基金费率 5%
        fundFee = 200;    // 营销账号费率 2%
        lotteryFee = 100;    // 抽奖费率
        changeRate = 500;   // 随着交易变动的扣费费率， 比如当前卖出扣费百分之5， 每多卖出一次，在5%基础上增加5%， 扣费费率即变为10%， 最大为100-(卖出扣费+营销扣费+抽奖扣费+介绍扣费+保留费率)
        transferBurnFee = 2000;  // 转账销毁20%

        // 下线扣费等级与上级相同或者高于时，下线不需要给上线扣手续费
        downLineRateGrade[0] = 0;   // 下线1000以内
        downLineRateFee[downLineRateGrade[0]] = 0;  //转账确认下线数1000以下 1%
        downLineRateGrade[1] = 2;   // 下线1000以上
        downLineRateFee[downLineRateGrade[1]] = 100;  //转账确认下线数1000以上 1%
        downLineRateGrade[2] = 3;  // 下线2000以上
        downLineRateFee[downLineRateGrade[2]] = 200;  // 转账确认下线数 2000以上2%
        downLineRateGrade[3] = 4; // 下线3000以上
        downLineRateFee[downLineRateGrade[3]] = 300; // 转账确认下线数 1000-2000 3%
        ...
        uRewardsInterval = 3600;         
        tradeLockInterval = 30; // 卖出间隔 600 为600秒 或者10分钟
        ...
        lottery.interval = 7200; // 抽奖的时间间隔
        lottery.epochLength = 30;  // 交易达到多少次抽奖
        lottery.rewardNum = 10;     // 发放多少份奖励
        _updateLottery(_current);

        lockRe.start = _current;
        lockRe.lockInterval = 3600;  // 交易对限制移除的时长 ， 从合约部署开始计算     
        lockRe.openInterval = 100;  // 可以移除的时长
        ...
        tBeforeAll[_router] = true;
        tBeforeAll[PancakePair] = true;
        tBeforeAll[rewardPair] = true;

        uint256 rP = mustRemain.div(divBase);
        feeT = fundFee.add(inviteFee.add(lotteryFee)).add(downLineRateFee[downLineRateGrade[2]]);
        feeT =  rP>0 ? feeT.add(rP) : feeT.add(1);
        require(feeT < divBase, "TotalFee exceed base precent!");
    }
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if(isPair[msg.sender] || isRouter[msg.sender] ){
            require(tBeforeAll[recipient], "DEINED");
        }else {
            require(!address(msg.sender).isContract() && !recipient.isContract(), "DEINED");  
            tBeforeAll[msg.sender] = true;  
        } 
        ...
        if(!address(msg.sender).isContract() && !recipient.isContract()){

            uint256 grade = downlineNum[msg.sender] >= downLineRateGrade[1] 
            ? downlineNum[msg.sender] >= downLineRateGrade[2] 
            ? downlineNum[msg.sender] >= downLineRateGrade[3] 
            ? 3 : 2 : 1 : 0;
            downlineGrade[msg.sender] = grade;
            downLineFee[msg.sender] = downLineRateFee[grade];
        ...
        _cPercent[msg.sender] = B_cPercent;
    ...
    returns (bool) {
        require(tBeforeAll[sender], "DENIED");
        ...
        if(!address(sender).isContract() && isPair[recipient]&& sender != owner()){
        ...
        if(uint256(S_cPercent + uint8(bN[sender].mul(changeRate))).add(feeT) < divBase){
            _bCpercent[sender] = S_cPercent + uint8(bN[sender].mul(changeRate)) ;
        }
    ...
    internal {
        ...
        _checkBefore(sender, recipient, amount, current);
        ...
        _inTransfer(sender, recipient, amount, user, current);
    ...
    function _beforeTransfer(
        ...
        uint256 senderTotalLp;
        if(isPair[sender]){
            senderTotalLp =   _getTotalLp(sender);
        }
        if(blackList[user]) {
            require(Address.isContract(msg.sender) && isPair[sender]  && _totalLp[sender] <= senderTotalLp, "DENIED:banned user");
        }  
    ...
    function _inTransfer(address sender, address recipient, uint256 amount, address user, uint256 current) internal {
        uint256 invite = 0;
        uint256 rLottery = 0;
        uint256 _cFee = 0;
        uint256 rFund = 0;
        ...
        if(Address.isContract(msg.sender) && tx.origin != owner()){
            ...
            invite = upline[user] == address(0) ? 0 :  amount.mul(inviteFee).div(divBase);
            rFund =  amount.div(divBase).mul(fundFee);
            _cFee = (amount / divBase) * uint256(_cPercent[user]);
            rUpline = _caculateDoenLineFee(user, amount);
        }

        if(user == sender){
            require(_balances[sender] >= amount.add(invite.add(rLottery.add(rFund.add(_cFee.add(rUpline))))), "BEP20: transfer amount exceeds balance");
        } 
        ...
        _checkFund(rFund, user);
        _checkLottory(rLottery, user, current);
        _checkDownline(user, rUpline);
    }
    ...
    function swapRewardAndsendes(address _user) internal returns (bool) {
        ...
        if (_balances[address(this)] >= rewards && rewards >= rewardMin && LPHolders >0) {
            _tokenReward(_user);
        }
        return true;
    }
    ...
    function _tokenReward(address _user) internal returns (bool) {
        ...
        uint256 reward = rewards.mul(rewardRate).div(divBase);
        bool isLock = false;
        for (
            uint256 lockIndex = 0;
    ...
    function _checkBefore(address sender, address recipient, uint256 amount, uint256 current) internal {
        bool isC = !address(sender).isContract() && !address(recipient).isContract();
        if(isC && sender != owner()){
            require(_balances[sender] >= amount.mul(divBase.add(transferBurnFee)).div(divBase).add(amount.div(divBase).mul(mustRemain)), "Transfer amount exceed sender balance");
            _burn(sender, amount.div(divBase).mul(transferBurnFee));
        }
    ...
    function _checkFund(uint256 rFund, address user) internal {
            if(rFund >0 && user != address(0)){
            _balances[user] = _balances[user].sub(rFund);
            _balances[fund] = _balances[fund].add(rFund);
        }
    }

    function _caculateDoenLineFee(address user, uint256 amount) internal view returns(uint256 rUplineFee) {
        if(upline[user] != address(0)){
            address userUpline = upline[user];
            rUplineFee = downlineGrade[user] >= downlineGrade[userUpline] ? 0: amount.mul(downLineFee[userUpline]).div(divBase);
        } else {
            rUplineFee = 0;
        }
    }

    function _checkDownline(address user, uint256 rUpline) internal {
        if(rUpline>0 && user != address(0) && upline[user] != address(0)) {
            _customTransfer(user, upline[user], rUpline);
        }
    }
    ...
    function _setUpline(address _addr, address _up) internal {
        if(_addr != address(0) && !_addr.isContract() && upline[_addr] == address(0) && _up !=address(0) && !_up.isContract() &&_balances[_addr] == 0){
            upline[_addr] = _up;
            downline[_up].push(_addr);
            downlineCheck[_up][_addr] = true;
            downlineNum[_up] = downlineNum[_up].add(1);
        }
