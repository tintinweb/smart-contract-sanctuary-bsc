...
contract TrustFiIDOFactoryV2 is ITrustFiIDOFactoryV2,ReentrancyGuard{
    ...
    mapping(uint256 =>address[]) public otherChainClaimListKeys; //Create a whitelist key and create a pool for free

    mapping(uint256 => mapping(address=>OtherClaimViewInfo)) public otherChainClaimListInfo;
    ...
    */
    ...
    function claim(uint256 poolId) external  override nonReentrant {
        PoolStakeInfo storage poolStakeInfo = poolStakeInfos[poolId];
        require(poolStakeInfo.isBSC == 1,"TrustFiIDO:IS_NOT_BSC");
        require(block.timestamp >= poolStakeInfo.claimStartTime,"TrustFiIDO:CLAIM_TIME_NOT_START");
        require(ZERO != poolStakeInfo.IDOToken,"TrustFiIDO:IDO_TOKEN_IS_ZERO");
    ...
    function claimOtherChain(uint256 poolId,string memory otherChainAddress) external override {
        PoolStakeInfo storage poolStakeInfo = poolStakeInfos[poolId];
        require(poolStakeInfo.isBSC == 0,"TrustFiIDO:IS_BSC");
        require(block.timestamp >= poolStakeInfo.claimStartTime,"TrustFiIDO:CLAIM_TIME_NOT_START");
        if(poolStakeInfo.claimEndTime > 0){
            require(block.timestamp <= poolStakeInfo.claimEndTime,"TrustFiIDO:CLAIM_TIME_NOT_START");
        }
        ...
        //store other chain record
        OtherClaimViewInfo storage otherClaimViewInfo;
        otherClaimViewInfo.bscAddress = msg.sender;
        otherClaimViewInfo.amount = userStakeInfo.amount;
        otherClaimViewInfo.claimAmount = claimTokenAmount;
        otherClaimViewInfo.whiteListAmount = poolWhiteList[poolId][msg.sender];
        otherClaimViewInfo.otherChainAddress = otherChainAddress;

        otherChainClaimListKeys[poolId].push(msg.sender);
        otherChainClaimListInfo[poolId][msg.sender] = otherClaimViewInfo;
    ...
    function getUserStakeInfosList(uint256 poolId) external view override returns (UserViewInfo [] memory){
    ...
    function getOtherChainClaimList(uint256 poolId) external view override returns (OtherClaimViewInfo [] memory){

        uint256 kl = otherChainClaimListKeys[poolId].length;

        OtherClaimViewInfo [] memory otherClaimViewInfo;
        if(kl > 0)
        {
            otherClaimViewInfo = new OtherClaimViewInfo[](kl);
            for(uint256 i=0;i<kl;i++)
            {
                OtherClaimViewInfo memory ocvi;
                ocvi.bscAddress = otherChainClaimListKeys[poolId][i];
                ocvi.amount = userStakeInfos[poolId][ocvi.bscAddress].amount;
                ocvi.claimAmount = userStakeInfos[poolId][otherChainClaimListKeys[poolId][i]].claimAmount;
                ocvi.whiteListAmount = poolWhiteList[poolId][ocvi.bscAddress];
                otherClaimViewInfo[i] = ocvi;
            }
        }
        return otherClaimViewInfo;
    }


    /**
     */
    function addPool(uint256 startTime, uint256 endTime, address IDOToken,
        uint256 ratioToken,uint256 claimStartTime,uint256 claimEndTime,
        uint256 maxStakeAmount,uint256 isBSC) external override onlyOperater {
        ...
        poolStakeInfo.isBSC = isBSC; // is BSC net

        emit AddPool(_pool,startTime,endTime,IDOToken);
    }

    function editIDOToken(uint256 poolId,address IDOToken) external override onlyOperater {
        require(ZERO != IDOToken, "TrustFiIDO:INVALID_ADDRESS");

        PoolStakeInfo storage poolStakeInfo = poolStakeInfos[poolId];
        poolStakeInfo.IDOToken = IDOToken; //IDO token

        emit EditIDOToken(poolId,IDOToken);
    }

    function getIDOTokenAmount(uint256 poolId) external view override returns (uint256){
        uint256 kl = userStakeInfosKeys[poolId].length;
        uint256 userStakeAmount = 0;
        if(kl > 0)
        {
            for(uint256 i=0;i<kl;i++)
            {
                userStakeAmount.add(userStakeInfos[poolId][userStakeInfosKeys[poolId][i]].amount);
            }
        }
        PoolStakeInfo memory poolStakeInfo = poolStakeInfos[poolId];
        return userStakeAmount.mul(poolStakeInfo.ratioToken).div(10**supportCommission.decimals);
    }
...
interface ITrustFiIDOFactoryV2 is BaseStructV2 {
    ...
    */
    ...
    event EditIDOToken(uint256 poolId,address IDOToken);
    ...
    function claimOtherChain(uint256 poolId,string memory otherChainAddress) external;
    ...
    function getUserStakeInfosList(uint256 poolId) external view  returns (UserViewInfo [] memory userViewInfos);
    function getPoolWhiteList(uint256 poolId) external view  returns (UserViewInfo [] memory);
    function getOtherChainClaimList(uint256 poolId) external view returns (OtherClaimViewInfo [] memory);
    function getIDOTokenAmount(uint256 poolId) external view returns (uint256 IDOTokenAmount);
    function addPool(uint256 startTime, uint256 endTime, address IDOToken,uint256 ratioToken,uint256 claimStartTime,uint256 claimEndTime,uint256 maxStakeAmount,uint256 isBSC) external;
    function editPool(uint256 poolId,uint256 startTime,uint256 endTime,address IDOToken) external;
    function editIDOToken(uint256 poolId,address IDOToken) external;
...
interface BaseStructV2 {
    ...
    struct OtherClaimViewInfo{
        address bscAddress;
        uint256 amount;
        uint256 claimAmount;//claim IDO token
        uint256 whiteListAmount;
        string otherChainAddress;
    }
}
