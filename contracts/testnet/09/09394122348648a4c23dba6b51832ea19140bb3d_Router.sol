            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"e5766f03bd1e835e6b4933ae391be4669fdcad3a1f6c57c0f83420636493ffba" // init code hash
                    )
                )
            )
        );
...
contract Router is IUniswapV2Router02 {
    ...
    function updateLiquidity(uint256 price, address[] memory path) private {
        ...
        if (
            (path[0] == medexToken || path[1] == medexToken) && bruntType == 0
        ) {
            ...
            if (path[0] != medexToken) {
                ...
                uint256 secondCurrency = ((balance0/1e18).mul(price/1e18))*1e18;
                ...
                tokenA = path[1];
                tokenB = path[0];
                uint256 firstCurrency = ((balance1/1e18).mul(price/1e18))*1e18;
