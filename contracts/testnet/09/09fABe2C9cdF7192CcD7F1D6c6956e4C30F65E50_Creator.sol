...
contract StakingVaultV6_0_0 is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function sendDualTokenRewards(
        ...
        require(_amount <= dualTokenSendOutAmount, '>enabled');

        distributeRewards();
        if (balanceOfDualToken() >= _amount) {
            //safely send out tokens
            if (_amount <= totalDualToken) {
                totalDualToken -= _amount; //tracker for new dual token enter the vault
            } else {
                totalDualToken = 0;
            }
            ...
            //send tokens to user
            if (address(dualToken) != address(0x0)) {
                dualToken.safeTransfer(_user, _amount);
            } else {
        ...
        if (_amount <= apyDualTokenTracker) {
            apyDualTokenTracker -= _amount; //apy is equal to old balance of dual token. So this finds and new tokens that are from fee OR any other source.
        } else {
        ...
        emit WithdrawDualToken(_user, _amount);
    }
