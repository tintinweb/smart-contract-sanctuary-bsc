...
contract JigsawDistributor is ReentrancyGuard, VRFConsumerBaseV2, Ownable {
    ...
    uint256 public currentTriadID;
    uint256 public triadRate = 2500;
    uint256[3] public triadHolderRates = [6000, 2500, 1000];
    mapping(uint256 => TriadResult) public triadResults;
    ...
    event SetTriadRate(uint256 rate);
    event SetTriadHolderRates(uint256 rateA, uint256 rateB, uint256 rateC);
     ...
     * @dev This function must be called by the owner of the contract.
     */
    function callTriad() external onlyOwner {
        require(initialized, "Contract not initialized");
        ...
        uint256 amount = address(this).balance;
        amount = amount * triadRate / 10000;
        uint256 amountA = amount * triadHolderRates[0] / 10000;
        uint256 amountB = amount * triadHolderRates[1] / 10000;
        uint256 amountC = amount * triadHolderRates[2] / 10000;

        payable(winnerA).transfer(amountA);
        payable(winnerB).transfer(amountB);
        payable(winnerC).transfer(amountC);

        currentTriadID = currentTriadID + 1;
        
        TriadResult storage triadResult = triadResults[currentTriadID];
        ...
        emit HolderDistributed(currentTriadID, winnerA, amountA, winnerB, amountB, winnerC, amountC);
    }
     ...
     * @dev This function must be called by the owner of the contract.
     */
    function setTriadRate(uint256 _rate) external onlyOwner {
        require(_rate > 0, "Rate must be greater than 0");
        triadRate = _rate;
        emit SetTriadRate(_rate);
    }
     ...
     * @dev This function must be called by the owner of the contract.
     */
    function setTriadHolderRates(uint256 _rateA, uint256 _rateB, uint256 _rateC) external onlyOwner {
        require(_rateA > 0, "Rate A must be greater than 0");
        require(_rateB > 0, "Rate B must be greater than 0");
        require(_rateC > 0, "Rate C must be greater than 0");
        require(_rateA + _rateB + _rateC < 10000, "Total rate must be less than 10000");

        triadHolderRates = [_rateA, _rateB, _rateC];
        emit SetTriadHolderRates(_rateA, _rateB, _rateC);
    }
