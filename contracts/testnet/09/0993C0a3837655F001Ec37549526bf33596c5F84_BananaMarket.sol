...
contract BananaMarket is  Ownable, ReentrancyGuard{
    ...
    event SaleCancelled(uint256 indexed itemId, address  token, uint256 amount, uint256  price, address indexed seller, bool indexed cancelled );
    event SaleComplete(uint256 indexed itemId, address token, uint256 amount, uint256 price, address indexed seller, bool indexed sold );
    //// Trades ////
    ...
    event TradeCancelled(uint256  itemId, address  token, uint256 amount, address swappedFor, uint256 indexed swappedForAmount, address indexed seller, bool indexed cancelled );
    event TradeComplete(uint256  itemId, address token, uint256 amount, address swappedFor, uint256 indexed swappedForAmount, address indexed seller, bool indexed sold );
    ...
    function purchaseItem(uint256 _itemId) external payable nonReentrant{
        ...
        emit SaleComplete(item.itemId, address(item.token), item.amount, item.totalPrice, address(item.seller), true);
    }
    ...
    function cancelOffer(uint256 _itemId) public  nonReentrant{
        ...
        emit SaleCancelled(item.itemId, address(item.token), item.amount, item.totalPrice, item.seller, true);
    }
    ...
    function acceptTrade(uint256 _itemId) external  nonReentrant{
        ...
        // Emit Purchased Event
        emit TradeComplete(trade.itemId, address(trade.token), trade.amount, address(trade.swappedFor), trade.swappedForAmount,  address(trade.seller), true);
    }
    ...
    function cancelTrade(uint256 _itemId) public  nonReentrant{
        ...
        // Emit Cancelled Event
        emit TradeCancelled(_itemId, address(trade.token), trade.amount,address(trade.swappedFor), trade.swappedForAmount, trade.seller, true);
    }
