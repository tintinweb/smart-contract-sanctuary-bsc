...
contract VeMoon is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    // Multisig Protocol Wallets
    address payable public marketingAddress = payable(0x3D500E07b89c990572798e9274fe5C7e48C4BC7d); 
    address payable public charityAddress = payable(0x2e6a0332870E53C857F246A8838919215C678c22); 
    address payable public dev1Address = payable(0x5e4911138134259845599457dD5893BB91d64F01); 
    address payable public dev2Address = payable(0x3e806b782795bb3AfbC0C0D7E52B0d827F7F6EC7); 
    address payable public busdAddress = payable(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    string private constant _name = "VeMoon";
    ...
    uint256 public _liquidityFee = 12; // liquidity + all team fees
    ...
    uint256 public _buyDev1Fee = 500;
    uint256 public _buyDev2Fee = 150;
    uint256 public _buyDevFee = 2;

    uint256 public _sellCharityFee = 5;
    uint256 public _sellMarketingFee = 5;
    uint256 public _sellDev1Fee = 750;
    uint256 public _sellDev2Fee = 225;
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    uint256 liquidityToken = contractTokenBalance.mul(2).div(100);
                    uint256 busdLiquidityToken = contractTokenBalance.sub(liquidityToken);

                    // Remove Hate Swap and Liquidity by breaking Token in proportion
                    addLiquidityToToken(liquidityToken);
                    
                    // Token Swapping in BNB
                    swapTokens(busdLiquidityToken);    
                    // Fetch balance of the token swapped for BNB
                    uint256 bnbBal = address(this).balance;
                    // Swap BNB for BUSD and send to Marketing and Staking Address
                    SwapAndLiquifyBNBtoBusd(bnbBal);
                }  
    ...
    function swapETHForTokensToBusd(uint256 amount) private lockTheSwap{
        // Generate the uniswap pair path of token -> WETH
        address[] memory path = new address[](2);
    ...
    function _takeVault(uint256 tVault) private {
        ...
        _rOwned[charityAddress] = _rOwned[charityAddress].add(rVault);
        if(_isExcluded[charityAddress])
            _tOwned[charityAddress] = _tOwned[charityAddress].add(tVault);
    }
    ...
    function setBuySellDevFee(uint256 _newBuyFee, uint256 _newSellFee) external onlyOwner{
        _buyDevFee = _newBuyFee;
        _sellDevFee = _newSellFee;
    }
    ...
    function setCharityAndMarketingFee(uint256 _sellRewardsPercent,uint256 _sellMarketingPercent,uint256 _buyRewardsPercent,uint256 _buyMarketingPercent ) external onlyOwner {
        _sellCharityFee = _sellRewardsPercent;
        _sellMarketingFee = _sellMarketingPercent;
        _buyCharityFee = _buyRewardsPercent;
        _buyMarketingFee = _buyMarketingPercent;
    }

    function setDevFee(uint256 _buydev1, uint256 _buydev2,uint256 _selldev1, uint256 _selldev2) external onlyOwner {
        _buyDev1Fee = _buydev1;
        _buyDev2Fee = _buydev2;
        _sellDev1Fee = _selldev1;
        _sellDev2Fee = _selldev2;
    }
    ...
    function manualSwapAndLiquifyBNB(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
    ...
    function SwapAndLiquifyBNBtoBusd(uint256 bnbLiquifyAmount) private lockTheSwap {
        ...
        // swap ETH for Tokens
        swapETHForTokensToBusd(bnbLiquifyAmount); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered

        // how much Tokens did we just swap into?
        uint256 newTokenBalance = IERC20(busdAddress).balanceOf(address(this));

        uint256 marketingFunds = newTokenBalance.mul(_buyMarketingFee).div(_liquidityFee);
        uint256 charityFunds = newTokenBalance.mul(_buyCharityFee).div(_liquidityFee);
        uint256 devFunds = newTokenBalance.sub(marketingFunds).sub(charityFunds);


        require(IERC20(busdAddress).transfer(marketingAddress,marketingFunds));
        require(IERC20(busdAddress).transfer(charityAddress,charityFunds));
        require(IERC20(busdAddress).transfer(dev1Address,devFunds/2));
        require(IERC20(busdAddress).transfer(dev2Address,devFunds/2));
