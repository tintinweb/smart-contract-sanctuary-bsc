...
contract ICO is ERC20, ERC20Pausable, Ownable, ReentrancyGuard {
    ...
    */
    function buy() payable nonReentrant public returns (bool success) {
        require(msg.value == 1 ether, 'Need to send 1 BNB');
        require(msg.sender.balance >= msg.value, "ICO: function buy invalid input");
        require(!isContributor(_msgSender()), "ICO: already contributed");
        contributors[_msgSender()] = true;
        return true;
    }

    function withdraw() whenNotPaused onlyEOA external returns (bool success) {
        require(isContributor(_msgSender()), "ICO: never contributed or already withdrew");
        uint256 amount = 1000 ether;
        _transfer(owner(), _msgSender(), amount);
        contributors[_msgSender()] = false;
        return true;
    }
    ...
    */
    function pullFunds(uint256 amount) public onlyOwner returns (bool success) {
      require(amount <= address(this).balance, "ICO: function pullFunds invalid input");
    ...
    function isContributor(address candidate_) public view returns (bool) {
        return (contributors[candidate_] == true);
    }
