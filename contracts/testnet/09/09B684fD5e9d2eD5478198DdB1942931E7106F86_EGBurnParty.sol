...
contract EGBurnParty is Ownable, KeeperCompatibleInterface {
    ...
    function checkUpkeep(bytes calldata checkData) external override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        for(i = 0; i < burnPartyCounter.current(); i++ ){
            if(burnPartyById[i].startDate > (block.timestamp - 6000) && burnPartyById[i].startDate < (block.timestamp + 6000) && burnPartyById[i].started == false){
                count++;
            }
        }
        ...
        return (list.length > 0, abi.encodePacked(list));
    }
