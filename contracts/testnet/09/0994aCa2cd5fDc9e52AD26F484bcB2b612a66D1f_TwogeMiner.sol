...
abstract contract MinerBasic {
    ...
    event Sell(address indexed adr, uint256 divs, uint256 amount, uint256 penalty);
    event RehireMachines(address _investor, uint256 _newMachines, uint256 _hiredMachines, uint256 _nInvestors, uint256 _referralDivs, uint256 _marketDivs, uint256 _DivsUsed);
...
contract InvestorsManager {
    ...
    function getTotalReferralsDivs() public view returns(uint256) { return totalReferralsDivs; }
    ...
    function setInvestorClaimedDivs(address addr, uint256 claimedDivs) internal { investors[addr].claimedDivs = claimedDivs; }
...
contract TwogeMiner is Ownable, MinerBasic, InvestorsManager, EmergencyWithdrawal {
    ...
    TwogeMinerConfigInterface reInterface;
    ...
    constructor(address _marketingAdd, address _recIface) {
        ...
        reInterface = TwogeMinerConfigInterface(_recIface);
    }
    ...
    function setExternalConfigAddress(address _recIface) public onlyOwner { reInterface = TwogeMinerConfigInterface(address(_recIface)); }
    ...
    function _rehireMachines(address _sender, address ref, bool isClaim) private {
        ...
        //send referral divs
        setInvestorDivsByReferral(getReferralData(_sender).investorAddress, getReferralData(_sender).referralDivs.add(SafeMath.div(divsUsed, 8)));
        setInvestorClaimedDivs(getReferralData(_sender).investorAddress, SafeMath.add(getReferralData(_sender).claimedDivs, SafeMath.div(divsUsed, 8))); 
        ...
        emit RehireMachines(_sender, newMachines, getInvestorData(_sender).hiredMachines, getNumberInvestors(), getReferralData(_sender).claimedDivs, marketDivs, divsUsed);
    }
    ...
    function _sellDivs(address _sender) private {
        ...
        emit Sell(_sender, divsValue, SafeMath.sub(divsValue,sellTax), penalty);
    }

    function _hireMachines(address _ref, address _sender, uint256 _amount) private {        
        uint256 divsBought = calculateHireMachines(_amount, SafeMath.sub(address(this).balance, _amount));
        
        if(reInterface.needUpdateEventBoostTimestamps()){
            reInterface.updateEventsBoostTimestamps();
        }
        ...
        payBuySellTax(fee);
        setInvestorClaimedDivs(_sender, SafeMath.add(getInvestorData(_sender).claimedDivs, divsBought));
    ...
    function capToMaxSell(uint256 divsValue, uint256 divs) public view returns(uint256, uint256){
    ...
    function divsRewardsIncludingTaxes(address adr) public view returns(uint256) {
        uint256 hasDivs = getMyDivs(adr);
    ...
    function calculateDivSellIncludingTaxes(uint256 divs, address adr) public view returns(uint256, uint256) {
        if(divs == 0){
            return (0,0);
        }
        uint256 totalTrade = calculateTrade(divs, marketDivs, address(this).balance);
    ...
    function calculateHireMachines(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return reInterface.applyROIEventBoost(calculateHireMachinesNoEvent(eth, contractBalance));
    }
    ...
    function getMyDivs(address adr) public view returns(uint256) {
        return SafeMath.add(getInvestorData(adr).claimedDivs, getDivsSinceLastHire(adr));
    }
    
    function getDivsSinceLastHire(address adr) public view returns(uint256) {        
        uint256 secondsPassed=min(DIVS_TO_HATCH_1MACHINE, SafeMath.sub(block.timestamp, getInvestorData(adr).lastHire));
