...
contract MYLPowerball {
    ...
    */
    ...
    modifier enoughamount(
        ...
        require(_amount >= _ticketCount.mul(_ticketPrice), "Insufficient bnb value");
        _;
    }
    ...
    mapping (uint256 => address[]) allAddressList;                                      //list of all address participating in a saleId
    mapping (uint256 => address[]) winner;                                              //winner address for a saleId
    mapping (uint256 => uint256) winningPowerBallNumber;                                //winning powerball number by saleId
    mapping (uint256 => mapping (address => uint256[]))  ticketNumberByAddress;          //user ticket number for a saleId
    mapping (uint256 => mapping (uint256 => address[]))  addressesByTicketNumber;        //list of addresses for ticketId
    mapping (uint256 => mapping (address => uint256))  totalSaleAmountByAddAndSaleID;    //list of addresses for ticketId
    mapping (uint256 => uint256)  totalSaleAmount;                                       //total collection for a saleId
    mapping (uint256 => uint256[])  winningAmount;                                       //winning price for a saleId
    mapping (uint256 => uint256)  saleStartTimeStamp;                                    //start timestamp for a saleId
    mapping (uint256 => uint256)  saleEndTimeStamp;                                      //end timestamp for a  saleId
    mapping (uint256 => uint256)  saleRunningStatus;                                     //sale running status for a saleId
    mapping (uint256 => uint256[])  winningNumber;                                       //winning lottery number for a saleId
    mapping (uint256 => uint256)  saleParticipants;                                      //total number sales per sale session
    
    uint256  elapsedTime;                                                                //variable to set time for powerball winning 
    uint256  saleIdNow = 1;                                                              //saleIdNow for sale now 
    address[]  AllParticipantAddresses;                                                  //list of all participants participated in the sale
    uint256  totalSaleAmountForAllSales;                                                 //total amount including all sales
    uint256  totalDonation;                                                              //total donated amount
    ...
    */
    ...
    function purchaseTicket(uint256 _ticketNumbers, uint256 ticketCount) 
      external
      enoughamount(msg.value,ticketCount)
      payable 
      returns(bool)
    {
        if(_ticketNumbers == 0){
        ...
        uint256 saleId;
        if((saleStartTimeStamp[saleIdNow].add(elapsedTime)) > block.timestamp){
            saleId = saleIdNow;
        } else {
            saleId = saleIdNow.add(1);
        }
        
        ticketNumberAllocation(saleId,msg.value,_ticketNumbers);
        
        transferToken(ticketCount);
       
        return true;
    }

    function ticketNumberAllocation(uint256 _saleId,uint256 _amount,uint256 _ticketNumbers)
       internal
    {
        AllParticipantAddresses.push(msg.sender);
        totalSaleAmount[_saleId] = totalSaleAmount[_saleId].add(_amount);
        totalSaleAmountForAllSales = totalSaleAmountForAllSales.add(_amount);
        totalSaleAmountByAddAndSaleID[_saleId][msg.sender] = totalSaleAmountByAddAndSaleID[_saleId][msg.sender].add(_amount);
       
        ticketNumberByAddress[_saleId][msg.sender].push(_ticketNumbers);
            
        allAddressList[_saleId].push(msg.sender);
        saleParticipants[_saleId]  = saleParticipants[_saleId].add(1);
    }
    ...
    */
    function declareWinner(uint256[] calldata _winningSequence, uint256 _powerballNumber, address payable[] calldata _winnerAddressArray, uint256[] calldata _winnerPositions, uint256[]  calldata _winnerAmountInWei) external payable onlyOwner returns(bool){
        ...
        saleStartTimeStamp[(saleIdNow.add(1))] = block.timestamp;
        saleIdNow = saleIdNow.add(1);
        return true;
    }
