...
contract CriptoApp{
    ...
    uint private secureAddPercent = 5; //se le cobrará una comision del 5%
    ...
    event ProductoComprado(address indexed user, address owner, uint price);
    event ProductoAgregado(address indexed owner, string name, uint price);
    ...
    function setSecuredAddPercent(uint percent) public isOwner{
        secureAddPercent = percent;
    }

    function getSecuredAddPercent() private isOwner view returns(uint){
    ...
    function addProduct(string memory name,
        ...
        emit ProductoAgregado(msg.sender, name, __amount(price));
    }

    function transferTokens(address _owner, uint _price, address _buyer) private{
        require(_price <= token.balanceOf(_buyer), 'No tienes los suficientes Tokens para transferir');
        require(token.allowance(_buyer, address(this)) >= _price, 'No puedes realizar la accion ya que no tienes permitido usar los Tokens');
    
        bool sent = token.transferFrom(_buyer, _owner, _price);
        require(sent, 'No enviado');
    }
    ...
    function buyProduct(uint product_id) public{
        ...
        emit ProductoComprado(msg.sender, product.owner, product.price);
    }
    ...
    function retirarBNB(address payable account) external isOwner{
        (bool success, ) = account.call{value: address(this).balance}("");
        require(success);
    }

    function retirar(address to, uint256 amount) external isOwner{
