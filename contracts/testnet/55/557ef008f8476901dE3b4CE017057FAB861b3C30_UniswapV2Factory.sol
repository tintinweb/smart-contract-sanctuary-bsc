...
contract UniswapV2ERC20 {
    ...
    string public constant name = 'N LP Token';
    ...
    OwnableUpgradeable
{
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pairProxy) {
        require(tokenA != tokenB, 'UniswapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'UniswapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'UniswapV2: PAIR_EXISTS'); // single check is sufficient

        UniswapV2Pair pairImpl = new UniswapV2Pair();

        bytes memory bytecode = type(ERC1967Proxy).creationCode;
        bytecode = abi.encodePacked(bytecode, abi.encode(address(pairImpl), ""));
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pairProxy := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        bytes memory data = abi.encodeWithSelector(
            UniswapV2Pair.initialize.selector, token0, token1
        );
        AddressUpgradeable.functionCall(pairProxy, data);

        getPair[token0][token1] = pairProxy;
        getPair[token1][token0] = pairProxy; // populate mapping in the reverse direction
        allPairs.push(pairProxy);
        emit PairCreated(token0, token1, pairProxy, allPairs.length);
    }
...
contract UniswapV2Pair is UniswapV2ERC20, Initializable, OwnableUpgradeable {
    using SafeMath  for uint;
