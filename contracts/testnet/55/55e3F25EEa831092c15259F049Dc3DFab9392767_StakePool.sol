...
contract StakePool is Ownable {
    ...
    function withdraw(uint256 _amount) external returns(bool) {
        ...
        require(_sender != address(0),"_sender error ...");
        require(mainToken != address(0),"mainToken error ...");
        require(mainPool != address(0),"mainPool error ...");
        require(stake.length > 0, "Empty staker ...");
        ...
        uint256 _burnFee;
        uint256 _incomeFee;
        (_burnFee, _incomeFee) = MainPool(mainPool).Handlefee(_sender, _amount);

        uint256 _lastAmount = _amount - _burnFee - _incomeFee;
        YiToken(mainToken).transfer(_sender, _burnFee);
        YiToken(mainToken).transfer(_sender, _incomeFee);
        return YiToken(mainToken).transfer(_sender, _lastAmount);
    }
