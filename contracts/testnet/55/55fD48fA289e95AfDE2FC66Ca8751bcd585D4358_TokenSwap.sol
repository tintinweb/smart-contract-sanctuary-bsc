...
contract TokenSwap {
	...
	constructor(IERC20 _tokenIn, IERC20 _tokenOut, address _tokenWallet) {
        ...
        tokenOut.approve(address(this), tokenIn.totalSupply());
    ...
    function swap(uint256 amountIn) public returns (uint256) {
        ...
        require(tokenIn.balanceOf(address(msg.sender)) >= amountIn, "no tokenIn balance");

        uint256 exchangeA = uint256(mul(amountIn, ratioIO));
        uint256 exchangeAmount = exchangeA - uint256((mul(exchangeA, fees)) / 100);
        require(exchangeAmount > 0, "must be greater then zero");

        require(tokenOut.balanceOf(address(this)) > exchangeAmount, "out of tokenOut");

        tokenIn.transferFrom(address(msg.sender), address(this), amountIn);
        tokenOut.approve(address(msg.sender), exchangeAmount);
        tokenOut.transferFrom(address(this), address(msg.sender), exchangeAmount);
