...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function _checkCancelRentingOrder(IMarketPlace.OrderKind _kind, uint _tokenId) private {
    ...
    require(!rentingOrder.rented, "order has rented");
  ...
  function setEnergyManager(address _energyManager) external onlyMainAdmin {
    energyManager = IEnergyManager(_energyManager);
  }
  ...
  function addLandContract(address _land) external onlyMainAdmin {
    require(_land != address(0), "Invalid address");
    nftTokens[IMarketPlace.OrderKind.land] = IGameNFT(_land);
  }
