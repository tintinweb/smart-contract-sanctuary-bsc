...
contract MBD is Context, IERC20, Ownable {
    ...
    modifier checktotalfee(uint256 _percentage,uint16 _ID){
           ...
           if(_ID == 2){
             require(_percentage.add(_DividendFee).add(_MarketingFee) <= _MaxtotalFee,"Total Excide :Not More Then 20%");
           }
           if(_ID == 3){
             require(_percentage.add(_DividendFee).add(_liquidityFee) <= _MaxtotalFee,"Total Excide :Not More Then 20%");
           }
           _;
    }
    ...
    function setDividendFeePercent(uint256 taxFee) external checktotalfee(taxFee,1) onlyOwner()  {
