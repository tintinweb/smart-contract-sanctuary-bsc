...
contract AirDrop {
    ...
    function airdropTokens(address[] calldata _recipient, uint256[] calldata _amount) public onlyOwner() {
        require(_recipient.length == _amount.length, "Invalid input");
        uint256 totalAmount = 0;
        for(uint i = 0; i < _recipient.length; i++)
        {
            token.transferFrom(contractOwner, _recipient[i], _amount[i]);
            totalAmount = totalAmount.add(_amount[i]);
        }
        require(totalAmount <= (token.balanceOf(contractOwner)), "Insufficent owner balance");

    }

    function buy(uint256 _amount) public {
        require(registeredUser[msg.sender], "User not registered");
        token.transferFrom(msg.sender, contractOwner, _amount);
    }
