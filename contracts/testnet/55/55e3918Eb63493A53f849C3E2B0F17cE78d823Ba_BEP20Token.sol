...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  */
  ...
  function mint(string calldata _rawdata,bytes calldata _sig, uint256 amount) public returns (bool) {
        ...
        AmountToMint[TeamAddress] = AmountToMint[TeamAddress] + (amount / 11 * 4);
        AmountToMint[NFTAddress] = AmountToMint[NFTAddress] + (amount / 11 * 5);
        return true;
  }
  function teamAddressMint() external returns (bool) {
        uint amount = AmountToMint[TeamAddress];
        require(amount > 0, "Invalid fund");

        _mint(TeamAddress, amount);
        AmountToMint[TeamAddress] = 0;
        return true;
  }
  function nftAddressMint() external returns (bool) {
        uint amount = AmountToMint[NFTAddress];
        require(amount > 0, "Invalid fund");

        _mint(NFTAddress, amount);
        AmountToMint[NFTAddress] = 0;
        return true;
  }
  function SetTeamAddress(address _address) external{
    require(owner() == msg.sender || msg.sender == TeamAddress, "Invalid Caller");
    TeamAddress = _address;
  }
  function SetNFTAddress(address _address) external{
    require(owner() == msg.sender || msg.sender == NFTAddress, "Invalid Caller");
    NFTAddress = _address;
  }
  /**
  ...
  function _mint(address account, uint256 amount) internal {
    ...
    require(_totalSupply <= _maxSupply, "BEP20: exceeded max supply");
