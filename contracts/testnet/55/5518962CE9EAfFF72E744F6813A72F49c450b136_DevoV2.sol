...
contract DevoV2 is IBEP20, Ownable {
    ...
    //Token Info
    string public constant name = "Devolution";
    string public constant symbol = "DeVo";
    ...
    function SetTaxes(
        ...
        require(buy <= MAX_TAX && sell <= MAX_TAX, "Tax exceeds max Tax");
        require(
            buyback + operations + liquidity == TAX_DENOMINATOR,
            "Taxes don't add up to denominator"
        );
        if (ZeroTax) require(buy == 0 && sell == 0, "Zero Tax is permanent");
        if (buy + sell == 0) ZeroTax = true;
        buyTax = buy;
        sellTax = sell;
        operationsTax = operations;
        liquidityTax = liquidity;
        buybackTax = buyback;
        emit OnSetTaxes(buy, sell, buyback, operations, liquidity);
    }
