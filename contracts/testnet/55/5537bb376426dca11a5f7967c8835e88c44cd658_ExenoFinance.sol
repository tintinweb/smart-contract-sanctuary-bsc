    ...
    IExenoFinanceNode
{
    ...
    bytes32 public constant ACTION_VOUCHER_CEDE = bytes32("voucher_cede");
    bytes32 public constant ACTION_VOUCHER_CLAIM = bytes32("voucher_claim");
    bytes32 public constant ACTION_ESCROW_CEDE = bytes32("escrow_cede");
    bytes32 public constant ACTION_ESCROW_REVOKE = bytes32("escrow_revoke");
    ...
    // Amount of accumulated fees (frontend | affiliate => amounts): first value is denominated in cash (i.e. native currency), while the second value is denominated in core token (i.e. EXN token)
    mapping(address => uint256[2]) public earnedFees;

    // Amount of overpaid fees (frontend => amounts): first value is denominated in cash (i.e. native currency), while the second value is denominated in core token (i.e. EXN token)
        ...
        external payable nonReentrant whenNotPaused
    {
        ...
        _processCredit(d, msg.sender, params);
    }
    ...
    /**
     * Gateway for deposits to this contract's account
     */
    ...
    {
        _collectDeposit(msg.sender, token, amount, msg.value);
        _processDeposit(address(this), token, amount);
    }

    /**
     * Gateway for debits made on behalf of this contract's account
     */
    function debitSelf(
        bytes calldata encodedData
    )
        external payable nonReentrant whenNotPaused
    {
        (ExenoFinance.Data memory d,, uint256[2] memory fee) = _unpackAndValidateTripleSignedData(encodedData, CLASS_DEBIT);
        _collectInboundCashPayments(d, fee, address(this), msg.value);
        _processDebit(d, address(this), bytes(""));
    }

    /**
     * Gateway for credits made on behalf of this contract's account
     */
    function creditSelf(
        bytes calldata encodedData
    )
        external payable nonReentrant whenNotPaused
    {
        (ExenoFinance.Data memory d,, uint256[2] memory fee) = _unpackAndValidateTripleSignedData(encodedData, CLASS_CREDIT);
        _collectOutboundCashPayments(d, fee, msg.value);
        _processCredit(d, address(this), bytes(""));
    }
        ...
        internal override nonReentrant whenNotPaused
    {
        ...
        if (operation == CLASS_DEBIT) {
            ...
            _processCredit(d, sender, params);
        ...
        internal
    {
        if (_isNative(token)) {
            ...
            ExenoFinance.transferFrom(sender, address(this), token, amount);
        }
    }
        ...
        internal
    {
        if (_isNative(d.token[0]) && d.explicit[0]) {
            ...
            _registerFees(Ccy.CASH, d.frontend, d.affiliate, fee[0], value - d.amount[0] - fee[0]);
        } else {
            if (d.explicit[0]) {
                ExenoFinance.transferFrom(sender, address(this), d.token[0], d.amount[0]);
            } else {
            ...
            _registerFees(Ccy.CASH, d.frontend, d.affiliate, fee[0], value - fee[0]);
        }
    }
        ...
        internal
    {
        if (_isCore(d.token[0]) && d.explicit[0]) {
            ...
            _registerFees(Ccy.CORE, d.frontend, d.affiliate, fee[1], value - d.amount[0] - fee[1]);
        } else {
            if (d.explicit[0]) {
                ExenoFinance.transferFrom(sender, address(this), d.token[0], d.amount[0]);
            } else {
            ...
            _registerFees(Ccy.CORE, d.frontend, d.affiliate, fee[1], value - fee[1]);
        }
    }
        ...
        internal
    {
        if (params.length > 0) {
            ...
            // ExenoFinance.validateOptionSell(d, token, amount);
            // ExenoFinance.emitSpinoffForOptionSell(d, token, amount);
        }
        ...
        internal
    {
        ...
        ExenoFinance.validateCredit(d, isCreditId(d.id), NETWORK, sender == address(this));
        creditIds[d.id] = true;
        ...
        internal pure returns(ExenoFinance.Data memory)
    {
        ...
        return(ExenoFinance.Data(id, frontend, affiliate, network, beneficiary, token, amount, timeout, method, explicit, memo));
    }
    ...
    /**
     * Register fees allocated to frontend & affiliate
     */
    function _registerFees(
        ...
        internal
    {
        earnedFees[frontend][uint8(ccy)] += earnedFee;
        earnedFees[affiliate][uint8(ccy)] += earnedFee;
        extraFees[frontend][uint8(ccy)] += extraFee;
    }
    ...
    {
        ExenoFinance.transferFrom(wallet, address(this), coreToken(), amount);
        frozenTokens += amount;
     ...
     * Release extra fees
     */
    // function releaseExtraFees(address frontend)
    //     external nonReentrant
    // {
    //     require(msg.sender == owner() || msg.sender == frontend,
    //         "EFN: unauthorized sender");
    //     require(extraFees[frontend][0] > 0 || extraFees[frontend][1] > 0,
    //         "EFN: no fees");
    //     emit ReleaseExtraFees(frontend, extraFees[frontend]);
    //     if (extraFees[frontend][0] > 0) {
    //         Address.sendValue(payable(frontend), extraFees[frontend][0]);
    //         extraFees[frontend][0] = 0;
    //     }
    //     if (extraFees[frontend][1] > 0) {
    //         ExenoFinance.transferTo(frontend, coreToken(), extraFees[frontend][1], availableTokens(coreToken()));
    //         extraFees[frontend][1] = 0;
    //     }
    // }
...
library ExenoFinance {
    ...
    bytes32 public constant METHOD_OPTION = bytes32("option");
    bytes32 public constant METHOD_OPTION_SELL = bytes32("option_sell");
        ...
        external view
    {
        ...
        } else if (d.method[0] == METHOD_OPTION) {
            require(d.method[1] == METHOD_OPTION,
                "EFN: method mismatch");
            require(d.network[0] == d.network[1],
                "EFN: network mismatch");
            require(d.timeout[1] > 0,
                "EFN: timeout mismatch");
        } else if (d.method[0] == METHOD_OPTION_SELL) {
            require(d.method[1] == METHOD_OPTION_SELL,
                "EFN: method mismatch");
            require(d.network[0] == d.network[1],
                "EFN: network mismatch");
            require(d.token[0] == d.token[1],
        ...
        external view
    {
        ...
        if (d.method[1] == METHOD_MINT || d.method[1] == METHOD_UNFREEZE) {
            ...
            require(d.network[0] == d.network[1],
                "EFN: network mismatch");
            require(d.token[0] == d.token[1] || selfmade,
                "EFN: sender mismatch");
            require(d.token[0] == d.token[1] || d.timeout[1] > 0,
                "EFN: timeout mismatch");
    ...
    function validateSwap(
        ...
        external pure
    {
        require(d.method[0] == ExenoFinance.METHOD_SWAP
            && d.method[1] == ExenoFinance.METHOD_SWAP,
            "EFN: method mismatch");
    }
        ...
        external pure
    {
        require(d.token[0] != d.token[1],
            "EFN: token mismatch");
        require(d.method[0] == METHOD_OPTION
            && d.method[1] == METHOD_OPTION,
            "EFN: method mismatch");
    }
        ...
        external pure
    {
        ...
        require(amount > 0,
            "EFN: amount mismatch");
        require(d.method[0] == METHOD_OPTION_SELL
            && d.method[1] == METHOD_OPTION_SELL,
            "EFN: method mismatch");
    }
        ...
        external pure
    {
        ...
        require(!processed[0] && !processed[1] && !processed[2],
            "EFN: credit a/y processed");
        require(d.method[0] == METHOD_VOUCHER
    ...
    function transferFrom(
        ...
        external
    {
        require(IERC20(token).allowance(payer, payee) >= amount,
            "EFN: unapproved a/t");
        require(IERC20(token).transferFrom(payer, payee, amount),
            "EFN: transfer failed");
    }
    ...
    function emitDebit(Data calldata d)
        external
    {
        emit Debit(d.id, d.frontend, d.affiliate, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.method, d.explicit, d.memo);
    }
    ...
    function emitCredit(Data calldata d)
        external
    {
        emit Credit(d.id, d.frontend, d.affiliate, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.method, d.explicit, d.memo);
    }
    ...
    function emitSwap(Data calldata d)
        external
    {
        emit Swap(d.id, d.frontend, d.affiliate, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.method, d.explicit, d.memo);
    }
    ...
    function emitOption(Data calldata d)
        external
    {
        emit Option(d.id, d.frontend, d.affiliate, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.method, d.explicit, d.memo);
    }
    ...
    function emitRevoke(Data calldata d)
        external
    {
        emit Revoke(d.id, d.frontend, d.affiliate, d.network, d.beneficiary, d.token, d.amount, d.timeout, d.method, d.explicit, d.memo);
    }
    ...
    /**
     * Publish a spinoff event resulting from an option
     */
    function emitSpinoffForOptionSell(
        Data calldata d,
        address token,
        uint256 amount
    )
        external
    {
        emit Spinoff(
            ...
            [d.token[0], token],
            [d.amount[0], amount],
            [int32(0), 0 - d.timeout[1]],
            [METHOD_OPTION_SELL, METHOD_OPTION_SELL],
            d.explicit,
            d.memo
        );
    }
