...
contract Raffle is Auth {
    ...
    function batchBuyLottoTicketFriend(address _friend, uint8 _numberOfTickets) external payable {
        ...
        // LottoInfo storage lot = allLotteries_[lotteryIdCounter_];
        // lot.raisedAmount += msg.value;

        // Emitting event with all information
        emit NewBatchMint(_friend, _numberOfTickets, totalCost, msg.value);
    }

    function batchBuyLottoTicket(uint8 _numberOfTickets) external payable notContract() { this.batchBuyLottoTicketFriend(msg.sender, _numberOfTickets); }
