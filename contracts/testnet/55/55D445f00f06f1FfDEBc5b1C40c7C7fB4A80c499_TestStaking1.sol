...
contract TestStaking1 is Ownable, IStakingLockable {
  ...
  function deposit(uint256 amount) external {
    ...
    IStakePool(stakingPool).addHolder(account);
  ...
  function setPoolInfo(IERC20Upgradeable _rewardToken, IERC20Upgradeable _stakingToken, address _stakingPool) external onlyOwner() {
    require(
      address(rewardToken) == address(0) && address(liquidityMining.stakingToken) == address(0) && address(stakingPool) == address(0),
