...
contract SwapPool is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _addLiquidity(
        ...
        if (useEth) {
            ...
            TransferHelper.safeTransferFrom(address(wrappedToken), msg.sender, address(this), amount0);
        }
        TransferHelper.safeTransferFrom(address(abnbcToken), msg.sender, address(this), amount1);
        if (nativeTokenAmount == 0 && ABNBCTokenAmount == 0) {
            ...
            lpToken.mint(msg.sender, (2 * amount0) / 10**18);
        } else {
    ...
    function _removeLiquidity(uint256 removedLp, bool useEth) internal virtual {
        ...
        if (useEth) {
            ...
            TransferHelper.safeTransfer(address(wrappedToken), msg.sender, amount0Removed);
        }
        TransferHelper.safeTransfer(address(abnbcToken), msg.sender, amount1Removed);
    ...
    function swapEth(
        ...
        return _swap(nativeToABNBC, amountIn, receiver, true);
    }
    ...
    function _swap(
        ...
        if (nativeToABNBC) {
            if (useEth) {
                ...
                TransferHelper.safeTransferFrom(address(wrappedToken), msg.sender, address(this), amountIn);
            }
            if (!excludedFromFee[msg.sender]) {
                ...
                if (integratorLockEnabled) {
                    ...
                    if (integratorFeeAmt > 0) {
                        TransferHelper.safeTransfer(address(wrappedToken), msg.sender, integratorFeeAmt);
                    }
                }
                nativeTokenAmount +=
                amountIn +
            ...
            TransferHelper.safeTransfer(address(abnbcToken), receiver, amountOut);
            emit Swap(msg.sender, receiver, nativeToABNBC, amountIn, amountOut);
        } else {
            TransferHelper.safeTransferFrom(address(abnbcToken), msg.sender, address(this), amountIn);
            if (!excludedFromFee[msg.sender]) {
                ...
                if (integratorLockEnabled) {
                    ...
                    if (integratorFeeAmt > 0) {
                        TransferHelper.safeTransfer(address(abnbcToken), msg.sender, integratorFeeAmt);
                    }
                }
                ABNBCTokenAmount +=
                amountIn +
            ...
            if (useEth) {
                ...
                TransferHelper.safeTransfer(address(wrappedToken), receiver, amountOut);
            }
            emit Swap(msg.sender, receiver, nativeToABNBC, amountIn, amountOut);

            uint256 abnbcBal = abnbcToken.balanceOf(address(this));
    ...
    function _withdrawOwnerFee(
        ...
        if (amount0 > 0) {
            ...
            if (useEth) {
                ...
                TransferHelper.safeTransfer(address(wrappedToken), msg.sender, amount0);
            }
        }
        if (amount1 > 0) {
            ownerFeeCollected.abnbcFee -= amount1;
            TransferHelper.safeTransfer(address(abnbcToken), msg.sender, amount1);
        }
    ...
    function _withdrawManagerFee(address managerAddress, bool useNative) internal virtual {
        ...
        if (feeRewards.nativeFee > 0) {
            ...
            if (useNative) {
                ...
                TransferHelper.safeTransfer(address(wrappedToken), managerAddress, feeRewards.nativeFee);
            }
        }
        if (feeRewards.abnbcFee > 0) {
            ...
            TransferHelper.safeTransfer(address(abnbcToken), managerAddress, feeRewards.abnbcFee);
        }
    ...
    function triggerRebalanceAnkr() external virtual nonReentrant onlyManager {
        ...
        if (isStake) {
            ...
            bnbPool.unstakeCerts(abnbcAmount);

            emit PoolUnstake(abnbcAmount);
        }
    }

    function approveToMaticPool() external virtual {
        TransferHelper.safeApprove(address(abnbcToken), address(bnbPool), type(uint256).max);
    }
    ...
    function skim() public virtual {
        ...
        if (contractBal > nativeTokenAmount) {
            ...
            emit NativeBalanceChanged(nativeTokenAmount);
        }
    }
    ...
    function flashFee(address token, uint256 amount) public view returns (uint128) {
        ...
        if (!(token == address(abnbcToken) || token == address(wrappedToken))) return 0;
