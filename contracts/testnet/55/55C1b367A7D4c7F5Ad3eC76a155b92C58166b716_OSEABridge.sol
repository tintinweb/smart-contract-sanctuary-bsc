...
contract OSEABridge {
    ...
    function send(uint16 dstChainId, address to, uint256 amount) external {
        require(isPaused == false, "PAUSED");
        require(amount > 0, "<= 0");
        require(token.allowance(msg.sender, address(this)) >= amount); // TODO: probably already handled by _debitFrom
        token.sendFrom(msg.sender, dstChainId, abi.encodePacked(msg.sender == taker ? treasury : (to == address(0) ? msg.sender : to)), amount, payable(msg.sender), address(0), bytes(""));
