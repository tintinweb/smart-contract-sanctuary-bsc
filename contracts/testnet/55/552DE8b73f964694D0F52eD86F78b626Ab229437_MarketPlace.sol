...
contract MarketPlace is Initializable {
  ...
  uint32 public liquidityPoolFee; // 60%
  ...
  function setLiquiditysPoolFee(uint32 value) public onlyOwner {
  ...
  function _transferFee(uint256 amount) private {
    ...
    require(liquidityAddress != address(0),"Liquiditys pool can't Zero!");
    ...
    uint256 feeLiquidityPool = amount.mul(liquidityPoolFee).div(10000);
    IERC20(tokenAddress).transferFrom(address(msg.sender), address(liquidityAddress), feeLiquidityPool);
  }
