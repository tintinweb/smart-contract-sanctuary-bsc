...
// File: contracts/NumiStake.sol

pragma solidity 0.6.12;

contract NumiStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the smart chef factory
    address public NUMI_POOL_FACTORY;
    ...
    // The block number when reward allocation ends.
    ...
    // The minimum supply for the user to get rewarded
    uint256 public minSupplyForReward;

    // reward token amount per 1 staked token
    ...
    // End lock when pool ends or not
    bool public endLockWhenPoolEnds = false;
    ...
    struct UserInfo {
        ...
        uint256 remainedReward; // rewards that were not harvested (when there is not enough reward token in the contract)
        uint256 lastDepositTime; // Last deposited time
    }
    ...
    event NewRewardPerToken(uint256 oldReward, uint256 newReward);
    event NewDepositFee(uint16 oldFee, uint16 newFee);
    event NewFeeAddress(address oldAddress, address newAddress);
    event NewPoolLimit(uint256 poolLimitPerUser);
    event NewMinSupplyForReward(uint256 oldValue, uint256 newValue);
    event RewardsStop(uint256 blockNumber);
    event Withdraw(address indexed user, uint256 amount);

    constructor() public {
        NUMI_POOL_FACTORY = msg.sender;
    }
     ...
     * @param _rewardPerToken: reward per token (in rewardToken)
     * @param _startBlock: start block
     * @param _endBlock: end block
     * @param _poolLimitPerUser: pool limit per user in stakedToken (if any, else 0)
     * @param _minSupplyForReward: minimum supply that a user should stake for getting reward
     * @param _lockTimeForReward: lock time for getting reward from the pool
    ...
    function initialize(
        ...
        uint256 _minSupplyForReward,
        uint256 _lockTimeForReward,
        uint16 _depositFee,
        address _feeAddress,
        address _admin
    ) external {
        require(!isInitialized, "Already initialized");
        require(msg.sender == NUMI_POOL_FACTORY, "Not factory");
        require(_feeAddress != address(0), "Invalid zero address");
        require(_rewardPerToken > 0, "Invalid zero value");
        require(
            _poolLimitPerUser == 0 || _minSupplyForReward <= _poolLimitPerUser,
            "Minimum supply for reward should be less than maximum limit"
        );
        ...
        minSupplyForReward = _minSupplyForReward;
        lockTimeForReward = _lockTimeForReward;
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        // Harvest is only avialable when the user is not locked
        if (!userLocked(msg.sender)) {
            uint256 pending = pendingReward(msg.sender);
            if (pending > 0) {
                uint256 rewardTransferred = safeRewardTransfer(
            ...
            // Withdraw before lock ends need to pay tax
            amountToWithdraw = _amount.sub(
                _amount.mul(taxForEarlyWithdraw).div(10000)
            );
        }
        ...
        returns (uint256)
    {
        ...
        if (address(stakedToken) == address(rewardToken)) {
            rewardBalance = rewardBalance.sub(stakedSupply);
        }
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        user.amount = 0;
        user.remainedReward = 0;
        user.lastRewardBlock = block.number;
        stakedSupply = stakedSupply.sub(amountToTransfer);

        if (userLocked(msg.sender)) {
            amountToTransfer = amountToTransfer.sub(
                amountToTransfer.mul(taxForEarlyWithdraw).div(10000)
            );
        }
    ...
    /**
     * @notice Update lock configuration
     * @dev Only callable by owner.
     * @param _lockTimeForReward: lock time for getting reward from the pool
     * @param _taxForEarlyWithdraw: tax percentage when trying to withdraw before lock ends
     * @param _endLockWhenPoolEnds: end lock when pool ends or not
     */
    function updateLockConfiguration(
        uint256 _lockTimeForReward,
        uint16 _taxForEarlyWithdraw,
        bool _endLockWhenPoolEnds
    ) external onlyOwner {
        require(_taxForEarlyWithdraw <= 10000, "Invalid tax percentage");
        lockTimeForReward = _lockTimeForReward;
        taxForEarlyWithdraw = _taxForEarlyWithdraw;
        endLockWhenPoolEnds = _endLockWhenPoolEnds;
    }

    /**
     * @notice Update pool limit per user
     * @dev Only callable by owner.
     * @param _minSupplyForReward: new pool limit per user
     */
    function updateMinSupplyForReward(uint256 _minSupplyForReward)
        external
        onlyOwner
    {
        require(
            poolLimitPerUser == 0 || _minSupplyForReward <= poolLimitPerUser,
            "Minimum supply for reward should be less than maximum limit"
        );
        emit NewMinSupplyForReward(minSupplyForReward, _minSupplyForReward);
        minSupplyForReward = _minSupplyForReward;
    }

    /*
     * @notice Update reward per token
     * @dev Only callable by owner.
     * @param _rewardPerToken: the reward per token
     */
    function updateRewardPerToken(uint256 _rewardPerToken) external onlyOwner {
        require(_rewardPerToken > 0, "No reward");
        emit NewRewardPerToken(rewardPerToken, _rewardPerToken);
        rewardPerToken = _rewardPerToken;
    }
    ...
    /**
     * @notice view function to see
     */
    function userLocked(address _user) public view returns (bool) {
        UserInfo storage user = userInfo[_user];
        if (endBlock <= block.number && endLockWhenPoolEnds) {
            return false;
        }
        if (user.lastDepositTime.add(lockTimeForReward) > now) {
            return true;
        }
        return false;
    }
