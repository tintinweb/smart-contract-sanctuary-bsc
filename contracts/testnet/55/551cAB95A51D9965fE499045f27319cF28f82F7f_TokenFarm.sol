...
contract TokenFarm {
    ...
    function getMultiProfit(uint256[]memory listprofileId) public view returns(uint256[] memory) {

       uint256[] memory result;

       for(uint i = 0; i <listprofileId.length; i++){
            uint256 data = getCurrentProfit(listprofileId[i]);
            result[i] = data;
        }
       return result;
    }

    function getMultiStake(uint256[]memory listprofileId) public view returns(uint256[] memory) {

       uint256[] memory result;

       for(uint i = 0; i <listprofileId.length; i++){
            uint256 data = getCurrentStakeUnlock(listprofileId[i]);
            result[i] = data;
        }
       return result;
    }
