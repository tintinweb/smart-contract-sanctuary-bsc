    ...
    OwnableUpgradeable
{
     ...
     */
    uint256 public _typeIdPool;

    /**
     *  @notice CombatantAddress address is address of membercard token.
     */
    address private combatantAddress;
     ...
     */
    function getLimitAmountOfOwner(address owner)
        public
        view
        returns (uint256)
    {
        uint256[] memory ids = ICombatant(combatantAddress).tokensOfOwner(
            owner
        );
        uint256 count;
        for (uint256 i = 0; i < ids.length; i++) {
            ICombatant.CombatantBox memory box = ICombatant(combatantAddress)
                .getCombatantBoxOf(ids[i]);
            if (
                uint256(box.typeId) == _typeIdPool &&
                box.startTimeOfStaking.add(_poolDuration) > block.timestamp &&
                box.startTimeOfStaking != 0
            ) {
                count++;
            }
        }
        ...
        return
            ICombatant(combatantAddress)
                .getLimitStakingOf(ICombatant.TypeId(_typeIdPool))
                .mul(count);
    }
    ...
    function getUserAmount(address user) public view returns (uint256) {
        return userInfo[user].totalAmount;
    }

    /**
     *  @notice Get start time of NFT.
     */
    function getStartTime(uint256 tokenId) public view returns (uint256) {
        ICombatant.CombatantBox memory box = ICombatant(combatantAddress)
            .getCombatantBoxOf(tokenId);

        return box.startTimeOfStaking;
    }

    /**
     *  @notice Get start time of NFT in identified user
     */
    function getStartTimeStakingNFT(address user, uint256 data)
        public
        view
        returns (uint256)
    {
        uint256[] memory ids = ICombatant(combatantAddress).tokensOfOwner(user);
    ...
    /**
     *  @notice Sync time and status of NFT
     */
    function syncTime() internal {
        // Update status NFT
        // all NFT must be set status
        // get time of box staked before
        bool isNewStaking = getStartTimeStakingNFT(_msgSender(), 1) == 0
            ? true
            : false;
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (box.startTimeOfStaking == 0) {
                // Update status FREEDOM to STAKING
                if (isNewStaking) {
                    ICombatant(combatantAddress).setStatusCombatantBox(
                        ids[i],
                        block.timestamp
                    );
                } else {
                    ICombatant(combatantAddress).setStatusCombatantBox(
                        ids[i],
                        getStartTimeStakingNFT(_msgSender(), 1)
                    );
                }
            }
        }
    ...
    function stake(uint256 _amount) public nonReentrant {
        ...
        syncTime();
        require(
            getLimitAmountOfOwner(_msgSender()) > 0,
            "Require to have NFT for staking in pool"
        );
        require(
            getStartTimeStakingNFT(_msgSender(), 1).add(12 weeks) >
                block.timestamp,
        ...
        if (_amount > 0) {
            require(
                _stakedAmount.add(_amount) <=
                    getLimitAmountOfOwner(_msgSender()),
    ...
    function pendingRewards(address _user) public view returns (uint256) {
        ...
        if (getStartTimeStakingNFT(_msgSender(), 1) <= block.timestamp) {
     ...
     */
    function requestWithdraw() public nonReentrant returns (uint256) {
        require(
            getStartTimeStakingNFT(_msgSender(), 2).add(_poolDuration) <
                block.timestamp &&
                getStartTimeStakingNFT(_msgSender(), 2) != 0,
            "Not allow withdraw at this time"
        );
        UserInfo storage user = userInfo[_msgSender()];
        user.lazy.isRequested = true;
        user.lazy.unlockedTime = block.timestamp + 1 days;
    ...
    function unstake(uint256 _amount) public nonReentrant {
        UserInfo storage user = userInfo[_msgSender()];
        require(
            getStartTimeStakingNFT(_msgSender(), 1).add(_poolDuration) <=
                block.timestamp,
            "Staking: StakingPool for NFT has not expired yet.."
        );
        require(
            user.lazy.isRequested && user.lazy.unlockedTime < block.timestamp,
            "Can withdraw after 24 hours"
        );
        // Update time NFT
        syncTime();
        user.lazy.isRequested = false;
        ...
        if (_amount > 0) {
            ...
            user.indexLength = user.indexLength.add(1);
            user.userHistory.push(
                UserHistory("Withdrawed", user.totalAmount, block.timestamp)
            );
        }
    ...
    function calReward(address _user) public view returns (uint256) {
        ...
        uint256 minTime = min(
            block.timestamp,
            getStartTimeStakingNFT(_user, 1).add(_poolDuration)
        );
