...
contract OwnerManagement is Ownable{
    ...
    event TranserOwnership(address indexed _owner, address indexed _newOwner);

    event AddressWhitelisted(
        address indexed _address
    );

    function addInstitution() private {
        address _institution = institutions[institutions.length-1];
        instInfo[_institution].name = IDocumentStoreInterface(_institution).getName();
        instInfo[_institution].email = IDocumentStoreInterface(_institution).getEmail();
        instInfo[_institution].contractAddress = _institution;
        instInfo[_institution].legalReference = IDocumentStoreInterface(_institution).getLegalReference();
        instInfo[_institution].intentDeclaration = IDocumentStoreInterface(_institution).getIntentDeclaration();
        instInfo[_institution].host = IDocumentStoreInterface(_institution).getHost();
    ...
    function setName(address _contract, string memory _name) internal {
        instInfo[_contract].name = _name;
    }

    function setEmail(address _contract, string memory _email) internal {
        instInfo[_contract].email = _email;
    }

    function setLegalReference(address _contract, string memory _legalReference) internal {
        instInfo[_contract].legalReference = _legalReference;
    }

    function setIntentDeclaration(address _contract, string memory _intentDeclaration) internal {
        instInfo[_contract].intentDeclaration = _intentDeclaration;
    }

    function setHost(address _contract, string memory _host) internal {
        instInfo[_contract].host = _host;
    }
    
    modifier onlyWhitelisted() {
        require(addressWhitelisted[msg.sender]);
        _;
    }    
}
...
contract DocumentStore is OwnableUpgradeable {
    ...
    function setEmail(string memory _email) external onlyOwner{
        email = _email;
    ...
    function setLegalReference(string memory _legalReference) external onlyOwner{
        legalReference = _legalReference;
    ...
    function setIntentDeclaration(string memory _intentDeclaration) external onlyOwner{
        intentDeclaration = _intentDeclaration;
    ...
    function removePublisher(address _publisher) external onlyOwner {
        require(publisherCheck(_publisher) == true);
    ...
    function removeAllPublishers() external onlyOwner {
        for (uint256 i; i < publishers.length; i++) {
            publishers.pop();
        }
    }

    function addPublishers(address[] memory _newPublishers) external onlyOwner {
        for (uint256 i; i < publishers.length; i++) {
            publishers.pop();
        }
    ...
    function addPublisher(address _newPublisher) external onlyOwner {
        require(publisherCheck(_newPublisher) == false);
        publishers.push(_newPublisher);
    }
    ...
    function getExpirationDate(bytes32 document)
        external
        view
        onlyIssued(document)
        returns (uint256)
    ...
    function isExpired(bytes32 document)
        public
        view
        onlyIssued(document)
        returns (bool)
    {
        return documentExpiration[document] > block.timestamp;
    }
    ...
    modifier onlyPublisher() {
        require(publisherCheck(msg.sender) == true, "Error: Only Publisher can revoke/issue documents"
        );
        _;
    }
