...
contract LUUINE is ILUUINE{
    ...
    function withdraw(uint256 amount) public whenNotPaused {
        ...
        require(userRecharge[_msgSender()] > 0, "withdraw amount too small2");
        userWithdraw[_msgSender()] += amount;

        require(userWithdraw[_msgSender()]/userRecharge[_msgSender()] - SysConfig.withdrawBei > 0, "withdraw amount too small3");
        //扣费
        _subAsset(_msgSender(), GOLD_NAME, amount);
        //提现
        _getUSDT().transfer(_msgSender(), withdrawAmount);
        _addRebate(_msgSender(),_msgSender(),withdrawAmount,6,0);
    }
    ...
    function _profit(address owner, uint256 amount) internal returns (bool) {
        ...
        for (uint256 i = 0; i < SysConfig.maxLevel; i++) {
            ...
            if(userLevel[parentAddress]>levelMax){
                //vip等级钱
                uint256 ping = 0;
                (level,ratePer,ping) = getLevelsIndex(userLevel[parentAddress] - 1);
                if (ratePer > 0) {
                    uint256 profitAmount = curAmount * ratePer / 10000;
                    _addAsset(parentAddress, GOLD_NAME, profitAmount);
                    _addRebate(parentAddress,owner,profitAmount,userLevel[parentAddress],i+1);
                }
                
                if(ping>0){
                    uint256 profitAmount = curAmount * ratePer * ping / 10000 / 10000;
                    _profit_ping(parentAddress,profitAmount,userLevel[parentAddress]);
                }
                //赋值等级只返一次
                levelMax = userLevel[parentAddress];
            }
        }
         //创始股东钱
        for(uint256 index=0;index<userHolder[HOLDER_NAME].length;index++){
            address parentAddress = userHolder[HOLDER_NAME][index];
            uint256 profitAmount = curAmount * SysConfig.holderPercent / 10000;
            _addAsset(parentAddress, GOLD_NAME, profitAmount);
            _addRebate(parentAddress,owner,profitAmount,5,0);
        }
        return true;
    }
    /**
    * 平级返佣
    */
    function _profit_ping(address owner, uint256 amount,uint256 level) internal returns (bool){
        address curAddress = owner;
        uint256 curAmount = amount;
        for (uint256 i = 0; i < SysConfig.maxLevel; i++) {
            if (curAmount <= 0) {
            ...
            if(userLevel[parentAddress] == level){
                //平级奖励
                _addAsset(parentAddress, GOLD_NAME, curAmount);
                _addRebate(parentAddress,owner,curAmount,4,0);
                break;
            }
        }
        return true;
    ...
    * 升级统计数据
     */
     function _upgrade(address owner) public returns(bool){
        address curAddress = owner;
        ...
        for (uint256 i = 0; i < SysConfig.maxLevel; i++) {
            ...
            for (uint256 j = 0; j < _levelStruct.length; j++) {
                //有效并且满足升级条件
                if(userStatusValid[parentAddress] == 1 && userTeam[parentAddress].length >= _levelStruct[j].zhiNum && userValid[parentAddress] >= _levelStruct[j].validNum){
     ...
     * 返佣记录
      */
      function _addRebate(address owner,address addr,uint256 price,uint256 types,uint256 level) public{
     ...
     * 充值记录
      */
    // function _addRecharge(address owner) public{
    //     RechargeStruct[] storage recharges = _userRecharge[owner];
    //     uint256 curTime = block.timestamp;
    //     uint256 price = SysConfig.rechargePrice;
    //     recharges.push(RechargeStruct(price,curTime));
    // }
      /**
      * 获取充值记录
       */
     ...
     */
    function getRebateLevelsIndex(uint256 index) public view returns (uint256, uint256) {
        if(index < getRebateLevelsTotal()){
     ...
     * Get Distribution Levels index
     */
    function getLevelsIndex(uint256 index) public view returns (uint256, uint256,uint256) {
        if(index < getLevelsTotal()){
            uint256 level = _levelStruct[index].level;
            uint256 ratePer = _levelStruct[index].percent;
            uint256 ping = _levelStruct[index].ping;
            return (level,ratePer,ping);
        }
        return (0,0,0);
    }
    /**
     ...
     */
    function getRebateLevelsTotal() public view returns (uint256) {
        return _rebateStruct.length;
    }
    function getLevelsTotal() public view returns (uint256) {
        return _levelStruct.length;
    }
    /**
     * Set rebate 比例
     */
    ...
    * 设置会员等级
    */
    // function setUserLevel(address owner,uint256 level) public
    // {
    //     require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Must have admin role to init config");
    ...
    * 设置参数
     ...
     * @dev Get 直推列表
     */
    function getUserCount(address owner) public view returns (uint256,uint256){
        return (userTeam[owner].length,userValid[owner]);
    }
    /**
     * Get Team Total
     */
    ...
    //     require(owner != address(0), "Address invalid");
    //     require(index < userTeam[owner].length, "User Team index out of bounds");
