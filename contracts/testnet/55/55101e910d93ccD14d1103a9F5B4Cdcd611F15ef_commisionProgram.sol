...
contract commisionProgram is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => AddToken) public token_info;
    ...
    event AddNewToken(uint256 indexed token_id, IERC20 token_address);
    event AddNewProduct(
     ...
     */
    function addToken(uint256 _token_id, IERC20 _token_address) external onlyAdmin{
        require(_token_address != token_info[_token_id].token_address, "Token already added!");

        AddToken storage tokenAdd = token_info[_token_id];

        tokenAdd.token_id = _token_id;
        tokenAdd.token_address = _token_address;

        emit AddNewToken(_token_id, _token_address);
    }
    ...
    function addProduct(
        uint256 _product_id,
        uint256 _product_token,
        uint256 _product_price,
        uint256 _product_stocks,
        string memory _product_name,
        string memory _product_image
    ) external onlyAdmin {
        require(_product_id != product_info[_product_id].product_id, "Product already added");
        require(_product_token == token_info[_product_token].token_id, "Token is not on the list");

        AddProduct storage productAdd = product_info[_product_id];
        bool status = true;

        productAdd.product_id = _product_id;
        productAdd.product_token = _product_token;
    ...
    function redeemProduct(
        ...
        require(_redeem_amount == product_info[_redeem_product].product_price, "Insufficient balance");

        IERC20 token_to_pay;
        uint256 token_number = 0;
        
        RedeemProduct storage productRedeem = redeemed[_redeem_id][msg.sender];
        AddProduct storage productAdd = product_info[_redeem_product];

        productRedeem.redeem_id = _redeem_id;
        productRedeem.redeem_product = _redeem_product;
        productRedeem.redeem_amount = _redeem_amount;
        productRedeem.redeem_quantity = _redeem_quantity;

        token_number = productAdd.product_token;
        AddToken memory tokenAdd = token_info[token_number];
        token_to_pay = tokenAdd.token_address;
        token_to_pay.safeTransferFrom(msg.sender, address(this), _redeem_amount);
    ...
    {
        uint256 length = product_count;

        uint256[] memory values = new uint256[](length);
        AddProduct[] memory betInfo = new AddProduct[](length);

        for (uint256 i = 0; i < length; i++) {
            values[i] += i;
            betInfo[i] = product_info[values[i]];
        }

        return (values, betInfo);
    }
