...
contract ICOFactory is Ownable, AccessControl {
    ...
    event ICOContractProjectOwnerUpdated(address indexed ICOAddress, address projectOwner);
    event ICOContractTokenDecimalUpdated(address indexed ICOAddress, uint256 decimal);
    event StableCoinAddressUpdated(address _stableCoinAddress);

    /**
     * @dev Set stable coin address.
     * Setup role for deployer.
     * @param _stableCoinAddress - New stable coin address for create new ICO contract.
     */
     ...
     * @param _paidIntervals - Initial token distribute interval array.
     * @param _paidPortions - Initial token distribute portion array.
    ...
    */
    function createICOContract(
        ...
        uint256 _startDate,
        uint256 _endDate,
        uint256[] memory _paidIntervals,
        uint256[] memory _paidPortions,
     ...
     * @param _projectOwner - Purchased user address (project owner).
     */
    function changeICOContractProjectOwner(address _ICOAddress, address _projectOwner)
        external
        onlyOwner
    {
     ...
     */
    function changeICOContractTokenDecimal(address _ICOAddress, uint256 _decimal)
        external
        onlyOwner
    {
        IICO ico = IICO(_ICOAddress);
    ...
    /**
     * @dev Set new address for ICO contract's stable coin address.
     * @param _stableCoinAddress - New stable coin address for create new ICO contract.
     */
    function setStableCoinAddress(address _stableCoinAddress) public onlyOwner {
        stableCoinAddress = _stableCoinAddress;
        emit StableCoinAddressUpdated(_stableCoinAddress);
    }
}
...
contract ICO is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    uint256[] public paidIntervals;
    uint256[] public paidPortions;
    ...
    event TokenBought(address userAddress, uint256 stableCoinAmount, uint256 tokenAmount);
    event TokenWithdrawn(address userAddress, uint256 tokenAmount);
    event StableCoinWithdrawn(address userAddress, uint256 stableCoinAmount);
    event TokenSwaped(address ownerAddress, uint256 tokenAmount, uint256 stableCoinAmount);
    ...
    constructor(
        ...
        token = IERC20(_initialTokenAddress);
        ...
        endDate = _endDate;
        paidIntervals = _paidIntervals;
        paidPortions = _paidPortions;
    ...
    */
    ...
    function buyToken(uint256 _stableAmount) external {
        require(block.timestamp > startDate && block.timestamp < endDate, "[ICO.buyToken] Not in buying period");
        require(stableCoin.allowance(msg.sender, address(this)) >= _stableAmount, "[ICO.buyToken] Not enought allowance");
        require(users[msg.sender].stableCoinAmount + _stableAmount <= buyingLimit, "[ICO.buyToken] Exceed user's buying limit");
        require(totalStableCoin + _stableAmount <= totalBuyingLimit, "[ICO.buyToken] Exceed total buying limit");
    ...
    function withdrawToken() external nonReentrant onlyBought(msg.sender) {
        require(totalToken > 0, "[ICO.withdrawToken] Cannot withdraw token if token balance is 0");
        require(block.timestamp > paidIntervals[0], "[ICO.withdrawToken] Not in withdrawing period");

        uint256 withdrawAmount = calculateWithdrawAmount(msg.sender);
        users[msg.sender].withdrawnAmount += withdrawAmount;
        safeTokenTransfer(msg.sender, withdrawAmount);
    ...
    function withdrawStableCoin() external nonReentrant onlyBought(msg.sender) {
        require(totalToken == 0, "[ICO.withdrawStableCoin] Cannot withdraw stable coin if token balance more than 0");
        require(block.timestamp > paidIntervals[0], "[ICO.withdrawStableCoin] Cannot withdraw stable coin before first interval");

        uint256 withdrawAmount = users[msg.sender].stableCoinAmount;
        delete users[msg.sender];
        safeStableCoinTransfer(msg.sender, withdrawAmount);
     ...
     */
    function swapTokenWithStableCoin() external nonReentrant onlyRole(PROJECTOWNER) {
        require(block.timestamp > endDate, "[ICO.swapTokenWithStableCoin] Cannot swap token before endDate");
        require(block.timestamp < paidIntervals[0], "[ICO.swapTokenWithStableCoin] Cannot swap token after first interval");

        uint256 tokenAmount = (totalStableCoin * rate) / tokenDecimal;
        uint256 stableCoinAmount = totalStableCoin;
        totalToken = tokenAmount;
        totalStableCoin = 0;

        token.safeTransferFrom(msg.sender, address(this), tokenAmount);
        safeStableCoinTransfer(msg.sender, stableCoinAmount);

        emit TokenSwaped(msg.sender, tokenAmount, stableCoinAmount);
    }
     ...
     */
    function calculateWithdrawAmount(address _userAddress) public view returns(uint256) {
        UserInfo memory user = users[_userAddress];
        uint256 withdrawAmount;
        uint256 interval;
        uint256 totalPaidPortion;
        uint256 currentTime = block.timestamp;
        if (currentTime > paidIntervals[0]) {
            for (uint256 i = paidIntervals.length - 1; i >= 0; i--) {
                if (currentTime >= paidIntervals[i]) {
                    interval = i;
                    break;
                }
            }
            for (uint256 j = 0; j <= interval; j++) {
                totalPaidPortion += paidPortions[j];
            }
            withdrawAmount = (totalPaidPortion * user.tokenAmount / 100) - user.withdrawnAmount;
        }
    ...
    /**
     * @dev Safty transfer Token.
     * @param _userAddress - User address to be transfered to.
     * @param _tokenAmount - Token amount to be transfered.
     */
    ...
    /**
     * @dev Safty transfer Stable Coin.
     * @param _userAddress - User address to be transfered to.
     * @param _tokenAmount - Stable Coin amount to be transfered.
     */
    function safeStableCoinTransfer(address _userAddress, uint256 _tokenAmount)
        internal
    {
        uint256 tokenBalance = stableCoin.balanceOf(address(this));
        if (_tokenAmount >= tokenBalance) {
            stableCoin.safeTransfer(_userAddress, tokenBalance);
        } else {
            stableCoin.safeTransfer(_userAddress, _tokenAmount);
        }
    }
}
