...
contract PandoAssemblyV3 is Ownable, IERC721Receiver, Pausable {
    ...
    function deposit(uint256[] memory tokenIds, address to) external whenNotPaused onlyMinUserLevel{
        ...
        require(EnumerableSet.length(user.nftIds) + tokenIds.length <= 9 + slotPurchased[to], 'Staking : stake more than slot purchased');

        // Effects
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require( minDroidBotLevelStaking <= droidBot.level(tokenId), "NFTStakingPool: caller is not the minimum droid bot level requirements");
