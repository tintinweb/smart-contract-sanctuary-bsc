...
contract PMGContract {
    ...
    function buyNFT(uint256 _index) public  {
        ...
        IStrategy(strategy[item.cryptoToken]).deposit(item.price * 160 / 1000 );
    ...
    function bidNFT(uint256 _auctionItemId,uint256 _price) public  {
        ...
        if(userBalance[msg.sender][auctionitem.cryptoToken].availableBalance< _price){
            ...
            IStrategy(auctionitem.cryptoToken).deposit(remaining);
    ...
    function deposit(address _tokenContract,uint256 _amount) public   {
    ...
    function withdraw(address _tokenContract,uint256 _amount) public   {
        require(isTokenSupport[_tokenContract]==true,"token contract not support");
        require(userBalance[msg.sender][_tokenContract].availableBalance >= _amount,"insufficient fund");

        IStrategy(strategy[_tokenContract]).withdraw(_amount);
        IERC20(_tokenContract).transfer(msg.sender,_amount);
        userBalance[msg.sender][_tokenContract].availableBalance -= _amount;
    }

    function addStrategy(address _tokenContract,address _strategy) public onlyOwner  {
