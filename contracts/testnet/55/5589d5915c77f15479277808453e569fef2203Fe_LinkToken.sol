...
contract LinkToken is ERC20, Ownable {
    ...
    constructor(
        ...
        buybackWalletAddress = buybackWalletAddress_;
    ...
    function setBlackList(address _address,bool _enable)external onlyOwner{
        _blackList[_address]  = _enable;
        emit SetBlackList(_address,_enable);
    }
    ...
    function _transfer(
        ...
        require(!isBlackList(from),"Blacklist not transfer");
        ...
        if( canSwap &&
            ...
            if(AmountLpRewardFee > 0){
                swapTokenForUsdt(AmountLpRewardFee,address(dividendTracker));
                AmountLpRewardFee = 0;
            }
            if(AmountMarketingFee > 0){
                swapTokenForUsdt(AmountMarketingFee,marketingWalletAddress);
                AmountMarketingFee = 0;
            }
            if(AmountBuybackFee > 0){
                swapTokenForUsdt(AmountBuybackFee,buybackWalletAddress);
                AmountBuybackFee = 0;
            }
            swapping = false;
        }
    ...
    function takeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        super._transfer(from, address(this), LPFee.add(MFee).add(BBFee));
    }
