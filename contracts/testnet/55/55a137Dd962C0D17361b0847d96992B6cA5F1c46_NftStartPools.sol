    ...
    ERC721Holder
{
    ...
    struct UserInfo {
        ...
        uint256 lockPeriod; // 5days 10days 15days ....
    ...
    // lock period to nft rarity to reward per block
    mapping(uint256 => mapping(uint256 => uint256)) public lockRarityRewards;
    ...
    event EventDeposit(address indexed user, uint256 indexed nftId, address nftAddr, uint256 depositedDate, uint256 lockPeriod);
    ...
    function initialize(
        ...
        require(address(_rewardToken) != address(0),"Invalid reward address");
        require(_nftContract != address(0),"Invalid _nftContract address");
    ...
    function setPeriodRarityReward(
        uint256 _period, 
        uint256 _rarity, 
        ...
        public onlyOwner 
    {
        require(_rarity >= 0 && _rewardPerBlock > 0 && _period > 0 ,"Invalid values");
        lockRarityRewards[_period][_rarity] = _rewardPerBlock;
    }

    function setNftContract(address _nftContract, bool _status) external onlyOwner {
        require(_nftContract != address(0),'invalid address');
        nftContracts[_nftContract] = _status;
    }

    function setEndStakingBlock(uint256 _endBlock) external onlyOwner {
        require(_endBlock >= block.number,"Invalid end block");
        endStakingBlock = _endBlock;
    }

    function calcPendingReward(address _nftContract, uint256 _nftId) internal view returns(uint256) {
        UserInfo storage stake = stakes[_nftContract][_nftId];
        IZombieWorldZNFT nftItem = IZombieWorldZNFT(_nftContract);
        (,uint256 rarity,,) = nftItem.getNFTInfo(_nftId);

        if (stake.stakingStaus == true && stake.stakedBlock > 0 && lockRarityRewards[stake.lockPeriod][rarity] > 0) {
            return (block.number - stake.stakedBlock).mul(lockRarityRewards[stake.lockPeriod][rarity]);
        } else {
            return 0;
        }
    }

    function pendingReward(address _nftContract, uint256 _nftId) external view returns(uint256){
        return calcPendingReward(_nftContract, _nftId);
    }
        ...
        external nonReentrant 
    {
        ...
        require(block.number < endStakingBlock,"Pool is finished");
        ...
        stake.nftId = _nftId;
        stake.stakedBlock = block.number;
        stake.harvestedAmount = 0;
        stake.nftAddress = _nftAddress;
        stake.lockPeriod = _lockPeriod;
        stake.endTimestamp = block.timestamp + _lockPeriod;
        stakeCount++;
        // transfering NFT to this contract
        IERC721(_nftAddress).safeTransferFrom(msg.sender, address(this), _nftId);

        emit EventDeposit(msg.sender, _nftId, _nftAddress, block.timestamp, _lockPeriod);
    }

    function safeERC20Transfer(IERC20 erc20, address _to, uint256 _amount)
      private
    {
     ...
     */
    function withdraw(uint256 _nftId, address _nftAddress, bool unStake) external nonReentrant {
        UserInfo storage stake = stakes[_nftAddress][_nftId];
        uint256 stakedBlocks = block.number - stake.stakedBlock;
        uint256 _amount;
        if (stakedBlocks >= stake.lockPeriod) {
            _amount = calcPendingReward(_nftAddress, _nftId);
        }
        ...
        if (unStake == true) {
            stakeCount--;
            delete stakes[_nftAddress][_nftId];
