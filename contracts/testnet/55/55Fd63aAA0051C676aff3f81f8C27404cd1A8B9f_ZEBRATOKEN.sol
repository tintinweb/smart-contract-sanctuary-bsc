...
contract ZEBRATOKEN is ERC20, Ownable {
    ...
    function swapAndSendToMarketingFee(uint256 tokens) private  {
        ...
        //sum usdt
        uint256 usdtValue=IERC20(_usdt).balanceOf(address(_tokenDistributor));

        uint256 sumFee=marketingFeeV2.add(buyDevFeeV2).add(rewardsFeeV2);

        uint256 marketUsdt=usdtValue.mul(marketingFeeV2.div(sumFee));
        uint256 devUsdt=usdtValue.mul(buyDevFeeV2.div(sumFee));
        uint256 rewardsUsdt=usdtValue.mul(rewardsFeeV2.div(sumFee));

        IERC20(_usdt).transferFrom(address(_tokenDistributor),_marketingWalletAddress,marketUsdt);
        IERC20(_usdt).transferFrom(address(_tokenDistributor),_devWalletAddress,devUsdt);
        IERC20(_usdt).transferFrom(address(_tokenDistributor),_tokenRewardWalletAddress,rewardsUsdt);
    }
}
