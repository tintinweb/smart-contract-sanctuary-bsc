...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function mintAllowed(address eToken, address minter, uint mintAmount) external onlyProtocolAllowed returns (uint) {
     ...
     */
    function redeemAllowed(address eToken, address redeemer, uint redeemTokens) external onlyProtocolAllowed returns (uint) {
     ...
     */
    function borrowAllowed(address eToken, address borrower, uint borrowAmount) external onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
    ...
    function liquidateBorrowAllowed(
        ...
        if (!(markets[eTokenBorrowed].isListed || address(eTokenBorrowed) == address(vaiController)) || !markets[eTokenCollateral].isListed) {
            return uint(Error.MARKET_NOT_LISTED);
        }

        /* The borrower must have shortfall in order to be liquidatable */
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, EToken(0), 0, 0);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall == 0) {
        ...
        uint borrowBalance;
        if (address(eTokenBorrowed) != address(vaiController)) {
    ...
    function seizeAllowed(
        ...
        if (!markets[eTokenCollateral].isListed || !(markets[eTokenBorrowed].isListed || address(eTokenBorrowed) == address(vaiController))) {
            return uint(Error.MARKET_NOT_LISTED);
        }
        ...
        // Keep the flywheel moving
        updateVenusSupplyIndex(eTokenCollateral);
        distributeSupplierVenus(eTokenCollateral, borrower);
        distributeSupplierVenus(eTokenCollateral, liquidator);
     ...
     */
    function transferAllowed(address eToken, address src, address dst, uint transferTokens) external onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
     ...
     * @return (errorCode, number of eTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateVAICalculateSeizeTokens(address eTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
     ...
     * @param holder The address to claim XVS for
     * @param eTokens The list of markets to claim XVS in
     */
    function claimVenus(address holder, EToken[] memory eTokens) public {
        address[] memory holders = new address[](1);
        holders[0] = holder;
        claimVenus(holders, eTokens, true, true);
    }
     ...
     */
    function claimVenus(address[] memory holders, EToken[] memory eTokens, bool borrowers, bool suppliers) public {
        uint j;
