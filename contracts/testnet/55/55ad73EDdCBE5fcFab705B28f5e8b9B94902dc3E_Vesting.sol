...
contract Vesting is Ownable, IVestingImportable {
    ...
    constructor(address _token, bool _isTestnet) {
        ...
        if (_isTestnet) {
            timeDenominator = 9500; //over 950 second period (will release 1% every second after TGE at 5%)
        }
        else {
    ...
    function claimableTokenAmount(address _address) public view returns (uint256) {
        ...
        if (!Sales[_address].tgeDistributed) {
            ...
            uint256 lastClaimTime = Sales[msg.sender].lastClaimTime;
            ...
            if (claimableAmount >= (totalAllocation - Sales[msg.sender].amountClaimed)) {
                claimableAmount = totalAllocation - Sales[msg.sender].amountClaimed;
            }
