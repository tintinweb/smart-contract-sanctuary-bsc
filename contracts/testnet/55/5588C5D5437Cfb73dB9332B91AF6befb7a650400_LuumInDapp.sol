...
contract LuumInDapp{
    ...
    // function __amountPerc2(uint _amount, uint decimals) private pure returns(uint){
    //     return _amount * (10 ** 2);
    // }
    ...
    // event purchaseDone(string nameProduct, uint totalPrice, address indexed client);
    ...
    // mapping (address => purchases) public MappingPurchases;
    ...
    function CrearProducto(
        ...
        {  
           MappingProductos[_id] = Producto (_id, _nombreProducto, _descripcion, _seccion, __amount(_precioTokens), _DiasCiclo, _PercAttMin, _PercAttMax, true, block.timestamp);
           MapProducts.push(Producto(_id, _nombreProducto, _descripcion, _seccion, __amount(_precioTokens), _DiasCiclo,  _PercAttMin, _PercAttMax, true, block.timestamp));
           Productos.push(_id);
           productsCounter += 1;
        ...
        emit ProductoAgregado(_id, _nombreProducto, _descripcion, _seccion, __amount(_precioTokens), _DiasCiclo, _PercAttMin, _PercAttMax, true, block.timestamp);
    }
    ...
    function OtorgarRendimiento(uint256 _idContract, uint _porcentajeRendimiento) public Unicamente(msg.sender) {
        ...
        if (true) {
            ...
            require (MappingProductos[_idContract].percAttMin <= _porcentajeRendimiento, "Este porcentaje es inferior al minimo requerido");
            require (MappingProductos[_idContract].percAttMax >= _porcentajeRendimiento, "Este porcentaje es superior al porcentaje maximo permitido");
