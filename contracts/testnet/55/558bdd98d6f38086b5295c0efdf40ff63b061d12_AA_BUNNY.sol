...
contract AA_BUNNY is Context, IERC20 { 
    ...
    */
    ...
    function swapAndLiquify(uint256 contractBalance) private lockTheSwap {
        ...
        // Get fee trackers
        uint256 trackingTotal = (Fee_M + Fee_L + Fee_D + Fee_S);

        // Get fee splits
        uint256 percent_L = Fee_L * accuracy / trackingTotal;
        uint256 percent_D = Fee_D * accuracy / trackingTotal;
        uint256 percent_S = Fee_S * accuracy / trackingTotal;
        uint256 percent_M = Fee_M * accuracy / trackingTotal;

        // Calculate number of tokens that need to be swapped
        uint256 token_half = contractBalance * percent_L / accuracy / 2;
        uint256 swapTokens = contractBalance - token_half;
        ...
        uint256 BNB_D = returned_BNB * percent_D / accuracy;
        uint256 BNB_S = returned_BNB * percent_S / accuracy;
        uint256 BNB_M = returned_BNB * percent_M / accuracy;

        uint256 bnb_half = returned_BNB - BNB_D - BNB_S - BNB_M;

        // Add liquidity 
        if (token_half > 0 && bnb_half > 0){
        ...
        // Take developer fee
        if (BNB_S > 0){
        sendToWallet(Wallet_SolidityDev, BNB_S);
        }
        ...
        uint256 remaining_BNB = address(this).balance;
        if(remaining_BNB > 0){
            sendToWallet(Wallet_Marketing, remaining_BNB);
        }
