...
contract RefundProxy is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getHoldersData() public view returns (uint256) {
        return holders.length;
    }

    function getReceivedData() public view returns (uint256) {
        return withdrawList.length;
    }

    function getHolderData(address _holderAddress) public view returns (uint256, uint256, bool) {
    ...
    function refund(address _holderAddress) external nonReentrant onlyWhitelist {
        ...
        withdrawList.push(_holderAddress);
