...
contract Test is Ownable, Rebasable
{
    ...
    {
		require(fee < 15, 'Test: Transaction fee should be less than 25%');
        BUY_FEE = fee;
    }

    function updateSellFee(uint16 fee) external onlyOwner
    {
		require(fee < 20, 'Test: Sell fee should be less than 25%');
        SELL_FEE = fee;
    }
   ...
   function transfer(address recipient, uint256 amount) public returns (bool) {
	...
	function _transfer(address sender, address recipient, uint256 amount) private 
	{
		...
		require(sender != address(0), "Test: cannot transfer from the zero address");
        require(recipient != address(0), "Test: cannot transfer to the zero address");
        require(amount > 0, "Test: Transfer amount must be greater than zero");
		...
		if(sender != owner() && recipient != owner() && !inSwapAndLiquify) {
            ...
            require(amount <= _maxTxAmount, "Test: Transfer amount exceeds the maxTxAmount.");
            }
    ...
    */
    ...
    function updateRewardAddress(address rewards_) external onlyOwner
    {
