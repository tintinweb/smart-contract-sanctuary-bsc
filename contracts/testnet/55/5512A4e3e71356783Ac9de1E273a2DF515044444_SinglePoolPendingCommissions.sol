    ...
    RewardRateConfigurable
{
    ...
    mapping(uint256 => mapping(address => UserRewardsInfo)) public userRewardsInfo;
    mapping(uint256 => mapping(address => uint256)) public partnersClaimRewards;

        // pool => user => level => amount
    mapping(uint256 => mapping(address => mapping(uint256 => uint256))) public userRewards;
    ...
    function updateRewards(
        ...
        for (uint256 i = 0; i < _distributionInfo.length; ++i) {
            ...
            if (_isDeposit) {
                userRewards[_pid][_distributionInfo[i].user][i] += _distributionInfo[i].amount;
    ...
    function _getUserShares(
        ...
        for (uint256 i = 0; i <= 24; ++i) {
            _userShares += userRewards[_pid][_user][i];
            if(i == _level) {
