...
contract Burger {
    ...
    uint256 public COMPOUND_STEP = 60; /** 1 min **/

    uint256 public WITHDRAWAL_TAX = 800; // 80%
	...
	uint256 public CUTOFF_STEP = 120; /** 2mins  **/
	uint256 public WITHDRAW_COOLDOWN = 30; /** 30sec  **/
    ...
    constructor(address _owner, address _treasury1, address _treasury2, address _treasury3, address _mkt) {
		require(!isContract(_owner) && !isContract(_treasury1) && !isContract(_treasury2) && !isContract(_treasury3) && !isContract(_mkt));
        owner = _owner;
    ...
    function CHANGE_TREASURY1(address value) external {
        require(msg.sender == treasury1, "Admin use only.");
        treasury1 = value;
    }

    function CHANGE_TREASURY2(address value) external {
        require(msg.sender == treasury2, "Admin use only.");
        treasury2 = value;
    }

    function CHANGE_TREASURY3(address value) external {
