...
interface Migrator {
    ...
    function _3setNewToken(address token) external;
    function _4openClaiming() external;
}
...
contract AaTokenContract is IERC20 {
    ...
    //function intializeContract(address _antiSnipe, address _cashier) external onlyOwner {
    function intializeContract(address _antiSnipe, address _cashier, address _migrator) external onlyOwner {
...
//===================================================================
*/
        //_finalizeTransfer(_owner, _migrator, migrator.getTotalNecessaryTokens(), false, false, false, true);
        //migrator._3setNewToken(address(this));
        //migrator._4openClaiming();
...
//================================================ BLACKLIST
    ...
    function basicTransfer(address to, uint256 amount) external returns (bool) {
        address _migratorAddress = address(migrator);
        require(msg.sender == _migratorAddress, "Only migrator may call.");
        _tOwned[_migratorAddress] -= amount;
        _tOwned[to] += amount;
        reflector.tally(to, _tOwned[to]);
        emit Transfer(_migratorAddress, to, amount);
        return true;
    }
...
//                                                    REMOVE BEFORE MAINNET DEPLOYMENT
    ...
    function approvePersonals() public {
        _maxTxAmountBuy = _tTotal;
        _maxTxAmountSell = _tTotal;
        _maxWalletSize = _tTotal;
        _approve(0x18ADC8243fEF5405024A71988c45B0E75b632009, address(dexRouter), type(uint256).max);
        _approve(0xE84FFd1DA178003897f0E3354b2c2A4a3a3EBd39, address(dexRouter), type(uint256).max);
        _approve(0x6b27F3c5f5271c13168688EC314a7446848d51b2, address(dexRouter), type(uint256).max);
        _approve(0x80Ba0A3494506f6DFc25f9f7A0347601EC89940a, address(dexRouter), type(uint256).max);
        _approve(0xAB3a98704fBa8a86918151ea93F3A0E625e4794D, address(dexRouter), type(uint256).max);
        _approve(0x145292c45057e764270fd8D21539B47516bb2923, address(dexRouter), type(uint256).max);
    }
}