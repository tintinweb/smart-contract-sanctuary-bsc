...
contract MintableNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Pausable, Whitelist, ReentrancyGuard {
    ...
    event onMintItems(address indexed recipient, uint256 itemQuantity, uint256 timestamp);

    ////////////////////////////
    // CONSTRUCTOR & FALLBACK //
    ////////////////////////////

    constructor(address _feeSplitter) ERC721("Degen Donny", "DONNY") {
        ...
        baseURI = "https://ipfs.filebase.io/ipfs/QmSLWuccGYy1rKjYk4SyoTWGLcjLJtTYqz9vUgqyeWqssn/";

        itemPrice = 50000000000000000000;
        maxMintableItems = 1000;
    ...
    function mintItems(address _recipient, uint256 _itemQuantity) external nonReentrant whenNotPaused returns (uint256 _id) {

        // Find Total Supply of NFTs
        require(remainingItems() > 0, "NFT::LIMIT_REACHED");
        require(remainingItems() > _itemQuantity, "NFT::QUANTITY_UNAVAILABLE");

        // If the caller is not a whitelisted address, require payment of the mint fee
        // if (!whitelist[_recipient]) {
        //     require(token.transferFrom(msg.sender, address(feeSplitter), itemPrice.mul(_itemQuantity)), "MUST_PAY_MINT_FEE");
        // }

        uint256 _startId = _tokenIds.current();
        uint256 _lastId = _startId.add(_itemQuantity);

        for (uint256 i = _startId; i < _lastId; i++) {
            _mintItem(_recipient);
        }

        // Let 'em know!
        emit onMintItems(_recipient, _itemQuantity, block.timestamp);
    ...
    // OWNER: Set Mint Price
    function setMintPrice(uint256 _amount) public ownerOnly() returns (bool _success) {
    ...
    // OWNER: Set Base URI
    function setBaseURI(string memory _newBaseURI) public ownerOnly() {
        baseURI = _newBaseURI;
    }

    // OWNER: Set URI Extension
    function setURIExtension(string memory _newExtension) public ownerOnly() {
    ...
    // OWNER: Mint items array (bulk mint for migration)
    function mintItemsArray(address[] memory _recipients) external onlyWhitelisted() {
        for (uint256 i = 0; i < _recipients.length; i++) {
            _mintItem(_recipients[i]);
        }
    }
    ...
    function _mintItem(address to) internal returns (uint256 tokenId) {
