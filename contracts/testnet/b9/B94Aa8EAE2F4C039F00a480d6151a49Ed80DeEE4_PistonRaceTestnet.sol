...
/*
    ...
    /** NOTE! this will be removed when testing is done. **/
     ...
     function TEST_DEPOSITTIME_USER() public /*onlyOwner*/ {
         users[msg.sender].deposit_time -= 24 hours;
     }
    ...
    // like a roll but taxed 20% and without the 5x rule
    ...
    function payoutOf(address _addr, bool isClaimedOut, bool useBoosterRate) public view returns(uint256 payout, uint256 max_payout, uint256 net_payout, uint256 sustainability_fee) {
        ...
        if(users[_addr].payouts < max_payout) {
            ...
            uint256 boosterRate = 0;
            if(useBoosterRate){
                boosterRate = variableTaxcontroller.getBoosterRate(_addr);
            }
            uint256 finalPayoutRate = basePayoutRate.add(boosterRate);

            //Using 1e18 we capture all significant digits when calculating available divs
            share = users[_addr].deposits.mul(finalPayoutRate.mul(1e18)); //divide the profit by payout rate
            share = share.div(100e18).div(24 hours); // divide by seconds in the day
            share = share.div(10); // back to normal base
    ...
    function switchUpline(address _addr, address _newUpline) external onlyOwner {
        address oldUpline = users[_addr].upline;
        if(users[oldUpline].referrals > 0){
            users[oldUpline].referrals--;
        }

        // decrease the old upline structure
        address _upline = oldUpline;
        for(uint8 i = 0; i < ref_depth; i++) {
            if(_upline == address(0)) break;
            users[_upline].total_structure--;
            _upline = users[_upline].upline;
        ...
        // increase the new upline structure
        _upline = _newUpline;
        for(uint8 i = 0; i < ref_depth; i++) {
            if(_upline == address(0)) break;
            users[_upline].total_structure++;
        ...
        users[_newUpline].referrals++;

        users[_addr].upline = _newUpline;
    }
