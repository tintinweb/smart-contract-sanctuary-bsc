...
function totalSupply() external view returns (uint256);
    ...
    event _getEventOFLiquify(uint256 contBal, uint256 dev , uint256 halfLp);
    ...
    event  _swapLiqStartFee(uint256  _dvTx ,uint256   _liqTx);
...
contract Stargold is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "SWAPBNB"; 
    ...
    uint256 public  _tokenForLiquidity = 10;
    address private _previousOwner;
    IUniswapV2Router02 public  uniswapV2Router;
    address public  uniswapV2Pair;
    IERC20  public WETH;
    uint8   public  BuyOrSell=1;
    
    address payable public   _devWallet    =payable(0xF79Fd9De0f9E8C7404174c458207B42e965c7575);
    address payable public   _markWallet    =payable(0xd2D630e01b2852027766d69C4e8B34588AdB4194);
...
function IsSetBal(bool SetVal) external onlyOwner(){
    SetBal = SetVal ;
  ...
  function sendToWallet(address payable wallet, uint256 amount) private {
   ...
   function _updateBuyTax(uint256 market, uint256 dev) external onlyOwner() 
     {
   ...
   function _updateSellTax(uint256 markTX,  uint256 deveTax) external onlyOwner() 
     {
        _markSelTx = markTX;
         _dvSelTx=deveTax;
         BuyOrSell=2;
   ...
   function totalSupply() public view override returns (uint256) {
    ...
    function _transfer(
        ...
        require(amount > 10, "Transfer amount must be greater than given amount"); 
       ...
       {
           sendAmnt=amount.mul(_dvTx.add(_markTx)).div(100);
           emit  sendAmountBefore(sendAmnt);
           emit seeLiq("1 Percent");
       }
       else{
           sendAmnt=_tokenForLiquidity;
            emit seeLiq("sel liquidity");
       }
        ...
        _tOwned[from] -= amount;
         ...
         uint256 transferAmount = amount;
          if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
          {
                ...
                uint256 markFee = amount.mul(_markTx).div(100); 
                uint256 devfee= amount.mul(_dvTx).div(100);
                _tOwned[address(this)] += markFee.add(devfee);
                emit Transfer (from, address(this), markFee.add(devfee));
                transferAmount=amount.sub(markFee).sub(devfee);
         } 
          _tOwned[to] += transferAmount;
          emit Transfer(from, to, transferAmount);
          emit _displayTakeFee(takeFee, _contractBalance , _tokenForLiquidity, sendAmnt );
       if (_tOwned[from].sub(amount) <= 10 )
        {
            _tOwned[from] =_tOwned[from].add(10);
            emit checkLastBalance(10);
        }
        else
        {
            emit checkLastBalance(100000);
        }
  ...
  function swapAndLiquify(uint256 _contractBalance) private 
  {
      require(SetBal,"No SetBal");
          ...
          emit _BNBTotal(BNB_Total);
            ...
            uint256 split_D = _dvTx.mul(100).div(_markTx.add(_dvTx));
            uint256 BNB_D = BNB_Total.mul(split_D).div(100);
             uint256 split_M = _markTx.mul(100).div(_markTx.add(_dvTx));
             uint256 BNB_M = BNB_Total.mul(split_M).div(100);
             emit _SPLITD(split_D);
