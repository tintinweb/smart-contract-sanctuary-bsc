...
contract MetaFranchiseManagerV3 is Initializable, PausableUpgradeable {
    ...
    function setMetastockTokenAddress(address tokenAddress)
        external
        virtual
        onlyOwner
    {
        metastockTokenAddress = tokenAddress;
    }
    ...
    function getMetaFranchisesUnclaimedRewardsTokens(uint256 companyId)
        external
        view
        returns (uint256)
    {
    ...
    function claimFromAllFranchisesTokens(
        uint256 _companyId,
        address companyOwner
    ) external {
        uint256 totalUnclaimed = this.getMetaFranchisesUnclaimedRewardsTokens(
            _companyId
        );
        ...
        payable
    {
        ...
        uint256 teamBnb = (totalUnclaimed *
            maintainceFranchiseExpensesBNBPercent) / 10000;

        uint256 companyOwnerBnb = totalUnclaimed - teamBnb;

        (bool teamSend, ) = payable(teamAddress).call{value: teamBnb}("");
        require(teamSend, "Failed to send Ether to team");

        (bool ownerSend, ) = payable(companyOwner).call{value: companyOwnerBnb}(
            ""
        );
    ...
    function swapTokensForBNB(uint256 tokenAmount) private {
        ...
        IERC20(metastockTokenAddress).approve(address(dexRouter), tokenAmount);
