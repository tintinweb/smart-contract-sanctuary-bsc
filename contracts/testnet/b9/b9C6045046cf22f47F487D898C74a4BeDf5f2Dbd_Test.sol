...
contract Test is Context, IERC20 { 
    ...
    address payable public Wallet_Marketing = payable(0xB880832fDD355600b2027B58bdA5401073fd0DB8); 
    address payable public Wallet_Charity = payable(0x4ce29788fB334433b98A2D1c79AE04cc82553E51);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 
    address private Wallet_Operation = (0x27b7bA02F77A1471d4bCD4499650f3c1698D5af6);
    ...
    uint256 public Percent_Charity = 10;
    ...
    constructor () {
        ...
        _owner = 0x27b7bA02F77A1471d4bCD4499650f3c1698D5af6;
        ...
        _isExcludedFromFee[Wallet_Operation] = true;
    ...
    function setTxLimit(uint256 maxTxPercent) external onlyOwner() {
      require(maxTxPercent > 499999999 * 10**_decimals, "Cannot set transaction amount less than 500.000.000 Tokens!");
        _maxTxAmount = (maxTxPercent);

    }

    function setSwapAndLiquifyEnabled(bool _true_or_false) public onlyOwner {
        swapAndLiquifyEnabled = _true_or_false;
        emit SwapAndLiquifyEnabledUpdated(_true_or_false);
    }

    function ManualSend(address Token_Address, uint256 percent_of_Tokens) public onlyOwner returns(bool _sent){
        uint256 totalManualsend = IERC20(Token_Address).balanceOf(address(this));
        uint256 removeManual = totalManualsend*percent_of_Tokens/100;
        _sent = IERC20(Token_Address).transfer(Wallet_Marketing, removeManual);
    }
    ...
    function EnableHappyBuy() external onlyOwner() 
    {
        _Tax_On_Buy = 0;
    }

    function DisableHappyBuy() external onlyOwner() 
    {
        _Tax_On_Buy = 8;
    }

     function updateSwapTrigger(uint8 amount) external onlyOwner {
        swapTrigger = amount;
    }

    function BotLock(address account, bool value) external onlyOwner{
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            uint256 tokens_to_D = contractTokenBalance * Percent_Charity / 100;
            ...
            uint256 split_M = Percent_Marketing * 100 / (Percent_AutoLP + Percent_Marketing + Percent_Charity);
            uint256 BNB_M = BNB_Total * split_M / 100;

            uint256 split_D = Percent_Charity * 100 / (Percent_AutoLP + Percent_Marketing + Percent_Charity);
            ...
            sendToWallet(Wallet_Charity, BNB_Total);
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(Wallet_Charity, removeRandom);
