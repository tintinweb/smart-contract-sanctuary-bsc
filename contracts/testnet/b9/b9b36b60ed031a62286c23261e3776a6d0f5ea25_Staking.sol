...
contract Staking is Initializable{
    ...
    function initialize(address _depositToken,address _rewardToken)external initializer{
        owner = msg.sender;
        depositToken = _depositToken;
        rewardToken = _rewardToken;
        lockInPeriod = 600;
        emergencyExitFees = 10;
        rewardRate = 180;
    }
    ...
    function stake(uint256 _amount) external{
        ...
        if(userCount[msg.sender]==1){
            unlockTime = block.timestamp + lockInPeriod;
        }
        else{
            unlockTime = userStake[msg.sender].unlockTime;
        }
        uint256 currentTime = block.timestamp;
        userStake[msg.sender].amount += _amount;
        userStake[msg.sender] = Stake(userStake[msg.sender].amount,unlockTime,currentTime);
    ...
    function withdraw(uint256 _amount) external {
        ...
        uint256 reward;
        uint256 stakedAmount = userStake[msg.sender].amount;
        uint256 unlockTime = userStake[msg.sender].unlockTime;
        uint256 depositTime = userStake[msg.sender].depositTime;
        ...
        if(_amount == stakedAmount){
            ...
            totalRewards += reward;
            userStake[msg.sender] = Stake(_amount,unlockTime,depositTime);
    ...
    function emergencyExit(uint256 _amount) external {
        ...
        if(_amount > stakedAmount){revert customErrors.amountGreaterThanStakedAmount(_amount);}
        if(stakedAmount <= 0){revert customErrors.noStakesFound();}
    ...
    function getUserReward()external view returns(uint256 _reward){
        if(userStake[msg.sender].unlockTime >= block.timestamp){
    ...
    function withdrawTokens()external onlyOwner{
        IERC20(depositToken).transfer(owner,IERC20(depositToken).balanceOf(address(this)));
    }
    ...
    function calculateReward(address user,uint256 _amount) public view returns (uint256) {
        uint256 reward = _amount * rewardRate * (block.timestamp - userStake[user].depositTime) / (365 days * 100*10);
        return reward;
    }
