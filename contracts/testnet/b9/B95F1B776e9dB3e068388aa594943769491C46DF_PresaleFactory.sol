...
contract PresaleFactory is ReentrancyGuard, Ownable {
    ...
    uint constant public TOKEN_PRICE = 1 * 10 ** 18;
    uint constant public TOKEN_LISTING_PRICE = 1 * 10 ** 18;
    uint constant public LIQUIDITY_PERCENT = 51;
    uint constant public HARD_CAP = 10 ether;
    uint constant public SOFT_CAP = 3 ether;
    ...
    function _addLiquidityOnOrbital() private returns(uint amountA, uint amountB, uint liquidity)
    {
        if(payingToken != address(0x0)) {
            ...
            presaleToken.approve(address(orbitalRouter), amountPresaleToken);

            uint amountPayingToken = 1 ether;
            IERC20(payingToken).approve(address(orbitalRouter), amountPayingToken);
            tokenReminder = IERC20(payingToken).balanceOf(address(this)) - amountPayingToken - totalSold;

            (amountA, amountB, liquidity) = orbitalRouter.addLiquidity(
                payingToken,
