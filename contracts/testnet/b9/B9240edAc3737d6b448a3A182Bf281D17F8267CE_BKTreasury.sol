...
import {IERC165, ERC165Checker} from "../oz/utils/introspection/ERC165Checker.sol";
    ...
    ERC721TokenReceiver
{
    ...
    function notifyERC20Transfer(
        ...
        if (
            token_ == address(0) ||
            token_ == address(this) ||
            __supportInterfaceUnchecked(token_, type(IERC721).interfaceId) ||
            __supportInterfaceUnchecked(token_, type(IERC1155).interfaceId)
    ...
    function _withdraw(
        ...
        } else if (__supportInterfaceUnchecked(token_, type(IERC721).interfaceId)) {
        ...
        else if (__supportInterfaceUnchecked(token_, type(IERC1155).interfaceId)) {
    ...
    function __checkInterface(
        ...
        if (!__supportInterfaceUnchecked(token_, interfaceId_))
            revert Treasury__InvalidFunctionCall();
    }

    function __supportInterfaceUnchecked(address account_, bytes4 interfaceId_) private view returns (bool) {
        (bool ok, bytes memory returnOrRevertData) = account_.staticcall(abi.encodeCall(IERC165.supportsInterface, (interfaceId_)));
        if (!ok) return false;
