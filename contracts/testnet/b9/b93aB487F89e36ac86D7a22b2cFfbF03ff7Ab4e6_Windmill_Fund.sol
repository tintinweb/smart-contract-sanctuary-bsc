...
contract Windmill_Fund is Base, Adressable, Math, Payable, AccessControl, With_DAORole, Terminatable, With_UpdaterRole, With_TradeManagerRole, With_TradeRole,
    ...
    function updateWithdrawFees(uint256 finalBalance, uint256 initBalance) external onlyRole(TRADE_ROLE){
        if (finalBalance > initBalance){
            uint256 fundBUSD = getFundBUSD();
            uint256 ratioTradeNumerator = (initBalance * 1e10) / fundBUSD;
            
            dynamicFees.startDynamicFeesNumerator = _getDynamicFees();
            dynamicFees.startDynamicFeesNumerator += ((finalBalance -  initBalance) * ratioTradeNumerator) / initBalance;
