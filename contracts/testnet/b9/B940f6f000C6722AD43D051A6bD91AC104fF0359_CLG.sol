...
contract CLG is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    function sell(uint256 tokenId, Currency memory price, address payable wallet, bytes memory ceekUserIdParam, address[] memory paymentTokenNft, string[] memory _allowedToBuyCeekUserId, address[] memory _allowedToBuyWallet) public {	
        ...
        if (price.payment_token != address(0) && price.value > 0) {
            // set wallet payment
            if (wallet != address(0)) {
    ...
    function beforeBuyCheck1(uint256 tokenId, Currency memory tokenPrice, address paymentTokenAddress) private view {
        uint256 i;
        bool ptOk1 = false;
        bool onSale = false;
        ...
        if (currency.payment_token != address(0) && currency.value > 0) {
            ...
            require((currency.value==tokenPrice.value && currency.payment_token==tokenPrice.payment_token), "CEEKLANDGATE_032");
        }
    ...
    function beforeBuyCheck2(uint256 tokenId, uint256 tokenPrice, bytes memory ceekUserIdParam, address paymentTokenAddress) private returns(address, string memory, Currency memory) {
        ...
        if (sellBidPrice[tokenId].value > 0) {
    ...
    // Buy option
    function buy(uint256 tokenId, Currency memory price, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {        
        uint256 token_price;
        beforeBuyCheck1(tokenId, price, address(payment_token));

        Currency memory defaultBidPriceCurrency = sellBidPrice[tokenId];
        
        if (address(payment_token) == price.payment_token) {
            token_price = defaultBidPriceCurrency.value;
        } else {
            (uint256 exRates1, uint256 exRates2) = getExchangeRate(defaultBidPriceCurrency.payment_token, address(payment_token));

            token_price = defaultBidPriceCurrency.value.mul(exRates1).div(exRates2);
        }
        ...
        emit Sale(tokenId, _wallets[tokenId], address(payment_token), defaultBidPriceCurrency, saleBidPriceCurrency, amount4owner);
        emit Commission(tokenId, admin, address(payment_token), defaultBidPriceCurrency, saleBidPriceCurrency, commissionRate, amount4admin);
