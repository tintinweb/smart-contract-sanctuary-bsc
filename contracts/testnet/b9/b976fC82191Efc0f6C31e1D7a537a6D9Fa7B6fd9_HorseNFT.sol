...
contract HorseConfig is  HorseStruct, OwnableUpgradeable, PausableUpgradeable {
  ...
  mapping (uint256 => uint256) public breadConfigs;
Utils public utils;
    ...
    function initialize() public initializer {
      // COMMON
        ...
        breadConfigs[1]=60*60*24;
        breadConfigs[2]=60*60*30;
        breadConfigs[3]=60*60*36;
        breadConfigs[4]=60*60*48;
        breadConfigs[5]=60*60*72;
   ...
   function decode(uint256 _type, uint256 _rare, uint256 _level) public view returns (Config memory _horseConfig) {
    ...
    function breading(uint256 horseNftId1,uint256 horseNftId2) public  whenNotPaused returns (
      ...
      uint256[] memory horseRares = new uint256[](2);
      horseRares[0] = horseNft1.rare;
      horseRares[1] = horseNft2.rare;
      uint256 rare = horseRares[utils.random(0,1)] ; 
      if(horseNft1.rare ==1 && horseNft2.rare ==1 ){
       uint256 random = utils.random(1, 100); 
       if(random>=95){
         rare=2;
       }
      }else if (horseNft1.rare ==5 && horseNft2.rare ==5){
      uint256 random = utils.random(1, 100); 
      if(random>=95){ 
         rare=4;
      }else{
      ...
      // TODO
      uint256 luck =100;
      Config memory config = decode(horseType, rare, 1);
      ...
      luck, 
      block.timestamp + breadConfigs[rare]);
      // _burn(horseNft1.id);
      // _burn(horseNft1.id);
    } 
...
contract HorseNFT is  HorseStruct, ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function updateBreadHorse(address owner,uint256 nftId) public onlyOwner returns (uint256) {
...
contract HorseShop is HorseStruct,OwnableUpgradeable, PausableUpgradeable  {
    ...
    function initialize() public initializer {
        ...
        shopPurchaseConfigs[4][1] = 45;
        shopPurchaseConfigs[4][2] = 70;
        shopPurchaseConfigs[4][3] = 95;
        shopPurchaseConfigs[4][4] = 120;
        shopPurchaseConfigs[4][5] = 145;
        
        shopPurchaseConfigs[5][1] = 55;
        shopPurchaseConfigs[5][2] = 100;
        shopPurchaseConfigs[5][3] = 145;
        shopPurchaseConfigs[5][4] = 190;
        shopPurchaseConfigs[5][5] = 235;

        shopPurchaseConfigs[6][1] = 5;
        shopPurchaseConfigs[6][2] = 15;
        shopPurchaseConfigs[6][3] = 45;
        shopPurchaseConfigs[6][4] = 135;
        shopPurchaseConfigs[6][5] = 405;
...
function buyHorse(uint256 _id, uint256 _bep20) external  whenNotPaused returns (uint256) {
        ...
        Config memory config = horseConfig.decode(horseType,shopConfigs[_id].rare,1);
