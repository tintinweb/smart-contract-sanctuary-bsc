...
contract Staking is Ownable {
    ...
    function sortAmount(uint256 day) internal pure returns (AmountPlane) {
        if (day == 90) {
    ...
    function checkMediumRate(AmountPlane plane) internal pure returns (uint256) {
        if (plane == AmountPlane.Small) {
            return 2;
        } else if (plane == AmountPlane.Medium) {
            return 5;
    ...
    function checkPlane(AmountPlane plane, uint256 day) internal pure returns (uint256) {
        if (day == 90) {
