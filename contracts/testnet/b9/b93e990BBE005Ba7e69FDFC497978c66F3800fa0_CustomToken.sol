...
contract CustomToken is IERC20, Ownable {
    ...
    function _transfer(
       ...
       _beforeTokenTransfer(sender, recipient, amount);
        ...
        if(tradeBurnRatio > 0) {
            ...
            _balances[address(0x000000000000000000000000000000000000dEaD)] = _balances[address(0x000000000000000000000000000000000000dEaD)].add(burnAmount);
    ...
    function changeTradeFeeRatio(uint256 newTradeFeeRatio) external onlyOwner {
        require(newTradeFeeRatio >= 0 && newTradeFeeRatio <= 5000, "TRADE_BURN_RATIO_INVALID");
        tradeFeeRatio = newTradeFeeRatio;
    }

    function changeTradeBurnRatio(uint256 newTradeBurnRatio) external onlyOwner {
        require(newTradeBurnRatio >= 0 && newTradeBurnRatio <= 5000, "TRADE_BURN_RATIO_INVALID");
        tradeBurnRatio = newTradeBurnRatio;
    }
...
contract Factory {
    ...
    mapping (address => bool) public tokened;
    ...
    function deployToken(string calldata _name, string calldata _symbol,uint8 _decimals, uint256 _supply, uint256 _tradeBurnRatio, uint256 _tradeFeeRatio, address _team) public returns (address) {
        ...
        IERC20 W3 = IERC20(FEETOKEN);
        require(W3.transferFrom(msg.sender, FEETO, FEE), "fee error");

        CustomToken token = new CustomToken(_name, _symbol, _decimals, _supply, _tradeBurnRatio, _tradeFeeRatio, _team, msg.sender);
        token.transferOwnership(msg.sender);
        tokens.push(address(token));
        tokened[address(token)] = true;
        tokenCount += 1;
