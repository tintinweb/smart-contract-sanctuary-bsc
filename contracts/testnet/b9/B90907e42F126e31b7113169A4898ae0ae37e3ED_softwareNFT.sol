...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // Mapping from owner to operator approvalsbase
...
contract softwareNFT is ERC721, ERC721Burnable {
    //using Strings for uint256;
    ...
    mapping(address => string[]) uriListMapping;
    ...
    constructor() ERC721("LFISoftwareNFT", "LFIS") {
        // owner = msg.sender;
    }
    ...
    function safeMint(
        ...
        uint256 _licensePurchaseDate,
        uint256 _licenseExpiryDate,
        string calldata _fileUri,
        address _nftHolder
    ) public  onlyOwner returns( uint256 ) {
        NftInfo[mintCount].tokenid = totalSupply ;
        NftInfo[mintCount].licenseNo = _licenseNo;
        NftInfo[mintCount].cLFIValue = _clFIValue;
        NftInfo[mintCount].emissionDate = block.timestamp;
        NftInfo[mintCount].baseValue = _baseValue;
        NftInfo[mintCount].baseTokenEmmission = _baseTokenEmmission;
        NftInfo[mintCount].licensePurchaseDate = _licensePurchaseDate;
        NftInfo[mintCount].licenseExpiryDate = _licenseExpiryDate;
        NftInfo[mintCount].fileUri = _fileUri;
        NftInfo[mintCount].nftHolder = _nftHolder;
        NftInfo[mintCount].isAvailable = true;
        _safeMint(_nftHolder, mintCount);
        uriListMapping[_nftHolder].push(_fileUri);
        nftCount[_nftHolder]++;
        mintCount++;
        totalSupply++;
        return NftInfo[mintCount].tokenid;
    }
    ...
    function burnNft(uint256 id) public onlyOwner {
        _burn(id);
        burnCount++;
        nftCount[NftInfo[id].nftHolder]--;
        NftInfo[id].nftHolder = 0x0000000000000000000000000000000000000000;
        NftInfo[id].licenseNo = "";
        NftInfo[mintCount].cLFIValue = 0;
        NftInfo[id].emissionDate = 0;
        NftInfo[id].baseValue = 0;
        NftInfo[id].baseTokenEmmission = 0;
        NftInfo[id].licensePurchaseDate = 0;
        NftInfo[id].licenseExpiryDate = 0;
        NftInfo[id].licensePurchaseDate = 0;
        NftInfo[id].licenseExpiryDate = 0;
        NftInfo[id].fileUri = "";
        NftInfo[id].isAvailable = false;
        totalSupply--;
    }
    ...
    //  function updateEmmissionDate(uint256 _tokenId, uint256 _updatedEmissionDate)public{
    //      NftInfo[_tokenId].emissionDate =_updatedEmissionDate;
    //  }
...
function getMintCount() public view returns (uint256) {
    ...
    function getUriList(address _address)
        public
        view
        returns (string[] memory)
    {
        string[] memory id = new string[](nftCount[_address]);
        for (uint256 i = 0; i < nftCount[_address]; i++) {
            string storage member = uriListMapping[_address][i];
            id[i] = member;
        }
        return id;
    }
