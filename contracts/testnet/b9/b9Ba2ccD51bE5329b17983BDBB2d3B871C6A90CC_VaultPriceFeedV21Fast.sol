...
contract VaultPriceFeedV21Fast is IVaultPriceFeedV3, Ownable {
    ...
    function setPricesWithBitsSingle(address _token, uint256[] memory _priceBits, uint256 _timestamp) external onlyUpdater {
    ...
    function _setPricesWithBitsSingle(address _token, uint256[] memory _priceBits, uint256 _timestamp) private {
        uint256 roundId = _batchRoundId.current();
        _batchRoundId.increment();
        fastTimeStamp = _timestamp;

        uint256 priceBits = _priceBits[0];
        uint256 tokenPrecision = tokenPrecisions[priceIndexLoc[_token]];
        require(tokenPrecision > 0, "not valid token");
        require(fastTimeStamp >= latestPriceFeedTime[_token], "data out of time");

        uint256 price = (priceBits >> 0) & BITMASK_32;
        uint256 adjustedPrice = price.mul(PRICE_PRECISION).div(tokenPrecision);
        prices[_token] = adjustedPrice;
        latestPriceFeedTime[_token] = fastTimeStamp;
        emit PriceUpdated(_token, adjustedPrice, roundId);
    }
