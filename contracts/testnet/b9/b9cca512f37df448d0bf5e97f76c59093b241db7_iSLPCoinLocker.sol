// created by isWorks
// certain functions credit to Skeleton Codeworks, and BokkyPooBahs
...
contract iSLPCoinLocker is OwnerAdminSettings {
    ...
    bool internal doShowReleaseTime = false;
    uint256 internal showReleaseTimeCalledTime;

    uint256 releaseTime;

    event ShowReleaseTime(address indexed Requester, bool indexed Show, uint256 indexed showReleaseTimeCalledTime);
  ...
  //Allows to unmask Release Time on web3 read/contract calls.

  function showReleaseTime(bool Show) external onlyOwnerNAdmins {
    require(Show || !Show, "True = Unmask Owner & Admin Addresses. False = Mask Owner & Admin Addresses.");
    doShowReleaseTime = Show;
    showReleaseTimeCalledTime = block.timestamp;
    emit ShowReleaseTime(msg.sender, Show, showReleaseTimeCalledTime);
  }

  //Lock Functions

    //Set or Extend Lock in YMDHMS format.
    function extendLockInYMDHMS(uint year, uint month, uint day, uint hour, uint minute, uint second) external onlyOwner {
        require(year >= 2022, "Year must be greater than or equal to 2022");
        ...
        uint256 newReleaseTime = _daysFromDate(year, month, day) * SECONDS_IN_DAY + hour * SECONDS_IN_HOUR + minute * SECONDS_IN_MINUTE + second;        
    ...
    //Set or Extend Lock
    function extendLockInTimestamp(uint256 newReleaseTime) external onlyOwner {
        require(newReleaseTime > block.timestamp, "Release time must be in the future");
        require(newReleaseTime > releaseTime, "Can't make release time shorter");
    ...
    function distributeTokens(IBEP20TokenTransfer token) external onlyOwner {
        ...
        require(Admin1Sign, "Admin1 Did not sign!");
        require(Admin2Sign, "Admin2 Did not sign!");
        require(Admin3Sign, "Admin3 Did not sign!");
    ...
    //Only shows ReleaseTime for 2 minutes after the Owner and Admin called the unmask function above.
    function whenIsReleaseTimestamp() external view returns (uint256) {
        if (doShowOwnerNAdmin && block.timestamp < showOwnerNAdminCalledTime + 120) {
            return releaseTime;
        } else {
        return 9999999999; 
        }
