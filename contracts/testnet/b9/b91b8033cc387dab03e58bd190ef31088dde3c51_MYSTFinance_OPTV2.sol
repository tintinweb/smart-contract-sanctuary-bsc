    ...
    ReentrancyGuardUpgradeable
{
    ...
    function _deposit(
        ...
        // transfer development fee to dev wallet
    ...
    // claim all rewards of user according to mst stake apr
    ...
    function claim(address investor, uint256 _apr) internal {
        require(
            ownedDeposits[investor].length > 0,
        ...
        uint256 allClaimableAmount = (lastRoiTime *
            investors[investor].totalLocked *
        ...
        if (
            investors[investor].claimedAmount + amountToSend >
            investors[investor].maxClaimableAmount
        ) {
            amountToSend = investors[investor].maxClaimableAmount.sub(
        ...
        IERC20Upgradeable(BUSDContract).safeTransfer(investor, amountToSend);
        if (_apr == 150 || _apr == 125 || _apr == 100) {
            IERC20Upgradeable(MSTContract).safeTransfer(investor, amountToSend);
        }
    ...
    function compoundAllReward() public nonReentrant {
        ...
        _deposit(amountToSend, _apr, developerFee);
        totalReward = totalReward.add(amountToSend);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 curReward = getallClaim(_investor, _apr);
        return curReward;
    }

    function getallClaim(address _investor, uint256 _apr)
        internal
        view
        returns (uint256)
    {
    ...
    // MXST Stake
    function stakeMSTToken(uint256 stakeAmount) external payable {
        require(stakeAmount > 0, "Stake amount should be correct");
        require(
            IERC20Upgradeable(MSTContract).balanceOf(_msgSender()) >=
                stakeAmount,
            "Insufficient Balance"
        );
        uint256 msxstAmount = stakeInfos[msg.sender].amount + stakeAmount;

        if (msxstAmount >= 50000e18 && msxstAmount <= 500000e18) {
            uint256 _apr = 75;

            // get remaing time of withdrawal period
            uint256 totalTime = (investors[msg.sender].startTime +
                withdrawPeriod) - block.timestamp;
            // calculate APR of next remaing period with new APR
            uint256 maxAmount = (totalTime *
                (investors[msg.sender].totalLocked) *
                _apr).div(percentRate * rewardPeriod);
            // get All Claim
            uint256 currentClaim = getAllClaimableReward(msg.sender);
            // Function Called for Claiming
            claimAllReward();
            // Add previous and current Max
            uint256 totalClaim = currentClaim + maxAmount;
            // Assign Total Claim to Max Claim
            investors[msg.sender].maxClaimableAmount = totalClaim;
        ...
        IERC20Upgradeable(MSTContract).transferFrom(
            _msgSender(),
            address(this),
            stakeAmount
        );
        ...
        stakeInfos[_msgSender()] = StakeInfo({
            ...
            endTS: block.timestamp + withdrawPeriod,
            amount: stakeAmount + stakeInfos[msg.sender].amount,
            claimed: 0
        });
    ...
    function withdrawMxst() external returns (bool) {
        require(
            addressStaked[_msgSender()] == true,
        ...
        IERC20Upgradeable(MSTContract).transfer(_msgSender(), stakeAmount);
    ...
    function setMST(address _mst) public onlyOwner {
        MSTContract = _mst;
    }
    ...
    {
        uint256 stakedAmnt = stakeInfos[_user].amount;
