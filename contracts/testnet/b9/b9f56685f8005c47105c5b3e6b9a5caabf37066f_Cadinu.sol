...
contract Cadinu is Icadinu, Context, Ownable {
    ...
    mapping (address => TimeLs[]) private _timeLocks;
    mapping (address => bool) private _whaleList;
    ...
    modifier preventTimeLock(address _sender, uint256 _value){
        uint256 _locked_value = timeLockValue(_sender);
        if(_locked_value > 0){
            uint256 _usableBalance = balanceOf(_sender) - _locked_value;
            require(_usableBalance > 0, "TimeLock: Full locked");
            require(_usableBalance >= _value, "TimeLock: Balance was locked");
        }
        _;
    }

    modifier preventAntiwhale(address _from,address _to, uint256 _value){
        if(_maxBalance > 0 && _whaleList[_to] == false){
            uint256 _whale_balance = balanceOf(_to) + _value;
            require(_whale_balance < _maxBalance, "Antiwhale: Reciever balance exceeds the Max Balance.");
        }
        _;
    }
    ...
    function __Cadinu_tiers_init() internal {
    ...
    function setMaxBalance(uint256 _value) public onlyOwner returns (bool){
        _maxBalance = _value;
        return true;
    }

    function addWhale(address _address) public onlyOwner returns (bool){
        require(_whaleList[_address] == false, "Address is whale");
        _whaleList[_address] = true;
        return true;
    }

    function removewhale(address _address) public onlyOwner returns (bool){
        require(_whaleList[_address] == true, "Address is not whale");
        _whaleList[_address] = false;
        return true;
    }

    function multiTransfer(address[] memory _recipients, uint[] memory _values) public returns (bool) {
        _multiTransfer(_msgSender(), _recipients, _values);
        return true;
    }

    function airTransfer(address[] memory _recipients, uint256 _value,uint256 _lockUntil) public returns (bool) {
        uint256 min_balance = _recipients.length * _value;
        require(min_balance < balanceOf(_msgSender()), "Address balance is lower than transaction amount");
        _airTransfer(_msgSender(), _recipients, _value, _lockUntil);
        return true;
    }
    ...
    function _addTier(
        ...
        if(!isExcludedFromReward(_ecoSystem)){
            excludeFromReward(_ecoSystem);
        }
        if(!isExcludedFromReward(_owner)){
            excludeFromReward(_owner);
        }
    ...
    private
    preventTimeLock(spender, amount)
    preventAntiwhale(spender, owner, amount)
    {
    ...
    private
    preventTimeLock(from, amount)
    preventAntiwhale(from, to, amount)
    isRouter(_msgSender())
    {
    ...
    function _multiTransfer(address from, address[] memory _recipients, uint[] memory  _values) private {
        //Disable Swap per Transaction
        bool _swap_status = swapAndLiquifyEnabled;
        if(_swap_status == true){
            swapAndLiquifyEnabled = false;
        }

        require(_recipients.length == _values.length, "Recipients length must be equals values length");
         for (uint i = 0; i < _recipients.length; i++) {
             _transfer(from, _recipients[i], _values[i]);
         }
        //restore swap status
        swapAndLiquifyEnabled = _swap_status;
    }

    function _airTransfer(address from, address[] memory _recipients, uint _value, uint256 _timestamp) private{
        //Disable Swap per Transaction
        bool _swap_status = swapAndLiquifyEnabled;
        if(_swap_status == true){
            swapAndLiquifyEnabled = false;
         ...
         for (uint i = 0; i < _recipients.length; i++) {
             _tokenTransfer(from,_recipients[i], _value, 0, false);
             if(_timestamp > 0){
                timeLock(_recipients[i], _value, _timestamp);
             }
         }
         //restore swap status
        swapAndLiquifyEnabled = _swap_status;
    ...
    function timeLock(address _account, uint256 _value, uint256 _timestamp) public onlyOwner {
        TimeLs memory _lock = TimeLs(_timestamp, _value);
        uint256 available_balance = balanceOf(_account) - timeLockValue(_account);
        require(available_balance >= _value, "TimeLock: Account balance is lower than value");
        _timeLocks[_account].push(_lock);
    }

    function timeUnLock(address _account, uint256[] memory _index) public onlyOwner {
        for (uint256 i = 0; i < _index.length; i++) {
            if(_timeLocks[_account].length > 1){
                _timeLocks[_account][i] = _timeLocks[_account][_timeLocks[_account].length - 1];
                _timeLocks[_account].pop();
            }else{
                _timeLocks[_account].pop();
            }
        }
    ...
    function timeLockInfo(address _account, uint256 _lockIndex) public view returns (TimeLs memory) {
        return _timeLocks[_account][_lockIndex];
    }

    function timeLockLength(address _account) public view returns(uint256){
        return _timeLocks[_account].length;
    }

    function timeLockValue(address _sender) public view returns(uint256 _locked_value) {
        TimeLs[] memory _locks = _timeLocks[_sender];
        uint256 _locked = 0;
        if(_locks.length > 0){
            for (uint i = 0; i < _locks.length; i++) {
                if(_locks[i].unlockAt >= block.timestamp){
                    _locked += _locks[i].value;
                }
            }
        }
        return _locked;
