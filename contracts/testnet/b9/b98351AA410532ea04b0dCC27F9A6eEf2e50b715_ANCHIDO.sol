...
contract ANCHIDO {
    ...
    uint256 public p1Period = 120 days;
    ...
    uint256 public p2Period = 90 days;
    ...
    function swap(uint256 amount) public {
        ...
        if(block.timestamp >= p1StartTime && block.timestamp <= p1FinishTime) {
            ...
            userBuyAmount[msg.sender][0] = userBuyAmount[msg.sender][0].add(tokenAmount);
            ...
            userBuyAmount[msg.sender][1] = userBuyAmount[msg.sender][1].add(tokenAmount);
    ...
    function getReleaseTokenAmount(address account) public view returns (uint256){
        ...
        if(block.timestamp >= onlineTime) {
            ...
            uint256 time = (block.timestamp.sub(onlineTime)).div(1 days);

            if(userBuyAmount[account][0] > 0) {
                if(time > p1Period)
                    time = p1Period;
                releaseToken = releaseToken.add(userBuyAmount[account][0].div(100).mul(10));
                releaseToken = releaseToken.add(userBuyAmount[account][0].div(percent).mul(p1DayRelease));
            }

            if(userBuyAmount[account][1] > 0) {
                if(time > p2Period)
                    time = p2Period;
                releaseToken = releaseToken.add(userBuyAmount[account][1].div(100).mul(10));
                releaseToken = releaseToken.add(userBuyAmount[account][1].div(percent).mul(p2DayRelease));
            }
    ...
    function getSwapTokenAmount(address account) public view returns (uint256){
        ...
        return userBuyAmount[account][0].add(userBuyAmount[account][1]);
