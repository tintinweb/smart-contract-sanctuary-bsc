...
contract SI is Ownable {
   ...
   address[] public teamMeron;
   address[] public teamWala;
   address add;
   address meronAndWala;
...
// The address of the player and => the user info
   ...
   function() public payable {}
    ...
    function bet(uint256 _gameId, uint8 _teamSelected, uint256 amount) public  {
            ...
            if ( _teamSelected == 1){
                totalBetsOne += amount;
                teamMeron.push(msg.sender);
                players.push(msg.sender);
            }
            else if(_teamSelected == 2){
                totalBetsTwo += amount;
                teamWala.push(msg.sender);
                players.push(msg.sender);
            }
    ...
    function allocatePrizes(uint256 _gameId, uint16 teamWinner) public onlyAgent {
        ...
        "Meron is = 1, Wala is = 2, draw/cancelled = 3");
        ...
        for(i = 0; i < teamMeron.length; i++){
            // Check that the address in this fixed array is not empty
            meronAndWala = teamMeron[i];
            winners[i] = meronAndWala;
            ...
            totalBets = totalBetsOne + totalBetsTwo;
        //We loop through the array of winners, to give ethers to the winners
        for(i = 0; i < teamWala.length; i++){
            // Check that the address in this fixed array is not empty
            meronAndWala = teamWala[i];
            winners[i] = meronAndWala;
            if(winners[i] != address(0)){
             add = winners[i];
             bets = playerInfo[add].amountBet;
