...
contract DaoStake is CreditMarket {
    ...
    function requestWithdraw(address token, uint256 amount) internal whenNotPaused nonReentrant {
        ...
        uint256 burnPapers = withdrawDdvValue
            .mul(accountInfo[msg.sender].papers)
            .mul(daoAssets[token].papersPerDDV)
            .div(accountInfo[msg.sender].ddv)
            .div(1e18);
        // UserBurnShares = WithdrawDdvValue * UserShares * WithdrawTokenSharesPerDDV / UserDdv
        uint256 burnShares = withdrawDdvValue
            .mul(accountInfo[msg.sender].shares)
            .mul(daoAssets[token].sharesPerDDV)
            .div(accountInfo[msg.sender].ddv)
            .div(1e18);
