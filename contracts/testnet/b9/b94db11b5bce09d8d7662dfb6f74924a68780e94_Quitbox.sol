...
contract Quitbox is Context, IERC20 { 
    ...
    */
    ...
    address payable public Wallet_Developer = payable(0x0123447F813F9AB49cf749ad57CdD2051A6C4604);
    ...
    event updated_Wallet_Developer(address indexed oldWallet, address indexed newWallet);
    ...
    */
    ...
    // Update the developer Wallet - Solidity developer
    function Update_Wallet_Developer(address payable wallet) external {
        require(wallet != address(0), "Can not be zero address");
        require(msg.sender == Contract_Developer, "Only developer can update");
        emit updated_Wallet_Developer(Wallet_Developer,wallet);
        Wallet_Developer = wallet;
    }
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Reduce solidity rounding errors 
        uint256 accuracy = 1000000;

        // Get fee splits
        uint256 percent_L = _FeeLiquidity * accuracy / _FeesTotal;
        uint256 percent_D = _FeeDeveloper * accuracy / _FeesTotal;
        uint256 percent_M = _FeeMarketing * accuracy / _FeesTotal;

        // Calculate number of tokens that need to be swapped
        uint256 token_half = contractTokenBalance * percent_L / accuracy / 2;
        uint256 swapTokens = contractTokenBalance - token_half;
        ...
        // Add liquidity 
        if (percent_L > 0){
            uint256 bnb_half = returned_BNB - ((returned_BNB * percent_D / accuracy) + (returned_BNB * percent_M / accuracy));
            addLiquidity(token_half, bnb_half);
            emit SwapAndLiquify(token_half, bnb_half, token_half);
        }

        // Take developer fee
        sendToWallet(Wallet_Developer, (returned_BNB * percent_D / accuracy));
