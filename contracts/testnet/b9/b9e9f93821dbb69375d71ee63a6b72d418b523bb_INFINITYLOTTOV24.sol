...
contract INFINITYLOTTOV24 is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(isStakingContract[recipient] && !isFeeExempt[sender]){    // if TO the Pool and sender NOT exempt. charge tax
        if(isStakingContract[sender] && !isFeeExempt[recipient]){    // if FROM the Pool and receiver not exempt. Charge tax
