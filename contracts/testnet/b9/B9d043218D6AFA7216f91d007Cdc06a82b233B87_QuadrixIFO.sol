 ...
 */
contract QuadrixProfile is AccessControl, ERC721Holder {
    ...
    uint256 public numberQuadrixToReactivate;
    uint256 public numberQuadrixToRegister;
    uint256 public numberQuadrixToUpdate;
    ...
    constructor(
        IBEP20 _QuadrixToken,
        uint256 _numberQuadrixToReactivate,
        uint256 _numberQuadrixToRegister,
        uint256 _numberQuadrixToUpdate
    ) public {
        QuadrixToken = _QuadrixToken;
        numberQuadrixToReactivate = _numberQuadrixToReactivate;
        numberQuadrixToRegister = _numberQuadrixToRegister;
        numberQuadrixToUpdate = _numberQuadrixToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends Quadrix to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        QuadrixToken.safeTransferFrom(_msgSender(), address(this), numberQuadrixToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer Quadrix token to this address
        QuadrixToken.safeTransferFrom(_msgSender(), address(this), numberQuadrixToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        QuadrixToken.safeTransferFrom(_msgSender(), address(this), numberQuadrixToReactivate);
    ...
    /**
     * @dev Claim Quadrix to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        QuadrixToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of Quadrix to register
     * Callable only by owner admins.
     */
    function updateNumberQuadrix(
        uint256 _newNumberQuadrixToReactivate,
        uint256 _newNumberQuadrixToRegister,
        uint256 _newNumberQuadrixToUpdate
    ) external onlyOwner {
        numberQuadrixToReactivate = _newNumberQuadrixToReactivate;
        numberQuadrixToRegister = _newNumberQuadrixToRegister;
        numberQuadrixToUpdate = _newNumberQuadrixToUpdate;
    }
 ...
 */
contract QuadrixIFO is IIFOV2, ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;
    ...
    QuadrixProfile public quadrixProfile;
    ...
    function initialize(
        ...
        address _QuadrixProfileAddress,
        uint256 _startBlock,
        uint256 _endBlock,
        ...
        quadrixProfile = QuadrixProfile(_QuadrixProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(quadrixProfile.getUserStatus(msg.sender), "Deposit: Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                quadrixProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
