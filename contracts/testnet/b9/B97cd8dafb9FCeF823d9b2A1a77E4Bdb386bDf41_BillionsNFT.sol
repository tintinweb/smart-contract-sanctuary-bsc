...
contract BillionsNFT is ERC4907, BillionsBaseContract, ReentrancyGuard {
    ...
    mapping(uint256 => bool) public usedCompanyId;
    mapping(address => uint256[]) public nftIdsOfPlayer;

    uint256[] public availableCompanyIds;
    ...
    event _BillionsNftMint(uint256 _nftId, uint256 _companyId);
    ...
    external payable Mintable returns (uint256) {
        ...
        require(!usedCompanyId[_companyId], "not available");

        uint256[] memory companyIds = availableCompanyIds;
        uint256 _availableCount = companyIds.length;
        ...
        usedCompanyId[_companyId] = true;
        nftIdsOfPlayer[msg.sender].push(_nftId);

        for(uint256 i = 0; i < _availableCount; i ++) {
            if(companyIds[i] == _companyId) {
                availableCompanyIds[i] = availableCompanyIds[_availableCount - 1];
                availableCompanyIds.pop();

                break;
            }
        }

        emit _BillionsNftMint(_nftId, _companyId);
        return _nftId;
    }

    function mintByERC20(uint256 _companyId) 
    ...
    function mintByPack() public  {
        uint256 _availableCount = availableCompanyIds.length;
        ...
        for(uint256 idx = 0; idx < 3; idx ++) {
            _rand = random(_rand);
            _availableCount = availableCompanyIds.length;
            uint256 _companyId = availableCompanyIds[_rand % _availableCount];
            ...
            availableCompanyIds[_rand % _availableCount] = availableCompanyIds[_availableCount - 1];
            availableCompanyIds.pop();
    ...
    function clearAvailableCompanyIds() external onlyOwner {
        delete availableCompanyIds;
    }

    function setAvailableCompanyIds(uint256[] memory _ids) external onlyOwner {
        availableCompanyIds = _ids;
    }

    function addAvailableCompanyId(uint256 _id) external onlyOwner {
        availableCompanyIds.push(_id);
    }
