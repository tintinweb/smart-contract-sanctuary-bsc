...
interface IIboV2Concluder {
    ...
    function initialize0(address _ibo, address _raisingToken, address _offeringToken, address _owner) external;
    // function initialize(...) external;   // This has dynamic arguments
    function conclude() external;
}

contract SimpleSendingRaisingTokensIboV2Concluder is IIboV2Concluder {
    using SafeMath for uint256;
    ...
    constructor() public {
        _name = "SimpleSendingRaisingTokens";
        _status = 0;
        _isInitialized = false;
    }

    function initialize0(address __ibo, address __raisingToken, address __offeringToken, address __owner) external override {
    ...
    function initialize(address _sendTo) external {
        require(!_isInitialized, 'Already initialized!');
...
contract SimpleSendingRaisingTokensIboV2ConcluderTemplate is IIboV2ConcluderTemplate {
    ...
    function createConcluder(address ibo, address raisingToken, address offeringToken, address owner, bytes memory callData) external override returns (address concluder) {
        bytes memory bytecode = type(SimpleSendingRaisingTokensIboV2Concluder).creationCode;
        ...
        SimpleSendingRaisingTokensIboV2Concluder(concluder).initialize0(ibo, raisingToken, offeringToken, owner);

        (bool success, ) = concluder.call(abi.encodePacked(bytes4(keccak256(bytes("initialize(address)"))), callData));
