...
contract Zzz is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "Zzz Token";
    string public _symbol = "ZZZ";
    ...
    uint256 public zzzInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(zzzInsuranceFundFee).add(firePitFee);
    ...
    address public zzzInsuranceFundReceiver;
    ...
    uint256 private _getCirculatingSupply;
    ...
    constructor() ERC20Detailed("Zzz", "ZZZ", uint8(DECIMALS)) Ownable() {
        //testnet
        ...
        treasuryReceiver = 0xCe2B0000731CE5926A060DbE276ED519decEF523;
        zzzInsuranceFundReceiver = 0x5280BD7D35b73C1f4559086F429D5dDe0EA6A624;
        firePit = DEAD;
    ...
    function _transferFrom(
        ...
        if (!excludedAccount) {
            require(
                ...
                "ERR: Can not sell more than 1% of the Circulating Supply"
            );
            uint256 blkTime = block.timestamp;
            uint256 onePercent = balanceOf(sender).mul(1).div(100);
            require(
                amount <= onePercent,
                "ERR: Can not sell more than 1% in One day"
            );
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(zzzInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(treasuryFee).div(
                treasuryFee.add(zzzInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(zzzInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(zzzInsuranceFundFee).div(
                treasuryFee.add(zzzInsuranceFundFee)
            ),
            gas: 30000
        }("");
        ...
        onlyOwner
    {
        ...
        require(
            ...
            "cannot blacklist zzz Token, Pair and Router PancakeSwap"
        );
    ...
    // function setLaunchDate(uint256 initrebaseStartTime, uint256 initRebaseTime)
    //     public
    //     onlyOwner
    // {
    //     require(
    //         block.timestamp < _initRebaseStartTime &&
    //             block.timestamp < _lastRebasedTime,
    //         "Already rebase started"
    //     );
