...
contract IdoProjectV2 is ReentrancyGuard, Initializable {
    ...
    modifier isWithdrawableAdmin() {
        bool check = block.timestamp > calculationTime && !isWithdrawAdmin;
        require(check, "!check");
        _;
    }
    ...
    //=================== External ===================== //
    function registerProject(bytes memory _signature, uint256 _slot, uint _userLevel) external onPoolSale checkSigner(_signature, _slot, _userLevel) {
        if(allowList.contains(msg.sender)) {
            require(!winners.contains(msg.sender), "User already register");
            uint _decimals = Decimal(address(tokenSale)).decimals();
            currency.safeTransferFrom(msg.sender, address(this), maxAllocation);
            _execute(msg.sender, maxAllocation, _decimals);
        } else {
            ...
            emit Register(msg.sender, projectId, commit[msg.sender], _slot, _userLevel);
        }
    }
    ...
    function setting(
        ...
        require(_dataUint.length == 6, "Not enough");
        ...
        require(_dataUint[4] >= _dataUint[5], "Total slot less than allow list");
        ...
        totalAllowList = _dataUint[5]; // total allow list
        _setCliffInfo(_timestamps, _percentages, _update);
        settingProject = true;
        emit Setting(projectId, _update, registerStartTime, calculationTime, rate, address(currency), address(tokenSale), admin, maxAllocation, _timestamps, _percentages, totalSlot, totalAllowList);
    }
    ...
    function addAllowList(address[] memory _users) external onlyOperator {
        require(_users.length > 0, "_users empty");
        require(_users.length + allowList.length() <= totalAllowList, "_users empty");
    ...
    function submitWinners(address[] memory _users) external onlyOperator {
        ...
        //set total slot
        require(winners.length() + _users.length + totalAllowList <= totalSlot, "Slot over flow");
        uint _decimals = Decimal(address(tokenSale)).decimals();
        for(uint i = 0; i < _users.length; i++ ){
            ...
            uint _decimals = Decimal(address(tokenSale)).decimals();
            _execute(_user, _allocation, _decimals);
        }
        if(winners.length() + totalAllowList == totalSlot) {
            poolEnd = true;
            emit PoolEnd(poolEnd, projectId);
        }
    }
    ...
    //=================== internal ======================//
    function _execute(address _user, uint _allocation, uint _decimals) internal {
        // set claim info
        ClaimInfo storage _data = claimInfo[_user];
        _data.totalToken = _allocation * 10 ** _decimals / rate;
    ...
    function getProgress() external view returns (uint _numerator, uint _denominator) {
        ...
        if (rate > 0) {
            ...
            _denominator = totalSlot * maxAllocation / rate; //total token receive
        }
    }
    ...
    function isAllowList(address _user) external view returns(bool) {
