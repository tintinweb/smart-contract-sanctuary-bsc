...
contract MrFoxNFT is Initializable,ERC721Upgradeable,ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
    ...
    uint256 internal _royaltyRootPercent; 
    uint256 internal _royaltyCompanyPercent;  
    uint256 internal _royaltyOwnerPercent; 


    function initialize(string memory baseTokenURI,address addressAdmin) initializer public {
        __ERC721_init("MrFox NFT", "FOX NFT");
    ...
    function mintTimeOf(uint256 tokenId) public virtual view returns (uint256) {
        _existsToken(tokenId);
    ...
    function setRoyaltyRootPercent(uint256 newRoyaltyRootPercent) public virtual onlyRole(DEFAULT_ADMIN_ROLE){
        _royaltyRootPercent = newRoyaltyRootPercent;
    }

    function royaltyRootPercent() public view virtual returns (uint256){
        return _royaltyRootPercent;
    }

    function setRoyaltyCompanyPercent(uint256 newRoyaltyCompanyPercent) public virtual onlyRole(DEFAULT_ADMIN_ROLE){
        _royaltyCompanyPercent = newRoyaltyCompanyPercent;
    }

    function royaltyCompanyPercent() public view virtual returns (uint256){
        return _royaltyCompanyPercent;
    }

    function setRoyaltyOwnerPercent(uint256 newRoyaltyOwnerPercent) public virtual onlyRole(DEFAULT_ADMIN_ROLE){
        _royaltyOwnerPercent = newRoyaltyOwnerPercent;
    }

    function royaltyOwnerPercent() public view virtual returns (uint256){
        return _royaltyOwnerPercent;
    }
