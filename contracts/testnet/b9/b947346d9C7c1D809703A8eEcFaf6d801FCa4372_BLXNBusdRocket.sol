...
contract BLXNBusdRocket is Context, Ownable, ReentrancyGuard {
    ...
    address constant devWallet = 0x2dA3008C98D3a251f759c5D239C28d10412628DB;
    address constant ideaWallet = 0xDc4C89455c63ee1301c9d10C3516EFBFD0011d19;
    address constant fundingWallet = 0x5a1a6D781562f505ED8CAE238760bddf21DE58Ad;
    address public tokenAdress;
    uint256 firstlaunchTime = 1661979600; 
    uint256 secondlaunchTime = 1662066000; 
    ...
    function MakeDeposit(uint256 amtx, address reffy) public noReentrant {
        ...
        TokenInterface.safeTransferFrom(msg.sender, address(this), amtx);
        TokenInterface.safeTransfer(devWallet, devFee);
        TokenInterface.safeTransfer(fundingWallet, fundingFee);
    ...
    function MakeClaim() public noReentrant {
        ...
        if (user.claimCount > 10) {
            ...
            // If you've claimed more than 10 times, your daily is 14%
        }
        if (user.claimCount > 15) {
            userDeps = user.totalDeps * 160;
            // If you've claimed more than 15 times, your daily is 16%
        }
        if (user.claimCount > 20) {
            userDeps = user.totalDeps * 180;
            // If you've claimed more than 20 times, your daily is 18%
        }
    ...
    function MakeWithdraw() public noReentrant {
        ...
        if (transferAmt + user.totalWithdrawn > user.maxPayout) {
            ...
            TokenInterface.safeTransfer(msg.sender, userNet);
            TokenInterface.safeTransfer(devWallet, devFee);
            TokenInterface.safeTransfer(ideaWallet, ideaFee);
            TokenInterface.safeTransfer(fundingWallet, fundingFee);
    ...
    function MakeRefWithdraw() public noReentrant {
        ...
        TokenInterface.safeTransfer(msg.sender, transferAmt);
