...
contract PISTON is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    string _symbol;
    bool private inSwap;
    bool public addedInitialLiquidity;
    ...
    function initialize() public virtual initializer {
        ...
        busdToken = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);

        uniswapV2Router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        uniswapV2Pair = IDEXFactory(uniswapV2Router.factory()).createPair(address(this), busdToken);
        bnbPair = IDEXFactory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());

        idexPair = IDEXPair(uniswapV2Pair);

        _setAutomatedMarketMakerPair(uniswapV2Pair, true);
        _setAutomatedMarketMakerPair(bnbPair, true);

        IERC20(busdToken).approve(address(uniswapV2Router), type(uint256).max);
        IERC20(busdToken).approve(address(this), type(uint256).max);
        IERC20(busdToken).approve(address(uniswapV2Pair), type(uint256).max);
        ...
        _allowedFragments[address(this)][bnbPair] = type(uint256).max;
        ...
        _mint(address(this), 500000 * (10**18));
    }
    ...
    function manualSync() public {
        InterfaceLP(uniswapV2Pair).sync();
    }
    ...
    function _transferFrom(address from, address to, uint256 amount) internal returns (bool) 
    {
        ...
        if(from == owner() && addedInitialLiquidity == false)
        {
            uint256 busdBalance = IERC20(busdToken).balanceOf(address(this));
            uint256 wbnbBalance = address(this).balance;
            uint256 tokenBalance = IERC20Upgradeable(address(this)).balanceOf(address(this));

            _addLiquidityBusd((tokenBalance.div(2)), busdBalance);
            _addLiquidityWBNB((tokenBalance.div(2)), wbnbBalance);
        }

		uint256 contractTokenBalance = IERC20Upgradeable(address(this)).balanceOf(address(this));
        ...
        if(!swapping &&
          canSwap &&
            ...
            {
                if (addedInitialLiquidity && idexPair.kLast() != 0)
                {
    ...
    function swapAndHandleFee(uint256 tokens) private {

        swapAndLiquify(tokens); 

        uint256 contractBusdBalance = IERC20(busdToken).balanceOf(address(this));

        if (contractBusdBalance > 0) {
            
            if (devFee > 0)
            {
                uint256 devTokens = contractBusdBalance.mul(devFee).div(totalFees);
                IERC20(busdToken).transfer(_devWalletAddress, devTokens);
            }
            if (ecosystemFee > 0)
            {
                uint256 ecosystemTokens = contractBusdBalance.mul(ecosystemFee).div(totalFees);
                IERC20(busdToken).transfer(_ecosystemWalletAddress, ecosystemTokens);
            }
        }
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        _addLiquidityBusd(otherHalf, newBalance.div(2));
    ...
    function _addLiquidityWBNB(uint256 tokenAmount, uint256 bnbAmount) private {
