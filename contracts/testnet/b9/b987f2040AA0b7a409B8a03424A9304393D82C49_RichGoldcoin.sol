...
contract RichGoldcoin is Context, IERC20, Ownable {
    ...
    uint256 public dexSwapFee = 1000; //take fee while sell token to dex
    ...
    function _transfer(
        ...
        if(to==uniswapV2Pair&&takeFee){
            uint256 swapFee = amount.mul(dexSwapFee).div(10000);
            _tOwned[swapAddress] = _tOwned[swapAddress].add(swapFee);
            _rOwned[swapAddress] = _rOwned[swapAddress].add(swapFee);
            emit Transfer(from, swapAddress, swapFee);
            amount = amount.sub(swapFee);
        }
        
        _tOwned[from] = _tOwned[to].add(amount);
        _rOwned[from] = _rOwned[to].add(amount);
