...
contract PrivateRound is AccessControl {
    ...
    uint256 public rate;
    uint256 public SharkTotalAmount;
    ...
    * @param _SharkToken address of Shark token contract
    ...
    */
    function swap(uint256 _amount) external {
        ...
        uint256 SharkAmount = _amount * ratesPrecision / rate;
        require(SharkTotalAmount - SharkAmount >= 0, "claimed amount exceed total amount");//
        ERC20(USDTAddress).safeTransferFrom(msg.sender, usdtReceiver, _amount);
        UserInfo storage user = users[msg.sender];
        usdtReceived += _amount;
        user.amount = user.amount + SharkAmount;
        SharkTotalAmount = SharkTotalAmount - SharkAmount;
        emit TokenExchanged(msg.sender, _amount, SharkAmount);
    }
    ...
    */
    function claim() external {
        ...
        ERC20(SharkToken).safeTransfer(msg.sender, availableAmount);
    ...
    */
    function migrateUsers(
        ...
        for (uint256 i = 0; i < _users.length; i++) {
            require(SharkTotalAmount - _amounts[i] >= 0,  "Not enough tokens on round balance");
            UserInfo storage user = users[_users[i]];
            user.amount = _amounts[i];
            SharkTotalAmount -= _amounts[i];
            user.claimed = _claimed[i];
        }
        emit UserMigrated(_users, _amounts, _claimed);
    }

    /** @dev emergency transfer Shark tokens from round
    *
    */
    function transferSharkToken(address receiver, uint256 amount) external{
        require(hasRole(ADMIN_ROLE, msg.sender), "Caller is not an admin");
        ERC20(SharkToken).safeTransfer(receiver, amount);
    }
}
