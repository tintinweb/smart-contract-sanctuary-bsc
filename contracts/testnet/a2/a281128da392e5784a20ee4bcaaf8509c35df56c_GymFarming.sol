...
contract GymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        validAddress(_bankAddress)
        validAddress(_busdAddress)
    {
        __Ownable_init();
        ...
        wbnbToBusdPair = [wbnbAddress, busdAddress];
    }
    ...
    function getTokenAmountUsdPrice(address token, uint256 amount)
        public
        view
        returns (uint256 usdAmount)
    {
        uint256 decimals = IERC20MetadataUpgradeable(token).decimals();
        uint256 tokenBaseAmount = 10**decimals;

        if (token == busdAddress) {
            usdAmount = amount / tokenBaseAmount;
        } else if (token == wbnbAddress) {
            usdAmount = (_getWbnbUsdPrice() * amount) / 1 ether;
        } else {
            ...
            uint256[] memory tokenPriceInBnb = IPancakeRouter02(routerAddress).getAmountsOut(
                tokenBaseAmount,
                path
            );

            uint256 wbnbPrice = _getWbnbUsdPrice();

            usdAmount =
                (tokenPriceInBnb[1] * wbnbPrice * amount) /
    ...
    function _getWbnbUsdPrice() public view returns (uint256) {
        uint256[] memory wbnbPrice = IPancakeRouter02(routerAddress).getAmountsOut(
            1,
            wbnbToBusdPair
        );
     ...
     */
    function set(uint256 _pid, uint256 _allocPoint) public onlyOwner poolExists(_pid) {
        massUpdatePools();
     ...
     * @param _amountTokenA: amount of token A for staking
     * @param _amountTokenB: amount of token B for staking
    ...
    function speedStake(
        ...
        uint256 lpTokensAmount = 0;

        if (_isBnbPool(lpToken)) {
            lpTokensAmount = _provideLiquidityBnb(
                lpToken,
                _amountTokenB,
                _amountAMin,
                _amountBMin,
        ...
        } else if (_isBusdPool(lpToken)) {
            lpTokensAmount = _provideLiquidityBusd(
                lpToken,
                _amountTokenA,
                _amountTokenB,
                _minAmountOutA,
            ...
            revert("GymFarming: Unsupported liquidity pool");
        }

        _deposit(_pid, lpTokensAmount, msg.sender);
    }

    function _isBnbPool(IPancakeswapPair pair) internal view returns (bool) {
        IPancakeRouter02 router = IPancakeRouter02(routerAddress);

        return pair.token0() == router.WETH() || pair.token1() == router.WETH();
    }

    function _isBusdPool(IPancakeswapPair pair) internal view returns (bool) {
        return pair.token0() == busdAddress || pair.token1() == busdAddress;
    }

    function _provideLiquidityBnb(
        ...
        if (lpToken.token0() == wbnbAddress) {
            token = IERC20Upgradeable(lpToken.token1());
        }

        if (_tokenAmount > 0) {
            token.safeTransferFrom(_from, address(this), _tokenAmount);
            token.approve(routerAddress, _tokenAmount);

            uint256 outAmount = _swapTokens(address(token), wbnbAddress, _tokenAmount, _deadline);

            bnbAmount += outAmount;
        }

        uint256 burnAmount = _percentage(bnbAmount, BUY_BACK_COMMISION);

        _buyAndBurnTokens(busdAddress, burnAmount, _minBurnAmount, _deadline);

        bnbAmount = address(this).balance;

        lpTokens = liquidityProvider.addLiquidityETHByPair{value: bnbAmount}(
            lpToken,
            address(this),
            _amountAMin,
            _amountBMin,
    ...
    function _provideLiquidityBusd(
        IPancakeswapPair lpToken,
        uint256 _busdAmount,
        uint256 _tokenAmount,
        ...
        uint256 busdTotal = _busdAmount;

        if (lpToken.token0() == busdAddress) {
            token = IERC20Upgradeable(lpToken.token1());
        }

        if (_busdAmount > 0) {
            IERC20Upgradeable(busdAddress).safeTransferFrom(_from, address(this), _busdAmount);
        }
        ...
        if (_tokenAmount > 0) {
            ...
            busdTotal += _swapTokens(address(token), busdAddress, _tokenAmount, _deadline);
        }

        uint256 burnAmount = _percentage(busdTotal, BUY_BACK_COMMISION);

        // _buyAndBurnTokens(busdAddress, burnAmount, _minBurnAmount, _deadline);

        busdTotal -= burnAmount;

        uint256 tokenBLiquidityAmount = busdTotal / 2;

        IERC20Upgradeable(busdAddress).approve(routerAddress, busdTotal);

        _swapTokens(busdAddress, address(token), tokenBLiquidityAmount, _deadline);

        token.approve(routerAddress, tokenBLiquidityAmount);

        (, , lpTokens) = IPancakeRouter02(routerAddress).addLiquidity(
            busdAddress,
            address(token),
            busdTotal - tokenBLiquidityAmount,
            tokenBLiquidityAmount,
            _minAmountOut,
            0,
            address(this),
            _deadline
     ...
     * @return uint256 amount of desired tokens received
     */
    ...
    function _buyAndBurnTokens(
        ...
        IERC20Upgradeable(paymentToken).safeTransfer(buyBack, spentAmount);

        burntAmount = IBuyAndBurn(buyBack).buyAndBurnToken(
            paymentToken,
            spentAmount,
            address(rewardToken),
            minBurnAmount,
    ...
    function _percentage(uint256 amount, uint256 percent) internal pure returns (uint256) {
    ...
    function claimAndDeposit(
        ...
        IWETH(wbnbAddress).deposit{value: _percentage(bnbAmount, BUY_BACK_COMMISION)}();
        IERC20Upgradeable(wbnbAddress).safeTransfer(
            buyBack,
            _percentage(bnbAmount, BUY_BACK_COMMISION)
        );

        IBuyAndBurn(buyBack).buyAndBurnToken(
            wbnbAddress,
            _percentage(bnbAmount, BUY_BACK_COMMISION),
            address(rewardToken),
            _minBurnAmt,
        ...
        if (user.lpTokensAmount > 0) {
            ...
            rewardToken.approve(routerAddress, pending);
    ...
    function autoDeposit(
        ...
        rewardToken.approve(routerAddress, _amount);
     ...
     */
    function withdraw(uint256 _pid) public nonReentrant poolExists(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
