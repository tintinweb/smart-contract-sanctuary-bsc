...
// ALL RIGHTS RESERVED
// metastartercrypt by SDDTech reserves all rights on this code. You may NOT copy these contracts.
...
// This is NOT for AMM LP tokens (such as metastarter), Please use our liquidity lockers for this.
// Locking LP tokens in this contract will not show in the metastartercrypt browser.
...
// Please be aware if you are locking tokens to prove to your commmetastarterty you have locked tokens for long term you should not use a premature unlocking condition
...
interface ImetastartercryptAdmin {
    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct LockParams {
        ...
        uint256 startEmission; // 0 if lock type 1, else a metastarterx timestamp
        uint256 endEmission; // the unlock date as a metastarterx timestamp (in seconds)
        address condition; // address(0) = no condition, otherwise the condition must implement IUnlockCondition
    }

    EnumerableSetUpgradeable.AddressSet private TOKENS; // list of all metastarterque tokens that have a lock
    mapping(uint256 => TokenLock) public LOCKS; // map lockID nonce to the lock
    uint256 public NONCE; // incremental lock nonce counter, this is the metastarterque ID for the next lock
    ...
    ImetastartercryptAdmin LPCX_ADMINS;
    ...
    function initialize() public initializer {
        ...
        NONCE = 0;
        MINIMUM_DEPOSIT = 100;
        LPCX_ADMINS = ImetastartercryptAdmin(msg.sender);
        FEES.tokenFee = 35;
     ...
     * amount: must be >= 100 metastarterts
    ...
    function lock(
        ...
        uint256 lockstartEmission, // 0 if lock type 1, else a metastarterx timestamp
        uint256 lockendEmission, // the unlock date as a metastarterx timestamp (in seconds)
     ...
     * or not give commmetastarterty enough insurance tokens will remain locked until the end date, in such a case, it can be revoked
     */
