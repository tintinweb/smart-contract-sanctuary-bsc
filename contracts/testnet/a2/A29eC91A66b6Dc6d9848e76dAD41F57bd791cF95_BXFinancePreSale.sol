...
abstract contract Crowdsale is Initializable {
   ...
   */
  function __Crowdsale_init_unchained(uint256 _rate, IERC20MetadataUpgradeable _token) internal onlyInitializing {
  ...
  function buyTokens(address _beneficiary) internal {
    ...
    if(!isContributed[_beneficiary]) {
      isContributed[_beneficiary] = true;
      totalContributors += 1;
    }
    tokensSold += tokens;
    ...
    virtual
  {
    ...
    uint256 userTempBnbBalance = userBnbBalance[_beneficiary] + _weiAmount;
    require(userTempBnbBalance >= params.minimumBuyLimit, "Min contribution not reached");
    require(userTempBnbBalance <= params.maximumBuyLimit, "Max contribution reached");
  }
  ...
  /**
    ...
    */
  function _changeToken(IERC20MetadataUpgradeable newToken) virtual internal {
    token = newToken;
  }
}
...
abstract contract TimedCrowdsale is Crowdsale {
   ...
   */
  function __TimedCrowdsale_init_unchained(uint256 _openingTime, uint256 _closingTime) internal onlyInitializing {
    // solium-disable-next-line security/no-block-members
...
contract BXFinancePreSale is Crowdsale, FinalizableCrowdsale, AccessControlUpgradeable {
    ...
    // fee(%) in token sold fee to BXFinanceOwner
    uint256 public tokenSoldFee;

    // bx owner account
    address public BXFinanceOwner;
    ...
    // bool if the pool is cancelled or not
    ...
    event UpdateDeadWallet(address indexed _deadWallet);
    event UpdateCapPercentage(uint256 capPercentage);
    event CancelPool(address indexed to, uint256 tokenAmount);

    function initialize(
      uint256 rate,                 // rate, in TKNbits
      IERC20MetadataUpgradeable token,      // the token
      ...
      uint256 _bnbRaisedFee,        // _bnbRaisedFee(%)
      address _BXFinanceOwner,      // BXFinance owner account
      bool whitelist             // whitelist enable disable bool
    ) initializer public {
      __Pausable_init_unchained();
      __Ownable_init_unchained();
      __Crowdsale_init_unchained(rate, token);
      __TimedCrowdsale_init_unchained(openingTime, closingTime);
      _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
      _setupRole(LAUNCHPAD_ROLE, msg.sender);

      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_routerAddress);
      address pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(address(token), _uniswapV2Router.WETH());
      if(pair == address(0)) {
       // Create a uniswap pair for this new token
       uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
        .createPair(address(token), _uniswapV2Router.WETH());
      } else {
        // if pair already exist update from getPair.
         uniswapV2Pair = pair;
      }
      ...
      BXFinanceOwner = _BXFinanceOwner;
      tokenSoldFee = _tokenSoldFee;
      bnbRaisedFee = _bnbRaisedFee;
      enableWhitelisting = whitelist;
      penalityFee = 10;
      deadWallet = address(0);
      capPercentage = 50;
    }

  function setParams(uint256 _minimumBuyLimit, uint256 _maximumBuyLimit, uint256 _softcap, uint256 _hardcap) external whenNotPaused onlyRole(LAUNCHPAD_ROLE) {
    ...
    uint256 getCapPercentage = calculateCapPercentage(_hardcap);
    require(_softcap >= getCapPercentage , "Set Params: Soft cap must be greater than or equal to 50% of the hard cap.");
    _grantRole(DEFAULT_ADMIN_ROLE, owner());
    _grantRole(LAUNCHPAD_ROLE, owner());
    params = IntializeParams(_minimumBuyLimit, _maximumBuyLimit, _softcap, _hardcap);
  }

  function calculateCapPercentage(uint256 _hardcap) internal view returns(uint256) {
    return (_hardcap * capPercentage) / 10 ** 2;
  }

  function updateCapPercentage(uint256 _capPercentage) external onlyOwner {
    require(_capPercentage >= 50 , "Cap Percentage: Cap percentage must be greater than 50%.");
    capPercentage = _capPercentage;
    emit UpdateCapPercentage(_capPercentage);
  }
    ...
    function changeToken(IERC20MetadataUpgradeable newToken) virtual external onlyOwner onlyWhileOpen whenNotPaused{
  ...
  function dexListing() private {
    ...
    // (%) of bnb raised to BXFinance owner
    uint256 fee = (bnbBalanceInContract * bnbRaisedFee) / 10**2;
    address payable _owner = payable(BXFinanceOwner);   
    _owner.transfer(fee);

    // (%) of token sold to BXFinance owner
    require(token.balanceOf(address(this)) > 0, "Finalize: Insufficient token balance");
    uint256 tokensForBXFinanceOwner = (tokensSold * tokenSoldFee) / 10 ** 4;
    token.transfer(BXFinanceOwner, tokensForBXFinanceOwner);
    ...
    uint256 refundBalance = tokenBalanceInContract - tokensForBXFinanceOwner - tokenForLiquidity - tokensSold;
    if(refundType == 1){
  ...
  */
  ...
  /**
    @dev emergency withdraw for users only bnb they contributed with penalty 10% to owner
  */
  ...
  /**
    @dev user can claim accumulated tokens corresponding to contributed bnb
  ...
  /**
    @dev update penality fee
    @param _penalityFee penality fee
   */
  ...
  function getLaunchPadDetails() external view returns (address, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
    return(
      address(token),
      rate,
      listingRate,
  ...
  function updateDeadWallet(address _deadWallet) external onlyOwner {
    deadWallet = _deadWallet;
    emit UpdateDeadWallet(_deadWallet);
  }

  function cancelPool() external onlyOwner {
    require(!isFinalized, "Finalized"); 
    uint256 tokenBalanceInContract = token.balanceOf(address(this));
    require(tokenBalanceInContract > 0, "No token balance");
    token.transfer(owner(), tokenBalanceInContract);
    isPoolCancelled = true;
    emit CancelPool(owner(), tokenBalanceInContract);
  }
