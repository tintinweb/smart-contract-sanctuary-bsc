    ...
    EIP712Upgradeable
{
    ...
    string private constant SIGNING_DOMAIN = "NFT_REFLECTION";
     ...
     * @param currentValue: Current value of settled assests
     * @param previousValue: Previouse value of settled assests
     */
    struct PoolInfo {
        IERC20Upgradeable payableTokenAddress;
    ...
    function initialize(
        ...
        poolInfo.push(
            PoolInfo({
                payableTokenAddress: IERC20Upgradeable(_wbnbAddress),
                currentValue: 0,
                previousValue: 0
            })
        );
        poolInfo.push(
            PoolInfo({
                payableTokenAddress: IERC20Upgradeable(_busdAddress),
                currentValue: 0,
                previousValue: 0
            })
        );
    ...
    modifier onlyEmailVerified(address _user) {
        require(isEmailVerified[_user], "NFTReflectionPool:: Only users with verified email");
        _;
    }
    ...
    modifier onlyWhitelisted() {
        require(
            ...
            "NFTReflectionPool: not whitelisted or owner"
        );
        _;
    }
    ...
    modifier onlyMunicipality() {
        require(
            ...
            "NFTReflectionPool: only Municipality"
        );
        _;
    }
    ...
    function verifyEmail(SignatureMessage calldata signatureMessage) external {
        require(!signatureUsed[signatureMessage.signature], "NFTReflectionPool:: Already used signature");
        address _messageSigner = _verify(signatureMessage);

        require(msg.sender == signatureMessage.userWallet, "NFTReflectionPool:: Invalid reward");
        require(_messageSigner == serviceSignerAddress, "NFTReflectionPool:: Invalid signer");
    ...
    /**
     * @notice Function for reward capture
     * @param _pid: Pool id of reflection
     */
    function captureRewards(
        uint256 _pid
    ) external onlyWhitelisted {

        PoolInfo storage pool = poolInfo[_pid];
        pool.previousValue = pool.currentValue; 
        pool.currentValue = 0;
    }

    /**
     * @notice Function for deposit to reflection pool
     * @param _pid: Pool id of reflection
    ...
    function deposit(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        pool.currentValue += _amount;
    }

    /**
     * @notice Function for claim rewards from reflection pool
     * @param _pid: Pool id of reflection
    ...
    function claim(
        ...
        IERC20Upgradeable(pool.payableTokenAddress).safeTransferFrom(address(this), msg.sender, _amount);
    }
