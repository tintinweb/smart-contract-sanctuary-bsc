...
contract VaultFacet is IVault {
    ...
    function increaseByCloseTrade(address token, uint256 amount) external override {
        LibAccessControlEnumerable.checkRole(Constants.SELF_ROLE);
        LibVault.deposit(token, amount);
    }

    function decreaseByCloseTrade(address token, uint256 amount) external override returns (ITrading.CloseSettleToken[] memory) {
        LibAccessControlEnumerable.checkRole(Constants.SELF_ROLE);
