...
contract DORToken is ERC20, Ownable {
    ...
    constructor(
        ...
        referralFeePercent[1] = (1 * BASE_RATIO) / 100;

        referralFeePercent[2] = (5 * BASE_RATIO) / 1000;
        referralFeePercent[3] = (5 * BASE_RATIO) / 1000;
        currentAllFee +=
            referralFeePercent[1] +
            referralFeePercent[2] +
            referralFeePercent[3];
    ...
    function setReferralFeePercent(uint256 loop, uint256 percent)
        external
        onlyOwner
        checkMaxFeeLimit(referralFeePercent[loop], percent)
    {
        require(
            loop == 1 || referralFeePercent[loop - 1] > 0,
            "can only be added sequentially"
        );
        referralFeePercent[loop] = percent;
    }
    ...
    function calculateFee(
        ...
        address referrer = referralHandle.referrers(account);
        for (uint256 i = 1; i != 20; i++) {
            uint256 referralReward = amount.mul(referralFeePercent[i]).div(
                BASE_RATIO
            );
            if (referralReward > 0) {
                realAmount = realAmount.sub(referralReward);
                if (referrer != address(0)) {
                    super._transfer(account, referrer, referralReward);
                } else {
                    super._transfer(account, vault, referralReward);
                }
            } else {
                break;
            }
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        swapTokensForToken(half, address(this), address(usdtToken));

        uint256 newBalance = usdtToken.balanceOf(address(this)).sub(
            initialBalance
        );
