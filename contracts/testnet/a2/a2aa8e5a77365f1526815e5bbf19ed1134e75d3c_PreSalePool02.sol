...
contract PreSalePool02 is Ownable, ReentrancyGuard, Pausable, RedKiteWhitelist {
    ...
    function buyTokenByEtherWithPermission(
        ...
        uint256 taxFee = _updatePurchasingState(weiAmount, tokens);

        investedAmountOf[address(0)][_candidate] = investedAmountOf[address(0)][
            _candidate
        ].add(weiAmount);

        emit TokenPurchaseByEther(msg.sender, weiAmount, taxFee, tokens);
    }
    ...
    function buyTokenByTokenWithPermission(
        ...
        uint256 taxFee = _updatePurchasingState(_amount, tokens);
    ...
    function _updatePurchasingState(uint256 _weiAmount, uint256 _tokens)
        internal
        returns (uint256 taxFee)
    {
        taxFee = _getTaxFee(_tokens);
        weiRaised = weiRaised.add(_weiAmount);
        tokenSold = tokenSold.add(_tokens);
        userPurchased[msg.sender] = userPurchased[msg.sender].add(
            _tokens.sub(taxFee)
        );
