...
import "../libs/SingleAssetDynamicRainMaker.sol";
import "../libs/CErc20Delegator.sol";

/// @title Strategy Maximizer - MasterApe
/// @author ApeSwapFinance
/// @notice MasterApe strategy for maximizer vaults
contract StrategyMaximizerOLA is BaseBananaMaximizerStrategy {
    using SafeERC20 for IERC20;

    // Farm info
    CErc20Delegator public immutable STAKE_TOKEN2;
    SingleAssetDynamicRainMaker public immutable RAIN_MAKER;
    ...
    {
        STAKE_TOKEN2 = CErc20Delegator(_oToken);
        RAIN_MAKER = SingleAssetDynamicRainMaker(_rainMaker);
    }
    ...
    function totalStake() public view override returns (uint256) {
        uint256 amount = (STAKE_TOKEN2.balanceOf(address(this)) *
            STAKE_TOKEN2.exchangeRateStored()) / 10**18;
        return amount;
    }
    ...
    function _vaultDeposit(uint256 _amount) internal override {
        _approveTokenIfNeeded(STAKE_TOKEN, _amount, address(STAKE_TOKEN2));
        STAKE_TOKEN2.mint(_amount);
    }

    /// @notice Handle withdraw of this strategy
    /// @param _amount Amount to remove from staking
    function _vaultWithdraw(uint256 _amount) internal override {
        STAKE_TOKEN2.redeem(_amount);
    }

    /// @notice Handle harvesting of this strategy
    function _vaultHarvest() internal override {
        RAIN_MAKER.claimComp(address(this));
    }
        ...
        returns (uint256)
    {
        uint256 rewards = _rewardTokenBalance() +
            RAIN_MAKER.compAccrued(address(this));
    ...
    function _emergencyVaultWithdraw() internal override {
        STAKE_TOKEN2.redeem(STAKE_TOKEN2.balanceOf(address(this)));
    }
...
interface SingleAssetDynamicRainMaker {
...
interface CErc20Delegator {
    function mint(uint256 _amout) external returns (uint256);

    function redeem(uint256 _redeemAmount) external returns (uint256);
