...
contract XXXXX {
	...
	uint256 constant public TIME_STEP = 180;             //testnet
	...
	constructor() {
        ...
        plans.push(Plan(0, 1080, 0.01 ether, 5 ether));
		//plans.push(Plan(0, 1080, 0.04 ether, 0.08 ether));
        plans.push(Plan(1, 1100, 0.02 ether, 5 ether));
		//plans.push(Plan(1, 1100, 0.08 ether, 0.12 ether));
        plans.push(Plan(2, 1120, 0.03 ether, 5 ether));
		//plans.push(Plan(2, 1120, 0.12 ether, 0.20 ether));
        plans.push(Plan(3, 1140, 0.04 ether, 5 ether));
		...
		plans.push(Plan(29, 1660, 0.05 ether, 0.05 ether));
	...
	function getUserLastDepositInfo(address userAddress) public view returns(uint8 plan, uint256 amount, uint256 start, uint256 finish, uint256 profit, bool isWithdrawn) {
	    User storage user = users[userAddress];
		if(user.deposits.length > 0){
			...
			isWithdrawn = user.deposits[users[userAddress].deposits.length - 1].isWithdrawn;
		}
