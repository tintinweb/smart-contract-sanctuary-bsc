...
BSC Defi - BNB Miner V2
  ...
  ~ 50 tickets max per user. 150 BNB = 50 tickets.
  ~ 0.005 bnb = 1 per ticket. based on user deposit/compound.
  ~ 100% of the pot will be converted to miners and be sent to the winning address.
...
contract BSC_Defi {
    ...
    constructor(address payable _owner, address payable _project, address payable _community) {
		require(!isContract(_owner) && !isContract(_project) && !isContract(_community));
        owner = _owner;
        project = _project;
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        (uint256 projectFee, uint256 communityFee) = getFees(eggValue);
        project.transfer(projectFee);
        community.transfer(communityFee);
        return projectFee.add(communityFee);
    }

    function getFees(uint256 eggValue) public view returns(uint256 _projectFee, uint256 _communityFee) {
        _projectFee = eggValue.mul(PROJECT).div(PERCENTS_DIVIDER);
        _communityFee = eggValue.mul(COMMUNITY).div(PERCENTS_DIVIDER);
    }
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    uint256 eggsReward = calculateEggBuy(currentPot, getBalance().sub(currentPot));
                    ...
                    /** record round **/
                    lotteryHistory.push(LotteryHistory(lotteryRound, winnerAddress, currentPot, participants, totalTickets));
