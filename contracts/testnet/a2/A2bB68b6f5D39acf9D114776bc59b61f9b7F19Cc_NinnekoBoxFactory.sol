...
import "./NinnekoBox.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract NinnekoBoxFactory is Ownable {
    NinnekoBox public ninnekoBox;
    address public operator;

    constructor() {
        ninnekoBox = NinnekoBox(0x5df4d874284DEcFc029a5426B147546D89d4Ef76);
        operator = 0xbA72B4784382aE12DF294AD1E05b450505Deca9B;
    }
    ...
    function batchMint(address[] memory addrs, uint256[] memory types) external onlyOperator {
        for (uint256 i = 0; i < addrs.length; i++) {
            ninnekoBox._mintBatchToAddress(addrs[i], 2, types);
        }
    }

    function setNinnekoBoxAddress(address add) external onlyOwner {
        ninnekoBox = NinnekoBox(add);
    }

    function changeNinnekoBoxOwner(address newOwner) external onlyOwner {
        ninnekoBox.transferOwnership(newOwner);
    }

    function setAddressTransferOrReceive(address _add, bool _isTransfer) external onlyOwner {
        ninnekoBox.setAddressTransferOrReceive(_add, _isTransfer);
    }
}