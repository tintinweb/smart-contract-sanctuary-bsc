...
contract Kuafu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RenyinTigerDividendTracker public dividendTracker;
    ...
    uint256 public buyDogeRewardsFee = 6;
    uint256 public sellDogeRewardsFee = 6;
    ...
    uint256 public AmountDogeRewardsFee = 0;
    ...
    constructor() ERC20("Kuafu", "Kuafu") {
        _totalSupply = 100000000000 * (10**18);

        dividendTracker = new RenyinTigerDividendTracker();

        _marketingWalletAddress = 0xf38596b3185d4a2246d479EF3Cc76A82b73fC3BD;
        _liquidityAddress = 0x17e71A722295D463537258E2E23e6cd3094509E2;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
            0xFa1beF08f979155e14b604c6a4f1A5b2C3209af9
        );
        ...
        excludeFromFees(_liquidityAddress, true);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "RenyinTiger: The dividend tracker already has that address"
        );

        RenyinTigerDividendTracker newDividendTracker = RenyinTigerDividendTracker(
                payable(newAddress)
            );

        require(
            newDividendTracker.owner() == address(this),
            "RenyinTiger: The new dividend tracker must be owned by the RenyinTiger token contract"
        );

        newDividendTracker.excludeFromDividends(address(newDividendTracker));
        newDividendTracker.excludeFromDividends(address(this));
        newDividendTracker.excludeFromDividends(owner());
        newDividendTracker.excludeFromDividends(_liquidityAddress);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "RenyinTiger: The router already has that address"
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "RenyinTiger: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "RenyinTiger: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "RenyinTiger: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "RenyinTiger: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            swapAndSendDividends(AmountDogeRewardsFee);
            swapping = false;
        }
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                CFee = amount.mul(buyDogeRewardsFee).div(100);
                AmountDogeRewardsFee += CFee;
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                AmountLiquidityFee += LFee;
                CFee = amount.mul(sellDogeRewardsFee).div(100);
                AmountDogeRewardsFee += CFee;
                DFee = amount.mul(deadFee).div(100);
            ...
            super._transfer(from, address(this), fees.sub(DFee).sub(MFee));
        }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        AmountDogeRewardsFee = AmountDogeRewardsFee - tokens;
...
contract RenyinTigerDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken(
            "RenyinTiger_Dividen_Tracker",
            "RenyinTiger_Dividend_Tracker"
        )
    {
        claimWait = 86400;
    ...
    function _transfer(
        ...
        require(false, "RenyinTiger_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "RenyinTiger_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main RenyinTiger contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "RenyinTiger_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "RenyinTiger_Dividend_Tracker: Cannot update claimWait to same value"
        );
