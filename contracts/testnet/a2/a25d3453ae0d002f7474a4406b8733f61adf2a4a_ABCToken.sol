...
contract ABCToken is Context, IERC20, Ownable {
    ...
    uint256 public _rewardHolderFee = 2;
    uint256 public _mktFee = 3;
    uint256 public _devFee = 3;
    uint256 public _wildlifeFee = 1;
    uint256 public _burnFee = 1;

    uint256 public _preRewardHolderFee;
    uint256 public _preMktFee;
    uint256 public _preDevFee;
    uint256 public _preWildlifeFee;
    uint256 public _preBurnFee;

    uint constant private DEMI = 100;

    string private constant _name = "ABC Token";
    string private constant _symbol = "ABC";
    uint8 private constant _decimals = 9;
    
    address payable private _devAddress = payable(0xD06170834c7A7AB6Ed0583d62884dEFeAB23a440);
    address payable private _mktAddress = payable(0x0FAFeB41f83342E3BDEFc3Ed06c6F36530e59049);
    address payable private _wildlifeAddress = payable(0x2045Eed281859d3a293B3a3624A67cfb6cd220A5);
    ...
    constructor (){
        ...
        _isExcludedFromFee[_wildlifeAddress] = true;
    ...
    function sendBNBToFee(uint256 amount) private {
        uint256 _wildlifeFund = amount.mul(_wildlifeFee).div(100);
        uint256 _amount = amount.sub(_wildlifeFund);
        _devAddress.transfer(_amount.div(2));
        _mktAddress.transfer(_amount.div(2));
        _wildlifeAddress.transfer(_wildlifeFund);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeWildlife( _rValues.rWildlife);
    ...
    function _takeWildlife(uint256 rWildlife) private {
         _rOwned[address(this)] = _rOwned[address(this)].add(rWildlife);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (TValuesStruct memory _tValues) {
        ...
        _tValues.tWildlife = tAmount.mul(_wildlifeFee).div(100);
        _tValues.tBurn = tAmount.mul(_burnFee).div(100);
        _tValues.tTransferAmount = (((((tAmount.sub(_tValues.tFee)).sub(_tValues.tDev)).sub(_tValues.tMkt)).sub(_tValues.tWildlife)).sub(_tValues.tBurn));
    }
    ...
    function _getRValues(uint256 tAmount, TValuesStruct memory _tValues, uint256 currentRate) private pure returns (RValuesStruct memory _rValues) {
        ...
        _rValues.rWildlife = _tValues.tWildlife.mul(currentRate);
