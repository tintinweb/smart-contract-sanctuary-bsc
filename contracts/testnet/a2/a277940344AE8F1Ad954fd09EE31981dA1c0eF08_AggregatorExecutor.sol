...
contract AggregatorExecutor is Ownable {
    ...
    uint256 private constant _FROM_ETH = 1 << 0;
    uint256 private constant _TO_ETH = 1 << 1;

    uint256 public DENOMINATOR_FEE = 10000;
//    IWBNB public WBNB = IWBNB(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // mainnet
    IWBNB public WBNB = IWBNB(0xD4Bb551E18CB643d1086deDF4bCada3A6a4AB38f);
    address public BNB = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;

    //=========== external functions ============//
    function swap(SwapStep[] calldata _swaps, address payable _receiver, uint256 _flags) external payable {
        //check input eth
        if(_flags & _FROM_ETH != 0) {
        ...
        //send eth to user
        if ( _flags & _TO_ETH != 0 && _swaps[_length - 1].toToken == address(WBNB)) {
    ...
    function _swap(
        ...
        //check price impact
        if (finish) {
            require(amountOut >= _step.amountOutMin, "AggregatorExecutor: price impact too high");
        }
