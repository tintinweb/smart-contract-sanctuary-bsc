...
contract ARTLP {
    ...
    function getReleaseRecords(address account)view public returns(uint256 [] memory pledgeTimeList,
        uint256 [] memory amountList,
        uint256 [] memory realeaseTimestampList){
        
        ReleaseRecord[] memory records = _releaseRecords[account];
    ...
    function getPledgeRecords(address account) view public returns (uint256 [] memory pledgeTimeList,
        ...
        isReleaseList=new uint8[](leng);
         for(uint256 i=0;i < leng;i++){
            pledgeTimeList[i]=records[i].timestamp;
            amountList[i]=records[i].amount;
            isReleaseList[i]=records[i].isRelease;
            i++;
        }
    }
