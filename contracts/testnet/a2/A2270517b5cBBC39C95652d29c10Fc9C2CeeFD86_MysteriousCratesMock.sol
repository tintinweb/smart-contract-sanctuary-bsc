...
@author Leo
*/
contract MysteriousCratesMock is ERC721Enumerable, AccessControl, Pausable {
   ...
   */
  function batchMintCrate(address[] calldata users) external onlyRole(DEFAULT_ADMIN_ROLE) {
   ...
   */
  function mintCrate(address user, uint256[] memory cardIds) public onlyRole(MINTER_ROLE) whenNotPaused returns (uint256) {
  ...
  function getIdsAsArray(uint256 crateId) public view returns (uint256[] memory) {
    uint256[] memory arr = new uint256[](3);
