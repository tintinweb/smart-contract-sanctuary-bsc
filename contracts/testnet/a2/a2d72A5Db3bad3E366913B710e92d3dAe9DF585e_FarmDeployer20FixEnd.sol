...
contract ERC20FarmFixEnd is Ownable, ReentrancyGuard, IERC20FarmFixEnd{
    ...
    function initialize(
        ...
        defaultRewardPPS = 10 ** (decimalsRewardToken - 4);
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        require(_amount > defaultStakePPS, "Below minimum amount");
    ...
    function setFarmValues(
        ...
        //start block
        uint256 prevStartBlock = startBlock;
        if (prevStartBlock != _startBlock) {
            require(_startBlock >= block.number, "Can't set past block");
            require(prevStartBlock >= block.number, "Staking has already started");
            startBlock = _startBlock;
            lastRewardBlock = _startBlock;
    ...
    function _transferStakeWithFee(address receiver, uint256 shares) private {
        ...
        _transferStake(receiver, transferAmount);
