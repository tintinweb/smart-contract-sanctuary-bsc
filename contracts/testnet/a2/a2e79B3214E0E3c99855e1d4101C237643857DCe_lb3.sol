...
interface IluckblocksNodes {
    function updateNodeInfoK(address caller, uint256 reward) external returns (bool);
...
contract lb3 is VRFConsumerBase,ReentrancyGuard,Ownable {
	using SafeMath for uint256;
	...
	IERC20 usd = IERC20(0x003eF6Ad3b1EFF56eA487E1561Ab3243a73a4b14); // USDC
    ...
    uint256 minimumPot = 3000 * 10**18;
    uint256 botReward = 10 * 10**18;
    ...
    uint internal TICKET_VAL = 20000000000000000000; //minimum amount (in wei) for getting registered on list
    ...
    uint public JACKPOT_CHANCE = 0.3 * 10**18; //the chance to hit the jackpot in percent after selected on raffle
        ...
        )
    {
        ...
        priceFeed = AggregatorV3Interface(0x957Eb0316f02ba4a9De3D308742eefd44a3c1719); // MATIC/USD Price feed
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 10 && block.timestamp > lastAutoSpin + 600,"autoSpin parameters not met");

        if(lbNodes.getUserActivation(_caller) == false){
            lbNodes.activateNodes(_caller,_nodes,2);
        }

        if(block.timestamp > lastAutoSpin + 660){
            lbNodes.resetQueue(2);
        } else if(lbNodes.updateNodeInfoK(_caller,botReward) == true){
    ...
    // Play Function
    function BuyTicket(address player,uint number1,uint number2) public nonReentrant {
        ...
        require(number1 <= 25 && number1 > 0 && number2 <= 25 && number2 > 0 && number1 != number2,"Failed: numbers has to be between 1 and 25 and not be equal.");
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
       ...
       require(getJackpotinUSD() >= 40 * 10**18, "Contract need to have atleast 40 USD in value.");
