...
contract TokenVesting is Ownable {
    ...
    modifier unlockedPartUpgrade(uint256 userCard) {
        if (getCurrentTime() > currentTime[userCard]) {
            uint256 full = (getCurrentTime() - currentTime[userCard]) /
                1 minutes;
            currentTime[userCard] += full * 1 minutes; //!!!
    ...
    function getPayedAndTotal(address user, uint256 cardId)
        external
        view
        returns (
            uint256,
        ...
        )
    {
        return (
            ...
            idToVestingCard[user][cardId].payed,
    ...
    function getBalance(uint256 amount) external view returns (uint256) {
        return ((amount * tokenProportion) / 10**10);
    }

    function getSunRes() external view returns (uint256) {
        return (sumReserved() / 10**10);
    }
        ...
        unlockedPartUpgrade(cardId)
    {
        ...
        //require(idToVestingCard[idToAddress[cardId]][cardId].totalAmount != idToVestingCard[idToAddress[cardId]][cardId].payed, "Already paid all");
        ...
        if (tokenProportion > 0) {
            for (uint256 i = 0; i <= counter; i++) {
                idToVestingCard[idToAddress[i]][i].totalAmount =
                    ...
                    proportion_;
                idToVestingCard[idToAddress[i]][i].changed = true;
                //idToVestingCard[idToAddress[i]][i].payed = idToVestingCard[idToAddress[i]][i].payed;
            }
        }
        //100 90
        //10000 9000
    ...
    function getInvestedAmount(address user, uint256 cardId)
        external
        view
        returns (uint256)
    {
    ...
    function getEndTokensAmount(address user, uint256 cardId)
        external
        view
        returns (uint256)
    {
    ...
    function alreadyUnlocked() public view returns (uint256) {
        uint256 full = (getCurrentTime() - maxStartVesting()) / 1 minutes;
        uint256 unlocked = 0;
        if (full >= 6) {
            unlocked = 6 * 15;
        } else {
