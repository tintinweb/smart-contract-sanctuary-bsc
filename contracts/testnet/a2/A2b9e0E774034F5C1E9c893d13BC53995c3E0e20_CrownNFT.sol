...
contract CrownNFT is ERC721Votes, ERC721Enumerable {
     ...
     */
    function setValidTarget(address target, bool permission) external onlyOwner {
    ...
    function _mint(address to, uint256 tokenId) internal override(ERC721) {
        require(totalSupply() < maxSupply, "More than total");
        uint256 randomNumber = uint256(
            keccak256(abi.encodePacked(block.timestamp, to, tokenId))
        );
        uint256 randomReduce = (randomNumber % 21) + 10;
        uint256 randomAprBonus = (uint256(
            keccak256(abi.encodePacked(block.timestamp, randomReduce, tokenId))
        ) % 11) + 5;
        // Create a random traits base on token id, timestamp, address of user
        CrownTraits memory traits = CrownTraits(
            randomReduce,
            randomAprBonus,
            0,
            false
        );
        _tokenIdToTraits[tokenId] = traits;
    ...
    function ownerToTokenArray(address account) public view returns(uint256[]memory){
    ...
    function getPastNumberOfTokenLocked(address account, uint256 blockNumber) public view returns(uint256){
    ...
    function getPastVotes(address account,uint256  blockNumber) public view override(Votes) returns(uint256){
        return super.getPastVotes(account, blockNumber) - getPastNumberOfTokenLocked(account, blockNumber);
    }
    ...
    function _transfer(
        ...
        if (
            !_validTarget[to] && !_validTarget[from] 
        ) {
