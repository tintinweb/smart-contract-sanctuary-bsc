...
contract SwapHelper is Ownable {
  ...
  function safeApprove(address token, address spender, uint amount) external onlyOwner {
...
contract Authorized is Ownable {
  ...
  function getAllPermissions(address wallet) external view returns (bool[] memory results) {
    results = new bool[](3);
    for (uint8 i = 0; i < 3; i++) results[i] = permissions[i][wallet];
  }

  function safeApprove(address token, address spender, uint amount) external isAuthorized(0) {
    IERC20(token).approve(spender, amount);
  }

  function safeTransfer(address token, address receiver, uint amount) external isAuthorized(0) {
...
contract chaintestg is GasHelper, ERC20 {
  // Token Info
  string constant NAME = "chaintestg";
  string constant SYMBOL = "chaintestg";
  string public constant URL = "www.chaintestg.org";

  // Maximum Supply: 100,000,000
  uint constant MAX_SUPPLY = 100_000_000e18;

  // Addresses
  address constant DEAD = 0x000000000000000000000000000000000000dEaD;
  address constant ZERO = 0x0000000000000000000000000000000000000000;
  address constant WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd ; // WBNB Mainnet
  address constant PANCAKE_ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // PancakeSwap Router Mainnet
  // address constant WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // WBNB Testnet
  // address constant PANCAKE_ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; // PancakeSwap Router Testnet

  // Fees
  uint FEE_POOL;
  uint FEE_REFLECT; 
  uint FEE_ADMINISTRATION_WALLET;
  uint constant public MAX_TOTAL_FEE = 2500; // Total fees combined are limited to 0-25%
  ...
  constructor() ERC20(NAME, SYMBOL) {
    ...
    // Fees - 2% total by default
    FEE_POOL = 100; // 1% default
    FEE_REFLECT = 50; // 0.5% default
    FEE_ADMINISTRATION_WALLET = 50; // 0.5% default
  ...
  function getReflectFee() public view returns (uint) {
    return FEE_REFLECT;
  }

  function getAdminFee() public view returns (uint) {
    return FEE_ADMINISTRATION_WALLET;
  }
  ...
  // ----------------- Fee Settings -----------------
  function setFees(uint reflect, uint pool, uint adminFee) public isAuthorized(1) {
    uint total = reflect + pool + adminFee;
    require(total <= MAX_TOTAL_FEE, "All rates and fee together must be lower than 25%");
    FEE_ADMINISTRATION_WALLET = adminFee;
    FEE_POOL = pool;
    FEE_REFLECT = reflect;
  }
  ...
  function _transfer(address sender, address receiver, uint amount) internal override {
    ...
    // Initial Check
    // 0Address cannot send or receieve tokens
    require(sender != ZERO && receiver != ZERO, "transfer from / to the zero address"); //  0Address cannot send or receieve tokens
