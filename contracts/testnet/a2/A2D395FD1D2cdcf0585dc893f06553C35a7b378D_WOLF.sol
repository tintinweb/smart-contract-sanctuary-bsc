...
contract WOLF is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = 'Wolf Inu';
    string public _symbol = 'WOLF';
    ...
    uint256 public gameTreasuryFee = 10;
    uint256 public WOLFInsuranceFundFee = 50;
    uint256 public firePitFee = 10;
    uint256 public totalFee = liquidityFee.add(gameTreasuryFee).add(WOLFInsuranceFundFee).add(firePitFee);
    ...
    address public WOLFInsuranceFundReceiver;
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 300000000 * 10**DECIMALS;
    ...
    constructor(
        ...
        gameTreasuryReceiver = msg.sender;
        WOLFInsuranceFundReceiver = msg.sender;
        infernoPit = ZERO;
    ...
    function takeFee(
        ...
        _gonBalances[infernoPit] = _gonBalances[infernoPit].add(gonAmount.div(feeDenominator).mul(firePitFee));
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_gameTreasuryFee.add(WOLFInsuranceFundFee))
        );
        ...
        emit Transfer(sender, infernoPit, firePitFee.div(_gonsPerFragment));
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(gameTreasuryReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(gameTreasuryFee).div(gameTreasuryFee.add(WOLFInsuranceFundFee)),
            gas: 30000
        }('');
        (success, ) = payable(WOLFInsuranceFundReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(WOLFInsuranceFundFee).div(
                gameTreasuryFee.add(WOLFInsuranceFundFee)
            ),
            gas: 30000
        }('');
    }
    ...
    function setFeeReceivers(
        ...
        address _WOLFInsuranceFundReceiver,
        address _infernoPit
    ) external onlyOwner {
        gameTreasuryReceiver = _gameTreasuryReceiver;
        WOLFInsuranceFundReceiver = _WOLFInsuranceFundReceiver;
    ...
    function setFee(
        ...
        uint256 _WOLFInsuranceFundFee,
        uint256 _firePitFee,
        uint256 _sellFee
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        gameTreasuryFee = _gameTreasuryFee;
        WOLFInsuranceFundFee = _WOLFInsuranceFundFee;
        firePitFee = _firePitFee;
        totalFee = liquidityFee.add(gameTreasuryFee).add(WOLFInsuranceFundFee).add(firePitFee);
        sellFee = _sellFee;
