...
contract LevelTeamStake is Initializable, OwnableUpgradeable {
    ...
    function unlockedLVL() public view returns (uint256) {
        ...
        uint256 effectiveDuration = _now <= startTime ? 0 : (_now - startTime) / EPOCH * EPOCH;
    ...
    function lock() external onlyOwner {
        require(!locked, "LevelTeamStake::lock: locked");
        require(
            allocation != 0 && allocation == LVL.balanceOf(address(this)), "LevelTeamStake::lock: allocation not valid"
        );
        require(startTime != 0, "LevelTeamStake::lock: start time not set");
        locked = true;
    ...
    function withdraw(uint256 _amount, address _receiver) external onlyOwner {
        require(_receiver != address(0), "LevelTeamStake::withdraw: Invalid address");
        uint256 _unlockedLVL = unlockedLVL();
        require(0 < _amount && _amount <= _unlockedLVL, "LevelTeamStake::withdraw: Invalid amount");
    ...
    function cooldown() external onlyOwner {
        (uint256 totalLVLStaked,,) = LEVEL_STAKE.userInfo(address(this));
    ...
    function claimLGO(address _receiver, uint256 _amount) external onlyOwner {
        require(_receiver != address(0), "LevelTeamStake::claimLGO: Invalid address");
        if (LEVEL_STAKE.pendingReward(address(this)) != 0) {
            LEVEL_STAKE.claimRewards(address(this));
        }
        require(_amount <= LGO.balanceOf(address(this)), "LevelTeamStake::claimLGO: Insufficient balance");
        LGO.safeTransfer(_receiver, _amount);
        emit ClaimLGO(_receiver, _amount);
    }

    /* ========== RESTRICTIVE FUNCTIONS ========== */

    function setAllocation(uint256 _allocation) external onlyOwner {
        require(!locked, "LevelTeamStake::setAllocation: locked");
        require(_allocation != 0, "LevelTeamStake::setAllocation: allocation not valid");
    ...
    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime >= block.timestamp, "LevelTeamStake::setStartTime: start time not valid");
        startTime = _startTime;

        emit StartTimeUpdated(startTime);
    }

    function recoverFund(address _receiver) external onlyOwner {
        require(!locked, "LevelTeamStake::recoverFund: locked");
        require(_receiver != address(0), "LevelTeamStake::recoverFund: Invalid address");
        uint256 lvlBalance = LVL.balanceOf(address(this));
        LVL.safeTransfer(_receiver, lvlBalance);

        emit RecoverFund(_receiver, lvlBalance);
    }
    ...
    event ClaimLGO(address indexed _receiver, uint256 _amount);
    event AllocationUpdated(uint256 _amount);
    event StartTimeUpdated(uint256 _startTime);
    event RecoverFund(address indexed _receiver, uint256 _amount);
}
