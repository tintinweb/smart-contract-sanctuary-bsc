...
contract SC2 is Ownable, OverflowToken {
    ...
    address public _eABCDADDRESS = 0x83b59E94FBE45b86e9762813F23e8C02E3b1f0B4;  //mainnet
    ...
    uint256[] public staking_period = [3600 * 24 * 1, 3600 * 24 * 2, 3600 * 24 * 3, 3600 * 24 * 4, 3600 * 24 * 5]; // plan A, B, C, D, E
    ...
    function stakeEABCD(uint256 amount, uint256 _type) public {
        ...
        emit stakeReal(msg.sender, amount, 2, _type, block.timestamp, 0);
    }
    ...
    function stakeBUSD(uint256 amount, uint256 _type) public {  
        ...
        emit stakeReal(msg.sender, amount, 1, _type, block.timestamp, 0);
    }
    function getRewardInTHALESForOwner( uint idx) internal view returns(uint256){
        ...
        if(stake_data[idx].tokenType == 1){
            reward = stake_data[ idx ].amount * reward_BUSD_amount * staking_weight[ stake_data[idx].stakeType ] * ( now - stake_data[idx].stakedDate) * ( 10 ** decimal_THALES)  / (10**decimal_BUSD) / (10**decimal_BUSD);
        }
        if(stake_data[idx].tokenType == 2){
            reward = stake_data[ idx ].amount * reward_eABCD_amount * staking_weight[ stake_data[idx].stakeType ] * ( now - stake_data[idx].stakedDate) * ( 10 ** decimal_THALES)  / (10**decimal_eABCD) / (10**decimal_eABCD);
        }
        ...
        for(uint i=0; i<stakeNum; i++){
            if(stake_data[i].tokenType > 0){
                ...
                if(stake_data[i].stakedDate + staking_period[stake_data[i].stakeType] <= block.timestamp){
                    ...
                    emit stakeEnd(stake_data[i].owner, stake_data[i].amount, stake_data[i].tokenType, stake_data[i].stakeType, block.timestamp, stake_data[i].claimedAmount, stake_data[i].stakedDate);
