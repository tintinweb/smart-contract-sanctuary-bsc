...
// EXCLUDE THIS CONTRACT from fees and reward ON LANDS TOKEN
...
contract LANDSPRIVATESALE1 is Ownable {
    ...
    IBEP20 private immutable _LANDS;
    IBEP20 private immutable _BUSD;
    ...
    constructor(address LANDS, address BUSD) {
        require(
            LANDS != address(0x0) || BUSD != address(0x0),
    ...
    function vesting(uint256 tokenAmount) public {
        ...
        uint256 cliff_ = startTime_ + 100; // Cliff => 1 month (2629743)
        uint256 duration_ = startTime_ + 100000; // Total Vesting Duration => 11 months (28927173)
    ...
    function release(
        ...
        require(
            amount > 0,
            "TokenVesting: release amount, have to be greater than zero! "
        );

        if (amount > vestingSchedule.amountTotal - vestingSchedule.released) {
            amount = vestingSchedule.amountTotal - vestingSchedule.released;
        }

        require(
            amount <= vestingSchedule.amountTotal - vestingSchedule.released,
            "TokenVesting: cannot release, more than you invested! "
        );
        ...
        _LANDS.transfer(beneficiaryPayable, amount);
        _LANDS.approve(beneficiaryPayable, amount);
        emit ReleasedAmount(beneficiaryPayable, amount);
    }
        ...
        returns (uint256)
    {
        ...
        if ((currentTime < vestingSchedule.cliff)) {
            ...
            uint256 vestedAmount = ((vestingSchedule.amountTotal - TGEAmount) *
                timeAfterCliffEnd) / vestingSchedule.slicePeriodSeconds;
            vestedAmount = vestedAmount + TGEAmount - vestingSchedule.released;
            return vestedAmount;
        }
    }
    ...
    function withdrawLeftOverLANDS() external onlyOwner {
        ...
        _LANDS.transfer(owner(), LANDSBalance_);
        _LANDS.approve(owner(), LANDSBalance_);
    ...
    function LANDSBalance() external view returns (uint256) {
        return _LANDS.balanceOf(address(this));
    }
