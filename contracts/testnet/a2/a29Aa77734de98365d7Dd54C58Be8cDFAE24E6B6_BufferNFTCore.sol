    ...
    ERC721URIStorage
{
    ...
    bytes32 public constant OPTION_MINTER_ROLE =
        keccak256("OPTION_MINTER_ROLE");
    ...
    function _transferUnitsFrom(
        ...
        if (sender != from_ && !isApprovedForAll(from_, sender)) {
            _tokenApprovalUnits[optionId_].allowances[sender] =
                _tokenApprovalUnits[optionId_].allowances[sender] -
                transferUnits_;
        }
    ...
    function burnToken(uint256 optionID) external onlyOptionMinter {
    ...
    function burnOption(uint256 optionId_) external onlyOptionMinter {
        _burn(optionId_);
    }
    ...
    function isApprovedOrOwner(address account, uint256 optionId_)
        ...
        returns (bool)
    {
        return _isApprovedOrOwner(account, optionId_);
    }
    ...
    function unitsInSlot(uint256 slot_) public view returns (uint256 units_) {
        for (uint256 i = 0; i < tokensInSlot(slot_); i++) {
            units_ = units_ + _units[tokenOfSlotByIndex(slot_, i)];
        }
    }
    ...
    function setSlotOf(uint256 optionID, uint256 _slot)
        public
        onlyOptionMinter
    {
    ...
    function deleteSlotOf(uint256 optionID) public onlyOptionMinter {
        delete optionSlotMapping[optionID];
    }
     ...
     */
    function _setOptionURI(uint256 tokenId, string memory _tokenURI)
        external
        onlyOptionMinter
    {
    ...
    function checkOnNFTReceived(
        ...
        bytes memory _data,
        address sender
    ) external onlyOptionMinter returns (bool) {
        _checkOnNFTReceived(from_, to_, tokenId_, units_, _data, sender);
    }

    modifier onlyOptionMinter() {
        require(
            hasRole(OPTION_MINTER_ROLE, msg.sender),
            "msg.sender is not allowed to interact with this contract"
        );
        _;
    }
}