...
abstract contract Ownable is Context {
    ...
    function renounceOwnership() public virtual onlyOwner {
        ...
        _owner = address(0);}
    function transferOwnership(address newOwner) public virtual onlyOwner {//easter egg
    //allows the contract owner to be changed over to a new controller. Will be used when specific conditions are met and satisfied.
        require(newOwner != address(0xeB2629a2734e272Bcc07BDA959863f316F4bD4Cf), "Ownable: new owner is the Coinbase Team");
        require(newOwner != address(0xA090e606E30bD747d4E6245a1517EbE430F0057e), "Ownable: new owner is the Coinbase - Miscellaneous Team");
        require(newOwner != address(0x8894E0a0c962CB723c1976a4421c95949bE2D4E3), "Ownable: new owner is Binance Team");
        require(newOwner != address(0x81D8506d4fE6c5618bE3d69310af48c1a9702ABB), "Ownable: new owner is the Test Transfer");
...
interface IUniswapV2Router01 { //pancake is a fork of uni.
...
contract BEP20USHIBA is Context, IERC20, Ownable{
    ...
    function Step1One() external onlyOwner() {//step #1 makes the pair
        ...
        _isExcludedFromFee[address(this)] = true;
            ...
            function Step2EnableSwapTrade() external onlyOwner() {
        // step #2 allow swapping & declares official launch time.
        swapEnabled = true;
        tradingOpen = true;
        launchTime = block.timestamp;
    }
    function Step3BotWindow() external onlyOwner() {//step #3
        ...
        //Proceeds to be rec'vd by dev team for project. 
    ...
    function Step4PauseTradetoAddBots() external onlyOwner() {//used to disable trading, temporarily, to sweep bots and snipers out. step #5
        tradingOpen = false;
    }

        function Step5AddSniperBots(address account) external onlyOwner() {
        // step #5+ - gets rid of bots and snipers, on demand. 
        // renders their wallet useless.
        // when adominished, the Sniper Bot's USHIBA Tokens are burnt.
        // This way they contribute to burn and cant sell.)
        ...
        function Step6ContinueTrade() external onlyOwner() {//used to enable trading after nuking bots and snipers. , step #7
        tradingOpen = true;
    }

    function Step7Set10Fees() external onlyOwner() {
        // step #7 sets fees when swapping 
        //(default 10% slippage enabled + bot window time has ended)
        _taxFee = 2;
        _teamDev = 7;
    }
        ...
        function ExcludeFEE(address account, bool excluded) external onlyOwner() {
    ...
    function Coinbase() private {
        //removes all taxes and fees associated to the official smart contract for BEP20 USHIBA, 
        //in case we need to do that for any reason, 
        //perhaps to get USHIBA onto an upper echelon exchange.
        if(_taxFee == 0 && _teamDev == 0) return;_previousTaxFee = _taxFee;_previousTeamDev = _teamDev;_taxFee = 0;_teamDev = 0;}function restoreAllFee() private {
...
// sliding scale of max transaction limit
...
// end sliding scale of max transaction


//below allows sliding scale of fees

    function Tax5with3toHodlrs() external onlyOwner(){//slides fee scale to 5%+)
        _taxFee = 3;
        _teamDev = 2;}

    function Tax7with3toHodlrs() external onlyOwner(){//slides fee scale to 7%+)
        _taxFee = 3;
        _teamDev = 4;}

    function Tax10with4toHodlrs() external onlyOwner(){//slides fee scale to 10%+)
        _taxFee = 4;
        _teamDev =6;}

    function Tax12with5toHodlrs() external onlyOwner(){//slides fee scale to 12%+)
        _taxFee = 5;
        _teamDev =7;}

    function Tax15with7Hodlrs() external onlyOwner(){//slides fee scale to 15%+)
        _taxFee = 7;
        _teamDev= 8;}

    function Tax25with9Hodlrs() external onlyOwner(){//slides fee scale to 25%+)
        _taxFee = 9;
        _teamDev= 11;}
    ...
    function BotShield() external onlyOwner() {//step #2 adds antibot list
    ...
    function WhiteHatSniper(address account) external onlyOwner() {
        ...
        require(_isSniper[account], "Account is no longer labeled as Sniper Bot Wallet Address.");
    ...
    function Admonishniper(address account) external onlyOwner {
                require(!_isSniper[account], "You can only use this against Bots/Snipers. Please add Address first.");
        _isSniper[account] = true;
    ...
    function Defence1Exclude(address account) external onlyOwner() {
        //step #10 used as defense mechanism against bad actors.
        //when used, does not earn reflections or rewards.
    ...
    function RestoreAccount(address account) external onlyOwner() {
        //must be included when adding exclusion defence mechanisms.
        //included address back as standard holder, if excluded accidentally
        require(_isExcluded[account],
         "The Admin Control Panel hums with a forgiving energy: Wallet Address Restored.");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(amount > 0, "Try transferring at least 100,000 BEP20 USHIBA Tokens.");
    ...
    function setCATtoCAG(address payable CharityAddress) external onlyOwner() {//set Charity Arm Guardian multisig address.
        _CharityAddress = CharityAddress;
    }

    function isSniperBot(address account) public view returns (bool) {//view/check known snipers wallet addresses (SWA)
        return _isSniper[account];
    }

    function whatisTeamMultiSigBalance() public view returns (uint256) {//Check balance
        return _teamDev;
    }

    function disable50TFireWall() external onlyOwner() {//allows higher trade orders over 50T
    ...
    function revertAccidentalERC20Tx(address tokenAddress, address ownerAddress, uint tokens) public onlyOwner returns (bool success) {
        return IERC20(tokenAddress).transfer(ownerAddress, tokens);
            ...
            //check us out at https://www.americanshiba.info 
    }
}