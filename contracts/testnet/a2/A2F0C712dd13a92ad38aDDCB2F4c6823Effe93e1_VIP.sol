...
interface IRevenue{
    function rechargeToken(address from,address token,uint256 amount,uint256 _type) external;
}
...
contract VIP is Ownable {
    ...
    address inviation;
    event Buy(address toAddress,uint256 price,address tokenAddress,uint256 timestamp,address operator,address inviter,string data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v );
    ...
    function buy(address revenueAddress,uint256 price,address tokenAddress,uint256 timestamp,address inviter,string memory data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v) public payable {
        ...
        if(Iinvitation(inviation).isInvited(msg.sender)){
            require(Iinvitation(inviation).getParentAddress(msg.sender) == inviter,"inviter is not this address");
        }else{
            Iinvitation(inviation).addSublevel(inviter,msg.sender);
        }

        string memory signDataStr = string(abi.encodePacked(string(abi.encodePacked("0x",addressToStr(revenueAddress))),uint2str(price),string(abi.encodePacked("0x",addressToStr(tokenAddress))),uint2str(timestamp),data));
        
        // bytes32 signData  =keccak256(abi.encodePacked(signDataStr));
        bytes32 signResult = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32",keccak256(abi.encodePacked(signDataStr))));
        ...
        if(tokenAddress == address(0)){
            ...
            IERC20(tokenAddress).transferFrom(msg.sender,revenueAddress,price);
        }
        
        hashList[msghash] = true;

        IRevenue(revenueAddress).rechargeToken( msg.sender,tokenAddress,price,1);
        
        emit Buy(revenueAddress, price, tokenAddress, timestamp,msg.sender,inviter, data,msghash, r, s, v);
    }
