...
// File: contracts/TokenVestingV4.sol
...
contract TokenVestingV4 is Ownable {
  using SafeMath for uint256;
   ...
   * @param releaseTimestamps releasetime array
   * @param releaseAmounts releaseAmount array
   * @param token duration in seconds of the period in which the tokens will vest
   */
  constructor(
    address beneficiary,
 ...
 function beneficiary() external view virtual returns (address) {
    ...
    function getReleasedAmount() external view virtual returns(uint256){
    uint256 releaseAmount = 0;
    for (uint256 i = 0; i < _releases.length; i++) {
