...
/**
 * @title EarlyDistribution
 */
contract EarlyDistribution is AccessControl {
    ...
    address[] public vestingSchedulesAddresses;
    ...
    error InvalidReleasesCount();
    error InvalidAddress();
    error NotBeneficiary();
    error BeneficiaryAlreadyRevoked();
    error NoReleasableAmount();
    error InsufficientBalance();
    error ScheduleIrrevocable();
     ...
     */
    constructor(address _token, uint256 _start, uint256 _duration, uint256 _releasesCount, bool _revocable) {
        if (address(_token) == address(0)) revert InvalidTokenAddress();
        if (block.timestamp >= _start) revert InvalidStartTime();
        if (_duration <= 0) revert InvalidDuration();
        if (_releasesCount <= 0) revert InvalidReleasesCount();

        token = IERC20(_token);
        start = _start;
        duration = _duration;
        releasesCount = _releasesCount;
        finish = start + _releasesCount * _duration;
  ...
  /****************************|
    ...
    function hasSchedule(address _beneficiary) external view returns (bool) {
        return !vestingSchedules[_beneficiary].revoked && vestingSchedules[_beneficiary].beneficiary != address(0);
    }
    ...
    function createVestingSchedule(
        ...
        if (_totalAmount <= 0) revert InvalidDuration();

        VestingSchedule storage Schedule = vestingSchedules[_beneficiary];
        require(Schedule.totalAmount == 0, "ADDRESS_ALREADY_INITIALIZED");
    ...
    function claim() public {
        ...
        if (Schedule.revoked == true) revert BeneficiaryAlreadyRevoked();

        uint256 unreleased = _releasableAmount(msg.sender, Schedule.releasedAmount );
        if (unreleased <= 0) revert NoReleasableAmount();
        uint256 balance = token.balanceOf(address(this));
        if (balance < unreleased) revert InsufficientBalance();

        Schedule.releasedAmount = Schedule.releasedAmount + unreleased;
        vestingSchedulesTotalAmount = vestingSchedulesTotalAmount - unreleased;
        token.safeTransfer(payable(msg.sender), unreleased);
    ...
    function revokeVesting(address _beneficiary, address receiver) 
        ...
        onlyOperator
    {
        if (revocable == false) revert ScheduleIrrevocable();
        ...
        if (unreleased > 0){
          vestingSchedulesTotalAmount = vestingSchedulesTotalAmount - unreleased;
          token.safeTransfer(payable(receiver), unreleased);
        }
    ...
    function _vestedAmount(address _beneficiary) internal view returns (uint256) {
        ...
        if (currentTime  < start) {
            return 0;
        } else if (currentTime  >= finish || Schedule.revoked) {
            return Schedule.totalAmount;
        } else {
            uint256 timeLeftAfterStart = currentTime - start;
            uint256 availableReleases = timeLeftAfterStart / duration;
            uint256 tokensPerRelease = Schedule.totalAmount / releasesCount;
