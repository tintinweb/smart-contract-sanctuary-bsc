...
contract TokenDistributor is Ownable, Pausable, ReentrancyGuard {
  ...
  constructor(
    ...
    require(parties_.length == shares_.length, "TD: invalid parties and shares");
    require(parties_.length > 0, "TD: empty parties");

    uint16 totalShare = 0;
    for (uint256 i = 0; i < parties_.length; i++) {
      uint16 share = shares_[i];
      require(share > 0, "TD: zero share");
      partyInfo[parties_[i]].share = share;
      _parties.add(parties_[i]);
      totalShare += share;
    }

    require(totalShare == TOTAL_SHARE, "TD: invalid shares");
  ...
  function releaseToken() public onlyAdmin whenNotPaused {
    _updateTokenDistributor();

    uint256 totalTokens = ssp.balanceOf(address(this));
    for (uint256 i = 0; i < _parties.length(); i++) {
      uint256 amountDistributing = totalTokens * partyInfo[_parties.at(i)].share / TOTAL_SHARE;
  ...
  function _updateTokenDistributor() private {
    uint256 nonCirculatingSupply = ssp.maxTotalSupply() - ssp.totalSupply();
    ssp.mint(address(this), nonCirculatingSupply);
  }

  function _distributeTokens(address _party, uint256 _amount) internal nonReentrant {
    require(_amount > 0, "TD: zero amount");
    ssp.safeTransfer(_party, _amount);
    partyInfo[_party].tokensDistributedToParty += _amount;
