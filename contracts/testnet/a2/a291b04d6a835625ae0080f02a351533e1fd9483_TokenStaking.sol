...
contract TokenStaking is Ownable {
    ...
    function booster(uint256 boostAmount) public {
        ...
        require(
        users.boostlaval.boostCount < 8,
        "Not enough the boost chance."
        );

        uint256 depositAmount = getUserdipositAddamount(msg.sender);
        require(depositAmount >= 20, "Your balance is low of require balance");
        if (depositAmount > 20 ether && depositAmount <= 40 ether) {
        require(
        boostAmount >= 1 ether,
        ...
        users.boostlaval.boostDuration = 7  minutes;
        users.boostlaval.boostTime = block.timestamp;
        } else {
        users.boostlaval.ratePerDay = 25;
        users.boostlaval.boostCount = 1;
        ...
        } else if (depositAmount > 160 ether && depositAmount <=200 ether) {
        require(
        boostAmount >= 5 ether,
    ...
    function withdrawToken(uint256 id) public {
        ...
        users.userStoreReward += calculateReward(msg.sender);
        uint256 checkAmount = addAmount.mul(20).div(100);
        ...
        users.refferaladdress = 0x0000000000000000000000000000000000000000;
