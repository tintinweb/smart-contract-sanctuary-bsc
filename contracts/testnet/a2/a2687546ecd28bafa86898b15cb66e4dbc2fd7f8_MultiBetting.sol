...
contract MultiBetting is Ownable, ReentrancyGuard {
   ...
   using SafeERC20 for IERC20;
    ...
    uint256 public latestRound = 0;// latest round
     ...
     */
    function executeRound(uint256 roundID, Position _gameResult, uint256 _idSI) external onlyAgent {
        require(_notBettable(roundID), "Round bettable");
        ...
        emit Result(roundID, _gameResult);
    }
    ...
    function executeMultiRound(
        ...
        Position[] calldata _gameResult,
        uint256[] calldata _idSI) external onlyAgent {
    
        uint256 exeCount = roundID.length;
        for(uint256 i = 0; i < exeCount; i++){
            require(_notBettable(roundID[i]), "Round bettable");
            Round storage round = rounds[roundID[i]];

            if(round.totalAmountBet == 0 || 
                _gameResult[i] == Position.DrawOrRefund){

                round.gameResult = _gameResult[i];
                round.idSI = _idSI[i];
            }
            else{
                round.gameResult = _gameResult[i];
                round.idSI = _idSI[i];
                _calculateRewards(roundID[i]);
            }
            
            
            emit Result(roundID[i], _gameResult[i]);
        }
    }

     
    function singleStartRound(uint256 _Round) external onlyAgent {
        _startRound(_Round);
    }

    /**
     * @notice Start multiple round
     * @param epoch: epoch
     */
    function multiStartRound(uint256[] calldata epoch) external onlyAgent {
        uint256 roundCounts = epoch.length;
        for(uint256 i = 0; i < roundCounts; i++){
            Round storage round = rounds[epoch[i]];
            round.idGame = epoch[i];
            round.gameStatus = true;
            round.totalAmountBet = 0;
            latestRound += 1;

            emit StartRound(epoch[i]);
        }
    }
    ...
    /**
     * @notice Set multiple bettable status
     * @dev Callable by Agent
     */
    function _setMultiBettableStatus(uint256[] calldata epoch, bool[] calldata _status) external onlyAgent{
       uint256 roundStatus = epoch.length;
       for (uint256 i = 0; i < roundStatus; i++){
            rounds[epoch[i]].gameStatus = _status[i];
       }
    }
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        if (round.gameResult == Position.Meron) {
           ...
           rewardAmount = ads;
           treasuryAmt = totalBets - ((totalBets * fee) / 10000);
        }
        // Wala wins
        else if (round.gameResult == Position.Wala) {
        ...
        emit RewardsCalculated(epoch, rewardAmount, treasuryAmt);
    }
