...
contract iSWLegacyMutualNCV1_01 is BankSettingsFunctions {
    ...
    function internalCompFunction(address adr, uint256 amtComp, bool isBWC, bool isAuto) internal {
        ...
        require(amtComp <= getMyLeXC(adr), "You CANNOT comp more than what you have!");
        ...
        if(block.timestamp.sub(user.lastCompTime) >= bonusCompTimer &&
    ...
    function stopAutoComp() external nonReentrant {
        require(initialized);
        Automation storage aUser = automations[msg.sender];
        require(aUser.WA == msg.sender, "You are NOT the account holder!");
        ...
        for(uint256 j = 1; j <= (compTimes + 1); j++ ) {
            ...
            //Makes sure no bonus comp gets counted if the timeAutoWasOn is less than the bonus comp timer.
            if(timeAutoWasOn >= bonusCompTimer){
