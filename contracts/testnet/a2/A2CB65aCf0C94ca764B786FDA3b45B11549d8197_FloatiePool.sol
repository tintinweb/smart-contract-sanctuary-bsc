...
// File: contracts/FloatieVault.sol
...
interface IMasterChefV2 {
    ...
    function pendingFloatie(uint256 _pid, address _user) external view returns (uint256);
...
contract FloatiePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 floatieAtLastUserAction; // keep track of floatie deposited at the last user action.
    ...
    IERC20 public immutable token; // floatie token.
    IBonusToken public immutable bonusToken; // splashie token.

    uint256 public bonusMultiplier = 100; // x1 multiplier for splashie

    IMasterChefV2 public immutable masterchefV2;

    address public boostContract; // boost contract used in Masterchef.
    address public VFloatie;
    ...
    event NewVFloatieContract(address VFloatie);
    ...
    /**
     * @notice Constructor
     * @param _token: Floatie token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: floatie pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
        IBonusToken _bonusToken,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(floatiePoolPID, balance);
        emit Init();
    }
    ...
    function changeBonusMultiplier(uint256 _multiplier) external onlyAdmin {
        bonusMultiplier = _multiplier;
    }

    /**
     * @notice Checks if the msg.sender is either the floatie owner address or the operator address.
     */
    modifier onlyOperatorOrFloatieOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or floatie owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onFloatiePoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.floatieAtLastUserAction;
    ...
    /**
     * @notice Unlock user floatie funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrFloatieOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Floatie Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VFloatie != address(0)) {
            IVFloatie(VFloatie).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.floatieAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Floatie Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VFloatie != address(0)) {
            IVFloatie(VFloatie).withdraw(msg.sender);
        }
        ...
        bonusToken.mint(msg.sender, currentAmount * bonusMultiplier / 100);

        if (user.shares > 0) {
            user.floatieAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.floatieAtLastUserAction = 0;
        }
    ...
    function harvest() internal {
        uint256 pendingFloatie = masterchefV2.pendingFloatie(floatiePoolPID, address(this));
        if (pendingFloatie > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(floatiePoolPID, 0);
    ...
    /**
     * @notice Set VFloatie Contract address
     * @dev Callable by the contract admin.
     */
    function setVFloatieContract(address _VFloatie) external onlyAdmin {
        require(_VFloatie != address(0), "Cannot be zero address");
        VFloatie = _VFloatie;
        emit NewVFloatieContract(VFloatie);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Floatie Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingFloatieRewards();
     ...
     * @return Returns total pending floatie rewards
     */
    function calculateTotalPendingFloatieRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingFloatie(floatiePoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingFloatieRewards()) * (1e18)) / totalShares);
    }
