...
contract VTC is IERC20, Ownable  {
    ...
    constructor(address tokenOwner) {
        _name = "virtual treasure coin";//全称
        _symbol = "FRP";//简称
        _decimals = 18;//精度
    ...
    function _tokenTransfer(
        ...
        address cur;
        uint256 transnviterFee = 0;
        if (takeFee) {
            cur = inviter[sender];
            bool flag = _isExcludedFromFee[cur];
            if (cur != address(0) && flag==false){
                transnviterFee = 10 **_decimals;
                transnviterFee = transnviterFee.div(1000);
                _takeTransfer(
                        sender,
                        cur,
                        transnviterFee,
        ...
        _rOwned[recipient] = _rOwned[recipient].add(
           rAmount.sub(transnviterFee)
        );
        emit Transfer(sender, recipient, tAmount.sub(transnviterFee));
    }
