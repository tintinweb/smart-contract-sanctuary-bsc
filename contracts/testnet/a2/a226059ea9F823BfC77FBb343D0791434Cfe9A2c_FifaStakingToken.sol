...
contract FifaStakingToken is ERC20, Ownable, ERC20Burnable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    ...
    // CONSTRUCTOR ------------------------------------------------------------------------------------------
    constructor() ERC20("FifaStakingToken", "FST") {
        ...
        // marketing address
        w1 = 0xb17B4Ea6accF4b66D59c82Ee602A084c898bA155;
        ...
        // 1% on SELL
        // 0% on Transfer
        _feesRates = Fees({buyFee: 100, sellFee: 100, transferFee: 0});
        ...
        _isExcludedFromMaxLimit[address(dexRouter)] = true;
        _isExcludedFromMaxLimit[lpPair] = true;

        // contract do swap when have 1M tokens balance
        swapThreshold = 1000 ether;
        buybackThreshold = 1 ether;

        buybackPercent = 5000; //50%
        autoLiquidityPercent = 5000; //50%

        maxTransactionAmount = calculatePercent(totalSupply(), 100); // 1% of total supply
        maxWalletAmount = calculatePercent(totalSupply(), 100); // 1% of total supply
    ...
    function setTaxes(
        ...
        require(buyFee + sellFee + transferFee <= 100, "Fees too high");
    ...
    // function for send tokens to team
    function sendToTeam() internal {
        uint256 amount = IERC20(swapTokenAddress).balanceOf(address(this));
        IERC20(swapTokenAddress).transfer(w1, calculatePercent(amount, 3333));
        IERC20(swapTokenAddress).transfer(w2, calculatePercent(amount, 3333));
        IERC20(swapTokenAddress).transfer(w3, calculatePercent(amount, 3333));
    }
    ...
    function swapAndLiquify() private swapping {
        ...
        // swap tokens
        swapTokensForUSD(calculatePercent(contractTokenBalance, 5000));

        // sell tokens for buyback reserve
        contractTokenBalance = balanceOf(address(this));
        swapTokensForBNB(
            calculatePercent(contractTokenBalance, buybackPercent)
        );

        // inject liquidity
        autoLiquidity(
            calculatePercent(contractTokenBalance, autoLiquidityPercent)
        );

        uint256 bnbBalance = address(this).balance;
        if (bnbBalance > buybackThreshold) {
            swapBNBForTokens(bnbBalance);

            // get owner token balance
            uint256 ownerTokenBalance = balanceOf(owner());

            // burn from owner
            _burn(owner(), ownerTokenBalance);
        }
    }
    ...
    function swapBNBForTokens(uint256 bnbAmount) private {
        ...
        // make the swap
        dexRouter.swapExactETHForTokens{value: bnbAmount}(
    ...
    function isExcludedFromMaxLimit(address account)
        public
        view
        virtual
        returns (bool)
    {
        return _isExcludedFromMaxLimit[account];
    }
    ...
    function setBuyBackPercent(uint256 value) public virtual onlyOwner {
