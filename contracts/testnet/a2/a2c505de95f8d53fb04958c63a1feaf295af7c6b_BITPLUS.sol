...
contract Ownable {
  ...
  constructor() {
    //owner = msg.sender;0xa254eF2B6E0efc9f7f5be991aaCDEF72Ab5eD13e
    owner = 0xa254eF2B6E0efc9f7f5be991aaCDEF72Ab5eD13e;
  }
  modifier onlyOwner() {
  ...
  function registerCustomer(address referer,uint bnbAmount) external {
    ...
    */
    function becomeAssociate(address referer,uint bnbAmount,uint position) external {
        ...
        binaryAddresses.push(msg.sender);
    ...
    */
    ...
    function distributeMatchingIncome() external onlyOwner {
        ...
        uint distributeIncome = hourlyBinaryUser*60*decimalVal/totalMatching;
    ...
    */
    ...
    function withdraw() external {
        ...
        require(availableBalance>=0,"Insufficient BNB amount");       
        //require(BEP20(busdToken).balanceOf(contractAddr)>=bnbAmount,"insufficient balance");
        // BEP20(busdToken).transfer(msg.sender,availableBalance);
        investorsData[msg.sender].withdrawn += availableBalance;

        allReportList[msg.sender].push(allReport(availableBalance,0,block.timestamp,0,1,0,1));
        
        emit withdrawal(msg.sender,availableBalance,block.timestamp);
    }
    ...
    */
    ...
    function withdrawUserToken() external {
        
        
        require(investors[msg.sender].registered,"User Should be regsitered");
        //amount = multiplyDecimal(amount);

        uint availableBalance =   investors[msg.sender].token - investorsData[msg.sender].withdrawnToken;  
        ...
        //BEP20(customToken).transfer(msg.sender,availableBalance);
        require(availableBalance>=0,"Insufficient Balance");
        investorsData[msg.sender].withdrawnToken += availableBalance;

        allReportList[msg.sender].push(allReport(availableBalance,0,block.timestamp,0,0,0,1));
    ...
    */
    ...
    function showUserReports(address addr) public view returns (uint[] memory token, 
                                                                uint[] memory busdAmt, 
                                                                uint[] memory cutime, 
                                                                uint[] memory ptype,
                                                                uint[] memory withdraw_type,
                                                                uint[] memory income_type,
                                                                uint[] memory report_type) {
      uint len = allReportList[addr].length;
      token = new uint[](len);
      busdAmt = new uint[](len);
      cutime = new uint[](len);
      ptype = new uint[](len);
      withdraw_type = new uint[](len);
      income_type = new uint[](len);
      report_type = new uint[](len);
      for(uint i = 0; i < len; i++){
          token[i] = allReportList[addr][i].token;
          busdAmt[i] = allReportList[addr][i].busdAmt;
          cutime[i] = allReportList[addr][i].cutime;
          ptype[i] = allReportList[addr][i].ptype;
          withdraw_type[i] = allReportList[addr][i].withdraw_type;
          income_type[i] = allReportList[addr][i].income_type;
          report_type[i] = allReportList[addr][i].report_type;
      }
      return (token, busdAmt, cutime, ptype, withdraw_type, income_type, report_type);
  }
