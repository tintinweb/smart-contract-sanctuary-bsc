    ...
    PausableUpgradeable
{
    ...
    event Head2HeadFundsSentToTreasury(uint256 amount);
    event Head2HeadBetLost(
        uint256 gameId,
        address better,
        uint256 index,
        bytes12 stockId
     ...
     * @dev Reverts when _updateMulAfterAmountInWei is 0 or
     * reverts when start timestamp < current timestamp or
     * reverts when start timestamp > end timestamp or
     * @param _stockIds bytes12 array of stock IDs
     ...
     * @dev Reverts when
     * (Game Is Not Created ||
     *  betting stockId not in game ||
     *  Given stock Not Listed In This Game ||
     *  Caller bet is less than minimum bet value or greater than max bet value)
     ...
     * Revert if game is already drawn,
     * Revert when game is not ended ,
     * Revert when winning stock ID is not present in this game ,
     ...
     * Revert when game is already drawn ,
     * Revert when rewards are already distributed ,
     * Emit event
        ...
        onlyRole(OPERATOR_ROLE)
    {
        ...
        if (
            head2headGames[_gameId].startGameStockPricesInWei[
                head2headGames[_gameId].stockIds[0]
            ] == 0
        ) {
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            for (uint256 j = 0; j < numOfBetsPlaced; ++j) {
                ...
                if (stockId == winningStockId) {
                    ...
                    emit Head2HeadBetLost(_gameId, betters[i], j, stockId);
                }
            }
        }
    ...
    function distRewardParams(
        ...
        bytes12 winningStockId = head2headGames[_gameId].winningStockId;
        bytes12[2] memory stockIds = head2headGames[_gameId].stockIds;
        ...
        if (!head2headGames[_gameId].rewardsDistributed) {
            head2headGames[_gameId].rewardsDistributed = true;
        }
        // UPDATE BETTER'S INFO IF THEY WIN AND DISTRIBUTE REWARDS
        for (uint256 i = 0; i < winners.length; ++i) {
            ...
            for (uint256 j = 0; j < numOfBetsPlaced; ++j) {
                ...
                if (bet.stockId == winningStockId && !bet.win) {
                    ...
                    emit Head2HeadBetLost(_gameId, winners[i], j, bet.stockId);
                }
            }
        }
    }
    ...
    function claimRewards(uint256 _gameId, address recepient) external {
        require(
            head2headGames[_gameId].rewardsDistributed,
            "Head2Head: Only after RewardDistribution"
        );
        address better = _msgSender();
        uint256 numOfBetsPlaced = bets[_gameId][better].length;
        bytes12 winningStockId = head2headGames[_gameId].winningStockId;
        for (uint256 j = 0; j < numOfBetsPlaced; ++j) {
            bytes12 stockId = bets[_gameId][better][j].stockId;
            if (stockId == winningStockId) {
                if (!bets[_gameId][better][j].win) {
                    ...
                    require(
                        ...
                        "Head2Head: Invalid Recepient"
                    );

                    // better.transfer(_winAmount);
                    emit Head2HeadGameWinAmountSent(
                        _gameId,
                        better,
                        j,
                    ...
                    revert("Head2Head: Rewards already claimed");
                }
            } else {
                emit Head2HeadBetLost(_gameId, better, j, stockId);
            }
        }
    }
        ...
        onlyRole(OPERATOR_ROLE)
    {
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            for (uint256 j = 0; j < numOfBetsPlaced; ++j) {
                ...
                _transferRewardsFromTreasury(betters[i], betAmount);
    ...
    function transferBetsToTreasury(uint256 amount) internal {
        ...
        require(success, "Head2Head:Treasury transfer Failed");
        emit Head2HeadFundsSentToTreasury(amount);
    }
    ...
    function getGameBetters(
        ...
        uint256 totalNumberOfBetters = head2headGames[_gameId].betters.length;
