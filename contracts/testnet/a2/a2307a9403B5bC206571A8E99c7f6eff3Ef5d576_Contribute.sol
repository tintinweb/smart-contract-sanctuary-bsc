...
contract Contribute is Context,Auth{
  ...
  function contribution(uint256 packageid,address referto) external noReentrant() returns (bool) {
    ...
    uint256 contractusd = a.balanceOf(address(this)).add(investamount);
  ...
  function claimMonth(uint256 month) external noReentrant() returns (bool) {
    require(!claiming);
    claiming = true;
    claimtopshared(month);
    claiming = false;
    return true;
  }
  ...
  function claimAll() external noReentrant() returns (bool) {
    ...
    while(i<thismounth()){
      if(
        topshared_exit[i]==false &&
        block.timestamp.sub(monthy_split)>i.mul(monthy_split).add(monthy_start) &&
        topshared_address[i]==msg.sender
      ){
      claimtopshared(i);
      i++;
      }    
  ...
  function getunclaim() external view returns (uint256) {
    uint256 i;
    uint256 result;
    while(i<thismounth()){
      if(
      ...
      result = result.add(wallet_topshared[i]);
      i++;
      }    
    }
    return result;
  ...
  function sortsharedscore(address account) internal {
    ...
    }else if ( previous1 < score && topshared_address[thismounth()] != account ) {
        secshared_address_4[thismounth()] = secadr3;
        secshared_address_3[thismounth()] = secadr2;
        secshared_address_2[thismounth()] = secadr1;
        secshared_address_1[thismounth()] = account;
    }else if ( previous2 < score && secshared_address_1[thismounth()] != account ) {
        secshared_address_4[thismounth()] = secadr3;
        secshared_address_3[thismounth()] = secadr2;
        secshared_address_2[thismounth()] = account;
    }else if ( previous3 < score && secshared_address_2[thismounth()] != account) {
        secshared_address_4[thismounth()] = secadr3;
        secshared_address_3[thismounth()] = account;
    }else if ( previous4 < score && secshared_address_3[thismounth()] != account) {
  ...
  function claimtopshared(uint256 month) internal {
