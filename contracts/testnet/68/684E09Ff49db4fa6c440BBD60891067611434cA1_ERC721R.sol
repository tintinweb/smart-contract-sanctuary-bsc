...
import "./internal/PieceCombine.sol";
  ...
  PieceCombine
{
  ...
  function burn2Mint(address account_, uint8 attribute_, uint8 rarityNum_) external {
    uint256[] memory burnIds = updateAfterClaimed(account_, attribute_, rarityNum_);
    ...
    uint256 tokenId;
    unchecked {
      tokenId = (++tokenIdTracker << 6) | (5 << 3) | 0;
    }

    _mint(account_, tokenId);

    emit Unboxed(account_, tokenId, 0, 5);
  }
...
contract PieceCombine {
  uint256 public constant NUM_RARITIES = 5;
  uint256 public constant NUM_ATTRIBUTES = 10;

  mapping(address => uint8[NUM_ATTRIBUTES]) public rarityBitMap;
  mapping(address => mapping(uint8 => mapping(uint8 => uint256[]))) public collectibles;
  mapping(address => uint8[NUM_ATTRIBUTES][NUM_RARITIES]) public collectibleCounter;

  function update(address account_, uint256 tokenId_) internal {
    (uint8 rarity, uint8 attribute) = infoOf(tokenId_);
    ++collectibleCounter[account_][attribute][rarity];
    collectibles[account_][attribute][rarity].push(tokenId_);
    rarityBitMap[account_][attribute] |= uint8(1 << rarity);
  }

  function pieces(address account_, uint8 rarity_, uint8 attribute_) external view returns (uint256[] memory) {
    return collectibles[account_][attribute_][rarity_];
  }

  function infoOf(uint256 tokenId_) public pure returns (uint8 rarity, uint8 attribute) {
    uint256 mask = ((1 << 3) - 1);
    rarity = uint8(tokenId_ & mask);
    attribute = uint8((tokenId_ >> 3) & mask);
  }

  function isCombinable(address account_, uint8 attribute_, uint8 rarityNum_) public view returns (bool) {
    return rarityBitMap[account_][attribute_] == (1 << rarityNum_) - 1;
  }

  function updateAfterClaimed(
    address account_,
    uint8 attribute_,
    uint8 rarityNum_
  ) internal returns (uint256[] memory burnIds) {
    require(isCombinable(account_, attribute_, rarityNum_), "NFT: PIECES_MISSING");

    uint8[NUM_ATTRIBUTES] memory collected = collectibleCounter[account_][attribute_];
    uint8 bitmap = rarityBitMap[account_][attribute_];
    uint8 length = rarityNum_;
    burnIds = new uint256[](length);
    uint256 groupLength;
    for (uint8 i; i < length; ) {
      // remove number of pieces each attribute
      --collected[i];
      // remove attribute bit if collected[i] == 0
      if (collected[i] == 0) bitmap &= uint8(~(1 << i));

      // remove tokenId by each attribute group
      groupLength = collectibles[account_][attribute_][i].length;
      burnIds[i] = collectibles[account_][attribute_][i][groupLength - 1];
      collectibles[account_][attribute_][i].pop();
    ...
    rarityBitMap[account_][attribute_] = bitmap;
    collectibleCounter[account_][attribute_] = collected;
  }
}