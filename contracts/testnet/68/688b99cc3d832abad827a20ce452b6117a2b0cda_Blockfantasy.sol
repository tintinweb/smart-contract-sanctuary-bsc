 ...
 */
 ...
 contract Blockfantasy is Ownable{
     ...
     mapping(uint256 => mapping(address => bool)) private claimedcanceledpool;
    ...
    function Joinevent(
        ...
        uint256[11] calldata _playersselected,
        uint256 pool
    ) public payable{
        ...
        claimedcanceledpool[pool][user] = false;
    }
    ...
    function Createteam(
        ...
        uint256 vicecaptain,
        uint256[11] calldata playersselected
        ) internal {
             require(captain > 0, "You must have a captain");
             require(vicecaptain > 0, "You must have a vice-captain");
             require(playersselected.length > 11, "You must have 11 selected players");
            currentteamid++;
    ...
    function editSelectedPlayers(uint256 eventID, uint256 team, uint256[11] calldata playersselected) public{
    ...
    function returnentryfee(uint256 poolid, address user) public {
        require(pools[poolid].canceled = true, "Pool has not been canceled");
        require(claimedcanceledpool[poolid][user] == true, "You have claimed pool");
        uint256 fee = pools[poolid].entryfee;
        payable(user).transfer(fee);
        claimedcanceledpool[poolid][user] = true;
    }
    ...
    function getEvent(uint256 eventID) public view returns (string memory, uint256[] memory, uint[] memory, uint256, uint256) {
    ...
    function getTeamdetails(uint256 team, uint256 eventID) public view returns (address, string memory, uint256, uint256, uint256[11] memory, uint256) {
    ...
    function getallteampoint(uint256 eventID, uint256 poolid) public {//should this be called by admin
        uint256[] memory boy = pools[poolid].teamsarray;
        for (uint256 i = 0; i < boy.length; i++){
            boy[i];
            uint256[11] memory tip=teams[eventID][boy[i]].selectedplayers;
            geteachteampoint(tip, eventID, boy[i], poolid,teams[eventID][boy[i]].selectedcaptain, teams[eventID][boy[i]].selectedvicecaptain);
        }
    }

    function geteachteampoint(uint256[11] memory userarray, uint256 eventID, uint256 team, uint256 pool, uint256 cpid, uint256 vpid) public returns (uint256) {
        for (uint256 i = 0; i < userarray.length; i++){
            ...
            uint256 removedcaptainandvice = totalteampoint.sub(removedpoint);
            teampoint = removedcaptainandvice.add(captainpoint).add(vicecaptainpoint);
            teams[eventID][team].teamscore;
