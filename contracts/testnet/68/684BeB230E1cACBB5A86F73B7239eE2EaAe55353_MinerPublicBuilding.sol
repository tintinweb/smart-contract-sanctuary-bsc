...
import "./interfaces/IMinerPublicBuilding.sol";

import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";

contract MinerPublicBuilding is Distributor, IMinerPublicBuilding {
    uint256 public minerMintingPriceInBNB;
    IActivatableNFT minerNFTContract;
    address municipalityAddress;
    address public brokerAddress;

    modifier onlyMunicipality() {
        require(
            msg.sender == municipalityAddress,
    ...
    modifier onlyBroker() {
        require(
            ...
            "MinerPublicBuilding: Caller is not a broker"
        );
        _;
    }

    function initialize(IERC20Upgradeable _distributionTokenAddress)
        external
        initializer
    {
    ...
    function mintMiners(address _user, uint _minersCount)
        external
        onlyMunicipality
        returns (uint256 tokenId)
    {
        require(
            address(minerNFTContract) != address(0),
            "MinerPublicBuilding: MinerNFTContract address is not initialized"
        );
        tokenId = minerNFTContract.totalSupply() + 1;
        for(uint i; i < _minersCount; i++){
            minerNFTContract.mintNFTTo(_user);
        }
    }


    function mintMiner(address _user)
        external
        onlyMunicipality
        returns (uint256 tokenId)
    {
        ...
        tokenId = minerNFTContract.mintNFTTo(_user);
    }

    function setMunicipalityAddress(address _municipality) external onlyOwner {
        municipalityAddress = _municipality;
    }


    function setBrokerContract(address _brokerAddress) external onlyOwner {
        require(
            _brokerAddress != address(0),
            "MinerPublicBuilding: Can't give 0x00 address"
        );
        require(
            brokerAddress != _brokerAddress,
            "MinerPublicBuilding: Broker address is same"
        );
        brokerAddress = _brokerAddress;
    }


    function setMinerNFTContract(IActivatableNFT _minerNFTContract)
        external
        onlyOwner
    {
        minerNFTContract = _minerNFTContract;
    }

    function setMinerMintingPrice(uint256 _minerMintingPriceInBNB)
        external
        onlyOwner
    {
        minerMintingPriceInBNB = _minerMintingPriceInBNB;
    }

        function addNFTStaking(address _userAddress) external onlyBroker {
        _addNFTStaking(_userAddress);
    }

    function removeNFTStaking(address _userAddress) external onlyBroker {
        _removeNFTStaking(_userAddress);
    }
}
