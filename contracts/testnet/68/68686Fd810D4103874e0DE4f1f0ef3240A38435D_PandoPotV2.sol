...
contract PandoPotV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _generateMegaTicket(uint256 _rand, uint256 _sample) internal view returns(uint256) {
        if (nSeeds > 0) {
            return _rand * seeds[_rand % nSeeds] % _sample;
        }
        return _rand % _sample;
    }

    function _generateMinorTicket(uint256 _rand, uint256 _sample, uint256 _nSeeds) internal view returns(uint256, uint256) {
        if (nSeeds > 0) {
            return (_rand * seeds[_rand % _nSeeds] % _sample, _rand * seeds[(_rand % _nSeeds + _nSeeds) / 2] % _sample);
        }
        return (_rand % _sample, _rand / _sample % _sample);
    }
    ...
    function _updateRound(uint256 _id) internal {
        ...
        if (_roundInfo.status == 0) {
            ...
            emit RoundPrize(_id, _megaUSD, _megaPSR, _minorUSD1, _minorPSR1, _minorUSD2, _minorPSR2);
        }
    }
    ...
    function _updateLuckyNumber() internal {
        ...
        emit RoundCompleted(currentRoundId, currentMegaNumber, currentMinorNumber1, currentMinorNumber2);
    }
    function updateLuckupNumberForQC(uint256 _megaNumber, uint256 _minorNumber1, uint256 _minorNumber2) external onlyOwner{
        if (currentRoundId > 0) {
            seeds.push(currentMegaNumber);
            seeds.push(currentMinorNumber1);
            seeds.push(currentMinorNumber2);
        ...
        currentMinorNumber1 = _minorNumber1 % minorSampleSpace;
        currentMinorNumber2 = _minorNumber2 % minorSampleSpace;
        RoundInfo storage _roundInfo = roundInfo[currentRoundId-1];
            _roundInfo.megaNumber = currentMegaNumber;
            _roundInfo.minorNumber1 = currentMinorNumber1;
            _roundInfo.minorNumber2 = currentMinorNumber2;
            _roundInfo.finishedAt = block.timestamp;
    ...
    function _calcMegaPrize(uint256 _roundId, uint256 _megaNumber) internal returns(uint256, uint256) {
        PrizeInfo memory _prize = PrizeInfo({
            ...
            nClaimed: 0,
            totalWining: nMegaTickets[_roundId][_megaNumber]
        });
        if (_prize.totalWining > 0) {
    ...
    function _calcMinorPrize(uint256 _roundId, uint256 _minorNumber) internal returns(uint256, uint256) {
        ...
        uint256 _totalWining = nMinorTickets[_roundId][_minorNumber];
        if ( _totalWining > 0) {
            ...
            _prize.totalWining += _totalWining;
            pendingUSD += _prize.USD;
            PSRForCurrentPot -= _prize.PSR;
        }
    ...
    function _liquidate(uint256 _type, uint256 _roundId, address _owner) internal {
        ...
        if (_type == 0 || _type == 1) {
            ...
            require(_megaPrize.expire < block.timestamp, 'PandoPot: !expire');
            require(_minorPrize.expire < block.timestamp, 'PandoPot: !expire');

            _totalUSD = _megaPrize.USD * (_megaPrize.totalWining - _megaPrize.nClaimed) / _megaPrize.totalWining +
                        _minorPrize.USD * (_minorPrize.totalWining - _minorPrize.nClaimed) / _minorPrize.totalWining;

            _totalPSR = _megaPrize.PSR * (_megaPrize.totalWining - _megaPrize.nClaimed) / _megaPrize.totalWining +
                        _minorPrize.PSR * (_minorPrize.totalWining - _minorPrize.nClaimed) / _minorPrize.totalWining;
        } else {
    ...
    function enter(address _receiver, uint256 _rand, uint256 _quantity) external whenNotPaused nonReentrant onlyWhitelist() {
        uint256 _megaTicket;
        uint256 _minorTicket1;
        uint256 _minorTicket2;
        uint256 _currentRoundId = currentRoundId;
        uint256[] memory _megaTickets = new uint[](_quantity);
        uint256[] memory _minorTickets = new uint[](_quantity * 2);
        uint256 _sampleSpace = megaSampleSpace;
        uint256 _minorSpace = minorSampleSpace;
        uint256 _nSeeds = nSeeds;
        uint256 _salt = block.timestamp;
        for (uint256 i = 0; i < _quantity; i++) {
            _megaTicket = _generateMegaTicket(_rand, _sampleSpace);
            (_minorTicket1, _minorTicket2) = _generateMinorTicket(_rand, _minorSpace, _nSeeds);

            megaTickets[_currentRoundId][_megaTicket][_receiver]++;
            minorTickets[_currentRoundId][_minorTicket1][_receiver]++;
            minorTickets[_currentRoundId][_minorTicket2][_receiver]++;

            nMegaTickets[_currentRoundId][_megaTicket]++;
            nMinorTickets[_currentRoundId][_minorTicket1]++;
            nMinorTickets[_currentRoundId][_minorTicket2]++;
            _megaTickets[i] = _megaTicket;
            _minorTickets[i] = _minorTicket1;
            _minorTickets[i + _quantity] = _minorTicket2;
            _rand = (_rand + i) * (_salt + i) % _sampleSpace;
        }
    ...
    function claim(uint256 _type, uint256 _roundId, uint256 _ticketNumber, address _receiver) external whenNotPaused nonReentrant {
        ...
        if (_type == 0) {
            ...
            if (_prizeInfo.expire >= block.timestamp) {
                ...
                uint256 _totalWiningTicket = _prizeInfo.totalWining;
                _USDAmount = _prizeInfo.USD * _nWiningTicket / _totalWiningTicket;
                _PSRAmount = _prizeInfo.PSR * _nWiningTicket / _totalWiningTicket;
                _prizeInfo.nClaimed++;
            } else {
    ...
    function distribute(address[] memory _leaderboards, uint256[] memory ratios) external onlyWhitelist whenNotPaused {
        ...
        PSRForPreviousPot = 0;
        lastDistribute = block.timestamp;
        emit Distributed(currentDistributeId, _leaderboards);
    }
    ...
    modifier onlyWhitelist() {
        require(whitelist[msg.sender] == true, 'PandoPot: caller is not in the whitelist');
        _;
    }
    ...
    event NewMegaTicket(uint256 roundId, address user, uint256[] number);
    event NewMinorTicket(uint256 roundId, address user, uint256[] numberx);
    ...
    event RoundCompleted(uint256 roundId, uint256 megaNumber, uint256 minorNumber1, uint256 minorNumber2);
    event RoundPrize(uint256 roundId, uint256 megaUSD, uint256 megaPSR, uint256 minorUSD1, uint256 minorPSR1, uint256 minorUSD2, uint256 minorPSR2);
    event RoundIdUpdated(uint256 newRoundId);
    event Distributed(uint256 distributeId, address[] leaderboards);
