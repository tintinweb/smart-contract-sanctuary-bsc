...
contract MintNft is ERC1155URIStorage, ReentrancyGuard {
    ...
    //     uint256 finalAmount;
    //     address nftOwner = _nftOwner;
    //     finalAmount = PriceBatch(_currentIds, _howMany);
    //     require(_amount == finalAmount, "amount");
    ...
    //         uint256 bnbPrice = toBnbPriceBatch(_currentIds, _howMany);
    //         require(msg.value >= bnbPrice, "wrong price");
    //         (bool sent, ) = ownerAddress.call{value: msg.value}("");
    //         require(sent, "send Ether");
    //         nftOwner = msg.sender;
    //     }
    //     _mintBatch(nftOwner, _currentIds, _howMany, "");
    //     for (uint256 i = 0; i < _currentIds.length; i++) {
    //         TxID++;

    //         string memory metaData = AllNfts[uint16(_currentIds[i])].metaData;
    //         Transaction memory newTransaction = Transaction(
    //             metaData,
    //             nftOwner,
    //             uint8(_currentIds[i]),
    //             _howMany[i],
    //             AllNfts[uint16(_currentIds[i])].amount,
    //             uint8(_currentIds[i])
    //         );
    //         transactions[TxID] = newTransaction;
    //     }
    ...
    // function toBnbPriceBatch(
    //     uint256[] memory _tokenIds,
    //     uint256[] memory _howMany
    // ) public view returns (uint256) {
    //     uint256 nftPrice = PriceBatch(_tokenIds, _howMany);
    //     return BnbPriceFeed(nftPrice);
    // }

    // function PriceBatch(uint256[] memory _tokenIds, uint256[] memory _howMany)
    //     public
    //     view
    //     returns (uint256)
    // {
    ...
    //         uint256 nftPrice = AllNfts[uint16(_tokenIds[i])].amount;
    //         nftPrice = nftPrice * _howMany[i];
    //         require(nftPrice == 0, "level");
    //         total += nftPrice;
    //     }
    //     return total;
    // }
