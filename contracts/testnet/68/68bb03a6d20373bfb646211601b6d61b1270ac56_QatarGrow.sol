...
contract QatarGrow is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 liquidityFeeOnBuy, uint256 marketingFeeOnBuy, uint256 burnFeeOnBuy);
    event UpdateSellFees(uint256 liquidityFeeOnSell, uint256 marketingFeeOnSell, uint256 burnFeeOnSell);
    event SwapAndLiquify(uint256 tokensSwapped,uint256 bnbReceived,uint256 tokensIntoLiqudity);
    event SwapAndSendMarketing(uint256 tokensSwapped, uint256 bnbSend);
    event SwapTokensAtAmountUpdated(uint256 swapTokensAtAmount);

    constructor () ERC20("QatarGrow", "QatarGrow") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        _totalFeesOnBuy    = liquidityFeeOnBuy  + marketingFeeOnBuy + burnFeeOnBuy;
        _totalFeesOnSell   = liquidityFeeOnSell + marketingFeeOnSell + burnFeeOnSell;

        marketingWallet = 0x340eF0849E7Bd73B31930c3C09458d007216a61d;
        ...
        maxWalletAmount = totalSupply() * 20 / 1000; // 1%
    }
    ...
    function updateBuyFees(uint256 _liquidityFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _burnFeeOnBuy) external onlyOwner {
        ...
        emit UpdateBuyFees(liquidityFeeOnBuy, marketingFeeOnBuy, burnFeeOnBuy);
    }

    function updateSellFees(uint256 _liquidityFeeOnSell, uint256 _marketingFeeOnSell, uint256 _burnFeeOnSell) external onlyOwner {
        ...
        emit UpdateSellFees(liquidityFeeOnSell, marketingFeeOnSell, burnFeeOnSell);
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (canSwap &&
            ...
            uint256 burnShare = burnFeeOnBuy + burnFeeOnSell;
            ...
            if (burnShare > 0) {
                uint256 burnTokens = contractTokenBalance * burnShare / totalFee;
                super._transfer(from, address(0xdead), burnTokens);
            }               
