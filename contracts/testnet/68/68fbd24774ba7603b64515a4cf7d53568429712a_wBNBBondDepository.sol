...
contract wBNBBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    IERC20 public immutable Marshmallow; // token given as payment for bond
    IWBNB9 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Marshmallow when receives principle
    ...
    constructor ( 
        address _Marshmallow,
        address _principle,
        address _treasury, 
        address _DAO,
        address _feed
    ) {
        require( _Marshmallow != address(0) );
        Marshmallow = IERC20(_Marshmallow);
        require( _principle != address(0) );
        principle = IWBNB9(_principle);
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Marshmallow ( underflow protection )
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Marshmallow.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Marshmallow.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Marshmallow.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Marshmallow.totalSupply().mul( terms.maxPayout )/ 100000;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Marshmallow supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Marshmallow.totalSupply();
    ...
    /**
     *  @notice calculate amount of Marshmallow available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Marshmallow) to the DAO
     *  @return bool
     */
    function recoverLostToken( IERC20 _token ) external returns ( bool ) {
        require( _token != Marshmallow, "NAT" );
