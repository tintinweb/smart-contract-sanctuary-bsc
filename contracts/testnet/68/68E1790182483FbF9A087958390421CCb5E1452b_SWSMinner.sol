...
contract SWSMinner is Ownable {
    ...
    address public minerToken = address(0x8be4Cc2d5ad703F5ba3dEAE173c4BCffba28df09);
    address public usdtToken = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    address public micUsdtPair = address(0x55ebE296d10d66509408d6D9002858B8EF311089);
    ...
    function setRecomCFG(uint256 _recomNeed) public onlyOwner {
    ...
    function withdraw(
        ...
        uint256 rewardPrice = pool.allocRate.mul(depositInfo.amount).mul(95).div(10000).div(currentprice);
        IERC20(minerToken).transfer(msg.sender, rewardPrice);
        ...
        while (pid != address(0)){
            ...
            if (i == 1){
                IERC20(minerToken).transfer(pid, rewardPrice.mul(10).div(100));
            }else if (i == 2){
                IERC20(minerToken).transfer(pid, rewardPrice.mul(5).div(100));
            }else if (i == 3){
                IERC20(minerToken).transfer(pid, rewardPrice.mul(3).div(100));
            }else if (i == 4){
                IERC20(minerToken).transfer(pid, rewardPrice.mul(2).div(100));
            }
            i = i+1;
        ...
        for (uint256 b = 0; b < userLevel1arr.length; b++) {
            IERC20(minerToken).transfer(userLevel1arr[b], rewardPrice.mul(10*userTeamDeposit[userLevel1arr[b]]).div(1000*userLevel1total));
        }
        ...
        for (uint256 d = 0; d < userLevel2arr.length; d++) {
            IERC20(minerToken).transfer(userLevel2arr[d], rewardPrice.mul(15*userTeamDeposit[userLevel2arr[d]]).div(1000*userLevel2total));
        }
        ...
        for (uint256 f = 0; f < userLevel3arr.length; f++) {
            IERC20(minerToken).transfer(userLevel3arr[f], rewardPrice.mul(25*userTeamDeposit[userLevel3arr[f]]).div(1000*userLevel3total));
        }
    ...
    function shareUsdt(uint256 shareUsdtAmount) external onlyOwner{
        ...
        for (uint256 i = 0; i < shareholders.length; i++) {
            ...
            if (userTokenDeposit>0){
                userUsdt[shareUser] = userUsdt[shareUser].add(userTokenDeposit.mul(shareUsdtAmount).div(platTotalAmount));
            }else{
    ...
    function getUsdt() public {
        ...
        if (userShareUsdt>0){
            ...
            if (recomFirst!= address(0)){
                ...
                if (depositSubTotal.amountLp.add(IERC20(micUsdtPair).balanceOf(recomFirst)).mul(totalUSDT).div(IERC20(micUsdtPair).totalSupply())>=recomNeed){
                ...
                if (recomSecond != address(0)){
                    ...
                    if (depositSubTotal.amountLp.add(IERC20(micUsdtPair).balanceOf(recomSecond)).mul(totalUSDT).div(IERC20(micUsdtPair).totalSupply())>=recomNeed){
