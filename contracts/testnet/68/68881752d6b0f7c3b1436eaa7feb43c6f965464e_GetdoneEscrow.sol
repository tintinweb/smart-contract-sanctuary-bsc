...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    event ContractDisputationProcessed(uint256 merchantId, uint256 appId, uint256 contractId, address sender, uint256 refund, address erc20, bool isEndTalent, bool isEndCustomer);
    ...
    modifier onlyAdmin(uint256 merchantId, uint256 appId) {
        address wallet = appId != 0 && apps[merchantId][appId] != address(0) ? apps[merchantId][appId] : merchants[merchantId];
        require(wallet == _msgSender(), "GetdoneEscrow: caller is not the admin");
        _;
    }

    modifier inWhitelist(uint256 merchantId, address erc20) {
    ...
    function createMilestone(uint256 merchantId, uint256 appId, uint256 contractId, MilestoneClientInput memory input) 
        ...
        whenNotPaused
        nonReentrant
        inWhitelist(merchantId, input.erc20)
    {
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        ...
        if (milestone.milestoneId == input.milestoneId) {
            require((milestone.status == MILESTONE_STATUS_CANCELLED || milestone.status == MILESTONE_STATUS_DECLINED), "GetdoneEscrow: Can not update milestone");
        }
        ...
        milestone.paid = 0;
        milestone.refund = 0;
        milestone.status = MILESTONE_STATUS_NEW;
    ...
    function approveMilestone(uint256 merchantId, uint256 appId, uint256 contractId, MilestoneTalentInput memory input) 
        public
        whenNotPaused
        nonReentrant
    {
        ...
        require(milestone.status == MILESTONE_STATUS_NEW, "GetdoneEscrow: Milestone does not allow to approve");
    ...
    function declineMilestone(uint256 merchantId, uint256 appId, uint256 contractId, MilestoneTalentInput memory input) 
        public
        whenNotPaused
        nonReentrant
    {
        ...
        require(milestone.status == MILESTONE_STATUS_NEW, "GetdoneEscrow: Milestone does not allow to decline");
        require(contractInfo.talent == talent, "GetdoneEscrow: can not change milestone if sender is not a talent");
        
        uint256 balance = milestone.price - milestone.paid - milestone.refund;

        require(contractInfo.customer == input.customer, "GetdoneEscrow: can not change sale if sender has not made one");
        ...
        nonReentrant
    {
        ...
        uint256 feeAmount = _calculateTransactionFee(balance, fee);
    ...
    function payLessMilestone(uint256 merchantId, uint256 appId, uint256 contractId, MilestoneTalentInput memory input) 
        public
        whenNotPaused
        nonReentrant
    {
        ...
        uint256 feeAmount = _calculateTransactionFee(milestone.payless, fee);
    ...
    function declinePayLessMilestone(uint256 merchantId, uint256 appId, uint256 contractId, MilestoneTalentInput memory input) 
        public
        whenNotPaused
        nonReentrant
    {
        ...
        nonReentrant
    {
        ...
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        require((!contractInfos[merchantId][appId][customer][contractId].isEndCustomer || !contractInfos[merchantId][appId][customer][contractId].isEndTalent), "GetdoneEscrow: contract has been ended");
        ...
        nonReentrant
    {
        ...
        address sender = _msgSender();
        ContractInfo storage contractInfo = contractInfos[merchantId][appId][customer][contractId];
        require((contractInfos[merchantId][appId][customer][contractId].isEndCustomer && !contractInfos[merchantId][appId][customer][contractId].isEndTalent), "GetdoneEscrow: contract cannot decline ending");
        require((!contractInfos[merchantId][appId][customer][contractId].isEndCustomer && contractInfos[merchantId][appId][customer][contractId].isEndTalent), "GetdoneEscrow: contract cannot decline ending");
        require((contractInfo.customer == sender || contractInfo.talent == sender ), "GetdoneEscrow: can not change contract if sender has not made one");
    ...
    function voteContractDisputation(uint256 merchantId, uint256 appId, uint256 contractId, bool vote, address customer, uint256 expireAt) 
        public
        whenNotPaused
        nonReentrant
    {
        ...
        require((!contractDisputations[merchantId][appId][customer][contractId].isCustomerVoted || !contractDisputations[merchantId][appId][customer][contractId].isTalentVoted), "GetdoneEscrow: contract cannot vote disputation");
        ...
        if (contractDispute.init == false) {
            contractDispute.init = true;
            contractDispute.contractId = contractId;
            contractDispute.isProcessed = false;
            contractDispute.expireAt = validExpire;
            contractDispute.isCustomerVoted = contractInfo.customer == sender ? vote : contractDispute.isCustomerVoted;
            contractDispute.isTalentVoted = contractInfo.customer != sender ? vote : contractDispute.isTalentVoted;
        } else {
            contractDispute.isCustomerVoted = contractInfo.customer == sender ? vote : (contractDispute.isCustomerVoted == true && vote == false ? false : contractDispute.isCustomerVoted);
            contractDispute.isTalentVoted = contractInfo.customer != sender ? vote : (contractDispute.isTalentVoted == true && vote == false ? false : contractDispute.isTalentVoted);
            contractDispute.expireAt = !contractDispute.isTalentVoted && !contractDispute.isCustomerVoted && vote == true ? validExpire : contractDispute.expireAt;
        }

        emit ContractDisputationVoted(merchantId, appId, contractId, sender, contractDispute.isTalentVoted, contractDispute.isCustomerVoted, validExpire);
    }
        ...
        onlyAdmin(merchantId, appId)
    {
        ...
        require(
            (
                (contractDisputations[merchantId][appId][customer][contractId].isCustomerVoted && contractDisputations[merchantId][appId][customer][contractId].isTalentVoted) ||
                (
                    (contractDisputations[merchantId][appId][customer][contractId].isCustomerVoted || contractDisputations[merchantId][appId][customer][contractId].isTalentVoted) &&
                    contractDisputations[merchantId][appId][customer][contractId].expireAt < block.timestamp
                )
            )
            , "GetdoneEscrow: contract cannot process disputation");
        require((!contractDisputations[merchantId][appId][customer][contractId].isProcessed), "GetdoneEscrow: contract cannot process disputation");
        require(isEnd(merchantId, appId, contractId, customer) == false, "GetdoneEscrow: contract has been ended");
        ...
        if (amountRefund > 0 && amountRefund <= contractBalance) {
            contractInfo.refund = contractInfo.refund + amountRefund;
            if (contractInfo.erc20 == address(0)) {
                address adminWallet = apps[contractInfo.merchantId][contractInfo.appId] != address(0) ? apps[contractInfo.merchantId][contractInfo.appId] : merchants[contractInfo.merchantId];
                payable(adminWallet).transfer(amountRefund);
            } else {
                address adminWallet = apps[contractInfo.merchantId][contractInfo.appId] != address(0) ? apps[contractInfo.merchantId][contractInfo.appId] : merchants[contractInfo.merchantId];
                address erc20 = contractInfo.erc20;
                IERC20Upgradeable(erc20).safeTransfer(adminWallet, amountRefund);
            }
        }
        contractDispute.isProcessed = true;

        emit ContractDisputationProcessed(contractInfo.merchantId, contractInfo.appId, contractInfo.contractId, _msgSender(), amountRefund, contractInfo.erc20, true, true);
    }
    ...
    function _payout(uint256 merchantId, uint256 appId, uint256 contractId, uint256 milestoneId, address receiver, uint256 price, address erc20)
        internal
    {
        uint256 fee = _calculateTransactionFee(price, appFees[merchantId][appId]);

        uint256 amount = price - fee;

        address adminWallet = apps[merchantId][appId] != address(0) ? apps[merchantId][appId] : merchants[merchantId];
