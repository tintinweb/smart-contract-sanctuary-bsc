...
contract NDCA_V1 is Ownable {
    ...
    */
    ...
    //TESTING
    function Test_UserAddress(uint256 _dcaIndex, uint256 _userIndex) external view returns(address) {
        pairStruct storage dca = neonDCAs[_dcaIndex];
        return dca.usersList[_userIndex];
    }
    function Test_MinAmount(uint256 _dcaIndex) external view returns(uint256) {
        pairStruct storage dca = neonDCAs[_dcaIndex];
        return minSrcAmount * 10 ** dca.srcDecimals;
    }
    function Test_UserInfo(uint256 _dcaIndex, uint256 _userIndex) external view returns(uint256 srcAmount, uint256 tau, uint256 dcaTimeEx, bool fundTransfer) {
        pairStruct storage dca = neonDCAs[_dcaIndex];
        address currentUser = dca.usersList[_userIndex];
        srcAmount = dca.users[currentUser].srcAmount;
        tau = dca.users[currentUser].tau;
        dcaTimeEx = dca.users[currentUser].dcaTimeEx;
        fundTransfer = dca.users[currentUser].fundsTransfer;
    }
    function Test_Transfer(uint256 _dcaIndex, uint256 _userIndex) external{
        pairStruct storage dca = neonDCAs[_dcaIndex];
        IERC20 srcToken = IERC20(dca.srcToken);
        address currentUser = dca.usersList[_userIndex];
        uint256 amount = dca.users[currentUser].srcAmount;
        require(srcToken.transferFrom(currentUser, neonRouter, amount),"NEON: Error Transfer");
    }
    function Test_Refund(uint256 _dcaIndex, uint256 _userIndex) external{
        pairStruct storage dca = neonDCAs[_dcaIndex];
        IERC20 srcToken = IERC20(dca.srcToken);
        address currentUser = dca.usersList[_userIndex];
        uint256 amount = dca.users[currentUser].srcAmount;
        require(srcToken.transferFrom(msg.sender, currentUser, amount),"NEON: Error Transfer");
    }
}