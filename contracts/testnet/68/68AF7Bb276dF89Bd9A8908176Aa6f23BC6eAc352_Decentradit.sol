...
contract Decentradit  {
    ...
    constructor (address _token ,uint _minTokenPost , uint _minTokenVote){
        owner = msg.sender;
        token = IBEP20(_token);
    ...
    function createPost(bytes32 _parentId, string calldata _title, string calldata _contentUri, bytes32 _categoryId, bool _timeDependent, uint256 _timePeriod  ) external {
        require(minTokenPost <= token.balanceOf(msg.sender)," minimum  token balance is needed to create post" );
    ...
    function voteUp(bytes32 _postId, uint8 _reputationAdded) external {
        require(minTokenVotes <= token.balanceOf(msg.sender)," minimum  token balance is needed for vote" );
        if( postRegistry[_postId].timeDependent == true ){
      require( block.timestamp <=  postRegistry[_postId].endTime, " time is up for vote" );
        }
    ...
    function addCategory(string calldata _category) external {
         require(minTokenPost <= token.balanceOf(msg.sender)," minimum  token balance is needed to create caterory" );
    ...
    function getPostCountperCategory(bytes32 _categoryId )public view returns (uint256) {
    ...
    function getVoteRegistry(address _address, bytes32 _postId) public view returns(bool){
    ...
    function getPost(bytes32 _postId) public view returns(address, bytes32, string memory, bytes32, int72, uint256, bytes32) {   
        return (
