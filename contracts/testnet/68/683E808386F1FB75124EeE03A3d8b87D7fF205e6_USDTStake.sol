...
contract USDTStake is Ownable,ReentrancyGuard,Pausable {
    ...
    constructor (IERC20 _stake) {
         ...
         planDetails[1].percent = 6e18;
         planDetails[2].percent = 12e18;
    ...
    function updateTokens (IERC20 _stake) public onlyOwner {
        stakedToken = _stake;
    }

    function stake(uint256 _amount,uint8 _plan) public nonReentrant whenNotPaused {
        require(_amount > 0 && _plan > 0 && _plan <= 2,"incorrect params");
        userID[msg.sender]++;
       ...
       // user.unstakeTime = user.depositTime + ((plans.duration/plans.claimCount)+12 hours);
        ...
        emit Stake(_msgSender(),_amount,_plan,userID[msg.sender],block.timestamp);
    }
    ...
    function unStake(uint256 _id) public nonReentrant whenNotPaused {
        ...
        user.userClaimCount++;
        emit Withdraw(msg.sender,user.depositAmount,user.userInPlan,_id,block.timestamp);
        user.depositAmount = 0;
    }
    
    function harvest (uint256 _id) public nonReentrant whenNotPaused {
        userDetails storage user = investDetails[msg.sender][_id];
        require(user.userInPlan != 3,"Plan3 no harvest");
        require(user.depositTime > 0 && user.depositAmount > 0,"Invalid user");
       // require(block.timestamp >= user.lastclaim + (user.unstakeTime - user.depositTime/user.maxClaimCount)+12 hours,"No time to claim");
        ...
        uint reward = user.depositAmount*(user.stakePercent/user.maxClaimCount)/100e18;
        ...
        emit Harvest(msg.sender,reward,user.userInPlan,_id,block.timestamp);
    }
