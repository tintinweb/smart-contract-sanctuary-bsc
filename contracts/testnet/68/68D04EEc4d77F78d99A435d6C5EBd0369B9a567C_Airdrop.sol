...
contract Airdrop is Ownable {
    ...
    uint256 private constant MAX_FOR_SALE = 10000e9; // 50k
    ...
    function airdropwithNFT(address _nft, uint256 _amount, bool isERC721, uint256 id) external {
        require(!isReceived[msg.sender], "You have already received token");
        require(_amount < MAX_FOR_AIRDROP, "invalid amount");
        require(address(SPOT) != address(0), "Not Set spot token address");
        if (isERC721 == true)
            require(IERC721(_nft).balanceOf(msg.sender) > 0, "No NFT Address");
        else if(isERC721 == false)
            require(IERC1155(_nft).balanceOf(msg.sender, id) > 0, "No NFT Address");
    ...
    function setWhitelist(uint256 _maxAmount) external onlyOwner {
        require(_maxAmount > 0, "invalid number set");
        MAX_FOR_AIRDROP = _maxAmount;
    }
