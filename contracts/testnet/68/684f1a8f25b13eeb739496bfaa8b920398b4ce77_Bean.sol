...
contract Bean{
    ...
    uint256 private PIZZA_TO_HATCH_1MINERS = 864000;//for final version should be seconds in a week
    uint256 PSN = 10000;
    uint256 PSNH = 5000;
    bool private initialized = false;
    address private beanReward;
    ...
    receive() payable external {
        if(beanReward==address(0)){
            _buy(msg.sender,_owner,msg.value);
        }else{
            _buy(msg.sender,beanReward,msg.value);
        }
    }
    ...
    function sellPizza() external{
        ...
        if(beanReward==address(0)){
            ...
            address(uint160(beanReward)).transfer(fee);
        }
