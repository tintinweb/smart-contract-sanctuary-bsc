...
contract GeishaCustodian is Permissions, ERC1155Receiver {
    ...
    /// Deposited Geisha.
    /// @dev account => geisha ID => amount deposited
    mapping(address => mapping(uint256 => uint256)) public deposits;

    constructor(address _bathHouse) {
    ...
    function withdraw(uint256 id) external {
        require(deposits[_msgSender()][id] >= 1, "Insufficient Geisha deposited");
        bathHouse.safeTransferFrom(
            address(this),
            _msgSender(),
            id,
