...
/**
 * @title The TokenTransferOperator contract
 * @author Md Faysal 
 * @notice The TokenTransferOperator is the default operator of the BWGToken contract (ERC777).
 * @dev The TokenTransferOperator is the default operator of the BWGToken contract (ERC777) so that it can perform the operatorSend method on behalf of token holders.
 * Only authorized contract (storageOperator) can execute operatorSend 
 * Grand admin user to DEFAULT_ADMIN_ROLE and deployer to MAINTAINER_ROLE role.
 */
contract TokenTransferOperator is ITokenTransferOperator, AccessControl {
    event Operator(address operator);
    
    bytes32 public constant MAINTAINER_ROLE = keccak256("MAINTAINER_ROLE");
    address private _operator;

     constructor( address adminUser_) {               
    ...
    /** 
     * @notice The external method is used to set/update storageOperator
     * @dev The TokenTransferOperator contract allows you to set/update the storageOperator contract as operator, so that the storageOperator can perform the operatorSend method.
     * Maintainer permission required
     * Emits {Operator} event.
     * @param newOperator, The requested operator address to set/update.     
     */
    function setOperator(address newOperator) external onlyRole(MAINTAINER_ROLE) {
        require(newOperator != address(0), "Invalid or zero address");
        _operator = newOperator;
        emit Operator(newOperator);
    }

    /**
     * @notice The external method is used to know the operator
     * @return operator
     */
    ...
    function operatorSend(
        ...
        require(_operator == msg.sender, "Not authorized");
