...
interface INFTcustom {
    function mintItem(address recipient, uint256 tokenType, uint256 faId, uint256 moId, uint256 lockAt) external returns (uint256);

    function mintMulti(address[] calldata _recipient, uint256[] calldata _tokenType, uint256[] calldata _faId, uint256[] calldata _moId, uint256[] calldata _lockAt) external returns (uint256[] memory);
}
...
contract CapverseMarket is Ownable, ReentrancyGuard {
    ...
    event PurchaseOrder(address nftToken, uint256 tokenID, uint256 orderID);
    ...
    function purchaseOrder(address nftToken, uint256 tokenID) public payable nonReentrant {
        ...
        emit PurchaseOrder(nftToken, tokenID, orderID);
    }
