...
contract UltraRichStaking is Ownable {
    ...
    function getCurrentClaimableAmount(address _user , uint _index) external view returns(uint currentDivident , uint256 withdrawableAmount){
        User storage user = users[_user];

        uint256  slots = (block.timestamp - user.stakes[_index].lastWithdrawTime) / withdrawLimitTime;
         uint256 withdrawableAmount = ((user.stakes[_index].amount * aprPercent) / percentDivider) * slots;

        uint256 percentage = (((block.timestamp -
            user.stakes[_index].lastWithdrawTime) * percentDivider) /
            stakingTime);
        uint256 currentDivident = ((user.stakes[_index].totalAmount *
            percentage) / percentDivider)-withdrawableAmount;
