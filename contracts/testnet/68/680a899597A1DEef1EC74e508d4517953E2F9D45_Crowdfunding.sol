...
contract Crowdfunding is AccessControlEnumerable, ReentrancyGuard {
    ...
    uint16 public brokerRewardRate = 25; //经纪人奖比例
    ...
    function addRegion(
        ...
        require(_multiplier > 10, "wrong _multiplier");
        regions.push(
            Region({
                period: _period,
                poolType: _poolType,
    ...
    function fund(
        ...
        require(amountLeft > 0, "funding archives target amount");
        ...
        //记录point
        // if (accounts[msg.sender].projectIds.length == 1) {}
    ...
    function finishProject(uint256 _regionIndex, uint256 _round) public {
        ...
        if (p.currentAmount == p.targetAmount) {
            ...
            if (_round >= 3 && projects[_regionIndex][_round - 3].state == 3) {
                ...
                regions[_regionIndex].poolAmount -= p3.refundAmount;
            }
        } else {
            //当期失败
            p.state = 4;
            ...
            if (_round > 0) {
                ...
                for (uint256 i = start; i < _round; i++) {
                    ...
                    if (pi.state == 3) {
                        ...
                        regions[_regionIndex].poolAmount -= pi.refundAmount;
                    }
                }
            }
            //当期奖励底池50%
            p.refundAmount = (regions[_regionIndex].poolAmount * bonusRate) / 1000;
            regions[_regionIndex].poolAmount -= p.refundAmount;
            //重置后面期数配置
    ...
    function resetProjects(uint256 _regionIndex, uint256 _round) internal {
        ...
        for (uint256 i = _round + 1; i < _length; i++) {
            ...
            uint256 _targetAmount = (projects[_regionIndex][i - 1].targetAmount * regions[_regionIndex].multiplier) / 10;
        ...
        )
    {
        ...
        if (p.state == 4) {
            ...
            rewardAmount = _currentAmount == 0 ? 0 : (_fundAmount * (1000 - brokerRewardRate) * p.refundAmount) / (_currentAmount * 1000);
            ...
            priorityFundAmount = _fundAmount;
            brokerReward = (_fundAmount * brokerRewardRate) / 1000;
        }
    }
    ...
    function getProjectIds(uint256 _regionIndex) public view returns (uint256[] memory pids) {
        pids = projectIds[msg.sender][_regionIndex];
    }

    function getHarvestableIds(uint256 _regionIndex) public view returns (uint256[] memory pids) {
        pids = harvestableIds[msg.sender][_regionIndex].values();
    }
}
