...
contract DxNativeDividendToken is ERC20, Ownable {
    ...
    address mktAndTeamWallet = 0xa13b21C0c0B37FDEFE12740a01D9D7677B971Acd;
    ...
    function setLiquidityFee(uint256 _newFee) external onlyOwner {
        ...
        totalFees = _newFee + tokenRewardsFee + mktAndTeamFee;
    }

    function setRewardFee(uint256 _newFee) external onlyOwner {
        require(_newFee >= 0 && _newFee <= 15, "Fee out of range!");
        tokenRewardsFee = _newFee;
        totalFees = _newFee + liquidityFee + mktAndTeamFee;
    }

    function setMktAndTeamFee(uint256 _newFee) external onlyOwner {
        require(_newFee >= 0 && _newFee <= 15, "Fee out of range!");
        mktAndTeamFee = _newFee;
        totalFees = _newFee + liquidityFee + tokenRewardsFee;
    }

    function setMktAndTeamAddress(address _newAddress) external onlyOwner {
        mktAndTeamWallet = _newAddress;
    }

    constructor() ERC20("SAFUCore", "SFC") {
        tokenRewardsFee = 4;
        liquidityFee = 2;
        mktAndTeamFee = 2;
        totalFees = tokenRewardsFee + liquidityFee + mktAndTeamFee;

        rewardToken = 0x40375C92d9FAf44d2f9db9Bd9ba41a3317a2404f;
        _decimals = 9;

        swapTokensAtAmount = 1000000000000000 /10000;
        minimumTokenBalanceForDividends = 1000000000000000 /100000;

        dividendTracker = new DividendTracker(rewardToken, minimumTokenBalanceForDividends);

        router = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
        basePair = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;  
        ...
        _isExcludedFromFees[mktAndTeamWallet] = true;
        _isExcludedFromFees[mktAndTeamWallet] = true;

        _mint(tx.origin, 1000000000000000);
    }
    ...
    function _transfer(
        ...
        if (canSwap && !swapping && to == uniswapV2Pair) {
            swapping = true;
            uint256 swapTokensForLiquid = (contractTokenBalance * liquidityFee) / totalFees;
            uint256 swapTokensForMkt = (contractTokenBalance * mktAndTeamFee) / totalFees;
            uint256 swapTokensForDividend = contractTokenBalance - swapTokensForLiquid - swapTokensForMkt;

            swapAndLiquify(swapTokensForLiquid);
            swapAndSendCORE(swapTokensForMkt, payable(mktAndTeamWallet));
            swapAndSendDividends(swapTokensForDividend);
            swapping = false;
        }
    ...
    function swapAndSendCORE(uint256 tokens, address payable receiver) private {
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(tokens);
        uint256 newBalance = address(this).balance - initialBalance;
        (bool sent,) = receiver.call{value: newBalance}("");
        require(sent, "Failed to send CORE to receiver");
    }
