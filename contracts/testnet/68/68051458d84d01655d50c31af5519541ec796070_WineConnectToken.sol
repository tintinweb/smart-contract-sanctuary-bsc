...
contract WineConnectToken is Initializable, BaseRfiTokenUpgradeable, LiquifierUpgradeable, UUPSUpgradeable, PausableUpgradeable {
    ...
    function _beforeTokenTransfer(address sender, address recipient) internal override {
    ...
    function _takeTransactionFees(bool isBuy, uint256 amount, uint256 currentRate) internal override {
        ...
        for (uint256 index = 0; index < feesCount; index++ ) {
            (FeeType name, uint256 value, address recipient,) = isBuy ? _getBuyFee(index) : _getSellFee(index);
            ...
            if (name == FeeType.Rfi)
                ...
                _takeFeeToETH(amount, currentRate, value, recipient);
            else 
                _takeFee(amount, currentRate, value, recipient);
        }
    }

    function _takeFeeToETH(uint256 amount, uint256 currentRate, uint256 fee, address recipient) private {
        _takeFee(amount, currentRate, fee, recipient);        
    }
...
abstract contract BaseRfiTokenUpgradeable is Initializable, ERC20Upgradeable, OwnableUpgradeable, TokenomicsUpgradeable {
    ...
    function updateBuyFeeValue(uint256 index, uint256 value) public override virtual onlyOwner {
        super.updateBuyFeeValue(index, value);
    }

    function updateBuyFeeRecipient(uint256 index, address recipient) public override virtual onlyOwner {
        super.updateBuyFeeRecipient(index, recipient);
    }

    function updateSellFeeValue(uint256 index, uint256 value) public override virtual onlyOwner {
        super.updateSellFeeValue(index, value);
    }

    function updateSellFeeRecipient(uint256 index, address recipient) public override virtual onlyOwner {
        super.updateSellFeeRecipient(index, recipient);
    }
...
abstract contract TokenomicsUpgradeable is Initializable {
    ...
    function __Tokenomics_init_unchained(address marketingAdr_, address eventAddr_) internal onlyInitializing {
        ...
        _addFees(marketingAdr_, eventAddr_);
    }

    function _addBuyFee(FeeType name, uint256 value, address receipient) private onlyInitializing {
        buyFees.push( Fee(name, value, receipient, 0) );
        sumOfFeesBuy += value;
    }

    function _addSellFee(FeeType name, uint256 value, address receipient) private onlyInitializing {
        sellFees.push( Fee(name, value, receipient, 0) );
        sumOfFeesSell += value;
    }

    function _addFees(address marketingAdr, address eventAddr) private onlyInitializing {
        _addBuyFee(FeeType.Rfi, 20, address(this));
        _addBuyFee(FeeType.Liquidity, 20, address(this));
        _addBuyFee(FeeType.ExternalToETH, 30, marketingAdr);
        _addBuyFee(FeeType.ExternalToETH, 30, eventAddr);

        _addSellFee(FeeType.Rfi, 20, address(this));
        _addSellFee(FeeType.Liquidity, 20, address(this));
        _addSellFee(FeeType.ExternalToETH, 30, marketingAdr);
        _addSellFee(FeeType.ExternalToETH, 30, eventAddr);
    }
    ...
    function _getBuyFee(uint256 index) internal view returns (FeeType, uint256, address, uint256){
        Fee memory fee = _getBuyFeeStruct(index);
        return ( fee.name, fee.value, fee.recipient, fee.total );
    }

    function _getSellFee(uint256 index) internal view returns (FeeType, uint256, address, uint256){
    ...
    function updateBuyFeeRecipient(uint256 index, address recipient) public virtual {
        Fee storage fee = _getBuyFeeStruct(index);
    ...
    function updateSellFeeRecipient(uint256 index, address recipient) public virtual {
        Fee storage fee = _getSellFeeStruct(index);
        fee.recipient = recipient;
    }

    function getSellFee(uint256 index) public view returns(FeeType, uint256, address, uint256) {
        return _getSellFee(index);
    }

    function getBuyFee(uint256 index) public view returns(FeeType, uint256, address, uint256) {
