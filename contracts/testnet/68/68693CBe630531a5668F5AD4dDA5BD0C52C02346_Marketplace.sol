...
contract Marketplace is IERC721Receiver, ReentrancyGuard {
    ...
    struct ItemTier {
        ...
        mapping(uint256 => uint256) listedItemAt;
        uint256 queueFront;
        uint256 queueBack;
    ...
    function fetchMarketItem(uint256 _tier, uint _itemId) public view returns (MarketItem memory, uint256 _marketItemId) {
        MarketItem memory item = tierData[_tier].itemListing[_itemId];
    ...
    function fetchMarketItems(uint256 _tier) public view returns (MarketItem[] memory) {
        ...
        for (uint i = 0; i < itemCount; i++) {
            if (tierData[_tier].itemListing[i + 1].owner == address(0)) {
                uint currentId = tierData[_tier].itemListing[i + 1]._itemId;
    ...
    function listItem(address _contract, uint256 _tokenId, uint256 _markup) public nonReentrant {
        ...
        uint256 _itemId = tierData[_tier]._listingIds.current();

        uint256 _price = (
            getMintPriceOf(_tier) + 
            (getMintPriceOf(_tier).mul(_markup.mul(5))).div(100)
        );
    
        tierData[getTierOf(_contract)].itemListing[_itemId] =  MarketItem(
            _itemId,
            _contract,
            _tokenId,
            msg.sender,
        ...
        addItemToQueue(_tier, _itemId);

        IERC721(_contract).transferFrom(msg.sender, address(this), _tokenId);

        emit MarketItemCreated(_itemId, _contract, _tokenId, msg.sender, address(0), _price);
    }
    ...
    function buyItemFromMarket(uint256 _tierId, address _recipient) internal returns (uint256) {
        ...
        (, uint256 _listingId) = fetchMarketItem(_tierId, fetchQueueFront(_tierId));

        // Get details of the listed item
        uint256 _price = tierData[_tierId].itemListing[_listingId]._price;
        uint256 _token = tierData[_tierId].itemListing[_listingId]._tokenId;
        address _seller = tierData[_tierId].itemListing[_listingId].seller;

        // Collect Payment for listed item
        require(IERC20(paymentToken).transferFrom(_recipient, _seller, _price), 'Must pay listing price');

        // Then give the recipient their item
        IERC721(_contract).transferFrom(address(this), _recipient, _token);
        
        // Set the item listing to the caller
        tierData[_tierId].itemListing[_listingId].owner = _recipient;
