...
contract BSCGiveGame {
    ...
    function initTables() private{
        ...
        tables.push(Table(100000000000000000,110,30,40,30));
    ...
    function payRefProfit(uint256 idUser, uint256 numberTable, uint val, bool rand) private{
        ...
        while(true){
            ...
            if(id2counttable[refid] >= numberTable){
                address payable _to = payable(id2address[refid]); 
                uint _val = 0;
                if(rand){
                ...
                if(rand){
                    tableNid2profitLost[numberTable][refid] += val * tables[numberTable].randomRefProfit / (10 * 100);
                }else{
                    tableNid2profitLost[numberTable][refid] += val * tables[numberTable].refProfit / 100;
                }               
            }               
        }
    }
    ...
    function payRandomProfit(uint256 numberTable, uint val) private{
        ...
        for(uint256 i = 0; i < randwinners.length; i++){
            ...
            if(tableNid2profit[numberTable][randwinners[i]] / tables[numberTable].thValue > tables[numberTable].maxProfit / 100) {
                for(uint256 j = 0; j < userintables[numberTable].length; j++){
                    if(userintables[numberTable][j] == randwinners[i]){
                        delete userintables[numberTable][j];
                    }
                }
            }
        }
