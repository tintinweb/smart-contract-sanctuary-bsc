...
contract YetiFrens is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_VIP = 1000;
    ...
    uint public maxMintAmountPerVIP = 2;
    ...
    function VIPMint(uint _quantity, bytes calldata signature) external payable{
        ...
        if(mintedAmountNFTsperWalletVIPSale[msg.sender] + _quantity > maxMintAmountPerVIP) revert("Max exceeded for VIP Sale");
    ...
    function WLMint(uint _quantity, bytes calldata signature) external payable {
        ...
        if(totalSupply() + _quantity > (MAX_VIP + MAX_WL)) revert("Max supply exceeded for WL");
        ...
        if(mintedAmountNFTsperWalletWhitelistSale[msg.sender] + _quantity > maxMintAmountPerWhitelist) revert("Max exceeded for Whitelist Sale");
    ...
    function currentState() external view returns (Step, uint, uint, uint, uint, uint, uint) {
        return (sellingStep, publicSalePrice, wlSalePrice, vipSalePrice, maxMintAmountPerVIP, maxMintAmountPerWhitelist, maxAmountPerTxnPublic);
    }
