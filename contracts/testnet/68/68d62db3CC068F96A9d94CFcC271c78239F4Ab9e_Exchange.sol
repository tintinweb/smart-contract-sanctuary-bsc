...
// File: contracts/priceSetter.sol
...
contract Exchange is Pausable , Ownable{
    ...
    IERC20 public cafe; 
    priceSetter  price = priceSetter(0xa3366a1e1b2FA9df63967f3050f99e089E7de5E5);
    
    constructor(address _Owner, address _cofeeToken) {
        wallet = _Owner;
        fee = 1000000000000000000;
        cafe = IERC20(_cofeeToken);
      
    }

    uint256 public token_price = price.price();

    uint256 public dateToUnlock = 1657861200;
    ...
    function modifyDate(uint256 _date) public onlyOwner {
        dateToUnlock = _date;
    }
    
    modifier waitingForDate() {
        require(block.timestamp < dateToUnlock);
        _;
    }
    ...
    function buyToken(uint _amount, address _exchange)  public  whenNotPaused {
        ...
        uint256 coffeprice =  price.price(); 
        ...
        //cofee token 8 decimals
        uint totalToken = (amount*10**8) / uint(coffeprice);
        require(cafe.balanceOf(address(this)) >= totalToken, "contract doesn't have enough tokens");
        require(cafe.transfer(msg.sender, totalToken));
        require(exchangeToken.transferFrom(msg.sender, address(this), _amount));
    ...
    function sellToken(uint _amount, address _exchange) public  whenNotPaused  {
        require (block.timestamp > dateToUnlock, "You can't sell tokens before the date");
        require(cafe.balanceOf(msg.sender) >= _amount/10**10, "You don't have enough balance");
        uint256 coffeprice = price.price();
        exchangeToken = IERC20(_exchange);
        uint256 totalexchangeToken = (_amount/10**8) * (uint(coffeprice));
        ...
        require(cafe.transferFrom(msg.sender, address(this), _amount));
        require(exchangeToken.transfer(wallet, feeAmount));
