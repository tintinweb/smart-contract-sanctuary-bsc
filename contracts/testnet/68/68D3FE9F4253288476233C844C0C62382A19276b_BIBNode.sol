...
contract BIBNode is PausableUpgradeable, OwnableUpgradeable, ERC721Upgradeable{
    ...
    struct Node {
        ...
        uint256 createTime;
        address[] stakingList;
        mapping(address => BIBFreeze) stakingMap;
        uint256[] subNodes; // 数量最多10个
        uint256 upNode;
        uint256 expireTime;
    }
    ...
    uint256 public freezeTime = 60*60*24*7;
    uint256 public stakeCapTimes = 50;
    // 球星卡合约地址
    address public cardNFTAddress;
    // BIB token 合约地址
    ...
    // 4. 
    function initialize(
        address _cardNFTAddress, address _bibToken
        ) initializer public {
        cardNFTAddress = _cardNFTAddress;
    ...
    function createNode(uint256 _cardNFTId, uint256 _bibAmount) external {
        // 0. 判断调用者是否已经拥有节点
        // 1. 用户BIB资金冻结，冻结前检查用户可用余额
        // 2. 转入球星卡
        // 3. mint 节点凭证 NFT
        address operator = _msgSender();
        require(balanceOf(operator) == 0, "Each address can only have one node");
        ...
        BIBFreeze storage bibFreeze = node.stakingMap[operator];
        bibFreeze.amount = _bibAmount;
        _mint(operator, ticket);
        ticketMap[operator] = ticket;
        nodeList.push(ticket);
        stakeNodesMap[operator].push(ticket);
    ...
    function stake(uint256 _ticket, uint256 _bibAmount) external returns(bool) {
        // 已经过期的质押需要释放
        // 判断余额是否够用，如果是自己节点，是否超过节点最大值，
        // 冻结BIB
        address operator = _msgSender();
        Node storage node = nodeMap[_ticket];
        require(node.createTime > 0, "Node not exist");
        require(getAvailableAmount(operator) >= _bibAmount, "Insufficient balance");
        uint256 _cardNFTId = getCardNFTByAddress(operator);
        if (node.ownerAddress == operator){
            uint256 limit = _getCardStakingLimit(_cardNFTId);
        ...
        BIBFreeze storage bibFreeze = node.stakingMap[operator];
        if (bibFreeze.amount == 0) {
            node.stakingList.push(operator);
            stakeNodesMap[operator].push(_ticket);
        }
        // TODO： 委托总量检查
        bibFreeze.amount = bibFreeze.amount.add(_bibAmount);
    ...
    function stakeNode(uint256 _ticket) external returns(bool) {
        Node storage node = nodeMap[_ticket];
        require(node.createTime > 0, "Node not exist");
        require(node.expireTime == 0, "Node is disband");
        require(node.subNodes.length < 10, "Node is full");
        // require(node.upNode == 0, "Node can't be staked");
        address operator = _msgSender();
        ...
        // require(fromNode.subNodes.length == 0, "You are not access to staked.");
        node.subNodes.push(fromTicket);
        fromNode.upNode = _ticket;
        return true;
    }
    
    function unStake(uint256 _ticket) external returns(bool) {
        address operator = _msgSender();
        Node storage node = nodeMap[_ticket];
        require(node.createTime > 0, "Node not exist");
        BIBFreeze storage bibFreeze = node.stakingMap[operator];
        ...
        bibFreeze.expireTime = _currentTime().add(freezeTime);
    }

    function unStakeNode(uint256 _ticket) external returns(bool) {
        Node storage node = nodeMap[_ticket];
        require(node.createTime > 0, "Node not exist");
        require(node.expireTime == 0, "Node is disband");
        require(node.subNodes.length < 10, "Node is full");
        ...
        uint256 index = node.subNodes.length;
        while(index > 0) {
            index--;
            if (node.subNodes[index] == fromTicket) {
                node.subNodes[index] = node.subNodes[nodeList.length - 1];
                node.subNodes.pop();
                break;
            }
        }
        fromNode.upNode = 0;
        fromNode.expireTime = _currentTime().add(freezeTime);
        return true;
    }

    function canClaimCardNFT() public view returns(bool){
        return _currentTime() > cardNFTFreezeMap[_msgSender()].expireTime;
    }

    function claimCardNFT() external {
        require(canClaimCardNFT(), "Freeze not end");
        address operator = _msgSender();
        CardNFTFreeze storage f = cardNFTFreezeMap[operator];
        require(f.expireTime > 0, "You don't have Card NFT");
        // TODO:: transfer card nft
    }
    ...
    function _transfer(
        ...
        Node storage node = nodeMap[tokenId];
        node.ownerAddress = to;
    ...
    function _getCardStakingLimit(uint256 _cardNFTId) internal returns(uint256) {
        // TODO:: 获取球星卡等级，给出其Staking上限
        return 2000000 * 10**18;
    }

    function freeExpireStake(address _account) public returns(bool) {
        uint256[] storage list = stakeNodesMap[_account];
        uint256 i = list.length;
        while (i > 0){
            i--;
            if (nodeMap[list[i]].expireTime < _currentTime()){
    ...
    function getFreezeAmount(address _account) public view returns(uint256) {
        ...
        for (uint256 i=0;i<list.length;i++){
            Node storage node = nodeMap[list[i]];
            BIBFreeze storage bibFreeze = node.stakingMap[_account];
            if((node.expireTime == 0 || node.expireTime >= _currentTime()) || bibFreeze.expireTime >= _currentTime()) {
                freezeAmount = freezeAmount.add(bibFreeze.amount);
            }
        }
        return freezeAmount;
    }
