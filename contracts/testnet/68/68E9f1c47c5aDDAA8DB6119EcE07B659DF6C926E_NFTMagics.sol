...
contract NFTMagics is ERC721URIStorage, Ownable {
  ...
  address AYRA = 0x911bA403442dD7736bf3f76b88227Fb043BDFAEa;
  ...
  function mint(
    ...
    if(_collectionId==0) {
        collectionId = NFTMarketplace(_marketplace).createCollection(msg.sender, 'unnamed', 'unnamed', _image, _image, _image);
    }
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ...
      NFTMarketplace(_marketplace).createMarketItem(newItemId, collectionId, msg.sender);
    }
  ...
  function setMarketplace(address marketplaceAddress) public {
    require(msg.sender == _contractOwner, 'Sender does not own the contract');
    _marketplace = marketplaceAddress;
  }
  function tokenMinter(uint256 tokenId) public returns (address){
    return nftItems[tokenId].minter;
  }
...
contract NFTMarketplace is ReentrancyGuard {
    ...
    event activeMarketItem(uint256 id);
    ...
    function createMarketItem(uint256 tokenId, uint256 collectionId, address owner)
        public
        isExistCollection(collectionId)
        nonReentrant
    {
        require( _nftContract.tokenMinter(tokenId) == owner, 'sender is not owner');
        ...
        if (rewards[msg.sender].owner != msg.sender) {
            _rewardCount.increment();
            uint256 rewardId = _rewardCount.current();
            rewardAddress[rewardId] = msg.sender;
        ...
        emit marketItemCreated(itemId, tokenId, collectionId);
    }
        ...
        nonReentrant
    {
        ...
        address minter = marketItemList[itemId].owner;
        ...
        if (saleToken == WBNB) {
            ...
            for (uint256 i = 0; i < rewardCount; i++) {
                ...
                if (rewards[_address].endDate > block.timestamp) {
                    rewards[_address].wbnb = rewards[_address].wbnb.add(
            ...
            require(
                IERC20(saleToken).balanceOf(msg.sender) > price,
                "Please submit the asking price in order to complete purchase"
            );
            ...
            if(royaltyValue > 0)  IERC20(saleToken).transferFrom(buyer, minter, royaltyValue);
            ...
            IERC20(saleToken).transferFrom(buyer, contractAddress, rewardValue);
            ...
            for (uint256 i = 0; i < rewardCount; i++) {
                ...
                if (rewards[_address].endDate > block.timestamp) {
                    if (saleToken == AYRA)
                        rewards[_address].ayra = rewards[_address].ayra.add(
                            rewardValue.div(rewardCount)
                        );
                    if (saleToken == ITHD)
                        rewards[_address].ithd = rewards[_address].ithd.add(
                            rewardValue.div(rewardCount)
                        );
                }
            }
    ...
    function updateMarketItem(
        ...
        emit updateItem(itemId, updatePrice, saleToken, giftAddress, royalty);
    }

    function claimRewards() external {
        if (rewards[msg.sender].endDate > block.timestamp) {
            uint256 ithdAmount = rewards[msg.sender].ithd;
            uint256 ayraAmount = rewards[msg.sender].ayra;
            uint256 wbnbAmount = rewards[msg.sender].wbnb;

            require(
                (ithdAmount > 0 || ayraAmount > 0 || wbnbAmount > 0),
                "Zero balance"
            );

            if (ithdAmount > 0) IERC20(ITHD).transfer(msg.sender, ithdAmount);
            if (ayraAmount > 0) IERC20(AYRA).transfer(msg.sender, ayraAmount);
            if (wbnbAmount > 0) payable(msg.sender).transfer(wbnbAmount);

            rewards[msg.sender].ithd = 0;
            rewards[msg.sender].ayra = 0;
            rewards[msg.sender].wbnb = 0;
        }
    }
