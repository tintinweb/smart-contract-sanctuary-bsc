...
contract YieldzToken is ERC20, Ownable {
    ...
    mapping (address => BondData) private userBond;
    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) public automatedMarketMakerPairs;

    event CreateBond(address indexed account, uint256 amount, uint256 period);
    ...
    function createBond(uint256 amount, uint256 period) external {
        BondData storage bond = userBond[msg.sender];

        require(period == 10 || period == 20, "Not Exact Bond");

        bond.bonder = msg.sender;
        bond.startTime = block.timestamp;
        bond.endTime = block.timestamp + period * 1 days;
        bond.amount = amount;
        bond.period = period;

        if (period == 10) {
            bond.rate = tenBondTotal * 10**18 / tenBondPrice / 10 days;
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, tenBondTotal * 10**18);
        } else {
            bond.rate = twentyBondTotal * 10**18 / twentyBondPrice / 20 days;
            IERC20(usdtAddress).transferFrom(msg.sender, treasury, twentyBondTotal * 10**18);
        }

        emit CreateBond(msg.sender, amount, period);
    }
