...
contract WorldCup is OwnableUpgradeable {
    ...
    RandomConsumer random;               // 随机数消费者合约
    ...
    function initialize(address soul_, address randomConsumer_, address wallet_) public initializer {
    ...
    function betting(uint amount_, uint team_) public {
        ...
        if (competitionType == 128 || competitionType == 64) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 90 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 90 / 100;
            }
            // 记录
            mapPlayerRecord[player].push(Instant({
                ...
                initAmount: amount_,
                amount: betNums,
                leverageAmount: leverageAmount * 90 / 100,
                team: team_,
                matchId: matchId
            }));
            // 总量
            mapInstantTotal[matchId].total += betNums;
        } else if (competitionType == 32) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 53 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 53 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                initAmount: amount_,
            ...
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 63 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 63 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                initAmount: amount_,
            ...
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 73 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 73 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                initAmount: amount_,
            ...
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 83 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 83 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                initAmount: amount_,
            ...
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += leverageAmount * 88 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += leverageAmount * 88 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                initAmount: amount_,
        ...
        if (mapPlayerRecord[player].length != 0) {
            ...
            if (winner == mapPlayerRecord[player][0].team) {
                ...
                if (championlen != 0 && mapPlayerChampion[player][championlen - 1].championIndex <= championIndex) {
                    mapPlayerChampion[player][championlen - 1].ssoulAmount += mapPlayerRecord[player][0].initAmount * 1000;
                } else {
                    mapPlayerChampion[player].push(Champion({
                        ssoulAmount: mapPlayerRecord[player][0].initAmount * 1000,
    ...
    function mapPlayerChampionLen(address player_) public view returns(uint) {
        return mapPlayerChampion[player_].length;
    }

    function mapPlayerRecordLen(address player_) public view returns(uint) {
        return mapPlayerRecord[player_].length;
    }
