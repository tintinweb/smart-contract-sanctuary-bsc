...
contract NFT_mining_pool1 is Ownable, Pausable {
...
//Events
    ...
    event changeVerificationNftAddress(address _lastAddress, address _newAddress);
    event changeRequiredNftsBalance(uint _lastRequiredNftsBalance, uint _newRequiredNftsBalance);

//Modifier

    ///@dev Check if the user has enough balance in the established token
    modifier checkAccountBalance() {
        uint _userBalance = IERC721(verificationNft).balanceOf(msg.sender);
        require(_userBalance == requiredNftsBalance, "Error: User has the wrong balance");
        _;
    }
...
//Constructor
    ...
    /**
        @param _verificationNft Nft that will be used to validate users
    ...
    */
    constructor(address _verificationNft, address _rewardToken, uint _requiredNftsBalance) {
...
//Update Contract Functions
    ...
    /**
        ...
        amount of Nft established.
    */
    function updateDataInContract() external whenNotPaused {
        for(uint i = 0; i < participants.length; i++){
            uint _userBalance = IERC721(verificationNft).balanceOf(participants[i]);

            if(_userBalance != requiredNftsBalance){
...
//Only Owner Functions
    ...
    ///@dev The owner can change the address of the validation Nft
    function setVerificationNft(address _newVerificationNft) public onlyOwner {
        require(_newVerificationNft != address(0), "Error: Address cannot be 0x00");
        require(
            _newVerificationNft != verificationNft, 
            "Error: The new address of cannot be the same as the last address"
        );

        address _lastVerificationNft = verificationNft;
        verificationNft = _newVerificationNft;

        emit changeVerificationNftAddress(_lastVerificationNft, verificationNft);
    }

    ///@dev The owner can change the minimum amount of Nft that a user must have to receive a reward
