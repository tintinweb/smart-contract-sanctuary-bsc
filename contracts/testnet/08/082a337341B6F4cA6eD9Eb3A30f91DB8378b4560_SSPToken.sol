...
import "./BEP20/BEP20Permit.sol";
import "./ISSPToken.sol";
import "../utils/EIP712.sol";
import "../utils/math/SafeCast.sol";
import "../access/SSPAccessControls.sol";

contract SSPToken is ISSPToken, BEP20Permit {
  struct Checkpoint {
    uint32 fromBlock;
    uint256 votes;
  }
  ...
  modifier onlyAdminOrTokenMinter() {
    require(sspAccessControls.hasAdminRole(_msgSender()) ||
      sspAccessControls.hasTokenMinterRole(_msgSender()),
      "SSP: only admin or TMR"
    );
    _;
  }

  constructor(address _sspAccessControls) BEP20Permit("Secure Service Prediction", "SSP", version) {
  ...
  function delegateBySig(
    ...
    require(expiry >= block.timestamp, "SSP: signature expiry");
    ...
    require(recoveredAddress != address(0), "SSP: invalid signature");
    require(nonce == nonces[recoveredAddress]++, "SSP: invalid nonce");
  ...
  function _mint(address _account, uint256 _amount) internal virtual override {
    ...
    require(totalSupply() <= maxTotalSupply(), "SSP: total supply risks overflowing votes");
...
abstract contract BEP20Permit is BEP20, EIP712 {
  ...
  constructor(string memory _name, string memory _symbol, string memory _version) BEP20(_name, _symbol) {
    uint256 chainId;
    assembly {
      chainId := chainid()
    }
  ...
  function permit(
    ...
    require(expiry >= block.timestamp, "BEP20: expiry");
    bytes32 digest = keccak256(
      abi.encodePacked(
        "\x19\x01",
        DOMAIN_SEPARATOR,
    ...
    address recoveredAddress = ecrecover(digest, v, r, s);
    require(recoveredAddress != address(0) && recoveredAddress == owner, "BEP20: invalid signature");
