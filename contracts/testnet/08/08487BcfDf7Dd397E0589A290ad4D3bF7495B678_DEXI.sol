...
contract DEXI is IERC20, Ownable {
    ...
    function _transferTaxed(address sender, address recipient, uint256 amount, uint8 tax) private {
        ...
        if (enableBurnFee) {
            uint256 tokenToBeBurnt=_calculateFee(amount, tax, burnFee);
            _balances[address(this)]=_balances[address(this)].sub((totalTaxedToken.sub(tokenToBeBurnt)));
            emit Transfer(sender, address(0), tokenToBeBurnt);
            emit Transfer(sender, address(this), (totalTaxedToken-tokenToBeBurnt));
        } else {
            _balances[address(this)]=_balances[address(this)].sub((totalTaxedToken));
