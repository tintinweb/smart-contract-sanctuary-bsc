...
contract Meta38Token is IERC20, Ownable {
    ...
    string private _name = "Meta40 Token";
    ...
    uint256 [3] public  levelAmounts = [10000*10**18,30000*10**18,50000*10**18];
    ...
    uint256 private _tTotal = 4100 * 10**8 * 10**18; //发行总量
    ...
    address public coinAddress = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd); //wbnb 地址
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
         if(_tOwned[address(this)] >= 1 * 10**4 * 10**18 && from !=address(this) && lpFeeShareTime.add(minPeriod) <= block.timestamp) {
    ...
    function _takeInviterFee(
       ...
       //  _inviterFee = _levelFeeRates[level].inviterFee;
        ...
        if (sender == uniswapV2Pair) {
            ...
            _tOwned[address(this)] = _tOwned[address(this)].add(tAmount.div(10000).mul(inviterFee));
         ...
         if (level==1) {
            ...
            for (int256 i = 0; i < 8; i++) {
                ...
                if(_tOwned[cur]>=minBalance)
                    ...
                    _tOwned[address(this)] = _tOwned[address(this)].add(curTAmount);
