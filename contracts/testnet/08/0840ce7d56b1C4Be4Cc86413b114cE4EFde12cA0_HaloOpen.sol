...
contract HaloOpen is OwnableUpgradeable {
    ...
    event Reward(address indexed addr, uint indexed reward, uint indexed amount);//1 for creation 2 for normal 3 for box 4 for shred 5 for bvg 6 for home 7 for pioneer
    ...
    function bond(address addr) internal {

    }

    function openBox(uint tokenId) external refreshTime {
        box.burn(tokenId);
    ...
    function extractNormal(uint amount) external refreshTime {
        ...
        if (amount == 2) {
            ...
            if (out > 80) {
                ...
                emit Reward(msg.sender, 4, 1);
                emit Reward(msg.sender, 5, 5 ether * 1e18 / BvgPrice / 1e18);
            }
        } else {
            uint out = rand(100);
            if (out > 85) {
                ...
                emit Reward(msg.sender, 4, 2);
                BVG.transfer(msg.sender, 10 ether * 1e18 / BvgPrice);
                emit Reward(msg.sender, 5, 10 ether * 1e18 / BvgPrice / 1e18);
            }
        }
    ...
    function extractCreation() external refreshTime {
        ...
        if (times == 0) {
            ...
            if (out > 50 && extractCreationAmount > 0) {
                ...
                extractTimes[msg.sender]++;
                emit Reward(msg.sender, 5, 20 ether * 1e18 / BvgPrice / 1e18);
            }
        } else {
            box.mint(msg.sender, 2);
            extractCreationAmount --;
    ...
    function extractPioneerPlanet(uint amount) external refreshTime {
        ...
        if (amount == 8) {
            ...
            if (out > 925) {
                ...
                emit Reward(msg.sender, 5, 50 ether * 1e18 / BvgPrice / 1e18);
            }
        }
        userInfo[currentDay][msg.sender].costAmount += amount;
        if (userInfo[currentDay][msg.sender].costAmount > openInfo[currentDay].costAmount) {
    ...
    function claimReward() refreshTime external {
        require(countingReward(msg.sender) > 0, 'no reward');
        BVG.transfer(msg.sender, countingReward(msg.sender));
        isClaimed[lastDay][msg.sender] = true;
    }

    function checkInfo(address addr) public view returns(uint,uint,uint,uint){
       return( extractTimes[addr],creationAmount,homePlanet,pioneerPlanet);
    }
}
