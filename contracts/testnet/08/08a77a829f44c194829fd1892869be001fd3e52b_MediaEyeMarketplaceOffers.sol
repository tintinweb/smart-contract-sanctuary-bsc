...
import "./interfaces/IMediaEyeSplitInfo.sol";
...
contract MediaEyeMarketplaceOffers is AccessControl, ReentrancyGuard {
    ...
    function updateConstantsByAdmin(
        ...
        if (_basisPointFee > 0) {
            require(_basisPointFee <= 500, "fee");
        ...
        internal
    {
        ...
        // get split info for collection
        MediaEyeOrders.Split memory split;
        if (IERC165(_offerSignature.nft.nftTokenAddress).supportsInterface(type(IMediaEyeSplitInfo).interfaceId)) {
            split = IMediaEyeSplitInfo(_offerSignature.nft.nftTokenAddress).getSplitInfo();
            if (split.royaltyBasisPoint > 0) {
                payoutToCreator = (_offerSignature.price * split.royaltyBasisPoint) / 10000;
            }
        }
        else if (IERC165(_offerSignature.nft.nftTokenAddress).supportsInterface(type(IERC2981).interfaceId)) {
            (royaltyArtist, payoutToCreator) = IERC2981(_offerSignature.nft.nftTokenAddress)
                .royaltyInfo(_offerSignature.nft.nftTokenId, _offerSignature.price);
            split.royaltyArtist = payable(royaltyArtist);
        }

        // payout to Charity/sellers
        uint256 remainingPayout = (_offerSignature.price - payoutToTreasury - payoutToCreator);
        if (
            split.charityBasisPoint > 0 &&
            split.charity != address(0)
        ) {
            payoutToCharity =
                (remainingPayout * split.charityBasisPoint) /
                10000;
        }
        if (
            split.splitBasisPoint > 0 &&
            split.splitRecipient != address(0)
        ) {
            payoutToSecondarySeller =
                (remainingPayout * split.splitBasisPoint) /
                10000;
        }
        uint256 payoutToSeller = (remainingPayout *
            (10000 -
                split.charityBasisPoint -
                split.splitBasisPoint)) / 10000;
...
library MediaEyeOrders {
    ...
    struct AuctionConstructor {
        ...
        address feeContract;
        address mediaEyeMarketplaceInfo;
        address[] priceFeeds;
    }
    ...
    struct PaymentChainlink {
        bool isValid;
        address quoteAddress;   // address of asset converting to
        address baseAddress;    // address of asset converting from
    }
