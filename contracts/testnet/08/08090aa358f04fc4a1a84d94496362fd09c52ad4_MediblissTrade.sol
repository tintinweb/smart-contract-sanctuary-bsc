...
interface MediblissCollective721 {
    function mint(
...
interface MediblissCollective1155 {
    function mint(
        string memory ipfsmetadata,
        address to,
        uint total,
...
contract MediblissTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    address public medibliss721;
    address public medibliss1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function _saleToken(
        ...
        address create;
        if(medibliss721    ==  _conAddr || medibliss1155    ==  _conAddr){
        if (ids[3] == 721) {
            (create, ldatas[0]) = MediblissCollective721(medibliss721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, ldatas[0]) = MediblissCollective1155(medibliss1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        }
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            MediblissCollective721(medibliss721).mint(
                ipfsmetadata,
                msg.sender,
                datas[2],
                id_
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], medibliss721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            MediblissCollective1155(medibliss1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], medibliss1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _medibliss721, address _medibliss1155)
        public
        onlyOwner
    {
        medibliss721 = _medibliss721;
        medibliss1155 = _medibliss1155;
    }
    function changeCollectionOwner(address to) public {
        MediblissCollective721(medibliss721).changeCollectionOwner(to);
        MediblissCollective1155(medibliss1155).changeCollectionOwner(to);
    }
