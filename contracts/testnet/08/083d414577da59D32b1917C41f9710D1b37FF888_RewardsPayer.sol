// Deployed address: 0x083d414577da59D32b1917C41f9710D1b37FF888
...
contract RewardsPayer is ReentrancyGuard, AccessControl {
    ...
    IControl controlContract =
        IControl(0x841f48398B2233575e69576e4d566736158d58Ce);
    IVaultRewards VaultRewardsContract =
    ...
    /// @dev salvamos la wallet del user y luego en el mapping interno salvamos la address de la collection y la estructura con sus rewards por coleccion.
    mapping(address => mapping(address => UserRewards))
        private collectionRewardsPerUser;
    ...
    function payBuilderReward(address _collection, address _token)
        public
        nonReentrant
    {
        ...
        uint256 amountReward = collectionRewardsPerUser[msg.sender][_collection]
            .rewardPerBuilder;
        if (amountReward == 0) {
            revert("You have no rewards available");
        }
        collectionRewardsPerUser[msg.sender][_collection].rewardPerBuilder = 0;
        collectionRewardsPerUser[msg.sender][_collection]
            .lastTimePayBuilder = block.timestamp;
    ...
    /// @dev this allow the user to withdraw his rewards per being a Holder of an NFT collection
    function payHolderReward(address _collection, address _token)
        public
        nonReentrant
    {
        if (_token != address(Busd) && _token != address(Usdc)) {
        ...
        collectionRewardsPerUser[msg.sender][_collection].rewardPerHolder = 0;
        collectionRewardsPerUser[msg.sender][_collection]
            .lastTimePayHolder = block.timestamp;
        VaultRewardsContract.withdraw(amountReward, _token);
        if (_token == address(Busd)) {
            Busd.transfer(msg.sender, amountReward);
        } else if (_token == address(Usdc)) {
        ...
        nonReentrant
    {
        ...
        uint256 amountRentReward = collectionRewardsPerUser[msg.sender][
            _collection
        ].rewardPerRent;
        if (amountRentReward == 0) {
            revert("You have no rewards available");
        }
        collectionRewardsPerUser[msg.sender][_collection].rewardPerRent = 0;
        collectionRewardsPerUser[msg.sender][_collection]
        ...
        nonReentrant
    {
        ...
        for (uint16 i = 0; i < _collections.length; i++) {
            amountRewards +=
                collectionRewardsPerUser[msg.sender][_collections[i]]
                    .rewardPerBuilder +
                collectionRewardsPerUser[msg.sender][_collections[i]]
                    .rewardPerHolder;
            ...
            amountRents = collectionRewardsPerUser[msg.sender][_collections[i]]
                .rewardPerRent;
    ...
    /// @dev this function calculate the user Rewards per collection
    /// @param _coleccion: address de la coleccion,
    /// @param _walletsUsers: el arreglo de wallets para los que se realizara el calculo por coleccion.
    /// @param _numNFTsMinted: el numero de NFTs que minteo el usuario.
    function CalcBuilderRewardsForCollection(
        ...
        // el script del autotask leera las wallets del cotrato walletofowner para que la envie a las funciones de calculo
        // de rewards y en base a eso hacer el calculo recorriendo el array en la funcion.
        ...
        for (uint16 i = 0; i < _walletsUsers.length; i++) {
            if (_numNFTsMinted[i] != 0) {
                ...
                collectionRewardsPerUser[_walletsUsers[i]][_coleccion]
                    .rewardPerBuilder += _numNFTsMinted[i] * PayPerNFT;
            }
        }
    }

    // CREAR UN CONTADOR QUE NOS DE EL NUMERO DE COLECCIONES, SOLO LOS DEV PUEDEN ACTUALIZAR ESTA FUNCION
    ...
    function CalcHolderRewardsForCollection(
        ...
        for (uint16 i = 0; i < _walletsUsers.length; i++) {
            if (_numNFTsholder[i] != 0) {
                // guardamos el calculo del reward por coleccion para cada usuario
                collectionRewardsPerUser[_walletsUsers[i]][_coleccion]
                    .rewardPerHolder += _numNFTsholder[i] * PayPerNFT;
            }
        }
    }
    ...
    function CalcRentRewardsForCollection(
        ...
        ILazyNFT CollectionContract = ILazyNFT(_coleccion);
        // Leer el mapping para obtener la address del vault de renta de la propiedad
        IVaultRewards vaultRentRewards = IVaultRewards(
            collectionToVault[_coleccion]
        );
    ...
    function getRewardsPerUser(address _walletUser, address _Collection)
        public
        view
        returns (UserRewards memory)
    {
        // recibe la coleccion  tambien para traer los datos por coleccion
        return collectionRewardsPerUser[_walletUser][_Collection];
    }
    ...
    // AGREGAR FUNCION PARA RESETEAR LA STRUCT SOLO PARA DESARROLLO.
    function clearRewards(address _wallet, address _collection) public {
        collectionRewardsPerUser[_wallet][_collection].rewardPerBuilder = 0;
        collectionRewardsPerUser[_wallet][_collection].rewardPerHolder = 0;
        collectionRewardsPerUser[_wallet][_collection].rewardPerRent = 0;
        collectionRewardsPerUser[_wallet][_collection].lastTimePayBuilder = 0;
        collectionRewardsPerUser[_wallet][_collection].lastTimePayHolder = 0;
        collectionRewardsPerUser[_wallet][_collection].lastTimePayRent = 0;
    }
}
