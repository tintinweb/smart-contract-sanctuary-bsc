...
contract MechaMasterNFT is Initializable, ERC721, Member {
    using String for string;
	using SafeERC20 for IERC20;
	using SafeMath for uint256;
    using String for uint256;
    
    uint256 public NFT_Id; // 預售 NFT 編號從 0 開始
    ...
    uint256 public userMintStartMax;
    uint256 public userMintCount;
    uint256 public userMintCountMax;
    ...
    event OfficalMint(address indexed origin, address indexed owner,uint256 power, uint256 TokenId, uint256 MCAprice);
    ...
    function initialize(IERC20 _usdt, IERC20 _mca, IERC20 _pmca, IUniswapV2Pair _pair) public initializer {
        __initializeERC721("MechaMaster NFT", "MechaMaster");
        __initializeMember();
        init();
        usdt = _usdt;
        mca = _mca;
    ...
    function init() internal initializer {
        userMintStartMax = 1 days; // 公開鑄造產量更新時間1天
        userMintCountMax = 150; // 公開鑄造釋放150個盲盒
    }
    ...
    function preOfficalMint() public returns(uint256) {
        require(block.timestamp >= preStart, "NOT start!");
        require(preCount < 2000 && onceCount < onceMax ,"Sale Over!");

        address user = msg.sender;
        uint256 needPay = 1;
        require(IERC20(pmca).balanceOf(user) >= needPay, "PreMCA not enough");
        
        preCount++;
        onceCount++;

        IERC20(pmca).transferFrom(user, manager.members("OfficalAddress"), needPay);
    ...
    function officalMint() public returns(uint256){
        ...
        emit OfficalMint(user, user, 10, NFT_Id, needPay);
        return NFT_Id;
    }
    ...
    function openBlindBox(uint256 tokenId) external {
        ...
        uint32 randomNum = _random();
        uint256 ethnicity = randomNum % 3 + 1;
        uint256 power = randomNum % 10 * 2 + 7;
        power = power < 18 ? 18 : power;
