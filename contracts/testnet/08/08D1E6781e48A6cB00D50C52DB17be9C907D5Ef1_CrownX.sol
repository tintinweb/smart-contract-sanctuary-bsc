   ...
   struct UserStruct {
    ...
    function NewRegistration(string memory sponcer_id,uint256 ownerAmt,uint256 adminAmt) public payable {
       ...
       require(!users[msg.sender].isExist, 'User exist');
        ...
        payable(ownerWallet).transfer(ownerAmt);
        payable(admin).transfer(adminAmt);
	...
	{
		 payable(ownerWallet).transfer(ownerAmt*1e18);
        payable(admin).transfer(adminAmt*1e18);
