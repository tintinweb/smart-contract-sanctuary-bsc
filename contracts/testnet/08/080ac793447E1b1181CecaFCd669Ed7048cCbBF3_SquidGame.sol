...
contract SquidGame is SquidGameBasic {
    ...
    constructor(address _ownerAddress, address _secWallet, address _regWallet, IERC20 _depositTokenAddress) {
        ...
        LevelPayments[1].timeToOpen = block.timestamp + 12 hours;
        LevelPayments[2].timeToOpen = block.timestamp + 24 hours;
        LevelPayments[3].timeToOpen = block.timestamp + 36 hours;
        LevelPayments[4].timeToOpen = block.timestamp + 48 hours;
        LevelPayments[5].timeToOpen = block.timestamp + 72 hours;
        LevelPayments[6].timeToOpen = block.timestamp + 96 hours;
        LevelPayments[7].timeToOpen = block.timestamp + 120 hours;
        LevelPayments[8].timeToOpen = block.timestamp + 144 hours;
        LevelPayments[9].timeToOpen = block.timestamp + 192 hours;
        LevelPayments[10].timeToOpen = block.timestamp + 240 hours;
        LevelPayments[11].timeToOpen = block.timestamp + 288 hours;
        LevelPayments[12].timeToOpen = block.timestamp + 336 hours;
        LevelPayments[13].timeToOpen = block.timestamp + 408 hours;
        LevelPayments[14].timeToOpen = block.timestamp + 488 hours;
    ...
    function sendETHDividends(uint8 level) private returns (bool) {
        ...
        if((users[prev_user].firstPayment==true) && (users[prev_user].secPayment==true))
        {
            if((users[prev_user].activeMainLevels[level+1]==true) && (level != 16)) {
                return payObgonCheck(prev_user, level);
        ...
        {
            if(users[prev_user].firstPayment==false)
            {
                users[prev_user].firstPayment = true;
            }else
            {
                users[prev_user].secPayment =true;
            }
            return payObgonCheck(prev_user, level);
        }
    }
    ...
    function UserRewards(address userAddress)public view returns (uint64[] memory){
        ...
        for (uint8 i=0; i < LAST_LEVEL; i++) 
        {
            uint32 level_length = TotalLevelUserCount(i+1);
            uint32 levelid =LevelPlace[i];
            rewardCount=0;
            if(level_length>0)
            {
                if((users[userAddress].activeMainLevels[i+2] == false) && (users[userAddress].firstPayment==true) && (users[userAddress].secPayment==true)){
                    rewardCount = 2;
                }
                else{
                ...
                rewardP=uint64(rewardCount*(levelPrice[i+1]*74/100)/1000000000);
