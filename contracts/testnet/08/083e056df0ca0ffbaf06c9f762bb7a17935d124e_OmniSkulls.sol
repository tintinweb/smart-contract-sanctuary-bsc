...
contract OmniSkulls is Ownable, ERC721, NonblockingReceiver {
    ...
    uint256 MAX_MINT_ETHEREUM = 8000;

    uint gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint) ERC721("OmniSkulls", "os") { 
        _owner = msg.sender;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 6, "OS: Max 5 NFTs per transaction");
        require(nextTokenId + numTokens <= MAX_MINT_ETHEREUM, "OS: Mint exceeds supply");
