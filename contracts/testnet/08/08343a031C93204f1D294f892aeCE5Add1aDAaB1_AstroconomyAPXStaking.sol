...
contract AstroconomyAPXStaking {
    ...
    function stakeTokens(uint256 _stakingAmount, uint256 _newStakingPlan) public {
    ...
    function unstakeTokens() public {
        ...
        if ( _totalReflectionPool > 0 ) {
            uint256 _percentOfPool = returnPercentageOfPool();
    ...
    function calculateRealRewardAmount(address _addressToCheck) public view returns (uint256) {
    ...
    function issueRewardTokens()  external onlyowner {

        for(uint256 i=0; i < _stakers.length; i++) {
            address _recipient = _stakers[i]; 
            uint256 _tempRewardDivisor = calculateRealRewardAmount(_recipient);
    ...
    function returnTotalStaked() public view returns (uint256) {
        return _totalStaked;        
    }    

    // Return the percentage of the pool the staker has staked
    function returnPercentageOfPool() public view returns (uint256) {
        ...
        _returnAmount = _returnAmount * 100;
