...
contract ShareTokenPool is Ownable {
    address private shareToken;
    event TransferShareToken(address to,uint256 amount);

    constructor(address _shareToken) {
        shareToken = _shareToken;
    }

    function transferShareToken(address to,uint256 amount) public onlyController {
        IERC20(shareToken).transfer(to,amount);
        emit TransferShareToken(to,amount);
    }
}