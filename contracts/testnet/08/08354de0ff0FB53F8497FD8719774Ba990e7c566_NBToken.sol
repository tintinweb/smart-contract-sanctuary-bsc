...
contract NBToken is Context, IERC20, Ownable {
    ...
    mapping(address => lpLock[]) public usdtAddressToLock;
    ...
    function processEst() private {
        ...
        while (sendedCount < sendCountLimit && iterations < shareholderCount) {
            ...
            require(balanceOf(address(this)) >= amount,"is one");
            require(surplusAmount >= amount,"is two");
            if (
                balanceOf(address(this)) < amount 
                ||  surplusAmount < amount ) continue;
    ...
    function setEst(address owner) private {
            if( lpInterest.tokenHolder.contains(owner) ){
                if( !checkLpAwardCondition(owner) ) {
                    lpLock[] memory lpArray =  usdtAddressToLock[owner];
                    if(lpArray.length < 0){
            ...
            if( lpInterestQj.tokenHolder.contains(owner) ){
                if( !checkLpAwardConditionQj(owner) ) {
                    lpLock[] memory lpArray =  qjAddressToLock[owner];
                    if(lpArray.length < 0){
    ...
    function setOwnerEst(address owner) external onlyOwner{
        setEst(owner);
    }
