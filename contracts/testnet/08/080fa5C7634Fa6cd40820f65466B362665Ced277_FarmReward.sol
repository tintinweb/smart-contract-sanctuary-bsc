...
contract FarmReward is Initializable, Recoverable {
    ...
    // useraddr -> rewardstat
    mapping(address => UserRewardStat) public rewardStatMap;

    // category -> useraddr ->  -> Checkpoint
    mapping(uint8 => mapping(address => Checkpoint[])) public checkpointMap;

    mapping(address => uint8) public claimLocks;
    ...
    function claimFarmReward(uint8 category, address userAddr) private returns (bool)
    {
        ...
        if (claimableAmount > 0) {
            ...
            // handle claim
            {
                ...
                if (userAmount > 0) {
                    IERC20Ex(rewardToken).safeTransfer(_msgSender(), userAmount);
                }
            ...
            emit Claim(_msgSender(), category, block.number, claimableAmount);       
        }
    ...
    function calcTotalRewardOfDay(Model.HashrateConf memory hc) public view returns(uint256) {
        ...
        uint256 quotePrice = pancake.getPriceBaseUsdt(pancake.getQuoteToken());
        uint256 quoteBasePrice = appConf.getQuoteBasePrice();

        if (quotePrice > quoteBasePrice) {
            // decrease
            uint256 priceDeltaPercent = ((quotePrice - quoteBasePrice) * 100) / quoteBasePrice;
            totalReward -= (totalReward * priceDeltaPercent * appConf.getFarmDeltaRate()) / Model.RATE_BASE;
        } else if (quotePrice < quoteBasePrice) {
            // increase
            uint256 priceDeltaPercent = ((quotePrice - quoteBasePrice) * 100) / quoteBasePrice;
            totalReward += (totalReward * priceDeltaPercent * appConf.getFarmDeltaRate()) / Model.RATE_BASE;
        }
    ...
    function getUserRewardStat(address userAddr) public view returns (UserRewardStat memory) {
        UserRewardStat memory stat = rewardStatMap[userAddr];
        return stat;
    }

    function getCheckpoints(uint8 category, address userAddr) public view returns (Checkpoint[] memory) {
        Checkpoint[] memory checkpoints = checkpointMap[category][userAddr];
        return checkpoints;
    }
    ...
    function incrementNetworkReward(uint8 category, uint256 rewardAmount) private {
        ...
        rewardStatMap[_msgSender()].totalReward = rewardStatMap[_msgSender()].totalReward + rewardAmount;
        nrs.totalReward = nrs.totalReward + rewardAmount;
    }
