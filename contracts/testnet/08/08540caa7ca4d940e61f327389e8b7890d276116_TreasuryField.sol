...
contract TreasuryField is ReentrancyGuard {
    ...
    uint256 public constant REWARD_RATE = 2976;
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            ...
            (((block.timestamp - s_lastUpdateTime) * REWARD_RATE/10**10));
    }
    ...
    function earned(address account) public view returns (uint256) {
        return
            ((s_balances[account] * (rewardPerToken() - s_userRewardPerTokenPaid[account]))) + s_rewards[account];
    }
    ...
    function stake(uint256 amount)
        ...
        moreThanZero(amount)
    {
        //require(80*s_balances[address(this)]/100 > s_totalSupply, "Maximum Participants reached");
        /**if (address(this) ==referer) {
            
        }**/
        s_totalSupply += amount;
        s_balances[msg.sender] += amount;
        /**else {
            s_totalSupply += amount*110/100;
            s_balances[msg.sender] += amount;
            s_balances[referer] += amount*10/100;
        }**/
        emit Staked(msg.sender, amount);
        //emit Staked(referer, amount*10/100);
    ...
    function withdraw(uint256 amount) external updateReward(msg.sender) nonReentrant {
        s_totalSupply -= amount*90/100;
        s_balances[msg.sender] -= amount;
        emit WithdrewStake(msg.sender, amount);
        bool success = s_stakingToken.transfer(msg.sender, amount*90/100);
        if (!success) {
            revert TransferFailed();
        }
    }
