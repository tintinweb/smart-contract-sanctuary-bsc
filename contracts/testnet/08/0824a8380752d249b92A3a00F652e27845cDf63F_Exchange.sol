...
contract Exchange is TRC21, Ownable {
    struct item {
        uint256[] tokenIds;
    }
    struct items {
    ...
    event _getTicket(address _from, uint256 ticket);
    event _setAward(address _to, address _game, uint256 tokenId, address _creator);
    ...
    function buyTicket(uint256 _ticket) public {
        require(tickets[_ticket].user == address(0));
        require(OwaDrawTrc21.transferFrom(msg.sender, address(this), ticketPrice));
        tickets[_ticket] = ticket(msg.sender, false);
        emit _getTicket(msg.sender, _ticket);
    }
    ...
    function setAwardTRC721(uint256 _ticket, address _user, address _creator) public onlyManager{
        require(validTicket(_ticket));
        uint256 _tokenId = OwaDrawTrc721.create(_user, _creator);
        awardDatas[_user].items[OwaDrawTrc721].tokenIds.push(_tokenId);
        awardDatas[_user].totalItem +=1;
        tickets[_ticket].isUsed = true;
        emit _setAward(_user, OwaDrawTrc721, _tokenId, _creator);
    }
