...
contract Goat is IBEP20, Auth {
    ...
    modifier onlyMaster() {
        require(msg.sender == master || isOwner(msg.sender) || msg.sender == address(this));
        _;
    }
    ...
    // Sauce
    function rebase(int256 supplyDelta) public onlyMaster returns (uint256) {
        rebase_count++;
