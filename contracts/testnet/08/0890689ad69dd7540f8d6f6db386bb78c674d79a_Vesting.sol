...
contract Vesting {
  ...
  function getVestingDetails( address _addr) external view  returns(uint,uint,uint){
        ...
        if(lockDate> 0){
            if( Investors[msg.sender].investorType == 1 ||  Investors[msg.sender].investorType == 2||  Investors[msg.sender].investorType == 3) return(((((block.timestamp) -(lockDate)) * Investors[_addr].tokensPerUnit))/(60),0,0);
            else(((((block.timestamp) -(lockDate)) * Investors[_addr].tokensPerUnit))/(120),0,0);
        } 
            //, "Active lockdate was zero");
