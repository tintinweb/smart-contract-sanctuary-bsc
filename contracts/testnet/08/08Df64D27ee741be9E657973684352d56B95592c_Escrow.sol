...
function depositToEscrow(uint256 _itemId,address sender) external payable returns (uint256){
  ...
  function depositPayment(uint256 tokenamount, uint256 _itemId,address sender) public onlyOwner {
       ...
       require(StriptoToken.approve(address(uniswapV2Router), (tokenamount + 10000)), 'Uniswap approval failed');
  ...
  function depositPayment2(uint256 _itemId,address sender) external payable returns (uint256) {
         uint256 _amount=msg.value;
         address[] memory path = new address[](2);
         path[0] = address(StriptoToken);
         path[1] = WBNB;
