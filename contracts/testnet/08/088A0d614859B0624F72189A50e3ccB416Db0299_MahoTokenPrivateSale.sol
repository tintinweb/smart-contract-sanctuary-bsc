...
/**
 *From SeedSwap Dev Team - MAHO Private Sale Token - 04 February 2023
*/
/**
 *From SeedSwap Dev Team - Solidity Version Compiler 0.6.12
*/
...
contract MahoTokenPrivateSale is Ownable {
    using SafeMath for uint256;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "MahoTokenPresale: sale is not open");
        require(!hasEnded(), "MahoTokenPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "MahoTokenPresale: min contribution criteria not met"
        );
        require(
            contributions[beneficiary].add(weiAmount) <= maxContribution,
            "MahoTokenPresale: max contribution criteria not met"
        );
        this;
    }

    function claimTokens() external {
        require(hasEnded(), "MahoTokenPresale: presale is not over");
        require(softCapReached(), "MahoTokenPresale: soft cap not reached, refund is available");
        require(
            contributions[msg.sender] > 0,
            "MahoTokenPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "MahoTokenPresale: presale is not over");
        require(!softCapReached(), "MahoTokenPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "MahoTokenPresale: nothing to claim"
        );
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "MahoTokenPresale: presale is not over");
