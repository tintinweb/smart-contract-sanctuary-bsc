...
contract BUSDVYNCSTAKE is ReentrancyGuard, Ownable {
    ...
    uint256 public slippage = 3; //can be modify using set_slippage() function(between 1-40%)
    ...
    event ChangePriceSetAddress(address newPriceSetAddresss);
    ...
    function changePriceSetAddress(address _address) public onlyOwner {
        ...
        emit ChangePriceSetAddress(_address);
    }
    ...
    function set_slippage(uint256 _slippage) public onlyOwner {
        require(_slippage > 0 && _slippage <= 40, "invalid slippage range");
        slippage = _slippage;
    ...
    function stake(uint256 amount) external nonReentrant {
        require(busd.transferFrom(msg.sender, address(this), amount),"unable to transfer");
        userInfo[msg.sender]
        ...
        require(busd.transfer(msg.sender, amount - (busdAdded + amountToSwap)), "unable to transfer left amount");
        }
        ...
        nonReentrant
    {
        ...
        if (unstakeOption == 1) {
            require(true == busd.transfer(msg.sender, _amount),"unable to transfer: option1");
            ...
            require(true == busd.transfer(msg.sender, busdAmount),"unable to transfer:busd,option2");
            require(true == vync.transfer(msg.sender, _vyncAmount),"unable to transfer:vync,option2");
        } else if (unstakeOption == 3) {
            uint256 vyncAmount = swapBusdToVync(_amount);
            require(true == vync.transfer(msg.sender, vyncAmount),"unable to transfer:option3");
        }
    ...
    function transferAnyERC20Token(
        ...
        require(true == IERC20(_tokenAddress).transfer(_to, _amount),"unable to transfer");
    }
