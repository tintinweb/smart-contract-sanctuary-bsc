...
contract DUCK is ERC20, Ownable{
    ...
    uint256 public buyInspireFee = 2;
    uint256 public sellInspireFee = 2;
    ...
    address public inspireWalletAddress = 0xe9aAe1cF234E36D4A15b59D7CfB7447b7C697e20;
    ...
    event MultiTransfer(address[] addresses, uint256 amount);
    ...
    constructor(
        ...
        require(buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyDeadFee).add(buyInspireFee) <= 25, "Total buy fee is over 25%");
        require(sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(sellDeadFee).add(sellInspireFee) <= 25, "Total sell fee is over 25%");

        uint256 totalSupply = 10000000000 * (10**18);
        swapTokensAtAmount = totalSupply.mul(2).div(10**6); // 0.002%
        uint256 tokenBalanceForReward = 10 ** 18;

        // use by default 300,000 gas to process auto-claiming dividends
        gasForProcessing = 300000;

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(0x885063EaCC2cB8F8be56A5cb21069E284AeF4BE1))
        );

        dividendTracker.initialize(
            rewardToken,
            tokenBalanceForReward
        );
    ...
    function setInspireWallet(address payable wallet) external onlyOwner{
        inspireWalletAddress = wallet;
        excludeFromFees(wallet, true);
    }
    ...
    function swapManual() public onlyOwner {
        ...
        if(AmountInspireFee > 0) swapAndSendToInspireFee(AmountInspireFee); 
        swapping = false;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                ...
                fees = LFee.add(RFee).add(MFee).add(DFee).add(IFee);
            }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        IERC20(rewardToken).transfer(marketingWalletAddress, newBalance);
        AmountMarketingFee = AmountMarketingFee - tokens;
    }

    function swapAndSendToInspireFee(uint256 tokens) private  {
        uint256 initialCAKEBalance = IERC20(rewardToken).balanceOf(address(this));
        swapTokensForCake(tokens);
        uint256 newBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(initialCAKEBalance);
        IERC20(rewardToken).transfer(inspireWalletAddress, newBalance);
        AmountInspireFee = AmountInspireFee - tokens;
    }
    ...
    function multiTransferTokens(address[] calldata addresses, uint256 amount) public returns (bool) {
        require(addresses.length <= 300, 'Not More than 300 address');
        uint256 tokens = addresses.length * amount * decimals();
        require(balanceOf(_msgSender()) >= tokens, 'Not enough tokens!');
        for (uint256 i = 0; i < addresses.length; i++){
            super._transfer(_msgSender(), addresses[i], amount);
        }
        emit MultiTransfer(addresses, amount);
        return true;
    }
}