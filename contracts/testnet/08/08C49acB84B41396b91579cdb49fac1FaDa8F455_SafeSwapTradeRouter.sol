        ...
        if (tokensFeeList[_assetIn].tokenInfo.isEnabled) {
            for (uint256 i; i < tokensFeeList[_assetIn].singleSwapFees.length; i++) {
                if (
                    ...
                    uint256 swapKindPercentage = (tokensFeeList[_assetIn].singleSwapFees[i].percentage * percent) /
                        tokensFeeList[_assetIn].tokenInfo.feePercentage;

                    _feeAmount = ((_feeAmount * swapKindPercentage) / percent);
