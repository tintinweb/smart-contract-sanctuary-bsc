...
interface ILpDeposit {
    function rwd(address token, uint256 amount) external returns (bool);
}
...
contract MetaPoint0519T is Ownable {
    ...
    function up() public {
        ...
        if (accounts[msg.sender].point == 0) {
            ...
            accounts[msg.sender].rootLevel = level;
            accounts[msg.sender].rootSn = sn;
        } else {
        ...
        IERC20(lp).safeTransfer(lpds, (expend * allot[4]) / 100);
    ...
    function grwCreator(uint256 amount) public onlyOwner {
        require(amount <= creatorBalance, "Insufficient creatorBalance");
        require(amount / effectTotal < 2e18, "Too much amount one time");
        acc += amount / effectTotal;
        creatorBalance -= amount;
    }

    function grwCreatorEach(uint256 each) public onlyOwner {
        uint256 amount = (effectTotal * each) / 1e18;
        require(amount <= creatorBalance, "Insufficient creatorBalance");
        require(each < 2e18, "Too much amount one time");
        acc += each;
        creatorBalance -= amount;
    }
    ...
    function take(address head, uint200 id) private returns (uint32 level_, uint32 sn_) {
        uint32 level = accounts[head].level;
        uint32 sn = accounts[head].sn;
        uint32 rootLevel = accounts[head].rootLevel;
        uint32 rootSn = accounts[head].rootSn;
        ...
        while (true) {
            ...
            while (j < (rootSn + 1) * uint32(2**i)) {
                if (tree[level][j] == 0) {
                    tree[level][j] = id;
                    console.log("this time point up(%s,%s)=>%s", level, j, id);
                    emit Log(level, j, id);
                    return (level, j);
                }
                j++;
    ...
    function concat(address addr, uint32 sn) private pure returns (uint200) {
        return uint200(sn) * 2**160 + uint160(addr);
    }
