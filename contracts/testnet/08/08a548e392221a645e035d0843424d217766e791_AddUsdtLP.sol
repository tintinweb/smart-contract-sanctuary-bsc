...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from == mainPair || to == mainPair) {
            ...
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) {
                ...
                if( contractTokenBalance>0 && !inSwap && openSwap ){
                  claimUsdt(contractTokenBalance);
                }
            }
        }
    ...
    function ownerClaimUsdt() external onlyOwner {
        claimUsdt( balanceOf(address(this) ) );
    } 

    function claimUsdt(uint256 feeAmount) private{
        //把usdt转到分红地址
