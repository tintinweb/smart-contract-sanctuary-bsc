...
contract Marketplace is AccessControlUpgradeable, ERC1155HolderUpgradeable {
    ...
    function initialize(
        ...
        __AccessControl_init_unchained();
        __ERC1155Holder_init_unchained();
        __Marketplace_init_unchained(msg.sender, _carbonCredit, _feeManager);
    }
    ...
    function list(
        ...
        require(carbonCredit.balanceOf(msg.sender, _tokenId) != 0, "need a token");
        require(_endTime > _startTime, "invalid time");
        require(_price != 0, "invalid price");
        require(carbonCredit.isApproved(_tokenId), "not approved");
        require(!carbonCredit.isExpired(_tokenId), "already expired");
        ...
        carbonCredit.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");

        emit List(_listingId, msg.sender, _tokenId, _acceptedToken, _price, _amount, _startTime, _endTime);
    }
        ...
        external
    {
        ...
        carbonCredit.safeTransferFrom(address(this), msg.sender, _listing.tokenId, _listing.amount, "");
    ...
    function buy(
        ...
        _safeDeposit(_listing.acceptedToken, msg.sender, _buyValue, msg.value);

        _safeTransfer(_listing.acceptedToken, _feeCollector, _platformFee);
        _safeTransfer(_listing.acceptedToken, _listing.owner, _finalAmount);

        carbonCredit.safeTransferFrom(address(this), msg.sender, _listing.tokenId, _amount, "");

        emit Buy(_listingId, msg.sender, _amount);
    }
    ...
    function offer(
        ...
        uint256 _startTime, 
        uint256 _endTime
    ) payable public returns (uint256 _offerId) {
        require(carbonCredit.exists(_tokenId), "not exitst");
        require(_endTime > _startTime, "invalid time");
        require(_price != 0, "invalid price");
        require(carbonCredit.isApproved(_tokenId), "not approved");
        require(!carbonCredit.isExpired(_tokenId), "already expired");
        ...
        _safeDeposit(_acceptedToken, msg.sender, _wantToBuyValue, msg.value);

        emit Offer(_offerId, msg.sender, _tokenId, _acceptedToken, _price, _amount, _startTime, _endTime);
    }
    ...
    function cancelOffer(
        ...
        _safeTransfer(_offer.acceptedToken, msg.sender, _offer.price.mul(_offer.amount));
    ...
    function sell(
        ...
        _safeTransfer(_offer.acceptedToken, _feeCollector, _platformFee);
        _safeTransfer(_offer.acceptedToken, msg.sender, _finalAmount);

        carbonCredit.safeTransferFrom(msg.sender, _offer.owner, _offer.tokenId, _amount, "");

        emit Sell(_offerId, msg.sender, _amount);
    }
    ...
    function _safeDeposit(
        ...
        if (_token == address(0)) { // native
            require(_msgValue >= _amount, "not enough native");
        } else { // erc20
...
import "../libraries/CarbonCredits.sol";

interface ICarbonCredit is IERC1155Upgradeable{
    function isApproved(uint256 tokenId) external view returns (bool);
