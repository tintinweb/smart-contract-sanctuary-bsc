...
interface IMarketplace {
    ...
    function buyByCurrency(uint256[] memory _nftId, uint256 _refCode) external;

    function buyByToken(uint256[] memory _nftId, uint256 _refCode) external;

    function sell(uint256[] memory _nftId) external;
...
contract Marketplace is IMarketplace, Ownable, ERC721Holder {
     ...
     * @param _nftId list NFT ID want to buy
    ...
    function buyByCurrency(
        ...
        require(_nftId.length > 0, "MARKETPLACE: INVALID LIST NFT ID");
        require(_nftId.length <= 20, "MARKETPLACE: TOO MANY NFT IN SINGLE BUY");
        // Prevent re-entrancy
        ...
        for (uint index = 0; index < _nftId.length; index++) {
            uint256 priceNftUsd = AnotherNFT(nft).getNftPriceUsd(_nftId[index]);
            require(priceNftUsd > 0, "MARKETPLACE: WRONG NFT ID TO BUY");
            require(
                AnotherNFT(nft).ownerOf(_nftId[index]) == address(this),
        ...
        for (uint index = 0; index < _nftId.length; index++) {
            try AnotherNFT(nft).safeTransferFrom(address(this), msg.sender, _nftId[index], data) {
                emit Buy(address(this), msg.sender, _nftId[index], refAddress);
    ...
    /**
     * @dev sell function
     * @param _nftId list NFT ID want to sell
     */
    function sell(uint256[] memory _nftId) public override notExpireForSelling {
        require(_nftId.length > 0, "MARKETPLACE: INVALID LIST NFT ID");
        // Prevent re-entrancy
        ...
        for (uint index = 0; index < _nftId.length; index++) {
            require(
                AnotherNFT(nft).ownerOf(_nftId[index]) == msg.sender,
        ...
        bytes memory data = abi.encode(msg.sender);
        for (uint index = 0; index < _nftId.length; index++) {
            try AnotherNFT(nft).safeTransferFrom(msg.sender, address(this), _nftId[index], data) {
                emit Sell(msg.sender, address(this), _nftId[index]);
            } catch (bytes memory _error) {
...
contract AnotherNFT is ERC721, Ownable, Pausable, ERC721Burnable {
