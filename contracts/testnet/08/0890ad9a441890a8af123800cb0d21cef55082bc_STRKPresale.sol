...
//SPDX-License-Identifier: UNLICENSED
  ...
  contract STRKPresale{
    ...
    function claimRoi() external payable{
      ...
      user.total_withdraw = user.total_withdraw.add(roi);
    ...
    function WithdrawBuyedToken () external payable {
      ...
      for(uint256 i = 0; i<user.roi.length; i++) {
        if(user.roi[i].timestamp.add(plan.locking_period)<block.timestamp){
          total=total.add(user.roi[i].amount);
          user.roi[i].isPricipleWithdraw =true;
        }
      }
