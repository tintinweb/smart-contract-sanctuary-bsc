...
contract ShiburaiStaking is Ownable {
    ...
    uint256[3] public durations = [10 minutes, 10 minutes, 10 minutes]; // lock durations
    ...
    function stake(uint256 amount, uint256 planIndex) public {
        ...
        userStakeInfo.reward = amount.mul(wpyBonus[planIndex]).div(
            PERCENT_DENOMINATOR
        );
        userStakeInfo.stakeTime = block.timestamp;
        userStakeInfo.unlockTime = block.timestamp.add(durations[planIndex]);
        userStakeInfo.perSecondReward = userStakeInfo.reward.div(WEEK);
    ...
    function withdraw(uint256 index) public {
        ...
        uint256 remaingReward = calculateRemainingReward(index);
    ...
    // ========================== View Functions ==========================
    function calculateRemainingReward(uint256 index)
        public
        view
        ...
        if (block.timestamp < userStakeInfo.unlockTime) {
            ...
            if (userStakeInfo.plan == 0) {
                ...
                if (durationInWeeks >= 8) {
                    reward = userStakeInfo.reward * 2;
                }
            }
            if (userStakeInfo.plan == 2) {
                if (durationInWeeks >= 16) {
                    reward = userStakeInfo.reward * 4;
                }
            }
        } else {
            reward = userStakeInfo.reward.mul(
        ...
        for (uint256 i = 1; i <= _user.stakeCount; i++) {
            if (!_user.stakingInfo[i].unstaked) {
                ...
                uint256 currentReward = duration.mul(
                    _user.stakingInfo[i].perSecondReward
                );
