...
contract Nft is Context, Ownable {
    ...
    address public nftAddress=0x608B9Bf67e56724DC1Db432023C99C371b112Af9;
    ...
    function Claim(uint256 nftId,uint256 seed,uint8 v, bytes32 r, bytes32 s) external
    {
        require(isClaimed[msg.sender][nftId]==false,"Allready claimed");
        string memory hash = keccak256(abi.encode(nftId, seed, msg.sender)).toString();
        address signer = hash.recover(v, r, s);
        require(signer == authAddress, "Invalid signature");
        IERC1155(nftAddress).mint(_msgSender(), nftId, 1);
        isClaimed[msg.sender][nftId] = true;
    }
