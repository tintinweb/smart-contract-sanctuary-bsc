...
contract SHUEYRHONRHONDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SHUEYRHONRHON_Dividend_Tracker", "SHUEYRHONRHON_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**2); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SHUEYRHONRHON_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SHUEYRHONRHON_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SHUEYRHONRHON contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SHUEYRHONRHON_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SHUEYRHONRHON_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SHUEYRHONRHON is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    SHUEYRHONRHONDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000000* (10**2);
    uint256 public swapTokensAtAmount = 2 * 10**4 * (10**2);
    ...
    constructor() ERC20 ("SHUEYRHONRHON", "SHUEYRHONRHON") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
    	...
    	dividendTracker = new SHUEYRHONRHONDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SHUEYRHONRHON: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SHUEYRHONRHON: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SHUEYRHONRHON: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SHUEYRHONRHON: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SHUEYRHONRHON: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SHUEYRHONRHON: Cannot update gasForProcessing to same value");
