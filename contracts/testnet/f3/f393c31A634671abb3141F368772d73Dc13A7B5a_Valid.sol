...
contract Valid is Context, IERC20, IERC20Metadata, Ownable {
    ...
    */
    ...
    //uint256 private constant MAX = 10 ** 6;
    //-----------------------
    bool private constant DEBUG = true;
    //-----------------------

    /* Basic token configuration */
    uint256 private constant SUPPLY = 10 * 10 ** 18;    // 10 * 10 ** 18
    string private constant COIN_NAME = "Valid Token";
    string private constant COIN_SYMBOL = "VALID";
    uint8 private constant DECIMALS = 9;   // 9

    /* Fee configuration */
    uint256 private constant TAX_FEE_PERCENT = 10;   // 3
    uint256 private constant LIQUIDITY_FEE_PERCENT = 0;     // 10

    /* Transaction and liquidity limit configuration */
    uint256 private constant MAX_TX_AMOUNT = SUPPLY; // TODO ?
    uint256 private constant MAX_SELL_LIQ_AMOUNT = 5000000;
    ...
    /* Balance */
    mapping (address => uint256) public _rOwned;   // TODO private
    mapping (address => uint256) public _tOwned;   // TODO private
    mapping (address => mapping (address => uint256)) private _allowances;

    /* Include/exlude */
    mapping (address => bool) private _isExcluded;
    mapping (address => bool) public _isExcludedFromFee;   // TODO private
    ...
    /* Total supply */
    uint256 public _tTotal;    // TODO private
    uint256 public _rTotal;    // TODO private
    uint256 public _tFeeTotal; // TODO private

    /* Liquidity */
    uint256 public _maxTxAmount;
    uint256 public _numTokensSellToAddToLiquidity;

    IUniswapV2Router02 public _uniswapV2Router; // TODO immutable
    address public _uniswapV2Pair; // TODO immutable
    ...
    address private _routerAddress; // TODO Remove? (It is only for debug!)
    ...
    // event SwapAndSend(uint256 tokensSwapped, uint256 bnbReceived, bool success);
    event Debug(bool overMinTokenBalance, uint256 contractTokenBalance, uint256 _numTokensSellToAddToLiquidity, bool _swapAndLiquifyEnabled, bool _inSwapAndLiquify, address from, address _uniswapV2Pair);
    ...
    constructor(address routerAddress) {
        ...
        _inSwapAndLiquify = false;
        _swapAndLiquifyEnabled = false;     // true
        ...
        // Exclude owner and this contract from paying fees
        _isExcludedFromFee[_msgSender()] = true;
        _isExcludedFromFee[address(this)] = true;

        // Exclude owner, this contract and swap router from getting rewards
        // IMPORTANT: It is permanent. There is no way to re-include any account into the rewarding system!
        excludeFromReward(_msgSender());
        excludeFromReward(address(this));
        excludeFromReward(routerAddress);
        excludeFromReward(_uniswapV2Pair);  // TODO Is it there in the original version somehow? It should be!
    ...
    function mint(uint256 amount) public onlyOwner {
        uint256 rTotalDiff = (MAX - (MAX % _tTotal)) - _rTotal;
        _tTotal += amount * 10 ** _decimals;
        _rTotal += (MAX - (MAX % _tTotal)) - rTotalDiff;
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {   // TODO external?
        if (DEBUG) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        emit Debug(overMinTokenBalance, contractTokenBalance, _numTokensSellToAddToLiquidity, _swapAndLiquifyEnabled, _inSwapAndLiquify, from, _uniswapV2Pair);

        // TODO from != _routerAddress ?!
        if (overMinTokenBalance && !_inSwapAndLiquify && from != _uniswapV2Pair && from != _routerAddress && _swapAndLiquifyEnabled) {
    ...
    // TODO Is owner allowed to modify tax fee after contract deployment?!!
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        _taxFee = taxFee;
    }

    // TODO Is owner allowed to modify liquidity fee after contract deployment?!!
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        _liquidityFee = liquidityFee;
    }

    // TODO Is owner allowed to modify this value after contract deployment?!!
    ...
    // TODO Is owner allowed to modify this value after contract deployment?!!
    // TODO What is the meaning of 'percent' here?! Check for errors!
    ...
    // TODO No special wallets?! External?
