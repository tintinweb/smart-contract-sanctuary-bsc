...
contract MelonVers is ERC721, Ownable{
    ...
    uint256 public usd;
    uint256 public usd10;
    uint256 public aafterchangerate;
    address public usdt = 0xC88887bCa276Af4D577a54f4F5376875d628c4a7;
    // uint256 public lastusd;
    uint256 public incrementAmount;
    uint256 public lockdays;
    // AggregatorV3Interface internal priceFeed;
    address public Directpurchase;
    uint256 public TXFEE;
    address public Admin;
    constructor(string[] memory countryName,uint256[] memory _Totalblock) ERC721("MELONVERS", "MV"){
        ...
        // -------
        //  0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419 -- eth mainnet
        // 	0x8A753747A1Fa494EC906cE90E9f37563A8AF630e -- Rinkeby Testnet
        usd = usd + 1000 * 10 ** 6;
        usd10 = usd10 + 3000 * 10 ** 6;
        aafterchangerate = aafterchangerate + 60 ;
        // lastusd =  lastusd + 1200 ;
        incrementAmount =  incrementAmount + 100 * 10 **6;
        lockdays = lockdays + 300;
        starttime = block.timestamp;
        Directpurchase = msg.sender;
        Admin = msg.sender;
    ...
    function changeAfterDays(uint256 _days) public onlyOwner returns(bool){
        aafterchangerate = _days;
        return true;
    }
    // function changeLastUsd(uint256 _Lastusd) public onlyOwner returns(bool){
    //     lastusd = _Lastusd;
    //     return true;
    // }
    function changeIncrementAmount(uint256 _amount) public onlyOwner returns(bool){
    ...
    function Getcurrentrate()public view returns(uint256,uint256){
        if(block.timestamp > starttime + (1 days * aafterchangerate) ){
            //uint256 letestprice = uint256(getLatestPrice());
            uint256 a = usd.add(incrementAmount);
            uint256 b = usd10.add(incrementAmount);
            //uint256 updaterate = uint256(a*10**18/letestprice);
            // return updaterate;
            return (a,b);
        }
        //uint256 price = uint256(getLatestPrice());
        //uint256 buy = ((usd*10**18) / price);
        //return buy;
        return (usd,usd10);
    }
    ...
    function changeUSD(uint256 _a,uint256 _a10) public onlyOwner returns(bool)  {
        usd = _a;
        usd10 = _a10;
        emit upd(usd,usd);
        emit upd(_a10,_a10);
    ...
    function AssignToken(int[] memory x,int[] memory y,address[] memory _a,string memory _countryname) public onlyAdmin returns(bool){
        ...
        for(uint256 i=0;i<x.length;i++){
            ...
            _mint(_a[i], id);
            emit onMint(id, x[i], y[i], _a[i],usd,0);
        ...
        mintedamountblock[_countryname][usd] = mintedamountblock[_countryname][usd] + x.length;
        return true;
    }
    function plentblockMint(int256 x,int256 y,string memory _countryname) private {
        ...
        _mint(_msgSender(), id);
        emit onMint(id, x, y, msg.sender,usd,buyprice);
        tokenlock[id] = block.timestamp + (1 days * lockdays);
    }
    ...
    function changeTXFEE(uint256 _A)public onlyOwner returns(bool){
        TXFEE = _A;
        return true;
    }
    ...
    function ChangeDirectpurchase(address _Directpurchase) public onlyOwner returns(bool){
    ...
    function sell(uint256 _tokenId, uint256 _price) public returns(bool){
        ...
        IERC721(address(this)).transferFrom(_msgSender(), address(this), _tokenId);
    ...
    // event refund(address biduser, uint256 bidamount);
    // event onBid(uint256 _tokenId, address biduser,uint256 bidamount);
    // function createAuction_(uint256[] memory _tokenId, uint256[] memory _startingTime, uint256[] memory _closingTime, uint256[] memory _reservePrice) public returns(bool){
    ...
    //         IERC721(address(this)).transferFrom(msg.sender, address(this), _tokenId[i]);
    //         sellstatus[_tokenId[i]] = true;
    //         emit sell_auction_create(_tokenId[i], msg.sender, _startingTime[i], _closingTime[i], _reservePrice[i], true);
    //     }
    ...
    //     require(auctionDetails[_tokenId].beneficiary == msg.sender || support == msg.sender,"48");
    ...
    //         payable(support).transfer(amount4admin_);
    ...
    //     require(auctionDetails[_tokenId].beneficiary == msg.sender,"81");
