...
contract MultiLogic is LogicUpgradeable {
    ...
    mapping(address => bool) public approvedTokensLogic;
     ...
     * @param _token Address of token
     */
    function testFunc(uint256 _amount, address _token) external {
        
        IERC20Upgradeable(_token).safeTransferFrom(
            multiStrategy[0].logicContract,
            address(this),
            _amount
        );
     ...
     * @param _token Address of token
     */
    function testFuncFromStorage(uint256 _amount, address _token) 
        external 
        onlyStorage
    {
        uint256 _count = multiStrategyLength;
        ...
        onlyStorage
    {
        ...
        for (uint256 i = 0; i < _count; i++) {
            ...
            if (logicBalance <= _amount) {
                IStrategyContract(sStrategy.strategyContract)
                ...
                tokenAvailableLogic[_token][sStrategy.logicContract] += logicBalance;
            } else {
                IStrategyContract(sStrategy.strategyContract)
                ...
                tokenAvailableLogic[_token][sStrategy.logicContract] += _amount;
            }
