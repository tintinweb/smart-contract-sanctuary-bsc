...
contract WOWNFT is ERC721, Pausable, Ownable {
    ...
    bool private privateSaleEnabled = false;
    uint256 private privateSaleLimit = 20000;
    ...
    modifier onlyAuthorized() {
        require(authorized[msg.sender] || owner() == msg.sender, "Ownable: caller is not authorized");
        _;
    }
     ...
     */
    function mintToken(uint256 _price) payable external whenNotPaused returns(uint256) {
        if (privateSaleEnabled) {                                                                               // If the privateSale is enabled
            if (whitelistEnabled) {
				        require(whitelistContains(msg.sender), "Whitelisted only");
			      }
            require(_price == tokenPrice, "Not enough BUSD");
            BUSDAddress.transferFrom(msg.sender, address(this), tokenPrice);                                     // On privateSale user pays with BUSD
        } else {                                                                                                // When OUT of privateSale
            if (!authorized[msg.sender] && msg.sender != owner()) {                                             // If the user who wants to mint is NOT an admin
                uint256 totalTokensAmount = 500e18 / WCoinToken.getTokenPrice();                              
                require(msg.value <= totalTokensAmount, "Price over transaction limit");                        // Check over $500 Transaction limit
                uint256 finalPrice = tokenPrice;
                if (feeEnabled) {    
                    uint256 BNBFee = getBNBFee();   
                ...
                require(_price == tokenPrice, "Not enough WCoin");

                WCoinToken.transferFrom(msg.sender, payable(address(this)), tokenPrice);                        // Pays with WCoinToken
            } 
        }
        ...
        tokensOwned[msg.sender].push(TokenMetadata(tokenID, tokenPrice, _baseURI(), block.timestamp));
        emit BoughtNFT(msg.sender, msg.value, tokenID);
        return tokenID;
    }

    function sell(uint256 _tokenID, uint256 _price, address _buyer) payable external whenNotPaused {
    ...
    function _getTokenFromUser(address _userAddress, uint256 _tokenID) private view returns (TokenMetadata memory) {
        require(_validateUser(_userAddress, _tokenID), "User is not the token owner");
        for (uint i = 0; i < tokensOwned[_userAddress].length; i++) {
            if (tokensOwned[_userAddress][i].tokenID == _tokenID) {
