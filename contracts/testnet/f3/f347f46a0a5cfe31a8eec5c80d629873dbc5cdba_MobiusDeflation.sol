...
contract MobiusDeflation is Modifier, Util {
    ...
    mapping(address => bool) private _isAuthorizedAddPool;

    uint private historyAmount;
    uint private usdtPoolAmount;
    uint private mobPoolAmount;
    ...
    constructor() {
        _isAuthorizedAddPool[msg.sender] = true;
        awardPoolRatio = 60;
        teamAwardRatio = 20;
        destroyRatio = 10;
        swapRatio = 5;
    ...
    function excludeAddPool(address _address) public onlyOwner {
        _isAuthorizedAddPool[_address] = false;
    }

    function includeAddPool(address _address) public onlyOwner {
        _isAuthorizedAddPool[_address] = true;
    }

    function isAuthorizedAddPool(address _address) public view returns (bool) {
        return _isAuthorizedAddPool[_address];
    }
    ...
    function joinDeflation(uint256 amountToWei) public isRunning nonReentrant returns (bool) {
        ...
        for(uint i = 0; i < joinCopies; i++) {
            (uint awardType, uint oneAward, uint twoAward, uint power) = lottery(block.number, i, joinCopies);
        ...
        historyAmount = historyAmount.add(amountToWei);
        uint intoPoolAmount = amountToWei.mul(awardPoolRatio).div(100);
        usdtPoolAmount = usdtPoolAmount.add(intoPoolAmount);
    ...
    function lottery(uint seed, uint salt, uint sugar) private returns (uint awardType, uint oneAward, uint twoAward, uint power) {
        // 1 - 100(both incl.)
        ...
        if(randomNumber >= 82) {
            ...
            awardType = 15;
            oneAward = usdtPoolAmount.mul(175).div(100000); // 0.175%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(175).div(100000); }
        } else if(randomNumber >= 37 && randomNumber <= 49) {
            awardType = 13;
            oneAward = usdtPoolAmount.mul(25).div(10000); // 0.25%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(25).div(10000); }
        } else if(randomNumber >= 26 && randomNumber <= 36) {
            awardType = 11;
            oneAward = usdtPoolAmount.mul(50).div(10000); // 0.5%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(50).div(10000); }
        } else if(randomNumber >= 17 && randomNumber <= 25) {
            awardType = 9;
            oneAward = usdtPoolAmount.mul(75).div(10000); // 0.75%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(75).div(10000); }
        } else if(randomNumber >= 10 && randomNumber <= 16) {
            awardType = 7;
            oneAward = usdtPoolAmount.mul(100).div(10000); // 1%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(100).div(10000); }
        } else if(randomNumber >= 5 && randomNumber <= 9) {
            awardType = 5;
            oneAward = usdtPoolAmount.mul(200).div(10000); // 2%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(200).div(10000); }
        } else if(randomNumber >= 2 && randomNumber <= 4) {
            awardType = 3;
            oneAward = usdtPoolAmount.mul(300).div(10000); // 3%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(300).div(10000); }
        } else {
            awardType = 1;
            oneAward = usdtPoolAmount.mul(500).div(10000); // 5%
            if(mobPoolAmount >= 1) { twoAward = mobPoolAmount.mul(500).div(10000); }
        }

        if(awardType != 17 && awardType != 19) {
            usdtPoolAmount = usdtPoolAmount.sub(oneAward);
            if(mobPoolAmount >= 1) {
                mobPoolAmount = mobPoolAmount.sub(twoAward);
    ...
    function getDeflationRecord(uint _number, address _address, uint index) public view returns (uint awardType, uint oneAward, uint twoAward, uint power) {
        awardType = deflationRecord[_number][_address][index].awardType;
        oneAward = deflationRecord[_number][_address][index].oneAward;
        twoAward = deflationRecord[_number][_address][index].twoAward;
        power = deflationRecord[_number][_address][index].power;
    }

    function getDeflationInfo() public view returns (uint networkAmount, uint usdtAmount, uint mobAmount) {
        networkAmount = historyAmount;
        usdtAmount = usdtPoolAmount;
        mobAmount = mobPoolAmount;
    }

    function addUsdtToPool(uint amountToWei) public {
        require(_isAuthorizedAddPool[msg.sender], "Modifier: The caller is not the approveAddress");
        usdtPoolAmount = usdtPoolAmount.add(amountToWei);
    }

    function addMobToPool(uint amountToWei) public {
        require(_isAuthorizedAddPool[msg.sender], "Modifier: The caller is not the approveAddress");
        mobPoolAmount = mobPoolAmount.add(amountToWei);
    }
