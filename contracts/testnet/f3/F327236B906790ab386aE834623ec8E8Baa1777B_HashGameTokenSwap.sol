...
contract HashGameTokenSwap is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   *play hash game
   ...
   function playgame(string memory internalTx_, address receiver_, uint256 amount_, bytes memory userSignature_) external nonReentrant validTx(internalTx_) swapDataNotExisted(internalTx_)
   {
         ...
         bytes32 hashgame = keccak256(abi.encodePacked(internalTx_, receiver_, amount_, userSignature_));
         resultHashgame = uint(hashgame);
         if(resultHashgame % 2 == 0)
         ...
         // emit event 
         emit SwapEvent(internalTx_,msg.sender, 0, amount_, userSignature_, resultHashgame);
   }
   ...
   function _validateSwapData(address receiver_, uint256 amount_) private {
      // Check delay 
      require(block.timestamp >= (lastGSwap[receiver_] + swapDelay), "playgame: Not to swap time yet");
      //Check max limit 
      if (maxSwapAmount != 0){
         require(amount_ <= maxSwapAmount, "playgame: Swap amount exceed max limit");
      }
