...
contract AuraStakingProgram  {
    ...
    function start_staking(uint256 walletUsedBusd,uint256 _programId,address referrer) public 
    {
        ...
        if(msg.sender!=owner)
        {
            ...
            {
                uint256 refBonus=(walletUsed.mul(REFERRAL_PERCENTS[i])).div(100000);
    ...
    function getStakingProgram(address _user) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory,uint256[] memory, bool[] memory, bool[] memory) 
    {
