...
contract Staking is ReentrancyGuard {
    ...
    constructor(IERC20 _tokenStake, IERC20 _tokenReward) {
        owner = msg.sender;
    ...
    function claimToken() external payable onlyStaker nonReentrant {
        uint256 reward;
        for (
            uint256 _timeStake = 0;
            _timeStake < stakeInfos[msg.sender].length;
            _timeStake++
        ) {
            if (
                stakeInfos[msg.sender][_timeStake].endTS < block.timestamp &&
                ...
                SafeERC20.safeTransfer(
                    tokenStake,
                    msg.sender,
                    stakeInfos[msg.sender][_timeStake].amount
                );
        ...
        require(
            ...
            "Not enough reward token in pool to claim"
        );
    ...
    function caculateRewardStake(uint256 _amount)
        internal
        view
        returns (uint256)
    {
        ...
        for (uint256 _month = 0; _month < 12; _month++) {
            _reward +=
                (_amount * (initialRewardRate - _month) * 30) /
                (100 * 360);
        }
        return _reward;
    }
