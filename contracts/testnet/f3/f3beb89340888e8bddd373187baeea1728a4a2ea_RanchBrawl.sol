...
contract RanchBrawl is Ownable, IERC721Receiver, Pausable, ReentrancyGuard {
    ...
    // notice 不接收指定id移除，只能指定位置移除，倒序排列
    ...
    function dissolveTeam(uint256 landid) public {
        ...
        if (team.dogeids.length > 0) {
            for (uint256 n = team.dogeids.length; n > 0; n--) {
                doge.safeTransferFrom(
                    address(this),
                    _msgSender(),
                    team.dogeids[n-1],
        ...
        if (team.sheepids.length > 0) {
            for (uint256 m = team.sheepids.length; m > 0; m--) {
                doge.safeTransferFrom(
                    address(this),
                    _msgSender(),
                    team.sheepids[m-1],
        ...
        if (team.farmerids.length > 0) {
            for (uint256 p = team.farmerids.length; p > 0; p--) {
                farmer.safeTransferFrom(
                    address(this),
                    _msgSender(),
                    team.farmerids[p-1],
