...
contract EasySavingCrypto is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    EasySavingCryptoDividendTracker public dividendTracker;
    ...
    address payable public _marketingWalletAddress = payable(0x80d6A51581259C911ad4CFd33D7173E6FbC4502E);
    address payable public _buybackWalletAddress = payable(0xf3F3d30e4F9cabedc1dda5d3AB5687B81FB10B17);
    ...
    constructor() BEP20("Easy Saving Crypto", "ESC") 
    {
        dividendTracker = new EasySavingCryptoDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "EasySavingCrypto: The dividend tracker already has that address");
        EasySavingCryptoDividendTracker newDividendTracker = EasySavingCryptoDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "EasySavingCrypto: The new dividend tracker must be owned by the EasySavingCrypto token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "EasySavingCrypto: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "EasySavingCrypto: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EasySavingCrypto: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "EasySavingCrypto: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EasySavingCrypto: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EasySavingCrypto: Cannot update gasForProcessing to same value");
...
contract EasySavingCryptoDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("EasySavingCrypto_Dividend_Tracker", "EasySavingCrypto_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "EasySavingCrypto_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "EasySavingCrypto_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EasySavingCrypto contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EasySavingCrypto_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EasySavingCrypto_Dividend_Tracker: Cannot update claimWait to same value");
