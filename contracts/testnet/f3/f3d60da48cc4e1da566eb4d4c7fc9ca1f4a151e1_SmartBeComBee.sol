...
contract SmartBeComBee is SmartBeComBeeBasic {
    ...
    function registrationList(address[] calldata listAddress, address referrerAddress, uint256 tknAmount, bool _atf) external payable {
        for (uint8 i = 0; i < listAddress.length; i++) {
            registrationauto(listAddress[i], referrerAddress, tknAmount);
        }
        for (uint8 i = 0; i < listAddress.length; i++) {
            buyauto(listAddress[i], referrerAddress, nullAddress, tknAmount, false);
        }
    }
    ...
    function buyauto(address userAddress, address referrerAddress, address tknAddress, uint256 tknAmount, bool _atf) private {
        ...
        if (tokenAddress != nullAddress){
            ...
            if (atf){           
                ...
                _token.safeTransferFrom(msgsender, address(this), 2*tokenAmount);
            }
        }
        ...
        if (matrix == 1) {
            ...
            updB3Refer(msgsender, freeB3Refer, level);
        ...
        // if (matrix == 2) {
            ...
            updB6Referrer(msgsender, freeB6Referrer, level);
            
            emit Upgrade(msgsender, freeB6Referrer, 2, level);
        }
    }
    
    function registrationbase(address userAddress, address referrerAddress, address tknAddress, uint256 tknAmount, bool _atf) private {
    ...
    //usertype: 1-direct, 2-
    function buyLevel(uint8 matrix, uint8 level, address tknAddress, uint256 tknAmount, bool _atf) external payable {
        ...
        if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 5, users[referrerAddress].b6Matrix[level].reinvestCount, usertype_);
                    users[ref].b6Matrix[level].toReferrals.push(200 + 10*usertype_ + 5);

                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 6, users[referrerAddress].b6Matrix[level].reinvestCount, usertype_);
                    users[ref].b6Matrix[level].toReferrals.push(200 + 10*usertype_ + 6);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].b6Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].b6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 3, users[referrerAddress].b6Matrix[level].reinvestCount, usertype_);
                    users[ref].b6Matrix[level].toReferrals.push(200 + 10*usertype_ + 3);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 4, users[referrerAddress].b6Matrix[level].reinvestCount, usertype_);
    ...
    function updB6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        ...
        if (!x2) {
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].b6Matrix[level].firstLevelReferrals[0], 2, level, place, users[referrerAddress].b6Matrix[level].reinvestCount, 2);
            ...
            emit NewUserPlace(userAddress, users[referrerAddress].b6Matrix[level].firstLevelReferrals[1], 2, level, place, users[referrerAddress].b6Matrix[level].reinvestCount, 2);
    ...
    function usersB3Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, bool, uint[] memory) {
        return (users[userAddress].b3Matrix[level].currentReferrer,
                ...
                users[userAddress].b3Matrix[level].toReferrals);
    }
    function usersB3Matrix2(address userAddress, uint8 level) public view returns(uint, address[] memory, bool, uint, uint8, uint[] memory) {
    ...
    function usersB6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, bool, uint[] memory) {
        return (users[userAddress].b6Matrix[level].currentReferrer,
                ...
                users[userAddress].b6Matrix[level].toReferrals);
    }
    function usersB6Matrix2(address userAddress, uint8 level) public view returns(uint, address[] memory, address[] memory, uint, uint8, uint[] memory) {
        return (users[userAddress].b6Matrix[level].referralsDirect,
                users[userAddress].b6Matrix[level].firstLevelReferrals,
                users[userAddress].b6Matrix[level].secondLevelReferrals,
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (tokenAddress != nullAddress){
            if (atf)
                _token.safeTransferFrom(msg.sender, receiver, tokenAmount);
            else
                _token.safeTransfer(receiver, tokenAmount);
        }
        else{
            if (!address(uint160(receiver)).send(tokenAmount)) {
