...
contract PancakeFlashSwap {
    ...
    function flashloan(address loanAsset, uint256 loanAmount) external {
        // Get the Factory Pair address for combined tokens
        address pair = IUniswapV2Factory(PANCAKE_FACTORY).getPair(loanAsset, BUSD);
        ...
        bytes memory data = abi.encode(loanAsset, loanAmount);
    ...
    function pancakeCall(address _sender, uint256 _amount0, uint256 _amount1, bytes calldata data) external {
        ...
        (address loanAsset, uint256 loanAmount) = abi.decode(data, (address, uint256));
