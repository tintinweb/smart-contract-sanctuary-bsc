...
library LibApxReward {
    ...
    bytes32 constant APX_REWARD_POSITION = keccak256("apollox.stake.reward.apx.storage");
    ...
    function apxRewardStorage() internal pure returns (ApxRewardStorage storage st) {
    ...
    //        ApxRewardStorage storage st = apxRewardStorage();
    //        return st.poolInfo.totalStaked;
    //    }
    ...
    function claimApxReward(address account) internal {
        ...
        if (pending > 0) {
            ...
            user.rewardDebt = user.amount.mul(pool.accAPXPerShare).div(1e12);
            require(st.rewardToken.balanceOf(address(this)) >= pending, "LibApxReward: Balance insufficient");
