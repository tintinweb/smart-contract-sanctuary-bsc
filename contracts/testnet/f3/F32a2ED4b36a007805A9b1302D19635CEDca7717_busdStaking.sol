...
contract busdStaking is Ownable {
    ...
    constructor() {
        token = IERC20(0x7D76154aDF864e5018313D838Cf2b60f21B905cA);
        Owner = msg.sender;
   ...
   function runLottery() external onlyOwner returns (uint256 LotteryWiner,address WinerTeamOwner) {
    ...
    function setToken(address _enterTokenAddress) external {
         token = IERC20(_enterTokenAddress);
    }
     function setMinDeposit(uint256 _amount) external {
         minimumDeposit =_amount ;
    }

 function setMaxDeposit(uint256 _amount) external {
        maxDeposit=_amount;
    }

 function setAccomulationCutoff(uint256 enterTime) external {
         accomulationCutoff = enterTime;
    }
