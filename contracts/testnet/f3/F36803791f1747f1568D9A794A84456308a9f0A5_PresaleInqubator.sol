    ...
    IPresalePublic
{
    ...
    function initialize(
        ...
        payMagnitude = 10**IERC20Metadata(BUSD).decimals();
     ...
     */
    function approve(bool yes) external {
        require(
        ...
        if (yes && currentTime > generalInfo.openTime - 2 days) {
            ...
            generalInfo.openTime = currentTime + 2 days;
    ...
    function register() external approved notCreator {
        ...
        investments[sender].roundInvestments = new uint256[](3);
        ...
        notCreator
    {
        if (!requested && LINK.balanceOf(address(this)) >= fee) {
            requested = true;
            bool tier1Enough = ((levelsCounter[1] * WINNING_CHANCE[0]) /
                DENOMINATOR) > 0;
            bool tier2Enough = ((levelsCounter[2] * WINNING_CHANCE[1]) /
                DENOMINATOR) > 0;
            bool tier3Enough = ((levelsCounter[3] * WINNING_CHANCE[2]) /
                DENOMINATOR) > 0;
            if (tier1Enough || tier2Enough || tier3Enough)
        ...
        if (currentTime <= generalInfo.openTime + 2 hours) {
            //MOON ROUND
            round = Rounds.MOON;
            ...
            if (roundTokenAllocation[round][10] == 0)
                _getTokenAllocation(round);
        } else if (currentTime >= generalInfo.openTime + 8 hours) {
            //PAPER ROUND
            round = Rounds.PAPER;
        ...
        uint256 userPart = (intermediate.eachPoolShare *
            POOL_WEIGHT[sender.level - 1]) /
            DENOMINATOR -
            investor.roundInvestments[uint256(round)];
        if (tokenAmount > userPart) {
        ...
        roundTokenAllocation[round][sender.level] -= tokenAmount;
        investor.roundInvestments[uint256(round)] += tokenAmount;
        intermediate.tokensForSaleLeft -= tokenAmount;
     ...
     * @notice Creator only
     */
    function withdrawTokens() external nonReentrant onlyPresaleCreator {
        require(
            (block.timestamp > generalInfo.closeTime &&
                intermediate.lpAddress == address(0) &&
                intermediate.raisedAmount < generalInfo.softCap) ||
    ...
    function getRound() public view returns (uint8) {
        ...
        if (
            ...
            //MOON ROUND
            return uint8(Rounds.MOON);
        } else if (
            currentTime >= generalInfo.openTime + 4 hours &&
            currentTime <= generalInfo.openTime + 6 hours
        ) {
            //DIAMOND ROUND
            return uint8(Rounds.DIAMOND);
        } else if (
            currentTime >= generalInfo.openTime + 8 hours &&
            currentTime <= generalInfo.closeTime
        ) {
            //PAPER ROUND
            return uint8(Rounds.PAPER);
        } else return 3;
    }

    /** @dev View function that returns max user investment
     */
    function getMaxInvestment(address user) public view returns (uint256) {
        uint8 round = getRound();
        uint256 i;
        uint256 n;
        uint256 totalShares;
        Investor memory sender = registerLevels[user];
        Investment memory investor = investments[user];
        if (round == 0) {
            if (sender.level < 4) return 0;
            if (roundTokenAllocation[Rounds.MOON][10] == 1) {
                return
            ...
            if (sender.level < 4 && !sender.lock) return 0;
            if (
                (sender.level > 0 && sender.level < 4) &&
                expandCalled &&
                !lotteryWhitelist[user]
            ) return 0;
            if (roundTokenAllocation[Rounds.DIAMOND][10] == 1) {
                return
                    (intermediate.eachPoolShare *
                        POOL_WEIGHT[sender.level - 1]) /
                    DENOMINATOR -
            ...
            if (roundTokenAllocation[Rounds.PAPER][10] == 1) {
                return
                    (intermediate.eachPoolShare *
                        POOL_WEIGHT[sender.level - 1]) /
                    DENOMINATOR -
    ...
    function _getPayAmount(uint256 _tokenAmount)
        ...
        returns (uint256)
    {
        return (_tokenAmount * generalInfo.tokenPrice) / payMagnitude;
    }
}
