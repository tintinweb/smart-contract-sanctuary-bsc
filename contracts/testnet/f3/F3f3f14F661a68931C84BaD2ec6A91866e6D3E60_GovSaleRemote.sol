...
interface IGovSaleRemoteFactory{
    ...
    function dstCrossChainManager() external view returns (address);
    ...
    function config(address, address, address, address, address, address, address) external;
...
import "../interfaces/IGovSaleRemoteFactory.sol";
import "../utils/AdminProxyManager.sol";
import "../utils/PublicSaleLibrary.sol";

contract GovSaleRemote is Initializable, UUPSUpgradeable, NonblockingLzApp, PausableUpgradeable, ReentrancyGuardUpgradeable, AdminProxyManager {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using PublicSaleLibrary for uint128;

    uint16 public immutable dstChainId = 10009; // mumbai
    ...
    IGovSaleRemoteFactory public factory;
    ...
    mapping(address => mapping(uint32 => bool)) public isOutOfSale;
    ...
    function init(
        ...
        factory = IGovSaleRemoteFactory(_msgSender());

        __UUPSUpgradeable_init();
        __Pausable_init();
        __ReentrancyGuard_init();
        __AdminProxyManager_init(tx.origin);
        __NonblockingLzApp_init(factory.endpoint());
    ...
    // function getAdapterParams() internal pure returns(bytes memory adapterParams){
    //     uint16 version = 1;
    ...
    function getPayload(
        ...
        uint256 invoiceIndex = getBuyerHistoryLength(_buyer);
        payload = abi.encode(targetSale, invoiceIndex, _amountIn, _buyer);
    }
    ...
    function estimateFees(
        ...
        uint256 fees;
        // (fees,) = lzEndpoint.estimateFees(dstChainId, address(this), getPayload(_amountIn, _buyer), false, getAdapterParams());
        (fees,) = lzEndpoint.estimateFees(dstChainId, address(this), getPayload(_amountIn, _buyer), false, bytes(""));
        messageFee = fees + factory.crossFee_d2().calcPercent2Decimal(uint128(fees));
    }
    ...
    function releaseToken(address _target, uint256 _amount) internal {
    ...
    function buyToken(uint128 _amountIn) external payable whenNotPaused nonReentrant {
        ...
        uint256 fees = estimateFees(_amountIn, sender);
        uint256 feeIn = msg.value;

        require(!isOutOfSale[sender][running] && running > 0 && feeIn >= fees, "bad");

        lzEndpoint.send{value: fees}( // {value: messageFee} will be paid out of this contract!
            dstChainId, // destination chainId
            abi.encodePacked(factory.dstCrossChainManager()), // destination address of Cross chain manager contract
            getPayload(_amountIn, sender), // abi.encode()'ed bytes
            payable(this), // (msg.sender will be this contract) refund address (LayerZero will refund any extra gas back to caller of send()
            address(0x0), // future param, unused for this example
            // getAdapterParams() // v1 adapterParams, specify custom destination gas qty
        ...
        invoices[sender].push(Invoice(
            setBuyer(sender),
            running,
            uint64(block.timestamp),
            tokenReceivedEstimate,
            _amountIn,
            feeCharged
        ));
        ...
        if(feeIn > fees) {
            (bool success,) = payable(sender).call{value: feeIn - fees}("");
    ...
    function _nonblockingLzReceive(
        ...
        (
            ...
            uint128 bought,
            bool chance
        ) = abi.decode(_payload, (address, uint32, uint256, uint128, uint128, bool));

        if(chance) isOutOfSale[buyer][boosterRunId] = chance;

        if(boosterRunId > 0){
            Invoice memory invoice = invoices[buyer][invoiceIndex];

            if(invoice.received > received) invoices[buyer][invoiceIndex].received = received;
            if(invoice.boosterId > boosterRunId) invoices[buyer][invoiceIndex].boosterId = boosterRunId;

            uint128 originAmountIn = invoice.bought;
            if(originAmountIn > bought){
                uint128 feeCharged = bought.calcPercent2Decimal(booster[boosterRunId].fee_d2);
                uint128 refundAmount = (originAmountIn + invoice.charged) - (bought + feeCharged);

                releaseToken(buyer, refundAmount);
                
                invoices[buyer][invoiceIndex].bought = bought;
                invoices[buyer][invoiceIndex].charged = feeCharged;
            }

            uint128 finalFee = invoices[buyer][invoiceIndex].charged;

            raised += bought;
            revenue += finalFee;
            purchasePerRound[buyer][boosterRunId] += received;
            booster[boosterRunId].tokenAchieved += received;

            emit TokenBought(boosterRunId, buyer, received, bought, finalFee);
        } else {
            removeInvoice(buyer, invoiceIndex);
        }
    }
    ...
    function removeInvoice(
        ...
        uint256 indexToMove = getBuyerHistoryLength(_buyer) - 1;
        invoices[_buyer][_indexToDelete] = invoices[_buyer][indexToMove];
        invoices[_buyer].pop();
    }
