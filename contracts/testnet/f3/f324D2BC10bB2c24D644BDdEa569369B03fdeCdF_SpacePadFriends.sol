...
contract SpacePadFriends is Pausable, Ownable, ReentrancyGuard {
    ...
    event UpdatedMaximumCapAmount(uint256 maximumCapAmount);
    ...
    function deposit(uint256 _amount, address _referrer)
        ...
        nonReentrant
    {
        require(_amount > 0, "Min deposit sould be higher that 0");
        IERC20 depositTokenAddress = IERC20(DEPOSIT_TOKEN_ADDRESS);
        balances[msg.sender] += _amount;

        require(
            maximumCap != 0 && maximumCap <= balances[msg.sender],
            "Maximum cap has been reached!"
        );
    ...
    function withdraw(address _recipient) external whenNotPaused onlyOwner {
        ...
        uint256 balance = IERC20(DEPOSIT_TOKEN_ADDRESS).balanceOf(
            address(this)
        );
    ...
    function depositOnReferrer(
        ...
        IERC20 depositTokenAddress = IERC20(DEPOSIT_TOKEN_ADDRESS);
        uint256 referralFeeAmount = (_amount * _referralFee) / 10000;
        uint8 level = 1;
     ...
     */
    function setDepositTokenAddress(address _depositTokenAddress)
        external
        onlyOwner
    {
    ...
    /**
     * @dev Set Maximum Cap Amount
     * @param _maximumCap amount of the maximum cap
     */
    function setMaximumCap(uint256 _maximumCap) external onlyOwner {
        maximumCap = _maximumCap;
        emit UpdatedMaximumCapAmount(_maximumCap);
    }
