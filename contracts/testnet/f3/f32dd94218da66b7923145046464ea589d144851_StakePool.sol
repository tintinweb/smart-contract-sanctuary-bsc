...
contract Wrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        uint256 targetAmount = amount.mul(95).div(100);
        inToken.safeTransfer(msg.sender, targetAmount);
        if (_balances[msg.sender] == 0) {
...
contract StakePool is Wrapper, Ownable, Adminable {
    ...
    uint256 ONE_DAY = 60;
    uint256 ONE_QUARTER = ONE_DAY * 120;
    ...
    constructor(
        ...
        ONE_DAY = one_day_;
        //一季度
        ONE_QUARTER = ONE_DAY * 120;
        periodFinish = starttime_ + ONE_QUARTER * 4 ;
        lastUpdateTime = starttime;
        //平均分配，四个时间
        secondOutAmountArr.push(firstTotal.div(ONE_QUARTER));
        secondOutAmountArr.push(secondTotal.div(ONE_QUARTER));
        secondOutAmountArr.push(thirdTotal.div(ONE_QUARTER));
        secondOutAmountArr.push(forthTotal.div(ONE_QUARTER));
        require(secondOutAmountArr.length == 4, "len4");
    }
    ...
    function stake(uint256 amount,address invitor)
    external
    ...
    checkStart
    {
        ...
        if(invitor != address(0)){
            require(balanceOf(invitor) > 0, 'invitor stake must > 0');
        }
         _bind(invitor, msg.sender);
    ...
    checkStart
    {
        ...
        uint256 bonusAmount =  amount.mul(3).div(100);
        uint256 foundAmount =  amount.mul(2).div(100);
        inToken.safeTransfer(FoundAddress, foundAmount);
        inToken.safeTransfer(BonusAddress, bonusAmount);
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            if (up != address(0)) {
                uint256 refAmount = reward.mul(8).div(10);
                refRewardMap[up] = refRewardMap[up].add(refAmount);
                emit AddRefRewardAmount(up, msg.sender, refAmount);
                address upup = refMap[up];
                if (upup != address(0)) {
                uint256 uprefamount = reward.mul(3).div(10);
                refRewardMap[upup] = refRewardMap[upup].add(uprefamount);
                emit AddRefRewardAmount(upup, msg.sender, refAmount);
            }
    ...
    function getPeriod(uint256 _time) public view returns (uint256){
        uint256 per = _time.sub(starttime).div(ONE_QUARTER);
        uint256 rem = _time.sub(starttime).mod(ONE_QUARTER);
        if (rem == 0) {
            if (per == 0) {
                return 1;
            }
    ...
    function getPeriodTimestamp(uint256 _period) public view returns (uint256){
        ...
        return starttime.add(ONE_QUARTER.mul(n));
    }
