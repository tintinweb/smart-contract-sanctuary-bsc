...
contract Referral is ReferralFetchers, Ownable {
    ...
    function AddAsAFriend(address customerAddress) external {
        // addressToCustomer[customerAddress] =
    ...
    function getReward(address friendAddress) public view returns (uint256) {
        ...
        IInsuranceInvestment.Investment[]
            memory insuranceInvestment = insuranceInvestmentContract
                .getAllInvestments(friendAddress);
        IRegularInvestment.Investment[]
            memory regularInvestment = regularInvestmentContract
                .getAllInvestments(friendAddress);
        uint256 flexvisBalance = flexvisContract.balanceOf(friendAddress);

        uint256 noOfInsuranceInvestment = getAllActiveInsuranceCount(
            insuranceInvestment
        );
        uint256 noOfRegularInvestment = getAllActiveRegularCount(
            regularInvestment
        );
        ...
        if (
            ...
            noOfInsuranceInvestment >= minInsuranceInvestmentCount ||
            noOfRegularInvestment >= minregularInvestmentCount
        ) {
    ...
    function getAllActiveInsuranceCount(
        IInsuranceInvestment.Investment[] memory investments
    ) public pure returns (uint256 count) {
        for (uint256 i = 0; i < investments.length; i++) {
            IInsuranceInvestment.Investment
                memory currentInvestment = investments[i];
    ...
    // This method returns the active investments in the list of all the investments
    function getAllActiveRegularCount(
        IRegularInvestment.Investment[] memory investments
    ) public pure returns (uint256 count) {
        for (uint256 i = 0; i < investments.length; i++) {
            IRegularInvestment.Investment
                memory currentInvestment = investments[i];
    ...
    function claimReward(address friendAddress) external {
        ...
        uint256 rewardPercentage = rewardPercentages[reward.rewardCount];
        ...
        // First time;
        if (rewardDays[rewardCount] == 0) {
            // First time claiming reward

            // Get the reward of this friend
            uint256 rewardAmount = getReward(friendAddress);
            ...
            uint256 rewardToClaim = (rewardPercentage * rewardAmount) / 100;
            ...
            newReward = Reward({
                ...
                percentageSumUp: rewardPercentage
            });
        }
        ...
        if (
            lastClaimingDay != 0 &&
            block.timestamp >= (lastClaimingDay + rewardDays[1]) &&
            block.timestamp < (lastClaimingDay + rewardDays[2])
        ) {
            // Get the reward to claim;
        ...
        // Last time
        if (block.timestamp >= (lastClaimingDay + rewardDays[2])) {
