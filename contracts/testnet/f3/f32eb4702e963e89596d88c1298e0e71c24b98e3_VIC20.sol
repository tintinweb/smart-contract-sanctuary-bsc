...
Max Wallet: 5000 + .01% * total buys in contract | caps at 50000
Taxes:
5% LP
5% Dev
*/
...
contract VIC20 is IBEP20, Auth {
    using SafeMath for uint256;

    string constant _name = "VIC20";
    string constant _symbol = "VIC20";
    ...
    uint256 public _walletMax = _totalSupply * 5 / 1000; //starts at .5%
    uint256 public _walletMaxCap = _totalSupply * 50 / 1000; //max wallet cap is 5%
    ...
    constructor () Auth(msg.sender) {      
        ...
        autoLiquidityReceiver = 0xB906b88b02903B8d5e282af6Dd555027Fd2d3F21; //LP receiver
        marketingWallet = 0xB906b88b02903B8d5e282af6Dd555027Fd2d3F21;  //Marketing wallet
	...
	function checkTxLimit(address sender, address recipient, uint256 amount) internal {
		if (sender != owner
            ...
            require(!whitelistRequired || whitelisted[recipient]);
            if (restrictWhales){
                uint256 newBalance = balanceOf(recipient) + amount;
                require(newBalance <= _walletMax);
            }
            _txNum = _txNum + 1;
            //increment max wallet (a vincent original feature thank you very much)
            if (_walletMax < _walletMaxCap){
                _walletMax = _totalSupply * (_txNum) / 10000; //.01% each buy
            }
        }
    }
	...
	function setWhitelistRequired(bool active) external authorized {
		whitelistRequired = active;
	}
    ...
    function solveRiddle(uint256 guess) public {
        if (guess == answer){
