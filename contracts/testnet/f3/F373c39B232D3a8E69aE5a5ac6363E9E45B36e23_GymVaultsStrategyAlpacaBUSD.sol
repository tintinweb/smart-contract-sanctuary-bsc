...
contract GymVaultsStrategyAlpacaBUSD is Ownable, ReentrancyGuard, Pausable {
    ...
    /// BUSD address
    address public constant busdAddress = address(0xC9D8207b45A0d41a4bdCA01D4Ed1640aE00f7D6f);
    ...
    constructor(
        ...
        uniRouterAddress = address(0xD00C7e4c6bDd8FE20159E31FaD5389482CDbE46B);
        notPublic = false;
        controllerFee = 0;
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        if (isAutoComp && !strategyStopped) {
            ...
            if (
                vault.token() == IVaultConfig(0x9f5a337774d6350682B06488E0AaAFB2a3d8eD8a).getWrappedNativeAddr()
                // address(this).balance > 0
            ) {
                IWETH(0xC9D8207b45A0d41a4bdCA01D4Ed1640aE00f7D6f).deposit{value: _wantAmt}();
            }
        }
