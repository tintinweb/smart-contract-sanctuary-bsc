...
contract Stacking is OwnedAndParsed {
    ...
    struct rarity{
        uint minTimeStep;
        uint condsLn;
        mapping(uint => uint[2]) condReward;
        IERC20 token;

    }
    mapping(uint => rarity) rarityRecords;
    ...
    function getRarity(uint id) public view returns (rarityInfo memory){
        rarity storage ptr = rarityRecords[id];
        rarityInfo memory res;
        res.minTimeStep = ptr.minTimeStep;
        res.condsLn = ptr.condsLn;
        res.token = ptr.token;
        return res;
    }
    function getRarityConds(uint id) public view returns (uint[] memory){
        rarity storage ptr = rarityRecords[id];
        uint[] memory conds = new uint[](ptr.condsLn*2);

        for(uint i=0; i<ptr.condsLn; i++){
            conds[2*i] = ptr.condReward[i][0];
            conds[2*i+1] = ptr.condReward[i][1];
        }
    ...
    function setRarity(uint id, uint mn, uint[] memory conds, IERC20 token) public onlyOwnerOrParser returns (bool){
        require(conds.length%2 == 0, "Conds length is not 2x");
        require(conds.length > 0, "At least 0 condition");

        rarity storage ptr = rarityRecords[id];
        ptr.minTimeStep = mn;
        ptr.condsLn = conds.length/2;
        ptr.token = token;

        for(uint i=0; i<conds.length; i++){
            ptr.condReward[i/2][i%2] = conds[i];
        }
    ...
    mapping(uint => mapping(uint => record)) stackRecords;

    function getStackRecord(uint chainId, uint internalId) public view returns (record memory){
        return stackRecords[chainId][internalId];
    }
    ...
    function putOnStack(uint chainId, uint internalId, uint rarityT, bytes memory sig) public currentOwner(chainId, internalId) returns (bool){
        record storage ptr = stackRecords[chainId][internalId];
        if(ptr.rarityType == 0){
            bytes32 hash = keccak256(abi.encodePacked(chainId, internalId, rarityT));
    	    ...
    	    require(rarityT != 0, "Rarity cant be 0");

    	    ptr.rarityType = rarityT;
        }

        ptr.startedTime = block.timestamp;
        ptr.paidUpTo = block.timestamp;

        return true;
    }

    function calculateTokens(record storage ptr, rarity storage ptrR) internal view returns (uint, uint){
        require(ptr.startedTime != 0, "Not started");
        require(ptr.rarityType != 0, "Not defined nft rarity");
        require(ptrR.minTimeStep != 0, "Not defined rarity");

        uint periodTimes = (ptr.paidUpTo-ptr.startedTime)/ptrR.minTimeStep;
        uint mxTimes = (block.timestamp-ptr.startedTime)/ptrR.minTimeStep;

        uint curR = 0;
        while(curR < ptrR.condsLn-1 && ptrR.condReward[curR][0] <= periodTimes){curR++;}

        uint s = 0;
        for(uint i = periodTimes; i < mxTimes; i++){
            s += ptrR.condReward[curR][1];
            if(curR < ptrR.condsLn-1 && ptrR.condReward[curR][0] <= i){
                curR++;
            }
        }
    ...
    function checkTokens(uint chainId, uint internalId) public view returns (uint, uint){
        record storage ptr = stackRecords[chainId][internalId];
        rarity storage ptrR = rarityRecords[ptr.rarityType];

        return calculateTokens(ptr, ptrR);

    }

    function releaseTokens(uint chainId, uint internalId) public returns (bool){
        record storage ptr = stackRecords[chainId][internalId];
        rarity storage ptrR = rarityRecords[ptr.rarityType];

        (uint leftTo, uint mxT) = calculateTokens(ptr, ptrR);

        if(leftTo == 0){
            return false;
        }
        ptr.paidUpTo = ptr.startedTime + ptrR.minTimeStep * mxT;

        Archives.Action[] memory toArch = new Archives.Action[](1);

        // release token from owner
    	toArch[0].a = leftTo;
    	toArch[0].token = ptrR.token;
    	toArch[0].z = owner;
    ...
    function releaseStack(uint chainId, uint internalId) public currentOwner(chainId, internalId) returns (bool){
        record storage ptr = stackRecords[chainId][internalId];
        rarity storage ptrR = rarityRecords[ptr.rarityType];

        (uint leftTo,) = calculateTokens(ptr, ptrR);
        ptr.startedTime = 0;

        Archives.Action[] memory toArch;

        if(leftTo != 0){
            toArch = new Archives.Action[](2);
            ...
            // get back to archive state
    	    toArch[1].tp = 2;
    	    toArch[1].a = chainId;
    	    toArch[1].b = internalId;

        }else{
            toArch = new Archives.Action[](1);
