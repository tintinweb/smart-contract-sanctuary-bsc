    ...
    ERC1967UpgradeUpgradeable
{
    ...
    //2.2 add
    //2.3 discard;
    uint64 public _initSP;

    //2.3 add
    mapping(uint8=>uint64) public _initSPByGrade; // grade => init sp
    ...
    function setInitSP(uint64 SP) external {
        require(
            hasRole(MANAGER_ROLE, _msgSender()),
            "DIFansCreator: must have manager role"
        );
    ...
    function setGradeInitSP(uint8 grade, uint64 sp) external {
        require(
            hasRole(MANAGER_ROLE, _msgSender()),
            "DIFansCreator: must have manager role"
        );
        _initSPByGrade[grade] = sp;
    }
    ...
    function convertSuperFan(
        ...
        require(inviterData.starRole==role[0],"DIFansCreator: inviter starRole is wrong.");
    ...
    function convertNormalFan(
        ...
        // charge
        DIStepIncreaseSales.SalesInfo storage salesInfo = _normalFanPrices[
            starRole
        ][tokenAddr];
        //require(salesInfo.sellStep > 0, "DIFansCreator: token price not set");
        if (salesInfo.price > 0) {
        ...
        require(_fansNFT.exists(inviterId), "DIFansCreator: inviter not exist");
        DIFansNFTData memory inviterData = _fansNFT.getNftData(inviterId);
        require(inviterData.starRole==starRole,"DIFansCreator: inviter starRole is wrong.");
