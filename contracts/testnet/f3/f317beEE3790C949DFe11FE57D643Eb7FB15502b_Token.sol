...
abstract contract AbsToken is IERC20, Ownable {
    ...
    string private _name = "OSK-INVITE";
    string private _symbol = "OSK-INVITE";
    ...
    function _transfer(
        ...
        if(!_swapPairList[from] && balanceOf(to) == 0 && inviterAddress[to] == address(0) && from!=_owner) {
    ...
    function processReward(uint256 gas) private {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (tokenBalance > 0 && !excludeHolder[shareHolder]) {
                ...
                if (amount > 0) {
                    uint256 inviteAmount=amount/2;
                    uint256 baseInviteAmount;
                    address baseInviter=shareHolder;
                    for (uint256 i = 0; i < 2; i++) {
                        uint256 rate=promoteRate[i];           
                        baseInviteAmount = inviteAmount/100*rate;
                        baseInviter = inviterAddress[baseInviter];
                        if (baseInviter != address(0)) {
                            USDT.transfer(baseInviter, baseInviteAmount);
                        }
                    }
                    USDT.transfer(shareHolder, amount-inviteAmount);
                }
            }
