...
contract DistributeNFT is Ownable, ReentrancyGuard {
    ...
    function distributeNFT(address[] memory _listAddr) external onlyOwner {
        for (uint256 i = 0; i < _listAddr.length; i++) {
            // mint NFT
            SMCNFT.safeMint(_listAddr[i]);
        }
    }
}
