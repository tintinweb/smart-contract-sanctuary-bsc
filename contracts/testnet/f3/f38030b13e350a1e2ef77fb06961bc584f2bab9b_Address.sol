...
contract PlugToken is owned {
    ...
    uint public totalIdorefCount;
    uint public totalAirdroprefCount;
    uint public totalrefDistribution;

    uint maxBurnLimit= 45*10**9*10**18;
...
//------------------------------------------END---------------------------------------------------------
    ...
    address public  _devMarketingWallet=0x2dDC148F6F65d90173E957cb1Fd8767DC148Ef50;
    address public  _lpWallet=0x64B3fe4E84EaE61de41D5EBF7bacf605F33d6CC5;
    address public  _teamWallet=0xc58F99Ed71aC578C78Fdc057A7a110256F216E6f;
    address public  _charityWallet=0xe5719f5792A0d493FDb3F770C8264897B607dFef;
    address public  _lockedWallet = 0x8ce7039A997dED36E94CDf3C362963aA0384C9d5;
    //--------------------------------------------------------------------
    ...
    mapping (address=>refIncome) public userRefEarning;
    ...
    function _transfer(address _from, address _to, uint _value) internal {
        ...
        if(isAirdropWallet[_from]){

            require(block.timestamp>airdropSaleTime,"please wait you can't sale now");
        }
        ...
        if(idoClosed==true){
            ...
            // when token burn hit maxBurnLimit it will stop
            uint256  burnFee = _value.mul(_burnFee).div(PERCENTS_DIVIDER);
            if (totalSystemTokenBurn+burnFee<maxBurnLimit){

                _burn(burnFee);
                totalSystemTokenBurn+=burnFee;
            }
    ...
    function buyToken(address ref) payable public returns(uint)
    {
        ...
        refDist(ref,tokenTotal,rate,false);
    ...
    mapping (address=>bool) public isAirdropWallet ;
    uint256 internal airdropClaimedIndex;
    uint256 public airdropFee = 0.05 * (1e18);
    uint public totalAirdropTokensSold;

    uint public airdropSaleTime;
    ...
    function setAirdropSaleTime(uint256 _timestamp) onlyOwner public returns(bool){

        airdropSaleTime=_timestamp;
        return true ;
    }
     ...
     */
    function claimPassiveAirdrop(address _ref) external payable returns(bool) {
        ...
        mintToken(msg.sender, airdropAmount);
        _totalBountyDistributed+=airdropAmount;
        totalAirdropTokensSold+=airdropAmount;

        passiveAirdropTokensSold += airdropAmount;
        airdropClaimed[airdropClaimedIndex][msg.sender] = true; 
        if(isAirdropWallet[msg.sender]==false){

            isAirdropWallet[msg.sender]=true;
        }

        refDist(_ref,airdropAmount,10,true);
        
        return true;
    }

    function refDist(address _ref,uint _totalToken,uint _rate,bool _isAirdrop) internal {

        if (_ref!=address(0)){

            
            uint refBonus = _totalToken*_rate/100; // 10% of user purchase 
            mintToken(_ref,refBonus);
            totalrefDistribution+=refBonus;
            _totalBountyDistributed+=refBonus;  //ref bonus allocated in Bounty
            if(_isAirdrop){

                    userRefEarning[_ref].airdropIncome+=refBonus;
                    totalAirdroprefCount++;

            }else{

                    userRefEarning[_ref].idoIncome+=refBonus;
                    totalIdorefCount++;
            }
            

           emit  refTokenBonusEvent(_ref,refBonus);
        }
     ...
     */
    function airdropACTIVE(address[] memory recipients,address[] memory referrals,uint256[] memory tokenAmount) external onlyOwner  returns(bool) {
        ...
        mintToken(recipients[i], tokenAmount[i]);
            ...
            totalAmnt+=tokenAmount[i];

            refDist(referrals[i],tokenAmount[i],10,true);

            if(isAirdropWallet[recipients[i]]==false){

            isAirdropWallet[recipients[i]]=true;
            }
        }

        _totalBountyDistributed+=totalAmnt;
        totalAirdropTokensSold+=totalAmnt;
