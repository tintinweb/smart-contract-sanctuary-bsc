...
abstract contract RoyaltyUpgradeable is
    ContextUpgradeable,
    OwnableUpgradeable
{
    using FixedPointMathLib for uint256; 

    mapping(address => mapping(uint256 => Royalty)) private _assetTokenRoyalty;
    mapping(address => Royalty) private _defaultRoyalty;
    ...
    /**
     * @notice Get the royalty information
     */
    function royaltyInfo(address _asset, uint256 _token, uint256 _salePrice) external view returns (address, uint256) {
        return _royaltyInfo(_asset, _token, _salePrice);
    }
    /**
     * @notice Sets the royalty information
     * @param _asset            Address of NFT
     * @param _tokenId          ID of token 
     * @param _receiver         Address of who receive royalty fee
    ...
    function setTokenRoyalty(
        ...
        require(_feeNumerator <= _denominator(), "Royalty fee will exceed salePrice");
        require(_receiver != address(0), "ERC2981: Invalid parameters");

        _assetTokenRoyalty[_asset][_tokenId] = Royalty(_receiver, _feeNumerator);
    }
    
    
    /**
     * @notice Sets the royalty information that all ids in the NFT contract will default to.
     * @param _asset        Address of NFT
     ...
     * - `_feeNumerator` cannot be greater than the fee denominator.
     */
    function setDefaultRoyalty(address _asset, address _receiver, uint96 _feeNumerator) external onlyOwner {
        require(_feeNumerator <= _denominator(), "Royalty fee will exceed salePrice");
        require(_receiver != address(0), "Invalid receiver");

        _defaultRoyalty[_asset] = Royalty(_receiver, _feeNumerator);
    }
    
    function _royaltyInfo(address _asset, uint256 _token, uint256 _salePrice) internal view returns (address, uint256) {
        Royalty memory royalty = _assetTokenRoyalty[_asset][_token];
        if(royalty.receiver == address(0)){
            royalty = _defaultRoyalty[_asset];
        }
        uint256 royaltyAmount = _salePrice.mulDivDown(royalty.royaltyFraction, _denominator());
     ...
     * @param _asset        Address of NFT
     */
    function deleteDefaultRoyalty(address _asset) external onlyOwner {
        delete _defaultRoyalty[_asset];
    }
     ...
     * @param _tokenId      ID of token
     */
    function resetTokenRoyalty(address _asset, uint256 _tokenId) external onlyOwner {
        delete _assetTokenRoyalty[_asset][_tokenId];
    }
...
import "./internal-upgradeable/RoyaltyUpgradeable.sol";
    ...
    IERC721ReceiverUpgradeable
{
    ...
    modifier onlyValidListing(uint256 _listingId) {
        ...
        require(selectedListing.listed == true, "invalid listing");
        _;
    }
    ...
    /**
     * @dev get all NFTs are on the marketplace
     */
    ...
    /**
     * @notice fetch marketItem by address _nftContract and token _tokenId
     * @param _nftContract      Address of _nftContract
     * @param _tokenId          Id of token
     */
    ...
    function _processRoyaltyPayment(
        ...
        uint256 marketplaceCut = _totalValue.mulDivDown(marketplaceFee, PERCENTAGE_FRACTION);
        ...
        if (_checkRoyalties(_selectedListing.nftContract)) {
            ...
            (address royaltyFeeReceipient, uint256 royaltyFeeAmount) = _royaltyInfo(_selectedListing.nftContract, _selectedListing.tokenId, _totalValue);
            if (royaltyFeeAmount > 0) {
                secondaryRoyalties = royaltyFeeAmount;
                _safeTransferFrom(
                    _currency, 
