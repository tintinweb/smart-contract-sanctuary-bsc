...
contract PayDohDividendDistributor is IDividendDistributor {
    address _token;
    ...
    function deposit() public payable {
        ...
        dividendsPerShare =
            dividendsPerShare +
            (dividendsPerShareAccuracyFactor * newBalance) /
            totalShares;
    }
...
contract PayDoh is ERC20, Ownable {
    IUniswapV2Router public uniswapV2Router;
    address public immutable uniswapV2Pair;

    PayDohDividendDistributor public dividendDistributor;

    address public deadAddress =
        address(0x0000000000000000000000000000000000000000);
    address public marketingFeeReceiver;

    uint256 private constant tSupply = 1e12; // 1 T tokens
    uint256 private constant DECIMALS = 1e18;

    uint256 public maxTx = 5 * tSupply / 1000 * (DECIMALS);               // 0.5%  of total supply, 5   B tokens
    uint256 public swapTokensAtAmount = 5 * tSupply / 1e4 * (DECIMALS);   // 0.05% of total supply, 500 M tokens
    uint256 public maxWallet = 20 * tSupply / 1000 * (DECIMALS);          // 2.0%  of total supply, 20  B tokens  
    ...
    uint256 private nBotBlocks;
    uint256 private aBotDuration;
    ...
    event SendDividends(uint256 FTMRewards);
    ...
    constructor() ERC20("PayDoh", "PayDoh") {
        ...
        dividendDistributor = new PayDohDividendDistributor(
            address(_uniswapV2Router),
            owner()
        );
        marketingFeeReceiver = address(0x4c6af714d50804Cc0a18d07Feb806A482cFE3618);
    ...
    function initiateAntiBot(uint256 _aBotDuration) public onlyOwner {
        require(!tradingIsEnabled, "Project already launched.");
        aBotDuration = _aBotDuration;
        antiBotActive = true;
        tradingIsEnabled = true;
    }

    function launch(uint256 _nBotBlocks, uint256 _cooldown)
        public
        onlyOwner
    {
        require(!hasLaunched, "Project already launched.");
        nBotBlocks = _nBotBlocks;
        launchBlock = block.number;
        cooldown = _cooldown;
        hasLaunched = true;

        emit Launch(_cooldown);
    }

    function updateDividendDistributor(address newAddress) public onlyOwner {
        require(
            newAddress != address(dividendDistributor),
            "PayDoh: The dividend distributor already has that address"
        );

        PayDohDividendDistributor newDividendDistributor = PayDohDividendDistributor(
                payable(newAddress)
            );

        require(
            newDividendDistributor._owner() == address(this),
            "PayDoh: The new dividend distributor must be owned by the Test token contract"
        );
        ...
        emit UpdateDividendDistributor(
            newAddress,
    ...
    function excludeFromDividends(address account, bool shouldExclude)
        public
        onlyOwner
    {
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "Test: gasForProcessing must be between 200,000 and 800,000"
        );
        require(
    ...
    function setFees(
        ...
        uint256 _rewardsFee,
        uint256 _sellFeeIncrease
    ) public onlyOwner {
        require(
            0 <= _rewardsFee && _rewardsFee <= 5,
            "Requested rewardsFee fee not within acceptable range."
        );
        require(
            0 <= _liquidityFee && _liquidityFee <= 5,
            "Requested liquidity fee not within acceptable range."
        );
        require(
            0 <= _marketingFee && _marketingFee <= 5,
            "Requested marketing fee not within acceptable range."
        );
        require(
            0 <= _sellFeeIncrease && _sellFeeIncrease <= 6,
            "Requested sell fee increase not within acceptable range."
        );
        require(
            0 < _marketingFee + _liquidityFee,
            "Total fee amount must be strictly positive."
        );
    ...
    function setTradeRestrictions(uint256 _maxTx, uint256 _maxWallet)
        public
        onlyOwner
    {
        require(
            _maxTx >= ((5 * tSupply) / 1000),
            "Requested max transaction amount too low."
        );
        require(
            _maxWallet >= ((20 * tSupply) / 1000),
            "Requested max allowable wallet amount too low."
        );
    ...
    {
        require(
            (5 * tSupply) / 1000 <= _swapTokensAtAmount &&
                _swapTokensAtAmount <= (2 * tSupply) / 100,
    ...
    function checkValidTrade(
        ...
        if (from != owner() && to != owner() && !isExcludedFromFees[from]) {
            ...
            if (isPair[from]) {
                require(
                    balanceOf(address(to)) + amount <= maxWallet,
                    "Token purchase implies maxWallet violation."
                );
            }
        }
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (antiBotActive) {
                if (isPair[from]) {
                    require(
                        !launchSniper[to],
                        "Do not buy before official launch."
                    );
                    if (!hasLaunched) {
                    ...
                    if (timeLastTrade[to] != 0) {
                        require(
                            block.number > timeLastTrade[to] + cooldown,
                            "Trade too frequent."
                        );
                    }
                ...
                if (block.number < launchBlock + nBotBlocks) {
                    fees = (amount * 99) / 100;
                } else {
                    antiBotActive = block.number > launchBlock + aBotDuration
                        ? false
                        : antiBotActive;
            ...
            super._transfer(
                from,
                (_burnFees ? deadAddress : address(this)),
                fees
            );
        }
        ...
        if (!isExcludedFromDividends[from])
            try
                dividendDistributor.setShare(payable(from), balanceOf(from))
            {} catch {}
        if (!isExcludedFromDividends[to])
            try
                dividendDistributor.setShare(payable(to), balanceOf(to))
            {} catch {}
    ...
    function shouldBurn() private view returns (bool) {
        ...
        return tradingIsEnabled && canBurn && !inBurn && !antiBotActive;
    }

    function planBurn(uint256 _burnNumerator, uint256 _burnDenominator)
        public
        onlyAuthorized
    {
        burnAmount =
            (tSupply * DECIMALS * _burnNumerator) /
            _burnDenominator;
    ...
    function swapTokens(uint256 tokens) private inSwap {
        ...
        (bool temp, ) = payable(marketingFeeReceiver).call{
            value: BNBForMarketing,
            gas: 30000
        }("");
        temp; //warning-suppresion
