...
import "./interface/ITokieNFT.sol";
import "./interface/ITokieNFTFactory.sol";
import "./interface/IGegoRuleProxy.sol";
import "./library/Governance.sol";

contract TokieNFTFactory is Governance, ITokieNFTFactory {
    ...
    mapping(uint256 => ITokieNFT.Gego) public _gegoes;
    ...
    ITokieNFT public _tokieNftToken ;

    bool public _isUserStart = false;

    uint256 private _randomNonce;

    constructor(address tokieNftToken) {
        _tokieNftToken = ITokieNFT(tokieNftToken);
    }
    ...
    {
        ITokieNFT.Gego storage gego = _gegoes[tokenId];
    ...
    function getGegoStruct(uint256 tokenId)
        ...
        returns (ITokieNFT.Gego memory gego){
    ...
    function mint(MintData memory mintData) public lock
    {
        ...
        _randomNonce++;
        ITokieNFT.Gego memory gego = _ruleProxys[mintData.nftType].generate(msg.sender, mintData.ruleId, _randomNonce);
        ...
        _tokieNftToken.mint(msg.sender, gegoId);
    ...
    function gmMint(MintData memory mintData, MintExtraData memory extraData) public {
        ...
        _tokieNftToken.mint(extraData.author, gegoId);
    ...
    function burn(uint256 tokenId) external override returns ( bool ) {
        ITokieNFT.Gego memory gego = _gegoes[tokenId];
        require(gego.id > 0, "not exist");

        _tokieNftToken.safeTransferFrom(msg.sender, address(this), tokenId);
        _tokieNftToken.burn(tokenId);
...
interface ITokieNFT is IERC721 {
...
import "./ITokieNFT.sol";

interface ITokieNFTFactory {
...
interface IGegoRuleProxy  {
    ...
    function destroy( address owner, ITokieNFT.Gego calldata gego ) external;

    function generate( address user,uint256 ruleId, uint256 randomNonce ) external view returns ( ITokieNFT.Gego memory gego );
