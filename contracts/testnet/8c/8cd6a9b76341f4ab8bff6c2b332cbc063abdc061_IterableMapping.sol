...
contract ColdWarTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("ColdWarToken_Dividend_Tracker", "ColdWarToken_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 15000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ColdWarToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ColdWarToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ColdWarToken contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ColdWarToken_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "ColdWarToken_Dividend_Tracker: Cannot update claimWait to same value");
...
contract ColdWarToken is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    ColdWarTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("ColdWarToken", "CWT") {
        
        BNBRewardsFee = 3;
        liquidityFee = 4;
        extraFeeOnSell = 3; // extra sell fee
        marketingFee = 4;
        marketingWallet = payable(0xAC834865dd8D0949a3DF2dE5c0367588b1428f99);

    
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new ColdWarTokenDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ColdWarToken: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ColdWarToken: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ColdWarToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ColdWarToken : Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ColdWarToken: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ColdWarToken: Cannot update gasForProcessing to same value");
