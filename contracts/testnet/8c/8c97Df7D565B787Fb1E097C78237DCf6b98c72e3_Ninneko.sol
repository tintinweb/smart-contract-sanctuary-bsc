...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event Fuse(uint256 indexed pet1Id, uint256 indexed pet2Id, uint256 indexed newPetId, uint256 ninoFee, uint256 mataFee);
    event PetUpgrade(uint256 indexed petId, uint256 indexed burnPetId, uint256 mataFee);
    ...
    function fuse(uint256 _pet1Id, uint256 _pet2Id) external whenNotPaused nonReentrant onlyPetOwner(_pet1Id) onlyPetOwner(_pet2Id) notInBlackList(_pet1Id) notInBlackList(_pet2Id) {
        ...
        _safeMint(msg.sender, petId);
        emit Fuse(_pet1Id, _pet2Id, petId, ninoFuseFee, mataFuseFee);
    }

    function upgradePet(uint256 _petId, uint256 _burnPetId) external whenNotPaused nonReentrant onlyPetOwner(_petId) onlyPetOwner(_burnPetId) notInBlackList(_petId) notInBlackList(_burnPetId) {
        burn(_burnPetId);
        MATAToken.safeTransferFrom(msg.sender, address(0xdEaD), mataUpgradeFee);
        emit PetUpgrade(_petId, _burnPetId, mataUpgradeFee);
    }
}
