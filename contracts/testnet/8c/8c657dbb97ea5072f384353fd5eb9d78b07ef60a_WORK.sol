...
interface IERC721 is IERC165 {
    ...
    function nftTypeId(uint256 tokenId) external view returns(uint256);
...
contract WORK is ERC721Holder,Ownable{
    ...
    IERC721 public nftContract = IERC721(0xA60bc174A4F8f9D29746aDCDF6601C64516b0F98);
    ...
    function pledge(uint256 _nftId) public {
		...
		uint256 currentTypeId = nftContract.nftTypeId(_nftId);
        require(pledgeMap[currentTypeId].isEnable,"Not Enable");
        ...
        emit Pledge(msg.sender, currentTypeId, _nftId);
    ...
    function getSalary(address _user, uint256 _nftId) public view returns(uint256){
		...
		uint256 endTime = userMap[_user][_nftId].startTime.add(pledgeMap[currentTypeId].cycle);
		uint256 currentTime = block.timestamp <= endTime ? block.timestamp : endTime;
		
		uint256 allSalary = (currentTime.sub(userMap[_user][_nftId].lastTime)).mul(pledgeMap[currentTypeId].output);
    ...
    function withdrawal(uint256 _nftId) public {
		...
		if(block.timestamp >= userMap[msg.sender][_nftId].startTime.add(pledgeMap[currentTypeId].cycle)){
	...
	function batchUserN(address _user)public view returns(userNs[] memory){
		...
		for(uint256 i = 0; i < length; i++){
			if( userPledgeNfts[_user][i] != 0){
				...
				userN[i].level = nftContract.nftTypeId(userN[i].nftId);   
