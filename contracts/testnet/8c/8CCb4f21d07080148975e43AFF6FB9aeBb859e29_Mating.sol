...
contract Mating is OwnableUpgradeable {
    ...
    function mating(uint myTokenId, uint targetTokenID) external {
        ...
        address invitor = refer.checkUserInvitor(msg.sender);
        ...
        if (invitor != address(0)) {
            boxClaimed[msg.sender]++;
            if (boxClaimed[msg.sender] == 5) {
                boxClaimed[msg.sender] = 0;
                uint[2] memory list;
                box.mint(refer.checkUserInvitor(msg.sender), list);
                emit RewardBox(msg.sender, invitor);
            }
            totalMatting[msg.sender]++;
            if (totalMatting[msg.sender] == 5) {
                item.mint(refer.checkUserInvitor(msg.sender), 15, 1);
                emit RewardCard(msg.sender, invitor);
            }
        }
    ...
    function coutingSelfCost(address addr, uint tokenId1, uint tokenId2) public view returns (uint, uint){
        (uint bvgCost1,uint bvtCost1) = coutingCost(msg.sender, tokenId1);
        (uint bvgCost2,uint bvtCost2) = coutingCost(msg.sender, tokenId2);
        uint bvgCost = (bvgCost1 + bvgCost2) / 2;
        uint bvtCost = (bvtCost1 + bvtCost2) / 2;
        uint rate = tec.checkUserTecEffet(addr, 3001);
        return (bvgCost * rate / 100, bvtCost * rate / 100);
    }
