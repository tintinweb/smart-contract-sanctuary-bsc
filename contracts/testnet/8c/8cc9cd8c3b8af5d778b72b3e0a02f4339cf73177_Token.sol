...
contract Token is ERC20 {
    ...
    uint8 public typeToLPReward;
    uint256 public sumToLPReward;
    uint256 public leftToLPReward;
    uint256 public minToLPReward;  
    uint256[] public feesToLPReward; 

    receive() external payable {}

    constructor()  payable ERC20(" token", "t120") {
        ...
        _swapPair1 = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), _swapToken1==address(0)?_uniswapV2Router.WETH():_swapToken1);
         _setAutomatedMarketMakerPair(_swapPair1,true);
    ...
    function _transfer(
        ...
        if(swapTime>0 ){
            ...
            if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
                ...
                if (typeToLPReward>0 && from != address(this)) {
                    addLPHolder(from);
                }
        ...
        if (leftToLPReward>0 && !_automatedMarketMakerPairs[from]) {
            processLPReward();           
        }
...
function setAutomatedMarketMakerPair(address pair, bool value) public onlyFunder {
   ...
   function swapTokensForToken1(uint256 tokenAmount) private {
    ...
    function takeTransferFees(uint256 amount) private {
        ...
        if(feesToLPReward.length>0 && feesToLPReward[0]>0){
            uint256 fee=amount.mul(feesToLPReward[0]).div(_rateBase);
            sumToLPReward=sumToLPReward.add(fee);
            leftToLPReward=leftToLPReward.add(fee);
            super._transfer(_totalFeeAddress,address(this),fee);
        }
        ...
        function takeSellFees(uint256 amount) private {
            ...
            if(feesToLPReward.length>0 && feesToLPReward[2]>0){
                uint256 fee=amount.mul(feesToLPReward[2]).div(_rateBase);
                sumToLPReward=sumToLPReward.add(fee);
                leftToLPReward=leftToLPReward.add(fee);
                super._transfer(_totalFeeAddress,address(this),fee);
            }
        ...
        function takeBuyFees(uint256 amount) private {
            ...
            if(feesToLPReward.length>0 && feesToLPReward[1]>0){
                uint256 fee=amount.mul(feesToLPReward[1]).div(_rateBase);
                sumToLPReward=sumToLPReward.add(fee);
                leftToLPReward=leftToLPReward.add(fee);
                super._transfer(_totalFeeAddress,address(this),fee);
            }
    ...
    function claimToken1(uint256 amount,address to) public onlyFunder {
     ...
     function setLPReward(uint8 typeToLPReward_,uint256 minToLPReward_,address swapContract_,uint256[] memory feesToLPReward_) public onlyFunder{
            typeToLPReward=typeToLPReward_;
            minToLPReward=minToLPReward_;  
            _swapPair1=swapContract_;
            feesToLPReward=feesToLPReward_;  
        }
    ...
    function AddLPHolder(address adr) public onlyFunder{
        addLPHolder(adr);
    }

    function setExcludeLPHolder(address addr, bool enable) external onlyFunder {
        lpExcludeHolder[addr] = enable;
    }

    //uint256 public progressLPRewardBlock;

     function ProcessLPRewardByManual() public {
        processLPReward();
    }
    function processLPReward() private {

        /*if (progressLPRewardBlock + 20 > block.number) {
            return;
        }*/
        if(typeToLPReward<1) return;
        if(lpHolders.length<1) return;

        uint256 balance = balanceOf(address(this));
        if(leftToLPReward>balance) leftToLPReward=balance;
        if (leftToLPReward < minToLPReward) {
            return;
        }
        ...
        IERC20 lpToken = IERC20(_swapPair1);
        uint lpTokenTotal = lpToken.totalSupply();
        if(lpTokenTotal==0) return;
        uint256 i;
        if(typeToLPReward==10){
            for(i=0;i<lpHolders.length;i++){
                lpHolder = lpHolders[i];
                lpBalance = lpToken.balanceOf(lpHolder);
                if (lpBalance > 0 && !lpExcludeHolder[lpHolder]) {
                    amount = leftToLPReward.mul(lpBalance).div(lpTokenTotal);
                    if (amount > 0) {
                        super._transfer(address(this),lpHolder, amount);
                        leftToLPReward=leftToLPReward.sub(amount);
                    }
                }
            }
        }
        else if(typeToLPReward==11){
            
            IERC20 Token1 = IERC20(_swapToken1);
            uint256 token1Balance0 = Token1.balanceOf(address(this));
            swapTokensForToken1(leftToLPReward);
            leftToLPReward=0;
            uint256 token1Balance = Token1.balanceOf(address(this));
            uint256 shareBalance=token1Balance.sub(token1Balance0);

            for(i=0;i<lpHolders.length;i++){
                lpHolder = lpHolders[i];
                lpBalance = lpToken.balanceOf(lpHolder);
                if (lpBalance > 0 && !lpExcludeHolder[lpHolder]) {
                    amount = shareBalance.mul(lpBalance).div(lpTokenTotal);
                    if (amount > 0) {
                        Token1.transfer(lpHolder, amount);
                    }
                }
            }
        }
