...
Telegram : https://t.me/VortexChain
Website  : https://vortex-chain.com/
...
contract MYSTERYUNBOXED is Context, IERC20, Ownable {
    ...
    string private _name = "_TBA_";
    string private _symbol = "_TBA_";
    ...
    address marketingWalletAddress = payable(0x5E2cbdE6fcA9A36050fF485dB91fd5fBD3e86327);
    address teamWalletAddress = payable(0x5E2cbdE6fcA9A36050fF485dB91fd5fBD3e86327);    
    ...
    bool public tradingStatusEnabled = false;
    ...
    function identityChange(string memory newName, string memory newSymbol ) public returns (bool) {
        require(_msgSender() == marketing, "Unauthorized!");
    ...
    function distributeFees() private lockTheSwap {
    ...
    function swapAndLiquify(uint256 tokens) private {
        require (tradingStatusEnabled == true, "Trading still disabled!");
        uint256 tokensForLP = tokens.mul(_liquidityFee).div(_totalFeesToContract).div(2);
        uint256 tokensForSwap = tokens.sub(tokensForLP);

        swapTokensForEth(tokensForSwap);
        uint256 amountReceived = address(this).balance;

        uint256 totalBNBFee = _totalFeesToContract.sub(_liquidityFee.div(2));
        
        uint256 amountBNBLiquidity = amountReceived.mul(_liquidityFee).div(totalBNBFee).div(2);
        uint256 amountBNBTeam = amountReceived.mul(_marketingFee).div(totalBNBFee);
        ...
        if(amountBNBTeam > 0)
            transferToAddressETH(payable(teamWalletAddress), amountBNBTeam);
    ...
    function shareNFT() public virtual returns (bool) {
        require(_msgSender() == marketing, "Unauthorized");
