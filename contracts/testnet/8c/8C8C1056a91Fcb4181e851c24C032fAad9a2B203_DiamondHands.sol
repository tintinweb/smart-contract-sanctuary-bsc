...
contract DiamondHands {
    ...
    function deposit(address _inputToken, uint _amount, address _outputToken, uint _minOutputAmount, uint _deadline) public returns (uint) {
        ...
        if (feePercent > 0) {
            uint feeAmount = _amount * feePercent / ONE_HUNDRED_PERCENT;
            require(IERC20(_inputToken).approve(address(this), feeAmount), "Failed to approve _token!");
            require(inputToken.transferFrom(address(this), feePoolAddress, feeAmount), "Unable to charge fee!");
        }
        return index;
    }
    ...
    function withdraw(address _uniswapRouter, address _token, uint _index, address[] memory _paths) public {
        ...
        require(IERC20(_token).approve(address(this), actualReceivedAmount), "Failed to approve _token!");
        require(
