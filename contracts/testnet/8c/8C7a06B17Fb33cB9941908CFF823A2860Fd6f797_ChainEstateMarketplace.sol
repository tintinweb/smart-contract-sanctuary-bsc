...
contract ChainEstateMarketplace is ReentrancyGuard, Ownable {
    ...
    if (idToMarketItem[itemId].seller != owner()) {
        ...
        for (uint256 i=0; i < reflectionTokenIds.length; i++) {
          if (CHESNFT.ownerOf(reflectionTokenIds[i]) == msg.sender) {
            payable(owner()).transfer(listingPrice / reflectionTokenIds.length);
          }
          else {
            payable(CHESNFT.ownerOf(reflectionTokenIds[i])).transfer(listingPrice / reflectionTokenIds.length);
          }
        }
    }
  }
  ...
  */
  function fetchUnsoldItemsCreated() public view returns (MarketItem[] memory) {
    uint totalItemCount = _itemIds.current();
    uint itemCount = 0;
    uint currentIndex = 0;
