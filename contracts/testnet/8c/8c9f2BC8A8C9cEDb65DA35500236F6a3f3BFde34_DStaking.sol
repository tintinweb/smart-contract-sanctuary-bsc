...
contract DStaking is Initializable, ContextUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private accReward; // Not distributed reward from early withraws
    ...
    uint256 public lastRewardTime; // Last time when rewards shared to accounts

    uint256 public PRECISION_FACTOR; // Percision factor

    uint256 public rewardPerWeek; // Reward amount bi-weekly
    ...
    address[] private stakers; // Stakers addresses - Used for mass Withdraw
    Statistics public statistics; // Staking app statistics
    PenaltyTiers[] public penaltyTiers; // Penalty tiers
    PenaltyTiers public outOfTiersPenalty; // Last penalty tier

    modifier isStaking() { // Can a use stake?
    ...
    // Get Penalty amount which user must pay for early withdraw
    ...
    // Admin mass Withdraw (all stakings after auto-migrations);
    ...
    // Take Penalty from early withdraw
    ...
    // Calculate pending accumulated penalties for user
    function pendignPenalty(address _user) external view returns (uint256) {
        UserInfo memory user = userInfo[_user];
        uint256 totalStaked = statistics.totalDeposit;
    ...
    // Admin recover wrong tokens which sent to this contract
    ...
    // Update reward per week before starting staking
