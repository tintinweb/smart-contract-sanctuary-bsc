...
contract Label2Earn is IBEP20, Auth {
    ...
    uint256 private marketingFeeSell = 4;
    uint256 private burnFeeSell = 1;
    uint256 private totalFeeSell = 7;
    uint256 private liquidityFeeBuy = 2;
    uint256 private marketingFeeBuy = 4;
    uint256 private burnFeeBuy = 1;
    uint256 private totalFeeBuy = 7;
    ...
    address private marketingFeeReceiver = 0xDcD8a7FaC7EA4FE3A2DF723D37B94d7D347489BE;
    ...
    function takeFee(address sender, address receiver, uint256 amount) internal returns (uint256) {
        if(receiver == pair) {
            ...
            // buy
            uint256 feeAmount = amount.mul(totalFeeBuy).div(100);
            if(totalFeeBuy > 0){
                uint256 burnAmount = 0; 
                if(burnFeeBuy > 0){
                    burnAmount = amount.mul(burnFeeBuy).div(100);
                    _balances[DEAD] = _balances[DEAD].add(burnAmount);
                    emit Transfer(sender, DEAD, burnAmount);
                }
                ...
                if(totalFeeBuy > burnFeeBuy){
                    newFeeAmount = feeAmount.sub(burnAmount);
                    liqamount = liqamount + (amount.mul(liquidityFeeBuy).div(100));
                    _balances[address(this)] = _balances[address(this)].add(newFeeAmount);
                    emit Transfer(sender, address(this), newFeeAmount);
                }
                return amount.sub(feeAmount);
    ...
    function swapBack() internal swapping {
        ...
        if(liqamount > 0){
            if(liqamount > contractTokenBalance){
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setFees(uint256 _liquidityFeeSell,  uint256 _marketingFeeSell, uint256 _burnFeeSell , uint256 _transferFee , uint256 _liquidityFeeBuy,  uint256 _marketingFeeBuy, uint256 _burnFeeBuy) external  onlyOwner {
        ...
        totalFeeBuy = _liquidityFeeBuy.add(_marketingFeeBuy).add(_burnFeeBuy);
        transferFee = _transferFee;
        emit feeChanged(_liquidityFeeSell , _marketingFeeSell , _burnFeeSell ,_liquidityFeeBuy , _marketingFeeBuy , _burnFeeBuy , _transferFee);
    }
    ...
    function getFees() external view returns (uint256 _liquidityFeeSell,  uint256 _marketingFeeSell, uint256 _burnFeeSell , uint256 _liquidityFeeBuy,  uint256 _marketingFeeBuy, uint256 _burnFeeBuy ,  uint256 _transferFee){
        return (liquidityFeeSell, marketingFeeSell ,burnFeeSell,  liquidityFeeBuy, marketingFeeBuy, burnFeeBuy , transferFee);        
    }
    ...
    event feeChanged(uint256 _liquidityFeeSell,  uint256 _marketingFeeSell, uint256 _burnFeeSell , uint256 _liquidityFeeBuy,  uint256 _marketingFeeBuy, uint256 _burnFeeBuy ,  uint256 _transferFee);
}