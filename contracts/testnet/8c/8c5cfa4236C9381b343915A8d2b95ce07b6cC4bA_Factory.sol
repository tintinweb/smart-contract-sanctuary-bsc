...
import "./interfaces/IBombPriceOracle.sol";
...
contract Factory is IFactory {
    ...
    IBombPriceOracle public bombPriceOracle;
    ...
    constructor(
        ...
        IBombPriceOracle _bombPriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        bombPriceOracle = _bombPriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "Bomb: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "Bomb: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "Bomb: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "Bomb: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) = bombPriceOracle.getPair(
            uniswapV2Pair
        );
        if (!oracleInitialized) bombPriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "Bomb: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "Bomb: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "Bomb: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "Bomb: UNAUTHORIZED");
