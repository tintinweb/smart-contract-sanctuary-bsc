    ...
    RewardRateConfigurable
{
    ...
    function updateRewards(
        ...
        for (uint256 i = 0; i < _distributionInfo.length; ++i) {
            uint256[25] storage _userRewards = userRewardsInfo[_pid][_distributionInfo[i].user]
                .userRewards;
            if(_getUserShares(_pid, 25, _distributionInfo[i].user) > 0) {
            ...
            userRewardsInfo[_pid][_distributionInfo[i].user].userRewardDebt = (_getUserShares(_pid, 25, _distributionInfo[i].user) * poolInfo[_pid].accRewardPerShare) / 1e18;
    ...
    function claimInternally(uint256 _pid,address _user) public {
        ...
        if(userTotalRewards > 0) {
            userActiveRewardsPercent = (userSharesByLevel * poolInfo[_pid].accRewardPerShare) / (userSharesByTotal * poolInfo[_pid].accRewardPerShare) * 100;
        }
    ...
    function _getPendingRewards(
        ...
        uint256 userActiveRewardsPercent = 0;
        if(userTotalRewards > 0) {
            userActiveRewardsPercent = (userSharesByLevel * _accRewardPerShare) / (userSharesByTotal * _accRewardPerShare) * 100;
        }
