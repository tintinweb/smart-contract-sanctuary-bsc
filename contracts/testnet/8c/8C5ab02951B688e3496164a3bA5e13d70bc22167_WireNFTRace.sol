...
abstract contract Ownable is Context {
 ...
 contract WireNFTRace is ERC721Enumerable, Ownable   {
  using Strings for uint256;
  IBEP20 public Token;
  ...
  uint256 public MinitngPricein_wire=1000000000000000000000;    // 1000 wire
  ...
  constructor(
      ...
      struct User 
    {
      ...
      function _chakUpline(address add, address _upline) public view returns(bool a){
  ...
  function mint_with_wire(uint256 _count ,uint256 amount,address  _upline) public 
     {
        ...
        if(total_users >= 2)
        {
           Token.transfer(_upline,5e10);
        }

        address  upline2 = users[_upline].upline;
        if(total_users >= 3)
        {
           Token.transfer(upline2,3e18);
        }

        address  upline3 = users[upline2].upline;
        if(total_users >= 4)
        {
           Token.transfer(upline3,2e18);
        }
