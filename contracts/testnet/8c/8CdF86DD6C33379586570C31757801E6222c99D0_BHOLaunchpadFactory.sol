...
contract BHOLaunchpadFactory is Initializable, SharedConstants {
    ...
    modifier onlyRole(string memory _stringRole) {
        ...
        require(
            addrDec.hasRoleV2(role, msg.sender),
            string(abi.encodePacked("account is missing role ", _stringRole))
        );
        _;
    }

    function initialize(IBHOStaking _staking, IAddressDecentralization _addrDec)
        public
        initializer
    {
        staking = _staking;
     ...
     * @dev collect BNB
     */
    function collectBNB() public onlyRole("manager") {
     ...
     */
    function collectToken(IERC20 _token) public onlyRole("manager") {
        _token.transfer(msg.sender, _token.balanceOf(address(this)));
    }

    /**
     * @dev update address staking
     */
    function updateStaking(IBHOStaking _staking) public onlyRole("manager") {
        staking = _staking;
    }

    /******************************************* Internal function below *******************************************/
    function _transferTokenSupply(
        IERC20Extented _payInToken,
        ...
        uint256 tokenAmount = _presaleRate.mul(_hardCap).div(
            10**_payInToken.decimals()
        );
        _token.transferFrom(msg.sender, _launchpadAddr, tokenAmount);
    }
}
...
contract BHOLaunchpad is Context, Whitelist, SharedConstants {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
     ...
     */
    function register() public checkWhoCanRegister {
