...
contract AthenaReferral is Ownable {
    using SafeBEP20 for IBEP20;

    mapping(address => bool) private operator;
    mapping(address => bool) private admin;
    // TraderContract address => user address => referrer address
    mapping(address => mapping(address => address)) private referrers;
    // TraderContract address => referrer address => referrals count
    mapping(address => mapping(address => uint256)) public referralsCount;
    // total referrals counts
    mapping(address => uint256) public totalReferralsCount;
    // TraderContract referrer address => total referral commissions
    ...
    event ReferralRecorded(
        address indexed traderContractAddr,
    ...
    /**
     * @dev Creates/deploys AthenaBank referral contarct
     */
    constructor() {
        ...
        onlyOperator
    {
        if (
            ...
            _referrer != address(0) &&
            _user != _referrer &&
            referrers[_msgSender()][_user] == address(0)
        ) {
            referrers[_msgSender()][_user] = _referrer;
            referralsCount[_msgSender()][_referrer] += 1;
            totalReferralsCount[_referrer] += 1;
            emit ReferralRecorded(_msgSender(), _user, _referrer);
        }
    }
        ...
        internal
    {
        if (_referrer != address(0) && _commission > 0) {
            referralCommissions[_msgSender()][_referrer] += _commission;
    ...
    // record referral commissions amount
    function recordReferralCommissions(
        address[] memory _referrers,
        uint256[] memory _commissions
    ) external onlyOperator {
        require(_referrers.length == _commissions.length, "Invalid input");

        for (uint256 i = 0; i < _referrers.length; i++) {
            recordReferralCommission(_referrers[i], _commissions[i]);
        }
    }

    // Get the referrer address that referred the user
    function getReferrer(address traderAddr, address _user)
        external
        view
        returns (address)
    {
        return referrers[traderAddr][_user];
    }

    // check address is operator
    function isOperator(address _user) external view returns (bool) {
        return operator[_user];
    }
    ...
    function addAdmin(address _admin) external onlyAdmin {
        require(_admin != address(0), "_admin should be non-zero address");
