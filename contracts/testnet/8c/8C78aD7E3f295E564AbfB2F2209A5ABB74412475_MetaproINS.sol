...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    mapping (uint256 => uint) public insCapacities;
    ...
    function create(uint256 tokenId, uint256 minCap, uint256 maxCap, uint256 pricePerToken, uint256 startBlock, uint256 endBlock, uint256 referralFee, bool multipleDeposits, bytes memory _data) external {
        ...
        require(pricePerToken > 0, "pricePerToken must ge greater than 0");
        require(referralFee <= 10000, "referralFee must be equal or less than 10000");
        require(token.balanceOf(msg.sender, tokenId) != 0, "Insufficient balance");
        ...
        INSConfiguration memory configuration = INSConfiguration(pricePerToken, startBlock, endBlock, minCap, maxCap, referralFee, multipleDeposits, msg.sender, true);
    ...
    function deposit(uint256 tokenId, uint256 amount, address _referrer) external nonReentrant {
        require(availableIns[tokenId].valid, "tokenId INS configuration not found");

        INSConfiguration memory configuration = availableIns[tokenId];
        uint256 currentCapacity = insCapacities[tokenId];

        if (configuration.maxCap != 0) {
            uint256 availableCapacity = configuration.maxCap - currentCapacity;
            if (currentCapacity >= configuration.maxCap && amount >= availableCapacity) {
                revert("current INS capacity filled");
            }
        }

        require(block.number >= configuration.startBlock, "INS not started");
        require(block.number <= configuration.endBlock, "INS ended");
        // require(amount >= configuration.pricePerToken, "amount can't be less than minimal configured by INS");
        ...
        if (configuration.multipleDeposits) {
            ...
            insCapacities[tokenId] = insCapacities[tokenId] + (amount.div(configuration.pricePerToken));
        } else {
    ...
    function withdraw(uint256 tokenId, bytes memory _data) external {
        ...
        INSConfiguration memory configuration = availableIns[tokenId];
        require(configuration.operator == msg.sender, "You must be operator of the token");

        require(block.number >= configuration.endBlock, "INS not ended");

        uint256 capacity = insCapacities[tokenId];
        if (configuration.minCap != 0) {
            if (capacity < configuration.minCap) {
                for (uint256 i = 0; i < deposits[tokenId].length; ++i) {
                    busd.transferFrom(address(this), deposits[tokenId][i].wallet, deposits[tokenId][i].amount);
        ...
        delete availableIns[tokenId];
        delete deposits[tokenId];
        delete insCapacities[tokenId];
    ...
    function processWithdraw(INSConfiguration memory configuration, uint256 tokenId, address operator, INSDeposit[] memory insWalletDeposits, bytes memory _data) private {
        for (uint256 i = 0; i < insWalletDeposits.length; ++i) {
            ...
            if (insWalletDeposits[i].referrer != address(0)){
                ...
                busd.transferFrom(address(this), insWalletDeposits[i].referrer, referrerFee);
            }

            uint256 tokenAmount = insWalletDeposits[i].amount.div(configuration.pricePerToken);
                
            busd.transferFrom(address(this), operator, depositAmount);
            busd.transferFrom(address(this), tressuryAddress, fee);
