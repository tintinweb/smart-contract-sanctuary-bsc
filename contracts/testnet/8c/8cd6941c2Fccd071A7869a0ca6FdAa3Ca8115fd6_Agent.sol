...
contract Agent is Ownable, IAgent {
    ...
    function initialize(address _token, address _usdt) external onlyOwner {
        require(_token != address(0) && token == address(0));
        require(_usdt != address(0) && usdt == address(0));
        token = _token;
        usdt = _usdt;
        pair = FACTORY.getPair(token, usdt);
    }
    ...
    function autoLiquidity() external override {
        if(_msgSender() != token) require(liquidityStock >= threshold, "Low liquidity stock");
        if(liquidityStock >= threshold) {
            //console.log("Liquidity stock: ", liquidityStock);
            ...
            if (_pairExisting(path)) {
                ...
                uint256 half = getOptimalAmountToSell(
                    int256(
                        token == IPair(pair).token0() ? reserve0 : reserve1
                    ),
                    int256(liquidityStock)
                );
                //console.log("Half to sell: ", half);
                uint256 anotherHalf = liquidityStock - half;
                //console.log("Another half: ", anotherHalf);
                uint256 usdtAmount = _swapTokensForTokens(half);
                //console.log("USDT amount: ", usdtAmount);
                //console.log("USDT balance: ", IERC20(usdt).balanceOf(address(this)));
                IERC20(usdt).approve(address(ROUTER), usdtAmount);
                anotherHalf = _addLiquidity(anotherHalf, usdtAmount);
                //console.log(IERC20(token).balanceOf(address(this)));
                //console.log("Another half again: ", anotherHalf);
                //console.log('here?');
                liquidityStock -= (anotherHalf + half);
                //console.log("Liquidity stock again: ", liquidityStock);
            }
        }
    }
    ...
    function _addLiquidity(uint256 amount0, uint256 amount1) internal returns(uint256 amount){
        /* uint256 balanceBefore = IERC20(pair).balanceOf(pair);
        (bool success, ) = address(ROUTER).delegatecall(
            abi.encodeWithSignature("addLiquidity(address, address, uint256, uint256, uint256, uint256, address, uint256)", token, usdt, amount0, amount1, 0, 0, owner(), block.timestamp)
        );
        if(success) amount = IERC20(pair).balanceOf(pair) - balanceBefore;
        console.log(success); */
        (amount, , ) = ROUTER.addLiquidity(token, usdt, amount0, amount1, 0, 0, owner(), block.timestamp);
    }
    ...
    function _swapTokensForTokens(
        ...
        return amounts[1];
        //return IERC20(usdt).balanceOf(address(this)) - balance;
    }
