...
contract EggNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 public constant STAKING_TOKEN_MINTER = keccak256("STAKING_TOKEN_MINTER");
    uint public constant MAX_ARRAY_LENGTH_PER_REQUEST = 30;

    string private _internalBaseURI;
    uint[4] public _levelTable;
    ...
    function setLevelTable(uint[4] calldata levelTable) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function tokenUsed(uint tokenId) external onlyRole(TOKEN_FREEZER) {
        _tokens[tokenId].isUsed = true;
    }

    function tokenUnUsed(uint tokenId) external onlyRole(TOKEN_FREEZER) {
        _tokens[tokenId].isUsed = false;
    }
    ...
    function mint(address to, string memory name) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
        ...
        _tokens[tokenId].tokenId = _lastTokenId;
        _tokens[tokenId].originOnwer = to;
        _tokens[tokenId].isUsed = false;
    ...
    function stakingMint(
        ...
        uint level,
        string memory name
    ) public onlyRole(STAKING_TOKEN_MINTER) nonReentrant {
        require(to != address(0), "Address can not be zero");
        _lastTokenId += 1;
        uint tokenId = _lastTokenId;
        _tokens[tokenId].createTimestamp = block.timestamp;
    ...
    function levelUp(uint[] calldata tokenId) public nonReentrant {
        ...
        for (uint i = 0; i < tokenId.length; i++) {
            require(keccak256(bytes(_tokens[tokenId[0]].name)) == keccak256(bytes(_tokens[tokenId[i]].name)), "Not the same name");
        }

        _mintLevelUp((currentLevel + 1), tokenId, _tokens[tokenId[0]].name);
    ...
    //Private functions --------------------------------------------------------------------------------------------
    function _mintLevelUp(uint level, uint[] memory tokenId, string memory _name) private {
        for (uint i = 0; i < tokenId.length; i++) {
        ...
        _tokens[newTokenId].tokenId = _lastTokenId;
        _tokens[newTokenId].originOnwer = msg.sender;
        _tokens[newTokenId].isUsed = false;
        _tokens[newTokenId].name = _name;

        _safeMint(msg.sender, newTokenId);
    }

    function getTokens(address account) public view returns (Token[] memory){
        uint256 numTokens = 0;
        for (uint i = 1; i <= _lastTokenId; i++) {
            if (_exists(i)) { 
        ...
        Token[] memory ret = new Token[](numTokens);
        uint256 counter = 0;
        for (uint i = 1; i <= _lastTokenId; i++) {
            if (_exists(i)) {

                if (ownerOf(i) == account) {
                    ret[counter] = _tokens[i];
                    counter++;
                }
            }
        }
    ...
    function getCountTokens(uint256 level) public view returns (uint256){
        uint256 numTokens = 0;
        for (uint i = 1; i <= _lastTokenId; i++) {
            if (_exists(i)) { 
                if ( _tokens[i].level == level) {
                    numTokens++;
                }
            }
        }
