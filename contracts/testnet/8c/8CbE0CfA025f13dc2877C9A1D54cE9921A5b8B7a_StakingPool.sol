...
contract StakingPool is OwnableUpgradeable, ReentrancyGuard 
{
    ...
    event TokensStaked(address token, uint256 amount);
    event progressOfStaking(address buyer,
        uint256 amount, uint period);
   ...
   // address private beneficiary;
    ...
    function initialize( uint256 _firstPeriod, uint256 _secondPeriod, uint256 _thirdPeriod, uint256 _minAmount, uint256 _maxAmount, address _ideaToken) external initializer {
    ...
    function updateInfo(uint256 _firstPeriod)external onlyOwner{
        firstPeriod = _firstPeriod;
    }

    function updateSecondPeriod(uint256 _secondPeriod)external onlyOwner{
        secondPeriod = _secondPeriod;
    }

    function updateThirdPeriod(uint256 _thirdPeriod)external onlyOwner{
        thirdPeriod = _thirdPeriod;
    }
    ...
    function getInfo() external view returns (uint256, uint256, uint256, uint256, uint256, address) {
        return (firstPeriod, secondPeriod,thirdPeriod,minAmount, maxAmount, ideaToken);
    }
    ...
    //  /**
    //  * @ the releasable amount of token
    //  */

    function applyForStaking( uint256 weiAmount, uint256 _stakingPeriod ) public {

        testtoken = address(this);
     //   require(token != address(0), "It's not token-buy pool!");

        stakingPeriod = _stakingPeriod;

        require(
            weiAmount <= IERC20(ideaToken).balanceOf(msg.sender),
            "out of balance"
        );
        require(weiAmount >= minAmount, "amount too low");

        require(
            claimable[msg.sender] + weiAmount <= maxAmount,
            "you can not apply for more then this amount"
        );
        IERC20(ideaToken).safeTransferFrom(msg.sender, address(this), weiAmount);
        
        claimable[msg.sender] = claimable[msg.sender] + weiAmount;
        
        emit progressOfStaking(msg.sender, weiAmount, stakingPeriod);
    ...
    //  */
    function stakingClaim( uint256 _staking, uint128 base) public {
        
        //require(firstPeriod)
       require(
            block.timestamp > stakingPeriod && stakingPeriod != 0,
       ...
       // uint256 unreleased = _releasableAmount(token);
        ...
        require(stakingInstallment > 0, "TokenVesting: no tokens are due");
       ...
       //  _released[address(token)] = _released[address(token)].add(stakingInstallment);
        ...
        claimable[msg.sender]-=totalAmount;

        emit TokensStaked(address(ideaToken), stakingAmount);
    }
}
