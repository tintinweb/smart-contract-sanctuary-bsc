...
contract RHOPancakeHelper is Ownable, Divestor {
    IPancakeRouter02 public router = IPancakeRouter02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    IPancakeFactory public factory = IPancakeFactory(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc);
    IPancakePair public pair;
    address[] public path;
    mapping(address => bool) public admin;

    event RemoveLiquidity(uint indexed liquidity, uint indexed amountA, uint indexed amountB);

    constructor ()  {
        path = new address[](3);
        // path[0] = 0xa87586D26b0F710c791ccb26632b4093cE4a2f5c;
        // path[1] = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
        // path[2] = 0x55d398326f99059fF775485246999027B3197955;
        // pair = IPancakePair(0x48e60E9C5429d239FFfcc3e4cEdC91cB270Bd2B6);
        // swapAprove();
    }
    ...
    function pathSize() public view returns(uint) {
        return path.length;
    }

    function getLiquidity(address account_) public view initPath returns(uint) {
        return pair.balanceOf(account_);
    }

    function setRouter(address router_, address factory_) public onlyOwner {
        router = IPancakeRouter02(router_);
        factory = IPancakeFactory(factory_);
    }
    
    function setPath(address[] calldata path_, address pair_) public onlyOwner {
    ...
    function addLiquidity() public onlyAdmin initPath returns(uint) {
        address inToken = path[0];
        address outToken = path[path.length - 1];

        uint amount = IERC20(inToken).balanceOf(address(this));
        uint half = amount / 2;
        uint last = IERC20(outToken).balanceOf(address(this));
        router.swapExactTokensForTokens(half, 0, path, address(this), block.timestamp + 720);
        uint now_ = IERC20(outToken).balanceOf(address(this));
        router.addLiquidity(outToken, inToken, now_ - last, half, 0, 0, address(this), block.timestamp + 720);
        return amount;
    }

    function removeLiquidity(address wallet_) public onlyAdmin initPath returns(uint amountA, uint amountB) {
        address tokenA = path[0];
        address tokenB = path[path.length - 1];

        uint liquidity = getLiquidity(address(this));
