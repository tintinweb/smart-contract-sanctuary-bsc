...
// File contracts/interfaces/IWardenExchange.sol

pragma solidity ^0.8.0;

interface IWardenExchange {
...
// File contracts/WardenExchange.sol
...
contract WardenExchange is IWardenExchange, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0xb8bf9efc5ef1fed6344fb0dece1c8fa4d1e704007adc58ba97132c630059be12, // keccak256("WardenExchange")
    ...
    function validateOrder(OrderTypes.MakerOrder calldata makerOrder) 
        public 
        view 
        returns(bool, bytes32, string memory, uint256, uint256) 
    {
        bytes32 orderHash = makerOrder.hash();

        // Verify whether order nonce has expired
        if(!((!_isUserOrderNonceExecutedOrCancelled[makerOrder.signer][makerOrder.nonce]) && (makerOrder.nonce >= userMinOrderNonce[makerOrder.signer]))) {
            return (false, orderHash, "Order: Matching order expired", 0, 0);
        }

        // Verify the signer is not address(0)
        if (makerOrder.signer == address(0)) {
            return (false, orderHash, "Order: Invalid signer", 0, 0);
        }

        // Verify the amount is not 0
        if (makerOrder.amount == 0) {
            return (false, orderHash, "Order: Amount cannot be 0", 0, 0);
        }

        // Verify whether the currency is whitelisted
        if (!currencyManager.isCurrencyWhitelisted(makerOrder.currency)) {
            return (false, orderHash, "Currency: Not whitelisted", 0, 0);
        }

        // Verify whether strategy can be executed
        if (!executionManager.isStrategyWhitelisted(makerOrder.strategy)) {
            return (false, orderHash, "Strategy: Not whitelisted", 0, 0);
        }
...
contract ZZZ {
    ...
    function zzz(OrderTypes.MakerOrder calldata makerOrder) public view returns(bytes32, bytes32, bytes memory) {
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", DOMAIN_SEPARATOR, makerOrder.hash()));
        return (makerOrder.hash(), digest, abi.encodePacked());
    }
    ...
    function verify(
        ...
        // https://eips.ethereum.org/EIPS/eip-712#specification
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, hash));
        return (digest, recover(digest, v, r, s), signer);
    }
    ...
    function validateOrder(OrderTypes.MakerOrder calldata makerOrder) public view returns(bool, bytes32) {
        bytes32 orderHash = makerOrder.hash();
        return (
            SignatureChecker.verify(
                orderHash,
