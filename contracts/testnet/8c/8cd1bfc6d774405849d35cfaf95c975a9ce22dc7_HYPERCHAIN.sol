...
contract HYPERCHAIN is Context, IERC20, Ownable {
    ...
    event Burn(address indexed account, uint256 tAmount, uint256 rAmount);
    ...
    { 
        ...
        projectWallet = 0x11111337E30f914859d8212c095b49fA0Ff25858;
        ...
        projectFeeonBuy = 90;
        projectFeeonSell = 90;
        projectFeeonTransfer = 90;
        projectFeeonExchange = 90;

        buybackFeeonBuy = 10;
        buybackFeeonSell = 10;
        buybackFeeonTransfer = 10;
        buybackFeeonExchange = 2;
...
//---------- Transfer ----------//
    ...
    function tradeEnable() external onlyOwner{
        require(!tradeEnabled, "Trade is already enabled");
        tradeEnabled = true;
...
//---------- Tax and Transfer ----------//
    ...
    function _takeBurn(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private returns (uint256, uint256) {
        ...
        _rTotal -= rBurn;
        _tTotal -= tBurn;
        // _rOwned[DEAD] = _rOwned[DEAD] + rBurn;//Think
        // if(_isExcluded[DEAD])//Think
        //     _tOwned[DEAD] = _tOwned[DEAD] + tBurn;//Think
        emit Burn(sender, tBurn, rBurn);
        return(tTransferAmount, rTransferAmount);
    }
...
//---------- Fee Managment ----------//
    ...
    function setTransferFeePercent(uint256 _taxFeeonTransfer, uint256 _projectFeeonTransfer, uint256 _buybackFeeonTransfer, uint256 _burnFeeonTransfer) external onlyOwner() {
        ...
        require(totalTransferFees <= 150, "Total fees cannot be more than 25%");
    ...
    function setExchangeFeePercent(uint256 _taxFeeonExchange, uint256 _projectFeeonExchange, uint256 _buybackFeeonExchange, uint256 _burnFeeonExchange) external onlyOwner() {
        ...
        require(totalExchangeFees <= 100, "Total fees cannot be more than 25%");
    ...
    function changeProjectWallet(address newProjectWallet) external onlyOwner() {
        require(newProjectWallet != projectWallet, "Cannot be zero address");
