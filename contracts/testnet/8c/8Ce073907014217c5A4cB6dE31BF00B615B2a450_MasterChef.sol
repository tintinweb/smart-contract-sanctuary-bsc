...
// MasterChef is the master of ROLLER. He can make ROLLER and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once ROLLER is sufficiently
...
contract MasterChef is IMasterChef, Ownable, ReentrancyGuard{
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of ROLLERs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accROLLERPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accROLLERPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. ROLLERs to distribute per block.
        uint256 lastRewardBlock; // Last block number that ROLLERs distribution occurs.
        uint256 accROLLERPerShare; // Accumulated ROLLERs per share, times 1e12. See below.
        uint8 poolType; // 0 for RollerLp, 1 for NonRollerLp, 2 for RollerBanker, 3 for NonRollerBanker
    }
    ...
    // ROLLER tokens created per block.
    uint256 public ROLLERPerBlock;
    // Bonus muliplier for early ROLLER makers.
    ...
    // The block number when ROLLER mining starts.
    ...
    // the percent of lp when calculating level
    uint256 public lpLevelPercent = 5000;
    // minimum percent when claiming ROLLER, default 20%
    uint256 public minClaimPercent = 2000;
    // the threshold to judge whether a user can claiming ROLLER fully, default 10%
    uint256 public fullyClaimTh = 1000;

    // LuckyRoller referral contract address.
    IReferral public referral;
    // RollerLevel contract address.
    ...
    event UpdateRollerPerBlock(uint256 rollerPerBlock);
    ...
    event SetRollerReferral(address _rollerReferral);
    event SetRollerLevel(address _RollerLevel);

    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event ClaimROLLER(address indexed user, uint256 indexed pid, uint256 amount);
    ...
    constructor(
        ...
        ROLLERPerBlock = _ROLLERPerBlock;
    ...
    function withdrawDevFee() public{
        ...
        uint256 ROLLERReward = multiplier.mul(ROLLERPerBlock);
        ROLLER.mint(dev0Addr, ROLLERReward.mul(dev0Percent).div(percentDec));
        ROLLER.mint(dev1Addr, ROLLERReward.mul(dev1Percent).div(percentDec));
        ROLLER.mint(dev2Addr, ROLLERReward.mul(dev2Percent).div(percentDec));
        ROLLER.mint(ecoAddr, ROLLERReward.mul(ecoPercent).div(percentDec));
        ROLLER.mint(treasuryAddr, ROLLERReward.mul(treasuryPercent).div(percentDec));
    ...
    // Update the given pool's ROLLER allocation point. Can only be called by the owner.
    ...
    // View function to see pending ROLLERs on frontend.
    function pendingROLLER(uint256 _pid, address _user) public view validPool(_pid) returns (uint256){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accROLLERPerShare = pool.accROLLERPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 ROLLERReward = multiplier.mul(ROLLERPerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(stakingPercent).div(percentDec);
            accROLLERPerShare = accROLLERPerShare.add(ROLLERReward.mul(1e12).div(lpSupply));
        }
        return user.pendingReward.add(user.amount.mul(accROLLERPerShare).div(1e12).sub(user.rewardDebt));
    }
    ...
    function updatePool(uint256 _pid) public validPool(_pid) {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ROLLERReward = multiplier.mul(ROLLERPerBlock).mul(pool.allocPoint).div(totalAllocPoint).mul(stakingPercent).div(percentDec);
        ROLLER.mint(address(this), ROLLERReward);
        pool.accROLLERPerShare = pool.accROLLERPerShare.add(ROLLERReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // add pending ROLLERs.
    function addPendingROLLER(uint256 _pid, address _user) internal validPool(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 pending = user.amount.mul(pool.accROLLERPerShare).div(1e12).sub(user.rewardDebt);
        if (pending > 0) {
            // add rewards
    ...
    // Deposit LP tokens to MasterChef for ROLLER allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant validPool(_pid) {
        ...
        addPendingROLLER(_pid, msg.sender);
        if(_amount > 0){
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
        }
        user.rewardDebt = user.amount.mul(pool.accROLLERPerShare).div(1e12);
        user.nextWithdrawUntil = block.timestamp.add(withdrawInterval);
        if(address(RollerLevel) != address(0)){
            RollerLevel.updateLevel(msg.sender);
        }
    ...
    // Safe ROLLER transfer function, just in case if rounding error causes pool to not have enough ROLLERs.
    function safeROLLERTransfer(address _to, uint256 _amount) internal {
        uint256 ROLLERBal = ROLLER.balanceOf(address(this));
        if (_amount > ROLLERBal) {
            ROLLER.transfer(_to, ROLLERBal);
        } else {
            ROLLER.transfer(_to, _amount);
        }
    }
    ...
    function getClaimRatio(uint256 _pid, address _user) public view returns (uint256 ratio){
        // claim all ROLLER for poolType = 0 and poolType = 2
        ratio = percentDec;
        PoolInfo storage pool = poolInfo[_pid];
        if(pool.poolType == 1){
            UserInfo storage user = userInfo[_pid][_user];
            if(address(RollerLevel) != address(0) && address(oracle) != address(0)){
                ...
                uint256 currentPoolLevel = pendingROLLER(_pid, _user);
                if(totalLevel > currentPoolLevel){
                    uint256 usefulLevel = totalLevel.sub(currentPoolLevel);
                    uint256 lpValue = oracle.getLpTokenValue(address(pool.lpToken), user.amount);
                    if(lpValue > 0 && fullyClaimTh > 0 && usefulLevel < lpValue.mul(fullyClaimTh).div(percentDec)){
                        // y = minClaimPercent + x * (10000 - minClaimPercent) / fullyClaimTh
                        uint256 x = usefulLevel.mul(percentDec).div(lpValue);
            ...
            if(address(RollerLevel) != address(0) && address(oracle) != address(0)){
                ...
                if(totalLevel > currentPoolLevel){
                    ...
                    if(bankerValue > 0 && fullyClaimTh > 0 && usefulLevel < bankerValue.mul(fullyClaimTh).div(percentDec)){
                        // y = x * (10000 - minClaimPercent) / fullyClaimTh + minClaimPercent
                        uint256 x = usefulLevel.mul(percentDec).div(bankerValue);
    ...
    function claimROLLER(uint256 _pid) public nonReentrant validPool(_pid) {
        ...
        if(user.pendingReward > 0) {
            ...
            if(ratio < percentDec){
                ...
                if(tmpAmount > 0){
                    safeROLLERTransfer(treasuryAddr, tmpAmount);
                }
                amount = amount.sub(tmpAmount);
                if(amount > 0){
                    safeROLLERTransfer(msg.sender, amount);
                }
            }else{
                safeROLLERTransfer(msg.sender, amount);
            }
            ...
            ClaimROLLER(msg.sender, _pid, amount);
        }
    }

    // Get pending level from MasterChef and advoid stack too deep
    function getRollerLevel(address account) public override view returns (uint256, uint256, uint256) {
        ...
        for(uint256 i = 0; i < poolInfo.length; i ++){
            if(poolInfo[i].poolType == 0){
                if(userInfo[i][account].amount > 0 && address(oracle) != address(0)){
                    ...
                    tmpLpQuantity = tmpLpQuantity.add(tmpValue.mul(lpLevelPercent).div(percentDec));
                    tmpQuantity = tmpQuantity.add(tmpValue.mul(lpLevelPercent).div(percentDec));
                }
                tmpValue = pendingROLLER(i, account);
                if(tmpValue > 0){
        ...
        if(EnumerableSet.contains(_tokenomicAddrs, account)){
            if(account == dev0Addr){
                devPending = getMultiplier(lastBlockDevWithdraw, block.number).mul(ROLLERPerBlock).mul(dev0Percent).div(percentDec);
            }else if(account == dev1Addr){
                devPending = getMultiplier(lastBlockDevWithdraw, block.number).mul(ROLLERPerBlock).mul(dev1Percent).div(percentDec);
            }else if(account == dev2Addr){
                devPending = getMultiplier(lastBlockDevWithdraw, block.number).mul(ROLLERPerBlock).mul(dev2Percent).div(percentDec);
            }else if(account == ecoAddr){
                devPending = getMultiplier(lastBlockDevWithdraw, block.number).mul(ROLLERPerBlock).mul(ecoPercent).div(percentDec);
            }else if(account == treasuryAddr){
                devPending = getMultiplier(lastBlockDevWithdraw, block.number).mul(ROLLERPerBlock).mul(treasuryPercent).div(percentDec);
            }
        }
        if(devPending > 0){
    ...
    function updateRollerPerBlock(uint256 newAmount) public onlyOwner {
        require(newAmount <= 100 * 1e18, 'Max per block 100 ROLLER');
        require(newAmount >= 1 * 1e15, 'Min per block 0.001 ROLLER');
        ROLLERPerBlock = newAmount;
        emit UpdateRollerPerBlock(ROLLERPerBlock);
    }
    ...
    function setLpLevelPercent(uint256 _percent) public onlyOwner {
        require(_percent <= percentDec, "range");
        lpLevelPercent = _percent;
    }
    ...
    function setReferral(address _rollerReferral) public onlyOwner {
        require(_rollerReferral != address(0), "Zero");
        referral = IReferral(_rollerReferral);
        emit SetRollerReferral(_rollerReferral);
    }
    ...
    function setRollerLevel(address _RollerLevelAddr) public onlyOwner {
        require(_RollerLevelAddr != address(0), "Zero");
        RollerLevel = IRollerLevel(_RollerLevelAddr);
        emit SetRollerLevel(_RollerLevelAddr);
    }
            ...
            if (address(referral) != address(0)){
                ...
                if (commissionAmount > 0) {
                    if (referrer != address(0)){
                        ROLLER.mint(address(referral), commissionAmount);
                ...
                uint256 commissionAmount = _pending.mul(referralCommissionRate).div(percentDec);
                if (commissionAmount > 0){
                    ROLLER.mint(treasuryAddr, commissionAmount);
            ...
            for(uint256 i = 0; i < poolInfo.length; i ++){
                ...
                if(lpSupply > 0){
                    if(poolInfo[i].poolType == 0 || poolInfo[i].poolType == 1){
                        tvlROLLER = tvlROLLER.add(oracle.getLpTokenValue(address(poolInfo[i].lpToken), lpSupply));
                    }else if(poolInfo[i].poolType == 2 || poolInfo[i].poolType == 3){
                        tvlROLLER = tvlROLLER.add(oracle.getBankerTokenValue(address(poolInfo[i].lpToken), lpSupply));
                    }
                }
            }
            if(tvlROLLER > 0){
                return oracle.getQuantityBUSD(address(ROLLER), tvlROLLER);
            }else{
                return 0;
            } 
        }else{
...
interface IReferral {
    ...
    /**
     * @dev Get the roller level of user.
     */
...
interface IMasterChef {
    ...
    function getRollerLevel(address user) external view returns (uint256, uint256, uint256);
}
