...
// File: contracts/interfaces/IPanrp1SwapLottery.sol

pragma solidity ^0.8.4;

interface IPanrp1SwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in RP1 per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in RP1 token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInRp1: price of a ticket in RP1
...
// File: contracts/MoonBizSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title MoonBiz Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract MoonBizSwapLottery is ReentrancyGuard, IPanrp1SwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInRp1 = 50 ether;
    uint256 public minPriceTicketInRp1 = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInRp1;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] rp1PerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInRp1;
        uint32 finalNumber;
    }
     ...
     * @param _rp1TokenAddress: address of the RP1 token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _rp1TokenAddress, address _randomGeneratorAddress) {
        rp1Token = IERC20(_rp1TokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of RP1 to this contract
        uint256 amountRp1ToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInRp1,
            _ticketNumbers.length
        );

        // Transfer RP1 tokens to this contract
        rp1Token.safeTransferFrom(address(msg.sender), address(this), amountRp1ToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInRp1 += amountRp1ToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInRp1ToTransfer
        uint256 rewardInRp1ToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInRp1ToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        rp1Token.safeTransfer(msg.sender, rewardInRp1ToTransfer);

        emit TicketsClaim(msg.sender, rewardInRp1ToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInRp1) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in RP1 for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].rp1PerBracket[j] =
                ...
                // A. No RP1 to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].rp1PerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInRp1 - amountToShareToWinners);

        // Transfer RP1 to treasury address
        rp1Token.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInRp1 += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInRp1 >= minPriceTicketInRp1) && (_priceTicketInRp1 <= maxPriceTicketInRp1),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInRp1: _priceTicketInRp1,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            rp1PerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInRp1: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(rp1Token), "Cannot be RP1 token");
    ...
    /**
     * @notice Set RP1 price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInRp1: minimum price of a ticket in RP1
     * @param _maxPriceTicketInRp1: maximum price of a ticket in RP1
     */
    function setMinAndMaxTicketPriceInRp1(uint256 _minPriceTicketInRp1, uint256 _maxPriceTicketInRp1)
        external
        onlyOwner
    {
        require(_minPriceTicketInRp1 <= _maxPriceTicketInRp1, "minPrice must be < maxPrice");

        minPriceTicketInRp1 = _minPriceTicketInRp1;
        maxPriceTicketInRp1 = _maxPriceTicketInRp1;
    }
     ...
     * @param _priceTicket price of a ticket (in RP1)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].rp1PerBracket[_bracket];
        } else {
            return 0;
        }
    }
