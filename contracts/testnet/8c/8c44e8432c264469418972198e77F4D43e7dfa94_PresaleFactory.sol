...
contract PresaleFactory is Ownable {
    ...
    uint256 public tokenPrice_BUSD = 1 * 10 ** 18;
    ...
    address payable presaleOwnerAddress = payable(0x51C0da7D74FB3e77398ee2C1340cAE97796fF5d1);

    mapping (address => uint256) private _userPaidBUSD;

    constructor(address _router, address _HYRI, address _busd) {
        _bsc01Router = IDEXRouter01(_router);
        _HYRIAddress = IHYRIToken(_HYRI);
    ...
    function buyTokensByBUSD(uint256 _amountPrice) external {
        ...
        uint256 decimalPrice = _amountPrice * 10 ** 18;
        uint256 tokenAmount = decimalPrice / tokenPrice_BUSD;

        uint256 currentPaid = _userPaidBUSD[msg.sender];
        require(currentPaid + decimalPrice >= minCapBUSD && currentPaid + decimalPrice <= maxCapBUSD, "PresaleFactory: The price is not allowed for presale.");
        
        // transfer BUSD to owners
        _busdAddress.transferFrom(msg.sender, address(this), decimalPrice);

        // transfer HYRI token to user
        _HYRIAddress.transfer(msg.sender, tokenAmount);

        // add BUSD user bought
        _userPaidBUSD[msg.sender] += decimalPrice;
    ...
    function buyTokensByBNB() external payable {
        ...
        // token amount user want to buy
        uint256 decimalPrice = amountPrice * 10 ** 18;
        uint256 tokenAmount = decimalPrice / tokenPrice_BUSD;
    ...
    function releaseFund() external onlyOwner {
        ...
        if(BNBbalance > 0)
            payable(address(this)).transfer(BNBbalance);
