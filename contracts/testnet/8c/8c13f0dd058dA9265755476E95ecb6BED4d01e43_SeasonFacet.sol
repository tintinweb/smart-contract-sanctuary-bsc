...
contract SeasonFacet is Sun {
    ...
    function sunrise() external {
        ...
        stepSilo(increase);
        incentivize(msg.sender, beanPrice);
    ...
    function incentivize(address account, Decimal.D256 memory price) private {
        uint256 rewardMultiplier = s.season.rewardMultiplier;
        if (rewardMultiplier > 100) rewardMultiplier = 100;
        uint256 incentive = LibIncentive.fracExp(rewardMultiplier, 100, incentiveTime(), 1);
        uint256 feeInEth = tx.gasprice.mul(s.season.costSunrice).div(Decimal.BASE);
        uint256 amount = incentive.add(Decimal.one().asUint256().div(price.asUint256()).mul(feeInEth));
