...
contract PreSendPayments is AutomationCompatibleInterface, Initializable, AccessControlUpgradeable  {
    ...
    */
    function initialize(address _treasuryAddress, address _aggregatorAddress, uint256 _sendPaymentFeesInterval) initializer external {
        ...
        affiliateRegistrationTime = YEAR_IN_SECONDS;
...
contract PreSendAffiliate is Initializable, AccessControlUpgradeable {
    ...
    uint256 public defaultAffiliateTier;

    // Number of affiliate tiers added to the contract. If this count is 2, for example,
    ...
    // Event to emit whenever an affiliate is updated (or removed by having their tier updated to 0).
    event affiliateUpdated(address indexed affiliate, uint256 affiliateTier);

    // Event to emit whenever the default affiliate tier is updated.
    event defaultAffiliateTierUpdated(uint256 indexed newDefaultAffiliateTier);
    ...
    */
    function addAffiliate(address affiliate) external {
        ...
        affiliateToTier[affiliate] = defaultAffiliateTier;

        affiliateToRegisteredTimestamp[affiliate] = block.timestamp;

        emit affiliateAdded(affiliate, defaultAffiliateTier);
    }

    /**
    @dev Affiliate admin only function to update an affiliate tier (or remove the affiliate by updating their tier to 0).
    @param affiliate the address of the affiliate
    @param tier the tier of the affiliate that determines the percentage of transfer fees they get. 0 for affiliate removal.
    */
    function updateAffiliate(address affiliate, uint256 tier) external onlyRole(AFFILIATE_ADMIN) {
        require(affiliate != address(0), "Affiliate address cannot be the zero address.");
    ...
    /**
    @dev Affiliate admin only function to change the default affiliate tier used when someone registers themselves as an affiliate.
    @param newDefaultAffiliateTier the new default affiliate tier, which has to be a valid tier above 0%
    */
    function updateDefaultAffiliateTier(uint256 newDefaultAffiliateTier) external onlyRole(AFFILIATE_ADMIN) {
        require(affiliateTierToPercentage[newDefaultAffiliateTier] > 0, "This affiliate tier does not exist or has been removed.");
        defaultAffiliateTier = newDefaultAffiliateTier;
        emit defaultAffiliateTierUpdated(newDefaultAffiliateTier);
    }
