...
library Whitelist {
    function verifySignatureWhenBuyDNFT(
        ...
        bytes32 messageHash = getMessageHashWhenBuyDNFT(_salePhase, _signer, _candidate);
    ...
    function verifySignatureWhenBuyKey(
        ...
        bytes32 messageHash = getMessageHashWhenBuyKey(_signer, _candidate);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        address signerAddress = getSignerAddress(ethSignedMessageHash, _signature);
        return signerAddress == _signer;
    }
    ...
    function getMessageHashWhenBuyDNFT(
        ...
        return keccak256(abi.encodePacked(_salePhase, _signer, _candidate));
    }

    function getMessageHashWhenClaimDNFT(
        address _signer,
        address _candidate,
        uint256 _amountOfToken,
        uint256 _totalClaimFunctionCall
    ) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_signer, _candidate, _amountOfToken, _totalClaimFunctionCall));
    }

    function getMessageHashWhenBuyKey(address _signer, address _candidate) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(_signer, _candidate));
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant TAX_RATE = 1200; // 12%
    ...
    modifier hasTreasuryAddress() {
        require(treasuryAddress != address(0), "DNFT: TREASURY ADDRESS NOT SET");
        _;
    }
    ...
    /*---------- MINT FUNCTIONS -----------*/
    function buyUsingBUSD(address _to, bytes memory _signature) external whenNotPaused hasTreasuryAddress nonReentrant {
        require(canBuy(_to), "DKEYNFT: CANNOT BUY");
        require(verifyBuySignature(_to, _signature), "DKEYNFT: INVALID SIGNATURE");

        uint256 BUSDBalance = BUSDToken.balanceOf(msg.sender);
        uint256 taxAmount = getTaxAmount(keyPrice);
        require(BUSDBalance >= keyPrice + keyPrice, "DKEYNFT: INSUFFICIENT BALANCE");

        BUSDToken.transferFrom(msg.sender, treasuryAddress, keyPrice + taxAmount);
        uint256 tokenId = safeMint(_to);

        emit BuyWithBUSD(_to, tokenId);
    }

    function buyUsingBNB(address _to, bytes memory _signature)
        external
        payable
        whenNotPaused
        hasTreasuryAddress
        nonReentrant
    {
        require(canBuy(_to), "DKEYNFT: CAN NOT BUY");
        require(verifyBuySignature(_to, _signature), "DKEYNFT: INVALID SIGNATURE");

        uint256 priceInBNB = convertBUSDToBNB(keyPrice);
        require(msg.value >= priceInBNB, "DKEYNFT: INSUFFICIENT BALANCE");
        payable(treasuryAddress).transfer(msg.value);

        uint256 taxAmount = getTaxAmount(keyPrice);
        uint256 BUSDBalanceOfBuyer = BUSDToken.balanceOf(msg.sender);
        require(BUSDBalanceOfBuyer >= taxAmount, "DKEYNFT: INSUFFICIENT BALANCE");
        BUSDToken.transferFrom(msg.sender, treasuryAddress, taxAmount);

        uint256 tokenId = safeMint(_to);

        emit BuyWithBNB(_to, tokenId);
    }
    ...
    function verifyBuySignature(address _to, bytes memory _signature) private view returns (bool) {
        require(msg.sender == _to, "DKEYNFT: WRONG CANDIDATE");
        return (Whitelist.verifySignatureWhenBuyKey(signer, _to, _signature));
    }
    ...
    function getTaxAmount(uint256 _priceInBUSD) private pure returns (uint256) {
    ...
    function convertBNBToBUSD(uint256 _BNBAmount) external view returns (uint256) {
        int256 BNBPerBUSD = getLatestPrice();
        uint8 decimals = getPriceFeedDecimals();
        uint256 BNBPerBUSDInUint256 = uint256(BNBPerBUSD);
    ...
    function safeTransferFrom(
        ...
        uint256 taxAmount = getTaxAmount(launchPrice);
        require(BUSDToken.balanceOf(from) >= taxAmount, "DKEYNFT: NOT ENOUGH BUSD");
        BUSDToken.transferFrom(from, treasuryAddress, taxAmount);
