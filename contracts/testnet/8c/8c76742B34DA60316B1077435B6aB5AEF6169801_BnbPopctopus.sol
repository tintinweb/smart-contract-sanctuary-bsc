...
contract BnbPopctopus {
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buyPopctopushiminers();}
    ...
    function BuyPopctopus(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startPopctopus(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many Popctopus per day user will receive based on BNB deposit */
    ...
    function buyPopctopushiminers() private{
