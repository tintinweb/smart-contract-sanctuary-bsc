...
contract MetaStake is Ownable,ReentrancyGuard,Pausable {
    ...
    function harvest(address _from,uint256 _id) public whenNotPaused {
        userDetails storage user = investDetails[_from][_id];
        require(user.depositTime > 0 && user.depositAmount > 0,"Invalid user");
        uint256 reward = this.viewReward(_from,_id);

        require(reward > 0,'NO_REWARD');
        tokenSafeTransfer(stakedToken,_from,reward);
    }
