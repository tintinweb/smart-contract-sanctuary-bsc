...
contract Presale {
    ...
    function Config(IERC20 _BACN, IERC20 _BUSD, uint256 _priceBUSD, uint256 _priceBNB, uint256 _priceBNBUSDT, uint256 _minSale, uint256 _endTimeSale, address[] memory promos) public isOwner returns (bool success) {
		...
		priceBNB = _priceBNB;
        require(_priceBNBUSDT > 0, "missing priceBNBUSDT value");
		priceBNBUSDT = _priceBNBUSDT;
        require(_minSale > 0, "missing minSale value");
        minSale = _minSale;//2700000
        uint256 incr = BUSD.decimals();
        for (uint8 i = 0; i < incr; i++){
            minSale *= 10;
        }
        ...
        //CLEAN
        for (uint256 i = 0; i < promoTotal; i++){
    ...
    function setPrice(uint256 _priceBUSD, uint256 _priceBNB, uint256 _priceBNBUSDT) public isOwner returns (bool success){
    ...
    function setStatus(uint256 status) public isOwner returns (bool success){
        statusSale = status;
        return true;
        //TODO revert to back if-else
        /*
        if((statusSale == 0 && status == 1) || (statusSale == 1 && status == 2) || (statusSale < 3 && status == 3)){
            if(status == 2){
                //IF SET END STATUS NEED CHECK AVAILABLE BALANCE & DATETIME
                require(receiveSale >= minSale, "Not possible end presale. Balance is less them Minimum");
                require(endTimeSale > block.timestamp, "Not possible end presale. Time for presale not ended");
            }
            if(status == 3){
                //IF SET FROZE STATUS NEED CHECK AVAILABLE BALANCE
	...
	function BuyBUSD(uint256 amount) public returns (bool success){
        require(statusSale == 1, "Presale not active");
        if(endTimeSale <= block.timestamp){
            //PRESALE ENDED
            statusSale = 2;
        }
        ...
        require(paid > 0, "BuyBUSD: amount for paid is not positive");
        uint256 allowance = BUSD.allowance(msg.sender, address(this));
        require(allowance >= paid, "BuyBUSD: Check the token allowance");
        require(BUSD.transferFrom(msg.sender, address(this), paid) == true, "BuyBUSD: Couldn't transfer BUSD from buyer");
        require(BACN.transfer(msg.sender, amount) == true, "BuyBUSD: Couldn't transfer BACN to buyer");
	...
	function BuyBNB() payable public {
        ...
        uint256 diff = 18 - BACN.decimals();
        uint256 up = 1;
        for (uint8 i = 0; i < diff; i++){
            up *= 10;
        }
        uint256 received = ( amount * priceBNB ) / up;
        ...
        receiveSale += amount * priceBNBUSDT;
	...
	function EndPresaleERC(IERC20 token_address) public isOwner {
        ...
        if(balance_ > 0){
            ...
            require(false, "EndPresaleERC: balance token is zero");
        }
    }
