...
interface IDividendDistributor {
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToBUSDThreshold) external;
...
contract DividendDistributor is IDividendDistributor {
    ...
    // BUSDswap contract address
    address BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;//BUSD TESTNET
    // bnb address
    ...
    uint256 public swapToBUSDSwapThreshold = 1 * (10 ** 18);
    ...
    constructor (address _router) {
        ...
        // : IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);//mainnet
        _token = msg.sender;
    }

    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution, uint256 _bnbToBUSDSwapThreshold) external override onlyToken {
        minPeriod = _minPeriod;
        minDistribution = _minDistribution;
        swapToBUSDSwapThreshold = _bnbToBUSDSwapThreshold;
    }
    ...
    function deposit() external override onlyToken {
        ...
        if (bnbBalance >= swapToBUSDSwapThreshold) {
            ...
            router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: swapToBUSDSwapThreshold}(
                0,
                path,
                address(this),
                block.timestamp
            ...
            uint256 amount = IERC20(BUSD).balanceOf(address(this)).sub(balanceBefore);
    ...
    function setBUSDSwapAddress(address nBUSD) external onlyToken {
        BUSD = nBUSD;
    }
 ...
 *  This Contract Awards BUSDSwap and BUSDCrib Daily to holders, weighted by how much you hold
 *  BUSDCrib Burn Wallet Gains 2% of the Distribution (contributing to asynchronous burning)
 *  This is due to the fact that we sent 2% of the supply to BUSDCrib's Burn Wallet on Launch
 *  
 *  Transfer Fee:  5%
 *  Buy Fee:       5%
 *  Sell Fee:     30%
 ...
 *  1% Marketing
 */
contract NGANUTOKENX is IERC20, Context, Ownable {
    ...
    address public BUSD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //BUSD TESTNET
    ...
    // token data
    string constant _name = "NGANU TOKENX";
    string constant _symbol = "NGANUX";
    ...
    // fees
    /*  17% BUSDSwap Token Reflections
        5% BUSDCrib Token Reflections
        5% Buy Back and Burn
        2% Liquidity
    ...
    */
    ...
    uint256 public reflectBUSDFee = 1000;
    ...
    address public marketingFeeReceiver = 0x5e277BdcA8655059b93c04A5FA8A035Cf5fe8842;
    ...
    // Uniswap Router V2 - Change to BUSDswap router 
    ...
    uint256 public totalBNBBUSDSwapReflections = 0;
    ...
    /**
     *  Swaps BUSDCrib for BNB if threshold is reached and the swap is enabled
     *  Uses BNB retrieved to:
     *      fuel the contract for buy/burns
     *      provide distributor with BNB for BUSDSwap
    ...
    /** Transfers BNB to BUSDSwap Distributor and Marketing Wallet */
    function transferToDistributorAndMarketing(uint256 distributorBNB, uint256 marketingBNB) internal {
        ...
        if (success) {
            try distributor.deposit() {totalBNBBUSDSwapReflections = totalBNBBUSDSwapReflections.add(marketingBNB);} catch {}
        }
    ...
    /**
     * Buys BUSDCrib with bnb in the contract, sending to target address
     */ 
    ...
    /** Sets Various Fees */
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _reflectBUSDFee, uint256 _marketingFee, uint256 _feeDenominator) external onlyOwner {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
        reflectBUSDFee = _reflectBUSDFee;
        marketingFee = _marketingFee;
        totalFeeSells = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_marketingFee).add(_reflectBUSDFee);
    ...
    function setBUSDSwapAddress(address nBUSD) public onlyOwner {
        distributor.setBUSDSwapAddress(nBUSD);
    }
