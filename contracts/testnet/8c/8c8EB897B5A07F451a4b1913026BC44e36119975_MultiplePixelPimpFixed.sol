...
// File: IMultiplePixelPimp.sol

// Multiple PixelPimp Fixed Price Marketplace contract
...
contract MultiplePixelPimpFixed is Ownable, ERC1155Holder {
	...
	function initialize(address _feeAddress) external onlyOwner {
		...
		createCollection("Cryptic Chaos", "https://ipfs.io/ipfs/QmaP2x6HvuJqmpjXAAucQJScLxg7Wsh4MaQLgXddyPAFrS", true);
	}
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
		bytes memory bytecode = type(MultiplePixelPimp).creationCode;
        ...
        IMultiplePixelPimp(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function multipleList(address _collection, uint256 _tokenId, address _tokenAdr, uint256 _amount, uint256 _price) public {
		...
		IMultiplePixelPimp nft = IMultiplePixelPimp(_collection);
	...
	function multipleDelist(uint256 _id) external {
		...
		IMultiplePixelPimp(pairs[_id].collection).safeTransferFrom(address(this), pairs[_id].owner, pairs[_id].tokenId, pairs[_id].balance, "delist Marketplace");
		pairs[_id].balance = 0;
    ...
    function multipleBuy(uint256 _id, uint256 _amount) external payable {
		...
		IMultiplePixelPimp(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, item.tokenId, _amount, "buy from Marketplace");
...
contract MultiplePixelPimp is ERC1155, AccessControl {
    using SafeMath for uint256;
    ...
    constructor() ERC1155("MultiplePixelPimp") {
        factory = msg.sender;
    }
