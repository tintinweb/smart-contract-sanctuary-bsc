...
contract EngineBusd {
    ...
    uint256 I_P = 0.0025 ether;
    uint256 D_P = 0.004 ether;
    ...
    constructor() {
        TokenAddress = 0x83dDc114d9d8A081A1756d235e3520d84D419FEE;
    ...
    function DEPO(address _ref, uint256 _amount) public {
       ...
       uint256 _amtx = SafeMath.sub(_amount,feeViewer(_amount));
      ...
      // Add value to over TVL
       ...
       UserQuery[msg.sender] = UserInvestment(msg.sender,totalCoins,_point + 1);
       
       BUSD.transferFrom(msg.sender,address(this),_amtx);
       BUSD.transferFrom(msg.sender,dev,feeViewer(_amount));
       ...
       refQuery[msg.sender] = UserRef(msg.sender,_ref);
    }
   ...
   function Stake(uint256 _amount, uint256 noOfDays) public {
       ...
       total_supply = SafeMath.add(total_supply,toGetCoins); // fixed stakes
       _deadline = SafeMath.add(_deadline,block.timestamp);
       uint256 _stakeId = stakeQuery[msg.sender].stakeId;
       if(stakeQuery[msg.sender].user_addr == address(0)) {
        
       UserQuery[msg.sender].amount = SafeMath.sub(UserQuery[msg.sender].amount,_amount);
       DepoQuery[msg.sender][_stakeId] = UserDepoMap(msg.sender,0,_amount,totalReward,block.timestamp,_deadline, false);
       stakeQuery[msg.sender] = stakeId(msg.sender,0);
       ...
       UserQuery[msg.sender].amount = SafeMath.sub(UserQuery[msg.sender].amount,_amount);
       DepoQuery[msg.sender][_stakeIdIncrease] = UserDepoMap(msg.sender,_stakeId + 1,_amount,totalReward,block.timestamp,_deadline, false);
   ...
   function Collect(uint256 _StakeId) public {
       // time testing and collecting stakes.
       require(block.timestamp>=DepoQuery[msg.sender][_StakeId].deadline, "Please Wait Till your Time comes for Collection");
       ...
       claimQuery[msg.sender] = UserCollected(msg.sender,_value);
    ...
    function Swap(uint256 _amount) public {
        ...
        require(block.timestamp>=withdrawTime[msg.sender].deadline,"Your next withdraw time is still left");
       ...
       BUSD.transfer(refQuery[msg.sender].ref_addr,refFee);
        ...
        withdrawTime[msg.sender] = UserWithdrawTime(msg.sender,block.timestamp,deadline);
     ...
     function AntiWhale(address _userAddr) public view returns(uint256) {
        ...
        uint256 total = SafeMath.div(SafeMath.mul(TVL,8),100);
     ...
     function feeViewer(uint256 _amount) public view returns(uint256) {
         ...
         return SafeMath.mul(_value,Dev_Fee);
     }
