...
contract OmniDragons is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    ...
    function mint(uint256 _mintAmountDragons) public payable{
       /*  require(!paused, "OMNI_DRAGONS: IS PAUSED");
        require(_mintAmountDragons > 0 && _mintAmountDragons <= maxMintPerTx, "OMNI_DRAGONS: CAN ONLY MINT 5 PER TX");
        require(nextTokenId + _mintAmountDragons <= maxSupply, "OMNI_DRAGONS: MAX_SUPPLY_FOR_CHAIN");
        require(_mintAmountDragons <= maxMintPerTx && balanceOf(msg.sender) < maxPerWallet, "OMNI_DRAGONS: MAX_2_OMNI_DRAGONS_PER_WALLET"); */
        require(msg.value >= mintPrice * _mintAmountDragons, "OMNI_DRAGONS: NOT ENOUGH ETHER");
        unchecked {
            for(uint256 i = 0; i < _mintAmountDragons; i++){
