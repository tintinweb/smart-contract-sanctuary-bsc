...
contract madprint is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 87654321;
    ...
    uint256 public rebaseFrequency = 30;
    uint256 public inception = block.timestamp;
    uint256 public nextRebase = inception + rebaseFrequency;
    ...
    uint256 private constant MAX_REBASE_FREQUENCY = 30;
    ...
    constructor() ERC20Detailed("cursive", "printmad", uint8(DECIMALS)) {
    ...
    function _deltaV() private {
      if(autoRebase = true && nextRebase <= block.timestamp) {
            _rebase();
            nextRebase = nextRebase + rebaseFrequency;
            _deltaV();
      } else if (nextRebase > block.timestamp) {
          _deltaV();
      }
    }
    ...
    function coreRebase(int256 supplyDelta) private returns (uint256) {
        ...
        if (supplyDelta == 0) {
            emit LogRebase(nextRebase, _totalSupply);
            return _totalSupply;
        }
