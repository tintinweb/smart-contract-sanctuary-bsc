...
contract MSDC is Context, IERC20, Ownable {
    ...
    uint256 public numTokensSellToAddToLiquidity = 2e14 *10**8;
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyLiquidityFee = liquidityFee;
        _buyBurnFee = burnFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyBurnFee <= 15, "Buy Fee should be 15% or less");
    }

    function setSellFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellLiquidityFee = liquidityFee;
        _sellBurnFee = burnFee;
        require( _sellTaxFee + _sellLiquidityFee + _sellBurnFee <= 15, "Sell Fee should be 15% or less");
    }
