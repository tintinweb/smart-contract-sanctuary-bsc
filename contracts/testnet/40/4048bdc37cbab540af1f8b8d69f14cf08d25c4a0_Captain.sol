...
interface Cashier {
    ...
    function claimDividends(address hobo) external;
...
contract Captain is IERC20 {
    ...
    string constant private _name = "Captain";
    string constant private _symbol = "CAPT";
    ...
    struct TaxWallets {
        ...
        address payable oceanFund;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xd3604b056b666ab676Ee76f17CD3008877E24FDf),
        oceanFund: payable(0xa9FFea293dFB16a350DEfD353EeED74a0D9141B1)
        });
...
//===============================================================================================================
    ...
    function setRatios(uint16 rewards, uint16 oceanFund, uint16 marketing) external onlyOwner {
        uint16 check = rewards + oceanFund + marketing;
        require(check == 99 || check == 100);
        require(rewards <= 40 && marketing <= 40 && oceanFund <= 40);
        _ratios.rewards = rewards;
        _ratios.oceanFund = oceanFund;
        _ratios.marketing = marketing;
        _ratios.total = check;
    }

    function setWallets(address payable marketing, address payable oceanFund) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.oceanFund = payable(oceanFund);
    }
    ...
    function claimDividends() external {
        reflector.claimDividends(msg.sender);
    }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        uint256 amountBNBReflection = (amountBNB * _ratios.rewards) / _ratios.total;

        try reflector.load{value: amountBNBReflection}() {} catch {}

        if(address(this).balance > 0){
            _taxWallets.oceanFund.transfer((amountBNB * _ratios.oceanFund) / _ratios.total);
