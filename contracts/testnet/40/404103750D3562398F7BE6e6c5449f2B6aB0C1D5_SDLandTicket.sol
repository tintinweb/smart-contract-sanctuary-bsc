...
interface ITicket {
    ...
    function isTokenOwner( uint256 tokenId, address owner ) external view returns (bool);
...
//-----------------------------------
contract SDLandTicket is Ownable, ERC721Enumerable, ITicket {
    //--------------------------------------
    // 定数
    //--------------------------------------    
    string constant private TOKEN_NAME = "SkyDOS Land Box";
    string constant private TOKEN_SYMBOL = "SDLB";
    uint256 constant private INDEX_OFS = 1;
    uint256 constant private SERIAL_OFS = 1;
    ...
    function mintTokensTo( uint256 boxId, uint256 num, address to ) external override onlyMintable {
        ...
        for( uint256 i=0; i<num; i++ ){
            uint256 serial = _boxMinted[index] + i + SERIAL_OFS;
            ...
            // トークン作成
            _safeMint( to, tokenId );   // イベント[Transfer]が発火する
    ...
    function burnTokensFrom( uint256[] calldata tokenIds, address[] calldata froms ) external override onlyBurnable {
        ...
        for( uint256 i=0; i<tokenIds.length; i++ ){
            ...
            // トークン焼却
            _burn( tokenIds[i] );   // イベント[Transfer]が発火する
    ...
    function isTokenOwner( uint256 tokenId, address owner ) external view override returns (bool) {
        return( _exists( tokenId ) && owner == ownerOf(tokenId) );
    }
