...
contract ExercisecAFA {
    ...
    function exercise0(uint256 _amount) external returns (bool) {
        Term memory info = terms[msg.sender];
        require(redeemable(info) >= _amount, 'Not enough vested');
        require(info.max.sub(info.claimed) >= _amount, 'Claimed over max');

        uint256 value = tokenAmountTo(tokenUsdt, _amount);
        IERC20(tokenUsdt).safeTransferFrom(msg.sender, address(this), value);
        IAFA(cAFA).burnFrom(msg.sender, _amount);

        // IERC20(tokenUsdt).approve(treasury, value);
        // /////////////////////////////////////////////////
        // uint256 AFAToSend = ITreasury(treasury).deposit(value, tokenUsdt, 0);
    ...
    // Allows wallet to redeem cAFA for AFA
    function exercise1(uint256 _amount) external returns (bool) {
        Term memory info = terms[msg.sender];
        require(redeemable(info) >= _amount, 'Not enough vested');
        require(info.max.sub(info.claimed) >= _amount, 'Claimed over max');
    ...
    function exercise2(uint256 _amount) external returns (bool) {
        Term memory info = terms[msg.sender];
        require(redeemable(info) >= _amount, 'Not enough vested');
        require(info.max.sub(info.claimed) >= _amount, 'Claimed over max');
    ...
    function exercise3(uint256 _amount) external returns (bool) {
        Term memory info = terms[msg.sender];
        require(redeemable(info) >= _amount, 'Not enough vested');
        require(info.max.sub(info.claimed) >= _amount, 'Claimed over max');
    ...
    function tokenAmountTo(address _token, uint256 _amount) public view returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(_token)).decimals()).div(10**IERC20Metadata(cAFA).decimals());
    }
    ...
    function setTokenUsdt(address _token) external returns (bool) {
        require(msg.sender == owner, 'Sender is not owner');
    ...
    function redeemable(Term memory _info) internal view returns (uint256) {
        return IAFA(AFA).totalSupply().mul(rateMax).div(percentDenominator).mul(_info.percent).div(percentDenominator).sub(_info.claimed);
    }
