...
contract UserLevel is Ownable, IUserLevel {
    ...
    // ================= EXTERNAL FUNCTIONS ================= //
    function getBonus(address user, address _contract) external view override returns(uint256, uint256) {
        if(msg.sender == address(0) || boostLevel[_contract].level.length == 0){
            return(0, ONE_HUNDRED_PERCENT);
        }

        uint _levelUser = getUserLevel(user);
    ...
    function updateUserExp(uint exp, uint expiredTime, bytes[] memory signature) external override{
        require(msg.sender != address(0),"UserLevel: Address not zero");
        require(block.timestamp <= expiredTime, "UserLevel: Expired time");

        bytes32 _hash = _prefixed(keccak256(abi.encodePacked(exp, expiredTime, msg.sender, address(this), _useNonce(msg.sender))));
        address[] memory _validatorSignature = _getValidatorSignature(_hash, signature);
        ...
        userInfo.exp = _old + exp;
        userInfo.level = _calculatorLevel(_old + exp, userInfo.level, baseLevel);
    ...
    function configBonus(address contractAddress, uint[] memory _bonus, uint[] memory _level) external onlyOwner override{
        require(_level.length == _bonus.length, "UserLevel: length not equal");

        BonusInfo storage _info = boostLevel[contractAddress];
        ...
        emit ConfigBonus(contractAddress, _bonus, _level);
    }
    ...
    // ================= INTERNAL FUNCTIONS ================= //
    function _getValidatorSignature(bytes32 hash, bytes[] memory signature) internal view returns (address[] memory) {
        uint length = signature.length;
        address[] memory signer = new address[](length);
        for(uint i = 0; i < length; i++){
            signer[i] = ECDSA.recover(hash, signature[i]);
            console.log("_getValidatorSignature: ", signer[i]);
        }
        return signer;
    }
    ...
    //================ONLY TESTING=========================
    function setUserExp(address _user, uint _exp) public onlyOwner{
        user[_user].exp = _exp;
        user[_user].level = _calculatorLevel(_exp, 0, baseLevel);
    }

    function getListLevelBonus(address _contract) public view returns(uint[] memory level, uint[] memory bonus){
        BonusInfo memory info = boostLevel[_contract];
        level = info.level;
        bonus = info.bonus;
    }
}
