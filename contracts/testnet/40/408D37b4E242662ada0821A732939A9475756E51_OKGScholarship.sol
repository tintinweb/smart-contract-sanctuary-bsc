...
contract OKGScholarship is Initializable, IOKGScholarship {
	...
	function scholarship(
		...
		require(!createdScholarship[id].active, "Scholarship already existed");
		...
		for (uint256 i; i < _heroIds.length; i++) {
			...
			restrict.restrict(address(heroes), _heroIds[i]);
		}
...
interface IRestrictionV2 is IRestriction {
	function restrict(address _token, uint256 _tokenId) external;
