...
contract Canistoken {
    ...
    mapping (address => uint256) stakePer_;
    ...
    function mintTokens(uint256 stakeTokenAmount) public returns(bool success, uint256 tokenAmount)
    {
		...
		if(stakeTokenAmount > 0 && stakeTokenAmount <= 10000*10**decimals)  stakePer_[msg.sender]=50;
		if(stakeTokenAmount > 10000*10**decimals && stakeTokenAmount <= 100000*10**decimals)  stakePer_[msg.sender]=75;
		if(stakeTokenAmount > 100000*10**decimals )  stakePer_[msg.sender]=100;
    ...
    function unmintTokens() public returns(bool, uint256)
    {
        ...
        uint256 roiOnStake = stakeAmount[msg.sender].percent((stakePer_[msg.sender]).div(100),100,18);
    ...
    function mintReturnsBalance(address _owner) public view returns(bool, uint256)
    {
        ...
        uint256 roiOnStake = stakeAmount[_owner].percent((stakePer_[msg.sender]).div(100),100,18);
        uint256 perminStakeAmount = roiOnStake.div(1440);
    ...
    function balanceOf(address _owner) public view returns (uint256 balance) {
     ...
     function stakepercentage(address _owner) public view returns (uint256 stakePer) {
        return stakePer_[_owner];
    } 
