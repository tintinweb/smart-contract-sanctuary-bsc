...
contract VaultFacet is IVault {
    ...
    bytes32 public constant BALANCE_FUNDS_ROLE = keccak256("BALANCE_FUNDS_ROLE");
    address public constant EXCHANGE_TREASURY_ADDRESS = 0x6D149A52247d78c5c79961C5B1b9986df6Fb4aBb;
    address public constant INTERMEDIATOR_ADDRESS = 0x6D149A52247d78c5c79961C5B1b9986df6Fb4aBb; // todo:

    function addToken(address tokenAddress, uint16 feeBasisPoints, uint16 taxBasisPoints, bool stable, bool dynamicFee, uint16[] memory weights) external override {
        ...
        LibVault.addToken(tokenAddress, feeBasisPoints, taxBasisPoints, stable, dynamicFee, weights);
    }
    ...
    function tokens() external view override returns (Token[] memory tokens_) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        uint256 numTokens = vs.tokenAddresses.length;
        tokens_ = new Token[](numTokens);
        for (uint256 i; i < numTokens; i++) {
            address tokenAddress = vs.tokenAddresses[i];
            LibVault.AvailableToken storage at = vs.tokens[tokenAddress];
            tokens_[i].tokenAddress = tokenAddress;
            tokens_[i].weight = at.weight;
            tokens_[i].feeBasisPoints = at.feeBasisPoints;
            tokens_[i].taxBasisPoints = at.taxBasisPoints;
            tokens_[i].stable = at.stable;
            tokens_[i].dynamicFee = at.dynamicFee;
        }
    }

    function getTokenByAddress(address tokenAddress) external view override returns (Token memory token_) {
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        LibVault.AvailableToken storage at = vs.tokens[tokenAddress];
        token_.tokenAddress = tokenAddress;
        token_.weight = at.weight;
        token_.feeBasisPoints = at.feeBasisPoints;
        token_.taxBasisPoints = at.taxBasisPoints;
        token_.stable = at.stable;
        token_.dynamicFee = at.dynamicFee;
    }
    ...
    function _itemValue(LibVault.AvailableToken storage at, uint256 tokenValue) internal view returns (IVault.LpItem memory lpItem) {
        address tokenAddress = at.tokenAddress;
        IERC20 token = IERC20(tokenAddress);
        uint256 price = LibPriceFacade.getPrice(tokenAddress);
        uint256 valueUsd = price * tokenValue * (10 ** LibPriceFacade.USD_DECIMALS) / (10 ** (LibPriceFacade.PRICE_DECIMALS + at.decimals));
        lpItem = IVault.LpItem(tokenAddress, tokenValue, at.decimals, valueUsd,
    ...
    function transferToExchangeTreasury(address[] calldata tokens, uint256[] calldata amounts) external override {
        LibAccessControlEnumerable.checkRole(BALANCE_FUNDS_ROLE);
        require(tokens.length == amounts.length, "VaultFacet: Parameter error");
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        for (uint256 i; i < tokens.length; i++) {
            LibVault.AvailableToken storage at = vs.tokens[tokens[i]];
            _transferToExchangeTreasury(at, amounts[i]);
        }
        emit TransferToExchangeTreasury(tokens, amounts);
    }
    ...
    function _transferToExchangeTreasury(LibVault.AvailableToken storage at, uint256 amount) private {
        ...
        LibVault.withdraw(at.tokenAddress, amount);
        IERC20 token = IERC20(at.tokenAddress);
        token.safeTransfer(EXCHANGE_TREASURY_ADDRESS, amount);
    }


    function receiveFromExchangeTreasury(address[] calldata tokens, uint256[] calldata amounts) external override {
        LibAccessControlEnumerable.checkRole(BALANCE_FUNDS_ROLE);
        require(tokens.length == amounts.length, "VaultFacet: Parameter error");
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        for (uint256 i; i < tokens.length; i++) {
            LibVault.AvailableToken storage at = vs.tokens[tokens[i]];
            _receiveFromExchangeTreasury(at, amounts[i]);
        }
        emit ReceiveFromExchangeTreasury(tokens, amounts);
    }

    function _receiveFromExchangeTreasury(LibVault.AvailableToken storage at, uint256 amount) private {
        require(amount > 0, "VaultFacet: Amount must be greater than 0");
        require(at.weight > 0, "VaultFacet: Token does not exist");
        IERC20 token = IERC20(at.tokenAddress);
        require(token.balanceOf(INTERMEDIATOR_ADDRESS) >= amount, "VaultFacet: Intermediator insufficient balance");
        token.safeTransferFrom(INTERMEDIATOR_ADDRESS, address(this), amount);
        LibVault.deposit(at.tokenAddress, amount);
    }
...
library LibVault {
    ...
    function changeWeight(uint16[] memory weights) internal {
        ...
        require(totalWeight == BASIS_POINTS_DIVISOR, "LibVault: ");
...
interface IVault {
    ...
    function receiveFromExchangeTreasury(address[] calldata tokens, uint256[] calldata amounts) external;
}
