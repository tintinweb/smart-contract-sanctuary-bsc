...
contract PancakeFlashSwap {
    ...
    function startArbitrage(address _tokenBorrow, uint256 _amount) external {
        IERC20(BUSD).safeApprove(PANCAKE_ROUTER, MAX_INT);
        IERC20(CAKE).safeApprove(PANCAKE_ROUTER, MAX_INT);
        IERC20(CROX).safeApprove(PANCAKE_ROUTER, MAX_INT);
    ...
    function pancakeCall(
        ...
        // place Trades
        uint256 trade1AcquiedCoin = placeTrade(BUSD, CROX, loanAmount);
        uint256 trade2AcquiedCoin = placeTrade(CROX, CAKE, trade1AcquiedCoin);
        uint256 trade3AcquiedCoin = placeTrade(CAKE, BUSD, trade2AcquiedCoin);
