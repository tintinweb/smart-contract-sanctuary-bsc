...
import "./interfaces/IPrivatePool.sol";
...
contract CirclePrivatePool is OwnableUpgradeable, IPrivatePool,ReentrancyGuard {
    ...
    event ContributionWithdrawn(address indexed pool_address, uint256 amount, address user);

    event Claimed(address indexed pool_address, uint256 total_claimed, uint256 pending_claim, address user_address);

    event Finalized(address indexed pool_address, address user, uint256 bnb_fee, uint256 total_raised, uint256 actual_raised);

    event Cancelled(address indexed pool_address, uint256 timestamp);

    event PoolUpdated(address indexed pool_address, address user, string details);

    event WhitelistChanged(bool _whitelist);
    event PoolTier2Started(uint256 timestamp);
    event PoolPublicStartAtChanged(uint256 timestamp, uint256 start_at);
    event PoolTierChanged(uint256 timestamp, uint256 end_at, uint256 tier);


    event EmergencyWithdrawn(address indexed pool_address, address user, uint256 amount, address pair);

    event UsersWhitelisted(address indexed pool_address, address user, address[] added_users, uint256 total_users, uint256 tier);
    event UsersDelisted(address indexed pool_address, address user, address[] removed_users, uint256 total_users);

    modifier inProgress() {
        require(poolState == PoolState.inUse, "Pool is either completed or cancelled");

        if(useWhitelisting && publicStartTime > block.timestamp) {
            if(isWhitelisted[msg.sender]) {
                if(whitelistTier[msg.sender] == 2) {
                    require(block.timestamp >= startTime && block.timestamp > tier1.endTime && block.timestamp < tier2.endTime && block.timestamp < endTime, "It's not time to buy");
                } else {
                    require(block.timestamp >= startTime && block.timestamp < tier2.endTime  && block.timestamp < endTime, "It's not time to buy");
                }
            } else {
    ...
    receive() external payable {
        if (msg.value > 0) contribute(msg.value);
    }

    function initialize(
        LibPresale.PrivateSale memory presale,
        ...
        require(presale.minContribution <= presale.maxContribution, "Min contribution amount must be less than or equal to max");
        ...
        if(presale.tier2.endTime > 0) {
            require(presale.startTime < presale.tier2.endTime, "Presale Start time must be before Tier 2 end time");
            require(presale.tier1.endTime < presale.tier2.endTime, "Tier 1 end time must be before Tier 2 end time");
            require(presale.endTime >= presale.tier2.endTime, "Presale End time must be after Tier 2 end time");
        }
        ...
        maxContribution = presale.maxContribution;
        ...
        poolDetails = presale.poolDetails;
    ...
    function addWhitelistedUsers(address[] memory users, uint256 tier) external {
        for (uint256 i = 0; i < users.length; i++) {
            setWhitelist(users[i], true, tier);
        }
        emit UsersWhitelisted(address(this), msg.sender,  users, whitelistedUsers.length(), tier);
    }

    function removeWhitelistedUsers(address[] memory users) external {
        for (uint256 i = 0; i < users.length; i++) {
            setWhitelist(users[i], false, 0);
        }
        emit UsersDelisted(address(this), msg.sender,  users, whitelistedUsers.length());
    }

    function setWhitelist(address user, bool whitelisted, uint256 tier) internal onlyOperator {
        if(whitelisted) {
            require(tier == 1 || tier == 2, "Tier must be 1 (Tier 1) or 2 (Tier 2)");
        }
    ...
    function getPoolInfo() external override view returns (uint8[] memory , uint256[] memory , string memory , string memory){
        ...
        return (state , info , tokenName , poolDetails);
    }
    ...
    function contribute(uint256 _funds) public payable inProgress {
        ...
        if(paymentToken == address(0)) {
            ...
            IERC20(paymentToken).transferFrom(msg.sender, address(this), requiredFunds);
        }
...
//        IPoolFactory(factory).addTopPool(address(this), totalRaised);
        uint256 volume = requiredFunds;
//        require(volume > 0, "Contribution too small to produce any volume");
    ...
    function withdrawContribution() external nonReentrant {
        ...
        emit ContributionWithdrawn(address(this), refundAmount, msg.sender);
    }


    function claim() public nonReentrant onlyGovernance {
        ...
        if(paymentToken == address(0)) {
            payable(governance).sendValue(withdrawable);
        } else {
            IERC20(paymentToken).safeTransfer(governance, withdrawable);
        }
    ...
    function finalize() external onlyOperator nonReentrant {
        ...
        ( uint256 bnbFee,,, ) = PoolLibrary.calculateFeeAndLiquidity(
            totalRaised,
            ethFeePercent,
            tokenFeePercent,
    ...
    0,0
        ...
        if(bnbFee > 0){
            if(paymentToken == address(0)) {
                ...
                IERC20(paymentToken).safeTransfer(adminWallet, bnbFee);
            }
        }

        purchasedOf[msg.sender] = remainingBnb;
...
//        IPoolFactory(factory).removeTopPool(address(this));
        emit Finalized(address(this), msg.sender, bnbFee, hardCap, totalRaised);
    }
    ...
    function cancel() external onlyOperator {
        ...
        IPoolFactory(factory).removePrivatePoolForToken(tokenName, address(this));
//        IPoolFactory(factory).removeTopPool(address(this));
    ...
    function emergencyWithdraw(address payable to_, uint256 amount_) external override onlyOwner {
        if(paymentToken == address(0)) {
            ...
            IERC20(paymentToken).safeTransfer(to_, amount_);
        }
    }

    function updatePoolDetails(string memory details_) external onlyOperator {
        poolDetails = details_;
        emit PoolUpdated(address(this), msg.sender, details_);
    }
    ...
    function startPublicSaleNow() external onlyOperator inProgress {
        publicStartTime = block.timestamp;
        useWhitelisting = false;
        emit PoolPublicStartAtChanged(block.timestamp, publicStartTime);
    }

    function startTier2SaleNow() external onlyOperator inProgress {
        tier1.endTime = block.timestamp - 1;
        emit PoolTier2Started(block.timestamp);
    }

    function changeWhitelist(bool _whitelist) external onlyOperator {
        useWhitelisting = _whitelist;
        emit WhitelistChanged(_whitelist);
    }

    function changeTierDates(uint256 _tier, uint256 _endTime) external onlyOperator notInProgress {
        require (poolState == PoolState.inUse, "Pool was either finished or cancelled");
        if(_endTime >= 0) {
            require(endTime > _endTime && startTime < _endTime, "Invalid End Time");
        } else {
            require(_endTime == 0, "Invalid End Time");
        }
        if(_tier == 1) {
            tier1.endTime = _endTime;
            if(_endTime == 0) {
                tier2.endTime = _endTime;
            }
        } else {
            require(tier1.endTime < _endTime, "Invalid End Time");
            tier2.endTime = _endTime;
            _tier = 2;
        }
        emit PoolTierChanged(block.timestamp, _endTime, _tier);
    }


    function getTier(uint256 _tier) public view returns (LibTier.Tier memory) {
