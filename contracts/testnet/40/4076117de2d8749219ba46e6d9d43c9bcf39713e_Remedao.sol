...
contract Remedao is IERC20Metadata, Ownable, IRemedao {
    ...
    modifier onlyVoteContract() {
        require(msg.sender != address(voteContract));
        _;
    }
    ...
    function currentMemeEpoch() public view override returns (uint256) {
        return (block.timestamp - _memeStartTime).div(5 minutes).add(1);
    }
    ...
    function buyToken(uint256 epoch) external override onlyVoteContract {
    ...
    function buyMeme(uint256 epoch) internal {
        ...
        address _tokenAddress = voteContract.highestVote(epoch);
        isBoughtTokens[epoch] = true;
        if (_tokenAddress == address(0)) {
            return;
        }
        memeTokenAddress[epoch] = _tokenAddress;
        (, , , uint8 lqType) = voteContract.getVote(_tokenAddress, epoch);
        address[] memory path;
        ...
        returns (address, uint256)
    {
        ...
        uint256 total = memeTokenAmount[epoch];
        if (epoch <= memeTokenClaim[who] || total == 0) {
            return (memeTokenAddress[epoch], 0);
        }
        uint256 gonAtEpoch = gonOfAt(who, epoch);
        uint256 amountToken = total.div(TOTAL_GONS).mul(gonAtEpoch);
        ...
        returns (uint256)
    {
        ...
        return snapshotted ? value : gonOf(account);
    }
