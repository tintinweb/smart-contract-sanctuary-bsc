...
abstract contract GlobalsAndUtility is Ownable {
    ...
    function _globalsLoad(GlobalsCache memory g) internal view {
        ...
        if (globals.activeStakesTotal != 0) {
            g._averageLockPeriod = (activeStakes[2] * 2 + activeStakes[3] * 3 + activeStakes[4] * 4
            + activeStakes[5] * 5 + activeStakes[6] * 6 + activeStakes[7] * 7) / globals.activeStakesTotal;
        } else {
...
contract Staking is GlobalsAndUtility, Withdrawal, SelfDestructible, Stoppable {
    ...
    constructor(IERC20 _stakingToken) {
        ...
        // 3 * 1% 1/12
        _changeAPR(2, 0, APR_BASIS * 1 * 1e5);
        aprDivider[2] = 12 * 1e5;
        stakePeriods.push(2);
        // 3 * 1.5% 1/6
        _changeAPR(3, 0, APR_BASIS * 15e4);
        aprDivider[3] = 6 * 1e5;
        stakePeriods.push(3);
        // 3 * 1.9% 1/4
        _changeAPR(4, 0, APR_BASIS * 19e4);
        aprDivider[4] = 4 * 1e5;
        stakePeriods.push(4);
        // 3 * 3% 1/2
        _changeAPR(5, 0, APR_BASIS * 3 * 1e5);
        aprDivider[5] = 2 * 1e5;
        stakePeriods.push(5);
        // 3 * 5% 1
        _changeAPR(6, 0, APR_BASIS * 5 * 1e5);
        aprDivider[6] = 1 * 1e5;
        stakePeriods.push(6);
        // 3 * 12% 2
        _changeAPR(7, 0, APR_BASIS * 12 * 1e5);
        aprDivider[7] = 1 * 5e4;
        stakePeriods.push(7);
    }
