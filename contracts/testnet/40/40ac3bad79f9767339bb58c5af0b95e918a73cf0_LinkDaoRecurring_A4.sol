...
contract LinkDaoRecurring_A4 is Ownable {
    ...
    function getTotalProfitForInvestment(uint256 _investmentID) public view returns(uint256 totalProfit){
        ...
        totalProfit = totalInvestments.mul(ROI_PERCENTAGE).mul(timePeriod).div(REWARD_PERIOD.mul(TOTAL_PERCENTAGE));
    ...
    function getWithdrawableTotalProfitForInvestment(uint256 _investmentID) public view returns(uint256 totalProfit,uint256 amountToRelease){
        ...
        totalProfit = totalInvestments.mul(ROI_PERCENTAGE).mul(timePeriod.div(REWARD_PERIOD)).div(TOTAL_PERCENTAGE);
