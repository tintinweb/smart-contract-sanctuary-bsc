...
contract StakingService is PausableByOwner, RecoverableByOwner {
	...
	uint40 private constant DISTRIBUTION_START_TIME = 1671148801;
	...
	event Test(uint256 amount); /// @dev Token accrued to the staker
    ...
    constructor(
		...
		supplyState.tokensForDay = 1251 * 10**18;
		supplyState.time = DISTRIBUTION_START_TIME;
		state.totalTokenSupply = _makeProgress(uint40(block.timestamp));
	...
	function airdrop(
		...
		for (uint256 i = 0; i < to.length; i++) {
			...
			require(transferred, "StakingService: CVR_FAILED_AIRDROP");
		}
	}
	...
	function tokenSupplyAmount() public {
        require(_msgSender() == tokenKeeper || _msgSender() == owner(), "TOKEN: no access");
		updateHistoricalRewardRate();		
	}
	...
	function _makeProgress(
		...
		if (theDay == supplyState.sDay) {
			tokenSupply = (maxTime - supplyState.time) * (supplyState.tokensForDay / DAY);
			supplyState.time = maxTime;
			return tokenSupply;
		}
		
		uint40 restOfSDay = DISTRIBUTION_START_TIME + (supplyState.sDay * DAY) - supplyState.time;		
		tokenSupply += restOfSDay * supplyState.tokensForDay / DAY;
		...
		uint40 restOfNewDay = maxTime - DISTRIBUTION_START_TIME - (theDay - 1) * DAY;
		tokSup = _percentState(theDay).mulu(tokSup);
		tokenSupply += tokSup * restOfNewDay / DAY;
	...
	function _percentState(uint16 sday) private view returns (int128 percent) {
		...
		uint16 startWeek = (sday - 1) % 7;
		if (week == 1 && startWeek == 0) {
			return supplyPercent_01;
		} else if (week >= 2 && week <= 4 && startWeek == 0) {
			return supplyPercent_02_04;
		} else if (week >= 5 && week <= 50 && startWeek == 0) {
			return supplyPercent_05_50;
		} else if (week >= 51 && week <= 134 && startWeek == 0) {
			return supplyPercent_51_134;
		} else if (week >= 135 && week <= 200 && startWeek == 0) {
			return supplyPercent_135_200;
		} else if (week >= 201 && week <= 429 && startWeek == 0) {
			return supplyPercent_201_429;
		} else if (week >= 430 && week <= 755 && startWeek == 0) {
			return supplyPercent_430_755;
		} else if (week >= 756 && week <= 1557 && startWeek == 0) {
			return supplyPercent_756_1557;
		} else if (week >= 1558 && week <= 2200 && startWeek == 0) {
			return supplyPercent_1558_2200;
		} else if (week >= 2201 && week <= 3274 && startWeek == 0) {
			return supplyPercent_2201_3274;
		} else if (week >= 3275 && week <= 3728 && startWeek == 0) {
			return supplyPercent_3275_3728;
		} else {
			return ABDKMath64x64.divu(1, 1);
		}		

	}

    function updateHistoricalRewardRate() public {
        uint256 currentTokenSupply = _makeProgress(uint40(block.timestamp));
