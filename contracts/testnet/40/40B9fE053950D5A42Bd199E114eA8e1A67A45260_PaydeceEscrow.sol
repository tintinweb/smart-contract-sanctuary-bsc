...
contract PaydeceEscrow is  ReentrancyGuard , Ownable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint256 _value,address _currency) external virtual   {
        require(escrows[_orderId].status == EscrowStatus.Unknown, "Escrow already exists");

        uint8 _decimals = IERC20(_currency).decimals();
        ...
        //Valida el Allowance
        require( IERC20(_currency).allowance( _buyer, address(this)) >= (_value+_amountFeeBuyer),"Seller approve to Escrow first");
    
        //TODO: validar la cantidad que envia el comprador, deberia ser el valor del asset + el buyerFee

        //Transfer USDT to contract
        IERC20(_currency).transferFrom(_buyer, address(this) , (_value+_amountFeeBuyer) );
    ...
    function _releaseEscrow (uint _orderId) private nonReentrant{
        ...
        uint8 _decimals = IERC20(escrows[_orderId].currency).decimals();         
        ...
        feesAvailable[IERC20(escrows[_orderId].currency)] += _amountFeeBuyer + _amountFeeSeller;
        ...
        IERC20(escrows[_orderId].currency).transfer( escrows[_orderId].seller, escrows[_orderId].value - _amountFeeSeller);   
    ...
    function refundBuyer(uint _orderId) external nonReentrant onlyOwner {       
        ...
        IERC20(escrows[_orderId].currency).transfer(escrows[_orderId].buyer, escrows[_orderId].value);
    ...
    */
    ...
    function withdrawFees(IERC20 _currency) external onlyOwner {        
        ...
        _currency.transfer(owner(), _amount);
