...
contract Payment is Ownable, ReentrancyGuard {
    ...
    // Event triggered when payToken[USDT] is deposited in contract
    event Deposit(string _invId, uint256 _amt);
    ...
    uint256 public totalPayTokenBal = 0;

    // bondInfo -> Information about the bond
    // BondInfo public bondInfo;

    // user address => tierId => tokensBought
    mapping(string => uint256) public invAmntInfo;

    // Setting up the Bond for the first time on the contract
    // Parameters are the struct parameters for BondInfo
    // Can only be called by owner
    function payForInv(string memory _invId, uint256 _amt) external {
        require(_amt > 0, "Invalid amount");
        IERC20(payToken).transferFrom(msg.sender, address(this), _amt);
        invAmntInfo[_invId] = invAmntInfo[_invId].add(_amt);
        totalPayTokenBal = totalPayTokenBal.add(_amt);
        emit Deposit(_invId, _amt);
    }

    // Updating the Bond
    // Can only activate or deactivate the bond and change minimum deposit
    // Can only be called by owner
    function withdrawPayTokenBalance() external onlyOwner {
        require(totalPayTokenBal > 0, "Nothing to withdraw");
        IERC20(payToken).transfer(
            msg.sender,
            totalPayTokenBal
        );
        emit Withdraw(totalPayTokenBal);
        totalPayTokenBal = 0;
    }
}
