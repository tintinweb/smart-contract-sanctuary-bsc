...
import {Sale} from "./contracts/Sale.sol";
...
contract ProjectLaunchFactory {
  ...
  address _owner;
  address internal adminAddress = 0x33F3ed84725FdbF32Ea78F3a576c0c35a92b7a7e;
  address internal blacklistContract = 0x9844568F3E376Fc0363E634D7D54D1EEa1969440;
  address internal couponContract = 0x7ea295C5A7C4a903B92FFF1786ED0f8F4bE87237;
  address internal vaultContract;
  address internal feeTo;

  address internal _address = msg.sender;
  address internal _contract = address(this);

  // Base launch fee
  mapping(uint256 => uint256) internal launchFee;
  ...
  Sale internal sale;
  address internal saleVault;

  uint256 internal convert = 10 ** 18;

  modifier enoughFee(uint256 _option) {
  ...
  function setRoundTokenMultiplier(uint _option, uint256 _round1MultT, uint256 _round2MultT, uint256 _round3MultT) external onlyOwner {
  ...
  function createSale(
  ...
  ) external payable enoughFee(_sale_details[0]) returns (address, address) {
    ...
    _sale_details[5] = launchFee[_sale_details[0]];
    _sale_details[6] = iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]);
    
    if (iCoupon(couponContract).getCouponActive(_address,_sale_details[7]) == 1) {
      _seed_details[9] = round1Mult[_sale_details[0]] + ((10000 - round1Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      _seed_details[10] = round1MultT[_sale_details[0]] + ((10000 - round1MultT[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      _presale_details[9] = round2Mult[_sale_details[0]] + ((10000 - round2Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      _presale_details[10] = round2MultT[_sale_details[0]] + ((10000 - round2MultT[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      _community_details[9] = round3Mult[_sale_details[0]] + ((10000 - round3Mult[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
      _community_details[10] = round3MultT[_sale_details[0]] + ((10000 - round3MultT[_sale_details[0]]) * iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7]) / 100) ;
    } else {
      _seed_details[9] = round1Mult[_sale_details[0]];
      _seed_details[10] = round1MultT[_sale_details[0]];
      _presale_details[9] = round2Mult[_sale_details[0]];
      _presale_details[10] = round2MultT[_sale_details[0]];
      _community_details[9] = round3Mult[_sale_details[0]];
      _community_details[10] = round3MultT[_sale_details[0]];
    }

    saleVault = (_sale_details[8] == 0 ? address(0) : iSaleVaultFactory(vaultContract).createVault(msg.sender, _sale_token));
    ...
    if(_sale_details[8] > 0) {
        iSaleVault(saleVault).setSaleContract(address(sale));
    }

    if (iCoupon(couponContract).getCouponActive(_address,_sale_details[7]) == 1) {
      payable(feeTo).transfer((launchFee[_sale_details[0]])*(100-((iCoupon(couponContract).getCouponDiscount(_address,_sale_details[7])))/100));
    } else {
      payable(feeTo).transfer(launchFee[_sale_details[0]]);
    }
    
    uint256 totalTokens = 0;
    totalTokens = totalTokens + (_seed_details[4] * _seed_details[0] / convert) + ((10000 - round1MultT[_sale_details[0]]) * (_seed_details[4] * _seed_details[0] / convert) / 10000);
    totalTokens = totalTokens + (_presale_details[4] * _presale_details[0] / convert) + ((10000 - round2MultT[_sale_details[0]]) * (_presale_details[4] * _presale_details[0] / convert) / 10000);
    totalTokens = totalTokens + (_community_details[4] * _community_details[0] / convert) + ((10000 - round3MultT[_sale_details[0]]) * (_community_details[4] * _community_details[0] / convert) / 10000);
    ...
    ERC20(_sale_token).transferFrom(msg.sender, address(sale), totalTokens + totalLiquidityTokens);

    if(_sale_details[7] > 0) {
      iCoupon(couponContract).useCoupon(_address,_sale_details[7]);
    }

    return (address(sale), address(saleVault));
  }
  ...
  function setVaultContract(address _vaultAddress) public onlyOwner {
    vaultContract = _vaultAddress;
  }
...
contract Sale is ReentrancyGuard {
    struct SaleDetails {
        address sale_token;
        uint256 canceled;
        uint256 hasSeedRound;
        uint256 hasPresaleRound;
        uint256 hasCommunityRound;
        uint256 listRate;
        uint256 saleOption;
        uint256 hasVesting;
        uint256 launchFee;
    ...
    address internal feeAddress = 0x33F3ed84725FdbF32Ea78F3a576c0c35a92b7a7e;
    address internal blacklistContract = 0x9844568F3E376Fc0363E634D7D54D1EEa1969440;
    address internal factoryContract;
    address internal vaultContract;
    address internal saleVaultContract;
    ...
    uint256 internal saleSetting;
    uint256 internal ownerWithdraw = 0;
    uint256 convert = 10 ** 18;
    ...
    constructor(
        ...
        saleVaultContract = _sale_vault;

        adminAddress = iSaleFactory(msg.sender).getAdmin();
    ...
    function init_sale (
        ...
        sale_details.hasLiquidity = _sale_details[8];
    ...
    function setSaleDetails(uint256 _sale_round, uint256[11] memory _sale_input) private {
        ...
        saleInfo.liquidityTokens = saleInfo.hardcap * saleInfo.mult / 10000 * sale_details.listRate / convert * saleInfo.liquidity / 100;
    }
    ...
    function userDeposit (uint256 _round) public payable nonReentrant {
        ...
        require(saleStatus.sale_raised_amount * token_rate <= ERC20(sale_details.sale_token).balanceOf(address(this)), "Token remain error");
            ...
            if(block.timestamp > sale_info[_round].end) {
                ...
                if(sale_details.hasVesting == 0) {
                    ...
                    if(block.timestamp >= cliffDate){
                        ...
                        claim_amount = (buyer.sale * (sale_vesting[_round].period_amount * ((block.timestamp - cliffDate) / sale_vesting[_round].period)) / 100) - buyer.claimed;
    ...
    function finalizeSale (uint8 _round) public nonReentrant {
        require(msg.sender == owner || msg.sender == adminAddress, "Not autrhoized: final");
        ...
        if(sale_info[_round].end > 0) {
            if(block.timestamp > sale_info[_round].end) {
                ...
                uint256 feeAmount = ((sale_status[_round].sale_raised_amount * (10000 - sale_info[_round].mult)) / 10000);
                uint256 liquidityAmount = (sale_status[_round].sale_raised_amount * sale_info[_round].liquidity / 100);
                ...
                if(liquidityAmount > 0) {
                    ERC20(sale_details.sale_token).transfer(address(saleVaultContract), liquidityToken);
                    iSaleVault(saleVaultContract).createLock{value:liquidityAmount}(_round, sale_details.sale_token, liquidityToken, liquidityAmount, sale_info[_round].end, sale_info[_round].end + (sale_info[_round].liquidityLock * 1 minutes));
                }

                // Owner payment
                payable(msg.sender).transfer(sale_status[_round].sale_raised_amount - feeAmount - liquidityAmount);
            }
        }
    }
    ...
    function getSaleStatusInfo(uint256 _round) public view returns (uint256 force_failed,uint256 sale_raised_amount,uint256 sale_sold_amount,uint256 sale_token_withdraw,uint256 sale_base_withdraw,uint256 sale_num_buyers) {
        ...
        return(saleStatus.force_failed,saleStatus.sale_raised_amount,saleStatus.sale_sold_amount,saleStatus.sale_token_withdraw,saleStatus.sale_base_withdraw,saleStatus.sale_num_buyers);
    }
    ...
    function changeAdmin () public onlyOwner {
        adminAddress = iSaleFactory(msg.sender).getAdmin();
    } 
    ...
    function changeVestingAdmin(uint256 _round, uint256[] memory _vesting) public {
        require(msg.sender == adminAddress, "You are not the admin!");
        require(sale_vesting[_round].TGE > sale_info[_round].end, "Vesting too early");
        require(sale_vesting[_round].cliff >= sale_vesting[_round].TGE || sale_vesting[_round].cliff == 0, "Cliff is less than TGE");
    ...
    function restartRound(uint256 _round, uint256 _start, uint256 _end) public payable onlyOwner {
        ...
        payable(adminAddress).transfer(restartFee);
