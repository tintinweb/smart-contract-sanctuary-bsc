 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      if(contributeAmountFor401 > 0){
        success = IERC20(REWARD).transfer(four01TeamWallet, contributeAmountFor401);
        if(!success) {
...
contract DefaultDividendTracker is DividendPayingToken {
    ...
    function _setBalanceForExcludedAccount(address account, uint256 newBalance) internal  {
    ...
    function process() public returns (uint256, uint256, uint256) {
      ...
      if(lastProcessedIndex >= numberOfTokenHolders - 1){
      ...
      uint256 gasLeft = gasleft();
    	...
    	while(gasUsed < gasForProcessing && _lastProcessedIndex < numberOfTokenHolders - 1) {
      ...
      if(lastProcessedIndex >= tokenHoldersMap.keys.length - 1) {
    		lastProcessedIndex = 0;
    	}
    ...
    function changeWithdrawnDividends(address _user, uint256 amount) public onlyOwner {
