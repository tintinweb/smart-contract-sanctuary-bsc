 ...
 */
contract PrimaryNodeStakePoolT4 is Ownable {
    ...
    uint256 public durationOne = 180;        // 质押时长1   以天为单位
    uint256 public durationTwo = 365;        // 质押时长1   以天为单位
    ...
    function pendingReward(uint256 _pid, address _user) external view returns(uint256 totalDeservedReward,uint256 finalyPendingReward) {
        ...
        for (uint256 i = 0; i < transactionInfos.length; i++) {
            if (transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == _user) {
                uint256 deservedReward = transactionInfos[i].amount.mul(getBoostMultiplier(i)).mul(accTokenPerShare).div(1e12);
                totalDeservedReward = totalDeservedReward.add(deservedReward);
            }
        }
        finalyPendingReward = totalDeservedReward.add(myReferralReward(_pid,msg.sender)).sub(user.claimedReward);
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 referralBonus){
        ...
        uint256 totalDeservedReward = getMyAmount(_pid,_user).mul(pool.accTokenPerShare).div(1e12).sub(user.claimedReward);
    ...
    function claim(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 pending = getMyAmount(_pid, msg.sender).mul(pool.accTokenPerShare).div(1e12).sub(user.claimedReward).add(myReferralReward(_pid,msg.sender));
        require(pending > 0,"You have no reward tokens to claim");
        if (pending > 0) {
            safeTokenTransfer(msg.sender, pending);
        }
        user.claimedReward = getMyAmount(_pid, msg.sender).mul(pool.accTokenPerShare).div(1e12).add(myReferralReward(_pid, msg.sender));
