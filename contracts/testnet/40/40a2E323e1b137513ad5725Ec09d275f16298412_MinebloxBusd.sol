...
contract MinebloxBusd is Context, Ownable , ReentrancyGuard {
    ...
    event Deposit (address indexed _by, uint amount);
    event Claim (address indexed _by, uint amount);
    event Withdraw (address indexed _by);
    event UnStake (address indexed _by);
    event WithdrawReferral (address indexed _by, uint amount);


    constructor() {
    tokenAdress = 0xBc2041C18BB361A1756FF16bc5aba75aE0d02EF3; 
    BusdInterface = IERC20(tokenAdress);
    }

    // invest function 
    function deposit(address _ref, uint256 _amount) public payable noReentrant  {
    ...
    function claimDailyRewards() public noReentrant{
        ...
        emit Claim (msg.sender, totalRewardAmount);
    ...
    function Ref_Withdraw() external noReentrant {
        ...
        emit WithdrawReferral (msg.sender, totalValue);
    }
