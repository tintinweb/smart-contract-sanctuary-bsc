...
interface IDao7BetFree{
    ...
    function getTokenMinAmount(address key) external view returns (uint256);
    function getTokenFee(address key) external view returns (uint256);
    function getTokenRebate1(address key) external view returns (uint256);
    function getTokenRebate2(address key) external view returns (uint256);
    function getTokenEnabed(address key) external view returns (bool);
}
...
interface IDao7Users{
    ...
    function registerOfBet(address key,uint256 inviteCode,address mainAddress) external;
}

interface IDao7TopicFree{
    function initialize(address _owner,address _mainContract,address _userAddress) external;
}

contract Dao7TopicFree {
    // version
    uint public version=1;
    // main contract
    IDao7BetFree mainContract;
    ...
    function initialize(address _owner,address _mainContract,address _userAddress) external{
        require(msg.sender == factory, 'Dao7: FORBIDDEN'); // sufficient check
        owner = _owner;
        mainContract = IDao7BetFree(_mainContract);
    ...
    function _bet(address token,address userAddress,uint256 _amount,uint256 optionId,uint256 inviteCode) private{
        bool tokenInserted = mainContract.tokenInserted(token);
        require(tokenInserted,"token not exists");
        bool tokenEnabled = mainContract.getTokenEnabed(token);
        require(tokenEnabled,"token not enabled");
        uint256 minAmount = mainContract.getTokenMinAmount(token);
        require(_amount >= minAmount,"bet amount too low");
        
        uint256 fee = mainContract.getTokenFee(token);
        uint256 rebate1 = mainContract.getTokenRebate1(token);
        uint256 rebate2 = mainContract.getTokenRebate2(token);
        if(!userContract.userInserted(userAddress)){
            userContract.registerOfBet(userAddress, inviteCode,mainContractAddress);
        }
        if(optionId == 1){
    ...
    function getDynamicOdds(address token,uint256 option) public view returns(uint256 odds,uint256 sumAmount,uint256 trueOptionAmount){
        ...
        uint256 tokenFee = mainContract.getTokenFee(token);
        trueOptionAmount = 0;
    ...
    function openBet(uint256 _trueOption) public{
        ...
        for(uint256 i = 0; i < mainContract.tokensLength(); i++){
            address token = mainContract.getTokenKeyAtIndex(i);
            uint256 tokenFee = mainContract.getTokenFee(token);
...
contract Dao7Factory{

    function createTopic(address _owner,address _mainContract,address _userAddress) external returns (address topic) {
        bytes memory bytecode = type(Dao7TopicFree).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_owner, _mainContract));
        assembly {
            topic := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IDao7TopicFree(topic).initialize(_owner,_mainContract,_userAddress);
    }
