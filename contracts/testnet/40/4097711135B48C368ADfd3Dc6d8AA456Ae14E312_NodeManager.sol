...
contract NodeManager is Initializable {
  ...
  function initialize(address _feeManager, address _nftAddress) public initializer {
    ...
    addTier('basic', 10 ether, 0.13 ether, 1 days, 5 ether, 100);
    addTier('light', 50 ether, 0.80 ether, 1 days, 15 ether, 20);
    addTier('pro', 100 ether, 2 ether, 1 days, 25 ether, 10);
  ...
  function burnNodes(uint256[] memory indice) public onlyOwner {
    uint32 count = 0;
  ...
  function swapIn(uint32 _chainId, string memory _tierName, uint32 _amount, bytes memory _sig) public payable {
    bytes32 r;
    bytes32 s;
    uint8 v;
    assembly {
    ...
    (uint256 claimableAmount,uint32 count,uint256[] memory nodeIndice) = _iterate(msg.sender, tierIndex, _amount);
    require(count==_amount, 'Insufficient node amount.');
    int32 limitedTime = 0;
    uint256[] storage userNodeIndice = nodesOfUser[msg.sender];
    for(uint32 i = 0;i<count;i++) {
      uint256 index = nodeIndice[i];
      Node storage node = nodesTotal[index - 1];
      node.claimedTime = uint32(block.timestamp);
      for (uint32 j = 0; j < userNodeIndice.length; j++) {
        if (index == userNodeIndice[j]) {
          userNodeIndice[j] = 0;
          break;
        }
      }
