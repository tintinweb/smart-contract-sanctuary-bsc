...
abstract contract Whitelist {
    ...
    function _verify(address _to, uint256 _amount, uint256 _price, bytes memory _signature, address signer) public view returns (address) {
        bytes32 messageHash = _getMessageHash(_to, _amount, _price);
        bytes32 ethSignedMessageHash = _getEthSignedMessageHash(messageHash);
        return _recoverSigner(ethSignedMessageHash, _signature);
    }
...
// File: LightNonFungibleToken.sol
...
contract LightNonFungibleToken is ERC721URIStorage, Ownable, Pausable, Whitelist {
    ...
    function whitelistMint(uint256 _maxAmount, uint256 _price, uint256 _amount, bytes memory _signature) external payable whenNotPaused {
        // require(_verify(msg.sender, _maxAmount, _price, _signature, _signer), "invalid arguments or not whitelisted");
