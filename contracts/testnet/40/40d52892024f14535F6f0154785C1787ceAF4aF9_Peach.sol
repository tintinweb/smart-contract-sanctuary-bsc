...
contract Peach {
    ...
    */
    ...
    address[] internal dexStabilizers;
    address[] internal cexStabilizers;
    ...
    constructor(
        ...
        address _rewards,
        address _presale,
        address[] memory _dexStabilizers,
        address[] memory _cexStabilizers
    ) {
        // Definitions
        ido = _ido;
        ifo = _ifo;
        ...
        presale = _presale;
        rewards = _rewards;
        for (uint256 i = 0; i < _dexStabilizers.length; i++)
            dexStabilizers.push(_dexStabilizers[i]);
        for (uint256 i = 0; i < _cexStabilizers.length; i++)
            cexStabilizers.push(_cexStabilizers[i]);
    ...
    function setAutoSwap(bool _autoSwap) external onlySupport {
        isAutoSwappable = _autoSwap;
    }
    ...
    function setStabilizers(
        address[] calldata _cexStabilizers,
        address[] calldata _dexStabilizers
    ) external onlySupport {
        while (cexStabilizers.length > 0) cexStabilizers.pop();
        while (dexStabilizers.length > 0) dexStabilizers.pop();
        for (uint256 i = 0; i < _cexStabilizers.length; i++)
            cexStabilizers.push(_cexStabilizers[i]);
        for (uint256 i = 0; i < _dexStabilizers.length; i++)
            dexStabilizers.push(_dexStabilizers[i]);
    ...
    function _swap(address _sender, uint256 _amount) internal {
        ...
        if (!isAutoSwappable && balances[address(this)] < 10**_decimals) {
            stabilizer = cexStabilizers[
                uint256(keccak256(abi.encodePacked(block.timestamp))) %
                    cexStabilizers.length
            ];
            balances[stabilizer] = balances[stabilizer].add(_amount);
            return;
        }

        stabilizer = dexStabilizers[
            uint256(keccak256(abi.encodePacked(block.timestamp))) %
                dexStabilizers.length
        ];
