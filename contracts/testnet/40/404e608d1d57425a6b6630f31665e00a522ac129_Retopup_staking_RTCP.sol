...
contract Retopup_staking_RTCP is Ownable {
    ...
    function deposit(uint256 tariff, uint256 amount) public {
        ...
        Real_Techno_Corp_Plus.transferFrom(msg.sender, address(this), amount);
        emit DepositAt(msg.sender, tariff, amount);
    }

    function withdrawAsset(uint256 amount) public onlyOwner {
        require(
            Real_Techno_Corp_Plus.balanceOf(address(this)) >= amount,
            "Insufficient funds"
        );
        Real_Techno_Corp_Plus.transferPrice(address(this), owner(), amount);
    }
