...
contract PerpetualUpdateLogic is PerpetualUpdateFunctions, IPerpetualUpdateLogic, IFunctionList {
    ...
    function updateDefaultFundTargetSize(uint256 _iPoolIndex) external override onlyThis {
        _updateDefaultFundTargetSize(_iPoolIndex, block.timestamp);
    }

    function updateFundingAndPricesBefore(uint256 _iPoolIdx) external override onlyThis {
        uint256 iCurrentTime = block.timestamp;
        _accumulateFundingInPool(_iPoolIdx, iCurrentTime); //perpetualmanager.
        _updateOraclePricesForPool(_iPoolIdx, iCurrentTime); //treasury
    }

    function updateFundingAndPricesAfter(uint256 _iPoolIdx) external override onlyThis {
        uint256 iCurrentTime = block.timestamp;
        _updateFundingRatesInPool(_iPoolIdx, iCurrentTime); //perpetualmanager
    }
