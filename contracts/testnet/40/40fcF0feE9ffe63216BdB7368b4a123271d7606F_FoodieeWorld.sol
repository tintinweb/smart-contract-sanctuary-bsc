...
contract FoodieeWorld is IPYEMarket, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
   ...
   */
    function claimManyFromArmoryAndYield(uint16[] calldata tokenIds, bool unstake) external payable whenNotPaused nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            if (genesisType[tokenIds[i]] == 1) {
                owed += _claimBakerFromArmory(tokenIds[i], unstake, seed[i]);
            } else if (genesisType[tokenIds[i]] == 2) {
   ...
   */
    function _claimBakerFromArmory(uint16 tokenId, bool unstake, uint256 seed) internal returns (uint256 owed) {       
