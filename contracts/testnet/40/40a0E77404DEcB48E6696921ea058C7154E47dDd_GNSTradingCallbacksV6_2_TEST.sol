...
*/
    ...
    function executeNftCloseOrderCallback(
        ...
        emit nftOnCallbacksCLOSE(true);
    }

    event nftOnCallbacksOPEN(bool status);
    event nftOnCallbacksCLOSE(bool status);
    ...
    function registerTrade(
        ...
        StorageInterfaceV5.TradeInfo memory tradeInfo = StorageInterfaceV5.TradeInfo(
            0,
            v.tokenPriceDai,
        ...
        openPriceTEST = trade.openPrice;

        savedTrade = trade;
        savedTradeInfo = tradeInfo;
        ...
        emit BoolAfterStoreTrade(true);

        emit TradeAfterStoreTrade(trade);

        emit TradeInfoAfterStoreTrade(tradeInfo);

        emit BoolBeforeStoreTradeReturn(true);

        return (trade, v.tokenPriceDai); // ESTE ES EL REAL
    }

    uint public openPriceTEST;

    StorageInterfaceV5.Trade public savedTrade;

    StorageInterfaceV5.TradeInfo public savedTradeInfo;

    function manuallyStoreTrade() public {
        storageT.storeTrade(savedTrade, savedTradeInfo);
    }

    event BoolAfterStoreTrade(bool isTrue);

    event TradeAfterStoreTrade(StorageInterfaceV5.Trade trade);

    event TradeInfoAfterStoreTrade(
        StorageInterfaceV5.TradeInfo tradeinfo
    );

    event BoolBeforeStoreTradeReturn(bool isTrue);
