...
contract AntiDumpLiquidContract is IERC20 {
    using SafeMath for uint256;  
   
    mapping(address => uint256) private _balances; 
    mapping (address => mapping (address => uint256)) private _allowances;

    string private _name = "ANTIDUMP_AUTOPAY";  
    string private _symbol =  "ADAPY";   
    ...
    function _transfer(address sender, address recipient,  uint256 amount) private swapMutexLock returns (bool) { 
        ...
        if (block.timestamp > (_accountDailyTxn[sender].txnTimeSecs.add(maxTransferTimeSecs))) {
            ...
            if(!(maxDailyWalletTransferPercent >= 100 || maxAmtAllowed >= amount))  { revert(); }
    ...
    function _baseTransfer(address from, address to, uint256 amount, uint isHasFees, bool is_buy) private returns (bool) {      
        ...
        if(!_isFeesExempted(from) && isHasFees > 0) {   
           ...
           uint256 amountLessFee = amount.sub(taxAmount);
