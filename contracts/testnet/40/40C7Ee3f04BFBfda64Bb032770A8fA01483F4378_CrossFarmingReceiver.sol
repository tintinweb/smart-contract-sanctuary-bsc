...
contract CrossFarmingReceiver is MessageSenderApp, MessageReceiverApp {
    ...
    // the operator for fallbackwithdraw.
    ...
    function executeMessage(
        ...
        if (request.msgType == DataTypes.MessageTypes.Deposit) {
            ...
            (bool success, ) = _sender.call{value: IMessageBus(messageBus).calcFee(_message)}(
...
contract CrossFarmingProxy is ReentrancyGuard {
    ...
    /// @dev only called when user deposit success on EVM chain but failed on BSC chain.
    ...
    /// fallback withdraw token deposit in masterchefv2
    /// @dev only called when user withdraw/emergencywithdraw success on EVM chain but failed on BSC chain
