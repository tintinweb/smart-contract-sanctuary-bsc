...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, bytes memory _signature) external {
    ...
    if (firstConsumeInDay || energy >= energyConsumption) {
      (userReward, dailyQuestCompleted) = _giveReward(_mission, firstConsumeInDay);
  ...
  function _giveReward(uint _mission, bool _firstConsumeInDay) private returns (uint, bool) {
    uint userReward;
    bool dailyQuestCompleted;
  ...
  function _calculateReward(uint _mission, bool _firstConsumeInDay) private returns (uint) {
    bool firstCompleteMission = _mission == biggestFinishMission[msg.sender];
  ...
  function _validateFinishGameBypassSignature(uint _mission, uint[] calldata _heroIds) private {
    require(_heroIds.length >= 3 && _heroIds.length <= 5, "PVE: HeroIds is invalid");
    for (uint i = 0; i < _heroIds.length - 1; i++) {
      require(_heroIds[i] < _heroIds[i + 1], "PVE: hero id is duplicated or wrong order");
    }
