...
contract MountSapphire is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    struct RewardToken {
        ...
        uint256 currentRewardPerDay;
        uint256 oldRewardPerDay;
        uint256 startTime;
        bool enabled;
    }

    uint256 public totalPowerInPool;
    ...
    mapping(uint256 => uint256) public blucamonPower;
    uint256[] public powerList;
    uint256[] public tokenPerDayList;

    event SetRewardToken(address _tokenAddress);
    ...
    function setOwner(address address_) external onlyOwner {
        require(address_ != address(0), "S_MTS_100");
    ...
    function setCondition(
        ...
        require(_tokenPerDayList.length - _powerList.length == 1, "S_MTS_200");
    ...
    function _updateTokenRewardPerDay() internal {
        ...
        rewardToken.oldRewardPerDay = rewardToken.currentRewardPerDay;
        for (uint256 index = 0; index < powerList.length; index++) {
            if (totalPower <= powerList[index]) {
                rewardToken.currentRewardPerDay = tokenPerDayList[index];
                return;
            }
        }
        rewardToken.currentRewardPerDay = tokenPerDayList[
            tokenPerDayList.length - 1
        ];
    }
    ...
    function getListBlucamonPower(uint256[] memory tokenIds)
        public
        view
    ...
    function getRewardToken() public view returns (address, uint256) {
        return (rewardToken.tokenAddress, rewardToken.currentRewardPerDay);
    }
    ...
    function setRewardToken(address _tokenAddress) public onlyOwner {
        require(_tokenAddress != address(0), "S_MTS_300");
        rewardToken.tokenAddress = _tokenAddress;
        rewardToken.startTime = block.timestamp;
        rewardToken.enabled = true;
        emit SetRewardToken(_tokenAddress);
    }

    function disableTokenReward() public onlyOwner {
        require(rewardToken.enabled, "S_MTS_400");
        rewardToken.enabled = false;
        emit DisableRewardToken();
    }

    function enableTokenReward() public onlyOwner {
        require(!rewardToken.enabled, "S_MTS_401");
        rewardToken.enabled = true;
    ...
    function getPendingReward(address _user) external view returns (uint256) {
        uint256 reward = 0;
        if (rewardToken.enabled == true) {
            if (totalPowerInPool == 0 || userInfo[_user].powerAll == 0) {
                reward = 0;
            } else {
                uint256 userTimestamp = userInfo[_user].startTime;
                uint256 userWeight = getUserWeight(_user);

                if (userTimestamp < rewardToken.startTime) {
                    uint256 oldRewardPerSecond = rewardToken
                        .oldRewardPerDay
                        .div(SECONDS_PER_DAY);
                    reward = oldRewardPerSecond
                        .mul(userWeight)
                        .div(EXA_CONSTANT)
                        .mul(rewardToken.startTime.sub(userTimestamp));
                    userTimestamp = rewardToken.startTime;
                    uint256 rewardPerSecond = rewardToken
                        .currentRewardPerDay
                        .div(SECONDS_PER_DAY);
                    uint256 currentReward = rewardPerSecond
                        .mul(userWeight)
                        .div(EXA_CONSTANT)
                        .mul(_getDepositTime(_user));
                    reward = reward.add(currentReward);
                    reward = reward.add(rewardDebt[_user]);
                } else {
                    uint256 rewardPerSecond = rewardToken
                        .currentRewardPerDay
                        .div(SECONDS_PER_DAY);
                    reward = rewardPerSecond
                        .mul(userWeight)
                        .div(EXA_CONSTANT)
                        .mul(_getDepositTime(_user));
                    reward = reward.add(rewardDebt[_user]);
    ...
    function updateRewardDebtAllUser() public {
        for (uint256 i = 0; i < listUserUpdate.length; i++) {
    ...
    function _updateRewardDebt(address _user) internal {
        if (rewardToken.enabled == true) {
            if (totalPowerInPool > 0 && userInfo[_user].powerAll > 0) {
                uint256 userWeight = getUserWeight(_user);
                if (userInfo[_user].startTime < rewardToken.startTime) {
                    uint256 reward;
                    uint256 oldRewardPerSecond = rewardToken
                        .oldRewardPerDay
                        .div(SECONDS_PER_DAY);
                    reward = oldRewardPerSecond
                        ...
                        .div(EXA_CONSTANT)
                        .mul(
                            rewardToken.startTime.sub(userInfo[_user].startTime)
                        );
                    userInfo[_user].startTime = rewardToken.startTime;

                    uint256 rewardPerSecond = rewardToken
                        .currentRewardPerDay
                        .div(SECONDS_PER_DAY);
                    reward = reward.add(
                        rewardPerSecond.mul(userWeight).div(EXA_CONSTANT).mul(
                            _getDepositTime(_user)
                        )
                    );
                    rewardDebt[_user] = rewardDebt[_user].add(reward);
    ...
    function _removeBlucamonFromUserInfo(uint256 index, address user) internal {
        ...
        userInfo[user].powerAll = userInfo[user].powerAll.sub(
            blucamonPower[blucamonTokenIds[index]]
        );
        totalPowerInPool = totalPowerInPool.sub(
            blucamonPower[blucamonTokenIds[index]]
        );
    ...
    function _validateBlucamons(address _sender, uint256[] memory blucamonIds)
        internal
        view
    {
        require(isBlucamonPowerExist(blucamonIds), "S_MTS_500");
        uint256 blucamonCount = blucamonIds.length;
        require(blucamonCount > 0, "S_MTS_600");
        require(
    ...
    function stake(uint256[] memory _blucamonsTokenIds) external {
        _validateBlucamons(msg.sender, _blucamonsTokenIds);
        _initListUser(msg.sender);
        _withdrawReward();
        for (uint256 i = 0; i < _blucamonsTokenIds.length; i++) {
            nftToken.safeTransferFrom(
                msg.sender,
                address(this),
                _blucamonsTokenIds[i]
            );
            ...
            userInfo[msg.sender].powerAll = userInfo[msg.sender].powerAll.add(
                blucamonPower[_blucamonsTokenIds[i]]
            );
            totalPowerInPool = totalPowerInPool.add(
                blucamonPower[_blucamonsTokenIds[i]]
            );
        }
        _updateTokenRewardPerDay();
    ...
    function unstake(uint256[] memory blucamonTokenIds) public nonReentrant {
        require(blucamonTokenIds.length > 0, "S_MTS_700");
        require(
            userInfo[msg.sender].blucamonTokenIds.length >=
                blucamonTokenIds.length,
            "S_MTS_701"
        );
        _withdrawReward();
        bool findToken;
    ...
    function emergencyUnstake() public {
        ...
        for (uint256 i = 0; i < blucamonTokenIds.length; i++) {
            ...
            totalPowerInPool = totalPowerInPool.sub(
                blucamonPower[blucamonTokenIds[i]]
            );
        }
    ...
    function updateBlucamonPower(Blucamon[] memory blucamons) public onlyOwner {
        for (uint256 i = 0; i < blucamons.length; i++) {
            ...
            if (!hasBlucamon[tokenId]) {
                ...
                blucamonPower[tokenId] = power;
            } else if (blucamonPower[tokenId] != power) {
                blucamonPower[tokenId] = power;
            }
        }
    }
