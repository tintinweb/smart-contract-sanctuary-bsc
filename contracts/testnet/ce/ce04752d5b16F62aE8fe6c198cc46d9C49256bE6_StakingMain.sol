...
contract StakingMain is IBEP721, IBEP721Metadata, Ownable {
    ...
    function setHubRouting(address _hub) external onlyOwner {
        require(Address.isContract(_hub), "StakingSet: Not a contract");
        Hub = IHubRouting(_hub);
    }
     
    function buySmartStaker(uint256 _setNum, uint _amount) external payable {
        _mint(msg.sender, tokenCount);
       // Hub.stake{value: msg.value}(_setNum, _amount, tokenCount);
    ...
    function getNFTFields(uint256 _id) external returns (uint256) {
        return Hub.totalAmountsOfRewards(_id);
    }
