// SPDX-License-Identifier: Some Cool Guy
...
contract HelloSir is ERC721, Ownable {
    using Strings for uint256;
    uint _currentId ;
    address payable _coolGuy;
    

    mapping (uint256 => bool) private _locked;
    constructor () ERC721(unicode"麻豆", unicode"麻豆"){
       for (uint i=0; i < 5;) {
        mint();
        unchecked {
            ++i;
    ...
    function mint() public {
        uint id = _currentId + _startTokenId();
        _mint(msg.sender, id);
        unchecked {
            ++_currentId;
        }
