...
interface IPoolForLens is IPool {
    ...
    function oracle() external view returns (IOracleForLens);
}
...
contract PoolLens {
    ...
    {
        IPoolForLens self = IPoolForLens(_pool);
        IOracleForLens oracle = self.oracle();
        ...
        uint256 indexPrice = IOracleForLens(address(oracle)).getLastPrice(_indexToken);
