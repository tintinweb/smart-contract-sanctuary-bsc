...
contract Reward is Ownable {
    ...
    event Award(uint256 missionId, address winner, uint256 amount);

    constructor(address rewardToken, address rewardWallet) {
    ...
    function sendReward(
        ...
        require(missionIds.length == winners.length, "Lengths mismatch");
        require(missionIds.length == amounts.length, "Lengths mismatch");
        for (uint256 i = 0; i < missionIds.length; i++) {
            _rewardToken.transferFrom(_rewardWallet, winners[i], amounts[i]);
            emit Award(missionIds[i], winners[i], amounts[i]);
        }
    }
}
