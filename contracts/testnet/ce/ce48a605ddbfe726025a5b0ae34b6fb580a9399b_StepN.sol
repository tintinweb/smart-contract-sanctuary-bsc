...
contract StepN {
	...
	function getUserActivePlan(address userAddress) public view returns(uint8) {
		return users[userAddress].activePlan;
	}
	...
	function getUserAvailablePlan(address userAddress) public view returns(uint256[] memory plan, uint256[] memory deposit) {
		User storage user = users[userAddress];
		uint8 availableIndex;
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (!user.deposits[i].isWithdrawn && block.timestamp > user.deposits[i].finish) {
                plan[availableIndex] = user.deposits[i].plan;
				deposit[availableIndex] = i;
				availableIndex++;
			}
		}
	}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 amount, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
