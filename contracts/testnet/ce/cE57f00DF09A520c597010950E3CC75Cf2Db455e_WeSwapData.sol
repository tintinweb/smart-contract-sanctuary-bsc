...
contract WeSwapData is Callerable, Strings, SafeMath {
    ...
    struct CoinIndexData {
        uint256 index;
        address tokenAddress;
    }
    mapping(address => mapping(uint256 => CoinIndexData)) mapUserCoinTokenPair;
    ...
    function addLiquidityToken(
        ...
        if (mapProviderTokenIndex[hash][account] > 0) {
            if (
    ...
    function _updateProviderSharePercentOfToken(
        ...
        if (mapProviderTokenNumber[hash] > 0) {
            percent =
                ((10 **
            ...
            for (uint256 i = 1; i <= mapProviderTokenNumber[hash]; ++i) {
                ...
                if (!p.removed && p.provider != provider) {
                    p.percent =
                        (p.percent * rePercent) /
                        (10 **
                            IERC20Metadata(
                ...
                if (p.provider == provider) {
                   percent = percent + (p.percent * rePercent) /
                        (10 **
                            IERC20Metadata(
                                mapAllLiquidityToken[hash].tokenAddress1
                            ).decimals());
    ...
    function addLiquidityCoin(
        ...
        uint256 newPercent = 0;
        if (exists) {
            newPercent = _updateProviderSharePercentOfCoin(
                tokenAddress,
                tokenAmount,
                account
            );
            ...
            newPercent = 10**IERC20Metadata(tokenAddress).decimals();
        }        

        if (mapProviderCoinIndex[tokenAddress][account] > 0) {
            if (
        ...
        if (mapUserLiquidityCoin[account][tokenAddress].exists) {
            ...
            ] = CoinIndexData(mapProviderCoinNumber[tokenAddress], tokenAddress);
        }
    ...
    function _updateProviderSharePercentOfCoin(
        ...
        if (mapProviderCoinNumber[token] > 0) {
            percent =
            ...
            for (uint256 i = 1; i <= mapProviderCoinNumber[token]; ++i) {
                ...
                if (p.provider == provider) {
                    percent = percent + (p.percent * rePercent) / (10**IERC20Metadata(token).decimals());
                    p.percent = percent;
                }
            }
        } else {
    ...
    function swapToken(
        ...
        require(fromTokenAmount > distributedAmount, "from amount smaller then distributedAmount");
        uint256 k = ltd.amount1 * ltd.amount2;

        uint256 actualFromTokenAmount = fromTokenAmount - distributedAmount;
        if(ltd.tokenAddress1 == fromTokenAddress) {
            require(ltd.amount1 > actualFromTokenAmount, " require ltd.amount1 > actualFromTokenAmount");        
            ...
            require(ltd.amount2 > actualFromTokenAmount, " require ltd.amount1 > actualFromTokenAmount");        
            resultAmount = ltd.amount1 - k / (ltd.amount2 + actualFromTokenAmount);
            ltd.amount2 = ltd.amount2 + actualFromTokenAmount;
            ltd.amount1 = ltd.amount1 - resultAmount;
        }
    ...
    function _distributeTokenSwapTokenFee(
        ...
        require(providerNumber > 0, " require providerNumber > 0");
    ...
    function _estimateTokenSwapTokenFee(
        ...
        for (uint256 i = 1; i <= providerNumber; ++i) {
            ...
            fee +=
                ((fromTokenAmount * feeRate / 1000) * patd.percent) /
                (10**IERC20Metadata(fromTokenAddress).decimals());
        }
    }
    ...
    function _estimateTokenSwapCoinFee(
        ...
        uint256 providerNumber = mapProviderCoinNumber[fromTokenAddress];
        uint256 allFee = (fromTokenAmount * feeRate) / 1000;
        for (uint256 i = 1; i <= providerNumber; ++i) {
    ...
    function _distributeCoinSwapTokenFee(
        ...
        for (uint256 i = 1; i <= providerNumber; ++i) {
            ...
            uint256 fee = (((coinAmount * feeRate) / 1000) * pacd.percent) /
                (10**IERC20Metadata(toTokenAddress).decimals());
    ...
    function _estimateCoinSwapTokenFee(
        ...
        uint256 providerNumber = mapProviderCoinNumber[toTokenAddress];
        uint256 allFee = (coinAmount * feeRate) / 1000;
        for (uint256 i = 1; i <= providerNumber; ++i) {
            ProviderAllCoinData storage pacd = mapProviderAllCoin[
                toTokenAddress
            ][i];
            fee += (allFee * pacd.percent) / (10**IERC20Metadata(toTokenAddress).decimals());
        }
    }
        ...
        )
    {
        ...
        if (!exists) {
            ...
            // if(ltd.tokenAddress2 == fromTokenAddress && ltd.tokenAddress1 == toTokenAddress) {
            //     fromTokenAddress = ltd.tokenAddress1;
            //     toTokenAddress = ltd.tokenAddress2;
            // }
    ...
    function _estimateTokenSwapResult(
        ...
        if(ltd.tokenAddress1 == fromTokenAddress) {
            ...
            resultAmount = ltd.amount1 - k / (ltd.amount2 + (fromAmount - fee));
        }
    }
    ...
    function getUserLiquidityTokenByIndex(address account, uint256 index)
        external
        view
        returns (
            bool res,
    ...
    function getUserLiquidityTokenByAddress(
        address account,
        address tokenAddress1,
        address tokenAddress2
    )
    ...
    function getUserLiquidityCoinByIndex(address account, uint256 index)
        external
        view
        returns (
            address tokenAddress,
    ...
    {
        CoinIndexData memory cid = mapUserCoinTokenPair[account][index];
        uint256 indexOfAll = cid.index;// mapUserCoinTokenPair[account][index];
        if (indexOfAll > 0) {
            tokenAddress = cid.tokenAddress;
    ...
    function getUserLiquidityCoinByTokenAddress(address account, address tokenAddress)
        external
        view
        returns (
            bool res,
