...
contract GameScoreKeeperOracleMultiMatches is
    ChainlinkClient,
    ConfirmedOwner,
    ReentrancyGuard,
    IChainLinkOracleSportDataMultiMatches
{
    ...
    uint256 private payment = (1 * LINK_DIVISIBILITY) / 100;
        ...
        returns (bytes32)
    {
       require(block.timestamp > lastTime[msg.sender] + times, "CLC03");
        ...
        request.addStringArray("matchIds", _matchIds);
        ...
        recordChainlinkFulfillment(_requestId)
    {
        ...
        IRegularCompetitionContractMultiMatches(regularAddress).setIsResult();
    }

    function getQueryString(
        IRegularCompetitionContract.BetOption[][] memory _multibet
    ) internal pure returns (string memory) {
        string memory queryString = "";
        for (uint256 j = 0; j < _multibet.length; j++){
            IRegularCompetitionContractMultiMatches.BetOption[] memory betOptions = _multibet[j];
            for (uint256 i = 0; i < betOptions.length; i++) {
                IRegularCompetitionContractMultiMatches.BetOption memory betOption = betOptions[i];
                string memory s;
                s = string(
                    abi.encodePacked(
            ...
            queryString = j != _multibet.length - 1 ? queryString.append('|') : queryString;
        }
        return queryString;
    }
    ...
    function getPayment() external override view returns (uint256) {
        return payment;
    }
    ...
    {
        bytes memory dataRefactor = abi.decode(data, (bytes));
        return abi.decode(dataRefactor, (uint256[][]));
    }
}
