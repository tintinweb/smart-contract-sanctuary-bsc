...
interface IToken {
	function getAvailableToken(address token) external view returns (TokenInfo memory tokeninfo);
...
contract Token is TokenData, IToken {
	...
	function getAvailableToken(address token) external view onlyAccepted returns (TokenInfo memory tokeninfo) {
