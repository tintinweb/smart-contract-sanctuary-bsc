...
import "./BombVesting.sol";

/**
 * @dev PrivateSaleBombVesting will be blocked and release 10% each month.
 * Hence, the vestingDuration should be 10 months from the beginning.
 *
 */
contract PrivateSaleBombVesting is BombVesting {
    constructor(
        address _token,
        address _owner,
        uint256 _vestingStartAt
    ) BombVesting(_token, _owner, _vestingStartAt, 1) {}
}

/**
 * @dev TeamBombVesting will be blocked for 1 year,
 ...
 *
 */
//contract TeamBombVesting is BombVesting {
//    //uint256 private SECONDS_PER_YEAR = 31536000;
//    constructor(
//        address _token,
//        address _owner,
//        uint256 _vestingStartAt
//    ) BombVesting(_token, _owner, (_vestingStartAt + 86400), 1) {} // second per day
//}

/**
 * @dev AdvisorBombVesting will be blocked for 1 year,
 * then releaseed linearly each month during the next year.
 * Hence, the _vestingStartAt should delay 1 year
 * and the vestingDuration should be 12 months.
 *
 */
//contract AdvisorBombVesting is BombVesting {
//    //uint256 private SECONDS_PER_YEAR = 31536000;
//    constructor(
//        address _token,
//        address _owner,
//        uint256 _vestingStartAt
//    ) BombVesting(_token, _owner, (_vestingStartAt + 31536000), 12) {}
//}

/**
 * @dev DexLiquidityBombVesting will be blocked for 1 month,
 ...
 *
 */
//contract DexLiquidityBombVesting is BombVesting {
//    //uint256 private SECONDS_PER_MONTH = 2628000;
//    constructor(
//        address _token,
//        address _owner,
//        uint256 _vestingStartAt
//    ) BombVesting(_token, _owner, (_vestingStartAt + 2628000), 20) {}
//}

/**
 * @dev ReserveBombVesting will be blocked for 1 year,
 * then releaseed linearly each month during the next 2 year.
 * Hence, the _vestingStartAt should delay 1 year
 * and the vestingDuration should be 24 months.
 *
 */
//contract ReserveBombVesting is BombVesting {
//    //uint256 private SECONDS_PER_YEAR = 31536000;
//    constructor(
//        address _token,
//        address _owner,
//        uint256 _vestingStartAt
//    ) BombVesting(_token, _owner, (_vestingStartAt + 31536000), 24) {}
//}

/**
 * @dev BombVestingFactory is the main and is the only contract should be deployed.
 * Notice: remember to config the Token address and approriate startAtTimeStamp
 */
contract BombVestingFactory {
    // put the token address here
    // This should be included in the contract for transparency
    address public BOMB_TOKEN_ADDRESS = 0x7FF9f6696319D2e183d715EE31EB6ec1cb9d56b7;

    // put the startAtTimeStamp here
    // To test all contracts, change this timestamp to time in the past.
    uint256 public startAtTimeStamp = 1648425600; // 28/3/2022- 12h

    // address to track other information
    address public owner;
    address public privateSaleBombVesting;
    //address public teamBombVesting;
//    address public advisorBombVesting;
//    address public dexLiquidityBombVesting;
//    address public reserveBombVesting;

    constructor() {
        owner = msg.sender;

        PrivateSaleBombVesting _privateSaleBombVesting = new PrivateSaleBombVesting(
            BOMB_TOKEN_ADDRESS,
            owner,
            startAtTimeStamp
        );
        privateSaleBombVesting = address(_privateSaleBombVesting);

//        TeamBombVesting _teamBombVesting = new TeamBombVesting(BOMB_TOKEN_ADDRESS, owner, startAtTimeStamp);
//        teamBombVesting = address(_teamBombVesting);

        //AdvisorBombVesting _advisorBombVesting = new AdvisorBombVesting(BOMB_TOKEN_ADDRESS, owner, startAtTimeStamp);
        //advisorBombVesting = address(_advisorBombVesting);

//        DexLiquidityBombVesting _dexLiquidityBombVesting = new DexLiquidityBombVesting(
//            BOMB_TOKEN_ADDRESS,
//            owner,
//            startAtTimeStamp
//        );
        //dexLiquidityBombVesting = address(_dexLiquidityBombVesting);

//        ReserveBombVesting _reserveBombVesting = new ReserveBombVesting(BOMB_TOKEN_ADDRESS, owner, startAtTimeStamp);
//        reserveBombVesting = address(_reserveBombVesting);
    }
}
...
abstract contract BombVesting is BEPOwnable {
    using SafeMath for uint256;

    // Address of Bomb Token.
    IBEP20 public bombToken;
    ...
    // vesting cliff is on day
    //(60*60*24)
    uint256 internal constant SECONDS_PER_DAY = 3600; //86400
    ...
    uint256 internal percent_unlease_per_day;
    ...
    function addBeneficiary(address _beneficiary, uint256 _amount) public onlyOwner {
        ...
        require(bombToken.transferFrom(_msgSender(), address(this), _amount), "cannot-transfer-token-to-this-contract");
    ...
    function claimVestedToken(address _beneficiary) public {
        ...
        uint256 daysVestable;
        uint256 tokenVestable;
        (daysVestable, tokenVestable) = calculateClaimable(_beneficiary);
        require(tokenVestable > 0, "nothing-to-be-vested");

        require(bombToken.transfer(_beneficiary, tokenVestable), "fail-to-transfer-token");

        // update data in blockchain storage
        Beneficiary storage bf = beneficiaries[_beneficiary];
        bf.daysClaimed = bf.daysClaimed.add(daysVestable);
    ...
    function calculateClaimable(address _beneficiary) private view returns (uint256, uint256) {
        ...
        if (elapsedDays >= vestingDuration) {
            ...
            uint256 daysVestable = elapsedDays.sub(bf.daysClaimed);
            uint256 tokenVestedPerDay = bf.initialBalance.div(vestingDuration);
            uint256 tokenVestable = daysVestable.mul(tokenVestedPerDay);
            return (daysVestable, tokenVestable);
        }
    }

    // view function to check status of a beneficiary
    function getBeneficiary(address _beneficiary) public view returns (
        uint256 initialBalance, uint256 daysClaimed, uint256 totalClaimed){

        Beneficiary storage bf = beneficiaries[_beneficiary];
        require(bf.initialBalance > 0, "beneficiary-not-found");
        return (bf.initialBalance, bf.daysClaimed, bf.totalClaimed);
    }
}
