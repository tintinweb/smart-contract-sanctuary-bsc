...
contract HexaFinityToken is IERC20, Ownable, ReentrancyGuard {
  ...
  constructor(address _router, address _taxFeeAddress) {
    ...
    //exclude owner, this contract address and taxFeeAddrss from paying fee
    _isExcludedFromFee[owner()] = true;
    _isExcludedFromFee[address(this)] = true;
    _isExcludedFromFee[taxFeeAddress] = true;

    //exclude tax receiver and burn address from receiving rewards
  ...
  function _tokenTransfer(
    ...
    if (burnAmt > 0) _transferStandard(sender, BURN_ADDRESS, burnAmt);
    if (taxFeeAmt > 0) _transferStandard(sender, taxFeeAddress, taxFeeAmt);
