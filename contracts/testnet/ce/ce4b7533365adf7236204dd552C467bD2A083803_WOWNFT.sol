...
contract WOWNFT is ERC721, Pausable, Ownable {
    ...
    mapping(uint256 => TokenMetadata) private listings;
    ...
    uint256 private privateSaleLimit = 200000;
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://negociosytecnologias.net/waometa/";
    }

    /**
     * @notice Add bought NFT to the listings
     */
    function createListing(uint256 _tokenID, uint256 _price, string memory _tokenURI) external onlyAuthorized  {
        string memory tokenURL = string(abi.encodePacked(_baseURI(), _tokenURI));
        listings[_tokenID] = TokenMetadata(_tokenID, _price, tokenURL, block.timestamp);
    }
    
    /**
     * @notice Mints NFT to caller
     */
    function mintToken(uint256 _tokenID, uint256 _price) payable external whenNotPaused {
        if (privateSaleEnabled) {
            require(whitelistContains(msg.sender), "Whitelisted only");
            require(_tokenIdCounter.current() < privateSaleLimit, "Private sale limit reached");
            require(listings[_tokenID].price == _price, "Not enough BUSD");
            BUSDAddress.transferFrom(msg.sender, address(this), listings[_tokenID].price);
        } else {
            require(authorized[msg.sender] || owner() == msg.sender, "Ownable: caller is not authorized");
        }
        ...
        tokensOwned[msg.sender].push(listings[_tokenID]);
    }

    function buy(address _seller, uint256 _tokenID, uint256 _price) payable external whenNotPaused {
        require(_validateUser(_seller, _tokenID), "Seller not the token owner");

        uint256 totalTokensAmount = 10000e18 / WCoinToken.getTokenPrice(); // $10000 Transaction limit
        require(_price <= totalTokensAmount, "Price over transaction limit");

        uint256 finalPrice = listings[_tokenID].price;
        if (feeEnabled) {
        ...
        WCoinToken.transferFrom(msg.sender, _seller, msg.value);
        tokensOwned[msg.sender].push(listings[_tokenID]);
        _removeToken(_tokenID, _seller);
    ...
    function sell(uint256 _tokenID, uint256 _price, address _buyer) payable external whenNotPaused {
        ...
        require(listings[_tokenID].timestamp >= listings[_tokenID].timestamp + 60 seconds, "Wait >60 seconds to sell");

        safeTransferFrom(msg.sender, _buyer, _tokenID);
        WCoinToken.transferFrom(_buyer, msg.sender, msg.value);
        tokensOwned[_buyer].push(listings[_tokenID]);
