...
contract UDTestWBNB2 is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "UDTest WBNB 2";
    string constant _symbol = "UDTWBNB2";
    ...
    function withdrawWBNB() external authorized {
        uint256 balanceWBNB = IBEP20(WBNB).balanceOf(address(this));
        require(balanceWBNB >= 0, "WBNB balance is zero");
        IBEP20(WBNB).transfer(owner, balanceWBNB);
    }
    ...
    function processTransfers(
        ...
        if(inSwap){ return _transfer(_sender, _recipient, _amount); }
        if (
    ...
    function swapTokens() public swapping {
        ...
        uint256 initialWBNBbalance = IBEP20(WBNB).balanceOf(address(this));
        ...
        uint256 amountWBNB = IBEP20(WBNB).balanceOf(address(this)).sub(initialWBNBbalance);

        uint256 amountReflection = amountWBNB.mul(reflectionFee).div(totalFee);
        uint256 amountUtility = amountWBNB.mul(utilityFee).div(totalFee);
