...
interface IUniswapV2Pair is IUniswapV2Locker {
...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _withdrawer) external payable nonReentrant {
    require(_unlock_date < MAX_UNLOCK_DATE, 'TIMESTAMP INVALID'); // prevents errors when timestamp entered in milliseconds
  ...
  function uniswapPairIsInitialised (address _token0, address _token1) external view returns (bool) {
    ...
    uint256 balance = IERC20Token(_token0).balanceOf(pairAddress);
    if (balance > 0) {
      return true;
    }
    return false;
  ...
  function lockLiquidity (IERC20Token _baseToken, IERC20Token _saleToken, uint256 _baseAmount, uint256 _saleAmount, uint256 _unlock_date, address payable _withdrawer) external {
    ...
    TransferHelper.safeApprove(pair, address(this), totalLPTokensMinted);
    uint256 unlock_date = _unlock_date > MAX_UNLOCK_DATE ? MAX_UNLOCK_DATE : _unlock_date;
    this.lockLPToken(pair, totalLPTokensMinted, unlock_date, _withdrawer);
  }
}
