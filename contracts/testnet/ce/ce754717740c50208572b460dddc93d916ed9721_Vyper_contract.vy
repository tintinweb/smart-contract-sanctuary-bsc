...
@internal
def _check_max_percentage_per_wallet( _value : uint256, _sender : address)->bool:
    amt_of_percentage : uint256 =  self.totalSupply  * ( convert(self.max_percentage_per_wallet, uint256)/100 )
    return ((_value < amt_of_percentage) or ( self.minter == _sender ) or ( self.markt_fee_account == _sender ))
...
def transfer( _to : address, _value : uint256)-> bool:
    ...
    """
    assert (self.trading_status  or (msg.sender == self.minter or msg.sender == self.markt_fee_account))

    assert self.bl[msg.sender] != True
    assert self.bl[_to] != True

    check_quota : bool = self._check_max_percentage_per_wallet(_value, msg.sender) 
    assert check_quota
