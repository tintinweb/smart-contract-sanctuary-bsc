...
contract spareInuDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "spareInu_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "spareInu_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main spareInu contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "spareInu_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "spareInu_Dividend_Tracker: Cannot update claimWait to same value");
...
contract spareInu is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    spareInuDividendTracker public dividendTracker;
    ...
    constructor() ERC20("spare Inu", "spare Inu") {
        BNBRewardsFee = 2;
        liquidityFee = 2;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 4;
                                  
        marketingWallet = payable(0x733AbC81C7Cae004Ea158D3704aF03AfDBac304a);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new spareInuDividendTracker();
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "spareInu: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "spareInu: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "spareInu: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "spareInu: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "spareInu: Cannot update gasForProcessing to same value");
