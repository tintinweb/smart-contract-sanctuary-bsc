...
contract PresalePool is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(
        ...
        if (_softcap > 0) softcap = _softcap;
        if (_maxInvest > 0) {
    ...
    function cancelSale() external onlyTokenOwner {
        require (!saleEnabled || (saleEnabled && block.timestamp < startTime), "sale already started");
        canceled = true;
    }
...
contract Launchpad is Ownable {
    ...
    function addPool(address _pool, string memory _logo) external onlyOwner {
