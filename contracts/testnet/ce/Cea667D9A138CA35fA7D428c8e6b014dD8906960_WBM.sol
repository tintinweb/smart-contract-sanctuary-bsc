...
contract WBM is IERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        require(projectFeeBps_ >= 0, "Invalid project fee");
        if (projectAddress_ == address(0)) {
            require(
                projectFeeBps_ == 0,
                "Cant set both project address to address 0 and project percent more than 0"
            );
        }
        require(
            taxFeeBps_ + liquidityFeeBps_ + projectFeeBps_ <= 10**4 / 4,
    ...
    function setTaxFeePercent(uint256 taxFeeBps) external onlyOwner {
        _taxFee = taxFeeBps;
        require(
            _taxFee + _liquidityFee + _projectFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
        ...
        )
    {
        ...
        uint256 tProjectFee = calculateProjectFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(
            tProjectFee
        );
        return (tTransferAmount, tFee, tLiquidity, tProjectFee);
    }
    ...
    function _takeProjectFee(uint256 tProject) private {
        if (tProject > 0) {
            uint256 currentRate = _getRate();
            uint256 rProject = tProject.mul(currentRate);
            _rOwned[_projectAddress] = _rOwned[_projectAddress].add(rProject);
            if (_isExcluded[_projectAddress])
                _tOwned[_projectAddress] = _tOwned[_projectAddress].add(
                    tProject
                );
            emit Transfer(_msgSender(), _projectAddress, tProject);
        }
    }
    ...
    function calculateProjectFee(uint256 _amount)
        ...
        returns (uint256)
    {
        if (_projectAddress == address(0)) return 0;
        return _amount.mul(_projectFee).div(10**4);
    }

    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _projectFee == 0) return;
