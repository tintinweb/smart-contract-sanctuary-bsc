...
contract CollectionAdmin is ICollectionAdmin, ManagedByNFTKEY {
    ...
    modifier onlyAdmins(address collectionAddress) {
        require(
            _hasManagingAuthority(collectionAddress, _msgSender()),
    ...
    /**
     * @dev Check if an account has managing admin authority
     */
    function _hasManagingAuthority(address collectionAddress, address account)
        private
        view
        returns (bool)
    {
        ...
        if (isCollectionAdmin(collectionAddress, account)) {
            return true;
        }
