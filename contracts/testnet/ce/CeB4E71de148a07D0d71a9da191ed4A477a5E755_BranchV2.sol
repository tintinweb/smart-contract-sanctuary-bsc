...
contract BranchV2 is ReentrancyGuard {
    ...
    address fuel = 0x7D4E15dC584AD1C0c766621173B646E6621244e0;
    address daos = 0x6ADFAd8cC6cfFd7226b13257248969786754ebdB;
    address fuelRescue = 0x7228AAe2BA96E881c12AbF524db3392c905052e6;
    // 管理员配置
    ...
    function processBrokerBonus(address addr, uint256 amount) private {
        ...
        while (true) {
            ...
            if (parentLevel == level && level == 6) {
                ...
                bonus = verifyBrokerBonus(msg.sender, bonus);
                amount -= bonus;
                break;
    ...
    function userPreDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        deposit.endFinalPayTimestamp = uint32(block.timestamp + (days_ + 1800) * 60);
        deposit.status = 10;
    ...
    function userCheckBlacklist(address addr) internal view returns (bool result) {
    ...
    // 处理待付尾款
    function randomStartFinalPay(address addr) public view returns (uint256 days_) {
        uint256 k;
    ...
    // 处理待取款
    function randomEndFinalPay(address addr) public view returns (uint256 days_) {
    ...
    // 每小时计算一次(暂时用实时计算)
    function depositWithdrawRatePercent() public view returns (uint256) {
    ...
    function randomFinalPayDays(uint256 end) public view returns (uint256 days_) {
        uint256 randomSum;
    ...
    function randomWithdrawDays(uint256 x) public view returns (uint256 days_) {
        uint256 randomSum;
