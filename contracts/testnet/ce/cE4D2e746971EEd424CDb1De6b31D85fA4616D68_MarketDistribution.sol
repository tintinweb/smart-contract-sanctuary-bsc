...
contract MarketDistribution is TokensRecoverable, IMarketDistribution {
    ...
    // baseToken = WBNB
    function distribute(uint256 _baseAmountForLiquidity, uint256 _rootedAndEliteAmountForLiquidity,
        uint256 _lockTimeInSeconds, uint256 _maxTransactionAmount, uint16 additionalDumpTaxRate,
        uint256 dumpTaxRateDurationInSeconds) public override {
    ...
    function createRootedEliteLiquidity(uint256 _baseAmountForLiquidity) private {
        ...
        uniswapV2Router.addLiquidity(address(eliteToken), address(rootedToken),
            rootedAndEliteAmountForLiquidity, rootedAndEliteAmountForLiquidity, 0, 0, address(this), block.timestamp);
    ...
    function createRootedBaseLiquidity() private {
        uniswapV2Router.addLiquidity(address(baseToken), address(rootedToken), baseAmountForLiquidity,
            rootedAndEliteAmountForLiquidity, 0, 0, liquidityController, block.timestamp);
    ...
    function getClaimInfo(address account) public view returns (uint256 claimed, uint256 available, uint256 remaining) {
        require (distributionComplete, "Distribution is not completed");
        require (msg.sender == address(marketGeneration), "Unauthorized");
        ...
        uint256 remaining = remainingClaim[account];
        uint256 claimed = totalClaim[account] - remainingClaim[account];

        if(claimTime[account] < endTime) {
            return (claimed, 0, remaining);
        }

        uint256 claimStartTime = claimTime[account] == 0 ? vestingPeriodStartTime : claimTime[account];
        uint256 available = (endTime.sub(claimStartTime)).mul(share).div(vestingDuration);

        return (claimed, available, remaining);
    }
