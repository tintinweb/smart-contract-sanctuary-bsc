...
contract HEXNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    event AutoAccepted(uint256 tokenId, uint256 creatorEarning);
    ...
    function acceptOfferBid(acceptOfferBidData memory _transferData) external payable {
        ...
        if(PLATFORM_SHARE_PERCENT > 0) {
            ...
            transferTokenToOwner(_transferData.newOwner, address(this), platformSharePercent);
        }
        uint256 royaltyPercent;
        if(ROYALTY_SPLIT > 0) {
            royaltyPercent = calculatePercentValue(amountToTransfer, ROYALTY_SPLIT);
            amountToTransfer = amountToTransfer-royaltyPercent;
            transferTokenToOwner(_transferData.newOwner, address(this), royaltyPercent);
        }
        transferTokenToOwner(_transferData.newOwner, tokenInfoById.currentOwner, amountToTransfer);
    ...
    function buyNFT(buyNFTData memory _buyData) external payable {
        ...
        if(ROYALTY_SPLIT > 0) {
            royaltyPercent = calculatePercentValue(amountToTransfer, ROYALTY_SPLIT);
            amountToTransfer = amountToTransfer-royaltyPercent;
            payable(_buyData.owner).transfer(royaltyPercent);
        }
    ...
    function acceptBid(acceptData memory _transferData) external payable onlyOwner {
        ...
        // }
        // else {
        //     transferTokenToOwner(_transferData.newOwner, tokenInfoById.currentOwner, _transferData.ownerShare);
        // }
        // if(_transferData.nft == address(this)) {
    ...
    function transferTokenToOwner(address from, address to, uint256 amount) private {
    ...
    function transferNFT(address cAddress, address from, address to, uint256 token) internal {
        NFT nftToken = NFT(cAddress);
