...
contract PreSaleBnbFairTeam is ReentrancyGuard {
    ...
    function buyToken() public payable allowed isHuman{
        require(!canClaim,"DEXPAD: pool is closed");
    ...
    function withdrawScamFundsBNB(uint256 _amountBNB) external onlyadminIDO {
        require(!allow,"DEXPAD: Stop preSale First");
    ...
    function withdrawLPTokens() external onlypreSaleOwner{
        require(canClaim == true,"DEXPAD: pool not initialized yet");
        require(block.timestamp > unLockTime,"DEXPAD: LockTime not over yet");
        require(!unLocked,"DEXPAD: Already Unlocked");
