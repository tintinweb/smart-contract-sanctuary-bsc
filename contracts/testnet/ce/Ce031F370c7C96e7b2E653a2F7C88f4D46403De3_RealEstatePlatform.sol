...
contract RealEstatePlatform is Ownable {
    ...
    struct Project {
        string symbol;
        uint256 startInvestDuration;
        uint256 endInvestDuration;
    ...
    function createProject(
        ...
        uint256 _startInvestDuration,
        uint256 _endInvestDuration,
        uint256 _startTime,
        uint256 _endTime,
        ...
        require(
            _endTime <= _startInvestDuration,
            "Error: _startInvestDuration must be later than _endTime"
        );
        require(
            _startInvestDuration < _endInvestDuration,
            "Error: _startInvestDuration must be later than _endInvestDuration"
        );
        require(
        ...
        project.startInvestDuration = _startInvestDuration;
        project.endInvestDuration = _endInvestDuration;
        ...
        isWhitelist
    {
        ...
        require(
            USDS.balanceOf(_msgSender()) >= tokenAmount,
            "Error: not enough USDR"
        );
        //payment
        USDS.safeTransferFrom(_msgSender(), address(this), tokenAmount);
    ...
    function voteAcceptOffer(uint256 offerIndex) external {
        ...
        require(
            block.timestamp > projects[buyerOffer.symbol].endTime ||
                stocks[buyerOffer.symbol].circulatingSupply ==
    ...
    //afer endInvestDuration or project sold
        ...
        isAdmin
    {
        ...
        require(
            projects[_symbol].isSold || block.timestamp > projects[_symbol].endInvestDuration,
            "Error: The project has not been sold and the endInvestDuration period has not expired"
        );
        ...
        //calculate reward
        uint256 totalRewardMax = SafeMath.div(
            SafeMath.mul(
                SafeMath.mul(
    ...
    //for Project sold or out of endInvestDuration
        ...
        external
    {
        ...
        uint256 tokenAmount = SafeMath.div(
            SafeMath.mul(_stockAmount, projects[_symbol].totalClaimValue),
            stocks[_symbol].totalSupply
        );
        ...
        if (projects[_symbol].isDistributed) {
            require(
                REXToken.approve(_msgSender(), tokenAmount),
                "REXToken approve failed!"
            );

            REXToken.safeTransfer(_msgSender(), tokenAmount);
            emit ClaimReward(
                _symbol,
                _stockAmount,
                tokenAmount,
            ...
            require(
                USDS.approve(_msgSender(), tokenAmount),
                "USDS approve failed!"
            );

            USDS.safeTransfer(_msgSender(), tokenAmount);
            emit ClaimRefundIdo(
                _symbol,
                _stockAmount,
                tokenAmount,
    ...
    function withdrawIdo(string calldata _symbol) external isAdmin {
        ...
        require(USDS.approve(_msgSender(), totalUsdb), "USDS approve failed!");
        USDS.safeTransfer(_msgSender(), totalUsdb);
