...
/**
 * @title Ziggyverse - Genesis Ziggy - NFT contract
 */
contract GenesisZiggy is ERC721Base, ERC721Reservable, ERC721Whitelist, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Address for address payable;

    address private immutable potsToken;
    address private immutable funLabsPassToken;
        ...
        ERC721Reservable(_maxReserved)
    {
        ...
        potsToken = _potsToken;
        funLabsPassToken = _funLabsPassToken;
    }

    /** 
     * @notice forward any BNB send to this contract to paymentReceiver 
     */
    ...
    function mint(address _to, uint256 _amount) external payable {
        require(!whitelistOnly || whitelisted(_to), "Not whitelisted");
        require(msg.value == priceFor(_to) * _amount || msg.sender == owner(), "Value sent does not match price");
    ...
    /**
     * @notice Change public price to `_price` in WEI.
     */
    ...
    /**
     * @notice Change whitelist price to `_price` in WEI.
     */
    ...
    function mintableSupply() external view returns (uint256) {
        return maxSupply - reservedTotal;
    }

    function whitelisted(address user) public view returns (bool) {
        return _whitelisted[user] 
            || IERC20(potsToken).balanceOf(user) > 0
            || IERC721(funLabsPassToken).balanceOf(user) > 0;
    }

    function priceFor(address user) public view returns (uint256) {
        return (whitelistOnly && whitelisted(user)) ? priceWhitelist : pricePublic;
    }
...
abstract contract ERC721Reservable is Ownable {
     ...
     */
    function grantReservedMints(address to, uint256 amount) external onlyOwner {
...
abstract contract ERC721Whitelist is Ownable {
    ...
    /**
     * @notice Grant whitelist mints to be used by the respective entry of `to`. Only callable by owner.
     */
    function grantWhitelistAccess(address[] memory to) external onlyOwner {
        for (uint256 i = 0; i < to.length; ++i) {
            _whitelisted[to[i]] = true;
        }
    }
}
