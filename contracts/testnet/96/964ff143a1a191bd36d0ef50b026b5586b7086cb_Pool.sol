...
contract Pool is Ownable {
    ...
    function stake(uint256 _amount) external {
        ...
        updateTotalShare(msg.sender, _amount, 1);
    }

    function unStake(uint256 _amount) external {
        require(
            _amount <= shares[msg.sender],
            "UNSTAKE_AMOUNT_MUST_LESS_SHARES"
        );
        stakeToken.transferFrom(address(this), msg.sender, _amount);
        updateTotalShare(msg.sender, _amount, 2);
    }
    ...
    function updateTotalShare(
        ...
        if (_type == 1) {
            ...
            totalShares += _amount;
            if (shares[sender] >= minAmount) {
                shareTime[sender] = block.timestamp;
            }
        } else {
            if (shares[sender] == _amount) {
                shareCount--;
            }
            if (shareTime[sender].add(stakeSeconds) <= block.timestamp) {
