...
contract PrivateSale is Ownable,ERC20 {
    IERC20 public token;
    ...
    uint256 public tokenPerBNB = 700;
    uint256 public claimEnabledStart;
    ...
    function getLastTxList() external view returns(LastTx[3] memory) {
        return lastTxList;
    }

    function claim() external returns(bool) {
        uint256 claimAmount = balanceOf(msg.sender);
        require(claimAmount > 0, "tokens limit!");
        token.transfer(msg.sender, claimAmount);
        super._burn(msg.sender, claimAmount);
        totalClaimed += claimAmount;
        return true;
    }

    function setClaimEnabled(uint256 enabledTime) external onlyOwner {
        require(privateSaleEndTimestamp < enabledTime, "Time should be more then sale end time");
        claimEnabledStart = enabledTime;
    }
    ...
    function deposit() public payable {
        ...
        totalDepositedEthBalance += take;
        deposits[msg.sender] += take;
    ...
    function recoverBEP20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        if (address(token) == tokenAddress) {
            ...
            uint256 reserved = totalBought - totalClaimed;
            if(tokenAmount > balance - reserved) {
                tokenAmount = balance - reserved;
            }
        }
    ...
    function setMaxDepositAmount(uint256 newValue) external onlyOwner {
        ...
        emit UpdateMaxDepositAmount(maximumDepositEthAmount, newValue);
