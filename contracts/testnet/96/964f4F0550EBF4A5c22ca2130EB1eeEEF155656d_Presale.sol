...
contract Presale is Ownable {
    ...
    uint256 public teamStakeUsdt = 200 * 10 ** 18;
    ...
    mapping(address => bool) public teamMaps;
    ...
    uint256[] public rewardLevel = [300, 200, 100, 50, 50, 50, 50, 50];
    uint256[] public teamRewardLevel = [600, 200, 100, 50, 50, 50, 50, 50];
    ...
    constructor(address _saleToken, address _usdtToken, uint256 _userWithdrawTime) public {
    ...
    function sale() public {
        ...
        for(uint256 i = 0; i < rewardLevel.length; i++) {
            ...
            uint256 reward = 0; 
            if(teamMaps[msg.sender]) {
                reward = getTokenNum * teamRewardLevel[i]/10000;
            } else {
    ...
    function getCanIdoAmount(address owner) public returns (uint256) {
    ...
    function stakeForTeam() public {
        IERC20(usdtToken).transferFrom(msg.sender, address(this), teamStakeUsdt);
        teamMaps[msg.sender] = true;
    }

    function withdrawForTeam() public {
        require(block.timestamp > userWithdrawTime);
        require(teamMaps[msg.sender], "Not is team");
        require(refferNumber[msg.sender] >= 15, "Not finished team task");
        IERC20(usdtToken).transfer(msg.sender, teamStakeUsdt);
    }
    ...
    function teamWithdraw() public onlyOwner {
        uint bal = IERC20(usdtToken).balanceOf(address(this));
        IERC20(usdtToken).transfer(teamAddress, bal);
    }
