...
contract PaybNftMarketplace is IPaybNftMarketplace, ERC721, IERC721Pausable{
    ...
    function mintFragment(address[] memory _to,uint256 _rootTokenId) public override onlyAdmin() onlyLaunchPadContract(){
        require(tokenAddress[_rootTokenId] == address(0) || tokenAddress[_rootTokenId].isContract(), "Token address isn`t a contract address");
        require(_to.length < 100, "Quantity is too big");
        require(_rootTokens[_rootTokenId], "Token isn`t root");
        
        for(uint256 i=0; i< _to.length; ++i){
            _safeMint(_to[i], tokenId, _rootTokenId, _tokenURIs[_rootTokenId]);
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
        ...
        if(tokenAddress[_tokenId] == address(0)) {
            ...
            if(_subOwnerFee > 0){
                IERC20(tokenAddress[_tokenId]).transferFrom(msg.sender, PaybPool, _subOwnerFee);
            }
            _pause(_tokenId);
            _transfer(ownerOf(_tokenId), msg.sender, _tokenId);
            if(_fragmentTokens[_tokenId]){
