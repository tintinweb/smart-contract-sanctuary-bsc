...
contract Presale is Initializable{
    ...
    address[] public addressesThatBought;
    ...
    function initialize(uint hardcap) external initializer{
        
        admin = payable(msg.sender);

        hardCap = hardcap;

        maximumPerAddress = 1000e18;
        minimumPerAddress = 0;     
    } 
...
receive() external payable{
    ...
    function buy() public payable{
        ...
        require(msg.value > minimumPerAddress, "Not Reaching Minimum");
        require(spentBNB[msg.sender]+ msg.value <= maximumPerAddress, "Exceeding Limit");
        require(raisedBNB + msg.value <= hardCap, "TM");//Too much, gone over hard cap


        if(spentBNB[msg.sender] == 0){
            buyersCount++;
            addressesThatBought.push(msg.sender);
            indexToAddress[buyersCount] = msg.sender;
        }
    ...
    function AdminWithdrawBNB(uint amount) external{
        require(msg.sender==admin, "NA");
        require(amount <= raisedBNB);
        raisedBNB -= amount;
