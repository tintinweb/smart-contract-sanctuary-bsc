...
contract Treasury is AccessControl {
    ...
    address public constant BONDS = 0x398753432d72078750C7c345767b173638d6dede;
    ...
    uint256 public rewardInterval = 24 * 3600;
    uint256 public nextRewardTime;
    uint16 public rewardPercent = 1000;
    ...
    function addPool(uint256 allocPoint, uint256 lockSecond) external onlyRole(OPERATOR_ROLE) {
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lockSecond: lockSecond, accRwardPerShare: 0}));
    }
    ...
    function updatePool(
        ...
        poolInfo[pid].lockSecond = lockSecond;
    }

    function setRewardParams(uint256 _rewardInterval, uint16 _rewardPercent) external onlyRole(OPERATOR_ROLE) {
    ...
    function mint(uint256 pid, uint256 amount) external {
        ...
        IERC20(USDT).safeTransferFrom(msg.sender, lendingPool, amount);
        uint256 tokenId = IBonds(BONDS).mint(msg.sender);
        bondsInfo[tokenId] = BondsInfo({pid: pid, amount: amount, startTime: block.timestamp, rewardDebt: (amount * poolInfo[pid].accRwardPerShare) / 1e18});
    ...
    function redeem(uint256 tokenId) external {
        ...
        require(block.timestamp > data.startTime + poolInfo[data.pid].lockSecond, "redeem time limited");
    ...
    function processReward() external {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            pool.accRwardPerShare += (rewardAmount * pool.allocPoint * 1e18) / (totalBondsAmount * totalAllocPoint);
        }
    ...
    function pending(uint256[] calldata tokenIds) external view returns (uint256[] memory rewards) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            rewards[i] = (data.amount * poolInfo[data.pid].accRwardPerShare) / 1e18 - data.rewardDebt;
        }
    }
    ...
    function harvest(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 rewardWithDebt = (data.amount * poolInfo[data.pid].accRwardPerShare) / 1e18;
            rewardAmount += (rewardWithDebt - data.rewardDebt);
            data.rewardDebt = rewardWithDebt;
        }
        if (rewardAmount > 0) IERC20(MUSD).safeTransfer(msg.sender, rewardAmount);
    }
}
