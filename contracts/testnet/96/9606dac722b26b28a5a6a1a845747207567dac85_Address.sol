...
contract TokensByGenDotCom is Context, IERC20 { 
    ...
    // Set owner  
    _owner = 0xEE9C2c3c5cBFc2884cb8b67709a1FD73966A782D;

    // Set basic token details
    _name               = "Leave Britney Alone";
    _symbol             = "BRITNEY";
    _decimals           = 4;
    ...
    event updated_fees(uint256 Marketing, uint256 Liquidity, uint256 Contract_Development_Fee);
    ...
    uint256 public _fee_Liquidity   = 0; // Set fees after deploying
    uint256 public _fee_Marketing   = 0; // Set fees after deploying
    uint256 public _fee_Contract    = 0; // Set fees after deploying
    ...
    */
    ...
    Fee applies to all Buys and Sell equally. 
    There is no fee on wallet to wallet transfers

    */


    // If contract development fee was set to 1% of transactions, this is included in the max fee limit
    function Set_Fees(uint256 Marketing, uint256 Liquidity) external onlyOwner {
        ...
        emit updated_fees(_fee_Marketing, _fee_Liquidity, _fee_Contract);
    }
    ...
    */
    ...
    function _transfer(
        ...
        // Wallet Limit
        if (!_isLimitExempt[to] && from != _owner)
            {
