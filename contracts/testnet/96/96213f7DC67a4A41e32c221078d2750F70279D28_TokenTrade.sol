...
contract TokenTrade is Modifier, Util {
    ...
    constructor() {
        ...
        pledgeToken = ERC20(0xEF9577Cd21EC077E38d6940776B0c873Bc27A6b5); 
    }
    ...
    function hang(address outputAddress, address receiveAddress, uint256 outputAmount, uint256 receiveAmount) public isRunning nonReentrant returns (bool) {
        ...
        order = Order(tempId, _pledgeAmount, outputAddress, receiveAddress,
                        outputAmount, receiveAmount, msg.sender, 0, _hangPoundage, _exchangePoundage, 0, 0);
    ...
    function exchange(uint256 _orderId, uint256 exchangeAmount) public isRunning nonReentrant returns (bool) {
        ...
        uint256 availableAmount = orderMapping[_orderId].outputAmount.sub(orderMapping[_orderId].outputedAmount);
        if(exchangeAmount > availableAmount) {
            _status = _NOT_ENTERED;
            revert("TokenTrade: Insufficient convertible amount");
        }
        ...
        uint256 price = toWei(orderMapping[_orderId].receiveAmount, 18).div(orderMapping[_orderId].outputAmount);
        uint256 _receiveAmount = backWei(exchangeAmount.mul(price), 18);

        uint256 receivePoundage = _receiveAmount.mul(orderMapping[_orderId].exchangePoundage).div(1000);
        if(receiveBalance < _receiveAmount.add(receivePoundage)) {
            _status = _NOT_ENTERED;
            revert("TokenTrade: Insufficient balance");
        }

        ERC20(_receiveAddress).transferFrom(msg.sender, address(this), _receiveAmount.add(receivePoundage));
        ERC20(orderMapping[_orderId].outputAddress).transfer(msg.sender, exchangeAmount);
        ERC20(_receiveAddress).transfer(orderMapping[_orderId].sellAdress, _receiveAmount);

        uint256 outputPoundage = exchangeAmount.mul(orderMapping[_orderId].hangPoundage).div(1000);
        ERC20(orderMapping[_orderId].outputAddress).transfer(poundageAddress, outputPoundage);
        ERC20(_receiveAddress).transfer(poundageAddress, receivePoundage);
        
        orderMapping[_orderId].outputedAmount = orderMapping[_orderId].outputedAmount.add(exchangeAmount);
        orderMapping[_orderId].receivedAmount = orderMapping[_orderId].receivedAmount.add(_receiveAmount);

        if(orderMapping[_orderId].outputAmount == orderMapping[_orderId].outputedAmount) {
    ...
    function cancel(uint256 _orderId) public isRunning nonReentrant returns (bool) {
        ...
        uint256 availableAmount = orderMapping[_orderId].outputAmount.sub(orderMapping[_orderId].outputedAmount);
        uint256 availablePoundage = availableAmount.mul(orderMapping[_orderId].hangPoundage).div(1000);
        ERC20(orderMapping[_orderId].outputAddress).transfer(msg.sender, availablePoundage);
        ERC20(orderMapping[_orderId].outputAddress).transfer(msg.sender, availableAmount);

        pledgeToken.transfer(msg.sender, orderMapping[_orderId].pledgeAmount);
    ...
    function cancelOrder(uint256 _orderId) public onlyApprove returns (bool) {
        ...
        uint256 availableAmount = orderMapping[_orderId].outputAmount.sub(orderMapping[_orderId].outputedAmount);
        uint256 availablePoundage = availableAmount.mul(orderMapping[_orderId].hangPoundage).div(1000);
        ERC20(orderMapping[_orderId].outputAddress).transfer(orderMapping[_orderId].sellAdress, availablePoundage);
        ERC20(orderMapping[_orderId].outputAddress).transfer(orderMapping[_orderId].sellAdress, availableAmount);
        pledgeToken.transfer(orderMapping[_orderId].sellAdress, orderMapping[_orderId].pledgeAmount);
    ...
    function getOrderAvailable(uint256 _orderId) public view returns (uint256 outputAmount, uint256 receiveAmount) {
        outputAmount = orderMapping[_orderId].outputAmount.sub(orderMapping[_orderId].outputedAmount);
        receiveAmount = orderMapping[_orderId].receiveAmount.sub(orderMapping[_orderId].receivedAmount);
    }
