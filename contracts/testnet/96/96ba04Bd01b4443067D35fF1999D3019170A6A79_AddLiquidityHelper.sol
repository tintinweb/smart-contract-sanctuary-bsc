...
contract AddLiquidityHelper is ReentrancyGuard, Ownable {
    ...
    address public solutionBearAddress;
    ...
    function addSolutionBearETHLiquidity(uint256 nativeAmount) external payable nonReentrant {
        require(msg.value > 0, "!sufficient funds");

        ERC20(solutionBearAddress).safeTransferFrom(msg.sender, address(this), nativeAmount);

        // approve token transfer to cover all possible scenarios
        ERC20(solutionBearAddress).approve(address(pancakeswapRouter), nativeAmount);
        ...
        if (ERC20(solutionBearAddress).balanceOf(address(this)) > 0)
            ERC20(solutionBearAddress).transfer(msg.sender, ERC20(solutionBearAddress).balanceOf(address(this)));
    }

    function addSolutionBearLiquidity(address baseTokenAddress, uint256 baseAmount, uint256 nativeAmount) external nonReentrant {
        ...
        uint256 dogsTokenBalance = ERC20(solutionBearAddress).balanceOf(address(this));

        if (baseTokenBalance > 0)
            ERC20(baseTokenAddress).safeTransfer(msg.sender, baseTokenBalance);

        if (dogsTokenBalance > 0)
            ERC20(solutionBearAddress).transfer(msg.sender, dogsTokenBalance);
    }

    function removeSolutionBearLiquidity(address baseTokenAddress, uint256 liquidity) external nonReentrant {
        address lpTokenAddress = IUniswapV2Factory(pancakeswapRouter.factory()).getPair(baseTokenAddress, solutionBearAddress);
     ...
     */
    function setSolutionBearAddress(address _solutionBearAddress) external onlyOwner {
        require(_solutionBearAddress != address(0), "_dogsTokenAddress is the zero address");
        require(solutionBearAddress != _solutionBearAddress, "dogsTokenAddress already set!");

        solutionBearAddress = _solutionBearAddress;
    }
