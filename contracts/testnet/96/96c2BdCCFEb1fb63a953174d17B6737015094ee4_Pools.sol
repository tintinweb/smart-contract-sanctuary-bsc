    ...
    function giveComm(uint amount, uint pid) internal {
        ...
        for(uint i = 0; i <= 7; i++) {
            ...
            if((i == 7 || from == _refferBy)) {
                ...
                uint comm = totalComm / (2 ** (i+1));
                if(i == 0) {
                    if(userTotalLock[_refferBy] > 0 && volumeOntree[_refferBy] >= conditionVolumeOnTree[i] * 1 ether) {
                        remainComm[_refferBy] += comm;
                        currentComm -= comm;
                    }
                ...
                else if(childs[_refferBy].downLine >= conditionMemOnTree[i] && volumeOntree[_refferBy] >= conditionVolumeOnTree[i] * 1 ether) {
                    remainComm[_refferBy] += comm;
                    currentComm -= comm;
