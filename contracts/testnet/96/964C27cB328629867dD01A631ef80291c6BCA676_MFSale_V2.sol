...
contract MFSale_V2 is MFSale {
    ...
    function _buy_V2(uint256[] memory quantity, address _investor) private {
    ...
    function buyForOwner_V2(PackData[] memory packData) external onlyOwner {
        for (uint256 i; i < packData.length; i++) {
            PackData memory data = packData[i];

            _buy_V2(data.quantity, data.investor);
        }
    }

    function recieveProfitOnBuyPhase() public {
        address seller = msg.sender;

        uint256 usdtProfit = getAvailableWithdrawnAmount();
        withdrawnAmount[seller] += usdtProfit;

        usdt.transfer(seller, usdtProfit);

        totalWithdrawnByInvestors += usdtProfit;
    }

    function getAvailableWithdrawnAmount() public view returns (uint256) {
        uint256 investorPart = (saleAmount[msg.sender] * decimals) /
            totalOnSale;
        uint256 investorSold = (totalSold * investorPart) / decimals;
        uint256 usdtProfit = investorSold *
            buybackPrice -
            withdrawnAmount[msg.sender];

        return usdtProfit;
    }

    function getAvailableWithdrawnAmountTest(address sender)
        public
        view
        returns (uint256)
    {
        uint256 investorPart = (saleAmount[sender] * decimals) / totalOnSale;
        uint256 investorSold = (totalSold * investorPart) / decimals;
        uint256 usdtProfit = investorSold *
            buybackPrice -
            withdrawnAmount[sender];

        return usdtProfit;
    }

    function getDeicmals() public view returns (uint256) {
        return decimals;
    }
}
...
contract MFSale is BaseDAO, BaseProxy {
    struct PackData {
        address investor;
        uint256[] quantity;
    }
    ...
    function buy(uint256[] memory quantity) external onlyOwner {
    ...
    function buyForOwner(PackData[] memory packData, address _payer)
        external
        onlyOwner
    {
        for (uint256 i; i < packData.length; i++) {
    ...
    function recieveProfit() external onlyOwner {
