...
contract PairManager is Ownable, IPoolFactory, IPoolManager {
  ...
  function registerAma(
    ...
    function updatePartnerInfo( string memory _name , string memory _username  ,uint256 _fees , bool _status , address _updatedAddress) public onlyAmaAddress{
        AmaPartner storage ama = amapartner[msg.sender];
        require(ama.ownerAddress != address(0) , "No Partner Avalible with Id");
        
        if(_updatedAddress == address(0) || _updatedAddress == msg.sender){
