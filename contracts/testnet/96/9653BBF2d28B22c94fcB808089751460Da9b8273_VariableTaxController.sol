...
contract VariableTaxController is OwnableUpgradeable {
    ...
    function getVariableTax(address _addr, uint256 _base_tax, uint256 _amount_to_claim, uint256 _amount_to_nitro) external view returns (uint256) {
        ...
        (,,,uint256 claim_backlog,) = _getUserStats(_addr, _amount_to_claim, _amount_to_nitro);
    ...
    function _getUserStats(address _addr, uint256 _amount_to_claim, uint256 _amount_to_nitro) internal view returns (uint256 nitro_amount, uint256 claim_amount, uint256 airdrop_amount, uint256 claim_backlog, uint256 nitro_lead){
        if(userStats[_addr].claim_amount.add(_amount_to_claim) > userStats[_addr].nitro_amount.add(userStats[_addr].airdrop_amount).add(_amount_to_nitro)){
            claim_backlog = userStats[_addr].claim_amount.add(_amount_to_claim).sub(userStats[_addr].nitro_amount.add(userStats[_addr].airdrop_amount).add(_amount_to_nitro));
        }else if(userStats[_addr].nitro_amount.add(userStats[_addr].airdrop_amount).add(_amount_to_nitro) > userStats[_addr].claim_amount.add(_amount_to_claim).add(_amount_to_nitro)){
            nitro_lead = userStats[_addr].nitro_amount.add(userStats[_addr].airdrop_amount).add(_amount_to_nitro).sub(userStats[_addr].claim_amount.add(_amount_to_claim));
        }
    ...
    function getUserStats(address _addr) external view returns(uint256 nitro_amount, uint256 claim_amount, uint256 airdrop_amount, uint256 claim_backlog, uint256 nitro_lead){
        return _getUserStats(_addr, 0, 0);
    }
