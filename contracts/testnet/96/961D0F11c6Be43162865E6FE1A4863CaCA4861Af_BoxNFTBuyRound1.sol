...
function checkBuyFlashSaleBoxWeb(address sender) public view returns(bool whitelistCanBuy, bool stakingCanBuy){ 
  uint totalBoughtFlashSaleBox=getTotalBoughtFlashSaleBox(sender);
  uint totalStaking=getTotalStaking(sender);
  uint totalCanBuyBox=0;
  if(indexOfWhitelist[sender] != 0 && totalBoughtFlashSaleBox < 1){
    whitelistCanBuy= true;
  }else{
    whitelistCanBuy= false;
  }
  if(totalStaking >= 1000000 && totalStaking <= 9999999){
    totalCanBuyBox=20;
  }else if(totalStaking >= 100000 && totalStaking <= 999999){
...
function getTotalBoughtBox(address sender) public view returns(uint){ 
  uint totalBoughtBox=0;
  for(uint32 i=1;i<=5;i++){
    uint totalBuyByRanks=_buyByRanks[i].current();
    for(uint32 x=1;x<=totalBuyByRanks;x++){
      if(boxes[sender][x].id!=0){
        totalBoughtBox++;
      }
    }
  }
  return totalBoughtBox;
}
