...
struct User {
	...
	return uniqueContractuser[index].userAddress;
     ...
     function withdrawunSoldToken() public onlyOwner {
		uint256 availableToken = IERC20(tokenContract).balanceOf(address(this)).sub(totalTokenSell);
...
isallow=1;
      ...
      uint256 price=plans[currentPresaleindex-1].rate;
            ...
            token=token.div(1000000000000000000).mul(10**decimalContract);
...
return dividend.sub(withdrawAmount);
   ...
   function withdraw() public {
		...
		IERC20(tokenContract).transfer(msg.sender,totalAmount);
