...
contract FixedAPRSoloStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address _tokenAddress, uint256 _apr, uint256 _lockDurationInSeconds, uint256 _exitPenaltyPerc) {
        stakingToken = IBEP20(_tokenAddress);
        rewardToken = IBEP20(_tokenAddress);

        apr = _apr;
        lockDuration = _lockDurationInSeconds * 1 seconds;
    ...
    function calculateNewRewards() public view returns (uint256) {
        ...
        return (((block.timestamp - pool.lastRewardTimestamp) * totalStaked) * apr / 100 / 365 seconds);
    }
    ...
    function updateLockDuration(uint256 secondsForLock) external onlyOwner {
        require(secondsForLock <= 365, "Lock must be 365 seconds or less.");
        lockDuration = secondsForLock * 1 seconds;
    }
