...
///@notice Swap assets cross-chain using 0x for first leg (from chain) and PancakeSwap for second leg (to chain)
contract SwapXcBsc is ISwap, ZetaInteractor, ZetaReceiver, Pausable {
    ...
    IUniswapV2Router02 public immutable iSwapRouter; // PancakeSwap Router
    ...
    constructor(
        ...
        iSwapRouter = IUniswapV2Router02(_swapRouter);
        iWETH9 = _WETH9;
        sZeroEx = _zeroEx;
        ...
        isValidMessageCall(_zetaMessage)
    {
        ...
        if (xcMessage.toToken == iZetaToken) {
            ...
            //If the out token is not Zeta, get it using PancakeSwap
            address[] memory path;
            if (xcMessage.toToken == iWETH9 || xcMessage.toToken == NATIVE) {
                path = new address[](2);
                path[0] = iZetaToken;
                path[1] = iWETH9;
            } else {
            ...
            if (xcMessage.toToken == NATIVE) {
                amounts = iSwapRouter.swapExactTokensForETH(
                    _zetaMessage.zetaValue,
                    xcMessage.toTokenMinAmount,
                    path,
                    recipientAddress,
                    block.timestamp + sMaxDeadline
                );
            } else {
                amounts = iSwapRouter.swapExactTokensForTokens(
                    _zetaMessage.zetaValue,
                    xcMessage.toTokenMinAmount,
                    path,
                    recipientAddress,
            ...
            toTokenFinalAmount = amounts[amounts.length - 1];
        ...
        isValidRevertCall(_zetaRevert)
    {
        ...
        if (xcMessage.revertToken == iZetaToken) {
            ...
            }
            {
                ...
                revertTokenFinalAmount = amounts[amounts.length - 1];
...
interface ISwap is ISwapXcEvents, ISwapXcErrors {}
