...
contract SpoilsOfWar is Context, Ownable {
    ...
    function computePayout(address _addr) view external returns(uint256 value) {
		...
		uint256 perc_add = SafeMath.div(player.downlines1.length,100) + SafeMath.div(player.total_battle_score, 100000);			
		for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
				...
				if(winning == 0){
					value += dep.amount * (to - from) * ((tarif.percent / tarif.life_days) + perc_add + levelno) / 8640000;	
				}else if(winning == player.fleet){
			        value += dep.amount * (to - from) * ((300 / tarif.life_days) + perc_add + levelno) / 8640000;	
				}else{
			        value += dep.amount * (to - from) * ((200 / tarif.life_days) + perc_add + levelno) / 8640000;	
				}				
    ...
    function setLevel(uint newval) public onlyOwner returns (bool success) {
        levelno = newval;
        return true;
    }
