...
contract CrossSave is IAxelarExecutable {
    ...
    // CHAIN DETAILS
    // FTM
    uint256 public ftmChainId = 4002;
    string public ftmDestinationChain = "Fantom";
    string public ftmDestinationAddress;
    ...
    struct Savings {
        ...
        uint256 interval;
        uint256 stopTime;
        uint256 option; // 5 - LOCKED and 6 - FLEXIBLE
        bool exist;
        bool locked;
    ...
    function save(
        ...
        if (_option != 5 || _option != 6) {
            revert("wron option");
        }

        if (msg.value < (_estimateGasCost1 + _estimateGasCost2)) {
        ...
        if (!savings[msg.sender].exist) {
            ...
            uint256 actualSavings = msg.value -
                (_estimateGasCost1 + _estimateGasCost2);
    ...
    function breakSaveEarly(
        ...
        if (
            ...
            if (savings[msg.sender].option == 5) {
                revert("Locked");
            } else {
    ...
    function unlockSavings(uint256 _estimateGasCost1, uint256 _estimateGasCost2)
        public
        payable
    {
        if (
            ...
            if (savings[msg.sender].option == 5) {
                ...
                require(ok, "!ok");
            } else if (savings[msg.sender].option == 6) {
    ...
    function withdrawSavings() public {
        ...
        if (savings[msg.sender].chainConfirmations < 2) {
    ...
    function updateDestinationAddresses(
        ...
        ftmDestinationAddress = _ftmaddr;
