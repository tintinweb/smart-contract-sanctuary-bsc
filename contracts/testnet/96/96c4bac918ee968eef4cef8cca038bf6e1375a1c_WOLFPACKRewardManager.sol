...
/// Eligible accounts are rewarded with WOLFPACK tokens in accordance with the set reward specific to each incentivised operation.

contract WOLFPACKRewardManager is Ownable, ReentrancyGuard {

    IERC20 public WPACK;

    uint256 private predictionRew = 5000000 * 10**18;
    uint256 private liquidityRew = 5000000 * 10**18;
    uint256 private managementRew = 1500000 * 10**18;
    
    address[] private authorizedCallers;

    mapping(address => uint256) private rewardBalances;

    constructor(address _wolfpack) {
    ...
    function synchronizeRewardNotification(address rewardee, bool predictionQualified, bool liquidityQualified, bool managementQualified) public nonReentrant {
