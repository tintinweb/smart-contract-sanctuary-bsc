...
contract Four01Programe is Ownable {

    using IterableMapping for IterableMapping.Map;

    address public rematicFinanceAdminAddress;
    ...
    uint256 totalNumberOfCredits = 3;
    ...
    uint256 totalNumberOfWithrawCredits = 3;
    ...
    bool public passivePayoutLaunched = false;

    modifier onlyRematicFinanceAdmin() {
        require(rematicFinanceAdminAddress == address(msg.sender), "Message sender needs to be RematicFinanceAdmin Contract");
        _;
    }
    ...
    function setRematicFinanceAdmin(address _address) public onlyOwner {
        require(rematicFinanceAdminAddress != address(_address), "same address is already set");
        rematicFinanceAdminAddress = _address;
    }
    ...
    function startPassivePayoutLaunched() public onlyOwner {
        require(passivePayoutLaunched != false, "PassivePayoutLaunched is already started");
        passivePayoutLaunched = true;
    }

    function pausePassivePayoutLaunched() public onlyOwner {
        require(passivePayoutLaunched != true, "PassivePayoutLaunched is already started");
        passivePayoutLaunched = false;
    }

    function addUser401OptIn(address _address, uint256 percentage ) public onlyOwner {
        require(optInHoldersMap.get(_address) != percentage, "already optIn with same percetage");
        optInHoldersMap.set(_address, percentage);
        contributionTimestampMap[_address] = block.timestamp;
    }

    function removeUser401OptIn(address _address ) public onlyOwner {
        require(optInHoldersMap.get(_address) > 0, "already optout");
        optInHoldersMap.remove(_address);
    }
    ...
    function process(address _address, uint256 rewardCount, uint256 _standardAmount) external onlyDefaultTokenTracker {
        ...
        four01kAmountMap[_address] = rewardCount;
        four01kMAmountMap[_address] = mCount;
    ...
    function getUserPercentageIn401Programe(address _address) public view returns (uint256){
        return optInHoldersMap.get(_address);
    }

    function withraw401KReward(address _account) public onlyRematicFinanceAdmin {
        // withdraw process
        if(optInHoldersMap.get(_account) > 0){ // check if user is optIn
            uint256 four01kAmount = four01kAmountMap[_account];
            uint256 mAmount = four01kMAmountMap[_account];
            uint256 contributionLife = block.timestamp - contributionTimestampMap[_account];
            uint256 creditPercentage = 0;

            emit WithdrawReady(_account, four01kAmount, mAmount, contributionLife);
            if(contributionLife < indexWithdrawTimeMap[1]){
                mAmount = 0;
                four01kAmount = four01kAmount * (100 - 25) / 100;
            }else{
                for(uint256 i = 1; i <= totalNumberOfWithrawCredits; i++){
                    if(contributionLife >= indexWithdrawTimeMap[i]){
                        if(indexWithdrawCreditPercentageMap[i] > 0){
                            creditPercentage = indexWithdrawCreditPercentageMap[i];
                        }
                    }
                }
            }
            // initialize mcount, four01kamount
            ...
            emit WithdrawDone(_account, four01kAmount, mAmount, contributionLife);
        }else{
            emit Error("withdraw", _account, "This user is not a participant in 401k programe.");
        }
    }

    function payforLongtermcontributers() public onlyOwner {
        uint256 aMonthTime = 30 * 86400;
        uint256 aYearTime = 365 * 86400;
    ...
    function updateCreditPercentageMap(uint256 index,  uint256 minPercentage, uint256 creditPercentage) external onlyRematicFinanceAdmin {
        ...
        console.log("indexCreditPercentageMap[index] ==>", indexCreditPercentageMap[index]);
        console.log("indexMinimumContributionMap[index] ==>", indexMinimumContributionMap[index]);
    }
}
