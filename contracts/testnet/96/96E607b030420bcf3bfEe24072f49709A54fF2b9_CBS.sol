...
contract CBS is ERC20,Ownable,Backend{
    ...
    function mintToken (BackendSigner memory backend) external {
        uint _totalSupply=0;
            ...
            uint intoWei = backend.amount * (1 ether);
            if(iswei){
             _totalSupply = currentSupply/(10**18);
            }
            require (backend.bankBalance == _totalSupply+backend.amount,'!Bankbalance');
            nonceChecker[msg.sender][backend.timestamp] = true;
            iswei = true;
            currentSupply += intoWei;
            _mint(msg.sender, intoWei);
    }
