...
// Enhance V3PreriodLock and add whitelist to access
 contract dividendMasterV3Private is Ownable, ReentrancyGuard{
    ...
    event SetFeeToReserved(address indexed OldAddress,address indexed NewAddress);
    ...
    function initialize(
       ...
       if(_finishBlockCount == 0){
        ...
        if(_limitShares ==0){
            _limitShares = stakedToken.totalSupply();
        }
    ...
    */
    function addHolder(address _addressToWhitelist) public onlyOwner {
        whitelistedHolders[_addressToWhitelist] = true;
    }
    function removeHolder(address _addressFromWhitelist)public onlyOwner{
        if(whitelistedHolders[_addressFromWhitelist])
        {
            whitelistedHolders[_addressFromWhitelist] = false;

        }

    }
    function verifyHolder(address _whitelistedAddress) public view returns(bool) {
        bool holderIsWhitelisted = whitelistedHolders[_whitelistedAddress];
        return holderIsWhitelisted;
    }
    modifier isWhitelisted(address _address) 
    {
        require(whitelistedHolders[_address], "You need to be whitelisted");
        _;
    }

    /*
       Dividend external features 
    */
    function depositedBalance() public  view returns (uint256) {
        return stakedToken.balanceOf(address(this));
    }
    function depositShares(uint256 _amount) external nonReentrant isWhitelisted(msg.sender) {
        
        require(isPoolReady(), "dividendMaster::depositShares: Pool isn't ready to stake");
        require(_amount > 0, "dividendMaster::depositShares: Amount should more than 0"); 
        require(checkNotExceedShares(_amount), "dividendMaster::depositShares: Amount exceeds remain shares");
        require(!isInDepositLockPeriod(),"dividendMaster::depositShares: Pool is temperaly lock before dividend distribution"); 
    ...
    function removeShares(uint256 _amount) external nonReentrant isWhitelisted(msg.sender) {

        require(isEnablePool, "dividendMaster::removeShares: Pool is disable to widthdraw");
        UserInfo storage user = userInfo[msg.sender];
        ...
        require(_amount > 0 , "dividendMaster::removeShares: Amount should more than 0");
    ...
    function depositReward(uint256 _amount) external nonReentrant onlyOwner {
        require(rewardToken.balanceOf(msg.sender) >= _amount, "dividendMaster::depositReward: Insufficient balance");
    ...
    */
    function distributeDividend() external nonReentrant onlyOwner {
        ...
        require(!isInDividingPeriod(), "dividendMaster::distributeReward: Too early to distribute balance");
        uint256 _dvdtodist = dividendBalance();
        require(_dvdtodist>0, "dividendMaster::distributeReward: Dividend token amount must > 0");
    ...
    function distributeFee() external nonReentrant onlyOwner {
        require(pendingFees >= 0, "dividendMaster::collectFee: Insufficient balance");
        uint256 stkBalance = stakedToken.balanceOf(address(this));
        require(stkBalance >= 0, "dividendMaster::collectFee: Insufficient balance");
        ...
        xdvdToken.mint(dviReserved, toreserved);  
 ...
 */
    ...
    function adminDividendWithdraw(uint256 _amount) external nonReentrant onlyOwner {
        require(_amount > 0, "dividendMaster::adminWithdraw: _amount should be higher than 0");
        require(_amount <= dividendBalance(), "dividendMaster::adminWithdraw: _amount should be less than or equal the total remaining reward");
    ...
    require(stakedToken.balanceOf(address(this)) >= 0, "dividendMaster::collectFee: Insufficient balance");
        require(pendingFees >= 0, "dividendMaster::collectFee: Insufficient balance");
        uint256 stkBalance = stakedToken.balanceOf(address(this));
        if(pendingFees >= stkBalance){
            pendingFees = stkBalance;
        ...
        onlyOwner
    {
        require(
            ...
            "dividendMaster::recoverWrongTokens: Cannot be staked token"
        );
        require(
    ...
    */
    function checkNotExceedShares(uint256 newshare) public view returns (bool isNotExceed) {
