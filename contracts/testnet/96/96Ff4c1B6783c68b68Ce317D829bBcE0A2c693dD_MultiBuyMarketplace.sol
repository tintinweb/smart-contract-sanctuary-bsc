...
contract MultiBuyMarketplace is Ownable {
    ...
    function setTransactionFee(uint256 _transactionFee) public onlyOwner {
        ...
        pecentPayment = 1000 - transactionFee;
    }
    ...
    function matchTransaction(
        ...
        require(
            paymentTokens[tokenPayment] == true,
            "Marketplace: invalid payment method"
        );
        require(seller.length == tokenAsset.length && tokenAsset.length == tokenId.length && tokenId.length == price.length && price.length == nonce.length && signature.length == nonce.length, "Check Input");
        BlackList list = BlackList(blacklist);
        require(seller.length < 50, "Limit buy 50 assets");
        uint256 totalPrice = GetPrice(price);
        IERC20 paymentContract = IERC20(tokenPayment);
        require(
            paymentContract.balanceOf(_msgSender()) >= totalPrice,
            "Marketplace: buyer doesn't have enough token to buy this item"
        );
        require(
            paymentContract.allowance(_msgSender(), address(this)) >= totalPrice,
            "Marketplace: buyer doesn't approve marketplace to spend payment amount"
        );
        verifyMatch(tokenPayment, seller, tokenAsset, tokenId, price, nonce, signature);
        //
        BatchTransaction(tokenPayment, seller, tokenAsset, tokenId, price, nonce, signature, list, paymentContract);
        TransferFee(totalPrice, paymentContract);
        return true;
    }
    function BatchTransaction(
        ...
        for(uint256 i = 0; i < seller.length; i++){
            ...
            IERC721 nft = IERC721(tokenAsset[i]);
            require(
                nft.ownerOf(tokenId[i]) == seller[i],
                "Marketplace: seller is not owner of this item now"
            );
           
            require(list.getTokenId(tokenAsset[1],tokenId[i]) < block.timestamp, "TokenId in blacklist");
            // transfer item to buyer
            nft.safeTransferFrom(seller[i], _msgSender(), tokenId[i]);
            uint256 payToSellerAmount = pecentPayment.mul(price[i]).div(1000);
            paymentContract.safeTransferFrom(_msgSender(), seller[i], payToSellerAmount);
            usedSignatures[signature[i]] = true;
            // emit sale event
            emitEvent([seller[i], tokenAsset[i], tokenPayment], [tokenId[i], price[i], nonce[i]], transactionFee.mul(price[i]).div(1000));
    ...
    function verifyMatch(
        ...
        for(uint256 i = 0; i < seller.length; i ++){
            require( 
                nonce[i].add(period) > block.timestamp && nonce[2] < block.timestamp,
                "Marketplace: item sold out"
            );
            require(
                !usedSignatures[signature[i]],
            ...
            require(
                ECDSA.recover(ethSignedMessageHash, signature[i]) == seller[i],
                "Marketplace: invalid seller signature"
            );
        }
    }
    function GetPrice(uint256[] memory price) internal pure returns(uint256){
    ...
    function TransferFee(uint256 amount, IERC20 paymentContract) internal {
