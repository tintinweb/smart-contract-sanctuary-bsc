...
contract SvinEN is Context, IERC20, Ownable {
    ...
    uint256 private _supply_total       = 100 * 10**12 * 10**_decimals;
    uint256 private _supply_reflected   = (MAX - (MAX % _supply_total));
    string private  _name               = "SvinEN";
    string private  _symbol             = "SVIN1";
    ...
    uint256 public _fee_dev_convert_limit = _supply_total * 1 / 10000;

    // Minimum Balance to maintain
    uint256 public _fee_marketing_min_bal = _supply_total * 2 / 100;
    uint256 public _fee_dev_min_bal = _supply_total * 2 / 100;
    ...
    // for burns and devs
    uint256 public _fee_dev = 50;
    uint256 private _fee_dev_old = _fee_dev;
    address payable private _wallet_dev = payable(0x401ac53e1623e26c498b9bD6dFe0f840BD37B790);

    // for burn
    uint256 public _fee_burn = 25;
    uint256 private _fee_burn_old = _fee_burn;
    address payable private _wallet_burn = payable(0xBB937ee619fC26a674C9a8e3F06B2494025b05eA);

    // for marketing
    uint256 public _fee_marketing = 250;
    uint256 private _fee_marketing_old = _fee_marketing;
    address payable private _wallet_marketing = payable(0x144018D2557C75222B0566B9169d3DE04Ef39F3E);
    ...
    uint256 public _maxWalletToken = ( _supply_total * 2 ) / 100;
    ...
    uint256 public _maxTxAmount =  ( _supply_total * 1 ) / 100;
...
/*  CORE INTERFACE FUNCTION */
    ...
    function ___wallets () public view returns(
        ...
        uint256 Dev_Fee_Minimum_Balance,
        uint256 Burn_Fee,
        ...
        return ( _fee_reflection, _fee_liquidity,
            ...
            _fee_dev,_fee_dev_convert_limit, _fee_dev_min_bal,
            _fee_burn,
            _wallet_marketing, _wallet_burn, _wallet_dev);
    }
...
// Reflect Finance core code
    ...
    function _getValues(uint256 tAmount) private view returns (
        ...
        uint256 tTransferAmount,uint256 tBurn, uint256 tDev, uint256 tLiquidity, uint256 tMarketing, uint256 tReflection) {
        tBurn           = ( tAmount * _fee_burn ) / _fee_denominator;
        tDev       = ( tAmount * _fee_dev ) / _fee_denominator;
        tLiquidity      = ( tAmount * _fee_liquidity ) / _fee_denominator;
        tMarketing           = ( tAmount * _fee_marketing ) / _fee_denominator;
        tReflection     = ( tAmount * _fee_reflection ) / _fee_denominator;

        tTransferAmount = tAmount - (tBurn + tDev + tLiquidity + tMarketing + tReflection);
...
// Made all parameters in alphabetical order
    function _setAllFees(uint256 burnFees, uint256 devFee, uint256 liquidityFees, uint256 marketingFee, uint256 reflectionFees) private {
        _fee_burn           = burnFees;
        _fee_dev        = devFee;
    ...
    function set_All_Fees_Triggers(uint256 dev_fee_convert_limit, uint256 marketing_fee_convert_limit) external onlyOwner {
        _fee_dev_convert_limit      = dev_fee_convert_limit;
        _fee_marketing_convert_limit         = marketing_fee_convert_limit;   
    }

    function set_All_Fees_Minimum_Balance(uint256 dev_fee_minimum_balance, uint256 marketing_fee_minimum_balance) external onlyOwner {
        _fee_marketing_min_bal       = marketing_fee_minimum_balance;
        _fee_dev_min_bal    = dev_fee_minimum_balance;
    }
    ...
    function set_All_Fees(uint256 Marketing_Fee, uint256 Burn_Fees, uint256 Liquidity_Fees, uint256 Reflection_Fees, uint256 DevFee) external onlyOwner {
        uint256 total_fees = Burn_Fees + DevFee + Liquidity_Fees +  Marketing_Fee + Reflection_Fees;
        require(total_fees < 4000, "Cannot set fees this high, pancake swap will hate us!");
        _setAllFees( Burn_Fees, DevFee, Liquidity_Fees, Marketing_Fee, Reflection_Fees);
    }
    ...
    function restoreAllFee() private {
        _setAllFees(_fee_burn_old, _fee_dev_old, _fee_liquidity_old, _fee_marketing_old, _fee_reflection_old);
    }
...
// Liquidity functions
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to != owner() && to != address(this)  && to != address(deadAddress) && to != uniswapV2Pair && to != _wallet_dev && to != _wallet_marketing){
        ...
        if(!inSwapAndLiquify && from != uniswapV2Pair){
            ...
            _fees_to_bnb(_fee_dev_convert_limit,_wallet_dev, _fee_dev_min_bal);
        }
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rReflection, uint256 tTransferAmount, uint256 tBurn, uint256 tDev, uint256 tLiquidity, uint256 tMarketing,  uint256 tReflection) = _getValues(amount);
