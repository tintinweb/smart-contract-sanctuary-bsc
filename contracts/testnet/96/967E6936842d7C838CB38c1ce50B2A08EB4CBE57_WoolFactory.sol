...
contract WoolFactory is Pausable, Whitelist, ReentrancyGuard {
    ...
    event onAddItem(address indexed caller, uint256 tier, address item, uint256 timestamp);
    event onSetRewardsToken(address indexed caller, address token, uint256 timestamp);

    event onRescueTokens(address indexed caller, address token, address recipient, uint256 amount, uint256 timestamp);
    ...
    function priceOf(uint256 _tier) public pure returns (uint256) {
    ...
    function claimTokens(address _user) whenNotPaused() onlyWhitelisted() nonReentrant() external returns (uint256) {
        ...
        if (claimable > 0) {
            ...
            rewardsToken.transfer(_user, claimable);
        }
    ...
    function addItem(uint256 _tier, address _item) external whenPaused() onlyOwner() {
        require(_item != address(0), "INVALID_ADDRESS");
        
        watchedContracts[_tier] = _item;

        emit onAddItem(msg.sender, _tier, _item, block.timestamp);
    }
    ...
    function setRewardsToken(address _newToken) public whenPaused() onlyOwner() {
        ...
        emit onSetRewardsToken(msg.sender, _newToken, block.timestamp);
    }

    // Rescue tokens from this contract (EMERGENCY ONLY)
    function rescueTokens(address _token, address _recipient, uint256 _amount) public onlyOwner() {
        IERC20(_token).transfer(_recipient, _amount);

        emit onRescueTokens(msg.sender, _token, _recipient, _amount, block.timestamp);
    }
