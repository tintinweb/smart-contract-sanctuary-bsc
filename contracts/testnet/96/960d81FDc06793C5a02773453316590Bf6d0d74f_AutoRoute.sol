...
contract AutoRoute is OwnableUpgradeable {
    ...
    function getAmountsOut(
        ...
        returns (uint256[] memory amounts)
    {
        require(paths.length >= 2, "AutoRoute: INVALID_PATHS");
    ...
    {   
        ...
        for (uint256 i = paths.length - 1; i > 0; i--) {
            ...
            if (compareStrings(poolType[i - 1], "AMM")) {
                address[] memory _paths = new address[](2);
                _paths[1] = paths[i];
                _paths[0] = paths[i - 1];
            ...
            } else if (compareStrings(poolType[i - 1], "DMM")) {
                IERC20[] memory _paths = new IERC20[](2);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        address fromToken = paths[0];
        address toToken = paths[paths.length - 1];
        uint256 amountIn_ = amounts[0];
        uint256 amountOut_ = amounts[amounts.length - 1];

        emit Swap(fromToken, toToken, amountIn_, amountOut_);
