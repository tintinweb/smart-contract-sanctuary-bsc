...
contract FurHintHelpers is BaseContract, FurLiquityBase {
        ...
        while (currentVaultuser != address(0) && remainingFURUSD > 0 && _maxIterations-- > 0) {
            ...
            if (netFURUSDDebt > remainingFURUSD) {
                if (netFURUSDDebt > MIN_NET_DEBT) {
                    ...
                    uint FUR = vaultManager.getVaultColl(currentVaultuser)
                        .add(vaultManager.getPendingFURReward(currentVaultuser));

                    uint newColl = FUR.sub(maxRedeemableFURUSD.mul(DECIMAL_PRECISION).div(_price));
