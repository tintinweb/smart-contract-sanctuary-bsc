...
contract ERC1155Demo is Initializable
  ...
  , uint256 _amount
    ...
    if (_from != msg.sender || !this.isApprovedForAll(_from, msg.sender)) {
      revert MaxSplaining({
        reason: "Max1155: not owner or approved for all"
      });
    }
  ...
  , uint256[] memory _amounts
    ...
    if (_ids.length != _amounts.length) {
      revert MaxSplaining({
        reason: "Max1155LZ: ids/amounts mismatch"
      });
    }    
    if (trustedRemoteLookup[_chainId].length == 0) {
      revert MaxSplaining({
        reason: "Max1155LZ: TR not set"
      });
    }
    ...
    if (messageFee > msg.value) {
      revert MaxSplaining({
        reason: "Max1155LZ: msg.value too low for txn"
      });
    }
...
abstract contract Max1155UUPS2981LZ is Initializable
  ...
  , bytes memory data
    ...
    if (to.isContract()) {
      try IERC1155TokenReceiver(to).onERC1155Received(operator, from, id, amount, data)
        ...
        if (reason.length == 0) {
          revert MaxSplaining({
            reason: "Max1155: transfer to non-ERC1155Receiver catch reason.length == 0"
          });
        } else {
          assembly {
  ...
  , bytes memory data
    ...
    if (to.isContract()) {
      try IERC1155TokenReceiver(to).onERC1155BatchReceived(operator, from, id, amount, data)
        ...
        if (reason.length == 0) {
          revert MaxSplaining({
            reason: "Max1155: transfer to non-ERC1155BatchReceiver catch reason.length == 0"
          });
        } else {
          assembly {
            revert(add(32, reason), mload(reason))
  ...
  , bytes memory _data
    ...
    if (_to == address(0)) {
      revert MaxSplaining({
        reason: "Max1155: mint to address(0)"
      });
    }
  ...
  , bytes memory _data
    ...
    if (len != _amounts.length) {
      revert MaxSplaining({
        reason: "Max1155: ids/amounts mismatch"
      });
    }
    address sender = msg.sender;
  ...
  , bytes memory _data
    ...
    if (_from == address(0)) {
      revert MaxSplaining({
        reason: "Max1155: burn from address(0)"
      });
    }
    address sender = msg.sender;
  ...
  */
  ...
  , uint256[] calldata _ids
    ...
    if (_owners.length != _ids.length) {
      revert MaxSplaining({
        reason: "Max1155: _owners/_ids mismatch"
      });
    }
  ...
  */
  ...
  , bytes calldata _data
    ...
    returns(bytes4) {
      revert MaxSplaining({
        reason: "Max1155: No tokens to minter"
      });
    }
  ...
  */
  function uri(
    ...
    if (!token1155.tokenExists(_id)) {
      revert MaxSplaining({
        reason: "Max1155: Token id non-existant"
      });
    } else {
  ...
  , bytes memory _payload
    ...
    if (msg.sender != address(endpoint)) {
      revert MaxSplaining({
        reason: "Max1155LZ: Not from endpoint"
      });
    }
    ...
    if (
      ...
      revert MaxSplaining({
        reason: "Max1155LZ: Not from trusted remote"
      });
    }
  ...
  , bytes memory _payload
    ...
    if (msg.sender != address(this)) {
      revert MaxSplaining({
        reason: "Max1155LZ: internals only for try/catch"
      });
    }
  ...
  , bytes calldata _payload
    ...
    if (failedMsg.payloadHash == bytes32(0)) {
      revert MaxSplaining({
        reason: "Max1155LZ: Message not stored"
      });
    }
    if (
      ...
      revert MaxSplaining({
        reason: "Max1155LZ: Wrong message"
      });
    }
