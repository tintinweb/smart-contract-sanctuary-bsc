...
contract VestingPlan{
    ...
    function setAddress(address []  memory  _address, uint256[] memory monthlyAmt, uint256[] memory totalCycle) public 
    {
      ...
      for(i=0;i<_address.length;i++)
      {
        if(accounts[_address[i]].id==0)
        {
            Account memory user = Account({
              ...
              cycleEnd:block.timestamp+(totalCycle[i]*30*24*60*60),
