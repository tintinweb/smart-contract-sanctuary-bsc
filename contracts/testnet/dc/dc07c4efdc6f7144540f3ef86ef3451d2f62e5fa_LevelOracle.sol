...
contract LevelOracle is Ownable, ILevelOracle {
    ...
    function configToken(
        ...
        require(chainlinkDeviation != 0 && chainlinkDeviation < PRECISION, "PriceFeed:invalidChainlinkDeviation");
    ...
    function _getPrice(address token, bool max) internal view returns (uint256) {
        ...
        (uint256 refPrice, uint256 lowerBound, uint256 upperBound) = _getReferencePrice(token);
        ...
        if (lastPrice > upperBound) {
            return max ? lastPrice : refPrice;
        }
        if (lastPrice < lowerBound) {
            return max ? refPrice : lastPrice;
        }
    ...
    function _getReferencePrice(address token)
        internal
        view
        returns (uint256 refPrice, uint256 lowerBound, uint256 upperBound)
    {
        TokenConfig memory config = tokenConfig[token];
