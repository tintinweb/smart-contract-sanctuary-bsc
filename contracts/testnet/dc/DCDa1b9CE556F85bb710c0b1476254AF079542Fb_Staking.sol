...
contract Staking is Ownable, Pausable {
    ...
    address public airDropContract = 0x5f822e69D5EbDEe73F60Ad4Aa8d821A15feea51F;

    uint256[] public refReward = [10, 5, 5];
    uint256 public rewardTokenRateUSD = 1000000000000000000;
    ...
    function deposit(uint256 _pid, uint256 _amount) public whenNotPaused {
        ...
        if (userContract != address(0)) {
            ...
            if (_ref != address(0)) {
                uint256 totalUSDDeposit = _amount.mul(package.rateUSD).div(1e18);
                // add reward f0
                commissionContract.add(_ref, totalUSDDeposit.mul(refReward[0]).div(100).div(rewardTokenRateUSD).mul(1e18));
                address ref = IUser(userContract).getRef(_ref);
                for (uint256 i = 1; i < refReward.length; i++) {
                    if (ref != address(0)) {
                        // add reward to Fn
                        commissionContract.add(ref, totalUSDDeposit.mul(refReward[i]).div(100).div(rewardTokenRateUSD).mul(1e18));
    ...
    function depositBNB(uint256 _pid) public whenNotPaused payable {
        ...
        if (userContract != address(0)) {
            address _ref = IUser(userContract).getRef(msg.sender);
            if (_ref != address(0)) {
                uint256 totalUSDDeposit = msg.value.mul(package.rateUSD).div(1e18);
                // add reward f0
                commissionContract.add(_ref, totalUSDDeposit.mul(refReward[0]).div(100).div(rewardTokenRateUSD).mul(1e18));
                address ref = IUser(userContract).getRef(_ref);
                for (uint256 i = 1; i < refReward.length; i++) {
