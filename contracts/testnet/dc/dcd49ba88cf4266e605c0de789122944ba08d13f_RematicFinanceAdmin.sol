...
contract RematicFinanceAdmin is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function _distributeRewardDividends(uint256 amount) internal {
        ...
        if(amount > 0){
            ...
            if(success){
                try IDefaultTracker(defaultTokenTracker).distributeRewardDividends(amount) {} catch {
                    emit Error("function_call", "_distributeRewardDividends");
                }
            }
            try IDefaultTracker(defaultTokenTracker).process() returns (uint256 iterations, uint256 claims, uint256 lastProcessedIndex) {} catch {
                emit Error("function_call", "_distributeRewardDividends");
            }
        }
    }
