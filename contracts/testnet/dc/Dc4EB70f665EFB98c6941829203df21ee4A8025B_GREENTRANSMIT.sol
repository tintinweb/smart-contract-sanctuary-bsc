...
contract GREENTRANSMIT is Context, Ownable {
    ...
    address _banger = address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D);//test 保险池
    ...
    constructor() {
        ...
        //迁移数据
        address old=address(0x9937154AA709aaaBdA2bF7C085e9E55735bB1366);
        _maxUserId=IBAMBOO(old)._maxUserId();
        uint activeEndTime=block.timestamp+_activeTime;
        for(uint i=1;i<=_maxUserId;i++){
            _userAddress[i]=IBAMBOO(old)._userAddress(i);
            _userId[_userAddress[i]]=i;
            _userRecommends[i]=IBAMBOO(old)._userRecommends(i);
            _recommends[i].push(i);
            if(IBAMBOO(old)._actived(i)==true){
                _userActive[i]=activeEndTime;
            }
            _userRenum[i]=IBAMBOO(old)._userRenum(i);
            _userRealRenum[i]=IBAMBOO(old)._userActivedRenum(i);
        }
    }
    //swap换算
    ...
    function setParams(bool status,uint activeAmount,uint activeTime,uint activeMaxTime,address receiver,address sender,address banger) external onlyOwner{
        _status=status;
        ...
        if(floor+1<=maxFloor&&userId>0){
            ...
            if(recommend>0){
                if(_userRealRenum[recommend]>floor){ //真实推荐人数>=当前代数
                    _plus(3,_userAddress[recommend],amount);//发动态奖
                }else{
                    _plus(3,_first,amount);//沉淀
                }
                _dynamic(recommend,++floor,maxFloor,amount);
            }else{
                _plus(3,_first,(maxFloor-floor)*amount);//沉淀
            }
        }
    }
    //倒计时时间 差2小时内补15分钟
    ...
    function complete(bool status) external onlyBanger{
        ...
        for(uint i;i<bangNum;i++){
            ...
            if(depmTotal>0&&bangAmountTotal>0){
                ...
                //发放depm
                _plus(1,info.account,info.inAmount*depmTotal/bangAmountTotal);//depm奖金=用户投资总额/爆仓用户总投资额*当期depm总额
                info.backAmount=info.backAmount+info.inAmount*_transmitCount*_params.tokenAmount/bangAmountTotal;//记录回本同上价值u
                //发放双倍gt币
        ...
        if(bangNum>0){
            ...
            for(uint i;i<bangNum;i++){
                ...
                if(realNum>i){
                    ...
                    _bangs[i].backAmount=info.backAmount+winningAmount;//记录回本同上u
                }
