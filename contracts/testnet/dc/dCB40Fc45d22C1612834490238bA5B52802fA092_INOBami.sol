...
contract INOBami is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    //uint256 userTotalPrice;
    
    //mapping
    mapping(address => bool) private priorWhiteList;
    mapping(address => bool) private fCFSWhiteList;
    mapping(address => uint256) private userStakeLevel;
    mapping(address => uint256) private NftQuantityPerUser;
    ...
    event WithDrawTokenComplete(
        uint256 userStakeLevel, 
        uint256 amountWithDraw
    );

    constructor(address _devWallet, address _nftAddress, address _tokenAddress, uint256 _price){
        devWallet = _devWallet;
        nftAddess = _nftAddress;
        tokenAddress = _tokenAddress;
        Price = _price;
    }
    function setAmountNftToDraw() private{
        amountNftToDraw = IERC721(nftAddess).balanceOf(address(this));
    }
function setLevelStake(uint256[] calldata _amountOfLevelStake, uint256[] calldata _maxNumberOfNftToBuy) public onlyOwner {
    ...
    function setPrice(uint256 _price) public {
        Price = _price;
        emit SetNewPrice(Price);
    }
    
    function BuyNft(uint256 purchaseQuantity) public {
        uint256 checkQuantity = NftQuantityPerUser[msg.sender];
        uint256 userTotalPrice;
        require(block.timestamp >= startTimeBuy,"NFT is not open for sale yet");
        require(block.timestamp <= endTimeBuy,"Time ended");
        require(priorWhiteList[msg.sender] == true, "You not in whitelist, please staking NFT for joined whitelist");
        LevelStake storage levelStake = levelStakeList[userStakeLevel[msg.sender]];
        ...
        NftQuantityPerUser[msg.sender] += purchaseQuantity;
        userTotalPrice = NftQuantityPerUser[msg.sender] * Price;
        amountNftToDraw-=purchaseQuantity;
        emit BuyNftComplete(purchaseQuantity, msg.sender, userTotalPrice, amountNftToDraw);
    }
    ...
    function ClaimNft() external nonReentrant{
        ...
        require(fCFSWhiteList[msg.sender]==true || priorWhiteList[msg.sender]==true, "You are not in white list");
        require(isClaimed[msg.sender]==false, "You have claimed");
        if(fCFSWhiteList[msg.sender]==true){
            require(amountNftToDraw > 0, "Out of stock");
            require(IERC20(tokenAddress).balanceOf(msg.sender)>=Price, "Not enough balance to pay");
            IERC20(tokenAddress).safeTransferFrom(msg.sender,devWallet,Price);
        ...
        if(priorWhiteList[msg.sender]==true){
            require(IERC20(tokenAddress).balanceOf(msg.sender)>=NftQuantityPerUser[msg.sender] * Price, "Not enough balance to pay");
IERC20(tokenAddress).safeTransferFrom(msg.sender,devWallet,NftQuantityPerUser[msg.sender] * Price);

            uint256[] memory arrayNftOfPrior = new uint256[](NftQuantityPerUser[msg.sender]);
            for(uint index = 0; index < arrayNftOfPrior.length; index++){
                arrayNftOfPrior[index] = ERC721Enumerable(nftAddess).tokenOfOwnerByIndex(address(this), index);
}
            for(uint jndex = 0; jndex < arrayNftOfPrior.length; jndex++){
                IERC721(nftAddess).transferFrom(address(this),msg.sender,arrayNftOfPrior[jndex]);
            }
            isClaimed[msg.sender] = true; 
        }
        emit ClaimNftComplete(msg.sender, isClaimed[msg.sender], devWallet);
    }
    ...
    function Staking(uint256 _levelStakeId) external nonReentrant{
        require(fCFSWhiteList[msg.sender] == false, "You are already on the FCFS whitelist");
        require(priorWhiteList[msg.sender] == false, "You are already on the Prior whitelist");
LevelStake storage levelStake = levelStakeList[_levelStakeId];
        ...
        if(maxNumberOfNftToBuy == 0)
        {
            fCFSWhiteList[msg.sender] = true;
        }
        else {
            priorWhiteList[msg.sender] = true;
        }
        userStakeLevel[msg.sender] = _levelStakeId;
        emit StakingComlete(_levelStakeId, amountOfLevelStake, maxNumberOfNftToBuy);
    }

    function viewBlance() public view returns(uint256 amountNft)
    {
        //amountNft = IERC721(nftAddess).balanceOf(address(this));
        amountNft = amountNftToDraw;
    }
    
    function withDrawNft() public onlyOwner nonReentrant {
...
emit WithDrawNftComplete(amountNft, arrayNftId[index]);
        ...
        for(uint256 i = 0; i < arrayNftId.length; i++)
        {
            IERC721(nftAddess).transferFrom(address(this), devWallet, arrayNftId[i]);
        }
        
    }

    function withDrawTokenStaking() public nonReentrant {
        require(block.timestamp > endTimeLock, "INO is locking");
        require(fCFSWhiteList[msg.sender] == false, "You can't withdraw");
        LevelStake storage levelStake = levelStakeList[userStakeLevel[msg.sender]]; 
        uint256 amountWithDraw = levelStake.amountOfLevelStake;
        IERC20(tokenAddress).safeTransfer(msg.sender, amountWithDraw);
        emit WithDrawTokenComplete(userStakeLevel[msg.sender], amountWithDraw);
    }

    function viewAmountWithDrawToken() public view returns(uint256 _amountWithDraw)
    {
        LevelStake storage levelStake = levelStakeList[userStakeLevel[msg.sender]]; 
