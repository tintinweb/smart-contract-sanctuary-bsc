...
contract LockedStaking is Ownable, ReentrancyGuard {
    ...
    function addTranche(
        ...
        if (rewardTokensToTransfer > 0) {
            IBEP20(stakeTokenAddr).transferFrom(_msgSender(), address(this), rewardTokensToTransfer.mul(DECIMALFACTOR));
        ...
        inStatus(_tid, Status.ACTIVE)
    {
        ...
        IBEP20(stakeTokenAddr).transferFrom(_msgSender(), address(this), _amount.mul(DECIMALFACTOR));
    ...
    function exitStaking(uint32 _tid) external validTranche(_tid) expiredTranche(_tid) noReentry {
        ...
        IBEP20(stakeTokenAddr).transfer(_msgSender(), amountToTransfer.mul(DECIMALFACTOR));
    ...
    function closeTranche(uint32 _tid) external inStatus(_tid, Status.EXPIRED) onlyOwner noReentry {
        ...
        for (uint256 index = arrLength; index > 0; index--) {
            ...
            totalAmount = mapUserInfo[addr][_tid].amount.add(totalRewards).mul(DECIMALFACTOR);
            IBEP20(stakeTokenAddr).transfer(addr, totalAmount);
    ...
    function closeStaking() external onlyOwner {
        ...
        IBEP20(stakeTokenAddr).transfer(owner(), currentBalance);
