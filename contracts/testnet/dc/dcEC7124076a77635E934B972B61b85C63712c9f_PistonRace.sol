...
contract PistonRace is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        pistonToken = IToken(address(0xa7fb4C2ACe237f0FbAeD33C92Cb1c34f66FDAF99)); // PISTON Token TESTNET
        tokenMint = ITokenMint(address(0xa7fb4C2ACe237f0FbAeD33C92Cb1c34f66FDAF99)); // PISTON Token TESTNET
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        // record user new deposit
        usersRealDeposits[_addr].deposits += _amount;
        if(STORE_BUSD_VALUE){
            usersRealDeposits[_addr].deposits_BUSD += pistonTokenPriceFeed.getPrice(_amount.div(1 ether)); // new cash in BUSD
        }
    ...
    function _refPayout(address _addr, uint256 _amount, uint256 _refBonus) internal {
        ...
        for(uint8 i = 0; i < ref_depth; i++) {
            ...
            if(users[_addr].ref_claim_pos == i) {
                ...
                users[_addr].rolls < this.maxRollOf(usersRealDeposits[_addr].deposits)){
    ...
    function _roll(address _addr) internal {
        ...
        uint256 roll_amount_final = this.rollAmountOf(_addr, payout_taxed);
    ...
    function rollAmountOf(address _addr, uint256 _toBeRolledAmount) view external returns(uint256 rollAmount) {
        
        //validate the total amount that can be rolled is 5x the users real deposit only.
        uint256 maxRollAmount = this.maxRollOf(usersRealDeposits[_addr].deposits);

        if(users[_addr].rolls < maxRollAmount) {

            rollAmount = _toBeRolledAmount;

            //get the net positive amount available for roll.
            if(users[_addr].rolls.add(rollAmount) > maxRollAmount) {
                rollAmount = maxRollAmount.sub(users[_addr].rolls);
            }
        }else{
    ...
    function maxRollOf(uint256 _amount) view external returns(uint256) {
    ...
    function eject() external {
        User memory user = users[msg.sender]; // user statistics
    ...
    function _checkNickname(string memory name) private view returns (bool){
        ...
        if(bytes(name).length > MAX_LENGTH_NICKNAME){
            return false; // too long allowable string
        }
