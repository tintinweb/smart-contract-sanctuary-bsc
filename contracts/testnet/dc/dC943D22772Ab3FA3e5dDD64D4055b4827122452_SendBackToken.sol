...
contract SendBackToken {
    ...
    /// @dev Received event fired when ETH/BNB is received by contract
    event Received(address indexed from, uint256 tokens);
    /// @dev Minted event fired when tokens are minted
    event Minted(address indexed from, uint256 tokens);
