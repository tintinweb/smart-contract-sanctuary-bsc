...
contract AdvancedMultiswap is Ownable{
    ...
    /**
        ...
        1. swapExactETHForTokens(0,  [WBNB, USDT], owner(), GMT Sunday October 8 2023 10:20:02 PM )
            ...
            contractAddressArray: [0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3] [RouterPancake]
            dataArray: 
    ...
    */
    function swapPayable(address[] memory contractAddressArray, bytes[] memory dataArray, uint availableGas) external payable onlyOwner{
        require(contractAddressArray.length > 0 && contractAddressArray.length == dataArray.length, "Invalid input");

        for(uint256 i; i < contractAddressArray.length; i++) {
            (bool success, bytes memory returndata) = contractAddressArray[i].call{value: msg.value, gas: availableGas}(dataArray[i]);
            //contractAddressArray[i].call{value: address(this).balance, gas: gasleft()}(dataArray[i]);
            //contractAddressArray[i].call{value: msg.value, gas: gasleft()}(dataArray[i]);
    ...
    /**
        ...
        1. swapExactTokensForTokens(500000000000000000 (0.5USDT), 0, [0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684, 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7], owner(), GMT Sunday October 8 2023 10:20:02 PM)
            ...
            And finally, remember to privide the ERC20 input token to this contract beforehand, because there is not .transferFrom in this
            function. So for example, if the input token of the swap is USDT with address 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684, please send
            sufficient amount to this contract so that it can be taken by the Router.
     */
    function swap(address[] memory contractAddressArray, bytes[] memory dataArray, uint availableGas) external onlyOwner{
        require(contractAddressArray.length > 0 && contractAddressArray.length == dataArray.length, "Invalid input");

        // This function is not payable, so it is not suitable for swaps that has ETH as input, but it is usable for
        // swaps which has ERC20 tokens as input. The important thing here is that before the swap the ERC20 token 
        ...
        // function is exclusively focused on doing SWAPS with ERC20 tokens as input. If this is the case, then yes,
        // adding the .transferFrom to this function is optimal, but having an external logic sending ERC20 tokens to this
        // contract automatically is also Ok if managed correctly, and it worked because I have done it.

        // For this code I will not hardcode the .transferFrom so the input ERC20 token must be sent to this contract beforehand

        // Hint if a decode of the dataArray is needed to perform the .transferFrom... but it needs more investigation because It cannot be used this way because the dataArray[0] has both function identifier + data, and I only want the data so this means that I would need to split the data from the identifier and it is not that easy
        //(uint amountIn, uint amountOutMin, address[] memory path, address receiver, uint deadline) = abi.decode(dataArray[0], (uint,uint,address[],address,uint)); // 
        //IERC20(path[0]).transferFrom(msg.sender, address(this), amountIn);

        for(uint256 i; i < contractAddressArray.length; i++) {
            (bool success, bytes memory returndata) = contractAddressArray[i].call{gas: availableGas}(dataArray[i]);
            require(success, string(returndata));
        }
    }
