...
contract ChainlinkPriceFacet is IChainlinkPrice {
    ...
    function addChainlinkPriceFeed(address tokenAddress, address priceFeed) external override {
        LibAccessControlEnumerable.checkRole(PRICE_FEED_OPERATOR_ROLE);
        require(tokenAddress != address(0), "ChainlinkPriceFacet: Token address can't be 0 address");
        require(priceFeed != address(0), "ChainlinkPriceFacet: Price feed address can't be 0 address");
        LibChainlinkPrice.addChainlinkPriceFeed(tokenAddress, priceFeed);
    }

    function removeChainlinkPriceFeed(address tokenAddress) external override {
        LibAccessControlEnumerable.checkRole(PRICE_FEED_OPERATOR_ROLE);
        require(tokenAddress != address(0), "ChainlinkPriceFacet: Token address can't be 0 address");
        LibChainlinkPrice.removeChainlinkPriceFeed(tokenAddress);
    }

    function getPriceFromChainlink(address tokenAddress) external view override returns (PriceInfo memory priceInfo) {
        LibChainlinkPrice.ChainlinkPriceStorage storage cps = LibChainlinkPrice.chainlinkPriceStorage();
        address priceFeed = cps.priceFeeds[tokenAddress].feedAddress;
        ...
        priceInfo = PriceInfo(uint256(price), oracle.decimals());
    }
    ...
    function chainlinkPriceFeeds() external view override returns (PriceFeedInfo[] memory priceFeeds) {
        ...
        for (uint256 i; i < numFeeds; i++) {
            address tokenAddress = cps.tokenAddresses[i];
            LibChainlinkPrice.PriceFeed storage pf = cps.priceFeeds[tokenAddress];
            priceFeeds[i].tokenAddress = tokenAddress;
...
interface IChainlinkPrice {
    ...
    function addChainlinkPriceFeed(address tokenAddress, address priceFeed) external;

    function removeChainlinkPriceFeed(address tokenAddress) external;

    function getPriceFromChainlink(address tokenAddress) external view returns (PriceInfo memory priceInfo);
