...
contract NewCryptoSabong is Ownable {
   ...
   function() public payable {}
    ...
    // function bet(uint96 _gameId, uint _teamSelected, uint96 amount) public  {
    ...
    function setWinners(uint96 _gameId, uint _teamWinner, bool wSet)public onlyAgent{
        tracker = _gameId;
        result = _teamWinner;
        wSet = true;
...
//     function meronCalculate(uint96 _gameID, uint _result, address betor) internal {
//         require(_gameID == tracker && _result == result, "Winner not set");
//         uint96 totalBets = 0;
//         uint96 ads = 0;
//         uint96 rewards = 0;
...
//             bets = playerInfo[betor].amountBet;


//             rewards = (bets * ads) / 10000;
//             _pendingBalance[betor] += rewards;

//         playerInfo[betor]._state = PlayerStatus.Not_Joined;
//         gameInfo[_gameID] = Game(_gameID, State.Finished);
//         totalBetsOne = 0;
//         totalBetsTwo = 0; 
//         bettingActive = true;
//         delete playerInfo[betor].amountBet;
//         delete playerInfo[betor].teamSelected;
//         delete playerInfo[betor];
//     }

//     function walaCalculate(uint96 _gameID, uint _result, address betor) internal {
//         require(_gameID == tracker && _result == result, "Winner not set");
//         uint96 totalBets = 0;
//         uint96 ads = 0;
//         uint96 rewards = 0;
...
//     function drawRefund(uint96 _gameId, uint _result, address betor) internal {
//         require(_result == 3 && _gameId == tracker, "Refund not set");
...
//         drawRefund(tracker, result, msg.sender);
...
//         require(winnerSet == true, "Winner not set");
//         require(bettingActive == false, "Betting is sTill");
//         if(playerInfo[msg.sender].teamSelected == 1){
//             meronCalculate(tracker, result, msg.sender);
//         }
//         else if(playerInfo[msg.sender].teamSelected == 2){
//             walaCalculate(tracker, result, msg.sender);
//         }
