...
contract Treasury is UniswapCommon{
    ...
    function test2(uint256 payAmount) external view returns(uint256 dolAmount){
        require(payAmount>0);
        require(address(_pay)!=address(0));
        require(address(_dol)!=address(0));
        uint112 reserve0;
        ...
        dolAmount=uniswapV2Router.quote(payAmount,reserve0,reserve1);
    }
    function test3(uint256 payAmount) external view returns(uint256[] memory temp){
        require(payAmount>0);
        require(address(_pay)!=address(0));
        require(address(_dol)!=address(0));
        uint112 reserve0;
        ...
        uint256 dolAmount=uniswapV2Router.quote(payAmount,reserve0,reserve1);
        temp[0]=payAmount.mul(addLpWreck).div(100);
        temp[1]=dolAmount.mul(addLpWreck).div(100);
    }
}