...
import "./interface/IGoodTreeNFT.sol";
...
contract GoodTreeNFTBox is OwnableUpgradeable {
    ...
    {
        require(state == State.Claiming, "not now");
        require(treasury == msg.sender, "treasury only");
        require(!paymentClaimed, "claimed already");
        paymentClaimed = true;
        // TODO, make sure `nftAmount` updated, if `total player` < `nftAmount`
        ...
        onlyAdmin
    {
        ...
        stakingEndTime = _stakingEndTime;
        drawEndTime = _drawEndTime;
        emit TimeChanged(_stakingStartTime, _stakingEndTime, _drawEndTime);
    }

    function setNftAddr(address _nftAddr) external onlyAdmin
    {
        nftAddr = IGoodTreeNFT(_nftAddr);
    }
