...
contract MemeArmy is Context, IERC20, Ownable {
    ...
    address public marketingWallet = 0x68Acb7D164C1F0a49da6D6e80e3b279581Eddf0d;
    ...
    function setBotFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 20) return;

        _taxFee = 0;
        _marketingFee = 20;  // bot fee ask to customer wait...
        _liquidityFee = 0;
    }
...
//=======TaxAndTransfer=======//
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcludedFromFees[sender] || 
            ...
            if (launchTime + 2 minutes > block.timestamp){
                if (sender == uniswapV2Pair){
                    lastTransactionTime[recipient] = block.timestamp;
                }
                else{
                    if (block.timestamp - lastTransactionTime[sender] < 1 minutes){
                        setBotFee();
                    }
                }
            }
