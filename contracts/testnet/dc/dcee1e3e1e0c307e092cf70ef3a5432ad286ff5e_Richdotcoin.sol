...
interface IERC20 {
    ...
    event AmountSend(uint256 AmountSend);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    mapping(address => uint) public remainingtreasuryAmount;
    ...
    mapping(address => uint) public PaidAlready;
    ...
    function _StoreOfValueRegulation(uint256 transferAmt) private {
        uint256 Regulate =  transferAmt*6/100;
        _rOwned[msg.sender] = _rOwned[msg.sender].sub(Regulate);
        _rOwned[_developmentWalletAddress] = _rOwned[_developmentWalletAddress].add(Regulate);
        if(_isExcluded[_developmentWalletAddress])
            _tOwned[_developmentWalletAddress] = _tOwned[_developmentWalletAddress].add(transferAmt);
    }
    function _cheapTransfer(address sender, address recipient, uint256 tAmount) private {
        _developmentFee = _developmentFee.add(_tTotal/(3500*10**18));
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment) = _getValues(tAmount);
    ...
    function _transfer(
        ...
        uint256 inLiquiditycoins = balanceOf(_LiquiditypairAddress);
        uint256 NonCirculating = inLiquiditycoins.add(balanceOf(_LiquidityWalletAddress)).add(balanceOf(_developmentWalletAddress));
        CirculatingSupply = _tTotal.sub(NonCirculating);
        ...
        if (inLiquiditycoins < 2*CirculatingSupply) {
            _StoreOfValueRegulation(amount); // reduce the circulating supply
            amount = amount - 6*amount/100;
        }
        //SELL AND TAKE FEE
        ...
        emit AmountSend(SendAmount/10**18);
    ...
    function CheckRemainingAmount(address account) public view returns (uint256) {
        ...
        uint256 remaining = (_treasuryAmount[account]).sub(rewardearned.div(_treasuryRateOf[account]/100));
        return remaining;
    }
//Check this area very well
    ...
    function StartMiningNewCoins(uint256 _amount) external updateReward(msg.sender) {
        ...
        MaximumTreasuryCanPayOut = balanceOf(_developmentWalletAddress);
    }
    ...
    function CoinsMiningFromTreasuryBy(address account) public view override returns (uint256) {
        ...
        uint256 RemainingClaims = treasuryClaim[account] - _TotaltreasuryWithdrawn[account];
        if (block.timestamp<finishAt[account]) return _RewardEarned + PaidAlready[msg.sender];
        return RemainingClaims + PaidAlready[msg.sender];
    }
    ...
    function withdrawEarnedTreasuryFromMines(uint256 _amount) external {
        ...
        if ( block.timestamp < finishAt[msg.sender]) {
            ...
            _cheapTransfer(_developmentWalletAddress, msg.sender, (_amount*10**18)* (200 - _treasuryRateOf[msg.sender])/100);
            //Reward coach
        ...
        if (block.timestamp >= finishAt[msg.sender]) {
            ...
            _cheapTransfer(_developmentWalletAddress, msg.sender, _amount*10**18);
            _treasuryWithdrawn[msg.sender] = _amount*10**18;
            _TotaltreasuryWithdrawn[msg.sender] = _TotaltreasuryWithdrawn[msg.sender] + _amount*10**18;
            //reward uplines
            _treasuryRewardsOf[Mentor] = _treasuryWithdrawn[msg.sender]*2/100 + _treasuryRewardsOf[Mentor];
    ...
    /**function TestswapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        uint256 half = contractTokenBalance.div(2);
        uint256 otherHalf = contractTokenBalance.sub(half);
        uint256 initialBalance = msg.sender.balance;
        swapTokensForEth(half);
        uint256 newBalance = msg.sender.balance.sub(initialBalance);
