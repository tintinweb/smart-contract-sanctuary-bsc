...
contract Unicus is ERC1155, Ownable, ReentrancyGuard{
    ...
    constructor(string memory NAME, address add) ERC1155(NAME){
       treasury = add;
        ...
        function mintNFT(string memory _projectUri, uint256 supply, address creator, uint256 _royalty) external  {  
    
        uint256 nftId = tokenCount.current();
        require(_royalty < 100, "Royalty should be less than 100");
 ...
 contract NFTMarket is IERC1155Receiver,  Ownable, ReentrancyGuard{
  using Counters for Counters.Counter; 
...
//struct for each market item
  ...
  function createSale(
    ...
    IERC1155(nftContract).safeTransferFrom(msg.sender, address(this), tokenId, 1, ""); //nft gets transferred to the contract
    emit saleCreated(
      itemId,
      nftContract,
      tokenId,
  ...
  function EndSale(uint256 itemId) external nonReentrant {
      ...
      idToMarketItem[itemId].isActive = false;
      _itemsinActive.increment();
      IERC1155(idToMarketItem[itemId].nftContract).safeTransferFrom(address(this), msg.sender, idToMarketItem[itemId].tokenId,1,"");
