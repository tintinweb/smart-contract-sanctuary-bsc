 ...
 * 
 * Developed by Fab (copied and redeployed by retard Retro Boy)
 */
contract BNBRiotSplitter is Recoverable {
    using SafeMath for uint256;

    IERC721Enumerable public immutable token;
    address public protestorsContract;
    ...
    function walletHoldings(address _wallet) external view returns (uint256) {
    return IERC721(protestorsContract).balanceOf(_wallet);
    }

    function setProtestorsContract(address _protestorsContract) external onlyOwner {
    _protestorsContract = protestorsContract;
    }
}
...
/**
 * @title Contract to allow recovering ERC20 and ERC721 tokens send to this contract by accident.
 * @dev Do not use this contract if it is meant to hold ERC20 or ERC721 tokens.
 * Developed by Fab
 */
