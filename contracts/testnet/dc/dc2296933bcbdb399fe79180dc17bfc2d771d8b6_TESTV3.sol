...
// File: @openzeppelin/contracts-upgradeable/token/BEP20/IBEP20Upgradeable.sol
...
// File: @openzeppelin/contracts-upgradeable/token/BEP20/BEP20Upgradeable.sol
...
contract TESTV3 is  Initializable ,ContextUpgradeable, BEP20Upgradeable, OwnableUpgradeable {
    using AddressUpgradeable for address;
    ...
    function TKN() internal initializer {
        __BEP20_init("Test", "TTT");
        ...
        _BuytaxFee = 0; // % of transactions will be redistributed among holders
        _BuyliquidityFee = 300; // % of transactions will be kept for liquidity
        _BuymarketingFee = 500; // % of transactions will be kept for marketing wallet
   
        _SelltaxFee = 0; // % of transactions will be redistributed among holders
        _SellliquidityFee = 500; // % of transactions will be kept for liquidity
        _SellmarketingFee = 500; // % of transactions will be kept for marketing wallet
   
        _transferTaxFee = 0;
        _transferLiquidityFee = 0;
        _transfermarketingFee = 0;
        _MarketingWalletAddress = 0x7ed5A5a6068c703aFd86e71E7acda14A1BED4607;
        _shouldSwapToBNB = false;
        swapAndLiquifyEnabled = true;

        _maxTxAmount = 1000000000 * 10**18;
        numTokensSellToAddToLiquidity = 500000000 * 10**18;
    ...
    function totalFees() public view returns (uint256) {
        return _taxFee.add(_liquidityFee).add(_marketingFee);
    }
    ...
    function changeMarketingWallet(address add) external onlyOwner()
    {
        _MarketingWalletAddress = add;
    }

    function changeMaxTxAmount(uint256 txAmount) external onlyOwner()
    {
    ...
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
     ...
     function changeBuyFee(uint256 buyTax, uint256 buyLiquidity, uint256 buyMarketing) public onlyOwner()
    {
    ...
    function changeSellFee(uint256 sellTax, uint256 sellLiquidity, uint256 sellMarketing) public onlyOwner()
    {
    ...
    function changeTransferFee(uint256 transferTax, uint256 transferLiquidity, uint256 transferMarketing) public onlyOwner()
    {
    ...
    function swapAndLiquify(uint256 tokensToLiquify) public onlyOwner lockTheSwap {
    ...
    function collectBNB(uint256 contractToken) public lockTheSwap {
        swapTokensForBNB(contractToken);
    }
    ...
    function withdrawBnb(uint256 _amount) public onlyOwner {
        ...
        IBEP20Upgradeable(_token).transfer(msg.sender,_amount);
    }
