...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing {
    ...
    mapping(address => bool) public listItemSupport;

    modifier onlyValidNFT(address nftToken) {
        require(nftToken != address(0) && nftToken == geniPass || listItemSupport[nftToken] || listNFTSupport[nftToken], "Leasing: invalid token");
        _;
    }
    ...
    function addListTokens(TokenSupport[] memory listNFT) external onlyOperator {
        require(listNFT.length > 0, "Leasing: empty");
        for(uint i = 0; i < listNFT.length; i++) {
            address nftToken = listNFT[i].nft;
            NFTType nftType = listNFT[i].nftType;
            require(nftToken != address(0), "Leasing: invalid address");
            if (nftType == NFTType.NFTSupport){
                listItemSupport[nftToken] = true;
            } else {
                listNFTSupport[nftToken] = true;
            }
            emit AddListTokens(nftToken, nftType);
        }
    }

    function removeListTokens(TokenSupport[] memory listNFT) external onlyOperator {
        require(listNFT.length > 0, "Leasing: empty");
        for(uint i = 0; i < listNFT.length; i++) {
            address nftToken = listNFT[i].nft;
            NFTType nftType = listNFT[i].nftType;
            require(nftToken != address(0), "Leasing: invalid address");
            if (nftType == NFTType.NFTSupport){
                listItemSupport[nftToken] = false;
            } else {
                listNFTSupport[nftToken] = false;
            }
            emit RemoveListTokens(nftToken, nftType);
        }
    }
    ...
    function putOnLeasing(LeasingCreate[] memory inputs) public override {
        for (uint i = 0; i < inputs.length; i++) {
            ...
            require(minDuration > 0 && maxDuration >= minDuration && pricePerWeek > 0, "Leasing: invalid duration or price");
            if (listNFTSupport[nftToken] && leasingGuild.gameGuild == address(0)) {
            ...
            if (!gameGuild[msg.sender] || gameGuild[msg.sender] && leasing.lender == address(0) && renter.renter == address(0)) {
                if (listNFTSupport[nftToken]) {
                    address guildAddress = leasingGuild.gameGuild;
                    require(guildAddress == address(0),"Leasing: Token already rented");
                }
            ...
            } else if (msg.sender == leasingGuild.gameGuild) {
                require(leasingGuild.price == 0, "Leasing: already registered");
                require(block.timestamp <= leasingGuild.startLending + leasingGuild.duration * weekPerSecond, "Leasing: overtime");
                setLockNFT(nftToken, tokenId, false, leasing.mortgageAmount);
    ...
    function setLockNFT(address nftToken, uint tokenId, bool active, uint mortgageAmount) internal {
        if (nftToken == geniPass) {
            if(IGENIPass(nftToken).isLocked(tokenId)) {
                IGENIPass(geniPass).unLockPass(tokenId);
                IGENIPass(geniPass).setActive(tokenId, active);
            }
        } else if (listItemSupport[nftToken]) {
    ...
    function register(address nftToken, uint tokenId, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek) internal onlyValidNFT(nftToken) returns (uint rentalAmount) {
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo storage leasingGuild = leasingGuildInfo[nftToken][tokenId];
        RenterInfo memory renter = renterInfo[nftToken][tokenId];
    ...
    function updateLeasing(address nftToken, uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerWeek) external override {
        ...
        if (msg.sender == guild.gameGuild) {
            require(guild.price > 0, "Leasing: not lending");
            ...
            require(leasing.price > 0, "Leasing: not lending");
    ...
    function cancelLeasing(address nftToken, uint tokenId, bytes memory sig) external override {
        ...
        if (msg.sender == guild.gameGuild) {
            ...
            if (guild.duration != 0 && block.timestamp > guild.startLending + guild.duration * weekPerSecond) {
                ...
                lockTransferNFT(nftToken, tokenId, true, sig, leasing.mortgageAmount);
                guild.minDuration = 0;
                guild.maxDuration = 0;
    ...
    function rent(address nftToken, uint tokenId, uint duration, uint price, bytes memory sig) external override {
        ...
        require(renter.renter == address(0) && leasing.lender != address(0) && msg.sender != leasing.lender, "Leasing: not leasing or already rented or invalid sender");
    ...
    function lockTransferNFT(address nftToken, uint tokenId, bool active, bytes memory sig, uint mortgageAmount) internal {
        if (nftToken == geniPass) {
    ...
    function rentMore(address nftToken, uint tokenId, uint addedDuration, uint addedRentalPrice) external override {
        ...
        if (msg.sender == renter.renter && guild.gameGuild != address(0)) {
            ...
            require(
                ...
                "Leasing: invalid or over max duration "
            );
    ...
    function finishRenting(address nftToken, uint tokenId) external override {
        ...
        LeasingInfo storage leasing = leasingInfo[nftToken][tokenId];
        LeasingGuildInfo storage guild = leasingGuildInfo[nftToken][tokenId];
        require(leasing.lender != address(0), "Leasing: not leasing");
        ...
        if (msg.sender == guild.gameGuild) {
            ...
            //guild rent -> lender
                ...
                transferNFT(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, guild.gameGuild);
                delete leasingGuildInfo[nftToken][tokenId];
            }
        //user return pass to lender/gameguild
        } else {
            transferNFT(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, renter.renter);
    ...
    function transferNFT(address nftToken, uint tokenId, uint mortgageAmount, address lender, address renter) internal {
        if (listNFTSupport[nftToken]) {
    ...
    function withdrawNFT(address nftToken, uint tokenId, address to) external onlyOperator {
        ...
        } else if (listNFTSupport[nftToken]) {
            if(IGENIPass(geniPass).isLocked(tokenId)) {
                IGENIPass(geniPass).unLockPass(tokenId);
            }
        }
...
interface IGEMUNIItem is IERC721 {
    ...
    event ItemCreated(uint indexed tokenId, address to);
