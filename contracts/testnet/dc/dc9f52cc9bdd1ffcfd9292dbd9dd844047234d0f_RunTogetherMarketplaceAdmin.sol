...
// File: RunTogetherMarketplaceAdmin.sol
...
contract RunTogetherMarketplaceAdmin is Ownable, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => address) public tokenIdToAddressMap; // check for what nft is sold in marketplace admin
    ...
    // max nft in transfer
    mapping(uint256 => uint256) public maxNftInTransferMap;
    uint256 public maxNftInTransfer;
    ...
    function updateBoxPrice(uint256[] calldata _boxTypes, uint256[] calldata _price) external nonReentrant onlyOwner {
        for (uint256 index = 0; index < _boxTypes.length; index++) {
            runTogetherBoxNftTypePriceMap[_boxTypes[index]] = _price[index];
        }
    }

    function initNftsInMarket() external nonReentrant onlyOwner {
        uint256 from = 0;
        for (uint256 index = 1; index <= totalBoxType; index++) {
            from += runTogetherBoxNftMap[index].length;
        }

        uint256 to = ERC721(runTogetherBoxNft).balanceOf(address(this));
        if (ERC721(runTogetherBoxNft).balanceOf(address(this)) > from + maxNftInTransfer) {
            to = from + maxNftInTransfer;
        }
        require(from < to, "No nft to init");
    ...
    function buyItem(uint256[] calldata _numOfboxTypes) external validOpenMarket nonReentrant {
        ...
        uint256 totalBox = 0;
        for (uint256 index = 1; index <= totalBoxType; index++) {
            uint256 numOfBox = _numOfboxTypes[index-1];
            require(numOfBox <= runTogetherBoxNftTypeCountMap[index], "Not enough box");
            totalPrice += numOfBox * runTogetherBoxNftTypePriceMap[index];

            // setup for claim
            runTogetherBoxNftTypeCountMap[index] -= numOfBox;
            totalNfts -= numOfBox;
            userBoxTypeMap[msg.sender][index] += numOfBox;
        ...
        // Cal trading fee
        uint256 feeTrading = calTradingFee(newPrice);
        uint256 sellerValueReceive = newPrice - feeTrading;

        // Trading action
        ERC20(paymentToken).safeTransferFrom(msg.sender, feeWallet, feeTrading);
        ERC20(paymentToken).safeTransferFrom(msg.sender, paymentWallet, sellerValueReceive);

        emit EventBuyCompleted(msg.sender, _numOfboxTypes, newPrice);
    }

    function claimItem() external nonReentrant {
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
        require(totalBoxOfUser[msg.sender] > 0, "You have no item to claim");

        for (uint256 index = 1; index <= totalBoxType; index++) {    
            if (userBoxTypeMap[msg.sender][index] > 0) {
                uint256 totalNft = 0;

                if (userBoxTypeMap[msg.sender][index] <= maxNftInTransferMap[index]) {
                    totalNft = userBoxTypeMap[msg.sender][index];
                } else {
                    totalNft = maxNftInTransferMap[index];
                }
    ...
    function setMaxNftInTransfer(uint256[] calldata _boxTypes, uint256[] calldata _maxNftInTransfer) external onlyOwner {
        ...
        for (uint256 index = 0; index < _maxNftInTransfer.length; index++) {
            maxNftInTransferMap[_boxTypes[index]] = _maxNftInTransfer[index];
            maxNftInTransfer += _maxNftInTransfer[index];
        }
    }
    ...
    // Set paymentWallet and feeWallet
    function setWallet(address _paymentWallet, address _feeWallet) external onlyOwner {
        paymentWallet = _paymentWallet;
        feeWallet = _feeWallet;
    }
    ...
    function cleanMarket() external nonReentrant onlyOwner {
        require(totalNfts > 0, "No NFT to clean");
        for (uint256 index = 1; index <= totalBoxType; index++) {
            if (runTogetherBoxNftTypeCountMap[index] > 0) {
                uint256 totalNft = maxNftInTransferMap[index];

                if (runTogetherBoxNftTypeCountMap[index] <= maxNftInTransferMap[index]) {
                    totalNft = runTogetherBoxNftTypeCountMap[index];
                }

                runTogetherBoxNftTypeCountMap[index] -= totalNft;
                totalNfts -= totalNft;
                for (uint256 j = 0; j < totalNft; j++) {
                    uint256 tokenId = runTogetherBoxNftMap[index][runTogetherBoxNftMap[index].length - 1];
