...
import './Helpers/ChainlinkClient.sol';
...
contract GTokenOpenPnlFeed is ChainlinkClient, IOpenTradesPnlFeed{
    ...
    constructor(
        ...
        && _oracles.length > 0
        && _job != bytes32(0), "WRONG_PARAMS_CONSTRUCTOR");
    ...
    function setgTokenOnce(IGToken _gToken) external{
        require(address(_gToken) != address(0), "WRONG_PARAMS");
        require(msg.sender == _gToken.admin(), "ONLY_ADMIN");
        if (!gTokenOnce) {
            gToken = _gToken;
            gTokenOnce = true;
        }
