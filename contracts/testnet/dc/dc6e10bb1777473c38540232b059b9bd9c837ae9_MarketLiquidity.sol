...
contract MarketLiquidity is Ownable, ReentrancyGuard {
    ...
    constructor(address _rewardsToken, address _WPACKStakingManager, uint256 _qualifier, uint256 _period) {
    ...
    function modifyWPACKStakingManagerAddr(address _WPACKStakingManager) external onlyOwner {
        WPACKStakingManager = _WPACKStakingManager;
    }
    ...
    function requestMarketPayout(uint256 payout) public returns (uint256) {
        require(checkNotificationSource(msg.sender), "unauthorized caller");
        uint256 ret;
        if (payout < rewards) {
            ret = payout;
    ...
    function distributeWithdrawalFee(uint256 fee) private {
        ...
        rewardsToken.transfer(WPACKStakingManager, dividend);
    }
