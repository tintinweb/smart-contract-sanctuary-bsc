        ...
        ERC20("LFW Claim IDO Pool", "LFW-IDO-Claim") 
{
    ...
    // address of LFW ADMIN
    address public LFW_ADMIN;
    ...
    // claim start at which timestamp
    uint256[] public claimStartAt ;

    // number of participant has been joined
    mapping(uint256 => uint256) public joinedCount;
    ...
    mapping(address => mapping(uint256 => uint256)) public whitelistAddress;
    ...
    constructor() {
        LFW_ADMIN = msg.sender;
    }

    /**
     * @notice set new admin for the SC
     * @dev only call by owner
     * @param _newAdmin: list whitelist address
     */
    function setAdm( address _newAdmin) external onlyOwner {
     ...
     * @param _claimTime: time to start claiming
     * @param _amount: allocation for each round
     */
    function setConfig(
        address _token,
    ...
    function addWhitelistAddress(
        ...
        for (uint256 index = 0; index < _addresses.length; index++) {
            for (uint256 round = 0; round < _roundNumber; round++) {
                whitelistAddress[_addresses[index]][round] = whitelistAddress[_addresses[index]][round] + 1;
            }
        }
    }
    ...
    function removeWhitelistAddress(
        ...
        for (uint256 index = 0; index < _addresses.length; index++) {
            for (uint256 round = 0; round < _roundNumber; round++) {
                whitelistAddress[_addresses[index]][round] = 0;
            }
        }      
    }
    ...
    function claim(uint256 _round) external {
        ...
        require(
            whitelistAddress[_msgSender()][index] > 0, 
        ...
        whitelistAddress[_msgSender()][index]  = whitelistAddress[_msgSender()][index] - 1;
