...
contract EatherBonus is Auth, EIP712Upgradeable {
  ...
  event BonusResult(address indexed user, bool success, uint quantity);
  ...
  function getBonus(bytes calldata _signature, uint _timestamp, uint _quantity, bytes32[] calldata _path, uint _index) external {
    ...
    if (success) {
      _giveReward(_quantity > maxEatherPerTurn ? maxEatherPerTurn : _quantity);
    }
    emit BonusResult(msg.sender, success, _quantity);
  }
