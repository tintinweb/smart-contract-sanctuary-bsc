        ...
        contract BnbYielding is ReentrancyGuard{
            ...
            function withdraw() external NonBlackListed {
                ...
                if(vipinvestor.vip = true){
                    require(vipinvestor.TimeForWithdraw == block.timestamp , "You are a vip investor and you cannot withdraw if the contract period has not ended");
                }
                ...
                if(vipinvestor.vip=true && amount > MAX_WITHDRAW){
                    ...
                    vipinvestor.statusFeeWithdraw = false;
                }

                if(vipinvestor.statusFeeWithdraw = false && amount > MAX_WITHDRAW && !iswhitelist(msg.sender)) {
                    investor.dividends = amount - MAX_WITHDRAW;
                    amount = MAX_WITHDRAW;
                }

                if(vipinvestor.statusFeeWithdraw = false && amount == MAX_WITHDRAW && investor.whithdrawCount >= MAX_TaxWITHDRAW_COUNT){
                ...
                if(vipinvestor.statusFeeWithdraw = true && vipinvestor.FeeWithdraw == 0) {
