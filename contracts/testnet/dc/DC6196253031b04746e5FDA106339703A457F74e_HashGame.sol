...
contract HashGame is ReentrancyGuard {
    ...
    function processDefaultGame(address _userAddress, bool _result) external onlyOperator nonReentrant {
    ...
    function processMvccGame(uint256 _gid, address _userAddress, bool _result) external onlyOperator nonReentrant {
    ...
    function joinMvcrGame(uint256 _gid) external nonReentrant {
        GameInfo storage _gameInfo = gameInfo[_gid];
        UserInfo storage _userMvcrInfo = userMvcrInfo[_gid][msg.sender];

        require(!_gameInfo.paused, "Contract paused, please try again later");
        require(_gid != 0, "Invalid game ID");
        require(_userMvcrInfo.currentGameCheckpoint == _userMvcrInfo.currentGameRound, "Processing previous game, please try again later");
        require(_userMvcrInfo.currentGameRound < maxRoundPerGame, "Exceeded max round per game");
        ...
        // Update user info
        _userMvcrInfo.gameActive = true;
        _userMvcrInfo.currentGameRound ++;
        _userMvcrInfo.currentGameRoundAmount = _amount;
        _userMvcrInfo.joinGameTimestamp = block.timestamp;
        _userMvcrInfo.endGameTimestamp = 0;

        emit JoinMvcrGame(_gid);
    }

    function processMvcrGame(uint256 _gid, address _userAddress, bool _result) external onlyOperator nonReentrant {
        UserInfo storage _userMvcrInfo = userMvcrInfo[_gid][_userAddress];
        require(_gid != 0, "Invalid game ID");
        require(_userMvcrInfo.gameActive, "No active game found");
        require(_userMvcrInfo.currentGameRound > _userMvcrInfo.currentGameCheckpoint, "No new game round to process");

        // Increment the checkpoint
        _userMvcrInfo.currentGameCheckpoint ++;

        if(_result) {    
            // Distribute rewards and reset user's game info if user won    
            require(BUSD.balanceOf(address(this)) >= _userMvcrInfo.currentGameRoundAmount, "Insufficient BUSD in contract");
            uint256 _currentGameRoundAmount = _userMvcrInfo.currentGameRoundAmount;

            resetMvcrGame(_gid, _userAddress);
            BUSD.transfer(_userAddress, _currentGameRoundAmount);
        } else if(_userMvcrInfo.currentGameRound == maxRoundPerGame && _userMvcrInfo.currentGameCheckpoint == maxRoundPerGame) {
            // Reset user's game info if user lost the last round
            resetMvcrGame(_gid, _userAddress);
        } 
    ...
    function distributePremiumRewards(uint256 _gid, address _userAddress, uint256 _amount) external onlyOperator nonReentrant {
        UserInfo storage _userInfo = userInfo[_gid][msg.sender];
        require(_gid != 0, "Invalid GID, no premium for this game");
    ...
    function resetMvcrGame(uint256 _gid, address _userAddress) internal {
        ...
        _userMvcrInfo.gameActive = false;
        _userMvcrInfo.currentGameRound = 0;
        _userMvcrInfo.currentGameRoundAmount = 0;
        _userMvcrInfo.currentGameCheckpoint = 0;
        _userMvcrInfo.joinGameTimestamp = 0;
        _userMvcrInfo.endGameTimestamp = block.timestamp;
    }
    ...
    function getMvcrNextRoundFee(uint256 _gid, address _userAddress) public view returns(uint256) {
        ...
        uint256 _amount = _gameInfo.initialGameFee;
        for(uint i=0; i<_userMvcrInfo.currentGameRound; i++) {
            _amount = _amount * 2;
        }
