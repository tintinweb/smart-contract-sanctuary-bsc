...
contract Lycan is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 LYCAN-WFTM LP -> LYCAN pool
    uint256 public constant INITIAL_LYCAN_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the LYCAN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("LYCAN", "LYCAN") {
        // Mints 1 LYCAN to contract creator for initial pool setup
    ...
    function _getLycanPrice() internal view returns (uint256 _lycanPrice) {
        try IOracle(lycanOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Lycan: failed to fetch LYCAN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _lycanPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_lycanPrice >= taxTiersTwaps[tierId]) {
    ...
    function setLycanOracle(address _lycanOracle) public onlyOperatorOrTaxOffice {
        require(_lycanOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints LYCAN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of LYCAN to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentLycanPrice = _getLycanPrice();
            currentTaxRate = _updateTaxRate(currentLycanPrice);
            if (currentLycanPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_lycanPool != address(0), "!_lycanPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_lycanPool, INITIAL_LYCAN_POOL_DISTRIBUTION);
