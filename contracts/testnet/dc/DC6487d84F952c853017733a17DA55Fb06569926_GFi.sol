...
contract GFi is Context, IBEP20, Ownable, ReentrancyGuard{ 
    ...
    uint256 totalFeeIfBuying 				        = buyLiquidityFee + buyRewardFee + buyEcosystemFee + buyDevelopmentFee + buyReserveFee;
    ...
    uint256 totalFeeIfSelling 				        = sellLiquidityFee + sellRewardFee + sellEcosystemFee + sellDevelopmentFee + sellReserveFee;

    uint256 liquidityFee                            = sellLiquidityFee;
    uint256 rewardFee                               = sellRewardFee;
    uint256 ecosystemFee                            = sellEcosystemFee;
    uint256 developmentFee                          = sellDevelopmentFee;
    uint256 reserveFee                              = sellReserveFee;
    uint256 totalFee                                = totalFeeIfSelling;
    ...
    constructor () {
        ...
        totalFeeIfBuying 				            = buyLiquidityFee + buyRewardFee + buyEcosystemFee + buyDevelopmentFee + buyReserveFee;
        totalFeeIfSelling 				            = sellLiquidityFee + sellRewardFee + sellEcosystemFee + sellDevelopmentFee + buyEcosystemFee + sellReserveFee;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 feeAmount = recipient == pair ? amount.mul(totalFeeIfSelling).div(100) 
                                : amount.mul(totalFeeIfBuying).div(100);
    ...
    function setBuyFees(uint256 _buyLiquidityFee, uint256 _buyRewardFee, uint256 _buyEcosystemFee, uint256 _buyDevelopmentFee, uint256 _buyReserveFee, uint256 _feeDenominator) external onlyOwner {
        ...
        require(totalFeeIfBuying < _feeDenominator, "Fees cannot be less than 10%");
    }
    ...
    function setSellFees(uint256 _sellLiquidityFee, uint256 _sellRewardFee, uint256 _sellEcosystemFee, uint256 _sellDevelopmentFee, uint256 _sellReserveFee, uint256 _feeDenominator) external onlyOwner {
 	    ...
 	    totalFeeIfSelling 		= sellLiquidityFee + sellRewardFee + sellEcosystemFee + sellDevelopmentFee + sellReserveFee;
        feeDenominator          = _feeDenominator;
        require(totalFeeIfSelling < _feeDenominator, "Fees cannot be more than 20%");
    }
