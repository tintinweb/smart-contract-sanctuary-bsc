...
contract VestingContract is Ownable, ReentrancyGuard {
    ...
    */
	...
	/**
		...
		@param _firstRelease		First month to start vesting
    ...
    */
	...
	/**
       	...
       	@dev Caller can be ANY
			...
			- Beneficiaries could be able to claim after `startAt + n * DURATION`
		@param _month		Month of vesting request
    */
	function claim(uint256 _month) external nonReentrant {
		//	If msg.sender is not beneficiary, `amounts` list should be empty
		...
		//	`_beneficiary` can call to claim token after due date
		if (_month != 0) {
			require(
				_month * DURATION + startAt <= block.timestamp,
				"Too early"
			);
		}
	
		uint256 _index = _month - policies[_beneficiary].first;
		uint256 _amount = policies[_beneficiary].amounts[_index];
		require(_amount != 0, "Zero amount");

		policies[_beneficiary].amounts[_index] = 0;
		releaseTokenTo(_beneficiary, _month, _amount);
	}
