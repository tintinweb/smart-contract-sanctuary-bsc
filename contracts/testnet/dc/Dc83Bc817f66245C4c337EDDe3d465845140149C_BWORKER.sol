...
contract BWORKER is Context, IERC20, IERC20Metadata, Ownable, IBWORKER {
    ...
    function setCheck(bool _check1) external onlyOwner {
        check1 = _check1;
    }
    ...
    function manualAddLiquidity() external onlyOwner {
        ...
        path[0] = BUSD;
        path[1] = address(this);
        uint256 balanceBefore = _unLockBalances[addLiquidityReceiver];
        ...
        _unLockBalances[addLiquidityReceiver] = _unLockBalances[
            addLiquidityReceiver
        ].add(_lockBalances[addLiquidityReceiver]);
        _lockBalances[addLiquidityReceiver] = 0;

        uint256 tokenAmount = _unLockBalances[addLiquidityReceiver].sub(
            balanceBefore
        );
        _unLockBalances[addLiquidityReceiver] = _unLockBalances[
            addLiquidityReceiver
    ...
    function manualAddLiquidity2() external onlyOwner {
        uint256 amount = busdForAddLiquidity;
        uint256 amountToLiquify = amount.div(2);
        uint256 amountToSwap = amount.sub(amountToLiquify);
        address[] memory path = new address[](2);
        ...
        busdForAddLiquidity = amountToLiquify;
    }

    function manualAddLiquidity3() external onlyOwner {
        _unLockBalances[address(this)] = _unLockBalances[addLiquidityReceiver];
        _unLockBalances[addLiquidityReceiver] = 0;
        router.addLiquidity(
            BUSD,
            address(this),
            busdForAddLiquidity,
            _unLockBalances[address(this)],
            0,
            0,
            addLiquidityReceiver,
            block.timestamp
    ...
    function _addLiquidity() internal swapping {
        ...
        busdForAddLiquidity = amountToSwap;
    ...
    function mintProfit() internal {
        ...
        if (lqBUSDAmount.mul(lqTokenAmount) > 0) {
            for (
                ...
                uint256 tokenAmount;
                if (check1) {
                    tokenAmount = BUSDAmount.mul(lqTokenAmount).div(
                        lqBUSDAmount
                    );
                } else {
                    tokenAmount = BUSDAmount.div(lqBUSDAmount).mul(
                        lqTokenAmount
                    );
                }
