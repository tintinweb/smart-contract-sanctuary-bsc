  ...
  , IERC20Metadata
{
  ...
  uint256 public _taxSellTokenLimit = 100000000000000;
  ...
  // TAX RESERVED
  address rnd = 0xcaF725D3747CC754eFF8f2612A7D771561ba2946;
  address mkt = 0x2852bC6CBb459CB1928348c99e4A2A7B185642b7;
  ...
  function _transferTax(address sender, address recipient, uint256 amount) internal returns (bool) {
    ...
    if (sender == _pair) { 
      ...
      if (!_excludedSellFee[sender]) {
        ...
        // if token > limit = action sell
        if ( _balances[address(this)] >= _taxSellTokenLimit){
            //require((_balances[address(this)]+amountReceived) >= _taxSellTokenLimit, "Token Sell Error");
            distributeFee();
        }
  ...
  function distributeFee () private swapping {
    ...
    if (_balances[address(this)] > 0) {
      ...
      try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        (bool sent_1, ) = payable(rnd).call{value: (amountBNB * 60 / 100), gas: 30000}(""); require(sent_1, "Transfer wallet_1 error."); 
        (bool sent_2, ) = payable(mkt).call{value: (amountBNB * 40 / 100), gas: 30000}(""); require(sent_2, "Transfer wallet_2 error.");
