...
contract fBurnStake is Ownable, ReentrancyGuard {
    ...
    address nftTokenAddress = 0x328fA466A35635bC428D184258b995d808dE7187; // Testnet NFT address
    //address nftTokenAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138; // Local NFT address
    ...
    uint256[] _durationArray = [0,14,30,60,90];
    uint256[] _withdrawalFeeArray = [0,0,0,0,0];
    uint256[] _unstakePenaltyArray = [0,80,80,80,80];
    
    
    constructor() {
        address _baseTokenAddress = 0x11CAb807C487762eEcbbCf04c50bdA53aC9743aC; 
        address _rewardTokensAddress = 0x11CAb807C487762eEcbbCf04c50bdA53aC9743aC;
    ...
    //Testing function
    function setLockTime(uint256 _stakeId, uint256 _days) nonReentrant unpaused external returns (bool){
        userStake memory userStakeDetails = userStakesById[_stakeId];
       
        require(userStakeDetails.owner == msg.sender,"You don't own this stake");
        require(userStakeDetails.status == 1,"You have already unstaked");

        userStakeDetails.lockedTime = block.timestamp - (_days * 1 days) ;
        userStakeDetails.unlockTime = userStakeDetails.lockedTime + (userStakeDetails.lockDuration * 1 days);
    ...
    function unstake(uint256 _stakeId) nonReentrant external returns (bool){
        ...
        if(isStakeLocked(_stakeId) && isPaused == false){
            ...
            //totalPenalty = claimableRewards + earlyUnstakePenaltyAmount;
    ...
    function testGetUnclaimedRewards(uint256 _stakeId, uint256 _stakeBalance, uint256 _rarityApplied, uint256 _lockDuration, uint256 _lockedDays) public view returns(uint256){
        ...
        uint unclaimedRewards = applicableRewards * testGetElapsedTime(_stakeId, _lockedDays);

        return unclaimedRewards; 
    }

    function testGetElapsedTime(uint256 _stakeId, uint256 _lockedDays) public view returns(uint256){
        //uint256 lapsedDays = ((block.timestamp - (_lockedDays * 1 days))/3600)/24; //3600 seconds per hour so: lapsed days = lapsed time * (3600seconds /24hrs)
        //return lapsedDays;
        ...
        if(block.timestamp > userStakeDetails.unlockTime){
            ...
            lapsedDays = ((block.timestamp - (_lockedDays * 1 days))/3600)/24; //3600 seconds per hour so: lapsed days = lapsed time * (3600seconds /24hrs)
        }
        return lapsedDays;  
    ...
    function withdrawRewardSupply(uint256 _amount) public onlyOwner paused returns(bool){
        bool success;
