...
contract invite  is Ownable {
  ...
  //查询指定用户是否有效的被邀请人以及被邀请人的对应的邀请人
    ...
    function getRankByAddress(uint _batchid,address _address) public view returns(uint){
        return inviteCenter[_batchid].school.getRankByAddress(_address);
    }
    function getRank(uint _batchid,uint top) public view  returns(school.rankDetail[] memory){
       return inviteCenter[_batchid].school.getTopDetail(top);
    }
    
    function getRewardData(address _address) public view returns(RewardData memory){
    ...
    */
    ...
    function _settlementUsdtHistory(address _user) internal {
        ...
        if (isRankFull(batchidH)){
           ...
           // r.RewardUsdt =  r.RewardUsdt.add(per.mul(r.curWeight));
            ...
            reward = per;
            //r.RewardUsdt =  r.RewardUsdt.add(per);
        }
    ...
    function _settlementUsdt(address _user) internal {
        if (isRankFull()){
            ...
            emit Reward(batchid,_user,1,1,per.mul(r.curWeight));
        }
    }
    ...
    function restart(uint256 _batchid) public onlyPledged  {
        ...
        inviteCenter[batchid].runkNumber = inviteCenter[batchid].school.listSize;
         ...
         _restart(_batchid,
         inviteCenter[batchid].lastToalBalanceUsdt,
        inviteCenter[batchid].lastToalBalanceErc20);
    }
}