...
contract ERC721RExample is ERC721A, Ownable {
    ...
    uint256 private constant salePrice = 0.15 ether;
    uint256 private constant discountPrice = 0.15*0.8 ether;

    uint256 private constant saleSupply = 269;
    uint256 private constant discountSupply = 10;
    uint256 private constant freeSupply = 20;

    uint256 private maxSaleMintAmount = 3;
    uint256 private maxDiscountMintAmount = 1;
    uint256 private maxFreeMintAmount = 1;
    ...
    function mint(uint256 mintType, uint256 quantity, bytes32[] calldata proof)
        ...
        payable
    {
        require(mintType == MINT_TYPE_SALE || mintType == MINT_TYPE_DISCOUNT || mintType == MINT_TYPE_FREE, "Invalid mint type");

        uint256 mintPrice = stakingPrice;
        bytes32 merkleRoot;
        ...
        if(mintType == MINT_TYPE_SALE) {
            ...
            saleMintedCount[msg.sender] += quantity;
            require(saleMintedCount[msg.sender] <= maxSaleMintAmount, "Max sale mint");
        } else
        if(mintType == MINT_TYPE_DISCOUNT) {
            ...
            discountMintedCount[msg.sender] += quantity;
            require(discountMintedCount[msg.sender] <= maxDiscountMintAmount, "Max discount mint");
        } else
        if(mintType == MINT_TYPE_FREE) {
            ...
            require(freeMintedCount[msg.sender] <= maxFreeMintAmount, "Max free mint");
        }
        ...
        emit Mint(msg.sender, _currentIndex-quantity, mintType, quantity, proof);
    }
    ...
    function refund(uint256[] calldata tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(stakingRefunded[tokenId] == false, "Already refunded");
            require(stakingExchanged[tokenId] == false, "Already exchanged");
        ...
        emit Refund(msg.sender, tokenIds);
    }
    ...
    function exchange(uint256[] calldata tokenIds) external {
        ...
        emit Exchange(msg.sender, tokenIds);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if(0 == bytes(baseURI).length) {
            return "https://res.cloudinary.com/wl-xyz/image/upload/v1660892694/Frame_14675_c76141cd2f.png";
        } else {
