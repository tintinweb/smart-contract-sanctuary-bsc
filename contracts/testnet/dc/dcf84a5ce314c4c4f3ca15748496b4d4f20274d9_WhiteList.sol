...
contract WhiteList is IWhiteList {
    ...
    function setWhiteList(
        ...
        _setWhiteList(token, merkleRoot, price, limit);
    }
    ...
    function whiteListPrice(address token, address addr, bytes32[] calldata merkleProof) external override view returns(uint256) {
        if(
                ...
                && verifyMerkle(token, addr, merkleProof)
                )
                return prices[token];
        return 0;
    }

    function verifyMerkle(address token, address addr, bytes32[] memory merkleProof) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(addr));
        return MerkleProof.verify(merkleProof, merkleRoots[token], leaf);
    }
}