...
contract MetaCrowd is AccessControlEnumerable {
    ...
    function parent_mapping(address[] calldata addresses, address[] calldata parents) external onlyRole(OPERATOR_ROLE) {
        ...
        MetaCrowd metaCrow = MetaCrowd(0xebc15896594Cd15Df2020A382499Bb1bF065f2B0);
        for(uint256 i = 0;i< addresses.length;i++) {
            address addr = addresses[i];
            {
            //1.用户基础数据 account
            (uint256 priorityFundAmount, uint256 freeWithdrawAmount,,,,uint8 vips,,address head,) = metaCrow.accounts(addr);
