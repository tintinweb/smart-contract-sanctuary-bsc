...
contract SafeOneChain is Ownable, IBEP20 {
    ...
    function excludeFromMaxWalletHoldingLimit(address account) external onlyOwner {
        _isExcludedFromWalletHoldingLimit[account] = true;
    }

    function includeInMaxWalletHoldingLimit(address account) external onlyOwner {
        require(
    ...
    {
        require(
            maxWalletHoldingAmount * 10**_decimals >= 3_000_000 * 10**_decimals,
            "Amount should be greater or equal to 0.1%"
        );
            ...
            if (overMinTokenBalance) {
                ...
                if (TotalTaxFee > 0) {
                    remainingLiquidityToken =
                        (contractTokenBalance * (_MarketingFee + _SeedFee + _TeamFee)) /
