...
import "./interfaces/IOracle1Inch.sol";
...
contract AltairMaster is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using HomoraMath for uint256;

    string public name;
    IStdReference bandPriceFeed;
    IChainlinkAggregator chainlinkPriceFeed;
    IPancakeRouter pancakeSwapRouter;
    IOracle1Inch oracle1Inch;
    ...
    function initialize(
        ...
        address oracle1InchCtx,
        address chainLinkPriceFeed,
        address factoryCtx
    ) public initializer {
        require(routerAddress != address(0), "Invalid router address");
        require(priceFeedAddress != address(0), "Invalid price feed address");
        require(oracle1InchCtx != address(0), "Invalid oracle 1inch address");
        require(
            chainLinkPriceFeed != address(0),
            "Invalid chainlink price feed address"
        );
        require(factoryCtx != address(0), "Invalid factory address");

        name = "AltairFund Master";
        bandPriceFeed = IStdReference(priceFeedAddress);
        pancakeSwapRouter = IPancakeRouter(routerAddress);
        oracle1Inch = IOracle1Inch(oracle1Inch);
        factory = IAltairFactory(factoryCtx);
        roles = IRoles(IAltairFactory(factoryCtx).treasury());
        chainlinkPriceFeed = IChainlinkAggregator(chainLinkPriceFeed);
    ...
    modifier onlyAdmin() {
        require(roles.isAdmin(msg.sender), "NOTADMIN");
        _;
    }
    ...
    function changeRouter(address addr) public onlyAdmin returns (bool) {
        require(addr != address(0), "Invalid router address");
        pancakeSwapRouter = IPancakeRouter(addr);
        return true;
    }
    ...
    function updateTokenNames(
        ...
        for (uint256 i; i < underlyingAddress.length; i++) {
            TokenNames[underlyingAddress[i]] = tokennames[i];
        }
    }

    function addPancakePriceToken(
        string[] memory tokenname,
        address[] calldata addressToken,
        bool[] memory usePancake
    ) public onlyAdmin {
        for (uint256 i; i < tokenname.length; i++) {
            pancakePriceToken[addressToken[i]] = usePancake[i];
        }
    }

    function getPriceByAllAddress(address from, address to)
        external
        view
        returns (uint256 rate)
    {
        if (pancakePriceToken[from]) {
            rate = getPancakePrice(from, to);
        } else {
            string memory fromToken = TokenNames[from];
            string memory toToken = TokenNames[to];
            IStdReference.ReferenceData memory data = bandPriceFeed
                .getReferenceData(fromToken, toToken);
            rate = data.rate;
        }
        return rate;
    }

    function getPriceByAddress(address targetAdd, string memory toTokenName)
        external
        view
        returns (uint256 rate)
    {
        if (pancakePriceToken[targetAdd]) {
            rate = getQuotes(targetAdd);
        } else {
            string memory fromToken = TokenNames[targetAdd];
            IStdReference.ReferenceData memory data = bandPriceFeed
                .getReferenceData(fromToken, toTokenName);
            rate = data.rate;
        }
        return rate;
    }

    function getPriceFromBand(string memory fromToken, string memory toToken)
        external
        view
        returns (uint256)
    {
    ...
    function getQuotes(address targetToken)
        public
        view
        returns (uint256 result)
    {
        result = getPancakePrice(targetToken, pancakeSwapRouter.WETH());

        return result;
    }

    function getPancakePrice(address from, address to)
        public
        view
        returns (uint256 amountOut)
    {
        address[] memory path;
        if (
            to == pancakeSwapRouter.WETH() || from == pancakeSwapRouter.WETH()
        ) {
            path = new address[](2);
            path[0] = from;
            path[1] = to;
    ...
    function getLpPrice(address token0, address token1)
        external
        view
        returns (uint256)
    {
        address pair = IPancakeFactory(pancakeSwapRouter.factory()).getPair(
            token0,
            token1
        );

        uint256 px0 = getPancakePrice(token0, pancakeSwapRouter.WETH());

        uint256 px1 = sqrt((1 * px0));

        uint256 totalSupply = IPancakePair(pair).totalSupply();

        (uint256 r0, uint256 r1, ) = IPancakePair(pair).getReserves();

        uint256 tokenReserveCumulative = sqrt(r0.mul(r1));

        // Calculate LP Token Price in accordance to the image above

        uint256 lpTokenPrice = tokenReserveCumulative
            .div(totalSupply)
            .mul(px1)
            .mul(2);

        return lpTokenPrice.mul(1e9);
    }

    function use1inchOracle(
        IERC20Upgradeable srcToken,
        IERC20Upgradeable dstToken
    ) external view returns (uint256 rate) {
        return oracle1Inch.getRate(srcToken, dstToken, true);
    }

    function changeChainlinkOracle(address addr) public onlyAdmin {
        chainlinkPriceFeed = IChainlinkAggregator(addr);
    }

    function useChainlinkOracle() external view returns (int256) {
        return chainlinkPriceFeed.latestAnswer() * 1e10;
    }
    ...
    function updateUseSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < _tokenaddress.length; i++) {
            useSupportingFeeOnTransferTokens[
                _tokenaddress[i]
            ] = _useSupportingFeeOnTransferTokens[i];
        }
    }

    function getUseSupportingFeeOnTransferTokens(address token)
        external
        view
        returns (bool)
    {
        return useSupportingFeeOnTransferTokens[token];
    }
    ...
    {
        return
            IPancakeFactory(pancakeSwapRouter.factory()).getPair(
                tokenA,
                tokenB
            );
    }
...
interface IChainlinkAggregator is HistoricAggregatorInterface {
