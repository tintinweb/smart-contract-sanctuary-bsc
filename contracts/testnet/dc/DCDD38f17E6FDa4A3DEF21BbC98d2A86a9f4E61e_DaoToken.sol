...
contract DaoToken is IERC20 ,Ownable {
    ...
    //首次发行量
    uint256 private _tTotal = 10000 * 1e9;
    ...
    //合约达到多少代币会给营销地址分红
    uint256 public minMarketingRewardAmount = 2e9;
    ...
    constructor() {
        ...
        //营销地址
        marketingAddress = 0x000014e9d69547c673Ef39fa60316462beD06bec;

        //USDT地址
        rewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;

        // Create a uniswap pair for this new token
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), rewardToken);
        _setAutomatedMarketMakerPair(uniswapV2Pair, true);
        _setAutomatedMarketMakerPair(IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH()), true);
    ...
    function _transfer(
        ...
        if (!swapping && minMarketingRewardAmount < _tOwned[address(this)] && !automatedMarketMakerPairs[from]) {
            swapping = true;
            swapAndSendDividends(_tOwned[address(this)]);
            swapping = false;
        }
        if(swapping){
            ...
            bool shouldSetInviter = balanceOf(to) == 0 && inviter[to] == address(0) && !automatedMarketMakerPairs[from];
