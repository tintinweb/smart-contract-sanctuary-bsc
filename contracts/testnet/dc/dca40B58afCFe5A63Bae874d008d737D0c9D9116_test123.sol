...
contract test123 is ERC20Detailed, Ownable {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        700 * 10**7 * 10**DECIMALS;

    uint256 public liquidityFee = 40;
    uint256 public treasuryFee = 30;
    uint256 public test123DivFee = 50;
    uint256 public sellFee = 20;
    uint256 public powerBurnFee = 50;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(test123DivFee).add(
            powerBurnFee
        );
    ...
    uint256 private constant MAX_SUPPLY = 50 * 10**12 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("test123", "test123", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x92bC95C43751a2C6bD1E9ECeE1a13aBDe141ac9d;
        treasuryReceiver = 0x813792Ca0B97AE542E3c21B54D031C78bD5216d7;   
        powerBurn = 0x000000000000000000000000000000000000dEaD;
    ...
    function takeFee(
        ...
        _gonBalances[powerBurn] = _gonBalances[powerBurn].add(
            gonAmount.div(feeDenominator).mul(powerBurnFee)
        );
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(test123DivFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(test123DivFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(test123DivFee).div(treasuryFee.add(test123DivFee))
        } () {} catch {}        
    }
