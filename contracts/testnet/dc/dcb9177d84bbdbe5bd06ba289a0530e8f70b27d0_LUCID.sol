...
contract LUCID is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "LUCID";
    string private _symbol = "LCDTEST";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0xE781c8f2402436E2D4D3823db0f9E513Ddce47e5); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0xE781c8f2402436E2D4D3823db0f9E513Ddce47e5); // BuyBack Address
    ...
    uint256 public _maxTxAmount = MAX_INT;
    uint256 public _walletMax = MAX_INT;
    ...
    constructor () {
        ...
        uint256 halfTotalSupply = _totalSupply.div(2);
        _balances[_msgSender()] = halfTotalSupply;
        _balances[address(this)] = halfTotalSupply;
        emit Transfer(address(0), _msgSender(), halfTotalSupply);
        emit Transfer(address(0), address(this), halfTotalSupply);
        _basicTransfer(address(this), deadAddress, halfTotalSupply);
    }
    ...
    function clearContract (address payable to) public onlyOwner {
    ...
    function burnTokens(uint256 amount) external onlyOwner {
       ...
       require(contractBalance >= amount,"Insufficient Tokens To Burn");

        _basicTransfer(address(this), deadAddress, amount);
    }
