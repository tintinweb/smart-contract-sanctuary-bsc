...
contract TShareSwapper is Operator {
   ...
   function init(
      ...
      if (_tbond != address(0)) tbond = IERC20(_tbond);
      if (_tshare != address(0)) tshare = IERC20(_tshare);
      if (_wftmAddress != address(0)) wftmAddress = _wftmAddress;
      if (_tombSpookyLpPair != address(0)) tombSpookyLpPair = _tombSpookyLpPair;
      if (_tshareSpookyLpPair != address(0)) tshareSpookyLpPair = _tshareSpookyLpPair;
      if (_daoAddress != address(0)) daoAddress = _daoAddress;
   }
   ...
   function estimateAmountOfTBond(uint256 _tshareAmount) external view returns (uint256) {
      uint256 tshareAmountPerTomb = getTShareAmountPerTomb();
      return _tshareAmount.mul(1e18).div(tshareAmountPerTomb);
   }
   ...
   function getTombPrice() public view returns (uint256) {
      return IERC20(wftmAddress).balanceOf(tombSpookyLpPair).mul(1e18).div(tomb.balanceOf(tombSpookyLpPair));
   }

   function getTSharePrice() public view returns (uint256) {
      return IERC20(wftmAddress).balanceOf(tshareSpookyLpPair).mul(1e18).div(tshare.balanceOf(tshareSpookyLpPair));
   }

   function getTShareAmountPerTomb() public view returns (uint256) {
      uint256 tombPrice = IERC20(wftmAddress).balanceOf(tombSpookyLpPair).mul(1e18).div(
         tomb.balanceOf(tombSpookyLpPair)
      );

      uint256 tsharePrice = IERC20(wftmAddress).balanceOf(tshareSpookyLpPair).mul(1e18).div(
         tshare.balanceOf(tshareSpookyLpPair)
      );
