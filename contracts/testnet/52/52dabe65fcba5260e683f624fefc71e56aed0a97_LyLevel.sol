...
contract LyLevel is Initializable, OwnableUpgradeable, IERC20 {
    ...
    function claimable(uint256 _batchId, address _account) public view returns (uint256) {
        if (_batchId > currentBatchId) {
            ...
            if (batchRewardDuration == 0) {
                return (totalReward / PRECISION) - _rewards[_batchId][_account];
            } else {
                BatchInfo memory batch = batches[_batchId];
                uint256 duration = block.timestamp >= (batch.allocatedTime + batchRewardDuration)
                    ? batchRewardDuration
                    : (block.timestamp - batch.allocatedTime);
                return (totalReward * duration / batchRewardDuration / PRECISION) - _rewards[_batchId][_account];
            }
        }
    }
    ...
    function setBatchRewardDuration(uint256 _duration) external onlyOwner {
        batchRewardDuration = _duration;
        emit BatchRewardDurationSet(_duration);
    }
    ...
    event BatchRewardDurationSet(uint256 duration);
}