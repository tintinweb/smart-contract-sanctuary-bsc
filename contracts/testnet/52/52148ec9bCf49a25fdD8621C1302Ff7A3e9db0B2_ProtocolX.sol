...
contract ProtocolXAdminController is Initializable {
    function initialize(
        ...
        LibProtocolX.initialize(name_, symbol_, router_, treasury_, starttime_, liquiditiyFee,treasuryFee,insuranceFundFee,sellAfterBurnerFee);
    }
...
library LibProtocolX {
    ...
    function initialize(
        ...
        s.gonsPerFragment = LibProtocolXConstants.TOTAL_GONS.div(
            s.totalSupply
        );
...
/*         LibProtocolXFeesManager.add(keccak256("LIQUIDITY_FEE"), liquiditiyFee);
    ...
    function getCirculatingSupply() internal view returns (uint256) {
        Storage storage s = _storage();
        uint256 circulatingSupply = LibProtocolXConstants.TOTAL_GONS.sub(s.gonBalances[LibProtocolXConstants.DEAD]);
         
        return balanceForGons(circulatingSupply);
        //return (LibProtocolXConstants.TOTAL_GONS.sub(s.gonBalances[LibProtocolXConstants.DEAD]).sub(s.gonBalances[LibProtocolXConstants.ZERO])).div(s.gonsPerFragment);
    }
