...
interface StakingPool {
    function updatePoolReward(uint256 yerstodayBurnAmount) external returns (bool);
}
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if(to== address(0x000000000000000000000000000000000000dEaD)){
            ...
            if(block.timestamp > startTime + 1 hours){
                ...
                StakingPool(stakingPool).updatePoolReward(dayDestroyAmount);
            }
        }
        //白名单地址不扣手续费
        if (feeWhitelist[from]) {
            ...
            if (to == pairAddress || from == pairAddress) {
                ...
                //1.5%到财富节点与主节点分红合约
                uint256 toNodeFee = amount * 15 / 1000;
                //
                balanceOf[from] -= amount;
                balanceOf[pairAddress] += toBuyBackFee;
                balanceOf[superNodeAddress] += toSuperNodeFee;
                balanceOf[nodeAddress] += toNodeFee;
                if (inviter[from] != address(0)) {
                    ...
                    //如果未绑定邀请关系，0.5%销毁
                    _burn(from, amount * 5 / 1000);
                }
                balanceOf[to] += amount * 95 / 100;
            } else {
                //绑定邀请关系
                if (to != address(0) && inviter[to] != address(0)) {
                    inviter[to] = from;
                }
                balanceOf[from] -= amount;
                balanceOf[to] += amount;
