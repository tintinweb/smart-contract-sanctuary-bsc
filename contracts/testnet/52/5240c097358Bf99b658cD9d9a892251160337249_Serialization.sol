...
contract Serialization is ReentrancyGuard {
    ...
    IMarketplace private _market = IMarketplace(0x0AC693CbEc234fC29d8738195E3514365c19d570); //TEST NETWORK CONTRACTS
    IFractionalSale private _fSale = IFractionalSale(0xDaBa7171B31A5cDde6e60354CBc9005be81264aA);
    ...
    function setContracts(address marketplaceAddr, address fractionalSaleAddr) external {
        require(msg.sender == _market.owner(), "StandardSale: only mp owner");
    ...
    function serialize(
        ...
        require(numberOfEditions > 0, "Serialization: editions cannot be zero");
        require(_idToSerializationInfo[itemId].editions == 0, "Serialization: already serialized");
        (address frToken, , ) = _fSale.getFractionInfo(itemId);
        require(frToken == address(0), "Serialization: cannot be fractional NFT");
    ...
    /// @param _saleToken Address of token to be used for sale
    function createSteppedSale(
        ...
        require(msg.sender == _idToSerializationInfo[itemId].collectionCreator, "Serialization: caller must be owner");
        require(_saleStartTime >= block.timestamp && _saleEndTime > _saleStartTime, "Serialization: invalid time");
    ...
    function purchaseEdition(uint256 itemId) external nonReentrant {
        if(_market.isPublic() == false) require(_market.isWhitelisted(msg.sender), "Serialization: not whitelisted");
        SteppedSale memory s_sale = _idToSteppedSale[itemId];
        SerializedNFT memory s_info = _idToSerializationInfo[itemId];
        require(block.timestamp > s_sale.saleStartTime && block.timestamp < s_sale.saleEndTime, "Serialization: sale ended");

        (uint price, ) = getStepPrice(itemId);
        uint tokenId = s_info.editions - s_sale.totalSold;
        require(tokenId > 0, "Serialization: sold out");
