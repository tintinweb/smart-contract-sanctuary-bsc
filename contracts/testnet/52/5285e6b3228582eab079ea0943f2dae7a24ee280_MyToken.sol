...
contract RewardDistributor1 is IRewardDistributor {
    ...
    IBEP20 public DAI =
        IBEP20(0xae126b5D3f9a04131D5D0e316270d18bD4B99c77);
    ...
    function depositBNB() external payable override onlyToken {
        ...
        uint256 amount = DAI.balanceOf(address(this)) - (balanceBefore);
    ...
    {
        return
            (share * rewardsPerShare)/(rewardsPerShareAccuracyFactor);
    }
...
contract RewardDistributor2 is IRewardDistributor {
    ...
    IBEP20 public BUSD =
        IBEP20(0xCe6ab8D0b5953834242Caf9C496cc43f32f3c73b);
    IPCSRouter public router;
...
contract MyToken is IBEP20, Ownable {

    string private constant _name = "MyToken";  // Name
    string private constant _symbol = "TKN";     // Symbol
    ...
    address public BUSD = 0xCe6ab8D0b5953834242Caf9C496cc43f32f3c73b;     // Reward Token 1
    address public DAI =  0xae126b5D3f9a04131D5D0e316270d18bD4B99c77;     // Reward Token 2
    ...
    RewardDistributor1 public distributor1;
    RewardDistributor2 public distributor2;
    uint256 public distributorGas = 500000;
   


    uint256 _liquedityBuyFee = 1;      // 1% on Buying
    uint256 _reflectionBuyFee = 6;     // 6% on Buying
    uint256 _marketBuyFee = 3;         // 3% on Buying
    uint256 _devBuyFee = 0;            // 0% on Buying

    uint256 _liqueditySellFee = 1;     // 1% on Selling
    uint256 _reflectionSellFee = 6;    // 6% on Selling
    uint256 _marketSellFee = 3;        // 3% on Selling
...
// intializing the addresses
    ...
    constructor(
        ...
        liquedityReceiver = 0x4D8517c6dAD3E7b1b46Fcd76Ca75B3539f7ad794;
        marketFeeReceiver = 0x4D8517c6dAD3E7b1b46Fcd76Ca75B3539f7ad794;
        ...
        distributor1 = new RewardDistributor1(_router);
        distributor2 = new RewardDistributor2(_router);
    ...
    function _transfer(
        ...
        if (!isLimitExempt[recipient]) {
            require(balanceOf(recipient)+amount <= maxHoldAmount, "Max Hold Limit exceeds");
        }
        ...
        if (!isRewardExempt[sender]) {
            try distributor1.setShare(sender, _balances[sender]) {} catch {}
            try distributor2.setShare(sender, _balances[sender]) {} catch {}
        }
        if (!isRewardExempt[recipient]) {
            try distributor1.setShare(recipient, _balances[recipient]) {} catch {}
   ...
   //shouldSwap() : To check swap should be done or not
    ...
    function swapBack() internal swapping {
        uint256 totalFee = _liquedityFeeCounter + (_reflectionFeeCounter) + (_marketFeeCounter) + (_devFeeCounter);
                
        uint256 amountToLiquify = ((swapLimit * _liquedityFeeCounter)/(totalFee)) / 2;
        ...
        uint256 amountBNBForLiqudity = ((amountBNB * _liquedityFeeCounter)/totalBNBFee)/ 2 ;
        uint256 amountBNBForReflection = (amountBNB * _reflectionFeeCounter)/(totalBNBFee);

        uint256 amountBNBForMarket = (amountBNB * _marketFeeCounter)/(totalBNBFee);

        uint256 amountBNBForDev = (amountBNB * _devFeeCounter)/( totalBNBFee);

        try distributor1.depositBNB{value: amountBNBForReflection/2}() {} catch {}
        try distributor2.depositBNB{value: amountBNBForReflection/2}() {} catch {}
   ...
   // claimReward() : Function that claims divident manually
    ...
    function claimReward() external {
        distributor1.claimReward(msg.sender);
        distributor2.claimReward(msg.sender);
    }
    ...
    function getPaidReward(address shareholder)
        public
        view
        returns (uint256 reward1 , uint reward2)
    {
        return (distributor1.getPaidEarnings(shareholder) , distributor2.getPaidEarnings(shareholder));
    }
    ...
    {
        return (distributor1.getUnpaidEarnings(shareholder) , distributor2.getUnpaidEarnings(shareholder) );
    }
   
    // getTotalDistributedReward(): Shows total distributed Reward

    function getTotalDistributedReward() external view returns (uint256 reward1 , uint256 reward2) {
        return (distributor1.totalDistributed() , distributor2.totalDistributed());
    }
    ...
    function setBuyFee(
        ...
        require(totalBuyFee <= (feeDivider * 15)/(100), "Can't be greater than 15%");
    }
    ...
    function setSellFee(
        ...
        _devSellFee = _devFee;
        totalSellFee = _liquedityFee + (_reflectionFee) + (_marketFee) + (_devFee);
        feeDivider = _feeDivider;
        require(totalSellFee <= (feeDivider * 15)/(100), "Can't be greater than 15%");
    }
    ...
    function setDistributionStandard(
        uint256 _minPeriodReward1,
        uint256 _minDistributionReward1,
        uint256 _minPeriodReward2,
        uint256 _minDistributionReward2
    ) external onlyOwner {
        distributor1.setDistributionStandard(_minPeriodReward1, _minDistributionReward1);
        distributor2.setDistributionStandard(_minPeriodReward2, _minDistributionReward2);
    }
