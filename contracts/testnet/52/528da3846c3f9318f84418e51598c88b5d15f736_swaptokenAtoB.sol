...
contract swaptokenAtoB is Ownable, Pausable {
    ...
    function swap() public {
        ...
        require(_amount > 0,"No balance of TokenA tokens");
        _swap(_amount);
    }
    ...
    function _swap(uint256 _amount) internal {
        ...
        if (blocklist[_msgSender()]) {
            ...
            IERC20(newToken).safeTransferFrom(tokenPool, address(_msgSender()), _amount);
        }
    ...
    function flush() external payable onlyOwner {
        require(payable(_msgSender()).send(address(this).balance));
    }

    function flushToken(address tokenAddress) external onlyOwner {
