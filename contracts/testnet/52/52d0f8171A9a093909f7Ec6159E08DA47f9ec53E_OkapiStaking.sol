...
contract OkapiStaking is OlympusAccessControlled {
    ...
    constructor ( 
        ...
        epoch = Epoch({
            ...
            endBlock: block.timestamp.add(_epochLength),
            distribute: 0
        });
    }
    ...
    function setRewardRate(uint _rate) external onlyGovernor() {
        rewardRate = _rate;
    }
    ...
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        IDistributor( distributor ).addRecipient(_recipient, rewardRate);
        return true;
    }

    function setVestingPeriod(uint _period, address _user) external onlyGovernor() {
        VestingPeriod memory periodInfo = vestingPeriodInfo[ _user ];
        vestingPeriodInfo[_user] = VestingPeriod({
            period: _period,
            user: _user,
    ...
    /**
        ...
        @param _amount uint
        @param _deductFee uint
        @param _trigger bool
     */
    function unstake( uint _amount, uint256 _deductFee, bool _trigger ) external {
        if ( _trigger ) {
            rebase();
        }
        IERC20( sOKP ).safeTransferFrom( msg.sender, address(this), _amount );
        IERC20( OKP ).approve( address(this), _amount );
         VestingPeriod memory periodInfo = vestingPeriodInfo[ msg.sender ];
         if(block.timestamp >= periodInfo.period) {
            IERC20( OKP ).safeTransfer( msg.sender, _amount );
         } else {
             uint256 newAmount = _amount.sub(_deductFee.mul(2));
             IOHM(OKP).burnFrom( msg.sender, _deductFee ); // burn 5% as a penalty fee
             IERC20( OKP ).safeTransfer( teamWallet, _deductFee ); // sent to team wallet according to penalty fee
             IERC20( OKP ).safeTransfer( msg.sender, newAmount );
         }
    }

   function setTeamWallet(address _teamWallet) external onlyGovernor() {
    ...
    function rebase() public {
        if( epoch.endBlock <= block.timestamp ) {
 ...
 function secondsToNextEpoch() external view returns (uint256) {
        return epoch.endBlock.sub(block.timestamp);
    }    /**
