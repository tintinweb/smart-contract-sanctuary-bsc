...
contract TestOnly is Context, IERC20, Ownable {
    ...
    string constant private _name = "Test Only 14";
    string constant private _symbol = "TST 14";
    ...
    IERC20 public pairCurrencyToken = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    constructor () {
        ...
        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this), address(pairCurrencyToken));
    ...
    function updateRouter(address newRouterAddress) public onlyOwner returns(address newPairAddress) {
        ...
        newPairAddress = IDexFactory(dexRouter_.factory()).getPair(address(this), address(pairCurrencyToken));

        if(newPairAddress == address(0)) { //Create If Doesnt exist
            newPairAddress = IDexFactory(dexRouter_.factory()).
                                createPair(address(this), address(pairCurrencyToken));
        }
    ...
    function transferToAddressETH(address payable recipient, uint256 amount) private {
        ...
        pairCurrencyToken.transfer(recipient, amount);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        swapBack(tokensForSwap);
        
        // uint256 amountReceived = pairCurrencyToken.balanceOf(address(this));
        //pairCurrencyToken.approve(address(dexRouter), amountReceived);

        // uint256 shares = totalShares - (lpShare / 2);
        
        // uint256 bnbForLiquidity = ((amountReceived * lpShare) / shares) / 2;
        // uint256 bnbForOperations = (amountReceived * operationsShare) / shares;
    ...
    function swapBack(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> pair currency
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(pairCurrencyToken);

        _approve(address(this), address(dexRouter), tokenAmount);
        uint256[] memory amountsOut = dexRouter.getAmountsOut(tokenAmount, path);

        // make the swap
        dexRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            tokenAmount,
            amountsOut[1], // accept any amount of ETH
            path,
