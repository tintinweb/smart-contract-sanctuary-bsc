...
contract Coin9419V2 is IERC20, Context, Ownable, Operator {
    ...
    address private _router =
        address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); //prod
    address private factoryAddress;
    address private usdtAddress =
        address(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); // prod

    address private marketingAddress =
        address(0x36FE48af2cd89A5054Af17Fc93705f84139afA8b); //
    address private repurchaseAddress =
        address(0x5F04143f0974d79fE279c0b0D5616CA764cE8A62); // default repurchase fee address
    address public tokenReceiver;

    address private foundationAddress =
        address(0x0dd9Da914D697D104Ca449323C95F75d9abbd30C);

    address private destroyFeeAddress = address(0);
    address private serviceFeeAddress =
        address(0xB5c261daf9d5cf8636F6054C056047D9CC742BEA);
    ...
    //----------Fee Config-----------//
    function setFoundationFeeAddress(
    ...
    function setRepurchaseFeeAddress(
        address _repurchageAddr
    ) external onlyOwner {
        require(
            _repurchageAddr != address(0),
            "Invalid repurchase fee address"
        );
     ...
     */
    function getAllOfConfigAddress()
        external
        view
    ...
    function getForefathers(
        address owner,
        uint num
    ) public view returns (address[] memory fathers) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (to == destroyFeeAddress) {
            uint _day = getDay();
            dayDestroyTotal[_day] = dayDestroyTotal[_day].add(
    ...
    function _repurchaseMbankPool() private {
        if (enableRepurchase) {
            try
                I9419Repurchase(repurchaseAddress).autoSwapAndAddToMarketing()
            {} catch {}
        }
    }
    ...
    function _dividendUsdt(uint256 _totalDiv) private {
        ...
        if (_totalUsdtBal > 0) {
            ...
            uint256 liquidityUsdt = _totalUsdtBal.sub(serviceUsdt).sub(
                foundationUsdt
            );
            if (serviceUsdt > 0) {
    ...
    event ParamEvent(
        ...
        uint256 tSerivceFee,
        uint256 tDestroyFee,
        uint256 tFoundationFee,
        uint256 tRepurchaseFee,
        uint256 tTransferAmount,
        string a
    );
    event FatherBonus(
        address indexed sender,
        address indexed father,
        uint256 bonus
    );

    function _dividendMarketingBonus(
        uint256 fatherBonus,
        uint256 buyerBonus,
        address recipient
    ) private {
    ...
    function _marketingBonus(
        address buyer,
        uint256 tAmount
    ) private view returns (uint256 fatherBonus, uint256 buyerBonus) {
