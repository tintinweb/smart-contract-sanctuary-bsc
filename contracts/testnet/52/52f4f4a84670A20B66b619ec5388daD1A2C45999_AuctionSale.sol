...
contract AuctionSale is ReentrancyGuard {
    ...
    /// @param itemId Item ID of NFT
    /// @param amountOfTokens Amount of ERC20 tokens to bid - Must be approved first
    function placeBid(uint256 itemId, uint256 amountOfTokens) external notBlacklisted nonReentrant {
        ...
        IERC20 sToken = IERC20(auction.saleToken);
        ...
        if(auction.highestBid == 0) {
            require(amountOfTokens >= auction.reserve, "AuctionSale: insufficient amount");
        }else{
            require(amountOfTokens > auction.highestBid, "AuctionSale: insufficient amount");
            _refundPrevHighestBidder(auction.highestBidder, auction.saleToken, auction.highestBid);
        }
        _idToAuction[itemId].highestBidder = msg.sender;
        _idToAuction[itemId].highestBid = amountOfTokens; 
    ...
    function acceptBid(uint256 itemId) external nonReentrant {
        ...
        require(auction.auctionEndTime < block.timestamp, "AuctionSale: already listed for sale");
        if(auction.highestBid == 0) {
