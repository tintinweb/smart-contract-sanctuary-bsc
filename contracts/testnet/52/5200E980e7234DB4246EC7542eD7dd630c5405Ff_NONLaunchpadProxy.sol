...
contract NONLaunchpadProxy is ILaunchpadProxy, Ownable, ReentrancyGuard {
    ...
    uint256 internal constant LPD_FLAG_MAX = 0;

    // process buy additional flag
    // whitelist max buy number
    uint256 internal constant BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM = 0;
    ...
    mapping(bytes4 => Launchpad) launchpads;

    // launchpad dynamic vars
    mapping(bytes4 => LaunchpadVar) launchpadVars;
    ...
    // process buy
    function processBuy(
        Launchpad memory lpad,
        AccountRoundsStats memory accountStats,
        uint256 roundsIdx,
        address sender,
        uint256 quantity,
        uint256 flag,
        ...
        string memory ret = checkLaunchpadBuy(
            ...
            sender,
            quantity,
            additional.length > BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM
                ? additional[BUY_ADDITIONAL_IDX_WL_MAX_BUY_NUM]
        ...
        if (flag == 0) {
            ...
            callLaunchpadBuy(lpad, roundsIdx, sender, quantity, sourceAddress);
            // increase sale num
            lpad.roundss[roundsIdx].saleQuantity =
                lpad.roundss[roundsIdx].saleQuantity +
                uint32(quantity);
        }
    ...
    function callLaunchpadBuy(
        ...
        LaunchpadRounds memory lpadRounds = lpad.roundss[roundsIdx];
        uint256 tokenId = lpadRounds.startTokenId + lpadRounds.saleQuantity;
        // example bytes4(keccak256("safeMint(address,uint256)")),
        bytes4 selector = lpad.abiSelectorAndParam[ABI_IDX_BUY_SELECTOR];
        bytes4 paramTable = lpad.abiSelectorAndParam[ABI_IDX_BUY_PARAM_TABLE];
    ...
    function addLaunchpadAndRounds(
        ...
        LaunchpadRounds[] memory roundss,
        bool lockParam,
        bool enable
    ...
    function updateRoundsStartTimeAndFlags(
        ...
        require(flags.length == LPD_FLAG_MAX, Enums.LPD_ROUNDS_FLAGS_ARRAY_LEN);
    ...
    function updateTargetContractAndABI(
        ...
        // buy selector check
        require(
            abiSelector[ABI_IDX_BUY_SELECTOR] != bytes4(0),
    ...
    function checkAddLaunchpadRounds(LaunchpadRounds memory rounds)
        external
        pure
    {
    ...
    function checkLaunchpadBuy(
        Launchpad memory lpad,
        AccountRoundsStats memory accStats,
        uint256 roundsIdx,
        address sender,
        uint256 quantity,
        ...
        // whitelist check
        if (lpadRounds.whiteListModel != WhiteListModel.NONE) {
            return
                checkWhitelistBuy(
                    lpad,
                    roundsIdx,
    ...
    {
        Launchpad memory lpad = launchpads[launchpadId];
        boolData = new bool[](2);
        ...
        )
    {
        ...
        boolData = new bool[](LPD_FLAG_MAX + 1);
        // launchpad enable
        boolData[0] = lpad.enable;
    ...
    {
        Launchpad memory lpad = launchpads[launchpadId];
        AccountRoundsStats memory accountStats = launchpadVars[launchpadId]
        ...
        boolData[0] = lpadRounds.whiteListModel != WhiteListModel.NONE;
    ...
    function isWhiteListModel(
        ...
        if (whiteListModel == WhiteListModel.NONE) {
            return false;
        }
