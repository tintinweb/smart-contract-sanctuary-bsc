...
contract Diamond is ERC20, Ownable {
    ...
    uint256 private minContractTokensToSwap = 2 gwei;
    ...
    uint256 public totalTokensForLiquidityPool;
    uint256 public minTokensForAddingLiquidityPool = 2 gwei;
    ...
    function manualSendAll() public onlyOwner {
    ...
    function setMinTokensForAddingLiquidityPool(uint256 _numToken)
        public
        onlyOwner
    {
        minTokensForAddingLiquidityPool = _numToken;
    }
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            //get the available tokens for swap by substracting the total tokens for LP and rewards
            uint256 availableTokens = balanceOf(address(this))
                .sub(totalTokensForLiquidityPool)
                .sub(totalTokenForRewards);

            if (availableTokens > minContractTokensToSwap) {
                if (!swapAll) {
                    availableTokens = minContractTokensToSwap;
                }
                distributeTokensEth(availableTokens);
            }
    ...
    //contract balances
    function overAllContractBalance() public view returns (uint256) {
        return balanceOf(address(this));
    }

    function availableContractBalanceForSwap() public view returns (uint256) {
        return
            balanceOf(address(this)).sub(totalTokensForLiquidityPool).sub(
                totalTokenForRewards
            );
    }
