...
contract BorrowNFT is Ownable {
    ...
    event BorrowETH(address user, uint256 borrowedUSDT, uint256 ethValue);
    event BorrowNFT(address user, uint256 borrowedUSDT, uint256[] tokenID);
    ...
    function borrowETH() external payable whenNotPaused ifNotBorrowed {
        ...
        emit BorrowETH(user, usdtReturn, msgValue);
    }

    function calculateETHReturn(uint256 ethValue) public view returns(uint256) {
        ethValue = ethValue * price;
    ...
    function borrowNFT(uint256[] memory tokenIds) external whenNotPaused ifNotBorrowed {    
        ...
        USDT.safeTransfer(user, usdtReturn);
        emit BorrowNFT(user, usdtReturn, tokenIds);
    }

    function transferFromcolletralNFT(address user, uint256[] memory tokenIds) private {
        for (uint256 i = 0; i < tokenIds.length; i++){
            collateralNFT.safeTransferFrom(user, address(this), tokenIds[i]);
        }
    } 

    function calculateNFTReturn(uint256 totalNFT) public view returns(uint256) {
    ...
    modifier checkRedemptionTime(address _borrower) {
        require( (borrowerInfo[_borrower].timestamp + endTime)  > block.timestamp , "Too late!.." );
        _;

    }

    function repay(uint256 _usdtAmount) external ifOnlyBorrowed(msg.sender) checkRedemptionTime(msg.sender) {
        address _user = msg.sender;
        require(_usdtAmount == calculateRedemptionWithInterest(_user), "Insufficient usdt amount to redeem collateral");
    ...
    function repayETHcollateral(address payable _borrower) private {
        ...
        _borrower.transfer(borrowerInfo[_borrower].amount);
    }
    ...
    function repayNFTcollateral(address _borrower) private {
        ...
        uint256[] memory tokenIds = borrowerInfo[_borrower].ids;
         for (uint256 i = 0; i <  tokenIds.length; i++){
            collateralNFT.safeTransferFrom(address(this), _borrower, tokenIds[i]);
            // borrowerInfo[_user].ids[i] = tokenIds[i]
        }
    }   
    ...
    function getInterest(address _user) public view returns(uint256) {
        ...
        if((borrowerInfo[_user].timestamp + endTime)  > block.timestamp){
           intrst = 1 + ((block.timestamp - borrowerInfo[_user].timestamp) / interval);
           intrst = (intrst * (feePcent)) * borrowerInfo[_user].usdtAmount / 10000;
        } else {
            intrst = (3 * (feePcent)) * borrowerInfo[_user].usdtAmount / 10000;
        } 

        return intrst;
    }

    function calculateRedemptionWithInterest(address _user) public view returns(uint256){
        uint256 pay = getInterest(_user);
        pay = pay + borrowerInfo[_user].usdtAmount;
        return pay;
    }
