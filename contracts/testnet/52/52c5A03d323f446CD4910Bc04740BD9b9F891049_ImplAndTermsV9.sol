...
contract ImplAndTermsV9 is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    function initialize(
        ...
        holdTimeRefMin = 0 days;
        holdTimeMax = 1 days;
        timeNormalizer = 1 hours;
    ...
    function unstake(uint[] calldata userStakeIds) external {
        ...
        for (uint i = 0; i < userStakeIds.length; i++) {
            ...
            amountOut = calcAmountOut(stakeAmount, block.timestamp, stakeTime, holdTime, 1e18);
            amountOut += inflationRatePerSec * (block.timestamp - stakeTime) * amountOut / 100e18;

            // stakeTokenAmountOut += amountOut;

            timeBonus = calcBonusTime(amountOut, holdTime, false);
        ...
        doTransferOut(stakeToken, msg.sender, stakeTokenAmountOut - notApprovedStakeTokenAmountOut);
    }
    ...
    function calcAmountOut(uint stakeAmount, uint currentTimestamp, uint stakeTime, uint holdTime, uint) public view returns (uint) {
        ...
        uint feeAmount = calcFee(stakeAmount, currentTimestamp, stakeTime, holdTime, 0);
