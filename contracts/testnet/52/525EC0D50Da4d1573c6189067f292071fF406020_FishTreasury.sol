    ...
    ReentrancyGuard
{
    ...
    event FixGun(address indexed user, uint256 amount, uint256 tokenId);
    ...
    // Mapping box number to box price
    mapping(uint256 => uint256) public boxPrice;

    // Mapping box number fix gun amount
    mapping(uint256 => uint256) public fixGunAmount;
    ...
    constructor (
        ...
        require(
            ...
            "Invalid FishTreasury wallet address"
        );

        require(
            address(_rewardWallet) != address(0), 
            "Invalid rewardSC address"
        );
     ...
     * @param _amountToFixGun: amount to fix gun
     */
    function setBoxProperties(
        uint256 _boxNumber,
        uint256 _amountToMint,
        uint256 _amountToFixGun
    ) public onlyOwner {
        boxPrice[_boxNumber] = _amountToMint;
        fixGunAmount[_boxNumber] = _amountToFixGun;
    }
    ...
    function swapDiamond(
        ...
        emit SwapDiamond(_msgSender(), _amount, diamondAmount, _serverId);
    ...
    function mintNFT(
        ...
        uint256 amountToMint = boxPrice[_boxNumber];
        uint256 balance = tokenAddress.balanceOf(_msgSender());

        require(
            balance >= amountToMint,
            "Insufficient token in your wallet"
        );

        require(
            _amount == amountToMint,
            "Wrong amount to mint"
        );
        ...
        // Mint NFTs
        uint256 tokenId =  totalMintedNFT[_boxNumber] += 1;
        tokenAddress.transferFrom(
            _receipient, 
            address(rewardWallet), 
            _amount
        );
        emit ClaimNFT(_receipient, _amount, tokenId);
    }       

    /**
     * @notice fix gun using BNB
     * @param _tokenId: token Id of gun that user want to fix
     * @param _tokenAddress: token address used to fix gun
     * @param _boxNumber: box number that user want to mint
     * @param _amount: amount used to fix gun
     */
    function fixGun(
        uint256 _tokenId,
        address _tokenAddress,
        ...
        uint256 amountToFixGun = fixGunAmount[_boxNumber];
        uint256 balance = tokenAddress.balanceOf(_msgSender());
        require(
            balance >= _amount,
            "Insufficient token in your wallet"
        ...
        require(
            _amount == amountToFixGun,
            "Wrong amount to fix gun"
        );
        ...
        emit FixGun(_msgSender(), _amount, _tokenId);
    }

    /**
     * @notice rescue Token
     * @param _tokenAddress: token Id of gun that user want to fix
     * @param _amount: token Id of gun that user want to fix
     */
    function rescueToken(
        ...
        IERC20 tokenAddress = IERC20(_tokenAddress);
        tokenAddress.transfer(_msgSender(), _amount);
    }
}
