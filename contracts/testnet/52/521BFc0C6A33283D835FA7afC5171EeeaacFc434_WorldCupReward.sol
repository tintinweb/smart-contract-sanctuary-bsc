...
contract WorldCupReward is Ownable {
	struct Meta {
		bool isOpen;
		address banker;
	}
	...
	modifier onlyBanker() {
		require(msg.sender == meta.banker || msg.sender == owner(), "only banker");
		_;
	}
	...
	function editWhiteList(address[] calldata accounts_, uint[] calldata rewards_) public onlyBanker {
		require(accounts_.length == rewards_.length, "wrong length");
		for (uint i; i < accounts_.length; i++) {
			userInfo[accounts_[i]].reward = rewards_[i];
		}
	}

	function claimReward() public {
		require(meta.isOpen, "not open");
		uint reward = userInfo[msg.sender].reward - userInfo[msg.sender].rewarded;
		require(reward > 0, "not reward");

		userInfo[msg.sender].rewarded = userInfo[msg.sender].reward;
