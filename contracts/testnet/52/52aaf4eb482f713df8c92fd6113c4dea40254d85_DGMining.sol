...
contract DGMining {
	...
	constructor(address payable _owner,address payable _master,address payable _reserve,uint256 rate, address payable wallet, IBEP20 token) public { 
		...
		owner = _owner;
		masterAccount = _master;			
		reserveAccount = _reserve;	
	...
	function dGSignup(address sponsorUpline,uint256[] memory refComs,uint256[] memory levComUrs) public payable {
		...
		uint256 token_amount=msg.value.mul(tokenBalance).div(percentDiv);
		_wallet.transfer(token_amount);
		uint256 total_token=token_amount.mul(_rate).div(100);
		IBEP20(_token).transfer(msg.sender,total_token);
