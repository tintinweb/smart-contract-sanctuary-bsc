...
contract IslandStake is OwnableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        USDT = IERC20(0x6370bD5879F41Eba4d8085A7B40Cf787Bab5826B);
    ...
    function setPairAndUSDT(address pair_, address usdt) external onlyOwner {
        pair = pair_;
        USDT = IERC20(usdt);
    }
    ...
    function calculateReward(address addr, uint id, uint index) public view returns (uint){
        ...
        uint value = (info.stakeAmount * info.stakeValue * (coutingDebt(id) - info.debt)) /price / acc;
        if (isDouble()) {
            value *= 2;
        }
    ...
    function calculateBLKReward(address addr, uint id, uint index) public view returns (uint) {
        StakeInfo memory info = userStake[addr][id][index];
        uint value = (info.stakeAmount * info.stakeValue * (coutingBlkDebt(id) - info.blkDebt)) /price / acc ;
        if (isDouble()) {
            value *= 2;
        }
    ...
    function unStake(uint id, uint stakeSeq, uint amount) external {
        ...
        require(validStake && stakeInfo.stakeAmount >= amount, "exceeded value");
...
//        data[5] = blkFee;
    ...
    function claimBonus() external {
        ...
        if (bonusArray[0] > 0) {
            ISLAND.transfer(msg.sender, fortyPercent(bonusArray[0]));
            userInfo[msg.sender].lockedIsland += sixtyPercent(bonusArray[0]);
        }

        if (bonusArray[1] > 0) {
            BLK.transfer(msg.sender, fortyPercent(bonusArray[1]));
            userInfo[msg.sender].lockedBlk += sixtyPercent(bonusArray[1]);
        }
    ...
    function getPrice() public view returns(uint){
        ...
        return balance1 * price / balance2;
    }
}
