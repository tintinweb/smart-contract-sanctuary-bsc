...
contract DET is IERC20, IERC20Metadata, Ownable, MiningToken {
    ...
    function _tokenTransferBuy(
        ...
        _tokenTransfer(from, to, amount * 10 / 100);
...
contract MiningToken is Epoch {
    ...
    uint256 private _totalSupplySession;
    ...
    function totalSupply() public view virtual returns (uint256) {
        ...
        return amount + epochTotal * (currentSession() - lastSession) / (EPOCH_PERIOD / SESSION_PERIOD);
    }
    ...
    function _tokenTransfer(
        ...
        uint256 initialAmount = amount * _totalSupplyInitial / _totalSupplySession;
    ...
    function updateMining() internal {
        ...
        uint256 epochTotal = amount * getMultiplyByEpoch(_currentEpoch) / MULTIPLY;
        amount += epochTotal * (currentSession() - lastSession) / (EPOCH_PERIOD / SESSION_PERIOD);
...
contract Epoch {
    ...
    uint256 constant public SESSION_PERIOD = 20;
    ...
    uint256 public lastSessionBlock;
    ...
    function updateEpoch() internal {
        ...
        uint256 sessionCount = (block.number - lastEpochBlock) / SESSION_PERIOD;
        lastSession = sessionCount;
        lastSessionBlock = lastEpochBlock + sessionCount * SESSION_PERIOD;
    }
    ...
    function currentSession() public view returns(uint256) {
        uint256 sessionCount = (block.number - currentEpochBlock()) / SESSION_PERIOD;
        return sessionCount;
    }

    function currentSessionBlock() public view returns(uint256) {
        uint256 sessionCount = (block.number - lastSessionBlock) / SESSION_PERIOD;
        return currentEpochBlock() + sessionCount * SESSION_PERIOD;
    }
}
