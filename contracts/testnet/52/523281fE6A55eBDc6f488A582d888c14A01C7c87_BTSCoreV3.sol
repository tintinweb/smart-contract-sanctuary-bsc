...
import "./interfaces/IBTSCoreV2.sol";
...
contract BTSCoreV3 is Initializable, IBTSCoreV2, ReentrancyGuardUpgradeable {
    ...
    modifier onlyOwner() {
        require(btsOwnerManager.isOwner(msg.sender) == true, "Unauthorized");
        _;
    }
    ...
    mapping(address => string) internal coinsAddress; //  a list of all supported coins
    ...
    function initialize(
        ...
        coins[_nativeCoinName] = address(NATIVE_COIN_ADDRESS);
    ...
    */
    function register(
        ...
        require(coinsAddress[_addr].compareTo(""), "AddressExists");
        ...
        if (_addr == address(0)) {
            ...
            coinsAddress[deployedERC20] = _name;
    ...
    */
    ...
    function updateCoinDb() external onlyOwner {
        coins[nativeCoinName] = address(NATIVE_COIN_ADDRESS);
        coinsAddress[address(NATIVE_COIN_ADDRESS)] = nativeCoinName;
        coinDetails[nativeCoinName].addr = address(NATIVE_COIN_ADDRESS);
        uint coinsLength = coinsName.length;
        for (uint256 i = 0; i < coinsLength; i++)
            if (!coinsName[i].compareTo(nativeCoinName)) {
                coinsAddress[coinDetails[coinsName[i]].addr] = coinsName[i];
            }
    }

    function setBTSOwnerManager(address _ownerManager) external override {
        require(owners[msg.sender] == true, "Unauthorized");
        require(_ownerManager != address(0), "InvalidAddress");
        btsOwnerManager = IBTSOwnerManager(_ownerManager);
    }
...
interface IBTSCoreV2 {
    ...
    */
    ...
    function setBTSOwnerManager(address _ownerManager) external;
