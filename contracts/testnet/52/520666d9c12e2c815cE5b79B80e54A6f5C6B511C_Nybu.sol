...
contract Nybu is Ownable, Pausable, ReentrancyGuard {
    ...
    uint public requestId = 1; // 0 reserved for automted oracle calls
    ...
    function getUserBets(address user, uint cursor, uint window) external view returns(QueryResult[] memory, uint, uint) {
       ...
       uint length = window;
        ...
        QueryResult[] memory results = new QueryResult[](length);

        for (uint i = 0; i < length; i++) {

            uint timestamp = userRounds[user][i+cursor];
            Bet memory bet = ledger[timestamp][user];
            Round memory round = rounds[timestamp];
            results[i].claimed = bet.claimed;

            // Conditions for an eligible claim
            if(!bet.claimed){

                results[i].timestamp = timestamp;

                uint activeRound = _ceilTimestamp(block.timestamp);
                if(timestamp == activeRound){
                    results[i].active = true;
                }

                if( (round.bearAmount == 0 && bet.position == Position.Bull) || (round.bullAmount == 0 && bet.position == Position.Bear) || round.result == Result.Cancelled || round.result == Result.Draw) {
                    results[i].claimableAmount = bet.amount;
                    continue;
                }

                if(round.result == Result.None){
                    results[i].needsOracle = true;
                    continue;
                }
                ...
                if (round.result == Result.BullWon && bet.position == Position.Bull) {
                    ...
                    results[i].claimableAmount = winnings - feeAmount;
        ...
        return (results, cursor+length, userRounds[user].length);
    }
