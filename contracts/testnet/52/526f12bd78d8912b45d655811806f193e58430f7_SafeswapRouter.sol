...
library SafeswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'657e70528399f8f9c7cc18c249fc386e6eb8ada02880cdb9eb601426a1e13b67' // init code hash
            )))));
    }
...
contract SafeswapRouter is ISafeswapRouter02  {
        ...
        returns (uint[] memory amounts)
    {
        ...
        // IERC20MetadataUpgradeable(WETH).transfer(msg.sender, amounts[amounts.length - 1]);        
        ...
        returns (uint[] memory amounts)
    {
        ...
        IWETHUpgradable(WETH).withdraw(amounts[amounts.length - 1]);        
        TransferHelper.safeTransferETH(to, amounts[amounts.length - 1]);
    }
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        external
        ...
        ensure(deadline)
    {
        ...
        IWETHUpgradable(WETH).withdraw(amountOut);
