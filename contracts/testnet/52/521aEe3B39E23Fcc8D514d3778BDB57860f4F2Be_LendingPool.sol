...
contract LendingPool is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    struct PoolInfo {
        ...
        uint16 liquidateRate; //清算比例 借贷金额+违约金额 超过抵押资产比例
    }
    ...
    function initialize() public initializer {
        ...
        treasury = 0xc519483b3584157E1F9544e32CA9a7a4b07b1E57;
    ...
    function addPool(
        ...
        poolInfo.push(PoolInfo({duration: duration, interestRate: interestRate, penaltyRate: penaltyRate, liquidateRate: liquidateRate}));
    }
    ...
    function updatePool(
        ...
        pool.liquidateRate = liquidateRate;
    }
    ...
    function borrow(uint256 pid, uint256 loanAmount) external {
        ...
        loans[loanId] = LoanData({pid: pid, loanAmount: loanAmount, collateralAmount: maiAmount, dueTime: block.timestamp + poolInfo[pid].duration, status: 0});
        loanIds[msg.sender].add(loanId);
        _loanId++;
        IERC20Upgradeable(MAI).safeTransferFrom(msg.sender, address(this), maiAmount);
    ...
    function repay(uint256 loanId) external {
        require(loanIds[msg.sender].contains(loanId), "invalid load id");
        ...
        IERC20Upgradeable(MAI).safeTransfer(msg.sender, data.collateralAmount);
        //违约金
        if (penaltyAmount > 0) {
    ...
    function liquidate(uint256 loanId) external {
        ...
        require(
            (collateralAmount * maiPrice * (1000 + poolInfo[data.pid].liquidateRate)) / 1000 < 1e18 * (data.loanAmount + penaltyAmount),
