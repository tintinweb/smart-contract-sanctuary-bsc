...
contract Promote is Initializable, Member {
    ...
    function initialize(IERC20 _mp, address _genesis) public initializer {
        __initializeMember();
        mp = _mp;
    ...
    function getGenBonus() public view returns (uint256[3] memory) {
        return genBonus;
    }
    ...
    function redem(address sender, uint256, uint256 amount) external onlyPool {
        ...
        for(uint8 i=0; i < 6; i++) {
            ...
            if(_user == address(0) || _user == genesis){
                break;
            }
            // 更新上線6代的有效人數
            if (changeToInvalid) {
    ...
    function userMint(address _user, uint256 amount) external validSender {
        ...
        for(uint8 gen = 1; gen <= 18; gen++) {
            ...
            IMasterChef._UserInfo memory _masterChefUser = IMasterChef(manager.members("nftMasterChef")).getUser(nGenUser);
            if(_masterChefUser.lvMore1Count == 0) {
                continue;
            }
            // 查詢直推有效人數
            // 直推有效1人可獲得第1代鑄造分潤、直推有效2人可獲得第2代鑄造分潤、依此類推至18代。
            uint256 numSS = userInfo[nGenUser].numSS;
            if(numSS < gen){
                continue;
            }
            // 預計派發金額
