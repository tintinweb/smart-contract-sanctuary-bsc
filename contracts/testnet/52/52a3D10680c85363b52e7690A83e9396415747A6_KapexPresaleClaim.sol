...
contract KapexPresaleClaim {
  ...
  uint256 public currentClaimDate;
  ...
  function calculateBNBToPresaleToken(address _address, uint256 _amount) public view returns (uint256) {
    ...
    string memory allocatedBand = kapexPresale.allocatedBand(_address);
  ...
  function getAvailableTokenToClaim(address _address) public view returns (uint256) {
    uint256 totalKapex = calculateBNBToPresaleToken(_address, getTotalBought(_address)); // 1173076923077400000000000000
    return ((totalKapex * getTotalClaimPercentage()) / feeDenominator) - totalClaim[_address];  // - 117307692307740000000000000
  }

  function getTotalClaimPercentage() private view returns (uint256) {
    uint256 _currentClaimDate = getCurrentClaimDate(); // 1649729484

    (, , uint256 day, uint256 hour, , ) = BokkyPooBahsDateTimeLibrary.timestampToDateTime(_currentClaimDate); // 1, 2
    uint256 interval = BokkyPooBahsDateTimeLibrary.diffMonths(kapexPresale.getStartDateClaim(), _currentClaimDate); // 0
    if (day > claimIntervalDay || (day == claimIntervalDay && hour >= claimIntervalHour) || interval == 0) { // 12 > 28 || 12 == 28 && 10 >= 12 || 1 == 0
      interval += 1;
    }
  ...
  function getTotalBought(address _address) public view returns (uint256) {
    return (kapexPresale.bought(_address) + kapexPresaleBuyRouter.bought(_address));
  }
  ...
  function claim(uint256 requestedAmount) public {
    ...
    uint256 remainingToken = calculateBNBToPresaleToken(msg.sender, getTotalBought(msg.sender)) -
      totalClaim[msg.sender];
