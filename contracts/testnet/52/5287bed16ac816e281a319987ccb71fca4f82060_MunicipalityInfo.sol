...
contract MunicipalityInfo is OwnableUpgradeable, ReentrancyGuardUpgradeable {
   ...
   function setNetGymStreetAddress(address _netGymStreetAddress) external onlyOwner {
    ...
    function getBundles(uint8 _paymentType) external view returns (uint256[6][17] memory bundleStats) {
        for (uint8 i = 0; i < 17; i++) {
            if (i < 13) {
                ...
                bundleStats[i] = [i, IMunicipality(municipalityAddress).newBundles(i).parcelsAmount, IMunicipality(municipalityAddress).newBundles(i).minersAmount, 0, discountPrice[0], price[0]];
            } else {
                ...
                bundleStats[i] = [i, IMunicipality(municipalityAddress).superBundlesInfos(i-13).parcelsAmount, IMunicipality(municipalityAddress).superBundlesInfos(i-13).minersAmount, IMunicipality(municipalityAddress).superBundlesInfos(i-13).upgradesAmount, discountPrice[0], price[0]];
            }    
        }
    }
...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        notZeroAddress
    {
        ...
        if(busdGymnetForMinting[0] > 0) {
            if( busdGymnetForMinting[1] > 0) {
               _transferToContract(busdGymnetForMinting[1]);
            }
        ...
        if (busdGymnetForMinting[0] > 0) {
            emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCEL, parcelIds, _sumBusdGymInBusd(busdGymnetForMinting));
        }
    }
    ...
    function mintMiners(uint256 _count, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress returns(uint256, uint256)
    {
        ...
        if (busdGymnetForMinting[0] > 0) {
            emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINER, minerIds, _sumBusdGymInBusd(busdGymnetForMinting));
        }
    ...
    function purchaseParcels(uint256 _parcelsAmount, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress {
        ...
        if( busdGymnetForSingleProd[1] > 0) {
            _transferToContract(busdGymnetForSingleProd[1]);
        }
        ...
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS, nftIds, _sumBusdGymInBusd(busdGymnetForSingleProd));
    }
    ...
    function purchaseBasicBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        ...
        if(bundlePurchasePrice[1] > 0) {
            _transferToContract(bundlePurchasePrice[1]);
        }        
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_BUNDLE, nftIds, _sumBusdGymInBusd(bundlePurchasePrice));
    ...
    function purchaseSuperBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        ...
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_SUPER_BUNDLE, nftIds, _sumBusdGymInBusd(bundlePurchasePrice));
    ...
    function purchaseParcelsBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress
    {
        ...
        usersMintableNFTAmounts[msg.sender].parcels += bundle.parcelsAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_PARCELS_BUNDLE, nftIds, _sumBusdGymInBusd(bundlePurchasePrice));
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function purchaseMinersBundle(uint8 _bundleType, uint256 _referrerId, uint8 _paymentType) external onlySaleActive notZeroAddress {
        ...
        usersMintableNFTAmounts[msg.sender].miners += bundle.minersAmount;
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_MINERS_BUNDLE, nftIds, _sumBusdGymInBusd(bundlePurchasePrice));
        emit BundlePurchased(msg.sender, _bundleType);
    }
    ...
    function upgradeStandardParcelsGroup(uint256[] memory _parcelIds, uint8 _paymentType) external onlySaleActive {
        ...
        if(totalUpgradePriceBusdGym[0] > 0) {
            if(totalUpgradePriceBusdGym[1] > 0) {
                _transferToContract(totalUpgradePriceBusdGym[1]);
            }
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_UPGRADE_STANDARD_PARCELS_GROUP, nftIds, _sumBusdGymInBusd(totalUpgradePriceBusdGym));
    }
    ...
    function upgradeStandardParcel(uint256 _parcelId, uint8 _paymentType) external onlySaleActive {
        ...
        if(usersMintableNFTAmounts[msg.sender].upgrades > 0) {
            ...
            if(upgradePriceBusdGym[1] > 0) {
                _transferToContract(upgradePriceBusdGym[1]);
            } 
        ...
        uint256[] memory nftIds = new uint256[](0);
        emit PurchaseMade(msg.sender, PURCHASE_TYPE_UPGRADE_STANDARD_PARCEL, nftIds, _sumBusdGymInBusd(upgradePriceBusdGym));
    }
    ...
    function _paymentCheck(uint8 _paymentType) private view returns(uint8) {
            if(!GymnetSaleActivated) {
            _paymentType = PAYMENT_TYPE_BUSD;
            return _paymentType;
        }
    }    
    ...
    function _priceInBusdGym(uint256 _price, uint8 _paymentType) private view returns(uint256[2] memory) {
        ...
        if(_paymentType == PAYMENT_TYPE_BUSD_GYMNET) {
            ...
            totalPriceBusdGym[1] = _price / _getGymnetPriceInBUSD() / 2;
        }
