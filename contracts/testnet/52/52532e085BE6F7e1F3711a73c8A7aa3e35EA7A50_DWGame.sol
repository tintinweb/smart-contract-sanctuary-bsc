...
contract Tractor is ERC721 {
    ...
    */
    ...
    function malfunction(uint256 _tractorId) external {
        require(msg.sender == game);
        tractors[_tractorId].brokenUntil = uint64(
            block.timestamp + 1000 * 52 weeks
        );
    }

    function repair(uint256 _tractorId, uint256 _hours) external {
        require(msg.sender == game);
        tractors[_tractorId].brokenUntil = uint64(
...
contract FarmerWaifu is ERC721 {
  ...
  */
    ...
    function infect(uint256 _farmerId) external {
        require(msg.sender == game);
        farmers[_farmerId].sickUntil = uint64(block.timestamp + 48 hours);
    }

    function heal(uint256 _farmerId, uint256 _hours) external {
        require(msg.sender == game);
    ...
    function extendLifespan(uint256 _farmerId, uint256 _hours) external {
        require(msg.sender == game);
...
contract ProxiedStorage is PeachMathematician {
    ...
    function _getCommission(
        ...
        if (swaps[_from] || _from == game || _to == game) {
...
contract DWGame {
    ...
    function claim(uint256[] memory _claimedAmounts) external {
        ...
        peach.transferFrom(refillPool, address(this), total);
        peach.transferFrom(address(this), msg.sender, total);
    }
}