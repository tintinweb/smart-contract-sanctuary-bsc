...
contract ClaimReward is Context, Ownable, Signer {
    event Claim(address indexed user, address[] tokens, uint256[] amounts, uint256 timestamp);
    ...
    function getMessageHash(uint256 timestamp, address user, address[] memory tokens, uint256[] memory amounts) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(timestamp, user, tokens, amounts));
    }

    function claim(uint256 timestamp, address[] memory tokens, uint256[] memory amounts, uint8 v, bytes32 r, bytes32 s) public {
        require(tokens.length == amounts.length, "ClaimReward: Length mismatch");
        require(permit(getMessageHash(timestamp, msg.sender, tokens, amounts), timestamp, v, r, s), "ClaimReward: Invalid signal");
        ...
        emit Claim(msg.sender, tokens, amounts, timestamp);
    }
