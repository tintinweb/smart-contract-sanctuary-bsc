...
contract Syndication is ERC20, Ownable{
    ...
    struct Terms {
        ...
        bool useDepository;  // if use depository when create offer or order
    ...
    constructor(
        ...
        terms.useDepository = _useDepository;
    ...
    function _buyNftOnMarketplace() internal returns (bool) {
        ...
        if (terms.statusTargetNFT == TargetNFTStatus.Fixed) {
            ...
            if ( totalValue > 0 ) {
                IProtectedMarketplace.NftInfo memory nftInfo = IProtectedMarketplace.NftInfo(terms.nftAddress, terms.tokenId);
    ...
    function _sellNftOnMarketplace() internal returns (bool) {
        ...
        IProtectedMarketplace.NftInfo memory nftInfo = IProtectedMarketplace.NftInfo(terms.nftAddress, terms.tokenId);
        IProtectedMarketplace.ProtectionArgs memory args = IProtectedMarketplace.ProtectionArgs(terms.useDepository, protectionRate, isFixedProtection, protectionTime);
        marketplace.createOrder(nftInfo, tokenPrice, args, true, block.timestamp.add(auctionTime));
