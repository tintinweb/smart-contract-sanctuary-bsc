...
contract VerifyExtractNft is Comn{
    ...
    // 转入NFT
    function intoNft(uint256 tokenId) external isRuning isMember isBlack nonReentrant{
        ERC721(targetContract).transferFrom(msg.sender,address (this),tokenId);
    }

    // 提取验证
    function extractVerify(address sender,string memory tokenIdStr,string memory seed, bytes32 _r, bytes32 _s,uint8 _v) external isRuning isMember isBlack nonReentrant  returns(bytes memory signaturn){
        ...
        extractNft(sender,tokenId,signaturn);
    }

    //提取NFT
    function extractNft(address sender,uint tokenId,bytes memory signaturn) private{
        ERC721(targetContract).transferFrom(address (this),sender,tokenId);
