...
contract CryptoWorldContract is Ownable,  ReentrancyGuard {
    ...
    //Entrada no contrato
    function joinTheGame(address _indicator) public payable
    {
    ...
    function _withdraw(uint256 withdrawValue) private 
    {
        ...
        uint256 valorTotalRecebivel = (valorTotalRecebivelSemTaxas - (valorTotalRecebivelSemTaxas / 100 * 2))  - fee;

        if (withdrawValue == 0) {
            if (valorTotalRecebivel > (totalAcumuladoAReceber - ((totalAcumuladoAReceber / 100) * 6))) {
        ...
        require((valorATransferir + totalSacado) < valorTotalRecebivel, "Valor nao liberado, consulte o regulamento");
    ...
    function setRenewAccount() nonReentrant external payable 
    {
        ...
        uint256 tetoTotal = totalAcumuladoAReceber + valueReservedForGames + ganhoDiario;//jackpot
    ...
    function goVegas() nonReentrant external payable
    {
    ...
    {
        return (diaFinal - diaInicio) / 60 / 60 / 24;
    }
    ...
    function getAccount(address _address) external view returns(uint entryDate, uint lastWithdrawal, uint256 depositedValue, address referrerAccount, uint256 totalAccumulatedReceive,
                        uint256 totalDrawee, uint256 bonusLoyalty, uint256 valueReservedForGames, uint16 numberOfReferrals, bool isParticipating,  uint8[] memory level, uint256[] memory accumulatedValuePerLevel)
