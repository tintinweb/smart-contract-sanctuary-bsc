...
contract GodzillaProject is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Godzilla Project Token", "GODZ") {

        address _owner = 0x33FAb482631484ce61360e239A6dCd1446D2931d;

        _decimals = 9;

        uint256 totalSupply = 1 * 1e11 * (10**_decimals);
        
        buyOperationFee = 3;
        buyLiquidityFee = 2;
        buyBurnFee = 1;
        buyTotalFees = buyOperationFee + buyLiquidityFee + buyBurnFee;
        
        sellOperationFee = 3;
        sellLiquidityFee = 2;
        sellBurnFee = 1;
        sellTotalFees = sellOperationFee + sellLiquidityFee + sellBurnFee;

        transferOperationFee = 1;
        transferLiquidityFee = 0;
        transferBurnFee = 0;
        transferTotalFees = transferOperationFee + transferLiquidityFee + transferBurnFee;

        operationWallet = 0x9D386144F7fd44d91B4f8Df45512745100D96d03; // set as operation wallet
    	burnWallet = deadAddress; // set as burnWallet
    ...
    function enableTrading() external onlyOwner {
        tradingActive = true;
        swapEnabled = true;
        tradingActiveBlock = block.number+2;
    }
    ...
    function updateBuyFees(uint256 _buyOperationFee, uint256 _buyLiquidityFee, uint256 _buyBurnFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _sellOperationFee, uint256 _sellLiquidityFee, uint256 _sellBurnFee) external onlyOwner {
    ...
    function updateTransferFees(uint256 _transferOperationFee, uint256 _transferLiquidityFee, uint256 _transferBurnFee) external onlyOwner {
        transferOperationFee = _transferOperationFee;
        transferLiquidityFee = _transferLiquidityFee;
        transferBurnFee = _transferBurnFee;
        transferTotalFees = transferOperationFee + transferLiquidityFee + transferBurnFee;
        require(transferTotalFees <=3, "Must keep fees at 3% or less");
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on buy
                ...
                tokensForOperation += fees * transferOperationFee / transferTotalFees;
    ...
    function swapBack() private {
        ...
        uint256 totalTaxTokens = tokensForLiquidity + tokensForBurn + tokensForOperation;
        
        if(contractBalance == 0 || totalTaxTokens == 0) {return;}
        
        // Halve the amount of liquidity tokens
        uint256 liquidityTokens = contractBalance * tokensForLiquidity / totalTaxTokens / 2;
        
        
        uint256 initialETHBalance = address(this).balance;

        if (tokensForBurn > 0){
            super._transfer(address(this), burnWallet, (contractBalance * tokensForBurn) / totalTaxTokens);
            tokensForBurn = 0;
        }

        if (tokensForOperation > 0){
            super._transfer(address(this), operationWallet, (contractBalance * tokensForOperation) / totalTaxTokens);
            tokensForOperation = 0;
        }
