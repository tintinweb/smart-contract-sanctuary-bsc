...
contract USDCoin is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    string constant _name = "USDCoin";
    string constant _symbol = "uBC";
    ...
    address public buyBackFeeReceiver =0x59a395CAAA08847a8631ECe8b6e2C0756a2a4199 ;
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        uint256 burnAmount = feeAmount.mul(burnFee).div(getTotalTax(sender,recipient));
    ...
    function getTotalTax(address sender, address recipient) internal view returns (uint256) {
        if (sender == pancakeV2BNBPair) {
            return buyTax;
        } else if (recipient == pancakeV2BNBPair) {
            return inCreaseFactorEnabled ? inCreaseFactor.mul(sellTax) : sellTax;
        } else {
            return transferTax;
        }
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = swapThreshold.mul(swapLiquidityFee).div(totalShares).div(2);
        ...
        try router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            ...
            uint256 totalBNBFee = totalShares.sub(swapLiquidityFee.div(2));
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _buyBackFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        ...
        burnFee = _burnFee;
        totalShares = _liquidityFee.add(_reflectionFee).add(_marketingFee).add(_buyBackFee).add(_burnFee);
        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _buyBackFeeReciever) external authorized {
