...
contract vmmNFTMarketplace is Ownable, ReentrancyGuard {
    ...
    event CanceledListNFT(
        address indexed nft,
        uint256 indexed tokenId
    );


    event CanceledAuctionNFT(
        address indexed nft,
        uint256 indexed tokenId
    );
    event AcceptedNFT(
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        emit CanceledListNFT(_nft, _tokenId);
    }
    ...
    function buyNFT(
        ...
        IERC721(listedNft.nft).transferFrom(
            listedNft.seller,
            msg.sender,
            listedNft.tokenId
        );
        ...
        isAuction(_nft, _tokenId)
    {
        ...
        require(block.timestamp > auction.endTime || block.timestamp < auction.startTime, "auction still working");
        require(auction.lastBidder == address(0), "already have bidder");

        
        delete auctionNfts[_nft][_tokenId];
        emit CanceledAuctionNFT(_nft, _tokenId);
