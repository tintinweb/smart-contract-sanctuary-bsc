...
contract QuintEssentialPool is Ownable {
    ...
    // IERC721 public nft = IERC721(); // Main
    IERC721 public nft = IERC721(0xEa7cc63683A7867ae2620F830a0FD850bA7d448e); // Test

    uint256 public poolDuration = 14 days;
    uint256 public tokenReward = 127;
    uint256 public rewardDivider = 10_000;
    ...
    function deposit(uint256 _amount) public {
        ...
        user.amount = _amount;
        user.reward = reward;
        user.endTime = block.timestamp.add(poolDuration);
        user.nftId = nft.safeMint(msg.sender, uri);
    ...
    function withdraw() public {
        User storage user = users[msg.sender];
        require(block.timestamp >= user.endTime, "Wait for end time");
    ...
    function SetNftUri(string memory _uri) external onlyOwner {
        uri = _uri;
    }
