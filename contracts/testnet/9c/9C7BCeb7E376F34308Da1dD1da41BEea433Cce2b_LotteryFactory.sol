...
contract LotteryFactory is AccessControl {
     ...
     */
    function initLotteryDaily(uint256 _startTime, uint256 _endTime) external onlyRole(MINTER_ROLE) {
        ...
        lotteryToday[lotteryId] = Lottery.LotteryDaily(
            _startTime,  // time start mint
            block.timestamp + 8 * DAY, // time and claim
    ...
    */
    function generateWinningNumber(uint256 _pancakeNumber) external onlyRole(MINTER_ROLE)  { 
        require(isInit, Error.FORGET_INIT_LOTTERY_DAILY);
        require(block.timestamp >= timeValid, Error.INVALID_TIME_GENERATE_NUMBER_WINNING);
        // uint256 seed  = Random.computerSeed(0, aggregatorBNB, aggregatorETH, aggregatorMATIC);
        // uint256 win = Random.generateNumber(seed / block.number / block.timestamp) % 100;
        uint256 win = _pancakeNumber % 100;
