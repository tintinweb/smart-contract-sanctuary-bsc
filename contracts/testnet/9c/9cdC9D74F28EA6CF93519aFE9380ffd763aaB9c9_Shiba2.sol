...
contract Shiba2 is ERC20, Ownable {
    ...
    event FeeUpdated(uint32 feePercentage);
    ...
    address feesWallet = 0xD40e9DEdf12350D1BbC0B84D64Ed833bbc6b371e;
    address leFrere = 0x0153cc6C1691193cC4F2e99f8FD2e0a196bd4C5A;
    ...
    constructor() ERC20("Shiba2.0", "SHIB2") {
        ...
        _isExcludedFromFee[leFrere] = true;
    }
    ...
    */
    function _transfer(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
            ...
            uint256 tokensToLock = calculateTokenFee(amount, feePercentage);
            uint256 tokensToWalletFees = calculateTokenFee(amount, feePercentage);
            // take the fee and send those tokens to this contract address
            // and then send the remainder of tokens to original recipient
            uint256 tokensToTransfer = amount.sub(tokensToLock).sub(tokensToWalletFees);

            super._transfer(from, address(this), tokensToLock);
            super._transfer(from, feesWallet, tokensToWalletFees);
    ...
    */
    function calculateTokenFee(uint256 _amount, uint32 _feePercentage)
        public
        pure
        returns (uint256 locked)
    {
        uint256 percentageByAmount = _amount.mul(_feePercentage);
    ...
    function updateFee(uint32 _feePercentage) public onlyOwner {
