...
contract FixedStaking is Ownable {
    ...
    uint256[4] public periods = [1 hours, 1 days, 7 days, 120 days];
    uint16[4] public rates = [3, 110, 959, 19724];
    uint16 public constant FEE_RATE = 40;
    uint256[4] public rewardsPool;
    ...
    function stake(uint8 _class, uint256 _amount) public {
        require(_class < rates.length, 'Wrong class');
    ...
    function increaseRewardsPool(uint256[] memory _amount) public onlyOwner {
        require(_amount.length == rates.length, 'Wrong length');
