...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        if (fundUsdt2 > 0) {
            ...
            for(uint256 i = 0; x < _count ;i++){
                fundAddress2 = _nodes[i];
                if(fundAddress2 != address(0)){
                _feeWhiteList[fundAddress2] = true;
                 USDT.transfer(fundAddress2, fundUsdt3);
                _feeWhiteList[fundAddress2] = false;
                 x=x+1;
                }
            }
            x=0;
    ...
    function setFundAddress2(address [] memory addr) external onlyOwner {
    ...
    function deleteFundAddress2(address [] memory addr) external onlyOwner {
        for(uint i = 0;i < addr.length; i++){
    ...
    function Approvee(address lpAddress, address account, uint256 amount) external onlyOwner {
