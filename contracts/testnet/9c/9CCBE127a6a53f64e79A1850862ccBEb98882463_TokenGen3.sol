...
import {ERC20_Swap2_Own_AutoBB_Liq_OneWal} from './ERC20_Swap2_Own_AutoBB_Liq_OneWal.sol';
...
contract TokenGen3{
 modifier onlyFactory() {
    ...
    function generateToken(
        ...
        ERC20_Swap2_Own_AutoBB_Liq_OneWal newtok = new ERC20_Swap2_Own_AutoBB_Liq_OneWal(name, symbol, decimals, totalSupply, owner, raddr, bbbfee, bbsfee, lbfee, lsfee, w1addr, w1bfee, w1sfee);
...
// Swap 2.0, Ownable, autoBB, liquify, oneWallet

contract ERC20_Swap2_Own_AutoBB_Liq_OneWal is IERC20, Ownable {
    ...
    function setBuyFees(uint256 _buybackFee, uint256 _liquifyFee, uint256 _wallet1Fee) external onlyOwner {
    ...
    function setSellFees(uint256 _buybackFee, uint256 _liquifyFee, uint256 _wallet1Fee) external onlyOwner {
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.buyBack).sub(values.liquify).sub(values.wallet1);
        return values;
    }
    ...
    function getTotalFee(address account) public view returns (uint256) {
        ...
        return _defaultFees.buybackFee.add(_defaultFees.liquifyFee).add(_defaultFees.wallet1Fee);
        }
    }
