...
contract Retroactive is IClaimDistributor, Pausable, Ownable {
  ...
  function claim(
    ...
    require(IBEP20(token).transfer(_msgSender(), amount), "RA: transfer failed");
  ...
  function withdraw() external whenPaused {
    ...
    require(IBEP20(token).transfer(withdrawAddress, IBEP20(token).balanceOf(address(this))), "RA: withdraw failed");
  }
}
