...
// File: OpenLegendary.sol

pragma solidity ^0.8.0;



contract OpenLegendary is Ownable {
    NFTCollection nftCollection;
    mapping(address => uint256) private adminlist;
    mapping(address => uint256) private blacklist;
    mapping(address => uint256) private whitelistOpen;

    uint256 public whitelistRate;

    mapping(address => uint256) public userState;
    uint256 private maxRarity;
    uint256 private countRarity = 0;
    ...
    constructor(
        ...
        addressReceiver = _addressReceiver;
        number = _number;
        cashPrice = _cashPrice * 10**18;
        nftCollection = NFTCollection(_nftCollection);
        maxRarity = 1;
        for (uint256 i = 0; i < _whitelistOpen.length; i++) {
            whitelistOpen[_whitelistOpen[i]] = 1;
        }
 ...
 */
    ...
    function openLengendary(uint256 nftId_1, uint256 nftId_2) eventGoingOn public returns(uint256){
        address recipient = msg.sender;
        uint256 rarity_1 = nftCollection.getRarityOfTokenId(nftId_1);
        uint256 rarity_2 = nftCollection.getRarityOfTokenId(nftId_2);
        if (rarity_1 >= 2 && rarity_2 >= 2 && (rarity_1 == 3 || rarity_2 == 3)){
            // uint256 open_number = random(10001);
            uint256 open_number = number;
            if (open_number == number && countRarity < maxRarity){
                uint256 _type = random(5) + 1;
                // MATK = MAoE(tokenAddress);
                // uint256 transferAmount = cashPrice;
                // if (whitelistOpen[recipient] == 1){
                //     transferAmount = cashPrice - cashPrice*whitelistRate/10000;
                // }
                // MATK.transferFrom(recipient, addressReceiver, transferAmount);
                nftCollection.transferFrom(recipient, addressReceiver, nftId_1);
                nftCollection.transferFrom(recipient, addressReceiver, nftId_2);
                uint256 res = getNFT(recipient, _type, 4);
                userState[recipient] = 1;
                return res;
            } else {
                userState[recipient] = 0;
                return 0;
            }
        } else {
            userState[recipient] = 0;
 ...
 */
    ...
    function addToWhiteListOpen(address user) public onlyAdmin {
        whitelistOpen[user] = 1;
        emit WhiteListOpenAdded(msg.sender, user);
    }

    function removeFromWhiteListOpen(address user) public onlyAdmin {
        whitelistOpen[user] = 0;
        emit WhiteListOpenRemoved(msg.sender, user);
    }
    ...
    function changeNumber(uint256 _number) public onlyAdmin {
        number = _number;
    }

    function changeCashPrice(uint256 _cashPrice) public onlyAdmin {
        cashPrice = _cashPrice * 10**18;
    }
    function changeWhiteListRate(uint256 _whitelistRate) public onlyAdmin {
        whitelistRate = _whitelistRate;
    }
...
================================================================
 */
    event WhiteListOpenAdded(address sender, address user);
    event WhiteListOpenRemoved(address sender, address user);
    event EventStarted(address user, uint256 status);
    event EventStopped(address user, uint256 status);
    event WithDraw(uint256 amount);
}