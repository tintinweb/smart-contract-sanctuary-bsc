...
contract inc2023cntr is ERC721FCOMMON {
    uint256 public tokenPrice = 100; 
    uint256 public constant MAX_TOKENS = 10;
	uint public constant MAX_RESERVE = 5;
    mapping(address => uint256) private userBalanceSaleMint;
    string private _data_s = "";
    mapping(address => bool) private unlocked;

    modifier onlyUnlocked
    {
        require(unlocked[msg.sender], "To get the secret first unlock Yourself");
        _;
    } 

    constructor() ERC721FCOMMON("inq2023", "INQ") {
        setBaseTokenURI("https://");
        _mint(msg.sender, 0);
    }
	    
    function mint() external payable {
        require(userBalanceSaleMint[msg.sender]<1,"err");
        require(msg.sender == tx.origin, "err");
        require(tokenPrice <= msg.value, "err");
        uint256 supply = totalSupply();
        require(supply < MAX_TOKENS, "err");
        userBalanceSaleMint[msg.sender] = 1;
        _mint(msg.sender, supply);
    }

    function IwantToWinShowMeTheSecretNow() external view returns (string memory)
    {
        return _data_s;
    }

    function setdata(string memory data_s) public onlyOwner {
        _data_s = data_s;
    }

    function setsdata(string memory data_s) public onlyOwner {
        _data_s = data_s;
    }
