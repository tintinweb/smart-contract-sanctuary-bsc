...
contract MyNameIsZkToken is ERC20, Bot {
    ...
    uint256 private constant _tTotal = 100000000 * (10 ** 18); // 100000000 tokens
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "MyZSk";
    string private constant _symbol = "MZSK";
    uint8 private constant _decimals = 18;

    uint256 public _taxFee = 0;       //3%分红本币
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 4; //3%流动性手续费
    uint256 private _previousLiquidityFee = _liquidityFee;
    address payable public liquidityAddress = payable(0xBda5fc5A5f6C24a73dA53c54107213A82c3d3099); // Liquidity Address

    uint256 public _marketingFee = 4; //4%发展营销
    address payable public marketingAddress = payable(0xc4b11Ea790602Cd5FB744825f8D59469B112e8e1); // Marketing Address

    uint256 public _burnFee = 4;      //4%销毁
    uint256 private _previousBurnFee = _burnFee;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD; // dead address

    uint256 public _charityFee = 0;   //慈善
    uint256 private _previousCharityFee = _charityFee;

    // beta router 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
    // prod router 0x10ED43C718714eb63d5aA57B78B54704E256024E
    address constant routerAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; //pancake route address
    ...
    uint256 private _maxNumTokenBalance = 1000000 * 10**(_decimals);
    uint256 private _minimumTokenBalance = 1000000 * 10**(_decimals);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        require (!isBot(from), 'Robot detected');
        ...
        if (contractTokenBalance >= _maxNumTokenBalance) {
            contractTokenBalance = _maxNumTokenBalance;
        }
        bool overMinTokenBalance = contractTokenBalance >= _minimumTokenBalance;
        uint256 totalTokensToSwap = _liquidityFee + _marketingFee;
        if (
            overMinTokenBalance &&
            !inSwapAndLiquidity &&
            totalTokensToSwap > 0 &&
            ...
            uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(totalTokensToSwap);    //营销钱包的币=该合约代币余额*营销手续费/总手续费
            swapAndSendToFee(marketingTokens);                                                           //发送给营销钱包手续费用的币

            uint256 swapTokens = contractTokenBalance.mul(_liquidityFee).div(totalTokensToSwap);         //添加流动性的币=该合约代币余额*流动性手续费/总手续费
            swapAndLiquidity(swapTokens);                                                                //添加流动性

            //add liquidity
            swapAndLiquidity(swapTokens);
        }
    ...
    function swapAndSendToFee(uint256 tokens) private lockTheSwap {
        ...
        if (address(this).balance > 1e18) {
            (bool sent,) = address(marketingAddress).call{value: address(this).balance}("");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (automatedMarketMakerPairs[sender]) { //buy
            burnAmt = amount.mul(_burnFee).div(100);
        }
    ...
    function setConfigFeeInfo(uint256 taxFee, uint256 liquidityFee, uint256 burnFee, uint256 charityFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function setNumTokenPercent(uint256 minimumToken, uint256 maxNumToken) external onlyOwner() {
        _minimumTokenBalance = minimumToken;
        _maxNumTokenBalance = maxNumToken;
    }
}
...
contract Bot is Ownable {
    ...
    address public charityWallet = 0xc3f15bA58f3810Db261901c4Bcf6D7a864440B41; //person address
    ...
    function addBots(address[] calldata _bots) public onlyOwner {
