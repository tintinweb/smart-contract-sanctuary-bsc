...
contract Lockable is Context {
    ...
    modifier validFreeze {
        require(_freezed == false, "BEP20: all token is freezed");
        _;
    }
    ...
    function _lock(address to) internal virtual {
        require(to != address(0), "BEP20: lock to the zero address");
...
contract BEP20Base is Context, BEP20, Ownable, Lockable {
    ...
    string internal constant TOKEN_LOCKED = "BEP20: Tokens is locked";

    constructor(string memory name_, string memory symbol_) BEP20(name_, symbol_){}
    ...
    function _updateTokenLockInfo() internal returns(LockInfo memory) {
        ...
        require(isEntity == true, "BEP20: There is not lockinfo");
    ...
    function tokensLockedAtTime(address to, uint256 amount, uint256 time, uint256 ratio, uint256 duration) public onlyOwner {
        require(to != address(0), "BEP20: lock to the zero address");
        require(amount > 0, "BEP20: amount is over zero");
...
contract TESTTELLUMO is BEP20Base {
    constructor() BEP20Base("TESTTELLUMO", "TESTTLM") {
