...
contract TaycanCoin is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x1AAe3B86803DdBd9B39E4EcC86BC1F68543cAc79;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 70000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "TaycanCoin";
    ...
    uint256 public basePercent = 200; /* porcentagem de queima de moedas a cada transação (100 = 1%, 200 = 2% por aí vai) */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tDevelopment) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        uint256 tokensToBurn = findOnePercent(rTransferAmount);
        uint256 tokensToTransfer = rTransferAmount.sub(tokensToBurn);
        _rOwned[recipient] = _rOwned[recipient].add(tokensToTransfer);
        _takeLiquidity(tLiquidity);
        _takeDevelopment(tDevelopment);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
