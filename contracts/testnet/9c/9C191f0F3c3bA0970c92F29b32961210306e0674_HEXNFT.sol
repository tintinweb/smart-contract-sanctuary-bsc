...
contract HEXNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    event RoyaltiesTransferred(RoyaltyResponse[] royaltyResponse);
    ...
    function transferRoyalties(uint256 royaltiesFees, Royalty[] memory nftRoyalty) internal returns (RoyaltyResponse[] memory) {
        RoyaltyResponse[] memory royaltyResponse;
        ...
        for(uint x = 0; x < nftRoyalty.length; x++) {
            ...
            royaltyResponse[x] = RoyaltyResponse(
        ...
        emit RoyaltiesTransferred(royaltyResponse);
    }
    function transferRoyaltiesToken(uint256 royaltiesFees, Royalty[] memory nftRoyalty, uint256 paymentType, address buyer) internal returns (RoyaltyResponse[] memory) {
        RoyaltyResponse[] memory royaltyResponse;
        uint256 transferredAmount = 0;
        uint256 commissionPercentAmount = 0;
        uint256 comissionTransferred = 0;
