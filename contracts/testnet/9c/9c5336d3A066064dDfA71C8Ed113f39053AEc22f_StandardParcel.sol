...
contract StandardParcel is Parcel, IStandardParcel {
    address internal municipalityContractAddress;
    ...
    mapping(uint256 => uint256) public aestheticNFTIdToParcelId;

    /// @notice Record of assignet miner on parcel
    mapping(uint256 => uint256) public minerIdToParcelId;

    /// @notice Amount of slots on Standard parcel
    uint8 public standardParcelSlotsLimit;

    /// @notice Amount of slots on upgraded parcel
    uint8 public upgradedParcelSlotsLimit;
    ...
    modifier onlyBrokerOrMunicipality() {
        require(
            msg.sender == brokerPBContractAddress ||
                msg.sender == municipalityContractAddress,
            "Parcel: This funcrion can be called only by Broker Public Building or Municipality  Contracts"
        );
        _;
    }
     ...
     */
    function setUpgradedParcelSlots(uint8 _upgradedParcelSlotsLimit)
        external
        onlyBrokerOrMunicipality
    {
        upgradedParcelSlotsLimit = _upgradedParcelSlotsLimit;
    ...
    function upgradeParcel(Parcel memory _parcel)
        external
        onlyBrokerOrMunicipality
    {
        uint256 _tokenId = _getTokenId(_parcel);
        require(
            _exists(_tokenId),
            "StandardParcel: Parcel does not exist or it is a Business Parcel"
        );
        require(
    ...
    function assignNFT(
        ...
        uint256 _tokenId
    ) external onlyBrokerOrMunicipality {
        uint256 parcelId = _getTokenId(_parcel);
        require(_exists(parcelId), "StandardParcel: Parcel does not exist");
        if (!_isLocked[parcelId]) {
            _changeTokenLocking(parcelId, true);
        }
    ...
    function unassignNFT(
        ...
        require(
            _exists(_tokenId),
            "StandardParcel: This parcel does not exist"
        );
        require(_isLocked[parcelId], "StandartParcel: This parcel is free");
        ...
        if (_activatableNFTType == ActivatableNFTs.Aesthetic) {
            ...
            for (
                uint256 i;
                i < parcels[parcelId].aestheticNFTTokenIds.length;
                i++
            ) {
                if (parcels[parcelId].aestheticNFTTokenIds[i] == _tokenId) {
                    parcels[parcelId].aestheticNFTTokenIds[i] = parcels[
                        parcelId
        ...
        if (
            parcels[parcelId].minerNFTTokenIds.length == 0 &&
            parcels[parcelId].aestheticNFTTokenIds.length == 0
        ) {
            _changeTokenLocking(parcelId, false);
        }
    ...
    {
        require(
            _exists(_getTokenId(_parcel)),
            "StandardParcel: Parcel does not exist or it is a Business Parcel"
        );

        return parcels[_getTokenId(_parcel)].upgradeStatus;
    }

    function sendMunicipalityAddress(address _municipality) external onlyOwner {
        require(
            _municipality != address(0),
            "StandartParcel: Cant't send 0x00 Address"
        );
        municipalityContractAddress = _municipality;
    }
}
...
contract Parcel is ERC721Base, OwnableUpgradeable, IParcel {
    address internal brokerPBContractAddress;
    uint256 internal maxCoordinate;
    mapping(uint256 => Parcel) allParcels;

    modifier onlyBroker() {
        require(
            msg.sender == brokerPBContractAddress,
            "Parcel: This funcrion can be called only by Broker Public Building Contract"
        );
        _;
    }

    function __Parcel_init() internal onlyInitializing {
        maxCoordinate = 10000;
    }
    ...
    function _setBrokerPBContractAddress(address _brokerPBContractAddress)
        internal
    {
        require(
    ...
    {
        tokenId = _getTokenId(_parcel);
    }

    function _getTokenId(Parcel memory _parcel)
        internal
        view
        returns (uint256 tokenId)
    {
        require(
            _parcel._x < maxCoordinate,
            "Parcel: x coordinate is out of the bounds"
        );
        require(
            _parcel._y < maxCoordinate,
            "Parcel: y coordinate is out of the bounds"
        );
        tokenId = _parcel._x + _parcel._y * maxCoordinate;
    }
    ...
    {
        return _getCordinatesFromId(_tokenId);
    }

    function _getCordinatesFromId(uint256 _tokenId)
        internal
        view
        returns (uint256 x, uint256 y)
    {
        require(_exists(_tokenId), "Parcel: Parcel isn't exists");
        x = allParcels[_tokenId]._x;
        y = allParcels[_tokenId]._y;
    }
    ...
    function mintParcelFor(address _beneficiary, Parcel memory _parcel)
        ...
        onlyBroker
    {
        uint256 tokenId = _getTokenId(_parcel);
        require(!_exists(tokenId), "Parcel: Parcel already exists");
        _mintFor(tokenId, _beneficiary);
        allParcels[tokenId] = _parcel;
        emit ParcelMinted(tokenId, _beneficiary);
    }
    ...
    function mintAdjacentQuadraticParcels(
        ...
        uint256 firstId = _getTokenId(_parcel);
        for (uint256 i = 1; i <= _size**2; i++) {
            if (i % _size == 0) {
                firstId += maxCoordinate;
            }
            _mintFor(firstId, _beneficiary);
            allParcels[firstId] = _parcel;
            firstId++;
            emit ParcelMinted(firstId, _beneficiary);
        }
    }
    ...
    function transferParcelFrom(
        ...
        _transferToken(_from, _to, _getTokenId(_parcel), "", false);
        emit TransferParcelFrom(_from, _to, _getTokenId(_parcel));
    }
    ...
    {
        uint256 tokenId;
        for (uint256 index; index < _parcels.length; index++) {
            tokenId = _getTokenId(_parcels[index]);
            _transferToken(_tokenOwner[tokenId], _to, tokenId, "", false);
            emit TransferParcelFrom(_tokenOwner[tokenId], _to, tokenId);
        }
    }
    ...
    function transferAdjacentQuadraticParcels(
        ...
        for (uint256 i = 1; i <= _size**2; i++) {
            ...
            _transferToken(_tokenOwner[firstId], _to, firstId, "", false);
            firstId++;
            emit TransferParcelFrom(_tokenOwner[firstId], _to, firstId);
        }
    }

    function parcelExists(Parcel memory _parcel) external view returns (bool) {
        return _exists(_getTokenId(_parcel));
    }
    ...
    function ownerOfParcel(Parcel memory _parcel)
        ...
        returns (address)
    {
        return _tokenOwner[_getTokenId(_parcel)];
    }
    ...
    {
        for (uint256 index; index < _parcel.length; index++) {
            ownerAddresses[index] = _tokenOwner[_getTokenId(_parcel[index])];
        }
    }
        ...
        returns (Parcel[] memory cordinates)
    {
        ...
        for (uint256 index; index < userTokens.length; index++) {
            (uint256 x, uint256 y) = _getCordinatesFromId(userTokens[index]);
            cordinates[index] = (Parcel(x, y));
        }
        return cordinates;
    }
}
...
interface IStandardParcel is IParcel {
    ...
    function sendMunicipalityAddress(address _municipality) external;
}
...
contract ERC721Base is IERC721Base {
    ...
    function getUserPurchaseTime(address _user)
        external
        view
        returns (uint256[2] memory)
    {
        uint256 _time = uint256(_userPurchaseDate[_user]);
        return [_time, _time + 4 weeks];
    }
