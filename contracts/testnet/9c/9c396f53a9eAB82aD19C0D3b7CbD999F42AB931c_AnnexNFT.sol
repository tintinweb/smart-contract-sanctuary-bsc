...
contract AnnexNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function createNFTs(createNftData memory _nftData) public {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _nftData.encodeKey, _nftData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_nftData.signature);
    ...
    function changeSellingStatus(sellingConfigData memory _sellingData) public {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _sellingData.encodeKey, _sellingData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_sellingData.signature);
        require(signer == msg.sender, "invalid signature");
        ...
        if(_sellingData.nft == address(this)) {
            ...
            tokenInfoById.price = _sellingData._newPrice;
    ...
    function placeBid(placeBidData memory _bidData) external {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _bidData.encodeKey, _bidData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_bidData.signature);
        require(signer == msg.sender, "invalid signature");
    ...
    function placeOffer(placeOfferData memory _offerData) external {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _offerData.encodeKey, _offerData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_offerData.signature);
        require(signer == msg.sender, "invalid signature");
    ...
    function cancelOfferBid( cancelBidOfferData memory _cancelData) external payable {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _cancelData.encodeKey, _cancelData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_cancelData.signature);
        require(signer == msg.sender, "invalid signature");
    ...
    function transferByAcceptOffer(transferByOfferData memory _transferData) external payable onlyNftOwner(_transferData.nftId) {
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _transferData.encodeKey, _transferData.nonce));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(_transferData.signature);
        require(signer == msg.sender, "invalid signature");
        ...
        if(PLATFORM_SHARE_PERCENT > 0) {
            ...
            if(_transferData.payThrough==1) {
                transferAnnToOwner(_transferData.newOwner, address(this), platformSharePercent);
            }
            else {
        ...
        if(_transferData.percent > 0) {
            uint256 royaltyPercent = calculatePercentValue(amountToTransfer, _transferData.percent);
            amountToTransfer = amountToTransfer-royaltyPercent;
            if(_transferData.payThrough==1) {
                transferAnnToOwner(_transferData.newOwner, address(this), royaltyPercent);
                uint256 amount = royaltiesANN[_transferData.nft];
                royaltiesANN[_transferData.nft] = amount + royaltyPercent;
            }
            else {
                transferwbnbToOwner(_transferData.newOwner, address(this), royaltyPercent);
                uint256 amount = royaltiesWbnb[_transferData.nft];
                royaltiesANN[_transferData.nft] = amount + royaltyPercent;
            }
        }
        if(_transferData.payThrough==1) {
            transferAnnToOwner(_transferData.newOwner, msg.sender, amountToTransfer);
        }
        else {
        ...
        emit OfferAccepted(_transferData.tokenId,bidsOffers[_transferData.nftId][_transferData.newOwner],msg.sender,_transferData.newOwner);
    }
    function transferByBid(transferByBidData memory _transferData) external payable onlyNftOwner(_transferData.nftId) {
        ...
        emit BidAccepted(_transferData.tokenId,bidsOffers[_transferData.nftId][_transferData.newOwner],msg.sender,_transferData.newOwner);
    }
