...
contract Ule_Buy_Router is Ownable {
    ...
    function swapExactETHForToken(uint256 value, address token) private returns (uint[] memory amounts )  
    {
    ...
    public
    onlyOwner
    {      payable(msg.sender).transfer(withdrawableBNB());     }

    function withdrawToken()
    public
    onlyOwner
    {     ULE.transfer(msg.sender, withdrawableToken());         }
    ...
    public
    view returns(uint256 amount_)
    {
        amount_ = ULE.balanceOf(address(this));
        return amount_ = amount_.sub((amount_.mul(30)).div(100));
    }
