...
contract MangoManIntelligentStaking is IERC20Staking {
    ...
    constructor(address _stakingToken) IERC20Staking(_stakingToken) {
        ...
        plans[3].apr = 15;
        plans[3].stakeDuration = 100 days;
        plans[3].earlyPenalty = 0;

        plans[4].apr = 18;
        plans[4].stakeDuration = 200 days;
        plans[4].earlyPenalty = 0;

        plans[5].apr = 21;
        plans[5].stakeDuration = 300 days;
    ...
    function unstake(uint256 _stakingId, uint256 _amount) public override {
        ...
        for (uint256 i = stakes[_stakingId][msg.sender].length; i > 0; i--) {
            ...
            //checkFixstake
            require(
                _stakingId >= 3  ?  block.timestamp >= _staking.endstakeAt : true,
