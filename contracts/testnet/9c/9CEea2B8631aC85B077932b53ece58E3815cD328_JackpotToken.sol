...
contract JackpotToken is Context, IERC20, Ownable {
    ...
    bool public gameLaunched = false;
    ...
    constructor(address cOwner) Ownable(cOwner) {
        ...
        _isExcludedFromFee.add(lockedLiquidity());

        _isExcludedFromSwapAndLiquify.add(uniswapV2Pair);
    }

    function launchJackpotGame(uint256 amountLiq, uint256 amountEvent, uint256 otherAmount) external onlyOwner {
        require(!presaleOpen, "Presale open");
        require(!gameLaunched, "Game already launch");
        /// launch at 0.175$
        uint256 liquidityTokenAmount = amountLiq;
        /// TODO : OTHER WALLET 
        _mint(lockedLiquidity(), liquidityTokenAmount);
        _mint(marketingWallet(), amountEvent);
        _mint(owner(), otherAmount);
    ...
    function getAmountBuyUser(address user) public view returns(uint256){
        return userAmountBuyPresale[user];
    }
