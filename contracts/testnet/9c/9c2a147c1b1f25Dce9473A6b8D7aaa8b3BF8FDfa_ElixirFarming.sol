 ...
 */
contract ElixirFarming is Ownable, Pausable, ReentrancyGuard {
     ...
     *  after this the contract is active, it set isPaused to false
     */
     ...
     */
    function _stake(address _from, uint256 _amount) private returns (bool) {
        _updateShare();
        ...
        if (!hasStaked[_from]) {
            ...
            if (deposits[_from].currentPeriod != period) {
                ...
                bool claim = _claimRewards(_from);
     ...
     */
    function viewOldRewards(address _from) public view returns (uint256) {
        if (
            isPaused == true ||
     ...
     */
    function calculate(address _from) public view returns (uint256) {
    ...
    function _withdraw(address _from, uint256 _amount) private returns (bool) {
        ...
        if (!isPaused && deposits[_from].currentPeriod == period) {
            stakedBalance -= _amount;
        }
        ...
        returns (bool)
    {
        ...
        endingBlock += _rewardsToBeAdded / rewPerBlock();
