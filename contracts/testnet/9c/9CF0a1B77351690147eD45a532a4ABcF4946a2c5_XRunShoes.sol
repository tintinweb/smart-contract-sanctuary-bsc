    ...
    AccessControlEnumerable
{
    ...
    /**
    @notice Role of administrative users allowed to expel a XRunShoes from the
    nest.
    ...
    mapping(address => bool) whitelistteam;

    uint256 public BoxAllowedPerAddress = 1;

    mapping(address => uint256) private BoxMintedByAddress;
    ...
    uint256 public whitelistPool = 9999;
     ...
     */
    modifier reducewhitelistPool(uint256 n) {
        require(n <= whitelistPool, "XRunShoes: PROOF pool exhausted");
        whitelistPool -= n;
        _;
    }
     ...
     */
    function setwhitelistMintOpen() external onlyRole(DEV) {
        whitelistMintOpen = true;
    }

    function foudingTeamMint(uint256 amount)
        public
        onlyRole(DEV)
    {
        _mintShoes(msg.sender, amount, true);
    }
    ...
    function whitelistMint() 
        public  
        reducewhitelistPool(whitelistPool)
    {
        require(whitelist[_msgSender()], "Address not whitelisted");
        require(
            BoxMintedByAddress[_msgSender()] < BoxAllowedPerAddress,
            "One pack per address"
        );
        require(whitelistMintOpen);
        _mintShoes(msg.sender, 10000-whitelistPool, true);
    }
    ...
    /**
    @notice Returns how many additional XRunShoes can be claimed with the PROOF
    token.
     */
     ...
     */
    function whitelistUnMint(address to, uint256 n)
        external
        onlyRole(DEV)
        reducewhitelistPool(n)
    {
        _mintShoes(to, n, true);
    }

    /**
    @dev tokenId to openBox start time (0 = not openBox).
     */
    mapping(uint256 => uint256) private openBoxStarted;

    /**
    @dev Cumulative per-token openBox, excluding the current period.
     */
    mapping(uint256 => uint256) private openBoxTotal;

    function openBoxPeriod(uint256 tokenId)
        external
        view
        returns (
            bool openBox,
    ...
    {
        uint256 start = openBoxStarted[tokenId];
        if (start != 0) {
            openBox = true;
            current = block.timestamp - start;
        }
        total = current + openBoxTotal[tokenId];
    }

    /**
    @dev MUST only be modified by safeTransferWhileopenBox(); if set to 2 then
    the _beforeTokenTransfer() block while openBox is disabled.
     */
    uint256 private openBoxTransfer = 1;

    /**
    @notice Transfer a token between addresses while the XRunShoes is minting,
    thus not resetting the openBox period.
     */
    function safeTransferWhileopenBox(
        address from,
        address to,
        uint256 tokenId
    ) external {
        require(ownerOf(tokenId) == _msgSender(), "XRunShoes: Only owner");
        openBoxTransfer = 2;
    ...
    /**
    @dev Block transfers while openBox.
     */
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                openBoxStarted[tokenId] == 0 || openBoxTransfer == 2,
                "XRunShoes: openBox"
            );
        }
    }
    ...
    /**
    @dev Emitted when a XRunShoes begins openBox.
     */
    event OpenBox(uint256 indexed tokenId);

    /**
    @dev Emitted when a XRunShoes stops openBox; either through standard means or
    by expulsion.
     */
    event UnOpenBox(uint256 indexed tokenId);

    /**
    @dev Emitted when a XRunShoes is expelled from the box.
     */
    event Expelled(uint256 indexed tokenId);

    /**
    @notice Whether openBox is currently allowed.
    @dev If false then openBox is blocked, but unopenBox is always allowed.
     */
    bool public openBoxOpen = false;

    /**
    @notice Toggles the `openBoxOpen` flag.
     */
    function setopenBoxOpen() external onlyRole(DEV) {
        openBoxOpen = true;
    }

    /**
    @notice Changes the XRunShoes's openBox status.
    */
    function toggleopenBox(uint256 tokenId)
        internal
        onlyApprovedOrOwner(tokenId)
    {
        uint256 start = openBoxStarted[tokenId];
        if (start == 0) {
            require(openBoxOpen, "XRunShoes: openBox closed");
            openBoxStarted[tokenId] = block.timestamp;
            emit OpenBox(tokenId);
        } else {
            openBoxTotal[tokenId] += block.timestamp - start;
            openBoxStarted[tokenId] = 0;
            emit UnOpenBox(tokenId);
        }
    }

    /**
    @notice Changes the XRunShoes' openBox statuss (what's the plural of status?
    statii? statuses? status? The plural of sheep is sheep; maybe it's also the
    plural of status).
    @dev Changes the XRunShoes' openBox sheep (see @notice).
     */
    function toggleopenBox(uint256[] calldata tokenIds) external {
        uint256 n = tokenIds.length;
        for (uint256 i = 0; i < n; ++i) {
            toggleopenBox(tokenIds[i]);
        }
    }

    /**
    @notice Admin-only ability to expel a XRunShoes from the nest.
    @dev As most sales listings use off-chain signatures it's impossible to
    detect someone who has OpenBox and then deliberately undercuts the floor
    ...
    because openBox would then be all-or-nothing for all of a particular owner's
    XRunShoes.
     */
    function expelFromBox(uint256 tokenId) external onlyRole(DEV) {
        require(openBoxStarted[tokenId] != 0, "XRunShoes: not OpenBox");
        openBoxTotal[tokenId] += block.timestamp - openBoxStarted[tokenId];
        openBoxStarted[tokenId] = 0;
        emit UnOpenBox(tokenId);
