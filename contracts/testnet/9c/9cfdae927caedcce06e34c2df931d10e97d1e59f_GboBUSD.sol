...
contract GboBUSD {
	...
	/*	if (!started) {
		...
		if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < 10; i++) {
                // check direct child
                if(users[upline1].users.length >=6){
                    token.transfer(upline1,2e18);
                    users[upline1].magicBonus.add(2e18);
                    break;
                }
	...
	function getUserReferrerDividends(address userAddress) public view returns (uint256) {
		User storage user = users[userAddress];
	...
	function getSiteInfo() public view returns(uint256 _totalInvested, uint256 _totalBonus, uint256 _n) {
		return(totalInvested, totalRefBonus,n);
	}
