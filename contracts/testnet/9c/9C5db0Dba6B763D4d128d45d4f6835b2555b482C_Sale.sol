...
import "./lock/interfaces/IKprnLock.sol";

contract Sale is State, Ownable, ReentrancyGuard {
    ...
    IERC20 private _acceptedCurrency;
    IERC20 private _saleToken;
    IKprnLock private _lockAddress;
    uint256 private _lockupTime;
    uint256 private _lockupTimestamp;
    uint256 private _lockVestingPercent;

    mapping (address => uint256) private _lockIds;
    ...
    constructor(
        ...
        require(date[1] > block.timestamp, "END_DATE: NOT_FUTURE_DATE");
        require(date[1] > _startDate, "END_DATE: LESS_THAN_START");
       
        _name = name;

        _acceptedCurrency = IERC20(addresses[0]);
        _saleToken = IERC20(addresses[1]);
        _decimals = IERC20Metadata(addresses[1]).decimals();
        _lockAddress = IKprnLock(addresses[2]);

        _lockupTime = lock[0];
        _lockVestingPercent = lock[1];
    ...
    function saleDetails() external view returns (bytes32, address[3] memory, uint256[2] memory, uint256[9] memory, bool, uint) {
        return (
            _name,
            [
    ...
    /**
     * @dev update _endDate to the sale contract
     */
    function extendSale(uint256 endDate) external onlyOwner {
        require(endDate > block.timestamp, "END_DATE: NOT_FUTURE_DATE");
        require(endDate > _startDate, "END_DATE: LESS_THAN_START");
    ...
    function finalize() external nonReentrant onlyOwner {
        ...
        // set _lockupTimestamp value
        _lockupTimestamp = _endDate.add(uint256(86400).mul(_lockupTime));

        // transfer the remaining balance to owner
        payable(owner()).transfer(address(this).balance);
        _acceptedCurrency.safeTransfer(owner(), _acceptedCurrency.balanceOf(address(this)));
        _saleToken.safeTransfer(owner(), balance().sub(_tokenSold));
        assert(_tokenSold >= balance());
    } 
    ...
    */
     ...
     */
    function purchase(uint256 amount) external nonReentrant whenActive  {
        require(amount > 0, "Sale: PURCHASE_AMOUNT_INVALID");
        if(_whitelist) {
        ...
        if(_hardCap.sub(_busdRaised) < _minBuy) {
            minbuy = _hardCap.sub(_busdRaised);
        }
        
        require(
            (block.timestamp >= _startDate && block.timestamp <= _endDate) && 
            _hardCap >= _busdRaised.add(amount) && 
            amount >= minbuy && _contributions[_msgSender()].add(amount) <= _maxBuy, 
            "Sale: INVALID"
        );
        ...
        _contributions[_msgSender()] = _contributions[_msgSender()].add(amount);
        _claimable[_msgSender()] = _claimable[_msgSender()].add(tokens);

        _acceptedCurrency.transferFrom(_msgSender(), address(this), amount);
        _busdRaised = _busdRaised.add(amount);
    ...
    function claim() external nonReentrant whenFinalized {
        require(_claimable[_msgSender()] > 0, "Sale: ALREADY_CLAIMED");
       ...
       _claimable[_msgSender()] = 0;
        ...
        if (_lockVestingPercent > 0) {
            uint256 lockAmount = total.mul(_lockVestingPercent).div(100);
            claimableAmount = total.sub(lockAmount);
            _addLock(lockAmount);
        }else {
            claimableAmount = total;
        }

        _saleToken.safeTransfer(_msgSender(), claimableAmount);
    ...
    */
    ...
    /** 
    * @dev Internal function to add Lock tokens to IKprnLock contract
    * contract must be finalized
    */
    function _addLock(uint256 amount) internal {
        uint lockId = _lockAddress.lock(
            _msgSender(),
            address(_saleToken),
            true,
            amount,
