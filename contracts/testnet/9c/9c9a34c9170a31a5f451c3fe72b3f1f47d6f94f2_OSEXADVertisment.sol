...
contract OSEXADVertisment is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    OSEXADVDividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = payable(0x01104d17ABCD3f915c3ceBfd44852944f6f340EF);
    ...
    constructor() public ERC20("Orbital Station Excelsior Advertisment Token", "OSEXAdv") {

    	dividendTracker = new OSEXADVDividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "OSEXADV: The dividend tracker already has that address");

        OSEXADVDividendTracker newDividendTracker = OSEXADVDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "OSEXADV: The new dividend tracker must be owned by the OSEXADV token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "OSEXADV: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "OSEXADV: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "OSEXADV: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "OSEXADV: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "OSEXADV: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "OSEXADV: Cannot update gasForProcessing to same value");
...
contract OSEXADVDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() public DividendPayingToken("OSEXADV_Dividen_Tracker", "OSEXADV_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1M+ tokens
    }

    function _transfer(address, address, uint256) internal override {
        require(false, "OSEXADV_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(false, "OSEXADV_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main OSEXADV contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 60 && newClaimWait <= 86400, "OSEXADV_Dividend_Tracker: claimWait must be updated to between 1 Min and 24 hours");
        require(newClaimWait != claimWait, "OSEXADV_Dividend_Tracker: Cannot update claimWait to same value");
