...
contract BNBHourTest {
	...
	uint constant public			TIME_STEP						= 24 hours;
	uint constant public			INVEST_MIN_AMOUNT				= 0.01 ether;			    // 0.01 BNB
	uint[] public					REFERRAL_PERCENTS				= [50, 40, 30, 20, 10];		// 5% 4% 3% 2% 1%
	uint constant public			PROJECT_FEE						= 30;					    // project fee 8% of deposit
	uint constant public			DEV_FEE							= 20;					    // dev fee 7% of deposit
	...
	uint constant public			INSURANCE_PERCENT				= 50;					    // insurance fee 10% of claim
	uint constant public			INSURANCE_LOWBALANCE_PERCENT	= 200;				    	// protection kicks in at 25% or lower
	...
	constructor(address payable _walletProject, address payable _walletDev) {
		...
		PLANS.push( TPlan(15,111) );
		PLANS.push( TPlan(16,112) );
		PLANS.push( TPlan(17,113) );
		PLANS.push( TPlan(18,114) );
		PLANS.push( TPlan(19,115) );
		PLANS.push( TPlan(20,116) );
		PLANS.push( TPlan(21,117) );
		PLANS.push( TPlan(22,118) );
		PLANS.push( TPlan(23,119) );
		PLANS.push( TPlan(24,120) );
		PLANS.push( TPlan(25,121) );
		PLANS.push( TPlan(26,122) );
		PLANS.push( TPlan(27,123) );
		PLANS.push( TPlan(28,124) );
		PLANS.push( TPlan(29,125) );
		PLANS.push( TPlan(30,126) );
	...
	function _createDeposit( address _user, uint _planIdx, uint _amount, bool _isReinvest ) internal returns(uint o_depIdx) {
		...
		TDeposit memory newDep = TDeposit( _planIdx, _amount, block.timestamp, block.timestamp + PLANS[_planIdx].durationHours * TIME_STEP, _isReinvest );
		...
		if(!_isReinvest) {
			...
			DEPOSIT_HISTORY[TOTAL_DEPOSITS] = THistoryDeposit( block.timestamp, PLANS[_planIdx].durationHours*TIME_STEP, _amount );
			TOTAL_DEPOSITS++;
