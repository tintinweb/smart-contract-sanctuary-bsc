...
contract FlashMall is AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public constant AMOUNT_THRESHOLD = 10e18;

    event LogTrade(address from, address merchant, uint256 amount);
    event LogContributeChange(address addr, uint256 amount);

    function processTrades(uint256 size) external {
        uint256 maxIndex = processIndex + size < trades.length ? processIndex + size : trades.length;
        for (uint256 i = processIndex; i < maxIndex; i++) {
            Trade memory trade = trades[processIndex + i];
            tradeContributeReward(trade.sender, trade.merchant, trade.contribute);
        }
    ...
    function initialize() external initializer {
        ...
        point = 0xb34Cb7A0396EefB70F414b21E7FE78a50C9D3f8E;
        pointFeeReceiver = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa;
        mai = 0xD87f4c66bb1F07520156f09449ECAa5Ebd5d090b;
        exchangePool = 0x6515579769bB410772bb03a87663C6284765925f;
        mdao = 0x221A126E0F2B4A6c5848689145589f9A229dc3aa;
        metabill = 0x369aE524F67FdC5CA7965e7A4c9b65Eb6261170E;

        members[_msgSender()] = true;
        pointRate = 7250;
        memberPrice = 1000e18;
        tradeContributeSelfRate = 50000;
        tradeContributeMdaoRate = 5000;
    ...
    function pointMint(uint256 amount) external {
        ...
        uint256 pointAmount = (amount * pointRate) / UNIT;
    ...
    function pointBack(uint256 amount) external {
        ...
        uint256 musdAmount = (amount * UNIT) / pointRate;
    ...
    function trade(address merchant, uint256 amount) external {
        ...
        processContributeAdd(mdao, (merchantContribute * tradeContributeMdaoRate) / UNIT);
        if (amount > AMOUNT_THRESHOLD) {
            tradeContributeReward(_msgSender(), merchant, merchantContribute);
        } else {
            trades.push(Trade({sender: msg.sender, merchant: merchant, contribute: merchantContribute}));
        }
    ...
    function tradeContributeReward(
        ...
        if (currentAgentLevel < 6) {
            ...
            processSpeedUp(mdao, (merchantContribute * diff) / UNIT);
        }
    ...
    function testReadHead(address addr) external view returns (address) {
        (, , address parent) = IExt(0x369aE524F67FdC5CA7965e7A4c9b65Eb6261170E).accounts(addr);
        return parent;
    }
