...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    modifier saleValid(uint256 saleId) {
        require(saleId > 0 && saleId <= lastedId, "Invalid sale id");
    ...
    function setProjectAddr(address _project) external {
    ...
    function creates(uint256 _projectId, SaleInput[] memory _saleInputs) external onlyManager(_projectId) {
        require(_saleInputs.length > 0, "Invalid param");
        ProjectInfo memory _project = project.getProject(_projectId);
        _project.isSingle ? IOSB721(_project.token).setApprovalForAll(address(project), true) : IOSB1155(_project.token).setApprovalForAll(address(project), true);
        ...
        for (uint256 i; i < _saleInputs.length; i++) {
            _sales[i] = _createSale(++id, _project, _saleInputs[i]);
        }
    ...
    function _createSale(uint256 saleId, ProjectInfo memory _project, SaleInput memory _saleInput) private returns(SaleInfo memory) {
        ...
        uint256 amountInput = _project.isSingle ? 1 : _saleInput.amount;
        uint256 currentAmount = project.getProject(_project.id).amount + amountInput;
        project.setTokenAmount(_project.id, currentAmount);
    ...
    function bid(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant saleValid(_saleId) {
        ...
        _price = (_project.isRaise ? sale.raisePrice : currentDutchPrice(_saleId)) * _amount;
    ...
    function close(ProjectInfo memory _project, SaleInfo memory _sale, address[] memory _accountBuyers, bool _isGive) external nonReentrant {
