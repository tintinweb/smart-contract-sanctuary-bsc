...
contract Wormhole is IWormhole, ERC1155Holder, Ownable, Pausable, ReentrancyGuard {
  ...
  IERC1155 public metaspacecyMysteriousBox;
  ...
  modifier onlyAdmin() {
    require(metaspacecyAccessControls.hasAdminRole(_msgSender()), "Wormhole: only admin");
    _;
  }
  ...
  constructor (
    ...
    address _metaspacecyMysteriousBox,
    ...
    metaspacecyMysteriousBox = IERC1155(_metaspacecyMysteriousBox);
  ...
  function balanceOfBox(uint256 boxId) public view override returns (uint256) {
    return metaspacecyMysteriousBox.balanceOf(address(this), boxId);
  }

  function balanceOfItem(uint256 itemId) public view override returns (uint256) {
    return metaspacecyCreatureAccessory.balanceOf(address(this), itemId);
  }

  function balanceOfTicketCard(uint256 cardId) public view override returns (uint256) {
    return metaspacecyTicketCard.balanceOf(address(this), cardId);
  }
  ...
  function buyTicketCard(uint256 amount) public override whenNotPaused nonReentrant {
    ...
    metaspacecyToken.burnFrom(_msgSender(), amount * TICKET_CARD_PRICE * BASE);
    metaspacecyTicketCard.safeTransferFrom(address(this), _msgSender(), metaspacecyTicketCard.currentIdSupported(), amount, "");
    emit BuyTicketCard(_msgSender(), metaspacecyTicketCard.currentIdSupported(), amount);
  }

  function canBuyTicketCard(address user, uint256 amount) public view override returns (bool) {
    if (metaspacecyTicketCard.isIdExpired(metaspacecyTicketCard.currentIdSupported())) {
      return false;
    }

    return (
      metaspacecyToken.balanceOf(user) >= amount * TICKET_CARD_PRICE * BASE &&
      balanceOfTicketCard(metaspacecyTicketCard.currentIdSupported()) >= amount
    );
  }
  ...
  function buyBox(uint256 optionId, uint256 amount) public override whenNotPaused nonReentrant {
    ...
    metaspacecyToken.burnFrom(_msgSender(), amount * BOX_PRICE[optionId] * BASE);
    metaspacecyMysteriousBox.safeTransferFrom(address(this), _msgSender(), optionId, amount, "");
  ...
  function canBuyBox(address user, uint256 optionId, uint256 amount) public view override returns (bool) {
    return (
      metaspacecyToken.balanceOf(user) >= amount * BOX_PRICE[optionId] * BASE &&
  ...
  function unBox(
    ...
    metaspacecyCreatureAccessory.safeTransferFrom(address(this), _toAddress, _optionId, _amount, _data);
  }
  ...
  function upgrade(uint256 toOptionId, uint256 amount) public override whenNotPaused nonReentrant {
    ...
    metaspacecyToken.burnFrom(_msgSender(), UPGRADE_FEE[toOptionId - 1] * amount * BASE);
    metaspacecyCreatureAccessory.safeTransferFrom(_msgSender(), address(this), toOptionId - 1, UPGRADE_AMOUNT[toOptionId - 1] * amount, "");
    metaspacecyCreatureAccessory.safeTransferFrom(address(this), _msgSender(), toOptionId, amount, "");
  ...
  function canUpgrade(address user, uint256 toOptionId, uint256 amount) public view override returns (bool) {
    return (
      ...
      metaspacecyCreatureAccessory.balanceOf(user, toOptionId - 1) >= UPGRADE_AMOUNT[toOptionId - 1] * amount &&
      balanceOfItem(toOptionId) >= amount &&
      metaspacecyToken.balanceOf(user) >= UPGRADE_FEE[toOptionId - 1] * amount * BASE
    );
  }
}
