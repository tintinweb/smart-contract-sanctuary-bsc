...
// Dependency file: contracts/src/interfaces/IUniswapV2Factory.sol
...
// Dependency file: contracts/src/interfaces/IUniswapV2Pair.sol
...
// Dependency file: contracts/src/interfaces/IUniswapV2Router01.sol
...
// Dependency file: contracts/src/interfaces/IUniswapV2Router02.sol

// pragma solidity >=0.6.2;

// import "contracts/src/interfaces/IUniswapV2Router01.sol";
...
// Root file: contracts/src/token/CDDSP.sol
...
// import "contracts/src/interfaces/IUniswapV2Factory.sol";
// import "contracts/src/interfaces/IUniswapV2Pair.sol";
// import "contracts/src/interfaces/IUniswapV2Router02.sol";
...
contract VCT is Context, IERC20, Ownable {
    ...
    //rewards period
    uint256 public rewardsClaimTimeSeconds = 60 * 60 * 24;
    ...
    string private constant _name = "VCT";
    ...
    //total supply
    uint256 private constant _tTotal = 500_000_000 ether; //500 millions
    ...
    function initContract(
        ...
        uint256 _feeRate,
        uint256 _rewardsClaimTimeSeconds
    ) external onlyOwner {
        ...
        feeRate = _feeRate;
        rewardsClaimTimeSeconds = _rewardsClaimTimeSeconds;
    }
    ...
    function openTrading() external onlyOwner {
        ...
        _takeFeeOnTransferEnabled = true;
    ...
    function getLastBnbRewardsClaim(address wallet)
        external
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        if (
            ...
            !_isPancakeSwapPair[from] &&
            from != address(pancakeSwapRouter)
        ) {

            //if sell check max amount
            if (
                to == pancakeSwapPair ||
                _isPancakeSwapPair[to] ||
                to == address(pancakeSwapRouter)
            ) {
    ...
    function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
        ...
        uint256 liquidityFeeBalance = _contractTokenBalance.mul(liquidityFee).div(
            _liquidityFeeTotal
        );
        ...
        uint256 rewardReceived = bnbBalanceUpdate.mul(bnbRewardsFee).div(
            _liquidityFeeTotal
        );

        bnbRewardsFeeBalance += rewardReceived;

        //increase total bnb rewards
        totalRewardsReceived += rewardReceived;
    ...
    function _liquidityFeeAggregate() private view returns (uint256) {
        return treasuryFee.add(bnbRewardsFee).add(buybackFee).add(liquidityFee);
    }
    ...
    function removeSniper(address account) external onlyOwner {
        require(
            account != address(pancakeSwapRouter),
    ...
    function calculateBNBRewards(address wallet) public view returns (uint256) {
    ...
    function claimBNBRewards() external {
        ...
        emit SendBNBRewards(_msgSender(), rewardsSent);
    }
