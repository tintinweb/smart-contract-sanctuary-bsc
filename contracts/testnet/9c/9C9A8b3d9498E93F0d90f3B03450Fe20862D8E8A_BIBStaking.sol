...
contract BIBStaking is PausableUpgradeable, OwnableUpgradeable {
    ...
    function transferNodeSetUp(address from, address to, uint256 _ticket) external onlyNode {
       ...
       _setUserBalance(from, _ticket, getUserStakeAmount(from));
       _setUserBalance(to, _ticket, getUserStakeAmount(to));
    }
    ...
    function _stake(address operator, uint256 _ticket, uint256 _bibAmount) internal returns(bool) {
        ...
        emit Staking(operator, _ticket, _bibAmount);
        updataNodeWigth(_ticket);
       _setUserBalance(operator, _ticket, getUserStakeAmount(operator));
        return true;
    }
...
interface IStructureInterface {
    function getNodeStakeAmount(uint256 _id) external view returns (uint256);
}
...
library StructuredLinkedList {
    ...
    function getIndex(List storage self, uint256 _node) internal view returns (uint256 index) {
        if (sizeOf(self) == 0) {
            return 0;
        }
        ...
        uint256 next;
        (, next) = getAdjacent(self, _HEAD, _NEXT);
        while ((next != 0) && ((_node == next) != _NEXT)) {
            index++;
            next = self.list[next][_NEXT];
        }
    }

    function getNodeByIndex(List storage self, uint256 _index) internal view returns (uint256) {
        if (sizeOf(self) == 0) {
            return 0;
        }
