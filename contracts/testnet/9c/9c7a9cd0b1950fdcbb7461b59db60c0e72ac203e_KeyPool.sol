...
contract KeyPool is Ownable {
    ...
    function getLPBusd(uint256 _LP) public view returns (uint256 _busd){
        ...
        if (pairToken == address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd) || pairToken == address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c)) {
            _busd = (_reserve0 / aLp * _LP * 2);
        } else {
            _busd = (_reserve1 / aLp * _LP * 2);
        }
    }
    ...
    function queryIncome() external view returns(uint256,uint256,uint256,uint256){
        address _staker = _msgSender();
