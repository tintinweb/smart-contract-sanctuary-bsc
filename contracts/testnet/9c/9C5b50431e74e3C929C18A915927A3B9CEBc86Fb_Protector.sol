...
//    address public busdToken = 0xe9e7cea3dedca5984780bafc599bd69add087d56; //mainnet
    address public busdToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //mainnet


    address public metaGoldAddress = 0xB0e6b5c8BF5954A8cb8443CC9742E6b1F508F6C1; //testnet

    constructor() {
            IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(uniswapRouter);
            uniswapV2Router = _uniswapV2Router;
            metaGold = IERC20(metaGoldAddress);
            tokenPair = IUniswapV2Pair(metaGoldAddress);
    }



    function getMaxTokenAllowedToSend() public view returns(uint256 maxTokens, uint256 maxTokensFilter) {
        uint256 mcusd = getMarketCapUSD();
        if( mcusd < minimalMarketCap) {
            return (0,0);
        }

        (uint256 reserve_a_initial,,) = tokenPair.getReserves();
        reserve_a_initial = reserve_a_initial.div(1*10**18);
        uint256 substracted = mcusd.sub(minimalMarketCap);
        uint256 max_price_impact = substracted.mul(uint256(100000)).div(minimalMarketCap); //price impact
    ...
    function getMarketCapUSD() public view returns(uint256 marketCap) {
        ...
        uint256 currentPrice = getPrice(metaGoldAddress, uniswapV2Router.WETH(), busdToken, 1 * 10**18)[2];
