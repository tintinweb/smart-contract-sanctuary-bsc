...
contract RenaisnBox {
    ...
    uint256 public epoch = 1800; // 86400
    ...
    function startTime() external view returns (uint256) {
        (, uint256 startTime_) = isNewEpoch();
        return startTime_;
    }

    function endTime() external view returns (uint256) {
        (, uint256 startTime_) = isNewEpoch();
        return startTime_ + epoch;
    }

    function totalNum() external view returns (uint256) {
        (bool isNew, ) = isNewEpoch();
        return isNew ? pendingNum() : _totalNum;
    }

    function boxNum() external view returns (uint256) {
        (bool isNew, ) = isNewEpoch();
        return isNew ? pendingNum() : _boxNum;
    }

    function isNewEpoch() private view returns (bool, uint256) {
        if (
            _startTime > 0 &&
            !isEnd &&
            pendingSaleNum > 0 &&
            ...
            if (_epoch_day > lastStartEpoch) {
                return (true, _epoch_day * epoch);
            }
        }

        return (false, _startTime);
    }
    ...
    function autoStart() private {
        (bool isNew, uint256 startTime_) = isNewEpoch();
        if (isNew) {
            _setStart(startTime_);
        }
    }
