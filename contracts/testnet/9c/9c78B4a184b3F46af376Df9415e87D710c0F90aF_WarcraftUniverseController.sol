...
contract WarcraftUniverseController is Ownable {
    ...
    event GetUserPoolReward(uint256 num,uint256 time,uint256 joinTime,uint256 circleTime,uint256 endTime,uint256 startTime,uint256 _reward);
    ...
    constructor() {
        ...
        pools[1] = Pool(0,1,0,0,0,30  seconds,0,0);
        pools[2] = Pool(0,2,0,0,0,90  seconds,0,0);
        pools[3] = Pool(0,3,0,0,0,180 seconds,0,0);
        pools[4] = Pool(0,4,0,0,0,365 seconds,0,0);
    ...
    function setPveAmount(uint256 amount) public onlyOwner {
        pveAmount = amount;
    }
    ...
    function outPool(uint256 poolId) public {
        ...
        if(userPoolRewardTime[msg.sender][myPool.id] < time){
            ...
            uint256 reward = myPool.amount.mul(rewardTime * 100 ).mul(myPool.rate).div(myPool.circleTime * 10000);
    ...
    function getUserPoolReward (address user, uint256 poolType) public  returns(uint256 poolReward) {
        ...
        for(uint256 i = 1; i <= users[user].poolIds; i ++ ){
            ...
            if(myPool.id == poolType && myPool.amount > 0){
                ...
                if(time > startTime){
                    ...
                    uint256 _reward = myPool.amount.mul(time.sub(startTime) * 100 ).mul(myPool.rate).div(myPool.circleTime * 10000);
                
                    poolReward += _reward;
                    
                    emit GetUserPoolReward(i,time,myPool.joinTime,myPool.circleTime,endTime,startTime,_reward);
                }               
