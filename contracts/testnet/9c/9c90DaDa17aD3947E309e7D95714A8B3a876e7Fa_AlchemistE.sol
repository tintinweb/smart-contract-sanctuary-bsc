...
import "../IncomingTransferSupport/IncomingTransferSupportUpgradeable.sol";

contract AlchemistE is ReentrancyGuardUpgradeable, AccessControlUpgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, IncomingTransferSupportUpgradeable {
    bytes32 public constant WHITE_LIST= keccak256("WHITE_LIST");
    Counters.Counter private next_id;
    address payable  _farmPool;
    address payable _lootDAO;
    address payable _alchemistDAO;
    mapping(uint256 => address) private EOAOf;
    IERC20Upgradeable private _loot; // to immutable
    ItokenURI private _skin;
    ...
    function initialize(address payable farmPool_, address payable lootDAO_, address payable alchemistDAO_, address loot_, address tokenURIEx, address erc20TransferHelper_) public initializer {
        __ERC721_init("Alchemist", "ALCH");
        __AccessControl_init_unchained();
        __IncomingTransferSupport_init(erc20TransferHelper_);
        ...
        _farmPool = farmPool_;
        _lootDAO = lootDAO_;
        _alchemistDAO = alchemistDAO_;
        _loot = IERC20Upgradeable(loot_);
        _skin = ItokenURI(tokenURIEx);
    ...
    function farmPool() public view returns (address) {
        return _farmPool;
    }

    function lootDAO() public view returns (address) {
        return _lootDAO;
    }

    function alchemistDAO() public view virtual returns (address) {
        return _alchemistDAO;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        require(tokenId >= 0 && tokenId < next_id.current(), "ID is invalid");
        string memory output = _skin.image(tokenId);
    ...
    function _beforeTokenTransfer(
        ...
        uint256 _royalty = 10*(10**18);
        if(from != address(0) && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !to.isContract() && to != EOAOf[tokenId]){
            uint256 allowanceOfErc20TransferHelper = _loot.allowance(EOAOf[tokenId], _erc20TransferHelper);
            if (IERC20TransferHelper(_erc20TransferHelper).isModuleApproved(EOAOf[tokenId]) && allowanceOfErc20TransferHelper >= _royalty) {
                _handleIncomingTransfer(_royalty, address(_loot));
            } else {
                uint256 allowanceOf = _loot.allowance(EOAOf[tokenId], address(this));
                address payer;
                ...
                _loot.safeTransferFrom(payer, address(this), _royalty);
            }
            _loot.safeTransfer(_alchemistDAO, _royalty*3/10);
            _loot.safeTransfer(_farmPool, _royalty*2/10);
            _loot.safeTransfer(_lootDAO, _royalty*5/10);
        }
        if(!to.isContract()){
            EOAOf[tokenId] = to;
        }
...
contract IncomingTransferSupportUpgradeable is Initializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    /// @notice The ZORA ERC-20 Transfer Helper
    IERC20TransferHelper public erc20TransferHelper;

    function __IncomingTransferSupport_init(address _erc20TransferHelper) internal onlyInitializing {
    ...
    function _handleIncomingTransfer(uint256 _amount, address _currency) internal {
        if (_currency == address(0)) {
            ...
            IERC20Upgradeable token = IERC20Upgradeable(_currency);
