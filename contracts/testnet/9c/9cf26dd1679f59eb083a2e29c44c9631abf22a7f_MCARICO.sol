...
contract MCARICO {
    ...
    uint256 public tokenSalePercent;
    uint256 public referralPercent;
    uint256 public referralUserPercent;
    ...
    uint256 public tokenLaunch;
       constructor(
         ...
         uint256 _tokenSalePercent,
         uint256 _totalVestingDays,
         uint256 _referralPercent,
         uint256 _referralUserPercent,
     ...
     function getequaltoken(uint256 _amount) public  returns (uint256,uint256){
      ...
      function getequaltokenref(uint256 _amount) public  returns (uint256,uint256){
        uint256 perToken = tokenbusdprice.mul(_amount);
        uint256 pending = perToken.div(1000000);
        uint256 transferpending = pending/100;
        return (pending,transferpending);
     ...
     function deposit(uint256 _amount) public {
        require(block.timestamp < tokenLaunch, 'Launch Time Completed');
        ...
        user.startRewardTime = tokenLaunch;
      }

      function depositWithReferral(uint256 _amount, address _address) public {
        require(block.timestamp < tokenLaunch, 'Launch Time Completed');
        UserInfo storage userParent = userInfo[_address];
        ...
        uint256 pending = perToken.div(1000000);
        user.totalToken += pending;
        uint256 referralPercentAmount = _amount * referralPercent/1000;
        userParent.referralbonus += referralPercentAmount;
        userParent.referralincome += referralPercentAmount;
        uint256 referraluserPercentAmount = _amount * referralUserPercent/1000;
      ...
      function pendingMcar(address useraddress) public view returns (uint256) {
       ...
       function pendingReferralMcar(address useraddress) public view returns (uint256) {
          UserInfo storage user = userInfo[useraddress];
          uint256 referralPending = user.referralbonus;
          return  referralPending ;
       }

      function availableMcar(address useraddress) public view returns (uint256) {
          UserInfo storage user = userInfo[useraddress];
          uint256 diff = block.timestamp - user.startRewardTime; //seconds
           uint256 available = 0;
          if(block.timestamp > user.startRewardTime && diff > totalVestingDays){
              uint calcMcar = diff / totalVestingDays;
              if(calcMcar >= 5){
                  calcMcar = 5;
              }
              uint256 totalToken = user.totalToken;
              available = uint256(calcMcar) * totalToken.mul(tokenSalePercent)/100;
              if(calcMcar == 5){
                available = available - user.claimToken;
              }
     ...
     function claimReferralMcar() public {
            UserInfo storage user = userInfo[msg.sender];
    ...
    function claimMcar() public {
            ...
            if( block.timestamp > user.startRewardTime && diff > totalVestingDays ){
                ...
                available = uint256(calcMcar) * totalToken.mul(tokenSalePercent)/100;
                available = available * 100000000;
                user.startRewardTime  =  user.startRewardTime + (calcMcar * totalVestingDays);
            }else{
                available = 0;
            }
    ...
    function settimeLaunch(uint256 launchtime) public {
        require(msg.sender==owner , "Not Owner");
        tokenLaunch=launchtime;
    }

    function settokenPercent(uint256 percent) public {
        require(msg.sender==owner , "Not Owner");
        tokenSalePercent = percent;
    }

    function settokenVestingPeriod(uint256 _days) public {
        require(msg.sender==owner , "Not Owner");
         totalVestingDays = _days;
    }

     function setreferralPercent(uint256 percent) public {
        require(msg.sender==owner , "Not Owner");
