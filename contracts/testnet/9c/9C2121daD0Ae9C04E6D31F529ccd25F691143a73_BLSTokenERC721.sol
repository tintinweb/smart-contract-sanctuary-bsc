...
// Thirdweb Contracts v3.1.10 (contracts/TokenERC721.sol)
    ...
    ITokenERC721
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("BLSTokenERC721");
    ...
    keccak256(
        "MintRequest(address to,address royaltyRecipient,uint256 royaltyBps,address primarySaleRecipient,string uri,uint256 price,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid,bool isExclusive)"
    );
    ...
    /// @dev Max bps in the thirdweb system Todo private
    ...
    mapping(uint256 => bool) public isExclusive;
    ...
    returns (uint256 tokenIdMinted)
    {
        ...
        if (_req.isExclusive) {
            isExclusive[tokenIdMinted] = true;
        }
    ...
    function _mintTo(address _to, string calldata _uri) internal returns (uint256 tokenIdToMint) {
        ...
        tokenPower[tokenIdToMint] = getRandomPower(_to, tokenIdToMint);
    ...
    function getRandomPower(address to, uint tokenId)
    internal
    view
    returns (uint256 power)
    {
        ...
        if (isExclusive[tokenId]) {
            power = randomNum == 0 || randomNum % MAX_POWER_EXCLUSIVE < 6
            ? 6
            : randomNum % MAX_POWER_EXCLUSIVE;
        } else {
            power = randomNum == 0 || randomNum % MAX_POWER_SIMPLE == 0
            ? 1
            : randomNum % MAX_POWER_SIMPLE;
        }
    }
