...
contract ChilliNft is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function _afterTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            uint256 lastTokenId = _tokenIdsOfHolder[from].tokenIdOfIndex[_tokenIdsOfHolder[from].count];
            ...
            delete _tokenIdsOfHolder[from].tokenIdOfIndex[_tokenIdsOfHolder[from].count];
            _tokenIdsOfHolder[from].indexOfTokenId[lastTokenId] = index;
            _tokenIdsOfHolder[from].tokenIdOfIndex[index] = lastTokenId;
            _tokenIdsOfHolder[from].count = _tokenIdsOfHolder[from].count.sub(1);
        }
        if (to != address(0)) {
            _tokenIdsOfHolder[to].count = _tokenIdsOfHolder[to].count.add(1);
            _tokenIdsOfHolder[to].tokenIdOfIndex[_tokenIdsOfHolder[to].count] = tokenId;
            _tokenIdsOfHolder[to].indexOfTokenId[tokenId] = _tokenIdsOfHolder[to].count;
        }
    }

    function getTokenId (address holder, uint256 index) public view returns (uint256) {
