...
contract SPLMint {
    ...
    event LogDeposit(address indexed user, uint256 USDTAmount, uint256 SPLAmount);
    ...
    //分红
    function setProfitUSDT(uint256 BUSDTAmount, address profit) private  {
        bool Profit = isProfits(msg.sender);
        if(Profit){
            USDTToken.safeTransferFrom(msg.sender, profit, BUSDTAmount);
            profitUSDT[profit] += BUSDTAmount; 
        }
    }
    //节点规则
    ...
    // 用户抵押
    function deposit(uint256 USDTAmount,address parent,address profit) public {
        //用户是否买入最大次数
        ...
        //初始化为100所以不能小于100
        require(USDTAmount >= 100 + pricetime, "error: num < 100");
        ...
        //给节点分红
        setProfitUSDT(BUSDTAmount,profit);
        ...
        // 记录日志
        emit LogDeposit(msg.sender, USDTAmount, SPLAmounts[msg.sender]);      
    }
