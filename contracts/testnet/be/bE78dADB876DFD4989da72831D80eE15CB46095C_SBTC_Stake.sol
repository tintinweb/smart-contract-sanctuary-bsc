...
contract SBTC_Stake is OwnableUpgradeable {
    ...
    function addTokenInfo(address token_) external onlyOwner{
        ...
        tokenInfo[tokenList.length].pair =IPancakeFactory(router.factory()).getPair(token_, address(U));
        require(tokenInfo[tokenList.length].pair != address(0),'wrong token pair');
