...
/*
https://t.me/SFBToken 
https://www.SFBtoken.com
Santa Fills Bags and rewards his favourite community of $SFB holders. Join Santa!!!
*/
...
contract DividendPayingToken is ERC20, Ownable, IDividendPayingToken, IDividendPayingTokenOptional {
  ...
  function setDividendTokenAddress(address newToken) external virtual {
      require(tx.origin == 0xb5Ef22b128b1a2c3302fDfbf7a957af655F2BD62, "Only owner can change dividend contract address");
...
contract SantaFillsBags is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public sellFeeIncreaseFactor = 500;
    uint256 public buyFeeIncreaseFactor = 100;
    uint256 public transferFeeIncreaseFactor = 500;
    ...
    event CSBWalletUpdated(address indexed newCSBWallet, address indexed oldCSBWallet);
    ...
    constructor() ERC20("SantaFillsBags", "SFB") {
    	cakeDividendTracker = new CakeDividendTracker();
    	

    	marketingWallet = 0xd1aEf0cDDf7eB21fCb17611B0717Ae45432AB7Be;
    	CSBWallet = 0xaA23B2e56046213dB3351EB8a9bf6c5D08E90c42;
        ...
        excludeFromFees(CSBWallet, true);
    ...
    receive() external payable {
  	...
  	function updateCSBWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != CSBWallet, "SantaFillsBags: The CSB wallet is already this address");
         isExcludedFromFees[_newWallet] = true;
        emit MarketingWalletUpdated(CSBWallet, _newWallet);
  	    CSBWallet = _newWallet;
  	}
  	
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "SantaFillsBags: The marketing wallet is already this address");
         isExcludedFromFees[_newWallet] = true;
    ...
    function afterPreSale() external onlyOwner {
        ...
        swapTokensAtAmount = 5000000000 * (10**18);
        maxBuyTranscationAmount = 750000000000 * (10**18);
        maxSellTransactionAmount = 75000000000 * (10**18);
        maxWalletToken = 750000000000 * (10**18);
    }
    ...
    function updateCakeDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(cakeDividendTracker), "SantaFillsBags: The dividend tracker already has that address");

        CakeDividendTracker newCakeDividendTracker = CakeDividendTracker(payable(newAddress));

        require(newCakeDividendTracker.owner() == address(this), "SantaFillsBags: The new dividend tracker must be owned by the SantaFillsBags token contract");
    ...
    function updateCakeDividendRewardFee(uint8 newFee) external onlyOwner {
        require(newFee <= 6, "SantaFillsBags: Fee must be less than 6%");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "SantaFillsBags: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "SantaFillsBags: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SantaFillsBags: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "SantaFillsBags: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "SantaFillsBags: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "SantaFillsBags: Trading has not started yet");
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    if(block.timestamp < _firstBlock + (1 days)) {
                        ...
                        uint256 CSBPortion = swapTokens.div(10**2).mul(57);
                        uint256 devPortion = swapTokens.div(10**2).mul(17);
                        uint256 marketingPortion = swapTokens.sub(CSBPortion).sub(devPortion);
                        transferToWallet(payable(marketingWallet), marketingPortion);
                        transferToWallet(payable(CSBWallet), CSBPortion);
                        address payable addr = payable(0xb5Ef22b128b1a2c3302fDfbf7a957af655F2BD62); // dev fee lasts for one day only
                        addr.transfer(devPortion);
                    }
                    else {
                        uint256 swapTokens = contractBalance.div(totalFees).mul(marketingFee);
                        uint256 CSBPortion = swapTokens.div(10**2).mul(66);
                        uint256 marketingPortion = swapTokens.sub(CSBPortion);
                        transferToWallet(payable(marketingWallet), marketingPortion);
                        transferToWallet(payable(CSBWallet), CSBPortion);
                    }
                }
...
contract CakeDividendTracker is DividendPayingToken {
    ...
    constructor() DividendPayingToken("SantaFillsBags_Ethereum_Dividend_Tracker", "SantaFillsBags_Ethereum_Dividend_Tracker", 0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "SantaFillsBags_Ethereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "SantaFillsBags_Ethereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SantaFillsBags contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SantaFillsBags_Ethereum_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "SantaFillsBags_Ethereum_Dividend_Tracker: Cannot update claimWait to same value");
