...
contract GrabStakev2 is Ownable{
    ...
    *
    ...
    // Added by Mab-Iggo for get start staking time
    
    function getLastStakeDate (address userAddress, uint poolID) public view returns(uint){
        if(pool[poolID].hasStaked[userAddress] == true){
            uint lastTimeStaked = pool[poolID].lastTimeUserStaked[userAddress];
            // uint periodSpentStaking = block.timestamp - lastTimeStaked;

            uint userTime_Left = lastTimeStaked - pool[poolID].stakingDuration; 

            return  userTime_Left;
        }else{
            return 0;
        }
    }
    ...
    function claimReward(uint _poolID) external {
        ...
        // uint periodSpentStaking = block.timestamp - stakeTime;
