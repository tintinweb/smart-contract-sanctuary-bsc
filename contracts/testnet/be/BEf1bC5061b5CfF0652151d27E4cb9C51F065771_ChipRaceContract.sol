...
contract ChipRaceContract is Ownable {
    using SafeERC20 for IERC20;
    ERC721Enumerable g_nftAddress;
    IERC20 g_craceToken;
    ...
    uint256 private g_lockTime;
    uint256 private feeAmount;
    uint256 private feePrice;

    event EnterChipRace(uint256 _tokenId, uint256 _poolId);
    event EmergencyExitChipRace(uint256 _tokenId, uint256 _feeAmount);
    event ClaimRewardsFromChipRace(uint256 _tokenId);
    event UpgradeNFT(uint256 _tokenId, uint256 _feeAmount);

    constructor(ERC721Enumerable _nftAddress, IERC20 _tokenAddress) {
        g_nftAddress = _nftAddress;
    ...
    function getOwnerOf(uint256 _tokenId) external view returns(address) {
        return g_nftAddress.ownerOf(_tokenId);
    }
    ...
    function enterChipRace(uint256 _tokenId, uint256 _poolId) external {
        ...
        require(g_carType[g_nftAddress.tokenURI(_tokenId)] == _poolId, "You can't enter this pool with this NFT");
        require(g_nftAddress.getApproved(_tokenId) == address(this), "Require NFT ownership transfer approval");

        g_nftAddress.safeTransferFrom(msg.sender, address(this), _tokenId);
    ...
    function emergencyExitChipRace(uint256 _tokenId, uint256 _amount) external {
        require(g_tokenInfo[_tokenId].owner == msg.sender, "You are not a owner of this NFT");
        require(block.timestamp - g_tokenInfo[_tokenId].timeStamp < g_lockTime, "Racing is already ended!");
        require(g_tokenInfo[_tokenId].isRunning == true, "You didn't run the race!");
        require(_amount == feePrice, "You don't have enough funds for emergency exit");
        
        g_nftAddress.safeTransferFrom(address(this), msg.sender, _tokenId);
        g_craceToken.safeTransferFrom(msg.sender, address(this), _amount);

        feeAmount = feeAmount + _amount;
        TokenInfo storage info = g_tokenInfo[_tokenId];
        info.isRunning = false;
        emit EmergencyExitChipRace(_tokenId, _amount);
    }

    function claimRewards(uint256 _tokenId) external {
        require(g_tokenInfo[_tokenId].owner == msg.sender, "You are not a owner of this NFT");
        require(g_tokenInfo[_tokenId].isRunning == true, "You didn't run the race!");
        require(block.timestamp - g_tokenInfo[_tokenId].timeStamp >= g_lockTime, "Racing is not ended yet!");

        g_nftAddress.safeTransferFrom(address(this), msg.sender, _tokenId);

        uint256 carType = g_carType[g_nftAddress.tokenURI(_tokenId)];
        TokenInfo storage info = g_tokenInfo[_tokenId];
        uint256 amount = g_minableAmount[carType][info.level] * 60 * 24;
        ...
        emit ClaimRewardsFromChipRace(_tokenId);
    }
    ...
    function updateScore(uint256 _score, uint256[] memory tokenIds, address caller) external {
        require(msg.sender == caller, "This function is called by only PvP contract");
