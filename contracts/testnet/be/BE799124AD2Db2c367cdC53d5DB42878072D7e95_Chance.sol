...
contract Chance is ERC721URIStorage, ERC721Enumerable, Ownable, ReentrancyGuard{
    ...
    event mintLog(address _addrMint, uint256 _id, uint256 round);
    ...
    function mintWIN(string memory _inviteCode) public payable returns (uint256) {
        ...
        emit mintLog(msg.sender, tokenId, round);
        return tokenId;
    }
    ...
    function withdrawByCommunity(address payable _to, uint256 _amount) public nonReentrant {
        require(msg.sender == DEVELOP_ADDR || msg.sender == COMMUNITY_ADDR, "Not allow");
        uint256 amount = accountMap[msg.sender];
        require(_amount <= amount, "Not allowed balance");
    ...
    function withdrawAmount(address payable _to, uint256 _amount) public nonReentrant {
        if (msg.sender != DEVELOP_ADDR || msg.sender != COMMUNITY_ADDR) {
            uint256 _iCount = ChanceInvite(_ChanceInvite).inviteCount(msg.sender);
...
contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  function setReader(address _r) public onlyOwner {
    reader = _r;
  }

  function getSeed(uint256 _s) public view returns(uint256){
    require(msg.sender == reader, "notAllow");
