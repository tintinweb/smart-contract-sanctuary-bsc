...
contract LamaToken is ERC20, ERC20Burnable, Ownable, Pausable, EmergencyGuard {
    ...
    constructor(
        ...
        address _uniswapRouterAddress) ERC20("LAMA", "LAMA") {
...
//            if (contractLamaTokenBalance > balanceOf(uniswapPair) / contractTokenDivisor) {
//                swapLamaTokensForBnb(contractLamaTokenBalance);
//            }

            uint256 contractBnbBalance = address(this).balance;
            if (contractBnbBalance > 0) {
                sendFeeToWallet(address(this).balance);
            }
        }
...
//            }
     ...
     * @param amount the amount to be transfered
     */
    function sendFeeToWallet(uint256 amount) private {

        (bool feeSuccess,) = feeWalletAddress.call{value : amount}("");
        require(feeSuccess, "Failed to send BNB to fee wallet address");
    ...
    function disperseFeesManually() external onlyOwner {
        uint256 contractBnbBalance = address(this).balance;
        sendFeeToWallet(contractBnbBalance);
    }
    ...
    */
    ...
    function setUniswapRouter(address value) external onlyOwner {
        require(
            value != address(0),
            "Lama: Cannot set Pancake Router to zero address"
        );
