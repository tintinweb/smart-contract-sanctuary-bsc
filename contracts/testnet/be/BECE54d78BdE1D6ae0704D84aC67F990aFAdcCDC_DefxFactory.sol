...
import "./interfaces/IDefxPair.sol";
import "./interfaces/IDefxToken.sol";
...
contract DefxFactory {
    ...
    function createPair(address cryptoAddress, string memory fiatCode) external returns (address pair) {
        ...
        IERC20(cryptoAddress).approve(PANCAKE_ROUTER, MAX_UINT);
    }
    ...
    function buyDefxSendRewards(address feeToken) external {
        ...
        IPancakeRouter02(PANCAKE_ROUTER).swapExactTokensForTokens(amountIn, 0, path, address(this), block.timestamp);
    }
...
contract DefxPair {
    ...
    uint256 public EXPIRY_BLOCKS = DealHelpers.EXPIRY_BLOCKS;
    ...
    function sendMessage(
        ...
        DealHelpers.sendMessage(deals[buyer][seller], buyer, seller, encryptedForSeller, encryptedForBuyer);
    }
    ...
    function cancelDeal(address buyer, address seller) external {
        ...
        DealHelpers.cancelDeal(factory, cryptoAddress, offer, deal, buyer, seller, dealLinks);
    }
    ...
    function confirmFiatReceived(address buyer) public {
        ...
        DealHelpers.confirmFiatReceived(factory, cryptoAddress, offer, deal, buyer, dealLinks);
    }
    ...
    function confirmFiatReceivedWithFeedback(
        ...
        DealHelpers.submitFeedbackFrom(factory, buyer, isPositive, desc);
    }

    function fiatSent(address seller) external {
        DealHelpers.fiatSent(deals[msg.sender][seller]);
    }

    function liquidateDeal(address buyer, address seller) external {
        DealHelpers.liquidateDeal(factory, cryptoAddress, deals[buyer][seller], buyer, seller, dealLinks);
    }

    function withdraw(bool _isBuy, uint256 _toWithdraw) external {
        OfferHelpers.withdraw(cryptoAddress, offers[msg.sender][_isBuy], _isBuy, _toWithdraw);
    }
    ...
    function editOfferAvailable(
        ...
        OfferHelpers.editOfferAvailable(cryptoAddress, offers[msg.sender][_isBuy], _isBuy, _available, _price, _min, _max);
    }
    ...
    function editOfferParams(
        ...
        OfferHelpers.editOfferParams(offers[msg.sender][_isBuy], _isBuy, _paymentMethods, _desc);
    }

    function editOfferPrice(bool _isBuy, uint256 _price) public {
        OfferHelpers.editOfferPrice(offers[msg.sender][_isBuy], _isBuy, _price);
    }

    function renewOffer(bool _isBuy) public {
        OfferHelpers._renewOffer(offers[msg.sender][_isBuy], _isBuy, msg.sender);
    }
}
