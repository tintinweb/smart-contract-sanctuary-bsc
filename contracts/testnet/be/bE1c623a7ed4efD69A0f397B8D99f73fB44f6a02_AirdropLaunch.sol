...
interface ILaunchMod {
  ...
  function getUnSoldTickets(uint256 _projectId, uint256 _fundingCycleId)
    external
    returns (uint256[] memory);
}
...
interface IMembershipPassBooth {
  ...
  function tierFeeOf(uint256 _projectId, uint256 _tierId) external view returns (uint256);

  function tierCapacityOf(uint256 _projectId, uint256 _tierId) external view returns (uint256);

  function suppliedByTierOf(uint256 _projectId, uint256 _tierId) external view returns (uint256);

  function membershipPassOf(uint256 _projectId) external view returns (IMembershipPass);

  function getRemainingAmount(uint256 _projectId, uint256 _tierId) external view returns (uint256);
...
contract AirdropLaunch is Initializable, ILaunchMod {
  event Launch(
    uint256 indexed projectId,
    uint256 indexed fundingCycleId,
  ...
  /**
   * @notice
   * Return the unsole tickets of given funding cycle
   *
  ...
  function launch(
    ...
    AirdropConfig memory _airdropConfig = abi.decode(_encodedParam, (AirdropConfig));
    uint256 _batchSize = _airdropConfig.receiver.length;
    if (
      _batchSize == 0 ||
      _batchSize != _airdropConfig.idToSend.length ||
      _batchSize != _airdropConfig.valueToSend.length
    ) {
      revert SizeNotMatch();
    }

    uint256[] memory _amountByTierIds = new uint256[](membershipPassBooth.tierSizeOf(_projectId));
    for (uint256 i; i < _batchSize; i++) {
      _amountByTierIds[_airdropConfig.idToSend[i]] += _airdropConfig.valueToSend[i];
      membershipPassBooth.mintTicket(
        _projectId,
        _airdropConfig.receiver[i],
        _airdropConfig.idToSend[i],
        _airdropConfig.valueToSend[i]
      );
    }

    for (uint256 i; i < _amountByTierIds.length; i++) {
      if (_amountByTierIds[i] != fundingCycles.getAutionedPass(_fundingCycleId, i).allocateAmount) {
        revert InsufficientBalance();
      }

      emit AirdropBatchMintTicket(_projectId, _fundingCycleId, i, _amountByTierIds[i]);
    }

    emit Launch(_projectId, _fundingCycleId, _airdropConfig);
  }
}