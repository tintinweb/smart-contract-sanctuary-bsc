...
contract Token is IERC20 {
    ...
    //uint256 public _step_size;
    uint256 public step_size;
    bool public can_buy;
    ...
    //function setStep(uint256 step_size) public onlyOwner {
      //  require(step_size >= 1, "Step size must be whole number greater than zero");
        //_step_size = step_size;
        //step_size of 100 equals 1%
    //}
    ...
    function setTargets() public onlyOwner {
        ...
        target_buy = price.add(price * ((step_size) / 1e4));
        target_sell = price.add(price * ((step_size) / 2e4));
    }
    ...
    function adjustTargets() private {
        target_sell = target_sell.add((target_buy) * ((step_size) / 2e4));
        target_buy = target_buy.add((target_sell) * ((step_size) / 1e4));
    }
    ...
    function accumulateFees() public onlyOwner {
        ...
        uint256 fee = mulDiv(tokenBalance, FEE, 1);
