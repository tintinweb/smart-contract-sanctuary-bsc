...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  modifier onlyValidClaimablePeriod {
    require(isValidClaimablePeriod(), "CLAIMABLE_PERIOD_ENDED");
    _;
  }

  modifier onlyAuctionSuccessfulAndFinalized {
   ...
   * @notice Commit ETH
   */
  function commitETH(bytes calldata signature) external payable onlyAuctionWindow {
   ...
   */
  function commitERC20Token(uint256 _amount, bytes calldata signature) external onlyAuctionWindow {
    require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
    require(isAllowlist(msg.sender, signature), "USER_NOT_IN_ALLOWLIST");
   ...
   */
  function withdrawETH(uint256 _amount) external onlyAuctionWindow nonReentrant {
    require(COMMIT_CURRENCY == ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
   ...
   * @notice Withdraw during the auction
   */
  function withdrawERC20Token(uint256 _amount) external onlyAuctionWindow nonReentrant {
    require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
   ...
   */
  function claimInstantToken() external onlyAuctionSuccessfulAndFinalized onlyValidClaimablePeriod {
   ...
   */
  function claimVestedLPToken() external onlyAuctionSuccessfulAndFinalized onlyValidClaimablePeriod {
   ...
   */
  function getTotalVestedLPTokenAmount(address _user) public view onlyAuctionSuccessfulAndFinalized returns (uint256) {
    require(COMMITMENTS[_user] > 0, "NO_COMMITMENTS");
  ...
  */
  ...
  function getUserReceivedInstantTokenAmount(address _user) external view returns (bool) {
  ...
  function _addCommitment(address _user, uint256 _amount) internal onlyAuctionWindow {
    require(_amount > 0, "INVALID_AMOUNT");
  ...
  function _withdrawCommitment(address _user, uint256 _amount) internal onlyAuctionWindow {
    require(_amount > 0, "INVALID_AMOUNT");
