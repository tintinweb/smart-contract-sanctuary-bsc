...
contract USDyVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    uint256 public interval; /** Use an interval in seconds and a timestamp to slow execution of Upkeep */
    ...
    uint256 public rebaseRewardSum; // sum of all rebase reward
    uint256[] public rebase_Reward; // divide all the value by rebaseRewardSum
    ...
    @param updateInterval updates the interval time
    */

    function initialize(address _admin, uint256 updateInterval)
        external
        initializer
    {
        Admin = IAdmin(_admin);
        ...
        IReceipt(USDs).initialize(_admin,address(this),"USDs","USDs");
        ...
        withdrawTax = 10;
        rebaseRate = 7;
        epochTime = 10 seconds; // note For testing 8 Hours = 10 seconds
        interval = 24 hours;
    ...
    function deposit(
        ...
        if (!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(
                ...
                "USDyVault: you can't interact in same block"
            );
        }
        IERC20(Admin.USDy()).safeTransferFrom(user, address(this), _amount);
        exchangeRatio = exchangeRatio == 0 ? 10**18 : exchangeRatio;
        console.log("mint amount",(_amount * (100 - depositTax) * (10**18)) / (exchangeRatio * 100));  
        IReceipt(USDs).mint(
            address(this),
            (_amount * (100 - depositTax) * (10**18)) / (exchangeRatio * 100)
        );
        // console.log(IReceipt(USDs).balanceOf(address(this)));

        UserDeposit[user] =
            (_amount * (100 - depositTax) * (10**18)) /
            (exchangeRatio * 100);
            // console.log("IReceipt(USDs).totalSupply()",IReceipt(USDs).totalSupply());
        exchangeRatio =
    ...
    */
    ...
    function distribute(uint256 _rebaseReward) internal {
        currentReward =
            currentReward +
            ((_rebaseReward * 10**18) / (IERC20(Admin.USDy()).totalSupply()));
    }
    ...
    */
    ...
    function rewards(address user, address token)
        public
        view
        returns (uint256)
    {
        return (
            ((_userAmount(user, token) * (currentReward - ss[msg.sender])) /
                10**18)
        );
    }
    ...
    */
    ...
    function reBaseRewards() external nonReentrant _isAdminOrOperator {
        ...
        IERC20(Admin.USDy()).safeTransfer(
            Admin.YSLBUSDVault(),
            (mintReward * rebase_Reward[0]) / rebaseRewardSum
        );

        IERC20(Admin.USDy()).safeTransfer(
            Admin.xYSLBUSDVault(),
            (mintReward * rebase_Reward[1]) / rebaseRewardSum
        );

        IERC20(Admin.USDy()).safeTransfer(
            Admin.USDyBUSDVault(),
            (mintReward * rebase_Reward[2]) / rebaseRewardSum
        );

        IERC20(Admin.USDy()).safeTransfer(
            Admin.BShareBUSDVault(),
            (mintReward * rebase_Reward[3]) / rebaseRewardSum
        );

        // IERC20(Admin.USDy()).safeTransfer(
        //     Admin.YSLVault(),
        //     (mintReward * rebase_Reward[5]) / rebaseRewardSum
        // );
        // IERC20(Admin.USDy()).safeTransfer(
        //     Admin.xYSLVault(),
        //     (mintReward * rebase_Reward[6]) / rebaseRewardSum
        // );
        ...
        distribute((mintReward * rebase_Reward[4]) / rebaseRewardSum);
    ...
    */
    ...
    function setRebaseRateUSDY(uint256 rate) external _isAdmin {
        require(rate > 0 && rate <= 30, "USDyVault: Invalid rate amount");
        emit SetterForUint(
            "USDyVault",
            address(this),
            rebaseRate,
    ...
    */
    ...
    function setRebaseRewardAndRebaseRewardSum(
        ...
        rebaseRewardSum = _rebaseRewardSum;
    ...
    */
    ...
    function claimRebaseReward(address user) public nonReentrant {
        console.log("claim_rebase");
        _claimRebaseReward(user);
    }

    function _claimRebaseReward(address user) internal {
        uint256 userAmount = ((UserDeposit[user] * exchangeRatio) /
            IERC20(address(Admin.USDy())).totalSupply());
        reward = ((userAmount * (currentReward - ss[user])) / 10**18);
        console.log("user",user,"reward",reward);
        IERC20(Admin.USDy()).safeTransfer(user, reward);
        ss[user] = currentReward;
    }

    function _userAmount(address user, address token)
        internal
        view
        returns (uint256)
    {
        return ((UserDeposit[user] * exchangeRatio) /
