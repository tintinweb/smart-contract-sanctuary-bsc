...
contract SmartBeComBee is SmartBeComBeeBasic {
    ...
    function updB6Referrer(address userAddress, address referrerAddress, uint8 level, Pay memory pay) private {
        ...
        address first0 = users[referrerAddress].b6Matrix[level].firstLevelReferrals[0];
        address first1 = users[referrerAddress].b6Matrix[level].firstLevelReferrals[1];
        ...
        if (first1 == userAddress) {
            updB6(userAddress, referrerAddress, level, false);
            return updB6ReferrerSecondLevel(userAddress, referrerAddress, level, pay);
        } else if (first0 == userAddress) {
            updB6(userAddress, referrerAddress, level, true);
            return updB6ReferrerSecondLevel(userAddress, referrerAddress, level, pay);
        }
        ...
        if (first0 == first1){
            ...
            if (users[first0].b6Matrix[level].firstLevelReferrals.length <= 
                users[first1].b6Matrix[level].firstLevelReferrals.length) {
                updB6(userAddress, referrerAddress, level, false);
            } else {
                updB6(userAddress, referrerAddress, level, true);
            }
    ...
    function updB6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        ...
        if (!x2) {
            users[first0].b6Matrix[level].firstLevelReferrals.push(userAddress);

            uint8 place = uint8(users[first0].b6Matrix[level].firstLevelReferrals.length);
            
            emit NewUserPlace(userAddress, first0, 2, level, place, users[referrerAddress].b6Matrix[level].reinvestCount, 2);
            ...
            //users[first0].b6Matrix[level].toReferrals.push(120+ place);
            //users[referrerAddress].b6Matrix[level].toReferrals.push(200 + 10*usertype_ + 2 + place);

            //set current level
            users[userAddress].b6Matrix[level].currentReferrer = first0;
        } else {
            users[first1].b6Matrix[level].firstLevelReferrals.push(userAddress);

            uint8 place = uint8(users[first1].b6Matrix[level].firstLevelReferrals.length);

            emit NewUserPlace(userAddress, first1, 2, level, place, users[referrerAddress].b6Matrix[level].reinvestCount, 2);
            ...
            //users[first1].b6Matrix[level].toReferrals.push(120+ place);
            //users[referrerAddress].b6Matrix[level].toReferrals.push(200 + 10*usertype_ + 4 + place);

            //set current level
            users[userAddress].b6Matrix[level].currentReferrer = first1;
        }
    }
