...
contract Marketplace is IERC721Receiver, ReentrancyGuard {
    ...
    struct ItemTier {
        ...
        uint256 oldestListingId;
        uint256 latestListingId;
    ...
    function oldestListingOfTier(uint256 _tier) public view returns (uint256) {
        return (tierData[_tier].oldestListingId);
    }

    // Fetch the back of the queue, of a tier
    function latestListingOfTier(uint256 _tier) public view returns (uint256) {
        return (tierData[_tier].latestListingId);
    }
    ...
    function listItem(address _contract, uint256 _tokenId, uint256 _markup) public nonReentrant {
        ...
        tierData[_tier].latestListingId = listingId;

        // Will overflow automatically and reset itself
        tierData[_tier].latestListingId++;
    ...
    function buyItemFromMarket(uint256 _tierId, address _recipient) internal returns (uint256) {
        ...
        uint256 _listingId = tierData[_tierId].oldestListingId;
        ...
        require(IERC20(paymentToken).transferFrom(_recipient, address(this), price), 'Must pay item price');
        ...
        // Sanity checks
        require(tierData[_tierId].oldestListingId != tierData[_tierId].latestListingId);

        // Clean-up previous entry
        tierData[_tierId].oldestListingId = 0x0;
        
        // Will overflow automatically and reset itself
        tierData[_tierId].oldestListingId++;
