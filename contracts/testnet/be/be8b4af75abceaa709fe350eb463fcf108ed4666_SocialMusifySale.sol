...
//wallet : 0x7b458F22c381fb536B410E9E341672B2C2e9B48E

//token:    0x31Af41193e68Ea27f81dc4c7Bfe7DA7b553c8e7C
...
contract SocialMusifySale {
    ...
    uint256 totalleft = openingTime.add(100);
    uint256 private key;
    ...
    function buyokens() external payable {
    ...
    function EndICO(uint256 _key) public payable {
...
contract timeLock {
    ...
    constructor(SocialMusify _token, uint256 _Key) {
        token = _token;
        admin = msg.sender;
        key = _Key;
    }

    function EndLock(uint256 _Key) public payable {
        require(msg.sender == admin);
        require(key == _Key);
        if (totalleft <= block.timestamp) {
            over = true;
