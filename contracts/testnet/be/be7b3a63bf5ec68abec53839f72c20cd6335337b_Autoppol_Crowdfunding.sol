     ...
     function payReferral(uint _level, address _user) internal {
        ...
        referer = userList[users[_user].referrerID];
         ...
         bool sent = false;
                        ...
                        sent = busd.transfer(address(uint160(referer)),LEVEL_PRICE);
            if (sent) {
       ...
       function paycoReferral(uint _level, address _user) internal {
        ...
        referer = userList[users[_user].coreferrerID];
         ...
         bool sent = false;
            ...
            sent = busd.transfer(address(uint160(referer)),LEVEL_PRICE/2);
            if (sent) {
       ...
       sent = busd.transfer(address(uint160(pool1Currentuser)),pool1_price/10*4);
            if (sent) {
                ...
                //users[userList[users[msg.sender].pool1Currentuser]].buyPoolBalance = users[userList[users[msg.sender].pool1Currentuser]].buyPoolBalance + (pool1_price / 10*4);
               // users[userList[users[msg.sender].referrerID]].buyPoolBalance = users[userList[users[msg.sender].referrerID]].buyPoolBalance - pool1_price;
