...
contract Mint is ERC721,Ownable, Pausable {
  error Soulbound();
  using Address for address payable;
  using Strings for uint256;
  using Counters for Counters.Counter;
  mapping(address => uint256) private status;
  uint256 private pricePerToken = 0.0001 ether;
  ...
  function tokenURI(uint256 tokenId)public view virtual override returns (string memory){
    ...
    if(status[msg.sender] == 1){
      return bytes(uri).length > 0 ? string(abi.encodePacked(uri,"")) : "";
    } else {
      return bytes(uri).length > 0 ? string(abi.encodePacked("https://file.suran.vip/kill.json","")) : "";
    }
  }
  ...
  function refund() public returns(uint256){
    require(status[msg.sender] == 1,"It has been refunded");
    tokenURI(tokenID[msg.sender]);
    status[msg.sender] = 2;
    nftamount = nftSub(nftamount);
    return nftamount;
  }
  ...
  //nftamount + 
  function nftAdd(uint256 a) internal pure returns (uint256) {
      require(a != 5, "NFT amount Insufficient");
      a++;
      return a;
  }
  ...
  function nftSub(uint256 a) internal pure returns (uint256) {
      require(a != 0, "NFT No refund");
      a--;
      return a;
  }

  /// @notice This function was disabled to make the token Soulbound. Calling it will revert
  function setApprovalForAll(address, bool) public override virtual {
      revert Soulbound();
  }

  /// @notice This function was disabled to make the token Soulbound. Calling it will revert
  function transferFrom(address,address,uint256) public override virtual {
      revert Soulbound();
  }

  /// @notice This function was disabled to make the token Soulbound. Calling it will revert
  function safeTransferFrom( address,address,uint256) public override virtual {
      revert Soulbound();
  }

  /// @notice This function was disabled to make the token Soulbound. Calling it will revert
  function safeTransferFrom(address,address,uint256,bytes memory) public override virtual {
      revert Soulbound();
  }

  function approve(address, uint256) public override virtual{
      revert Soulbound();
  }

  function transferOwnership(address) public override virtual{
      revert Soulbound();
  }
}