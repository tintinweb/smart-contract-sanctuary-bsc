...
import "./RegularCompetitionContractMultiMatches.sol";
...
contract GuaranteedCompetitionContractMultiMatches is
    RegularCompetitionContractMultiMatches,
    IGuaranteedCompetitionContractMultiMatches
{
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < betOptions.length; i++) {
            totalLength += betOptions[i].length;
        }
        require(betIndexs.length == totalLength, "GCC05");

        uint256 totalOptions;
        for (uint256 i = 0; i < betOptions.length; i++) {
            for (uint256 j = 0; j < betOptions[i].length; j++) {
                require(
                    betIndexs[j + totalOptions] <=
                        betOptions[i][j].brackets.length,
                    "GCC06"
                );
            }
            totalOptions = i == 0
                ? totalOptions + betOptions[i].length - 1
                : totalOptions + betOptions[i].length;
        }
...
contract RegularCompetitionContractMultiMatches is
    CompetitionContract,
    IRegularCompetitionContractMultiMatches
{
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    CompetitionMulti public competition;
    ...
    mapping(string => BetOption[]) public betOptionsMap;
    ...
    function setBetOptions(BetOption[] memory _betOptions) external override {}

    function setBetOptions(BetOption[][] memory _multiBetOptions)
        external
        override
        onlyConfigurator
        onlyLock
    {
        require(_multiBetOptions.length > 0, "RCC04");
        ...
        for (uint256 i = 0; i < competition.competitionId.length; i++) {
            require(_multiBetOptions[i].length > 0, "RCC04");

            for (uint256 j = 0; j < _multiBetOptions[i].length; j++) {
                require(
                    sportManager.checkSupportedAttribute(
                        competition.sportTypeAlias,
                        _multiBetOptions[i][j].attribute
                    ),
                    "RCC06"
                );

                require(_checkBetOption(j, _multiBetOptions[i]), "RCC07");
                if (
                    sportManager.checkTeamOption(
                        _multiBetOptions[i][j].attribute
                    )
                ) {
                    ...
                    betOptions[i].push(
                        BetOption({
                            mode: Mode.Team,
                            attribute: _multiBetOptions[i][j].attribute,
                            id: "0",
                            brackets: brackets
                        })
                    );
                } else {
                    betOptions[i].push(_multiBetOptions[i][j]);
                }
            }
            betOptionsMap[competition.competitionId[i]] = betOptions[i];
        }
    }
        ...
        onlyOwner
    {
        ...
        require(betIndexs.length == totalLength, "RCC10");
    ...
    function _getResult() internal view returns (bytes32 _key, bool _success) {
        ...
        (uint256[][] memory result, ) = IChainLinkOracleSportDataMultiMatches(
            oracle
        ).getData(requestID);
        ...
        require(result.length == totalLength, "RCC19");

        uint256[] memory betWin = new uint256[](totalLength);

        uint256 count;
        for (uint256 j = 0; j < betOptions.length; j++) {
            for (uint256 i = 0; i < betOptions[j].length; i++) {
                BetOption memory betOption = betOptions[j][i];
                if (sportManager.checkTeamOption(betOptions[j][i].attribute)) {
                    string memory team = String.toString(
                        result[j][i] / decimalOfResult
                    );
                    if (team.compare(competition.team1[j])) {
                        betWin[count] = 0;
                    } else if (team.compare(competition.team2[j])) {
                        betWin[count] = 1;
        ...
        returns (bool)
    {
        ...
        if (_betOptions[_index].mode == Mode.Team) {
            ...
            if (_betOptions[_index].mode == Mode.Player) {
                uint256 lastBraket = _betOptions[_index].brackets[
                    _betOptions[_index].brackets.length - 1
                ];
                if (
                    attribute.attributeSupportFor ==
                    ISportManager.AttributeSupportFor.None ||
