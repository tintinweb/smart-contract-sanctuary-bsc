...
contract AutomateRegistrar is IVersion, ConfirmedOwner, IReceiver {
  ...
  event RegistrationApproved(bytes32 indexed hash, string displayName, uint256 indexed taskId);
  ...
  function register(
    ...
    if (_autoApproveIfNecessary(config, sender)) {
      ...
      _approve(name, taskContract, gasLimit, adminAddress, checkData, amount, hash);
    } else {
  ...
  function approve(
    ...
    _approve(name, taskContract, gasLimit, adminAddress, checkData, request.balance, hash);
  }
  ...
  function _approve(
    ...
    // register task
    uint256 taskId = automateRegistry.registerTask(taskContract, gasLimit, adminAddress, checkData);
    // fund task
    ...
    emit RegistrationApproved(hash, name, taskId);
  }
