...
contract STRKFarmingStaking {
  ...
  function stake(uint256 amount,uint8 planid) external payable {
    ...
    totalStaked+=amount;
    users[msg.sender].roi.push(ROI(planid,amount,block.timestamp,false));
  ...
  function getUserRoiDetails (address user ,uint256 roi_index) external view returns (uint8,uint256 ,uint256 ,bool) {
    return (users[user].roi[roi_index].planid,users[user].roi[roi_index].amount,users[user].roi[roi_index].timestamp, users[user].roi[roi_index].isPricipleWithdraw);
  }
