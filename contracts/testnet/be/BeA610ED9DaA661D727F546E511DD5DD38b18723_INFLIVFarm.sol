...
contract INFLIVFarm is Initializable, OwnableUpgradeable {
  ...
  function initialize() public initializer {
    ...
    Referrals = IReferrals(0xEb3697d4a6cFA2Fc92F6500cF6b192481123dFAf);
  ...
  // 		else
  // 		{
  // 		   governanceTax = amount - amountUsed;
  // 		   if(governanceTax > 0)
  // 		   {
  // 		      IERC20Upgradeable(rewardToken).approve(address(rewardToken), governanceTax);
  //               IGovernance(rewardToken).addGovernanceFee(governanceTax);
  ...
  function deposit(uint256 amount, uint256 stakingID) external {
    ...
    if (stakingID == 0) {
      ...
      mapUserInfo[msg.sender][stakingID].amount += amount;
      mapUserInfo[msg.sender][stakingID].rewardDebt = (mapUserInfo[msg.sender][
        stakingID
      ].amount * accTokenPerShare);
      totalStaked += amount;
        ...
        if (stakingID == 0) {
          ...
          for (uint256 i = 0; i < ids.length; i++) {
            if (
              ...
              .deposit[stakingID][ids[i]].amount;
              mapUserInfo[msg.sender][stakingID].rewardDebt = (mapUserInfo[
                msg.sender
              ][stakingID].amount * accTokenPerShare);
              if (mapUserInfo[msg.sender][stakingID].farm == 1) {
                IERC20Upgradeable(stakedToken[0]).safeTransfer(
                  address(msg.sender),
                  mapUserInfo[msg.sender][stakingID]
    ...
    returns (uint256)
  {
    if (mapUserInfo[user][stakingID].amount > 0) {
      uint256 pending = (mapUserInfo[user][stakingID].amount *
        accTokenPerShare) - mapUserInfo[user][stakingID].rewardDebt;
      return pending;
    } else {
      return 0;
    }
