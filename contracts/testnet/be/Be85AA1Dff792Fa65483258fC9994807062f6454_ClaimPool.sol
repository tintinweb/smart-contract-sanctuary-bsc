...
contract ClaimPool is IClaimPool, Adminable, ERC165Upgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     *  @notice projectOwner is address of project owner
     */
    ...
    event WithdrawnCollection(address indexed collectionAddress, address indexed to, uint256 amount);
     ...
     * @param _projectOwner Adress of the project's owner
     */
    function initialize(
        address owner_,
        address _project,
    ...
    ) public initializer notZeroAddress(owner_) notZeroAddress(_project) notZeroAddress(_projectOwner) {
        __ERC165_init();
        __Adminable_init();
     ...
     */
    function addBudget(address _collectionAddress, uint256 _budget) external onlyProject {
    ...
    function addBudgetUse(address _collectionAddress, uint256 _amount) external onlyTaskManager {
        require(IProject(project).taskManager() == _msgSender(), "Caller is not permitted");
    ...
    function withdrawCollection(
        ...
        address _to,
        uint256 _amount
    ) external nonReentrant notZeroAddress(_collectionAddress) notZeroAddress(_to) notZeroAmount(_amount) onlyProject {
        require(
            collectionClaimPool[_collectionAddress] >= budgetInUseOf[_collectionAddress] &&
                collectionClaimPool[_collectionAddress] - budgetInUseOf[_collectionAddress] >= _amount,
        ...
        emit WithdrawnCollection(_collectionAddress, _to, _amount);
    }
     ...
     */
    function transferToReward(address _collectionAddress, uint256 _amount)
        external
        nonReentrant
        notZeroAddress(_collectionAddress)
        notZeroAmount(_amount)
        onlyTaskManager
    {
        require(
            budgetInUseOf[_collectionAddress] >= transferToRewards[_collectionAddress] &&
                budgetInUseOf[_collectionAddress] - transferToRewards[_collectionAddress] >= _amount,
            "Amount exceeds balance"
        );
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        TransferHelper._transferToken(paymentToken, totalAmount, address(this), projectOwner);
