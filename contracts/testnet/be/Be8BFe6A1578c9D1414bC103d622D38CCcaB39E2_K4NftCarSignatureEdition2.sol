...
contract K4NftCarSignatureEdition2 is ERC721, Ownable, ReentrancyGuard {
    ...
    function mintHotWalletUsingEther(
        uint256[] memory tokenId,
        uint256 quantity,
        address to
    ) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            if (tokenId[i] <= NFTTOTALSUPPLY && !_exists(tokenId[i])) {
                ...
                emit NFTMinted(to, tokenId[i], quantity, false, _CONTRACTID);
            }
        }
    }
    ...
    function addToWhitelist(address[] memory user) public onlyOwner {
        for (uint256 i = 0; i < user.length; i++) {
            require(!whitelist[user[i]], "User is already on the whitelist.");
            whitelist[user[i]] = true;
            hotWalletAddress.push(user[i]);
        }
    }

    function removeFromWhitelist(address[] memory user) public onlyOwner {
        address[] memory hotWalletAddresses = hotWalletAddress;
        for (uint256 i = 0; i < user.length; i++) {
            require(
                whitelist[user[i]],
                "User is already removed from the whitelist."
            );
            for (uint256 j = 0; j < hotWalletAddress.length; j++) {
                if (hotWalletAddress[j] == user[i]) {
                    delete hotWalletAddresses[j];
                    hotWalletAddresses[j] = hotWalletAddresses[
