  ...
  AccessControlEnumerable
{
  ...
  /* ----- Black/white list ----- */
  function addWhitelist(address proxy) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(whitelist[proxy] == false, "Cosmetic: already on whitelist");

    whitelist[proxy] = true;
  }

  function removeWhitelist(address proxy) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(whitelist[proxy], "Cosmetic: not on whitelist");

    whitelist[proxy] = false;
  }

  function addBlacklist(address proxy) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(blacklist[proxy] == false, "Cosmetic: already on blacklist");

    blacklist[proxy] = true;
  }

  function removeBlacklist(address proxy) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function setMintFactory(address factory) external {
    grantRole(MINTER_ROLE, factory);
  }

  function removeMintFactory(address factory) external {
    revokeRole(MINTER_ROLE, factory);
  }

  /* ----- Migrate ----- */
  function migrate(Tokens[] calldata tokens) external onlyRole(DEFAULT_ADMIN_ROLE) {
