...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  uint public discount; // decimal 3 // TODO remove when deploy mainnet
  ...
  function takeOrder(IMarketPlace.OrderKind _kind, uint _tokenId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    if (orderType == IMarketPlace.OrderType.trading) {
      ...
      revert("_markRentingOrderAsRented success");
    }
    if (currentPrice > 0) {
      _releaseFund(_kind, maker, currentPrice, _tokenId, _paymentCurrency);
      revert("_releaseFund success");
    }
    if (orderType == IMarketPlace.OrderType.trading) {
...
//        energyManager.updatePoint(msg.sender, 1);
  ...
  function setContracts(address _heroNft, address _itemNft, address _treasury, address _fotaToken, address _busdToken, address _usdtToken, address _fotaPricer, address _citizen, address _gameProxyContract, address _eatherTransporter, address _energyManager) external onlyMainAdmin {
