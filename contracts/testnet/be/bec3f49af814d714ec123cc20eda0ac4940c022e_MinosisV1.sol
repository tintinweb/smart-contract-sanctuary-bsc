...
contract MinosisV1 is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function _transfer(
     ...
     function distributeFee() internal {
        ...
        if(contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            // swap tokens in contract address to bnb
            ...
            // Send bnb to Development address
            ...
            if(buyBackFeeEnabled) swapETHForTokens(address(this), buyBackWalletAddress, (transferredBalance * buyBackFee) / totalSwapFee);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tSwapFee, uint256 tBurnFee) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rSwapFee) = getRValues(tAmount, tFee, tSwapFee, tBurnFee);
        {
            address from = sender;
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tSwapFee - tBurnFee;
        return (tTransferAmount, tFee, tSwapFee, tBurnFee);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tSwapFee, uint256 tBurnFee) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount - rFee - rSwapFee - rBurnFee;
    ...
    function removeAllFee() internal {
        if(taxFee == 0 && totalSwapFee == 0 && burnFee == 0) return;
