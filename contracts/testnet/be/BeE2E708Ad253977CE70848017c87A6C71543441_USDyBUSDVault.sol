...
/**
@dev USDY-BUSD vault
Note After deployment must set BNB and SwapPage address via setter to deposit BNB in vault.
 */
contract USDyBUSDVault is AccessControl, Initializable {

    IAdmin public Admin; //Admin  address
    address public USDs_lp; // USDs-LP token address
    uint256 public epoch = 8 hours; //epoch time
    uint256 public currentTimestamp; 
    uint256 public rebaseRate = 50;
    uint256 public depositTax = 10;
    uint256 public exchangeRate = 10 **18;//co-efficient 
    mapping(address=>uint256) public UserDeposit;
    IUniswapV2Pair public USDy_BUSD; // USDy-BUSD lp pair address
    IUniswapV2Router02 public router; //router address
 
    bytes32 public constant EPOCH_ROLE = keccak256("EPOCH_ROLE"); //role byte for admin
    ...
    @param _USDy_BUSD USDy-BUSD pair address
    @param _router router address
     */
    function initialize(address _admin,address owner,  address _USDy_BUSD, 
        address _router) external initializer {
        
        Admin = IAdmin(_admin);
        USDy_BUSD =  IUniswapV2Pair(_USDy_BUSD);
        router = IUniswapV2Router02(_router);
        USDs_lp = Clones.clone(Admin.masterNTT());
        INTT(USDs_lp).initialize(address(this), "USDs_lp", "USDs_lp");
    ...
    @param amountLp amount to deposit 
     */ 
   function deposit(address user, uint amountLp) external {
        require(amountLp > 0, "USDyBUSDVault: Amount can't be zero");
        // if(amountTokenA != 0){
        //  require(amountTokenB != 0, "USDyBUSDVault : You cannot deposit USDy in this vault");
        // }
        address[] memory path = new address[](2);
        path[0] = Admin.USDy();
        path[1] = Admin.BUSD();
        // require(USDy_BUSD.balanceOf(msg.sender) >= _amount,"USDyBUSDVault: insufficent balance");
        // if(!isReceipt){
        //     if( amountTokenB > 0 && amountTokenA == 0){
                
        //             IERC20(BUSD).transferFrom(user,address(this),amountTokenB);
        //             IERC20(BUSD).approve(address(router),(amountTokenB * 5/10));
        //             path[0] = BUSD;
        //             path[1] = address(USDy);
        //             uint amountA = IUniswapV2Router02(router).swapExactTokensForTokens((amountTokenB *5/10),1,path,address(this),block.timestamp + 1000)[1];
        //             USDy.burn(address(this), (amountA * 4/10));
        //             USDy.transfer(Treasury,(amountA * 6/10));
        //             uint amountB = IERC20(BUSD).balanceOf(address(this));
        //             IERC20(BUSD).transfer(Treasury, (amountB *4/10));
        //             IERC20(BUSD).approve(address(router),(amountB *6/10));
        //             USDy.approve(address(router),(amountA *6/10));
        //             (,,amountLp)=IUniswapV2Router02(router).addLiquidity(address(USDy),BUSD,(amountA *6/10),(amountB *6/10),1,1,address(this),block.timestamp + 1000);
        //     }
        //     else{
        //         if(amountTokenA > 0 && amountTokenB >0){
        //             USDy.transferFrom(user,address(this),amountTokenA);
        //             USDy.burn(address(this),(amountTokenA * 4/10));
        //             // USDy.transfer(Treasury,(amountTokenA * 6/10));
        //             IERC20(BUSD).transferFrom(user,address(this),amountTokenB);
        //             IERC20(BUSD).transfer(Treasury,(amountTokenB * 4/10));
        //             IERC20(BUSD).approve(address(router),(amountTokenB * 6/10));
        //             USDy.approve(address(router),(amountTokenA *6/10));
        //             (,,amountLp)=IUniswapV2Router02(router).addLiquidity(address(USDy),BUSD,(amountTokenA *6/10),(amountTokenB *6/10),1,1,address(this),block.timestamp + 1000);

        //         }
        //     }
        // }
        // else{
        USDy_BUSD.transferFrom(user,address(this),amountLp);
        USDy_BUSD.approve(address(router), amountLp * 4 / 10);
        (uint amountA, uint amountB) = router.removeLiquidity(Admin.USDy(), Admin.BUSD(), amountLp * 4 / 10, 1, 1, address(this), block.timestamp + 20 minutes);
        if(USDy_BUSD.token0() == Admin.USDy()){
            IUSDY(Admin.USDy()).burn(address(this), amountA);
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(), amountB);
        } else {
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(), amountA);
            IUSDY(Admin.USDy()).burn(address(this), amountB);
        }
        INTT(USDs_lp).mint(address(this), (amountLp * 90) / exchangeRate);
        exchangeRate = (IUSDY(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)) / (INTT(USDs_lp).totalSupply());
        UserDeposit[user] += (amountLp * (100 - depositTax) * (10 ** 18))/ (exchangeRate * 100);
    // }
   }
     ...
     */
    function withdraw(address user, bool isReciept, uint _amount) public {
        require(isReciept == false ,"USDyBUSDVault: User can only withdraw USDy");
        require(_amount <= UserDeposit[user],"USDyBUSDVault: Invalid Amount");
        IUSDY(Admin.USDy()).transfer(msg.sender,(_amount * exchangeRate) / 10** 18);
        INTT(USDs_lp).burn(address(this), _amount);
        UserDeposit[user]-=_amount;
        exchangeRate = (IUSDY(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)) / (INTT(USDs_lp).totalSupply());
    }

    // function swapLpToToken(address wantToken, uint lpAmount, bool isBNB) external {
    //     require((USDy_BUSD.balanceOf(msg.sender) > 0 || INTT(USDs_lp).balanceOf(msg.sender) > 0), 'USDyBUSDVault: You have zero LP tokens.');
    //     if((USDy_BUSD.balanceOf(msg.sender) == 0) && (INTT(USDs_lp).balanceOf(msg.sender) > 0)) {
    //         withdraw();
    //         require(false, 'USDyBUSDVault: LP tokens withdraw from vault succesfully. Deposit LP again');
    //     }
    //     USDy_BUSD.transferFrom(msg.sender, address(this), lpAmount); 
    //     USDy_BUSD.transfer(swapPage, lpAmount);
    //     ISwapPage(swapPage).getLPToToken(msg.sender, USDy_BUSD, wantToken, address(USDy), BUSD, lpAmount, isBNB);
    // }

// for testing epoch is 1 hour, for main net epoch is 8 hours
    function rebaseRewards() external onlyRole(DEFAULT_ADMIN_ROLE){
        require(block.timestamp >= 1 hours + currentTimestamp,"USDyBUSDVault : Rebase happens only after this epoch");
        uint mintReward = ((INTT(USDs_lp).totalSupply() * exchangeRate) * rebaseRate) / 100;
        IUSDY(Admin.USDy()).mint(address(this),mintReward);
        currentTimestamp = block.timestamp;
    }

    function setRebaseRateUSDyBUSD(uint256 _rate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_rate != 0, "USDyBUSDVault: Rate cannot be set to zero");
        rebaseRate = _rate;
    }
...
interface INTT {
    ...
    function totalSupply() external view returns (uint256);
    function balanceOf(address account) external view returns (uint256);
    function initialize(address operator, string memory name_, string memory symbol_) external;
}
