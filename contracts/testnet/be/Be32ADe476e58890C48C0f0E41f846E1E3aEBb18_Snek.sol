...
contract Snek is IERC20 {

  string public constant symbol = "SNEK";
  string public constant name = "Snek token";
  ...
  function setMinter(address _minter) external {
    require(msg.sender == minter, "SNEK: Not minter");
    minter = _minter;
  }

  function approve(address _spender, uint _value) external override returns (bool) {
    require(_spender != address(0), "SNEK: Approve to the zero address");
  ...
  function _mint(address _to, uint _amount) internal returns (bool) {
    require(_to != address(0), "SNEK: Mint to the zero address");
  ...
  function _transfer(address _from, address _to, uint _value) internal returns (bool) {
    require(_to != address(0), "SNEK: Transfer to the zero address");

    uint fromBalance = balanceOf[_from];
    require(fromBalance >= _value, "SNEK: Transfer amount exceeds balance");
  unchecked {
  ...
  function transferFrom(address _from, address _to, uint _value) external override returns (bool) {
    ...
    if (spenderAllowance != type(uint).max) {
      require(spenderAllowance >= _value, "SNEK: Insufficient allowance");
    unchecked {
