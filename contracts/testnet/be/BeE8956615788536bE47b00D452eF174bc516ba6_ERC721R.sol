...
contract ERC721R is IERC721R, UUPSUpgradeable, EIP712Upgradeable, ERC721PresetMinterPauserAutoIdUpgradeable {
  ...
  function mintFixed(address user, uint256 rarity, uint256 attributeId) external onlyRole(OPERATOR_ROLE) {
    uint256 tokenId;
    unchecked {
      tokenId = (++tokenIdTracker << 6) | (attributeId << 3) | rarity;
    }
  ...
  function createBox() external {
    ...
    uint256 attributeId;
    uint256 rarity;
    for (uint256 index = 0; index < ownerTokenCount;) {
      uint256 tokenId = tokenOfOwnerByIndex(user, index);
      attributeId = (tokenId >> 3) & (2 ** 3 - 1);
      rarity = tokenId & (2 ** 3 - 1);
      if(attributeId == _attributeId && count < _length){
        ++ count;
      }
      // if (attributeId == _attributeId && tokenIds[rarity] == 0 && count < _length) {
      //   tokenIds[rarity] = tokenId;
      //   ++count;
      // }
      // if(attributeId == _attributeId) {
      //   tokenIds[rarity] = tokenId;
      //   ++ count;
      // }
      unchecked {
    ...
    require(count == _length, "Not enough NFT to get box");
       
    // for (uint256 index = 0; index < _length;) {
    //   burn(tokenIds[index]);
    //   unchecked {
    //     ++index;
    //   }
    ...
    uint256 boxId = (++tokenIdTracker << 6) | (5 << 3) | 5;
    emit Test(_attributeId, _length);
    emit Unboxed(user, boxId, 5, 5);
  }
...
interface IERC721R {
  ...
  event Test(uint256 indexed attributeId, uint8 indexed rarity);
