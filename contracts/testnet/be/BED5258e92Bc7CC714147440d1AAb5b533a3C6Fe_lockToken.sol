...
contract lockToken is owned{
    ...
    */
    function createMultipleLocks(address _tokenAddress, address _withdrawalAddress, uint256[] _amounts, uint256[] _unlockTimes) public returns (uint256 _id) {
        require(_amounts.length > 0,'token amount is Zero');
        require(_amounts.length == _unlockTimes.length,'amounts must equal unlockTimes');
        ...
        for(i=0; i<_amounts.length; i++){
            require(_amounts[i] > 0,'token amount is Zero');
            require(_unlockTimes[i] < 10000000000,'Enter an unix timestamp in seconds');
            
            //require(Token(_tokenAddress).approve(this, _amounts[i]), 'Approve tokens failed');
            //require(Token(_tokenAddress).transferFrom(msg.sender, this, _amounts[i]), 'Transfer of tokens failed');
    ...
    */
    function withdrawTokens(uint256 _id) public {
        require(block.timestamp >= lockedToken[_id].unlockTime, 'Tokens are locked, unlockTime not meet');
