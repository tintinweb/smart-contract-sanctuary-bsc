...
// 盲盒代币交易
contract TokenExchange is ITokenExchange, Caller, Fallback {
    ...
    function make(
        ...
        address token, //盲盒代币合约
        uint256 amount, //盲盒代币数量
        address quoteToken, //报价代币合约
        uint256 quoteAmount, //报价代币总包数量
        uint256 ts //结束时间
    ) external override {
        require(tokens.containsAddressSet(token), "1181");
        ...
        require(0 < amount, "1175");
        config.getApproveProxy().transferFromERC20(
            token,
            msg.sender,
    ...
    function delivery_(uint256, OrderInfo memory order_) private {
        ...
        asset.claimMainOrERC20(
            order_.quoteToken,
            config.getNFTCreator(),
            creatorFee
        );
        asset.claimMainOrERC20(order_.quoteToken, order_.maker, makerFee);
        asset.claimMainOrERC20(order_.token, order_.bidder, order_.amount);
    }
