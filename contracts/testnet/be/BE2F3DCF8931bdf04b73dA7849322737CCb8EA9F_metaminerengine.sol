...
contract metaminerengine is Context, IBEP20, Ownable {
  ...
  mapping (uint256 => uint256) private _Struct_Miner_Stamina;
  ...
  function uplevel() public returns (bool) {
    ...
    _Struct_Miner_PickaxeUp[_minerId[msg.sender]].add(1);
    return true;
  }
  ...
  function mintMiner(bool ingot) public returns (uint256) {
    ...
    if ( ingot == true ) {
        ...
        _getrarity();     
        if ( _rarity >= _Struct_Miner_Rarity[_minerId[msg.sender]] ) {
            _previousCoral(msg.sender);
            _newMiner(_rarity,msg.sender);
            _minerId[msg.sender] = _MinerCount;
            return _Struct_Miner_Rarity[_minerId[msg.sender]];
        } else {
            _previousCoral(msg.sender);
            _newMiner(_rarity,msg.sender);
  ...
  function mining() public returns (bool) {
    ...
    uint256 mine = _currentcoral(msg.sender);
  ...
  function claim() public returns (bool) {
    ...
    a.transferFrom(address(this),msg.sender,claimtoken);
  ...
  function updateGameEcosystem(uint256 chest,uint256 pickaxe,uint256 miner,uint256 uplv,uint256 marketfee,uint256 lv,uint256 rw,uint256 rate,bool mintable,uint256 cap,bool market,uint256 minclaim) public onlyOwner returns (bool) {
    _mintfee_chest = chest;
    _mintfee_pickaxe = pickaxe;
    _mintfee_miner = miner;
    _mintfee_uplevel = uplv;
    _mintfee_market = marketfee;
    _maxlevel = lv;
    _rewardrate = rw;
    _goldmintrate = rate;
    _goldmintable = mintable;
    _chestcap = cap;
    _marketplace = market;
    _minimalclaim = minclaim;
    return true;
  }
  ...
  function staminaOf(uint256 id) external view returns (uint256) {
    require( id > 0,"BEP20: nft id out of range");
    require( id < _MinerCount,"BEP20: nft id out of range");
    return _Struct_Miner_Stamina[id];
  }

  function updateStamina(uint256 id,uint256 amount) public onlyOwner returns (bool) {
    require( id > 0,"BEP20: nft id out of range");
    require( id < _MinerCount,"BEP20: nft id out of range");
    _Struct_Miner_Stamina[id] = amount;
    return true;
  }
  ...
  function NftUI(uint256 id,uint256 layer) external view returns (uint256) {
  ...
  function random() internal returns (uint256) {
    uint256 randomnumber = uint(keccak256(abi.encodePacked(now, msg.sender, nonce))) % 9;
  ...
  function _newMiner(uint256 newRarity,address newOwner) internal {
      ...
      _Struct_Miner_Stamina[_MinerCount] = block.timestamp;
      ...
      _Struct_Miner_Layer_1[_MinerCount] = random().add(newRarity);
      _Struct_Miner_Layer_2[_MinerCount] = random().add(newRarity);
      _Struct_Miner_Layer_3[_MinerCount] = random().add(newRarity);
      _Struct_Miner_Layer_4[_MinerCount] = random().add(newRarity);
  }
