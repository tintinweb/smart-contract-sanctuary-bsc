...
contract SportFantansyPrediction is Ownable{
    ...
    event MatchCreated(uint256 matchId, string stTeam,string stLogo, string ndTeam, string ndLogo, uint256 startTime);
    event MatchResulted(uint256 matchId,string stTeam,string stLogo, string ndTeam, string ndLogo, uint256 startTime,bool isEnded, uint256 result);
    ...
    constructor(){}
     ...
     function addMatch(uint256 _startTime,string memory _stTeam,string memory _stLogo,string memory _ndTeam, string memory _ndLogo) public onlyOwner{
        require(_startTime > block.timestamp, "time error");
        
        uint256 matchId = matchCount;

        _matches.push(Match(matchId,_stTeam,_stLogo,_ndTeam,_ndLogo,false,_startTime, 0));

        matchCount = matchCount +1;
        currentMatchCount = currentMatchCount +1;

        emit MatchCreated(matchId,_stTeam,_stLogo,_ndTeam,_ndLogo,_startTime);
    }
    ...
    function updateResult(uint256 _matchId,uint256 _stResult,uint256 _ndResult)public onlyOwner{
        ...
        emit MatchResulted(_matchId, _matches[_matchId].stTeam, _matches[_matchId].stLogo,_matches[_matchId].ndTeam, _matches[_matchId].ndLogo, _matches[_matchId].startTime, _matches[_matchId].isEnded, _matches[_matchId].result);
    }
    ...
    function isMatchStarted(uint256 _matchId) external view returns(bool) {
        for(uint256 i = 0; i< _matches.length; i++){
            if(_matches[i].matchId == _matchId){
                return block.timestamp > _matches[i].startTime ;
            }
        }
        revert("not found matchId");
    }

    function isMatchEnded(uint256 _matchId) external view returns(bool) {
        for(uint256 i = 0; i< _matches.length; i++){
            if(_matches[i].matchId == _matchId){
                return _matches[i].isEnded;
            }
        }
        revert("not found matchId");
    }
