...
contract wMarketplace is Ownable, ReentrancyGuard {
  ...
  function getRate() public view returns (MarketRate memory) {
    ...
    //MarketRate memory rate = _getValidRate(msg.sender);

    //return (rate.isActive, rate.listingPrice, rate.cancelPrice, rate.feePercent);
  ...
  function getActiveMarketItemsCount() public view returns (uint256) {
    ...
    for (uint256 i = 0; i < itemsCount; i++) {
      ...
      if (_items[i + 1].status == MarketItemStatus.Active)
  ...
  function findActiveMarketItem(address tokenContract, uint256 tokenId) public view returns (uint256){
    ...
    for (uint256 i = 0; i < itemsCount; i++) {
      ...
      if (_items[i + 1].status == MarketItemStatus.Active && _items[i + 1].tokenContract == tokenContract && _items[i + 1].tokenId == tokenId)
  ...
  function getCustomRate(address adr) public view onlyOwner returns (MarketRate memory){
    ...
    // (bool isActive, uint256 listingPrice, uint256 cancelPrice, uint feePercent) = _getRate(adr);

    // return (isActive, listingPrice, cancelPrice, feePercent);
    ...
    // MarketRate memory rate = _getRate(adr);
  ...
  function removeRate(address adr) public onlyOwner {
  ...
  function _getRate(address adr) private view returns (MarketRate memory) {

    return _rates[adr];

    //return (_rates[adr].isActive, _rates[adr].listingPrice, _rates[adr].cancelPrice, _rates[adr].feePercent);
...

  {
    ...
    if (_rates[adr].isActive){
      ...
      // (isActive, listingPrice, cancelPrice, feePercent) = _getRate(adr);
    ...
    //(isActive, listingPrice, cancelPrice, feePercent) = _getRate(address(0));
