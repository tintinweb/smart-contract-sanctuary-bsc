...
contract BoxVRF2 is VRFV2WrapperConsumerBase, ConfirmedOwner {
    ...
    uint256 public nftOddsFixed;

    event GetBox(address indexed user);
    ...
    function setBoxInfo(
        ...
        boxPrice = _boxPrice;
        nftOdds = _nftOdds;
        nftOddsFixed = ((~uint256(0)) / 100) * nftOdds;
    }

    function setVRFInfo(uint32 _callbackGasLimit, uint16 _requestConfirmations)
        external
        onlyOwner
    {
    ...
    function get() external {
        ...
        //transfer VNS
        require(IERC20(vnsAddress).balanceOf(address(this))>boxPrice*100,"not enough vns");
        require(IERC20(vnsAddress).transfer(msg.sender,boxPrice*100),"transfer vns failed");
        
        //nft
        if (requests[userRequestId[msg.sender]].randomWords < nftOddsFixed) {
            //mint nft
            IVNSNFT(nftAddress).blindBoxTo(msg.sender);
            emit GetBox(msg.sender);
        }
