...
contract Farming_BUSD { //change with pair-name
    ...
    function Stake(uint256 _amount, address _wallet, uint plan) onlyOwner public{
    ...
    function stake(uint _amount, uint plan,address _refAdrress) external updateReward(msg.sender) {
        // require(plan<6,"wrong plan");
        uint256 _endtime = 62208000;
        
        if(plan == 0){
            _endtime = 62208000; //7D 604800 test 1
        }
        else if(plan == 1){
        ...
        RefAddress[msg.sender] = _refAdrress;
        uint256 devfee = 0;
        devfee = _amount*6/100;
    ...
    function withdraw() external updateReward(msg.sender) { // unstake all
         uint256 _amount;
        ...
        if(stakingToken.balanceOf(address(this))>_balances[msg.sender]){  // if pool have staking token larger than balance user stake
            if((block.timestamp - stakeStart[msg.sender]) < stakeEnd[msg.sender]){
                uint _amountToWithdraw = _amount - (_amount*100 / 101); // penalty 99
                _totalSupply -= _amount;
                _balances[msg.sender] = 0;
                stakingToken.transfer(msg.sender, _amountToWithdraw);
                stakingToken.transfer(dev_fee_wl, _amountToWithdraw*26); // fee 26% on total user depo
                ...
                stakingToken.transfer(dev_fee_wl, _amount*1/100);
                emit WitdrawStaked(msg.sender, _amount, block.timestamp, false);
            }
        }
        else{
                _balances[msg.sender] = 0; // balences = 0
    ...
    function getReward() external updateReward(msg.sender) {
        ...
        if((block.timestamp - stakeStart[msg.sender]) < lockedTime){
            ...
            uint reward_ref = reward*20/100; // for ref reward
            rewardsToken.transfer(RefAddress[msg.sender], reward_ref); // for ref reward
    ...
    function getAvailableRUserReward() external view returns(uint){
        return ((_balances[msg.sender] *
                (rewardPerToken() - userRewardPerTokenPaid[msg.sender])) / 1e18) +
            rewards[msg.sender];
    }
    
    function getRefAvailableUserReward() external view returns(uint){
        return (((_balances[msg.sender] *
                (rewardPerToken() - userRewardPerTokenPaid[msg.sender])) / 1e18) +
            rewards[msg.sender])*20/100;
    }
