...
contract Trade {
    ...
    event Assetmint(address indexed from,uint256[] indexed tokenIds);
    ...
    mapping (uint256 => TokenRarityType) public tokenId2Rarity ;
    mapping (uint256 => TokenRarityType) public tokenType2Rarity ;
    ...
    function verifySign(uint256 tokenLength,uint256 seed,Sign memory sign) internal view  {
        ...
        require(owner == getSigner(hash, sign),"Sign: owner sign verification failed");
    }
    ...
    function getNumber(uint256 salt) internal returns(uint256[] memory){
    ...
    function getCategory(uint256 tokenId) internal pure returns(uint256) {
        ...
        else if((tokenId >=11401)&&(tokenId<=16400)){
            return 1;
        }
        else if((tokenId >=16401)&&(tokenId<=19000)){
            return 2;
        }
        else if((tokenId >=19001)&&(tokenId<=19920)){
            return 3;
        }
    ...
    function getTokenType(uint256[] memory tokenIds) internal pure returns(uint256[] memory){
        uint256[] memory tokenRarity = new uint256[](10);
    ...
    function mint(address nftaddress, uint256 tokenLength, uint256 seed, Sign memory sign) public returns(uint256[] memory){
        // uint256[]  memory tokenid = new uint256[](10);
        uint256[] memory tokenRarity = new uint256[](10);
