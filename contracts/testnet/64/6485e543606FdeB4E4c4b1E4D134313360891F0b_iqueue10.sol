...
contract iqueue10 is IERC20, Ownable {
    ...
    string constant _name = "testing contract 09";
    string constant _symbol = "test 09";
    ...
    address public buyBackFeeWallet;
    ...
    constructor() {
        ...
        devFeeWallet = 0xbb83e670b41C4df06968Afd81b55A71A24D97dbf;
        buyBackFeeWallet = 0xb6658AE3BD78a03403c0b6989E9c1083c1cf5907;

        _allowedFragments[address(this)][address(router)] = type(uint256).max;
        pairContract = IUniswapV2Pair(pair);

        isAuthorized[buyBackFeeWallet] = true;

        _totalSupply = INITIAL_FRAGMENTS_SUPPLY;
        _gonBalances[buyBackFeeWallet] = TOTAL_GONS;
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _initRebaseStartTime = block.timestamp;
        _lastRebasedTime = block.timestamp;
        _autoRebase = true;
        _autoAddLiquidity = true;
        _isFeeExempt[buyBackFeeWallet] = true;
        _isFeeExempt[address(this)] = true;

        swapThreshold = (_totalSupply * 10) / 10000;

        // _transferOwnership(treasuryReceiver);
        emit Transfer(address(0x0), buyBackFeeWallet, _totalSupply);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 tokensToBuyBack = swappedBnb.mul(buyBackSwap).div(
            buyBackSwap.add(devSwap)
        );

        uint256 tokensToDev = swappedBnb.sub(tokensToBuyBack);

        if (tokensToBuyBack > 0 && buyBackSwap > 0) {
            payable(buyBackFeeWallet).transfer(tokensToBuyBack);
        }

        if (tokensToDev > 0 && devSwap > 0) {
            payable(devFeeWallet).transfer(tokensToDev);
        }
    ...
    function updateBuyFees(
        ...
        buyTotalFee = burn.add(buyBack).add(liquidity).add(dev);

        require(buyTotalFee <= 25, "Fees can not be greater than 25%");
    }
    ...
    function updateSellFees(
        ...
        sellTotalFee = burn.add(buyBack).add(liquidity).add(dev);

        require(sellTotalFee <= 25, "Fees can not be greater than 25%");
    }
    ...
    function updateSwapPercentages(
        ...
        totalSwap = buyBack.add(liquidity).add(dev);
    }
    ...
    function setFeeReceivers(address _buyBack, address _dev)
        external
        onlyOwner
    {
        buyBackFeeWallet = _buyBack;
        devFeeWallet = _dev;
    }
