...
contract XXXXXX {
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        uint256 initialAmount = depositAmount;
		if (user.deposits.length == 0) {
			user.checkpoint = block.timestamp;
            userCount += 1;
            if(userCount <= USERS_COUNT_MAX && user.referrer != address(0)){
                initialAmount = initialAmount.add(initialAmount.mul(USERS_COUNT_BONUS).div(PERCENTS_DIVIDER));
            }
			emit Newbie(msg.sender);
		}

		(uint256 percent, uint256 profit, uint256 finish) = getResult(plan, depositAmount);
		user.deposits.push(Deposit(plan, percent, initialAmount, depositAmount, profit, block.timestamp, finish, block.timestamp, 0));
	...
	function withdraw(uint256 depositId) public {
        ...
        if(user.deposits[depositId].plan != 1){
            ...
            if(user.deposits[depositId].plan == 0){
                withdrawFee = WITHDRAW_FEE1[planDay];
            } else{
                withdrawFee = WITHDRAW_FEE2[planDay];
            }
    ...
    function compound(uint256 depositId) public {
		...
		user.deposits.push(Deposit(depositPlan, percent, totalAmount, totalAmount, profit, block.timestamp, finish, block.timestamp, 0));
    ...
    function _updateDepositorRank(address _user) private {
		if(depositorWinners.length == 0){
			depositorWinners.push(_user);
		} else if(dayInfo[prizeCycle][_user].depositAmount > dayInfo[prizeCycle][depositorWinners[prizeCycle]].depositAmount){
            depositorWinners[prizeCycle] = _user;
        }
	}

    function _updateLeaderRank(address _user) private {
		if(leaderWinners.length == 0){
			leaderWinners.push(_user);
		} else if(dayInfo[prizeCycle][_user].leaderAmount > dayInfo[prizeCycle][leaderWinners[prizeCycle]].leaderAmount){
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 initialAmount, uint256 finalAmount, uint256 profit, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
