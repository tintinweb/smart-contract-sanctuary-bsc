    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    function setBaseURI(uint8 _tier, string memory _baseURI)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        baseURIByTier[_tier] = _baseURI;
    }
    ...
    function lootBoxMint(
        address _to,
        uint8 _tier,
        SharedStructs.SpaceVehicleTierConfig memory _spaceVehicleConfig
    ) public onlyRole(MINTER_ROLE) {
        uint256 _tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(_to, _tokenId);
        emit Minted(_to, _tokenId);
        vehiclesInfoMap[_tokenId].typeId = _spaceVehicleConfig
            .spaceVehicleTypeId;
        vehiclesInfoMap[_tokenId].typeName = _spaceVehicleConfig
            .spaceVehicleTypeName;
        vehiclesInfoMap[_tokenId].collectionId = _spaceVehicleConfig
            .collectionId;
        vehiclesInfoMap[_tokenId].collectionName = _spaceVehicleConfig
            .collectionName;
        vehiclesInfoMap[_tokenId].firstOwner = _to;
        _setTokenURI(_tokenId, baseURIByTier[_tier]);
    }
