...
constructor() {
    ...
    function Take_chest (uint256 num_Track, uint256 chest_num) public {
       address sender = msg.sender;
        ...
        if(chest_num>=4){
            address addressNFTWA;
            uint256 ID_NFTWA;
                addressNFTWA = Coll.GetID_Collection(NumColl_NFTWA);
                ID_NFTWA = CNF.Get_NFTWA_num(NumColl_NFTWA);
                NFTWA_get=INFTWA(address(addressNFTWA));
                
                if(NFTWA_get.get_Total_NFTWA()+1<=Coll.Get_Max_id_Collection(ID_NFTWA)) {
                     NFTWA_TAKE (sender,addressNFTWA);
                }
                else {
                    NumColl_NFTWA++;
                    addressNFTWA = Coll.GetID_Collection(NumColl_NFTWA);
    ...
    function NFTWA_TAKE (address adr, address addressNFTWA_) private {
        NFTWA_get=INFTWA(address(addressNFTWA_));
        NFTWA_get.Mint_NFTW(adr);
    }
