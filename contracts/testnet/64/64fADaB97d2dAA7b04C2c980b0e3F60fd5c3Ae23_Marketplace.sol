...
contract Marketplace is ReentrancyGuard {
    ...
    /**
     * @dev Creates a market item listing, requiring a listing fee and transfering the ISTEPBoxNft token from
    ...
    function createMarketItem(
        ...
        address creator = ISTEPBoxNft(nftContractAddress).getTokenCreatorById(tokenId);
    ...
    /**
     * @dev Creates a market sale by transfering msg.sender money to the seller and ISTEPBoxNft token from the
...
contract ISTEPBoxNft is Context, AccessControlEnumerable, ERC721Enumerable, ERC721Burnable {
    ...
    constructor(address runNftAddress, address _marketplaceAddress) ERC721("ISTEP Box NFT", "ISTEPBN") {
        ...
        runNft = InterfaceISTEPNft(runNftAddress);
    ...
    function setBaseURI(string memory baseTokenURI) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ISTEPBoxNft admin role required");
        _baseTokenURI = baseTokenURI;
    }

    function setupMinterRole(address account, bool _enable) external {
        require(account != address(0), "ISTEPBoxNft account cann't equal address 0x");
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ISTEPBoxNft admin role required");
        if (_enable) {
    ...
    function mint(address toAddress, uint256 boxType) external returns (uint256) {
        require(hasRole(MINTER_ROLE, _msgSender()), "ISTEPBoxNft minter role required");
        require(toAddress != address(0), "ISTEPBoxNft cann't mint zero address");
        require(boxType > 0, "ISTEPBoxNft boxType must greater than 0");
    ...
    function openBox(uint256 tokenId) external {
        require(allowOpenBox, "ISTEPBoxNft open box is not available now");

        // check owner of tokenId
        require(ERC721.ownerOf(tokenId) == msg.sender, "ISTEPBoxNft sender is not owner of tokenId");
