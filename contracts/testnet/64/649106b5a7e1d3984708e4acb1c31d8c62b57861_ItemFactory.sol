...
// File contracts/interface/IMilkPartial.sol


pragma solidity ^0.8.0;

interface IMilkPartial {
...
pragma solidity ^0.8.0;
contract ItemFactory is ERC1155,Ownable  {
    ...
    mapping(address => LootData) public lootData;
    ...
    constructor(address milkContractAddress,string memory uri) ERC1155(uri) {
    ...
    function claim(address claimer, uint256 entropy) external timeChecker(claimer) {

        // generate a single random number and bit shift as needed
        rNum = randomNum(entropy);

        // roll and pick the rarity level of the reward
        uint256 randRarity = rNum % _legendaryRoll;
    ...
    //     // always a box
        ...
        // handle MILK or ITEMS
            ...
            rewardType = rNum % uint256(EType.BOX);

            // convert the reward mapping data to min and max
            (min,max,ids) = abi.decode(
                _rewardMapping[rewardType][rewardRarity], (uint256, uint256, uint256[])
            );
            
            // // do some bit shifting magic to create random min max
            uint256 rewardAmount = lootData[claimer].min + (rNum) % (lootData[claimer].max - lootData[claimer].min + 1);

            // Give a MILK reward
            if (rewardType == uint256(EType.MILK)) {
                IMilkPartial(_milkContractAddress).gameMint(claimer, rewardAmount);
                rewardData = abi.encode(rewardAmount);
            }

            // // Give an item reward
            else {
                uint256 index = (rNum) % lootData[claimer].ids.length;
                _mint(claimer, lootData[claimer].ids[index], rewardAmount, "");
                rewardData = abi.encode(lootData[claimer].ids[index], rewardAmount);
            }
        }
    ...
    function randomNum(uint256 entropy) public view returns (uint256) {
    ...
    function setReward(uint256 rewardType, uint256 rewardRarity, bytes calldata rewardData) external onlyOwner {
        (min,  max,  ids) = abi.decode(
    ...
    modifier timeChecker(address _claimer) {
      uint256 _timeCheck =  _lastUpdate[_claimer];
      require(_timeCheck > 86400,"Claime:You can claim after 24 hours");
      _;
   }
}