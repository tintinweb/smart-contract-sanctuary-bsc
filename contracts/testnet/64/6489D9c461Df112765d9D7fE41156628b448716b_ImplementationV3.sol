...
contract ImplementationV3 is ImplementationV2{
    ...
    */
    function rewardPerToken(bytes memory poolId) public view returns (uint256) {
        ...
        uint256 rewardPool = pool.rewardFund * (currentTimestamp - pool.lastUpdateTime) * 1e18;
    ...
    function apy(bytes memory poolId) public view returns (uint256){
        return rewardPerToken(poolId) * ONE_YEAR_IN_SECONDS;
    }
...
contract ImplementationV2 is ImplementationV1 {
     ...
     * @param (0) tokenId, (1) price, (2) expTime 
    ...
    /**
     * @dev handle rent 
     * @param (0) tokenId, (1) totalPrice, (2) expDate,
     * @param (0) signer, (1) tokenAddress, (2) renter 
     * @param (0) rentId
     * @param (0) rentRequestSignature
     */
    function handleRent(
        uint256[] memory ,
        address[] memory ,
...
contract Upgradeable is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    event StakingEvent(
        ...
        bytes poolId,
        bytes internalTxID,
        uint256 eventType // (0) stake, (1) unstake, (2) claimReward
    );
