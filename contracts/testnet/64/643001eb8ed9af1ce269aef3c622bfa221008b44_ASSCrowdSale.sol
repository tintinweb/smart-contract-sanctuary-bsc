...
contract ASS is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0x29837FdAFD5F3520c27c43251cB4070f305679BA;
...
library SafeERC20 {
    ...
    function safeTransfer(ASS token, address to, uint256 value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transfer.selector, to, value));
    }

    function safeTransferFrom(ASS token, address from, address to, uint256 value) internal {
        callOptionalReturn(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    function safeApprove(ASS token, address spender, uint256 value) internal {
    ...
    function safeIncreaseAllowance(ASS token, address spender, uint256 value) internal {
        uint256 newAllowance = token.allowance(address(this), spender).add(value);
        callOptionalReturn(token, abi.encodeWithSelector(token.approve.selector, spender, newAllowance));
    }

    function safeDecreaseAllowance(ASS token, address spender, uint256 value) internal {
     ...
     */
    function callOptionalReturn(ASS token, bytes memory data) private {
...
contract CROWDSALE is Context, ReentrancyGuard {
     ...
     */
    constructor ( uint256 rate, address payable wallet, ASS token)  {
     ...
     */
    function token_() public view returns (ASS) {
        return _token;
    }
    ...
    function buyTokens(address beneficiary) public nonReentrant payable {
        ...
        uint256 actualTokenAmount = (_getTokenAmount(weiAmount)).div(10**9);


        // update state
        _weiRaised = _weiRaised.add(weiAmount);

        _processPurchase(beneficiary, actualTokenAmount);
        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, actualTokenAmount);
...
contract AllowanceCrowdsale is CROWDSALE {
     ...
     */
    constructor (address tokenWallet,uint256 rate, address payable wallet, ASS token) CROWDSALE(rate,wallet,token)   {
...
contract ASSCrowdSale is AllowanceCrowdsale,Ownable {
    using SafeMath for uint256;
    ...
    constructor(address tokenWallet,uint256 rate, address payable wallet, ASS token,uint256 minimumContribution,uint256 maxContribution) AllowanceCrowdsale(tokenWallet,rate,wallet,token)  {
