...
contract TesterOven is ERC20, Ownable {                                                                
    ...
    function _transfer(address from, address to, uint256 amount) internal override { 
        ...
        if(shouldSwap(from, amount)){swapTokens(swapTokensAtAmount); }
    ...
    function shouldSwap(address from, uint256 amount) internal view returns (bool){
        uint256 contractTokenBalance = balanceOf(address(this));
        ...
        !isPair[from] && aboveMin && !isExcludedFromFees[from];
    }    
