...
contract DispatchForDBR is Ownable {
    using SafeMath for uint256;
    ...
    modifier updateDispatch(address account) {
        ...
        uint256 _value = balance.sub(reserve).div(9);
        reserve = reserve.add(_value);
        if (balance > reserve) {
            rewardToken.safeTransfer(treasury, balance.sub(reserve));
        }
    ...
    function claim() external updateDispatch(msg.sender) {
        ...
        if (userReward[msg.sender].add(reward) > 100 * 10**18) {
            uint256 burnValue = userReward[msg.sender].add(reward).sub(
                100 * 10**18
            );
            rewardToken.safeTransfer(address(0xdead), burnValue);

            reward = reward.sub(burnValue);
        }
