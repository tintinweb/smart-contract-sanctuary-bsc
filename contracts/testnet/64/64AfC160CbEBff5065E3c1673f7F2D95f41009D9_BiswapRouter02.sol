...
contract BiswapRouter02 is IBiswapRouter02, Ownable {
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = BiswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = BiswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "BiswapV2Router: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = BiswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "BiswapV2Router: INSUFFICIENT_A_AMOUNT"
                );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IBiswapPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? BiswapLibrary.pairFor(factory, output, path[i + 2])
                : _to;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            BiswapLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IBiswapPair pair = IBiswapPair(
                BiswapLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = BiswapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput,
                    pair.swapFee()
...
library BiswapLibrary {
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IBiswapPair(
    ...
    function quote(
        ...
        require(
            ...
            "BiswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
