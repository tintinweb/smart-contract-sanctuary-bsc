...
contract LPMiningIII is Ownable {
    ...
    function getPool(uint pid_) public view returns(
                                                        bool _status,
                                                        uint _endTime,
                                                        string memory t0, 
                                                        string memory t1, 
        ...
        address lpAddr = address(poolInfo[pid_].lpToken);
        address _t0 = IPair(lpAddr).token0();
        address _t1 = IPair(lpAddr).token1();
        _status = pool.status;
        _endTime = pool.endTime;
        t0 = IBEP20(_t0).symbol();
        t1 = IBEP20(_t1).symbol();
        output = IBEP20(poolInfo[pid_].outPutToken).symbol();
        userClaimed = userInfo[pid_][msg.sender].claimed;
    }
    ...
    function deposit(uint256 pid_, uint256 amountIn_, address inv_) public {
        ...
        require (pool.status || block.timestamp < pool.endTime, 'deposit no good, status');
    ...
    function addPool(IPair lpToken_, IBEP20 outPutToken_, uint dailyOut_, uint endTime_, bool withUpdate_) public onlyOwner {
