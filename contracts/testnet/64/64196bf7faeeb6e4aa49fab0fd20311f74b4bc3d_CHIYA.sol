...
contract CHIYA is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingWalletAddress = payable(0x0cF656f56cB646297EE1728D8877007845DA35ce); // Marketing Address
    address payable public treasuryWalletAddress = payable(0xc713a94F3eA0127F581f8c51BA759b612068282d); // treasury Walletaddress
    ...
    string private _name = "CHIYA";
    string private _symbol = "CHIYA";
    ...
    constructor () public  {
        ...
        _totalLiqFee = _liquidityFee.add(_marketingFee).add(_treasuryFee);
    ...
    function setTaxes(uint256 newRewardFee, uint256 newLiquidityFee, uint256 newMarketingfee, uint256 newTreasuryFee, uint256 newBurnFee) external onlyOwner() {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(forWallets > 0 && _marketingFee.add(_treasuryFee) > 0)
        {
            uint256 initialBalance = address(this).balance;
            swapTokensForEth(forWallets);
            uint256 newBalance = address(this).balance.sub(initialBalance);
    
            uint256 marketingShare = newBalance.div(_marketingFee.add(_treasuryFee)).mul(_marketingFee);
            ...
            if(techShare > 0)
                transferToAddressETH(treasuryWalletAddress, techShare);
        }
    }
