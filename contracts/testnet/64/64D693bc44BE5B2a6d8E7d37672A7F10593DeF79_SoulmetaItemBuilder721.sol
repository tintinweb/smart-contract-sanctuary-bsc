...
contract SoulmetaItemBuilder721 is Ownable, ERC721Enumerable, ERC721URIStorage{
    // for inherit
    ...
    function mint (address player_, uint tokenId_) public returns (bool) {
        ...
        _setTokenURI(tokenId_, tokenId_.toString());
    ...
    function tokenURI(uint256 tokenId_) override(ERC721URIStorage, ERC721) public view returns (string memory) {
        require(_exists(tokenId_), "F: nonexistent token");
        return string(abi.encodePacked(_myBaseURI(), "/", super.tokenURI(tokenId_)));
    }
