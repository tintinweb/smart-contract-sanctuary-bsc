...
contract SwapContract is Ownable {
    ...
    uint256 public amountBToken;
   	constructor() {
        ...
        pair2 = IPancakeSwapFactory(router.factory()).getPair(usct, stb);
        if (pair2 == address(0)) {
    ...
    function removeLiquidity(uint256 amount) private returns (uint256, uint256){
        IPancakeSwapPair(pair).approve(address(this), amount);
    ...
    function buySTB1(uint256 amount) public {
        require(amount > 0, "zero");
        _usdcContract.transferFrom(msg.sender, address(this), amount);
    ...
    function buySTB2(uint256 amount) public {
        require(amount > 0, "zero");
        _usdcContract.transferFrom(msg.sender, address(this), amount);
        ...
        _stbContract.approve(address(router), amount);
        stbAmount = router.swapExactTokensForTokens(amount, 0, path, address(this), block.timestamp);
        amountAToken = stbAmount[0];
        amountBToken = stbAmount[1];
    }
    function buySTB3(uint256 amount) public {
        require(amount > 0, "zero");
        _usdcContract.transferFrom(msg.sender, address(this), amount);
        
        _usctContract.mint(msg.sender,  tax / 100);

        _stbContract.transfer(msg.sender,  (100 - tax) / 100);
        _usctContract.mint(msg.sender,  tax / 100);
        createLiquidity(tax / 100);
    }
