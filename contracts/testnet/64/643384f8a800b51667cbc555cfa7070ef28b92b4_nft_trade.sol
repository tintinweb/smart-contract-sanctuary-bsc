...
interface minter {
    ...
    function tokenType(uint _tokenID) external view returns(uint);
}
...
contract nft_trade is ERC721Holder, ERC1155Holder, Ownable, Pausable {
    using Counters for Counters.Counter;
    Counters.Counter private lastOfferId;

    IERC20 public usdtTokenContract;
    address public feeReceiverContract;
    ISafeTransfer public inHouseNftAddress;
    
    uint128 public fee = 30;
    uint128 public minTradePrice = 100 * 10 ** 18;
    uint128 public maxTradePrice = 340000000000000000000 * 10 ** 18;
    ...
    event minTradePriceChanged (
        uint128 prevPrice,
        uint128 newPrice
    );

    event maxTradePriceChanged (
        uint128 prevPrice,
        uint128 newPrice
    );
    ...
    function addSupportedContracts(ISafeTransfer _contract, bool _isItErc1155) public onlyOwner {
        supportedContracts[_contract] = _isItErc1155 ? 2 : 1;
        emit AddedSupportedContracts(_contract, _isItErc1155);
    }

    function setUSDTTokenContract(IERC20 _usdtTokenContract) public onlyOwner returns(bool)
    {
        usdtTokenContract = _usdtTokenContract;
        return true;
    }

    function setInHouseNftAddress(ISafeTransfer _inHouseNftAddress) public onlyOwner returns(bool) {
        inHouseNftAddress = _inHouseNftAddress;
        return true;
    }

    function setFeeReceiverContract(address _feeReceiverContract) public onlyOwner returns(bool)
    {
        feeReceiverContract = _feeReceiverContract;
        return true;
    }
    ...
    function setminTradePrice(uint128 _price) external onlyOwner {
        require(_price >= 0, "CANNOT_BE_BELOW_0");

        emit minTradePriceChanged(
            minTradePrice,
            _price
        );

        minTradePrice = _price;
    }

    function setmaxTradePrice(uint128 _price) external onlyOwner {
        require(_price >= 0, "CANNOT_BE_BELOW_0");

        emit maxTradePriceChanged(
            maxTradePrice,
            _price
        );

        maxTradePrice = _price;
    }
    ...
    function createOffer(
        ...
        require(_price > minTradePrice, "PRICE_BELOW_MINIMAL_PRICE");
        require(_price < maxTradePrice, "PRICE_ABOVE_MAX_PRICE");
    ...
    function takeOffer(uint256 _offerId) external whenNotPaused {
        require(usdtTokenContract.balanceOf(_msgSender()) >= uint256(offers[_offerId].price), 'TOKEN_AMOUNT_NOT_SUFFICIENT');
        ...
        usdtTokenContract.transferFrom(_msgSender(), offers[_offerId].owner, (offers[_offerId].price - _fee));
        if (_fee > 0) {
            // transfer fee to contract
            usdtTokenContract.transferFrom(_msgSender(), feeReceiverContract, _fee);
            if(offers[_offerId].nftContractAddress == inHouseNftAddress) minter(feeReceiverContract).payReward(fee,offers[_offerId].assetId);
        }
    ...
    function setfee(uint128 _newfee) external onlyOwner {
        require(_newfee < 1000, 'CANNOT_BE_ABOVE_1000');
        require(_newfee >= 0, 'CANNOT_BE_BELOW_0');
    ...
    function calculateFee(uint128 _price) public view returns (uint256){
        return uint256(_price) * uint256(fee) / 1000;
    }
    ...
    function adminReturnAccidentallySentToken(IERC20 _usdtTokenContractAddress) external onlyOwner {
        uint256 _amount = _usdtTokenContractAddress.balanceOf(address(this));
        _usdtTokenContractAddress.transfer(_msgSender(), _amount);
    }
