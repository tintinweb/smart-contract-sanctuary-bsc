...
contract SendStar is ERC721, ERC721Enumerable, Ownable {
    ...
    address public receptorWallet;

    mapping(address => bool) private _allowList;
    mapping(uint => uint) public priceCategories;

    constructor(address _buyToken, address _receptorWallet) ERC721("SendStart NFT", "SendStar") {
        priceCategories[0] = 0.1 ether;
        priceCategories[1] = 0.1 ether;
        priceCategories[2] = 0.1 ether;
        priceCategories[3] = 0.1 ether;
        buyToken = _buyToken;
        receptorWallet = _receptorWallet;
    }
    ...
    function mint(uint numberOfTokens, uint categoryId) public {
        uint256 ts = totalSupply();
        require(saleIsActive, "Sale must be active to mint tokens");
        require(numberOfTokens <= MAX_TX_MINT, "Exceeded max token purchase");
        require(ts + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(balanceOf(msg.sender) + numberOfTokens <= MAX_PER_WALLET, "Max Per Wallet");
        require(priceCategories[categoryId] * numberOfTokens <= IERC20(buyToken).balanceOf(msg.sender), "You dont have enough tokens");

        uint tokensToSend = priceCategories[categoryId] * numberOfTokens;

        bool sent = IERC20(buyToken).transferFrom(msg.sender, receptorWallet, tokensToSend);

        require(sent, "Required token transfer");
    ...
    function setPriceCategory(uint categoryId, uint price) public onlyOwner {
        priceCategories[categoryId] = price;
    }
}
