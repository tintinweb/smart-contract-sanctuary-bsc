...
contract LaunchpadNFT is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function leftToSend(uint256 _index) public view returns (uint256){
        require(_index <= launches.length, "Wrong index");
        return launches[_index].totalCount - launches[_index].sendCount;
    }
        ...
        returns (uint256)
    {
        ...
        require(checkSoldLimits(msg.sender, _launchIndex), "limit exceeding");
        ...
        returns (uint256)
    {
        ...
        Launchpad storage _launch = launches[_launchIndex];
        require(checkSendLimits(_receiver, _launchIndex), "limit exceeding");
     ...
     */
    function checkSoldLimits(address user, uint256 launchIndex) internal view returns(bool){
    ...
    function checkSendLimits(address user, uint256 launchIndex) internal view returns(bool){
        Launchpad memory launch = launches[launchIndex];
        return
            boughtCount[user] < maxToUser &&
            launch.sendCount < launch.totalCount;
    }
