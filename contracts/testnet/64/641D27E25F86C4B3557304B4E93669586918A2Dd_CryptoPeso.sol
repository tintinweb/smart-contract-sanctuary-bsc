...
contract CryptoPeso is IERC20, Ownable, Pausable,ReentrancyGuard {
    ...
    // Fees
    uint256 public mintFee             = 97000;              // 3.00% mint fee
    uint256 public sellFee                = 97000;             // 3.00% redeem fee 
    uint256 public transferFee        = 97000;             // 3.00%  transfer fee
    uint256 public stableSwapFee  = 97000;             // 3.00% stable swap fee
        // 0.05% stable swap fee
    ...
    */
    ...
    function _mintWithBacking(uint256 numBUSD, address recipient) internal whenNotPaused returns (uint256) {
        require(
            tokenActivated || canTransactPreLaunch[msg.sender],
            'Token Not Activated'
        );
    ...
    function _mintTo(address recipient, uint256 received, uint256 totalBacking, uint256 oldPrice) private whenNotPaused returns(uint256) {
    ...
    */
    function setFees(uint256 _mintFee, uint256 _transferFee, uint256 _sellFee, uint256 _stableSwapFee) external onlyOwner {
        ...
        require(_stableSwapFee >= 97000);      // capped at 3% fee         
