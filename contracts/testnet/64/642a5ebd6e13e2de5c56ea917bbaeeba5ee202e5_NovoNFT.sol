    ...
    PausableUpgradeable
{
    ...
    function getBagWeight(address _address) public view returns (uint256) {
        if (balanceOf(_address) == 0 || getLockedAmountByAddress(_address) == 0)
            return 0;
        return (getLockedAmountByAddress(_address) * (10**18)) / totalBagSize;
    }

    function getTimeWeight(address _address) public view returns (uint256) {
        if (balanceOf(_address) == 0 || getLockedAmountByAddress(_address) == 0)
            return 0;
        ...
        return
            ((uint80(block.timestamp) - mapStakers[tokenId].stakingTimestamp) *
