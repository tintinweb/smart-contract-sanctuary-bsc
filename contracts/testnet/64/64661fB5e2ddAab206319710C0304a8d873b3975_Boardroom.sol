...
contract Boardroom is OwnableUpgradeSafe, ShareWrapper, ContractGuard {
    ...
    modifier onlyOperator() {
        require(operator == msg.sender || owner() == msg.sender, "Boardroom: caller is not the operator/owner");
        _;
    }
    ...
    function setLockUp(uint256 _withdrawLockupEpochs) external onlyOwner {
        require(_withdrawLockupEpochs <= 300, "_withdrawLockupEpochs: out of range");
        withdrawLockupEpochs = _withdrawLockupEpochs;
    }

    function toggleDepositAllowed() external onlyOwner {
    ...
    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        ...
        if (directors[msg.sender].rewardEarned > 0) {
            claimReward();
        }
        super.stake(amount);
    ...
    function exit() public onlyOneBlock directorExists updateReward(msg.sender) {
        uint256 _amount = balanceOf(msg.sender);
        require(_amount > 0, "Boardroom: Nothing to withdraw");
        require(directors[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Boardroom: still in withdraw lockup");
        _sacrificeReward();
        super.withdraw(_amount, _amount);
        emit Exited(msg.sender, _amount);
    }
    ...
    function _sacrificeReward() internal updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            IBasisAsset(address(grain)).burn(reward);
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            directors[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
            directors[msg.sender].rewardEarned = 0;
            grain.safeTransfer(msg.sender, reward);
            emit RewardPaid(msg.sender, reward);
        }
    }

    function claimAndRestake() external updateReward(msg.sender) {
        uint256 reward = directors[msg.sender].rewardEarned;
        require(reward > 0, "Boardroom: No reward");
        directors[msg.sender].rewardEarned = 0;
        emit RewardPaid(msg.sender, reward);
        super.stake(reward);
        directors[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
    ...
    function rescueStuckErc20(IERC20 _token) external onlyOwner {
        ...
        require(address(_token) != address(grain), "grain");
        require(address(_token) != address(share), "share");
        _token.safeTransfer(operator, _token.balanceOf(address(this)));
    }
}
...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount, uint256 subtractedAmount) internal virtual {
