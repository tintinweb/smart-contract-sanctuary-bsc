...
contract StakingTest is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        rewPerDay = (rewAmount * 1e24) / _duration;
        pool = PoolInfo(_start, 0);
        start = _start;
        end = _start + _duration * 5 minutes;
    }
    ...
    function stake(uint256 amount) external nonReentrant {
        ...
        user.amount += amount;
        require(user.amount >= minStake && user.amount <= maxStake, "Limits");
        user.rewDebt = (user.amount * pool.accRewPerShare) / 1e24;
        totalStaked += amount;
        require(
    ...
    function unstake() external nonReentrant {
        require(block.timestamp <= end + 30 minutes * 5, "Stake is lost");
    ...
    function claimRestTyz() external onlyOwner nonReentrant {
        require(
            block.timestamp > end + 30 minutes * 5 ||
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (
            block.timestamp >= pool.lastUpdateDayTimestamp + 5 minutes &&
        ...
        return (user.amount * accRew) / 1e24 + user.accRew - user.rewDebt;
    }
    ...
    function updatePool() internal {
        ...
        uint256 reward = multiplier * rewPerDay;
        pool.lastUpdateDayTimestamp += 5 minutes * multiplier;
        if (totalStaked == 0) {
            if (pool.lastUpdateDayTimestamp < end) {
                rewPerDay +=
                    reward /
                    ((end - pool.lastUpdateDayTimestamp) / 5 minutes);
            }
            return;
        }
    ...
    function getMultiplier(uint256 from, uint256 to)
        ...
        returns (uint256)
    {
        return to <= end ? (to - from) / 5 minutes : (end - from) / 5 minutes;
    }
}
