...
contract BulkAirDrop {
   ...
   mapping (address => mapping (address => uint256)) private _AirDropBEP20;
    ...
    function AirDropBEP20(address referraladdress,address tokencontract,uint256 tokenQty,address payable[] memory addresses, uint[] memory amount) payable public  {   
    ...
    function AirDropBNB(address referraladdress,address payable[] memory addresses, uint[] memory amount) payable public  {  
        require(addresses.length == amount.length, "The length of 2 array should be the same");
        uint256 totalamount = getSum(amount);
