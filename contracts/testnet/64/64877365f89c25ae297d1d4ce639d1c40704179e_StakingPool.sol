...
contract StakingPool is Upgradable {
   ...
   */
    ...
    */
     ...
     * @param configs: startDate(0), endDate(1), duration(2), endStakedTime(3), stakingLimit_for_Linear(4),stopPool(5),exchangeRateRewardToStaking(5)
   */
    function createPool(string[] memory strs, address[] memory addr, uint256[] memory data, uint256[] memory configs) external onlyAdmins {
        ...
        if(configs[4] == 0 ) {
            ...
            PoolInfo memory pool = PoolInfo(addr[0], addr[1], 0, 0, data[0], data[0], 0, 0, 0, 1, configs, data[1],1, data[2]);
            poolInfo[strs[0]] = pool;
        }
