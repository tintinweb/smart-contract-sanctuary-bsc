...
contract BulldogeWrapper {
    using SafeMath for uint256;

    IBulldoge public stakedToken;

    uint256 private _totalReflections;
    mapping(address => uint256) private _reflections;

    modifier exemptFee() {
        stakedToken.setIsFeeExempt(msg.sender, true);
        _;
        stakedToken.setIsFeeExempt(msg.sender, false);
    }

    constructor(address _stakedToken) {
        stakedToken = IBulldoge(_stakedToken);
    }
...
contract BulldogeBoost is BulldogeWrapper, RewardDistributionRecipient {
    using SafeMath for uint256;
    ...
    ) BulldogeWrapper(_stakedToken) RewardDistributionRecipient() {
