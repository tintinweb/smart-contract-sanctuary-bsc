...
contract FeeHandlerWithOracle is IFeeHandler, AccessControl, ERC20Safe {
    ...
    event FeeHandlerSetFeeOracle(
        address indexed _this,
        address indexed oracleAddress
    );

    event FeeHandlerSetSystemToken(
        address indexed _this,
        address indexed systemToken
    );

    event FeeHandlerSetFeeProperties(
        address indexed _this,
        uint32 gasUsed, 
        uint16 feePercent
    );
    ...
    function setFeeOracle(address oracleAddress) external onlyAdmin {
        ...
        emit FeeHandlerSetFeeOracle(address(this), oracleAddress);
    }

    /**
        @notice Sets the fee system token address for fee.
        @param systemToken Fee system token fee address.
     */
    function setSystemToken(address systemToken) external onlyAdmin {
        _systemToken = systemToken;

        emit FeeHandlerSetSystemToken(address(this), systemToken);
    }
    ...
    function setFeeProperties(uint32 gasUsed, uint16 feePercent) external onlyAdmin {
        ...
        emit FeeHandlerSetFeeProperties(address(this), gasUsed, feePercent);
    }
     ...
     */
    function collectFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) payable external onlyBridgeOrRouter {
     ...
     */
    function calculateFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) external view returns(uint256 fee, address tokenAddress) {
        return _calculateFee(sender, fromDomainID, destinationDomainID, resourceID, depositData, feeData);
    }

    function _calculateFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) internal view returns(uint256 fee, address tokenAddress) {
        /**
            Message:
            ber * 10^18:  uint256
            ter * 10^18:  uint256
        ...
        */
        ...
        address tokenHandler = IBridge(_bridgeAddress)._resourceIDToHandlerAddress(oracleMessage.resourceID);
        address tokenAddress = IERCHandler(tokenHandler)._resourceIDToTokenContractAddress(oracleMessage.resourceID);
...
interface IBridge {
    /**
        ...
        @return uint256 The {_domainID} that is currently set for the Bridge contract.
     */
    function _domainID() external returns (uint256);
