...
contract Staking is Ownable, IERC721Receiver {
    ...
    function getCurrentStealPrice() public view returns (uint256) {
        if (block.timestamp <= stealChangeStartTime + 3600) return stealChangePriceGoldenHour;
    ...
    function changeStealPrice(uint256 newPrice) external onlyOwner {
        stealPrice = newPrice * 10 ** 16;
    }

    function changeStealPriceGoldenHour(uint256 newPrice, uint256 startTime) external onlyOwner {
        if (startTime == 0) stealChangePriceGoldenHour = block.timestamp;
        stealChangePriceGoldenHour = newPrice * 10 ** 16;
