...
contract CRSSCompensator is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 claimable = (user.totalCrssOwed *
            ((block.number - compensationStartBlock) /
                compensationLengthInBlocks)) - user.crssClaimed;
        if (_amount >= claimable) {
            if (_vesting) {
                ...
                uint256 adjustedAmount = (_amount * 4) / 5;
    ...
    function withdrawVested(uint256 _id) public nonReentrant {
        ...
        userWithdrawn[msg.sender] += userVest.crssAmount;
        totalWithdrawn += userVest.crssAmount;
    ...
    function massWithdrawVested() public nonReentrant {
        ...
        userWithdrawn[msg.sender] += totalCrss;
        totalWithdrawn += totalCrss;
    ...
    function withdrawAndGetSCRSS(uint256 _id) public nonReentrant {
        ...
        userWithdrawn[msg.sender] += userVest.crssAmount;
        totalWithdrawn += userVest.crssAmount;
        IsCRSS(sCrssToken).enterFor(userVest.crssAmount, msg.sender);
        //IERC20(crssToken).transfer(msg.sender, userVest.crssAmount);
    }
    ...
    function massWithdrawAndGetSCRSS() public nonReentrant {
        ...
        userWithdrawn[msg.sender] += totalCrss;
        totalWithdrawn += totalCrss;
        IsCRSS(sCrssToken).enterFor(totalCrss, msg.sender);
    }
