...
contract BnbSmartChain is ContractData {
    constructor() {
        ...
        accountsInfo[firstUser].unlockedLevel = 0;
        ...
        accountsEarnings[firstUser].depositValue = 0.1 ether;
        networkSize += 1;
    }
  ...
  // --------------------- PUBLIC METHODS ---------------------------
    ...
    function withdrawalTotal() external {
        ...
        uint tax = (freeToWithdrawl * cumulativeWPMFee) / 100;
        payable(sender).transfer(freeToWithdrawl - tax);
        //Pays residual bonus
        ...
        while(stopPayingResidual == false) {
            ...
            if (accountsInfo[_addressResidualReferral].registered == true && accountsInfo[_addressResidualReferral].depositTotal >= minAmountToGetBonus && accountsInfo[_addressResidualReferral].unlockedLevel >= residualLevel && residualLevel < (_unilevelPercents.length + 1) ) {
                uint residualBonusAmount = (tax * _unilevelPercents[residualLevel - 1]) / 1000; // RESIDUAL BONUS
    ...
    function withdrawalPercent(uint _amount) external {
        ...
        uint tax = (_amount * cumulativeWPMFee) / 100;
        payable(sender).transfer(_amount - tax);
  ...
  // --------------------- PRIVATE METHODS ---------------------------
    ...
    function _registerDeposit(address sender, uint amount, uint8 _type) private {
        ...
        if (_type == 0 || _type == 2) {
            ...
            //Pays network fee
            uint valueTax = (amount * wpmFeePercent) / 100;
            cumulativeNetworkFee += valueTax;
            uint comulativeFee = cumulativeNetworkFee;
            if (comulativeFee > 0.015 ether && address(this).balance >= comulativeFee) {
                payable(networkUser).transfer(comulativeFee);
...
contract ContractData is Authorized {
  ...
  uint cumulativeWPMFee = 4;


  address constant firstUser = 0x3c0bC8835c922D41055EBC4eBA3A337dc54E531e;
  address constant networkUser = 0x2EB3D75586D4433dd77B281c7241AA4036a40aba;
