    ...
    OwnableUpgradeable
{
    ...
    {
        address quoteToken;
        (, quoteToken, pairManager) = isPosiDexSupportPair(path[0], path[path.length - 1]);
    ...
    function setUniSwpRouter(IUniswapV2Router02 _newUniSwpRouter) public onlyOwner {
    ...
    function isApprove(address token) public view returns(bool){
        return  IERC20(token).allowance(address(this), address(uniSwapRouterV2)) > 0 ? true : false;
    }
