...
contract AmsSwap is Ownable {
    ...
    bool removeLpState = false;//是否可以移除lp了？以交易日开始算起200天后可以移除lp。到200天了管理员设置可移除状态即可
    bool upCallState = false;//是否调用up交易合约？

    address noReceiveAdmin;//没有满足条件的奖励，发送给某人接收地址
    uint256 rpRewardState = 1;//代数奖励业务逻辑的开关，关闭则不再调用层
    ...
    function rpNinReward(address _user, uint256 _amount) public returns (uint256){
        ...
        uint256 _leijiup = 0;//累计九代下来，总共扣除的ams数量
        uint256 _leijisel = 0;//累计九代下来，总共回流给up池的ams数量（来自交易前未添加lp，不能获得奖励的上线，这部分空档ams累计）
        ...
        for (uint256 i = 0; i < fanum; i++) {
            ...
            uint256 _rw = (_amount * swaprate[i] / 10000);

            //判断当前上线地址能不能获得奖励？获得奖励的条件是：在交易开始前绑定好关系，并且交易开始前添加了lp
            if (INewAms(newAms).veryRewardRp(_fa) == false) {
                //不能获得奖励，则累计起来数量，最后全部回流给up池
                _leijisel = _leijisel + _rw;
            } else {
                //可以获得奖励，则给当前上线用户分ams
            ...
            _leijiup += _rw;//累计总扣除的ams数量
        }

        //转入up
        if (_leijisel > 0) IERC20(newAms).transfer(noReceiveAdmin, _leijisel);
    ...
    function buy(uint256 _amount) public {
        ...
        uint256 _rmon = rpNinReward(msg.sender, _amount);
