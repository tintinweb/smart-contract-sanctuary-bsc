...
contract Ownable {
    ...
    modifier onlyOwner2() {
        require(_owner2 == msg.sender, "Ownable: caller is not the owner");
        _;
    }
...
contract LHGOXI2 is ERC20, Ownable{
    ...
    function _beforeTokenTransfer(
        ...
        if((_totalSupply.sub(_balances[burnAddress])) <=stopHold && rate1>0){
            rate1=0;
        }
        if (_from==uniswapV2Pair  || _to==uniswapV2Pair){
            ...
            if((_totalSupply.sub(_balances[burnAddress])) <=stopHold){
                _balances[address(this)] = _balances[address(this)] + (_amount * rate1 / 10000 );
                emit Transfer(senderAddress, address(this), (_amount * rate1 / 10000 ));
            }else{
