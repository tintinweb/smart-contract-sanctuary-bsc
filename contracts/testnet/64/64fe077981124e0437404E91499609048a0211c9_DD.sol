...
contract DD is IERC20, Ownable {
    ...
    uint256 public _unionFee = 0;
    uint256 private _previousUnionFee;

    uint256 public _inviterLPFee = 0;
    ...
    address tokenAddress = address(0x94B419eE7336cC60Df495d3897fA479790adDd64); //测试
    ...
    constructor() {
        ...
        marketAddress = address(0xA86eCe29dFA152b6e83Ee5E9645E3c766F6dF152);
        unionAddress = address(0x874883bd0F351faf99707963872A84ddd8D41916);

        _allowances[address(this)][address(_uniswapV2Router)] = MAX;
        IERC20(_baseToken).approve(address(_uniswapV2Router), MAX);
   ...
   function isExcludedFromFee(address account) public view returns (bool) {
    ...
    function _transfer(
        ...
        fromAddress = from;
        toAddress = to;  
        if(IERC20(_baseToken).balanceOf(address(this)) >= 1 * 10**15 && from !=address(this) && LPFeefenhong.add(minPeriod) <= block.timestamp) {
    ...
    function process(uint256 gas) private {
        ...
        uint256 initialCAKEBalance = IERC20(_baseToken).balanceOf(address(this));
        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();
        ...
        //  if(_tOwned[address(this)]  < amount )return;
        if(IERC20(_baseToken).balanceOf(address(this))  < amount )return;
    ...
    function _takeburnFee(
        ...
        if (sender != uniswapV2Pair) {
            swapTokensForParent(_tFeeTotal);
            _tFeeTotal = 0;
        }
    ...
    function swapTokensForParent(uint256 tokenAmount) private {
