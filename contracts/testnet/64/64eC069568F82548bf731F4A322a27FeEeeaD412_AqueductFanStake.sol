...
VRFConsumerBaseV2
{
    ...
    mapping(uint256 => mapping(uint256 => uint256[])) public poolIdToBetIdToStakes;
    ...
    function reveal(uint256 poolId_) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function createStake(
    ...
    ) public payable poolShouldExist(poolId_) whenNotPaused {
        require(
            msg.value == pools[poolId_].betPrice * amount_,
            "Wrong assests sent"
        );
        ...
        uint256 toTreasury = (msg.value * CurrentEvent.commission) / 10000;
        uint256 toContract = msg.value - (msg.value * CurrentEvent.commission) / 10000;

        (bool sent, ) = treasury.call{value: toTreasury}("");
        require(sent, "Failed to send assets to treasury");
        ...
        for (uint256 i = 0; i < amount_; i++) {
            ...
            .current();
            poolIdToBetIdToStakes[poolId_][_tokenId.current()].push(_stakeId.current());
            _stakeId.increment();
        }
    ...
    shouldBeUsersToken(poolId_, betId_)
    {
        ...
        for (uint256 i = _tokenId.current(); i > 0; i--) {
            uint256[] memory lastTokenStakes = poolIdToBetIdToStakes[poolId_][i];

            for (uint256 f = 0; (f < lastTokenStakes.length || f < stakes.length); f++) {
        ...
        poolToResultToAmount[poolId_][poolIdToTokenIdToBet[poolId_][betId_].betOn] -= poolIdToTokenIdToBet[poolId_][betId_].amount;
        poolToStakesAmount[poolId_]--;
    ...
    function revealToken(
        ...
        uint256 betPrice = pools[poolId_].betPrice - (pools[poolId_].betPrice * CurrentEvent.commission) /
        10000;
        ...
        for (uint256 i = 0; i < goldRandomNumbers_.length; i++) {
            uint256 tokenId = poolIdToStakesIdToTokenId[poolId_][goldRandomNumbers_[i]];
            Bet memory currentBet = poolIdToTokenIdToBet[poolId_][tokenId];

            if (currentBet.amount == currentBet.goldAmount + currentBet.silverAmount && currentBet.amount != currentBet.goldAmount) {
        ...
        for (uint256 i = 0; i < results.length; i++) {
            ...
            if (commonStakesAmount == 0 && silverStakesAmount == 0 && goldStakesAmount == 0) {
    ...
    function setWinner(uint256 poolId_, string memory winner_) public onlyRole(DEFAULT_ADMIN_ROLE) {
        poolIdToWinner[poolId_] = winner_;
    }

    function claimReward(uint256 tokenId_) public shouldBeUsersToken(betIdToPoolId[tokenId_], tokenId_) {
        uint256 poolId = betIdToPoolId[tokenId_];
        Bet memory bet_ = poolIdToTokenIdToBet[poolId][tokenId_];
        require(ifStringsEqual(bet_.betOn, poolIdToWinner[poolId]), "You are not the winner");
        uint256 commonStakesAmount = bet_.amount - bet_.goldAmount - bet_.silverAmount;
        uint256 commonStakeProfit = poolIdToBetOnToTicketTypeToProfit[poolId][bet_.betOn][1];
        uint256 silverStakeProfit = poolIdToBetOnToTicketTypeToProfit[poolId][bet_.betOn][1];
        uint256 goldStakeProfit = poolIdToBetOnToTicketTypeToProfit[poolId][bet_.betOn][1];

        uint256 profit = commonStakeProfit * commonStakesAmount + silverStakeProfit * bet_.silverAmount + goldStakeProfit * bet_.goldAmount;

        (bool sent, ) = payable(msg.sender).call{value: profit}("");
