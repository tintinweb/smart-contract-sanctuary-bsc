...
contract LuckyRatsToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "LuckyRats";
    string private constant _symbol = "LR";
    uint8 private constant _decimals = 9;
    
    address payable private _developmentAddress = payable(0x94b154a8944C3ca2FD489306B4aa5cAF9A96Fd17);
    address payable private _marketingAddress = payable(0x1C17F7873Dba304b232c7F87C66966d66f87C562);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router)) {
                if(block.timestamp - startBlock < 100 ) { _taxFee = 10; }    
                else {_redisFee = _redisFeeOnBuy;
                _taxFee = _taxFeeOnBuy;} 
            }
    
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if(block.timestamp - startBlock < 100 ) { _taxFee = 14; }                
                else {_redisFee = _redisFeeOnSell;
