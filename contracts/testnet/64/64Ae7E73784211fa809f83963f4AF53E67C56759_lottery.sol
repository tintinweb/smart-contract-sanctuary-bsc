...
contract lottery  is Ownable, Initializable {
    ...
    IBEP20 public token;
    uint public subsequentClaim = 100e18;

    uint public claimPeriod = 1;
    ...
    function initialize(address tokenAdd) external initializer {
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount) external onlyOwner {
        require(account.length < 2000,"lottery  : length < 2000");
        require((account.length == amount.length) ,"lottery  : length mismatch");
     

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "lottery  : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "lottery  : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "lottery  : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"lottery  : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "lottery  : user not exist");
        require(user_.totalClaimed < user_.balance, "lottery  : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "lottery  : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "lottery  : insufficient balance");
            require(payable(owner()).send(amount), "lottery  : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "lottery  : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "lottery  : transfer failed");
        }
    }
}