...
contract TimeLockNonTransferablePool is TimeLockPool {
    ...
    // reward not enabled
    // FIXME: TypeError: Trying to override non-virtual function. Did you forget to add "virtual"?
    // function claimRewards(address) external override {
    //     revert("CLAMING_DISABLED");
    // }

    function depositForWhitelist() external {
        // TODO: Validation
        require(depositToken.balanceOf(_msgSender()) >= STAKE_AMOUNT, "depositForWhitelist: not enough balance");
        require(depositsOf[_msgSender()].length == 0, "depositForWhitelist: already deposit");
        
        uint256 _split = 10;
        uint256 _amountPerSplit = STAKE_AMOUNT / _split;
        uint256 _vestingPerSplit = 30 days / _split;
        for (uint256 i = 0; i < _split; i++) {
            _deposit(
                _amountPerSplit, 
                MIN_LOCK_DURATION + (i * _vestingPerSplit),
                _msgSender()
            );
        }
    }
...
contract TimeLockPool is BasePool, ITimeLockPool {
    ...
    function deposit(uint256, uint256, address) external pure override {
        revert("GENERAL_DEPOSIT_DISABLED");
    }
    ...
    // TODO: remove multiplier concept?, maybe not needed as the rewards are not used
