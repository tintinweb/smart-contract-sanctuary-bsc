    ...
    ERC1155Burnable
{
    ...
    mapping(uint256 => mapping(address => bool)) public StakeNftOwner;
    uint256 public tokenLimitMin = 100000000000000000000;
    uint256 public discountPercentage = 2;
    ...
    EpochNFT1155 public  EpochNFT1155M;
    ...
    constructor(    
        ...
        EpochNFT1155 _EpochNFT1155M = EpochNFT1155(adminContract);
        // set the rest of the contract variables
        EpochNFT1155M = _EpochNFT1155M;
    ...
    function getServiceFeeESC() public view returns (uint256) {
        return  EpochNFT1155M.getServiceFeeESC();
    }

    function getServiceFee() public view returns (uint256) {
        return  EpochNFT1155M.getServiceFee();
    }
    ...
    function mint(
        ...
        if(whiteListuserArray[msg.sender] == true) {
            isStaking[id] = isstaking;
        }
    ...
    function stakeNft(uint256 tokenId, uint256 NOFToken) public {
        if(whiteListuserArray[msg.sender] == true && isStaking[tokenId] == true &&  _balances[tokenId][msg.sender] > 0) {
            if (checkOrder[tokenId][msg.sender] == true) {
                delete order_place[msg.sender][tokenId];
                checkOrder[tokenId][msg.sender] = false;
            }
            tokenTrans(tokenId, msg.sender, address(this), NOFToken);
            StakeNftOwner[tokenId][msg.sender] == true;
        }
    }

    function unStakeNft(uint256 tokenId, uint256 NOFToken) public{
        if(StakeNftOwner[tokenId][msg.sender] == true) {
            tokenTrans(tokenId, address(this), msg.sender, NOFToken);
            StakeNftOwner[tokenId][msg.sender] == false;
        }
    }
    ...
    function _saleToken(
        ...
        uint256 serviceVal = EpochNFT1155M.getServiceFee();
        if(keccak256(bytes(bidtoken)) == keccak256(bytes("ESC")))
        {
             serviceVal = EpochNFT1155M.getServiceFeeESC();
        }

        address payable admin =  payable(EpochNFT1155M.getOwner());
