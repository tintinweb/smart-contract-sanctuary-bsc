...
contract KyotoLaunchpad is OwnableUpgradeable {
    ...
    event IDORewarded(string indexed idoID, address tokenAddress, bool isRewarded);
    event SetMerkleRoot(string indexed idoID, bytes32 merkleRoot);
    event Invest(string indexed idoID, address investor, uint256 investment);
    event IDOAdded(string indexed idoID, address idoOwner, address idoToken);
    event IDOCancelled(string indexed idoID);
    ...
    //accepts BNB sent with no txData
    receive() external payable {}
    //refuses BNB sent with txData that does not match any function signature in the contract
    fallback() external {}
    ...
    function setFee(uint _feePercentage) 
        ...
        require(
            _feePercentage > 0,
            "KyotoLaunchPad : smart contract fee cannot be 0"
        );
    ...
    function addPotentialOwner(address potentialOwner) 
        ...
        require(
            ...
            "KyotoLaunchPad: potential owner address must not be 0"
        );
        require(
            potentialOwner != _owner,
            "KyotoLaunchPad: owner cannot be potential owner"
        );
    ...
    function addIDO(string calldata idoID, IDO calldata ido) 
        ...
        require(
            ...
            "KyotoLaunchPad: project already exists"
        );
        ...
        require(
            ...
            "KyotoLaunchpad: project tokens transfer to plaform failed"
            );
    ...
    function getInvestmentAmount(string calldata idoID) 
        ...
        require(feePercentage>0, 
                "KyotoLaunchpad: set smart contract fee");
    ...
    function distributeTokens(string calldata idoID) 
        ...
        require(
            ...
            "KyotoLaunchPad: project close time not reached"
        );
        ...
        for (
            ...
            require(
                IERC20Upgradeable(ido.tokenAddress).transfer(
                    ...
                    "KyotoLaunchpad: project tokens transfer to investor failed"
                );
        }
        ...
        if (totalTokensToDistribute > 0) {
            require(
                IERC20Upgradeable(ido.tokenAddress).transfer(
                    ...
                    "KyotoLaunchpad: project tokens transfer to owner failed"
                );
        }
    ...
    function cancelAndRefundIDO(string calldata idoID) 
        ...
        require(
            ...
            "KyotoLaunchPad: investors already rewarded"
        );
        ...
        if(_idoInvestment.investors.length == 0){
            ...
            require(
                IERC20Upgradeable(_ido.tokenAddress).transfer(
                    ...
                    "KyotoLaunchpad: project tokens transfer failed"
                );
        }
        else{
            for (
    ...
    function acceptOwnership() external {
        require(
            ...
            "KyotoLaunchPad: caller is not potential owner"
        );
    ...
    function investInWhitelistPhase(
        ...
        require(
            _amount > 0,
            "KyotoLaunchPad: investment cannot be 0"
        );
        require(
            _idos[idoID].winnersOutTimestamp <= block.timestamp,
            "KyotoLaunchPad: whitelist phase not started"
        );
        require(
            _idos[idoID].idoCloseTimestamp > block.timestamp,
    ...
    function _validateIDOData(IDO calldata ido) private view {
        ...
        require(
            ...
            "KyotoLaunchPad: token supply less than tokens for distribution"
        );
        require(
            ido.idoOwner != address(0),
            "KyotoLaunchPad: address of project owner cannot be 0"
        );
        ...
        require(
            ...
            "KyotoLaunchPad: whitelist open time cannot be in the past"
        );
        require(
            ido.whitelistOpenTimestamp < ido.winnersOutTimestamp,
            "KyotoLaunchPad: whitelist must precede winners announcement"
        );
        require(
            ido.winnersOutTimestamp <= ido.publicInvestmentStartTimestamp,
            "KyotoLaunchPad: winners announcement must precede investing time"
        );
        require(
            ido.publicInvestmentStartTimestamp < ido.idoCloseTimestamp,
            "KyotoLaunchPad: investing must precede project closing time"
        );
        require(
            ido.publicSlots > 0,
            "KyotoLaunchPad: number of slots cannot be 0"
        );
        require(
            ido.targetAmount > 0,
            "KyotoLaunchPad: target amount cannot be 0"
        );
        require(
            ido.minimumInvestment > 0,
            "KyotoLaunchPad: minimum investment cannot be 0"
        );
        require(
            (ido.targetAmount / ido.tokenPrice) <= ido.tokensForDistribution,
            "KyotoLaunchpad: token supply would not suffice"
        );
        require(ido.isRewarded == false, "KyotoLaunchPad: project cannot be rewarded");
    }
    ...
    function _invest(string calldata idoID, uint256 _amount) private {
        ...
        require(
                ...
                "KyotoLaunchPad: investment amount exceeds target"
            );
    ...
    function _checkThatIDOIndNotRewarded(string calldata idoID) private view {
        require(
            _idos[idoID].isRewarded == false,
            "KyotoLaunchPad: project has already been rewarded"
        );
    }
