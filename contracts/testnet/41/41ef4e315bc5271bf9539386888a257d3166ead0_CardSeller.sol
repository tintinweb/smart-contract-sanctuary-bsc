...
error CardSellerInputArraysLengthMissmatch();
error CardSellerCardsWrongOrder();
error CardSellerCardNotDefined();
error CardSellerZeroCardAmount();
error CardSellerZeroBuyAmount();
error CardSellerZeroPrice();
error CardSellerNoMoreRemainAmount();
error CardSellerStartDateNotDefined();
error CardSellerNotStarted();
error CardSellerNotEnoughMoney();
error CardSellerFailedToTransferMoney();
...
contract CardSeller is AccessControlEnumerableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    struct Sale {
        ...
        uint256 price; //usd, related to Chainlink pricefeed, 1000000 = 0.01$
    ...
    IERC20 private _token; // token

    mapping(CardType => Card) private _cards;
    mapping(CardType => Sale) private _sales;
    CardType private _activeSaleCardType;
    ...
    function initialize(
        ...
        address cardMintPassAddress,
        address cardWhiteListAddress,
        address cardDiscountAddress,
        address cardGiftAddress
    ...
    function __CardSeller_init_unchained(
        ...
        _cards[CardType.MINT_PASS] = Card(cardMintPassAddress);
        _cards[CardType.WHITE_LIST] = Card(cardWhiteListAddress);
        _cards[CardType.DISCOUNT] = Card(cardDiscountAddress);
        _cards[CardType.GIFT] = Card(cardGiftAddress);
    }

    receive() external payable nonReentrant {
        _buy(_activeSaleCardType, 1);
    }
    ...
    function setTokenPair(address tokenAddress, address tokenBUSDAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _tokenBUSD = IERC20(tokenBUSDAddress);
        _token = IERC20(tokenAddress);
    }

    function getTokenPair() external view returns (address tokenAddress, address tokenBUSDAddress) {
        return (address(_token), address(_tokenBUSD));
    }

    function setCardContract(CardType cardType, address cardContract) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _cards[cardType] = Card(cardContract);
    }

    function getCardContract(CardType cardType) external view returns (address) {
        return address(_cards[cardType]);
    }

    function setActiveSaleCardType(CardType cardType) external onlyRole(SALE_ROLE) {
        _activeSaleCardType = cardType;
    }

    function getActiveSaleCardType() external view returns (CardType) {
        return _activeSaleCardType;
    }

    function getSale(CardType cardType) public view returns (Sale memory) {
        return _sales[cardType];
    }

    function getActiveSale() external view returns (Sale memory) {
        return getSale(_activeSaleCardType);
    }

    function getSaleRemainAmount(CardType cardType) public view returns (uint256) {
        return _sales[cardType].totalRemainAmount;
    }

    function getActiveSaleRemainAmount() external view returns (uint256) {
        return getSaleRemainAmount(_activeSaleCardType);
    }

    function getSalePrice(CardType cardType) public view returns (uint256) {
        return _sales[cardType].price;
    }

    function getActiveSalePrice() external view returns (uint256) {
        return getSalePrice(_activeSaleCardType);
    }
    ...
    function _checkAmountsOrder(uint128[] memory amounts) internal pure returns (uint128 totalAmount) {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            if (amounts[i] == 0) revert CardSellerZeroCardAmount();
            if (i > 0 && amounts[i] > prevAmount) revert CardSellerCardsWrongOrder();
    ...
    function startSale(
        ...
        if (start == 0) revert CardSellerStartDateNotDefined();
        if (price == 0) revert CardSellerZeroPrice();
        if (levels.length != amounts.length) revert CardSellerInputArraysLengthMissmatch();
        uint128 totalAmount = _checkAmountsOrder(amounts);

        _sales[cardType] = Sale(
            start,
            duration,
            price,
            levels,
    ...
    function finishSale(CardType cardType) external onlyRole(SALE_ROLE) {
        if (_sales[cardType].start == 0) revert CardSellerNotStarted();
        delete _sales[cardType].levels;
        delete _sales[cardType].amounts;
        delete _sales[cardType].remainAmounts;
        delete _sales[cardType];
    }
    ...
    function _getAmountPriceCost(Sale memory sale, uint128 buyAmount) internal pure returns (uint128 amount, uint256 cost) {
        if (buyAmount == 0) revert CardSellerZeroBuyAmount();
        amount = buyAmount;
        if (amount > sale.totalRemainAmount) {
            amount = sale.totalRemainAmount;
        }
        cost = amount * sale.price;
    }

    function _getSale(CardType cardType) internal view returns (Sale memory sale) {
        sale = _sales[cardType];
        if (sale.start == 0 || block.timestamp < sale.start || (sale.duration != 0 && block.timestamp > sale.start + sale.duration))
            revert CardSellerNotStarted();
        if (sale.totalRemainAmount == 0) revert CardSellerNoMoreRemainAmount();
    }
    ...
    function _processSale(
        ...
        if (address(card) == address(0)) revert CardSellerCardNotDefined();
        uint256 index;
        for (uint256 i = 0; i < amount; i++) {
            ...
            index = sale.levels.length > 1 // revert inside the _getRandomLevelIndex should not fire due to `amount <=sale.totalRemainAmount`
                ? _getRandomLevelIndex(sale.totalRemainAmount, sale.amounts, sale.remainAmounts, uint160(buyer) + i + index)
                : 0;
            card.mint(buyer, sale.levels[index]);
    ...
    function _buy(CardType cardType, uint128 buyAmount) internal {
        if (msg.value == 0) revert CardSellerNotEnoughMoney();
        Sale memory sale = _getSale(cardType);
        (uint128 amount, uint256 costBUSD) = _getAmountPriceCost(sale, buyAmount);

        // process and update remain amounts
        _sales[cardType].remainAmounts = _processSale(sale, _cards[cardType], _msgSender(), amount);
        uint256 bnbPrice = getBNBper1USD();
        uint256 value = (costBUSD * bnbPrice) / 1 ether;
        if (msg.value < value) revert CardSellerNotEnoughMoney();
        // update total remain amount
        _sales[cardType].totalRemainAmount -= amount;
        ...
        if (!success) revert CardSellerFailedToTransferMoney();
    }

    function _buyForTokenBUSD(CardType cardType, uint128 buyAmount) internal {
        Sale memory sale = _getSale(cardType);
        (uint128 amount, uint256 costBUSD) = _getAmountPriceCost(sale, buyAmount);
        // process and update remain amounts
        _sales[cardType].remainAmounts = _processSale(sale, _cards[cardType], _msgSender(), amount);
    ...
    function _buyForToken(CardType cardType, uint128 buyAmount) internal {
        Sale memory sale = _getSale(cardType);
        (uint128 amount, uint256 costBUSD) = _getAmountPriceCost(sale, buyAmount);

        // process and update remain amounts
        _sales[cardType].remainAmounts = _processSale(sale, _cards[cardType], _msgSender(), amount);
        // try to transfer token from buyer and swap it in pool
        address[] memory path = new address[](2);
        path[0] = address(_token);
        path[1] = address(_tokenBUSD);
        uint256[] memory amounts = _router.getAmountsIn(costBUSD, path);
        _token.transferFrom(_msgSender(), address(this), amounts[0]);
        _token.approve(address(_router), amounts[0]);
        amounts = _router.swapTokensForExactTokens(amounts[1], amounts[0], path, _wallet, block.timestamp);
        // update total remain amount
        _sales[cardType].totalRemainAmount -= amount;
    }

    function sellActive(address buyer, uint128 amountUSD) external {
        sell(_activeSaleCardType, buyer, amountUSD);
    }
    ...
    function sell(
        ...
        address buyer,
        uint128 buyAmount
    ) public onlyRole(SALE_ROLE) nonReentrant {
        Sale memory sale = _getSale(cardType);
        (uint128 amount, ) = _getAmountPriceCost(sale, buyAmount);
        // process and update remain amounts
        _sales[cardType].remainAmounts = _processSale(sale, _cards[cardType], buyer, amount);
        // update total remain amount
        _sales[cardType].totalRemainAmount -= amount;
    }

    function buyActive(uint128 buyAmount) external payable {
        buy(_activeSaleCardType, buyAmount);
    }

    function buy(CardType cardType, uint128 buyAmount) public payable nonReentrant {
        _buy(cardType, buyAmount);
    }

    function buyActiveBUSD(uint128 buyAmount) external {
        buyForTokenBUSD(_activeSaleCardType, buyAmount);
    }

    function buyForTokenBUSD(CardType cardType, uint128 buyAmount) public nonReentrant {
        _buyForTokenBUSD(cardType, buyAmount);
    }

    function buyActiveForToken(uint128 buyAmount) external {
        buyForToken(_activeSaleCardType, buyAmount);
    }

    function buyForToken(CardType cardType, uint128 buyAmount) public nonReentrant {
        _buyForToken(cardType, buyAmount);
    }
...
error CardCallerIsNotOwnerNorApproved();
error CardTransferWhilePaused();
error CardTokenNotExists();
error CardMaxUsesCountReached();
error CardMaxOwnsCountReached();
error CardZeroUseCount();
error CardWrongInputParams();
    ...
    ERC721URIStorageUpgradeable
{
    ...
    bytes32 public constant HOLDER_ROLE = keccak256("HOLDER_ROLE"); //allows to hold many cards
    ...
    function initialize(
        ...
        __Card_init(name, symbol, baseURI, ownsCount, usesCount);
    }
    ...
    function __Card_init(
        ...
        __Card_init_unchained(baseURI, ownsCount, usesCount);
    }
    ...
    function setLevelURIs(uint128[] memory levelIds, string[] memory levelURIs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (levelIds.length == 0 || levelIds.length != levelURIs.length) revert CardWrongInputParams();
    ...
    function _burn(uint256 tokenId) internal override(ERC721URIStorageUpgradeable, ERC721Upgradeable) {
        if (!_isApprovedOrOwner(_msgSender(), tokenId)) revert CardCallerIsNotOwnerNorApproved();
        super._burn(tokenId);
        // clean state
    ...
    function useCard(uint256 tokenId, uint128 count) external onlyRole(OPERATOR_ROLE) returns (CardMeta memory meta) {
        if (!_exists(tokenId)) revert CardTokenNotExists();
        if (count == 0) revert CardZeroUseCount();
        meta = _meta[tokenId];
        if (meta.uses + count > _maxUsesCount) revert CardMaxUsesCountReached();
        meta.uses += count;
        //save uses count
        ...
        )
    {
        ...
        revert CardMaxUsesCountReached();
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (paused()) revert CardTransferWhilePaused();

        // if not burning, check max own count of the receiver
        if (to != address(0) && _maxOwnsCount > 0 && balanceOf(to) >= _maxOwnsCount && !hasRole(HOLDER_ROLE, to)) {
            revert CardMaxOwnsCountReached();
        }
    }
