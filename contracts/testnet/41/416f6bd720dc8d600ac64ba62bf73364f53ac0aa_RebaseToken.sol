...
// This is denominated in Fragments, because the dings-fragments conversion might change before
    ...
    */
    ...
    function getEarnedRewards(address tokenAddress, address investor) public view returns (uint256 rewards){
       if(tokenAddress== lpaddress)
       { 
            return  calculateRewards(tokenAddress, investor, stakinglpapy);
       }
       else
       {
            return calculateRewards(tokenAddress, investor, APY);
       }
    ...
    function claimRewards(address tokenAddress) public {
        uint256 rewards=1;
        if(tokenAddress== lpaddress){ 
            rewards = calculateRewards(tokenAddress, msg.sender, stakinglpapy);
       }
       else{
            rewards = calculateRewards(tokenAddress, msg.sender, APY);
       }
