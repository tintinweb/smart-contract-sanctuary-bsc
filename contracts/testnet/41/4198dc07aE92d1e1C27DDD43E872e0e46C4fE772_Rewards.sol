...
contract Rewards is Initializable {
    ...
    uint256 public pendingEBSC;
    uint256 public ebscStakeholder;

    uint256 public bnbStakeholder;
    ...
    event ClaimedReward(bool isReflection, address receiver, uint256 amount);

    /**
     ** @dev Initialize Function
     ** @param _staking: Staking Contract
     ** @param _admin: Admin Contract
     ** @param _EBSC: EBSC contract
     */
    function initialize(
        address _staking,
        address _admin,
    ...
    /**
    @dev Checking epoch for reward distribution
     */
    function checkEpoch() public view returns (bool) {
        return (lastEpoch + epochDuration <= block.timestamp);
    }

    /**
    @dev Setting epoch for reward distribution
     */
    function setEpoch() public onlyStaking {
        require(checkEpoch(), "Cant distribute");
     ...
     */
    function setEpochDuration(uint256 _time) external onlyOperator {
        require(_time > 60, "time should be greater than 1 min");
        epochDuration = _time;
    }

    /**
    @dev set BNBstakeholders users 
    @param _tier Tier eligible for BNB reward
     */
    function setBnbStakeholders(uint256 _tier) external onlyOperator {
        bnbStakeholder = _tier;
    }

    /**
    @dev set ebscstakeholders users 
    @param _level Lock Level eligible for EBSC reward
     */
    function setEbscStakeholders(uint256 _level) external onlyOperator {
        ebscStakeholder = _level;
    }
    ...
    /**
     * @dev returns reflection of the user
     */
    function getReflection() public view returns (uint256) {
        return (EBSC.balanceOf(address(this)) - totalEBSC);
    }

    /**
    @dev Users can withdraw EBSC
    @param user user address
    @param _amount Withdraw amount 
     */
    function withdrawEBSC(address user, uint256 _amount) public onlyStaking {
        totalstakeEBSC = totalstakeEBSC.sub(
            _amount,
            "totalstakedEBSC - amount fail"
        );
        s1EBSC[user] = SEBSC;
    }

    /**
    @dev Distribute EBSC tokens  
    @param amount Amount to be distributed 
     */
    function distributionEBSC(uint256 amount) public onlyStaking {
        if (amount != 0) {
            if (totalstakeEBSC != 0) {
                EBSC.safeTransferFrom(
                    marketingWallet,
                    address(this),
                    amount / 2
                );
    ...
    function setShareForEBSCReward(
        ...
        uint256 lock;
        uint256 amount;
        (, lock, amount, ) = staking.getUserState(user);
        if (_prevLock == 2) {
            totalstakeEBSC -= _amount;
        } else if (_prevLock == 3) {
            totalstakeEBSC -= (_amount * 15) / 10;
        } else if (_prevLock == 4) {
            totalstakeEBSC -= (_amount * 30) / 10;
        }
        if (lock == 3) {
            amount = (amount * 15) / 10;
        } else if (lock == 4) {
    ...
    /**
    @dev Calculates Pending EBSC reward
    @param user User address
     */
    function userPendingEBSC(address user) public onlyStaking {
        uint256 amount;
        uint256 lock;
        (, lock, amount, ) = staking.getUserState(user);
        if (lock == 3) {
    ...
    /**
    @dev return EBSC reward of the user
    @param user user address
    @param amount amount staked 
    @param lock lock level
     */
    function getEBSCReward(
        address user,
        uint256 amount,
        uint256 lock
    ) public view returns (uint256) {
        if (lock <= ebscStakeholder) {
            return (previousRewardEBSC[user]);
        } else {
            if (lock == 3) {
                amount = (amount * 15) / 10;
            } else if (lock == 4) {
            ...
            return (previousRewardEBSC[user] +
                (amount * (SEBSC - s1EBSC[user])) /
                10**18);
        }
    }

    /**
     * @dev user will claimEBSC
     */
    function claimEBSC() public {
        ...
        reward = getEBSCReward(msg.sender, amount, lock);
        previousRewardEBSC[msg.sender] = 0;
        totalEBSC = totalEBSC.sub(reward, "totalEBSC - reward fail");
        s1EBSC[msg.sender] = SEBSC;
        EBSC.safeTransfer(msg.sender, reward);
        emit ClaimedReward(true, msg.sender, reward);
    }

    /**
     @dev Set Total BNB for reward distribution
     */
    function setTotalBNB(uint256 _amount) public onlyStaking {
        valueNotDistribute += _amount;
    }

    /**
    @dev Set BNB Share for users
    @param user User address
    @param _amount Amount Staked
     */
    function setShareForBNBReward(address user, uint256 _amount)
        public
        onlyStaking
    {
        totalstakeBNB += _amount;
        s1BNB[user] = SBNB;
    }

    /**
    @dev Distributes BNB rewards
     */
    function distributionBNB() public onlyStaking {
        if (totalstakeBNB != 0 && valueNotDistribute != 0) {
            SBNB += ((valueNotDistribute) * 10**18) / totalstakeBNB;
            totalBNB += valueNotDistribute;
            valueNotDistribute = 0;
        }
    }

    /**
    @dev Set pendingbnb of the user
    @param user user address 
    @param amount amount staked 
     */
    function userPendingBNB(address user, uint256 amount) public onlyStaking {
        previousRewardBNB[user] += (amount * (SBNB - s1BNB[user])) / 10**18;
    }

    /**
     @dev return BNB rewards of the user
    ...
    /**
    @dev Sets marketing Wallet
    @param _address Wallet Address
     */
    function setMarketingWallet(address _address)
        external
        onlyOperator
        validation(_address)
    {
    ...
    function claimBNB() public {
        ...
        reward = getBNBReward(msg.sender, amount, tier);
        require(reward != 0, "Nothing to claim");
        previousRewardBNB[msg.sender] = 0;
        totalBNB = totalBNB.sub(reward, "totalBNB - reward fail");
    ...
    */
    ...
    /**
     @dev set ebsc address
     @param _address ebsc address
     */
    function setEBSC(address _address)
        external
        validation(_address)
        onlyOperator
    {
