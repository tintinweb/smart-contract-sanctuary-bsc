...
contract MSTStakePool is Ownable, Pausable, ReentrancyGuard {
    ...
    function withdraw(uint256 _orderId) external whenNotPaused {
        ...
        // OrderInfo
        uint256 pendingRewards = ((orderInfo.lockedWeight * accWeightPerShare) /
            PRECISION_FACTOR);
        ...
        returns (uint256)
    {
        ...
        if ((block.number > lastRewardBlock) && (totalWeightStaked != 0)) {
            ...
            return
                (orderInfos[_orderId].lockedWeight * adjustedWeightPerShare) /
                PRECISION_FACTOR;
        } else {
            return
                (orderInfos[_orderId].lockedWeight * accWeightPerShare) /
                PRECISION_FACTOR;
        }
    }
    ...
    function _calculatePendingRewards(uint256 _orderId)
        ...
        returns (uint256)
    {
        if (!totalOrderIds.contains(_orderId)) {
            return 0;
        }
        if ((block.number > lastRewardBlock) && (totalWeightStaked != 0)) {
            uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
