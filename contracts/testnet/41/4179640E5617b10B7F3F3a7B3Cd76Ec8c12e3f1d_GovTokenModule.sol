...
bytes32 constant GovTokenModuleId = keccak256('govtokenimpl.dabot.module');

contract GovTokenModule is GovernToken, IDABotModule {

    constructor(IRoboFiERC20 vics) GovernToken(vics) {
    ...
    {
        name = "GovTokenModule";
        version = "v0.1.20220301";
        moduleId = GovTokenModuleId;
    }

    function onRegister(address moduleAddress) external override { 
        emit ModuleRegistered("GovTokenModule", GovTokenModuleId, moduleAddress);
    }

    function onInitialize(bytes calldata) external pure override {
        // empty by purpose
    }
}
