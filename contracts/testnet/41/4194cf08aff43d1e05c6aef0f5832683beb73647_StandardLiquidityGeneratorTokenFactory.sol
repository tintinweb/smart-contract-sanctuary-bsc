...
contract StandardLiquidityGeneratorToken is IERC20Upgradeable, OwnableUpgradeable, BaseToken {
    using SafeMath for uint256;
    using Address for address payable;
...
contract StandardLiquidityGeneratorTokenFactory is TokenFactoryBase {
    using Address for address payable;
    StandardLiquidityGeneratorToken[] public liquidityTokens;
    ...
    function create(
        ...
        StandardLiquidityGeneratorToken(payable(token)).initialize(
        msg.sender,
        name,
        symbol,
        totalSupply,
        ...
        liquidityTokens.push(StandardLiquidityGeneratorToken(payable(token)));
        assignTokenToOwner(msg.sender, token, 2);
        emit TokenCreated(msg.sender, token, TokenType.liquidityGenerator, 1);
    }

    function getTokens() public view returns(StandardLiquidityGeneratorToken[] memory) {
