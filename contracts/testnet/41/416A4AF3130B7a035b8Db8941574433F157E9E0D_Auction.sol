...
contract Auction is AccessControlEnumerable, Initializable {
    ...
    event AuctionRemoved(uint indexed id);
    event HighestBidIncreased(address  indexed bidder, uint amount, uint indexed id);
    ...
    function pushToAuction(uint _tokenId, uint _auctionTimeLength, uint _auctionTimeExtension, uint _minToExtend, uint _minimumBid) external noContract {
        require(hasRole(GOV_ROLE, _msgSender()) || openToPublic, "Not open to public");
        require(_minimumBid > 0, "Minimum bid cannot be 0");
        // Minimum bid == 0
        ...
        require(_auctionTimeLength >= minAuctionTimeLength && _auctionTimeLength <= maxAuctionTimeLength && maxAuctionTimeLength >= _minToExtend, "Invalid auction length");
        // Auction time length < minAuctionTimeLength

        // Transfer NFT to this contract
        nftContract.transferFrom(_msgSender(), address(this), _tokenId);
        uint endTime = block.timestamp + _auctionTimeLength;

        auctions.push(createAuction(endTime, _msgSender(), _minimumBid - 1, _auctionTimeExtension, _minToExtend, block.timestamp, _auctionTimeLength, _tokenId));
        emit AuctionCreated(_msgSender(), _minimumBid, auctions.length);
    }
    ...
    function bid(uint _id) external payable noContract {
        NftAuction storage auction = auctions[_id];
        require(auction.deleted == 0, "Auction is deleted");
    ...
    function removeAuction(uint _id) external {
        NftAuction memory auction = auctions[_id];
        require(auction.deleted == 0, "Auction is deleted");
        require(auction.highestBidder == address(0), "Cannot remove active auction");
        require(auction.seller == _msgSender(), "Not the seller of the auction");
        auction.deleted = 1;
        nftContract.transferFrom(address(this), auction.seller, auction.id);
        emit AuctionRemoved(_id);
    }
    ...
    function endAuction(uint _id) external noContract {
        ...
        if (auction.highestBidder == address(0)) {
            ...
            nftContract.transferFrom(address(this), auction.highestBidder, auction.id);
            // transfer NFT to buyer

            uint paymentFee = auction.highestBid * auctionFeePercent / 1000;
            uint divShare = paymentFee * auctionDividendShare / 1000;
            takerDividends += divShare;
            // count fee towards dividends
            adminDividends += paymentFee - divShare;
            // count fee to admins

            payable(auction.seller).transfer(auction.highestBid - paymentFee);
    ...
    function minBid(uint _id) public view returns (uint minIncrement, uint minimumBid) {
        NftAuction storage auction = auctions[_id];
        uint minIncrementPerc = auction.highestBid * minIncrementPercentage / 1000;
        minIncrement = minIncrementPerc > minIncrementAmount ? minIncrementPerc : minIncrementAmount;
        minimumBid = auction.highestBid + minIncrement;
    }

    /// @notice Returns auction data with its id in the list
    /// @param _id in the list auction
    function getAuction(uint _id) public view returns (NftAuction memory auction) {
        auction = auctions[_id];
    }

    /// @notice Returns auction length
