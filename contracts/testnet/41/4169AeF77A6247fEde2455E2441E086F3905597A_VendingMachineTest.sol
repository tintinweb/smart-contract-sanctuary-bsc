...
contract VendingMachineTest is Ownable {
    ...
    struct PayTokenInfo {
        IERC20 payToken;
        uint256 conversionFactore;
    }

    PayTokenInfo[] public AllowedCrypto;


	function addCurrency(
        IERC20 _payToken,
        uint256 _conversionFactore
    ) public onlyOwner {
        AllowedCrypto.push(
            PayTokenInfo({
                payToken: _payToken,
                conversionFactore: _conversionFactore
            })
        );
    }

	function getCredits(address _to, uint256 _mintAmount, uint256 _pid) public payable {
        PayTokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 payToken;
        payToken = tokens.payToken;
        uint256 cost;
        cost = tokens.conversionFactore;
        ...
        IMintable20(creditToken).mint(_to, _mintAmount);
    }
    ...
    function withdrawAllowedCrypto(uint256 _pid) public payable onlyOwner() {
    	PayTokenInfo storage tokens = AllowedCrypto[_pid];
    	IERC20 paytoken;
    	paytoken = tokens.payToken;
