...
contract SingleStaking is Ownable, ReentrancyGuard {
    ...
    function emergencyRewardWithdraw(uint256 _amount) external onlyOwner {
        uint256 tokenAmt = rewardToken.balanceOf(address(this));
        require(tokenAmt >= _amount, "Amount to withdraw too high");

        if(address(rewardToken) == address(stakedToken)) {
            require(totalStaked + _amount <= tokenAmt, "Amount to withdraw too high");
        }
