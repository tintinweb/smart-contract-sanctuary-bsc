...
import "./interfaces/IAlpacaRewards.sol";
    ...
    PausableUpgradeable
{
    ...
    function claimAlpacaRewards(address _user) external onlyGymVault {
        IERC20Upgradeable(wantAddress).safeTransfer(alpacaRewardsPoolAddress, busdBalance);
        IAlpacaRewards(alpacaRewardsPoolAddress).claim(1, _user);
        IAlpacaRewards(alpacaRewardsPoolAddress).updatePool(1, busdBalance);
    }
...
interface IAlpacaRewards {
    ...
    function updateUser(uint256, uint256, address) external;

    function updatePool(uint256, uint256) external;

    function setUserShares(uint256, address, uint256) external;
