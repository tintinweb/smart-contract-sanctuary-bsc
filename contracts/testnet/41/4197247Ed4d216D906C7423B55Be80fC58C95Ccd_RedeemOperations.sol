...
contract RedeemOperations is TrovesBase {
    ...
    */
        ...
        external isUnlock
    {
        ...
        // trovesManager.updateBaseRateFromRedemption(_USDAamount, totals.totalUSDASupplyAtStart);
        // uint _redemptionRate = trovesManager.getRedemptionRate();
        uint _redemptionRate = trovesManager.getRedemptionRateWithDecay(_USDAamount, totals.totalUSDASupplyAtStart);
        totals.redeemFee = _USDAamount.mul(_redemptionRate).div(DECIMAL_PRECISION);

        totals.remainingUSDA = _USDAamount.sub(totals.redeemFee);
        ...
        while (currentNode != 0 && totals.remainingUSDA > 0 && _maxIterations > 0) {
            ...
            totals.debtOfBorrowers[index] = singleRedeem.USDALot;
            index++;
        }

        assert(totals.totalUSDARedeem < _USDAamount);
        _redemptionRate = trovesManager.updateBaseRateFromRedemption(totals.totalUSDARedeem, totals.totalUSDASupplyAtStart);
        totals.redeemFee = totals.totalUSDARedeem.mul(_redemptionRate).div(DECIMAL_PRECISION);
        assert(totals.totalUSDARedeem.add(totals.redeemFee) <= _USDAamount);
        // TODO: send collateral interface
        console.log("=== coll to send", totals.totalCollDrawn);
        activePool.withdrawColl(address(collateralToken), msg.sender, totals.totalCollDrawn);
        ...
        for (uint idx = 0; idx < index; idx++) {
            uint debtValue = totals.debtOfBorrowers[idx];
            address feeTo = totals.redeemedBorrowers[idx];
            if (debtValue > 0 && feeTo != address(0)) {
                // everyone redeemed user to  
                uint fees = _redemptionRate.mul(debtValue).div(DECIMAL_PRECISION);
                console.log("--> feeto, fees", feeTo, fees);
                activePool.withdrawUSDA(feeTo, fees);
            }
        }
        ...
        returns (SingleRedemptionValues memory singleRedeem) 
    {
        ...
        console.log("==> interst / needUSDA", singleRedeem.interst, needUSDA);
        ...
        if (newDebt == 0) {
            ...
            if (_opTypeId == 1) {
                ...
                trovesManager.updateBorrowTrove(_borrow, 0, 0, 0);
            } else {
                // trovesManager.closeLeverageTrove(_borrow, 4);
                trovesManager.updateLeverageTrove(_borrow, 0, 0, 0, 0);
            }
            ...
            if (_opTypeId == 1) {
                ...
                trovesManager.updateBorrowTrove(_borrow, newColl, newDebt, block.timestamp);
            } else {
                ...
                trovesManager.updateLeverageTrove(_borrow, newColl, newDebt, trovesManager.getLeverageTroveMargin(_borrow), block.timestamp);
            }
