    ...
    Ownable
{
    ...
    mapping(address => bool) public whileListAsset;

    function setWhileListAsset(address asset, bool isWhileList) public onlyOwner {
        whileListAsset[asset] = isWhileList;
    }

    function getPosiNFTFactory() external override view returns (address) {
    ...
    function verifyRequire(
        ...
        uint256 collateralAmountTransfer
        ...
        if (
            _getLengthString(bondInformation.description) > 300 ||
            _getLengthString(bondInformation.description) == 0
        ) return false;
        if (
            _getLengthString(bondInformation.bondSymbol) > 10 ||
            _getLengthString(bondInformation.bondSymbol) == 0
        ) return false;

        if (
            !whileListAsset[assetInformation.underlyingAsset] ||
            !whileListAsset[assetInformation.faceAsset]
        ) return false;

        //        if (bondInformation.startSale < block.timestamp + 1 hours) return false;

        if (
            bondInformation.issuePrice == 0 ||
            assetInformation.faceValue == 0 ||
            bondInformation.totalSupply == 0 ||
    ...
    function verifyRequireToken(
        uint256 collateralAmount,
        uint256 collateralAmountTransfer
    ) internal view returns (bool) {
        if (
            collateralAmountTransfer < collateralAmount ||
            collateralAmount < 10_000 * 10**18 ||
            !_coverRFI(collateralAmount, collateralAmountTransfer)
        ) {
            return false;
        }
        return true;
    ...
    function verifyRequireNFT(uint256[] memory nftIds, uint256 collateralAmount)
        internal
        view
        returns (bool)
    {
        (bool isPass, uint256 totalPairAmount) = checkNft(nftIds);

        if (!isPass) return isPass;

        if (
            nftIds.length > 30 ||
            totalPairAmount < collateralAmount ||
            collateralAmount < 10_000 * 10**18 ||
            !_coverRFI(collateralAmount, totalPairAmount)
        ) {
            return false;
        }
        return true;
    ...
    function checkNft(uint256[] memory nftIds)
        internal
        view
        returns (bool, uint256 totalPairAmount)
    {
    ...
    function verifyAddCollateral(
        ...
        if (underlyingAssetType == 0) {
            require(
                amountTransferAdded.length == 1,
                "amountAdded length must be 1"
            );
            return _coverRFI(amountTransferAdded[0], amountAdded);
        }
        if (underlyingAssetType == 1) {
            ...
            if (!isPass) return isPass;
            return _coverRFI(amountAdded, totalPairAmount);
        }
        return true;
    }
    ...
    function verifyRemoveCollateral(
        ...
        if (underlyingAssetType == 1) {
            ...
            uint256 amount99PercentRemove = _calc99Percent(totalRemove);
            if (underlyingAmount <= amount99PercentRemove) {
                return 0;
            }
            (, uint256 totalPairAmount) = checkNft(nftIds);

            if (
                !_coverRFI(
                    underlyingAmount - amount99PercentRemove,
                    totalPairAmount - totalRemove
                )
            ) {
                return 0;
            }

            return amount99PercentRemove;
        }
        return 0;
    }
    ...
    function liquidated(uint256 underlyingAssetType, uint256[] memory nftIds)
        external
    {
        if (underlyingAssetType == 1) {
            for (uint256 i = 0; i < nftIds.length; i++) {
                _posiNFTFactory.burn(nftIds[i]);
            }
        }
    }

    function getTokenMapped(address nft) external override view returns (address) {
        return mappingNftToToken[nft];
    }

    function setTokenMapped(address nft, address tokenMapped) external {
    ...
    function _coverRFI(
        ...
        return _calc99Percent(collateralAmountTransfer) >= collateralAmount;
    }

    function _calc99Percent(uint256 amount) internal view returns (uint256) {
        return (amount * 9_900) / 10_000;
    }


    //TODO remove when on prod
    uint256 public mockTime;
    function setMockTime(uint256 mockTime_) public {
        mockTime = mockTime_;
    }
    ...
    ReentrancyGuard
{
    ...
    {
        setIssuer(issuer_);
        BondStatus.setDuration(bondInformation.duration);
        BondStatus.active(
    ...
    function initialize(
        IPositionBondFunction positionBondFunction_,
        address chainLinkPriceFeed_,
        address positionAdmin_
    ) external {
        require(!initialized, "PositionBondLending: already initialized");
        initialized = true;
        positionBondFunction = positionBondFunction_;
        LoanRatio.initChainLinkPriceFeed(chainLinkPriceFeed_);
        PositionAdmin.initAdmin(positionAdmin_);
    }
    ...
    function isNotReachMaxLoanRatio() public view returns (bool) {
        uint256 bondAmountInPhase = BondStatus.getStatus() == Status.Pending
            ? bondSupply
            : totalSupply();
        return
    ...
    function active(
        ...
        uint64 _active,
        uint64 _maturity
    ) public override(IPositionBond, BondStatus) {}
    ...
    // If underlyingAsset is NFT, amountsRemoved must be format [1,3,5]. This is index of NFTs issuer want to remove
        ...
        nonReentrant
    {
        uint256 totalAmountRemoved = positionBondFunction
            .verifyRemoveCollateral(
                ...
                UnderlyingAssetLending._underlyingAssetAmount(),
                UnderlyingAssetLending.getUnderlyingAssetType()
            );
        require(totalAmountRemoved > 0, "Not pass remove");
        require(
            LoanRatio.isNotReachMaxLoanRatio(
                UnderlyingAssetLending._underlyingAssetAmount() -
                    totalAmountRemoved,
    ...
    function addCollateral(
        uint256[] calldata amountTransferAdded,
        uint256 amountAdded
    ) public payable onlyIssuer onlyActive nonReentrant {
        require(
    ...
    function cancel() public onlyIssuer onlyPending nonReentrant {
        BondStatus._setUnderlyingAssetCanceled();
    ...
    function claimUnderlyingAsset()
        public
        override(IPositionBond, UnderlyingAssetLending)
        onlyIssuer
        onlyReadyToClaim
        nonReentrant
    {
    ...
    function claimSoldAmount(uint256 amount)
        ...
        nonReentrant
    {
        FaceAssetLending._transferOut(amount);
     ...
     */
    function claimRemainderUnderlyingAsset() external onlyActive onlyIssuer {
    }

    function purchase(uint256 amount) external override {
    }

    /**
     * @dev see {IPositionBond-purchase}
     */
    function purchaseBondLending(uint256 amount)
        external
        payable
        onlyOnSale
        notIssuer
        nonReentrant
    {
        ...
        FaceAssetLending._transferIn(_faceAmount);
     ...
     */
    function repay() external virtual onlyIssuer onlyActive {
    }


    function repayBondLending() external virtual payable onlyIssuer onlyActive {
    ...
    function liquidate() external onlyPositionAdmin {
        if (isLiquidate()) {
            if (UnderlyingAssetLending.getUnderlyingAssetType() == 1) {
                address posiNftFactory = address(
                    positionBondFunction.getPosiNFTFactory()
                );
                UnderlyingAssetLending.aprroveBurnNFT(posiNftFactory);
            ...
            emit Liquidated(address(this), msg.sender);
        }
    }
    ...
    function isLiquidate()
        public
        view
        override(UnderlyingAssetLending, FaceAssetLending)
        returns (bool)
    {
        if (BondStatus.getStatus() == Status.Repaid) {
            return false;
        }
        return
            (BondStatus._isMatured() ||
    ...
    function _transferUnderlyingAsset()
        internal
        override(UnderlyingAssetLending, BondStatus)
    {}
...
abstract contract Issuer {
    ...
    // TODO consider use this function
...
abstract contract BondStatus {
    ...
    function _updateTimePhase() internal virtual {
        ...
        require(_active < _maturity, "!time");
        require(getStatus() == Status.OnSale, "!OnSale");
        statusData.active = Timestamp.wrap(_active);
        statusData.maturity = Timestamp.wrap(_maturity);
        liquidationTime = Timestamp.wrap(_maturity);
    ...
    function setDuration(uint64 duration_) internal {
        ...
        liquidationTime = Timestamp.wrap(
            statusData.active.unwrap() + duration.unwrap()
        );
    }
}
...
abstract contract UnderlyingAssetLending {
    ...
    IERC721 private _underlyingAssetPosiNFT;
    uint256[] private _posiNFTsID;

    IERC20 private _underlyingAssetEther;
    ...
    function getUnderlyingAssetType() internal view virtual returns (uint256) {
        return underlyingAssetType;
    }

    function getNfts() internal view virtual returns (uint256[] memory) {
        return _posiNFTsID;
    }
    ...
    function _addCollateral(
        ...
        if (underlyingAssetType == 0) {
            ...
            require(msg.value >= amountAdded, "Not enough ether");
        }
    ...
    function _transferUnderlyingAsset() internal virtual {
        ...
        //            require(
        //                _underlyingAsset.balanceOf(address(this)) >= _underlyingAmount,
        //                "need to cover deflection fees"
        //            );
        //        }
    }

    function isLiquidate() public view virtual returns (bool) {}
    ...
    function aprroveBurnNFT(address positionFactory) internal {
    ...
    function decompose(address positionFactory, address tokenPar) internal {
        for (uint256 i = 0; i < _posiNFTsID.length; i++) {
            IPosiNFTFactory(positionFactory).burn(_posiNFTsID[i]);
        }
...
abstract contract FaceAssetLending {
    ...
    constructor(
        ...
        require(
            faceAssetTypeType_ == 0 || faceAssetTypeType_ == 2,
            "not supported"
        );
...
interface IPositionBond {
    ...
    event Liquidated(address bondAddress, address caller);
...
abstract contract PositionAdmin {
    ...
    event PositionAdminChanged(address oldAdmin, address newAdmin);

    modifier onlyPositionAdmin() {
        require(msg.sender == positionAdmin, "only position admin");
        _;
    }

//    function positionAdmin() public view virtual returns (address) {
//        return _positionAdmin;
//    }

//    function changePositionAdmin(address newAdmin) public onlyPositionAdmin {
//        emit PositionAdminChanged(_positionAdmin, newAdmin);
//        _positionAdmin = newAdmin;
//    }

    function initAdmin(address newAdmin) internal virtual {
        require(positionAdmin == address(0), "admin already set");
...
abstract contract LoanRatio {
    uint256 private liquidateRate = 8000;
    uint256 private maxLoanRatio = 6500;
    uint256 twapInterval = 3 * 24 * 3600;
    ...
    function initChainLinkPriceFeed(address chainLinkPriceFeed_) internal {
        require(
            address(chainLinkPriceFeed) == address(0x0),
            "!ChainLink already initialized"
        );
    ...
    function _getLoanRatio(
        ...
        uint256 twapUnderlyingAsset = chainLinkPriceFeed.getTwapPrice(
            priceFeedKeyUnderlyingAsset,
            twapInterval
        );
        uint256 twapFaceAsset = chainLinkPriceFeed.getTwapPrice(
            priceFeedKeyFaceAsset,
            twapInterval
        );

        return
            (twapFaceAsset * amountFaceAsset * 10000) /
            (twapUnderlyingAsset * amountUnderlyingAsset);
    }
        ...
        returns (bool)
    {
        ...
        return loanRatio >= liquidateRate && loanRatio > 0;
    }
