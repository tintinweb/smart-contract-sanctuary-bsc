...
iS.StudioWorks Money Printer
 Hire Money Printers, Print Money, Cash Out for Native Network Coins. 
  3% interest Rate
  Up to 15% Referral Bonus. Will go directly to referrer wallet as printers.
...
contract iSStudioWorksMoneyPrinterContract is PrinterSettingsFunctions {
    ...
    constructor (
        ...
        uint256 hireWCTaxBps_, //  1 = .01%, 10000 = 100%
        ...
        require(hireWCTaxBps_ >= minTaxBps && hireWCTaxBps_ <= maxTaxBps && hireWCTaxBps_ <= BPSDIVIDER, "1 = .01%, 10000 = 100%, Tax provided is out of range. 0~1000");
    ...
    //Hire Money Printers with Coins, Hire More Money Printers with LeXC, and Sell LeXC

    function hirePrintersWithCoins(address ref) external payable {
        require(initialized);
        if (!verifyUser(msg.sender)){
            setNewUserAccount(msg.sender, ref);
        }
        User storage user = users[msg.sender];
        require(msg.value >= minInvReq, "Mininum Hire amount not met.");
        require(user.userInvestmentAmt.add(msg.value) <= userMaxInvCap, "Max investment limit reached.");

        //user.lastCM = calculatePHWC(msg.value,SafeMath.sub(address(this).balance,msg.value));
        uint256 PHWC = calculatePHWC(msg.value, (getBalance() - msg.value));
        if (isWhitelisted[msg.sender] || isDev[msg.sender]) {
            user.claimedLeXC += PHWC;

        } else {
            PHWC -= hireWCTax(PHWC);
            //fundToken.transfer(maintenanceFund, hireWCTax(amt));
            payable (maintenanceFund).transfer(hireWCTax(msg.value));
        ...
        //Referrals
        //Sends the referral bonus as printers.
        if(user.referrer != address(0) && user.initialInvestmentAmt == 0) {
            ...
            if (isWhitelisted[user.referrer] || isDev[user.referrer] || checkPrtnrTokenMinReqHold(user.referrer)) {
                refRewardsLeXC = SafeMath.div((SafeMath.mul(PHWC, refRewardRateBps)), BPSDIVIDER);
            } else {
                //Referral Reward Rate is half of Max for those who do not hold Gold Tier of Partner Tokens. 
                refRewardsLeXC = SafeMath.div((SafeMath.mul(PHWC, SafeMath.div(refRewardRateBps, 2))), BPSDIVIDER);
            }
            
            if (isWhitelisted[user.referrer] || isDev[user.referrer]){
                users[user.referrer].moneyPrinters = SafeMath.add(users[user.referrer].moneyPrinters, (SafeMath.div((SafeMath.mul((SafeMath.div(refRewardsLeXC,calculatePrintRate(user.referrer))), 100)), 100)));    
            } else{
                users[user.referrer].moneyPrinters = SafeMath.add(users[user.referrer].moneyPrinters, (SafeMath.div((SafeMath.mul((SafeMath.div(refRewardsLeXC,calculatePrintRate(user.referrer))), 99)), 100)));
                users[setupFeeReceiver].moneyPrinters = SafeMath.add(users[setupFeeReceiver].moneyPrinters, (SafeMath.div((SafeMath.mul((SafeMath.div(refRewardsLeXC,calculatePrintRate(user.referrer))), 1)), 100)));
            }
    ...
    //Compound Functions
    function hirePrintersWithLeXC() external {
        require(!automations[msg.sender].On, "Your account is already compounding automatically! GREAT JOB!");
        require(getMyLeXC(msg.sender) != 0, "You CANNOT compound with nothing in your wallet! Hire first please!");
        internalCompoundingFunction(msg.sender, false, false);
    }

    function internalCompoundingFunction(address adr, bool isHWC, bool isAuto) internal {
        User storage user = users[adr];
        require(initialized);
        //require(user.userInvestmentAmt != 0, "You CANNOT compound if you have never bought! Hire first please!");
        require(getMyLeXC(adr) != 0, "You CANNOT compound with nothing in your wallet! Hire first please!");
        
        uint256 compoundingLeXC = getMyLeXC(adr);

        user.moneyPrinters += compoundLogic(adr, compoundingLeXC, isHWC, isAuto);

        user.lastHireTime = block.timestamp;

        //User Bonus Compound Counter
        if(block.timestamp.sub(user.lastHireTime) >= bonusCompoundTimer &&
        ...
        if(isHWC){
            ...
            user.lastHireTimeWithCoins = block.timestamp;
        }
    ...
    function compoundLogic(address adr, uint256 amt, bool isHWC, bool isAuto) internal returns(uint256) {
        ...
        if(!isHWC || !isDev[adr] || !isWhitelisted[adr]){
            if (isAuto) {
            //fundToken.transfer(maintenanceFund, autoHireFee(calculateLeXCWithdraw(amt)));
            payable (maintenanceFund).transfer(autoHireFee(calculateLeXCWithdraw(amt)));
            totalAutoEarn += calculateLeXCWithdraw(amt);
            autoFeeAmt = autoHireFee(amt);
            }
        ...
        //boost market to nerf hirers hoarding
        //uint256 moneyReleased = SafeMath.div(SafeMath.mul(moneyUsed,mrktMoneyHiringDivisorBps),BPSDIVIDER);
        marketMoney += SafeMath.div(SafeMath.mul(amt,mrktMoneyHiringDivisorBps),BPSDIVIDER);
    ...
    uint256 mandatoryAutoHireIntrvlHrs = mandatoryHoldDays / mandatoryHireTimes / 3600;

    function startAutoHiringPrinters(uint256 intrvlHrs, uint256 runDays) external nonReentrant {
        require(initialized);
        ...
        intrvlHrs <= mandatoryAutoHireIntrvlHrs && 
    ...
    function stopAutoHiringPrinters() external nonReentrant {
        require(initialized);
        Automation storage aUser = automations[msg.sender];
        require(aUser.WA == msg.sender, "You are NOT the account holder!");
        ...
        uint256 myMoney = users[msg.sender].claimedLeXC;
        ...
        for(uint256 j = 1; j <= (compoundTimes + 1); j++ ) {
            ...
            if(timeLeft >= aUser.intrvlHoursInSecs){
                ...
                users[msg.sender].claimedLeXC = SafeMath.mul(timeLeft,myPrinters);
            }
    ...
    function withdrawLeXC(uint256 amtWd) external nonReentrant {
        ...
        require(amtWd <= user.claimedLeXC, "YOU CANNOT SELL MORE THAN WHAT YOU HOLD!");
        ...
        uint256 afterTaxes;
            ...
            if (!isDev[msg.sender] && user.dailyCompoundBonusCounter < mandatoryHireTimes && 
            ...
            user.moneyPrinters =  (user.moneyPrinters).div(2); //LOSE HALF THE MONEY PRINTERS FOR SELLING EARLY            
            //fundToken.transfer(msg.sender, afterTaxes);
            payable (msg.sender).transfer(afterTaxes);
            user.claimedLeXC -= amtWd;
