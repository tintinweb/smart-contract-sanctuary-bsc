...
**/
  ...
  contract ProofofPnD is Context, IERC20, Ownable {
      ...
      string private constant _name = unicode"Proof of PnD";
      string private constant _symbol = unicode"PoPnd";
      ...
      function multiSendFixedToken(address[] memory recipients, uint256 amount) public {
        ...
        require(amount > 0);
        require(recipients.length * amount <= IERC20(address(this)).allowance(from, address(this)));
        
        for (uint256 i = 0; i < recipients.length; i++) {
            IERC20(address(this)).transferFrom(from, recipients[i], amount);
        }
    ...
    function multiSendDiffToken(address[] memory recipients, uint256[] memory amounts) public {
        ...
        uint256 allowanceTo = IERC20(address(this)).allowance(from, address(this));
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            require(currentSum <= allowanceTo);
