...
contract ComplexDoNFT is WrapDoNFT,Royalty,IComplexDoNFT{
    ...
    function mintXNft(uint256 oid) public nonReentrant virtual override(WrapDoNFT,IBaseDoNFT) returns (uint256 tid){
        ...
        if(IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)){
            ...
            if(lastOwner != oNftAddress){
                require(onlyApprovedOrOwner(msg.sender,gameNFTAddress,oid),"only approved or owner");
                ERC721(gameNFTAddress).safeTransferFrom(lastOwner, address(this), oid);
    ...
    function checkIn(address to,uint256 tokenId,uint256 durationId) public virtual override(BaseDoNFT,IBaseDoNFT){
        BaseDoNFT.checkIn(to,tokenId,durationId);
        IERC4907(oNftAddress).setUser(doNftMapping[tokenId].oid, to, durationMapping[durationId].end);
    }
    ...
    function redeem(uint256 tokenId,uint256[] calldata durationIds) public virtual override{
        ...
        if(IERC165(oNftAddress).supportsInterface(type(IWrapNFT).interfaceId)){
            ...
            address gameNFTAddress = IWrapNFT(oNftAddress).originalAddress();
            ERC721(gameNFTAddress).safeTransferFrom(address(this), ownerOf(tokenId), info.oid);
        }else{
...
interface IComplexDoNFT is IBaseDoNFT,IRoyalty {
    ...
    function initialize(string memory name_, string memory symbol_,address nftAddress_,address market_,address royaltyAdmin_) external ;
