...
contract GBridge is ReentrancyGuardUpgradeable {
    ...
    function _SendToken(
        ...
        TransactionsTypes.TokenType _tokenType = chains[_chainId][token].tokenType;
        if (_tokenType == TransactionsTypes.TokenType.ChainToken && totalLockedAmount[token] >= rest_amount) {
            totalLockedAmount[token] -= rest_amount;
            payable(to).transfer(rest_amount);
        } else if (_tokenType == TransactionsTypes.TokenType.ERCToken && totalLockedAmount[token] >= rest_amount ) {
            totalLockedAmount[token] -= rest_amount;
            IERC20(token).transfer(to, rest_amount);
        } else if (_tokenType == TransactionsTypes.TokenType.OtherToken) {
            IGBridgeToken(token).mint(to, rest_amount);
        } else {
            revert NoCorrectTransferFound();
        }
    ...
    function Claim(
        ...
        _CheckProof(_submissionId, _fromChainId, _token, _amount, _proofs);
        _SendToken(_fromChainId, _token, _to, _amount, _claimFee);
