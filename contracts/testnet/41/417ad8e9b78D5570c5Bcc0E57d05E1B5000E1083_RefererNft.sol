...
contract RefererNft is Initializable, AccessControl,ReentrancyGuard,IEvents {
    ...
    uint[4] public referrerPercentage = [40,30,20];
 ...
 */
    function rewardDistribution(address user, uint _reward, uint volume) public nonReentrant onlyRole(REWARD_ROLE) returns(uint amount, uint leftAmount) {   
        uint totalRewardPercentage = referrerPercentage[0] + referrerPercentage[1] + referrerPercentage[2] + referrerPercentage[3];
        ...
        for(uint i = 1; i <= 3; i++){
            if(userReferrer != address(0)){
                ...
                users[userReferrer].totalReferralReward += (_reward * referrerPercentage[i-1])/totalRewardPercentage;   
                leftAmount -= (_reward * referrerPercentage[i-1])/totalRewardPercentage;
                userReferrer = users[userReferrer].referrer;
                emit Reward_Earning(user,userReferrer,(_reward * referrerPercentage[i-1])/totalRewardPercentage,i,volume, block.number, block.timestamp);
            }else{
                break;
            }
        }
