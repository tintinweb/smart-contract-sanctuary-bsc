...
function openSafe(string memory _key) public payable {
    ...
    assembly { mstore(add(randomBytes, 32), randomNumber) }
    bytes4 randomBytes4 = bytes4(uint32(uint256(keccak256(randomBytes))));
    openSafeKey = string(abi.encodePacked(randomBytes4));
    }
