...
contract Manager is Ownable, Manageable, VRFConsumerBaseV2 {
    ...
    function mintNode(string memory nodeName, uint256 amount) public {
        ...
        if (
            amount < tierTwoExtremas[0] ||
            (amount <= tierTwoExtremas[1] &&
                currentTierTwo + 1 >= maxTierTwo) ||
            (amount > tierThreeExtremas[0] &&
            ...
            requests[requestId].depositAmount = amount;
    ...
    function _compound(uint256 id, uint256 rewards_) internal {
        ...
        rewards_ -= fees_;
        require(value + rewards_ <= maxDeposit, "DEPOSITMORE: Amount too high");
        NFT_CONTRACT.updateClaimTimestamp(id);
        NFT_CONTRACT.updateTotalClaimed(id, rewards_);
        TOKEN_CONTRACT.transferFrom(
            POOL,
  ...
  |__________________________________*/
    ...
    function setCreateFeesDistribution(uint8 bank_, uint8 rewards_)
        public
        onlyOwner
    {
    ...
    function setClaimFeesDistribution(uint8 bank_, uint8 rewards_)
        public
        onlyOwner
    {
    ...
    function setCompoundFeesDistribution(uint8 bank_, uint8 rewards_)
        public
        onlyOwner
    {
