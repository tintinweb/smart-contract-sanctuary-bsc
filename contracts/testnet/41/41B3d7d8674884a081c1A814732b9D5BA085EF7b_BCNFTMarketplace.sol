...
contract BCNFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
            ...
            payable(ownerMap[_id]).transfer(_sellerValue);
            payable(creatorMap[_id]).transfer(_royaltyValue);
            payable(stakingPool).transfer(_commissionValue.div(7).mul(5)); // 5% tax goes staking pool
            payable(developmentAddress).transfer(_commissionValue.div(7).mul(2)); // 2% goes development address
        } else {
        ...
        auctionNotExpired(tokenId)
    {
        ...
        if (auctions[tokenId].amount == 0) {
            ...
            if (
                ...
                payable(auctions[tokenId].bidder).transfer(
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if (winner == address(0)) {
            ...
            if (
                ...
                payable(adminRecoveryAddress).transfer(
                    _commissionValue.div(5)
                );
                payable(developmentAddress).transfer(
                ...
                if (Creator == creatorMap[tokenId]) {
                    ...
                    payable(creatorMap[tokenId]).transfer(
                        _royaltyValue
                    );
                    payable(Creator).transfer(
