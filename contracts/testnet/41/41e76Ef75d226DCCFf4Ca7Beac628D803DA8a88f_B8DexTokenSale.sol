...
contract B8DexTokenSale is IB8DexTokenSale, Ownable {
    ...
    event LogClaim         (uint32 window, uint256 time, address user, uint256 amount1, uint256 amount2, uint256 amount3, uint256 amount);
...
//        b8dToken.burn(amount);
    ...
    function _claim(
        ...
        uint256 reward = userBuys[sender][buyStatus] * window.percent / 100;
        uint256 reward1 = userBuys[sender][buyStatus] * window.percent;
        uint256 time = _time();

        claimed[sender][buyStatus][windowIndex] = true;
        b8dToken.transfer(sender, reward);

        emit LogClaim(windowIndex, time, sender, userBuys[sender][buyStatus], window.percent, reward1, reward);
    }
