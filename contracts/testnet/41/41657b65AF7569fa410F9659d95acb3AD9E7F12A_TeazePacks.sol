...
contract TeazePacks is Ownable, Authorizable, Whitelisted, ReentrancyGuard {
    ...
    constructor(address _nftContract, address _farmingContract) {
    ...
    function checkIfLootbox() internal {
        ...
        if ((previousBalance.add(heldAmount)).sub(heldAmount) >= maxRewardAmount) {
            //create lootbox
            ...
            for (uint256 x = 0; x < nftPerLootbox; ++x) {
                ...
                NFTInfo storage nftinfo = nftInfo[LootboxNFTids[lootboxid][x]];
