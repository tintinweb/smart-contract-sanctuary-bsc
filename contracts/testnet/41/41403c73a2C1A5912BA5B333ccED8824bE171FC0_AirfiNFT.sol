...
contract AirfiNFT is AccessControlEnumerable, ERC721URIStorage, ERC721Enumerable, Pausable, Ownable {
    ...
    function _doMint (address to, string memory tokenURI) private {
   ...
   function batchMint(address to, uint number, string[] memory tokenURIs) public onlyRole(MINTER_ROLE) {
       require(tokenURIs.length == number, "BatchMint: The inputs have the same length");
       for (uint i = 0; i < number; i++) {
           _doMint(to, tokenURIs[i]);
       }
   }
