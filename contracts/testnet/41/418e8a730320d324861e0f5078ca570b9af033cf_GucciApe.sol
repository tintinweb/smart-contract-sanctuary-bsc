...
/**
 * Gucci Ape (GAPE) Dual Rewards with USDT Dividends
 * First Dual Reward with Auto-Staking, Auto-Compounding & USDT Reflections
 *
 * Website: https://gucciape.finance
 * Whitepaper: https://gucciape.gitbook.io/docs/
 * Twitter:  https://twitter.com/gucciapefinance
 * Telegram : t.me/gucciapefinance
 */
...
contract GucciApe is ERC20Detailed, Ownable {
    ...
    uint256 public gucciDividendFee = 70;
    uint256 public sellFee = 20;
    uint256 public autofirePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(gucciDividendFee).add(
            autofirePitFee
        );
    ...
    address public gucciDividendReceiver;
    ...
    constructor() ERC20Detailed("Gucci Ape", "GAPE", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x3B7b7121335642F6C9F84Fc61147eacbfF799eA8;
        treasuryReceiver = 0x6075289D09e4F252CEc1c9ab3cFc663fda4Efb92;   
        ...
        gucciDividendReceiver = address(distributor);
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(gucciDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(gucciDividendFee)
            ),
            gas: 30000
        }("");

        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(gucciDividendFee).div(treasuryFee.add(gucciDividendFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no GAPE token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
