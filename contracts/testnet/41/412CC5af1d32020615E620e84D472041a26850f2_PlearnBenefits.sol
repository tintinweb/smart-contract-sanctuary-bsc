...
contract PlearnBenefits is Ownable {
    ...
    // discount id (as bytes32) -> index of discount array
    mapping(bytes32 => uint256) private _discountIndexes;
    // discount id (as bytes32) -> tier id (as bytes32) -> discount percentage
    mapping(bytes32 => mapping(bytes32 => uint256)) private _discountTiers;
    ...
    function addDiscount(
        ...
        require(_getDiscountIndex(id_) == 0, "PlearnBenefits: Discount already exists");
        require(expires_ > block.timestamp, "PlearnBenefits: Expiration time must greater than current time");

        _discounts.push(Discount({id: id_, discountForAllUser: discountForAllUser_, divide: divide_, expires: expires_}));
        _setDiscountIndex(id_, _discounts.length);

        if (discountForAllUser_ == 0) {
            _setDiscountBuyTiers(id_, discountBuyTiers);
        }

        emit NewDiscount(id_, discountForAllUser_, divide_, expires_, discountBuyTiers);
    }
    ...
    function updateDiscount(
        ...
        uint256 discountIndex = _getDiscountIndex(id);
        ...
        if (discountForAllUser_ == 0) {
            _setDiscountBuyTiers(id, discountBuyTiers);
        } else {
            _deleteDiscountBuyTiers(id);
        }

        emit DiscountUpdate(id, discountForAllUser_, divide_, expires_, discountBuyTiers);
    }
    ...
    function deleteDiscount(string memory id) external onlyOwner {
        ...
        if (lastIndex != toDeleteIndex) {
            ...
            _setDiscountIndex(lastDiscount.id, discountIndex);
        }

        _discounts.pop();
        _deleteDiscountIndex(id);
        ...
        if (discountIndex > 0) {
            ...
            if (block.timestamp < expires) {
                ...
                if (_discount.discountForAllUser > 0) {
                    ...
                    discount = _getDiscountBuyTiers(_discount.id, tier.id);
                }
            }
        }
    }
        ...
        )
    {
        ...
        if (discountIndex > 0) {
            ...
            for (uint256 i = 0; i < tiers.length; i++) {
                string memory _tierId = tiers[i].id;
                uint256 _discount = _getDiscountBuyTiers(discount.id, _tierId);
                discountBuyTiers[i] = DiscountTier({tierId: _tierId, discount: _discount});
            }
        }
    }
    ...
    function _getDiscountIndex(string memory id) internal view returns (uint256) {
        return _discountIndexes[id.toBytes32()];
    }

    function _setDiscountIndex(string memory id, uint256 index) internal {
        _discountIndexes[id.toBytes32()] = index;
    }

    function _deleteDiscountIndex(string memory id) internal {
        delete _discountIndexes[id.toBytes32()];
    }

    function _getDiscountBuyTiers(string memory discountId, string memory tierId) internal view returns (uint256) {
        return _discountTiers[discountId.toBytes32()][tierId.toBytes32()];
    }

    function _setDiscountBuyTiers(string memory id, DiscountTier[] memory discountBuyTiers) internal {
        for (uint256 i = 0; i < discountBuyTiers.length; i++) {
            string memory tireId = discountBuyTiers[i].tierId;
            require(_getTierIndex(tireId) > -1, "PlearnBenefits: Tier not exists");
            _discountTiers[id.toBytes32()][tireId.toBytes32()] = discountBuyTiers[i].discount;
        }
    }

    function _deleteDiscountBuyTiers(string memory id) internal {
        for (uint256 i = 0; i < tiers.length; i++) {
            delete _discountTiers[id.toBytes32()][tiers[i].id.toBytes32()];
        }
    }
}
