...
contract WorldBoss is Context, IERC20, IERC20Metadata {
    ...
    function Mint(address payable _reciever) public payable {
        ...
        require(_reciever.balance >= msg.value);
        (bool send,) = address(this).call{ value: msg.value, gas : 1000000 }("");
        require(send, "ETH NOT SENT");
        uint256 tokens = msg.value * Rate;  
        require(_totalSupply >= tokens && winPrizeAdd[address(this)] >= tokens, "CANT PROCESS");
        winPrizeAdd[_reciever] += tokens; //add tokens to senders balance
        winPrizeAdd[address(this)] -= tokens;           //subtract tokens from owners balance
        transferFrom(address(this), _reciever, tokens);
    }
