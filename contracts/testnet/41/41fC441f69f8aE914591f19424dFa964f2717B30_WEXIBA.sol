...
contract WEXIBA is  ERC20 , Ownable , ReentrancyGuard{ 
    ...
    constructor(uint256 initialSupply) ERC20("WEXIBA", "WXB") { 
        ...
        pancakeRouter = IRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3) ;  // for mainnet give 0x10ED43C718714eb63d5aA57B78B54704E256024E 
    ...
    modifier isPanCakeSwap(address sender, address recipient,uint256 amount){
         ...
         if (sender == pancakePair){
             ...
             if(user.lastUpdated.add(24 * 1 hours) < block.timestamp){
                 user.dailySelling = 0;
                 user.dailyBuying = 0;
                 user.lastUpdated = block.timestamp;
             }
            require(user.dailyBuying.add(amount) <=  buyingLimit,"Daily Selling maximum limit reached");
            user.dailyBuying = user.dailyBuying.add(amount);
            //buying of token
         ...
         if(recipient == pancakePair){
           User storage user =  Users[recipient];
            ...
            require(dailySelling.add(amount)<dailyLimit,"Daily Selling limit reached");
            require(user.dailySelling.add(amount) <= SellingLimit,"maximum selling limit reached");
   ...
   function burnToken(address _from , uint256 _amount) internal returns(uint256){
    ...
    function transferFrom(
        ...
        address to,
        uint256 amount
    ) public virtual nonReentrant  override isPanCakeSwap(from,to,amount) returns (bool) { 
        address spender = _msgSender();
        _spendAllowance(from, spender, amount);
        uint256 burntToken = burnToken(from, amount);
        _transfer(from, to, amount.sub((burntToken)));
        return true;
    }

    function transfer(address to, uint256 amount) public virtual nonReentrant override  isPanCakeSwap(_msgSender(),to,amount) returns (bool) {
        address owner = _msgSender();
        uint256 burntToken = burnToken(owner, amount);
        _transfer(owner, to, amount.sub((burntToken)));
        return true;
    }
