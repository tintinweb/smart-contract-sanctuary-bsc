...
contract Staking is GlobalsAndUtils, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function getStake(address stakeOwner, uint256 stakeId) external view returns(Stake memory, uint256) {
          UserXTickets storage xticket = xTickets[stakeOwner][stakeId];
        Stake memory usrStake = stakes[stakeOwner][stakeId];
         (, uint256 stakedDays) = _getUserStakedTime(usrStake);
        uint256 alreadyClaimed = block.timestamp.sub( xticket.claimTime);
    ...
    function _getUserStakedTime(Stake memory usrStake) private view returns (uint256 unixStakedTime, uint256 stakedDays) {
        ...
        stakedDays = unixStakedTime.div(10 minutes);
    }
    ...
    function _getClaimableXTickets(address _stakeOwner, uint256 _stakeId) private returns(uint256) {
        ...
        if(xticket.claimable == 0){
            xticket.claimable = usrStake.stakedAt;
        }
