...
contract BCKGAME is IERC721Receiver {
    ...
    function _harvest(uint256 _tokenId, uint8 _position) private {
        ...
        if (reward > 0) {
            ...
            if (land.times > 0) {
                ...
                uint256 _epoch_day = land.nextHarvestAt / epoch;
                uint256 _epoch_day2 = game.nextHarvestAt / epoch;
        ...
        returns (uint256, uint256)
    {
        ...
        if (_tokenId > 0) {
            ...
            if (game.status && _lv > 0) {
                uint256 reward1 = ((_lv / 5 + 2) * _reward) / 100;
                uint256 reward2 = ((_lv * 5 + 5) * _reward) / 1000;
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > land.nextHarvestAt && land.times > 0) {
            return (getGameLevel(_tokenId) * 8 + 35) * 10000 * baseBck;
        }
        return 0;
    }
