    ...
    ICryptonaireLottery
{
    ...
    // Keeps track ticket id per lottery
    mapping(uint256 => uint32) public currentTicketId;
    ...
    mapping(uint256 => mapping(uint32 => bool)) private _purchasedTicket;
        ...
        nonReentrant
    {
        ...
        require(
            currentTicketId[_lotteryId] + _ticketNumbers.length <=
                maxNumberTicketsPerLottery,
            "Too many tickets per lottery"
        );
        ...
        for (uint256 i = 0; i < _ticketNumbers.length; i++) {
            ...
            uint32 _currentTicketId = currentTicketId[_lotteryId];
            ...
            _tickets[_lotteryId][_currentTicketId] = Ticket({
        ...
        nonReentrant
    {
        ...
        require(
            currentTicketId[_lotteryId] == maxNumberTicketsPerLottery,
            "Ticket not sold out"
        );
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInUSDT) *
                (BASE_DENOMINATOR - _lotteries[_lotteryId].treasuryFee))
        ) / BASE_DENOMINATOR;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInUSDT - amountToShareToWinners);
    ...
    function claimTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            require(
                currentTicketId[_lotteryId] > thisTicketId,
                "TicketId too high"
            );
            require(
                msg.sender == _tickets[_lotteryId][thisTicketId].owner,
                "Not the owner"
            );

            // Update the lottery ticket owner to 0x address
            _tickets[_lotteryId][thisTicketId].owner = address(0);
    ...
    function viewTicketNumber(uint256 _lotteryId, uint32 _ticketId)
        ...
        returns (uint32)
    {
        return _tickets[_lotteryId][_ticketId].number;
    }
     ...
     */
    function viewTicket(uint256 _lotteryId, uint32 _ticketId)
        external
        view
        returns (Ticket memory)
    {
        return _tickets[_lotteryId][_ticketId];
    }
    ...
    {
        uint256 length = currentTicketId[_lotteryId];
        uint32[] memory ticketNumbers = new uint32[](length);

        for (uint32 i = 0; i < length; i++) {
            ticketNumbers[i] = _tickets[_lotteryId][i].number;
        }
    ...
    function viewNumbersAndStatusesForTicketIds(
        ...
        for (uint256 i = 0; i < length; i++) {
            ticketNumbers[i] = _tickets[_lotteryId][_ticketIds[i]].number;
            if (_tickets[_lotteryId][_ticketIds[i]].owner == address(0)) {
    ...
    function viewRewardsForTicketId(
        ...
        if ((_ticketId > maxNumberTicketsPerLottery)) {
            return 0;
        }
        ...
        )
    {
        ...
        uint32[] memory lotteryTicketIds = new uint32[](length);
        uint32[] memory ticketNumbers = new uint32[](length);
        bool[] memory ticketStatuses = new bool[](length);
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            ticketNumbers[i] = _tickets[_lotteryId][lotteryTicketIds[i]].number;

            // True = ticket claimed
            if (_tickets[_lotteryId][lotteryTicketIds[i]].owner == address(0)) {
                ticketStatuses[i] = true;
            } else {
        ...
        )
    {
        ...
        uint32[] memory lotteryTicketIds = new uint32[](
    ...
    function _calculateRewardsForTicketId(
        ...
        uint32 userNumber = _revertNumber(
            _tickets[_lotteryId][_ticketId].number
        );
