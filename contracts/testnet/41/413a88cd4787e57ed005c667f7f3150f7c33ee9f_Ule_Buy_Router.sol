...
contract Ule_Buy_Router is Ownable {
    ...
    payable
    {
        ...
        (uint256 amountToken, uint256 amountETH, uint256 liquidity) = addLiquidity(returnValues[1],half);
        returnToken = (amountToken.mul(SWAPTokenPercentage)).div(100);
    ...
    function addLiquidity(uint256 _amount,uint256 half) public payable returns (uint amountToken, uint amountETH, uint liquidity)
    {
        return Router.addLiquidityETH{value:half}(
            address(ULE),
            _amount,
            0,
            0,
