...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    function getLuckyStatus(uint256 _luckyId) external view override returns (Status) {
        return _lucks[_luckyId].status;
    }

    function getLuckyNumbers(uint256 _luckyId) external view override returns (uint32[] memory) {
        return _lucks[_luckyId].luckyNumbers;
    }

    function getReserveNumbers(uint256 _luckyId) external view override returns (uint32[] memory) {
        return _lucks[_luckyId].reserveNumbers;
    }
    ...
    {
        id = _getTicketIdFromTicketNumber(_luckyId, _ticketNumber);
        owner = _tickets[id].owner;
        ...
        )
    {
        ...
        all = _userTicketIdsPerLuckyId[_user][_luckyId].length;
    }

    function isEnableReserve(uint256 _luckyId) public view returns (bool) {
        return _lucks[_luckyId].enableReserve;
    }
