...
interface IBEP20 {
    ...
    event ReferralReward(address indexed referrer, uint256 referralReward);
}
...
contract DividendDistributor is IDividendDistributor {
    ...
    address public RWRDToken = 0x2170Ed0880ac9A755fd29B2688956BD959F933F8;
    IBEP20 public RWRD = IBEP20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8);
...
contract SAC is IBEP20, Auth {
    ...
    string constant _name = "SAC";
    string constant _symbol = "SAC";
    ...
    uint32 public buyCooldownTimerInterval = 60;
    bool public sellCooldownEnabled = false;
    uint32 public sellCooldownTimerInterval = 60;
    bool public cooldownEnabled = false;
    uint256 public cooldownTimerInterval = 120;
    // uint256 public cooldownTimerInterval = 60 * 60 * 24 * 7;
    uint256 public cooldownLimit = 340000 * 10**8;

    bool public walletTransferEnabled = false;
    uint256 public walletTransferLimit = 340000 * 10**8;

    // uint256 public cooldownLimitStartingTime = block.timestamp;
    mapping (address => uint) private buyCooldownTimer;
    mapping (address => uint) private sellCooldownTimer;
    mapping (address => uint) private cooldownTimer;
    mapping (address => uint256) private userSellCooldownTimer;
    mapping (address => uint256) private userMaxSellTxLimit;
    mapping (address => uint256) private userMaxTransferTxLimit;
    mapping (address => uint256) private cooldownLimitStartingTime;
    mapping (address => uint256) private walletTransferCountdown;
    mapping (address => bool) private walletTransferBanned;
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0xc030337eD98ae3BEE7c07658CDBb2F62FeE37B42;
        marketingFeeReceiver = 0x9C433aEAE3A746e729b665430c0fEAc3E4832789;
        growthfundFeeReceiver = 0xE9e0F5D1E0c3063B47f3035953501Aa56312Bf4E;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {        
        ...
        if(cooldownEnabled && cooldownLimit != 0 && recipient == pair){
            require(amount <= cooldownLimit, "Amount is greater than weekly cap");
        }

        require(walletTransferBanned[sender] || walletTransferBanned[recipient] == false, "You are banned, please contact support.");

        if(walletTransferEnabled && sender != pair && recipient != pair && isWhitelisted[recipient] == false && isWhitelisted[sender] == false){
            if(walletTransferCountdown[sender] >= walletTransferLimit){
                walletTransferBanned[sender] = true;
                walletTransferBanned[recipient] = true;
            }else if(walletTransferCountdown[sender] < walletTransferLimit){
                walletTransferBanned[sender] = false;
                if(walletTransferCountdown[sender] + amount < walletTransferLimit){
                    walletTransferCountdown[sender] = walletTransferCountdown[sender] + amount;
                }else{
                    walletTransferBanned[sender] = true;
                    walletTransferBanned[recipient] = true;
                    require(walletTransferCountdown[sender] + amount < walletTransferLimit, "Transfer amount is greater than the cap.");
                }
            }
        }
        ...
        if (recipient == pair && cooldownEnabled && isWhitelisted[recipient] == false && isWhitelisted[sender] == false) {   
            if(amount > cooldownLimit){
                require(!cooldownEnabled, "Amount is greater than selling cap");
            }
            require(amount <= cooldownLimit,"Amount is greater than selling cap");
            if(cooldownLimitStartingTime[sender] != 0){
                if(cooldownLimitStartingTime[sender] < block.timestamp){
                    cooldownLimitStartingTime[sender] = block.timestamp + cooldownTimerInterval;
                    cooldownTimer[sender] = 0;
                }
            } else {
                    cooldownLimitStartingTime[sender] = block.timestamp + cooldownTimerInterval;
                    cooldownTimer[sender] = 0;
            ...
            require(cooldownTimer[sender] + amount <= cooldownLimit,"Selling cap reached");
            
            if(cooldownTimer[sender] != 0) {
                cooldownTimer[sender] = amount + cooldownTimer[sender];
            } else {
                cooldownTimer[sender] = amount;
            }
        }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isBuy) {
            ...
            if(referrerRewardEnabled && isWhitelisted[recipient] && isFirstBuy[recipient]) {//referredbuy
                ...
                emit ReferralReward(recipient, userReferralReward[referParent[recipient]]);
    ...
    function checkRemainingTimeForAnAddress(address _user) public view returns(uint256) {
        return cooldownLimitStartingTime[_user];
    }
    
    function isBanned(address _user) public view returns(bool) {
        return walletTransferBanned[_user];
    }
    
    function checkRemainingSellLimitForAnAddress(address _user) public view returns(uint256) {
        if(cooldownTimer[_user] == 0){
            return cooldownLimit;
        }else{
            return cooldownLimit - cooldownTimer[_user];
        }
    }
    ...
    function LDXCDE(bool LDXCDE_1, uint32 LDXCDE_2, bool LDXCDE_3, uint32 LDXCDE_4) public onlyOwner {
    ...
    function LDXCDEW(bool LDXCDEW_1, uint256 LDXCDEW_2, uint256 LDXCDEW_3) public onlyOwner {
        cooldownEnabled = LDXCDEW_1;
        cooldownTimerInterval = LDXCDEW_2;
        cooldownLimit = LDXCDEW_3 * 10**8;
    }
    
    function LDXCDET(bool LDXCDET_1, uint256 LDXCDET_2) public onlyOwner {
        walletTransferEnabled = LDXCDET_1;
        walletTransferLimit = LDXCDET_2 * 10**8;
    }
    
    function LDXCDEUB(address LDXCDEUB_1) public authorized {
        walletTransferBanned[LDXCDEUB_1] = false;
        walletTransferCountdown[LDXCDEUB_1] = 0;
    }
