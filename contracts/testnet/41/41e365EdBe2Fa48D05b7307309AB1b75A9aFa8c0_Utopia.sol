...
contract Utopia is  Ownable {
    ...
    function getMintPrices(uint256 mintPrices_) public onlyOwner {
    ...
    function setRecommender(address recommender) public {
        ...
        if(nowTime() > lastTime && lastTime != 0) {
            userStatisticsReward();
            lastTime = nowTime();
        }
    }
    ...
    function adminStatisticsReward() public onlyAdmin {
        statisticsReward();
    }

    function userStatisticsReward() public {
        if(autoStatisticsReward){
            statisticsReward();
        }
    }
