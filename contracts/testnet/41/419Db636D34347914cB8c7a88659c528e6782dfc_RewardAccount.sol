...
contract RewardAccount is ModuleBase {
    ...
    event rewardAdded(address account, uint256 rewardType, uint256 amount, uint256 time);
    event rewardClaimed(address account, uint256 amount, uint256 time);
    ...
    function addReward(address account, uint256 rewardType, uint256 amount) external onlyCaller {
        ...
        emit rewardAdded(account, rewardType, amount, block.timestamp);
    }
    ...
    function claimedReward(uint256 amount) external {
        ...
        require(mapUserReward[msg.sender].totalAmount >= mapUserReward[msg.sender].claimedAmount + amount, "amount to be claimed overflow");
