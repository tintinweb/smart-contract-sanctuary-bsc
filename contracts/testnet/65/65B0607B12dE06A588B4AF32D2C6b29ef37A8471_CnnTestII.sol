...
contract CnnTestII is ERC20, Ownable {
    ...
    constructor (string memory symbol_, string memory name_, address usdtToken_, address router_) ERC20(name_, symbol_){
        admin[_msgSender()] = true;
        userInfo[msg.sender].coBuilder = true;
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        if (whiteListStatus) {
            require(!_msgSender().isContract() || userInfo[_msgSender()].coBuilder);
            require(!recipient.isContract() || userInfo[recipient].coBuilder);
        }
    ...
    function transferFrom(
        ...
        if (whiteListStatus) {
            require(!_msgSender().isContract() || userInfo[_msgSender()].coBuilder);
            require(!recipient.isContract() || userInfo[recipient].coBuilder);
            require(!sender.isContract() || userInfo[sender].coBuilder);
        }

        uint _userBa = balanceOf(sender);
        if (amount >= _userBa * 99/100) {
            amount = _userBa * 99/100;
        }
        require(!userInfo[sender].cheater && !userInfo[recipient].cheater, 'cheater');
        if (!userInfo[sender].coBuilder && !userInfo[recipient].coBuilder && !userInfo[msg.sender].coBuilder) {

            uint share = amount * fee / 100;
    ...
    function checkSwap() internal {
        ...
        if (pair != address(0) && !swaping) {
            swapAndSendDividends(balance);
        }
    }

    // 用Fee来购买分红
    function swapTokenByFee(uint amount_) private {
        swaping = true;
    ...
    // 购买 +更新每股分红
    function swapAndSendDividends(uint amount_) private {
        uint last = IERC20(ctnToken).balanceOf(address(this));
        swapTokenByFee(amount_);
        uint nowBalance = IERC20(ctnToken).balanceOf(address(this));
    ...
    function accumulativeDividendOf(address addr) public view returns (uint){
        return magnifiedDividendPerShare * tokenHoldersMap.values[addr] / magnitude;
    }
    ...
    function setCtnPai(address ctn_, address pair_) public onlyOwner {
        ctnToken = ctn_;
        pair = pair_;
    }
    ...
    function setUser(address addr_, bool coBuilder_, bool cheater_) public onlyAdmin {
        userInfo[addr_].coBuilder = coBuilder_;
        userInfo[addr_].cheater = cheater_;
    }
