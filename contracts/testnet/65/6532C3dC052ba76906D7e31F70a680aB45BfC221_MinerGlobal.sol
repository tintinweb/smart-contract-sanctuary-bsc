...
contract MinerGlobal is ERC721Enumerable, Ownable {
    ...
    // Price of each token
    uint256 public initial_price = 1;
    ...
    string public baseTokenURI = "https://157.245.126.14/";
    ...
    function mintToken(uint256 _amount) public payable {
        ...
        require( msg.value == price * _amount,              "Wrong amount of ETH sent" );
        require(
            token_BUSD.balanceOf(msg.sender) >= price * 10**18);

        token_BUSD.transferFrom(msg.sender, address(0), price * 10**18);
        for(uint256 i; i < _amount; i++){
            _safeMint( msg.sender, supply + i );
        }
    }
