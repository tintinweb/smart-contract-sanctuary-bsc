...
contract MineralSwap {
    ...
    uint8 constant internal buyFee_ = 25;
   ...
   /*================================
        ...
        public
    {
        ...
        uint256 _dividends = SafeMath.div(SafeMath.mul(_ethereum, sellFee_), 100); //10%卖出手续费
        uint256 _devPool = SafeMath.div(SafeMath.mul(_ethereum,15),1000);//1.5%放入开发池
                devPool = SafeMath.add(devPool,_devPool);//将1.5%加入开发池
        uint256 _treasurePool = SafeMath.div(SafeMath.mul(_ethereum,5),100);//5%放入宝箱
        treasuryAddress.transfer(_treasurePool);//转入宝箱
        uint256 _taxedEthereum = SafeMath.sub(_ethereum, _dividends);
        _dividends = SafeMath.sub(_dividends,_devPool);//8.5% = 10%-1.5%
        _dividends = SafeMath.sub(_dividends,_treasurePool);//3.5% = 8.5%-5%
        totalDividends = SafeMath.add(totalDividends,_dividends);//将剩余3.5放入总手续费池
        ...
        returns(uint256)
    {
        // data setup
        address _customerAddress = msg.sender;
        uint256 _undividedDividends = SafeMath.div(SafeMath.mul(_incomingEthereum,buyFee_),100); //买入手续费 25%
        uint256 _referralBonus = SafeMath.div(SafeMath.mul(_incomingEthereum, 20), 100); //推荐总共20%
        uint256 _dividends = SafeMath.sub(_undividedDividends, _referralBonus);//剩余5%
        uint256 _devPool = SafeMath.div(SafeMath.mul(_incomingEthereum,15),1000);//1.5%放入开发池
                _dividends =  SafeMath.sub(_dividends, _devPool);//剩余3.5% = 5%-1.5%
                devPool = SafeMath.add(devPool,_devPool);//将1.5%加入开发池
        uint256 _taxedEthereum = SafeMath.sub(_incomingEthereum, _undividedDividends);//扣除手续费后的实际购买本金
        uint256 _amountOfTokens = ethereumToTokens_(_taxedEthereum);
        ...
        internal
    {
        ...
        if (_affAddr1 != address(0) && (tokenBalanceLedger_[_affAddr1] >= stakingRequirement || _affAddr1 == devAddress_)) {
            _affRewards = SafeMath.div(SafeMath.mul(_incomingEthereum, 13), 100);
        ...
        if (_affAddr2 != address(0) && (tokenBalanceLedger_[_affAddr2] >= stakingRequirement || _affAddr1 == devAddress_)) {
            _affRewards = SafeMath.div(SafeMath.mul(_incomingEthereum, 10), 100);
            _affSent = SafeMath.sub(_affSent,_affRewards);
        ...
        if (_affAddr3 != address(0) && (tokenBalanceLedger_[_affAddr3] >= stakingRequirement || _affAddr1 == devAddress_)) {
            _affRewards = SafeMath.div(SafeMath.mul(_incomingEthereum, 7), 100);
            _affSent = SafeMath.sub(_affSent,_affRewards);
