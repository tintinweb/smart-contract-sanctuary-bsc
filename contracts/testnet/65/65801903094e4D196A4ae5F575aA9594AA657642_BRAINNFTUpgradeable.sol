    ...
    NFTInfoUpgradeable
{
    ...
    function isValidName(string memory value) external view returns(bool) {
        return (!names[value] && bytes(value).length > 3 && bytes(value).length < 33);
    }

    function renameBRAIN(address account, uint256 tokenID, string calldata value) external onlyProxy {
