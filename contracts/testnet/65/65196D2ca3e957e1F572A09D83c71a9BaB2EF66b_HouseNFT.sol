...
contract HouseNFT is ERC721, Ownable {
    ...
    mapping (address => uint) ownerHouseCount;  // House count of users
    uint nextID = 0;
    uint nextRareID = 0;
    uint maxRere;
    ...
    */
    ...
    function mint(address to, bool isRare) public onlyOwner {
        ...
        // Increase users house account
        ownerHouseCount[to]++;
    }

    function _beforeTokenTransfer(address from, address to, uint tokenId) internal view override {
       House storage house = houses[tokenId];

       require(house.depositedBalance == 0, "Please withdraw tokens");
       // TODO: require to harvest from the house
    }
    ...
    */
    function getHousesByOwner(address _owner) public view returns(uint[] memory) {
        uint256[] memory result = new uint256[](ownerHouseCount[_owner]);
        uint counter;
