...
contract ArborStakingWallet is Ownable {
  ...
  IStakingToken public immutable dividendToken;
  ...
  constructor(IStakingToken _mainToken, IStakingToken _divToken, address _stakingContract) {
  ...
  function withdrawReward(address receiver, uint256 _amount) external onlyOwnerOrDeployer {
    ...
    if (amountDividenToWithdraw > 0) {
      IERC20(address(dividendToken)).transfer(receiver, _amount);
    }
  ...
  function _getDividenShare(uint256 _amount) internal view returns (uint256) {
    ...
    uint256 _dividendBalance = IERC20(dividendToken).balanceOf(address(this));
  ...
  function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwnerOrDeployer {
    ...
    require(_tokenAddress != address(dividendToken), "Cannot be main dividenToken");
