...
contract Candystar is BaseNFT, ReentrancyGuard{
    ...
    //          events
    /////////////////////////////////
    event mintCSNFTEvent(address to, uint256 tokenId);

    constructor(string memory baseUri, address managerAddr)
    BaseNFT("Candystar NFT","CSNFT", baseUri, managerAddr) {
        _tokenIdTracker.increment() ;
    }

    function tokenURI(uint256 tokenId) public view virtual override(BaseNFT) returns (string memory) {
        return string(abi.encodePacked(_baseURI(),"/candystar/", Strings.toString(tokenId)));
    }

    function batchMintCSNFT(address [] memory to, uint256 [] memory count) external onlyRole(MANAGER_ROLE) {
        require(to.length == count.length && count.length > 0, "Parameter Error") ;
        for(uint256 i = 0; i < to.length; i++) {
            _mintNft(to[i], count[i]) ;
        }
    }

    function mintSingle(address to, uint256 count) external onlyRole(MANAGER_ROLE) {
        _mintNft(to, count) ;
    }

    function _mintNft(address to, uint256 count) private {
        require(count > 0, "Incorrect number of mint nft") ;
        require(to != address(0x0), "Incorrect mint address") ;
        for(uint256 i = 0; i < count; i++) {
            uint256 tokenId = _tokenIdTracker.current() ;
            _safeMint(to, tokenId);
            _tokenIdTracker.increment() ;
            emit mintCSNFTEvent(to, tokenId) ;
        }
    }

    function _migration(uint256 tokenId, uint256 gene, address owner) internal override virtual returns (bool){}
}
