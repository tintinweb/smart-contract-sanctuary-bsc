...
contract SeedVesting{
  ...
  function setUsers(address[] calldata  _userAddress, uint256[] calldata _amount) external {
  ...
  function claimRead(address _beneficiary) public view returns(uint256,uint256,uint256){
    ...
    return (totalEmissions,amount,users[beneficiary].amount);
