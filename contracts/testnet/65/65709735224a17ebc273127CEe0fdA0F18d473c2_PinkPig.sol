...
contract PinkPig is IERC20, Context, Ownable {
    ...
    uint256 public _commonDiv = 1000; //Fee DIV

    uint256 public _buyMarketingFee = 10; //1%
    uint256 public _sellMarketingFee = 10;//1%

    uint256 public _buyLiquidityFee = 20; //2%
    uint256 public _sellLiquidityFee = 20; //2%

    uint256 public _buyCofounderFee = 10; //1%
    uint256 public _sellCofounderFee = 10; //1%

    uint256 public _buyWhitListFee = 10; //1%
    uint256 public _sellWhitListFee = 10; //1%

    uint256 public _buyMaxMinFee = 10;//1% max 0.5% min 0.5%
    uint256 public _sellMaxMinFee = 10; // 1% max 0.5% min 0.5%

    uint256 public _buySegmentFee = 20; //2% 48 address bonus
    uint256 public _sellSegmentFee = 20; //2% 48 address bonus
    ...
    address public ppcAddress;
    address public usdtAddress = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);// testnet
    // address public usdtAddress = address(0x55d398326f99059fF775485246999027B3197955); // prod

    address public marketFeeAddress;
    address public lpFeeAddress;
    address public cofounderFeeAddress;
    address public whiteListFeeAddress;
    address public transferFeeAddress;

    address public dayMaxMinFeeAddress; // max min fee dividend contract address
    address public segmentFeeAddress; // segment fee  contract address

    // DividendTracker public dividendTracker; //Divident Contract
    ...
    uint256 public constant basicCoin = 1*10**18;// 1 coin

    uint256 public startTime;
    uint256 public beforetime; // Before Bonus Time
    ...
    address public dayMaxPriceAddr;
    uint256 public dayMaxPrice;

    address public dayMinPriceAddr;
    uint256 public dayMinPrice;

    mapping(uint => address) public dayOfMaxPriceAddr;
    mapping(uint => address) public dayOfMinPriceAddr;

    uint256 public lastDayTime;

    mapping(address => bool) public whitelist;
    mapping(address => bool) public cofounder;

    uint256 public limitBonusAmount = 100*10**18;// 100U
    uint256 public limitWhiteAmount = 300*10**18;// 300U
    uint256 public limitCofounderAmount = 600*10**18;//600U
    ...
    function getLastDayMaxMinPriceAddress(uint256 day) public view returns (address, address) {
        require(day > 0, "the day of price record must be a number");
        address maxPriceAddr = dayOfMaxPriceAddr[day];
        address minPriceAddr = dayOfMinPriceAddr[day];
        return (maxPriceAddr, minPriceAddr);
    }
    ...
    function getSegmantAddrs(uint day) public view returns(address[] memory addrs) {
        addrs = new address[](48);
        for(uint i = 0; i < 48; i++){
            addrs[i] = segmantBonus[day][i];
        }
    }
    ...
    function _segmantTime() private {
        ...
        if( lastSegmantRecordTime == 0 ){
            ...
            uint minute;
            uint second;
            (year, month, day, hour, minute, second) = DateTimeLibrary.timestampToDateTime(nowTime);
            uint buildMinute = 0;
            if(minute >= 30){
                buildMinute = 30;
            }
            ...
            refreshSegmant = true;
        } else if ( nowTime.sub(lastSegmantRecordTime) >= priceRecordTime ) {
            uint period = nowTime.sub(lastSegmantRecordTime) / priceRecordTime;
            if(period >= 1) {
                for(uint i = 0; i < period ; i++){
                    //if period time no transaction, add 0x0 to segmant map
                    segmantBonus[getDay()][lastSegmantRecordIndex] = address(0);
    ...
    event RecordMaxMinPrice(address maxPriceAddr, address minPriceAddr, uint256 maxPrice, uint256 minPrice);

    function _checkDiffDay() private {
        if( lastDayTime == 0 ){
            lastDayTime = block.timestamp;
        } else {
            uint diffDay = DateTimeLibrary.diffDays(lastDayTime, block.timestamp);
            if(diffDay >= 1){
                _checkSegmantTime();

                dayOfMaxPriceAddr[getDay()] = dayMaxPriceAddr;
                dayOfMinPriceAddr[getDay()] = dayMinPriceAddr;
                lastDayTime = block.timestamp;
                emit RecordMaxMinPrice(dayMaxPriceAddr, dayMinPriceAddr , dayMaxPrice, dayMinPrice);
    ...
    function _checkSegmantTime() private {
    ...
    function _recordMaxMinPrice(address buyer) private {
        uint256 price = ppcPrice();
        if(dayMaxPrice == 0){
            dayMaxPrice = price; // initial max price
        }

        if(dayMinPrice == 0){
            dayMinPrice = price; // initial min price
        }

        if(price > dayMaxPrice){
            dayMaxPrice = price;
            dayMaxPriceAddr = buyer;
        } else if(price < dayMinPrice){
            dayMinPrice = price;
            dayMinPriceAddr = buyer;
        }
    }

    function _recordSegmant(address buyer) private {
        segmantBonus[getDay()][lastSegmantRecordIndex] = buyer;
        lastSegmantRecordIndex++;
    }
    ...
    function ppcPrice() public view returns (uint256) {
        return calTokenPrice(basicCoin); // 1 ppc worth usdt
    }

    function calTokenPrice(uint256 _value) internal view returns(uint256) {
        address _ppcWbnbPair = IUniswapV2Factory(factoryAddress).getPair(ppcAddress, wbnbAddress);
        ...
        uint256 _ppcPriceWbnb = token0 == _ppcWbnbPair ? reserve1.mul(_value).div(reserve0) : reserve0.mul(_value).div(reserve1);
        // PPC worth USDT
