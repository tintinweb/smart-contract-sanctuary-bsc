...
contract Token is Context, IERC20, Ownable {
    ...
    uint8 public _stakingFee = 0;
    uint8 private _previousStakingFee = _stakingFee;
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 marketingandDevelopmentFee, uint8 stakingFee) external onlyOwner() {
        ...
        require(stakingFee >= 0 && stakingFee <=maxStakingFee,"LF err");
        _taxFee = taxFee;
    ...
    function setStakingWallet(address payable newStakingWallet) external onlyOwner {
        require(newStakingWallet != address(0), "ZERO ADDRESS");
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _stakingFee + _marketingandDevelopmentFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingandDevelopmentFee == 0 && _stakingFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint8 totFee  = _liquidityFee + _stakingFee + _marketingandDevelopmentFee; 
        ...
        if(_stakingFee != 0){
            spentAmount = contractTokenBalance.div(totFee).mul(_stakingFee);
            swapTokensForStaking(spentAmount);
            totSpentAmount = totSpentAmount + spentAmount;
        }
    ...
    function swapTokensForStaking(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = pcsV2Router.WETH();
