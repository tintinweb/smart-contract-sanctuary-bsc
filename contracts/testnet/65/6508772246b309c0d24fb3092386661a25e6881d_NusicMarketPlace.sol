...
contract NusicMarketPlace {
    ...
    // address=>listingId => willing_to_pay
    // mapping(uin)
    ...
    function createBid(uint256 listingId, uint256 biddingAmount) public {
        Listing memory listing = getListing[listingId];
        uint256 expiresAt = block.timestamp + (listing.auctionPeriod * 1 days);
        require(block.timestamp < expiresAt, "Auction ended");
        if (listing.creator == address(0)) revert ListingNotFound();
        if (biddingAmount < listing.askPrice) revert WrongValueSent();

        biddings[msg.sender][listingId] = biddingAmount;
    }
