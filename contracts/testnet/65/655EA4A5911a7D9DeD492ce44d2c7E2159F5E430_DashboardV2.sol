...
contract DashboardV2 {
    ...
    ICronaSwapFactory private factory;
    IERC20 public reward; // CRONA
    ...
    constructor(address _weth, address _usdc, address _reward, address _master, address _factory) public {
        ...
        factory = ICronaSwapFactory(_factory);
        _owner = msg.sender;
    }
    ...
    function rewardPerYearOfPool(uint pid) view public returns (uint) {
        ...
        return master.cronaPerSecond().mul(multiplier).mul(SEC_PER_YEAR).mul(allocPoint).div(master.totalAllocPoint());
    }
    ...
    function valueOfAsset(address asset, uint amount) public view returns (uint valueInETH, uint valueInUSD) {
        ...
        } else if (keccak256(abi.encodePacked(ICronaSwapPair(asset).symbol())) == keccak256("Crona-LP")) {
            if (ICronaSwapPair(asset).token0() == address(weth) || ICronaSwapPair(asset).token1() == address(weth)) {
                valueInETH = amount.mul(weth.balanceOf(address(asset))).mul(2).div(ICronaSwapPair(asset).totalSupply());
                valueInUSD = valueInETH.mul(ethPriceInUSD()).div(1e18);
            } else {
                uint balanceToken0 = IERC20(ICronaSwapPair(asset).token0()).balanceOf(asset);
                (uint token0PriceInETH,) = valueOfAsset(ICronaSwapPair(asset).token0(), 1e18);

                valueInETH = amount.mul(balanceToken0).mul(2).mul(token0PriceInETH).div(1e18).div(ICronaSwapPair(asset).totalSupply());
                valueInUSD = valueInETH.mul(ethPriceInUSD()).div(1e18);
            }
        } else {
...
interface IMasterChef {
    ...
    function userInfo(uint256 _pid, address _user) external view returns (uint256 amount, uint256 workingAmount, uint256 rewardDebt);

    function poolInfo(uint _pid) view external returns (address lpToken, uint256 workingSupply, bool boost, uint256 allocPoint, uint256 lastRewardTime, uint256 accCronaPerShare);
