...
contract DefiKings is ERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0x1904359446894C41733e963661D0c5C1B2Da7C58);
    ...
    constructor() ERC20("Defi Kings", "DFK") {
        ...
        totalSellFees = BNBRewardsSellFee.add(liquidityFee).add(marketingFees).add(burnFee);
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            if (isSelling){
                burn = amount.mul(burnFee).div(100);
                super._transfer(from, address(deadAddress), burn);
            }

            amount = amount.sub(fees).sub(burn);

            if (isSelling) {
                sellAmount = sellAmount.add(fees).add(burn);

            } else {
                buyAmount = buyAmount.add(fees).add(burn);
            }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        (bool successOp2, ) = address(marketingAddress).call{
            value: otherHalf
        }("");
