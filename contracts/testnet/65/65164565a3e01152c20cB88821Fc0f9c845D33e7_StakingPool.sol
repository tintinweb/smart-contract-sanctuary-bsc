    ...
    AccessControlUpgradeable
{
    ...
    function _getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) private view returns(uint256 pendingReward) {
    ...
    function getPendingReward(uint256 _poolId, address _account, uint256 _stakedId) external view returns(uint256) {
        ...
        // return pendingReward;
        uint256 pendingReward = _getPendingReward(_poolId, _account, _stakedId);
        return pendingReward;
    }
    ...
    function getTotalReward(uint256 _poolId, address _account) external view returns(uint256 totalReward) {
        ...
        for(uint256 i = 0; i < len; i++) {
            totalReward += _getPendingReward(_poolId, _account, i);
        }
    ...
    function testSubTime(uint256 _poolId, address _account, uint256 _stakedId)  external view returns(uint256) {
        StakingData storage user = userStakingData[_poolId][_account];
        UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        console.log('block.timestamp: ',block.timestamp);
        console.log('userStaking.stakeTime: ',userStaking.stakeTime);
        console.log('total: ', block.timestamp  - userStaking.stakeTime);
        console.log('ONE_YEAR: ',ONE_YEAR);
        return (userStaking.balance * userStaking.APR * (block.timestamp + 2592000 - userStaking.stakeTime)) / (ONE_YEAR * 1e20);
    }

    function a(uint256 _poolId, address _account, uint256 _stakedId)  external view returns(uint256) {
        StakingData storage user = userStakingData[_poolId][_account];
        UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        return userStaking.balance * userStaking.APR;
    }

    function b(uint256 _poolId, address _account, uint256 _stakedId)  external view returns(uint256) {
        StakingData storage user = userStakingData[_poolId][_account];
        UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        return block.timestamp;
    }
    function c(uint256 _poolId, address _account, uint256 _stakedId)  external view returns(uint256) {
        StakingData storage user = userStakingData[_poolId][_account];
        UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        return userStaking.stakeTime;
    }

    function d(uint256 _poolId, address _account, uint256 _stakedId)  external view returns(uint256) {
        StakingData storage user = userStakingData[_poolId][_account];
        UserStakingData storage userStaking = user.stakingDatas[_stakedId];
        return ONE_YEAR * 1e20;
    }
