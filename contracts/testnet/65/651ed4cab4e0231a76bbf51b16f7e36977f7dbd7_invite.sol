...
contract invite  is Ownable{
    ...
    function setValidInvite(address _address) public {
        ...
        emit SetValidInvite(batchindex,_address);
        //计算排序
        (Rank.rankChange[] memory changes,bool isfull) = _inviteData[batchindex].rankData.updateRank(_AddBeInvited,_inviteData[batchindex].inviteNumber[_AddBeInvited]);
        ...
        for (uint i = 0;i< changes.length;i++){ 
            ...
            if (oldWeight != newWeight) {
                if (!recordPool){ 
                    ...
                    globalEarning.earningUsdt[batchindex].recordPool();
                    globalEarning.earningErc20[batchindex].recordPool();
