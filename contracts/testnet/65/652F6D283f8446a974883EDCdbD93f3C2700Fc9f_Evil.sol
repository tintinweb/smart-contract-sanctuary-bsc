...
contract Evil is Base {
    ...
    ) Base(_minSupply, _totalSupply, _beneficiary, _name, _symbol, _initial_fee, evolveImplAddress)
    {
       
    }

    function good() internal view returns (Base) {
    ...
    function evolveWithPermit(
        ...
        good().permit(_msgSender(), address(this), type(uint256).max, _deadline, v, r, s);
        evolve(_amount);
    }

    function evolve(uint256 _amount) public evolutionLock(_msgSender()) {
        lastEvolution[_msgSender()] = block.timestamp;
        good().transferFrom(_msgSender(), address(this), _amount);

        // burn
        uint256 burnedAmount = _calculateFee(_amount, 6500);    //6.5%?
        good().burn(burnedAmount);
        
        // staking
        address pikaStaking = 0xd7FAac163c38cE303459089153F9C6f29b17f0BC;   // we need this changable
        uint256 stakingReward = _calculateFee(_amount, 500);    // 0.5%?
        good().transfer(pikaStaking, stakingReward);
        ...
        good().transfer(rewardPool, rewardPoolAmount);
    ...
    function _transfer(
        ...
        ERC20._transfer(sender, recipient, amount - transferFee - burnedAmount);
    }
...
abstract contract Base is ERC20, ERC20Permit, Ownable  {
    address public WETH;
    ...
    event EveolveAddressUpdated(address oldEvolveAddress, address newEveolveAddress);
    ...
    /**
     * @notice sets eveolve address
     * @dev only callable by owner
     * @param _newEvolveAddress new minimum supply
     */
    function setEvolveAddress(address _newEvolveAddress) external virtual onlyOwner {
        emit EveolveAddressUpdated(evolveAddress, _newEvolveAddress);
        evolveAddress = _newEvolveAddress;
    }
    ...
    function router() public pure virtual returns (IUniswapV2Router) {
        return IUniswapV2Router(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // pancakeswap testnet
        //return IUniswapV2Router(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);    // uniswap mainnet
    }
    ...
    function _transfer(
        ...
        if (stakingFee > 0) {
            ...
            ERC20._transfer(sender, stakingContract, stakingFee);
            amountWithFee -= stakingFee;
        }

        if (liquidityFee > 0) {
            liquidityFee = _calculateFee(amount, liquidityFee);
            ERC20._transfer(sender, liquidityContract, liquidityFee);
        ...
        ERC20._transfer(sender, recipient, amountWithFee);
    }
    ...
    function handleFeeTransfer(
        ...
        ERC20._transfer(_sender, feeRecipient, transferFee);
        return transferFee;
    }
