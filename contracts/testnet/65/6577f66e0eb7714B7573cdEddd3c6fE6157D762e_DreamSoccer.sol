...
// File: contracts/DreamSoccer.sol
...
contract DreamSoccer is Context, Ownable, IERC20   {
    ...
    string private constant _name = "DreamSoccer";
    string private constant _symbol = "DSoccer";
    ...
    modifier onlyDev() {	
        require(owner() == _msgSender() || _TreasuryAddress == _msgSender(), "Caller is not the dev");	
        _;	
    }
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyDev {
        require(taxFee < 2, "taxFee cannot be more than 2.");
        _taxFee = taxFee;
    }
    
    function setTotalFeePercent(uint256 taxLiquidityFee, uint256 taxTreasuryFee) external onlyDev {
        require(taxLiquidityFee < 2, "taxLiquidityFee cannot be more than 2.");
        require(taxTreasuryFee < 4, "taxTreasuryFee cannot be more than 4.");
    ...
    function setNewTreasuryAddress(address payable TreasuryAddress) public onlyDev {
        _TreasuryAddress = TreasuryAddress;
        _isExcludedFromFee[_TreasuryAddress] = true;
    }

    function manualswap() public onlyDev  {

        uint256 contractBalance = balanceOf(address(this));
        swapForTreasury(contractBalance);
    }

    function manualsend() public onlyDev  {
