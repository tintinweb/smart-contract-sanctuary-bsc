...
/* Token contract */
contract TESTB is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //In case for reflection. Can leave as is if 0
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEV  = 0x6499F8036F85169ab3026e799a2f711a091800d8; //Dev Wallet Addres
    // ---------------> Set marketing wallet at 495
    ...
    // Name and symbol
    string constant _name = "TESTB";
    string constant _symbol = "TSTB";
    ...
    // GREED 
    // Triggers when user sells more than once within 3600sec. See Line 485 (Greed is currently Disabled)
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver =0x3b42107C29051Ed79c33C3dA03Aa8939e5Ecca2f; // Marketing Wallet
    ...
    function takeFee(address sender, uint256 amount, bool isSell, bool GREEDMode) internal returns (uint256) {
        ...
        // Check if we are GREEDMode
        if (GREEDMode){
            if(isSell){
                feeAmount = amount.mul(totalFee).div(feeDenominator);
                // If we are selling so up the selling tax to 1.5x
                // feeAmount = amount.mul(totalFee).mul(3).div(2).div(feeDenominator);
            } else {
                feeAmount = amount.mul(totalFee).div(feeDenominator);
                // If we are buying so cut our taxes in half
