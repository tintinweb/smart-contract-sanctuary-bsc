...
contract Marketplace is Ownable, ERC721Holder, ERC1155Holder, IMarketplace {
  ...
  constructor(address nftAddress, address treasureAddress) {
    require(nftAddress != address(0) && treasureAddress != address(0), 'NFTMarketplace: Address input is zero address');
  ...
  modifier canBuyNFT(uint256 orderId_, address buyer_) {
    ...
    require(orderOfNFT[orderId_].buyer == address(0), 'NFTMarketplace: buyer must be zero');
    _;
  }
  ...
  modifier canBuyTreasure(uint256 orderId_, address buyer_) {
    ...
    require(orderOfTreasure[orderId_].buyer == address(0), 'NFTMarketplace: buyer must be zero');
    _;
  }
  ...
  function addOrderTreasure(
    ...
    _order.seller = _msgSender();
    _order.treasureId = treasureId_;
    _order.paymentToken = paymentToken_;
    _order.price = price_;
    _order.amount = amount_;
    _order.totalPrice = price_ * amount_;

    treasureContract.safeTransfer(_msgSender(), address(this), treasureId_, amount_);
    emit OrderAdded(_orderId, _msgSender(), treasureId_, paymentToken_, price_);
  }

  function cancelOrderNFT(uint256 orderId_) external override {
  ...
  function cancelOrderTreasure(uint256 orderId_) external override {
    OrderTreasure storage _order = orderOfTreasure[orderId_];
    require(_order.buyer == address(0), 'NFTMarketplace: buyer must be zero');
    require(_order.seller == _msgSender(), 'NFTMarketplace: must be owner');
    uint256 _treasureId = _order.treasureId;
    uint256 _amount = _order.amount;
    delete orderOfTreasure[orderId_];
    treasureContract.safeTransfer(address(this), _msgSender(), _treasureId, _amount);
    emit OrderCancelled(orderId_);
  }

  function buyOrderNFT(uint256 orderId_) external payable override canBuyNFT(orderId_, _msgSender()) {
    OrderNFT storage _order = orderOfNFT[orderId_];
    ...
    emit OrderMatched(orderId_, _order.seller, _order.buyer, _order.nftId, _order.paymentToken, _order.price);
  }

  function buyOrderTreasure(uint256 orderId_) external payable override canBuyTreasure(orderId_, _msgSender()) {
    OrderTreasure storage _order = orderOfTreasure[orderId_];
    _order.buyer = _msgSender();
    IERC20(_order.paymentToken).safeTransferFrom(_msgSender(), _order.seller, _order.totalPrice);
    treasureContract.safeTransfer(address(this), _msgSender(), _order.treasureId, _order.amount);
    emit OrderMatched(orderId_, _order.seller, _order.buyer, _order.treasureId, _order.paymentToken, _order.totalPrice);
  }
}
...
import '@openzeppelin/contracts/access/Ownable.sol';
import '@openzeppelin/contracts/utils/Counters.sol';
import './IRelipaNFT.sol';

contract RelipaNFT is ERC721Holder, ERC721Enumerable, Ownable, IRelipaNFT {
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIdCount;
  string private _baseTokenURI;
  mapping(uint256 => Metadata) private _metadataOfTokenId;
  uint32 private _timeExpireDate = 3 days;
  uint16 private _discount = 10;
  ...
  constructor() ERC721('RLP_HDN', 'RLP') {}
  ...
  function transferNFT(
    ...
    address to,
    uint256 tokenId
  ) external override CheckAddress(to) checkTokenId(tokenId) {
...
contract RelipaTreasure is ERC1155, ERC1155Holder, Ownable, IRelipaTreasure {
  ...
  constructor(string memory uri_, address _NFTaddress) ERC1155(uri_) {
  ...
  function safeTransfer(
    ...
    uint256 treasureId,
    uint256 amount
  ) public override CheckAmount(amount) CheckAddress(to) {
    require(amount <= balanceOf(from, treasureId), "Amount must be less or equal than sender treasure's amount");
    _safeTransferFrom(from, to, treasureId, amount, '');
    emit safeTransferEvent(from, to, treasureId, amount);
  }
...
interface IMarketplace {
  ...
  function buyOrderNFT(uint256 orderId_) external payable;

  function buyOrderTreasure(uint256 orderId_) external payable;
}
