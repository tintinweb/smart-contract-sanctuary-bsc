...
contract Coin2Fish is ERC20, Ownable {
    ...
    string public _tokenSymbol = "C2FRT";
    ...
    constructor() ERC20(_tokenName, _tokenSymbol, owner()) {
        ...
        _isExcludedFromFees[developerAddress] = true;
        _isExcludedFromFees[marketingAddress] = true;
        _isExcludedFromFees[moderatorAddress] = true;

        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */
        _mint(owner(), _tokenTotalSupplyInteger * (10 ** 18));
    }
    ...
    function _transfer(
        ...
        if (takeFee && automatedMarketMakerPairs[from]) {
            uint256 developerAmount = amount.mul(developerTaxFee).div(100);
            uint256 marketingAmount = amount.mul(marketingTaxFee).div(100);
            uint256 moderatorAmount = amount.mul(moderatorTaxFee).div(100);
            uint256 liquidityAmount = amount.mul(liquidityTaxFee).div(100);
            ...
            super._transfer(from, developerAddress, developerAmount);
            super._transfer(from, marketingAddress, marketingAmount);
            super._transfer(from, moderatorAddress, moderatorAmount);
            super._transfer(from, owner(), liquidityAmount);
        }
        super._transfer(from, to, amount);
    }

    function updateTaxFees(uint256 _developerTaxFee, uint256 _marketingTaxFee, uint256 _moderatorTaxFee, uint256 _liquidityTaxFee, uint256 _burnTaxFee) external onlyOwner {
        ...
        uint256 sellTotalFees = developerTaxFee + marketingTaxFee + moderatorTaxFee + liquidityTaxFee + burnTaxFee;
        require(sellTotalFees <= 10, "Must keep fees at 10% or less");
    }

    function swapAndAddLiquidity(uint256 contractTokenBalance) private lockTheSwap {
