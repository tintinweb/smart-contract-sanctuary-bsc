    ...
    AccessControl
{
    ...
    function buyLands(
        ...
        emit bought(_zone, _x, _y, msg.sender, tokenIdMint, _isLock);
    }
    ...
    function _lockLand(uint256 _tokenId, address _sender) external {
        for (uint256 i = 0; i < allLands.length; i++) {
            if (_tokenId == allLands[i].tokenId) {
                require(
                    allLands[i].isLocked == false,
                    "The land mustn't be locked"
                );
                require(
                    allLands[i].wallet == _sender,
                    "The land must be owned by sender"
                );
                allLands[i].isLocked = true;
            }
        }
    }
