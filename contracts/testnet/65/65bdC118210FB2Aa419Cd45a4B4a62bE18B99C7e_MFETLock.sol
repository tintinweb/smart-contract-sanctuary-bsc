...
contract MFETLock is Context, Ownable, ReentrancyGuard {
    ...
    function createLock(
        ...
        (uint256 id, uint256 totalLockAmount) = _createLock(
            user,
            amount,
            unlockTime,
            token
    ...
    function createMultipleLocks(
        ...
        address token
    ) external nonReentrant returns (uint256[] memory, uint256) {
        (
            uint256[] memory lockIds,
            uint256 totalLockAmount
        ) = _createMultipleLocks(user, amounts, unlockTimes, token);
        emit LogDeposit(token, _msgSender(), totalLockAmount);
        return (lockIds, totalLockAmount);
    }
    ...
    function _createMultipleLocks(
        ...
        uint256[] memory lockIds = new uint256[](unlockTimes.length);
