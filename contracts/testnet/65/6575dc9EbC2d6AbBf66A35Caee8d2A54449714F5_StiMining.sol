...
contract StiMining is Ownable {
    ...
    struct PoolInfo {
        ...
        uint endDebted;
        uint tvl; 
        // startTime    endTime    lockStart    lockInterval
        uint[4] times;
        uint invRate;
    }
    ...
    function editAdmins(address[] memory account_, bool isAdd_) public onlyOwner {
    ...
    function poolTimes(uint pId_) public view returns(uint[4] memory) {
        return poolInfo[pId_].times;
    }

    function viewUserInfo(uint pId_, address account_) public view returns(UserInfo memory) {
    ...
    function viewInfo(uint pId_, address account_) public view returns(string[3] memory names, uint[2] memory decimlas, address[3] memory addrInfos, uint[6] memory infos, uint[5] memory uInfos) {
        ...
        names[1] = pool.iptToken.symbol();
        names[2] = pool.optToken.symbol();
        ...
        infos[4] = pool.times[2] + pool.times[3];
        infos[5] = pool.invRate;


        uInfos[0] = user.stake;
        uInfos[1] = getReward(pId_, account_);
        uInfos[2] = user.claimed;
        uInfos[3] = user.invReward - user.invRewarded;
        uInfos[4] = user.invRewarded;
    }

    function getPoolInfo(uint pId_) public view returns(string[3] memory names , uint[4] memory infos) {
        PoolInfo memory pool = poolInfo[pId_];
    ...
    function getPoolInfoBatch(uint stage_) public view returns(string[3][] memory names , uint[4][] memory infos) {
    ...
    function setTm(uint pId_, uint stage_) public {
        PoolInfo storage pool = poolInfo[pId_];
        if (stage_ == 0) {
            pool.times[0] = block.timestamp + 60;
            pool.times[1] = block.timestamp + 120;
        } else if (stage_ == 1) {
            pool.times[0] = block.timestamp - 10;
            pool.times[1] = block.timestamp + 120;
        } else if (stage_ == 2) {
            pool.times[0] = block.timestamp - 120;
            pool.times[1] = block.timestamp - 60;
        } else if (stage_ == 3) {
            pool.times[2] = block.timestamp - 10;
            pool.times[3] = block.timestamp + 120;
        } else if (stage_ == 4) {
            pool.times[2] = block.timestamp - 120;
            pool.times[3] = block.timestamp - 60;
        }
    }
    ...
    function coutingDebt(uint pId_) public view returns (uint) {
        ...
        uint newDebt = pool.tvl > 0 ? pool.rate * (block.timestamp - pool.lastTime) * ACC / pool.tvl + pool.debted : 0 + pool.debted;
        return newDebt;
    }
    
    // calculate user reward
    function getReward(uint pId_, address account_)  public view  returns (uint) {
        UserInfo memory user = userInfo[pId_][account_];

        uint debt = coutingDebt(pId_);
        uint reward = (debt - user.debt) * user.stake / ACC;
        reward += user.toClaim;
        return reward;
    }


    function coutingDebt1(uint pId_, uint tm_) public view returns (uint) {
        PoolInfo memory pool = poolInfo[pId_];
        if (pool.endDebted != 0) {
            return pool.endDebted;
        }

        uint newDebt = pool.tvl > 0 ? pool.rate * tm_ * ACC / pool.tvl + pool.debted : 0 + pool.debted;
        return newDebt;
    }
    
    // calculate user reward
    function getReward1(uint pId_, address account_, uint tm_)  public view  returns (uint) {
        UserInfo memory user = userInfo[pId_][account_];

        uint debt = coutingDebt1(pId_, tm_);
        uint reward = (debt - user.debt) * user.stake / ACC;
        reward += user.toClaim;
        return reward;
    }
    ...
    function updatePool(uint pid_, uint value_, bool isAdd_) private returns(uint) {
        PoolInfo storage pool = poolInfo[pid_];
        ...
        if (pool.endDebted == 0 && block.timestamp >= pool.times[1])  {
            pool.endDebted = pool.tvl > 0 ? pool.rate * (pool.times[1] - pool.lastTime) * ACC / pool.tvl + pool.debted : 0 + pool.debted;
        }
    ...
    function stake(uint pId_, uint amount_, address inviter_) public {
        PoolInfo storage pool = poolInfo[pId_];
        UserInfo storage user = userInfo[pId_][msg.sender];
        require(amount_ > 0, "wrong amount");
        require (block.timestamp >= pool.times[0] && block.timestamp < pool.times[1], "wrong time"); 
        if (user.stake > 0) {
            user.toClaim = getReward(pId_, msg.sender);
        }
    ...
    function _claimReward(uint pId_) private returns(uint reward) {
        reward = getReward(pId_, _msgSender());
        if (reward == 0) {
            return reward;
        }
    ...
    function _takeInvReward(uint pId_, uint reward_, address inviter_) private returns(uint invReward_) {
        if (inviter_ == address(0)) {
            inviter_ = address(this);
        }
        invReward_ = reward_ * poolInfo[pId_].invRate / 100;
        userInfo[pId_][inviter_].invReward += invReward_;
    }
    ...
    function claimReward(uint pId_) public {
        ...
        if (pool.times[2] != 0) {
            require(block.timestamp > pool.times[2], "not release");
            uint tm = block.timestamp - (user.lastClaimTime == 0 ? pool.times[2] : user.lastClaimTime);
            reward = (user.toClaim - user.claimed) / pool.times[3] * tm;
        }

        user.toClaim = 0;
        user.lastClaimTime = block.timestamp;
        uint afterReward = (reward - _takeInvReward(pId_, reward, user.inviter));
    ...
    function addPool(uint[4] memory times_, address[2] memory tokens_, uint maxOut_, uint invRate_) public  returns(uint _pid) {
        ...
        poolInfo.push(PoolInfo({
            ...
            endDebted: 0,
            tvl: 0,
            times:  [times_[0], times_[1], times_[2], times_[3]],
            // startTime: times_[0],
            // endTime: times_[1],
            // lockStart: times_[2],
            // lockInterval: times_[3],
            invRate: invRate_
        }));
