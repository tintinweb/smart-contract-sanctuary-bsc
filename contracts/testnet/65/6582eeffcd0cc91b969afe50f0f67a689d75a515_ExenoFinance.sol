    ...
    IExenoFinanceNode
{
    ...
    bytes32 private constant MODE_INTERNAL = bytes32("i");
    ...
    function _collectInboundCorePayments(
        ...
        internal
    {
        if (_isCore(d.token[0]) && d.mode[0] != MODE_INTERNAL) {
        ...
        internal
    {
        if (params.length > 0) {
            ...
            if (action == ACTION_CREDIT_REWARD) {
                ...
                if (d.amount[0] == 0 && d.token[0] == d.token[1]) {
                    d.amount[0] = amount;
                }
                d.amount[1] = amount;
            } else {
    ...
    function _processSwap(
        ...
        internal
    {
        ExenoFinance.validateSwap(d, d.mode[0] == MODE_INTERNAL);
...
library ExenoFinance {
    ...
    function validateSwap(
        ...
        external
    {
        require(d.beneficiary[0] != address(this) || implicit,
            "EFN: mode mismatch");
        require(d.method == METHOD_SWAP,
            "EFN: method mismatch");
        _emitSwap(d);
