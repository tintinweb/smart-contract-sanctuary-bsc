...
contract PaybNftMarketplace is IPaybNftMarketplace, ERC721, IERC721Pausable{
    ...
    function setPool() public onlyOwner(){
        PaybPool = new PaybNftPool(address(this));
    }

    function getAddressPool() public view returns(address){
        return address(PaybPool);
    }
    ...
    function mint(address _to,address _token, string memory _uri, uint256 _royalty, bool _isRoot) public payable override {
    ...
    function mintFragment(address _to,uint256 _rootTokenId, uint256 _amount) public payable override onlyAdmin() {
        require(tokenAddress[_rootTokenId] == address(0) || tokenAddress[_rootTokenId].isContract(), "Token address isn`t a contract address");
        require(_amount < 100, "Quantity is too big");
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
        ...
        require(ownerOf(_tokenId) != msg.sender, "is already owner of token");
        ...
        if(tokenAddress[_tokenId] == address(0)) {
            ...
            PaybPool.subOwnerFeeBalance{value: _subOwnerFee}();
            _pause(_tokenId);
            _transfer(ownerOf(_tokenId), msg.sender, _tokenId);
            uint256 _amountEarn = _subOwnerFee.div(_subOwners[_rootId].length-1 > 0 ? _subOwners[_rootId].length-1 : 1);
            
            if(_fragmentTokens[_tokenId] && _subOwners[_rootIdOf[_tokenId]].length > 0){
                for(uint256 i=0; i< _subOwners[_rootIdOf[_tokenId]].length; ++i){
            ...
            IERC20(tokenAddress[_tokenId]).transferFrom(msg.sender, ownerOf(_tokenId), _price.sub(_creatorRoyalty + _platformFee + _subOwnerFee));
            ...
            IERC20(tokenAddress[_tokenId]).transferFrom(msg.sender, address(PaybPool), _subOwnerFee);
            _pause(_tokenId);
            _transfer(ownerOf(_tokenId), msg.sender, _tokenId);
            uint256 _amountEarn = _subOwnerFee.div(_subOwners[_rootId].length-1 > 0 ? _subOwners[_rootId].length-1 : 1);
    ...
    function setDefaultAmountEarn(address _user, address _tokenAddress) external override{
        require(msg.sender == getAddressPool(), "Not address Pool");
...
contract PaybNftPool is Ownable{
    address PaybMKT;
    using Address for address;
    using SafeMath for uint256;
    ...
    function subOwnerFeeBalance() onlyOwner() public payable{
    }
    ...
    function withdraw(address _tokenAddress) external {
        ...
        if(_tokenAddress == address(0)){
            require(subOwnerFee > 0, "Current amount Earn is 0");
            require(address(this).balance >= subOwnerFee, "Balance not enought");
            payable(msg.sender).transfer(subOwnerFee);
            IPaybNftMarketplace(PaybMKT).setDefaultAmountEarn(msg.sender, _tokenAddress);
        }else{
            require(subOwnerFee > 0, "Current amount Earn is 0");
            require(IERC20(_tokenAddress).balanceOf(address(this)) >= subOwnerFee, "Balance not enought");
...
interface IPaybNftMarketplace {
    ...
    function mint(address _to,address _token, string memory _uri, uint256 _royalty, bool _isRoot) external payable;

    function mintFragment(address _to,uint256 _rootTokenId, uint256 _amount) external payable;
