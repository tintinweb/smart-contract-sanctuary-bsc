...
contract SpaceXComponents is ERC721, Ownable {
    ...
    uint timeZone = 6 * 60 * 60 / 480;

    uint oneDaySecond = 86400 / 480; //todo


    constructor (address USDContract) ERC721 ('NFTLong', 'NFTShort') {
    ...
    function todayZero() public view returns (uint) {
        return block.timestamp - block.timestamp % oneDaySecond - timeZone;
    }
    ...
    function viewDividend(address account) public view returns (uint) {
        ...
        for (uint i = 0; i< accountIndex[account].length; i++) {
            if (block.timestamp >=
                oneDaySecond - mintTime[ accountIndex[account][i] ] % oneDaySecond
                + mintTime[ accountIndex[account][i] ]
                - timeZone
            ) {
                count += 1;
            }
