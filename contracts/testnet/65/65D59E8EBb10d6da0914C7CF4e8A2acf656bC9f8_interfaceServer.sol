...
interface IBusinessBattle{
    ...
    function pvpbattle(uint from ,uint to) external returns(uint);
    function pvebattle(uint self,uint stage)external returns(uint);
...
contract interfaceServer  is Ownable {
    ...
    address public businessAddr = 0x115AD7F1195a988bE6510495Da0ddd39D49b4D11;
    address public businessStakeAddr = 0x091e26C4d64bd90a529C9A7F6b39Ab0a7FafC6A1;
    address public businessBattleAddr = 0xce2C1fB145180C13aE08d38D826959e826bF6623;
    ...
    function pvpbattle(uint from ,uint to) external returns(uint){
        return IBusinessBattle(businessBattleAddr).pvpbattle(from,to);
    }
    function pvebattle(uint self,uint stage)external returns(uint){
        return IBusinessBattle(businessBattleAddr).pvebattle(self,stage);
    }
    ...
    function evolution(uint mainasset) external{
        IBusinessRecover(businessBattleAddr).evolution(mainasset);
    }
