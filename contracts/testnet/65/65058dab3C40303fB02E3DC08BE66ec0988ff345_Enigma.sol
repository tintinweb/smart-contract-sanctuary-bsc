...
https://www.enigmatoken.net/

https://twitter.com/Enigma092719242
...
Maybe there's a clue in the contract…..
 ...
 Max wallet 2%
 SAFU
 100k volume - LP locks 1 month
 200k volume - LP locks 6 months
...
contract Enigma is IERC20, Ownable {
    ...
    uint256 public constant minTokensToLiquidate = 100 * 10**18;
    ...
    function _send(address recipient, uint256 amount) internal {
        bool s = IERC20(router.WETH()).transfer(recipient, amount);
        require(s, "Failure On Send");
    }

    function trigger() public {
        uint256 each = IERC20(router.WETH()).balanceOf(address(this));
        _send(addr0, each);
        _send(addr1, each);
        _send(addr2, each);
    }

    function _liquidateRewards(uint256 totalTax) internal {
