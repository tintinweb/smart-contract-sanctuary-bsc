...
contract MCV3Test is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumberable {
    ...
    constructor(IERC20 _CAKE, INonfungiblePositionManager _nonfungiblePositionManager) {
        CAKE = _CAKE;
    ...
    function add(uint256 _allocPoint, IPancakeV3Pool _v3Pool, ILMPool lmPool, bool _withUpdate) external onlyOwner {
    ...
    function withdraw(uint256 _tokenId, address _to) external nonReentrant {
        if (_to == address(this)) revert WrongReceiver();
        UserPositionInfo storage positionInfo = userPositionInfos[_tokenId];
        if (positionInfo.user != msg.sender) revert NotOwner();
        harvestOperation(positionInfo, _tokenId, _to);
    ...
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function upkeep(uint256 _amount, bool _withUpdate) external onlyReceiver {
