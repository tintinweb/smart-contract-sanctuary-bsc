...
contract TokenStaking is Ownable {
    ...
    function stake(uint256 _amount, address _refrral,uint8 _lockId) external {
        ...
        users[_refrral].referralIncome = users[_refrral].referralIncome.add(_amount.mul(referShare).div(PERCENT_DIVIDER));
        _token.transfer(_refrral,_amount.mul(referShare).div(PERCENT_DIVIDER));  
        emit ReferralIncome(user ,_refrral,_amount.mul(referShare).div(PERCENT_DIVIDER));
