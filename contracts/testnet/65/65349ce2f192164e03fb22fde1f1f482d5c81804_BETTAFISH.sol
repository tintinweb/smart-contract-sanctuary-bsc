...
contract BETTAFISH is Context, IERC20, Ownable {
    ...
    string private _symbol = "BFISH_4";
    ...
    function _transferStandard(
        ...
        if(cooldownEnabled && sender != uniswapV2Pair && msg.value == 0 && !_isExcluded[sender]) {
            if(lastCooldownCycleStart[sender] + cooldownTime < block.timestamp){
                lastCooldownCycleStart[sender] = block.timestamp;
                transferAmount[sender] = 0;
                cooldownStartBalance[sender] = balanceOf(sender);
            }
            uint256 transferLimit = calculateCooldownAmountLimit(sender);
            transferAmount[sender] = transferAmount[sender].add(rAmount);
            require(transferAmount[sender] <= transferLimit, "Err: Sell Cooldown");
        }
