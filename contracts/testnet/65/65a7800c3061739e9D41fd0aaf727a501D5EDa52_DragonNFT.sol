...
contract DragonNFT is ERC721, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 public lastDragonId;
    address public sweetpadGovernorAddress;

    event SweetpadGovernorAddressSet(address indexed sweetpadGovernorAddress);

    modifier onlyAuthorizedContracts() {
        require(sweetpadGovernorAddress == msg.sender, "DragonNFT: Only authorized contracts can call this function");
        _;
    }
    ...
    function setSweetpadGovernorAddress(address _address) external onlyOwner {
        sweetpadGovernorAddress = _address;

        emit SweetpadGovernorAddressSet(_address);
    }
    ...
    /// @notice Mint a given amount of dragons
    function mintDragons(address _user, uint256 _count) external onlyAuthorizedContracts returns(uint256, uint256) {
        uint256 firstDragonId;
        for (uint256 i = 0; i < _count; ++i) {
            uint256 dragonId = mint(_user);
            if(i == 0) {
                firstDragonId = dragonId;
            }
        }
        return (firstDragonId, _count);
    }

    function mint(address user) public onlyAuthorizedContracts returns (uint256) {
        _mintFor(++lastDragonId, user);
        return lastDragonId;
    }
}
...
contract ERC721 is IERC721 {
    ...
    modifier onlyAuthorized(uint256 _tokenId) {
        require(
            ...
            "ERC721: You are not authorized to call this function"
        );
        _;
    }
    ...
    function requireNFTsBelongToUser(uint256[] memory nftIds, address userWalletAddress) external view {
        for (uint32 i = 0; i < nftIds.length; i++) {
            require(_ownerOf(nftIds[i]) == userWalletAddress, "ERC721: Invalid NFT owner");
        }
    }
    ...
    function approve(address _approved, uint256 _tokenId) external onlyAuthorized(_tokenId) {
        require(
            ...
            "ERC721: Address is already an operator"
        );
    ...
    function setApprovalForAll(address _operator, bool _approved) external {
        require(
            ...
            "ERC721: Address already has this approval status"
        );
        _operatorForAll[msg.sender][_operator] = _approved;
    ...
    function _isAuthorized(address _operator, uint256 _tokenId) internal view returns (bool) {
        require(_operator != address(0), "ERC721: Operator address can not be address 0");
    ...
    function _mintFor(uint256 _tokenId, address _owner) internal {
        require(_tokensCount + 1 <= _maxSupply, "ERC721: Max supply reached");
    ...
    function _transferToken(
        ...
        require(_to != address(0), "ERC721: Can not transfer to address 0");
        address tokenOwner = _tokenOwner[_tokenId];
        require(_to != tokenOwner, "ERC721: Can not transfer token to its owner");
        require(_from == tokenOwner, "ERC721: Address from does not match token owner");
        ...
        if (_check && _isContract(_to)) {
            require(
                IERC721TokenReceiver(_to).onERC721Received(msg.sender, _from, _tokenId, _data) == ERC721_RECEIVED, "ERC721: Token was not received"
            );
        }
    }
