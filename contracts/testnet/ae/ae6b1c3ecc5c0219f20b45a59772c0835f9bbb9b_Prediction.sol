...
contract Prediction is Ownable, Pausable {
    ...
    uint256 public minBetAmount = 10000; // minimum betting amount (denominated in wei)
    uint256 public treasuryFee; // treasury rate (e.g. 200 = 2%, 150 = 1.50%)
    address public treasuryAddress; // treasury amount that was not claimed
    ...
    event Bet(address indexed sender, uint256 indexed epoch, Result indexed result, address token, uint256 amount);
    ...
    event TreasuryClaim(address indexed token, uint256 amount);
    ...
    function bet(uint256 epoch, Result result, uint256 amount) external payable whenNotPaused {
        require(_bettable(epoch), "Round not bettable");
        
        require(amount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0 || ledger[epoch][msg.sender].result == result, "Can only bet once per round");

        Round storage round = rounds[epoch];
        round.totalAmount += amount;
        if(result == Result.A)
            round.aAmount += amount;
        else if(result == Result.B)
            round.bAmount += amount;
        else
            require(false, "Bet result error");

        // Update user data
        BetInfo storage betInfo = ledger[epoch][msg.sender];
        betInfo.result = result;
        betInfo.amount += amount;
        userRounds[msg.sender].push(epoch);
        _safeTransfer(round.rewardToken, msg.sender, address(this), amount);
        emit Bet(msg.sender, epoch, result, round.rewardToken, amount);
    }

    function claim(uint256 epoch) external {
        uint256 reward; // Initializes reward

        require(block.timestamp > rounds[epoch].startTimestamp, "Round has not started");
        require(rounds[epoch].closeTimestamp != 0, "Round has not ended");

        // Round valid, claim rewards
        if (rounds[epoch].result != Result.Cancle) {
        ...
        emit Claim(msg.sender, epoch, rounds[epoch].rewardToken, reward);

        if (reward > 0) {
            _safeTransfer(rounds[epoch].rewardToken, address(this), address(msg.sender), reward);
        }
    }
     ...
     */
    function setRound(uint256 epoch, Result result) external whenNotPaused onlyOperator {
        Round storage round = rounds[epoch];
        require(round.closeTimestamp == 0, "Round already set");
        round.closeTimestamp = block.timestamp;
        round.result = result;
        _calculateRewards(epoch);
        //IERC20(round.rewardToken).approve(treasuryAddress, round.treasuryAmt);
        _safeTransfer(round.rewardToken, address(this), treasuryAddress, round.treasuryAmt);
    }
     ...
     */
    function startRound(uint256 startTimestamp, uint256 lockTimestamp, address rewardToken) external whenNotPaused onlyOperator {
        currentEpoch = currentEpoch + 1;
        _startRound(currentEpoch, startTimestamp, lockTimestamp, rewardToken);
    }
    ...
    function recoverToken(address _token, uint256 _amount) external onlyOwner {
        _safeTransfer(_token, address(this), address(msg.sender), _amount);
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            round.closeTimestamp != 0 &&
            round.result != Result.Cancle &&
            betInfo.amount != 0 &&
            !betInfo.claimed &&
            ((round.result == betInfo.result) ||
    ...
    function _calculateRewards(uint256 epoch) internal {
        ...
        // A wins
        if (round.result == Result.A) {
            rewardBaseCalAmount = round.aAmount;
            treasuryAmt = (round.totalAmount * treasuryFee) / 10000;
            rewardAmount = round.totalAmount - treasuryAmt;
        }
        // B wins
        else if (round.result == Result.B) {
        ...
        round.treasuryAmt = treasuryAmt;

        emit RewardsCalculated(epoch, round.rewardToken, rewardBaseCalAmount, rewardAmount, treasuryAmt);
    }
     ...
     */
    function _safeTransfer(address token, address from, address to, uint256 value) internal {
        if(token == address(0)){
     ...
     * @param epoch: epoch
     */
    function _startRound(uint256 epoch, uint256 startTimestamp, uint256 lockTimestamp, address rewardToken) internal {
        Round storage round = rounds[epoch];
        round.startTimestamp = startTimestamp;
        round.lockTimestamp = lockTimestamp;
        round.epoch = epoch;
        round.rewardToken = rewardToken;
        round.totalAmount = 0;
