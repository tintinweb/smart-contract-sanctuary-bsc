...
contract BetScore {
    ...
    function withdraw() payable onlyOwner public {
        ...
        IERC20(i_busdContractAddress).transfer( msg.sender, balance);
    }

    function claim() payable public {
        if(!isAnswered) revert('The answer is not submitted');
        bool canClaim = false;
        address[] memory winners = scoreToAddresses[answer.team1Score][answer.team2Score];
        ...
        uint256 balance = getBalance();
        IERC20(i_busdContractAddress).transfer(msg.sender, min(claimAmount, balance));
    }
