...
contract ActivatableNFT is ERC721Base, OwnableUpgradeable, IActivatableNFT {
    ...
    modifier onlyBranchsPublicBuildingOrMunicipality() {
        require(
            msg.sender == branchPublicBuildingContractAddress ||
                msg.sender == municipalityContractAddress,
            "ActivatableNFT: This function can be called only by this Branch's Public Building Contract Or Municipality Contract"
        );
        _;
    }
    ...
    function setMunicipalityContractAddress(
        address _municipalityContractAddress
    ) external onlyOwner {
        municipalityContractAddress = _municipalityContractAddress;
    }
    ...
    function mintNFTTo(address _to)
        external
        onlyBranchsPublicBuildingOrMunicipality
    ...
    {
        if (_lockStatus) {
            IDistributor(branchPublicBuildingContractAddress).addNFTStaking(
                _ownerOf(_tokenId)
            );
        } else {
            IDistributor(branchPublicBuildingContractAddress).removeNFTStaking(
                _ownerOf(_tokenId)
            );
        }
    }
