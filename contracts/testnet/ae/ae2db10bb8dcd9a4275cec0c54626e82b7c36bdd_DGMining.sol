...
contract DGMining {
	...
	function dGSignup(address sponsorUpline,uint256[] memory levComUrs) public payable {
	   ...
	   //Direct Referral or Spill Over Commission
        ...
        if(availableTokensICO>=tokens){   
           availableTokensICO=availableTokensICO - tokens;                     
          _deliverTokens(msg.sender,tokenAmount,tokens);                    
        }  
        ...
        address receiver;        
			...
			if(tcount >= 1){
				for(uint256 i=1; i<=tcount; i++){
                    ...
                    _deliverTokens(receiver,rewards[userkey],tokens);
				}
    ...
    function _deliverTokens(address beneficiary,uint256 weiAmount, uint256 tokenAmount) internal {
