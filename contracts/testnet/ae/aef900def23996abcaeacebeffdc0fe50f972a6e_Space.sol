...
contract Space is BEP20 { 
  ...
  event RecoverMarketingFund(address targetAddress, uint256 amount);
  ...
  function whatIsMarketing() external view onlyOwnerNAdmins returns (address) {
    return super.getMarketing();
  }

  function whatIsLPLocker() external view onlyOwnerNAdmins returns (address) {
    return super.getLPLocker();
  }
  ...
  function whatIsBaseFee() external view onlyOwner returns (uint256) {
    return baseFee;
  }
  ...
  function recoverMarketingFund(address targetAddr) external onlyOwner {
    require(targetAddr != address(0), "Ownable: address CANNOT be zero address.");
    require(Admin1EmSign, "Admin1 Did not sign!");
    require(Admin2EmSign, "Admin2 Did not sign!");
    require(Admin3EmSign, "Admin3 Did not sign!");    
    uint256 amount = address(super.getMarketing()).balance;
    (bool sent,) = payable(targetAddr).call{value: amount, gas: transferGas}("");
    require(sent, "Tx failed");
    emit RecoverMarketingFund(targetAddr, amount);
  }
