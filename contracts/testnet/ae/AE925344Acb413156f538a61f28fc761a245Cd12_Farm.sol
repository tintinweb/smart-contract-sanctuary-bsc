...
interface IbridgePool {
     ...
     function wrappedAsset(address poolAddress) external view returns (address);
}
...
contract Farm is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        // && bridgePool.wrappedAsset(address(pool.lpToken)) != address(0)
         if(bridgePool.validPool(address(pool.lpToken)) ){
          uint256 balanceBefore = IERC20(bridgePool.wrappedAsset(address(pool.lpToken))).balanceOf(address(this));
          ...
          uint256 balanceAfter = IERC20(bridgePool.wrappedAsset(address(pool.lpToken))).balanceOf(address(this));
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if(!(pool.lpToken.balanceOf(address(this)) >= _amount)){
          if(bridgePool.validPool(address(pool.lpToken)) && bridgePool.wrappedAsset(address(pool.lpToken)) != address(0)){
          uint256 balanceBefore = pool.lpToken.balanceOf(address(this));
          IERC20(bridgePool.wrappedAsset(address(pool.lpToken))).safeApprove(address(bridgePool) ,  _amount);
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if(!(pool.lpToken.balanceOf(address(this)) >= user.amount)){
          ...
          uint256 balanceBefore = pool.lpToken.balanceOf(address(this));
          IERC20(bridgePool.wrappedAsset(address(pool.lpToken))).safeApprove(address(bridgePool) ,  user.amount);
