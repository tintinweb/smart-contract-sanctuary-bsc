...
contract Kodi is Ownable, ERC20 {
	...
	function setFeeOnSelectedWalletTransfers(address account, bool value) public onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "Kodi: The selected wallet is already set to the value ");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet, address newSalaryWallet, address newBuyBackWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "Kodi: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "Kodi: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
		if(salaryWallet != newSalaryWallet) {
			require(newSalaryWallet != address(0), "Kodi: The salaryWallet cannot be 0");
			emit WalletChange('salaryWallet', newSalaryWallet, salaryWallet);
			salaryWallet = newSalaryWallet;
		}
		if(buyBackWallet != newBuyBackWallet) {
			require(newBuyBackWallet != address(0), "Kodi: The buyBackWallet cannot be 0");
