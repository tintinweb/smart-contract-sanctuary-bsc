...
contract DeepLiqutityIDO {
    ...
    constructor(
        ...
        busdtToken=_busdtToken;
        tokenbusdprice=_tokenbusdprice;
        owner=msg.sender;
        receiver=msg.sender;
        priceProvider = pp;
    }
    function depositBUSDT(uint256 _amount) public {
        require (_amount > 0, 'need amount > 0');
        busdtToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        busdtToken.transfer(receiver ,_amount);
    ...
    function depositBUSD(uint256 _amount) public {
     ...
     function latestbnbprice() public view returns(uint price){
      (,int latestPrice,,,) = priceProvider.latestRoundData();
      uint myprice = uint256(latestPrice);
      return myprice;
     }
    ...
    function safeWithDrawKUKU(uint256 _amount,address addr) public  {
    ...
    function safeWithDrawUsdt(uint256 _amount,address addr) public  {
          require(msg.sender==owner , "Not Owner");
          busdtToken.transfer(addr ,_amount);
     }
