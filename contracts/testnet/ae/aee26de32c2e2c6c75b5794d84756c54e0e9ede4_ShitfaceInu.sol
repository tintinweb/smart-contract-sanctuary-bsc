...
contract ShitfaceInu is Ownable, ERC20 {
	...
	function _swapAndLiquify() private {
		...
		uint256 amountBNBStaking = bnbBalanceAfterSwap * _stakingFee / totalBNBFee;
		uint256 amountBNBHolders = bnbBalanceAfterSwap - (amountBNBLiquidity + amountBNBMarketing + amountBNBSalary + amountBNBStaking);

		payable(marketingWallet).transfer(amountBNBMarketing);
		payable(salaryWallet).transfer(amountBNBSalary);
		payable(stakingAddress).transfer(amountBNBStaking);
