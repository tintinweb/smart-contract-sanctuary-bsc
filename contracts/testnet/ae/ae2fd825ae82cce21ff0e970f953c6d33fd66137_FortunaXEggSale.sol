...
// File: contracts/activity/FortunaXEggSale.sol
...
contract FortunaXEggSale is ReentrancyGuard, Multicall{
    uint public price;
    bool public on_sale;
    ...
    event Sale(uint indexed tokenId, address indexed from);

    constructor(INFT _nft, uint _price) {
        nft = _nft;
        price = _price;
        owner = msg.sender;
        on_sale = true;
    ...
    function configPrice(uint _price) external onlyOwner {
        price = _price;
    }
    ...
    function sale(address to, uint count) payable external nonReentrant onSale {
        require(msg.value >= price * count, "error");
        for(uint i = 0; i < count; i ++ ) {
            supply_count = supply_count - 1;
            uint tokenId = nft.mintNFT(to);
            require(tokenId <= supply_count, "");
            INFTProperty fortunaXEggProperty = INFTProperty(nft.nftProperty());
            fortunaXEggProperty.setUintProperty(tokenId, "stage", 0);
            emit Sale(tokenId, to);
        }
    }
