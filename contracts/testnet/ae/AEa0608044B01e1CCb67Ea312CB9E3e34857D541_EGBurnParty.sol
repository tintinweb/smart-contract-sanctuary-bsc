...
contract EGBurnParty is Ownable, KeeperCompatibleInterface {
    ...
    */
    function addBurnToken(address tokenAddress, address burnAddress, uint256 minStakeAmount, uint256 maxStakeAmount) external onlyOwner {
        ...
        require(maxStakeAmount >= minStakeAmount, "EGBurnParty: Max stake amount should be bigger than the min stake amount.");
    ...
    */
    function setMaxStakeAmount(address tokenAddress, uint256 maxStakeAmount) external onlyOwner {
        require(tokenAddress != address(0), "EGBurnParty: The zero address should not be added as a burn token");
        require(maxStakeAmount >= burnTokens[tokenAddress].minStakeAmount, "EGBurnParty: Max stake amount should be bigger than the min stake amount.");
        require(existBurnToken(tokenAddress) == true, "EGBurnParty: The token is not added as a burn token.");
    ...
    */
        ...
        external payable
    {
        ...
        BurnParty memory party = BurnParty({
            ...
            maxStakeAmount: _burnToken.maxStakeAmount - stakeAmount,
            stakeCounter: 1,
            started: true,
            cancelled: false,
            ended: false
    ...
    */
    function stakeBurnParty(uint256 partyId, uint256 tokenAmount) external payable{
        ...
        require(tokenAmount <= _party.maxStakeAmount, "EGBurnParty: Amount required to burn should be smaller than the available stake amount.");
        ...
        if(stakesList[msg.sender][partyId] == 0){
            _party.stakeCounter ++;
            _party.maxStakeAmount -= tokenAmount;
        }
