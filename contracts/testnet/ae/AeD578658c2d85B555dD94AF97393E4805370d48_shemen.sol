...
contract shemen is Ownable{
   ...
   // mapping(uint => Result) private result;
    ...
    event ResultDataEvent(address indexed sender,uint betNum,uint randomNum,uint amount,uint income,address inviter,uint inviterIncome);
    constructor()  {}
    ...
    function betResult(bytes32 reqId, uint randomNum) internal{
        ...
        emit ResultDataEvent(request[reqId].addr,request[reqId].betNum,randomNum,request[reqId].amount,income,inviter,inviterIncome);
