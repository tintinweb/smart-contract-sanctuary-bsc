...
contract DAOAdmin is IDAO, ReentrancyGuard, AccessControl {
    ...
    mapping(address => uint256) private _unlockBalance; // address=> block.timestamp
    ...
    function withdraw(uint256 amount) external {
        if (amount > _balances[msg.sender]) {
            revert DAOInsufficientFunds(_balances[msg.sender]);
        }

        if (_unlockBalance[msg.sender] > block.timestamp) {
            revert DAOFrozenBalance(_unlockBalance[msg.sender]);
        }
    ...
    function addProposal(
        ...
        proposal.endTimeOfVoting = block.timestamp + _debatingPeriodDuration;
    ...
    function vote(uint256 proposalId, bool supportAgainst) external {
        ...
        uint256 powerOfVoting = _balances[msg.sender];
        ...
        if (_unlockBalance[msg.sender] < proposal.endTimeOfVoting) {
            _unlockBalance[msg.sender] = proposal.endTimeOfVoting;
        }
    ...
    function addWhiteList(address[] calldata account, bool[] calldata status)
        external
        onlyRole(DAO_ROLE)
    {
    ...
    function test(uint256 val) external {
        ...
        function test2(uint256[] memory val) external {
        value = val[0];
    }
            function test3(bool[] memory val) external {
        // value = val[0];
    }
}
...
interface IDAO {
    ...
    event Finish(uint256 index, VotingStatus votingStatus);
