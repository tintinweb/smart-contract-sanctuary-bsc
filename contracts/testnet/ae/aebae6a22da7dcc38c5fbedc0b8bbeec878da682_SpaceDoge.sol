...
//////////////////////////////////////////////
// SpaceDoge - Buy&Stake sDOGE and earn wETH
// https://spacedoge.money
// https://t.me/spacedoge_money
// https://twitter.com/spacedoge_money
//////////////////////////////////////////////
...
contract SpaceDoge is Context, Ownable {
    ...
    constructor(address _RouterAddress) public {
        ...
        _symbol = "sDOGE";
        _decimals = 18;
        _totalSupply = 1e7 * 1e18; /// initial supply 10.000.000
        _balances[msg.sender] = _totalSupply;

        buyFees.reward = 10;
        buyFees.operation = 10;
        buyFees.liquidity = 10;
        buyFees.poolfee = 30;

        sellFees.reward = 10;
        sellFees.operation = 10;
        ...
        PancakeSwapV2Pair = IPancakeSwapFactory(_PancakeSwapRouter.factory())
    ...
    function excludeAddressFromFee(address user, bool _isExclude)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                _maxTxAmount >= amount,
                "BEP20: transfer amount exceeds max transfer amount"
            );
        ...
        if (!isExcludeFromFee[sender]) {
            if (sender == PancakeSwapV2Pair) {
                // buy fee
                recieveAmount = recieveAmount.mul(1000 - getTotalBuyFee()).div(
                    1000
                );
                _balances[rewardAddress] += amount
                    .mul(buyFees.reward)
                    .div(1000);
                _balances[operationAddress] += amount.mul(buyFees.operation).div(
                    1000
                );
                _balances[poolAddress] += amount.mul(buyFees.poolfee).div(1000);
                _balances[address(this)] += amount.mul(buyFees.liquidity).div(
                    1000
                );
                ...
                emit Transfer(
                    sender,
                    rewardAddress,
                    amount.mul(buyFees.reward).div(1000)
                );
                emit Transfer(
                    sender,
                    operationAddress,
                    amount.mul(buyFees.operation).div(1000)
                );
                emit Transfer(
                    sender,
                    poolAddress,
                    amount.mul(buyFees.poolfee).div(1000)
                );
                emit Transfer(
                    sender,
                    address(this),
                    amount.mul(buyFees.liquidity).div(1000)
                );
            } else if (recipient == PancakeSwapV2Pair) {
                // sell fee
                recieveAmount = recieveAmount.mul(1000 - getTotalSellFee()).div(
                        1000
                    );
                _balances[rewardAddress] += amount
                    .mul(sellFees.reward)
                    .div(1000);
                _balances[operationAddress] += amount.mul(sellFees.operation).div(
                    1000
                );
                _balances[poolAddress] += amount.mul(sellFees.poolfee).div(
                    1000
                );

                _balances[address(this)] += amount.mul(sellFees.liquidity).div(
                    1000
                );
                ...
                emit Transfer(
                    sender,
                    rewardAddress,
                    amount.mul(sellFees.reward).div(1000)
                );
                emit Transfer(
                    sender,
                    operationAddress,
                    amount.mul(sellFees.operation).div(1000)
                );
                emit Transfer(
                    sender,
                    poolAddress,
                    amount.mul(sellFees.poolfee).div(1000)
                );
                emit Transfer(
                    sender,
                    address(this),
                    amount.mul(sellFees.liquidity).div(1000)
                );
            }
        }
