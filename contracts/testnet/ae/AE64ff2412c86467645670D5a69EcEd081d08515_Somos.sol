...
contract Somos {
   // address payable public devloperAddress;
    ...
    constructor(address ownerAddress) {
       ...
       busd = IERC20(0x37B7d3aB47f967BfdE28719500d7A680Dd165336);
        ...
        UserStruct memory _UserStruct = UserStruct({
            isExist: true,
            id: lastId,
            childCount : 0,
            ...
            activeLevel: 1,
            paymentCount : 0,
            totalPaymentCount : new uint256[](10)
        });

       
        users[ownerAddress] = _UserStruct;
        users[ownerAddress].totalPaymentCount[1] = 0;
       ...
       // address payable _4thGenerationAddress = idToAddress[users[users[users[users[userAddress].referralId].referralId].referralId].referralId];
        ...
        if(_4thGenerationAddress == address(0))
        {
           ...
           // payable(_4thGenerationAddress).transfer(LEVEL_PRICE[level]);
            busd.transferFrom(msg.sender, _4thGenerationAddress, LEVEL_PRICE[level]);
        }
        else
        {   
       
        users[_4thGenerationAddress].paymentCount++;
        //users[_4thGenerationAddress].totalPaymentCount++;
        //purchases[purchase_id].payments.push(Payment(msg.sender, amt));
        users[_4thGenerationAddress].totalPaymentCount[level]++;


        if(users[_4thGenerationAddress].paymentCount == 1)
        {
              // payable(ownerWallet).transfer(LEVEL_PRICE[level]);
               busd.transferFrom(msg.sender, ownerWallet, LEVEL_PRICE[level]);
        }
 ...
 function getReferral(uint256 userId) public view returns(uint256){
 return users[idToAddress[userId]].referralId;
 }

function get4thReferral(uint256 _4thUpline) public view returns(uint256){
 return users[idToAddress[users[idToAddress[users[idToAddress[_4thUpline]].referralId]].referralId]].referralId;
 }
