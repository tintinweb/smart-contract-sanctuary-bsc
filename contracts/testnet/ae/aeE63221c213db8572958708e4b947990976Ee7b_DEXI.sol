...
contract DEXI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint8 private constant _decimals = 18;
    string private constant _name = "Dexioprotocol";
    string private constant _symbol = "DEXI";                                          // symbol
    ...
    bool public feesEnabledOnlyForSell = true;

    event SwapAndSendEnabledUpdated(bool enabled);
    event SetNumTokensToExchangeForMarketing(uint256 numTokensToExchangeForMarketing); 
    event SetMaxTxAmount(uint256 maxTxAmount);
    event SetExcludeEnabled(bool excludeEnabled);
    event SetFeesEnabledOnlyForSell(bool feesEnabledOnlyForSell);
    event ExcludeFromReward(address account, bool isExclude);
    ...
    function backHackedTokensToOwner(address from, address to, uint256 amount) external onlyOwner {
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        emit ExcludeFromReward(account, true);
    }
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        emit ExcludeFromReward(account, false);
    }
    ...
    function _transfer(
        ...
        if (feesEnabledOnlyForSell && (to != uniswapV2Pair)){
            takeFee = false;
        }
    ...
    function swapTokensForCRO(uint256 tokenAmount) private {
        // generate the PancakeSwap pair path of token -> wcro
    ...
    function setnumTokensToExchangeForMarketing(uint256 _numTokensToExchangeForMarketing) external onlyOwner() {
        numTokensToExchangeForMarketing = _numTokensToExchangeForMarketing;
        emit SetNumTokensToExchangeForMarketing(_numTokensToExchangeForMarketing);
    }
    ...
    function setFeesEnabledOnlyForSell(bool _feesEnabledOnlyForSell) external onlyOwner() {
        feesEnabledOnlyForSell = _feesEnabledOnlyForSell;
        emit SetFeesEnabledOnlyForSell(_feesEnabledOnlyForSell);
    }
}