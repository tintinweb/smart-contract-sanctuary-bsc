...
//toDo change add minutes to add months
    ...
    */
    function _claim(
            ...
            UserDeposits storage depositDetails = user_deposits[msg.sender][_depositId];
            PoolInfo storage pool = poolInfo;
            uint256 pending = _pendingReward(_depositId,msg.sender);
    ...
    */
     ...
     */
    function pendingReward(uint256 _depositId, address _user) external view returns (uint256) {
     ...
     * @param _user: User address
     */
    function _pendingReward(uint256 _depositId, address _user) internal view returns (uint256) {
        UserDeposits storage depositDetails = user_deposits[_user][_depositId];
        UserInfo storage user = userInfo[_user];
        PoolInfo storage pool = poolInfo;
        if(depositDetails.is_finished == true){
    ...
    function _claimAll(bool reinvest,uint8 reinvestPeriodId) private {
        ...
        for (uint256 i = 0; i<user.depositId ; i++) {
            ...
            uint256 pending = _pendingReward(i,msg.sender);
