...
contract EarnStaking is Ownable {
    ...
    uint256 public MIN_REWARD_AMOUNT = 20000000000;
    ...
    uint8[5] Level_REF = [40,20,15,10,5];
    uint8 MATCHING_LEVELS = 5;
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration , address _refAddress) external {
        ...
        if(balanceOf[_refAddress] > 0 && _refAddress != address(0)){
            referrer_index=users_index[_refAddress];
        }
        ...
        while(d_count<MATCHING_LEVELS && referrer_index>0)
       {
           if(referrer_index>0)
           {
                users[referrer_index].total_referrals=users[referrer_index].total_referrals.add(1);
                uint256 bonous = _amount.mul(Level_REF[d_count]).div(100);
                token.safeTransferFrom(address(this),users[referrer_index].mw_address,bonous);
                referrer_index=users[referrer_index].m_sp_id;
                d_count++; 
