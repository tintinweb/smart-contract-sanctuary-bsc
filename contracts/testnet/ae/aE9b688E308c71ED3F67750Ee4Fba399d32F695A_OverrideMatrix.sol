...
contract OverrideMatrix is AccessControl {
    ...
    function _set6Location(address _setKey, address _setValue, uint256 _setGrade, uint256 _setLocation) internal {
        if (_setLocation == 0) {
            ...
            if (
                accountInfoList[accountInfoList[_setValue].matrix6Grade[_setGrade].vertex].referRecommender == address(0) ||
                !accountInfoList[accountInfoList[accountInfoList[_setValue].matrix6Grade[_setGrade].vertex].referRecommender].gradeExist[_setGrade]
            ) {
