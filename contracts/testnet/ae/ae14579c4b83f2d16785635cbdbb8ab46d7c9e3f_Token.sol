...
contract Token is Context, IERC20, Ownable {
    ...
    uint8 public maxMarketingandDevelopmentFee = 10;
    ...
    uint8 public _marketingandDevelopmentFee = 0;
    uint8 private _previousMarketingandDevelopmentFee = _marketingandDevelopmentFee;
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        numTokensToSell = amountOfTokenWei.mul(1).div(500);
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 marketingandDevelopmentFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <=maxTaxFee,"TF err");
        require(liquidityFee >= 0 && liquidityFee <=maxLiqFee,"LF err");
        require(marketingandDevelopmentFee >= 0 && marketingandDevelopmentFee <=maxMarketingandDevelopmentFee,"BBF err");
        _taxFee = taxFee;
    ...
    function setSwapAndLiquifyEnabled(bool _enabled) public onlyOwner {
        ...
        function setnumTokensToSell(uint256 _amount) public onlyOwner {
        numTokensToSell = _amount;
    }
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _marketingandDevelopmentFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingandDevelopmentFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint8 totFee  = _liquidityFee + _marketingandDevelopmentFee; 
        uint256 spentAmount = 0;
        uint256 totSpentAmount = 0;

        if(_marketingandDevelopmentFee != 0){
            spentAmount = contractTokenBalance.div(totFee).mul(_marketingandDevelopmentFee);
            swapTokensForMarketing(spentAmount);
