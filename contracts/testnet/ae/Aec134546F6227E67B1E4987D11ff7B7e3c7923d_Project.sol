...
contract Project is IProject, ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable nonReentrant {
        ...
        if (_projectInput.isPack) {
            require(isCreateNewToken ? _projectInput.isSingle : nftChecker.isERC721(token), "Only single token for pack");
            if (!_projectInput.isFixed) {
                require(_projectInput.maxPricePack > _projectInput.minPricePack && _projectInput.minPricePack > 0, "Invalid price");
                require(_projectInput.priceDecrementAmtPack > 0 && _projectInput.priceDecrementAmtPack <= _projectInput.maxPricePack - _projectInput.minPricePack, "Invalid price");
            }
        ...
        project.fixedPricePack = _projectInput.fixedPricePack;
        project.maxPricePack = _projectInput.maxPricePack;
        project.minPricePack = _projectInput.minPricePack;
        project.priceDecrementAmtPack = _projectInput.priceDecrementAmtPack;
