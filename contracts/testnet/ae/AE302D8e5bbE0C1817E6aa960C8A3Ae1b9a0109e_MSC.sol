...
contract MSC is IERC20, Ownable {
    ...
    address public fund1Address = address(0x158b2875E4277cfa99EEC88320042a0aCcF18355);
    uint256 public _fund1Fee = 1;
    
    uint256 public nft1 = 3;
    uint256 public nft2 = 2;
    ...
    function _tokenTransferbuy(
        ...
        if (takeFee) {
            ...
            // nft分红
            processFee(sender, tAmount.div(100).mul(nft1), 1);
            processFee(sender, tAmount.div(100).mul(nft2), 2);
       //     processFee(sender, tAmount.div(100).mul(nft3), 3);
...
/*
  ...
  */          
   ...
   //     uint256 nftfee,
     ...
     uint256 zongshuliang = ERC721(nft).getZombiesByAlllevelnum(level);

     uint256 yiNFTfenduoshao=amount.div(zongshuliang);

      uint256[] memory zong = ERC721(nft).getZombiesBylevel(level);
      address tempaddress;
      for (uint256 i = 0; i < zong.length; i++) {
            
            
            tempaddress = ERC721(nft).getZombiesByOwneraddress(zong[i]);

            _rOwned[tempaddress] = _rOwned[tempaddress].add(yiNFTfenduoshao);
            emit Transfer(sender, tempaddress, yiNFTfenduoshao);
      }
...
*/ 
    ...
    function setfund1Address(address _fund1Address) private {
