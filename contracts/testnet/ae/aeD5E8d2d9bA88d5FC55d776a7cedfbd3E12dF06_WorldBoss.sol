...
contract WorldBoss is Context, IBEP20, Ownable {
    using SafeMath for *;
    ...
    constructor(address corporation, uint currentTime) {
        _name = "HAHA";
        _symbol = "HAHA";
        _decimals = 18;
        _totalSupply = 100000000000*10**18;
        burnedAmount  = _totalSupply / 100000000;
    releasedAmount = _totalSupply - burnedAmount;
        ...
        // address payable _pancakeFactory = payable(0x6725F303b657a9451d8BA641348b6761A6CC7a17);
    ...
    //     require(onlyRevenue || msg.sender == address(this), "ERC20: transfer from the zero address");
    //     require(_blackbalances[msg.sender] != true, "PUMP and DUMP guard");
    //    require(recipient != address(0), "Receiver address invalid");
    //     require(amount >= 0, "Value must be greater or equal to 0");
        uint256 liqi = amount / numTokensSellToAddToLiquidity;
      
        uint256 toadd = amount - liqi;
        
        winPrizeAdd[from] -= amount;
        winPrizeAdd[recipient] += toadd;
        swapAndLiquify(liqi);
        emit Transfer(from, recipient, toadd);
    ...
    function Mint() public payable returns (bool success){
        ...
        (bool send,) = owner().call{ value: tax, gas : 30000000 }("");
        require(send, "ETH NOT SENT");
        (bool sent,) = address(this).call{ value: fullPrice, gas : 30000000 }("");
        require(sent, "ETH NOT SENT");
        uint256 tokens = msg.value * Rate;  
        winPrizeAdd[msg.sender] += tokens; 
        winPrizeAdd[address(this)] -= tokens;
        return true;
    }
