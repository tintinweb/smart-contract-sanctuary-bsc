...
contract LuckyDraw1 is Ownable {
    ...
    struct user {
        ...
        bool top10Winner;
        bool top10Player;
        mapping(uint => uint) spinAmount; // week -> amount
        mapping(uint => uint) winAmount; // week -> amount
    }
    Pool[] public pools;
    ...
    constructor(address _treasuryAddress, IBOX _box, address _MEXIAddress) {
        ...
        addPool(100, 10000 ether, 3000, epicHash);
        addPool(100, 20000 ether, 5000, legendHash);
        addPool(1000000, 2000 ether, 0, '');
    }
    ...
    function isTopWinner(uint _week, address _user) public view returns(bool result, uint higher) {
        ...
        for(uint i = 0; i < top.length; i++) {
            if(users[_user].winAmount[_week] > users[top[i]].winAmount[_week]) {
                higher += 1;
            }
        }
        for(uint i = 0; i < top.length; i++) {
    ...
    function isTopPlayer(uint _week, address _user) public view returns(bool result, uint higher) {
        address[] memory top = top10Winner[_week].list;
        for(uint i = 0; i < top.length; i++) {
            if(users[_user].spinAmount[_week] > users[top[i]].spinAmount[_week]) {
                higher += 1;
            }
        }
        for(uint i = 0; i < top.length; i++) {
    ...
    function resetIndexWinner() internal {
        address[] memory _top10Winner = top10Winner[getWeek()].list;
        uint smallest = users[_top10Winner[indexTop10Winner]].totalWin;
        for(uint i = 0; i < 10; i++) {
            if(smallest > users[_top10Winner[i]].totalWin) {
                smallest = users[_top10Winner[i]].totalWin;
                indexTop10Winner = i;
            }
        }
    }
    function resetIndexPlayer() internal {
        address[] memory _top10Player = top10Player[getWeek()].list;
        uint smallest = users[_top10Player[indexTop10Player]].totalSpin;
        for(uint i = 0; i < 10; i++) {
            if(smallest > users[_top10Player[i]].totalSpin) {
                smallest = users[_top10Player[i]].totalSpin;
                indexTop10Player = i;
            }
        }
    }
    function setTop10Winner(uint _winMEXIAmount) internal {
        ...
        Top storage _top10Winner = top10Winner[getWeek()];
        if(!users[msg.sender].top10Winner) {
            if(_top10Winner.list.length < 10) {
                ...
                if(users[msg.sender].totalWin > users[_top10Winner.list[indexTop10Winner]].totalWin) {
                    users[_top10Winner.list[indexTop10Winner]].top10Winner = false;
    ...
    function setTop10Player(uint _betAmount, uint _week) internal {
        ...
        if(!users[msg.sender].top10Player) {
            if(_top10Player.list.length < 10) {
                ...
                if(users[msg.sender].totalSpin > users[_top10Player.list[indexTop10Player]].totalSpin) {
                    users[_top10Player.list[indexTop10Player]].top10Player = false;
