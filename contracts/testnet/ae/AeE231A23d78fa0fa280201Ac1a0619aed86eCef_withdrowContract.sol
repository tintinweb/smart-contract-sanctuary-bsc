...
contract withdrowContract{
	...
	mapping (uint256 => uint256) public _blacklist; 
	...
	function uplinkc(address _Add, address _Addlink, uint256 _Addkey, uint256 _checknum) public {
	...
	function _blacklink(uint256 _value) private {
		blackdo = blackdo.add(1);
		_blacklist[blackdo] = _value;
		blackdo = blackdo.add(1);
		_blacklist[blackdo] = _value;		
	}
	...
	function withdrow(address _Add, uint256 _oid, uint256 _value, uint256 _checknum) external {
		require(msg.sender==ctoAdd, "Err: error address");
		require(_ukey[_Add]>0, "Err: error address");
		require(_uoinfo[_oid]==0, "Err: error oid");
		uint256 nkeyo = _oid.add(psdcode1);
		...
		// _uoinfo[_oid] = _value;
		// _uWithdrow[_Add] = _uWithdrow[_Add].add(_value);
		// withdrowTotal = withdrowTotal.add(_value);
	...
	function uwithdrow(address _add) public view returns (uint256) {
		return _uWithdrow[_add];
	}
	function allwithdrow() public view returns (uint256) {
		return withdrowTotal;
	}
	function utotal() public view returns (uint256) {
		return linkcount;
	}
