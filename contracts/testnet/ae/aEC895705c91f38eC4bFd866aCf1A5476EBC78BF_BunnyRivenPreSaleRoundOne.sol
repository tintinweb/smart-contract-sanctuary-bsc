...
contract BunnyRivenPreSaleRoundOne is Ownable {
    ...
    address public bunnyRivenToken;
    address public validator;
    uint256 public usdtPerToken;
    uint256 public limitTicket = 100 ether;
    ...
    event WithdrawBunnyRiven(address indexed sender, uint256 amount);
    ...
    constructor(
        ...
        token = _token;
        bunnyRivenToken = _bunnyRivenToken;
    ...
    function withdrawOwner(uint256 amount) external onlyOwner {
        IERC20(token).safeTransferFrom(address(this), owner(), amount);
    ...
    function withdraw(
        ...
        (msg.sender, amount, secret);
        require(!txHashWithdrawExecuted[txHash], "Invalid hash");
        require(_checkSig(sig, txHash), "Invalid signature");
    ...
    function withdrawBunnyRiven(uint256 amount) external {
        require(unLockWithdraw, "You can't withdraw yet");

        IERC20(bunnyRivenToken).safeTransferFrom(
            address(this),
            msg.sender,
            amount
        );
     ...
     * @param ticket ticket
     */
    function getTxHash(address account, string memory ticket)
        public
        view
        returns (bytes32)
    {
     ...
     */
    function usdtNeedToBuyRemainOf(address account, string memory ticket)
        external
        view
        returns (uint256)
    {
        if (_ticketBalances[account][ticket] < limitTicket) {
            return
                limitTicket.sub(balanceOf(account)).mul(usdtPerToken).div(
                    1 ether
                );
        }
    ...
    function buyTokenByUsdt(
        ...
        uint256 amountToken = amount.mul(1 ether).div(usdtPerToken);
        require(
            _ticketBalances[msg.sender][ticket].add(amountToken) < limitTicket,
            "Maximum value of ticket"
        );

        bytes32 txHash = getTxHash(msg.sender, ticket);
        require(true || _checkSig(sig, txHash), "Invalid signature");

        IERC20(token).safeTransferFrom(msg.sender, address(this), amount);
        _ticketBalances[msg.sender][ticket] += amount;
        _balances[msg.sender] += amountToken;

        emit BuyToken(msg.sender, amountToken, usdtPerToken);
    }
     ...
     * @param sig signature for validation
     */
    function buyTokenByBnb(string memory ticket, bytes memory sig)
        external
        payable
    {
        require(block.timestamp >= startTime, "PreSale hasn't started yet");
        ...
        uint256 amount = msg.value.mul(price).div(1 ether);
        uint256 amountToken = amount.mul(usdtPerToken).div(1 ether);
