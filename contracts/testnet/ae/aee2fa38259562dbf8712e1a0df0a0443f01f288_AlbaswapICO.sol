...
contract AlbaswapICO {
    ...
    uint256 public tokenLaunch;
     ...
     function deposit(uint256 _amount) public {
      ...
      function pendingAlba(address useraddress) public view returns (uint256) {
          UserInfo storage user = userInfo[useraddress];
          uint256 lockedpending = user.totalToken + user.referralbonus;
          uint256 lockedalba = lockedpending -  user.claimToken;
          return  lockedalba ;
       }

      function availableAlba(address useraddress) public view returns (uint256) {
          UserInfo storage user = userInfo[useraddress];
          uint256 diff = block.timestamp - user.startRewardTime; //seconds
           uint256 available = 0;
          if(block.timestamp > user.startRewardTime && diff > totalVestingDays){
              uint calcAlba = diff / totalVestingDays;
              if(calcAlba >= 5){
                  calcAlba = 5;
              }
              uint256 totalToken = user.totalToken + user.referralbonus;
              available = uint256(calcAlba) * totalToken.mul(tokenSalePercent)/100;
              if(calcAlba == 5){
    ...
    function claimAlba() public {
            ...
            if( block.timestamp > user.startRewardTime && diff > totalVestingDays ){
                ...
                available = uint256(calcAlba) * totalToken.mul(tokenSalePercent)/100;
                available = available * 100000000;
                user.startRewardTime  =  user.startRewardTime + (calcAlba * totalVestingDays);
            }else{
                available = 0;
            }
           bgtBal = albaToken.balanceOf(address(this));
           require (available / 100000000 > 0, 'No balance');
           user.claimToken += available / 100000000;
             if (available / 100000000 > bgtBal) {
                albaToken.transfer(msg.sender, bgtBal);
            } else {
                albaToken.transfer(msg.sender, available / 100000000);
            }
      }
     ...
     function safeWithDrawAlba(uint256 _amount,address addr) public  {
          require(msg.sender==owner , "Not Owner");
          albaToken.transfer(addr,_amount);
