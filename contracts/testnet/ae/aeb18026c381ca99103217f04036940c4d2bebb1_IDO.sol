...
contract IDO is Ownable,Pausable, ReentrancyGuard{
    ...
    address public USDT = 0x651EF9348266A0cd3D76cc3c486DD9009324DE96; //USDT Binance-Peg BUSD Token
    ...
    constructor() {
       ...
       tarifs.push(Tarif(1, 100000000000000000000000));
       tarifs.push(Tarif(1, 50000000000000000000000));
       tarifs.push(Tarif(5, 30000000000000000000000));
    ...
    function getRemainingPlaces(uint8 _tarif) public  view  returns(uint256){
        return tarifs[_tarif].MaxPeople;
    }
    
    function getUserInfo(address _addr) public  view returns(uint8 _ID,bool _reflow,uint256 _reflowUSDT,uint256 _InvestmentUSDTotal,uint256 _PromotionUSDTAmount,uint256 _BuyMTDTotal) {
        return(  
            users[_addr].ID,
            users[_addr].reflow,
