...
contract Pool is ERC20, Initializable, Ownable {
     ...
     * @param _referral referral address to pay commission to
    ...
    function deposit(
        ...
        if (_referral != address(0) && _referral != msg.sender) {
            require(_referral.isContract() == false, "INVALID_REFERRAL_ADDR");
            require(referrals[msg.sender] == address(0), "MUST_EMPTY_REFERRAL");
            referrals[msg.sender] = _referral;

            emit Referral(msg.sender, _referral);
        }
        ...
        if (poolValues == 0) {
            ...
            lpAmount = amount.mul(totalSupply()).div(poolValues);
        }

        if (lpAmount == 0) {
            revert("LPAMOUNT_ZERO");
        }
        ...
        emit Deposit(msg.sender, amount, lpAmount, block.timestamp);
    }
    ...
    */
    function exit(
        ...
        // pay fee
        uint256 feeForMaster;
        if (config.masterFee > 0) {
            feeForMaster = (config.masterFee * amountExitExpect) / 10000;
            require(
                IERC20(config.depositToken).transfer(owner(), feeForMaster)
            );

            accMasterFee += feeForMaster;
        }

        uint256 feeForReferral;
        if (config.referralFee > 0 && referrals[msg.sender] != address(0)) {
            feeForReferral = (config.referralFee * amountExitExpect) / 10000;
        ...
        uint256 totalExit = amountExitExpect.sub(
            feeForMaster.add(feeForReferral)
        );
        require(IERC20(config.depositToken).transfer(msg.sender, totalExit));
    ...
    function _swap(Route memory route) internal {
        uint256 amountInETH;
    ...
    function getSignDeposit(uint256 poolValues, uint256 expire)
        public
        view
        returns (bytes32)
    {
