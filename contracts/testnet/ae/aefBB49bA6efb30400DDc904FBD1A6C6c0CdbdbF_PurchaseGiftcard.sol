...
contract PurchaseGiftcard is ERC2771Context, Ownable {
    ...
    function purchaseByCurrency(
        ...
        require(_msgSender().balance > 0, "Insufficient token balance.");
    ...
    modifier onlyOperater() {
        require(operators[_msgSender()], "You are not Operator");
        _;
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < purchaseHistory[_buyer].length; i++) {
            if (
                keccak256(
                    abi.encodePacked(purchaseHistory[_buyer][i].orderID)
