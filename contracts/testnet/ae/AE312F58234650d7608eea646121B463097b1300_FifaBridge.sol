...
contract FifaBridge is SignData, Ownable, Pausable, Signed {
    using SafeMath for uint256;
    ...
    function setToken(address _token) public onlyOwner {
        fifaToken = _token;
        IBEP20(fifaToken).approve(address(this), uint256(- 1));
    }
    ...
    function init(address _token, uint256 _timeLimit, uint256 _limitWithdraw, address _hub) public {
    ...
    function changeHub(address _hub) external onlyOwner {
        hub = _hub;
    }
    ...
    function sendInGame(uint256 _amount) external whenNotPaused {
        IBEP20(fifaToken).safeTransferFrom(msg.sender, hub, _amount);

        uint256 nonceDepositLast = nonceDeposits[msg.sender];
        userDatas[msg.sender].totalDeposit = userDatas[msg.sender].totalDeposit.add(_amount);
        depositLogs[msg.sender][nonceDepositLast] = DepositLogs(_amount, block.number);
        nonceDeposits[msg.sender] = nonceDeposits[msg.sender].add(1);

        emit Deposit(msg.sender, nonceDepositLast, _amount);
    }

    function getAllInfo(address _user) public view returns (uint256, uint256, uint256,uint256,uint256){
        return (nonceDeposits[_user], nonces[_user], nonceClaims[_user],userDatas[_user].totalDeposit,userDatas[_user].totalClaim);
    }

    function withdraw(
        uint256 _amount,
        uint256 _earnInGame,
        uint256 _spendInGame,
        uint256 _timestamp,
        uint8 v,
        bytes32 r,
        ...
        address signer = getSigner(
            keccak256(
                abi.encode(address(this), this.withdraw.selector, msg.sender, nonceClaims[msg.sender], nonces[msg.sender], _earnInGame, _spendInGame, _amount, _timestamp)
            ), v, r, s
        );

        require(isSigner[signer], "Forbidden");
        require(userDatas[msg.sender].last.add(timeLimit) <= block.timestamp, "Exceed limit per withdrawal");

        uint256 totalEarn = userDatas[msg.sender].totalDeposit.add(_earnInGame);
        uint256 totalSpend = userDatas[msg.sender].totalClaim.add(_spendInGame);
        ...
        IBEP20(fifaToken).safeTransfer(msg.sender, _amount);
        userDatas[msg.sender].last = block.timestamp;

        emit Claim(msg.sender, nonceClaims[msg.sender], userDatas[msg.sender].totalDeposit, userDatas[msg.sender].totalClaim, _amount, _earnInGame, _spendInGame);

        userDatas[msg.sender].totalClaim = userDatas[msg.sender].totalClaim.add(_amount);
        nonceClaims[msg.sender]++;
        nonces[msg.sender]++;

    }

    function clearERC20(address token, address to, uint256 amount) external onlyOwner {
