...
// File contracts/libraries/NFTStructs.sol
...
// File contracts/interfaces/ITreasureLandExchange.sol
...
// File contracts/interfaces/IPolicy.sol
...
interface IRoyaltyManager {
    function addRoyalty(address policy) external;

    function removeRoyalty(address policy) external;

    function isRoyaltyWhitelisted(address policy) external view returns (bool);

    function viewWhitelistedRoyalties(
        uint256 cursor,
        uint256 size
    ) external view returns (address[] memory, uint256);

    function viewCountWhitelistedRoyalties() external view returns (uint256);
}
...
// File contracts/TreasureLandExchange.sol
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @notice Update royalty manager
     * @param _royaltyManager new royalty manager address
     */
    function updateRoyaltyManager(address _royaltyManager) external onlyOwner {
        require(
            _royaltyManager != address(0),
            "updateRoyaltyManager: Cannot be null address"
        );
        royaltyManager = IRoyaltyManager(_royaltyManager);
    ...
    function matchAskWithTakerBid(
        ...
        bool isExecutionValid = IPolicy(makerAsk.policy).canExecuteTakerBid(
            takerBid,
            makerAsk
        );

        require(isExecutionValid, "Policy: Execution invalid");
        ...
        _transferFeesAndFunds(
            ...
            takerBid.taker,
            makerAsk.signer,
            advanceOrder.items[itemIndex].price
        );

        // Execution part 2/2
        _transferNonFungibleToken(
            advanceOrder.items[itemIndex].itemType,
            advanceOrder.items[itemIndex].collection,
            makerAsk.signer,
            takerBid.taker,
            advanceOrder.items[itemIndex].tokenId,
            advanceOrder.items[itemIndex].amount
        );
    ...
    function matchBidWithTakerAsk(
        ...
        bool isExecutionValid = IPolicy(makerBid.policy).canExecuteTakerAsk(
            takerAsk,
            makerBid
        );
    ...
    function buyNFT(
        AdvanceOrder calldata advanceOrder,
        uint256 itemIndex,
        address taker
    ) external payable {
        require(
    ...
    function _transferNonFungibleToken(
        ...
        if (itemType == ItemType.ERC1155) {
            ...
            IERC1155 instance = IERC1155(collection);
            require(
                instance.balanceOf(from, tokenId) > amount,
                "1155 balance error"
            );
            instance.safeTransferFrom(from, to, tokenId, amount, "");
        } else if (itemType == ItemType.ERC721) {
            // erc721 safeTransferFrom
            IERC721 instance = IERC721(collection);
            require(instance.ownerOf(tokenId) == from, "Not Owner");
