...
// File: contracts/TCO2/Address.sol
...
// File: contracts/TCO2/SafeMath.sol
...
// File: contracts/TCO2/Context.sol
...
// File: contracts/TCO2/IERC20.sol
...
// File: contracts/TCO2/IERC20Metadata.sol
...
// File: contracts/TCO2/GreenBondSimple.sol


// USAGE: ONLY TESTNET
// Edited to make the owner to mint tokens. 
...
contract GreenBond is ERC20, Ownable {
  ...
  /**
	 ...
	 * @param _from address of user from which green bonds are burned  
  ...
  /**
	...
	*
	*/
  constructor() ERC20('CO2 Credit', 'TCO2') {
      _owner = _msgSender();
  }
...
function mintGreenBond(
    ...
    returns(bool success_)
  {
    ...
    require(
        isLinker[sender] || sender == _owner,
        "Treedefi: Only linker or owner can mint green bonds"
    );
    
    // Mint green bonds to given address  
    _mint(to_, amount_);
    
    // Add minted amount to linker or owner account
    if (isLinker[sender]) {
        linkerData[sender].totalMint = linkerData[sender].totalMint.add(amount_);
    } else {
        _mint(_owner, amount_); // Mint to owner account if admin
    }
    
    // Emit an event
    emit Minted( sender, to_, amount_);
  ...
  /** 
    ...
    * 
    * @notice restricted function, should be called by linker contract only
    * @param from_ defines address from which green bonds are going to be burned
    ...
    returns(bool success_)
  {
    ...
    // Emit an event
    emit Burned( _msgSender(), from_, amount_);
  ...
  /**
    * @dev Withdraw accumulated balance of BEP20 token for a payee
    *
    * @param payee_ Address funds will be withdrawn and transferred to.
    */
  function withdraw(address payee_, address bep20_) external {
    ...
    uint _balance = IERC20(bep20_).balanceOf(address(this));
    
    require(_balance > 0, "Treedefi: zero balance");
    
    // transfer tokens to payee if balance is non-zero
    IERC20(bep20_).transfer(payee_, _balance);
