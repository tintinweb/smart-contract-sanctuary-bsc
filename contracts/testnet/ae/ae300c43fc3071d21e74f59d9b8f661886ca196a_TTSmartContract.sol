...
contract TTSmartContract {
    ...
    function registration(address referrerAddress, address tknAddress, uint256 amount) external payable {
        registrationbase(msg.sender, referrerAddress, tknAddress, amount);
    }
    
    function registrationbase(address userAddress, address referrerAddress, address tknAddress, uint256 amount) private {
    ...
    function buyLevel(uint8 matrix, uint8 level, address tknAddress, uint256 amount) external payable {
        ...
        if (tokenAddress != nullAddress){
            ...
            levelCost = amount;            
            uint256 allowance = _token.allowance(msg.sender, address(this));
            require(allowance >= levelCost, "Check the token allowance");

            _token.safeTransferFrom(msg.sender, address(this), levelCost);
        }
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level) private {
        ...
        if (tokenAddress != nullAddress){
            _token.safeTransfer(receiver, levelCost);
        }
        else{
            address(uint160(receiver)).transfer(levelCost);
            // if (!address(uint160(receiver)).send(levelCost)) {
