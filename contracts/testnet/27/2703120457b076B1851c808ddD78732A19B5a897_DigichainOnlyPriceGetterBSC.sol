...
// File: polycrystal-on-chain-stats/contracts/interfaces/IDigiPair.sol
...
// File: polycrystal-on-chain-stats/contracts/libraries/DigichainOnlyPriceGetterBSC.sol
...
// This library provides simple price calculations for DigiSwap tokens, accounting
...
library DigichainOnlyPriceGetterBSC {

//    using AMMData for AmmData;

    

    address public constant FACTORY = 0x6126e30dE58A77De3d7649E06a76635EfBeF2FfC; //DigiFactory

    bytes32 public constant INITCODEHASH = hex'0ab8e09f150a2977a5e3ea39638efb929fa2fc264ff2b0677919c1663f4ef3ff'; // for pairs created by DigiFactory
    ...
    address constant BUSD_WBNB_PAIR = 0x2Cd90ecb596E8b4bEf7E3b062010Bffb51B2d92c; // busd is token1
    ...
    function getBNBPrice() public view returns (uint) {
        ...
        (uint daiReserve, uint wbnbReserve0,) = IDigiPair(DAI_WBNB_PAIR).getReserves();

        (uint wbnbReserve1, uint busdReserve,) = IDigiPair(BUSD_WBNB_PAIR).getReserves();

        (uint usdtReserve, uint wbnbReserve2,) = IDigiPair(USDT_WBNB_PAIR).getReserves();
    ...
    //tokens using digichain prices. If the provided token is not a LP, it will attempt to price the token as a
    ...
    function getRawLPPrice(address lp, uint bnbPrice) internal view returns (uint) {
        ...
        try IDigiPair(lp).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {

            

            address token0 = IDigiPair(lp).token0();

            address token1 = IDigiPair(lp).token1();

            uint totalSupply = IDigiPair(lp).totalSupply();
    ...
    function pairTokensAndValue(address token, address peg) private view returns (uint tokenNum, uint pegValue) {
        ...
        try IDigiPair(tokenPegPair).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
