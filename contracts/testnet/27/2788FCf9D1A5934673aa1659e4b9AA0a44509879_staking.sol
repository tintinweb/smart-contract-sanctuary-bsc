...
contract staking is  Context, IBEP20, IBEP20Metadata {
  ...
  uint256 operatingFee = 0;
  uint256 lockfee = 0;
  uint256 public harvestFee = 400000000000000;  //0.0004 bnb per person per harvest
  uint256 burnFee = 0;
  uint256 claimFee = 0;
  uint256 public interval = 120;
  ...
  // uint256 private rewardEndtime = block.timestamp+62208000; //720 days
    ...
    mapping(address => uint256) public userStartLockTime;
    ...
    function lockandmint(uint amount) public {
        ...
        if (!firstlock[msg.sender]){
            userStartLockTime[msg.sender] =  block.timestamp;
...
function viewHarvest (address account) public view returns (uint256) {
        ...
        uint balance = balanceOf(account);
        uint lap;  
        require(block.timestamp <= rewardEndtime,"reward period is finished, please harvest remaining amount");  
        if ((balance >= 1 ) && (block.timestamp <= rewardEndtime)) {            
        lap = ((block.timestamp - lastHarvestTime[account]) * REWARD_RATE)* 1e20;
...
//         lap = ((block.timestamp - lastHarvestTime[msg.sender]) / 360) * 360000000000000;        
//         }
//         else if ( (block.timestamp >= rewardEndtime) && (lastHarvestTime[msg.sender] <= rewardEndtime)){             
//         lap = ((rewardEndtime - lastHarvestTime[msg.sender]) / 360) * 360000000000000;  
...
// }
    ...
    function updateusers() public{
     ...
     function harvest() external payable updateAccountReward(msg.sender) {
    //    function harvest() external updateAccountReward(msg.sender) {
         require(userStartLockTime[msg.sender] + interval  <= block.timestamp," after 1 day user will harvest");
        supplyDenominator = wrapped_totalSupply;
        uint256 reward = rewardHarvest[msg.sender];
     require(msg.value >= harvestFee," harvest fee required");
        ...
        return reward ;
         ...
         function changeinterval(uint _time)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        interval = _time;
        return interval;
        }

        function changeOperatingFee(uint _fee)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        operatingFee = _fee;
        return operatingFee;
        }
        function changeLockFee(uint _fee)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        lockfee = _fee;
        return lockfee;
        }
        function changeHarvestFee(uint _fee)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        harvestFee = _fee;
        return harvestFee;
        }
        function changeBurnFee(uint _fee)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        burnFee = _fee;
        return burnFee;
        }
        function changeClaimFee(uint _fee)public returns (uint256) {
        require(zeroOwner == msg.sender, "You cannot change.");
        claimFee = _fee;
        return claimFee;
    }
    ...
    function addburn(uint amount) public  returns(burnStruct memory burnTable){
        require(userStartLockTime[msg.sender] + interval  <= block.timestamp," after 1 day user will burn the vlfi token");
        require(amount > 0, " greater than 0");
    ...
    payable(zeroOwner).transfer(_self.balance);
    }
