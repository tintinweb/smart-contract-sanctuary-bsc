...
abstract contract Ownable is Context {
    ...
    constructor() {
        //0xAe9A020579805FD6994E84ba2BDf90e36EEbcC3A
        _owner = _msgSender();
...
contract SWV8 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "SWV8";
    string private _symbol = "SWV8";
    uint256 private _initSupply = 100000000 * 10**18;

    uint256 public buyTokenThreshold;
    uint256 public sellTokenThreshold;

    uint256 public buyFee = 60; // 6% init buyFee
    uint256 public sellFee = 80; // 8% init sellFee

    // Fees

    uint256 public buyMarketingFee = 20; // 2%
    uint256 public buySantaPresentFee = 20; // 2%
    uint256 public buyTeamFee = 0; // 0%
    uint256 public buyLpFee = 20; // 2%
    uint256 private dexBuyFeeTotal; // Sum of all buyfees

    uint256 public sellMarketingFee = 20; // 2%
    uint256 public sellSantaPresentFee = 40; // 4%
    uint256 public sellTeamFee = 0; // 0%
    uint256 public sellLpFee = 20; // 2%
    uint256 private dexSellFeeTotal; // sum of all sellFees

    address public marketingWallet;
    address public santaPresentWallet;
    ...
    constructor() {
        ...
        taxAddress = 0x101c40D5d49a8CbC423A09Eee209fd2aDA2a220a; // alt 0xAe9A020579805FD6994E84ba2BDf90e36EEbcC3A
        marketingWallet = 0x2F611816fE2849F73CF377d36Bcb32Ad0347EF23;
        santaPresentWallet = 0x3046CE78AFb84591e8Da8e948924f6CD95d9cc0C;
        TeamWallet = 0x3046CE78AFb84591e8Da8e948924f6CD95d9cc0C;

        maxTokenPercentage = 25; // initial 0.25%
    ...
    function excludeFromFeePreset() internal {
        ...
        _isExcludedFromFee[santaPresentWallet] = true;
    ...
    function modifyMaxTokenPercentage(uint256 _newPercent) public onlyOwner {
        ...
        maxTokenAllowance = _totalSupply.mul(maxTokenPercentage).div(10000);
        uint256 inPercent = maxTokenPercentage.div(100);
    ...
    function setBuySantaPresentFee(uint256 _newFee) public onlyOwner {
        buySantaPresentFee = _newFee;
        setBuyTotalTax();
    }
    ...
    function setSellSantaPresentFee(uint256 _newFee) public onlyOwner {
        sellSantaPresentFee = _newFee;
        setSellTotalTax();
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (_isPairAddress[recipient]) {
            ...
            if (
                overSellThreshold && !inSwapAndLiquify && _from != pairAddress
            ) {
                uint256 sellBalance = accumulatedSellFees;
                swapAndLiquify(sellBalance);
                // zero accumulated sell fees and falsify overSellThreshold ... (reset)
        ...
        if (_isPairAddress[sender]) {
            ...
            if (overBuyThreshold) {
                uint256 buyBalance = accumulatedBuyFees;
                swapAndLiquify(buyBalance);
    ...
    function swapAndLiquify(uint256 contractBalance) private lockTheSwap {
        ...
        // To determine if buy or sell fees
        if (overSellThreshold) {
            // To send BNB to respective addresses ( Individually Taxed acc to sell values) .
            uint256 _marketingFee = workingBal.mul(sellMarketingFee).div(
                dexSellFeeTotal
            );
            payable(marketingWallet).transfer(_marketingFee);

            uint256 _sPFee = workingBal.mul(sellSantaPresentFee).div(
                dexSellFeeTotal
            );
            payable(santaPresentWallet).transfer(_sPFee);

            uint256 _teamFee = workingBal.mul(sellTeamFee).div(dexSellFeeTotal);
            payable(TeamWallet).transfer(_teamFee);

            uint256 _LpFee = workingBal.mul(sellLpFee).div(dexSellFeeTotal);
            payable(taxAddress).transfer(_LpFee);
        }

        if (overBuyThreshold) {
            // To send BNB to respective addresses ( Individually Taxed acc to buy values) .
            uint256 _marketingFee = workingBal.mul(buyMarketingFee).div(
                dexBuyFeeTotal
            );
            payable(marketingWallet).transfer(_marketingFee);

            uint256 _sPFee = workingBal.mul(buySantaPresentFee).div(
                dexBuyFeeTotal
            );
            payable(santaPresentWallet).transfer(_sPFee);
        ...
        lockTheSwap
    {
        ...
        uint256 _sPFee = workingBal.mul(buySantaPresentFee).div(dexBuyFeeTotal);
        payable(santaPresentWallet).transfer(_sPFee);
...
/*
    Max wallet 1%, option to change it to higher number. [check]
    Wallet 2 wallet transfer no tax . [check]
    Tokenomics . [check]
    Lp tax . [check]
    Marketing tax . [check]
    Santa present tax. [check]
    Team tax . [check]
    Max 10% tax possible in total . [check]
    Buy and sell tax needs to be set differently to have different % of tax. [check] [check]
    Example ( buy tax 8% and sell 10% )
    Minting not possible . [check]
    Trading pause not possible . [check]
    No Blacklist function. [check]
    Whitelist function ( if needed for Volume bot ). [check]
    ...
    Marketing wallet: 0xD8b4182A878b9b7Fb5BfE639f3188a5eBd602a46

    SantaPresentWallet: 0xe8B05Faf82bb940E13ca12bcC43Da75d671C460b

    TeamWallet: 0x74370fa9585e840233952c3E7a34f508371Ba31f
*/