...
contract GENIPassSaleV2 is IGENIPassSaleV2, IERC721ReceiverUpgradeable, PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    mapping(address => mapping(uint => ItemSaleInfo)) public itemSaleInfos;
    mapping(address => mapping(address => mapping(uint => uint))) public itemPointCount;
    mapping(address => uint) public itemPoint;
    ...
    function setReferalBoxPoint(address token, uint point) external onlyOperator {
        require(point > 0, "GENIPassSale: invalid piont");
        itemPoint[token] = point;
        emit SetReferralItemPoint(token, point);
    }
    ...
    function putOnSale(address token, uint tokenId, uint price, uint startTime, uint expirationTime) public override whenNotPaused onlyOperator {
        ...
        if (token == address(geniPass)) {
            ...
            ItemSaleInfo storage saleInfo = itemSaleInfos[token][tokenId];
            saleInfo.price = price;
            saleInfo.startTime = startTime;
            saleInfo.expirationTime = expirationTime;
        
            emit ItemsPutOnSale(token, tokenId, saleInfo.price, treasury, startTime, expirationTime);
        }
    }
    ...
    function updateSale(address token, uint tokenId, uint price) external override onlyOperator whenNotPaused {
        ...
        if(token == address(geniPass)) {
            ...
            emit ItemsUpdateSale(token, tokenId, saleInfo.price, treasury);
        }
    }
    ...
    function removeFromSale(address token, uint tokenId) external override onlyOperator whenNotPaused nonReentrant {
        ...
        if(token == address(geniPass)) {
            ...
            uint startTime = itemSaleInfos[token][tokenId].startTime;
            require(startTime > 0, "GENIPassSale: not for sale");

            IERC721(token).transferFrom(address(this), msg.sender, tokenId);

            emit ItemsRemoveFromSale(token,tokenId, treasury);
            delete itemSaleInfos[token][tokenId];
        }
    }
    ...
    function purchase(address token, uint tokenId, uint buyPrice) external override whenNotPaused nonReentrant {
        ...
        if (token == address(geniPass)) {
            ...
            uint payPrice = calculatePayPrice(buyer, salePrice, token);
            ...
            saleInfo.startTime = 0;
            saleInfo.expirationTime = 0;
            emit ItemsBought(token, tokenId, buyer, treasury, saleInfo.price, payPrice);
        }
    }
    ...
    function purchaseWithReferral(address token, uint tokenId, uint buyPrice, address referralAddress, uint nonce, bytes memory signature) external override whenNotPaused nonReentrant {
        ...
        if(token == address(geniPass)) {
            ...
            uint passAmount = passCount[referralAddress][monthIndex] + getReferalPoint(address(geniPass),tokenId);
            ...
            uint itemPointAmount = itemPointCount[token][referralAddress][monthIndex] + getReferalPoint(token, tokenId);
            
            salePrice = payPrice * (1000000 -  getDiscountReferral(itemPointAmount)) / decimalRate / 100;
            geni.safeTransferFrom(buyer, treasury, salePrice);
            if (getDiscountReferral(itemPointAmount) > 0) {
                geni.safeTransferFrom(buyer, referralAddress, payPrice - salePrice);
            }

            IERC721(token).transferFrom(address(this), buyer, tokenId);

            itemPointCount[token][referralAddress][monthIndex] = itemPointAmount;

            saleInfo.startTime = 0;
            saleInfo.expirationTime = 0;
            emit ItemsBoughtWithReferral(token, tokenId, buyer, treasury, referralAddress, nonce, saleInfo.price, payPrice, payPrice - salePrice);
        }
    ...
    function withdrawNftEmergency(TokenInfoWthdraw[] memory inputs) external onlyOperator {
        for (uint i = 0; i < inputs.length; i++) {
            ...
            if(token == address(geniPass)) {
                delete saleInfos[tokenId];
            } else if (token == address(mysteryBox)) {
                delete itemSaleInfos[token][tokenId];
            }
        }
    }
    ...
    function getReferalPoint(address token, uint passId) internal view returns (uint poins) {
        if (token == address(geniPass)) {
            ...
            poins = itemPoint[token];
        }
        
    }

    function getReferalPointPass() public view returns (uint stone, uint topaz, uint citrine, uint ruby, uint diamond) {
    ...
    function calculatePayPrice(address buyer, uint salePrice, address nftToken) public view returns (uint payPrice) {
        payPrice = IERC721(nftToken).balanceOf(buyer) > 0 ? (salePrice - (salePrice * discountRate / 100 / decimalRate)) : salePrice;
    }
...
interface IGENIPassSaleV2 {
    ...
    event SetReferralItemPoint(address token, uint point);
    ...
    event ItemsUpdateSale(address token, uint indexed itemId, uint newPrice, address seller);
    event ItemsRemoveFromSale(address token, uint indexed itemId, address seller);
    event ItemsBought(address token, uint indexed itemId, address buyer, address seller,  uint256 price, uint discountedPrice);
    event ItemsBoughtWithReferral(address token, uint indexed tokenId, address buyer, address seller, address referal, uint nonce, uint256 price, uint discountedPrice, uint referalBonus);
