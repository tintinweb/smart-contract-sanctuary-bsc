...
contract Staking is Ownable, ReentrancyGuard{
    ...
    function stake(uint256 tokenId) external nonReentrant{
        require(stakigEnabled, "Stake is not enabled");
    ...
    function setStakingEnabled(bool _enabled) public {
        require(HAPPY_MONKEY != address(0), "HAPPY_MONKEY is not available");
        require(HAPPY_COIN != address(0), "HAPPY_MONKEY is not available");
        stakigEnabled = _enabled;
        if(startTimestamp == 0) startTimestamp = block.timestamp;
        emit StakingEnabled(_enabled);
    }

    function setMultiplierPerLevel(uint256 _level, uint256 _multiplierPerLevel) public {
