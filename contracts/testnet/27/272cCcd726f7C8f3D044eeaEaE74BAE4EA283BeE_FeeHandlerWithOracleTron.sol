 ...
 */
contract FeeHandlerWithOracleTron is IFeeHandler, AccessControl, ERC20Safe {
    ...
    function _calculateFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) internal view returns(uint256 fee, address tokenAddress) {
        ...
        // txCost = dstGasPrice (tron fee limit in sun currency) * Token Effective Rate (rate of dest base currency to token) / 1e6
        uint256 txCost = oracleMessage.dstGasPrice * oracleMessage.ter / 1e6;
