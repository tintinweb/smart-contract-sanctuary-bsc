...
contract ARA  is ERC20 {
    ...
    function _transfer(
        ...
        if((ammPairs[from]  || ammPairs[to]) && swapstatus == false){
            require( from == address(_tokenOwner) || to == address(_tokenOwner)|| from == address(fundAddress) || to == address(fundAddress)||from == address(_owner) || to == address(_owner),"only _tokenOwner!");
        }
