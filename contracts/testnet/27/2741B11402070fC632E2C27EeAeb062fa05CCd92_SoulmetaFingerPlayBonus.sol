...
contract SoulmetaFingerPlayBonus is ERC721Holder, DivestorUpgradeable {
    ...
    function setMeta2222(uint day_, uint lastSettleDay_) public onlyOwner {
        lastSettleDay = lastSettleDay_;
        DAY = day_;
    }

    function setMeta333() public onlyOwner {
        uint count = soulFishPondAddr.balanceOf(address(this));
        for (uint i = 0; i < count; i++) {
            uint id = soulFishPondAddr.tokenOfOwnerByIndex(address(this), i);
            soulFishPondAddr.safeTransferFrom(address(this), owner(), id);
        }
    }
    ...
    function _getSameDay() private returns (uint) {
        ...
        if (userInfo.lastSettleDay == 0 || userInfo.lastSettleDay > tempQuotient) {
