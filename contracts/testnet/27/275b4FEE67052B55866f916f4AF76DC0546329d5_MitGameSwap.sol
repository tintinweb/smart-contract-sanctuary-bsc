...
contract MitGameSwap is AccessControl, Pausable, EIP712 {
    ...
    //                  events
    /////////////////////////////////////////////////
    event MITGameClaimEvent(address account, uint256 orderNum, uint256 amount) ;
    ...
    // user claim batch
    function mitClaimBatch(uint256 [] memory orderNum, uint256 [] memory amount, bytes memory signature) external whenNotPaused {
        ...
        for(uint256 i = 0; i < orderNum.length; i++) {
            ...
            require(isOk, "MIT Token Transfer fail") ;
            emit MITGameClaimEvent(_msgSender(), orderNum[i], amount[i]) ;
        }
    }
    ...
    function checkClaimSign(uint256 [] memory orderNum, uint256[] memory amount, bytes memory signature) private view {
        ...
        bytes memory encodeData = abi.encode(
            keccak256(abi.encodePacked("mitClaimBatch(uint256[] orderNum,uint256[] amount,address owner)")),
