...
abstract contract MementoRecipe is NativeCoinSender, Ownable {
    ...
    address public everRiseToken = 0xC17c30e98541188614dF99239cABD40280810cA3;
...
contract StakingAchievement is MementoRecipe {
    ...
    function claimStakingAchievement(uint256 tokenId, uint256 stakeNftId) external payable {
        address from = _msgSender();
...
contract MementoRise is EverRiseTokenManaged, royaltyHandler, NativeCoinSender, ERC165, IERC1155, IMementoRise, IERC1155MetadataURI, IOpenSeaCollectible {
    ...
    function mint(address to, uint256 tokenId, uint256 amount) external {
        ...
        emit TransferSingle(address(this), address(0), to, tokenId, amount);
    }
    ...
    function create(uint256 tokenId, uint256 amount) external payable {
        ...
        emit TransferSingle(address(this), address(0), from, tokenId, amount);
    }
    ...
    function _transmuteMultiple(ITransmuteMultipleRecipe transmutator, uint256[] calldata toTokenIds, uint256[] calldata toAmounts, uint256[] calldata fromIds, uint256[] calldata fromAmounts) private {
        ...
        emit TransferBatch(address(this), from, address(0), fromIds, fromAmounts);
        ...
        emit TransferBatch(address(this), address(0), from, toTokenIds, toAmounts);
    }
    ...
    function transmuteSingle(uint256 toTokenId, uint256 toAmount, uint256[] calldata fromIds, uint256[] calldata fromAmounts) external payable walletLock(_msgSender()) {
        ...
        // Mint new tokens
        AddBalance(from, toTokenId, toAmount);
        emit TransferSingle(address(this), address(0), from, toTokenId, toAmount);
    }
    ...
    function _doSafeBatchTransferAcceptanceCheck(address operator, address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) private view {
        if (isContract(to)) {
    ...
    function _doSafeTransferAcceptanceCheck(address operator, address from, address to, uint256 id, uint256 amount, bytes memory data) private view {
        if (isContract(to)) {
    ...
    function bridgeNftsOut(address from, uint256[] calldata ids, uint256[] calldata amounts) external {
        ...
        emit TransferBatch(address(this), from, address(0), ids, amounts);
    ...
    function bridgeNftsIn(address to, uint256[] calldata ids, uint256[] calldata amounts) external {
        ...
        emit TransferBatch(address(this), address(0), to, ids, amounts);
