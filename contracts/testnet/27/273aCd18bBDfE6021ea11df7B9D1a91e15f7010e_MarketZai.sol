...
contract MarketZai is ERC721Enumerable, Ownable {
    ...
    uint256 public lastOwnerTicketsCreation;
    uint256 public blockPerDay = 28800; //testnet 17280 ::: mainNet 43200

    constructor(address _BZAI) ERC721("Ticket_ZAI_NFT", "TICKET") {
        BZAI = IERC20(_BZAI);
    ...
    uint256[4] public dutchMultiplicators = [200,400,800,10000];

    function setGameAddresses(address _address) external onlyOwner{
        gameAddresses = IAddresses(_address);
    }

    function createTickets() external onlyOwner {
        require(block.timestamp >= lastOwnerTicketsCreation + 30 days,"Function active only 1 time by month max");
        lastOwnerTicketsCreation = block.timestamp;
        address claimNFTsAddress = gameAddresses.getClaimNFTsAddress();
        for(uint256 i = 0 ; i < 30 ; ){
            _mintTicket(claimNFTsAddress);
        }
    }

    function setMultiplicators(uint256[4] memory _dutchMultiplicators) external onlyOwner {
        require(
    ...
    function buyBronzeZai(string memory _name, uint256 _maxPrice) external returns(uint256){
       return  _buyZai(0,_maxPrice,_name,msg.sender);
    }  

    function buySilverZai(string memory _name, uint256 _maxPrice) external returns(uint256){
        return _buyZai(1,_maxPrice,_name,msg.sender);
    }

    function buyGoldZai(string memory _name, uint256 _maxPrice) external returns(uint256){
        return _buyZai(2,_maxPrice,_name,msg.sender);
    }

    function buyPlatinumZai(string memory _name, uint256 _maxPrice) external returns(uint256){
        return _buyZai(3,_maxPrice,_name,msg.sender);
    }

    function _buyZai(uint256 _state, uint256 _maxPrice,string memory _name, address _user) internal returns(uint256 zaiId){
        ...
        zaiId = _generateRandomZai(_name, _user, _state);
        emit ZaiBorned(_state, zaiId, msg.sender);

    }

    function getZaiPrice(uint256 _state) external view returns(uint256){
        return _getZaiPrice(_state);
    } 

    function getLastZaiSaleDatas(uint256 _state) external view returns(uint256 price, uint256 blockNumber){
    ...
    function _getZaiPrice(uint256 _state) internal view returns(uint256){
        INursery I = INursery(gameAddresses.getNurseryAddress());
        uint256 _price;
        uint256 _nurseriesPreMintedNumber = I.getPreMintNumber();

        for(uint256 i = 1 ; i <= _nurseriesPreMintedNumber ;){
            ZaiStruct.EggsPrices memory _eggPrices = I.getEggsPrices(i);

            if(_state == 0 ){
                _price += _eggPrices.bronzePrice * dutchMultiplicators[0] /100;
            }
            if(_state == 1 ){
                _price += _eggPrices.silverPrice * dutchMultiplicators[1] /100;
            }
            if(_state == 2 ){
                _price += _eggPrices.goldPrice * dutchMultiplicators[2] /100;
            }
            if(_state == 3 ){
                _price += _eggPrices.platinumPrice * dutchMultiplicators[3] /100;//100;
            }

            unchecked {++ i; }
        }

        _price = _price / _nurseriesPreMintedNumber;
    ...
    function _calculateDutchPrice(uint256 _state, uint256 _price) internal view returns(uint256){
        ...
        uint256 _priceToReturn = _price;
        if(_nbOfDaysPast > 0){
            // divide per 2 for each days past
            for(uint256 i = 0 ; i < _nbOfDaysPast ; ){
                _priceToReturn /= 2;
                unchecked {++ i ;}
            }
        }
        // calculate value to reduce for each block past (each days price is divide by 2)
        uint256 _toReducePerBlock = _priceToReturn / blockPerDay / 2; 
        // calculate number of block in the current day
        uint256 _pastBlockInCurrentDay = _pastBlock - (_nbOfDaysPast * blockPerDay);
        //reduce nber of block past in the day multiply by value to reduce per block
        _priceToReturn = _priceToReturn - (_pastBlockInCurrentDay * _toReducePerBlock);
        
        return (_priceToReturn);
    ...
    function _randomMint (string memory _name)
        internal
        returns (uint256 zaiId)
    {
        bytes32 _id = keccak256(abi.encodePacked(msg.sender, block.timestamp, nonce, _name));
        nonce = uint256(_id);
