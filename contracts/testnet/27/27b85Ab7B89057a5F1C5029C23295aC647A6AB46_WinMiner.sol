...
WinMiner is set up to passively reward holders in the stablecoin BUSD. When you deposit your BUSD using the dApp you will receive:
...
- Extra stacking compound bonus when compounding every 12hrs+ for all holders of Quantic token based on a tiered system.
- Extra Referral Bonuses for all holders of Quantic token based on a tiered system.
- 3% of the total tax will be used to buy-back Quantic tokens.
...
- Investors can choose compound intervals between 4 and 24 hours (suggest to choose 13-24 hours to receive stacking Quantic/Partnership bonuses)
...
contract WinMiner {
    ...
    // IBEP20 quantic = IBEP20(0x7700Edc3DBb30cBB7603212E061c804220c3cA54); /// mainnet
    IBEP20 quantic = IBEP20(0xD7C1b1bE2e0D6568F152a1B8BA40BF7CCfD3EC4f); /// testnet
    ...
    constructor(address _bbadr, address _devadr, address _mktadr, address _autoadr, address _executoradr) {
    ...
    function buyQubic(address ref, uint256 amount) public{
        require(contractStarted);
        User storage user = users[msg.sender];
        require(automations[msg.sender].day < 1, "Holder is automated!");
    ...
    function runAutomate() external {
        require(msg.sender == owner || msg.sender == executoradr, "Admin use only.");
        require(contractStarted);
    ...
    function PRC_MARKET_QUBIC_DIVISOR(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 50); /** 50 = 2% **/
        MARKET_QUBIC_DIVISOR = value;
    }
