...
contract PoolFactory is IPoolFactory, Initializable, ProxyFactory {
    event PoolLogicCreated(address newPoolLogic, bytes data);
    event PoolManagerLogicCreated(address newPoolManagerLogic, bytes data);
    ...
    function createPool(
        ...
        address newPoolManagerLogic = deploy(msg.sender, poolManagerLogicData, 1);
        ...
        address newPoolLogic = deploy(msg.sender, poolLogicData, 0);

        IPoolManagerLogic(newPoolManagerLogic).setPoolLogic(newPoolLogic);

        deployedPools.push(newPoolLogic);

        emit PoolLogicCreated(newPoolLogic, poolLogicData);
        emit PoolManagerLogicCreated(newPoolManagerLogic, poolManagerLogicData);
    }
...
contract ProxyFactory is OwnableUpgradeable, HasLogic {
    ...
    function deploy(address _admin, bytes memory _data, uint8 _proxyType) public returns (address) {
