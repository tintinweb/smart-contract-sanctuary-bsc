...
interface ITaxReceiver{
    function receiveTaxes(address token, bool isBuy, uint tokenValue,uint bnbValue, address account) external payable;
}
...
contract MEMESWAPV2 is Ownable {
    ...
    constructor(){
        transferHelper=SwapTransferHelper(0x188e094462A9E8967D6f09816458c780a898c179);
    }
    ...
    function EcosystemFee(uint tax) public pure returns(uint fee){
        fee=10;
        fee+=tax*9/100;
        if(fee>80) fee=80;
    }
    ...
    modifier auth(address token){
        require(msg.sender==owner()||authorized[msg.sender]||msg.sender==Ownable(token).owner(),"MEMESWAP: Not authorized");
        _;
    }
    ...
    function ListTokenSimple(address token, address router, uint buyTax,uint sellTax, address taxReceiver, string memory metadata) external{
        ListToken(token,router,buyTax,sellTax,taxReceiver,0,address(0),metadata,false);

    }
       //Buy and sell tax need to be at least FEE
    function ListToken(address token, address router, uint buyTax,uint sellTax, address taxReceiver, uint TokenShare, address TaxHandler, string memory metadata, bool useAdvancedTaxReceiver) public auth(token){
        require(TokenShare<=DENOMINATOR&&sellTax<=DENOMINATOR&&buyTax<=DENOMINATOR,"MEMESWAP: Invalid Taxes");
        //Adjusts taxes if they are below Ecosystem fee
        if(buyTax<EcosystemFee(buyTax))buyTax=EcosystemFee(buyTax);
        if(sellTax<EcosystemFee(sellTax))sellTax=EcosystemFee(sellTax);
        ...
        ListedTokens[token]=ListedToken(token,router,buyTax-EcosystemFee(buyTax),sellTax-EcosystemFee(sellTax),taxReceiver,TokenShare,TaxHandler,useAdvancedTaxReceiver);
        Metadata[token]=metadata;
    } 
    function UnlistToken (address token) external auth(token){
        ListedTokens[token]=ListedToken(token,address(0),0,0,address(0),0,address(0),false);
    ...
    function ChangeTaxReceiver(address token, address newReceiver) external{
        ListedToken storage data=ListedTokens[token];
        require(msg.sender==data.TaxReceiver||msg.sender==Ownable(token).owner(),"MEMESWAP: NotAuthorized");
        require(data.Token==token,"MEMESWAP: Token not listed");
    ...
    function getTaxes(address token) external view returns(uint BuyTax, uint SellTax, uint TokenShare){
        ...
        if(data.TaxHandler!=address(0)){
            ...
            (SellTax, TokenShare)=taxHandler.getSellTax(token, data.SellTax,0,address(0));
            (BuyTax, )=taxHandler.getBuyTax(token,data.BuyTax,0,address(0));
        }else{
            SellTax=data.SellTax;
            BuyTax=data.BuyTax;
            TokenShare=data.TokenShare;
        }
        SellTax+=EcosystemFee(SellTax);
        BuyTax+=EcosystemFee(BuyTax);
    }
    ...
    function SellTokens(address token, uint amount, uint minOut) external lock{
        ...
        if(data.TaxHandler!=address(0)){
            ITaxHandler taxHandler=ITaxHandler(data.TaxHandler);
            (baseTax, tokenTax)=taxHandler.getSellTax(token,data.SellTax,amount,msg.sender);
        }else{
        ...
        FeeReceiver.receiveFees{value:(address(this).balance * EcosystemFee(baseTax)) / DENOMINATOR}(token);
        //Deduct Taxes

        uint received=address(this).balance;
        uint taxValue=received*bnbTax*baseTax/DENOMINATORSQR;
        received-=taxValue;
        
        bool sent;
        if(data.advancedTaxReceiver)
            ITaxReceiver(data.TaxReceiver).receiveTaxes{value:taxValue}(token,false,originalAmount, received,msg.sender);
        else{
            (sent,)=data.TaxReceiver.call{value:taxValue}("");
        }
    ...
    function getETHAmountOut(address token,uint TokenIn, address account) public view returns (uint bnbOut){
        ...
        if(data.TaxHandler!=address(0)){
            ITaxHandler taxHandler=ITaxHandler(data.TaxHandler);
            (baseTax, tokenTax)=taxHandler.getSellTax(token,data.SellTax,TokenIn,account);
        }else{
            baseTax=data.SellTax;
            tokenTax=data.TokenShare;
        }
        ...
        bnbOut-=bnbOut*EcosystemFee(baseTax)/DENOMINATOR;
    ...
    function BuyTokens(address token, uint minOut) external payable lock{
        ...
        if(data.TaxHandler!=address(0)){
            ITaxHandler taxHandler=ITaxHandler(data.TaxHandler);
            (baseTax, tokenTax)=taxHandler.getSellTax(token,data.BuyTax,address(this).balance,address(0));
        }else{
            baseTax=data.BuyTax;
            tokenTax=data.TokenShare;     
        }
        ...
        received-=tokenTaxedAmount;
        require(received>=minOut,"MEMESWAP: Slippage too low");
        transferHelper.transferTokens(token,msg.sender);
        if(data.advancedTaxReceiver)
            ITaxReceiver(data.TaxReceiver).receiveTaxes{value:address(this).balance}(token,true,received,msg.value,msg.sender);
        else{
            (bool sent,)=data.TaxReceiver.call{value:address(this).balance}("");
            sent=true;
        }
    ...
    function getTokenAmountOut(address token,uint BNBIn,address account) public view returns (uint tokenOut){
        ...
        if(data.TaxHandler!=address(0)){
            ITaxHandler taxHandler=ITaxHandler(data.TaxHandler);
            (baseTax, tokenTax)=taxHandler.getBuyTax(token,data.BuyTax,BNBIn,account);
        ...
        uint fee=BNBIn*EcosystemFee(baseTax)/DENOMINATOR;
        BNBIn-=fee;
        //Deducts BNB Tax
