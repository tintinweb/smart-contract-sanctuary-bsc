...
contract DakShowIBCO is Ownable, KeeperCompatibleInterface {
	...
	event DepositTRX(address indexed account, uint256 amount);
	...
	function depositUSDT(uint256 amount) external checkTimeDeposit {
		...
		uint256 dakWithdraw = _getEstReceivedToken(userShare);

		require(
			70000000 * DECIMALS >=
				dakWithdraw + totalDakIBCO,
		...
		USDT.transferFrom(msg.sender, ColdWallet, amount);

		DakShow.transferFrom(owner(), msg.sender, dakWithdraw / 12);

		provided[msg.sender] += userShare;

		totalUSDT += amount;

		totalDAKWithDraw[msg.sender] += dakWithdraw;
	...
	function depositTRX(uint256 amount) external checkTimeDeposit {
		...
		TRX.transferFrom(msg.sender, ColdWallet, amount);
		...
		emit DepositTRX(msg.sender, amount);
	}
	...
	function depositUSDC(uint256 amount) external checkTimeDeposit {
		...
		USDC.transferFrom(msg.sender, ColdWallet, amount);
	...
	function depositBTC(uint256 amount) external checkTimeDeposit {
		...
		BTC.transferFrom(msg.sender, ColdWallet, amount);
	...
	function depositETH(uint256 amount) external checkTimeDeposit {
		...
		ETH.transferFrom(msg.sender, ColdWallet, amount);
		...
		returns (uint256)
	{
		uint256 params = (1335 * (_userShare + totalProvided)) /
			...
			225625000000;
		return
			((20000000 * sqrt(params)) / 8010 - 1186017480) *
			10**16 -
			totalDakIBCO;
	}
	...
	function getEstTokenPrice() public view returns (uint256) {
		return 2403 * (totalDakIBCO/DECIMALS) * 10**5 + (57 * DECIMALS) / 20000;
	}
	...
	function buyRemainingQuantity() external {
		...
		require(
			70000000 * DECIMALS >= currentDakRemaining,
			"Insufficient DakShow token in contract"
		);
		...
		BUSD.transferFrom(msg.sender, ColdWallet, amount);

		DakShow.transferFrom(owner(), msg.sender, currentDakRemaining / 12);
