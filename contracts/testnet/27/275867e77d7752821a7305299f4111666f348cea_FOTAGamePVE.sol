...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  event LandLordUpdated(uint id, address landLord, uint timestamp);
  ...
  event GameFinished(address indexed user, uint mission, uint userReward, uint currentEnergy, bool dailyQuestCompleted, uint dayPassed);
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    ...
    emit GameFinished(msg.sender, _mission, userReward, energy, dailyQuestCompleted, dayPassed);
  }
  ...
  function updateLandLord(uint _id, address _landLord) external {
    require(msg.sender == address(landLordManager) || _isMainAdmin(), "PVE: only landLord manager");
    missions[_id].landLord = _landLord;
    landLordManager.syncLandLord(_id);
    emit LandLordUpdated(_id, _landLord, block.timestamp);
  }
