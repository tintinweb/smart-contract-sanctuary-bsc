...
contract ElvantisMysteryBox is Ownable, ERC721Holder, ERC1155Holder {
    ...
    function addRarity(uint8 _weight, uint256[] memory _tokenIds)
        external
        onlyOwner
    {
        require(_tokenIds.length > 0, "ElvantisMysteryBox: TokenIds must not be empty");
        totalWeight += _weight;
        require(totalWeight <= MAX_WEIGHT, "ElvantisMysteryBox: Weight Limit exceeds");
    ...
    function updateRarity(uint256 rarityId) external onlyOwner {
    ...
    function updateRaritiesList(uint256 rarityId, uint256 tokenIdIndex, uint256 tokenId) internal {
    ...
    //Buy pack
    function buyPack(uint256 quantity) external payable {
        require(enabled, "ElvantisMysteryBox: Disabled!");
        require(rarities.length > 0, "ElvantisMysteryBox: Rarities length is zero");
    ...
    function takeFee(uint256 quantity) internal {
        uint256 _devFee = (price * quantity * developmentFee) / FEE_DENOMINATOR;
        uint256 _burnFee = (price * quantity * burnFee) / FEE_DENOMINATOR;

        IERC20(paymentToken).safeTransferFrom(msg.sender, DEAD_ADDRESS, _burnFee);
        IERC20(paymentToken).safeTransferFrom(msg.sender, feeRecipient, _devFee);
    }
