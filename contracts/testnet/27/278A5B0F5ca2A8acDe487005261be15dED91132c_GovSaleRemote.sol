...
interface ICrossChainManagerRemote {    
	function buyToken(bytes calldata, bytes calldata, uint256) external;
...
interface IGovSaleRemoteFactory{
    ...
    function crossChainManagerRemote() external view returns (address);
...
import "../layerZero/interfaces/ILayerZeroEndpoint.sol";
import "../interfaces/ICrossChainManagerRemote.sol";
...
contract GovSaleRemote is
    ...
    function init(
        ...
        endpoint = ICrossChainManagerRemote(factory.crossChainManagerRemote()).lzEndpoint();
    ...
    function estimateFees(
        ...
        (fees,) = endpoint.estimateFees(dstChainId, factory.crossChainManagerRemote(), getPayload(_amountIn, _buyer), false, getAdapterParams());
    ...
    function buyToken(uint128 _amountIn) external payable whenNotPaused nonReentrant {
        ...
        require(!isOutOfSale[sender][running] && running > 0 && feeIn >= feeNeeded, "bad");
        ...
        raised += _amountIn;
        revenue += feeCharged;
        purchasePerRound[sender][running] += tokenReceivedEstimate;
        booster[running].tokenAchieved += tokenReceivedEstimate;

        ICrossChainManagerRemote(factory.crossChainManagerRemote()).buyToken(getPayload(_amountIn, sender), getAdapterParams(), tax);
    }

    mapping(address => uint128) public purchase;
    mapping(address => uint128) public feeGiven;

    function receiveCall(bytes calldata _payload) external {
        require(_msgSender() == factory.crossChainManagerRemote(), "unknown");

        (
            , uint128 remoteRaised,
            uint128 remoteRevenue,
            address[] memory remoteUsers,
            uint128[] memory remoteUsersAmount,
            uint128[] memory remoteUsersFee
        ) = abi.decode(_payload, (address, uint128, uint128, address[], uint128[], uint128[]));
        ...
        for(uint16 i = 0; i < remoteUsers.length; ++i){
            ...
            uint128 oldPurchase = purchase[buyer];
            if(oldPurchase > remoteUsersAmount[i]){
                uint128 payback = ((oldPurchase - remoteUsersAmount[i]) * price) + (feeGiven[buyer] - remoteUsersFee[i]);
                purchase[buyer] = remoteUsersAmount[i];
                feeGiven[buyer] = remoteUsersFee[i];
