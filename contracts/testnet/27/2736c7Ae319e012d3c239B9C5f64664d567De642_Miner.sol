...
contract Miner is IMiner, TokenWithdrawable {
    ...
    function deposit(address refAddress) external override payable {
        ...
        _distributeRefBonus(msg.sender, msg.value);
    ...
    function compoundReward() public override {
        uint reward = calculateReward(msg.sender);

        depositedBalances[msg.sender] += reward;
        lastUpdatedTimes[msg.sender] = block.timestamp;
    ...
    function collectTreasury(address to) external onlyOwner {
        (bool isSuccess,) = to.call{ value: treasury }("");
        require(isSuccess);
    }
...
interface IMiner {
    event Deposited(address account, uint value);
