...
contract dgtlzStaking is IdgtlzStaking, Ownable {
  ...
  mapping(address => mapping(uint256 => uint256)) private _rewardAmount;
  mapping(address => mapping(uint256 => uint256)) private withdrawAmountForStaker;
  mapping(address => mapping(uint256 => uint256)) private rewardedAmount;
  mapping(address => mapping(uint256 => uint256)) private lockDuration; 
  ...
  mapping(address => mapping(uint256 => uint256)) private plan;
  uint private _withdrawAmount;
  uint private totalStakeAmount;
  uint private totalRewardAmount;
  uint private totalAmount_;
  uint256 private unStakableAmount;
  uint public lockPeriod;
  uint256 private _6month = 180 * 1 days;
  uint256 private _1year = 365 * 1 days;
  uint public claimPeriod;
  uint private penaltyAmount;
  ...
  function staking(uint amount, uint duration) external {
    ...
    if (duration == 1) {
      ...
      if (amount > 2000 * decimals && amount < 5000 * decimals) {
  ...
  function withdrawBenefit() external onlyOwner {
    ...
    if(totalAmount_ > totalStakeAmount + totalRewardAmount) {
      ...
      uint airdropAmount = (_deltaAmount * 40) / 100;
  ...
  function _calculationReward(uint pastTime, uint256 amount, uint256 interestPercent) internal view returns(uint256) {
    return pastTime * amount  * interestPercent / 100 / lockPeriod;
  }
  ...
  function claim(uint256 duration) external {
    ...
    interestPeriod[msg.sender][duration] = block.timestamp + claimPeriod;
    uint256 pastTime = block.timestamp - claimDate[msg.sender][duration];
    claimDate[msg.sender][duration] = block.timestamp;
    _rewardAmount[msg.sender][duration] = _calculationReward(pastTime, stakeAmount[msg.sender][duration], interestRate[msg.sender][duration]);
  ...
  function unStake(uint256 duration) external {
    ...
    if(lockDuration[msg.sender][duration] > block.timestamp) {
      if (plan[msg.sender][duration] == 1) {
        ...
        uint256 getPenalty = getPercent(stakeAmount[msg.sender][duration], lockDuration[msg.sender][duration]);
