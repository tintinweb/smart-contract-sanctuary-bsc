...
contract GLMNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    function setAdminList(address[] memory _list) public nonReentrant onlyOwner {
        require(_list.length > 0, "admin list is zero");

        for (uint256 nIndex = 0; nIndex < _list.length; nIndex++) {
            require(_list[nIndex] != address(0), "admin is zero address");
        }
        adminList = _list;
    }
    ...
    modifier onlyAdmin() {
        require(onlyAdminCheck(msg.sender) == true, "only admin can operate");
    ...
    function mintItem(address recipient) public onlyAdmin returns (uint256) {
    ...
    function mintItems(address recipient, uint256 numberOfTokens) external onlyAdmin {
        require(recipient != address(0), "recipient is zero address");
    ...
    function mintMulti(address[] memory recipient) public onlyAdmin {
    ...
    function allotProfits(address token, uint256 amount) external onlyAdmin nonReentrant {
    ...
    function allotProfitsByNumber(address token, uint256 amount, uint256 start, uint256 num) external onlyAdmin nonReentrant {
        require(token != address(0), "token is zero address");
        require(amount > 0, "amount cannot be 0");
