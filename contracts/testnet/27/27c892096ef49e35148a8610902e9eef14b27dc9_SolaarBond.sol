...
contract SolaarBond is ERC721, Ownable {

    modifier onlyBond() {
        require( approvedBonds[msg.sender], "Not an approved bond" );
        _;
    }
    ...
    uint private _bondsOwnershipLimit = 10; // how many bonds can be owned by a single wallet
    mapping( address => bool ) public approvedBonds; // only bond contracts can mint
    mapping( uint => bool ) public bondActive; // only inactive bonds are transferrable
    mapping( uint => address ) public bondMinter; // bond that minted the NFT

    string private baseURI;

    constructor( string memory _baseURI) ERC721('Solaar Bond', 'SOLAAR:BOND') {
        baseURI = _baseURI;
    }
    ...
    function bondsOwnershipLimit() public view returns (uint256) {
        return _bondsOwnershipLimit;
    }
    ...
    function _transfer(
        ...
        require(!bondActive[tokenId], "Active bonds cannot be transferred");
        require(balanceOf(to) <= bondsOwnershipLimit(), "Bonds ownership limit reached");
    }
