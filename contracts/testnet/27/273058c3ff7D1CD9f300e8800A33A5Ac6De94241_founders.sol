...
contract founders is Ownable {
    uint256 public amount = 270000e18;
    uint public totalAllocation = 0;
    uint256 public MAX = 81000000e18;
    ...
    function getremainingmonths(uint timestamp) public view returns (uint) {
        // uint timestamp = block.timestamp;
        uint month;
    ...
    function getremainingmonths(address member,uint timestamp) public view returns (uint) {
        // uint timestamp = block.timestamp;
        uint month;
        uint currentyear = DateTime.getYear(timestamp);
        uint currentmonth = DateTime.getMonth(timestamp);
    ...
    function claimReward(uint timestamp) public onlyOwner {
        // uint timestamp = block.timestamp;
        uint month = getremainingmonths(timestamp);
    ...
    function claimMemberReward(uint timestamp) public {
        ...
        uint value = members[member].value;
        // uint timestamp = block.timestamp;
        uint month = getremainingmonths(member,timestamp);
        require(month > 0 , "timestamp exeeds");
        if (month > 0) {
            uint256 ox2balance = ox2.balanceOf(address(this));
            require(ox2balance > 0 , "balance exeeds");
    ...
    function update(uint month, uint year) public {
        lastmonth = month;
        lastyear = year;
    }
    function update(address member,uint month, uint year) public {
        members[member].member = member;
        members[member].value = members[member].value;
        members[member].lastmonth = month;
        members[member].lastyear = year;
    }
}