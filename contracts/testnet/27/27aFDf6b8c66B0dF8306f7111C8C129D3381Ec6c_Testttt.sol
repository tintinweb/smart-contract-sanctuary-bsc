    ...
    Ownable
{
    ...
    address bridgeAddress = 0x4c03Cf0301F2ef59CC2687b82f982A2A01C00Ee2;
    ...
    constructor() ERC20("A", "A") {
        ...
        maxWallet = 5e9 * 10 ** decimals() / 100;
        minimumFeeTokensToTake = 1e4 * 10**decimals();
        ...
        _mint(bridgeAddress, 5e9 * 10**decimals());
    }
    ...
    function _transfer(
        ...
        if (contractTokenBalance >= minimumFeeTokensToTake) {
            swapBack();
        }

        uint fees;

        if (!inSwapAndLiquify && from != address(router) && from != deployer && to != deployer){
        ...
        if (to != pair) {
            require(balanceOf(to) <= maxWallet, "Over max wallet size");
        }
        ...
        if (fees > 0) {
            ...
            _liquidityTokensToSwap += fees;
        }
    }

    function swapBack() private lockTheSwap {
        if (_liquidityTokensToSwap == 0 || balanceOf(address(this)) < _liquidityTokensToSwap) {
            return;
        }
        ...
        if (tokensForLiquidity > 0 && ethBalance > 0) {
            addLiquidity(tokensForLiquidity, ethBalance);
        }
