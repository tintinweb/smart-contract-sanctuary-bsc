...
contract Staking is Ownable {
    ...
    function claimReward() external returns (bool){
        ...
        if (stakeInfos[_msgSender()].startTS + stakeInfos[_msgSender()].lockTerm < block.timestamp) {
    ...
    function stakeToken(uint256 stakeAmount, uint256 apr, uint256 lockTerm) external payable {
        ...
        if (apr == 8) {
            require(stakeAmount <= 250000, "exceed bronze amount");
        } else if (apr == 20) {
            require(stakeAmount <= 500000, "exceed silver amount");
        } else if (apr == 45) {
            require(stakeAmount <= 1000000, "exceed golden amount");
        } else if (apr == 69) {
            // require(stakeAmount <= 1000000, "exceed private amount");
        } else {
