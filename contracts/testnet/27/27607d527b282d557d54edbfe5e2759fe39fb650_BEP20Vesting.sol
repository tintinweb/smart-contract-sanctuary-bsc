...
contract BEP20Vesting is Ownable {
    ...
    function setParameter(uint256 _lockDuration,
        uint256 _duration,
        uint256 _upfrontAmount,
        uint256 _interval) public onlyOwner {
    ...
    function addBeneficiaryBatch(
        address[] calldata _beneficiary, uint256[] calldata _amount
    ) public onlyOwner {
        require(_beneficiary.length == _amount.length, "array's length isn't equal");
        for(uint256 i=0; i<_beneficiary.length;i++){
            addBeneficiary(_beneficiary[i],_amount[i],lockDuration,duration,upfrontAmount,interval);
        }
    }

    function addBeneficiarySimply(
        ...
        addBeneficiary(_beneficiary,_amount,lockDuration,duration,upfrontAmount,interval);
    }
    /**
    ...
    returns (uint256, uint256)
    {
        ...
        if (
            ...
            block.timestamp < info.interval.add(info.lastReleasedAt)
        ) {
            return (info.released, info.lastReleasedAt);
        }
     ...
     */
    function releaseBeneficiaryTokens(uint256 start, uint256 end) public onlyOwner {
