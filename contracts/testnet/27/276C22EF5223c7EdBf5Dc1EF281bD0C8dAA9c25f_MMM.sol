...
contract MMM is Ownable{
    ...
    uint64[5] private packagePrice = [100e6, 500e6, 1000e6, 2000e6, 3000e6]; 
    ...
    function deposit(uint256 _packageID) external {
        ...
        if(order.start != 0)
            user.withdrawable += uint256(packagePrice[_packageID]) + ((uint256(packagePrice[_packageID]) * freezeMultiply * uint256(packageIncomePecent[_packageID])) / baseDivider);
