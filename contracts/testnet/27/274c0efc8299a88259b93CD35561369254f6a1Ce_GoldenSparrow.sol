...
Buy Fee 2% Marketing, 1% Auto Burn, 2% Reflection
Sell Fee 2% Marketing, 1% Auto Burn, 4% Reflection
...
contract GoldenSparrow is Context, IERC20, Ownable {  
    ...
    uint256 public _ReflectionSellPer = 4;
    ...
    function _takeMarketingFee(uint256 tMarketing,uint TaxType) private {
        ...
        if(TaxType==1){
            ...
            ReflectionShare=tMarketing.mul(40).div(100);
            BurningShare=tMarketing.mul(20).div(100);
        }
        else if(TaxType==2){
            MarketingShare=tMarketing.mul(28).div(100);
            ReflectionShare=tMarketing.mul(58).div(100);
            BurningShare=tMarketing.mul(14).div(100);
        }
        uint256 rMarketing = MarketingShare;
    ...
    function _reflection() private {
      ...
      for(uint8 i = 0; i < tokenHolder.length; i++) {
         uint256 _tokenHolderShare = (balanceOf(tokenHolder[i]).mul(_totalReflection)).div(totalSupply());
