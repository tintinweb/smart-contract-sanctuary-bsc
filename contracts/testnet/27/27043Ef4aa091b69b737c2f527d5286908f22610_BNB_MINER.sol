...
contract BNB_MINER {
    ...
    /** base parameters **/
    uint256 public EGGS_TO_HIRE_1MINERS = 864000; // 10% ROI Daily ~ 3650% APY  
    uint256 public REFERRAL = 80; // 8% Referral bonus
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public _Dev = 30;
    uint256 public _Treasury = 20;
    ...
    uint256 public WITHDRAW_DEV = 50;
    uint256 public WITHDRAW_TREASURY = 200;
    ...
    struct User {
        ...
        uint256 minerCompoundCount; //added to monitor miner consecutive compound without cap
        uint256 lastWithdrawTime;
    }

    mapping(address => User) public users;

    constructor(address payable _dev, address payable _treasury) {
		require(!isContract(_dev) && !isContract(_treasury));
        owner = msg.sender;
        devWallet = _dev;
    ...
    function startMining(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == owner) {
    ...
    function ReSupplyFleet(bool isCompound) public {
    ...
    function sellCargo() public{
        ...
        if(user.dailyCompoundBonus < COMPOUND_FOR_NO_TAX_WITHDRAWAL){
            ...
            uint256 devWithdraw = eggValue.mul(WITHDRAW_DEV).div(PERCENTS_DIVIDER);
            uint256 treasuryWithdraw = eggValue.mul(WITHDRAW_TREASURY).div(PERCENTS_DIVIDER);
            uint256 earlyWithdraw = eggValue.mul(WITHDRAWAL_TAX).div(PERCENTS_DIVIDER);

            devWallet.transfer(devWithdraw);
            treasuryWallet.transfer(treasuryWithdraw);

            eggValue = eggValue.sub(devWithdraw).sub(treasuryWithdraw).sub(earlyWithdraw);
        }else{
    ...
    function LaunchFleet(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        uint256 dev = eggValue.mul(_Dev).div(PERCENTS_DIVIDER);
        uint256 treasury = eggValue.mul(_Treasury).div(PERCENTS_DIVIDER);
        devWallet.transfer(dev);
        treasuryWallet.transfer(treasury);
        return treasury.add(dev);
    }
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _miners,
	 ...
	 uint256 _totalWithdrawn, uint256 _referralEggRewards, uint256 _dailyCompoundBonus, uint256 _minerCompoundCount, uint256 _lastWithdrawTime) {
    ...
    function PRC_DEV_EARLY_WITHDRAW(uint256 value) external{
       require(msg.sender == owner, "Admin use only.");
        require(value <= 90);
        WITHDRAW_DEV = value; 
    }

    function PRC_TREASURY_EARLY_WITHDRAW(uint256 value) external{
       require(msg.sender == owner, "Admin use only.");
        require(value <= 90);
        WITHDRAW_TREASURY = value; 
    }

    function PRC_TREASURY(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        require(value <= 30);
        _Treasury = value;
    }
