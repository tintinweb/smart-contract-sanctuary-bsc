...
interface IStaking {
     ...
     * if rewards to claim
     */
    function withdrawForce(uint amount) external;
...
contract Staking is IStaking, Ownable {
    ...
    uint8 public fixedAPY; //immutable
    ...
    uint public forceWithdrawlFee = 50;
    uint public veryEarlyWithdrawlFee = 25;
    uint private veryEarlyWithdrawlDays = 14 days;
    uint public littleEarlyWithdrawlFee = 15;
    uint private littleEarlyWithdrawlDays = 28 days;
    uint public normalWithdrawlFee = 10;
    ...
    mapping(address => uint) private _userlockupEndTime;
    mapping(address => uint) private _userStakingEndtime;


    address [] private stakeHolders;
    mapping(address => bool) public existingStakeHolders;
    ...
    function updateStakingDetails(uint8 apy, uint8 _lockDurationInDays,uint8 _durationInDays, uint8 _veryEarlyFeeDays, uint8 _veryEarlyFee,uint8 _littleEarlyFeeDays, uint8 _littleEarlyFee, uint8 _normalFee ) external onlyOwner returns(bool) {
        require(_veryEarlyFeeDays > 0);
        require(_veryEarlyFeeDays < _littleEarlyFeeDays);
        require(_durationInDays > 0);
        require(_littleEarlyFeeDays > 0);
        require(_veryEarlyFee >0);
        require(_littleEarlyFee > 0);
        require(_normalFee > 0);
        fixedAPY = apy;
        lockupDuration = _lockDurationInDays * 1 days;
        endPeriod = block.timestamp + (_durationInDays * 1 days);
        veryEarlyWithdrawlDays = _veryEarlyFeeDays * 1 days;
        littleEarlyWithdrawlDays = _littleEarlyFeeDays * 1 days;
        veryEarlyWithdrawlFee = _veryEarlyFee;
        littleEarlyWithdrawlFee = _littleEarlyFee;
        normalWithdrawlFee = _normalFee;
        return true;
    }
    function veryEarlyDays() external view returns(uint256)
    {
        return veryEarlyWithdrawlDays/86400;
    }
    function littleEarlyDays() external view returns(uint256)
    {
        return littleEarlyWithdrawlDays / 86400;
    }
    function lockupDays() external view returns(uint256)
    {
        return lockupDuration / 86400;
    }
    ...
    function deposit(uint amount) external override {
        ...
        if(existingStakeHolders[_msgSender()] == false) {
            stakeHolders.push(_msgSender());
            existingStakeHolders[_msgSender()] = true;
        }
        ...
        _userlockupEndTime[_msgSender()] = block.timestamp + lockupDuration;
        _userStakingEndtime[_msgSender()] = block.timestamp + stakingDuration;
    ...
    function withdraw(uint amount) external override {
        require(
            block.timestamp >= _userlockupEndTime[_msgSender()],
        ...
        token.safeTransfer(address(this), fee);
     ...
     * if rewards to claim
     */
    function withdrawForce(uint amount) external override {
        require(amount > 0, "Amount must be greater than 0");
        require(
            amount <= staked[_msgSender()],
            "Amount higher than stakedAmount"
        ...
        _totalStaked -= amount;
        staked[_msgSender()] -= amount;
        uint fee = amount * forceWithdrawlFee/100;
        token.safeTransfer(_msgSender(), amount - fee);
        token.safeTransfer(address(this), fee);
    ...
    function calculateFee(uint amount) private view returns(uint) {
        if(_userStartTime[_msgSender()] + veryEarlyWithdrawlDays <= block.timestamp) {
            return amount * veryEarlyWithdrawlFee /100;
        } else if(_userStartTime[_msgSender()] + littleEarlyWithdrawlDays <= block.timestamp) {
            return amount * littleEarlyWithdrawlFee/100;
        } else {
            return amount * normalWithdrawlFee/100;
        }
    }
    ...
    function withdrawAll() external override {
        ...
        _userlockupEndTime[_msgSender()] = 0;
        _totalStaked -= staked[_msgSender()];
        uint stakedBalance = staked[_msgSender()];
        staked[_msgSender()] = 0;
        uint fee = calculateFee(stakedBalance);
        token.safeTransfer(_msgSender(), stakedBalance - fee);
        token.safeTransfer(address(this),fee);
    ...
    function withdrawResidualBalance() external onlyOwner {
        require(
            ...
            "Withdraw 1 year after endPeriod"
        );
    ...
    /**
     * @notice Update rewards for all stakeholders before updating APY
     * @dev THIS API call will be costly as it will loop trough all the stakeholders
     * to update rewards based on previous APY.
     */
    function setAPYAfterRewardsUpdate(uint8 apy) external onlyOwner {
        for (uint i=0; i<stakeHolders.length; i++) {
            if(staked[stakeHolders[i]] > 0) {
              _rewardsToClaim[stakeHolders[i]] =  _calculateRewards(stakeHolders[i]);
            }
        }
        fixedAPY = apy;
    }
    ...
    function userLockupPeriod(address adr) external view returns(uint) {
        return _userlockupEndTime[adr];
    }
}