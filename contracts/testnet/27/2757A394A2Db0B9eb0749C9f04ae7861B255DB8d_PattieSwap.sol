...
contract PattieSwap is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private whiteListed;   // No fee for this users
    mapping (address => bool) private blackListed;   // Not able to send token

    mapping (address => bool) private _isExcluded;   // excluded from reflection reward
    ...
    function burn(uint256 tAmount) external onlyOwner returns(bool) {
        uint256 currentRate =  _getRate();
