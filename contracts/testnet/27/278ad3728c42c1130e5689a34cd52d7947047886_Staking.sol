...
contract Staking is Ownable {
    ...
    constructor(address _rewardToken, address _nativeToken, uint256 startBlock , uint256 _harvestInterval, uint256 _withdrawInterval, uint256 _withdrawFee, uint256 _rewardPerBlock, address _serviceFeeReciever, uint256 _serviceFee) public payable {
        require(address(rewardToken) == address(0) && address(pool.stake) == address(0), 'Tokens already set!');
        ...
        payable(_serviceFeeReciever).transfer(_serviceFee);

    }
    
    // function startPool(address _rewardToken, address _nativeToken, uint256 startBlock , uint256 _harvestInterval, uint256 _withdrawInterval) external payable{
    //     require(address(rewardToken) == address(0) && address(pool.stake) == address(0), 'Tokens already set!');
    //     require(pool.lastRewardBlock == 0, 'Pool already started');
    //     // require(address(rewardToken) != address(0) , "Reward Token not set");
    //     rewardToken = IERC20(_rewardToken);
    ...
    //     payable(owner()).transfer(poolFees);
    ...
    function withdraw(uint256 amount) external {
        ...
        if (amount > 0) {
            ...
            if(canWithdraw(msg.sender)) {
                ...
                uint256 fee = amount.mul(withdrawFee).div(10000);
                if (fee > 0) {
                    
                    pool.stake.safeTransfer(owner(), fee);
                    uint256 amountMinusFee = amount.sub(fee);
                   
                    pool.stake.safeTransfer(address(msg.sender), amountMinusFee);
...
//             pool.stake.safeTransfer(address(msg.sender), amountToTransfer);
...
//   }
    ...
    function updateWithdrawFee(uint256 _withdrawFee) external onlyOwner {
        withdrawFee = _withdrawFee;
    }
