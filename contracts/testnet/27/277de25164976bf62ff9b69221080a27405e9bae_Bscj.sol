...
struct Player {
  ...
  Deposit[] deposits;
  uint256[5] structure; //[CAN_SET_MAX_REF_LEVELS]
}
contract Bscj {
    ...
    constructor(address payable devAddr, address payable markAddr) {
    ...
    function launch() external {
        ...
        require(!isLaunched, "Contract already launched");

        isLaunched = true;
    }
    function setContractStartParameters(
        address _tokenCAddr,
        ...
        require(!isLaunched, "cannot after launch");
        require(msg.sender == contractOwner && !isContractParamsSet, "cannot");
        ...
        require(BONUS_LINES_COUNT <= CAN_SET_MAX_REF_LEVELS, "Cannot set more than 5 levels");
    ...
    function _deposit(address _investor, uint8 _tarif, address _upline, uint256 amount) private {
        ...
        require(_tarif >= TARIF_MIN_DAYS && _tarif <= TARIF_MAX_DAYS, "Invalid tarif");
        require(amount > 0, "Deposit amount cannot be 0");
        require(MINIMUM_DEPOSIT == 0 || amount >= MINIMUM_DEPOSIT, "Must deposit more than minimum amount");
        require(MAXIMUM_DEPOSIT == 0 || amount <= MAXIMUM_DEPOSIT, "Must deposit less than maximum amount");
    ...
    function withdraw() external {
        ...
        if (useNativeCoin) {
		    ...
		    contractBalance = tokenCAddress.balanceOf(address(this));
        }
    ...
    function _setUpline(address _addr, address _upline) private {
        //give ref to existing investors only
        if (players[_upline].deposits.length == 0) { 
            players[_addr].upline = address(0);
        } else {
