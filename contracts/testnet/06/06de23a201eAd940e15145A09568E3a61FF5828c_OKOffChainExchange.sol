...
import "./OffChainExchangeCore.sol";
...
interface BatchTransferNFTManager{

    function proxy(
        address dest,
        uint256 howToCall,
        bytes memory calldataValue
    ) public returns (bool result,bytes memory returnData);
}
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    */
    ...
    address public batchMerkleValidator;
    ...
    function setupBatchMerkleValidator(address _batchMerkleValidator)
    public
    onlyOwner
    {
        batchMerkleValidator = _batchMerkleValidator;
    }
    ...
    function setupWETH(address _WETH)
    public
    onlyOwner
    {
        WETH = _WETH;
    ...
    address public  batchNFTTransferManagerAddr;
    function setupBatchNFTTransferAddr(address _batchNFTTransferManagerAddr) public
    onlyOwner
    {
        batchNFTTransferManagerAddr = _batchNFTTransferManagerAddr;
    }
    ...
    function atomicMatch(
        ...
        /* EFFECTS */
        if (batchMerkleValidator != sell.target){
        ...
        if(batchMerkleValidator == sell.target&&sell.paymentToken==WETH){
        ...
        if (delegateProxy != address(0)) {
            ...
            if (batchMerkleValidator != sell.target){
                ...
                //检查
                (tempData.callResult,tempData.callResultData) = BatchTransferNFTManager(batchNFTTransferManagerAddr).proxy( sell.target,
                    uint256(sell.howToCall),
                    sell.calldata);


                tempData.nftOwner = MyTools.bytesToAddress(tempData.callResultData);
