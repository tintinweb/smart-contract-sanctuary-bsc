...
contract TokieNFTMintProxy is Governance, IGegoRuleProxy{
    ...
    ITokieNFTFactory public _factory = ITokieNFTFactory(address(0));
        ...
        onlyGovernance
    {
     ...
     function setFactory( address factory )
        ...
        onlyGovernance
     {
        _factory = ITokieNFTFactory(factory);
     }

    function cost( MintParams calldata params) external override returns (  uint256 mintAmount,address mintErc20 ){
        require(_factory == ITokieNFTFactory(msg.sender)," invalid factory caller");
       (mintAmount,mintErc20) = _cost(params);
    }

    function destroy(  address owner, ITokieNFT.Gego calldata gego) external override {
        require(_factory == ITokieNFTFactory(msg.sender)," invalid factory caller");
    ...
    function generate( address user , uint256 ruleId, uint256 randomNonce ) external override view returns (  ITokieNFT.Gego memory gego ){
        require(_factory == ITokieNFTFactory(msg.sender), " invalid factory caller");
