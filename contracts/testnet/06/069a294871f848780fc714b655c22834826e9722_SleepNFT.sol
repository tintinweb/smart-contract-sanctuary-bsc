    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setIsPublicMintEnabled(bool _isEnabled) external onlyOwner {
    ...
    function mint(string[] calldata _tokenURIs, uint256 _quantity)
        external
        payable
        nonReentrant
    {
        ...
        require(
            msg.value == _quantity.mul(mintPrice),
        ...
        require(
            mints[msg.sender].add(_quantity) <= maxPerWallet,
            "Mint quantity will exceed max per wallet"
        );

        for (uint256 i; i < _quantity; i++) {
            uint256 newTokenID = totalSupply().add(1);
            mints[msg.sender] = mints[msg.sender].add(1);
    ...
    function teamPreMint(string[] calldata _tokenURIs)
        external
        onlyOwner
        teamPreMinting
    {
        ...
        require(
            totalSupply().add(teamMintQuantity) <= maxSupply,
            "Cannot fulfil mint quantity"
        );
        require(
