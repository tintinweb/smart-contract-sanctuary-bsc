...
contract DepositBUSD is Ownable {
    ...
    // function depositTest(uint256 _tokens) public returns(bool success) {
    //     require(_tokens > 0);
    //     //Approve before calling this
    //     require(busd.allowance(msg.sender, address(this)) >= _tokens);
    //     // msg.sender.delegatecall(
    //     //     bytes4(
    //     //         keccak256(
    //     //             // strConcat(
    //     //                 addressToString(busd),
    //     //                 ".transferFrom(address msg.sender, address address(this), uint256 _value)"
    //     //             // )
    //     //         )
    //     //     ),
    //     //     _tokens
    //     // );
    //     // require(busd.transferFrom(msg.sender, address(this), _tokens));
    ...
    function transferTo(address addr, uint256 amount)
        public
    ...
    function WithdrawBUSDUsers(uint256 amount) public returns (bool) {
        ...
        idToListTransaction[_idTransaction].from = msg.sender;
        idToListTransaction[_idTransaction].value = amount;
        idToListTransaction[_idTransaction].name = "WithdrawBUSDs";
    ...
    function DepositBNB() public payable returns (bool) {
        ...
        require(_value >= 0, "Deposit False");
        return true;
    }
