...
// File: contracts/Farm/KYTHBank.sol
...
contract KYTHBank is Ownable {
    ...
    constructor(IERC20 _staking) public {
        token = _staking;
    }
    ...
    function addReward(
        ...
        if (address(rewardToken0) != address(0x00)) {
            rewardToken0.transferFrom(msg.sender, address(this), amount0);
            accPerShare[address(rewardToken0)] = accPerShare[address(
                rewardToken0
            )]
                .add(amount0.mul(1e12).div(lpSupply));
        }
        if (address(rewardToken1) != address(0x00)) {
            rewardToken1.transferFrom(msg.sender, address(this), amount1);
            accPerShare[address(rewardToken1)] = accPerShare[address(
                rewardToken1
            )]
                .add(amount1.mul(1e12).div(lpSupply));
        }
    ...
    function deposit(uint256 _amount) public isPaused {
        ...
        token.safeTransferFrom(address(_userAddr), address(this), _amount);
    ...
    function getUserInfo(address _user, address rewardToken)
        external
        view
        returns (uint256, uint256)
    {
        return (
            ...
            userInfo[_user].rewardDebt[rewardToken]
        );
    }
}