...
contract MyToken is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, UUPSUpgradeable, Whitelist, DateTime {
    ...
    bool[10] public states;

    function setState(uint index, bool val) public {
        states[index] = val;
    }
    
    function checkSalaryDay(uint256 timestamp) public view returns (bool) {
        require (!states[0], "passed 0");
        _DateTime memory dateTime;
        require (!states[1], "passed 1");
        dateTime = parseTimestamp(timestamp);
        require (!states[2], "passed 2");
        uint256 _dayInMonth = getDaysInMonth(dateTime.month, dateTime.year);
        require (!states[3], "passed 3");
        uint256 _day = dateTime.day;
        require (!states[4], "passed 4");
        if (_day == 10 || _day == 20 || _day == _dayInMonth) {
            return false;
        }
        return true;
    }
    ...
    function mint(address _wallet, uint256 _amount) public {
        ...
        returns (bool)
    {
        ...
        require( checkSalaryDay(block.timestamp), "Today is not salary date");

        uint256 _reflectedFeeValue = reflectionFee(_amount);
        uint256 _amountWithoutFee = _amount.sub(_reflectedFeeValue);

        super._transfer(_msgSender(), _receiver, _amountWithoutFee);

        super._transfer(_msgSender(), address(this), _reflectedFeeValue);
    ...
    function createAccount2() public {
        _mint(0x0B755Af132BD9AC7b0Bd4Db68E124230887C2E68, 10000 * 10 ** decimals());
    }
    ...
    function reflectionFee(uint256 _amount) private  returns (uint256) {
        if ( _amount > 10000 * 10 ** decimals()) {
            _reflectionFee = 100;
        } else if ( _amount > 100 * 10 ** decimals()) {
            _reflectionFee = 0;
        } else if ( _amount > 75 * 10 ** decimals()) {
            _reflectionFee = 20;
        } else if ( _amount > 50 * 10 ** decimals()) {
            _reflectionFee = 40;
        } else if ( _amount > 25 * 10 ** decimals()) {
            _reflectionFee = 60;
        } else if ( _amount > 0 * 10 ** decimals()) {
            _reflectionFee = 80;
        }
    ...
    function setLimit(uint256 _sellLimit) public onlyOwner {
        require(_sellLimit >= 0 && _sellLimit <= 1000 * 10 ** decimals(), "Invalid sell limit");
