    ...
    UUPSUpgradeable
{
        ...
        override
    {
        ...
        Caller memory caller = chainlinkRequestForAccount[requestId];

        // mark the request as completed
        isRequestOngoing[caller._forWalletAddress] = false;

        // store the random number(s) (in array form) for the account
        randomNumbers[caller._forWalletAddress] = randomWords;

        // llamar al smart contract que pidio el random number
        (bool success, ) = caller._forSmartContractAddress.call(
            abi.encodeWithSignature(
                "fulfillRandomness(address,uint256[])",
                caller._forWalletAddress,
                randomWords
            )
        );

        require(success, "RNG: Failed to call the smart contract!");
    }

    function addConsumer(address consumerAddress)
        public
