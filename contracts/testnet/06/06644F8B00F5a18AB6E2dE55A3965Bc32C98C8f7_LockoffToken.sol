...
import "../interfaces/ILockOff.sol";
...
contract LockoffToken is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 23 * 10**8 * 10**DECIMALS;
    uint256 private constant TOTAL_GONS = MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
    uint256 private constant MAX_SUPPLY = ~uint128(0);

    address public presale = 0x0000000000000000000000000000000000000000;
    address public airdrop = 0x0000000000000000000000000000000000000000;

    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    address public ZERO = 0x0000000000000000000000000000000000000000;

    address public liquidityReceiver = 0x8F0162bd9603f502Db3264cbf4E2D6Ee60eb24A1;
    address public treasuryReceiver = 0x8F0162bd9603f502Db3264cbf4E2D6Ee60eb24A1;
    address public riskFreeValueReceiver = 0x8F0162bd9603f502Db3264cbf4E2D6Ee60eb24A1;
    address public usdtToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684; //mainnet
    ...
    constructor() ERC20Detailed("Token Test", "CAM", uint8(DECIMALS))
    {
        router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //mainnet 0x2Bf55D1596786F1AE8160e997D655DbE6d9Bca7A
    ...
    function checkUnlocked(address _sender, uint256 _amount) public view returns (bool) {
        require(_sender != address(0x0), "Recipient zero address");
        uint256 amount = _gonBalances[_sender].div(_gonsPerFragment);
        if (amount < _amount) {
            return false;
        }
        uint256 presaleUnlocked = 0;
        uint256 airdropUnlocked = 0;
        
        if (presale != address(0) ) {
            presaleUnlocked = ILockOff(presale).totalLocking(_sender);
        }

        
        if (airdrop != address(0) ) {
            airdropUnlocked = ILockOff(airdrop).totalLocking(_sender);
        }

        return  amount.sub(presaleUnlocked).sub(airdropUnlocked) >= _amount;
    }

    function totalLocking(address _sender) public view returns (uint256 _presale, uint256 _airdrop) {
        return (ILockOff(presale).totalLocking(_sender), ILockOff(airdrop).totalLocking(_sender));
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(checkUnlocked(sender, amount), "Insufficient" );
    ...
    function setLocker(address _presale, address _airdrop) external onlyOwner {
        presale = _presale;
        airdrop = _airdrop;
    }
...
pragma solidity ^0.8.0;
interface ILockOff {
    function totalLocking(address) external view returns (uint256);
}
