...
contract EAPool is Ownable, ReentrancyGuard {
    ...
    function DepoMiscProfitss(MiscEntry [] memory misc) public noReentrant {
    ...
    function DepoEAProfits(uint256 pamt) public noReentrant{
        ...
        require (pamt > 5000, "Minimum 5000 BUSD required.");
    ...
    function WithdrawMiscProfits() public noReentrant{

        EaUser storage user = EaUserKey[msg.sender];

        require (user.pendingWithdraw > 0, "No custom rewards are available for you.");
    ...
    function WithdrawEAProfits(uint256 pnum) public noReentrant{
        
        EaUser storage user = EaUserKey[msg.sender];
        EAProf storage profit = EProf[pnum];

        require (profit.eaendtime > block.timestamp, "Time to claim has passed. Rewards will roll over.");
    ...
    function transferLeftover(uint256 profitnumber) public onlyOwner{
