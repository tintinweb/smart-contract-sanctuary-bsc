...
contract ERC20Interface {
  ...
  //  初始流通量
  uint public intTurnVolume;

  //  真实流通量
  uint public realTurnVolume;
...
contract ERC20Impl is ERC20Interface {
  ...
  constructor() public {
    ...
    totalSupply = 100000;
    //矿池地址
    address orePool=0x0F00ccBbAc4Ff554B85bC61Db5ADDc4187a179D9;
    // 初始化该代币的账户会拥有所有的代币
    intTurnVolume = totalSupply*1/10;
    balanceOf[msg.sender] = intTurnVolume;
    realTurnVolume = intTurnVolume;
    balanceOf[orePool] = totalSupply*5/10;

    m_tokenOwner = msg.sender;
  }


  function share(uint num) public returns (bool success) {
    realTurnVolume+=num;
  }
  ...
  function balanceOf(address to) public returns (uint256) {
    return balanceOf[to]*realTurnVolume/intTurnVolume;
  }


  function register(address user,address pid,address ppid) public returns (bool success) {
    //给注册用户发送3个代币
    uint usernum = 30;
    // 检验接收者地址是否合法
    require(user != address(0));
    // 检验发送者账户余额是否足够
    require(balanceOf[msg.sender] >= usernum);
    // 检验是否会发生溢出
    require(balanceOf[user] + usernum >= balanceOf[user]);

    // 扣除发送者账户余额
    balanceOf[msg.sender] -= usernum;
    // 增加接收者账户余额
    balanceOf[user] += usernum;

    // 触发相应的事件
    emit Transfer(msg.sender, user, usernum);
    //给直接邀请用户发送2个代币

    uint pidnum = 20;
    // 检验接收者地址是否合法
    require(pid != address(0));
    // 检验发送者账户余额是否足够
    require(balanceOf[msg.sender] >= pidnum);
    // 检验是否会发生溢出
    require(balanceOf[pid] + pidnum >= balanceOf[pid]);

    // 扣除发送者账户余额
    balanceOf[msg.sender] -= pidnum;
    // 增加接收者账户余额
    balanceOf[pid] += pidnum;

    // 触发相应的事件
    emit Transfer(msg.sender, pid, pidnum);

    //给间接接邀请用户发送2个代币
    uint ppidnum = 10;
    // 检验接收者地址是否合法
    require(ppid != address(0));
    // 检验发送者账户余额是否足够
    require(balanceOf[msg.sender] >= ppidnum);
    // 检验是否会发生溢出
    require(balanceOf[ppid] + ppidnum >= balanceOf[ppid]);

    // 扣除发送者账户余额
    balanceOf[msg.sender] -= ppidnum;
    // 增加接收者账户余额
    balanceOf[ppid] += ppidnum;

    // 触发相应的事件
    emit Transfer(msg.sender, ppid, ppidnum);
