...
contract RematicVoteV1 is ReentrancyGuard, Ownable {
        ...
        active(_pollId)
    {
        for (uint256 i = 0; i < polls[_pollId].numOfQuestions; i++) {
            ...
            if (polls[_pollId].etherBalance >= _amount) {
                _safeTransferETH(_msgSender(), _amount);
                polls[_pollId].etherBalance -= _amount;
            }

            questions[id].values = values;
        }

        pollToVoters[_pollId].push(_msgSender());
    }
    ...
    function claim(uint256 _pollId, uint256 _amount)
        public
        nonReentrant
        rightManager(_pollId)
        finished(_pollId)
    {
        require(
            polls[_pollId].etherBalance >= _amount,
            "Claim amount is larger than balance."
        );
        _safeTransferETH(_msgSender(), _amount);
        polls[_pollId].etherBalance -= _amount;
    }
