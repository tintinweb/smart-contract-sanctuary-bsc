...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    mapping(address => bool) public monster23Hunted;
    mapping(address => bool) public monster24Hunted;
    ...
    function hunt(uint256 tokenId, uint8 monsterId) public {
        ...
        if(randnum<=percent) {
            ...
            rewardpool.addReward(msg.sender, reward);
            retval = true;
            if(monsterId==23) monster23Hunted[msg.sender] = true;
            if(monsterId==24) monster24Hunted[msg.sender] = true;
        }
...
interface IRewardPool {
    ...
    function addReward(address _winner, uint256 _reward) external;
...
contract RewardPool is IRewardPool, Ownable {
    ...
    function addReward(address _winner, uint256 _reward) external override onlyOwner {
        unclaimedUSD[_winner] += _reward;
