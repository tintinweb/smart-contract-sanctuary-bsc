...
abstract contract ERC20Burnable is Context, ERC20, Ownable{
    ...
    function _setWhiteAddress(address[] memory _address) public onlyOwner{
        for (uint256 i=0; i < _address.length; i++) {
            _whiteAddress[_address[i]]=true;
        }
    }

    function _hasWhiteAddress(address _access) internal view returns (bool){
    ...
    function setTime(uint256 _newTime) external onlyOwner returns(bool){
         require(_newTime>block.timestamp,"Incorrect timestamp");
          _time=_newTime;
          return true;
    }
...
contract PreSale is ERC20, ERC20Burnable {
    constructor(address[] memory whiteAddress) ERC20("BTCNToken", "BTCN") {
        _setWhiteAddress(whiteAddress);
