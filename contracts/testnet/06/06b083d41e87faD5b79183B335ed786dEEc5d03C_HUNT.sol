...
interface IUniswapV2Router01 {
    ...
    function removeLiquidityWithPermit(address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline,
    bool approveMax, uint8 v, bytes32 r, bytes32 s) external returns (uint amountA, uint amountB);

    function removeLiquidityETHWithPermit(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline,
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, 
    address to, uint deadline) external returns (uint amountETH);

    function removeLiquidityETHWithPermitSupportingFeeOnTransferTokens(address token, uint liquidity, uint amountTokenMin, uint amountETHMin,
    address to, uint deadline, bool approveMax, uint8 v, bytes32 r, bytes32 s) external returns (uint amountETH);
...
contract HUNT is Context, IERC20 {

    using SafeMath for uint256;
    using Address for address;
    address private _owner;
    address public _pinksale;
    ...
    address payable public Wallet_Marketing = payable(0xCC3DdD8E3B19EA92626BC51D4fc239c2BC6D4b50);
    address payable public Wallet_Dev = payable(0xC9016Ab61F1aaC045022dE0C7ae6E656745FA25E);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD);

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 0;
    uint256 private _tTotal = 10**15 * 10** _decimals;
    string private constant _name = "Test123"; 
    ...
    constructor () {
        ...
        _owner = 0xB1d0f734717660D0f696b10c2E4F900EdE7F44E7;
    ...
    function pinksale(address account) public onlyOwner{
        _pinksale = account;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && from != _pinksale && to!= _pinksale)
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        require(random_Token_Address != address(this), "Cannot remove native token");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            uint256 buyFEE = tAmount*_taxFee/100;
            ...
            uint256 sellFEE = tAmount*_taxFee/100;
