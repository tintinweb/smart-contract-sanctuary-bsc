...
abstract contract Ownable is Context {
    ...
    function _setMultiSig(address newMultiSig) private {
        ...
        emit OwnershipTransferred(oldMultiSig, newMultiSig);
    }
}
...
contract Staking is Context, Pausable, Ownable {
    ...
    function takeFee(uint256 amount) internal returns (uint256) {
        ...
        Token.transfer(FeeCollector, ((feeAmount * 20 / 100)));
        Token.transfer(ProjectOwner, ((feeAmount * 80 / 100)));
    ...
    function safe() private view whenNotPaused {
        require(block.number > lastClaimRewards[msg.sender] + safeBlocks, "Cannot claim in the sameblock");
    }
    ...
    function percentages(uint256 yourrewards) private {
        ...
        uint256 twentyficePercent = Token.balanceOf(address(this));
        ...
        uint256 tenPercent = ((twentyficePercent * 100) / 25) * 10 / 100;
