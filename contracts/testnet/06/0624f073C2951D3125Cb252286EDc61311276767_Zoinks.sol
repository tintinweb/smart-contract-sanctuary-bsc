...
/// @title ZOINKS token contract.
contract Zoinks is IERC20Metadata, Ownable {
    ...
    uint256 constant private BURN_PERCENT = 9900;
    uint256 constant private REBASE_INFLATION_PERCENT = 10100;
    ...
    uint256 private _adjustmentFactor = PRBMathUD60x18.fromUint(1);
    
    mapping(address => uint256) private _adjustedBalances;
    ...
    /// @param busd_ BUSD token contract address.
    /// @param pancakeSwapRouter_ PancakeSwap router address.
    constructor(
        address busd_,
    ...
    )
    {
        ...
        IERC20(busd).approve(pancakeSwapRouter, type(uint256).max);
    }
    
    /// @notice Configures the contract.
    /// @dev Сan only be called by the owner of the contract.
    /// @param authority_ Address of the EOA that has access to the applyTWAP function call.
    /// @param seniorage_ Seniorage contract address.
    /// @param pulse_ Pulse contract address.
    /// @param snacks_ SNACKS token contract address.
        ...
        onlyOwner
    {
        ...
        if (IERC20(address(this)).allowance(address(this), snacks) == 0) {
            IERC20(address(this)).approve(snacks, type(uint256).max);
        }
    }
    
    /// @notice Sets or resets buffer.
    /// @param buffer_ New value for buffer.
    function setBuffer(uint256 buffer_) external onlyOwner {
        buffer = buffer_;
    }
    
    function createPair(address pancakeSwapFactory_) external onlyOwner {
        pancakeSwapPair = IFactory(pancakeSwapFactory_).createPair(address(this), busd);
    }
    
    /// @notice Creates `amount_` tokens in exchange for BUSD at a ratio of 1:1.
    /// @param amount_ Amount of ZOINKS tokens to mint.
    function mint(uint256 amount_) external {
        ...
        require(
            _totalSupply + amount_ <= MAX_SUPPLY,
    ...
    /// @notice Exchanges BUSD tokens for ZOINKS tokens via ApeSwap, BiSwap or PancakeSwap
    /// @param amountIn_ Amount of BUSD tokens to exchange.
    /// @param amountOutMin_ The minimum expected amount of ZOINKS tokens to receive after the exchange.
    /// @param deadline_ The time after which the transaction is considered failed.
        ...
        external
    {
        ...
        emit Swap(msg.sender, amountIn_, amounts[1]);
    }
    
    /*
    * @notice Burns tokens or distributes inflationary emission depending on the value of TWAP.
    * @dev Can only be called by the authority address. It is
    * called once every 12 hours.
    */
    function applyTWAP() external onlyAuthority {
        uint256 TWAP = getTWAP();
        if (TWAP < BURN_PERCENT) {
            _burn(TWAP);
        } else if (TWAP > REBASE_INFLATION_PERCENT) {
            _distributeInflationaryEmission(TWAP);
        }
    ...
    function transfer(
        ...
        returns (bool)
    {
        uint256 adjustedAmount = amount_.mul(_adjustmentFactor);
        if (to_ == snacks) {
            _adjustedBalances[msg.sender] -= adjustedAmount;
            _adjustedBalances[to_] += amount_;
        } else if (msg.sender == snacks) {
            _adjustedBalances[msg.sender] -= amount_;
    ...
    */
    ...
    function balanceOf(address account_) public view override returns (uint256) {
        if (account_ == snacks) {
            ...
            return _adjustedBalances[account_].div(_adjustmentFactor);
        }
    }
    
    /// @notice Returns circulating supply.
    function circulatingSupply() public view returns (uint256) {
        return _totalSupply - balanceOf(address(0)) - balanceOf(DEAD_ADDRESS);
    }
    
    /// @notice Returns time-weighted average price.
    function getTWAP() public view returns (uint256 TWAP) {
        TWAP = IERC20(busd).balanceOf(pancakeSwapPair) * BASE_PERCENT / balanceOf(pancakeSwapPair);
    }
    ...
    */
    ...
    /*
    * @notice Burns tokens from all holders by a certain percentage
    * under certain conditions.
    * @param TWAP_ TWAP in 12 hours.
    */
    function _burn(uint256 TWAP_) private {
        uint256 burningPercentage = BASE_PERCENT - TWAP_;
        uint256 supplyDelta = _totalSupply * burningPercentage / BASE_PERCENT;
        _adjustmentFactor = _adjustmentFactor
            .mul(_totalSupply - balanceOf(snacks))
            .div((_totalSupply - balanceOf(snacks)) - supplyDelta);
        _totalSupply -= supplyDelta;
    }
    
    /// @notice Distributes inflationary emission under certain conditions.
    /// @param TWAP_ TWAP in 12 hours.
    function _distributeInflationaryEmission(uint256 TWAP_) private {
        uint256 inflationaryEmission = (TWAP_ - BASE_PERCENT) * circulatingSupply() / (BASE_PERCENT * buffer);
        require(
            _totalSupply + inflationaryEmission <= MAX_SUPPLY,
            "Zoinks: max supply exceeded"
        );
        _mint(poolRewardDistributor, inflationaryEmission * POOL_REWARD_DISTRIBUTOR_PERCENT / BASE_PERCENT);
        _mint(seniorage, inflationaryEmission * SENIORAGE_PERCENT / BASE_PERCENT);
        uint256 amountOfZoinksToBuySnacks = inflationaryEmission * BUY_SNACKS_PERCENT / BASE_PERCENT;
        _mint(address(this), amountOfZoinksToBuySnacks);
        uint256 balanceBefore = IERC20(snacks).balanceOf(address(this));
        ISnacksBase(snacks).buyTotal(amountOfZoinksToBuySnacks);
        uint256 difference = IERC20(snacks).balanceOf(address(this)) - balanceBefore;
        IERC20(snacks).safeTransfer(DEAD_ADDRESS, difference * ISOLATE_PERCENT / BASE_PERCENT);
        IERC20(snacks).safeTransfer(pulse, difference * PULSE_PERCENT / BASE_PERCENT);
    }
}
