...
contract TestToken is ERC20Detailed, Ownable {
    ...
    uint256 public _addLiqPeriod = 1 hours;
    uint256 public _totalSupply;
    uint256 public _maxTxPercent = 5;

    uint256 public swapThreshold = 100 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("TestToken102", "$TT102", uint8(DECIMALS)) Ownable() {
    ...
    function takeFee(
        ...
        if(feeAmount > 0) {
            uint256 forLiq = feeAmount.mul(liquidityFee).div(_totalFee);
    ...
    function swapBack() internal swapping {
        ...
        bool success;
        uint256 bnbReceived = address(this).balance;
        uint256 bnbForTreasury = bnbReceived.mul(aeternaTreasuryFee).div(aeternaTreasuryFee.add(aeternaDefenceFee));
        uint256 bnbForDefence = bnbReceived.sub(bnbForTreasury);
