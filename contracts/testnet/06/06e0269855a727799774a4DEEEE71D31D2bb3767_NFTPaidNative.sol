...
contract NFTPaidNative is ERC721, Ownable {
    ...
    function setRequiredFund(uint256 _requiredFund) external onlyOwner {
        requiredFund = _requiredFund;
    }

    function safeMint(address to) external payable {
        bool isOwner = owner() == _msgSender();
        if (!isOwner && msg.value != requiredFund) revert IncorrectFunds({ sent: msg.value, required: requiredFund });
