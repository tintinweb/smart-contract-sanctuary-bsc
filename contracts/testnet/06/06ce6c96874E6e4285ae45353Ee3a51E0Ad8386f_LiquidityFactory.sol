...
contract LiquidityFactory is ILiquidityFactory,Ownable {
    ...
    mapping(address=>bool) public wsbPair;
    ...
    function _addLiquidity(
        ...
        address pair = ISwapFactory(_factory).pairFor(tokenA, tokenB);
        ...
        (uint reserveA,uint reserveB) = ISwapFactory(_factory).getReserves(tokenA, tokenB);

        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = ISwapFactory(_factory).quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'LiquidityFactory: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = ISwapFactory(_factory).quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidity(
        ...
        address pair = ISwapFactory(_factory).pairFor(_tokenA, _tokenB);
    ...
    function addLiquidityETH(
        ...
        address pair = ISwapFactory(_factory).pairFor(token, WBNB);
