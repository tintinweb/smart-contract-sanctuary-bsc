...
contract FeeDistributorFactory is Ownable {
    mapping(address => address) private feeDistributors;
    ...
    function getFeeDistributor(address token) external view returns (address) {
        return feeDistributors[token];
    }
    ...
    function createFeeDistributor(
        ...
        require(feeDistributors[_token] == address(0));
        FeeDistributorV6 f = new FeeDistributorV6(
            IERC20(_token),
            _dualToken,
            router,
            _dualTokenVault
        ...
        feeDistributors[_token] = address(f);
    }
}
