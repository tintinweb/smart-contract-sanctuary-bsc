...
contract BodaV2 is ERC20, Ownable {
    ...
    uint256 public totalFees = busdDividendRewardsFee.add(liquidityFee).add(marketingFee).add(burnFee);
    ...
    constructor() ERC20("BODA", "BODAV2") {
    	...
    	marketingWallet = 0xAC1edfC88374B2543d686AB3f33d025B0F520c62; 
    ...
    function updateBurnFee(uint256 _burnFee) external onlyOwner{
        burnFee = _burnFee;
        totalFees = burnFee.add(liquidityFee).add(busdDividendRewardsFee).add(marketingFee);

    }

    function updateLiquidityFee(uint256 _liquidityFee) external onlyOwner{
        liquidityFee = _liquidityFee;
        totalFees = liquidityFee.add(marketingFee).add(busdDividendRewardsFee).add(burnFee);

    }

    function updateMarketingFee(uint256 _marketingFee) external onlyOwner{
        marketingFee = _marketingFee;
        totalFees = marketingFee.add(liquidityFee).add(busdDividendRewardsFee).add(burnFee);
    }

    function updateBusdDividendRewardsFee(uint256 _busdDividendRewardsFee) external onlyOwner{
        busdDividendRewardsFee = _busdDividendRewardsFee;
        totalFees = marketingFee.add(liquidityFee).add(busdDividendRewardsFee).add(burnFee);
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || excludedAccount, "Boda: Trading has not started yet");
        ...
        if(takeFee) {
            ...
            if(burnShare>0) {
                super._transfer(from, deadAddress, burnShare);
            }
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialBUSDBalance = IERC20(busdDividendToken).balanceOf(address(this));

        swapTokensForBUSD(tokens);
        uint256 newBalance = (IERC20(busdDividendToken).balanceOf(address(this))).sub(initialBUSDBalance);
        IERC20(busdDividendToken).transfer(marketingWallet, newBalance);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private{
        swapTokensForBUSD(tokens);
        uint256 dividends = IERC20(busdDividendToken).balanceOf(address(this));
        bool success = IERC20(busdDividendToken).transfer(address(busdDividendTracker), dividends);

          if (success) {
            busdDividendTracker.distributeDividends(dividends);
