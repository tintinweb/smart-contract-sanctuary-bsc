...
contract AltairFactory is Initializable, ERC1155HolderUpgradeable {
    ...
    event RolesContractUpdated(address indexed _rolesContract);
    event SwapContractUpdated(address indexed _swapContract);
    event FundMasterContractUpdated(address indexed _fundMaster);
    event TreasuryMasterContractUpdated(address indexed _treasury);
    event FeeUpdated(uint256 indexed _fee);
    event ManagerFeeUpdated(uint256 indexed _managerFee);
    event Subscribed(address indexed _fund);
    event FreeTrialUpdated(uint256 indexed _freeTrial);
    event MonthlyCostUpdated(uint256 indexed _m);
    event SubscribeFeeUpdated(uint256 indexed _fee);
    event redeemFeeUpdated(uint256 indexed _fee);
    ...
    function initialize(
        ...
        shop = _shop;
        altairSwap = _altairSwap;
        naliAltair = NaliAltair(_naliAltair);
        naliToken = INaliToken(_naliToken);
        naliNftTicket = INaliNftTicket(_naliNftTicket);
        customToken = IERC20Upgradeable(_customToken);
        treasury = _treasury;
        swapContract = _swapContract;
        fundMaster = _fundMaster;
        fee = 100;
        subscribeFee = 100;
        redeemFee = 100;
        NATIVE = _NATIVE;
    ...
    function getFundId(address _address) public view returns (uint256) {
        return fundsId[_address];
    }

    function totalSupply(address _fund) external view returns (uint256) {
        uint256 _fundId = getFundId(address(_fund));
        return naliAltair.totalSupply(_fundId);
    }
    ...
    function subscribe(address _fund, bool _useNali) external returns (bool) {
        require(funds.contains(_fund));
    ...
    function changeSubscribeFee(uint256 _fee)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setUseSwapInfo(bool _bool) external onlyAdmin returns (bool) {
        useSwapInfo = _bool;

        return true;
    }

    function changeManagerFee(uint256 _managerFee)
        external
        onlyAdmin
        returns (bool)
    {
        ...
        returns (bool)
    {
        ...
        emit TreasuryMasterContractUpdated(address(treasury));

        return true;
    }

    function setMaster(IAltairMaster _fundMaster)
        external
        onlyAdmin
        returns (bool)
    {
        ...
        emit FundMasterContractUpdated(address(fundMaster));

        return true;
    }

    function setShop(IAltairShop _shop) external onlyAdmin returns (bool) {
        shop = _shop;

        return true;
    }

    function setAltairSwap(IAltairSwap _altairSwap)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function setSwapContract(IAltairSwapInfo _swapContract)
        external
        onlyAdmin
        returns (bool)
    {
    ...
    function changeMonthlyCost(uint256 _m) external onlyAdmin returns (bool) {
        monthlyCost = _m;
        ...
        returns (bool)
    {
        ...
        emit FreeTrialUpdated(freeTrial);
    ...
    function adminUpdateProxyCallContract(address _proxyCallContract)
        external
        onlyAdmin
    {
        _updateProxyCallContract(_proxyCallContract);
    ...
    function adminUpdateRolesContract(address _rolesContract)
        external
        onlyAdmin
    {
        _updateRolesContract(_rolesContract);
