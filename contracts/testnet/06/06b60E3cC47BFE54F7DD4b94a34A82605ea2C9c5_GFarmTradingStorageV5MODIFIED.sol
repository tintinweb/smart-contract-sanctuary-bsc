...
// File: contracts\GFarmTradingStorageV5MODIFIED.sol
pragma solidity 0.8.7;

contract GFarmTradingStorageV5MODIFIED {
    ...
    TokenInterfaceV5 public token = TokenInterfaceV5(0x0000000000000000000000000000000000000000); // Requires update
    ...
    address public gov = 0x0000000000000000000000000000000000000001;
    address public dev = 0x0000000000000000000000000000000000000001;
    ...
    constructor(address _dai, address _link) {
        dai = TokenInterfaceV5(_dai);
        linkErc677 = TokenInterfaceV5(_link);
        gov = msg.sender;
        dev = msg.sender;
    }
    ...
    function updateToken(TokenInterfaceV5 _newToken) external onlyGov{
        ...
        require(address(_newToken) != address(0), "Wrong token address");
        token = _newToken;
    ...
    event DATAS(string s, Trade t);
    event DATAU(string s, TradeInfo ti);

    // Manage stored trades
    function storeTrade(Trade memory _trade, TradeInfo memory _tradeInfo) external {


        emit DATAS("_trade",_trade);
        emit DATAU("_tradeInfo", _tradeInfo);
    ...
    event DATA(string s, uint v);
    event DATAB(string s, bool b);
    // Manage dev & gov fees
    function handleDevGovFees(uint _pairIndex, uint _leveragedPositionSize, bool _dai, bool _fullFee) external returns(uint fee){
        fee = _leveragedPositionSize * priceAggregator.openFeeP(_pairIndex) / PRECISION / 100;

        emit DATAB("_dai", _dai);
        emit DATAB("_fullFee", _fullFee);
        emit DATA("_leveragedPositionSize", _leveragedPositionSize);
        emit DATA("_pairIndex", _pairIndex);
        emit DATA("priceAggregator.openFeeP(_pairIndex)", priceAggregator.openFeeP(_pairIndex));
        emit DATA("PRECISION", PRECISION);
        emit DATA("fee", fee);

        if(!_fullFee){ fee /= 2; emit DATA("fee2", fee);}
        emit DATA("govFeesDai", govFeesDai);
        emit DATA("devFeesDai", devFeesDai);
        emit DATA("govFeesToken", govFeesToken);
        emit DATA("devFeesToken", devFeesToken);
        if(_dai){
            govFeesDai += fee;
            devFeesDai += fee;
        }else{
