...
contract NFT is Ownable, ERC721, IERC721Receiver {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    address public constant usdAddress =
        0x47A01F129b9c95E63a50a6aa6cBaFDD96bEb4C6F; //usdt合约
    uint256 public maxNft = 999999999;

    bool public disableTransfer = true; //是否禁止转账
    bool public isCheckMiner = false; //是否仅Miner可以铸造
    mapping(address => bool) public miners; //仅仅允许此列表地址可铸造,使用
    bool public isCheckWhite = false; //是否仅白名单可铸造
    mapping(address => bool) public whiteList; //仅仅允许此列表地址可铸造

    mapping(address => uint256) public autherNft; //创作者=>作品
    mapping(uint256 => uint256) public nftPrice; //nft价值
    mapping(uint256 => uint256) public nftPower; //nft能量
    mapping(uint256 => uint256) public nftLevel; //nft级别
    mapping(uint256 => bool) public nftUsed; //nft已使用

    mapping(uint256 => Goods) public goodsInfo; // nft信息
    struct Goods {
        uint256 price;
        uint256 power;
    }
    ...
    constructor() ERC721("Nostep NFT", "NST-NFT") {
        miners[owner()] = true;
    }
    ...
    //设置miner
    function setMiner(address[] memory addrlist, bool _value)
        external
        onlyOwner
    {
        require(addrlist.length > 0, "lsit empty error");
        for (uint256 i = 0; i < addrlist.length; i++) {
            miners[addrlist[i]] = _value;
        }
    }

    //设置最大铸造总量
    function setMaxNft(uint256 _value) external onlyOwner {
        maxNft = _value;
    }
    ...
    {
        //铸造个数限制
        require(_tokenIds.current() < maxNft, "maxNft overflow error");
        //只能铸造一次
        require(autherNft[caller] == 0, "Do not repeat mint");
        ...
        if (isCheckMiner) {
            ...
            require(
                whiteList[msg.sender] || miners[msg.sender],
        ...
        uint256 _price = goodsInfo[id].price;
        uint256 _power = goodsInfo[id].power;
        ...
        nftLevel[newItemId] = id;
        nftPrice[newItemId] = _price;
        nftPower[newItemId] = _power;
        autherNft[caller] = newItemId;
        return newItemId;
    }

    //激活nft并收取铸造费用
    function useNftPrice(address account_)
        external
        onlyMiner
        returns (uint256 tokenId_)
    {
        tokenId_ = autherNft[account_];

        require(tokenId_ > 0, "Not mint error");
        require(!nftUsed[tokenId_], "NFT has been used");
        require(ownerOf(tokenId_) == account_, "NFT owner error");

        if (nftPrice[tokenId_] > 0) {
        ...
        nftUsed[tokenId_] = true;

        emit UseNft(ownerOf(tokenId_), msg.sender, tokenId_);
    }
    ...
    function setInvalidTokenUrl(uint256 tokenId_, string memory tokenUrl_)
        external
        onlyOwner
    {
        _setTokenURI(tokenId_, tokenUrl_);
    }
