...
contract Treasury is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function initialize(address _pool, address _weth, address _ethUnwrapper, address _rewardToken) external initializer {
        ...
        require(_rewardToken != address(0), "Treasury::invalid address");
        pool = IPool(_pool);
        weth = IWETH(_weth);
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    function distribute(address _receiver, uint256 _amount) external onlyLgoRedemptionPool {
        require(rewardToken != address(0), "Treasury::reward token not set");
        require(_receiver != address(0), "Treasury::invalid receiver address");
        IERC20(rewardToken).safeTransfer(_receiver, _amount);
        emit TokenDistributed(lgoRedemptionPool, rewardToken, _receiver, _amount);
    }

    function distributeToken(address _receiver, address _tokenOut, uint256 _lpAmount, uint256 _minAmountOut)
        external
        onlyLgoRedemptionPool
    {
        ...
        if (_tokenOut == address(weth)) {
            actualAmountOut = _removeLiquidity(rewardToken, _tokenOut, _lpAmount, _minAmountOut, address(this));
            _safeUnwrapETH(actualAmountOut, _receiver);
        } else {
            actualAmountOut = _removeLiquidity(rewardToken, _tokenOut, _lpAmount, _minAmountOut, _receiver);
        }
    ...
    {
        uint256 amountOut = _addLiquidity(_token, _amount, rewardToken, _minAmountOut, address(this));
        emit LLPConverted(_token, address(rewardToken), _amount, amountOut);
    }
        ...
        onlyRole(CONTROLLER_ROLE)
    {
        ...
        pool.swap(_fromToken, _toToken, _minAmountOut, address(this));
    ...
    function setRewardToken(address _rewardToken) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_rewardToken != address(0), "Treasury::invalid reward token");
        rewardToken = _rewardToken;
    ...
    function _removeLiquidity(
        ...
        pool.removeLiquidity(rewardToken, _tokenOut, _lpAmount, 0, _receiver);
