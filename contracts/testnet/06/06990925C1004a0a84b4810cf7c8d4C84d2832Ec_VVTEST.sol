...
contract VVTEST is IERC20 {
    ...
    string constant private _name = "VVTEST";
    string constant private _symbol = "VV";
    ...
    struct TaxWallets {
        ...
        address payable vvtest;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xCB2DB213D64A1212FCD5DbE8F13469Aea6F25955),
        vvtest: payable(0xB0F9b21067c0d8238a86e27eE8211CA5b143fcA9)
        });
...
//===============================================================================================================
    ...
    function setWallets(address payable marketing, address payable vvtest) external onlyOwner {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.vvtest = payable(vvtest);
    }

    function setRatios(uint16 reflection, uint16 vvtest, uint16 littleBeans, uint16 marketing, uint16 liquidity) external onlyOwner {
        _ratios.reflection = reflection;
        _ratios.vvtest = vvtest;
        _ratios.littleBeans = littleBeans;
        _ratios.marketing = marketing;
        _ratios.liquidity = liquidity;
        _ratios.totalSwap = marketing + vvtest + littleBeans + liquidity;
    ...
    function setvvtestEnabled(bool enabled) external onlyOwner {
        if (enabled) {
            littleBeansEnabled = false;
        }
        vvtestEnabled = enabled;
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal lockTheSwap {
        ...
        bool success;
        uint256 vvtestBalance = (amtBalance * ratios.vvtest) / ratios.totalSwap;
        uint256 littleBeansBalance = (amtBalance * ratios.littleBeans) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - (vvtestBalance + littleBeansBalance);
        if (ratios.vvtest > 0) {
            (success,) = _taxWallets.vvtest.call{value: vvtestBalance, gas: 35000}("");
        }
