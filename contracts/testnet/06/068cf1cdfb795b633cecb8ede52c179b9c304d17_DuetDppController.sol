...
contract DuetDppController is Adminable, DuetDppLpFunding {
    ...
    function init(
        ...
        require(IDODOV2(dppAddress)._OWNER_() == dppAdminAddress, "dpp admin not match");
        _WETH_ = weth;
        ...
        )
    {
        ...
        (shares, , ) = _buyShares(msg.sender, baseAdjustedInAmount, quoteAdjustedInAmount);
        // reset dpp pool
    ...
    // test 
    /*
    function changeDppAdminOwner(address newOwner) onlyAdmin public{
        IDPPOracleAdmin(_DPP_ADMIN_ADDRESS_).transferOwnership(newOwner);
    }
    
    function claimDppAdminOwnership() onlyAdmin public {
        IDPPOracleAdmin(_DPP_ADMIN_ADDRESS_).claimOwnership();
    }
    */
}