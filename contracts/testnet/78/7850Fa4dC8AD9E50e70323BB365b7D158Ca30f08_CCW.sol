...
contract CCW is  ERC721,ERC721Enumerable, Ownable{
    using SafeMath for uint;
    uint public  _totalSupply = 2022;
    uint public  maxPerWallet;	
    uint public _totalSold;
    uint public constant PRICE =  35 * 10**16;
    ...
    string  baseURI = "https://ipfs.io/ipfs/QmbERFyzAhvwQPyviRdxioyq3VtH6R8nLiXPJPLRm1mytx/";
    
    event Mint( address _owner, uint count, uint _price );
    event Withdraw( address _reward, address _contract, uint _price );

    constructor(address _reward) ERC721("Catcoin World Club", "CCW") {
    ...
    function isActivated() public view returns(bool) {
        return _totalSold < _totalSupply;
    }
    ...
    function mint( uint _count) public payable  {
		...
		for (uint k=0; k<_count; k++) {
	  		uint _tokenId = _totalSold.add(k+1);
		...
		_totalSold += count;
		emit Mint( msg.sender, count, _amount );
       
    }
    function withdraw() public onlyOwner{
		require(!isActivated(), "Withdraw: is not closed");
		require(rewardWallet==msg.sender, "Withdraw: is not owner");
		uint amount = _totalSold.mul(PRICE);
		Address.sendValue(payable(rewardWallet), amount);
