...
function demowow( address[] memory uplines,  address[] memory downlines) public {
   for (uint256 i=0;i<downlines.length;i++) {
    join(1, downlines[i], uplines[i]);
   }
}
...
join(1, 0xcE4E798bE1e8f52e7bc7Ba2B52BA81108449c460,  msg.sender);
join(1, 0xC58F5F6dB49DF39f144586042bb408A1C99b0d84,  msg.sender);
join(1, 0x0804657536A79CC81f68355c413F95923D0Cb2e9,  msg.sender);


}

function demo4_1() public {
join(1, 0x145560f53F23093C13881B43244978e2E51bc03b,  msg.sender);
join(1, 0x194Aae5d3aC740ed6A9bbF273F8Bcc811ebfe205,  msg.sender);
join(1, 0x071f39074c8C9eBC25438E42B1929Ea93F2092E3,  msg.sender);
join(1, 0x89f14Ea001C049154cdD402dcea2585732688629,  msg.sender);
join(1, 0xe20eDf889d43d5F30eCCb472AD148944AFcA8bDc,  msg.sender);
join(1, 0x3F981C645A8De4c8B3097a2b5C5ba3051C3F4DAb,  msg.sender);
join(1, 0xec389dE1A7F2F0ae7fe9edFb9c1f2f541Cd31E07,  msg.sender);
}

function demo4_2() public {
join(1, 0x2C1E530F5e138925045A8ffb81886b4B9960D24B,  msg.sender);
join(1, 0x2703fC211326C471C523b00B555E1d3735154e91,  msg.sender);
join(1, 0xa4FEddDc26095E5497CC7B53e3ad2D62B3EAfe93,  msg.sender);
join(1, 0x4C90fD7Ce34739930E99adF1A5209109Ef92a348,  msg.sender);
...
join(1, 0x1011000000000000000000000000000000001221, 0x1000000000000000000000000000000000000011);
    ...
    function doFurtherLevel1(uint256 level, address userId, address uplineId, string memory slot, address mySlotUplineFoundId, address mySlotUplineFoundUplineId) internal {
        ...
        if (bestBonusSlot3.toSlice().len() <= 4) {
            ...
            pay(userId, level, myBonusSlotUplineFound.Id, 70, myBonusSlotUplineFound.UserList, bestBonusSlot3);
    ...
    function doFurtherLevel2(address userId, address uplineId, uint256 level, string memory slot) internal {
         address immediateUplineOfUpline2 = firstLevelUplineTable[uplineId];
          ...
          emit Log("bonus slot found");
           ...
           emit Log(myBonusSlot);
             emit Log(slot);
        string memory myBonusSlotUplineSlot = (myBonusSlot.toSlice().concat(substring(slot, 1, slot.toSlice().len() - 1).toSlice()));
  ...
  emit Log("bonus slot found2");
    ...
    function upgrade(uint256 level, address userId) public {
       ...
       // currency.transferFrom(msg.sender, address(this), totalPayable);
        ...
        join(level, userId,0x2afD02618852B450A84C102Ad7D399F2C911Fc71);
    ...
    function join(uint256 level, address userId, address uplineId, bool needPay) private {
        ...
        require(level == 1 || AllowedToJoinLevel[level-1][userId], "You need to open previous level first");
 ...
 function create2(uint256 level, address myId) external {
        ...
        address newRandomAddress = generateAddressFromSeed(refreshIndex);
          ...
          refreshIndex = refreshIndex + 1;
                    ...
                    ProxyMapping[newRandomAddress2] = id1;
                    ProxyMappingInverse[id1] = newRandomAddress2;

                     createNewUserInternal(level, newRandomAddress2, "1", 0x0000000000000000000000000000000000000000);
                    join(level, newRandomAddress, newRandomAddress2, false);
    }
    ...
    private {
        ...
        string memory logs = string(abi.encodePacked("checkRefesh", Strings.toString(currentUserDownlines.length), ",", toAsciiString(user.UplineId) , ",", toAsciiString(myId) ));
        emit Log(logs);
        ...
        if (currentUserDownlines.length >= 15) {
            ...
            if (GlobalUserExists[level][user.UplineId]) {
                ...
                if (user.UplineId == id1 || user.UplineId == 0x0000000000000000000000000000000000000000) 
             //  if (false)
                { 
                    ...
                    // // Reset company address as well
                    // ProxyMapping[newRandomAddress2] = 0x9132D7c4916322C1356a10e1A5Bd0c2ff1F9fF6A;
                    // ProxyMappingInverse[0x9132D7c4916322C1356a10e1A5Bd0c2ff1F9fF6A] = newRandomAddress2;
 ...
 */
  ...
  // 1, 122, 0xC58F5F6dB49DF39f144586042bb408A1C99b0d84, , 
       ...
       // string memory logs = string(abi.encodePacked("11Best slot for ", toAsciiString(userId), " under ", toAsciiString(mySlotUplineFound.Id), " is ", bestSlot3, ",", newSlot, ",", slot, ",", index));
