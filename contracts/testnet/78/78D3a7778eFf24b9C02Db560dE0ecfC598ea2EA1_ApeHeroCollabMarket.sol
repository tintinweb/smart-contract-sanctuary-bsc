...
contract ApeHeroCollabMarket is Ownable, AccessControlEnumerable, Pausable, ReentrancyGuard {
    ...
    function _refund(Item memory item, uint receiptId) internal {
        ...
        if (item.nativePayment) {
            payable(RECEIPTS[_itemId][receiptId].account).transfer(item.price);
        } else {
            IERC20(address(item.paymentToken) != address(0) ? item.paymentToken : POWAddress)
            .transfer(RECEIPTS[_itemId][receiptId].account, item.price);
        }
        RECEIPTS[_itemId][receiptId].refunded = true;
        ITEM_BALANCES[_itemId] -= item.price;

        emit Refunded(RECEIPTS[_itemId][receiptId].account, item.price, _itemId, receiptId, block.timestamp);
    }
    ...
    function finalizeItem(uint itemId) external onlyAdminAndOperator {
        ...
        if (_item.mode == ItemMode.RAFFLE) {
            ...
            require(_winners.length > 0, "Winner");
    ...
    function getReceiptsForItem(uint itemId, uint[] memory receipts, uint page, uint limit) public view returns (Receipt[] memory) {
        if (receipts.length > 0) {
        ...
        if (page == 0 && limit == 0) {
            _start = 0;
            _end = _item.currentSupply;
        } else {
            _start = (page * limit) + 1;
            _end = limit > 0 ? (_start + limit) : (_item.currentSupply + 1);
        }

        Receipt[] memory _receipts = new Receipt[](_end - _start);
        for (uint _idx = _start; _idx < _end; _idx++) {
            _receipts[_idx] = RECEIPTS[itemId][_idx];
        }
    ...
    function bid(uint itemId, uint payment, string calldata data) external payable whenNotPaused nonReentrant {
        ...
        require((_item.expiresOn > _now) && (_item.status == ItemStatus.ACTIVE), "Item expired or inactive");

        uint _currentWinningReceiptId = AUCTION_WINNERS[itemId];
        require(payment >= (RECEIPTS[itemId][_currentWinningReceiptId].payment + _item.minimumIncrement), "Invalid bid");
        if (_item.nativePayment) {
            require(payment == msg.value, "Invalid bid");
            ITEM_BALANCES[itemId] += msg.value;
        } else {
            uint _balance = IERC20(address(_item.paymentToken) != address(0) ? _item.paymentToken : POWAddress).balanceOf(_buyer);
            require(_balance >= payment, "Insufficient balance");
            IERC20(address(_item.paymentToken) != address(0) ? _item.paymentToken : POWAddress)
                .transferFrom(_buyer, address(this), payment);
            ITEM_BALANCES[itemId] += payment;
        }

        // winning bid
        uint _newReceiptId = _item.currentSupply + 1;
        RECEIPTS[itemId][_newReceiptId] = Receipt({
            receiptId: _newReceiptId,
            itemId: itemId,
            account: _buyer,
            originalAccount: _buyer,
            amount: 1,
            payment: payment,
            data: data,
        ...
        // adds 10 minutes if bid within 3 minutes of expiration
        if ((_item.expiresOn - _now) <= 180) {
            ItemManager.extendItemExpiration(itemId, 600);
        }
    ...
    function buy(uint itemId, uint amount, string calldata data) external payable whenNotPaused nonReentrant {
        ...
        require((_item.expiresOn >= _now) && (_item.status == ItemStatus.ACTIVE), "Item expired or inactive");
        require(
            ((_item.maxPerAccount == 0) || (ACCOUNT_RECEIPTS[_buyer][itemId].length() + amount <= _item.maxPerAccount)) &&
            ((_item.maxSupply == 0) || (_item.currentSupply + amount <= _item.maxSupply)),
            "Sold-out or buying more than available"
        );

        uint _totalCost = _item.price * amount;
        if (_item.nativePayment) {
            require(msg.value >= _totalCost, "Insufficient balance");
            ITEM_BALANCES[itemId] += msg.value;
        } else {
            uint _balance = IERC20(address(_item.paymentToken) != address(0) ? _item.paymentToken : POWAddress).balanceOf(_buyer);
            require(_balance >= _totalCost, "Insufficient balance");
            IERC20(address(_item.paymentToken) != address(0) ? _item.paymentToken : POWAddress)
                .transferFrom(_buyer, address(this), _totalCost);
            ITEM_BALANCES[itemId] += _totalCost;
        }

        uint _receiptId;
        if (_item.mode == ItemMode.RAFFLE) {
            if (ACCOUNT_RECEIPTS[_buyer][itemId].length() > 0) {
                _receiptId = ACCOUNT_RECEIPTS[_buyer][itemId].at(0);
                RECEIPTS[itemId][_receiptId].amount += amount;
                RECEIPTS[itemId][_receiptId].payment += _totalCost;
                RECEIPTS[itemId][_receiptId].updatedOn += _now;
            } else {
                _receiptId = _item.currentSupply + 1;
                RECEIPTS[itemId][_receiptId] = Receipt({
            ...
            for (uint _idx = 0; _idx < amount; _idx++) {
                _receiptId = _item.currentSupply + _idx + 1;
                RECEIPTS[itemId][_receiptId] = Receipt({
                    receiptId: _receiptId,
                    itemId: itemId,
                    account: _buyer,
