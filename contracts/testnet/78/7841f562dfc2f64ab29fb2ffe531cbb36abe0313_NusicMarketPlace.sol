...
contract NusicMarketPlace {
    ...
    // erc20_token_address => bool
    mapping(address => bool) public ERC20TokenList;
    mapping(address => uint256) public ERC20TokenAmount;

    mapping(uint256 => address) currentTokenOwners;
    mapping(uint256 => address) originalTokenOwners;
    ...
    function buyListing(uint256 listingId) public payable {
        ...
        address creator = originalTokenOwners[listing.tokenId]; // ALPHA

        require(creator != address(0), "ZERO ADDRESS");

        address currentOwner = currentTokenOwners[listing.tokenId]; //ALPHA
        uint256 askPrice = listing.askPrice;

        uint256 m = (askPrice * marketFee) / hundred;

        askPrice -= m;
        uint256 creatorFee = (askPrice * (listing.royalty * 10**18)) / hundred;
        askPrice -= creatorFee;
        ...
        // Currency is ERC20
            ...
            ERC20TokenAmount[address(listing.ERC20Token)] = m;
        ...
        currentTokenOwners[listing.tokenId] = msg.sender; // BETA
    ...
    function claimAuction(uint256 listingId) public payable {
        Listing memory listing = getListing[listingId];
        Bid memory bid = getBid[listingId];
        ...
        require(msg.value >= bid.bidAmount, "Insufficient funds");
    ...
    function setERC20Token(address _ERC20Token) public onlyOwner {
        ...
        ERC20TokenList[_ERC20Token] = true;
    }

    function withdrawERC20Token(address _ERC20Token) public onlyOwner {
        require(_ERC20Token != address(0), "Invalid Token");
        ERC20(_ERC20Token).transferFrom(
            address(this),
            owner,
            ERC20TokenAmount[_ERC20Token]
        );
    }
