...
contract SPRO is ERC20Detailed, Ownable {
    ...
    uint256 public MAX_SELL_LIMIT = 10;
    ...
    uint256 public biggestBuyerPeriod = 600;
    uint256 public immutable launchTime = block.timestamp;
    uint256 public biggestBuyerFeeAmount = 0;
    ...
    function _transferFrom(
        ...
        if (shouldSwapBack()) {
            ...
            // (uint autoBurnBusdAmount) = swapBack();
            // buyBack(autoBurnBusdAmount);
        }
    ...
    function takeFee(
        ...
        treasuryFeeCollect = treasuryFeeCollect.add(_treasuryFee);
        insuranceFeeCollect = insuranceFeeCollect.add(insuranceFundFee);
        firePitFeeCollect = firePitFeeCollect.add(firePitFee);

        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(insuranceFundFee).add(firePitFee))
        );        
        ...
        biggestBuyerFeeAmount = biggestBuyerFeeAmount.add(gonAmount.div(100));
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToSwapTreasuryAndSIF = amountToSwap.mul(treasuryFeeCollect.add(insuranceFeeCollect)).div(treasuryFeeCollect.add(insuranceFeeCollect).add(firePitFeeCollect));
        ...
        uint256 amountBUSDToTreasuryAndSIFAndFirepit = IERC20(busdToken).balanceOf(address(this)).sub(balanceBusdBefore);

        // send BUSD to treasury receier
        IERC20(busdToken).transferFrom(address(this), treasuryReceiver, amountBUSDToTreasuryAndSIFAndFirepit.mul(treasuryFeeCollect).div(treasuryFeeCollect.add(insuranceFeeCollect)));
        // send BUSD to insurance receiver
        IERC20(busdToken).transferFrom(address(this), insuranceFundReceiver, amountBUSDToTreasuryAndSIFAndFirepit.mul(insuranceFeeCollect).div(treasuryFeeCollect.add(insuranceFeeCollect)));
        
        // send autoburn amount to dead
        _transferFrom(address(this), firePit, amountToSwap.sub(amountToSwapTreasuryAndSIF));
        ...
        emit Transfer(
            address(this),
            firePit,
            amountToSwap.sub(amountToSwapTreasuryAndSIF)
        );
    }
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        return txForSwapback >= swapbackPeriod;
    }
    ...
    function setSellLimit(uint _limit) external onlyOwner {
        require(_limit <= MAX_SELL_LIMIT, "Exceed max 5%");
        sellLimit = _limit;
    }
    ...
    function _checkAndPayBiggestBuyer(uint256 _currentPeriod) private swapping {
        ...
        if (
            ...
            if ((biggestBuyerFeeAmount > 0) && (_gonBalances[autoLiquidityReceiver] > biggestBuyerFeeAmount)) {
                uint256 amountToSwap = biggestBuyerFeeAmount.div(_gonsPerFragment);
                if( amountToSwap == 0 ) {
                    return;
                }
                
                _gonBalances[autoLiquidityReceiver] = _gonBalances[autoLiquidityReceiver].sub(biggestBuyerFeeAmount);
                _gonBalances[address(this)] = _gonBalances[address(this)].add(biggestBuyerFeeAmount);

                uint256 balanceBefore = biggestBuyer[_prevPeriod].balance;
                _swapTokensForBNB(amountToSwap, biggestBuyer[_prevPeriod]);
                uint256 _rewardAmount = biggestBuyer[_prevPeriod].balance.sub(balanceBefore);

                totalBiggestBuyerPaid = totalBiggestBuyerPaid + _rewardAmount; 
                biggestBuyerPaid[_prevPeriod] = _rewardAmount;
                biggestBuyerFeeAmount = 0;
