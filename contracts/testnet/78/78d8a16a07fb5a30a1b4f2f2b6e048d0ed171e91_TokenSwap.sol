...
constructor(IBEP20 _first, uint256 _conversionMultiplier, uint256 _conversionDivisor, uint _durationInMinutes) public {
    // owner = msg.sender;
    firstToken = _first;
...
function swap(uint256 _value) public beforeDeadline isNotPause returns (bool) {
    require(pause == false, "Swap is paused");
...
function setPause(bool val) public onlyOwner returns (bool) {
    pause = val;
    return pause;
}

function checkIfPause()
    external
    view
    returns (bool){
        return pause;
