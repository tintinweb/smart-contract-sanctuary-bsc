...
contract RewardDistributor is IERC20 {
    ...
    string private constant _name = 'DFA Rewards';
    string private constant _symbol = 'DFAR';
    ...
    */
    ...
    function _sendReward(address user) internal {
        ...
        userInfo[user].totalClaimed[currentRewardToken] += pending;
        // send reward to user
        if (pending > 0) {
    ...
    function getTotalClaimed(address user, address token) external view returns (uint256) {
        return userInfo[user].totalClaimed[token];
    }

    function viewUserClaimsForAllRewardTokens(address user) external view returns (uint256[] memory) {
        uint len = allRewardTokens.length;
        uint256[] memory claims = new uint256[](len);
        for (uint i = 0; i < len;) {
            claims[i] = userInfo[user].totalClaimed[allRewardTokens[i]];
            unchecked { ++i; }
        }
        return claims;
    }

    function viewTotalRewardsForAllRewardTokens() external view returns (uint256[] memory) {
        uint len = allRewardTokens.length;
        uint256[] memory totalRewards = new uint256[](len);
        for (uint i = 0; i < len;) {
            totalRewards[i] = rewardToken[allRewardTokens[i]].totalRewards;
            unchecked { ++i; }
        }
        return totalRewards;
    }
