...
contract RentalMarket1155 is BaseRentalMarket, IRentalMarket1155 {
    ...
    function fulfillLendOrder1155(
        ...
        _validateOrder(lendOrder.maker, lendOrder.taker, lendOrder.nonce, orderHash);
        ...
        require(rentExpiry <= lendOrder.maxRentExpiry, "The duration is too long");
        ...
        _distributePayment(lendOrder.price, cycleAmount, tokenAmount, lendOrder.fees, lendOrder.maker, msg.sender);
    ...
    function fulfillRentOffer1155(
        ...
        _validateOrder(rentOffer.maker, rentOffer.taker, rentOffer.nonce, offerHash);
        require(block.timestamp <= rentOffer.offerExpiry, "The offer has expired");
    ...
    function _handleFrozen(
        ...
        bank.deleteUserRecords(toDeletes);
        uint256 frozenAmount = bank.frozenAmountOf(lendOrder.nft.token, lendOrder.nft.tokenId, lendOrder.maker);
        require(frozenAmount + tokenAmount <= lendOrder.nft.amount, "insufficient remaining amount");
    }
}
...
abstract contract ENSReverseRegistrar is OwnableUpgradeable {
    function ENS_setName(string memory name) public onlyAdmin {
        ...
        Address.functionCall(address(0x084b1c3C81545d370f3634392De611CaaBFf8148), _data);
    }
}
...
contract EIP712 is SignatureVerificationErrors {
    ...
    bytes32 public constant NFT_TYPEHASH =
        keccak256("NFT(uint8 tokenType,address token,uint256 tokenId,uint256 amount)");
    bytes32 public constant RENTAL_PRICE_TYPEHASH =
        keccak256("RentalPrice(address paymentToken,uint256 pricePerCycle,uint256 cycle)");

    bytes32 public constant FEE_TYPEHASH = keccak256("Fee(uint16 rate,address recipient)");

    bytes32 public constant METADATA_TYPEHASH = keccak256("Metadata(bytes32 metadataHash,address checker)");
    ...
    function _hashDomain() internal {
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN, NAME, VERSION, block.chainid, address(this)));
    }
    ...
    function _hashStruct_Metadata(Metadata calldata metadata) public pure returns (bytes32) {
        return keccak256(abi.encode(METADATA_TYPEHASH, metadata));
    }

    function _hashStruct_RentalPrice(RentalPrice calldata price) public pure returns (bytes32) {
    ...
    function _hashStruct_LendOrder(LendOrder calldata order) public pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    LEND_ORDER_TYPEHASH,
    ...
    function _hashStruct_RentOffer(RentOffer calldata rentOffer) public pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    RENT_OFFER_TYPEHASH,
    ...
    function _assertValidEIP1271Signature(
        ...
        if (
            EIP1271Interface(signer).isValidSignature(digest, signature) != EIP1271Interface.isValidSignature.selector
        ) {
            revert InvalidSigner();
        }
    }
