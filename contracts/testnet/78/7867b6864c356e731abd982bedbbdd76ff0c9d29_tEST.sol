...
// File: contracts/IminiCOREVault.sol

pragma solidity 0.6.12;

interface IminiCOREVault {
...
contract FeeApprover is OwnableUpgradeSafe {
    ...
    function initialize(
        address _miniCOREAddress,
        address _WETHAddress,
        address _uniswapFactory
    ) public initializer {
        OwnableUpgradeSafe.__Ownable_init();
        miniCORETokenAddress = _miniCOREAddress;
        WETHAddress = _WETHAddress;
        tokenUniswapPair = IUniswapV2Factory(_uniswapFactory).getPair(WETHAddress,miniCORETokenAddress);
        feePercentX100 = 11;
    ...
    address miniCORETokenAddress;
    address miniCOREVaultAddress;
    ...
    function setminiCOREVaultAddress(address _miniCOREVaultAddress) public onlyOwner {
        miniCOREVaultAddress = _miniCOREVaultAddress;
    }
    ...
    function calculateAmountsAfterFee(
        ...
        ) public  returns (uint256 transferToAmount, uint256 transferToFeeDistributorAmount)
        {
            ...
            if(sender == miniCOREVaultAddress  || sender == tokenUniswapPair ) { // Dont have a fee when miniCOREVault is sending, or infinite loop
...
contract NBUNIERC20 is INBUNIERC20, OwnableUpgradeSafe {
    ...
    function getSecondsLeftInLiquidityGenerationEvent() public view returns (uint256) {
        ...
        return contractStartTimestamp.add(11 days).sub(block.timestamp);
    }

    function liquidityGenerationOngoing() public view returns (bool) {
        console.log("liquidity generation ongoing", contractStartTimestamp.add(11 days) < block.timestamp);
        return contractStartTimestamp.add(11 days) > block.timestamp;
    }
    ...
    function _transfer(
        ...
        if(transferToFeeDistributorAmount > 0 && feeDistributor != address(0)){
            ...
            if(feeDistributor != address(0)){
                IminiCOREVault(feeDistributor).addPendingRewards(transferToFeeDistributorAmount);
            }
        }
    }
...
contract tEST is NBUNIERC20 {
    ...
    modifier canMint() {
        require(!_mintingFinished, "TCORE: minting is finished");
        _;
    }
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "miniCORE::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "miniCORE::delegateBySig: invalid nonce");
        require(now <= expiry, "miniCORE::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "miniCORE::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying miniCORE tokens (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "miniCORE::_writeCheckpoint: block number exceeds 32 bits");
