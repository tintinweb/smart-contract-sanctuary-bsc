...
contract Presale is Ownable, Pausable, ReentrancyGuard {
    ...
    bool private checkReindex = false;
...
//        isApprove = true;
    ...
    function waitingListUser() external view returns(address[] memory) {
        address[] memory result = new address[](waitingList.length - base);
        for(uint i = base; i < waitingList.length; i ++) {
            result[i - base] =  waitingList[i];
        }
        return result;
    }
