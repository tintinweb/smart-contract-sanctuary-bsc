...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	function _updateEpoch() internal
	{
		...
		uint256 _totalActiveBalance = totalActiveBalance;
		...
		while (_lastWeek > lastWeek) {
			...
			uint256 _epochs = (_nextWeek - lastEpoch) / EPOCH_DURATION;
			{
				...
				totalActiveBalance = totalActiveBalance * _rate / 100e16;
			...
			rebaseRatePerEpoch[weekIndex] = nextRebaseRatePerEpoch;
			{
				...
				totalShares += totalActiveBalance;
			}
		}
		...
		// allocate new supply
		uint256 _newSupply = totalActiveBalance - _totalActiveBalance;
		if (_newSupply > 0) {
			bypass_ = true;
