...
contract Anito is ERC721, Pausable, Ownable, ERC721Enumerable, ERC721Burnable, ReentrancyGuard {
    ...
    event AnitoSummoned(address indexed player, uint256 indexed tokenId);
    ...
    constructor(address _launchpadAddress) ERC721("Anito", "ANITO") {
        operationsAddresses[msg.sender] = true;
        maxSummonCount = 7;
        revealed = false;
        /**
        * Launchpad Address
        * Testnet: 0x9642617Bbbd552FA363e04CD082703Be403fC25B
        */
        LAUNCHPAD = _launchpadAddress;
        LAUNCH_MAX_SUPPLY = 10000;
    }
    ...
    function setMaxSummonCount(uint256 _maxSummonCount) public onlyOperators {
        maxSummonCount = _maxSummonCount;
    }

    function safeMintInternal(address to, uint256 tokenId, bool summoning) internal {
        _safeMint(to, tokenId);
        if(summoning){
            emit AnitoSummoned(to, tokenId);
        }
    }
    ...
    function safeMintSingle(address player) external onlyOperators returns(uint256) {
        ...
        safeMintInternal(player, tokenId, false);
        return tokenId;
    }
    ...
    function executeSummoning(address player, uint256 master1, uint256 master2) external onlyOperators {
        ...
        anitoInfo[master1].summonCount++;
        anitoInfo[master2].summonCount++;

        summons[master1].push(tokenId);
        summons[master2].push(tokenId);

        safeMintInternal(player, tokenId, true);
    }

    function getAnitoOfAddress(address player)
        public
        view
        returns (uint256[] memory, AnitoInfo[] memory)
    {
    ...
    function mintTo(address to, uint size) external onlyLaunchpad {
        ...
        for (uint256 i=1; i <= size; i++) {
            _tokenIdCounter.increment();
            uint256 tokenId = _tokenIdCounter.current();
            safeMintInternal(to, tokenId, false);
            LAUNCH_SUPPLY++;
        }
    }
}
