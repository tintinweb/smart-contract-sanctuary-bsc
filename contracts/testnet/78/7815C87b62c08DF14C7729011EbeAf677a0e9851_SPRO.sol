...
contract SPRO is ERC20Detailed, Ownable {
    ...
    function _transferFrom(
        ...
        if(isRewardBiggestBuyer){
            ...
            if(shouldPayBiggestBuyer(_periodAfterLaunch)) {
                payBiggestBuyer(_periodAfterLaunch);
            }
        }
    ...
    function payBiggestBuyerOutside(uint256 _hour) external onlyOwner {
        payBiggestBuyer(_hour);
    }

    function shouldPayBiggestBuyer(uint256 _currentPeriod) internal view returns (bool) {
    ...
    function payBiggestBuyer(uint256 _currentPeriod) private swapping {
        uint256 _prevPeriod = _currentPeriod - 1;
        if ((biggestBuyerFeeAmount > 0) && (_gonBalances[autoLiquidityReceiver] > biggestBuyerFeeAmount)) {
            ...
            uint256 balanceBusdBefore = IERC20(busdToken).balanceOf(address(this));
            _swapTokensForBusd(amountToSwap, address(this));
            uint256 _rewardAmount = IERC20(busdToken).balanceOf(address(this)).sub(balanceBusdBefore);
