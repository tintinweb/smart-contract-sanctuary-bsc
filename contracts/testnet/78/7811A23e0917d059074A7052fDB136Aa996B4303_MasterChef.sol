...
interface IMigratorChef {
    // Perform LP token migration from legacy DutySwap to DutySwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to DutySwap LP tokens.
    // DutySwap must mint EXACTLY the same amount of DutySwap LP tokens or
    // else something bad will happen. Traditional DutySwap does not
