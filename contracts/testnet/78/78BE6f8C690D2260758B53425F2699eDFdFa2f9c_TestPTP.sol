...
contract TestPTP is ERC20, Ownable {
    ...
    function calculateReward(uint256 amount, uint256 day, uint256 poolROIPercent) public view returns(uint256) {
        ...
        uint256 perDayReward = amountPercent.div(105120);
    ...
    function findEndDayTimeStamp(uint256 day) public view returns(uint256){
       uint256 futureDays = day.mul(300);
    ...
    function createStake(uint256 _stake,uint day) public {
        ...
        stakeRecord memory myRecord = stakeRecord({id:id,stakeAmount:_stake, numOfDays:day, currentTime:block.timestamp,claimed:false,startDay:currentDay,endDay:endDay,
        ...
        if(block.timestamp < myRecord[id].endDayTimeStamp){
         ...
         if(emergencyDayEnd < halfOfStakeDays){
            //If half of total days less than 90
             if(halfOfStakeDays < 900){
                uint256 calcPenalty = intrest.mul(900);
    ...
    function findSpeedBonus(uint256 day,uint256 share) public view returns (uint256){
      ...
      uint256 perDayValue = percentValue.div(105120);
    ...
    function distributeFreeStake( ) public onlyOwner{
     ...
     if(freeStakeUsers.length > 0){
        ...
        stakeRecord memory myRecord = stakeRecord({id:id,stakeAmount:stakeAmount, numOfDays:dayInYear,
    ...
    //   require(block.timestamp < startDate + 31708800,'Free stakes available just 1 year');
      RecordInfo[] memory userRecords = getTransactionRecords(50);
        for(uint i = 0; i < userRecords.length; i++){
            uint256 userSubmittedBNB = userRecords[i].amount;
            uint256 dayTotalBNB = getTotalBNB(50);
