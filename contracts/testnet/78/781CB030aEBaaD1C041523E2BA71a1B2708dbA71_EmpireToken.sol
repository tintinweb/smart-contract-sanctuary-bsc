...
contract EmpireToken is IERC20, Ownable {
  ...
  event LogSetLiquidityWallet(address indexed setter, address liquidityWallet);
  ...
  event LogTransferByBridge(address indexed from, address indexed to, uint256 tAmount);
  ...
  function sendToBurn(uint256 tBurn) private {
    ...
    if (_isExcluded[address(this)]) _tOwned[address(this)] = _tOwned[address(this)] - tBurn;
  ...
  function _takeFee(
    ...
    if (shouldTakeFee == true) {
      ...
      emit Transfer(sender, address(this), tLiquidity + tMarketing + tBurn + tTeam);
    }
  }
  ...
  function transferByBridge(
    ...
    _approve(from, _msgSender(), balanceCheck(_allowances[from][_msgSender()], tAmount, "ERC20: transfer amount exceeds allowance"));
