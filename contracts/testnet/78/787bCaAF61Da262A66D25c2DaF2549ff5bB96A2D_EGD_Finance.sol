...
contract EGD_Finance is OwnableUpgradeable {
    ...
    function _processRefer(address addr, uint amount) internal {
        uint stamp = 86400 - (block.timestamp % 86400);
        ...
        for (uint i = 0; i < 10; i ++) {
            ...
            if (userInfo[temp].refer >= i + 1 || userInfo[temp].isRefer) {
                uint last = EGD.balanceOf(temp);
                router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount * referRate[i] / 20, 0, path, temp, block.timestamp + 7200000);
                uint news = EGD.balanceOf(temp);
                userInfo[temp].referReward += news - last;
                userDailyReferReward[temp][stamp] += news - last;
                left -= amount * referRate[i] / 20;
            }
    ...
    function getUserReferReward(address addr) public view returns(uint,uint){
        uint stamp = 86400 - (block.timestamp % 86400);
        return(userInfo[addr].referReward,userDailyReferReward[addr][stamp]);
    }
    ...
    function stake(uint amount) external {
        ...
        userSlot[msg.sender][stakeId].rates = amount * 6 / 1000 / 86400;
    ...
    function calculateReward(address addr, uint slot) public view returns (uint){
        ...
        return (totalRew * 1e18 / getEGDPrice());
    }
