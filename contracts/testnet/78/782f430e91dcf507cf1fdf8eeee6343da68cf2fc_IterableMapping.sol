...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "$EUS: The router already has that address");
...
contract EARNUSDT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _operationToSwap;
    ...
    constructor() ERC20("EARNUSDT", "$EUS", 18) {
        ...
        buyInvestorFee = 0;
        buyMarketingFee = 3;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyOperationsFee + buyBuybackFee + buyInvestorFee + buyMarketingFee; // 12%
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
  	...
  	function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
  	  require(isContract(ammAddress), "$EUS: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "$EUS: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "$EUS: The new dividend tracker must be owned by the $EUS token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "$EUS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "$EUS: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "$EUS: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "$EUS: The operations wallet is already this address");
    ...
    function updateMarketingkWallet(address newWallet) external onlyOwner {
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$EUS: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$EUS: Cannot update gasForProcessing to same value");
    ...
    // @dev User Callable Functions start here! ---------------------------------------------
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$EUS: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "$EUS: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$EUS: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "$EUS: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "$EUS: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "$EUS: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "$EUS: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "$EUS: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "$EUS: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$EUS: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if(takeFee) {
        	...
        	if(automatedMarketMakerPairs[from]) {
                ...
                _operationToSwap += fees * buyMarketingFee / buyTotalFees;
            }
            ...
            if(automatedMarketMakerPairs[to]) {
                ...
                _operationToSwap += fees * sellMarketingFee / sellTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _buyBackTokensToSwap + _marketingTokensToSwap + _InvestorTokensToSwap + _rewardTokensToSwap + _operationToSwap;
        ...
        uint256 bnbForTeam = bnbBalance.mul(_operationToSwap).div(totalTokensToSwap);
        ...
        (success,) = address(marketingWallet).call{value: bnbForTeam}("");
