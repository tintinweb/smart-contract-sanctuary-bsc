...
//   address private _reflectingTokenAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // Mainnet - BUSD Token
    ...
    uint256 public minPeriod = 2 hours; // min 2 hour delay
...
contract TestGenesis is IBEP20, Ownable {
    
    using SafeMath for uint256;

    string _name = "TestGenesis";
    string _symbol = "<Tgen>";
    uint8 constant _decimals = 9;
    uint256 constant _totalSupply = 10 ** 9 * (10 ** _decimals); // 1B tokens
    uint256 constant _maxHold = _totalSupply / 50; // 2% max wallet
    uint256 constant _feeDenominator = 1000;
    uint256 public liquidityFee = 50;
    uint256 public marketingFee = 30;
    uint256 public reflectionFee = 70;
    uint256 public totalFee = liquidityFee + reflectionFee + marketingFee; // Reflections fee - 7%, Liquidity fee - 5% , Marketing Fee - 3% 
    ...
    uint256 launchedAt;
    uint256 private _swapThreshold = _totalSupply / 200; // 0.5% - This will be gas heavy at launch but be much less so over time
...
//	address constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c; // Mainnet - WBNB Token
    ...
    address constant _marketingFeeReceiver = 0x3256C7f7e27eea2B3f6d48438a2b89D03FE3Fddb;
    ...
    function swapBack() internal swapping {
        ...
        )
        {
            ...
            (bool marketSuccess, ) = payable(_marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
    ...
    function clearDistributor() public onlyDev {
        try distributor.process(distributorGas) {} catch {}
    }
    ...
    function setFeesOnNormalTransfers(bool _enabled) public onlyDev() {
        feesOnNormalTransfers = _enabled;
    }

    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee) public onlyDev(){
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        marketingFee = _marketingFee;
        totalFee = _liquidityFee.add(_reflectionFee).add(_marketingFee);
         require (totalFee <= 150 , "requires total tax less than or equal to 150" ); //hardcodes max tax
    }
//preserved if liquidity fee needs to be sent elsewhere to create more liquidity pairs
    function setLiqFeeReceiver(address _autoLiquidityReceiver) public onlyDev() { 
        autoLiquidityReceiver = _autoLiquidityReceiver;
    }

     function setisFeeExempt(address holder, bool exempt) external onlyDev {
    isFeeExempt[holder] = exempt;
    }
    function setisDividendExempt(address holder, bool exempt) external onlyDev {
