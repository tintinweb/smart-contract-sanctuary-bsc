...
contract TokenMigration is ReentrancyGuard, Context, Ownable {
    ...
    function withdrawTokens() external nonReentrant {
        ...
        if (claims < lockDays.length) {
            uint256 activeTime = user.migrateStartTime + lockDays[claims];
            require(
                block.timestamp >= activeTime &&
                    block.timestamp <= activeTime + 86400,
    ...
    function migrate() external nonReentrant returns (uint256) {
        ...
        console.log("--- migrate function => userV1Balance: ", userV1Balance);

        require(userV1Balance > 0, "The balance is greater than zero");
        uint256 amtToMigrate = userV1Balance / 10;
        console.log("--- migrate function => amtToMigrate: ", amtToMigrate);
    ...
    function getInfoPeriod(address _addr) external view returns (uint256) {
        UserInfo storage user = userInfo[_addr];
        uint256 nextDay = lockDays[user.claimedIndex + 1];
        uint256 currenctDay = block.timestamp - (block.timestamp % 86400);
        uint256 duration = currenctDay - user.migrateStartTime;
        console.log("duration : ", nextDay - duration);
        return nextDay - duration;
    }
