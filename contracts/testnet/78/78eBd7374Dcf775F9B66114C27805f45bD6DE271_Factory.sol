...
import './sc_official/MysteryBox.sol';
import './sc_official/NFT721.sol';
import './sc_official/Treasury.sol';
...
{
    // get presale SC
    mapping(address => mapping(string => address)) public getContract;
    address internal constant LFW_TOKEN = 0x5a5A4F6E025F7896c7801b0b378495a513f49cD2;
    
    event SubmitNFT(
        address contractAddress,
        string name,
        string symbol,
        address whitelistMinter,
        address boxMinter,
        uint256 total,
        string baseUri
    );

    event SubmitMysteryBox(
        address contractAddress,
        address treasuryWallet, 
        bool isUsedBNB,
        address tokenAddress,
        uint256 numberOfBox,
        uint256[] priceEachBox
    );

    event SubmitTreasury(
        address contractAddress,
    ...
    function submitNFT(
        ...
        bytes memory _bytecode = type(NFT721).creationCode;
        ...
        // random salt
        bytes32 salt = keccak256(abi.encodePacked(LFW_TOKEN, msg.sender));

        // pair is the NFT contract address
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        // function to get presale SC address for user
        getContract[msg.sender]["NFT"] = pair;

        emit SubmitNFT(pair, _name, _symbol, _whitelistMinter, _boxMinter, _total, _baseUri);
    }



    function submitMysteryBox(
        ...
        bytes memory _bytecode = type(MysteryBox).creationCode;
        bytes memory bytecode = abi.encodePacked(
            _bytecode, 
            abi.encode(
                _treasuryWallet, 
        ...
        // function to get presale SC address for user
        getContract[msg.sender]["MysteryBox"] = pair;

        emit SubmitMysteryBox(pair, _treasuryWallet, _isUsedBNB, _tokenAddress, _numberOfBox, _priceEachBox);
    }

    function submitTreasury(
        address _tokenAddress,
        address _treasuryAddress,
        uint256 _conversionRate        
    ) external returns (address pair) {
        bytes memory _bytecode = type(Treasury).creationCode;
        bytes memory bytecode = abi.encodePacked(
            _bytecode, 
            abi.encode(
                _tokenAddress, 
        ...
        // function to get presale SC address for user
        getContract[msg.sender]["Treasury"] = pair;

        emit SubmitTreasury(pair, _tokenAddress, _conversionRate);
    }
}
    ...
    ReentrancyGuard
{
    ...
    {
        require(address(_treasuryWallet) != address(0), "Invalid treasury wallet address");
    ...
    /**
     * @notice set new treasury wallet
     * @dev only call by owner
    ...
    function setBasePrice(uint256 _boxNumber, uint256 _price) public onlyOwner{
        require(_boxNumber != 0 && _price !=0, "Treasury: Parameters must not be 0");
    ...
    function mintNFT(uint256 _boxNumber) external payable nonReentrant {
        ...
        uint256 NFTId = NFTIds[_boxNumber][randomSlot];
        ...
        emit ClaimNFT(_receipient, NFTId);
    }
    ...
    ERC721Enumerable
{
    ...
    {
        whitelistedAddress[_whitelistMinter] = true;
        WhitelistBoxMinter memory newBox = WhitelistBoxMinter(
            _total,
            0
        );
        whitelistBoxMinters[_boxMinter] = newBox;
        baseUri = _baseUri;
    }


    function mintWhitelist(uint256 tokenId) public {
        require(whitelistedAddress[msg.sender], "You are not whitelisted");
     ...
     */
    function whitelist(address _minter) public onlyOwner {
        whitelistedAddress[_minter] = true;
    }
    ...
    ReentrancyGuard
{
    ...
    function depositNFT(
        ...
        IERC721 nft = getNftContract(_nftAddress);
    ...
    function withdrawNFT(
        ...
        getNftContract(_nftAddress).safeTransferFrom(
            address(this),
            msg.sender,
            asset.tokenId
        );
    ...
    //@dev swap token to gem, transfer token to treasury wallet
    function swapGem(uint256 amount, uint256 serverId) external nonReentrant {
        require(amount > 0, "SZ");
        uint256 allowance = tokenAddress.allowance(
            address(msg.sender),
            address(this)
        );
        require(allowance >= amount, "Check the token allowance");

        //TODO: safeTransferFrom only for IERC20Upgradeable, we should change token to IERC20Upgradeable
        require(
            tokenAddress.transferFrom(msg.sender, treasuryWallet, amount) ==
                true,
            "Transfer failed"
        );
        // game will handle gem.div(conversionRate)
