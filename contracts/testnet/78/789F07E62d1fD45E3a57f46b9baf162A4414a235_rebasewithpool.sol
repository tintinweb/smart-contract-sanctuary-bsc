...
//////////////////////////////////////////////////////////////////////////////////////////
    ...
    function getNftEarnedRewards(uint256 tokenid) public view returns(uint256 rewards){
        return calculateNftRewards(tokenid);
    }
    ...
    function claimNFTRewards(uint256 tokenId) public {
        ...
        uint256 rewards = 1;
        rewards = calculateNftRewards(nftinv.tokenId);
        if(rewards >0)
        {
            uint256 dingValue = rewards.mul(_dingsPerFragment);
            _transfer(address(this),msg.sender,dingValue);
    ...
    function calculateNftRewards(uint256 tokenId) private view returns(uint256 rewards){
        bytes32 uniqueKey = getNftUniqueKey(msg.sender,tokenId);
        nftInvestment memory nftinv = getnftInvestment(uniqueKey);
        uint256 timenow = block.timestamp;
