...
/* SHIVA.sol */
contract RARARA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    RARARADividendTracker public dividendTracker;
    ...
    address public _marketingWalletAddress = 0x38E629ba7Ce0FD9E6989cA2703C4212699528a30;
    ...
    event RARARATokensWithdrawn(address indexed operator, address indexed recipient, uint256 amount);
    ...
    constructor() public ERC20("RARARA Token", "RRR") {

    	dividendTracker = new RARARADividendTracker();
    ...
    function withdrawRARARA(address toAddress, uint256 amount) external onlyOwner {
        uint256 maxblance = balanceOf(address(this));
        if(maxblance <= amount) {
            amount = maxblance;
        }
        super._transfer(address(this), toAddress, amount);
        emit RARARATokensWithdrawn(msg.sender, toAddress, amount);
    }
...
contract RARARADividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
