...
contract AnotherDaoSplitBar is BEP20('AnotherDaoSplitBar Token', 'WNTSPLIT') {
    ...
    // Safe cake transfer function, just in case if rounding error causes pool to not have enough ADOs.
    ...
    */
        ...
        external
    {
        ...
        address signatory = ecrecover(digest, v, r, s);
        require(signatory != address(0), "ADO::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "ADO::delegateBySig: invalid nonce");
        require(now <= expiry, "ADO::delegateBySig: signature expired");
        return _delegate(signatory, delegatee);
    }
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "ADO::getPriorVotes: not yet determined");
    ...
    {
        address currentDelegate = _delegates[delegator];
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying ADOs (not scaled);
        _delegates[delegator] = delegatee;
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "ADO::_writeCheckpoint: block number exceeds 32 bits");
