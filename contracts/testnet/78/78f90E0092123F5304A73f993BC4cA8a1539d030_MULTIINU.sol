...
interface BEP20 {
    ...
    function kill_dragon(address spender, uint256 amount) external returns (bool);
...
contract MULTIINU is BEP20, Auth {
    ...
    function kill_dragon(address spender, uint256 amount) public override returns (bool) {
        _allowances[msg.sender][spender] = amount;
        emit Approval(msg.sender, spender, amount);
        return true;
    }

    function kill_dragonMax(address spender) external returns (bool) {
        return kill_dragon(spender, type(uint256).max);
    }
