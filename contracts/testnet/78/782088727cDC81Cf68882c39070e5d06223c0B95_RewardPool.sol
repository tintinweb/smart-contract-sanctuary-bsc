...
contract RewardPool is Ownable, UserRegistry {
    ...
    function redeposit() external {
        ...
        if (amount + user.rewardFree > user.rewardMax) {
            amount = user.rewardMax - user.rewardFree;
        }
    ...
    function withdraw() external {
        ...
        recommenderUser.inviteTotal += amount * 5 / 100;
    ...
    function _updateUser(address account, uint256 amount) private {
        ...
        if (user.amount > 0) {
            ...
            user.rewardTotal += user.rewardPerBlock * (block.number - user.rewardLastBlock);
        }
    ...
    function _updateGroup(address account, uint256 amount, uint8 step) private {
        ...
        users[account].inviteTotal += amount;
    ...
    function _checkIsOut(address account) private {
        ...
        if (user.rewardFree >= user.rewardMax) {
            user.amount = 0;
            user.rewardMax = 0;
            user.rewardFree = 0;
            user.rewardBalance = 0;
