...
contract JewelToken is ERC20, Ownable, Authorizable {
    ...
    // fee
    uint64 public IN_FEE_PERCENTAGE = 4;
    uint64 public OUT_FEE_PERCENTAGE = 8;
    mapping(address => bool) public feeWhiteList;
    mapping(address => bool) public feeFreeList;
    ...
    onlyProfile(sender, recipient) {
        ...
        if(!feeFreeList[sender] && !feeFreeList[recipient]) {
            if(!feeWhiteList[sender]) {
                fee = fee.add(amount.mul(OUT_FEE_PERCENTAGE).div(100));
            }
            if(!feeWhiteList[recipient]) {
                fee = fee.add(amount.mul(IN_FEE_PERCENTAGE).div(100));
            }
            if (fee > 0) {
    ...
    function addFeeWhiteList(address _toAdd) public onlyAuthorized {
        feeWhiteList[_toAdd] = true;
    }

    function removeFeeWhiteList(address _toRemove) public onlyAuthorized {
        feeWhiteList[_toRemove] = false;
    }

    function addFeeFreeList(address _toAdd) public onlyAuthorized {
        feeFreeList[_toAdd] = true;
    }

    function removeFeeFreeList(address _toRemove) public onlyAuthorized {
        feeFreeList[_toRemove] = false;
    }
}
...
contract MasterGardener is Ownable, Authorizable, ReentrancyGuard {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    ...
                    lockAmount = pending.mul(lockPercentage).div(1000);
    ...
    function deposit(uint256 _pid, uint256 _amount, address _ref) public {
        depositFor(msg.sender, _pid, _amount, _ref);
    }

    // Deposit LP tokens to MasterGardener for JEWEL allocation.
    function depositFor(address _receiver, uint256 _pid, uint256 _amount, address _ref) public nonReentrant {
        require(
            _amount > 0,
        ...
        UserGlobalInfo storage current = userGlobalInfo[_receiver];

        if (refer.referrals[_receiver] > 0) {
            refer.referrals[_receiver] = refer.referrals[_receiver] + _amount;
    ...
    function withdrawFee(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (
            ...
            return uint256(10000).sub(userFeeStage[6].mul(10));
    ...
    function BPR(uint256 _pid) public view returns (uint256) {
        ...
        uint256 multiplier = getMultiplier(block.number - 1, block.number);
        return multiplier.mul(REWARD_PER_BLOCK).mul(pool.allocPoint).div(totalAllocPoint).div(lpSupply);
    }
