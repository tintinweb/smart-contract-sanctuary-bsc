...
// SPDX-License-Identifier: MIT
// Author: Megacy PTE.LTD
pragma solidity ^0.8.4;

interface IMegacyNFT {
...
interface IMegacyNFTFactory is IERC721Enumerable {
    ...
    function isMegacyNFT(address _nft) external view returns (bool);
}
...
import "./interfaces/IMegacyNFT.sol";
import "./interfaces/IMegacyNFTFactory.sol";
import "./MCYMarketplaceFee.sol";
import "./MCYMarketplacePaymentTokens.sol";

// Create auction, 
// Bid place,

abstract contract MCYMarketplaceAuctions is Ownable, MCYMarketplaceFee, MCYMarketplacePaymentTokens {
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        IMegacyNFT kuiperNft = IMegacyNFT(_nft);
        ...
        payToken.transfer(MCYMarketplaceFee.platformRecipient, platformFeeTotal);
...
contract MCYMarketplaceFee is Ownable {
    ...
    function changePlatformRecipient(address _platformRecipient) external onlyOwner {
        require(_platformRecipient != address(0), "can't be 0 address");
        platformRecipient = _platformRecipient;
    }
...
abstract contract MCYMarketplacePaymentTokens is Ownable {
...
import "./MCYMarketplaceAuctions.sol";
import "./interfaces/IMegacyNFT.sol";
// import "./interfaces/IMegacyNFTFactory.sol";
import "./MCYMarketplaceFee.sol";

/* Megacy NFT Marketplace
    List NFT, 
    Buy NFT, 
    Offer NFT, 
    Accept offer, 
    & support Royalty
*/
contract MegacyNFTMarketplace is Ownable, ReentrancyGuard, MCYMarketplaceFee, MCYMarketplacePaymentTokens, IERC721Receiver {
    using SafeERC20 for IERC20;

    // IMegacyNFTFactory private immutable megacyNFTFactory;
    ...
    mapping(address => mapping(uint256 => ListNFT)) private listingNFTs;
    // ListNFT[] public allNFTOnMarket;
    // mapping(ListNFT => uint) private nftsOnMarketIndexes;
    // address[][] public addressListNFTs;
    ...
    constructor(
        // IMegacyNFTFactory _megacyNFTFactory
    ) 
        MCYMarketplaceFee(msg.sender)
    {
        // megacyNFTFactory = _megacyNFTFactory;
    }
    ...
    // modifier isMegacyNFT(address _nft) {
    //     require(megacyNFTFactory.isMegacyNFT(_nft), "not megacy NFT");
    //     _;
    // }

    modifier isListedNFT(address _nft, uint256 _tokenId) {
        ListNFT memory listedNFT = listingNFTs[_nft][_tokenId];
        require(
    ...
    function withdrawNFT(address _nft, uint256 _tokenId, address _to)
        public onlyOwner
    {
        IERC721Enumerable nft = IERC721Enumerable(_nft);
        require(nft.ownerOf(_tokenId) == msg.sender, "not nft owner");
    ...
    function listNft(
        ...
        //isMegacyNFT(_nft) //isPayableToken(_payToken) 
        IERC721Enumerable nft = IERC721Enumerable(_nft);
        require(nft.ownerOf(_tokenId) == msg.sender, "not nft owner");
        ...
        listingNFTs[_nft][_tokenId] = ListNFT({
            nft: _nft,
            tokenId: _tokenId,
            seller: msg.sender,
        ...
        emit ListedNFT(_nft, _tokenId, msg.sender, address(this), _price);
    }
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IERC721Enumerable(_nft).safeTransferFrom(address(this), msg.sender, _tokenId);
        // int index = this.findElementInArray(listingNFTs[_nft][_tokenId], this.allNFTOnMarket);
        delete listingNFTs[_nft][_tokenId];
        // if(index >= 0) delete allNFTOnMarket[index];
    }
    ...
    function buyNFT(
        ...
        address _payToken,
        uint256 _price
    ) external payable nonReentrant isListedNFT(_nft, _tokenId) {
        ListNFT storage listedNft = listingNFTs[_nft][_tokenId];
        require(
            listedNft.seller != address(0) && !listedNft.sold,
            "not listed"
        );
        require(!listedNft.sold, "nft already sold");
        require(_price >= (listedNft.price + calculatePlatformFee(listedNft.price)), "invalid price");
        ...
        if(_payToken == address(0)) {
            // address _previousOwner = listedNft.seller;
            // address _newOwner = msg.sender;
            // uint256 platformFeeTotal = calculatePlatformFee(listedNft.price);
            // address payable buyer = payable(msg.sender);
            address payable owner = payable(listedNft.seller);
            // uint256 totalPrice = msg.value;
            owner.transfer(listedNft.price);
            payable(platformRecipient).transfer(totalPrice-listedNft.price);
        ...
        // IMegacyNFT nft = IMegacyNFT(listedNft.nft);
        ...
        IERC721Enumerable(listedNft.nft).safeTransferFrom(
            address(this),
            msg.sender,
            listedNft.tokenId
        );
    ...
    function isListingNFT(address _nft, uint256 _tokenId) public view returns (bool) {
        if(listingNFTs[_nft][_tokenId].seller != address(0) && !listingNFTs[_nft][_tokenId].sold) {
            return true;
        }
        return false;
    }
    ...
    // function fetchMarketTokens() public view returns(ListNFT[] memory){
    ...
    //     ListNFT[] memory items = new ListNFT[](unsoldItemCount);
    //     for(uint i = 0; i < itemCount; i++){
    //         if(idToMarketToken[i + 1].owner == address(0)){
    //             uint currentId = i + 1;
    //             ListNFT storage currentItem = idToMarketToken[currentId];
    ...
    // function fetchMarketNFTs() public view returns(ListNFT[] memory){
    //     uint currentIndex = 0;
    //     // looping over the number of items createdd (if number has not been sold populate the array)
    //     ListNFT[] memory items = new ListNFT[]();
    //     for(uint i = 0; i < listingNFTs.co; i++){
    //         if(idToMarketToken[i + 1].owner == address(0)){
    //             uint currentId = i + 1;
