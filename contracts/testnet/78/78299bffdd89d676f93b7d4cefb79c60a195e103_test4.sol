...
contract test4 {
	...
	uint256 constant public SOW_MIN_AMOUNT = 1 wei;
	uint256[] public REFERRAL_PERCENTS = [40, 20, 10, 7, 3, 1];
	...
	event NewSow(address indexed user, uint16 plan, uint256 amount);
	...
	constructor(address payable wallet) {
        ...
        plans.push(Plan(80, 40));
        plans.push(Plan(365, 30));
      
	}

	function sow(uint16 plan, address referrer) public payable {
		if (!started) {
	...
	function getPlanInfo() public view returns(uint16 plan, uint256 time, uint256 percent) {
	...
	function getUserSowsInfo(address userAddress) public view returns(uint16 plan, uint256 percent, uint256 amount, uint256 start, uint256 finish) {
	    uint256 index = 0;
	    User storage user = users[userAddress];
