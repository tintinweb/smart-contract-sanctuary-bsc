...
import "../Utils/IPromote.sol";
import "../Utils/IPromote_2_0.sol";
...
interface IMasterChef {
    function userStakeTokenId(address _address) external returns(uint256);
}

contract NFT_MasterChef_2_0 is Initializable, OwnableUpgradeable, Member {
    ...
    //     625*10000 // 第 6 階
    // ]
    uint256[] public PowerLowerLimit;
    ...
    //     750*10000-1 // 第 6 階
    // ]
    uint256[] public PowerUpperLimit;

    // 每階對應的每區塊獎勵
    // [
    //     432*1e18 / uint256(28800), // 初階
    //     3000*1e18 / uint256(28800), // 第 1 階
    //     6000*1e18 / uint256(28800), // 第 2 階
    //     5000*1e18 / uint256(28800), // 第 3 階
    //     4000*1e18 / uint256(28800), // 第 4 階
    //     3000*1e18 / uint256(28800), // 第 5 階
    //     2000*1e18 / uint256(28800) // 第 6 階
    // ]
    uint256[] public IntervalMining;
    ...
    // 滿產後獎勵
    // 1000*1e18 / uint256(28800)
    uint256 public MAX_REWARD_MINING;

    uint256[4] public rewardPercentage;
    ...
    function init() internal initializer {
        ...
        // 1-3級佔比35%（含盲盒）
        rewardPercentage[0] = 35;
        // 4-6級佔比15%
        rewardPercentage[1] = 15;
        // 7-9級佔比25%
        rewardPercentage[2] = 25;
        // 10級佔比25%
        rewardPercentage[3] = 25;
        ...
        // 750万 為滿產
        MAX_REWARD_WEIGHT = 750 * 10000;

        // 滿產後獎勵
        MAX_REWARD_MINING = 1000*1e18 / uint256(28800);
    }

    function changeNewRewardParam(
        uint256[] memory _PowerLowerLimit,
        uint256[] memory _PowerUpperLimit,
        uint256[] memory _IntervalMining,
        uint256 _MAX_REWARD_WEIGHT,
        uint256 _MAX_REWARD_MINING
    ) external {
        require(msg.sender == manager.members("owner"), "onlyOwner");
         PowerLowerLimit = _PowerLowerLimit;
        PowerUpperLimit = _PowerUpperLimit;
        // 每階對應的每區塊獎勵
        IntervalMining = _IntervalMining;

        // 750万 為滿產
        MAX_REWARD_WEIGHT = _MAX_REWARD_WEIGHT;

        // 滿產後獎勵
        MAX_REWARD_MINING = _MAX_REWARD_MINING;
    }
    ...
    function getBalances(address _user) external view returns (uint256[] memory) {
        return userInfo[_user]._nftBalances;
    }
    
    function balanceOfByIdex(address account) public view returns(uint256){
        return userInfo[account]._nftBalances[0];
    }
    ...
    function pendingSushi(address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        uint256 tokenId = userStakeTokenId[_user];
        if(tokenId == 0){
            return user.pendingReward;
        }
        uint8 _pid = getPidByTokenId(tokenId);
        PoolInfo storage pool = poolInfo[_pid];

        uint256 lpSupply = pool.lpSupply;
        uint256 accSushiPerShare = pool.accSushiPerShare;
        
        if ((totoalPower < minimalPower && accSushiPerShare == 0) || totoalPower == 0 || block.number < startBlock) {
            return 0;
        }

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 sushiReward = multiplier * sushiPerBlock * 100 / rewardPercentage[_pid];
    ...
    function updatePool(uint8 _pid) public {
        ...
        uint256 lpSupply = pool.lpSupply;
        if ((totoalPower < minimalPower && pool.accSushiPerShare == 0) || totoalPower == 0 || block.number < startBlock) {
            pool.lastRewardBlock = block.number;
            return;
        }

        uint256 newSushiPerBlock = 0;
        if (totoalPower>=MAX_REWARD_WEIGHT){
            // 滿產後固定
            newSushiPerBlock = MAX_REWARD_MINING;
        } else {
            // 未達啟動算力
            if (totoalPower >= PowerLowerLimit[0]) {

                uint256 len = PowerLowerLimit.length;
                for(uint256 i=0;i<len;i++){
                    if (totoalPower >= PowerLowerLimit[i] && totoalPower <= PowerUpperLimit[i]) {
                        newSushiPerBlock = IntervalMining[i];
                        break;
                    }else{
                        // 防呆未來維護，避免0的情境，理論上不會進來
                        newSushiPerBlock = sushiPerBlock;
                    }
                }
            }
        }
    ...
    function getCurrentTotalReward() external view returns(uint256 result){
        for(uint8 i = 0; i<4; i++){
            PoolInfo storage pool = poolInfo[i];
            if (totoalPower < minimalPower || totoalPower == 0) {
            ...
            if (multiplier == 0) {
                ...
                result += pool.totalReword + multiplier * sushiPerBlock * 100 / rewardPercentage[i];
            }
        }
    ...
    function deposit(uint256 tokenID) public{
        ...
        require(userStakeTokenId[msg.sender] == 0 && IMasterChef(manager.members("NFTStakePool")).userStakeTokenId(msg.sender) == 0, "can only stake one nft");
    ...
    function deposit_In(uint8 _pid, uint256 _power) internal {
        IPromote_2_0(manager.members("MCAStakePool2")).newDeposit(msg.sender, _power);
    ...
    // 更新權重金額
    function updateUserAmount(uint8 _pid, address _user, uint256 amount, bool isAdd) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_user];
        // 領獎
        claimReward(_user);
    ...
    function _stake(uint8 _pid,uint256 tokenID) internal returns(uint256 power){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[msg.sender];
        require(INFT(nft).ownerOf(tokenID) == msg.sender,"not ownerOf");
        ...
        // 更新個人/全網權重
        updateUserAmount(_pid, msg.sender, power, true);
    ...
    function withdraw() external {
        ...
        getReward();
        uint256 _power = withdrawNFT(_pid,tokenid);
        withdraw_In(_pid, _power);
    ...
    function withdraw_In(uint8 _pid, uint256 _power) internal {
        IPromote_2_0(manager.members("MCAStakePool2")).redem(msg.sender , _power);
    ...
    function getReward() public {
        ...
        pending = pending.sub(userStakeWarProfit[msg.sender].loseAmount).add(userStakeWarProfit[msg.sender].winAmount).add(userStakeWarProfit[msg.sender].legionWinAmount);
        uint256 _mintAmont;
        if(pending>0){
            ...
            emit GetReward(msg.sender, _mintAmont);
            ...
            userStakeWarProfit[msg.sender].legionWinAmount = 0;

            userStakeRewardPendingTime[msg.sender] = block.number.add(sevenDays);
        }
    }

    function withdrawNFT(uint8 _pid,uint256 tokenID) internal returns(uint256 power){
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[msg.sender];
        ...
        // 更新個人/全網權重
        updateUserAmount(_pid, msg.sender, power, false);
    ...
    function StakeWarDistribute(address winerUser, address loserUser, uint256 bonus) external {
        ...
        uint256 loserPending = pendingSushi(loserUser).add(userStakeWarProfit[loserUser].winAmount).add(userStakeWarProfit[loserUser].legionWinAmount).sub(userStakeWarProfit[loserUser].loseAmount);
        ...
        // 如果上線為有效用戶
        if (f != address(0)) {
            if (userInfo[f]._nftBalances.length > 0) {
                dist1 = result.mul(5).div(100);
                userStakeWarProfit[f].legionWinAmount += dist1;
        ...
        // 如果上線二代為有效用戶
        if (ff != address(0)) {
            if (userInfo[ff]._nftBalances.length > 0) {
                dist2 = result.mul(1).div(100);
                userStakeWarProfit[ff].legionWinAmount += dist2;
    ...
    function getPidByTokenId(uint256 tokenId) public view returns(uint8) {
        INFT.starAttributesStruct memory nftAttr_1 = INFT(manager.members("nft")).getStarAttributes(tokenId);
        uint8 pid;
        if(nftAttr_1.level < 4) {
            pid = 0;
        } else if(nftAttr_1.level < 6){
            pid = 1;
        } else if(nftAttr_1.level < 10){
            pid = 2;
        } else if(nftAttr_1.level == 10){
            pid = 3;
        }
...
interface IPromote{
    ...
    function isGamer(address sender) external view returns (bool);
}

// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
interface IPromote_2_0{
    struct _UserInfo {
       // 以2代内的權重加總(包含自己)
        uint256 down2GenWeight;
    ...
    function newDeposit(address sender, uint256 amount) external;
    function redem(address sender, uint256 amount) external;
    function checkLevel(address user) external;
}
