...
contract MemberCard is Context, ERC165, IERC721, IERC721Metadata, Initializable {
    ...
    mapping(address => mapping(address => bool)) private _operatorApprovals; // chấp thuận người điều hành
    ...
    mapping(uint256 => Membership) private _membership;// thành viên

    mapping(uint256 => bool) private _frozen;// đóng băng

    mapping(uint256 => bool) private _transferRestriction; //hạn chế chuyển nhượng

    mapping(address => bool) private _vendors;// người bán
     ...
     */
    constructor() {}
    // thay thế cho constructor và chỉ có thể gọi 1 lần duy nhất
    ...
    // validate chỉ cho phép owner thực thi
    modifier onlyOwner() {
        require(owner() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }

    // validate chỉ cho phép vendor thực thi
    modifier onlyVendor() {
    ...
    // kiểm tra member có đang active không
