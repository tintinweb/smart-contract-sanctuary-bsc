...
contract Deflation is Context, IBEP20, Ownable {
    using SafeMath for uint256;
...
function updateFee(uint256 _burnFee,uint256 _charityFee, uint256 _txFee) onlyOwner() public{
require(_burnFee < 100 && _charityFee < 100 && _txFee < 100);
        _BURN_FEE = _burnFee * 100;
_CHARITY_FEE = _charityFee* 100;
_TAX_FEE = _txFee* 100;
ORIG_BURN_FEE = _BURN_FEE;
...
emit Transfer(address(this), address(0), tBurn);
    ...
    function serviceCreator() public pure returns (string memory) {
        return "https://www.coinmarkt.info/";
    } 
