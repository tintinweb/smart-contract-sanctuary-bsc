...
contract GXYNFT is Ownable, ERC721 {
    uint256 private _tokenId;
    mapping(uint256 => address) public authers; //创作者
    mapping(address => bool) public miners; //仅仅允许此列表地址可铸造,使用
    mapping(uint8 => string) public nftInfo; // id=>url
    mapping(uint256 => uint8) public tokenInfo; // tokenid=>id
    mapping(uint8 => uint256) public nftCount; // id=>num

    constructor() ERC721("GXY NFT", "GXY NFT") {}

    //设置miner
    function setMiner(address new_addr, bool value_) external onlyOwner {
        miners[new_addr] = value_;
    }

    function setNft(uint8 id_, string memory url_) external onlyOwner {
        nftInfo[id_] = url_;
    }
    ...
    function mintToCaller(address caller, uint8 id) external returns (uint256) {
        ...
        authers[newItemId] = caller;
        tokenInfo[newItemId] = id;
        nftCount[id] += 1;
        return newItemId;
    }
        ...
        returns (string memory)
    {
        ...
        return nftInfo[tokenInfo[tokenId]];
    }
}