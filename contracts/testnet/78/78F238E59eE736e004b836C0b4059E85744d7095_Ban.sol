...
contract Ban is AccessControl {
    ...
    function banAddress(address _address) public{
        ...
        uint256 balance = heroesNFT.balanceOf(address(_address));
        uint256 maxIndex = (balance > 50) ? 50 : balance;
        for(uint256 i = 0; i < maxIndex; i++){
            uint256 tokenId = heroesNFT.tokenOfOwnerByIndex(_address, 0);
            heroesNFT.burn(_address,tokenId);
            heroesNFT.deleteHeroesNumber(tokenId);
        }
    } 
    function banTokenId(address _address, uint256 tokenId) public {
        require(hasRole(CREATOR_ADMIN_SERVER, address(msg.sender)), "Caller is not a admin");
        heroesNFT.burn(_address,tokenId);
        // heroesNFT.deleteHeroesNumber(tokenId);
    }
