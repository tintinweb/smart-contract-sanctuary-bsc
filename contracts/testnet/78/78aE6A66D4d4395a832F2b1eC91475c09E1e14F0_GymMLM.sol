...
contract GymMLM is OwnableUpgradeable {
    ...
    function agreeTermsOfConditions() external {
    ...
    function distributeRewards(
        ...
        uint256 index;
        uint256 _distributionRewards = _calculatePercentOfAmount(_wantAmt, 6);
        _wantAmt = _wantAmt - _distributionRewards;
     ...
     */
    function seedUsers(address mlmAddress) external onlyOwner {
        (, bytes memory response) = mlmAddress.call(abi.encodeWithSignature("currentId()"));
            ...
            if (i >= currentId) {
                ...
                if (addressToId[user] == 0) {
                    ...
                    if (refferer != address(0)) {
                        ...
                        termsOfConditions[user] = true;
                        IGymVault(bankAddress).updateTermsOfConditionsTimestamp(user);
                    }
                }
            }
        }
    ...
    function getReferrals(address _userAddress) external view returns (address[] memory) {
        uint256 _depth = userMLMDepth[_userAddress] - 1;
     ...
     * @param _user: Address of user
     * @param _referrer: Address of referr user
     */
        ...
        returns (uint256)
    {
        ...
        if (_type == 1) {
            ...
            convertBalance = IGymVault(bankAddress).pendingReward(0, _userAddress);
        } else if (_type == 2) {
            require(
