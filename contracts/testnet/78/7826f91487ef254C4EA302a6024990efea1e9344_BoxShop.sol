...
function buyBox(uint256 _id, uint256 _bep20) external  whenNotPaused returns (uint256) {
        require(isActive == true,'Shop not active');
        require(_bep20 != 0,'value must be greater than 0');
        require(_bep20 == boxSaleConfigs[_id].price,'value must be greater than 0');
        ...
        uint256 id = boxNFT.buyBox(msg.sender,boxSaleConfigs[_id].boxType,boxSaleConfigs[_id].rank);
        return id;
    }
  ...
  function setConfig(uint256 _id,uint256 _boxType, uint256 _rank,uint256 _price, bool _isActive,uint256 _limit) public onlyOwner{
      boxSaleConfigs[_id].id = _id;
      boxSaleConfigs[_id].boxType = _boxType;
      boxSaleConfigs[_id].price = _price;
      boxSaleConfigs[_id].rank = _rank;
...
contract ShoeNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    modifier onlyBoxNFTOrOwnerOrShoeShop {
      require(msg.sender == boxNFT || msg.sender == owner()||msg.sender == shoeShop );
      _;
    }
...
function transfer(uint256 _nftId, address _target)
  ...
  function setShoeShop(address _shoeShop) public onlyOwner{
    shoeShop=_shoeShop;
  }
