...
contract Tinad is Context, IERC20, Ownable {
    ...
    uint256 public totalFeesTax = _taxFee.add(_liquidityFee).add(_marketingFee).add(_burnFee);
    ...
    bool public transactionCooldownEnabled = false;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance.mul(_marketingFee).div(totalFeesTax);
            uint256 burnTokens = contractTokenBalance.mul(_burnFee).div(totalFeesTax);

            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(totalFeesTax);
    ...
    function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
        _burnFee = burnFee;
        totalFeesTax = _taxFee.add(_marketingFee).add(_liquidityFee).add(_burnFee);
