...
contract Municipality is IMunicipality, OwnableUpgradeable, EIP712Upgradeable {
    ...
    IActivatableNFT public minerContractAddress;
    ...
    modifier initialized() {
        require(
            address(minerContractAddress) != address(0) &&
...
function buyBundle(
        ...
        for(
            ...
            if(i % bundle.parcelSize == 0) {
                currentY++;
            }
            currentX++; 
            newParcel._x = currentX;
            newParcel._y = currentY;
            brokerPublicBuilding.mintParcel(_parcel, msg.sender); 
    ...
    function setMinerContractAddress(address _minerContractAddress) external onlyOwner {
        minerContractAddress = IActivatableNFT(_minerContractAddress);
    }
    ...
    function _calculateBundlePrice(
        ...
        finalPrice = finalPrice * bundles[uint256(_bundleType)].discount / 1000;
    }
