...
contract TestToken is Context, IERC20Metadata, Ownable {
    ...
    string private constant SYMBOL = "TET";
    uint8 private constant DECIMALS = 18;

    uint256 private constant _totalSupply = 69 * 10 ** DECIMALS;
    ...
    uint256 public _sixtyNineBuyFee;
    uint256 public _liquidityBuyFee;
    uint256 public _marketingSellFee;
    uint256 public _sixtyNineSellFee;
    uint256 public _liquiditySellFee;
    uint256 public constant _maxFeePercentage = 15;
    address public _marketingWallet;
    address public _liquidityWallet;
    uint256 public _marketingBalance;
    uint256 public _sixtyNineBalance;
    uint256 public _liquidityBalance;
    mapping(address => bool) private _feeExclusions;

    //Sixty nine
    uint256 public _rewardCounter;
    uint256 public constant SIXTY_NINE = 6;
    //    uint256 public constant SIXTY_NINE = 69;
    uint256 public _rewardMinBuy = 1 * 10 ** (DECIMALS) / 1000; //0.1 eth
    ...
    function setTransactionUpperLimit(uint256 limit) external onlyOwner {
    ...
    function setFees(
        ...
        uint256 sixtyNineBuyFee,
        uint256 liquidityBuyFee,
        uint256 marketingSellFee,
        uint256 sixtyNineSellFee,
        uint256 liquiditySellFee
    )
    public
    onlyOwner
    {
        require(_maxFeePercentage >= marketingBuyFee + sixtyNineBuyFee + liquidityBuyFee);
        require(_maxFeePercentage >= marketingSellFee + sixtyNineSellFee + liquiditySellFee);
        _marketingBuyFee = marketingBuyFee;
        _sixtyNineBuyFee = sixtyNineBuyFee;
        _liquidityBuyFee = liquidityBuyFee;
        _marketingSellFee = marketingSellFee;
        _sixtyNineSellFee = sixtyNineSellFee;
    ...
    function takeFees(uint256 amount, uint8 transferType) private returns (uint256) {
        ...
        uint256 sixtyNinePercentage = transferType == BUY ? _sixtyNineBuyFee : _sixtyNineSellFee;
        ...
        uint256 sixtyNineFee = amount.mul(sixtyNinePercentage).div(100);
        uint256 liquidityFee = amount.mul(liquidityPercentage).div(100);

        _marketingBalance += marketingFee;
        _sixtyNineBalance += sixtyNineFee;
        _liquidityBalance += liquidityFee;

        uint256 totalFee = marketingFee.add(sixtyNineFee).add(liquidityFee);
    ...
    function swapFees() private swapping {
        ...
        emit TakeFee(_marketingBalance, ethToMarketing);
        _marketingBalance = 0;
    }
    ...
    function reward(address recipient) private {
        _rewardCounter = 0;
        basicTransfer(address(this), recipient, _sixtyNineBalance);
        _sixtyNineBalance = 0;
    }
    ...
    returns (uint256)
    {
        return _marketingBuyFee.add(_sixtyNineBuyFee).add(_liquidityBuyFee);
    }
