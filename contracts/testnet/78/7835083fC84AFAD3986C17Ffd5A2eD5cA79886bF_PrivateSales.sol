...
contract PrivateSales is Ownable {
    ...
    uint256 public coinPriceInUSD = 7500000;
    ...
    constructor(address token_, address priceFeedAddress_)  {
        token = IERC20(token_);
        priceFeed = AggregatorV3Interface(priceFeedAddress_);

    }

    /// @dev setting the releaseTime
    /// @param releaseTime_ uint64 value of releaseTime

    function setReleaseTime(uint64 releaseTime_) external onlyOwner {
        require(releaseTime == 0, "PrivateSales: releaseTime can't be twice");
        require(releaseTime_ >= block.timestamp, "PrivateSales: releaseTime_ can't be lower than block timestamp");
    ...
    /// @dev view function to get latest price for BNB to USD
    ...
    function buy() external payable {
        ...
        require(msg.value > 0, "PrivateSales: Zero amount");
...
contract PrivateSalesVesting is Context {
    ...
    constructor(
        ...
        require(beneficiary_ != address(0), "PrivateSalesVesting: beneficiary is zero address");
        require(duration_ > 0, "PrivateSalesVesting: duration is zero");
        require(token_ != address(0), "PrivateSalesVesting: beneficiary is zero address");
