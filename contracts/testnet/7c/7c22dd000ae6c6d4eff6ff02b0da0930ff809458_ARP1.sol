...
contract ARP1 is Context, Ownable, IBEP20, BEP20Detailed {
  ...
  using SafeMath for uint256;
    ...
    address public developmentWallet = 0xa3455c95c4839266755dA27b4066525D0CAf51a2;
    address public buybackyWallet = 0xa3455c95c4839266755dA27b4066525D0CAf51a2;
    ...
    constructor () BEP20Detailed(" Advance Remit Protocol-1", "ARP1", 18) {
      _owner = owner() ;
    ...
    function setFeePercent(uint256 updatedMarketingFee,uint256 updatedDevelopmentFee,uint256 updatedBuybackFee,uint256 updatedBurnFee, uint256 updatedLiquidityFee) external onlyOwner() {
        marketingFee = updatedMarketingFee;
        developmentFee = updatedDevelopmentFee;
        buybackFee = updatedBuybackFee;
        burnFee = updatedBurnFee;
    ...
    function _transfer(address sender, address recipient, uint amount) internal{
       ...
       uint256 marketingAmount = amount.mul(marketingFee).div(100);
            uint256 developmentAmount = amount.mul(developmentFee).div(100);
            ...
            amount = amount.sub(marketingAmount.add(developmentAmount).add(buybackAmount).add(burnAmount).add(liquidityAmount));
            _balances[marketingWallet] = _balances[marketingWallet].add(marketingAmount);
            _balances[developmentWallet] = _balances[developmentWallet].add(developmentAmount);
            _balances[burnWallet] = _balances[burnWallet].add(burnAmount);
            _balances[buybackyWallet] = _balances[buybackyWallet].add(buybackAmount);
            ...
            emit Transfer(sender, buybackyWallet, buybackAmount);
