...
contract TokenionDeposit {
    ...
    function setTotalSupply(uint256 _supply, bool add) public onlyToken{
        if(add){
            totalSupply += _supply;
        }else{
            totalSupply -= _supply;
    ...
    function delivery(
        ...
        if(TokenionToken(token).balanceOf(address(this), tokenID) > 0){
            if(TokenionToken(token).balanceOf(address(this), tokenID) >= (amount / priceToken) + bonus[amount]){
                TokenionToken(token).safeTransferFrom(address(this), user, tokenID, (amount / priceToken) + bonus[amount], "");
                emit ReceivedTokens(user, tokenID, (amount / priceToken) + bonus[amount]);
            }else{
                TokenionToken(token).safeTransferFrom(address(this), user, tokenID, TokenionToken(token).balanceOf(address(this), tokenID), "");
                emit ReceivedTokens(user, tokenID, TokenionToken(token).balanceOf(address(this), tokenID));
            }
        }
	}
...
contract TokenionToken is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    function mint(address account, uint256 id, uint256 amount, bytes memory data)
        ...
        onlyOwner
    {
        TokenionDeposit(_depositContract[id]).setTotalSupply(amount, true);
        ...
        onlyOwner
    {
        for(uint i; i < ids.length; i++){
            TokenionDeposit(_depositContract[ids[i]]).setTotalSupply(amounts[i], true);
        }
    ...
    function burn(address account, uint256 id, uint256 amount, bytes memory data)
        ...
        onlyOwner
    {
        TokenionDeposit(_depositContract[id]).setTotalSupply(amount, false);
        _burn(account, id, amount);
    }

    function burnBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data)
        public
        onlyOwner
    {
        for(uint i; i < ids.length; i++){
            TokenionDeposit(_depositContract[ids[i]]).setTotalSupply(amounts[i], false);
        }
