...
contract Lend is IERC721Receiver, ReentrancyGuard, AccessProtected,EIP712Base {
    ...
    bytes32 private constant OFFER_TYPEHASH =
        keccak256(bytes("Offer(address buyer,uint256 price,uint256 listingId,uint256 timestamp,uint256 expiryTime,uint256 rentduration)"));
    ...
    function batchclaimNft() external nonReentrant {
        ...
        require(listingIds.length>0,"Nothing to claim");
        uint256[] memory claimedNFTlist=new uint256[](len);
        ...
        if(listings[listingId].onLend && block.timestamp > listings[listingId].rentexpire){
            listings[listingId].onLend=false;
            claimedNFTlist[count]=listingId;
            count++;
    ...
    function hashOffer(Offer memory offer) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(OFFER_TYPEHASH, offer.buyer, offer.price, offer.listingId, offer.timestamp, offer.expiryTime,offer.rentduration)
            );
    }
    ...
    function acceptOffer(
        ...
        require(!listings[listingId].onLend, "Listing is already Lended");
        ...
        require(listings[listingId].duration>block.timestamp+rentperiod,"Praposed days exceeds listing duration");
        
        listings[listingId].rentperiod=rentperiod;
        uint256 chargedamount=(listings[listingId].rentperiod.div(86400)).mul(amountperday);
        listings[listingId].onLend = true;
        listings[listingId].isActive = false;
        listings[listingId].rentexpire = block.timestamp + listings[listingId].rentperiod;
        listings[listingId].price=chargedamount;
        // Transfer the seed Tokens
        transferWithTreasury(offerSender, seller, chargedamount);
        
        issueGameToken(listingId);
        
        emit Rented(listingId,listings[listingId].tokenIDs, seller, offerSender, chargedamount, block.timestamp, false, true);
    }

    function issueGameToken(uint256 listingId) private {
        for (uint256 i = 0; i < listings[listingId].tokenIDs.length; i++) {
            nftGameToken.issueToken(
                msg.sender,
                listings[listingId].tokenIDs[i],
    ...
    function Userclaimablelisting()public view returns (uint256[] memory){
        uint256[] memory listingIds=userlistings[_msgSender()];
        uint256 len=listingIds.length;
        uint256 count=0;
