...
contract Nybu is Ownable, Pausable, ReentrancyGuard {
    ...
    function getUnclaimedUserBets(address user, uint cursor, uint window) external view returns(QueryResult[] memory, uint, uint) {
       ...
       uint length = window;
        ...
        for (uint i = 0; i < length; i++) {
            ...
            if(!bet.claimed){
                ...
                uint activeRound = _ceilTimestamp(block.timestamp);
                if(timestamp == activeRound || timestamp >= activeRound - interval){
                ...
                if (round.result == Result.BullWon && bet.position == Position.Bull) {
                    ...
                    results[i].winnings = winnings - feeAmount;
