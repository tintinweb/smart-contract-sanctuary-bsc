...
contract Clover_Seeds_Token is IBEP20, Auth, Pausable {
   using SafeMath for uint256;
    ...
    // @Dev Sell tax..
    uint16 private _sellTeamFee = 12000;
    uint16 private _sellLiquidityFee = 16000;
    
    // @Dev Buy tax..
    uint16 private _buyTeamFee = 2000;
    uint16 private _buyLiquidityFee = 1000;

    uint16 private _TeamFeeWhenNoNFTs = 7000;
    uint16 private _LiquidityFeeWhenNoNFTs = 14000;
    uint16 private _MarketingFeeWhenNoNFTs = 7000;
    uint8 private _liquidityThreshold = 10;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (shouldSwapBack(sender, recipient)) { swapFee(); }
    ...
    function shouldSwapBack(address sender, address recipient) internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && (block.timestamp - releaseTimeStamp) >= releaseDuration;
    }
   ...
   function setMaxWallet(uint256 amount) external onlyOwner() {
    ...
    function setReleaseDuration(uint256 duration) public onlyOwner {
        require(duration > 0, "Release Duration can't be zero!");
