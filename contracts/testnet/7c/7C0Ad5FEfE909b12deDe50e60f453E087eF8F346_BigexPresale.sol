...
contract BigexPresale is Ownable, Pausable {
	...
	function buyTokenBNB() public payable whenNotPaused {
		...
		require(minBuyBnb <= _paymentAmount && _paymentAmount <= maxBuyBnb, "Presale: min max buy is not valid");
		require(totalBuyBnb(msg.sender) + _paymentAmount <= maxBuyBnb, "Presale: limit buy token");

		// check balance payment bnb before buy token
		require(address(msg.sender).balance >= _paymentAmount, "Presale: balance not enough");
