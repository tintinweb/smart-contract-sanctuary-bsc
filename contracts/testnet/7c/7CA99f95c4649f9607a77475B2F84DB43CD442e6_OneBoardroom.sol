...
interface ITreasury is IEpoch {
    ...
    function getDarkoUpdatedPrice() external view returns (uint256);
    ...
    function getDarkoLockedBalance() external view returns (uint256);

    function getDarkoCirculatingSupply() external view returns (uint256);
    ...
    function previousEpochDarkoPrice() external view returns (uint256);
...
contract OneBoardroom is ShareWrapper, ContractGuard {
    ...
    IERC20 public darko;
    IERC20 public wftm;
    IERC20 public dark; // second collateral
    ...
    uint256 public darkCollateralDiscountPercent;
    uint256 public darkoTokenSupplyTarget;
    ...
    function initialize(
        ...
        maxCollateralPercent = 9000; // 90%
        darkCollateralDiscountPercent = 2000; // 20% discount if pay by DARK
        darkoTokenSupplyTarget = 200000000 ether;
    ...
    function setDark(IERC20 _dark) external onlyOperator {
        dark = _dark;
    }
    ...
    function setDarkCollateralDiscountPercent(uint256 _darkCollateralDiscountPercent) external onlyOperator {
        require(_darkCollateralDiscountPercent <= 5000, "too high"); // <= 50%
        darkCollateralDiscountPercent = _darkCollateralDiscountPercent;
    }

    function setDarkoTokenSupplyTarget(uint256 _darkoTokenSupplyTarget) external onlyOperator {
        require(_darkoTokenSupplyTarget >= 100000000 ether, "too low"); // >= 100 million DARKO
        darkoTokenSupplyTarget = _darkoTokenSupplyTarget;
    }
    ...
    function canClaimReward() public view returns (bool) {
        ...
        // return _treasury.previousEpochDarkoPrice() >= 1e18 && _treasury.getNextExpansionRate() > 0; // current epoch and next epoch are both expansion
    }

    function getCollateralPercent() public view returns (uint256 _collateralPercent) {
        uint256 _darkoSupply = darko.totalSupply();
        uint256 _darkoTokenSupplyTarget = darkoTokenSupplyTarget;
        if (_darkoSupply >= _darkoTokenSupplyTarget) {
            _collateralPercent = maxCollateralPercent;
        } else {
            _collateralPercent = _darkoSupply.mul(10000).div(_darkoTokenSupplyTarget);
    ...
    function getDarkOverWftmPrice() public view returns (uint256) {
        return (darkOracle == address(0)) ? 1e18 : uint256(IEpoch(darkOracle).getPegPriceUpdated());
    }
    ...
    function getCollateralForClaimAllPegTokens(address _member) public view returns (uint256 _numOfPegTokens, address[] memory _pegTokenAddresses, uint256[] memory _earnedPegTokens, uint256[] memory _collateralForPegTokens, uint256 _totalCollateral, uint256 _totalDarkCollateral) {
        ...
        _totalDarkCollateral = _totalCollateral.mul(getDarkOverWftmPrice()).div(1e18);
        _totalDarkCollateral = _totalDarkCollateral.sub(_totalDarkCollateral.mul(darkCollateralDiscountPercent).div(10000)); // sub discount
    }
    ...
    function claimReward(bool _useDarkCollateral) external {
    ...
    function _claimReward(bool _useDarkCollateral) internal updateReward(msg.sender) {
        uint256 reward = members[address(darko)][msg.sender].rewardEarned;
        if (reward > 0) {
            if (_useDarkCollateral) {
                dark.safeTransferFrom(msg.sender, address(this), getTotalDarkCollateralForClaimAllPegTokens(msg.sender));
            } else {
                wftm.safeTransferFrom(msg.sender, address(this), getTotalCollateralForClaimAllPegTokens(msg.sender));
            }
        }
    ...
    function allocateSeigniorage(address _token, uint256 _amount) external onlyOneBlock onlyTreasury {
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        ...
        require(address(_token) != address(darko), "darko");
