...
contract NekoStaking is Ownable, Pausable, ReentrancyGuard {
    ...
    function stake(uint256 _pid, uint256 _amount) public whenNotPaused nonReentrant {
        ...
        IERC20(NINO_TOKEN).transferFrom(address(msg.sender), address(this), _amount);
    ...
    function unStake(uint256 _pid) public whenNotPaused {
        ...
        if(pool.limitTokenStake == 0){
            IERC20(NINO_TOKEN).transfer(address(msg.sender), user.amount);
            user.amount = 0;
        }else{
        ...
        IERC20(NINO_TOKEN).transfer(address(msg.sender), totalReward);
        user.unclaimed = 0;
    }
    ...
    function _release(uint256 _pid) private returns(uint256) {
        ...
        require(IERC20(NINO_TOKEN).balanceOf(address(this)) > 0, "TokenTimelock: no tokens to release");
        ...
        if (countRelease[msg.sender] + cliff >= TIME_LINEAR) {
            IERC20(NINO_TOKEN).transfer(msg.sender, balanceOf[msg.sender]);
            balanceOf[msg.sender] = 0;
        } else {
            nextRelease[msg.sender] = nextRelease[msg.sender] + PERIOD.mul(cliff);
            balanceOf[msg.sender] = balanceOf[msg.sender].sub(amount);

            IERC20(NINO_TOKEN).transfer(msg.sender, amount);
        }
    ...
    function getBalance() public view returns (uint256) {
        return IERC20(NINO_TOKEN).balanceOf(address(this));
    }
