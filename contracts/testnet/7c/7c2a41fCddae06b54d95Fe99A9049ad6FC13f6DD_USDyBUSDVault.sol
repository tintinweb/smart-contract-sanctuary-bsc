...
contract USDyBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,bool isBUSD) nonReentrant external _lastExchangeRatio _completePause whenNotPaused {
        ...
        exchangeRatio = IERC20(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)/(IERC20(USDs_lp).totalSupply());
    ...
    function _tax(uint amount) internal{
        ...
        if(Admin.buyBackActivation()){
            if(Admin.buyBackActivationEpoch() + (3 * Admin.epochDuration()) < block.timestamp) {
                    10**18 <= IUniswapV2Router02(router).getAmountsOut(10**18,path)[1] ? 
                        Admin.setBuyBackActivation(false) : Admin.setBuyBackActivationEpoch();
                }
            IERC20(BUSD).safeTransfer(Admin.TeamAddress(), (amount * 20 * 25)/10000);
            path[0] = BUSD;
            path[1] = Admin.USDy();
            ...
            if (BUSDAmount != 0) {
                ...
                IERC20(BUSD).safeTransfer(Admin.TeamAddress(), (amount * 1) / 100);
            }
            if (leftAmount != 0) {
