...
contract TheNetworkSociety is Context, Ownable {
    ...
    function calculateDepositTax(uint256 value) private returns (uint256 adjustedAmount, uint256 taxAmount){
        taxAmount = payTax(value);
        adjustedAmount = value.sub(taxAmount);
        return (adjustedAmount, taxAmount);
    }
    ...
    function deposit(address addr, uint256 amount) private isInitialized {
        ...
        (uint256 amountPostTax, ) = calculateDepositTax(amount);
    ...
    function drawLastDepositWinner() private {
        if(lastDepositEnabled && block.timestamp.sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) {
            ...
            (uint256 netReward, ) = calculateDepositTax(lastDepositPoolBalance);
    ...
    function airdrop(address receiver, uint256 amount) public isInitialized {
        ...
        (uint256 payout, ) = calculateDepositTax(amount);
    ...
    function payTax(uint256 amount) private returns(uint256) {
        uint256 dvtTax = amount.mul(devtTax).div(dividerPercent); // 3.5%
        uint256 admTax = amount.mul(adminTax).div(dividerPercent); // 5% 
        uint256 mktTax = amount.mul(marketTax).div(dividerPercent); // 1.5%
