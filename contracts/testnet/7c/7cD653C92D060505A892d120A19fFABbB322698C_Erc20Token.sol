...
contract ProductMarketing is Ownable {
    ...
    constructor() {
        ...
        users[ownerId].parentid = ownerId;
    ...
    function registerUser(uint referralId) public returns (bool) {
        ...
        if (referralId == 0) referralId = ownerId;
        ...
        User storage parentuser = _getUserById(referralId);
        ...
        registeredUser.parentid = parentuser.id;
        registeredUser.registerdate = block.timestamp;

        parentuser.lastreferrer = block.timestamp;

        usersId[userAddress] = registeredUser.id;
        usersAddress[registeredUser.id] = userAddress;
        referrers[parentuser.id].push(registeredUser.id);

        userCount++;

        for (uint lv = 1; lv <= configs.levels; lv++) {
            uint percent = 0;// configs.percents[lv];
            if (percent == 0) continue;

            uint comssion = (registerprice * percent) / 100;

            User storage _prnt = parentuser;
            uint level = lv;
            // if (lv >= configs.lockedlevel && _prnt.id > ownerId) {
            //     if (!parentuser.isunlocklevels) {
            //         _prnt = _getUserById(ownerId);
            //         level = 99;
            //     }
            // }
    ...
    function unlockLevels() public returns (bool) {
        address userAddress = address(msg.sender);

        require(usersId[userAddress] > 0, "Not found user!");

        User storage user = _getUserByAddress(userAddress);
        //require(!user.isunlocklevels, "Levels is unlocked!");

        uint unlockprice =0;// configs.unlockfee * 10 ** stablecoincontract.decimals();

        require(
            stablecoincontract.balanceOf(userAddress) >= unlockprice,
            "Insufficient stablecoin balance!"
        );
        require(
            stablecoincontract.allowance(userAddress, address(this)) >=
        ...
        require(
            stablecoincontract.balanceOf(address(this)) >= unlockprice,
            "Transfer stablecoin failed!"
        );

        stablecoincontract.transfer(owner(), unlockprice);

        //user.isunlocklevels = true;
