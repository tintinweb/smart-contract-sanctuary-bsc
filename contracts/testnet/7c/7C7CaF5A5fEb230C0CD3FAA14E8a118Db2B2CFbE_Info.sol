...
contract Info is OwnableUpgradeable{
    ...
    function cowInfoes(uint tokenId) external view returns(uint[18] memory info1,bool[3] memory info2, uint[2] memory parents){
        info2[0] = cattle.isCreation(tokenId);
        info2[1] = stable.isUsing(tokenId);
        info2[2] = cattle.getAuldt(tokenId);
        ...
        info1[16] = stable.growAmount(info1[14],tokenId);
        info1[17] = stable.feedingTime(tokenId);
...
interface IStable {
    ...
    function feedingTime(uint tokenId) external view returns(uint);
