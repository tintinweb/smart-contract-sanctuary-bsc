...
contract ClashingBlades is IBEP20, Auth {
    using SafeMath
    for uint256;
    ...
    address DEV = 0x0B836c4E5e11C1F8110888C74e6472A65AA72a50;
    address P2E = 0x1F24c17371cac7BD0960815b7F2C0D35181aA660;

    string constant _name = "ClashingBlades";
    string constant _symbol = "LYKA";
    ...
    constructor() Auth(msg.sender) {
        ...
        isFeeExempt[address(P2E)] = true;

        isTxLimitExempt[msg.sender] = true;
        isTxLimitExempt[address(DEV)] = true;
        isTxLimitExempt[address(P2E)] = true;
        ...
        isTimelockExempt[address(DEV)] = true;
        isTimelockExempt[address(P2E)] = true;


        isDividendExempt[pair] = true;
        isDividendExempt[address(this)] = true;
        isDividendExempt[address(DEV)] = false;
        isDividendExempt[address(P2E)] = false;
        ...
        _balances[address(P2E)] = _totalSupply/2;
        emit Transfer(address(0), msg.sender, _totalSupply/2);
        emit Transfer(address(0), address(P2E), _totalSupply/2);
    }
    ...
    function setMaxWalletPercent_base1000(uint256 maxWallPercent_base1000) external onlyOwner() {
        require(maxWallPercent_base1000 >= 1, "Max Wallet Percentage cannot be less than 1%");
        _maxWalletToken = (_totalSupply * maxWallPercent_base1000) / 1000;
    }

    function setMaxTxPercent_base1000(uint256 maxTXPercentage_base1000) external onlyOwner() {
        require(maxTXPercentage_base1000 >= 1, "Max Transaction Percentage cannot be less than 1%");
        _maxTxAmount = (_totalSupply * maxTXPercentage_base1000) / 1000;
    }

    function setTxLimit(uint256 amount) external onlyOwner() {
        require(amount > 0 * 10 ** _decimals, "MaxTransaction Limit has to be 1% or more");
        _maxTxAmount = amount;
    }
    ...
    function tradingStatus(bool _status) public onlyOwner {
        require(!tradingOpen, "Trading Status can only be set to true once and never false after");
        tradingOpen = _status;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee) external onlyOwner {
        ...
        require(totalFee < 15, "Fees cannot be more than 15%");
    }
    ...
    function setP2EWallet(address _p2eWallet)  external onlyOwner {
        P2E = _p2eWallet;
    }
    ...
    function multiTransfer_fixed(address[] calldata addresses, uint256 tokens) external authorized {
