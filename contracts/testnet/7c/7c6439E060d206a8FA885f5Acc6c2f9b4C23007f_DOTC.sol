...
/*  bytes32 internal constant _rebaseTime_  = "rebaseTime";
   ...
   /* function migrate(address vault_) external governance {
    ...
    function setFreeCurrencyList(bytes32[] memory list) external governance {
        freeCurrencyList = list;
    }
    function isFreeCurrency(bytes32 currency) public returns(bool iclude){
        for(uint i=0; i<freeCurrencyList.length; i++){
            if(currency==freeCurrencyList[i]){
                iclude=true;
                break;
            }
        }
    ...
    function deliver(uint takeID) virtual external nonReentrant returns(uint vol) {
        ...
        if(isFreeCurrency(makes[makeID].currency)){
            ...
            uint256 fee = _payFee(takeID, makes[makeID].asset, vol);
