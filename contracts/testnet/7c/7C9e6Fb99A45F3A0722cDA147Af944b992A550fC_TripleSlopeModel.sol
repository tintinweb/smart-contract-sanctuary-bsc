...
contract TripleSlopeModel {
  ...
  function getInterestRate(uint256 debt, uint256 floating) external pure returns (uint256) {
    ...
    } else if (utilization > CEIL_SLOPE_1 && utilization < CEIL_SLOPE_2) {
      // Between 60% and 90% - 20% APY
      return uint256(MAX_INTEREST_SLOPE_2) / 365 days;
    } else if (utilization > CEIL_SLOPE_2 && utilization < CEIL_SLOPE_3) {
      // Between 90% and 100% - 20%-150% APY
      return
        (MAX_INTEREST_SLOPE_2.add(
            (utilization.sub(CEIL_SLOPE_2)).mul(
              MAX_INTEREST_SLOPE_3.sub(MAX_INTEREST_SLOPE_2)
            ).div(CEIL_SLOPE_3.sub(CEIL_SLOPE_2))
          )
        ) / 365 days;
    } else {
