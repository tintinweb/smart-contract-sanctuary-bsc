...
contract MYERC721 is ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    mapping(uint256 => NFT) public NFTdetails;
    mapping(uint256 => string) tokensURI;
    mapping(address => bool) public isManager;
    mapping(uint256 => bool) public isFirstSale;
    struct NFT {
        address creator;
    }
        ...
        returns (uint256 tokenId)
    {
        ...
        setRoyalties(tokenId, payable(msg.sender), 1);
    }
    ...
    function setSecondSale(uint256 _tokenId) external {
        isFirstSale[_tokenId] = false;
    }
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address receiver, uint256 royaltyAmount) {
      ...
      if(_royalties.length > 0) {
          if(isFirstSale[_tokenId]){
            return (_royalties[0].account, (_salePrice * _royalties[0].value) / 1000);
          }else{
            return (_royalties[1].account, (_salePrice * _royalties[1].value) / 1000);
          }
