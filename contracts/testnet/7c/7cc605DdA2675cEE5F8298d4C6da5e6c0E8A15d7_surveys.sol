...
contract surveys is Ownable {
    ...
    function getSurveysBalances(uint256 _surveyId)
        public
        view
        returns (uint256, uint256)
    {
        return (
            surveyValidatorsBalance[_surveyId],
            surveyParticipantsBalance[_surveyId]
        );
    }
    ...
    function splitFunds(
        ...
        if (valProfit > 0 && providersProfit > 0) {
            ...
            uint256 balance = _amount.sub(validatorsAmt + providersAmt);

            surveyValidatorsBalance[newSurveyID] += validatorsAmt;
            surveyParticipantsBalance[newSurveyID] += providersAmt;
            surveyBalance[newSurveyID] += balance;
