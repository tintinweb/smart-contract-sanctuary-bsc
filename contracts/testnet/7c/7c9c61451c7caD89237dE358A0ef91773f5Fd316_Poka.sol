...
contract  Poka {
   ...
   event logSZ(uint16 won,uint16 yzbl,uint256 yzs);
   struct Game{
        uint128 id;
        uint16 key;
        uint16 bl;
    ...
    constructor(){
        owner  = msg.sender ;
        token = 0xE97F46C26ef27eeDCfB9bf705f33436A30A68346;
        id  = 1000;
    }
...
function runHorse(uint16[][] memory btms) external{
  ...
  uint256 amount_256 = amount *1e18;
  uint256 won_256;
  require(userAmount[addr] >= amount_256 ,"betm amount cannot greater than balance");
  userAmount[addr] -= amount_256;
