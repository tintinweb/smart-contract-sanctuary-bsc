...
contract ReedBricks is Context, IBEP20, Ownable, ReedLock {
    ...
    function purchase(address _from, uint256 _amount) public payable {
        totalBNBSold = totalBNBSold + _amount;
        uint256 amount = _amount * unitsOneBNBCanBuy;
        require(_rOwned[address(this)] >= amount);

        _rOwned[address(this)] = _rOwned[address(this)].sub(amount);
        _rOwned[_from] = _rOwned[_from].add(amount);

        emit Transfer(address(this), _from, amount);
