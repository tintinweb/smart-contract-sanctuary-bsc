...
contract MGNToken is ERC20, ERC20Burnable, Pausable, AccessControl, Ownable {
    ...
    uint256[2] public collectingRatio = [10,90];
    // 1 BUSD = 10 MGN
    uint256[2] public swapRatio = [1,11];

    constructor(address _tokenAddress) ERC20("MGNToken", "MGN") {
    ...
    function BUSDtoMGN(uint256 _amount) public {
        require(token.balanceOf(msg.sender) >= _amount,"Balance is not enough");
        require(_amount > 0, "The amount > 0");
        require(token.allowance(msg.sender, address(this)) >= _amount, "Allowance balance is not enough");
        uint256 _amountafterFee = getMinimumRecieve(_amount);
        // transfer fee (BUSD)
        token.transferFrom(msg.sender, _globalWallet, (_amount*swapFee/10000));
        // Transfer remaining BUSD
        token.transferFrom(msg.sender, address(this), (_amountafterFee *collectingRatio[0] /100));
        token.transferFrom(msg.sender, _globalWallet, (_amountafterFee *collectingRatio[1] /100));
        // Mint MGN == remaining BUSD
        mint(msg.sender, _amountafterFee*swapRatio[0]/swapRatio[1]);
    }

    function MGNtoBUSD(uint256 _amount) public {
        require(balanceOf(msg.sender) >= _amount,"Sender's balance is not enough");
        require(_amount > 0, "The amount > 0");
        require(token.balanceOf(address(this)) >= _amount,"Global wallet's balance is not enough");
        uint256 _amountafterFee = getMinimumRecieve(_amount);
        // Transfer BUSD = remaining MGN
        token.transfer(msg.sender, _amountafterFee*swapRatio[1]/swapRatio[0]);
        // Burn remaining MGN
        burn(_amount);
    }
