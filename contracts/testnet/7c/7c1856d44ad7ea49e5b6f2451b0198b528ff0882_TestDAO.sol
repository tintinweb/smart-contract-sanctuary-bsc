...
contract TestDAO is Context, IBEP20, Ownable {
    ...
    //base liquidity market burn bonus 
    uint256[5] public _taxes = [2,4,2,2,5];
    ...
    uint256 private numTokensToSwap =  1 * 10**10 * 10**18;
    ...
    event UpdateTaxes(uint256[5] targetTaxes,uint256 totalTax);
    ...
    function setTaxes(uint256[5] memory targetTaxes) external onlyOwner() {

        _taxes = targetTaxes; 

        _totalTax = 0;
        _totalTax = _totalTax.add(targetTaxes[0]).add(targetTaxes[1]).add(targetTaxes[2]).add(targetTaxes[3]).add(targetTaxes[4]);
 
        require(_totalTax <= 15,"total tax can't be greater than 15");

        emit UpdateTaxes(targetTaxes,_totalTax);
    }
    ...
    function _transfer(
        ...
        //enable tx limit against the whales
        if(
        ...
        //transfer amount, it will take  baseFee,liquidityFee,marketFee,burnFee,bonusFee 
        //2+4+2+2+5 = 15%
