...
contract AnnexIronNFTVault is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable  {
  ...
  //
  function initialize(address _annexNFT, address _claimToken,address _aNN,address _aANN) public initializer {
    owner = msg.sender;
    _daySecond = 86400;
    ...
    annexNFT = IERC721Upgradeable(_annexNFT);             // ANNEX NFT
    aNN = IERC20Upgradeable(_aNN);                        // ANNEX TOKEN
    aANN = IABep20Interface(_aANN);                       // ANNEX A-TOKEN
    tusd = IERC20Upgradeable(_claimToken);                // TUSD
