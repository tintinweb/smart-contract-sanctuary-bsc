...
contract Test {
    ...
    function getHashRate(address miner, uint256 tier_index) public view returns(uint256) {
        ...
        if(since_last_withdraw.div(24 hours) >= 5){
            hash_rate -= HASH_RATE_COMPENSATOR.mul(2);
        }
        ...
        if(Miners[miner].downLinesCount >= 3){
            if(Miners[miner].downLinesCount > REF_COMPENSATION_FACTOR){
                ...
                hash_rate += Miners[miner].downLinesCount * HASH_RATE_COMPENSATOR;
            }else{
        ...
        if((Miners[miner].withdrawalCount > LIBERTY_WITHDRAWALS) && (Miners[miner].withdrawalCount > Miners[miner].depositCount)){
            uint256 tx_diff = Miners[miner].withdrawalCount.sub(Miners[miner].depositCount);
