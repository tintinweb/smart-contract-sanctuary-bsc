...
contract SmartCashNetwork is Context, Ownable {
    ...
    uint256 private constant compoundTax = 300; //3% deducted to the amount to be compounded.
    uint256 private constant eventSustainabilityTax = 1000; //10% stays in the contract

    uint256 private constant userMaxPayout = 32000; //320% max profit
    ...
    uint256 private constant invitePercent = 300; //3% for 1.5% referrer and 1.5% referee.
    ...
    uint256 private constant maxEventShare = 1000 ether;

    //time steps
    uint256 private timeStep = 1 days;
    uint256 private cutOffTimeStep = 2 days; //max accumulation time.
    ...
    address private previousLastDepositWinner;
    uint256 private previousLastDepositRewards;

    //top deposit
    uint256 private topDepositCurrentRound = 1; //current round
    uint256 private topDepositPoolBalance; //current pool balance
    uint256 private topDepositCurrentAmount; //current pool balance
    address private topDepositPotentialWinner; //current last deposit address.
    ...
    ERC20 private token = ERC20(0xc46CCBE42Afdf64cc4DA7e56DCd60eE9bF1B743B); //0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56 //change to own token 18 decimals.
    ...
    function updateTimeStampForTesting(uint256 cutOffStep, uint256 lastDepositStep, uint256 dayStep) public onlyOwner {
        cutOffTimeStep      = cutOffStep;    
        lastDepositTimeStep = lastDepositStep;
        timeStep            = dayStep;
    }

    constructor(address leader) {
        require(!Address.isContract(leader), "Network Leader should not be a contract address.");
        networkLeader = leader;
    }
    ...
    function invest(address sponsor, uint256 amount) public isInitialized {
        ...
        deposit(addr, amount, false);
    }
    
    //investment airdrop for promotions and marketing. token payments will be directly invested instead of being given out to avoid dumps.
    function invest(address addr, address sponsor, uint256 amount) public isInitialized {
        sponsor = validateNetworkSponsor(sponsor);
		setupSponsorshipDetails(addr, sponsor);
        deposit(addr, amount, true);
    }
    ...
    function deposit(address addr, uint256 amount, bool isAirdrop) private isInitialized {
        User storage user = users[addr];
        require(user.networkSponsor != address(0) || addr == networkLeader, "Invalid network sponsor.");
        require(amount >= depositMinimum || user.totalDirectDeposits <= depositMaximum, "Either mininum deposit not reached or maximum investment reached.");

        address transferFromAddr = isAirdrop ? msg.sender : addr;
        token.transferFrom(transferFromAddr, address(this), amount);
        ...
        uint256 amountPostTax = amount.sub(amount.mul(variableTax[0]).div(dividerPercent)); //less 10% to the amount to be deposited for precision.
    ...
    function poolLastDeposit(address userAddress, uint256 amount) private {
        ...
        lastDepositPoolBalance = lastDepositPoolBalance.add(poolShare) > maxEventShare ? 
        lastDepositPoolBalance.add(maxEventShare.sub(lastDepositPoolBalance)) : lastDepositPoolBalance.add(poolShare);
    ...
    function drawLastDepositWinner() private {
        if(lastDepositEnabled && block.timestamp.sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) {
            ...
            uint256 netReward = lastDepositPoolBalance.sub(lastDepositPoolBalance.mul(eventSustainabilityTax).div(dividerPercent));
            ...
            previousLastDepositWinner  = lastDepositPotentialWinner;
            previousLastDepositRewards = netReward;
    ...
    function poolTopDeposit(address addr, uint256 amount) private {
	    ...
	    uint256 poolShare = amount.mul(eventPercent).div(dividerPercent); // 1% of user's total deposit

        topDepositPoolBalance = topDepositPoolBalance.add(poolShare) > maxEventShare ? 
        topDepositPoolBalance.add(maxEventShare.sub(topDepositPoolBalance)) : topDepositPoolBalance.add(poolShare);

        if(amount > topDepositCurrentAmount){
            topDepositCurrentAmount = amount;
            topDepositPotentialWinner = addr;
        }
    }

    // draw top depositor event.
    function drawTopDepositPool() private {
        if(topDepositEnabled && block.timestamp.sub(topDepositLastDrawAction) >= timeStep) {
            
            uint256 netReward = topDepositPoolBalance.sub(topDepositPoolBalance.mul(eventSustainabilityTax).div(dividerPercent));
            usersBonus[topDepositPotentialWinner].topDepositBonus += netReward; // only for statistics
            users[topDepositPotentialWinner].totalDirectDeposits  += netReward; // reward goes to user deposit instead of being withdrawn.
            users[topDepositPotentialWinner].totalDepositAmount   += netReward;
            emit TopDepositPayout(topDepositPotentialWinner, netReward, block.timestamp);

            previousTopDepositWinner  = topDepositPotentialWinner;
            previousTopDepositRewards = netReward;
    ...
    function validateNetworkSponsor(address addressForChecking) view private returns(address sponsor) {
        ...
        || users[addressForChecking].totalDirectDeposits <= 0 ? networkLeader : addressForChecking; //network leader is only by default and will not be used.
    }
    ...
    function initializeAndSetupNetwork(uint256 amount) public onlyNetworkLeader {
        ...
        lastDepositLastDrawAction = block.timestamp;
        topDepositLastDrawAction  = block.timestamp;
        deposit(networkLeader, amount, false);
    }
    ...
    function airdrop(address receiver, uint256 amount) public isInitialized {
        ...
        uint256 payout = amount.sub(amount.mul(variableTax[0]).div(dividerPercent));
    ...
    function lastDepositInfo() view external returns(uint256 currentRound, uint256 currentBalance, uint256 currentStartTime, address currentPotentialWinner, uint256 previousReward, address previousWinner) {
        ...
        previousReward         = previousLastDepositRewards;
        previousWinner         = previousLastDepositWinner;
    }

    // top deposits information.
    function topDepositInfo() view external returns(uint256 currentRound, uint256 currentTopDeposit, address currentPotentialWinner, uint256 currentBalance, uint256 currentStartTime, uint256 previousReward, address previousWinner) {
        currentRound           = topDepositCurrentRound;
        currentTopDeposit      = topDepositCurrentAmount;
        currentPotentialWinner = topDepositPotentialWinner;
        currentBalance         = topDepositPoolBalance;
        currentStartTime       = topDepositLastDrawAction;
        previousReward         = previousTopDepositRewards;
        previousWinner         = previousTopDepositWinner;
    }
    ...
    function updateNetworkLeader(address addr) external onlyOwner {
        require(!Address.isContract(addr), "Address should not be a contract address.");	
        networkLeader = addr; 
    }
    ...
    // run event triggers. 
    function runDrawEvents() external isInitialized { // run draw depending on restrictions
        drawTopDepositPool();
        drawLastDepositWinner();      
    }
    ...
    function injectAndAddEventRewards(uint256 eventToFeed, uint256 amount) external onlyOwner isInitialized {
        require(eventToFeed == 1 || eventToFeed == 2, "Invalid Event Type.");   
        require(amount >= 1 ether && amount <= 50 ether, "Invalid Amount."); 
        token.transferFrom(msg.sender, address(this), amount);
        if (eventToFeed == 1){
            lastDepositPoolBalance = amount;
        }
        else if (eventToFeed == 2) {
            topDepositPoolBalance = amount;
        }
    }
...
/** 
    The Smart Cash Banker.
        Innovation through combination, a sustainable yield farming built in Binance Smart Chain.
    ...
    - Every withdraw/sell action, will have a 0.5% deduction to the current user yield that will start at 1.6%.
        - if the users current daily yield is between 1% to 1.5%, Then when the user sell/withdraw their dividends the daily yield will go back to 1%
        - if the users current daily yield is 1.6% and above, then yield will decrease 0.5% 
        (i.e user current yield is 3%, after sell/withdraw yield will decrease to 2.5%)
    - User can earn 320% of his including real invested amount. (ie. deposited = 100 ether, max payout = 320 ether)
    ...
    * Every sell/withdraw the compound count will be set back to 0.  
    ...
    1.5% Referral Bonus and 1.5% Referee Cashback Bonus:
    - 1.5% goes to the Referrer and Referee's real deposit investments.
    ...
    - Rewards are capped to 1000 ether per round for each event respectively.
    ...
    - 1% of each deposit will be put into the last deposit rewards pool.
    - Rewards are capped to 1000 ether per round.
    - If there is no new deposit after the last deposit after 6 hours, Last Deposit Winner will be drawn.
    - Reward goes directly to user as direct deposit instead of making it withdrawable/claimable which will increase the investors potential earnings over time.
    - Feature will continue to run during the life of the project to help with the inflow of investment everyday.
    
      *Event Rewards are subject for 10% sustainability tax. 
      In the event that there is no pool balance on any of the event when it is enabled, the team will have the ability to add funds for rewards.
