...
contract MonsterMarket is Initializable, OwnableUpgradeable {
    ...
    // 设置定价
    function setPrice(uint ssr_, uint sr_, uint s_, uint r_, uint n_) public onlyOwner {
        // USDT 价格 单位 ether
        rarityPrice = Rarity({
            SSR: ssr_,
            SR: sr_,
    ...
    function buyMonster(uint id_, uint method_) public {
        ...
        uint tokenid = monster.getIdList(id_)[0].tokenid;
        uint rarity = monster.getTokenIdRarity(tokenid);
        require(rarity != 0, "buyMonster: Does_not_exist");
        ...
        if (rarity == 1) {
            // SSR
            pricing = rarityPrice.SSR;
        } else if(rarity == 2) {
            // SR
            pricing = rarityPrice.SR;
        } else if(rarity == 3) {
            // S
            pricing = rarityPrice.S;
        } else if(rarity == 4) {
            // R
            pricing = rarityPrice.R;
        } else if(rarity == 5) {
            // N
            pricing = rarityPrice.N;
        }
...
contract MonsterNft is Ownable, ERC721Enumerable {
    ...
    struct Total {
        uint totalAmount;       // 总量
        uint remainAmount;      // 余量
    }

    // tokenid => Monster
    mapping(uint => Monster) public mapMonster;
    // monsterId => Monster[]
    mapping(uint => Monster[]) public mapMonsterMonsterId;
    // rarity => Monster[]
    mapping(uint => Monster[]) public mapMonsterRarity;
    // monsterId => Total
    mapping(uint => Total) public mapTotal;
    ...
    constructor () ERC721("ARENA Monster NFT", "ARENA") {}
    ...
    // 官方铸造
    function mint(uint id_, uint rarity_) public onlyOwner {
        require(rarity_ > 0 && rarity_ <= 5, "mint: Rarity_level_error");
        
        _tokenIdCounter.increment();
        uint tokenId = _tokenIdCounter.current();

        mapMonster[tokenId] = Monster({
            tokenid: tokenId,
            monsterId: id_,
            rarity: rarity_
        });
        mapMonsterMonsterId[id_].push(mapMonster[tokenId]);
        mapMonsterRarity[rarity_].push(mapMonster[tokenId]);
        mapTotal[id_].totalAmount += 1;
        mapTotal[id_].remainAmount += 1;
    ...
    // 查询 tokenid 稀有度
    function getTokenIdRarity(uint tokenid_) public view returns(uint) {
        return mapMonster[tokenid_].rarity;
    }

    // 查询一种类型的 tokenid list
    function getIdList(uint id_) public view returns(Monster[] memory) {
        return mapMonsterMonsterId[id_];
    }

    // 删除以卖出的tokenid
    function delIdList(uint id_) public onlyMarket {
        uint len = mapMonsterMonsterId[id_].length;
        mapMonsterMonsterId[id_][0] = mapMonsterMonsterId[id_][len - 1];
        mapMonsterMonsterId[id_].pop();
    }
}
