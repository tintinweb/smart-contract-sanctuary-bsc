    ...
    RobinNFTStorageV1
{
    ...
    /// @param buyer who owner the NFT
    /// @param level the level of mint NFT
    /// @param howmany number of NFT to buy
    function buyID(address buyer, uint level, address buyToken, uint howmany) public payable returns (uint256[] memory) {
        // 验资
    ...
    /// @param level the level of mint NFT
    /// @param howmany number of NFT to buy
    function buyIDAdmin(address buyer, uint level, uint howmany) external returns (uint256[] memory) {
        require(howmany >= 1, "invalid howmany");
        require(msg.sender == owner() || msg.sender == admin, "no auth");
        uint256[] memory ids = new uint256[](howmany);
        for (uint i = 0; i < howmany; i ++) {
            ids[i] = _buyID(buyer, level);
        }
    ...
    function _buyID(address buyer, uint level) private returns (uint256) {
        ...
        attr.level = level;
	attr.times = 5;
        emit BuyID(buyer, tokenId, level);
        return tokenId;
    }
    ...
    /// @param sender the address of the synthesize owner
    /// @param from the tokenIds for synthesize
    /// @param rsgAmt the mint fee of RSG
    /// @param rsgcAmt the mint fee of RSGC
    /// @param resultLevel the minted token level. 0 is failed, do not mint new ID NFT
    function synthesize(
        address sender,
        uint256[] memory from,
        uint256 rsgAmt,
        ...
        uint256[] memory propsCount) public onlyOwner returns (uint256) {
        // address sender = _msgSender();

        return _synthesize(from, rsgAmt, rsgcAmt, resultLevel, sender, propsKey, propsCount);
    }
    ...
    function _synthesize(uint256[] memory from,
        ...
        require(propsKey.length == propsCount.length, "props key/value should equal");
        require(rsg.balanceOf(sender) >= rsgAmt, "not enough RSG");
        require(rsgc.balanceOf(sender) >= rsgcAmt, "not enough RSGC");

        rsg.transferFrom(sender, owner(), rsgAmt);
        rsgc.transferFrom(sender, owner(), rsgcAmt);
        ...
        uint256 tokenId = mint(resultLevel, msg.sender);
        NFTAttributes storage attr = attributesOf[tokenId];
