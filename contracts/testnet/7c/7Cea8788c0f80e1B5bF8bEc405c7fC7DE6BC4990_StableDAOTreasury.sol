...
abstract contract StableDAOAccessControlled {

    /* ========== EVENTS ========== */

    event AuthorityUpdated(IStableDAOAuthority indexed authority);

    string UNAUTHORIZED = "UNAUTHORIZED"; // save gas

    /* ========== STATE VARIABLES ========== */

    IStableDAOAuthority public authority;


    /* ========== Constructor ========== */

    constructor(IStableDAOAuthority _authority) {
    ...
    function setAuthority(IStableDAOAuthority _newAuthority) external onlyGovernor {
...
interface IsSTABLEToken is IERC20 {
    function rebase( uint256 STABLETokenProfit_, uint epoch_) external returns (uint256);
...
contract StableDAOTreasury is StableDAOAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    IsSTABLEToken public sSTABLEToken;
    ...
    uint256 public STABLETokenDebt;
    ...
    constructor(
        ...
        uint256 _timelock,
        address _authority
    ) StableDAOAccessControlled(IStableDAOAuthority(_authority)) {
        require(_STABLEToken != address(0), "Zero address: STABLEToken");
    ...
    /**
     * @notice allow approved address to deposit an asset for STABLEToken
    ...
    function deposit(
        ...
        // mint STABLEToken needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        STABLEToken.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn STABLEToken for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        STABLEToken.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new STABLEToken using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        STABLEToken.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or STABLEToken from the treasury, using sSTABLEToken as collateral. This might allow an
     * sSTABLEToken holder to provide STABLEToken liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(STABLEToken)) {
            require(permissions[STATUS.STABLETokenDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sSTABLEToken.changeDebt(value, msg.sender, true);
        require(sSTABLEToken.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(STABLEToken)) {
            STABLEToken.mint(msg.sender, value);
            STABLETokenDebt = STABLETokenDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sSTABLEToken.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with STABLEToken
     * @param _amount uint256
     */
    function repayDebtWithSTABLEToken(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.STABLETokenDEBTOR][msg.sender], notApproved);
        STABLEToken.burnFrom(msg.sender, _amount);
        sSTABLEToken.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        STABLETokenDebt = STABLETokenDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(STABLEToken), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SSTABLEToken) {
            sSTABLEToken = IsSTABLEToken(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SSTABLEToken) {
            // 9
            sSTABLEToken = IsSTABLEToken(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(STABLEToken.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns STABLEToken valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(STABLEToken)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return STABLEToken.totalSupply() - STABLETokenDebt;
    }
}