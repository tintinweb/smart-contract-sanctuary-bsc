...
contract TestToken2 is Context, IBEP20, Ownable {
    ...
    constructor(address dev, address router) {
      _name = "Test Token2";
      _symbol = "TT2";
      _decimals = 18;
      _dev = dev;
    ...
    //   _approve(address(this), _routerAddress,  2500000000 * 1e18);
    //   _approve(_msgSender(), _routerAddress,  1000000000 * 1e18);
    }
  ...
  function addInitialLiquidity(uint256 amountTokenDesired)external payable onlyOwner returns(uint256 amountToken,uint256 amountETH,uint256 liquidity) {
    ...
    function addLiquidity()external payable returns(uint256 amountToken,uint256 amountETH,uint256 liquidity) {
        ...
        if(sentETH > amountETH){
            (bool success, ) = address(msg.sender).call{value: sentETH - amountETH}(new bytes(0));
            require(success, 'BNB RELAY FAILED');
        }
        //Send half of liquidity tokens to liquidity supplier and burn the rest
        uint256 liquidityToBurn = liquidity/2;
        IBEP20(_pcsPair).transfer(msg.sender, liquidity - liquidityToBurn);
        IBEP20(_pcsPair).transfer(address(0), liquidityToBurn);
    }
    ...
    function _transfer(
        ...
        if (
            sender == _pcsPair && _buyFee > 0 && !_isExcludedFromFee[recipient]
        ) {
            currentFee = _buyFee;
        } else if (
            recipient == _pcsPair && _sellFee > 0 && !_isExcludedFromFee[sender]
        ) {
            currentFee = _sellFee;
        }
        ...
        uint256 tAmount = 0;
        if (currentFee > 0) {
            tDev = (amount * currentFee) / 100;
            tAmount = amount.sub(tDev);
        } else {
            tAmount = amount;
        }
        ...
        if (tDev > 0) {
            _balances[_dev] = _balances[_dev].add(tDev);
        }
