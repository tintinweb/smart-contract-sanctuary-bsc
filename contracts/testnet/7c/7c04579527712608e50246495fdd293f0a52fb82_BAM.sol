...
contract BAM is Ownable, ReentrancyGuard {
    ...
    function stakeBNB() external payable {
        ...
        userDetails[msg.sender].activeStake.stakeAmountHistory.push(amountAfterDeduction);
    ...
    function updateUserEarnings(address _user) private {
        ...
        if (userDetails[_user].activeStake.lastUpdated < block.timestamp) {
            if (userDetails[_user].activeStake.isActive) {
                ...
                // console.log("timePassed before addition", timePassed);

                timePassed += block.timestamp - lastUpdated;
                // console.log("timePassed after addition", timePassed);
                ...
                // console.log("persecondEarning", earningPerSecond);

                if (userDetails[_user].downlineBalance > 100 * 10**18) {
                    // console.log("Adding residualCommission");
                ...
                // console.log("earnings after Fee", earningsAfterFee);
                // ************************************************************

                // console.log(
                //     "residual commission before addition",
                ...
                // console.log(
                //     "residual commission after addition",
                //     userDetails[_user].activeStake.totalResidualCommission
                // );
    ...
    function maxAllowedWithdrawal(address _user) public view returns (uint256) {
        ...
        uint256[] memory withdrawals = userDetails[_user].withdrawalrequestedAmount;
        ...
        for (uint256 i = withdrawals.length; i > 0; i--) {
            // console.log("TIME COMP | withdrawalTime",withdrawalTime[i-1]);
            // console.log("TIME COMP | 1 day before",block.timestamp - 1 days);
            // console.log(withdrawalTime[i-1] <= (block.timestamp - 1 days));
            ...
            if (withdrawalTime[i - 1] > (block.timestamp - 1 days)) {
                ...
                // console.log("24 hour withINN",_24HourWithdrawals);
            } else {
                break;
            }
        }
        // console.log("|||||||||||||||||||||||||||||||||||||||");
        // console.log("# of withdrawals",withdrawals.length);

        // console.log("24 hour with",_24HourWithdrawals);
        // console.log("|||||||||||||||||||||||||||||||||||||||");
    ...
    function withdrawEarnings(uint256 _amount) external nonReentrant {
        ...
        userDetails[msg.sender].withdrawalAmountRecieved.push(amountAfterDeduction);
        ...
        // console.log("amountAfterDeduction", amountAfterDeduction);
        // console.log("difference", _amount - amountAfterDeduction);
        // console.log("---------------------------------------------");
