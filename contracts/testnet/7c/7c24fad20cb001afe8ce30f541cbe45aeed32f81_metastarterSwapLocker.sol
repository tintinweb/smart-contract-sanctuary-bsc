...
// ALL RIGHTS RESERVED
// metastartercrypt reserves all rights on this code. You may NOT copy these contracts.

// This contract locks metastarterSwap in the liquidity tokens. Used to give investors peace of mind a token team has locked liquidity
// and that the metastarter tokens cannot be removed from metastarterSwap until the specified unlock date has been reached.
...
interface ImetastarterSwapPair {
...
contract metastarterSwapLocker is
    Initializable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    ImetastarterFactory public metastarterSwapFactory;
    ...
    struct TokenLock {
        ...
        uint256 lockID; // lockID nonce per metastarter pair
        address owner;
    }
    ...
    mapping(address => TokenLock[]) public tokenLocks; //map metastarter pair to all its locks
    ...
    struct FeeStruct {
        ...
        uint256 liquidityFee; // fee on metastarter liquidity tokens
    ...
    function initialize() public initializer {
        ...
        metastarterSwapFactory = ImetastarterFactory(
    ...
    /**
     * @notice set the migrator contract which allows locked lp tokens to be migrated to metastarterSwap v3
     */
     ...
     * @param _lpToken the metastarter token address
     * @param _amount amount of LP tokens to lock
     * @param _unlock_date the metastarterx timestamp (in seconds) until unlock
     * @param _referral the referrer address if any or address(0) for none
     * @param _fee_in_eth fees can be paid in eth or in a secondary token such as LPCX with a discount on metastarter tokens
    ...
    function lockLPToken(
        ...
        // ensure this pair is a metastarter pair by querying the factory
        ImetastarterSwapPair lpair = ImetastarterSwapPair(address(_lpToken));
        address factoryPairAddress = metastarterSwapFactory.getPair(
            lpair.token0(),
            lpair.token1()
        );
        require(factoryPairAddress == address(_lpToken), "NOT metastarter");
        ...
        // record the lock for the metastarterpair
    ...
    function relock(
        ...
        // send metastarter fee to dev address
        TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
    }
    ...
    /**
     * @notice migrates liquidity to metastarterSwap v3
     */
    function migrate(
        address _lpToken,
        uint256 _index,
    ...
    function setSwapFactory(address _factory)
        ...
        returns (bool)
    {
        metastarterSwapFactory = ImetastarterFactory(_factory);
        return true;
    }
}
