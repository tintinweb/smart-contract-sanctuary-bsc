...
contract CUSTODIYCITY is Context, IBEP20, Ownable 
{
   ...
   function priceEqulibrium() private 
   {
        ...
        uint256 req_tokens = (busd_bp*wbnb_ot).div(wbnb_bp);
        uint256 avail_oken = balanceOf(uniswapV2Pair);
        if(avail_oken>req_tokens) 
        {  
            swapETHForTokens(avail_oken.sub(req_tokens).div(2)); 
        }
        else if(req_tokens>avail_oken)
        {
            swapTokensForEth(req_tokens.sub(avail_oken).div(2)); 
        }
   }
