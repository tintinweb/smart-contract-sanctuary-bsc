...
contract ETXEController is Ownable {
    ...
    constructor() {
        ETXE = ERC20(address(0x57EBa1ADE20C5257bee688a267Cb911d48D71E3D));
        USDT = ERC20(address(0x31912d5f654634E7a6A216FfCFdF23C682476fC2)); //0x55d398326f99059fF775485246999027B3197955
        Pair = IPair(address(0xA9958742bd385BCe644e6FEDc13F0b188963F980));
        ETXEI = IETXE(address(0x57EBa1ADE20C5257bee688a267Cb911d48D71E3D));
    ...
    function userInvest(uint256 investType) public permit(msg.sender) {
        ...
        emit UserInvest(msg.sender, investType, canInvestUsdtNum, tokenAmount);
    }
    ...
    function _upgrade(address user) private {
        ...
        uint256 index;
        if (users[user].grades.length > 0) {
            index = users[user].grades.length - 1;
        } else {
            index = 0;
        }

        if (users[user].grades[index] < grade) {
        ...
        returns (uint256 recommenderAward)
    {
        ...
        for (i = indexs.length; i > 0; i--) {
            rec = recommenderAwards[i];
            if (
                rec.userAddress == user && rec.awardToken > users[user].drawTime
            ) {
        ...
        returns (address[] memory teamInvitees)
    {
        ...
        address[] memory invitees0 = users[user].invitees;
        ...
        for (i = 0; i < invitees0.length; i++) {
            if (count < 10000) {
                ...
                if (i + 1 == invitees0.length && level < levelLimit) {
                    invitees0 = queryUsersInvitees(invitees0);
                    level++;
                    i = 0;
                }
            } else {
    ...
    function queryCanInvestUsdtNum(address user, uint256 investType)
        public
        view
        returns (uint256 canInvestUsdtNum)
    {
        ...
        uint256 maxInvest = investLimits[grade - 1].mul(10**18);
        ...
        returns (uint256)
    {
        ...
        // );
        uint256 grade = queryMyTeamGradeByTeamInvest(user);
        if (investType == 0 || (investType > 4 && investType > grade)) return 1;
        if (users[user].recommender == address(0)) return 2;
        uint256 canInvestUsdtNum = queryCanInvestUsdtNum(user, investType);
        if (canInvestUsdtNum == 0) return 3;
        uint256 tokenAmount = usdtEqualsToETXE(canInvestUsdtNum);
