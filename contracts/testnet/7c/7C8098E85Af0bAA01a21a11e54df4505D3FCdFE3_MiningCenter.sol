...
contract MiningCenter is Ownable {
    ...
    /**
     * @notice manage miningBoosterType
     ...
     * @notice get number of income token vs wbnb
     */
    function getAvailableMintedAmount(address _owner)
        public
        view
        returns (uint256)
    {
        ...
        if (boosterStatus.boosterStyle == 0) {
            if (boosterStatus.beginStamp + 5 minutes >= block.timestamp) {
                return 1;
            } else {
                return 0;
            }
    ...
    function dailyMining(uint256 _nftType, uint256 _nftNum) external {
        ...
        BoosterInfo memory boosterStatus = miningBoosterStatus[msg.sender];
        uint256 availableDays = getAvailableMintedAmount(msg.sender);
        ...
        for (
            ...
            for (
                ...
                NftHistory memory nftHistory = nftHistory[msg.sender][nftType][
                    i
                ];
                uint256 expectedMiningAmount = (nftHistory.priceBNB *
                    baseMiningRate) / 10000;
                if (nftHistory.nftLevel > 0) {
                    expectedMiningAmount =
                        (((nftHistory.priceBNB *
                            (nftUpgradeType[nftHistory.nftLevel - 1]
                                .upgradeRate + 100)) / 100) * baseMiningRate) /
                        10000;
                }
                ...
                if (
                    ...
                    _nftType == nftType &&
                    nftHistory.nftNum == _nftNum &&
                    incomeIsland.balanceOf(msg.sender, nftType) > 0
                ) {
                    totalMintedAmount = availableDays * expectedMiningAmount;
                } else if (incomeIsland.balanceOf(msg.sender, nftType) > 0) {
                    totalMintedAmount =
                        totalMintedAmount +
                        availableDays *
                        expectedMiningAmount;
