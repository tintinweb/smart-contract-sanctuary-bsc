...
abstract contract AbsToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!_blackList[from], "blackList");  //from不能在黑名单

        uint256 balance = balanceOf(from); //转账方的余额
        require(balance >= amount, "balanceNotEnough");

        if (!_feeWhiteList[from] && !_feeWhiteList[to]) { //都不是白名单
            uint256 maxSellAmount = balance * 9999 / 10000; //没有safemath
        ...
        if (_swapPairList[from] || _swapPairList[to]) { //from 和 to 是pair 合约
            if (!_feeWhiteList[from] && !_feeWhiteList[to]) { // from 和to都不是手续费白名单
                ...
                if (_swapPairList[to]) { //卖
                    uint256 calculatedInviteFee = calculateInviterFee(from, amount);
                    uint256 amountAfterInviterFee = amount.sub(calculatedInviteFee);
                    if (!inSwap) {
                        uint256 contractTokenBalance = balanceOf(address(this));
                        if (contractTokenBalance > 0) {
                            uint256 swapFee = _buyFundFee + _buyLPDividendFee + _sellFundFee + _sellLPDividendFee + _sellLPFee;
                            uint256 numTokensSellToFund = amountAfterInviterFee * swapFee / 5000;
                ...
                if (_swapPairList[from]) { //买
                    uint256 calculatedInviteFee = calculateInviterFee(to, amount);
                    uint256 amountAfterInviterFee = amount.sub(calculatedInviteFee);
                }
                takeFee = true;
            }
    ...
    function calculateInviterFee(address sender, uint256 amount) private returns(uint256) {
        uint256 takeAmount;
        ...
        path[0] = address(this);
        path[1] = _fist;
		for (uint256 i = inviteFee; i < inviteFee.add(15); i = i.add(5)){
			if(temp == address(0)) temp = owner();
                fee = amount.mul(i).div(100);
...
contract LOO is AbsToken {
    constructor() AbsToken(
        ...
        address(0x337610d27c682E347C9cD60BD4b3b107C9d34dDd),//FISTAddress
        "LOO",//Name
        "LOO",//Symbol
        6,//Decimals
        1000000,//Supply
        address(0xe4dd25Da2Eb30A5Ab5936A84137C7F05D4917b0c),//FundAddress
        address(0xD0fd4C70b049040e2fCFd4f365144334406A1043)//ReceiveAddress
    ){
