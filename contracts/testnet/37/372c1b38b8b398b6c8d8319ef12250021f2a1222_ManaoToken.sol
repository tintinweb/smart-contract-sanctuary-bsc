...
contract ManaoToken is BEP20Token {
    ...
    function weeklyClaimed(address _to, ClaimType claimType) external onlyAuthorized {
        ...
        require(_weeklyClaimedFlag[_to] == false, "Daily claim can only perform once.");
    ...
    function resetWeeklyClaimed() external onlyAuthorized {

        uint256 length = allAddresses.length;

        for (uint256 i = 0; i < length; i++) {

            _weeklyClaimedFlag[allAddresses[i]] = false;
