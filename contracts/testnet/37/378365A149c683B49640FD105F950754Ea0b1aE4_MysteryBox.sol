...
// File: contracts/OnchainRandom.sol
...
interface IOnChainRandom {
    ...
    function userNonces(address _user) external view returns (uint256 _userNonce);
...
contract OnChainRandom is IOnChainRandom, Context, Ownable, EIP712 {
    ...
    bytes32 private constant USER_ENTROPY_TYPE_HASH =
        keccak256(
            "UserEntropy(address userAddr,uint256 userNonce)"
        );
    ...
    constructor(address _authority) EIP712(NAME, VERSION) {
        require(_authority != address(0), "_authority cannot be a zero address");
        authority = _authority;
        emit AuthorityChanged(address(0), _authority);
    }

    function changeAuthority(address _newAuthority) external onlyOwner {
        require(_newAuthority != address(0), "_newAuthority cannot be a zero address");
        ...
        returns (uint256 _result) 
    {
        ...
        require(
            ...
            _userEntropy.userAddr != _authorityEntropy.requestorContractAddr, 
            "Invalid user"
        );
        require(
        ...
        require(
            keccak256(_userEntropy.userSig) == keccak256(_authorityEntropy.userSig), 
...
// This is a proof-of-concept of a mystery box using 
// the OnChainRandom contract as the random number generator

contract MysteryBox is Ownable {
    IOnChainRandom public randomMaker;

    event MBoxMinted(
        IOnChainRandom.UserEntropy indexed userEntropy, 
        IOnChainRandom.AuthorityEntropy authorityEntropy,
        uint256 indexed blockNumber,
        bytes32 indexed prevBlockHash,
        uint256 blockTimestamp,
        uint256 result
    ...
    constructor(address _randomMaker) {
        require(_randomMaker != address(0), "_randomMaker cannot be a zero address");
        randomMaker = IOnChainRandom(_randomMaker);
    }

    function buyMBox(
        IOnChainRandom.UserEntropy calldata _userEntropy, 
        IOnChainRandom.AuthorityEntropy calldata _authorityEntropy
    ) 
        external
        returns (uint256 _result) 
    {
        require(
            _userEntropy.userAddr == _msgSender(), 
            "Invalid user"
        );

        _result = randomMaker.random(_userEntropy, _authorityEntropy);

        // Use the _result as a random number for minting the mystery box nft
        // ...
        // ...
        // ...
        // ...
