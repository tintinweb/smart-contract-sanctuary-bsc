...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    */
    ...
    struct Order {
        ...
        function setupCopyrightMap(OKRoyaltyFeeManager feeManager) public onlyOwner{
        ...
        returns (bool)
        {
            ...
            if (order.feeMethod == FeeMethod.SplitFee && (order.makerProtocolFee < minimumMakerProtocolFee ||
        ...
        okRoyaltyFeeManager.calculateRoyaltyFeeAndGetRecipient(sell.target,tempData.tokenId,price);
        ...
        if (sell.feeMethod == FeeMethod.SplitFee) {
            ...
            if (sell.paymentToken == address(0)) {
                ...
                transferTokens(sell.paymentToken, sell.maker, royaltyFeeRecipient, royaltyFeeAmount);
            }
        }
        ...
        function setTransferManager(TransferNFTManager _transferManager)public onlyOwner{
        ...
        reentrancyGuard
        {
               ...
               if(delegateProxy != address(0)){
                    ...
                    require(sell.maker==tempData.nftOwner,"the sell's maker is not nftOwner!");


                    //verify to
                    tempData.result = MyTools.getSlice(37, 68, sell.calldata);
                    tempData.result = MyTools.getSlice(13, 32, tempData.result);
                    tempData.buyerAddress = MyTools.bytesToAddress(tempData.result);
                    require(buy.maker==tempData.buyerAddress,"the buy's maker is not buyerAddress!");
                    ...
                    require(tempData.transferTokenIdBuy==tempData.transferTokenIdSell,"the target tokenId is not match!");
                    ...
                    if(sell.calldata.length > 100){
                        ...
                        require(tempData.sellAmount == tempData.buyAmount,"buy amount and sell amount not match!");
                    }
                    bool callResult = transferManager.proxy(sell.target,uint(sell.howToCall),sell.calldata);
...
contract Exchange is ExchangeCore {
    ...
    payable
    {
        ...
        function validateErc( bytes32 hash,  uint8 v,
        bytes32 r,
        bytes32 s,address maker) public view returns (bool){
...
contract OKOffChainExchange is Exchange {
    ...
    function changeCopyrightReceiver(address protocolFeeAddress) public onlyOwner{
     ...
     */
    function init(ProxyRegistry registryAddress, TokenTransferProxy tokenTransferProxyAddress, ERC20 tokenAddress, address protocolFeeAddress) public {
