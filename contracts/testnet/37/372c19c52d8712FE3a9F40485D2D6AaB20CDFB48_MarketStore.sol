...
contract MarketStore is ERC1155Receiver, IERC721Receiver {
    ...
    */
    ...
    function buyMarketItem(uint256 itemIndex) public payable {
        ...
        // send BNB to seller
        payable(idToMarketItem[itemIndex].seller).transfer(price - taxSell); // transfer BNB
    ...
    {
        require(idToMarketItem[itemId].isSold, "Item is not on sale");
        idToMarketItem[itemId].isSold = false;
    }

    function restoreItem(uint256 itemId)
        external
        onlyOwnerOf(idToMarketItem[itemId].seller)
    {
        require(!idToMarketItem[itemId].isSold, "Item is on sale");
        ...
        onlyOwnerOf(idToMarketItem[itemId].seller)
    {
        ...
        require(!item.isSold, "Market: Item is on sale");
        if (item.quantity > 1) {
            // quantity > 1 => scepter
            ScepterNFTContract.safeTransferFrom(
                address(this),
