...
contract JaxFarming is Ownable{
    ...
    IERC20 constant haber = IERC20(0x0b23345140ACb88a120055c02bdaabba22dBbFa7); // 0xd6AF849b09879a3648d56B5d43c6e5908a74CA83
    ...
    function _create_farm(uint plan, uint lp_amount, uint busd_amount) internal {
        ...
        uint wjxn_in_busd = wjxn.balanceOf(address(this)) * get_price(address(wjxn), address(busd));
    ...
    function _buy_wjxn(uint busd_amount) internal view returns(uint) {
        return busd_amount / get_price(address(wjxn), address(busd));
    }

    function get_price(address token0, address token1) internal view returns(uint) {
    ...
    function harvest(uint farm_id) public {
        ...
        uint pending_reward_haber = pending_reward_busd * 1e8 / get_price(address(wjxn), address(busd));
