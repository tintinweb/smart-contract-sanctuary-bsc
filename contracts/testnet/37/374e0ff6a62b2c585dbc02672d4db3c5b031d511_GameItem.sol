...
contract GameItem is ERC721URIStorage, Ownable {
    ...
    event buyItemEvent(address _player, uint256 tokenId, uint256 amount);
    ...
    //Buy Item
    function buyItem(string memory item, string memory tokenURI)
        external
        returns (uint256)
    {
        if (abi.encodePacked(pricing[item]).length == 0) {
            require(false, "The Item don't have price yet.");
        }
        if (abi.encodePacked(quantity[item]).length == 0) {
            require(false, "The Item does not register yet.");
        }
        if (!infinity[item]) {
            require(quantity[item] > 0, "The item sold out.");
        }

        uint256 amount = pricing[item];
        ECR20.transferFrom(
            _msgSender(),
            ownerWallet,
        ...
        emit buyItemEvent(_msgSender(), newItemId, amount);
        return newItemId;
    }
