...
contract Ownable {
    ...
    event _removeAdmin(
        address indexed adminAddress, bool status
    );
    ...
    function removeAdmin(address account) public onlyOwner{
        ...
        emit _removeAdmin(account, false);
    }
...
contract BLUEGREEN is ERC20Detailed, Ownable {
    ...
    string public constant _name = "blue green";
    string public constant _symbol = "BGN";
    ...
    uint256 public buyLCBRFee = 20;

    uint256 public sellLiquidityFee = 30;
    uint256 public sellDevelopmentGuardFee = 20;
    uint256 public sellInsuranceGuardFee = 40;
    uint256 public sellRewardFee= 40;
    uint256 public sellBurnFee = 20;
    uint256 public sellLCBRFee = 30;
    ...
    uint256 blacklistElapseTime = block.timestamp.add(14 days);
    uint256 listingTime = 0;
    ...
    address public routerPairAddress;
    ...
    uint256 public publicSaleAmount = 3 * 10**6 * 10**DECIMALS;
    uint256 private amountOnOtherNetwork = 0;
    uint256 private constant TOTAL_GONS =
        MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);

    uint256 public constant MAX_SUPPLY = 50 * 10**6 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("BlueGreen", "BGN", uint8(DECIMALS)) Ownable() {
        ...
        routerPairAddress = _pair;
        dividendDistributor = new DividendDistributor();
        address _owner = msg.sender;
        
        autoLiquidityReceiver = routerPairAddress;
        developmentGuardReciever = 0x4816fEC583401c2117e924dC95BF21d37819FbD0; 
        insuranceGuardFeeReceiver = 0xE9C5378B3005a83038Db5c1cEBbd999c2333Af16;
        LCBRFeeReceiver = 0x6a705DD24522230A428E186F946065101CE833AE;
        firePit = DEAD;
        ...
        maxTransactionAmount = _totalSupply * 10 / 100; // 10% maxTransactionAmountTxn
        ...
        _transferFrom(msg.sender,address(this),publicSaleAmount);
    ...
    function rebase() public {
        ...
        uint256 fRebaseAmount = 6000000 * 10 ** DECIMALS;
        uint256 sRebaseAmount =  8000000 * 10 ** DECIMALS;
        uint256 tRebaseAmount =  16000000 * 10 ** DECIMALS;
        uint256 foRebaseAmount = 24000000* 10 ** DECIMALS;
        uint256 lRebaseAmount = 30000000 * 10 ** DECIMALS;
    ...
    function _transferFrom(
        ...
        require(!isLocked[sender],"You cannot perform transaction your token is locked");
        require(!blacklist[sender] && !blacklist[recipient] && blacklistElapseTime > _lastAddLiquidityTime, "in_blacklist" );

        //if(inSwap){ return _basicTransfer(sender, recipient, amount);}
        
       //if(routerPairAddress == sender || routerPairAddress ==recipient){
        if (shouldRebase()) {
           rebase();
        }
       ...
       //}
        ...
        // Dividend tracker
        if (!isDividendExempt[sender] && isLocked[sender]) {
        ...
        if (!isDividendExempt[recipient] && isLocked[recipient]) {
    ...
    function swapBack() internal swapping {
        ...
        amountToSwap = amountToSwap - (publicSaleAmount + totalPoolAmount + amountOnOtherNetwork);
        if( amountToSwap == 0) {
            return;
        }
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum > (_totalSupply * 1 / 1000)/1e9, "Cannot set maxTransactionAmount lower than 0.1%");
    ...
    function enableTrading() external onlyOwner {
        ...
        listingTime = block.timestamp.add(14 days);
        _initRebaseStartTime = block.timestamp;
        _lastRebasedTime = block.timestamp;
        _autoRebase = true;
    ...
    function setBuyFees (uint256 _liquidityFee, uint256 _developmentGuardFee, 
        ...
        require(listingTime > block.timestamp, "You can't change taxes now.");
    ...
    function checkTokenLock(address _addr) public view returns(bool){
        return isLocked[_addr];
    }
    ...
    uint256 period = _period * vestingPeriod * 2;
    ...
    isLocked[investor] = true;
    isDividendExempt[investor] = false;
    ...
    function unstake() public {
        ...
        isLocked[investor] = false;
        isDividendExempt[investor] = true;
    ...
    _buysCount++;
    publicSaleAmount -= totalToCollect;
    emit buysReport(msg.sender, msg.value, totalToCollect, referBy, block.timestamp);
    
    }

    function getStakerInfo(uint _id)public view returns(uint, address, address, uint,bool) {
      ...
      //  require(stakerbuys.staker==msg.sender || stakerbuys.staker==owner(),"You are not permitted to get the staker info");
        return(stakerbuys.id,stakerbuys.staker,stakerbuys.refererAddress,stakerbuys.bnbPaid,stakerbuys.releaseStatus);
    }
    ...
    function transferToOtherNetwork(uint256 amount) public{
        amountOnOtherNetwork += amount;
        _transferFrom(msg.sender,address(this), amount);
        
    }
    function transferBackToMainNetwork(address recipient, uint256 amount) public onlyAdmin{
        require(amount <= amountOnOtherNetwork,"No fund to return to user");
        amountOnOtherNetwork -= amount;
        _transferFrom(address(this), recipient, amount);
        
    }

    function getBalanceInOtherNetwork() public view returns(uint256){
        return amountOnOtherNetwork;
    }
