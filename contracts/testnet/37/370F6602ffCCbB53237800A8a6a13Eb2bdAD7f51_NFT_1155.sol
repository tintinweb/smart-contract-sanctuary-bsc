...
//TODO Implement Unicity for tokens and fix compilation errors between fathers functions
contract NFT_1155 is ERC1155, Ownable {
    ...
    function mintBatch( address to, uint256[] memory amounts ) public {
        require( amounts.length >= 1 , "ERC1155: amounts length must grater than or equal to 1" );
    ...
    function burnBatch( address from, uint256[] memory tokenIDs, uint256[] memory amounts ) public {
        require( tokenIDs.length == amounts.length, "ERC1155: tokenIDs and amounts length must be equals" );
