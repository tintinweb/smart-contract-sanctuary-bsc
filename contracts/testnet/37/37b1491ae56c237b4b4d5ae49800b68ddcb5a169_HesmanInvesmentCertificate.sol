...
contract HesmanInvesmentCertificate is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable, ReentrancyGuard {
    ...
    constructor(address _paymentToken, address _devWallet) ERC721("Hesman Invesment Certificate", "HESIC") {
        paymentToken = ERC20(_paymentToken);
        devWallet = _devWallet;
    }

    function _baseURI() internal pure override returns (string memory) {
        return "https://nft.hesman.net/info/token/";
    }
    ...
    function pauseBuy(bool _status) public onlyOwner {
        isPauseBuy = _status;
    }
    ...
    function setBlacklist(address[] calldata _users) public onlyOwner {
        for (uint256 index = 0; index < _users.length; index++) {
            isBlacklistUserMap[_users[index]] = true;
        }
    }

    function removeBlacklist(address[] calldata _users) public onlyOwner {
        for (uint256 index = 0; index < _users.length; index++) {
            isBlacklistUserMap[_users[index]] = false;
        }
    }

    function addSpecialPrice(uint256[] calldata tokens, uint256[] calldata amounts) public onlyOwner {
    ...
    function removeSpecialPrice(uint256[] calldata tokens) public onlyOwner {
        for (uint256 index = 0; index < tokens.length; index++) {
    ...
    function setLimitRange(uint256 _limit1, uint256 _limit2, uint256 _limit3) public onlyOwner {
        limitLevel1 = _limit1;
        limitLevel2 = _limit2;
        limitLevel3 = _limit3;
    }

    function setAmountBuyRange(uint256 _amount1, uint256 _amount2, uint256 _amount3) public onlyOwner {
    ...
    function safeMint(address to) public onlyOwner validSupply {
    ...
    function batchSafeMint(uint256[] calldata _numbers, address[] calldata _accounts) public onlyOwner {
        ...
        for (uint256 index = 0; index < _accounts.length; index++) {
            for (uint256 jndex = 0; jndex < _numbers[jndex]; jndex++) {
                safeMint(_accounts[index]);
            }
        }
    }
    ...
    function buyMultiNft(uint256 _number) public nonReentrant {
        for (uint256 index = 0; index < _number; index++) {
            buyNft();
        }
    }
