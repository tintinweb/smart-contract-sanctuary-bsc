...
abstract contract AbsToken is IERC20, Ownable {
    ...
    // 到达3000个币量，就进行兑换
    uint256 private numTokensSellToFund = 500 * 10**18;
  ...
  //  TokenDistributor private _tokenDistributor;
    ...
    // 半天利率1.39%，每日利率复利2.08%，分母为10000
    uint256 private _profitRate = 139;
    // 利率的分母
    uint256 private constant _profitDivBase = 10000;
    // 利率计算周期，每12小时结算
    uint256 private constant _profitPeriod = 1 minutes;
    //最近计算复利的时间
    ...
    constructor(
      ...
      /*  mainPair = ISwapFactory(_swapRouter.factory()).createPair(
        ...
        _feeWhiteList[DividendAddress] = true; // NFT钱包地址
  ...
  /*  function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        // } else {
        //     USDT.transferFrom(address(_tokenDistributor), fundAddress, usdtDividend);
        // }
    ...
    function countProfit(
        ...
        emit Test(101);
        if (amount == 0 && recipientRealAmount == 0) {
            emit Test(102);
    ...
    function calculateProfit() public {
        // 复利一年结束
        if (block.timestamp - _startTimeDeploy > 365 days) {
            return;
        }
        ...
        // 相差不到12小时，不计算复利
        if (block.timestamp - lastRewardTime < _profitPeriod) {
            return;
        }
        ...
        unchecked {
            total = (total * (_profitDivBase + _profitRate)) / _profitDivBase;
        }
        if (total < _rTotal) {
            // 超过最大值溢出了，直接忽略复利计算
            _lastRewardTime = lastRewardTime + _profitPeriod;
            return;
        }
...
// 所有参数按照需求可自行修改
contract UU12Token is AbsToken {
    constructor()
        AbsToken(
            // 名称
            "UU22Token",
            ...
            1 * 10**8,
            // 存币钱包地址
            address(0xb94097CEB0eEfbC2E1BFBed028B48f1040C728E8),
            // 营销钱包地址
            address(0xb94097CEB0eEfbC2E1BFBed028B48f1040C728E8),
            // nft钱包
            address(0xb94097CEB0eEfbC2E1BFBed028B48f1040C728E8)
