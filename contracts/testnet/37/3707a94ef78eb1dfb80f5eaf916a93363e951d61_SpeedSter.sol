...
contract SpeedSter is ISpeedSter, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "SpeedSter: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "SpeedSter: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __SpeedSter_init_unchained(_router);
    }

    function __SpeedSter_init_unchained(address _router) internal initializer {
        _name = "SpeetSter";
        _symbol = "SSTER";
        _decimals = 9;
        ...
        //exclude marketing and this contract from fee
        _isExcludedFromFee[owner()] = true;
        _isExcludedFromFee[address(this)] = true;

        __SpeedSter_tiers_init();

        emit Transfer(address(0), _msgSender(), _tTotal);
    }

    function __SpeedSter_tiers_init() internal initializer {
        _defaultFees = _addTier(0, 500, 500, 0, 0, address(0), address(0));
        _addTier(100, 200, 300, 200, 200, address(0), address(0));        
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "SpeedSter: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "SpeedSter: decreased allowance below zero"));
        return true;
    }
    ...
    //Account Included or Excluded From REWARD or FEE
    ...
    //Whitelisting and Blacklisting Addresses in Different Tiers
    ...
    preventBlacklisted(_account, "SpeedSter: Selected account is in blacklist")
    {
        require(_account != address(0), "SpeedSter: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "SpeedSter: Invalid address");
        require(_accountsTier[_account] > 0, "SpeedSter: Account is not in whitelist");
    ...
    //Fees and Addresses Check in FeeTiers
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        uint256 _fees = _tier.charityFee.add(_tier.liquidityFee).add(_tier.taxFee).add(_tier.marketingFee).add(_tier.burnFee);
        require(_fees <= _maxFee, "SpeedSter: Fees exceeded max limitation");

        return _tier;
    }

    function checkFeesChanged(FeeTier memory _tier, uint256 _oldFee, uint256 _newFee) internal view {
        uint256 _fees = _tier.charityFee
        .add(_tier.liquidityFee)
        .add(_tier.taxFee)
        .add(_tier.marketingFee)
        .add(_tier.burnFee)
        .sub(_oldFee)
        .add(_newFee);
    ...
    function setCharityFeePercent(uint256 _tierIndex, uint256 _charityFee) external onlyOwner() checkTierIndex(_tierIndex) {
        FeeTier memory tier = feeTiers[_tierIndex];
        checkFeesChanged(tier, tier.charityFee, _charityFee);
        feeTiers[_tierIndex].charityFee = _charityFee;
        if(_tierIndex == 0) {
            _defaultFees.charityFee = _charityFee;
        }
    }
    ...
    function setMarketingFeePercent(uint256 _tierIndex, uint256 _marketingFee) external onlyOwner() checkTierIndex(_tierIndex) {
        FeeTier memory tier = feeTiers[_tierIndex];
        checkFeesChanged(tier, tier.marketingFee, _marketingFee);
        feeTiers[_tierIndex].marketingFee = _marketingFee;
        if(_tierIndex == 0) {
    ...
    function setCharityFeeAddress(uint256 _tierIndex, address _charity) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_charity != address(0), "SpeedSter: Address Zero is not allowed");
        excludeFromReward(_charity);
        feeTiers[_tierIndex].charity = _charity;
        if(_tierIndex == 0) {
            _defaultFees.charity = _charity;
        }
    }

    function setMarketingFeeAddress(uint256 _tierIndex, address _marketing) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_marketing != address(0), "SpeedSter: Address Zero is not allowed");
        excludeFromReward(_marketing);
        feeTiers[_tierIndex].marketing = _marketing;
        if(_tierIndex == 0) {
            _defaultFees.marketing = _marketing;
        }
    }
    ...
    //Computing and Distributing the Fees accordingly
    ...
    function _getValues(uint256 tAmount, uint256 _tierIndex) private view returns (FeeValues memory) {
        ...
        uint256 tTransferFee = tValues.tLiquidity.add(tValues.tCharity).add(tValues.tOwner).add(tValues.tBurn);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tValues.tFee, tTransferFee, _getRate());
        return FeeValues(rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tCharity, tValues.tLiquidity, tValues.tFee, tValues.tOwner, tValues.tBurn);
    }
    ...
    function _getTValues(uint256 tAmount, uint256 _tierIndex) private view returns (tFeeValues memory) {
        ...
        tFeeValues memory tValues = tFeeValues(
            0,
            calculateFee(tAmount, tier.charityFee),
        ...
        tValues.tTransferAmount = tAmount.sub(tValues.tCharity).sub(tValues.tFee).sub(tValues.tLiquidity).sub(tValues.tOwner).sub(tValues.tBurn);
        return tValues;
    }
    ...
    //Private Internal BEP20 functions
    function _approve(
        address owner,
        address spender,
        uint256 amount
    ...
    private
    preventBlacklisted(owner, "SpeedSter: Owner address is blacklisted")
    preventBlacklisted(spender, "SpeedSter: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "SpeedSter: Address is blacklisted")
    preventBlacklisted(from, "SpeedSter: From address is blacklisted")
    preventBlacklisted(to, "SpeedSter: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    function _takeFees(address sender, FeeValues memory values, uint256 tierIndex) private {
        ...
        _takeFee(sender, values.tCharity, feeTiers[tierIndex].charity);
        _takeFee(sender, values.tOwner, feeTiers[tierIndex].marketing);
