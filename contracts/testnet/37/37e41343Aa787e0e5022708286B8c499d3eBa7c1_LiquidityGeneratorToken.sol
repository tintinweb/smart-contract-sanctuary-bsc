...
contract LiquidityGeneratorToken is IERC20, Ownable, BaseToken {
    ...
    uint256 public TotalFees  = MarketingFee.add(_liquidityFee);
    ...
    constructor(
        ...
        require(
            marketingFeeBps_ + liquidityFeeBps_  <= 10**4 / 4,
    ...
    function setMarketingFeePercent(uint256 marketingFeeBps) external onlyOwner {
        require(marketingFeeBps +  _liquidityFee < TotalFees, "Fees can only be lowered.");
        MarketingFee = marketingFeeBps;
        require(
            MarketingFee + _liquidityFee <= 10**4 / 4,
            "Total fee is over 25%"
        );
    }
    ...
    function setLiquidityFeePercent(uint256 liquidityFeeBps)
        ...
        onlyOwner
    {
         require(liquidityFeeBps + MarketingFee < TotalFees,"Fees can only be lowered.");
    ...
    function calculateMarketingFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(MarketingFee).div(10**4);
    }
    ...
    function removeAllFee() private {
        if (MarketingFee == 0 && _liquidityFee == 0) return;
    ...
    function _transfer(
        ...
        //transfer amount, it will take Marketing, burn, liquidity fee
