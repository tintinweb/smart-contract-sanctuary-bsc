...
contract DelegateMyZai is Ownable {
    ...
    modifier onlyZaiOwner(uint256 _zaiId){
        require(IZaiNFT(gameAddresses.getZaiAddress()).ownerOf(_zaiId) == msg.sender, "Not your zai");
        _;
    }

    modifier onlyFight(){
        require(gameAddresses.getFightAddress() == msg.sender, "Address not accepted");
        _;
    }
    ...
    function getNumberOfDelegationPages() external view returns(uint256){
        if(EnumerableSet.length(inDelegationProcess) == 0){
            return 0;
        }else{
            return ((EnumerableSet.length(inDelegationProcess) / 25) + 1);
        }
    }

    function getZaisInDelegation(uint256 _page) external view returns(uint256[25] memory){

        uint256[25] memory zaisDelegated;
        uint256 length = EnumerableSet.length(inDelegationProcess);
        uint256 start = (_page - 1) * 25;

        for(uint256 i = start; i < start + 25 ;){
            if(i >= length){
                break;
            }
            zaisDelegated[i - start] = EnumerableSet.at(inDelegationProcess,i);

            unchecked { ++i ; }
        }
        return zaisDelegated;
    }

    function getNumberOfScholarDelegationPages(address _scholar) external view returns(uint256){
        if(EnumerableSet.length(scholarDelegated[_scholar]) == 0){
            return 0;
        }else{
            return ((EnumerableSet.length(scholarDelegated[_scholar]) / 25) + 1);
        }
    }

    function getDelegatedToScholar(address _scholar, uint256 _page) external view returns(uint256[25] memory){
        uint256[25] memory zaisDelegated;
        uint256 length = EnumerableSet.length(scholarDelegated[_scholar]);
        uint256 start = (_page - 1) * 25;
        ...
        for(uint256 i = start; i < start + 25 ;){
            ...
            uint256 _zaiId = EnumerableSet.at(scholarDelegated[_scholar],i);
            zaisDelegated[i - start] = _zaiId;
    ...
    function getinDelegationProcessNumber() external view returns(uint256) {
        return EnumerableSet.length(inDelegationProcess);
    }
    ...
    function _isFreeForScholar(uint256 _zaiId) internal view returns (bool){
        if(!EnumerableSet.contains(inDelegationProcess, _zaiId)){
            return false;
        }else{
    ...
    function openMyZaiToDelegation(
        ...
        uint256 _contractDuration) external onlyZaiOwner(_zaiId) {
            ...
            if(!EnumerableSet.contains(inDelegationProcess, _zaiId)){
                EnumerableSet.add(inDelegationProcess, _zaiId);
            }
            if(!EnumerableSet.contains(ownerDelegated[msg.sender], _zaiId)){
                EnumerableSet.add(ownerDelegated[msg.sender], _zaiId);
            }

            m.ownerAddress = msg.sender;
            m.contractDuration = _contractDuration * 1 days;
            m.percentageForScholar = _percentageForScholar;
    }

    function setUnrenewable(uint256 _zaiId) external onlyZaiOwner(_zaiId){
        require(EnumerableSet.contains(ownerDelegated[msg.sender], _zaiId),"Zai is not delegated");
    ...
    function cancelMyZaiDelegation(uint256 _zaiId) external onlyZaiOwner(_zaiId) {
        ...
        EnumerableSet.remove(ownerDelegated[msg.sender],_zaiId);
        EnumerableSet.remove(inDelegationProcess,_zaiId);

        if(_delegateDatas[_zaiId].scholarAddress != address(0x0)){
            EnumerableSet.remove(scholarDelegated[_delegateDatas[_zaiId].scholarAddress],_zaiId);
        }
    ...
    function updateLastScholarPlayed(uint256 _zaiId) external onlyFight returns(bool){
        _delegateDatas[_zaiId].lastScholarPlayed = block.timestamp;
        return true;
    }
    ...
    function takeDelegation(uint256 _zaiId) external {
        ...
        if(m.scholarAddress != address(0x0)){
            EnumerableSet.remove(scholarDelegated[m.scholarAddress],_zaiId);
        }

        m.contractEnd = block.timestamp + m.contractDuration;
        m.lastScholarPlayed = block.timestamp;
        m.scholarAddress = msg.sender;
        EnumerableSet.add(scholarDelegated[msg.sender],_zaiId);
    }
    ...
    function renewDelegation(uint256 _zaiId) external {
        ...
        require(block.timestamp >= m.contractEnd - 1 days, "Renewable is possible only 1 days nefore contract end");
    ...
    function gotDelegationForZai(uint256 _zaiId) external view returns(ZaiStruct.ScholarDatas memory scholarDatas){
        ...
        address RnftAddress = gameAddresses.getRentMyNftAddress();

        if(RnftAddress != address(0x0)){
            scholarDatas.guildeDatas = IGuildeDelegation(RnftAddress).getRentingDatas(gameAddresses.getZaiAddress(),_zaiId);
        }

        scholarDatas.delegateDatas = _delegateDatas[_zaiId];
        if(scholarDatas.delegateDatas.ownerAddress == address(0x0)){
            scholarDatas.delegateDatas.ownerAddress = IZaiNFT(gameAddresses.getZaiAddress()).ownerOf(_zaiId);
        }
    ...
    function canUseZai(uint256 _zaiId, address _user) external view returns(bool){
        ...
        address RnftAddress = gameAddresses.getRentMyNftAddress();
        ZaiStruct.ScholarDatas memory scholarDatas;
        ...
        return(
            scholarDatas.delegateDatas.ownerAddress == _user ||
            scholarDatas.delegateDatas.scholarAddress == _user ||
            scholarDatas.guildeDatas.renterOf == _user
        );
