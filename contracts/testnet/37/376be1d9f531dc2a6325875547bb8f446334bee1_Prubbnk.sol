...
contract Prubbnk is Context, Ownable {
    ...
    //arrays
    uint256[] private daysCount = [5, 10, 15, 20, 25]; //5 days, 10 days, 15 days, 20 days, 25 days
    uint256[] private variableTax = [2000, 1500, 1000, 400, 200, 0]; //20%, 15%, 10%, 5%, 2%, 0%
    ...
    function withdraw() public isInitialized {
        ...
        if(payoutPostTax > 0 && user.totalDepositPayouts < maxPayout){ // avoid reverts
            //yieldPercentage will be deducted 1% every withdraw, starts at 3.05% else, if daily yield is less than or equal to 3% yield it goes back to 2%
            if(user.yieldPercentage >= baseYieldPercent && user.yieldPercentage <= 300) { //between 2% to 3%
                user.yieldPercentage = baseYieldPercent;        
            }
            else if(user.yieldPercentage > 300) { // 1% decrease if greater than 1.50%
