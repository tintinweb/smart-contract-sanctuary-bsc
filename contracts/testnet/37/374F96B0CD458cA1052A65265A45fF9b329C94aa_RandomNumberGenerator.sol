...
contract RandomNumberGenerator is VRFConsumerBaseV2, Ownable {
    ...
    function _shuffle() internal {
        for (uint256 i = 0; i < numWords; i++) {
            uint256 _index = s_randomWords[i] % numWords;
            if (_index % 2 == 0) {
                s_randomWords[i] = ((s_randomWords[i] % PRECISION) + (s_randomWords[_index] % PRECISION)) * (s_randomWords[i] % PRECISION);
            } else {
                s_randomWords[i] = ((s_randomWords[i] % PRECISION) + (s_randomWords[_index] % PRECISION)) * (s_randomWords[_index] % PRECISION);
            }
        }
    }
    ...
    function shuffle(uint256 _rounds) external {
        require(_rounds > 0, 'Random: Round = 0');
        for (uint256 t = 0; t < _rounds; t++) {
            _shuffle();
        }
    }

    function computerSeed(uint256 salt) external view returns (uint256) {
        uint256 seed =
        uint256(
            keccak256(
                abi.encodePacked(
        ...
        uint256 rand = s_randomWords[seed % numWords] % PRECISION;
        seed = ((seed % PRECISION + rand) % PRECISION) * rand % PRECISION;

        if (salt > 0) {
            seed = (seed + salt) % PRECISION * salt;
        }
