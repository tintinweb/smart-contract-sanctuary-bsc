...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    function create(
        ...
        // Add insId to wallet
        addInsToWallet(configuration, msg.sender);
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        finalizeInsByWallet(insConfiguration, msg.sender);
    ...
    function addInsToWallet(
        INSConfiguration memory _ins,
        ...
        for (uint256 i = 0; i < walletIns[_walletAddress].length; i++) {
            if (walletIns[_walletAddress][i].ins.insId == _ins.insId)
                insAdded = true;
        }
