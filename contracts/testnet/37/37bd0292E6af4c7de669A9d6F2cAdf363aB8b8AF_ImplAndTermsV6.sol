...
contract ImplAndTermsV6 is Storage, Ownable, DoTransfer, ERC20Init {
    ...
    function stakeInternal(address staker, uint tokenAmount, uint holdTime, address referer, address beneficiary, bool harvest) internal {
        ...
        uint stakerLPAmount = calcStakerLPAmount(amountIn, holdTime, currentRate, harvest);

        if (referer != address(0)) {
            require(holdTime >= holdTimeRefMin, "ImplAndTerms::stakeInternal: holdtime with referer must be more than holdTimeRefMin");

            uint refererBonus = calcRefererLPAmount(stakerLPAmount);
            stakeFresh(referer, 0, 0, refererBonus);

            stakerLPAmount += refererBonus;
        }
        ...
        if (harvest) {
            ...
            require(getHarvestBonusTime(holdTime) != 0, "ImplAndTerms::stakeInternal: bad hold time");
    ...
    function calcStakerLPAmount(uint amountIn, uint holdTime, uint rate, bool harvest) public view returns (uint) {
        uint lpAmount = amountIn * 1e18 / rate;
        return lpAmount + calcBonusTime(lpAmount, holdTime, harvest);
    }

    function calcBonusTime(uint lpAmount, uint holdTime, bool harvest) public view returns (uint) {
        if (harvest) {
            return lpAmount * bonusTime[holdTime] / 100e18;
        } else {
