...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    function checkLastTransfer(address account) public view returns (uint256) {
        return transferTimeFrom[account];

    }

    function checkIfQualified(address account) public view returns (uint8) {
        if (block.timestamp - transferTimeFrom[account] > 300){ //5 minute hold
            return 5;
        }
        if (block.timestamp - transferTimeFrom[account] > 240){ //4 minute hold
            return 4;
        }
        if (block.timestamp - transferTimeFrom[account] > 180){ //3 minute hold
            return 3;
        }
        if (block.timestamp - transferTimeFrom[account] > 120){ //2 minute hold
            return 2;
        }                
        else {
            return 1;
    ...
    function _transfer(
        ...
        if (_isSniper[from])
            ...
            require ((to == uniswapV2Pair
                    ...
                    && (block.timestamp >= transferTimeFrom[from] + 2 weeks), // Can only transfer every 2 weeks
        ...
        if (_isTeamWallet[from])
            ...
            require (_isExcluded[to] // Can only sell or burn, cannot transfer to others
                    ...
                    && block.timestamp >= transferTimeFrom[from] + 2 minutes, // Can only transfer every 2 minutes
        ...
        if (_isEmploymentWallet[from]) 
            ...
            require (to == Wallet_Treasury || to == uniswapV2Pair // Can only sell or transfer to Treasury
                    ...
                    && block.timestamp > transferTimeFrom[from] + 7 days, // Can only sell once every week after unlock
        ...
        if (_isDonationRecipient[to])
            ...
            require (((heldTokens + amount) <= _maxDonationRecipientHold) 
                    ...
                    && block.timestamp > transferTimeTo[to] + 36 hours) // 36 hour donation cooldown
                    ||
                    (amount == _donationLimit2 // Can only donate 2500 tokens at a time
                    && block.timestamp > transferTimeTo[to] + 8 hours) // 8 hour donation cooldown
                    ||
                    (amount == _donationLimit4 // Can only donate 500 tokens at a time
                    && block.timestamp > transferTimeTo[to] + 60 minutes) // 1 hour donation cooldown
                    ||
                    (amount == _donationLimit3 // Can only donate 100K tokens at a time
                    && block.timestamp > transferTimeTo[to] + 20 days)), // 20 days donation cooldown
                    "Donation cooldown times as follows: Donate 100K (20d), 10K (36h), 2500 (8h), 500 (1h)."
            );
            transferTimeTo[to] = block.timestamp;
        ...
        if (_isDonationRecipient[from]) 
            ...
            require ((amount == donationTransferMax && _isEmploymentWallet[to]) // Send 250,000 tokens to Employment Wallet
                    || ((amount == donationTransferMin && to == uniswapV2Pair)
                       && (block.timestamp > transferTimeFrom[from] + 7 days)) // Sell 99,900 tokens every 7 days
