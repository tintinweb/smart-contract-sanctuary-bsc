...
contract MasterKey3_4 is ERC721A, Ownable,Pausable,IMK {
 IContracts_TITIMITI Contr;
   ...
   string private URL;
    ...
    constructor( ) ERC721A("MasterKey3_4 - NFT by TITIMITI", "MK3_4", 10, 500000) {
    ...
    function MINT_Price_MasterKey_3_4 (address adr, uint256 amount)  public  {
    ...
    function SkillUP_3_4 (uint256 ID)  public {
        address sender =msg.sender;
        uint256 Sid = Skill[ID];
        uint256 skE = skillUP_c.getSkillUP_EX_3(Sid);
        uint256 EXid = EX[ID];
        uint256 W = Wear[ID];
        require(Sid<4,"You duttigli maximum in pumping this master key");
        require(EXid>=skE,"Not enough experience to level up"); 
        skillUP_c.UP_3(Sid, sender);
        EX[ID]=EXid - skE;
        Wear[ID]=W+skillUP_c.getSkillUP_Wear_3(Sid);
        Skill[ID]=Sid+1;
    }

    function WearUP_3_4 (uint256 ID, uint256 num_RepairKit) public {
         address sender =msg.sender;
        skillUP_c.GNFTtrans(num_RepairKit,sender);
        Wear[ID] = Wear[ID] + skillUP_c.GetWearPlus(num_RepairKit);
    }

    function OpeningMasterKey_3_4 (uint256 ID, uint256 ChestLevel_) public {
        uint256 OW =skillUP_c.getOpening_Wear();
        require(Wear[ID] >= OW);
        Wear[ID] =  Wear[ID]-OW;
        EX[ID] = EX[ID]+ skillUP_c.getChestLevel(ChestLevel_);  
    }
