...
contract Token is ERC20, Ownable {
    ...
    function setLimitFee(uint256 value) external onlyOwner {
        limitFee = value;
    }
    ...
    function addBot(address[] memory bots) external onlyOwner {
    ...
    function _transfer(
        ...
        bool isBot = false;
        bool isLimit = false;
        if (block.number <= addLpBlock + antiBotBlock) {
            isBot = true;
        }
        if (block.timestamp <= addLpTime + limitTime) {
            isLimit = true;
        }
        if (addLpBlock == 0 && to == uniswapV2Pair && balanceOf(to) == 0) {
            addLpBlock = block.number;
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                if (isBot) {
                    marketingFee = amount.mul(botFee.sub(buyLiquidityFee).sub(buyDeadFee)).div(100);
                }
            } else {
                // sell
                ...
                if (isLimit) {
                    marketingFee = amount.mul(limitFee.sub(sellLiquidityFee).sub(sellDeadFee)).div(100);
                }
            }
            fees = liquidityFee.add(marketingFee);
