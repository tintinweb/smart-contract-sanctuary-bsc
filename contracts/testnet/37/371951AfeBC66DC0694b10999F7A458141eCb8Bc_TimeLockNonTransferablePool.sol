...
// Credit: Great thanks to Merit Circle team for the original source code.
// Ref: https://github.com/Merit-Circle/merit-liquidity-mining
...
contract TimeLockNonTransferablePool is TimeLockPool, PublicSale, ReentrancyGuard {
    uint256 public constant STAKE_AMOUNT = 5000 ether; // 5000 TOKEN
    ...
    event DepositForWhitelist(address indexed receiver, uint256 amount);
    event WithdrawAllAvailable(address indexed receiver, uint256 amount);
    ...
    /// @notice disable direct deposit
    function deposit(uint256, uint256, address) external override pure {
        revert("DIRECT_DEPOSIT_DISABLED");
    }

    /// @notice reward not enabled
    function claimRewards(address) external override pure {
        revert("REWARDS_DISABLED");
    }
    
    /// @notice deduct token and split into 5 splits and deposit with different lock time
    /// @dev The 1st deposit will be unlocked in 1 year, 2nd will be 1 year 7 days, and so on.
    function depositForWhitelist() external whenPublicSaleActive nonReentrant {
        require(++whitelistCount <= maxWhitelistCount, "depositForWhitelist: whitelist already full");
        ...
        uint256 amountPerSplit = STAKE_AMOUNT / split; // deposit 1000 tokens each split
        ...
        emit DepositForWhitelist(_msgSender(), STAKE_AMOUNT);
    }

    /// @notice withdraw all unlocked deposits, will do nothing if no unlocked deposit
    /// @dev loop withdraw from last deposit position to ensure all deposits are visited
    function withdrawAllAvailable() external nonReentrant {
        ...
        for (uint256 i = depositLength; i > 0; i--) {
            // need to minus here to avoid underflow in loop
            uint256 depositId = i - 1; 
            Deposit memory _deposit = depositsOf[_msgSender()][depositId];

            // withdraw if unlocked
            if (block.timestamp >= _deposit.end) {
                totalWithdraw += _deposit.amount;
                withdraw(depositId, _msgSender());
            }
        }

        emit WithdrawAllAvailable(_msgSender(), totalWithdraw);
    }
}
