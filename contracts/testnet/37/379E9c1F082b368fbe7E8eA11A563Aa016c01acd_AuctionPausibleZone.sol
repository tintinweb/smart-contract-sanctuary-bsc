...
import { PausableZone } from "../zones/PausableZone.sol";
import { SeaportInterface } from "../interfaces/SeaportInterface.sol";
import { Order } from "../lib/ConsiderationStructs.sol";
import {
    AuctionPausibleZoneInterface
} from "./AuctionPausibleZoneInterface.sol";
import { ERC721 } from "@openzeppelin/contracts/token/ERC721/ERC721.sol";

contract AuctionPausibleZone is PausableZone, AuctionPausibleZoneInterface {
    function fulfillOrder(
        SeaportInterface seaport,
        Order calldata order,
        bytes32 fulfillerConduitKey,
        address bidWinner
    ) external payable isOperator returns (bool fulfilled) {
        fulfilled = seaport.fulfillOrder{ value: msg.value }(
            order,
            fulfillerConduitKey
        );

        // Send NFT to winner of auction
        ERC721(order.parameters.offer[0].token).safeTransferFrom(
            address(this),
            bidWinner,
...
interface AuctionPausibleZoneInterface {
    function fulfillOrder(
        SeaportInterface seaport,
        Order calldata order,
        bytes32 fulfillerConduitKey,
