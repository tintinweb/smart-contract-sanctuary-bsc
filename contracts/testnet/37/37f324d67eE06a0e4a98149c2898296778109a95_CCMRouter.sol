...
abstract contract TaxableRouter is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @notice Export how many tax receivers there are for a certain token.
    /// @notice Needed for external services to query over all tax receivers.
    function tokenETHTaxReceiverCount(address token) external view returns(uint count) {
        count = tokenETHTaxes[token].length;
    }
