...
contract Municipality is IMunicipality, OwnableUpgradeable {
    ...
    function initialize (address _buyAndBurn) external initializer {
    ...
    function mintParcel(
        ...
        require(
            _value >= parcelMintingPricesInBUSD[uint256(_parcel._type)],
    ...
    function sendBackendAddress(address _backend) external onlyOwner {
    ...
    function sendBundles(Bundle[6] memory _bundles) external onlyOwner {
        for (uint256 i; i < 6; i++) {
...
interface IMunicipality {
    ...
    function sendBackendAddress(address _backend) external;

    function setVBTCContractAddress(address _vbtc) external;

    function setGymContractAddress(address _gym) external;

    function sendBundles(Bundle[6] memory _bundles) external;
