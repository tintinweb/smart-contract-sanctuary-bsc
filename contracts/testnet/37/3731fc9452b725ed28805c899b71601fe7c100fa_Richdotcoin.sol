...
//Check this area very well
    ...
    function StartMintingNewCoins(uint256 _amount) external updateReward(msg.sender) {
        ...
        //restake function
            ...
            if (block.timestamp < finishAt[msg.sender]) {
                PaidAlready[msg.sender] = (rewardearned).sub(_TotaltreasuryWithdrawn[msg.sender]).add(PaidAlready[msg.sender]);
            }
            else {
                PaidAlready[msg.sender] = (treasuryClaim[msg.sender]).sub(_TotaltreasuryWithdrawn[msg.sender]).add(PaidAlready[msg.sender]);
            }
    ...
    function CoinsMintedFromTreasuryBy(address account) public view override returns (uint256) {
        ...
        return remainingClaims[account];
    }
    ...
    function withdrawTreasury(uint256 _amount) external {
        ...
        if (block.timestamp >= finishAt[msg.sender]) {
            require(_amount*10**18 < remainingClaims[msg.sender]);
