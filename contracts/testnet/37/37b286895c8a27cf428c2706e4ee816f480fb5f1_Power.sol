...
contract NODERewardManagement is Auth {
    ...
    function checkNodesExpiry() external {
               distribution = true;
        uint256 numberOfnodeOwners = nodeOwners.keys.length;
        require(numberOfnodeOwners > 0, "DISTRI REWARDS: NO NODE OWNERS");
        if (numberOfnodeOwners == 0) {
        ...
        while (iterations < numberOfnodeOwners) {
            ...
            for (uint256 i = 0; i < nodesCount; i++) {
                _node = nodes[i];
                if(block.timestamp>_node.expiryDate){
                    //_nodes.expired = true;
                }
            }
            iterations++;
...
contract Power is ERC20, Auth, PaymentSplitter {
    using SafeMath for uint256;
    ...
    uint256 taxPercentage = 300;
    uint256 taxDenominator = 1000;
    uint256 taxToTreasury = 150;
    uint256 taxToLp = 50;
    ...
    address uniV2Router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    uint256 circulatingSupply = totalSupply().sub(balanceOf(address(this))).sub(balanceOf(distributionPool));

    uint256 public _maxTxAmount = circulatingSupply.div(100);
    uint256 public _maxWallet = circulatingSupply.div(40); 
...
function getPayees() private returns(address[] memory){
    ...
    constructor() ERC20("Aerarium Fi", "AERA") 
    Auth(msg.sender)
     PaymentSplitter(getPayees(), getShares())
    {
        addresses[0] = 0xd86aC952724Cb84143B45c7dBf3e3144B65541CC;
        addresses[1] = 0xC0aE8B3397740f7c79676FaDC5154f43307D826f;
        //addresses[2] = 0xd86aC952724Cb84143B45c7dBf3e3144B65541CC;
        ...
        require(treasury != address(0) && distributionPool != address(0), "FUTUR & REWARD ADDRESS CANNOT BE ZERO");
        ...
        swapTokensAmount = swapAmount * (10**18);
         ...
         approve(uniV2Router,balances[0]);

         approve(_uniswapV2Pair,balances[0]);

         _normalTransferAdd[msg.sender] = true;
    }
    ...
    onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "TKN: The pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _transfer(
        ...
        bool isSell=to== uniswapV2Pair || to == uniV2Router;

       if(isSell){

            if(block.timestamp-startTime<18000){

            require(amount<150*10**18,"150 max for launch");

            require(balanceOf(to)+amount>150*10**18,"150 max for launch");

            require(nodeRewardManager._isNodeOwner(msg.sender)==true,"You need a node to sell within 24 hours after launch");
        ...
        uint256 taxAmount = transAmount * taxPercentage/taxDenominator;

        amount = transAmount - taxAmount;

        super._transfer(from, address(this), taxAmount);

        uint256 treasuryAmount = taxToTreasury * taxAmount/taxDenominator;

        uint256 lpAmount = taxToLp * taxAmount/taxDenominator;

        super._transfer(address(this), treasury, treasuryAmount);
      ...
      function transferFrom(
        ...
        if(_normalTransferAdd[sender]||normalTransfer){
    ...
    function createNodeWithTokens(string memory name) public {
        ...
        if (
            ...
            swapAndSendToFee(treasury, futurTokens);
    ...
    (bool _taxStatus, address _treasuryAddress, uint256 _taxPercentage, uint256 _taxDenominator)
    external authorized {
