...
contract Shop is Ownable {
    ...
    function buy(Cat _type, uint num) external payable ValidType(_type) {
        ...
        if (_type == Cat.ChanKwongYan) {
            ...
            bool ok = IBEP20(usdt).transferFrom(msg.sender, address(this), amount);
    ...
    function withdraw() external onlyOwner {
        uint balanceUSDT = IBEP20(usdt).balanceOf(address(this));
        if (balanceUSDT > 0) {
            bool success = IBEP20(usdt).transfer(msg.sender, balanceUSDT);
