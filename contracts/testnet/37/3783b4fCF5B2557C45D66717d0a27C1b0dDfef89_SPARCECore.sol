...
contract RoyaltiesV2Impl is AbstractRoyalties, RoyaltiesV2, IERC2981 {
    ...
    */
    function royaltyInfo(uint256 id, uint256 _salePrice) external virtual override view returns (address receiver, uint256 royaltyAmount) {
...
interface SPARCESpawningManager {
  ...
  function isRebirthAllowed(uint256 _sparceId, uint256 _genes) external view returns (bool);
}

interface SPARCERetirementManager {
  function isRetirementAllowed(uint256 _sparceId, bool _rip) external view returns (bool);
}

interface SPARCEMarketplaceManager {
  function isTransferAllowed(address _from, address _to, uint256 _sparceId) external view returns (bool);
}

interface SPARCEGeneManager {
  function isEvolvementAllowed(uint256 _sparceId, uint256 _newGenes) external view returns (bool);
}
...
contract SPARCECore is ERC721, SPARCEDependency {
  struct SPARCE {
    uint256 genes;
    uint256 bornAt;
  }
  ...
  event SPARCESpawned(uint256 indexed _sparceId, address indexed _owner, uint256 _genes);
  event SPARCERebirthed(uint256 indexed _sparceId, uint256 _genes);
  event SPARCERetired(uint256 indexed _sparceId);
  event SPARCEEvolved(uint256 indexed _sparceId, uint256 _oldGenes, uint256 _newGenes);

  constructor() ERC721("SPARC-E", "SPARC-E", "https://SPARC-E/") {
    owner = msg.sender;
    sparce.push(SPARCE(0, block.timestamp));
    _spawnSparce(0, msg.sender); 
    _spawnSparce(0, msg.sender); 
    _spawnSparce(0, msg.sender); 
    _spawnSparce(0, msg.sender); 
  }
  ...
  function getSparce(
    ...
    external
    view
    mustBeValidToken(_sparceId)
    returns (uint256 /* _genes */, uint256 /* _bornAt */)
  {
    SPARCE storage _sparce = sparce[_sparceId];
    return (_sparce.genes, _sparce.bornAt);
  }

  function spawnSparce(
    uint256 _genes,
    address _owner
  )
    external
    ...
    returns (uint256)
  {
    return _spawnSparce(_genes, _owner);
  }

  function rebirthSparce(
    uint256 _sparceId,
    uint256 _genes
  )
    external
    onlySpawner
    mustBeValidToken(_sparceId)
    whenRebirthAllowed(_sparceId, _genes)
  {
    SPARCE storage _sparce = sparce[_sparceId];
    _sparce.genes = _genes;
    _sparce.bornAt = block.timestamp;
    emit SPARCERebirthed(_sparceId, _genes);
  }

  function retireSparce(
    uint256 _sparceId,
    bool _rip
  )
    external
    onlyByeSayer
    whenRetirementAllowed(_sparceId, _rip)
  {
    _burn(_sparceId);

    if (_rip) {
      delete sparce[_sparceId];
    }

    emit SPARCERetired(_sparceId);
  }

  function evolveSparce(
    uint256 _sparceId,
    uint256 _newGenes
  )
    external
    onlyGeneScientist
    mustBeValidToken(_sparceId)
    whenEvolvementAllowed(_sparceId, _newGenes)
  {
    uint256 _oldGenes = sparce[_sparceId].genes;
    sparce[_sparceId].genes = _newGenes;
    emit SPARCEEvolved(_sparceId, _oldGenes, _newGenes);
  }

  function _spawnSparce(uint256 _genes, address _owner) private returns (uint256 _sparceId) {
    sparce.push(SPARCE(_genes, block.timestamp));
    _sparceId = sparce.length-1;
    _mint(_owner, _sparceId);
    emit SPARCESpawned(_sparceId, _owner, _genes);
  }
}