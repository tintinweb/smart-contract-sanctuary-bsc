...
contract Shinu is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 300_000_000 * 10**9;
    ...
    address public marketingWallet = 0xE3F903e2D98e55098837aB6782EAe967f67b9c08;
    address public developmentWallet = 0x68186B0100e7795De10942Af4f5e604C10CBB6c8;
    address public charityWallet = 0x9e017A6c541387997264446860747Fff35502656;
    ...
    Taxes public buyTaxes = Taxes(0, 3, 1, 1, 0);
    Taxes public sellTaxes = Taxes(0, 4, 1, 1, 1);
    ...
    function setTradingStatus(
        ...
        swapEnabled = state;
        deadline = _deadline;
        launchtax = Taxes(rfi, marketing, liquidity, development, charity);
    ...
    function setBuyTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _charity) public onlyOwner {
        require(_rfi <= 1, "Reflection Tax on buys can not be higher than 1%");
        require(_marketing <= 10, "Marketing Tax on buys can not be higher than 10%");
        require(_liquidity <= 3, "Liquidity Tax on buys can not be higher than 3%");
        require(_development <= 4, "development Tax on buys can not be higher than 4%");
        require(_charity <= 4, "charity Tax on buys can not be higher than 4%");
       
       buyTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_charity);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _development, uint256 _charity) public onlyOwner {
        require(_rfi <= 1, "Reflection Tax on sells can not be higher than 1%");
        require(_marketing <= 10, "Marketing Tax on sells can not be higher than 10%");
        require(_liquidity <= 3, "Liquidity Tax on sells can not be higher than 3%");
        require(_development <= 4, "development Tax on sells can not be higher than 4%");
        require(_charity <= 4, "charity Tax on sells can not be higher than 4%");
       
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_development,_charity);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rdevelopment, to_return.rcharity) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell, bool useLaunchTax) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tdevelopment-s.tcharity;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rdevelopment-rcharity;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rdevelopment,uint256 rcharity) {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 10) {
            require(to != pair, "Sells not allowed for first 10 blocks");
        }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tdevelopment+ s.tcharity);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.development + temp.charity) * 2;
        ...
        uint256 buybackAmt = unitBalance * 2 * temp.charity;
        if(buybackAmt > 0){
            payable(charityWallet).sendValue(buybackAmt);
        }
