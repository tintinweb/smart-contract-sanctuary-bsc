...
// File: contracts/Digital Luxury Token.sol


pragma solidity ^0.8.0;
contract DECA_Token is ERC20, Ownable {
    ...
    constructor() ERC20("DECA TOKEN", "DECAT") {
    ...
    function claimAirdrop(address _refer) public returns (bool success){
      ...
      //  _burn(address(this), ((aAmt * 150) / 100)); //burn amount 150% greater than claim amount--we are Digital Luxury Token!!!
        return true;
      }
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are Digital Luxury Token!!!
    sTot ++;
  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function LPDigitalLuxuryTokenLocked_Token(IERC20 token, uint256 values) public onlyOwner {
        address payable _owner =  payable(msg.sender);
