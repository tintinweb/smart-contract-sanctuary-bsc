...
contract CryptoPet is Context, Ownable, ERC721Burnable {
	using Counters for Counters.Counter;

	// Maps the token ID to a base64 string of the TomoPunk
	...
	// Maps addresses to the number of TomoPunks they have minted
    mapping(address => uint) private minted;

	// Maximum number of tokens that can be minted
    uint32 public constant MAX_MINT = 100;

	// Required value to transfer to mint a TomoPunk 
    uint256 public constant PUNK_PRICE = 0 ether;
	...
	constructor () ERC721('Crypto Pet', 'CPT'){
		for(uint i = 0; i < 20; i++) {
            mint(msg.sender);
        }
	}
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{ "name": "TestPunk #', Strings.toString(tokenId), '", "image": "', bytes(getPunkImage(tokenId)), '", "attributes": [', getPunkAttributes(tokenId), '] }'))));
    ...
    function mint(address to) public virtual payable {
        ...
        require(msg.value >= PUNK_PRICE || msg.sender == owner(), "Error: Minting costs 2 TOMO!");
