    ...
    UUPSUpgradeable
{
    ...
    function _createOrder(
        ...
        if (
            isHuukNFTs[_tokenAddress] &&
            IHuukNFT(_tokenAddress).getCreator(_tokenId) == _owner &&
            amountFirstSale[_tokenAddress][_tokenId] == 0 &&
            lastBuyPriceInUSDT[_owner][
    ...
    function _buy(
        ...
        if (_paymentToken == order.paymentToken) {
            exactPaymentAmount = orderAmount.mul(ZOOM_FEE + xUser).div(
                ZOOM_FEE
            );
        } else {
        ...
        if (_paymentToken == huuk && discountForHuuk > 0) {
            exactPaymentAmount = exactPaymentAmount.sub(
                orderAmount.mul(discountForHuuk).div(ZOOM_FEE)
            );
        }
    ...
    returns (bool)
    {
        return _acceptBid(msg.sender, _bidId, _quantity);
    }
    ...
    function _match(
        ...
        if (buyerRef != address(0)) {
            ...
            if (discountForBuyer > 0) {
                ...
                _paid(
                    _paymentToken,
                    _buyer,
                    orderAmount.mul(discountForBuyer).div(ZOOM_FEE)
                );
            }
        }
        if (isHuukNFTs[order.tokenAddress]) {
            ...
            if (
                ...
                if (sellerRef != address(0)) {
                    {
                        ...
                        if (
                            ...
                            amountToSellerRef = orderAmount
                                .mul(xCreator)
                                .mul(ZOOM_FEE - yRefRate)
                                .div(ZOOM_FEE**2); // take 15%*50% fee
                            amountToSeller = amountToSeller.sub(
                                orderAmount.mul(xCreator).div(ZOOM_FEE)
                            );
                            // take 15% fee
                            amountFirstSale[order.tokenAddress][
                                order.tokenId
                        ...
                        } else {
                            {
                                ...
                                a = a.mul(ZOOM_FEE - yRefRate).div(ZOOM_FEE ** 2);
                                amountToSellerRef = amountToSellerRef.add(a);
                            }
                        }
                    ...
                    if (
                        ...
                        uint256 b = amountFirstSale[order.tokenAddress][
                            order.tokenId
                        ].mul(_price).mul(xCreator).div(ZOOM_FEE);
                ...
                amountToSeller = amountToSeller.sub(
                    orderAmount.mul(xUser).div(ZOOM_FEE)
                );
                if (sellerRef != address(0)) {
                    _paid(
                        _paymentToken,
                        sellerRef,
                        orderAmount.mul(xUser).mul(ZOOM_FEE - yRefRate).div(
                            ZOOM_FEE ** 2
                        )
                    );
                }
    ...
    bytes32 public constant BID_HASH =
        keccak256(
            "Bid(uint256 amount,uint256 price,bytes signature)"
        );
    ...
    function lazyBuy(
        ...
        require(_amount <= _order.amount, "buy-amount-exceed");

        uint256 tokenId;

        if (_order.token.isERC721) {
            tokenId = IHuuk721(_order.token.token).create(
                _order.token.owner,
                _order.token.uri,
                _order.token.royaltyFee,
                _order.token.nonce,
                _signature
            );
        } else {
            tokenId = IHuuk1155(_order.token.token).create(
                _order.token.owner,
                _order.token.maxSupply,
                _order.token.initialSupply,
                _order.token.royaltyFee,
                _order.token.uri,
                bytes(""),
                _order.token.nonce,
    ...
    function lazyBid(
        ...
        uint256 _price,
        bytes calldata _signature,
        bytes calldata _bidSignature
    ) external payable whenNotPaused returns (bool) {
        require(
            _recoverBid(_amount, _price, _signature, _bidSignature) ==
                _order.token.owner,
            "signer-owner-mismatched"
        );
        bytes32 _id = keccak256(
            abi.encodePacked(
                _order.token.token,
...
//                _order.price
//            );
            _createOrderStackTooDeep(_order);
        }
        ...
        return _acceptBid(_order.token.owner, bidId, _amount);
    }

    function _createOrderStackTooDeep(LazyOrder memory _order) internal whenNotPaused returns (bool) {
        _createOrder(
            _order.token.owner,
            _order.token.token,
            _order.paymentToken,
    ...
    function _recoverBid(
        ...
        bytes32 ethTypedDataHash = _hashTypedDataV4(
            _getBidHash(_amount, _price, _signature)
        );
        address signer = ECDSAUpgradeable.recover(ethTypedDataHash, _bidSignature);
    ...
    function _getBidHash(
        ...
        return keccak256(abi.encode(BID_HASH, _amount, _price, _signature));
    }
