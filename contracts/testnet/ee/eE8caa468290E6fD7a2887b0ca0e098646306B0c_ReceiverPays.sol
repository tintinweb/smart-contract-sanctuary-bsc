...
contract ReceiverPays is Ownable {
    ...
    function claimPayment(uint256 amountBGS, uint256 amountFP, string memory nonce, bytes memory signature) external {
        ...
        if(amountBGS>0) rewardsTokenBGS.transfer(msg.sender, amountBGS*10**decimalTokenBGS);

        if(amountFP>0) rewardsTokenFP.transfer(msg.sender, amountFP*10**decimalTokenFP);
    }
    ...
    function getUsedNonce(string memory nonce)
        ...
        returns (bool)
    {
        if(usedNonces[nonce] == true) return true;
        else return false;
        //return false;
    }
}