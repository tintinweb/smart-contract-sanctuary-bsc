...
contract sifuInuStaking is Ownable, ReentrancyGuard{
  ...
  //  using SafeBEP20 for IBEP20;
   ...
   // Invest[] public invests;
    ...
    function deposit(uint256 _amount) external nonReentrant {
     ...
     function transferToken(uint256 _amount, IBEP20 _token, address _to) external nonReentrant {
    ...
    function sendToken(address investorAddress, uint256 amount) private{
     ...
     token.transfer(investorAddress, amount);
      //  restoreAllFee();
    }
    ...
    function releaseToken(address investorAddress, uint256 amount) private{
       // IBEP20 token = IBEP20(token);
        // removeAllFee();
        //uint256 allowance = token.allowance(address(this), investorAddress);
        //require(allowance >= amount, "Check the token allowance");
        //token.transferFrom(address(this), investorAddress, amount);
        // restoreAllFee();
    }

    function calculateProfit(uint256 amount, uint256 _interestRate) public pure returns (uint256){
        return amount + ((_interestRate *  amount) / (100*100)) ;
    }
    ...
    function claimInvestment(uint256 _id) public {
        ...
        token.transfer(investor, totalEarned);
 ...
 //   STORE DATA
    ...
    function recoverTokensFromContract(address _tokenAddress, uint256 _amount) external onlyOwner {                               
        ...
        // Withdraw ERC20 tokens that are potentially stuck to users
    function sendTokenFromContract(address _tokenAddress, address recipient, uint256 _amount) external {                               
         uint256 balance = IBEP20(_tokenAddress).balanceOf(recipient);
        require(balance >= _amount, "insufficient balance");
        IBEP20(_tokenAddress).transfer(recipient, _amount);
    }
