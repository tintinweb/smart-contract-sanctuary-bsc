...
// ERC721URIStorage,
    ...
    ///@notice total limitations of nft that can be minted
    uint256 public limitedSupply;

    ///@notice blockNumber when contract is deployed
    uint256 public blocknumber;

    ///@notice Price to be paid while minting nft
	...
	event NftBurned(
     ...
     constructor(
        ...
        limitedSupply = _limitation;
        blocknumber = block.number;  
        mintingPrice = _price;
		artistRoyalty = _artistfee;
		artist = _artist;
		transferOwnership(_owner);
		for(uint256 i =0; i <_whitelist.length; i++){
			whitelistAddress.add(_whitelist[i]);

		}
		lockedTime= calculateTime(timelimit);
		baseUri = _uri;
	...
	modifier onlyWhitelisted(address _minter){
		if(block.timestamp <= lockedTime){
			require(whitelistAddress.contains(_minter),"Not authorized");
		}

		// require((block.timestamp <= lockedTime && whitelistAddress.contains(_minter)),"Not authorized");
		_;
	} 
	...
	///@dev anyone can mint
    function mint(string calldata _tokenUri, uint256 _quantity) onlyWhitelisted(msg.sender) public payable {
		uint256 newId = _tokenIds.current();
		require(newId <= (limitedSupply-1),"Total limit exceed");
        checkAmount(msg.sender);
        _tokenIds.increment();
		transferfees(payable(owner()), mintingPrice);
		setArtistRoyalty(newId, artist, uint96(artistRoyalty));
    ...
    ///@notice change Minting Price
    ///@param _price new minting price to set
    ///@dev only the owner can change the base minting price
    function changeMintingPrice( uint256 _price) public onlyOwner() 
	{
        mintingPrice = _price;
    }

	function getWhitelist() public view returns(address [] memory){
		 address [] memory addresses = new address[](whitelistAddress.length());
		 for(uint256 i =0;i<whitelistAddress.length();i++){
			addresses[i] = whitelistAddress.at(i);
		 }
		 return addresses;
	...
	function removeWhitelist(address [] memory _address) public onlyOwner() {
		for(uint256 i =0;i<_address.length; i++){
			whitelistAddress.remove(_address[i]);
		}
	}

	function addWhiteListAddress(address [] memory _whitelist) public onlyOwner() {
		for(uint256 i =0; i <_whitelist.length; i++){
			whitelistAddress.add(_whitelist[i]);
		}
	}
	...
	///@param buyer address of the minter/buyer
	///@dev checks the value sent with selling price and return excessive amount to the buyer address
	function checkAmount( address buyer) internal {
		require(msg.value >= mintingPrice, 'Insufficient amount');
		uint256 amountToRefund = msg.value - mintingPrice;
		transferfees(payable(buyer), amountToRefund);
	}


	function calculateTime(uint256 time) internal view returns(uint256){
		return (block.timestamp + time * 1 minutes);
	}
	...
	function transferfees(
		...
		require(txSuccess, 'Failed to pay');
	}
