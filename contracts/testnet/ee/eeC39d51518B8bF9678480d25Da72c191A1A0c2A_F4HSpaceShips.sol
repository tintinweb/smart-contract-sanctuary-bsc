...
contract F4HSpaceShips is ERC721, Ownable{
    ...
    uint256 public mintPriceInWeiF4H;
    ...
    event LogSetMintPriceInWeiF4H(uint256 price);
    ...
    constructor(
        ...
        uint256 _mintPriceInWeiF4H,
        address _f4h) 
        ERC721(name, ticker) {
        require(_mintPriceInWeiBNB != 0, "MintPriceInWeiBNB cannot be 0");
        require(_mintPriceInWeiF4H != 0, "MintPriceInWeiBNB cannot be 0");
        baseURI = _baseURI;
        mintPriceInWeiF4H = _mintPriceInWeiF4H;
    ...
    function setMintPriceInWeiF4H(uint256 _mintPriceInWeiF4H) external onlyOwner{
        require(mintPriceInWeiF4H != _mintPriceInWeiF4H, "Already set to this Value");
       
        mintPriceInWeiF4H = _mintPriceInWeiF4H;
        emit LogSetMintPriceInWeiF4H(_mintPriceInWeiF4H);
    }

    function setShipLevel(uint256 tokenId, uint256 _level) public onlyOwner{
    ...
    function mint(uint256 num) external payable {
        if(msg.value == 0){
           uint256 mintPriceF4H = num * mintPriceInWeiF4H;
           require(IERC20(f4h).balanceOf(msg.sender) >= mintPriceF4H, "Not enough F4H sent");
           require(IERC20(f4h).allowance(msg.sender, address(this)) >= mintPriceF4H, "Not enough f4h sent");
           for(uint256 i=0; i < num; i++){
            ...
            IERC20(f4h).transferFrom(msg.sender, address(this), mintPriceF4H);
        }else{
