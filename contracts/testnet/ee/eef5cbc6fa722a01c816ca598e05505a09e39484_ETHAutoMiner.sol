...
contract ETHAutoMiner is Context, Ownable {
    address eth = 0x5FD6eB55D12E759a21C09eF703fe0CBa1DC9d88D;
    ...
    uint256 private nextActionTime = 3600 * 24 * 7;
    uint256 private actionTime = 3600 * 24;
    ...
    //This function calculates the compound amount, 
    //This function should be called first before we can re-deposit the amount 
    function compound() public {
        checkState();
        
        uint256 amount = getRewardsSinceLastDeposit(msg.sender);
        uint256 maxCompound = users[msg.sender].realDeposit * (1 + maxCompoundPercent / 100);

        require(users[msg.sender].deposit < maxCompound, "err: shouldn't compound over max compound");
        
        if (users[msg.sender].deposit + amount > maxCompound) {
            amount = maxCompound - users[msg.sender].deposit;
        }
    ...
    //Collect
    //collects the amount we have deposited
...
//Monthly Return: 3%
    ...
    //Deposit BUSD
    //This Function is used to deposit the amount
    function deposit(
        //address ref, 
        ...
        users[msg.sender].withdraw += rewards;
...
//Re-Deposit  
//This Function re-deposits the deposit ammount with bouns and rewards that user recvied 
...
//this function transfer bonus of user 
...
// This fuction should be called first before start deposting
