...
contract DepositContractQF {
    ...
    constructor(address _dataContract1, address _dataContract2) {
        iDataContractQF = IDataContractQF(_dataContract1);
        iDataContractQF2 = IDataContractQF2(_dataContract2);
    }

    function setDataContractAddress(
        address _dataContract1,
        address _dataContract2
    ) external {
    ...
    function deposit(
        ...
        iDataContractQF.updateAddressOnUserInfo(_pid, msg.sender, _referral);
        ...
        require(
            userInfo.totalAmount.add(_amount) > iDataContractQF2.getMinimumDepositSize(_pid),
            "Deposit is less than the minimum required"
        );

        (bool poolIsPrivate, bool isUserThere) = iDataContractQF2
            .getPoolIsPrivateForUser(_pid, msg.sender);
        if (poolIsPrivate) {
            require(isUserThere, "User does not have pre-approval");
        ...
        iDataContractQF.addDepositDetailsToDataContract(
            QueueFinanceLib.AddDepositModule({
                addDepositData: QueueFinanceLib.AddDepositData({
                    ...
                    sender: _sender,
                    prevSeqId: _updatedForLevelDeposits
                        .depositInfo
                        .previousSequenceID,
                    poolTotalStaked: _allDepositData.poolInfo.totalStaked,
                    poolLastActiveSequence: _allDepositData
                        .poolInfo
                        .lastActiveSequence,
