...
contract LodicoinToken is ERC20Detailed, Ownable {
    ...
    function takeFee(
        ...
        LCBR_TOKEN_AMOUT += gonAmount.mul(_LCBRFee).div(100);
    ...
    function calculateProfit(uint256 _stakeAmount) public view returns (uint256){
        ...
        return(
            (((_stakeAmount * 100 / totalPoolAmount)) * LCBR_TOKEN_AMOUT) / 100
        );
    }
    ...
    function getInvestmentRecord(address addr) public view returns(uint256, uint256, uint256, bool){
        return (stakes[addr].amountLock, stakes[addr].lockTime, stakes[addr].unLockTime, stakes[addr].releaseStatus);
    }
    ...
    function shareProfit() public {
        ...
        if(LCBR_TOKEN_AMOUT > 0 && numberOfStakers > 0){
        for(uint256 i = numberOfStakers; i >= 0; i--){
            invest = stakes[allStakers[i]];
            uint256 stakerAmount = invest.amountLock;
            if(stakerAmount > 0 ){
               uint reward = calculateProfit(stakerAmount);
               if(LCBR_TOKEN_AMOUT > reward){
               invest.amountEarned += reward;
               LCBR_TOKEN_AMOUT -= reward;
               }
               else{
                invest.amountEarned += LCBR_TOKEN_AMOUT;
                LCBR_TOKEN_AMOUT -= LCBR_TOKEN_AMOUT;
                break;
               }
