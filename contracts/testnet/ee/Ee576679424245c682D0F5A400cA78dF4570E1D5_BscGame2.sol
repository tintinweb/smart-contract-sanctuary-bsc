...
contract BscGame2 {
        ...
        if(leaderStart==0){
            if(reward_array.length>0){
                ...
                for(uint8 i = 0; i < reward_array.length; i++) {
                    ...
                    userInfo[reward_array[i]].incomeArray[10]+=distLAmount;
                }
                leaderStart=1;
            }
    ...
    function withdraw(uint256 _amount) public{
        ...
        require(bonus >= 1e18, "You cross you limit amount");
        player.totalWithdraw+=bonus;
        uint256 tempSplit=bonus.mul(30).div(100);
        player.split+=tempSplit;
        uint256 wamount=bonus.sub(tempSplit);
        uint256 adminCharge=bonus.mul(5).div(100);
        wamount=wamount.sub(adminCharge);
        busd.transfer(msg.sender,wamount);
        busd.transfer(feeReceiver,adminCharge);
    }
