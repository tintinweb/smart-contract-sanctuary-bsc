...
contract RICKROLLED is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("We're no strangers to love\nYou know the rules and so do", "RICKROLLED") {
        ...
        uint256 totalSupply = 690000 * 1e18;

        maxBuyAmount = (totalSupply * 69) / 10000; // 0.69% maxTransactionAmountTxn
        maxSellAmount = (totalSupply * 69) / 10000; // 0.69% maxTransactionAmountTxn
        maxWalletAmount = (totalSupply * 69) / 10000; // 0.69% maxWallet
        thresholdSwapAmount = totalSupply  / 10000; // 0.01% swap wallet
        buyTotalFees = 69;
    ...
    function updateFees(uint256 _buyTotalFees, uint256 _sellTotalFees) external onlyOwner{
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            // on sell
            if (marketPair[recipient] && sellTotalFees > 0) {
                fees = amount.mul(sellTotalFees).div(1000);
            }
            // on buy
            else if (marketPair[sender] && buyTotalFees > 0) {
