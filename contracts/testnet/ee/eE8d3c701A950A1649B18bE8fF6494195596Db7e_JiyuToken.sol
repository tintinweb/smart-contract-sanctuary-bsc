...
contract JiyuToken is Context, IBEP20, LockToken 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public stakingAddress = payable(0x8e7c0Be454cbc94640BC31116e04ac3a4F461184);
    ...
    uint256 public _stakingFee = 4;
    uint256 private _previousMarketingFee = _stakingFee;

    uint256 _saleLiquidityFee = 4;
    ...
    constructor () 
    {
        ...
        uniswapV2Pair = IUniswapV2Factory(0x6725F303b657a9451d8BA641348b6761A6CC7a17)
        ...
        _isExemptFromTxLimit[stakingAddress] = true;
    ...
    {
        uint256 _totalFees = _liquidityFee.add(_stakingFee).add(_saleLiquidityFee).add(_saleMarketingFee);

        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee+_saleLiquidityFee).div(_totalFees).div(2);
        uint256 swapableTokens = contractTokenBalance.sub(tokensForLiquidity);
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableTokens); 

        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 ethForLiquidity = newBalance.mul(_liquidityFee+_saleLiquidityFee).div(_totalFees).div(2);
        ...
        stakingAddress.transfer(ethForMarketing);
    ...
    {
        return _amount.mul(_liquidityFee+_stakingFee).div(100);
    }
    ...
    function restoreAllFee() private 
    {
        ...
        _stakingFee = _previousMarketingFee;
    }

    function setSaleFee() private 
    {
        _liquidityFee = _saleLiquidityFee;
        _stakingFee = _saleMarketingFee;
    }
    ...
    function prepareForPresale() public onlyOwner 
    {
        ...
        _stakingFee = 0;
        _previousMarketingFee = _stakingFee;
