...
contract BNFT is IBNFT, ERC721WithPermit, ERC721URIStorage, ERC721Enumerable, ERC721Burnable, ERC721TransferFee, Ownable, Creator, Lockable, NonFungibleType, PrimarySale, ReentrancyGuard {
    ...
    constructor(string memory name_, string memory symbol_) payable ERC721WithPermit(name_, symbol_) {
        address sender = _msgSender();
        _setCreator(sender);
        _setupPrimarySaleRecipient(sender);
        _setTransferFee(0xC1f998491A29daB5AB41699872A85E731082E25a, sender, 0);
        _setType(1, address(0), 10000000000000000, 10, 2);
        _setType(2, address(0), 20000000000000000, 10, 2);
        _setType(3, address(0), 30000000000000000, 10, 2);
        _setType(4, address(0), 40000000000000000, 10, 3);
        _setType(5, address(0), 50000000000000000, 10, 1);
        _idCounter = 1;
    }
    ...
    function buy(uint256 typeNFT_, uint256 quantity_) external payable override nonReentrant {
        ...
        if (typeInfo.paymentToken == address(0)) {
            if (msg.value < total) revert NFT__InsufficientBalance();
            CurrencyTransferLib.safeTransferNativeToken(_recipient, total);
        } else {
            CurrencyTransferLib.safeTransferERC20(typeInfo.paymentToken, sender, _recipient, total);
        }

        _setSold(typeNFT_, quantity_);
        _batchMint(sender, typeNFT_, quantity_);
    }

    function _batchMint(address account_, uint256 typeNFT_, uint256 quantity_) internal {
        uint256 totalMint = quantity_ * _typeInfo[typeNFT_].quantity;
        uint256 tokenId = _idCounter;
        uint256 creator_;
        creator_ = _creator << 96;

        emit Registered(account_, typeNFT_, tokenId, totalMint, _sold[typeNFT_]);

        for (uint256 i; i < totalMint; ) {
            unchecked {
    ...
    function _transfer(address from_, address to_, uint256 tokenId_) internal override(ERC721, ERC721WithPermit) {
        super._transfer(from_, to_, tokenId_);
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721, ERC721Enumerable, ERC721TransferFee) {
        super._beforeTokenTransfer(from, to, tokenId, batchSize);
    }

    function supportsInterface(bytes4 interfaceId) public view override(ERC721, ERC721Enumerable, ERC721WithPermit) returns (bool) {
...
interface IBNFT {
    ...
    event Registered(address indexed user, uint256 indexed typeNFT, uint256 tokenId, uint256 totalMint, uint256 soldByType);
...
abstract contract ERC721TransferFee is BusinessAddresses, ERC721 {
...
import { Strings, ERC721 } from "@openzeppelin/contracts/token/ERC721/ERC721.sol";
...
abstract contract ERC721WithPermit is ERC721, IERC4494 {
...
abstract contract PrimarySale is IPrimarySale {
    ...
    function setPrimarySaleRecipient(address saleRecipient_) external override {
