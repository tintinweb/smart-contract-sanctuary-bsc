...
contract EverSAFU is ERC20Detailed, Ownable {
    ...
    uint256 public safuDividendFee = 80;
    ...
    function takeFee(
        ...
        uint256 _safuDividendFee = safuDividendFee;
        uint256 _sell = 10;

        if (recipient == pair) {
            _totalFee = totalFee.add(sellFee);
            _treasuryFee = treasuryFee.add(_sell);
            _safuDividendFee = safuDividendFee.add(_sell);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(_safuDividendFee))
        );
