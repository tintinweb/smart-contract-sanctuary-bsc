...
https://bscscan.com/address/0x94D8a26620C1874cD2df91110585DEAAf88B686E#code
...
contract KwagoInu is IERC20, Ownable {
    using SafeMath for uint256;

    address constant ROUTER        = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address constant WBNB          = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address constant DEAD          = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO          = 0x0000000000000000000000000000000000000000;

    string _name = "PI Kwago";
    string _symbol = "KWAGO";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 100_000_000 * (10 ** _decimals);
    uint256 public _maxWalletSize = (_totalSupply * 1) / 1000;   // 1% 
    ...
    // *
    uint256 bliquidityFee = 1; 
    uint256 bgiveawayFee = 1;  
    uint256 bmarketingFee = 1;   
    uint256 breflectionFee = 1;   
    uint256 btotalFee = 2;  
    uint256 bfeeDenominator = 100; 
    // *
    uint256 sliquidityFee = 2; 
    uint256 sgiveawayFee = 2;  
    uint256 smarketingFee = 2;   
    uint256 sreflectionFee = 2;   
    uint256 stotalFee = 4;  
    uint256 sfeeDenominator = 100; 
    // *
    uint256 _lpBurnFee = 10;
    uint256 _lpLockedFee = 90;  
    ...
    constructor () {
        ...
        marketingFeeReceiver = 0xe0306F2a52E62235A11b46183Fa9ad3Fb960bE4E;
        giveawayFeeReceiver =  0x2158c0B3967490640917650D31B1CdF13067B87f;
    ...
    // *
    function viewBFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (bliquidityFee, bmarketingFee, bgiveawayFee, breflectionFee, btotalFee, bfeeDenominator);
    }
    // *
    function viewSFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (sliquidityFee, smarketingFee, sgiveawayFee, sreflectionFee, stotalFee, sfeeDenominator);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // *
        if(shouldSwapBack()){ swapBack(_type); }
    ...
    function swapBack(uint _type) internal swapping {

        // *
        if (_type == 1) {
            liquidityFee = bliquidityFee; 
            giveawayFee = bgiveawayFee;  
            marketingFee = bmarketingFee;   
            reflectionFee = breflectionFee;   
            totalFee = btotalFee;  
            feeDenominator = bfeeDenominator;
        } else if (_type == 2) {
            liquidityFee = sliquidityFee; 
            giveawayFee = sgiveawayFee;  
            marketingFee = smarketingFee;   
            reflectionFee = sreflectionFee;   
            totalFee = stotalFee;  
            feeDenominator = sfeeDenominator;
        } else {
            liquidityFee = 0; 
            giveawayFee = 0;  
            marketingFee = 0;   
        ...
        // *
        if (amountBNBGiveaway > 0) {
            (bool success,) = payable(giveawayFeeReceiver).call{value: amountBNBGiveaway, gas: 30000}("");
            require(success, "receiver rejected transfer");
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private { 
        ...
        uint256 toToLo = (tokenAmount.mul(_lpLockedFee)).div(10**2); 
        uint256 toToBu = tokenAmount.sub(toToLo); 
        uint256 etToLo = (ethAmount.mul(_lpLockedFee)).div(10**2); 
        uint256 etToBu = ethAmount.sub(etToLo); 
        router.addLiquidityETH{value: etToLo}( address(this), toToLo, 0, 0, autoLiquidityReceiver, block.timestamp ); 
        router.addLiquidityETH{value: etToBu}( address(this), toToBu, 0, 0, DEAD, block.timestamp );
    ...
    function changeFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _giveawayFee) external onlyOwner {
        ...
        require(totalFee <= 20, "Fees must be less than 20%");
    }

    // *
    function changeSFees(uint256 _sliquidityFee, uint256 _sreflectionFee, uint256 _smarketingFee, uint256 _sgiveawayFee) external onlySetter {
        sliquidityFee = _sliquidityFee;
        sreflectionFee = _sreflectionFee;
        smarketingFee = _smarketingFee;
        sgiveawayFee = _sgiveawayFee;
        stotalFee = sliquidityFee.add(sreflectionFee).add(smarketingFee).add(sgiveawayFee);
        require(stotalFee <= 20, "Fees must be less than 20%");
    } 
    // *
    function changeBFees(uint256 _bliquidityFee, uint256 _breflectionFee, uint256 _bmarketingFee, uint256 _bgiveawayFee) external onlySetter {
        bliquidityFee = _bliquidityFee;
        breflectionFee = _breflectionFee;
        bmarketingFee = _bmarketingFee;
        bgiveawayFee = _bgiveawayFee;
        btotalFee = bliquidityFee.add(breflectionFee).add(bmarketingFee).add(bgiveawayFee);
        require(btotalFee <= 20, "Fees must be less than 20%");
    }
