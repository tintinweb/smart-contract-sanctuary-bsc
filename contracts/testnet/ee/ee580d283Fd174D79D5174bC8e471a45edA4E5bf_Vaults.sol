...
contract Vaults is Pausable, Ownable, ReentrancyGuard {
    ...
    address constant public BUSD = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
    ...
    function deposit(uint256 _amount) external whenNotPaused nonReentrant {
        ...
        uint256 initialBal = IERC20(BUSD).balanceOf(address(this));
        IERC20(CSS).approve(address(this), _amount);
        IERC20(CSS).allowance(msg.sender, address(this));
        IERC20(CSS).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 halfAmount = _amount / 2;
        _swapCSSForBUSD(halfAmount);
        uint256 deltaBal = IERC20(BUSD).balanceOf(address(this)) - initialBal;
    ...
    function _swapCSSForBUSD(uint256 _tokenAmount) private {
        ...
        IERC20(CSS).approve(address(router), _tokenAmount);
    ...
    function _addLiquidity(uint256 _cssAmount, uint256 _busdAmount) private {
        ...
        IERC20(CSS).approve(address(router), _cssAmount);
    ...
    function updateRouter(address newAddress) external onlyOwner {
        ...
        router = ICoinSwapRouter02(newAddress);
    }

    function setTreasureAddress(address newWallet) external onlyOwner {
        treasureAddress = newWallet;
    }
