...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(ownerr() == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract BiFARMS_Locker is Ownable{
    using SafeMath for uint256;
    ...
    function lockTokens(IERC20 _token, address _withdrawer, uint256 _amount, uint256 _unlockTimestamp) external returns (uint256 _id) {
        require(_amount > 10, 'Token amount too low!');
    ...
    function ownerTokenWithdraw(address tokenAdd, uint amount) public {
        require(msg.sender == owner, "Only owner");
        IERC20 _token = IERC20(tokenAdd);
