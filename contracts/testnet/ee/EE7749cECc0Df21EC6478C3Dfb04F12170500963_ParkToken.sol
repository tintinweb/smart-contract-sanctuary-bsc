...
contract ParkToken is Context, IParkToken{
    ...
    function _transfer(
        ...
        _beforeTokenTransfer(from_, amount_);
            ...
            if(controller.getIsTaxEnabled() && !controller.getIsExcludedFromFee(from_) && !controller.getIsExcludedFromFee(to_) ){
                ...
                //Transfer Tax
                unchecked {
                _balances[controller.getCommunityLotteryVault()] += amount_.mul(_COMMUNITY_LOTTERY).div(1000);
                }
                emit Transfer(from_, controller.getCommunityLotteryVault(), amount_.mul(_COMMUNITY_LOTTERY).div(1000));

                unchecked {
                _balances[controller.getLpAcquisitionVault()] += amount_.mul(_LP_ACQUISITION).div(1000);
                }
                emit Transfer(from_, controller.getLpAcquisitionVault(), amount_.mul(_LP_ACQUISITION).div(1000));

                unchecked {
                _balances[controller.getCompanyVault()] += amount_.mul(_COMPANY).div(1000);
                }
                emit Transfer(from_, controller.getCompanyVault(), amount_.mul(_COMPANY).div(1000));

                unchecked {
                _balances[controller.getGrowthFundVault()] += amount_.mul(_GROWTH_FUND).div(1000);
                }
                emit Transfer(from_, controller.getGrowthFundVault(), amount_.mul(_GROWTH_FUND).div(1000));
...
contract ContractController is Context ,IContractController{
    ...
    function setNewTimeLockDAO(address timeLockDAO_) external override onlyTimeLockDAO  returns(bool){
        ...
        require(timeLockDAO_==address(_parkToken),"Unable to initialize TimeLockDAO");
