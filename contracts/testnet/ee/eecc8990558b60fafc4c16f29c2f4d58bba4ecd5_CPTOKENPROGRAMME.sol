...
contract CPTOKENPROGRAMME {
    ...
    mapping (uint256 => Program) private _program;
    mapping (uint256 => bool) private _usedProgramId;


    mapping (uint256 => mapping (uint256 => bool)) private _programLinkToIssuance;  // one to many
    mapping (uint256 => uint256) private _issuanceLinkToProgram;    //  one to one


    function setProgram(uint256 _programId, uint256 _effectiveDate, uint256 _expiryDate) external {
    ...
    event SetProgram (uint256 indexed _programId, address indexed _owner);
    event LinkIssuance (uint256 indexed _programId, uint256 indexed _issuanceId);
