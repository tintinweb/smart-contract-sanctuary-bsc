    ...
    function _resolveBet(Bet storage bet, bool wins, uint256 rolled, uint256 payout) internal returns (uint256) {
        ...
        if (wins) {
            if (referrer != address(0)) {
                ...
                payReferralCommission(token, user, payout.mul(referralCommissionRate).div(10000));
            } else {
...
contract Dice is Game {
    ...
    constructor(address chainlinkCoordinatorAddress, IReferral _referral) Game(chainlinkCoordinatorAddress, 1, _referral) {}
