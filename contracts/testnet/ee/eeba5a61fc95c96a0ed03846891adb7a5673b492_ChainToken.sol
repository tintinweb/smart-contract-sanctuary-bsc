...
contract ChainToken is Context, IERC20, Ownable {
    ...
    event FeesChanged(uint256 rfi,uint256 dev,uint256 liquidity,uint256 burn);
    ...
    constructor (
        ...
        swapTokensAtAmount = (_tTotal * 5 / 10000) * 10 ** DECIMALS;
    ...
    function retrieveETH(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient balance");
        payable(msg.sender).transfer(weiAmount);
    }
    
    function retrieveToken(address _tokenAddr, address _to, uint _amount) external onlyOwner {
    ...
    function setTaxes(uint256 _rfi, uint256 _dev, uint256 _liquidity, uint256 _burn) external onlyOwner {
        require((_rfi + _dev + _liquidity + _burn) < 100, "Total taxes cannot be higher than 100%");
        taxes.rfi = _rfi;
        taxes.dev = _dev;
        taxes.liquidity = _liquidity;
        taxes.burn = _burn;
        emit FeesChanged(_rfi, _dev, _liquidity, _burn);
    }
