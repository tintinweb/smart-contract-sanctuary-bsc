...
import './interfaces/IAnnexERC20.sol';
...
import './AnnexPair.sol';

contract AnnexFactory is IAnnexFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(AnnexPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'Annex: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'Annex: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'Annex: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(AnnexPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IAnnexPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'Annex: FORBIDDEN');
        feeTo = _feeTo;
    }
...
import './AnnexERC20.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';
import './interfaces/IERC20.sol';
import './interfaces/IAnnexFactory.sol';
import './interfaces/IAnnexCallee.sol';
