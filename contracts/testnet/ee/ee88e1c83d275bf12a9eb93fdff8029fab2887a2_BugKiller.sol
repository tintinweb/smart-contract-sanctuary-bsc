...
contract BugKiller is ERC721Enumerable, ReentrancyGuard, Ownable, Whitelist  {

    string public baseTokenURI = "ipfs://1/";

    uint256 public MAX_BUG_KILLER = 10000;
    uint256 public namingBugPrice = 0.1 ether;
    ...
    mapping(uint256 => string) public nameBug;
    mapping(address => uint256) public balanceBug;

    BugCoin public bugCoin;

    event NameChanged(string name);

    constructor() ERC721("BugKiller", "BugKiller") {
    }
    ...
    function setBalanceBug(address wallet, uint256 _newBalance) external onlyOwner {
        balanceBug[wallet] = _newBalance;
    }

    function setNamingPrice(uint256 _namingPrice) external onlyOwner {
        namingBugPrice = _namingPrice;
    }
  ...
  function withdrawAllERC20(address _tokenContract, uint256 _amount) public onlyOwner {
    ...
    function mintAdmin(uint256[] calldata tokenIds, address _to) public payable onlyOwner {
        require(totalSupply() + tokenIds.length <= MAX_BUG_KILLER, "Minting would exceed max supply of NFTs");

        for(uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < MAX_BUG_KILLER, "Invalid token ID");
            require(!_exists(tokenIds[i]), "Tokens has already been minted");

            if (totalSupply() < MAX_BUG_KILLER) {
                _safeMint(_to, tokenIds[i]);
                balanceBug[_to] += 1;
            }
        }
        //update reward on mint
        bugCoin.updateBounsOnMint(_to, tokenIds.length);
    }

    function mint(uint256 _amount) public payable nonReentrant {
        require(mintIsActive, "Mint is closed");
        require(totalSupply() + _amount <= MAX_BUG_KILLER, "Minting would exceed max supply of NFTs");
        ...
        for(uint256 i = 0; i < _amount; i++) {
            if (totalSupply() < MAX_BUG_KILLER) {
                ...
                balanceBug[msg.sender] += 1;
            }
        }
        //update reward on mint
        bugCoin.updateBounsOnMint(msg.sender, _amount);
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override nonReentrant {
        bugCoin.updateBouns(from, to, tokenId);
        balanceBug[from] -= 1;
        balanceBug[to] += 1;
    ...
    function getReward() external payable {
        ...
        bugCoin.updateBouns(msg.sender, address(0), 0);
    ...
    function changeName(uint256 _tokenId, string memory _newName) public {
        ...
        for (uint256 i; i < totalSupply(); i++) {
            if (bytes(nameBug[i]).length != bytes(_newName).length) {
                continue;
        } else {
            require(keccak256(abi.encode(nameBug[i])) != keccak256(abi.encode(_newName)), "name is used");
        }
        }

        bugCoin.transferFeeFrom(msg.sender,nameAddress,namingBugPrice); 
        nameBug[_tokenId] = _newName;
