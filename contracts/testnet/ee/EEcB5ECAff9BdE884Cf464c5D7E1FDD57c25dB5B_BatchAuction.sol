...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  /**
   * @notice Get or Create Pancake LP Token
   * @return LP Token Address
   */
  function getOrCreatePancakeLPToken(address _token1, address _token2) external returns (address) {
    require(_token1 != address(0), "INVALID_TOKEN1_ADDRESS");
    require(_token2 != address(0), "INVALID_TOKEN2_ADDRESS");

    address lpTokenAddress = IPancakeFactoryV2(pancakeFactoryV2).getPair(_token1, _token2);
    if(lpTokenAddress == address(0)) {
      lpTokenAddress = IPancakeFactoryV2(pancakeFactoryV2).createPair(_token1, _token2);
    }
