...
contract RionPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 rionAtLastUserAction; // keep track of rion deposited at the last user action.
    ...
    event NewVRionContract(address VRion);
    ...
    /**
     * @notice Constructor
     * @param _token: Rion token contract
     * @param _masterchefV2: MasterChefV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: rion pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(rionPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the rion owner address or the operator address.
     */
    modifier onlyOperatorOrRionOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or rion owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onRionPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.rionAtLastUserAction;
    ...
    /**
     * @notice Unlock user rion funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrRionOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Rion Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in RION)
    ...
    function depositOperation(
        ...
        if (VRion != address(0)) {
            IVRion(VRion).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.rionAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Rion Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VRion != address(0)) {
            IVRion(VRion).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.rionAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.rionAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending RION tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingRion = masterchefV2.pendingRion(rionPoolPID, address(this));
        if (pendingRion > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(rionPoolPID, 0);
    ...
    /**
     * @notice Set VRion Contract address
     * @dev Callable by the contract admin.
     */
    function setVRionContract(address _VRion) external onlyAdmin {
        require(_VRion != address(0), "Cannot be zero address");
        VRion = _VRion;
        emit NewVRionContract(VRion);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Rion Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingRionRewards();
     ...
     * @return Returns total pending rion rewards
     */
    function calculateTotalPendingRionRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingRion(rionPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingRionRewards()) * (1e18)) / totalShares);
    }
