...
// Note that this pool has no minter key of SkyG (rewards).
// Instead, the governance will call SkyG distributeReward method and send reward to this pool at the beginning.
contract SkyGRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. SkyGs to distribute per block.
        uint256 lastRewardTime; // Last time that SkyGs distribution occurs.
        uint256 accSkyGPerShare; // Accumulated SkyGs per share, times 1e18. See below.
    ...
    uint256 public skygPerSecond;
    uint256 public runningTime = 731 days;
    ...
    constructor(
        ...
        if (_skyg != address(0)) skyg = IERC20(_skyg);
        poolStartTime = _poolStartTime;
        poolEndTime = poolStartTime.add(runningTime);
        skygPerSecond = TOTAL_REWARDS.div(runningTime); // =0.0009499924000607996 SKYG/sec = 60000 SKYG / (731 days * 24h * 60min * 60sec)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "SkyGRewardPool: caller is not the operator");
        _;
    }

    modifier onlyOperatorOrTreasury() {
        require(operator == msg.sender || treasury == msg.sender, "SkyGRewardPool: caller is not the operator/treasury");
        _;
    }
    ...
    function getRewardPerSecond() external override view returns (uint256) {
        return skygPerSecond;
    }

    function checkPoolDuplicate(IERC20 _token) internal view {
        uint256 length = poolInfo.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "SkyGRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's SkyG allocation point. Can only be called by the owner.
    ...
    function getGeneratedReward(uint256 _fromTime, uint256 _toTime) public view returns (uint256) {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime) return poolEndTime.sub(poolStartTime).mul(skygPerSecond);
            return poolEndTime.sub(_fromTime).mul(skygPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime) return _toTime.sub(poolStartTime).mul(skygPerSecond);
            return _toTime.sub(_fromTime).mul(skygPerSecond);
        }
    }
    ...
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            accSkyGPerShare = accSkyGPerShare.add(_skygReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _skygReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accSkyGPerShare = pool.accSkyGPerShare.add(_skygReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external override nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                _safeSkyGTransfer(msg.sender, _pending);
    ...
    function _withdraw(address _account, uint256 _pid, uint256 _amount) internal {
        ...
        uint256 _pending = user.amount.mul(pool.accSkyGPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            _safeSkyGTransfer(_account, _pending);
    ...
    // Safe skyg transfer function, just in case if rounding error causes pool to not have enough SkyGs.
    function _safeSkyGTransfer(address _to, uint256 _amount) internal {
        uint256 _skygBal = skyg.balanceOf(address(this));
        if (_skygBal > 0) {
            if (_amount > _skygBal) {
                skyg.safeTransfer(_to, _skygBal);
            } else {
                skyg.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function updateRewardRate(uint256 _newRate) external override onlyOperatorOrTreasury {
        ...
        uint256 _oldRate = skygPerSecond;
        massUpdatePools();
