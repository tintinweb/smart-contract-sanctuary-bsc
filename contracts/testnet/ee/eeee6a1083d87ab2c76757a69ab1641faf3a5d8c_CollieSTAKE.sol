...
contract CollieContext {
    ...
    uint256 public oneDayTime = 1 seconds;
    ...
    event TierListUpdated(uint256 level, uint256 percentage, uint256 apy, uint256 minTxAmount,uint256 maxTxAmount, uint256 penalty, uint256 maxallowed,uint256 minallowed);
}
...
contract CollieSTAKE is CollieContext, Ownable, ReentrancyGuard {
    ...
    constructor() {
        token = IERC20(0xadc73096073c83b8A525455ad649eebE3a38Ba74);
        rewardingWallet = 0x294d0487fdf7acecf342ae70AFc5549A6E90f3e0;

        pools[1]  = Pool(30,  1000, 1,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[2]  = Pool(60,  1500, 2,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[3]  = Pool(90,  2000, 3,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[4]  = Pool(120, 2500, 4,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[5]  = Pool(150, 3000, 5,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[6]  = Pool(180, 3500, 6,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[7]  = Pool(210, 4000, 7,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[8]  = Pool(240, 4500, 8,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[9]  = Pool(270, 5000, 9,  0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[10] = Pool(300, 5500, 10, 0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[11] = Pool(330, 6000, 11, 0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
        pools[12] = Pool(365, 6500, 12, 0, 10000000000000000000000, 3, 0, 900000000000000000000000000000, 100);
    }
    ...
    /**
      ...
      */
    function flipEarlyStake() public onlyOwner {
    ...
    /**
      ...
      */
    function setApy(uint256 apy, uint256 tier) public onlyOwner {
        pools[tier].apy = apy;
    }
    ...
    function changeMinTxAmount(uint256 tier, uint256 _minTxAmount) external onlyOwner {
        Pool storage pool = pools[tier];
        pool.minTxAmount = _minTxAmount;
        emit StakeMinAmountUpdated(pool.minTxAmount);
    }

    function changeMaxTxAmount(uint256 tier, uint256 _maxTxAmount) external onlyOwner {
        Pool storage pool = pools[tier];
        require(
            _maxTxAmount > pool.minTxAmount,
            "Max amount must be greater than min amount"
        );
        pool.maxTxAmount = _maxTxAmount;
        emit StakeMaxAmountUpdated(pool.maxTxAmount);
    }
    ...
    // for emergency
    function changePenalty(uint256 tier, uint256 _penalty) public onlyOwner {
        Pool storage pool = pools[tier];
    ...
    function updatePool(uint256 lockPeriod, uint256 apy, uint256 tier, uint256 minTxAmount,uint256 maxTxAmount, uint256 penalty, uint256 maxallowed, uint256 minallowed, uint256 maxAllowedWallets) public onlyOwner{
        Pool storage pool = pools[tier];
        pool.lockPeriod = lockPeriod;
        pool.apy = apy;
        pool.tier = tier;
        pool.minTxAmount = minTxAmount;
        pool.maxTxAmount = maxTxAmount;
        pool.penalty = penalty;
        pool.maxallowed = maxallowed;
        pool.minallowed = minallowed;
        pool.maxAllowedWallets = maxAllowedWallets;
        emit TierListUpdated(tier, lockPeriod, apy, minTxAmount, maxTxAmount, penalty, maxallowed, minallowed);
    }
    
    /**
      * Public function to stake the $STATE tokens. It takes one argument of amount as input.
      */
    function stake(uint256 amount, uint256 tier) public nonReentrant {
        require(tier > 0 && tier <=10, "Incorrect Tier");
    ...
    function _stakeTokens(uint256 _amount, uint256 _tier) internal {
        ...
        require(_amount >= pool.minTxAmount && _amount <= pool.maxTxAmount, "Amount Should Be Between Min and Max Limit");
    ...
    /**
      ...
      */
    function UnstakeTokens(uint256 tier) public nonReentrant {
        require(isUnStakeActive, "Unstaking Is Paused");

        _unstakeTokens(_msgSender(), tier);
    }

    /**
      * Internal function to unstake the tokens. It takes two argument as an input and called from public function.
      */
    function _unstakeTokens(address _address, uint256 tier) internal {
        User storage wUser = deposit[tier][_address];
        require(wUser.stakedAmount > 0, "Stake First To Unstake Tokens");

        if(apyEnabled) {
            _claim(_address, tier);
        }
        ...
        deposit[tier][_msgSender()] = User(0, 0, 0, 0, 0, Pool(0,0,0, 0,0,0,0,0,0));
    ...
    function EmergencyUnstake(uint256 tier) public nonReentrant{
        User memory wUser = deposit[tier][_msgSender()];
    ...
    function _earlyUnstakeTokens(uint256 _tier) internal{
        ...
        token.transfer(_msgSender(), wUser.stakedAmount.sub(_fee));

        if(apyEnabled) {
            _claim(_msgSender(), _tier);
        }

        deposit[_tier][_msgSender()] = User(0, 0, 0, 0, 0, Pool(0,0,0, 0,0,0,0,0,0));
        totalStaked-=wUser.stakedAmount;
        totalStakers--;
    ...
    /**
      ...
      */
    function checkReward(address _address, uint256 tier) public view returns (uint256) {
        User memory cUser = deposit[tier][_address];
        Pool memory pool = pools[tier];
        
        if(block.timestamp + oneDayTime > cUser.lastClaimTime) {
          ...
          stakedtime = stakedtime / oneDayTime;

            uint256 reward = pool.apy.mul(stakedtime).mul(cUser.stakedAmount).div(10000).div(365);
