...
contract BabyJack is ERC20, Ownable {
    ...
    address public _dividendToken = 0x6ce8dA28E2f864420840cF74474eFf5fD80E65B8;
    ...
    constructor() ERC20("BabyJack", "BABYJACK") {
    ...
    receive() external payable {
  	...
  	function setMaxSellTransaction(uint256 maxTxn) external onlyOwner {
        require(maxTxn >= 1500000000, "Max sell transaction amount must be at least 1500000000");
  	    maxSellTransactionAmount = maxTxn * (10**18);
  	}
  	
    function setMarketingFee(uint256 fee) external onlyOwner {
         require(fee >= 1 && fee <= 12, "BabyJack: Marketing fee must be between 1 and 12");
        marketingFee = fee;
    }

  	function setMaxWalletToken(uint256 maxToken) external onlyOwner {
        require(maxToken >= 1500000000, "Max wallet token must be at least 1500000000");
    ...
    function updateMarketingFee(uint8 newFee) public onlyOwner {
        require(newFee >= 0 && newFee <= 15, "BabyJack: Dividend reward tax must be between 0 and 15");
        marketingFee = newFee;
    }
