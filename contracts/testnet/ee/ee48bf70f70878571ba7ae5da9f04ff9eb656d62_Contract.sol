...
contract Contract is Context, IERC20 { 
    ...
    address public Wallet_Tokens;                       // Token Fee Collection Wallet (Fund Staking Pools, Giveaways, Liquidity, etc.)
    address payable public Wallet_BNB;                  // BNB Fee Collection Wallet (Marketing, Team, etc.)
    address payable public TokensByGEN_Affiliate;       // TokensByGEN Affiliate Wallet and Discount Code
    ...
    // Token social link will appear on BSCScan 'read' contract page
    ...
    constructor (string memory _TokenSymbol, string memory _TokenName, address payable _OwnerWallet, uint256 _TotalSupply, uint256 _Decimals, string memory _Website, address payable _DiscountCode, uint256 _ContractFee) {
    ...
    TokensByGEN_Affiliate = _DiscountCode;
  ...
  ///  IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
   ...
   ////// uint256 public swapTrigger = 11;    // After 10 transactions with a fee swap will be triggered 
    uint256 public swapTrigger = 4;    // After 10 transactions with a fee swap will be triggered 
    ...
    */
    ...
    // Set Buy Fees
    function Contract_Setup_3__Fees_on_Buy(uint256 BNB_on_BUY, 
        ...
        require(BNB_on_BUY + Liquidity_on_BUY + Reflection_on_BUY + Burn_on_BUY + Tokens_on_BUY + _fee_Buy_Contract <= 20, "Fees too high"); 
    ...
    // Set Sell Fees
    function Contract_Setup_4__Fees_on_Sell(uint256 BNB_on_SELL,
        ...
        // Buyer protection - max fee can not be set over 20% (including possible 1% contract fee if chosen)
        require(BNB_on_SELL + Liquidity_on_SELL + Reflection_on_SELL + Burn_on_SELL + Tokens_on_SELL + _fee_Sell_Contract <= 20, "Fees too high"); 
    ...
    */
    ...
    function Contract_Setup_5__Wallet_Limits(
        ...
        require(Max_Tokens_Per_Transaction > _tTotal / 1000, "Max transaction must be greater than 1% of Supply");
        require(Max_Total_Tokens_Per_Wallet > _tTotal / 1000, "Max wallet must be greater than 1% of Supply");
    ...
    function Contract_Setup_6__OpenTrade() external onlyOwner {
        ...
        _fee_Buy_Contract   = ContractFee;
        _fee_Sell_Contract  = ContractFee;
        _SwapFeeTotal_Buy   = _fee_Buy_Liquidity + _fee_Buy_BNB + _fee_Buy_Contract;
        _SwapFeeTotal_Sell  = _fee_Sell_Liquidity + _fee_Sell_BNB + _fee_Sell_Contract;
    ...
    */
    ...
    function Remove_Contract_Fee_For_2BNB() external onlyOwner payable {
    ...
    */
    ...
    function Update_Wallet_Liquidity(address wallet) external onlyOwner {
        ...
        emit updated_Liquidity_Wallet(Wallet_Liquidity,wallet);
    ...
    function Update_Wallet_Tokens(address wallet) external onlyOwner {
        ...
        require(wallet != address(0), "Can not be zero address");
        emit updated_Tokens_Wallet(Wallet_Tokens,wallet);

        // Remove old wallet limit exempt status
        _isLimitExempt[Wallet_Tokens] = false;
