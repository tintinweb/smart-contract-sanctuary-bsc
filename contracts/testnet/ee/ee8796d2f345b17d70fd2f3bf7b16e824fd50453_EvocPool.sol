...
contract EvocPool is Ownable, Pausable {
    ...
    IEvocMasterV2 public immutable EvocmasterV2;
    ...
    /**
     * @notice Constructor
     * @param _tokenEVCT: Evoc token contract
     * @param _EvocmasterV2: EvocMasterV2 contract
     * @param _admin: address of the admin
     * @param _treasury: address of the treasury (collects fees)
     * @param _operator: address of operator
     * @param _pid: cake pool ID in EvocMasterV2
     */
    constructor(
        IERC20 _tokenEVCT,
        IEvocMasterV2 _EvocmasterV2,
        address _admin,
        address _treasury,
        address _operator,
        uint256 _pid
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        dummyToken.approve(address(EvocmasterV2), balance);
        EvocmasterV2.deposit(cakePoolPID, balance);
        emit Init();
    }
     ...
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in EVCT)
    ...
    function depositOperation(
        ...
        if (VEvoc != address(0)) {
            IVEvct(VEvoc).deposit(_user, _amount, _lockDuration);
        }
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VEvoc != address(0)) {
            IVEvct(VEvoc).withdraw(msg.sender);
        }
        ...
        // Harvest token from EvocmasterV2.
        harvest();
    ...
    /**
     * @notice Harvest pending EVCT tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingEvoc = EvocmasterV2.pendingEvoc(cakePoolPID, address(this));
        if (pendingEvoc > 0) {
            uint256 balBefore = available();
            EvocmasterV2.withdraw(cakePoolPID, 0);
     ...
     */
    function inCaseTokensGetStuck(address _tokenEVCT) external onlyAdmin {
        require(_tokenEVCT != address(token), "Token cannot be same as deposit token");

        uint256 amount = IERC20(_tokenEVCT).balanceOf(address(this));
        IERC20(_tokenEVCT).safeTransfer(msg.sender, amount);
    }
    ...
    function calculateTotalPendingEvocRewards() public view returns (uint256) {
        uint256 amount = EvocmasterV2.pendingEvoc(cakePoolPID, address(this));
        return amount;
    }
