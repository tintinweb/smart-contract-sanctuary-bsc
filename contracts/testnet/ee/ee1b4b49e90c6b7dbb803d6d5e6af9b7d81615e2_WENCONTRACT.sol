...
//    address private busdToken = 0xaD6D458402F60fD3Bd25163575031ACDce07538D; //DAI on Ropsten
    ...
    constructor() ERC20Detailed("Wenburn1", "WENB1", uint8(DECIMALS)) {
  ...
  //      router = IDEXRouter(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); //Uniswap Ropsten
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal whenNotPaused returns (bool) {
        ...
        if (automatedMarketMakerPairs[recipient] &&! excludedAccount && amount <= onePercent && amount <= maxSellTransactionAmount) {
          ...
          //  uint256 onePercent = balanceOf(sender).mul(txfee).div(100); //Should use variable
            ...
            else if( (blkTime < tradeData[sender].lastTradeTime + TwentyFourhours) && (( blkTime > tradeData[sender].lastTradeTime)) && (( tradeData[sender].tradeAmount + amount <= onePercent))){
