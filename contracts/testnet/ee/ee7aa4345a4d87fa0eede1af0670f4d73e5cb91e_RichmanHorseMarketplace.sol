...
contract RichmanHorseMarketplace is Context, Ownable, Pausable
{
   ...
   struct BidStat
   {
      ...
      uint128 acceptedDateTime;
      uint256 tokenAmount;
      uint256 etherPrice;
      BiDStatus currentStatus;
   }

   enum BiDStatus { Actual, Accepted, Deleted }
   ...
   function makeBid(bytes32 lotId, uint256 tokenAmount) external payable whenNotPaused
   {
      ...
      BidStat memory bStat = BidStat(
      {
         ...
         , currentStatus: BiDStatus.Actual
      });
   ...
   function removeBid(bytes32 bidId) external whenNotPaused
   {
      ...
      for (uint256 i = 0; i < _userBids[sender].length; ++i)
      {
         if (_userBids[sender][i] == bidId)
         {
            ...
            // statistics
            int256 bidStatIdx = _getLotStatBidIndex(bid.lotId, bidId);
            if (bidStatIdx > -1)
            {
               BidStat storage bStat = _lotBidsStat[bid.lotId][uint256(bidStatIdx)];
               bStat.currentStatus = BiDStatus.Deleted;
            }
   ...
   function acceptBid(bytes32 bidId) external whenNotPaused
   {
      ...
      if (bidStatIdx > -1)
      {
         ...
         bStat.acceptedDateTime = getCurrentTime();
         bStat.currentStatus = BiDStatus.Accepted;
      }
   }
