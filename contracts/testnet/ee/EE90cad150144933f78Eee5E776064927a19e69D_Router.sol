...
interface IPancakePair {
    ...
    function tokennft(address token, uint id) external view returns (bool);
}
...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB, uint id) internal pure returns (address pair) {
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'a0c224019d33aee153ff179dfd4b716b439413b5167b24f9d0913a3b26b7da63'   // Change to INIT_CODE_PAIR_HASH of Pancake Factory
            )))));
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path, uint[] memory id) internal view returns (uint[] memory amounts) {
        ...
        for (uint i; i < path.length - 1; i++) {
            ...
            bool checknft = IPancakePair(pairFor(factory, path[i+1], path[i], id[i])).tokennft(path[i+1], id[i]);
...
contract Router is ERC1155Holder{
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to, uint[] memory id) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            bool checknft = IPancakePair(PancakeLibrary.pairFor(factory, input, output,id[i])).tokennft(output, id[i]);
    ...
    function swapExactTokensForTokens(
        ...
        bool checknft = IPancakePair(PancakeLibrary.pairFor(factory, path[0], path[1], id[0])).tokennft(path[0], id[0]);
        if(checknft)
