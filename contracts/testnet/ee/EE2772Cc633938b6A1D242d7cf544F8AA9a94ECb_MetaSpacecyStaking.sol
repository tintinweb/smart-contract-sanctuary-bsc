...
import "../token/IMetaSpacecyToken.sol";
import "../interfaces/IShares.sol";
import "../interfaces/IMetaSpacecyStaking.sol";
import "../interfaces/IMetaSpacecyTicketCard.sol";

contract MetaSpacecyStaking is ERC1155Holder, IShares, IMetaSpacecyStaking, Ownable, ReentrancyGuard, Pausable {
  using EnumerableSet for EnumerableSet.UintSet;

  string public constant name = "MetaSpacecy Staking";
  ...
  IERC1155 public metaspacecyCreatureAccessory;
  IMetaSpacecyToken public metaspacecyToken;
  IMetaSpacecyTicketCard public metaspacecyTicketCard;
  ...
  modifier onlyAdmin() {
    require(metaspacecyAccessControls.hasAdminRole(_msgSender()), "MSS: only admin");
    _;
  }

  constructor(
    address _metaspacecyCreatureAccessory,
    address _metaspacecyToken,
    address _metaspacecyTicketCard,
    address _metaspacecyAccessControls,
    uint256 _limitSlot
  ) {
    metaspacecyCreatureAccessory = IERC1155(_metaspacecyCreatureAccessory);
    metaspacecyToken = IMetaSpacecyToken(_metaspacecyToken);
    metaspacecyTicketCard = IMetaSpacecyTicketCard(_metaspacecyTicketCard);
  ...
  function maxBalanceMST() public view override returns (uint256) {
    return (metaspacecyToken.maxTotalSupply() * STAKING_REWARD_SHARES) / TOTAL_SHARES;
  }

  function currentBalanceMST() public view override returns (uint256) {
    return metaspacecyToken.balanceOf(address(this));
  }

  function balanceOfAsset(uint256 id) public view override returns (uint256) {
    return metaspacecyCreatureAccessory.balanceOf(address(this), id);
  }

  function balanceOfTicketCard(uint256 id) public view override returns (uint256) {
    return metaspacecyTicketCard.balanceOf(address(this), id);
  }

  function canStake(StakingOption option, uint256 tokenId, uint256 amount) public view override returns (bool) {
    return (
      balanceOfTicketCard(metaspacecyTicketCard.currentIdSupported()) >= estimatedTC + _calculateTicketCard(option, tokenId, amount) &&
      currentBalanceMST() >= estimatedReward + _calculateReward(option, tokenId, amount)
    );
  }

  function canHarvest(address user, uint256 slotIndex) public view override returns (bool) {
    require(slotIndex < numberStaking[user], "MSS: NE");
  ...
  function burnExpiredTC(uint256 id) public onlyAdmin {
    metaspacecyTicketCard.burn(address(this), id, balanceOfTicketCard(id));
  }
  ...
  function withdrawMST(uint256 amount) public onlyAdmin whenPaused {
    metaspacecyToken.transfer(_msgSender(), amount);
  ...
  function stake(StakingOption option, uint256 tokenId, uint256 amount) public override whenNotPaused {
    require(canStake(option, tokenId, amount), "MSS: CNS");
    require(slotStaking[_msgSender()].length() < limitSlot, "MSS: FS");
  ...
  function harvest(uint256 slotIndex) public override whenNotPaused {
    require(slotStaking[_msgSender()].contains(slotIndex), "MSS: NE");

    StakingInfo memory stakingInfo = userStakings[_msgSender()][slotIndex];
    require(canHarvest(_msgSender(), slotIndex), "MSS: EARLY");
  ...
  function _transferNFTAsset(address _from, address _to, uint256 _tokenId, uint256 _amount) internal {
    require(metaspacecyCreatureAccessory.balanceOf(_from, _tokenId) >= _amount, "MSS: IF");
    metaspacecyCreatureAccessory.safeTransferFrom(_from, _to, _tokenId, _amount, "");
  }

  function _transferNFTTicketCard(address _to, uint256 _amount) internal {
    require(balanceOfTicketCard(metaspacecyTicketCard.currentIdSupported()) >= _amount, "MSS: IF");
    metaspacecyTicketCard.safeTransferFrom(address(this), _to, metaspacecyTicketCard.currentIdSupported(), _amount, "");
  }

  function _transferRewardToken(address _to, uint256 _amount) internal {
    require(currentBalanceMST() >= _amount, "MSS: IF");
    metaspacecyToken.transfer(_to, _amount);
  }
...
interface IMetaSpacecyToken is IERC20 {
...
interface IShares {
  ...
  function currentBalanceMST() external view returns (uint256);
  function maxBalanceMST() external view returns (uint256);
}
...
interface IMetaSpacecyTicketCard is IERC1155 {
