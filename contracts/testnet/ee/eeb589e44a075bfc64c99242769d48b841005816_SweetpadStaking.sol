...
contract SweetpadStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function unfreezeSWT(uint256 amount_) external {
        FreezeInfo memory freezeData = freezeInfo[msg.sender];
        ...
        _unfreezeSWT(msg.sender, amount_, powerDelta);
    }
    ...
    function getAllStakersInfo() external view returns(FreezeInfo[] memory) {
        FreezeInfo[] memory stakersInfo = new FreezeInfo[](stakers.length());
        for (uint256 i; i < stakersInfo.length; i++) {
            stakersInfo[i] = freezeInfo[stakers.at(i)];
        }
        return stakersInfo;
    }

    function seedUser(address[] calldata _users, FreezeInfo[] memory _usersInfo) external onlyOwner {
        require(_users.length == _usersInfo.length, "SweetpadStaking: length missmatch");
        for (uint256 i; i < _users.length; i++) {
            freezeInfo[_users[i]] = _usersInfo[i];
            stakers.add(_users[i]);
        }
    }
    ...
    function _freeze(
        ...
        FreezeInfo storage freezeData = freezeInfo[account_];
        freezeData.frozenUntil = block.number + period_;
        freezeData.period += period_;
        freezeData.frozenAmount += amount_;
        freezeData.power += power_;
        freezeData.asset = asset_;
    ...
    function _unfreezeSWT(
        ...
        if (amount_ == freezeInfo[account_].frozenAmount) {
            totalPower[account_] -= freezeInfo[account_].power;
            delete freezeInfo[account_];
        } else {
            totalPower[account_] -= power_;
            freezeInfo[account_].frozenAmount -= amount_;
            freezeInfo[account_].power -= power_;
        }
    ...
    function _unfreezeLP(address account_) private {
        FreezeInfo memory freezeData = freezeInfo[account_];
        totalPower[account_] -= freezeData.power;
        uint256 amount = freezeData.frozenAmount;
        delete freezeInfo[account_];
        totalFrozenLP -= amount;
        stakers.remove(account_);
        emit UnFreeze(account_, amount, 1);

        IERC20Upgradeable(lpTokenAddress).safeTransfer(account_, amount);
    }
