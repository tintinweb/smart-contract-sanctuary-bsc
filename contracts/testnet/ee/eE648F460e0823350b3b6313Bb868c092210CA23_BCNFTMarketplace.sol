    ...
    contract BCNFTMarketplace is ReentrancyGuard, Ownable {
        ...
        function buy(uint256 _id, uint256 _price, string memory paymentType) external payable {
            ...
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
                ...
                uint256 _commissionValue = salePrice[_id].mul(marketFeeForBNB).div(1000);
                uint256 _sellerValue = salePrice[_id].sub(_commissionValue);
                attemptBNBTransfer(payable(_previousOwner), _sellerValue);
                attemptBNBTransfer(payable(adminRecoveryAddress), _commissionValue.mul(5).div(7)); // 5% tax goes charity wallet
                attemptBNBTransfer(payable(developmentAddress), _commissionValue.mul(2).div(7)); // 2% goes development address
            } else {
            ...
            auctionNotExpired(tokenId)
        {
            ...
            if (auctions[tokenId].amount == 0) {
                ...
                if (
                    ...
                    attemptBNBTransfer(payable(auctions[tokenId].bidder), auctions[tokenId].amount);
                } else {
                    require(
            ...
            onlyCreatorOrWinner(tokenId)
        {
            ...
            if (winner == address(0)) {
                ...
                if (
                    ...
                    attemptBNBTransfer(payable(Creator), amount.sub(_commissionValue));
                    attemptBNBTransfer(payable(adminRecoveryAddress), _commissionValue.mul(5).div(7));
                    attemptBNBTransfer(payable(developmentAddress), _commissionValue.mul(2).div(7));
                } else {
