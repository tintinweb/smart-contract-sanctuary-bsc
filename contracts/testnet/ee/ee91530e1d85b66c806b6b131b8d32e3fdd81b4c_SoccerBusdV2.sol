...
contract SoccerBusdV2 is Ownable , ReentrancyGuard {
    ...
    function withdrawal() public noReentrant {
        ...
        if(autoClaimEnabled[msg.sender]){
            uint256 autoClaimFee = SafeMath.div(SafeMath.mul(totalAmountToWithdraw,auto_claim_tax),100);
            totalAmountToWithdraw = SafeMath.sub(aval_withdraw2,autoClaimFee);
        }
    ...
    function DailyRoi(uint256 _amount) public view returns(uint256) {
        return SafeMath.div(SafeMath.mul(_amount,getROI()),100);
    }
