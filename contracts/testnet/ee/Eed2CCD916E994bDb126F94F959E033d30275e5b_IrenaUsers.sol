...
contract IrenaUsers is Owned {
    ...
    ShoesStruct[] public getAllShoes;
    ...
    function setIrenaTokenAddress(address _irena) public {
      irena = IBEP20(_irena);
    }

    function setUsdtTokenAddress(address _usdt) public {
      usdt = IBEP20(_usdt);
    }
    ...
    function depositTokens(uint256 _shoePid, uint256 _amount) public{
    ...
    function withdrawTokens(uint256 _shoePid) public {
    ...
    function addShoe( uint256[] memory _shoeInfo, string memory _shoeName, uint256 _shoePid) public onlyOwner returns (ShoesStruct[] memory) {
      ...
      getAllShoes.push(shoeDetails[_shoePid]);
      return getAllShoes;
    }

    function buyShoe(uint256 _amount, uint256 _shoePid) public returns (UsersStruct memory){
      ...
      userDetails[msg.sender].userShoes.push(_shoePid);
      ...
      getAllShoes[_shoePid].shoeStockLeft = getAllShoes[_shoePid].shoeStockLeft.sub(1);
   ...
   function userSettings() public onlyOwner returns (UsersStruct memory){
     userDetails[msg.sender].userIsBlocked = !userDetails[msg.sender].userIsBlocked;
     return userDetails[msg.sender];
   }

   function getUserInfo() public view returns (UsersStruct memory){
     return userDetails[msg.sender];
   }
    

   function getUserShoeInfo(uint256 _shoePid) public view returns (UserShoeInfo memory){
     return userShoeDetails[msg.sender][_shoePid];
   }

   function getUserShoeList() public view returns (uint256[] memory){
     return userDetails[msg.sender].userShoes;
   }

   function getAllShoeDatas() public view returns (ShoesStruct[] memory){
     return getAllShoes;
   }
