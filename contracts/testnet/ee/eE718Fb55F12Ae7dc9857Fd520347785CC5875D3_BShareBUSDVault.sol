...
/**
@dev This contract is for Admin.BShare()-Admin.BUSD() vault 
 */
contract BShareBUSDVault is AccessControl, Initializable {
    ...
    // address Admin.BShare(); // Admin.BShare() token address
    // address public Admin.BUSD();
    address public router;
    // address public Admin.tokenSwap();
    ...
    function deposit(address user,uint amountLp) external {
        ...
        // if(isReceipt){
            // uint before = IERC20(BShareBUSD).balanceOf(address(this));
            //todo sell
            // amountLp = IPancakeMaster(masterChef).userInfo(id, user).amount- before;
        // }else if(amountTokenA != 0 && amountTokenB != 0){
        //     path[0] = IUniswapV2Pair(BShareBUSD).token0();
        //     path[1] = IUniswapV2Pair(BShareBUSD).token1();
        //     IERC20(path[0]).safeTransferFrom(user, address(this), amountTokenA);
        //     IERC20(path[1]).safeTransferFrom(user, address(this), amountTokenB);
        //     IERC20(IUniswapV2Pair(BShareBUSD).token0()).approve(router,amountTokenA);
        //     IERC20(IUniswapV2Pair(BShareBUSD).token1()).approve(router,amountTokenB);
        //     (,,amountLp) = IUniswapV2Router02(router).addLiquidity(
        //             path[0], path[1], amountTokenA, amountTokenB, 1, 1, address(this), block.timestamp + 1678948210);
        // }else if(amountTokenA != 0){
        //     IERC20(Admin.BUSD()).safeTransferFrom(user, address(this), amountTokenA);
        //     IERC20(Admin.BUSD()).approve(Admin.tokenSwap(),amountTokenA);
        //     ITokenSwap(Admin.tokenSwap()).swapTOKENtoLP(Admin.BUSD(), address(BShareBUSD), amountTokenA, address(this));
        //     amountLp = IERC20(address(BShareBUSD)).balanceOf(address(this));
        // }else if(amountLp != 0){
            IERC20(address(BShareBUSD)).transferFrom(user, address(this), amountLp);
        // }
    ...
    function withdraw(address user,bool isReciept, uint _amount) external{
        ...
        if(isReciept){
            IERC20(BShareBUSD_A).safeTransfer(user,_amount);
        }
        else{
            uint balance = (_amount * exchangeRatio)/10 ** 18;
            // if(!tokenA && !tokenB && !isReciept){
                IERC20(address(BShareBUSD)).safeTransfer(user,balance);
            // }else{
            //     IERC20(address(BShareBUSD)).safeApprove(router,balance);
            ...
            //     if(tokenA && tokenB){
            //         IERC20(BShareBUSD.token0()).safeTransfer(user, amountA);
            //         IERC20(BShareBUSD.token1()).safeTransfer(user, amountB);
            //     }else{
            //         address[] memory path = new address[](2);
            //         path[0] = Admin.BShare();
            //         path[1] = Admin.BUSD();
            //         IERC20(Admin.BUSD()).safeTransfer(user, IERC20(Admin.BUSD()).balanceOf((address(this))));
            //         uint balanceBShare = IERC20(Admin.BShare()).balanceOf((address(this)));
            //         IERC20(Admin.BShare()).approve(router, balanceBShare); //BSC Testnetapeswap router address
            //         uint convertedBShare = IUniswapV2Router02(router).swapExactTokensForTokens( 
            //             balanceBShare,
            //             0,
            //             path,
            //             address(this),
            ...
            //     }
            // }
            INTT(BShareBUSD_A).mint(address(this),balance);
        }
    ...
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        ...
        INTT(BShareBUSD_A).mint(user, balance);
        exchangeRatio = (IERC20(address(BShareBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(BShareBUSD_A).totalSupply());
        return(balance);
    }
