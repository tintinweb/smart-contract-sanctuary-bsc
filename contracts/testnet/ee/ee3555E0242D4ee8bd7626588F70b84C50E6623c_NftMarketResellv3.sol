...
contract NftMarketResellv3 is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    struct NftStoragebyId{
        uint256 nftId;
    }

    mapping(uint256 => MarketItem) public idToMarketItem;
    mapping(uint256 => NftStoragebyId) public idToNftStorage;
    ...
    function listNft(uint256 tokenId, uint256 price) external payable nonReentrant {
        require(idToMarketItem[tokenId].tokenId == 0,"NftMarketResell: tokenId != 0");
        require(idToNftStorage[tokenId].nftId == 0,"NftMarketResell: nftId != 0");
        ...
        idToNftStorage[newItemId] = NftStoragebyId({
            nftId: tokenId
        });
    ...
    //  function nftListing() public view returns (MarketItem[] memory){
    //     uimt256 nftCount = nft.totalSupply();
    //     uint currentIndex = 0;
    //     List[] memory nfts = new List[](nftCount);
    //     for (uint256 i = 0; i <= nftCount; i++) {
    //         if(idToMarketItem[i + 1].holder == address(this)){
    //             uint currentId = i + 1;
    //             List storage currentNft = idToMarketItem[currentId];
