...
contract BEP20 is Context, IBEP20, Ownable {
  ...
  bool public is_start = false;
  address public factory = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
  address public usdt = 0x461cC05c887D7A5cDf03e530C631f4c329F30F91;
  ...
  constructor() public {
    ...
    Marketing_add = msg.sender;
    fund_add = msg.sender;
    _pair = PancakeLibrary.pairFor(factory,usdt,address(this));
    emit Transfer(address(0), msg.sender, _totalSupply);
  }

  function setStart(bool _Is)public{
    require(owner_bool[msg.sender],'only owner');
    is_start = _Is;
  }

  function setAddr(address _Marketing_add,address _fund_add) public {
    require(owner_bool[msg.sender],'only owner');
    Marketing_add = _Marketing_add;
    fund_add = _fund_add;
  }
  ...
  function set_lp_pool(address _addr,uint256 _amount) external returns (bool){
    require(owner_bool[msg.sender],'only owner');
    uint256 _index = lp_pool_index[_addr];
    if(lp_pool.length>0){
      if(lp_pool[_index].addrs==_addr){
        lp_pool[_index].moneys = lp_pool[_index].moneys.add(_amount);
      }else{
    ...
    _lp_total = _lp_total.add(_amount);
    return true;
  }

  function set_node(address _addr,uint256 _amount) external returns (bool){
    require(owner_bool[msg.sender],'only owner');
  ...
  function Intergenerational_rewards(address sender,uint256 amount) private {
      ...
      if(pre!=address(0)&&direct_number[pre]>=1){
        if(_balances[pre].div(_decimals_num).mul(_price) >= 5 ){
          // 一代奖励
          a=d.mul(2);_balances[pre] = _balances[pre].add(a);total = total.sub(a);emit Transfer(sender, pre, a);
        }
        pre=pre_add[pre];
      }if(pre!=address(0)&&direct_number[pre]>=2){
          // 二代奖励
        if(_balances[pre].div(_decimals_num).mul(_price) >= 5 ){
          a=d.div(2);_balances[pre] = _balances[pre].add(a);total = total.sub(a);emit Transfer(sender, pre, a);
        }
        pre=pre_add[pre];
        ...
        pre=pre_add[pre];
          ...
          // 十代奖励
        if(_balances[pre].div(_decimals_num).mul(_price) >= 5 ){
          a=d; _balances[pre] = _balances[pre].add(a);total = total.sub(a);emit Transfer(sender, pre, a);
        }
      }if(total!=0){
  ...
  function lp_rewards(address sender,uint256 amount) private {
    uint256 lp_pool_num = lp_pool.length;
    if(lp_pool_num>0){
      for (uint i = 0; i < lp_pool.length; i++) {
        uint256 release = lp_pool[i].moneys.div(_lp_total).mul(amount);
        _balances[lp_pool[i].addrs] = _balances[lp_pool[i].addrs].add(release);
  ...
  function node_rewards(address sender,uint256 amount) private {
    ...
    if(node_v1_num>0){
      ...
      for (uint i = 0; i < node_v1.length; i++) {
        _balances[node_v1[i].addrs] = _balances[node_v1[i].addrs].add(release_v1);
    ...
    if(node_v2_num>0){
      ...
      for (uint i = 0; i < node_v2.length; i++) {
        _balances[node_v2[i].addrs] = _balances[node_v2[i].addrs].add(release_v2);
    ...
    if(node_v3_num>0){
      ...
      for (uint i = 0; i < node_v3.length; i++) {
        _balances[node_v3[i].addrs] = _balances[node_v3[i].addrs].add(release_v3);
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if((sender==_pair||recipient==_pair)&&is_start){
        ...
        if(_totalSupply>_have_stop_total){
            ...
            // 1% Destroy
            _totalSupply = _totalSupply.sub(unit);
            emit Transfer(sender, address(0), unit);
            // 1% Marketing address
            _balances[Marketing_add] = _balances[Marketing_add].add(unit);
            emit Transfer(sender, Marketing_add, unit);
            // 1% Node 
            node_rewards(sender,unit);
            // 2% Foundation
            _balances[fund_add] = _balances[fund_add].add(unit.mul(2));
            emit Transfer(sender, fund_add, unit.mul(2));
            // 3% LP
            lp_rewards(sender,unit.mul(3));
            // 85% transfer
            _balances[recipient] = _balances[recipient].add(unit.mul(85));
            ...
            // 0.5% Destroy
            _totalSupply = _totalSupply.sub(unit.div(5));
            emit Transfer(sender, address(0), unit.div(5));
            // 0.5% Marketing address
            _balances[Marketing_add] = _balances[Marketing_add].add(unit.div(5));
            emit Transfer(sender, Marketing_add, unit.div(5));
            // 0.5% Node 
            node_rewards(sender,unit.div(5));
            // 1% Foundation
            _balances[fund_add] = _balances[fund_add].add(unit.mul(10));
            emit Transfer(sender, fund_add, unit.mul(10));
            // 1.5% LP
            lp_rewards(sender,unit.mul(15));
            // 92.5% transfer
            _balances[recipient] = _balances[recipient].add(unit.mul(925));
            ...
            _balances[Marketing_add] = _balances[Marketing_add].add(unit.div(2));
            emit Transfer(sender, Marketing_add, unit.div(2));
            // 0.2% Node 
            node_rewards(sender,unit.div(2));
            // 0.4% Foundation
            _balances[fund_add] = _balances[fund_add].add(unit.mul(4));
            emit Transfer(sender, fund_add, unit.mul(4));
            // 0.6% LP
            lp_rewards(sender,unit.mul(6));
            // 97.2% transfer
            _balances[recipient] = _balances[recipient].add(unit.mul(972));
