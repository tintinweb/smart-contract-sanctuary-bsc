...
contract BCLNFT is ERC721, Ownable{
    ...
    uint256 public nativateFee = 2000;
    ...
    function mint(string memory petanId,string memory uri,uint256 _tokenamount) public returns(bool){
        ...
        uint256 _id = TokenInfo[petanId];
        address _nftcreater = info[_id].owner;
    ...
    function mint(string memory petanId,string memory _cid) public payable returns(bool){
        ...
        uint256 adminfee = (_tokenamount.mul(nativateFee)).div(10000);
        uint256 owneramout = _tokenamount.sub(adminfee);
    ...
    function stopminting(string memory petanId,bool status) public returns(bool){
        onlyvalidUser(msg.sender);
        require(citizencontract.userRole(_msgSender()) > 1 ,"is not minter or childAddress");
        uint256 _id = TokenInfo[petanId];
        require(info[_id].owner == msg.sender || owner() == msg.sender,"is not call petan owner");
        info[_id].isactivate = status;
        validcid[petanId] = status;
        return true;
    }

    function changeData(string memory petanId,uint256 _BNB,uint256 _token,uint256 total) public returns(bool){
        onlyvalidUser(msg.sender);
        require(citizencontract.userRole(_msgSender()) > 1 ,"is not minter or childAddress");
        uint256 _id = TokenInfo[petanId];
    ...
    function AppoveFoePatenId(string[] memory pattenId,bool[] memory status) public returns(bool){
        require(_msgSender() == owner() || isAdmin[_msgSender()], "NFT: Not Owner");
    ...
    function changeMarketpalceAddress(address _address) public onlyOwner returns(bool){
        address oldaddress = nftmarketpalce;
        nftmarketpalce = _address;
        emit changeMarketPlaceAddress(oldaddress,nftmarketpalce);
        return true;
    }
    ...
    function changeFee(uint256 native,uint256 token) public onlyOwner returns(bool){
        uint256 oldnativeFee = nativateFee;
