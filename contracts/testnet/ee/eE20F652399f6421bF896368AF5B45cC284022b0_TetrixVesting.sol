...
interface ITetrixToken is IERC20 {
  ...
  IVesting
{
  ...
  //the timestamp when all accounts can start releasing $TETRIX
  //November 1, 2022 12:00AM GMT
  uint64 public constant UNLOCK_DATE = 1667260800;

  //this is the contract address for $TETRIX
  ITetrixToken public immutable TETRIX_TOKEN;
  //this is the contract address for the $TETRIX treasury
  address public immutable TETRIX_TREASURY;
  //this is the contract address for the $TETRIX economy engine
  address public immutable TETRIX_ECONOMY;

  // ============ Store ============

  //the ETH price per $TETRIX
  uint256 public currentTokenPrice;
  //the $TETRIX limit that can be vested
  uint256 public currentTokenLimit;
  //the total $TETRIX that are currently allocated
  ...
  //mapping of address to $TETRIX vesting
  mapping(address => uint256) public vestingTokens;
  //mapping of address to $TETRIX already released
   ...
   */
  constructor(ITetrixToken token, address treasury, address economy) {
  ...
  /**
   * @dev Release $TETRIX that have already vested.
   *
  ...
  function release(address beneficiary) public nonReentrant {
    ...
    //next mint tokens
    address(TETRIX_TOKEN).functionCall(
      abi.encodeWithSelector(
        TETRIX_TOKEN.mint.selector, 
        beneficiary, 
        releasable
      ), 
    ...
    emit ERC20Released(address(TETRIX_TOKEN), beneficiary, releasable);
  }
    ...
    external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    //now withdraw
    Address.sendValue(payable(TETRIX_TREASURY), amount);
  }
    ...
    external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE)
  {
    ...
    //add to the withdrawn
    currentTotalWithdrawn += amount;
    Address.sendValue(payable(TETRIX_ECONOMY), amount);
  }
