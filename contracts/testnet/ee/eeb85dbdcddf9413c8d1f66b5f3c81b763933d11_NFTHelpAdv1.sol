...
contract NFTHelpAdv1 is Ownable {
    ...
    function rentHero(uint256 _tokenID) external lock haveNft {
    ...
    function unrentHero(uint256 _tokenID) external haveNft {
        (, , , , uint8 _status,,) = IYiBoxNFT(NFTToken).tokenBase(_tokenID);
        require(_status == 5 || _status == 6,"6 status error");
        IYiBoxNFT(NFTToken).setStatus(_msgSender(), _tokenID, 4);
    }

    function unlockBox(uint256 _num) external lock haveNft {
    ...
    function farm(uint256 _tokenID) external haveNft haveHashpool {
    ...
    function unfarm(uint256 _tokenID) external haveNft haveHashpool {
        IHashratePool(HashratePool).settlementAll();
        (, , , , uint8 _status,,) = IYiBoxNFT(NFTToken).tokenBase(_tokenID);
        require(_status == 4,"4 status error");
