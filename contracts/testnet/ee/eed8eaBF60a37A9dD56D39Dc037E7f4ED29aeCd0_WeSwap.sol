...
contract WeSwap is Ownable, Lockable, Strings, SafeMath {
    ...
    event debugUInt256_app(uint256 tag, uint256 value);
    event debugAddress_app(uint256 tag, address addr);
    event debugBool_app(uint256 tag, bool value);
    event debugBytes32_app(uint256 tag, bytes32 value);
    ...
    function swapToken(
        ...
        emit debugAddress_app(801, fromTokenAddress);
        emit debugUInt256_app(802, fromTokenAmount);
        emit debugAddress_app(803, toTokenAddress);
        ...
        emit debugBool_app(1001, exists);
        emit debugBytes32_app(1002, hash);
        ...
        if (exists) {
            ...
            emit debugUInt256_app(2001, resultAmount);
            ok = true;
        } else {
            (
                bool hasBridge,
