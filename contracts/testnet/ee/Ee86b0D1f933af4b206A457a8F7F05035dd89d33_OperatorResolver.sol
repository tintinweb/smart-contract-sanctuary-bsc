...
import "./abtracts/MixinOperatorResolver.sol";

/// @title Operator Resolver implementation
/// @notice Resolve the operators address
contract OperatorResolver is IOperatorResolver, Initializable, OwnableUpgradeable {
    ...
    function getOperator(bytes32 name)
        external
        view
        override
    ...
    /// @inheritdoc IOperatorResolver
    function areOperatorsImported(
        bytes32[] calldata names,
        Operator[] calldata destinations
    ) external view override returns (bool) {
        uint256 namesLength = names.length;
        require(
            namesLength == destinations.length,
            "OR: INPUTS_LENGTH_MUST_MATCH"
        );
        for (uint256 i = 0; i < namesLength; i++) {
            if (
                operators[names[i]].implementation !=
                destinations[i].implementation ||
    ...
    function importOperators(
        ...
        require(
            names.length == operatorsToImport.length,
            "OR: INPUTS_LENGTH_MUST_MATCH"
        );
        bytes32 name;
    ...
    function rebuildCaches(MixinOperatorResolver[] calldata destinations)
        public
        onlyOwner
    {
...
import "../interfaces/IOperatorResolver.sol";
import "../interfaces/IZoneCapitalFactory.sol";
import "../abtracts/OnlyDelegateCall.sol";

/// @title Mixin operator resolver
/// @notice Store in cache operators name and address/selector
abstract contract MixinOperatorResolver is Initializable, OnlyDelegateCall {
    ...
    function __MixinOperatorResolver_init(address _resolver)
        internal
        initializer
    {
        __MixinOperatorResolver_init_unchained(_resolver);
    }

    function __MixinOperatorResolver_init_unchained(address _resolver)
        internal
        initializer
    {
    ...
    function resolverOperatorsRequired()
        public
        view
        virtual
    ...
    function requireAndGetAddress(bytes32 name)
        internal
        view
        returns (IOperatorResolver.Operator memory)
    {
        IOperatorResolver.Operator memory _foundAddress = operatorCache[name];
        require(
            _foundAddress.implementation != address(0),
            string(abi.encodePacked("MOR: MISSING_OPERATOR: ", name))
        );
    ...
    function callOperator(
        IZoneCapitalFactory.Order calldata _order,
        address _inputToken,
        address _outputToken
    ) internal onlyDelegateCall returns (bool success, uint256[] memory amounts) {
        IOperatorResolver.Operator memory _operator = requireAndGetAddress(
            _order.operator
        );
        console.log("Call operator %s", _operator.implementation);
        ...
        (success, data) = _operator.implementation.delegatecall(
            bytes.concat(_operator.selector, _order.callData)
        );
