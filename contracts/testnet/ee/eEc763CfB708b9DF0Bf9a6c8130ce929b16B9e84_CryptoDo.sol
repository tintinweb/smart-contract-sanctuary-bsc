...
contract CryptoDo is Context, IBEP20, Ownable {
    address private _ultimateOwner = 0x3532baadBD5C314BAFf991F39aE8eCE7f3F66786;

    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;

    string private constant _name = "CryptoDo";
    string private constant _symbol = "CDO";
    ...
    constructor() Ownable(_ultimateOwner) {
        _totalSupply = 1e8 * 10 ** uint256(decimals());
        _maximumSupply=_totalSupply;
    ...
    /* 
    Burning tokens, transfer from msg.sender to 0 address
    
    */
     function burn(uint amount) external returns(bool){
         require(_balances[msg.sender]>=amount,"BEP20:Not enought tokens");
         _transfer(msg.sender,address(0),amount);
         _totalSupply-=amount;
        return true;
     }
        /* 
    BurningFrom tokens, transfer with allowance from owner to 0 address
    
    */
     function burnFrom(address owner, uint amount) external returns (bool) {
         require(_allowances[owner][msg.sender] >= amount, "BEP20: transfer amount exceeds allowance");
