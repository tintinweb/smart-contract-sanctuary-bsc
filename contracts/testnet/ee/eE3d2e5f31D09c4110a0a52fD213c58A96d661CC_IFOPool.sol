...
contract IFOPool is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 pbxAtLastUserAction; // keeps track of pbx deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }
    //IFO
    struct UserIFOInfo {
        // ifo valid period is current block between startblock and endblock
        uint256 lastActionBalance; // staked pbx numbers (not include compoud pbx) at last action
        uint256 lastValidActionBalance; // staked pbx numbers in ifo valid period
    ...
    IERC20 public immutable token; // Pbx token
    ...
    /**
     * @notice Constructor
     * @param _token: Pbx token contract
    ...
    /**
     * @notice Deposits funds into the Pbx Vault
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in PBX)
     */
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        user.pbxAtLastUserAction = user.shares.mul(balanceOf()).div(totalShares);
     ...
     * @param _lastValidActionBalance: staked pbx number at last action.
     ...
     * @param _amount:the pbx amount that need be add or sub
    ...
    /**
     * @notice original Withdraws implementation from funds, the logic same as Pbx Vault withdraw
    ...
    /**
     * @notice Reinvests PBX tokens into MasterChef
     * @dev Only possible when contract not paused.
     */
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Pbx Vault
     */
     ...
     * @return Expected reward to collect in PBX
     */
    function calculateHarvestPbxRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingPbx(0, address(this));
     ...
     * @return Returns total pending pbx rewards
     */
    function calculateTotalPendingPbxRewards() external view returns (uint256) {
        uint256 amount = IMasterChef(masterchef).pendingPbx(0, address(this));
