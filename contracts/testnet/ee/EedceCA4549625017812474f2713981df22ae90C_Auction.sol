...
contract Auction is Ownable {
    ...
    function getBidsByLot(uint256 lotKey) external view returns (Bids.Item[] memory) {
        return bids.getByLot(lotKey);
    }

    function getTopBids() external view returns (Bids.Item[] memory) {
        return bids.getTopBids();
    }
...
library Bids {
    ...
    struct Set {
        Item[] top;
        mapping(uint256 => Item[]) bids;
        mapping(uint256 => uint256) indexOfTopBidByLot;
        mapping(address => uint256) lastBiddedLotOfBidder;
    }

    function add(Set storage set, uint256 lotKey, address bidder, uint256 amount) internal {
        Item memory bid = Item({ lotKey: lotKey, amount: amount, bidder: bidder, timestamp: block.timestamp });

        set.bids[lotKey].push(bid);

        set.lastBiddedLotOfBidder[bidder] = lotKey;

        // add to top bids array when is the first bid of the lot, otherwise replace
        if (set.bids[lotKey].length == 1) {
            set.top.push(bid);

            set.indexOfTopBidByLot[lotKey] = set.top.length - 1;
        } else {
            set.top[set.indexOfTopBidByLot[lotKey]] = bid;
        }
    }

    function getByLot(Set storage set, uint256 lotKey) internal view returns (Item[] memory) {
        return set.bids[lotKey];
    }

    function getTopBids(Set storage set) internal view returns (Item[] memory) {
        return set.top;
    }

    function getTopBidByLot(Set storage set, uint256 lotKey) internal view returns (Item memory) {
        if (set.bids[lotKey].length == 0) return Item({ lotKey: lotKey, amount: 0, bidder: address(0), timestamp: 0 });

        return set.bids[lotKey][set.bids[lotKey].length - 1];
    }
