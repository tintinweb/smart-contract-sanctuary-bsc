...
interface INFTMarketplaceRoyalty {
    ...
    // Who can set: ERC721 owner and NFT owner
    event SetRoyalty(
...
interface INFTMarketplace {
    ...
    function withdrawBidForTokenBNB(address erc721Address, uint256 tokenId)
        external;
    ...
    function getTokenListings(
        address erc721Address
    ) external view returns (Listing[] memory, string[] memory);
    ...
    function getAccountTokenListed(
        address erc721Address,
        uint256 tokenId,
        address account
    ...
    ReentrancyGuard
{
    ...
    uint256 private _actionTimeOutRangeMax = 5259486; // This can extend by owner is contract is working smoothly
    uint256 private _bidbalance;
    ...
    function enterBidForTokenBNB(
        ...
        onlyAllowedExpireTimestamp(expireTimestamp)
    {
        Bid memory bid = Bid(tokenId, msg.value, msg.sender, expireTimestamp);
        ...
        override
    {
        ...
        (bool BidderSuccess, /* bytes memory data */) = payable(msg.sender).call{value: bid.value, gas: 30000}("");
        require(BidderSuccess, "receiver rejected BNB transfer");

        _bidbalance -= bid.value;
    ...
    function acceptBidForTokenBNB(
        ...
        (bool WithdrawSuccess, /* bytes memory data */) = payable(msg.sender).call{value: existingBid.value - _serviceFee - _royaltyFee, gas: 30000}("");
        require(WithdrawSuccess, "receiver rejected BNB transfer");
        (bool ServiceSuccess, /* bytes memory data */) = payable(owner()).call{value: _serviceFee, gas: 30000}("");
        require(ServiceSuccess, "receiver rejected BNB transfer");
        if (_royaltyRecipient != address(0) && _royaltyFee > 0) {
        (bool RoyaltySuccess, /* bytes memory data */) = payable(_royaltyRecipient).call{value: _royaltyFee, gas: 30000}("");
        require(RoyaltySuccess, "receiver rejected BNB transfer");
        }
    ...
    function getAccountListings(
        address erc721Address,
        address account
    ) external view override returns (Listing[] memory accountListings) {
     
        uint256 tokenCount = numTokenListings(erc721Address);
        uint256 querySize = tokenCount;
            accountListings = new Listing[](querySize);
            for (uint256 i = 0; i < querySize; i++) {
                accountListings[i] = getAccountTokenListed({
                    erc721Address: erc721Address,
                    tokenId: _erc721Market[erc721Address].tokenIdWithListing.at(
                        i
                    ),
                    account: account
                });
