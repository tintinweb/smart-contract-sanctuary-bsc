...
contract Exchange is ExchangeDomain, Initializable, ERC2771ContextUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, FeeComp, RoyaltyComp, BidComp {
    ...
    function createAndToBid(CreateAndToBidRequest calldata createAndToBidReq) public {
        if (createAndToBidReq.asset.assetType == LibAsset.AssetType.ERC721) {
            SingleNFT singleNft = SingleNFT(createAndToBidReq.asset.token);
            singleNft.exchangeMint(_msgSender(), createAndToBidReq.asset.tokenId, createAndToBidReq.uri, address(this), _msgSender(), createAndToBidReq.royalty);
        }
        else if (createAndToBidReq.asset.assetType == LibAsset.AssetType.ERC1155){
            MultiNFT multiNft = MultiNFT(createAndToBidReq.asset.token);
            multiNft.exchangeMint(_msgSender(), createAndToBidReq.asset.tokenId, createAndToBidReq.asset.tokenAmount,  createAndToBidReq.uri, _msgSender(), createAndToBidReq.royalty, address(this));
        }
    } 

    function bidVerify(BidVerifyRequst calldata bidVerifyReq)
        public
    {
        BidInfo storage bidInfo = bidinfos[_msgSender()][bidVerifyReq.asset.token][bidVerifyReq.asset.tokenId];
        if (bidInfo.tokenAmount != 0){
            return;
        }

        bidinfos[_msgSender()][bidVerifyReq.asset.token][bidVerifyReq.asset.tokenId] = BidInfo(bidVerifyReq.tokenAmount, bidVerifyReq.currency, bidVerifyReq.lowprice, address(0), bidVerifyReq.expirationTime);
    }

    function bid(ForwarderUpgradeable.ForwardRequest calldata bidVerifyReq, bytes calldata bidVerifySign, BidRequest calldata bidReq) 
        public
        payable
    {
        bool execOk;
        (execOk, ) = ForwarderUpgradeable(_trustedForwarder).execute(bidVerifyReq, bidVerifySign);
        require(execOk,"Exchange: Exec bidVerify faild");

        BidInfo storage bidInfo = bidinfos[bidReq.seller][bidReq.asset.token][bidReq.asset.tokenId];
    ...
    function rewardBid(ForwarderUpgradeable.ForwardRequest calldata createAndToBidReq, bytes calldata createAndToBidSign, BidRequest calldata bidReq)
        public
    {
        bool execOk;
        (execOk, ) = ForwarderUpgradeable(_trustedForwarder).execute(createAndToBidReq, createAndToBidSign);
        require(execOk,"Exchange: Exec createAndToBid faild");

        BidInfo storage bidInfo = bidinfos[bidReq.seller][bidReq.asset.token][bidReq.asset.tokenId];

        require(block.timestamp > bidInfo.expirationTime, "Exchange: The bids time period is not over yet");

        delete bidinfos[bidReq.seller][bidReq.asset.token][bidReq.asset.tokenId];

         if (bidReq.asset.assetType == LibAsset.AssetType.ERC721) {
            SingleNFT singleNft = SingleNFT(bidReq.asset.token);
            singleNft.transferFrom(bidReq.seller, _msgSender(), bidReq.asset.tokenId);
        }
        else if (bidReq.asset.assetType == LibAsset.AssetType.ERC1155){
            MultiNFT multiNft = MultiNFT(bidReq.asset.token);
            multiNft.safeTransferFrom(bidReq.seller, _msgSender(), bidReq.asset.tokenId, bidReq.asset.tokenAmount, "Transfer by Exchange");
        }

        uint256 feeAmount = _payFee(address(this), bidInfo.currency, bidInfo.price);
        uint256 royaltyAmount = _payRoyalty(bidReq.asset.token, bidReq.asset.tokenId, address(this), bidInfo.currency, bidInfo.price);
        uint256 remainingAmount = bidInfo.price - feeAmount - royaltyAmount;

        if(bidInfo.currency == address(0)){
            payable(bidReq.seller).transfer(remainingAmount);
        }else{
            IERC20Upgradeable erc20 =  IERC20Upgradeable(bidInfo.currency);
            require(erc20.transferFrom(address(this), address(this), royaltyAmount), "BidComp: Bid with ERC20 token failed");
        }
    ...
    function _swap(BuyRequest calldata buyReq) internal {
        ...
        require((sellInfo.price <= sellInfo.price),"Exchange: match price faild");
        ...
        uint256 feeAmount = _payFee(_msgSender(), sellInfo.currency, sellInfo.price);
        uint256 royaltyAmount = _payRoyalty(buyReq.asset.token, buyReq.asset.tokenId, _msgSender(), sellInfo.currency, sellInfo.price);
...
contract ExchangeDomain {
    ...
    mapping (address => mapping (address => mapping (uint256 => BidInfo))) public bidinfos;
}
