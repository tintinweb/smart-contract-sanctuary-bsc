...
contract Manager is AccessControl, ReentrancyGuard, VRFConsumerBase {
    ...
    event RaffleEnded(
        ...
        uint256 amountRaisedInBNB,
        uint256 amountRaisedInBUSD,
        uint256 randomNumber
    );
    ...
    // Event sent when entries are sold out
    event EntrySoldOut(
        uint256 indexed raffleId,
        uint256 indexed totalEntries
    );
    ...
    event RaffleCancelled(uint256 indexed raffleId, uint256 amountRaisedInBUSD, uint256 amountRaisedInBNB);
    ...
    event SetWinnerTriggered(uint256 indexed raffleId, uint256 indexed entriesLength);

    /* every raffle has a fixed price for each entry bought*/
    struct PriceStructure {
        uint256 priceInBUSD;
        uint256 priceInBNB;
    }

    mapping(uint256 => PriceStructure) public prices;
    ...
    struct RaffleStruct {
        ...
        uint256 totalEntries; // A winner is set whenever totalEntries is reached
        ...
        uint256 amountRaisedInBNB; // funds raised so far
        uint256 amountRaisedInBUSD;
    ...
    struct ClaimStruct {
        ...
        uint256 amountSpentInBNB;
        uint256 amountSpentInBUSD;
        bool claimed;
    }
    ...
    // The operator role is operated by an administrator
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR");
    // The patron role is operated by a backend application
    bytes32 public constant PATRON_ROLE = keccak256("PATRON");
    ...
    payable(0x442aC5eE186Bd775A8AFbcE5Afa89F7c814a7835);
    // BUSD contract instance
    BUSDInterface BUSDtoken = BUSDInterface(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee);
    ...
    {
        _setupRole(OPERATOR_ROLE, 0x3b278B98A55Eb0E6F4969947f5187D4Ad361DCa8);
        _setupRole(PATRON_ROLE, 0xea974837A5b5E99d925D507a2125D40f311C6a2B);
    ...
    /// @param _price The price of entries the customer could purchase
    ...
    function createRaffle(
        ...
        PriceStructure calldata _price,
        ...
        priceInBUSD: _price.priceInBUSD,
        priceInBNB: _price.priceInBNB
        });
        prices[raffles.length - 1] = p;
    ...
    */
    ...
    /// It is checked the maximum number of entries per user is not reached
    ...
    /// @param _raffleId: id of the raffle
    /// @param _numEntries: number of entries player wanna buy
    ...
    function buyEntry(
        ...
        require(_numEntries > 0, "howMany is 0");
        require(
        ...
        require(
            claimsData[hash].numEntriesPerUser + _numEntries <=
            raffles[_raffleId].maxEntries,
            "Bought too many entries"
        );
        require(raffles[_raffleId].entriesLength + _numEntries <= raffles[_raffleId].totalEntries, "Exceeded total entries allowed");

        if (msg.value > 0) {
            require(
                msg.value >= prices[_raffleId].priceInBNB * _numEntries,
                "msg.value must be greater than the price"
            );
            raffles[_raffleId].amountRaisedInBNB += msg.value;
            claimsData[hash].amountSpentInBNB += msg.value;
        } else {
            uint256 cost = prices[_raffleId].priceInBUSD * _numEntries;
            require(
                BUSDtoken.balanceOf(msg.sender) >= cost,
                "BUSD balance must be greater than the price"
            );
            bool success = BUSDtoken.transferFrom(msg.sender, address(this), cost);
            require(success, "Pay in BUSD failed");
            raffles[_raffleId].amountRaisedInBUSD += cost;
            claimsData[hash].amountSpentInBUSD += cost;
        }

        // add a new element to the entriesBought array, used to calc the winner
        EntriesBought memory entryBought = EntriesBought({
        player: msg.sender,
        currentEntriesLength: raffles[_raffleId].entriesLength + _numEntries
        });
        entriesList[_raffleId].push(entryBought);

        // update the field entriesLength, used in frontend to avoid making extra calls
        raffles[_raffleId].entriesLength = raffles[_raffleId].entriesLength + _numEntries;
        //update claim data
        claimsData[hash].numEntriesPerUser += _numEntries;

        if (raffles[_raffleId].entriesLength >= raffles[_raffleId].totalEntries) {
            emit EntrySoldOut(
                _raffleId,
                raffles[_raffleId].totalEntries
            );
        }
    ...
    function giveBatchEntriesForFree(
    ...
    ) external nonReentrant onlyRole(PATRON_ROLE) {
        require(
            raffles[_raffleId].status == STATUS.ACCEPTED,
            "Raffle is not in accepted"
        );
        ...
        require(raffles[_raffleId].entriesLength + freePlayersLength <= raffles[_raffleId].totalEntries, "Exceeded total entries allowed");
    ...
    function transferNFTAndFunds(
        ...
        uint256 amountForSellerInBNB = raffle.amountRaisedInBNB;
        uint256 amountForSellerInBUSD = raffle.amountRaisedInBUSD;
        if (amountForSellerInBNB > 0) {
            (bool sent, ) = raffle.seller.call{value: amountForSellerInBNB}("");
            require(sent, "Failed to send BNB");
        }
        if (amountForSellerInBUSD > 0) {
            bool sent = BUSDtoken.transfer(raffle.seller, amountForSellerInBUSD);
            require(sent, "Failed to send BUSD");
        }

        emit RaffleEnded(
            _raffleId,
            raffle.winner,
            raffle.amountRaisedInBNB,
            raffle.amountRaisedInBUSD,
            _normalizedRandomNumber
        );
    }

    /// @param _raffleId Id of the raffle
    /// @notice the operator finish the raffle, if the total entries number has been reached
    ...
    onlyRole(PATRON_ROLE)
    {
        ...
        require(
            raffle.totalEntries <= raffle.entriesLength,
        ...
        emit SetWinnerTriggered(_raffleId, raffle.entriesLength);
    }
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        emit RaffleCancelled(_raffleId, raffle.amountRaisedInBNB, raffle.amountRaisedInBUSD);
    }
    ...
    function claimRefund(uint256 _raffleId) external nonReentrant {
        ...
        raffle.amountRaisedInBNB = raffle.amountRaisedInBNB - claimData.amountSpentInBNB;
        raffle.amountRaisedInBUSD = raffle.amountRaisedInBUSD - claimData.amountSpentInBUSD;

        claimData.claimed = true;
        if (claimData.amountSpentInBNB > 0) {
            (bool sent, ) = msg.sender.call{value: claimData.amountSpentInBNB}("");
            require(sent, "Fail send refund BNB");
        }
        if (claimData.amountSpentInBUSD > 0) {
            bool sent2 = BUSDtoken.transfer(msg.sender, claimData.amountSpentInBUSD);
            require(sent2, "Fail send refund BUSD");
        }
        emit Refund(_raffleId, claimData.amountSpentInBNB, claimData.amountSpentInBUSD, msg.sender);
    }
    ...
    onlyRole(OPERATOR_ROLE)
    {
        ...
        if (raffle.amountRaisedInBNB > 0) {
            (bool sent, ) = destinationWallet.call{value: raffle.amountRaisedInBNB}("");
            require(sent, "Fail send BNB to OhMyNFT");
        }
        if (raffle.amountRaisedInBUSD > 0) {
            bool sent2 = BUSDtoken.transfer(destinationWallet, raffle.amountRaisedInBUSD);
            require(sent2, "Fail send BUSD to OhMyNFT");
        }
        emit RemainingFundsTransferred(_raffleId, raffle.amountRaisedInBNB, raffle.amountRaisedInBUSD);

        raffle.amountRaisedInBNB = 0;
        raffle.amountRaisedInBUSD = 0;
    }
