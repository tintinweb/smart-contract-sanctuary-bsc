...
contract Marketplace is Ownable{
    ...
    function enableProfit(uint256 tokenID) external onlyOwner {
        require(!profit[tokenID].status, "You have already enabled");
        Profit memory _profit = Profit(true, 40, 60);
        profit[tokenID] = _profit;
    }

    function disableProfit(uint256 tokenID) external onlyOwner {
        require(profit[tokenID].status, "You haven't enabled");
