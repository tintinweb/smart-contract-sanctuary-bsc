    ...
    TokenSaver
{
   ...
   */
    function deposit(address _depositToken, uint256 _amount, uint8 _stakingPeriodMonth, address _receiver) external {
        ...
        uint256 depositId = depositsOf[_receiver].length;
        ...
        emit Deposited(depositId, _depositToken, _amount, block.timestamp, _endPeriod, _aFactor, _receiver, _msgSender());
    }
   ...
   */
    function destroyLock(uint256 _depositId) external {
    ...
    function _withdraw(uint256 _depositId, uint256 _amount) internal {
        ...
        require(userDeposit.amount > 0 && userDeposit.amount >= _amount, "TimeLockPool.withdraw: Insufficient balance");
        ...
        if (userDeposit.amount == 0) {
            delete depositsOf[_msgSender()][_depositId];
        }

        IERC20Upgradeable(userDeposit.depositToken).safeTransfer(_msgSender(), _amount);

        depositsOf[_msgSender()][_depositId] = userDeposit;
        // emit Withdrawn(_depositId, _msgSender(), _msgSender(), userDeposit.amount, block.timestamp);
    }

    function claimAndWithdraw(uint256 _depositId) public {
        ...
        (, ,uint256 shareAmount) = calcDividendsMultiplier(userDeposit.depositToken, userDeposit.aFactor, userDeposit.period, userDeposit.amount);
    ...
    external view returns (uint8 aFactor, uint256 reword) {
        aFactor = aFactors[_period];
        (,,reword) = calcDividendsMultiplier(_token, aFactor, _period, _amount);
    }

    // function getDividendsMultiplier(address _token, uint8 _aFactor, uint8 _period, uint256 _amount)
    // internal view returns (uint256 tokenAmountReword)
    // {
    //     uint256 dividends = (_amount.mul(_aFactor) / PERIOD_DECIMALS).mul(_period * PERIOD_DECIMALS / APY_PERIOD) / PERIOD_DECIMALS;
    //     uint256 dividendsOfUsd = depositTokensRates[_token].mul(dividends) / TOKEN_DECIMALS;
    //     tokenAmountReword = dividendsOfUsd / rewordTokenRate * TOKEN_DECIMALS;
    // }

    function calcDividendsMultiplier(address _token, uint8 _aFactor, uint8 _period, uint256 _amount)
    public view returns (uint256 dividends, uint256 dividendsOfUsd, uint256 tokenAmountReword)
    {
        dividends = (_amount.mul(_aFactor) / PERIOD_DECIMALS).mul(_period * PERIOD_DECIMALS / APY_PERIOD) / PERIOD_DECIMALS;
