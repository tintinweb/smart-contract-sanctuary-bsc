...
contract SmoltingInu is ERC20, Ownable {
    uint256 private constant ONE_HOUR = 5 minutes;
    uint256 private constant PERCENT_DENOMENATOR = 1000;
    address private constant DEAD = address(0xdead);

    uint256 public coinFlipMinBalancePerc = (PERCENT_DENOMENATOR * 25) / 100; // 25% user's balance
    ...
    uint256 private _liquifyRate = (PERCENT_DENOMENATOR * 1) / 1000; // 0.1%
    ...
    function flipCoin(uint256 _percent) external returns (bool) {
        ...
        _approve(address(this), address(uniswapV2Router), _amountForMarket);
    ...
    function _checkAndPayBiggestBuyer() private {
        if (block.timestamp - periodStartTime[period] >= ONE_HOUR) {
            ...
            if (
                ...
                if (_before > 0) {
                    uint256 _buyerAmount = (_before *
                        biggestBuyRewardPercentage) / PERCENT_DENOMENATOR;
                    totalReward += _buyerAmount;
                    payable(biggestBuyer[_prevPeriod]).transfer(_buyerAmount);
                    require(
                        address(this).balance >= _before - _buyerAmount,
                        "too much ser"
                    );
                    biggestBuyerPaid[_prevPeriod] = _buyerAmount;
                }
            }
        }
    }
