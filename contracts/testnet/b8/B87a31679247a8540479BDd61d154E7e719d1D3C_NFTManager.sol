...
contract NFTManager is IERC721Receiver, Ownable {
    ...
    mapping(address => mapping (address => uint256[])) private tokensOfOwner;
    ...
    function getNFTOwner(address _contractAddress, uint256 _tokenId) external view returns (address) {
        return ownerOf[_contractAddress][_tokenId];
    }

    function getUserDepositedNFT(address _contractAddress, address _userAddress) external view returns (uint256[] memory) {
        return tokensOfOwner[_contractAddress][_userAddress];
    }

    function getUserDepositedNFTLength(address _contractAddress, address _userAddress) external view returns (uint256) {
