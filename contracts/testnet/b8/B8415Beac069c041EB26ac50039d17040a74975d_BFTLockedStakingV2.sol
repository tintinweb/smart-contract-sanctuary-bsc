...
contract BFTLockedStakingV2 is IERC20Staking {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public periodicTime = 365 days;
    uint256 planLimit = 3;
    uint256 public refRate = 3;
    ...
    function referralStake(uint256 _stakingId, uint256 _amount, address _referrer) public {   
        ...
        if ((_referrer != msg.sender) && (_referrer != address(0))) {
            if(userRef[msg.sender].status == false){
                ...
                userRef[_referrer].totalRef += 1;
                userRef[_referrer].totalEarning = userRef[_referrer].totalEarning.add(refReward);
                userRef[_referrer].claimableEarning = userRef[_referrer].claimableEarning.add(refReward);                                
            }                      
        }
    ...
    function claimReferralEarnings(uint256 _ramount) public {
