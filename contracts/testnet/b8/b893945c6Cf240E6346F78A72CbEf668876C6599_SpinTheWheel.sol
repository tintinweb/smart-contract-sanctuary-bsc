...
contract SpinTheWheel {
    ...
    function spin(uint256 amount) external notPaused {
        ...
        if (rewardPos >= _rewards.length) {
            revert("SpinTheWheel__RandomError");
        }
    ...
    function getRandomReward(uint256 seed) private returns (uint256) {
        uint256 data = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender, seed)));
