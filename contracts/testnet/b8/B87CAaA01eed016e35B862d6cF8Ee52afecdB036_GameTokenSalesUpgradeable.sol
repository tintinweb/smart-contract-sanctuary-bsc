...
contract GameTokenSalesUpgradeable is
    Initializable,
    OwnableUpgradeable,
    ...
    WithdrawableUpgradeable
{
    ...
    uint256 public gPIPBagCount;

    mapping(uint256 => gPIPBag) public gPIPBags;

    event AddGPIPBag(
        uint256 indexed id,
        uint256 indexed gPIPAmount,
    ...
    function buyGPIPBag(uint256 id) external {
        if (id > gPIPBagCount) revert InvalidID();
        gPIPBag memory gpb = gPIPBags[id];
        IERC20Upgradeable(paymentToken).safeTransferFrom(
            _msgSender(),
            treasury,
            gpb.price
        );
        emit BuyGPIPBag(_msgSender(), gpb.gameID, gpb.gPIPAmount);
    }
    ...
    function addGPIPBag(
        ...
        if (gPIPAmount == 0) revert InvalidGPIPAmount();
        if (price == 0) revert InvalidPrice();
        gPIPBags[gPIPBagCount] = gPIPBag({
            gPIPAmount: gPIPAmount,
            price: price,
            gameID: gameID
        });
        emit AddGPIPBag(gPIPBagCount, gPIPAmount, price, gameID);
        gPIPBagCount++;
    }

    function editGPIPBag(
        uint256 id,
        uint256 gPIPAmount,
        uint256 price,
        uint256 gameID
        ...
        if (gPIPAmount == 0) revert InvalidGPIPAmount();
        if (price == 0) revert InvalidPrice();
        gPIPBag storage gpipbag = gPIPBags[id];
        gpipbag.gPIPAmount = gPIPAmount;
        gpipbag.price = price;
        gpipbag.gameID = gameID;
        emit EditGPIPBag(id, gPIPAmount, price, gameID);
    }

    function getGPIPBags() external view returns (gPIPBag[] memory) {
        gPIPBag[] memory result = new gPIPBag[](gPIPBagCount);
        for (uint256 x; x < gPIPBagCount; x++) {
            result[x] = gPIPBags[x];
        }
        return result;
    }
