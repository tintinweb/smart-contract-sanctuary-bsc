...
interface IBEP20Upgradeable {
    ...
    function buyNft(uint256 amount, address spender) external;
    ...
    BEP721BurnableUpgradeable
{
    ...
    function initialize() public initializer {
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("Artalux", "ART");
    }
    ...
    function mint(string memory ipfsmetadata, uint256 tokenId) public {
        _creator[tokenId] = msg.sender;
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, ipfsmetadata);

        balances[tokenId][msg.sender] = 1;
        emit TokenId(msg.sender, tokenId);
    }
    ...
    function saleWithToken(
        ...
        IBEP20Upgradeable t = IBEP20Upgradeable(tokenAss);
        uint256 balanceAmount = t.balanceOf(msg.sender);
        require(balanceAmount >= amount, "Insufficient balance");
        require(balances[tokenId][from] > 0, "Is Not a Owner");
        require(amount > 0, "Amount must be greater than zero");
        t.buyNft(amount, msg.sender);
