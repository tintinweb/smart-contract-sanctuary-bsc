...
contract NFTSwap is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC20Upgradeable public NFT4Play;

    // 0 = DAI,1 = USDC,2 = USDT 
    IERC20Upgradeable[3] public stableCoins;

    address public treasury;
    address public legendaryMedalOwner;
    ...
    struct Game{
        ...
        mapping(uint => uint) cardsPriceIn4Play;
    }
    ...
    event ChangeGameMintable(uint _gameId,bool _mintable);
    event SetOneLegendaryMedalPriceIn4play(uint _price);
    event SetOneLegendaryMedalPriceInDollar(uint _price);
    event ChangeMedalDiscount(uint _discount);
    event SetLegendaryMedalOwner(address _medalOwner);
    event ChangeStatusOfLegendaryMedalsSale(bool _status);
    event ChangeStatusForCards4PlayFixedPrice(bool _status);
    event ChangeStatusOfMedal4PlayFixedPrice(bool _status);
    event ChangeCardDiscount(uint _discount);
    ...
    function initialize(address[3] memory _tokens,address _medalContract,address _treasury,address _priceConsumer) external initializer {
        __Ownable_init();
        for(uint i=0; i< _tokens.length; i++){
            require(_tokens[i] != address(0),"NFTSwap: token address is not valid");
            stableCoins[i] = IERC20Upgradeable(_tokens[i]);
        }
    ...
    modifier validGameId(uint _gameId){
        require(totalListedGame >= _gameId && games[_gameId].mintable,"NFTSwap: use valid game Id");
        _;
    }
    ...
    function set4PlayToken(address _nft4Play) external onlyOwner(){
        ...
        NFT4Play = IERC20Upgradeable(_nft4Play);
    ...
    function addNewGameCard(address _cardContract,uint[] memory _priceInStable,uint[] memory _priceIn4Play) external onlyOwner(){
        ...
        require(!isCardlisted[_cardContract],"NFTSwap: _cardContract is already listed");
        ...
        require(_priceInStable.length == _priceIn4Play.length,"NFTSwap: array length must be same");
        totalListedGame = totalListedGame.add(1);
        Game storage game = games[totalListedGame];
        game.mintable = true;
        game.cardContract = PlayingCard(_cardContract);
        for(uint8 i = 1; i <= _priceIn4Play.length; i++){
            game.cardsPriceIn4Play[i] = _priceIn4Play[i-1];
            game.cardsPriceInStable[i] = _priceInStable[i-1];   
        }
    ...
    function changeGameMintable(uint _gameId,bool _mintable) external onlyOwner(){
        ...
        emit ChangeGameMintable(_gameId,_mintable);
    }
    ...
    function setCardsPriceIn4Play(uint _gameId,uint256[] memory _cardTypes, uint256[] memory _price) external onlyOwner() {
        ...
        for(uint8 i = 0; i < _cardTypes.length; i++){
            ...
            games[_gameId].cardsPriceIn4Play[_cardTypes[i]] = _price[i];
    ...
    function setLegendaryMedalOwner(address _medalOwner) external onlyOwner(){
        ...
        legendaryMedalOwner = _medalOwner;
        emit SetLegendaryMedalOwner(_medalOwner);
    }
    ...
    function changeStatusOfLegendaryMedalsSale(bool _status) external onlyOwner(){
        ...
        emit ChangeStatusOfLegendaryMedalsSale(_status);
    }
    ...
    function changeStatusOfMedal4PlayFixedPrice(bool _status) external onlyOwner(){
        ...
        emit ChangeStatusOfMedal4PlayFixedPrice(_status);
    }
    ...
    function changeStatusForCards4PlayFixedPrice(bool _status) external onlyOwner(){
        ...
        emit ChangeStatusForCards4PlayFixedPrice(_status);
    }
    ...
    function changeCardDiscount(uint _discount) external onlyOwner(){
        ...
        emit ChangeCardDiscount(_discount);
    }
    ...
    function changeMedalDiscount(uint _discount) external onlyOwner(){
        ...
        emit ChangeMedalDiscount(_discount);
    }
    ...
    */
    ...
    function _isPriceSet(uint _gameId,uint _cardType) internal view returns (bool){
        return games[_gameId].cardsPriceInStable[_cardType] > 0;
    }
    ...
    function _mintCardType(address _address,uint _gameId,uint _cardType,uint _amountOfCards) internal{
        if(_cardType == 5 || _cardType == 4){
            ...
            for(uint8 i=0; i < _amountOfCards; i++){
                ...
                if(random == _cardType){
                    games[_gameId].cardContract.mintCard(_address,_cardType.add(1));
                }
                else{
    ...
    function _purchaseMedals(uint _amountOfMedal) internal{
        ...
        require(medalContract.balanceOf(legendaryMedalOwner,legendaryMedalId) >= _amountOfMedal,"NFTSwap: medalOwner balance is low");
        medalContract.safeTransferFrom(legendaryMedalOwner,_msgSender(),legendaryMedalId,_amountOfMedal,"");
    }
    ...
    function payableAmountForLegendary(uint256 _amountOfMedal) public view returns(uint256){
        ...
        return dollerAmount.mul(nft4PlayInDollar()).div(10**18).mul(_amountOfMedal);
    }

    /**
     * @dev purchase Legendary card using stable coin
    */

    function purchaseLegendaryMedal(uint _index,uint _amountOfMedal) external nonReentrant{
        require(legendaryMedalsPriceInStable > 0,"NFTSwap: Price is not set");
        require(stableCoins.length > _index,"NFTSwap: use valid index");
        uint totalPrice = legendaryMedalsPriceInStable.mul( _amountOfMedal);
        stableCoins[_index].safeTransferFrom(_msgSender(),treasury,totalPrice);
        _purchaseMedals(_amountOfMedal);
        emit PurchaseLegendaryMedal(_msgSender(),_index,_amountOfMedal,totalPrice);
    }

    /**
     * @dev purchase Legendary card using 4Play token
    */

    function purchaseLegendaryMedal4play(uint _amountOfMedal) external nonReentrant{
        uint totalPrice;
        ...
        NFT4Play.safeTransferFrom(_msgSender(),treasury,totalPrice);
        _purchaseMedals(_amountOfMedal);
        emit PurchaseLegendaryMedal4play(_msgSender(),_amountOfMedal,totalPrice);
    }
    ...
    */
    ...
    function swapWithMedals(uint _gameId,uint256 _medalId,uint _amountOfMedal) external validGameId(_gameId) whenNotPaused nonReentrant{   
        require(medalContract.balanceOf(_msgSender(),_medalId) >= _amountOfMedal,"NFTSwap: medal balance is low");
        require(_amountOfMedal > 0,"NFTSwap: _amountOfMedal must be greater than zero");
        medalContract.burn(_msgSender(),_medalId,_amountOfMedal);
        _mintCardType(_msgSender(),_gameId,_medalId,_amountOfMedal);
        if(isSetSwapedMedals){
            userSwapedMedals[_msgSender()][_medalId] = userSwapedMedals[_msgSender()][_medalId].add(_amountOfMedal);
        }
        emit SwapWithMedals(_msgSender(),_gameId,_medalId,_amountOfMedal);
    }
    ...
    */
    ...
    function buyCardsWithStableCoin(uint256 _gameId,uint256 _index,uint _cardType,uint256 _noOfCards) external validGameId(_gameId) isPriceSet(_gameId,_cardType) whenNotPaused nonReentrant{
        ...
        uint totalPrice = games[_gameId].cardsPriceInStable[_cardType].mul( _noOfCards);
        stableCoins[_index].safeTransferFrom(_msgSender(),treasury,totalPrice);
        _mintCardType(_msgSender(),_gameId,_cardType,_noOfCards);
        emit BuyCardsWithStableCoin(_msgSender(),_gameId,_index,_cardType,_noOfCards,totalPrice);
    } 
    ...
    */
    ...
    function buyCardsWithNFT4Play(uint256 _gameId,uint _cardType,uint256 _noOfCards) external validGameId(_gameId) whenNotPaused nonReentrant{
        require(games[_gameId].cardContract.totalCardType() > _cardType && _cardType > 0,"NFTSwap: card type is not valid");
        require(_noOfCards > 0,"NFTSwap: no of card must be greater than zero");
        uint totalPrice;
        if(card4PlayFixedPrice){
            require(games[_gameId].cardsPriceIn4Play[_cardType] > 0,"NFTSwap: card price is not set");
            totalPrice = games[_gameId].cardsPriceIn4Play[_cardType].mul(_noOfCards);
        }
        else{
        ...
        NFT4Play.safeTransferFrom(_msgSender(),treasury,totalPrice);
        _mintCardType(_msgSender(),_gameId,_cardType,_noOfCards);
        emit BuyCardsWithNft4Play(_msgSender(),_gameId,_cardType, _noOfCards,totalPrice);
    }

    function payableAmount(uint _gameId,uint _cardType,uint256 _noOfCards) public view returns(uint256){
        require(_isPriceSet(_gameId,_cardType),"NFTSwap: card price is not set");
        uint dollerAmount = (games[_gameId].cardsPriceInStable[_cardType].mul(DENOMINATOR.sub(cardDiscount))).div(DENOMINATOR);
        return dollerAmount.mul(nft4PlayInDollar()).div(10**18).mul(_noOfCards);
    } 
    ...
    function getGameDetail(uint _gameId) external view returns(bool,PlayingCard){
        Game storage game = games[_gameId];
        return (game.mintable,game.cardContract);
    }

    function getCardPriceInStable(uint _gameId,uint _cardType) external view returns(uint){
        return games[_gameId].cardsPriceInStable[_cardType];
    }

    function getCardPriceIn4Play(uint _gameId,uint _cardType) external view returns(uint){
        return games[_gameId].cardsPriceIn4Play[_cardType];
    }
