...
contract GodzillaProject is ERC20, Ownable {
    ...
    function swapBack() private {
        ...
        uint256 amountToSwapForETH = contractBalance.sub(liquidityTokens).sub(tokensForBurn).sub(tokensForOperation);
        
        uint256 initialETHBalance = address(this).balance;

        if (tokensForBurn > 0){
            super._transfer(address(this), burnWallet, tokensForBurn);
            tokensForBurn = 0;
        }

        if (tokensForOperation > 0){
            super._transfer(address(this), operationWallet, tokensForOperation);
