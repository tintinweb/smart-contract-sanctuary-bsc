...
contract CharacterMarketPlace is OwnableUpgradeable, PausableUpgradeable  {
    struct Auction {
        uint256 highestBid;
        uint256 closingTime;
    ...
    function initByOwner(CharacterNFT _characterNFT, address _tokenBEP20) public onlyOwner{
    ...
    function closeAuctionSudden(uint256 _nftId) external whenNotPaused {
        ...
        characterNFT.transfer(_nftId, auctions[_nftId].originalOwner);
...
function openAuction(
        ...
        require(characterNFT.ownerOf(_nftId) == msg.sender, "Not NFT owner");
        characterNFT.approveMarketPlace(address(this),_nftId);
        // NFT Transfer to contract
        characterNFT.transfer(_nftId, address(this));
    ...
    function buy(uint256 _nftId, uint256 _amount) external whenNotPaused {
        ...
        // Transfer NFT to Highest Bidder
        characterNFT.transfer(_nftId, auctions[_nftId].highestBidder);
