...
contract WeRent is IWeRent, ERC721Holder, ERC1155Receiver, ERC1155Holder, Initializable {
    ...
    uint256 private constant SECONDS_IN_DAY = 300;
    ...
    function takeFee(uint256 _rent, bool _useNativeToken) private returns (uint256 fee) {
        fee = _rent * rentFee;
        fee /= 10000;
        if (_useNativeToken) {
        ...
        returns (uint256)
    {
        ...
        uint256 takenFee = takeFee(sendLenderAmt, useNativeToken);
    ...
    function distributeClaimPayment(LendingRenting memory _lendingRenting) private {
        ...
        uint256 takenFee = takeFee(maxRentPayment, useNativeToken);
    ...
    function setExtratoken(address _extraToken) external onlyAdmin {
        extraTokenAddress = ERC20Upgradeable(_extraToken);

        emit ExtraToken(_extraToken);
    }
}
