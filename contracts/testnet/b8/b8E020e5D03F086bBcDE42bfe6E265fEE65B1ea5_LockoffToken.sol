...
contract LockoffToken is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    function checkLocked(address _sender, uint256 _amount) internal view returns (bool) {
        require(_sender != address(0x0), "Recipient zero address");
        if (_gonBalances[_sender] < _amount) {
            return false;
        }
        uint256 presaleLocked = 0;
        if (presale != address(0) ) {
            presaleLocked = ILockOff(presale).totalLocked(_sender);
        }
        uint256 airdropLocked = 0;
        if (airdrop != address(0) ) {
            airdropLocked = ILockOff(airdrop).totalLocked(_sender);
        }
        uint256 avaiable = _gonBalances[_sender].sub(presaleLocked).sub(airdropLocked);
        return  avaiable > _amount;
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(checkLocked(sender, amount), "Insufficient" );
