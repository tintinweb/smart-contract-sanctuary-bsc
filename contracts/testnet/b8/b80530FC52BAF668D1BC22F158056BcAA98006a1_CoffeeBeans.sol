...
contract CoffeeBeans is Context, Ownable {
    using SafeMath for uint256;

    uint256 private beans_TO_Grind_1MINERS = 1080000; //for final version should be seconds in a day
    uint256 private PSN = 10000;
    uint256 private PSNH = 5000;
    uint256 private devFeeVal = 3;
    bool private initialized = false;
    address payable private recAdd;
    mapping (address => uint256) private GrinderyMiners;
    mapping (address => uint256) private claimedbeans;
    mapping (address => uint256) private lastGrind;
    mapping (address => address) private referrals;
    uint256 private marketbeans;
    
    constructor() {
        recAdd = payable(msg.sender);
    }
    
    function Grindbeans(address ref) public {
        require(initialized);
        ...
        uint256 newMiners = SafeMath.div(beansUsed,beans_TO_Grind_1MINERS);
        GrinderyMiners[msg.sender] = SafeMath.add(GrinderyMiners[msg.sender],newMiners);
        claimedbeans[msg.sender] = 0;
        lastGrind[msg.sender] = block.timestamp;
        
        //send referral beans
        claimedbeans[referrals[msg.sender]] = SafeMath.add(claimedbeans[referrals[msg.sender]],SafeMath.div(beansUsed,6));
        
        //boost market to nerf miners hoarding
        marketbeans=SafeMath.add(marketbeans,SafeMath.div(beansUsed,5));
    }
    
    function sellbeans() public {
        require(initialized);
    ...
    function getMyMiners(address adr) public view returns(uint256) {
        return GrinderyMiners[adr];
    }
    
    function getMybeans(address adr) public view returns(uint256) {
        return SafeMath.add(claimedbeans[adr],getbeansSinceLastGrind(adr));
    }
    
    function getbeansSinceLastGrind(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(beans_TO_Grind_1MINERS,SafeMath.sub(block.timestamp,lastGrind[adr]));
        return SafeMath.mul(secondsPassed,GrinderyMiners[adr]);
    }
