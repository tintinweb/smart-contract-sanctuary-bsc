...
contract XMSG is Context, IERC20, Ownable {
    ...
    function getAmountNeeded(uint _amountOut) external view returns(uint) {
        (uint112 token0,uint112 token1,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        uint tokenTotalOut;
        if(token0 > token1){
            tokenTotalOut = uniswapV2Router.getAmountIn(_amountOut, token0, token1);
        }else{
            tokenTotalOut = uniswapV2Router.getAmountIn(_amountOut, token1, token0);
        }
        return tokenTotalOut;
    }

    function buyCredits(uint256 _amountIn, uint256 usd, bool withToken) external payable returns(uint256){ // 
        _marketingFee = 0;
        uint256 amount = 0;
        if(withToken){
            ...
            amount = uint256(bnbPrice * 10**10).mul(bnbTotalOut);
            ...
            path[1] = address(this);
            int bnbPrice = useManualPrice?latestBnbPrice:getLatestBNBPrice();
            amount = uint256(bnbPrice* 10**10).mul(msg.value);
            ...
            ERC20(uniswapV2Router.WETH()).withdraw((IERC20)(uniswapV2Router.WETH()).balanceOf(address(this)));
            ...
            uint256 payment = newBNB - initialBNB;
            _xMsgWallet.transfer(payment);
