...
contract LotteryNFT is ERC721A, ERC721AQueryable, Ownable {
...
contract LotteryOwnable {
    ...
    function initOwner(address theOwner) internal {
        _owner = theOwner;
...
// 4 numbers
contract ACETLottery is LotteryOwnable {
    using SafeMath for uint256;
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "Lottery::onlyAdmin:: only admin can call");
        _;
    }

    modifier inDrawingPhase() {
        require(!drawed(), "Lottery::inDrawingPhase:: drawed, can not buy now");
        require(!drawingPhase, "Lottery::inDrawingPhase:: drawing, can not buy now");
        _;
    }
    ...
    function reset() external onlyAdmin {
        ...
        if (getMatchingRewardAmount(issueIndex - 1, 4) == 0) {
            amount = getTotalRewards(issueIndex - 1)
                                .mul(allocation[0])
                                .div(100);
        ...
        amount = getTotalRewards(issueIndex - 1)
                            .mul(allocation[3])
                            .div(100);
        busd.transfer(devAddress, amount/2);
        busd.transfer(partnerAddress, amount/2);
    ...
    function drawing(uint256 _externalRandomNumber) external onlyAdmin {
        ...
        // 2
        _structHash = keccak256(abi.encode(totalAmount, _blockhash, gasLeft, _externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly { _randomNumber := add(mod(_randomNumber, _maxNumber), 1)}
        winningNumbers[1] = uint8(_randomNumber);

        // 3
        _structHash = keccak256(abi.encode(_blockhash,lastTimestamp,gasLeft,_externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly { _randomNumber := add(mod(_randomNumber, _maxNumber), 1)}
        winningNumbers[2] = uint8(_randomNumber);

        // 4
        _structHash = keccak256(abi.encode(_blockhash, gasLeft, _externalRandomNumber));
        _randomNumber = uint256(_structHash);
        assembly { _randomNumber := add(mod(_randomNumber, _maxNumber), 1)}
    ...
    function internalBuy(uint256 _price, uint8[4] memory _numbers) internal {
        ...
        for (uint256 i = 0; i < 4; i++) {
            require(_numbers[i] <= maxNumber, "Lottery::internalBuy:: exceed the maximum");
        }
    ...
    function injectReward(uint256 _price, uint8[4] memory _numbers) external onlyAdmin
    {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers)        external        inDrawingPhase
    {
        ...
        for (uint256 i = 0; i < 4; i++) {
            require(_numbers[i] <= maxNumber, "Lottery::buy:: exceed number scope");
        }
    ...
    {
        require(_price >= minPrice, "Lottery::multiBuy:: price must above minPrice");
        uint256 totalPrice = 0;
        for (uint256 i = 0; i < _numbers.length; i++) {
            for (uint256 j = 0; j < 4; j++) {
                require(_numbers[i][j] <= maxNumber && _numbers[i][j] > 0, "Lottery::multiBuy:: exceed number scope");
            }
    ...
    function claimReward(uint256 _tokenId) external {
        require(msg.sender == lotteryNFT.ownerOf(_tokenId), "Lottery::claimReward:: not from owner");
        require(!lotteryNFT.getClaimStatus(_tokenId), "Lottery::claimReward:: claimed");
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        for (uint256 i = 0; i < _tickets.length; i++) {
            require(msg.sender == lotteryNFT.ownerOf(_tickets[i]), "Lottery::multiClaim:: not from owner");
            require(!lotteryNFT.getClaimStatus(_tickets[i]), "Lottery::multiClaim:: claimed");
    ...
    {
        require(_issueIndex <= issueIndex, "Lottery::getTotalRewards:: _issueIndex <= issueIndex");
    ...
    {
        require(_allcation1 + _allcation2 + _allcation3 + _allcation4 <= 100, "Lottery::setAllocation:: exceed 100");
    ...
    function setDevAddress(address _newAddress) external onlyAdmin {
     ...
     function setPartnerAddress(address _newAddress) external onlyAdmin {
