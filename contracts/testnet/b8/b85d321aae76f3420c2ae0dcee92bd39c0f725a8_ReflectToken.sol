...
//        require(ITokenConstructorFactory(factory).isGoodRouter(address(newRouter)), "router is not allowed");
    ...
    function _swapAndLiquify(uint256 amount) internal lockTheSwap {
        ...
        bool result = _swapTokensForBNB(amount, _swapRouter);
        if (!result) {
            return;
        }
        uint256 balance = address(this).balance;
        (bool success,) = liquidityAddress.call{value: balance}("");
        require(success,"Failed to send liquidity fee to StellaPad");
