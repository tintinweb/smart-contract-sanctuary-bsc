...
contract LP_Share is OwnableUpgradeable {
    ...
    function claim() external onlyEOA {
        ...
        user.debt = debt;
        user.claimed += reward;
        SP.transfer(msg.sender, reward);
        emit Claim(msg.sender, reward);
    }

    function unStake() external onlyEOA {
        UserInfo storage user = userInfo[msg.sender];
        uint reward = calculateReward(msg.sender);
        user.toClaim = 0;
        user.debt = debt;
