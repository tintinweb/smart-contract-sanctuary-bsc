...
// File: types/FunnyAccessControlledV2.sol
...
abstract contract FunnyAccessControlledV2 {
    /* ========== EVENTS ========== */

    event AuthorityUpdated(IFunnyAuthority authority);
    ...
    function initializeAuthority(IFunnyAuthority _newAuthority) internal {
        if (authority != IFunnyAuthority(address(0))) revert AUTHORITY_INITIALIZED();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }

    function setAuthority(IFunnyAuthority _newAuthority) external {
        _onlyGovernor();
        authority = _newAuthority;
        emit AuthorityUpdated(_newAuthority);
    }
...
// File: interfaces/ITreasuryExtender.sol
...
// File: interfaces/IAllocator.sol
 ...
 */
contract TreasuryExtender is FunnyAccessControlledV2, ITreasuryExtender {
    ...
    constructor(address treasuryAddress, address authorityAddress)
        FunnyAccessControlledV2(IFunnyAuthority(authorityAddress))
    {
