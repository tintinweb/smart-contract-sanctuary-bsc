...
contract EquipmentERC1155 is ERC1155, SubBase {

    using SafeMath for uint256;


    uint256 private totalEquipment = 0;
    uint8 private tokenType = 2;

    struct Equipment {
        uint256 id;
        string equipmentName;
        string equipmentDescribe;
        uint8 childType;  // 1 hat, 2 foot, 3 hand, 4 accessories
        uint32 aggressivity;
        uint32 defensive;
        uint32 healthPoint;
        uint8 aggressivityType; // Positive when 0, negative when 1.
        uint8 defensiveType;
    ...
    mapping(address => uint256[]) internal userEquipments;

    event equipmentCreated(uint8 tokenType, uint8 childType, uint256 indexed id, address indexed owner, uint256 boxId,  uint256 created);

    constructor() ERC1155("https://cryptosandwiches.com/api/metadata/equipments/") {}
    ...
    */
    function createEquipment(
    ...
    ) onlyBellyBox external {
        uint8 aggressivity = createRandom(0, 3);
        uint8 defensive = createRandom(0, 3);
        uint8 healthPoint = createRandom(0, 3);
        uint8 aggressivityType;
        uint8 defensiveType;
        uint8 healthPointType;
        if (createRandom(0, 10) % 2 == 0){ aggressivityType = 0; } else { aggressivityType = 1; }
        if (createRandom(0, 10) % 2 == 0){ defensiveType = 0; } else { defensiveType = 1; }
        if (createRandom(0, 10) % 2 == 0){ healthPointType = 0; } else { healthPointType = 1; }
        
        Equipment memory equipment = Equipment({
            id: totalEquipment,
            equipmentName: _name,
            equipmentDescribe: _describe,
            childType: childType, // 1 hat, 2 foot, 3 hand, 4 accessories
        ...
        userEquipments[recipient].push(totalEquipment);
        emit equipmentCreated(tokenType, childType, totalEquipment, recipient, boxId, equipment.created);
        nextEquipmentId();
    }
    ...
    function getEquipment(uint256 _id) public view returns (
        ...
        adh[3] = equipment.aggressivityType;
        adh[4] = equipment.defensiveType;
        adh[5] = equipment.healthPointType;
    ...
    function getUserEquipments(address user) public view returns (uint256[] memory) {
        return userEquipments[user];
    }

    function useEquipment(uint256 id) internal {
        equipments[id].used = true;
    }

    function name() external pure returns (string memory) {
        return "cryptoSandwichwiches equipment"; 
    }

    function symbol() external pure returns (string memory) {
        return "csw equipment";
    }
