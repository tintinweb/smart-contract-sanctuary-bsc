...
contract ERC721OnmeUpgradeable is
	Initializable,
	ERC721Upgradeable,
	...
	OwnableUpgradeable
{
	...
	event CreateERC721Onme(address owner, string name, string symbol);
	event CreateERC721OnmeUser(address owner, string name, string symbol);
	event OnmeCreator(uint256 tokenId, address creator);

	/// @custom:oz-upgrades-unsafe-allow constructor
	constructor() initializer {}

	function __ERC721Onme_init_unchained(
		string memory name,
	...
	function __ERC721Onme_init(
		string memory name,
		string memory symbol,
		string memory baseURI,
		address transferProxy
	) public initializer {
		__ERC721Onme_init_unchained(name, symbol, baseURI, transferProxy);

		emit CreateERC721Onme(_msgSender(), name, symbol);
	}

	function __ERC721OnmeUser_init(
		string memory name,
		string memory symbol,
		string memory baseURI,
		address transferProxy
		...
		emit CreateERC721OnmeUser(_msgSender(), name, symbol);
	}
	...
	function safeMint(
		...
		_setTokenRoyalty(tokenId, creator, royaltyNumerator);
		creators[tokenId] = creator;

		emit OnmeCreator(tokenId, creator);
	}
