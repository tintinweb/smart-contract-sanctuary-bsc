...
contract GovernanceStrategy is IGovernanceStrategy {
  address public immutable SeedXToken;

  /**
   * @dev Constructor, register tokens used for Voting and Proposition Powers.
   * @param seedXToken The address of the SeedX token contract.
   **/
  constructor(address seedXToken) {
    SeedXToken = seedXToken;
  ...
  function _getPowerByTypeAt(
    address user,
    uint256 blockNumber,
    IGovernancePowerDelegationERC20.DelegationType powerType ) internal view returns (uint256) {
    return
      IGovernancePowerDelegationERC20(SeedXToken).getPowerAtBlock(user, blockNumber, powerType); 

  }

  /**
   * @dev Returns the total supply of SeedX token at a specific block number.
   * @param blockNumber Blocknumber at which to fetch SeedX token supply.
   * @return Total SeedX token supply at block number.
   **/
  function _getTotalSupplyAt(uint256 blockNumber) internal view returns (uint256) {
    ISeedXToken seedXToken = ISeedXToken(SeedXToken);
    uint256 snapshotsCount = seedXToken._totalSupplySnapshotsCount();

    // Iterate in reverse over the total supply snapshots, up to index 1.
    for (uint256 i = snapshotsCount - 1; i != 0; i--) {
      GovernancePowerDelegationERC20Mixin.Snapshot memory snapshot = seedXToken._totalSupplySnapshots(i);
      if (snapshot.blockNumber <= blockNumber) {
        return snapshot.value;
      }
    }
    ...
    GovernancePowerDelegationERC20Mixin.Snapshot memory firstSnapshot = seedXToken._totalSupplySnapshots(0);
