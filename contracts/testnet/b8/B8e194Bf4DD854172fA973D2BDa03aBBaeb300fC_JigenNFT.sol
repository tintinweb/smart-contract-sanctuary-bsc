...
contract JigenNFT is ERC721Enumerable, Ownable {
    ...
    uint256 public nestingLockPeriod = 28 days;
    ...
    mapping(uint256 => uint256) nestingStarted;

    event TokenRedeemed(uint256 tokenId, address redeemer);
    event NestingToggled(uint256 tokenId);
    ...
    function endRedeemPeriod() public onlyOwner {
    ...
    function toggleNesting(uint256 tokenId) public onlyUnpaused {
        require(
            ownerOf(tokenId) == msg.sender,
            "Not owner of token"
        );
        require(!isRedeemed[tokenId], "Can't nest redeemed token");
        if (nestingStarted[tokenId] == 0) {
            nestingStarted[tokenId] = block.timestamp;
        } else {
            nestingStarted[tokenId] = 0;
        }
        emit NestingToggled(tokenId);
    }

    function redeem(uint256 tokenId) public payable onlyUnpaused {
        require(isRedeemPeriod, "Can't redeem right now");
        require(
            block.timestamp - nestingStarted[tokenId] >= nestingLockPeriod,
            "Nesting period is not over"
        );
        require(
            ownerOf(tokenId) == msg.sender,
    ...
    modifier isRedeemable(uint256 tokenId) {
        require(nestingStarted[tokenId] == 0, "Token currently nested");
        _;
    }
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override isRedeemable(tokenId) {
        ...
        )
    {
        ...
        total = current + nestingStarted[tokenId];
    }
