...
contract Fountain is BEP20, Whitelist {
    string public constant name = "BAKER Liquidity Token";
     ...
     * @param input_amount Amount of BNB or Tokens being sold.  bnb to be bought 100
     * @param input_reserve Amount of BNB or Tokens (input type) in exchange reserves. bnb available
     * @param output_reserve Amount of BNB or Tokens (output type) in exchange reserves. drip reserve
    ...
    function getInputPrice(
        ...
        uint256 input_amount_with_fee = input_amount * (990);
    ...
    function getOutputPrice(
        ...
        uint256 denominator = (output_reserve - (output_amount)) * (990);
    ...
    function bnbToTokenOutput(
        ...
        uint256 bnb_sold = getOutputPrice(
            tokens_bought,
            address(this).balance - (max_bnb),
            token_reserve
        );
        // Throws if bnb_sold > max_bnb
        uint256 bnb_refund = max_bnb - (bnb_sold);
        if (bnb_refund > 0) {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 bnb_amount = (amount * (address(this).balance)) /
            total_liquidity;
    ...
    function removeLiquidity(
        ...
        _totalSupply = total_liquidity - (amount);
        (bool success, ) = payable(msg.sender).call{value: bnb_amount}("");
...
contract BEP20 {
        ...
        returns (bool)
    {
        unchecked {
            _approve(
                spender,
                msg.sender,
                _allowed[spender][msg.sender] + addedValue
            );
        }
        return true;
    }
        ...
        returns (bool)
    {
        unchecked {
            _approve(
                spender,
                msg.sender,
                _allowed[spender][msg.sender] - subtractedValue
            );
        }
        return true;
    }
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        _approve(account, msg.sender, _allowed[account][msg.sender] - (amount));
    }
}
