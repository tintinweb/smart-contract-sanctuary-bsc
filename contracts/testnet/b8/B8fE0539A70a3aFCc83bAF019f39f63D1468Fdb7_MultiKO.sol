...
contract  MultiKO is ERC20, ReentrancyGuard {
    string public constant symbol = "MultiKO"; //MultiKO
    string public constant name = "MultiKO"; 
...
//   uint32 public PromoPerc;   
   ...
   uint256 public claimableBNBearnings;
    ...
    bool public _CalculatinMultiKOards; 
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "MultiKO"); //this contract token
       ...
       require(amount<=_maxTxAmount);
         if((recipient==_pancakePairAddress))                   //sell
           ...
           if(_AutoCalcRewards) //work moved to other functions due spread gas fees
            {
              if(_CalculatinMultiKOards == true)
...
return (slices);
    ...
    function ChooseSinglePayoutToken(uint32 PayoutTokenNumber) public ChecMultiKOrWork() {
       ...
       //token numbers + percentage slice
        function ChooseMultiplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
                                            uint32 Token3,uint32 Slice3,uint32 Token4,uint32 Slice4,
                                            uint32 Token5,uint32 Slice5,uint32 Token6,uint32 Slice6) public ChecMultiKOrWork() {
...
function _StartRewardCalculation() private {
   if((_CalculatinMultiKOards == false)&&(HolderPotBNB > 0))
   {
...
function CalculateNRewards(uint32 counts) public onlyAdmin() RefundGas(){
  ...
  require(_CalculatinMultiKOards == true);
  if(counts > 0)
...
function CalculateRewards(uint32 counts) private returns (bool){
   ...
   {   
    ...
    uint32 N_calculated;
        ...
        while((_CalculatinMultiKOards == true)&&(currenttry<maxtries))
         ...
         if(HolderPos >= N_holders) //all rewards for this cycle have been calculated
           ...
           _CalculatinMultiKOards = false;  //all rewards calculated
           LastRewardCalculationTime = block.timestamp;
...
function ClaimMyRewards() public ChecMultiKOrWork()
{ 
...
function setMyAutoPayout(bool Checked) public ChecMultiKOrWork() returns (bool)
{
