...
contract AutoCompounder is Ownable {
    ...
    function addRewardBaseTokens(address[] memory _tokens) public onlyOwner{
    ...
    function swapRewardsToLp() external returns (uint256 liquidity) {
        return swapRewardsToLpForUser(msg.sender);
    }

    function swapRewardsToLpForUser(address _user) public returns (uint256 liquidity) {
        require(
            msg.sender == _user ||
            msg.sender == multiFeeDistribution
        , "MFD || user only");
        claimAndSwapToBase(_user);
        liquidity = convertBaseToLPandStake(_user);
    }

    function claimAndSwapToBase(address _user) internal {
        ...
        for(uint256 i = 0; i < rewardBaseTokens.length; i++){
            ...
            if(underlying != baseToken){
                ...
                IUniswapV2Router02(uniRouter).swapExactTokensForTokens(
                    amount, 0, rewardToBaseRoute[underlying], address(this), block.timestamp + 600
                );
            }
        }
    }

    function convertBaseToLPandStake(address _user) internal returns (uint256 liquidity) {
        uint256 baseBal = IERC20(baseToken).balanceOf(address(this));
        if(baseBal != 0){
            IERC20(baseToken).safeApprove(lockZap, baseBal);
            liquidity = ILockZap(lockZap).zapWETH(baseBal, address(this), _user, false);
        }
    }
}
...
interface ILockZap {
    function zapWETHWithLockDuration(uint256 amount, address onBehalf, bool borrow, uint256 locktype) external returns (uint256 liquidity);
    function zapWETH(uint256 amount, address from, address onBehalf, bool borrow) external returns (uint256 liquidity);
}
