...
contract RunTogetherPoolProposal is Ownable, ReentrancyGuard {
     ...
     */
    function deposit(uint256 _amount, address _user, uint8 _v, bytes32 _r, bytes32 _s)
        external
        nonReentrant
        isRun
        isValidTime
        isValidBlock
    {
        require(_user == VerifyMessage(keccak256(abi.encodePacked(_amount, _user)), _v, _r, _s), "Invalid signature");
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function withdraw(uint256 _amount, address _user, uint8 _v, bytes32 _r, bytes32 _s) external nonReentrant isRun {
        require(_user == VerifyMessage(keccak256(abi.encodePacked(_amount, _user)), _v, _r, _s), "Invalid signature");
        require(blacklistMap[_user] == false, "You are in Blacklist");
        UserInfo storage user = userInfo[_user];
