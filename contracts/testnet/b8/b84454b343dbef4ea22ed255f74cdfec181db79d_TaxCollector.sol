...
contract TaxCollector is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyWhitelist() {
        require(msg.sender == tokenAddress || msg.sender == whitelistedUser , "GymFarming:: Only bank");
        _;
    }
...
// all initialize parameters are mandatory
    ...
    function swapTokensForBNB() public returns (bool){
   ...
   function _sendAmount() public  {
      uint256 amountDivided = address(this).balance / 2;
       IWETH(address(this)).transfer(sellTaxAddress,amountDivided);
       IWETH(address(this)).transfer(buyTaxAddress,amountDivided);
   }
}
