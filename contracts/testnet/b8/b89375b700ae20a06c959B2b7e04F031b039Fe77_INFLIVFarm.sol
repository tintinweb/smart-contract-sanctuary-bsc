...
interface IGovernance {
   function addGovernanceFee(uint256 amount) external view returns (bool);
}
...
contract INFLIVFarm is Initializable, OwnableUpgradeable{
    ...
    function initialize() public initializer {
		...
		Referrals = IReferrals(0xb0166fC1e65923Ac6450B38cc788030e2D6D06E0);
		USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
		rewardToken = address(0x3852681108e668042cEfa89DF9d61473fbbF457e);
    }
	...
	function activePackage(uint256 package, address sponsor) external {
		...
		IERC20Upgradeable(rewardToken).approve(address(rewardToken), governanceTax);
		IGovernance(rewardToken).addGovernanceFee(governanceTax);
		...
		referralUpdate(msg.sender, TokenRequired, governanceTax);
	...
	function referralUpdate(address sponsor, uint256 amount, uint256 amountUsed) private {
		address nextReferrer = Referrals.getSponsor(sponsor);
		
		uint256 i;
		uint256 level;
		uint usedFund = amountUsed;
		for(i=0; i < 256; i++) 
		{
			...
			{
			   uint256 governanceTax0 = amount - amountUsed;
			   if(governanceTax0 > 0)
			   {
			      IERC20Upgradeable(rewardToken).approve(address(rewardToken), governanceTax0);
	              IGovernance(rewardToken).addGovernanceFee(governanceTax0);
			   }
		       break;
			}
		    nextReferrer = Referrals.getSponsor(nextReferrer);
		}
		uint256 governanceTax1 = amount - amountUsed;
	    if(governanceTax1 > 0)
	    {
		   IERC20Upgradeable(rewardToken).approve(address(rewardToken), governanceTax1);
		   IGovernance(rewardToken).addGovernanceFee(governanceTax1);
	    }
    }
	...
	function deposit(uint256 amount, uint256 stakingID) external {
		...
		if(stakingID == 0)
		{
			...
			mapUserInfo[msg.sender][stakingID].rewardDebt = (mapUserInfo[msg.sender][stakingID].amount * accTokenPerShare);
			totalStaked += amount;
	...
	function updatePool(uint256 amount) external {
		IERC20Upgradeable(rewardToken).safeTransferFrom(address(msg.sender), address(this), amount);
		
		uint256 newReward = amount;
		accTokenPerShare += newReward / totalStaked;
	...
	function pendingReward(address user, uint256 stakingID) public view returns (uint256) {
		...
		{
            uint256 pending = (mapUserInfo[user][stakingID].amount * accTokenPerShare) - mapUserInfo[user][stakingID].rewardDebt;
			return pending;
        } 
		else 
		{
