...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    function declareWinner(uint256 _lotteryId) public onlyOwner {
        Lottery storage lottery = lotteries[_lotteryId];
        uint256 r = 8932208280731882869226127220105518934448807580381218038708455558120339712381; //random[_lotteryId].randomValue;
        ...
        for (uint256 k = 0; k < lottery.noOfWinner; k++) {
            uint256 winnerIndex = uint256(keccak256(abi.encode(r, k))) %
                totalParticipants;
            address winnerAddr = lottery.participants[winnerIndex];
            lottery.winners.push(winnerAddr);
            checkWinner[_lotteryId][lottery.participants[winnerIndex]] = true;
            for (uint256 j = 0; j < totalParticipants; j++) {
                if(winnerAddr == lottery.participants[(totalParticipants -1) - j] && j > 0){
                    totalParticipants--;
                    j--;
                    num++;
                } else if(winnerAddr == lottery.participants[j]) {                    
                    lottery.participants[j] = lottery.participants[(totalParticipants -1) - j];
                }
                
            }
            totalParticipants = totalParticipants - participatedUsersTickets[_lotteryId][winnerAddr] + num;
            num = 0;
        }
        lottery.isFinished = true;
    }
