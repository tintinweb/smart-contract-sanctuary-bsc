...
contract SwishFishGuilds is Ownable {
    ...
    function guildPayment(string memory guild_name_) internal {
        ...
        uint256 _creator_amount = _payment_amount.sub(_community_amount);
        uint256 _members = 0;
        bool sent;
        ...
        for (uint i=0; i < _guild.guild_member_address.length; i++) {
            if (guild_member_level_join[_guild.guild_member_address[i]] < _guild.level) {
                _payments[_members] = _guild.guild_member_address[i];
                _members++;
            }
        }
        if (_members > 0 ) {
            ...
            for (uint i=0; i < _payments.length; i++) {
                (sent,) = _payments[i].call{value: _member_amount}("");
