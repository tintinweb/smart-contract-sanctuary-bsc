...
contract VestingVault is Initializable, OwnableUpgradeable, UUPSUpgradeable  {
    ...
    event UpdateTimeInSeconds(uint256 _timeInSeconds);
        ...
        external
    {
        ...
        if(tokenGrants[_recipient].amount == 0){
            Grant memory grant = Grant({
                startTime: currentTime().add(_lockDurationInMonths.mul(timeInSeconds)),
                amount: _amount,
    ...
    function calculateGrantClaim(address _recipient) public view returns (uint256, uint256) {
        ...
        uint256 elapsedMonths = currentTime().sub(tokenGrant.startTime).div(timeInSeconds).add(1); 
        uint256 monthsVested = uint256(elapsedMonths.sub(tokenGrant.monthsClaimed)); // 2 = 4 - 2 
        // If over vesting duration, all tokens vested
        if (elapsedMonths > tokenGrant.vestingDuration) {
            uint256 remainingGrant = tokenGrant.amount.sub(tokenGrant.totalClaimed);
            return (monthsVested, remainingGrant);
        } else {
     ...
     * @param _timeInSeconds update the time in unix format
     */
    function updateTimeInSeconds(uint256 _timeInSeconds) external onlyOwner {
      timeInSeconds = _timeInSeconds;
      emit UpdateTimeInSeconds(timeInSeconds);
    }
