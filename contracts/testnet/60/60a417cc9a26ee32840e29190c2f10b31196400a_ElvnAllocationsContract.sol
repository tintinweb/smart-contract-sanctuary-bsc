...
contract ElvnAllocationsContract is Ownable {
    ...
    function getTierAllocations(uint _tierId) public view returns (Allocation[] memory){
        uint[] memory _projectId = this.tierProjects(_tierId);
        uint[] memory _allocationId = new uint[](_projectId.length);
        for(uint i=0;i < _projectId.length; i++){
            _allocationId[i] = allocationsOfTier[_tierId][_projectId[i]];
        }
    ...
    function getPlayerAllocations(uint _playerId) public view returns (Allocation[] memory){
        uint[] memory _projectId = this.playerProjects(_playerId);
        uint[] memory _allocationId = new uint[](_projectId.length);
        for(uint i=0;i < _projectId.length; i++){
            _allocationId[i] = allocationsOfPlayer[_playerId][_projectId[i]];
        }
        Allocation[] memory _allocations = new Allocation[](_allocationId.length);
        for(uint i=0;i < _allocationId.length; i++){
            _allocations[i] = allocationDatabase[_allocationId[i]];
