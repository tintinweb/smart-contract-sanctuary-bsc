...
contract NestNFTAuction is NestFrequentlyUsed {
    ...
    function startAuction(address nftAddress, uint tokenId, uint48 price, uint cycle) external {
        require(tokenId < 0x1000000000000000000000000, "NFTAuction:must nest nft");
        require(cycle > 1 hours && cycle < 0x100000000, "NFTAuction:cycle not valid");
    ...
    function bid(uint index, uint48 price) external {
        ...
        // TODO: 重入问题
        TransferHelper.safeTransferFrom(NEST_TOKEN_ADDRESS, msg.sender, address(this), uint(price) * 0.0001 ether);
        if (bidder != address(0)) {
            TransferHelper.safeTransfer(NEST_TOKEN_ADDRESS, auction.bidder, (uint(lastPrice) + reward) * 0.0001 ether);
        }
    }
    ...
    function endAuction(uint index) external {
        ...
        console.log("ts: %d", uint(auction.price - auction.reward));
        TransferHelper.safeTransfer(NEST_TOKEN_ADDRESS, auction.owner, uint(auction.price - auction.reward) * 0.0001 ether);
