...
contract NewToken is Context, IBEP20, Ownable {
    ...
    address public BUSD = address(0x5f54994F2F82A674059EF4a80f325Aa11324deb8);
    ...
    function setFeeCountersOnSelling(uint256 amount) public {
    ...
    // IBEP20(_tokenIn).approve(address(dexRouter), _amountIn);

    // //path is an array of addresses.
    // //this path array will have 3 addresses [tokenIn, WETH, tokenOut]
    // //the if statement below takes into account if token in or token out is WETH.  then the path is only 2 addresses
    // address[] memory path;
    // if (_tokenIn == dexRouter.WETH() || _tokenOut == dexRouter.WETH()) {
    //   path = new address[](2);
    //   path[0] = _tokenIn;
    //   path[1] = _tokenOut;
    // } else {
    ...
    //     IDexRouter(dexRouter).swapExactTokensForETHSupportingFeeOnTransferTokens(_amountIn, _amountOutMin, path, _to, block.timestamp);
    // }
    ...
    function distributeAndLiquify(address from, address to) private {
        ...
        if (
            ...
            dexRouter.swapExactETHForTokens{value: bnbFormarket}(
                0,
                path,
                marketWallet,
                block.timestamp + 300
            ...
            dexRouter.swapExactETHForTokens{value: bnbForDev}(
                0,
                path,
                devWallet,
                block.timestamp + 300
