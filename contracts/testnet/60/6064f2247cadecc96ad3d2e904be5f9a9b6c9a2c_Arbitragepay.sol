...
contract Arbitragepay {
    ...
    uint256 public constant INVEST_MIN_AMOUNT = 0.2 ether;
    ...
    function invest(address payable referrer) public payable beforeStarted() {
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < DepositSlab.length; i++) {
                if (upline != address(0)) {
                    ...
                    // if(users[upline].structure[0] >=2 &&  !users[upline].is_networker)
                    // {
        ...
        if (user.deposits.length == 0) {
            user.checkpoint = block.timestamp.add(5 minutes);
        ...
        user.deposits.push(Deposit(msg.value, block.timestamp.add(5 minutes)));
    ...
    function getTimer(address userAddress) public view returns (uint256) {
        return users[userAddress].checkpoint.add(7 minutes);
    }
