...
contract Catpay is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        if(!takeFee && isActiveFees){
            ...
            uint256 lfee = _takeLiquidity(receivedAmount);
            uint256 mfee = _takeMarketing(receivedAmount);
            uint256 bfee = _takeBurn(receivedAmount);
            receivedAmount = receivedAmount - (lfee + mfee + bfee);
