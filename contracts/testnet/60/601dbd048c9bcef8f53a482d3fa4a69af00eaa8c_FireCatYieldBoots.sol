...
contract FireCatYieldBoots is IFireCatYieldBoots, ModifyControl, FireCatTransfer {
    ...
    */
    ...
    function isStakeable(address user_, uint256 amount_) public view returns (bool) {
        ...
        return amount_ > 1e18 && cycle.staked.add(amount_) <= maxStakeTotalAmount && amount_ <= maxStakeAmount;
    }
    ...
    function stake(uint256 amount_) external beforeStake isBanned(msg.sender) nonReentrant returns (uint256) {
        ...
        uint256 actualAddAmount = burn(stakeToken, msg.sender, amount_);
        uint256 totalStakedNew = _totalStaked.add(actualAddAmount);

        uint256 addYield = yieldOf(actualAddAmount);
        _updateUserCycle(msg.sender, actualAddAmount, addYield);
