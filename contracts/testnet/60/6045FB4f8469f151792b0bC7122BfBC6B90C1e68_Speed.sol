...
contract Speed {
    ...
    function closeTable() internal {
        ...
        if (
            block.timestamp >
            adrToInfo[IdToTable[adrToInfo[msg.sender].tableId].leader].timest +
                deadline
        ) {
            lost =
                (levlToPayment * 3 * (100 - (adminFee + refFee))) /
                100 +
                adrToLostMoney[IdToTable[adrToInfo[msg.sender].tableId].leader];
        }
    ...
    function tableInfo(uint256 tableId) external view returns (Table memory) {
