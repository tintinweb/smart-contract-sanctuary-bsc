...
contract FlapCoinSwap is Ownable {
    using SafeMath for uint256;

    IBEP20 public tokenA;
    IBEP20 public tokenB;
    uint256 public rate = 177000000000000000; 
    
    constructor(address _tokenA, address _tokenB
    ) {
        tokenA = IBEP20(_tokenA);  // BUSD
        tokenB = IBEP20(_tokenB);  // FLAP
    }

    function swap(uint256 _amount) public {
        uint256 _payback = _amount.mul(rate/1e18);
    
        require(tokenB.balanceOf(address(this)) > _payback, "insufficient contract balance");
