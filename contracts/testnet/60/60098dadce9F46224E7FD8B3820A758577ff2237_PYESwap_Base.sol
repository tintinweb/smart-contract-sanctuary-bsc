...
contract Liquify is ILiquify {
    ...
    function buyAndLiquify() external onlyToken swapping {
        ...
        uint256 initialTokenBalance = IERC20(_token).balanceOf(address(this));

        buyTokens(amountToSwap);

        uint256 amountTokenToAdd = IERC20(_token).balanceOf(address(this)).sub(initialTokenBalance);

        addLiquidity(amountTokenToAdd, amountWBNBtoAdd);
    ...
    function addLiquidity(uint256 amountTokenToAdd, uint256 amountWBNBtoAdd) private {
...
// File: contracts/PYESwap_Base.sol
...
contract PYESwap_Base is IPYE, Context, Ownable {
    ...
    uint256 public swapThreshold = 1 * 10**15; // 0.001 WBNB
    bool inSwap;
    ...
    bool public buyAndLiquifyEnabled = true;
    event BuyAndLiquifyEnabledUpdated(bool enabled);
    event BuyAndLiquify(
        uint256 wbnbSwapped,
        uint256 tokensReceived,
        uint256 wbnbIntoLiqudity
    ...
    constructor(address _marketing, uint256 _marketingFee, uint256 _liquidityFee) {
    ...
    function setLiquidityPercent(uint256 _liquidityFee) external onlyOwner {
        _defaultFees.liquidityFee = _liquidityFee;
        _updatePairsFee();
    }

    function setLiquifyThreshold(uint256 _liquifyThreshold) external onlyOwner {
        swapThreshold = _liquifyThreshold;
    }
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.marketing).sub(values.liquidity);
        return values;
    }
    ...
    function shouldBuyAndLiquify() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && IERC20(WBNB).balanceOf(liquifyAddress) >= swapThreshold;
    //    return swapEnabled && IERC20(WBNB).balanceOf(address(this)) >= swapThreshold;
    }
    ...
    function _takeFees(FeeValues memory values) private {
        ...
        _takeFee(values.liquidity, liquifyAddress);
    }
    ...
    function depositLPFee(uint256 amount, address token) public onlyExchange {
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                IERC20(token).transfer(liquifyAddress, liquidityFeeAmount);
            }
        }
    }
