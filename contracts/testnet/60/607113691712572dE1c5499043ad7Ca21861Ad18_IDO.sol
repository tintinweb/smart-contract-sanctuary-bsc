...
contract IDO is Auth, EIP712Upgradeable {
  ...
  event FreeBought(uint indexed eventId, uint amount, uint price, uint timestamp);
  event WhitelistBought(uint indexed eventId, uint amount, uint price, uint timestamp);
  ...
  function freeBuy(uint _eventId, uint _amount, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    emit FreeBought(_eventId, _amount, eventDetail.freeUser.price, block.timestamp);
  }
  ...
  function whitelistBuy(bytes calldata _signature, uint _eventId, uint _totalToken, uint _price, USDCurrency _usdCurrency) external eventExist(_eventId) {
    ...
    emit WhitelistBought(_eventId, _totalToken, _price, block.timestamp);
  }
