...
contract pass is ERC721Full, Ownable {
    using SafeMath for uint256;


    string public tokenURIPrefix = "https://elfinkingdom.com/wapi/elfin/";
    mapping(uint256 => address) public passRecord;

    event minted(address indexed toAddr , uint256 tokenId);
    event pass(address indexed passAddress, uint256 indexed tokenId);
    event transfered(address indexed fromAddr,address indexed toAddr, uint256 tokenId);
    address public blackHole = address(0x0000000000000000000000000000000000000001);
    ...
    function batchMint(address[] memory _to, uint256[] memory _tokenId) public onlyMinter {
        for (uint i = 0; i < _tokenId.length; i++) {
            _mint(_to[i], _tokenId[i]);
        }
    }

    function doPass( uint256 tokenId) public {
        safeTransferFrom(msg.sender,blackHole,tokenId);
        passRecord[tokenId] = msg.sender;
        emit pass(msg.sender,tokenId);
    }
