...
contract MockIgniteBondDepository is Ownable {
  ...
  address public immutable TINDER; // token given as payment for bond
  address public immutable principle; // token used to create bond
  address public immutable treasury; // mints TINDER when receives principle
  ...
  struct Bond {
    uint256 payout; // TINDER remaining to be paid
    uint256 vesting; // Blocks left to vest
    uint256 lastBlock; // Last interaction
    uint256 pricePaid; // In KDAI, for front end viewing
  }
  ...
  function deposit(
    ...
    require(payout >= 10000000, "Bond too small"); // must be > 0.01 TINDER ( underflow protection )
    ...
    /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) TINDER
         */
    IERC20(principle).safeTransferFrom(msg.sender, address(this), _amount);
    IERC20(principle).approve(address(treasury), _amount);

    ITreasury(treasury).deposit(_amount, principle, profit, true);

    if (fee != 0) {
      // fee is transferred to dao
      IERC20(TINDER).safeTransfer(DAO, fee);
    }
  ...
  function redeem(address _recipient) external returns (uint256) {
    ...
    if (percentVested >= 10000) {
      ...
      IERC20(TINDER).safeTransfer(_recipient, info.payout); // send payout
      return info.payout;
    } else {
      // if unfinished
      ...
      IERC20(TINDER).safeTransfer(_recipient, payout); // send payout
      return payout;
    }
  }
  ...
  function maxPayout() public view returns (uint256) {
    return IERC20(TINDER).totalSupply().mul(terms.maxPayout).div(100000);
  }
  ...
  /**
   *  @notice converts bond price to KDAI value
   *  @return price_ uint
   */
  ...
  /**
   *  @notice calculate current ratio of debt to TINDER supply
   *  @return debtRatio_ uint
   */
  function debtRatio() public view returns (uint256 debtRatio_) {
    uint256 supply = IERC20(TINDER).totalSupply();
  ...
  /**
   *  @notice calculate amount of TINDER available for claim by depositor
   *  @param _depositor address
  ...
  /**
   *  @notice allow anyone to send lost tokens (excluding principle or TINDER) to the DAO
   *  @return bool
   */
  function recoverLostToken(address _token) external returns (bool) {
    require(_token != TINDER);
