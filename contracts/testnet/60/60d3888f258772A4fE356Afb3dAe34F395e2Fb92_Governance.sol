...
contract Loan {
    ...
    function updateDurationPaymentPeriodAPRInerestRate(uint64 _duration, uint64 _paymentPeriod, uint8 _APRinterestRate) public onlyOwner onlyPending{
        require(_APRinterestRate >= 3 && _APRinterestRate <= 30, "APR interest Rate should be in 3% ~ 30%");
        duration = _duration;
        paymentPeriod = _paymentPeriod;
        aPRInerestRate = _APRinterestRate;
    ...
    function payBack(uint256 acceptId, uint256 _payBackAmount) public onlyBorrower {
        ...
        (uint256 mPaybackAmount, uint256 feeAmount) = getPaybackAmount(acceptId, msg.sender);
        ...
        acceptLoanMap[msg.sender][acceptId].paidAmount += mPaybackAmount - feeAmount;
        acceptLoanMap[msg.sender][acceptId].paidCount += 1;

        emit PaidBack(msg.sender, _payBackAmount);
        
    }

    function payOff(uint256 acceptId, uint256 _payBackAmount) public onlyBorrower {

        (uint256 mPayOff, uint256 feeAmount) = getPayOffAmount(acceptId, msg.sender);

        require(_payBackAmount >= mPayOff, "not enough payback amount!");

        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance >= mPayOff, "Caller did not approve tokens for loan payback!");

        IERC20(token).transferFrom(msg.sender, owner, mPayOff - feeAmount);
        IERC20(token).transferFrom(msg.sender, teamWallet, feeAmount);

        acceptLoanMap[msg.sender][acceptId].paidAmount += mPayOff - feeAmount;
        acceptLoanMap[msg.sender][acceptId].paidCount += 1;
    ...
    function getOwedAmount(address _borrower) public view returns (uint256 result){

        uint256 index = 0;
        while(index < acceptLoanMap[_borrower].length){
            result += (acceptLoanMap[_borrower][index].tokenAmount - acceptLoanMap[_borrower][index].paidAmount);
            index ++;
        }
        return result;
    }

    function getPaybackAmount(uint256 acceptId, address _account) public view returns (uint256, uint256) {

        Accept memory acceptLoan = acceptLoanMap[_account][acceptId];
        uint64 np = duration/paymentPeriod; // number of payment
        uint256 totalIrt = acceptLoan.tokenAmount * aPRInerestRate / 100 * duration / 365;
        uint256 rftxPaymentFee = ((acceptLoan.tokenAmount + totalIrt) * payBackFee / 100) / np;
        return (rftxPaymentFee + (acceptLoan.tokenAmount + totalIrt) / np, rftxPaymentFee);
    }

    function getPayOffAmount(uint256 acceptId, address _account) public view returns (uint256, uint256) {

        Accept memory acceptLoan = acceptLoanMap[_account][acceptId];
        uint256 payOffAmnt = 0;
        if(acceptLoan.tokenAmount > acceptLoan.paidAmount){
            payOffAmnt = acceptLoan.tokenAmount - acceptLoan.paidAmount;
        }
        uint64 np = duration/paymentPeriod; // number of payment
        uint256 totalIrt = acceptLoan.tokenAmount * aPRInerestRate / 100 * duration / 365;
        uint256 rftxPaymentFee = ((acceptLoan.tokenAmount + totalIrt) * payBackFee / 100) / np;

        return (rftxPaymentFee + payOffAmnt, rftxPaymentFee);
    }
...
interface ILoan {
    ...
    function acceptId() external view returns (uint256);
