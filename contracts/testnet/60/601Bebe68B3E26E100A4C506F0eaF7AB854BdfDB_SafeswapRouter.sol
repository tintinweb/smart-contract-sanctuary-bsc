...
// File: safemoon/Safeswap_Periphary_V2/libraries/Initializable.sol
...
// File: safemoon/Safeswap_Periphary_V2/libraries/lib.sol
...
library SafeswapLibrary {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"a46a62841c4f2ee1992d40caa83038881c3aae58e78721f13105cfa0fe4a87c6" // init code hash
                        )
                    )
                )
            )
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = ISafeswapPair(
            pairFor(factory, tokenA, tokenB)
        ).getReserves();
    ...
    function quote(
        ...
        require(
            ...
            "SafeswapLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
// File: safemoon/Safeswap_Periphary_V2/SafeswapRouter.sol
...
contract SafeswapRouter is ISafeswapRouter02, Initializable {
    ...
    function getTokenDeduction(address token, uint256 amount)
        external
        view
        returns (uint256, address)
    {
        if (nameToInfo[token].enabled == false || killSwitch == true)
            return (0, address(0));
    ...
    function registerToken(
        ...
        if (!isUpdate) {
            require(
                nameToInfo[tokenAddress].tokenAddress == address(0),
    ...
    function switchSTPToken(address _tokenAddress) public onlyOwner {
        require(
            nameToInfo[_tokenAddress].isDeleted == false,
            "token already deleted"
        );
    ...
    function _addLiquidity(
        ...
        (uint256 reserveA, uint256 reserveB) = SafeswapLibrary.getReserves(
            factory,
            tokenA,
            tokenB
        );
        if (reserveA == 0 && reserveB == 0) {
            ...
            uint256 amountBOptimal = SafeswapLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "SafeswapRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = SafeswapLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "SafeswapRouter: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? SafeswapLibrary.pairFor(factory, output, path[i + 2])
                : _to;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (
            ...
            killSwitch == false &&
            (nameToInfo[path[0]].sellFeePercent > 0)
        ) {
            uint256 deduction = (amountIn *
                nameToInfo[path[0]].sellFeePercent) / ONE;
            amountIn = amountIn - deduction;
            TransferHelper.safeTransferFrom(
                path[0],
                to,
                nameToInfo[path[0]].feesAddress,
                deduction
            );
        }
        amounts = SafeswapLibrary.getAmountsOut(factory, amountIn, path);
        ...
        if (
            ...
            killSwitch == false &&
            (nameToInfo[path[1]].buyFeePercent > 0)
        ) {
            uint256 amountOut = amounts[amounts.length - 1];
            uint256 deduction = (amountOut *
                nameToInfo[path[1]].buyFeePercent) / ONE;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            ...
            "SafeswapRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        if (
            nameToInfo[path[1]].enabled == true &&
            killSwitch == false &&
    ...
    {
        require(path[path.length - 1] == WETH, "SafeswapRouter: INVALID_PATH");
        uint256[] memory oldamounts = SafeswapLibrary.getAmountsOut(
            factory,
            amountIn,
            path
        ); // ,amountIn,
        require(
            oldamounts[oldamounts.length - 1] >= amountOutMin,
            "SafeswapRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (msg.value > amounts[0])
            TransferHelper.safeTransferETH(to, msg.value - amounts[0]);
    }

    function _swapSupportingFeeOnTransferTokens(
        address[] memory path,
        address _to
    ) internal virtual returns (uint256 amount0Out, uint256 amount1Out) {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = SafeswapLibrary.sortTokens(input, output);
            ISafeswapPair pair = ISafeswapPair(
                SafeswapLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountInput =
                    ...
                    reserveInput;
                amountOutput = SafeswapLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
            ...
            if (
                nameToInfo[output].enabled == true &&
                killSwitch == false &&
                (nameToInfo[output].buyFeePercent > 0)
            ) {
                uint256 deduction = (amountOutput *
                    nameToInfo[output].buyFeePercent) / ONE;
