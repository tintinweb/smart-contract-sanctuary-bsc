...
contract KongCoin is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address public developerAddress = 0xf3DA821a0ED650FF3b94fc9E5470a26f9c65d84B;
    address public stakepoolAddress = 0xf3DA821a0ED650FF3b94fc9E5470a26f9c65d84B;
    ...
    string private _name = "Kong Coin";
    string private _symbol = "KOCO";
    ...
    uint256 public _stakepoolFee = 2;
    uint256 private _previousStakepoolFee = _stakepoolFee;

    uint256 _saleLiquidityFee = 4;
    uint256 _saleDeveloperFee = 2;
    uint256 _saleBuybackFee = 2;
    uint256 _saleStakepoolFee = 8;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 _totalFee = _liquidityFee.add(_developerFee).add(_buybackFee).add(_stakepoolFee);

        uint256 liqAndBuybackTokens = contractTokenBalance.mul(_liquidityFee+_buybackFee).div(_totalFee);

        uint256 walletsTokens = contractTokenBalance.sub(liqAndBuybackTokens);
        swapAndSendToFee(walletsTokens);
        
        uint256 hlafLiquidityTokens = liqAndBuybackTokens.mul(_liquidityFee).div(_liquidityFee+_buybackFee).div(2);
        uint256 swapableTokens = liqAndBuybackTokens.sub(hlafLiquidityTokens);

        uint256 initialBalance = address(this).balance;
        swapTokensForEth(swapableTokens); 

        uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 bnbForLiquidity = newBalance.mul(_liquidityFee).div(_liquidityFee+_buybackFee).div(2);
        addLiquidity(hlafLiquidityTokens, bnbForLiquidity);
        emit SwapAndLiquify(swapableTokens, bnbForLiquidity, hlafLiquidityTokens);
        //the remaining bnb will stay here for buyback.
    }
    ...
    {
        uint256 _totalFee = _developerFee.add(_stakepoolFee);

        swapTokensForBUSD(tokens);
        uint256 newBalance = (IBEP20(BUSD).balanceOf(address(this)));

        uint256 developerShare = newBalance.mul(_developerFee).div(_totalFee);
        uint256 stakepoolShare = newBalance.sub(developerShare);

        IBEP20(BUSD).transfer(developerAddress, developerShare);
        IBEP20(BUSD).transfer(stakepoolAddress, stakepoolShare);
    ...
    {
        uint256 _totalFee = _liquidityFee.add(_developerFee).add(_buybackFee).add(_stakepoolFee);
        return _amount.mul(_totalFee).div(100);
    }
    ...
    function restoreAllFee() private 
    {
        ...
        _stakepoolFee = _previousStakepoolFee;
    }
    ...
    function setSaleFee() private 
    {
        ...
        _stakepoolFee = _saleStakepoolFee;
    }
    

    event updateBuyFee(uint256 totalFee, uint256 timestamp);
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 developerFee, uint256 buybackFee, uint256 stakepoolFee) 
    external onlyOwner()
    {
        ...
        _stakepoolFee = stakepoolFee;
        _previousStakepoolFee = stakepoolFee;

        uint256 totalFee = liquidityFee.add(developerFee).add(buybackFee).add(stakepoolFee);
    ...
    function setAllSaleFeePercentages(uint256 liquidityFee, uint256 developerFee, uint256 buybackFee, uint256 stakepoolFee) 
    external onlyOwner()
    {
        _saleLiquidityFee = liquidityFee;
        _saleDeveloperFee = developerFee;
        _saleBuybackFee = buybackFee;
        _saleStakepoolFee = stakepoolFee;
    ...
    {
        developerAddress = _stakepoolAddress;
    }
