...
contract VIP is Ownable {
    ...
    event Buy(address toAddress,uint256 price,address tokenAddress,uint256 timestamp,string data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v );

    constructor(address _funAddress){
        funAddress = _funAddress;
    }

    function buy(address toAddress,uint256 price,address tokenAddress,uint256 timestamp,string memory data,bytes32 msghash,bytes32 r,bytes32 s,uint8 v) public payable {
        ...
        string memory signDataStr = string(abi.encodePacked(string(abi.encodePacked("0x",addressToStr(toAddress))),
        uint2str(price),
        ...
        if(tokenAddress == address(0)){
            ...
            IERC20(tokenAddress).transferFrom(msg.sender,toAddress,price);
        }
        
        hashList[msghash] = true;
        emit Buy(toAddress, price, tokenAddress, timestamp, data,msghash, r, s, v);
    }
    ...
    function withdrawToken(address tokenAddress,address to) public onlyController {
