...
contract MarketPlace  {
    ...
    CyverseAdmin public _cyverseAdmin;
    ...
    modifier rGuardPod() {
        require(!_rpodGuard, "LOCKED");
        _rpodGuard = true;
        _;
        _rpodGuard = false;
    }

    modifier rGuardDiver() {
        require(!_rdiverGuard, "LOCKED");
        _rdiverGuard = true;
        _;
        _rdiverGuard = false;
    }
    ...
    function addDiverToMarket(uint diverId_, uint salePrice_) external rGuardDiver {
    ...
    function removeDiverFromMarket(uint diverId_) external rGuardDiver {
    ...
    function getMarketDiversByPage(uint cursor, uint howMany) public view returns (
        ShareDef.MarketDiver[] memory, uint newCursor) {
        if (howMany > marketDiverList.length - cursor) {
            howMany = marketDiverList.length - cursor;
        }

        ShareDef.MarketDiver[] memory values = new ShareDef.MarketDiver[] (howMany);
        for (uint i = 0; i < howMany; i++) {
            values[i] = marketDiverList[cursor + i];
        }

        return (values, cursor + howMany);
    }
    ...
    function buyMarketDiver(uint diverId_) external rGuardDiver {
    ...
    function TransferToAddress(uint mtype, uint id, address wallet) external {
        ...
        require(
            ICredz(_cyverseAdmin.coinAddress()).ebalance(msg.sender) >= credzToCharge, 
            "NSF"
        ); //insuficient funds
        ICredz(_cyverseAdmin.coinAddress()).inGamePurchase(msg.sender, credzToCharge, transferDist);
        if (mtype == 1) { //DIVERS
    ...
    function addPodToMarket(uint podId_, uint salePrice_) external rGuardPod {
        Player player = Player(
            CyverseFactory(
                _cyverseAdmin.factoryAddress()
            ).getPlayerAddressWithAddress(msg.sender)
    ...
    function getMarketPodsByPage(uint cursor, uint howMany) public view returns (
        ShareDef.MarketPod[] memory, uint newCursor) {
        if (howMany > marketPodList.length - cursor) {
            howMany = marketPodList.length - cursor;
        }

        ShareDef.MarketPod[] memory values = new ShareDef.MarketPod[] (howMany);
        for (uint i = 0; i < howMany; i++) {
            values[i] = marketPodList[cursor + i];
        }
    ...
    function removePodFromMarket(uint podId_) external rGuardPod {
    ...
    function buyMarketPod(uint podId_, address sellerWallet_, uint rarity) external rGuardPod{
        require(podId_ < marketPodList.length, "INVALID");
        require(marketPodList[podId_].status == ShareDef.STATUS.ACTIVE, "INVALID");
        require(marketPodList[podId_].walletAddress == sellerWallet_, "NS");
        require(marketPodList[podId_].pod.rarity == uint8(rarity), "NS2");
