...
contract Splat is ERC721A, Ownable {
    ...
    mapping(uint256 => string) public transformations;

    // Contract address -> token id -> bool
    mapping(address => mapping(uint256 => bool)) public addressTokenSplatted;
    ...
    event Setter(
        uint256 indexed splatId,
        address indexed usingContractNFT,
        uint256 indexed tokenId
    );
    ...
    function hasTokenBeenSplatted(address _contractAddress, uint256 _tokenId)
        public
        view
        returns (bool)
    {
    ...
    function splat(
        ...
        if (hasTokenBeenSplatted(usingContractNFT, usingTokenId))
            revert AlreadySplatted();
    ...
    function promoMint(address _to, uint256 _count) external onlyOwner {
