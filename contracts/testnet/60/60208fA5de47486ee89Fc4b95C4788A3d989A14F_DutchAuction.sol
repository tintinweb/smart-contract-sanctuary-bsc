    ...
    IERC721ReceiverUpgradeable
{
    ...
    modifier isDiffPriceInBounds(uint256 startPrice, uint256 minPrice) {
        require(
            startPrice >= 400000 * minPrice,
            "Error: incorrect percentage"
        );
        _;
    }
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        ...
        if (_getAuctionState(nftContractAddress, tokenId) == AuctionState.OPEN){
            if ((block.timestamp - nftContractAuction.auctionStart) > auctionPeriod.decreasePeriod){
                ...
                if ((nftContractAuction.startPrice - amountToDecrease) > (nftContractAuction.startPrice - pricetoMin)){
    ...
    function _checkAuctionParameters(
        ...
        isDiffPriceInBounds(startPrice, minPrice)
    {
...
abstract contract PriceDecrease is Initializable, OwnableUpgradeable {
    ...
    public pure returns(uint256){
        uint256 decreasesteps = (deadlineTime - auctionStart)/decreasePeriod;//172800/600 = 288
        uint256 stepsToDecrease = (startPrice - minPrice) / (decreasesteps - 1);//10000/288 = 34,72
