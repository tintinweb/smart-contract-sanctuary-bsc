...
contract Zap is Ownable, Pausable, Initializable {
    ...
    event ZapOutBNB(address lpToken, address token, uint256 lpTokenAmount, uint256 tokenAmount , uint256 bnbAmount);
    event ZapOutBEP20(address lpToken, address token0, address token1, uint256 lpTokenAmount, uint256 token0Amount , uint256 token1Amount);
    ...
    */
    ...
    )
    {
        ...
        address[] memory path =  routePath(other, _toToken);
        uint256[] memory amounts = SwapLibrary.getAmountsOut(factory, sellAmount, path);
    ...
    function getPriceImpactWhenZapOut(address fromToken, uint256 amountIn) public view returns (uint256 priceImpact) {
        if (!isLpToken(fromToken)) {
            priceImpact = calculatePriceImpact(fromToken, amountIn, WBNB);
        }
    }
    ...
    function zapOut(address fromToken, uint256 amountIn, uint256 amountOutMinA, uint256 amountOutMinB) external whenNotPaused {
        IERC20(fromToken).safeTransferFrom(msg.sender, address(this), amountIn);
        _approveTokenIfNeeded(fromToken);

        if (!isLpToken(fromToken)) {
            uint256 amountOut = _swapTokenForBNB(fromToken, amountIn, msg.sender);

            require(amountOut >= amountOutMinA, 'Zap: INSUFFICIENT_OUTPUT_AMOUNT');
            ...
            if (token0 == WBNB || token1 == WBNB) {
                ...
                (uint256 amountToken, uint256 amountETH) = ROUTER.removeLiquidityETH(token, amountIn, amountOutMinA, amountOutMinB, msg.sender, block.timestamp);
                emit ZapOutBNB(fromToken, token, amountIn, amountToken, amountETH);
            } else {
                (uint256 amountA, uint256 amountB) =  ROUTER.removeLiquidity(token0, token1, amountIn, amountOutMinA, amountOutMinB, msg.sender, block.timestamp);
                emit ZapOutBEP20(fromToken, token0, token1, amountIn, amountA , amountB);
            }
        }
    }
