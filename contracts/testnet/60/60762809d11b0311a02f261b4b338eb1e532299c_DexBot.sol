...
contract DexBot is Initializable {
    ...
    function stake(uint256 amount, uint256 plan) public {
        ...
        test = userstake.amount;
        test1 = lockingPeriod[plan];
        test2 = plan;
        test3 = block.timestamp;
        test4 = userstake.date;
        test5 = block.timestamp - userstake.date;
        test6 = (userstake.amount *
            ((lockingPeriod[plan] * 1e8) / (2592000 * plan)));
        test7 = ((lockingPeriod[plan] * 1e8) / (2592000 * plan));
