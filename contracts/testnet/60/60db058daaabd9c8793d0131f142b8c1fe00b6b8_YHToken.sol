...
contract YHToken is Ownable, ERC20 {
    ...
    constructor(
        ...
        _mint(owner(), 100000000 * oneEther);
        init();
    }
    ...
    function _transferWithBuyFee(
        ...
        uint256[5] memory feeAmounts = [
            (feeAmount * 300) / _totalBuyFee,
            (feeAmount * 200) / _totalBuyFee,
            (feeAmount * 150) / _totalBuyFee,
            (feeAmount * 100) / _totalBuyFee,
            (feeAmount * 50) / _totalBuyFee
        ];

        address dest = to;
        for(uint8 i = 0; i < 5; i ++) {
            address recommender = referrals[dest];
            if(recommender == address(0)) {
                _transfer(from, 0x7DEfe04FEEd55f771226627f1c7550Ca89742D91, feeAmounts[i]);
            } else {
                _transfer(from, recommender, feeAmounts[i]);
            }
            dest = recommender;
        }
    ...
    function _transferFrom(
        ...
        _beforeTokenTransfer(from, to, amount);
        if (isContract(to) == false && referrals[to] == address(0) && isContract(from) == false)
            referrals[to] = from;
