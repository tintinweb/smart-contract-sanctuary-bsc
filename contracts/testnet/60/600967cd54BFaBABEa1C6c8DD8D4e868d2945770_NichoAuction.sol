...
contract NichoAuction is Ownable{
    ...
    mapping(address => mapping(uint256 => uint256)) tokenIdToAuctionId;
    ...
    event AddItem(uint256 auctionId, address tokenAddress, uint256 tokenId, string uri, uint256 price);
    event ItemCancel(uint256 auctionId);
    event PlacedBid(uint256 auctionId, address bider, uint256 bidPrice);
    event SoldOut(uint256 auctionId, uint256 soldPrice, address winner);
    event CancelBid(uint256 auctionId, address auctionOwner);

    constructor() {
        WETH = address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd); // Devnet
        // WETH = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // Mainnet
    ...
    function addAuctionItem(
        ...
        external {
            ...
            uint256 auctionId = auctionCounter;
            {
                ...
                item.uri = uri;
                item.highPrice = itemParam.desiredMinPrice;
                item.deadline = itemParam.deadline;
            ...
            auctionCounter++;
            emit AddItem(auctionId, itemParam.tokenAddress, itemParam.tokenId, uri, itemParam.desiredMinPrice);
    }
    ...
    function acceptBid(uint256 auctionId, uint256 bidderId)
        ...
        external {
            ...
            if (IOERC721(item.tokenAddress).getApproved(item.tokenId) == address(this)) {
                ...
                IWETH(WETH).withdraw(buyer.bidPrice);
                IWETH(WETH).transfer(msg.sender, buyer.bidPrice);
                buyer.bidPrice = 0;
    ...
    function cancelBid(uint256 auctionId)
        ...
        external {
            ...
            require(totalBidAmount > 0, "Withdraw: Insufficient amount");
            IWETH(WETH).withdraw(totalBidAmount);
            IWETH(WETH).transfer(msg.sender, totalBidAmount);
