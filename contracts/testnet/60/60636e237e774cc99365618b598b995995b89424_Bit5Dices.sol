...
contract Bit5Dices is ERC721Enumerable, Pausable {
    ...
    constructor (address _usdt) ERC721("Bit5 Dices", "BITDICES") {
        USDT = _usdt;
        uint256 endTime = block.timestamp + 365 days;
        saleConfig = SaleConfig(block.timestamp, 1000, endTime);
    }
    ...
    function setUpSale(uint256 _startTime, uint256 _maxCount, uint256 _duration) public onlyOwner {
        require(_startTime > 0, "Bit5Dices_Setup: Start time can't be 0.");
        require(_startTime > block.timestamp, "Bit5Dices_Setup: Start time can't be history.");
        require(_maxCount > 0, "Bit5Dices_Setup: Max count can't be 0");
        require(_duration > 0, "Bit5Dices_Setup: Duration can't be 0");
    ...
    function mint (uint256 _amount) public virtual whenNotPaused {
        ...
        require(_amount > 0, "Bit5Dices_Mint: Can't mint zero Weapon.");
        require(_saleConfig.startTime > 0, "Bit5Dices_Mint: Sale is not active now.");
        require(
            block.timestamp >= _saleConfig.startTime,
            "Bit5Dices_Mint: Please wait a bit until sell time."
        );
        require(
            block.timestamp <= _saleConfig.startTime + _saleConfig.duration,
            "Bit5Dices_Mint: Sale has ended."
        );
        require(_amount <= _saleConfig.maxCount, "Bit5Dices_Mint: Count exceeds max mint.");
        require(totalCount + _amount <= MAX_SUPPLY, "Bit5Dices_Mint: Max supply exeeded");

        require(IERC20(USDT).balanceOf(msg.sender) > price * _amount, "Bit5Dices_Mint: Insufficient USDT");
    ...
    function burn (uint256 tokenId) public {
        require(isBurnEnabled, "Bit5Dices_Burn: Burning is currently disabled.");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Bit5Dices_Burn: Burn caller is not the owner nor approved."
        );
