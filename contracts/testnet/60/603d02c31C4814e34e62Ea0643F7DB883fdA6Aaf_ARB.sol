...
contract ARB is IERC20, Ownable {
    ...
    function _transfer(
        ...
        bool takeFee = false;
        if(uniswapV2Pair==to || uniswapV2Pair==from){
                 takeFee = true;
        }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(1000).mul(destroyRate),
                currentRate
            );
            
            _reflectFee(
                rAmount.div(1000).mul(reflectRate),
                tAmount.div(1000).mul(reflectRate)
            );
            rate = destroyRate+reflectRate;
        }
