...
contract BondlyLaunchPad is Ownable {
    ...
    function isEligbleToBuy(uint256 _cardId, uint256 tier, bytes32[] calldata whitelistProof) public view returns (uint256) {
        ...
        if (startTime != 0 && block.timestamp >= startTime) {
            ...
            uint256 _currentCardBoughtAmount = _cardHistory.purchasedHistories[_cardId][msg.sender];
            uint256 _cardLimitPerWallet = _cardLimitsPerWallet[_cardId][tier];

            if(_currentCardBoughtAmount >= _cardLimitPerWallet) return 0;

            uint256 _currentSaleBoughtAmount = _saleHistory.purchasedHistories[_currentCard.saleNumber][msg.sender];
            uint256 _saleLimitPerWallet = _saleLimitsPerWallet[_currentCard.saleNumber][tier];
            if(_currentSaleBoughtAmount >= _saleLimitPerWallet) return 0;

            uint256 _cardAvailableForPurchase = _cardLimitPerWallet - _currentCardBoughtAmount;
            uint256 _saleAvailableForPurchase = _saleLimitPerWallet - _currentSaleBoughtAmount;

            uint256 _availableForPurchase = _cardAvailableForPurchase > _saleAvailableForPurchase ? _saleAvailableForPurchase : _cardAvailableForPurchase;
    ...
    function purchaseNFT(uint256 _cardId, uint256 _amount, uint256 tier, bytes32[] calldata whitelistProof) external payable {
        ...
        uint256 _availableForPurchase;
        {
            ...
            require(
                _currentCardBoughtAmount < _cardLimitPerWallet && _currentSaleBoughtAmount < _saleLimitPerWallet,
            ...
            _availableForPurchase = _cardAvailableForPurchase > _saleAvailableForPurchase ? _saleAvailableForPurchase : _cardAvailableForPurchase;
        
            if (_availableForPurchase > _amount) {
                _availableForPurchase = _amount;
            }

            _cards[_cardId].currentAmount = _cards[_cardId].currentAmount - _availableForPurchase;

            _cardHistory.purchasedHistories[_cardId][msg.sender] = _currentCardBoughtAmount + _availableForPurchase;

            _saleHistory.purchasedHistories[_currentCard.saleNumber][msg.sender] = _currentSaleBoughtAmount + _availableForPurchase;
        }
    ...
    function setCardsLimitsPerWallet(
        ...
        require(_cardNumbers.length == _limitsPerWallet.length, 
