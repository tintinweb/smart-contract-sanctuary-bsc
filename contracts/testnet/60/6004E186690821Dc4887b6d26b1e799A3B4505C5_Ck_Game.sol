...
contract Ck_Game is OwnableUpgradeable {
    ...
    function newRound(address owner_) public onlyOwner {
        _newRound(owner_);
    }
    ...
    function enter(uint positionId) external onlyEOA checkRoundUpgrade(positionId) {
        ...
        if (!refer.isRefer) {
            refer.isRefer = true;
            nodeInfo.initNodeDebt += ckAmount * 2 / 100;
            nodeInfo.marketDebt += ckAmount * 3 / 100;
        }
        ...
        if (msg.sender != pos.root) {
            _processReferAmount(msg.sender, ckAmount);
        }
    ...
    function buyInitNode() external onlyEOA {
        ...
        userInfo[msg.sender].initDebt = nodeInfo.initNodeDebt;
    }

    function buyMarketNode() external onlyEOA {
        require(!userInfo[msg.sender].isMarketNode && userInfo[msg.sender].ownerOfNode == 0, 'already have node');
        require(userInfo[msg.sender].position != 0, 'must bond 1 position');
        U.transferFrom(msg.sender, address(this), marketNodePrice);
        userInfo[msg.sender].isMarketNode = true;
        nodeInfo.marketNodeAmount ++;
        userInfo[msg.sender].marketDebt = nodeInfo.marketDebt;
    }

    function checkNodeReward(address addr) public view returns (uint rew){
        rew = 0;
        if (userInfo[addr].ownerOfNode != 0) {
            rew = nodeInfo.initNodeDebt - userInfo[addr].initDebt;
        }
        if (userInfo[addr].isMarketNode) {
            rew = nodeInfo.marketDebt - userInfo[addr].marketDebt;
        }
    }
    ...
    function claimNodeReward() external onlyEOA {
        ...
        CK.transfer(msg.sender, rew);
        if (userInfo[msg.sender].isMarketNode) {
            userInfo[msg.sender].marketDebt = nodeInfo.marketDebt;
    ...
    function calculateFailRelease(address addr) public view returns (uint){
        ...
        for (uint i = start; i <= nowRound; i++) {
            if (user.roundStake[i] && !user.claimRound[i]) {
                rew += roundInfo[positionId][i].totalCk / 10 / roundInfo[positionId][i].newUserList.length;
            }
        }
        return rew;
    }
...
//            address root = position[userInfo[addr].position].root;
            ...
            while (true) {
                ...
                if (tempLevel > lastLevel) {
                    ...
                    lastLevel = tempLevel;
                } else if (tempLevel == lastLevel && sameLevelLeft > 0) {
                    sameLevelLeft --;
    ...
    function checkPositionInfo(address addr) public view returns (uint round,
        uint totalAmount,
        uint startTime,
        uint endTime,
        uint userLimit,
