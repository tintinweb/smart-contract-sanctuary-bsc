...
contract BSHAREBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function initialize(
        ...
        IReceipt(BSHAREBUSD_A).initialize(_admin,address(this), "BSHARE-BUSD", "BSHARE-BUSD-A");
        depositTax = 10;
        withdrawTax = 0;
        exchangeRatio = 10 ** 18;
    }
    ...
    modifier _lastExchangeRatio(){
        ...
        if(lastExchangeRatio > exchangeRatio){
            require(lastExchangeRatio > exchangeRatio, "BSHAREBUSDVault: exchangeRatio decreased.");        
            _pause();
        }
    }
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "BSHAREBUSDVault: address is Blacklisted");
        require(amount > 0,"BSHAREBUSDVault: LPamount can't be 0");
        require(isBUSD,"BSHAREBUSDVault: Only Deposit BUSD");
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "BSHAREBUSDVault: you can't interact in same block");
            require(!isContract(msg.sender),"BSHAREBUSDVault: No external contract interaction");
        }
        IERC20(Admin.BUSD()).safeTransferFrom(user,address(this),amount);
        IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(), amount / 10);
        uint reserveBSHARE = IERC20(Admin.BShare()).balanceOf(address(BSHAREBUSD));
        uint reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(BSHAREBUSD));
        ...
        if(IERC20(Admin.BShare()).balanceOf(address(this)) >= BSHARERequire){
            percentYSL = 30;
            ...
            percentYSL = 15;
            USDyAmount = (amount * percentYSL)/100;
            ...
            reserveBSHARE = IERC20(Admin.BShare()).balanceOf(address(BSHAREBUSD));
            reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(BSHAREBUSD));
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + (amount* (10 ** 18)/USDyPrice));
        }
        convertBUSDToLP((amount * percentYSL/100), (reserveBSHARE * (amount * percentYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.BShare(), address(BSHAREBUSD));
        ...
        exchangeRatio = USDyOfDollarValue * (10 ** 18)/(IERC20(BSHAREBUSD_A).totalSupply());
        emit Deposit("BSHAREBUSDVault",address(this),user,amount,block.number,block.timestamp);
    }
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio whenNotPaused{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "BSHAREBUSDVault: address is Blacklisted");
        require(_amount <= UserDeposit[user],"BSHAREBUSDVault: Invalid Amount");
        if(UserDeposit[user] > 0){
            _claimRebaseReward(user);
        }
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        currentReward = currentReward + ((_rebaseReward * 10 ** 18)/(IERC20(BSHAREBUSD).totalSupply()));
    }
    ...
    */
    ...
    function setDepositTax(uint _depositTax) external _isAdmin{
        ...
        emit SetterForUint("BSHAREBUSDVault",address(this),depositTax,_depositTax,block.number,block.timestamp);
    ...
    */
    ...
    function _claimRebaseReward(address user) internal {
        userAmount = ((UserDeposit[user] * exchangeRatio)/ IERC20(address(BSHAREBUSD)).totalSupply());
    ...
    */
    ...
    function _userAmount(address user) internal view returns(uint){
        return((UserDeposit[user] * exchangeRatio)/ IERC20(address(BSHAREBUSD)).totalSupply());
    }
