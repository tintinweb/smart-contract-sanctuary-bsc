...
contract DDBNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721(name, symbol) {
      ...
      _wallet = 0xD71B67869731981EE1633Ad538Fd0D91070aEBA7;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(!paused, "DDBNFT: minting has not started yet");
    require(_mintAmount > 0, "DDBNFT: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "DDBNFT: you can only mint 10 at a time");
    require(lastSupply >= _mintAmount,  "DDBNFT: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "DDBNFT: total cost isn't match");
    }
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    ...
    require(_winner != address(0), "DDBNFT: zero address");
    require(_amount > 0, "DDBNFT: you have to mint at least one");
    require(lastSupply != 0,  "DDBNFT: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"DDBNFT: URI query for nonexistent token");
