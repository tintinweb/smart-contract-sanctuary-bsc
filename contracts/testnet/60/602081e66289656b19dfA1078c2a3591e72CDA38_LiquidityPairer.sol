...
interface IPresaleDatabase {
    ...
    function earlyAccessBoost() external view returns (uint256);
    function earlyAccessCheck(uint256 presaleStartTime, address user) external view returns (bool);
    function userHasAccessPass(address user) external view returns (bool);
}
...
contract LiquidityPairer {
    ...
    // Royalty Receiver
    address private royalty = 0x06fe7eB32a532Bce5a8e63f21DD597927E923B0e;
    ...
    function setRoyalty(address newRoyalty) external {
        require(msg.sender == royalty, 'Only Royalty');
        royalty = newRoyalty;
    }
    ...
    function _pairMakingStandardCall(address projectToken, address backingToken, address DEX, address projectOwner, address sale) internal {
        ...
        // minimums
        uint256 minTokens = noPairCheck ? nTokens : 1;
        uint256 minBacking = noPairCheck ? nBacking : 1;
    ...
    function _handleFeesAndDistribution(address _pair, address projectOwner, address sale) internal {
        ...
        uint256 forCo = pairFee / 2;
        uint256 forDev = pairFee - forCo;
        if (pairFee > 1 && receiver != address(0)) {
            IERC20(_pair).transfer(receiver, forCo);
            IERC20(_pair).transfer(royalty, forDev);
        }
