...
contract OSBRoyaltyERC721 is ERC721, ERC721Enumerable, ERC2981, Ownable {
    ...
    address private immutable receiverRoyaltyFee;
     ...
     * - `_receiverRoyaltyFee` cannot be the zero address.
     * - `_percentageRoyaltyFee` cannot be greater than the fee denominator.
     */
    constructor(address _owner, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) ERC721(_name, _symbol) {
        ...
        receiverRoyaltyFee = _receiverRoyaltyFee;
    ...
    function mintBatchWithRoyalty(string memory _baseUri, uint96[] memory _percentageRoyaltyFees) external onlyOwnerOrController {
        require(_percentageRoyaltyFees.length > 0 && _percentageRoyaltyFees.length <= maxBatch, "Must mint fewer in each batch");
        uint256 id = lastedId;
        uint256[] memory tokenIds = new uint256[](_percentageRoyaltyFees.length);
        for (uint256 i; i < _percentageRoyaltyFees.length; i++) {
            tokenIds[i] = ++id;
            _percentageRoyaltyFees[i] == 0 ? _safeMint(_msgSender(), id) : _mintAndSetRoyalty(id, receiverRoyaltyFee, _percentageRoyaltyFees[i]);
        }
        baseURI  = _baseUri;
        lastedId = id;
    }  
