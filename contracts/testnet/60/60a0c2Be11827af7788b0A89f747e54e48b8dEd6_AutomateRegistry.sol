  ...
  ReentrancyGuardUpgradeable
{
  ...
  mapping(address => EnumerableSetUpgradeable.UintSet) private _userActiveTasks;
  ...
  event TaskPaused(uint indexed id, address caller);
  event TaskUnpaused(uint indexed id, address caller);
  event TaskCheckDataUpdated(uint indexed id, bytes newCheckData);
    ...
    )
  {
    ...
    if (task.isPaused) revert PausedTask();
  ...
  {
    if (_task[id].isPaused) revert PausedTask();
  ...
  function pauseTask(uint256 id) external onlyActiveTask(id) onlyOwnerOrAdmin(id) {
    ...
    _userActiveTasks[_task[id].admin].remove(id);
    emit TaskPaused(id, msg.sender);
  }

  function unpauseTask(uint256 id) external onlyActiveTask(id) onlyOwnerOrAdmin(id) {
    if (!_task[id].isPaused) revert TaskNotPaused();
    _task[id].isPaused = false;
    _userActiveTasks[_task[id].admin].add(id);
    emit TaskUnpaused(id, msg.sender);
  }

    /**
   * @notice update the check data of an task
   * @param id the id of the task whose check data needs to be updated
   * @param newCheckData the new check data
   */
  function updateCheckData(uint256 id, bytes calldata newCheckData) 
  external 
  onlyOwnerOrAdmin(id)
  onlyActiveTask(id) {
    _checkData[id] = newCheckData;
    emit TaskCheckDataUpdated(id, newCheckData);
  }
  ...
  function userActiveTasks(address user) external view returns (uint256[] memory) {
    return _userActiveTasks[user].values();
  }
