...
contract MeToken is Ownable, IERC20 {
    ...
    uint256 public waitTrusteeshipNum;//等待兑换的ME
    ...
    function _transferWithSellFee(address from, address to, uint256 amount) internal {
        ...
        _basicTransfer(from, address(this), amountA);
        waitTrusteeshipNum += amountA;
        if(waitTrusteeshipNum > 0){
            tokenTrustenshipTransfer(waitTrusteeshipNum);//币值管理
        }
        _burn(from,amountB);
        _basicTransfer(from, _security_fund, amountC);
