...
contract StorageOperator is Ownable {
    ...
    IBWGToken private iBWGToken;
    IERC777 private token;
    ITokenTransferOperator private tokenTransferOperator;
    
    constructor(address _baseContract, address _fundAddress, uint256 _storageCostRate, uint256 _transactionFee ) {     
        iBWGToken = IBWGToken(_baseContract); 
        token = IERC777(_baseContract);
        tokenTransferOperator = ITokenTransferOperator(token.defaultOperators()[0]); 
