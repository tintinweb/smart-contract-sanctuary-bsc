...
/** 
    ...
    🖌 7 ways to earn in 1 investment
      ...
      🖊  Lottery Every 6 hours.
      🖊  Top Referrer Rewards every 24 hours.
      🖊  Last Deposit Jackpot Rewards every 12 hours.
    ...
    🖌 5 Unique Features:
      ...
      🖊  Flip Coin - Miner Betting Feature runs every 6 hours.
      🖊  Horse Race - Miner Betting Feature runs every 8 hours. 
...
contract RykerVip_BUSD {
    ...
    uint256 public maxIncentiveBalance = 5000 ether;
    uint256 public maxWalletDepositLimit = 10000 ether;
    ...
    uint256 private horseBettingTimeStep = 8 hours;
    uint256 private lastDepositTimeStep = 12 hours;
    uint256 private compoundTimeStep = 24 hours; //set to 24 hours before deploying to main
    uint256 private topReferrerTimeStep = 24 hours;
    uint256 private incomeWithdrawalTimeStep = 0;  //set to 4 hours before deploying to main
    ...
    uint256 private topReferrerBalance;
    ...
    constructor(address _development, address _lateInvFund, address _marketing, address _autoCompoundExecutor) {
		require(!isContract(_development) && !isContract(_lateInvFund) && !isContract(_marketing) , "Not a user address.");
    ...
    function executeAutoCompound(address _addr) external {
        ...
        emit AutoCompoundEvent(_addr, getCurTime());
    }
    ...
    function initializeContract(address addr, uint256 amount) public onlyOwner {
        if (!contractStarted) {
            ...
            TOP_REFERRER_START_TIME = getCurTime();
            LAST_DEPOSIT_START_TIME = getCurTime();
            HORSE_RACE_START_TIME = getCurTime();
            invest(addr, amount);
    ...
    function compoundAddress(bool isCompound, address _address) internal {
        ...
        if(getCurTime().sub(user.lastHatch) >= compoundTimeStep) {
    ...
    function sellProfit() public nonReentrant {
        ...
        if(user.lastHatch.add(incomeWithdrawalTimeStep) > getCurTime()) revert("Withdrawals can only be done after withdraw cooldown.");
    ...
    function chooseWinners() external {
        ...
        if(LOTTERY_ACTIVATED && (getCurTime().sub(LOTTERY_START_TIME) >= lotteryTimeStep || participants >= 200 || currentPot >= maxIncentiveBalance)) {
            drawLotteryWinner();
        }

        if(TOSS_COIN_ACTIVATED && getCurTime().sub(TOSS_COIN_START_TIME) >= tossCoinTimeStep && currentHeadsBetCount > 0 && currenttailsBetCount > 0) {
            drawTossCoinRoundWinners();
        }

        if(HORSE_BETTING_ACTIVATED && getCurTime().sub(HORSE_RACE_START_TIME) >= horseBettingTimeStep && currentUserHorseBettors.length > 0) {
            drawHorseRaceRoundWinners();
        }

        if(LAST_DEPOSIT_ACTIVATED && getCurTime().sub(LAST_DEPOSIT_START_TIME) >= lastDepositTimeStep && totalLastDepositJackpot > 0 && potentialLastDepositWinner != address(0)) {
            drawLastDepositWinner();
        }
        
        if(TOP_REFERRER_ACTIVATED && getCurTime().sub(TOP_REFERRER_START_TIME) >= topReferrerTimeStep) {
            drawTopReferrerPool();
        }
   }
    ...
    function poolDeposits(address _addr, uint256 _amount) private {
        ...
        if(topReferrerBalance.add(pool_amount) > maxIncentiveBalance){       
            topReferrerBalance += maxIncentiveBalance.sub(topReferrerBalance);
        }
        else{
            topReferrerBalance += pool_amount;
        }
        ...
        for(uint256 i = 0; i < topReferrerBonusArr.length; i++) {
            ...
            if(totalDepositPool[topReferrerRound][upline] > totalDepositPool[topReferrerRound][poolTop[i]]) {
    ...
    function drawTopReferrerPool() private {
        ...
        uint256 draw_amount = topReferrerBalance.div(10);
        ...
        for(uint256 i = 0; i < topReferrerBonusArr.length; i++) {
            ...
            uint256 win = draw_amount.mul(topReferrerBonusArr[i]) / 100;
            ...
            topReferrerBalance -= win;

            emit PoolPayoutEvent(poolTop[i], minerRewards, getCurTime());
        }
    ...
    function lastDepositEntry(address userAddress, uint256 amount) private {
        ...
        if(totalLastDepositJackpot.add(share) > maxIncentiveBalance){       
            totalLastDepositJackpot += maxIncentiveBalance.sub(totalLastDepositJackpot);
        }
        else{
    ...
    function drawLastDepositWinner() private {
        ...
        emit LastBuyEvent(currentLastBuyRound, potentialLastDepositWinner, busdReward, minerRewards , getCurTime());
        ...
        if(participants != 0){
            ...
            for(uint256 i = 0; i < participants; i++){
                if((random >= init_range[i]) && (random <= end_range[i])){
                    ...
                    tokenERC20.transfer(autoCompoundExecutor, devtTax);

                    /** record round **/
                    emit LotteryEvent(currentLotteryRound, winnerAddress, minerRewards, participants, totalTickets, getCurTime());
    ...
    function getRandomValue() private view returns(uint256) {
        ...
        return uint256(keccak256(abi.encode(_blockhash,getCurTime(), currentPot, block.difficulty, marketEggs, getContractBalance())));
    }
    ...
    function drawTossCoinRoundWinners() private contractActivated {
        ...
        emit TossCoinEvent(currentTossCoinRound,  winner,  currentTossCoinPot,  winnerOdds, winnersCount,  getCurTime());
    ...
    function drawHorseRaceRoundWinners() private contractActivated {
        ...
        emit HorceRaceEvent(currentHorseRaceRound, winner, winnerOdds, winnersCount, getCurTime());
    ...
    function payFees(uint256 eggValue, bool isSell, bool fromFixedYield) internal returns(uint256) {
        ...
        uint256 totalTax =  devtTax.add(marketTax).add(lateInvFundTax); // 7%
    ...
    function poolTopInfo() view external returns(address[5] memory addrs, uint256[5] memory deps) {
        for(uint256 i = 0; i < topReferrerBonusArr.length; i++) {
            if(poolTop[i] == address(0)) break;
            addrs[i] = poolTop[i];
            deps[i] = totalDepositPool[topReferrerRound][poolTop[i]];
        }
    }
    ...
    function getSiteInfo() public view returns (uint256 _totalStaked, uint256 _totalDeposits, uint256 _totalCompound, uint256 _totalRefBonus, uint256 _totalLotteryMinerBonus, uint256 _topReferrerBalance, uint256 _pool_leader) {
        return (totalStaked, totalDeposits, totalCompound, totalRefBonus, totalLotteryMinerBonus, topReferrerBalance, totalDepositPool[topReferrerRound][poolTop[0]]);
    }
    ...
    function ENABLE_FLIPCOIN(bool value) external onlyOwner {
        ...
        if(value){
            ...
            drawTossCoinRoundWinners(); //run existing function before turning off.
    ...
    function ENABLE_LOTTERY(bool value) external onlyOwner {
        ...
        if(value){
            ...
            drawLotteryWinner(); //run existing function before turning off.
    ...
    function ENABLE_TOP_REFERRER(bool value) external onlyOwner {
        ...
        if(value){
            ...
            drawTopReferrerPool(); //run existing function before turning off.
    ...
    function ENABLE_HORSE_BETTING(bool value) external onlyOwner {
        ...
        if(value){
            ...
            drawHorseRaceRoundWinners(); //run existing function before turning off.
    ...
    function ENABLE_LAST_DEPOSIT_REWARDS(bool value) external onlyOwner {
        ...
        if(value){
            ...
            drawLastDepositWinner(); //run existing function before turning off.
    ...
    function setCurTimeForTesting(uint256 timeToAdd) external{
        isTEST = true;
        TESTTIME = TESTTIME+timeToAdd;
    }
     

    function generateTestInvestors(uint max) public  {
        uint maxCt = max;
        uint iteration = 0;
    
        while(iteration<=maxCt){

            address randomAddress= address(uint160(uint(keccak256(abi.encodePacked(iteration, blockhash(block.number))))));
            investTest(randomAddress);
            iteration++;
        }
    ...
    function investTest(address testUser) private {
    require(!isContract(testUser), "Not a user ");
        User storage user = users[testUser];
        user.userDeposit = user.userDeposit.add(1 ether);
        user.initialDeposit = user.initialDeposit.add(1 ether);
        user.claimedEggs = user.claimedEggs.add(1 ether);
        user.yieldLastWithdrawTime = getCurTime();
        totalStaked = totalStaked.add(1 ether);
        user.miners = user.miners.add(1000);
        user.claimedEggs = 0;
        user.lastHatch = getCurTime().sub(compoundTimeStep);     
