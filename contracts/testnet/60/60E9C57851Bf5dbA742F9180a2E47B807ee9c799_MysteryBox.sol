    ...
    ReentrancyGuard
{
    ...
    constructor(
        ...
        uint256 _numberOfBox,
        uint256[] memory _priceEachBox
    )
    {
        require(address(_treasuryWallet) != address(0), "Invalid fishTreasury wallet address");
        require(_priceEachBox.length == _numberOfBox, "Not consistent");
        isInitialized = true;
        ...
        for (uint256 index = 0; index < _priceEachBox.length; index++) {
            basePrice[index] = _priceEachBox[index];
        }
    }
     ...
     */
    function setBasePrice(uint256 _boxNumber, uint256 _price) public onlyOwner{
        require(_boxNumber != 0 && _price !=0, "FishTreasury: Parameters must not be 0");
     ...
     */
    function addNFTsIntoBoxByTokenIdRange(uint256 _boxNumber, uint256 _startId, uint256 _endId)
    external onlyOwner {
     ...
     */
    function addNFTsIntoBox(uint256 _boxNumber, uint256[] memory _tokenIds)
    external onlyOwner {
    ...
    function mintNFT(uint256 _boxNumber) external payable nonReentrant {
        ...
        if (betaWhitelisted) {
            require(isWhitelisted[_receipient],
                "You need to be whitelisted to test the beta game");
        }

        require(NFTIds[_boxNumber].length > 0, "Run out of NFT");

        // base bnb for each box number
        uint256 amountToTransfer = basePrice[_boxNumber];
        ...
        if (isUsedBNB){
            (bool sent, ) = payable(treasuryWallet).call{value: amountToTransfer}("");
            require(sent, "Failed to send Ether");
        } else {
            tokenAddress.transferFrom(_msgSender(), address(treasuryWallet), amountToTransfer);
        }
        emit ClaimNFT(_receipient, fishNFTId);
    }
