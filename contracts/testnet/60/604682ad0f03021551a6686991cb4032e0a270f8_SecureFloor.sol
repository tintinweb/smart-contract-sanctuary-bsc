...
contract SecureFloor is Ownable {
    ...
    uint256 public classNumbers;    // Numbers of created classes (start from 1)

    mapping(address => uint256) public getClassId; // token address => classId
    ...
    event CreateOffer(address creator, Parameters p, uint256 classId, uint256 nonceId);
    ...
    function createOffer(Parameters calldata p) external payable returns(uint256 classId, uint256 nonceId) {
        ...
        emit CreateOffer(msg.sender, p, classId, nonceId);
