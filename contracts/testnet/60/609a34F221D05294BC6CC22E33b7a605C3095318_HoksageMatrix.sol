...
contract HoksageMatrix is Ownable {
  ...
  function finalize() public onlyOwner() returns (bool) {
    require(!users[address(this)].registered,"already finalize");

    lastid = lastid.add(1);
    address2id[address(this)] = lastid;
    id2address[lastid] = address(this);

    users[address(this)].registered = true;
    users[address(this)].invitor = address(0);
    uint256 i = 1;

    do{
    users[address(this)].activelevel[i] = true;
    i++;
    }while(i<=15);

    total_invest[address(this)] = cost[1];

    return true;
  }

  function registerExt(address register,address invitor) public payable returns (bool) {
  ...
  function buynewslotExt(address account,uint256 level) external payable returns (bool) {
  ...
  function getMatrix32(address viewer,uint256 level,uint256 viewcycle) public view returns (address[] memory){
    ...
    if(viewcycle>0){
        ...
        path[2] = getMatrixTree(viewer,level,0);
        path[3] = getMatrixTree(viewer,level,1);
    }
    path[4] = getMatrixTree(path[2],level,0);
    path[5] = getMatrixTree(path[2],level,1);
    path[6] = getMatrixTree(path[3],level,0);
    path[7] = getMatrixTree(path[3],level,1);
    path[8] = getMatrixTree(path[4],level,0);
    path[9] = getMatrixTree(path[4],level,1);
    path[10] = getMatrixTree(path[5],level,0);
    path[11] = getMatrixTree(path[5],level,1);
    path[12] = getMatrixTree(path[6],level,0);
    path[13] = getMatrixTree(path[6],level,1);
    path[14] = getMatrixTree(path[7],level,0);
    path[15] = getMatrixTree(path[7],level,1);
    path[16] = getMatrixTree(path[8],level,0);
    path[17] = getMatrixTree(path[8],level,1);
    path[18] = getMatrixTree(path[9],level,0);
    path[19] = getMatrixTree(path[9],level,1);
    path[20] = getMatrixTree(path[10],level,0);
    path[21] = getMatrixTree(path[10],level,1);
    path[22] = getMatrixTree(path[11],level,0);
    path[23] = getMatrixTree(path[11],level,1);
    path[24] = getMatrixTree(path[12],level,0);
    path[25] = getMatrixTree(path[12],level,1);
    path[26] = getMatrixTree(path[13],level,0);
    path[27] = getMatrixTree(path[13],level,1);
    path[28] = getMatrixTree(path[14],level,0);
    path[29] = getMatrixTree(path[14],level,1);
    path[30] = getMatrixTree(path[15],level,0);
    path[31] = getMatrixTree(path[15],level,1);
    return path;
  }
  ...
  function getMatrixTree(address account,uint256 level,uint256 branch) internal view returns (address) {
    ...
    if(users[account].matrix[level].reinvestcount>0){
  ...
  function isColor(address account,address viewer,uint256 level) external view returns (uint256) {
    if(users[account].matrix[level].upline == address(0) ){
      return 1;
    }else if(users[account].matrix[level].upline == viewer){
      return 2;
    }else if(
      users[account].matrix[level].upline == getUpline(viewer,level,0)
      || users[account].matrix[level].upline == getUpline(viewer,level,1)
      || users[account].matrix[level].upline == getUpline(viewer,level,2)
      || users[account].matrix[level].upline == getUpline(viewer,level,3)
    ){
      return 3;
    }else{
      return 4;
  ...
  function shouldReinvest(address account,uint256 level) internal view returns (bool) {
    if(
        users[getUpline(account,level,0)].matrix[level].referee.length==2 &&
        users[getUpline(account,level,1)].matrix[level].referee.length==2 &&
        users[getUpline(account,level,2)].matrix[level].referee.length==2 &&
        users[getUpline(account,level,3)].matrix[level].referee.length==2
    ){
        return true;
    }else{
        return false;
