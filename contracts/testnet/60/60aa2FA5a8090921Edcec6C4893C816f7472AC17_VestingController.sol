...
contract VestingController is Ownable, Withdrawable, ReentrancyGuard {
    ...
    function claimTokens(uint256 totalAllocationWei, bytes32[] calldata proof) external payable nonReentrant {
        require(!_isPermissionless(), 'This vesting does not require proofs');
        require(
    ...
    function _ensurePermittedRole() internal {
        if (token.isPermissioned() && !token.hasRole(token.PERMITTED_ROLE(), address(this))) {
