...
contract MetaMinerTokenTestNet is Context, IBEP20, Ownable {
  ...
  constructor() public {
    _name = "Meta-Miner TestNet";
    _symbol = "$INGOT TestNet";
    _decimals = 18;
  ...
  function _takeFee(address account, uint256 amount) internal {
    ...
    _balances[_deadAddress] = _balances[_deadAddress].add(Fee);
    _balances[account] = _balances[account].sub(Fee);
    emit Transfer(account, _deadAddress, Fee);
    ...
    _balances[_supportAddress] = _balances[_supportAddress].add(Fee);
    _balances[account] = _balances[account].sub(Fee);
