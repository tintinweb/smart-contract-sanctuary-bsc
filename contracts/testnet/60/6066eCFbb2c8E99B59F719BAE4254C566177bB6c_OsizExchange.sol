...
library BytesLibrary {
    ...
    function recover(
        ...
        bytes32 fullMessage = keccak256(
            abi.encodePacked("\x19BNBereum Signed Message:\n32", hash)
        );
        return ecrecover(fullMessage, v, r, s);
    }
}

// File: contracts/lib/IwBNB.sol
...
contract TransferSafe {
    ...
    function BEP1155safeMintwithuriTransferFrom(
        IOsizStore token,
        address from,
        address to,
        uint256 id,
        ...
        require(
            ...
            "TransferSafe:BEP1155safeMintWithUriTransferFrom:: transaction Failed"
        );
    }
}

contract OsizExchange is OrderState, TransferSafe {
        ...
        if (order.key.sellAsset.assetType == AssetType.BEP721 || order.key.buyAsset.assetType == AssetType.BEP721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_OsizStore), "invalid sell asset");
                    BEP1155safeMintTransferFrom(
                        IOsizStore(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        1,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_OsizStore), "invalid buy asset");
                    BEP1155safeMintTransferFrom(
                        IOsizStore(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        1,
    ...
    function transferSellFee(
        ...
        } else if (order.key.buyAsset.token == wBNB) {
            transferwBNBFee(
                order.buying,
                order.key.owner,
                buyer,
    ...
    function transferBuyFee(
        ...
        if (order.key.sellAsset.token == wBNB) {
            transferwBNBFee(
                order.selling,
                buyer,
                order.key.owner,
    ...
    function transferwBNBFee(
        ...
        if ((secoundaryFee > 0) && (royaltyReceipt != address(0x00))) {
            require(
                IwBNB(wBNB).transferFrom(buyer, royaltyReceipt, secoundaryFee),
                "Failed royalty fee transfer"
            );
        }
        if (remaining > 0) {
            require(
                IwBNB(wBNB).transferFrom(buyer, _seller, remaining),
                "Failed transfer"
            );
        }
    }
    ...
    function fiatMBNBod(uint256 _type,address _nftAddress , address payable _from ,address _to,address payable _royalty,uint256 royaltyFee ,uint256 _id,string memory uri ,uint _nftType,uint amount)external payable{
        ...
        if(_type == 1){
            require(_nftAddress == address(_OsizStore),"Invalid Token Address");
             BEP1155safeMintwithuriTransferFrom(IOsizStore(_OsizStore),_from,_to,_id,1,uri);  
        }
        else{
            require(_nftAddress != address(0x00),"Invalid Token Address");
            require(_nftType == 1 || _nftType == 2,"Invalid NFT Type");
            if (_nftType == 1){
                 BEP721safeTransferFrom(IBEP721(_nftAddress),_from,_to,_id);
            }
            else{
                BEP1155safeTransferFrom(IBEP1155(_nftAddress),_from,_to,_id,1);
            }
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _OsizStore = IOsizStore(newMintableStore);
    ...
    function OsizStore() external view returns(address){
        return address(_OsizStore);
    }
