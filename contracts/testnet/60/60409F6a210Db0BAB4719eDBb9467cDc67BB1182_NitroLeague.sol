...
contract NitroLeague is INitroLeague, Context, Ownable, TokenWithdrawer {
    ...
    function createRaceEvent(string calldata raceEventID, uint256 raceEventType, string memory title, string memory uri) external override returns(address) {
        ...
        IRaceEvent raceEvent = IRaceEvent(raceEventFactory.newRaceEvent(
            address(this), raceEventType, title, uri
        ));
...
contract RaceEvent is IRaceEvent, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    constructor(address nitroLeague_, uint256 raceEventType_, string memory title_, string memory uri_) {
...
contract Race is IRace, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    /// List of players.
    //mapping(address => uint) public playersJoiningDate; // player => timestamp.
    ...
    function addPlayers(address payable[] memory players_) external override onlyOwner {
        ...
        for (uint256 i = 0; i < players_.length; i++) {
            //require(playersJoiningDate[players_[i]] == 0, string(abi.encodePacked("A player is already added")));
            players.push(players_[i]);
            //playersJoiningDate[players_[i]] = block.timestamp;
