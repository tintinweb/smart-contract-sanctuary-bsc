...
contract MetafameSaleMTv14_0729 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, Status{
    ...
    function purchaseBoxVCsale(
        ...
        public 
        {
        require(block.timestamp >= vcRound[vcRoundNumber].startTime, "MetafameSale: Not reach vc-sale time");
        require(block.timestamp <= vcRound[vcRoundNumber].endTime, "MetafameSale: End of sale");
        require(!endsaled(), "MetafameSale: End of sale");
        require(vcRound[vcRoundNumber].alreadySaled + number <= vcRound[vcRoundNumber].limit, "MetafameSale: Reach the sale limit of this round");
        require(metafame1155.getAllocatedBoxQuantity() + number <= metafame1155.getMaxSupply() - _FOR_OWNER, "Metafame1155: Reach the max supply (public)");
        require(vcAlreadyMinted[_msgSender()] + number <= maxPurchaseLimitVC, "MetafameSale: Exceed allowed mint number");
        ...
        require(payAmount == busdPerBoxVC * number, "Insufficient BUSD");
    ...
    function purchaseBoxPresale(
        ...
        require(metafame1155.getAllocatedBoxQuantity() + number <= metafame1155.getMaxSupply() - _FOR_OWNER, "Metafame1155: Reach the max supply (public)");
        require(whiteListAlreadyMinted[_msgSender()] + number <= maxPurchaseLimitWL, "MetafameSale: Exceed allowed mint number");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf) , "MetafameSale: The account is not in the whitelist");
        require(gender < 2, "MetafameSale: Wrong Gender Type");
        require(skin < 4, "MetafameSale: Wrong Skin Type");
        ...
        require(payAmount == busdPerBoxWL * number, "Insufficient BUSD");
    ...
    function purchaseBoxPublicSale(
        ...
        require(metafame1155.getAllocatedBoxQuantity() + number <= metafame1155.getMaxSupply() - _FOR_OWNER, "Metafame1155: Reach the max supply (public)");
        require(alreadyMinted[msg.sender] + number <= maxPurchaseLimit, "MetafameSale: Exceed purchese limit");
        require(gender < 2, "MetafameSale: Wrong Gender Type");
        require(skin < 4, "MetafameSale: Wrong Skin Type");
        ...
        require(payAmount == busdPerBox * number, "Insufficient BUSD");
    ...
    function makeOver( 
        ...
        require(payAmount == makeOverPrice, "Insufficient BUSD");
    ...
    function refresh( 
        ...
        require(refreshed[tokenId][refreshRoundNumber] == false, "MetafameSale: Thsi NFT was refreshed in this round");
        require(payAmount == refreshPrice, "Insufficient BUSD");
    ...
    function getVCRoundSaleLimit(uint256 roundNumber) onlyOwner external view returns (uint256) {
        return vcRound[roundNumber].limit;
    }

    function getVCRoundSaled(uint256 roundNumber) onlyOwner external view returns (uint256) {
        return vcRound[roundNumber].alreadySaled;
    }
    ...
    function getRefreshRoundSaleLimit(uint256 roundNumber) onlyOwner external view returns (uint256) {
        return rfRound[roundNumber].limit;
    }

    function getRefreshRoundRefreshed(uint256 roundNumber) onlyOwner external view returns (uint256) {
        return rfRound[roundNumber].alreadyRefreshed;
    }
