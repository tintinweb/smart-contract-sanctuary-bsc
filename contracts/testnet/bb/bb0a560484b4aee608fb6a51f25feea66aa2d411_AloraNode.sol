...
contract AloraNode is ERC20Upgradeable {
   ...
   //Air-Drop Method 
   uint256 private _maxCapTeamAllocation  = 100000e18;
   uint256 public teamAllocationAmount; 
    function mintTeamAllocation(address _teamAllocation,uint256 _amount) public onlyOwner {
        // require(teamAllocationAmount > _maxCapTeamAllocation,"Team Allocation exceed limite");
        _mint(_teamAllocation,_amount);
        teamAllocationAmount +=  _amount;
    }
    uint256 private _maxCapReserveAllocation  = 100000e18;
    ...
    function mintReserveAllocation(address _reserveAllocation,uint256 _amount) public onlyOwner {
        // require(reserveAllocationAmount > _maxCapReserveAllocation,"Reserve Allocation exceed limite");
        _mint(_reserveAllocation,_amount);
        reserveAllocationAmount +=  _amount;
    }
    uint256 private _maxCapReservePool  = 700000e18;
    uint256 public reservePoolAmount; 
    function mintReservePool(address _reservePool,uint256 _amount) public onlyOwner {
        // require(reservePoolAmount > _maxCapReservePool,"Reserve Pool exceed limite");
        _mint(_reservePool,_amount);
    ...
    /// @dev Add liquidity
    uint256 private _maxCapliquidityPool  = 100000e18;
    uint256 public liquidityPoolAmount; 
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) public  onlyOwner {
        require(liquidityPoolAmount > _maxCapliquidityPool,"liquidity Pool exceed limite");
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _mint(address(this), tokenAmount); 
        liquidityPoolAmount += tokenAmount;
        // add the liquidity
