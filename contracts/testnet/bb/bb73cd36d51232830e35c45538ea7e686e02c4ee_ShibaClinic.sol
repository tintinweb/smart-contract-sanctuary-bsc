...
contract ShibaClinic is Context, IBEP20, Ownable {
    ...
    event SetLimit(uint32 , LimitedAddress);
   ...
   function setWalletLimit(address wallet, uint256 amount,uint day) onlyOwner() public{
    ...
    function CheckWalletLimit(address wallet) private view returns(uint256){
      ...
      return Amount * _DECIMALFACTOR;
    }

    
    function ReportWalletLimit(address wallet) public view returns(LimitedAddress[] memory){

        LimitedAddress[] memory mylist = new LimitedAddress[](_addressLimited[wallet].length);
        ...
        for  (uint i=0 ; i < _addressLimited[wallet].length;i++)
        {
           ...
           if(_Limited[limitedId].EndDate >= block.timestamp){
               mylist[i] = LimitedAddress(_Limited[limitedId].Wallet,_Limited[limitedId].Amount,_Limited[limitedId].EndDate);
           }
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require((balanceOf(sender) - CheckWalletLimit(sender)) > amount,"Your account is limited.");
