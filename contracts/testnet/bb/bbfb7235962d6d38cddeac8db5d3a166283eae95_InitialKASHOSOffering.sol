...
contract InitialKASHOSOffering is Ownable {
    ...
    uint256 public maxSupply = 150000000000000000000;
    ...
    uint256 public maxInvestment = 3000000000000000000;
    ...
    constructor (
        ...
        require(_kashos != IERC20(address(0)));
        mock = _kashos;
    ...
    function increaseSupply(uint256 increasedSupply) public onlyOwner {
        maxSupply += increasedSupply; // If in case of more demand on `KASHOS` supply
    }
    ...
    function investwhitelistBNB(address _investor) public payable returns(bool status){
        ...
        require(totalKASHOSDistributed <= maxSupply, 'Out of KASHOS in Pre-Sale!');
        ...
        if(investors[_investor]._name != address(0)){ // Checking for validations
            uint256 _totalKASHOSGiven = investors[_investor]._kashosStored;
            uint256 _maxKASHOS = (maxInvestment.mul(kashosPerBNB));
            require(_totalKASHOSGiven < _maxKASHOS, 'Whoa! Thats a lot of investment from you! :)');
        }
        ...
        uint256 kashosDistributed = (amountInvested.mul(dividend)).div(kashosPerBNB); // KASHOS Calculated
    ...
    function setMaxSupply(uint256 _invest) public onlyOwner {
        maxSupply = _invest;
    }
