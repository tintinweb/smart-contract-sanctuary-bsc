...
contract BSG {
    ...
    uint256 private constant minDeposit = 25000000000000000000;
    uint256 private constant maxDeposit = 2000000000000000000000;
    uint256 private constant freezeIncomePercents = 3000;
    uint256 private constant timeStep = 3 minutes;
    uint256 private constant dayPerCycle = 10 minutes; 
    uint256 private constant dayRewardPercents = 150;
    uint256 private constant maxAddFreeze = 2 minutes;
    ...
    address[2] public feeReceivers = [0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7,0x714674009B8e0F873DBa410c29d936517cd7ce2a];
    ...
    function userRewardForWithdraw(address _userAddress) public view returns(uint256) {
        (uint256 staticReward, uint256 staticSplit) = _calCurStaticRewards(_userAddress);
        uint256 splitAmt = staticSplit;
        uint256 withdrawable = staticReward;

        (uint256 dynamicReward, uint256 dynamicSplit) = _calCurDynamicRewards(_userAddress);
        withdrawable = withdrawable.add(dynamicReward);
        splitAmt = splitAmt.add(dynamicSplit);
