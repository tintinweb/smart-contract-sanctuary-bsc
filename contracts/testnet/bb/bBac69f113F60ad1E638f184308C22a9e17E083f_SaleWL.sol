...
contract SaleWL {
    ...
    StakingMP constant private staking = StakingMP(0x5c8c420685d437c53CA296b021A796533A95407f); // Staking With List
    ...
    function getUserLimitNow(address userAddress) public view returns(uint256){
        ...
        if(getMaxBusdUser(userAddress) == 0){
            return 0;
        }
        if(getLeftToken() < _minBUSD.div(_RATE).mul(_divider)){
            return 0;
        }
        uint256 limit = getMaxBusdUser(userAddress);
        if(block.timestamp < _START_TIME.add(_SALE_DAYS)){
            if(users[userAddress].round == _round){
                limit = getMaxBusdUser(userAddress).sub(users[userAddress].invested[_round]);
            }
        }
        if(block.timestamp >= _START_TIME.add(_SALE_DAYS) && getLeftToken() >= _minBUSD.div(_RATE).mul(_divider)){
            limit = getMaxBusdUser(userAddress);
        }
        if(block.timestamp >= _START_TIME.add(_SALE_DAYS) && getLeftToken() < _minBUSD.div(_RATE).mul(_divider)){
            limit = 0;
        }
