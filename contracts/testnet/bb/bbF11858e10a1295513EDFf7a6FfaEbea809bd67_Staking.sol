...
contract Staking is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 tvl;            // Total value locked in the contract
    ...
    function stake(uint256 _pid, uint256 _amount) external nonReentrant {  
        ...
        require(_poolInfo.token.balanceOf(msg.sender) >= _amount, "Insufficient token");
        
        // Transfer token into the contract
        bool _status = _poolInfo.token.transferFrom(msg.sender, address(this), _amount);
        require(_status, "Failed to transfer token");
    ...
    function unstake(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(!_poolInfo.paused, "Contract paused");
        require(_userInfo.stakedAmount >= _amount, "Invalid amount");
        require(_poolInfo.token.balanceOf(address(this)) >= _amount, "Insufficient token in contract");
    ...
    function getUserInfoByPid(uint256 _pid, address _userAddress) external view returns (UserInfo memory) {
        return userInfo[_pid][_userAddress];
    }

    function getPoolInfo(uint256 _index) external view returns (PoolInfo memory) {
        return poolInfo[_index];
    }
    ...
    event Stake(address indexed account, uint256 pid, uint256 amount);
    event Unstake(address indexed account, uint256 pid, uint256 amount);
    event RegisterPool(address token);
    event UpdatePool(uint256 pid, address token, bool paused);
