...
contract Staking is Ownable, ReentrancyGuard{
    ...
    uint256 public ratesChangesCount;
    ...
    constructor(address _HAPPY_COIN, address _HAPPY_MONKEY) {
        ...
        MIN_HAPPY_BALANCE = 5000 * 10**18;
        rewardsRatesPerSecond = [46296290000000, 69444440000000, 115740740000000, 231481480000000, 578703700000000];
    }
    ...
    function getPendingRewardsById(uint256 tokenId) public view returns (uint256){
        ...
        if(currentCycle == 0) return  current_rewardsRate * (current_time - stakeTimestamp);
        ...
        rewards = rewards * multiplierPerLevel[IHappyMonkey(HAPPY_MONKEY).getNftLevel(tokenId)];
    ...
    function emergency(uint256 tokenId) external onlyOwner{
    ...
    function emergencyInBulk(uint256[] memory tokenIds) external onlyOwner{
        require(!stakingEnabled, "Staking must be disabled");
        uint256 size = tokenIds.length;
        uint256 i;
        address staker;
