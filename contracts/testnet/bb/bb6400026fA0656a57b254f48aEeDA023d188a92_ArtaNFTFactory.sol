...
contract ArtaNFTFactory is AccessControl, Initializable {
    ...
    function beforeMint(address token, bytes32[] memory merkleProof) public returns (uint256) {
        require(nftMap[msg.sender], 'NFT contract not found');
        bool allOpen = whiteList.getCollectionAllOpen(token);
        uint256 price = whiteList.whiteListPrice(msg.sender, token, merkleProof);
        require(allOpen || price != 0, 'Mint not opening');
        if(price != 0)
            whiteList.addWhiteListUsedCount(msg.sender, token);
        return price;
    }
    ...
    function setWhiteList(
        ...
        whiteList.setWhiteList(token, merkleRoot, price, limit);
    }
}