...
contract ReitCoin is Ownable {
    ...
    function claim() external {
        ...
        uint256 _id;
        if (msg.sender == foundersTeam) {
            _id = 1;
        } else {
            _id = 2;
        }

        currentTeam = idToTeam[_id];
        require(
        ...
        uint256 tokensToBeClaimed = (currentTeam.teamAmount *
            currentTeam.nextClaimShare) / (10**4);
    ...
    function calculateVestedTokens(address _userAddress) external view returns(uint256){
     ...
     if((block.timestamp>currentTeam.claimTimestamp)&&(currentTeam.teamAmount!=currentTeam.claimedAmount)){
         return (currentTeam.teamAmount * currentTeam.nextClaimShare) / (10**4);
     }
     else {
         return 0;
     }
