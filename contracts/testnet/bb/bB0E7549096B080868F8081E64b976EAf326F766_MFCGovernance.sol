...
contract MFCGovernance is AdminAgent, RegistrarClient {
  ...
  struct MigrationProposal {
    ...
    address mfcTradingLicenseDestination;
    address mfcExchangeCapDestination;
    address mfcCollateralTreasuryDestination;
    uint256 endsAt;
  }
  ...
  MFCToken private _mfc;
  address private _busdTreasuryAddress;
  address private _mfcTradingLicenseAddress;
  address private _mfcExchangeCapAddress;
  address private _mfcCollateralTreasuryAddress;
  ...
  modifier hasMigrationAddresses() {
    ...
    require(_mfcTradingLicenseAddress != address(0), "MFCTradingLicense address not set");
    require(_mfcExchangeCapAddress != address(0), "MFCExchangeCap address not set");
    require(_mfcCollateralTreasuryAddress != address(0), "MFCCollateralTreasury address not set");
    _;
  }
  ...
  function startMigrationProposal(
    ...
    require(
      busdtDestination != address(0) &&
        mfcTradingLicenseDestination != address(0) &&
        mfcExchangeCapDestination != address(0) &&
        mfcCollateralTreasuryDestination != address(0),
      "Invalid address"
    );
    ...
    _migrations[++_migrationNonce] = MigrationProposal(
      busdtDestination,
      mfcTradingLicenseDestination,
      mfcExchangeCapDestination,
      mfcCollateralTreasuryDestination,
      endsAt
    );
  ...
  function executeMigrationProposal() external hasMigrationAddresses hasProposal onlyAdminAgents {
    ...
    // execute MFCTradingLicense migration
    MFCTradingLicense _mfcTradingLicense = MFCTradingLicense(_mfcTradingLicenseAddress);
    _mfcTradingLicense.setMigration(_migrations[_migrationNonce].mfcTradingLicenseDestination);
    ...
    // execute MFCCollateralTreasury migration
    MFCCollateralTreasury _mfcCollateralTreasury = MFCCollateralTreasury(_mfcCollateralTreasuryAddress);
    _mfcCollateralTreasury.setMigration(_migrations[_migrationNonce].mfcCollateralTreasuryDestination);
  }
  ...
  function updateAddresses() public override onlyRegistrar {
    _mfc = MFCToken(_registrar.getMFCToken());
    _busdTreasuryAddress = _registrar.getBUSDT();
    _mfcTradingLicenseAddress = _registrar.getMFCMembership();
    _mfcExchangeCapAddress = _registrar.getMFCExchangeCap();
    _mfcCollateralTreasuryAddress = _registrar.getMFCCollateralLoan();
  }
}
