...
contract CoinFlip is Ownable, ReentrancyGuard {
    ...
    constructor(address _apple, address _devWallet) {
        Apple = _apple;
    ...
    function viewAppleBalance() external view returns (uint256) {
    ...
    function flipCoinAndMakeDisbursable(uint256 _sessionId) external returns (uint256) {
        ...
        _sessions[currentSessionId].appleForDisbursal = (
            ((_sessions[_sessionId].collectedApple) * (10000 - _sessions[_sessionId].devFee))) / 10000;
    ...
    function autoInject(uint256 _sessionId, uint256 _amount) internal {
        ...
        _sessions[_sessionId].collectedApple += _amount;
