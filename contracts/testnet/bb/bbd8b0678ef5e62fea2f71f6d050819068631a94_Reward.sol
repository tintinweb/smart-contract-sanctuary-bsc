...
contract Reward {
    ...
    function claimReward(uint tokenId, uint startEpoch, uint endEpoch) public returns (uint reward) {
        ...
        for (uint i = startEpoch; i <= endEpoch; i++) {
            ...
            checkEpoch(i);
            (uint reward_i, bool finished) = _pendingRewardSingle(tokenId, userLastClaimTime[tokenId][i], epoch);
            if (reward_i > 0) {
                reward += reward_i;
        ...
        if (startEpoch <= userFirstClaimable[tokenId]) {
            ...
            userFirstClaimable[tokenId] = firstUnclaimable;
        }
