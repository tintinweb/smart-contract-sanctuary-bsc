...
struct SwapData {
   address curToken; // token of the current network
   string desToken; // corresponding token of other network
   address curUser; //address of the current network
   string desUser; // corresponding address of other network
   address signer;
   uint256 amount;
   string side;
}
...
contract Monstera is Initializable, UUPSUpgradeable, OwnableUpgradeable, SignatureUtils {
   /**
   ...
   */
   ...
   mapping(string => bool) public desAcceptedTokens; // accepted tokens of the destination chain 
   ...
   function setDesAcceptedToken(string memory token_, bool status_) external onlyAdmins{
      desAcceptedTokens[token_] = status_;
   }
   /* ====================== MAIN FUNCTIONS ====================== */
   
   /**
    * @dev user will lock their accepted token0 in order to recevie des accepted token1 from the destination chain
    */
   function lock(
   ...
   /**
    * @dev prerequisite: user lock/burn their des accepted token1 in the coressponding contract in the destiantion chain. -> to get signature and unlock token0 in this chain
    */
   function unlock(
      string memory transactionId_,
      address curToken_,  
   ...
   function _validateSwapData(
      ...
      require(acceptedTokens[token0_] ==true, "Lock: Invalid cur network token");
      require(desAcceptedTokens[token1_] == true, "Lock: Invalid des network token");
   }
...
contract SignatureUtils {
    ...
    {
        bytes32 messageHash = getMessageHash(transactionId_, token0_, token1_, user0_, user1_, amount_);
