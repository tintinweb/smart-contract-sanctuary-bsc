...
contract Box is Ownable{
    ...
    Game public  _game;
    BADHero _badHero = BADHero(0xCDdB3Df2ecEa4A23ddf36644B82920677be3FFB2);
    GetFee public getFeeRate = GetFee(0x62b3681D7d8C9c2C2D490a5785E7D0aE12864201);
    ...
    struct monsterInfo{
     ...
     // 监听事件
    event BuyBox(uint32 index,uint256 price);
    event OpenBox(uint256 indexed rarity,uint256 indexed tokenId,uint256 monsterId);
    // 买盲盒
    function buyBox()  public {
        ...
        _boxId +=1;
        _badHero.initHeroEq(msg.sender);
        emit BuyBox(1,uint256(amount));
    }
    ...
    function openBox(uint32 _index)  public {
        ...
        (nftKindId,monsterId,_monster.ce,_monster.armor,_monster.luk,_monster.name) = _badHero.getMonsterType();
        ...
        emit OpenBox(nftKindId, tokenId,monsterId);
    }
    ...
    function getBoxPrice() public view returns(uint[] memory amounts){
        amounts = getFeeRate.getUsdtPrice(_boxPrice);
        return amounts;
    }
    ...
    function setBADHero(address _tokenAddress)public onlyOwner{
        _badHero = BADHero(_tokenAddress);
    }
