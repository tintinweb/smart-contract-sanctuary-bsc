...
contract RatelNFT is ERC721A, VRFConsumerBase, Ownable, ReentrancyGuard {
  ...
  uint internal chainlinkFee = 0.1 * 10**18; // 0.1 LINK token
  ...
  */
  ...
  function _generateCharacters(MintMode mode, uint level, uint babyId) internal returns(uint){
    uint newId = characters.length + 1; //从1开始 
    uint base = 40;
    uint generation =1;
    uint gender=0;
    /*
    ...
    if (mode == MintMode.BREED) {
      ...
      randomResult = uint(keccak256(abi.encode(tokenIdToSeed[babies[babyId].father_tokenId], tokenIdToSeed[babies[babyId].mother_tokenId])));
        ...
        if ((newId % level2_rand == 0 && count_level_2 < 950) || (level == 2)) {
              base = 60;
              count_level_2 +=1;
              level2_rand = 0; //重新随机
        }

        if (( newId % level3_rand == 0 && count_level_3 < 50 ) || (level == 3)) {
              base = 80;
              count_level_3 +=1;
              level3_rand = 0; //重新随机
        }
...
*/
    ...
    uint strength =  base ;// 力量
    uint dexterity = base ; //敏捷
    uint luck = base ;     //运气
    uint defense =  base ; //防御
    uint wisdom = base ;  //智慧
    uint attack =  base ;   //攻击
    ...
    // mintable(_quantity) 
  {
    ...
    //   require(_quantity<=2, "Max two can mint");
...
//   }
  ...
  function getRatelListByOwner(address owner) public view returns (uint[] memory) {
      ...
      uint counter = 0;
      for (uint i = 1; i < characters.length + 1; i++) {
          if (ownerOf(i) == owner) {
              result[counter] = i;
              counter++;
          }
  ...
  function getOwnerListByTokenId(uint startTokenId, uint endTokenId) external view returns (address[] memory) {
