...
// File: contracts/Gun_Falttern_v1_buy_n_guns.sol
...
// File: contracts/Fishgun_Gun_ERC721.sol
...
contract GUN_GunFishCrypto is ERC721{
    ...
    uint public price_token_per_gun=1000;
    ...
    uint[] price_upgrade_gun_type0 = [1000*10**18,2000*10**18,3000*10**18,4000*10**18,5000*10**18];
    uint[] price_upgrade_gun_type1 = [1100*10**18,2100*10**18,3100*10**18,4100*10**18,5100*10**18];
    uint[] price_upgrade_gun_type2 = [1200*10**18,2200*10**18,3200*10**18,4200*10**18,5200*10**18];
    uint[] price_upgrade_gun_type3 = [1300*10**18,2300*10**18,3300*10**18,4300*10**18,5300*10**18];
    ...
    function unBoxGun(address _player, uint box_tokenId) public{
        require(msg.sender==boxAddress, "[1] You are not allowed to mint guns");
        require(boxNFT.ownerOf(box_tokenId)==_player, "[2] Wrong token ID owner");
        uint gunAmount = boxNFT.getBoxInfo2(box_tokenId);
        if(gunAmount>0){
            for(uint count=1; count<=gunAmount; count++){
                _tokenIds.increment();
                uint256 newItemId = _tokenIds.current();
                _mint(_player, newItemId);
                Guns.push(GunInfo(_player, newItemId, 1));
            }
        }
    }

    function upgradeGun(uint tokenId_Gun_1, uint tokenId_Gun_2, uint tokenId_Gun_3) public{

        require(ownerOf(tokenId_Gun_1)==msg.sender && ownerOf(tokenId_Gun_2)==msg.sender && ownerOf(tokenId_Gun_3)==msg.sender, "[2] Wrong token ID owner");
        
        uint level_gun1;
        uint level_gun2;
        uint level_gun3;
        for(uint count=0; count<Guns.length; count++){
            if(Guns[count].gunTokenId==tokenId_Gun_1){
                level_gun1 = Guns[count].level;
            }
            if(Guns[count].gunTokenId==tokenId_Gun_2){
                level_gun2 = Guns[count].level;
            }
            if(Guns[count].gunTokenId==tokenId_Gun_3){
                level_gun3 = Guns[count].level;
            }
        }

        require(level_gun1==level_gun2 && level_gun1==level_gun3 && level_gun1>=1 && level_gun1<=4, "[3] Wrong guns resources.");
        ...
        level_gun1 = level_gun1 + 1;
        Guns.push(GunInfo(msg.sender, newItemId, level_gun1));
    }
    ...
    function client_upgradeGun(uint tokenIdGun_1, uint tokenIdGun_2, uint tokenIdGun_3, uint _type, uint tokenAmountPos) public returns(address, uint, uint, uint, uint, uint, uint){
        require(_type<=3, "Wrong Type"); // 0 to 4
        
        if(_type==0){
            require(tokenAmountPos<price_upgrade_gun_type0.length, "[-3]Wrong token amount");
            ...
            return(msg.sender, tokenIdGun_1, tokenIdGun_2, tokenIdGun_3, _type, tokenAmountPos, price_upgrade_gun_type0[tokenAmountPos] );
        }
        
        if(_type==1){
            require(tokenAmountPos<price_upgrade_gun_type1.length, "[-3]Wrong token amount");
            ...
            return(msg.sender, tokenIdGun_1, tokenIdGun_2, tokenIdGun_3, _type, tokenAmountPos, price_upgrade_gun_type1[tokenAmountPos] );
        }

        if(_type==2){
            require(tokenAmountPos<price_upgrade_gun_type2.length, "[-3]Wrong token amount");
            ...
            return(msg.sender, tokenIdGun_1, tokenIdGun_2, tokenIdGun_3, _type, tokenAmountPos, price_upgrade_gun_type2[tokenAmountPos] );
        }

        if(_type==3){
            require(tokenAmountPos<price_upgrade_gun_type3.length, "[-3]Wrong token amount");
            ...
            return(msg.sender, tokenIdGun_1, tokenIdGun_2, tokenIdGun_3, _type, tokenAmountPos, price_upgrade_gun_type3[tokenAmountPos] );
        }
