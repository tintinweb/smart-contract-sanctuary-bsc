...
library Uint256Array {
	...
	function SubItem(uint256[] memory _Array) internal view returns (uint256[] memory _Return) {
		uint256 count;
		for (uint256 i = 0; i < _Array.length; i++) {
			if (_Array[i] != 0) count++;
			else break;
...
library SmartMatrix {
	...
	function SelectAllChildrenNodeIDInNode(
		...
		uint256[] memory a = new uint256[](3);//(type(uint256).max);
...
contract Tuktu is TuktuBase, ITuktu {
	...
	function TestSelectAllNode(
		uint256 _NodeID,
		uint8 _Matrix
	) public view returns (uint256[] memory _NodeIDs, uint256 _NodeCount, uint256[] memory _Test, uint256 _TestLength) {
		(_NodeIDs, _NodeCount) = SmartMatrix.SelectAllChildrenNodeIDInNode(Nodes[_NodeID], Nodes, _Matrix);
		_Test = _NodeIDs.SubItem();
