...
contract NDBReferral is INDBReferral, Ownable {
    ...
    function updateReferrer(address _old, address _current) external onlyOperator {
        require(_old != address(0), "new referrer is the zero address");
        require(_current != address(0), "current referrer is the zero address");
        address[] memory listUser = referredUsers[_old];
        referredUsers[_current] = listUser;

        for (uint i =0;i < referralsCount[_old];i++)
        {
            referrers[listUser[i]] = _current;    
        }
        referralsCount[_current]= referralsCount[_old];
