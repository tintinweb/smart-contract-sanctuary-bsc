...
////////////////////////////////////////// IOracleHistory
    ...
    function consultByHistoryTx(address token, uint256 timeFrom, uint256 timeTo)
        public
        returns(uint256)
    {
        return _consultByHistory(token, timeFrom, timeTo);
...
////////////////////////////////////////// methods for read oracle history
    ...
    function _consultByHistory(address _token, uint256 _timeBegin, uint256 _timeEnd) internal view returns (uint256) {
        ...
        (uint256 _cumulativePrice, uint256 _cumulativeTime) = __getCumulativePriceAndTimeBySegments(
        ...
        if (lastSegment == historiesCount - 1) {
            ...
            uint256 _priceEnd = __getTokenPriceInUSDT(_token);
            // Get the start period for the last segment
            (uint256 _periodBegin,) = oracleHistory[_token].at(lastSegment);
            // Calculate the period between the start of the last segment and the end time
            uint256 period = _timeEnd - _periodBegin;
            ...
            (uint256 _periodBegin, ) = oracleHistory[_token].at(lastSegment);
            (, uint256 _priceEnd) = oracleHistory[_token].at(lastSegment + 1);
            // Calculate the period between the start of the last segment and the end time
            uint256 period = _timeEnd - _periodBegin ;
        ...
        (uint256 _periodEnd, uint256 _priceEnd) = oracleHistory[_token].at(firstSegment);
        // Calculate the period between the start time and the end of the first segment
        uint256 period = _periodEnd - _timeBegin;
    ...
    function __getCumulativePriceAndTimeBySegments(
    ...
    function showOracleHistory(address _token) public view returns (uint256[] memory blockTimestamps, uint256[] memory prices) {
        uint256 historiesCount = oracleHistory[_token].length();
        uint256[] memory blockTimestamps = new uint256[](historiesCount);
        uint256[] memory prices = new uint256[](historiesCount);
