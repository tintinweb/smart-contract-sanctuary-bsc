...
contract ERC20 {
    ...
    uint256 public rewardRate = 20; //奖励比例 20 / 1000 2%
    ...
    address public upgradeAddress = 0x0000000000000000000000000000000000000000; //升级合约地址
    ...
    function upgrade() public onlyOwner {
        ...
        uint256 _amount = IERC20(upgradeAddress).balanceOf(msg.sender);
    ...
    function unstake(uint256 amount) public virtual returns (bool) {
        require(
            amount > 0 && stakedOf[msg.sender] >= amount,
            "Staking: out of staked"
        );
        //自动领取上一次的收益
        _reward();
        stakedOf[msg.sender] -= amount;
        ...
        returns (uint256)
    {
        if (block.timestamp > rewardAt[_account] + rewardDuration) {
            ...
            uint256 baseReward = (stakedOf[_account] * rewardRate) / 1000;
