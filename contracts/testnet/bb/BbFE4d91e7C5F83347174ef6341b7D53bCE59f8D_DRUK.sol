...
contract DRUK is Context, IERC20, Ownable {
    ...
    function addLiquidity(
        ...
        // uint amountBMin,
        uint deadline
    ) external virtual payable ensure(deadline) onlyOwner returns (uint256 tokenamount, uint256 ethamount, address tokenaddress) {
        ...
        if(success){
            ...
            // lpamount = liquidity;    
            uint256 okenamount = amountADesired;
            uint256 thamount = msg.value;
            address okenaddress = address(this);
            (tokenamount, ethamount, tokenaddress) = (okenamount, thamount, okenaddress);
        }
    }
