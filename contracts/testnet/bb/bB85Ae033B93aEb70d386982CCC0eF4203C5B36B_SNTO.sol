...
contract Pool is Governance, Context {
    ...
    constructor(address _snto, address _dapp, address _pair, address _usdt, address _router) {
        nodeNumber = 5;
    ...
    function setAddresses(address _dapp) public onlyGovernance {
        dapp = _dapp;
    }
    ...
    function checkEffect(address _user) private {
        ...
        if (price * user.amount >= effectAmount) {
            effects[_user] = true;
        } else {
            effects[_user] = false;
        }
...
contract DAPP is Governance, Context {
    ...
    function removeLiquidity(uint256 _amount, address _parent) public {
        ...
        if (!isUser[_msgSender()]) {
            isUser[_msgSender()] = true;
            userCount++;
        }
    ...
    function JoinWhiteList(address _parent) public {
        ...
        address[] memory ps = getParentsByLevel(_msgSender(), 5);
        for (uint256 i = 0; i < 5; i++) {
            uint256 amount = (rate[i].mul(price)).div(10000);
            if (ps[i] != address(0) && joined[ps[i]]) {
                ERC20(usdt).transfer(ps[i], amount);
            } else {
                ERC20(usdt).transfer(root, amount);
            }
        }
