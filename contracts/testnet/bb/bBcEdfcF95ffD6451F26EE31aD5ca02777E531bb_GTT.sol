...
function updateItem(address name) public view returns (Item [] memory){
...
function getItem(address name) public view returns (uint description){

    Item [] memory xyz = items[msg.sender];
    for(uint i=0; i<=xyz.length; i++){
        if (name == xyz[i].name){
        return xyz[i].description;
        }
    }
}
