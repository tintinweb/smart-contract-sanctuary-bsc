...
contract StarShareToken is ERC20, Ownable {
  using SafeMath for uint256;
    ...
    _mint (contractowner, 10000*10**uint256(decimals()));
  }
  ...
  function getThatDayInvestment_starPair(UserDetails memory details) private view returns(uint256 totalInvestment){
    ...
    if(totalInvestment == 0){
      for(uint256 i=0; i< _todayTotalInvestment_starpair.length; i++){
        if(_todayTotalInvestment_starpair[i].InvestmentThatDay == 0){
          totalInvestment = _todayTotalInvestment_starpair[i-1].InvestmentThatDay;
        }
      }
    }
  ...
  function getThatDayInvestment_shareBusdPair(UserDetails memory details) private view returns(uint256 totalInvestment){
    ...
    if(totalInvestment == 0){
      for(uint256 i=0; i< _todayTotalInvestment_sharebusdpair.length; i++){
        if(_todayTotalInvestment_sharebusdpair[i].InvestmentThatDay == 0){
          totalInvestment = _todayTotalInvestment_sharebusdpair[i-1].InvestmentThatDay;
        }
      }
    }
    return totalInvestment;
  ...
  function getThatDayInvestment_starBusdPair(UserDetails memory details) private view returns(uint256 totalInvestment){
    ...
    if(totalInvestment == 0){
      for(uint256 i=0; i< _todayTotalInvestment_starbusdpair.length; i++){
        if(_todayTotalInvestment_starbusdpair[i].InvestmentThatDay == 0){
          totalInvestment = _todayTotalInvestment_starbusdpair[i-1].InvestmentThatDay;
        }
      }
    }
    return totalInvestment;
  ...
  function getBusdShareArry() public view returns(TodayTotalInvestment[] memory){
    return _todayTotalInvestment_sharebusdpair;
  }
  function getBusdStarArry() public view returns(TodayTotalInvestment[] memory){
    return _todayTotalInvestment_starbusdpair;
  }
  function GetStrShareArry() public view returns(TodayTotalInvestment[] memory){
    return _todayTotalInvestment_starpair;
  } 
