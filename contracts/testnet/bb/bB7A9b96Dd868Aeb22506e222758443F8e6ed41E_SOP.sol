...
contract SOP is IERC20,Ownable{
    ...
    event AuctionTransfer(uint256 id,uint256 amount);
    ...
    function auctionTransfer(uint256 id,address recipient,uint256 amount) public returns (bool) {
        _transfer(msg.sender, recipient, amount.mul(100-_fee).div(100));
        _transfer(msg.sender, _master, amount.mul(_fee).div(100));
        emit AuctionTransfer(id,amount);
        return true;
    }
