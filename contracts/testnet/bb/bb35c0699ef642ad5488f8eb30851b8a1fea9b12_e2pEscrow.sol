...
contract e2pEscrow is Stoppable, SafeMath {
  ...
  uint public commissionFeePercentage = 2000000000000000;
    ...
    returns(bool)
  {
    ...
    require(msg.value > 0);
    uint commissionFee = getCommissionFeePercentage(msg.value);
   ...
   * @return True if success.
   */
  function ChangeCommissionFeePercentage(uint _newCommissionFeePercentage)
                          public
                          whenNotPaused
                          whenNotStopped
                          onlyOwner
   ...
   */
  function getCommissionFeePercentage(uint _amount)
            internal
            constant
    returns (uint amount) 
  {
        uint commissionFee = safeDiv((safeDiv(safeMul(safeMul(_amount,10**decimal),commissionFeePercentage),safeMul(100,10**decimal))),10**decimal);
        return (commissionFee);
  }
