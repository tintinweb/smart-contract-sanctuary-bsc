...
string constant ERR_RECOVERED_MSG_SENDER = "err_recovered_msg_sender";

// Signed message hash does not match hash created in smart contract
string constant ERR_INVALID_MESSAGE_HASH = "err_invalid_message_hash";

// Message signed by non whitelisted address
string constant ERR_SIGNER_NOT_WHITELISTED = "err_signer_not_whitelisted";

// Method is trying to transfer 0 tokens
string constant ERR_TOKEN_ZERO_VALUE = "err_token_zero_value";

// Lower payment limit not reached
string constant ERR_PAYMENT_TOO_LOW = "err_payment_too_low";

// Upper payment limit reached
string constant ERR_PAYMENT_TOO_HIGH = "err_payment_too_high";

// Contract tokens limit would be exceeded if assigned to user
string constant ERR_NO_AVAILABLE_TOKENS = "err_no_available_tokens";

// Method is not available in current sale stage
string constant ERR_INVALID_SALE_STATUS = "err_invalid_sale_status";

// User has 0 locked tokens
string constant ERR_ZERO_LOCKED_VALUE = "err_zero_locked_value";

// User has 0 tokens to unlock with current unlocked percent
string constant ERR_NOTHING_TO_CLAIM = "err_nothing_to_claim";

// User needs to claim all tokens before calling this
string constant ERR_LOCKED_TOKENS_NOT_WITHDRAWN = "err_locked_tokens_not_withdrawn";

// User has never bought any tokens
string constant ERR_NO_BALANCE_EXISTS = "err_no_balance_exists";

// User has no bonus to withdraw
string constant ERR_NO_BONUS_APPLIED = "err_no_bonus_applied";

// User already claimed bonus
string constant ERR_BONUS_ALREADY_CLAIMED = "err_bonus_already_claimed";
...
contract PrivateSale is Ownable {
  ...
  constructor(PrivateSaleArgs memory args) {
    SaleData memory sale;

    sale.status = SaleStatus.BUY;
    sale.locked = 0;
    sale.unlockedPercent = 0;
    sale.sigVerifierEnabled = true;

    sale.token = args.token;
    sale.usd = args.usd;
    sale.limit = args.limit;
    sale.rate = args.rate;
    sale.minPayment = args.minPayment;
    sale.maxPayment = args.maxPayment;
  ...
  // Add bonuses
  function addBonuses(Bonus[] memory bonuses) external onlyOwner {
    _copyBonusesToStorage(bonuses);
  }

  // Remove bonuses by their indices
  function removeBonuses(uint256[] memory indices) external onlyOwner {
  ...
  function _validateBuyOrder(
    ...
    require(usdValue >= _sale.minPayment, ERR_PAYMENT_TOO_LOW);
    require(usdValue <= _sale.maxPayment, ERR_PAYMENT_TOO_HIGH);

    uint256 tokenValue = _calculateTokenValue(usdValue, _sale.rate, _sale.usd.decimals());
  
    require(tokenValue > 0, ERR_TOKEN_ZERO_VALUE);

    uint256 withTip = _applyTip(tokenValue, signer);

    require(_availableTokens() >= withTip, ERR_NO_AVAILABLE_TOKENS);
  ...
  function _validateClaimBonusOrder() internal view returns (uint256) {
    ...
    require(user.maxBalance != 0, ERR_NO_BALANCE_EXISTS);
    require(user.balance == 0, ERR_LOCKED_TOKENS_NOT_WITHDRAWN);
    require(!user.bonusClaimed, ERR_BONUS_ALREADY_CLAIMED);

    uint256 bonus = _getBonusForUser(user);
    require(bonus > 0, ERR_NO_BONUS_APPLIED);
  ...
  function _removeBonusesFromStorage(uint256[] memory indices) internal {
  ...
  function _verifySignature(bytes32 hash, bytes memory sig) internal view returns (address) {
    if (!_sale.sigVerifierEnabled) return address(0);

    (address recovered, ECDSA.RecoverError err) = ECDSA.tryRecover(hash, sig);
    require(err == ECDSA.RecoverError.NoError, ERR_INVALID_SIGNATURE);
    require(recovered != msg.sender, ERR_RECOVERED_MSG_SENDER);

    bytes32 messageHash = ECDSA.toEthSignedMessageHash(abi.encodePacked(msg.sender.toHexString()));
    require(messageHash == hash, ERR_INVALID_MESSAGE_HASH);

    require(_whitelist[recovered], ERR_SIGNER_NOT_WHITELISTED);

    return recovered;
  }

  // Whitelist signer address
  function _addToWhitelist(address[] memory list) internal {
  ...
  function _removeFromWhitelist(address[] memory list) internal {
    for (uint256 i = 0; i < list.length; i++) {
  ...
  modifier onlyWhen(SaleStatus status) {
    require(_sale.status == status, ERR_INVALID_SALE_STATUS);
    _;
  }
  ...
  modifier onlyWhenOneOf(SaleStatus option1, SaleStatus option2) {
    ...
    require(current == option1 || current == option2, ERR_INVALID_SALE_STATUS);
    _;
  }
}