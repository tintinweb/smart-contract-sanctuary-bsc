    ...
    PriceControl
{
        ...
        onlyOwner
    {
        ...
        //QuantitiesByTier memory quantitiesByTier = quantitiesByTierMap[tier];
    }
    ...
    function _objectsGeneration(uint8 tier) internal {
        ...
        for (uint256 i = 0; i < objects.length; i++) {
            SharedStructs.ItemInfo memory itemInfo = itemInfoByTier[objects[i]][
                tier
            ];
            IObject(objects[i]).randomMint(msg.sender, tier, itemInfo);
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _singleObjectsGeneration(uint8 tier) internal {
        /*
        address[] memory objects = objectsByTier[tier];
        for (uint256 i = 0; i < objects.length; i++) {
            SharedStructs.ItemInfo memory itemInfo = itemInfoByTier[objects[i]][
    ...
    /// @notice This functions generate the objects, in this case a set of ERC721 tokens
    /// @param tier The tier of the purchase
    function _batchObjectsGeneration(uint8 tier) internal {
        // TODO
    }
...
interface IObject is IERC721Upgradeable {
    function randomMint(
        address to,
        uint8 tier,
        SharedStructs.ItemInfo memory _tier
    ) external;

    function lootBoxMintSpaceShip(
        address to,
        string memory uri,
        ClanSpaceshipInfo memory _clanSpaceshipInfoMap
    ) external;
}
