...
contract MetaStaking is Ownable {
        ...
        returns (bool)
    {
        ...
        warmupInfo[_recipient] = Claim({
            ...
            gons: info.gons.add(IMeta(sMETA).gonsForBalance(_amount)),
    ...
    function claim(address _recipient) public {
        ...
        if (epoch.number >= info.expiry && info.expiry != 0) {
            ...
            IWarmup(warmupContract).retrieve(
                _recipient,
                IMeta(sMETA).balanceForGons(info.gons)
            );
        }
    }
    ...
    function rebase() public {
        if (epoch.endTime <= uint32(block.timestamp)) {
            IMeta(sMETA).rebase(epoch.distribute, epoch.number);
            ...
            uint256 staked = IMeta(sMETA).circulatingSupply();
