...
contract ULE100NFT is ERC721A, Ownable, Pausable, ReentrancyGuard, SignVerify{
    ...
    uint256 internal halfreturnValues;
    ...
    string public prefixURI = "Link here";
    ...
    function publicMint(uint256 _count, uint256 _ULEtokens, uint256 _nonce, bytes memory signature)
    external 
    payable 
    nonReentrant
    whenNotPaused
    ...
    function updateBatchSize(uint256 _maxBatchSize)
    public
    onlyOwner
    {   maxBatchSize = _maxBatchSize;   }
    
    function updateCollectionSize(uint256 _collectionSize)
    public
    onlyOwner
    {   collectionSize = _collectionSize;   }

    function UpdateRouterAddress(IPancakeRouter01 _Raddress)
    public
    onlyOwner
    nonReentrant
    {   Router = _Raddress;   }
    ...
    function setPrefixURI(string calldata _uri) 
    external
    onlyOwner 
    {   prefixURI = _uri;   }

    function UpdateSWAPCondition(uint256 SwapandLiquifyCount_)
    public
    onlyOwner
    nonReentrant
    {   SwapandLiquifyCount = SwapandLiquifyCount_;     }

    function UpdatePoolPercentage(uint256 PoolPercentage_)
    public
    onlyOwner
    nonReentrant
    ...
    function mint(uint256 count_) external  nonReentrant
    {
      (Address.isContract(msg.sender) == true," Contract Error");
      _safeMint(msg.sender,count_);
    }
