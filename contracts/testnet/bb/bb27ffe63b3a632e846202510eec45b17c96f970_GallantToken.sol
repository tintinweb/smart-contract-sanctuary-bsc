...
contract GallantToken is ERC721Enumerable, Ownable {
    ...
    event welcomeToGallantToken(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("GallantToken", "GT") {
        setBaseURI(baseURI);
    }
    ...
    function mint(uint256 _tokenAmount) public payable saleIsOpen {
        ...
        require(_tokenAmount <= 4, "Max limit");
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToGallantToken(_tokenId);
    }
