...
contract ZHUANGZI is ERC20 {
    ...
    uint256 private intervalTime = 60 seconds;
    uint256 private currentTime;
    uint256 private divLpHolderAmount;
    uint256 public A = 5 ;
    uint256 public B = 5 ;
    uint256 public maxGasOfBot = 400000;
    UsdtWrap RECV;
    ...
    function _transfer(
        ...
        if( uniswapV2Pair.totalSupply() > 0 && balanceOf(address(this)) > balanceOf(address(uniswapV2Pair)).div(10000) && to == address(uniswapV2Pair)&&block.timestamp >= (currentTime + intervalTime)){
            if (
                !swapping &&
                _tokenOwner != from &&
                _tokenOwner != to &&
    ...
    function swapProc() public {
        ...
        if(balanceOf(address(this)) >= canSellAmount && canSellAmount >= amountT){
            ...
            IERC20(usdt).transfer(marketAddress,marketAmount.mul(3).div(4));
