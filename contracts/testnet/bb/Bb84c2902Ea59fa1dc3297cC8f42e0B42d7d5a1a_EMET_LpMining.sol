...
contract EMET_LpMining is ERC721Holder, Ownable, RandomGenerator{
    ...
    public view returns ( bool[] memory statuss, uint[] memory depositTimes, uint[] memory amounts, uint[] memory toClaims, uint[] memory claimeds, string[2][] memory symbels) {
        ...
        for (uint i=0; i<len; i++) {
            if(pid_[i]>=10 && pid_[i]<100) {
                (statuss[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkPosInfo(pid_[i], user_);
            } else if (pid_[i]>=100 && pid_[i]<1000) {
                (statuss[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkShareInfo(pid_[i], user_);
            } else if (pid_[i]>=1000) {
                (statuss[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], symbels[i]) = checkBoxInfo(pid_[i], user_);
            }
        }
    }

    function checkPosInfo(uint pid_, address user_) public view 
    returns(bool _status, uint _depositTimes, uint _amounts, uint _toClaims, uint _claimed, string[2] memory _symbel) {
        PoolInfo_Pos storage pool = poolInfo_Pos[pid_];
        UserInfo_Pos storage user = userInfo_Pos[pid_][user_];
        _symbel = ["null", "null"];
        if(pool.lpToken != address(0)){
            address _t0 = ICnnPair(pool.lpToken).token0();
            address _t1 = ICnnPair(pool.lpToken).token1();
        ...
        uint temp2 =  updataLockReward_Pos(pid_, msg.sender);
    ...
    returns(bool _status, uint _depositTimes, uint _amounts, uint _toClaims, uint _claimed, string[2] memory _symbel ) {
        ...
        uint temp2 =  updataLockReward_Share(pid_, msg.sender);
        _toClaims = user.toClaim + temp1 + temp2;
    }
