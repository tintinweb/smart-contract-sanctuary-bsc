...
contract LiquidityGeneratorFactory is AccessControl, Pausable {
    ...
    function createToken(string memory tokenName, string memory tokenSymbol, uint8 tokenDecimal, uint256 totalSupply, address marketingAddress, address router) external onlyRole(OPERATOR_ROLE) whenNotPaused {
        address token = Clones.clone(implementation);
        ILiquidityGeneratorToken(token).initialize(msg.sender, tokenName, tokenSymbol, tokenDecimal, totalSupply, marketingAddress, router);
