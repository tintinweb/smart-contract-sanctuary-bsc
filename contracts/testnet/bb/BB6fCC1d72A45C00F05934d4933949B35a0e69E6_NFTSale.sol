    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier isTransferAllowed(uint256 _tokenId) {
        require(status[_tokenId] || msg.sender == owner, "access not allowed");
        _;
    }
    ...
    event AirdropNFT(address[] to, uint256 mintAmount);
        ...
        isLimitAchieve(_mintAmount)
    {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            tokenID.increment();
            _safeMint(_to[i], tokenID.current());
            tokenMetadataCID[tokenID.current()] = _uri[i - 1];
            status[tokenID.current()] = false;
        }

        emit AirdropNFT(_to, _mintAmount);
    }
