...
contract HoksageMatrix is Ownable {
  ...
  function getUpline(address account,uint256 level,uint256 layer) public view returns (address) {
  ...
  function findClosePart(address account,uint256 level) public view returns (address) {
    if(account==address(0)){ return address(0); }
    address host = getUpline(account,level,3);
    uint256 currentcycle = users[host].matrix[level].reinvestcount;
