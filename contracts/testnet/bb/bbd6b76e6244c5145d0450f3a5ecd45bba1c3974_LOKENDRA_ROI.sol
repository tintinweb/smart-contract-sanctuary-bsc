...
contract LOKENDRA_ROI {
   ...
   function getNextReward() public view returns (uint) {
    ...
    function Registration(uint _referrerID, uint256 REGESTRATION_FESS) public payable {
        require(!users[msg.sender].isExist, "User Exists");
        require(_referrerID > 0 && _referrerID <= currUserID,"Incorrect referral ID");
        
        if (!isTokenAcceptance) {
            if(msg.value >0){
            require(stableCoin.allowance(msg.sender, address(this)) >= REGESTRATION_FESS,"NEED_TO_APPROVE_TOKEN"); 
            stableCoin.transferFrom(msg.sender, address(this), REGESTRATION_FESS);
            tokenReward =  REGESTRATION_FESS / tokenPrice() * 1e18;
            }
        } else {        
            require(REGESTRATION_FESS == REGESTRATION_FESS, "Incorrect Value");
            require(token.allowance(msg.sender, address(this)) >= REGESTRATION_FESS,"NEED_TO_APPROVE_TOKEN");
            token.transferFrom(msg.sender, address(this), REGESTRATION_FESS);
            tokenReward = REGESTRATION_FESS / tokenPrice() * 1e18;
        }
