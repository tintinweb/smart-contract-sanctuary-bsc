...
contract SafePet is AccountChangable, Utils {
  ...
  uint256 FACTOR = 1e9;
  address public TOKEN_CONTRACT_ADDRESS = EMPTY_ADDRESS;
  ...
  AnimTransaction[] public animTransactions;
  ...
  function getNewRank(uint256 revenue, bool invested) internal view returns (uint256) {
  ...
  function upgrade(uint256 level, uint256 numberOfAnims) public mustBeActiveInvestor {
    ...
    AnimTransaction memory animTransaction = AnimTransaction(investorAddress, level, decreaseCount, ANIM_CHANGE_REASON_UPGRADE_DECREASE);
    animTransactions.push(animTransaction);
    ...
    if (successCount > 0) {
      ...
      animTransaction = AnimTransaction(
        investorAddress,
        level.add(1),
        successCount,
        ANIM_CHANGE_REASON_UPGRADE_INCREASE
  ...
  function transferAnim(
    ...
    AnimTransaction memory animTransaction = AnimTransaction(from, level, numberOfAnims, ANIM_CHANGE_REASON_SEND_TRANSFER);
    animTransactions.push(animTransaction);
    increaseAnim(to, level, numberOfAnims);
    emit AnimChange(to, level, numberOfAnims, ANIM_CHANGE_REASON_RECEIVE_TRANSFER);
    animTransaction = AnimTransaction(to, level, numberOfAnims, ANIM_CHANGE_REASON_RECEIVE_TRANSFER);
    animTransactions.push(animTransaction);
  }
  ...
  function sellAnim(uint256 level, uint256 numberOfAnims) public mustBeActiveInvestor {
    ...
    AnimTransaction memory animTransaction = AnimTransaction(investorAddress, level, numberOfAnims, ANIM_CHANGE_REASON_SELL);
    animTransactions.push(animTransaction);
    pay(fundAdmin, fee);
  ...
  function getToTalAnimTransactions() public view returns (uint256) {
    return animTransactions.length;
  }

  function getAnimTransactions(uint256 fromIndex, uint256 toIndex) public view returns (AnimTransaction[] memory) {
    uint256 length = animTransactions.length;
    AnimTransaction[] memory emptyResponse = new AnimTransaction[](0);
    ...
    AnimTransaction[] memory result = new AnimTransaction[](normalizedToIndex - fromIndex + 1);
    for (uint256 index = fromIndex; index <= normalizedToIndex; index++) {
      result[index - fromIndex] = animTransactions[index];
    }
    return result;
  }
}
