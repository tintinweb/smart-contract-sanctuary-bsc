...
contract StockTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("StockTest_Dividend_Tracker", "StockTest_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**18); //must hold 1000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "StockTest_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "StockTest_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main StockTest contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "StockTest_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");   
        require(newClaimWait != claimWait, "StockTest_Dividend_Tracker: Cannot update claimWait to same value");
...
contract StockTest is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    StockTestDividendTracker public dividendTracker;
    ...
    constructor() BEP20("StockTest", "STT") 
    {
        BNBRewardsFee = 5;
        liquidityFee = 2;
        marketingFee = 3;
        ...
        marketingWallet = payable(0x56180a278B8801Ba32bd3E8f4b8ea641516d6ce5);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

        dividendTracker = new StockTestDividendTracker();
    ...
    {
        require(newAddress != address(uniswapV2Router), "StockTest: The router already has that address");
    ...
    {
        require(_isExcludedFromFees[account] != excluded, "StockTest: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "StockTest: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "StockTest: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "StockTest: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "StockTest: Cannot update gasForProcessing to same value");
