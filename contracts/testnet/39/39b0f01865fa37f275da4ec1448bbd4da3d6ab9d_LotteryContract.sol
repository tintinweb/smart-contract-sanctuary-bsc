...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    {
        Lottery storage lottery = lotteries[_lotteryId];
        winners = new address[](lottery.participants.length);
        for (uint256 k = 0; k < lottery.noOfWinner; k++) {
            uint256 winnerIndex = uint256(keccak256(abi.encode(random[_lotteryId].randomValue, k))) % lottery.participants.length;
            address winnerAddr = lottery.participants[winnerIndex];
            winners[k] = winnerAddr;
