...
contract OvalRegularPledge is Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    // mapping(uint256 => uint256) private poolMap;
    // mapping(uint256 => mapping(address => uint256)) private pledgeMap;
    ...
    //  Add a "duration" for Pledge
    function pushPool(uint256 startTime, uint256 duration) external onlyOwner {
        require(duration > 0, "duration can't be zero");
        require(pools.length < 2, "The number of pools cannot exceed 2");
        pools.push (
            PledgePool(autoIncrementId(), startTime, duration)
        );
    }

    //  Reset durations
    // function resetPools(PledgePool[] calldata _pools) external onlyOwner {
    //     require(_pools.length > 0, "Activities can't be empty");
    //     delete pools;
    //     for (uint256 i = 0; i < _pools.length; i++) {
    //         require(_pools[i].duration > 0, "duration can't be zero");
    //         pools.push(_pools[i]);
    //         pools[i].id = autoIncrementId();
    //     }
    // }

    function changeDuration(uint256 poolIndex, uint256 _duration) external onlyOwner {
        require(poolIndex < pools.length, "Index out of boundry");
        require(pools[poolIndex].startTime <= block.timestamp, "Activity has started");
        pools[poolIndex].duration = _duration;
    }

    /**
     * Pledge, index: the subscript of durations
     */
    function pledge(uint256 amount, uint256 poolIndex) external whenNotPaused nonReentrant {
        require(poolIndex < pools.length, "Index out of boundry");
        require(pools[poolIndex].startTime <= block.timestamp, "Activity has not started");
        require(amount > 0, "Pledge amount can't be zero");
        require(token != address(0), "Invalid token address");
        //        require(_msgSender() != owner(), "Sender can't be owner");
        IBEP20(token).safeTransferFrom(_msgSender(), address(this), amount);
        uint256 poolId = pools[poolIndex].id;
        // poolMap[poolId] = poolMap[poolId].add(amount);
        // pledgeMap[poolId][_msgSender()] = pledgeMap[poolId][_msgSender()].add(amount);
        uint256 identifier = autoIncrementId();
        uint256 deadline = getDeadline(pools[poolIndex].duration);
        ...
        emit Pledge(token, _msgSender(), address(this), amount, identifier, poolId, block.timestamp, deadline);
    }
    ...
    function airdrop(AirdropItem calldata item) private {
        ...
        // uint256 duePrinciple;
        // for (uint256 j = 0; j < principleMap[item.poolId][item.wallet].length; j++) {
        //     for (uint256 k = 0; k < item.pledgeIds.length; k++) {
        //         if(principleMap[item.poolId][item.wallet][j].id == item.pledgeIds[k]) {
        //             // filter[item.pledgeIds[k]] = item.pledgeIds[k];
        //             duePrinciple = duePrinciple.add(principleMap[item.poolId][item.wallet][j].amount);
        //         }
        //     }
        // }
        ...
        // pledgeMap[item.poolId][item.wallet] = pledgeMap[item.poolId][item.wallet].sub(duePrinciple, "Reward amount exceeds pledge balance");
        // poolMap[item.poolId] = poolMap[item.poolId].sub(duePrinciple, "Reward amount exceeds total pledge balance");
    }

    /**
    * Claim the interest due by 'id'
    */
    function claimPrinciple(uint256 pledgeId, uint256 poolIndex) external whenNotPaused nonReentrant {
        require(pledgeId <= primaryId && poolIndex < pools.length, "Invalid id");
        require(token != address(0), "Invalid token address");
        uint256 _index;
        bool isExits = false;
        uint256 poolId = pools[poolIndex].id;
    ...
    */
    function claimAllDuePrinciple(uint256 poolIndex) external whenNotPaused nonReentrant {
        require(poolIndex <= primaryId && poolIndex < pools.length, "Invalid id");
        require(pools[poolIndex].startTime <= block.timestamp, "Activity has not started");
        uint256 poolId = pools[poolIndex].id;
        uint256 dueAmount = 0;
        for (uint256 j = 0; j < principleMap[poolId][_msgSender()].length; j++) {
     ...
     */
    function claimAllDueInterest(uint256 poolIndex) external whenNotPaused nonReentrant {
        require(poolIndex < pools.length, "Invalid id");
        require(token != address(0), "Invalid token address");
        uint256 dueAmount;
        uint256 poolId = pools[poolIndex].id;
     ...
     */
    function claimInterest(uint256 interestId, uint256 poolIndex) external whenNotPaused nonReentrant {
        require(poolIndex <= primaryId && poolIndex < pools.length, "Invalid id");
        uint256 poolId = pools[poolIndex].id;
        require(token != address(0), "Invalid token address");
        uint256 _index;
    ...
    function withdraw(address _token) external onlyOwner {
        ...
        IBEP20(_token).safeTransfer(_msgSender(), balance);
        emit Withdraw(_token, address(this), _msgSender(), balance);
    }
    ...
    function getPrincipleLength(uint256 poolIndex) external view returns (uint256) {
        require(poolIndex < pools.length, "Invalid id");
        return principleMap[pools[poolIndex].id][_msgSender()].length;
    }

    function getInterestLength(uint256 poolIndex) external view returns (uint256) {
        require(poolIndex < pools.length, "Invalid id");
        return interestMap[pools[poolIndex].id][_msgSender()].length;
    }

    function getPrincipleRecord(uint256 index, uint256 poolIndex) external view returns (uint256, uint256, uint256) {
        require(poolIndex < pools.length, "Invalid id");
        uint256 poolId = pools[poolIndex].id;
    ...
    function getInterestRecord(uint256 index, uint256 poolIndex) external view returns (uint256, uint256, uint256) {
        require(poolIndex < pools.length, "Invalid id");
        uint256 poolId = pools[poolIndex].id;
    ...
    // get pool details
    // function getPledgePoolInfo(uint256 poolIndex) external view returns (uint256, uint256, uint256) {
    //     require(poolIndex < pools.length, "Index out of boundry");
    //     return (
    //     poolMap[pools[poolIndex].id],
    //     pledgeMap[pools[poolIndex].id][_msgSender()],
    //     pools[poolIndex].duration
    //     );
    // }
