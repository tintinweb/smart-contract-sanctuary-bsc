...
contract OPTC_Stake is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    function buyCard(uint amount, address invitor) external onlyEOA updateDaily {
        ...
        nft.mint(msg.sender, amount, times);
    ...
    function _calculateReward(uint tokenId, uint price, uint otherAmount) public view returns (uint rew, bool isOut, uint left){
        SlotInfo storage slot = slotInfo[tokenId];
        uint _debt = countingDebt();
        ...
        if (rew >= maxAmount) {
            ...
            isOut = true;
            left = otherAmount;
        } else if (rew + otherAmount >= maxAmount) {
            isOut = true;
            left = otherAmount - (maxAmount - rew);
            rew = maxAmount;
        } else {
            rew += otherAmount;
            left = 0;
    ...
    function calculateRewardAll(address addr) public view returns (uint rew){
        uint dynamic = refer.checkUserToClaim(addr);
        UserInfo storage user = userInfo[addr];
        uint price = OPTC.lastPrice();
        uint _rew;
        uint left = dynamic;
        for (uint i = 0; i < user.cardList.length; i++) {
            (_rew,,left) = _calculateReward(user.cardList[i], price, left);
            rew += _rew;
        }
        if (user.nodeId != 0) {
            (_rew,,) = _calculateReward(getNodeId(addr, user.nodeId), price, 0);
            rew += _rew;
        }
        return rew;
    }
    ...
    function claimAllReward() external onlyEOA updateDaily checkStart {
        uint dynamic = refer.checkUserToClaim(msg.sender);
        UserInfo storage user = userInfo[msg.sender];
        uint rew;
        uint _debt = countingDebt();
        uint price = OPTC.lastPrice();
        uint left = dynamic;
        {
            ...
            for (uint i = 0; i < lists.length; i++) {
                cardId = user.cardList[i];
                slot = slotInfo[cardId];
                (_rew, isOut, left) = _calculateReward(cardId, price, left);
                rew += _rew;
                if (isOut) {
            ...
            if (user.nodeId != 0) {
                ...
                (_rew,,) = _calculateReward(id, price, 0);
                rew += _rew;
                slotInfo[id].debt = _debt;
            }
        }
        uint total = getOptValue(rew, price);
        refer.claimReward(msg.sender, dynamic - left);
        OPTC.transfer(msg.sender, rew);
        user.claimed += rew;
        _subPower(total, _debt);
    }

    function _claim(uint tokenId, uint price, uint _debt) internal {
        (uint _rew,bool isOut,) = _calculateReward(tokenId, price, 0);
        SlotInfo storage slot = slotInfo[tokenId];
        UserInfo storage user = userInfo[msg.sender];
        if (isOut) {
    ...
    function pullOutCard(uint tokenId) external onlyEOA checkStart {
        ...
        nft.changePower(tokenId, slotInfo[tokenId].leftQuota);
    }
    ...
    function addNode(uint tokenId) external onlyEOA {
        ...
        slot.power = getOptValue(node.getCardTotalCost(tokenId), getOPTCPrice());
        slot.owner = msg.sender;
    ...
    function pullOutNode() external onlyEOA {
        ...
        uint id = getNodeId(msg.sender, cardId);
        _claim(id, price, _debt);
        node.transferFrom(address(this), msg.sender, cardId);
        delete nodeOwner[cardId];
        delete userInfo[msg.sender].nodeId;
        delete slotInfo[id];
