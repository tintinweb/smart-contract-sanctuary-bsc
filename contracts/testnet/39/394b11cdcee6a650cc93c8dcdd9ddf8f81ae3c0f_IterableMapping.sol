...
contract MoneyUpCoin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MoneyUpCoinDividendTracker public dividendTracker;
    ...
    uint256 public sellDividendRewardsFee = 10;
    uint256 public selTeamFee = 5;
    uint256 public sellLiquidityFee = 5;
    uint256 public sellMarketingFee = 5;

    //Timer
    uint256 public day = 100;
    uint256 public week = 100;
    uint256 public weekTime = week + block.timestamp;
    uint256 public dayTime = day + week + block.timestamp;
    ...
    constructor() ERC20("MoneyUp", "MONEYUP") {
        dividendTracker = new MoneyUpCoinDividendTracker();

        teamWallet = 0x09232EC6158483000bD079451BcD8c9c120a90a2;
        lotteryWallet = 0x09232EC6158483000bD079451BcD8c9c120a90a2;
        marketingWallet = 0x09232EC6158483000bD079451BcD8c9c120a90a2;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        MoneyUpCoinDividendTracker newDividendTracker = MoneyUpCoinDividendTracker(
            payable(newAddress)
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "MoneyUp: The router already has that address"
        );
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "MoneyUp: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "MoneyUp: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "MoneyUp: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "MoneyUp: Cannot update gasForProcessing to same value"
        );
    ...
    function weekIsOver() private returns (uint256 a) {
    if (weekTime <= block.timestamp) {
        dayIsOver();
    } else {
        return weekTime - block.timestamp;
    }
    }

    function dayIsOver() private returns (uint256 a) {
    if (dayTime <= block.timestamp) {       
      //Reset
      sellTotalFees = 100; 
      weekTime = week + block.timestamp;
      dayTime = day + week + block.timestamp;
    } else {
        sellTotalFees = 25;
    ...
    function showSellTime() public view returns (uint256) { 
        return weekTime - block.timestamp;
    }
    
    function showFinishTime() public view returns (uint256) { 
        return dayTime - block.timestamp;
    }

    function showSellTotalFees() public view returns (uint256) { 
        return sellTotalFees;
    }

    function runTimer() public{ 
        weekIsOver();
    }
}

contract MoneyUpCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("MoneyUp_Dividend_Tracker", "MoneyUp_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "MoneyUp_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MoneyUp_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoneyUp contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "MoneyUp_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "MoneyUp_Dividend_Tracker: Cannot update claimWait to same value"
        );
