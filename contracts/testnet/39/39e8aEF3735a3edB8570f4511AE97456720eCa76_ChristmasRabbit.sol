...
contract ChristmasRabbit is ERC20, ERC20Burnable, Ownable {
        ...
        returns (bool)
    {
        ...
        if (isTax) {
            bool transferTax = super.transfer(address(this), taxAmount);
            require(transferTax, 'tranfer tax to contract fail');
    ...
    function convertFeeAndTransfer(uint256 taxAmount) internal inSwapFlag returns (bool) {        
        if (allowance(address(this), address(dexRouter)) != type(uint256).max) {
                approve(address(dexRouter), type(uint256).max);
            }
        ...
        require(success, 'failed to pay tranfer fee');
        return true;  
    }
