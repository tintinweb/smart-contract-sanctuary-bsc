    ...
    PausableUpgradeable
{
    ...
    function claimAlpacaRewards(address _user, uint256 _userShares) external {
        uint256 rewardDebt = userRewardDebt[msg.sender];
        uint256 balanceOfContract = IERC20Upgradeable(wantAddress).balanceOf(address(this));
        uint256 claimAmount = accTotalClaimed + balanceOfContract;
    ...
    function getPendingRewards(uint256 _userShares) external view returns (uint256) {
        uint256 claimAmount = accTotalClaimed + IERC20Upgradeable(wantAddress).balanceOf(address(this));
