...
import "./interfaces/IMasterDigichain.sol";

/// @title Admin MasterDigichain proxy contract used to add features to MasterDigichain admin functions
/// @dev This contract does NOT handle changing the dev address of the MasterDigichain because that can only be done
///  by the dev address itself
/// @author TommyChain (DigiDev)
/// @notice Admin functions are separated into onlyOwner and onlyFarmAdmin to separate concerns
contract MasterDigichainAdmin is Ownable {
    ...
    /// @notice Farm admin can manage master digichain farms and fixed percent farms
    address public farmAdmin;
    /// @notice MasterDigichain Address
    IMasterDigichain immutable public masterDigichain;
    /// @notice Address which is eligible to accept ownership of the MasterDigichain. Set by the current owner.
    address public pendingMasterDigichainOwner = address(0);
    /// @notice Array of MasterDigichain pids that are active fixed percent farms
    uint256[] public fixedPercentFarmPids;
    /// @notice mapping of MasterDigichain pids to FixedPercentFarmInfo
    ...
    /// @notice Percentage of base pool allocation managed by MasterDigichain internally
    ...
    /// @notice Max multiplier which is possible to be set on the MasterDigichain
    uint256 constant public MAX_BONUS_MULTIPLIER = 4;

    event SetPendingMasterDigichainOwner(address pendingMasterDigichainOwner);
    ...
    constructor(
        IMasterDigichain _masterDigichain,
        address _farmAdmin
    ) public {
        masterDigichain = _masterDigichain;
    ...
    /// @notice Set an address as the pending admin of the MasterDigichain. The address must accept afterward to take ownership.
    /// @param _pendingMasterDigichainOwner Address to set as the pending owner of the MasterDigichain.
    function setPendingMasterDigichainOwner(address _pendingMasterDigichainOwner) external onlyOwner {
        pendingMasterDigichainOwner = _pendingMasterDigichainOwner;
        emit SetPendingMasterDigichainOwner(pendingMasterDigichainOwner);
    }

    /// @notice The pendingMasterDigichainOwner takes ownership through this call
    /// @dev Transferring MasterDigichain ownership away from this contract renders this contract useless. 
    function acceptMasterDigichainOwnership() external {
        require(msg.sender == pendingMasterDigichainOwner, "not pending owner");
        masterDigichain.transferOwnership(pendingMasterDigichainOwner);
        pendingMasterDigichainOwner = address(0);
    }

    /// @notice Update the rewardPerBlock multiplier on the MasterDigichain contract
    /// @param _newMultiplier Multiplier to change to
    function updateMasterDigichainMultiplier(uint256 _newMultiplier) external onlyOwner {
        require(_newMultiplier <= MAX_BONUS_MULTIPLIER, 'multiplier greater than max');
        masterDigichain.updateMultiplier(_newMultiplier);
    }

    /// @notice Helper function to update MasterDigichain pools in batches 
    /// @dev The MasterDigichain massUpdatePools function uses a for loop which in the future
    ///  could reach the block gas limit making it incallable. 
    /// @param pids Array of MasterDigichain pids to update
    function batchUpdateMasterDigichainPools(uint256[] memory pids) external {
        for (uint256 pidIndex = 0; pidIndex < pids.length; pidIndex++) {
            masterDigichain.updatePool(pids[pidIndex]);
        }
    }

    /// @notice Obtain detailed allocation information regarding a MasterDigichain pool
    /// @param pid MasterDigichain pid to pull detailed information from
    ...
    /// @return poolJamuPerBlock Amount of JAMU given to the pool per block
    /// @return poolJamuPerDay Amount of JAMU given to the pool per day
    /// @return poolJamuPerMonth Amount of JAMU given to the pool per month
    function getDetailedPoolInfo(uint pid) external view returns (
        ...
        uint256 poolJamuPerBlock,
        uint256 poolJamuPerDay,
        uint256 poolJamuPerMonth
    ) {
        uint256 jamuPerBlock = masterDigichain.cakePerBlock() * masterDigichain.BONUS_MULTIPLIER();
        ( lpToken, poolAllocationPoint,,) = masterDigichain.getPoolInfo(pid);
        totalAllocationPoints = masterDigichain.totalAllocPoint();
        poolAllocationPercentMantissa = (poolAllocationPoint.mul(1e18)).div(totalAllocationPoints);
        poolJamuPerBlock = (jamuPerBlock.mul(poolAllocationPercentMantissa)).div(1e18);
        // Assumes a 3 second blocktime
        poolJamuPerDay = poolJamuPerBlock * 1200 * 24;
        poolJamuPerMonth = poolJamuPerDay * 30;
    }
    ...
    /// @notice Add a batch of farms to the MasterDigichain contract
    ...
    function addMasterDigichainFarms(
        ...
        if (_withMassPoolUpdate) {
            masterDigichain.massUpdatePools();
        }

        for (uint256 index = 0; index < _allocPoints.length; index++) {
            masterDigichain.add(_allocPoints[index], address(_lpTokens[index]), false);
    ...
    /// @notice Add a batch of farms to the MasterDigichain contract
    /// @dev syncs fixed percentage farms after update
    /// @param _pids Array of MasterDigichain pool ids to update
    /// @param _allocPoints Array of allocation points to set each pid
    /// @param _withMassPoolUpdate Mass update pools before update
    /// @param _syncFixedPercentageFarms Sync fixed percentage farm allocations
    function setMasterDigichainFarms(
        ...
        uint256 pidIndexes = masterDigichain.poolLength();
        for (uint256 index = 0; index < _pids.length; index++) {
            require(_pids[index] < pidIndexes, "pid is out of bounds of MasterDigichain");
            // Set all pids with no update
            masterDigichain.set(_pids[index], _allocPoints[index], false);
    ...
    /// @dev Must be a new MasterDigichain pid and below the max fixed percentage 
    /// @param _pid MasterDigichain pid to create a fixed percentage farm for
    ...
    function addFixedPercentFarmAllocation(
        ...
        require(_pid < masterDigichain.poolLength(), "pid is out of bounds of MasterDigichain");
        require(_pid != 0, "cannot add reserved MasterDigichain pid 0");
    ...
    /// @param _pid MasterDigichain pid linked to fixed percentage farm to update
    /// @param _allocPercentage Percentage based in PERCENTAGE_PRECISION
    /// @param _withMassPoolUpdate Mass update pools before update
    /// @param _syncFixedPercentageFarms Sync fixed percentage farm allocations
    function setFixedPercentFarmAllocation(
        ...
        if(_allocPercentage == 0) {
            // Disable fixed percentage farm and MasterDigichain allocation
            ...
            // NOTE: The MasterDigichain pool allocation is left unchanged to not disable a fixed farm 
    ...
    /// @notice Get the total percentage allocated to fixed percentage farms on the MasterDigichain
    /// @dev Adds the total percent allocated to fixed percentage farms with the percentage allocated to the JAMU pool. 
    ///  The MasterDigichain manages the JAMU pool internally and we need to account for this when syncing fixed percentage farms.
    ...
    /// @notice Run through fixed percentage farm allocations and set MasterDigichain allocations to match the percentage.
    /// @dev The MasterDigichain contract manages the JAMU pool percentage on its own which is accounted for in the calculations below. 
    function _syncFixedPercentFarms() internal {
        ...
        uint256 masterDigichainTotalAllocation = masterDigichain.totalAllocPoint();
        ( ,uint256 poolAllocation,,) = masterDigichain.getPoolInfo(0);
        ...
        for (uint256 index = 0; index < numberOfFixedPercentFarms; index++) {
            ( ,uint256 fixedPercentFarmAllocation,,) = masterDigichain.getPoolInfo(fixedPercentFarmPids[index]);
        ...
        uint256 nonPercentageBasedAllocation = masterDigichainTotalAllocation.sub(poolAllocation).sub(currentTotalFixedPercentFarmAllocation);
        ...
        for (uint256 index = 0; index < numberOfFixedPercentFarms; index++) {
            ...
            masterDigichain.set(fixedPercentFarm.pid, newFixedPercentFarmAllocation, false);
