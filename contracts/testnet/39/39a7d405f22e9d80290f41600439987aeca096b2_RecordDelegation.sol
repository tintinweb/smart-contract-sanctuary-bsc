...
contract RecordDelegation {
    ...
    function validatorStakes(string memory ecosystemId, string memory chainId, string memory validatorAddress) external view returns (Delegation[] memory) {
        ...
        for (uint256 i = 0; i < delegatorInfo[chainData][validatorAddress].length; i++) {
            delegations[count].validator = validatorAddress;
            delegations[count].user = delegatorInfo[chainData][validatorAddress][i];
            delegations[count].amount = delegationInfo[chainData][validatorAddress][delegatorInfo[chainData][validatorAddress][i]];
        }
        return delegations;
    }
    ...
    function userStakes(string memory ecosystemId, string memory chainId, string memory userAddress) external view returns (Delegation[] memory) {
        ...
        for (uint256 i = 0; i < userValidatorInfo[chainData][userAddress].length; i++) {
            delegations[count].validator = userValidatorInfo[chainData][userAddress][i];
            delegations[count].user = userAddress;
            delegations[count].amount = delegationInfo[chainData][userValidatorInfo[chainData][userAddress][i]][userAddress];
        }
        return delegations;
    }
}