 ...
 */
contract TaroChestOpener is UUPSUpgradeable, TwoStageOwnableUpgradeable, PausableUpgradeable, ERC1155HolderUpgradeable, VRFConsumerBaseV2Upgradeable {
    ...
    function openChests(uint256 requestId) external whenNotPaused {
        ...
        if (tokenId == TaroNFTConstants.HERO_CHEST_ID) {
            (tokens, amounts) = _pickHeroes(from, count, _randomNumberGenerator.randomNumbers(requestId)[0]);
        } else if (tokenId == TaroNFTConstants.ITEM_CHEST_ID) {
            (tokens, amounts) = _pickItems(from, count, _randomNumberGenerator.randomNumbers(requestId)[0]);
        } else {
        ...
        _taroNft.mintBatch(from, tokens, amounts, "");
    ...
    returns (uint256[] memory tokens, uint256[] memory amounts) {
        ...
        for (uint32 i=0; i<count; i++) {
            ...
            // console.log(i, tokens[i], amounts[i]);
        }
    }
