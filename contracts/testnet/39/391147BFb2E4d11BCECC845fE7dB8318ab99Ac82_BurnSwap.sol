...
contract BurnSwap is Pausable, Ownable {
    ...
    */
    function purchaseOBURN(uint256 amountOBURN, uint256 amountUSDC, uint256 slippage) external {
        ...
        uint[] memory amountsFetched = quickSwapRouter.getAmountsIn(amountOBURN, path);

        if (amountUSDC == 0) {
            amountUSDCNeeded = amountsFetched[0] * ((100 + slippage) / 100);
        }
        ...
        if (amountUSDC > 0) {
            ...
            amountsFetched = quickSwapRouter.swapExactTokensForTokens(
                amountUSDCAfterTax,
                minimumOBURNNeeded,
                path,
                address(this),
            ...
            amountsFetched = quickSwapRouter.swapTokensForExactTokens(
        ...
        _oburn.transfer(msg.sender, amountsFetched[1]);

        if (amountsFetched[0] < amountUSDCAfterTax) {
            _usdc.transfer(msg.sender, amountUSDCAfterTax - amountsFetched[0]);
        }

        emit oburnBuy(msg.sender, amountsFetched[0], amountsFetched[1]);
    }
    ...
    */
    function sellOBURN(uint256 amountOBURN, uint256 amountUSDC, uint256 slippage) external {
        ...
        uint[] memory amountsFetched = quickSwapRouter.getAmountsIn(amountUSDC, path);

        if (amountOBURN == 0) {
            amountOBURNNeeded = amountsFetched[0] * ((100 + slippage) / 100);
        }
        ...
        addressToUSDCCollected[msg.sender] == amountsFetched[1] / oburnSellFee;
        USDCCollected += amountsFetched[1] / oburnSellFee;    

        _usdc.transfer(msg.sender, amountsFetched[1] * (100 - oburnSellFee) / 100);

        if (amountsFetched[0] < amountOBURNNeeded) {
            _oburn.transfer(msg.sender, amountOBURNNeeded - amountsFetched[0]);
        }        

        emit oburnSell(msg.sender, amountsFetched[1], amountsFetched[0]);
    }
