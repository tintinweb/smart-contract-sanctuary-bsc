...
contract RtlolPackFunctions is AccessControl, ReentrancyGuard {
    ...
    BitMaps.BitMap private rewardClaimed;
    ...
    event LotteryRun(uint256 id, bytes32 lotteryHash, uint256 random);
    ...
    // The transaction based randomness is combined by a deterministic verifier based signiture that is only revealed after the lottery is run
    // This ensures that miners without the verifier private key can't meaningully influence the results

    function runLottery(bytes32 lotteryHash, uint256 prizeCount) external onlyEditor {

        uint256 random = uint256(keccak256(abi.encode(block.difficulty, block.timestamp, lotteryHash)));
        lotteryResults[rewardCount] = LotteryResult({
            lotteryHash: lotteryHash,
            random: random
        });
        emit LotteryRun(rewardCount, lotteryHash, random);
        rewardCount += prizeCount;
    }

    function getWinningTicket(bytes memory randSig, uint256 roll, uint256 ticketCount) public pure returns (uint256) {
    ...
    function claimReward(
        ...
        require(!rewardClaimed.get(lotteryId + prizePostition - 1), "Already claimed");
        rewardClaimed.set(lotteryId + prizePostition - 1);
        
        require(prizePostition > 0 && prizePostition < prizeCount, "Invalid position");
        ...
        uint256 winningTicket = getWinningTicket(randSig, _result.random, ticketCount);
        require (winningTicket >= first && winningTicket <= last, "not winner");

        _hashString = abi.encode(merkleRoot, ticketCount, prizeCount);
        ...
        bytes32 _hash = keccak256(_hashString);
        require(_hash == _result.lotteryHash, "invalid transaction");

        _hash = keccak256(abi.encode(_msgSender(), first, last)); // node
