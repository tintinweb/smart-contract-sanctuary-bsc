...
contract SoulmetaDivide is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    event DivideLog(address indexed player, address firstRecommender, address secondRecommender, address tertiaryRecommender, address knight);
    ...
    function removeApi(uint tokenId_) public{
        ...
        require(_msgSender() == nftOwner, "no permission");
    ...
    function castApi(uint8 theme_, uint8 group_, uint amount_, uint soulCount_, address recommender_) public{
        ...
        require(soulCount_ >0 || (amount_ > 0 && amount_ % 100 == 0), "invalid parameter");

        require(recommender_ != _msgSender(), "Can not be your own recommender");

        //用户每铸造100个需要支付4u
        rewardAddr.usdt.safeTransferFrom(_msgSender(), address(this), (amount_ / 100) * 4 ether);

        //消耗soul的数量
        require(soulCount_ == 1 || soulCount_ % 5 == 0, "soulCount err");
        ...
        //组装插入数组中的索引
        uint boxIndex = (theme_ * 10000) + (group_ * 1000) + count;  //位表示， 千位起表示主题，百位，表示性别，个十位表示销毁的soul数量
   ...
   *  @params
    ...
    function extractApi(uint8 theme_, uint8 group_, address recommender_) public{
        ...
        while(true){
            ...
            uint partIndex = (theme_ * 10000) + (group_ * 1000);  //池子索引前缀
    ...
    function _divide(address recommender_, address caster_, uint amount_) private{
        uint share = amount_ / 100;    //份

        //固定奖池获得1u
        totalAmount.jackpot += (share * 1 ether);
        rewardAddr.usdt.safeTransfer(rewardAddr.jackpot, share * 1 ether);

        //通过铸造者地址区分铸造或者开启
        if(caster_ == address(0)){
            //铸造者地址为空，为铸造操作
            totalAmount.platform += (share * 0.8 ether);       //平台分成0.8
            totalAmount.shareHolder += (share * 1.2 ether);    //股东会1.2

            rewardAddr.usdt.safeTransfer(rewardAddr.platform, share * 0.8 ether);
            rewardAddr.usdt.safeTransfer(rewardAddr.shareHolder, share * 1.2 ether);
        }else{
            //开启操作
            castWallet[caster_] += 1 ether;                    //铸造者分成1u

            totalAmount.platform += (share * 0.2 ether);       //平台0.2
            totalAmount.shareHolder += (share * 0.8 ether);    //股东0.8

            rewardAddr.usdt.safeTransfer(rewardAddr.platform, share * 0.2 ether);
            rewardAddr.usdt.safeTransfer(rewardAddr.shareHolder, share * 0.8 ether);
        }

        //没有推荐人,
        if(recommender_ == address(0)){
            recommender_ = rewardAddr.receiveAddr;
        }
        ...
        address recommenderFirst;     //当前推荐人
        address recommenderSecond;     //二级推荐人
        ...
        if(referrers[_msgSender()] != address(0)){
            ...
            //将当前推荐人赋值供外部使用
            recommenderFirst = referrers[_msgSender()];

            totalAmount.recommender += (share * 0.4 ether);   //推荐总奖金增加
        }else{
            //判断是否为新推荐
            if(recommender_ != rewardAddr.receiveAddr){
                referrersWallet[recommender_] += (share * 0.4 ether);
                receiveCount -= (share * 0.4 ether);

                recommenderFirst = recommender_;
                referrers[_msgSender()] = recommender_;    //绑定推荐关系

                totalAmount.recommender += (share * 0.4 ether);   //推荐总奖励增加
            }
        }


        //二级推荐判断，当前推荐人存在，并且也存在推荐他的人
        if(recommenderFirst != address(0) && referrers[recommenderFirst] != address(0)){
            referrersWallet[referrers[recommenderFirst]] += (share * 0.3 ether);   //二级推荐人获得(share * 0.3)

            //初始分成金额减少 share * 0.3s
            receiveCount -= (share * 0.3 ether);

            //设定三级推荐人，供外部使用
            recommenderSecond = referrers[recommenderFirst];

            totalAmount.recommender += (share * 0.3 ether);   //推荐总奖励增加
        }


        //三级推荐人判断，存在二级推荐人，且二级推荐人有自己的推荐人，即三级推荐人
        if(recommenderSecond != address(0) && referrers[recommenderSecond] != address(0)){
            referrersWallet[referrers[recommenderSecond]] += (share * 0.2 ether);   //三级推荐人获得(share * 0.2)

            //初始分成金额减少 share * 0.2
            receiveCount -= (share * 0.2 ether);

            totalAmount.recommender += (share * 0.2 ether);   //推荐总奖励增加
        }

        /*****    骑士判断  *****/
        address knight;

        (address selfKnight,,) = rewardAddr.soulTeam.teams(_msgSender());
        if(selfKnight != address(0)){
            //用户已经绑定骑士
            knight = selfKnight;
        }else if(teams[_msgSender()] != address(0)){
            knight = teams[_msgSender()];
        }else if(teams[recommenderFirst] != address(0)){
            knight = teams[recommenderFirst];
        }else if(teams[recommenderSecond] != address(0)){
            knight = teams[recommenderSecond];
        }else{
            (address leader,,) = rewardAddr.soulTeam.teams(recommenderFirst);
            if(leader != address(0)){
                knight = leader;
            }
        }
        ...
        if(teams[_msgSender()] == address(0)){
            teams[_msgSender()] = knight;
        }

        if(knight != address(0)){
            //存在骑士
            totalAmount.community += (share * 0.1 ether);     //社区获得(share * 0.1)
            rewardAddr.usdt.safeTransfer(address(rewardAddr.soulTeam), share * 0.1 ether);

            //初始分成金额减少
            receiveCount -= (share * 0.1 ether);

            rewardAddr.soulTeam.addReward(knight, share * 0.1 ether);
        }
        ...
        emit DivideLog(_msgSender(), recommenderFirst, recommenderSecond, referrers[recommenderSecond], knight);
    ...
    function withdrawal() public{
        require(referrersWallet[_msgSender()] > 0 || castWallet[_msgSender()] > 0, "Sorry, your credit is running low");
    ...
    function setTeamsFrom(address user_, address communityAddr_) public{
        ...
        require(teams[user_] == address(0), "already bound");

        (address targetKnight,,) = rewardAddr.soulTeam.teams(communityAddr_);
        require(targetKnight != address(0), "community does not exist");
