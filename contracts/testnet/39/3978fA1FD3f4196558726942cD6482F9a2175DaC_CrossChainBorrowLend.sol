    ...
    ReentrancyGuard
{
    constructor(
        ...
        // token decimals
        state.borrowingTokensDecimals[borrowingAsset_] = borrowTokenDecimals_;
    }
    ...
    function whitelistBorrowToken(bytes32 _token, uint8 _decimals)
        external
        onlyOwner
    {
        require(_decimals != 0, "invalid decimals");
        state.borrowingTokensDecimals[_token] = _decimals;
    }
    ...
    function updateTargetChainId(uint16 _newId) external onlyOwner {
        state.targetChainId = _newId;
    }
    ...
    */
    ...
    function verifyAssetMetaFromBorrow(BorrowMessage memory params)
        ...
        returns (bool)
    {
        return params.header.borrowAddress == state.borrowingAssetAddress;
    }
    ...
    function targetChainId() public view returns (uint16) {
        return state.targetChainId;
    }
...
contract CrossChainBorrowLendGetters is Context, CrossChainBorrowLendState {
    ...
    function bytesToAddress(bytes32 bys) public pure returns (address addr) {
        assembly {
...
contract CrossChainBorrowLendMessages {
    ...
    function decodeBorrowMessage(
        bytes memory serialized // todo: make it internal
    ) internal pure returns (BorrowMessage memory params) {
        uint256 index = 0;
        ...
        params.collateralDetail = decodeCollateralDetail(
            serialized.slice(index, index += 151)
        );
        require(
...
contract CrossChainBorrowLendStorage {
    struct State {
        ...
        mapping(bytes32 => uint8) borrowingTokensDecimals;
        address owner;
    }
}
