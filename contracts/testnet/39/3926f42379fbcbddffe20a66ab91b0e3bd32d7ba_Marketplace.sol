    ...
    MarketplaceStorage
{
    ...
    function delistItem(address nftAddress, uint256 assetId) public {
        ...
        require(item.id != 0, "Asset not published");
        require(
            nftRegistry.ownerOf(assetId) == deleteBy,
            "The seller is no longer the owner"
        );
    ...
    // ) public payable {
    //     require(
    //         msg.value >= offerPrice && msg.value > 0,
    ...
    //     itemOffers[nftAddress][assetId][offerer] = ItemOffer({
    //         id: item.id,
    ...
    // function cancleOfferItem(address nftAddress, uint256 assetId) public {
    //     _requireERC721(nftAddress);
    ...
    //     ItemOffer memory offerData = itemOffers[nftAddress][assetId][offerer];
    //     uint256 offerPrice = offerData.offerPrice;
    //     payable(offerer).transfer(offerPrice);
    //     delete itemOffers[nftAddress][assetId][offerer];

    //     emit ItemOfferCanceled(
    //         item.id,
    //         assetId,
    //         item.seller,
    //         nftAddress,
    ...
    //     require(seller != address(0), "Invalid address");
    //     require(
    //         seller == msg.sender && seller == nftRegistry.ownerOf(assetId),
    //         "The seller is no longer the owner"
    //     );
    ...
    //     nftRegistry.safeTransferFrom(seller, offerer, assetId);

    //     items[nftAddress][assetId].seller = offerer;
    //     items[nftAddress][assetId].status = ItemStatus.BOUGHT;
    //     items[nftAddress][assetId].price = offerPrice;

    //     delete itemOffers[nftAddress][assetId][offerer];

    //     emit ItemOfferApproved(
    //         item.id,
    //         assetId,
    //         seller,
    //         nftAddress,
