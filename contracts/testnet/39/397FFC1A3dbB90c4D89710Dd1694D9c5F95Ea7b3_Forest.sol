...
contract Forest {
   ...
   using SafeMath for uint256;
    ...
    //fost合约
    address internal constant FostToken = 0xE8244F1C4dd01e3952A2b08D6718A6dc590c8f1a;
    //kft合约
    address internal constant KftToken = 0xB36523546e2AC1c2A7157b73F1B359C60761370d;
    //fost接收地址
    address internal constant fostAddress = 0x6e2000ac526985F2E557152A93adc445Df286d59;
    //kft接收地址
    address internal constant kftAddress = 0x6e2000ac526985F2E557152A93adc445Df286d59;
    //kft质押数量
    ...
    function kftPledge(address addr) public returns(bool){
        ...
        //2592000
        kft.realseTime = block.timestamp.add(0);
    ...
    //fost质押.
    function fostPledge(address addr, uint level, uint amount) public returns(bool){
        ...
        if(pre.status == 1 && pre.gear > 0){
            uint cb = calPersonalContri(pre.king);
            uint dirReward = cb * 1/10;
        ...
        if(ppre.status == 1 && ppre.gear > 0){
            uint cb = calPersonalContri(ppre.king);
            uint indirReward = cb * 8/100;
    ...
    function addBalance(uint balanceType, uint amount, address addr) public returns(bool){
        Balance[] storage b = balancesArry[addr];
        bool bl = false;
        uint totalAmount;
        for(uint i = 0; i < b.length; i++){
            if(b[i].blanceType == balanceType){
                totalAmount = b[i].amount.add(amount);
                b[i].amount = totalAmount;
            }
        }
        return bl;
    }
