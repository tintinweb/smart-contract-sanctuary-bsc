...
contract Igniter is IERC20, IERC20Metadata, Context, Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        {            
            ...
            {            
                uint256 walletClaimBalance1 = address(this).balance - priorBalance;
            ...
            // revert("Something went wrong swapping your tokens");
        }
    }

    function checkBurnSwapClaimValue() external view returns (uint256 _balance) {
        require(burnSwapState, "Burn swap is currently not available");
        _balance = claimBalance[msg.sender];
    }
    ...
    function claimBurnSwap(uint16 percentOfSwapValue, address to) external nonReentrant {
        require(burnSwapState, "Burn swap is currently not available.");
        require(claimBalance[msg.sender] > 0, "Nothing to claim!");
        require(percentOfSwapValue > 0 && percentOfSwapValue <= 100, "Claim percent not in range.");

        IIncinerator _incinerator = IIncinerator(
            contractAddresses.burnerContract
        );

        uint256 claimable = claimBalance[msg.sender];
        uint256 claiming = ((claimable *  percentOfSwapValue) / 100);
         
        claimBalance[msg.sender] -= claiming;

        address[] memory buyPath = new address[](2);
        buyPath[0] = dexRouter.WETH();
        buyPath[1] = contractAddresses.burnerContract;

        address[] memory reserveValuePath = new address[](2);
        reserveValuePath[0] = contractAddresses.burnerContract;
        ...
        uint[] memory gasTokenReserveValue = dexRouter.getAmountsOut(tokenBalance, reserveValuePath);
        uint256 ethValue = gasTokenReserveValue[gasTokenReserveValue.length - 1];
        
        require(ethValue > claiming, "Amount of Claim is too large for the reserves.");
        ...
        nonReentrant
    {
        require(0 < amount, "Zero Tokens");
        require(
            IERC20(contractAddresses.burnerContract).balanceOf(address(this)) >=
                amount,
            "Not enough tokens to send"
        );
        require(
            IERC20(contractAddresses.burnerContract).transfer(
                destination,
                amount
            ),
            "transfer failed"
