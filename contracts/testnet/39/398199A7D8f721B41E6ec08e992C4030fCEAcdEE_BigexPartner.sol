...
contract BigexPartner is Ownable, Pausable {
	using SafeMath for uint256;

	IERC20 public TOKEN;
	address public otherInvestmentContract;

	uint256 public timePeriod; // time unlock every month
	uint256 public timeClaimPeriod; // 7 days
	uint256 public timeAfterListing;
	uint256 public receivePercentage;

	mapping(address => uint256[]) public userLockDetail;
	mapping(address => bool) public whiteList;
	mapping(address => uint256) public claimAmountInfo;
	mapping(address => uint256) public timeClaim;

	event Claim(address to, uint256 amount, uint256 timestamp);

	constructor() {
		TOKEN = IERC20(0x2F6c6cE689C0231919713c5556213cF3895Fed3B);
		receivePercentage = 5;
		timePeriod = 2592000;
		timeClaimPeriod = 604800;
		timeAfterListing = 1661990400;
	}

	function claim() public whenNotPaused {
		require(whiteList[msg.sender], 'wallet not whitelist');
		require(timeClaim[msg.sender] <= block.timestamp, 'can not claim now');
		require(claimAmountInfo[msg.sender] > 0, 'amount need greater than 0');

		TOKEN.transfer(msg.sender, claimAmountInfo[msg.sender]);
		userLockDetail[msg.sender].push(claimAmountInfo[msg.sender]);

		timeClaim[msg.sender] = timeClaim[msg.sender].add(timeClaimPeriod);
		emit Claim(msg.sender, claimAmountInfo[msg.sender], block.timestamp);
	}
	...
	function getITransferInvestment(address _wallet) external view returns (uint256) {
		...
		for (uint256 i = 0; i < userLockDetail[_wallet].length; i++) {
			...
			if (timeAfterListing > 0 && block.timestamp > timeAfterListing) {
				uint256 unlockAmount = userLockDetail[_wallet][i].mul(receivePercentage).div(100).mul(
					block.timestamp.sub(timeAfterListing).div(timePeriod)
				);
				if (unlockAmount > 0) {
	...
	function setClaimAmountInfo(address[] memory _wallets, uint256[] memory _amounts) public onlyOwner {
		for (uint256 i = 0; i < _wallets.length; i++) {
			claimAmountInfo[_wallets[i]] = _amounts[i];
		}
	}

	function setWhiteList(address[] memory _wallets, uint256[] memory _timeClaim, bool _result) public onlyOwner {
		for (uint256 i = 0; i < _wallets.length; i++) {
			whiteList[_wallets[i]] = _result;
			timeClaim[_wallets[i]] = _timeClaim[i];
		}
	}

	function setTimeClaim(address _wallet, uint256 _timeClaim) public onlyOwner {
		timeClaim[_wallet] = _timeClaim;
	}

	function setTimeClaimPeriod(uint256 _timeClaimPeriod) public onlyOwner {
		timeClaimPeriod = _timeClaimPeriod;
	}
	...
	function setTimeAfterListing(uint256 _timeAfterListing) public onlyOwner {
		timeAfterListing = _timeAfterListing;
	}

	function setReceivePercentage(uint256 _receivePercentage) public onlyOwner {
