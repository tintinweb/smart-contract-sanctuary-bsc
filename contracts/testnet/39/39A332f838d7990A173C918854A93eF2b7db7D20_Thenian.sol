...
contract Thenian is ERC721Enumerable, Ownable {
    ...
    function mintFirst(bytes32[] memory proof) public payable {
        ...
        require(block.timestamp < SALE_START_TIMESTAMP + 2400, "First round has ended.");
    ...
    function mintSecond(uint256 amount, bytes32[] memory proof) public payable {
        require(block.timestamp >= SALE_START_TIMESTAMP + 2400, "Second round has not started yet.");
        require(block.timestamp < SALE_START_TIMESTAMP + 3600, "Second round has ended.");
        require(verifyLeaf(proof, msg.sender), "Not whitelisted.");
        require(NFT_PRICE.mul(amount) == msg.value, "BNB value sent is not correct");
        require(secondMint[msg.sender].add(amount) <= MAX_PER_MINT, "Can only claim one in the first round");
