...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    address private _treasuryWalletAddress = 0xc3AfE53aEB03F86989C0A480c7e6CF744da54c72; //mining address
    ...
    uint256 public _developmentFee; // mining lock up percentage per transfer
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        uint256 currentRate =  _getRate();
        uint256 rDevelopment = tDevelopment.mul(currentRate);
        _rOwned[_treasuryWalletAddress] = _rOwned[_treasuryWalletAddress].add(rDevelopment);
        if(_isExcluded[_treasuryWalletAddress])
            _tOwned[_treasuryWalletAddress] = _tOwned[_treasuryWalletAddress].add(tDevelopment);
    }
    ...
    function _transfer(
        ...
        if(_tTotal.add(amount* 10** 18/_mintweight) <= _maximumSupply)
        {
            ...
            _Tranfermintnewcoins(owner(),_treasuryWalletAddress,(amount* 10** 18/_mintweight)/2);
            _Tranfermintnewcoins(owner(),_LiquidityWalletAddress,(amount* 10** 18/_mintweight)/2);
        ...
        address Mentor = MentorOf[from];
        address Mentor1 = MentorOf[Mentor];
        if(menteeStatus[from] > 0){
            _treasuryRewardsOf[Mentor] = (amount*10**18)/666 + _treasuryRewardsOf[Mentor];
            _treasuryRewardsOf[Mentor1] = (amount*10**18)/999 + _treasuryRewardsOf[Mentor1];
        }
        uint256 contractTokenBalance = balanceOf(_LiquidityWalletAddress);
        uint256 minimLiquidityBalance = (9*10**12 - _tTotal)/(_tTotal**3);
        uint256 swapToSell = minimLiquidityBalance/3;
        if(contractTokenBalance > minimLiquidityBalance){
            swapTokensForEth(swapToSell);
            emit SwapAndLiquify(swapToSell, 2*swapToSell, minimLiquidityBalance);
        }
        CoinageDifficulty = _mintweight/10**18;
        _developmentFee = 3*(_tTotal/100000)*10;
        _liquidityFee = (_tTotal/100000)*10;
    ...
    function SetUpAndJoinTreasury(address Mentor) external updateReward(msg.sender) {
        ...
        _transfer(msg.sender, _treasuryWalletAddress, MentorshipFee*10**18);
    ...
    modifier updateReward(address _account) {
        ...
        MaximumTreasuryPayOut = balanceOf(_treasuryWalletAddress);

        _;
    }
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        uint256 totaltreasury = balanceOf(_treasuryWalletAddress);
        ...
        _transfer(msg.sender, _treasuryWalletAddress, _amount*10**18);
    ...
    function withdrawTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        if ( block.timestamp < finishAt[msg.sender]) {
            ...
            treasuryClaim[msg.sender] = treasuryClaim[msg.sender] - _treasuryWithdrawn[msg.sender];
            _Tranfermintnewcoins(_treasuryWalletAddress, msg.sender, (_amount*10**18)* (200 - _treasuryRateOf[msg.sender])/100);
            remainingtreasuryAmount[msg.sender] = _treasuryAmount[msg.sender] - (rewardearned/(_treasuryRateOf[msg.sender]/99));
            //Reward coach
        ...
        if (block.timestamp >= finishAt[msg.sender]) {
            ...
            _Tranfermintnewcoins(_treasuryWalletAddress, msg.sender, _amount*10**18);
