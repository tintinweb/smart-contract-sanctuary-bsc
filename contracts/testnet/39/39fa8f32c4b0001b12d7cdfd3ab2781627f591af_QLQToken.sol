...
contract QLQMutualAid is Ownable, QLQOwner {
    ...
    /*
    * ClaimHistory is structure of historical quantity of payour
    * `countOfPayout` will represent total count that requested by users
    * `totalApprovedPayout` will represent total amount approved payout that already received
    * `lastClaim` will represent last claim request
    */
    struct ClaimHistory {
    ...
    /*
    * ClaimDetail is structure that contain detail of claim succeed
    * `id` will represent unique identity claim. The field will be set to autoincrement.
    * `claimID` will generate QolaQ foundation as certificate of claim identity.
    * `payoutAmount` will represent amount of payout
    * `votingCount` will represent voting of qty from community
    * `votingScore` will represent automatic score from QolaQ foundation
    * `note` will represent free text notes from QolaQ Foundation
    * `timeStamp` will represent timestamp of the claim
    * `walletAddressPayout` will represent target of address payout
    */
    struct ClaimDetail {
        uint256 id;
        string  claimID;
    ...
    **/
    function doClaim(address userAddress, ClaimDetail memory claimDetail) internal returns (bool) {
        ...
        require(claimDetail.votingScore <= 5, "Voting score rate should be in range 1-5");
