...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    function _sell(uint256 tokenAmount, uint256 deadline, uint256 minBNBOut) public nonReentrant ensure(deadline) payable returns (bool) {
        ...
        uint256 amountBNB = tokensToSwap.mul(liquidity).div(_balances[address(this)]);
        ...
        taxBalance = taxBalance.add(tokenAmount.mul(liquidity).div(_balances[address(this)])).sub(amountBNB);
    ...
    function getValueOfHoldings(address holder) public view returns(uint256) {
        return _balances[holder].mul(liquidity).div(_balances[address(this)]);
    }
