...
contract ChainPalTransaction is Ownable, ReEntrancyGuard {
    ...
    address CP = 0x7EF2e0048f5bAeDe046f6BF797943daF4ED8CB47;
    ...
    function createTransaction(
        ...
        if (compareToIgnoreCase(_paymentReceiveCurrency, "CP")) {
            transaction.paymentAmount = calculatedPaymentAmountForCP(
                _paymentAmount
            );
        } else {
        ...
        if (compareToIgnoreCase(_feePaymentCurrency, "CP")) {
            feeValues = returnUpdatedArray(_fees);
        }
    ...
    function refundFees(Transaction memory _transaction) private {
        if (
            ...
            address currency = supportedCurrency[
                upper(_transaction.transactionDetails.feePaymentCurrency)
            ].val;
        ...
        returns (bool)
    {
        if (compareToIgnoreCase(name, "USDT")) {
            ...
            return true;
        } else if (compareToIgnoreCase(name, "BUSD")) {
            return true;
        }
        return false;
    }
     ...
     */
    function returnUpdatedArray(uint256[] memory _myArray)
        internal
        view
        returns (uint256[] memory)
    {
        uint256[] memory newArray = new uint256[](_myArray.length);
        address currency = supportedCurrency[upper("CP")].val;
        for (uint256 i = 0; i < _myArray.length; i++) {
            uint256 transactionFeePercentage = IBEP20(currency).FeeRewardPct();
     ...
     */
    function calculatedPaymentAmountForCP(uint256 _amount)
        internal
        view
        returns (uint256)
    {
