...
contract ShibaCardsUpgrader is
  ShibaCardsPayable,
  ShibaCardsMintable,
  ShibaCardsDealable
{
  event Upgraded(uint256 from, uint256 to);

  /**
   * @dev Upgrades many cards of a kind to the next edition.
   */
  function upgrade(uint256 id) public payable {
    uint256 edition = dealer.getEditionByTokenId(id);
    require(edition != 3, "Not upgradeable");
    require(minter.balanceOf(_msgSender(), id) >= 10, "Insufficient balance.");
    ...
    minter.burn(_msgSender(), id, 10);
    minter.mintSpecific(_msgSender(), ids, amounts);

    emit Upgraded(id, id + 1);
  }
}
