...
library Counters {
 ...
 abstract contract Unbox is Ownable {
        ...
        bool private StopAndStartBox = false;
  ...
  function StopBox(bool __StopAndStartBox) public onlyOwner {
      StopAndStartBox = __StopAndStartBox;
  }

  function _unboxClass(address _usr) internal{
      require(StopAndStartBox, "RHT:Precisa estar ativado para gerar um Box");
...
library CounterNFT {
 ...
 contract RewardHunter is ERC1155, Ownable, Unbox, ReentrancyGuard {
  ...
  function _forwardFundsRht(uint256 _amount) private {
      IBEP20(token).transferFrom(msg.sender, wallet, _amount);
    }
  
  function _forwardFundsContract(uint256 _amount) private {
      IBEP20(token).transfer(wallet, _amount);
    }
  ...
  mapping(address => viewIds) private generateId;

  function viewNewId(address _usr) public view returns(uint newId) {
    viewIds memory ids = generateId[_usr];
    ids.newIds = _tokenIds.current();
    return ids.newIds;
  }
  ...
  function buyNfts(uint _tokenId, uint _price) public nonReentrant  {
      ...
      NftUsers storage ids = idToMarketItem[_tokenId];
          ...
          // Salvo o saldo da taxa para o Owner retirar depois
           ...
           _forwardFundsContract(divisorTax);
    ...
    function getListingPriceBNB() public view returns (uint256) {
      return listingPrice;
    }

    // Consegue visualizar o preço de cada NFT em RHT com base em suas definições
    function getListingPriceRHT() public view returns (uint256) {
      return priceInRHT;
    }
