...
contract ERC20Vesting is Ownable{
    ...
    function changeCliff(uint _cliff) external onlyOwner {
        cliff = _cliff + block.timestamp;
    }

    function changeDuration(uint _duration) external onlyOwner {
        duration = _duration;
    }

    function changeVestingStarted(bool _vestingStarted) external onlyOwner {
        vestingStarted = _vestingStarted;
    }
