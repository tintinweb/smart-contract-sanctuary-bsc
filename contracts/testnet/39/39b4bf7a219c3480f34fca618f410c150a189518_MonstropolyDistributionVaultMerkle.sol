...
import {IDistributionVaultAllocation} from
    "dappcomposer-contracts/contracts-upgradeable/interfaces/IDistributionVaultAllocation.sol";
import {Ownable2StepUpgradeable} from "openzeppelin-contracts-upgradeable/access/Ownable2StepUpgradeable.sol";
import {UUPSUpgradeable} from "openzeppelin-contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";

contract MonstropolyDistributionVaultMerkle is
    DistributionVaultMerkleUpgradeable,
    Ownable2StepUpgradeable,
    UUPSUpgradeable
{
    IDistributionVaultAllocation private _vault;

    constructor() {
        _disableInitializers();
    }

    function initialize(address owner_, address _token, address vault_) public initializer {
        bytes32 emptyMerkle;
        ...
        _vault = IDistributionVaultAllocation(vault_);
    }

    function _distribute(address account, address receiver, uint256 amount) internal override {
        uint256 available_ = _vault.available(address(this));
        _vault.distribute(address(this), available_);
        super._distribute(account, receiver, amount);
    }
