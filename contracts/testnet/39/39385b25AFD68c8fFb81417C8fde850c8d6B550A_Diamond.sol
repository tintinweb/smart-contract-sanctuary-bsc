...
contract Diamond is ERC20, Ownable {
    ...
    uint256 public autoLP = 20; //20% auto LP

    //remaining fees will distribute to the teams
    uint256 public devFee = 60; //60%  dinger team
    uint256 public marketingFee = 40; //40% diamond team

    uint256 private partialSellPercentage = 5; //5% contract balance

    address private devWalletAddress;
    address private marketingWalletAddress;

    uint256 public ethBalance;
    uint256 public ethRewardBalance;

    uint256 private totalClaimedEthReward;
    ...
    mapping(uint256 => ClaimedEthReward) private historyClaimedEthRewards;

    event BuyFees(address from, address to, uint256 fees);
    event SellFees(address from, address to, uint256 fees);
    ...
    function _transfer(
        ...
        if (
            ...
            (!isExcludedFromFee[_from] || !isExcludedFromFee[_to])
        ) {
            //take fees
    ...
    function setTaxes(uint256 _baseBuyTax, uint256 _baseSellTax)
        external
        onlyOwner
    {
    ...
    function setPartialSellPercentage(uint256 _partialSellPercentage)
        external
        onlyOwner
    {
        partialSellPercentage = _partialSellPercentage;
    }
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            //sell tokens
            fees = _amount.mul(baseSellTax).div(100);
            remainingAmount = _amount.sub(fees);
            super._transfer(_from, address(this), fees);
    ...
    function distributeTokensEth() private {
        uint256 initialETHBalance = address(this).balance;

        uint256 partialTokensToSell = thisBalance()
            .mul(partialSellPercentage)
            .div(100);

        // For Add Liquidity
        uint256 tokensForLiquidity = partialTokensToSell.mul(autoLP).div(100);

        uint256 halfLiquidity = tokensForLiquidity.div(2);
        swapTokensForEth(halfLiquidity);

        uint256 ethForAddLiquidity = address(this).balance.sub(
            initialETHBalance
        );

        addLiquidity(halfLiquidity, ethForAddLiquidity);

        // // For token distribution
        // uint256 tokensForFees = partialTokensToSell.sub(tokensForLiquidity);
        // swapTokensForEth(tokensForFees);
        // uint256 ethForFees = address(this).balance.sub(initialETHBalance);

        // uint256 devFeesToSend = ethForFees.mul(devFee).div(100);
        // uint256 marketingFeesToSend = ethForFees.mul(marketingFee).div(100);

        // //if there is remaining balance
        // uint256 remainingEthForFees = ethForFees.sub(devFeesToSend).sub(
        //     marketingFeesToSend
        // );
