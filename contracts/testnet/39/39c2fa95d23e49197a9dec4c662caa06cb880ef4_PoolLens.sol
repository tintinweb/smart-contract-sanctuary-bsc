...
contract PoolLens {
        ...
        returns (uint256)
    {
        ...
        if (isStable) {
            ...
            SignedInt memory shortPnl = PositionUtils.calcPnl(Side.SHORT, totalShortSize, averageShortPrice, price);
            SignedInt memory aum = SignedIntOps.wrap(poolAmount).sub(reservedAmount).mul(price).add(guaranteedValue);
            return aum.sub(shortPnl).toUint();
        }
    }
    ...
    function getAssetPoolAum(IPoolForLens _pool, address _token, bool _max) external view returns (uint256) {
        ...
        SignedInt memory sum = SignedIntOps.wrap(0);
        ...
        for (uint256 i = 0; i < nTranches; ++i) {
            ...
            if (isStable) {
                sum = sum.add(asset.poolAmount * price);
            } else {
                uint256 averageShortPrice = _pool.averageShortPrices(_tranche, _token);
                SignedInt memory shortPnl =
                    PositionUtils.calcPnl(Side.SHORT, asset.totalShortSize, averageShortPrice, price);
                sum = SignedIntOps.wrap(asset.poolAmount).sub(asset.reservedAmount).mul(price).add(
                    asset.guaranteedValue
                ).add(sum).sub(shortPnl);
            }
        }
