    ...
    AccessControl
{
    ...
    //设置逻辑合约地址
    function setLogicAddress(address exchangeAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        LOGIC_CONTRACT = exchangeAddress;
    }
     ...
     */
    function logicMint(address to) public returns (uint256) {
        require(msg.sender == LOGIC_CONTRACT, "only call by logic contract");
        uint256 supply = totalSupply();
        require(supply < MAX_SUPPLY, "reach MaxSupply");
     ...
     * 不设置itemId的情况下，递增铸造
     */
    function increaseMint(address to)
        public
        onlyRole(MINTER_ROLE)
        returns (uint256)
    {
