...
// File contracts/WoMPaymentGateway.sol
...
contract WoMPaymentGateway is WoMWithdrawable, EIP712 {
    using SafeERC20 for IERC20;
    ...
    bytes32 public immutable _WITHWoMW_TYPEHASH =
    ...
    function setPaymentChannel(uint256 channelId, address newOrderSigner, address newCashier) external onlyRole(PAYMENT_MANAGER_ROLE) {
        require((newOrderSigner != address(0) && newCashier != address(0))
            || (newOrderSigner == address(0) && newCashier == address(0)), "WoMPaymentGateway: newOrderSigner is address(0) while newCashier is not, and vice versa");
    ...
    function payment(uint256 channelId, uint256 invoiceNo, uint256 deadline, IERC20 tokenContract, uint256 amount, bytes memory signature) external {
        require(block.timestamp <= deadline, "WoMPaymentGateway: expired deadline");
        ChannelData storage paymentChannel = paymentChannels[channelId];
        address orderSigner = paymentChannel.orderSigner;
        address cashier = paymentChannel.cashier;
        require(orderSigner != address(0) && cashier != address(0), "WoMPaymentGateway: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "WoMPaymentGateway: order already paid");
        ...
        require(signer == orderSigner, "WoMPaymentGateway: invalid signature");
    ...
    function withdraw(uint256 channelId, uint256 invoiceNo, uint256 deadline, IERC20 tokenContract, address recipient, uint256 amount, bytes memory signature) external {
        ...
        require(orderSigner != address(0) && cashier != address(0), "WoMPaymentGateway: invalid channelId");
        require(!paymentChannel.usedInvoices[invoiceNo], "WoMPaymentGateway: order already paid");
        bytes32 structHash = keccak256(abi.encode(_WITHWoMW_TYPEHASH, channelId, invoiceNo, deadline, tokenContract, recipient, amount));
        bytes32 hash = _hashTypedDataV4(structHash);
        address signer = ECDSA.recover(hash, signature);
        require(signer == orderSigner, "WoMPaymentGateway: invalid signature");
