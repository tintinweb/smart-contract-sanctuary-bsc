...
import "./interfaces/IVKswap.sol";


contract KswapPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 kswapAtLastUserAction; // keep track of kswap deposited at the last user action.
    ...
    event NewVKswapContract(address VKswap);
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(kswapPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the kswap owner address or the operator address.
     */
    modifier onlyOperatorOrKswapOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or kswap owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onKswapPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function updateUserShare(address _user) internal {
        ...
        if (user.shares > 0) {
            if (user.locked) {
                ...
                totalShares -= user.shares;
                user.shares = 0;
                uint256 earnAmount = totalAmount - user.kswapAtLastUserAction;
    ...
    /**
     * @notice Unlock user kswap funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrKswapOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Kswap Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in KSWAP)
    ...
    function depositOperation(
        ...
        if (VKswap != address(0)) {
            IVKswap(VKswap).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.kswapAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Kswap Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VKswap != address(0)) {
            IVKswap(VKswap).withdraw(msg.sender);
        }
        ...
        if (user.shares > 0) {
            user.kswapAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.kswapAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Harvest pending KSWAP tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingKswap = masterchefV2.pendingKswap(kswapPoolPID, address(this));
        if (pendingKswap > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(kswapPoolPID, 0);
    ...
    /**
     * @notice Set VKswap Contract address
     * @dev Callable by the contract admin.
     */
    function setVKswapContract(address _VKswap) external onlyAdmin {
        require(_VKswap != address(0), "Cannot be zero address");
        VKswap = _VKswap;
        emit NewVKswapContract(VKswap);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Kswap Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingKswapRewards();
     ...
     * @return Returns total pending kswap rewards
     */
    function calculateTotalPendingKswapRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingKswap(kswapPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingKswapRewards()) * (1e18)) / totalShares);
    }
