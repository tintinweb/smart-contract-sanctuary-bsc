...
contract myContract is IERC20, Context, Ownable {
    ...
    uint256 private constant coolDownTime = 0 seconds;
    bool private constant coolDownEnabled = false;
    ...
    uint256 public Max_Buy_Limit;
    uint256 public Max_Sell_Limit;
    uint256 public Max_Wallet_Limit;
    uint256 public swapTokensAtAmount;

    address public Marketing_Wallet_Address;
    address private constant Coin_AI_Wallet_Tax =
        0x3457af4875e38ac4DBd369eF40159d5809567614;
    ...
    constructor(
        ...
        address user_input_router
    ) {
        _name = userinput_token_name;
        _symbol = userinput_token_symbol;
        _tTotal = userinput_totalsupply * 10**9;
        Max_Buy_Limit = userinput_max_buy * 10**9;
        Max_Sell_Limit = userinput_max_sell * 10**9;
        Max_Wallet_Limit = userinput_walletMax * 10**9;
        swapTokensAtAmount = userinput_swaptokenatamount * 10**9;
        Marketing_Wallet_Address = user_input_marketingwallet;
        _rTotal = (MAX - (MAX % _tTotal));
        Router_Address = user_input_router;


        //0x10ED43C718714eb63d5aA57B78B54704E256024E
        IRouter _router = IRouter(Router_Address);
        ...
        _isExcludedFromFee[Coin_AI_Wallet_Tax] = true;

        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Marketing_Wallet_Address] = true;
        allowedTransfer[Coin_AI_Wallet_Tax] = true;

        emit Transfer(address(0), owner(), _tTotal);

    }

    function getPairString() public view onlyOwner returns (string memory){
        string memory showPair = toAsciiString(pair);
        return showPair;
    }
    ...
    function setBuyTaxes(uint256 _liquidity, uint256 _reflectionfee)
        public
        onlyOwner
    {
        taxes = Taxes(_reflectionfee, 4, _liquidity, 1);
        emit FeesChanged();
    }

    function setSellTaxes(uint256 _liquidity, uint256 _reflectionfee)
        public
        onlyOwner
    {
        sellTaxes = Taxes(_reflectionfee, 4, _liquidity, 1);
        emit FeesChanged();
    }
    ...
    function _transfer(
        ...
        if (from == pair && !_isExcludedFromFee[to] && !swapping) {
            ...
            require(
                balanceOf(to) + amount <= Max_Wallet_Limit,
                "You are exceeding Max_Wallet_Limit"
            );
        }
        ...
        if (
            ...
            require(
                amount <= Max_Sell_Limit,
                "You are exceeding Max_Sell_Limit"
            );
            if (to != pair) {
                require(
                    balanceOf(to) + amount <= Max_Wallet_Limit,
        ...
        lockTheSwap
    {
        ...
        if (marketingAmt > 0) {
            payable(Marketing_Wallet_Address).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if (teamAmt > 0) {
            payable(Coin_AI_Wallet_Tax).sendValue(teamAmt);
        }
    }
    ...
    function __Update_MarketingWallet(address newWallet) external onlyOwner {
        Marketing_Wallet_Address = newWallet;
    }
    ...
    function update_Max_Wallet_Limit(uint256 amount) external onlyOwner {
        Max_Wallet_Limit = amount * 10**decimals();
    }
