...
// File: ZeroKnowledgeProof/EBSZKOwnership.sol
...
contract EBSZKOwnership is EBSAddress, EBSPayment, ERC721Holder, Initializable {
    bytes private _ownerSignature;
    bytes private _listSignature;
    ...
    event ChangeKey(
        bytes indexed oldKey,
        bytes indexed newKey
    );
    ...
    modifier onlyZKOperator{
        require(EBS_CONTROL.isZKOperator(msg.sender), "EBSZK-Ownership: Must have ZK_OPERATOR_ROLE");
        _;
    }
    ...
    modifier validCaller(
        ...
        if (EBS_CONTROL.checkKey(key)) require(EBS_CONTROL.isZKOperator(msg.sender), "EBSZK-Ownership: Must have ZK_OPERATOR_ROLE");
        else{
            ...
            require (ECDSA.recover(signerMessage, _ownerSignature) == msg.sender, "EBSZK-Ownership: Invalid caller");
        }
        _;
    }
    ...
    function initialize(
        ...
        uint256 id_,
        uint256 amounts_
    ) external onlyZKOperator initializer {
    ...
    function activate() external onlyZKOperator{
        require(!_activated, "EBSOwnership: Activated");
    ...
    function changeKey(
        ...
        require(_activated, "EBSOwnership: Inactivated");
        bytes memory oldSignature = _ownerSignature;
        _ownerSignature = newSignature;
        _listSignature = abi.encode(bytes32(0));
    ...
    function list(
        ...
        _listSignature = newSignature;

        emit List(newSignature);
    }
