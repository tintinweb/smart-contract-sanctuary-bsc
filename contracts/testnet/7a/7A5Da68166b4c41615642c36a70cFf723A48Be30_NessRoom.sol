...
contract NessRoom is OwnableUpgradeSafe, ReentrancyGuard {
    ...
    function initialize(
        ...
        epochRewardLimit = 100000; // 100k LP DARK/CRO ~ $100k (weekly)
    }
    ...
    function setLockUp(uint256 _withdrawLockupEpochs) external onlyOwner {
        require(_withdrawLockupEpochs <= 56, "withdrawLockupEpochs: out of range"); // <= 2 week
        withdrawLockupEpochs = _withdrawLockupEpochs;
    }

    function setPriceChecker(address _priceChecker) external onlyOwner {
        require(_priceChecker != address(0), "priceChecker address cannot be 0 address");
        priceChecker = _priceChecker;
    }
    ...
    function setWithdrawFee(uint256 _withdrawFee) external onlyOwner {
        require(_withdrawFee <= 1500, "Max withdraw fee is 15%");
        withdrawFee = _withdrawFee;
    }

    function setTargetAPR(uint256 _targetAPR) external onlyOwner {
        require(_targetAPR <= 150000, "Max target APR is 1500%");
    ...
    function setEpochRewardLimit(uint256 _epochRewardLimit) external onlyOwner {
        require(_epochRewardLimit <= 200000, "Limit is too high");
    ...
    function getSharePrice() public view returns (uint256) {
        return IPriceChecker(priceChecker).getLpPriceToUsd(share);
    }

    function getRewardPrice() public view returns (uint256) {
        return IPriceChecker(priceChecker).getTokenPriceToUsd(reward);
    }
