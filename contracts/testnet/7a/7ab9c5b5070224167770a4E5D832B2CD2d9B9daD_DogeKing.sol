...
contract IterableMapping {
    ...
    function set(address key, uint256 val) internal {
        if (inserted[key]) {
            values[key] = val;
        } else {
            inserted[key] = true;
    ...
    IterableMapping
{
        ...
        )
    {
        ...
        if (index >= 0) {
            if (uint256(index) > lastProcessedIndex) {
                ...
                uint256 processesUntilEndOfArray = size() > lastProcessedIndex
                    ? size().sub(lastProcessedIndex)
                    : 0;
    ...
    {
        uint256 numberOfTokenHolders = size();
        ...
        while (gasUsed < gas && iterations < numberOfTokenHolders) {
            ...
            address account = getKeyAtIndex(_lastProcessedIndex);
...
contract DogeKing is ERC20, Ownable {
    ...
    constructor(
        ...
        bytes memory bytecode = type(BABYTOKENDividendTracker).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(address(this)));
        address dividendTracker_;
        assembly {
            dividendTracker_ := create2(
                0,
                add(bytecode, 32),
                mload(bytecode),
                salt
        ...
        dividendTracker = BABYTOKENDividendTracker(dividendTracker_);
    ...
    function setTokenBurnFee(uint256 value) external onlyOwner {
        burnFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(
            burnFee
        );
