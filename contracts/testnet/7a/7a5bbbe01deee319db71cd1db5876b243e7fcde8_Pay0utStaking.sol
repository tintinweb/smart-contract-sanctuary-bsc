...
contract Pay0utStaking is Ownable {
    ...
    constructor (
        address _HustlerToken,
        address _stakedHustlerToken
    ) {
        require(_HustlerToken != address(0), "IA0");
        require(_stakedHustlerToken != address(0), "IA0");
        HustlerToken = _HustlerToken;
        stakedHustlerToken = _stakedHustlerToken;
    }

    event Stake(address indexed user, uint256 amount);
    event Withdraw(address indexed user, uint256 amount);
    event BondStakeHustler(address indexed bond, address indexed user, uint256 amount);

    function stakeHustler(uint256 deposit) public {  //deposit Hustler Token for staking and log timestamp of transaction
        if(startStake[msg.sender] == 0) {
            if(ITREASURY20(treasuryAddress).mintingStatus()) {
                    ...
                    uint256 allowance = IERC20(HustlerToken).allowance(msg.sender, address(this));
                    require(allowance >= deposit, "Check the token allowance");
                    IERC20(HustlerToken).transferFrom(msg.sender, address(this), deposit);
                    IMINT20(HustlerToken).burn(deposit);
                    IMINT20( stakedHustlerToken ).stakeMint( msg.sender, deposit );
    ...
    function unstakeHustler() public { //unstake all Hustler Token from staking
        if(withdrawalsActive) {
            amount = contributions[msg.sender];
            uint256 transfertoken = amount;
            require(IERC20( stakedHustlerToken ).transferFrom( msg.sender, address(this), transfertoken ), "Transfer Failed (No Auth) / Staked balance has been altered");
            IMINT20( stakedHustlerToken ).burn( transfertoken );
            IMINT20(HustlerToken).stakeMint( msg.sender, transfertoken );
            ...
            IMINT20( HustlerToken ).stakeMint( msg.sender, rewardFinal );
    ...
    function addBondContribution(uint256 deposit, address userAddress) public { //Bonds that were staking for the user in the holding period will add the total to the user's contribution and Hustler their owed tokens to start the next period of staking with higher earnings
        ...
        rewardPlus = TotalStake.mul(rewardIndex);
        rewardFinal = rewardPlus.div(10e10); 
        IMINT20( HustlerToken ).stakeMint( msg.sender, rewardFinal ); // Hustler the user's owed tokens from staking
        emit BondStakeHustler(msg.sender, userAddress, rewardFinal); 
        startStake[msg.sender] = currentBlockstamp; // resets stake time
        IMINT20( stakedHustlerToken ).stakeMint( userAddress, deposit ); //Mints staked tokens to user's wallet
