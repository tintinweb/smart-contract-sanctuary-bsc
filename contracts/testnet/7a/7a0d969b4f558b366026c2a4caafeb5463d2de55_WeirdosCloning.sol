...
// File contracts/WeirdosCloning.sol
...
contract WeirdosCloning is ERC721A, Ownable {
    ...
    bytes32 public freeClaimlistMerkleRoot;
    ...
    uint256 public constant MINT_PRICE_APE = 0.1 ether;
    uint256 public constant MINT_DATE = 30 days;

    mapping(address => uint256) public freeClaimAddressToMinted;
    mapping(address => uint256) public creamAddressToMinted;

    constructor(string memory _baseURI)
        ERC721A("TheWeirdos", "TWEIRD")
    {
        baseURI = _baseURI;
    }
    ...
    function creamMint(
        ...
        require(
            creamAddressToMinted[_msgSender()] + count <= allowance,
    ...
    function publicMint(uint256 count) public payable callerIsUser {
        ...
        require(MINT_DATE > block.timestamp, "Public sale is not active");
        require(totalSupply + count < MAX_SUPPLY, "Exceeds public mint supply");
        require(count * MINT_PRICE_ETH == msg.value, "Invalid funds provided");
        _safeMint(_msgSender(), count);
    }

    function withdraw() public {
        require(weirdoBankAddress != address(0),"ZERO BANK ADDRESS");
