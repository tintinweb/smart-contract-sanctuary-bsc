...
contract Auction {
    ...
    mapping(uint => mapping(uint => record)) auctionRecords;

    function getAuctionRecord(uint chainId, uint internalId) public view returns (record memory){
        return auctionRecords[chainId][internalId];
    }
    ...
    function putOnAuction(uint chainId, uint internalId, uint auctionEnd, IERC20 token, uint minPrice) public currentOwner(chainId, internalId) returns (bool){

    	record storage ptr = auctionRecords[chainId][internalId];

    	require(ptr.curBid == 0, "Auction ongoing");
    	require(archive.isTokenAllowed(address(token)), "Token is not allowed");


    	ptr.auctionEnd = block.timestamp + auctionEnd;
    	ptr.token = token;
    	ptr.curBid = minPrice;
    	ptr.winner = msg.sender;
    ...
    function bid(uint chainId, uint internalId, uint q, address api) public returns (bool){
    	record storage ptr = auctionRecords[chainId][internalId];

    	address curOwner = archive.getCurrentOwner(chainId, internalId);

    	require(ptr.curBid != 0, "Not auctioned or someone else bid is processed");
    	require(ptr.auctionEnd >= block.timestamp, "auction finished");
    	require(ptr.curBid < q, "you cannot bid lower than previous");
    	require(curOwner != msg.sender, "you cannot bid for your own nft");

    	if(ptr.winner == curOwner){
    	    // min price
    	    ptr.curBid = 0;
    	}

        // pay previous bidder + (pay api + pay fee + pay royalties + pay to owner)
        Archives.Action[] memory toArch = new Archives.Action[](2);

        toArch[0].a = ptr.curBid;
        toArch[0].b = block.chainid;
        toArch[0].token = ptr.token;
        toArch[0].z = msg.sender;
        toArch[0].x = ptr.winner;


        toArch[1].tp = q - ptr.curBid;
        toArch[1].a = chainId;
    ...
    function release(uint chainId, uint internalId) public returns (bool){

    	record storage ptr = auctionRecords[chainId][internalId];
    	require(ptr.curBid != 0, "Not auctioned");
    	require(ptr.auctionEnd < block.timestamp, "auction is still going");
