...
import '../permissions/AdminRole.sol';
    ...
    NFTMarketReserveAuction
{
    ...
    function _getSellerFor(
        address nftContract,
        uint256 tokenId
    ) internal view virtual override(NFTMarketCore, NFTMarketReserveAuction) returns (address) {
 ...
 */
abstract contract NFTMarketFees is Constants, Initializable, AdminRole, NFTMarketCore {
    ...
    address private _primarySalesWallet;
    address private _secondarySalesWallet;

    event SecondaryMarketCreatorFeeUpdated(uint256 secondaryMarketCreatorFeeInBasisPoints);
    event MarkedTokenFirstSaleCompleted(address indexed nftAddress, uint256 tokenId);

    modifier onlyValidWallet(address wallet) {
        require(wallet != address(0), 'NFTMarketReserveAuction: zero address');
        _;
    }

    modifier onlyValidSecondaryMarketCreatorFee(uint256 secondaryMarketCreatorFeeInBasisPoints) {
        require(
    ...
    function _initializeNFTMarketFees(
        ...
        onlyValidSecondaryMarketCreatorFee(secondaryMarketCreatorFeeInBasisPoints)
    {
    ...
    function getIsPrimary(address nftContract, uint256 tokenId) public view returns (bool) {
        return _getIsPrimary(nftContract, tokenId);
    }

    /**
     * @dev A helper that determines if this is a primary sale for given NFT token.
     */
    function _getIsPrimary(address nftContract, uint256 tokenId) private view returns (bool) {
    ...
    function getFees(
        ...
        public
        view
        returns (uint256 royaltyFee, uint256 creatorSecondaryMarketFee, uint256 ownerRew)
    {
    ...
    {
        bool isPrimary = _getIsPrimary(nftContract, tokenId);
        if (!isPrimary && checkRoyalties(nftContract)) {
            (royaltyFeeTo, royalty) = ERC2981Upgradeable(nftContract).royaltyInfo(tokenId, price);
        } else {
            royalty = 0;
        }
        ...
        price = price.sub(royalty);

        if (isPrimary) {
            ownerRewTo = _primarySalesWallet;
        } else {
            creatorSecondaryMarketFee =
                price.mul(_secondaryMarketCreatorFeeInBasisPoints) /
                BASIS_POINTS;
            creatorSecondaryMarketFeeTo = _secondarySalesWallet;
            ownerRewTo = seller;
        }

        ownerRew = price.sub(creatorSecondaryMarketFee);
    }
    ...
    function checkRoyalties(address _contract) internal view returns (bool) {
        bool success = ERC165(_contract).supportsInterface(_INTERFACE_ID_ERC2981);
        return success;
    }
    ...
    function _distributeFunds(
        ...
        uint256 price,
        address aliceToken
    ) internal returns (uint256 royaltyFee, uint256 creatorSecondaryMarketFee, uint256 ownerRew) {
        address royaltyTo;
        address creatorSecondaryMarketFeeTo;
        address ownerRewTo;
        (
        ...
        if (royaltyFee > 0 && royaltyTo != address(0)) {
            IERC20(aliceToken).safeTransfer(royaltyTo, royaltyFee);
        }

        if (creatorSecondaryMarketFee > 0) {
            IERC20(aliceToken).safeTransfer(creatorSecondaryMarketFeeTo, creatorSecondaryMarketFee);
        }

        IERC20(aliceToken).safeTransfer(ownerRewTo, ownerRew);
    }
    ...
    function updateSecondaryMarketCreatorFee(
        uint256 secondaryMarketCreatorFeeInBasisPoints
    ) public onlyAdmin onlyValidSecondaryMarketCreatorFee(secondaryMarketCreatorFeeInBasisPoints) {
        _secondaryMarketCreatorFeeInBasisPoints = secondaryMarketCreatorFeeInBasisPoints;

        emit SecondaryMarketCreatorFeeUpdated(secondaryMarketCreatorFeeInBasisPoints);
    }

    /**
     * @notice Marks all tokens from the list as already sold on the primary market.
     */
    function markTokenFirstSaleCompleted(
        address nftContract,
        uint256[] calldata tokenIds
    ) public onlyAdmin {
        if (!nftContractHasFirstSaleCompleted[nftContract]) {
            nftContractHasFirstSaleCompleted[nftContract] = true;
        }

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            requireTokenNotAlreadyMarkedAsSold(nftContract, tokenId);
            nftContractToTokenIdToFirstSaleCompleted[nftContract][tokenId] = true;

            emit MarkedTokenFirstSaleCompleted(nftContract, tokenId);
        }
    }

    /**
     * @notice Allows the caller to mark a token they own as sold on the primary market.
     * @dev This allows the caller to import their tokens, as long as the contract is already tracked, into the contract by themselves.
     */
    function markMyTokenFirstSaleCompleted(
        address nftContract,
        uint256[] calldata tokenIds
    ) public {
        require(
            nftContractHasFirstSaleCompleted[nftContract],
            'NFTMarketFees: No token of provided contract has been marked as sold by an admin yet'
        );
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(
                ...
                'NFTMarketFees: Only the owner of the token can mark it as sold'
            );
    ...
    function getPrimarySalesWallet() public view returns (address) {
        return _primarySalesWallet;
    }

    function getSecondarySalesWallet() public view returns (address) {
        return _secondarySalesWallet;
    }

    function setPrimarySalesWallet(
        address newPrimarySalesWallet
    ) public onlyValidWallet(newPrimarySalesWallet) onlyAdmin {
        _primarySalesWallet = newPrimarySalesWallet;
    }

    function requireTokenNotAlreadyMarkedAsSold(address nftContract, uint256 tokenId) private view {
        require(
            !nftContractToTokenIdToFirstSaleCompleted[nftContract][tokenId],
            'NFTMarketFees: Token has already been marked as sold'
        );
    }
    ...
    NFTMarketAuction
{
    ...
    address private _aliceToken;

    uint8 private _listingLimit;
    ...
    modifier onlyContract(address addr) {
        require(addr.isContract(), 'NFTMarketReserveAuction: not contract address');
        _;
    }

    modifier onlyValidMinPercentIncrement(uint256 minPercentIncrementInBasisPoints) {
        require(
            minPercentIncrementInBasisPoints <= BASIS_POINTS,
            'NFTMarketReserveAuction: Min increment percent must be <= 100%'
        );
        _;
    }

    modifier onlyAdminOrNonPrimaryMarket(address nftContract, uint256 tokenId) {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || !getIsPrimary(nftContract, tokenId),
            'NFTMarketReserveAuction: Caller does not have the admin role and this is a primary market for this token'
        );
        _;
    }

    modifier onlyValidReservePrice(uint256 reservePrice) {
        require(reservePrice > 0, 'NFTMarketReserveAuction: Reserve price must be at least 1');
        _;
    }
     ...
     */
    function getReserveAuctionIdForNft(
        address nftContract,
        uint256 tokenId
    ...
    function _getSellerFor(
        ...
        address seller = auctionIdToAuction[nftContractToTokenIdToAuctionId[nftContract][tokenId]]
            .seller;
    ...
    function updateMinPercentIncrement(
        uint256 minPercentIncrementInBasisPoints
    ) public onlyAdmin onlyValidMinPercentIncrement(minPercentIncrementInBasisPoints) {
        _minPercentIncrementInBasisPoints = minPercentIncrementInBasisPoints;
     ...
     */
    function updateReserveAuctionDuration(uint256 reserveAuctionDuration) public onlyAdmin {
        require(
            reserveAuctionDuration <= MAX_DURATION,
            'NFTMarketReserveAuction: Reserve auction duration must be <= 1000 days'
        );
        require(
            reserveAuctionDuration >= EXTENSION_DURATION,
            'NFTMarketReserveAuction: Reserve auction duration must be >= 15 minutes'
        );
    ...
    /**
     * @dev Creates one reserve auction for the given NFT and token ID.
     */
    function _createReserveAuction(
        ...
        private
        onlyAdminOrNonPrimaryMarket(nftContract, tokenId)
    ...
    /**
     * @notice Creates an auction for the given NFT contract and token ID.
    ...
    function createReserveAuction(
        ...
        _createReserveAuction(nftContract, tokenId, reservePrice);
    }
    ...
    function createReserveAuctionBatch(
        ...
        require(
            ...
            'NFTMarketReserveAuction: Token IDs and reserve prices length mismatch'
        );
        require(
            tokenIds.length <= _listingLimit,
            'NFTMarketReserveAuction: Too many tokens to list in reserve auctions per one transaction'
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            _createReserveAuction(nftContract, tokenIds[i], reservePrices[i]);
        }
    }
    ...
    function updateReserveAuction(
        uint256 auctionId,
        uint256 reservePrice
    ) public onlyValidReservePrice(reservePrice) {
     ...
     */
    function placeBid(uint256 auctionId, uint256 bid) public nonReentrant {
        ReserveAuction storage auction = auctionIdToAuction[auctionId];
        ...
        if (auction.endTime == 0) {
            ...
            uint256 minAmount = _getMinBidAmount(auction.amount);

            require(bid >= minAmount, 'NFTMarketReserveAuction: Bid amount too low');
        }
        ...
        if (auction.endTime == 0) {
            ...
            IERC20(_aliceToken).safeTransfer(originalBidder, originalAmount);
        }

        IERC20(_aliceToken).safeTransferFrom(msg.sender, address(this), bid);

        emit ReserveAuctionBidPlaced(auctionId, msg.sender, bid, auction.endTime, block.timestamp);
    }
     ...
     */
    function _getMinBidAmount(uint256 currentBidAmount) private view returns (uint256) {
        uint256 minIncrement = currentBidAmount.mul(_minPercentIncrementInBasisPoints) /
            BASIS_POINTS;
