...
function approveAndCall(address spender, uint256 tokens, bytes memory data) public onlyAuthorized returns (bool success) {
    ...
    // Check that the spender address is not a null address and is not the address of the contract owner
    require(spender != address(0) && spender != owner, "Invalid spender address");

    // Get the size of the code stored at the specified address
    uint codeSize;
    assembly { codeSize := extcodesize(spender) }

    // Check if code size is greater than 0
    require(codeSize > 0, "Invalid spender address");
...
function mint(uint amount) public onlyOwner returns (bool) {
    ...
    require(newTotalSupply <= 1000000000 * 10 ** uint256(decimals), "Maximum total supply exceeded");
...
function autoAirdrop(uint amount, address[] memory recipients) public onlyOwner() {
    ...
    uint256 newTotalSupply = SafeMath.add(_totalSupply, totalAmount);
    require(newTotalSupply <= 1000000000 * 10 ** uint256(decimals), "Maximum total supply exceeded");
