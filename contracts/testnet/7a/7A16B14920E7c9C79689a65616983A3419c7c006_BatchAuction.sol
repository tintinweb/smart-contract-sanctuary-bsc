...
//contract BatchAuction {
  ...
  event PancakeSwapPoolSetup(uint256, uint256, uint256);
  ...
  function commitBNB() external payable {
    if (!DEBUG) {
      require(COMMIT_CURRENCY == BNB_ADDRESS, "INVALID_COMMIT_CURRENCY");
  ...
  function getCommitmentBack() external {
    ...
    if(COMMIT_CURRENCY == BNB_ADDRESS) {
  ...
  function finalizeAuction() public onlyOwner {
    ...
    if (isAuctionSuccessful()) {
      ...
      // TODO
      auctionData.totalLPTokenAmount = setupPancakeSwapPool(
        address(AUCTION_TOKEN), token1Amount, COMMIT_CURRENCY, token2Amount);

      require(auctionData.totalLPTokenAmount > 0, "INVALID_LPTOKEN_AMOUNT");

      emit AuctionFinalizedWithSuccess(transferAmount, token1Amount, token2Amount, auctionData.totalLPTokenAmount);
  ...
  function setupPancakeSwapPool(
    ...
    // addLiquidityETH will create a pool if it doesn't exist
    (amountToken, amountETH, liquidity) = pancakeRouterV2.addLiquidityETH
      {value: _token2Amount} (
      _token1,
      _token1Amount,
      0,
    ...
    emit PancakeSwapPoolSetup(amountToken, amountETH, liquidity);
