...
contract PRUEBAS is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedMax;
    ...
    function excludeFromMAX(address account) public onlyOwner {
        _isExcludedMax[account] = true;
    }
    
    function includeInMAX(address account) public onlyOwner {
        _isExcludedMax[account] = false;
    }      
    ...
    // Tax dev divide x 3 wallet
 
    function _takeDev(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_devWalletAddress] = _rOwned[_devWalletAddress].add(rDev/3);                 //((rDev /3) *1);
        if(_isExcluded[_devWalletAddress])
            _tOwned[_devWalletAddress] = _tOwned[_devWalletAddress].add(tDev/3);

         _rOwned[_MkWalletAddress] = _rOwned[_MkWalletAddress].add(rDev/3);
        if(_isExcluded[_MkWalletAddress])   
            _tOwned[_MkWalletAddress] = _tOwned[_MkWalletAddress].add(tDev/3);

        _rOwned[_ChaWalletAddress] = _rOwned[_ChaWalletAddress].add(rDev/3);
        if(_isExcluded[_ChaWalletAddress])    
            _tOwned[_ChaWalletAddress] = _tOwned[_ChaWalletAddress].add(tDev/3);
    }
    ...
    function BEP20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        // do not allow recovering self token
        //require(tokenAddress != address(this), "Self withdraw");
        IERC20(tokenAddress).transfer(_devWalletAddress, tokenAmount);
    }
