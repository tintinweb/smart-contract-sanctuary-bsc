...
contract StakeVault is ReentrancyGuard, Ownable {
    ...
    mapping(address => address) public getPositionForAsset;

    mapping(address => uint256) public getIdForAsset;
    ...
    function registerAsset(
        ...
        address stakePositionAsset = IStakePosition(_stakePosition)
            .getStakeToken
            .address;
        require(
            stakePositionAsset == _stakeToken,
            "Must have correct stake token and position addresses."
        );
        ...
        getPositionForAsset[_stakeToken] = _stakePosition;
        getIdForAsset[_stakeToken] = stakeAssets.length - 1;
    }

    // function updateAsset(address _stakeToken, address _stakePosition) external {
    //     address stakePositionAsset = IStakePosition(_stakePosition)
    //         .getStakeToken
    //         .address;
    //     require(
    ...
    //     require(
    //         positionForAsset[_stakeToken] != address(0),
    //         "Create new position"
    //     );
    ...
    function checkStakeDuplicate(address _token) internal view {
        uint256 length = stakeAssets.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            require(stakeAssets[pid].stakeToken != _token, "Existing Token");
        }
    }
