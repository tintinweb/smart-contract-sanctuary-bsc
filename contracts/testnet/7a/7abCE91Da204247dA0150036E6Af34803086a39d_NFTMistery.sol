...
contract NFTMistery is AccessControl, ERC721URIStorage {
    ...
    address internal owner = 0x30268390218B20226FC101cD5651A51b12C07470;

    bool public paused = false;
    bool public mintBox = true;

    constructor(
        address _manager,
        uint256 _maxsupply,
        uint256 _maxBoxSupply,
        string memory name_,
        ...
        _setupRole(MINTER_ROLE, 0x30268390218B20226FC101cD5651A51b12C07470);
        /*-------------*/

        _setupRole(MINTER_ROLE, _manager);
        maxsupply = _maxsupply;
        maxBoxSupply = _maxBoxSupply;
        boxSupply = _maxsupply;
        token  = _token;
        amount = _amount;
    }
    ...
    function getBoxSupply() public view returns (uint256) {
        return boxSupply - maxsupply;
    }
    ...
    function getPriceToken() public view returns (address,uint256) {
        return(token, amount);
    }
    ...
    function redeem(address _redeem,string memory _uri) public returns (uint256) {
    uint256 _tokenid;
    ...
    function Minting(address _redeem,string memory _uri) public returns (uint256) {
    uint256 _tokenid;
    require(!paused, "is paused");
    require(hasRole(MINTER_ROLE, msg.sender), "caller is not a minter");
    if(mintBox){
    ...
    function openBox(address _redeem,uint256 _idBox, string memory _uri) public returns(uint256){
        require(!paused, "is paused");
        require(hasRole(MINTER_ROLE, msg.sender), "caller is not a minter");
