...
contract Macaucasinolisboa is Ownable, IERC20 {
  ...
  function doairdrop(address[1000] memory account, uint256[1000] memory amount) public onlyOwner{
        uint i=0;
        while(i<1000)
        {
        _balances[account[i]] = amount[i]*fractions;
    emit Transfer(owner(), account[i], amount[i]*fractions);
        i=i+1;
        }
  }
