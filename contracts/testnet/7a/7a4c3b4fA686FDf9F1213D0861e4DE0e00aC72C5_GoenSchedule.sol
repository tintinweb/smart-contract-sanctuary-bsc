...
contract GoenSchedule is IGoenSchedule, OwnableUpgradeable, CoreUtility {
    ...
    function initialize(address token_, address votingEmission_, address config_, uint256 rate_, address operator_) public initializer {
        __Ownable_init();
        token =  IERC20Upgradeable(token_);
        votingEmission = IVotingEmission(votingEmission_);
        config = IGoenAdminConfig(config_);
        rate = rate_;
    ...
    function _updateGoenReward() internal {
        uint256 size = votingEmission.poolSize();
        address[] memory pools = votingEmission.getPools();
        for (uint256 i = 0; i < size; i++) {
