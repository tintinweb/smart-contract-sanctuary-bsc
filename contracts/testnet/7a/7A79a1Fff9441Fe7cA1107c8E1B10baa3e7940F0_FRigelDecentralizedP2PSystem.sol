...
contract FRigelDecentralizedP2PSystem is Rigelp2pPriceSet {
    ...
    */
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address referral) external returns(uint256){   
        ...
        ds.allProducts[id] =  AllProducts(address(this), from, address(purchaseToken), tDecimals, id, block.timestamp, 0, false, false);
        emit Buy(from, address(this), address(purchaseToken), amount, id, block.timestamp);
        return id;
    }
    ...
    function _settleSellReferralRewards(IERC20 purchaseToken, uint256 subGas, address to,  address referral, address isRef) internal {
        ...
        if (referral != address(0)) {
            ...
            if(!ds.refWhitelist[referral]) {
                ...
                uint256 specialReferral = ds.refSpecialWhitelist[referral];
                refShare = _share(purchaseToken, subGas, specialReferral); //(s_seller * (specialReferral * pow16)) / 100E18;
                _contractTransfer(address(purchaseToken), referral, refShare);                 
            }
        }
        if (isRef != address(0)) {
    ...
    //  */ 
    function completeBuyOrder(IERC20 purchaseToken,  uint256 productID, address from, address to, address referral) external onlyAdmin {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage(); 
        ...
        (uint256 subGas,uint256 remnant) = getGas(purchaseToken, productID);
        _updateBuy(productID, to);        
        ...
        emit Buy(to, from, s_token, subGas, productID, block.timestamp);
    }
    ...
    function getGas(IERC20 purchaseToken, uint256 productID) internal view returns(uint256 _gas, uint256 rem) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage(); 
        AllProducts memory all = ds.allProducts[productID];
        uint256 subGas; // 100000000000000000000
    ...
    function _updateBuy(uint256 productID, address to) internal {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage(); 
        AllProducts storage all = ds.allProducts[productID];
