...
import "./IHexaFinityRouter01.sol";

interface IHexaFinityRouter02 is IHexaFinityRouter01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        address token,
        uint256 liquidity,
        uint256 amountTokenMin,
...
library HexaFinityLibrary {
    using SafeMath for uint;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'HexaFinityLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'HexaFinityLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'fcf989d6de84e449394298b443e385fb42cc5eadab6d7e7e151c60cbd83f8af7' // init code hash
            ))));
    }
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IHexaFinityPair(pairFor(factory, tokenA, tokenB)).getReserves();
        (reserveA, reserveB) = tokenA == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
    }

    // given some amount of an asset and pair reserves, returns an equivalent amount of the other asset
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'HexaFinityLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'HexaFinityLibrary: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'HexaFinityLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'HexaFinityLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'HexaFinityLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'HexaFinityLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'HexaFinityLibrary: INVALID_PATH');
...
import '../interfaces/IHexaFinityRouter01.sol';
...
contract RouterEventEmitter {
    ...
    function swapExactTokensForTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapExactTokensForTokens.selector, amountIn, amountOutMin, path, to, deadline
        ));
        assert(success);
    ...
    function swapTokensForExactTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapTokensForExactTokens.selector, amountOut, amountInMax, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
    ...
    function swapExactETHForTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapExactETHForTokens.selector, amountOutMin, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
    ...
    function swapTokensForExactETH(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapTokensForExactETH.selector, amountOut, amountInMax, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
    ...
    function swapExactTokensForETH(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapExactTokensForETH.selector, amountIn, amountOutMin, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
    ...
    function swapETHForExactTokens(
        ...
        (bool success, bytes memory returnData) = router.delegatecall(abi.encodeWithSelector(
            IHexaFinityRouter01(router).swapETHForExactTokens.selector, amountOut, path, to, deadline
        ));
        assert(success);
        emit Amounts(abi.decode(returnData, (uint[])));
    }
