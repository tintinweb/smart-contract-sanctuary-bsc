...
contract ACMIDO is Ownable, ReentrancyGuard {
    ...
    function addRelationEx(address recommer) external nonReentrant {
        ...
        if (stat) {
            IERC20(USD).transfer(father, 10 * 10**18);
    ...
    function addTokenContract(address tokenContract, address tokenRouter) external onlyOwner {
        acmClaim.addTokenContract(tokenContract, tokenRouter);
    }
    ...
    function setAcmClaimOwnerAndRelationOwner(address addr) external onlyOwner {
        acmClaim.transferOwnership(addr);
        relation.transferOwnership(addr);
    }
}