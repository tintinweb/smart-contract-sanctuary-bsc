...
// File: contracts/interfaces/IICCLottery.sol

pragma solidity ^0.8.4;

interface IICCLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in ICC per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in ICC token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInICC: price of a ticket in ICC
 ...
 */
contract ICCLottery is ReentrancyGuard, IICCLottery, Ownable {
    ...
    uint256 public maxNumberTicketsPerBuyOrClaim = 1_000_000;

    uint256 public maxPriceTicketInICC = 50 ether;
    uint256 public minPriceTicketInICC = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInICC;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] iccPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInICC;
        uint32 finalNumber;
    }
     ...
     */
    constructor(address _iccToken, address _randomGeneratorAddress) {
        iccToken = IERC20(_iccToken);
        ...
        nonReentrant
    {
        ...
        // Calculate number of ICC to this contract
        uint256 amountICCToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInICC,
            _ticketNumbers.length
        );

        // Transfer ICC tokens to this contract
        iccToken.safeTransferFrom(address(msg.sender), address(this), amountICCToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInICC += amountICCToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInICCToTransfer
        uint256 rewardInICCToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInICCToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        iccToken.safeTransfer(msg.sender, rewardInICCToTransfer);

        emit TicketsClaim(msg.sender, rewardInICCToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInICC) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in ICC for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].iccPerBracket[j] =
                ...
                // A. No ICC to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].iccPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInICC - amountToShareToWinners);

        // Transfer ICC to treasury address
        iccToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        iccToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInICC += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInICC >= minPriceTicketInICC) && (_priceTicketInICC <= maxPriceTicketInICC),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInICC: _priceTicketInICC,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            iccPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInICC: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(iccToken), "Cannot be ICC token");
    ...
    /**
     * @notice Set ICC price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInICC: minimum price of a ticket in ICC
     * @param _maxPriceTicketInICC: maximum price of a ticket in ICC
     */
    function setMinAndMaxTicketPriceInICC(uint256 _minPriceTicketInICC, uint256 _maxPriceTicketInICC)
        external
        onlyOwner
    {
        require(_minPriceTicketInICC <= _maxPriceTicketInICC, "minPrice must be < maxPrice");

        minPriceTicketInICC = _minPriceTicketInICC;
        maxPriceTicketInICC = _maxPriceTicketInICC;
    }
     ...
     * @param _priceTicket price of a ticket (in ICC)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].iccPerBracket[_bracket];
        } else {
            return 0;
        }
    }
