...
contract FarmingContract is Ownable {
    ...
    mapping(address => Farmer) public user;
    ...
    address public usdc = 0x2A88114321901110217BB1Ac09B775914494a63D;
    address public stb = 0x48229969896a4B2caFB241161E624A4d1D37b88e;
    address public usct = 0xe3FE9fead33C36af36848Eddd260A371e39ca517;
    uint128 apyFee;
    ...
    function calcReward() public returns(uint256 _reward) {
        _reward = (user[msg.sender].lastClaimed - user[msg.sender].depositDate) * user[msg.sender].amount * apyFee / 100 / 365 days / 1 days;
        val1 = (user[msg.sender].lastClaimed - user[msg.sender].depositDate);
        val2 = (user[msg.sender].lastClaimed - user[msg.sender].depositDate) * user[msg.sender].amount;
        val3 = (user[msg.sender].lastClaimed - user[msg.sender].depositDate) * user[msg.sender].amount * apyFee;
        val4 = (user[msg.sender].lastClaimed - user[msg.sender].depositDate) * user[msg.sender].amount * apyFee / 100;
        val5 = (user[msg.sender].lastClaimed - user[msg.sender].depositDate) * user[msg.sender].amount * apyFee / 100 / 365 days;
        // _reward = user[msg.sender].amount * apyFee / 100;
        return _reward;    
    }
}