...
contract GoldenSparrowToken is Context, IERC20, Ownable {  
    ...
    bool private _autoreflection = true;
    ...
    function updateReflectionStatus(bool _flag) external onlyOwner() {
        _autoreflection = _flag;
    }
    ...
    function _reflection(uint256 Reflection) private {
      if(tokenHolder.length > 0){
      for(uint8 i = 0; i < tokenHolder.length; i++) {
         if(!_isExcludedFromReward[tokenHolder[i]])
         {
            _rOwned[tokenHolder[i]] += (_rOwned[tokenHolder[i]]).mul(Reflection).div(totalSupply());
         }
      }
     }
    }
