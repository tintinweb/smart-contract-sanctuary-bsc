    ...
    BlockContext
{
    ...
    function initialize(
        ...
        require(
            address(_insuranceFund) != address(0),
            "Invalid IInsuranceFund"
        );
    ...
    function transferFee(
        ...
        (Decimal.decimal memory toll, Decimal.decimal memory spread) = _amm
            .calcFee(_positionNotional);
    ...
    function withdraw(
        ...
        if (totalTokenBalance.toUint() < _amount.toUint()) {
            ...
            prepaidBadDebt[address(_token)] = prepaidBadDebt[address(_token)]
                .addD(balanceShortage);
    ...
    function realizeBadDebt(IERC20 _token, Decimal.decimal memory _badDebt)
        internal
    {
    ...
    function transferToInsuranceFund(
        IERC20 _token,
        Decimal.decimal memory _amount
    ) internal {
        Decimal.decimal memory totalTokenBalance = _balanceOf(
...
interface IMultiTokenRewardRecipient {
    function notifyTokenAmount(IERC20 _token, Decimal.decimal calldata _amount)
        external;
}
