...
function Donation(uint256 _planId, uint256 _timestamp) public virtual;
...
function transferTrx(address payable _to,uint256 _planId) public payable {
 (bool sent,) = _to.call{value: plans[_planId].HelpAmount }("");
...
contract TronExpert is TronAbstract{
    ...
    function referralDonation(uint256 _planId, uint256 _timestamp, address _referrer) public {
        ...
        transferTrx(payable(_ghAddress),_planId);
       
        if(plan.HelpHash[index].gotHelp1 == address(0)){
            plan.HelpHash[index].gotHelp1= _ghAddress;
        }
        else{
            plan.HelpHash[index].gotHelp2= _ghAddress;
        }
    ...
    function Donation(uint256 _planId, uint256 _timestamp) public override {
        ...
        // require(_ghAddress != address(0), "Sorry, Help address cannot be zero!!");
        // require(!isActive(_ghAddress,_planId),"You are already active in this plan");
