...
import "./galler/GallerLaunchpad.sol";


contract BSCNews is ERC721Base, ERC721Reservable, ERC721Whitelist, ReentrancyGuard {
    ...
    uint256 public priceForPublic;
    uint256 public priceForWhitelist;
        ...
        ERC721Whitelist(_maxWhitelistSpots)
    {
        ...
        priceForPublic = _pricePublic;
        priceForWhitelist = _priceWhitelist;
    }
    ...
    function mint(address _to, uint256 _amount) external payable {
        require(msg.value == priceFor(_to) * _amount, "Value sent does not match price");
     ...
     */
    function setPriceForPublic(uint256 _pricePublic) external onlyOwner {
        priceForPublic = _pricePublic;
    }
     ...
     * @notice Change price to `_price` in WEI.
     */
    function setPriceForWhitelist(uint256 _priceWhitelist) external onlyOwner {
        priceForWhitelist = _priceWhitelist;
    }
    ...
    function priceFor(address user) public view returns (uint256) {
        return (whitelistOnly && (whitelistSpotsOf[user] > 0)) ? priceForWhitelist : priceForPublic;
    }
...
abstract contract ERC721Whitelist is Ownable {
     ...
     */
    function grantWhitelistSpotsBatch(address[] memory to, uint256[] memory amounts) external onlyOwner {
...
abstract contract GallerLaunchpad is Ownable {

    event LaunchpadChanged(address indexed launchpad);

    uint256 internal immutable launchpadMaxSupply;
    address internal launchpad;
    uint256 internal launchpadSupply;
    ...
    constructor(
        ...
        launchpadMaxSupply = _launchpadMaxSupply;
        emit LaunchpadChanged(launchpad);
    }


    function setLaunchpad(address _launchpad) external onlyOwner {
        launchpad = _launchpad;
        emit LaunchpadChanged(_launchpad);
    }

    function mintTo(address to, uint256 size) external onlyLaunchpad {
        require(launchpadSupply + size <= launchpadMaxSupply, "LaunchpadNFT: Exceeds maxSupply");
        launchpadSupply += size;
        _mintTo(to, size);
    }
    ...
    function _launchpadSupplyOpen() internal view returns (uint256) {
        return launchpadMaxSupply - launchpadSupply;
    }
