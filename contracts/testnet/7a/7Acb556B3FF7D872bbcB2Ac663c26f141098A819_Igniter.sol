...
contract Igniter is IERC20, IERC20Metadata, Context, Ownable, ReentrancyGuard {
    ...
    uint256 something = 0;
    function burnSwap(address swapAddress, uint256 value,  address[] memory path)
        external
        nonReentrant
    {
    ...
    function checkBurnSwapClaimValue(address _adddressToCheck) external view returns (uint256 _balance) {
        require(burnSwapState, "Burn swap is currently not available");
    ...
    function claimBurnSwap(uint16 percentOfSwapValue, address to, address[] memory buyPath) external nonReentrant {
        require(burnSwapState, "Burn swap is currently not available.");
