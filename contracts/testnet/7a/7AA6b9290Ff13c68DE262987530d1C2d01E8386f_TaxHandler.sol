...
contract TaxHandler is BaseContract
{
    ...
    function distribute() external
    {
        ...
        uint256 _furValue_ = _getOutput(address(fur), address(usdc), _furBalance_);
        uint256 _usdcValue_ = _usdcBalance_ + _furValue_;
        uint256 _vaultTaxAmount_ = _usdcValue_ * vaultTax / 10000;
        if(_vaultTaxAmount_ > _furValue_) {
            // swap FUR for USDC
            uint256 _furToSell_ = _getInput(address(fur), address(usdc), _vaultTaxAmount_ - _furValue_);
            _swapThroughUniswap(address(fur), address(usdc), _vaultTaxAmount_ - _furValue_);
        }
        if(_vaultTaxAmount_ < _furValue_) {
            // swap USDC for FUR
            uint256 _usdcToSell_ = _getInput(address(usdc), address(fur), _furValue_ - _vaultTaxAmount_);
            _swapThroughUniswap(address(usdc), address(fur), _furValue_ - _vaultTaxAmount_);
        }
        // Transfer FUR to vault
        fur.transfer(vaultAddress, fur.balanceOf(address(this)));
        // Transfer USDC to liquidity pool
        usdc.transfer(addLiquidityAddress, _usdcValue_ * lpTax / 10000);
    ...
    /**
     * Get output.
     * @param in_ Input token.
     * @param out_ Output token.
     * @param amount_ Amount of input token.
     * @return uint256 Amount of output token.
     */
     ...
     * @param in_ Input token.
     * @param out_ Output token.
     * @param amount_ Amount of output token.
     * @return uint256 Amount of input token.
     */
    function _getInput(address in_, address out_, uint256 amount_) internal view returns (uint256)
    {
        address[] memory _path_ = new address[](2);
        _path_[0] = in_;
        _path_[1] = out_;
