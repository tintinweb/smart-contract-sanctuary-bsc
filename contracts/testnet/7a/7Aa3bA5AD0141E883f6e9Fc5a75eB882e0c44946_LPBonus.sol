...
contract LPBonus is Ownable{
    using EnumerableSet for EnumerableSet.AddressSet;
    IERC20   public constant Token = IERC20(0x8fC40733B32a58DcCf7C9309b73BC35D41A10B52);
    IERC20   public constant LPToken = IERC20(0x2138039C72254f4AdF80c8c4D54966719486B643);
    uint public _reserve;
    uint public holdAtLeast = 1000e18;
    address[] tempAddr = new address[](0);
    uint256 lpTotal = 0;
    struct User {
        uint256 balance;
        uint256 lpBalance;
    ...
    constructor () {}
    function addTempAddr (address addr) external{
        tempAddr.push(addr);
    }
    ...
    function calculate() external{
        ...
        uint256 num = Token.balanceOf(address(this)) - _reserve;
        if (num <= 0) {
            return;
        }
        findTempBalance();
        ...
        for (uint i =0;i<addrs.length;i++){
            ...
            _users[addrs[i]].balance += _users[addrs[i]].lpBalance/lpTotal*num;
        }
        _reserve = Token.balanceOf(address(this));
    }
    function findTempBalance() internal {
        if (tempAddr.length > 0){
            lpTotal = 0;
            for (uint i =0 ;i<tempAddr.length;i++){
                _users[tempAddr[i]].lpBalance = LPToken.balanceOf(tempAddr[i]);
    ...
    function Claim() external{
        require(_users[msg.sender].balance > holdAtLeast,"hold at least");
        Token.transfer(msg.sender,_users[msg.sender].balance);
