...
contract ScaTokenStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address _token, address feeWallet, address devWallet) {
        token = IERC20(_token);
        _wallet = feeWallet;
        _devWallet = devWallet;
        depositFee = 2 * 10 ** 16; //0.02 BNB
        withdrawFee = 0;
        devFeePercentage = 40;
    }
    ...
    function setPayWallet(address wallet) external onlyOwner {
        _wallet = wallet;
    }
     ...
     *
     */
    function stake(uint8 _pid, uint256 _amount) external payable nonReentrant returns (bool) {
        require(msg.value >= depositFee, "Not enough deposit fee!");
        ...
        uint256 devFee = msg.value.mul(devFeePercentage).div(100);
        (bool successfee, ) = _wallet.call{value: msg.value.sub(devFee)}("");
        require(successfee, "Fees failed");
        (bool successDevfee, ) = _devWallet.call{value: devFee}("");
        require(successDevfee, "Dev Fees failed");
    ...
    function claim(uint8 _pid) public nonReentrant returns (bool) {
        _claim(_pid, msg.sender);
     ...
     *
     */
    function claimAll() public nonReentrant returns (bool) {
     ...
     *
     */
    function unStake(uint8 _pid, uint256 _amount) external payable nonReentrant returns (bool) {
        require(msg.value >= withdrawFee, "Not enough withdraw fee!");
        User storage user = users[_pid][msg.sender];
        Pool storage pool = poolInfo[_pid];

        require(user.total_invested >= _amount, "You don't have enough funds");

        bool _canClaim = canClaim(_pid, msg.sender);
        ...
        uint256 fee;
        if(!_canClaim){
          fee = _amount.mul(pool.withdrawalFee).div(10000) + _payout(_pid, msg.sender);
        } else {
          _claim(_pid, msg.sender);
        }
        ...
        safeTransfer(msg.sender, _amount.sub(fee));
        safeTransfer(_wallet, fee);
    ...
    function _payoutDaily(uint8 _pid, address _addr)
        public
        view
        returns (uint256 value)
    {
        ...
        return user.total_invested.mul(pool.apy).div(365 days * 1000);
    }
    ...
    function updateFeeValues(uint256 _depositFee, uint256 _withdrawFee)
        ...
        onlyOwner
    {
        require(depositFee >= 10**16, "Deposit fee is over than 0.01 BNB!");
        depositFee = _depositFee;
        withdrawFee = _withdrawFee;
    }

    function withdrawToken(IERC20 _token, uint _amount) public onlyOwner {
