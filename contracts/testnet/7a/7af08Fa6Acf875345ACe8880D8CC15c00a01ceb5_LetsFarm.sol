...
contract LetsFarm is Upgradeable, SignerRecover, IERC721ReceiverUpgradeable {
    ...
    function _claimRewardsInternal(
        ...
        uint256[2] memory maxWithdrawals = getMaxWithdrawalNew(
            msg.sender,
            false,
            false
        );

        if (toTransferHpw > maxWithdrawals[1]) {
            toTransferHpw = maxWithdrawals[1];
            _hpwRewards = _user.hpwRewardClaimed + toTransferHpw;
        }

        if (toTransferHpl > maxWithdrawals[0]) {
            toTransferHpl = maxWithdrawals[0];
    ...
    function masterDistributeRewards(
        uint256[3] memory _rewardsAndTime, // total [hpl, hpw]
        ...
        require(
            block.timestamp < _rewardsAndTime[2],
    ...
    function _masterDistributeRewardsInternal(
        uint256[3] memory _rewards, //total
        address[] memory _scholarAddresses,
        uint256[] memory _commissions,
        uint256[] memory _scholarHPLSpents, //total spent by this scholar
        uint256[] memory _scholarHPWSpents, //total spent by this scholar
        ...
        if (toTransferHpl > maxWithdrawals[0]) {
            toTransferHpl = maxWithdrawals[0];
            _rewards[0] = _user.hplRewardClaimed + toTransferHpl;
        }

        if (toTransferHpw > maxWithdrawals[1]) {
            toTransferHpw = maxWithdrawals[1];
            _rewards[1] = _user.hpwRewardClaimed + toTransferHpw;
        }
        ...
        for (uint256 i = 0; i < _scholarAddresses.length; i++) {
            ...
            if (_scholarHPLAmounts[i] > _scholarHPLSpents[i]) {
                ...
                if (
                    ...
                    scholarRewards[_scholarAddresses[i]]
                        .totalHPLClaimedForThisScholar +=
                        (_scholarClaimable * 100) /
                        _commissions[i];
    ...
    function updateHPLRewards(address _master, uint256 _rewards)
        external
        onlyOwner
    {
        userInfo[_master].hplRewardClaimed = _rewards;
    }

    function updateHPWRewards(address _master, uint256 _rewards)
        external
        onlyOwner
    {
        userInfo[_master].hpwRewardClaimed = _rewards;
    }
    ...
    function getMaxWithdrawal(
        ...
        uint256 _maxPerLand = maxWithdrawPerLand != 0
            ? maxWithdrawPerLand
            : (3000 ether);
        uint256 _maxHPLPerLand = _maxPerLand / 6;
        uint256 _maxPerWildLand = maxWithdrawPerWildLand != 0
            ? maxWithdrawPerWildLand
            : (800 ether);
        uint256 _maxHPLPerWildLand = _maxPerLand / 10;
        if (_forGuild) {
            if (normalLand < 25) {
                _maxPerLand = 2500 ether;
            }
        }
        uint256 maxWithdrawal = (normalLand *
            ...
            wildLandCount *
            _maxPerWildLand);
        uint256 maxWithdrawalHPL = (normalLand *
            _maxHPLPerLand +
            wildLandCount *
            _maxHPLPerWildLand);
        ...
        if (!_forGuild) {
            ...
            if (maxWithdrawalHPL > 2000 ether) {
                maxWithdrawalHPL = 2000 ether;
            }
        }

        // //check hpw staked
        // {
        //     address masterChefAddress = 0x84acfDc85F652bb85817b8C70fE80251A2Ada1b3;
        //     if (getChainId() == 56) {
        //         masterChefAddress = 0x69C01f0bef6123a92248Cc4F35638760F9059497;
        //     }
        //     (uint256 _hpwStaked,,,,) = IMasterChef(masterChefAddress).getUserInfo(1, _user);
        //     uint256 _maxWithdrawByStake = 500 ether;
        //     if (_hpwStaked > 0) {
        //         _maxWithdrawByStake = _hpwStaked;
        //     }

        //     if (maxWithdrawal > _maxWithdrawByStake) {
        //         maxWithdrawal = _maxWithdrawByStake;
        //     }
        // }

        return [maxWithdrawalHPL, maxWithdrawal];
    }

    function getMaxWithdrawalNew(
        address _user,
        bool _tightCheck,
        bool _forGuild
    ) public view returns (uint256[2] memory) {
        address masterChefAddress = getMasterChefAddress();

        (, , , , IMasterChef.TokenDeposit[] memory deposits) = IMasterChef(
            masterChefAddress
        ).getUserInfo(1, _user);

        //compute withdrawalable hpw amount
        uint256 maxWithdrawalByStake = 1000 ether;
        for (uint256 i = 0; i < deposits.length; i++) {
            uint256 remainingLockedTime = 0;
            if (deposits[i].lockedUntil > block.timestamp) {
                remainingLockedTime = deposits[i].lockedUntil - block.timestamp;
            }
            remainingLockedTime = 1 + (remainingLockedTime / 86400);
            if (remainingLockedTime > 14) {
                remainingLockedTime = 14; //14 days
            }
            maxWithdrawalByStake += ((deposits[i].tokenAmount * remainingLockedTime) /
                14);
        }
        ...
        if (maxWithdrawal > maxWithdrawalByStake) {
            maxWithdrawal = maxWithdrawalByStake;
        }
    ...
    function isLandDeposit(address _user, uint256 _tokenId)
        external
        view
        returns (bool)
    {
        address _land = getLandContract();
        return nftUserInfo[_land][_user].tokenIdToIndex[_tokenId] > 0;
    }
}
