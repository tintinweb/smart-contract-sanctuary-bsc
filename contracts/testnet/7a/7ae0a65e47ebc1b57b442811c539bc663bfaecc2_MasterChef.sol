...
interface ISyrupBar {
    ...
    function safeTvcTransfer(address _to, uint256 _amount) external;
}
...
contract MasterChef is Ownable, ReentrancyGuard, Whitelist {
    ...
    // Bonus muliplier for early TVC makers.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. TVC to distribute per block.
        uint256 lastRewardBlock;  // Last block number that TVC distribution occurs.
        uint256 accTvcPerShare;   // Accumulated TVC per share, times 1e12. See below.
    }
    ...
    // The TVC TOKEN!
    ITvcToken public tvcToken;
    // The SYRUP TOKEN!
    ISyrupBar public syrup;

    // TVC tokens created per block.
    ...
    // The block number when TVC mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 tvcPerBlock);
    ...
    // View function to see pending TVC on frontend.
    function pendingTvc(uint256 _pid, address _user) external view returns (uint256) {
        
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accTvcPerShare = pool.accTvcPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 tvcReward = multiplier.mul(tvcPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTvcPerShare = accTvcPerShare.add(tvcReward.mul(1e12).div(lpSupply));
        }
        uint pendingRewards = user.amount.mul(accTvcPerShare).div(1e12).sub(user.rewardDebt);
        if (tvcToken.isMintable(pendingRewards)) {
    ...
    // Update the given pool's TVC allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (tvcToken.isMintable(tvcReward)) {
            tvcToken.mint(address(syrup), tvcReward);
        }
        if (tvcToken.isMintable(tvcReward.div(10))) {
            tvcToken.mint(devAddr, tvcReward.div(10));
        }

        pool.accTvcPerShare = pool.accTvcPerShare.add(tvcReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for TVC allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant nonContract {
        require(tvcToken.isMintable(0), "Farming has been ended.");
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTvcPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
                safeTvcTransfer(msg.sender, pending, _pid);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTvcPerShare).div(1e12);
    ...
    function enterStaking(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTvcPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                syrup.safeTvcTransfer(msg.sender, pending);
            }
        }
        if(_amount > 0) {
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Safe TVC transfer function, just in case if rounding error causes pool to not have enough TVC.
    function safeTvcTransfer(address _to, uint256 _amount, uint256 _pid) internal {
        uint256 boost = 0;
        syrup.safeTvcTransfer(_to, _amount);
        if (enableNFTBoost) {
            boost = getBoost(_to, _pid).mul(_amount).div(100);
            if (boost > 0) {
                if (tvcToken.isMintable(boost)) {
                    tvcToken.mint(_to, boost);
                }
            }
        }
    }
    ...
    function updateEmissionRate(uint256 _tvcPerBlock) public onlyOwner {
        massUpdatePools();
        tvcPerBlock = _tvcPerBlock;
        emit UpdateEmissionRate(msg.sender, _tvcPerBlock);
    }
