...
contract EcpsMarket  {
    ...
    */
     ...
     * 结束拍卖
     */
     function markupEnd(uint _tokenId, address[] memory bidAddress) external  {
        require(!commodityList[_tokenId].ended, "Auction has ended");
        commodity = commodityList[_tokenId];
        ...
        if(commodityList[_tokenId].coin == 1){
            for (uint i = 0; i < bidAddress.length; i++) {
                COP.transferFrom(address(this), bidAddress[i], bidMap[_tokenId][bidAddress[i]]);
            }
        } else if(commodityList[_tokenId].coin == 2){
            for (uint i = 0; i < bidAddress.length; i++) {
                ROC.transferFrom(address(this), bidAddress[i], bidMap[_tokenId][bidAddress[i]]);
            }
        }
