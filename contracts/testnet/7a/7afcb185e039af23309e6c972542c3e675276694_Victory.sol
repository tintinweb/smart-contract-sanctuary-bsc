...
contract Victory is IERC20, Ownable , AccessControl ,Pausable{
    ...
    function setBallot(address node, uint256 amount) external blackList whenNotPaused {
        ...
        cooldownTimer[msg.sender] = block.timestamp + 432000;//lock 5 days
    ...
    function _takeLpFee(address sender,uint256 tAmount) private {
        ...
        if( !isBallot[sender] || setBallotAddr[sender] == address(0) ) {
            _basicTransfer(sender, address(0), tAmount);
            return;
        }
    ...
    function _transferStandard(
        ...
        _takeLpFee(sender, tAmount.div(10000).mul(_lpBallotFee));
