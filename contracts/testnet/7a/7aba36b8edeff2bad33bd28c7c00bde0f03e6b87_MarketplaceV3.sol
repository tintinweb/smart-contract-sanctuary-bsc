...
// File contracts/marketplace/interfaces/IOnRamp.sol
...
interface IOnRamp {
  ...
  function slippageMinValueOf(uint256 _amountIn) external view returns (uint256);

  function slippageMaxValueOf(uint256 _amountIn) external view returns (uint256);

  function swapFeeOf(uint256 _amountIn) external view returns (uint256);
...
contract MarketplaceV3 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
  ...
  function _convertBusdToMht(uint256 price) internal {
    uint256 busdPrice = onRamp.BusdAmountOut(price);
    uint256 swapFee = onRamp.swapFeeOf(busdPrice);

    uint256 busdNecessary = busdPrice + swapFee;

    busd.transferFrom(msg.sender, address(this), busdNecessary);
    busd.approve(address(onRamp), busdNecessary);

    uint256 balanceBefore = acceptedToken.balanceOf(address(this));

    onRamp.buyMHT(busdNecessary, price);

    uint256 balanceAfter = acceptedToken.balanceOf(address(this));
    uint256 balanceDiff = balanceAfter - balanceBefore;

    require(balanceDiff == price, "Invalid balance conversion");

    acceptedToken.approve(address(this), price);
  }
  ...
  function setOnRamp(address _onRamp) external onlyRole(OPERATOR_ROLE) {
    require(_onRamp != address(0), "OnRamp address cannot be 0");
    onRamp = IOnRamp(_onRamp);
  }
