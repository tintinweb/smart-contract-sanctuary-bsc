...
contract MetaproAirdrop is Ownable, ERC1155Holder {
    ...
    mapping(uint => address[]) public whitelists;
    ...
    function addAddresses(uint _tokenId, address[] memory _addresses) external onlyOwner {
        ...
        for (uint i; i < _addresses.length; i++) {
            whitelists[_tokenId].push(_addresses[i]);
    ...
    function addAddress(uint _tokenId, address _address) external onlyOwner {
        require(_tokenId > 0, "token id cannot be 0");
        whitelists[_tokenId].push(_address);
        emit WhitelistAddressAdd(_tokenId, _address);
    }

    function removeAddress(uint _tokenId, address _address) external onlyOwner {
        require(_tokenId > 0, "token id cannot be 0");
        for (uint i; i < whitelists[_tokenId].length; i++) {
            if (whitelists[_tokenId][i] == _address) {
                delete whitelists[_tokenId][i];
                emit WhitelistAddressRemove(_tokenId, _address);
            }
        }
    }

    function whitelisted(uint _tokenId) public view returns (address[] memory) {
        return whitelists[_tokenId];
    }  

    function claim(uint _tokenId, bytes memory data) public returns (bool) {        
        require(_tokenId > 0, "token id cannot be 0");

        address claimer = _msgSender();

        address[] memory claimers = claimed[_tokenId];
        for (uint i; i < claimers.length; i++) {
            if (claimers[i] == claimer) {
                return false;
            }
        }

        require(token.balanceOf(address(this), _tokenId) > 0, "balance cannot be 0");

        address[] memory whitelist = whitelists[_tokenId];
        for (uint i; i < whitelist.length; i++) {
            if (whitelist[i] == claimer) {
                token.safeTransferFrom(address(this), claimer, _tokenId, 1, data);
