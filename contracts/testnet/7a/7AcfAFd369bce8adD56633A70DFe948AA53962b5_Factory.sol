...
contract BondNFT is ERC721Upgradeable, BondStruct {
    ...
    function initialize(string memory name_, string memory symbol_, address issuer_) initializer public returns (bool) {
    ...
    //custom functions
    function issue(address receiver_, uint256 quantity_, uint256 amount_, uint256 maturity_, uint256 interest_) external onlyIssuer {
        for (uint i = 0; i < quantity_; i ++) {
            currentId ++;
            _mint(receiver_, currentId);
            bondInfo[currentId].issueDate = block.timestamp;
            bondInfo[currentId].lastHarvest = block.timestamp;
            bondInfo[currentId].maturity = maturity_;
            bondInfo[currentId].amount = amount_;
            bondInfo[currentId].interest = interest_;
        }
    }
...
interface BondStruct {
    ...
    struct BondInfo {
        ...
        uint256 interest; // last time harvest
    }
    ...
    struct BondIssuerConfig {
        uint256 totalFundRaise;
        uint256 startTime;
        uint256 penaltyFee;
        address beneficiary ;
        address currency;
...
contract Factory is BondStruct {
    function createBondNFT(address issuer, string memory _name, string memory _symbol) external returns (address) {
        ...
        bool check = BondNFT(_address).initialize(_name, _symbol, issuer);
