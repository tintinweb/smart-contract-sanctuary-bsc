...
contract UnipetDepositPackage is Ownable {
    ...
    function deletePackage(uint256 _id) external onlyOwner {
        delete packages[_id];
        ...
        )
    {
        ...
        uint256[] memory tokenAmounts = new uint256[](packageIds.length);
        for (uint256 i = 0; i < packageIds.length; i++) {
            ids[i] = packages[packageIds[i]].id;
            amounts[i] = packages[packageIds[i]].amount;
            tokenAmounts[i] = (packages[packageIds[i]].amount * 1000000) / price;
        }
        return (ids, amounts, tokenAmounts);
    }

    function buy(uint256 _id) external requireOpen {
        //require(packages[_id].active, "Package not valid");
        uint256 tokenOutput = (packages[_id].amount * 1000000) / price;

        require(
            token.transferFrom(_msgSender(), vaultWallet, tokenOutput),
            "MAT transfer fail"
        );
        emit Buy(
            _msgSender(),
