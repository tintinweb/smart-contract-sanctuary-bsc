...
contract ElonTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "Elontest";
    ...
    constructor() {
        ...
        _marketingWalletAddress = 0x75c1adcD2Be84124FAc5967830b807f065287410;
    ...
    function updateBuyFee(uint256 taxFee, uint256 marketingFee, uint256 liquidityFee) external onlyOwner{
        _buyTaxFee = taxFee;
        _buyMarketingFee = marketingFee;
        _buyLiquidityFee = liquidityFee;
        require(_buyTaxFee + _buyMarketingFee + _buyLiquidityFee <= 10, "Must Keep fees at 10% or less");
    }

    function updateSellFee(uint256 taxFee, uint256 marketingFee, uint256 liquidityFee) external onlyOwner{
        _sellTaxFee = taxFee;
        _sellMarketingFee = marketingFee;
        _sellLiquidityFee = liquidityFee;
        require(_sellTaxFee + _sellMarketingFee + _sellLiquidityFee <= 10, "Must Keep fees at 10% or less");
    }

    function setMaxWalletTokend(uint256 _maxToken) external onlyOwner {
  	    maxWalletToken = _maxToken * (10**18);
          require(maxWalletToken >= _tTotal * 1 / 100, "Must Keep max wallet at 1% or more");
  	}
