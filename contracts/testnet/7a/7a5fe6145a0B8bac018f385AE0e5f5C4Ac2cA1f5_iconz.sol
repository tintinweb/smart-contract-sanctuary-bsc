...
//ajouter un contrat wowiwow renommé pour gérer les fusions

interface iicoz {
    function mint(uint256 _nbTokens, address _receiver) external payable;

}

contract iconz is ERC721, ERC721Enumerable, Ownable, Clubs {
    ...
    uint256 public supastarCounter = 10001;
    uint256 public maxSupplySupastar = 15000;
    uint256 public lastIconzId = 0;
    uint256 public lastSupastazId = 0;


    address t1 = 0x2A8a699C7e3f713C848eB2966d5603C3fe798c81;
    address t2 = 0x685ed98E9903808Dd9b71DBf28765f6383Eac732;
    address t3 = 0x5a1a586cF66f92540175F9932D2649f58b923abd;
    address t4 = 0xebc470928f759E064Bf28833805a692575444C2d;

    address public supastazAddress;

    iicoz public superstarz ;
    uint public lastIdMinted = 0;

    constructor() ERC721("Icoz", "ICOZ") {
        _saleStarted = false;
    }
    ...
    function setSupastazContract(address _address) public {
        supastazAddress = _address;
        superstarz = iicoz(supastazAddress);
    }


    function mint(uint256 _nbTokens) external payable whenSaleStarted override{
    ...
    function mintSupastar(uint256 _nbTokens) internal whenSaleStarted {
        require(supastarCounter + _nbTokens <= maxSupplySupastar, "Not enough Tokens left.");

        for (uint256 i; i < _nbTokens; i++) {
            _safeMint(msg.sender, supastarCounter + i);
        }

    }


    // function superstarzMint(uint256 _nbTokens) external payable whenSaleStarted {
    //     require(_nbTokens < 21, "You cannot mint more than 20 Tokens at once!");
    //     require(supply + _nbTokens <= maxSupply - _reserved, "Not enough Tokens left.");
    //     require(_nbTokens * _price <= msg.value, "Inconsistent amount sent!");
    ...
    function fusion(uint256 _idOne, uint256 _idTwo) external payable whenSaleStarted {
        _burn(_idOne);//au lieu du burn, plus simple de les envoyer sur une adresse
        _burn(_idTwo);
        mintSupastar(1);
    }
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 _split = _balance.mul(20).div(100);

        require(payable(t1).send(_balance.sub(_split)));
        require(payable(t2).send(_balance.sub(_split)));
        require(payable(t3).send(_balance.sub(_split)));
        require(payable(t4).send(_balance.sub(_split)));
    }
