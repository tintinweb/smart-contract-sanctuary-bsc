...
contract WOLFPACKStakingManager is Ownable, ReentrancyGuard {
    ...
    function withdrawStake(uint256 amount) external nonReentrant {
        require(amount > 0 && amount <= balances[_msgSender()], "withdraw: invalid amount");
        totalStaked -= amount;
        balances[_msgSender()] = balances[_msgSender()] - amount;
    ...
    function pendingReward(address staker) public view returns(uint256 _pendingReward) {
        uint256 totalDividends = contractRewardBalance();
        uint256 perc = (balances[staker] * 100) / totalStaked;
        uint256 allocation = (totalDividends * perc) / 100;
        return allocation;
    }

    function claimReward() external nonReentrant {
        uint256 rew = pendingReward(_msgSender());
        REWARD.safeTransferFrom(address(this), _msgSender(), rew);
        emit ClaimedReward(_msgSender(), rew);
    }

    function stakedWPACK(address staker) external view returns(uint256 WPACKStaked, uint perc) {
        return (balances[staker], (balances[_msgSender()] * 100) / totalStaked);
    }
