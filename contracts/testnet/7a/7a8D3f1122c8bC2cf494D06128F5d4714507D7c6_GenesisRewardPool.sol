...
contract GenesisRewardPool {
    ...
    // TESTNET
    //uint256 public tokenPerSecond = 0.66667 ether; // 2400 TOKEN / (1h * 60min * 60s)
    ...
    constructor(
        address _token,
        address _patnerToken,
        address _feeAddress,
        uint256 _poolStartTime
    ) public {
        require(block.timestamp < _poolStartTime, "late");
        if (_token != address(0)) token = IERC20(_token);
        if (_patnerToken != address(0)) patnerToken = _patnerToken;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.token) == patnerToken) {
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (TOKEN or lps) if less than 90 days after pool ends
