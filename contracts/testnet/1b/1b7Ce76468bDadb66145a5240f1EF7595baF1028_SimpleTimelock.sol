...
contract SimpleTimelock {
    ...
    uint256 public timePeriod1 = 180 * 24 * 3600;
    uint256 public timePeriod2 = 90 * 24 * 3600;
    uint256 public ratio1 = 20;
    uint256 public ratio2 = 10;
    ...
    function transferTimeLockedTokensAfterTimePeriod(
        ...
        require(
            balances[to] - alreadyWithdrawn[to] > 0,
        ...
        uint256 times = withdrawTimes[to];
        require(times < 9, "Withdraw all locked token");
        uint256 unlockAmount;
        if (times == 0) {
            require(
                block.timestamp - initialTimestamp > timePeriod1,
                "Tokens are only available after correct time period has elapsed"
            );
            unlockAmount = balances[to].mul(ratio1).div(100);
        } else {
            require(
                block.timestamp - lastWithdrawTime[to] > timePeriod2,
                "Tokens are only available after correct time period has elapsed"
            );
            unlockAmount = balances[to].mul(ratio2).div(100);
        }
        alreadyWithdrawn[to] = alreadyWithdrawn[to].add(unlockAmount);
        withdrawTimes[to]++;
        lastWithdrawTime[to] = block.timestamp;
        token.safeTransfer(to, unlockAmount);
        emit TokensUnlocked(to, unlockAmount);
    }
    ...
    function updateTimePeriod(
        uint256 _timePeriod1,
        uint256 _timePeriod2
    ) public onlyOwner {
