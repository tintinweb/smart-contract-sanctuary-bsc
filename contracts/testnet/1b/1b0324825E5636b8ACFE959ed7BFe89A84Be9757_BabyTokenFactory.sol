...
contract BabyTokenFactory is Ownable, ReentrancyGuard {
    BABYTOKEN[] public babyTokenAddresses;
    event BabyTokenCreated(address indexed owner, address indexed babyToken);
    ...
    function createBabyToken(
        ...
        address[4] memory _addrs, // reward, router, marketing wallet, dividendTracker
        uint256[3] memory _feeSettings, // rewards, liquidity, marketing
        uint256 _minimumTokenBalanceForDividends,
        address _serviceFeeReceiver,
        uint256 _serviceFee
    ) external nonReentrant returns(address) {
        BABYTOKEN token = new BABYTOKEN(
            _name,
            _symbol,
            _totalSupply,
            _addrs,
        ...
        emit BabyTokenCreated(msg.sender, address(token));
        return address(token);
    }

    function getBabyToken(uint256 _index) external view returns (BABYTOKEN) {
        require(_index < babyTokenAddresses.length, "Index is over");
        return babyTokenAddresses[_index];
    }
}
...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    function initialize(
        ...
        require(!isInitialized, "BABYTOKENDividendTracker: Already initialized");
        require(msg.sender == babyToken, "BABYTOKENDividendTracker: Caller must be baby");
    ...
    function setBabyToken(address _baby) public {
        babyToken = _baby;
    }
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
        ...
        dividendTracker.setBabyToken(address(this));
