...
contract Sale is ISale, ContextUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
     ...
     */
    mapping(uint256 => uint256[]) public currentSalesInPack;
    ...
    /**
     * @dev To check Project is valid
     */
    modifier projectIsValid(uint256 _id, bool _isCheckByProjectId) {
        ProjectInfo memory _project = project.getProject(_isCheckByProjectId ? _id : sales[_id].projectId); 
    ...
    function creates(address _caller, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external nonReentrant onlyProject returns (uint256) {
        ...
        for (uint256 i = 0; i < _saleInputs.length; i++) {
            ...
            _sales[i] = _createSale(_caller, lastId.current(), _isCreateNewToken, _isSetRoyalty, _projectInfo, _saleInputs[i]);
        }
    ...
    function _createSale(address _caller, uint256 _saleId, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _project, SaleInput memory _saleInput) private returns (SaleInfo memory) {
        ...
        if (_project.isPack) {
            currentSalesInPack[_project.id].push(lastId.current());
        } else if (!_project.isFixed) {
    ...
    function close(uint256 _closeLimit, ProjectInfo memory _project, SaleInfo memory _sale, uint256 _totalBuyersWaitingDistribution, bool _isGive) external onlyProject nonReentrant returns (uint256) {
        ...
        for (uint256 i = 0; i < buyersWaiting.length; i++) {
            ...
            if (moneyPaid > 0) {
                Helper.safeTransferNative(billInfo.account, moneyPaid);
            } else {
     ...
     */
    function buy(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant projectIsValid(_saleId, false) {
        ...
        require(projectInfo.isSingle ? _amount == 1 : _amount > 0 && _amount <= saleInfo.amount, "Invalid amount");

        uint256 currentSalePrice = projectInfo.isFixed ? saleInfo.fixedPrice :
        getCurrentDutchPrice(projectInfo.saleStart, projectInfo.saleEnd, saleInfo.dutchMaxPrice, saleInfo.dutchMinPrice, saleInfo.priceDecrementAmt);

        //slither-disable-next-line incorrect-equality
        require(projectInfo.isFixed ? msg.value == (currentSalePrice * _amount) : msg.value >= (currentSalePrice * _amount), "Invalid value");
        ...
        _sharing(projectInfo, saleInfo, _amount, currentSalePrice, msg.value);
    }
     ...
     * @param _amount Token amount
     */
    function buyPack(uint256 _projectId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant projectIsValid(_projectId, true) {
        ...
        require(currentSalesInPack[_projectId].length > 0, "Sold out");
        require(_amount > 0 && _amount <= currentSalesInPack[_projectId].length, "Invalid amount");

        uint256 currentSalePrice = projectInfo.isFixed ? projectInfo.fixedPricePack : 
        getCurrentDutchPrice(projectInfo.saleStart, projectInfo.saleEnd, projectInfo.maxPricePack, projectInfo.minPricePack, projectInfo.priceDecrementAmtPack);

        //slither-disable-next-line incorrect-equality
        require(projectInfo.isFixed ? msg.value == (currentSalePrice * _amount) : msg.value >= (currentSalePrice * _amount), "Invalid value");

        if (_amount < currentSalesInPack[_projectId].length) {
            currentSalesInPack[_projectId] = Helper.shuffle(currentSalesInPack[_projectId]);
        }

        uint256 sendFromIndex = currentSalesInPack[_projectId].length - 1;
        uint256 pricePerSale = msg.value / _amount;

        for (uint256 i = 0; i < _amount; i++) {
            uint256 saleId = currentSalesInPack[_projectId][sendFromIndex - i];
            SaleInfo storage saleInfo = sales[saleId];

            currentSalesInPack[_projectId].pop();
            saleInfo.amount = 0;
            ...
            _sharing(projectInfo, saleInfo, 1, currentSalePrice, pricePerSale);
        }
     ...
     * @param _sale Sale info
     * @param _amount Token amount
     * @param currentSalePrice Current sale price
     * @param _msgValue Value sent by caller
     */ 
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 currentSalePrice, uint256 _msgValue) private {
        uint256 supperAdminProfit = 0; 
        uint256 royaltyProfit = 0;
        uint256 sellerProfit = 0;
        uint256 residualPrice = 0;

        if (_msgValue > currentSalePrice) residualPrice = _msgValue - currentSalePrice;
        
        // Calculate royal fee
        (address royaltyReceiver, uint256 royaltyFee) = getRoyaltyInfo(_project.id, _sale.tokenId, currentSalePrice);
        royaltyProfit = royaltyFee;

        // Calculate fee and profit
        if (_project.isCreatedByAdmin) {
            supperAdminProfit = currentSalePrice - royaltyProfit;
        } else {
            // admin fee
            supperAdminProfit = _getPriceToPercent(currentSalePrice, _project.profitShare);
            sellerProfit = currentSalePrice - supperAdminProfit;
