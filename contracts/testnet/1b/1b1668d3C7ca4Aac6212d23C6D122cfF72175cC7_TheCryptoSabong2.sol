...
contract TheCryptoSabong2 is Ownable, ReentrancyGuard {
   ...
   using SafeERC20 for IERC20;
    ...
    /**
     * @notice Bet Wala position
     * @param epoch: epoch
     * @param amount: amount
     */
    function betWala(uint256 epoch, uint256 amount) external nonReentrant  {
        ...
        require(ledger[epoch][msg.sender].amount == 0 || 
        ledger[epoch][msg.sender].position == Position.Wala, "Can only bet in one team");
    ...
    /**
     * @notice Bet Meron position
     * @param epoch: epoch
     * @param amount: amount
     */
    function betMeron(uint256 epoch, uint256 amount) external nonReentrant {
        require(epoch == currentRound, "Bet is too early/late");
        require(_bettable(epoch), "Round not bettable");
        require(amount >= minimumBet, "Bet amount must be greater than minBetAmount");
        require(ledger[epoch][msg.sender].amount == 0 || 
        ledger[epoch][msg.sender].position == Position.Meron, "Can only bet in one team");
        Round storage round = rounds[epoch];
        BetInfo storage betInfo = ledger[epoch][msg.sender];
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
           ...
           // Round valid, claim rewards
            if (rounds[epochs[i]].gameResult == Position.Meron) {
                ...
                addedReward = ledger[epochs[i]][msg.sender].amount * round.oddsAmount;
            }
    ...
    function _calculateRewards(uint256 epoch) internal {
     ...
     /**
     * @notice Determine if a round(epoch) is valid for receiving bets
    ...
    function _bettable(uint256 epoch) internal view returns (bool) {
     ...
     /**
     * @notice Determine if a round(epoch) is closed
