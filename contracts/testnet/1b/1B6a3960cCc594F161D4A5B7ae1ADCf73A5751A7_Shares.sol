...
contract Shares is ERC721, WhitelistEnabledFor {
    ...
    function issuanceOfShares(uint _nrToIssue) public onlyRole(ADMIN) { 
        require(!capLocked, "No more shares can be issued");
        require(cap == circulating, "You can not issue more shares before minting existing ones");
        ...
        if(cap == 0){globalStart = block.timestamp;}
        cap += _nrToIssue;
        emit IssuanceOfShares(circulating, cap - _nrToIssue, cap);
    }
    ...
    function calculateTokenDistribution(address _token) public {
        require(cap != 0, "Must issue shares");
        ClaimPeriod[] storage claimPeriods = token[_token];
        if(claimPeriods.length == 0){
            claimPeriods.push();
            claimPeriods[0].start = globalStart;
            claimPeriods[0].startCap = cap;
            claimPeriods[0].startMaxTokenId = cap + burnCount;
        }
        ...
        if(block.timestamp < activePeriod.start + periodLength){
            ...
            nextPeriod.startCap = cap;
            nextPeriod.startMaxTokenId = cap + burnCount;
    ...
    function mint(address _to, uint _quantity) public onlyRole(MINT) {
        uint target = circulating + _quantity;
        require(target <= cap, "Cap overflow");
        uint idTarget = target + burnCount;
        for (uint256 index = circulating + burnCount + 1; index <= idTarget; index++) {
            _safeMint(_to, index);
        }
        emit IssuerMintOfShares(circulating, target);
        circulating = target; 
    }
    ...
    function claimEarnings(address _token, uint _claimPeriod, address _owner, uint[] calldata _shareIds) public {
    ...
    function claimEarningsNative(uint _claimPeriod, address _owner, uint[] calldata _shareIds) public {
    ...
    function _totalToGet(address _token, uint _periodIndex, address _owner, uint[] calldata _shareIds) internal returns (uint totalToGet){
        ...
        if(claimWL){
            require(whitelist[_owner], "Address not whitelisted");
        }
    ...
    function lockCap() public onlyRole(ADMIN){
        require(cap != 0, "Can not lock contract without issuing any shares");
        capLocked = true;
        emit Lock(2);
    }

    function setPeriodAndDelay(uint _periodLength, uint _flushDelay) public onlyRole(ADMIN){
        require(globalStart == 0, "Contract already started");
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override {
        ...
        if (!(from == address(0) || hasRole(DEFAULT_ADMIN_ROLE, msg.sender))) {
            ...
            if(transferWL) {
                require(whitelist[from] && whitelist[to],"Invalid token transfer");
            }
        }
    }
...
abstract contract WhitelistEnabledFor is Whitelist {
    ...
    bool public transferWL;
    bool public claimWL;
    bool public transferBlocked;

    event AcquireWL (bool on);
    event TransferWL (bool on);
    event ClaimWL (bool on);
