...
contract BEP20RewardVaultVip is Ownable {
    ...
    /// Deposit staking token to a user into the contract to earn rewards.
    /// @dev Since this contract needs to be supplied with rewards we are
    ///  sending the balance of the contract if the pending rewards are higher
    /// @param _amount The amount of staking tokens to deposit
    /// @param _address The address to deposit
    function depositToAddress(uint256 _amount, address _address)
        external
        onlyOwner
    {
        require(block.number < bonusEndBlock, "Deposit: pool already ended");
        ...
        emit Deposit(_address, finalDepositAmount);
    }
    ...
    // Withdraw from user. EMERGENCY ONLY.
    function emergencyStakeWithdraw(address _from, uint256 _amount)
        external
        onlyOwner
    {
        require(_amount <= rewardBalance(), "not enough rewards");
        ...
        user.amount = user.amount.sub(_amount);
        pool.lpToken.safeTransfer(address(_from), _amount);
        totalStaked = totalStaked.sub(_amount);
        emit EmergencyRewardWithdraw(_from, _amount);
    }
