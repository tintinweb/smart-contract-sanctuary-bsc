...
contract StreamStake is AccessControl, ReentrancyGuard {
    ...
    event Stake(address indexed wallet, uint256 amount, uint256 date, uint256 lockTimePeriod);
    ...
    event StakeFactorAdded(uint256 timePeriod, uint256 stakeFactor);
    ...
    event RewardTokenTransferred(address receiverAddress, uint256 amount);

    struct User {
        uint48 stakeTime;
        uint48 unlockTime;
        uint256 lockedTime;
        uint256 stakeRewardFactor;
    ...
    uint256[] lockTimePeriod; // time in seconds a user has to wait after calling unlock until staked token can be withdrawn
    ...
    constructor(address _stakingToken, address _rewardToken, uint256[] memory _lockTimePeriod) {
        require(_stakingToken != address(0), "Staking Token: address = 0");
        require(_rewardToken != address(0), "Reward Token: address = 0");
     ...
     */
    function setLockTimePeriod(uint256[] memory _lockTimePeriod)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        lockTimePeriod = _lockTimePeriod;
    ...
    // add reward factor
    function addRewardFactor(uint256 _timePeriod, uint256 _stakeFactor) public onlyRole(DEFAULT_ADMIN_ROLE) {
        bool check = false;
        for(uint8 i=0; i < lockTimePeriod.length; i++) {
            if(lockTimePeriod[i] == _timePeriod) {
                check = true;
            }
        }

        require(check == true, "Time Period: Not exist");
        stakeReward[_timePeriod] = _stakeFactor;
    }

    /**
     * Transfer reward token by owner.
     * @param _amount Reward token amount
     */
    function transferRewardToken(uint256 _amount)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_amount != 0, "Amount should be greater than 0");
        IERC20(rewardToken).safeTransfer(msg.sender, _amount);
        emit RewardTokenTransferred(msg.sender, _amount);
    }
        ...
        returns (uint256)
    {
        ...
        if(block.timestamp > user.unlockTime) {
            timePeriod = user.unlockTime - user.stakeTime;
        } else {
            timePeriod = block.timestamp - user.stakeTime;
        }

        return timePeriod * user.rewardPerSec;
    }
        ...
        returns (uint256 claimableRewardTokens)
    {
        if (address(rewardToken) == address(0)) {
            ...
            return userTotalRewards(_staker); // safe
        }
    }
        ...
        returns (User storage user)
    {
        ...
        if(user.alreadyStake == true) {
            user.stakeTime = user.stakeTime;
        }
        else {
     ...
     */
    function _stake(uint256 _amount, uint256 _lockTimePeriod, address _stakerAddress)
        internal
        returns (uint256)
    {
        require(_amount > 0, "Stake amount: stake amount must be > 0");
        bool check = false;
        for (uint8 i = 0; i < lockTimePeriod.length; i++) {
            if (lockTimePeriod[i] == _lockTimePeriod) {
                check = true;
            }
        }

        require(check != false, "Lock Time: Lock time period isn't valid");

        User storage user = _updateRewards(_stakerAddress); // update rewards and return reference to user

        require(user.alreadyStake != true, "User Err: User already staked");

        user.stakeAmount = toUint160(user.stakeAmount + _amount);
        tokenTotalStaked += _amount;

        uint8 count = 0;
        bool checkStakingTime = false;
        for(uint8 i = 0; i < lockTimePeriod.length; i++) {
            if(lockTimePeriod[i] == _lockTimePeriod) {
                count = i;
                checkStakingTime = true;
            }
        }
        require(checkStakingTime == true, "Days: Invalid Days");
        user.stakeRewardFactor = stakeReward[_lockTimePeriod];
        (user.totalAward, user.rewardPerSec) = calculateAPY(_amount, stakeReward[_lockTimePeriod], _lockTimePeriod);

        user.unlockTime = toUint48(
            block.timestamp + _lockTimePeriod.mul(60)
        );
        user.lockedTime = _lockTimePeriod.mul(60);
        user.alreadyStake = true;
        ...
        emit Stake(_stakerAddress, _amount, toUint48(block.timestamp), _lockTimePeriod); // = user.stakeTime
        return _amount;
    }
    ...
    function _withdraw(uint256 amount) internal returns (uint256) {
        require(amount > 0, "Withdraw: Amount to withdraw should be > 0");
        ...
        require(
            ...
            "Withdraw: Staked tokens are still locked"
        );
        user.stakeAmount -= toUint160(amount);
        tokenTotalStaked -= amount;
        user.alreadyStake = false;
    ...
    function _claim() internal returns (uint256) {
        require(rewardToken != address(0), "Reward Err: No reward token contract");
        uint256 earnedRewardTokens = getEarnedRewardTokens(msg.sender);
        require(earnedRewardTokens > 0, "Reward Err: no tokens to claim");

        User storage user = userMap[msg.sender];
        user.accumulatedRewards = 0;

        if(block.timestamp > user.unlockTime) {
            user.stakeTime = user.unlockTime; // will reset userClaimableRewards to 0
        }
        else {
    ...
    // stake tokens
    function stake(uint256 _amount, uint48 _lockTimePeriod)
        external
        nonReentrant
        returns (uint256)
    {
        return _stake(_amount, _lockTimePeriod, msg.sender);
    }

    // stake on behalf
    function stakeOnBehalf(uint256 _amount, uint48 _lockTimePeriod, address _stakerAddress)
        external
        nonReentrant
        returns (uint256)
    {
        return _stake(_amount, _lockTimePeriod, _stakerAddress);
    }
    ...
    // withdraw check lock time period
    function checkLockTimePeriod() public view returns (uint256[] memory) {
    ...
    // calculate apy
    function calculateAPY(uint256 _amount, uint256 _interestRate, uint256 _timePeriod) internal pure returns (uint256, uint256) {
        uint256 interestRate = _interestRate.div(100);
        uint256 calculate = interestRate.div(_timePeriod);
        calculate = 1000000000000000000 + calculate;

        if(_timePeriod == 1) {
            calculate = calculate ** _timePeriod;
        }
        else {
            uint256 tempCalculateAPY = calculate;
            for(uint8 i = 1; i < _timePeriod; i++) {
                calculate = calculate.mul(tempCalculateAPY);
                calculate = calculate.div(1000000000000000000);
            }
        }
        calculate = _amount.mul(calculate);
        calculate = calculate.div(1000000000000000000) - _amount;

        uint256 calculateApyPerSec = calculate.div(60*(_timePeriod));
        return(calculate, calculateApyPerSec);
    }

    // get the apy
    function getAPY(uint256 _amount, uint256 _interestRate, uint256 _timePeriod) public pure returns (uint256, uint256) {
        return (calculateAPY(_amount, _interestRate, _timePeriod));
    }
}
