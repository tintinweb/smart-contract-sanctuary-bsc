...
contract Lottery is Ownable {
    ...
    constructor(ICOMM _commTreasury, address _asset2BuyAdress) {
    ...
    function spin(uint _pid, uint nonce, uint number, uint amount) public {
        ...
        require(amount >= p.minTokenAmount && amount < p.maxTokenAmount, "Lottery::spin: amount invalid");
...
//        if(_poolReward > 0) IERC20(asset2BuyAdress).safeTransferFrom(_msgSender(), address(this), _poolReward);
        pools.push(Pool(_minTokenAmount, _maxTokenAmount, _decimal, _referCommission, true));
    }
