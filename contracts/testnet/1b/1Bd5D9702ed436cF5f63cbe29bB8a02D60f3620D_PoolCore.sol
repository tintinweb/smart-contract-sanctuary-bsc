...
import "./PoolPermissions.sol";
import "./IPoolCore.sol";
import "../interfaces/IPoolToken.sol";

/// @title Source of truth
/// @notice maintains roles, access control, gov, and the treasury
contract PoolCore is IPoolCore, PoolPermissions, Initializable {
    /// @notice the address of the Gov contract
    IPoolToken public override gov;

    function init(address _gov) external override initializer {
        _setupGovernor(msg.sender);

        _setGov(address(_gov));
    }

    /// @notice sets Gov address to a new address
    /// @param token new Gov address
    function setGov(address token) external override onlyGovernor {
        _setGov(token);
    }

    /// @notice sends Gov tokens from treasury to an address
    /// @param to the address to send Gov to
    /// @param amount the amount of Gov to send
    function allocateGov(address to, uint256 amount)
        external
        override
        onlyGovernor
    {
        IPoolToken _gov = gov;
        require(
            _gov.balanceOf(address(this)) >= amount,
            "Core: Not enough Gov"
        );

        _gov.transfer(to, amount);

        emit GovAllocation(to, amount);
    }

    function _setGov(address token) internal {
        gov = IPoolToken(token);
        emit GovUpdate(token);
    }
}
...
import "./IPoolPermissions.sol";

/// @title Access control module
contract PoolPermissions is IPoolPermissions, AccessControlEnumerable {
    ...
    bytes32 public constant override FUNDMANAGER_ROLE =
        keccak256("FUNDMANAGER_ROLE");
    ...
    constructor() {
        ...
        _setRoleAdmin(FUNDMANAGER_ROLE, GOVERN_ROLE);
    }
    ...
    /// @notice grants fundmanager role to address
    /// @param fundmanager new fundmanager
    function grantFundmanager(address fundmanager)
        external
        override
        onlyGovernor
    {
        grantRole(FUNDMANAGER_ROLE, fundmanager);
    }
    ...
    /// @notice grants fundmanager role to address
    /// @param fundmanager new fundmanager
    function revokeFundmanager(address fundmanager)
        external
        override
        onlyGovernor
    {
        revokeRole(FUNDMANAGER_ROLE, fundmanager);
    }
    ...
    /// @notice checks if address is a fund manager
    /// @param _address address to check
    /// @return true _address is a fund manager
    function isFundmanager(address _address)
        public
        view
        override
        returns (bool)
    {
        return hasRole(FUNDMANAGER_ROLE, _address);
    }
...
import "./IPoolPermissions.sol";
import "../mixins/IERC4626.sol";
import "../interfaces/IPoolToken.sol";

/// @title Core Interface
interface IPoolCore is IPoolPermissions {
    // ----------- Events -----------

    event GovUpdate(address indexed _gov);
    event GenesisGroupUpdate(address indexed _genesisGroup);
    event GovAllocation(address indexed _to, uint256 _amount);
    event GenesisPeriodComplete(uint256 _timestamp);

    // ----------- Governor only state changing api -----------

    function init(address _gov) external;

    // ----------- Governor only state changing api -----------

    function setGov(address token) external;

    function allocateGov(address to, uint256 amount) external;

    // ----------- Getters -----------

    function gov() external view returns (IPoolToken);
}
...
import "./IPoolPermissionsRead.sol";

/// @title Permissions interface
interface IPoolPermissions is IAccessControl, IPoolPermissionsRead {
    // ----------- Governor only state changing api -----------
    ...
    function grantFundmanager(address fundmanager) external;
    ...
    function revokeFundmanager(address fundmanager) external;
    ...
    function FUNDMANAGER_ROLE() external view returns (bytes32);
}
...
interface IPoolPermissionsRead {
    // ----------- Getters -----------
...
abstract contract IERC4626 is ERC20 {
    ...
    */
    function assetsOf(address user)
        public
        view
        virtual
        returns (uint256 amount);
