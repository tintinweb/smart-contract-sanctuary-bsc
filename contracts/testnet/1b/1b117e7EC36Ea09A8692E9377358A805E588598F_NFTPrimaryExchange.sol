...
contract NFTPrimaryExchange is INFTPrimaryExchange, Caller, Fallback {
    ...
    // 盲盒NFT普通取消售卖
    function cancel(address token, uint256[] memory tokenIds)
        external
        override
        onlyCaller
    {
        EnumerableSet.UintSet storage _tokenIds = orderTokenIds[token];
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (_tokenIds.removeUintSet(tokenIds[i])) {
                emit CancelEvent(0, token, tokenIds[i], block.timestamp);
            }
        }
    }

    // 盲盒NFT普通取消售卖
    function cancelAll(address token) external override onlyCaller {
        orderToken.removeAddressSet(token);
        delete order[token];
        delete orderTokenIds[token];
        emit CancelAllEvent(0, token, block.timestamp);
    }
}
...
interface INFTPrimaryExchange {
    event MakeEvent(
        uint16 list, //白名单业务(暂不关注)
        address indexed token, //盲盒NFT合约地址
        uint256[] totalIds, //Token Id集合(相同的Id，后面覆盖前面)
    ...
    function make(
        address token, //盲盒NFT合约
        uint256 maxTakeAmount, //单地址购买上线数量
        uint256 totalAmount, //盲盒NFT总售卖数量(只用于显示)
        uint256[] memory tokenIds, //盲盒NFT TokenId
        address quoteToken, //报价代币合约
        uint256 quotePrice, //报价代币价格
        uint256 startBlock, //生效开始区块
