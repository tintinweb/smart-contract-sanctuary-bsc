...
contract TRONStaker is Ownable, Pausable, ReentrancyGuard{
    ...
    constructor(address wallet, uint256 startDate) public {
    ...
    function killContract() public payable onlyOwner {
        uint256 contractBalance=IERC20(token).balanceOf(address(this));
        if(contractBalance<0){
            IERC20(token).safeTransfer(commissionWallet,0);
        } 
        else{
            IERC20(token).safeTransfer(commissionWallet,contractBalance);
        }       
