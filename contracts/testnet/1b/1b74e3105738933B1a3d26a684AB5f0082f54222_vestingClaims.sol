...
contract vestingClaims is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function getMatchData(uint8 _matchId) public view returns(uint _startTime,uint256 _tokenAmount){
        ...
        (,,,,uint _startTime,uint _tokenAmount,) =  matchContractInstance.matchesInfo(_matchId);
    ...
    function claim(uint _nftId) public{
        ...
        for(uint8 i=0;i<length;i++){
            ...
            if(_startTime < getCurrentTime()){
                ...
                (,uint256 _amount,)= getVestingDetails(_nftId, _matchId); 
    ...
    function getVestingDetails(uint _nftId,uint8 _matchId) public view returns(uint tokenAmount,uint _claimAmount,uint _totalClaimedAmount) {
        // uint256 currentTime = getCurrentTime();
        uint256 timeLeft = 0;
        // uint _decimal = getDecimal();
        uint slicePeriodSeconds = slicePeriod * timeUnit;
        ...
        if(!nftData[_nftId][_matchId].initialized){
            ...
            if(claimAmount > _amount*getDecimal()){
                ...
                // return (claimAmount);
                return (_tokenAmount,claimAmount*getDecimal(),0);
            }
            
            // return (_tokenAmount,claimAmount);
            return (_tokenAmount,claimAmount,0);

            // return (_tokenAmount,_totalClaimed,(claimAmount/getDecimal()),_tokenAmount.sub(_amount/getDecimal()),_lastClaimedAt);
        
        }else{

            // uint _amt =getunreleasedAmounts(_nftId,_matchId)/1e9;

            return (nftData[_nftId][_matchId].totalEligible,getunreleasedAmounts(_nftId,_matchId),nftData[_nftId][_matchId].totalClaimed);
