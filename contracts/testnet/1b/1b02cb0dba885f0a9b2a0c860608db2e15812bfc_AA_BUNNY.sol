...
contract AA_BUNNY is Context, IERC20 { 
    ...
    */
    ...
    function swapAndLiquify(uint256 contractBalance) private lockTheSwap {
        ...
        // Get fee splits
        uint256 percent_L = Fee_L * accuracy / tokensTotal;
        uint256 percent_D = Fee_D * accuracy / tokensTotal;
        uint256 percent_S = Fee_S * accuracy / tokensTotal;
        uint256 percent_M = Fee_M * accuracy / tokensTotal;
        ...
        // Add liquidity 
        if (percent_L > 0){
            uint256 bnb_half = returned_BNB - ((returned_BNB * percent_D / accuracy) + (returned_BNB * percent_S / accuracy) + (returned_BNB * percent_M / accuracy));
            addLiquidity(token_half, bnb_half);
            emit SwapAndLiquify(token_half, bnb_half, token_half);
        }

        // Take developer fee
        sendToWallet(Wallet_SolidityDev, (returned_BNB * percent_S / accuracy));
        
        // Take development and charity fee
        sendToWallet(Wallet_Development, (returned_BNB * percent_D / accuracy));
