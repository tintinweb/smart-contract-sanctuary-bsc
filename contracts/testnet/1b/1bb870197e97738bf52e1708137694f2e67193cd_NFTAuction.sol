...
contract NFTAuction {
    ...
    constructor() {
        ...
        feesPercentage = 1000;
        feesRecevier= 0xAd5292D3D35F57CC0D7876cfD7B583DC99637b0d;
    ...
    function _makeBid(
        ...
        paymentAccepted(
            ...
            _tokenId,
            _erc20Token,
            getbasePrice(_tokenAmount , feesPercentage )
        )
        ...
        )
    {
       ...
       address erc20Token = _erc20Token;
        _reversePreviousBidAndUpdateHighestBid(
            _nftContractAddress,
            _tokenId,
            getbasePrice(tokenAmount , feesPercentage )
        );
        emit BidMade(
            _nftContractAddress,
            _tokenId,
            msg.sender,
            getbasePrice(msg.value , feesPercentage ),
            erc20Token,
            getbasePrice(tokenAmount,feesPercentage)
        );
        _updateOngoingAuction(_nftContractAddress, _tokenId);
        ...
        onlyApplicableBuyerBatch(_nftContractAddress, _batchTokenIds)
    {
        ...
        uint256 base_price = getbasePrice(msg.value , feesPercentage);
        for (uint256 i = 0; i < _batchTokenIds.length; i++) {
            
            _makeBatchBid(_nftContractAddress, _batchTokenIds[i], _erc20Token, base_price);
        }
    }
    ...
    function _updateHighestBid(
        ...
        if (_isERC20Auction(auctionERC20Token)) {
            ...
            uint256 basePrice = getbasePrice(msg.value , feesPercentage);
            nftContractAuctions[_nftContractAddress][_tokenId]
                .nftHighestBid = basePrice;
        }
        nftContractAuctions[_nftContractAddress][_tokenId]
    ...
    function _reversePreviousBidAndUpdateHighestBid(
        ...
        if (prevNftHighestBidder != address(0)) {
            uint256 addFees = _getPortionOfBid(prevNftHighestBid , feesPercentage);
            _payout(
                _nftContractAddress,
                _tokenId,
                prevNftHighestBidder,
                prevNftHighestBid + addFees
            );
        }
    }
    ...
    function getbasePrice(
        ...
        returns (uint256) {
        return  _amount - (_amount - (_amount * 100 / (100 + (_per / 100))));
    }
