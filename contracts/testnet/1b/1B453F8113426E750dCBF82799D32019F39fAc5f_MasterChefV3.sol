...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumberable {
    ...
    function onERC721Received(address, address _from, uint256 _tokenId, bytes calldata) external returns (bytes4) {
        ...
        uint256 rewardGrowthInside = LMPool.getRewardGrowthInside(cache.tickLower, cache.tickUpper);
    ...
    function withdraw(uint256 _tokenId, address _to) external nonReentrant {
        ...
        if (address(LMPool) != address(0)) {
            ...
            LMPool.updatePosition(positionInfo.tickLower, positionInfo.tickUpper, -int128(positionInfo.boostLiquidity));
        }
