...
contract MyToken is Context, IBEP20, Ownable 
{
    ...
    address payable public developerAddress = payable(0x00A267A6451d6E5E6fBBdcEb7122852B130d6EC3);
    address payable public charityAddress = payable(0xdb995a5e8564Af80C411374fAf951cE267558DAA);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap 
    {
        ...
        uint256 _totalFee = _developerFee.add(_charityFee).add(_saleDeveloperFee).add(_saleCharityFee);
        uint256 bnbForDeveloper = newBalance.mul(_developerFee.add(_saleDeveloperFee)).div(_totalFee);
        uint256 bnbForCharity  = newBalance.sub(bnbForDeveloper);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 tLiquidity = calculatedeveloperFee(tAmount);
    ...
    {
        return _amount.mul(_developerFee+_charityFee).div(100);
    }
    ...
    function setAllBuyFeePercentages(uint256 developerFee, uint256 marketingFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _developerFee.add(_charityFee);
    ...
    function setAllSaleFeePercentages(uint256 developerFee, uint256 marketingFee) 
    external onlyOwner()
    {
        _saleDeveloperFee = developerFee;
        _saleCharityFee = marketingFee;
        uint256 totalFee = developerFee.add(marketingFee);
