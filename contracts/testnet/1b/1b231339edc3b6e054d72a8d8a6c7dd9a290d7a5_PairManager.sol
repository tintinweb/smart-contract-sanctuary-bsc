...
contract PairManager is Ownable, IPoolFactory, IPoolManager {
  ...
  mapping(string => AmaPartner) public amapartner;
  ...
  modifier onlyAmaAddress() {
    emit sender(msg.sender);
    require(poolFactories.contains(msg.sender) || owner() == msg.sender , "Not a whitelisted factory");
    _;
  }
  ...
  function registerAma(
      ...
      uint8 fees
        ...
        amapartner[_id] =  
         ...
         emit addAma(_id, _ownerAddress, email, fees, false);
    }

    function updatePartnerAddress( string memory _id , address _address ) public onlyAmaAddress{
        AmaPartner storage ama = amapartner[_id];
        require(bytes(ama.id).length != 0 , "No Partner Avalible with Id");
        amaPartnerAddress.remove(amapartner[_id].ownerAddress);
        amapartner[_id].ownerAddress = _address;
        amaPartnerAddress.add(_address);
    }

    function updatePartnerInfo( string memory _id , address _address , uint256 _fees , bool _status) public onlyAmaAddress{
        AmaPartner storage ama = amapartner[_id];
        require(bytes(ama.id).length != 0 , "No Partner Avalible with Id");
        amaPartnerAddress.remove(amapartner[_id].ownerAddress);
        amapartner[_id].ownerAddress = _address;
        amaPartnerAddress.add(_address);
        amapartner[_id].fees = _fees;
        amapartner[_id].status = _status;
    }
