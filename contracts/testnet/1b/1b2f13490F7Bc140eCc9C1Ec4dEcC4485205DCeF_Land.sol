...
contract Land is DotEarthERC721, ILand {
    mapping(uint256 => PurchaseType) public purchaseTypes;
    ...
    function mint(uint256 tokenId, PurchaseType purchaseType_)
        external
        override
        onlyMinterRole
    {
        _mint(msg.sender, tokenId);
        purchaseTypes[tokenId] = purchaseType_;
    }
    ...
    function isPurchased(uint256 tokenId) public view returns (bool) {
        return purchaseTypes[tokenId] != PurchaseType.Undefined;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (purchaseTypes[tokenId] == PurchaseType.PartnerDeal) {
            require(
                unlockOfPartnerToken < block.timestamp,
                "Cannot transfer Partner token yet"
            );
        }
    }
...
interface ILand {
    ...
    function mint(uint256 tokenId_, PurchaseType purchaseType_) external;
...
abstract contract DotEarthERC721 is ERC721Royalty, IDotEarth721 {
    ...
    constructor(
        ...
        prevalidateAddresses_(accessControllerAddr, dotEarthRoyaltyAddr);
        DotEarthRoyalty royalty_ = DotEarthRoyalty(dotEarthRoyaltyAddr);
        prevalidateDenominators_(royalty_);
        royalty = royalty_;
        accessController = ContractControlList(accessControllerAddr);
        baseURI = baseURI_;
        _setDefaultRoyalty(dotEarthRoyaltyAddr, royalty.sumOfNumerators());
    }
...
contract ContractControlList is AccessControl {
    ...
    constructor(address admin_) {
        ...
        _setupRole(LAND_OWNER_ROLE, msg.sender);
