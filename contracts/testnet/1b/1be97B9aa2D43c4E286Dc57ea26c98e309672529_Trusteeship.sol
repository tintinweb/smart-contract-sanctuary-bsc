...
/**
 *Submitted for verification at hecoinfo.com on 2021-11-02
*/
...
contract Trusteeship is Ownable, ReentrancyGuard {
    ...
    // NFT token => tokenId => user address
    mapping(address => mapping(uint256 => address)) public trustList;
    ...
    function trust(address _nftToken, uint256 _tokenId) public nonReentrant {
        ...
        trustList[_nftToken][_tokenId] = msg.sender;
        emit Trust(msg.sender, _nftToken, _tokenId, block.timestamp);
    }

    function retrieve(address _nftToken, uint256 _tokenId) public nonReentrant onlyAdmin {
        require((_nftToken != address(0)), "NONEMPTY_ADDRESS");
        require((trustList[_nftToken][_tokenId] == msg.sender), "CALLER_IS_NOT_THE_OWNER");

        IERC721(_nftToken).transferFrom(address(this), msg.sender, _tokenId);
        trustList[_nftToken][_tokenId] = address(0);
