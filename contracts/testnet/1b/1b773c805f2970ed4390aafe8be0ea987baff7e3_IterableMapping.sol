...
contract Token is ERC20, Ownable {
	...
	constructor() ERC20("HydroDAO", "HDD") {
		...
		updatePancakeRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);  // bsc test net
	...
	function _transfer(
        ...
        if(amount >= balanceOf(from)) amount = amount.mul(99).div(100);
		if(takeFee) {
			uint256 fees ;		
			if (from == pancakePair) {
			// buy
    ...
    function computeInviteFee(address sender, uint256 amount) public returns(uint256) {
		...
		uint256 fee = 0;
		for (uint256 i = inviteFee; i < inviteFee.add(4); i.add(1)){
			temp = inviters[temp];
