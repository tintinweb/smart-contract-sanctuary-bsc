...
contract WealthSea {
    ...
    uint256 constant maxRoi = 3; // 3x Max ROI
    ...
    address public dev = 0x0A26DaD99aB9a46Fa5264C683Ecc08dc7C8f4B96;
    ...
    function withdrawDivs() public returns (uint256 withdrawAmount){
        ...
        if(user.totalWiths == 0 && x > (user.totalInits * maxRoi)){
            x = user.totalInits * maxRoi;
        }else if((user.totalWiths + x) > user.totalInits * maxRoi) {
            x = user.totalInits * maxRoi - user.totalWiths;
        }else{
            require(user.totalWiths <= (user.totalInits * maxRoi),"You cannot withdraw more than 3x of total investments");
        }
    ...
    function setLastWithdrawTime(address _userAddress, uint256 _timestamp) public {
        User storage user = UsersKey[_userAddress];
