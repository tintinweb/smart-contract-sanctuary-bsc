    ...
    AccessControlUpgradeable
{
    ...
    string public uri;
    event SetURI(string _uri);
    event Mint(address _tp, uint256 _tokenId);
    mapping(uint256 => address) private sellerOwner;
    mapping(uint256 => uint256) private priceOf;
    event listedonMarket(uint256 nftID, address seller, uint256 price);
    event purchased(uint256 nftID, address seller, uint256 price);
    ...
    function __farmGameInit(
        ...
        grantRole(BURNING_ROLE, _msgSender());
        grantRole(VALIDATOR_ROLE, _msgSender());
    }
    ...
    function buyFromMarket(uint256 _nftId) external payable returns (bool) {
        ...
        require(sellerOwner[_nftId] != _msgSender(), "Cannot Buy My Own Item");
        require(
            msg.value >= priceOf[_nftId],
            "Amount should be greater than or Equal"
        );
    ...
    function mintAddr(address _to, string memory _tokenURI)
        external
        onlyRole(ADMIN_ROLE)
        returns (uint256)
    {
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId == maxnft, " maximum limit reaches");
        _mint(_to, tokenId);
    ...
    function setMaxNft(uint256 _id)
        public
        onlyRole(ADMIN_ROLE)
        returns (uint256)
    {
