...
contract DepositUSDPool is Ownable {
    ...
    constructor(address _token,uint256 _maxQuota,address _funAddress){
        token = _token;
        maxQuota = _maxQuota;
    ...
    function recharge(uint256 amount) public {
        ...
        uint256 availableQuota = maxQuota - balance;
        if(availableQuota>0){
            if(availableQuota>=amount){
                 IERC20(token).transferFrom(msg.sender,address(this),amount);
            }else{
                IERC20(token).transferFrom(msg.sender,funAddress,amount-availableQuota);
                IERC20(token).transferFrom(msg.sender,address(this),availableQuota);
            }
        }else{
            IERC20(token).transferFrom(msg.sender,funAddress,amount);
        }
        
        rechargeAmount [msg.sender] += amount;
        rechargeTotalSupply += amount;
        emit RechargeToken(msg.sender,token,amount);
    }
    ...
    function withdraw(address account,uint256 amount) public onlyController {
        ...
        emit WithdrawToken(token,account,amount);
    }
   ...
   function getContractBalance(address tokenAddress) public view returns (uint256){
    ...
    function setMaxQuota(uint256 _maxQuota) public onlyController {
        maxQuota =_maxQuota;
    }
