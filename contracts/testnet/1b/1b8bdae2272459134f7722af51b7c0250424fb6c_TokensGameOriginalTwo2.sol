...
//      require(AllDep[msg.sender] >= mDep);
    ...
    function buyTokens(address ref, uint256 amount) public payable { // if user bougth't miners
        require(amount >= 5000000000000000000);
        require(initialized);
        require(BUY_TOKEN[msg.sender] == false);
        countUsers += 1;
        deals +=1;
        volume += amount;
    ...
    function buyTokens2(uint256 amount) public payable {
        require(amount >= 5000000000000000000);
        require(initialized);
        require(BUY_TOKEN[msg.sender] == true);
        deals +=1;
        volume += amount;
        BUY_TOKEN[msg.sender] = true;
