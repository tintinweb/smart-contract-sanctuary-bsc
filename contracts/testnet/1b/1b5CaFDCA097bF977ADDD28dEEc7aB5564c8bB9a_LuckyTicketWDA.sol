...
contract LuckyTicketWDA is ReentrancyGuard, VRFConsumerBase {
    ...
    event NewBonus(uint256 startTime);
    ...
    function setValidTarget(address target, bool permission) external {
    ...
    function setTicket5000WDA() external onlyValidTarget {
        ...
        if (totalTicketSoldPerPeriod >= minimumPlayer) {
            ...
            for (uint256 i = 0; i < _winnerCount; i++) {
                ...
                _removeElement(_newIndexWinningTicket5000WDA);
            }
        }
    }

    function setTicket100000WDA() external onlyValidTarget {
        // if too few player => return cost of ticket
        if (totalTicketSoldPerPeriod >= minimumPlayer) {
            ...
            _payWinner(indexWinningTicket, 10000);
            _removeElement(indexWinningTicket);
        }
    }


    function _removeElement(uint256 index) internal {
    ...
    function _payWinner(uint256 index, uint256 _rewardAmount)
        internal
    {
        uint256 _reward = _rewardAmount * 10 ** 18;
        Ticket memory _ownerTicket = allTicket[index];
        if (_ownerTicket.proposalPercentage != 0) {
            uint256 proposalToWDA = _reward * _ownerTicket.proposalPercentage / 100;
        ...
        if (_rewardAmount == 5000) {
            ownerWinningTicket5000WDA[ownerOfTicket[index]].push(TicketReward(
                _reward,
                _ownerTicket.purchaseDate + lockTime
            ));
        } else {
            ownerWinningTicket100000WDA[ownerOfTicket[index]].push(TicketReward(
                _reward,
                _ownerTicket.purchaseDate + lockTime
            ));
        }
    ...
    function genRandomNumber() public onlyValidTarget {
        getRandomNumber();
        lastTimeCheck = block.timestamp;
        emit NewBonus(block.timestamp);
    }
        ...
        returns (uint256)
    {
        ...
        if (ticketType == 0) {
            for (
                ...
                TicketReward memory reward = ownerWinningTicket5000WDA[msg.sender][i]; 
            ...
            for (
                ...
                TicketReward memory reward = ownerWinningTicket100000WDA[msg.sender][i]; 
                if (reward.claimableDate <= block.timestamp) {
                    earned += reward.amount;
    ...
    modifier onlyValidTarget() {
        require(validTarget[msg.sender]);
        _;
    }
