...
contract ElectSmartContract is Ownable  {
    ...
    function endVote() public onlyOwner {
        require(voteStart == phase.voting, "Vote not started"); //Check
        candidateCount = 0;
        voterCount = 0;
        cycleCount++;
    ...
    function addCandidate(string memory _name) public onlyOwner {
        ...
        require(voteStart == phase.setUp, "Vote not started"); //check
        

        candidateCount++; //increment total candidates

        totalCandidate[candidateCount] = candidate(candidateCount, _name, 0); //set
    }

    function editCandidate (uint _id, string memory _name) public onlyOwner {

        //checks 
        require(voteStart == phase.setUp, "Vote not started");
    ...
    mapping (address => userVoted) private hasVoted;

    function addVote(uint _idCount) public {

        //checks
        require(voteStart == phase.voting, "Vote not started");
        ...
        token.transferFrom(msg.sender, deductAddress, 1);

        totalCandidate[_idCount].votes++;//set
        hasVoted[msg.sender].cycleVoted = cycleCount;
        hasVoted[msg.sender].votedFor = _idCount;
    }

    function cancelVote() public {
        require(voteStart == phase.voting, "Voting ended");
