...
contract NftMarketFactory is Ownable {
    ...
    function createMarketPair(
        ...
        NftMarket(pair).initialize(_nft, _feeMarketReceiver, _feeMarketNum, _feeCreatorReceiver, _feeCreatorNum);
        getPair[_nft] = pair;
        allPairs.push(pair);
...
contract NftMarket is ReentrancyGuard {
    ...
    // 10 min
    uint256 public updateInterval = 600;
    // tokenId -> owner
    ...
    function sellItem(uint256 _tokenId, uint256 _price) external {
        ...
        nftOrderIndexs[_tokenId] = nftOrders.length - 1;
    ...
    function remove(uint256 index) internal {
        ...
        for (uint256 i = index; i < nftOrders.length - 1; i++) {
            nftOrders[i] = nftOrders[i + 1];
        }
