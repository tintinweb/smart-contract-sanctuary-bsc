...
contract EmeraldFarm is AccessControlEnumerable
{
    ...
    function deposit(
        ...
        if (user.amount > 0) {
            ...
            _safeRewardTransfer(pool.rewardToken, _msgSender(), pending);
        }
        
        IERC20(address(pool.lpToken)).safeTransferFrom(_msgSender(), address(this), _amount);
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) public onlyRole(DEFAULT_ADMIN_ROLE) {
        massUpdatePools();
        rewardPerBlock = _rewardPerBlock;
    }

    function updatebonusEndBlock(uint256 _bonusEndBlock) public onlyRole(DEFAULT_ADMIN_ROLE) {
        massUpdatePools();
        bonusEndBlock = _bonusEndBlock;
    }

    function updatebonusMultiPlier(uint256 _bonusMultiPlier) public onlyRole(DEFAULT_ADMIN_ROLE) {
        massUpdatePools();
