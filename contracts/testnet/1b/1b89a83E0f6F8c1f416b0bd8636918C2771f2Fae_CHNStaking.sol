...
contract CHNStaking is OwnableUpgradeable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && supply != 0) {
            ...
            accCHNPerShare = accCHNPerShare.add(reward.mul(1e12).div(supply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 reward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accCHNPerShare = pool.accCHNPerShare.add(reward.mul(1e12).div(supply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function stake(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCHNPerShare).div(1e12).sub(user.rewardDebt);
