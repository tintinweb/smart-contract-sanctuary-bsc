...
contract DTM   {
   ...
   struct Profit {
    ...
    constructor(address _usdtAddr, address _defaultRefer, address  _feeReceivers1,address  _feeReceivers2,address _keepContract) public {
        usdt = IERC20(_usdtAddr);
        feeReceivers1 = _feeReceivers1;
        feeReceivers2 = _feeReceivers2;
        keepContract = _keepContract;
        KcContract = KC(_keepContract);
...
//减产机制 0-1.5年 10000 ，1.5-3年 8000 ，3-4.5年 5.12，4.5后4.096 
  function Reduceproduction()public view  returns(uint256)  {
        ...
        // 4
        else if(timeDifference > yearTime *3){
            proportion = 5120;
        }  
        return proportion; 
    }
    ...
    function updateFreed(address _address,uint256 _amount) external {     
        rewardInfo[_address].level5Freezed = _amount;
    }
    ...
    function withdraw() external {
        ...
        uint256  withdrawable =userRewards.capitals.add(userRewards.statics).add(userRewards.level4Released).add(userRewards.level5Released).add(userRewards.luck).add(userRewards.top).add(userRewards.keep); 
    ...
    function _setKeepFund(uint256 _bal) private{
           if(_bal<=500e18){
            ...
            for(uint256 i=orders.length-1;i>=0;i--){            
             ...
             if(userInfo[orders[i].user].totalRevenue>userInfo[orders[i].user].maxDeposit  || bal < orders[i].amount){
               continue; 
             }
             bal -= orders[i].amount;
             rewardInfo[orders[i].user].keep = orders[i].amount;
    ...
    function _deposit(address _user, uint256 _amount) private { 
        ...
        uint256 profit =  _amount.mul(12).mul(Reduceproduction()).div(100).div(10000);
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        if(!isUnfreezeCapital){ 
            ...
            if(userReward.level5Freezed > 0){
                uint256 release ;
                if(_amount <= userReward.level5Freezed && userReward.level5Freezed >=2000e18 ){
                    release = 2000e18;
                } else{
                 release = 0;   
                }
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
             ...
             if(i > 4){
                    ...
                    reward = newAmount.mul(300).div(baseDivider);
                    uint256 trueReward = reward.mul(7).div(10);
                    uint256 splitReward = reward.sub(trueReward);
                    upRewards.split = upRewards.split.add(splitReward);
                    upRewards.level4Released = upRewards.level4Released.add(trueReward);
