...
contract Factory {
    ...
    function name(address _tokens) public view returns(string memory) { return IERC20(_tokens).symbol(); }
    
    function creatAir(address _token ,uint256 _amount) external { 
        uint8 decimal = IERC20(_token).decimals();
        Airdrop Fair = new Airdrop(_token, msg.sender,_amount*(10**decimal));  
