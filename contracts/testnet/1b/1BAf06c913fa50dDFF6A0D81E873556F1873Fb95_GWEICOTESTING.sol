...
contract GWEICOTESTING is Context, Ownable {
    IBEP20 public Token2;
    IBEP20 public Token3;
    IBEP20 public token;    
    ...
    struct sale{
      ...
      uint256 percentageOfLimit1;
      uint256 percentageOfLimit2;
      uint256 percentageOfLimit3;
      bool hasStart;
    }    
    ...
    address public USDT=0x337610d27c682E347C9cD60BD4b3b107C9d34dDd; // test 
    ...
    constructor()  {
      ...
      priceFeedUSDT = AggregatorV3Interface(0x90c069C4538adAc136E051052E14c1cD799C41B7); // test 
      token=IBEP20(0xBf4B5f6c4B23A9d5adf9B81C23986d4bAb465C07); // test token 0x0145634232364EbA74F6FBaCa309CeA587287c30
      Token2 = IBEP20(BUSD);
      Token3 = IBEP20(USDT);
    }
    ...
    */
    function setPercentDivisor(uint256 _divisor) public onlyOwner{
      require(_divisor>0,"Must be greater then zero");      
      PERCENTS_DIVIDER = _divisor;
    }
    ...
    */
    function setSale(uint256 _startDate,uint256 _endDate,uint256 _tokenPerUsd,uint256 _minimumDeposite,uint256 _maximumDeposite,uint256 _percentageOfLimit1,uint256 _percentageOfLimit2,uint256 _percentageOfLimit3,bool _hasStart) public onlyOwner{
      clearSale();
      sales[saleType].startDate = _startDate;
      sales[saleType].endDate = _endDate;
      ...
      sales[saleType].percentageOfLimit1 = _percentageOfLimit1;
      sales[saleType].percentageOfLimit2 = _percentageOfLimit2;
      sales[saleType].percentageOfLimit3 = _percentageOfLimit3;    
    }
    /**
    * @dev BUY TOKEN FROM BNB.
    */
    function buyToken() public payable{
      ...
      uint256 convertedAmount = msg.value.mul(getLatestPriceBnb());
      require(convertedAmount>=sales[saleType].minimumDeposite,"Minimum Amount Not reached");
      require(convertedAmount<=sales[saleType].maximumDeposite,"Maximum Amount reached");  

      uint256 numberOfTokens=bnbToToken(msg.value,BNB);        
      if(convertedAmount>=sales[saleType].minimumDeposite && convertedAmount<=100*1e18){
          uint256 bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit1).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));             
      }else if(convertedAmount>100*1e18 && convertedAmount<=1000*1e18){
          uint256 bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit2).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(convertedAmount>1000*1e18 && convertedAmount<=sales[saleType].maximumDeposite){
          uint256 bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit3).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else{
        revert("No Limit has met");
      }
    }
    /**
    ...
    */   
    function swapTokens(address _token,uint256 _amount) public{
      ...
      if(_token == BUSD){
        convertedAmount = (_amount).mul(getLatestPriceBUSD()<0 ? 1 : getLatestPriceBUSD());
      }else if(_token == USDT){
        convertedAmount = (_amount).mul(getLatestPriceUSDT()<0 ? 1 : getLatestPriceUSDT());
      }    
      ...
      uint256 bonus = 0;
      // CHECK DEPOSITE AMOUNT CONDITON BASED ON LIMIT
      if(convertedAmount>=sales[saleType].minimumDeposite && convertedAmount<=100*1e18){
          bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit1).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(convertedAmount>100*1e18 && convertedAmount<=1000*1e18){
          bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit2).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));   
      }else if(convertedAmount>1000*1e18 && convertedAmount<=sales[saleType].maximumDeposite){
          bonus = numberOfTokens.mul(sales[saleType].percentageOfLimit3).div(PERCENTS_DIVIDER);
          token.transfer(msg.sender,numberOfTokens.add(bonus));  
      }else{ 
        revert("No Limit has met");
      }
      ...
      if(_token == BUSD){
        ...
        Token3.transferFrom(msg.sender,address(this),_amount);
      }          
    ...
    */   
    function bnbToToken(uint256 _amount, address currency) public view returns(uint256){
        ...
        numberOfTokens = bnbToUsd.mul(sales[saleType].tokenPerUsd);
    ...
    function withdraw(address _currency) public onlyOwner{
        ...
        if(_currency == BUSD){              
            ...
            Token3.transfer(owner(),Token3.balanceOf(address(this)));   
        }else{            
