...
contract SpaceBallNFT is ERC721URIStorage, Member {
    ...
    constructor() ERC721("SpaceBallBox", "SpaceBallBox") {}
    ...
    ) external CheckPermit("NFTConfig") returns (uint256) {
        uint256 tokenId = awardItem(_user, _tokenURI);
        cards[tokenId] = CardInfo(_cardGrade, _cardType);
        return tokenId;
    }
    ...
    function openBox(
        ...
        uint256 _cardGrade,
        uint256 _cardType
    ) external CheckPermit("OpenBox") returns (uint256) {
        uint256 tokenId = awardItem(_user, tokenURI);
        cards[tokenId] = CardInfo(_cardGrade, _cardType);
        return tokenId;
    }
    ...
    {
        return (cards[tokenId].cardGrade, cards[tokenId].cardType);
    }
}
