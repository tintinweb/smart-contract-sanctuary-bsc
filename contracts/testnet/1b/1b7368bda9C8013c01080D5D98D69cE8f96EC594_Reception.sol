...
contract Reception is Ownable {
  ...
  event Reserved(address player0, address player1, uint256 baseAmount);
  event RoomCreated(uint256 roomId, string reservedId);
  ...
  mapping(string => Reserve) reservedRooms;
  ...
  function isReservedUser(string memory _id, address user) internal view returns (bool) {
    if (user == reservedRooms[_id].player0 || user == reservedRooms[_id].player1) {
      return true;
    }
    return false;
  }
  ...
  // user deposit
  function userDeposit(string memory reservedId) public {
    require(token.balanceOf(msg.sender) >= reservedRooms[reservedId].baseAmount, "Insufficient Balance!");
    require(isReservedUser(reservedId, msg.sender), "Not Your Room");
    require(block.timestamp < reservedRooms[reservedId].startTime, "Expired!");
    reservedRooms[reservedId].jointCount++;
    Reserve storage reserved = reservedRooms[reservedId];
    if (reserved.jointCount == 2) {
      Room newRoom = new Room(token, address(this));
      uint256 realBaseAmount = reserved.baseAmount;
      if(devFee > 0) {
        realBaseAmount -= devFee;
        require(token.transferFrom(reserved.player0, devAddr, devFee), "Player0 Fee: failed!");
        require(token.transferFrom(reserved.player1, devAddr, devFee), "Player1 Fee: failed!");
      }
      // deposit token;
      require(token.transferFrom(reserved.player0, address(newRoom), realBaseAmount), "player0 can't deposit tokens");
      require(token.transferFrom(reserved.player1, address(newRoom), realBaseAmount), "player1 can't deposit tokens");
      // roomid
      ...
      emit RoomCreated(roomId, reservedId);
    }
  }
  ...
  // call by challenger
  function reserveRoom (string memory _dbId, address player0, address player1, uint256 baseAmount) public {
    reservedRooms[_dbId].player0 = player0;
    reservedRooms[_dbId].player1 = player1;
    reservedRooms[_dbId].baseAmount = baseAmount;
    reservedRooms[_dbId].startTime = block.timestamp.add(depositTime);
    emit Reserved(player0, player1, baseAmount);
  }
