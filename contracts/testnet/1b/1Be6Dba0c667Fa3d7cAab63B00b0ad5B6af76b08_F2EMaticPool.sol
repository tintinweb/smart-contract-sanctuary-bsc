...
contract F2EMaticPool {
    ...
    function payPoolPaymentReferral(
        ...
        if (users[_user].referrerID > 0) {
            ...
            users[referer].incomeOnIncome +=
                (Pool_Entry_fee[_poolno] * 5) /
                100;
    ...
    function buyPool(uint256 poolId) public payable {
        ...
        if (poolusers[poolId][currentUseraddress].payment_received < 3) {
            payable((currentUseraddress)).transfer(
                percentage(Pool_Entry_fee[poolId], 60)
            );
        ...
        payable((devWallet)).transfer((Pool_Entry_fee[poolId] * 500) / 1000);
        payable((ownerWallet)).transfer((Pool_Entry_fee[poolId] * 500) / 1000);
    }
    ...
    function _deposit(address _user, uint256 amount) internal {
        ...
        busd.transfer(devWallet, (amount * 75) / 1000);
        busd.transfer(ownerWallet, (amount * 75) / 1000);
