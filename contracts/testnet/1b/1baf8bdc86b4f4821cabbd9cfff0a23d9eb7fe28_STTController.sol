...
contract STTController is Ownable {
    ...
    constructor() {
        ...
        STTD = STTData(payable(address(0xb7bfD8be7dfEaAD3a5b9814FF6e52c6437c0D60F)));
        STTQ = STTQuery(address(0xcb3298DFd8E525D45a1962b677ADca485E4a0429));
    }
    ...
    function userTokenTransfer(address user, address to, uint256 amount) public {
        require(STTQ.queryUserGammaValue(user) > 0 && STTQ.queryUserGammaValue(to) > 0 && amount > 0, "You must add liquidity");
        ERC20(STTD.stt()).transferFrom(user, to, amount);

        STTD.addUserTokenRecords(user, 2, -int(amount), 6 + 10);
        STTD.addUserTokenRecords(to, 2, int(amount), 6 + 10);
    }
