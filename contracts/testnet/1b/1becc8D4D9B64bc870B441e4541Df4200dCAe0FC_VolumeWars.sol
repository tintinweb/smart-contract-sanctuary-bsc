...
contract VolumeWars is OwnableUpgradeable, ProxySafeVRFConsumerBase  {
    ...
    function distributeLegendaryDividends() external onlyOwner {
        ...
        legendaryPool = 0;
        payable(address(legendaryDividend)).call{value: distributedAmount}("");
    ...
    function getRandomInt(uint16 max, uint256 seed, uint8 nonce) pure private returns(uint8) {
...
contract LegendaryDividend is DividendPayingToken {
    ...
    function stakedNFTCollectionOf(address owner) external view returns (CollectionNFT[] memory nfts) {
        uint256 nftBalance = stakedNFTBalanceOf(owner);
        CollectionNFT[] memory nftCollection;
        for (uint8 i = 0 ; i < nftBalance; i++) {
            (uint256 tokenId, uint16 tokenSet, uint8 tokenNumber) = dataOfTokenOfOwnerByIndex(owner, i);
            nftCollection[i] = CollectionNFT({
                id: tokenId,
                num: tokenNumber,
                set: tokenSet,
                staked: true
    ...
    function stakeLegendary(uint256 tokenId) external {
        ...
        (uint16 _set, uint8 num) = rewardNFT.metadataOf(tokenId);
