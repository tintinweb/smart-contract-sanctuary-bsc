...
contract DogeChampionsTournament is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, VRFConsumerBaseV2Upgradeable, ITournament, KeeperCompatibleInterface
{
    ...
    function checkUpkeep(bytes calldata /*checkData*/) external view override returns (bool upkeepNeeded, bytes memory /*performData*/) {
        ...
        upkeepNeeded = endDate != 0 && block.timestamp > endDate;
    }
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        if (block.timestamp > tournamentIdToTournament[currentTournamentId.current()].endTimestamp)
        {
    ...
    function register(uint256 tournamentId, uint256[4] memory tokenIds) public payable override nonReentrant
    {
        ...
        for(i = 0; i < 4; i++)
        {
            if(tokenIds[i] != 0)
            {
                for(j = 0; j < 3; j++)
                {
                    if(calculateCRTChance(block.timestamp * (i + j + currentTournamentId.current() + tournamentIds.current())) < 25) // 25% chance to hit CRT
                    {
     ...
     */
    function calculateCRTChance(uint256 nonce) internal returns(uint256)
    {
        uint256 randomNumber = s_randomWords[0];
        s_randomWords[0] = ((randomNumber * block.timestamp) % 10000) * nonce;
        return uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce, randomNumber))) % 100;
    }
