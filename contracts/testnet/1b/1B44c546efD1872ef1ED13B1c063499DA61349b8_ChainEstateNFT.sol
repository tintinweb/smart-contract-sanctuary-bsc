...
contract ChainEstateNFT is ERC721URIStorage, Ownable {
    ...
    constructor(address _marketplaceAddress, address oldNFTAddress) ERC721("ChainEstateNFT", "CHESNFT") {
        ...
        for (uint id = 1; id <= oldNFTCount; id++) {
            createToken(oldNFTContract.tokenURI(id), oldNFTContract.tokenIdToPropertyId(id), oldNFTContract.tokenIdToListingFee(id));
        }
    }
    ...
    * @return the ID of the newly minted NFT
    ...
    /**
    * @dev Only owner function to mint a new NFT to someone other than the owner that's associated with a property.
    * @param tokenURI the token URI on IPFS for the NFT metadata
    * @param propertyId the ID of the property the NFT will be associated with
    * @param listingFee the fee the user pays when putting the NFT for sale on the marketplace
    * @return the ID of the newly minted NFT
     */
    function createTokenNonOwner(string memory tokenURI, uint256 propertyId, uint256 listingFee) private returns (uint) {        
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
