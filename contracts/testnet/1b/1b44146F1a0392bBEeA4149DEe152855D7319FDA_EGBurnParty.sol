...
contract EGBurnParty is Ownable, KeeperCompatibleInterface {
    ...
    function test() public returns (bool, bytes memory) {
        uint256 count = 0;
        for(uint256 i = 1; i <= burnPartyCounter.current(); i++){
            if(burnPartyById[i].startDate < block.timestamp && burnPartyById[i].started == false){
                count++;
            }
        }

        uint256[] memory _countArray = new uint256[](count);
        count = 0;
        for(uint256 i = 1; i <= burnPartyCounter.current(); i++){
            if(burnPartyById[i].startDate < block.timestamp && burnPartyById[i].started == false){
                _countArray[count] = i;
        ...
        return (_countArray.length > 0, abi.encode(_countArray));
    }

    function test1() external{
        (bool hello, bytes memory checkData) = test();
        uint256[] memory _countArray = abi.decode(checkData, (uint256[]));
        uint256[] memory _performData = abi.decode(abi.encode(_countArray), (uint256[]));
        for(uint256 i = 0; i < _performData.length; i++){
            BurnParty storage party = burnPartyById[i];
            party.started = true;
        }
    ...
    function performUpkeep(bytes calldata performData) external override {
        uint256[] memory _performData = abi.decode(performData, (uint256[]));
        for(uint256 i = 0; i < _performData.length; i++){
            BurnParty storage party = burnPartyById[i];
            party.started = true;
        }
