...
contract DexHero is ERC20, Ownable {
    ...
    mapping(address => uint32) private expires;
    ...
    function expiresOn(address user) public view returns (uint32){
        return expires[user];
    }
    ...
    function mint(address to, uint32 mintAmount) private {
        ...
        if(expires[to]<timestamp)expires[to]=uint32(timestamp);
        expires[to] += (mintAmount * 2592000); // months
    }
    ...
    receive() external payable {
        ...
        sendETH(msg.sender, msg.value - priceETH);
        usdSums[msg.sender] += uint32(getUSDPerToken(mintAmount) / 1e16);
    }
