...
contract PawSwap is Ownable {
    ...
    function buyOnPawSwap (
        ...
        if (_taxes.liquidityTax != 0) {
            ...
            liquidityEth = msg.value * (_taxes.liquidityTax / 2) / 10**(_taxes.feeDecimal + 2);
        ...
        if (_taxes.liquidityTax != 0) {
            // add to the LP
            uint256 liquidityTokens = newTokenBalance * (_taxes.liquidityTax / 2) / 10**(_taxes.feeDecimal + 2);
