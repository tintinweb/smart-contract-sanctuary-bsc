...
contract Lend is IERC721Receiver, ReentrancyGuard, AccessProtected {
    ...
    NFTGameToken public immutable nftGameToken;
    ...
    uint256 public treasuryPercentage=5;
    ...
    event NftClaimed(
     ...
     event BatchNftClaimed(
       uint256 inactivelisting,
       address indexed _lender
    ...
    constructor(
        ...
        address _nftGameToken
    ) {
        require(_seed.isContract(), "_seed must be a contract");
        ...
        nftGameToken = NFTGameToken(_nftGameToken);
    }
    function adminsettreasuryPercentage(uint256 feesamount) external onlyAdmin{
       require(feesamount<=100,"fee percent cannot be set greater than 100");
        treasuryPercentage =feesamount;
    ...
    function transferWithTreasury(
        ...
        uint256 treasuryFee = _amount.mul(treasuryPercentage).div(100);
        seed.safeTransferFrom(_from, treasuryAddress, treasuryFee);
        seed.safeTransferFrom(_from, _to, _amount.sub(treasuryFee));
    }
    ...
    function lendNft(
        ...
        userlistings[_msgSender()].push(listingId);
    ...
    function rentNft(uint256 listingId,uint256 totaldays) external nonReentrant {
        require(listings[listingId].isActive, "NFT is not Active");
        require(!listings[listingId].onLend, "NFT is already Lended");
        require(listings[listingId].duration > block.timestamp, "Listing Expired");
        uint256 praposalduration=totaldays.mul(86400);
        require(listings[listingId].duration>block.timestamp+praposalduration,"Praposed days exceeds listing duration");
        address seller = listings[listingId].lender;
        address buyer = _msgSender();
        uint256 price = listings[listingId].price;
        uint256 chargedamount=(listings[listingId].rentperiod.div(86400)).mul(price);
        listings[listingId].rentperiod=praposalduration;
        listings[listingId].onLend = true;
        listings[listingId].isActive = false;
        listings[listingId].rentexpire = block.timestamp + listings[listingId].rentperiod;
        
        // Transfer the seed Tokens
        transferWithTreasury(buyer, seller, chargedamount);

        // Transfer Game Token
        for (uint256 i = 0; i < listings[listingId].tokenIDs.length; i++) {
            nftGameToken.issueToken(
                msg.sender,
    ...
    function claimNft(uint256 listingId) external nonReentrant {
        ...
        for (uint256 i = 0; i < listings[listingId].tokenIDs.length; i++) {
            ...
            nftGameToken.burn(listings[listingId].tokenIDs[i]);
        }
    ...
    function batchclaimNft() external nonReentrant {
        uint256[] memory listingIds=userlistings[_msgSender()];
        uint256 len=listingIds.length;
        uint256 count=0;
        uint256 count1=0;
        uint256 inactiveCount;
        for(uint256 j=0;j<listingIds.length;j++){ 
        uint256 listingId= listingIds[j];
        for (uint256 i = 0; i < listings[listingId].tokenIDs.length; i++) {
        if(listings[listingId].onLend && block.timestamp > listings[listingId].rentexpire){
        ...
        if(listings[listingId].onLend==false){
            inactiveCount++;
        }
        }
        if(userlistings[_msgSender()].length==inactiveCount){
            delete userlistings[_msgSender()];
        }
        emit BatchNftClaimed(
            inactiveCount,
            _msgSender()
