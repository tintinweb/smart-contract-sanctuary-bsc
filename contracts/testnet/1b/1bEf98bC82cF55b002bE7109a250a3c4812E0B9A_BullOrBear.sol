    ...
    UUPSUpgradeable
{
    ...
    */
    ...
    uint256[] public betLvls;
    uint256[] public maxBetLvls;

    /// CONSTANTs
    uint256 public constant MAX_TREASURY_FEE = 10_000;
    uint256 public constant MAX_CANCEL_FEE = 10_000;
    ...
    function getBetLvls() public view returns (uint256[] memory) {
        return betLvls;
    }

    function getMaxBetLvls() public view returns (uint256[] memory) {
        return maxBetLvls;
    }
    ...
    function setBetLevelData(uint256[] memory _betLvls, uint256[] memory _maxBetLvls) external whenPaused onlyAdmin {
        ...
        emit NewBetLevelData(currentEpoch, betLvls, maxBetLvls);
    }
    ...
    function getMaxBetAmount(uint256 betCounter) public view returns (uint256 maxBetAmount) {
        for (uint256 index = betLvls.length - 1; index >= 0; index--) {
            if (betCounter >= betLvls[index]) {
                return maxBetLvls[index];
            }
        }
        return 0;
    }
