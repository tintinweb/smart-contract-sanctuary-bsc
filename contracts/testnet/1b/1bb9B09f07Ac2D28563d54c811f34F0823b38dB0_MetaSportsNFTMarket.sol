...
contract MetaSportsNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(Token).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
        require(_token != Token, "Operations: Cannot recover Token");
     ...
     * @param _withBNB: whether the token is bought with BNB (true) or Token (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
