...
contract Staking is AccessLevel {
    ...
    event Stake(uint256 stakeId, uint256 lockTime, address staker);
    event UpgradeStake(uint256 stakeId, uint256 oldLockTime, uint256 newLockTime, address staker);
    ...
    @param rewardRate_ reward tokens per second
     */
    function initialize(address tokenAddress_, uint rewardRate_) initializer external {
        __AccessLevel_init(msg.sender);
    ...
    */
    function rewardPerToken() public view returns (uint) {
        ...
        return rewardPerTokenStored + (((block.timestamp - lastUpdateTime) * rewardRate * 1e18) / totalTokensStakedWithBonusTokens);
    }
    ...
    function earned(address account_) public view returns (uint) {
        return ((balances[account_] * (rewardPerToken() - userRewardPerTokenPaid[account_])) / 1e18) + rewards[account_];
    }
    ...
    function stake(uint amount_, uint lockTime_) external updateReward(msg.sender) {
        ...
        emit Stake(stakingNonce[msg.sender], lockTime_, msg.sender);
    ...
    /** Upgrade stake lock time for higher rewards
    */
    function upgradeStake(uint stakeId_, uint newLockTime_) external updateReward(msg.sender) {
        require(stakingEnabled, "STAKING_DISABLED");
        require(bonusTokenMultiplier[newLockTime_] > 0, "LOCK_TIME_ERROR");

        StakingInfo storage info = stakingInfoForAddress[msg.sender][stakeId_];
        require(info.lockTime > newLockTime_, "SMALLER_LOCK_TIME");

        emit UpgradeStake(stakeId_, info.lockTime, newLockTime_, msg.sender);

        uint amount = info.tokensStaked;
        uint tokensWithBonus = amount * bonusTokenMultiplier[newLockTime_] / DIVISOR;
        info.stakingTime = block.timestamp;
        info.lockTime = newLockTime_;
        info.timeToUnlock = block.timestamp + newLockTime_;
        info.tokensStakedWithBonus = tokensWithBonus;

        getRewardInternal();
    }

    /** Unstake function
    * Users cannot unstake until the tokens are unlocked, except for users that chose to unlock for 0 months
    @param stakeId_ the stake id to unstake
     */
    function unstake(uint stakeId_) external updateReward(msg.sender) {
        StakingInfo storage info = stakingInfoForAddress[msg.sender][stakeId_];
        require(info.timeToUnlock < block.timestamp, "UNSTAKE_TOO_EARLY");
    ...
    function getReward() external updateReward(msg.sender) {
        require(lastClaimedTimestamp[msg.sender] <= block.timestamp, "CANNOT_CLAIM_YET");
        getRewardInternal();
    }
     ...
     */
    function setBonusMultiplier(uint[] calldata durations_, uint[] calldata mutiplier_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    ...
    {
        StakingInfo[] memory stakingList = new StakingInfo[](stakingNonce[userAddress_]);
    ...
    /**
    @dev Returns stake info in the following order (timeToUnlock, stakingTime, lockTime, tokensStaked)
    @param ownerAddress_ address of the owner
    @param stakeNonce_ stake nonce for identification
    */
    function getStakeInfo(address ownerAddress_, uint stakeNonce_) public view returns (uint, uint, uint, uint) {
        StakingInfo memory info = stakingInfoForAddress[ownerAddress_][stakeNonce_];
        return (info.timeToUnlock, info.stakingTime, info.lockTime, info.tokensStaked);
    }
}
