...
contract TestStaking3 is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant BLOCK_TIME = 1;
    ...
    function getRemainingReward(address stakingToken, address rewardToken) public view returns (uint256) {
        ...
        uint256 lastRewardBlock = campaign.stopBlock > 0 ? campaign.stopBlock : block.timestamp; 
    ...
    function getApr(address stakingToken, address rewardToken) public view returns (uint256) {
        if (campaigns[stakingToken][rewardToken].finishBlock < block.timestamp) return 0;
    ...
    function _getApr(address stakingToken, address rewardToken, uint256 rate) internal view returns(uint256) {
        if (campaigns[stakingToken][rewardToken].totalStakes == 0 || fiatContract.getLatestPrice(stakingToken) == 0) return 0;
        ...
        onlyOwner
    {
        ...
        campaigns[stakingToken][rewardToken].finishBlock = block.timestamp;
    }
    ...
    function rewardPerToken(address stakingToken, address rewardToken) public view override returns (uint256) {
        if (campaigns[stakingToken][rewardToken].totalStakes == 0 || campaigns[stakingToken][rewardToken].rewardAmount == 0) return campaigns[stakingToken][rewardToken].rewardPerTokenGlobal;
    ...
    function lastUpdatedBlock(address stakingToken, address rewardToken) public view returns (uint256) {
        return block.timestamp < campaigns[stakingToken][rewardToken].finishBlock ? block.timestamp : campaigns[stakingToken][rewardToken].finishBlock;
    }
        ...
        updateReward(msg.sender, stakingToken, rewardToken)
    {
        ...
        if (campaign.totalStakes == 0 && campaign.finishPeriod > 0) {
            uint256 blockPeriod = campaign.finishBlock >= 0 ? campaign.finishBlock.sub(block.timestamp) : 0;
            campaign.startBlock = block.timestamp;
        ...
        updateReward(address(0), stakingToken, rewardToken)
    {
        ...
        uint256 startPeriod = (campaign.startPeriod > 0 && campaign.finishPeriod >= block.timestamp && campaign.totalStakes == 0) ? campaign.startPeriod : block.timestamp;
        ...
        campaign.lastUpdateBlock = block.timestamp;
    ...
    function _calculateRate(address stakingToken, address rewardToken, uint256 rewardAmount, uint256 blockPeriod) internal {
        ...
        if (block.timestamp >= campaign.finishBlock) {
            campaign.rewardAmount = rewardAmount;
            if (campaign.totalStakes != 0) campaign.startBlock = block.timestamp;
            campaign.startPeriod = block.timestamp;
            campaign.finishBlock = block.timestamp.add(blockPeriod);
        } else {
            campaign.rewardAmount = getRemainingReward(stakingToken, rewardToken).add(rewardAmount);
            campaign.finishBlock = (campaign.startBlock > 0 ? campaign.startBlock : block.timestamp).add(blockPeriod);
        }
    ...
    function _stopReward(address stakingToken, address rewardToken) internal {
        Campaign storage campaign = campaigns[stakingToken][rewardToken];
        campaign.stopBlock = block.timestamp;
        campaign.rate = 0;
    }
