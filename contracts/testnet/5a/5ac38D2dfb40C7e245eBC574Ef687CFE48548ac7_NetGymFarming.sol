...
contract NetGymFarming is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setTreasuryAddress(address _newTreasury) external onlyOwner {
        require(address(_newTreasury) != address(0), "Treasury Address can not be 0");
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            ...
            if (_amount > rewardTokenBal) {
                ...
                IGymMLM(relationship).distributeRewards(rewardTokenBal, address(rewardToken), msg.sender);
