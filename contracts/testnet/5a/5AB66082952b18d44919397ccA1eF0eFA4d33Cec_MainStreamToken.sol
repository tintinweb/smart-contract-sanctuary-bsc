...
contract MainStreamToken is BEP20, MinterRole {
    ...
    uint256 public minSwapAmount = 100000000;   //0.1 MainStream tokens
    ...
    /**z
     * @notice Constructs the MainStream token contract.
     */
    constructor(address _treasury) public BEP20("MainStreamToken", "MainStream") {
        operator = _msgSender();
    ...
    /// @dev overrides transfer function to meet tokenomics of MainStream
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "MainStream::transferOperator: new operator is the zero address");
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "MainStream:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "MainStream:: Sell Fee can't exceed 100%");
    ...
    function updateFeeWallet(address _treasury) public onlyOperator {
        require( _treasury != address(0), "MainStream:: tax fee address is zero" );
