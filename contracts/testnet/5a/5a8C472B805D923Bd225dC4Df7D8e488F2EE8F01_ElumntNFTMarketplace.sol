...
contract ElumntNFTMarketplace is Initializable, UUPSUpgradeable, OwnableUpgradeable, Verification {
    ...
    function callMint1155(address nft, uint256 totalQuantity) internal returns(uint256) {
        mint1155Data memory mintData = mint1155Data(
            payable(msg.sender),
            nft,
            totalQuantity
        );
        return mint1155(mintData);
    }
    function buy721Process(single721Data memory _nftData, uint8 currency, address erc20token) internal {
    ...
    function buy1155Proces(single1155Data memory _nftData, uint8 currency, address erc20token) internal {
        uint256 platformSharePercent;
        uint256 singleNFTAmount = _nftData.amount;
        ...
        if(_nftData.tokenId == 0) {
            mint1155Data memory mintData = mint1155Data(
                payable(msg.sender),
                _nftData.nft,
                _nftData.totalQuantity
            );
            _nftData.tokenId = mint1155(mintData);
        }
        ...
        transfer1155(_nftData.nft, _nftData.owner, msg.sender, _nftData.quantity, _nftData.tokenId);
    ...
    function sweepCollection1155(sweepCollection1155Data memory _buyData) external payable onlyProxy {
        ...
        for(uint8 i = 0; i < nftsData.length; i++) {
            buy1155Proces(nftsData[i], _buyData.currency, _buyData.erc20token);
        }
    }
