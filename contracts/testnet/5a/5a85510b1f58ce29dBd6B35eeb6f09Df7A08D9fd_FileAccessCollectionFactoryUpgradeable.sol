...
contract FileAccessCollectionFactoryUpgradeable is OwnableUpgradeable  {
    ...
    function newFileAccessContract(
        ...
        BeaconProxy proxy = new BeaconProxy (
            ...
            abi.encodeWithSelector(
                IFileAccessCollectionUpgradeable(address(0)).initialize.selector, _msgSender(), name, description
            )
        );

        address proxyAddress = address(proxy);
        workspaceCollections[workspaceId].push(proxyAddress);
        contractsAddresses.push(proxyAddress);
        emit AddNewFileAccessContract(proxyAddress, workspaceId, name, description);
        return proxyAddress;
    }
