...
contract RunTogetherPool is Ownable, ReentrancyGuard {
    ...
    uint256[] public feeUnstake = [30, 10]; // fee 3%, 1% unstake
    uint256[] public feeUnstakeDuration = [2 * 24 * 60 * 60, 30 * 24 * 60 * 60]; // fee unstake in 2 days, 30 days
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function withdraw(uint256 _amount) external nonReentrant isRun {
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
        UserInfo storage user = userInfo[msg.sender];
        ...
        if (_amount > 0) {
            ...
            stakedToken.safeTransfer(msg.sender, amountWithdraw);
        }
