 ...
 */
contract Pontual is Context, IBEP20, Ownable, ReentrancyGuard {
     ...
     * construction.
     */
    string private constant _NAME = "Pontual Certificadora";
    string private constant _SYMBOL = "Pcd";
    ...
    address public developmentWallet = 0x363a3599fCa9a394f41E78e41EcC953cae13698D;
    ...
    address public charityWallet = 0x36B5ac8784907041457049fd918ab5EC0818a866;
    uint256 private _previouscharityFee = _charityFee;

    /**
     * @dev Percentage of the marketing, development, and team fee.
     */   
    uint256 public _adrianFee = 1;
    address public adrianWallet = 0xEFedb45952781Cbb182365DB784D4dF1a71895F4;
    uint256 private _previousadrianFee = _adrianFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 adrianAmt = amount.mul(_adrianFee).div(100);
        

        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(charityAmt).sub(adrianAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(charityAmt).sub(adrianAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(charityAmt).sub(adrianAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(burnAmt).sub(developmentAmt).sub(charityAmt).sub(adrianAmt)));
        }
        ...
        _transferStandard(sender, adrianWallet, adrianAmt);
