...
contract ICO is Ownable {      
    ...
    constructor (address addr) payable {
        ...
        //uint256 lngt = getOrderLength(msg.sender);
        users[msg.sender].orders++;
        orderInfos[msg.sender][0] = OrderInfo(
            0, 
            block.timestamp, 
            unfreezeTime,
            false,
    ...
    function register(uint256 sp) external returns(bool){
        ...
        require(users[upline].orders > 0, "Sponsor not Active.");
        ...
        //uint256 drs = users[upline].directs;
        directs[upline][users[upline].directs]=msg.sender;
    ...
    function deposit(uint256 amount) external returns (bool) {
        ...
        orderInfos[msg.sender][users[msg.sender].orders] = OrderInfo(
            tokens, 
            block.timestamp, 
            unfreezeTime,
            false,
    ...
    function unfreeze(uint256 ordrId)external returns(uint256,uint256,uint256,uint256){
        
        OrderInfo storage order = orderInfos[msg.sender][ordrId];
        User storage user = users[msg.sender];

        uint256 blnc = uint256(block.timestamp).sub(order.start);
        uint256 sryh= blnc.div(rewardAfter);
        uint256 newIncome = order.amount.mul(rewardPer).div(baseDivider);
        uint256 desIncome = newIncome.mul(sryh);
        uint256 incm = desIncome.sub(order.freezed);

        if(incm>0 && order.amount > order.freezed.add(incm) && order.freezed.add(incm)<=newIncome.mul(24)){
       ...
       // return true;
        return (sryh,newIncome,desIncome,incm);
    ...
    function calculateBusinessReward(address addr) public view returns(uint256){
        ...
        if(my_business>0){
            uint256 myextb ;
            if(my_business.div(1e18)>=100000){
                myextb = businessInRatio(addr, uint256(100000).mul(1e18));
                if(myextb >= uint256(100000).mul(1e18)){
                    ret = rewardincome[0].mul(1e18);
                }
            }            
            if(my_business.div(1e18)>=500000){
                myextb = businessInRatio(addr, uint256(500000).mul(1e18));
                if(myextb>= uint256(500000).mul(1e18)){
                    ret = (ret.add(rewardincome[1])).mul(1e18);
                }
            }
            
            if(my_business.div(1e18)>=2500000){
                myextb = businessInRatio(addr, uint256(2500000).mul(1e18));
                if(myextb>=uint256(2500000).mul(1e18)){
                    ret = (ret.add(rewardincome[2])).mul(1e18);
                }
            }
            
            if(my_business.div(1e18)>=10000000){
                myextb = businessInRatio(addr, uint256(10000000).mul(1e18));
                if(myextb>= uint256(10000000).mul(1e18)){
                    ret = (ret.add(rewardincome[3])).mul(1e18);
                }
            }
            
            if(my_business.div(1e18)>= 50000000){
                myextb = businessInRatio(addr,  uint256(50000000).mul(1e18));
                if(myextb>=uint256(50000000).mul(1e18)){
                    ret = (ret.add(rewardincome[4])).mul(1e18);
                }
            }
            
            if(my_business.div(1e18)>= 250000000){
                myextb = businessInRatio(addr,  uint256(250000000).mul(1e18));
                if(myextb>=uint256(250000000).mul(1e18)){
                    ret = (ret.add(rewardincome[5])).mul(1e18);
                }
            }
        }
        return ret;
    ...
    function businessInRatio(address _usr,uint256 amnt)internal view returns(uint256){        
        ...
        if(amnt>0){
            ...
            address upl; 
            for(uint32 j=0;j<users[_usr].directs;j++){
                upl = directs[_usr][j];
    ...
    function migrate(address usr, uint256 _id,address _sponsor, uint40 _directs, uint256 _balance, uint256 _commision, uint256 _claimed_income, uint256 _total_deposit, uint256 _team_deposit, bool _status, uint256 _claimed_reward, uint40 _timestamp)public returns(bool){
        addresses[_id]=usr;
        users[usr].id= _id;        
    ...
    function migrateOrder(address usr, uint256 _cnt,uint256[] memory _amount, uint256[] memory _start, uint256[] memory _unfreeze, uint256[] memory _freezed)public returns(bool){
        
        users[usr].orders = uint40(_cnt);
    ...
    function migrateDirect(address usr, uint256 _cnt,address[] memory _dirs) public returns(bool){
