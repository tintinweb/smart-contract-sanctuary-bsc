...
event Hi(string s);
    ...
    event HiU(uint s);
    event HiB(bool s);
    function storeTradeHi(Trade calldata _trade) external onlyTrading{
        emit Hi("Hola");
        emit HiA(_trade.trader);
        emit HiU(_trade.pairIndex);
        emit HiU(_trade.index);
        emit HiU(_trade.initialPosToken);
        emit HiU(_trade.positionSizeDai);


        emit HiU(_trade.openPrice);
        emit HiB(_trade.buy);
        //emit HiU(_trade.leverage);
        //emit HiU(_trade.tp);
        //emit HiU(_trade.sl);
    }
    // Manage stored trades
    function storeTradePart1(Trade calldata _trade) external onlyTrading{
    ...
    function storeOpenTradeInfoFromCallback(TradeInfo calldata _tradeInfo, address trader, uint pairIndex, uint index) external onlyTrading{
        openTradesInfo[trader][pairIndex][index] = _tradeInfo;
    }
    ...
    function updateOpenInterestDai(uint _pairIndex, uint _leveragedPosDai, bool _open, bool _long) public onlyTrading{
