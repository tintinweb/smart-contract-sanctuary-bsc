...
contract TreasuryNode is Initializable,AccessControl {
  ...
  bytes32 public constant ONLY_APROVE_ROLE = keccak256("ONLY_APROVE_ROLE");
  ...
  function isCancelOperator(address account) public view returns (bool) {
    return isAdmin(account) || isSuperOperator(account) || hasRole(ONLY_CANCEL_ROLE, account);
  }
  function isAproveOperator(address account) public view returns (bool) {
    return isAdmin(account) || isSuperOperator(account) || hasRole(ONLY_APROVE_ROLE, account);
  }
  function withdraw() public onlyAdmin{
        (bool success, ) = payable(msg.sender).call{value: address(this).balance}("");
        require(success);
  }
  function withdrawCoin(address erc20) public onlyAdmin{
       IERC20(erc20).transfer(msg.sender,IERC20(erc20).balanceOf(msg.sender));
  }
}