...
contract DexBot is Initializable {
    ...
    function timeDifferenceandReward(uint256 _pid,address _userAddress) public view returns (uint256,uint256) {
        StakePoolInfo storage pool = poolInfo[_pid];
        UserInfo storage userstake = userInfo[_pid][_userAddress];
        uint256 multiplier;
        uint256 userPending;
        uint256 perMonthsec = 86400 * pool.duration;
            ...
            if(multiplier > 0) {
                 userPending =  userstake.stakeReward + (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100);
            }
          return (multiplier,userPending);
    }
