...
// Developed by: Lazarus.ETH (akbarmurtaza)
interface IERC20 { 
...
contract Cleartoken is Context, IERC20, Ownable, ReEntrancyGuard {
    ...
    mapping (address => bool) public _isDividendReceiver; //mapping for eligible holder
    ...
    address[] public _dividendReceiver; // storing Addresses of eligible holders
    uint256 public _purseDividends;
    //uint256 public _lastDistributionTime;
    uint256 public _dividendDistributionIncrement = 30 days;
   
    address payable public _charityAddress = payable(0x0f6192f2520Dfc32B46f10c4bD1345147a57f6Bc); // Charity Address
    address payable public _devAddress = payable(0x0f6192f2520Dfc32B46f10c4bD1345147a57f6Bc); //dev Address

    uint256 existingContibutors = 0;
    ...
    uint256 public _burnFee = 0; // burn fee 0.0%
    ...
    uint256 public _charityFee = 0; // charity fee 0%
    uint256 private _previousCharityFee = _charityFee; // charity fee

    uint256 public _devFee = 0; // dev fee 0%
    uint256 private _previousdevFee = _devFee; // dev fee
    
    uint256 public _liquidityFee = 50; // actual liquidity fee 5%
    uint256 public _transferFee = 60; // actual transfer fee 6% on both buy and sell
    uint256 private _previousLiquidityFee = _liquidityFee; // restore actual liquidity fee

    uint256 private _totalLiquidityFee = _charityFee.add(_liquidityFee).add(_devFee); // liquidity + charity + dev fee on each transaction
    ...
    uint256 public _maxTxAmount = 2000 * 10**6 * 10**9; // max allowed tokens tranfer per transaction
    uint256 public  numTokensSellToAddToLiquidity = 50 * 10**6 * 10**9; // min token liquidity fee collected before swapandLiquify
    uint256 public _maxTokensPerAddress            = 10000 * 10**9; // Max number of tokens that an address can hold
    ...
    constructor () {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1); // TO-DO Chnage Adress While Deploying
    ...
    function setCharityFeePercent(uint256 Fee) external onlyOwner() {
        _charityFee = Fee;
        _totalLiquidityFee = _liquidityFee.add(_charityFee).add(_devFee);
    }
    ...
    /**  
     * @dev Blacklist a single wallet from receiving dividends
     */
    function blacklistFromDividend(address account) public onlyOwner(){
        _isDividendReceiver[account] = false;
    }

    /**  
     * @dev Whitelist a single wallet to receive dividends
     */
    function whitelistFromDividend(address account) public onlyOwner(){
        _isDividendReceiver[account] = true;
    }
    ...
    function removeAllFee() private {
        if(_totalTaxFee == 0 && _totalLiquidityFee == 0 && _burnFee == 0 && //To-DO add check
    ...
    function _transfer(
        ...
        if(_msgSender() != owner())
        {
            uint256 dividendToAdd = amount.mul(_dividendFromtx).div(1000);
            amount = amount - dividendToAdd;
            _purseDividends = _purseDividends + dividendToAdd;

            // Base transfer fee deduction
            uint256 baseTransferdeductAmount = amount.mul(_transferFee).div(1000);
            amount = amount - baseTransferdeductAmount;
        }
    ...
    function swapAndLiquify(uint256 tokenBalance) private lockTheSwap {
        ...
        if(_devFee > 0)
        {
            uint256 devAmount = tokenBalance.mul(_devFee);
            devAmount = devAmount.div(_totalLiquidityFee);
            liquidityAmount = tokenBalance.sub(devAmount);
            // swap dev tokens for ETH
            //swapTokensForEth(devAmount);

            // send tokens to dev
            swapTokensForEth(_devAddress, devAmount);
    ...
    /*
    function distributeDividendsManual() public onlyOwner() {
        distributeDividends();
    }

    function distributeDividends() private {
        uint256 _stampCheck;
        _stampCheck = _lastDistributionTime.sub(block.timestamp).div(86400);
        if(_stampCheck >= 30)
        {
            uint256 dividendPerAccount = _purseDividends.div(_dividendReceiver.length);
            for(uint256 i=0;i<_dividendReceiver.length;)
            {
                if(_isDividendReceiver[_dividendReceiver[i]] == false)
                {
                    continue;
                }
                else{
                    _transfer(owner(),_dividendReceiver[i],dividendPerAccount);
                    _tDividendReceived[_dividendReceiver[i]] = _tDividendReceived[_dividendReceiver[i]] + dividendPerAccount;
                }
                i++;
            }
            _purseDividends = 0;
            _lastDistributionTime = block.timestamp + _dividendDistributionIncrement;
            
        
    }
}*/

    function claimDividend() external noReentrant() {
        require(_isDividendReceiver[_msgSender()] == true,"msg.sender not eligible for dividends");
        uint256 numdaysLastclaim = _lastDividendClaim[_msgSender()].sub(block.timestamp).div(86400);
        require(numdaysLastclaim > 30,"Not enough time elapsed since last dividend claim");
        uint256 dividendPerAccount = _purseDividends.div(_dividendReceiver.length);
        _transfer(owner(),_msgSender(),dividendPerAccount);
        _tDividendReceived[_msgSender()] = _tDividendReceived[_msgSender()] + dividendPerAccount;
    }
}