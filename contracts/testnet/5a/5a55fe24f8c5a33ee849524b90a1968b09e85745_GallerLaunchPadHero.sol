...
contract GallerLaunchPadHero is BaseLaunchPad {

  function initialize(address _launchPadManager, address _launchPadPartnerAddress, uint16 _classId, uint _price) public initializer {
...
import "../interfaces/ILaunchPadManager.sol";

abstract contract BaseLaunchPad is Auth {
  ILaunchPadManager public launchPadManager;
  uint16 public classId;
  uint public price;

  function getMaxLaunchpadSupply() external view returns (uint) {
    return launchPadManager.maxTotalSupply();
  }

  function getLaunchpadSupply() external view returns (uint) {
    return launchPadManager.totalSupply();
  }

  function setClassId(uint16 _classId) external onlyMainAdmin {
    require(_classId > 0, "classId is invalid");
    classId = _classId;
  }

  function setPrice(uint _price) external onlyMainAdmin {
    require(_price > 0, "price is invalid");
    price = _price;
  }

  function mintTo(address _to, uint _size) virtual external;
}
