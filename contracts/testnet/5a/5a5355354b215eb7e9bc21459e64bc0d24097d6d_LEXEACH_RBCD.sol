...
contract LEXEACH_RBCD {
    ...
    constructor(address _token, address stableCoin_, address exam_) public {
        ...
        currUserID++;
        REGESTRATION_FESS = 500000000000000000 * nthRootSol(currUserID*1e48 , 24);
    ...
    function Registration(uint _referrerID, uint _coreferrerID,uint256 _amountToken, uint256 _amountUSDT) public payable {
        ...
        //    if (!isTokenAcceptance) {
            ...
            }else if(_amountUSDT > 0){
            require(_amountUSDT == REGESTRATION_FESS, "Incorrect Value");
            require(stableCoin.allowance(msg.sender, address(this)) >= _amountUSDT,"NEED_TO_APPROVE_TOKEN"); 
            stableCoin.transferFrom(msg.sender, address(this), _amountUSDT);
            tokenReward =  _amountUSDT / tokenPrice() * 1e18;
            } else {
            
            require(_amountToken == REGESTRATION_FESS / tokenPrice() * 1e18, "Incorrect Value");
            require(token.allowance(msg.sender, address(this)) >= _amountToken,"NEED_TO_APPROVE_TOKEN");
            token.transferFrom(msg.sender, address(this), _amountToken);
            tokenReward = _amountToken ;
            isToken = true;
        }
        ...
        users[userList[users[msg.sender].referrerID]].referredUsers =
            users[userList[users[msg.sender].referrerID]].referredUsers +
            1;
        users[userList[users[msg.sender].referrerID]].coreferredUsers =
            users[userList[users[msg.sender].referrerID]].coreferredUsers +
            1;    
        if(isRewarding){
        ...
        autoPool(msg.sender,msg.value, isToken);

        payReferral(1, msg.sender, msg.value, isToken);
        paycoReferral(1, msg.sender, msg.value, isToken);
        if (isTokenAcceptance) {
    ...
    function autoPool(address _user, uint256 _value, bool isToken) internal {
        bool sent = false;
        ownerPaid = false;
        uint i;
    ...
    function payReferral(uint _level, address _user, uint _value, bool isToken) internal {
        address referer;
        ...
        if (sent) {
            ...
            if (_level < 10 && users[referer].referrerID >= 1) {
                payReferral(_level + 1, referer, _value, isToken);
        ...
        if (!sent) {
            ...
            payReferral(_level, referer,_value, isToken);
        }
    }

      function paycoReferral(uint _level, address _user, uint _value, bool isToken) internal {
        address referer;
        ...
        if (sent) {
            ...
            if (_level < 10 && users[referer].coreferrerID >= 1) {
                paycoReferral(_level + 1, referer,_value, isToken);
            } else {
                sendBalance(_value, isToken);
            }
        }
        if (!sent) {
            //  emit lostMoneyForLevelEvent(referer, msg.sender, _level, now);
            paycoReferral(_level, referer, _value, isToken);
        }
    }

    function gettrxBalance(uint256 _value,bool isToken) public view returns (uint) {
        // if (!isTokenAcceptance) {
            if(_value>0){
    ...
    function sendBalance(uint _value, bool isToken) private {
        users[ownerWallet].income = users[ownerWallet].income + gettrxBalance(_value, isToken);
        // if (!isTokenAcceptance) {
            if(_value>0){
            if (!address(uint160(ownerWallet)).send(gettrxBalance(_value,isToken))) {}
            }else if(!isToken){
                // if(!stableCoin.transferFrom(msg.sender,address(uint160(ownerWallet)),gettrxBalance(_value))){}
                if(!stableCoin.transfer(address(uint160(ownerWallet)),gettrxBalance(_value,isToken))){}
