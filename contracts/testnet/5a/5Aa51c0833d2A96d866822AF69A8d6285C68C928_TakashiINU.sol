...
contract TakashiINU is IERC20, Ownable {
    ...
    address public marketingAddress= address(0x1C342c8cD3Cca235F4EF22AAc208519060bCf1CB); //Marketing walletAddress
    address public devAddress= address(0x1A88979d4fB33c3D4072DDB67Ed57d256f780B79); //Dev walletAddress
    ...
    constructor() 
    {
        // pancakeswapV2Router = IUniswapV2Router02(address(0x10ED43C718714eb63d5aA57B78B54704E256024E)); // Mainnet
        pancakeswapV2Router = IUniswapV2Router02(address(0xD99D1c33F9fC3444f8101754aBC46c52416550D1)); // TestNet
        ...
        isExcludedFromFee[pancakeswapV2Pair]=true;
        isExcludedFromFee[address(pancakeswapV2Router)]=true;
        mintAddresses();
    }
    ...
    function _transfer(address from,address to,uint256 amount) internal {
        ...
        //anti-whale checks 
        if(from!=owner() && //skip if adding liquidity
        to!=pancakeswapV2Pair && //swap tokens for eth
        from!=pancakeswapV2Pair && // skip swap for eth for tokens
        from!=address(pancakeswapV2Router) //skip if removing liquidity
        ){
        ...
        if(balanceOf(address(this))>maxCollectionTillSwap && //check accumalation
         from!=pancakeswapV2Pair && //remove liquidity pair to router skip
          from!=address(pancakeswapV2Router) && //remove liquidity router to To skip
           from!=owner() && //add liquidity skip
            !inSwapAndLiquidity //lock
            ){
    ...
    function swapTokensAndAddLiquidity() private Lock{
        ...
        uint256 swappedBNB = address(this).balance-contractOldBNB;
        //add liquidity
