...
contract LaunchpadFactory is ReentrancyGuard, Ownable, Pausable {
    ...
    public pure returns (uint256 tokens) {
        tokens = (hardcap * presaleRate) / 1 ether;
        tokens = tokens + ((hardcap * liquidityPercent* listingRate) / 10000 ether);
    }
    ...
    payable nonReentrant whenNotPaused {
        ...
        LaunchpadPresale(payable(clone)).initialize(
            feeAddress,
            token,
            router,
            stakingContract,
        ...
        LaunchpadPresale(payable(clone)).transferOwnership(msg.sender);
...
contract LaunchpadPresale is Initializable, ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 _influencerFee, uint256 _finalizeForOwner) public initializer {
    ...
    /**
     * @dev Enable/Disable Whitelist Tier
     */
    function changeWhitelistStatus(bool _status) external {
        wpar.whitelist = _status;
    }
    ...
    function buy() public payable nonReentrant whenNotPaused {
        ...
        require(isFinalized==false, "Finalized");
        require(totalBought + msg.value <= par.hardcap, "Sold out");
    ...
    function finalize() external onlyOwner {
        require(totalBought >= percent(par.hardcap, 9900) || block.timestamp > par.endTime, "Sale is not ended");

        if (block.timestamp > par.endTime && ((block.timestamp - par.endTime) < finalizeForOwner)) {
    ...
    function finalizeManual() external onlyOwner {
        isFinalized = true;
        finalizedAt = block.timestamp;
        fail = false;
    }
     ...
     */
    function safeFail() external onlyOwner {
     ...
     * @dev Withdraw BNB 
     */
    function withdrawBNB(uint256 weiAmount, address to) external onlyOwner {
        require(address(this).balance >= weiAmount, "insufficient BNB balance");
        (bool sent, ) = payable(to).call{value: weiAmount}("");
        require(sent, "Failed to withdraw");
    }
        ...
        if (isFinalized) {
            ...
            if (vpar.vesting) {
                if (vpar.vestingCyclePeriod > 0) {
                    cycles = (block.timestamp - finalizedAt) / vpar.vestingCyclePeriod;
