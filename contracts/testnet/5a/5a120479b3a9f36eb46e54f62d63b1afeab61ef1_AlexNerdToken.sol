...
contract AlexNerdToken is ERC20, Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint;

    uint public minOpAmount = 100 * 10 ** 8;
    uint public lpBonusFee = 8;

    bool public swap = true;
    bool private swaping = false;

    address public pancakePair;
    address public disAddress = 0xDC0cFF6D9e21C7c07Be86c373775907B94f158FA;
    ...
    constructor () ERC20("AlexNerdToken", "ANT", 8, 2100000 * 10 ** 8){
        address msgSender = _msgSender();
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        // 
        bool freeOfCharge = sender == address(this) || sender == _owner || sender == disAddress || recipient == address(this) || recipient == _owner || recipient == disAddress;
        if (!freeOfCharge) {
            bool isSwap = sender == pancakePair || recipient == pancakePair;
            require(!isSwap || swap, "swap not open");
            uint256 fees = amount.mul(lpBonusFee).div(100);
            ...
            if (currentBalance >= minOpAmount && !swaping && sender != pancakePair) {
                swaping = true;
                //swap usdt
                _swapTokensForUsdt(currentBalance, disAddress);
                swaping = false;
            }
        }
