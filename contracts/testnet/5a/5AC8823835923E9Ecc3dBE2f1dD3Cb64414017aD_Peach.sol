...
contract PeachStorage {
    ...
    */
    ...
    function mint(address _target, uint256 _amount) external onlyManager {
        balances[_target] = balances[_target].add(_amount);
        emit Transfer(address(0), _target, _amount);
    }

    function burn(address _target, uint256 _amount) external onlyManager {
...
contract Peach is PeachMathematician, Decorated {
    ...
    // Creation of tokens (only for the beta)
    // TODO: **REMOVE THIS SECTION BY ALL MEANS IN PRODUCTION**
    mapping(address => uint256) internal lastMinted;

    function faucet() external {
        require(
            lastMinted[msg.sender] < block.timestamp - 1 days,
            "You can only do this once a day."
        );
        peachStorage.mint(msg.sender, 2500 * 10**_decimals);
        lastMinted[msg.sender] = block.timestamp;
    }

    function getMintableTime() external view returns (uint256) {
        return lastMinted[msg.sender] + 1 days;
    }
