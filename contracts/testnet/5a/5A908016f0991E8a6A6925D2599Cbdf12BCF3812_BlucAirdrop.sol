...
contract BlucAirdrop is Ownable {
    ...
    constructor(address _token, uint256 _claimTimestamp) {
        require(_token != address(0), "S_BAD_100");
        token = _token;
        claimTimestamp = _claimTimestamp;
    }

    function setOwner(address _address) external onlyOwner {
        require(_address != address(0), "S_BAD_200");
    ...
    function claim() external {
        require(block.timestamp >= claimTimestamp, "S_BAD_300");
        require(isWhitelist(), "S_BAD_400");
        IERC20(token).safeTransfer(msg.sender, whitelistReward[msg.sender]);
