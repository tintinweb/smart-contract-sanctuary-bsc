...
*/
contract FCG is Initializable, ERC20Upgradeable, ERC20PausableUpgradeable, OwnableUpgradeable {
    ...
    function setMintableAddress(address managementContractAddress) public onlyOwner {
        require(managementContractAddress != address(0), 'FCG Token: SaleAddress is not defined.');
        // Storing address of the mintable contract
        mintableAddress = managementContractAddress;
        _approve(address(this), mintableAddress, type(uint256).max);
        TokenMinterUpdated(managementContractAddress);
    }

    /**
     * @dev Any call to transfer tokens should not do anything in case of the token is not on pause.
     */
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override(ERC20Upgradeable, ERC20PausableUpgradeable) whenNotPaused {
    ...
    function initTokens() private {
        ...
        uint256 totalSupply = 16511676000 * 10 ** decimals();
     ...
     */
    function distributeTokens(address receiver, uint256 amount) external onlyMinter whenNotPaused returns (bool success) {
        require(receiver != address(0), 'FCG Token: Receiver is not defined.');
        require(amount > 0, 'FCG Token: Amount is not defined.');

        require(balanceOf(address(this)) >= amount, 'FCG Token: Amount of tokens to be minted exceeds the balance.');
     ...
     */
    function setMinter(address _minter) public onlyMinter whenPaused {
        require(_minter != address(0), 'FCG Token: Minter should be defined.');
     ...
     */
    modifier onlyMinter() {
        require(msg.sender == mintableAddress, 'FCG Token: Caller is not the Minter.');
        _;
    }
}