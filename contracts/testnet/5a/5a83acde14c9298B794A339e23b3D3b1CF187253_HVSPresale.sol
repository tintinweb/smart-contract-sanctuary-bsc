...
contract HVSPresale is Context, ReentrancyGuard, Ownable {
    ...
    function getTokenPrice() public view returns (uint256) {
        if(block.timestamp < presale_info.presale_start)
            return 0;
        else if((block.timestamp >= presale_info.presale_start) && (block.timestamp < (presale_info.presale_start + 1 minutes)))
            return 3;
        else if((block.timestamp >= (presale_info.presale_start + 1 minutes)) && (block.timestamp < (presale_info.presale_start + 3 minutes)))
            return 5;
        else if((block.timestamp >= (presale_info.presale_start + 3 minutes)) && (block.timestamp < (presale_info.presale_start + 7 minutes)))
            return 8;
        else if((block.timestamp >= (presale_info.presale_start + 7 minutes)) && (block.timestamp < (presale_info.presale_start + 30 minutes)))
            return 40;
        return 0;
    }
    ...
    function userWithdrawTokens() external nonReentrant {
        ...
        if(msg.sender == owner()) {
            require(IBEP20(presale_info.sale_token).balanceOf(address(this)) > (status.sold_amount - status.token_withdraw - buyers[msg.sender].sale), "HVS Presale: Not allow to withdraw by owner");
            TransferHelper.safeTransfer(address(presale_info.sale_token), msg.sender, (IBEP20(presale_info.sale_token).balanceOf(address(this)) - (status.sold_amount - status.token_withdraw - buyers[msg.sender].sale)));
