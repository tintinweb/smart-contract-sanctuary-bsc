...
contract PresaleProxy is Owned {
    ...
    constructor(address _depolyer,address _staker) public {
        ...
        _tiers.push(Tier({
            _min: 30000 * 10 ** 18,
            _max: 99999999999999993000000000 * 10 ** 18,
            _percent: 3000
        }));
    }
    ...
    function setPoolID(uint256 _id) external onlyOwner {
        PID = _id;
    }
    ...
    function getTierPerUser(address _user) external view returns (uint256) {
        uint256 amount = IStaker(staker).getUserStaked(PID,_user);
        uint256 tier = 0;
        for(uint256 i = 0; i  < _tiers.length ; i++){
            if(_tiers[i]._min >= amount && _tiers[i]._max <= amount){
                    tier = i;
            }
        }
        return tier;
    ...
    function getisEligble(address _user) external view returns (bool) {
        uint256 amount = IStaker(staker).getUserStaked(PID,_user);
        return amount >= _tiers[0]._min;
    }

    function getTierMinLimit() external view returns (uint256) {
        return _tiers[0]._min;
    }

    function getTierPercentage(uint256 _id) external view returns (uint256) {
        return _tiers[_id]._percent;
    }

    function setTierInfo(uint256 _id,uint256 _min,uint256 _max,uint256 _percent) external onlyOwner {
        _tiers[_id]._percent = _percent;
        _tiers[_id]._min = _min;
        _tiers[_id]._max = _max;
    }
