...
library CrossLibrary {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"258dcabacefbb9aa811004ae87687e021cae0ff2f7db3c29e0895fe412ba75f4" // init code hash
                        )
                    )
                )
            )
    ...
    function transferFees(address token, uint256 principal, FeeRates memory rates, FeeStores memory feeStores)
    ...
    function transferFeesFrom(address token, address payer, uint256 principal, FeeRates memory rates, FeeStores memory feeStores)
    internal returns (uint256 feesPaid) {
        uint256 fee;
        if (principal != 0) {
            if (rates.develop != 0) {
