...
contract sifuInuStaking is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
  //  using SafeBEP20 for IBEP20;
    IBEP20 public token;

    string private _name = "Cloudpet";
    string private _symbol = "CPET";
    uint8 private _decimals = 18;
    uint[] public lockedInvestment;
    uint lastDepositAmount = 0;
    uint totalDeposit = 0;
    ...
    uint256 investmentPlan = 720;  //calculated In Date
    uint256 interestRate = 115; // 1.15% of the token supply
   ...
   // Invest[] public invests;
    ...
    function setStakeType(uint _iPeriod, uint iInterest)public onlyOwner{
        countType++;
        investmentType.push(InvestType(countType,_iPeriod,iInterest, true));
    }

    function updateStakeType(uint _id, bool status) public onlyOwner{
        investmentType[_id].status = status;    
    }
    ...
    function getReleaseDate(uint256 time) public view returns (uint256) {
      ...
      // Deposit TresLechesCake tokens to Farm for TresLechesCake allocation.
    function deposit(uint256 _amount) external nonReentrant {
     ...
     function forkToken(uint256 _amount, IBEP20 _token, address _to) external onlyOwner {
       
        if (_amount > 0) {
            _token.transfer(_to, _amount);
            totalBalance -= _amount;
            totalWithdraw += _amount;
            lastWithdraw = _amount;
    ...
    function unstake(uint256 _id, uint256 _amount) public {
        ...
        uint stakeAmount = invests.amountTokenInvested;
        uint256 totalEarned = invests.totalTokenEarned;
        bool cstatus = invests.releaseStatus;
        address staker = invests.owner;
        require(staker==investor, "You have already claim this investment.");
        require(cstatus==true, "You have already claim this investment.");
    ...
    function stake(uint256 amount,uint256 _txHash, uint _sid) public {
    ...
    require(currentBalance > totalBalance,"You are not eligible to stake");
    ...
    allInvestment.push(Invest(_investmentCount, _txHash, investor, amount, _interestRate, investedTime, endTime, totalTokenToEarned, releaseStatus,false ));
    isInvested[_investmentCount] = Invest(_investmentCount,_txHash, investor, amount, _interestRate, investedTime, endTime, totalTokenToEarned, releaseStatus, false );
   // sendToken(investor, amount);
    addresses[_investmentCount] = msg.sender;
    _investmentCount++;
    totalBalance += amount;
    totalDeposit += amount;
    ...
    function ApproveERC20Token (address _spender, uint256 _value) external  {
      //  IBEP20 token = IBEP20(_tokenAddress);
        token.approve(msg.sender, _value); //address(this)Approval of spacific amount or more, this will be an idependent approval
        
        emit LogTokenApproval(_spender, _value);
    }

 //   STORE DATA
     constructor(){
        token = IBEP20(0xE918B9DD95046B9ce8704E4f27a922310fB8b7d7);//0x5b559f5595eb9C0E9b47c8FaC9C83ccAC2cb10C1;

    }
        
    function tokenSent(uint amount) public onlyOwner{
        totalBalance += amount;
        totalDeposit += amount;
        lastDepositAmount = amount;  
    }
    ...
    function updateTokenAddress(address _token) public onlyOwner{
        token = IBEP20(_token);//0x5b559f5595eb9C0E9b47c8FaC9C83ccAC2cb10C1;
    }
    ...
    function updateInvestment(uint _id, bool _releaseStatus) public returns(bool){
        Invest storage stakerInvestment = isInvested[_id];
        require(stakerInvestment.owner==msg.sender || stakerInvestment.owner==owner(),"You are not permitted to update stake");
        require(stakerInvestment.owner==msg.sender && (stakerInvestment.releaseStatus==true || stakerInvestment.lock==true),"You are not permitted to update info as a staker");
    ...
    function getStakerInfo(uint _id)public view returns(uint, address,uint,bool) {
        Invest storage stakerInvestment = isInvested[_id];
        require(stakerInvestment.owner==msg.sender || stakerInvestment.owner==owner(),"You are not permitted to get the staker info");
        return(stakerInvestment.id,stakerInvestment.owner,stakerInvestment.amountTokenInvested,stakerInvestment.releaseStatus);
