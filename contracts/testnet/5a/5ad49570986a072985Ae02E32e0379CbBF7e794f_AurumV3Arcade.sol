...
contract AurumV3Arcade is Context, Ownable {
    ...
    * ---------------------------------------------- */
    ...
    function updatePlayerRanking(address _account, GameOutcome _outcome, int _rankScore) private {              
        ...
        if(_outcome == GameOutcome.WIN){
            ...
            if(playerRankings[_account].rankScore > _rankScore){
                playerRankings[_account].rankScore -= _rankScore;
            }else{
                playerRankings[_account].rankScore = 0;
            }
    ...
    * ---------------------------------------------- */
    ...
    function endContest(uint256 _contestId, address payable[] memory _winners,  uint256[] memory _payOutPercentage, int[] memory _rankingIncrement) external onlyOwner {
        uint256 contestIndex = getContestIndex(_contestId);
        require(_winners.length > 0 , "Winners arrays must be > 0");
        require(_payOutPercentage.length > 0 , "PayOutPercentage arrays must be > 0");
        require(_rankingIncrement.length > 0 , "RankingIncrement arrays must be > 0");
        require(_winners.length == _payOutPercentage.length && _winners.length == _rankingIncrement.length , "Arrays must be the same length");
        require(contests[contestIndex].numOfcontestants > 0, "Contest has no contestents");

        // payout winners
        for(uint i = 0; i < _winners.length; i++){            
            uint256 reward = contests[contestIndex].contestPot*_payOutPercentage[i]/feeDenomiator;
            updatePlayerRanking(_winners[i], GameOutcome.WIN, _rankingIncrement[i]);
            AURUM.transfer(_winners[i], reward);
        }
