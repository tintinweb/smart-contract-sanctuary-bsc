...
contract EBSControl is Ownable, EBSAddress, AccessControl, Pausable {
    ...
    function initialize() external onlyOwner{
        ...
        _grantRole(OPERATOR_ROLE, EBS_MARKETPLACE);
        _grantRole(OPERATOR_ROLE, EBS_PREORDER);
    }

    function addBlacklistUser(address user) external onlyRole(ADMIN_ROLE) {
        _grantRole(BLACKLIST_USER, user);
    }

    function removeBlacklistUser(address user) external onlyRole(ADMIN_ROLE) {
        _revokeRole(BLACKLIST_USER, user);
    }

    function isBlacklistUser(address user) external view returns (bool){
        if (hasRole(BLACKLIST_USER, user)) 
            return true;
        return false;
    }

    function addWhitelistUser(address user) external onlyRole(ADMIN_ROLE) {
        _grantRole(WHITELIST_USER, user);
    }

    function removeWhitelistUser(address user) external onlyRole(ADMIN_ROLE) {
        _revokeRole(WHITELIST_USER, user);
    }

    function isWhitelistUser(address user) external view returns (bool){
        if (hasRole(WHITELIST_USER, user)) 
            return true;
        return false;
    }
    ...
    function isOperator(address user) public view returns (bool){
        if (hasRole(OPERATOR_ROLE, user) || hasRole(ADMIN_ROLE, user))
            return true;
        return false;
    }

    function checkUserValidSignature(
        bytes calldata sign, 
        bytes32 message, 
        address user
    ) public view returns (bool){
        bytes32 sellerMessage = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", message));
        bytes32 sellerSignatureHash = keccak256(abi.encodePacked(sign));

        if (!_usedHash[sellerSignatureHash] && ECDSA.recover(sellerMessage, sign) == user) return true;

        return false;
    }

    function checkSignerValidSignature(
        bytes calldata sign,
        bytes32 message
    ) public view returns (bool){
        bytes32 signerMessage = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", message));
        bytes32 signerSignatureHash = keccak256(abi.encodePacked(sign));

        if (!_usedHash[signerSignatureHash] && hasRole(SIGNER_ROLE, ECDSA.recover(signerMessage, sign))) return true;
    ...
    function useSignature(
        ...
        require(isOperator(msg.sender), "EBSControl: Must have ADMIN_ROLE or OPERATOR_ROLE");
        _usedHash[hashSign] = true;
    }
