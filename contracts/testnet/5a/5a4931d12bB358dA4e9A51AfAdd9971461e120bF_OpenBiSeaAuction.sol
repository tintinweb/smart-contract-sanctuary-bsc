...
contract OpenBiSeaAuction is Ownable,IERC721Receiver,IERC1155Receiver {
    ...
    function checkTokensForClaim(
        ...
        uint256 amountTotalForCustomerUSDwei = consumersRevenueAmount[customer].mul(priceMainToUSD);
        if (amountTotalForCustomerUSDwei < 0 ether) { //10000 ether) {
            return (0,0,0,false);
        }
        ...
        if (_consumersReceivedMainTokenLatestDate[customer] > block.timestamp.sub(4 weeks)) {
            return (tokensForClaim, amountTotalForCustomerUSDwei,percentOfSales,newCustomer);// already receive reward 4 weeks ago
        }
        (tokensForClaim, percentOfSales) = _tokensToDistribute(amountTotalForCustomerUSDwei,priceMainToUSD,newCustomer);
        return (tokensForClaim, amountTotalForCustomerUSDwei,percentOfSales,newCustomer);
    }
