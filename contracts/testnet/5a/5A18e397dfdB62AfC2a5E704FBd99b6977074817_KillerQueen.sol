...
contract KillerQueen is ReentrancyGuard{
    ...
    uint private _adminFee = 200;

    address private _operator;
    address private immutable _ADMIN;
    address private _adminsContract;

    mapping(uint => Round) private _idToRound;
    mapping(address => mapping(uint => BetInfo)) private _userBet;
    mapping(address => uint) private _userRounds;
    ...
    function betBull() external payable notContract {
        ...
        _userRounds[msg.sender]++;
    }

    function betBear() external payable notContract {
        require(msg.value >= 0.01 ether);
        require((msg.value / 10000) * 10000 == msg.value, "too small");
    ...
    function getReward(uint _round) external notContract nonReentrant {
        ...
        payable(_adminsContract).transfer(_fee);
    }
    ...
    function changeAdminFee(uint _fee) external onlyAdmin {
        require(_fee <= 1000);
        _adminFee = _fee;
    }

    function changeAdminsContract(address _newContract) external onlyAdmin {
        _adminsContract = _newContract;
    }

    // GETTERS
     function getUserBets(address _user) public view returns (uint[] memory) {
        uint notClaimedRounds = _userRounds[_user];
        uint[] memory ownerRounds = new uint[](notClaimedRounds);
        uint ownedRoundIndex;
        uint whileRound;
        while (ownedRoundIndex < notClaimedRounds && whileRound < _currentEpoch) {
            Round memory round = _idToRound[whileRound];
            BetInfo memory userBet = _userBet[_user][whileRound];
            if (userBet.amount > 0 && userBet.claimed == false && userBet.position == round.winner) {
    ...
    function getCurrentAndNetRoundStatus() external view returns (Round memory, uint, Round memory, uint, Round memory, uint) {
        return ( _idToRound[_currentEpoch - 1], _currentEpoch - 1,_idToRound[_currentEpoch], _currentEpoch, _idToRound[_currentEpoch + 1], _currentEpoch + 1);
    }
    ...
    function getBetInfo(address _user) public view returns (BetInfo memory) {
        return _userBet[_user][_currentEpoch];
    }

    function getBetInfoForRound(address _user, uint _round) public view returns (BetInfo memory) {
    ...
    function _calculateAdminFee(uint _win) private view returns (uint) {
