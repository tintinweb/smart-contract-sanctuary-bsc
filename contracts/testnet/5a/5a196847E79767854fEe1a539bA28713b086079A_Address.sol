...
contract CoinLock is ReentrancyGuard {
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map coin to all its locks
  ...
  function lockToken (address _token, uint256 _amount, uint256 _unlock_date) external nonReentrant {
    ...
    uint256 nowBalance = IERC20(_token).balanceOf(address(this));

    uint256 amountLocked = nowBalance - beforeBalance;

    require(amountLocked > 0,"INSUFFICIENT");

    require(_unlock_date > block.timestamp,"time error");
    ...
    emit onDeposit(_token, msg.sender, token_lock.amount, token_lock.lockDate, token_lock.unlockDate);
  }
