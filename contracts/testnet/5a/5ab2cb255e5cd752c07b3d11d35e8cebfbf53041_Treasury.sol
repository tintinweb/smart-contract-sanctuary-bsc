...
contract Treasury is Auth {
  ...
  function setContracts(address _heroNft, address _itemNft) external onlyMainAdmin {
...
//    eatherTransporter = IEatherTransporter(_eatherTransporter);
  ...
  function takeFounding(IMarketPlace.OrderKind _kind, uint16 _classId, IMarketPlace.PaymentCurrency _paymentCurrency) external whenNotPaused {
    ...
    if (discount > 0) {
      currentPrice = currentPrice * (100000 - discount) / 100000;
    }
    _takeFund(currentPrice, _paymentCurrency);
  ...
  function updateDiscount(uint _discount) external onlyMainAdmin {
    discount = _discount;
    emit DiscountUpdated(discount);
  }
