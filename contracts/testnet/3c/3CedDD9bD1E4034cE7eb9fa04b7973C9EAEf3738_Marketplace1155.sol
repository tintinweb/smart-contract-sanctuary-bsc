...
contract Marketplace1155 is ERC1155HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event ListingBought(uint256 indexed id, address indexed buyer, uint256 tokenAmount);

    event ListingBid(uint256 indexed id, address indexed bidder, uint256 amount, uint256 timeEnd);
    ...
    event Bumped(uint256 indexed listingId, uint256 amount);

    event BumpSet(address indexed bump);

    // CONSTRUCTOR

    function initialize(address bump_, uint256 gracePeriod_) public initializer {
    ...
    function getActiveListings() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _lastListingId; i++) {
            if ( listings[i].claimed != true /*&& listings[i].timeEnd > block.timestamp*/) {
                list[index] = i;
                index++;
            }
        }
    ...
    function createListing(
        ...
        IERC1155Upgradeable(collection).safeTransferFrom(msg.sender, address(this), tokenId, tokenAmount, new bytes(0));
    ...
    function cancel(uint256 listingId) external {
        require(listings[listingId].seller == msg.sender, "No owned listing");
        ...
        listings[listingId].lastBidder = msg.sender;
        listings[listingId].claimed = true;
        IERC1155Upgradeable(listings[listingId].collection).safeTransferFrom(
            address(this),
            msg.sender,
    ...
    function bump(uint256 listingId_, uint256 amount_) external {
        ...
        listingsAdditional[listingId_].bump += amount_;
        listings[listingId_].currency.safeTransferFrom(msg.sender, bumpAccount, amount_);

        emit Bumped(listingId_, listingsAdditional[listingId_].bump);
    }
    ...
    function setBump(address account_) external onlyOwner {
        bumpAccount = account_;

        emit BumpSet(bumpAccount);
    }
    ...
    function _createListing(
        ...
        require(tokenAmount > 0, "Low token amount"); 
        ...
        listings[_lastListingId].tokenAmount = tokenAmount;
        ...
        listingsAdditional[_lastListingId].bidStep = bidStep;
    ...
    function _buy(uint256 listingId_, uint256 amount_) private {
        require(listings[listingId_].listingType == ListingType.FixedPrice, "Buys are only for fixed price");
        require(block.timestamp >= listings[listingId_].timeStart, "Sale hasn't started yet");
        require(block.timestamp < listings[listingId_].timeEnd, "Sale has finished");
        require(listings[listingId_].lastBidder == address(0), "Already purchased");
        require(listings[listingId_].tokenAmount >= amount_, "Amount to much");

        listings[listingId_].currency.safeTransferFrom(
            msg.sender,
            address(this),
            listings[listingId_].minimalBid.mul(amount_)
        );

        listings[listingId_].lastBid = listings[listingId_].minimalBid.mul(amount_);
        listings[listingId_].tokenAmount -= amount_;
        if (listings[listingId_].tokenAmount == 0) {
            listings[listingId_].lastBidder = msg.sender;
            listings[listingId_].claimed = true;
        }
        IERC1155Upgradeable(listings[listingId_].collection).safeTransferFrom(
            address(this), 
            msg.sender,
            listings[listingId_].tokenId,
            amount_,
            new bytes(0)
        );

        _distributeValue(listingId_);

        emit ListingBought(listingId_, msg.sender, amount_);
    }
    ...
    function _bid(uint256 listingId, uint256 amount) private {
        ...
        require(
            //amount >= listings[listingId].lastBid + currencies[address(listings[listingId].currency)].bidStep,
        ...
        emit ListingBid(listingId, msg.sender, amount, listings[listingId].timeEnd);
    }
