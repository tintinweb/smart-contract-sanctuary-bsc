...
contract NFTCollection is ERC721A, ERC2981, Ownable {
    ...
    constructor(
        ...
        for (uint i = 0; i < _phaseArgs.length; i++) {
            require(_phaseArgs[i].currencies.length == _phaseArgs[i].currenciesPrice.length);
            require(_phaseArgs[i].whiteListCurrencies.length == _phaseArgs[i].whiteListCurrenciesPrice.length);    
                for (uint j = 0; j < _phaseArgs[i].currencies.length; j++) {
                    phases[i].currenciesPrice[_phaseArgs[i].currencies[j]] = _phaseArgs[i].currenciesPrice[j];
                ...
                for (uint j = 0; j < _phaseArgs[i].whiteListCurrencies.length; j++) {
                    phases[i].whitelistCurrencyPrice[_phaseArgs[i].whiteListCurrencies[j]] = _phaseArgs[i].whiteListCurrenciesPrice[j];
                      if(_phaseArgs[i].whiteListCurrenciesPrice[j]==0){
                    phases[i].whiteListCurrencies[_phaseArgs[i].whiteListCurrencies[j]] = true;

                    }
                }   
            
        emit CollecctionPhase(i, _phaseArgs[i].currencies, _phaseArgs[i].currenciesPrice,_phaseArgs[i].whiteListCurrencies,_phaseArgs[i].whiteListCurrenciesPrice,_phaseArgs[i].isWhiteListed,_phaseArgs[i].WhiteListStartTime, _phaseArgs[i].WhiteListEndTime,_phaseArgs[i].maxNFTPerUser,_phaseArgs[i].maxQuantity);
        }
    }
    ...
    function setMaxNFTPerUser(uint256 _amount, uint256 _phaseId) external onlyOwner {
        phases[_phaseId].maxNFTPerUser = _amount;
    }

    function setMaxQuantity(uint256 _amount, uint256 _phaseId) external onlyOwner {
    ...
    */
    ...
    function mint(
        ...
        require(phases[_phaseId].startTime < block.timestamp && phases[_phaseId].endTime> block.timestamp,"");
        ...
        if (
            ...
            uint256 whitelistedFee = phases[_phaseId].whitelistCurrencyPrice[_currency];
            if (whitelistedFee > 0) {
            ...
            require(
                phases[_phaseId].currenciesPrice[_currency] > 0 || phases[_phaseId].currencies[_currency],
