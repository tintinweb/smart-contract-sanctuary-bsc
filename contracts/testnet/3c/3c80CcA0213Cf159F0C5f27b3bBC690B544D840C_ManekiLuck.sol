...
contract ManekiLuck is Context, IERC20, Ownable {
    ...
    uint randNonce = 0;
    string private _name = "ManekiLuck";
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            if(tokenAmountForLotteryParticipant <= amount){
                lotteryParticipants.set(recipient,amount);
            }
        }
            ...
            if(_balances[address(recipient)]< tokenAmountForLotteryParticipant)
                lotteryParticipants.remove(recipient);
            if(_balances[address(sender)]< tokenAmountForLotteryParticipant)  
