...
contract SPORTBET {
    ...
    function addGame(string memory _gameName, string memory _team1, string memory _team2, uint256 _betDeadline) external onlyAdmin {
        ...
        GamesData[(games.length - 1)].end_time = _betDeadline;

        emit gameAdded(_gameName, _team1, _team2, _betDeadline);
    }
        ...
        for(uint256 i = 0; i < GamesData[_gameId].uniqueaddress.length; i++){
            ...
            if(_winner == 0){
                if(GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].team1 > 0){
                    touser = (prize * (GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].team1 / prize)) / divider;
                    user.toclaim += touser;
                    user.gains += touser;
            ...
            if(_winner == 1){
                if(GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].team2 > 0){
                    touser = (prize * (GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].team2 / prize)) / divider;
                    user.toclaim += touser;
                    user.gains += touser;
                }
            ...
            if(_winner == 2){
                if(GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].drawgame > 0){
                    touser = (prize * (GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].drawgame / prize)) / divider;
                    user.toclaim += touser;
                    user.gains += touser;
                }
                user.losts += GamesData[_gameId].participants[GamesData[_gameId].uniqueaddress[i]].team2;
    ...
    function betOnGame(uint256 _gameId, uint256 _team, uint256 _bet) external {
        require(block.timestamp < GamesData[_gameId].end_time, "Not time to do that");
        require(_team > 2, "Only 3 ways!");
        require(_bet > min_bet, "Bet below min bet amount!");

        players_data storage user = players[msg.sender];

        if(_bet < user.balance){
            user.balance -= _bet;
        }else{
            uint256 amountFromWallet = _bet - user.balance;
            user.balance = 0;
    ...
    function claimPrize() external {
        require(players[msg.sender].toclaim > 0);
