    ...
    ReentrancyGuard
{
    ...
    function openStarterPack(uint256 boxId) public checkOpenPack(boxId) {
        ...
        for (uint256 index = 0; index < totalItemInPack; index++) {
            uint256 tokenId = FactoryNFT.mint(_msgSender());
        ...
        emit OpenPackSuccess(boxId, _msgSender(), address(FactoryNFT),tokenIds);
    }
    ...
    function setFactory (address _addressFactory) public onlyOwner {
        FactoryNFT = AETHR_NFT(_addressFactory);
    }
...
contract UserPool is AccessControlEnumerable, Ownable, ReentrancyGuard {
    ...
    {
        poolUser[user] -= total;
        BUSD.transferFrom(address(this), user, total);
