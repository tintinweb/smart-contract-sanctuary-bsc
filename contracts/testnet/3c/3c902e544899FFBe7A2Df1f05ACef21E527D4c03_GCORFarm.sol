...
contract GCORFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GCRO
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;         // How many allocation points assigned to this pool. GCRO to distribute per block.
        uint256 lastRewardBlock;    // Last block number that GCRO distribution occurs.
        uint256 accTokenPerShare;   // Accumulated GCRO per share, times 1e12. See below.
    ...
    // The block number when GCRO mining starts.
    ...
    // Update the given pool's GCRO allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending GCRO on frontend.
    ...
    // View function to see if user can harvest GCRO.
    ...
    // Deposit LP tokens to MasterChef for GCRO allocation.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough GCRO.
