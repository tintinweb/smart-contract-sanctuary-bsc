...
contract PoolFactory is Initializable, OwnableUpgradeable, IPoolFactory {
    ...
    struct PoolInfo {
        ...
        //PoolValue value;
        uint256 vesting_Percent; // vesting
        //uint256 usdPrice;
        uint256 tokenTarget; //how much that you want to raise with us
        uint256 ratio; //trad value
        address weiToken;  //busd
        uint256 minWei;   //min allocation
        uint256 maxWei;   // max allocation
  ...
  //  PoolValue[] public poolvalues;
    ...
    function registerPool(
       ...
       //string memory _meta,
        ...
        require(_tokenTarget > _maxWei, "Token target can't be less than maxWei!");
        require(_ratio  > 0, "ratio can't be zero!");
        require(_minWei > 0, "minWei can't be zero!");
        require(_maxWei > 0, "maxWei can't be garter than token target!");
...
//ssh -i ideapad.pem [email protected] 
    ...
    constructor(
       ...
       // claimable1=claimable[msg.sender];
        //vestingPeriod=claimTime+ 1000 seconds;
        vestedTerm=0;
        poolId = _poolId;
        token = _token;
    ...
    function setBaseData(
     ...
     //   string memory _meta
        ...
        //vestingPeriod=_claimTime+ 1000 seconds;
...
//   }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            users[userCounter] = _addresses[i];
            userCounter ++;
            whitelistaddresses = _addresses;
        }
    }

    function getwhiteListingCount() public view returns(uint256 count){
        return userCounter;
    }
    ...
    function checkWhitelistedAddress(address _address) public view returns (bool isIndeed)
    {
    ...
    function vestingClaim() external {
       // require(claimable[msg.sender]==0,'you  do not have enough tokens');
        ...
        require(
            block.timestamp > claimTime && claimTime != 0,
            "claiming not yet started"
        );
        require(block.timestamp >vestingPeriod[msg.sender], "vesting time not yet started");
            if(block.timestamp>=vestingPeriod[msg.sender]){
            //vestingPeriod=block.timestamp+300 seconds;
            //vestingPeriod = 10 seconds;
            vestingPeriod[msg.sender] = block.timestamp+1000 seconds;
            vestedTerm+=1;
            }
        require(vestingTerm[msg.sender] <= vestedTerm,'vesting term to claim not started yet');
            ...
            if(claimable[msg.sender]>vestingInstallment){
                ...
                require(vestingInstallment1 < token.balanceOf(address(this)) , "balance is low");
