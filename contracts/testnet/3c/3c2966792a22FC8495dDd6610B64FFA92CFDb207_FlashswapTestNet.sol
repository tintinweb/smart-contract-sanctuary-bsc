...
contract FlashswapTestNet is IPancakeCallee {
    address private immutable  WETH;
    address private immutable UNISWAP_FACTORY;
    address private UNISWAP_ROUTER = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1;
    ...
    function initFlashloan(address _tokenBorrow,address to, uint amount) external{
        ...
        uint tokenRecieved = IERC20PAN(WETH).balanceOf(address(this));
    ...
    function pancakeCall(address _sender,uint _amount0,uint _amount1, bytes calldata _data) external override {
        ...
        //Swap token
        uint ethRecieved = IERC20PAN(WETH).balanceOf(address(this));
        emit log("Token Borrowed Amount",ethRecieved);

        path[0] = _amount0 == 0 ? token1 : token0;
        path[1] = _amount0 == 0 ? token0 : token1;
        IERC20PAN(WETH).approve(UNISWAP_ROUTER, token0==WETH?_amount0:_amount1);
        emit log("Token ETH Allowance",IERC20PAN(WETH).allowance(address(this),UNISWAP_ROUTER));      

        IPancakeRouter2(UNISWAP_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(token0==WETH?_amount0:_amount1,0,path,address(this),deadline);

        //Swap to Eth
        if (path[0] == WETH) {
            TokenBorrow = path[1];
            tokenRecieved = IERC20PAN(TokenBorrow).balanceOf(address(this));
            IERC20PAN(path[1]).approve(UNISWAP_ROUTER, tokenRecieved);
        }else if (path[1] == WETH) {
            TokenBorrow = path[0];
            tokenRecieved = IERC20PAN(TokenBorrow).balanceOf(address(this));
            IERC20PAN(path[0]).approve(UNISWAP_ROUTER, tokenRecieved);
            
        }
        emit log("Token Recieved 4rmSwap Amount",tokenRecieved);

       emit log("Token Allowance",IERC20PAN(TokenBorrow).allowance(address(this),UNISWAP_ROUTER));

        path[0] = TokenBorrow;
        path[1] = WETH;
      IPancakeRouter2(UNISWAP_ROUTER).swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenRecieved, 0, path, address(this), deadline);
        ...
        IERC20PAN(WETH).transfer(pair,rePay);
    }
}