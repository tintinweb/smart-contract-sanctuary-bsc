...
contract RatelNFT is ERC721A, VRFConsumerBase, Ownable, ReentrancyGuard {
  ...
  uint256 public _teamTotal = 600;

  // max airdrop  quota allowed
  uint public maxAirdropCount = 400;

  // max presale quota allowed
  uint public maxPrivateSale = 3000;
  ...
  uint256 public mintedByOwner = 0; //团队总和
  ...
  //flag for admin mint
  bool internal adminmint = false; 

  //flag for airdrop
  bool internal airdrop = false; 
  ...
  bytes32 public airdrop_merkleRoot;
  ...
  */
  ...
  function toggleModeChange(uint flag) public onlyOwner returns (bool) {
      ...
      if (flag ==2) airdrop = !airdrop;
      if (flag ==3) adminmint = !adminmint;
      return true;
  }
   ...
   */
  function _mint(address _to, uint _level, uint _quantity, uint babyId) internal {
    ...
    mintable(_quantity)  
  {
    ...
    return _mint(_msgSender(), 0, _quantity, 0);
  ...
  //白名单免费领取空投
  function airdropRedeem(bytes32[] calldata _merkleProof)
        public
        offline
        onlyNonContract
  {
  ...
  function getMintMode() internal view returns(MintMode) {
    if (adminmint) return MintMode.ADMINMINT;
    if (airdrop) return MintMode.AIRDROP;
    ...
    online
  {
    ...
    require(babyOwner[msg.sender], "Not owner error");
