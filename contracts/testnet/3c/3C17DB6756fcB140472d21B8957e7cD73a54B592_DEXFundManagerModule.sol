...
library RoboFiAddress {
    ...
    function isCertToken(address account) internal view returns(bool) {
        return account.supportsInterface(type(IDABotCertToken).interfaceId);
    }

    function isGovernToken(address account) internal view returns(bool) {
        return account.supportsInterface(type(IDABotGovernToken).interfaceId);
    }
...
contract DEXFundManagerModule is FundManagerModule {
    ...
    function moduleInfo() external pure override returns(string memory name, string memory version, bytes32 moduleId) {
        name = "DEXFundManagerModule";
        version = "v0.1.220401";
        moduleId = IDABotFundManagerModuleID;
    }

    function _fundSource() internal view override returns(address) {
        return address(this);
    }
}
...
abstract contract FundManagerModule is DABotModule, IDABotFundManagerModuleEvent {
    ...
    function _fundSource() internal view virtual returns(address) {
        return msg.sender;
    }
    ...
    function _updatePnl(AwardingDetail calldata pnl) internal {
        ...
        if (pnl.compound > 0) {
            if (pnl.compoundMode == 0) 
                asset.safeTransferFrom(_fundSource(), address(certToken), pnl.compound);
    ...
    function _awardSingle(address certToken, IERC20 asset, address benefitciary, uint reward, uint share, uint totalShare) internal {
        ...
        if (benefitciary == BOT_CREATOR_BENEFITCIARY) {
            asset.safeTransferFrom(_fundSource(), meta.botOwner, amount);
            ...
            asset.safeTransferFrom(_fundSource(), address(this), amount);
            ...
            asset.safeTransferFrom(_fundSource(), address(vaultManager), amount);
            _awardVault(vID, 3, amount, [uint(80), 100, 120]);
        } else {
            asset.safeTransferFrom(_fundSource(), benefitciary, amount);
