...
contract StakingMain is IBEP721, IBEP721Metadata, Ownable {
    ...
    function buySmartStaker(uint256 _setNum, uint _amount) external payable {
        ...
        if (WBNB != purchaseToken) {
            //SafeERC20(purchaseToken).safeTransferFrom(purchaseToken, msg.sender, stakingSet, _amount);
        ...
        Address.functionCallWithValue(address(Hub), abi.encodeWithSignature("stake(uint256,uint256,uint256)",_setNum,_amount,tokenCount), msg.value);
