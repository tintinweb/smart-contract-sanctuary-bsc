...
contract Factory is ERC721, Ownable {
    ...
    constructor() ERC721("MyDoggies", "DOG") {}
    ...
    function setTokenMetadata(uint256 tokenId,string memory dogName, string memory race, string memory svg) internal {
        _tokenURIs[tokenId] = Attr(dogName,race,svg,genRandom());
    }

    function genRandom() internal view returns (Gen memory) {
        return Gen(uint8(random(0, 255)),uint8(random(0, 255)),uint8(random(0, 255)),uint8(random(0, 255)));
    }
        ...
        returns (string memory)
    {
        string memory json = Base64.encode(
          bytes(string(
            abi.encodePacked(
                '{"name": "', _tokenURIs[tokenId].name, '",',
                '"image_data": "', getSvg(tokenId), '",',
                '"attributes": [{"trait_type": "Race", "value": ', _tokenURIs[tokenId].race, '"}',
                ']}'    
            )))
        );
