...
import '@openzeppelin/contracts/security/Pausable.sol';
import '@openzeppelin/contracts/security/ReentrancyGuard.sol';
import {StakingVaultV6_0_0} from './StakingFactoryV6.sol';
...
contract RewardDistributorV6 is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function runway() public view returns (uint256) {
        StakingVaultV6_0_0.VaultVariables memory _variables = StakingVaultV6_0_0(
            payable(vault)
        ).getVaultVariables();
        uint256 tokensPerDay = (_variables.rewardsPerBlock * 24 * 3600) / 3; // day in seconds div by seconds per block
        return token.balanceOf(address(this)) / tokensPerDay;
    }
    ...
    function pendingRewards() public view returns (uint256) {
        ...
        StakingVaultV6_0_0.Controls memory _controls = StakingVaultV6_0_0(payable(vault))
            .getAllControls();
        uint256 lastAction = StakingVaultV6_0_0(payable(vault)).lastVaultAction();
        uint256 expectedToken = (block.timestamp - lastAction) *
            _variables.rewardsPerBlock;
        //if not enough tokens in distributor return total in distributor
        if (!_controls.dualTokenVault) {
