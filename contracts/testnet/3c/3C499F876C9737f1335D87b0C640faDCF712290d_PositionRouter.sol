    ...
    OwnableUpgradeable
{
    ...
    function initialize(ISpotFactory _factory, ISpotHouse _spotHouse, IUniswapV2Router02 _uniSwapRouterV2)
        public
        initializer
    {
...
//            amounts = uniSwapRouterV2.getAmountsOut(amountIn, path);
    ...
    {
        address quoteToken;
        require(path.length == 2, "!L");
        (, quoteToken, pairManager) = isPosiDexSupportPair(path[0], path[1]);

        if (quoteToken == path[0]) {
            // Buy
            // path[0] -> path[1] and path[0] is quote
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (pairManagerAddress != address(0)){
            ...
            if (side == SpotHouseStorage.Side.BUY) {
                // quote
                (sizeOut, openOtherSide)  = pairManager.getAmountEstimate(amountIn, true, false);
                amounts[0] = sizeOut;
            }else {
                (sizeOut, openOtherSide)  = pairManager.getAmountEstimate(amountIn, false, true);
        ...
        returns (uint[] memory amounts)
    {
        ...
        if (pairManagerAddress != address(0)){
            ...
            if (side == SpotHouseStorage.Side.BUY) {
                // quote
                (sizeOut, openOtherSide)  = pairManager.getAmountEstimate(amountOut, true, true);
                amounts[0] = sizeOut;
            }else {
                (sizeOut, openOtherSide)  = pairManager.getAmountEstimate(amountOut, false, false);
                amounts[0] = openOtherSide;
            }
