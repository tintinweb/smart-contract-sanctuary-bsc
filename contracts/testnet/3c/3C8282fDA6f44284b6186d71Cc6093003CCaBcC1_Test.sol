...
contract Test {
    ...
    function test4() external view returns (uint) {
        address[] memory tokens = new address[](2);
        tokens[0] = bnbWrapAddress;
        tokens[1] = _token;
        uint[] memory prices = pancakeContract.getAmountsOut(1000000000000000000, tokens);
        return prices[0];
    }

    function test5() external view returns (uint) {
        address[] memory tokens = new address[](2);
        tokens[0] = bnbWrapAddress;
        tokens[1] = _token;
        uint[] memory prices = pancakeContract.getAmountsOut(1000000000000000000, tokens);
