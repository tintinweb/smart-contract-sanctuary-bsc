...
contract BscJungle {
    ...
    function _deposit(address _investor, uint16 _tarif, address _upline, uint256 amount) private {
    ...
    function deposit(uint16 _tarif, address _upline, uint256 amount) external {
        require(!useNativeCoin, "Contract uses native coin");
        _deposit(msg.sender, _tarif, _upline, amount);
    }
    function depositCoin(uint16 _tarif, address _upline) external payable {
