...
contract DreamHorseController is Ownable {
    ...
    function getSellsOrSolds(uint256 types) public view returns(Sell[] memory sells) {
        ...
        for(uint256 i = 0; i < DHN.counter(); i++){
            ...
            if(types == 1 && tokenIdToHeros[tokenId].isOnsell){
                sells[counter] = tokenIdToSells[tokenId];
                counter++;
            }

            if(types == 2 && tokenIdToSells[tokenId].sold){
                sells[counter] = tokenIdToSells[tokenId];
                counter++;
            }
        }  
