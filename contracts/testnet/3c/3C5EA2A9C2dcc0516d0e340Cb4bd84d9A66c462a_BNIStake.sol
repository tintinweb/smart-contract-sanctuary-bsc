...
contract BNIStake {
    ...
    uint256[4] public Bonus = [10, 15, 20, 25];
    ...
    constructor(address payable _owner, address token1, address token2) {
        ...
        totalFee = 100;
        maxStakeableToken = 50000000000000000000000000;
        percentDivider = 1000;
        minimumStakeToken = 1000000000000000000000;
    }
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 BNIVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(BNIVAL)).div(1e9);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address BNI_WBNB = 0x80c72ceFDCdF30F3639c859D29B2ccBd677af5C7; // BNI_WBNB pancake pool address
        IERC20 BNITOKEN = IERC20(0xBA130BD0B85b0D688E27a93909919D9d97D000f4); // BNI Token address 
        uint256 WBNBSUPPLYINBNI_WBNB =(WBNBTOKEN.balanceOf(BNI_WBNB));
        uint256 BNISUPPLYINBNI_WBNB = (BNITOKEN.balanceOf(BNI_WBNB));
        uint256 BNIUSDVAL = (((WBNBSUPPLYINBNI_WBNB.mul(1e9)).div((BNISUPPLYINBNI_WBNB))).mul(BNBPRICE)).div(1e18);
        return BNIUSDVAL;
    }
