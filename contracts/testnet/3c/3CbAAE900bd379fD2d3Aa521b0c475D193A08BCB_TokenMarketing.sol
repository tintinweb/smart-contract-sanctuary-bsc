...
contract TokenMarketing is IERC20, Ownable, ReentrancyGuard {
    ...
    bool inSwap;
    bool public swapForMarketingAndCharityEnabled;
    ...
    constructor(InitialSettings memory initialSettings) {
        ...
        // number of tokens to swap and send to marketing/charity wallets
        numTokensBeforeSwap = (_totalSupply * 5) / 1000;

        swapForMarketingAndCharityEnabled = true;
    ...
    function totalFees() public view returns (uint256) {
        return _charityFee + _marketingFee;
    }
    ...
    function setCharityFee(uint256 newCharityFee) external onlyOwner {
        require(newCharityFee + _marketingFee <= 25, "Total fee is over 25%");
    ...
    function setMarketingFee(uint256 newMarketingFee) external onlyOwner {
        require(newMarketingFee + _charityFee <= 25, "Total fee is over 25%");
    ...
    function removeAllFees() external onlyOwner {
        if (_charityFee == 0 && _marketingFee == 0) return;
    ...
    function setSwapEnabled(bool _enabled) public onlyOwner {
        swapForMarketingAndCharityEnabled = _enabled;
    ...
    function _swapTokensAndDistributeETH(uint256 tokensToSwap) private lockTheSwap {
        ...
        uint256 currentTotalFees = totalFees();

        if (currentTotalFees > 0) {
            uint256 ethToMarketing = (ethSwapped / currentTotalFees) * _marketingFee;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (isOverMinBalanceBeforeSwap && !inSwap && from != uniswapV2Pair && swapForMarketingAndCharityEnabled) {
