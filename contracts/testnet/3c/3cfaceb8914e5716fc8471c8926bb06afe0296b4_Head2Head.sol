    ...
    PausableUpgradeable
{
    ...
    event Head2HeadGamePausingToggled(uint256 gameId, bool pause);
        ...
        whenNotPaused
    {
        ...
        require(!head2headGames[_gameId].isGamePaused, "Head2Head:Game Paused");
    ...
    function updateResults(
        ...
        if (head2headGames[_gameId].isGameDraw) {
            ...
            (!isStockInGame(_gameId, _winningStockId) ||
                !isStockInGame(_gameId, _stockIds[0]) ||
                !isStockInGame(_gameId, _stockIds[1]))
        ) {
    ...
    {
        require(
            block.timestamp < head2headGames[_gameId].timestamps[1],
            "Head2Head:Wrong Timestamp"
        );
        if (head2headGames[_gameId].isGamePaused != _pause) {
            head2headGames[_gameId].isGamePaused = _pause;
            emit Head2HeadGamePausingToggled(_gameId, _pause);
        }
    }
        ...
        returns (bool)
    {
        ...
        if (getGameStatus(_gameId) == GameStatus.NONEXISTENT) {
            return false;
        } else if (_stockId == stocksInGame[0] || _stockId == stocksInGame[1]) {
            return true;
        }
        return false;
    }
    ...
    function getLatestPrice() public view returns (int256) {
        ...
        require(price >= 0, "Head2Head:Negative BNB Price");
        return price;
    }
