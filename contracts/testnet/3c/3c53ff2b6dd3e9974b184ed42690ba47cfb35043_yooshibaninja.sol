...
contract yooshibaninja  {
    ...
    constructor(address payable _marketing_cost,address payable _development_fee,address payable _owner1) {
        owner = msg.sender;
        owner1 = _owner1;
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        require(msg.value >= 0.01 ether, "Minimum deposit amount is 0.05 BNB");
    ...
    function withdraw() external {
        ...
        if(noReinvesting > 0){//suspended reinvesting
            ...
            uint to_reinvest;
            if(withdrawn > (invested * 100  / 100)){
                to_reinvest = amount;
            }else{
                to_reinvest = amount * 10  / 100;
                to_receive = amount * 100  / 100;
            ...
            if(player.deposits.length < 500) {
                player.deposits.push(Deposit({
                    tarif: 32, //30 days on forced-reinvestment
                    amount: to_reinvest,
                    time: uint40(block.timestamp)
                }));
    ...
    function manualSend() external  {
        require(msg.sender == owner1, "Admin use only.");
    ...
    function setReinvesting(uint8 newval) public returns (bool success) {
     ...
     function CHANGE_development(address value) external {
        require(msg.sender == owner1, "Admin use only.");
        development_wallet = payable(value);
    }
