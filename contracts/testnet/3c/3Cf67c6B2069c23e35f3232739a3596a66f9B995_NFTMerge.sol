...
contract NFTMerge is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public MIN_DOGE = 2;
    uint256 public MAX_DOGE = 10;
    uint256 public PERCENT_ADD = 8;
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 public FEE_MERGE = 0.005 ether;
    uint256 public BOSST_RATE = 50*1e6 ether;
    uint256 public PERCENT_BOOST = 10;

    event FeeSale(uint256 indexed tokenId, address buyer, uint256 fee);
    event Merge(uint256 indexed tokenId, address merger, bool isSuccess);
    ...
    function setFeeMerge(uint256 _fee) external onlyOwner {
        FEE_MERGE = _fee;
    }
    ...
    function setPercent(uint256 _add, uint256 _boost) external onlyOwner {
        PERCENT_BOOST = _boost;
        PERCENT_ADD = _add;
    }

    function setDoge(uint256 _min, uint256 _max) external onlyOwner {
        MIN_DOGE = _min;
        MAX_DOGE = _max;
    }
     ...
     * @dev Merge NFT
     */
    function mergeNFT(uint256[] memory tokenIds, bool isBoost)
        public
        payable
        nonReentrant
        whenNotPaused
    {
        if(isBoost) {
            require(
                nftToken.allowance(_msgSender(), address(this)) >= BOSST_RATE,
                "Token allowance too low"
            );
        }
        require(tokenIds.length <= MAX_DOGE, "too much doge");
        require(tokenIds.length >= MIN_DOGE, "not enough nft");
        require(msg.value == FEE_MERGE, "not enough fee");
        nftToken.transferFrom(_msgSender(), feeWallet, BOSST_RATE);
        feeWallet.transfer(FEE_MERGE);
        randManager.randMod(_msgSender(), 100);
        uint256 rareRand = randManager.currentRandMod();
        uint256 minRare = nftCore.getNFT(tokenIds[0]).rare;
        uint256 percent = PERCENT_ADD.mul(tokenIds.length);
        if(isBoost) {
            percent = percent.add(PERCENT_BOOST);
        }
        if (rareRand <= percent) {
            for (uint256 index = 0; index < tokenIds.length; index++) {
                ...
                require(
                    keccak256(
                        abi.encodePacked(nftCore.getNFT(tokenIds[index]).class)
                    ) == keccak256(abi.encodePacked("Uka Doge")),
                    "not a doge"
                );
                require(nftCore.getNFT(tokenIds[index]).rare == minRare, "require all nft the same rare");
                require(nftCore.getNFT(tokenIds[index]).rare < 4, "can not merge, max rare");
                nft.transferFrom(_msgSender(), deadAddress, tokenIds[index]);
            }
            ...
            emit Merge(tokenId, _msgSender(), true);
        } else {
            for (uint256 index = 0; index < tokenIds.length; index++) {
                require(
                    nft.ownerOf(tokenIds[index]) == _msgSender(),
