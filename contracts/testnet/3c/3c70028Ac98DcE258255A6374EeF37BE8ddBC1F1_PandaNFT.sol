...
contract PandaNFT is ERC721A, Ownable, ReentrancyGuard {
  ...
  function presaleMint(uint256 quantity,string calldata salt, bytes calldata sig) external payable nonReentrant onlyEOA {
    if (!isPresaleActive())              revert SaleNotStarted();
    if (!isWhitelisted(msg.sender, salt, sig)) revert AccountNotWhitelisted();
    if (hasMintedPresale(msg.sender, quantity)) revert AmountExceedsWhitelistLimit();
    if (totalSupply() + 1 > MAX_SUPPLY)  revert AmountExceedsSupply();
    if (msg.value < getSalePrice())     revert IncorrectPayment();
    if (quantity > MAX_MINTS_PER_TX)     revert AmountExceedsTransactionLimit();

    if(_mintedWhitelist[msg.sender] > 0){
  ...
  //Disable this for live system.
  function setVerifier(address _newSigner) external onlyOwner {
    _verifier = _newSigner;
  }

  function hasMintedPresale(address account, uint256 quantity) public view returns (bool) {
    if(_mintedWhitelist[account] > 0){
       return _mintedWhitelist[account] + quantity > MAX_MINTS_PER_TX;
    } else {
      return quantity > MAX_MINTS_PER_TX;
    }
  }

  function isWhitelisted(address account, string calldata salt, bytes calldata sig) public view returns (bool) {
    return ECDSA.recover(keccak256(abi.encode(salt, address(this), account)).toEthSignedMessageHash(), sig) == _verifier;
  }
