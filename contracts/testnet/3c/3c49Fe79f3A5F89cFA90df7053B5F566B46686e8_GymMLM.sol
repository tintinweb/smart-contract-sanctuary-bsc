...
contract GymMLM is OwnableUpgradeable {
    ...
    * @notice User qualification
     ...
     */
    function isUniqueDirectPartner(address _userAddress, address _referrAddress) private view onlyOwner returns(bool) {
     ...
     */
    function updateDirectPartners(address _referrer, address[] calldata _directPartners) external onlyOwner {
        if (directPartners[_referrer].length != 0) {
            for(uint32 i = 0; i < _directPartners.length; i++){
                if (isUniqueDirectPartner(_referrer, _directPartners[i])) {
                        directPartners[_referrer].push(_directPartners[i]);
                }
            }
        } else {
             directPartners[_referrer] = _directPartners;
        }
    }
    ...
    function getPendingRewards(
        ...
        for (uint32 i = 0; i < _directChilds.length; i++) {
            _rewardsPendingTotal += getPendingRewards(_directChilds[i], _level, _depth + 1);
        }
...
interface IAccountant {
    ...
    function calculatePendingComissionsBasedOnBorrowedAmount(address _userAddress, uint256 _comissionsValue) external returns (uint256 amount);
}
