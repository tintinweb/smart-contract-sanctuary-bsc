        ...
        if (takeFee) {
            if (automatedMarketMakerPairs[to]) {
                ...
                _jackpotTokens = _jackpotTokens.add(
                    totalSellFeesTokens.mul(jackpotFee).div(totalFees)
                );
                uint256 burnTokens = totalSellFeesTokens.mul(burnFee).div(totalFees);
                super._burn(from, burnTokens);
                amount = amount.sub(totalSellFeesTokens.sub(burnTokens));
                super._transfer(
                    from,
                    address(this),
