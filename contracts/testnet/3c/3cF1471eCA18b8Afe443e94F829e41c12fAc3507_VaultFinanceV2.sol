...
contract DividendDistributor is IDividendDistributor {
    ...
    INovationRouter02 router =
        INovationRouter02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // PancakeRouter
...
contract VaultFinanceV2 is ERC20, Ownable {
    ...
    modifier onlyOwnerOrSentinel() {
        require(
            _msgSender() == owner() || _sentinels[_msgSender()] == true,
            "Error: Caller is not owner neither sentinel"
        );
        _;
    }
    ...
    function canTransferBeforeTrading(address account)
        public
        view
        returns (bool)
    {
        ...
        returns (bool)
    {
        require(
            spender != PCS_ROUTER,
            "Error: Account cannot be the Pancakeswap Router"
        );
    ...
    function _transfer(
        ...
        require(
            from != PCS_ROUTER && msg.sender != PCS_ROUTER && to != PCS_ROUTER,
            "Error: Account cannot be the Pancakeswap Router"
        );
        require(
            !blacklisted(from) && !blacklisted(to),
            "Error: Blacklisted sender or recipient"
        );

        if (!_canTransferBeforeTradingIsEnabled[msg.sender]) {
            require(
                tradingEnabled,
                "Error: Cannot transfer before trading is enabled"
            );
        }
