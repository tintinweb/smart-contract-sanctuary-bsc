...
contract Token is ERC20, PauseOwners {
	...
	uint256 private minBalanceToTransfer = 10;
	...
	function takeFees(
		...
		if (
			sender != pair &&
			tokenBalance >= minBalanceToTransfer * (10**decimals())
		) {
			uint256 proportion = (tokenBalance * 100) /
				(developmentFee + marketingFee + liquidityFee);
			uint256 devCut = (tokenBalance * developmentFee * proportion) /
				10000;
			uint256 marCut = (tokenBalance * marketingFee * proportion) / 10000;
			uint256 liqCut = (tokenBalance * proportion) / 10000;
			...
			if (liquidityTaxEnabled && !inSwapAndLiquify) {
				swapAndLiquify(liqCut);
			}
		}
