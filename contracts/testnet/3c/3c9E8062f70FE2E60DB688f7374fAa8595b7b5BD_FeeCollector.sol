...
/**
 * @title FeeCollector
 * @notice It receives and distribute fees to staking addresses
 */
contract FeeCollector is ReentrancyGuard, AccessControl {
    ...
    function convertCurrencyToRewardToken(
        ...
        require(address(rewardConvertor) != address(0), "Convertor not set");
        require(token != address(rewardToken), "Reward token given");

        uint256 amountToConvert = IERC20(token).balanceOf(address(this));
        require(amountToConvert != 0, "Convert: zero amount");
