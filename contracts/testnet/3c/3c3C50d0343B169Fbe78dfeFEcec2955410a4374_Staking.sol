...
contract Staking is Ownable, ERC721Holder {
    ...
    function claim(string memory _pairId) public returns (uint256) {
        ...
        require(isClaimedForInterval(_pairId, msg.sender), "Nothing to claim.");
        ...
        returns (uint256)
    {
        ...
        if (stakingPairMap[_pairId].isOneTimeReward) {
            if (
                userTotRewards[_pairId][_userAddress] >=
                stakingPairMap[_pairId].rewardAmount
            ) {
                return 0;
            }
            return stakingPairMap[_pairId].rewardAmount;
        }
