...
contract LVT is ERC20, Ownable, PaymentSplitter {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isWhitelisted[to] && to != address(this)) {          // buy
            ...
            super._transfer(from, address(this), marketingFeeAmount);
            uint256 newTokenBalance = balanceOf(address(this));
            uint256 buyMarketingFeeAmount = newTokenBalance.sub(initTokenBalance);
 
            swapAndSendToFee(marketingPool, buyMarketingFeeAmount); 
        }
