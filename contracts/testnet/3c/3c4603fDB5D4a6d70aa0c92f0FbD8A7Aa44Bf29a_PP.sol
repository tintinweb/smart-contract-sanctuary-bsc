...
contract PP is Context, Ownable,ReentrancyGuard {
    ...
    ) 
    {
        ...
        p_totalSupply = 10000000*(10**FMT.decimals());


    }

    
    function getDataPP(address _user)public view returns(uint,bool,uint,uint,PData memory) {
        return (p_totalSupply,p_state,USDT.balanceOf(_user),FMT.balanceOf(_user),pData);
    }
    ...
    function startPP(uint _price,uint _amount,uint _minBuy,uint _maxBuy,uint _rewardRate) public onlyOwner returns (bool){
        require(p_state==false,"PP already started.");
        require(p_totalSupply>=_amount,"Insufficient supply.");
        require(_amount>=_maxBuy,"_maxBuy must <=_amount");
        require(_minBuy>=(10**FMT.decimals()),"_minBuy must >= 1*(10**decimals)");
    ...
    function buy(uint256 _amount,address _inviter) public {
        ...
        uint amount = _amount;
        uint amountU = amount*pData.price/1000;
        address _tokenManager=FMT.getTokenManager();

        if(_amount>=pData.balance){
            amount = pData.balance;
            amountU = amount*pData.price/1000;
            p_state = false;
        }
        require(USDT.balanceOf(_sender) >= amountU,"Insufficient USDT balance!");
        require(USDT.allowance(_sender,address(this)) >= amountU,"Insufficient allowance!");

        TransferHelper.safeTransferFrom(address(USDT), _sender, address(_tokenManager), amountU);

        
        FMT.safeRegisterUser(_sender, _inviter);
        FMT.addBuyAmount(_sender,amount);

        uint _reward = amount/1000*pData.rewardRate;

        pData.balance -= (amount+_reward);
        p_totalSupply -= (amount+_reward);
        
        FMT.pTransfer(_sender,amount);
        FMT.pTransfer(FMT.getInviter(_sender)[0],_reward);
