...
contract PancakeERC20 {
    ...
    function transferFrom(address from, address to, uint value) external returns (bool) {
        ...
        if (allowance[from][msg.sender] != uint256(MAX_INT)) {
...
contract PancakePair is PancakeERC20, ERC1155Holder {
    using SafeMath  for uint;
    using UQ112x112 for uint224;

    uint public constant MINIMUM_LIQUIDITY = 10**3;
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)')));
    bytes4 private constant SELECTOR1 = bytes4(keccak256(bytes('safeTransferFrom(address,address,uint256,uint256,"")')));
    ...
    function _safeTransfer1155(address to, uint value) private {
        (bool success, bytes memory data) = token1.call(abi.encodeWithSelector(SELECTOR1, address(this), to, id, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'Pancake: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1, uint256 _id) external {
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        uint256 MAX_INT_112 = 2**112 - 1;
        require(balance0 <= uint112(MAX_INT_112) && balance1 <= uint112(MAX_INT_112), 'Pancake: OVERFLOW');
    ...
    function skim(address to) external lock {
        ...
        _safeTransfer1155(to, IERC1155(_token1).balanceOf(address(this),id).sub(reserve1));
    }

    // force reserves to match balances
    function sync() external lock {
        _update(IERC20(token0).balanceOf(address(this)), IERC1155(token1).balanceOf(address(this),id), reserve0, reserve1);
    }
}
...
contract PancakeFactory  {
    ...
    function createPair(address tokenA, address tokenB, uint256 id) external returns (address pair) {
        ...
        IERC1155(tokenB).balanceOf(address(this), id);
        require(token0 != address(0), 'Pancake: ZERO_ADDRESS');
        require(getPair[token0][token1][id] == address(0), 'Pancake: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(PancakePair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1, id));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IPancakePair(pair).initialize(token0, token1, id);
