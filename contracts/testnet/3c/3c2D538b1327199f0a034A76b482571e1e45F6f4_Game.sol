...
contract Game is ReentrancyGuard, Nonce {
    bytes32 public constant MONSTER_TYPEHASH =
        keccak256(
            "mintMonsterWithGame(uint monsterId, uint256 nonce, bytes memory signature)"
        );
    bytes32 public constant LAND_TYPEHASH =
        keccak256(
            "mintLandWithGame(uint landId, uint256 nonce, bytes memory signature)"
        );
    bytes32 public constant ITEM_TYPEHASH =
        keccak256(
            "mintItemWithGame(uint256[] memory tokenAndAmount, uint256 nonce, bytes memory signature)"
        );
    ...
    function mintItemWithGame(
        uint256[] memory tokenAndAmount,
        uint256 nonce,
        bytes memory signature
    )
        external
        nonReentrant
        onlySignerWithUint256Array(
            ITEM_TYPEHASH,
            tokenAndAmount,
            nonce,
            signature
    ...
    {
        require(tokenAndAmount.length == 2, "data tokenAndAmount is wrong!!");
...
contract Nonce {
    ...
    modifier onlySignerWithUint256(bytes32 _identity, uint256 _value, uint256 _nonce, bytes memory _signature) {
     ...
     modifier onlySignerWithUint256Array(bytes32 _identity, uint256[] memory _value, uint256 _nonce, bytes memory _signature) {
        bytes32 digest = keccak256(abi.encodePacked(this, _identity, msg.sender, _value, _nonce));
        require(_validate(digest, _nonce, _signature), "Oracle: No permission");
        _;
        _nonces[msg.sender][_nonce] = true;
