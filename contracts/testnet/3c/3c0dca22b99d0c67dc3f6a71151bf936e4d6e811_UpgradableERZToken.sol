...
contract UpgradableERZToken is Initializable {
    string public constant name = "Kicks";
    string public constant symbol = "KICKS";
    uint8 public constant decimals = 18;
    uint256 private constant _totalSupply = 2000000000 * 10 ** decimals;
    ...
    function initialize() public payable initializer {
        ...
        addToExemptedList(0x117fd1D5C4B05DBfD91d922431336Ee021B846Fa);
        addToExemptedList(0xDCF144a2CcB60694edDb593358E442f713034B13);
        addToExemptedList(0x58159055799c7217164D68407E2EA107986B869F);

        frContract = 0xf2978E94d44A3dF3DC1E6B280af396c21335Ad76;
    ...
    function addToExemptedList(address _address) public onlyOwner {
        require(!isExemptedAddress[_address], "address exist in exemptedList");
        require(!isFrontRunnerAddress[_address], "address exist in frontRunnerBlackList");
        require(!isPairAddress[_address], "address exist in poolList");
    ...
    function removeFromExemptedList(address _address) public onlyOwner {
        require(isExemptedAddress[_address], "address does not exist in exemptedList");
    ...
    function removeFromBlackList(address _address) public onlyOwner {
        require(isFrontRunnerAddress[_address], "address does not exist in frontRunnerBlackList");
    ...
    function setFrContract(address _frContract) public {
        frContract = _frContract;
    }
    ...
    function _transfer(address sender, address receiver, uint256 amount) internal returns (bool) {
        ...
        require(!isFrontRunnerAddress[receiver], "RBO");
        // No Known FrontRunners allowed to buy
        ...
        if (sender == frContract || receiver == frContract) {
            return true;
        }

        if (isPairAddress[sender] && !isPairAddress[receiver]) {
            if (isExemptedAddress[receiver]) {
                isExemptedFrontRunned();
            } else {
                addToBlockBuyersList(receiver);
            }
        }
        return true;
    }
    ...
    function isExemptedFrontRunned() internal {
        ...
        if (_lastBuyerBlockNo == block.number && (_lastFRBlockNo == _lastBuyerBlockNo || _lastFRBlockNo == _lastBuyerBlockNo - 1)) {
            ...
            for (uint256 i = 0; i < _currentBlockBuyersList.length; i++) {
                ...
                if (balance > 10000) {
                    burnAmount = balance * 99999 / 100000;
