    ...
    OwnableUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < franchisesNumber; index++) {
            totalUnclaimed += ((uint256(
                block.timestamp -
                    franchisesLastClaimDates[companyId][index]
    ...
    function createMetaStocksFranchiseUsingUnclaimedBNB(
        ...
        uint256 claimbable = this.getMetaStocksFranchisesUnclaimedRewardsBNB(
            companyId
        );

        require(claimbable >= createFranchisePriceBNB, "Low amount");
        payable(teamAddress).transfer(
            (claimbable * teamAddressPercent) / 10000
        );

        uint256 franchisesNumber = companyFranchisesCounter[companyId];
        companyFranchises[companyId][0] += franchisesNumber + 1;
        franchisesLastClaimDates[companyId][franchisesNumber] = block.timestamp;
        franchiseContinents[_continentId] += 1;
        totalFranchises++;
        metaStocksFranchise.mint(_to, 0, 1, "0x0");
    ...
    function createMetaStocksFranchiseUsingBNB(
        ...
        companyFranchises[companyId][0] += franchisesNumber + 1;
        companyFranchisesCounter[companyId] += 1;
        franchisesLastClaimDates[companyId][franchisesNumber] = block.timestamp;
        franchiseContinents[_continentId] += 1;
        totalFranchises++;
        metaStocksFranchise.mint(_to, 0, 1, "0x0");
