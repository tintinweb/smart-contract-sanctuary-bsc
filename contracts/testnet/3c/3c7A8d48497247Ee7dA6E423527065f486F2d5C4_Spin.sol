...
contract Spin is Ownable {
    ...
    mapping(address => uint256) private spinTicket;
    mapping(address => uint256) private adminlist;
    mapping(address => uint256) private blacklist;
    mapping(uint256 => uint256) private countRarity;
    mapping(uint256 => uint256) private maxRarity;
    mapping(address => uint256) private userLastSpin;
    mapping(address => uint256) private userFirstSpinTime;
    mapping(address => uint256) private userSpinnedTimePerDay;
    ...
    uint256 randNonce = 0;
    uint256 status = 1;              
    uint256 private limitTime = 82800;
    uint256 private ticketPrice;

    uint256 private maxTicketCanBeBought = 5;
    uint256 private maxTicketPerDay = 10;
    ...
    uint256 private awardBUSD;
    uint256 private awardToken;
    ...
    constructor(
        ...
        awardBUSD = _awardBUSD * 10**18;
        awardToken = _awardToken * 10**18;
 ...
 */
    ...
    modifier limitSpinPerDay(address account) {
        require(
            userSpinnedTimePerDay[account] < maxTicketPerDay,
            "Not time to claim"
        );
        _;
    }
    ...
    modifier onlySpinTicket(address wallet) {
        require(spinTicket[wallet] > 0, "Only Lucky Wallet");
        _;
    }
...
================================================================
*/
    function rewardBUSD(address recipient) internal {
        IERC20 busd = IERC20(BUSDAddress);
        MATK.transfer(recipient, awardBUSD);
    }

    function rewardToken(address recipient) internal {
        MATK = MAoE(tokenAddress);
        MATK.transfer(recipient, awardToken);
    }

    function rewardNFT(address recipient) internal returns (uint256) {
...
*/
    ...
    function buyTicket(uint256 numofTicket)
        public
        isNotInBlackList(msg.sender)
        isNotAddressZero(msg.sender)
    {
        ...
        MATK = MAoE(tokenAddress);
        uint256 amount = ticketPrice * numofTicket;
        MATK.transferFrom(recipient, addressReceiver, amount);
        spinTicket[recipient]+=numofTicket;
    }

    function rollSpin() public {
        address recipient = msg.sender;
        checkSpin(recipient);
        spin(recipient);
        userSpinnedTimePerDay[recipient] ++;
    }
    function checkSpin(address recipient) internal {
        if (userFirstSpinTime[recipient] + limitTime < block.timestamp){
            userFirstSpinTime[recipient] = block.timestamp;
            userSpinnedTimePerDay[recipient] = 0;
        }
    }

    function spin(address recipient)
        internal
        limitSpinPerDay(recipient)
        isNotInBlackList(recipient)
        eventGoingOn
        onlySpinTicket(recipient)
        isNotAddressZero(recipient)
        returns (uint256)
    {
        spinTicket[recipient]--;
        uint256 ran1 = random(6)+1;
        uint256 ran2 = random(6)+1;
        if (ran1 != ran2) {
            return 4;
        }
        userLastSpin[recipient] = ran1;
        if (ran1 == 1) {
            rewardBUSD(recipient);
            return ran1;
        } else if (ran1 == 2) {
            rewardNFT(recipient);
            return ran1;
        } else if (ran1 == 3) {
            rewardToken(recipient);
            return ran1;
        } else {
            return ran1;
        }
    ...
    /* 
================================================================
                ADD TO/REMOVE FROM LIST SPIN WALLET
================================================================
 */
    function addToSpinWallet(address wallet, uint256 numNFT) public onlyAdmin {
        spinTicket[wallet] = numNFT;
        emit SpinWalletAdded(msg.sender, wallet, numNFT);
    }

    function removeFromSpinWallet(address wallet) public onlyAdmin {
        spinTicket[wallet] = 0;
        emit SpinWalletRemoved(msg.sender, wallet);
    }
 ...
 */
    ...
    function changeTicketPrice(uint256 _ticketPrice) public onlyAdmin {
        ticketPrice = _ticketPrice * 10**18;
    }
    function changeAwardBUSD(uint256 _awardBUSD) public onlyAdmin {
        awardBUSD = _awardBUSD * 10**18;
    }
    function changeAwardToken(uint256 _awardToken) public onlyAdmin {
        awardToken = _awardToken * 10**18;
    }

    function changeLimitTime(uint256 _limit) public onlyAdmin {
        limitTime = _limit;
    }

    function changeMaxTicket(uint256 _maxTicket) public onlyAdmin {
        maxTicketPerDay = _maxTicket;
    }

    function changeMaxTicketCanBeBought(uint256 _maxTicket) public onlyAdmin {
        maxTicketCanBeBought = _maxTicket;
    }
...
================================================================
 */
    function getSpinTicket (address user) public view returns (uint256) {
        return spinTicket[user];
    }

    function getUserLastSpin (address user) public view returns (uint256) {
        return userLastSpin[user];
    }
    /*
================================================================
                            EVENT
 ================================================================
 */
    event SpinWalletAdded(address user, address wallet, uint256 numNFT);
    event SpinWalletRemoved(address user, address wallet);
    event EventStarted(address user, uint256 status);
    event EventStopped(address user, uint256 status);
    event WithDraw(uint256 amount);
}