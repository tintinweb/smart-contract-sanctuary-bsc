...
contract TokenMultidrop is Ownable {
    ...
    uint256 public freeTrialLimit = 0;
    uint256 public minimumTokens = 0;
    uint256 public minimumAmt = 0;
    ...
    function setMinimumIbulls(uint256 _amt) public onlyOwner returns(bool){
        minimumAmt = _amt;
        return true;
    }

    function setMinimumTokens(uint256 _minimum) public onlyOwner returns(bool success) {
        require(_minimum >= 0, "Tokens should be greater than 0");
        minimumTokens = _minimum;
        return true;
    }
    ...
    function becomeMember(uint256 _day) public payable returns(bool success) {
        ...
        if(token.balanceOf(msg.sender) >= minimumAmt){
            _fee -= _fee * tokenHoldersDiscountPer / 100;
        }
        if(token.balanceOf(msg.sender) < minimumTokens){
            require(msg.value >= dropUnitPrice + _fee, "Not enough funds");
            payable(owner()).transfer(dropUnitPrice);
        }
