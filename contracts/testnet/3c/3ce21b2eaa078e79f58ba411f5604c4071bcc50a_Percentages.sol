...
contract Percentages {
    ...
    uint public feedingLifespan = 2 days;
    ...
    function getTimeOfDeathDiffWithNowAndLifespan2() external view returns (uint){
        return (timeOfDeath - block.timestamp) * (feedingLifespan) / 10000;
    }
    ...
    function getOverallHealthByID() external view returns (uint){
        if(timeOfDeath < block.timestamp) return 0;
            ...
            uint256 healthValue = (timeOfDeath - block.timestamp) * (feedingLifespan) / 10000;
            return healthValue;
        }
    }
}