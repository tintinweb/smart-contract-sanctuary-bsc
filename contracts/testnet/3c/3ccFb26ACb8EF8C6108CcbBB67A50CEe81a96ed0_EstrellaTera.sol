...
contract EstrellaTera is Ownable, Pausable, ReentrancyGuard{
    ...
    mapping(uint256 => bool) private checkMinting;
    ...
    mapping(address => uint256 ) public totalETAReceived;
    ...
    constructor()
    {
        USDTToken = IERC20(0x8cFC55D638339BD3B6bCa7fe49004BBf0b9B1235);
        USDACEToken = IERC20(0xc7e2fE9814714612499c5C7455995a98Df2B1E24);
        ETAToken = IERC20(0x106420571B5A57bE49a79a0bAccE7854bE4376fe);
    ...
    whenNotPaused
    {
        ...
        if(roundbool == false)
        {
            ...
            { 
                ...
                TotalTokenInRound[round] = TotalTokenInRound[round].add(rounndDetails[1]);
                ...
                if(rounndDetails[2] > rounndDetails[0])
                {
                    ...
                    TotalTokenInRound[round] = TotalTokenInRound[round].add(rounndDetails[3]);
                ...
                if(IERC20(tokenAddress) == USDTToken)
                {
                    ...
                    totalETAReceived[msg.sender] += tokentransfer;
                    uint256 OwnePercentage = levelPercentage_[0].add(levelPercentage_[1]).add(levelPercentage_[4]);
                    USDTToken.transferFrom(msg.sender, owner(), OwnePercentage);
                    uint256 contractPercentage = levelPercentage_[2].add(levelPercentage_[3]);
                    USDTToken.transferFrom(msg.sender, address(this), contractPercentage);
            ...
            if(rounndDetails[2] > 1)
            {
                ...
                while(remainingbuyerToken > 0)
                {
                    ...
                    {
                    totalTokenMint = totalTokenMint.add(remainingbuyerToken);
                    // TotalTokenInRound[round] = TotalTokenInRound[round].add(remainingbuyerToken);
                    ...
                    else
                    {
                        ...
                        totalTokenMint = totalTokenMint.add(TokenBuy_User[0]);
                        // TotalTokenInRound[round] = TotalTokenInRound[round].add(TokenBuy_User[0]);
                        buyerSellTotalToken[SellerAddress][round][PreviousRound][seller_Count[PreviousRound]] = 
    ...
    private{
        ...
        if(_totalamount > 0)
        {
            ...
            if(IERC20(_tokenAddress) == USDTToken)
            {
                ...
                USDTToken.transferFrom(msg.sender, owner(), levelPercentage_[1]);
                buyerReferralCommission(msg.sender,levelPercentage_[2],levelPercentage_[3]);
            ...
            }else if(IERC20(_tokenAddress) == USDACEToken)
            {
                ...
                USDTToken.transfer(owner(), levelPercentage_[1]);
                buyerReferralCommission(msg.sender,levelPercentage_[2],levelPercentage_[3]);
                sellerReferralCommission(_SellerAddress,levelPercentage_[4]);
            }    
        }
    ...
    returns(uint256)
    {
        uint256 TotalToken = (TotalTokenInRound[PreviousRound].mul(Selling_Percents[round-PreviousRound])).div(baseDivider);
        return TotalToken;
    }
    ...
    function checkSellerOrder() public view returns(uint256 _price, uint256 _TokensellerRemaining){
            address SellerAddress = buyer_address[PreviousRound][seller_Count[PreviousRound]];
            uint256 sellerTokenPrice = buyerToken_Price[SellerAddress][PreviousRound][seller_Count[PreviousRound]];
