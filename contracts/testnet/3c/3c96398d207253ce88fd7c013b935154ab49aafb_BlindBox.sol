...
contract  BlindBox is  Ownable, ReentrancyGuard  {
    ...
    event BuyBox(address indexed buyer, uint256 indexed materialId, uint256 receivedAmount);
    event BuyMultiBox(address indexed buyer, uint256 indexed materialId, uint256 totalMaterialAmount, uint256[] receivedAmountArray );

    constructor() public {
        acceptCurrency = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        materialsAddress = IMaterials(0xeDEe132Fcf90FE3157C389706b30c6A1c7A6C88D);
        ...
        _materialIdToPrice[1] = 5 * 1e18;
        _materialIdToPrice[2] = 8 * 1e18;
        _materialIdToPrice[3] = 70 * 1e18;
        _materialIdToPrice[4] = 45 * 1e18;
        _materialIdToPrice[5] = 50 * 1e18;
    }

    function getMaterialIdInfo(uint256 materialId) public view returns (uint256[] memory){
        require(_materialIdAvailable[materialId], "this material id is not available");
        uint256[] memory infoList = new uint256[](3);
        infoList[0]=(_materialIdToPrice[materialId]);
        infoList[1]=(_materialIdBaseAmount[materialId]);
        infoList[2]=(_materialIdRandomRange[materialId]);
        return infoList;
    }
    ...
    function buyMultipleBox(uint256 materialId, uint256 times) public nonReentrant{
        require(_materialIdAvailable[materialId], "this material id is not available");
        require(times>0, "invalid amount");
        ...
        uint256[] memory boxList =  new uint256[](times);
        uint256 totalMaterialAmount = 0;
        for (uint i =0;i < times; i++) {
            uint256 amount = randNumber(materialId);
            boxList[i] = amount;
            totalMaterialAmount += amount;
        }
        acceptCurrency.transferFrom(_msgSender(), address(this), totalPrice);
        materialsAddress.mint(_msgSender(), materialId, totalMaterialAmount, '');
        emit BuyMultiBox(_msgSender(), materialId, totalMaterialAmount, boxList);
    }
