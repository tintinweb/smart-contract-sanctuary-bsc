...
contract DCBTokenclaim is Ownable {
  ...
  //a b c b d
  bytes32 internal constant ANSWER_HASH =
    0x0580bf12a650ba98bda44aa23f3892360ee6f14eabfe07151f9cd7c5476cb70c;
  ...
  uint8[] private weights = [0, 0, 0, 12, 24, 48];
  ...
  constructor(
    ...
    claimInfo.createDate = block.timestamp;
    claimInfo.startDate = _startDate;
    claimInfo.endDate = _endDate;
    claimInfo.token = IERC20(_token);
  }
  ...
  function registerForAllocation(bytes memory _sig) external returns (bool) {
    ...
    require(_tier >= 3, "Minimum Gold tier required");
  ...
  function transferToken(uint256 _amount, address _to) external onlyOwner returns (bool) {
    /** check if contract have enough funds  */
    require(claimInfo.token.balanceOf(address(this)) >= _amount, "Not enough funds in contract");
    claimInfo.token.transfer(_to, _amount);
