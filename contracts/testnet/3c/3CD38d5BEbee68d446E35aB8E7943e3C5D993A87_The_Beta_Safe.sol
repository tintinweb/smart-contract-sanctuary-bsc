...
function listMyRDID() public view returns (ExtendedStructRD [] memory){
     ...
     ExtendedStructRD[] memory items1 = new ExtendedStructRD[](activeTradeCount);
      for (uint i=0; i< RDcountItem; i++){
             if(RDMap2[msg.sender][i+1].useraddress == msg.sender){
                uint currentId = RDMap[msg.sender][i+1].id;
                ExtendedStructRD storage currentItem = RDMap2[msg.sender][currentId];
                items1[current] = currentItem;
                current +=1;
             }
        }
