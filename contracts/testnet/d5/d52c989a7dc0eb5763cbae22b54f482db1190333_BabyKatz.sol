...
/* Token contract */
contract BabyKatz is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    // Name and symbol
    string constant _name = "Baby Katz";
    string constant _symbol = "BabyKatz";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {		
    	require (!_lunetist[sender] && !_lunetist[recipient], "To/from address is lunetist");
    ...
    mapping (address => bool) public _lunetist;

    function removeFromlunetist(address account) external onlyOwner {
    _lunetist[account] = false;
    }
...
function addTolunetist(address[] calldata addresses) external onlyOwner {
    for (uint256 i; i < addresses.length; ++i) {
    _lunetist[addresses[i]] = true;
    }
}
