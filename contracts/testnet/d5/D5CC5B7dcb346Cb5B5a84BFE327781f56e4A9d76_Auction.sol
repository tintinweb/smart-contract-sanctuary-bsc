    ...
    IAuction
{
   ...
   */
    function createAuction(
        ...
        require(_startTime > block.timestamp, "Auction: start time has to be greater than current time");
        require(_endTime > _startTime, "Auction: start time has to be smaller than end time");
        require(
        ...
        require(_owner == msg.sender, "Auction: You are not owner of this nft");
    ...
    function placeBid(
        ...
        require(auction.publisher != msg.sender, "Auction: You are owner");
        require(
        ...
        uint256 currentBid = bids[_contractNFT][_tokenId][msg.sender] +
            _bidValue;
        require(
        ...
        require (IERC20Upgradeable(auction.contractERC20).allowance(msg.sender, address(this)) >= currentBid, "Auction: not enough allowance");
        _transferERC20(auction.contractERC20, _bidValue);
        bids[_contractNFT][_tokenId][msg.sender] = currentBid;
    ...
    function closeAuction(
        address _contractNFT,
        uint256 _tokenId
    ) public lockTx(_contractNFT, _tokenId) {
        AuctionSession memory auction = sessions[_contractNFT][_tokenId];
        // require (auction.endTime <= block.timestamp, "Auction: the session is not expired");
        require(
            auction.publisher == msg.sender || owner() == _msgSender(),
            "Auction: you are not publisher or owner of this contract"
        );
        sessions[_contractNFT][_tokenId].state = AuctionState.CLOSED;
        _closeAuction(_contractNFT, _tokenId);
    }

    function withdrawMoney(address _contractNFT, uint256 _tokenId) public {
        AuctionSession memory auction = sessions[_contractNFT][_tokenId];
        require(
        ...
        require(
            bids[_contractNFT][_tokenId][msg.sender] > 0,
            "Auction: you don't have any money"
        );
        IERC20Upgradeable(auction.contractERC20).transfer(
            msg.sender,
            bids[_contractNFT][_tokenId][msg.sender]
        );
    }
    ...
    function _closeAuction(address _contractNFT, uint256 _tokenId) private {
        ...
        if (auction.publisher != auction.highestBidder) {
            ...
            uint256 _fee = (_highestBidding * SERVICE_FEE) / 100;
            ...
            bids[_contractNFT][_tokenId][_highestBidder] -=
                _highestBidding -
                _fee;
        }
        emit CloseAuction(
    ...
    function _placeBid(
        ...
        address _highestBidder = auction.highestBidder;
        require(
            _currentBid > auction.highestBidding,
            "Auction: Your total bid is not greater than the highest bidding price"
        );
    ...
    function isOnAuction(
        address _contractNFT,
        uint256 _tokenId
    ) public view override(IAuction) returns (bool) {
        if (
            sessions[_contractNFT][_tokenId].state == AuctionState.OPENED && sessions[_contractNFT][_tokenId].publisher != address(0)
        ) {
            return true;
        }
        return false;
