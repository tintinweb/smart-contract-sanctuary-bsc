...
contract DWFarmGoods {
    ...
    modifier onlyOracle() {
        require(oracles[msg.sender], "You are not an oracle.");
        _;
    }
    ...
    function unsetOracle(address _oracle) external onlySupport {
    ...
    function burn(
        ...
        require(balances[_target][_materialIndex] >= _amount, "Not enough balance for this material.");
...
contract Tractor is ERC721 {
    ...
    */
    ...
    mapping(uint256 => uint256) private tractorToSaleIndex;
    ...
    modifier onlyOracle() {
        require(oracles[msg.sender], "You are not the oracle.");
        _;
    }
    ...
    function multimint(
        address _newOwner,
        uint256[] memory _durabilities,
        uint8[] memory _levels
    ) external onlyOracle {
        uint64 _builtAt = uint64(block.timestamp);
        for (uint256 i = 0; i < _levels.length; i++) {
            uint64 _goodThrough = uint64(
                block.timestamp + _durabilities[i] * 1 days
            );
    ...
    function transferFrom(
        ...
        if (msg.sender == game) {
            ...
            require(
                tractors[_tractorId].goodThrough <= block.timestamp,
                "This tractor is still eorking. You need to wait for it to expire."
            );
        }
    ...
    function sell(uint256 _tractorId, uint256 _sellPrice) external {
        ...
        require(
            !tractors[_tractorId].forSale,
            "This tractor is already for sale."
        );
    ...
    function unsell(uint256 _tractorId) external {
        require(
            msg.sender == tractorToOwner[_tractorId],
            "You are not the owner of such tractor."
        );
    ...
    function burn(uint256 _tractorId) external {
        require(
            msg.sender == tractorToOwner[_tractorId] || msg.sender == game,
    ...
    function list(uint256 _offset) external view returns (uint256[13] memory) {
        ...
        uint256 _quantity = 0;
        uint256 i;
        for (i = _offset; i < forSale.length && _quantity < 12; i++) {
            if (
    ...
    function setUsed(uint256 _tractorId) external {
...
contract FarmerWaifu is ERC721 {
  ...
  */
    ...
    mapping(uint256 => uint256) private farmerToSaleIndex;
    ...
    function multimint(
        address _newOwner,
        uint256[] memory _dnas,
        uint256[] memory _durabilities,
        uint8[][] memory _waifuPowers
    ) external onlyOracle {
        uint64 _birthday = uint64(block.timestamp);
        for (uint256 i = 0; i < _dnas.length; i++) {
            uint64 _alive = uint64(block.timestamp + _durabilities[i] * 1 days);
    ...
    function transferFrom(
        ...
        if (msg.sender == game) {
            ...
            require(
                farmers[_farmerId].aliveUntil <= block.timestamp,
    ...
    function sell(uint256 _farmerId, uint256 _sellPrice) external {
        ...
        require(
            !farmers[_farmerId].forSale,
            "This farmer is already for sale."
        );
    ...
    function unsell(uint256 _farmerId) external {
        require(
            msg.sender == farmerToOwner[_farmerId],
            "You are not the owner of such farmer."
        );
    ...
    function burn(uint256 _farmerId) external {
        require(
            msg.sender == farmerToOwner[_farmerId] || msg.sender == game,
    ...
    function getWP(uint256 _farmerId) external view returns (uint8[] memory) {
    ...
    function setUsed(uint256 _farmerId) external {
    ...
    function extendLifespan(uint256 _farmerId, uint256 _hours)
        external
        onlyGame
    {
        farmers[_farmerId].aliveUntil = uint64(
            farmers[_farmerId].aliveUntil + _hours * 1 hours
        );
    }
...
contract PeachStorage {
    ...
    constructor() {
        ...
        balances[0x197b53e2ea9500ecFC0D6E34F061174eB1D9a9bc] = _totalSupply / 3;
        TGE = block.timestamp;
    }
    ...
    */
    ...
    function transferFrom(
        ...
        require(_to != address(0), "ERC20: transfer to the zero address");
        require(
            allowances[_from][_spender] >= _amount || _spender == _from,
            "Allowance is lower tan requested funds"
        );
        if (_spender != _from)
            allowances[_from][_spender] = allowances[_from][_spender].sub(
                _amount
            );
        _transfer(_from, _to, _amount, _commission);
        emit Transfer(_from, _to, _amount.sub(_commission));
...
contract ProxiedStorage is PeachMathematician {
    ...
    function _getCommission(
        ...
        if (
            swaps[_from] || _from == game || _to == game || msg.sender == game
        ) {
...
contract DWGame {
    ...
    address private farmGoodsAddress = address(0);
    address private owner = msg.sender;
    address private support;
    ...
    DWFarmGoods private farmgoods = DWFarmGoods(farmGoodsAddress);
    mapping(uint256 => uint256) mintPrices;
    mapping(uint256 => uint256) harvestPrices;
    mapping(uint256 => uint256) fieldWP;
    mapping(uint256 => uint256) fieldMaterialIndex;
    uint256 private healPrice = 5;
    uint256 private repairPrice = 10;
    uint256 private licensePrice = 2;
    uint256 private decimalMultiplier = 10**21;
    uint256 private mintUnitGas = 3 * 10**15 wei;
    uint256 private harvestGas = 4 * 10**15 wei;
    uint256 private marketplaceGas = 2 * 10**15 wei;
    uint256 private healGas = 2 * 10**15 wei;
    uint256 private repairGas = 2 * 10**15 wei;
    uint256 private licenseGas = 2 * 10**15 wei;

    uint256 private rewardComission = 5;
    bool private inCrisis;

    event MintRequest(address _wallet, uint256 _quantity, uint256 _type);
    event HarvestRequest(
        address _wallet,
        uint256 _field,
        uint256[] _tractorIds,
        uint256[] _farmerIds
    ...
    function setFarmGoodsAddress(address _farmGoodsAddress)
        external
        onlySupport
    {
        farmgoods = DWFarmGoods(_farmGoodsAddress);
    }
    ...
    function mint(uint256 _quantity, uint256 _type) external payable {
        ...
        peach.transferFrom(
            ...
            refillPool,
            mintPrices[_type]
                .mul(decimalMultiplier.div(peach.getCurrentPrice()))
                .mul(_quantity)
        );
    ...
    function setMaterialForField(uint256 _field, uint256 _materialIndex)
        external
        onlySupport
    {
        fieldMaterialIndex[_field] = _materialIndex;
    }

    function goHarvest(
        uint256 _field,
        uint256[] memory _tractorIds,
        ...
        (bool sent, ) = rewardPool.call{value: _rewardGas}("");
        require(
            sent,
            "Failed to send BNB to Reward Pool, please contact the administrator."
        );
        (sent, ) = bella.call{value: _harvestGas}("");
        require(
            sent,
            "Failed to send BNB to Bella Peach, please contact the administrator."
        );
        // TODO: incorporate license logic here
        // Verify the tractors have enough space
        uint256 spacing = 0;
        for (uint256 i = 0; i < _tractorIds.length; i++) {
            if (
                tractor.ownerOf(_tractorIds[i]) == msg.sender &&
                tractor.getAvailability(_tractorIds[i]) &&
                tractor.getTTL(_tractorIds[i]) >= uint64(block.timestamp) &&
                tractor.getBrokenUntil(_tractorIds[i]) <=
                uint64(block.timestamp)
            ) {
                spacing += tractor.getSpace(_tractorIds[i]);
                tractor.setUsed(_tractorIds[i]);
        ...
        emit HarvestRequest(msg.sender, _field, _tractorIds, _farmerIds);
    }
    ...
    function buyFarmer(uint256 _farmerId) external payable {
        ...
        (bool sent, ) = rewardPool.call{value: msg.value}("");
        require(
            sent,
            "Failed to send BNB to Reward Pool, please contact the administrator."
        );
    ...
    function sellHarvestToOracle(uint256[] memory _amounts) external {
        uint256 total = 0;
        for (uint256 i = 0; i < _amounts.length; i++) {
            total += _amounts[i] * farmgoods.getPrice(i);
            if (_amounts[1] > 0) {
                farmgoods.burn(msg.sender, i, _amounts[i]);
            }
        }
        peach.transfer(msg.sender, (total * 1000) / peach.getCurrentPrice());
    }
    ...
    function claim(uint256[] memory _claimedAmounts) external {
        uint256 total = 0;
        for (uint256 i = 0; i < _claimedAmounts.length; i++) {
            uint256 value = _claimedAmounts[i].mul(farmgoods.getPrice(i)).div(
                peach.getCurrentPrice()
            );
            if (value > 0) {
                total = total.add(value);
                farmgoods.burn(msg.sender, i, _claimedAmounts[i]);
            }
        }
        total = total.mul(10**18);
        peach.transferFrom(refillPool, msg.sender, total);
    }

    function heal(uint256 _farmerId) external payable {
        require(
            msg.value >= healGas,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = refillPool.call{value: msg.value}("");
        require(
            sent,
            "Failed to send BNB to Refill Pool, please contact the administrator."
        );

        peach.transferFrom(
            msg.sender,
            refillPool,
            healPrice.mul(decimalMultiplier.div(peach.getCurrentPrice()))
        );
        farmer.heal(_farmerId, 12);
    }

    function multiHeal(uint256[] memory _farmerIds) external payable {
        require(
            msg.value >= healGas * _farmerIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = refillPool.call{value: msg.value}("");
        require(
        ...
        peach.transferFrom(
            msg.sender,
            refillPool,
            healPrice.mul(_farmerIds.length).mul(
                decimalMultiplier.div(peach.getCurrentPrice())
            )
        );

        for (uint256 i = 0; i < _farmerIds.length; i++) {
            farmer.heal(_farmerIds[i], 12);
        }
    }

    function repair(uint256 _tractorId) external payable {
        require(
            msg.value >= repairGas,
            "Not enough BNB sent for the heal transaction."
        );
        ...
        peach.transferFrom(
            msg.sender,
            refillPool,
            repairPrice.mul(decimalMultiplier.div(peach.getCurrentPrice()))
        );
        tractor.repair(_tractorId, 12);
    }

    function multiRepair(uint256[] memory _tractorIds) external payable {
        require(
            msg.value >= repairGas * _tractorIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = refillPool.call{value: msg.value}("");
        require(
        ...
        peach.transferFrom(
            msg.sender,
            refillPool,
            repairPrice.mul(_tractorIds.length).mul(
                decimalMultiplier.div(peach.getCurrentPrice())
            )
        );

        for (uint256 i = 0; i < _tractorIds.length; i++) {
            tractor.repair(_tractorIds[i], 12);
        }
    }

    function addLicense(uint256 _farmerId, uint256 _days) external payable {
        require(
            msg.value >= licenseGas,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = refillPool.call{value: msg.value}("");
        require(
        ...
        peach.transferFrom(
            msg.sender,
            refillPool,
            licensePrice.mul(decimalMultiplier.div(peach.getCurrentPrice()))
        );
        farmer.setLicensed(_farmerId, _days);
    }

    function addLicenses(uint256[] memory _farmerIds, uint256 _days)
        external
        payable
    {
        require(
            msg.value >= licenseGas * _farmerIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = refillPool.call{value: msg.value}("");
        require(
        ...
        peach.transferFrom(
            msg.sender,
            refillPool,
            licensePrice.mul(_farmerIds.length).mul(
                decimalMultiplier.div(peach.getCurrentPrice())
            )
        );
        for (uint256 i = 0; i < _farmerIds.length; i++) {
            farmer.setLicensed(_farmerIds[i], _days);
        }
    }
}