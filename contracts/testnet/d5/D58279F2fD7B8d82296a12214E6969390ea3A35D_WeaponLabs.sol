...
contract WeaponLabs is Ownable, ECIOHelper {
    ...
    //Forge Fragment
    function forgingWeapon(uint256 tokenAmount) public returns (bool) {
        uint256 rate = tokenAmount * ratePerPieces;
        if(rate >= 100){
            return true;
        }else{
            randomNumber = RANDOM_CONTRACT(RandomWorkerContract).startRandom();
            uint16 randomNumberForNFTType = getNumberAndMod(randomNumber, 2, 1000);

            if(uint16(rate) > uint16(randomNumberForNFTType)){
                return true;
            }else{
                return false;
            }
