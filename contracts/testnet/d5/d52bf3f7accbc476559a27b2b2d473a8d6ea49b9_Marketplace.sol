...
contract Marketplace is Ownable {
    ...
    */
    function OfferAndMint(tuple memory values, string memory uri, uint8 v, bytes32 r, bytes32 s, IERC721 contractAddress) public returns (uint256) {
    ...
    function signVerification (address seller, address buyer, uint256 deadline, uint256 amount, uint8 sv, bytes32 sr, bytes32 ss,  uint8 bv, bytes32 br, bytes32 bs) internal view returns (bool){
        ...
        require(seller == ecrecover(getSignedHash(keccak256(abi.encodePacked(seller))), sv, sr, ss), "Seller signature is Invalid");
        require( buyer == ecrecover(getSignedHash(keccak256(abi.encodePacked(buyer))), bv, br, bs), "Buyer signature is Invalid");
