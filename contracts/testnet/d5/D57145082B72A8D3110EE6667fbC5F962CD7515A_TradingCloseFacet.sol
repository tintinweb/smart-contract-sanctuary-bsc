...
contract TradingCloseFacet is ITradingClose, OnlySelf {
    ...
    function _closeTrade(bytes32 tradeHash, IOrderAndTradeHistory.CloseInfo memory ci, IOrderAndTradeHistory.ActionType aType) private {
        IOrderAndTradeHistory(address(this)).closeTrade(tradeHash, ci, aType);
    }

    struct CloseTradeTuple {
        bytes32 tradeHash;
        ITrading.OpenTrade ot;
        IVault.MarginToken mt;
        IPairsManager.FeeConfig feeConfig;
        uint256 marketPrice;
        uint256 closePrice;
        int256 fundingFee;
    }
    ...
    function closeTradeCallback(bytes32 tradeHash, uint upperPrice, uint lowerPrice) external onlySelf override {
        ...
        int256 longAccFundingFeePerShare = ITradingCore(address(this)).updatePairPositionInfo(ot.pairBase, marketPrice, ot.qty, ot.isLong, false);
        ...
        uint256 closeFee = _settleForCloseTrade(ts, CloseTradeTuple(tradeHash, ot, mt, feeConfig, marketPrice, closePrice, fundingFee));
        _closeTrade(
            tradeHash,
            IOrderAndTradeHistory.CloseInfo(uint64(closePrice), int96(fundingFee), uint96(closeFee), 0),
            IOrderAndTradeHistory.ActionType.CLOSE
        );

        emit CloseTradeSuccessful(ot.user, tradeHash);
        _removeOpenTrade(ts, ot, tradeHash);
    }

    function _settleForCloseTrade(
        LibTrading.TradingStorage storage ts, CloseTradeTuple memory tuple
    ) internal returns (uint256 closeFee){
        uint256 closeNotionalUsd = tuple.closePrice * tuple.ot.qty;
        int256 pnl = int256(tuple.ot.isLong ? closeNotionalUsd - tuple.ot.entryPrice * tuple.ot.qty : tuple.ot.entryPrice * tuple.ot.qty - closeNotionalUsd)
        * int256(10 ** tuple.mt.decimals) / int256(1e18 * tuple.mt.price);

        // openTradeReceive + closeFee + userReceive + lpReceive == 0
        // closeFee >= 0 && userReceive >= 0
        int256 openTradeReceive = - int256(uint256(tuple.ot.margin)) - tuple.fundingFee;
        closeFee = closeNotionalUsd * tuple.feeConfig.closeFeeP * (10 ** tuple.mt.decimals) / (1e4 * 1e10 * tuple.mt.price);
        uint256 userReceive;
        int256 lpReceive;
        ...
        _settleAsset(ts, SettleAssetTuple(tuple.ot, tuple.tradeHash, openTradeReceive, closeFee, userReceive, lpReceive));
        return closeFee;
    }
    ...
    function executeTpSlOrLiq(TpSlOrLiq[] memory arr) external override {
        ...
        require(arr.length > 0, "TradingTriggerFacet: Parameters are empty");
        for (UC i = ZERO; i < uc(arr.length); i = i + ONE) {
            TpSlOrLiq memory t = arr[i.into()];
            // todo: 更新  LibPriceFacade 里的价格
            require(t.price > 0, "TradingTriggerFacet: price must be greater than 0");
            if (t.executionType == ExecutionType.TP_SL) {
                _executeTpOrSl(t.tradeHash, t.price);
            } else {
                _executeLiquidate(t.tradeHash, t.price);
            }
        }
    }

    function _executeTpOrSl(bytes32 tradeHash, uint64 marketPrice) private {
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ITrading.OpenTrade storage ot = ts.openTrades[tradeHash];
        require(ot.margin > 0, "TradingTriggerFacet: Trade information does not exist");
        ...
        if ((ot.isLong && closePrice > ot.entryPrice) || (!ot.isLong && closePrice < ot.entryPrice)) {
            _closeTrade(
                tradeHash,
                IOrderAndTradeHistory.CloseInfo(uint64(closePrice), int96(fundingFee), uint96(closeFee), 0),
                IOrderAndTradeHistory.ActionType.TP
            );
            emit TriggerTakeProfit(ot.user, tradeHash);
        } else {
            _closeTrade(
                tradeHash,
                IOrderAndTradeHistory.CloseInfo(uint64(closePrice), int96(fundingFee), uint96(closeFee), 0),
                IOrderAndTradeHistory.ActionType.SL
            );
            emit TriggerStopLoss(ot.user, tradeHash);
        }
        _removeOpenTrade(ts, ot, tradeHash);
    }


    function _executeLiquidate(bytes32 tradeHash, uint64 marketPrice) private {
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ITrading.OpenTrade storage ot = ts.openTrades[tradeHash];
        require(ot.margin > 0, "TradingTriggerFacet: Trade information does not exist");
        ...
        _closeTrade(
            tradeHash,
            IOrderAndTradeHistory.CloseInfo(uint64(closePrice), int96(fundingFee), uint96(closeFee), 0),
            IOrderAndTradeHistory.ActionType.LIQUIDATED
        );

        emit TriggerLiquidate(ot.user, tradeHash);
        _removeOpenTrade(ts, ot, tradeHash);
    }
    ...
    function _removeOpenTrade(
        ...
        bytes32[] storage userTradeHashes = ts.userOpenTradeHashes[msg.sender];
...
library LibFeeManager {
    ...
    struct FeeManagerStorage {
        ...
        mapping(address => IFeeManager.FeeSummary) feeSummaries;
...
interface IFeeManager {
    ...
    function feeSummary(address token) external view returns (FeeSummary memory);
