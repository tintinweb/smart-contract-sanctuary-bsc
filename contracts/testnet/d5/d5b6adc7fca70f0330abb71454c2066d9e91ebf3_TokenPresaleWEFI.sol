...
contract TokenPresaleWEFI is Ownable, ReentrancyGuard {
    ...
    uint8 public referralCodeBonusPercentage = 20;
    ...
    event ReferralCommissionPercentageChanged(uint256, uint256);
    event ReferralCodeBonusPercentageChanged(uint256, uint256);
    ...
    function changeReferralCodeBonusPercentage(uint8 newPercentage) external onlyOwner returns (bool) {
        uint256 oldPercentage = referralCodeBonusPercentage;
        referralCodeBonusPercentage = newPercentage;

        emit ReferralCodeBonusPercentageChanged(oldPercentage, newPercentage);
        return true;
    }
    ...
    function changeReferralCommissionPercentage(uint8 newPercentage) external onlyOwner returns (bool) {
        ...
        emit ReferralCommissionPercentageChanged(oldPercentage, newPercentage);
        return true;
    }
    ...
    function _buyToken(uint256 referralCode) private {
        ...
        if(referrer != address(0)){
            commissionedTokens = _commissionedTokens(buyableTokens);
            buyableTokens += (buyableTokens*referralCodeBonusPercentage)/100;
        }
    ...
    function _sendToBuyerVesting(address beneficiary, uint256 amount) private {
        if(buyerVestingCliff == 1 && buyerVestingDuration == 1){
            require(tokenAddress.transfer(beneficiary, amount), "TokenPresaleWEFI: token WEFI transfer to buyer not succeeded");
        }
        else{
    ...
    function _sendToReferrerVesting(address beneficiary, uint256 amount) private {
        if(referrerVestingCliff == 1 && referrerVestingDuration == 1){
                require(tokenAddress.transfer(beneficiary, amount), "TokenPresaleWEFI: token WEFI transfer to referrer not succeeded");
        }
        else{
            require(tokenAddress.approve(address(vestingContractAddress), amount), "TokenPresaleWEFI: token WEFI approve to vesting contract not succeeded");
    ...
    function withdrawBnbBalance() external onlyOwner returns (bool) {
        require(payable(owner()).send(address(this).balance), "Failed to send BNB");
    ...
    function withdrawContractTokenBalance(uint256 amount) external onlyOwner returns (bool)
    {
