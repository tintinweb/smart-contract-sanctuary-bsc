...
contract BNBRegistrarControllerV2 is Ownable, EIP712 {
    ...
    constructor(address _sid_signer,
    ...
    function register(string calldata name, address owner, uint duration, bytes calldata signature) external payable {
      registerWithConfig(name, owner, duration, address(0), address(0), signature);
    }
	...
	require(
            _verify(
                _hash(name, label, owner, duration, resolver, addr),
                signature
            ),
            "Invalid signature"
        );
    ...
    function _hash(
        ...
        return
            _hashTypedDataV4(
        		...
        		abi.encode(
        		    keccak256(
        			"SID(string name,bytes32 label,address owner,uint duration,address resolver,address addr)"
        			),
			    name,
			    label,
			    owner,
