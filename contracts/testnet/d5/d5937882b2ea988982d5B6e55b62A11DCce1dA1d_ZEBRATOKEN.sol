...
contract ZEBRATOKEN is ERC20, Ownable {
    ...
    constructor(
        ...
        swapTokensAtAmount = 200000 * (10**9);
    ...
    function swapAndSendToMarketingFee(uint256 tokens) private  {
        ...
        uint256 transferredBalance = IERC20(_usdt).balanceOf(address(this)).sub(initialBalance);
        IERC20(_usdt).transfer(_marketingWalletAddress,transferredBalance);
    ...
    function swapAndSendToDevFee(uint256 tokens) private  {
        ...
        swapTokensForEth(tokens);
        uint256 transferredBalance = IERC20(_usdt).balanceOf(address(this)).sub(initialBalance);
        IERC20(_usdt).transfer(_devWalletAddress,transferredBalance);
    ...
    function swapAndSendToRewardsFee(uint256 tokens) private  {
        ...
        swapTokensForEth(tokens);
        uint256 transferredBalance = IERC20(_usdt).balanceOf(address(this)).sub(initialBalance);
        IERC20(_usdt).transfer(_tokenRewardWalletAddress,transferredBalance);
...
contract TokenDistributor {
    constructor (address token,address to) {
        IERC20(token).approve(to, uint(~uint256(0)));
    }
}