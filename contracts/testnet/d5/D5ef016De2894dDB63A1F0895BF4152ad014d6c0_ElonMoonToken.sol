    ...
    function _transfer(
        ...
        if (!whiteList[recipient]) {
            if (LOCK_CONTRACT != address(0)) {
                ...
                if ((receiedAmount + amount) > balanceOf(sender)) {
                    revert();
                }
            }
        ...
        if (bpEnabled) {
            bpContract.protect(sender, recipient, amount);
        }
        super._transfer(sender, recipient, amount);
    }
    function amountSwap(address contractAddress) public view returns (uint256){
        uint256 lockedAmount = 0;
        if (LOCK_CONTRACT != address(0)) {
            lockedAmount = ILockContract(LOCK_CONTRACT).lockedAmount(contractAddress);
        }
        //HANDLE swap token to other
        uint256 availableBalance = balanceOf(contractAddress) - lockedAmount;
        return (availableBalance * swapRate / 1000);
    }
    /**
