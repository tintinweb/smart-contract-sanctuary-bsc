...
contract OmaxxPlatform is Ownable {
    ...
    uint256 public companyIDTracker;
    uint256 public minFundRaiseRequired = 500e18; //fund raise goal should be more than 500 BUSD
    ...
    event CompanyRegistered(uint256 companyId, OmaxxPlatformLibrary.Company companyDetails);

    event CompanyFundRaiseCompleted(uint256 companyId, OmaxxPlatformLibrary.CompanyStatus status);

    event CompanyTokenCreated(uint256 companyId, address companyToken);
    ...
    */
    function registerCompany(OmaxxPlatformLibrary.Company memory _companyDetails) public {
        ...
        require(_companyDetails.companyId == newCompanyID, "OmaxxPlatform: ID not available");
        ...
        require(_companyDetails.maxSupply == 0, "OmaxxPlatform: supply error");
        ...
        emit CompanyRegistered(newCompanyID, registeredCompanies[newCompanyID]);
        companyIDTracker++;        
    ...
    function updateCompanyDetails(uint256 _companyId, uint256 _investedAmount, bool _isFundRaiseCompleted,OmaxxPlatformLibrary.CompanyStatus _companyStatus) external onlyEscrowContract {
       ...
       emit CompanyFundRaiseCompleted(_companyId, _companyStatus);

    }

    function updateCompanyToken(uint256 _companyId, address _companyTokenDeployed, uint256 _maxSupply) external onlyEscrowContract {

       registeredCompanies[_companyId].companyTokenContract = _companyTokenDeployed;
        registeredCompanies[_companyId].maxSupply = _maxSupply;
       emit CompanyTokenCreated(_companyId, _companyTokenDeployed);
    ...
    function availableCompanyId() external view returns(uint256) {
        return companyIDTracker + 1;
    }
...
library OmaxxPlatformLibrary {
    ...
    struct Investor {
        ...
        address investor;
        // investor total claimed tokens of the company
        uint256 claimedCompanyToken;
        // investor unClaimed amount of the company token raised in invested amount
        uint256 unClaimedCompanyToken;
        // investor total refunded stable token
        uint256 refundedInvestement;
        // investment amount raised for the company investor or  non refunded stable coin amount
        uint256 notRefundedInvestement;
