...
contract StiMining is Ownable {
    ...
    struct PoolInfo {
        ...
        IBEP20 iptToken;
        IBEP20 optToken;
        uint rate;       // How many CNN daily output from this pool.
        uint lastTime;  // Last time that CNN distribution occurs.
        uint debted; // debt
        uint endDebted;
        uint tvl; 
        uint startTime;
    ...
    function getPoolList(uint stage_) public view returns(uint[] memory opt) {
        opt = new uint[](poolInfo.length);
        uint size;
        
        for (uint i; i < poolInfo.length; i++) {
            uint start = poolInfo[i].startTime;
            uint end = poolInfo[i].endTime;
            if (stage_ == 0) {
    ...
    function viewInfo(uint pId_, address account_) public view returns(string[3] memory names, uint[2] memory decimlas, address[3] memory addrInfos, uint[5] memory infos, uint[5] memory uInfos) {
        ...
        infos[1] = pool.startTime;
        infos[2] = pool.endTime;
        infos[3] = pool.lockStart;
        infos[4] = pool.lockStart + pool.lockInterval;
    ...
    function getPoolInfo(uint pId_) public view returns(string[3] memory names , uint[4] memory infos) {
        ...
        infos[2] = pool.startTime;
        infos[3] = pool.endTime;
    ...
    function setTm(uint pId_, uint stage_) public {
        ...
        if (stage_ == 0) {
            pool.startTime = block.timestamp + 60;
            pool.endTime = block.timestamp + 120;
        } else if (stage_ == 1) {
            pool.startTime = block.timestamp - 10;
            pool.endTime = block.timestamp + 120;
        } else if (stage_ == 2) {
            pool.startTime = block.timestamp + 10;
            pool.endTime = block.timestamp + 120;
    ...
    function updatePool(uint pid_, uint value_) private returns(uint) {
        PoolInfo storage pool = poolInfo[pid_];
        ...
        if (pool.endDebted == 0 && block.timestamp >= pool.endTime)  {
            pool.endDebted = pool.tvl > 0 ? pool.rate * (pool.endTime - pool.lastTime) * ACC / pool.tvl + pool.debted : 0 + pool.debted;
        }
    ...
    function stake(uint pId_, uint amount_, address inviter_) public {
        ...
        require (block.timestamp >= pool.startTime && block.timestamp < pool.endTime, "wrong time"); 
        if (user.stake > 0) {
        ...
        user.debt = updatePool(pId_, amount_);
        user.stake += amount_;
    ...
    function unStake(uint pId_) public {
        ...
        pool.optToken.transfer(_msgSender(), user.stake );
        
        user.stake = 0;
        user.debt = 0;

        updatePool(pId_, user.stake);
    }
    ...
    function claimReward(uint pId_) public {
        ...
        if (pool.lockStart != 0) {
            require(block.timestamp > pool.lockStart, "not release");
            uint tm = block.timestamp - (user.lastClaimTime == 0 ? pool.lockStart : user.lastClaimTime);
            reward = (user.toClaim - user.claimed) / pool.lockInterval * tm;
    ...
    function addPool(uint[4] memory times_, address[2] memory tokens_, uint maxOut_) public  returns(uint _pid) {
