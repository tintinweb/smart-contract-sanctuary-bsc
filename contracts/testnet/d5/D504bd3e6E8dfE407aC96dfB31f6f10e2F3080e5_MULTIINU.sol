...
contract MULTIINU is BEP20, Auth {
    ...
    // switch Trading
    function StatusDeTrade(bool _status, bool _ab) external onlyOwner {
        if(!_status || _ab){
            require(launchMode,"Cannot stop trading after launch is done");
        }
        tradingOpen = _status;
        antibot = _ab;
        emit config_StatusDeTrade(tradingOpen);
    }

    function StatusDeTrade_launchmode(uint256 confirm) external onlyOwner {
...
event config_StatusDeTrade(bool Status);
