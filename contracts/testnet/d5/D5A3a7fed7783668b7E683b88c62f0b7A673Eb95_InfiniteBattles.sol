...
contract InfiniteBattles is Ownable {
    ...
    function battleWinner(uint256 blockId_, address winner_) external {
        if(blocks[blockId_].owner != winner_){ // Block owner changes
            address currentBlockOwner = blocks[blockId_].owner;
            
            // remove block id from this users portfolio
            EnumerableSet.remove(users[currentBlockOwner].blockIdsOwn, blockId_);
            ...
            blocks[blockId_].rewardsCalUntilBlock = block.number;
        }else{
    ...
    function pendingRewards(address user_) view external returns(uint256) {
    ...
    function createBlock(IERC20 rewardToken_, uint256 amountRewards_, uint256 runsForDays_) external onlyOwner {

        uint256 tokenPerBlock = amountRewards_ / (runsForDays_ * 28800);
        blocks[blockIds] = Block(
            blockIds,               // blockId
            rewardToken_,           // rewardToken_
            0,                      // battleStartedBlock
            amountRewards_,         // rewards
            tokenPerBlock,          // tokensPerBlock
            0,                      // battleFee
            address(0),             // owner
            0,                      // attacker
            address(0),             // attackerAddress   
            0
        );
        blockIds++;
    }
