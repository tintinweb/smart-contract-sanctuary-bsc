...
contract KoalaDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("Koala_Dividend_Tracker", "Koala_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18);
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Koala_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Koala_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Koala contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Koala_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"); 
        require(newClaimWait != claimWait, "Koala_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Koala is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Koala", "Koala") 
    {
        ...
        devWallet = payable(0x4f07daEA862D10983B9c3416cB4D5117e23654F5);

        totalFees = ETHRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy
    ...
    function updateAllFeeRates(uint256 _ETHRewardsFee,  uint256 _liquidityFee, uint256 _marketingFee, uint256 _devFee, uint256 _extraSellFee) 
    external returns(bool)
    {
        require(_extraSellFee<=4, "Extra Sell Fee Can be 10% or less");
        totalFees = _ETHRewardsFee.add(_liquidityFee).add(_marketingFee).add(_devFee);
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Koala: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Koala: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Koala: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Koala: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Koala: gasForProcessing must be between 200,000 and 500,000");
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForEth(swapableTokens, address(this)); 
        uint256 deltaBalance = address(this).balance-initialBalance;
        uint256 ethToAddLiquidityWith = deltaBalance.mul(liquidityFee).div(totalFees).div(2);

        addLiquidity(halfLiquidityTokens, ethToAddLiquidityWith);


        uint256 ethForMarketing = deltaBalance.mul(marketingFee).div(totalFees);
        marketingWallet.transfer(ethForMarketing);

        uint256 ethForDev = deltaBalance.mul(devFee).div(totalFees);
