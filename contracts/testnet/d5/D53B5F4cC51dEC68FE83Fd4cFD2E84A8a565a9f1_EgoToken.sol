...
contract EgoToken is AxelarExecutable, IERC20CrossChain, Pausable, ERC20, AccessControl, ERC20Permit {
    ...
    //Can send tokens in enother network
    ...
    /** @notice Burn tokens from address of transaction sender
        * @param amount Amonut of tokens for transfer
    */
    function burn(uint256 amount) external {
        _burn(msg.sender, amount);
    }
    ///need to check that you ca not burn tokens of enother user, if approveal doesnt exit and you sign messsage for enother 
    ...
    ) public payable override 
    {
        _checkRole(BRIDGE_MASTER_ROLE, destinationAddress);
        {
            bytes32 message = keccak256(
                abi.encodePacked(block.chainid, msg.sender, amount, address(this), _salt));
