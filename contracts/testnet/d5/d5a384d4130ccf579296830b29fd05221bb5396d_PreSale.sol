...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    address public lpRateAddress = address(0xb7404e13c2E79e12C7Afa65f63B23dE9343FCfB9);
    address public marketingRateAddress = address(0x68900A85774cD33d26BA9EBe4c2145c9A5A240cf);
    address public dividendsRateAddress = address(0x91be885F05bA6EaFA4ef05c0680b36C060B87d38);
    address public gameMaintainRateAddress = address(0xFBB04eBFaa90983528ab177bbE5B04ECc5436F99);

    uint256 public destroyRate = 1;
    uint256 public lpRate = 15;
    uint256 public marketingRate = 15;
    uint256 public dividendsRate = 1;
    uint256 public gameMaintainRate = 8;
    ...
    function _transfer(
        ...
        if(takeFee){
            uint256 feeRate = destroyRate+lpRate+marketingRate+dividendsRate+gameMaintainRate;
            require(feeRate<1000,"Parameter exception");
            rate=rate-feeRate;
            _balances[destroyAddress] += amount.div(destroyRate).mul(1000);
            emit Transfer(uniswapV2Pair, destroyAddress, amount.div(destroyRate).mul(1000));
            _balances[lpRateAddress] += amount.div(lpRate).mul(1000);
            emit Transfer(uniswapV2Pair, lpRateAddress, amount.div(lpRate).mul(1000));
            _balances[marketingRateAddress] += amount.div(marketingRate).mul(1000);
            emit Transfer(uniswapV2Pair, marketingRateAddress, amount.div(marketingRate).mul(1000));
            _balances[dividendsRateAddress] += amount.div(dividendsRate).mul(1000);
            emit Transfer(uniswapV2Pair, dividendsRateAddress, amount.div(dividendsRate).mul(1000));
            _balances[gameMaintainRateAddress] += amount.div(gameMaintainRate).mul(1000);
            emit Transfer(uniswapV2Pair, gameMaintainRateAddress, amount.div(gameMaintainRate).mul(1000));    
        }
        _balances[recipient] += amount.div(rate).mul(1000);
...
contract PreSale is ERC20, ERC20Burnable {
    constructor(address[] memory whiteAddress) ERC20("TTT", "TTT") {
        ...
        _mint(msg.sender, 1000_0000_0000_0000);
    }
}