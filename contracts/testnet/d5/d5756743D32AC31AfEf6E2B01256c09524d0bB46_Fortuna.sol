...
contract Fortuna {
    ...
    constructor() public {
        ...
        owner = ownerAddress;
        marketing_address = address(0xBe0aF8dc78f28Ac32c40b158CE73631561E3BBdF);
        lottery_address = address(0x55aF5a9F659A2dFC55B79DBB32D268c096944d2F);
    ...
    function buyNewLevel( uint8 level) external payable {
        ...
        require(!users[msg.sender].activeX3Levels[level], "level already activated");
            ...
            payable(lottery_address).transfer(msg.value * 200 / 1000);

            (uint256 random_ref_id ,  uint256 random_ref_level) = getLineWinnerByLevel(level);
            address random_ref = idToAddress[random_ref_id]; 
            
            payable(random_ref).transfer(msg.value * 665 / 1000);

            users[random_ref].activeCycleLevels[random_ref_level]--;
            users[random_ref].activeEndedCycleLevels[random_ref_level]++;
            ...
            (address ref1, address ref2, address ref3) = usersX3MatrixForRewards(users[msg.sender].id,level);
            
            payable(ref1).transfer(msg.value * 3 / 100 );
            payable(ref2).transfer(msg.value * 2 / 100 );
            payable(ref3).transfer(msg.value  / 100 );

            payable(lottery_address).transfer(address(this).balance);
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        (uint256 random_ref_id ,  uint256 random_ref_level) = getLineWinnerByLevel(1);
        ...
        (address ref1, address ref2, address ref3) = usersX3MatrixForRewards(users[userAddress].id,1);
    ...
    function getActiveCycle(uint256 userid, uint256 level) public view returns(uint256 ) {

        return users[idToAddress[userid]].activeCycleLevels[level] ;
    }
    function getLineWinnerByLevel(uint256 level) public view returns(uint256 , uint256) {
        
        
        if(level>16 || level<1) return (1,16);
    ...
    function usersX3MatrixForRewards(uint256 userid, uint8 level) public view returns(address, address, address) {
        
        address userAddress = idToAddress[userid];
        address ref1=findFreeX3Referrer( userAddress, level);
        address ref2=findFreeX3Referrer(ref1, level);
        address ref3=findFreeX3Referrer(ref2, level);
