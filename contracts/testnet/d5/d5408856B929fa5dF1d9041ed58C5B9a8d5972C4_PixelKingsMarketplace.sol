...
contract PixelKingsMarketplace is Ownable {
    ...
    function buyBox(Box _box, uint256 _amount) external {
        //verificar
        require(_amount >= boxValue[_box], "Not enough amount");

        if (privateSale) {
            for (uint256 i; i <= witheList.length; i++) {
                if (witheList[i] == msg.sender) {
                    address owner = owner();
                    ERC20(tokenAddress1).transferFrom(
                        msg.sender,
                        owner,
                        _amount
                    );
                    if (_box == Box.BronzenBox) {
                        userBox[msg.sender] = "BronzenBox";
                    } else if (_box == Box.SilverBox) {
                        userBox[msg.sender] = "SilverBox";
                    } else if (_box == Box.GoldenBox) {
                        userBox[msg.sender] = "GoldenBox";
                    } else if (_box == Box.GreenBox) {
                        userBox[msg.sender] = "GreenBox";
                    } else if (_box == Box.BlueBox) {
                        userBox[msg.sender] = "BlueBox";
                    }
                }
            }
        } else if (!privateSale) {
    ...
    function openBronzenBox(
        ...
        external
        dontHaveBox(msg.sender, "BronzenBox")
        returns (HeroS memory hero)
    {
        ...
        //TODO - MINT NEW NFT
        heroContract.mintHero(msg.sender, hero.uri, hero.name, uint(hero.class), uint(hero.rarity));
    }

    function openSilverBox(Class _class, string memory _module)
        external
        payable
    {}

    function openGoldenBox(Class _class, string memory _module)
        external
        payable
    {}

    function openGreenBox(Class _class, string memory _module)
        external
        payable
    {}

    function openBlueBox(Class _class, string memory _module)
        external
        payable
    {}
    ...
    function setHero(address _address) external onlyOwner {
        heroContract = Hero(_address);
    }
}
...
contract Hero is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Hero", "HR") {}
     ...
     function mintHero(address _recipient, string memory _tokenURI, string memory _name, uint _class, uint _rarity)
        external
        onlyOwner
        returns (uint256)
    {
