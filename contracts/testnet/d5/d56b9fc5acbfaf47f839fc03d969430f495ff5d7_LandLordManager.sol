...
contract LandLordManager is Auth {
  ...
  function initialize(address _mainAdmin, address _fotaToken, address _pve) public initializer {
  ...
  function syncLandLord(uint _mission) external {
    _syncLandLord(_mission);
  }

  function _syncLandLord(uint _mission) private {
    lands[_mission].landLord = gamePve.getLandLord(_mission);
  }

  function giveReward(uint _mission, uint _amount) external {
    require(lands[_mission].landLord != address(0), "Please setup landLord first");
    fotaToken.transfer(lands[_mission].landLord, _amount);
  }

  function setContracts(address _fotaToken, address _pve) external onlyMainAdmin {
    fotaToken = IFOTAToken(_fotaToken);
    gamePve = IFOTAGame(_pve);
  }

  function _syncLandLords() private {
    for(uint i = 1; i <= 30; i++) {
      _syncLandLord(i);
    }
  }
