...
contract CBTower is Ownable, ICBTower {
    ...
    IERC20 public USDT = IERC20(0x508526c280539a5B547F82c0DB8A2b2D70aCFDd4);
    ...
    struct UserInfo {
        ...
        // NEW
        bool even;
        uint256 lastEvenFloor;
        uint256 lastEvenAmount;
        bool odd;
        uint256 lastOddFloor;
        uint256 lastOddAmount;
    }

    mapping (address => uint256) public claimedByUser; // FOR TEST
    ...
    address private immutable genericReferral = 0xE14C017B21c31d6E874Eb93cD2d4aa4C2BCea855;

    bool public building = true;

    event FloorFinished(uint256 floor);
    event SuccessfullyInvested(address user, uint256 floor, uint256 amount);
    event DividendsClaimed(address user, uint256 amount);
    ...
    function invest(address _referral, uint256 amount) public {
        ...
        if (user.joinTime + 2 days >= block.timestamp && user.level1 != address(0) && user.level1 != genericReferral) {
            IERC20(BCT).mint(_msgSender(), amount * 4 / 10);
        } else {
            IERC20(BCT).mint(_msgSender(), amount * 2 / 10);
        }
    }

    function changeReferral(address newReferral) public {
        UserInfo storage user = userInfo[_msgSender()];
        UserInfo storage referral = userInfo[newReferral];
        require(user.joinTime != 0, "User has not participated");
        require(user.level1 == genericReferral, "User already has set referral");
        user.level1 = newReferral;
        user.level2 = referral.level1;
        user.level3 = referral.level2;
        user.level4 = referral.level3;
        user.level5 = referral.level4;
    ...
    /// @dev uncomment to enable different reinvest ratios
    // function changeReinvestRatio(uint256 newRatio) public {
    //     require(newRatio >= 500, "Minimum 0.5");
    //     require(newRatio <= 1000, "Maximum 1");
    //     withdrawPendingUSDT();
    //     UserInfo storage user = userInfo[_msgSender()];
    //     uint256 oldAm;
    //     if (user.even) {
    //         oldAm += user.lastEvenAmount;
    //     }
    //     if (user.odd) {
    //         oldAm += user.lastOddAmount;
    //     }

    //     floorReinvestAmounts[currentFloor+1][user.reinvestRatio] -= oldAm;
    //     user.reinvestRatio = newRatio;
    //     floorReinvestAmounts[currentFloor+1][user.reinvestRatio] += oldAm;
    // }
    ...
    function withdrawPendingUSDT() public {
        UserInfo storage user = userInfo[_msgSender()];
        require(user.joinTime != 0, "User has not participated");
        (uint256 amount, bool even, bool odd) = getWithdrawableUSDT(_msgSender());
        if (amount > 0) {
            /// @dev uncomment to pay bonuses for reinvest
            // _payReferrals(amount);
            // uint256 BCTamount = amount * 2 / 10;
            // DividendPayingTokenInterface(address(BBT)).distributeDividends(BCTamount);
            // IERC20(BCT).mint(_msgSender(), amount * 2 / 10);
            claimedByUser[_msgSender()] += amount;
            if (even) {
                if (currentFloor % 2 > 0) {
                    user.lastEvenFloor = currentFloor - 1;
                } else {
                    user.lastEvenFloor = currentFloor;
                }
            }
            if (odd) {
                if (currentFloor % 2 > 0) {
                    user.lastOddFloor = currentFloor - 1;
                } else {
                    user.lastOddFloor = currentFloor;
                }
            }
    ...
    function getWithdrawableUSDT(address _user) public view returns (uint256 amount, bool even, bool odd) {
        UserInfo storage user = userInfo[_user];
        if (user.even) {
            if ( (currentFloor - user.lastEvenFloor) / 2 > 0) {
                amount += 2 * user.lastEvenAmount * (1000 - user.reinvestRatio) / 1000 * ( (currentFloor - user.lastEvenFloor) / 2);
                even = true;
            }
        }
        if (user.odd) {
            if ( (currentFloor - user.lastOddFloor) / 2 > 0) {
                amount += 2 * user.lastOddAmount * (1000 - user.reinvestRatio) / 1000 * ( (currentFloor - user.lastOddFloor) / 2);
                odd = true;
            }
        }
    }
    ...
    function getBuildTime() internal view returns (uint256 buildTime) {
        buildTime = (currentFloor + 1) * 4 days;
    }
    ...
    function _initiateNewFloor() internal {
        ...
        /// @dev If reinvest ratio could be changed, change 501 to 1001
        for (uint256 i = 500; i < 501; i++) {
            if (floorReinvestAmounts[currentFloor][i] > 0 && currentFloor > 1) {
                uint256 _amount = floorReinvestAmounts[currentFloor][i] * i / 1000;
                floorReinvestAmounts[currentFloor+1][i] = _amount;
                if (currentFloor > 1) {
                    _startAmount += floorReinvestAmounts[currentFloor-1][i] * i / 1000;
                }
            }
        }
    ...
    function _invest(uint256 amount) internal returns (uint256 left){ // change to internal
        ...
        if (leftForCurrentFloor_ > amount) {
            ...
            floorReinvestAmounts[currentFloor+1][user.reinvestRatio] += 2*amount;
            _updateOddEven(amount);
            emit SuccessfullyInvested(_msgSender(), currentFloor, amount);
        } else {
            left = amount - leftForCurrentFloor_;
            floor.totalInvested += leftForCurrentFloor_;
            floorReinvestAmounts[currentFloor+1][user.reinvestRatio] += 2*leftForCurrentFloor_;
            _updateOddEven(leftForCurrentFloor_);
            emit SuccessfullyInvested(_msgSender(), currentFloor, leftForCurrentFloor_);
        }
    ...
    function _updateOddEven(uint256 amount) internal {
        UserInfo storage user = userInfo[_msgSender()];
        if (currentFloor % 2 == 0) {
            user.even = true;
            user.lastEvenFloor = currentFloor;
            user.lastEvenAmount = amount;
        } else {
            user.odd = true;
            user.lastOddFloor = currentFloor;
            user.lastOddAmount += amount;
        }
    }
