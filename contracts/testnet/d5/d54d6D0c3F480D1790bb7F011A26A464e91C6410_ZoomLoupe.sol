...
contract ZoomLoupe is Ownable, Pausable{
    IERC20 MITICOIN;
    IERC20 USDT;
    IERC721 NFTWA;
    ...
    uint256 Num_NFTW =200;
    uint256 Max_NFTWA= 10000;

    address Wallet = 0x6AFa5C9D22231Db5364De76751085eAC8f1E791e;
...
constructor() {
        ...
        NFTWA = IERC721(0x6c7404026ae93399A93A0489e1F77F38BaE95512); 
        TrueChest =ITrue_chest(0x7F8F1eF8C969b1d289e2ee7a74bc83cA15245e4a);
        Take = ITake(0xe2382d212108270333dCa401D4D59220e760DBB3);////
        Detals = IERC1155 (0x336ef3077f67664f0Cd5e9220c3e15aCF8ce2893);
    }
...
///////
    ...
    function Take_chast (uint256 num_Track, uint256 chest_num) public {
        TrueChest.Take_chest(num_Track, chest_num,msg.sender);
       
        MITICOIN.transferFrom(address(this), msg.sender, Take.Get_One_chest_MITI(num_Track,chest_num));
        USDT.transferFrom(address(this),msg.sender,Take.Get_One_chest_USDT(num_Track, chest_num));

        if(chest_num>=4){
        if (Num_NFTW<=Max_NFTWA){
        NFTWA.safeTransferFrom(Wallet, msg.sender, Num_NFTW);
        Num_NFTW++;
        }
        }
    }

    function Take_detal() public {
        uint256 Summ = TrueChest.Get_detal(msg.sender);
        require(Summ>=1);
        Detals.safeTransferFrom(Wallet,msg.sender, 30, Summ, "");
        TrueChest.take_detal(Summ,msg.sender);
    }
