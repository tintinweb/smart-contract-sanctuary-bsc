...
contract EI is Context, IERC20, Ownable {
    ...
    uint256 public repoTrigger = 5 * 10**18;                                        
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(pairs[sender] && !noFee[sender] && !noFee[recipient]) {
            ...
            fee = debugBnbDiff.div(buyFee);  
            

            payable(marketingWallet).transfer(fee.mul(buyMarketingFee));
            
            lpAccumulator += fee.mul(buyLpFee);
            payable(lpWallet).transfer(fee.mul(buyLpFee));
