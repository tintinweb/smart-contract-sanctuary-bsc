...
contract CoinPrinter is Context, Ownable {
    ...
    uint256 private PRINTER_TO_HATCH_1MINERS = 1080000; // For final version should be seconds in a day
    ...
    function hatchPrinters(address ref) public {
        ...
        uint256 newMiners = SafeMath.div(printersUsed,PRINTER_TO_HATCH_1MINERS);
    ...
    function sellPrinters() public {
        ...
        recAdd.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(printerValue,fee));
    }
    ...
    function getPrintersSinceLastHatch(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(PRINTER_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastHatch[adr]));
