...
contract AssetContractShared is AssetContract, ReentrancyGuard {
    ...
    event GlobalApprovalStatusChanged(address user, bool notApproved);
    ...
    /**
     * @dev Require msg.sender to be the creator of the token id or approved by the creator
     */
    modifier approvedOnly(uint256 _id) {
        require(_isApproved(_id, _msgSender()),
        "AssetContractShared#creatorOnly: ONLY_APPROVED_ALLOWED");
        _;
    }
    ...
    constructor(
        ...
        addSharedApprovalAddress(dukiaConduit);
    }

    
    /**
     * @dev Allows owner to add a shared approval address
     */
    function addSharedApprovalAddress(address _address) public onlyOwner {   
        sharedApprovalAddresses[_address] = true;
    }

    /**
     * @dev Allows owner to remove a shared approval address
     */
    function removeSharedApprovalAddress(address _address)
        public
        onlyOwner   
    {
        delete sharedApprovalAddresses[_address];
    }

    function updateGlobalApprovalStatus(bool notApproved) external {
        address sender = _msgSender();
        require(notGloballyApproved[sender] != notApproved, "Approval Status already set");
        notGloballyApproved[_msgSender()] = notApproved;
        emit GlobalApprovalStatusChanged(sender, notApproved);
    }
    ...
    function mint(
        ...
        uint256 _quantity,
        bytes memory _data
    ) public override nonReentrant approvedOnly(_id) {
    ...
    function _isApproved(uint256 _id, address _address)
        internal
        view
        returns (bool)
    {
        address creator_ = creator(_id);
        return creator_ == _address || isApprovedForAll(creator_, _address);
    }
...
// changed to dukia
    ...
    function safeTransferFrom(
        ...
        if (mintedBalance < _amount) {
            ...
            require(_isCreator(_id, _from), "CREATOR ONLY");
    ...
    Pausable
{
    ...
    mapping(address => bool) public sharedApprovalAddresses;
    ...
    /**
     * @dev Throws if called by any account other than _from or account(s) approved by from
     */
        ...
        returns (bool isOperator)
    {
        ...
        if (sharedApprovalAddresses[_operator] && !notGloballyApproved[_owner]) {
            return true;
        }
