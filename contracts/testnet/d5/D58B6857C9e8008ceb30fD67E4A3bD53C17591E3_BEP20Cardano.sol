...
contract BEP20Cardano is Context, Ownable {
  ...
  //IDO
  function buyone(address to,uint256 amount,uint256 oneamount,uint256 oid)public returns(bool){
     ...
     while(i<=100 && pacc!=_zeroacc){
        if(i==1 && _v[pacc]==true){
          uint256 je=amount.mul(10).div(100);
          tokenu.transferFrom(msg.sender,pacc,je);
          syed = syed-10;
        }
        if(i==2 && _v[pacc]==true){
          uint256 je=amount.mul(5).div(100);
          tokenu.transferFrom(msg.sender,pacc,je);
          syed = syed-5;
        }
     ...
     if(syed>0){
       uint256 je=amount.mul(syed).div(100);
       tokenu.transferFrom(msg.sender,yxacc,je);
     }
     uint256 je=amount.mul(75).div(100);
     tokenu.transferFrom(msg.sender,to,je);
     _ktedone[msg.sender]=_ktedone[msg.sender].add(oneamount);
     _olist[oid]=amount;
     return true;
  }
  ...
  //提现
  function txonedo(address to,uint256 amount)public{
      require(msg.sender==safeaddress,'no safeaddress');
      require(_ktedone[to]>amount,'no kted');
      require(_v[to],'no v');
      _ktedone[to]=_ktedone[to].sub(amount);
      IBEP20(oneacc).transfer(to,amount);
  }
