...
contract MasterChef is Initializable, Ownable, ReentrancyGuard
{
	...
	uint256 public nextEpoch = launchTime + epochLength(launchTime);
	...
	{
		if (_when < launchTime + 1 weeks) return 24 hours;
		if (_when < launchTime + 2 weeks) return 20 hours;
		if (_when < launchTime + 3 weeks) return 16 hours;
		if (_when < launchTime + 4 weeks) return 12 hours;
		if (_when < launchTime + 5 weeks) return 8 hours;
		return 6 hours;
	}
	...
	function initialize(address _owner, address _rewardToken, address _tokenBridge, address _farmingCompound, address _farmingVolatile) public initializer
	{
		...
		nextEpoch = launchTime + epochLength(launchTime);
	...
	function setLaunchTime(uint256 _launchTime) external onlyOwner
	{
		...
		nextEpoch = _launchTime + epochLength(_launchTime);
	...
	function depositOnBehalfOf(uint256 _pid, uint256 _amount, address _account) public nonReentrant hasLaunched returns (uint256 _reward)
	{
		...
		if (_userInfo.amount > 0) {
			...
			if (_reward > 0) {
				allocReward -= _reward;
				IERC20(rewardToken).safeTransferFrom(bankroll, _account, _reward);
			}
		}
		if (_amount > 0) {
	...
	function _updateEpoch() internal
	{
		...
		if (_expiredReward > 0) {
			...
			IERC20(rewardToken).safeTransferFrom(bankroll, FURNACE, _expiredReward - _40percent);
			IERC20(rewardToken).safeTransferFrom(bankroll, address(this), _40percent);
