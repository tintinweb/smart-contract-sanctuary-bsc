...
contract GodStaking is Ownable, ReentrancyGuard {
    ...
    constructor() {
        token = IERC20(0x878b81DCE4B208DA5299c1215B60C280376929F3);
        poolInfo.push(PoolInfo( 130,  8, block.timestamp, 15 days,  60 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo( 323, 10, block.timestamp, 15 days, 120 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo( 584, 12, block.timestamp, 15 days, 180 seconds, 0, 0, 2000, 1000, true));
        poolInfo.push(PoolInfo(1500, 15, block.timestamp, 15 days, 360 seconds, 0, 0, 2000, 1000, true));
    }
    ...
    function extendPoolLength(uint256 _poolId, uint256 _duration) external onlyAuthorized{
        PoolInfo storage pool = poolInfo[_poolId];
        pool.poolLength += _duration;
    }

    // function timeLeftForThePool(uint256 _poolId) public view returns (uint256) {
    //     PoolInfo storage pool = poolInfo[_poolId];
    //     if(block.timestamp < pool.startEpoch + pool.poolLength){
    //         return pool.startEpoch + pool.poolLength - block.timestamp;
    //     } else {
    //         return 0;
    //     }
    // }

    // function timeLeftForAllPools() external view returns (uint256[] memory) {
    //     uint256 pools = poolInfo.length;
    //     uint256[] memory timeLeft = new uint256[](pools);

    //     for(uint256 i = 0; i < pools; ++i){
    //         timeLeft[i] = timeLeftForThePool(i);
    //     }
    //     return timeLeft;
    // }
    ...
    // function getLiveStakedAmountForPool(uint256 _poolId) external view returns(uint256) {
    //     PoolInfo storage pool = poolInfo[_poolId];
    //     return pool.liveStakedAmount;
    // }

    // function getLiveStakedAmounts() external view returns(uint256[] memory) {
    //     PoolInfo[] storage pool = poolInfo;

    //     uint256 pools = poolInfo.length;
    //     uint256[] memory liveStakedAmounts = new uint256[](pools);

    //     for(uint256 i = 0; i < pools; ++i){
    //         liveStakedAmounts[i] = pool[i].liveStakedAmount;
    //     }
    //     return liveStakedAmounts;
    // }
