...
contract MultiSaleArkadian is BEP20Burnable, Ownable{
  ...
  */
  function getPriceToken(address tokenAddress, uint256 amount) public view virtual returns (uint256) {
    address tokenPairAddress = IUniswapV2Factory(factory_address).getPair(tokenAddress, bnb_Address);
    ...
    uint256 priceInBnb = getpriceBNB(amount);
    if(bnb_Address == _tokenPair.token0()){
      ...
      return (priceInBnb).div(pricebnb.mul(_DECIMALFACTOR)); // return amount of Tokens needed to buy Box
    }else{
      (uint256 ResToken,uint256 ResBNB,) = _tokenPair.getReserves();
      uint256 pricebnb = ResBNB.div(ResToken);
      return (priceInBnb).div(pricebnb.mul(_DECIMALFACTOR)); // return amount of BNB needed to buy Box
    }
  }
