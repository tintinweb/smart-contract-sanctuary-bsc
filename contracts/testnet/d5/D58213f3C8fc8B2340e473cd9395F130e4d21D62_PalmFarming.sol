...
contract PalmFarming is Ownable {
    ...
    constructor(
        ...
        _addPool(_palmToken, _stakingRewardPerBlock, _stakingCooldownPeriod);
    }
    ...
    function setPoolInfo(
        ...
        if (_poolInfo.id == 0) {
            _addPool(_token, _rewardPerBlock, _cooldownPeriod);
        } else {
            updatePool(_token);
    ...
    function _addPool(
        ...
        uint64 mintStartBlock = palmToken.mintStartBlock();
        if (mintStartBlock == 0) {
            revert Errors.MintStartBlockNotSet();
        }
        ...
        poolInfo[_token] = PoolInfo({
            ...
            lastRewardBlock: Math
                .max(_getBlockNumber(), mintStartBlock)
                .toUint64(),
...
interface IPalmToken is IERC20 {
    function mintStartBlock() external view returns (uint64);
