...
contract BinancePay is Initializable, OwnableUpgradeable {
    ...
    function distributeRoyal(uint256 _loopcnt,address[] memory _addresses,uint256[] memory _amount)
        public
    {
        for(uint256 i=0; i < _loopcnt;i++){
            users[_addresses[i]].earnedAmount += _amount[i];
            users[_addresses[i]].totalearnedAmount += _amount[i];
            users[_addresses[i]].royalityincome += _amount[i];
            users[_addresses[i]].globalroyality1income += _amount[i];
            users[_addresses[i]].globalroyality2income += _amount[i];
        }
    ...
    function updateTeamRoyalityifMissed(address _address,uint256 _amount) public onlyOwner {
