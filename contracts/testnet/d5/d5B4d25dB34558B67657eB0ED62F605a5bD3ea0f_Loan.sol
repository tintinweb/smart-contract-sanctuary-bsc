...
contract Loan is ReentrancyGuard {
    ...
    uint256 public dayHours = 24;

    uint64 public paymentDue = 5;
    ...
    function cancel() public onlyOwner onlyPending nonReentrant {
        status = 0;
    ...
    function payBack(uint256 acceptId, uint256 _payBackAmount) public onlyBorrower nonReentrant {
        ...
        if(block.timestamp < lastPaymentTime + (paymentPeriod - paymentDue) * dayHours * 3600){
            revert("Payment Due Error!");
        }


        (uint256 principleToPay, uint256 interestToPay, uint256 feeToPay, uint256 extraPaneltyToPay) = getPaybackAmount(acceptId, msg.sender);

        uint256 totalPay = principleToPay + interestToPay + feeToPay + extraPaneltyToPay;
        ...
        IERC20(token).transferFrom(msg.sender, teamWallet, feeToPay + extraPaneltyToPay);
        ...
        acceptLoanMap[msg.sender][acceptId].lastPaymentTime = lastPaymentTime + paymentPeriod * 3600;

        // update credit amount in governace contract
        IGovernance(governance).increaseCreditAmountOfUser(msg.sender, principleToPay);

        emit PaidBack(msg.sender, principleToPay, interestToPay, feeToPay, extraPaneltyToPay, false);
        
    }

    function payOff(uint256 acceptId, uint256 _payBackAmount) public onlyBorrower nonReentrant {
        ...
        emit PaidBack(msg.sender, principleToPay, interestToPay, feeToPay, 0, true);
    ...
    returns (uint256 principleToPay, uint256 interestToPay, uint256 feeToPay, uint256 extraPaneltyToPay) {

        bool extraPaneltyFee = false;
        if(block.timestamp > acceptLoanMap[_account][acceptId].lastPaymentTime + paymentPeriod * 3600 * dayHours){
            extraPaneltyFee = true;
        }
        ...
        if(extraPaneltyFee){
            extraPaneltyToPay = principleToPay * 10 / 100; // 10%
        }
    ...
    function updateDayHours(uint256 _dayHours) public onlyOwner onlyPending {
        dayHours = _dayHours;
    }
    ...
    function getAcceptedLoanDetail(address _borrower, uint256 _acceptId) external view 
    returns (uint64 paidDays, uint256 aCceptedAt, uint256 lastPaymentTime) {
        paidDays = acceptLoanMap[_borrower][_acceptId].paidDays;
        aCceptedAt = acceptLoanMap[_borrower][_acceptId].acceptedAt;
        lastPaymentTime = acceptLoanMap[_borrower][_acceptId].lastPaymentTime;
...
interface IGovernance {
    ...
    function increaseCreditAmountOfUser(address _borrower, uint256 _amount) external;
