...
contract privatesale is Ownable, Initializable {
    ...
    IBEP20 public token;
    uint public price;
    uint public totalNumberOfVesting = 18;
    uint public initialClaim = 6e18;
    uint public subsequentClaim = 5220000000000000000;
    ...
    function startSale() external onlyOwner {
        require(isSalePaused, "Private : isSalePaused should be true");
        isSalePaused = false;
    }

    function finishSale() external onlyOwner {
        require(!isSalePaused, "Private : isSalePaused should be false");
        isSalePaused = true;
        saleClosedTime = block.timestamp;
    }

    function setReleaseDate(uint timestamp) external onlyOwner {
    ...
    function buy() external payable {
        require(!isSalePaused, "Private : isSalePaused should be false");
        require(price > 0, "Private : price > 0");
        require(msg.value > 0, "Private : msg.value > 0");
    ...
    function claim() external {
        userStruct storage user_ = user[msg.sender];
        require(isSalePaused, "Private : isSalePaused should be true");
        require(releaseDate != 0, "Private : releaseDate < block.timestamp");
        require(user_.totalClaimed < user_.balance, "Private : total claim < total balance");
        
        if(user_.lastClaim == 0) {
            user_.lastClaim = releaseDate;
        }
        uint totDays;
        uint lastClaimTimestamp = user_.lastClaim;
        uint claimAmount;

        if(block.timestamp > releaseDate + claimPeriod[0] + (totalNumberOfVesting *  claimPeriod[1] )) {
            user_.lastClaim += (claimPeriod[0] + (claimPeriod[1] * totalNumberOfVesting));
            claimAmount = user_.balance - user_.totalClaimed;
        } else if(user_.totalClaimed == 0) {
            require((lastClaimTimestamp ) < block.timestamp, "wait till next claim");
    ...
    function getReward(address userAddress) external view returns (uint reward) {
        ...
        if((!isSalePaused) || (releaseDate == 0) || (user_.totalClaimed > user_.balance))
            return 0;
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Private : insufficient balance");
            require(payable(owner()).send(amount), "Private : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Private : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Private : transfer failed");
        }
    }
}