...
import "./MFCCollateralLoanBase.sol";


contract MFCCollateralLoan is MFCCollateralLoanBase {
  constructor(
  ...
  ) MFCCollateralLoanBase(mfcExchangeAddress_,mfcExchangeFloorAddress_,mfcAddress_,busdAddress_,busdComptrollerAddress_,backendAdminAgents,backendAgents)
  {}

  /**
   * @dev For testing: create a loan; no funds required nor transferred.
   */
  function test_createLoan(uint256 busdAmount, uint256 loanDurationInSecs) external {
    ...
    uint256 endsAt = startsAt + loanDurationInSecs;
  ...
  /**
   * @dev For testing: force close a loan, no questions asked.
   * Same code as closeLoan() except this can close any loan,
   * active or not.
   */
  function test_forceCloseLoan(address borrower, uint256 loanId) external {
  ...
  function testEvent_ExtendLoan(address borrower, uint256 loanId, uint256 endsAt) external {
    emit ExtendLoan(borrower,loanId, endsAt);
  }

  function testEvent_closeLoan(address borrower, uint256 loanId, uint256 collateralMFCTransferred) external {
    emit CloseLoan(borrower, loanId, collateralMFCTransferred);
  }

  function testEvent_createOffer(
    address borrower,
    uint256 loanId,
    uint256 quantity,
    uint256 price,
  ...
  function testEvent_closeOffer(uint256 loanId, uint256 timestamp) external {
...
contract MFCCollateralLoanBase is BackendAgent, ExchangeCheck {
  ...
  MFCExchange internal _mfcExchange;
  MFCExchangeFloor internal _mfcExchangeFloor;
  MFCToken internal _mfc;
  IBEP20 internal _busd;
  BUSDT internal _busdt;
  address internal _mfcExchangeCapAddress;
  address internal _busdComptrollerAddress;
  address internal _deployer;
  uint256 internal _loanTermsNonce = 0;
  uint256 internal _loansNonce = 0;
  uint256 internal _totalLoanValue = 0;
  uint256 internal _maxOpenLoans = 2;
  ...
  mapping(uint256 => LoanTerm) internal _loanTerms;
  mapping(address => mapping(uint256 => Loan)) internal _loans;
  mapping(address => uint8) internal _openLoans;
  ...
  event PayLoan(address borrower, uint256 loanId, uint256 busdAmount, uint256 remainingPrincipalBUSD, uint256 collateralMFC, uint256 collateralReturned, uint256 interestPaid, uint256 paidAt);
  ...
  event TradeOffer(address borrower, uint256 loanId, address buyer, uint256 sellerQuantity, uint256 buyerQuantity, uint256 unfilledQuantity, uint256 excessBUSD, uint256 timestamp);
  ...
  function tradeOffer(address borrower, uint256 loanId, uint256 amountBUSD) external onlyValidMember(_msgSender()) onlyActiveOffer(borrower, loanId) {
    ...
    emit PayLoan(borrower, loanId, calc.effectiveBusdPaidOff, loan.remainingPrincipalBUSD, loan.collateralMFC, 0, calc.accruedInterest, loan.lastPaidAt);
  }
  ...
  function _getCollateralMFCForLoanBUSD(uint256 busdAmount) internal view returns (uint256) {
    uint256 mfcPrice = _mfcExchangeFloor.getPrice();
  ...
  function _payLoanBUSD(address borrower, uint256 loanId, uint256 busdAmount) private {
    ...
    emit PayLoan(borrower, loanId, busdAmount, loan.remainingPrincipalBUSD, loan.collateralMFC, collateralReturned, accruedInterest, loan.lastPaidAt);
  }
  ...
  function _transferToExchangeCap(uint256 amount) internal {
