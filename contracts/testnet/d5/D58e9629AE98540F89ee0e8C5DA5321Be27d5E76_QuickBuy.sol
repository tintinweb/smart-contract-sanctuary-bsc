...
contract QuickBuy is Initializable,Ownable {
    ...
    struct User {
        ...
        mapping(bytes32 => mapping(uint=>Stake)) stakes;
        uint stakeLength;
        uint checkpoint;
    }
    ...
    event Staking(bytes32 _pairHash, address user, uint pairAmount, uint buyQnt , uint price, uint priceImpact ,uint fee);
    ...
    function createPair(address _token ,address _pairtoken, uint _amount, uint _price,bool _communitySellEnable,bool _withStaking,uint _stakingShare) external  {
        ...
        quickpair[_pairHash].isStakingEnable = _withStaking;
    ...
    function _buy(bytes32 _pairHash,uint _amount,address _refferal) internal lock(_pairHash) returns(uint,uint,uint) {
        ...
        if(!pair.isStakingEnable){
            ...
            emit Staking(_pairHash, msg.sender, _amount, qnt, _price, priceImpact, fee);
        }
    ...
    function _sell(bytes32 _pairHash, uint _amount,address _refferal) internal lock(_pairHash) returns(uint,uint,uint) {
        ...
        require(quickpair[_pairHash].baseBalance<=quickpair[_pairHash].totalLPSupply,"Low Liqudity");
    ...
    function buyTokensForTokensWithRefferal(bytes32 _pairHash, uint _amount, address _refferal) external  {
        ...
        if(pair.isStakingEnable){
           stake(_pairHash, msg.sender, pair.staking_share, amtout);
        } else{
   ...
   function stake(bytes32 _hash ,address _user, uint _share ,uint _amount) internal  {
       Stake memory _stake = Stake(_hash,block.timestamp,_share,_amount); 
       users[_user].stakes[_hash][users[_user].stakeLength]= _stake;
       users[_user].stakeLength +=1;
   }

   function getUserRoi(bytes32 _hash , address _user ) public  view returns(uint) {
        uint256 totalAmount;
        User storage user_ = users[_user];
       mapping(uint=> Stake) storage _stakes  = users[_user].stakes[_hash];

        for(uint i=0; i<users[_user].stakeLength; i++) {
            uint256 finish = _stakes[i].start+365 days;
            if (user_.checkpoint < finish) {
                uint256 share = (_stakes[i].amount*_stakes[i].share)/100;
                uint256 from = _stakes[i].start > user_.checkpoint ? _stakes[i].start : user_.checkpoint;
                uint256 to = finish < block.timestamp ? finish : block.timestamp;
                if (from < to) {
                    totalAmount = totalAmount+(share*(to-from))/(365 days);
                }
            }
        }
        return totalAmount;
   ...
   function withdraw(bytes32 _hash) external {
        Pair storage _pair = quickpair[_hash];
        uint ttlRoi =  getUserRoi(_hash, msg.sender);
        users[msg.sender].checkpoint =block.timestamp;
        IERC20(_pair.baseToken).transfer(msg.sender, ttlRoi);
   }    

   function getStakeDetails(bytes32 _hash, address user, uint index) external view returns(Stake memory) {
        return users[user].stakes[_hash][index];
   }
