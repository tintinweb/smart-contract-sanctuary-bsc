...
contract Token is ERC20, PauseOwners {
	...
	uint256 private minBalanceToSellAndTransfer = 10;
	...
	function takeFees(
		...
		if (
			sender != pair &&
			tokenBalance >= minBalanceToSellAndTransfer * (10**decimals())
		) {
			...
			uint256 liqCut = (tokenBalance * liquidityFee * proportion) / 10000;
	...
	function swapAndTransferWalletFees(
		...
		uint256 ethBalance = swapTokensToETH(amountToken);
		if (ethBalance > 0) {
			uint256 proportion = (ethBalance * 100) /
				(developmentFee + marketingFee);
			uint256 devCut = (ethBalance * developmentFee * proportion) / 10000;
			uint256 marCut = (ethBalance * marketingFee * proportion) / 10000;
