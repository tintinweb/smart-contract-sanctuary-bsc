...
contract SoulSetKnight is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    function setProductMap(uint id_, bool isOpen_, address addr_, uint volume_) public onlyOwner {
        productMap[id_].isOpen = isOpen_;
        productMap[id_].tokenAddr = addr_;
        productMap[id_].volume = volume_;
    }

    /*
     *  用户设置骑士
     **/
    function setTeamsFrom(address communityAddr_, uint productId_) public{
        //查询用户soul
        uint amount = dataAddr.soulToken.balanceOf(_msgSender());
        uint day = block.timestamp/864000;
        if(communityAddr_ == address(0)){
            if(amount > 0){
                require(productMap[productId_].isOpen, "Project not open");
                //soul大于0，可以领取
                require(userReceiveDate[productId_][_msgSender()] != day, "Time not reached");
                IERC20Upgradeable(productMap[productId_].tokenAddr).transfer(_msgSender(), productMap[productId_].volume * 1 ether);
