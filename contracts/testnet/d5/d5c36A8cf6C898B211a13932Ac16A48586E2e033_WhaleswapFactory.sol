...
contract WhaleswapFactory is IWhaleswapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(WhaleswapPair).creationCode));
...
contract WhaleswapPair is IWhaleswapPair, WhaleswapERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        if (data.length > 0) IWhaleswapCallee(to).uniswapV2Call(msg.sender, amount0Out, amount1Out, data);
        balance0 = IERC20(_token0).balanceOf(address(this));
        balance1 = IERC20(_token1).balanceOf(address(this));
        }
