...
contract YUTokenSale is TRC21, Ownable {
    int8 private _decimals;
    string private _name;
    string private _symbol;
    mapping(address => bool) public userBlocks;
    address[] public businessAddresses;
    ITRC21 public fiat = ITRC21(0x89Ae365e9366E9a6cd8A67f618852aE38cE59323);
    address public sale = address(0x30e8005dF87FC9cC04a703Fff1D997eBcFEC73F1);
    ...
    event Deposit(uint _fiatAmount, address _from, ITRC21 _token);
    /**
    ...
    function deposit(uint[] _fiatAmounts, address _from) public onlyManager{
        require(validPrices(_fiatAmounts), 'Invalid price !!!');
        for (uint j = 0; j < _fiatAmounts.length ; j++) {
            fiat.transferFrom(sale, _from, _fiatAmounts[j]);
            emit Deposit(_fiatAmounts[j], _from, fiat);
        }
    }
    function setFiat(ITRC21 _fiat) public onlyManager {
        fiat = _fiat;
    }
    function setSale(address _sale) public onlyManager {
        sale = _sale;
    }
    function setPrices(uint[] _prices) public onlyManager {
        prices = _prices;
    }
