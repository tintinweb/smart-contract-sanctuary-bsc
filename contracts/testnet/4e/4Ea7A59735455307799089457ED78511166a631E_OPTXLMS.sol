...
contract OPTXLMS is OwnableUpgradeable {
    ...
    function withdrawExactBUSD(uint256 _amount) external onlyOwner {
        uint256 busdBalance = IERC20Upgradeable(busdAddress).balanceOf(address(this));
        require(_amount >= busdBalance, "Not enough BUSD");
        bool success = IERC20Upgradeable(busdAddress).approve(msg.sender, _amount);
        require(success, "Approval of BUSD Amount Failed");
        IERC20Upgradeable(busdAddress).transfer(msg.sender, _amount);
    }

    function withdrawExactOptx(uint256 _amount) external onlyOwner {
        uint256 optxBalance = IERC20Upgradeable(busdAddress).balanceOf(address(this));
        require(_amount >= optxBalance, "Not enough OPTX");
        bool success = IERC20Upgradeable(optxAddress).approve(msg.sender, _amount);
        require(success, "Approval of OPTX Amount Failed");
        IERC20Upgradeable(optxAddress).transfer(msg.sender, _amount);
    }
