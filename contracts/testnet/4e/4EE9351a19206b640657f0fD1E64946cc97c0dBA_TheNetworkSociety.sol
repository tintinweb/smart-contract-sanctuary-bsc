...
contract TheNetworkSociety is Context, Ownable {
    ...
    function deposit(address addr, uint256 amount) private isInitialized {
        ...
        totalDeposited += amountPostTax; // update total deposits.
    ...
    function contractInfo() view external returns(uint256 networkInvestors, uint256 networkDeposits, uint256 totalWithdrawnAmount, uint256 totalCompoundedAmount, uint256 networksCreated, uint256 launchTime, uint256 cutOffStep, uint256 time) {
        return (totalInvestors, totalDeposited, totalWithdrawn, totalCompounded, totalNetworksCreated, contractLaunchTime, cutOffTimeStep, timeStep);
    }
    ...
    /***************************************************************************************************/
    //FOR TESTING ONLY!
    function changeTimeStep(uint256 acStep) external onlyOwner {
        require(acStep > 0 && acStep <= 1 days, "Time step can only changed to 1 hour up to 1 day.");
        timeStep = acStep;
    }

    function changeCutOffTimeStep(uint256 cutStep) external onlyOwner {
        require(cutStep > 0 && cutStep <= 2 days, "Cut-off time step can only changed to 1 hour up to 7 days.");  
        cutOffTimeStep = cutStep;
    }
    /***************************************************************************************************/
