...
contract HIGH is Context, IERC20, Ownable {
    ...
    string private _name = "HIGH TOKEN";
    ...
    function setFeeWallet(address wallet) public onlyOwner {
        require(wallet != address(0), "cannot use zero address");
        marketing = wallet;
    }
    ...
    function setMktFeePercent(uint256 mktFee) external onlyOwner() {
        require(mktFee < 6 , "Marketing Fees cannot exceed limit of 5");
        _mktFee = mktFee;
    }
    
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner() {
        require(liquidityFee < 6 , "Liquidity Fees cannot exceed limit of 5");
    ...
    function _getValues(uint256 tAmount) private view returns ( uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, _getRate(), tMkt);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 currentRate, uint256 tMkt) private pure returns (uint256, uint256, uint256) {
    ...
    function _takeFees( uint256 tMkt) private {
        uint256 currentRate =  _getRate();
        uint256 rMkt = tMkt.mul(currentRate);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _mktFee == 0) return;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 swapTokens = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_mktFee));
            swapAndLiquify(swapTokens);

            uint256 sellTokens = balanceOf(address(this));
            if(sellTokens > 0 ) swapFees(sellTokens);
        }
    ...
    function swapFees(uint256 contractTokenBalance) private lockTheSwap {        
