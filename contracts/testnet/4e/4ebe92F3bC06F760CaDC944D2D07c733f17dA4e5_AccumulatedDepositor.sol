...
contract AccumulatedDepositor is LogicUpgradeable {
     ...
     */
    function setStargateReserveGas(
        uint256 _stargateReserveGas
    ) external onlyOwner {
...
contract AccumulatedDepositorOld is LogicUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    event ReceivedOnDestination(
        ...
        address accountAddress,
        uint8 successFlag
    ); // succesFlag : 0 - success, 1 - dstGasForCall is below reservedGas, 2 - error on depositOnBehalf, 3 - force transfer by owner
    ...
    function sgReceive(
        ...
        // If msg.sender is owner, force send token to storage
        if (msg.sender == owner()) {
            require(
                IERC20Upgradeable(_token).balanceOf(address(this)) >= amountLD,
                "AD6"
            );
            IStorage(storageContract).depositOnBehalf(
                amountLD,
            ...
            emit ReceivedOnDestination(_token, amountLD, accountAddress, 3);
            return;
        }
