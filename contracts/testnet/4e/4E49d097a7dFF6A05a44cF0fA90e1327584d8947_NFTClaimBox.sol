...
contract NFTClaimBox is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public limitDoges;
    mapping(uint256 => uint256) public limitRares;
    uint256 public FEE_OPEN = 0.001 ether;
    ...
    function setLimitRare(uint256 rare, uint256 limit) public onlyOwner {
        limitRares[rare] = limit;
    }

    function setFeeOpen(uint256 _fee) public onlyOwner {
        FEE_OPEN = _fee;
    }
    ...
    function openBox(uint256 boxId, uint256 nftCommonId)
        ...
        payable
    {
        require(msg.value == FEE_OPEN, "required fee open");
        ...
        require(
            keccak256(abi.encodePacked(nftCore.getNFT(nftCommonId).class)) ==
        ...
        require(limitDoges[nftCommonId] <= limitRares[nftCore.getNFT(nftCommonId).rare], "over limit");
        limitDoges[nftCommonId] = limitDoges[nftCommonId].add(1);
        ...
        if (typeBox == 0) {
            if (rare == 3) {
                ...
                if (rareRand > 50 && rareRand <= 200) {
                    rareNftDoge = 2;
                }
                if (rareRand > 200 && rareRand <= 500) {
                    rareNftDoge = 1;
                }
                if (rareRand > 500) {
                    rareNftDoge = 0;
        ...
        if (typeBox == 1) {
            if (rare == 4) {
                ...
                if (rareRand > 50 && rareRand <= 100) {
                    rareNftDoge = 3;
                }
                if (rareRand > 100 && rareRand <= 200) {
                    rareNftDoge = 2;
                }
                if (rareRand > 200 && rareRand <= 500) {
                    rareNftDoge = 1;
        ...
        emit OpenBox(tokenId, _msgSender(), boxId, typeBox, nftCommonId);
    }
