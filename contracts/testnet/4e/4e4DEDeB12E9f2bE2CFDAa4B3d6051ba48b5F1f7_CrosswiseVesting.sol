...
contract CrossLPFarm is Ownable, ReentrancyGuard, ICRSSEmission {
    ...
    struct UserInfo {
        ...
        bool isCompounding; //user setting which continously gives users rewards in lpToken instead of CRSS
    ...
    function switchCollectOption(uint256 _pid) public {
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (pool.endBlock <= block.number && pool.endBlock != 0) {
            return;
        }
        updatePool(_pid);
...
contract CrosswiseVesting is ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => UserWithdrawn)) public userWithdrawn;
    ...
    function claimAllRewards(uint256 _pid) public nonReentrant {
        uint256 totalCrss;
        uint256 totalLp;
        ...
        //should spend close to half gas

        if (lpToken != address(0)) {
            userWithdrawn[msg.sender][_pid].lpTokensWithdrawn += uint128(
                totalLp
            );
            userVesting[msg.sender][_pid].lpTokensVesting -= uint128(totalLp);
            IERC20(lpToken).transfer(msg.sender, totalLp);
            // totalVesting[crssAddress] -= totalCrss;
        }
        if (totalCrss > 0) {
            userVesting[msg.sender][_pid].crssVesting -= uint128(totalCrss);
            userWithdrawn[msg.sender][_pid].crssWithdrawn += uint128(totalCrss);
        ...
        /* userVesting[msg.sender][vestingInstance.tokenAddress] -= adjustedReward;
        totalVesting[vestingInstance.tokenAddress] -= adjustedReward;
        userWithdrawn[msg.sender][
            vestingInstance.tokenAddress
        ] += adjustedReward;*/
    }
    ...
    function initiateFarmVestingInstance(
        ...
        UserVesting memory userVest = getUserVesting(msg.sender, _pid);
        if (_tokenAddress == crssAddress) {
            //  totalCrssVesting += _amount;
            emit CrossFarmVest(msg.sender, _amount, _vestingPeriod);
            address referrer = ICrssReferral(referralContract).getReferrer(
                _address
            );
            if (referrer != address(0)) {
        ...
        //userVesting[msg.sender][_pid] = UserVesting({crssVesting: userVest.crssVesting +  })
        //totalVesting[_tokenAddress] += _amount;
    ...
    function getUserEarned(address _user, uint256 _pid)
        public
        view
        returns (UserWithdrawn memory)
    {
        return userWithdrawn[_user][_pid];
    }
    ...
    function claimRewards(uint256 _pid, uint256 _id) public nonReentrant {
        VestingInstance storage vestingInstance = vestingInstances[msg.sender][
            _pid
        ][_id];
        ...
        if (vestingInstance.tokenAddress != crssAddress) {
            ...
            userWithdrawn[msg.sender][_pid].crssWithdrawn += uint128(
                adjustedReward
            );
            address referrer = ICrssReferral(referralContract).getReferrer(
                msg.sender
