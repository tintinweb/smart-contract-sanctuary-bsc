...
library TransferHelper {
  ...
  function _safeTransferETH(address to_, uint value) internal {
    (bool success, bytes memory data) = to_.call(
      abi.encodeWithSelector(bytes4(keccak256(bytes('transfer(uint256)'))), value)
    );
    require(success && (data.length == 0 || abi.decode(data, (bool))));
  }
}
        ...
        uint(
          keccak256(
            abi.encodePacked(
              hex'ff',
              factory,
              keccak256(abi.encode(token0, token1, token2)),
              hex'98bde01f39dd5257f28e2431d4d8bb1b4047b11458ce1498cdf8b47eee3db410'
            )
          )
        )
      )
