...
contract Mail is OwnableUpgradeable{
    ...
    event BondRelationship(address indexed player1,address indexed player2,uint indexed types);
    ...
    function bond(address[2] memory addr,uint types, bytes32 r, bytes32 s, uint8 v) public {
         ...
         emit BondRelationship(addr[0],addr[1],types);
     }
    
    function claimMail(uint8[] memory rewardType, uint8[] memory rewardId, uint[] memory rewardAmount, uint id, bytes32 r, bytes32 s, uint8 v) public {
         ...
         bytes32 hash = keccak256(abi.encodePacked(hash1, hash2, hash3));
         address a = ecrecover(hash, v, r, s);
         require(a == banker, "not banker");
         ...
         for (uint i = 0; i < rewardType.length; i ++){
             ...
             _processMail(rewardType[i],rewardId[i],rewardAmount[i]);
         }
         emit ClaimMail(msg.sender,id);
     }
     
     function _processMail(uint types,uint rewardId,uint amount) internal {
         if(types == 1){
             if(rewardId == 1){
