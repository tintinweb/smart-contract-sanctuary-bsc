...
abstract contract ERC721URIStorage is ERC721Enumerable{
    ...
    function open(string memory url, uint lastTokenId) external onlyOwner {
        ...
        require(0 < lastTokenId && lastOpenTokenId < lastTokenId && lastTokenId <= totalSupply(), 'Invalid tokenid');
...
contract YGMint is ERC721URIStorage{
    ...
    uint mintEveryAmount = 12 * 10 ** 15;
    uint firstLeave = 1 * 10 ** 15;
    uint secondtLeave = 5 * 10 ** 14;
    ...
    constructor() ERC721("TestEthMintNft", "EthMintNft") {

    }

    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://QmaXBheDZqCz2Zh1HLDS1n4g3nC4fXY9n8n9wuHn7MY8QV";
    }
    ...
    function safeMint(address payable _recommender, uint mintCount) external payable checkRecommender(_recommender){

        require(isStart, 'Not started');

        require(totalSupply() + mintCount <= mintTotal, 'Maximum mint quantity exceeded');

        address minter = _msgSender();
        require(0 < mintCount && ((balanceOf(minter) + mintCount) <= mintMax), 'Invalid quantity');
    ...
    modifier checkRecommender(address payable _recommender){
        ...
        if(recommender[msg.sender] == address(0)){
            ...
            require(isMint[_recommender] || 0 < balanceOf(_recommender), "Recommender no mint");
