...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
    ...
    function swapAndLiquify(uint256 PartnerId) private lockTheSwap {
        ...
        uint256 SoldAmount = item.PartnerValues._totalTokenSold.mul(item.PartnerValues._launchPriceRate);
        uint256 liquidPercent = SoldAmount.mul(item.PartnerStatus._liquidityPercentage).div(100);
        uint256 tokenAmount = liquidPercent.div(item.PartnerValues._listingPriceRate);
        ...
        for (uint256 i = 0; i < item._paymentType.length; i++) {
            ...
            if (amount > 0) {
                ...
                if (!_compareString(item._paymentType[i], "BNB")) {
                    ...
                    uint256 dexBalance = ContractAdd.balanceOf(address(this));
                    if(balance <= dexBalance){
                        swapTokensForBNB(balance, payAddress); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered;
        ...
        uint256 tokensReceived = (item.Balance["BNB"].amount)
            .mul(item.PartnerStatus._liquidityPercentage).div(100);
    ...
    function swapTokensForBNB(uint256 tokenAmount, address contractAdd_) private {
