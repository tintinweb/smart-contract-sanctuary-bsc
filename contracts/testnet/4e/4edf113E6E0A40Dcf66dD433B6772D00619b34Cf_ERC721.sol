...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata ,Manage {
    ...
    event Mint(address indexed from, address indexed to, uint256 indexed tokenId, uint256 tokenType);
    /**
     ...
     * Emits a {Transfer} event.
     */
    function mint(address to, uint256 tokenType) public virtual onlyOwner {
        require(to != address(0), "ERC721: mint to the zero address");
        uint256 tokenId=_parseInt(Strings.toString(++_tokenId));
        ...
        emit Mint(address(0), to, tokenId, tokenType);

        _afterTokenTransfer(address(0), to, tokenId);
    }
    
    function mintList(address to, uint256 tokenType,uint256 number) public onlyOwner {
    ...
    function _parseInt(string memory _a) internal pure returns (uint _parsedInt) {
        bytes memory bresult = bytes(_a);
        bool decimals = false;
        for (uint i = 0; i < bresult.length; i++) {
            if ((uint(uint8(bresult[i])) >= 48) && (uint(uint8(bresult[i])) <= 57)) {
                _parsedInt *= 10;
                _parsedInt += uint(uint8(bresult[i])) - 48;
