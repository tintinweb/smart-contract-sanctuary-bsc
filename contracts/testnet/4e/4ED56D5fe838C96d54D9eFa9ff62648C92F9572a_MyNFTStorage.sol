...
contract MyNFTStorage is ERC1155Holder, ReentrancyGuard {
    ...
    function cancelOrder(uint orderId) public {
        ...
        emit CancelOrder(
            myOrder.uuidTransaction
        );
    }

    function buyOrder(uint orderId) payable public nonReentrant {
        ...
        uint256 fee = SafeMath.div(SafeMath.mul(orderPrice, order.feePercent), 10000);
        ...
        if (order.tokenPayment == address(0x0)) {
            ...
            payment = IERC20(order.tokenPayment);
        ...
        if (!isNativePayment) {
            ...
            if (fee > 0)
                payable(address(admin.getFeeAccount())).transfer(fee);
        }
        ...
        for (uint i = 0; i < royalties.length; i++) {
            if (royalties[i].account == address(0x0))
                continue;
            if (royalties[i].value <= 0)
                continue;
            if (royalties[i].account == msg.sender)
                continue;

            uint256 shared = SafeMath.div(SafeMath.mul(realOrderPrice, royalties[i].value), 10000);
        ...
        if (!isNativePayment)
        {
            ...
            payable(address(order.owner)).transfer(SafeMath.sub(realOrderPrice, royaltyShared));
        }
        ...
        emit BuyOrder(order.uuidTransaction);
    }
