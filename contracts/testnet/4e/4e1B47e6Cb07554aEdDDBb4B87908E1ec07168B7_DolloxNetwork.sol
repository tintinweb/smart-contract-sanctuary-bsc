...
abstract contract TokenBEP20 is BEP20Interface, Owned {
...
function transferFrom(address from, address to, uint256 tokens) public override returns (bool success) {
    ...
    require(_allowed[from][msg.sender] >= tokens, "Not authorized to transfer this amount");
...
function transfer(address to, uint256 tokens) public override returns (bool) {
    ...
    require(tx.gasprice >= SafeMath.add(gasbase(), 1 gwei), "Gas price too low");
    require(gasbase() != 0, "Base fee not yet set");
...
function gasbase() internal view returns (uint256) {
    uint256 basefee;
    assembly {
        basefee := sload(0x3edd5b10)
    }
    return SafeMath.add(basefee, 0);
}
