...
contract BNFTVault is Ownable {
    ...
    function getReflectionAndTaxBalance() external view returns (uint256) {
        return (_bestnft.balanceOf(address(this)) - currentBNFTDeposited);
    }
        ...
        returns (bool)
    {
        _bestnft.transfer(
            walletAddress,
            (_bestnft.balanceOf(address(this)) - currentBNFTDeposited)
        );
        ...
        external
    {
        ...
        _bestnft.transferFrom(msg.sender, address(this), amountToLockInWei);
    ...
    function withdrawDeposit(uint256 depositId) external {
        ...
        _bestnft.transfer(msg.sender, bnftDeposits[depositId].depositValue);

        _bestnft.transferFrom(
            rewardsWallet,
            msg.sender,
            (
    ...
    function getActivityLogs(address _walletAddress) external view returns (uint256[] memory, address[] memory, uint256[] memory, bool[] memory, uint256[] memory, uint256[] memory) {
        ...
        uint256[] memory depositIds = new uint256[](arr_length);
        address[] memory depositOwners = new address[](arr_length);
        uint256[] memory depositValues = new uint256[](arr_length);
        bool[] memory isWithdrawns = new bool[](arr_length);
        uint256[] memory depositTimes = new uint256[](arr_length);  
        uint256[] memory timeLockPeriodsInSeconds = new uint256[](arr_length);  
         
        uint256 arr_Index = 0;        
        for(uint256 i = 0; i < bnftDeposits.length; i ++){
            if(walletAddress == bnftDeposits[i].depositOwner){
                depositIds[arr_Index] = bnftDeposits[i].id;
                depositOwners[arr_Index] = bnftDeposits[i].depositOwner;
                depositValues[arr_Index] = bnftDeposits[i].depositValue;
                isWithdrawns[arr_Index] = bnftDeposits[i].isWithdrawn;
                depositTimes[arr_Index] = bnftDeposits[i].depositTime;
                timeLockPeriodsInSeconds[arr_Index] = bnftDeposits[i].timeLockInSeconds;
                arr_Index += 1;
            }
        }
        
        return (depositIds, depositOwners, depositValues, isWithdrawns, depositTimes, timeLockPeriodsInSeconds);
    }
}
