...
contract ERC20VestingToken is Ownable {
    ...
    mapping(address => uint256) private _hasBalances;
    ...
    function withdraw(uint256 pid, uint256 amount) public payable {
        ...
        _allBalances[msg.sender] = _allBalances[msg.sender].add(amount);
    ...
    function canWithDraw() external {
        ...
        if (_lockBalances[msg.sender] >= amount) {
            _lockBalances[msg.sender] = _lockBalances[msg.sender].sub(amount);

            _hasBalances[msg.sender] = _hasBalances[msg.sender].add(amount);

            _token.safeTransfer(msg.sender, amount);
        } else {
            _lockBalances[msg.sender] = 0;

            _hasBalances[msg.sender] = _hasBalances[msg.sender].add(_lockBalances[msg.sender]);

            _token.safeTransfer(msg.sender, _lockBalances[msg.sender]);
        }

        emit CanWithDrawn(msg.sender, amount);
    }

    function vestedTokenForRole(address account) public view returns(uint){
        return ((block.timestamp - _cliffBalances[account]) / _cliff)* _canBalances[account]-_hasBalances[account];
    }
    ...
    function getAllBalanceOf(address account) public view returns (uint256) {
        return _allBalances[account];
    }

    function getHasBalanceOf(address account) public view returns (uint256) {
        return _hasBalances[account];
    }
