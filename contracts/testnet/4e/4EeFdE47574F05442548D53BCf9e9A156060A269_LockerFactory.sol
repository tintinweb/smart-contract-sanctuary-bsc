...
contract LockerFactory is Ownable {
    ...
    function createLocker(
        ...
        Locker(locker).initialize(lockType, param1, param2, param3, token, uri);
...
contract Locker is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        emit LockerInitialized(lockType, _param1, _param2, _param3, address(token), uri);
    }
    ...
    function startProposal(uint256 _unlockPercent, uint256 _endTime) external onlyOwner {
        ...
        proposal.totalPower = totalDeposited - _getUnlockedAmount(totalDeposited);
    ...
    function getClaimableAmount(address user) public view returns (uint256) {
        return _getUnlockedAmount(userInfo[user].amount) - userInfo[user].claimedAmount;
    }

    function getVotingPower(address user) public view returns (uint256) {
        return userInfo[user].amount - _getUnlockedAmount(userInfo[user].amount);
    }

    /* internal functions */

    function _getUnlockedAmount(uint256 amount) internal view returns (uint256) {
