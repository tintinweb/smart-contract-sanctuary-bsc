...
contract MarketPlace is IMarketPlace, Ownable, ReentrancyGuard, Pausable {
    using SafeBEP20 for IBEP20;

    ERC721 private _erc721;

    MarketStorage private _marketStorage;

    uint256 private constant MAX_INT_TYPE = type(uint256).max;

    constructor(ERC721 erc721, MarketStorage marketStorage) {
        _erc721 = erc721;
        _marketStorage = marketStorage;
    }

    function setTokenAddress(ERC721 newERC721) external onlyOwner {
        require(
            address(newERC721) != address(0),
            "MarketPlace: setTokenAddress query for the zero address"
        );
        _erc721 = newERC721;
    }

    function setMarketStorage(MarketStorage newMarketStorage)
        external
        onlyOwner
    {
        require(
            address(newMarketStorage) != address(0),
            "MarketPlace: setMarketStorage query for the zero address"
        );
    ...
    function listing(
        ...
        _listing(tokenId, currency, price);
    ...
    function batchListing(
        ...
        require(
            tokenIds.length == currencies.length,
            "MarketPlace: tokenIds and currencies length mismatch"
        );
        require(
            tokenIds.length == prices.length,
            "MarketPlace: tokenIds and prices length mismatch"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _listing(tokenIds[i], currencies[i], prices[i]);
        }
        emit BatchListing(
            _msgSender(),
            tokenIds,
    ...
    function unListing(uint256 tokenId) public whenNotPaused {
        _unListing(tokenId);
        emit UnListing(_msgSender(), tokenId, block.timestamp);
    }

    function batchUnListing(uint256[] memory tokenIds) public whenNotPaused {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _unListing(tokenIds[i]);
        }
        emit BatchUnListing(_msgSender(), tokenIds, block.timestamp);
    }
        ...
        whenNotPaused
    {
        ...
        address currency;
        uint256 price;
        (owner, currency, price) = _marketStorage.getItem(tokenId);
        require(
            _erc721.ownerOf(tokenId) == _msgSender(),
            "MarketPlace: updatePrice caller is not owner"
        );
        require(
            owner != address(0),
            "MarketPlace: updatePrice query for none listing token"
        );
        _marketStorage.updateItem(tokenId, owner, currency, newPrice);
        emit PriceUpdate(
            _msgSender(),
            tokenId,
            price,
    ...
    function batchUpdatePrice(
        ...
        require(
            tokenIds.length == newPrices.length,
            "MarketPlace: tokenIds and newPrices length mismatch"
        );
        for (uint256 i = 0; i < tokenIds.length; i++) {
            updatePrice(tokenIds[i], newPrices[i]);
        }
    }
    ...
    function buy(uint256 tokenId) external payable whenNotPaused nonReentrant {
        ...
        (owner, currency, price) = _marketStorage.getItem(tokenId);
        require(
            _erc721.exists(tokenId),
            "MarketPlace: buy query for nonexistent token"
        );
        require(
            owner != address(0),
            "MarketPlace: buy query for none listing token"
        );
        require(owner != _msgSender(), "MarketPlace: buy caller is owner");
        address feeOwner;
        uint256 commission;
        uint256 fee;
        (fee, commission, feeOwner) = _marketStorage.getFeeMarket();
        address newOwner = _msgSender();
        uint256 feeAmount = (fee * price) / commission;
        ...
        _erc721.safeTransferFromLockedToken(owner, _msgSender(), tokenId);
        emit Purchase(
            owner,
            newOwner,
            tokenId,
    ...
    function _listing(
        ...
        require(
            _erc721.ownerOf(tokenId) == _msgSender(),
            "MarketPlace: listing caller is not owner"
        );
        require(0 <= price, "MarketPlace: price mismatch");
        address owner;
        (owner, , ) = _marketStorage.getItem(tokenId);
        require(
            owner == address(0),
            "MarketPlace: listing query for already listing token"
        );
        require(
            !_erc721.isLocked(tokenId),
            "MarketPlace: listing query for locked token"
        );
        _marketStorage.addItem(tokenId, _msgSender(), currency, price);
        _erc721.lock(tokenId, MAX_INT_TYPE);
    }

    function _unListing(uint256 tokenId) private {
        address owner;
        (owner, , ) = _marketStorage.getItem(tokenId);
        require(
            owner != address(0),
            "MarketPlace: unListing query for none listing token"
        );
        require(
            owner == _msgSender(),
            "MarketPlace: unListing caller is not owner"
        );
        _marketStorage.deleteItem(tokenId);
...
interface IMarketPlace {
    ...
    event UnListing(address indexed owner, uint256 tokenId, uint256 time);
    event BatchUnListing(
        address indexed owner,
        uint256[] tokenId,
        uint256 time
    );
    ...
    function batchUnListing(uint256[] memory tokenIds) external;
