...
contract Cryptopump is ContextUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address _ownerAddress) public initializer {
        ...
        LEVEL_PERIOD[1] = 30 minutes;

        LEVEL_PRICE[2] = 0.000002 ether;
        LEVEL_PERIOD[2] = 30 minutes;

        LEVEL_PRICE[3] = 0.000003 ether;
        LEVEL_PERIOD[3] = 30 minutes;

        LEVEL_PRICE[4] = 0.000004 ether;
        LEVEL_PERIOD[4] = 30 minutes;

        LEVEL_PRICE[5] = 0.000005 ether;
        LEVEL_PERIOD[5] = 30 minutes;
    ...
    function regUser(uint256 _referrerID) external payable {
        ...
        users[msg.sender].levelExpired[1] = block.timestamp.add(LEVEL_PERIOD[1]);
    ...
    function buyLevel(uint256 _level) external payable {
        ...
        if (_level == 1) {
            require(msg.value == LEVEL_PRICE[1], "Incorrect Value");
            users[msg.sender].levelExpired[1] = users[msg.sender].levelExpired[1].add(LEVEL_PERIOD[1]);
            ...
            if (users[msg.sender].levelExpired[_level] == 0)
                users[msg.sender].levelExpired[_level] = block.timestamp + LEVEL_PERIOD[_level];
            else users[msg.sender].levelExpired[_level] += LEVEL_PERIOD[_level];
        }
        loopCheck[msg.sender] = 0;
    ...
    */
    function updatePeriod(uint256 _level, uint256 _period) public returns(bool) {
        require(msg.sender == ownerAddress, "only OwnerWallet");
