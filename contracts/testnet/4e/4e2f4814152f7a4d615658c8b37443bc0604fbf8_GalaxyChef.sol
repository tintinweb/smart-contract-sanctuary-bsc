...
contract GalaxyChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 fixedApr;
        uint256 tokenType; // 0 for glf token, 1 for other token, 2 for stable lp token, 3 for other lp
        address token1BusdLPaddress;
        uint256 directCommission; //commission pay direct for the Leader;
    }

    // The WEDEX TOKEN!
    GalaxyFinanceToken public glf;
    // Dev address.
    ...
    constructor(
        GalaxyFinanceToken _glf,
        uint256 _startBlock,
        address _glfLPAddress
    ) public {
        glf = _glf;
        startBlock = _startBlock;
        glfLPAddress = _glfLPAddress;
    ...
    function getGlfPrice() public view returns (uint256) {
        // glfLPAddress
        return IBEP20(busdAddress).balanceOf(glfLPAddress).mul(1e2).div(glf.balanceOf(glfLPAddress));
    }
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            uint256 WedexReward = multiplier.mul(pool.fixedApr).mul(getLPPrice(_pid)).mul(1e12).div(getGlfPrice()).div(BLOCKS_PER_YEAR.mul(100));
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 wedexReward = multiplier.mul(pool.fixedApr).mul(getLPPrice(_pid)).mul(pool.totalAmount).mul(1e12).div(BLOCKS_PER_YEAR.mul(100)).div(getGlfPrice());

        glf.mint(devAddress, wedexReward.div(1e12).div(10));
        glf.mint(address(this), wedexReward.div(1e12));
    ...
    function payDirectCommission(uint256 _pid,uint256 _amount, address referrer) internal {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        uint256 directCommissionAmount = _amount.mul(lpPrice).mul(pool.directCommission).div(getGlfPrice());
        safeWedexTransfer(referrer,directCommissionAmount);
    ...
    function safeWedexTransfer(address _to, uint256 _amount) internal {
        uint256 wedexBal = glf.balanceOf(address(this));
        if (_amount > wedexBal) {
            glf.transfer(_to, wedexBal);
        } else {
            glf.transfer(_to, _amount);
        }
    }
    ...
    function setGlfLpAddress (address _glfLPAddress) public onlyOwner {
        glfLPAddress = _glfLPAddress;
    }
        ...
        if (depth < referDepth) {
            if (address(glxReferral) != address(0)) {
                ...
                if (commissionAmount > 0 && _referrer!=address(0)) {
                    glf.mint(_referrer, commissionAmount);
