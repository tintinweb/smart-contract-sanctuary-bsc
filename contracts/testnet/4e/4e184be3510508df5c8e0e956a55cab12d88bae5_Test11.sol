...
contract Test11 is ERC20, Ownable {
    ...
    Test11DividendTracker public dividendTracker;
    ...
    constructor() ERC20("Test11", "$FD") {
        ...
        dividendTracker = new Test11DividendTracker(
            payable(this),
            router,
    ...
    function updateFees(
        ...
        require(
            totalSellFees <= 10 && totalBuyFees <= 10,
            "total fees cannot exceed 10% sell or buy"
        );
    ...
    function _transfer(
        ...
        if (amount == 0) {
            ...
            uint256 totalFees = RewardsFee.add(
                liquidityFee + marketingFees + teamFees
            );
...
contract Test11DividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        Test11Contract = Test11(mainContract);
    ...
    function _transfer(
        ...
        require(false, "Test11_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "Test11_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Test11 contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(Test11Contract);

            uint256 prevBalance = Test11Contract.balanceOf(address(this));
            ...
            {
                uint256 received = Test11Contract.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    Test11Contract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
