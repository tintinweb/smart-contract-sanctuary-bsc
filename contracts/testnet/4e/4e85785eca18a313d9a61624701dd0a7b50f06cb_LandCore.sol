...
// File: contracts\token\ERC721\ERC721URIStorage.sol
...
contract LandStorage{
    ...
    struct LandTypeInfo{
        uint8 landType;
        uint256 landPrice;
        uint256 landTotalNumber;
        uint256 landNumber;
    }

    // LandService
    // Num => LandTypeInfo
    mapping (uint8 => LandTypeInfo) public LandTypeInfos;
    uint[] public LandTokenIds;
}
...
// File: contracts\Data\DataControl.sol


pragma solidity ^0.8.9;
abstract contract DataControl is ILandCore{
  event SetDataAdmin(address sender, address newAddr);

  address public dataAdminAddress;

  constructor() {
    dataAdminAddress = msg.sender;
  }
  modifier onlyDataAdmin() {
    require(msg.sender == dataAdminAddress, "sender is not matched");
    _;
  }

  function setDataAdmin(address _newAddress) external onlyDataAdmin {
    require(_newAddress != address(0));
    dataAdminAddress = _newAddress;
    emit SetDataAdmin(msg.sender, _newAddress);
  }

  function getDataAdmin() external view override returns(address){
...
pragma solidity ^0.8.9;
abstract contract LandService is LandStorage, DataControl{
    function setLandTypeInfos(LandTypeInfo[] memory _landTypeInfos) internal onlyDataAdmin {
        for (uint i = 0; i < _landTypeInfos.length; i++) {
            LandTypeInfo memory _landTypeInfo = _landTypeInfos[i];
            LandTypeInfos[_landTypeInfo.landType] = LandTypeInfo(_landTypeInfo.landType, _landTypeInfo.landPrice, _landTypeInfo.landTotalNumber, _landTypeInfo.landNumber);
            emit setLTConfig(
                _landTypeInfo.landType,
                _landTypeInfo.landPrice,
                _landTypeInfo.landTotalNumber,
                _landTypeInfo.landNumber
            );
        }
    }

    function updateLandTypeInfos(LandTypeInfo[] memory _landTypeInfos) public onlyDataAdmin {
        uint allSetNum = 0;
        for(uint i = 0; i < _landTypeInfos.length; i++) {
            LandTypeInfo memory _landTypeInfo = _landTypeInfos[i];
            allSetNum += _landTypeInfo.landTotalNumber;
        }

        require(allSetNum <= landNum, "The total amount of land cannot be greater than setting");

        for (uint j = 0; j < _landTypeInfos.length; j++) {
            LandTypeInfo memory _landTypeInfo = _landTypeInfos[j];
            LandTypeInfo memory landTypeInfo = LandTypeInfos[_landTypeInfo.landType];
            if(landTypeInfo.landType == _landTypeInfo.landType) {
                require(_landTypeInfo.landTotalNumber >= landTypeInfo.landTotalNumber, "Single type land total num cannot be less than the previous setting");
                LandTypeInfos[_landTypeInfo.landType] = LandTypeInfo(landTypeInfo.landType, _landTypeInfo.landPrice, _landTypeInfo.landTotalNumber, landTypeInfo.landNumber);
            } else {
                LandTypeInfos[_landTypeInfo.landType] = LandTypeInfo(_landTypeInfo.landType, _landTypeInfo.landPrice, _landTypeInfo.landTotalNumber, 0);
            }
            emit updateLTConfig(
                landTypeInfo.landType,
                _landTypeInfo.landPrice,
                _landTypeInfo.landTotalNumber,
                landTypeInfo.landNumber
            );
        }
    }

    function getLandTypeInfos(uint8[] memory _landTypes) public view returns(LandTypeInfo[] memory) {
        LandTypeInfo[] memory _lt = new LandTypeInfo[](_landTypes.length);
        uint8 j = 0;
        for (uint8 i = 0; i< _landTypes.length; i++) {
            _lt[j]= LandTypeInfos[_landTypes[i]];
            j++;
        }
        return _lt;
    }
...
abstract contract LandOwnerShip is ERC721URIStorage, LandService{
    ...
    function _mint(address to, uint256 tokenId) internal override {
        require(tokenId <= landNum,'More than land total amount');
        super._mint(to, tokenId);
    }

    function getOffShopLand(address from, uint _landId) internal returns(address _seller, uint _timestamp){
        ILandMarket landMarketInter = ILandMarket(fyFarmDep.landMarketAddress());
...
abstract contract LandFactory is LandOwnerShip{
    ...
    function _createLand(address _to, uint _newItemId) internal {
        _safeMint(_to, _newItemId);
        _setTokenURI(_newItemId, _newItemId.toString());
        LandTokenIds.push(_newItemId);
    }
    ...
    function getLandTokenIds() external view override returns(uint[] memory) {
        return LandTokenIds;
    }
}
...
pragma solidity ^0.8.9;
contract LandCore is LandFactory{
    using Counters for Counters.Counter;
    using SafeMath for uint256;
    Counters.Counter private _tokenIds;
    constructor(address _fyFarmDep) ERC721("MyFanyoyFarm", "MFYF")  {
        fyFarmDep = FyFarmDep(_fyFarmDep);
    }

    function createToken(uint8 _landType) public returns (uint) {
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        LandTypeInfo memory _lt = LandTypeInfos[_landType];
        require(_lt.landNumber < _lt.landTotalNumber, "The current type land has been sold out");

        IFYToken fyTokenInter = IFYToken(fyFarmDep.fyTokenAddress());
        fyTokenInter.transferAccountToAddress(msg.sender, _lt.landPrice);

        _createLand(msg.sender, newItemId);
        LandTypeInfos[_landType].landNumber = _lt.landNumber.add(1);
    
        emit NewLand(msg.sender, newItemId, _landType, _lt.landPrice, LandTypeInfos[_landType].landNumber, block.timestamp);
        return newItemId;
    }
}