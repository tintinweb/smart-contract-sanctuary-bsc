...
contract BELPNFT is ERC721Enumerable, BelpAuth {
    struct LevelUpPrice {
        uint partner;
        uint belp;
    }
    ...
    uint8 public maxRarity = 1;
    uint8 public maxLevel = 1;

    // ADDRESSES
    address deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public feeReceiver = 0xCd0c4b921B4a17896511e4720d6ce8F63EE8813D;
    address public belpAddress = 0xe79bdf3b0650Ef21a5BAc9625f7ea2732918E66c;
    IBEP20 private BelpAddress;
    address public partnerAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    IBEP20 private PartnerToken;
    ...
    // Base URI
    string private _baseUriExtended = "https://babyelephant.finance/api/erc721/";

    constructor() ERC721("Baby Elephant", "BELP NFT") BelpAuth(msg.sender) {
        BelpAddress = IBEP20(belpAddress);
        PartnerToken = IBEP20(partnerAddress);
    }
    ...
    function _mintCardsOfPack(address user, uint8 packId, uint8 numOfCards) internal {
        ...
        require(addressMintedMap[msg.sender] + numOfCards <= maxPerWallet, "Max total mint reached");
        ...
        if (!isAuthorizedFor(msg.sender, Permission.Mint)){
            addressMintedMap[msg.sender] += numOfCards;
        }
    }
    ...
    function levelUp(uint heroId) external {
        ...
        require(
            (
                levelData.partner == 0 ||
                (levelData.partner > 0 && BelpAddress.balanceOf(msg.sender) >= levelData.partner)
            ) &&
            (
                levelData.belp == 0 ||
                (levelData.belp > 0 && PartnerToken.balanceOf(msg.sender) >= levelData.belp)
            )
        ,"Not enough fee");

        if(levelData.partner > 0){
            require(BelpAddress.transferFrom(msg.sender, address(this), levelData.partner), "Partner Token transfer failed");
        }

        if(levelData.belp > 0){
            require(PartnerToken.transferFrom(msg.sender, address(this), levelData.belp), "BELP transfer failed");
        }

        if(newLevel == 1){
            hero.level = newLevel;

            try IBelpReflect(reflectAddress).registerNewMint( heroId ) {} catch {}
    ...
    function setMaxPerWallet(uint32 _max) external {
    ...
    function setPartnerAddress(address _newAdr) external {
        authorizedFor(Permission.AdjustVariables);

        belpAddress = _newAdr;
        BelpAddress = IBEP20(_newAdr);
    }

    function setBelpAddress(address _newAdr) external {
        authorizedFor(Permission.AdjustVariables);

        partnerAddress = _newAdr;
        PartnerToken = IBEP20(_newAdr);
    }
    ...
    function addLevelUpPrice(uint _partner, uint _belp) public{
        authorizedFor(Permission.ManageAttributes);

        levelUpPrices.push(LevelUpPrice(_partner, _belp));
    }

    function editLevelUpPrice(uint8 level, uint _partner, uint _belp) public {
        authorizedFor(Permission.ManageAttributes);

        require(level < levelUpPrices.length,"Invalid level");

        levelUpPrices[level].partner = _partner;
        levelUpPrices[level].belp = _belp;
    }
    ...
    function adminKillHero(uint heroId) external {
        ...
        try IBelpReflect(reflectAddress).updateBurnedToken( heroId ) {} catch {}
