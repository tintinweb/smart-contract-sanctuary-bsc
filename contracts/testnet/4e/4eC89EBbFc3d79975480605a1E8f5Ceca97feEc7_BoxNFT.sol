...
contract BoxNFT is ERC721URIStorage, Member {
    ...
    function buyBox(bytes memory data) public payable {
        ...
        if (payToken != address(0)) {
            IERC20(payToken).transfer(getTokenAddr, amount);
        } else {
            payable(getTokenAddr).transfer(amount);
        }
    ...
    function awardItem(address player, string memory tokenURI)private
        returns (uint256)
    {
        _tokenIds.increment();
    ...
    function setGetTolenAddr(address _addr) public CheckPermit("Config") {
        getTokenAddr = _addr;
    }
}
