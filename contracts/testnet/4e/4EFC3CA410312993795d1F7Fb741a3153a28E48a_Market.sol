...
contract Market {
  ...
  uint8 public constant OP_MIN = 0;
  uint8 public constant OP_MAX = 10;

  uint8 public constant OP_COMPLETE_SELL = 1;
  uint8 public constant OP_COMPLETE_BUY = 2;

  uint8 public constant OP_BUY = 3;
  uint8 public constant OP_ACCEPT_BUY = 4;
  uint8 public constant OP_CANCEL_BUY = 5;
  uint8 public constant OP_REJECT_BUY = 6;
  uint8 public constant OP_BID = 7;
  uint8 public constant OP_COMPLETE_AUCTION = 8;
  uint8 public constant OP_ACCEPT_AUCTION = 9;

  uint8 public TOKEN_721 = 1;
  uint8 public TOKEN_1155 = 2;
  ...
  event UpdateSinger(address signer, bool isValid);
  event InventoryUpdate(uint256 indexed id, Inventory inventory);
  ...
  function execution(
    ...
    require(detail.txDeadline > block.timestamp, "transaction reached ");
    require(marketSigners[detail.signer], "invalid market signer");

    require(detail.opcode > OP_MIN && detail.opcode < OP_MAX, "invalid opcode");
    ...
    if (hasSignedIntention(detail.opcode)) {
      bytes memory encodeInt = abi.encode(intent);
      require(
        keccak256(encodeInt) == detail.intentionHash,
        "intention hash does not match"
      );
      require(
        isSignatureValid(sigIntent, keccak256(encodeInt), intent.user),
  ...
  function _validateOpCode(uint256 opCode) internal pure {}

  function updateMarketSigner(
    address[] memory toAdds,
    address[] memory toRemoves
  ) public {
    for (uint256 i = 0; i < toAdds.length; i++) {
      marketSigners[toAdds[i]] = true;
      emit UpdateSinger(toAdds[i], true);
    }

    for (uint256 i = 0; i < toRemoves.length; i++) {
      marketSigners[toAdds[i]] = false;
      emit UpdateSinger(toAdds[i], false);
    }
  }
  ...
  function _newSellDeal(
    ...
    require(!hasInv(id), "inventory already exist");
    require(deadline > block.timestamp, "sell has already ended");

    _saveBundle(id, bundle);
    _transferBundle(id, seller, buyre);

    if (_isNative(currentcy)) {
      require(price == msg.value, "price == msg.value");
      weth.deposit{ value: price }();
    } else {
      currentcy.safeTransferFrom(buyre, address(this), price);
    }
    ...
    emit InventoryUpdate(id, inventories[id]);
  }

  function _saveBundle(uint256 id, TokenPair[] calldata bundle) internal {
    require(bundle.length > 0, "bundles is empty");
    inventoryTokenCounts[id] = bundle.length;
    for (uint256 i = 0; i < bundle.length; i++) {
      inventoryTokens[id][i] = bundle[i];
    }
  }
  ...
  function _isNative(IERC20 currentcy) public pure returns (bool) {
    return address(currentcy) == address(0);
  }
  ...
  function _completeTransaction(uint256 id) internal {
    ...
    _transfer(inv.currency, inv.seller, inv.price);
  ...
  function _transfer(
    ...
    if (_isNative(currency)) {
      ...
      currency.safeTransfer(to, value);
    }
  }
}
