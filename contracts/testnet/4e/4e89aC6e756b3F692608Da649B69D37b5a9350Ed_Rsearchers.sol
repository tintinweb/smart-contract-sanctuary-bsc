...
import "./IRsearchers.sol";


contract Rsearchers is Ownable, Pausable,IRsearchers { 
    IERC20 MITI;
    IContracts_TITIMITI Contr;

    mapping (uint256 => mapping(address => bool)) public OneMITI;
    mapping (uint256 => uint256) public TotalRecorded;
    ...
    function distribution (address[20]memory Adrs,uint256 Summ) public virtual override onlyOwner {
        uint256 X =0;
        while (X<Num-1){
            uint256 Summ_=Summ/X+1*D;
            MITI.transfer(Adrs[X], Summ_*_decimals_);
            X++;
        }
        Track++;
        Off_On=false;
    } 
    function disMITI (uint256 NumTrack) public {
        address sender = msg.sender;
        require(OneMITI[NumTrack][sender]=true);
        MITI.transfer(sender, _decimals_);
        OneMITI[NumTrack][sender]=false;
    }

    function Add_Rsearchers () public {
        require(Off_On==true);
        require(OneMITI[Track][msg.sender]==false);
        OneMITI[Track][msg.sender]=true;
        TotalRecorded[Track]++;
    }
    ...
    function set_Off_On () public onlyOwner {
        if(Off_On==true){
            Off_On=false;
        }
        if(Off_On==false){
...
interface IRsearchers {
    function distribution (address[20]memory Adrs,uint256 Summ) external;
