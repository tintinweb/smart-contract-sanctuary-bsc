...
// File contracts/marketplace/interfaces/IUniswapV2Router.sol
...
contract MarketplaceV3 is Initializable, AccessControlUpgradeable, PausableUpgradeable {
   ...
   * @param orderId - Order ID
   */
  function executeOrder(uint256 orderId, bool payWithBusd) external whenNotPaused {
    Order memory order = getOrder(orderId);
    ...
    if (payWithBusd) {
      ...
      _buyMHT(order.price);
      acceptedToken.approve(address(this), order.price);
    }
    ...
    if (ownerCutPerMillion > 0) {
      ...
      if (vault == address(0)) {
        acceptedToken.burnFrom(customSender, saleShareAmount);
        emit FeeBurned(saleShareAmount, orderId);
      } else {
        acceptedToken.transferFrom(customSender, vault, saleShareAmount);
      }
    }

    // Transfer sale amount to seller
    acceptedToken.transferFrom(customSender, seller, price - saleShareAmount);
  ...
  function _buyMHT(uint256 _mhtAmountOut) public whenNotPaused returns (uint256[] memory amounts) {
