...
contract Footballer is ERC721, Ownable {
    ...
    uint256 _commonPrice = 2000 * 10 ** 9;
    uint256 _rarePrice = 3000 * 10 ** 9;
    uint256 _legendaryPrice = 4500 * 10 ** 9; 
    ...
    function getNFTInformation(uint256 nftID) external view returns (uint256 Attribute, uint256 Energy, uint256 lastestUpdate) {
        ...
        if (characters[nftID].currentEnergy == 2) {
            ...
            lastestUpdate = 8 hours - block.timestamp + characters[nftID].lastestUpdate;

        } else if (characters[nftID].currentEnergy == 1) {

            lastestUpdate = 16 hours - block.timestamp + characters[nftID].lastestUpdate;

        } else if  (characters[nftID].currentEnergy == 0) {

            lastestUpdate = 24 hours - block.timestamp + characters[nftID].lastestUpdate;
    ...
    function priceMint(uint256 attribute, address user) private {
        ...
        require(ERC20(_tokenReward).balanceOf(user) >= _price, "You are not enough tokens to buy"); 

        ERC20(_tokenReward).transferFrom(user, _operator , _price);    
    ...
    function Training(uint256 nftID, address user) external onlyOperator {
      ...
      if (characters[nftID].lastestUpdate + 8 hours > block.timestamp) { // 0 Energy
          ...
          characters[nftID].currentEnergy == 2;
    ...
    function update0Energy(uint256 nftID) private {

        characters[nftID].currentEnergy = characters[nftID].currentEnergy - 1;

    }
    function update1Energy(uint256 nftID) private {
    ...
    function update2Energy(uint256 nftID) private {
