...
contract Token is ERC20, Pausable {
    ...
    uint256 public stakingFeesBuy = 100; // percentage to put aside for stakig rewards when buying
    uint256 public BBDDWalletFeesBuy = 0; // percentage to send to BBDD wallet when buying
    uint256 public reserveWalletFeesBuy = 100; // percentage to send to reserve wallet when buying

    uint256 public burnFeesSell = 100; // percentage to burn when selling
    uint256 public stakingFeesSell = 200; // percentage to put aside for staking rewards when selling
    uint256 public BBDDWalletFeesSell = 100; // percentage to send to BBDD wallet when selling
    uint256 public reserveWalletFeesSell = 100; // percentage to send to reserve wallet when selling

    bool public tradingEnabled; // enable trading for all
    bool public feesEnabled = true; // enable fees for all swap operations

    uint256 public MAX_TAX = 1500; // maximum allowed tax per transaction
    ...
    address public reserveWallet; // Reserve wallet
    address public stakingWallet; // Staking wallet for rewards holders

    uint256 public toBBDD; // BBDD fees to be swapped to USDC
    uint256 public toReserve; // Reserve fees to be swapped to USDC
    uint256 public toStaking; // Staking fees to be swapped to USDC
    ...
    ///@param _stakingWallet, staking wallet
    constructor(
        address _router,
        address _BBDDWallet,
    ...
    function zeroAllTaxes() external onlyOwner {
        ...
        stakingFeesBuy = 0;
        BBDDWalletFeesBuy = 0;
        reserveWalletFeesBuy = 0;

        burnFeesSell = 0;
        stakingFeesSell = 0;
        BBDDWalletFeesSell = 0;
        reserveWalletFeesSell = 0;
    ...
    /// @param  _stakingFeesBuy, staking fees when buying
    /// @param  _stakingFeesSell, staking fees when selling
    /// @param  _BBDDWalletFeesBuy, BBDD fees when buying
    /// @param  _BBDDWalletFeesSell, BBDD fees when selling
    /// @param  _reserveWalletFeesBuy, reserve fees when buying
    /// @param  _reserveWalletFeesSell, reserve fees when selling
    function setAllTaxes(
        uint256 _burnFeesBuy,
        uint256 _burnFeesSell,
        uint256 _stakingFeesBuy,
        uint256 _stakingFeesSell,
        uint256 _BBDDWalletFeesBuy,
        uint256 _BBDDWalletFeesSell,
        uint256 _reserveWalletFeesBuy,
        uint256 _reserveWalletFeesSell
    ) external onlyOwner {
        burnFeesBuy = _burnFeesBuy;
        stakingFeesBuy = _stakingFeesBuy;
        BBDDWalletFeesBuy = _BBDDWalletFeesBuy;
        reserveWalletFeesBuy = _reserveWalletFeesBuy;
        require(
            burnFeesBuy +
                stakingFeesBuy +
                BBDDWalletFeesBuy +
                reserveWalletFeesBuy <=
                MAX_TAX,
            "TNEC : Buy fees cannot exceed the MAX"
        );

        burnFeesSell = _burnFeesSell;
        stakingFeesSell = _stakingFeesSell;
        BBDDWalletFeesSell = _BBDDWalletFeesSell;
        reserveWalletFeesSell = _reserveWalletFeesSell;
        require(
            burnFeesSell +
                stakingFeesSell +
                BBDDWalletFeesSell +
                reserveWalletFeesSell <=
                MAX_TAX,
            "TNEC : Buy fees cannot exceed the MAX"
        );
    ...
    /// @notice setMaxFees to new number, only Owner can call this function
    /// @param _max is the new allowed fees
    function setMaxFees(uint256 _max) public onlyOwner {
        MAX_TAX = _max;
    }
    ...
    /// @notice setreserveWallet, update the reserve wallet, only Owner can call this function
    /// @param  _newreserveWallet, new reserve address
    function setreserveWallet(address _newreserveWallet) public onlyOwner {
        require(_newreserveWallet != address(0), "cannot be the zero address");
        reserveWallet = _newreserveWallet;
    }

    /// @notice setStakingWallet, update the reserve wallet, only Owner can call this function
    /// @param  _newStakingWallet, new staking address
    ...
    /// @return toStakingWallet value to be transfered to the staking wallet when buying
    /// @return toBBDDWallet value to be transfered to the BBDD wallet when buying
    /// @return toReserveWallet value to be transfered to the reserve wallet when buying
    ...
    function applyFeesBuy(uint256 amountIn)
        ...
        returns (
            ...
            uint256 toReserveWallet,
            uint256 toBurn
        )
    {
        toStakingWallet = (amountIn * stakingFeesBuy) / DENOMINATOR;
        toBBDDWallet = (amountIn * BBDDWalletFeesBuy) / DENOMINATOR;
        toReserveWallet = (amountIn * reserveWalletFeesBuy) / DENOMINATOR;
        toBurn = (amountIn * burnFeesBuy) / DENOMINATOR;
        amountOut =
            amountIn -
            (toStakingWallet + toBBDDWallet + toReserveWallet + toBurn);
    }

    /// @notice required to apply the correct fees taxes when selling
    /// @param amountIn, value received and to be dispatched
    /// @return amountOut value to be transfered
    /// @return toStakingWallet value to be transfered to the staking wallet when selling
    /// @return toBBDDWallet value to be transfered to the BBDD wallet when selling
    /// @return toReserveWallet value to be transfered to the reserve wallet when selling
    ...
    {
        toStakingWallet = (amountIn * stakingFeesSell) / DENOMINATOR;
        toBBDDWallet = (amountIn * BBDDWalletFeesSell) / DENOMINATOR;
        toReserveWallet = (amountIn * reserveWalletFeesSell) / DENOMINATOR;
    ...
    ///@notice swapTokensForUSDCAndTransfer, required to swap tokens for USDC and transfer to specific address. Called only by the owner
    ///@param to, address to receiver the amount swaped
    function swapTokensForUSDCAndTransfer(address to) public onlyOwner {
        uint256 amount = _getToCorrectValue(to);

        require(amount > 0, "Not enough balance to swap");
    ...
    ///@notice swapTokensForUSDCAndTransfer, required to swap all stored tokens -from transactions fees- for USDC and transfer to specific addresses
    /// (toBBDD, toStaking, toReserve). Called only by the owner
    function swapTokensForUSDCAndTransfer() public onlyOwner {
        address[] memory _path = new address[](2);
    ...
    function _transfer(
        ...
        if (!whitelist[sender] && !whitelist[recipient] && feesEnabled) {
            require(tradingEnabled, "Trading is not actif yet");
            if (sender == mainLP) {
                (
                    uint256 _amountOut,
                    uint256 _toStakingWallet,
                    uint256 _toBBDDWallet,
                    uint256 _toReserveWallet,
                    uint256 _toBurn
                ) = applyFeesBuy(amount);
                ...
                super._transfer(
                    sender,
                    address(this),
                    _toStakingWallet / 2 + _toBBDDWallet + _toReserveWallet
                );
                super._transfer(sender, stakingWallet, _toStakingWallet / 2);

                // store how many tokens to swap for usdc
                toBBDD += _toBBDDWallet;
                toReserve += _toReserveWallet;
                toStaking += _toStakingWallet / 2;

                _burn(sender, _toBurn);
            } else if (recipient == mainLP) {
                (
                    uint256 _amountOut,
                    uint256 _toStakingWallet,
                    uint256 _toBBDDWallet,
                ...
                _swapTokensForUSDCAndTransfer(
                    _toStakingWallet / 2,
                    stakingWallet
                );

                // send tax to BBDD wallet
                _swapTokensForUSDCAndTransfer(_toBBDDWallet, BBDDWallet);

                // send to reserve wallet
                _swapTokensForUSDCAndTransfer(_toReserveWallet, reserveWallet);
    ...
    ///@notice _getToCorrectValue, required to get specific amount stores on the smart contract for a specific address
    ///@param to, address to receiver the amount swaped
    ///@param _toSwap, amount to be swaped
    function _getToCorrectValue(address to) internal returns (uint256 _toSwap) {
        if (to == stakingWallet) {
            _toSwap = toStaking;
            toStaking = 0;
        } else if (to == reserveWallet) {
            _toSwap = toReserve;
            toReserve = 0;
        } else if (to == BBDDWallet) {
            _toSwap = toBBDD;
            toBBDD = 0;
        }
        return _toSwap;
