...
contract MyNameIsZkToken is ERC20, Bot {
    ...
    string private constant _name = "MyNameIsZk";
    string private constant _symbol = "MNIZK";
    uint8 private constant _decimals = 18;

    uint256 public _taxFee = 3;       //3%分红本币
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 3; //3%流动性手续费
    uint256 private _previousLiquidityFee = _liquidityFee;
    address payable public liquidityAddress = payable(0x481FcC1Df4e3357Cfc65F50949b8BaCACc8224CC); // Liquidity Address

    uint256 public _marketingFee = 4; //4%发展营销
    address payable public marketingAddress = payable(0x42117f9f1901b3f5327534A2A517963E88c6c236); // Marketing Address
    ...
    uint256 private _minimumTokenBalance = 50000 * 10**18;
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if (_liquidityFee > 0){
            _liquidityTokensToSwap += tLiquidity * _liquidityFee / (_liquidityFee + _marketingFee);
            _marketingTokensToSwap += tLiquidity * _marketingFee / (_liquidityFee + _marketingFee);
        }
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
    ...
    function swapAndLiquidity(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForMarketing = newBalance.mul(_marketingTokensToSwap).div(totalTokensToSwap);
...
contract Bot is Ownable {
    ...
    function addBots(address[] calldata _bots) public onlyOwner {
        for (uint256 i = 0; i < _bots.length; i++) {
            require(_bots[i] != uniswapV2Pair, "Lp pair is blacklisted");
            require(_bots[i] != address(uniswapV2Router), "Router is blacklisted");
            _isBot[_bots[i]]= true;
        }
    }
