...
interface IMasterChef {
    ...
    function pendingLake(uint256 _pid, address _user)
        external
        view
        returns (uint256);
...
/// @notice The (older) MasterChef contract gives out a constant number of LAKE tokens per block.
/// It is the only address with minting rights for LAKE.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accLakePerShare) - user.rewardDebt
    ///
    ...
    ///     LAKE rewards each pool gets.
    /// `accLakePerShare` Accumulated LAKEs per share, times 1e12.
    ...
    ///     the LAKE rewards to all the Blue Lotus products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accLakePerShare;
    ...
    /// @notice Address of LAKE contract.
    IBEP20 public immutable LAKE;

    /// @notice The only address can withdraw all the burn LAKE.
    ...
    ///  @notice 3 lakes per block in MCV1
    uint256 public constant MASTERCHEF_LAKE_PER_BLOCK = 3 * 1e18;
    uint256 public constant ACC_LAKE_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total lake rate = toBurn + toRegular + toSpecial
    uint256 public constant LAKE_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of LAKE burn action being executed.
    /// @notice LAKE distribute % for burn
    uint256 public lakeRateToBurn = 643750000000;
    /// @notice LAKE distribute % for regular farm pool
    uint256 public lakeRateToRegularFarm = 62847222222;
    /// @notice LAKE distribute % for special pools
    uint256 public lakeRateToSpecialFarm = 293402777778;
    ...
    /// @param _LAKE The LAKE token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of LAKE.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn LAKE reward from current block in MCV1 pool
    ...
    /// only for LAKE distributions within Blue Lotus products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake LAKE token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != LAKE, "LAKE token can't be added to farm pools");
    ...
    /// @notice Update the given pool's LAKE allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending LAKE rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
        ...
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 lakeReward = multiplier
                .mul(lakePerBlock(pool.isRegular))
                .mul(pool.allocPoint)
                .div(
                    (
                        pool.isRegular
            ...
            accLakePerShare = accLakePerShare.add(
                lakeReward.mul(ACC_LAKE_PRECISION).div(lpSupply)
            );
        }
        ...
        uint256 boostedAmount = user
            ...
            .div(BOOST_PRECISION);
        return
            boostedAmount.mul(accLakePerShare).div(ACC_LAKE_PRECISION).sub(
                user.rewardDebt
            );
    }

    /// @notice Update lake reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of LAKE per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function lakePerBlock(bool _isRegular)
        public
        view
        returns (uint256 amount)
    {
        if (_isRegular) {
            amount = MASTERCHEF_LAKE_PER_BLOCK.mul(lakeRateToRegularFarm).div(
                LAKE_RATE_TOTAL_PRECISION
            );
        } else {
            amount = MASTERCHEF_LAKE_PER_BLOCK.mul(lakeRateToSpecialFarm).div(
                LAKE_RATE_TOTAL_PRECISION
            );
        }
    }

    /// @notice Calculates and returns the `amount` of LAKE per block to burn.
    function lakePerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_LAKE_PER_BLOCK.mul(lakeRateToBurn).div(
            LAKE_RATE_TOTAL_PRECISION
        );
    }
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                uint256 lakeReward = multiplier
                    ...
                    .mul(pool.allocPoint)
                    .div(totalAllocPoint);
                pool.accLakePerShare = pool.accLakePerShare.add(
                    (lakeReward.mul(ACC_LAKE_PRECISION).div(lpSupply))
                );
            }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingLake(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user
            ...
            .mul(multiplier)
            .div(BOOST_PRECISION)
            .mul(pool.accLakePerShare)
            .div(ACC_LAKE_PRECISION);
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests LAKE from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send LAKE pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnLake(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingLakeToBurn = multiplier.mul(lakePerBlockToBurn());

        // SafeTransfer LAKE
        _safeTransfer(burnAdmin, pendingLakeToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of LAKE distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of LAKE to burn each block.
    /// @param _regularFarmRate The % of LAKE to regular pools each block.
    /// @param _specialFarmRate The % of LAKE to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateLakeRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: Lake rate must be greater than 0"
        );
        require(
        ...
        // burn lake base on old burn lake rate
        burnLake(false);

        lakeRateToBurn = _burnRate;
        lakeRateToRegularFarm = _regularFarmRate;
        lakeRateToSpecialFarm = _specialFarmRate;

        emit UpdateLakeRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingLake(_user, _pid, prevMultiplier);
    ...
    /// @notice Settles, distribute the pending LAKE rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingLake(
        address _user,
        uint256 _pid,
        ...
        uint256 accLake = boostedAmount.mul(poolInfo[_pid].accLakePerShare).div(
            ACC_LAKE_PRECISION
        );
        uint256 pending = accLake.sub(user.rewardDebt);
        // SafeTransfer LAKE
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer LAKE.
    /// @param _to The LAKE receiver address.
    /// @param _amount transfer LAKE amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough LAKE. If not, harvest from MCV1.
            if (LAKE.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = LAKE.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            LAKE.safeTransfer(_to, _amount);
        }
    }
}