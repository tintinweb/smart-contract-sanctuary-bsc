...
interface URI:
    def tokenURI(_tokenId: uint256) -> String[128]: pure

interface Socks:
    def totalSupply() -> uint256: pure
...
ownerOf: public(HashMap[uint256, address])                     # map(tokenId, owner)
balanceOf: public(HashMap[address, uint256])                   # map(owner, balance)
getApproved: public(HashMap[uint256, address])                 # map(tokenId, approvedSpender)
isApprovedForAll: public(HashMap[address, HashMap[address, bool]]) # map(owner, map(operator, bool))
supportsInterface: public(HashMap[bytes32, bool])              # map(interfaceId, bool)
ownerIndexToTokenId: HashMap[address, HashMap[uint256, uint256]]   # map(owner, map(index, tokenId))
tokenIdToIndex: HashMap[uint256, uint256]                      # map(tokenId, index)
...
@external
def __init__(_unisock0: address):
    self.name = 'Unisocks'
    self.symbol = 'S0CKS'
    self.minter = msg.sender
    self.socks = Socks(_unisock0)
