...
contract StakeItV2 is Ownable {
        ...
        ensurePackageStatus(pack)
    {
        ...
        if (packIdToRedeem != 0)
            require(
                ...
                "StakeIt: User not found in this id!"
            );
        else {
    ...
    function stake(
        ...
        require(stcPrice > 0, "StakeIt: Invalid STC amount");
        require(
            validateHash(msg.sender, stcPrice, pack, expiry, v, r, s),
            "Invalid signature"
        );
        userDetails storage userdetails = UserDetails[msg.sender];
        ...
        uint stcAmountToStake = viewStcAmountToStake(pack, stcPrice);

        require(stcAmountToStake > userdetails.packages[userPackId][pack].stakedAmount, "StakeIt: BUSD capacity overflow!");
        uint existingStcToStake = (stcAmountToStake - userdetails.packages[userPackId][pack].stakedAmount);
        ...
        userdetails.packages[userPackId][pack].stakedAmount += existingStcToStake;
        userdetails.packages[userPackId][pack].rewardDept = (rewardPerShare *
            userdetails.packages[userPackId][pack].stakedAmount);
        totalStakedAmount += existingStcToStake;
        package.poolSTCamount += existingStcToStake;
        require(
            STC_Addr.transferFrom(msg.sender, address(this), existingStcToStake),
            "StakeIt: Tx failed!"
        );

        setHashCompleted(
            prepareHash(msg.sender, existingStcToStake, pack, expiry),
            true
        );
        emit Staked(pack, msg.sender, existingStcToStake);
    }

    function viewStcAmountToStake(Pack pack, uint stcPrice) public view returns(uint r) {
