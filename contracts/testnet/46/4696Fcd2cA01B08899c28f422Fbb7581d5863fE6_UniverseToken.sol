...
contract UniverseToken is IERC20Metadata, Context, Ownable {
    ...
    uint256 internal constant _tTotal = 5 * 10**_decimals * 10**9;
    ...
    constructor () {
        ...
        _isExcludedFromFee[rewardSystemAddress] = true;
   ...
   function decimals() external pure override returns (uint8) {
    ...
    function setRewardSystemAddress(address _rewardSystemAddress) external onlyOwner() {
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
       (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 marketingFee) = _getValues(tAmount);
        ...
        // UniverseNFT.distribute(_takeFee(marketingFee, rewardSystemAddress));
    ...
    function setMarkingAddress(address _marketingAddress) external onlyOwner {
        require(_marketingAddress != deadAddress, "The address should avoid becoming the dead wallect");
        require(_marketingAddress != address(0), "The address should avoid becoming the zero address");
    ...
    function _takeFee(uint256 fee, address feeRecevier) internal returns(uint256) {
       uint256 currentRate =  _getRate();
