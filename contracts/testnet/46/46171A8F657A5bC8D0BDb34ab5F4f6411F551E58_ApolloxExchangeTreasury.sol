...
contract ApolloxExchangeTreasury is Ownable, Pausable {
    ...
    event Withdraw(uint256 indexed id, address indexed to, address indexed currency, bool isETH, uint256 amount);
    event Claim(uint256 indexed id, address indexed to, address indexed currency, bool isETH, uint256 amount);
    ...
    function withdraw(bytes calldata message, bytes calldata signature) external whenNotPaused {
        ...
        if (result) {
            emit Withdraw(id, to, currency, isETH, amount);
        }
    }
    ...
    function claim(bytes calldata message, bytes calldata signature) external whenNotPaused {
        ...
        bool result = _claim(id, to, currency, isETH, amount, deadline);
        if (result) {
            emit Claim(id, to, currency, isETH, amount);
        }
    }
