...
contract Token is IBEP20, Auth {
    ...
    address private immutable i_dead;
    address private immutable i_zero;
    address public reward = 0x33e10AE71654aFc506ff11cDDa920f190DF9394B;
    ...
    uint256 public s_maxTxAmount = s_totalSupply / 100;
    uint256 public s_maxWalletToken = s_totalSupply / 50;
    ...
    uint256 public swapThreshold = s_totalSupply * 10 / 10000;
    bool private inSwap;
    ...
    constructor(
        ...
        address deadAddress,
        address zeroAddress,
        address marketingFeeReciever,
        address teamFeeReciever
    ) Auth(msg.sender) {
        s_name = _name;
        s_symbol = _symbol;
        i_decimals = _decimals;
        s_totalSupply = 100 * 10**6 * 10**_decimals;
        ...
        distributor = new DividendDistributor(address(msg.sender), reward);
        ...
        marketingFeeReceiver = marketingFeeReciever;
        teamFeeReceiver = teamFeeReciever;
    ...
    function balanceOf(address account) public view override returns (uint256) { return s_balances[account]; }
    function allowance(address holder, address spender) external view override returns (uint256) { return s_allowances[holder][spender]; }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        if(s_allowances[sender][msg.sender] != uint256(-1)){
            s_allowances[sender][msg.sender] = s_allowances[sender][msg.sender].sub(amount, "Insufficient Allowance");
        }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(i_dead) && recipient != pair && recipient != marketingFeeReceiver && recipient != teamFeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
            uint256 heldTokens = balanceOf(recipient);
            require((heldTokens + amount) <= s_maxWalletToken,"Total Holding is currently limited, you can not buy that much.");}
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && s_balances[address(this)] >= swapThreshold;
    }
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return s_totalSupply.sub(balanceOf(i_dead)).sub(balanceOf(i_zero));
    }
