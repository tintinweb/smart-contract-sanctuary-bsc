...
contract CirclePoolManager is OwnableUpgradeable, IPoolManager {
    ...
    function getUserContributedPoolInfo(
        ...
        for (uint256 i = start; i <= end; i++) {
            if(_poolState[_contributedPoolsOf[user].at(i)] == 1) {
                (
                address token,
                uint8[] memory saleType,
                uint256[] memory info,
                ...
                string memory name,
                string memory poolDetails
                ) = IPrivatePool(_contributedPoolsOf[user].at(i)).getPoolInfo();
                lockInfo[currentIndex] = CumulativeLockInfo(
                    _pools.at(i),
                    address(0),
                    saleType[0],
