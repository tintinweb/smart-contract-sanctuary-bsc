...
contract FeeManager is Ownable {
    ...
    event OwnerCommissionFee(uint256 commissionFee);

    // Market fee on sales
    uint256 public commissionFee;
    // uint256 public constant maxcommissionFee = 1000; // 10% cut
    

    /**
     * @dev Sets the share cut for the owner of the contract that's
     *  charged to the seller on a successful sale
     * @param _commissionFee - Share amount, from 0 to 99,999
     */
    function setOwnerCommissionFee(uint256 _commissionFee) external onlyOwner {
        commissionFee = _commissionFee;
        emit OwnerCommissionFee(commissionFee);
    }
}
...
contract Marketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
    ...
    uint56 public indexCount;
    uint256 public leftFromMarket;
     ...
     */
    function initialize(address _acceptedToken, uint256 _commissionFee) public {
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
    ...
    function cancelOrder(address _nftAddress, uint256 _assetId) public whenNotPaused {
        ...
        // delete nft from allListing.      
        deleteNftFromList(_nftAddress, _assetId);
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) public whenNotPaused {
        ...
        if (FeeManager.commissionFee > 0) {
            // Calculate sale share
            saleShareAmount = _priceInWei.mul(FeeManager.commissionFee).div(1e4);
     ...
     * @param _priceInWei - Price in Wei for the supported coin
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) internal {
        // Check nft registry
    ...
    function acceptOffer( address _nftAddress, uint256 _assetId) public whenNotPaused{
        ...
        // item seller is the only allowed to accept a offer
        require(order.seller == msg.sender, "Marketplace: unauthorized sender");
        ...
        if (FeeManager.commissionFee > 0) {
            // Calculate sale share
            saleShareAmount = off.price.mul(FeeManager.commissionFee).div(1e4);
    ...
    function allListing(uint16 skip, uint16 limit)public view returns(AllNft[] memory){
        uint256 size = limit - skip;
        // uint256 nftInMarket = indexCount - leftFromMarket;
        // if(size > nftInMarket){
        //     size = nftInMarket;
        // }
        AllNft[] memory lists = new AllNft[](size);
        uint256 temp = limit;
        uint8 temp2 = 0;

        for(uint16 i = skip; i < temp && i < indexCount; i++){
            AllNft storage list = listNFt[i];
            if(list.nftAddress == address(0)){
                temp = temp + 1;
                continue;         
            }
            lists[temp2] = list; 
    ...
    function deleteNftFromList(address _nftAddress, uint256 _assetId) internal{
        for(uint16 i = 0; i< indexCount; i++){
            AllNft memory list = listNFt[i];
            if(list.assetId == _assetId && list.nftAddress == _nftAddress){
                delete listNFt[i];
            }
