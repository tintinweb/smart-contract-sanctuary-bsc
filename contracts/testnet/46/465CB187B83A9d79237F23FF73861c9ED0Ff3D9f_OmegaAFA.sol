...
contract OmegaAFA is Ownable, ERC20Permit {
    ...
    function addLiquiAfa() internal swapping {
        if (liquiAfaReceive > 0) {
            ...
            liquiAfaReceive = 0;
            IAlfaDao(autoLiquiReceiver).addLiquiAfa(addAmount);
            _lastAddLiquiTimeAfa = block.timestamp;
            emit SwapAndLiquify(addAmount);
        }
    }
    ...
    function addLiquiOma() internal swapping {
        if (liquiOmaReceive > 0) {
            ...
            liquiOmaReceive = 0;
            IAlfaDao(autoLiquiReceiver).addLiquiOma(addAmount);
    ...
    function shouldAddLiquiAfa() internal view returns (bool) {
        return _autoAddLiquidity && !inSwap && _lastAddLiquiTimeAfa > 0 && block.timestamp >= (_lastAddLiquiTimeAfa + autoLiquidityInterval);
    }

    function shouldAddLiquiOma() internal view returns (bool) {
        return _autoAddLiquidity && !inSwap && _lastAddLiquiTimeOma > 0 && block.timestamp >= (_lastAddLiquiTimeOma + autoLiquidityInterval);
    }
