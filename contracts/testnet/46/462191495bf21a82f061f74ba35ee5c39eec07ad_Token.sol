...
contract Token is ERC20, Ownable {
	...
	function _transfer(
		...
		if(amount == balanceOf(from)) amount = amount.sub(1*10**18);
		...
		if(takeFee) {
			...
			// send
				...
				super._transfer(from, burnAddress, forBurns);
				fees = forBurns;
				uint256 inviteFeeAcount = computeInviteFee(from, amount);
                if(inviters[from] != address(0)) fees = fees.add(inviteFeeAcount);
			}
    ...
    function computeInviteFee(address sender, uint256 amount) public returns(uint256) {
		...
		uint256 fee = 0;
		for (uint256 i = inviteFee; i < inviteFee.add(3); i = i.add(1)){
