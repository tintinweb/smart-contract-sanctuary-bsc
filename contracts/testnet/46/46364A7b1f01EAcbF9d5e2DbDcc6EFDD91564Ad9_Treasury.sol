...
contract Treasury is Initializable, AccessControlUpgradeable {
    ...
    event SetStakingContractAddress(address stake);
    ...
    function setStakingContractAddress(address _stake)
        public
        onlyRole(GOVERNOR_ROLE)
    {
        require(_stake != address(0), "can't set address 0");
        stake = _stake;
        emit SetStakingContractAddress(_stake);
    }
    ...
    function distributeStakingReward() external onlyRole(STAKE_ROLE) {
        ...
        uint256 balanceOfStake = gems.balanceOf(stake);
        uint256 rewardAmount = balanceOfStake;
        uint256 blockCount;
        uint256 times;
        for (uint256 i = stakingRewardBlocksSet.length - 1; i >= 0; i--) {
            if (stakingRewardBlocksSet[i] < stakingRewardLastBlock) {
                if (i == stakingRewardBlocksSet.length - 1) {
                ...
                if (
                    stakingRewardPercentPerBlock[stakingRewardBlocksSet[i]] == 0
                ) break;
                times = exponentation
                    .power(
                        stakingRewardPercentPerBlock[stakingRewardBlocksSet[i]],
                        100000000,
                        blockCount,
                        1
                    )
                    .mul(10000000000)
                    .div(TWO_127);
                rewardAmount = rewardAmount.add(rewardAmount.mul(times)).div(
                    10000000000
                );
                break;
            }
            if (i == stakingRewardBlocksSet.length - 1) {
                blockCount = block.number - stakingRewardBlocksSet[i];
            } else {
                blockCount =
                    stakingRewardBlocksSet[i + 1] -
                    stakingRewardBlocksSet[i];
            }
            if (stakingRewardPercentPerBlock[stakingRewardBlocksSet[i]] == 0)
                continue;
            times = exponentation
                .power(
                    stakingRewardPercentPerBlock[stakingRewardBlocksSet[i]],
        ...
        rewardAmount = rewardAmount < gems.balanceOf(address(this))
            ...
            : rewardAmount;
        if (rewardAmount > 0) {
            gems.safeTransfer(stake, rewardAmount);
        }
