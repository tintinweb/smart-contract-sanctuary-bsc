...
*
*
* --- Functionality added specific to the CUSDToken ---
*
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Bank of Cronos contracts) in external
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending CUSD directly to a Bank of Cronos
* core contract, when they should rather call the right function.
*
* 2) sendToPool() and returnFromPool(): functions callable only Bank of Cronos Loans core contracts, which move CUSD tokens between Bank of Cronos <-> user.
*/

contract CUSDToken is CheckContract, ICUSDToken {
    using SafeMath for uint256;

    uint256 private _totalSupply;
    string constant internal _NAME = "CUSD Stablecoin";
    string constant internal _SYMBOL = "CUSD";
    ...
    // --- Functions for intra-BankofCronos calls ---
    ...
    (
        ...
        override
    {
        require(deadline >= block.timestamp, 'CUSD: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01',
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount,
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner || recoveredAddress != address(0) , 'CUSD: invalid signature');
    ...
    // TODO: update this when we've finalized where CUSD can/can't be sent.
    // May want to make it possible to send to adddress(0) for burns or something
    function _requireValidRecipient(address _recipient) internal view {
        require(
            _recipient != address(0) &&
            _recipient != address(this),
            "CUSD: Cannot transfer tokens directly to the CUSD token contract or the zero address"
        );
        // require(
        ...
        //     "CUSD: Cannot transfer tokens directly to the StabilityPool, TroveManager or BorrowerOps"
        // );
    }

    function _requireValidMinter() internal view {
        require(validMinters[msg.sender], "CUSDToken: Caller is not Valid Minter");
    }
    ...
    function _requireCallerIsBOorTroveMorSP() internal view {
        require(
            ...
            "CUSD: Caller is neither BorrowerOperations nor TroveManager nor StabilityPool"
        );
    }

    function _requireCallerIsStabilityPool() internal view {
        require(msg.sender == stabilityPoolAddress, "CUSD: Caller is not the StabilityPool");
    }

    function _requireCallerIsTMLorSP() internal view {
        require(
            msg.sender == stabilityPoolAddress || msg.sender == troveManagerLiquidationsAddress,
            "CUSD: Caller is neither TroveManagerLiquidator nor StabilityPool");
    }
...
interface ICUSDToken is IERC20, IERC2612 {
    ...
    event CUSDTokenBalanceUpdated(address _user, uint _amount);
