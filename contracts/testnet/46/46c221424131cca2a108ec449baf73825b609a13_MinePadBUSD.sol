...
contract MinePadBUSD is Context, Ownable , ReentrancyGuard, Events {
    ...
    function deposit(address _ref, uint256 _amount) public noReentrant  {
        ...
        require(SafeMath.add(BusdInterface.balanceOf(address(this)),_amount) <= Max_reward_pool, "Max_Alloc_Reached.");
        if(!checkAlready()){
            ...
            uint256 start_20_percent_count = block.timestamp + 35 minutes;
    ...
    function _updateAllocations(uint256 _amount) internal {        
        ...
        if (SafeMath.add(currentDistribution, yourAllocate) <= Max_reward_pool) {
            ...
            for (uint256 i; i < lent; ) {
                ...
                vesting_percent[i] = _calculate(yourAllocate, _period.p_period);
    ...
    function _claimedCheck() internal returns(uint256) {
        ...
        for (uint256 i; i < lent; ) {
            ...
            if (vestP == currentlyClaimed) {
                _vesting[msg.sender].nextClaimTime = _vesting[msg.sender].vestingPeriod[i + 1];
