...
contract BlockChatUpgradeable2 is IBlockChatUpgradeable2, AccessControlUpgradeable, UUPSUpgradeable {
    mapping(bytes32 => uint64[]) public recipientMessageListMap;
    mapping(uint256 => Message) public messageMap;
    uint64 public messageLength;
    ...
    function getMessageHash(
        ...
        return bytes26(keccak256(abi.encodePacked(sender, recipientHashList, content, createDate,createBlock)));
    }
    ...
    function createMessage(bytes32[] memory recipientHashList, string memory content) external override {
        ...
        messageMap[messageLength] = Message(
            getMessageHash(msg.sender, recipientHashList, content, block.timestamp,block.number),
            uint48(block.number)
        );
