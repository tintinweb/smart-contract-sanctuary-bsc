...
contract Router is ERC1155Holder{
    ...
    function getNftInPair(address[] memory path, uint[] memory id) public view returns (bool nft) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
        ...
        // }
        nft = IPancakePair(pairAddress(path[1], path[0], id[0])).getNft(path[1], id[0]);
    }
}
