...
contract Fusion is OwnableUpgradeable, ERC721Holder {
    ...
    event NumberOfFused(uint256 numberOfFused);
    ...
    function testMintSeedmon(uint256 mainTokenId, uint256 materialTokenId, address to) external returns (uint256 fused) {  
        (,SeedmonAttribute.UniqueProfile memory uniqueProfile, SeedmonAttribute.Stat memory mainStat) = seedmon.tokenInfo(mainTokenId);
        // (,, SeedmonAttribute.Stat memory materialStat) = seedmon.tokenInfo(materialTokenId);
            fused = uniqueProfile.numberOfFused;
            // SeedmonAttribute.Stat memory pendingStat;
            // pendingStat.vitality = 
            
            emit NumberOfFused(fused);
