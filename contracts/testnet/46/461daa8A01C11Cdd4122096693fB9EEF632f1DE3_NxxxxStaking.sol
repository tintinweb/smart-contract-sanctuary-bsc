...
contract NxxxxStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block time when Pool mining ends.
    uint256 public bonusEndTime;

    // The block time when Pool mining starts.
    ...
    // Withdraw fee (default 5%)
    uint16 public withdrawFee = 500;
    ...
    event NewStartAndEndBlockTimes(uint256 startTime, uint256 endTime);
    event AprValueUpdated(uint256 oldAprValue, uint256 newAprValue);
    event NewFreezeBlockTimes(uint256 freezeStartTime, uint256 freezeEndTime);
     ...
     * @param _aprValue: apr value
     * @param _startTime: start block time
     * @param _bonusEndTime: end block time
    ...
    function initialize(
        ...
        require(
            _aprValue >= MIN_APR_VALUE && _aprValue <= MAX_APR_VALUE,
            "Invalid apr value range"
        );
        require(
            block.timestamp < _startTime,
            "Invalid past time for pooling starts"
        );
        require(
        ...
        // Set the lastClaimedTime as the startTime
     ...
     * @param _stakingAmount: amount in staking decimal
     * @param _multiplier: time multiplier
     */
    function _getRewardAmount(uint256 _stakingAmount, uint256 _multiplier)
        internal
        view
        returns (uint256)
    {
        return
            _stakingAmount
                .mul(_multiplier)
                .mul(aprValue)
                .mul(10**uint256(rewardToken.decimals()))
                .div(10000)
                .div(365 days)
                .div(uint256(stakedToken.decimals()));
    }

    /*
     * @notice Return reward multiplier over the given _from to _to block time.
     * @param _from: block time to start
     * @param _to: block time to finish
     */
    ...
    function claim() external nonReentrant {
        ...
        require(
            ...
            "Current block time should be after lastClaimedTime"
        );

        uint256 claimedAmount = calculateTotalPendingCakeRewards();
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            user.saunaAtLastUserAction = (user.amount.mul(totalStakings)).div(
                totalShares
            );
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            if (canWithdraw(msg.sender)) {
                ...
                if (
                    ...
                    if (half > 0 && otherHalf > 0) {
                        if (isBnbPairToken) {
                            ...
                            uint256 newETHBalance = address(this).balance.sub(
                                initialETHBalance
                            );
                    ...
                    if (
                        ...
                        if (feeAmount > 0) {
                            if (withdrawFeeMethod == 1) {
                                ...
                                if (half > 0 && otherHalf > 0) {
                                    if (isBnbPairToken) {
                                        uint256 initialETHBalance = address(
                                            this
                                        ).balance;
                ...
                if (user.amount > 0) {
                    user.saunaAtLastUserAction = (
                        user.amount.mul(totalStakings)
                    ).div(totalShares);
                } else {
     ...
     * Can only be called by the owner.
     */
    function updateAprValue(uint256 _aprValue) external onlyOwner {
        require(
            _aprValue <= MAX_APR_VALUE && _aprValue >= MIN_APR_VALUE,
            "Invalid apr value range"
        );
        require(_aprValue != aprValue, "Already set same value");
        aprValue = _aprValue;

        emit AprValueUpdated(aprValue, _aprValue);
    }
    ...
    function calculateClaimRewards() external view returns (uint256) {
        uint256 multiplier = _getMultiplier(lastClaimedTime, block.timestamp);
        uint256 claimedAmount = _getRewardAmount(totalStakings, multiplier);
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return
            totalShares == 0
                ? uint256(stakedToken.decimals())
                : totalStakings.mul(uint256(stakedToken.decimals())).div(
                    totalShares
                );
    }
     ...
     */
    function updateStartAndEndBlockTimes(
        uint256 _startTime,
        uint256 _bonusEndTime
    ) external onlyOwner {
        require(
            block.timestamp < startTime || block.timestamp > bonusEndTime,
            "Pool has started"
        );
        require(
            _startTime < _bonusEndTime,
            "New start block time must be before than new end block time"
        );
        require(
            block.timestamp < _startTime,
            "New start block time must be later than current block time"
        );

        startTime = _startTime;
        bonusEndTime = _bonusEndTime;

        emit NewStartAndEndBlockTimes(_startTime, _bonusEndTime);
    }
     ...
     */
    function updateFreezaBlockTimes(
        uint256 _freezeStartTime,
        uint256 _freezeEndTime
    ) external onlyOwner {
        require(
            _freezeStartTime < _freezeEndTime,
            "New freeze start block time must be lower than new end block time"
        );
        require(
            block.timestamp < _freezeStartTime,
            "freeze start block time must be higher than current block time"
        );

        freezeStartTime = _freezeStartTime;
        freezeEndTime = _freezeEndTime;
        emit NewFreezeBlockTimes(freezeStartTime, freezeEndTime);
    }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (startTime >= block.timestamp) {
            ...
            if (user.lastRewardTime == 0) {
                multiplier = _getMultiplier(startTime, block.timestamp);
            }
            return _getRewardAmount(user.amount, multiplier);
        }
        return 0;
    }
