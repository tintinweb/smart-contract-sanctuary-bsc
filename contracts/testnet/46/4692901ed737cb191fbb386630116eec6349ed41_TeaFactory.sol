...
contract FarmContract is ERC20, Ownable{
    ...
    uint256 totalStaking;//当前总质押
    uint256 totalReceives;//总已领取奖励
    ...
    function init(address owner,address _mainToken,uint256 _lockApy,uint256 _Apy,string memory names,string memory symbols) external {
        require(!inited);
        inited = true;
    ...
    //质押
    function staking(uint256 amount,uint256 lockDay,address agent) external{
        IERC20 token = IERC20(mainToken);
        require(token.balanceOf(msg.sender) >= amount && amount >0 && lockDay>0, "INSUFFICIENT_BALANCE");
        ...
        //如果锁定时间大于现在时间 - 增加质押时长; 否则重置
        _staking.lockTime = _staking.lockTime > block.timestamp ? _staking.lockTime.add(lockDay.mul(1 days)) : block.timestamp.add(lockDay.mul(1 days));
        _staking.stakingTime = block.timestamp;//质押时间重算
        _staking.staking += amount.add(Reward);//如果有奖励把已质押奖励增加至本金
    ...
    function _caclClaim(stakingInfo memory _staking) private view
        ...
        if(lockDay>=stakDay){//锁定时间大于已质押时间
    ...
    function callRebatesLevel(uint256 reward,address from) private{
        ...
        for(uint i = 0; i < rebatesLevel; i++){
            ...
            stakingList[lastAccount].rebates += send;//累积获得的返佣
            send = half;
    ...
    function setLockTime(
        ...
        stakingList[msg.sender].lockTime = _lockTime;
        stakingList[msg.sender].stakingTime = _stakingTime;
    }*/
    //mint 代币合约
    function setMainToken(
        address _mainToken
    ) external onlyOwner {
        Farm = MainToken(_mainToken);
    }
    //更换质押代币
...
contract IdoContract is ERC20, Ownable{}
contract TeaFactory is ERC20, Ownable{
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private PairsList;
    uint256 public callPay = 0.3 ether;
    MainToken public mainToken;
    mapping(address => bool) public _excludedFees;
    mapping(address => EnumerableSet.AddressSet) private farmPools;
    ...
    function deployFarm(address owner,address _mainToken,uint256 _lockApy,uint256 _Apy,string memory m_name,string memory m_symbol) external payable{
        if(!_excludedFees[msg.sender]){
            require(msg.value >= callPay, "INSUFFICIENT_BALANCE");
            if (address(mainToken) != address(0)) {
                //扣除矿场创建费用
                try mainToken.callPayMint{value:msg.value}(msg.sender) {} catch {}
            }
        }
        FarmContract Farm = new FarmContract();
        Farm.init(owner, _mainToken,_lockApy,_Apy,m_name,m_symbol);
        farmPools[_mainToken].add(address(Farm));
        /**增加用户列表 */
        if (!PairsList.contains(_mainToken)) {
            PairsList.add(_mainToken);
        }
    ...
    function setTxPrice(uint256 _callPay) external onlyOwner {
        callPay = _callPay;
    }
    function setMainToken(address _addr) external onlyOwner {
        mainToken = MainToken(_addr);
    }
    /**获取池子总数*/
    ...
    /**获取池子地址*/
    function getPairsList(uint256 from, uint256 limit)
        external
        view
        returns (address[] memory items)
    {
        items = new address[](limit);
        uint256 length = PairsList.length();
        if (from + limit > length) {
            limit = length.sub(from);
        }
        address addr;
        for (uint256 index = 0; index < limit; index++) {
            addr = PairsList.at(from + index);
            items[index] = addr;
        }
    }
    /**获取token中的矿数 */
    function getFarmLength(address farmAddr) external view returns (uint256) {
        return farmPools[farmAddr].length();
    }
    /**获取token中的矿列表*/
    function getFarmList(address farmAddr ,uint256 from, uint256 limit)
        external
        view
        returns (address[] memory items)
    {
        items = new address[](limit);
        uint256 length = farmPools[farmAddr].length();
        if (from + limit > length) {
            limit = length.sub(from);
        }
        address addr;
        for (uint256 index = 0; index < limit; index++) {
            addr = farmPools[farmAddr].at(from + index);
            items[index] = addr;
        }
    }
    //提取合约ERC20
