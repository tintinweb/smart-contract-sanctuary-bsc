  ...
  mapping(address => RankIncome[]) public rankIncomes;
   ...
   function initialize(address payable _ownerWallet ,address payable _supportWallet,IBEP20Token _tokenAddress) public {
    ...
    function invest(address _referrer,uint _amount) public payable {
       ...
       user.deposits.push(Deposit(_amount,0,0, block.timestamp, 0,true,0));
...
value = value.add(payMoney);
value = value.add(bonus).add(rankIncome);
...
user.deposits[user.deposits.length-1].withdrawn = user.deposits[user.deposits.length-1].withdrawn.add(roi);
user.rankWithdraw = user.rankWithdraw.add(rankIncome);
// uint totalwithdrawToken = (totalwithdrawAmount.div(tokenPrice)).mul(1e8);
user.dataWithdrwan = user.dataWithdrwan.add(bosster);
...
payable(msg.sender).transfer(totalwithdrawTokenBNb);
  ...
  function getcurrentPASSIVEincome(address userAddress) public view returns (uint256 _amount){
        ...
        (uint progress,) = progressReport(userAddress);
         if(progress < max){
             uint count =  user.deposits.length;
             if(count>0){
                _amount = getUserPassiveIncome(userAddress).sub(user.deposits[count-1].withdrawnp);
 ...
 function getLatestPrice() public view returns (int) {
  ...
  function getDepositInfo(address _user,uint index) public view returns(
    uint amountUSD,uint withdrawn,uint tokenAmount,uint start,bool roiActive,uint withdrwanUSD,uint withdrawnp){
