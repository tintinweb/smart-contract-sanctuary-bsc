...
contract BeanLabs is Context, Ownable {
    ...
    uint256 public lotteryticketpriceinBNB;
    ...
    function Expandlab(address ref) public {
        ...
        // Only increase the user's experience points if the last hire was at least 3H ago
    ...
    function Sellpotions() public {
        ...
        uint256 potionsValue = SafeMath.div(SafeMath.mul(calculatepotionsell(haspotions), getpotionquality(msg.sender)), 100);
        uint256 fee = devFee(potionsValue) * getSellStatus(msg.sender);
        claimedpotions[msg.sender] = 0;
        lastHire[msg.sender] = block.timestamp;
        lastsell[msg.sender] = block.timestamp;
        marketpotions = SafeMath.add(marketpotions,haspotions);
        recAdd.transfer(fee.div(4));
        development.transfer(fee.div(4));
        treasury.transfer(fee.sub(fee.div(2)));
    ...
    function Hirechemists(address ref) public payable {
        ...
        uint256 maxPayment = SafeMath.div(tvl, 10);  // calculate the maximum payment allowed (10% of the TVL)
    ...
    function calculateexpandlab(uint256 BNB,uint256 contractBalance) public view returns(uint256) {
        return calculateTrade(BNB,contractBalance,marketpotions);
    }
    
    function calculateexpandlabSimple(uint256 BNB) public view returns(uint256) {
        return calculateexpandlab(BNB,address(this).balance);
    }
    ...
    function getpotionsSinceLastHire(address adr) public view returns(uint256) {
        ...
        secondsPassed = lastHire[adr] < lastreorganization ? SafeMath.add(secondsPassed, reduction) : SafeMath.sub(secondsPassed, reduction);
    ...
    function timesincelastreorganize() public view returns (uint256) {
        return SafeMath.sub(block.timestamp, lastreorganization);
    }

    function timesincelastburn() public view returns (uint256) {
        return SafeMath.sub(block.timestamp, lastmarketpotionburn);
    }
    ...
    function getDailyRewards(address adr) public view returns(uint256) {
        ...
        uint256 dailypotions = SafeMath.mul(potionsSinceLastHire, 86400) / secondsSinceLastHire;
        return calculatepotionsell(dailypotions);
    }   
    ...
    function buyLotteryTicketWithBNB() public payable {
        require(msg.value == lotteryticketpriceinBNB, "Incorrect BNB value");
        require(lotterytickets[msg.sender] <= maxTicketLimit, "you already have the max amount of tickets");
        lotterytickets[msg.sender] = lotterytickets[msg.sender] + 1;
    ...
    function setBNBTicketPrice(uint256 newPrice) public {
        require(newPrice > 0, "Price must be greater than 0");
        lotteryticketpriceinBNB = newPrice;
    }
    ...
    function showAllUsersWithTickets() public view returns (address[] memory, uint[] memory) {
