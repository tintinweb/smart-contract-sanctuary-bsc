...
contract Marketplace is Ownable, MarketplaceStorage {
    ...
    function buyItem(address nftAddress, uint256 assetId) public payable {
        ...
        require(!blacklist[assetId], "Asset is backlisted");
        ...
        require(item.priceAnta == 0, "Asset must list by BNB");
        ...
        items[nftAddress][assetId].price = 0;
    ...
    function buyItemByAnta(
        ...
        require(item.price == 0, "Asset must list by ANTA");
        ...
        items[nftAddress][assetId].seller = sender;
        items[nftAddress][assetId].priceAnta = 0;
        items[nftAddress][assetId].status = ItemStatus.BOUGHT;
    ...
    function delistItem(address nftAddress, uint256 assetId) public {
        ...
        address seller = item.seller;
        require(seller != address(0), "Invalid address");
        require(seller == msg.sender, "Only seller can delist");
    ...
    function addAssetIdBlacklist(uint256[] memory assetIds) external onlyOwner {
        for (uint256 i = 0; i < assetIds.length; i++) {
            blacklist[assetIds[i]] = true;
        }
    }

    function removeAssetIdBlacklist(uint256[] memory assetIds)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < assetIds.length; i++) {
            blacklist[assetIds[i]] = false;
        }
    }
}
