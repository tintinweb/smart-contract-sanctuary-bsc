...
Ownable
{
    ...
    function batchTransferTokensFromOneToMany(address token, address from, address[] memory toAccounts, uint256 amount)
    external
    onlyOwner
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            erc20Token.transferFrom(from, toAccounts[i], amount);
        }
    }
