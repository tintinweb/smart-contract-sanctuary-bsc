...
contract LpStakingPool is USDTWrapper,Ownable {
   using SafeMath for uint256;
    ...
    event RewardPaid(address indexed user, uint256 reward, uint level);
    ...
    function getReward() public updateReward(msg.sender) checkStart {
        ...
        if (reward > 0) {
            ...
            if (referrerAddress[msg.sender] != address(0)) {
                one_reward = reward * 10 / 100;
                stakeOutToken.safeTransfer(referrerAddress[msg.sender], one_reward);
                 emit RewardPaid(msg.sender, reward, 1);
                if (referrerAddress[referrerAddress[msg.sender]] != address(0)) {
                     two_reward = reward * 20 /100;
                     stakeOutToken.safeTransfer(referrerAddress[referrerAddress[msg.sender]], two_reward);
                     emit RewardPaid(msg.sender, reward, 2);
                } 
            } 

             stakeOutToken.safeTransfer(msg.sender, reward - one_reward - two_reward);
