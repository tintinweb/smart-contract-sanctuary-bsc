...
contract Test is Ownable {
    ...
    function fundToken(address addressToken, uint256 amount) external onlyAdmin{
        IERC20 MATK = IERC20(addressToken);
        MATK.transferFrom(msg.sender ,  address(this), amount);
        tokenInContract[addressToken] += amount;
    ...
    function addTokenList(address addressToken) external onlyAdmin{
        token[numOfToken] = addressToken;
        numOfToken++;
    }
    ...
    function checkAmount(address tokenSwap, uint256 tokenFundForAccount) public view returns (bool) {
        ...
        require(tokenAllowance > tokenFundForAccount," :)))");
        return true;
    }
    ...
    function fund(address tokenAddress, uint256 funds) public isNotInBlackList{
        ERC20 eRC20 = ERC20(tokenAddress);
        require(funds < maxFunds,"> max");
        require(funds > minFunds,"< min");
        userAmount[msg.sender][tokenAddress] += funds;
        eRC20.transferFrom(msg.sender, address(this), funds);
    }

    function game(address tokenType) public isNotInBlackList{
        ERC20 eRC20 = ERC20(tokenType);
        uint256 funds = userAmount[msg.sender][tokenType];
        uint256 fundsForAccount= tokenCalculation(funds);
        if(checkAmount(tokenType,fundsForAccount) == false){
            ERC20 nERC20 = ERC20(BUSD);
            nERC20.transfer(msg.sender, fundsForAccount);
        }
        require(checkAmount(tokenType,fundsForAccount),"address not enough tokens");
        userAmount[msg.sender][tokenType] -= funds;
        eRC20.transfer(msg.sender, fundsForAccount);

    }


    function tokenCalculation(uint256 funds) public view returns(uint256){
