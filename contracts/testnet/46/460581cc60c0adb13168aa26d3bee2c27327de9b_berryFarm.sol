...
contract Ownable {
  address public CEO; 
  address payable public marketingTeam;
  address payable public employmentTeam;
  address payable public devTeam;
  uint public marketingFee = 7;
  uint public employmentFee = 5 ; 
  uint public antiwhaleOnFee = 5 ; 
  uint public antiwhaleOffFee = 50;
  ...
  constructor() {
    CEO = msg.sender;
    marketingTeam = payable(msg.sender);
    employmentTeam = payable(msg.sender);
    devTeam = payable(msg.sender);
...
contract berryFarm is Ownable {   
  ...
  function BuyBerry(address referer) external payable {
      ...
      uint marketingAmount = msg.value*marketingFee/100;
      marketingTeam.transfer(marketingAmount);

      uint employmentAmount = msg.value*employmentFee/100;
      employmentTeam.transfer(employmentAmount);
 ...
 function reinvest() external  {
  ...
  function rewardBnb(address user) public view returns (uint bnbAmt) {
    ...
    uint withdrawalFee = investor.antiwhale == true ? antiwhaleOnFee : antiwhaleOffFee;
  ...
  function SellBerry(address payable addressAt) external {
    require (msg.sender==addressAt,"address should be same");
    uint withdrawalFee = antiwhaleOffFee;
    if(investors[addressAt].antiwhale == true) {
      ...
      withdrawalFee = antiwhaleOnFee;
    }
    uint amount = profit();
    uint feeAmount = (amount*withdrawalFee)/100;
    uint withdrawFeeAmtBnb = (feeAmount*(10**18))/berryPricePerBnb;
    
    uint withdrawableAmt = amount - feeAmount;
    uint bnbAmt = (withdrawableAmt*(10**18))/berryPricePerBnb;
    require(amount >= 1 );
      addressAt.transfer(bnbAmt);
      devTeam.transfer(withdrawFeeAmtBnb); // send fee
