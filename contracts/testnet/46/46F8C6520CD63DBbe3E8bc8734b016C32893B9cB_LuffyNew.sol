...
contract LuffyNew is IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if (isAntiSniperEnable()) {
            if (isContract(from) && from != address(dexRouter) && from != pairAddress) {
                revert("Do not accept transactions originating from a contract.");
            }
        }
        //prevent any wallet hold more than 1.5% supply except address(0) & pair
        ...
        if ((currentWalletBalance + amount) > maximumAmountTokenPerWallet) {
            if (to != address(0) && to != pairAddress) {
                revert("A wallet can only hold up to 1.5% total supply.");
            }
        }
