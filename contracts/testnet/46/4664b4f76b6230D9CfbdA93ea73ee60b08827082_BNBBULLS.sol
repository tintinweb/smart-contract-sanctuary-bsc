 ...
 */
contract BNBBULLS is ERC721Base {
    ...
    string private _name = "BNBBULLS";
    string private _symbol = "BnbBull";
    string private baseUrl;

    uint256 public immutable maxMintPerTx = 30;
    uint256 public immutable maxSupply = 1111;
    uint256 public immutable presaleDuration = 30 minutes;
    uint256 public immutable presaleMaxTokens = 11;
    uint256 public immutable presaleMaxSpots = 600;

    uint256 public price = 0.15 ether;
    uint256 public pricePresale = 0.12 ether;
     ...
     */
    function mintPresale(address player, uint256 _amount) external payable {
     ...
     * @param _amount: amount of nfts to mint
     */
    function mint(address player, uint256 _amount) external payable {
        ...
        require(player != address(0), "ERC20: mint from the zero address");
        require(_amount > 0 && _amount <= maxMintPerTx, "Invalid amount");
    ...
    function awardItem(address player, uint256 _amount) public payable {
        require(saleIsActive, "Sale not started yet");
        require(block.timestamp >= presaleEnd || totalSupply() >= presaleMaxTokens, "Wait for presale to end");
        require((totalSupply() + _amount) <= maxSupply, "Exceeds max supply");
        require(player != address(0), "ERC20: mint from the zero address");
    ...
    function ownerAwardItem(address player, uint256 _amount)
        public
        payable
        onlyOwner
    {
