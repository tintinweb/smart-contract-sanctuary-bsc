...
contract INFLIVFarm is Initializable, OwnableUpgradeable{
	...
	function activePackage(uint256 package, address sponsor) external {
		...
		require(ERC20Upgradeable(rewardToken).balanceOf(msg.sender) >= tokenRequired, "balance not available for activation");
		...
		ERC20Upgradeable(rewardToken).transferFrom(address(msg.sender), address(this), tokenRequired);
		...
		for(i=0; i < 256; i++) 
		{
			...
			{   
                if(lastActiveTime[nextReferrer] + 30 days >= block.timestamp && 21 > level)
				{
				    ...
				    IERC20Upgradeable(rewardToken).transfer(address(nextReferrer), reward);
				    amountUsed += reward;
		...
		if(stakingID == 0)
		{
			if(mapUserInfo[msg.sender][stakingID].amount > 0) 
			{
			   ...
			   if(pending > 0) 
			   {
				   ...
				   IGovernance(rewardToken).distributeFee(governanceTax);
				   ERC20Upgradeable(rewardToken).transfer(address(msg.sender), pending - governanceTax);
			   }
			}
			else
			{
			...
			totalStaked += amount;
			ERC20Upgradeable(stakedToken[0]).transferFrom(address(msg.sender), address(this), amount);
		} 
		else 
		{
			...
			{
			    ERC20Upgradeable(stakedToken[1]).transferFrom(address(msg.sender), address(this), amount);
			}
		}
	    ...
	    if(mapUserInfo[msg.sender][stakingID].amount > 0) 
		{
			...
			if (pending > 0) 
			{
			   ...
			   {   
			       ...
			       ERC20Upgradeable(stakedToken[0]).transfer(address(msg.sender), mapUserInfo[msg.sender][stakingID].amount);
			   ...
			   else
			   {
				   ...
				   for(uint i=0; i < ids.length; i++)
				   {
				       if(mapUserInfo[msg.sender][stakingID].deposit[stakingID][ids[i]].locked == 1 && mapUserInfo[msg.sender][stakingID].deposit[stakingID][ids[i]].endTime <= block.timestamp)
					   {
						  ...
						  if(mapUserInfo[msg.sender][stakingID].farm == 1)
						  {
						      ERC20Upgradeable(stakedToken[0]).transfer(address(msg.sender), mapUserInfo[msg.sender][stakingID].deposit[stakingID][ids[i]].amount);
						  }
						  else
						  {
						      ERC20Upgradeable(stakedToken[1]).transfer(address(msg.sender), mapUserInfo[msg.sender][stakingID].deposit[stakingID][ids[i]].amount);
						  }
	...
	function updatePool(uint256 amount) external {
		ERC20Upgradeable(rewardToken).transferFrom(address(msg.sender), address(this), amount);
		if(totalStaked > 0)
		{
	...
	function migrateTokens(address tokenAddress, address receiver, uint256 tokenAmount) external onlyOwner{
	   ...
	   require(ERC20Upgradeable(tokenAddress).balanceOf(address(this)) >= tokenAmount, "Insufficient balance on contract");
	   
	   ERC20Upgradeable(tokenAddress).transfer(address(receiver), tokenAmount);
	...
	function setRewardToken(address tokenAddress) external onlyOwner{
	   ...
	   ERC20Upgradeable(rewardToken).approve(address(rewardToken), type(uint256).max);
