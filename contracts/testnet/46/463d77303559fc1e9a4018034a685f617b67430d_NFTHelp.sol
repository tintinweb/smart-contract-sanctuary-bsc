...
contract NFTHelp is Ownable {
    ...
    mapping(uint256 => uint256[]) public sellGroups;
    mapping(uint256 => uint256) public sellPrices;
    mapping(uint256 => address) public sellCoinType;
    uint256[] public sellidx;
    uint256 public GroupIdx = 1;
    ...
    function sellBox(uint256 _num, uint256 _price, address _coinType) external lock haveNft returns(uint256 _groupID) {
        require(_coinType == payToken || _coinType == payNFTToken, "cointype error");
    ...
    function sellHero(uint256[] memory _tokens, uint256 _price, address _coinType) public lock haveNft haveHashpool returns(uint256 _groupID) {
        // require(NFTToken != address(0),"NFTToken error");
        require(_coinType == payToken || _coinType == payNFTToken, "cointype error");
        IHashratePool(HashratePool).settlementAll();
