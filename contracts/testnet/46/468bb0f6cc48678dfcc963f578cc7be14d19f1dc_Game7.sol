...
contract Game7 is Ownable  {
    ...
    function smallAttack(uint256 _tokenId) external ownerNft(_tokenId) {
        ...
        if(status){
            _setResultLastOfToken(_tokenId, 10);
        }else{
            _setResultLastOfToken(_tokenId, 1);
        }
    }
    ...
    function _setResultLastOfToken(uint256 _tokenId, uint256 _result) private{
        resultLastOfToken[_tokenId] = _result;
    }
    ...
    function getResultLastOfToken(uint256 _tokenId) external view returns(uint256){
        return resultLastOfToken[_tokenId];
    }
