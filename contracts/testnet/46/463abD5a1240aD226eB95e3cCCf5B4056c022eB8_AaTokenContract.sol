...
contract AaTokenContract is IERC20 {
    ...
    string constant private _name = "Crypto FC";
    string constant private _symbol = "CFC";
    ...
    struct TaxWallets {
        ...
        address payable betting;
    }

    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d),
        betting: payable(0x60808eaaED5AfA68DC2d7B1b8B3efF84c846754d)
    });
...
//===============================================================================================================
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 betting) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.marketing = marketing;
        _ratios.betting = betting;
        _ratios.totalSwap = liquidity + marketing + betting;
        uint256 total = _taxRates.buyFee + _taxRates.sellFee;
        require(_ratios.totalSwap <= total, "Cannot exceed sum of buy and sell fees.");
    }

    function setWallets(address payable marketing, address payable betting) external onlyOwner {
        require(marketing != address(0) && betting != address(0), "Cannot be zero address.");
        _taxWallets.marketing = payable(marketing);
        _taxWallets.betting = payable(betting);
    }
    ...
    function contractSwap(uint256 contractTokenBalance) internal inSwapFlag {
        ...
        bool success;
        uint256 bettingBalance = (amtBalance * ratios.betting) / ratios.totalSwap;
        uint256 marketingBalance = amtBalance - bettingBalance;
        if (ratios.marketing > 0) {
            (success,) = _taxWallets.marketing.call{value: marketingBalance, gas: 35000}("");
        }
        if (ratios.betting > 0) {
            (success,) = _taxWallets.betting.call{value: bettingBalance, gas: 35000}("");
        }
    }
