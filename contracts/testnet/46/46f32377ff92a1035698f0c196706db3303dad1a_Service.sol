...
interface IService {
    ...
    function aquaLockOf(address collection, uint256 tokenId) external view returns (uint256);

    event GammaLock(address indexed collection, uint256 indexed tokenId, uint256 amount, address account);

    event AquaLock(address indexed collection, uint256 indexed tokenId, uint256 amount, address account);

    event GammaUnlock(address indexed collection, uint256 indexed tokenId, uint256 amount, address account);

    event AquaUnlock(address indexed collection, uint256 indexed tokenId, uint256 amount, address account);

    event TokenBurn(address indexed collection, uint256 indexed tokenId);
}

// File: Pink/Context.sol
...
contract Service is Ownable, IService {
    ...
    mapping(address => mapping(uint256 => uint256)) _aquaLocks;
    ...
    // ----- EVENTS ----- //
    event KOTH_ENABLED(address indexed collection, uint96 feeNumerator, address indexed owner);
    event KOTH_SET(address indexed collection, uint96 feeNumerator, address indexed owner);
    event KOTH_TAKEN(address indexed collection, uint256 indexed tokenId, uint256 price, address indexed owner);

    // ----- CONSTRUCTOR ----- //
    constructor(address gamma, address aqua, address mintMachine) {
        _gammaToken = IERC20(gamma);
        _aquaToken = IERC20(aqua);
        _platformAddress = _msgSender();
        _gammaDecimals = IERC20Metadata(gamma).decimals();
    ...
    function aquaLockOf(address collection, uint256 tokenId) external view override returns (uint256) {
        return _aquaLocks[collection][tokenId];
    }
    ...
    // ----- MUTATION FUNCTIONS ----- //
    function lockGamma(address collection, uint256 tokenId, uint256 amount) override external {
        ...
        emit GammaLock(collection, tokenId, amount, _msgSender());
    }

    function lockAqua(address collection, uint256 tokenId, uint256 amount) external {
        require(amount > 0, "Service: amount is zero");
        require(_aquaToken.transferFrom(_msgSender(), address(this), amount), "Service: failed to transfer tokens for lock");

        _aquaLocks[collection][tokenId] += amount;

        emit AquaLock(collection, tokenId, amount, _msgSender());
    }

    function unlock(address collection, uint256 tokenId) external {
        require(IERC721(collection).ownerOf(tokenId) == _msgSender(), "Service: caller is not the token owner");

        if(_gammaLocks[collection][tokenId] > 0) {
            require(_gammaToken.transfer(_msgSender(), _gammaLocks[collection][tokenId]), "Service: failed to transfer tokens for unlock");
            
            emit GammaUnlock(collection, tokenId, _gammaLocks[collection][tokenId], _msgSender());
            _gammaLocks[collection][tokenId] = 0;
        }

        if(_aquaLocks[collection][tokenId] > 0) {
            require(_aquaToken.transfer(_msgSender(), _aquaLocks[collection][tokenId]), "Service: failed to transfer tokens for unlock");

            emit AquaUnlock(collection, tokenId, _aquaLocks[collection][tokenId], _msgSender());
            _aquaLocks[collection][tokenId] = 0;
        }

        IERC721(collection).safeTransferFrom(_msgSender(), DEAD, tokenId);

        emit TokenBurn(collection, tokenId);
    }
    ...
    function enableKotH(address collection, uint96 feeNumerator) external {
        ...
        emit KOTH_ENABLED(collection, feeNumerator, _msgSender());
    }
    ...
    function setKotH(address collection, uint96 feeNumerator) external {
        ...
        emit KOTH_SET(collection, feeNumerator, _msgSender());
    }

    function takeKotH(address collection, uint256 tokenId, uint256 price) external {
        require(_isKotH[collection], "Service: KotH is disabled");
        require(price >= _koths[collection][tokenId].price * 110 / 100 && price >= 10 ** _gammaDecimals, "Service: price is below the min requirement");
        ...
        emit KOTH_TAKEN(collection, tokenId, price, _msgSender());
    }
}
