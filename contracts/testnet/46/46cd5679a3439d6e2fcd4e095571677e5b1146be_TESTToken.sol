...
contract TESTToken is ERC20, Ownable {
    ...
    uint256 public ultraBurnFee = 5;
     uint256 public treasuryFee = 3;
    uint256 public sellFeeTreasuryAdded = 2;
    uint256 public sellfeeUBAdded = 5;
 uint256 public totalBuyFee = treasuryFee.add(ultraBurnFee);
   uint256 public totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellfeeUBAdded);
bool public pauseSell = false;
    address public ultraBurnReceiver = 0x9CCE932283183F637e4870a63bDf1e6C348DbB64;
    address public  treasuryReceiver = 0x94DC0b13E66ABa9450b3Cc44c2643BBb4C264BC7;
    ...
    function setMaxSellTransaction(uint256 _maxTxn) external onlyOwner {
     ...
     function setFees( uint256 _treasuryFee, uint256 _ultraBurnFee, uint256 _sellFeeTreasuryAdded,  uint256 _sellfeeUBAdded) external onlyOwner {
        require(
            _treasuryFee <= MAX_FEE_RATE &&
           _ultraBurnFee <= MAX_FEE_RATE &&
            _sellFeeTreasuryAdded <= MAX_FEE_RATE &&
            _sellfeeUBAdded <= MAX_FEE_RATE,
            "wrong"
        );

      
        ultraBurnFee = _ultraBurnFee;
         treasuryFee = _treasuryFee;
        sellFeeTreasuryAdded = _sellFeeTreasuryAdded;
        sellfeeUBAdded = _sellfeeUBAdded;
        totalBuyFee = treasuryFee.add(sellfeeUBAdded);
        totalSellFee = totalBuyFee.add(sellFeeTreasuryAdded).add(sellfeeUBAdded);

       
    }
      function setFeeReceivers( address _ultraBurnReceiver, address _treasuryReceiver) external onlyOwner {
        ultraBurnReceiver = _ultraBurnReceiver;
