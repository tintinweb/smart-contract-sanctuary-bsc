...
contract MinosisV1 is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function feeDistribution(address feeWalletAddress, uint16 walletType) internal {
        if(enableFee){
            enableFee = false;
        ...
        transferBNBToAddress(payable(feeWalletAddress), currentBalance, walletType = 1);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tMarketingFee, uint256 tDevelopmentFee, uint256 tBurnFee) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rMarketingFee, uint256 rDevelopmentFee, ) = getRValues(tAmount, tFee, tMarketingFee, tDevelopmentFee, tBurnFee);
        {
            address from = sender;
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tMarketingFee - tDevelopmentFee - tBurnFee;
        return (tTransferAmount, tFee, tMarketingFee, tDevelopmentFee, tBurnFee);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tMarketingFee, uint256 tDevelopmentFee, uint256 tBurnFee) internal view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 currentRate = getRate();
        uint256 tAmount = amount;
        uint256 rAmount = tAmount * currentRate;
        uint256 rFee = tFee * currentRate;
        uint256 rMarketingFee = tMarketingFee * currentRate;
        uint256 rDevelopmentFee = tDevelopmentFee * currentRate;
        uint256 rBurnFee = tBurnFee * currentRate;
        uint256 rTransferAmount = rAmount - rFee - rMarketingFee - rDevelopmentFee - rBurnFee;
        return (rAmount, rTransferAmount, rFee, rMarketingFee, rDevelopmentFee, rBurnFee);
    }
    ...
    function takeBurnFee(uint256 tBurnFee) internal {
        ...
        buyBackWalletAmount += tBurnFee;
    }

    function removeAllFee() internal {
        if(taxFee == 0 && marketingFee == 0 && developmentFee == 0 && burnFee == 0) return;
