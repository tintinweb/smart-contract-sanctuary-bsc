...
contract EPICAvatar is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable, AccessControl{
    ...
    constructor(address _redeemToken, string memory name, string memory symbol) ERC721(name, symbol) {
    ...
    function safeMint(address owner, string memory uri, uint256 tokenId) public onlyRole(MINTER_ROLE) {
    ...
    function buy(uint256 clan) public {
        ...
        require(balanceOf(msg.sender) <= 5, "Avatar: Maximum hold only 5 avatars per wallet");
        require(token.balanceOf(msg.sender) >= _totalBalance,"Token: Token not enough");
        require(token.allowance(msg.sender, address(this)) >= _totalBalance,"Token: Allowance isn't allocate");
        ...
        uint256 _random = uint256(keccak256(abi.encodePacked(text,block.difficulty,block.timestamp,block.number,block.difficulty)));
        ...
        clans[clan].tokenId[_index] = clans[clan].tokenId[clans[clan].tokenId.length - 1];
