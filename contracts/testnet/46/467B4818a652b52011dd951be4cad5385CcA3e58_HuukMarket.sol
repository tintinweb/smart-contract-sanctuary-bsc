...
//                )
    ...
    function _match(
        ...
        if (buyerRef != address(0)) {
            uint256 amountToBuyerRef = (orderAmount * xUser * (ZOOM_FEE - yRefRate)) / (ZOOM_FEE**2);
            ...
            if (amountFirstSale[order.tokenAddress][order.tokenId] > 0 && (creator == order.owner)) {
                if (sellerRef != address(0)) {
                    ...
                    if (amountFirstSale[order.tokenAddress][order.tokenId] >= _quantity) {
                        amountToSellerRef = (orderAmount * xCreator * (ZOOM_FEE - yRefRate)) / (ZOOM_FEE**2);
                        // take 15%*50% fee
                        amountToSeller = amountToSeller - ((orderAmount * xCreator) / ZOOM_FEE);
                        // take 15% fee
                        amountFirstSale[order.tokenAddress][order.tokenId] = amountFirstSale[order.tokenAddress][order.tokenId] - _quantity;
                    } else {
                        uint256 a = amountFirstSale[order.tokenAddress][order.tokenId] * _price * xCreator;
                        amountToSeller = amountToSeller - (a / ZOOM_FEE);
                    ...
                    if (amountFirstSale[order.tokenAddress][order.tokenId] >= _quantity) {
                        ...
                        uint256 b = (amountFirstSale[order.tokenAddress][order.tokenId] * _price * xCreator) / ZOOM_FEE;
