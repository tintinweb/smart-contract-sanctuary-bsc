...
contract WomensWorld is ERC721A, Ownable{
    ...
    function mint(uint256 quality, bytes32[] memory proof) public payable {
        require(quality + totalSupply() <= maxSupply, "NFT supply is full");

        if (!excludedAccount[msg.sender]) {
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            bool whitelisted = MerkleProof.verify(proof, whitelistRoot, leaf);
            uint256 payFee;
            require(quality <= 5, "Exceed maximum mint count");
            if (!isPublic) {
                uint256 minted = mintedNumber[msg.sender];
                require(whitelisted, "Sender is not in whitelist");
                require(minted + quality <= 5 && balanceOf(msg.sender) + quality <= 5 , "Exceed maximum");
                if (minted + quality > 2 && minted < 2) {
                    payFee = whitePrice * (quality - minted);
                }
                else if (minted + quality > 2 && minted > 2) {
                    payFee = whitePrice * quality;
                }
    
                whiteMintedNumber[msg.sender] += quality;
            } else {
                if (whitelisted) {
                    require(balanceOf(msg.sender) + quality <= 8, "Exceed maximum");
                    uint256 whited = whiteMintedNumber[msg.sender];
                    if (whited < 3) {
                        if (quality > whited) {
                            payFee = blackPrice * (quality - whited);
                            whiteMintedNumber[msg.sender] = 3;
                        }
                        else payFee = blackPrice * quality;
