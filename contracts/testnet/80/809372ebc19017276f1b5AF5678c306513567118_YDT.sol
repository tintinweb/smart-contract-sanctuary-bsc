...
contract YDT is ERC20, AccessControl, ReentrancyGuard {
    ...
    mapping(address => uint256) public YDTpoolFund;
    ...
    constructor (string memory _name, string memory _symbol, address _RYDTtokenAddress) ERC20(_name, _symbol) {
    ...
    function getPriceData(address _token, uint256 _amount) public view returns (uint256) {
        Token memory acceptedToken = acceptedTokens[_token];
        require(acceptedToken.accepted, "Token not accepted");
        uint256 _price;
    ...
    function _toPrecision(
        ...
        uint256 tokenUnit = _amount / (10**uint128(n));
    ...
    function donateToProject(uint _proposalId, uint256 _amount, address _token) public nonReentrant {
        require(isAcceptedToken[_token], "Unaccepted token");
        ...
        uint256 projectShare = (_amount * 70) / 100;
        uint256 poolShare = (_amount * 30) / 100;
        erc20.safeTransferFrom(msg.sender, proposer, _amount);
        uint fund = getPriceData(_token, _amount);
        _transfer(address(this),msg.sender, fund);
        uint256 donationValue = fund * 70 / 100;
        YDTpoolValue += fund * 30 / 100;
        simiDAO.receivedDonation(_proposalId, donationValue);
        erc20.safeTransferFrom(msg.sender, address(this), _amount);
        YDTpoolFund[_token] += poolShare;
        totalDeposit[_token] += _amount;
        emit DepositToken(msg.sender, _token, _amount, fund);
    ...
    function claimLotteryPrize(uint256 _amount, address _token) public {
        Token memory acceptedToken = acceptedTokens[_token];
        require(acceptedToken.accepted, "Token not accepted");
        require(isRydtSet, "RYDT has not been set");
        ...
        int128 decimalFactor = int128(acceptedToken.priceFeedPrecision) - int128(acceptedToken.decimals);
        uint256 tokenUnit = _amount / (10**uint128(acceptedToken.decimals - 3));
        uint256 total = tokenUnit / _price;
        if (decimalFactor > 0) {
            total = total / (10**uint128(decimalFactor - 3));
        } else if (decimalFactor < 0) {
            total = total * (10**uint128(-1 * decimalFactor - 3));
        }
        erc20.safeTransferFrom(address(this), msg.sender, total);
        YDTpoolValue -= total;
        YDTpoolFund[_token] -= _amount;
        emit ClaimedPrize(_token, _amount);
    }
    /**
