...
contract NonFungibleToken is Ownable, ERC721A, ERC721APausable {
    ...
    address payable public fundsWallet = payable(0x61A7f53f78cf61Ed93dd5906101755E3D482F962);
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(string memory name, string memory symbol, uint _totalSupply, address _owner)  ERC721A(name, symbol) {
        transferOwnership(_owner);
        maxToMint = _totalSupply;
...
contract Factory {
   ...
   function setAdmin(address _admin) public {
       require(admin == _admin, "ERROR: must be an admin");
       admin = payable(_admin);
   }

   function addMinter(address _minter) public {
       require(admin == msg.sender, "ERROR: must be an admin");
       minter[_minter] = true;
   }
   ...
   function CreateNFT(string memory name, string memory symbol, uint _totalSupply, address _owner) public payable returns(address){

     require(msg.value >= deploymentFee, "ERROR: must pay required fees");
     require(minter[msg.sender], "ERROR: must be an admin");

     (bool success, ) = admin.call{ value: msg.value }("");
     require(success, "Address: unable to pay admin");

     NonFungibleToken newNFT = new NonFungibleToken(name, symbol, _totalSupply, _owner);
