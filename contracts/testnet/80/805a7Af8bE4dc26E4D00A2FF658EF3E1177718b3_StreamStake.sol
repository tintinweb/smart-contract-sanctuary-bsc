...
contract StreamStake is AccessControl, ReentrancyGuard {
    ...
    uint256 public stakeRewardFactor90;
    ...
    constructor(address _stakingToken, address _rewardToken) {
        ...
        // set some defaults
        lockTimePeriod = [30, 60, 90];
        stakeRewardFactor30 = 1000 * 1 days;
        stakeRewardFactor60 = 800 * 1 days;
        stakeRewardFactor90 = 600 * 1 days;
    ...
    function setStakeRewardFactor90(uint256 _stakeRewardFactor)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        stakeRewardFactor90 = _stakeRewardFactor;
        emit StakeRewardFactorChanged(_stakeRewardFactor);
    }
        ...
        returns (uint256)
    {
        ...
        if (_lockTimePeriod == 30) {
            user.stakeRewardFactor = stakeRewardFactor30;
        } else if (_lockTimePeriod == 60) {
            user.stakeRewardFactor = stakeRewardFactor60;
        } else if (_lockTimePeriod == 90) {
            user.stakeRewardFactor = stakeRewardFactor90;
        } else {
            revert("Invalid lock time period");
        }

        user.unlockTime = toUint48(
            block.timestamp + _lockTimePeriod.mul(86400)
        );
        user.lockedTime = _lockTimePeriod.mul(86400);
