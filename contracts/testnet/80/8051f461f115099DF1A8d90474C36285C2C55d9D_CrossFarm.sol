...
contract CrossFarm is WiredFarm, ICrossFarm, BaseRelayRecipient, SessionManager {
    ...
    // uint256 public nonVestBurnRate;
    // address public compoundFeeAccount;
    // uint256 public compoundFeeRate;
    // address public instantClaimFeeAccount;
    // uint256 public instantClaimFeeRate;

    string private constant sForbidden = "Forbidded";
    ...
    modifier canWire() override virtual {
        require(_msgSender() == owner(), sForbidden);
        _;
    }
    ...
    function initialize(
        ...
        require(_msgSender() == IWiredToken(_crss).getOwner(), sForbidden);
        _owner = _msgSender();
        ...
        feeParams.treasuryAccount = 0x23C6D84c09523032B08F9124A349760721aF64f6;
        feeParams.crssReferral = address(0);
        feeParams.referralCommissionRate = 100; // 0.1%
        feeParams.maximumReferralCommisionRate = 1000; // 1.0%
        feeParams.nonVestBurnRate = 25000; // 25.0%
        feeParams.compoundFeeAccount = 0x23C6D84c09523032B08F9124A349760721aF64f6;
        feeParams.compoundFeeRate = 10000; // 25%
        feeParams.instantClaimFeeAccount = 0x23C6D84c09523032B08F9124A349760721aF64f6;
        feeParams.instantClaimFeeRate = 25000; // 25.0%
        ...
        poolInfo.push(
            PoolInfo({
                ...
                OnOff: Struct_OnOff(0, 0, SubPool(0, 0)),
                OnOn: Struct_OnOn(0, 0, SubPool(0, 0), SubPool(0, 0)),
                OffOn: Struct_OffOn(0, 0, SubPool(0, 0), SubPool(0, 0)),
                OffOff: Struct_OffOff(0, 0, SubPool(0, 0))
            })
        );
        bonusMultiplier = 1;
    ...
    function setToken(address _token) public override(WiredFarm) virtual {
    ...
    function setTreasuryAddress(address _treasuryAccount) external {
        require(_msgSender() == feeParams.treasuryAccount && _treasuryAccount != address(0), sForbidden);
        emit SetTreasuryAddress(feeParams.treasuryAccount, _treasuryAccount);
        feeParams.treasuryAccount = _treasuryAccount;
    }
    ...
    function setReferralCommissionRate(uint256 _referralCommissionRate) external override virtual onlyOwner {
        require(
            _referralCommissionRate <= feeParams.maximumReferralCommisionRate,
    ...
    /**
    * @dev Set the basice fees applied to collecting users' rewards.
    * No need to check integrity becasue collect fees are applied sequentially, not in parallel, to principal.
    */
    function setNonVestBurn(uint256 _nonVestBurnRate) external onlyOwner {
        feeParams.nonVestBurnRate = _nonVestBurnRate;
    }

    function setCompoundingFee(address _compoundFeeAccount, uint256 _compoundFeeRate) external onlyOwner {
        require(_compoundFeeAccount != address(0), sForbidden);
        feeParams.compoundFeeAccount = _compoundFeeAccount;
        feeParams.compoundFeeRate = _compoundFeeRate;
    }

    function setInstantClaimFee(address _instantClaimFeeAccount, uint256 _instantClaimFeeRate) external onlyOwner {
        require(_instantClaimFeeAccount != address(0), sForbidden);
        feeParams.instantClaimFeeAccount = _instantClaimFeeAccount;
        feeParams.instantClaimFeeRate = _instantClaimFeeRate;
    }
    ...
    */
    function add(
        ...
        IERC20Upgradeable lpToken = IERC20Upgradeable(_lpToken);

        require(_depositFeeRate <= depositFeeLimit, "Deposit fee out of range");
        require(pairs[address(lpToken)].token0 != address(0), "Invalid LP token");

        // Check if this duplicates a lp token
        for (uint i = 0; i < poolInfo.length; i++) {
            if (poolInfo[i].lpToken == lpToken) {
                revert("Duplicated LP token");
            }
        }
        ...
        uint256 lastRewardBlock = startBlock < block.number ? block.number : startBlock;
        require(10 <= _allocPoint && _allocPoint <= 100, "Invalid alloc point");
    ...
    */
    function set(
        ...
        bool _withUpdate,
        uint256 _depositFeeRate
    ) public override onlyOwner validPid(_pid) {
        require(_depositFeeRate <= depositFeeLimit, "Deposit fee out of range");
        if (_withUpdate) {
            massUpdatePools();
        }
        uint256 prevAllocPoint = poolInfo[_pid].allocPoint;
        require(_allocPoint > 10 && _allocPoint < 100, "Invalid alloc point");
    ...
    /**
    * @dev Get the pools integral and consistent.
    * Note: The first, default, Crss staking pool has 25% of the total allocation points.
    */
    function updateStakingPool() internal {
        ...
        if (points != 0) {
            points = points / 4; // --------------- Crss pool will have 20% of rewards.
    ...
    * Note: Make sure the migrator creates a CrossPair type pair.
    */
    ...
    /**
    * Migrate the give pool. Can only be called by the owner.
    * Note: Make sure the migrator creates a CrossPair type pair.
    */
    function migrate(uint256 _pid) public override onlyOwner {
        ...
        require(bal == newLpToken.balanceOf(address(this)), sForbidden);
        pool.lpToken = newLpToken;
        
        emit Migrate(address(lpToken), address(newLpToken));
    }
    ...
    */
    ...
    function pendingCrss(uint256 _pid, address _user) external view override validPid(_pid) returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        return user.amount * accCrssPerShare / 1e12 - user.rewardDebt;
    }
    ...
    */
    function massHarvestRewards() public override virtual returns (uint256 rewards) {
        ...
        for (uint256 pid = 0; pid < len; pid ++) {
            ...
            if (user.accumulatedRewards > 0) {  // updating gas spending is 4 times reading gas spending, for storage slots.
                rewards += user.accumulatedRewards;
                user.accumulatedRewards = 0;
            }
        }

        if (rewards > 0) {
            FarmLibrary.tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, _msgSender(), rewards);
        }
    }
    ...
    */
    function massStakeRewards() external override virtual {
        uint256 rewards = massHarvestRewards();
        enterStaking(rewards);
    }
    ...
    function massCompoundRewards() external override virtual {
        ...
        for (uint256 pid = 0; pid < len; pid ++) {
            ...
            UserInfo storage user = userInfo[pid][userAddr];
            uint256 newLpAmount = FarmLibrary.changeCrssInXTokenToLpInFarm(
                address(pool.lpToken), crossContracts, user.accumulatedRewards);
            FarmLibrary.rebaseDeposit(pool, user, newLpAmount, true);  // true: addNotSubract
            user.accumulatedRewards = 0;
        }
    }
    ...
    */
    ...
    function _updatePool(uint256 _pid) internal validPid(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 crssReward = multiplier.mul(crssPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        ICrssToken(crossContracts.token).mint(crossContracts.xToken, crssReward);
    ...
    */
    ...
    function dailyPatrol() public override virtual returns (bool done) {
        ...
        uint256 currDay = block.timestamp /  (1 days);
        if (lastPatrolDay < currDay ) {
            ...
            for (uint256 pid; pid < poolInfo.length; pid ++) {
                ...
                FarmLibrary.takeRewardsCollectively(pool, user, msgSender, feeParams, crossContracts);
            }
            lastPatrolDay = currDay;
    ...
    */
    function switchCollectOption(uint256 _pid, CollectOption newOption) public override validPid(_pid) {
        ...
        _updatePool(_pid);
        if (dailyPatrol() == false)
        FarmLibrary.takePendingReward(pool, user, _msgSender(), feeParams, crossContracts);
        ...
        if (orgOption != newOption) {
            ...
            FarmLibrary.rebaseDeposit(pool, user, userAmount, false); // false: addNotSubract
            user.collectOption = newOption;
            FarmLibrary.rebaseDeposit(pool, user, userAmount, true); // true: addNotSubract

            emit SwitchCollectOption(_msgSender(), _pid, newOption);
        }
    }
    ...
    */
    function deposit(uint256 _pid, uint256 _amount) public override validPid(_pid) {
    ...
    /**
    * @dev Deposit CRSS tokens to gain reward emission.
    */
    function enterStaking(uint256 _amount) public override {
        _openSession(SessionType.EnterStaking);
    ...
    */
    function _deposit(uint256 _pid, uint256 _amount) internal validPid(_pid) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        ...
        if (_amount > 0) {
            ...
            // If Pool's deposit fee rate is bigger than zero, slice fee and send it to devAddr and treasuryAccount
            if (sessionParams.isOriginAction && pool.depositFeeRate > 0) {
                uint256 depositFee = _amount * pool.depositFeeRate / FeeMagnifier;
                uint256 treasury = depositFee / 2;
                pool.lpToken.safeTransfer(feeParams.treasuryAccount, treasury);
                (address devFeeAddr, , ,) = sessionFees.feeStores();
                pool.lpToken.safeTransfer(devFeeAddr, depositFee - treasury);
                _amount -= depositFee;
            }

            FarmLibrary.rebaseDeposit(pool, user, _amount, true); // true: addNotSubract
        }
    ...
    */
    function withdraw(uint256 _pid, uint256 _amount) public override validPid(_pid) {
    ...
    /**
    * @dev Withdraw CRSS tokens deposited in the past.
    */
    function leaveStaking(uint256 _amount) public override {
        _openSession(SessionType.LeaveStaking);
    ...
    /**
    * @dev Implement the withdraw function.
    */
    function _withdraw(uint256 _pid, uint256 _amount) internal virtual {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        require(user.amount >= _amount, "Exceeds balance");
        ...
        if (_amount > 0) {
            ...
            FarmLibrary.rebaseDeposit(pool, user, _amount, false); // false: addNotSubract
        }
        emit Withdraw(_msgSender(), _pid, _amount);
    }

    /**
    * @dev Find the total amoutn of unlocked Crss amount kept in the user's vesting process.
    */
    function totalWithdrawableVest(uint256 _pid) public view validPid(_pid) returns (uint256 amount) {
        UserInfo storage user = userInfo[_pid][_msgSender()];
        VestChunk[] storage vestList = user.vestList;

        amount = FarmLibrary.getMatureVestPieces(vestList, vestMonths);
    }

    function vestAccumulated(uint256 _pid) public override virtual validPid(_pid) {
        _openSession(SessionType.VestAccumulated);

        UserInfo storage user = userInfo[_pid][_msgSender()];
        uint256 amount = user.accumulatedRewards;

        if (amount > 0) {
            if (sessionParams.isOriginAction) {
                FeeRates memory rates; (rates.develop, rates.buyback, rates.liquidity, rates.treasury) 
                = _getFeeRates(sessionParams.sessionType);
                amount -= _payFee(crossContracts.xToken, amount, rates, false);
            }

            user.vestList.push(
                VestChunk( {
                    principal: user.accumulatedRewards,
                    withdrawn: 0,
    ...
    function compoundAccumulated(uint256 _pid) public override virtual validPid(_pid) {
        ...
        if (amount > 0) {
            ...
            uint256 newLpAmount = FarmLibrary.changeCrssInXTokenToLpInFarm(
                address(pool.lpToken), crossContracts, amount);
            FarmLibrary.rebaseDeposit(pool, user, newLpAmount, true);  // true: addNotSubract
    ...
    function claimAccumulated(uint256 _pid) public override virtual validPid(_pid) {
        _openSession(SessionType.ClaimAccumulated);
        ...
        if (amount > 0) {
            ...
            FarmLibrary.tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, _msgSender(), amount);
    ...
    */
    ...
    function withdrawVest(uint256 _pid, uint256 _amount) public override validPid(_pid) {
        _openSession(SessionType.WithdrawVest); 

        dailyPatrol();

        require(_amount < ICrssToken(crossContracts.token).balanceOf(address(this)), sForbidden );

        UserInfo storage user = userInfo[_pid][_msgSender()];
        VestChunk[] storage vestList = user.vestList;

        FarmLibrary.tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, _msgSender(), _amount);
        emit WithdrawVest(_msgSender(), _pid, _amount);

        _amount = FarmLibrary.harvestVestPieces(vestList, vestMonths, _amount);
        require(_amount == 0, "Inconsistent");
    ...
    */
    ...
    function emergencyWithdraw(uint256 _pid) public override validPid(_pid) {
        // No _openSession(SessionType.EmergencyWithdraw);
        ...
        FarmLibrary.rebaseDeposit(pool, user, amount, false); // false: addNotSubract
...
enum CollectOption { OnOff, OnOn, OffOn, OffOff } // Compound_On Vest_Off is the default.
...
struct FarmFeeParams {
    ...
    address compoundFeeAccount;
    uint256 compoundFeeRate;
    address instantClaimFeeAccount;
    uint256 instantClaimFeeRate;
}
...
import "../../session/interfaces/IWiredFarm.sol";
...
interface ICrossFarm {
    ...
    event Migrate(address previouslpToken, address newLpToken);
    event WithdrawVest(address indexed user, uint256 poolId, uint256 amount);
    ...
    function massStakeRewards() external;
    ...
    function vestAccumulated(uint256 _pid) external;
    function compoundAccumulated(uint256 _pid) external;
    function claimAccumulated(uint256 _pid) external;
}
...
import "./interfaces/IWiredFarm.sol";
...
abstract contract WiredFarm is IWiredFarm, WireBase {

    CrossContracts public override crossContracts;

    string private constant sForbidden = "WiredFarm: Forbidded";
    string private constant sZeroAddress = "WiredFarm: Zero address";

    modifier pairInformer () override virtual {
        require(msg.sender == crossContracts.token, sForbidden);
        _;
    }
    ...
    function setMaker(address _maker) public override virtual canWire {
        require(msg.sender == IWiredMaker(_maker).getOwner(), sForbidden);
        require(_maker != address(0), sZeroAddress);
        crossContracts.maker = _maker;
        emit SetMaker(_maker);
    }

    function setTaker(address _taker) public override virtual canWire {
        require(msg.sender == IWiredTaker(_taker).getOwner(), sForbidden);
        require(_taker != address(0), sZeroAddress);
        crossContracts.taker = _taker;
    ...
    function setToken(address _token) public override virtual canWire {
        ...
        crossContracts.token = _token;
        emit SetToken(_token);
    }

    function setXToken(address _xToken) external override virtual canWire {
        crossContracts.xToken = _xToken;
        emit SetXToken(_xToken);
    }
}
...
library FarmLibrary {
    ...
    function changeLpTokensToCrssInFarm(
        ...
        if (address(sourceLpToken) != address(0) && lpAmount > 0) {
            ...
            if (sourceLpToken == token) {
                ...
                uint256 balance0_old = IERC20Upgradeable(token0).balanceOf(address(this));
                uint256 balance1_old = IERC20Upgradeable(token1).balanceOf(address(this));
                IMaker(maker).removeLiquidity(token0, token1, lpAmount, 0, 0, address(this), block.timestamp);
                uint256 amount0 = IERC20Upgradeable(token0).balanceOf(address(this)) - balance0_old;
                uint256 amount1 = IERC20Upgradeable(token1).balanceOf(address(this)) - balance1_old;
    ...
    function changeCrssInXTokenToLpInFarm(address targetLpToken, CrossContracts memory crossContracts, uint256 amountCrssInXToken) 
    public returns (uint256 newLpAmountInFarm) {
        if (targetLpToken != address(0) && amountCrssInXToken > 0) {
            uint256 balance0 = ICrssToken(crossContracts.token).balanceOf(address(this));
            tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, address(this), amountCrssInXToken);
            uint256 balance1 = ICrssToken(crossContracts.token).balanceOf(address(this));
            uint amountCrssInFarm = balance1 - balance0;

            if (targetLpToken == crossContracts.token) {
                ...
                bool foundDirectSwapPath;
                {
                    address pair0 = IMaker(crossContracts.maker).getPair(crossContracts.token, token0);
                    address pair1 = IMaker(crossContracts.maker).getPair(crossContracts.token, token1);
                    foundDirectSwapPath = (crossContracts.token == token0 || pair0 != address(0)) && (crossContracts.token == token1 || pair1 != address(0));
                }
                require(foundDirectSwapPath, "Swap path not found");

                uint256 amount0 = amountCrssInFarm / 2;
                uint256 amount1 = amountCrssInFarm - amount0;
                amount0 = _swapExactCrssToNonCrss(ITaker(crossContracts.taker), crossContracts.token, crossContracts.token, token0, amount0);
                amount1 = _swapExactCrssToNonCrss(ITaker(crossContracts.taker), crossContracts.token, crossContracts.token, token1, amount1);
                
                require( amount0 > 0 && amount1 > 0, "Swap failed");
                uint256 balance0 = ICrossPair(targetLpToken).balanceOf(address(this));
                IERC20Upgradeable(token0).safeIncreaseAllowance(crossContracts.maker, amount0);
                IERC20Upgradeable(token1).safeIncreaseAllowance(crossContracts.maker, amount1);
                IMaker(crossContracts.maker).addLiquidity(token0, token1, amount0, amount1, 0, 0, address(this), block.timestamp);
                uint256 balance1 = ICrossPair(targetLpToken).balanceOf(address(this));
    ...
    function _swapExactCrssToNonCrss(
        ...
        if (tokenTo == token) {
            ...
            uint256 balance0 = IERC20Upgradeable(tokenTo).balanceOf(address(this));
            ...
            resultingAmount = IERC20Upgradeable(tokenTo).balanceOf(address(this)) - balance0;
        } else {
            resultingAmount = amount;
        }
    }
    ...
    function swapExactTokenForToken(
        ...
        if (tokenFr != tokenTo) {
            uint256 _tokenToAmt = IERC20Upgradeable(tokenTo).balanceOf(address(this));
            ...
            tokenToAmount = IERC20Upgradeable(tokenTo).balanceOf(address(this)) - _tokenToAmt;
        } else {
            return amount;
        }
    }

    function getMatureVestPieces(VestChunk[] storage vestList, uint256 vestMonths) external view returns (uint amount) {
        for (uint256 i = 0; i < vestList.length; i++) {
            uint256 elapsed = block.timestamp - vestList[i].startTime;
    ...
    function harvestVestPieces(VestChunk[] storage vestList, uint256 vestMonths, uint amount) external returns (uint256 _amount) {
        _amount = amount;
        uint256 len = vestList.length;
        ...
        for (uint256 i = 0; i < len;) {
            if (_amount > 0) {
                ...
                if (unlockAmount > _amount) {
                    vestList[i].withdrawn += _amount;
                    _amount = 0;
                } else {
                    vestList[i].withdrawn += unlockAmount;
                    _amount -= unlockAmount;
                }

                if (vestList[i].withdrawn  == vestList[i].principal) {
                    for (uint256 j = i; j < vestList.length - 1; j++) {
                        vestList[j] = vestList[j + 1];
                    }
                    vestList.pop();
                }
                i++;
    ...
    * Reward gain, instead, takes place in _updatePool, for pools, and _takePendingRewards, for branches and subpools.
    */
    function rebaseDeposit(PoolInfo storage pool, UserInfo storage user, uint256 amount, bool addNotSubtract) external {
        if (amount > 0) {

            user.amount = addNotSubtract ? user.amount + amount : user.amount - amount;
            user.rewardDebt = user.amount * pool.accCrssPerShare / 1e12;
            ...
            if (user.collectOption == CollectOption.OnOff) {
                ...
                pool.OnOff.rewardDebt = (pool.OnOff.sumAmount + pool.OnOff.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12;
                user.debt1 = user.amount * pool.OnOff.Comp.accPerShare / 1e12;

                // console.log("rebase-OnOff: payroll, accPayroll, debtPayrol; deposit, accDeposit, debtDeposit");
                // console.log(pool.OnOff.sumAmount + pool.OnOff.Comp.sumAccumulated, pool.accCrssPerShare, pool.OnOff.rewardDebt);
                // console.log(amount, pool.OnOff.Comp.accPerShare, user.debt1);
                ...
                pool.OnOn.rewardDebt = (pool.OnOn.sumAmount + pool.OnOn.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12;
                ...
                pool.OffOn.rewardDebt = pool.OffOn.sumAmount *  pool.accCrssPerShare / 1e12;
                ...
                pool.OffOff.rewardDebt = pool.OffOff.sumAmount *  pool.accCrssPerShare / 1e12;
                user.debt1 = user.amount * pool.OffOff.Accum.accPerShare / 1e12;
            }
        }
    }

    function takeRewardsCollectively(PoolInfo storage pool, UserInfo storage user, 
    address msgSender, FarmFeeParams memory feeParams, CrossContracts memory crossContracts) public {
        uint256 subPoolPending;

        //-------------------- OnOff SubPool Group Takes -------------------- Compound On, Vest Off

        subPoolPending = (pool.OnOff.sumAmount + pool.OnOff.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12 - pool.OnOff.rewardDebt;

        if (subPoolPending > 0) {
            uint256 feePaid = subPoolPending * feeParams.nonVestBurnRate / FeeMagnifier;
            ICrssToken(crossContracts.token).burn(crossContracts.xToken, feePaid);
            subPoolPending -= feePaid;
            feePaid = subPoolPending * feeParams.compoundFeeRate / FeeMagnifier;
            tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, feeParams.compoundFeeAccount, feePaid);
            subPoolPending -= feePaid;

            uint256 newLpAmountInFarm = changeCrssInXTokenToLpInFarm(address(pool.lpToken), crossContracts, subPoolPending);
            _addToSubPool(pool.OnOff.Comp, pool.OnOff.sumAmount, newLpAmountInFarm);

            pool.OnOff.rewardDebt = (pool.OnOff.sumAmount + pool.OnOff.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12; // Pending is used out. Debt catches up.
        }

        //-------------------- OnOn SubPool Group Takes -------------------- Compound On, Vest On

        subPoolPending = (pool.OnOn.sumAmount + pool.OnOn.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12 - pool.OnOn.rewardDebt;
        if (subPoolPending > 0 && pool.OnOn.sumAmount > 0) {
            uint256 halfToCompound = subPoolPending / 2;

            uint256 feePaid = halfToCompound * feeParams.compoundFeeRate / FeeMagnifier;
            tolerableCrssTransferFromXTokenAccount(crossContracts.xToken, feeParams.compoundFeeAccount, feePaid);
            halfToCompound -= feePaid;

            uint256 newLpAmountInFarm = changeCrssInXTokenToLpInFarm(address(pool.lpToken), crossContracts, halfToCompound);
            _addToSubPool(pool.OnOn.Comp, pool.OnOn.sumAmount, newLpAmountInFarm);

            uint256 halfToVest = subPoolPending - halfToCompound;
            _addToSubPool(pool.OnOn.Vest, pool.OnOn.sumAmount, halfToVest);

            pool.OnOn.rewardDebt = (pool.OnOn.sumAmount + pool.OnOn.Comp.sumAccumulated) *  pool.accCrssPerShare / 1e12; // Pending is used out. Debt catches up.
        }


        //-------------------- OffOn SubPool Group Takes -------------------- Compound Off, Vest On

        subPoolPending = pool.OffOn.sumAmount *  pool.accCrssPerShare / 1e12 - pool.OffOn.rewardDebt;
        if (subPoolPending > 0) {
            uint256 halfToVest = subPoolPending / 2;
            _addToSubPool(pool.OffOn.Vest, pool.OffOn.sumAmount, halfToVest);

            uint256 halfToSend = subPoolPending - halfToVest;
            _addToSubPool(pool.OffOn.Accum, pool.OffOn.sumAmount, halfToSend);

            pool.OffOn.rewardDebt = pool.OffOn.sumAmount *  pool.accCrssPerShare / 1e12; /// Pending is used out. Debt catches up.
        }


        //-------------------- OffOff SubPool Group Takes -------------------- Compound Off, Vest Off

        subPoolPending = pool.OffOff.sumAmount *  pool.accCrssPerShare / 1e12 - pool.OffOff.rewardDebt;
        if (subPoolPending > 0) {
            uint256 feePaid = subPoolPending * feeParams.nonVestBurnRate / FeeMagnifier;
            ICrssToken(crossContracts.token).burn(crossContracts.xToken, feePaid);
            subPoolPending -= feePaid;

            _addToSubPool(pool.OffOff.Accum, pool.OffOff.sumAmount, subPoolPending);

            pool.OffOff.rewardDebt = pool.OffOff.sumAmount *  pool.accCrssPerShare / 1e12;  // Pending is used out. Debt catches up.
        }
    }
    ...
    */
    ...
    function takePendingReward(PoolInfo storage pool, UserInfo storage user, 
    address msgSender, FarmFeeParams memory feeParams, CrossContracts memory crossContracts) external {
        //-------------------- Pay referral fee outside of user's pending reward --------------------
        uint256 userPending = user.amount * pool.accCrssPerShare / 1e12 - user.rewardDebt; // This is the only place user.rewardDebt works explicitly.
        if (userPending > 0) {
            _mintReferralCommission(msgSender, userPending, feeParams, crossContracts);
            // Do not update user.rewardDebt, as this is not beginning a new amount interval.
        }

        //-------------------- Take rewards collectively in this pool -----------------------------------------------------
        takeRewardsCollectively(pool, user, msgSender, feeParams, crossContracts);
        ...
        if (user.collectOption == CollectOption.OnOff && user.amount > 0) {
            ...
            if (userCompound > 0) {
                pool.OnOff.Comp.sumAccumulated -= userCompound;
            ...
            if (userCompound > 0) {
                pool.OnOn.Comp.sumAccumulated -= userCompound;
                user.amount += userCompound;  //---------- Compound
            ...
            if (userVest > 0) {
                pool.OnOn.Vest.sumAccumulated -= userVest;
            ...
            if (userVest > 0) {
                pool.OffOn.Vest.sumAccumulated -= userVest;
                user.vestList.push( VestChunk ( {principal: userVest, withdrawn: 0, startTime: block.timestamp } ) ); //---------- Put in vesting.
                user.debt1 = user.amount * pool.OffOn.Vest.accPerShare / 1e12;
            }

            uint256 userSend = userAmount * pool.OffOn.Accum.accPerShare / 1e12 - user.debt2;
            if (userSend > 0) {
                pool.OffOn.Accum.sumAccumulated -= userSend;
                user.accumulatedRewards += userSend; //---------- Accumulate.
                user.debt2 = user.amount * pool.OffOn.Accum.accPerShare / 1e12;
            }    

        } else if (user.collectOption == CollectOption.OffOff && user.amount > 0) {
            uint256 userSend = user.amount * pool.OffOff.Accum.accPerShare / 1e12 - user.debt1;
            if (userSend > 0) {
                pool.OffOff.Accum.sumAccumulated -= userSend;
                user.accumulatedRewards += userSend; //---------- Accumulate.
                user.debt1 = user.amount * pool.OffOff.Accum.accPerShare / 1e12;
            }
        }
    ...
    */
    function _mintReferralCommission(address _user, uint256 principal, FarmFeeParams memory feeParams, CrossContracts memory crossContracts) internal {
        if (feeParams.crssReferral != address(0) && feeParams.referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && referrer != _user && commission > 0) {
                ICrssToken(crossContracts.token).mint(referrer, commission);
    ...
    function _addToSubPool(SubPool storage subPool, uint256 totalShare, uint256 newAmount) internal {
        subPool.sumAccumulated += newAmount;
...
import "../session/WiredXToken.sol";
import "./interfaces/IXCrssToken.sol";
import "./interfaces/ICrssToken.sol";
...
contract xCrssToken is ERC20Upgradeable, WiredXToken, IXCrssToken, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function safeCakeTransfer(address _to, uint256 _amount) public override {
        require(msg.sender == farm, "Cross: Forbidden");
        uint256 cakeBal = ICrssToken(token).balanceOf(address(this));
        if (_amount > cakeBal) {
            ICrssToken(token).transfer(_to, cakeBal);
        } else {
            ICrssToken(token).transfer(_to, _amount);
        }
    }
    ...
    function getOwner() external view override (IXCrssToken, WiredXToken) virtual returns (address) {
        return owner();
    }
    ...
    function setFarm(address _crssFarm) external override (IXCrssToken, WiredXToken) onlyOwner {
        require( _crssFarm != address(0), "Cross: Zero address");
        farm = _crssFarm;
    }
...
interface IWiredFarm is IWireBase {

    event SetMaker(address maker);
    event SetTaker(address taker);
    event SetToken(address token);
    event SetXToken(address xToken);

    function crossContracts() external view returns (address, address, address, address, address, address);
...
import "./interfaces/IWiredXToken.sol";
import "./interfaces/IWiredToken.sol";
import "./interfaces/IWiredFarm.sol";

import "./WireBase.sol";

abstract contract WiredXToken is IWiredXToken, WireBase {

    address public override token;
    address public override farm;


    string private constant sForbidden = "WiredToken: Forbidded";
    string private constant sZeroAddress = "WiredToken: Zero address";

    modifier pairInformer () override virtual {
        require(msg.sender == farm, sForbidden);
        _;
    }
    ...
    function _isExternalCall() internal view virtual returns (bool) {
        return ! (msg.sender == token || msg.sender == farm);
    }
    ...
    function setFarm(address _farm) external override virtual canWire {
        require(msg.sender == IWiredFarm(_farm).getOwner(), sForbidden);
        require(_farm != address(0), sZeroAddress);
        farm = _farm;
        emit SetFarm(_farm);
    }
...
import "./IWireBase.sol";
interface IWiredXToken is IWireBase {
