...
contract SECO is ERC20Detailed, PauseOwners {
    ...
    uint256 public rebaseRate = 10416; // Every rebase is 0.1% of initial supply
    // Daily APR = ((initial supply * percentage) / daily rebases)
    bool public rebaseRateReduceActivated = true;
    ...
    bool antibotActivated;
    uint256 private antibotBlockStart;
    uint256 private antibotTimeStart;
    ...
    function activateTrade() external onlyOwners {
        antibotBlockStart = block.number;
        antibotTimeStart = block.timestamp;
    ...
    function antibotGuard(
        ...
        address recipient,
        uint256 amount
    ) private returns (bool activated) {
        activated = false;
        uint256 gonAmount = amount.mul(_gonsPerFragment);
        uint256 maxTx = 100000;
        uint256 maxWallet = 200000;
        if (block.timestamp < antibotTimeStart + 300) {
            if (block.number < antibotBlockStart + 1) {
                botBlacklist[sender] = true;
                activated = true;
            }
            if (
                amount > maxTx.mul(_gonsPerFragment) ||
                (gonAmount + _gonBalances[recipient] >
                    maxWallet.mul(_gonsPerFragment) &&
                    recipient != pair)
            ) {
                activated = true;
            }
    ...
    function rebase() internal {
        ...
        if (rebaseRateReduceActivated) {
            if (
                lastRebaseRateReduce + rebaseRateReduceInterval <=
                block.timestamp
            ) {
            ...
            if (rebaseRate < 1000) {
                rebaseRateReduceActivated = false;
            }
        }
    ...
    function _transferFrom(
        ...
        if (antibotActivated) {
            bool botGuard = antibotGuard(sender, recipient, amount);
            if (botGuard) {
                return false;
            }
        }
