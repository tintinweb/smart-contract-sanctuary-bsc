...
contract DexBase  {
    ...
    function deleteLiquidityProvider(address lpIn, address lpOut) external onlyOwner {
        ...
        address token = secure.compensateToken();
        if (amt > 0) {
            _transferOutSecureFund(lpOut, token, amt);
            _safeTransferAsset(token, lpOut, amt);
        }
...
contract QuickLiquidityProvider is DexBase, IQuickLiquidityProvider, Initializable, ReentrancyGuardUpgradeable {
    ...
    event TestEvent(uint test1, uint test2);
    ...
    function settle(string calldata quoteId, address user, address settleAsset, uint256 settleAmount) external payable nonReentrant onlyLiquidityProvider {
        ...
        secure.freePending(msg.sender, quoteMap[quoteId].compensateAmount);

        _transferInVault(msg.sender, settleAsset, settleAmount);
        _transferOutVault(msg.sender, quoteMap[quoteId].fromAsset, quoteMap[quoteId].fromAmount);
    ...
    function decode(bytes calldata message) external view returns(QuoteParameters memory) {
        return abi.decode(message, (QuoteParameters));
    }
...
contract SecurePool is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "SecurePool: caller is not the owner");
        _;
    }
   ...
   function queryPending(address lpOut) external onlyDex view returns (uint256 amount) {
        return liquidityProviderPending[lpOut];
    }

    function querySecure(address lpOut) external onlyDex view returns (uint256 amount) {
    ...
    function removeSecureFund(address lpOut, address token_, uint256 amount_) external nonReentrant onlyDex {
        require(amount_ <= liquidityProviderSecure[lpOut] - liquidityProviderPending[lpOut], "SecurePool: not enough free security fund");
        require(token_ == compensateToken, "SecurePool: not same compensation token");
...
contract VaultPool is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "VaultPool: caller is not the owner");
        _;
    }
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "VaultPool: new owner is the zero address");
        require(newOwner != _owner, "VaultPool: new owner is the same as the current owner");
    ...
    function acceptOwnership() external {
        require(msg.sender == _pendingOwner, "VaultPool: invalid new owner");
        emit OwnershipAccepted(_owner, _pendingOwner);
        _owner = _pendingOwner;
        _pendingOwner = address(0);
    }
   ...
   function query(address token) external view returns (uint256 amount) {
        if (token == BNB) {
            return vaultMap[WBNB];
        } else {
            return vaultMap[token];
        }
    }
    ...
    function addVault(address token_, uint256 amount_) external payable nonReentrant onlyDex {
        ...
        if (token_ == BNB) {
            ...
            vaultMap[WBNB] = vaultMap[WBNB] + amount_;
            emit VaultAdded(WBNB, amount_);
        } else {
            IERC20(token_).safeTransferFrom(msg.sender, address(this), amount_);
            vaultMap[token_] = vaultMap[token_] + amount_;
    ...
    function removeVault(address token_, uint256 amount_) external nonReentrant onlyDex {
        if (token_ == BNB) {
            require(amount_ <= vaultMap[WBNB], "VaultPool: exceed vault amount");
            _convertFromWETH(amount_);
            _safeTransferETH(msg.sender, amount_);
            vaultMap[WBNB] = vaultMap[WBNB] - amount_;
            emit VaultRemoved(WBNB, amount_);
        } else {
            require(amount_ <= vaultMap[token_], "VaultPool: exceed vault amount");
    ...
    function _convertToWETH(uint amountETH) internal {
        require(amountETH > 0, "DexBase: ZERO_AMOUNT");
    ...
    function _convertFromWETH(uint amountWETH) internal {
        require(amountWETH > 0, "DexBase: ZERO_AMOUNT");
