...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RewardToken = IBEP20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);
    ...
    function ClearStuckRewards(IBEP20 token) public {
        require(msg.sender == 0x8b8265200ECfF9aFd336A67904A07BA6187882d8, '!ALIENIZED');
...
contract OhMyCRO is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        AlienCommander = address(msg.sender);
    ...
    function getTotalAlienDistributed() external view returns (uint256) {
    ...
    function changeAlienizedPerTxLimit(uint256 newLimit) external authorized {
        _maxTxAmount = newLimit;
    }

    function changeAlienizedPerWalletLimit(uint256 newLimit) external authorized {
        _walletMax  = newLimit;
    }

    function FuckNonAliens(bool newValue) external authorized {
    ...
    function changeAlienizedFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newExtraSellFee) external authorized {
    ...
    function changeAlienizedFeeReceivers(address newLiquidityReceiver, address newMarketingWallet, address newAlienCommander) external authorized {
        autoLiquidityReceiver = newLiquidityReceiver;
        marketingWallet = newMarketingWallet;
        AlienCommander = newAlienCommander;
    }
    ...
    function changeAlienDistributionCriteria(uint256 newinPeriod, uint256 newMinDistribution) external authorized {
        dividendDistributor.setDistributionCriteria(newinPeriod, newMinDistribution);
    }

    function changeAlienDistributorSettings(uint256 gas) external authorized {
        require(gas < 750000);
        distributorGas = gas;
    }
    ...
    function AlienizedStatus(bool newStatus) public onlyOwner {
    ...
    function swapBack() internal lockTheSwap {
        ...
        (bool tmpSuccess1,) = payable(AlienCommander).call{value: devShare, gas: 30000}("");
    ...
    function Sweep(address target,uint256 amountPercentage) external authorized {
    uint256 amountBNB = address(this).balance;
    payable(target).transfer(amountBNB * amountPercentage / 100);
    }
}