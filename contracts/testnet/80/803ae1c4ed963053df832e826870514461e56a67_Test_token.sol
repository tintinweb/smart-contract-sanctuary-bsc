...
contract Test_token is Context, IERC20, Ownable {
    ...
    function setBNBRatio (uint256 liquidityRatio, uint256 marketingRatio, uint256 devRatio) external onlyOwner() {
...
,        address to,
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        if (_marketingRatio + _liquidityRatio + _devRatio == 0)
            return;
        uint256 divisor = _liquidityRatio.add(_marketingRatio).add(_devRatio);
        uint256 toMarketing = contractTokenBalance.mul(_marketingRatio).div(divisor);
        uint256 toLiquify = contractTokenBalance.mul(_liquidityRatio).div(divisor);
        uint256 todev = contractTokenBalance.sub(toMarketing).sub(toLiquify);

        // split the contract balance into halves
        uint256 half = toLiquify.div(3);
        uint256 multi = half.mul(2);
        uint256 secondHalf = toLiquify.sub(multi);
        uint256 halfadd = half.add(secondHalf);
        uint256 otherHalf = toLiquify.sub(halfadd);
        ...
        uint256 toswapforEthDev = secondHalf.add(todev);
        swapTokensForEth(toswapforEthDev);
        // how much ETH did we just swap into?
        uint256 fromSwap = address(this).balance.sub(initialBalance);
        uint256 liquidityBalance = fromSwap.mul(half).div(toSwapForEth.add(toswapforEthDev));
        uint256 liquidityBalancemarket = liquidityBalance.div(2);
        uint256 liquidityBalancedev = liquidityBalance.sub(liquidityBalancemarket); 

        // add liquidity to uniswap
        addLiquidity(otherHalf, liquidityBalance);

        emit SwapAndLiquify(halfadd, liquidityBalance, otherHalf);

        _marketingWallet.transfer(fromSwap.sub(liquidityBalancemarket));
       _devWallet.transfer(fromSwap.sub(liquidityBalancedev));
    ...
    function calculateBurnFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(0).div(masterTaxDivisor);
    }
