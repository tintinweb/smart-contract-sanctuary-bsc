...
contract MarketPlaceV2 is MarketPlaceV2Auth, EIP712Upgradeable, PausableUpgradeable {
  ...
  IGameNFT public itemNft; // TODO remove on mainnet deployment
  ...
  function summonShardHero(uint16 _classId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    (uint gemAmount) = rewardManager.summonHero(msg.sender, heroPrice * rewardManager.gemRate());
    _validateSignature(_classId, _timestamp, _signature);
    uint tokenId = heroNft.mintHero(address(this), _classId, heroPrice * rewardManager.gemRate(), 0);
  ...
  function summonHero(uint16 _classId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    (uint gemAmount) = rewardManager.summonHero(msg.sender, heroPrice * rewardManager.gemRate());
    _validateSignature(_classId, _timestamp, _signature);
    uint tokenId = heroNft.mintHero(msg.sender, _classId, heroPrice * rewardManager.gemRate(), 0);
    emit HeroSummoned(msg.sender, msg.sender, tokenId, block.timestamp, 0, gemAmount, 1);
  }

  function summonPrestigeHero(uint16 _classId, bytes memory _signature, uint _timestamp) external whenNotPaused {
    (uint gemAmount) = rewardManager.summonPrestigeHero(msg.sender, heroPrice * rewardManager.gemRate());
    _validateSignature(_classId, _timestamp, _signature);
    uint tokenId = heroNft.mintHero(msg.sender, _classId, heroPrice * rewardManager.gemRate(), 0);
