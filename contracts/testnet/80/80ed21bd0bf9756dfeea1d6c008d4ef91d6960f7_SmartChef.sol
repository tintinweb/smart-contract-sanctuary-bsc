...
// File: MoonFORCE/MoonFORCE_token/contracts/interfaces/IBEP20.sol
...
// File: MoonFORCE/MoonFORCE_token/contracts/libs/SafeBEP20.sol
...
// File: MoonFORCE/MoonFORCE_token/contracts/libs/BEP20.sol
...
// File: MoonFORCE/MoonFORCE_token/contracts/Fuel.sol
...
// File: MoonFORCE/MoonFORCE_token/contracts/SmartChef.sol
...
contract SmartChef is Ownable, ReentrancyGuard, ERC20 {
    using SafeMath for uint256;
...
// ----------------- BEGIN WBNB Variables -----------
    ...
    uint256 public rewardsPerShareAccuracyFactor = 10 ** 36; // Keeping same accuracy factor in the MF Token Variables

// ----------------- BEGIN MF Token Variables -----------

    uint256 public unallocatedMFRewards;
    uint256 public totalRewardsMF;
    uint256 public totalDistributedMF;
    uint256 public rewardsPerShareMF;
...
// ----------------- END MF Token Variables -----------    
    ...
    function setShare(address staker, uint256 amount) internal {
        ...
        shares[staker].totalExcludedMF = getCumulativeRewardsMF(shares[staker].amount);
    }

    function depositWBNBToStakingContract(uint256 _amount) external onlyToken {
        if (totalShares == 0) {unallocatedWBNBRewards = unallocatedWBNBRewards.add(_amount); return; } 
        
        if (unallocatedWBNBRewards > 0) {
            uint256 amount = _amount.add(unallocatedWBNBRewards);
            ...
            unallocatedWBNBRewards = 0;
        } else {
            totalRewardsWBNB = totalRewardsWBNB.add(_amount);
            rewardsPerShareWBNB = rewardsPerShareWBNB.add(rewardsPerShareAccuracyFactor.mul(_amount).div(totalShares));
        }    
    }
...
// ------------------- BEGIN MF TOKEN FUNCTIONS ---------------
    ...
    function depositMFToStakingContract(uint256 _amountMF) external onlyMoonshotMechanism {
        if (totalShares == 0) {unallocatedMFRewards = unallocatedMFRewards.add(_amountMF); return; } 
        
        if (unallocatedMFRewards > 0) {
            uint256 amount = _amountMF.add(unallocatedMFRewards);
            totalRewardsMF = totalRewardsMF.add(amount);
            rewardsPerShareMF = rewardsPerShareMF.add(rewardsPerShareAccuracyFactor.mul(amount).div(totalShares));
            unallocatedMFRewards = 0;
        } else {
            totalRewardsMF = totalRewardsMF.add(_amountMF);
            rewardsPerShareMF = rewardsPerShareMF.add(rewardsPerShareAccuracyFactor.mul(_amountMF).div(totalShares));
        }   
    }

    function getCumulativeRewardsMF(uint256 share) internal view returns (uint256) {
        return share.mul(rewardsPerShareMF).div(rewardsPerShareAccuracyFactor);
    }

    function getUnpaidEarningsMF(address staker) public view returns (uint256) {
        if(shares[staker].amount == 0){ return 0; }

        uint256 stakerTotalRewardsMF = getCumulativeRewardsMF(shares[staker].amount);
        uint256 stakerTotalExcludedMF = shares[staker].totalExcludedMF;

        if(stakerTotalRewardsMF <= stakerTotalExcludedMF){ return 0; }

        return stakerTotalRewardsMF.sub(stakerTotalExcludedMF);
    }

    function distributeRewardMF(address staker) internal {
        if(shares[staker].amount == 0){ return; }

        uint256 amount = getUnpaidEarningsMF(staker);
        if(amount > 0){
            totalDistributedMF = totalDistributedMF.add(amount);
            IBEP20(stakedToken).transfer(moonshotMechanism, amount);
            IBEP20(stakedToken).transferFrom(moonshotMechanism, staker, amount);
            stakerClaims[staker] = block.timestamp;
            shares[staker].totalRealisedMF = shares[staker].totalRealisedMF.add(amount);
            shares[staker].totalExcludedMF = getCumulativeRewardsMF(shares[staker].amount);
        }
    }

    function claimFORCE() external {
        distributeRewardMF(msg.sender);
    }
    ...
    // @dev change the name of a presale token
    function editPresaleToken(uint256 _index, string memory _newName) external onlyOwner {
        if (bytes(_newName).length!=0) {
            presaleTokenList[_index].presaleTokenName = _newName;
        }
    }
