...
contract DevsMiner is Ownable, ReentrancyGuard {
    Token devsToken;

    // 1 Month (34 * 24 * 60 * 60)
    uint256 _planExpired = 2937600;

    uint8 public interestRate = 10; // Earn 0.34% per month
    ...
    function claimReward() external returns (bool) {
        ...
        devsToken.transfer(_msgSender(), totalTokens);
    ...
    function stakeToken(uint256 stakeAmount) external payable {
        ...
        require(
            devsToken.balanceOf(_msgSender()) >= stakeAmount,
            "Insufficient Balance"
        );

        devsToken.transferFrom(_msgSender(), address(this), stakeAmount);
        totalStakers++;
        addressStaked[_msgSender()] = true;
