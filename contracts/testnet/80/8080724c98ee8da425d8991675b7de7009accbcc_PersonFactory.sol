...
contract PersonFactory is  Ownable {
    ...
    event TakingAmount(address indexed user,uint failAmount,uint amount);
    ...
    uint private splitTime = 1 hours;

    // uint private pInfoSplite = 1 days + 2 hours;
    uint private pInfoSplite =  1 hours + 10 minutes;
    ...
    mapping(address=>mapping(uint=>mapping(uint=>uint))) public userBuyAmount;
    ...
    function _isPaying(uint _pLevel,uint _version) private view returns(bool){
        uint _startTimestamp = getVersionStartTimestamp(_pLevel,_version);
        if(_startTimestamp<block.timestamp&&_startTimestamp>block.timestamp.add(splitTime)){
            return true;
        }
        return false;
    }
    ...
    function _isSuccess(uint _pLevel,uint _version) private view returns(bool){
        ...
        uint _startTimestamp = getVersionStartTimestamp(_pLevel,_version);
        if(_versionSumAmount==pInfoSellAmount[_pLevel][_version]
            &&block.timestamp<_startTimestamp.add(splitTime)){
            return true;
        }
        return false;
    }
    ...
    function getValidDirectUser(address _user) public view returns (uint){
        ...
        for(uint i = 0;i<myInvites[_user].length;i++){
            address cUser = myInvites[_user][i];
            if(userInfo[cUser].payAmount>500*oneU){
                _validUser = _validUser + 1;
            }
        }
        return _validUser;
    ...
    function getUserLevel(address _user) public view returns(uint){
        if(userInfo[_user].payAmount>=1000000*oneU&&getValidUser(_user)>=400&&getValidUser(_user)>=100){
            return 4;
        }
        if(userInfo[_user].payAmount>=500000*oneU&&getValidUser(_user)>=300&&getValidUser(_user)>=50){
            return 3;
        }
        if(userInfo[_user].payAmount>=150000*oneU&&getValidUser(_user)>=200&&getValidUser(_user)>=30){
            return 2;
        }
        if(userInfo[_user].payAmount>=50000*oneU&&getValidUser(_user)>=100&&getValidUser(_user)>=15){
            return 1;
        }
        return 0;
    }
    ...
    function getUserBuyIncome(address _user) public view returns (uint){
        ...
        for(uint i = 0 ;i<userPayInfo[_user].length;i++){
            if(getVersionStatus(userPayInfo[_user][i].pLevel,userPayInfo[_user][i].version)==7){
                ...
                _amount = _amount.add(_adding.add(1130).mul(_buyAmount).div(1000));
            }
        }
        return _amount;
    }

    function _getLevelSumBeforeVersionFail(uint _pLevel) private view returns (uint){
        if(_pLevel<=3){
            return uint(0);
        }
        uint _rAmount = 0;
        for(uint i = 0;i<=_pLevel-3;i++){
    ...
    function getUserFailIncome(address _user) public view returns (uint){
        ...
        for(uint i = 0 ;i<userPayInfo[_user].length;i++){
            if(getVersionStatus(userPayInfo[_user][i].pLevel,userPayInfo[_user][i].version)==4){
                uint _before = _getLevelSumBeforeVersionFail(userPayInfo[_user][i].pLevel).mul(2).div(100);
                uint _buyAmount =  userPayInfo[_user][i].amount;
    ...
    function getUserFailBeforeIncome(address _user) public view returns (uint,uint){
        ...
        for(uint i = 0 ;i<userPayInfo[_user].length;i++){
            if(getVersionStatus(userPayInfo[_user][i].pLevel,userPayInfo[_user][i].version)==4){
                ...
                _plate = _amount.add(_buyAmount.mul(5).div(100));
            }
        }
    ...
    function getUserLevelIncome(address _user) public view returns (uint){
        ...
        if(_userLevel==0){
            ...
            uint _inviteAmount = getUserInviteAmount(_user,0);
            uint _inviteAmountSameLevel = getUserInviteAmountSameLevel(_user,0,_userLevel);
            uint _inviteAmountUpLevel = getUserInviteAmountUpLevel(_user,0,_userLevel);
            uint _levelAmount = _inviteAmount.sub(_inviteAmountSameLevel).sub(_inviteAmountUpLevel);
            uint _amount = _levelAmount.mul(_userLevel).div(100).add(_inviteAmountSameLevel.mul(5).div(1000));
            return _amount; 
        }
    }

    function getUserInviteAmount(address _user,uint _amount) public view returns (uint){
        uint _rAmount = _amount;
        for(uint i = 0;i<myInvites[_user].length;i++){
            _rAmount = _rAmount.add(userInfo[myInvites[_user][i]].payAmount);
            _rAmount = getUserInviteAmount(myInvites[_user][i],_rAmount);
        }
        return _rAmount;
    }
    ...
    function getUserInviteAmountUpLevel(address _user,uint _amount,uint _level) public view returns(uint){
        uint _rAmount = _amount;
        for(uint i = 0;i<myInvites[_user].length;i++){
            address _cUser = myInvites[_user][i];
            uint _cLevel = getUserLevel(_cUser);
    ...
    function getDirectAmount(address _user) public view returns (uint){
        uint _amount = 0;
        for(uint i = 0;i<myInvites[_user].length;i++){
            _amount = _amount.add(getUserBuyIncome(myInvites[_user][i]));
        }
        return _amount;
    }
    ...
    function getIndirectAmount(address _user) public view returns(uint){
        uint _amount = 0;
        for(uint i =0;i<myInvites[_user].length;i++){
            for(uint j = 0;j>myInvites[myInvites[_user][i]].length;j++){
                _amount = _amount.add(getUserBuyIncome(myInvites[myInvites[_user][i]][j]));
            }
        }
        return _amount;
    }
    ...
    function checkAmount(address _user,uint _amount,uint _pLevel,uint _version) public view returns(bool){
        ...
        if(_dayMin>_amount||_dayMax<_amount) {
            return false;
        }
        ...
        if(_leftAmount<_amount.add(_payed)) {
            return false;
        }
        return true;
    }
    ...
    function getMyInvitesLv2Num(address _owner) public view returns(uint _num){
        ...
        if(direct.length>0){
            for(uint i = 0;i<direct.length;i++){
                _num = _num + getMyInvitesLv2(direct[i]).length;
            }
        }
    }
    ...
    function getMyInvitesLv2(address _owner) public view returns(address[] memory ){
        ...
        for(uint i = 0;i<_direct.length;i++){
            address[] memory _indirect = getMyInvitesLv2(_direct[i]);
    ...
    function addPInfo(uint _timesStamp,uint _sumAmount) public onlyOwner{
        pLevel = pLevel + 1;
        pInfo[pLevel].startTimestamp = _timesStamp;
    ...
    function withdrawPlateform(uint _amount) public{
         address _user = msg.sender;
        _takingAmount(_user);
        require(userInfo[_user].plateAmount>=_amount,"balance not enough");
        userInfo[_user].plateAmount = userInfo[_user].plateAmount.sub(_amount);
        _safeTransfer(usdtAddr,msg.sender,_amount);
    ...
    function _takingAmount(address _user) private {
        uint _amount = getUserIncome(_user);
        uint _tAmount = userInfo[_user].takingAmount;
        uint _rAmount = _amount.sub(_tAmount);
        userInfo[_user].takingAmount = _tAmount;
        ...
        uint _failAmount = _failSum.sub(userInfo[_user].failBack);
        userInfo[_user].failBackAmount = userInfo[_user].failBackAmount.add(_failAmount);
        uint _failPlate = _failBeforePlate.sub(userInfo[_user].failBackPlate);
        userInfo[_user].failBackPlate = _failBeforePlate;
        uint _pAmount = _swapPlateformCoin(_failPlate);
        userInfo[_user].plateAmount = userInfo[_user].plateAmount.add(_pAmount);
        emit TakingAmount(_user,_failAmount,_pAmount);
    }
