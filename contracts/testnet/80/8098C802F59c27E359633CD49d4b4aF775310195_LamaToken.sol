    ...
    * @param user address of the user than will now have to pay transaction fees.
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (swapEnabled && !pairAddresses[_msgSender()]) {
            uint256 contractLamaTokenBalance = IERC20(contractAddress).balanceOf(contractAddress);
    ...
    function swapLamaTokensForBnb(uint256 amount) internal swapping {
        // Only swap if more than 1e-8 tokens are in contract to avoid "UniswapV2: K or INSUFFICIANT_INPUT_AMOUNT" error
        if (amount > 10 ** 8) {
