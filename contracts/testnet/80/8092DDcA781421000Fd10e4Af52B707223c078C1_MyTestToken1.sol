...
/**
 * Leonicornswap Token, with transfer limit
 */
contract MyTestToken1 is ERC20, Ownable {
    // Initial supply, 400M tokens
    uint256 public constant INITIAL_SUPPLY = 400_000_000 ether;
    ...
    constructor() ERC20("MyTestToken", "MyTK") {
    ...
    function _enforceTransferLimit(address from, uint256 amount) internal {
        ...
        require(amount <= transferLimit, "LEONS: daily limit exceeds");

        if ((block.timestamp - uTransfer.lastTime) >= 1 days) {
            uTransfer.lastTime = block.timestamp;
            uTransfer.perDayTransfer = amount;
        } else {
            require((amount + uTransfer.perDayTransfer) <= transferLimit, "LEONS: daily limit exceeds");
            uTransfer.perDayTransfer += amount;
        }
    }
