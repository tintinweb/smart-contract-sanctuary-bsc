...
/// @title SingleDynamicPullPayment
	...
	IVersionedContract
{
	 ...
	 */
	function createBillingModel(address _payee) public override returns (uint256 billingModelID) {
	...
	function subscribeToBillingModel(
		...
		bm.subscriptions[newSubscriptionID].name = _name;
		bm.subscriptions[newSubscriptionID].settlementToken = _settlementToken;
		...
		bm.subscriptions[newSubscriptionID].pullPayments[newPullPaymentID].executionTimestamp = block
			.timestamp;

		//update the ids
		bm.subscriptionIDs.push(newSubscriptionID);
		bm.subscriptions[newSubscriptionID].pullPaymentIDs.push(newPullPaymentID);
		...
		emit NewSubscription(_billingModelID, newSubscriptionID, newPullPaymentID, msg.sender);
		...
		returns (uint256 billingModelID)
	{
		...
		require(
			_newPayee != address(0) && _newPayee != _billingModels[_billingModelID].payee,
...
interface ISingleDynamicPullPayment {
	...
	function createBillingModel(address _payee) external returns (uint256 billingModelID);
