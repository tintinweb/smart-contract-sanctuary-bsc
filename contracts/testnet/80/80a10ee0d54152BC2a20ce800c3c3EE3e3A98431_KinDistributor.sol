...
interface IKinStakes {
    function stakeGift(address beneficiary, address underlying, uint amount) external;
}

contract KinDistributor {
    address public immutable kin;
    address public immutable kinStakes;
    string public constant name = "KIN Distributor";
    ...
    constructor(address kin_, address kinStakes_) {
        kin = kin_;
        kinStakes = kinStakes_;
        owner = msg.sender;
        Utils.safeApprove(kin_, kinStakes_, type(uint).max);
    }
    ...
    /// @param token Address of token being claimed (ie KIN)
    ...
    function claim(address account, address token, uint claimable, bytes32[] calldata proof, address stake) external {
        ...
        if (stake == address(0)) {
            ...
            require(token == kin, "can only auto-stake KIN");
            IKinStakes(kinStakes).stakeGift(account, stake, amount);
        }
    }
}
