...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
    ...
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _feeRecipient) public {
        want = _want;
        poolId = _poolId;
        chef = _chef;
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 feeAmount = nativeBal.mul(letFarmFee).div(MAX_FEE);
...
abstract contract FeeManager is StratManager {
  ...
  uint256 public letFarmFee = MAX_FEE - STRATEGIST_FEE - callFee;

  function setCallFee(uint256 _fee) public onlyManager {
    require(_fee <= MAX_CALL_FEE, "!cap");

    callFee = _fee;
    letFarmFee = MAX_FEE - STRATEGIST_FEE - callFee;
  }
