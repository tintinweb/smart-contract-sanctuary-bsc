...
contract BUSDBuffet is IBEP20, TEGAuth {
    ...
    bool public freeze_contract = false; //testing
    // bool public freeze_contract = true;
    bool public swapEnabled = true; //testing
    ...
    event Testparams(string message);

    // added variables
    bool public tradabled = false;
    mapping (address => bool) public isTradableExempt;
    ...
    // change the scope to private
    mapping (address => bool) public isAirDropDistributionExempt; 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        emit Testparams("Testing passed inSwap");
    ...
    // added functions
    function setIsTradableExempt(address holder, bool exempt) external authorizedFor(Permission.ExcludeInclude) {
        isTradableExempt[holder] = exempt;
    }

    function setIsHighTaxExempt(address holder, bool exempt) external authorizedFor(Permission.ExcludeInclude) {
        isHighTaxExempt[holder] = exempt;
    }

    function setIsAirDropDistributionExempt(address holder, bool exempt) external authorizedFor(Permission.ExcludeInclude) {
        isAirDropDistributionExempt[holder] = exempt;
    }

    function setIsTransferFeeExempt(address holder, bool exempt) external authorizedFor(Permission.ExcludeInclude) {
        isTransferFeeExempt[holder] = exempt;
    }

    function setIsAllExempt(address holder, bool exempt) external authorizedFor(Permission.ExcludeInclude) {
        isDividendExempt[holder] = exempt;
        isFeeExempt[holder] = exempt;
        isTxLimitExempt[holder] = exempt;
        isTradableExempt[holder] = exempt;
    ...
    function setTradabled(bool _enabled) external authorizedFor(Permission.AdjustContractVariables) {
        tradabled = _enabled;
    }
