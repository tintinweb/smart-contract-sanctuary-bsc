...
contract Fortuna {
    ...
    uint256 private constant dayPerCycle = 1 hours; // 15 days
    uint256 private constant dayRewardPercents = 800;
    ...
    uint256[15] private level5Percents = [100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                if(i > 4){
                    ...
                    if( userInfo[upline].level == 4) {
                        reward = newAmount.mul(level4Percents[i]).div(baseDivider); // L5 comm
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                    if( userInfo[upline].level == 3) {
                        reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider); // L4 comm
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                }else if(i > 0){
                    if( userInfo[upline].level == 2) {
                        reward = newAmount.mul(level4Percents[i]).div(baseDivider); // L3 Comm
                        thirdLevelReward.level3Freezed = thirdLevelReward.level3Freezed.add(reward);
                    }
                    if( userInfo[upline].level == 1) {
                        reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider); //L2 comm
                        thirdLevelReward.level3Freezed = thirdLevelReward.level3Freezed.add(reward);
                    }
                }else{
