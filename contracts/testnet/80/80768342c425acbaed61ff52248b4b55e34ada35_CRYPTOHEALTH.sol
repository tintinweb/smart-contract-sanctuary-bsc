...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    address private Wallet_Donation = 0x74762D067EAe44a58a43FFA4A5C9527E19a5957f;
    ...
    uint256 private _teamSellStart = 1652089498; //To be updated to 1749556800 (June 10, 2025 12:00:00 GMT )for Final Contract
    uint256 private _presaleEnd = 	1652089498; //To be updated 

    uint256 private _employmentHold = 1 * 10**8 * 10**9; //Maximum hold limit of 10% of total supply for Team Wallets
 
    uint256 private numTokensSellToAddToLiquidity = 15 * 10**5 * 10**9;

    //For Donation Recipient Wallets
    uint256 private donationTransferMin = 999 * 10**2 * 10**9; //Sell 99,900 tokens at a time
    uint256 private donationTransferMax = 25 * 10**4 * 10**9; //Transfer 250K to Employment Wallet at a time
    uint256 private burnTransferAmt = 1 * 10**3 * 10**9; //Burn 1K at a time
    uint256 public _maxDonationRecipientHold = 2 * 10**5 * 10**9; //Hold 0.20% of the supply
    uint256 public _donationLimit = 1 * 10**4 * 10**9; //Receive 10K at a time
    uint256 public _donationThreshold = 1 * 10**6 * 10**9;

    uint256 public _sniperSellLimit = 5 * 10*4 * 10**9; //Snipers can only sell 50K tokens at a time
    ...
    constructor() payable {
        ...
        transferFee.liquidityFee = 5;
        transferFee.developmentFee = 5;
        transferFee.treasuryFee = 30;
        ...
        // Test Wallets
        _isDonationRecipient[Wallet_Donation] = true;
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(_isDonationRecipient[account] && balanceOf(account) <= _donationThreshold,
                "Can exclude Donation Recipient if with balance below 1M.");
    ...
    function setTransferFee(uint16 liq, uint16 develop, uint16 treasury, uint16 tax) external onlyOwner {
        require(liq + develop + treasury + tax <= 40, "Fees are capped at 40%"); 
    ...
    function setNumTokensSellToAddToLiquidity(uint256 numTokens)
        ...
        onlyOwner
    {
        require (numTokensSellToAddToLiquidity >= 1 * 10**5 * 10**9,
        "Cannot set a lower value."
        );
        numTokensSellToAddToLiquidity = numTokens;
    }

    function setDonationLimit(uint256 donationLimit) external onlyOwner {
        require ((_donationLimit == 1 * 10**2 * 10**9)
                 || (_donationLimit == 1 * 10**3 * 10**9)
                 || (_donationLimit == 1 * 10**4 * 10**9)
                 || (_donationLimit == 1 * 10**5 * 10**9), 
                 "Can only be set at a specific value.");
    ...
    function _transfer(
        ...
        if (_isSniper[from])
            {
            if (block.timestamp >= 1844251200) {
            require ((to == uniswapV2Pair
                    && amount <= _sniperSellLimit) // Can sell 50K tokens at a time
                    || (to == Wallet_Treasury
                    && amount <= _maxTxAmount) // Treasury will manually buy back at half price
                    && (block.timestamp >= transferTime[from] + 2 minutes 
                    && transferTime[from] >= 0), // Can only transfer every 2 minutes
            ...
            transferTime[from] = block.timestamp;
                ...
                require (to == _burnWallet && amount <= _maxTxAmount); // Treasury will manually buy back at 1/4 price
            }
        ...
        if (_isEmploymentWallet[from]) 
            ...
            require (to == Wallet_Treasury || to == uniswapV2Pair // Can only sell or transfer to Treasury
                    ...
                    && block.timestamp > transferTime[from] + 2 minutes, // Can only sell every 2 minutes
        ...
        if (_isDonationRecipient[to] && from != owner())
            ...
            require ((heldTokens + amount) <= _maxDonationRecipientHold 
                    && amount == _donationLimit // Can only donate 10K tokens at a time
                    && block.timestamp > transferTime[to] + 2 minutes, // Can only donate every 2 minutes
                    "Donation Recipient Wallets can only receive 10K tokens from a holder every 2 minutes. "
            );
        ...
        if (_isDonationRecipient[from] && to != owner()) 
            {
            uint256 heldTokens = balanceOf(to);
            require ((amount == donationTransferMax && _isEmploymentWallet[to]) // Send 250K tokens to Employment Wallet
                    || ((amount == donationTransferMin && to == uniswapV2Pair)
                       && ((heldTokens - amount) >= _donationThreshold)) // Sell 99,900 tokens at a time
