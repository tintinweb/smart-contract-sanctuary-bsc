...
contract MFETTransformFromTokenGo {
    ...
    function transformTokensFor(
        uint256 _amount,
        address _user,
        string memory _type
    ) external onlyOwner {
        transformTokens(_amount, _type, _user);
        emit onTransformToken(_user, _type, _amount, block.timestamp);
    }
    ...
    function transformTokens(
        ...
        tokenTransformLedger_[_user][_type] += _amount;
        // add to globals
        totalTransformToken_[_type] += _amount;
