...
contract Test {
 ...
 uint256 public rewardForValidatorSetChange;
    ...
    event initConsensusState(uint64 initHeight, bytes32 appHash,uint256 pointer,uint256 length,uint256 test);
    event mylog1(uint256 x);
     function init() external {
        ...
        // length คือ ขนาดของ
        (pointer, length,test) = Memory.fromBytes(INIT_CONSENSUS_STATE_BYTES);
        // emit MyLog(pointer);
        ...
        emit initConsensusState(initialHeight, cs.appHash,pointer,length,test);
    }
    ...
    event decodeConsensusStatePtr(uint256 ptr);
    function decodeConsensusState(
        uint256 ptr,
        uint256 size,
        bool leaveOutValidatorSet
    ) internal returns (ConsensusState memory, uint64) {
        ptr = ptr + 8;
        emit decodeConsensusStatePtr(ptr);
        uint64 height;
        /* solium-disable-next-line */
        assembly {
            height := mload(ptr)
        ...
        if (!leaveOutValidatorSet) {
            ...
            (dest, length,test) = Memory.fromBytes(cs.nextValidatorSet);
...
library Memory {
    ...
    function fromBytes(bytes memory bts) internal pure returns (uint addr, uint len,uint test) {
        len = bts.length;
