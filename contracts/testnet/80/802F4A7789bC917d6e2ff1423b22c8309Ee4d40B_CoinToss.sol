...
contract CoinToss is Ownable, ReentrancyGuard{
    ...
    function setMinBetAmountByToken(address token, uint256 minBetAmount) external onlyOwner {
        require(isRightToken(token));
        require(minBetAmount > 0);

        minBetAmountByToken[token] = minBetAmount;
    }
    ...
    */
    ...
    function placeBet(uint option, address token, uint256 amount) external notOwner {
        ...
        require(isRightToken(token));
        require(amount >= minBetAmountByToken[token]);
        require(option <= uint(BetOption.TAIL));

        uint256 allowance = IERC20(token).allowance(msg.sender, address(this));
        require(allowance > amount, "Bet Amount exceeds allowance");

        BetOption result = flipCoin();
        BetOption playerOption = BetOption(option);
        uint fee = amount * 2 * betFee / 100;
        uint totalPrize = amount * 2 - fee;
        ...
        newBetResult.token = token;
        newBetResult.amount = amount;
        newBetResult.option = playerOption;

        if (playerOption == result) {
            newBetResult.wasSuccess = true;
            newBetResult.prize = totalPrize;
            IERC20(token).safeTransfer(msg.sender, totalPrize);
        } else {
    ...
    function sendAllToOwner() external onlyOwner {
        ...
        IERC20(CRONA).transfer(owner(), IERC20(CRONA).balanceOf(address(this)));
    }

    function isRightToken(address token) private view returns(bool) {
        return token == DAI || 
        token == USDC || 
        token == WETH || 
        token == USDT || 
    ...
    */
    function flipCoin() private view returns(BetOption) {
        ...
        return BetOption(uint(keccak256(abi.encodePacked(block.timestamp, betIndex))) % 2);
    }
