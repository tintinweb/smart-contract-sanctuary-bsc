...
contract TEStTOKEN1 is ERC20, Ownable {
    ...
    address public devWallet = 0x89B01e974aea069c322AeA54a109ABAE7E82a856;

    DividendTracker public dividendTracker;
    LotteryContract public lotteryContract;
    
    uint256 public lotteryFireAmount = 10000000000000000;

    uint256 public maxSellTransactionAmount = 1 * 10 ** 11 * (10**9);
    uint256 public swapTokensAtAmount = 2 * 10 ** 9 * (10**9);
    uint256 public maxHoldAmount = 1 * 10 ** 12 * (10 ** 9);
    ...
    uint256 public eligibleAmountForLottery = 200 * 10 ** 6 * 10 ** 9;
    ...
    constructor() public ERC20("TESTTOKEN1", "TESTTOKEN") {
    ...
    receive() external payable {
  	...
  	function setLotteryContractAddress (address payable addr) public onlyOwner {
  	    lotteryContract = LotteryContract(addr);
  	}
  	
  	function setDevWalletAddress (address payable addr) public onlyOwner {
  	    devWallet = addr;
  	}
    
  	function updateLotteryFireAmount(uint256 amount) public onlyOwner {
  	    lotteryFireAmount = amount * 10 ** 18;
  	}
  	
  	function updateLotteryEligibleAmount(uint256 amount) public onlyOwner {
  	    eligibleAmountForLottery = amount * 10 ** 9;
  	}
  	
  	function updateMaxHoldAmount (uint256 amount) public onlyOwner {
  	    maxHoldAmount = amount * 10 ** 9;
  	}
  	
  	function updateMaxSellAmount (uint256 amount) public onlyOwner {
  	...
  	function excludeFromLottery(address account, bool excluded) public onlyOwner {
  	    require(_isExcludedFromLottery[account] != excluded, " Account is already the value of 'excluded'");
        _isExcludedFromLottery[account] = excluded;
        if (excluded){
            if (_bAddedHolderList[account]){ removeholder(account); }
        } else {
            if (balanceOf(account) >= eligibleAmountForLottery && !_bAddedHolderList[account]){ addholder(account); }
        }
  	}
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[to] && to != uniswapV2Pair){
            require(balanceOf(to) <= maxHoldAmount, "Wallet hold amount exceeds limit!");
        }
        ...
        if (!_isExcludedFromLottery[from] && balanceOf(from) < eligibleAmountForLottery && _bAddedHolderList[from]){ removeholder(from); }
        if (!_isExcludedFromLottery[to] && balanceOf(to) >= eligibleAmountForLottery && !_bAddedHolderList[to] && to != uniswapV2Pair){ addholder(to); }
        
        if (address(lotteryContract).balance > lotteryFireAmount){
    ...
    function _awardRandomEligibleHolder() private {
        if (_listOfHolders.length > 0){
           uint256 rndVal = random(100, 1000000, address(lotteryContract).balance) % _listOfHolders.length;
           lotteryContract.withdraw(_listOfHolders[rndVal]);
        }
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 marketingBNB = address(this).balance.div(totalFees - liquidityFee).mul(devFee);
        uint256 lotteryBNB = address(this).balance.div(totalFees - liquidityFee).mul(lotteryFee);
        (bool success,) = address(devWallet).call{value: marketingBNB}("");
        (success,) = address(lotteryContract).call{value: lotteryBNB}("");
...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() public DividendPayingToken("_Dividend_Tracker", "_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 200000000 * (10**9); //must hold 200000000+ tokens
    }
...
contract LotteryContract is Ownable {
    ...
    function withdraw (address reciever) public onlyOwner {
        ...
        if (success){
            emit withdrawBNB(reciever, balance);    
        }
