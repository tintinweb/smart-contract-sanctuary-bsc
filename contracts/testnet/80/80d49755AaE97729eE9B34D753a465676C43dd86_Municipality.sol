...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    function setNFTContractAddresses(address[4] memory _nftContractAddresses) external onlyOwner {
    ...
    function setDistributionReceiverAddresses(address[10] memory _distributionReciverAddresses) external onlyOwner {
    ...
    function purchaseBundle(ParcelsMintSignature calldata _mintingSignature, uint8 _bundleType,
        uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        // uint256 lastMinerAddress = IMinerNFT(minerV1NFTAddress).lastMinerId();
        // _attachMinersToParcels(_mintingSignature.parcels, lastMinerAddress);
    }
    ...
    function applyVoucher(uint256 numRequestedVouchers, uint256 minerId) external notZeroAddress {
        ...
        require(numAvailableVouchers >= numRequestedVouchers, "Municipality: Not enough vouchers on balance");
    ...
    function getUserMiners(address _user) public view returns (AttachedMiner[] memory) {
    ...
    // function _attachMinersToParcels(Parcel[] memory _parcels, uint256 _lastMinerAddress) private {
    //     uint256 parcelsCount = _parcels.length;
    //     uint minerId = _lastMinerAddress - parcelsCount + 1;
    //     for(uint256 parcelIndex; parcelIndex < parcelsCount; parcelIndex++){
    //         Parcel memory parcel = _parcels[parcelIndex];
    //         uint256 parcelId = IParcelInterface(standardParcelNFTAddress).getParcelId(parcel.x, parcel.y, parcel.parcelLandType);
    //         uint256[] memory miners; 
    //         for(uint256 minerIndex; minerIndex < standardParcelSlotsCount; minerIndex++) {
    //             miners[minerIndex] = minerId;
    //             minerParcelMapping[minerId] = parcelId;
    //             minerId++;
    //             IMinerNFT(minerV1NFTAddress).updateTokenLocking(minerId, true);
    //             uint256 minerHashrate = IMinerNFT(minerV1NFTAddress).hashrate();
    //             IMining(miningAddress).deposit(msg.sender, minerId, minerHashrate);
    //         }
    //         parcelMinersMapping[parcelId] = miners;
    //     }
    // }
...
/// @notice Buy and burn previously distributed amount
    function _buyAndBurnGYMNET(uint256 _buyAndBurnAmountGYM) private {
        ...
        _buyAndBurnAmountGYM = 0;
    }

    // function _provideVBTCLiquidity(uint256 _vbtcLiquidityAmount) private {
    //     _addLiquidity(busdAddress, _vbtcLiquidityAmount, 0, block.timestamp + 100);
    // }
    ...
    function _requireNumberNotEqual(uint256 _numberOne, uint256 numberTwo) private pure {
        require(
            ...
            "Municipality: this numbers can not be equal"
        );
    }
    ...
    function _mintParcels(Parcel[] calldata parcels) private
    {
        for (uint256 i = 0; i < parcels.length; ++i) {
            ...
            require(
                !IParcelInterface(standardParcelNFTAddress).parcelExists(parcel.x, parcel.y, parcel.parcelLandType),
                "Municipality: Parcel already exists as a standard parcel"
            );
            require(
                !IParcelInterface(businessParcelNFTAddress).parcelExists(parcel.x, parcel.y, parcel.parcelLandType),
            ...
            IParcelInterface(addr).mint(msg.sender, parcel.x, parcel.y, parcel.parcelLandType);
        }
    }
    ...
    function _verifySigner(ParcelsMintSignature memory mintParcelSignature) private view  {
        ...
        for(uint256 index; index < mintParcelSignature.parcels.length; index++){
            ...
            address signer = ECDSAUpgradeable.recover(_digest, mintParcelSignature.signatures[index]);
