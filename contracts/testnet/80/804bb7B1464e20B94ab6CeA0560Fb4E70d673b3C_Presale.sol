...
contract Presale is Ownable {
    ...
    vestingContractInterface vestingAddress;
    ...
    function setvestingAddress(address _vestingAddress) external onlyOwnerAndAdmin {
        vestingAddress = vestingContractInterface(_vestingAddress);
    }
    ...
    function setAdminForpreSale(address _address) public onlyOwnerAndAdmin{
        vestingContractInterface(vestingAddress).setAdmin(_address);
    }
          
    function setTimeUnit(uint _unit) public onlyOwnerAndAdmin{
        vestingContractInterface(vestingAddress).setTimeUnit(_unit);
    }
    ...
    function buyToken(uint8 _roundId) public payable  {
        ...
        require(!vestingContractInterface(vestingAddress).getIslaunched(_roundId),"Already Listed!");
    ...
    function vestingCreate(
        ...
        // else{
            ...
            vestingContractInterface(vestingAddress).createVesting(_userAddress,_roundId,tokenAmount);
    ...
    function createRoundData( 
        ...
        uint256 _tgePrecentage
            ...
            vestingContractInterface(vestingAddress).setRoundData(_roundId,_totalTokensForSale,_tokenPrice,_totalvestingDays,_vestingStartTime,_vestingSlicePeriod,_tgePrecentage);
        }
    ...
    //     return(vestingContractInterface(vestingAddress).getLaunchedAt(_roundId));
    // }
// this function has to be internal
    ...
    function getVestingId(address _walletAddress) public view returns(uint256[] memory) {
        return vestingContractInterface(vestingAddress).getVestingIds(_walletAddress);        
    }
 
    function getTimeUnit() public view returns(uint _timeUnit){
        return vestingContractInterface(vestingAddress).timeUnit();
    }

    // function editListedAt(uint256 _time) public {
    //     return vestingContractInterface(vestingAddress).editListedAt(_time);

    // }

    function launchRound(uint8 _roundId, uint256 _vestingStartTime) public onlyOwnerAndAdmin {
         vestingContractInterface(vestingAddress).launchRound(_roundId,_vestingStartTime);
    }

    function getClaimAmount(address _walletAddress,uint256 _vestingId) public view returns(uint _claimAmount) {
        return vestingContractInterface(vestingAddress).getClaimableAmount(_walletAddress,_vestingId);
    }
    ...
    function getUserVestingData(address _address,uint256 _vestingId) public view returns(
        ...
        return vestingContractInterface(vestingAddress).userClaimData(_address,_vestingId);
    ...
    function getRoundData(uint8 _roundId) public view returns(bool,uint256,uint256,uint256,uint256,uint256,uint256) {
        return( vestingContractInterface(vestingAddress).getRoundData(_roundId)) ;
    }
