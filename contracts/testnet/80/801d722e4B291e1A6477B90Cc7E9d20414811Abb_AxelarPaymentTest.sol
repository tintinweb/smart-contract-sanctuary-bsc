...
contract AxelarPaymentTest is IAxelarExecutable {
    event Payment(
    ...
    event Received(string indexed sourceChain, string indexed sourceAddress);
    ...
    function pay(string memory _chainName, string memory _targetAddress)
        external
        payable
    {
        ...
        gateway.callContract(_chainName, _targetAddress, "");

        emit Payment(value, balanceBefore, balanceAfter);
    }
    ...
    function _execute(
        ...
        emit Received(sourceChain, sourceAddress);
    }
}
