...
contract balancer is Ownable {
  ...
  function setAuth(address _auth) public onlyOwner {
	...
	function getProposalAbove() public view returns (uint256) {
      address pair = pairBusdLit;
      address litToken = IUniswapV2Pair(pair).token0();
      address busdToken = IUniswapV2Pair(pair).token1();
      uint256 balanceTokenLit = IERC20(litToken).balanceOf(pair);
      uint256 balanceTokenBusd = IERC20(busdToken).balanceOf(pair);

      uint256 desiredPeg = balanceTokenBusd / goodPeg;
      uint256 desiredAmountOutBUSD = (desiredPeg - balanceTokenLit) / 2; 
      
      return desiredAmountOutBUSD;
      
      // return getAmountInMin(_router, busdToken, litToken, desiredAmountOutBUSD);
