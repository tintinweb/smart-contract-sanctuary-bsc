...
contract GoldenNugget is Context, Ownable {
	...
	function BuryGold(address ref) public {
        ...
        uint256 eggValue = calculateGoldsell(GoldUsed);

        devFeePool = SafeMath.add(devFeePool, devFee(eggValue));
        ...
        if(randomNumber == reinvestmentCount) {
            ...
            internalPool = 0;
            reinvestmentCount = 0;
            randomNumber = requestRandomNumber(startNumber, endNumber);
        }
    ...
    function getGoldSinceLastBury(address adr) private view returns(uint256) {
    ...
    function addInternalBonus() public payable onlyOwner {
        require(initialized);
        internalPool = SafeMath.add(internalPool, msg.value);
    }
}