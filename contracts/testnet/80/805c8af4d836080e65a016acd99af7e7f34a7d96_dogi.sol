...
contract dogi is Context, IERC20, Ownable {
    ...
    address public charityWallet = 0x15119e6C8aEc26ed729716b91A38f180C0209Aa5;
    ...
    uint256 public _maxWalletSize = 100000000000 * 10**6;
    uint256 private numTokensSellToAddToLiquidity =100000000000 * 10**6;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(marketingAmount).sub(charityAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(marketingAmount).sub(charityAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(charityAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(marketingAmount).sub(charityAmount));
        } else {
            _transferStandard(sender, recipient, amount.sub(marketingAmount).sub(charityAmount));
        }
