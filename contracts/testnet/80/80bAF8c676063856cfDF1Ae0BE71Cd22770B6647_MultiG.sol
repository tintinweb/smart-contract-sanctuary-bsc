...
contract  MultiG is ERC20, ReentrancyGuard {
    string public constant symbol = "MultiG"; //MultiG
    string public constant name = "MultiG"; 
...
//   uint32 public PromoPerc;   
   ...
   uint256 public claimableBNBearnings;
    ...
    bool public _CalculatinMultiGards; 
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "MultiG"); //this contract token
...
return (slices);
    ...
    function ChooseSinglePayoutToken(uint32 PayoutTokenNumber) public ChecMultiGrWork() {
       ...
       //token numbers + percentage slice
        function ChooseMultiplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
                                            uint32 Token3,uint32 Slice3,uint32 Token4,uint32 Slice4,
                                            uint32 Token5,uint32 Slice5,uint32 Token6,uint32 Slice6) public ChecMultiGrWork() {
...
function _StartRewardCalculation() private {
   if((_CalculatinMultiGards == false)&&(HolderPotBNB > 0))
   {
...
function CalculateNRewards(uint32 counts) public onlyAdmin() RefundGas(){
  ...
  require(_CalculatinMultiGards == true);
  if(counts > 0)
...
function CalculateRewards(uint32 counts) private returns (bool){
   ...
   {   
    ...
    uint32 N_calculated;
        ...
        while((_CalculatinMultiGards == true)&&(currenttry<maxtries))
         ...
         if(HolderPos >= N_holders) //all rewards for this cycle have been calculated
           ...
           _CalculatinMultiGards = false;  //all rewards calculated
           LastRewardCalculationTime = block.timestamp;
...
function ClaimMyRewards() public ChecMultiGrWork()
{ 
...
function setMyAutoPayout(bool Checked) public ChecMultiGrWork() returns (bool)
{
