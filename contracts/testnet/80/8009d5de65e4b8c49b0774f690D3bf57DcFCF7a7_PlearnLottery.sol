...
/** @title Plearn Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract PlearnLottery is IPlearnLottery, ReentrancyGuardUpgradeable, OwnableUpgradeable {
        ...
        nonReentrant
    {
        ...
        uint256 amountPlearnToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPlearn,
        ...
        _lotteries[_lotteryId].amountCollectedInPlearn += amountPlearnToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPlearnToTransfer
        uint256 rewardInPlearnToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPlearnToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        plearnToken.safeTransfer(msg.sender, rewardInPlearnToTransfer);

        emit TicketsClaim(msg.sender, rewardInPlearnToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPlearn) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in Plearn for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].plearnPerBracket[j] =
                ...
                // A. No Plearn to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].plearnPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPlearn - amountToShareToWinners);

        // Transfer Plearn to treasury address
        plearnToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        plearnToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInPlearn += _amount;
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            plearnPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPlearn: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].plearnPerBracket[_bracket];
        } else {
            return 0;
        }
    }
