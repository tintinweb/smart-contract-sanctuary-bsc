...
*/
    ...
    */
    ...
    address private _platformWallet = 0x34742629108AcE4d4a5aDC1764Fd2a08db52Dc50;

    address private _distributionWallet = 0x294658373ADBDBe836e2E841BB1996ceA9e56Fe3;

    uint256 public _lastUserId;

    uint256 public _minDistributionPerWallet = 40000 * 10 ** 9; //change to 100000 * 10 ** 9 for mainnet
    ...
    function start() external onlyOwner {
        ...
        for (uint i = 0; i<64; i++) {
            _revealDates[i] = first + i * 600;
        }
    }


    function sendReward(uint256 from, uint256 perPage, uint256 amount) external {
        require(_msgSender() == _distributionWallet, "Unauthorized wallet");
        require((perPage > 0 && from + perPage <= _lastUserId), "Invalid distribution range");
        require(amount >= _minDistributionPerWallet, "Amount too low to distribute");
        require(perPage * amount <= address(this).balance, "Insufficient balance in pool");
        bool sent;
        for (uint i = from; i <= from + perPage; i++) {
            (sent, ) = payable(_idToAddress[i]).call{value: amount}("");
        }
    }
    ...
    function setLeader(address account, bool isLeader) external onlyOwner {
        ...
        users[account].isLeader = isLeader;
    }


    function setMinDistributionPerWallet(uint amount) external onlyOwner {
        _minDistributionPerWallet = amount;
    }

    function setDistributionWallet(address account) external onlyOwner {
        require(account != address(0), "Zero address not allowed");
        _distributionWallet = account;
    }


    function setPlatformWallet(address account) external onlyOwner {
        require(account != address(0), "Zero address not allowed");
        _platformWallet = account;
    }
    ...
    function findFreePlace(address account, uint256 level) public view returns (address) {
    ...
    function findUpline(address account, uint256 level) public view returns(address) {
        address ref = users[account].referral;
    ...
    function processPayment(address account, uint256 level) internal {
        ...
        (bool sent, ) = payable(_developerWallet).call{value: totalAmount * _developerFee / 100}("");
        require(sent, "Failed to send BNB");
        emit Payment(_developerWallet, "developer", totalAmount * _developerFee / 100);
        uint256 rest = totalAmount - (totalAmount * _developerFee / 100) - (totalAmount / 20);
        for (uint i = 0; i < 3; i++) {
        ...
        (sent, ) = payable(_platformWallet).call{value: rest}("");
        require(sent, "Failed to send BNB");
        emit Payment(_platformWallet, "platform", rest);
    }
