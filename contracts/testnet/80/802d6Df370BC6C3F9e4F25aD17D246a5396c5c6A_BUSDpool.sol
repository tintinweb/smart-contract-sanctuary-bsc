...
contract BUSDpool is Ownable {
  ...
  event BuyBox(uint256 indexed userId, uint256 orderId, uint256 amount);
  ...
  function buyBox(uint256 userId, uint256 orderId, uint256 amount) external {
    ...
    emit BuyBox(userId, orderId, amount);
  }

  function withdraw() external onlyOwner {
    uint256 currentBalances = IERC20(i_BUSD).balanceOf(address(this));
    if(currentBalances < 0) revert BUSDpool__EmptyBalance();
    bool success = IERC20(i_BUSD).transfer(owner(), currentBalances);
    if(!success) revert BUSDpool__WithdrawFailed();
    emit Withdraw(currentBalances);
  }
