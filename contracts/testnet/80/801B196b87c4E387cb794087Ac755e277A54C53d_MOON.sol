...
// File: PyeSwap_token/contracts/interfaces/IMOON.sol
...
// File: PyeSwap_token/contracts/MOON.sol
...
contract MOON is IMOON, Context, Ownable {
    ...
    // Allocate BNB in contract
    mapping (address => uint256) public ledgerMoonshot;
    mapping (address => uint256) public ledgerBuyback;
    ...
    string constant _name = "MeekaPup";
    string constant _symbol = "MEEKA";
    ...
    function _takeFees(FeeValues memory values) private {
        ...
        _takeFee(values.mechanism, address(this));
        _takeFee(values.reflection, address(this));
        _takeFee(values.buyBack, address(this));
    }
    ...
    function getMoonshotBalance()
        public view returns (uint256){
        return ledgerMoonshot[WBNB];
    }

    function getBuybackBalance()
        public view returns (uint256){
        return ledgerBuyback[WBNB];
    }
    ...
    function depositETH(uint256 _value) internal {
        ...
        uint256 calcStoredFee = _defaultFees.mechanismFee.add(_defaultFees.taxFee).add(_defaultFees.buybackFee);
        uint256 calcStoredMshot = _defaultFees.mechanismFee.div(calcStoredFee);
        uint256 calcStoredReflect = _defaultFees.taxFee.div(calcStoredFee);
        uint256 calcStoredBuyback = _defaultFees.buybackFee.div(calcStoredFee);
        totalBountys[WBNB] = amount.mul(calcStoredReflect).add(totalBountys[WBNB]);
        bountysPerShare[WBNB] = bountysPerShare[WBNB].add(bountysPerShareAccuracyFactor.mul(amount).div(totalShares));
        ledgerMoonshot[WBNB] = amount.mul(calcStoredMshot).add(ledgerMoonshot[WBNB]);
        ledgerBuyback[WBNB] = amount.mul(calcStoredBuyback).add(ledgerBuyback[WBNB]);
    }
