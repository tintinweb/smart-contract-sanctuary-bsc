...
contract Mining is Ownable, ReentrancyGuard {
    ...
    // 开始新的一轮。前提条件是上一期必须是没有质押满，并且时间也结束了
    function startTurns(uint256 _total) public onlyOwner {
        // 判断上一轮的最后一期是否满足结束条件
    ...
    // 用户质押
    // 假的触发事件
    event USERDEP1(address eteUsdtLp, address _user,address addressThis, uint256 lpAmount);
    event USERDEP2(address ete, address leader, uint256 _v1);
    event USERDEP3(address ete, address team, uint256 _v2);
    function userDeposit(uint256 lpAmount) public nonReentrant {
        ...
        emit USERDEP1(eteUsdtLp,_user,address(this),lpAmount);
        // 触发质押事件
        ...
        if(_nowIssueMsg.userCount == 0 && _nowIssueMsg.nft1Count == 0 &&  _nowIssueMsg.nft2Count == 0) {
            ...
            if(_nowIssue > 1) {
                ...
                emit USERDEP2(ete,leader,_v1);
                emit USERDEP3(ete,team,_v2);                
                // 增加已经领取金额。
    ...
    // 用户领取收益+本金
    event USERTA0(address eteUsdtLp, address user, uint256 count);
    event USERTA1(address ete, address user, uint256 _earnMy);
    event USERTA2(address ete, address super1, uint256 _earnMySuper);
    event USERTA3(address ete, address superOther25, uint256 _earnMyOtherSuper);
    event USERTA4(address eteUsdtLp, address user, uint256 count);
    function userTake(bytes32 order) public nonReentrant {
        ...
        if(_orderIssueMsg.status == 4) {
            ...
            emit USERTA0(eteUsdtLp,msg.sender,_orderMsg.count);
