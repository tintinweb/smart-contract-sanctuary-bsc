...
contract ChainXStaking is ERC20, Ownable {
    ...
    address public walletReceiver = 0x23b7120AB419aaB3637169EA2685ba133ef1A2A8;
    ...
    constructor() ERC20("ChainedX Staking Protocol", "ChainedX-SP") {
        interestRate = 8;
    ...
    function getRewardPerBlock(uint256 amount) public view returns (uint256) {
        uint256 rewardPerBlock = (amount * interestRate) /
        ...
        returns (uint256)
    {
        ...
        uint256 startCountBlock = _stakers[account].checkPointBlock;
        uint256 rewardPerBlock = getRewardPerBlock(balance);
        uint256 availableBlock = (block.number - startCountBlock);
        uint256 availableClaimReward = rewardPerBlock * availableBlock;
    ...
    function _getForecastRewardByAmount(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 rewardPerDay = getRewardPerBlock(amount);
        uint256 availableDay = (timeEnd - block.timestamp) / 1 days;
    ...
    function claimReward(address account) public payable {
        uint256 availableReward = getAvailableClaimReward(account);
        ...
        require(
            ...
            "msg.value must be between bnbFee and 0.1"
        );

        _claimReward(account, msg.value);
    }

    function _claimReward(address account, uint256 bnbAmount) internal {
        uint256 availableReward = getAvailableClaimReward(account);

        payable(walletReceiver).transfer(bnbAmount);

        uint256 fee = (availableReward * 5) / 100;

        uint256 claimAmount = availableReward - fee;

        IERC20 ERC20token = IERC20(stakingToken);
        ERC20token.safeTransfer(walletReceiver, fee);
        ERC20token.safeTransfer(account, claimAmount);

        totalClaimedReward += availableReward;
        totalAllocationReward -= availableReward;

        _stakers[account].checkPoint = block.timestamp;
        _stakers[account].claimedReward += availableReward;
        _stakers[account].allocationReward -= availableReward;
        _stakers[account].checkPointBlock = block.number;
    }
    ...
    function deposit(uint256 amount) external payable {
        ...
        if (_stakers[msg.sender].balance > 0) {
            ...
            uint256 allocation = _stakers[msg.sender].allocationReward;
            totalAllocationReward -= allocation;
            _stakers[msg.sender].allocationReward = 0;
        } else {
