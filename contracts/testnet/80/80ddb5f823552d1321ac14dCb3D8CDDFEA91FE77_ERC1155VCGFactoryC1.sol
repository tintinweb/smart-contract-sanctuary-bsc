...
import "../tokens/erc-1155/ERC1155VCG.sol";
import "@openzeppelin/contracts/proxy/beacon/BeaconProxy.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC1155VCG token.
 *
 * The beacon should be initialized before call ERC1155VCGFactoryC1 constructor.
 *
 */
contract ERC1155VCGFactoryC1 is Ownable {
    address public beacon;

    event Create1155VCGProxy(address proxy);
    ...
    function createToken(
        ...
        address beaconProxy = deployProxy(
            getData(_name, _symbol, baseURI, contractURI),
            salt
        );
        ERC1155VCG token = ERC1155VCG(address(beaconProxy));
        token.transferOwnership(_msgSender());
        emit Create1155VCGProxy(beaconProxy);
    }
    ...
    function getData(
        ...
        return
            abi.encodeWithSelector(
                ERC1155VCG.__ERC1155VCG_init.selector,
                _name,
                _symbol,
                baseURI,
                contractURI
...
contract ERC1155VCG is ERC1155Base {
  ...
  function __ERC1155VCG_init(
    ...
    __ERC1155VCG_init_unchained(_name, _symbol, baseURI, baseExtension, contractURI);
  ...
  function __ERC1155VCG_init_unchained(
    ...
    __ERC1155Base_init_unchained(_name, _symbol, baseURI, baseExtension);
...
contract ERC1155Base is Initializable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, HasContractURI {
  using CountersUpgradeable for CountersUpgradeable.Counter;
  string public name;
  string public symbol;
  string private stringUri;
  ...
  function setUriExtension(string calldata _baseExtension, string calldata _baseUri) public onlyOwner {
    baseExtension = _baseExtension;
    stringUri = _baseUri;
  }

  function uri(uint256 _tokenId) public view override returns (string memory) {
    return string(abi.encodePacked(stringUri, StringsUpgradeable.toString(_tokenId), baseExtension));
  }
