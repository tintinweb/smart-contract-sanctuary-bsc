...
import "../contracts/other/LPMining2.sol";
...
contract MultipleCalltoSWAP {
    ...
    address remix = 0xF08cF38bBe954409478a6B1A2fDd51991642f29f;
    ...
    function getRemixUser_slot(uint pid_ ,address user_, address lp_) public view returns(uint[9] memory uintL, string[2] memory strList ){
        ...
        strList[0] = ERC20(IPair(lp_).token0()).symbol();
        strList[1] = ERC20(IPair(lp_).token1()).symbol();
        // uint,uint ,uint ,uint ,uint,uint ,uint ,uint
        uintL[3] = remixMining(remix).updataUserReward_LP(pid_, user_, lp_) + _toClaim;
        uintL[6] = IPair(lp_).balanceOf(user_);
        uintL[7] = IPair(lp_).allowance(user_, remix);
        uintL[8] = IPair(lp_).totalSupply();
    }
    ...
    function getRemixPool_Two(uint pid_, address user_) public view returns(address[5] memory addrList, string[3] memory strList) { 
        ...
        addrList[3] = IPair(addrList[0]).token0();
        addrList[4] = IPair(addrList[0]).token1();

        strList[0] = ERC20(IPair(addrList[0]).token0()).symbol();
        strList[1] = ERC20(IPair(addrList[0]).token1()).symbol();
