...
contract SWAP is ISWAP, KSWAPContract, USDTContract, CPEContract, KingContract {
    ...
    function sellCPE(uint256 amount) external override returns (bool) {
        ...
        uint256 kAmount = amount.mul(900000).div(1000000).mul(cepPrice);
        _transferFromKing(address(this), msg.sender, kAmount);
    ...
    function buyCPE(uint256 orderNo) external override returns (bool) {
        ...
        uint256 amountOut = _get_U_K_AmountsOut(uAmount);
        uint256 amountOutMin = amountOut.mul(1000000).div(950000);
        ...
        uint256 cpeAmount = amounts[1].div(cpePrice);
        ...
        _buyRecord[msg.sender].push(
            Orders(mySelfNumber, amounts[1].div(mySelfNumber), amounts[1])
        );
        return true;
    }
