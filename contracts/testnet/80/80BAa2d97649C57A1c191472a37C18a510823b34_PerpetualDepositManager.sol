...
contract PerpetualDepositManager is PerpetualBaseFunctions, PerpetualUpdateFunctions, IFunctionList, IPerpetualDepositManager {
     ...
     * @param   _fAmount     The amount of collateral in 'collateral currency' to deposit.
     */
    function depositToDefaultFund(uint256 _poolId, int128 _fAmount) external override onlyOwner {
    ...
    function _deposit(
        ...
        address _traderAddr,
        int128 _fAmount
    ) internal updateFundingAndPrices(_perpetual.poolId) {
     ...
     *
     */
    function transferEarningsToTreasury(uint256 _poolId, int128 _fAmount) external override onlyOwner {
        require(_poolId <= iPoolCount, "pool index out of range");

        // force update of target default fund size
        PerpStorage.LiquidityPoolData storage liquidityPool = liquidityPools[_poolId];
        liquidityPool.iLastTargetPoolSizeTime = block.timestamp - liquidityPool.iTargetPoolSizeUpdateTime - 1;
        _updateDefaultFundTargetSize(_poolId, block.timestamp);
