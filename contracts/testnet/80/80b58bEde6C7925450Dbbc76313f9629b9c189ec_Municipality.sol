...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    function repairAllMiners(uint8 currencyType) external payable {
        ...
        _distributeAmounts(selectedCurrencyPrice, currencyType, OPERATION_TYPE_PURCHASE_REPAIR);
    ...
    ) private view returns (bool isInList){
        for(
            uint index;
    ...
    function _distributeAmounts(
        ...
        uint256 busdAmount = currency == CURRENCY_TYPE_BNB || currency == CURRENCY_TYPE_WBNB
            ? _convertBNBToBUSD(amount)
            : amount;
        ...
        _transferAmountTo(
            ...
            busdAmount * distributionPercentages[1] / 100
        );
        buyAndBurnAmountVBTC += busdAmount * distributionPercentages[2] / 100;
        uint256 liquidityAmount = distributionPercentages[3] * busdAmount / 100;
        ...
        uint netGymStreetDistributionAmount = busdAmount * distributionPercentages[6] / 100;
        _transferAmountTo(netGymStreetAddress, netGymStreetDistributionAmount);
        INetGymStreet(netGymStreetAddress).distributeRewards(
            netGymStreetDistributionAmount, 
            busdAddress,
            msg.sender
        );
    ...
    function _transferAmountTo(address _walletAddress, uint256 _amount) private {
