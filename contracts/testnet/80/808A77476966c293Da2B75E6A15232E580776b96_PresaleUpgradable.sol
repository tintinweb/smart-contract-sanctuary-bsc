...
abstract contract Referral is Initializable, OwnableUpgradeable {
    ...
    function getTotalFundRaised() external view returns (uint) {
    ...
    function gevelRefferalBonus() external view returns (uint256) {
    ...
    function getInactiveTime() external view returns (uint256) {
    ...
    function isRewardActiveReferrers() external view returns (bool) {
    ...
    function getRefereeBonusRateMap()
        external
        view
    ...
    function setRefereeBonusRateMap(uint256[] memory _refereeBonusRateMap)
        external
        onlyOwner
    {
    ...
    function getreferredAddresses(address addr) external view returns (address[] memory) {
        return accounts[addr].referredAddresses;
    }
     ...
     */
    function getRefereeBonusRate(uint256 amount) private view returns (uint256) {
    ...
    function AddReferrerAdmin(address payable referrer, address _referree)
        external
        onlyOwner
        returns (bool)
    {
        if (referrer == address(0)) {
            ...
            return false;
        } else if (isCircularReference(referrer, _referree)) {
            emit RegisteredRefererFailed(
                _referree,
                referrer,
                "Referee cannot be one of referrer uplines"
            ...
            return false;
        } else if (accounts[_referree].referrer != address(0)) {
            emit RegisteredRefererFailed(
                _referree,
                referrer,
                "Address have been registered upline"
        ...
        Account storage userAccount = accounts[_referree];
        Account storage parentAccount = accounts[referrer];

        userAccount.referrer = referrer;
        userAccount.lastActiveTimestamp = getTime();
        parentAccount.referredCount = parentAccount.referredCount + 1;
        parentAccount.referredAddresses.push(_referree);

        emit RegisteredReferer(_referree, referrer);
        return true;
    }
     ...
     */
    function payReferral(uint256 value, address _referee)
        internal
        returns (uint256)
    {
        ...
        for (uint i; i < levelRate.length; i++) {
            ...
            if (
                ...
                uint c = value.mul(levelRate[i]).div(LevelDecimals);
        ...
        updateActiveTimestamp(_referee);
     ...
     */
    function updateActiveTimestamp(address user) private {
    ...
    function Referral_init(
        ...
        require(
            _levelRate.length <= MAX_REFEREE_BONUS_LEVEL,
...
contract PresaleUpgradable is Referral, UUPSUpgradeable, PausableUpgradeable {
    ...
    function initialize() external initializer {
        ...
        TokenName = IERC20_EXTENDED(TokenContractAddress).name();
        TokenDecimals = IERC20_EXTENDED(TokenContractAddress).decimals();
        TokenTotalSupply = IERC20Upgradeable(TokenContractAddress).totalSupply();

        levelRateNew = [25, 11, 9, 8, 6, 4, 3, 4];
        levelMapNew = [1, 100];
        ...
        Referral_init(
            ...
            levelRateNew,
            levelMapNew,
            payable(0x7a0DeC713157f4289E112f5B8785C4Ae8B298F7F)
        );
        ...
        returns (uint256 tokenValue)
    {
        ...
        uint256 BNBValue = (_bnbValue * bnbPrice) / (10**TokenDecimals);
        tokenValue = BNBValue * PricePerUSDT;
        tokenValue = tokenValue / (10**TokenDecimals);
    }

    function BuyWithBNB(address payable _referrer)
        public
        payable
        whenNotPaused
    {
