...
contract Stake is Ownable, ReentrancyGuard {
    ...
    function addStakers(
        ...
        for(uint256 i = 0; i < _staker.length; i++) {
            ...
            Detail storage detail = detailStaker[staker];
    ...
    function editStaker(
        ...
        Detail storage detail = detailStaker[_staker];
    ...
    function viewStaker(uint256 _page) public view returns (address[] memory) {
        ...
        require(_page * 100 < listStaker.length, "Page is not exist");
        uint256 startIndex = 100 * (_page - 1);
        address[] memory _listStaker = new address[](100);
    ...
    function unstake() public nonReentrant mustNotPaused {
        Detail storage detail = detailStaker[msg.sender];
