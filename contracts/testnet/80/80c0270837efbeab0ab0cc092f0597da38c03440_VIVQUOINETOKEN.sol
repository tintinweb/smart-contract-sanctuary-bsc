...
contract Stakeable {
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
     ...
     */
    uint256 internal rewardPerDay = 6600;
    uint256 internal rewardPerDay_1 = 100000;
    uint256 internal rewardPerDay_2 = 133333;
    ...
    */
     ...
     */
      function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256){
             ...
             if(_current_stake.amount>=1000*10**18 && _current_stake.amount<500000*10**18)
            {
            return (((block.timestamp - _current_stake.since) / 1 hours) * _current_stake.amount) / rewardPerDay;
            }
            if(_current_stake.amount>=500000*10**18 && _current_stake.amount<2500000*10**18)
            {
            return (((block.timestamp - _current_stake.since) / 1 hours) * _current_stake.amount) / rewardPerDay_1;
            }
            if(_current_stake.amount>=2500000*10**18)
            {
            return (((block.timestamp - _current_stake.since) / 1 hours) * _current_stake.amount) / rewardPerDay_2;
            }
           //return (((block.timestamp - _current_stake.since) / 1 hours) * _current_stake.amount) / rewardPerDay;
      }

     function calculateStakeDay(Stake memory _current_stake) internal view returns(uint256)
      {
    ...
    */
     function _withdrawStake(uint256 amount, uint256 index) internal returns(uint256){
        ...
        require(current_stake.amount >= amount, "Staking: Cannot withdraw more than you have staked");
         ...
         if (stake_day +30 days >= block.timestamp)
         {
...
*/
contract VIVQUOINETOKEN is Ownable, Stakeable{
  ...
  */
  function _mint(address account, uint256 amount) internal {
    ...
    require(account != address(0), "QUOINETOKEN: cannot mint to zero address");
  ...
  */
  function _burn(address account, uint256 amount) internal {
    require(account != address(0), "QUOINETOKEN: cannot burn from zero address");
    require(_balances[account] >= amount, "QUOINETOKEN: Cannot burn more than the account owns");
  ...
  *  - sender balance most be = or bigger than amount
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "QUOINETOKEN: transfer from zero address");
    require(recipient != address(0), "QUOINETOKEN: transfer to zero address");
    require(_balances[sender] >= amount, "QUOINETOKEN: cant transfer more than your account holds");
   ...
   *   - owner and spender cannot be zero address
    */
    function _approve(address owner, address spender, uint256 amount) internal {
      require(owner != address(0), "QUOINETOKEN: approve cannot be done from zero address");
      require(spender != address(0), "QUOINETOKEN: approve cannot be to zero address");
    ...
    *   - The caller must have a allowance = or bigger than the amount spending
    ...
    function transferFrom(address spender, address recipient, uint256 amount) external returns(bool){
      ...
      require(_allowances[spender][msg.sender] >= amount, "QUOINETOKEN: You cannot spend that much on this account");
      // Transfer first
   ...
   */
    ...
    *
    ...
    function stake(uint256 _amount) public {
      ...
      require(_amount < _balances[msg.sender], "QUOINETOKEN: Cannot stake more than you own");
