...
contract FBITokenTest is ERC20, ERC20Burnable, Ownable {
    using SafeERC20 for IERC20;
    using Address for address;
    using SafeMath for uint;

    uint256 public _teamFee = 15;
    uint256 public _buyHandlingFee = 10;
    uint256 public _sellHandlingFee = 20;

    uint256 public _totalTaxIfBuying = _teamFee.add(_buyHandlingFee);
    uint256 public _totalTaxIfSelling = _teamFee.add(_sellHandlingFee);
    ...
    address public teamWalletAddress = 0xdF96EC81d67BB821A6cb8Fba0195D27ce1Bb072f;
    IWrap public wrap = IWrap(0xAfCc9147F2C9763B0e7ae1Ba1fBAFEbDa497c989);
    ...
    constructor () ERC20("FBITokenTest", "FBITT", 18, 10000000000 * 10 ** 18) {
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        bool freeOfCharge = sender == address(this) || sender == _owner  || recipient == address(this) || recipient == _owner;
        if (!freeOfCharge) {
            ...
            if (currentBalance > 0 && !swaping && sender != pancakePair) {
                swaping = true; 
                if(_buyFeeAmount > 0) {
                    _swapTokensForUsdt(_buyFeeAmount.mul(_teamFee).div(_totalTaxIfBuying), teamWalletAddress);
                }
                if(_sellFeeAmount > 0) {
                    _swapTokensForUsdt(_sellFeeAmount.mul(_teamFee).div(_totalTaxIfSelling), teamWalletAddress);
                }
                _buyFeeAmount = 0;
                _sellFeeAmount = 0;
                swaping = false;
