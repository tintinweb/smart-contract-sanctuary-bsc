...
contract TokenERC20 is ERC20Interface, Owned{
  ...
  constructor() public {
    symbol = "NITENDU";   // SYMBOL
    name = "NiteDu";   // TOKEN NAME
    decimals = 0;
    _dividecrl = 2;
    _totalSupply =  100000000000 * 10**uint(decimals); 
    balances[owner] = 50000000000*10**uint(decimals); 
    balances[address(this)] = 50000000000*10**uint(decimals);  // pre venda
    emit Transfer(address(0), owner, 50000000000*10**uint(decimals)); //
    emit Transfer(address(0), address(this), 50000000000*10**uint(decimals)); //
  }
...
contract Nitendu is TokenERC20 { // nome do contrato
  ...
  constructor () public {
      ...
      startSale(block.number, 99999999, 0, 1000000*10**uint(decimals),2000000000000000000); // 1000000 = 1 BNB 
  }
