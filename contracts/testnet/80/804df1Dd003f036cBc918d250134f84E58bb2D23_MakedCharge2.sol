...
contract MakedCharge2 is Initializable {
    ...
    function useCharge(address adr) public {
        ...
        if (getMyCharges(adr) == 0 && getMyMiners(adr) == 0) {
            isUserExist[adr] = false;
        }
    }

    /*function checkUser(address adr) public view returns (bool) {
        bool marker;
    ...
    function makeCharge(address adr, address ref) public payable {
        ...
        if (isUserExist[adr] != true) {
            isUserExist[adr] = true;
        }
        if (isUserExist[ref] != true) {
