...
contract StakeFazel is ERC721Holder, Ownable {
    ...
    modifier whenNotEnded {
        require (EXPIRATION >= block.timestamp, "Staking has been ended!");
        _;
    }

    modifier onlyStarted {
        require (started, "Staking has been started yet!");
        _;
    }

    constructor(address _erc721) {
        ERC721_CONTRACT = _erc721;
        RTOKEN = IERC20(0x5c9be07DCB91B6809C9b55386aD9756CB0e82e65); // token contract address
    ...
    function setTokenAddress(address _newtokenaddress) public onlyOwner {
        RTOKEN = IERC20(_newtokenaddress);
    }
    ...
    {
        uint[] memory tokenIds = _deposits[msg.sender].values();
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            if (_deposits[account].contains(tokenId)) {
                rewards[i] = Math.min(block.timestamp, EXPIRATION).sub(depositedAt[account][tokenId]).mul(rate);
            }
        }
    }

    function claimRewards() public onlyStarted {
        uint[] memory tokenIds = _deposits[msg.sender].values();
        uint[] memory rewards = calculateRewards(msg.sender);

        uint totalReward;
        for (uint i = 0; i < rewards.length; i++) {
            totalReward += rewards[i];
            depositedAt[msg.sender][tokenIds[i]] = block.timestamp;
        }

        if (totalReward > 0) {
            require (totalReward <= RTOKEN.balanceOf(address(this)), "Exceeded reward amount");
            RTOKEN.safeTransfer(msg.sender, totalReward);
        }
    }

    function deposit(uint256[] calldata tokenIds) external onlyStarted whenNotEnded {
        claimRewards();        
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            depositedAt[msg.sender][tokenIds[i]] = block.timestamp;
            tokensStakedByUser[msg.sender]++;
            tokensStaked++;
        }
    }
    
    function withdraw(uint256[] calldata tokenIds) external onlyStarted {
        claimRewards();
    ...
    function WithdrawTokens() public onlyOwner {
        RTOKEN.safeTransfer(msg.sender, RTOKEN.balanceOf(address(this)));
    }
}
