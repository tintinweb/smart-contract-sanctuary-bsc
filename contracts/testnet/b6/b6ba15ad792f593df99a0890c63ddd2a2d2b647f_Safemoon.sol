...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    uint256 private tSupply;
     ...
     uint256 private raSupply;
    uint256 private taSupply;

    uint256 public totReflection;
    uint256 public _rTotalSupply;
    ...
    function excludeFromReward(address account) public onlyOwner() {
        ...
        uint256 rSupplyTemp = _rTotal;   /////changed
        uint256 tSupplyTemp = _tTotal;   /////changed


        for (uint256 i = 0; i < _excluded.length; i++) {
             rSupplyTemp = rSupplyTemp.sub(_rOwned[_excluded[i]]);   //changed
             tSupplyTemp = tSupplyTemp.sub(_tOwned[_excluded[i]]);   //changed
        }

        rSupply = rSupplyTemp;  /////changed
        tSupply = tSupplyTemp;  /////changed

    }

    function REFValues() public view returns(uint256,uint256){
        uint256 a;
        uint256 b;
        ...
        b = _tTotal;
        for (uint256 i = 0; i < _excluded.length; i++) {
             a = a.sub(_rOwned[_excluded[i]]);
             b = b.sub(_tOwned[_excluded[i]]);
         }
        return (a,b);
    }
    ...
    function _getValues(uint256 tAmount, uint256 _tierIndex) public view returns (FeeValues memory) {
    ...
    function _getTValues(uint256 tAmount, uint256 _tierIndex) public view returns (tFeeValues memory) {
        FeeTier memory tier = feeTiers[_tierIndex];
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tTransferFee, uint256 currentRate) public pure returns (uint256, uint256, uint256) {
    ...
    function _getCurrentSupply() public view returns(uint256, uint256) {
        ...
        uint256 t1Supply = tSupply;
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > _rTotal || _tOwned[_excluded[i]] > _tTotal) return (_rTotal, _tTotal);
        r1Supply = r1Supply.sub(_rOwned[_excluded[i]]);           
        }
        if (r1Supply < _rTotal.div(_tTotal)) return (_rTotal, _tTotal); 
        return (r1Supply, t1Supply);
    }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        tSupply = tSupply.add(_values.tFee);  
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(_values.rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(_values.rTransferAmount);
        tSupply = tSupply.sub(_values.tFee);  
        _takeFees(sender, _values, tierIndex);
        _reflectFee(_values.rFee, _values.tFee);
        emit Transfer(sender, recipient, _values.tTransferAmount);
    }
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        _tOwned[recipient] = _tOwned[recipient].add(_values.tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(_values.rTransferAmount);
        tSupply = tSupply.sub(_values.tTransferAmount.add(_values.tEchoSystem).add(_values.tLiquidity).add(_values.tBurn));  
        _takeFees(sender, _values, tierIndex);
        _reflectFee(_values.rFee, _values.tFee);
        emit Transfer(sender, recipient, _values.tTransferAmount);
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        _rOwned[sender] = _rOwned[sender].sub(_values.rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(_values.rTransferAmount);
        tSupply = tSupply.add(_values.tTransferAmount.add(_values.tEchoSystem).add(_values.tLiquidity).add(_values.tBurn));  
        _takeFees(sender, _values, tierIndex);
        _reflectFee(_values.rFee, _values.tFee);
        emit Transfer(sender, recipient, _values.tTransferAmount);
    }
