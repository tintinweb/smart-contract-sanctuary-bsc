...
contract TreenvestToken is ERC20, ERC20Burnable, Pausable, AccessControl, ERC20Permit {
  ...
  modifier notPaused() {
    if (!hasRole(TRADER_ROLE, tx.origin) || !hasRole(TRADER_ROLE, _msgSender())) {
  ...
  function mint_token(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
    _mint(to, amount);
  }

  function burn_token(address account, uint256 amount) public onlyRole(MINTER_ROLE) {
