...
contract NFTMarket is ReentrancyGuard {
    ...
    constructor() {
        TokenAddr = 0x1314B0a78C150146b093E477BE31c60222bceE63;
    ...
    event MarketItemSold(uint256 indexed itemId, address indexed nftContract, uint256 indexed tokenId, address seller, address buyer, uint256 price, bool sold);

    function createMarketItem(address nftContract,uint256 tokenId,uint256 price) public payable nonReentrant {
        require(price > 0, "Price must at least be 1 TestingAC");
        require(msg.value == listingPrice,"msg.value must be equal to listing price, 0.025 eth");
        _itemIds.increment();
        uint256 itemId = _itemIds.current();
        idTtoMarketItem[itemId] = MarketItem(itemId,nftContract,tokenId,payable(msg.sender),payable(address(0)),price,false);
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        IERC20(TokenAddr).transferFrom(msg.sender, address(this), price); //transfer tokens to NFTMarket
        ...
        IERC20(TokenAddr).transfer(idTtoMarketItem[itemId].seller, price); //transfer tokens to seller from NFTMarket
