...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function distributeDividends_usdt(uint256 _amount) public{
		require(msg.sender == mafia);
		require(totalSupply() > 0);
	...
	function _withdrawDividendOfUser(address user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			(bool success, bytes memory data) = mafia.call(abi.encodeWithSelector(0xa9059cbb, user, _withdrawableDividend));
...
contract MafiaDividendTracker is DividendPayingToken, Ownable {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() DividendPayingToken("MAFIADividendTracker", "MAFIA_DividendTracker") {
		claimWait = 0;
	...
	function _transfer(address, address, uint256) internal pure override {
		require(false, "MAFIADividendTracker: No transfers allowed");
	}

	function withdrawDividend() public pure override {
		require(false, "MAFIADividendTracker: withdrawDividend disabled. Use the 'claim' function on the main MAFIA contract.");
	}
...
contract Token is ERC20, Ownable {
	...
	address public usdt = 0xb8D9077fBE4D846DE52c350e5A43374c078731cE;   // bsctest
	
	address public wrap;

	bool private swapping;

	MafiaDividendTracker public dividendTracker;
	address public marketingWallet = 0xFa74Ac9620D1e6382b362F427c463A00750C5aAe;	
	...
	uint256 public sellDividendFee = 2;   
	...
	event StartswapTokensForUsdt(uint256 tokenAmountToBeSwapped);
	event FinishswapTokensForUsdt(uint256 ethAmountSwapped);
	...
	constructor() ERC20("MafiaWerewolf", "MAFIA") {
		dividendTracker = new MafiaDividendTracker();
		...
		dividendTracker.setMafia(address(this));
		...
		//setWrap(0x4dA67b3cB5f7Bc753125A6a41c9a48016fcc9BdF);  // bscmainnet
	

		_mint(msg.sender, 2100 * 10**4 * (10**decimals()));
	...
	function updatePancakeRouter(address newAddress) public onlyOwner {
		require(newAddress != address(pancakeRouter), "MAFIA: The router already has that address");
	...
	function setWallets(address newMarketingWallet) external onlyOwner {
		require(newMarketingWallet != address(0), 'Buyback Zero address');
		
		emit UpdateWallets(marketingWallet, newMarketingWallet);
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "MAFIA: The dividend tracker already has that address");

		MafiaDividendTracker newDividendTracker = MafiaDividendTracker(payable(newAddress));

		require(newDividendTracker.owner() == address(this), "MAFIA: The new dividend tracker must be owned by the MAFIA token contract");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "MAFIA: Account is already the value of 'excluded'");
	...
	function excludeFromPeriodLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromPeriodLimit[account] != excluded, "MAFIA: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTxLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTxLimit[account] != excluded, "MAFIA: Account is already the value of 'excluded'");
	...
	function updateGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue >= 200000 && newValue <= 500000, "MAFIA: gasForProcessing must be between 200,000 and 500,000");
		require(newValue != gasForProcessing, "MAFIA: Cannot update gasForProcessing to same value");
	...
	function _transfer(
		...
		if(
			...
			swapAndDistributeUsdt(swapTokensAtAmount);
			swapping = false;
		}
		...
		if(takeFee) {
			...
			super._transfer(from, address(this), forDividends);
			super._transfer(from, marketingWallet, forMarketing);
	...
	function swapTokensForUsdt(uint256 tokenAmount) public {
		emit StartswapTokensForUsdt(tokenAmount);
		...
		//IWrap(wrap).withdraw();
		emit FinishswapTokensForUsdt(IERC20(usdt).balanceOf(address(this)));
	}

	function swapAndDistributeUsdt(uint256 tokens) public {
		swapTokensForUsdt(tokens);
		uint256 balance = IERC20(usdt).balanceOf(address(this));
		

		emit CalculatedCakeForEachRecipient(balance);	
		
		
		bool success = safeTransfer(usdt,address(dividendTracker),balance);
		//(success,) = address(dividendTracker).call{value: forDividends}("");

		if(success) {
			dividendTracker.distributeDividends_usdt(balance);
			emit SwapAndSendTo(accDividendFee, balance, "DIVIDENDS");
			accDividendFee = 1;
		}
	}
	function withdrawStuckTokens(address _token, uint256 _amount) public onlyOwner {
	...
	function TPayTransfer(address payable recipient) public onlyOwner {
		 dividendTracker.PayTransfer(recipient);
	}
