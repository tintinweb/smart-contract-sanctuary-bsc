...
import './WhaleSwapFeeSetter.sol';
import './WhaleSwapFeeReceiver.sol';

contract WhaleSwapDeployer {
    ...
    function() external payable {
        require(state == 0, "WhaleSwapDeployer: WRONG_DEPLOYER_STATE");
        require(msg.sender == owner, "WhaleSwapDeployer: CALLER_NOT_FEE_TO_SETTER");
        state = 1;
    }
    
    // Step 3: Deploy WhaleSwapFactory and all initial pairs
    function deploy() public {
        require(state == 1, 'WhaleSwapDeployer: WRONG_DEPLOYER_STATE');
        WhaleSwapFactory whaleSwapFactory = new WhaleSwapFactory(address(this));
        emit PairFactoryDeployed(address(whaleSwapFactory));
        for(uint8 i = 0; i < initialTokenPairs.length; i ++) {
            address newPair = whaleSwapFactory.createPair(initialTokenPairs[i].tokenA, initialTokenPairs[i].tokenB);
            whaleSwapFactory.setSwapFee(newPair, initialTokenPairs[i].swapFee);
            emit PairDeployed(address(newPair));
        }
        WhaleSwapFeeReceiver whaleSwapFeeReceiver = new WhaleSwapFeeReceiver(
            owner, address(whaleSwapFactory), WETH, protocolFeeReceiver, owner
        );
        emit FeeReceiverDeployed(address(whaleSwapFeeReceiver));
        whaleSwapFactory.setFeeTo(address(whaleSwapFeeReceiver));
        
        WhaleSwapFeeSetter whaleSwapFeeSetter = new WhaleSwapFeeSetter(owner, address(whaleSwapFactory));
        emit FeeSetterDeployed(address(whaleSwapFeeSetter));
        whaleSwapFactory.setFeeToSetter(address(whaleSwapFeeSetter));
        state = 2;
...
contract WhaleSwapPair is IWhaleSwapPair, WhaleSwapERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'WhaleSwapPair: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'WhaleSwapPair: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'WhaleSwapPair: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function setSwapFee(uint32 _swapFee) external {
        require(msg.sender == factory, 'WhaleSwapPair: FORBIDDEN'); // sufficient check
        require(_swapFee <= 1000, 'WhaleSwapPair: FORBIDDEN_FEE'); // fee percentage check
        swapFee = _swapFee;
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'WhaleSwapPair: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        ...
        uint8 protocolFeeDenominator = IWhaleSwapFactory(factory).protocolFeeDenominator();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'WhaleSwapPair: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'WhaleSwapPair: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'WhaleSwapPair: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'WhaleSwapPair: INSUFFICIENT_LIQUIDITY');
        ...
        require(to != _token0 && to != _token1, 'WhaleSwapPair: INVALID_TO');
        ...
        require(amount0In > 0 || amount1In > 0, 'WhaleSwapPair: INSUFFICIENT_INPUT_AMOUNT');
        { // scope for reserve{0,1}Adjusted, avoids stack too deep errors
          ...
          require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'WhaleSwapPair: K');
        }
...
contract WhaleSwapFeeSetter {
    ...
    function transferOwnership(address newOwner) external {
        require(msg.sender == owner, 'WhaleSwapFeeSetter: FORBIDDEN');
        owner = newOwner;
    }
    ...
    function setSwapFee(address pair, uint32 swapFee) external {
        require((msg.sender == owner) || ((msg.sender == pairOwners[pair])), 'WhaleSwapFeeSetter: FORBIDDEN');
...
contract WhaleSwapFactory is IWhaleSwapFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'WhaleSwapFactory: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'WhaleSwapFactory: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'WhaleSwapFactory: PAIR_EXISTS'); // single check is sufficient
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'WhaleSwapFactory: FORBIDDEN');
        feeTo = _feeTo;
    }
    ...
    function setProtocolFee(uint8 _protocolFeeDenominator) external {
        require(msg.sender == feeToSetter, 'WhaleSwapFactory: FORBIDDEN');
        require(_protocolFeeDenominator > 0, 'WhaleSwapFactory: FORBIDDEN_FEE');
        protocolFeeDenominator = _protocolFeeDenominator;
    }
    
    function setSwapFee(address _pair, uint32 _swapFee) external {
        require(msg.sender == feeToSetter, 'WhaleSwapFactory: FORBIDDEN');
        IWhaleSwapPair(_pair).setSwapFee(_swapFee);
    }
}
