...
contract UberTaxiController is Ownable {
    ...
    struct User {
        ...
        uint256 UTAXINum; //UTaxi数量

        uint256 totalDeposit; //推荐用户总入金

        uint256 UTAXIReward; //UTaxi推荐奖励
    ...
    mapping(uint256 => uint256) public utaxiProduceTotal; //UTAXI产出总量

    uint256 public uExchangeTTotal; //U兑换T的总量

    mapping(uint256 => uint256) public vehicleTotals; //车辆类型的总量

    uint256 public marketTransactionTotal; //市场交易总量

    uint256 public marketDealTotal;  //市场成交总量
    ...
    uint256 public serviceCharge = 10; //兑换手续费

    uint256 public transferCharge = 10; //转账手续费

    uint256 public sellFee = 10;//商城购买手续费

    uint256 public commissionFee = 0;//返佣比例

    uint256 public recommenderFee = 5;//推荐人奖励
    ...
    constructor() {
        ...
        TAXI20 = ERC20(0x25700c024d12d2838b6262730C6Af6FB0141Ef8d);
        ...
        vehicles[3] = Vehicle("Audi",         3, 4000*10**6, 3600*10**6, 4.2*10, 1000, true);
        vehicles[4] = Vehicle("BMW",          4, 6000*10**6, 5400*10**6, 4.4*10, 1000, false);
        vehicles[5] = Vehicle("MercedesBenz", 5, 8000*10**6, 7200*10**6, 4.6*10, 1000, false);
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        if (tokenType == 1) {//taxi
            ...
            require(users[msg.sender].UTAXINum >= vehicles[index].uPirce, "You don't have enough Utaxi");
    ...
    function vehicleGetReward(uint256 index) public isUsers(index) {
        ...
        uint256 vehicleTypeNum = allVehicles[index].types;
        //UTaxi的产出总量
        utaxiProduceTotal[vehicleTypeNum] += profit;
    ...
    function exchange(uint256 exchangeNum) public {
        ...
        require(users[msg.sender].UTAXINum >= exchangeNum, "Your UTaxiNum is insufficient");
    ...
    function playerTransfer(address adds, uint256 transferNum) public {
        ...
        require(users[msg.sender].UTAXINum >= transferNum, "Your UTaxiNum is insufficient");
