...
contract VotingEscrow is IVotingEscrow, ApolloxERC721EnumerableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function forceWithdraw(uint tokenId, uint256 unlockAmount) private{
        ...
        require(_canForcedWithdraw, "Operation not supported");

        uint256 lockedAmount = uint256(uint128(locked.amount));
        if(unlockAmount == 0){
            unlockAmount = lockedAmount;
        }else{
            require(unlockAmount <= lockedAmount, "unlockAmount more than lockedAmount");
        }
        ...
        uint256 penalty = unlockAmount * Math.max(_minRatio, Math.min(_maxRatio, _penaltyK * timeLeft / MAX_TIME)) / 100;
        //update check point
        if(amountLeft > 0){
            //Todo more check
