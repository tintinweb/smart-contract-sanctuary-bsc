...
contract ALEXStaking is Ownable, ReentrancyGuard {
    ...
    event TakeDepositFee(address indexed from, address indexed to, uint256 value);
    ...
    constructor(PoolInfo memory arg1) {   
        ...
        feeInfo[0].marketingShareForEmergency=5;
        feeInfo[0].marketingShareForDeposit=5;
    }
    ...
    function canWithdraw(uint256 _poolId, address _user) public view returns (bool) {
        return (canWithdrawTime(_poolId,_user)==0);
    }

    function canWithdrawTime(uint256 _poolId, address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_poolId][_user];
        PoolInfo storage pool = poolInfo[_poolId];

        require(user.isInitizilated , "First to Stake");
        require(pool.isInitizilated , "First to be Initizilated");
        
        if ((block.timestamp < user.lastDepositTime + pool.withdrawLockPeriod) && (block.number < pool.bonusEndBlock)){
            return user.lastDepositTime + pool.withdrawLockPeriod -  block.timestamp;
        }else{
            return 0;
        }
    }

    function getReferralCodeInfo(address _user) external view returns (ReferralInfo memory){
         return referralCodes[whoReferralCodes[_user]];
    }
    ...
    function pendingReward(uint256 _poolId,address _user) external view returns (uint256) {
        ...
        require(pool.isInitizilated , "First to Initilaze Pool");
        require(user.isInitizilated , "First to Stake");

        uint256 stakedTokenSupply = pool.balance;
    ...
    function deposit(uint256 _poolId,uint256 _amount,bytes32 _referallCode) external nonReentrant {
        UserInfo storage user = userInfo[_poolId][msg.sender];
        PoolInfo storage pool = poolInfo[_poolId];
        FeeInfo storage fees = feeInfo[_poolId];
        require(pool.isInitizilated , "First to Initilaze for Pool");
        require(pool.stakedToken.balanceOf(msg.sender)>= _amount,"Insufficient balance");
        if (pool.hasUserLimit) {
            require((_amount + user.amount) <= pool.poolLimitPerUser , "User amount above limit");
        }

        _updatePool(_poolId);
        if (user.amount > 0 && pool.HarvestEnabled) {
        ...
        if (_amount > 0 ) {
            ...
            if(fees.depositFee > 0) {
                ...
                if(referralCodes[_referallCode].own!=address(0) && referralCodes[_referallCode].own!=address(msg.sender)){
                    _depositfee = (2*_depositfee)/4;
                    referralReward = (( _amount * _depositfee ) / 10000 ) / 2 ;
                    pool.stakedToken.safeTransfer(address(referralCodes[_referallCode].own), referralReward ); 
                    referralCodes[_referallCode].Totalamount+=referralReward;
                    referralCodes[_referallCode].count+=1;
                }
                ...
                emit TakeDepositFee(address(this),address(this),fee-(fee * fees.marketingShareForDeposit )/ 10);
            }
        ...
        if(pool.HarvestEnabled || (user.amount == _amount && _amount > 0)){
            user.rewardDebt = (user.amount * pool.accTokenPerShare ) / pool.PRECISION_FACTOR;
        }else if(user.amount>_amount){
            uint256 reward=((user.amount-_amount) * pool.accTokenPerShare ) / pool.PRECISION_FACTOR -user.rewardDebt;
            user.rewardDebt = (user.amount * pool.accTokenPerShare ) / pool.PRECISION_FACTOR-reward;
        }
    ...
    function deposit(uint256 _poolId,uint256 _amount) external nonReentrant {
        ...
        PoolInfo storage pool = poolInfo[_poolId];
        FeeInfo storage fees = feeInfo[_poolId];
        bytes32 _referallCode=bytes32("0x");
        require(pool.isInitizilated , "First to Initilaze for Pool");
        require(pool.stakedToken.balanceOf(msg.sender)>= _amount,"Insufficient balance");
        if (pool.hasUserLimit) {
            require((_amount + user.amount) <= pool.poolLimitPerUser , "User amount above limit");
    ...
    function withdraw(uint256 _poolId,uint256 _amount) external nonReentrant {
        ...
        require(canWithdraw(_poolId,address(msg.sender)),"withdraw still locked");
    ...
    function canEmergencyWithdraw(uint256 _poolId,address user) external view returns(bool){
        if(userInfo[_poolId][user].amount==0){
            return false;
        }
        return true;
    }
    ...
    function emergencyWithdraw(uint256 _poolId) external nonReentrant {
        ...
        require(amountToTransfer > 0,"You have no money!");
        user.amount = 0;
        user.rewardDebt = 0;
        user.isInitizilated=false;
        if (amountToTransfer > 0) {
            if(pool.withdrawLockPeriod > 0) {
                ...
                uint256 marketing = (fee * fees.marketingShareForEmergency )/ 10;
                pool.stakedToken.safeTransfer(fees.markettingWallet, marketing);
                emit TakeDepositFee(address(this),address(this),fee-(fee * fees.marketingShareForEmergency )/ 10);
    ...
    function addPool(PoolInfo memory arg1) external onlyOwner {
        ...
        feeInfo[_poolId].marketingShareForEmergency=5;
        feeInfo[_poolId].marketingShareForDeposit=5;
    }
    ...
    function setPRECISION_FACTOR(uint256 _poolId,uint newValue) external onlyOwner {
        PoolInfo storage pool = poolInfo[_poolId]; 

        require(block.number < pool.startBlock, "Pool has started");
        require(newValue < 62 , "Should be inferio to 62");

        uint256 decimalsRewardToken = uint256(pool.rewardToken.decimals());
        require(decimalsRewardToken < newValue, "Should be inferio to 62");

        pool.PRECISION_FACTOR =  uint256(10**(newValue - decimalsRewardToken));
    }
