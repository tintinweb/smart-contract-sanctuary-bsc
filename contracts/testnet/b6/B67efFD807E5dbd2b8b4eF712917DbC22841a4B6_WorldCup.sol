...
contract WorldCup is OwnableUpgradeable {
    ...
    struct Match {
        ...
        // uint winner;                  // 胜利方 1 - team1 | 2 - team2
    }
    ...
    function setMatchId(uint matchId_) public onlyOwner {
        matchId = matchId_;
    }

    // function setStatus(bool status_) public onlyOwner {
    //     status = status_;
    //     mapMatch[matchId].status = 2;
    // }

    // 设置输赢池子数量 - 收益
    // function setPool() public onlyOwner {
    //     uint[] memory randomNums = random.getRandom(mapMatch[matchId].roller);
    //     require(mapMatch[matchId].winner == 0, "setPool: Has_been_settled");

    //     if (randomNums[0] > randomNums[1]) {
    //         mapMatch[matchId].winner = 1;
    //     } else {
    //         mapMatch[matchId].winner = 2;
    //     }
    //     // 比赛结束
    //     setStatus(false);
    // }
    ...
    function openCompetition(uint team1_, uint team2_) public onlyOwner {
        ...
        if (matchId % 127 > 1) {
            // 判断当前是否有比赛
            random.getRandom(mapMatch[matchId].roller);
        }
    ...
    function betting(uint amount_, uint team_) public {
        ...
        // 查询用户soul
        uint balance = soulToken.balanceOf(player);
        
        require(balance > 0 && balance >= amount_ && amount_ != 0, "betting: Soul_insufficient_balance");
        ...
        if (mapPlayerInfo[player].leverage == 0) {
        ...
        // 该场比赛 杠杆 * 数量
        uint leverageAmount = mapPlayerInfo[player].leverage * amount_;
        ...
        if (competitionType == 128 || competitionType == 64) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_;
            }
            // 记录
            ...
            // 总量
            mapInstantTotal[matchId].total += amount_;
        } else if (competitionType == 32) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_ * 53 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_ * 53 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                amount: amount_ * 53 / 100,
                leverageAmount: leverageAmount * 53 / 100,
                team: team_,
                matchId: matchId
            }));

            // 总量
            mapInstantTotal[matchId].total += amount_ * 53 / 100;
            mapChampionTotal[championIndex].soulAmount += amount_ * 37 / 100;
        } else if (competitionType == 16) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_ * 63 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_ * 63 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                amount: amount_ * 63 / 100,
                leverageAmount: leverageAmount * 63 / 100,
                team: team_,
                matchId: matchId
            }));

            // 总量
            mapInstantTotal[matchId].total += amount_ * 63 / 100;
            mapChampionTotal[championIndex].soulAmount += amount_ * 27 / 100;
        } else if (competitionType == 8) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_ * 73 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_ * 73 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                amount: amount_ * 73 / 100,
                leverageAmount: leverageAmount * 73 / 100,
                team: team_,
                matchId: matchId
            }));

            // 总量
            mapInstantTotal[matchId].total += amount_ * 73 / 100;
            mapChampionTotal[championIndex].soulAmount += amount_ * 17 / 100;
        } else if (competitionType == 4) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_ * 83 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_ * 83 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                amount: amount_ * 83 / 100,
                leverageAmount: leverageAmount * 83 / 100,
                team: team_,
                matchId: matchId
            }));

            // 总量
            mapInstantTotal[matchId].total += amount_ * 83 / 100;
            mapChampionTotal[championIndex].soulAmount += amount_ * 7 / 100;
        } else if (competitionType == 2) {
            if (team_ == 1) {
                mapInstantTotal[matchId].team1Amount += amount_ * 88 / 100;
            } else {
                mapInstantTotal[matchId].team2Amount += amount_ * 88 / 100;
            }
            mapPlayerRecord[player].push(Instant({
                player: player,
                amount: amount_ * 88 / 100,
                leverageAmount: leverageAmount * 88 / 100,
                team: team_,
                matchId: matchId
            }));

            // 总量
            mapInstantTotal[matchId].total += amount_ * 88 / 100;
            mapChampionTotal[championIndex].soulAmount += amount_ * 2 / 100;
        }
    ...
    function _setProceeds(address player) private {
        ...
        if (winner == mapPlayerRecord[player][0].team) {
            ...
            // (投注数量 / 赢者池子) * 败者池子数量 
            if (winner == 1) {
                totalBonus += mapPlayerRecord[player][0].leverageAmount * 1 gwei / mapInstantTotal[index].team1Amount * mapInstantTotal[index].team2Amount / 1 gwei;
            } else {
                totalBonus += mapPlayerRecord[player][0].leverageAmount * 1 gwei / mapInstantTotal[index].team2Amount * mapInstantTotal[index].team1Amount / 1 gwei;
            } 
            // 记录本金
        ...
        if (matchId % 127 == 0) {
            ...
            mapPlayerInfo[player].championEarnings = mapPlayerInfo[player].ssoulAmount * 1 gwei / mapChampionTotal[championIndex].soulAmount * mapChampionTotal[championIndex].soulAmount / 1 gwei;
            mapPlayerInfo[player].destroySsoul = mapPlayerInfo[player].ssoulAmount;
            // 当天收益结算
            mapPlayerInfo[player].ssoulAmount = 0;
        }
    ...
    function getInstantEarnings() public {
        ...
        require(earnings != 0 || principal != 0, "getInstantEarnings: No_bonus");
        ...
        if (nowTimestamp > (endTime % 1 days) && mapPlayerInfo[player].championEarnings != 0) {
            // 领取冠军收益   
            getChampionEarnings();
        }
    }
    ...
    // 领取冠军池收益
    function getChampionEarnings() public onlyOwner {
        uint nowTimestamp = block.timestamp % 1 days;
    ...
    function getParticipate(address player_) public view returns (bool) {
        if (mapPlayerRecord[player_].length == 0) {
            return true;
        } else {
            return mapPlayerRecord[player_][0].matchId != matchId;
        }
    }
    ...
    function setDelay(uint16 delay_) public onlyOwner {
