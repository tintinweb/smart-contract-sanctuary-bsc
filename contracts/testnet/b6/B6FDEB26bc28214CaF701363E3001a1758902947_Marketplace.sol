...
contract Marketplace is Ownable, Pausable {
    ...
    function setBlackList(address _blackAddress, bool is_ban) public onlyOwner {
        blacklist[_blackAddress] = is_ban;
    }
    ...
    function matchTransaction (
        ...
        //  NFT blacklist  sale ban
        require(!blacklist[addresses[1]], "NFT in blacklist");
    ...
    function batchMatchTransaction(address[3][] calldata addressList, uint256[3][] calldata valuesList ,bytes[] calldata signatureList  )public whenNotPaused returns(bool){
        address[3] memory addresses;
        uint256[3] memory values;
        bytes memory signature;
        for (uint i = 0; i< signatureList.length;i++){
            ...
            matchTransaction(addresses, values,signature) ;
        }
        return true;
    }

    function forceBatchMatchTransaction(address[3][] calldata addressList, uint256[3][] calldata valuesList ,bytes[] calldata signatureList  )public whenNotPaused returns(bool){
        address[3] memory addresses;
        uint256[3] memory values;
        bytes memory signature;
        for (uint i = 0; i<signatureList.length; i++){
        ...
        if  (  nft.ownerOf(values[0]) != addresses[0]){
            continue;
        }
        // Check payment approval and buyer balance
        IERC20 paymentContract = IERC20(addresses[2]);
