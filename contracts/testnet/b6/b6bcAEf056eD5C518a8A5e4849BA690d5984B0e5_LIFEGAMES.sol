...
contract LIFEGAMES is IERC20 {
   ...
   event TransferedToPool(address, uint256);
    ...
    // initialize upgradable contract
        ...
        addresses[2] = 0xe24a7ECA6fDf71EF057bd77a1EF8B21A5ae8A1E6; // Marketing
        addresses[3] = 0x7AF13ceEcF3Cd06ebE0A305fb2994cba21A30B65; // Rewards
        addresses[4] = 0x7FE850239D59b0AA8923963874948E68B6833E21; // Equipo
        addresses[5] = 0xd235eD438FB2D6Bd428F5AEdF67bc8AB03bcFB96; // Liquidez
        addresses[7] = 0x26dF456853F3EFa617a2fE392c525ba0784670D6; // Advisors
        addresses[8] = 0x23DA0303B3540dD51ED8EdB73c3B57b0d123E9A0; // Pre-venta
        addresses[9] = 0x09f33F64aAADf6A02956C9732b25d42DD9c2d4bC; // BUSD BuyBackBurn
        ...
        _isExcludedFromFee[addresses[9]] = true;

        // excluded from rewards
        _isExcluded[addresses[2]] = true;
        _isExcluded[addresses[3]] = true;
        _isExcluded[addresses[4]] = true;
        _isExcluded[addresses[5]] = true;
        _isExcluded[addresses[6]] = true;
        _isExcluded[addresses[7]] = true;
        _isExcluded[addresses[8]] = true;
        _isExcluded[addresses[9]] = true;
        maxBuyLimit = 1000000000000000000000; // 1000 BUSD
        ...
        setBusdBuyBurnAddress(addresses[9]);
    ...
    function createPair() public onlyOwner() {
        lpPair = IFactoryV2(dexRouter.factory()).createPair(busdAddress, address(this));
        lpPairs[lpPair] = true;
    }
    ...
    function setPairAddress(address add1) public onlyOwner() {
        ...
        _liquidityRatioHolders[msg.sender] = true; // todo errror
    ...
    function _transfer(
        ...
        if (transferDelayEnabled && block.number < (initialBlock + (1200))) {
            ...
            if (from == lpPair && maxBuyLimit > 0) {
                uint256 buyTokenAmountPriceInBUSD = getOutEstimatedTokensForTokens(address(this), busdAddress, amount);
                require(buyTokenAmountPriceInBUSD <= maxBuyLimit, "Max Buy Limit.");
            }
    ...
    */
    ...
    function sendToPools() internal {
        ...
        // get busd contract balance
        uint256 busdContractBalance = IERC20(address(busdAddress)).balanceOf(contractAddress);

        if (busdContractBalance > 0) {

            uint256 busdForLiquidityAmount = (busdContractBalance * _ratios.liquidityRatio) / (_ratios.total);
            ...
            uint256 busdBuyBurnAmount = (busdContractBalance * _ratios.buyBurnRatio) / (_ratios.total);
            ...
            uint256 busdReserveAmount = (busdContractBalance * _ratios.buyBurnRatio) / (_ratios.total);
