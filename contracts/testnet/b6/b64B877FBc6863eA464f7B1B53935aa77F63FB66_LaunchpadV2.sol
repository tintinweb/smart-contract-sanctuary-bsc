...
contract LaunchpadV2 is Ownable, ReentrancyGuard {
    ...
    event Mint(address indexed contractAddress, CampaignMode mode, address userAddress, address payeeAddress, address platformFeeAddress, uint256 size, uint256 fee, uint256 paltformFee);
    ...
    function mint_(address contractAddress, uint256 batchSize, CampaignMode mode) internal {
        ...
        // normal and white-list mint share MaxLaunchpadSupply
        require(ILaunchpadNFT(campaign.contractAddress).getLaunchpadSupply() + batchSize <= ILaunchpadNFT(campaign.contractAddress).getMaxLaunchpadSupply(), "reach campaign max supply");
    ...
    function mintBasicCheck(Campaign memory campaign, uint256 batchSize, CampaignMode mode) private view {
    ...
    function campaignCheck(Campaign memory campaign, CampaignMode mode) private view {
        ...
        require(campaign.maxSupply == ILaunchpadNFT(campaign.contractAddress).getMaxLaunchpadSupply(), "campaign max supply invalid");
        require(campaign.maxSupply > 0, "max supply can't be 0");
        ...
        require(campaign.maxBatch > 0, "max batch invalid");
        require(campaign.maxPerAddress > 0, "max per address can't be 0");
    }
