...
contract MinerToken is AccessControl, IERC20 {
    ...
    constructor(address recipient, address router02, address usdt) {
        ...
        _projectAddress = payable(0xBe248d7E4A33d05F13Aa7A3BE3773Beee3c67A65);
        _foundationAddress = payable(0xBe248d7E4A33d05F13Aa7A3BE3773Beee3c67A65);
        _liquidityAddress = payable(0xBe248d7E4A33d05F13Aa7A3BE3773Beee3c67A65);
    }
    ...
    function dailySettlement() external onlyRole(SETTLEMENT_ROLE) returns (bool) {
        for (uint i = 0; i < _accountList.length; i++) {
            ...
            if (block.timestamp.sub(info.settleTime) >= _airDropInterval && _balances[_accountList[i]] > 0) {
                ...
                info.cacheAmount = info.cacheAmount.add(_balances[_accountList[i]].mul(_airdrop).div(_cardinality));
            }
        }
        return true;
    }
    ...
    function dailyIncome() external onlyRole(INCOME_ROLE) returns (bool) {
        for (uint i = 0; i < _accountList.length; i++) {
            accountInfo storage info = _accountAirdrop[_accountList[i]];
            if (block.timestamp.sub(info.incomeTime) >= _airDropInterval && info.cacheAmount > 0) {
                info.incomeTime = block.timestamp;
                _balances[_accountList[i]] = _balances[_accountList[i]].add(info.cacheAmount);
                emit Transfer(address(0), _accountList[i], info.cacheAmount);
                info.cacheAmount = 0;
            }
        }
        return true;
    ...
    function _beforeTokenTransfer(
        ...
        if (recommender[to] == address(0) && !from.isContract() && !to.isContract()) {
            recommender[to] = from;
        }
            ...
            if (address(from) == address(uniswapV2Pair)) {
                ...
                for (uint256 i = 0; i < buyMarketRates.length; i++) {
                    if (account == address(0)) {
                        break;
                    } else if (_balances[account] > _airdropLimit) {
                        ...
                        emit Transfer(address(this), recommender[to], newFee);
    ...
    function swapAndLiquify(uint256 tokenAmount) internal {
        ...
        IERC20(USDT_ADDRESS).approve(address(uniswapV2Router), _totalSupply);
    ...
    function swapTokensForUSDT(uint256 tokenAmount) internal {
        ...
        uint256 usdtBalance = IERC20(USDT_ADDRESS).balanceOf(_liquidityAddress);
