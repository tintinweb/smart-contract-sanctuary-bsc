    ...
    UUPSUpgradeable
{
    ...
    function getUserRefereeAddress(
        address _address
    ) external view returns (address[] memory userReferees) {
        Account storage userAccount = accounts[_address];
        uint256 length = userAccount.referredAddresses.length;
        for (uint256 i; i < length; i++) {
            userReferees[i] = userAccount.referredAddresses[i];
        }
    }
    ...
    function getUserTeamRefereeAddress(
        ...
        Account storage userAccount = accounts[_address];
        uint256 length = userAccount.teamAddress.length;
        for (uint256 i; i < length; i++) {
            userTeamReferees[i] = userAccount.teamAddress[i];
        }
    }
