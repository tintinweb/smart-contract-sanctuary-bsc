...
contract ROI {
    ...
    uint256 public investedProfit = 200;
    ...
    function withdraw() external {
        ...
        uint256 profit = investedProfit.mul(investor.totalInvested).div(100);
    ...
    function collect(address _addr) internal {
        ...
        if (secPassed > 0 && investor.time > 0) {
            ...
            if (secPassed > 0) {
                collectProfit = (
                    uint256(investor.totalInvested).mul(per).div(percentDivider)
                ).mul(secPassed).div(TIME_STEP);

                if (
                    uint256(investor.totalWithdrawn).add(collectProfit) >
                    uint256(investor.totalInvested).mul(MAX_LIMIT).div(10)
                ) {
                    collectProfit = (
                        uint256(investor.totalInvested).mul(MAX_LIMIT).div(10)
                    ).sub(uint256(investor.totalWithdrawn));
                }
            }
