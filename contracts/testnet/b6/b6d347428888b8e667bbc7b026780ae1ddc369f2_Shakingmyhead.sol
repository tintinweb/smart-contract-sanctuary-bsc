...
contract Shakingmyhead is ERC20, Ownable {
 ...
 function updatedevWallet(address _newWallet) external onlyOwner {
    ...
    function setBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _rewardFee, uint256 _devFee ,uint256 totalFeesBuy) public onlyOwner {
        _marketingFeeBuy = _marketingFee;
        _LpFeeBuy = _liquidityFee;
        _busdDividendRewardsFeeBuy = _rewardFee;
        _devFeeBuy = _devFee;
        totalFeesBuy = _marketingFeeBuy.add(_LpFeeBuy).add(_busdDividendRewardsFeeBuy).add(_devFeeBuy);
        require(totalFeesBuy <= 50, "max 50%");
    }
    function setSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _rewardFee, uint256 _devFee ,uint256 totalFeesSell) public onlyOwner {
        _marketingFeeSell = _marketingFee;
        _LpFeeSell = _liquidityFee;
        _busdDividendRewardsFeeSell = _rewardFee;
        _devFeeSell = _devFee;
        totalFeesSell = _marketingFeeSell.add(_LpFeeSell).add(_busdDividendRewardsFeeSell).add(_devFeeSell);
        require(totalFeesSell <= 50, "max 50%");
    }
