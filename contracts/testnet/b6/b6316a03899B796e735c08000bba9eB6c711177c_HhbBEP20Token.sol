...
contract HhbBEP20Token is Context, IBEP20, Ownable {
    ...
    //lp矿池手续费
    uint public uniswapbuyfee = 1;
    uint public uniswapsalefee = 9;
    ...
    address public mainacc = address(0xE7d05302De8EDAc0D15869D92dc9aFFE2044a999); //接收全部代币的钱包

    constructor() public {
        _name = "火火币";
        _symbol = "HHB";
        _decimals = 18;
        _totalSupply = 49 * 10 ** 4 * 10 ** 18;
        maxuid = 0;

        _balances[mainacc] = _totalSupply;
        _whitearr[mainacc] = true;
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (_whitearr[sender] != true && _whitearr[recipient] != true && recipient == uniswapV2Pair) {
            ...
            //销毁
            _tokenTransfer(sender, burnacc, amount.mul(5).div(100));
            sxf -= 50;

            // 给上级发奖励
            while (nowpid > 0 && i < 7) {
                if (nums[nowpid] <= i) continue;
                //推荐人数不够不让拿
                pacc = uids_[nowpid];
                if (i == 0) {
