...
contract YTON is Ownable{
	...
	function invest(address referrer) public payable {
        ...
        usdt.transfer(marketingAddress, msg.value.mul(MARKETING_FEE).div(PERCENTS_DIVIDER)); //10%
    ...
    function getUserTimeRate(address userAddress) public view returns(uint256){
        User storage user = users[userAddress];
        uint256 timeMultiplier = (now.sub(user.checkpoint)).div(TIME_STEP);  //当前时间和充值时间的差值在除以1days   0.015451
        return timeMultiplier;    //10.0001+0.015451  = 10.0001+0.015451=10.015551
    }
