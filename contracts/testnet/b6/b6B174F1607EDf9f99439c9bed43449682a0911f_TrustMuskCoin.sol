...
contract TrustMuskCoin is Context, IERC20, Ownable {
    ...
    string private _name = 'Trust Musk Coin';
    string private _symbol = 'TMC';
    uint8 private _decimals = 18;
    uint256 public beginTime;
    uint256 public endTime;
    uint256 public presaleValue = 2000000000 * 10**18;
    ...
    constructor(
        ...
        _approve(address(this), owner(), 2000000000 * 10**18);
    ...
    function presale() public payable {
        require(block.timestamp >= beginTime, "Time error!");
        require(block.timestamp < endTime, "Sale ended!");
        require(presaleValue != 0, "Check presaleValue!");
        
        uint amount = msg.value * 250000;
        _transfer(address(this), _msgSender(), amount);
    }

    bool FirstLiquidityStatus = true;

    function addFirstLiquidity(uint valueLiquidity) public onlyOwner {
        require(FirstLiquidityStatus, "Liqidity already added!");
        addLiquidity(valueLiquidity, address(this).balance);
        FirstLiquidityStatus = false;
    }
    ...
    bool bTime = true;
    function setBeginTime() external onlyOwner {
        require(bTime, 'BeginTime already set');
        beginTime = block.timestamp;
        bTime = false;
    }

    bool eTime = true;
    function setEndTime(uint timeEndBlock) external onlyOwner {
        require(eTime, 'EndTime already set');
        endTime = timeEndBlock;
        eTime = false;
    }
