...
contract RewardDistribution is Ownable,Pausable {
    ...
    uint256 public getamountout;
    
    mapping (bytes => bool) private signStatus;
    
    EnumerableSet.AddressSet private userInfo;
    
    address private signerAddress;

    event claimEvent(address indexed user,uint256 amount,uint256 time);

    constructor(address _signerAddress,address _token, uint256 _startTime,uint256 _endTime){
    ...
    function timeUpdate(uint256 startTime,uint256 endTime) public onlyOwner {
        startDate = startTime;
        endDate = endTime;
    }
    ...
    function claim(uint256 amount,uint256 deadline,bytes memory signature) public whenNotPaused {
        address user = _msgSender();
        require(startDate < block.timestamp && endDate > block.timestamp, "Time issues");
        ...
        getamountout = getamountout.sub(amount);
        
        emit claimEvent(user,amount,block.timestamp);
    }
