...
contract FAN_MVP_FCFS {
    ...
    BEP20 busd = BEP20(0x7182Bc441b0ef15C965117f1EdF9B879499E38AC); // BUSD address testnet (CHQ)
   ...
   function updateLiveBNBPrice() external {
    ...
    function deposit() external payable {
        ...
        uint getUsdAmount = (amount * priceOfBNB)/100000000;
        dep[sender].amounts.push(amount);
        dep[sender].times.push(block.timestamp);
        totalDeposit += getUsdAmount;
        uint tokens = (amount * priceOfBNB / (100000000/buyPriceDecimal)) / buyPrice;
    ...
    function depositBusd(uint amount) external {
        ...
        address sender = msg.sender;
        require(busd.allowance(sender, address(this)) >= amount,"Insufficient Allowance");
