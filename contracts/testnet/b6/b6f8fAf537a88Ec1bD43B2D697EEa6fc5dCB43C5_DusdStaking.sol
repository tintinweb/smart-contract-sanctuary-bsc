...
contract DusdStaking is StakeInfo {
    ...
    constructor(address usd_) {
        usdAddress = usd_;
        depositAddress = IUSD(usdAddress).depositAddress();
    ...
    function userInfo(address token, address _account)
        ...
        returns (UserInfo memory _user)
    {
        return _userInfo[token][_account];
    }
    ...
    function updatePool(address token) private returns (uint256) {
        if (
            poolInfo[token].allocPoint > 0 &&
            poolInfo[token].totalStaked > 0 &&
            lastBounsToken > 0
        ) {
            uint256 _reward = (lastBounsToken * poolInfo[token].allocPoint) /
                totalAllocPoint;
        ...
        returns (uint256)
    {
        ...
        if (_user.stakedOf > 0) {
            ...
            uint256 _epoch_day = block.timestamp / bounsDuration;
            if (
                _epoch_day > lastBounsEpoch && poolInfo[_token].allocPoint > 0
            ) {
        ...
        returns (uint256)
    {
        ...
        if (_user.stakedOf > 0) {
            uint256 _accPerShare = poolInfo[_token].accPerShare;
            if (poolInfo[_token].allocPoint > 0) {
                _accPerShare +=
                    (getPendingReward() * 1e12) /
                    poolInfo[_token].totalStaked;
            }
            pending = ((_user.stakedOf * _accPerShare) / 1e12) - _user.rewardOf;
        }
        return pending;
