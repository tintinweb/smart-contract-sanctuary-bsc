...
contract DigipadStaking is Ownable {
    ...
    constructor(
       ...
       uint256 _emergencyWithdrawFess
        ...
        pooldata[30].returnPer = 3000; // 30%

        //90 days
        pooldata[90].lockupDuration = 90;
        pooldata[90].returnPer = 5000; // 50%

        //180 days

        pooldata[180].lockupDuration = 180;
        pooldata[180].returnPer = 8000; // 80%

        //365 days
        pooldata[365].lockupDuration = 365;
        pooldata[365].returnPer = 12000; // 120%
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
        ...
        if(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed){
            ...
            if(block.timestamp >= orderInfo.endtime){
                uint256 reward = (orderInfo.amount.mul(orderInfo.returnPer)).div(10000*365*orderInfo.lockupDuration);
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration ) external {
        ...
        uint256 userReward = (_amount.mul(pool.returnPer)).div(10000*365*_lockupDuration);
    ...
    function withdraw(uint256 orderId) external {
        ...
        uint256 reward = (amount.mul(orderInfo.returnPer)).div(10000*365*orderInfo.lockupDuration);
        uint256 claimAvlible =  reward.sub(orderInfo.claimedReward);
    ...
    function emergencyWithdraw(uint256 orderId) external {
        ...
        uint256 userReward = (orderInfo.amount.mul(orderInfo.returnPer)).div(orderInfo.lockupDuration);
        totalRewardPending = (totalRewardPending.sub(userReward)).sub(fees);
