...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    //mapping(uint256 => uint256) public nftIndexList;
    ...
    function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun {
        ...
        for(uint256 index = 0; index < _idNftList.length; index++){
            ...
            //nftIndexList[_idNftList[index]] = nftIdList[msg.sender].length-1;
    ...
    function unstakeNft(uint256[] calldata _idNftList) public nonReentrant {
        ...
        //bool notExistNft = false;
        // uint256[] storage indexList;
        // for(uint256 index = 0; index < _idNftList.length; index++){
        //     for(uint256 jndex = 0; jndex < nftIdList[msg.sender].length; jndex++){
        //         if(_idNftList[index] == nftIdList[msg.sender][jndex]){
        //             indexList.push(jndex);
        //         }
        //     }
        // }

        // require(indexList.length == _idNftList.length, "NFT not exist in list");
        
        for (uint256 index = 0; index < _idNftList.length; index++) {
            // Check if NFT exist in list
            require(checkExistNft(_idNftList[index]) == true, "NFT not exist in list");
            // Give back NFT
            uint256 nftId = nftIdList[msg.sender][findNftIndex(_idNftList[index])];
            //IERC721(hesicNftAddress).transferFrom(address(this), msg.sender, nftId);
            IERC721(hesicNftAddress).transferFrom(address(this), msg.sender, _idNftList[index]);

            // swap NFT position in list
            if(_idNftList[index] != nftIdList[msg.sender][userNft-index-1]){
                nftIdList[msg.sender][findNftIndex(_idNftList[index])] = nftIdList[msg.sender][userNft-index-1];
                nftIdList[msg.sender][userNft-index-1] = nftId;
            }
    ...
    function getInterest(address user) public view returns(uint256 nftTotalGain){ 
        nftTotalGain = (block.number - stakeBlock[user]).mul(nftIdList[msg.sender].length).mul(minInterest);
    }
