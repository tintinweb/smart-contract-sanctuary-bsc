...
contract Jackpot is Context, IERC20 {
  ...
  function changeTaxDist(uint256 marketing, uint256 jackpothr,uint256 jackpotday, uint256 burn, uint256 lp) public onlyOwner {
...
*/
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
    ...
    swapTokensForBNB(tokens_to_LP_Half + tokens_to_M + tokens_to_D + tokens_to_J);
    uint256 BNB_Total = address(this).balance - balanceBeforeSwap;

    uint256 split_M = (Percent_Marketing * 100) /
      (Percent_AutoLP + Percent_Marketing + Percent_Hourly_Jackpot + Percent_Daily_Jackpot);
...
uint256 public dailyPool; 
  ...
  function triggerLotteryDrawing() public onlyOwner isLotteryCompleted {
   ...
   address payable winnerAddress =  payable(winningTicket.addr);
      sendToWallet(winnerAddress, hourlyPool);
          emit LogWinnerFound(
      currentLotteryId,
...
address payable winnerAddressDaily =  payable(winningTicketDaily.addr);
      sendToWallet(winnerAddressDaily, dailyPool);
