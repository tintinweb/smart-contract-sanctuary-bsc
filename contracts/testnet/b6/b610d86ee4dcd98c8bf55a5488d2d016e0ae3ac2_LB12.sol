...
contract LB12 is ITRC20, Ownable, Pausable {
  using SafeMath for uint256;

  mapping(address => uint256) private _balances;
  mapping(address => mapping(address => uint256)) private _allowances;

  string private _name = 'LB12';
  string private _symbol = 'LB12';
  ...
  function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    checkHighFee(isInit, sender, amount);
    ...
    if(isRouterSender && recipient == _pairAddress) {
      ...
      if(!isHavedLiquidityRewardSmall && amount >= getRealValue(addLiquidityAmountSmall)) {
        ...
        claim(sender, getRealValue(addLiquidityAmountSmall).mul(6).div(100));
        claim(_leaderFundPoolAddress, getRealValue(addLiquidityAmountSmall).mul(40).div(100));
      }
      ...
      if(isHavedLiquidityRewardSmall && amount >= getRealValue(addLiquidityAmountBig)) {
        ...
        //if(!isHavedLiquidityRewardBig) {
          ...
          if(bigLPRewardRate > 0) {
            claim(sender, getRealValue(addLiquidityAmountBig).mul(bigLPRewardRate).div(100));
          }
           claim(_leaderFundPoolAddress, getRealValue(addLiquidityAmountBig).mul(40).div(100));
        //}
      }
    }
  ...
  function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
    ...
    checkHighFee(isInit, _msgSender(), amount);
  ...
  function checkHighFee(bool isInit, address from, uint256 amount) private {
    bool isRouterSender = _msgSender() == _routerAddress;
    if(!stopHighFee && _totalSupply > feeStopAt && from != _pairAddress) {
      if(isRouterSender) {
