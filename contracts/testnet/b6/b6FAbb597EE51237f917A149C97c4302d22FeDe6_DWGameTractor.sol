...
// File: contracts/DWGameTractor.sol
...
contract DWGameTractor {
    using SafeMath for uint256;
    DWRandomizator randomizator;
    DWNFTGenerator generator;
    DWGamified game;
    Tractor tractor;
    Peach peach;
    mapping(uint256 => uint256) public repairPrices;
    uint256 mintPrice;

    // Wallet => Request ID => quantity
    mapping(address => mapping(uint256 => uint256)) internal mintRequests;

    event MintRequest(uint256 _id);
    ...
    function setRepairPrices(uint256[] calldata _prices) external {
        for (uint256 i = 0; i < _prices.length; i++) {
            repairPrices[i] = _prices[i];
        }
    }
    ...
    function mint(uint256 _quantity) external payable {
        ...
        (bool sent, bytes memory data) = game.rewardPool().call{
            value: _rewardGas
        }("");
        require(
            sent,
        ...
        (sent, data) = game.bella().call{value: _mintGas}("");
        require(
            sent,
    ...
    function resolveMint(uint256 _id, address _target) external {
        for (uint256 i = 0; i < mintRequests[_target][_id]; i++) {
            ...
            uint8 level = generator.generateTractorRarity(seed);
            uint256 durability = generator.generateDurability(seed, level, 1);
            tractor.mint(_target, durability, level);
        }
        // Already minted
        mintRequests[_target][_id] = 0;
    }

    function buy(uint256 _tractorId) external payable {
        uint256 _tractorPrice = tractor.sellPrice(_tractorId);
        // Use the gas fee provided
        require(
            msg.value >= _tractorPrice,
            "Not enough BNB sent for the marketplace transaction."
        );

        // Make the PEACH transaction
        address payable _tractorOwner = payable(tractor.ownerOf(_tractorId));
        uint256 _commission = _tractorPrice.mul(game.rewardComission()).div(10);

        (bool sent, ) = _tractorOwner.call{
            value: _tractorPrice.sub(_commission)
        }("");
        require(sent, "Failed to send BNB to the tractor's current owner.");

        (sent, ) = game.rewardPool().call{
            value: _tractorPrice.sub(_commission)
        }("");
        require(sent, "Failed to charge commission.");
        // Make the purchase
    ...
    function repair(uint256 _tractorId) external payable {
        require(
            msg.value >= game.repairGas(),
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = game.refillPool().call{value: msg.value}("");
        require(
            sent,
        ...
        peach.transferFrom(
            msg.sender,
            game.refillPool(),
            repairPrices[tractor.getSpace(_tractorId).sub(1)].mul(
    ...
    function multiRepair(uint256[] memory _tractorIds) external payable {
        require(
            msg.value >= game.repairGas() * _tractorIds.length,
            "Not enough BNB sent for the heal transaction."
        );
        (bool sent, ) = game.refillPool().call{value: msg.value}("");
        require(
        ...
        uint256 totalRepairPrice = 0;
        for (uint256 i = 0; i < _tractorIds.length; i++) {
            tractor.repair(_tractorIds[i], 12);
            totalRepairPrice += repairPrices[
                tractor.getSpace(_tractorIds[i]).sub(1)
            ];
        }
        peach.transferFrom(
            msg.sender,
            game.refillPool(),
            totalRepairPrice.mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
    }
    ...
    function burn(uint256 _tokenId) external {
        tractor.burn(_tokenId);

        // TODO: decide the price here
    }
}