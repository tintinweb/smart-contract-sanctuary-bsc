...
contract OpenFundsV2 is Context, Ownable , ReentrancyGuard {
    ...
    function deposit(uint256 plan , uint256 _amount) public noReentrant  {
        ...
        if (plan == 1) {
            ...
            investPlan1[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 minutes,300);
        } else if (plan == 2) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan2[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 minutes,500);
        } else if (plan == 3) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan3[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 minutes,800);
        } else {
    ...
    function withdrawal(uint256 plan) public noReentrant {
        ...
        BusdInterface.transfer(msg.sender,userWeellyReturn );
        BusdInterface.transfer(dev,wFee);
        

        if(plan == 1) {
            investPlan1[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 days,3);
            totalWithdrawPlan1[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + userWeellyReturn);
        } else if (plan == 2) {
            totalWithdrawPlan2[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + userWeellyReturn);
            investPlan2[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 days,5);
        } else if (plan == 3) {
            totalWithdrawPlan3[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + userWeellyReturn);  
            investPlan3[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 7 days,8);
        } else {
            require(false,'Wrong plan!');
        }
    ...
    function unStake(uint256 plan) external noReentrant {
        ...
        uint256 UnstakeValueCore = SafeMath.div(SafeMath.mul(UnstakeValue,7),100);

        uint256 UnstakeValueReturn = SafeMath.div(SafeMath.mul(UnstakeValue, 3),100);
