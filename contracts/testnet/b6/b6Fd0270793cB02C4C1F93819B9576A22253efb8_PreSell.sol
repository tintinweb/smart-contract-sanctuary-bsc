...
contract PreSell is ModuleBase, SafeMath, Lockable, ECDSA {
    ...
    function buyMMT(uint256 usdtAmount, bytes32 parentId) external lock {
        ...
        mapBuy[++buyLength] = BuyData(msg.sender, usdtAmount, mmtAmount, block.timestamp, parentId, true);
    ...
    function claimaibot(uint8 claimType) external lock {
        ...
        if(claimType == 2) {
            ...
            mapClaimAI[msg.sender] = ClaimData(msg.sender, claimType, mmtAmount, true);
            require(IERC20(auth.getFarmToken()).transfer(msg.sender, mmtAmount));
        } else {
            mapClaimAI[msg.sender] = ClaimData(msg.sender, claimType, 0, true);
        }
    }
    ...
    function claimIPhone(uint8 claimType) external lock {
        ...
        if(claimType == 2) {
            ...
            require(IERC20(auth.getFarmToken()).balanceOf(address(this)) >= mmtAmount, "insuff fund");
            mapClaimIPhone[msg.sender] = ClaimData(msg.sender, claimType, mmtAmount, true);
            require(IERC20(auth.getFarmToken()).transfer(msg.sender, mmtAmount), "transfer err");
        } else {
            mapClaimIPhone[msg.sender] = ClaimData(msg.sender, claimType, 0, true);
        }
    }
