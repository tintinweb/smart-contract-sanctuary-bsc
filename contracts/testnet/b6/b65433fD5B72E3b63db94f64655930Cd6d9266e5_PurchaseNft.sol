...
contract PurchaseNft is Ownable {
    ...
    function purchase(Quality _quality)  payable public lockPurchase {        
        ...
        if ((_group == Group.A || _group == Group.B) && _whitelistSettings[_group].endTime >= block.timestamp && _whitelistSettings[_group].startTime < block.timestamp ) {
        ...
        nft.purchaseMint(tokenId, fees, tokenId.toString());
    ...
    function setLook(bool status) public onlyOwner {
        isLock = status;
    }

    modifier lockPurchase() {
        require(isLock == false, "Purchase is Lock");
        _;
    }
...
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base {
    ...
    function purchaseMint(uint256 tokenId, Fee[] memory _fees, string memory tokenURI ) public onlySigner {
         _mint(msg.sender, tokenId, _fees);
        _setTokenURI(tokenId, tokenURI);
    }
