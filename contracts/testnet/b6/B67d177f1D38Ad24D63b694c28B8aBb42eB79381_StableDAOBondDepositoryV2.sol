...
abstract contract FrontEndRewarder is StableDAOAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable STABLEToken; // reward token

    constructor(IStableDAOAuthority _authority, IERC20 _STABLEToken) StableDAOAccessControlled(_authority) {
    ...
    function getReward() external {
        ...
        STABLEToken.transfer(msg.sender, reward);
    }
...
interface INoteKeeper {
    ...
    struct Note {
        uint256 payout; // gSTABLEToken remaining to be paid
    ...
    function redeem(
        ...
        bool _sendgSTABLEToken
    ) external returns (uint256);

    function redeemAll(address _user, bool _sendgSTABLEToken) external returns (uint256);
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgSTABLEToken internal immutable gSTABLEToken;
    IStaking internal immutable staking;
    ITreasury internal treasury;

    constructor(
        IStableDAOAuthority _authority,
        IERC20 _STABLEToken,
        IgSTABLEToken _gSTABLEToken,
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _STABLEToken) {
        gSTABLEToken = _gSTABLEToken;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of STABLEToken due to the user
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({
                payout: gSTABLEToken.balanceTo(_payout),
        ...
        // note that only the payout gets staked (front end rewards are in STABLEToken)
     ...
     * @param _sendgSTABLEToken    send payout as gSTABLEToken or sSTABLEToken
     * @return payout_     sum of payout sent, in gSTABLEToken
     */
    function redeem(
        address _user,
        uint256[] memory _indexes,
        ...
        if (_sendgSTABLEToken) {
            gSTABLEToken.transfer(_user, payout_); // send payout as gSTABLEToken
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as sSTABLEToken
        }
    }
     ...
     * @param _sendgSTABLEToken    send payout as gSTABLEToken or sSTABLEToken
     * @return             sum of payout sent, in gSTABLEToken
     */
    function redeemAll(address _user, bool _sendgSTABLEToken) external override returns (uint256) {
        return redeem(_user, indexesFor(_user), _sendgSTABLEToken);
    }
     ...
     * @return payout_     the payout due, in gSTABLEToken
...
interface IBondDepository {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in STABLEToken (false, default)
    ...
    struct Terms {
        ...
        uint64 maxDebt; // 9 decimal debt maximum in STABLEToken
    }
...
/// @title StableDAO Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract StableDAOBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _STABLEToken, _gSTABLEToken, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _STABLEToken.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gSTABLEToken due
        ...
        )
    {
         ...
         *
         * where
         * payout = STABLEToken out
         * amount = quote tokens in
         * price = quote tokens : STABLEToken (i.e. 42069 DAI : STABLEToken)
         */
        payout_ = ((((_amount * 1e18) / (10**metadata[_id].quoteDecimals)) * 1e18) / price) ;
         ...
         *
         * this is either the number of STABLEToken that the market can sell
        ...
        // purchased, and how much STABLEToken has been sold
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
             ...
             * is 10,000 STABLEToken. max payout would be 1,000 STABLEToken (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in STABLEToken or quote), initial price / STABLEToken (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         * interval. for example, if capacity is 1,000 STABLEToken, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 STABLEToken.
         */
        ...
        emit CreateMarket(id_, address(STABLEToken), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in STABLEToken decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of STABLEToken to be paid in STABLEToken decimals
     *
     * @dev 1e18 = STABLEToken decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in STABLEToken decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in STABLEToken decimals
     */
