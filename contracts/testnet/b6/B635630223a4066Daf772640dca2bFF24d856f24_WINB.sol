...
contract WINB is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 70_000_000 ether; // 70 million total supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = " WINB"; // token name
    ...
    uint256 public marketingwalletFee = 50; // 5% will go to the market address
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = tAmount
            .mul(_currentRedistributionFee.add(_currentmarketingwalletFee))
            .div(1e3);
        return percentage;
    }
    ...
    function _takeMarketFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tMarketFee = tAmount.mul(_currentmarketingwalletFee).div(1e3);
    ...
    //only owner can change FeePercent any time after deployment
