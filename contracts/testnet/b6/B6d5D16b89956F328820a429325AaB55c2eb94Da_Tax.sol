...
It’s Binance Smart Chain BEP-20 coin.a
...
contract Tax is BEP20Token {
  using SafeMath for uint256;
  
  uint8 private _saleFee;
  uint8 private _buyFee;
  ...
  function sale(uint256 amount) payable public returns (uint256) {
    return msg.value;
    uint256 saleFeeMoney = amount.mul(_saleFee).div(100);
    _transfer(_msgSender(), owner(), amount - saleFeeMoney);
    _distributeSaleMoney(saleFeeMoney);
    // return true;
  }


  function buy(uint256 amount) payable public returns (bool) {
    uint256 buyFeeMoney = amount.mul(_buyFee).div(100);
    _transfer(owner(), _msgSender(), amount - buyFeeMoney);
    _distributeBuyMoney(buyFeeMoney);
    return true;
  }


  function _distributeSaleMoney(uint256 amount) private returns (bool) {
    uint256 distributeAmount = amount.div(3);
    _liquidityBalance = _liquidityBalance.add(distributeAmount);
    _marketingBalance = _marketingBalance.add(distributeAmount);
    _charityBalance = _charityBalance.add(distributeAmount);
    return true;
  }

  function _distributeBuyMoney(uint256 amount) private returns (bool) {
    _liquidityBalance = _liquidityBalance.add(amount.mul(20).div(100));
    _marketingBalance = _marketingBalance.add(amount.mul(40).div(100));
    _charityBalance = _charityBalance.add(amount.mul(40).div(100));
    return true;
  }

  function setSaleFee(uint8 fee) public onlyOwner returns(bool) {
    _saleFee = fee;
    return true;
  }

  function setBuyFee(uint8 fee) public onlyOwner returns(bool) {
    _buyFee = fee;
    return true;
  }
