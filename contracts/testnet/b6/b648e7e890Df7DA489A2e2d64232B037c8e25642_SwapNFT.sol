...
contract SwapNFT is Accept {
  ...
  function purchaseOrder(uint256 orderNumber) external payable {
    ...
    if (keccak256(bytes(tokenName)) == keccak256(bytes("bnb"))) {
    ...
    if (!order.isSingle) {
      for (uint256 i = 0; i < order.bundleIds.length; i++) {
        require(ierc721.isApprovedForAll(msg.sender, address(this)) == true || ierc721.getApproved(order.bundleIds[i]) == address(this), "NOT_APPROVED");
        ierc721.safeTransferFrom(order.seller, msg.sender, order.bundleIds[i]);
        isTokenListed[order.bundleIds[i]] = false;
      }
    } else {
        require(ierc721.isApprovedForAll(msg.sender, address(this)) == true || ierc721.getApproved(order.tokenId) == address(this), "NOT_APPROVED");
  ...
  function updatePrice(uint256[] calldata _orderNumbers, uint256 _percentage) external {
  ...
  function makeOffer(
    ...
    require(!isOrderExist[orderNumber] ,"ORDER_EXIST");
    if (keccak256(bytes(tokenType)) == keccak256(bytes("bnb"))) {
  ...
  function acceptOffer(uint256 orderNumber) external {
    ...
    uint256 _royality = computeRoyality(order.price, order.royalityPercent);
    address creatorAdd = ierc721.creator(order.tokenId);
    if (keccak256(bytes(tokenOf[tokenAddress])) == keccak256(bytes("bnb"))) {
