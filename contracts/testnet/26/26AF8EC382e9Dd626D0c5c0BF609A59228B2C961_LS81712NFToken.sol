...
/**
 * @title LS81712NF | Landshare LLC
 * @author Stobox Technologies Inc.
 * @dev LS81712NF ERC20 Token | This contract is opt for digital securities management.
 */
 
contract LS81712NFToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "LS81712NF: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "LS81712NF: funds are frozen."
        );
        _;
    }

    /**
     * @notice LS81712NFToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Landshare 817 12th Ave N Fargo Security Token";
        _symbol = "LS81712NF";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
     ...
     * @param _value LS81712NFg that enables or disables the whitelist.
     */
     ...
     * @param _value LS81712NFg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "LS81712NF: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "LS81712NF: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "LS81712NF: _x must not be equal to _y");
        require(_x > 0, "LS81712NF: _x must be greater than 0");
        require(_y > 0, "LS81712NF: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "LS81712NF: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "LS81712NF: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "LS81712NF: transfer from the zero address");
        require(_recipient != address(0), "LS81712NF: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "LS81712NF: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "LS81712NF: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "LS81712NF: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "LS81712NF: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "LS81712NF: approve from the zero address");
        require(_spender != address(0), "LS81712NF: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "LS81712NF: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "LS81712NF: transfer exceeds your transaction count limit");
        }
    }
}