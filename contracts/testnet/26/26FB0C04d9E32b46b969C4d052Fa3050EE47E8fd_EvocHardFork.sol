...
contract EvocHardFork is Ownable, ReentrancyGuard {
    ...
    function changeFork(uint256 tokenAmount, address contractAdd) public isPausable {
    ...
    function createHardFork(
        ...
        require(
            _quantityTokens > 0 && _quantityTokens <= dexBalance,
            "Insufficient amount of tokens for this transaction"
        );
        require(
            ContractToken.transferFrom(msg.sender, address(this), _quantityTokens),
            "A transaction error has occurred. Check for approval."
        );
    ...
    function cancelHardfork(address _contractAdd) public isPausable {
        require(
            HardForkRegister[_contractAdd].exist,
            "A HardFork does not exist, check the contract or create it first"
        );
        ...
        IBEP20 ContractAdd = IBEP20(HardForkRegister[_contractAdd]._contractAdd);
        uint256 dexBalance = ContractAdd.balanceOf(address(this));
        if(dexBalance > 0){
            ContractAdd.transfer(HardForkRegister[_contractAdd]._OwnerAdd, dexBalance);
        }
        HardForkRegister[_contractAdd].exist = false;
    }
