...
contract HiVpn is IHiVpn, Secure {
  ...
  function paymentListDetails() external view override returns (Payment[] memory) {
    Payment[] memory details = new Payment[](paymentList.length);

    for (uint256 i = 0; i < paymentList.length; i++) {
      details[i] = payment[paymentList[i]];
    }
  ...
  function paymentDetailFromIndex(
    uint256 index
  ) external view override returns (Payment memory) {
    return payment[paymentList[index]];
  }
...
interface IHiVpn {
  ...
  function paymentListDetails() external view returns (Payment[] memory);

  function paymentListIds() external view returns (uint256[] memory);

  function paymentDetailFromIndex(uint256 index) external view returns (Payment memory);
