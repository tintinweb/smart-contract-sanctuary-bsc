  ...
  contract STRKPresale{
    ...
    function getAvilableForHarvest(address user) public view returns (uint256 amount) {
        for(uint256 i = 0; i<users[user].roi.length; i++) {
           if(users[user].roi[i].timestamp.add(plan.locking_period)<block.timestamp && users[user].roi[i].isPricipleWithdraw!=true){
            amount=amount.add(users[user].roi[i].amount);
          }
      }
    }
     
     function getLockedBuyTokens(address user) public view returns (uint256 amount) {
        for(uint256 i = 0; i<users[user].roi.length; i++) {
           if(users[user].roi[i].isPricipleWithdraw!=true){
            amount=amount.add(users[user].roi[i].amount);
          }
      }
    }
    ...
    function WithdrawBuyedToken () external payable {
      ...
      for(uint256 i = 0; i<users[msg.sender].roi.length; i++) {
        if(users[msg.sender].roi[i].timestamp.add(plan.locking_period)<block.timestamp && users[msg.sender].roi[i].isPricipleWithdraw!=true){
