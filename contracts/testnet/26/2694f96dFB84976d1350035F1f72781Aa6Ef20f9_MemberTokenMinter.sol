 ...
 *
 * This contract accepts deposit token and stables. Locks it up
 */
contract MemberTokenMinter is
	...
	Pausable
{
		...
		isAllowedContract(msg.sender)
	{
		...
		require(
			depositToken.balanceOf(address(this)).add(_amount) <=
				maxNakedDepositCap(),
			'Exceeds deposit cap'
		);
		...
		uint256 memberTokenAmount = _amount.mul(multiplierBP).div(10000);
		require(
		...
		isAllowedContract(msg.sender)
	{
		...
		uint256 depositTokenAmount = _amount.mul(10000).div(multiplierBP);

		require(
			depositTokenAmount <= user.amount,
			"Amount can't exceed tracked balance"
		);

		// lock should have expired for a user to withdraw
		require(block.number > user.lockStartBlock.add(lockBlocks));

		// update user balance
		user.amount = user.amount.sub(depositTokenAmount);

		// transfer out tokens
		depositToken.safeTransfer(msg.sender, depositTokenAmount);

		//burn member tokens
		IBasisAsset(address(memberToken)).burnFrom(msg.sender, _amount);
