...
contract StakingVote is Ownable, ReentrancyGuard {
    ...
    /// @param _stakeToken the stake token added to the pool
    /// @param _timeLock the time for locking tokens
    function add(
        uint256 _allocPoint,
        IERC20 _stakeToken,
        uint256 _timeLock,
    ...
    /// @param _to To block
    /// @param _poolMultiplier The pool multiplier parameter
    ...
    function getMultiplier(
        ...
        return _to.sub(_from).mul(_poolMultiplier);
    }
    ...
    function pendingVoteToken(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accVoteTokenPerShare = accVoteTokenPerShare.add(
                resourceReward.mul(1e12).div(lpSupply)
            );
        }
        return
            user.amount.mul(accVoteTokenPerShare).div(1e12).sub(
                user.rewardDebt
            );
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accVoteTokenPerShare = pool.accVoteTokenPerShare.add(
            voteTokenReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }
    ...
    function safeVoteTokenTransfer(address _to, uint256 _amount) internal {
        uint256 voteTokenBal = voteToken.balanceOf(address(this));
        if (_amount > voteTokenBal) {
            voteToken.safeTransfer(_to, voteTokenBal);
        } else {
