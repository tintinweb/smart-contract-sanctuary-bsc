...
contract SKRWhitelisted is Context {
    ...
    WhitelistRound[] public _skrWhitelistRounds;

    uint256 public _skrTimestamp;
    address public _skrPairAddress;
    ...
    /*
     * createSKRWhitelist - Call this after initial Token Generation Event (TGE)
     *
    ...
    function createSKRWhitelist(
        ...
        _skrPairAddress = pairAddress;

        if (durations.length > 0) {
            delete _skrWhitelistRounds;

            for (uint256 i = 0; i < durations.length; i++) {
                // _skrWhitelistRounds.push(
                    // WhitelistRound(durations[i], amountsMax[i])
                    WhitelistRound storage whitelistRound = _skrWhitelistRounds.push();
    ...
    /*
     * modifySKRWhitelistAddresses - Define what addresses are included/excluded from a whitelist round
     *
     * index - 0-based index of round to modify whitelist
     * duration - period in seconds from SKR event or previous whitelist round
     * amountMax - max amount (TOKEN decimals) for each whitelist round
     *
     */

    function modifySKRWhitelist(
        uint256 index,
        uint256 duration,
        uint256 amountMax,
        ...
        require(index < _skrWhitelistRounds.length, "Invalid index");
        require(amountMax > 0, "Invalid amountMax");

        if (duration != _skrWhitelistRounds[index].duration)
            _skrWhitelistRounds[index].duration = duration;

        if (amountMax != _skrWhitelistRounds[index].amountMax)
            _skrWhitelistRounds[index].amountMax = amountMax;

        for (uint256 i = 0; i < addresses.length; i++) {
            _skrWhitelistRounds[index].addresses[addresses[i]] = enabled;
        }
    }

    /*
     *  getSKRWhitelistRound
     *
     *  returns:
     *
    ...
    {
        if (_skrTimestamp > 0) {
            uint256 wlCloseTimestampLast = _skrTimestamp;

            for (uint256 i = 0; i < _skrWhitelistRounds.length; i++) {
                WhitelistRound storage wlRound = _skrWhitelistRounds[i];
    ...
    /*
     * _applySKRWhitelist - internal function to be called initially before any transfers
     *
     */

    function _applySKRWhitelist(
        address sender,
        address recipient,
        uint256 amount
    ) internal {
        if (_skrPairAddress == address(0) || _skrWhitelistRounds.length == 0)
            return;

        if (
            _skrTimestamp == 0 &&
            sender != _skrPairAddress &&
            recipient == _skrPairAddress &&
            amount > 0
        ) _skrTimestamp = block.timestamp;

        if (sender == _skrPairAddress && recipient != _skrPairAddress) {
            //buying

            (uint256 wlRoundNumber, , , , , ) = getSKRWhitelistRound();

            if (wlRoundNumber > 0) {
                WhitelistRound storage wlRound = _skrWhitelistRounds[
                    wlRoundNumber.sub(1)
                ];

                require(
                    wlRound.addresses[recipient],
                    "SKR - Buyer is not whitelisted"
                );
                ...
                require(
                    ...
                    "SKR - Amount exceeds whitelist maximum"
                );
...
contract Skippers is Context, IBEP20, Ownable, SKRWhitelisted {
    using SafeMath for uint256;
    ...
    function _transfer(
        ...
        // AntiBot feature
        _applySKRWhitelist(from, to, amount);
