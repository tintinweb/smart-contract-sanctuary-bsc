...
// File contracts/NftVaultSlashExtension.sol
...
contract NftVaultSlashExtension is ISlashCustomPlugin, Ownable {
    ...
    mapping(string => DepositInfo) private depositInfo;
    ...
    /**
     * @notice receive payment from Slash
     */
    function receivePayment(
        address receiveToken, 
        uint256 amount, 
        ...
        require(initialized, "this contract has not initialized");
        DepositInfo memory info = depositInfo[paymentId];
        require(amount > 0, "invalid amount");
        require(info.amount == amount, "amount does not match");
        require(receiveToken != address(0), "invalid token");
        require(info.token == receiveToken, "token does not match");
        ...
        IERC20(receiveToken).universalApprove(
            vault,
            amount
        );

        INftVault(vault).deposit(
            info.nftAddress,
            info.nftTokenId,
            info.token,
            info.amount
        ...
        emit ReceivePayment(tx.origin, info.nftAddress, info.nftTokenId, receiveToken, amount, paymentId, optional);
    }
    ...
    /**
     * @notice set payment ID, NFT information, deposit token and amount
     * payer should set these params before to make payment
     * @param _paymentId payment ID for Slash
     * @param _nftAddress key NFT contract address
     * @param _nftTokenId key NFT token ID
    ...
    function setPaymentId(
        ...
        DepositInfo memory nft;
        nft.nftAddress = _nftAddress;
        nft.nftTokenId = _nftTokenId;
        nft.token = _token;
        nft.amount = _amount;
        depositInfo[_paymentId] = nft;
    }
