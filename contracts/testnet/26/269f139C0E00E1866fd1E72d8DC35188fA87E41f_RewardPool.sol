...
contract RewardPool is Ownable, UserRegistry {
    ...
    // address private constant LLW_ADDRESS = address(0x269f139C0E00E1866fd1E72d8DC35188fA87E41f);
    // address private constant PAIR_ADDRESS = address(0x9F66E92217905B55fdbEe378E206DA3933c400b0);
    ...
    address private constant LLW_ADDRESS = address(0xAb1E81126060e08AB6805ae89812F01e2117F481);
    ...
    function getPriceLLW(uint256 amount) public view returns(uint256) {
        return getPrice(LLW_ADDRESS, USDT_ADDRESS, amount);
    }

    function getPriceUSDT(uint256 amount) public view returns(uint256) {
        return getPrice(USDT_ADDRESS, LLW_ADDRESS, amount);
    }
    ...
    function depositLLW(uint256 amount, address recommender) external {
        uint256 usdtAmount = getPriceLLW(amount) * 5;
        IERC20(LLW_ADDRESS).transferFrom(msg.sender, BURN_ADDRESS, amount);
    ...
    function depositUSDT(uint256 amount, address recommender) external {
        uint256 llwAmount = getPriceUSDT(amount) / 5;
        IERC20(USDT_ADDRESS).transferFrom(msg.sender, address(this), amount);
        IERC20(LLW_ADDRESS).transferFrom(msg.sender, BURN_ADDRESS, llwAmount);
    ...
    function withdraw() public {
        ...
        IERC20(USDT_ADDRESS).transfer(msg.sender, amount * 92 / 100);
        ...
        recommenderUser.inviteBalance += amount * 5 / 100;
    ...
    function exit() external {
        ...
        if (user.amount > 0) {
            IERC20(USDT_ADDRESS).transfer(msg.sender, user.amount);
            user.amount = 0;
            user.rewardMax = 0;
            user.rewardFree = 0;
            user.rewardBalance = 0;
    ...
    function _updateGroup(address account, uint256 amount, uint8 step) private {
        ...
        users[account].inviteBalance += amount;
...
contract LLW is IERC20, IERC20Metadata, Ownable {
    address private constant ROUTER_ADDRESS = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address private constant USDT_ADDRESS = 0xd2Cc0A0d84cE778c4907044f73604f36a45aA389;
    ...
    string private _name = "LLW Token";
    string private _symbol = "LLW";
    ...
    uint256[2] private _buyAmounts = [20 * 10**USDT_DECIMALS, 50 * 10**USDT_DECIMALS];
