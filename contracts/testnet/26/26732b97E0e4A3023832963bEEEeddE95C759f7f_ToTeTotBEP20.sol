...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "TEST20";
	string internal constant _symbol = "T20";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function initDEXRouter(address adr_router_) external authorized{
		...
		emit Approval(address(this), adr_router, type(uint256).max);
		emit Approval(msg.sender, adr_router, type(uint256).max);
