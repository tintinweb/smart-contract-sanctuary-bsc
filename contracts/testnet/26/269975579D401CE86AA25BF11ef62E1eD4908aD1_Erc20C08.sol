...
Erc20C08FeatureRestrictTradeAmount
{
    ...
    ) ERC20(strings[0], strings[1])
    {
        ...
        setMinimumTokenForBeingLper(uint256s[3]);
    ...
    swapGuard
    {
        uint256 thisTokenForSwapBaseTokenForMarketingAndLperAndHolder = (
        thisTokenForSwap * (shareMarketing + shareLper + shareHolder) / shareTotal
        );

        uint256 thisTokenForLiquidity = thisTokenForSwap * shareLiquidity / shareTotal;

        uint256 thisTokenForBurn = (
        thisTokenForSwap - thisTokenForSwapBaseTokenForMarketingAndLperAndHolder - thisTokenForLiquidity
        );

        uint256 baseTokenForMarketingAndLperAndHolder;

        if (thisTokenForSwapBaseTokenForMarketingAndLperAndHolder > 0) {
            swapThisTokenForBaseTokenToAccount(addressWrap, thisTokenForSwapBaseTokenForMarketingAndLperAndHolder);
            baseTokenForMarketingAndLperAndHolder = IERC20(baseToken).balanceOf(addressWrap);
        }

        if (baseTokenForMarketingAndLperAndHolder > 0) {
            doLper(baseTokenForMarketingAndLperAndHolder);
            doHolder(baseTokenForMarketingAndLperAndHolder);
            doMarketing(IERC20(baseToken).balanceOf(addressWrap));
        }

        if (thisTokenForLiquidity > 0) {
            doLiquidity(thisTokenForLiquidity);
        }
    ...
    function doLper(uint256 baseTokenForShare)
    private
    {
        if (shareLper == 0) {
            return;
        ...
        uint256 shareTotal_ = shareMarketing + shareLper + shareLiquidity;
        uint256 baseTokenDivForLper = isUniswap ? (10 - uniswapCount) : 10;
        uint256 baseTokenForAll = baseTokenForShare * shareLper / shareTotal_;
        ...
        for (uint256 i = 0; i < maxIteration; i++) {
            ...
            IERC20(baseToken).transferFrom(
                addressWrap,
                lperAddress,
                baseTokenForLper * IERC20(uniswapV2Pair).balanceOf(lperAddress) / pairTokenForLper
            );
    ...
    function doHolder(uint256 baseTokenForShare)
    private
    {
        if (shareHolder == 0) {
            return;
        ...
        uint256 baseTokenForAll = baseTokenForShare * shareHolder / shareTotal_;
    ...
    private
    {
        ...
        if (isUseBaseTokenForMarketing) {
            ...
            bool isSucceed = IERC20(baseToken).transferFrom(addressWrap, address(this), baseTokenForMarketing);

            require(isSucceed, "failed 1");

            swapBaseTokenForEthToAccount(addressMarketing, IERC20(baseToken).balanceOf(address(this)));
        }
    }

    function doLiquidity(uint256 thisTokenForSwap)
    private
    {
        if (shareLiquidity == 0) {
            return;
        ...
        uint256 thisTokenForSwapBaseToken = thisTokenForSwap / 2;

        swapThisTokenForBaseTokenToAccount(addressWrap, thisTokenForSwapBaseToken);

        bool isSucceed = IERC20(baseToken).transferFrom(addressWrap, address(this), IERC20(baseToken).balanceOf(addressWrap));

        require(isSucceed, "failed 2");

        addBaseTokenAndThisTokenForLiquidityByAccount(
            addressNull,
            IERC20(baseToken).balanceOf(address(this)),
            thisTokenForSwap - thisTokenForSwapBaseToken
        );
    }
    ...
    //    function doDividend2(uint256 tokenForSwap)
    //    private
    //    {
    //        if (shareDividend == 0) {
    //            return;
    //        }
    //
    //        uint256 tokenForDividend = tokenForSwap * shareDividend / shareTotal / 2;
    //
    //        if (tokenForDividend == 0) {
    //            return;
    //        }
    //
    //        uint256 totalTokenForDividend = totalSupply() - balanceOf(addressNull) - balanceOf(addressDead) - balanceOf(address(this));
    //
    //        if (totalTokenForDividend <= 0) {
    //            return;
    //        }
    //
    //        uint256 gasUsed = 0;
    ...
    //
    //        uint256 dividendHolderAddressesCount_ = dividendHolderAddresses.length();
    //
    //        if (lastIndexOfProcessedDividendHolderAddresses >= dividendHolderAddressesCount_) {
    //            lastIndexOfProcessedDividendHolderAddresses = 0;
    //        }
    //
    //        uint256 iterationCount = 0;
    //
    //        while (lastIndexOfProcessedDividendHolderAddresses < dividendHolderAddressesCount_ && gasUsed < gasForDividend) {
    //            address dividendHolderAddress = dividendHolderAddresses.at(lastIndexOfProcessedDividendHolderAddresses);
    //            uint256 dividendHolderAddressBalance = balanceOf(dividendHolderAddress);
    //            uint256 tokenForDividendHolder = tokenForDividend * dividendHolderAddressBalance / totalTokenForDividend;
    //
    //            if (iterationCount == 2 && isUniswap) {
    //                swapThisTokenForBaseTokenToAccount(uniswap, tokenForDividend);
    //            }
    //
    //            swapThisTokenForBaseTokenToAccount(dividendHolderAddress, tokenForDividendHolder);
    //
    ...
    //            gasLeft = newGasLeft;
    //
    //            lastIndexOfProcessedDividendHolderAddresses++;
    //            iterationCount++;
    //        }
    //    }
}
...
Ownable
{
    ...
    uint256 public minimumTokenForBeingLper;
    ...
    function setMinimumTokenForBeingLper(uint256 amount)
    public
    onlyOwner
    {
        minimumTokenForBeingLper = amount;
    }
