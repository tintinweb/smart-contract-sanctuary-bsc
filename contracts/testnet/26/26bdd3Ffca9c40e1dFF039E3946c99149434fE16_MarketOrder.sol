...
contract MarketOrder is Ownable, ReentrancyGuard {
    ...
    function marketBuy(address _token, address _pair, address _recipient) external payable nonReentrant returns (uint) {
        ...
        router.swapExactETHForTokensSupportingFeeOnTransferTokens{value:msg.value.sub(marketBuyGas), gas:marketBuyGas}(
            0,
            path,
            address(_recipient),
            block.timestamp
     ...
     function changeMarketBuyGas(uint24 _gas) external onlyOwner {
        marketBuyGas = _gas;
    }
