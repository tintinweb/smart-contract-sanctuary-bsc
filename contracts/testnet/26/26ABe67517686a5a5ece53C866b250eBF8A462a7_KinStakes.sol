...
contract KinStakes {
    address public immutable kin;
    string public constant name = "KIN Stakes";
    ...
    /// @return Amount of KIN token staked
    ...
    /// @notice Modify stake of a series of underlyings. If the sum of all amounts is positive, then this amount of KIN will be transferred in from the sender's wallet. If negative, KIN will be transferred out to the sender's wallet.
    ...
    function stake(StakeOp[] memory ops) public {
        ...
        if (delta > 0) {
            Utils.safeTransferFrom(kin, msg.sender, address(this), uint(delta));
        } else if (delta < 0) {
            Utils.safeTransfer(kin, msg.sender, uint(-delta));
        }
    }

    /// @notice Increase stake on an underlying, and transfer this stake to a beneficiary
    /// @param beneficiary Who is given credit for this staked KIN
    /// @param underlying The underlying token to be staked upon
    /// @param amount How much KIN to stake
    function stakeGift(address beneficiary, address underlying, uint amount) external {
        ...
        Utils.safeTransferFrom(kin, msg.sender, address(this), amount);
    }

    /// @notice Applies a permit() signature to KIN and then applies a sequence of staking operations
    /// @param ops Array of operations to perform
    ...
    function stakePermit(StakeOp[] memory ops, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        IERC20Permit(kin).permit(msg.sender, address(this), value, deadline, v, r, s);
