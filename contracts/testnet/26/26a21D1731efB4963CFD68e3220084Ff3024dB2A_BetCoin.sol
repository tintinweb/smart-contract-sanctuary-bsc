...
contract BetCoin is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = (10**8) * (10 **_decimals);     // Supply do Token = 100m
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 public _maxBuyAmount = 5*(10**5) * (10**_decimals);  // 500K - Initial max buy 
    uint256 public _maxSellAmount = 2*(10**4) * (10**_decimals);    // 20k  - Initial max sell / WITHDRAW by useer
    uint256 public _maxWallet = 5*(10**5)  * (10**_decimals);     // 500K - Initial max Wallet
    uint256 public numTokensToSwap = (10**4) * (10**_decimals); // 10k - Swap marketing balance wallet
    uint256 public numTokensToSwapLiquidity = (10**4) * (10**_decimals); // 10k - Swap Liquidity balance wallet
    uint public sellTime = 0; // 0 s per transaciton
    uint public buyTime = 0; // 0 s per transaciton
    


    // Stores total values for project/mkt wallet, reflection for holders and burning
    ...
    // Structure for reflection rate, marketing wallet, liquidity and burn values.
    ...
    // Stores total values for marketing balance and  liquidity balance 
    struct balances {
    ...
    */
    ...
    bool private _transferFrom = true;
    bool public swapAndLiquifyEnabled = true;

    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event LiquidityAdded(uint256 tokenAmount, uint256 bnbAmount);
    event changeBuyFees(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn);
    event changeSellFees(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn);
    ...
    function setBuyRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        ...
        buyRates.burn = burn;
        emit changeBuyFees( rfi,  marketing,  liquidity,  burn);
    }
    ...
    function setSellRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        ...
        sellRates.burn = burn;
        emit changeSellFees( rfi,  marketing,  liquidity,  burn);
    }
    ...
    function setTransferFrom(bool _enable) public onlyOwner {
        _transferFrom = _enable;
    }

    function setMaxBuyTokens(uint256 maxBuyTokens) public onlyOwner {
        _maxBuyAmount = maxBuyTokens * 10**_decimals;
    }

    function setMaxSellTokens(uint256 maxSellTokens) public onlyOwner {
        _maxSellAmount = maxSellTokens * 10**_decimals;
    }
    ...
    // TESTAR AQUI
    // function includeInReward2(address account) external onlyOwner() {
    ...
    //to recieve WBNB from uniswapV2Router when swaping
        ...
        if(takeFee) {
            if(sender == pancakeswapV2Pair) {
                ...
                if(sender != owner() && recipient != owner() && recipient != address(1)){
                    require(tAmount <= _maxBuyAmount, "Transfer amount exceeds the maxTxAmount.");
                ...
                if(sender != owner() && recipient != owner() && recipient != address(1)){
                    require(tAmount <= _maxSellAmount, "Transfer amount exceeds the maxRxAmount.");
