...
contract TheNetworkSociety is Context, Ownable {
    ...
    function withdraw() public isInitialized {
        ...
        if(payoutPostTax > 0 && user.totalDepositPayouts < maxPayout){ // avoid reverts
            //yieldPercentage will be deducted 0.5% every withdraw, starts at 2.1% else, if daily yield is less than or equal to 2% yield it goes back to 1.5%
            if(user.yieldPercentage >= baseYieldPercent && user.yieldPercentage <= 200) { //between 1.5% to 2.0%
                user.yieldPercentage = baseYieldPercent;        
            }
            else if(user.yieldPercentage > 200) { // 0.5% decrease if greater than 2.0%
