...
contract RuleB5M10 is IBEP20 {
    ...
    function addDirectorWallet(address account) public onlyOwner returns (bool) {
        ...
        _wallets[account].next_release_time = block.timestamp + ( Variables._director_lock_days * Variables._time_multiplier );
    ...
    function addInvestorWallet(address account) public onlyOwner returns (bool) {
        ...
        _wallets[account].locked_on = block.timestamp;
        _wallets[account].next_release_time = block.timestamp + ( Variables._investor_lock_days * Variables._time_multiplier );
        _wallets[account].current_release_iteration = 0;
        _wallets[account].current_release_amount = 0;
        _total_investors += 1;
        ...
        for (uint256 i = 0; i < _total_directors; i++) {
            if ( _wallets[_directors[i]].current_release_iteration < Variables._director_total_release_iteration ) {
                if ( _wallets[_directors[i]].next_release_time <= block.timestamp ) {
                    // get multiplier
                    uint256 multiplier = (((block.timestamp - _wallets[_directors[i]].next_release_time) / Variables._time_multiplier) / Variables._director_release_every_days_after_locking) + 1;
                    ...
                    _wallets[_directors[i]].next_release_time += ( Variables._director_release_every_days_after_locking * multiplier * Variables._time_multiplier );
        ...
        for (uint256 i = 0; i < _total_investors; i++) {
            if ( _wallets[_investors[i]].current_release_iteration < Variables._investor_total_release_iteration ) {
                if ( _wallets[_investors[i]].next_release_time <= block.timestamp ) {
                    uint256 multiplier = (((block.timestamp - _wallets[_investors[i]].next_release_time) / Variables._time_multiplier) / Variables._investor_release_every_days_after_locking) + 1;
                    ...
                    _wallets[_investors[i]].next_release_time += ( Variables._investor_release_every_days_after_locking * multiplier * Variables._time_multiplier );
    ...
    function _transfer(
        ...
        if ( _wallets[sender].is_director || _wallets[sender].is_investor ) {
            ...
            if ( _wallets[sender].current_release_iteration < Variables._director_total_release_iteration ) {
                require(
