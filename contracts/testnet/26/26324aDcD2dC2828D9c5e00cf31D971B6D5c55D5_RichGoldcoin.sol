...
contract RichGoldcoin is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        _tokenSwap(from,to,amount,takeswapFee);
    ...
    function _tokenSwap(address sender, address recipient, uint256 amount,bool takeswapFee) private {
        if(!takeswapFee)
            removeAllFee();
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount);
