...
contract Stakeable is Ownable, ReentrancyGuard {
  ...
  function harvest() external nonReentrant {
    ...
    require(rewards <= address(this).balance, "Insufficient reward funds");
    stakers[msg.sender].unclaimedRewards = 0;
    stakers[msg.sender].timeOfLastUpdate = block.timestamp;
  ...
  function withdraw(uint256 _amount) external nonReentrant {
    ...
    require(_amount <= address(this).balance, "Insufficient pool funds");
    stakers[msg.sender].timeOfLastUpdate = block.timestamp;
  ...
  function withdrawTaxes() external onlyOwner {
    require(address(this).balance - weiStaked > 0, "Taxes vault empty");
    token.transfer(msg.sender, address(this).balance - weiStaked);
  }
}
