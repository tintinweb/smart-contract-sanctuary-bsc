...
contract SynthrIssuer is Owned, MixinSystemSettings, IIssuer {
    ...
    mapping(address => uint256) public coveredDebtShare;
    ...
    function resolverAddressesRequired() public view override returns (bytes32[] memory addresses) {
        ...
        newAddresses[6] = CONTRACT_LIQUIDATOR;
        // newAddresses[7] = CONTRACT_DEBTCACHE;
        newAddresses[7] = CONTRACT_SYNTHREDEEMER;
        newAddresses[8] = CONTRACT_SYSTEMSTATUS;
        newAddresses[9] = CONTRACT_ISSUED_SYNTH_AGGREGATOR;
        newAddresses[10] = CONTRACT_LIQUIDATOR_REWARDS_MANAGER;
        newAddresses[11] = CONTRACT_REWARDESCROW_V2_MANAGER;
        newAddresses[12] = CONTRACT_SYNTHR_BRIDGE;
    ...
    function rewardEscrowV2(bytes32 key) internal view returns (IRewardEscrowV2) {
        return IRewardEscrowV2(rewardEscrowV2Manager().getRewardEscrow(key));
    }
    ...
    {
        (alreadyIssued, totalSystemDebt, anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(coveredDebtShare[_issuer], sUSD);
    ...
    function _maxIssuableSynths(address _issuer) internal view returns (uint256) {
        ...
        uint256 destinationValue = _collateralOnChain(_issuer);
    ...
    function _collateral(address account) internal view returns (uint256) {
        ...
        if (address(rewardEscrowV2Manager()) != address(0)) {
            balance = balance.add(rewardEscrowV2Manager().totalBalanceOf(account));
        }

        if (address(liquidatorRewardsManager()) != address(0)) {
            balance = balance.add(liquidatorRewardsManager().totalLiquidateEarned(account));
        }

        return balance;
    }

    function _collateralOnChain(address account) internal view returns (uint256) {
        uint256 balance = synthetix().chainBalanceOf(account);

        if (address(rewardEscrowV2Manager()) != address(0)) {
            balance = balance.add(rewardEscrowV2Manager().totalBalanceOfOnChain(account));
        }

        if (address(liquidatorRewardsManager()) != address(0)) {
            balance = balance.add(liquidatorRewardsManager().totalLiquidateEarnedOnChain(account));
        }
    ...
    function debtBalanceOf(address _issuer, bytes32 currencyKey) external view returns (uint256 debtBalance) {
        // What was their initial debt ownership?
        uint256 debtShareBalance = coveredDebtShare[_issuer];
    ...
    function issuableSynthExpected(
        ...
        (uint256 alreadyIssued, , ) = _debtBalanceOfAndTotalDebt(coveredDebtShare[_issuer], sUSD);
        uint256 currentCollateral = _collateralOnChain(_issuer);
    ...
    function issueSynthsOnBehalf(
        ...
        (synthAmount, debtShare) = _issueSynths(issueForAddress, destChainId, amount, false);
    }
    ...
    function issueMaxSynthsOnBehalf(
        ...
        _requireCanIssueOnBehalf(issueForAddress, from);
        (synthAmount, debtShare) = _issueSynths(issueForAddress, 0, destChainId, true);
    }
    ...
    function burnSynthsOnBehalf(
        ...
        (synthAmount, debtShare) = _voluntaryBurnSynths(burnForAddress, synthKey, amount, false);
    }
    ...
    function burnSynthsToTargetOnBehalf(
        ...
        _requireCanBurnOnBehalf(burnForAddress, from);
        (synthAmount, debtShare) = _voluntaryBurnSynths(burnForAddress, synthKey, 0, true);
    }
    ...
    /// @return totalRedeemed the total amount of collateral (SNX) to redeem
        ...
        )
    {
        ...
        uint256 penalty = isSelfLiquidation ? getSelfLiquidationPenalty() : getSnxLiquidationPenalty();
        ...
        totalRedeemed = amountToLiquidate.multiplyDecimal(SafeDecimalMath.unit().add(penalty));
        ...
        uint256 transferableBalance = synthetix().chainBalanceOfPerKey(account, _collateralKey);
    ...
    function destIssue(
        ...
        // ISynthetixDebtShare sds = synthetixDebtShare();
        // sds.mintShare(_account, _debtShare);
        synths[_synthKey].issue(_account, _synthAmount);
        liquidatorRewardsManager().updateEntry(_account);
    ...
    function fixCoveredDebtShare(address _account, uint256 _amountToFix) external onlySynthetix {
        coveredDebtShare[_account] = _amountToFix;
    }
    ...
    function _burnSynths(
        ...
        (, debtShareBurnt) = _removeCoveredDebtShare(debtAccount, amountBurntInUSD, false);
    ...
    function _voluntaryBurnSynths(
        ...
        (uint256 existingDebt, , bool anyRateIsInvalid) = _debtBalanceOfAndTotalDebt(
            synthetixDebtShare().balanceOfCrossChain(from),
            sUSD
        );
        // max issuable synth in USD
        uint256 maxIssuableSynthsForAccount = _maxIssuableSynthsCrossChain(from);
        _requireRatesNotInvalid(anyRateIsInvalid);

        // (uint256 existingDebtOnChain, , ) = _debtBalanceOfAndTotalDebt(synthetixDebtShare().balanceOf(from), sUSD);
        // require(existingDebtOnChain > 0, "No debt to forgive");
    ...
    function _addToDebtRegister(address from, uint256 amount) internal returns (uint256) {
        // important: this has to happen before any updates to user's debt shares
        liquidatorRewardsManager().updateEntry(from);
    ...
    function _removeFromDebtRegister(
        ...
        uint256 sharesToRemove = _sharesForDebt(debtToRemove);
        // sds.burnShare(from, sharesToRemove < currentDebtShare ? sharesToRemove : currentDebtShare);
        // return sharesToRemove < currentDebtShare ? sharesToRemove : currentDebtShare;
    ...
    function _addCoveredDebtShare(address from, uint256 amount) internal returns (uint256) {
        uint256 debtShares = _sharesForDebt(amount);
        if (debtShares == 0) {
            coveredDebtShare[from] = coveredDebtShare[from].add(amount);
            return amount;
        } else {
            coveredDebtShare[from] = coveredDebtShare[from].add(debtShares);
            return debtShares;
        }
    }

    function _removeCoveredDebtShare(address from, uint256 amount, bool isMax) internal returns (uint256, uint256) {
        uint256 currentDebtShare = coveredDebtShare[from];
        uint256 totalSharesToRemove = _sharesForDebt(amount);
        if (isMax) {
            if (currentDebtShare > totalSharesToRemove) {
                coveredDebtShare[from] = currentDebtShare.sub(totalSharesToRemove);
            } else {
                coveredDebtShare[from] = 0;
            }
            ...
            if (currentDebtShare > sharesToRemove) {
                coveredDebtShare[from] = currentDebtShare.sub(sharesToRemove);
            } else {
                coveredDebtShare[from] = 0;
            }
    ...
    function _verifyCircuitBreaker() internal returns (bool) {
        ...
        lastDebtRatio = uint256(rawRatio);
