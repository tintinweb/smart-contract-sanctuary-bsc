...
contract LINCOLNSALE_LAUNCHPAD is Ownable, ReentrancyGuard{
    ...
    function endSales(uint _id) public nonReentrant{
        ...
        if(block.timestamp > endTime && tStatus == false){
         ...
         if(lpType==1){
             ...
             addToLiquidity(address(_tokenAddress),tBalance,lpShare,adminAddress);
         }
    ...
    function approve(address token, uint amount) public{
        IBEP20(token).approve(address(this), amount);
    }

    function addToLiquidity(address poolAddr, uint256 tokenAmount, uint256 ethAmount, address addr) public {
