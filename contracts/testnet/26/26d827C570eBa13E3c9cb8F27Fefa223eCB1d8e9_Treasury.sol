...
ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        require(_garage != address(0), "initialize: Invalid address");
        ...
        path.push(IPancakeRouter(router).WETH());
        path.push(garage);
        totalBNB = 1 ether;
    }
    ...
    function validatePayout() external {
        uint256 balance = IERC20Upgradeable(garage).balanceOf(address(this));
        uint256[] memory getGarageAmountOneBNB = IPancakeRouter(router).getAmountsOut(totalBNB, path);
        uint256 totalAmount = getGarageAmountOneBNB[1].mul(100);
        if (balance >= totalAmount && IGovernance(dao).nextInvesteeFund() < IGovernance(dao).nextInvestee()) {
            fundInvestee(totalAmount, getGarageAmountOneBNB[1].mul(20));
        }
    }
    ...
    function fundInvestee(uint256 totalAmount, uint burnAmount) internal nonReentrant {
        ...
        IERC20Upgradeable(garage).transfer(DEAD_ADDRESS, burnAmount);
        IERC20Upgradeable(garage).transfer(investee, totalAmount.sub(burnAmount));
    }
