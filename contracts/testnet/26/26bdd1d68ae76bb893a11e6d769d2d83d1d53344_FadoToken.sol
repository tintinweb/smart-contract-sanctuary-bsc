...
/** 
 *  SourceUnit: c:\Projects\fado\smart-contracts\contracts\token\FadoToken.sol
*/
...
contract FadoToken is AccessControlEnumerable, ERC20Burnable, ERC20Pausable, ERC20Capped {
    ...
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "FadoToken: must have admin role to call");
        _;
    }
    ...
    function mint(address to, uint256 amount) public virtual {
        require(hasRole(MINTER_ROLE, _msgSender()), "FadoToken: must have minter role to mint");

        _mint(to, amount);
    }

    function pause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "FadoToken: must have pauser role to pause");

        _pause();
    }

    function unpause() public virtual {
        require(hasRole(PAUSER_ROLE, _msgSender()), "FadoToken: must have pauser role to unpause");
    ...
    function setBPContract(address addr) public onlyAdmin {
        require(address(bpContract) == address(0), "FadoToken: can only be initialized once");
    ...
    function setBPDisableForever() public onlyAdmin {
        require(!bpDisabledForever, "FadoToken: bot prevention disabled");
