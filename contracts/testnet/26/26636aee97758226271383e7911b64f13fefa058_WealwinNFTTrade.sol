...
contract WealwinNFTTrade is Initializable, OwnableUpgradeable {
    ...
    function updateAdminAddress(address _adminAddress) public onlyOwner {
        adminAddress = _adminAddress;
    }

    function updateServiceType(uint256 _type) public onlyOwner {
        serviceType = _type;
    }

    function updateAdminFees(uint256 _adminFee) public onlyOwner {
        adminFee = _adminFee * 1e18;
    }

    function addTokenType(string memory _type, address _tokenAddress)
        public
        onlyOwner
    {
        tokentype[_type] = _tokenAddress;
    }
    ...
    function acceptBId(
        ...
        address bidder = bidaddr;
        IERC20Upgradeable t = IERC20Upgradeable(tokentype[tokenAdd]);
        ...
        t.transferFrom(bidder,msg.sender,netamount);
        if(roy > 0 && royaltyAddr!=0x0000000000000000000000000000000000000000){
            t.transferFrom(bidder,royaltyAddr,roy);
        }
        t.transferFrom(bidder,adminAddress, _adminfee);

        salestruct.conAddr = _conAddr;
        salestruct.tokenId = tokenId;
        salestruct.buyer = bidaddr;
        salestruct.nftType = nftType;
        salestruct.nooftoken = nooftoken;
        salestruct.from = msg.sender;
        transferNft(
            salestruct.conAddr,
            salestruct.tokenId,
            salestruct.buyer,
    ...
    function saleToken(
        ...
        for (uint256 i = 0; i < from.length; i++) {
            bytes32 message = prefixed(
                keccak256(abi.encodePacked(from[i], nonce[i]))
            );
            require(
                recoverSigner(message, signature[i]) == from[i],
                "wrong signature"
            );
            ...
            require(netamount + roy + _adminfee <= amount[i], "Invalid calc");
            payable(royaltyAddr[i]).transfer(roy);
            payable(adminAddress).transfer(_adminfee);
            payable(from[i]).transfer(netamount);

            salestruct.conAddr = _conAddr[i];
            salestruct.tokenId = tokenId[i];
            salestruct.buyer = msg.sender;
        ...
        )
    {
        ...
        uint256 netamount;
        if ( _serviceType == 1 ){
            netamount = amount.sub(fee).sub(roy);
        }else if ( _serviceType == 2 ){
            netamount = amount.add(fee).sub(roy);
        }else  if ( _serviceType == 3 ){
            netamount = amount.add(fee).add(roy);
        }
