...
contract VOOLAToken is IERC20, Ownable, Util {
    ...
    */
    ...
    function _transfer(
        ...
        if((sender == swapRouter || recipient == swapRouter) && !_isExcludedFromFee[recipient] && !_isExcludedFromFee[sender]) {
            ...
            // uint price = queryTokenToUsdtPrice(); 
    ...
    */
    ...
    /*
     * @dev  查询token相对usdt价值 | 所有人调用 | 获取1个voola等值的usdt数量
     */
    function queryTokenToUsdtPrice() public view returns (uint256){
    ...
    /*
     * @dev  查询usdt相对token价值 | 所有人调用 | 获取1个usdt等值的token数量
     */
    function queryUsdtToTokenPrice() public view returns (uint256){
        uint112 usdtSum;//LP池中,usdt总和
        uint112 voolaSum;//LP池中,voola总和
        uint32 lastTime;//最后一次交易时间
        (usdtSum,voolaSum,lastTime) = PancakePair(swapRouter).getReserves(); 

        uint256 voolaToUsdtPrice = Util.mathDivisionToFloat(voolaSum,usdtSum,18); 
        return voolaToUsdtPrice;
    }
