...
contract DarkDAO is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function withdrawNFT(uint256[] memory _tokenIds) external nonReentrant {
        ...
        uint256 _amountWantWithdraw = 0;
        (, uint256 _amountAvailable) = this.getLimitedAmount(msg.sender);
        for (uint256 i = 0 ; i < _len; i++){
            ...
            uint256 _level = IDarkNFT(darkNFT).getTokenLevel(_tokenId);
            _amountWantWithdraw  = _amountWantWithdraw.add(limitedAmount[_level - 1]);
        }
        require(_amountAvailable > _amountWantWithdraw, 'Can not withdraw NFT, need to with your DCF first');
    }
    ...
    function getLimitedAmount(address _user) public view returns (uint256 _amountLimited, uint256 _amountAvailable){
        ...
        for (uint256 i = 0; i < _len; ++i) {
            ...
            uint256 _level = IDarkNFT(darkNFT).getTokenLevel(_tokenId);
            _amountLimited  = _amountLimited.add(limitedAmount[_level - 1]);
        }
        _amountAvailable =  _amountLimited.sub(_balances[_user]);
    }

    function getAmountLevel() public view returns (uint256[5] memory) {
