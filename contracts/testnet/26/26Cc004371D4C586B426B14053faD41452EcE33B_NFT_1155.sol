...
contract NFT_1155 is ERC1155, ERC1155URIStorage, ERC1155Supply, Ownable {
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            bool isAmountEqBalance = amounts[i] == ERC1155.balanceOf( from, ids[i] );

            // Minting case
            if ( from == address(0) ) {

                // Add ID into _active and _owned
                _active.push( ids[i] );
            ...
            if ( to == address(0) ){
                ...
                // Remove ID from _active and _owned
                if ( amounts[i] == ERC1155Supply.totalSupply( ids[i] ) ) _active = pop( _active, ids[i] );
                if ( isAmountEqBalance ) _owned[ from ] = pop( _owned[ from ], ids[i] );
