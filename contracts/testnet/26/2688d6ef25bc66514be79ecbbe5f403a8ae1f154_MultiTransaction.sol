...
contract RecoToken is BEP20('RecoSwap Token', 'RECO') {
...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    RecoToken public reco;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public recoPerBlock;
    // Bonus muliplier for early reco makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _recoPerBlock,
        uint256 _startBlock
    ) public {
        reco = _reco;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        recoPerBlock = _recoPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 recoReward = multiplier.mul(recoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(recoReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 recoReward = multiplier.mul(recoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        reco.mint(devaddr, recoReward.div(10));
        reco.mint(address(this), recoReward);
        pool.accEggPerShare = pool.accEggPerShare.add(recoReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe reco transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 recoBal = reco.balanceOf(address(this));
        if (_amount > recoBal) {
            reco.transfer(_to, recoBal);
        } else {
            reco.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _recoPerBlock) public onlyOwner {
        massUpdatePools();
        recoPerBlock = _recoPerBlock;
    }
}
