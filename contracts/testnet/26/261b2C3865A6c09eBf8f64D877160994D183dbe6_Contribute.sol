...
interface IERC20 {
  ...
  function burnt(address account,uint256 amount) external;
...
contract Contribute is Context,Auth{
  ...
  mapping (uint256 => bool) public topshared_exit;
  ...
  monthy_split = 60*60*24; // half hour
  ...
  //contribute state
  externaladdress[1] = 0x02221bC3cd8504a72fab9cC22997367792ed874C; //dev fee
  externaladdress[2] = 0x3A8F93b86464d16643544C2642a55601dc786Fb6; //marketing
  externaladdress[3] = 0x30FaEdFE03850C7cf248B4d598eC1237aB556196; //caller
  externaladdress[4] = 0x9f15E9F4a94fe72A092F104B05448F3f3C694024; //staking
  externaladdress[5] = 0x3E24d024C002f2948d4e16dA696e1ecb68e8BcBc; //top shared
  externaladdress[6] = 0x3C5d2c6bd23608867D3313D70b1011c927A0143b; //dividend
  ...
  function getAmountTokenOut(uint256 _usdin) external view returns (uint256) {
    ...
    uint256 calculateprice = getprice(calculateusd);
  ...
  function directBuy(uint256 amountusd) external noReentrant() returns (bool) {
    ...
    uint256 recaiveamount = amountusd.div(boughtoutprice);
    uint256 decimals = b.decimals();
    recaiveamount = recaiveamount.mul(10**decimals);
    a.transferFrom(msg.sender,address(this),amountusd);
    b.faucet(msg.sender,recaiveamount);
    //create tx
    txorder = txorder.add(1);
    txamountin[txorder] = amountusd;
    txamountout[txorder] = recaiveamount;
  ...
  function directSell(uint256 amounttoken) external noReentrant() returns (bool) {
    ...
    IERC20 b = IERC20(token);
    uint256 decimals = b.decimals();
    uint256 usdtamount = amounttoken.div(10**decimals).mul(getcurrentprice());
    if(!isAdmin[msg.sender]){
        usdtamount = usdtamount.mul(700).div(1000);
    ...
    b.burnt(msg.sender,amounttoken);
  ...
  function contribution(uint256 packageid,address referto) external noReentrant() returns (bool) {
    ...
    uint256 contractusd = a.balanceOf(address(this));
    uint256 boughtoutprice = getprice(contractusd);
    uint256 recaiveamount = investamount.div(boughtoutprice);
    recaiveamount = recaiveamount.mul(10**18);
    b.faucet(msg.sender,recaiveamount);
  ...
  function claimtopshared(uint256 month) external noReentrant() returns (bool) {
    require(topshared_exit[month]==false);
    require(block.timestamp.sub(monthy_split)>month.mul(monthy_split).add(monthy_start));
    require(topshared_address[month]==msg.sender);
    topshared_exit[month] = true;
    IERC20 a = IERC20(USDT);
    uint256 claimamount = wallet_topshared[month];
    a.transferFrom(externaladdress[5],msg.sender,claimamount);
    hash_earned[msg.sender][6] = hash_earned[msg.sender][6].add(claimamount);
    return true;
  }
  ...
  function dividendusd() internal {
    ...
    uint256 dividendamount = dividend.div(participants);
    address triggeradr;
