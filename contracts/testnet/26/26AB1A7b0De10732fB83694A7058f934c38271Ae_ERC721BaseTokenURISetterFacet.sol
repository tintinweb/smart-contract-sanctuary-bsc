...
import "../interfaces/IERC721BaseTokenURISetterFacet.sol";

contract ERC721BaseTokenURISetterFacet is IERC721BaseTokenURISetterFacet {
    function initialize(bytes calldata data_) external payable override {
        string memory baseTokenUri = abi.decode(data_, (string));
        LibERC721BaseTokenURI.setBaseTokenTokenURI(baseTokenUri);
    ...
    function cleanUp() external payable override {
        LibERC721BaseTokenURI.setBaseTokenTokenURI("");
    }
...
interface IERC721BaseTokenURISetterFacet is IInitializableFacet {
    function encode(string calldata baseTokenURI_)
        external
        pure
        returns (bytes memory);
