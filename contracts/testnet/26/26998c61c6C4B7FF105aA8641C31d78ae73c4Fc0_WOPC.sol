...
contract WOPC is BEP20 {
    ...
    // Swap and Liquify Disabled
    // In swap and liquify
    // bool private _inSwapAndLiquify;

    // Max Holding Rate Disabled
    ...
    // WOPC dev Fee address
    address public devAddress;

    // Swap and Liquify Disabled
    // Min amount to liquify. (default 100 WOPC Token)
    ...
    //                 require(amount <= maxHolding() - balanceOf(recipient) && balanceOf(recipient) <= maxHolding(), "WOPC::antiWhale: Transfer amount would result in a balance bigger than the maxHoldingRate");
    //             }
    //         }
    //     }
    ...
    /**z
     * @notice Constructs the WOPC token contract.
     */
    constructor(address _devAddress) public BEP20("World Of Pump Coin", "WOPC") {
    ...
    function mint(address _to, uint256 _amount) public onlyOwner {
        require(wopcSupply().add(_amount) <= cap, "WOPC: cap exceeded");
        _mint(_to, _amount);
        // _moveDelegates(address(0), _delegates[_to], _amount);
    }

    /// @dev overrides transfer function to meet tokenomics of WOPC
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            // require(taxAmount == devFeeAmount.add(liquidityAmount), "WOPC::transfer: DevFeeAmount or LiquidityAmount value invalid");

            // default 97% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount.add(taxAmount), "WOPC::transfer: Tax value invalid");

            // super._transfer 은 bep20에서 오는걸 한번더 사용!
            // Distributing WOPCs (DevAddress, Recipient)
            super._transfer(sender, devAddress, taxAmount);
            // super._transfer(sender, address(this), liquidityAmount);
            super._transfer(sender, recipient, sendAmount);        
        }
    }

    /// @dev Swap and liquify Disabled
    ...
    //     // generate the WOPC pair path of token -> weth
    ...
    //     require(newPCSRouter != address(0), "WOPC::transferPCSRouter: new PCSRouter is the zero address");
    ...
    function updateDevAddress(address _devAddress) public onlyOperator {
        require(_devAddress != address(0), "WOPC: dev address is zero");
    ...
    //     require(_maxHoldingRate >= 100, "WOPC::updateMaxHoldingRate: Max holding rate must not be below the minimum rate.");
    ...
    function wopcSupply() public view returns (uint256) {
    ...
    //     require(lpToken == address(0), "WOPC: LP Token Transfer can be only be set once");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "WOPC::transferOperator: new operator is the zero address");
    ...
    /**
     * @dev Remove to exclude from fee.
     * Can only be called by the current operator.
     */
    function setRemoveExcludeFromFee(address _account) public onlyOperator {
