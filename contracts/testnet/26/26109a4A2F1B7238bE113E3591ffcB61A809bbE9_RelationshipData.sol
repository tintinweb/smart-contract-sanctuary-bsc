...
contract RelationshipData is SafeMath, ModuleBase {
    ...
    // SystemSetting SystemSetting(moduleMgr.getSystemSetting());
    // SystemAuth SystemAuth(moduleMgr.getSystemAuth());
    ...
    function addHistory(address parent, address child, uint8 rType) external onlyCaller {
        relHistoryCount ++;
    ...
    function _getUpstreamParent(address addr, uint256 layer)
        ...
        returns (bool res, address parent)
    {
        (, , uint256 _sharedLayer) = SystemSetting(moduleMgr.getSystemSetting()).getSharedSetting(0);
