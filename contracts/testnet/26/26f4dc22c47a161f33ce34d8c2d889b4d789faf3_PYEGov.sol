...
contract PYEGov{
    ...
    function getWeight(uint256 _prop) internal returns(uint256){
        ...
        if(snap){
            ...
            wt = IGov(tokenaddress).balanceOfAt(msg.sender, IGov(tokenaddress).getCurrentSnapshotId());
    ...
    function delWeight(address del)public{
        ...
        // uint256 snapper = IGov(tokenaddress).getCurrentSnapshotId();

        // uint256 wt = IGov(tokenaddress).balanceOfAt(msg.sender, snapper);

        delegated[msg.sender] += getWeight(currprop);
    ...
    function combineWeight() internal {
        ...
        for(i = 0; i < weight[msg.sender].delegators.length ; i++){
            ...
            weight[msg.sender].votes += delegated[del];
    ...
    function removemydelegators() internal {
        ...
        for(i = 0; i < weight[msg.sender].delegators.length ; i++){
            ...
            weight[del].votes = delegated[del];
