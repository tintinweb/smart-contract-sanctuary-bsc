...
contract CatheonStaking is CatheonAccessControlled {
    ...
    function stake(
        ...
        //mint xCatheon
        _amount = _amount.mul(xCatheon.balanceOf(address(this))).div(Catheon.balanceOf(address(this))) ;
    ...
    function unstake(
        ...
        require(_amount <= info.deposit, "Insufficient xCatheon!");

        amount_ = _amount;               
        
        if (block.timestamp - info.mint_at < warmupPeriod) { //calculate fee of 20%
            amount_ = amount_.mul(Catheon.balanceOf(address(this))).div(xCatheon.balanceOf(address(this)))*8/10;
        }else{
            amount_ = amount_.mul(Catheon.balanceOf(address(this))).div(xCatheon.balanceOf(address(this)));
        }
