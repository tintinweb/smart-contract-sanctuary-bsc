    ...
    RandomGenerator
{
    ...
    event BuyBoxMuti(address indexed user, uint indexed amount);
    event OpenBox(
    ...
    function initialize() external initializer {
        ...
        MpoNft = 0xF14C8eCdB0597bF41265B618bbC353Ba1bEd9B8c;
        MpoBox = 0xBE208a6f138Db4E76298700Dff3C8fb52697eeb6;
        MpoToken = 0xD7C3BEa505347a2E7A6F6721cD8a1893C679b864;
        WALLET = msg.sender;
        ...
        cardIds = [uint(10001), uint(10002), uint(10003)];
        amounts = [uint(1), uint(10), uint(200)];
        boxInfo.category = cardIds.length;
        boxInfo.boxCardId = 10000;
        uint total;
        for (uint i = 0; i < cardIds.length; i++) {
            boxInfo.bag[i] = BoxBag({cardId: cardIds[i], amount: amounts[i]});
            total += amounts[i];
        }
        boxInfo.maxAmount = total;
        boxInfo.currentAmount = 0;
    ...
    function setTechAddr(address techAddr_) public {
        techAddr = techAddr_;
    }
    ...
    function setBoxPrice(uint boxPrice_) public onlyOwner {
        boxPrice = boxPrice_;
    }

    function setCostUsdtProportion(uint costUsdtProportion_) public onlyOwner {
        require(costUsdtProportion_ <= 100, "out of 100");
        costUsdtProportion = costUsdtProportion_;
    }

    function setReset(uint maxTimes_, uint resetPrice_) public onlyOwner {
    ...
    function setBoxInfo(uint[3] calldata cardIds_, uint[3] calldata amounts_)
        public
    {
    ...
    function setBuyBoxCostToken(address token_) public onlyOwner {
    ...
    function buyBoxMuti(uint amount_) external {
        for (uint i; i < amount_; i++) {
            buyBox();
        }
        emit BuyBoxMuti(msg.sender, amount_);
    }
    ...
    function resetNft(uint tokenId_) public returns (uint) {
        require(isOpen && block.timestamp >= startTime, "not open");
        MpoNfts(MpoBox).burn(tokenId_);

        uint level = _randomResetLevel();
        uint cardId = restInfo.resetPool[level].cardId;

        restInfo.restTimes += 1;
        MpoNfts(MpoBox).mint(msg.sender, cardId);
    ...
    function calculateUserReward(address user_) public view returns (uint re) {
        ...
        (tokenIdss, cardIdss) = MpoNfts(MpoNft).tokenOfOwnerForAll(user_);
        if (cardIdss.length > 0) {
            for (uint i; i < cardIds.length; i++) {
                if (cardIdss[i] == cardIds[0]) {
                    re += calculateCardReward(cardIds[0], tokenIdss[i]);
                }

                if (cardIdss[i] == cardIds[1]) {
                    re += calculateCardReward(cardIds[1], tokenIdss[i]);
                }

                if (cardIdss[i] == cardIds[2]) {
    ...
    function _updataUserReward(address user_) internal returns (uint re) {
        ...
        for (uint i; i < cardIds.length; i++) {
            ...
            } else if (cardIdss[i] == cardIds[1]) {
                re += _updataClaimed(cardIds[1], tokenIdss[i]);
            } else if (cardIdss[i] == cardIds[2]) {
    ...
    function _updataDebt(uint cardId_, uint amount_) internal returns (uint) {
        ...
        singleDebt[cardId_] += amount_ / current;
        return amount_ / current;
    }
    ...
    function distribute(uint amount_) public {
        require(amount_ > 0, "0");
...
contract MpoNfts is Ownable, ERC721Enumerable, ERC721URIStorage {
