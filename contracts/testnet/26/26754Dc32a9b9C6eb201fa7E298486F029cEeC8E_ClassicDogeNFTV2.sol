    ...
    ERC721Burnable
{
    ...
    uint256 public mintPrice = 150000000000000000000;
    uint256 public whitelistMintPrice = 100000000000000000000;
    ...
    function upgradeBaseURI(string memory _baseURI) public onlyOwner {
        baseURI = _baseURI;
    }
    ...
    function addBatchToWhitelisted(address[] calldata _addresses)
        public
        onlyOwner
    {
    ...
    function addToWhitelisted(address _address) public onlyOwner {
        require(
            ...
            "NFT: Address is already whitelisted!"
        );
    ...
    function removeFromWhitelisted(address _address) public onlyOwner {
        require(
            whitelistedAddress[_address],
            "NFT: Address is not whitelisted!"
        );
    ...
    function whitelistMintNFT(uint256 amount) public payable {
        ...
        require(
            ...
            "NFT: Max mint amount per session exceeded"
        );
        require(
            balanceOf(msg.sender) < maxMintAmount,
            "NFT: Whitelisted address can mint maximum 5 NFTs"
        );
        require(
            msg.value >= whitelistMintPrice * amount,
            "NFT: Insufficient mint fees!"
        );
        for (uint256 i = 0; i < amount; i++) {
            internalMint(msg.sender);
        }
    ...
    // ------------------------------------------------------------------------
    function withdrawTokens(address tokenAddress, uint256 tokenAmount)
        public
        virtual
        onlyOwner
    {
