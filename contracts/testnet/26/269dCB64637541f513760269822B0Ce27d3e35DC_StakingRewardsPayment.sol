    ...
    ReentrancyGuardUpgradeable
{
    ...
    */
    ...
    function earnedByNonce(
        ...
        if (stakeTime[address(stakingPool)][account][nonce] == 0)
            curStakeTime = stakeInfo.stakeTime;
        else curStakeTime = stakeTime[address(stakingPool)][account][nonce];
        return
            (stakeInfo.rewardsTokenAmount *
                (curCheckTime - curStakeTime) *
                stakeInfo.rewardRate) / (100 * stakingPool.rewardDuration());
    }
    ...
    {
        for (uint256 i = 0; i < stakingPool.stakeNonces(account); i++) {
        ...
        onlyAllowedStakings(stakingPool)
    {
        ...
        require(nonce < stakingPool.stakeNonces(user), "Invalid nonce");
        uint256 reward = earnedByNonce(user, nonce, stakingPool);

        if (reward > 0) {
            ILockStakingRewardFixedAPY.StakeNonceInfo
                memory stakeInfo = stakingPool.stakeNonceInfos(user, nonce);
            if (block.timestamp > stakeInfo.unlockTime)
                stakeTime[address(stakingPool)][user][nonce] = stakeInfo
                    .unlockTime;
            else stakeTime[address(stakingPool)][user][nonce] = block.timestamp;
            (stakingPool.rewardsToken()).safeTransfer(user, reward);
        }
    }
    ...
    function getReward(ILockStakingRewardFixedAPY stakingPool)
        ...
        onlyAllowedStakings(stakingPool)
    {
        uint256 reward = earned(msg.sender, stakingPool);

        if (reward > 0) {
            _getReward(msg.sender, stakingPool, reward);
        }
    }
    ...
    function getRewardForUser(
        ...
        uint256 reward = earned(user, stakingPool);
        if (reward > 0) {
            _getReward(user, stakingPool, reward);
        }
    }
    ...
    function _getReward(
        ...
        for (uint256 i = 0; i < stakingPool.stakeNonces(user); i++) {
            ILockStakingRewardFixedAPY.StakeNonceInfo
                memory stakeInfo = stakingPool.stakeNonceInfos(user, i);
            if (block.timestamp > stakeInfo.unlockTime)
                stakeTime[address(stakingPool)][user][i] = stakeInfo
                    .unlockTime;
            else stakeTime[address(stakingPool)][user][i] = block.timestamp;
        }
        (stakingPool.rewardsToken()).safeTransfer(user, reward);
    }
