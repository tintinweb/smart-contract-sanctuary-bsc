...
contract Presale is Ownable {
    ...
    function withdrawProjectExpenses() public onlyOwner {
        require(projectExpensesWithdrawn < 10 * 1e18, "Project Expenses have been withdrawn");
        uint256 balance = BUSD.balanceOf((address(this)));

        if(projectExpensesWithdrawn + balance < 10 * 1e18){
            BUSD.transfer(projectExpensesAddress, balance);
            projectExpensesWithdrawn += balance;
        } else {
            BUSD.transfer(projectExpensesAddress, 10 * 1e18 - projectExpensesWithdrawn);
            projectExpensesWithdrawn = 10 * 1e18;
        }
    }
