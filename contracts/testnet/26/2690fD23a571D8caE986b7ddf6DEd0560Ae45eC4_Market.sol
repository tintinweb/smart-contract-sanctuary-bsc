...
contract Market is OwnableContract,ReentrancyGuardUpgradeable,IMarket{
    ...
    function createLendOrder(
        ...
        require(paymentToken==address(0) || IERC165(paymentToken).supportsInterface(type(IERC20).interfaceId),"paymentToken invalid");
        if(supportERC20){
            if(paymentToken != address(0)){
                require(IComplexDoNFT(nftAddress).isSupportToken(paymentToken),"doNFT not support ERC20");
            }
        }else{
            require(paymentToken==address(0),"not support ERC20 now");
        }
