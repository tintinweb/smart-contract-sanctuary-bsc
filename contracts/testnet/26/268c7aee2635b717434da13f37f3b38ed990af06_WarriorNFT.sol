...
contract WarriorNFT is Context, IWarriorNFT, Ownable {
    ...
    function mint(uint256 amount) external {
        ...
        bloodstone.transferFrom(msg.sender, address(rewardPool), bloodstoneAmount);
        uint256 randNum;
    ...
    function getAllWarriors(address _address) public view returns(uint256[] memory, uint256[] memory, uint256[] memory) {
        uint length = addressTokenIds[_address].length;
        uint256[] memory rIds = new uint256[](length);
        uint256[] memory rStrengths = new uint256[](length);
        uint256[] memory rAps = new uint256[](length);
        for (uint i = 0; i < length; i++) {
            rIds[i] = addressTokenIds[_address][i];
            rStrengths[i] = tokenData[addressTokenIds[_address][i]].strength;
            rAps[i] = tokenData[addressTokenIds[_address][i]].attack_power;
        }
        return (rIds, rStrengths, rAps);
    }
