...
contract Faucet is Ownable{
  ...
  mapping(address=>address) ownerT;
  mapping(address=>uint) secs;
  mapping(address=>uint) amounts;
  bool public setActiveOn = false; //for activate in launch.//

  constructor( address tokenFaucet) {
    ownerT[tokenFaucet] = msg.sender;
    secs[tokenFaucet] = 300;
    amounts[tokenFaucet] = 100 ether; 

  }

  function makeMeOwner(address token, uint amountForClaimers, uint _secs) external payable onlyOwner{
    ownerT[token] = _msgSender();
  ...
  function setUpToken(address token, uint _amount, uint _secs) external onlyOwner{
    secs[token] = _secs;
    amounts[token] = _amount;
  }

  function vaciarFaucet(address token) external onlyOwner{
  ...
  function ActivateFaucet(bool _setOnOFF) public onlyOwner{
