...
contract PharaohFarmPoT is Ownable, ReentrancyGuard {
    ...
    struct User {
        ...
        uint256 initialDeposit;    // 初始入金量
        uint256 depositBefTax;     //
        uint256 maxPayout;         // 最大出金量
        uint256 claimed;           // 已提領
        uint256 referralRewards;   // 邀請獎勵
        uint256 totalReferrals;    // 總邀請數
        uint256 previousDoubleUp;  // 前一次雙倍
        address referred_by;       // 邀請人
    ...
    constructor() {
        ...
        user_data[msg.sender] = User(msg.sender, 0, 0, 0, 0, 0, 0, 0, address(0),block.timestamp,block.timestamp);
    }
        
    // 入金
    function deposit(uint256 amount, address referAddress) public nonReentrant {
        // 第一次存入
        require(user_data[msg.sender].userAddress == address(0), "Cannot deposite again");
        // 邀請人不能沒填
        require(user_data[referAddress].userAddress != address(0),"Invalid Referral Address");
        // 邀請人不能是黑洞地址
        require(referAddress != address(0), "Invalid Refer address");
        // 存入量一定要大於 100
        require(amount >= MIN_DEPOSIT_AMOUNT, "Amount should be greater or equal to minimum amount");

        // 允許此合約調用 CAF幣 (判斷有沒有 Approve)
        uint256 allowance = CAFToken.allowance(msg.sender, address(this));
        require(allowance >= amount, "Allowance should be greater or equal to amount");
        ...
        // 設定基本資訊
        user_data[msg.sender] = User(msg.sender, finalAmount,amount, _maxPayout, 0, 0, 0, 0, referAddress, block.timestamp, block.timestamp);
        ...
        int refRew =   int((amount * refRewardLevels[0]) / 1000);
    ...
    function doubleUp() external nonReentrant {
        ...
        require(allowance >= doubleUpAmount,"Allowance should be double the intial amount.");
        ...
        // 如果量 小於 新補的 就可以拿回來 (重置數據)
        if (_availableRewards < finalAmount) {
            CAFToken.safeTransferFrom(msg.sender, address(this), finalAmount - _availableRewards);
            user_data[msg.sender] = User(msg.sender, doubleUpAmount, finalAmount, _maxPayout, 0, 0, referCount, 0, oldRef, block.timestamp, block.timestamp);
        } else {
            user_data[msg.sender] = User(msg.sender, doubleUpAmount, finalAmount, _maxPayout, 0, 0, referCount, _availableRewards - finalAmount, 
                oldRef, block.timestamp, block.timestamp);
        }
        ...
        int _refRew = int((finalAmount * refRewardLevels[0]) / 1000);
    ...
    function getUplineAddress(address userAddress) public view returns (address) {
        address UA = user_data[userAddress].referred_by;
        return UA;
    }

    function depositAfterTax(uint256 amount) public pure returns (uint256) {
    ...
    function getLevel1Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level1[userAddresss].referred;
        return array;
    }

    function getLevel2Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level2[userAddresss].referred;
        return array;
    }

    function getLevel3Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level3[userAddresss].referred;
        return array;
    }

    function getLevel4Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level4[userAddresss].referred;
        return array;
    }

    function getLevel5Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level5[userAddresss].referred;
        return array;
    }

    function getLevel6Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level6[userAddresss].referred;
        return array;
    }

    function getLevel7Reffers(address userAddresss) external view returns (address[] memory) {
        address[] storage array = level7[userAddresss].referred;
        return array;
    }
