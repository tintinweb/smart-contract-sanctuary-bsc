...
contract surveys is Ownable {
    ...
    function getSurveyFee(uint256 fee) public view returns (uint256) {
        (, int256 price, , , ) = priceFeed.latestRoundData();
        uint256 adjustedPrice = uint256(price) * 10**10;
        uint256 convertedFee = fee * (10**18);
        uint256 costToEnterSurvey = (convertedFee * 10**18) / adjustedPrice;
        return costToEnterSurvey;
    }
    ...
    function enrollForSurvey(
        ...
        uint256 planAmount = getSurveyFee(listOfPlans[_planID].minAmount);
