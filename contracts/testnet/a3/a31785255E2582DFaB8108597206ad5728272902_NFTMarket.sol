...
contract NFTMarket is INFTMarket, Ownable, Pausable, IERC721Receiver {
    ...
    // Constructor
    // ############
    constructor(IERC20 _exchangeToken, address _taxRecipient) {
    ...
    // Views
    // ############
    function getSellerOfNFTID(uint256 _tokenId) public view override returns (address) {
    ...
    function getListingIDs() public view override returns (uint256[] memory) {
    ...
    function getListingIDsPage(uint8 _limit, uint256 _pageNumber) public view override returns (uint256[] memory) {
        EnumerableSet.UintSet storage set = listedTokenIDs;
        uint256 itemAmount = set.length();
        uint256 pageEnd = _limit * (_pageNumber + 1);
        uint256 tokensSize = itemAmount >= pageEnd ? _limit : itemAmount - (_limit * _pageNumber);
    ...
    function getNumberOfListingsBySeller(address _seller) public view override returns (uint256) {
        EnumerableSet.UintSet storage listedTokens = listedTokenIDs;

        uint256 amount = 0;
        for (uint256 i = 0; i < listedTokens.length(); i++) {
            if (listings[listedTokens.at(i)].seller == _seller) amount++;
        }

        return amount;
    }

    function getListingIDsBySeller(address _seller) public view override returns (uint256[] memory tokens) {
    ...
    function getNumberOfListings() public view override returns (uint256) {
        return listedTokenIDs.length();
    }

    function getPrice(uint256 _id) public view override returns (uint256) {
    ...
    function addListing(uint256 _id, uint256 _price) public override isNotListed(_id) whenNotPaused {
    ...
    function cancelListing(uint256 _id) public override isListed(_id) isSellerOrAdmin(_id) {
    ...
    function purchaseListing(uint256 _id) public override isListed(_id) whenNotPaused {
        ...
        emit PurchasedListing(msg.sender, listing.seller, _id, finalPrice);
    }
    ...
    function executePurchaseLogic(uint256 _id) private returns (uint256, Listing memory) {
        ...
        exchangeToken.safeTransferFrom(msg.sender, listing.seller, finalPrice);

        return (finalPrice, listing);
    }

    function setTaxRecipient(address _taxRecipient) public override onlyOwner {
        taxRecipient = _taxRecipient;
    }

    function setExchangeToken(address _exchangeToken) public onlyOwner {
        taxRecipient = _exchangeToken;
    }

    function setTax(uint256 _tax) public override onlyOwner {
        require(_tax < RATE_DENOMINATOR, "tax is too large");
        tax = _tax;
    }

    function payTax(uint256 amount) internal {
        exchangeToken.safeTransferFrom(msg.sender, taxRecipient, amount);
    }
