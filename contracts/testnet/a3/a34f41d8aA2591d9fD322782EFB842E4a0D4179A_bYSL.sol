 ...
 */
contract bYSL is ERC20, Pausable, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE"); // byte for minter role
    address public constant APE_SWAP_ROUTER_ADDRESS =
        0xaC00460afef1D85C86964E80CDD4007B288CEaFD; // ape swap router address.
    uint256 public average; // average price of the bysl token.
    uint256 public ProtocolPrice; // Protocol price of bysl token.
    uint256 public dailyAvgIncrease = 1; //DailyAverageIncrease is 1%.
    uint256 public PriceDenominator = 10000;// Denominator for price calculation.
    uint256 public durationForIncrement = 24 hours;//only 1 time transaction can be held in every 24 hrs(i.e.either sell or transfer).
    uint256 public lockTransactionTime = 10 seconds;//Transaction Lock time will be 10 seconds only.
    ...
    constructor(
        ...
        ProtocolPrice =
            ((IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) + IERC20(Admin.BUSD()).balanceOf(Admin.POL())) *
                100) /
            10000;
...
/**
    @dev Protocol Price increases in every 24 hrs.
     */
    function increaserPriceProtocol() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            block.timestamp >= durationForIncrement + startTime,
            "bYSL: Can increase price only after 24 hour cycle"
        );
        ProtocolPrice = (ProtocolPrice + ((average * dailyAvgIncrease) / 100));
        startTime = block.timestamp;
        average = 0;
    }
/**
...
/**
    @dev setting Daily Average Increase
     */
    function setDailyAvgIncrease(uint256 _dailyAvgIncrease)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        dailyAvgIncrease = _dailyAvgIncrease;
    }
/**
    @dev Setting Duration for Increment
     */
    function setDurationForIncrement(uint256 _durationForIncrement)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        durationForIncrement = _durationForIncrement;
    }
    ...
    function _transfer(
        ...
        require(
            ...
            "bYSL: you cann't interact in same block"
        );
        if (isContract(sender) || isContract(recipient)) {
            require(
                ...
                "bYSL: No external contract interact with bYSL"
            );
        }
        require(
            restrictTransfer[sender] != block.number,
        ...
        require(transactionTimeLimit[sender] + lockTransactionTime <= block.timestamp,
            "YSL: transactionTimeLimit is greater than current time");
            
        average =
            (((
                (IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) + IERC20(Admin.BUSD()).balanceOf(Admin.POL()))
            ) / PriceDenominator) + average) /
            2;
...
//    function purchaseBYSLWithAmountOut(uint256 BYSL_AMOUNT) external {
//         require(
//             BYSL_AMOUNT > 0,
//             "BYSL: PurchaseBYSLWithAmountOut: VALUE CANNOT BE ZERO"
//         );
//         IERC20(BUSD).transferFrom(msg.sender, POL, getValueBUSD(BYSL_AMOUNT));
//         IBYSL(BYSL).mint(msg.sender, BYSL_AMOUNT);
//         // IUSDY(USDY).mint(temporaryHolding_contract, getValueBUSD(BYSL_AMOUNT));
//     }
//     function purchaseBYSLWithAmountIn(uint256 BUSD_AMOUNT) external {
//         require(
//             BUSD_AMOUNT > 0,
//             "SWAP: PurchaseBYSLWithAmountIn: VALUE CANNOT BE ZERO"
//         );
//         IERC20(BUSD).transferFrom(msg.sender, POL, BUSD_AMOUNT);
//         IBYSL(BYSL).mint(msg.sender, getValueBYSL(BUSD_AMOUNT));
//         // IUSDY(USDY).mint(temporaryHolding_contract, BUSD_AMOUNT);
//     }
//     function getValueBUSD(uint256 bysl) public view returns (uint256) {
//         return
//             bysl *
//             ((IERC20(BUSD).balanceOf(treasury) + IERC20(BUSD).balanceOf(POL)) /
//                 40000);
//     }

//     /**
//     @dev get amountOut for given BUSD

//     @param busd amount of busd for user want to buy bYSL
//      */
//     function getValueBYSL(uint256 busd) public view returns (uint256) {
//         return ((IERC20(BUSD).balanceOf(treasury) +
//             IERC20(BUSD).balanceOf(POL)) / (busd * 40000));
//     }
