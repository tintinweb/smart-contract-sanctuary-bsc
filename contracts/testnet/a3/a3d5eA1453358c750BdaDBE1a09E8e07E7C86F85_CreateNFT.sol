...
contract CreateNFT is ERC721URIStorage, Ownable {
    ...
    createdNfts[] private CreateNFTs;
        ...
        returns (uint256)
    {
        ...
        createdNfts memory newNft = createdNfts(msg.sender, tokenURI, tokenId);
        CreateNFTs.push(newNft);
    ...
    function getNftVaultAddress() external view returns (address) {
        return nftVaultAddress;
    }

    function getNFTs() external view returns (createdNfts[] memory) {
        return CreateNFTs;
    }
    ...
    function transferNft(uint256 _tokenId) public returns (string memory) {
        require(ownerOf(_tokenId) == msg.sender, "CreateNFT: not owner");
        require(_tokenId > 0, "invalid TokenId");
        uint256 CreatedNFTLENGTH = CreateNFTs.length;

        for (uint256 i = 0; i < CreatedNFTLENGTH; i++) {
            if (CreateNFTs[i].nftTokenId == _tokenId) {
                transferFrom(msg.sender, nftVaultAddress, _tokenId);
                CreateNFTs[i].owner = nftVaultAddress;
            }
        }
