...
library SafeMath {
 ...
 contract hl is IERC20, Ownable {
   ...
   function swapAndLiquify(uint256 tAmount) private lockTheSwap {
         //计算流动性一半的代币,((数量*回流税) / 总税 )/2 
        uint256 tokensForLP = tAmount.mul(_buyLiquidityFee).div(_totalBuyFee).div(2);
        //将剩下的一半代币换成bnb
        ...
        //计算总数分红,5 - (流动性比例 / 2 )
        uint256 totalBNBFee = _totalBuyFee.sub(_buyLiquidityFee.div(2));
        //计算流动性分红, 总数bnb * 流动性比例 * 总数bnb分红手续费
        uint256 amountBNBLiquidity = amountReceived.mul(_buyLiquidityFee).div(totalBNBFee).div(2);
