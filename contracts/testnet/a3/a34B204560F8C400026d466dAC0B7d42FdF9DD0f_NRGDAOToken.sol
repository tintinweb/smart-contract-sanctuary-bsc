...
contract NRGDAOToken is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    address public rewardTokenReceiver;

    address public busdTokenReceiver;
    ...
    constructor(
        ...
        uint256 minimumTokenBalanceForDividends_
    ) payable ERC20("NRG-DAO", "NRG-DAO") {

        BUSDToken = _BUSDToken;
        rewardToken = _rewardToken;

        uint256 totalSupply_ = 10000000 * 1e18;
        ...
        rewardTokenReceiver = address(new TokenReceiver(address(rewardToken)));

        busdTokenReceiver = address(new TokenReceiver(address(BUSDToken)));
    ...
    function swapTokensForToken(uint256 tokenAmount) private {
        ...
        uint bal = IERC20(BUSDToken).balanceOf(busdTokenReceiver);
        if( bal > 0 ){
            IERC20(BUSDToken).transferFrom(busdTokenReceiver,address(this),bal);
        }
    }
    ...
    function swapTokensForCake(uint256 tokenAmount) private {
        ...
        uint bal = IERC20(rewardToken).balanceOf(rewardTokenReceiver);
        if( bal > 0 ){
            IERC20(rewardToken).transferFrom(rewardTokenReceiver,address(this),bal);
        }
    }

     function addLiquidity(uint256 tokenAmount, uint256 BUSDTokenAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        IERC20(BUSDToken).approve(address(uniswapV2Router), BUSDTokenAmount);
