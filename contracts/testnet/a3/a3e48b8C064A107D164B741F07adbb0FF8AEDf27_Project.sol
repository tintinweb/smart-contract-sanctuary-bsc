...
interface IClaimPool is IERC165Upgradeable {
    ...
    function transferReward(address _collectionAddress, uint256 _totlAmount) external;
...
interface IProject {
    ...
    function checkValidCollectionPercent(uint256 _projectId) external view returns (bool);
...
contract Project is IProject, Validatable {
    ...
    event CreatedProject(uint256 indexed projectId);
    ...
    function createProject(
        ...
        emit CreatedProject(projectInfo.projectId);
    }
    ...
    function removeProject(uint256 _projectId) external validProjectId(_projectId) onlyProjectOwner(_projectId) {
        require(projects[_projectId].status, "Project deleted");
     ...
     */
    function addCollection(uint256 _projectId, CollectionInfo memory _collection)
        external
     ...
     */
    function removeCollection(uint256 _projectId, address _collectionAddress)
        external
        validProjectId(_projectId)
        onlyProjectOwner(_projectId)
    {
        ...
        notZero(_amount)
    {
        ...
        CollectionInfo memory collectionInfo = collectionInfos[_collection][projectInfo.projectId];
        uint256 newReward = (_amount * collectionInfo.rewardPercent) / DENOMINATOR;
        IClaimPool(projectInfo.claimPool).updateBudget(_collection, rewardCollection + newReward);
    ...
    /**
     * @notice Check valid collection percent
     * @dev    Everyone can call this function
     * @param _projectId id of project
     */
    function checkValidCollectionPercent(uint256 _projectId) external view returns (bool) {
        if (!projects[_projectId].status) return false;
        uint256 _totalPercent = 0;
        for (uint256 i = 0; i < collectionAddress[_projectId].length(); i++) {
            _totalPercent += collectionInfos[collectionAddress[_projectId].at(i)][_projectId].rewardPercent;
        }
        return _totalPercent == DENOMINATOR;
    }
