...
function getBatteryPublic(address _owner, uint256 _id) public view returns (
  ...
  // function encode(Battery memory battery)  external view returns  (uint256) {
    ...
    value |= battery.isDeleted << 164;
    return value;
  }

  function initByOwner(address _batteryMarketPlace, address _boxNFT,address _cyborgLab, CharacterNFT _characterNFT, IBEP20 _nova, Utils _utils) public onlyOwner{
...
function updateBatteryIndexToOwner(uint256 nftId,address owner) public onlyOwner  {
  ...
  function wakeHero(uint256 _batteryNft1, uint256 _batteryNft2, uint256 _feeWakeHeroNova) public whenNotPaused {
        require(ownerOf(_batteryNft1) == msg.sender, "Not NFT owner");
        require(ownerOf(_batteryNft2) == msg.sender, "Not NFT owner");
        (, uint256 batteryTypeNft1, uint256 chargeCompletedAtNft1,)= getBatteryPublic(msg.sender,_batteryNft1);
        (, uint256 batteryTypeNft2, uint256 chargeCompletedAtNft2,)= getBatteryPublic(msg.sender,_batteryNft2);
        ...
        burn(_batteryNft1);
        burn(_batteryNft2);
        updateBattery(msg.sender,_batteryNft1,_batteryNft1,0,0,1);
        updateBattery(msg.sender,_batteryNft2,_batteryNft2,0,0,1);
  ...
  function setNova(IBEP20 _nova) public onlyOwner{
    nova=_nova;
  } 
