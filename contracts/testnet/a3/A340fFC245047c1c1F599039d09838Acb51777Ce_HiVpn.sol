...
contract HiVpn is IHiVpn, Secure {
  ...
  function findPlan(uint256 value) public view override returns (uint8) {
    ...
    for (uint8 i = 1; i < PLAN_PRICE.length; i++) {
      if (lessValue >= PLAN_PRICE[i] && moreValue <= PLAN_PRICE[i]) {
        return i;
      }
    }
    return 0;
  ...
  function Pay(uint pay_id, uint256 plan, address referrer) public payable override {
    ...
    paymentList.push(pay_id);

    emit NewPayment(_msgSender(), referrer, pay_id, value);
  }
  ...
  // Modify User
  function changeUserBuy(address user, uint256 index, uint256 pay_id) external onlyOwner {
  ...
  function paymentDetails(
    ...
    Payment memory details = payment[pay_id];
...
abstract contract Secure {
  ...
  uint256[6] public PLAN_ID = [0, 51, 5, 31, 32, 47];
  ...
  function getPlanIds() external view returns (uint256[6] memory) {
    return PLAN_ID;
  }

  function getPlanPrices() external view returns (uint256[6] memory) {
    return PLAN_PRICE;
  }
