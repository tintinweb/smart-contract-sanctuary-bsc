...
contract SeedSlippage is Modifier, Util {
    ...
    function sellSlippage(uint256 amountToWei) public onlyApprove returns (address [] memory slippageAddresses, uint256 [] memory slippageAmounts) {
        ...
        uint256 overplusAmount = usdtToSeedPrice.mul(amountToWei.sub(slippageAmount)).div(10 ** 18);
        if(protectAmount >= overplusAmount) {
            slippageAddresses[5] = address(this);
            slippageAmounts[5] = amountToWei.sub(slippageAmount);
            protectToken.transfer(msg.sender, overplusAmount);
        }
