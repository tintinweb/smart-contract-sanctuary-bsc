...
contract OLBTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    address public olb721;
    address public olb1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function _saleToken(
        ...
        address create;
        if (ids[3] == 721) {
            (create, ldatas[0]) = OlbCollective721(olb721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, ldatas[0]) = OlbCollective1155(olb1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        if (
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], olb721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            OlbCollective1155(olb1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], olb1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _olb721, address _olb1155)
        public
        onlyOwner
    {
        olb721 = _olb721;
