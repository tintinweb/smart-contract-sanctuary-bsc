...
contract CrossLPFarm is Ownable, ReentrancyGuard, ICRSSEmission, ICrossFarm {
...
interface ICrossVesting {
    ...
    function userPoolVestInfo(address _user, uint256 _pid)
        ...
        view
        returns (
            UserVesting memory vestingTokens,
            UserWithdrawn memory earnedTokens,
...
contract CrosswiseVesting is ReentrancyGuard {
        ...
        )
    {
        ...
        earnedTokens = getUserEarned(_user, _pid);
        (
            vestedCrss,
            vestedLpTokens,
            nextUnlock
        ) = getUserPendingAndNextUnlocked(_user, _pid);
    ...
    function userFarmPoolInfo(address _user, uint256 _pid)
        ...
        returns (
            ...
            bool canWithdrawLocked, //this determines if enough time passed to withdraw deposited LP tokens in a locked pool
            bool compounding, // same as isUserCompounding()
            UserVesting memory vestingTokens, //this returns total amount of vesting crss and lpTokens in a {uint,uint} format
            UserWithdrawn memory earnedTokens, //this returns total amount of withdrawn crss and lpTokens in a {uint,uint} format
        ...
        nonReentrant
    {
        ...
        userVesting[msg.sender][_pid].crssVesting -= uint128(adjustedReward);
        userWithdrawn[msg.sender][_pid].crssWithdrawn += uint128(
            adjustedReward
        );
        address referrer = ICrssReferral(referralContract).getReferrer(
