...
contract RoyaltyFeeManager is IRoyaltyFeeManager, Ownable {
    ...
    function calculateRoyaltyFeeAndGetRecipient(
        ...
        (address[] memory receiver, uint256[] memory royaltyAmount) = royaltyFeeRegistry.royaltyInfo(collection, amount);
        ...
        if (receiver.length == 0) {
            if (IERC165(collection).supportsInterface(INTERFACE_ID_ERC2981)) {
                ...
                (receiver[0], royaltyAmount[0]) = IERC2981(collection).royaltyInfo(tokenId, amount);
            }
        }
