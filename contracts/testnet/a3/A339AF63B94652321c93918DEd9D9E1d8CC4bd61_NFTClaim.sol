...
contract NFTClaim is Ownable, ReentrancyGuard {
    ...
    event AddNftBalance(address _token, address _user, uint256 _num, uint256 _orderId, uint256 _at);
    event SetNftBalance(address _token, address _user, uint256 _num, uint256 _at);
    event AddNftBalanceMulti(address[] _token, address[] _user, uint256[] _num, uint256[] _orderId, uint256 _at);
    ...
    function addNftBalance(address _token, address _user, uint256 _num, uint256 _orderId) external nonReentrant onlyAdmin {
        ...
        emit AddNftBalance(_token, _user, _num, _orderId, block.timestamp);
    }
    ...
    function addNftBalanceMulti(address[] calldata _tokenArr, address[] calldata _userArr, uint256[] calldata _numArr, uint256[] calldata _orderIdArr) external nonReentrant onlyAdmin {
        ...
        emit AddNftBalanceMulti(_tokenArr, _userArr, _numArr, _orderIdArr, block.timestamp);
    }
