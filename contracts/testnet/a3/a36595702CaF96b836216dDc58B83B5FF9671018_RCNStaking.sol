...
contract RCNStaking is Ownable {
    ...
    address public PENTASNFT;
    address constant RCNT_ADDRESS = 0x2370f1c27da5048038724843C258168f452A7e4a; /* RCNT Token Contract Address */
    
    uint256[] internal StakeableToken;
    ...
    //function to set Pentas contract address
    function setPENTASNFT(address _PENTASNFT) public onlyOwner {
        PENTASNFT = _PENTASNFT;
        return;
    }

    //function to set Pentas NFT to become stakeable,set NFTid on Pentas
    function setStakeableToken(uint256[] calldata tokenId) public onlyOwner {
        delete StakeableToken;
        StakeableToken = tokenId;
    }

    function deleteStakeableToken() public onlyOwner {
        delete StakeableToken;
    }
    

    //view token that staked by addresss
    ...
    //stake your Pentas NFT
    function stakeByIds(uint256[] memory tokenIds) public {

        for (uint256 i = 0; i < tokenIds.length; i++) {
            
            require(isStakeableToken(tokenIds[i]), "Not stakeable");
            require(IERC721(PENTASNFT).ownerOf(tokenIds[i]) == msg.sender && tokenIdToStaker[tokenIds[i]] == nullAddress, "Token must be stakable by you!");
            IERC721(PENTASNFT).transferFrom(msg.sender,address(this), tokenIds[i]);
    ...
    //unstake all of your NFT if you have multiple of it
    function unstakeAll() public {
        ...
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            ...
            IERC721(PENTASNFT).transferFrom(address(this),msg.sender,tokenId);
            ...
            RichCatsToken(RCNT_ADDRESS).transferFrom(owner(),msg.sender,totalRewards);
    }

    //unstake one by one of your NFT
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721(PENTASNFT).transferFrom(address(this),msg.sender,tokenIds[i]);
    ...
    //claim your ERC-20 token by Pentas NFTid
    function claimByTokenId(uint256 tokenId) public {
        require( tokenIdToStaker[tokenId] == msg.sender, "Token is not claimable by you!");

        RichCatsToken(RCNT_ADDRESS).transferFrom(owner(),msg.sender,((block.timestamp - tokenIdToTimeStamp[tokenId]) * EMISSIONS_RATE));

        tokenIdToTimeStamp[tokenId] = block.timestamp;
    }

    //claim your ERC-20 token by all pentas tokens you have stake it
    ...
    //view who stake the NFT
    function getStaker(uint256 tokenId) public view returns (address) {
        return tokenIdToStaker[tokenId];
    }

    //return true if token stakeable,false if token not stakeable
    function isStakeableToken(uint256 _tokenId) public view returns (bool) {
        for (uint i = 0; i < StakeableToken.length; i++) {
      if (StakeableToken[i] == _tokenId) {
          return true;
      }
    }
    return false;
