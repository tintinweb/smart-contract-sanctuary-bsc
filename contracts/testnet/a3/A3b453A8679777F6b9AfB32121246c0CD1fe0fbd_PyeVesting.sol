...
contract PyeVesting is Ownable, ReentrancyGuard {
    ...
    uint256 public startTime; // beginning of 30 day vesting window (block #)
    uint256 public totalAllotments; // sum of every holder's Allotment.total (PYE tokens)
    uint256 public claimableApple;
    uint256 public claimableCherry;
    uint256 accuracyFactor = 1 * 10**18;
    ...
    function getClaimed() public view returns (uint256, uint256, uint256) {
        return 
            (Allotments[msg.sender].claimedPYE,
             Allotments[msg.sender].claimedApple,
             Allotments[msg.sender].claimedCherry);
    }
    ...
    function claimPyeTokens() external notContract() {
    ...
    function checkThenTransfer(uint256 _withdrawablePYE, uint256 _withdrawableApple, uint256 _withdrawableCherry, uint256 _available) internal {
        require(_withdrawablePYE <= _available && _withdrawablePYE > 0 && _withdrawablePYE <= IERC20(PYE).balanceOf(address(this)) , 
    ...
    function calculateWithdrawableAmounts() external view returns (uint256, uint256, uint256) {
    ...
    function getvalues() public returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 original = Allotments[msg.sender].allotedPYE; // initial allotment
        uint256 withdrawn = Allotments[msg.sender].claimedPYE; // amount user has claimed
        uint256 available = original.sub(withdrawn); // amount left that can be claimed
        uint256 tenPercent = (original.mul((1 * 10**18))).div(10 * 10**18); // 10% of user's original allotment;
        ...
        totalAllotments = totalAllotments.add(1);
        return (original, withdrawn, available, tenPercent, weightedAllotment, withdrawableApple, withdrawableCherry);
    }
    ...
    // ******************************************************
    // ******************************************************
    // ------------- TESTING FUNCTIONS -- REMOVE FOR MAINNET!
    // ******************************************************
    // ******************************************************
    // ******************************************************
    // ******************************************************
    ...
    function setTotalAllotments(uint256 _totalAllotments) public {
        totalAllotments = _totalAllotments;
    }
