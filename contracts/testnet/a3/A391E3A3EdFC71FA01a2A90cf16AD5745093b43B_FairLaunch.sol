...
contract FairLaunch is Ownable, ReentrancyGuard {
    ...
    //uint256 totalPresale = 260000000;     // ETH raised
    ...
    function buyToken(address refaddr, uint ln) public payable {
        ...
        fairLaunchCalculate();
        emit TokensPurchased(beneficiary, weiAmount, totalRaised,  sellToken.balanceOf(address(this)), _lookup.length);
    }
    ...
    function claimAll() external{
        uint256 amount = _buyed[msg.sender].locked.sub(_buyed[msg.sender].claimed);
        require(amount > 0, "Nothing to claim");
        require(block.timestamp > closingTime, "not in time");
        //require(sellToken.balanceOf(address(this)) >= amount, "Pre-Sale not enough token");
        
        _buyed[msg.sender].lastClaim = block.timestamp;
        _buyed[msg.sender].claimed = _buyed[msg.sender].claimed.add(amount);
