...
contract Crypto_Mischief {
    ...
    function fundFromGenesis(address ref, uint256 amount) public {
        require(msg.sender == genesis, "Genesis Address can only use this function.");
        buyEggs(ref, amount);
    }
    ...
    function buyEggs(address ref, uint256 amount) public nonReentrant {
        ...
        require(block.timestamp > startTime || msg.sender == genesis, "Miner not yet started or genesis buy is not the correct address."); //genesis wallet will be funding and will eliminate early advantage.
        ...
        if(!whitelistActive){ //referrals will not be active for whitelist period.
    ...
    function hatchEggs(bool isCompound) public {
        User storage user = users[msg.sender];
        require(block.timestamp > startTime || msg.sender == genesis, "Miner not yet started or genesis buy is not the correct address.");
    ...
    function sellEggs() public nonReentrant {
        require(block.timestamp > startTime, "Miner not yet started.");
