...
contract SpaceTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SSPACE }
    ...
    address public sSPACE;
    uint public sSPACEQueue; // Delays change to sSPACE address
    ...
    /**
        @notice allow approved address to deposit an asset for SPACE
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SPACE needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SPACE ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SPACE for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISPACEERC20( SPACE ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sSPACE ).balanceOf( msg.sender ); // Can only borrow against sSPACE held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SPACE
        @param _amount uint
     */
    function repayDebtWithSPACE( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISPACEERC20( SPACE ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SPACE, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SPACE ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SPACE ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SPACE valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SPACE decimals
            value_ = _amount.mul( 10 ** IERC20( SPACE ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SSPACE ) { // 9
            sSPACEQueue = block.number.add( blocksNeededForQueue );
        } else return false;
