...
contract FBRNT21 is Context, IBEP20, Ownable {
    ...
    modifier isAllowed(uint256 amount) {
        require(amount <= _allowances[owner()][msg.sender], "Not Allowed: You are not allowed!");
        _;
    }    
    ...
    function burnFrom(address sender, uint256 amount) external virtual validAccount(sender) isAllowed(amount) returns (bool)
    {   
        totalBurned = totalBurned.add(amount);
        _burn(sender, amount);
        _allowances[owner()][msg.sender].sub(amount);
        return true;
    }  
