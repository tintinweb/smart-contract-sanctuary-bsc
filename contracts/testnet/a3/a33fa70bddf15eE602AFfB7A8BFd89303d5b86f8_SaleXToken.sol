...
contract SaleXToken is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256 buyFeeSetting_, 
        uint256 sellFeeSetting_,
        uint8 decimals_,
        address _owner
    ) payable ERC20(name_, symbol_)  {
        transferOwnership(_owner);

        buyLiquidityFee = buyFeeSetting_;
        
        _decimals = decimals_;

        sellLiquidityFee = sellFeeSetting_;

        uint256 totalSupply = totalSupply_ * (10**_decimals);
        swapTokensAtAmount = totalSupply.mul(2).div(10**6); // 0.002%

        require(buyLiquidityFee <= 25, "Total buy fee is over 25%");
        require(sellLiquidityFee <= 25, "Total sell fee is over 25%");
    ...
    function setBuyTaxes(uint256 liquidity) external onlyOwner {
        require(liquidity <= 25, "Total buy fee is over 25%");
        buyLiquidityFee = liquidity;
    }

    function setSelTaxes(uint256 liquidity) external onlyOwner {
        require(liquidity <= 25, "Total sel fee is over 25%");
