...
contract TestPositionBondCommit is PositionBondCommit {
    // MOCK VARIABLES
    bool _canPurchase;
    uint256 _issuePrice;
    uint256 _bondAmount;
    ...
    ReentrancyGuard
{
    ...
    /**
     * @dev see {IPositionBond-activeCommit}
     */
    function activeCommit(
        ...
        super.activeCommit(_commit, _distribution, _active, _maturity);
    ...
    function commit(uint256 amountCommit)
        external
        onlyCommit
        purchasable
        nonReentrant
    {
        ...
        require(amountCommit > 0, "invalid amount commit");
        require(
        ...
        uint256 amountCommitValidate = BondMath.validateAmountCommit(
            amountCommit,
            amountCommittedMapping[_trader].amountCommitted,
            maxAmountCanCommit
        );
    ...
    function distribute() external onlyDistribution nonReentrant {
        address _trader = msg.sender;
        require(
            !amountCommittedMapping[_trader].alreadyDistributed(),
            "already distributed"
        );
        uint256 bondAmount = amountDistribution(_trader);
    ...
    function amountDistribution(address _trader) public view returns (uint256) {
        uint256 totalAmountCommitted = FaceAsset._balanceOf();

        return
            BondMath.calculateBondAmountDistribution(
                amountCommittedMapping[_trader].amountCommitted,
    ...
    function _canCommit(address _trader)
        internal
        view
        virtual
        returns (uint256)
...
abstract contract FaceAsset {
    ...
    function _calculateFaceValueOut(uint256 bondAmount)
        ...
        returns (uint256)
    {
        return BondMath.calculateFaceValue(bondAmount, _faceValue);
    }
...
abstract contract BondStatus {
    ...
    modifier onlyDistribution() {
        require(_isDistribution(), "only distribution");
        _;
    }
...
abstract contract HoldTokenAndStake {
        ...
        returns (uint256)
    {
        ...
        uint256 lpPosiBnb = _getLpPosiBnb(holder) + _posiBnbVault.lpOf(holder);
        uint256 totalStaked = _getAmountPoolClassic(holder) +
            _getAmountPoolNFT(holder) +
            _getPosiByLpPosiBusd(_lpPosiBusd) +
            _getPosiByLpPosiBnb(lpPosiBnb);

        return _convertAmountPosiToBusd(totalStaked);
    }
...
interface IPositionBondCommit {
    ...
    function commit(uint256 amountCommit) external;
    ...
    function amountDistribution(address _trader)
        external
        view
        returns (uint256);
...
library BondMath {
    ...
    function calculateBondAmountDistribution(
        uint256 amountCommitted,
        uint256 totalAmountCommitted,
        uint256 bondSupply
    ) internal pure returns (uint256 bondAmount) {
        bondAmount = (bondSupply * amountCommitted) / totalAmountCommitted;
        return bondAmount;
    }
}
