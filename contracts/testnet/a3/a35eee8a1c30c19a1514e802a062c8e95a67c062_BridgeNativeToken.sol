...
contract BridgeNativeToken{
  ...
  function sendXanaNativeToken (address _tokenAddress, address _address, uint256 _amount) external onlyBridge whenNotPaused{
    IERC20(_tokenAddress).withdraw( _address, _amount);
  }
  function captureXanaNativeToken (uint256 _amount) external payable whenNotPaused{
    emit _captureXanaNativeToken(msg.sender, _amount);
  }
