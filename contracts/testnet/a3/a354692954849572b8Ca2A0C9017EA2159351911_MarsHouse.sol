...
contract MarsHouse is ERC721("Mars House", "Mars"), IERC721Receiver, ERC721Enumerable, Ownable {
    ...
    uint256 constant public MAX_NFT = 5000;
    uint256 constant public MAX_SUPER_RARE = 500;
    uint256 constant public MAX_VERY_RARE = 1000;
    uint256 constant public MAX_RARE = 1500;
    uint256 constant public MAX_NORMAL = 2000;
    ...
    event unstaked(address owner, uint256 tokenId, uint256 rewards, uint256 fee);
    event claimed(address owner, uint256 amount, uint256 fee);

    constructor() {}
      
    function withdraw(address _to) external onlyOwner {
        require(_to != address(0), "You can't withdraw zero address");
    ...
    function mintNFT(uint256 _numOfTokens, uint256 _mintType) external payable {
    ...
    function getRarityTokenCount(uint256 _rarity) external view returns (uint256) {
    ...
    function setTokenBNBPrice(uint256 _price) external onlyOwner {
        bnb_price = _price;
    }

    function getTokenBNBPrice() external view returns (uint256) {
        return bnb_price;
    }

    function setTokenLHCPrice(uint256 _price) external onlyOwner {
        lhc_price = _price;
    }

    function getTokenLHCPrice() external view returns (uint256) {
        return lhc_price;
    }

    function setBaseURI(string memory _pBaseURI, uint256 _rarity) external onlyOwner {
        if(_rarity == 1) {
            baseURI1 = _pBaseURI;
        } else if (_rarity == 2) {
            baseURI2 = _pBaseURI;
    ...
    function getBaseURIs() external view returns (string[] memory) {
    ...
    function setRarityRewards(uint256 _rarity, uint256 _rewards) external onlyOwner {
        if(_rarity == 1) {
            super_rare_reward = _rewards;
        } else if (_rarity == 2) {
            very_rare_reward = _rewards;
        } else if (_rarity == 3) {
            rare_reward = _rewards;
        } else {
            normal_reward = _rewards;
        }
    }
    /**
    ...
    function getOwnerTokens(address _addr) external view returns (string memory) {
        string memory json;

        json = "[";
        uint256 token_id = 0;
        for (uint256 i = 0; i < ERC721.balanceOf(_addr); i++) {
            token_id = ERC721Enumerable.tokenOfOwnerByIndex(_addr, i);
    ...
    function getStakeTokens(address _addr) external view returns (string memory) {
        string memory json = "[";
        for(uint256 i = 0; i < stakeTokens[_addr].length; i++) {
            json = string(abi.encodePacked(json,
                "{\"tokenId\":\"", string(abi.encodePacked(
                    stakeTokens[_addr][i].tokenId.toString(), ",", 
                    stakeTokens[_addr][i].rarity.toString(), ",", 
                    isStaked[stakeTokens[_addr][i].tokenId].toString())),
                "\",\"createdTime\":\"", string(abi.encodePacked(
                    stakeTokens[_addr][i].claimedBalance.toString(), ",", 
                    stakeTokens[_addr][i].createdTime.toString(), ",", 
                    stakeTokens[_addr][i].lastClaimedTime.toString())),
                "\"}"));
            if(i < stakeTokens[_addr].length - 1) {
                json = string(abi.encodePacked(json, ","));
            }
        }
        json = string(abi.encodePacked(json, "]"));
    ...
    function stake(uint256 _tokenId) external returns (uint256) {
    ...
    function checkUnstake(uint256 tokenId) external view returns (uint256[] memory) {
    ...
    function _check(uint256 _tokenId, uint256 _timestamp, address _addr) private view returns (uint256[] memory) {
        ...
        require(stakeTokens[_addr][_index - 1].owner == _addr, "Token owner does not match");

        uint256 stakedTime = _timestamp - stakeTokens[_addr][_index - 1].createdTime;
        uint256 claimedTime = _timestamp - stakeTokens[_addr][_index - 1].lastClaimedTime;
        uint256 claimedBalance = stakeTokens[_addr][_index - 1].claimedBalance;
        uint256 _rarity = tokenInfo[stakeData[_index - 1].tokenId];

        uint256 unstakeFee = 0;
        uint256 rewardToken = 0;
        if(_rarity == 1) {
            rewardToken = super_rare_reward.mul(claimedTime).div(86400);
        } else if(_rarity == 2) {
            rewardToken = very_rare_reward.mul(claimedTime).div(86400);
        } else if(_rarity == 3) {
            rewardToken = rare_reward.mul(claimedTime).div(86400);
        } else {
            rewardToken = normal_reward.mul(claimedTime).div(86400);
        }
        if(stakedTime <= 2 minutes ) {
            unstakeFee = (claimedBalance + rewardToken).mul(30).div(100);
        } else if (stakedTime <= 3 minutes) {
            unstakeFee = (claimedBalance + rewardToken).mul(10).div(100);
        }
    ...
    function unstake(uint256 _tokenId, uint256 timestamp) external {
        require(isStaked[_tokenId] == 1, "Token has not been staked");
        require(timestamp <= block.timestamp, "Parameter is not correct");

        uint256[] memory check_result = _check(_tokenId, timestamp, msg.sender);
        ...
        require(_index > 0, "Not found the token");
        require(stakeTokens[msg.sender][_index - 1].owner == msg.sender, "Token owner does not match");

        ERC20(coinAddress).transferFrom(adminAddress, msg.sender, check_result[0]);
    
        if(check_result[1] > 0) {
            ERC20(coinAddress).transferFrom(msg.sender, address(this), check_result[1]);
            ERC20(coinAddress).transfer(adminAddress, check_result[1]);
        }
        userRewardBalance[msg.sender] += (check_result[0] - check_result[1]);
        isStaked[_tokenId] = 2;
    ...
    function claim() external returns (uint256[] memory) {
        ...
        for(uint256 i = 0; i < stakeTokens[msg.sender].length; i++) {
            if(isStaked[stakeTokens[msg.sender][i].tokenId] == 1) {
                ...
                if(tokenInfo[stakeTokens[msg.sender][i].tokenId] == 1) {
                    amount = super_rare_reward.mul(claimedTime).div(86400);
                } else if (tokenInfo[stakeTokens[msg.sender][i].tokenId] == 2) {
                    amount = very_rare_reward.mul(claimedTime).div(86400);
                } else if (tokenInfo[stakeTokens[msg.sender][i].tokenId] == 3) {
                    amount = rare_reward.mul(claimedTime).div(86400);
                } else {
                    amount = normal_reward.mul(claimedTime).div(86400);
                }
                fee = claimedTime <= 2 minutes ? fee.add(amount.mul(50).div(100)) : 0;
        ...
        require(rewards > 0, "You didn't earn reward token");
        if(staked_nfts >= 10) {
    ...
    function getClaimedRewardsByUser(address _addr) external view returns (uint256[] memory) {
        ...
        for(uint256 i = 0; i < stakeData.length; i++) {
            if(isStaked[stakeData[i].tokenId] == 1) {
                ...
                if(tokenInfo[stakeData[i].tokenId] == 1) {
                    ...
                    rewards = claimedTime <= 2 minutes ? rewards.add(super_rare_reward * claimedTime * 50 / 8640000 ) : rewards.add(super_rare_reward * claimedTime / 86400);
                } else if (tokenInfo[stakeData[i].tokenId] == 2) {
                    rewardsPerSecond = rewardsPerSecond.add(veryRareRewardPerSecond);
                    rewards = claimedTime <= 2 minutes ? rewards.add(very_rare_reward * claimedTime * 50 / 8640000) : rewards.add(very_rare_reward * claimedTime / 86400);
                } else if (tokenInfo[stakeData[i].tokenId] == 3) {
                    rewardsPerSecond = rewardsPerSecond.add(rareRewardPerSecond);
                    rewards = claimedTime <= 2 minutes ? rewards.add(rare_reward * claimedTime * 50 / 8640000) : rewards.add(rare_reward * claimedTime / 86400);
                } else {
                    rewardsPerSecond = rewardsPerSecond.add(normalRewardPerSecond);
                    rewards = claimedTime <= 2 minutes ? rewards.add(normal_reward * claimedTime * 50 / 8640000) : rewards.add(normal_reward * claimedTime / 86400);
                }
            }
        }
        if(staked_nfts >= 10) {
