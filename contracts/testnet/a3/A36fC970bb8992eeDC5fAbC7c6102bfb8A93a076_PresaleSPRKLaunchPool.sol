...
contract PresaleSPRKLaunchPool is Ownable, ReentrancyGuard {
    ...
    function buyPSPRKWithBNB(uint256 _investAmount) external nonReentrant {
            ...
            address uniswapV2Pair1 = IUniswapV2Factory(uniswapV2Router.factory()).getPair(pSaleToken, uniswapV2Router.WETH());
             if(uniswapV2Pair1==address(0)){
            
            uniswapV2Pair1 = IUniswapV2Factory(uniswapV2Router.factory()).createPair(pSaleToken, uniswapV2Router.WETH());

            } 
            (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast) = IUniswapV2Pair(uniswapV2Pair1).getReserves();
            ...
            payable(address(this)).transfer(_investAmount);
            ERC20(pSaleToken).safeTransfer(msg.sender, pSPRKAmount);
