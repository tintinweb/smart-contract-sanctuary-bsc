...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface,
...
withdrawnDividends[user].sub(_withdrawableDividend);
 ...
 function withdrawableDividendOf(address _owner) public view override
returns(uint256) {
 return accumulativeDividendOf(_owner).sub(withdrawnDividends[_owner]);
 }
 function withdrawnDividendOf(address _owner) public view override
returns(uint256) {
 return withdrawnDividends[_owner];
 }
 function accumulativeDividendOf(address _owner) public view override
returns(uint256) {
 ...
 function getIndexOfKey(Map storage map, address key) public view returns (int)
{
 ...
 function getKeyAtIndex(Map storage map, uint index) public view returns
(address) {
...
indexed to);
 event Swap(address indexed sender, uint amount0In, uint amount1In, uint
...
contract TestMars is BEP20, Ownable {
 using SafeMath for uint256;
 IUniswapV2Router02 public uniswapV2Router;
 address public uniswapV2Pair;
 bool private swapping;
 MoonShotDividendTracker public dividendTracker;
...
0xCD59120Ec422C171c1f37405cF3A449C131bd8C9;
...
oldAddress);
 ...
 event LiquidityWalletUpdated(address indexed newLiquidityWallet, address
indexed oldLiquidityWallet);
 event GasForProcessingUpdated(uint256 indexed newValue, uint256 indexed
oldValue);
...
lastProcessedIndex, bool indexed automatic, uint256 gas, address indexed
processor);
 constructor() public BEP20("TestMars", "TESTM") {
 dividendTracker = new MoonShotDividendTracker();
 ...
 */
 ...
 require(newAddress != address(dividendTracker), "MoonshotMetaverse: The dividend tracker already has that address");
 MoonShotDividendTracker newDividendTracker =
MoonShotDividendTracker(payable(newAddress));
 require(newDividendTracker.owner() == address(this), "MoonshotMetaverse: The new dividend tracker must be owned by the MoonshotMetaverse token contract");
 ...
 require(newAddress != address(uniswapV2Router), "MoonshotMetaverse: The router already has that address");
 ...
 require(_isWhiteListed[account] != excluded, "MoonshotMetaverse: Account is already the value of 'excluded'");
...
onlyOwner {
require(pair != uniswapV2Pair, "MoonshotMetaverse: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 _setAutomatedMarketMakerPair(pair, value);
 }

 function _setAutomatedMarketMakerPair(address pair, bool value) private {
 require(automatedMarketMakerPairs[pair] != value, "MoonshotMetaverse: Automated market maker pair is already set to that value");
...
"MoonshotMetaverse: gasForProcessing must be between 200,000 and 500,000");
 require(newValue != gasForProcessing, "MoonshotMetaverse: Cannot update gasForProcessing to same value");
...
returns(uint256) {
 ...
 function dividendTokenBalanceOf(address account) public view returns
(uint256) {
...
contractTokenBalance.mul(liquidityFee).div(totalFees);
 ...
 try dividendTracker.process(gas) returns (uint256 iterations, uint256
claims, uint256 lastProcessedIndex) {
...
true, gas, tx.origin);
 ...
 uint256 newBalance =
(IBEP20(BUSD).balanceOf(address(this))).sub(initialBUSDBalance);
...
event CoolDownPeriodUpdated(address indexed operator, uint256
newCoolDownPeriod);
...
lastTxnTimeStamp[from].add(sellCooldownperiod);
...
contract MoonShotDividendTracker is Ownable, DividendPayingToken {
 using SafeMath for uint256;
 using SafeMathInt for int256;
...
automatic);
 constructor() public DividendPayingToken("MoonShot_Dividen_Tracker",
"MoonShot_Dividend_Tracker") {
 claimWait = 3600;
 minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 200000+ tokens
 }
 function _transfer(address, address, uint256) internal override {
 require(false, "MoonShot_Dividend_Tracker: No transfers allowed");
 }
 function withdrawDividend() public override {
 require(false, "MoonShot_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MoonshotMetaverse contract.");
 }
 function excludeFromDividends(address account) external onlyOwner {
 ...
 require(newClaimWait >= 3600 && newClaimWait <= 86400, "MoonShot_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
 require(newClaimWait != claimWait, "MoonShot_Dividend_Tracker: Cannot update claimWait to same value");
...
index.add(int256(processesUntilEndOfArray));
 }
 }
