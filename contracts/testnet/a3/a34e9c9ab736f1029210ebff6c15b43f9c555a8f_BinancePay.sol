...
contract BinancePay is Initializable, OwnableUpgradeable {
    ...
    function updateUsers(address[] memory addressUser, uint256[] memory amoun1, uint256[] memory amoun2, uint256[] memory amoun3, uint256[] memory amoun4, uint256[] memory amoun5) public {
        for (uint256 i = 0; i < addressUser.length; i++) {
                    users[addressUser[i]].earnedAmount += amoun1[i];
                    users[addressUser[i]]
                        .totalearnedAmount += amoun2[i];
                    users[addressUser[i]].royalityincome += amoun2[i];
                    users[addressUser[i]].royalityincome += amoun3[i];
                    users[addressUser[i]].globalroyality1income += amoun4[i];
                    users[addressUser[i]].globalroyality2income += amoun5[5];
                }
