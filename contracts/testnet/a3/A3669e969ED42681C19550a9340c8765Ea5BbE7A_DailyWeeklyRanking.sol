...
contract DailyWeeklyRanking is Ownable{
    ...
    mapping(address => uint256) public dailyScore;
    mapping(address => uint256) public lastDayScores;
    mapping(address => uint256) public weeklyScore;
    mapping(address => uint256) public lastWeekScores;
    ...
    modifier onlyFightingContract() {
        require(
            gameAddresses.getFightAddress() == msg.sender,
    ...
    function _updateCountersScores(address _user, uint256 _xpWin) internal returns(bool){
        if(_lastFightTimestamp[_user] < dayBeginingTimestamp ){
            lastDayScores[_user] = dailyScore[_user];
            dailyScore[_user] = 0;
        }
        if(_lastFightTimestamp[_user] < weekBeginingTimestamp ){
            lastWeekScores[_user] = weeklyScore[_user];
            weeklyScore[_user] = 0;
        }
        _lastFightTimestamp[_user] = block.timestamp;
        dailyScore[_user] += _xpWin;
        weeklyScore[_user] += _xpWin;
        return true;
    }

    function updatePlayerRankings(address _user, uint256 _xpWin) external onlyFightingContract returns(bool){
        // update 
    ...
    function _updateDailyRanking(address _user) internal {
        ...
        ChallengeDatas memory p = ChallengeDatas(_user,dailyScore[_user]);

        if(d[9].userScore >= p.userScore){
            return;
        }else{
            // check if in ranking
            bool isOnranking;
    ...
    function _updateWeeklyRanking(address _user) internal {
        ...
        ChallengeDatas memory p = ChallengeDatas(_user,weeklyScore[_user]);

        if(w[9].userScore >= p.userScore){
            return;
        }else{
            // check if in ranking
            bool isOnranking;
