...
contract LayerFacet is Helper {
    ...
    function withdraw(uint256 liquidity, uint32[] calldata crates, uint256[] calldata amounts) external payable nonReentrant {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        (uint256 balance) = checkLiq(liquidity); // calc amount DLP for burn
        dlp().burnFrom(msg.sender, liquidity);
        s.reserveLP = s.reserveLP - balance; // update reserves
    ...
    function updateReserve(uint32[] calldata crates, uint8 slippage) external payable nonReentrant returns (uint256 getLP) {
        ...
        ITopcornProtocol(s.c.topcornProtocol).claimTopcorns(crates); // call TopCorn Protocol (claim LP).
...
contract Helper is ReentrancyGuard {
    ...
    function countLP() internal view returns (uint256 lpBegin) {
        (lpBegin, ) = ITopcornProtocol(s.c.topcornProtocol).lpDeposit(address(this), ITopcornProtocol(s.c.topcornProtocol).season());
    }
    ...
    function checkLiq(uint256 liquidity) internal view returns (uint256 balance) {
