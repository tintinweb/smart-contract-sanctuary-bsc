...
contract SaltsToken is Context, IERC20, Ownable {
    ...
    */
    ...
    uint8 totalTax;
    uint8 totalTaxDecimals;
    bool TaxEnabled;

    function enableTax(uint8 tax, uint8 taxDecimals_) public onlyOwner  {
        require(!TaxEnabled, "Reward feature is already enabled.");
        require(tax > 0, "Tax must be greater than 0");
        // tax decimals + 2 must be less than token decimals. 
        // because tax rate is in percentage
        require(taxDecimals_ + 2  <= decimals, "Tax decimals must be less than token decimals - 2");
        
        TaxEnabled = true;
        setTax(tax, taxDecimals_);
 
    }

    function setTax(uint8 tax, uint8 taxDecimals) internal {
        require(TaxEnabled, "Auto burn feature must be enabled. Try the enableAutoBurn function.");
        totalTax = tax;
    ...
    // TODO : Sync tTotalTax and rTotalTax contract will be done 
    ...
    function getTValues(ValuesOfAmount memory values, bool deductTransferFee, address _user)  private {       
        if (deductTransferFee) {
            ...
            // TODO: IF Burn rate is 4% , we should use 40% here 
    ...
    function _register(address _user, address _referer) internal {
        ...
        isRegistered[_user] = true;
            ...
            // TODO: keep is false for a user who is registering without referal 
            // To keep him out of the refTax loop ??
        

        // line 518
        // TODO: remove this 
        function refTax(address user) internal view returns(uint8) {
           uint8 reftax_;
            if(!hadRefTax[user]) {
                reftax_= 0;
            }
        ...
        /*
            now refTax is updated to values 
            now make a function to distribute tRefAmount
        */
