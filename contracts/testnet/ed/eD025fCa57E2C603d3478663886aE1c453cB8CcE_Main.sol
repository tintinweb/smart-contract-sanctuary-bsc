...
Ownable
{
  ...
  function openBox(uint256 _flag) public payable {
    ...
    _bal = _nft.balanceOf(msg.sender);
    require(_bal > 0, "openBox: balance not good");
    _tokenId = _nft.tokenOfOwnerByIndex(msg.sender, 0);
    _nft.transferFrom(msg.sender, address(this), _tokenId);
    if (p < cfg[1]) {
      //0.1 three
      IERC20(three).transfer(msg.sender, cfg[0]);
    } else if (p >= cfg[1] && p < cfg[3]) {
      //0.5 three
      IERC20(three).transfer(msg.sender, cfg[2]);
    } else if (p >= cfg[3] && p < cfg[5]) {
      //1 three
      IERC20(three).transfer(msg.sender, cfg[4]);
    } else if (p >= cfg[5] && p < cfg[7]) {
      //2 three
      IERC20(three).transfer(msg.sender, cfg[6]);
