...
contract PartnershipFactory is Ownable {
    uint public count;
    mapping(uint => address) public nftPartner;

    event NewPartnershipNFT(uint _id, address _nft);

    function createNFT(string memory _URI, bool _hasWhiteList, address _owner, string memory _name, string memory _symbol) external onlyOwner {
        count++;
        PartnershipNFT _nft = new PartnershipNFT(_URI, _hasWhiteList, _owner, _name, _symbol);
        nftPartner[count] = address(_nft);
        emit NewPartnershipNFT(count, address(_nft));
    }
