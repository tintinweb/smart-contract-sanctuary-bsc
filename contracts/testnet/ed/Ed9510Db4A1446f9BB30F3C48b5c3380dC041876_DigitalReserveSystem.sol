...
contract DigitalReserveSystem is IDRS {
    ...
    function collateralHealthCheck(
        ...
        uint256 requiredAmount = _calCollateral(stableCredit, linkId, stableCredit.totalSupply(), heart.getCollateralRatio(collateralAssetCode)).div(heart.getDenominator());
    ...
    function _calExchangeRate(IStableCredit credit, bytes32 linkId, uint256 stableCreditAmount) private view returns (uint256) {
        ...
        uint256 priceInCollateralPerAssetUnit = heart.getCollateralRatio(credit.collateralAssetCode()).mul(credit.peggedValue()).mul(stableCreditAmount).div(heart.getPriceContract(linkId).get()).div(heart.getDenominator());
...
contract StableCredit is ERC20, ERC20Detailed {
    ...
    public ERC20Detailed(_code, _code, 18) {
