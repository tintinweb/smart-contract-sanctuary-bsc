...
// File: contracts/pool/staking1155/IStakingPool.sol
...
// File: contracts/pool/staking1155/Upgradable.sol
...
contract Upgradable1155 is Ownable {
...
// File: contracts/pool/staking1155/StakingNFT.sol


pragma solidity ^0.8.0;


contract StakingNFT is Upgradable1155 {
    ...
    */
    function stakeNFT1155(string[] memory strs, uint256[] memory tokenIds, uint256[] memory amounts) external poolExist(strs[0]) notBlocked payable{
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            //check staking limit
            require(amounts[i] <= pool.stakingLimit, "Pool staking limit is exceeded");
            ...
            stakeNFT1155ByUser[poolId][tokenIds[i]][msg.sender] = amounts[i];

            totalStakedNFT1155BalancePerUser[msg.sender] += amounts[i];

            // Update user staked balance by token address
            totalStakedNFT1155BalanceByNFT[pool.stakingToken][msg.sender] += amounts[i]; 

            // Update user staked balance by pool
            stakedBalancePerUser[poolId][msg.sender] += amounts[i];
            
            // Update pool staked balance
            pool.stakedBalance += amounts[i];

            // Update staking limit
            pool.stakingLimit -= amounts[i];
            
            // Update total staked balance by token address and tokenId
            totalAmountNFT1155Staked[pool.stakingToken][tokenIds[i]] += amounts[i];

            stakingDatas1155[poolId][msg.sender][tokenIds[i]][strs[i+1]] = data;
            emit StakeNFT(pool.stakingToken, msg.sender, address(this), tokenIds[i], amounts[i], poolId, strs[i+1]);
        }
     ...
     * @param amount: amout
    */
    function unstakeNFT1155(string[] memory strs, uint256 tokenId, uint256 amount) external poolExist(strs[0]) notBlocked payable {
        string memory poolId = strs[0];
        PoolInfo storage pool = poolInfo[poolId];
        ...
        require(amount <= data.amount, "1155Unstake: Unstake amount exceed staked balance");
        ...
        // Update reward
        data.claimableReward = earnedFor1155(msg.sender,tokenId, strs);
        ...
        totalAmountNFT1155Staked[pool.stakingToken][tokenId] -= amount;
        ...
        emit StakeNFT(pool.stakingToken,address(this), msg.sender, tokenId, amount, poolId, strs[1]);
    }
    ...
    */
    ...
    function earnedFor1155(address account,uint256 tokenId, string[] memory strs) 
        public
        view 
        returns(uint256)
    {
        ...
        // Flexible pool
        if (pool.poolType == 0) {
            amount = data.amount * (rewardPerToken(poolId)) / 1e20;
        } else { 
            ...
            amount = (currentTimestamp - data.lastUpdateTime) * data.amount * pool.configs[4];
        }
    ...
    */
    ...
    function setTrustPayAddress(address _address) public onlyController {
        require(_address != address(0), "Address is the zero address");
        _setTrustPayAddress(_address);
    }

    function _setTrustPayAddress(address _address) internal {
        trustPayAddress = _address;
    }
    /**
    ...
    */
    ...
    function initialize() external {
        trustPayAddress = 0x92B9b9FaCE61D2868E6C093B315e37123300b2A5;
        feeRecipientAddress = address(IStakingPool(trustPayAddress).feeRecipientAddress()); // Address receive tax fee
        taxFee = uint256(IStakingPool(trustPayAddress).taxFee()); // Tax fee when stake, restake, unstake, claim
