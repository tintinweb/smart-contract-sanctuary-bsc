...
contract Treasury is ITreasury, ContractGuard, ReentrancyGuard {
    ...
    modifier checkEpoch() {
        ...
        epoch_ = epoch_.add(1);
        epochSupplyContractionLeft = (getDarkgPrice() > darkgPriceCeiling)
            ? 0
            : IERC20(darkg).totalSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    // oracle
    function getDarkgPrice() public view override returns (uint256 darkgPrice) {
    ...
    function getDarkgUpdatedPrice()
        public
        view
        override
        returns (uint256 _darkgPrice)
    {
    ...
    function getBurnableDarkgLeft()
        external
        view
        returns (uint256 _burnableDarkgLeft)
    {
        uint256 _darkgPrice = getDarkgPrice();
        if (_darkgPrice <= darkgPriceOne) {
            uint256 _bondMaxSupply = IERC20(darkg)
                .totalSupply()
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableDarkg = _maxMintableBond
                    .mul(getBondDiscountRate())
                    .div(1e18);
                _burnableDarkgLeft = Math.min(
    ...
    function getBondPremiumRate() public view override returns (uint256 _rate) {
        ...
        if (_darkgPrice > darkgPriceCeiling) {
            if (premiumPercent == 0) {
                ...
                uint256 _premiumAmount = _darkgPrice
                    .sub(darkgPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
    ...
    function getDarkgLockedBalance()
        public
        view
        override
        returns (uint256 _lockedBalance)
    {
        ...
        for (uint256 i = 0; i < _length; i++) {
            _lockedBalance = _lockedBalance.add(
                _darkg.balanceOf(darkgLockedAccounts[i])
            );
        }
    }

    function getDarkgCirculatingSupply()
        public
        view
        override
        returns (uint256)
    ...
    function initialize(
        ...
        boardroomWithdrawFee = 200; // 2% when contraction
        ...
        skygRewardPoolExpansionRate = 0.002121913580246914 ether; // 550000 skyg / (3000 days * 24h * 60min * 60s)
        skygRewardPoolContractionRate = 0.004243827160493828 ether; // 2x
    ...
    function setEpochLength(uint256 _epochLength) external onlyOperator {
        require(
            _epochLength >= 1 hours && _epochLength <= 24 hours,
            "out of range"
        );
    ...
    function setBoardroomWithdrawFee(uint256 _boardroomWithdrawFee)
        external
        onlyOperator
    {
    ...
    function setSkygRewardPoolRates(
        uint256 _skygRewardPoolExpansionRate,
        uint256 _skygRewardPoolContractionRate
    ) external onlyOperator {
        require(
            _skygRewardPoolExpansionRate <= 0.5 ether &&
                _skygRewardPoolExpansionRate <= 0.5 ether,
            "too high"
        );
        require(
            _skygRewardPoolContractionRate >= 0.05 ether &&
                _skygRewardPoolContractionRate >= 0.05 ether,
            "too low"
        );
    ...
    function setDarkgPriceCeiling(uint256 _darkgPriceCeiling)
        external
        onlyOperator
    {
        require(
            _darkgPriceCeiling >= darkgPriceOne &&
                _darkgPriceCeiling <= darkgPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
    ...
    {
        require(
            _allocateSeigniorageSalary <= 10000 ether,
            "Treasury: dont pay too much"
        );
    ...
    function setDarkgLockedAccounts(address[] memory _darkgLockedAccounts)
        external
        onlyOperator
    {
    ...
    function buyBonds(uint256 _darkgAmount, uint256 targetPrice)
        external
        override
        onlyOneBlock
        checkOperator
        nonReentrant
    {
        ...
        require(
            _darkgAmount <= epochSupplyContractionLeft,
        ...
        require(
            newBondSupply <= _darkgSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );
        ...
        nonReentrant
    {
        ...
        require(
            IERC20(darkg).balanceOf(address(this)) >= _darkgAmount,
            "Treasury: treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _darkgAmount)
        );
        ...
        _updateDarkgPrice();
        if (regulationStats != address(0))
            IRegulationStats(regulationStats).addRedeemed(epoch_, _darkgAmount);
        ...
        nonReentrant
    {
        ...
        if (epoch_ < bootstrapEpochs) {
            ...
            if (previousEpochDarkgPrice > darkgPriceCeiling) {
                ...
                uint256 _percentage = previousEpochDarkgPrice.sub(
                    darkgPriceOne
                );
                ...
                if (
                    _skygRewardPool != address(0) &&
                    IRewardPool(_skygRewardPool).getRewardPerSecond() !=
                    skygRewardPoolExpansionRate
                ) {
                    IRewardPool(_skygRewardPool).updateRewardRate(
                        skygRewardPoolExpansionRate
                    );
                }
    ...
    function tokenTransferOwnership(address _token, address _operator)
        external
        onlyOperator
    {
    ...
    function boardroomSetLockUp(uint256 _withdrawLockupEpochs)
        external
        onlyOperator
    {
